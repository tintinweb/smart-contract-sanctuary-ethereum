...
contract Token is IERC20, Ownable, TokenTools {
    ...
    ) payable TokenTools(_serviceAdmin) {
        _name = _name_;
        _symbol = _symbol_;
    ...
    function recoverToken(address tokenAddress, uint256 tokenAmount) public onlyOwner {
        // do not allow recovering self token, only for different tokens
