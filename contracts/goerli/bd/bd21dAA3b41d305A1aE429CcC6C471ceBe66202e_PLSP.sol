...
contract PLSP is ERC20, Ownable {
    ...
    // FIXME: This is for Rinkeby Network, change to Mainnet before launch
    address public wethAddr = 0xc778417E063141139Fce010982780140Aa0cD5Ab;
    address public uniswapV2FactoryAddr = 0x5C69bEe701ef814a2B6a3EDD4B1652CB9cc5aA6f;
    ...
    uint256 public totalTokenHolding;
    uint256 public currentAirdropMinimum;
    bool public isAirdropping;
    mapping(address => BalanceTrack) public balanceTrack;
    ...
    function _beforeTokenTransfer(
        ...
        _trackBalanceChange(from, to, amount);
    }
    ...
    /**
     * @dev tracks the transfer during the airdrop and stores the exact balance
     * of the moment just before the airdrop
     *
     * @param from sender address
     * @param to receiver address
     */
    function _trackBalanceChange(
        address from,
        address to,
        uint256
    ) internal {
    ...
    function _logBalance(address addr) internal {
        if (isUserAddress(addr)) {
            BalanceTrack storage change = balanceTrack[addr];
            if (change.timestamp < lastAirdropTime) {
                change.balance = balanceOf(addr);
    ...
    /**
     * @dev get uniswap pair address between PLSP and PLS
     */
    function calculatePairAddress() public view returns (address) {
        IUniswapV2Factory uniFactory = IUniswapV2Factory(uniswapV2FactoryAddr);
        address realPair = uniFactory.getPair(address(this), wethAddr);
        return realPair;
        // return address(0);
    }
    ...
    /**
     * @dev airdrops PLSP to token holders depending on the amount of holding
    ...
    function performAirdrop() external returns (uint256) {
        ...
        if (!isAirdropping) {
            // Start a new airdrop
            currentAirdropMinimum = potLottery.airdropMinimum();
            totalTokenHolding = _getEligibleTokenAmount(currentAirdropMinimum);
            lastAirdropTime = block.timestamp;
            totalAirdropAmount = potLottery.airdropPool();
    ...
    function _continueAirdrop() internal returns (uint256 airdropped) {
        ...
        for (uint8 count = 0; count < 150 && i < totalAirdropUserCount; i++) {
            ...
            if (balanceTrack[user].timestamp >= lastAirdropTime) {
                balance = balanceTrack[user].balance;
            }

            if (balance < currentAirdropMinimum) {
                continue;
            }

            uint256 amount = (balance * totalAirdropAmount) / totalTokenHolding;
            amount > 0 && transfer(user, amount);
            airdropped += amount;
            count++;
        }
    ...
    /**
     * @dev burns PLSP token accumulated in the burn pool on the PotLottery
     * Contract
     *
     * @return burnt amount
     *
     * NOTE: The caller of this fuction will burn his PLSP tokens, so it is
     * recommended to be called by PotLottery Contract
     */
    ...
    /**
     * @dev gives PLSP token accumulated in the lottery pool to the selected
     * winnner
     *
    ...
    function performLottery() external returns (address) {
        ...
        uint256 lotteryMinimum = potLottery.lotteryMinimum();
        address winner = _determineLotteryWinner(lotteryMinimum);
     ...
     *
     */
    function _determineLotteryWinner(uint256 minimum) internal view returns (address) {
        uint256 randomNumber = getRandomNumber();
        uint256 winnerValue = randomNumber % _getEligibleTokenAmount(minimum);
        address winner;

        for (uint256 i = 0; i < tokenHolders.length; i++) {
            uint256 balance = balanceOf(tokenHolders[i]);
    ...
    /**
     * @dev gets the total balance of eligible users for lottery,
     * airdrop, etc.
     */
    function _getEligibleTokenAmount(uint256 minimum) internal view returns (uint256) {
        uint256 total;
    ...
    /**
     * NOTE: this is just testing function, should be deleted
     */
    function testWinnerPick() external view returns (uint256) {
        uint256 total;
...
interface IPotLottery {
    ...
    function airdropMinimum() external view returns (uint256);

    function lotteryMinimum() external view returns (uint256);

    function fullFillRandomness() external view returns (uint256);

    function getPulsePrice() external view returns (uint256 price);

    function swapAccumulatedFees() external;

    function burnAccumulatedPLSP() external;

    function airdropAccumulatedPLSP() external returns (uint256);
}
