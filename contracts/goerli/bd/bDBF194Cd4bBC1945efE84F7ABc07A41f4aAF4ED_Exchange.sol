...
abstract contract AssetMatcher is Initializable {
    ...
    function _matchAssetOneSide(
        ...
        if (classLeft == LibAsset.ERC721_LAZY_ASSET_CLASS) {
            if (classRight == LibAsset.ERC721_LAZY_ASSET_CLASS) {
                return _simpleMatch(leftAssetType, rightAssetType);
            }
            return LibAsset.AssetType(0, "");
        }
    ...
    PausableUpgradeable
{
    ...
    function _decodeOrderData(bytes memory data)
        internal
        pure
    ...
    function _parse(LibOrder.Order memory order)
        internal
        pure
    ...
    function cancelExtention(LibOrder.Input calldata input)
        external
        override
        nonReentrant
        whenNotPaused
    {
        require(input.maker != address(0), "address is not zero");
        require(_msgSender() == input.maker, "not a maker");
        require(input.salt != 0, "0 salt can't be used");
        LibOrder.Order memory orderNormal = LibOrder.Order(
            input.maker,
            input.makeAssetNormal,
            input.taker,
            input.takeAsset,
            input.salt,
            input.start,
        ...
        LibOrder.Order memory orderExclusive = LibOrder.Order(
            input.maker,
            input.makeAssetExclusive,
            input.taker,
            input.takeAsset,
            input.salt,
            input.start,
        ...
        LibOrder.Order memory orderLimited = LibOrder.Order(
            input.maker,
            input.makeAssetLimited,
            input.taker,
            input.takeAsset,
            input.salt,
            input.start,
        ...
        bytes32 orderKeyHashNormal = LibOrder._hashKey(orderNormal);
        bytes32 orderKeyHashExclusive = LibOrder._hashKey(orderExclusive);
        bytes32 orderKeyHashLimited = LibOrder._hashKey(orderLimited);
        require(fills[orderKeyHashNormal] != UINT256_MAX, "already canceled");
        require(
            fills[orderKeyHashExclusive] != UINT256_MAX,
            "already canceled"
        );
        require(fills[orderKeyHashLimited] != UINT256_MAX, "already canceled");
        fills[orderKeyHashNormal] = UINT256_MAX;
        fills[orderKeyHashExclusive] = UINT256_MAX;
        fills[orderKeyHashLimited] = UINT256_MAX;

        emit Cancel(input.salt, bytes4(keccak256("SELL_ORDER")), "0x");
    }
        ...
        whenNotPaused
    {
        ...
        emit Cancel(order.salt, bytes4(keccak256("OFFER")), "0x");
    }

    /**
        @notice verify signature and match orders 
        @param input the left order of the match
        @param signatureLeft the signature of left order
        @param orderRight the right order of the match
        @param signatureRight the signature of right order
     */
    function matchOrderExtention(
        LibOrder.Input calldata input,
        bytes calldata signatureLeft,
        LibOrder.Order calldata orderRight,
        bytes calldata signatureRight
    ) external payable override nonReentrant whenNotPaused {
        require(input.dataType == LibOrderData.V1, "Invalid match order");

        _validateExtentionFull(input, signatureLeft);
        if (input.taker != address(0)) {
            require(
                orderRight.maker == input.taker,
                "leftOrder.taker verification failed"
            );
        }
        if (orderRight.taker != address(0)) {
            require(
                orderRight.taker == input.maker,
                "rightOrder.taker verification failed"
            );
        }

        require(orderRight.typeMint.length != 0, "typeMint invlaid");

        LibOrder.Order memory orderLeft;

        if (orderRight.typeMint == LibLazyMint.MINT_NORMAL) {
            orderLeft = LibOrder.Order(
                input.maker,
                input.makeAssetNormal,
                input.taker,
                input.takeAsset,
        ...
        } else if (orderRight.typeMint == LibLazyMint.MINT_EXCLUSIVE) {
            orderLeft = LibOrder.Order(
                input.maker,
                input.makeAssetExclusive,
                input.taker,
        ...
        } else if (orderRight.typeMint == LibLazyMint.MINT_LIMITED) {
            orderLeft = LibOrder.Order(
                input.maker,
                input.makeAssetLimited,
                input.taker,
    ...
    function matchOrders(
        ...
        require(orderLeft.dataType == LibOrderData.V1, "Invalid match order");
    ...
    */
    ...
    function _validateExtentionFull(
        LibOrder.Input calldata input,
        bytes calldata signature
    ) internal view {
        LibOrder._verifyTimeExtention(input);
        _verifySignatureExtention(input, signature);
    }
...
interface IExchange {
    ...
    function cancelExtention(LibOrder.Input memory input) external;
    function cancel(LibOrder.Order memory order) external;

    /**
        @notice verify signature and match orders 
        @param order the left order of the match
        @param signatureLeft the signature of left order
        @param orderRight the right order of the match
        @param signatureRight the signature of right order
     */
...
library LibAsset {
    ...
    function _hash(AssetType memory assetType) internal pure returns (bytes32) {
        return
            keccak256(
                abi.encode(
                    ASSET_TYPE_TYPEHASH,
    ...
    function _hash(Asset memory asset) internal pure returns (bytes32) {
        return
            keccak256(
                abi.encode(
                    ASSET_TYPEHASH,
                    _hash(asset.assetType),
                    asset.value,
                    asset.token,
                    asset.tokenId
                )
...
library LibLazyMint {
    bytes4 constant MINT_NORMAL = bytes4(keccak256("NORMAL"));
    bytes4 constant MINT_EXCLUSIVE = bytes4(keccak256("EXCLUSIVE"));
    bytes4 constant MINT_LIMITED = bytes4(keccak256("LIMITED"));
...
library LibMath {
    ...
    function _safeGetPartialAmountFloor(
        ...
        if (_isRoundingErrorFloor(numerator, denominator, target)) {
    ...
    function _safeGetPartialAmountCeil(
        ...
        if (_isRoundingErrorCeil(numerator, denominator, target)) {
            revert("rounding error");
        }
...
library LibOrder {
    ...
    bytes32 constant ORDER_TYPEHASH =
        keccak256(
            "Order(address maker,Asset makeAsset,address taker,Asset takeAsset,uint256 salt,uint256 start,uint256 end,bytes4 dataType,bytes data,bytes4 typeMint)Asset(AssetType assetType,uint256 value,address token,uint256 tokenId)AssetType(bytes4 assetClass,bytes data)"
        );

    bytes32 constant INPUT_TYPEHASH =
        keccak256(
            "Input(address maker,Asset makeAssetExclusive,Asset makeAssetLimited,Asset makeAssetNormal,address taker,Asset takeAsset,uint256 salt,uint256 start,uint256 end,bytes4 dataType,bytes data,bytes4 typeMint)Asset(AssetType assetType,uint256 value,address token,uint256 tokenId)AssetType(bytes4 assetClass,bytes data)"
        );
    ...
    struct Input {
        address maker;
        LibAsset.Asset makeAssetExclusive;
        LibAsset.Asset makeAssetLimited;
        LibAsset.Asset makeAssetNormal;
        address taker;
        LibAsset.Asset takeAsset;
        uint256 salt;
        uint256 start;
    ...
    function _calculateRemaining(
        ...
        if (isMakeFill) {
            ...
            takeValue = LibMath._safeGetPartialAmountFloor(
            ...
            makeValue = LibMath._safeGetPartialAmountFloor(
                order.makeAsset.value,
                order.takeAsset.value,
                takeValue
            );
    ...
    function _hashKey(Order memory order) internal pure returns (bytes32) {
        return
            keccak256(
                abi.encode(
                    order.maker,
                    LibAsset._hash(order.makeAsset.assetType),
                    LibAsset._hash(order.takeAsset.assetType),
                    order.salt,
                    order.data
                )
            );
        ...
        return
            keccak256(
                abi.encode(
                    ORDER_TYPEHASH,
                    order.maker,
                    LibAsset._hash(order.makeAsset),
                    order.taker,
                    LibAsset._hash(order.takeAsset),
                    order.salt,
                    order.start,
                    order.end,
                    order.dataType,
    ...
    function _hash(Input memory input) internal pure returns (bytes32) {
        return
            keccak256(
                abi.encode(
                    INPUT_TYPEHASH,
                    input.maker,
                    LibAsset._hash(input.makeAssetExclusive),
                    LibAsset._hash(input.makeAssetLimited),
                    LibAsset._hash(input.makeAssetNormal),
                    input.taker,
                    LibAsset._hash(input.takeAsset),
                    input.salt,
                    input.start,
                    input.end,
                    input.dataType,
    ...
    function _verifyTimeExtention(LibOrder.Input memory input) internal view {
        require(
            input.start == 0 || input.start < block.timestamp,
            "Order start validation failed"
        );
        require(
            input.end == 0 || input.end > block.timestamp,
            "Order end validation failed"
        );
    }
}
...
library LibPart {
    ...
    function _hash(Part calldata part) internal pure returns (bytes32) {
...
library LibSignature {
    ...
    function _recover(
        ...
        if (v > 30) {
            ...
            signer = ecrecover(_toEthSignedMessageHash(hash), v - 4, r, s);
        } else {
...
library LibTransfer {
    function _transferEth(address to, uint value) internal {
    ...
    EIP712Upgradeable
{
    ...
    function _verifySignatureExtention(LibOrder.Input memory input, bytes calldata signature)
        internal
        view
    {
        if (input.salt != 0) {
             if (_msgSender() != input.maker) {
                bytes32 hash = LibOrder._hash(input);
                address signer = address(0);
                if (signature.length == 65) {
                    signer = _hashTypedDataV4(hash)._recover(signature);
                }
                if (signer != input.maker) {
                    if (input.maker.isContract()) {
                        require(
                            IERC1271(input.maker).isValidSignature(
            ...
            if (input.maker != address(0)) {
                require(_msgSender() == input.maker, "maker is not tx sender");
            } else {
                input.maker = _msgSender();
            }
        }
    }

    function _verifySignature(LibOrder.Order memory order, bytes calldata signature)
        internal
        view
    {
        if (order.salt != 0) {
             if (_msgSender() != order.maker) {
                bytes32 hash = LibOrder._hash(order);
                address signer = address(0);
                if (signature.length == 65) {
                    signer = _hashTypedDataV4(hash)._recover(signature);
                }
...
import "../token/erc721/interfaces/INFT721.sol";

abstract contract TransferExecutor {
    ...
    function _transferNFT(
        ...
        if (asset.assetType.assetClass == LibAsset.ERC721_ASSET_CLASS) {
            ...
            IERC721Upgradeable(asset.token).safeTransferFrom(
                from,
                to,
                asset.tokenId,
                ""
            );
        } else if (asset.assetType.assetClass == LibAsset.ERC721_LAZY_ASSET_CLASS) {
            require(asset.value == 1, "erc721 value error");
            INFT721(asset.token).mint(
                to,
                typeMint
            );
        } else {
...
abstract contract TransferManager is TransferExecutor {
    using SafeMathUpgradeable for uint256;
