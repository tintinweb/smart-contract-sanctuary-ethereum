    ...
    IRebalancer
{
    ...
    /// @notice swap information data structure
    struct SwapInfo {
        // swap adapter
        address swapAdapter;
        // encoded path for a provided swap adapter
        bytes path;
    }
    ...
    /// @notice AccessControl arbitraget role
    bytes32 public constant ARBITRAGER_ROLE = keccak256("ARBITRAGER_ROLE");

    /// @notice swap info for tokens pair
    /// @notice tokenFrom => tokenTo => swapInfo
    mapping(address => mapping(address => SwapInfo)) public swapInfo;

    /// @notice stable token for specific market
    mapping(address => address) public marketStable;

    /// @notice requires msg.sender to have ARBITRAGER_ROLE
    modifier onlyArbitrager() {
        require(
            gac.userHasRole(ARBITRAGER_ROLE, msg.sender),
            "R: not-arbitrager"
        );
        _;
    }

    /// @notice contract initializer in upgradeable pattern
    function initialize(address protocolConfig, address _bst)
        external
        initializer
    {
    ...
    function rebalance(address _market)
        external
        virtual
        override
        onlyArbitrager
        whenNotPaused
    {
        RebalanceAmount memory upd = IRebalanceCalculation(
            IMarket(_market).getRiskCalc()
        ).rebalanceAmount();
        ...
        require(
            upd.deltaAP != 0 ||
                ...
                upd.sellToken != 0 ||
                upd.sellStable != 0,
            "R: rebalance-not-needed"
        );

        _rebalance(upd, _market);
    }
    ...
    function setSwap(
        ...
        swapInfo[_swapFrom][_swapTo] = SwapInfo({
            swapAdapter: _swapAdapter,
            path: _swapPath
        });
    }

    /// @notice sets stable token address for particular market
    /// @param market market address
    /// @param token stable token address
    function setStableForMarket(address market, address token)
        external
        virtual
        onlyGovernance
    {
        marketStable[market] = token;
    }
    ...
    /// @inheritdoc IRebalancer
    function createTakerPayoutOnClose(address _market, uint assetAmount)
        external
        view
        override
        returns (Payout memory)
    {
        // insert liquidity analize logic here
        return Payout(assetAmount, 0);
    }

    /// @inheritdoc IRebalancer
    function createTakerPayoutOnCancel(address _market, uint assetAmount)
        external
        view
        override
        returns (Payout memory)
    ...
    /// @inheritdoc IRebalancer
    function createTakerPayoutOnClaim(address _market, uint stableAmount)
        external
        view
        override
        returns (Payout memory)
    {
        // insert liquidity analize logic here
        return Payout(0, stableAmount);
    }

    /// @inheritdoc IRebalancer
    function createMakerPayoutOnWithdraw(address _market, uint stableAmount)
        external
        view
        override
        returns (Payout memory)
    ...
    /// @inheritdoc IRebalancer
    function createMakerPayoutOnAbandon(address _market, uint stableAmount)
        external
        view
        override
        returns (Payout memory)
    ...
    /// @inheritdoc IRebalancer
    function isRebalanceNeeded(address _market)
        external
        view
        virtual
        override
    ...
    /// @notice returns stable to sell for particular market
    function getSellStable(address market) public view returns (address) {
        return marketStable[market];
    }

    /// @notice returns stable to buy for particular market
    function getBuyStable(address market) public view returns (address) {
        return marketStable[market];
    }

    /// @dev internal rebalance impl for prevent stack is too deep error
    function _rebalance(RebalanceAmount memory upd, address _market) internal {
        console.log("_rebalance", upd.sellToken, upd.sellStable);
        if (upd.sellToken > 0) {
            IERC20 token = IERC20(address(IMarket(_market).ASSET()));

            address stableToken = getBuyStable(_market);
            SwapInfo storage swap = swapInfo[address(token)][stableToken];
            require(swap.swapAdapter != address(0), "R: invalid-swapInfo");

            token.safeTransferFrom(_market, address(this), upd.sellToken);

            _approveToMax(token, swap.swapAdapter, upd.sellToken);

            uint amountOut = ISwap(swap.swapAdapter)
                .swapExactTokensForTokensMult(
                    swap.path,
                    upd.sellToken,
                    0,
                    address(bst)
                );
            uint bstAmount = (amountOut * (10**18)) /
                (10**IERC20Metadata(stableToken).decimals());
            bst.mint(bstAmount, _market);
            upd.deltaCP += int(bstAmount);
        }
        if (upd.sellStable > 0) {
            IERC20 token = IMarket(_market).ASSET();

            address stableToken = getSellStable(_market);
            SwapInfo storage swap = swapInfo[stableToken][address(token)];
            require(swap.swapAdapter != address(0), "Invalid swapInfo");

            bst.withdraw(stableToken, upd.sellStable, _market, address(this));
            _approveToMax(
                IERC20(stableToken),
                swap.swapAdapter,
                upd.sellStable
            ...
            upd.deltaAP += int(
                ISwap(swap.swapAdapter).swapExactTokensForTokensMult(
                    swap.path,
                    upd.sellStable,
                    0,
                    _market
        ...
        address state = IMarket(_market).getState();
        IState(state).update(
            upd.deltaAP,
            upd.deltaAR,
            upd.deltaCP,
