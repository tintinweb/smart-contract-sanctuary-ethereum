...
// File: contracts/RaphTest.sol
pragma solidity ^0.8.0;

contract RaphTest is ERC721A, Ownable {
  using Strings for uint256;
  ...
  uint256 public setMaxRTPerTxn = 10; 
  uint256 public maxRTPerWallet = 20; 
  uint256 public totalPresaleTickets = 6000;
  ...
  mapping(address => uint256) public _totalRTPresale;
  mapping(address => uint256) public _totalRTMainsale;
  
  constructor() ERC721A("RaphTest", "RT", maxSupply, maxSupply) {
    setBaseURI("ipfs://.../");
  ...
  // internal
    ...
    function preSaleMint(uint256 _mintAmount) public payable {
        if (msg.sender != owner()){
            ...
            require(_mintAmount <= maxRTPerWallet, "Max Mint amount per session exceeded");
        }
        require(_mintAmount > 0, "Need to mint at least 1 RT");
        ...
        require(_totalRTPresale[msg.sender] + _mintAmount <= maxRTPerWallet,
        ...
        // minting presale NFT
        _totalRTPresale[msg.sender] = _totalRTPresale[msg.sender] + _mintAmount;
  ...
  function saleMint(uint256 _mintAmount) public payable {
        if (msg.sender != owner()){
            require(!paused, "Public sale is not live, can't mint");
            require(_mintAmount <= setMaxRTPerTxn, "Max mint amount exceeded");
        ...
        require(
            _totalRTMainsale[msg.sender] + _mintAmount <= maxRTPerWallet,
            "exceeded presale total mints per wallet"
            );
        if (msg.sender != owner()) {
            if (_giftFreeMint[msg.sender]==false){
  ...
  function setMaxRTPerWallet(uint256 _limit) public onlyOwner() {
    maxRTPerWallet = _limit;
  }
...
//   function to update the presale NFT amount
  ...
  function setMaxRTPerTransaction(uint256 _newmaxMintAmount) public onlyOwner() {
    setMaxRTPerTxn = _newmaxMintAmount;
  }
