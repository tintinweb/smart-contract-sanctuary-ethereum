...
contract PTest is ERC721AQueryable, Ownable{
    ...
    uint256 public whitelistMintDate = 1669298400; // 2022-11-24T14:00:00Z
    uint256 public publicMintDate = 1669377600; // 2022-11-25T12:00:00Z
    uint256 public maxPublicPrivateMint = 10;
    uint256 public maxWhitePrivateMint = 10;

    address[] private _payees;
    uint256[] private _payeeShares;
    uint256 private _payeeShareTotal;
...
// event PermanentURI(string _value, uint256 indexed _id);
    ...
    ERC721A(name_, symbol_) {
        ...
        _payees = payees_;
        _payeeShares = payeeShares_;
        for (uint256 i = 0; i < _payeeShares.length; i++) {
            _payeeShareTotal = _payeeShareTotal + _payeeShares[i];
        }
    }
    ...
    function mintWhitelist(uint256 amount, bytes32[] calldata _merkleProof) public payable {
        require(block.timestamp >= whitelistMintDate, "mintWhitelist: The whitelist sale is not enabled");
    ...
    function mintPublic(uint256 amount) public payable {
        require(block.timestamp >= publicMintDate, "mintPublic: The public sale is not enabled");
    ...
    function _privatePayout() internal virtual {
        uint256 balance = address(this).balance;

        for (uint256 i = 0; i < _payees.length; i++) {
            (bool os, ) = payable(_payees[i]).call{value: balance * _payeeShares[i] / _payeeShareTotal}('');
            require(os);
        }
    }

    function getPersonalMintCount() public view returns (uint256) {
