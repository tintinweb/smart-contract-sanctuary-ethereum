...
contract CelestialPortalMainland is Ownable {
    ...
    modifier onlyAuth() {
        require(
            auth[msg.sender],
            "CelestialPortalMainland: Unauthorized to use the portal"
        );
        _;
    }
    ...
    function replayCall(
        ...
        if (required_)
            require(succ, "CelestialPortalMainland: Replay call failed");
    ...
    function retrieveAll(
        ...
        (
            ...
            uint256 fbxAmount
                ...
                data,
                (uint256[], Freak[], uint256[], CelestialV2[], uint256)
            );
        castleMainland.retrieveFreaks(msg.sender, freakIds, freaksAttributes);
        castleMainland.retrieveCelestials(
            msg.sender,
            celestialIds,
            celestialAttributes
        );
    ...
    function retrieveFreaks(
        ...
        txsCompleted[txId] = true;
        (uint256[] memory freakIds, Freak[] memory freaksAttributes) = abi
            .decode(data, (uint256[], Freak[]));
        castleMainland.retrieveFreaks(msg.sender, freakIds, freaksAttributes);
    ...
    function retrieveCelestials(
        ...
        (
            uint256[] memory celestialIds,
            CelestialV2[] memory celestialsAttributes
        ) = abi.decode(data, (uint256[], CelestialV2[]));
        castleMainland.retrieveCelestials(
            msg.sender,
            celestialIds,
            celestialsAttributes
        );
        ...
        return
            keccak256(
                abi.encodePacked(
                    ...
                    string(abi.encodePacked(_messageHash))
                )
            );
    }
