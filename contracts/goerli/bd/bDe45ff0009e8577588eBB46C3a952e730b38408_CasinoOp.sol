...
contract CasinoOp {
  ...
  function settleBet(uint _commitment) internal {
    ...
    uint val = 2 * proposedBet[_commitment].value;
    if (randomA ^ randomB % 2 == 0) {
      // A wins
      payable(proposedBet[_commitment].sideA).transfer(val);
      emit BetAssert(_commitment, proposedBet[_commitment].sideA, acceptedBet[_commitment].sideB, proposedBet[_commitment].sideA);
    } else {
      // B wins
      payable(acceptedBet[_commitment].sideB).transfer(val);
      emit BetAssert(_commitment, proposedBet[_commitment].sideA, acceptedBet[_commitment].sideB, acceptedBet[_commitment].sideB);
    }
  }
