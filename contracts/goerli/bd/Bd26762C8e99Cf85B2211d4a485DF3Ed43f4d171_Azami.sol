...
contract Azami is Ownable, ERC721A, ERC2981, ReentrancyGuard {
    ...
    constructor(uint256 maxSupply_, uint256 batchSize_) ERC721A("Azami", "AZAMI") IERC2981() {
    ...
    function whitelistMint(uint256 quantity) external payable callerIsUser {
        ...
        uint256 startTime = uint256(saleConfig.limitedSaleStartTime);
        require(
            price != 0 && block.timestamp >= startTime,
    ...
    function publicSaleMint(uint256 quantity) external payable callerIsUser {
        ...
        require(quantity <= batchSize, "can not mint this many");
        require(totalSupply() + quantity <= maxSupply, "reached max supply");
        _safeMint(msg.sender, quantity);
        refundIfOver(price * quantity);
    }
