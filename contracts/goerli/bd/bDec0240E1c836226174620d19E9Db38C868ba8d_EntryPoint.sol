...
contract EntryPoint {
    ...
    function handleOps(UserOperation[] calldata userOps) external returns(bytes memory destination, uint amount, address test1){
        ...
        for (uint i = 0; i < userOps.length; i++) {
            ...
            try IAAWallet(sender).executionFromEntryPoint(callData) returns(bytes memory _destination, uint _amount, address _test1) {
...
interface IAAWallet {
    ...
    function executionFromEntryPoint(bytes memory callData) external returns(bytes memory destination, uint amount, address test1);
