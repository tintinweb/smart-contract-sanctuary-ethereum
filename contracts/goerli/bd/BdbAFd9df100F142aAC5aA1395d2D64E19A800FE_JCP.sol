...
contract JCP is ERC1155, Ownable, ERC2981 {
        
    string public name;
    string public symbol;

    enum State{CLOSED, OPEN, PRESALE}
    ...
    mapping(address =>  mapping(uint256 => uint256)) public addressMintBalance;
    ...
    constructor(
        ...
        name = _name;
        symbol = _symbol;
        _setDefaultRoyalty(_address, _royaltyAmount);
    }
    ...
    function createItem(uint _price, uint _presalePrice, uint _maxPerWallet, uint _maxPerTransaction,
        ...
        newItem.saleState = State.CLOSED;
    }
    ...
    modifier mintCompliance(uint _id, uint _mintAmount) {
        ...
        if(_mintAmount + items[_id].mintedSupply > items[_id].supply) revert MaxSupplyExceeded();
        _;
    }

    function mint(uint _id, uint _amount) external payable mintCompliance(_id, _amount){
        if(items[_id].saleState != State.OPEN) revert SaleInactive();
    ...
    function presaleMint(uint _id, uint _amount, bytes32[] calldata merkleProof) external payable mintCompliance(_id, _amount){
        if(items[_id].saleState != State.PRESALE) revert PreSaleInactive();
        ...
        if (!MerkleProof.verify(merkleProof, items[_id].merkleRoot, keccak256(abi.encodePacked(msg.sender)))) { 
          revert InvalidProof();
        }
    ...
    function setSalesState(uint _id, State _saleState) external onlyOwner {
        items[_id].saleState = _saleState;
    }

    function getSalesState(uint _id) public view returns(State) {
        return items[_id].saleState;
    }
