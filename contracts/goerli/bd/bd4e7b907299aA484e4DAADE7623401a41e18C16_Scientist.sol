    ...
    OwnableUpgradeable
{
    ...
    function setNewTranche(
        ...
        emit SetNewTranche(newSigner, newStagesID, newPrice, newAmount, newMaxClaimed, block.timestamp);
    }
    ...
    function _create(address _account) internal {
        ...
        _addScientist(_account, tokenId);
         (tokenId, msg.sender, feeNumerator);
    }
...
interface IScientistRepository {
    ...
    event RemoveScientist(uint256 tokenId, ScientistData.Scientist scientist, uint256 timestamp);

    function addScientist(address account, uint256 tokenId) external;
