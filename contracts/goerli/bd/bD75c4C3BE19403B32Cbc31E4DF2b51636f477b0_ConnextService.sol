...
contract ConnextService is Ownable, Pausable, IMdexCrossChainSwap {
    ...
    MdexController private controller;
    ...
    function execute(bytes calldata payload) external whenNotPaused returns (bytes calldata) {
        ...
        IERC20(assetAddress[destinationDomain]).approve(address(controller), amount);
    ...
    function connextSwap(uint256 amount, bytes calldata payload) internal {
        ...
        (, , originDomain, destinationDomain) = abi.decode(payload, (address, address, uint32, uint32));
...
contract MdexCrossChainSwap is Ownable, Pausable {
    ...
    constructor(MdexController _mdexController) {
        mdexController = _mdexController;
    }
    ...
    function swap(
        ...
        _crossChainSwap(amount - netAmount, payload);
    }
    ...
    function setCrossChainSwap(IMdexCrossChainSwap _serviceCrossChainSwap) public onlyOwner {
        serviceCrossChainSwap = _serviceCrossChainSwap;
    }
    ...
    function _crossChainSwap(uint256 amount, bytes calldata payload) private {
        IERC20(stableCoin).approve(address(mdexController), amount);
