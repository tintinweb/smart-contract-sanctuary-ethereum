...
import {IBasePositionManager} from "@kyber/interfaces/periphery/IBasePositionManager.sol";
import {AntiSnipAttackPositionManager} from "@kyber/periphery/AntiSnipAttackPositionManager.sol";
import {Router, IRouter} from "@kyber/periphery/Router.sol";
import {QuoterV2, IQuoterV2} from "@kyber/periphery/QuoterV2.sol";
 ...
 */
// TODO: add multicall
contract IntermediateRouter is ERC721Holder {
    AntiSnipAttackPositionManager public positionManager;
    ...
    constructor(
        AntiSnipAttackPositionManager positionManager_,
        Router router_,
        QuoterV2 quoter_,
        IERC721 nft_,
        vToken vtoken_
        ...
        returns (uint256 positionId)
    {
        ...
        // transferring ALL ETH to positionManager in this call
        // the required amount of ETH (qty0 or qty1) gets converted into WETH & deposited if new pool deployed
        // the remaining ETH remains in the positionManager contract
        // that ETH gets used during .mint()
        // and the remaining ETH is refunded back at last via .refundEth()
        (, uint256 qty0, uint256 qty1) = positionManager
            .createAndUnlockPoolIfNecessary{value: msg.value}(
            token0_,
            token1_,
            FEE,
            params.currentSqrtP
        ...
        uint256 amount1Min;
        if (isVToken0) {
            amount0Desired = vTokensAmount - qty0;
            amount0Min = amount0Desired;
            amount1Desired = msg.value - qty1;
        } else {
            amount0Desired = msg.value - qty0;
            amount1Desired = vTokensAmount - qty1;
            amount1Min = amount1Min;
        }

        IBasePositionManager.MintParams memory mintParams = IBasePositionManager
            .MintParams({
                token0: token0_,
                token1: token1_,
                fee: FEE,
    ...
    /**
     * @notice User should have given Position NFT approval to this contract, else revert
     */
    function removeLiquidity(RemoveLiquidityParams calldata params) external {
        ...
        IBasePositionManager.RemoveLiquidityParams
            memory removeParams = IBasePositionManager.RemoveLiquidityParams({
        ...
        ) = positionManager.removeLiquidity(removeParams);

        if (additionalRTokenOwed > 0) {
            IBasePositionManager.BurnRTokenParams
                memory rBurnParams = IBasePositionManager.BurnRTokenParams({
                    tokenId: params.positionId,
                    amount0Min: 0,
                    amount1Min: 0,
                    deadline: block.timestamp
                });
            (, uint256 feeAmount0, uint256 feeAmount1) = positionManager
                .burnRTokens(rBurnParams);
        ...
        // get our vtoken and weth from positionManager to this contract
        positionManager.transferAllTokens(
            address(vtoken),
            vTokenAmt,
            address(this)
        );
        positionManager.transferAllTokens(WETH, wethAmt, address(this));
        ...
        if (fractionalVTokenAmt > 0) {
            ...
            IRouter.ExactInputSingleParams memory swapParams = IRouter
            ...
            uint256 fractionalWethAmt = router.swapExactInputSingle(swapParams);
        ...
        returns (uint256 wethReceived)
    {
        ...
        IRouter.ExactInputSingleParams memory swapParams = IRouter
            .ExactInputSingleParams({
                ...
                limitSqrtP: params.limitSqrtP
            });
        wethReceived = router.swapExactInputSingle(swapParams);
    ...
    function buyNFTs(BuyNFTsParams calldata params) external payable {
        ...
        IRouter.ExactOutputSingleParams memory swapParams = IRouter
            .ExactOutputSingleParams({
                ...
                deadline: params.deadline,
                amountOut: vTokenAmt,
                maxAmountIn: msg.value,
                limitSqrtP: params.limitSqrtP
            });
        router.swapExactOutputSingle{value: msg.value}(swapParams);
     ...
     */
    function quoteBuyNFTs(uint256[] memory nftIds, uint160 limitSqrtP)
        external
        returns (uint256 ethRequired)
    {
        ...
        IQuoterV2.QuoteOutput memory output = quoter.quoteExactOutputSingle(
            quoteParams
        );
        ethRequired = output.returnedAmount;
    }
...
contract QuoterV2 is IQuoterV2, ISwapCallback {
    ...
    function swapCallback(
        ...
        (address tokenIn, address tokenOut, uint24 feeUnits) = path
            .decodeFirstPool();
        IPool pool = _getPool(tokenIn, tokenOut, feeUnits);
        require(address(pool) == msg.sender, "invalid sender");
        (uint160 afterSqrtP, , int24 nearestCurrentTickAfter, ) = pool
            .getPoolState();
    ...
    function _handleRevert(
        ...
        output.initializedTicksCrossed = PoolTicksCounter
        ...
        )
    {
        ...
        while (true) {
            ...
            initializedTicksCrossedList[i] = quoteOutput
        ...
        )
    {
        ...
        uint256 i = 0;
        while (true) {
            (address tokenOut, address tokenIn, uint24 feeUnits) = path
                .decodeFirstPool();
...
library PoolTicksCounter {
    function countInitializedTicksCrossed(
        ...
        (int24 tickLower, int24 tickUpper) = (nearestCurrentTickBefore <
            nearestCurrentTickAfter)
