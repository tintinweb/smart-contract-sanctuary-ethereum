...
contract BuyBackBot is Pausable, ReentrancyGuard, WETHWrap {
    ...
    function buyBackDID(uint256 amount) public onlyOwner {
        _sellRewardTokenToDID(amount);
    }
    ...
    function _sellRewardTokenToDID(uint256 _amount) internal returns (bool) {
        ...
        try uniswapRouter.exactInputSingle(params) returns (uint256 amountOut) {
            emit ConversionToDID(_amount, amountOut);
            return true;
        } catch {
