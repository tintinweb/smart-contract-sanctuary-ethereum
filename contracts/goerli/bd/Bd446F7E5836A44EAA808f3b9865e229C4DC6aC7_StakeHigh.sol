...
contract StakeHigh is Pausable, Ownable, ReentrancyGuard {

    Token HighToken;

    uint256 public planDuration = 180;

    uint256 _planExpired = 360;
    ...
    function claimReward() external returns (bool){
        ...
        HighToken.transfer(_msgSender(), totalTokens);
    ...
    function stakeToken(uint256 stakeAmount) external payable whenNotPaused {
        ...
        require(HighToken.balanceOf(_msgSender()) >= stakeAmount, "Insufficient Balance");
        
           HighToken.transferFrom(_msgSender(), address(this), stakeAmount);
            totalStakers++;
