...
error NFTTimedEditionCollection_Minting_Has_Ended(uint256 mintEndTime);
  ...
  LazyMintedCollection
{
  ...
  function mintCountTo(
    ...
    if (mintEndTime.hasExpired()) {
      revert NFTTimedEditionCollection_Minting_Has_Ended(mintEndTime);
    }
  ...
  /**
   * @inheritdoc TimeLimitedCollection
   */
  function numberOfTokensAvailableToMint()
    public
    view
    override(INFTDropCollectionMint, TimeLimitedCollection)
 ...
 * @author HardlyDifficult
 */
abstract contract TimeLimitedCollection is Initializable {
