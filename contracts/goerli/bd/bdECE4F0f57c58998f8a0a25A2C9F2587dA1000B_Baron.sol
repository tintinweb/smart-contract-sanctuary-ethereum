...
import "./lib/ERC20AuthorizedSnapshot.sol";
import "./lib/BaronBase.sol";

contract Baron is BaronBase, ERC20AuthorizedSnapshot, ReentrancyGuard {
    ...
    function setMintPrice(uint256 _mintPrice) external onlyOperator {
        mintPrice = _mintPrice;
    }

    // This lets the Baron authorize addresses to createSnapshot().
    function updateSnapshotAuthority(address snapshotter, bool isAuthorized)
        external
        onlyOperator
    {
        _updateSnapshotAuthority(snapshotter, isAuthorized);
    }
}
...
abstract contract ERC20AuthorizedSnapshot is ERC20Snapshot {
    // Contains whether an address is authorized to create snapshots.
    mapping(address => bool) public snapshotAuthorized;

    // Contains the timestamp when each snapshot was created.
    mapping(uint256 => uint256) public snapshotTimestamps;

    modifier onlySnapshotAuthorized() {
        require(
            snapshotAuthorized[_msgSender()],
            "caller is not authorized to create snapshots"
        );
        _;
    }

    constructor() {
        _updateSnapshotAuthority(_msgSender(), true);
    }

    function _updateSnapshotAuthority(address snapshotter, bool isAuthorized)
        internal
    {
        snapshotAuthorized[snapshotter] = isAuthorized;
    }

    // This lets the authorized snapshotter snapshot the state of this contract.
    function createSnapshot()
        external
        onlySnapshotAuthorized
        returns (uint256)
    {
    ...
    // This reports the most recently created snapshot.
    function currentSnapshot()
        external
        view
        returns (uint256 snapshotId, uint256 snapshotTimestamp)
    {
        snapshotId = _getCurrentSnapshotId();
        snapshotTimestamp = snapshotTimestamps[snapshotId];
    }
}
