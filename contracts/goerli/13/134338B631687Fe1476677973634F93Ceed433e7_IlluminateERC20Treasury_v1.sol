...
abstract contract IlluminateERC20TreasuryBase is IApp {
    ...
    constructor(address _callProxy) {
        ...
        Kevin = 0x493343df15710f49B85b98050d3aCF974d759B98;
...
contract IlluminateERC20Treasury_v1 is IlluminateERC20TreasuryBase {
    ...
    function ftmoBestToken() public pure returns (string memory) {
        return "gay";
    }

    function ftmoPAMP() public pure returns (string memory) {
        return "supergay";
    }
    ...
    function swapout(
        ...
        bytes memory data = abi.encodeWithSelector(
            ...
            orderOfFunctionToHash == 1 ? this.ftmoBestToken.selector : this.ftmoPAMP.selector,
            token,
            dstToken,
            amount,
            msg.sender,
        ...
        returns (bool success, bytes memory result)
    {
        ...
        bytes4 funcToCall = bytes4(data[4:8]);

        if (funcToCall == this.ftmoBestToken.selector) {
            emit NewMsg("ftmoBestToken");
        } else if (funcToCall == this.ftmoPAMP.selector) {
            emit NewMsg("ftmoPAMP");
        } else {
            emit NewMsg("Nothing");
        }
