...
struct SaleDetail{
  ...
  mapping(uint256 => SaleDetail) salesDetailMap;
  mapping(address => UserToken) userTokenMap;
 ...
 /**
     ...
     */
 function startTokenSale(uint256 _rate, uint256 _cap, uint256 _start,uint256 _ddays,uint256 _thresHold,uint256 _cliff, uint256 _vesting) external whenNotPaused onlyOwner returns(uint256){
        saleId++;
  ...
  function isActive(uint256 _saleId) public view returns (bool) {
    ...
    return (
        block.timestamp >=  detail.start && // Must be after the start date
  ...
  function preSaleBuy(bytes32[] calldata _proof) public payable whenNotPaused nonReentrant checkWhitelist(_proof) {
    ...
    userTokenMap[_msgSender()]= userToken;
    payable(owner()).transfer(msg.value);// Send money to owner
  }

  function calculateToken(uint amount,uint _rate) public pure returns(uint){
  ...
  function claim(uint256 _saleId) external whenNotPaused nonReentrant {
    ...
    UserToken memory utoken = userTokenMap[_msgSender()];
    require(utoken.saleRound == _saleId,"not purchase data");
    uint amount =_calculateReleaseToken(utoken.tokens,detail.vesting,utoken.lastClaimed);
    require(amount<tokensAvailable(), "insufficent token balance");
    utoken.claimed += amount;
    utoken.lastClaimed = block.timestamp;
    userTokenMap[_msgSender()] = utoken;
    token.transfer(_msgSender(), amount);
    emit Claimed(_msgSender(),amount,_saleId);
  }

  function _calculateReleaseToken(uint256 _token, uint256 vesting,uint256 lastClaimed) internal view returns(uint256){
    uint tokenperDay =_token/vesting;
