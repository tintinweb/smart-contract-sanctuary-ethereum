...
contract DOTS is ERC721AQueryable, AccessControl, VRFConsumerBaseV2 {
    ...
    mapping(uint256 => ShinyRoll) shinyRolls;
    ...
    /**
     * @notice admin function to initiate VRF transaction for shiny distribution
     * @param _fromTokenId first token id to participate
     * @param _toTokenId last token id to participate
     * @param _minEVOStage min evo stage for token to participate
     * @param _shinyAmount amount of shinies to distribute
     */
    function rollShinyDots(
        uint64 _fromTokenId,
        uint64 _toTokenId,        
        uint64 _minEVOStage,
        uint64 _shinyAmount
        ...
        shinyRolls[requestId] = ShinyRoll({
            fromTokenId: _fromTokenId,
            toTokenId: _toTokenId,            
            minEVOStage: _minEVOStage,
            shinyAmount: _shinyAmount
        });
    }

    /**
     * @notice callback to retrieve random number for shiny distribution
     * @param requestId id of the request made by rollShinyDots
     * @param randomWords the actual random number
     */
    function fulfillRandomWords(
        uint256 requestId,
        uint256[] memory randomWords
    ) internal override {
        ShinyRoll memory shinyRoll = shinyRolls[requestId];

        uint256 numAssigned;
        uint256 nonce;
        uint256 tempShinyToken;

        while (numAssigned < shinyRoll.shinyAmount) {
            tempShinyToken = (uint256(keccak256(abi.encodePacked(randomWords[0], nonce))) % (shinyRoll.toTokenId - shinyRoll.fromTokenId + 1)) + shinyRoll.fromTokenId;

            if(metadata[tempShinyToken].evoNum >= shinyRoll.minEVOStage && !metadata[tempShinyToken].isShiny) {
                metadata[tempShinyToken].isShiny = true;

                unchecked {
                    numAssigned++;
                }

                emit ShinyRolled(tempShinyToken);
            }
            unchecked {
                nonce++;
            }               
    ...
    function mint(uint256 _evoNum, uint256 _numDots) external {
        ...
        uint256 _tokenId = _nextTokenId() + 1;
    ...
    function _normalize(
        ...
        uint256[8] memory _traitIds = [
            ...
            _getId(_rand, uint256(64), uint256(65520), NUM_EYEWEAR),
        ...
        if (_traitIds[5] == 40) _traitIds[7] = 0;
    ...
    function _generateMask(uint256 _selector) private pure returns (uint256 _mask) {
        ...
        if (_selector > 1637) {
            if (_selector < 11468) { // 5 trait dot (15%)
                if (_selector < 4931) _mask = _mask | _eightBitMask << 48; // 33.5% clothing selected
                else if (_selector < 8175) _mask = _mask | _eightBitMask << 40; // 33% headgear selected
                else if (_selector < 11419) _mask = _mask | _eightBitMask << 32; // 33% mouthgear selected
                else _mask = _mask | _eightBitMask << 56; // 0.5% eargear selected
            } else if (_selector < 47512) { // 6 trait dot (55%)
                if (_selector < 25741) _mask = _mask | _eightBitMask << 40 | _eightBitMask << 48; // 39.6% no mouthgear, accessory
                else if (_selector < 43582) _mask = _mask | _eightBitMask << 32 | _eightBitMask << 48; // 49.5% no headgear, accessory
                else if (_selector < 43906)  _mask = _mask | _eightBitMask << 48 | _eightBitMask << 56; // 0.9% no headgear, mouthgear
                else if (_selector < 47438) _mask = _mask | _eightBitMask << 32 | _eightBitMask << 40; // 9.8% no clothing, accessory
                else if (_selector < 47470) _mask = _mask | _eightBitMask << 40 | _eightBitMask << 56; // 0.089% no clothing, mouthgear
                else _mask = _mask | _eightBitMask << 32 | _eightBitMask << 56; // 0.111% no clothing, headgear
            } else if (_selector < 63896) { // 7 trait dot (25%)
                if (_selector < 61590) _mask = _mask | _eightBitMask << 32 | _eightBitMask << 40 | _eightBitMask << 48; // 85.93% no accessory
                else if (_selector < 62298) _mask = _mask | _eightBitMask << 40 | _eightBitMask << 48 | _eightBitMask << 56; // 4.32% no mouthgear
                else if (_selector < 63076) _mask = _mask | _eightBitMask << 32 | _eightBitMask << 48 | _eightBitMask << 56; // 4.75% no headgear
