...
contract Platform is ReentrancyGuard {
    ...
    /// @return id of the bribe created.
    function createBribe(
        ...
        unchecked {
            ...
            bribes[newBribeID] = Bribe({
                ...
                endTimestamp: getCurrentPeriod() + ((numberOfPeriods + 1) * WEEK),
    ...
    function _claim(address user, uint256 bribeId) internal nonReentrant returns (uint256 amount) {
        ...
        // Address of the target gauge.
        address gauge = bribes[bribeId].gauge;
        // End timestamp of the bribe.
        uint256 endTimestamp = bribes[bribeId].endTimestamp;
        ...
        if (end <= currentPeriod) return 0;
        if (lastVote >= currentPeriod) return 0;
        ...
        uint256 _amountWithMaxPrice = _bias.mulWadDown(bribes[bribeId].maxRewardPerVote);
        // Distribute the min between the amount based on votes, and price.
        amount = min(amount, _amountWithMaxPrice);
        // Reward Token.
        address rewardToken = bribes[bribeId].rewardToken;
    ...
    function _rollOverToNextPeriod(uint256 bribeId, uint256 leftOver, uint256 nextPeriodTimestamp) internal {
        ...
        if (newNumberOfPeriods > 0) {
            ...
            bribe.totalRewardPerPeriod = nextTotalRewardPerPeriod[bribeId];
            ...
            delete nextTotalRewardPerPeriod[bribeId];
    ...
    function _updateRewardPerToken(uint256 bribeId) internal {
        ...
        if (gaugeSlope > 0 && rewardPerToken[bribeId] == 0) {
    ...
    function getPeriodsLeft(uint256 bribeId) external view returns (uint256 periodsLeft) {
        Bribe memory bribe = bribes[bribeId];
        uint256 endTimestamp = bribe.endTimestamp;
    ...
    /// @notice Next total reward per period amount next period.
    mapping(uint256 => uint256) public nextTotalRewardPerPeriod;
    ...
    function increaseBribeDuration(
        ...
        uint256 leftOver = bribe.totalRewardAmount - claimPerBribe[_bribeId] + _increasedAmount;

        emit BribeDurationIncreaseQueued(
            _bribeId,
            newNumberOfPeriods,
            leftOver,
            nextTotalRewardPerPeriod[_bribeId] = leftOver.mulDivDown(1, periodsLeft + _additionnalPeriods),
            _newMaxPricePerVote
            );
    }
    ...
    function createBribeTestnet(
        ...
        unchecked {
            ...
            bribes[newBribeID] = Bribe({
                ...
                rewardToken: rewardToken,
                numberOfPeriods: numberOfPeriods,
                endTimestamp: getCurrentPeriod() + ((numberOfPeriods) * WEEK),
                maxRewardPerVote: maxRewardPerVote,
                totalRewardAmount: totalRewardAmount,
                totalRewardPerPeriod: totalRewardPerPeriod,
                blacklist: blacklist
