...
contract NFTMarketplace is ReentrancyGuard, Ownable {
    ...
    function setFeeInfo(
        ...
        require(Address.isContract(_nftContract), "The address must be the NFT contract.");
        require(_feePercent + _marketFeePercent <= 10000, "The sum of fees already exceeded.");
        _feeData[_nftContract].marketFeePercent = _marketFeePercent;
        _feeData[_nftContract].feePercent = _feePercent;
        _feeData[_nftContract].feeAccount = payable(_feeAccount);
    ...
    function createMarketSale(
        ...
        uint256 feeAmount = (_feeData[_nftContract].feePercent * _price) / 10000;
        ...
        (bool successSeller, ) = payable(_seller).call{
            value: _price - feeAmount - marketFee
        }("");
        require(
            successSeller,
            "Transfer to seller could not be processed."
        );
        (bool successFee, ) = payable(_feeData[_nftContract].feeAccount).call{
            value: feeAmount
        }("");
        require(
            successFee,
