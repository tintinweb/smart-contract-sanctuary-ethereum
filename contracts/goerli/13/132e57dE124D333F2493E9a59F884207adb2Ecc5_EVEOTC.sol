...
interface IEVEOTCStakes {
    ...
    function getOptionDiscount(address user) external view returns (uint8, uint8);
}
...
contract EVEOTCTokens is Owner, ReentrancyGuard {
    ...
    // Manually set usd price for a user token
...
contract EVEOTC is EVEOTCTokens {
    ...
    uint32 public commission_option_trades_sell;
    uint32 public commission_option_trades_buy;

    // staking smart contract
    IEVEOTCStakes public stakes;

    /**
     * An offer can be either a Smart Trade Offer or an Option Trade
     */
    struct Offer {
        ...
        uint256 premium;    // premium price or percentage
                            // If smart_trade is true, then this is a percentage
                            // If smart_trade is false, this is a price
        ...
        uint16 commission_sell;   // selling commission at the moment of Offer creation
    ...
    struct Purchase {
        ...
        uint256 timestamp;
        address buyer;
        uint32  commission_buy;   // buying commission at the moment of Purchase creation
                                  // commission settings in the smart contract may change over time, we make commission decisions based on the settings at the time of trade creation
     ...
     * @param _commission_option_trades_sell commission to charge to sellers on an option trade
     * @param _commission_option_trades_buy commission to charge to buyers on an option trade
     *
     */
    function commissionSet(
        uint16 _commission_smart_trades_sell, 
        uint16 _commission_smart_trades_buy, 
        uint16 _commission_option_trades_sell, 
        uint16 _commission_option_trades_buy
    ) public isOwner {
        ...
        require(_commission_option_trades_sell <= 10000, "Option trades seller commission cannot be greater than 100");
        require(_commission_option_trades_buy <= 10000, "Option trades buyer commission cannot be greater than 100");
        // update contract parameters
        commission_smart_trades_sell = _commission_smart_trades_sell;
        commission_smart_trades_buy = _commission_smart_trades_buy;
        commission_option_trades_sell = _commission_option_trades_sell;
        commission_option_trades_buy = _commission_option_trades_buy;
    }
    ...
    function surplus(uint256 _available, bool _smart_trade) public view returns (uint256 _surplus, uint16 _commission_sell) {
        
        // discount on commissions
        uint8 stake_discount = 0;
        ...
        if (_smart_trade) {
            ...
            (stake_discount,) = stakes.getSmartDiscount(msg.sender);
        } else {
            commission_sell = commission_option_trades_sell;
            (stake_discount,) = stakes.getOptionDiscount(msg.sender);
        }
        ...
        if (stake_discount > 0) {
            ...
            commission_sell = commission_sell * (100 - stake_discount) / 100;
        }
     ...
     *               If smart_trade is true, then, if > 0 is a custom price in USD (including 8 decimals in the value), if 0 means a market price
     *               If smart_trade is false, then this is the strike price
     * @param _discount discount percentagefor smart trades
     ...
     *
     */
    function addOffer(bool _smart_trade, address _from, address[] memory _to, uint256 _available, uint256 _price, uint256 _discount, 
        ...
        // calculate the surplus from the seller to pay commissions
        (uint256 _surplus, uint16 _commission_sell) = surplus(_available, _smart_trade);
        ...
        trades.push(Offer(_smart_trade, msg.sender, _from, _to, _available, 0, _surplus, 0, _price, _discount, _premium, _time, _commission_sell));
        ...
        emit NewOffer( index, _smart_trade, msg.sender, _from, _to, _available, _surplus, _price, _discount, _premium, _time);
     ...
     */
    function cancelTrade(uint256 _index) external nonReentrant isNotPaused {
    ...
    function buySmartTrade(uint256 _index, address _to, uint256 _amount) external nonReentrant isNotPaused {
        ...
        uint256 _admin_commission = _amount * trades[_index].commission_sell / 10000;
        
        if (_admin_commission > 0) {
            require(IERC20(trades[_index].from).transfer(getOwner(), _admin_commission), "OTC: error paying commissions to owner");
            trades[_index].filled_surplus += _admin_commission;
        }

        // calculates the commission at the moment of buying, to consider if the user has an active stake / discount
        (, uint8 buyer_commission) = stakes.getSmartDiscount(msg.sender);
        uint32 _commission_buy = commission_smart_trades_buy * (100 - buyer_commission) / 100;

        // The buyer got assigned the amount bought
        smart_trades_purchases.push(Purchase(_index, trades[_index].from, _to, _amount, 0, 0, block.timestamp, msg.sender, _commission_buy, true));
    ...
    function getPurchasedWithdrawableTokens(uint256 _purchased_index) public view returns(uint256 _amount) {
        ...
        if (trades[smart_trades_purchases[_purchased_index].offer_index].time > 0) {
            // if vesting time, then amount available: elapsed * amount bought / time 
            available = elapsed * smart_trades_purchases[_purchased_index].amount / trades[smart_trades_purchases[_purchased_index].offer_index].time;
        }
    ...
    function getPurchasedTokens(uint256 _purchased_index) external nonReentrant isNotPaused {
        ...
        // get offer index
        uint256 _offer_index = smart_trades_purchases[_purchased_index].offer_index;

        // smart trades only
        require(trades[_offer_index].smart_trade, "OTC: this is not a smart trade");
        ...
        // If the buyer has to pay commissions:
        // the buyer pays the commission based on the commission set at the moment of transaction
        // this is to avoid liquidity errors in case the admin changes the commission in the middle
        uint256 _admin_commission = available_to_withdraw * smart_trades_purchases[_purchased_index].commission_buy / 10000;

        if (_admin_commission > 0) {
            require(IERC20(trades[_offer_index].from).transfer(getOwner(), _admin_commission), "OTC: error paying commissions to owner");
        }

        // withdraw tokens, minus admin commission
        require(IERC20(smart_trades_purchases[_purchased_index].from).transfer(msg.sender, available_to_withdraw - _admin_commission), "OTC: error doing the withdraw");
    ...
    function estimatePremiumAmount(uint256 _index, address _to) external view returns (uint256 estimation) {
        ...
        return trades[_index].premium * 10**IERC20(_to).decimals() / pay_token_price;
     ...
     *
     */
    function payPremium(uint256 _index, address _to, uint256 _amount) external nonReentrant isNotPaused {
        ...
        uint256 premium_amount = trades[_index].premium * 10**IERC20(_to).decimals() / pay_token_price;

        // Pays the premium amount to seller
        require(IERC20(_to).transferFrom(msg.sender, trades[_index].owner, premium_amount));

        // calculates the commission at the moment of buying, to consider if the user has an active stake / discount
        (, uint8 buyer_commission) = stakes.getOptionDiscount(msg.sender);
        uint32 _commission_buy = commission_option_trades_buy * (100 - buyer_commission) / 100;

        // Reserve the tokens to the buyer: open a purchase with zero withdrawn
        option_trades_purchases.push(Purchase(_index, trades[_index].from, _to, _amount, premium_amount, 0, block.timestamp, msg.sender, _commission_buy, false));
    ...
    // return the offer index of a purchase if it is a valid option trade
    function validateOptionTrade(uint256 _purchase_index) internal view returns (uint256 offer_index) {
        ...
        // Requires that this is an option trade
        require(!trades[_offer_index].smart_trade, "OTC: this is not an option trade");
     ...
     *
     */
    function payOptionTrade(uint256 _purchase_index) external nonReentrant isNotPaused {

        // validates the purchase and get the offer index if it is valid
        uint256 _offer_index = validateOptionTrade(_purchase_index);
        ...
        uint256 _surplus = option_trades_purchases[_purchase_index].amount * trades[_offer_index].commission_sell / 10000;
     ...
     * This function should be used by owner only in case of emergency to custody and protect user funds
     * As an owner of the contract you should remember that:
     *   You are encourage to use our technology only for lawful purposes in accordance with the laws of your country.
     *   Not to use our technology to mislead the public as to the value, outcome or performance related to this smart contracts.
     *   Commit to protect and safeguard the funds of the users of the smart contract
     */
    function dangerouslyWithdrawFundsShouldBeUsedOnlyToPreserveAndProtectCustomersFunds(address token, uint256 amount) external isOwner {
        require(IERC20(token).transfer(msg.sender, amount), "OTC: error canceling offer");
    }
