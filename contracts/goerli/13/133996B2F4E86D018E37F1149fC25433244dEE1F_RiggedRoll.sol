...
error RiggedRoll__NotEnoughEthInWallet();
error RiggedRoll__TransactionFailed();
error RiggedRoll__RequestedAmountIsMoreThanBalance();
...
contract RiggedRoll is Ownable {
    ...
    function withdraw(address _addr, uint256 _amount) external onlyOwner() {
        if(_amount > address(this).balance) {
            revert RiggedRoll__RequestedAmountIsMoreThanBalance();
        }
    ...
    function riggedRoll() public {
        if(address(this).balance < 0.002 ether) {
            revert RiggedRoll__NotEnoughEthInWallet();
        }
        ...
        if(roll > 2) {
            revert("Next roll is not the winner!");
        }
