...
contract Staking is Ownable, Pausable, ReentrancyGuard {
    ...
    function withdraw(uint256 amount) external nonReentrant {
        ...
        require(block.timestamp >= stakeInfo[_msgSender()].enteredAt + 1800, "Cannot unstake yet");
        _manageReward(_msgSender());
        totalStaked -= amount;
        stakeInfo[_msgSender()].amount -= amount;
    ...
    function pendingReward(address account) public view returns(uint256) {
        return ((stakeInfo[account].amount * 15 * (block.timestamp - stakeInfo[account].updatedAt)) / 3600);
    }
