...
contract LandNFT_V1 is Initializable, ERC721EnumerableUpgradeable, OwnableUpgradeable {
    ...
    function initialize(
        ...
        for (uint256 i; i < __tokenIdList.length; i++) {
    ...
    function supplyInfo() public view returns(
        ...
        return (super.totalSupply(), maxSupplyMint);
    }
    ...
    function buy(
        ...
        require(_mintNum < MAX_SUPPLY_MINT.sub(super.totalSupply()), "LandNFT: buy: c");
        ...
        return transferTokens(_mintNum, tokenPrice);
    }
     ...
     */
    function transferTokens(uint256 _mintNum, uint256 _tokenPrice) internal returns(string[] memory) {
        string[] memory tokenHashList;
        ...
        for (uint256 i; i < _mintNum; i++) {
            for (uint256 index; index < MAX_SUPPLY_MINT; index++) {
                ...
                if (!isNonMintTokenIds[tokenId]) {
                    ...
                    tokenHashList[index] = tokenURI(tokenId);
                    tokenIdListIndex++;
                    break;
                } else {
                    tokenIdListIndex++;
