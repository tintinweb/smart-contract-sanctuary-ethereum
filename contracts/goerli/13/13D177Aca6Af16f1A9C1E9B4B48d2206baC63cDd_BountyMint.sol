...
contract BountyMint is ERC721A, Pausable, Ownable, ReentrancyGuard {
    ...
    uint256 public constant MAX_PRE_MINT = 6; // This actually means 2 mints but we say 5 for minor gas saving
    uint256 private constant MAX_RESERVED = 200;
    uint256 public giftCount = 0;
    uint256 public PRESALE_PRICE = 0.0001 ether; // TO UPDATE
    uint256 public PRICE = 0.0001 ether; // TO UPDATE
    ...
    constructor(string memory _cURI, string memory _metaBURI, address _developer, address _treasury) public ERC721A ("Bounty", "BTY") {
        _contractURI = _cURI;
    ...
    /**
     * @dev mint functions: public is open to anyone
     * premit requires whitelist
     * gift_mint can only be called by owner
     */
    function saleMint(uint256 qty) checkTokenSupplyAvailable(qty) validPayment(qty, PRICE) nonReentrant external payable {
        require(saleLive, "Public Mint is currently closed");
        _mint(msg.sender, qty);
    }
    
    // Presale
    // FUNCTION IS WIP NEEDS TO BE COMPLETED AND TESTED
    function presaleMint(uint256 qty, bytes32[] calldata merkleProof) validPayment(qty, PRESALE_PRICE) addressWhitelisted(merkleProof, whitelistRoot) nonReentrant external payable {
        require(presaleLive, "Presale Mint is currently closed");
        require(presaleMintCount[msg.sender] + qty < MAX_PRE_MINT, "Maximum number of tokens for this address reached");
    ...
    /**
     * @dev getOwnersTokens will return an array of all the tokens belonging to that address
     */
    ...
    {
        require(balanceOf(_owner) > 0, "Wallet contains no Bounty tokens");
    ...
    /**
     * @dev returnAddressesOfHolder will return an array of addresses belonging to the submitted tokenIds
     */
    ...
    /**
     * @dev On-chain royalty details.
     * NOTE: HAVE NOT IMPLEMENTED THIS VIA OZ CONTRACT YET
     */
    ...
    //     // NOTE
    //     // THE BELOW ROYALTY AMOUNT TO BE CONFIRMED WITH B
    //     return (address(treasury), (salePrice * 5) / 100);
    // }

    /**
     * @dev Below are functions for testing
     */
