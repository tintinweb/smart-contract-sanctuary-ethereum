...
contract AssetManager is IAssetManager, Ownable {
    ...
    event PoolCreated(address pool, IAssetPool.Pool info, bool isPublic);
    event TokenSupportAdded(address token, string name, string symbol);
    ...
    constructor(
        ...
        for (uint i; i < _tokens.length; i++) {
            ...
            if (token != address(0)) {
                ...
                emit TokenSupportAdded(token, _token.name(), _token.symbol());
            } else {
                emit TokenSupportAdded(token, "Ether", "ETH");
            }
    ...
    function createPool(IAssetPool.Pool memory pool, bool isPublic) external payable {
        ...
        if (config.token == address(0)) value = msg.value;
            ...
            config.token.transferFrom(msg.sender, address(this), config.initAmount);
            config.token.approve(address(assetPool), type(uint).max);
        }
        assetPool.deposit{value: value}(config.token, config.initAmount);
        emit PoolCreated(address(assetPool), pool, isPublic);
    }
    ...
    function getUsdBalance(address account) external view returns (uint usdBalance) {
    ...
    function addTokenSupported(address token) public onlyOwner {
    ...
    function _isERC20(address token) internal view returns (bool) {
        ...
        if (!token.isContract()) return false;
        (bool success1, bytes memory result1) = token.staticcall(abi.encodeWithSignature("name()"));
        if (!success1) return success1 && result1.length > 0;
        (bool success2, bytes memory result2) = token.staticcall(abi.encodeWithSignature("symbol()"));
        if (!success2) return success2 && result2.length > 0;
        (bool success3, bytes memory result3) = token.staticcall(abi.encodeWithSignature("decimals()"));
        if (!success3) return success3 && result3.length > 0;
        (bool success4, ) = token.staticcall(abi.encodeWithSignature("totalSupply()"));
        if (!success4) return success4;
        (bool success5, ) = token.staticcall(abi.encodeWithSignature("balanceOf(address)", sender));
        if (!success5) return success5;
        (bool success6, ) = token.staticcall(
            abi.encodeWithSignature("allowance(address,address)", sender, sender)
        );
        if (!success6) return success6;
        return true;
    }
}
