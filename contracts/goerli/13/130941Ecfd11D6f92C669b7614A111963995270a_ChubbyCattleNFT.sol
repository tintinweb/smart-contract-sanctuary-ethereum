  ...
  MathFunctions
{
  ...
  uint256 private constant PUBLIC_STAGE = 2;

  uint256 private constant TIERS_COUNT = 2;
  ...
  constructor(
    ...
    ERC721A("Chubby Cattle NFT", "CHUBBYCATTLE") 
  {
  ...
  function mint(
    ...
    whenNotPaused
    nonReentrant
    stageCheck(_msgSender(), _merkleProof)
    tierCheck(_tier, false)
    supplyCheck(_tier, _quantity)
  {
    ...
    if(msg.value < 0.05 ether * _quantity) {
    ...
    _afterTokenMints(_tier, _quantity, 0);
  }
  ...
  {
    uint256 price = tierPrices[_tier];
    return price == 0 ? price : usdToEth(price);
  }
  ...
  */
  ...
  )
  {
    ...
    if(stage < PUBLIC_STAGE && !MerkleProof.verify(_merkleProof, merkleRoot, keccak256(abi.encodePacked(_to)))) {
  ...
  {
    if(_tier > TIERS_COUNT || (!privileged && tierPrices[_tier] == 0)) {
      revert InvalidTier();
    }
    _;
  }
    ...
    returns (uint256)
  {
    ...
    // return mulDiv(amount, 10**(_decimals), uint256(price));
    return 0.05 ether;
  }
    ...
    returns (int256, uint8)
  {
    ...
    _decimals = priceFeed.decimals(Denominations.ETH, Denominations.USD);
    ...
    override
  {
    if(_quantity == 1 &&
      address(0) != _from
      && block.timestamp > tokenLocks[_tokenId]) {
    ...
    internal
  {
    ...
    for(uint256 i = nextTokenId - _quantity; i < nextTokenId; i++) {
      tokenTiers[i] = _tier;
      tokenLocks[i] = _lockTimestamp;
    }
    tierMints[_tier] += _quantity;
    tierLimits[_tier] -= _quantity;
  }
