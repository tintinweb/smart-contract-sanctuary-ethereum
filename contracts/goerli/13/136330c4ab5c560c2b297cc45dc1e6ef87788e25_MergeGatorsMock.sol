...
contract MergeGators is
    ...
    //COLLECTION SIZES
    uint constant COLLECTION_SIZE = 1000000;
    ...
    uint32 private constant NUM_WORDS = 500;
    ...
    function verify(address whitelisted, uint8 v, bytes32 r, bytes32 s) internal virtual {
        ...
        require(_whitelistedAddr[recoveredAddress] == 1, "Invalid signer" );
   ...
   /**
    ...
    function VRFMint(address to, uint256 _quantity) internal {
        ...
        uint moddedRng = random(to) % MAX_CHANCE_VALUE;
    ...
    function random(address to) internal returns(uint256){
        ...
        return uint256(keccak256(abi.encodePacked(to, _randomWords[RandomCounter.current()])));
    ...
    function whitelistMint(address to, uint count, uint8 v, bytes32 r, bytes32 s ) public {  
        if (count <= 0) revert callErr();
        if (saleStatus == SaleStatus.PRESALE || saleStatus == SaleStatus.PUBLIC) {
            //check coupon
            verify(to, v, r, s);
            //check limits - per wallet
            if (_whitelistMintedCount[to] + count >= tokens_per_WL) revert callErr();
            //check limits - WL sale allowance
            if (WLCounter.current() + count >= collection_size_WL) revert callErr();

            _whitelistMintedCount[to] += count;

            alligatorMint(to, count);

            WLCounter.increment(count);
        }
    }
