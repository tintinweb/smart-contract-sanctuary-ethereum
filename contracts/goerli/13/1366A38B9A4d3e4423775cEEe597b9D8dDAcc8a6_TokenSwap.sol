...
contract TokenSwap is FxBaseRootTunnel {
  ...
  constructor(address _cpManager, address _fxRoot, address _governanceFund) FxBaseRootTunnel(_cpManager, _fxRoot) {
  ...
  function convertETHtoEFT() external payable {
    ...
    governanceFund.transfer(msg.value);
  }
  ...
  function depositTreasury() external payable {
    require(msg.value > 0, "Must send ETH");
    uint256 eftAmount = msg.value * conversionFactor;
    bytes memory message = abi.encodeWithSelector(Treasury.mintForUser.selector, eftAmount);
    _sendMessageToChild(message);
    governanceFund.transfer(msg.value);
  }
  ...
  function _processMessageFromChild(
    ...
    payable(user).transfer(amountEft / conversionFactor);
  }
