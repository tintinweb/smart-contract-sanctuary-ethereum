...
contract Minter is Context, Ownable, Pausable, ReentrancyGuard {
  ...
  /// @notice Open access requires the sender to pay the price of the token.
  ...
  function mintReveller() public payable nonReentrant {
    if (pool.length == 1) {
  ...
  /// Mint Patron ///
  /// @notice Mints 2 NFTs at once from the revller contract.
  /// @notice Can only be called by the minter owner / vault.
  /// @param _tokenIdPatron The ID of the Patron's NFT.
  /// @param _tokenIdReserve The ID of the Reserve NFT.
  /// @param _toPatron The address that will receive the NFT.
  /// @param _toReserve The address that will receive the NFT.
  /// @param _uriPatron The URI of the Patron's NFT.
  /// @param _uriReserve The URI of the Reserve NFT.
  function mintPatron(
    uint256 _tokenIdPatron,
    uint256 _tokenIdReserve,
    address _toPatron,
    address _toReserve,
    string memory _uriPatron,
    string memory _uriReserve
  ) public onlyOwner {
    /// @dev Create token reference
    ERC721withVotesAndEIP2981 Token = ERC721withVotesAndEIP2981(token);

    /// @dev Mint patron
    Token.mint(_toPatron, _tokenIdPatron, _uriPatron);

    /// @dev Mint reserve
    Token.mint(_toReserve, _tokenIdReserve, _uriReserve);
  }
