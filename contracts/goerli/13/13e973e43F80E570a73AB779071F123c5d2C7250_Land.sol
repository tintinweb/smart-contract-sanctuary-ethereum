...
contract Land is ERC721A, Ownable {
    ...
    uint256 public constant ALLOW_LIST_UNLOCK_TIME = 1667833200;
    ...
    uint256 public investorR1StartTime;
    uint256 public investorR2StartTime;
    uint256 public investorStartTokenId;
    uint256 public totalInvestorMinted;
    mapping(address => uint256) public investorMinted;
    uint256 public constant MAX_INVESTOR_LANDS = 500;
    uint256 public constant INVESTOR_MINT_PRICE = 0.0005 ether;       // 0.5 ether

    uint256 public mintStartTime;
    uint256 public mintEndTime;
    uint256 public mintStartTokenId;
    uint256 public constant MINT_START_PRICE = 0.0008 ether;        // 0.8  ether
    uint256 public constant MINT_END_PRICE = 0.0006 ether;          // 0.6  ether
    uint256 public constant DISCOUNT_RATE = 0.00001 ether;          // 0.01 ether
    ...
    bool public bonusMarketingFeePaid = false;
    address payable public marketingWallet;
    uint256 public constant MARKETING_FEE_PERCENT = 4;
    ...
    function claimLands(
        ...
        external
        mustBetween(claimableStartTime, investorR1StartTime)
    ...
    function investorR1MintLands(
        ...
        external
        payable
        mustBetween(investorR1StartTime, investorR2StartTime)
        onlyVerified(Round.InvestorR1, maxLands, signature)
        mustMatchPrice(INVESTOR_MINT_PRICE, numLands)
    {
        _investorMintLands(numLands, maxLands);
    }

    function investorR2MintLands(
        uint256 numLands
    )
        external
        payable
        mustBetween(investorR2StartTime, mintStartTime)
        mustMatchPrice(INVESTOR_MINT_PRICE, numLands)
    {
        _investorMintLands(numLands, maxMintPerAddress);
    }
        ...
        payable
    {
        ...
        uint256 mintCost = _getMintPrice(elapsed) * numLands;
        ...
        if (elapsed <= 1800 && !bonusMarketingFeePaid && totalSupply() - mintStartTokenId + 1 >= 500) {
            bonusMarketingFeePaid = true;
            _payBonusMarketingFee();
        }

        if (elapsed <= 3600 && totalSupply() == MAX_LANDS) {
    ...
    function getMintPrice() external view returns (uint256) {
        require(_mintActive(), "Mint not started");
        return _getMintPrice(block.timestamp - mintStartTime);
    }

    // Internal functions

    function _getMintPrice(uint256 elapsed) internal pure returns (uint256) {
        if (elapsed <= 360) {
            return MINT_START_PRICE;
        } else if (elapsed >= 7200) {
            return MINT_END_PRICE;
        } else {
            uint256 _m = elapsed / 360;
            return MINT_START_PRICE - DISCOUNT_RATE * _m;
        }
    }
    ...
    function _investorMintLands(uint256 numLands, uint256 maxLands) internal {
        require(
            numLands + investorMinted[msg.sender] <= maxLands,
            "Max lands per address exceeded"
        );
        require(
            totalInvestorMinted + numLands <= MAX_INVESTOR_LANDS,
            "Max investor lands exceeded"
        );

        if (investorStartTokenId == 0) {
            investorStartTokenId = _nextTokenId();
        }

        investorMinted[msg.sender] += numLands;
        totalInvestorMinted += numLands;
        _mintLands(msg.sender, numLands);
    }
    ...
    function _payBonusMarketingFee() internal {
        if (marketingWallet != address(0)) {
            uint256 amount = address(this).balance * MARKETING_FEE_PERCENT / 100;
    ...
    function _beforeTokenTransfers(
        ...
        if (allowListStartTokenId == 0 || startTokenId < allowListStartTokenId) {
            ...
            require(block.timestamp >= ALLOW_LIST_UNLOCK_TIME, "Transfer is not enabled");
        } else if (investorStartTokenId == 0 || startTokenId < investorStartTokenId) {
    ...
    function setInvestorStartTime(uint256 _r1StartTime, uint256 _r2StartTime) external onlyOwner {
        investorR1StartTime = _r1StartTime;
        investorR2StartTime = _r2StartTime;
    }
