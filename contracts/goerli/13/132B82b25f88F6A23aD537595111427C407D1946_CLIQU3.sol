...
contract CLIQU3 is ERC721A, DefaultOperatorFilterer, Ownable  {
    ...
    uint256 public stage1Counter = 0;
    ...
    bool public stage_1 = true;
    bool public stage_2 = false;

    address public previosCollection = 0x77d644480BBD9A109DA23D6e0Bf5A48443d7e0Af;
    ...
    constructor(
        ...
        setOtherAddressContract(_initOtherContract);
    
        //teamList = [0x404AA2D581598A25FFf9C100566205d7337d5E94, 0x3A2dF6cDC1cE999FEc5959e6586f2C2376296B2b, 0x5B38Da6a701c568545dCfcB03FcB875f56beddC4];
    ...
    function calcWorth(uint256 quantity, bytes32[] calldata _merkleProof) internal returns(uint) {
       uint256 worth = cost * quantity;

        if(_merkleProof.length > 0) {
            require(!pausedWL, "WL minting is paused");
            require(_numberMinted(msg.sender) + quantity <= maxMintPerAddressLimit, "Exceeded the limit for wl");
            
            bytes32 leaf = keccak256(abi.encodePacked(msg.sender));

            require(MerkleProof.verify(_merkleProof, merkleRootDiscounted, leaf), "You arent registered in wl");

            uint limitForStage = 1000 + getMintedTokensByTeam() + (stage_2 ? stage1Counter : 0);

            if(stage_1){
                stage1Counter = stage1Counter + quantity;
            } 

            require(totalSupply() + quantity <= limitForStage,  "Exceeded the limit for current stage of 1000 tokens" );

            

            return worth - (worth * discount) / 100;
        }
    ...
    function getMintedTokensByTeam() view internal returns(uint256){
        uint256 sum;
        for (uint i = 0; i < teamList.length; i++) {
            sum+= _numberMinted(teamList[i]);
        }
        return sum;
    }
    ...
    function teamMint(uint256 quantity) external {
        if(msg.sender != owner()){
            require(quantity <= 5, "5 tokens limit for team");
            require(_numberMinted(msg.sender) + quantity <= 5, "Exceeded the limit for team mint");
        }
        uint256 k = 0;
        for (uint i = 0; i < teamList.length; i++) {
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        ...
        if(revealed == false) {
            return string(abi.encodePacked(prefix, notRevealedUri, "/", tokenId.toString(), ".json"));
        }
    ...
    function setPercentPerTransfer(uint256 _newPercent) public onlyOwner{
        percentPerTransfer = _newPercent;
    }
    ...
    function switchStages() public onlyOwner{
        stage_1 = false;
        stage_2 = true;
    }
