...
contract ShibaDogeArmy is ERC1155Supply, Ownable, ReentrancyGuard, Pausable {
    string public name;
    ...
    event EFT_Equipped(uint256 id);
    event EFT_Unequipped(uint256 id);

    constructor(string memory _name, 
        string memory _uri, 
        IERC721 _DogeArmy, 
        IERC721 _ShibaArmy) ERC1155(_uri) {
        name      = _name;
        DogeArmy  = _DogeArmy;
    ...
    function mintNFT(
        ...
        _mint(msg.sender, NFTSupply + 1, 1, data);

        applyMintedTraits(NFTSupply + 1, traits);
    ...
    uint8 public idShiftModifier = 20;

    error NFTCannotBeEquippedAsEFT(uint256 id);
    function determineTraitSlot(uint256 id) public view returns (uint256) {
        uint256 slot = (id >> idShiftModifier);
    ...
    // EFT ID's start with their slot in the first byte of the ID
    // 0x1... to 0xF...

    // NFT IDs start with 0x0
    mapping(uint256 => uint24[15]) public nftTraitsArray;


    // SALES, NOT DONE
     function createEFTSale(
        uint256 id,
        uint256 supply,
        uint256 price
    ) external onlyOwner {}

    function pauseEFTSale(uint256 id) external onlyOwner {}

    function cancelEFTSale(uint256 id) external onlyOwner {}

    mapping(uint256 => tokenSaleData) public tokenSales;

    function buyEFT(uint256 id, bytes calldata data)
        external
        payable
        whenNotPaused
    {
        require(
            tokenSales[id].supply > 0 &&
                block.timestamp < tokenSales[id].endtime,
            "Sale has ended"
        );
        require(msg.value == tokenSales[id].price, "Incorrect Funds Sent");
        require(
            tokenSales[id].minted < tokenSales[id].supply,
            "Sale is Sold Out"
        );

        _mintEFT(msg.sender, id, 1, data);
    }
