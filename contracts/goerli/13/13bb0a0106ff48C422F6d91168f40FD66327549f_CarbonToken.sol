...
/// @notice This contract is for minting DNG carbon emission packets.
/// process:
///     1. Read currentTokenId.
///     2. Create DNG JSON file and publish it to IPFS with the naming convention:
///             a. currentTokenId+1.json (i.e. currentTokenId == 3, data payload would be published to IPFS as "4.json").
///     3. Execute mint().

contract CarbonToken is ERC721, Ownable {
    ...
    constructor() ERC721("PROVE DNG Token", "DNG") {
    ...
    /// @param  _receiver is the wallet that is going to receive the nft.
    ...
    function withdraw() external onlyOwner {
        ...
        require(balance > 0, "CarbonToken.sol::withdraw() Insufficient ETH balance");

        (bool success,) = owner().call{value: balance}("");
        require(success, "CarbonToken.sol::withdraw() Unable to withdraw funds, recipient may have reverted");
    }
    ...
    function withdrawERC20(address _contract) external onlyOwner {
        require(_contract != address(0), "CarbonToken.sol::withdrawERC20() Contract address cannot be zero address");

        uint256 balance = IERC20(_contract).balanceOf(address(this));
        require(balance > 0, "CarbonToken.sol::withdrawERC20() Insufficient token balance");

        bool success = IERC20(_contract).transfer(owner(), balance);
        require(success, "CarbonToken.sol::withdrawERC20() Transfer failed");
    }
}