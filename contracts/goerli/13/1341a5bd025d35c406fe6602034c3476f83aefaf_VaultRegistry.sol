...
contract VaultFactory is IVaultFactory {
    ...
    /// @param _plugins Addresses of plugin contracts
    /// @param _selectors List of selectors on plugins
...
contract VaultRegistry is IVaultRegistry {
    ...
    /// @notice Creates a new vault with permissions, plugins, and initialization calls, and transfers ownership to a given owner
    ...
    /// @notice Creates a new vault with permissions, plugins and initialization calls
    /// @param _merkleRoot Hash of merkle root for vault permissions
    /// @param _plugins List of plugins
    /// @param _selectors List of selectors on plugins
    /// @param _calls List of initialization calls
    ...
    function create(
        ...
        vault = createFor(_merkleRoot, address(this), _plugins, _selectors);
    }

    /// @notice Creates a new vault with permissions, plugins, and initialization calls for a given controller
    /// @param _merkleRoot Hash of merkle root for vault permissions
    ...
    /// @notice Creates a new vault with permissions, plugins, and intialization calls for the message sender
    /// @param _merkleRoot Hash of merkle root for vault permissions
    /// @param _plugins List of plugins
    /// @param _selectors List of selectors on plugins
    /// @param _calls List of initialization calls
    ...
    /// @notice Creates a new vault with permissions, plugins, and initialization calls for an existing collection
    /// @param _merkleRoot Hash of merkle root for vault permissions
    ...
    function _deployVault(
        ...
        vault = VaultFactory(factory).getNextAddress(
            tx.origin,
            _owner,
            _merkleRoot
        );
        vaultToToken[vault] = VaultInfo(_token, ++nextId[_token]);
        VaultFactory(factory).deployFor(
            _merkleRoot,
            _owner,
            _plugins,
            _selectors,
