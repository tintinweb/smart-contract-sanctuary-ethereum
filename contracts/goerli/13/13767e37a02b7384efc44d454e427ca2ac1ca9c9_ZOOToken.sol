...
abstract contract BasicToken is Ownable, ERC20Basic {
    ...
    */
    ...
    function _calculateFee(address _from, address _to, uint256 _value) internal view returns(uint256 fee, uint feeTeam, uint256 feeLiquid) {
        ...
        if (_to == router || _from == UniswapV3Pool) {
    ...
    function _shareFee(address _from, uint feeTeam, uint256 feeLiquid) internal {
        ...
        if (feeLiquid > 0) {
            balances[address(this)] = balances[address(this)].add(feeLiquid);
            emit Transfer(_from, address(this), feeLiquid);
        }
    }

    function _mintNewPosition(uint256 amount0ToMint, uint256 amount1ToMint, uint24 poolFee) internal returns(uint256) {
        TransferHelper.safeApprove(address(this), address(nonfungiblePositionManager), amount0ToMint);
        TransferHelper.safeApprove(pairedToken, address(nonfungiblePositionManager), amount1ToMint);
        ...
        (   uint256 tokenId,
            ,
            uint256 amount0,
            uint256 amount1) = nonfungiblePositionManager.mint(params);
        ...
        if (amount1 < amount1ToMint) {
            // TransferHelper.safeApprove(pairedToken, address(nonfungiblePositionManager), 0);
            uint256 refund1 = amount1ToMint - amount1;
            TransferHelper.safeTransfer(pairedToken, msg.sender, refund1);
        }
        return tokenId;
    }

    function addLiquidityPosition(uint256 amountAdd0, uint256 amountAdd1) internal {
        if(tokenID == 0) {
            _mintNewPosition(amountAdd0, amountAdd1, 3000);
        } 
            ...
            (, ,  uint256 amount1) = nonfungiblePositionManager.increaseLiquidity(params);
        
    }

    function _swapBack(uint256 _amount) internal returns(uint256 amountOut) {
    ...
    function _swapLiquidify() internal {
        uint256 _myBalance = balances[address(this)];
        if (_myBalance >= 1000E18) {
            uint256 swapAmount = _myBalance.div(2);
            uint256 amountOut = _swapBack(swapAmount);
            addLiquidityPosition(swapAmount, amountOut);
        }
    }
...
contract ZOOToken is Pausable, StandardToken {
    ...
    function setDexAddresses(address _routerAddress, address _UniswapV3Pool) external onlyOwner {
        router = _routerAddress;
        UniswapV3Pool = _UniswapV3Pool;
        emit DexChanged(router, UniswapV3Pool);
    }



    // Forward ERC20 methods to upgraded contract if this one is deprecated
    function transfer(address _to, uint256 _value) override  public whenNotPaused {
        if (deprecated) {
    ...
    function setNonfungiblePositionManager(INonfungiblePositionManager newPositionmanager, address _pair, ISwapRouter _swapRouter) external onlyOwner {
        nonfungiblePositionManager = newPositionmanager;
        pairedToken = _pair;
        swapRouter = _swapRouter;
        emit PositionManager(address(nonfungiblePositionManager), pairedToken, address(swapRouter));
    }

    // for new position
    event PositionManager(address indexed newPosition, address indexed NewPair, address indexed router);
    ...
    // When Router and UniswapV3Pools DexChanged
    event DexChanged(address indexed newRouter, address indexed newUniswapV3Pool);
}