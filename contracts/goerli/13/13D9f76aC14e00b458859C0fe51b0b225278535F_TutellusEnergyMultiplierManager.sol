...
contract TutellusEnergyMultiplierManager is UUPSUpgradeableByRole {
    ...
    function check(address _contract, uint8 _type) public view returns(bool) {
        return _multiplierType[_contract] == _type;
    }
    ...
    function _getEnergyMultiplier(address _contract) internal view returns(uint256) {
        if (_multiplierType[_contract] == 1) {
            return _getMultiplierStaking();
        } else if (_multiplierType[_contract] == 2) {
            return _getEnergyMultiplierFarming(_contract);
        }
        return 0;
    }
    ...
    function _getEnergyMultiplierFarming(address _contract) public view returns (uint256) {
