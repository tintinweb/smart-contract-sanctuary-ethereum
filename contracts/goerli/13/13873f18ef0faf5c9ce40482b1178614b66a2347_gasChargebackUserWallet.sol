...
contract gasChargebackUserWallet {
     ...
     * @param currentSequence - the next valid unique nonce in sequence order
     * @param to - the address that the metaTransaction is to be sent to
     * @param value - how much ETH to sent to the recipient
     * @param data - any additional payload for the metaTransaction
     */
    function metaTransactionMessage(uint256 thisChainId, uint256 currentSequence, address to, uint256 value, bytes calldata data) public pure returns (bytes memory){
        //returns a hash of a dummy metaTransaction
        return abi.encodePacked(thisChainId, currentSequence, to, value, data); 
    }
