...
contract CounterNFT is ERC721URIStorage, Ownable {
    ...
    function nftMint() public onlyOwner {
        _tokenIds.increment();
        uint256 newTkenId = _tokenIds.current();

        _mint(_msgSender(), newTkenId);

        string memory jsonFile = string(abi.encodePacked('metadata', Strings.toString(newTkenId), '.json')); 
        _setTokenURI(newTkenId, jsonFile);
        emit TokenURIChanged(_msgSender(), newTkenId, jsonFile);
    }
