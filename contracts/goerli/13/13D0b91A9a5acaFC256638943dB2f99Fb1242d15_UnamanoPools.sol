...
contract UnamanoPools is Ownable,ReentrancyGuard {
    ...
    struct PoolInfo {
        IERC20 candyToken;
        uint256 startBlock;
        uint256 endBlock; // 第一次质押后，更新endBlock
    ...
    struct UNA {
        address creator; // 项目发行人
        uint256 unlockTime;         // unlockTime
        uint256 maximumStaking;
        uint8 status; // 项目状态， 1 - 正常  2 - 暂停
        address multisignatureWallet; // 项目多签管理钱包
    ...
    address public treasury;// 国库
    

    event AddPool(uint256 indexed pid, uint256 indexed lockDays, uint256 indexed candySupply, string authorizationCode);// 项目发行人发布项目
    // event AddPool0(address indexed user, address indexed lpToken, address candyToken);
    // event AddPool1(uint256 indexed lockDays, uint256 indexed candySupply, uint256 candyPerBlock);
    // event AddPool2(uint256 indexed maximumStaking, address indexed assetManagementAddr, string authorizationCode);// 项目发行人发布项目

    event AddCandy(address indexed user, uint256 indexed pid, uint256 candyAmount);// 项目方发行人追加糖果
    event Deposit(address indexed user, uint256 indexed pid, uint256 amount);// 用户质押
    event Withdraw(address indexed user, uint256 indexed pid, uint256 amount);// 用户退出质押
    event EmergencyWithdraw(address indexed user, uint256 indexed pid, uint256 amount);// 紧急退出
    event ProjectAward(uint256 indexed pid, uint256 amount); // 项目方领取stETH奖励

    constructor(address _unamanoRegister, address _treasury, IERC20 _lpToken) {
    ...
    function addPool(uint256 _lockDays, uint256 _candyPerBlock,
        ...
        if(_lockDays > 0) {
           _unlockTime = _lockDays.mul(3600).add(block.timestamp);  //锁定的时间，单位秒 1d=86400 1h=3600
        }
        ...
        poolInfo.push(PoolInfo({
            ...
            endBlock: block.number + _candySupply.div(_candyPerBlock),
