...
contract TEST is ERC721A, Ownable{
    ...
    function mintWL(bytes32[] calldata proof) public payable {
        ...
        require(!_mintedAddress[msg.sender], "Already minted!"); // 地址没有mint过
        bytes32 leaf = keccak256(abi.encodePacked(msg.sender));//计算叶子hash
        bool verified = MerkleProof.verify(proof, merkleRoot, leaf);// Merkle检验
        require(verified, "Invalid merkle proof"); 
        
        _safeMint(msg.sender, 1); // mint
        _mintedAddress[msg.sender] = true; // 记录mint过的地址
    }
