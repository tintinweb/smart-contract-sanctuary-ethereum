...
contract EthPortal is IPortal, NonblockingLzApp, Auth {
    ...
    constructor(address _pow, address _endpoint) NonblockingLzApp(_endpoint) {
        POW = IERC20(_pow);
    }
    ...
    function bridge(
        ...
        (uint messageFee, ) = lzEndpoint.estimateFees(l2ChainId, address(this), payload, false, adapterParams);
        if (address(this).balance < messageFee) revert InsufficientBalance();

        // send LayerZero message
        _lzSend(
            messageFee, // gas to be paid out of this contract
...
abstract contract LzApp is Ownable, ILayerZeroReceiver, ILayerZeroUserApplicationConfig {
    ...
    function _lzSend(uint256 _messageFee, uint16 _dstChainId, bytes memory _payload, address payable _refundAddress, address _zroPaymentAddress, bytes memory _adapterParams) internal virtual {
        bytes memory trustedRemote = trustedRemoteLookup[_dstChainId];
        require(trustedRemote.length != 0, "LzApp: destination chain is not a trusted source");
        lzEndpoint.send{value: _messageFee}(_dstChainId, trustedRemote, _payload, _refundAddress, _zroPaymentAddress, _adapterParams);
    }
