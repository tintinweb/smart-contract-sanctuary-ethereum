...
contract TheVillage is ERC721r, Ownable {
    ...
    function _processTokenMinted() internal {
        uint[] memory tokens = _getTokenToProcess();

        for (uint i = 0; i < tokens.length; i++) {
            uint tokenId = tokens[i];
            if (tokenData[tokenId].tokenId == 0) {
                tokenData[tokenId].tokenId = tokenId;
                //TODO Update Mainnet supply
    ...
    function _getTokenToProcess() internal view returns (uint[] memory) {
        uint supply = maxSupply();
        uint[] memory tmp = new uint[](supply);
        uint index = 0;
