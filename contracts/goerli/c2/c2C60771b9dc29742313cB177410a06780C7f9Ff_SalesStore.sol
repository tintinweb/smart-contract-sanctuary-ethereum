...
interface ISalesItem is IERC165{
    ...
    function getConsumedAllocation(address _target, uint16 _currentSaleIndex) external view returns(uint16);
    function setConsumedAllocation(address _target, uint16 _currentSaleIndex, uint16 _consumed) external;
    function addConsumedAllocation(address _target, uint16 _currentSaleIndex, uint16 _consumed) external;
    function updateToken(uint256 tokenId) external;
}
