...
contract Sale {
    ...
    function exists(string memory _item) private view returns (bool) {
        for (uint i = 0; i < itemList.length; i++) {
            if (
                keccak256(abi.encodePacked(itemList[i])) ==
    ...
    function getInvoice(
        ...
        require(exists(_item), "Invalid Entry");
...
contract eInvoice {
    ...
    constructor(address _association, address _taxAuthority, address _vatTokenAddress) {
    ...
    function create(
        ...
        require(taxAuthority.isAddressRegistered(association), "Organisation not Registered with Tax Authority");
    ...
    function purchase() public payable {
        require(msg.value == invoice.totalPrice - invoice.vatToken, "Total Amount Mismatch");
        require(msg.sender == invoice.buyer, "Buyer Mismatch");
        require(
            vat.balanceOf(msg.sender) >= invoice.vatToken,
            "Insufficient Token Balance"
        );
        if (invoice.netVatAmount != 0) {
            vat.mint(invoice.seller, invoice.netVatAmount);
            sellerContract.deposit{value: invoice.totalPrice - invoice.netVatAmount}();
        }
        else {
            vat.transferFrom(msg.sender, address(this), invoice.vatToken);
            sellerContract.deposit{value: invoice.totalPrice - invoice.vatToken}();
        }
        invoice.paid = true;
    }
