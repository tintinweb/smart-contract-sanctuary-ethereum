...
// todo
// 1. ensure no funds are stored
// 2. ensure no assets are stored
// 3. ensure assets can be withdraw if accidentally stored
// 4. calculate and transfer schmint fee
// 5. check if user is eligible for free schmint
    ...
    SchedulerStorage
{
    ...
    function modifySchmint(
        ...
        // cannot modify schmint which is executed
        if (schmints[_schmintId].isSchminted) {
            revert AlreadyExecuted();
        }
        schmints[_schmintId].data = _newData;
        schmints[_schmintId].value = _newValue;
        schmints[_schmintId].gasPriceLimit = _gasPriceLimit;
    ...
    function cancelSchmint(uint256 _schmintId) external onlyOwner {
        ...
        IOps(ops).cancelTask(schmints[_schmintId].taskId);
...
contract AssetRecoverable {
    function _recoverERC721(address _erc721, address _receiver, uint256 _id) internal virtual {
        IERC721Upgradeable(_erc721).transferFrom(address(this), _receiver, _id);
    }

    function _recoverERC20(address _erc20, address _receiver, uint256 _amount) internal virtual {
        IERC20Upgradeable(_erc20).transfer(_receiver, _amount);
    }

    function _recoverERC1155(address _erc1155, address _receiver, uint256 _id, uint256 _amount) internal virtual {
        IERC1155Upgradeable(_erc1155).safeTransferFrom(address(this), _receiver, _id, _amount, "");
    }
}
