    ...
    IERC721Receiver
{
    // ==== Variables declaration ====
        ...
        uint256 public rewardETH; // ETH rewards
        ...
        uint256 private entriesTracker;
    ...
    function setRewardsETHPerWinner(uint256 _rewardETH) external onlyOwner {
    ...
    function pickWinners() public onlyOwner whenNotPaused {
        ...
        for(uint256 i = 0; i < numberOfWinners; i++) {
            require(numberOfWinners == rewardNFTs.length, "More winners than rewards");
            uint256 index = uint256(keccak256(abi.encodePacked(randomResult, i))) % entriesTracker; //generate random index 
            address payable winner = payable(participantByEntryIndex[index]); //identify winner
            if (prizeMode == 0){
                ...
                require(address(this).balance >= rewardETH, "Contract doesn't have enough ETH");
                winner.transfer(rewardETH); //transfer ETH to winner
                rewardSent++;
            }
