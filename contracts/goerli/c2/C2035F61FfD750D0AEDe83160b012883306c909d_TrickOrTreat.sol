...
/******************************************************************************\
* Trick or Treat
* An on-chain game of risk and luck by Michael Hirsch
...
* X winners: 1 Golly Ghost (The number of ghosts left will determine how many can win)
...
contract TrickOrTreat is ERC721Holder, ReentrancyGuard, Ownable, Pausable {
    ...
    function numberOfNFTsWonByTokenId(uint tokenId) public view returns (uint) {
        require(randomData != 0, "Winners have not been selected yet");
        uint position = ((tokenId + randomData) * PRIME_NUMBER) % allTokenIds.length;
        
        if (position == 0) {
            return allTokenIds.length > 20 ? 20 : allTokenIds.length;
        } else if (position < 11) {
            return allTokenIds.length - 20 >= 10 * position ? 10 : allTokenIds.length - 20 - (10 * position);
        } else if (position < 31) {
            return allTokenIds.length - 120 >= 5 * (position - 10) ? 5 : allTokenIds.length - 120 - (5 * (position - 10));
        } else if (allTokenIds.length - 220 > 0 && position < allTokenIds.length - 220 + 31) {
            return allTokenIds.length - 220 >= position - 30 ? 1 : 0;
        }
        return 0;
    }

    function numberOfNFTsWon(address entrant) external view returns (uint) {
        require(randomData != 0, "Winners have not been selected yet");
        
        uint numberWon;
        for (uint i = 0; i < tokensByOwner[entrant].length; i += 1) {
            numberWon += numberOfNFTsWonByTokenId(tokensByOwner[entrant][i]);
        }
    ...
    function enter(uint[] memory tokenIds) external nonReentrant whenNotPaused {
        require(randomData == 0, "Winners have already been selected");
        ...
        for (uint i = 0; i < tokenIds.length; i += 1) {
            ...
            if (tokensByOwner[msg.sender].length == 0) {
                entrants.push(msg.sender);
                tokensByOwner[msg.sender] = [tokenIds[i]];
            } else {
                tokensByOwner[msg.sender].push(tokenIds[i]);
            }
