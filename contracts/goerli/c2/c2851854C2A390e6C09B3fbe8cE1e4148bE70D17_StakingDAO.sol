...
contract StakingDAO is OwnableUpgradeable, ReentrancyGuardUpgradeable, IStakingDAO {
  ...
  function depositShares(
    ...
    /**
     * @dev Temporary disable for integration.
     * Production code:
  ...
  function depositValidator(
     ...
     */
    _getETHDeposit().deposit{value: 10 ** 18}(pubkey, withdrawal_credentials, signature, deposit_data_root); // replace this by production code above.
  ...
  function _setMinStakeAmount(uint256 amount) internal virtual {
    ...
    require(stakeableAmount % amount == 0, "StakingDAO: The maximal stakeable amount must be divisible by the minimum stake amount.");
  ...
  function _isStakeable() internal view returns (bool) {
     ...
     */
    return _totalAmount < VALIDATOR_AMOUNT; // replace this by production code above.
  }
  ...
  function _stake(address stakeholder, address payable recipient, uint32[] calldata operatorIds) internal virtual {
    ...
    require(amount >= _minStakeAmountRequired, "StakingDAO: stake amount under threshold.");

    if (_minStakeAmountRequired > 0) {
      require(amount % _minStakeAmountRequired == 0, "StakingDAO: The stake amount must be divisible by the minimum stake amount.");
    }
