...
contract Foo is ERC721A, Ownable {

  // Checks if your address has minted
  mapping(address => bool) public addressHasMinted;

  // Tracks the SplatId to the URI
  mapping(uint => string) private _transformations;

  // todo: getter for if an ID has been used
  ...
  bool mintEnabled = true; // todo, false
  bool splatEnabled = true; // todo: false
  ...
  event PunkSetter(uint indexed splatId, address indexed usingContractNFT, uint indexed tokenId);

  // Constructor

  // todo: add 'by us' below
  constructor() ERC721A("Foo", "FOO") {
    // _mint(msg.sender, 5); // todo: remove
  }
  ...
  // todo remove
  // just emits an event for testing
  function debug_transform(uint dittoId, address usingContractNFT, uint usingTokenId) external {
  ...
  function spluge(uint splatId, address usingContractNFT, uint usingTokenId) external {
