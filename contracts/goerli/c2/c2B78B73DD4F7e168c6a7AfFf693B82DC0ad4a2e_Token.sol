    ...
    UUPS
{
    ...
    bytes32 public constant ELECTION_COMMISSIONER_ROLE =
        keccak256("ELECTION_COMMISSIONER_ROLE");
    ...
    function initialize(AccessControl accessControlAddress, string memory uri_)
        public
        initializer
    {
        _paused = false;
    ...
    function delegate(
        ...
        address delegatee,
        uint256 tokenId
    ) external onlyRole(ELECTION_COMMISSIONER_ROLE) {
...
contract AccessControl is Initializable, ContextUpgradeable, UUPS {
    ...
    // address public constant ANY_MEMBER_GROUP_ADDRESS =
    ...
    modifier onlyRole(bytes32 role) {
        require(hasRole(role, _msgSender()), "AccessControl: not authorized");
        _;
    }
    ...
    function renounceRole(bytes32 role, address account) public {
        require(account == _msgSender(), "AccessControl: owner only");
        _roles[role].members[account] = 0;
        _roleMembers[role].remove(account);
        emit RoleSet(role, account, 0, _msgSender());
    }

    function setAdminRole(bytes32 role, bytes32 adminRole) public {
        bytes32 previousAdminRole = getAdminRole(role);
        require(
            hasRole(adminRole, _msgSender()),
        ...
        returns (address, uint256)
    {
        ...
        uint256 roleEnd = _roles[role].members[member];
