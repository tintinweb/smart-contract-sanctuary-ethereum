...
contract ConduitModule {
    ...
    constructor(address _conduitController) {
        ...
        (_CONDUIT_CREATION_CODE_HASH, ) = conduitController
...
import "@sushiswap/core/contracts/uniswapv2/interfaces/IUniswapV2Pair.sol";
import "./ConduitModule.sol";
import "./TransferModule.sol";
import { pairFor, sortTokens, getAmountsIn, getAmountsOut } from "../lib/LibSushi.sol";
import { TokenSource } from "../lib/LibFFRoomService.sol";

contract LegacySushiModule is TransferModule {
    ...
    ) TransferModule(_conduitController) {
        factory = _factory;
    ...
    function _legacySwapExactOut(
        ...
        require(
            ...
            "_legacySwapExactOut/EXCESSIVE_AMOUNT_IN"
        );

        transferERC20From(
            path[0],
            pairFor(factory, path[0], path[1], pairCodeHash),
            amountIn,
            tokenSource,
            transferData
        );
    ...
    function _legacySwapExactIn(
        ...
        require(
            ...
            "_legacySwapExactIn/EXCESSIVE_AMOUNT_OUT"
        );
    ...
    function _swap(
        ...
        for (uint256 i; i < path.length - 1; i++) {
            ...
            address to = i < path.length - 2
                ? pairFor(factory, output, path[i + 2], pairCodeHash)
                : _to;
...
contract TransferModule is ConduitModule {
    constructor(address _conduitController)
        ConduitModule(_conduitController)
    {}
    ...
    ///      FFRoomService contract or FFRoomService's Seaport Conduit.
    /// @param token        The ERC20 token to transfer.
    /// @param to           The recipient of the transfer.
    ...
    ///      FFRoomService contract or Seaport Conduit.
    /// @param token        The ERC721 token to transfer.
    /// @param to           The recipient of the transfer.
...
import "./LegacySushiModule.sol";

contract TransformationModule is LegacySushiModule {
    ...
    constructor(
        ...
        bytes32 _pairCodeHash,
        address _conduitController
    ) LegacySushiModule(_factory, _pairCodeHash, _conduitController) {
        WETH = _weth;
    }
    ...
    function unwrapNativeToken(uint256 amount, address payable to)
        public
        payable
    {
