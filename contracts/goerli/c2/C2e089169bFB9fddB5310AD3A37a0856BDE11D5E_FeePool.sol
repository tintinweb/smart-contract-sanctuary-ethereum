...
contract FeePool is Proxyable, LimitedSetup, MixinSystemSettings, IFeePool {
    ...
    struct FeePeriod {
        uint64 feePeriodId;
        uint64 startTime;
        uint256 allNetworksSnxBackedDebt;
    ...
    function resolverAddressesRequired() public view override returns (bytes32[] memory addresses) {
        ...
        newAddresses[7] = CONTRACT_COLLATERALMANAGER;
        newAddresses[8] = CONTRACT_ISSUED_SYNTH_AGGREGATOR;
        newAddresses[9] = CONTRACT_FUTURES_MARKET_MANAGER;
    ...
    function closeCurrentFeePeriod() external issuanceActive {
        ...
        (uint256 snxBackedDebt, ) = allNetworksSnxBackedDebt();
        ...
        ISynthetixBridgeToOptimism(
            resolver.requireAndGetAddress(CONTRACT_SYNTHETIX_BRIDGE_TO_OPTIMISM, "Missing contract: SynthetixBridgeToOptimism")
        ).closeFeePeriod(snxBackedDebt, debtSharesSupply);
    }

    function closeSecondary(uint256 allNetworksSnxBackedDebt, uint256 allNetworksDebtSharesSupply) external onlyRelayer {
     ...
     * @notice Close the current fee period and start a new one.
     */
    function _closeSecondary(uint256 allNetworksSnxBackedDebt, uint256 allNetworksDebtSharesSupply) internal {
     ...
     */
    function claimFees(bytes32 currencyKey) external issuanceActive optionalProxy returns (bool) {
     ...
     */
    function claimOnBehalf(address claimingForAddress, bytes32 currencyKey) external issuanceActive optionalProxy returns (bool) {
    ...
    function _claimFees(address claimingAddress, bytes32 currencyKey) internal returns (bool) {
        ...
        if (availableRewards > 0) {
            ...
            _payRewards(claimingAddress, currencyKey, rewardsPaid);
        }
     ...
     */
    function _recordRewardPayment(uint256 snxAmount) internal returns (uint256) {
        // Don't assign to the parameter
        uint256 remainingToAllocate = snxAmount;
    ...
    function _payRewards(
        ...
        rewardEscrowV2(currencyKey).appendVestingEntry(account, snxAmount, escrowDuration);
    }
    ...
    function feesByPeriod(address account) public view returns (uint256[2][FEE_PERIOD_LENGTH] memory results) {
        ...
        for (uint256 i = FEE_PERIOD_LENGTH - 1; i > 0; i--) {
            ...
            if (lastFeeWithdrawal < periodId) {
                userOwnershipPercentage = sds.sharePercentOnPeriod(account, uint256(periodId));
    ...
    function effectiveDebtRatioForPeriod(address account, uint256 period) external view returns (uint256) {
        ...
        return synthetixDebtShare().sharePercentOnPeriod(account, uint256(_recentFeePeriods[period].feePeriodId));
    }
