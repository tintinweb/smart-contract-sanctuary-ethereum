...
contract ERC20 is IERC20, ERC165, AccessControl {
    ...
    function transferFrom(address _from, address _to, uint _amount) external override returns (bool) {
        require(
            ...
            "The amount to withdraw is exceeded holder balance."
        );
        require(
            _amount <= allowance[_from][msg.sender],
            "You have no allowance for this amount"
        );
    ...
    function approve(address _to, uint _amount) external override returns (bool) {
        allowance[msg.sender][_to] = _amount;
        emit Approval(msg.sender, _to, _amount);
        return true;
    }

    function mint(address _to, uint _amount) external override {
        require(
    ...
    function burn(uint _amount) external override {
        require(
            ...
            "You are not the burner"
        );
        require(
            _amount <= balanceOf[msg.sender],
            "The amount to burn is exceeded your balance."
        );
    ...
    function burnFrom(address _from, uint _amount) external override {
        require(
            hasRole(BURNER_ROLE, msg.sender),
            "You are not the burner"
        );
        require(
            _amount <= balanceOf[_from],
            "The amount to burn is exceeded balance."
        );
