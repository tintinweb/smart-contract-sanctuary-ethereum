...
contract FarmReward is IFarmReward, Initializable, Pausable, Ownable {
    ...
    mapping(address => mapping(uint256 => Model.Checkpoint[])) public checkpointMap;
    EnumerableSet.AddressSet checkpointAddrSet;
    mapping(address => uint256[]) checkpointStakingIndexMap;
    ...
    function _claimAll(address userAddr) public whenNotPaused needInit {
        ...
        require(farmStaking.getCurrentStakingCount(userAddr) > 0, "FarmReward: staking count is 0");

        Model.StakingRecord[] memory stakingRecords = farmStaking.getStakingRecords(userAddr);

        for (uint256 index = 0; index < stakingRecords.length; index++) {
            // ignore unstaked record
            if (stakingRecords[index].status == Model.STAKING_STATUS_UNSTAKED) {
                continue;
            }

            _claim(userAddr, index, stakingRecords[index]);
        }
    }

    function claimAll() public whenNotPaused needInit claimLock {
        _claimAll(_msgSender());
    }

    function claim(uint256 stakingIndex) public whenNotPaused needInit claimLock {
        ...
        _claim(_msgSender(), stakingIndex, stakingRecords[stakingIndex]);
    }

    function proxyClaim(address userAddr, uint256 stakingIndex) public onlyFarm whenNotPaused needInit claimLock {
        // staking record
        IFarmStaking farmStaking = IFarmStaking(appConf.getFarmAddr().farmStakingAddr);
        Model.StakingRecord[] memory stakingRecords = farmStaking.getStakingRecords(userAddr);
        require(stakingRecords.length > stakingIndex, "FarmReward: index out of range");

        _claim(userAddr, stakingIndex, stakingRecords[stakingIndex]);
    }

    function _claim(address userAddr, uint256 stakingIndex, Model.StakingRecord memory stakingRecord) private {
        ...
        // calc rewardAmount
        uint256 rewardAmount = _calcStakingRewardAmount(userAddr, stakingIndex, stakingRecord);
        ...
        if (!checkpointAddrSet.contains(userAddr)) {
            ...
            for (uint256 index = 0; index < checkpointStakingIndexMap[userAddr].length; index++) {
                if (checkpointStakingIndexMap[userAddr][index] == stakingIndex) {
                    found = true;
                    break;
                }
            }
            ...
            if (!found) {
                checkpointStakingIndexMap[userAddr].push(stakingIndex);
            }
        }

        checkpointMap[userAddr][stakingIndex].push(Model.Checkpoint({
        ...
        emit Claim(userAddr, stakingIndex, rewardNftToken, rewardNftType, rewardNftTokenGen, rewardTokenIds, block.timestamp);
    }

    function _calcStakingRewardAmount(address userAddr, uint256 index, Model.StakingRecord memory stakingRecord) private view returns(uint256) {
        uint256 startBlock = stakingRecord.stakingBlockNumber;
        uint256 startTime = stakingRecord.stakingTime;

        Model.Checkpoint[] storage checkpoints = checkpointMap[userAddr][index];
        if (checkpoints.length > 0) {
            startBlock = checkpoints[checkpoints.length - 1].blockNumber;
            startTime = checkpoints[checkpoints.length - 1].timestamp;
        } else {
            startBlock = stakingRecord.stakingBlockNumber;
            startTime = stakingRecord.stakingTime;
        }

        // calc reward
        uint256 rewardAmount = 0;
        uint256 elapsedTime = block.timestamp - stakingRecord.stakingTime;
    ...
    function calcRewardAmount(address userAddr, uint256 stakingIndex) public view override needInit returns(uint256) {
        IFarmStaking farmStaking = IFarmStaking(appConf.getFarmAddr().farmStakingAddr);
        Model.StakingRecord[] memory stakingRecords = farmStaking.getStakingRecords(userAddr);
        require(stakingRecords.length > stakingIndex, "index out of range");

        uint256 rewardAmount = _calcStakingRewardAmount(userAddr, stakingIndex, stakingRecords[stakingIndex]);
        return rewardAmount;
    }

    function getCheckpointAddrs() public view override returns(address[] memory) {
    ...
    function importCheckpoints(address farmRewardAddr) public needInit onlyOwner {
    ...
    function importCheckpointsByUserAddr(address farmRewardAddr, address userAddr) public needInit onlyOwner {
    ...
    function _importCheckpointsByUserAddr(address farmRewardAddr, address userAddr) private {
        for (uint256 index = 0; index < checkpointStakingIndexMap[userAddr].length; index++) {
            uint256 stakingIndex = checkpointStakingIndexMap[userAddr][index];
            Model.Checkpoint[] memory checkpoints = IFarmReward(farmRewardAddr).getCheckpoints(userAddr, stakingIndex);

            if (checkpoints.length > 0) {
                // remove old data
                delete checkpointMap[userAddr][stakingIndex];
