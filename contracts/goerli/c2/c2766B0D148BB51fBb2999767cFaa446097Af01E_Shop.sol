...
contract Shop is Ownable, IShop {
  ...
  AggregatorV3Interface public priceFeed;
	...
	function getItemPriceInEth(uint256 itemId) public view returns (uint256) {
		uint256 itemPriceInEth = (getItemPriceInUsd(itemId)  * 10 ** 16) / uint256(getLatestPriceEthUsd());
	...
	function getTotalAmountInEth(uint256[] calldata itemIds, uint256[] calldata amounts) public view returns (uint256) {
		...
		// Eth is 18 decimals
		// Usd is 6 decimals
		// multiple with 10 ** (6 - 8 + 18) = 10**16 to convert totalAmount into Eth wei
		uint256 totalAmount = (getTotalAmountInUsd(itemIds, amounts) * 10 ** 16) / uint256(getLatestPriceEthUsd());
		return totalAmount;
	}
