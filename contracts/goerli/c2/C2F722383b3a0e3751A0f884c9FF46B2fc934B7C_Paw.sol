...
contract Paw is ERC20Burnable, Operator {
  using SafeMath8 for uint8;
  using SafeMath for uint256;

  // Initial distribution for the first 24h genesis pools
  uint256 public constant INITIAL_GENESIS_POOL_DISTRIBUTION = 11000 ether;
  // Initial distribution for the day 2-5 Paw-WFTM LP -> Paw pool
  uint256 public constant INITIAL_PAW_POOL_DISTRIBUTION = 140000 ether;
  ...
  /**
   * @notice Constructs the Paw ERC-20 contract.
   */
  constructor(uint256 _taxRate, address _taxCollectorAddress) ERC20("Paw", "Paw") {
    // Mints 1 Paw to contract creator for initial pool setup
    require(_taxRate < 10000, "tax equal or bigger to 100%");
    require(_taxCollectorAddress != address(0), "tax collector address must be non-zero address");

    excludeAddress(address(this));

    // TODO: Update the minting for deployer
  ...
  function _getPawPrice() internal view returns (uint256 _pawPrice) {
    try IOracle(pawOracle).consult(address(this), 1e18) returns (uint144 _price) {
      return uint256(_price);
    } catch {
      revert("Paw: failed to fetch Paw price from Oracle");
    }
  }

  function _updateTaxRate(uint256 _pawPrice) internal returns (uint256) {
    if (autoCalculateTax) {
      for (uint8 tierId = uint8(getTaxTiersTwapsCount()).sub(1); tierId >= 0; --tierId) {
        if (_pawPrice >= taxTiersTwaps[tierId]) {
  ...
  function setPawOracle(address _pawOracle) public onlyOperatorOrTaxOffice {
    require(_pawOracle != address(0), "oracle address cannot be 0 address");
  ...
  /**
   * @notice Operator mints Paw to a recipient
   * @param recipient_ The address of recipient
   * @param amount_ The amount of Paw to mint to
  ...
  function transferFrom(
    ...
    if (autoCalculateTax) {
      uint256 currentPawPrice = _getPawPrice();
      currentTaxRate = _updateTaxRate(currentPawPrice);
      if (currentPawPrice < burnThreshold) {
        burnTax = true;
      }
    }
  ...
  function distributeReward(
    ...
    require(_pawPool != address(0), "!_pawPool");
    require(_airdropWallet != address(0), "!_airdropWallet");
    rewardPoolDistributed = true;
    _mint(_genesisPool, INITIAL_GENESIS_POOL_DISTRIBUTION);
    _mint(_pawPool, INITIAL_PAW_POOL_DISTRIBUTION);
