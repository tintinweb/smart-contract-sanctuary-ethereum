...
contract Board is Initializable, UUPSUpgradeable, OwnableUpgradeable, PriceConsumerV3 {
    ...
    event OraclePriceChanged (address oldOracleAddress, address newOraclePrice);
    ...
    function changeFee(uint256 _fee) external {
        ...
        emit FeeChanged (fee, _fee);  ///to check
    }

    function changeOraclePrice(address _oracle) external onlyDAO {
        oracle = _oracle;

        emit OraclePriceChanged (_oracle, oracle);  ///to check
    }
