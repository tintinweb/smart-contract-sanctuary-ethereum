...
contract FakeArtMinter is Ownable{
    event FakeArtMinted(address sender, uint256 tokenId);
    uint256 constant COST_OF_ART = 0.002 ether;
    ...
    function validatePublicSale() internal returns (bool) {
        if (publicSale) {
            return true;
        } 
        return false;
    ...
    function mint() external payable {
        ...
        if (isPublic()) {
            require(validatePublicSale(), "Public sale not started yet");
        }
