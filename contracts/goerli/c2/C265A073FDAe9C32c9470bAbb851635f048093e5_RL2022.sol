...
contract RL2022 is ERC721Enumerable, Ownable {
    /*-----------------------------VARIABLES------------------------------*/
    enum SaleStates {
        CLOSED,
        PRESALE,
    ...
    uint256 public tokenIdBusinessTier = 1;
    uint256 public tokenIdProductiveTier = 500;
    uint256 public tokenIdIndividualTier = 2000;

    uint256 public productiveTierPrice = 2 ether; // 2000000000000000000 wei
    uint256 public individualTierPrice = 3.3 ether; // 3500000000000000000 wei
    uint256 public businessTierPrice = 7.5 ether; // 7500000000000000000 wei
    ...
    modifier checkMint(
        ...
        bool mintFlag,
        uint256 mintPrice,
        SaleStates currentSaleState
    ) {
        if (currentSaleState != saleState) revert InvalidSaleState();
    ...
    /*--------------------------CONSTRUCTOR-------------------------------*/
    constructor() ERC721("RL2022", "RL2022") {}

    /*--------------------------MINT FUNCTIONS----------------------------*/
    function mintProductiveTier(uint256 nMints)
        external
        payable
        checkMint(
            nMints,
    ...
    {
        mint(msg.sender, nMints, tokenIdProductiveTier);
        emit Minted(msg.sender, nMints);
    }

    // function mintIndividualTier(uint256 nMints)
    //     external
    //     payable
    //     checkMint(
    //         nMints,
    ...
    //             _mint(msg.sender, tokenIdIndividualTier++);
    //         }
    //     }
    //     emit Minted(msg.sender, nMints);
    // }

    // function mintBusinessTier(uint256 nMints)
    //     external
    //     payable
    //     checkMint(
    //         nMints,
    ...
    //     unchecked {
    //         for (uint256 i = 0; i < nMints; ++i) {
    //             _mint(msg.sender, tokenIdBusinessTier++);
    //         }
    //     }
    //     emit Minted(msg.sender, nMints);
    // }
    ...
    // function mintProductiveTierPresale(
    //     uint256 nMints,
    ...
    function setBusinessTierPrice(uint256 _price) external onlyOwner {
        businessTierPrice = _price;
    }

    function setIndividualTierPrice(uint256 _price) external onlyOwner {
        individualTierPrice = _price;
    }

    function setProductiveTierPrice(uint256 _price) external onlyOwner {
        productiveTierPrice = _price;
    }
    ...
    /// @dev 0 = CLOSED, 1 = RAFFLE, 2 = PUBLIC
    ...
    function withdraw(uint256 amount) external onlyOwner {
        _withdraw(address(0xD9db2f388BDC61C4f013452CAD5cd0845F22cC42), amount);
    }
    ...
    function withdrawSplit() external onlyOwner {
        ...
        _withdraw(
            address(0xD9db2f388BDC61C4f013452CAD5cd0845F22cC42),
            (contractBalance * 15) / 100
        );
        _withdraw(
            address(0xbC7693F4EeaB7DEafF3aD6CDD801aBCa283Fb4CF),
            (contractBalance * 15) / 100
        );
        _withdraw(
            address(0x69Be2B16673a74b0683e944eEfF154961d91c6Bd),
