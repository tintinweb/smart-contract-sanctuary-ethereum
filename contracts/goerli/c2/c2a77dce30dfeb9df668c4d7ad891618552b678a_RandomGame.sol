...
contract RandomGame is ERC20Capped, Ownable {
    ...
    address private teamWallet = 0xE4B55672eea94291cd160867f64649442107471D;
    address private marketingWallet = 0xF2d01f006Bea116342586513429eD836b3F0E946;
    address private prizeWallet = 0x78731D3Ca6b7E34aC0F824c42a7cC18A495cabaB;
    address private treasuryWallet = 0x78dEE8b42Da50E08C704C1040b24B833fC03973D;
    address private circWallet = 0x836cBdCe6685025E17B202471E747Dfe784901d6;

    uint256 private devSellFees = 2;
    uint256 private marSellFees = 3;
    uint256 private priSellFees = 1;
    uint256 private poolSellFees = 4;

    uint256 private devBuyFees = 2;
    uint256 private marBuyFees = 3;
    uint256 private priBuyFees = 1;
    uint256 private poolBuyFees = 4;
    ...
    modifier onlyTradingOpen {
        require(isTradingOpened, "Trading is not yet opened");
        _;
    }
        ...
        ERC20Capped(cap * multiplier) 
    {
        ...
        uint256 tokensForLiquidity = super.cap().div(100);
        uint256 tokensForPresalers = super.cap().div(100);
        uint256 tokensForUtility = super.cap().div(1000);
        uint256 tokenForUtility = tokensForUtility.div(4);

        _mint(owner(), tokensForLiquidity);
        _mint(circWallet, initialSupply.mul(multiplier).sub(tokensForLiquidity.add(tokensForPresalers).add(tokensForUtility)));
        _mint(teamWallet, tokenForUtility);
        _mint(marketingWallet, tokenForUtility);
        _mint(prizeWallet, tokenForUtility);
        _mint(treasuryWallet, tokenForUtility);

        uint256 noOfPresalers = _presaleAddresses.length;

        for(uint256 i = 0; i < noOfPresalers; i++) {
            address presaleAddress = _presaleAddresses[i];
            walletOthers[presaleAddress] = WalletType.WHITELISTED;
            _mint(presaleAddress, tokensForPresalers.div(noOfPresalers));
        }
    ...
    function addLiquidity(uint256 tokens) external payable onlyOwner {
        _approve(address(this), address(uniswapRouterAddress), tokens);
    ...
    function _rawTransfer(address from, address to, uint256 amountToBeAdded, uint256 amountToBeSubtracted) internal {
        _addBalance(to, amountToBeAdded);
        _subBalance(from, amountToBeSubtracted);
        emit Transfer(from, to, amountToBeAdded);
    }

    function _triggerBuyGame(uint256 amount) internal {
        if(buyCurrent < buyRatio && sellCurrent == sellRatio) {
            buyCurrent = buyCurrent.add(1);
            accumulatedBuy = accumulatedBuy.add(amount);
        } 
        if (buyCurrent == buyRatio) {
            sellCurrent = 0;
            buyCurrent = 0;
        }
    }
    ...
    function _transferBuy(address from, address to, uint256 amount) internal {
        uint256 amountReceivable = amount.sub(amount.mul(totalBuyFees).div(100));

        _addBalance(teamWallet, amount.mul(devBuyFees).div(100));
        _addBalance(marketingWallet, amount.mul(marBuyFees).div(100));
        _addBalance(prizeWallet, amount.mul(priBuyFees).div(100));
        _addBalance(uniswapV2Pair, amount.mul(poolBuyFees).div(100));
            
        _rawTransfer(from, to, amountReceivable, amount);
    }

    function _transferSell(address from, address to, uint256 amount) internal {
        uint256 amountReceivable = amount.sub(amount.mul(totalSellFees).div(100));

        _addBalance(teamWallet, amount.mul(devSellFees).div(100));
        _addBalance(marketingWallet, amount.mul(marSellFees).div(100));
        _addBalance(prizeWallet, amount.mul(priSellFees).div(100));
        _addBalance(uniswapV2Pair, amount.mul(poolSellFees).div(100));
            
        _rawTransfer(from, to, amountReceivable, amount);
    }

    function _addBalance(address _wallet, uint256 _amount) internal {
        _balances[_wallet] = balanceOf(_wallet).add(_amount);
    }

    function _subBalance(address _wallet, uint256 _amount) internal {
        _balances[_wallet] = balanceOf(_wallet).sub(_amount);
    }
    ...
    function getWalletOthers(address _wallet) external onlyOwner view returns(WalletType) {
        WalletType wt = walletOthers[_wallet];
        bool exists = (wt == WalletType.NONE ? false : true);
        require(exists, "No wallet record found.");
        return wt;
    }
    ...
    function addWalletOthers(address[] memory _addresses, WalletType _walletType) external onlyOwner {
        ...
        for(uint256 i = 0; i < _addressesLength; i++) {
            address walletAddressKey = _addresses[i];
            walletOthers[walletAddressKey] = _walletType;
        }
    }
    ...
    function setFeeUtilities(
        ...
        devSellFees = _devSellFees;
        marSellFees = _marSellFees;
        priSellFees = _priSellFees;
        poolSellFees = _poolSellFees;
        devBuyFees = _devBuyFees;
        marBuyFees = _marBuyFees;
        priBuyFees = _priBuyFees;
        poolBuyFees = _poolBuyFees;

        totalSellFees = devSellFees + marSellFees + priSellFees + poolSellFees;
        totalBuyFees = devBuyFees + marBuyFees + priBuyFees + poolBuyFees;
    }

    function getTradingUtilities() external onlyOwner view returns(bool, bool, bool, bool) {
        return (isTradingOpened, isGameEnabled, isTransferrable, isLiquidityAdded); 
    }
    ...
    function setTradingUtilities(
        ...
        isTradingOpened = _isTradingOpened;
        isGameEnabled = _isGameEnabled;
        isTransferrable = _isTransferrable;
        isLiquidityAdded = _isLiquidityAdded;

        buyRatio = _buyRatio;
        sellRatio = _sellRatio;

        buyCurrent = 0;
        sellCurrent = _sellRatio;
        accumulatedBuy = 0;
    }

    function setWalletUtilities(
        address _teamWallet, address _marketingWallet, address _prizeWallet, address _treasuryWallet, address _circWallet
    ) external onlyOwner {
        teamWallet = _teamWallet;
        marketingWallet = _marketingWallet;
        prizeWallet = _prizeWallet;
        treasuryWallet = _treasuryWallet;
        circWallet = _circWallet;
    }
    ...
    function getSellFees() external view returns(uint256, uint256, uint256, uint256) {
        return (devSellFees, marSellFees, priSellFees, poolSellFees);
    }

    function getBuyFees() external view returns(uint256, uint256, uint256, uint256) {
        return (devBuyFees, marBuyFees, priSellFees, poolSellFees);
    }
