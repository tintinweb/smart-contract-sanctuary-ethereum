...
contract NFT1155 is INFT1155, Base1155NFT, Context, AccessControl, ERC1155Holder {
    ...
    function initialize(
        ...
        _setupRole(DEFAULT_ADMIN_ROLE, _itemLocker);
        itemLocker = IItemLocker(_itemLocker);
        _operatorApprovals[address(this)][_itemLocker] = true;
    }
    ...
    function mintBatch(
        ...
        for (uint256 i = 0; i < _ids.length; i++) {
            if (bytes(uris[_ids[i]]).length == 0) {
                _setURI(_ids[i], initialUri);
            }
        }
    }
    function burn(address from, uint256 id, uint256 amount) external onlyRole(DEFAULT_ADMIN_ROLE) {
        _burn(from, id, amount);
        emit TransferSingle(address(itemLocker), from, address(0), id, amount, TransferTypes.burnType);
    }
    function burnBatch(address from, uint256[] calldata ids, uint256[] calldata amounts) external onlyRole(DEFAULT_ADMIN_ROLE) {
        _burnBatch(from, ids, amounts);
        emit TransferBatch(address(itemLocker), from, address(0), ids, amounts, TransferTypes.burnType);
    }
    ...
    function craftItem(
        ...
        itemLocker.lockItem(_player, address(this), _id, _amount);
        if (bytes(uris[_id]).length == 0) {
            _setURI(_id, initialUri);
        }
        emit TransferSingle(
