...
import "../modules/shared-services/interfaces/IAccessControl.sol";
...
abstract contract BaseUpgradeable is UUPSUpgradeable {
  ...
  modifier onlyOwner() {
    if (!IAccessControl(registry.getAccessControl()).isOwner(msg.sender)) {
  ...
  modifier onlyAdmin() {
    if (!IAccessControl(registry.getAccessControl()).isAdmin(msg.sender)) {
