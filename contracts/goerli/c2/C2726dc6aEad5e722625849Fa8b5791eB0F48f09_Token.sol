...
contract Token is Ownable, ERC721AQueryable, ReentrancyGuard, PaymentSplitter, OperatorFilterer {
    ...
    ERC721AQueryable public constant BrainPodToken = ERC721AQueryable(0xE897341005E359858670B63E71A12095bc769a08);
    ...
    PaymentSplitter(_payees, _shares) {
        ...
        _saleConfig.publicClaimStartTime = publicClaimStartTime;
    }
    ...
    function claimMint() external {
        uint256 _saleStartTime = uint256(_saleConfig.publicClaimStartTime);
        uint256[] memory tokenIds = BrainPodToken.tokensOfOwner(msg.sender);

        require(
            _saleStartTime != 0 && block.timestamp >= _saleStartTime,
            "Claim mint has not started yet"
        );

        require(
            tokenIds.length > 0,
            "Address holds no brain pods"
        );

        uint256 totalPodsAmount = tokenIds.length;

        require(
            totalSupply() + totalPodsAmount <= _totalAmount,
            "Exceeds maximum supply, can not mint this many biggies"
        );

        // burn existing biggies
        for(uint i = 0; i < tokenIds.length; i++) {
            BrainPodToken.burnToken(tokenIds[i]);
        }

        // mint biggies
        _safeMint(msg.sender, totalPodsAmount);
    }
     ...
     */
    function setPublicClaimStartTime(uint256 timestamp) external onlyOwner {
        _saleConfig.publicClaimStartTime = timestamp;
    }
