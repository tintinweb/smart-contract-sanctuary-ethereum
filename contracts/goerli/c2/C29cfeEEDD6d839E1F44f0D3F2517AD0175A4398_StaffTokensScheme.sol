...
contract StaffTokensScheme {
  ...
  /// @notice Add a new token to the scheme
  /// @param tokenSymbol 3 or 4 letter token 'ticker'
  /// @param token Ethereum address of the token contract
  /// @dev Required before any payments of this token can be made.
  function owner_registerToken(string calldata tokenSymbol, address token) external ownerOnly {
  ...
  /// @notice Remove an existing token from the scheme
  /// @param tokenSymbol 3 or 4 letter token 'ticker'
  /// @dev Prevents further payments for this token. Does not remove existing beneficiaries. Does not withdraw any token funds.
  function owner_revokeToken(string calldata tokenSymbol) external ownerOnly {
    bytes32 symbolHash = keccak256(abi.encode(tokenSymbol));
  ...
  /// @param tokenSymbol 3 or 4 letter token 'ticker' (must be a registered token)
  /// @param account Ethereum address of the beneficiary
  /// @param award Total award amount (in full decimals for the token)
  ...
  function admin_addBeneficiary(string calldata tokenSymbol, address account, uint256 award, uint256 numPayments, Date calldata startDate) external ownerOrAdminOnly {
    if (account == address(0)) revert AddressIsZero();
  ...
  function admin_removeBeneficiary(string calldata tokenSymbol, address account) external ownerOrAdminOnly {
    address token = getTokenAddress(tokenSymbol);
  ...
  /// @param tokenSymbol 3 or 4 letter token 'ticker'
  /// @param account Ethereum address of the beneficiary
  function admin_runSinglePayment(string calldata tokenSymbol, address account) external ownerOrAdminOnly {
    address token = getTokenAddress(tokenSymbol);
  ...
  function claim() external {
    ...
    for (uint256 i; i < tokenSymbolsLength;) {
      address token = getTokenAddress(TokenSymbols[i]);
  ...
  /// @notice Get the current surplus/deficit for a token in the scheme
  /// @param tokenSymbol 3 or 4 letter token 'ticker'
  ...
  function getTokenAddress(string memory tokenSymbol) private view returns (address token) {
    token = TokenAddress[keccak256(abi.encode(tokenSymbol))];
