...
contract Welcomy is ERC721URIStorage {
    ...
    function getNumberOfApartments() external view returns (uint256) {
        return apartments.length;
    }

    function getNumberOfReservations() external view returns (uint256) {
        return reservations.length;
    }

    function getCoordinates(
        uint256 _apartmentId
    ) external view returns (Coordinates memory) {
        require(_apartmentId < apartments.length, "Invalid apartment");
        return apartments[_apartmentId].coordinates;
    }
