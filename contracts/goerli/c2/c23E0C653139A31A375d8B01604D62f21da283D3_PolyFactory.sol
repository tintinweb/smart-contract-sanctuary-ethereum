 ...
 * @title PolyFactory
 * @author Ondo Finance
 * @notice This contract serves as a Factory for the upgradable poly token contract array.
 *         Upon calling `deployPoly` the `guardian` address (set in constructor) will
 *         deploy the following:
 *         1) Poly - The implementation contract, ERC20 contract with the initializer disabled
 ...
 *                                          `_logic' is set to `address(poly)`.
 *
 *         Following the above mentioned deployment, the address of the PolyFactory contract will:
 ...
 * @notice `deployPoly` is designed to be called once this factory is to be used once,
 *          e.g. throwaway contract.
 */

contract PolyFactory is IMulticall {
  bytes32 public constant MINTER_ROLE = keccak256("MINTER_ROLE");
  bytes32 public constant PAUSER_ROLE = keccak256("PAUSER_ROLE");
   ...
   *
   * @notice 1) Will automatically revoke all deployer roles
   *            granted to address(this)
   *         2) Will grant DEFAULT_ADMIN & PAUSER_ROLE(s) to
   *            `guardian` address specified in constructor.
   *         3) Will transfer ownership of the proxyAdmin to
   *            `guardian` address specified in constructor.
   *
   * @notice Guardian will need to grant Treasury: MINTER_ROLE
   */
  function deployPoly(string calldata name, string calldata ticker)
    external
    onlyGuardian
    returns (
      address,
    ...
    )
  {
    ...
    TokenProxy polyProxy =
      new TokenProxy(address(impl), address(polyProxyAdmin), "");
    Poly polyProxied = Poly(address(polyProxy));
    polyProxied.initialize(name, ticker);
    ...
    return (address(polyProxied), address(polyProxyAdmin), address(impl));
  }
   ...
   */
  event PolyDeployed(address proxy, address proxyAdmin, address implementation);
