...
contract RandomBox
{
    ...
    //*****************************************************************************************************************************************
    function createBox(string calldata boxName, string calldata comment) public {  // 뽑기 생성
        randomBoxs[boxName].creator = msg.sender;
        randomBoxs[boxName].name = boxName;
        randomBoxs[boxName].remainPercents = 1000;
        randomBoxs[boxName].comment = comment;
    }
    //*****************************************************************************************************************************************
    function setItem(string calldata boxName, string calldata name, uint percent, string calldata comment) public returns (bool) {
        require(msg.sender == randomBoxs[boxName].creator); // Only creator
        ...
        for (uint i=0; i<randomBoxs[boxName].items.length; i++)
            if (randomBoxs[boxName].items[i].percent == percent)
                    require(false, "Overlapped Percentage"); //[Fail]:중복된 아이템 확률

        for (uint i=0; i<randomBoxs[boxName].items.length; i++){
            if (keccak256(abi.encodePacked(randomBoxs[boxName].items[i].name)) == keccak256(abi.encodePacked(name))){
                randomBoxs[boxName].remainPercents += randomBoxs[boxName].items[i].percent;
                if ((randomBoxs[boxName].remainPercents - percent) <= 0) {
                    randomBoxs[boxName].remainPercents -= randomBoxs[boxName].items[i].percent;
                    require(false, "Percentage Overflow"); //[Fail]:퍼센트 초과
                }
                ...
                return true; //[Success]:아이템 수정
            }
        }

        // if Item ID doesn't exists
        require((randomBoxs[boxName].remainPercents - percent) >= 0, "Percentage Overflow"); //[Fail]:퍼센트 초과
        randomBoxs[boxName].items.push(Item(name, percent, comment));    // Add Item
        randomBoxs[boxName].remainPercents -= percent;
        return true; //[Success]:아이템 추가
    }
    //*****************************************************************************************************************************************
    function getItemNameByIndex(string calldata boxName, uint num) public view returns (string memory) {  // get 아이템 이름
        require(num <= randomBoxs[boxName].items.length, Strings.toString(getItemsLengthOfBox(boxName)));    // reason : Invalid item number.
        return randomBoxs[boxName].items[num].name;
    }

    function getItemPercentByName(string calldata boxName, string calldata name) public view returns (uint) {  // get 아이템 뽑기 확률
        for (uint i=0; i<randomBoxs[boxName].items.length; i++){
            if(keccak256(abi.encodePacked(randomBoxs[boxName].items[i].name)) == keccak256(abi.encodePacked(name)))
                return randomBoxs[boxName].items[i].percent;
        }
        return 0;
    }
    ...
    //*****************************************************************************************************************************************
    function openRandomBox(string calldata boxName, uint randNonce) public view returns (string memory) {
        require(keccak256(abi.encodePacked(randomBoxs[boxName].name)) == keccak256(abi.encodePacked(boxName)), "Unknown RandomBox");  // 뽑기 should exist
