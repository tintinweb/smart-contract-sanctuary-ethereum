...
contract OnChainNft is ERC721Enumerable, Ownable {
  ...
  string[] public wordValues = [
    "Kobe Bryant", "Kevin Durant", "LeBron James", "Kyrie Irving", "James Harden"
  ];
  // 物件
  struct Word{
    string name;
  ...
  function buildImage(uint256 _tokenId)public view returns(string memory){
      ...
      return Base64.encode(bytes(abi.encodePacked(
          ...
          '<rect height="500" width="500" stroke="#000" fill="hsl(',currentWord.bgHue,',50%,25%)"/>',
          '<text dominant-baseline="middle" text-anchor="middle" fill="hsl(',currentWord.textHue,',100%,80%)" font-size="24" y="50%" x="50%">',currentWord.value,'</text>',
          '</svg>'
      )));
  }
