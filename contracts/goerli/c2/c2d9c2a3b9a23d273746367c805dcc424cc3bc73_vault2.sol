...
import "contracts/interfaces/ILayer0.sol";
import "contracts/interfaces/ILayer1.sol";
import "contracts/interfaces/ITokenUri.sol";
import "hardhat/console.sol";
import "@openzeppelin/contracts/token/common/ERC2981.sol";

/**
 *  @dev layer1 = collection contract.
 *  traitId = tokenId in collection contract.
 */
struct Trait {
    uint256 layer1;
    uint256 traitId;
...
contract vault2 is Ownable, ReentrancyGuard, ERC2981 {
    //-----------------------------------
    //------------ Constants ------------
    //-----------------------------------
    ...
    uint256 constant MAX_PER_TX = 1000;
    ...
    ILayer1 public immutable genesisTraits;
    ...
    /// @dev tokenUri module (contract address casted to ITokenUri interface).
    ITokenUri public tokenUriContract;

    string public DNAImageUri = "";

    /// @dev saves in array every allowlisted contract. contractsArray[0] = genesisTraits
    address[] public contractsArray;

    /**
     *  @dev tokenId to array of Trait. This array has 20 postions and in each of them
     *  the information of an nft can be saved so that vaults know what nfts each tokenId has
     */
    mapping(uint256 => Trait[MAX_TRAITS]) public tokenIdToTrait;

    ///  @dev address of allowlisted contract to position in the array + 1. contractsMapping[genesisTraits] = 1.
    mapping(address => uint256) public contractsMapping;

    /// @dev tokenId to bool. Whether tokenId has claimed tokens or not.
    mapping(uint256 => bool) public traitsMinted;

    /// @dev emited traits are claimed
    event traitsClaimed(uint256 indexed tokenId);

    /// @dev emited traits are changed.
    event traitsChanged(
    ...
    /// @dev emited when new collection is allowlisted
    event contractAllowlisted(
        address indexed _contract,
        uint256 indexed number
    );

    /// @dev emited when collection is removed from allowlist
    ...
    constructor(
        ...
        genesisTraits = ILayer1(_genesisTraits);
    ...
    function setDNAuri(string memory _DNAImageUri) public onlyOwner {
        DNAImageUri = _DNAImageUri;
    }

    /**
     *  @notice allows owner to set tokenUriContract
     *  @dev tokenUriContract may change if we want to add new vaults or
     *  improve the way the metadata is returned onchain or offchain.
     *
     *  @param _tokenUriContract address of the new contract
     */
    ...
    function mint(uint256 amount) public payable nonReentrant {
        ...
        totalSupply += amount;
        ILayer0(regensZero).postGenesisMint(amount, _msgSender());
    }

    /**
     *  @notice allows token owner or controller to change the traits saved in tokenIdToTrait.
     *  this way owner or controller can change the traits being used by their NFT. With this function one
     *  is allowed to change the appearence of their NFT. Token must not be locked, traits should
     *  already be minted and the 3 array parameters must have the same lenght.
     *  @dev Parameters are translated to the following. For layer[x] (this will be the position changed in tokenIdToTrait)
     *  new tokenIdToTrait.layer1 will be contracts[x] and tokenIdToTrait.traitId will be newTraits[x]. Then function tranfers
     *  the old trait in the postion layers[x] of tokenIdToTrait array (if there was one stored)
     *  to tokenId owner and transfers (with transerSpecial, so that token doesn't need to be approver) the new trait to vault.
     *  This change, as described above, will be made if some requirements are met:
     *   - Collection must exist.
     *   - Owner or controller of tokenId must be owner of traitId.
     *   - traitId must be on correct position this means: in tokenIdToTrait the traitId of tokenIdToTrait.traitId must be in this ranges
     *     for each postion: [[1-999.999],[1.000.001-1.999.999],...,[19.000.001-19.999.999]].
     *  If all this requirements are met, changed[x] = 1, otherwise changed[x] = 0. After everything traitsChanged is emited
     *  contracts[x] = 0 and newTraits[x] = 0 means that owner or controller want to whitdraw trait on layers[x] but not add a new one.
     *
     *  @param tokenId tokenId that ig goint to get it's traits changed.
     *  @param layers postions on the array to change.
     *  @param contracts number of the collection that the new traits belong to
     *  @param newTraits traitId in collection of the new trait.
     *  @return changed An array of unit256 it indicates if the trait has been made. It has the same lenght as layers.
     */
    function changeTraits(
        ...
        require(
            ILayer0(regensZero).tokenIdDNA(tokenId) == address(this),
            "TraitVault: The DNA does not correspond to this vault"
        );
        require(
            ILayer0(regensZero).getTokenTimelock(tokenId) < block.timestamp,
        ...
        require(
            IERC721(regensZero).ownerOf(tokenId) == _msgSender() ||
                ILayer0(regensZero).getController(tokenId) == _msgSender(),
        ...
        ILayer0(regensZero).changeLastTraitModification(tokenId);
        ...
        for (uint256 i = 0; i < layers.length; i++) {
            ...
            if(
                (
                    ...
                    (
                        ...
                        (
                            IERC721(contractsArray[layerContract]).ownerOf(traitId) != owner
                        ) && (
                            IERC721(contractsArray[layerContract]).ownerOf(traitId) != _msgSender()
                        )
                    ) || (
            ...
            if (traitId > 0) {
                ILayer1(contractsArray[layerContract]).transferSpecial(
                    traitId,
                    owner
                );
            }
    ...
    /**
     *  @notice returns uri for tokenId (tokenUri)
     *  @dev rzTokenUri module handles the return value, tokenId must exist.
     */
