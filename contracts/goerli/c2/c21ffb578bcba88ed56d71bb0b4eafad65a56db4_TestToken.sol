    ...
    ReentrancyGuardUpgradeable
{
    ...
    function __CrossChainEnabledPolygonChild_init(address fxChild)
        internal
        onlyInitializing
    {
        __ReentrancyGuard_init_unchained();
        __CrossChainEnabledPolygonChild_init_unchained(fxChild);
    }

    function __CrossChainEnabledPolygonChild_init_unchained(address fxChild)
        internal
        onlyInitializing
    {
    ...
    function __functionDelegateCall(address target, bytes memory data)
        private
 ...
 * _Available since v4.6._
 */
abstract contract CrossChainEnabledAnyswapUpgradeable is
    Initializable,
...
{
    /// @custom:oz-upgrades-unsafe-allow state-variable-immutable
    // underlying variable. For our AnySwap frontend to recognize your token.
    // This should be set as address(0) for native anyswap support
    address public underlying;

    address private _sender;

    // https://docs.multichain.org/developer-guide/permissionless-token-bridging
    bytes32 public constant ANYSWAP_BRIDGE_ROLE = keccak256("ANYSWAP_BRIDGE_ROLE");

    function __CrossChainEnabledAnyswapUpgradeable_init(address router)
        internal
        onlyInitializing
    {
        __ERC20Burnable_init();
        __AccessControl_init();
        __CrossChainEnabledAnyswapUpgradeable_init_unchained(router);
    }

    function __CrossChainEnabledAnyswapUpgradeable_init_unchained(address router)
        internal
        onlyInitializing
    {
        underlying = address(0);
        _sender = 0x0000000000000000000000000000000000000000;
        if(router != address(0)) _grantRole(ANYSWAP_BRIDGE_ROLE, router);
    }
     ...
     */
    function _isCrossChain() internal view virtual override returns (bool) {
        return hasRole(ANYSWAP_BRIDGE_ROLE, _msgSender());
    }
...
contract TestToken is
    ...
    CrossChainEnabledAnyswapUpgradeable
{
    ...
    function initialize(address fxChild, address anyswapRouter) public initializer {
        __ERC20_init("Tester", "TEST");
        __ERC20Burnable_init();
        __Pausable_init();
        __AccessControl_init();
        ...
        __CrossChainEnabledPolygonChild_init(fxChild);
        __CrossChainEnabledAnyswapUpgradeable_init(anyswapRouter);
    ...
    function mint(address to, uint256 amount) public override {
        if (hasRole(MINTER_ROLE, _msgSender())) _mint(to, amount);
        else super.mint(to, amount);
    }
    ...
    function _crossChainSender()
        ...
        returns (address)
    {
        return super._crossChainSender();
    }
}