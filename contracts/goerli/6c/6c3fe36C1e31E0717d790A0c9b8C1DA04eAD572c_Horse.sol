...
contract Horse is Ownable, ERC721AWithRoyalties, Pausable, PaymentSplitter {
    uint256 _tokenIdCounter;
    address private skyFallsPyramidAddress;
    address private ExtMintAddress;
    address private AvatarAddress;
    ...
    function addTokenIDs(uint256 start, uint256 stop) public onlyOwner {
        for (uint256 i = start; i < stop; i++) {
            tokenIDs.push(i);
        }
    }

    function addRandNums(address to, uint256[] memory ids) public {
    ...
    function preMint(uint256 num) public onlyOwner {
        _safeMint(address(this), num);
    }

    function setRandomSeed(uint256 _randSeed) public {
        require(msg.sender == Index.getAddress("VRFV2"));
        randSeed = _randSeed;
    }
    ...
    function mintTransferReq(uint256[] memory ids) public whenNotPaused {
        refreshContracts();
    ...
    function mintTransferFul() public whenNotPaused {
        ...
        for (uint256 i = 0; i < ids.length; i++) {
            ...
            _transferFrom(
                address(this),
                msg.sender,
                tokenIDs[((randnums[i] % (ids.length - 1)) + 1)]
            );
            tokenIDs[((randnums[i] % (ids.length - 1)) + 1)] = tokenIDs[
                tokenIDs.length - 1
            ];
            tokenIDs.pop();
        }
    ...
    function freeMintVoS(address to, uint256 amount_req)
        external
        whenNotPaused
    {
        require(
    ...
    function checkStable(uint256 horseID) public view returns (uint256) {
        return stables[horseID].farmID;
    }

    function checkStableExp(uint256 horseID) public view returns (uint256) {
