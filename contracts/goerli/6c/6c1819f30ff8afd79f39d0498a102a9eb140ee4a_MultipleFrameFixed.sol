...
// File: contracts/MultipleFrameFixed.sol
...
contract MultipleFrameFixed is Ownable, ERC1155Holder {
	...
	event MultiItemListChanged(address collection, uint256 tokenId, uint256 pairId, uint256 pairBalance);
	...
	event MultiOfferCreated(Offer offer);
    event MultiOffersCreated(Offer[] offers);
    event MultiOfferCancelled(address collection, uint256 tokenId, uint256 offerId);
    event MultiOfferAccepted(address sender, Offer offer);
    event MultiLazyOfferAccepted(uint256 tokenId);
	...
	function onMakeOffer(address _collectionId, uint256 _tokenId, address tokenAdr, uint256 _offerPrice, uint256 _offerAmount ) external{
        require(tokenAdr != address(0x0), "Offer should be with wrapping token");
        IMultipleNFT nft = IMultipleNFT(_collectionId);
		...
		newOffer.offerAmount = _offerAmount;
        newOffer.active = true;
        offers[_collectionId][_tokenId].push(newOffer);
        emit MultiOfferCreated(newOffer);
    }
    ...
    function onCancelOffer(address _collectionId, uint256 _tokenId, uint256 _offerId ) external{
		...
		offer.offerAmount = 0;
        offer.active = false;
        emit MultiOfferCancelled(_collectionId, _tokenId, _offerId);
    }

    function onAcceptOffer(address _collectionId, uint256 _tokenId, uint256 _offerId, uint256 _offerAmount, uint256 _pairId) public {
        require(_offerId <= offers[_collectionId][_tokenId].length && offers[_collectionId][_tokenId][_offerId].offerId == _offerId, "Could not find offer");
        Offer memory offer = offers[_collectionId][_tokenId][_offerId];
        require(offer.active, "Not Exist");
        IMultipleNFT nft = IMultipleNFT(offer.collection);
        uint256 nft_token_balance = nft.balanceOf(msg.sender, offer.tokenId);
		require(nft_token_balance > 0, "Your owned amount is not enough");
        require(offer.tokenAdr != address(0x0), "Offer should be with wrapping token");

        uint256 feeAmount = offer.offerPrice.mul(_offerAmount).mul(feeAdmin).div(PERCENTS_DIVIDER);
		uint256 createrAmount = offer.offerPrice.mul(_offerAmount).mul(nft.royalties(_tokenId)).div(PERCENTS_DIVIDER);
		uint256 ownerAmount = offer.offerPrice.mul(_offerAmount).sub(feeAmount).sub(createrAmount);
        ...
        // transfer governance token to owner
        require(governanceToken.transferFrom(offer.owner, msg.sender, ownerAmount));

        nft.safeTransferFrom(msg.sender, offer.owner, offer.tokenId, _offerAmount, "buy from Marketplace");

        if(_pairId >= 0 && _pairId < 1e8){
            Pair storage pair = pairs[_pairId];
			
            if (pair.bValid){
				if (_offerAmount > (nft_token_balance - pair.balance))pair.balance = pair.balance - (_offerAmount - (nft_token_balance - pair.balance));
				if (pair.balance == 0){
					pair.bValid = false;
                	emit MultiItemDelisted(offer.collection, offer.tokenId, _pairId);
				}else{
					emit MultiItemListChanged(offer.collection, offer.tokenId, _pairId, pair.balance);
				}
            }
        }
        ...
        //     }
        // }
		offer.offerAmount = offer.offerAmount - _offerAmount;
		if (offer.offerAmount == 0){
        	offer.active = false;
		}
        emit MultiOfferAccepted(msg.sender, offer);
    }

    function onLazyAcceptOffer(LazyItem memory lazyItem, Offer[] memory _offers, uint256 _offerId) external returns (uint256){
        IMultipleNFT nft = IMultipleNFT(lazyItem.collection);
        uint256 tokenId = nft.addItem(lazyItem.supply, lazyItem.royalty, lazyItem.tokenURI, msg.sender, lazyItem.lazyTokenId, address(this));
        for (uint256 i = 0 ; i < _offers.length ; i++){
            _offers[i].tokenId = tokenId;
            offers[lazyItem.collection][tokenId].push(_offers[i]);
        }
        emit MultiOffersCreated(offers[lazyItem.collection][tokenId]);
        onAcceptOffer(lazyItem.collection, tokenId, _offerId, 1e8, 1e8);
        emit MultiLazyOfferAccepted(tokenId);
        return tokenId;
    }
