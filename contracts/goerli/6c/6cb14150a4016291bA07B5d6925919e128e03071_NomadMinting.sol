...
contract NomadMinting is Ownable {
  ...
  constructor(string memory _metaDataUri, bytes32 Node) {
    _uri = _metaDataUri;
    parentNode = Node;
  }
  ...
  IERC721Receiver
{
  ...
  string public constant name = "Nomad";
  ...
  modifier onlyAuthorized() {
    if (!authorized[msg.sender] || !(msg.sender == owner())) {
      revert Unauthorised("0x", msg.sender);
    }
    _;
  }
