...
contract AtlendisMining is Ownable {
    ...
    event RewardsRescued(address to, uint256[] rescued);
    ...
    // mining parameters
    address public immutable nft; // nft to stake into the contract
    address public immutable pools; // Atlendis main pools contract
    bytes32 public immutable poolHash; // target pool of the mining program

    // global state
    uint256 public totalDeposits = 0;

    // reward tokens
    address[] public tokens; // token rewards
    struct User {
        uint256 earned; // total token rewards for the user
        uint256 lastEarningsPerBaseDeposit; // last user update for the token
    }
    struct RewardToken {
        uint256 rate; // tokens distributed per second
        uint256 pendingRewards; // rewards to be claimed by stakers
        uint256 earningsPerBaseDeposit; // ratio to track the earnings per nft value unit
    ...
    mapping(uint256 => StakedNFT) public stakedNFTs; // nft centric data

    // user metrics
    mapping(address => uint256) public deposits; // underlying nft deposited value
    ...
    /**
     * @notice Get user data for a target token
     **/
    function getUserForToken(address token, address user)
        external
        view
        returns (uint256, uint256)
    {
        User memory _user = rewardTokens[token].users[user];
        return (_user.earned, _user.lastEarningsPerBaseDeposit);
    }

    /**
     * @notice Get updated rewards for target user and token
     **/
    function getRewardForUser(address _token, address _user)
        external
        view
        returns (uint256)
    {
        ...
        uint256 lastLiquidityRatio = user.lastEarningsPerBaseDeposit == 0
            ? 1e18
            : user.lastEarningsPerBaseDeposit;

        uint256 newEarningsPerBaseDeposit = token.earningsPerBaseDeposit;
        if (totalDeposits > 0) {
            uint256 newRewards = (block.timestamp - token.lastUpdate) *
                token.rate;
            uint256 contractTokenBalance = ERC20(_token).balanceOf(
                address(this)
            );
            if (contractTokenBalance < (newRewards + token.pendingRewards)) {
                newRewards = contractTokenBalance - token.pendingRewards;
            }
            newEarningsPerBaseDeposit =
                token.earningsPerBaseDeposit +
                (newRewards * 1e18) /
                totalDeposits;
        }
        ...
        return
            user.earned +
            (userDeposits * (newEarningsPerBaseDeposit - lastLiquidityRatio)) /
            1e18;
    }
    ...
    /**
     * @notice udpate all rewards data
     * modifier ran before each user action
     * tracks rewards at the contract level for all users as well as
     * the individual level for the caller
     **/
    modifier updateRewards(address from) {
        ...
        for (uint256 i = 0; i < tokens.length; i++) {
            ...
            if (totalDeposits > 0) {
                uint256 newRewards = (block.timestamp - token.lastUpdate) *
                    token.rate;
                uint256 contractTokenBalance = ERC20(tokens[i]).balanceOf(
                    address(this)
                );
                if (
                    contractTokenBalance < (newRewards + token.pendingRewards)
                ) {
                    newRewards = contractTokenBalance - token.pendingRewards;
                }
            ...
            uint256 lastEarningsPerBaseDeposit = user
                .lastEarningsPerBaseDeposit == 0
                ? 1e18
                : user.lastEarningsPerBaseDeposit;
            user.earned +=
                (userDeposits *
                    (token.earningsPerBaseDeposit -
                        lastEarningsPerBaseDeposit)) /
                1e18;
            user.lastEarningsPerBaseDeposit = token.earningsPerBaseDeposit;
        }
        _;
    }
     ...
     * The specified rate is the amount of tokens per second distributed to stakers
     **/
    function addToken(address token, uint256 rate) external onlyOwner {
        tokens.push(token);
        rewardTokens[token].rate = rate;
        rewardTokens[token].earningsPerBaseDeposit = 1e18;
    ...
    /**
     * @notice Get remaining rewards back
     * Under special circumstances - target pool is defaulted or closed - the owner
     * can call this function to get the remaining rewards from the contract
     * Only unclaimed rewards are taken off the contract, earned rewards can still be claimed
     * Rewards will stop accruing once this function is called
     **/
    function rescueRewards(address to)
        external
        onlyOwner
        updateRewards(_msgSender())
    {
        ...
        for (uint256 i = 0; i < tokens.length; i++) {
            toRescue[i] =
                ...
                rewardTokens[tokens[i]].pendingRewards;
    ...
    /**
     * @notice Stake an Atlendis position to get rewards
     * Send nft to the mining contract and start accruing token rewards
     * If a new token reward is added during staking, the nft will start accruing
     * the new rewards starting from the addition timestamp
     **/
    function stake(uint256 tokenId) external updateRewards(_msgSender()) {
        ...
        require(amount > 1e15, "Position amount too low");

        totalDeposits += amount;
        deposits[_msgSender()] += amount;
        stakedNFTs[tokenId] = StakedNFT({deposit: amount, owner: _msgSender()});
     ...
     * Sends back the staked nft and stop accruing rewards
     * Rewards have to be claimed separately
     **/
    function withdraw(uint256 tokenId) external updateRewards(_msgSender()) {
        require(
            _msgSender() == stakedNFTs[tokenId].owner,
            "Caller is not the owner of the target token"
        );

        totalDeposits -= stakedNFTs[tokenId].deposit;
        deposits[_msgSender()] -= stakedNFTs[tokenId].deposit;
     ...
     * Send accrued token rewards
     * Rewards for staked nfts won't stop accruing
     * Staked nfts have to be withdrawn separately
     **/
    function claim() external updateRewards(_msgSender()) {
        uint256[] memory toClaim = new uint256[](tokens.length);
        for (uint256 i = 0; i < tokens.length; i++) {
            RewardToken storage token = rewardTokens[tokens[i]];
            User storage user = token.users[_msgSender()];

            toClaim[i] = user.earned;
            token.pendingRewards -= user.earned;
            user.earned = 0;

            ERC20(tokens[i]).safeTransfer(_msgSender(), toClaim[i]);
        }

        emit Claim(_msgSender(), toClaim);
    }
}
...
contract MiningFactory {
    ...
    event MiningCreated(address contractAddress, bytes32 poolHash);
    ...
    address public immutable borrowerPools;
    ...
    constructor(address _positionManager, address _borrowerPools) {
     ...
     **/
    function deployNewMining(bytes32 id, bytes32 poolHash) external {
        require(
        ...
        emit MiningCreated(address(deployedMining), poolHash);
    }
}