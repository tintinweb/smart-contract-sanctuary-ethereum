...
contract HerosOfValindar is ERC721r, Ownable, ReentrancyGuard, ProxyRegistry {
    ...
    uint256 public maxMintsPerTX = 25;
  ...
  function publicMint(uint256 tokenAmount) public payable {
    ...
    require(userMintedTokens + tokenAmount <= mintedAllowedCap, "User Minting Amount Reached");
    require(tokenAmount > 0 && tokenAmount <= maxMintsPerTX, "Can't mint over Max Mints Per TX");
  ...
  function setParams(uint256 _manualDistCap, uint256 _mintedAllowedCap, uint256 _whitelistLimit, uint256 _maxMintsPerTX, uint256 _whitelistPrice, uint256 _publicPrice, string memory _secretImage,
  string memory _tokenBaseURI, string memory _newBaseExtension) external onlyOwner {
