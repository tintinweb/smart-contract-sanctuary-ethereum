...
contract SwaprFee is BaseGovernanceUpgradeable {
    ...
    function getBaseOrderFee(uint subjectAmount, address token) public view returns(uint baseFee) {
        baseFee = _getBaseFee(OrderFee, subjectAmount, token);
        if(baseFee > OrderFee.priceCap){
            baseFee = OrderFee.priceCap;
        }
    }
    ...
    /// @param token supported payment token
    /// @return baseFee base calculated fee
    function getBaseAuctionFee(uint subjectAmount, address token) public view returns(uint baseFee) {
        baseFee = _getBaseFee(AuctionFee, subjectAmount, token);
        if(baseFee > AuctionFee.priceCap){
            baseFee = AuctionFee.priceCap;
        }
    }
        ...
        if(isTokenSupported(paymentToken)){
            if(fee > 0){
                if(paymentToken == address(0)){
                    ...
                    fundToken.safeTransferFrom(_msgSender(), feeReceiver, allowance);
                }
    ...
    /// @return baseFee base calculated fee
    function _getBaseFee(FeePricing memory feePricing, uint subjectAmount, address token) internal view returns(uint baseFee) {
        
        require(feePricing.percentBaseFee <= EXP, "BIGGER_THAN_MAX");

        uint percentBaseFee = subjectAmount * feePricing.percentBaseFee / EXP;

        uint baseRecount = (feePricing.fixedBaseFee * BPS / _getPrice(oracleFeedForToken[token]) * BPS);
        uint fixedBaseFee = baseRecount * EXP / DPS;

        //uint fixedBaseFee = (feePricing.fixedBaseFee / _getPrice(oracleFeedForToken[token])) * EXP;

        if(feePricing.isFixedBaseFee && feePricing.isPercentBaseFee){
            baseFee = fixedBaseFee + percentBaseFee;
        }
        if(feePricing.isFixedBaseFee && !feePricing.isPercentBaseFee){
            baseFee = fixedBaseFee;
        }
        if(!feePricing.isFixedBaseFee && feePricing.isPercentBaseFee){
            baseFee = percentBaseFee;
        }
    ...
    function _calculatePriceCap(FeePricing memory feePricing, address token) internal view returns (uint priceCapTokens) {
        uint capRecount = (feePricing.priceCap * BPS / _getPrice(oracleFeedForToken[token]) * BPS);
        priceCapTokens = capRecount * EXP / DPS;
    }
...
contract SwaprGL is BaseGovernanceUpgradeable, ListingHelper {
    ...
    function getEXP(address acceptedToken) public view returns(uint exp){
        if(acceptedToken == address(0)){
            exp = EXP;
        }else{
            exp = 10 ** IERC20MetadataUpgradeable(acceptedToken).decimals();
        }
    }
    ...
    function createListing(uint listingType, bytes calldata data) external onlyMarketplace(data) {

        address sender = _msgSender();
        (bytes memory listingData,) = abi.decode(data, (bytes, bytes));
        (bool isValid, address lock, uint nftId, bytes memory sig, string memory res) = _validateListing(sender, listingType, listingData);
        require(isValid, res);
        swaprWallet.lockNFT(sig, lock, nftId, sender);

    }

    /// @notice Dispose the deposit record of the seller after successful listing creation or completion
    /// @dev Only proceeds with valid marketplace signature
    /// @param data contains the encoded string
    function disposeFeeRecord(bytes calldata data) external onlyMarketplace(data) {
        (bytes memory feeData,) = abi.decode(data, (bytes, bytes));
        swaprFee.disposeFeeRecord(feeData);
    }
    ...
    function updateListing(uint listingType, bytes calldata data) external onlyMarketplace(data) {
        ...
        require(block.timestamp - createdOn <= listingModTimeLimit, "MODIFICATION_TIME_EXCEEDED");
...
contract ListingHelper {
    ...
    function _validateListing(address sender, uint listingType, bytes memory listingData) internal view returns(bool isValid, address lock, uint nftId, bytes memory sig, string memory res) {
        ...
        if(_verify(sender, listingData)){
            ...
            if(listingType == 1){
                //Auction
                (isValid, lock, nftId, res) = _validateAuctionCapability(listingData);
            }else if(listingType == 2){
                //Order
                (isValid, lock, nftId, res) = _validateOrderCapability(listingData);
            }else{
    ...
    returns(
        ...
        uint tokenId, string memory err
    ){
        ...
        if(isValid){err = ""; lockAddr = auction.lock; tokenId = auction.nftId;}
    ...
    returns(
        bool isValid, address lockAddr, 
        uint tokenId, string memory res
    ){
        ...
        if(isValid){res = ""; lockAddr = order.lock; tokenId = order.nftId;}
