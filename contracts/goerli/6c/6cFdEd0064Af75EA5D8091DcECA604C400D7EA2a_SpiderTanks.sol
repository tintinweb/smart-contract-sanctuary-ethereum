...
contract SpiderTanks is IERC1155, ERC165, Pausable, CommonConstants, Ownable, IERC1155MetadataURI, UpdatableOperatorFilterer  {
    ...
    function broadcastURIEvent(uint256 _tokenId) public creatorOnly {
        ...
        if (isNonFungible(_tokenId)) {
            uint256 baseType = getNonFungibleBaseType(_tokenId);
            uint256 instanceId = getNonFungibleIndex(_tokenId);
            _uri = string(abi.encodePacked(baseURI, Strings.toString(baseType), "/", Strings.toString(instanceId)));
        } else {
            _uri = string(abi.encodePacked(baseURI, Strings.toString(_tokenId)));
        }   
