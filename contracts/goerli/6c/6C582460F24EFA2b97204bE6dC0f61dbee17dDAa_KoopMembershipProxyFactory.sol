...
contract KoopMembership is KoopCore, OwnableUpgradeable {
    ...
    */
    ...
    function setMintPrice(
        ...
        _setMintPrice(_tierId, _newPrice);
    }
    ...
    function mintTo(uint256 _tierId, uint256 _mintCount, address _destination) onlyOwner external nonReentrant {
        _mintTo(_tierId, _mintCount, _destination);
    }
...
contract KoopCore is Merkle, ERC721URIStorageUpgradeable, ERC2981Upgradeable, ReentrancyGuard {
    ...
    event MintPriceSet(uint256 tierId, uint256 price);
    ...
    */
    ...
    /** 
        @notice updates the mint price of a specified member badge tier
    */
    /// @param _tierId tier id to update
    ...
    function _setMintPrice(
        ...
        require(_tierId < memberBadgeTiers.length, "KoopCore::_setMintLimit(): Cannot set mint limit for a tier that does not exist");
        memberBadgeTiers[_tierId].price = _newPrice;
        emit MintPriceSet(_tierId, _newPrice);
    }
    ...
    */ 
    ...
    function mintMemberBadge(uint256 _tierId, uint256 _mintCount, bytes32[] calldata _merkleProof) external payable nonReentrant {
        MemberBadgeTier memory tier = _preMint(_tierId, _mintCount);
        ...
        _multiMint(tier, _mintCount, msg.sender);
    }

    /// @notice mints a number of tokens from a specified tier to the specified desintation.
    /// @param _tierId tier to mint tokens from
    /// @param _mintCount number of tokens ot mint
    /// @param _destination destination for tokens
    function _mintTo(uint256 _tierId, uint256 _mintCount, address _destination) internal {
        MemberBadgeTier memory tier = _preMint(_tierId, _mintCount);
        _multiMint(tier, _mintCount, _destination);
    }


    /// @notice premint checks to ensure a sale has begun and there is enough supply for the specified mint count
    /// @param _tierId tier to mint tokens from
    /// @param _mintCount number of tokens ot mint
    function _preMint(uint256 _tierId, uint256 _mintCount) internal returns (MemberBadgeTier memory) {
        require(memberBadgeTiers.length > 0, "Koop::preMint(): a crowdfund has not begun yet");
        require(_tierId < memberBadgeTiers.length, "Koop::preMint(): requested tier ID does not exist");
        require(_mintCount > 0, "Koop::preMint(): must mint at least 1 token");
        MemberBadgeTier memory tier = memberBadgeTiers[_tierId];
        require(tier.currentId + _mintCount - 1 <= tier.endingId, "Koop::preMint(): cannot mint more tokens than are left");
        return tier;
    }

    /// @notice internal function to handle logic for minting multiple tokens of a specified tier to a specified destination
    /// @param _tier instance of `Tier` to mint tokens from
    /// @param _mintCount number of tokens ot mint
    /// @param _destination destination for tokens
    function _multiMint(MemberBadgeTier memory _tier, uint256 _mintCount, address _destination) internal {
        uint256 mintId = _tier.currentId;
        memberBadgeTiers[_tier.id].currentId += _mintCount;

        bytes32 addressAndTierIndexHash = keccak256(abi.encodePacked(msg.sender, _tier.id));
        ...
        for(uint256 i = 0; i < _mintCount; i++) {
            mintedIds[i] = mintId;
            _safeMint(_destination, mintId);
            mintId++;
        }
        
        emit MemberBadgeMint(_destination, msg.value, _tier.id, mintedIds);
    }
