...
contract ScopeGuard is FactoryFriendly, BaseGuard {
    ...
    mapping(address => bool) public bannedOwners;

    /// @dev Set whether or not calls can be made to an address.
    /// @notice Only callable by owner.
    /// @param owner Address to be allowed/disallowed.
    /// @param allow Bool to allow (true) or disallow (false) calls to target.
    function setOwnerAllowed(address owner, bool allow) public onlyOwner {
        bannedOwners[owner] = allow;
    }

    /// @dev Returns bool to indicate if delegate calls are allowed to a target address.
    /// @param owner Address to check.
    function isOwnerAllowed(address owner)
        public
        view
        returns (bool)
    {
        return (bannedOwners[owner]) ? true : false;
    }
    ...
    function checkTransaction(
        ...
        require(!bannedOwners[sender], "Account is not allowed to perform transactions");
    }
