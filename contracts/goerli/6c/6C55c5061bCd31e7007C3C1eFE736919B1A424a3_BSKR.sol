...
contract BSKR is BaseBSKR {
    ...
    function __BSKR_init(
        ...
        for (uint8 index = 0; index < _sisterOAs.length; ++index) {
    ...
    //     require(_getsNoRfi[wallet], "Wallet is already excluded");
    //     for (uint256 index = 0; index < _noRfi.length; index++) {
    ...
    /**
     * Calculates the wallet balance taking into wallet reflection received
     */
    function balanceOf(address wallet) public view override returns (uint256) {
        if (_getsNoRfi[wallet]) return _balances[wallet];

        require(_rBalances[wallet] <= _rTotal, "BSKR: Amount too large");
        uint256 currentRate = _getRate();
    ...
    function giftReflection(uint256 tAmount) external whenNotPaused {
        ...
        require(!_getsNoRfi[sender], "BSKR: Excluded wallet");
...
// TODO do want to add reentrancy guard for any functions?
