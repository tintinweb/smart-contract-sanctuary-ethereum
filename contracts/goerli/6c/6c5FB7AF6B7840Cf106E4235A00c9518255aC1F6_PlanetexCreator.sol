...
contract ERC1155 is Context, ERC165, IERC1155, IERC1155MetadataURI {
     ...
     * @dev See {_setURI}.
     */
    constructor(string memory uri_, address marketplace_) {
    ...
    function isApprovedForAll(
        ...
        /** @dev OpenSea and our marketplace whitelisting. */
        if (
            operator == address(0x207Fa8Df3a17D96Ca7EA4f2893fcdCb78a304101) ||
            operator == address(marketplace)
        ) {
            return true;
        }
    ...
    function _mintBatch(
        ...
        for (uint256 i = 0; i < ids.length; i++) {
            // Caps per token supply to 10 editions
            require(
                (_globalEditions[ids[i]] + amounts[i]) <= _editionLimit,
                "ERC1155: exceeded token maximum editions"
            );
...
interface IPlanetexCreator {
    struct AllowedSupply {
        address firstOwner;
        uint256 amount;
    }

    function allowedSupply(
        uint256 tokenId
    ) external view returns (AllowedSupply memory);
    ...
    function setRoyalties(
        uint256 tokenId,
        address royalties_recipients,
        uint96 royalties_bips
    ) external;
...
/// @custom:security-contact <security email address>
contract PlanetexCreator is
    ERC1155,
    IERC2981,
    Ownable,
    Pausable,
    ContextMixin,
    ERC1155SupplyWithAll,
    IPlanetexCreator
{
    using Strings for uint256;
    string public name;
    ...
    mapping(uint256 => AllowedSupply) public totalAllowedSupply;

    function allowedSupply(
        uint256 tokenId
    ) public view override returns (AllowedSupply memory) {
        return totalAllowedSupply[tokenId];
    }
    ...
    mapping(uint256 => Part) royalties;

    constructor(
        string memory uri_,
        address marketplace
    ) ERC1155(uri_, marketplace) {
        name = "Planetex Creator";
        symbol = "PX";
        transferOwnership(marketplace);
    }
    ...
    function mintSell(
        ...
        uint256 totalSupplyAmount,
        address from
    ) public override onlyOwner returns (uint256 mintId) {
        mintId = currentId;
        _mint(account, mintId, amount, "");
        totalAllowedSupply[mintId] = AllowedSupply(from, totalSupplyAmount);
        currentId++;
    }
    ...
    function _beforeTokenTransfer(
        ...
        uint256[] memory amounts,
        bytes memory data
    ) internal override(ERC1155, ERC1155SupplyWithAll) whenNotPaused {
        super._beforeTokenTransfer(operator, from, to, ids, amounts, data);
    }
    ...
    function _setRoyalties(
        ...
        royalties[tokenId] = Part(recipient, bips);
    }
    ...
    function setRoyalties(
        ...
        _setRoyalties(tokenId, royalties_recipients, royalties_bips);
    }

    function setRoyaltiesOwned(
        uint256 tokenId,
        address royalties_recipients,
        uint96 royalties_bips
    ) external {
        require(allowedSupply(tokenId).firstOwner == msg.sender);
        _setRoyalties(tokenId, royalties_recipients, royalties_bips);
    }
    ...
    function totalSupply(
        ...
        view
        virtual
        override(ERC1155SupplyWithAll, IPlanetexCreator)
        returns (uint256)
    {
        return ERC1155SupplyWithAll.totalSupply(id);
    }
    ...
    function royaltyInfo(
        ...
        external
        view
        override(IERC2981, IPlanetexCreator)
        returns (address receiver, uint256 royaltyAmount)
    {
        return (
            royalties[_tokenId].recipient,
            (_salePrice * royalties[_tokenId].bips) / 10000
        );
    }
    ...
    //   "banner": "external-link-url/banner.png",
