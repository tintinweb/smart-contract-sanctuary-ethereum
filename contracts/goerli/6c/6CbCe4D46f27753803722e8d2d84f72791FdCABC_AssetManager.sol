...
contract AssetManager {
    ...
    function buy(string calldata name, uint amount) external payable {
        ...
        require(assets[name].creator != address(0), "Asset does not exist");
    ...
    function balanceOf(address account, string calldata name) external view returns (uint) {
        return holders[name][account];
    }
}