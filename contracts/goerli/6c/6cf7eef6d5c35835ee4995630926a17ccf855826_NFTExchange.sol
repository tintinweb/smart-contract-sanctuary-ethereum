...
contract NFTExchange is NFTExchangeLayout, ExchangeInterface {
    ...
    function onERC721Received(
        ...
        if (operator != address(this)) {
            ...
            if (order.token != msg.sender || order.tokenId != tokenId) revert TokenIsNotReceived();
            if (order.seller != from) revert InvalidSender();
            _sell(ItemType.ERC721, order, 1);
        }
    ...
    function onERC1155Received(
        ...
        if (value > type(uint8).max) revert OverflowMaxValue(type(uint8).max);

        if (operator != address(this)) {
            OriginOrder memory order = abi.decode(data, (OriginOrder));
            if (order.token != msg.sender || order.tokenId != id) revert TokenIsNotReceived();
            if (order.seller != from) revert InvalidSender();

            _sell(ItemType.ERC1155, order, uint8(value));
        }
    ...
    function allowVote(bytes32 orderHash) external view {
        ...
        if (_getStatus(o) != OrderStatus.Listing) {
            revert OrderIsNotListing();
        }
        if (votepoolCount(o.contextId) == 0) revert NotSupportVote();
    }
    ...
    function reopen(bytes32 oldOrderHash, OriginOrder calldata order) external override nonReentrant {
        ...
        _sell(oldOrder.nftType, order, oldOrder.amount);
    }
    ...
    function buy(bytes32 orderHash) external payable override nonReentrant {
        ...
        if (status != OrderStatus.Listing) revert OrderIsNotInListing();
        ...
        if (kind == OrderKind.FixedPrice) {
            if (offerPrice != reservePrice) revert OfferPriceNotEqualClosePrice();
            sold = true;
            ...
            if (offerPrice < _getMinAuctionPrice(order)) revert OfferPriceTooLow();
        ...
        // update last bid and return
        {
            ...
            // return last
            if (last != address(0)) _transferETH(payable(last), lastBid, true);
    ...
    function cancel(bytes32 orderHash) external override nonReentrant {
        ...
        if (msg.sender != order.seller) revert InvalidOrderOwner();
        if (order.lastBidder != address(0)) revert DisableCancelOrder();
        if (order.status != OrderStatus.Unknown) revert DisableCancelOrder();
        // disable cancel when order is expired.
        //slither-disable-next-line timestamp
        if (order.endTime < block.timestamp) revert DisableCancelOrder();

        VoteContext memory ctx = VoteContextCenter.version(order.contextId);
        for (uint256 i = 0; i < ctx.pools.length; i++) {
            if (VotePoolInterface(ctx.pools[i].pool).voted(orderHash)) revert DisableCancelOrder();
        }

        order.status = OrderStatus.Canceled;
        _transferNFTOut(order, order.seller);
    ...
    function _sell(
        ...
        if (order.seller == address(0)) revert MissingAddress();
        //slither-disable-next-line timestamp
        if (order.endTime <= block.timestamp) revert InvalidEndTime();
        if (order.startingPrice < MIN_STARTING_PRICE) revert InvalidStartingPrice();
        if (order.reservePrice > 0 && order.reservePrice < order.startingPrice) revert InvalidReservePrice();

        // Long lifetime can be set for fixed-price order without voting.
        {
            if (order.endTime > block.timestamp + MAX_ORDER_LIVE_TIME) {
                OrderKind kind = _getKind2(order.reservePrice, order.startingPrice);
                bool canVote = order.votePools.length > 0;
                if (kind != OrderKind.FixedPrice || canVote) {
        ...
        // emit
        emit NewOrder(order.seller, orderHash, order.externalData);
    }
    ...
    function _getStatus(Order storage order) internal view returns (OrderStatus) {
        ...
        if (order.status != OrderStatus.Unknown) return order.status;
        // check real status
        //slither-disable-next-line timestamp
    ...
    function _getKind(Order storage order) internal view returns (OrderKind) {
        ...
        return _getKind2(order.startingPrice, order.reservePrice);
    }

    function _getKind2(uint256 startingPrice, uint256 reservePrice) private pure returns (OrderKind) {
        if (reservePrice == 0) {
            return OrderKind.AuctionWithoutReserve;
        } else {
            return startingPrice == reservePrice ? OrderKind.FixedPrice : OrderKind.AuctionWithReserve;
        }
    }

    function _getMinAuctionPrice(Order storage order) internal view returns (uint256) {
        uint120 last = order.lastBid;
        return last == 0 ? order.startingPrice : (last + (last * MinQuoteIncreaseFactor) / FACTOR_DENOMINATOR);
    }
    ...
    function _sold(bytes32 orderHash) internal {
        ...
        {
            for (uint256 i = 0; i < ctx.pools.length; i++) {
                // should send rewards to voter if the voter has voted.
    ...
    function _transferNFTOut(Order storage order, address to) internal {
        if (order.nftType == ItemType.ERC721) {
            IERC721(order.token).transferFrom(address(this), to, order.tokenId);
        } else if (order.nftType == ItemType.ERC1155) {
            IERC1155(order.token).safeTransferFrom(address(this), to, order.tokenId, order.amount, "");
        } else {
            revert NotSupportedType();
        }
    }
    ...
    function _transferETH(
        ...
        if (to == address(0)) revert MissingAddress();
    ...
    function resetDefaultVoteContext(VoteContext calldata forFirst, VoteContext calldata forOther) external {
        if (msg.sender != config.owner()) revert InvalidSender();
...
contract VoteContextCenter {
    ...
    function _loadOrSave(bool forFirstBid, VotePoolMeta[] memory pools) internal returns (bytes32 id) {
        ...
        // check and save it.
        // if the voting pools is empty, it means voting is not supproted.
        if (pools.length > 0) {
    ...
    function votepoolCount(bytes32 id) internal view returns (uint256) {
        return _versions[id].pools.length;
    }
