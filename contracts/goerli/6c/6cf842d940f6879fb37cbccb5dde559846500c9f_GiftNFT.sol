...
contract GiftNFT is ERC721URIStorage, Ownable {
    ...
    function mint(address _to, address nftContractAddress, uint256 nftTokenId) public payable {
        require(msg.value >= COST, "Insufficient funds");
        require(MAX_SUPPLY >= _tokenIds.current(), "Max number has been minted");

        //increment tokenId - started at 0//
        _tokenIds.increment();
        _gifts[_tokenIds.current()] = Gift(msg.sender, nftContractAddress, nftTokenId);
    ...
    function exchangeGift(uint256 tokenId) public onlyOwnerOf(tokenId){
    ...
    function burnToken(uint256 tokenId) public onlyOwnerOf(tokenId) {
        _burn(tokenId);
    }
