...
contract SynthrIssuer is Owned, MixinSystemSettings, IIssuer {
    ...
    function _collateral(address account) internal view returns (uint256) {
        ...
        if (synthetixDebtShare().balanceOfCrossChain(account) > 0) {
            if (address(rewardEscrowV2()) != address(0)) {
                balance = balance.add(
                    rewardEscrowV2()
                        .balanceOfCrossChainInUSDC(account)
                        .multiplyDecimal(synthetixDebtShare().balanceOf(account))
                        .divideDecimal(synthetixDebtShare().balanceOfCrossChain(account))
                );
            }
            ...
            if (address(liquidatorRewards()) != address(0)) {
                balance = balance.add(
                    liquidatorRewards()
                        .earnedCrossChainInUSDC(account)
                        .multiplyDecimal(synthetixDebtShare().balanceOf(account))
                        .divideDecimal(synthetixDebtShare().balanceOfCrossChain(account))
                );
            }
