...
contract Test is ERC721A, Ownable {
    ...
    function mint(uint256 _amount) external payable {
        require(!paused, "Test - The Contract is Paused");
        require(
            totalSupply() + _amount <= maxSupply,
            "Test - max NFT limit exceeded"
        );
        if (msg.sender != owner()) {
            require(
                _amount <= maxMintAmount,
                "Test - max mint amount limit exceeded"
            );
            uint256 ownerMintedCount = addressMintedBalance[msg.sender];
            require(
                ownerMintedCount + _amount <= maxMintAmount,
                "Test - max NFT per address exceeded"
            );
            require(
                msg.value >= cost * _amount,
                "Test - insufficient ethers"
            );
        }
