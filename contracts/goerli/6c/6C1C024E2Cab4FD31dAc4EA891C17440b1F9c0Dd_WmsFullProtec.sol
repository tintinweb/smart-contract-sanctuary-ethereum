...
// File: contracts/WmsFullProtec.sol
...
interface IWmsToken {
    ...
    function fragmentToWms(uint256 value) external view returns (uint256);
    function wmsToFragment(uint256 wms) external view returns (uint256);
    function balanceOfUnderlying(address who) external view returns (uint256);
    function burn(uint256 amount) external;
}

contract WmsFullProtec is Ownable {
    ...
    constructor(IWmsToken _wms, uint256 _lockDuration, bool _depositsEnabled) {
        wms = _wms;
    ...
    function deposit(uint256 _amount) external {
        ...
        IERC20(address(wms)).safeTransferFrom(address(msg.sender), address(this), _amount);
        wms.burn(_amount);
    ...
    function withdraw(uint256 _amount) external {
        ...
        user.amount -= _amount;
        totalStaked -= _amount;
        wms.mint(address(msg.sender), _amount);
