...
contract RandomNFT is Ownable {
    ...
    uint256 private _maxAmountPart;
    ...
    constructor(uint256 maxAmountPart_, string memory secret_) {
        _maxAmountPart = maxAmountPart_;
        _signer = msg.sender;
        _secret = secret_;
    }

    function changeMaxPart(uint256 maxAmountPart_) external onlyOwner {
        _maxAmountPart = maxAmountPart_;
    }
    ...
    function makeLottery(
        ...
        lot.arrayPosInMeta = _meta.length;
    ...
    function closeLottery(uint64 id, address winner, uint256 inputTime, bytes memory _sig) external returns (bool) {
        address caller = msg.sender;
