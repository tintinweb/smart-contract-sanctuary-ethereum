...
contract DappCampNFT is ERC721Enumerable, Ownable {
    ...
    {
        string memory token = pluck(tokenId, "COLLECTION", collection);
        
        string memory imgPart = '<svg xmlns="http://www.w3.org/2000/svg" preserveAspectRatio="xMinYMin meet" viewBox="0 0 100 100"><rect width="100" height="100" fill="blue" /></svg>';
    
        string memory output = Base64.encode(bytes(string(abi.encodePacked('{"name": "', token, '", "description": "Bring colors to life.", "image": "data:image/svg+xml;base64,', Base64.encode(bytes(imgPart)), '"}'))));
