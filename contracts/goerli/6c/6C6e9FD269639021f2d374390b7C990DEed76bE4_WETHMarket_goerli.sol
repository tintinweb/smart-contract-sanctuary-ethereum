...
interface IMarketManagement {
    /// @notice updates market state by a provided algorithm
...
interface IRebalanceCalculation {
    ...
    function calculateRebalanceAmount(MarketState memory makerState, int Pclaim)
        external
        view
    ...
    IMarket
{
        ...
        returns (uint id)
    {
        ...
        ICalculation(calc).validateTakerPos(risk, term);

        // calculate floor price
        (int price, , ) = IState(state).price();
        uint floor = (((uint(price) * risk) / 10000) * DIVIDER_STABLE()) /
            DIVIDER_ORACLE;
        ...
        allTakerPositions[takerPositionCounter] = pos;
        EnumerableSet.add(userPositions[account].taker, takerPositionCounter);

        if (hook != address(0))
            IMarketCallback(hook).onProtected(takerPositionCounter);

        emit Protect(
            address(this),
            account,
            takerPositionCounter,
            floor,
            pos.assetAmount,
            risk,
            term,
        ...
        whenNotPaused
    {
        ...
        bool liquidation = posPremium >= pos.assetAmount;

        uint amountToReturn = pos.assetAmount - posPremium;
        Payout memory payout = IRebalancer(rebalancer).createTakerPayoutOnClose(
            address(this),
            amountToReturn
        );
        ...
        whenNotPaused
    {
        ...
        uint claimSizeInStable = ((pos.assetAmount - posPremium) * pos.floor) /
            DIVIDER_ASSET();

        console.log("CLaim in stable: %s", claimSizeInStable);

        Payout memory payout = IRebalancer(rebalancer).createTakerPayoutOnClaim(
            address(this),
            claimSizeInStable
        );
    ...
    function cancel(
        ...
        bool unwrap,
        address stable
    ) public virtual override onlyPermittedAdapter whenNotPaused {
        IState(state).update();
        ...
        uint amount = pos.assetAmount - posPremium;
        Payout memory payout = IRebalancer(rebalancer)
        ...
        returns (uint id)
    {
        ...
        ICalculation(calc).validateMakerPos(tier, term);
        ...
        allMakerPositions[makerPositionCounter] = pos;
        EnumerableSet.add(userPositions[account].maker, makerPositionCounter);

        if (hook != address(0))
            IMarketCallback(hook).onDeposited(makerPositionCounter);

        emit Deposit(
            address(this),
            account,
            makerPositionCounter,
            amount,
            tier,
            term,
            flags
...
import "../interfaces/IProtocolConfig.sol";
import "../interfaces/IMarketRiskStates.sol";
...
interface IRebalancer {
    ...
    /// @param deltaY Yield Target delta value delta
