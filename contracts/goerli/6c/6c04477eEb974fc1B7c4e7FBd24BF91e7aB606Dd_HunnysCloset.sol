...
contract HunnysCloset is ERC721A, Ownable {
    ...
    mapping (uint256 => bool) public hunnsCloned;

    constructor (string memory newBaseURI, address hunnysAddress) ERC721A ("Hunnys Closet", "HC") {
        setBaseURI(newBaseURI);

        // Deploy with Aurora Capsules contract address
        hu = Hunnys(hunnysAddress);
    }
    ...
    function clonExists(uint tokenId) public view returns(bool) {
        return hunnsCloned[tokenId];
    }

    function mintToken(uint256[] calldata _ownedHunnysTokens) public {
        for (uint256 i = 0; i < _ownedHunnysTokens.length; i++) {
            require(hu.ownerOf(_ownedHunnysTokens[i]) == msg.sender,                            "Require to be token holder");
            require(!hunnsCloned[_ownedHunnysTokens[i]],                                        "Clone for ID already exists");
        }

        for (uint256 i = 0; i < _ownedHunnysTokens.length; i++) {
            hunnsCloned[_ownedHunnysTokens[i]] = true;
        }

        _safeMint( msg.sender, _ownedHunnysTokens.length );
    }
    ...
    function withdrawTeam(uint256 amount) public payable onlyOwner {
        ...
        require(payable(a1).send(percent * 100)); // 100% 
    }
}
