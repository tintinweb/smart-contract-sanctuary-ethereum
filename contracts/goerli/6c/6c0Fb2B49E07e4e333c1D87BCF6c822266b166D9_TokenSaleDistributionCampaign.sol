...
contract TokenSaleDistributionCampaign is ClaimManager, DistributionCampaign {
  constructor(
    address _saleManager,
    bytes32 _saleId,
    address _claimToken
  ) ClaimManager(_saleManager, _saleId, _claimToken) {
  ...
  function isClaimed(uint256 index) public pure override returns (bool) {
    revert(
      "TokenSaleDistributionCampaign: This function signature is not supported. Use alternate isClaimed fn with address"
    );
  }
}
