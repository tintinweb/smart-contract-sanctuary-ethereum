...
/**
    TESTNET VERSION... NOT DROPPING UNTIL FEB 14.
    ...
    | '--'D|| '--'E|| '--'C|| '--'K| ((V)) '--'N|| '--'F|| '--'T|
    `------'`------'`------'`------'  '-'`------'`------'`------'
GLITCH DECK NFT is an art project by Jim Dee (@SwiggaJuice on Twitter).
It consists of 8,100 playing-card-themed, ANIMATED glitch-art NFTs with 
dynamic traits (meaning, when two NFTs have the same trait, they may
appear differently on each card, giving even more uniqueness to each
token than normal generative sets). Of the 8,100 tokens: 2,699 are
available free for any Deck of Degeneracy NFT holder (each DoD token may
claim one Glitch Deck token until Feb 14, 2024), 100 will be held back 
in the project wallet, and 5,301 are available as public mints.
For info see tinyurl.com/glitchNFTs or GlitchDeck.xyz. 
Thanks & good luck! --Jim from GenerativeNFTs.io.
*/
...
contract GLITCHDECK is Ownable, ERC721A, PaymentSplitter, DefaultOperatorFilterer {
    ...
    uint public maxSupply  = 8100;    // Hard-coded in setMaxSupply() also.
    uint public mintPrice  = 0 ether; // sb 0.042 on deploy (well, probably... tbd)
    uint public promoPrice = 0 ether; // sb 0.035 on deploy (well, probably... tbd)
    ...
    // Deployer wallet reserves the 2,699 claimable NFTs for one year.
    // After Feb 14, 2024, and left will be returned to the main supply
    ...
    // GEN1 ERC721Enumerable contract addrress:
    // DoD MAINNET: 0x8b3c69abc8c588e3eceb2eedf3bccf8fea950241
    // DoD GOERLI (temp): 0x6128aC22b13C03B98d150c2AdC775ACf137f57fE
    address public ERC721_contract = 0x6128aC22b13C03B98d150c2AdC775ACf137f57fE; // MAINNET HERE!
        ...
        PaymentSplitter(distro, distro_shares)
    {
        ...
        accounts[teamclaim[1]] = Account(  50, 0, 1); // Deployer wallet.
   ...
   // FREE CLAIM FUNCTIONALITY:
    ...
    function claimFreeNFTs() external payable noReentrant {
        ...
        uint numMapped = 0;
        uint claimLimit = 100;  // limit # free mints to 100/tx, just for good measure.
