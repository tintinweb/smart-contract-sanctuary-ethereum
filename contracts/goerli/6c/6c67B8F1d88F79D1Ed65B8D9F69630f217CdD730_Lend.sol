...
contract Lend is IERC721Receiver, ReentrancyGuard {
  ...
  event Mint(address _veNFTAddress, uint256 _nftId, uint256 _cheeTokenAmount, uint256 _lockDurationBlocks, uint256 _tokenAmount, address _minter);
  event Redeem(address _veNFTAddress, uint256 _nftId, uint256 _tokenAmount, address _redeemer);
  event Liquidate(address _veNFTAddress, uint256 _nftId, uint256 _tokenAmount, address _liquidator);
  ...
  function mint(
    ...
    emit Mint(_veNFTAddress, _nftId, _cheeTokenAmount, _lockDurationBlocks, _amount.toUint256(), msg.sender);
  }
  ...
  function redeem(address _veNFTAddress, uint256 _nftId) external nonReentrant {
    ...
    emit Redeem(_veNFTAddress, _nftId, _amount.toUint256(), msg.sender);
  }
  ...
  function liquidate(address _veNFTAddress, uint256 _nftId) external nonReentrant onlyPermissioned {
    ...
    IERC20(supportedInterestToken[_veNFTAddress]).safeTransferFrom(msg.sender, treasuryAddress, _total);
    IVeNFT(_veNFTAddress).safeTransferFrom(address(this), msg.sender, _nftId);
    emit Liquidate(_veNFTAddress, _nftId, _amount.toUint256(), msg.sender);
  }
