...
{   
    ...
    mapping(uint => mapping(uint => address)) public BrightListArtist;  // Returns Address Of Minting Receiver For Artist Mint 
    mapping(uint => mapping(uint => bool)) public MintedArtist;         // Returns Boolean If Artist MintPass ID Has Minted Or Not
    mapping(uint => mapping(uint => uint)) public MintedTokenID;
    mapping(uint => mapping(uint => uint)) public SelectedTimeslot;     // Returns Selected TimeSlot Corresponding To Artist Mint Pass

    mapping(address => bytes32) private Role;                           // BRT Minter Role Mapping
    mapping(uint => mapping(uint=>string)) public _Data;                // [ArtistID][TicketID] => Data
    ...
    /**
     * @dev Emitted When A New Artist Is Created
     */
    ...
    /**
     * @dev Emitted When Artist Contract Addresses Are Modified
     */
    event ArtistContractAddressesChanged(uint[] ArtistIDs, address[] NewAddresses);
    ...
    constructor()
    { 
        Artists[15]._MintPass = 0xB3Fe389025e3aFb92Cb010EB88cE39499B55160f;
        ...
        Role[msg.sender] = _MINTER_ROLE;                                      // BRT Minter #15
        // IERC20(0x989F573BfA0DF488C0806937E7Eb3FbeC4538218).approve(           // CryptoCitizens
    ...
    function _LiveMintArtist(uint ArtistID, uint[] calldata TicketIDs) external onlyMinter 
    {
        unchecked
        {
            for(uint TicketID; TicketID < TicketIDs.length; TicketID++)
            {
                ...
                MintedTokenID[ArtistID][TicketIDs[TicketID]] = MintedWorkTokenID;
    ...
    function _LiveMintArtistBatch(uint[] calldata ArtistIDs, uint[] calldata TicketIDs) external onlyMinter 
    {
        ...
        unchecked
        {
            for(uint Index; Index < TicketIDs.length; Index++)
            {
                ...
                MintedTokenID[ArtistIDs[Index]][TicketIDs[Index]] = MintedWorkTokenID;
     ...
     */
    function __ChangeArtistMintingAddresses(uint[] calldata ArtistIDs, address[] calldata NewAddresses) external onlyOwner
    {
        ...
        {
            address OldArtistContractAddress = Artists[ArtistIDs[ArtistID]]._Minter;
            Artists[ArtistIDs[ArtistID]]._MintPass = NewAddresses[ArtistID];
            emit ArtistContractAddressChanged(OldArtistContractAddress, NewAddresses[ArtistID]);
        }
    }
    ...
    function __ChangeArtistMaxQR(uint[] calldata ArtistIDs, uint[] calldata MaxQRIndex) external onlyOwner
    {
        for(uint x; x < ArtistIDs.length; x++)
        {
            Artists[ArtistIDs[x]]._MaxQR = MaxQRIndex[x];
        }
    }
    ...
    function __ChangeArtistCurrentQR(uint[] calldata ArtistIDs, uint[] calldata CurrentQRIndexes) external onlyOwner
    {
        for(uint x; x < ArtistIDs.length; x++)
        {
            Artists[ArtistIDs[x]]._CurrentQR = CurrentQRIndexes[x];
        }
    }
    ...
    function __ChangeArtistMintPasses(uint[] calldata ArtistIDs, address[] calldata NewAddresses) external onlyOwner
    {
        ...
        for(uint ArtistID; ArtistID < ArtistIDs.length; ArtistID++)
        {
            Artists[ArtistIDs[ArtistID]]._MintPass = NewAddresses[ArtistIDs[ArtistID]];
        }
        emit ArtistContractAddressesChanged(ArtistIDs, NewAddresses);
    }

    /**
     * @dev Modifies The Current ArtBlocks ProjectID
     **/
    function __ChangeArtBlocksProjectID(uint ArtistID, uint ArtBlocksProjectID) external onlyOwner 
    { 
        uint OldProjectID = Artists[ArtistID]._ProjectID;
        Artists[ArtistID]._ProjectID = ArtBlocksProjectID; 
        emit ModifiedProjectID(OldProjectID, Artists[ArtistID]._ProjectID);
    }

    /**
     * @dev Modifies ERC20 BRT Minting Token Address
     */
    function __ChangeERC20Address(uint ArtistID, address NewERC20Address) external onlyOwner
    {
        address OldAddress = Artists[ArtistID]._ERC20;
        Artists[ArtistID]._ERC20 = NewERC20Address;
        emit ERC20AddressChanged(OldAddress, Artists[ArtistID]._ERC20);
    }

    /**
     * @dev Overwrites QR Allocation
     */
    function __OverwriteQRAllocations(uint ArtistID, address[] calldata Addresses, uint[] calldata Amounts) external onlyOwner
    {
        require(Addresses.length == Amounts.length, "LiveMintArtist: Input Arrays Must Match");
        for(uint x; x < Addresses.length; x++)
        {
            QRAllocation[ArtistID][Addresses[x]] = Amounts[x];
        }
     ...
     */
    function __IncrementQRAllocations(uint ArtistID, address[] calldata Addresses, uint[] calldata Amounts) external onlyOwner
    {
        require(Addresses.length == Amounts.length, "LiveMintArtist: Input Arrays Must Match");
        for(uint x; x < Addresses.length; x++)
        {
     ...
     */
    function __NewRoot(uint ArtistID, bytes32 NewRoot) external onlyOwner
    {
        Artists[ArtistID]._Root = NewRoot;
        emit MerkleRootChanged(NewRoot);
    }
    ...
    { 
        return BrightListArtist[ArtistID][TicketID]; 
    }
    ...
    { 
        return SelectedTimeslot[ArtistID][TicketID]; 
    }
    ...
    //         {
    //             TimeSlotsBooked[x][y] = SelectedTimeslot[ArtistIDs[x]][y];
    //         }
    //     }
            ...
            for(uint TokenID; TokenID < Range; TokenID++)
            {
                if(
                    ...
                    (
                        ...
                        ||
                        IERC721(Artists[ArtistIDs[ArtistID]]._MintPass).ownerOf(TokenID) != address(0)
                    )
                ) 
                { 
