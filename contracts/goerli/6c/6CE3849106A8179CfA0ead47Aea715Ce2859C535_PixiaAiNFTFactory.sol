...
contract PixiaAiNFT is ERC721A {
    using Strings for uint256;
...
import "./PixiaAiNFT.sol";

interface ISingleNFT {
	function initialize(address creator, uint256 royaltyBasisPoints) external;
}

contract PixiaAiNFTFactory{
    address[] public collections;
	address factory;

	uint256 PRICE = 3 ether;
	uint256 royaltyBasisPoints = 300; //3%
	...
	function setPrice(uint256 _price) external {
		require(msg.sender == factory, "Admin can change the price.");
		PRICE = _price;
	}

	function setTaxFee(uint256 _fee) external {
		require(msg.sender == factory, "Admin can change the tax fee.");
		royaltyBasisPoints = _fee;
	}

	function createCollection(bool isFree) external payable returns(address collection)  {
		require(msg.value >= (isFree ? 0 : PRICE), "Small Amount");
		bytes memory bytecode = type(PixiaAiNFT).creationCode;
        bytes32 salt = keccak256(abi.encodePacked(msg.sender, block.timestamp));
        assembly {
            collection := create2(0, add(bytecode, 32), mload(bytecode), salt)
        }
        ISingleNFT(collection).initialize(msg.sender, isFree ? royaltyBasisPoints : 0);
