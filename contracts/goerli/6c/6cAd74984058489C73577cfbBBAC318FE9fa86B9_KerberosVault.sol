...
import "./Constants/ERCInterfaces.sol";
import "./Constants/StatusCodes.sol";
import "./Errors/CommonErrors.sol";
import "./Interfaces/IPawnTicket.sol";
import "./Structs/Loan.sol";
import "./Utils/NFTReceiver.sol";

contract KerberosVault is Ownable, ReentrancyGuard, NFTReceiver {
    ...
    /// @dev Declare Redemption of Settled Loan Event
    event Redeemed(
        uint256 _loanId,
        address _liquidator,
        address _nft,
    ...
    constructor(address benefic) {
        if(benefic == address(0)) { revert ZeroAddress(); }
        beneficiary = benefic;
        loaningOpen = true;
    ...
    function redeemDefaulted(uint256 loanId) external payable nonReentrant {
        ...
        loans[loanId].status = STATUS_CLOSED;
        ...
        emit Redeemed(loanId, _msgSender(), settledLoan.nftAddress, settledLoan.nftId);
    }

    /// @notice redeem your NFT Pawn Ticket for a settled loan
    /// @dev Loan must be settled in order to call
    /// @param loanId Loan ID #
    function redeemSettled(uint256 loanId) external nonReentrant {
        // Load Loan data
        Loan memory settledLoan = loans[loanId];
        // Loan must be settled, in order to call function
        if(settledLoan.status != STATUS_SETTLED) { revert InvalidStatus(); }

        loans[loanId].status = STATUS_CLOSED;

        // Only Pawn Ticket Holder
        address pawnTicketOwner = IERC721(pawnTicketContract).ownerOf(loanId);

        if(pawnTicketOwner != _msgSender()) { revert NotAuthorized(); }
        ...
        uint256 finalBill = (settledLoan.loanAmount + interest);
        // Take Payment, need NFT Pawn Ticket to claim 
        if(!IERC20(settledLoan.tokenAddress).transfer(_msgSender(), finalBill)){ revert InvalidAmount(); }

        emit Redeemed(loanId, _msgSender(), settledLoan.nftAddress, settledLoan.nftId);
    }

    /// @notice Settle an active loan by paying the amount plus interest
    /// @dev The Kerberos Vault will need to be approved for the specific ERC-20
    /// @param loanId Loan ID #
    function settleLoan(uint256 loanId) external nonReentrant {
        ...
        {
            loans[loanId].status = STATUS_DEFAULTED;
            revert LoanExpired();
        }
        ...
        // Loan is no longer active
        loans[loanId].status = STATUS_SETTLED;
        // Take Payment, need NFT Pawn Ticket to claim 
        if(!IERC20(settledLoan.tokenAddress).transferFrom(_msgSender(), address(this), finalBill - fee)) 
        { revert InvalidAmount(); }
        // Pay the Protocol Beneficiary 
        if(!IERC20(settledLoan.tokenAddress).transferFrom(_msgSender(), beneficiary, fee)) 
        { revert InvalidAmount(); }
    ...
    function upgradeContracts(
        ...
        if(newPawn == address(0) || newTicket == address(0)) { revert ZeroAddress(); }
...
bytes1 constant STATUS_SETTLED   = 0x02;
bytes1 constant STATUS_DEFAULTED = 0x03;
bytes1 constant STATUS_CLOSED    = 0x04;
