            ...
            payable(
                keccak256(
                    abi.encodePacked(
                        ...
                        keccak256(abi.encodePacked(type(Platform).creationCode, abi.encode(gaugeController)))
                    )
...
contract Platform is ReentrancyGuard {
    ...
    event Claimed(address indexed user, address indexed rewardToken, uint256 indexed bribeId, uint256 amount);

    event BribeDurationIncreaseQueued(
        uint256 id, uint8 numberOfPeriods, uint256 totalRewardAmount, uint256 totalRewardPerPeriod
    );
    ...
    function createBribe(
        ...
        unchecked {
            ...
            bribes[newBribeID] = Bribe({
                ...
                endTimestamp: (block.timestamp + (++numberOfPeriods * WEEK)) / WEEK * WEEK,
            ...
            activePeriodPerBribe[newBribeID] = Period(0, (block.timestamp + WEEK) / WEEK * WEEK, totalRewardPerPeriod);
    ...
    function _claim(address user, uint256 bribeId) internal nonReentrant returns (uint256 amount) {
        ...
        emit Claimed(user, rewardToken, bribeId, amount);
    }
    ...
    function claimable(address user, uint256 bribeId) external view returns (uint256 amount) {
        ...
        uint256 rewards;
        if (currentPeriod == bribe.endTimestamp) {
        ...
        // Distribute the min between the amount based on votes, and price.
        amount = min(amount, _amountWithMaxPrice);
        amount.mulWadDown(BASE_UNIT - factory.platformFee());
    }
    ...
    function getActivePeriodPerBribe(uint256 bribeId) public view returns (uint8) {
        ...
        return uint8(numberOfPeriods - periodsLeft);
    }
