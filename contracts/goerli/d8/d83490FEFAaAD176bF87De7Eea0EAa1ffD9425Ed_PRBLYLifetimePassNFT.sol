...
contract PRBLYLifetimePassNFT is ERC721, ERC721Burnable, ERC2981, Ownable {
    ...
    uint96 public royaltyPercent = 500; // denominator is 10000, so this is 5%
    ...
    function allowListMint(bytes32[] calldata proof) external payable {
        // only the address on the merkle tree can mint, no others - means tree can be public
        address account = _msgSender();
        require(allowListMintOpen == true, 'AllowListMint is not open');
        require(_claims[account] == false, 'Claimed Already');
        require(_verify(proof, _leaf(account)), 'Invalid Proof');
        require(msg.value >= allowListMintPrice, 'Ether value sent lower than allow list mint price');
        _mintOne(account);
