...
import {PoolLogic} from '../libs/logic/PoolLogic.sol';
import {ReserveLogic} from '../libs/logic/ReserveLogic.sol';
import {EModeLogic} from '../libs/logic/EModeLogic.sol';
import {SupplyLogic} from '../libs/logic/SupplyLogic.sol';
import {FlashLoanLogic} from '../libs/logic/FlashLoanLogic.sol';
import {BorrowLogic} from '../libs/logic/BorrowLogic.sol';
import {LiquidationLogic} from '../libs/logic/LiquidationLogic.sol';
import {DataTypes} from '../libs/types/DataTypes.sol';
import {BridgeLogic} from '../libs/logic/BridgeLogic.sol';
 ...
 * @title Pool contract
 * @author Monetaria
 * @notice Main point of interaction with an Monetaria protocol's market
 * - Users can:
 *   # Supply
 *   # Withdraw
 *   # Borrow
...
contract Pool is VersionedInitializable, PoolStorage, IPool {
  ...
  function MAX_NUMBER_RESERVES() public view virtual override returns (uint16) {
    return ReserveConfig.MAX_RESERVES_COUNT;
  }
  ...
  function finalizeTransfer(
    ...
    require(msg.sender == _reserves[asset].mTokenAddress, Errors.CALLER_NOT_MTOKEN);
 ...
 * @author Monetaria, inspired by the OpenZeppelin Initializable contract
...
library SupplyLogic {
  ...
  /**
   * @notice Implements the supply feature. Through `supply()`, users supply assets to the Monetaria protocol.
   ...
   * @param reservesList The addresses of all the active reserves
   * @param eModeCategories The config of all the efficiency mode categories
   * @param usersConfig The users config mapping that track the supplied/borrowed assets
  ...
  function executeFinalizeTransfer(
    ...
    mapping(uint256 => address) storage reservesList,
    mapping(uint8 => DataTypes.EModeCategory) storage eModeCategories,
    mapping(address => DataTypes.UserConfigMap) storage usersConfig,
    ...
    if (params.from != params.to && params.amount != 0) {
      DataTypes.UserConfigMap storage fromConfig = usersConfig[params.from];
      ...
      if (params.balanceToBefore == 0) {
        DataTypes.UserConfigMap storage toConfig = usersConfig[params.to];
        if (
          ValidationLogic.validateUseAsCollateral(
            reservesData,
            reservesList,
   ...
   * @param reservesList The addresses of all the active reserves
   * @param eModeCategories The config of all the efficiency mode categories
   * @param userConfig The users config mapping that track the supplied/borrowed assets
  ...
  function executeUseReserveAsCollateral(
    ...
    if (useAsCollateral) {
      require(
        ValidationLogic.validateUseAsCollateral(reservesData, reservesList, userConfig, reserveCache.reserveConfig),
...
library BorrowLogic {
   ...
   * Monetaria protocol proportionally to their collateralization power. For isolated positions, it also increases the
   * isolated debt.
...
import {Helpers} from '../../libs/helpers/Helpers.sol';
import {DataTypes} from '../../libs/types/DataTypes.sol';
import {ReserveLogic} from './ReserveLogic.sol';
import {ValidationLogic} from './ValidationLogic.sol';
import {GenericLogic} from './GenericLogic.sol';
...
library LiquidationLogic {
  ...
  function executeLiquidationCall(
    ...
    DataTypes.UserConfigMap storage userConfig = usersConfig[params.user];
    ...
    (
      vars.collateralMToken,
      vars.collateralPriceSource,
      vars.debtPriceSource,
      vars.liquidationBonus
    ) = _getConfigData(eModeCategories, collateralReserve, params);

    vars.userCollateralBalance = vars.collateralMToken.balanceOf(params.user);
    ...
    if (params.receiveMToken) {
      _liquidateMTokens(reservesData, reservesList, usersConfig, collateralReserve, params, vars);
    } else {
      _burnCollateralMTokens(collateralReserve, params, vars);
    }
    ...
    if (vars.liquidationProtocolFeeAmount != 0) {
      vars.collateralMToken.transferOnLiquidation(
        params.user,
        vars.collateralMToken.RESERVE_TREASURY_ADDRESS(),
        vars.liquidationProtocolFeeAmount
      );
    }
    ...
    emit LiquidationCall(
      ...
      vars.actualCollateralToLiquidate,
      msg.sender,
      params.receiveMToken
    );
  }

  /**
   * @notice Burns the collateral mTokens and transfers the underlying to the liquidator.
   * @dev   The function also updates the state and the interest rate of the collateral reserve.
   * @param collateralReserve The data of the collateral reserve
   * @param params The additional parameters needed to execute the liquidation function
   * @param vars The executeLiquidationCall() function local vars
   */
  function _burnCollateralMTokens(
  ...
  /**
   * @notice Liquidates the user mTokens by transferring them to the liquidator.
   * @dev   The function also checks the state of the liquidator and activates the mToken as collateral
  ...
  function _liquidateMTokens(
    ...
    uint256 liquidatorPreviousMTokenBalance = IERC20(vars.collateralMToken).balanceOf(msg.sender);
    vars.collateralMToken.transferOnLiquidation(
      params.user,
      msg.sender,
      vars.actualCollateralToLiquidate
    ...
    if (liquidatorPreviousMTokenBalance == 0) {
      DataTypes.UserConfigMap storage liquidatorConfig = usersConfig[msg.sender];
      if (
        ValidationLogic.validateUseAsCollateral(
          reservesData,
          reservesList,
  ...
  /**
   * @notice Returns the config data for the debt and the collateral reserves.
   * @param eModeCategories The config of all the efficiency mode categories
   * @param collateralReserve The data of the collateral reserve
   * @param params The additional parameters needed to execute the liquidation function
   * @return The collateral mToken
  ...
  {
    IMToken collateralMToken = IMToken(collateralReserve.mTokenAddress);
    ...
    if (params.userEModeCategory != 0) {
      ...
      if (
        EModeLogic.isInEModeCategory(
          params.userEModeCategory,
          collateralReserve.config.getEModeCategory()
        )
      ) {
    ...
    return (collateralMToken, collateralPriceSource, debtPriceSource, liquidationBonus);
  }
    ...
    )
  {
    ...
    vars.collateralDecimals = collateralReserve.config.getDecimals();
    vars.debtAssetDecimals = debtReserveCache.reserveConfig.getDecimals();
...
contract PoolStorage {
  ...
  // Map of users address and their Config data (userAddress => userConfig)
  mapping(address => DataTypes.UserConfigMap) internal _usersConfig;
