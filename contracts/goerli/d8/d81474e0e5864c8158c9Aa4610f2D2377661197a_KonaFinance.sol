...
contract KonaFinance is KonaOracle, AccessControl {
    ...
    function createLoan(uint256 _providerID, uint256 _loanID, uint256 _amount, address _borrower, uint256 _maturity, uint256 _repayments, uint256 _totalLocked, uint256 _apy, uint256[] memory _feeAmounts, address[] memory _feeWallets) public {
        ...
        for (uint i = 0; i < _feeAmounts.length; i++) {
            loan.fees.amounts.push(_feeAmounts[i]);
            loan.fees.wallets.push(_feeWallets[i]);
        }
    ...
    function repay(uint256 _amount, uint256 _providerID, uint256 _loanID) public {
        ...
        Loan memory loan = providers[_providerID].loans[_loanID];
        Fees memory fees = loan.fees;

        uint256 lenderTotal = _amount;

        for (uint i = 0; i < fees.amounts.length; i++) {
            uint256 total = _amount * fees.amounts[i] / 10000;
            feesToCollect[fees.wallets[i]] += total;
            lenderTotal -= total;
        }
    ...
    function getLoanFees(uint256 _providerID, uint256 _loanID) public view returns (uint256[] memory _amounts, address[] memory _wallets) {
        Loan memory loan = providers[_providerID].loans[_loanID];

        if (providers[_providerID].valid && loan.valid) {
            _amounts = loan.fees.amounts;
            _wallets = loan.fees.wallets;
        }
    }
