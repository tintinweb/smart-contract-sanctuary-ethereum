...
import "./MarketplaceFeeLogic.sol";
...
contract MarketplaceLotLogicWithFee is
    MarketplaceFeeLogic,
    Payout2981Support,
    ...
    ERC721Holder
{
    ...
    /// @dev Send contract eth fee pool to owner

    function unlockETH(address _to) external onlyOwner {
        _unlockETH(_to);
    }

    /// @dev Send contract token ERC-20 fee pool to owner

    function unlockTokens(IERC20 tokenAddress) external onlyOwner {
        _unlockTokens(tokenAddress);
    }
        ...
        nonReentrant
    {
        ...
        if (offer.saleToken == address(0)) {
            ...
            _topUpFee(offer.saleToken, amount);

            (bool sent, ) = payable(offer.creator).call{
                value: msg.value - _calculatePart(amount) - royaltyAmount
            }("");
...
contract MarketplaceFeeLogic {
    ...
    /// @dev Subtract percentage from user action
    ...
    function _unlockTokens(IERC20 tokenAddress) internal {
