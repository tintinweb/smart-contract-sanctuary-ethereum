...
import {ICrossChainForwarder} from 'ghost-crosschain-infra/contracts/interfaces/ICrossChainForwarder.sol';
import {GovernanceCore, CrossChainMandateUtils} from './GovernanceCore.sol';

contract Governance is GovernanceCore {
  address public immutable CROSS_CHAIN_MANAGER;
  uint256 public immutable EXECUTION_GAS_LIMIT;
  ...
  function _executePortal(CrossChainMandateUtils.Payload memory payload)
    ...
    override
  {
    ICrossChainForwarder(CROSS_CHAIN_MANAGER).forwardMessage(
      payload.chain,
      payload.mandateProvider,
      EXECUTION_GAS_LIMIT,
      abi.encode(payload.payloadId)
    );
  }
}
  ...
  OwnableWithGuardian
{
  ...
  function createProposal(
    ...
    IVotingPortal(votingPortal).forwardMessage(
      proposalId,
      blockHash,
    ...
    returns (State)
  {
    ...
    if (
      ...
      proposal.creationTime + PROPOSAL_EXPIRATION_TIME < block.timestamp
    ) {
...
interface IGovernanceCore {
  ...
  enum State {
    ...
    Created, // created, voting in progress
   ...
   * @param snapshotBlockHash blockHash of when the proposal was created, as to be able to get the correct balances on this specific block
   * @param ipfsHash ipfs has containing the proposal metadata information
   * @param blockNumber number of the block when the proposal was created
   */
