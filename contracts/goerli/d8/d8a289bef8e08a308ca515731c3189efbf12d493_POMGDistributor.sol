...
contract POMGDistributor is Ownable {
    using SafeMath for uint256;
	...
	uint private distributionTypeCount = 5; // 5 days
	uint256 public distributionStart = 1669438800; // Saturday, November 26, 2022 5:00:00 AM GMT
	...
	function addSwapTransaction(bytes32 txhash, address owner, uint256 amount) external onlySigner{		
		...
		uint256 claimCheckpoint = distributionStart + (((block.timestamp - distributionStart) / 86400) * 86400);
		if(block.timestamp < claimCheckpoint){
            claimCheckpoint = distributionStart;
        }
        claimCheckpoint -= 86400; //first claim = distribution start
	...
	function claimById(uint256 _swapId) public {		
		...
		if(pendingClaim > 0){
            ...
            uint256 claimCount = (timestamp - userSwap.claimCheckpoint) / 86400;
            uint256 claimCheckpoint = userSwap.claimCheckpoint + ( claimCount * 86400);
	...
	function pendingClaimById(uint256 _swapId) public view returns (uint256) {
		...
		if(userSwap.claimAmount - userSwap.claimedAmount > 0){
			uint256 pendingClaimDay = (timestamp - userSwap.claimCheckpoint) / 86400;
