...
import "./interfaces/IPriceOracleRouter.sol";
import "./interfaces/IPriceOracle.sol";

contract PriceOracleRouter is IPriceOracleRouter, BaseUpgradeable {
  // chain => oracle type => oracle
  /// @inheritdoc IPriceOracleRouter
  mapping(ChainId => mapping(OracleType => address)) public oracles;
  ...
  function getPrice(
    ...
    address oracleAddress = oracles[chainId][oracleType];
    if (oracleAddress != address(0)) {
      return IPriceOracle(oracleAddress).getPrice(a, b);
    }
  ...
  function setOracle(
    ...
    oracles[chainId][oracleType] = oracle;
  }
}
...
import "../modules/shared-services/interfaces/IAccessControlList.sol";
import "./libraries/ErrorCodes.sol";

abstract contract BaseUpgradeable is UUPSUpgradeable {
  ...
  modifier onlyOwner() {
    if (!IAccessControlList(registry.getAccessControl()).isOwner(msg.sender)) {
  ...
  modifier onlyAdmin() {
    if (!IAccessControlList(registry.getAccessControl()).isAdmin(msg.sender)) {
...
interface IPriceOracleRouter {
  ...
  /**
   * Return the address of the oracle for a given chain and oracle type
   * @param chainId The id of the chain
   * @param oracleType The type of the oracle
   * @return oracle The address of the oracle. Returns the zero address if not found
   */
  function oracles(ChainId chainId, OracleType oracleType) external view returns (address);

  /**
   * Return the price of token A, denominated in token B
   * @param chainId The id of the chain
   * @param oracleType The type of the oracle
  ...
  /**
   * Set the address of the oracle for the given chain and oracle type
   * @param chainId The id of the chain
   * @param oracleType The type of the oracle
