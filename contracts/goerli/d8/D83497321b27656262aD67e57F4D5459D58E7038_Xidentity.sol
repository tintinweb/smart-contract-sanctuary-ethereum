 ...
 * @author Luke Wei
 *
 * Xidentity Projects - Elemnt
 */
contract Xidentity is
    ERC2981,
    ERC4907A,
    Ownable,
    AccessControl,
    ...
    ERC721AQueryable
{
    ...
    event MaxSupplyChanged(uint256 indexed _maxSupply);
    ...
    modifier whenAllowlistActive() {
        ...
        require(
            mintConfig.mintStartTime != 0 &&
                mintConfig.mintStartTime <= block.timestamp,
            "Allowlist mintable is not active"
        );
        _;
    }
    ...
    // state of allowlist minting
    mapping(address => mapping(uint16 => uint16)) public allowlistMinted;
    // total amount of gold minted per wallet
    mapping(address => mapping(uint16 => uint16)) public publicMinted;
    ...
    constructor(
        ...
        _grantRole(AUCTION_ROLE, _owner);
    ...
    function allowlistMint(uint16 _amount, bytes32[] calldata _proof)
        external
        payable
        onlyEOA
        whenAllowlistActive
        nonReentrant
    {
        bytes32 leaf = keccak256(
            abi.encodePacked(_seriesCounter.current(), msg.sender)
        );
        require(
            MerkleProof.verify(
                _proof,
                merkleTreeRoots[_seriesCounter.current()],
                leaf
            ),
        ...
        require(
            allowlistMinted[msg.sender][uint16(_seriesCounter.current())] +
                _amount <=
                mintConfig.mintLimit,
            "Already minted"
        );

        allowlistMinted[msg.sender][
            uint16(_seriesCounter.current())
        ] += _amount;

        _mint(msg.sender, 1);

        refundIfOver(mintConfig.mintPrice * _amount);
    }

    function publicMint(uint16 _amount)
        external
        payable
        onlyEOA
        ...
        nonReentrant
    {
        require(totalSupply() + _amount <= maxSupply, "Reached max supply");
        require(
            publicMinted[msg.sender][uint16(_seriesCounter.current())] +
                _amount <=
                mintConfig.mintLimit,
            "Already minted"
        );

        publicMinted[msg.sender][uint16(_seriesCounter.current())] += _amount;
    ...
    function auctionMint(address _to, uint256 _amount)
        external
    ...
    function setupMintConfig(
        ...
        if (_isPublicMint) {
            ...
            mintConfig = MintConfig(false,_mintStartTime,_mintLimit,_mintPrice);
        }
        emit MintConfigUpdated(mintConfig);
    }

    function setPause() external onlyRole(MANAGER_ROLE) {
        isPause = !isPause;
        emit PausedStateUpdated(isPause);
    }

    function setMaxSupply(uint256 _maxSupply) external onlyRole(MANAGER_ROLE) {
        maxSupply = _maxSupply;
    }

    function setSeriesNumber(uint256 _series) external onlyRole(MANAGER_ROLE) {
        if (_seriesCounter.current() < _series) {
            _seriesCounter.increment();
        } else {
            _seriesCounter.decrement();
        }
    }

    function setMerkleRoot(uint256 _series, bytes32 _newRoot)
        external
        onlyRole(MANAGER_ROLE)
    {
        merkleTreeRoots[_series] = _newRoot;
    }
    ...
    function withdraw(address _valut)
        external
        onlyRole(MANAGER_ROLE)
        nonReentrant
    {
        uint256 balance = address(this).balance;
        payable(_valut).transfer(balance);
    }
    ...
    function supportsInterface(bytes4 interfaceId)
        ...
        view
        virtual
        override(ERC721A, ERC4907A, ERC2981, AccessControl)
        returns (bool)
    {
