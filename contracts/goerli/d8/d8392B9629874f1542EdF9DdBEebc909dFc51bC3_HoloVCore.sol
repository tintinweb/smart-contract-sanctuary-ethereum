    ...
    ERC721Pausable
{
    ...
    bytes32 public constant ASTRO_ENHANCER_ROLE =
        keccak256("ASTRO_ENHANCER_ROLE");
    ...
    event VFused(address indexed _owner, bytes32 indexed _id, uint256 _value);
        ...
        ERC721(tokenName, symbol)
    {
        ...
        _setupRole(ASTRO_ENHANCER_ROLE, _msgSender());
        _setupRole(ASTRO_MECHANIC_ROLE, _msgSender());
        _setupRole(ASTRO_BLACKSMITH_ROLE, _msgSender());
    }
    ...
    ) public virtual onlyAstroEnhancer {
    ...
    function vForge(
        ...
        _burn(_fuser1ID);
        _burn(_fuser2ID);
        _holoProject(_fuser, _neoHoloVLazulith);
        if (_burnVlxAmt > 0) {
            _burnVlx(_burnVlxAmt, _fuser);
        }
    }
    ...
    function _burnVlx(uint256 _burnVlxAmt, address _fuser) internal {
        IVelox velox = IVelox(veloxAddress);
        velox.burn(_burnVlxAmt, _fuser);
    }
    ...
    modifier onlyAstroEnhancer() {
        require(
            hasRole(ASTRO_ENHANCER_ROLE, _msgSender()),
            "HOLOV: must be an Astro Enhancer to Enhance"
        );
        _;
    }
    modifier onlyAstroMechanic() {
        require(
            ...
            "HOLOV: must be an Astro Mechanic to VFuse"
        );
        _;
    }
    modifier onlyAstroBlacksmith() {
        require(
            ...
            "HOLOV: must be an Astro Blacksmith to Forge"
        );
        _;
    }
}
...
contract Velox is ERC20, AccessControl {
    ...
    function burn(uint256 amount, address caller) public {
        _burn(caller, amount);
    }
