    ...
    OperatorFilterer
{
    ...
    address private _thePassAddress;
    mapping(uint256 => uint256) private _passTypeIdForTokenId;
    bool private _redeemActive;
    ...
    constructor(
        ...
        uint96 royaltyFee_,
        string memory uri_,
        address thePassAddress_
    ) ERC721A(name_, symbol_) {
        _grantRole(DEFAULT_ADMIN_ROLE, _msgSender());
        _setDefaultRoyalty(royalty_, royaltyFee_);
        _thePassAddress = thePassAddress_;
        _tokenBaseURI = uri_;
        _registerForOperatorFiltering();
        operatorFilteringEnabled = true;
    }

    function redeem(uint256 passTypeId) external {
        IThePass thePass = IThePass(_thePassAddress);
        IERC1155 erc1155 = IERC1155(_thePassAddress);

        require(_redeemActive, "Redemption is not active");
        require(erc1155.balanceOf(_msgSender(), passTypeId) > 0, "No pass");

        thePass.burn(passTypeId, _msgSender(), 1);
        _mint(_msgSender(), 1);
        _passTypeIdForTokenId[totalSupply()] = passTypeId;
    }
    ...
    function thePassAddress() external view returns (address) {
        return address(_thePassAddress);
    }

    function redeemActive() external view returns (bool) {
        return _redeemActive;
    }

    function setThePassAddress(
        address thePassAddress_
    ) external onlyRole(DEFAULT_ADMIN_ROLE) {
        _thePassAddress = thePassAddress_;
    }
    ...
    function setRedeemActive(
        bool isRedeemActive_
    ) external onlyRole(DEFAULT_ADMIN_ROLE) {
        _redeemActive = isRedeemActive_;
    }
        ...
        return
            bytes(baseURI).length != 0
                ? string(
                    abi.encodePacked(
                        baseURI,
                        _toString(_passTypeIdForTokenId[tokenId])
                    )
                )
                : "";
    }
