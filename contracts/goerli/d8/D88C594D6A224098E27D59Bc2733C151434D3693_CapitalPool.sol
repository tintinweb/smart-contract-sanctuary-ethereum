...
contract CapitalPool is ICapitalPool, OwnableUpgradeable, AbstractDependant {
    ...
    function addWithdrawalHardSTBL(uint256 _stblAmount) external override onlyYieldGenerator {
        hardUsdtAccumulatedBalance = hardUsdtAccumulatedBalance.add(_stblAmount);
    }
        ...
        returns (uint256 _requiredLiquidity)
    {
        ...
        _requiredLiquidity = _pendingWithdrawlAmount
            .add(_pendingClaimAmount)
            .add(_pendingRewardAmount)
            .add(_pendingWithdrawalDemandAmount);
    }
    ...
    function preventAttack() public onlyMaintainer {
        attack = true;
    }

    /// @param _attack bool that prevent transfer out of too much funds
    function setAttack(bool _attack) public onlyOwner {
        attack = _attack;
    }
    ...
    function defiHardRebalancing() public onlyOwner {
        ...
        if (_lostAmount > 0 && _totalDeposit > _lostAmount) {
            ...
            demandBook.updateLostInDefi(_lostPercentage);
    ...
    function fundClaim(
        ...
        _actualAmount = _withdrawFromLiquidityCushion(claimProvenance.claimer, _stblClaimAmount);
    ...
    function _withdrawFromLiquidityCushion(address _sender, uint256 _stblAmount)
        ...
        returns (uint256 _actualAmount)
    {
        require(!attack, "CP: Security from attack");
...
interface IDemandBookLiquidity {
    ...
    function transferFundsAtValidation(uint256 _liquidityAmount, uint256 _reinsurancePoolAmount)
        external;
