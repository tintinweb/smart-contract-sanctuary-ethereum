...
abstract contract BaseERC20 is Context, IERC20, Ownable, AccessControl {
    ...
    function setMaxTxPercent(uint16 maxTxBips) external onlyRole(MANAGER_ROLE) {
...
/**
 * @title LBSKR - Light(easily) Brings of Serenity, Knowledge and Richness
...
contract LBSKR is BaseBSKR, Stakeable {
    using Address for address;
    ...
    constructor(
        ...
        _tBalance[address(this)] = _TOTAL_SUPPLY / 2;
    ...
    // TODO add description
    function excludeFromFee(address account) external onlyRole(MANAGER_ROLE) {
        _paysNoFee[account] = true;
    }

   // TODO add description
    function includeInFee(address account) external onlyRole(MANAGER_ROLE) {
    ...
    //         rSupply = rSupply.sub(_rBalance[_noRfi[i]]);
    //         tSupply = tSupply.sub(_tBalance[_noRfi[i]]);
    //     }
    ...
    function swapTokensForTokens(uint256 tokenAmount) private {
        ...
        path[0] = address(this);
        path[1] = _BSKRContract;

        _approve(address(this), address(dexRouterV2), tokenAmount);

        // make the swap
        dexRouterV2.swapExactTokensForTokens(
            tokenAmount,
    ...
    // TODO add description
    function setBSKRContract(address newBSKRContract)
        external
