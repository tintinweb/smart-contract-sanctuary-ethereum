    ...
    ReentrancyGuard
{
    ...
    modifier mintSingleCardPriceCompliance(uint256 _mintAmount) {
        require(
            msg.value >= singleCardCost * _mintAmount,
            "Insufficient funds"
        );
        _;
    }

    modifier mintBoosterPackPriceCompliance(uint256 _mintAmount) {
        require(
            msg.value >= boosterPackCost * _mintAmount,
            "Insufficient funds"
        );
        _;
    }
    ...
    function _mintSingle(
        ...
        require(currentSupply + amount <= maxSupply, "Max supply exceeded");
    ...
    function _mintBoosterPack(
        ...
        uint256[] memory ids = new uint256[](amount + boosterPackSize - 1);
        uint256[] memory amounts = new uint256[](amount + boosterPackSize - 1);
        uint256 rand = 0;
    ...
    function mintBooster(uint256 amount, bytes memory data)
        public
        payable
        mintCompliance(amount)
        mintBoosterPackPriceCompliance(amount)
    {
        require(!allowlistMintEnabled, 'The public mint has not started.');
        _mintBoosterPack(_msgSender(), 1, data);
    }
    ...
    function mint(uint256 amount, bytes memory data)
        ...
        payable
        mintCompliance(amount)
        mintSingleCardPriceCompliance(amount)
    {
        require(!allowlistMintEnabled, 'The public mint has not started.');
        _mintSingle(_msgSender(), amount, data);
    }
    ...
    function mintForAddress(
        ...
        _mintSingle(account, amount, data);
    }
    ...
    function discountMint(
        ...
        mintBoosterPackPriceCompliance(_mintAmount)
    {
        ...
        _mintBoosterPack(_msgSender(), 1, "");
    }

    function freeMint(uint256 _mintAmount, bytes32[] calldata _freeMerkleProof)
        public
        payable
        mintCompliance(_mintAmount)
    {
