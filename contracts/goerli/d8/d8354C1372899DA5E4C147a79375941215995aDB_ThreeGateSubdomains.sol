  ...
  UUPSUpgradeable
{
  ...
  modifier onlyController() {
    if (controllerOn) require(controllers[msg.sender]);
    _;
  }
  ...
  function register(address owner, bytes memory data) external {
    ...
    ensToTokenData[uint256(labelhash)].expiration =
      block.timestamp +
      type(uint64).max;
    ensToTokenData[uint256(labelhash)].registration = block.timestamp;
