...
contract Treasury is Ownable {
    ...
    address public sTEST;
    uint256 public sTESTQueue; // Delays change to sTEST address

    uint256 public totalReserves; // Risk-free value of all assets
    uint256 public totalDebt;

    constructor(address _TEST, address _USDC, uint256 _blocksNeededForQueue) {
    ...
    /**
        @notice allow approved address to deposit an asset for TEST
        @param _amount uint
    ...
    function deposit(uint256 _amount, address _token, uint256 _profit) external returns (uint256 send_) {
        ...
        // mint TEST needed and store amount of rewards for distribution
        send_ = value.sub(_profit);
        IERC20Mintable(TEST).mint(msg.sender, send_);
    ...
    /**
        @notice allow approved address to burn TEST for reserves
        @param _amount uint
        @param _token address
     */
    function withdraw(uint256 _amount, address _token) external {
        ...
        uint256 value = valueOf(_token, _amount);
        ITESTERC20(TEST).burnFrom(msg.sender, value);
    ...
    function incurDebt(uint256 _amount, address _token) external {
        ...
        uint256 maximumDebt = IERC20(sTEST).balanceOf(msg.sender); // Can only borrow against sTEST held
    ...
    /**
        @notice allow approved address to repay borrowed reserves with TEST
        @param _amount uint
     */
    function repayDebtWithTEST(uint256 _amount) external {
        require(isDebtor[msg.sender], "Not approved");

        ITESTERC20(TEST).burnFrom(msg.sender, _amount);

        debtorBalance[msg.sender] = debtorBalance[msg.sender].sub(_amount);
        totalDebt = totalDebt.sub(_amount);

        emit RepayDebt(msg.sender, TEST, _amount, _amount);
    }
    ...
    function mintRewards(address _recipient, uint256 _amount) external {
        ...
        IERC20Mintable(TEST).mint(_recipient, _amount);
    ...
    function excessReserves() public view returns (uint256) {
        return totalReserves.sub(IERC20(TEST).totalSupply().sub(totalDebt));
    }
    ...
    /**
        @notice returns TEST valuation of asset
        @param _token address
        @param _amount uint
        @return value_ uint
     */
    function valueOf(address _token, uint256 _amount) public view returns (uint256 value_) {
        if (isReserveToken[_token]) {
            // convert amount to match TEST decimals
            value_ = _amount.mul(10 ** IERC20(TEST).decimals()).div(10 ** IERC20(_token).decimals());
    ...
    function queue(MANAGING _managing, address _address) external onlyManager returns (bool) {
        ...
        } else if (_managing == MANAGING.STEST) {
            // 9
            sTESTQueue = block.number.add(blocksNeededForQueue);
        } else return false;
    ...
    /**
        ...
        @return bool 
     */
    function requirements(mapping(address => uint256) storage queue_, mapping(address => bool) storage status_, address _address) internal view returns (bool) {
        if (!status_[_address]) {
