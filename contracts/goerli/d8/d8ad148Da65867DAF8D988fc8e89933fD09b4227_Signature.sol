...
contract Signature {
    ...
    bytes32 constant ORDERMAIN_TYPEHASH = keccak256(
    ...
    bytes32 constant ORDEROPTION_TYPEHASH = keccak256(
    ...
    function hash2(OrderMain memory orderMain) internal pure returns (bytes32) {
        return keccak256(abi.encode(
            ORDERMAIN_TYPEHASH,
            orderMain.seller,
            orderMain.buyer,
    ...
    function hash3(OrderOption memory orderOption) internal pure returns (bytes32) {
        return keccak256(abi.encode(
            ORDEROPTION_TYPEHASH,
    ...
    function hash4(Order memory order) internal pure returns (bytes32) {
        return keccak256(abi.encode(
            ORDER_TYPEHASH,
            hash2(order.orderMain),
            hash3(order.orderOption)
        ));
    }
