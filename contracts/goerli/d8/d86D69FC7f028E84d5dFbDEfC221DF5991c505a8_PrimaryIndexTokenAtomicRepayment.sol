...
contract PrimaryIndexTokenAtomicRepayment is Initializable, AccessControlUpgradeable, ReentrancyGuardUpgradeable {
    ...
    function getLendingToken(address user, address projectToken) public view returns(address actualLendingToken) {
        address usdcToken = primaryIndexToken.usdcToken();

        uint borrowedUSD = primaryIndexToken.totalOutstanding(user, projectToken, usdcToken);

        address currentLendingToken = primaryIndexToken.lendingTokenPerCollateral(user, projectToken);
    ...
    function repayAtomic(address prjToken, uint collateralAmount, bytes memory buyCalldata) public {
        ...
        if (collateralBalanceLeft > 0) {
            ...
            primaryIndexToken.deposit(prjToken, collateralBalanceLeft, address(this), msg.sender);
        }
        ...
        primaryIndexToken.repay(prjToken, lendingToken, amountRecive, address(this), msg.sender);
