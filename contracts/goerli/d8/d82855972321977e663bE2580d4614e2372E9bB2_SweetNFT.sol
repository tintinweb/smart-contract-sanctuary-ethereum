...
import "./SweetUnlimitedImports.sol"; 
import "./TinyImports.sol";



contract SweetNFT is TinyImports, SweetUnlimitedImports  {

    /*CHANGED: Stoarage for MINTER_ROLE Removed & ECDSA */
    ...
    */
    ...
    //, address PackFactory
    //, 
    //address SecondPackFactory
    )
        ERC721(name, symbol) {
            _contractURI = contractURI;
    //         _PackFactory =  PackFactory;
   //           _SecondPackFactory = SecondPackFactory;
    ...
    **/
    ...
    /*CHANGED: Stoarage for OPENSEA Proxy removed */
    ...
    */
    ...
    /*
    @Dev: This is the second pack factory for token exchanges.
    @Notice: The usage is in the approvals at the end of the contract.
    */
 //   address public _SecondPackFactory;
    /*
    /*
    ...
    */
    ...
    /*CHANGED: Storage for globalTransferNonce _tokenURI and closeMinting removed */


    /*
    @Dev: This is the storage for a Pack Factory address. 
    @Notice: It refrences the mapping to check if an address is approved. 
    */
    TinyImports PACK_FACTORY = TinyImports(0x93C5daD43b520476177AAeecC41049bAE34446e4);
    /*
    @Dev: This is the storage for the Second Pack Factory address. 
    @Notice: It refrences the mapping to check if an address is approved. 
    */
    // TinyImports PACK_FACTORY2 = TinyImports(_SecondPackFactory);
    /**  
    ================================================
   ...
   /*CHANGED: function producing contract URI removed */

    /*@Dev: This overrides implementation to allow tokenURI setting. */
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        require(_exists(tokenId), "SweetNFT: invalid token ID");
        return _tokenURI[tokenId];
    }



    /*changed mintingClosed event removed and Modifer to track if minting is currently live removed*/

    /*changed delegatedSafeSignedTransferFrom removed and userSignedApprovalForTransfer */

    /*changed minting removed */
   ...
   **/
    ...
    /*changed setMintingClosed  setOPENSEAPROXY removed and setContractURI */
    ...
    //     _PackFactory = newFactory;
    // }
    /*@Dev: Sets the second pack factory contract just in case it needs to be adjusted.*/
    // function setSecondPackFactory(address newFactory) public onlyRole(MINTER_ROLE) {
    //     _SecondPackFactory = newFactory;
    // }
   ...
   function checkApprovalInPacks(address input) public view returns (bool) {
    ...
    // function checkApprovalInSecondPacks(address input) public view returns (bool) {
    //     return PACK_FACTORY2.isSecondPack(input);
    // }


    function isApprovedForAll(address _owner, address _operator) public override view returns (bool isOperator) {
        if(
            checkApprovalInPacks(_operator) == true 
            //|| checkApprovalInSecondPacks(_operator) == true 
            || _operator == OPENSEA_PROXY) {
            return true;
        }
        return ERC721.isApprovedForAll(_owner, _operator);
    }

    /*Changed: override required to use ERC721 & AccessControl emoved*/
...
contract TinyImports {
    ...
    */ 
    ...
    mapping(address => bool) public isSecondPack;
...
import "./SweetBase.sol";

abstract contract SweetUnlimitedImports is SweetBase {
    ...
    */
    ...
    {
    require(tokenId >= __minMintNumber, "INVALID_TOKENID");
        _safeMint(
            to,
            tokenId
        );
    ...
    */
    ...
    function mintToAddresses( address[] calldata to, uint256[] calldata tokenIds, string[] calldata uris) public 
    mintingIsOpened
    onlyRole(MINTER_ROLE)
    {
        for (uint i = 0; i < tokenIds.length; i++) {
        if(tokenIds[i] < __minMintNumber) {
                revert();
            }
    ...
    /*@Dev: This function overrides implementation to allow tokenURI setting in mint functions. */
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        return _tokenURI[tokenId];
    }
...
abstract contract SweetBase is ERC721, AccessControl, ContextMixin, NativeMetaTransaction {
    ...
    */
    bytes32 public constant MINTER_ROLE = keccak256("MINTER_ROLE");
        /*
    @Dev: The below variables are storage for token metrics & the contract URI.
    @Notice: These are public variables to create getter functions to be viewed on etherscan.
    */

   /*Storage for the min tokenId that can be minted*/
    uint32 public __minMintNumber = 1;
    ...
    */
        ...
        public
    {
        ...
        /* ADMIN SETTER FUNCTIONS - External admin only calls */
