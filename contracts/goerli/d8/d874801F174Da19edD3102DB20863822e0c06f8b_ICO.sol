...
contract ICO {
    ...
    modifier onlyUnpaused() {
        if (isPaused) {
            revert("Contributions/redemptions are paused.");
        }
        _;
    }
    ...
    function advancePhase(Phase expectedCurrent) external onlyOwner {
        if (phase != expectedCurrent || phase == Phase.Open) {
    ...
    function contribute() external payable onlyUnpaused {
        ...
        if (
            (phase == Phase.Seed && (totalRaised > TOTAL_SEED_LIMIT)) ||
        ...
        if (
            (phase == Phase.Seed &&
                (contributionRecord[msg.sender] > INDIVIDUAL_SEED_LIMIT)) ||
            (phase == Phase.General &&
                (contributionRecord[msg.sender] > INDIVIDUAL_GENERAL_LIMIT))
        ) {
