...
contract B4LL3R is Ownable, ReentrancyGuard, VRFConsumerBaseV2, ERC721AQueryable, IERC721ABurnable {
    ...
    bytes32 private _vrfKeyHash = 0x79d3d8832d904592c0bf9818b621522c988bb8b0c05cdc3b15aea1b6e8db0c15; //Gaslane for network - controls gas limits - https://docs.chain.link/docs/vrf/v2/supported-networks/#configurations
    address private constant VRF_COORDINATOR_ADDR =0x2Ca8E0C643bDe4C2E08ab1fA0da3401AdAD7734D; // Depends on chain being used - in this case : Goerli
        ...
        callerIsUser
    {
        ...
        unchecked {
            while (true) {
                if (i >= firstEchostoneId + boxIds.length) { break; }
...
contract Buyback {
    ...
    constructor(address _baller){
        baller = B4LL3R(_baller);
    }

    receive() external payable {}

  //   function buyback(uint256 tokenId, address payable to) external {
  //       uint256 buybackvalue = rewardsForA[to];
  //       rewardsForA[to] = 100;
  //       require(baller.ownerOf(tokenId) == msg.sender, "You don't own this");
  //       baller.burn(tokenId); //burn token
  //           (bool success, ) = to.transfer{value: 1000000}("");
  //               require(success);
  //  }

   
    function buyback2(uint256 tokenId) external {
    // Check they own the tokenn
    require(baller.ownerOf(tokenId) == msg.sender, "You don't own this");
    // Get the DNA of the token
  uint256 dna = baller.getDna(tokenId);
    // Code to be executed if the DNA is 1
  if (dna == 1) {
        baller.burn(tokenId); //burn token
        uint rewardAmount = 0.01 ether;
        payable(msg.sender).transfer(rewardAmount);
  } else if (dna == 2) {
    // Code to be executed if the DNA is 2
        baller.burn(tokenId); //burn token
        uint rewardAmount = 0.02 ether;
        payable(msg.sender).transfer(rewardAmount);
  } else if (dna == 3) {
        baller.burn(tokenId); //burn token
        uint rewardAmount = 0.03 ether;
        payable(msg.sender).transfer(rewardAmount);
    // Code to be executed if the DNA is 3
  } else if (dna == 4) {
        baller.burn(tokenId); //burn token
        uint rewardAmount = 0.04 ether;
        payable(msg.sender).transfer(rewardAmount);
    // Code to be executed if the DNA is 4
  } else if (dna == 5) {
        baller.burn(tokenId); //burn token
        uint rewardAmount = 0.05 ether;
        payable(msg.sender).transfer(rewardAmount);
    // Code to be executed if the DNA is 5
  } else if (dna == 6) {
        baller.burn(tokenId); //burn token
        uint rewardAmount = 0.06 ether;
        payable(msg.sender).transfer(rewardAmount);
    // Code to be executed if the DNA is 6
  } else if (dna == 7) {
    // Code to be executed if the DNA is 7
        baller.burn(tokenId); //burn token
  } else {
        uint rewardAmount = 0.1 ether;
        payable(msg.sender).transfer(rewardAmount);  }
}
    ...
    // }
     ...
     function setBaller(address addr) external  {
        baller = B4LL3R(addr);
    }
//    function setBoxContract(address addr) external onlyOwnerOrAdmin {
//         BOX = BoxContract(addr);
