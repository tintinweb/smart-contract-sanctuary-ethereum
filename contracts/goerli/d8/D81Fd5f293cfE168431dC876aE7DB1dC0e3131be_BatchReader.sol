...
contract BatchReader 
{
    ...
    function readWalletOwnedMoments(
        ...
        {
            IERC721 Moments = IERC721(0xe745243b82ebC46E5c23d9B1B968612c65d45f3d); // MOMENTS
            uint[] memory temp = new uint[](100);
            uint NumOwnedTokens;
            uint ProjectStartingTokenID = ProjectIDs[x] * 1000000;
            for(ProjectStartingTokenID; ProjectStartingTokenID <= ProjectStartingTokenID + 100; ProjectStartingTokenID++)
            {
                try Moments.ownerOf(ProjectStartingTokenID) 
                {
                    if(Moments.ownerOf(ProjectStartingTokenID) == Wallet)
                    {
                        temp[NumOwnedTokens] = ProjectStartingTokenID;
                        NumOwnedTokens++;   
                    }
                } catch { }
            }
