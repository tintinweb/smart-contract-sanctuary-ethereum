...
abstract contract AllocatableERC721Upgradeable is Initializable, ERC721Upgradeable {
    ...
    event Lock(uint256 tokenId, uint64 expires);

    function __AllocatableERC721_init(string memory name_, string memory symbol_, uint256 allocIdx_) internal onlyInitializing {
        __AllocatableERC721_init_unchained(name_, symbol_, allocIdx_);
    }

    function __AllocatableERC721_init_unchained(string memory name_, string memory symbol_, uint256 allocIdx_) internal onlyInitializing {
        __ERC721_init(name_, symbol_);
        _allocIdx = allocIdx_;
    }
    
    function allocationMinting(address[] calldata accountList, uint64 expires) external virtual;
    ...
    function _lockup(uint256 tokenId, uint64 expires) internal virtual {
        _lock[tokenId] = expires;
        emit Lock(tokenId, expires);
    }
    ...
    UUPSUpgradeable
{
    ...
    function initialize(
        ...
        public initializer
    {
        __AllocatableERC721_init(name_, symbol_, allocIdx_);
    ...
    function airdrop(address[] calldata accountList) external {
    ...
    function allocationMinting(address[] calldata accountList, uint64 expires) external override onlyOwner {
        for (uint256 i = 0; i < accountList.length; i++) {
            uint256 tokenId = currentAllocIdx();
            _safeMint(accountList[i], tokenId);
            _lockup(tokenId, expires);
            _increaseAllocIdx();
        }
    }
