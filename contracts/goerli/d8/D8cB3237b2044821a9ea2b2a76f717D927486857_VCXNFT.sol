...
contract VCXNFT is ERC721A, Ownable, ReentrancyGuard {
    ...
    uint public constant MAX_RESERVED_TOKENS = 100;
    ...
    uint public constant MAX_WHALE_TOKENS = 300;
    ...
    /// @notice WhalePledge mapping
    mapping(address => WhalePledge) public addressWhalePledge;

    /// @notice Set of addresses that pledged whale spots
    EnumerableSet.AddressSet private whaleSpotPledges;

    /// @notice Pledge for whale spots
    /// @param quantity Quantity to mint
    /// @dev Pledged amount will be refunded if max whale tokens is minted
    function pledge(uint quantity) external payable nonReentrant {
        require(salePhase == 1, "Wrong sale phase (should be 1)");
        require(quantity == 2 || quantity == 5, "Quantity must be 2 or 5");
        require(msg.value >= quantity * price, "Not enough ETH sent");
        require(
            addressWhalePledge[msg.sender].quantity == 0,
            "Already pledged"
        );

        addressWhalePledge[msg.sender] = WhalePledge(
            quantity,
            price,
            block.timestamp,
            false,
        ...
        whaleSpotPledges.add(msg.sender);
    }

    /// @notice Cancel whale pledge
    /// @dev Refunds pledged amount
    function cancelPledge() external nonReentrant {
        ...
        uint refund = addressWhalePledge[msg.sender].quantity *
            addressWhalePledge[msg.sender].price;
        addressWhalePledge[msg.sender] = WhalePledge(0, 0, 0, false, false);

        payable(msg.sender).transfer(refund);
        whaleSpotPledges.remove(msg.sender);
    }

    /// @notice Mint whale spot tokens from pledges
    /// @dev Only callable by owner
    function mintWhaleSpotTokens() public onlyOwner {
        require(salePhase == 2, "Wrong sale phase (should be 2)");

        for (uint i = 0; i < whaleSpotPledges.length(); i++) {
            address pledgeAddress = whaleSpotPledges.at(i);
            uint quantity = addressWhalePledge[pledgeAddress].quantity;
            uint pledgePrice = addressWhalePledge[pledgeAddress].price;
            bool minted = addressWhalePledge[pledgeAddress].minted;
            bool refunded = addressWhalePledge[pledgeAddress].refunded;

            if (minted || refunded || quantity == 0) {
                continue;
            }

            if (whaleTokensMinted + quantity > MAX_WHALE_TOKENS) {
                uint refund = (whaleTokensMinted +
                    quantity -
                    MAX_WHALE_TOKENS) * pledgePrice;
                payable(pledgeAddress).transfer(refund);
                addressWhalePledge[pledgeAddress].refunded = true;
            } else {
                _mint(pledgeAddress, quantity);
                addressWhalePledge[pledgeAddress].minted = true;
    ...
    /// @param quantity Quantity to mint. should be only 1 for Platinum & Gold, and up to 5 wallet max in public
    ...
    function crossMint(
        ...
        if (salePhase == 3 || salePhase == 4) {
            require(
                addressPlatinumCount[to] < 2 &&
                addressGoldCount[to] < 2,
                "Address already minted once during platinum & gold presale"
            );

            require(verifyCrossmintWhitelist(merkleProof, to), "Invalid merkle proof for crossmint whitelist");
        }
    ...
    function mintPlatinum(
        ...
        require(addressPlatinumCount[msg.sender] < 2, "Already minted max limit of 2");
    ...
    function mintGold(
        ...
        if (verifyPlatinumWhitelist(merkleProof, msg.sender)) {
            require(
                addressPlatinumCount[msg.sender] < 2,
                "Already minted 2 during platinum presale"
            );
        } else {
            require(
                verifyGoldWhitelist(merkleProof, msg.sender),
                "Invalid merkle proof"
            );
            require(
                addressGoldCount[msg.sender] < 2,
                "Already minted 2 during gold presale"
            );
        }
    ...
    function pledgeMint(address to, uint quantity) public payable nonReentrant {
        require(
    ...
    /// @dev This function can only be called by either owner or devguy. The split is hard-coded at 75% to owner and 25% to devguy.
