...
interface IPersonaNFT is IPersona {
    function transferFrom(address, address, uint256) external;

    function rarityOfNFT(uint256) external view returns (PersonaNFTRarity);
...
interface IPersonaCredit {
    function getCreditFromStaking(address, uint16) external;
}

contract PersonaNFTStake is IPersona, Ownable {
    using EnumerableSet for EnumerableSet.UintSet;

    uint256 public constant rewardInterval = 1 days;
    mapping(address => EnumerableSet.UintSet) stakedNFTsByHolder;
    mapping(PersonaNFTRarity => uint8) public rewardPointByRarity;
    ...
    event ClaimeCredit(address indexed holder, uint16 amount);

    constructor(address _personaNFT, address _personaCredit) {
    ...
    function stake(uint256[] memory _tokenIds) external {
        for (uint i; i < _tokenIds.length; i++) {
            require(
                IPersonaNFT(personaNFT).ownerOf(_tokenIds[i]) == msg.sender,
                "Not your token!"
            );
            stakedNFTsByHolder[msg.sender].add(_tokenIds[i]);
            IPersonaNFT(personaNFT).approve(address(this), _tokenIds[i]);
            IPersonaNFT(personaNFT).transferFrom(
                msg.sender,
                address(this),
                _tokenIds[i]
            );
    ...
    function calcTotalCreditsByHolder(
        ...
        for (uint256 i; i < tokenIds.length; i++) {
            ...
            if (block.timestamp >= lastClaimedTime[tokenId] + rewardInterval) {
                credit +=
                    (rewardPointByRarity[IPersonaNFT(personaNFT).rarityOfNFT(tokenId)] *
                        (block.timestamp - lastClaimedTime[tokenId])) /
                    rewardInterval;
            }
        }
        return uint16(credit);
    }
    ...
    function claimCredit() public {
        ...
        for (uint256 i; i < tokenIds.length; i++) {
            uint256 tokenId = tokenIds[i];
            if (block.timestamp >= lastClaimedTime[tokenId] + rewardInterval) {
                uint256 passed = (block.timestamp - lastClaimedTime[tokenId]) /
                    rewardInterval;
                credit += rewardPointByRarity[IPersonaNFT(personaNFT).rarityOfNFT(tokenId)] * passed;
                lastClaimedTime[tokenId] += passed * rewardInterval;
            }
        }
        IPersonaCredit(personaCredit).getCreditFromStaking(
            msg.sender,
            uint16(credit)
        );
        emit ClaimeCredit(msg.sender, uint16(credit));
    }
    ...
    function getlastClaimedTimes(uint256[] memory _tokenIds) external view returns(uint256[] memory) {
    ...
    function setRewardPointByRarity(
        PersonaNFTRarity[] memory _rarities,
