...
function fetchMarketItems() public view returns (MarketItem[] memory) { 
 ...
 //its RetrnNFTs Function From Auction  
 function _returnNft(uint256 highestBid, uint256 itemId)  public returns(bool){
  
  if(highestBid == 0){   
  MarketItem memory marketitem = idToMarketItem[itemId]; 
  if(getNftType(marketitem.nftContract) == 0){
      IERC721(marketitem.nftContract).transferFrom(address(this), marketitem.owner, itemId);
      return true ; 
      }else if (getNftType(marketitem.nftContract) == 1) {
      IERC1155(marketitem.nftContract).safeTransferFrom(address(this), marketitem.seller, itemId , marketitem.amount , "");
      return true ; 
      }else{
      require(false , "cant transfer");
      }}}
  ...
  function cancelListed(uint256 itemId) public returns(bool){ 
    MarketItem memory marketitem = idToMarketItem[itemId];
    if (marketitem.owner == msg.sender){
    if(getNftType(marketitem.nftContract) == 0){
        IERC721(marketitem.nftContract).transferFrom(address(this), marketitem.owner, itemId);
        return true ; 
        }else if (getNftType(marketitem.nftContract) == 1) {
        IERC1155(marketitem.nftContract).safeTransferFrom(address(this), marketitem.owner, itemId , marketitem.amount , "");
        return true ; 
        }else{
        require(false , "cant transfer");
        }
