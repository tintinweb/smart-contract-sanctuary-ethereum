 ...
 * @title ArtBuyout
 */
contract ArtBuyout is Context, ReentrancyGuard, AccessControl {
    ArtTokenVault public immutable tokenAT;
    mapping(address => uint256) private votesAT;
    uint256 private totalVotesAT;
    uint256 public immutable endTimestamp;
    // totalVotesAT should be greater than this value for the ArtBuyout to be successful
    uint256 private successVoteThreshold;

    constructor(
        ArtTokenVault _tokenAT,
    ...
    function voteFor() public {
        ...
        uint256 balance = tokenAT.balanceOf(_msgSender());
        votesAT[_msgSender()] = balance;
        totalVotesAT = totalVotesAT + balance;
        tokenAT.transferFrom(_msgSender(), address(this), balance);
    }
...
import "./ArtBuyout.sol";

contract ArtTokenVault is ERC20Upgradeable, AccessControlUpgradeable {
    event ERC20CrowdsaleCreated(address crowdsaleAddress, address tokenAddress);
    event ArtBuyoutCreated(address buyoutAddressAddress);
    ...
    ArtTokenERC721 private nftContract;
    ...
    function claimPlatformFee() public {
        ...
        require(
            _msgSender() == settings.platformAdmin(),
            "Only Admin can claim"
        );
        _mint(
            settings.platformFeeWallet(),
            (totalSupply() * settings.platformFeeBps()) / 10000
        );
    }
    ...
    function burnNFT(uint256 tokenId) public onlyRole(SELLER_ROLE) {
        require(
            crowdsaleStatus() != CrowdsaleStatus.IN_PROGRESS,
            "Crowdsale in progress"
        );
    ...
    function deployNewERC20Crowdsale(
        ...
        if (crowdsaleStatus() == CrowdsaleStatus.NOT_PLANNED) {
            _mint(address(newCrowdsale), supply);
            //_burn(address(crowdsale), balanceOf(address(crowdsale)));
        } else {
//            _transfer(
//                address(crowdsale),
//                address(newCrowdsale),
//                balanceOf(address(crowdsale))
//            );
        }
    ...
    function deployBuyout(uint256 amountBT) public returns (address) {
        ...
        require(amountBT > crowdsale.totalPriceBT());
        uint256 successVoteThreshold = totalSupply() / 2;
        ArtBuyout newBuyout = new ArtBuyout(
            this,
            block.timestamp + settings.buyoutDurationSeconds(),
            successVoteThreshold
        );
        emit ArtBuyoutCreated(address(newBuyout));
        crowdsale.tokenBT().transferFrom(
            _msgSender(),
            address(newBuyout),
            amountBT
        ); // TODO: check success
 ...
 */
contract ERC20FTCrowdsale is ICrowdsale, Context, ReentrancyGuard, Whitelist {
    ...
    IERC20 private immutable pTokenBT;
    ...
    // @notice Total amount of tokens that each buyer can buy with one or multiple transactions
    uint256 public immutable maxPurchaseBT;
    uint256 private immutable pTotalPriceBT;
     ...
     * @param artTokens amount of tokens reserved
     */
    ...
    /**
     * Event for art tokens claim after successful crowdsale
     * @param crowdsale The address of the crowdsale contract
     * @param buyer who claimed the tokens
     * @param artTokens amount claimed
     */
    event TokensClaimed(
        address indexed crowdsale,
        address indexed buyer,
    ...
    /**
     * Event for base tokens refund after unsuccessful crowdsale
     * @param crowdsale The address of the crowdsale contract
    ...
    constructor(
        ...
        require(
            sellerRetainedAmount <= crowdsaleAmount,
        ...
        pTotalPriceBT = costBT(crowdsaleAmount);
    ...
    function buyTokens(
        uint256 amountBT
    ) public nonReentrant onlyRole(PARTICIPANT_ROLE) {
    ...
    function _postValidatePurchase(
        ...
        require(
            ...
            "Total allocation is incorrect in _postValidatePurchase"
        );
    }
    ...
    function _processPurchase(
        ...
        bool ok = pTokenBT.transferFrom(buyer, address(this), amountBT);
    ...
    function claimSaleProceeds() public virtual nonReentrant returns (uint256) {
        require(
            seller == _msgSender(),
        ...
        collectedBT = 0;
        bool ok = pTokenBT.transfer(seller, amountBT);
        require(ok, "Token transfer not successful");
        return amountBT;
    }

    function claimTokens() public virtual nonReentrant returns (uint256) {
        require(
        ...
        returns (uint256)
    {
        ...
        bool ok = pTokenBT.transfer(_msgSender(), refundBT);
        require(ok, "Token transfer not successful");
        return refundBT;
    }
    ...
    function totalPriceBT() public view override returns (uint256) {
        return pTotalPriceBT;
    }

    function tokenBT() public view override returns (IERC20) {
        return pTokenBT;
    }
}
...
interface ICrowdsale is IWhitelist {
    ...
    function totalPriceBT() external returns (uint256);

    function tokenBT() external returns (IERC20);
}
