...
contract HarvestResolver is IResolver, Ownable {
  ...
  /// @notice Emitted when a keeper status is updated.
  event SetKeeper(address keeper, bool status);
  ...
  /// @dev A mapping of the registered keepers.
  ...
  /// @notice Enables or disables a keeper from calling protected keeper-only functions.
  ///
  /// @param keeper The address of the target keeper.
  /// @param status The status to set for the target keeper.
  ...
  function addHarvestJob(
    ...
    harvestJobs[yieldToken] = HarvestJob(active, alchemist, block.timestamp, minimumHarvestAmount, minimumDelay);

    emit SetHarvestJob(active, yieldToken, alchemist, minimumHarvestAmount, minimumDelay);
      ...
      if (h.active) {
        ...
        if (ytp.enabled) {
          ...
          if (
            ...
            minimumAmountOut = minimumAmountOut - minimumAmountOut / 1000; // 0.1% slippage
            return (
              true,
  ...
  function recordHarvest(address yieldToken) external onlyKeeper {
