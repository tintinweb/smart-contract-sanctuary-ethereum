...
contract ShillyDegensClub is ERC20, Ownable {
    using SafeMath for uint256;
	...
	uint256 public maxTxAmount = 200000 * (10**9);
	
	address public marketingFeeAddress = 0x0c0B870bBb5b052Df8b9C04E5f43191Dd9E4B2EA;
    ...
    constructor() ERC20("Shilly Degens Club", "SDCv2") {
	...
	function setMarketingFee(uint256 buy, uint256 sell, uint256 p2p) external onlyOwner {
	    require(liquidityFee[0].add(buy)  <= 2500 , "Max fee limit reached for 'BUY'");
		require(liquidityFee[1].add(sell) <= 2500 , "Max fee limit reached for 'SELL'");
		require(liquidityFee[2].add(p2p)  <= 2500 , "Max fee limit reached for 'P2P'");
	...
	function setLiquidityFee(uint256 buy, uint256 sell, uint256 p2p) external onlyOwner {
	    require(marketingFee[0].add(buy)  <= 2500 , "Max fee limit reached for 'BUY'");
		require(marketingFee[1].add(sell) <= 2500 , "Max fee limit reached for 'SELL'");
		require(marketingFee[2].add(p2p)  <= 2500 , "Max fee limit reached for 'P2P'");
	...
	function _transfer(address from, address to, uint256 amount) internal override {
		...
		if (!swapping && canSwap && swapEnable && automatedMarketMakerPairs[to]) {
			...
			uint256 tokenToSwap = tokenToMarketing.add(liquidityHalf);
