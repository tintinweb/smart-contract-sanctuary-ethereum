...
library Orders {
    ...
    uint8 public constant BUY_INVERTED_TYPE = 4; // Consumed also in TwapDelay
    uint8 private constant SELL_TYPE = 5;
    uint8 public constant SELL_INVERTED_TYPE = 6; // Consumed also in TwapDelay
    ...
    function decodeType(uint256 internalType) internal pure returns (OrderType) {
    ...
    function _enqueueOrder(Data storage data, Order memory order) private {
        order.orderId = ++data.newestOrderId;
        data.orderQueue[order.orderId] = getOrderDigest(order);
    }

    function enqueueDepositOrder(Data storage data, Order memory order) internal {
        _enqueueOrder(data, order);

        emit DepositEnqueued(order.orderId, order);
    }

    function enqueueWithdrawOrder(Data storage data, Order memory order) internal {
        _enqueueOrder(data, order);

        emit WithdrawEnqueued(order.orderId, order);
    }

    function enqueueSellOrder(Data storage data, Order memory order) internal {
        _enqueueOrder(data, order);

        emit SellEnqueued(order.orderId, order);
    }

    function enqueueBuyOrder(Data storage data, Order memory order) internal {
        _enqueueOrder(data, order);
    ...
    function isRefundFailed(Data storage data, uint256 orderId) internal view returns (bool) {
        return data.failure[orderId];
    }

    function markRefundFailed(Data storage data) internal {
        data.failure[data.lastProcessedOrderId] = true;
    }
    ...
    function withdraw(Data storage data, WithdrawParams calldata withdrawParams) external {
        ...
        enqueueWithdrawOrder(
            ...
            Order(
                0,
                WITHDRAW_TYPE,
                timestampToUint32(block.timestamp) + data.delay, // validAfterTimestamp
    ...
    function dequeueCanceledOrder(Data storage data, uint256 orderId) internal {
        ++data.lastProcessedOrderId;
        require(orderId == data.lastProcessedOrderId, 'OS72');
    }

    function dequeueDepositOrder(
        Data storage data,
        uint256 orderId,
        uint8 orderType
    ) external {
        ++data.lastProcessedOrderId;
        require(orderId == data.lastProcessedOrderId, 'OS72');
        require(orderType == DEPOSIT_TYPE, 'OS32');
    }

    function dequeueWithdrawOrder(
        Data storage data,
        uint256 orderId,
        uint8 orderType
    ) external {
        ++data.lastProcessedOrderId;
        require(orderId == data.lastProcessedOrderId, 'OS72');
        require(orderType == WITHDRAW_TYPE, 'OS32');
    }

    function dequeueSellOrder(
        Data storage data,
        uint256 orderId,
        uint8 orderType
    ) external {
        ++data.lastProcessedOrderId;
        require(orderId == data.lastProcessedOrderId, 'OS72');
        require(orderType == SELL_TYPE || orderType == SELL_INVERTED_TYPE, 'OS32');
    }

    function dequeueBuyOrder(
        Data storage data,
        uint256 orderId,
        uint8 orderType
    ) external {
        ++data.lastProcessedOrderId;
        require(orderId == data.lastProcessedOrderId, 'OS72');
        require(orderType == BUY_TYPE || orderType == BUY_INVERTED_TYPE, 'OS32');
    }
