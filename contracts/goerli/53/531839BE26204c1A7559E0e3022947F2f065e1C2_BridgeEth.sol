...
function mint (string memory  _tokenName, string memory _tokenSymbol, uint8 _decimals, uint256 _amount, address _receiver, address _nativeToken) external onlyOwner { 
          Token token;           
...
function unlock(address _nativeToken, uint256 _amount, address _receiver) external  onlyOwner {  
        Token(_nativeToken).approve(msg.sender, _amount); 
...
function burn (address _nativeToken, uint256 _amount) payable external {    
        if (msg.value != fee ) {
           revert YouMustPayTheServiceFee ();
        }
