...
contract RoundImplementation is AccessControlEnumerable, Initializable, ReentrancyGuard {
    ...
    function getTotalVotes(address account) public view returns (uint) {
        uint256 daoTokenVotes = uint256(daoToken.getPriorBalance(account, startBlock));
        uint256 sbtVotes = IDonationSBT(donationSBT).balanceOf(account) * 50; // 50 credits for SBT holders
