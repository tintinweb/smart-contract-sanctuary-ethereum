...
contract ProtocolManager is ModuleAccessController {
    address public immutable bond;
    address public immutable bumpOracle;
    address public immutable config;

    constructor(address _gac, address _bond, address _oracle, address _config) 
    ...
    function updateBonding() external {
        // update BUMP price
        uint lastPrice = IBumpOracle(bumpOracle).updateBumpPrice();

        address[] memory assets = IProtocolConfig(config).getAssets();
        uint _lenght = assets.length;
        for (uint i = 0; i < _lenght; i++) {
            address asset = assets[i];
            IState state = IState(IMarket( IProtocolConfig(config).getMarket(asset) ).getState() );
