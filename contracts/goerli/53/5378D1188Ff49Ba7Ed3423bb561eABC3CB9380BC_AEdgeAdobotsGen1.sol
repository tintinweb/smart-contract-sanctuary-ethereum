...
contract AEdgeAdobotsGen1 is ERC721Tradable, IERC2981, ReentrancyGuard {
    ...
    function _bulkRequire(uint256 num, uint256 mintPrice) internal {
        require(msg.value == (mintPrice * num), 'Please provide exact mint amount!');
        require(_totalMinted() + (num - 1) < MAX_SUPPLY, 'Maximum number of tokens minted');
    ...
    function getMintSettings() external view returns (MintSettings memory) {
        MintSettings memory mintSettings = MintSettings(
            MAX_SUPPLY, MAX_PER_WALLET, MINT_PRICE, WHITELIST_PRICE, MINTING_ENABLED, WHITELIST_ENABLED, MINTING_CLOSED, MERKLE_ROOT, count()
        );
