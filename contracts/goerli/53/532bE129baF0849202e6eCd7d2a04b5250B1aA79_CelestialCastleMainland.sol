 ...
 */
contract CelestialCastleMainland is Ownable, IERC721Receiver, ICelestialCastle {
    ...
    modifier onlyPortal() {
        require(
            msg.sender == address(portalMainland),
            "CelestialCastle: sender is not the portal"
        );
        _;
    }
    ...
    function initialize(
        ...
        freaksNGuilds = IFnGNew(newFreaksNGuilds);
    ...
    function travel(
        ...
        bytes[] memory calls = new bytes[](
            (freakIds.length > 0 ? 1 : 0) +
                (celestialIds.length > 0 ? 1 : 0) +
                (fbxAmount > 0 ? 1 : 0)
        );
    ...
    function initiateFreaksTravel(uint256[] calldata freakIds)
        internal
        returns (bytes memory)
    {
        ...
        for (uint256 i = 0; i < freakIds.length; i++) {
            require(
                freaksNGuilds.isFreak(freakIds[i]),
                "CelestialCastle: not a freak"
            );
        ...
        return
            abi.encode(
                CelestialPortalMessages.RETRIEVE_FREAKS,
                abi.encode(freakIds, freaks)
            );
    }

    function initiateCelestialsTravel(uint256[] calldata celestialIds)
        internal
        returns (bytes memory)
    {
        CelestialV2[] memory celestials = new CelestialV2[](
            celestialIds.length
        );
        for (uint256 i = 0; i < celestialIds.length; i++) {
            require(
                !freaksNGuilds.isFreak(celestialIds[i]),
                "CelestialCastle: not a celestial"
            );
            celestials[i] = freaksNGuilds.getCelestialAttributes(
                celestialIds[i]
            );
            freaksNGuilds.transferFrom(
                msg.sender,
                address(this),
                celestialIds[i]
            );
        ...
        return
            abi.encode(
                CelestialPortalMessages.RETRIEVE_CELESTIALS,
                abi.encode(celestialIds, celestials)
            );
    }

    function initiateFreaksBucksTravel(uint256 fbxAmount)
        internal
        returns (bytes memory)
    {
        ...
        return
            abi.encode(
                CelestialPortalMessages.RETRIEVE_FBX,
                abi.encode(fbxAmount)
            );
    }
    ...
    function retrieveFreaks(
        ...
        for (uint256 i = 0; i < freakIds.length; i++) {
            ...
            freaksNGuilds.updateFreakAttributes(
                freakIds[i],
                freakAttributes[i]
            );
        }
    ...
    function retrieveCelestials(
        ...
        for (uint256 i = 0; i < celestialIds.length; i++) {
            ...
            freaksNGuilds.updateCelestialAttributes(
                celestialIds[i],
                celestialAttributes[i]
            );
        }
    }
