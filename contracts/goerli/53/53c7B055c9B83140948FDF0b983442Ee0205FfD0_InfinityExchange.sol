...
contract InfinityExchange is ReentrancyGuard, Ownable {
  ...
  function matchOrders(
    ...
    for (uint256 i = 0; i < numSells; ) {
      ...
      _matchOrders(sells[i], buys[i], constructs[i], startGasPerOrder, protocolFeeBps, wethTransferGasUnits, weth);
      unchecked {
        ++i;
      }
    }
  ...
  function _matchOrders(
    ...
    (bool orderVerified, uint256 execPrice) = verifyMatchOrders(
      sellOrderHash,
      buyOrderHash,
      sell,
      buy,
  ...
  function _takeOrders(
    ...
    _execTakeOrders(makerOrderHash, makerOrder, takerItems, execPrice, makerOrder.isSellOrder);
  }
