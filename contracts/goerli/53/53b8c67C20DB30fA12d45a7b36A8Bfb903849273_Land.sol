...
contract Land is ERC721, ERC2981, ReentrancyGuard, Ownable, Pausable {
    ...
    */
    ...
    /**
    * @dev Throws if address is not on the public whitelist
    *
    * @param _publicMerkleProof address proof passed by caller
    */
    modifier isPublicWhitelist(bytes32[] memory _publicMerkleProof) {
        require(
            isValidPublicMerkleProof(
                _publicMerkleProof,
                keccak256(abi.encodePacked(msg.sender))
            ),
            "Not on public whitelist"
        );
        _;
    }

    /**
    * @dev Throws if address is not on the private whitelist
    *
    * @param _privateMerkleProof address proof passed by caller
    */
    modifier isPrivateWhitelist(bytes32[] memory _privateMerkleProof) {
        require(
            isValidPrivateMerkleProof(
                _privateMerkleProof,
                keccak256(abi.encodePacked(msg.sender))
            ),
            "Not on private whitelist"
        );
        _;
    }
    ...
    */
    ...
    /**
    * @dev Restricted mint function requires bytes32[] public merkleProof
    *
    ...
    * @param _publicMerkleProof address proof for whitelisting
    */
    function publicMint(
        uint256 _tokenId,
        string memory _tokenURI,
        bytes32[] memory _publicMerkleProof
    )
        external
        payable
        whenNotPaused
        ...
        isPublicWhitelist(_publicMerkleProof)
    ...
    /**
    * @dev Restricted mint function requires bytes32[] private merkleProof
    *
    * @dev Unsafe: doesn't execute `onERC721Received` on the receiver.
    *
    * @dev Creates new token with token id specified
    ...
    * @param _tokenId token id to mint
    * @param _tokenURI token uri of token id to mint
    * @param _privateMerkleProof address proof for whitelisting
    */
    function privateMint(
        uint256 _tokenId,
        string memory _tokenURI,
        bytes32[] memory _privateMerkleProof
    )
        external
        whenNotPaused
        isCallerValid
        isTotalMintedExceedsMaxSupply(SINGLE_MINT_QTY)
        isNFTBalanceExceedsMaxMintPerAddress(msg.sender, SINGLE_MINT_QTY)
        isPrivateWhitelist(_privateMerkleProof)
        saleModeSingle(_tokenId)
    {
        _mintNFT(msg.sender, _tokenId, _tokenURI);
    ...
    */
    ...
    /**
    * @dev Validate public merkleProof
    *
    * @param _publicMerkleProof address proof
    ...
    */
    function isValidPublicMerkleProof(
        ...
        returns (bool)
    {
        return MerkleProof.verify(_publicMerkleProof, publicMerkleRoot, _merkleLeaf);
    }

    /**
    * @dev Validate private merkleProof
    *
    * @param _privateMerkleProof address proof
    * @param _merkleLeaf address to validate the proof
    */
    function isValidPrivateMerkleProof(
        bytes32[] memory _privateMerkleProof,
        bytes32 _merkleLeaf
    )
        private
        ...
        returns (bool)
    {
        return MerkleProof.verify(_privateMerkleProof, privateMerkleRoot, _merkleLeaf);
    }
