...
contract ChonkyNFT is IChonkyNFT, ERC721, VRFConsumerBase, OwnableInternal {
    ...
    uint256 internal constant PRICE = 70 * 10**13;
    // Price per unit if buying >= AMOUNT_DISCOUNT_ONE
    uint256 internal constant PRICE_DISCOUNT_ONE = 65 * 10**13;
    // Price per unit if buying MAX_AMOUNT
    uint256 internal constant PRICE_DISCOUNT_MAX = 60 * 10**13;
    ...
    function getChonkyAttributes(uint256 _tokenId)
        external
        view
        returns (uint256[4] memory)
    {
        ...
        uint256 genomeId = _getGenomeId(_tokenId);
        uint256 genome = _getGenome(genomeId);
        uint256 setId = IChonkySet(l.chonkySet).getSetId(genome);
        uint256[12] memory attributes = ChonkyGenomeLib.parseGenome(genome);

        return
            IChonkyAttributes(l.chonkyAttributes).getAttributeValues(
                attributes,
                setId
            );
    }
