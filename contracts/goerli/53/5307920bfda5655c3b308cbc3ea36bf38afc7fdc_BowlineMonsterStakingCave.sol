    ...
    ERC1820Implementer
{
        ...
        onlyRewardUpdater
    {
        ...
        require(
            _totalBonus > 0,
            "MonsterStakingCave: No Bonus to be distributed"
        );
    ...
    {
        uint256 stakedTokensLength = _tokens.length;
        require(stakedTokensLength > 0, "MonsterStakingCave: No Tokens in calldata");
        for (uint256 i; i < stakedTokensLength; i = unsafe_inc(i)) {
            ejectToken(_tokens[i].contractAddress, _tokens[i].tokenIdentifier);
        }
    }
            ...
            if (_simulateUnaccounted) {
                totalRewards +=
                    ((((block.timestamp - stakedTokens[i].lastRewardedAt) *
                        10000) / rewardPeriodInSeconds) * tokenReward) /
                    10000 -
                    periodsToReward *
                    tokenReward;
            } else {
    ...
    modifier onlyRewardUpdater() {
        require(
            ...
            "MonsterStakignCave: Only REWARD_UPDATE or OWNER."
        );
        _;
    }
}
