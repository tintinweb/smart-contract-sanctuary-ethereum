...
contract AcrocalpyseMarketplace is Initializable, OwnableUpgradeable, PausableUpgradeable {
    ...
    event ReclaimedBids(address bidder, uint256[] itemIds, uint256[] quantity, uint256 timestamp);
    ...
    function reclaimBids(
        ...
        // verifying the signature
        bytes32 hash = ECDSAUpgradeable.toEthSignedMessageHash(
            keccak256(abi.encodePacked(_msgSender(), itemIds, quantity))
        );
        require(ECDSAUpgradeable.recover(hash, signature) == signerAddress, "Invalid Access");
        ...
        for (uint256 i = 0; i < itemIds.length; i++) {
            ...
            if (!bidder.isRefunded) {
                totalPaperToClaim += bidder.bidAmount * quantity[i];
        ...
        emit ReclaimedBids(_msgSender(), itemIds, quantity, block.timestamp); // solhint-disable-line not-rely-on-time
    }
