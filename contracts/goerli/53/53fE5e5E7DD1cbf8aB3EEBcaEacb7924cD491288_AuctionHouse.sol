    ...
    ERC721Holder
{
    ...
    function unpause() external override onlyOwner {
        ...
        if ((auction.startTime == 0 || auction.settled) && vesselverse.supplyLeft()) {
            _createAuction();
        }
    }
...
interface IAuctionHouse {
    ...
    event AuctionCreated(uint256 indexed vesselverseId, uint256 startTime, uint256 endTime);

    event AuctionBid(uint256 indexed vesselverseId, address sender, uint256 value, bool extended);

    event AuctionExtended(uint256 indexed vesselverseId, uint256 endTime);

    event AuctionSettled(uint256 indexed vesselverseId, address winner, uint256 amount);
