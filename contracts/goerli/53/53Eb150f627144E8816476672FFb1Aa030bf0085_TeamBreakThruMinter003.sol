...
contract TeamBreakThruMinter003 is ERC1155Burnable, Ownable, DefaultOperatorFilterer {
    string public name = "TeamBreakThruMinter003";
    string public symbol = "TBTM003";
    string CouponAddressCode = "";
    string public contractUri = "https://dev.teambreakthru.net/collections/TeamBreakThruMinter003/contract";
    address public CouponSigner; // Address of the wallet that generated the coupons 
    address MintSender;
    uint256 public MintPrice = 0.00 ether; // Price of each NFT in Ethereum during Public Sale
    ...
    constructor() ERC1155("https://dev.teambreakthru.net/collections/TeamBreakThruMinter003/token_{id}") {
        idTracker.increment();
    }
    ...
    function setTokenType(bool _tokentypesingle) public onlyOwner
    {
    ...
    function mint(uint256 amount, uint256 allotted, Coupon memory coupon) external payable {
        require(MintEnabled, "Mint not enabled");
        require((MaxTokenAmount <=  TokenAmount[Token]+amount) || (MaxTokenAmount == 0), "Exceeds Max Token Amount");
        require(msg.value >= MintPrice * amount, "Not enough eth");
        require(amount + addressToMinted[_msgSender()]._MintsByAddress < allotted + 1, "Exceeds Max Allotted");
        if(!MintPublic){
            MintSender = msg.sender;
        }else{
            MintSender = 0x0000000000000000000000000000000000000000;
        }
        ...
        addressToMinted[_msgSender()]._MintsByAddress += amount;
        if (TokenTypeSingle) {   
