...
import './utils/CoolUpgradeableERC1155ErrorsAndEvents.sol';

contract CoolERC1155 is
  ...
  ErrorsAndEvents
{
  ...
  // @dev Mapping to hold max limit for a given token
  mapping(uint256 => uint256) public _maxLimit;
  ...
  function mint(address to, uint256 tokenId, uint256 amount) external onlyMinter {
    uint256 maxLimit = _maxLimit[tokenId];
    unchecked {
      if (maxLimit > 0) {
        uint256 currentSupply = totalSupply(tokenId);
        if (currentSupply + amount > maxLimit) {
          revert MaxLimitReached(maxLimit);
        }
      }
    }
  ...
  /// @param tokenId The token id of the token to mint
  /// @param amount The amount of tokens to mint
  /// @param data Extra data for backend
  function mint(
    address to,
    uint256 tokenId,
    uint256 amount,
  ...
  /// @notice Set max limit for a given token
  /// @dev Can only be called by a manager
  /// @param tokenId The token id of the token to set max limit
  /// @param maxLimit The max limit of the token
  function setMaxLimit(uint256 tokenId, uint256 maxLimit) external onlyManager {
    _maxLimit[tokenId] = maxLimit;

    emit MaxLimitUpdated(tokenId, maxLimit);
  }
...
contract ErrorsAndEvents {
  ...
  event MaxLimitUpdated(uint256 tokenId, uint256 maxLimit);
