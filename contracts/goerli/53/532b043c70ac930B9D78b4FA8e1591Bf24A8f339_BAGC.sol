...
contract BAGC is OwnableUpgradeable, ERC721URIStorageUpgradeable, ERC721EnumerableUpgradeable {
    using StringsUpgradeable for uint256;
    using ECDSA for bytes32;

    string public baseURI;
    address public invitationNFTAddress;
    address public goodsNFTAddress;
    address private relayerAddress;

    uint256 private _numUserAvailableTokens;

    uint256[12] userTokenBoundaries;
    ...
    event OwnerMinted(address to, uint256 tokenId);
    ...
    function initialize(
        ...
        _numUserAvailableTokens = maxSupply_;
    }
    ...
    /// @notice the function used to mint tokens other than user volume
    /// @dev the function that only owner can mint
    /// @param to The address that the token is minted to.
    function ownerMint(address to, uint256 tokenId) public onlyOwner {
        if (
            (userTokenBoundaries[0] <= tokenId && tokenId <= userTokenBoundaries[0] + 7 - 1) ||
            (userTokenBoundaries[2] <= tokenId && tokenId <= userTokenBoundaries[2] + 8 - 1) ||
            (userTokenBoundaries[4] <= tokenId && tokenId <= userTokenBoundaries[4] + 240 - 1) ||
            (userTokenBoundaries[6] <= tokenId && tokenId <= userTokenBoundaries[6] + 765 - 1) ||
            (userTokenBoundaries[8] <= tokenId && tokenId <= userTokenBoundaries[8] + 1468 - 1) ||
            (userTokenBoundaries[10] <= tokenId && tokenId <= userTokenBoundaries[10] + 2400 - 1)
        ) {
            revert("BAGC: invalid tokenId");
        }

        _safeMint(to, tokenId);

        IGoodsNFT(goodsNFTAddress).mint(to, tokenId);

        emit OwnerMinted(to, tokenId);
    }

    /// @notice the function used to mint tokens of user volume
    ...
    function userMint(
        ...
        uint256 tokenId = getRandomAvailableTokenId(msg.sender, _numUserAvailableTokens);

        _safeMint(msg.sender, tokenId);

        --_numUserAvailableTokens;

        IGoodsNFT(goodsNFTAddress).mint(msg.sender, tokenId);
    ...
    /// @dev the function to update the invitationNFTAddress
    function updateInvitationNFTAddress(address invitationNFTAddress_) public onlyOwner {
        invitationNFTAddress = invitationNFTAddress_;
    }
    ...
    /// @dev the function to update the goodsNFTAddress
    function updateGoodsNFTAddress(address goodsNFTAddress_) public onlyOwner {
        goodsNFTAddress = goodsNFTAddress_;
    }

    function updateBoundaries(uint256[12] memory boundaries) public onlyOwner {
        userTokenBoundaries = boundaries;
    }
        ...
        returns (uint256)
    {
        ...
        return getRealTokenId(getAvailableTokenAtIndex(randomIndex, updatedNumAvailableTokens));
    }
    ...
    function getRealTokenId(uint256 tokenId) internal view returns (uint256) {
        if (1 <= tokenId && tokenId <= 7) {
            return userTokenBoundaries[0] + tokenId - 1;
        } else if (7 < tokenId && tokenId <= 7 + 8) {
            return userTokenBoundaries[2] + tokenId - 1 - 7;
        } else if (7 + 8 < tokenId && tokenId <= 7 + 8 + 240) {
            return userTokenBoundaries[4] + tokenId - 1 - 7 - 8;
        } else if (7 + 8 + 240 < tokenId && tokenId <= 7 + 8 + 240 + 765) {
            return userTokenBoundaries[6] + tokenId - 1 - 7 - 8 - 240;
        } else if (7 + 8 + 240 + 765 < tokenId && tokenId <= 7 + 8 + 240 + 765 + 1468) {
            return userTokenBoundaries[8] + tokenId - 1 - 7 - 8 - 240 - 765;
        } else if (
            7 + 8 + 240 + 765 + 1468 < tokenId && tokenId <= 7 + 8 + 240 + 765 + 1468 + 2400
        ) {
            return userTokenBoundaries[10] + tokenId - 1 - 7 - 8 - 240 - 765 - 1468;
        } else {
            revert("BAGC: invalid tokenId");
        }
    }
    ...
    /// @param userSigner The address of the user signer.
    /// @param invitationTokenId The tokenId of the invitation NFT.
    /// @param userSignature The signature of the user.
    /// @param relayerSignature The signature of the relayer.
    function verifySignature(
        ...
        string memory originMessage = string(
            abi.encodePacked(
                "I agree to the terms of reveal the BAGC NFT.\n\n",
                "[Invitation Token ID]:",
                invitationTokenId.toString()
            )
        );
        string memory size = bytes(originMessage).length.toString();

        bytes32 recoverMessageHash = keccak256(
            abi.encodePacked("\x19Ethereum Signed Message:\n", size, originMessage)
        );

        address recoverUserSigner = recoverMessageHash.recover(userSignature);
        address recoverRelayerSigner = recoverMessageHash.recover(relayerSignature);
        if (recoverUserSigner != userSigner) {
            return (success = false, message = "BAGC: Invalid user signer");
        }
