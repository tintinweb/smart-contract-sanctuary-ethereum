...
contract NFTIntegrationDemo is Ownable, ReentrancyGuard, ERC721A {
    ...
    uint256 public maxReapMintAmount = 1;
    uint256 public maxAllowMintAmount = 2;
    uint256 public maxPublicMintAmount = 2;

    uint256 public maxReapAmount = 1111;
    uint256 public maxAllowAmount = 5555;
    ...
    function _preMint(bytes32[] memory proof, uint256 num) internal{
        ...
        if(status == Status.ReapMint){
            ...
            require(maxReapAmount - totalSupply() >= num, "Max supply reached");

            require(mintedReapAddress[msg.sender] + num <= maxReapMintAmount, "Minting more than the max supply for a single address");
            ...
            require(maxReapAmount + maxAllowAmount - totalSupply() >= num, "Max supply reached");

            require(mintedAllowAddress[msg.sender] + num <= maxAllowMintAmount, "Minting more than the max supply for a single address");
    ...
    function _publicMint(uint256 num) internal
    {
        ...
        require(MAX_TOTAL_SUPPLY - totalSupply() - num >= MAX_TEAM_HOLD, "Max supply reached");

        require(mintedPublicAddress[msg.sender] + num <= maxPublicMintAmount, "Minting more than the max supply for a single address");
    ...
    function setMaxMintNumber(uint256 _maxReapMintAmount, uint256 _maxAllowMintAmount, uint256 _maxPublicMintAmount) public onlyOwner{
        maxReapMintAmount = _maxReapMintAmount;
        maxAllowMintAmount = _maxAllowMintAmount;
        maxPublicMintAmount = _maxPublicMintAmount;

    }

    function setMaxNumber(uint256 _maxReapAmount, uint256 _maxAllowAmount) public onlyOwner{
        maxReapAmount = _maxReapAmount;
        maxAllowAmount = _maxAllowAmount;
    }
