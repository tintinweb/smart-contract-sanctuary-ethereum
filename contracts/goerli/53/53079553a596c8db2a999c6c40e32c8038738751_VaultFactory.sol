...
contract VaultFactory is IVaultFactory, Owned {
    IVaultLoanValidator private _vaultLoanValidator;

    mapping(address => address) private _vaultLoanValidators;

    mapping(address => bool) private _vaults;

    event VaultCreated(address _vaultAddress);

    event ClosedVault(address _vault);

    event DestroyedVault(address _vault);

    event LoanRemoved(address _vault, address _loan);

    error VaultNotExistError(address _vault);

    error VaultLoanValidatorNotSetError();

    error NotAuthorizedError(address _expected);

    ProtocolFeeData private _protocolFeeData;

    mapping(address => bool) private _validLoans;

    modifier onlyController(address _vault) {
        Vault vault = Vault(_vault);
        address controller = vault.getController();
        if (controller != msg.sender) {
            revert NotAuthorizedError(controller);
        }
        _;
    }
    ...
    function deploy(
        Vault.Parameters memory _vaultParameters,
        bytes calldata _validatorParameters
    ) external returns (address) {
        if (address(_vaultLoanValidator) == address(0)) {
            revert VaultLoanValidatorNotSetError();
        }
        _vaultParameters.originalAcceptedLoans = _vaultLoanValidator
            .getAcceptedLoans();
        address _vault = address(new Vault(_vaultParameters));
        _vaults[_vault] = true;
        _vaultLoanValidators[_vault] = address(_vaultLoanValidator);
        _vaultLoanValidator.addVault(_vault, _validatorParameters);
        emit VaultCreated(_vault);
        return _vault;
    }

    function getVaultLoanValidator() external view returns (address) {
        return address(_vaultLoanValidator);
    }

    function closeVault(address _vault) external {
        Vault(_vault).closeVault(msg.sender);

        emit ClosedVault(_vault);
    }

    function vaultExists(address _maybeVault) external view returns (bool) {
        return _vaults[_maybeVault];
    }

    function setVaultLoanValidator(address vaultLoanValidator)
        external
        onlyOwner
    {
        _vaultLoanValidator = IVaultLoanValidator(vaultLoanValidator);
    }
    ...
    function getVaultLoanValidator(address _vault)
        ...
        returns (address)
    {
        return _vaultLoanValidators[_vault];
    }

    function upgradeVaultLoanValidator(
        address _vault,
        bytes calldata _newParameters
    ) external onlyController(_vault) {
        if (!_vaults[_vault]) {
            revert VaultNotExistError(_vault);
        }
        _vaultLoanValidator.upgradeVault(
            _vault,
            _vaultLoanValidators[_vault],
            _newParameters
        );
        address[] memory loans = _vaultLoanValidator.getAcceptedLoans();
        Vault(_vault).addAcceptedLoans(loans);
        _vaultLoanValidators[_vault] = address(_vaultLoanValidator);
    }

    function removeAcceptedLoan(address _vault, address _loan)
        external
        onlyController(_vault)
    {
        Vault(_vault).removeAcceptedLoan(_loan);
        emit LoanRemoved(_vault, _loan);
    }

    function withdrawRemainingFundsAndDestroy(address _vault) external {
        Vault(_vault).withdrawRemainingFundsAndDestroy(msg.sender);

        emit DestroyedVault(_vault);
    }

    function setProtocolFeeData(ProtocolFeeData calldata _data)
        external
        onlyOwner
    {
        _protocolFeeData = _data;
    }

    function getProtocolFeeData()
        external
        view
        returns (ProtocolFeeData memory)
    {
        return _protocolFeeData;
    }
}
