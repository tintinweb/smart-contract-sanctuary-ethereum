...
error TransferRoyaltyToCreatorFailed();
        ...
        for (uint i = 0; i < recipients.length; ++i) {
            ...
            if (receiver != address(0)) {
                ...
                if (!RoyaltyTransferSuccess) {
                    weth.deposit{value: royalty}();
                    bool WETHTransferSuccess = weth.transfer(receiver, royalty);
                    if (!WETHTransferSuccess) {
                        revert TransferRoyaltyToCreatorFailed();
                    }
                }
