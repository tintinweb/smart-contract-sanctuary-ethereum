...
contract MoonCatRescue {
  ...
  function rescueCat(bytes32 seed) activeMode returns (bytes5) {
    ...
    return _rescueCatId(catId);
  }

  function rescueCatId(bytes5 catId) onlyOwner activeMode returns (bytes5) {
    return _rescueCatId(catId);
  }


  function rescueCatIds(bytes5[] memory catIds) onlyOwner activeMode {
    for(uint i=0; i<catIds.length; i++) {
        _rescueCatId(catIds[i]);
    }
  }

  /* registers and validates cats that are found */
  function _rescueCatId(bytes5 catId) activeMode internal returns (bytes5) {
  ...
  function disableBeforeActivation() onlyOwner {
  ...
  function activateInTestMode() onlyOwner { //
