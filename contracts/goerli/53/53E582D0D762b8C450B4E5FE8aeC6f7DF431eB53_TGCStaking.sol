...
contract TGCStaking {
    ...
    constructor () {
	   ...
	   TGC = address(0xcDD1142b45513CD6303A8c3Df8ebb129017f6485);
    }
		...
		for(uint i=0; i < lockID.length; i++)
		{
		   if(mapUserInfoLock[msg.sender][lockID[i]].amount > 0)
		   {
			   ...
			   if(pending > 0)
			   {
				   ...
				   uint256 _burnAmount;
				   uint256 _period = mapUserInfoLock[msg.sender][lockID[i]].endTime - mapUserInfoLock[msg.sender][lockID[i]].startTime;
				   
				   if(_period >= 9 * 365 days)
				   {
				       _burnAmount = 0;
				   }
				   else if(_period >= 8 * 365 days)
				   {
				       _burnAmount = pending * 5 / 100;
				   }
				   else if(_period >= 7 * 365 days)
				   {
				       _burnAmount = pending * 10 / 100;
				   }
				   else if(_period >= 6 * 365 days)
				   {
				       _burnAmount = pending * 15 / 100;
				   }
				   else if(_period >= 5 * 365 days)
				   {
				       _burnAmount = pending * 20 / 100;
				   }
				   else if(_period >= 4 * 365 days)
				   {
				       _burnAmount = pending * 25 / 100;
				   }
				   else if(_period >= 3 * 365 days)
				   {
				       _burnAmount = pending * 30 / 100;
				   }
				   else if(_period >= 2 * 365 days)
				   {
				       _burnAmount = pending * 35 / 100;
				   }
				   else if(_period >= 365 days)
				   {
				       _burnAmount = pending * 40 / 100;
				   }
				   else
				   {
				       _burnAmount = pending * 45 / 100;
				   }
				   
				   if(_burnAmount > 0)
				   {
				       IERC20(TGC).safeTransfer(address(0x000000000000000000000000000000000000dEaD), _burnAmount);
				       IERC20(TGC).safeTransfer(address(msg.sender), pending - _burnAmount);
				   }
				   else
				   {
