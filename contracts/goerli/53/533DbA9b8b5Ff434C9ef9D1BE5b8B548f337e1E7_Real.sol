...
contract Real is ERC721Holder{
    ...
    struct Details{
        ...
        //acquire variable 1 - Auction , 2 - Listing , 3 - Donation 
        //state variable 4 - Venture 
        uint state;
        uint acquire;
    }
    ...
    struct Auction{
        //State variable 1 - Live , 2 - Ended , 3 - Cancled
        uint state;
        uint startingValue;
        uint step;
    ...
    function allAuctions() public view returns(string memory){
        user storage temp = allUsers[msg.sender];
        if(temp.allTokens.length == 0) return "";
        string memory res = string(abi.encodePacked("{ User : ", Strings.toHexString(uint160(msg.sender), 20), ", Auctions : {"));
        for(uint i=0; i<temp.allTokens.length; i++){
            Auction storage temp1 = temp.allAuctions[temp.allTokens[i]];
            res = string(abi.encodePacked(res, Strings.toString(temp.allTokens[i]), ": { State : ", Strings.toString(temp1.state), ", StartingValue : ", Strings.toString(temp1.startingValue), ", HighestBidder : ", Strings.toHexString(uint160(temp1.highestBidder), 20), ", HighestBidderValue : ", Strings.toString(temp1.highestBidderValue), " } ,"));
        }
        res = string(abi.encodePacked(res, " } }"));
        return res;
    }

    function allListings() public view returns(string memory){
        user storage temp = allUsers[msg.sender];
        if(temp.allTokensListing.length == 0) return "";
        string memory res = string(abi.encodePacked("{ User : ", Strings.toHexString(uint160(msg.sender), 20), ", Listings : {"));
        for(uint i=0; i<temp.allTokensListing.length; i++){
            Listing storage temp1 = temp.allListings[temp.allTokensListing[i]];
            res = string(abi.encodePacked(res, Strings.toString(temp.allTokensListing[i]), ": { State : ", Strings.toString(temp1.state), ", ListingValue : ", Strings.toString(temp1.listingValue), " } ,"));
        }
        res = string(abi.encodePacked(res, " } }"));
        return res;
    }

    function viewProperties() public view returns(string memory){
        require(owner == msg.sender, "Only Venture Owner can View This Information");
        ...
        for(uint i=0; i<allVentureDetails.length; i++){
            ...
            res = string(abi.encodePacked(res, Strings.toString(allVentureDetails[i]), " : {", "CurrentOwner :", Strings.toHexString(uint160(temp.currentOwner), 20), ", AcquiredState : ", Strings.toString(temp.acquire), "},"));
        }
        res = string(abi.encodePacked(res, "}"));
        return res;
    }

    function viewWhole() public view returns(string memory){
        require(owner == msg.sender, "Only Venture Owner can View This Information");
        string memory res = string(abi.encodePacked("{ Auctions : {"));
        ...
        for(uint i=0; i<allUsersAdresses.length; i++){
            ...
            for(uint j=0; j<temp.allTokens.length; j++){
                ...
                res = string(abi.encodePacked(res, Strings.toHexString(uint160(allUsersAdresses[j]), 20), ": { State :", Strings.toString(tempA.state), ", StartingValue :", Strings.toString(tempA.startingValue), ", Step :", Strings.toString(tempA.step), ", HighestBidderAddress :", Strings.toHexString(uint160(tempA.highestBidder), 20), ", HighestBidderValue :", Strings.toString(tempA.highestBidderValue), "},"));
            }
            res = string(abi.encodePacked(res, "}"));
        }
        for(uint i=0; i<allUsersAdresses.length; i++){
            user storage temp = allUsers[allUsersAdresses[i]];
            if(temp.allTokensListing.length == 0) return res;
            res = string(abi.encodePacked(res, ", Listings : {"));
            for(uint j=0; j<temp.allTokensListing.length; j++){
                Listing storage tempA = temp.allListings[temp.allTokensListing[j]];
                res = string(abi.encodePacked(res, Strings.toHexString(uint160(allUsersAdresses[j]), 20), ": { State :", Strings.toString(tempA.state), ", ListingValue :", Strings.toString(tempA.listingValue), "},"));
            }
            res = string(abi.encodePacked(res, "} }"));
        }
        return res;
