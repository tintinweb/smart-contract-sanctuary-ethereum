...
contract MajrCanon is Ownable, Pausable {
  ...
  */
  ...
  /// @notice The minimum percentage of the total supply of staked Majr IDs involved in voting (for and against votes combined) required for the proposal vote to pass and to be able to be queued for execution, expressed in basis points
  ...
  modifier validProposalId(uint256 _proposalId) {
    require(proposalCount.current() > _proposalId && _proposalId >= 0, "MajrCanon: Proposal ID is out of bounds.");
    _;
  }
  ...
  modifier validPreSetProposalId(uint256 _proposalId) {
    require(preSetProposalCount.current() > _proposalId && _proposalId >= 0, "MajrCanon: Pre set proposal ID is out of bounds.");
    _;
  }
  ...
  function vote(uint256 _proposalId, bool _support) external whenNotPaused validProposalId(_proposalId) {
    ...
    require(receipt.hasVoted == false, "MajrCanon: Voter already voted.");
  ...
  function votePreSet(uint256 _preSetProposalId, uint256 _option) external whenNotPaused validPreSetProposalId(_preSetProposalId) {
    ...
    require(preSetProposal.state == PreSetProposalState.Active, "MajrCanon: Voting is not active for this proposal.");
    ...
    require(preSetReceipt.hasVoted == false, "MajrCanon: Voter already voted.");
  ...
  /**
   * @notice Returns the receipt of the user's vote on the specified proposal
   * @param _proposalId uint256
  ...
  function propose(string calldata _title, string calldata _shortDescription, string calldata _longDescriptionURI) external payable whenNotPaused {
    require(msg.sender == owner() ? stakingContract.balanceOf(msg.sender) >= 0 : stakingContract.balanceOf(msg.sender) >= proposalThreshold, "MajrCanon: Insufficient number of staked Majr IDs to propose a proposal.");
    require(msg.sender == owner() ? msg.value >= 0 : msg.value == proposalCost, "MajrCanon: The exact fee for the proposal cost must be sent.");
  ...
  /**
   * @notice Approves the specified proposal and puts it into the active state
   * @param _proposalId uint256
   * @dev Only guardian council can call it
   */
  ...
  /**
   * @notice Places the specified proposal into the execution queue
   * @param _proposalId uint256
   * @dev Only guardian council can call it
   */
  ...
  /**
   * @notice Changes the specified proposal's state to executed and removes it from the execution queue
   * @param _proposalId uint256
   * @dev Only guardian council can call it
   */
  ...
  /**
   * @notice Changes the specified proposal's state to canceled
   * @param _proposalId uint256
   * @dev Only guardian council can call it
   */
  ...
  /**
   * @notice Returns the voting power of the specified voter (a square root of their staked balance in the MAJR's staking contract, rounded down to the nearest integer)
  ...
  /**
   * @notice Sets the new proposal cost fee
   * @dev Only guardian council can call it
   */
  ...
  /**
   * @notice Sets the new quorum votes required to pass a proposal
   * @dev Only guardian council can call it
   */
