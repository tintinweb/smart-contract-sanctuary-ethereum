...
abstract contract MerkleProof {
    bytes32 internal _freeMerkleRoot;
    bytes32 internal _wlMerkleRoot;

    // FM
    function _setFreeMerkleRoot(bytes32 merkleRoot_) internal virtual {
        _freeMerkleRoot = merkleRoot_;
    }

    function isValidFreeCount(address address_, uint256 freeCount, bytes32[] memory proof_) public view returns (bool) {
        bytes32 _leaf = keccak256(abi.encodePacked(address_, freeCount));
        for (uint256 i = 0; i < proof_.length; i++) {
            _leaf = _leaf < proof_[i] ? keccak256(abi.encodePacked(_leaf, proof_[i])) : keccak256(abi.encodePacked(proof_[i], _leaf));
        }
        return _leaf == _freeMerkleRoot;
    }
...
contract test is ERC721Enumerable, Ownable, MerkleProof, ReentrancyGuard {
  using Strings for uint256;
  string baseURI;
    //Project Settings
  ...
  function freeMint(uint256 _amount, uint256 vipCount, bytes32[] memory proof_) external virtual nonReentrant {
    ...
    require(isValidFreeCount(msg.sender, vipCount, proof_), "Invalid Vip Count!");
