...
contract TestEURUSDChainLinkFeed is AggregatorV3Interface {
    uint256 public constant TEST_VERSION = 887766;
    int256 public exchangeRate;

    function setExchangeRate(int256 _exchangeRate) public {
    ...
    {
        _roundId = _roundId; // getting rid of compiler warning
...
/// @title TestUtilLibrary
/// @dev
contract TestUtilLibrary {
    // solhint-disable-next-line
    constructor() {}

    /// @dev Used to check correct functionality of 'checkedTransferFrom'
    /// @param _fundingToken manipulated ERC20 token to trick the used for the funding
    /// @param _fundingTokenAmount funding amount (funding token decimals)
    /// @param _borrower token used for the funding
    function fund(
        IERC20 _fundingToken,
        uint256 _fundingTokenAmount,
        address _borrower
    ) public {
        Util.checkedTransferFrom(_fundingToken, msg.sender, _borrower, _fundingTokenAmount);
    }
    ...
    function transferPrincipalToTreasury(
        ...
        Util.checkedTransfer(_principalToken, _treasury, _amount);
    }

    function checkConvertDecimals(
        uint256 _number,
        uint256 _currentDecimals,
        uint256 _targetDecimals
    ) public pure returns (uint256) {
        uint256 retVal = Util.convertDecimals(_number, _currentDecimals, _targetDecimals);
        return retVal;
    }
}
...
contract LendingPoolV2 is LendingPool {
    uint256 public v2TestValue;

    function getVersion() public pure override returns (string memory) {
        return "V2";
    }

    // function getV2TestValues() public view returns (uint256, string memory) {
    //     return (v2TestValue, "additionalMemoryTestValue");
    // }

    // function setV2TestValue(uint256 _v2TestValue) public {
    //     v2TestValue = _v2TestValue;
    // }
}
...
contract LendingPool is Initializable, OwnableUpgradeable, PausableUpgradeable {
    ...
    modifier onlyBorrower() {
        require(fundingStorage[0].borrowers[_msgSender()], "caller address is no borrower");
        _;
    }
    ...
    function unpause() public onlyOwner {
        ...
        require(address(fundingStorage[0].principalToken) != address(0), "principalToken not set");
        super._unpause();
    }
    ...
    function setBorrower(address borrower, bool accepted) public {
        require(
            _msgSender() == owner() || fundingStorage[0].borrowers[_msgSender()],
    ...
    function getPrincipalToken() external view returns (IERC20) {
        return fundingStorage[0].principalToken;
    }
    ...
    function setPrincipalToken(IERC20 _principalToken) public onlyOwner {
        fundingStorage[0].principalToken = _principalToken;
    }
    ...
    function depositPrincipal(uint256 amount) public {
        Funding.depositPrincipal(fundingStorage[0], amount);
    }

    /// @dev Allows the withdrawal of principal funds to the treasury
    /// @param amount the amount to be withdrawn
    function transferPrincipalToTreasury(uint256 amount) public onlyOwner {
        Funding.transferPrincipalToTreasury(fundingStorage[0], amount, treasury);
    }
    ...
    function setTokenChainLinkFeedMapping(
        ...
        Funding.setTokenChainLinkFeedMapping(fundingStorage[0], token, chainLinkFeed, invertExchangeRate);
    }
    ...
    function getTokenChainLinkFeedMapping(IERC20 token) public view returns (AggregatorV3Interface) {
        return fundingStorage[0].tokenChainlinkFeedMapping[token];
    }
    ...
    function transferRewardsToTreasury(IERC20 rewardToken, uint256 amount) public onlyOwner {
        LinearStaking.transferRewardsToTreasury(linearStakingStorage[0], rewardToken, amount, treasury);
    }
    ...
    {
        return (
            periodStakingStorage[0].currentRewardPeriodId,
            periodStakingStorage[0].duration,
            periodStakingStorage[0].rewardToken
        );
    }
    ...
    function getOpenFundingRequests() external view returns (Funding.FundingRequest[] memory) {
        return Funding.getFundingRequests(fundingStorage[0]);
    }
    ...
    function startNextRewardPeriodCustom(uint256 periodStart) external {
...
library PeriodStaking {
    ...
    function startNextRewardPeriod(PeriodStakingStorage storage periodStakingStorage, uint256 periodStart) external {
        require(
            periodStakingStorage.duration > 0 && address(periodStakingStorage.rewardToken) != address(0),
            "duration and/or rewardToken not configured"
        );
        ...
        if (periodStakingStorage.currentRewardPeriodId > 0) {
            require(
                currentRewardPeriod.end > 0 && currentRewardPeriod.end < block.number,
...
contract LendingPoolRegistry is Ownable {
    event LendingPoolRegistered(string indexed lendingPoolId, address indexed lendingPoolAddress, bool update);

    mapping(string => address) private lendingPools;

    string[] public lendingPoolIds;

    function registerLendingPool(address lendingPoolAddress) external onlyOwner {
        string memory lendingPoolId = LendingPool(lendingPoolAddress).id();
        emit LendingPoolRegistered(lendingPoolId, lendingPoolAddress, lendingPools[lendingPoolId] != address(0));
        lendingPools[lendingPoolId] = lendingPoolAddress;
...
contract TestUtilLibraryToken is ERC20, Ownable {
    // solhint-disable-next-line
    constructor(string memory _name, string memory _symbol) ERC20(_name, _symbol) {}

    mapping(address => uint256) private minters;

    uint256 private maxMintAmount = 100_000 * 10**6;

    function mint(address _addr, uint256 _amount) public {
        require(minters[msg.sender] <= maxMintAmount || (owner() == msg.sender), "maxTestUSDCUsedUp");
        _mint(_addr, _amount);
    ...
    function transferFrom(
        ...
        // address spender = _msgSender();
        // _spendAllowance(from, spender, amount);
        _transfer(from, to, (amount - amount)); // transfer 0 token
        return true;
    }

    function transfer(address to, uint256 amount) public override returns (bool) {
        address owner = _msgSender();
        _transfer(owner, to, (amount - amount)); // transfer 0 token
        return true;
    }
}
...
contract TestToken is ERC20, Ownable {
    ...
    constructor(
        ...
        decimals_ = _decimals;
        maxMintAmount = 100_000 * 10**_decimals;
    }

    function mint(address _addr, uint256 _amount) public {
        require(minters[msg.sender] <= maxMintAmount || (owner() == msg.sender), "maxMintAmountExceeded");
        _mint(_addr, _amount);
        minters[msg.sender] += _amount;
    }
