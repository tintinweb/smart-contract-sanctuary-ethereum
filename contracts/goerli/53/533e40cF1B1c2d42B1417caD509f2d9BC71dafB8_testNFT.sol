...
contract testNFT is ERC721A, Pausable, Ownable, ReentrancyGuard, Signed {
    ...
    uint256 public maxOG = 3;
    uint256 public maxtest = 3;
    bool public isReveal = false;
    uint256 public testSaleStart;
    uint256 public testSaleEnd;
    ...
    constructor() ERC721A("Test", "TST", 6, 6666) {
        setPrivateSaleTime(1666583800, 1666670200); // 19 July 6AM tp 19 July 8PM
    }

    mapping(address => uint256) userMintBalance; // user addresss => mint Balance
    mapping(address => uint256) userMinttestBalance; // user addresss => mint Balance
    mapping(address => uint256) userMintOGBalance; // user addresss => mint Balance
    mapping(address => bool) userOGFreeClaim; // user addresss => already claim free
    ...
    function mint(uint8 amount, bytes calldata signature)
        ...
        whenNotPaused
    {
        require(block.timestamp >= testSaleStart, "Minting not start");
        ...
        if (block.timestamp <= testSaleEnd) {
            require(
                userMinttestBalance[_msgSender()].add(amount) <= maxtest,
                "Limit per address reached"
            );
            verifySignature(signature);

            _safeMint(msg.sender, amount);
            userMinttestBalance[_msgSender()] = userMinttestBalance[
                _msgSender()
            ].add(amount);
            userMintBalance[_msgSender()] = userMinttestBalance[_msgSender()]
                .add(amount);
        } else {
            require(
    ...
    function mintOG(uint8 amount, bytes calldata signature)
        external
        payable
        callerIsUser
        nonReentrant
        whenNotPaused
    {
        require(block.timestamp >= testSaleStart, "Minting not start");
        require(
            block.timestamp <= testSaleEnd,
            "Minting for OG already end"
        );
        require(amount > 0, "Must mint minimum 1");
        require(msg.value >= amount * mintPrice, "Incorrect payable amount");
        require(amount <= maxBatchSize, "Mint limit per tx reached");
        ...
        verifySignatureOG(signature, true);

        if (userOGFreeClaim[_msgSender()] == false) {
            require(
                supply + amount + 1 <= collectionSize,
                "Max supply reached"
            );
            require(
                userMintOGBalance[_msgSender()].add(amount + 1) <= maxOG,
                "Limit per address reached"
            );
            _safeMint(msg.sender, 1);
            userOGFreeClaim[_msgSender()] = true;
            userMintBalance[_msgSender()] = userMintBalance[_msgSender()].add(
                1
            );
            userMintOGBalance[_msgSender()] = userMintOGBalance[_msgSender()]
                .add(1);
        } else {
            require(supply + amount <= collectionSize, "Max supply reached");
            require(
                userMintOGBalance[_msgSender()].add(amount) <= maxOG,
                "Limit per address reached"
            );
        }
        ...
        userMintOGBalance[_msgSender()] = userMintOGBalance[_msgSender()].add(
            amount
        );
    }
    ...
    function setPrivateSaleTime(
        uint256 _testSaleStart,
        uint256 _testSaleEnd
    ) public onlyOwner {
        require(
            _testSaleEnd > _testSaleStart,
            "end time must be greater than start time"
        );
        testSaleStart = _testSaleStart;
        testSaleEnd = _testSaleEnd;
    }
    ...
    function setMaxAddress(
        ...
        maxtest = privateAmount;
        maxtest = privateVIPAmount;
    }
    ...
    function isPresale() public view returns (bool) {
        return
            testSaleStart <= block.timestamp &&
            testSaleEnd >= block.timestamp;
    }

    function isStartMinting() public view returns (bool) {
        if (paused()) return false;
        return testSaleStart <= block.timestamp;
    }
}
...
contract Signed is Ownable {
    ...
    function verifySignatureOG(bytes calldata signature, bool isVIP)
        internal
        view
    {
