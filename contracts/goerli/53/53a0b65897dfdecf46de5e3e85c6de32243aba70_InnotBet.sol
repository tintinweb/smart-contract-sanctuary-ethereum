...
contract InnotBet {
        ...
        returns (address, uint256)
    {
        ...
        _betInstances[id]._winner = winnerStruct(_winner, block.number, value);


        for (uint256 i = 0; i < _betInstances[id]._maxNumbers; i++) {
            uint256[] memory nothing;
            address better = _betInstances[id]._bets[i];
            _betInstances[id]._addressBets[better] = nothing;
            _betInstances[id]._bets[i] = address(0);
        }
