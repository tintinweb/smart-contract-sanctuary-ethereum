...
contract NiftyContract is ERC721A, Ownable, DefaultOperatorFilterer {
  ...
  uint256 public publicCost = 0.044 ether;
  ...
  function whitelistMint(uint256 _amount, Coupon memory _coupon) external payable mintConfigs(_amount) {
    ...
    require(numberMinted(msg.sender) + _amount <= preSaleMaxMint, "Out of mints");
    bytes32 digest = keccak256(
			abi.encode(CouponType.Allowlist, _amount, msg.sender)
		);
  ...
  function publicMint(uint256 _amount) external payable mintConfigs(_amount) {
    ...
    require(numberMinted(msg.sender) + _amount <= publicMaxMint, "Out of mints");
