...
contract OwnerLendMarketplace is OwnerMarketplace {
    constructor(IContractRegistry registry_) Marketplace(registry_) {}

    // *** The first case, OWNER creates rental offer ***
    function ownerCreatesRental(string memory registryTokenName_, uint256 rentTokenId_, uint64 validityTime_,
        uint64 rentDuration_, address[] memory allowedToRent_, address paymentAddress_, uint256 paymentAmount_, bool rentExtensionAllowed_
        ...
        rentOffers[++lastRentId] = RentOffer ({
            ...
            rewardConfiguration: _createRewardConfig()
        });
    ...
    function userAcceptsAvailableRental(uint256 rentId_) external whenNotPaused payable {
        return _userAcceptsAvailableRental(rentId_);
    }
