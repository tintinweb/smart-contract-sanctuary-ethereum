...
contract BlueCreators is Ownable, PaymentSplitter {
    ...
    mapping(uint => Subscription) private _subscribers;
    mapping(uint => Subscription) private _communities;
    ...
    **/
    function individualSubscribe(uint discord_user_id, Version version, uint month, string calldata code) external payable {
        ...
        bool has_code = bytes(code).length > 0;
        if(has_code) {
        ...
        uint month_bought = _subscribers[discord_user_id].month;
    ...
    * @param month The amount of month for the subscription plan
    **/
    function communitySubscribe(uint discord_guild_id, Version version, uint month) external payable {
        if(tx.origin != msg.sender) revert CallerIsContractError();
    ...
    **/
    function getSubscriber(uint discord_user_id) public view returns (Subscription memory subscriber) {
    ...
    **/
    function getCommunity(uint discord_guild_id) public view returns (Subscription memory community) {
