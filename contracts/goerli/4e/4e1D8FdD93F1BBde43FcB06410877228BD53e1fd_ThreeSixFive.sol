...
contract ThreeSixFive is ERC721A, Ownable {
    ...
    uint256 public SUPPLY = 366;
    uint256 public MAX_TX = 3;
    uint256 public MAX_ALLOW_LIST = 3;
    uint256 public PRICE = 0.08 ether;
    address public crossmintAddress;
    string private baseURI;
    bool public revealed;

    constructor() ERC721A("Three Six Five", "365") {}
    ...
    modifier mintCompliance(uint256 quantity) {
        if (quantity > MAX_TX) revert ExceedsMaxMint();
        if (totalSupply() + quantity > SUPPLY) revert SupplyExhausted();
    ...
    function mint(uint256 quantity) external payable mintCompliance(quantity) noContract {
        if (mintState != MintState.open) revert MintStateNotActive();
    ...
    function allowListMint(uint256 quantity, bytes32[] calldata proof)
        ...
        noContract
    {
        if (mintState != MintState.allowList) revert MintStateNotActive();
        bytes32 leaf = keccak256(abi.encodePacked(msg.sender));
        if (!MerkleProof.verifyCalldata(proof, merkleRoot, leaf)) revert NotOnAllowList();
        if (_numberMinted(msg.sender) + quantity > MAX_ALLOW_LIST) revert AllocationExhausted();
    ...
    function changeMaxTransaction(uint256 _newMax) external onlyOwner {
        MAX_TX = _newMax;
    }

    function changeMaxAllowList(uint256 _newMax) external onlyOwner {
        MAX_ALLOW_LIST = _newMax;
    }
