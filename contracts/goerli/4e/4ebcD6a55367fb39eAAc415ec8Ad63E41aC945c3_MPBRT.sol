...
contract MPBRT is ERC721MP, Ownable, ReentrancyGuard
{
    ...
    struct MintPassInformation
    {
        ...
        uint _Price;
        uint _TotalAvailableForPurchase;
        uint _UnixSaleStartTimeBrightlist;
        uint _UnixSaleStartTimePublic;
        uint _UnixUserRedeemTimestamp;
        uint _MaxPurchaseAmountPerTx;
        bytes32 _Root;
        bytes32 _MINTER_ROLE;
        bool _AllArtistPassesMinted; 
    }
    MintPassInformation MintPass = MintPassInformation(
        address(0),               // [0] -> _ArtBlocksMintingContract
        10,                       // [1] -> _ArtBlocksProjectID
        0.000018 ether,           // [2] -> _Price
        2,                        // [3] -> _TotalAvailableForPurchase
        1662490800,               // [4] -> _UnixSaleStartTimeBrightlist
        1662492600,               // [5] -> _UnixSaleStartTimePublic
        2662416400,               // [6] -> _UnixUserRedeemTimestamp
        1,                        // [7] -> _MaxPurchaseAmountPerTx
        0x0,                      // [8] -> _Root
        keccak256("MINTER_ROLE"), // [9] -> _MINTER_ROLE
        false                     // [10] -> _AllArtistPassesMinted
    );

    address public _ArtBlocksMintingContract;
    uint public _ArtBlocksProjectID;
    uint public _Price = 0.000018 ether;
    uint public _TotalAvailableForPurchase = 2;
    uint public _UnixSaleStartTimeBrightlist = 1662404400; //test
    uint public _UnixSaleStartTimePublic = 1662404400; //test
    // uint public _UnixSaleStartTimeBrightlist = 1662490800;
    // uint public _UnixSaleStartTimePublic = 1662492600;
    uint public _UnixUserRedeemTimestamp = 2662416400;
    uint public _MaxPurchaseAmountPerTx = 1;
    bytes32 public _Root;
    bytes32 public _MINTER_ROLE = keccak256("MINTER_ROLE");
    bool private _AllArtistPassesMinted;

    mapping(address=>bytes32) public _Role;
    ...
    event Purchased(address Recipient, uint Amount, uint Value);
    ...
    ************/
     ...
     */
    function Purchase(uint Amount, bool Status) public payable nonReentrant
    {
        require(block.timestamp >= _UnixSaleStartTimePublic, "MP: Sale Not Active For Public Purchases");
        require(Amount == _MaxPurchaseAmountPerTx, "MP: Amount Too High");
        if(_totalMinted() + Amount > _TotalAvailableForPurchase) { Amount = _TotalAvailableForPurchase - _totalMinted(); }
     ...
     */
    function PurchaseBrightList(uint Amount, bool Status, bytes32[] calldata Proof) public payable nonReentrant
    {
        require(block.timestamp >= _UnixSaleStartTimeBrightlist, "Sale Not Active For Brightlist Purchases");
        require(Amount == _MaxPurchaseAmountPerTx, "MP: Amount Too High");
        if(_totalMinted() + Amount > _TotalAvailableForPurchase) { Amount = _TotalAvailableForPurchase - _totalMinted(); }
        require(Amount > 0, "MP: Sold Out");
        require(_totalMinted() + Amount <= _TotalAvailableForPurchase, "MP: Overflow");
    ...
    {
        require(block.timestamp > _UnixUserRedeemTimestamp, "MP: Cannot Redeem Pass Unitl After The Start Time");
        if(_exists(TokenID))
        {
            ...
            _burn(TokenID);
            uint _MintedWorkNumber = 69420;
            // uint _MintedWorkNumber = IArtBlocks(_ArtBlocksMintingContract).purchaseTo(msg.sender, _ArtBlocksProjectID);
            emit LiveMintCompleteArtist(msg.sender, _ArtBlocksProjectID, TokenID, _MintedWorkNumber, _DelegateStatus[TokenID]);
        }
    }
    ...
    {
        require(block.timestamp > _UnixUserRedeemTimestamp, "MP: Cannot Redeem Pass Unitl After The Start Time");
        for(uint TokenID; TokenID < _TotalAvailableForPurchase; TokenID++)
        {
            if(_exists(TokenID))
            {
    ...
    function _LiveMintArtist(uint[] calldata TicketIDs) external onlyMinter
    {
        for(uint TokenID; TokenID < TicketIDs.length; TokenID++)
        {
            if(_exists(TicketIDs[TokenID]))
            {
                ...
                emit LiveMintCompleteArtist(Recipient, _ArtBlocksProjectID, TicketIDs[TokenID], _MintedWorkNumber, _DelegateStatus[TokenID]);
            }
        }
    }
    ...
    *******************/
    ...
    {
        require(_totalMinted() == _TotalAvailableForPurchase, "MPBRT: One-Time Run");
        for(uint _TicketID; _TicketID < _totalMinted(); _TicketID++)
        {
            if(_exists(_TicketID))
            {
                ...
                _burn(_TicketID);
                uint _MintedWorkNumber = IArtBlocks(_ArtBlocksMintingContract).purchaseTo(Recipient, _ArtBlocksProjectID);
                emit LiveMintCompleteArtist(Recipient, _ArtBlocksProjectID, _TicketID, _MintedWorkNumber, _DelegateStatus[_TicketID]);
            }
        }
        _mint(msg.sender, 1); // To Show Completed Mint Metadata On NFT Exchanges
        _AllArtistPassesMinted = true;
    }

    /**
     * @dev Mints Metadata Token Upon Sale Conclusion
     */
    function __OwnerMintMetadataToken() external onlyOwner
    {
        require(!_AllArtistPassesMinted, "LiveMint Artist Already Complete");
        require(_totalMinted() == _TotalAvailableForPurchase, "MP: Cannot Mint Metadata Token Before All Mint Passes Have Been Minted");
        require(_burnCounter == _totalMinted(), "MP: All Mint Passes Must Be Burned Before Metadata Token Can Be Minted");
        _mint(msg.sender, 1);
    }
     ...
     */
    function __NewArtBlocksProjectID(uint NewID) external onlyOwner { _ArtBlocksProjectID = NewID; }

    /**
     * @dev Changes The Max Purchase Amount
     */
    function __NewMaxPurchaseAmount(uint Amount) external onlyOwner { _MaxPurchaseAmountPerTx = Amount; }

    /**
     * @dev Changes The Unix Start Time Of The Brightlist Sale
     */
    function __NewUnixStartTimeBrightlist(uint Timestamp) external onlyOwner { _UnixSaleStartTimeBrightlist = Timestamp; }
    
    /**
     * @dev Changes The Unix Start Time Of The Public Sale
     */
    function __NewUnixStartTimePublic(uint Timestamp) external onlyOwner { _UnixSaleStartTimePublic = Timestamp; }

    /**
     * @dev Changes The Unix Start Time Of User LiveMint Redeems
     */
    function __NewUnixStartTimeUserMints(uint Timestamp) external onlyOwner { _UnixUserRedeemTimestamp = Timestamp; }

    /**
     * @dev Changes Unix Start Times Of Both Sales
     */
    function __NewUnixStartTimes(uint Public, uint Brightlist) external onlyOwner
    {
        _UnixSaleStartTimePublic = Public;
        _UnixSaleStartTimeBrightlist = Brightlist;
    }
     ...
     * @dev Adds A Minter
     */
    function __MinterAdd(address Minter) external onlyOwner { _Role[Minter] = _MINTER_ROLE; }
    ...
    *****************/
    ...
    function viewSaleState() public view returns ( 
        ...
        return(
            _UnixSaleStartTimePublic,
            _UnixSaleStartTimeBrightlist,
            _Price,
            _totalMinted()
        );
    }
    ...
    function viewWalletHoldingsAndDelegateStatus(address Wallet) public view returns (
        ...
        uint[] memory _TokenIDs = new uint[](_TotalAvailableForPurchase);
        bool[] memory _DelegateStatuses = new bool[](_TotalAvailableForPurchase);
        uint _Found;
        for(uint x; x < _TotalAvailableForPurchase; x++)
        {
            if(_exists(x))
            {
    ...
    **************/
    ...
    modifier onlyMinter
    {
        require(_Role[msg.sender] == _MINTER_ROLE, "OnlyMinter: Caller Is Not Approved BRT Minter");
        _;
    }
}
