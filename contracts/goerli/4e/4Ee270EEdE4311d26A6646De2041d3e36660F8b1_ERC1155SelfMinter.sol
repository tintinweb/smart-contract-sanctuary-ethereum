...
contract ERC1155SelfMinter is ERC1155, ReentrancyGuard, DefaultOperatorFilterer, Ownable {
    ...
    function initialize(
        ...
        uint256 _fees,
        uint256[] memory _limitPerWalletPerTier,
        uint256[] memory starts,
        uint256[] memory ends,
        bytes32[] memory merkleRoots
    ) public onlyAdminOrOwner(msg.sender) {
        if(initialized) revert AlreadyInitialized();
        if(starts.length != ends.length) revert ArrayLengthsDiffer();
        if(starts.length != merkleRoots.length) revert ArrayLengthsDiffer();
        if(starts.length != _limitPerWalletPerTier.length) revert ArrayLengthsDiffer();
        ...
        for (uint256 i = 0; i < starts.length; i++) {
            tiers[i + 1] = Tier(i + 1, starts[i], ends[i], merkleRoots[i], _limitPerWalletPerTier[i]);
        }
    ...
    function setSaleConfig(
        ...
        // Possibly need to remove the index shift + 1
        for (uint256 i = 0; i < starts.length; i++) {
            tiers[i + 1] = Tier(i + 1, starts[i], ends[i], merkleRoots[i], _limitPerWalletPerTier[i]);
        }
    }
    ...
    function partnerAdd(address partnerWallet, uint256 _sharePct) external onlyMultiSig(msg.sender) {
    ...
    function partnerUpdateSharePct(address partnerWallet, uint256 _sharePct) external onlyMultiSig(msg.sender) {
    ...
    function setTierMerkleRoots(bytes32[] memory _merkleRoots) public onlyAdminOrOwner(msg.sender) {
        for (uint256 i = 0; i < _merkleRoots.length; i++){
            tiers[i].merkleRoot = _merkleRoots[i];
        }
    }

    function setLimitPerWalletPerTier(uint256[] memory _limitPerWalletPerTier) public onlyAdminOrOwner(msg.sender) {
        for (uint256 i = 0; i < _limitPerWalletPerTier.length; i++){
            tiers[i].limitPerWalletPerTier = _limitPerWalletPerTier[i];
        }
    }
    ...
    function setTokenIdMaxSupply(uint256 _tokenId, uint256 _maxSupply) public onlyAdminOrOwner(msg.sender) {
        if(totalSupplyPerId[_tokenId] > _maxSupply) revert TotalSupplyGreaterThanMaxSupply();
        maxSupplyPerId[_tokenId] = _maxSupply;
    }
    ...
    function partnerTierMint(
        ...
        if (address(token) == address(0)) {
            _partnerAmount = _calcPartnerShare(msg.value, partners[partnerWallet].sharePct);
            _forjAmount = msg.value - _partnerAmount;


            (bool sent, bytes memory data) = treasuryWallet.call{value: _forjAmount}("");
            require(sent, "Failed to send Ether");
            ...
            _partnerAmount = _calcPartnerShare(mintFee, partners[partnerWallet].sharePct);
            _forjAmount = mintFee - _partnerAmount;

            ERC20(token).transferFrom(msg.sender, treasuryWallet, _forjAmount);
    ...
    function burn(address from, uint256[] memory ids, uint256[] memory amounts) public {
        ...
        for(uint256 i=0; i < ids.length; i++){
            ...
            totalSupplyPerId[ids[i]] -= amounts[i];
        }
    ...
    function _checks(
        ...
        Tier memory tier = tiers[tierNo];
        bytes32 leaf = keccak256(abi.encodePacked(mintAddress, tierNo));
        bytes32 root = tier.merkleRoot;

        if(!MerkleProof.verify(_merkleProof, root, leaf)) revert InvalidProof();
    ...
    function _mint(address to, uint256 id, uint256 amount, bytes memory data) internal override {
        ...
        if(totalSupplyPerId[id] + amount > maxSupplyPerId[id]) revert TotalSupplyGreaterThanMaxSupply();
        totalSupplyPerId[id] += amount;
        mintedPerAddress[to] += amount;
    }

    function _calcPartnerShare(uint256 _value, uint256 _sharePct) internal pure returns(uint256) {
        return _value * _sharePct / 100;
    }
}