...
contract ERC721AContract is ERC721A, Ownable, PaymentSplitter {
    ...
    function canMint(address _address, uint256 _quantity, bytes32[] memory _proof) private returns (bool) {
        ...
        if(_preSaleIsActive) {
            ...
            if (token.salePhase == 2) {
                uint16 mintedAmount = hasMinted[_address] + uint16(_quantity);
    ...
    function mintWithFiat(address _address, bytes32[] memory _proof, uint256 _quantity) public payable onlyFiatMinter {
        if (canMint(_address, _quantity, _proof)) {
