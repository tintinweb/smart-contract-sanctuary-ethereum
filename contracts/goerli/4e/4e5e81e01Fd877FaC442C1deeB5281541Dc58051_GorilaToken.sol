...
contract GorilaToken {
    ...
    event TaxBuyPercentageUpdated(uint256 percentage);
    event TaxSellPercentageUpdated(uint256 percentage);
    ...
    function modifyTaxBuyPercentage(uint256 _percentage) public onlyOwner {
        require(_percentage >= 0 && _percentage <= 100, "Tax percentage must be between 0 and 100.");

        taxBuyPercentage = _percentage;
        emit TaxBuyPercentageUpdated(_percentage);
    }

    function modifyTaxSellPercentage(uint256 _percentage) public onlyOwner {
        require(_percentage >= 0 && _percentage <= 100, "Tax percentage must be between 0 and 100.");

        taxSellPercentage = _percentage;
        emit TaxSellPercentageUpdated(_percentage);
    }
