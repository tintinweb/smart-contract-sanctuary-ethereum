...
import {Plugin, IPlugin} from "@aragon/osx/core/plugin/Plugin.sol";
...
contract veTokenVotingSetup is PluginSetup {
    ...
    /// @notice Thrown if token is not set.
    error TokenNotSet();
    ...
    {
        // TODO: Decode `_data` to extract the params needed for deploying and initializing `TokenVoting` plugin, and the required helpers

        (
            MajorityVotingBase.VotingSettings memory votingSettings,
            // TODO: reusing the same struct for both token and veToken, maybe create separate structs
        ...
        IERC20 token = IERC20(tokenSettings.addr);

        veToken _ve = new veToken(
            tokenSettings.name,
            tokenSettings.symbol,
            token
        );

        IVotesUpgradeable ve = IVotesUpgradeable(address(_ve));

        // TODO: should `token` take part in voting as well, or only allow `ve` to vote?
        ...
        // TODO: we dont need checks cause we deployed token, maybe add checks in the future
        // if (token != address(0)) {
        //     if (!token.isContract()) {
        ...
        // TODO: check if token is veToken, otherwise we need to wrap it
        // } else {
        //     revert TokenNotSet();
        // }

        helpers[0] = address(token);
...
// veToken is minted when a user stakes tokens in the DAO. It is used to track voting power.
// voting power depends on the amount of veToken and the duration of the stake. Max duration is 4 years.
// voting power = veToken * sqrt(duration)
// veToken is burned when a user unstakes tokens from the DAO.

contract veToken is ERC20, Votes {
    IERC20 public token;

    uint256 public constant MAX_DURATION = 4 * 365 days;
    uint256 immutable deployedAt;

    // locked tokens and durations
    struct LockedToken {
        uint256 amount;
        uint256 deadline;
    }
    ...
    event Lock(address indexed account, uint256 amount, uint256 deadline);
    ...
    function lock(
        ...
        require(account == msg.sender, "veToken: only owner can lock");

        require(deadline - block.timestamp <= MAX_DURATION, "veToken: invalid lock duration");
        require(deadline > block.timestamp, "veToken: deadline must be in the future");

        // lock tokens
        token.transferFrom(account, address(this), amount);

        // calculate voting power
        uint256 votingPower = calculateVotingPower(amount, deadline);

        console.log("veToken: lock", account, amount, votingPower);

        // mint veToken
        _mint(account, votingPower);
        _transferVotingUnits(address(0), account, votingPower);

        // add locked tokens
        lockedTokens[account].push(LockedToken(amount, deadline));

        emit Lock(account, amount, deadline);
    }

    function burn(address account, uint256 index) external virtual {
        require(account == msg.sender, "veToken: only owner can burn");

        LockedToken memory lockedToken = lockedTokens[account][index];

        require(lockedToken.amount > 0, "veToken: no locked tokens");
        require(lockedToken.deadline <= block.timestamp, "veToken: tokens are locked");

        // burn veToken
        uint256 votingPower = calculateVotingPower(
            lockedToken.amount,
            lockedToken.deadline
        );
        console.log("veToken: burn", account, lockedToken.amount, votingPower);
        _burn(account, votingPower);
        _transferVotingUnits(account, address(0), votingPower);

        // remove locked tokens
        delete lockedTokens[account][index];

        // unlock tokens
        token.transfer(account, lockedToken.amount);

        emit Burn(account, lockedToken.amount);
    }
    ...
    function calculateVotingPower(
        ...
        uint256 votingPower = amount * (deadline - deployedAt);
    ...
    function _getVotingUnits(
        ...
        console.log("veToken: _getVotingUnits", account, balanceOf(account));
