...
contract Registry is IRegistry, Ownable2StepUpgradeable {
    error RegistryError(string reason);
    ...
    function setZapDepositor(address _zap) external override {
        _checkOwner();
        if ((_zap == address(0))) {
            revert RegistryError("Cannot set zap to zero address");
        }
        zapDepositor = _zap;
    ...
    function setPrincipalTokenFactory(
        ...
        if ((_newPrincipalTokenFactory == address(0))) {
            revert RegistryError(
                "Cannot set principalToken factory to zero address"
            );
        }
    ...
    function addPrincipalToken(address _principalToken) external override {
        ...
        if (!(principalTokens.add(_principalToken))) {
            revert RegistryError("Error adding principal token");
        }
    ...
    function removePrincipalToken(address _principalToken) external override {
        ...
        if (!(principalTokens.remove(_principalToken))) {
            revert RegistryError("Error removing principal token");
        }
