...
contract ShibElonStakeMaster is Ownable{
    ShibElonStake public anyTokenStakeContract;
   
    address[] public StakeShibElonEarnShibElonContract;
    address[] public StakeShibElonEarnAnyContracts;
    address[] public StakeAnyEarnAnyContracts;
    ...
    function createStake(uint256 _type, address _stakeToken, address _rewardToken) public onlyOwner {
        ...
        anyTokenStakeContract = new ShibElonStake(_stakeToken,_rewardToken,msg.sender);
        
        if(_type==1){
            StakeShibElonEarnShibElonContract.push(address(anyTokenStakeContract));
        }else if(_type==2){
            StakeShibElonEarnAnyContracts.push(address(anyTokenStakeContract));
        }else if(_type==3){
            StakeAnyEarnAnyContracts.push(address(anyTokenStakeContract));
        }
    }
