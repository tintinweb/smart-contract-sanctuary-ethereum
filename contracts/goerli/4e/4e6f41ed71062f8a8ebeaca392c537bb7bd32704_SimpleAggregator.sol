...
abstract contract FlashLoanAggregator is FlashLoanAave {
    function flashLoan(bytes calldata _params) internal {
        _flashLoanAave(_params);
    }
}
...
interface ISwapRouter is IUniswapV3SwapCallback, IMulticallExtended {
...
//    }
    ...
    function _getCToken(IERC20 token) internal pure returns(ICERC20) { // goerli
        if (token.isETH()) {  // ETH
            return ICERC20(0x64078a6189Bf45f80091c6Ff2fCEe1B15Ac8dbde);
        }
        if (token == IERC20(0x2899a03ffDab5C90BADc5920b4f53B0884EB13cC)) {  // DAI
            return ICERC20(0x0545a8eaF7ff6bB6F708CbB544EA55DBc2ad7b2a);
        }
...
//    ISwapRouter public constant router = ISwapRouter(0x68b3465833fb72A70ecDF485E0e4C7bD8665Fc45);
    ISwapRouter public constant router = ISwapRouter(0xBa60b6e6fF25488308789E6e0A65D838be34194e); // goerli
...
//    address public provider = 0xB53C1a33016B2DC2fF3653530bfF1848a515c8c5;
    address public provider = 0x5E52dEc931FFb32f609681B8438A51c675cc232d; // goerli
    ...
    function _flashLoanAave(bytes calldata _flashLoanParams) public {
        (
...
contract OracleChainLink {
    ...
    function _getChainLinkOracleByToken(IERC20 token) private pure returns (IAggregator) {
        if (token == IERC20(0x2899a03ffDab5C90BADc5920b4f53B0884EB13cC)) {
            // DAI
            return IAggregator(0xD4a33860578De61DBAbDc8BFdb98FD742fA7028e); // eth usd
        }
...
contract ProtocolCompound is CompoundUtils, OracleChainLink {
    ...
    function _redeem(IERC20 token, uint256 amount) internal {
        ...
        console.log("cToken amount", amount);
        console.log("exchangeRateCurrent", cToken.exchangeRateCurrent());

        (uint error, uint liquidity, uint shortfall) = cToken.comptroller().getAccountLiquidity(address(this));
        console.log("liquidity", liquidity);

        uint code = cToken.redeem(amount);
        console.log("code", code);
        require(code == 0, "something went wrong");
    }
    ...
    function _repay(IERC20 token, uint256 amount) internal {
        ...
        if (token.isETH()) {
            ...
            token.universalApprove(address(cToken), amount);
            uint code = cToken.repayBorrow(amount);
            console.log("_repay code", code);
        }
    }
