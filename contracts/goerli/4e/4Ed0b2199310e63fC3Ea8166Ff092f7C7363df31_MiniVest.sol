...
/// @dev As expected. Experimental. COH test in prod. last min change in vesting schedule.
...
contract MiniVest is ReentrancyGuard {
    ...
    function withdrawAvailable(address _token) public virtual nonReentrant returns (bool s) {
        ...
        if (iv % k < block.timestamp) {
            delete  vestings[_token][_msgSender()];
            s = IERC20(_token).transfer(_msgSender(), (iv / k) * oneToken);
            require(s, "Transfer failed");

            emit VestingCompleted(_token, _msgSender(), iv / k);
        } else {
            // uint256 eligibleAmount = (iv / k) - ((((iv / k) / (iv % k))  *  ( (iv % k) -   (((iv % k)  - ((iv % k)  - block.timestamp) )))));
            uint256 remainingBalance = iv / k * oneToken;
            uint256 valuePerSecond = remainingBalance / (iv % k);
            uint256 secondsToGo = iv % k - block.timestamp;
            uint256 totalTIme = remainingBalance / valuePerSecond;
            uint256 elapsed = totalTIme - secondsToGo;

            uint256 eligibleAmount = elapsed * valuePerSecond;
            vestings[_token][_msgSender()] = (iv / k - (eligibleAmount / oneToken)) * k + (iv % k);

            s = IERC20(_token).transfer(_msgSender(), eligibleAmount);
            require(s, "Transfer failed");
