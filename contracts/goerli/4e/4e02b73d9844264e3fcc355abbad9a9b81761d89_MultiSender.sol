...
contract MultiSender is OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    uint256 public senderFee = 0.1 ether;//200 POM to move a transaction 
    ...
    function batchSendERC20(
        ...
        require(msg.value >= senderFee, "INSUFFICIENT_FUNDS");
    ...
    function sendERC721(address _NFTaddress,
        address[] memory _targets,
        uint256[] memory _ids) public payable  {
        require(msg.value >= senderFee, "INSUFFICIENT_FUNDS");
        require(_targets.length == _ids.length, "MultiSender: _recipients and _ids not equal");
        IERC721 token = IERC721(_NFTaddress);
        for (uint256 i = 0; i < _targets.length; i++) {
            token.safeTransferFrom(msg.sender, _targets[i], _ids[i]);
        }
    }

    function setsenderPrice(uint256 _price) public onlyOwner {
        senderFee = _price;
    }
