...
contract SBT_TYPE3{
  ...
  function mint(address owner,uint256 userid,uint256 salt,bytes memory signature) external returns(bytes32){
    ...
    emit registerd(owner,userid);
    return messagehash;
  }

  function regist(address owner,uint256 userid,uint256 salt,bytes memory signature) external returns(bytes32){
    bytes32 messagehash = keccak256(abi.encode(owner,userid,salt));
    require(verify(messagehash,signature),"INVAILED SIGNATUER");
    emit registerd(owner,userid);
    return messagehash;
