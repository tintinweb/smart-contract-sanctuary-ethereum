...
abstract contract Protected is Ownable, EIP712 {
    ...
    bytes32 internal constant _MINT_TYPEHASH =
        // prettier-ignore
        keccak256(
            "BuyWithMintParams(address seller,uint256 listingId,uint256 tokenId,uint256 editions,string meta,address currency,uint96 feeRate,uint256 price,uint256 startTime,uint256 endTime,address royaltyReceiver,uint96 royalty)"
        );
    bytes32 internal constant _BUY_TYPEHASH =
        // prettier-ignore
        keccak256(
            "BuyParams(address seller,address tokenAddress,uint256 listingId,uint256 tokenId,uint256 editions,address currency,uint96 feeRate,uint256 price,uint256 startTime,uint256 endTime)"
        );
    bytes32 internal constant _MINT_OFFER_TYPEHASH =
        // prettier-ignore
        keccak256(
            "SellWithMintParams(address buyer,uint256 offerId,uint256 tokenId,uint256 editions,string meta,address currency,uint96 feeRate,uint256 price,uint256 endTime,address royaltyReceiver,uint96 royalty)"
        );
    bytes32 internal constant _BUY_OFFER_TYPEHASH =
        // prettier-ignore
        keccak256(
            "SellParams(address buyer,address tokenAddress,uint256 offerId,uint256 tokenId,uint256 editions,address currency,uint96 feeRate,uint256 price,uint256 endTime)"
        );

    bytes32 internal constant _PLATFORM_TYPEHASH =
        keccak256("PlatformParams(address buyer,bytes sellerSignature)");
    bytes32 internal constant _PLATFORM_OFFER_TYPEHASH =
        keccak256("PlatformOfferParams(address seller,bytes buyerSignature)");
...
contract Marketplace is Protected {
    ...
    constructor(address nft, address _platform, address payable _vault) {
        internalNFT = nft;
        platform = _platform;
        vault = _vault;
    }
    ...
    /// @param platformData Address of the buyer & seller signature
    ...
    function buyWithMint(
        ...
        uint256 fee = (mintData.price * mintData.feeRate) / FEE_DENOMINATOR;
    ...
    function buy(
        ...
        uint256 fee = (buyData.price * buyData.feeRate) / FEE_DENOMINATOR;
    ...
    /// @param mintData Params of mint
    /// @param platformData Address of the seller & buyer signature
    /// @param platformSignature Platform's signature
    function sellWithMint(
        SellWithMintParams calldata mintData,
        PlatformOfferParams calldata platformData,
        bytes calldata platformSignature
    ) external onlySeller(platformData.seller) {
        _checkMoney(
            mintData.buyer,
            mintData.currency,
            mintData.price,
    ...
    function cancelAuctionWithMint(
        ...
        PlatformParams calldata platformData,
        bytes calldata platformSignature
    ) external onlySeller(mintData.seller) {
        _checkActive(mintData.startTime, mintData.endTime);
    ...
    function executeAuction(
        ...
        if (block.timestamp <= buyData.endTime)
            revert TooEarly(buyData.endTime, block.timestamp);

        if (toCancel || !IVault(vault).isBidExist(buyData.listingId)) {
            _cancelAuction(buyData, platformData, platformSignature);
        } else {
            _acceptBid(buyData, platformData, platformSignature);
        }
    ...
    function _pay(
        ...
        if (currency == address(0)) {
            ...
            IERC20(currency).safeTransferFrom(buyer, vault, fee);
        }
