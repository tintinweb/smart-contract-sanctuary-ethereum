...
contract AssetFactory {
    event NewAsset(uint assetId, string name);
    ...
    function _createAsset(string memory _name) internal {
        assets.push(Asset(_name));
        uint id = assets.length - 1;
        AssetToOwner[id] = msg.sender;
        ownerAssetCount[msg.sender]++;
        emit NewAsset(id, _name);
    }
}
...
contract AssetHelper is AssetFactory {
    modifier onlyOwnerOf(uint _zombieId) {
        require(msg.sender == AssetToOwner[_zombieId]);
        _;
    }

    function changeName(uint _assetId, string calldata _newName) external {
        require(msg.sender == AssetToOwner[_assetId]);
        assets[_assetId].name = _newName;
    }
...
contract AssetOwnership is AssetHelper, ERC721 {
    ...
    function transferFrom(
        ...
        require(
            AssetToOwner[_tokenId] == msg.sender ||
                assetApprovals[_tokenId] == msg.sender
        );
