...
// File contracts/GlangerStaking.sol
...
contract GlangerStaking is Ownable, ReentrancyGuard{
    ...
    function stakeBatch(uint256[] calldata _tokenIds) external nonReentrant {
    ...
    function withdraw(uint256 tokenId) external nonReentrant {
        ...
        remove(tokenId, stakers[msg.sender].tokenIds);
        stakers[msg.sender].timeOfLastUpdate = block.timestamp;
    }
    ...
    function withdrawBatch(uint256[] calldata _tokenIds) external nonReentrant {
        ...
        for (uint256 i; i< len; i++) {
            ...
            remove(i, stakers[msg.sender].tokenIds);
        }
        stakers[msg.sender].timeOfLastUpdate = block.timestamp;
    }
    ...
    function userStakeInfo(address _user) public view returns (uint256 _tokensStaked, uint256 _availableRewards){
        return (stakers[_user].tokenIds.length, availableRewards(_user));
    }
