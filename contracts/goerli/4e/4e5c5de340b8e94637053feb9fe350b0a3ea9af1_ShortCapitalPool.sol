...
contract CapitalPool {
    ...
    modifier notZeroAddress(address addr) {
        if (addr == address(0)) revert InputNotValid();
        _;
    }
    ...
    constructor(TPGToken _tpg, BondingSurface sbs, address _manager, address _reserve, ERC20 _asset) {
        surface = sbs;
        tpg = _tpg;
...
import "topos/CapitalPool.sol";
import {ShortBondingSurface} from "./ShortBondingSurface.sol";
import {ShortTPGToken} from "./ShortTPGToken.sol";

/// @title Short asset pool
/// @notice Pool holding an asset. Assets are deposited in exchange for TPG and in the
/// 				case of malfeasance will be used to cover shortfalls. The capital pool can
/// @custom:invariant asset.balanceOf(capitalPool) > 1
/// @custom:invariant asset.balanceOf(capitalPool) =< 1e36
contract ShortCapitalPool is CapitalPool {
    constructor(ShortTPGToken _stpg, ShortBondingSurface _sbs, address _manager, address _reserve, ERC20 _asset)
        CapitalPool(_stpg, _sbs, _manager, _reserve, _asset)
    {}
}
