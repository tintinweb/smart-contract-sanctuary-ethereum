 ...
 *
 */
contract RoundImplementation is AccessControlEnumerable, Initializable, ReentrancyGuard {
    // --- Libraries ---
    using Address for address;
    ...
    event Voted(uint256 indexed proposalId, uint256 votes, address indexed voter);

    event Distributed(uint256 indexed proposalId, uint256 matchedAmount);
    ...
    IDonationSBT public donationSBT;
    ...
    /// @notice Start Block as a checkpoint for prior votes
    uint256 public startBlock;

    /// @notice Tracker for proposals ID
    Counters.Counter private _proposalIdTracker;

    /// @notice Propsals for all the applications
    mapping(uint256 => MetaPtr) public proposals;
    ...
    function initialize(bytes calldata encodedParameters) external initializer {
        ...
        (
            ...
            DaoTokenInterface _daoToken,
            IDonationSBT _donationSBT,
            address _token,
    ...
    function totalProposals() public view returns (uint) {
        return _proposalIdTracker.current();
    }
    ...
    /// @param newMatchingAmount new matchingAmount
    function updateMatchingAmount(uint256 newMatchingAmount) external onlyRole(ROUND_OPERATOR_ROLE) {
        // slither-disable-next-line timestamp
        require(applicationsEndTime >= block.timestamp, "updateMatchingAmount: application time is ended");

        emit MatchingAmountUpdated(matchingAmount, newMatchingAmount);

        matchingAmount = newMatchingAmount;

    }

    /// @notice Update blacklist state of an Application (only by ROUND_OPERATOR_ROLE)
    /// @param proposalId application id
    /// @param blacklist new blacklist state
    function blacklistProposal(uint256 proposalId, bool blacklist) external onlyRole(ROUND_OPERATOR_ROLE) {
        require (isBlacklisted[proposalId] != blacklist, "blacklistProposal: Already set");

        isBlacklisted[proposalId] = blacklist;

        emit BlacklistUpdated(proposalId, blacklist);
    }
    ...
    /// @param applicationPtr appliction Ptr
    /// @param wallet address to receive matching amount
    function applyToRound(string memory applicationPtr, address wallet)
        external
    {
        uint256 id = totalProposals();
        _proposalIdTracker.increment();

        proposals[id] = MetaPtr(applicationPtr, wallet, 0);
        ...
        emit NewProjectApplication(id, applicationPtr, wallet);
    }
    ...
    function vote(bytes[] memory encodedVotes) external payable {
        ...
        require(receipt.hasVoted == false, "vote: voter already voted");

        // Total voting power as of applicationsStartTime Block
        uint256 daoTokenVotes = uint256(daoToken.getPriorBalance(msg.sender, startBlock));
        uint256 sbtVotes = IDonationSBT(donationSBT).balanceOf(msg.sender) * 50; // 50 credits for SBT holders

        uint256 totalVotes = daoTokenVotes + sbtVotes;
        ...
        for (uint256 i = 0; i < encodedVotes.length; i++) {
            ...
            require(
                ...
                "vote: Invalid proposal ID"
            );
            require(totalVotes >= _amount, "vote: Insufficient prior votes");

            unchecked {
                totalVotes = totalVotes - _amount;
            }

            MetaPtr storage proposal = proposals[_proposalID];
            uint256 quadraticVotes = Math.sqrt(_amount);

            proposal.forVotes = proposal.forVotes + quadraticVotes;
            
            receipt.proposals[i] = uint8(_proposalID);
            receipt.votes[i] = uint96(_amount);
    ...
    function distribute() external nonReentrant {
        // slither-disable-next-line timestamp
        require(block.timestamp > roundEndTime,"distribution: round is still active");  

        uint256 totalQuadraticVotes = _getTotalQuadraticVotes();

        for (uint256 i = 0; i < totalProposals(); i++) {
            MetaPtr memory proposal = proposals[i];
            ...
            if (token == address(0)) {
                ...
                AddressUpgradeable.sendValue(payable(proposal.wallet), matchedAmount);   
            } else {
