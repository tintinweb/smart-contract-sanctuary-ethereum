...
contract FineArtMFers is ERC721A, ReentrancyGuard{ 
	...
	bytes32 public giftClaimMerkleRoot;
	...
	function mintExtraGift(uint256 mintQty, bytes32[] calldata _merkleProof) external payable{
		if(sale == false) revert Paused();
		if(mintQty * cost != msg.value) revert EthValue();
		if(mintQty > 10) revert MintLimit();
		unchecked {mintQty += 1 + (mintQty / 5); }
		
		if(giftClaimed[msg.sender] == true) revert AccessDenied();

		if(!MerkleProof.verify(_merkleProof, giftClaimMerkleRoot, keccak256(abi.encodePacked(msg.sender)))) revert AccessDenied();
		if(mintQty + _totalMinted() > _maxSupply) revert MaxSupply();
