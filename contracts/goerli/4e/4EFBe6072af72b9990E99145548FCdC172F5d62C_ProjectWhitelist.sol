...
{
    event PassUnlocked(uint256 balance, string discordId);
    ...
    function initialize(        
        ...
        __UriManager_init_unchained("https://ipfs.io/ipfs/QmZxxqiUfHpq1bAHMaRLZcJEWsySgdUAMeF1FqGkJwFaeW/",".json");
    ...
    function burn(uint256[] calldata tokenIds, string memory discordId) public virtual {
        for(uint256 i = 0; i < tokenIds.length; i++) {
            _burn(tokenIds[i], true);
        }

        emit PassUnlocked(tokenIds.length, discordId);
    }
