...
contract FoxPayMetadataDescriptor is IERC3525MetadataDescriptor {
    ...
    function _generateSVGGiftMessage(
        string memory giftMessage_
    ) internal pure returns (string memory) {
        // bool isLong = true;
        (bytes memory former, bytes memory latter) = _splitString(giftMessage_, 5);
        return
            string(
                abi.encodePacked(
                    /* solhint-disable */
                    _generateSVGGiftMessagePart(former, '"618.455"'),
                    _generateSVGGiftMessagePart(latter, '"676.455"')
                    /* solhint-enable */
                )
            );
    }
    ...
    function _splitString(string memory str_, uint256 length_) internal pure returns (bytes memory, bytes memory) {
        uint256 totalLength = bytes(str_).length;
        bytes memory former = new bytes(length_);
        bytes memory latter = new bytes(totalLength - length_);

        for (uint256 i=0; i < length_; i++) {
            former[i] = bytes(str_)[i];
            if (i >= totalLength - length_) {
                latter[i] = bytes(str_)[i + length_];
            }
        }
