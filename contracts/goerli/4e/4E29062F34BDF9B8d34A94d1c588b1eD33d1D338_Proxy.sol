...
contract Proxy {
    ...
    constructor() {
        Coinflip c = new Coinflip(17348);
        _contract = address(c);
    }

    function callFlipCoin(uint256 _guess) public payable {
        Coinflip c = Coinflip(_contract);
        address caller = msg.sender;
        c.flipCoin(_guess, caller);
    }
...
contract Coinflip is VRFConsumerBaseV2 {
     ...
     */
    function flipCoin(uint256 _guess, address caller) public payable returns (uint256 requestId) {
        require(results[caller] != 42, 'Flip in progress.');
