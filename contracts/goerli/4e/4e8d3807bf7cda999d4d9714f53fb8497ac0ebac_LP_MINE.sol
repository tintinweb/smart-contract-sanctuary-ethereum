...
contract LP_MINE {
    ...
    address cake = 0x9781ec470226169f58Bf158F18f2A82cA43bDD45;
    ...
    function sellEggs() public {
        ...
        IERC20(cake).transfer(ceoAddress, fee2);
        IERC20(cake).transfer(ceoAddress2, fee-fee2);
        IERC20(cake).transfer(address(msg.sender), SafeMath.sub(eggValue,fee));
    }
    function buyEggs(address ref, uint256 amount) public {
        require(initialized);

        IERC20(cake).transferFrom(address(msg.sender), address(this), amount);
    ...
    function calculateEggSell(uint256 eggs) public returns(uint256) {
        return calculateTrade(eggs,marketEggs,IERC20(cake).balanceOf(address(this)));
    }
    function calculateEggBuy(uint256 eth,uint256 contractBalance) public view returns(uint256) {
        return calculateTrade(eth,contractBalance,marketEggs);
    }
    function calculateEggBuySimple(uint256 eth) public returns(uint256){
        return calculateEggBuy(eth,IERC20(cake).balanceOf(address(this)));
    }
