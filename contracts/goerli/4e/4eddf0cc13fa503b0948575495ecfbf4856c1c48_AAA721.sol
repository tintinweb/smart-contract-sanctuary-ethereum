...
// File src/module/RevealModule.sol

pragma solidity 0.8.13;

contract RevealModule is Ownable {
    string public AAA_PROVENANCE = "";
    uint256 public REVEAL_TIMESTAMP;

    constructor() {}

    function setProvenanceHash(string memory provenanceHash) public onlyOwner {
        AAA_PROVENANCE = provenanceHash;
    }
...
contract AAA721 is ERC721A, RevealModule, AllowList {
    ...
    function mint(uint256 quantity) external payable {
        ...
        for (uint256 i = 0; i < quantity; ++i) {
            _grantSubdomain(Strings.toString(_nextTokenId()), msg.sender);
        }
    }
    ...
    function computeNamehash(string memory _name) public pure returns (bytes32 namehash) {
        ...
        namehash = keccak256(
            abi.encodePacked(namehash, keccak256(abi.encodePacked('test')))
        );
        namehash = keccak256(
    ...
    function _grantSubdomain(string memory _subName, address recipient) internal {
        ...
        ENS(ENS_REGISTRY).setSubnodeOwner(ensNameHash, subName, recipient);
    }
