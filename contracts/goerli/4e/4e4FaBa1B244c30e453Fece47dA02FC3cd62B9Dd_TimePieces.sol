...
contract TimePieces is Satoshigoat {
	...
	// NFT721 constant public timeNft = NFT721(0xdFFA952cF0e7f297C855C196eB959Bf026C82A4e);//mainnet
	...
	constructor() Satoshigoat("GUD & EVOL x TIME", "", "ipfs://") 
	{
    	...
    	// purchasePrice = 0.33 ether;
		purchasePrice = 0.00001 ether;
	}
		...
		returns (string memory) 
	{
		...
		return string(abi.encodePacked(_baseURI(), getHash(tid)));
	}
	...
	//@dev Purchase either a GUD or EVOL version
	function purchase(address payable to, bool good)
		external
		payable
		nonReentrant
		enoughSupply(1)
		purchaseArgsOK(to, msg.value, 1)
	{
		...
		//check that this address owns a timepieces nft or plug 
		if (sugoiBalance == 0 && plugBalance == 0)
			revert DataError("need to own Plug or TIMEPieces x LOGIK");
		...
		//no plugs => `to` needs to own TIMEPieces x LOGIK
		bool found;
		uint i;
		for (i = 0; i < 2; i++) {
		...
		if (!found) 
			revert DataError("not owner of TIMEPieces x LOGIK NFT");
	...
	function burn(uint256 tid) external
	{
		...
		if (!_stringsEqual(getHash(tid), _evolHash))
	...
	function getHash(uint256 tid) internal view returns (string memory)
	{
		if (_gudOrEvol[tid]) return _gudHash;
		else return _evolHash;
	}

	//@dev Change a token id in `_timeNftIds`
	function setTokenIDAtIndex(uint256 newTokenId, uint16 idx) external isSquad 
	{
		...
		if (0 > idx || idx >= 14) 
			revert DataError("idx out of bounds");
	...
	function setGudHash(string calldata newHash) external isSquad notEqual(_gudHash, newHash)
	{
		_gudHash = newHash;
	}

	//@dev [BACKUP METHOD] Update `_evolHash`
	function setEvolHash(string calldata newHash) external isSquad notEqual(_evolHash, newHash)
	{
		_evolHash = newHash;
	}
