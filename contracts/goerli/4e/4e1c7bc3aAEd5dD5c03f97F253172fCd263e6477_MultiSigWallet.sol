...
contract MultiSigWallet {
    ...
    function submitErc20MultiTransaction(address tokenAddress, bytes[] memory methodData) public onlyOwner {
        uint txIndex = transactions.length;
        ...
        for (uint256 j = 0; j < methodData.length; j++) {
            ...
            emit SubmitTransaction(msg.sender, txIndex, tokenAddress, 0, methodData[j]);
        }
    }
}