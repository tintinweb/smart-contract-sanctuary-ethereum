...
contract RainshowerUniswapModule is Modifiers
{
    ...
    function onFundWithTokens () external returns(bool res) {
        ...
        res = IERC20Minimal(tokensForBorrow.tokenAddress).transferFrom(msg.sender, address(this), tokensForBorrow.ammount);
        return res;
    }
    ...
    function onFundWithMarginAndOpen (uint256 _extraMargin) external returns(uint256) {
        ...
        return swapExactInputSingle(tokensForBorrow.tokenAddress, swap.baseToken, tokensForBorrow.ammount, swap.poolFee, address(this), swapRouter);
    }
    ...
    function onClose () external onlyAddress(borrowTaker) returns(bool) {
        ...
        swapExactOutputSingle(swap.baseToken, tokensForBorrow.tokenAddress, tokensForBorrow.ammount, swap.poolFee, tokenBalance, address(this), swapRouter);

        tokenBalance = IERC20Minimal(swap.baseToken).balanceOf(address(this));
        IERC20Minimal(swap.baseToken).transfer(borrowTaker, tokenBalance);
        tokenBalance = IERC20Minimal(tokensForBorrow.tokenAddress).balanceOf(address(this));
        IERC20Minimal(tokensForBorrow.tokenAddress).transfer(borrowMaker, tokenBalance);

        return true;
    }

    function onLiquidation () external pure returns(bool) {
        return false;
    }
