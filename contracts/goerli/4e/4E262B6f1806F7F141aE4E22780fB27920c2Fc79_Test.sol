...
contract Test {
    ...
    function send(address sender) external payable {
...
abstract contract FxBaseRootTunnel is Const {
    ...
    function init(address _checkpoint, address _fxRoot, address _bridge) external {
        require(address(checkpointManager) == address(0) && address(fxRoot) == address(0) && address(manager) == address(0), "FxBaseRootTunnel: already set");
    ...
    function _validateAndExtractMessage(bytes memory inputData) internal returns (bytes memory) {
        ...
        try manager.exit(inputData) {
            ...
            message = abi.encode(WITHDRAW_TOKEN, abi.encode(withdrawer, logRLPList[2].toUint()));
