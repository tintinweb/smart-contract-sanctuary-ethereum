...
error DopeCatYachtClubNft__BreedIndexOutOfRange();
...
contract DopeCatYachtClubNft is VRFConsumerBaseV2, ERC721URIStorage, Ownable {
    ...
    mapping(bytes32 => uint8) s_nftComboHashToImageId;

    event NftRequested(uint256 indexed _requestId, address _requester);
    event NftMinted(Breed _dogBreed, address _dogOwner);
    ...
    // function generateDopeCatCombo() {
    //     for (uint8 = 0; i < COLLECTION_SIZE; i++) {
    //         console.log()
    //     }
    // }
        ...
        returns (Breed)
    {
        ...
        revert DopeCatYachtClubNft__BreedIndexOutOfRange();
    }
