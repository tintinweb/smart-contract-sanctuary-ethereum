...
import "@chocolate-factory/contracts/whitelist/WhitelistUpgradable.sol";
    ...
    CustomPaymentSplitterUpgradeable
{
   ...
   function initialize(
    ...
    __UriManager_init_unchained(
      'https://nft-assets.app.saishumiraishoujo.com/19sk1z1/jsons/',
      '.json'
    );
    ...
    // Black
    setPrice(1, 0.044 ether);
    updateBalanceLimit(1, 5);

    // Platinum
    setPrice(2, 0.054 ether);
    updateBalanceLimit(2, 4);
    
    // Gold
    setPrice(3, 0.074 ether);
    updateBalanceLimit(3, 3);

    // Silver
    setPrice(4, 0.089 ether);
    updateBalanceLimit(4, 2);

    // Bronze
    setPrice(5, 0.099 ether);
    updateBalanceLimit(5, 1);

    for(uint256 i = 0; i < roots.length; i++) {
        updateMerkleTreeRoot(uint8(i+1), roots[i]);
    }
  }

  function _startTokenId() pure internal override(ERC721AUpgradeable) returns (uint256) {
    return 1;
  }

    function phaseOneMint(uint256 amount_, bytes32[] calldata proof_, uint8 stage_)
        external
        payable
        onlyWhitelisted(stage_, msg.sender, proof_)
    {
        require(stage == 1, "Phase One not live");
        require(stage_ == 1 || stage_ == 2 || stage_ == 3, "Black || Platinum || Gold Only");
    ...
    function phaseTwoMint(uint256 amount_, bytes32[] calldata proof_, uint8 stage_)
        external
        payable
        onlyWhitelisted(stage_, msg.sender, proof_)
    {
        require(stage == 2, "Phase Two not live");
        require(stage_ == 4 || stage_ == 5, "Silver || Bronze Only");
        uint8 _stage = uint8(stage_);
        _increaseBalance(_stage, msg.sender, amount_);
        _callMint(msg.sender, amount_);
        _handlePayment(amount_ * price(_stage));
