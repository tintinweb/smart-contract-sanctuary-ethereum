...
contract NOX_Uniques is ERC1155DUpgradeable, IERC2981 {
     ...
     * @param _contractURI Contract URI of the contract.
     */
    function initialize(string memory _metadata, string memory _contractURI) virtual public initializer {
        ...
        for (uint256 i = 0; i < 14/*MAX_SUPPLY*/; i++) {
            unchecked {id++;}
     ...
     */
    function supportsInterface(bytes4 interfaceId) public view virtual override (ERC1155DUpgradeable/*, IERC2981*/) returns (bool) {
        return (
     ...
     */
    function setURI(string memory _uri) virtual public onlyAdmin {
        require(keccak256(abi.encodePacked(contractURI)) != keccak256(abi.encodePacked(_uri)), "Error: uri already setted");
        contractURI = _uri;
    }

    /**
     * @notice Setter to change the ContractURI.
     * @param _uri True or false, depends on the new desired state.
     */
    function setContractUri(string memory _uri) virtual public onlyAdmin {
        require(keccak256(abi.encodePacked(contractURI)) != keccak256(abi.encodePacked(_uri)), "Error: uri already setted");
        contractURI = _uri;
    }
 ...
 */
contract ERC1155DUpgradeable is Initializable, ContextUpgradeable, ERC165Upgradeable, IERC1155Upgradeable, IERC1155MetadataURIUpgradeable {
