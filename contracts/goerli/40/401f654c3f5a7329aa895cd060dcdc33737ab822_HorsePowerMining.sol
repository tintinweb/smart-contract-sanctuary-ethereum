...
contract HorsePowerMining is Ownable {
    ...
    function startMining(address _tokenAddress, uint256 _tokenId) external initialized(_tokenAddress) {
        ...
        require(!minerMapping[_msgSender()][_tokenAddress][_tokenId].isActive, "Token already mining");

        // Transfer token from _msgSender() to this contract
        OmnihorseNFTs[_tokenAddress].transferFrom(_msgSender(), address(this), _tokenId);

        // Push Miner struct instance to user's miner mapping 
        minerMapping[_msgSender()][_tokenAddress][_tokenId] = Miners(_tokenAddress, _tokenId, block.timestamp, block.timestamp, _msgSender(), true);

        // emit event for off chain tracking
        emit miningStarted(_tokenAddress, _tokenId, _msgSender());
    }
    ...
    function endMining(address _tokenAddress, uint256 _tokenId) external {
        ...
        require(minerMapping[_msgSender()][_tokenAddress][_tokenId].isActive, "Token is not mining");

        // Reset minerMapping data
        minerMapping[_msgSender()][_tokenAddress][_tokenId].startTime = 0;
        minerMapping[_msgSender()][_tokenAddress][_tokenId].isActive = false;

        // Return NFT to user's wallet
        OmnihorseNFTs[_tokenAddress].transferFrom(address(this), _msgSender(), _tokenId);

        // emit event for off chain tracking
        emit miningEnded(_tokenAddress, _tokenId, _msgSender());
    }
    ...
    function getReward(address _miner, address _tokenAddress, uint256 _tokenId) public view returns(uint256){
        ...
        uint256 baseReward = (block.timestamp - minerMapping[_miner][_tokenAddress][_tokenId].lastClaimed) * multiplier;
        if(boost == 0) {
            return baseReward;
        }
     ...
     */
    function withdrawReward(address _tokenAddress, uint256 _tokenId) external {
        uint256 reward = getReward(_msgSender(), _tokenAddress, _tokenId);
        
        require(reward > 0, "Reward is 0");

        minerMapping[_msgSender()][_tokenAddress][_tokenId].lastClaimed = block.timestamp;
        bool success = OMH.transfer(_msgSender(), reward);
    ...
    function addLiquidity(uint256 amount) external onlyOwner {
        OMH.transferFrom(_msgSender(), address(this), amount);
    }

    function setMultiplier(uint256 mult) external onlyOwner {
        multiplier = mult;
    }
