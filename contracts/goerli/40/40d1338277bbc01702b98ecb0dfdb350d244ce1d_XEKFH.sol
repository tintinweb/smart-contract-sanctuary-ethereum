 ...
 */
contract PBTBase is ERC721, IPBT {
    using ECDSA for bytes32;

    mapping(uint256 => bool) auctionTokenId;
    ...
    function _transferTokenWithChip(
        ...
        uint256 tokenId = _getTokenDataForChipSignature(
            signatureFromChip,
            blockNumberUsedInSig
        ).tokenId;
    ...
    /* -----------------------------------------
      オークションなどで転送を許可したいときに設定
    -------------------------------------------- */
    function _setAuctionMode(uint256 tokenId, bool boolean) internal virtual {
        auctionTokenId[tokenId] = boolean;
    }

    function approve(address to, uint256 tokenId) public virtual override {
        require(
            auctionTokenId[tokenId] == true,
