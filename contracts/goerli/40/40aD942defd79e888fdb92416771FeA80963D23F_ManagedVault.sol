...
/// @notice Thrown when given performanceFeeBPS is higher than maximum
error InvalidPerformanceFeeBPS();

/// @notice Thrown when given managementFeeBPS is higher than maximum
error InvalidManagementFeeBPS();

/// @notice Thrown when claim period hasn't started yet
/// @param claimPeriodStartTimestamp First timestamp of given epochs claim period
/// @param currentTimestamp Current timestamp
error EarlyClaim(uint claimPeriodStartTimestamp, uint currentTimestamp);

/// @notice Thrown when claim period has passed
error LateClaim();
...
contract ManagedVault is Initializable, OwnableUpgradeable, ERC20Upgradeable, IManagedVault {
    ...
    // Essentially guarantees that performance fees are updated before mints take place.
    //  * PerformanceFeeUpdated: Requires PricesSet. Sets to PerformanceFeeUpdated.
    //  * PricesSet: Sets to PricesSet.
    enum State { PerformanceFeeUpdated, PricesSet }

    /* ============ CONSTANTS ============ */
    
    // 4% in basis points
    int256 constant MAX_MANAGEMENT_FEE_BPS = 400;
    
    // 40% in basis points
    int256 constant MAX_PERFORMANCE_FEE_BPS = 4000;
    
    // 100% in basis points
    int256 constant HUNDRED_PERCENT_BPS = 10000;

    /* ============ State Variables ============ */
    State state;

    // Management fee
    int256 public accumulatedManagementFee;
    int256 public managementFeeBPS;
    uint256 public updateManagementFeeTimestamp;

    // Performance fee
    int256 public accumulatedPerformanceFee;
    int256 public performanceFeeBPS;
    uint256 public updatePerformanceFeeLastVaultPrice;
    uint256 public performanceFeeClaimFirstTimestamp;
    uint256 public performanceFeeClaimTimeCooldown;
    uint256 public performanceFeeClaimTimeWindowLength;

    // Redemptions
    uint256 public previousEpochsVaultTokenTotalAmount;
    uint256 public currentEpochVaultTokenTotalAmount;
    ...
    // address => epoch => total vault token amount for the address and the epoch
        ...
        initializer
    {
        ...
        // Set the updatePerformanceFeeLastVaultPrice right if the contract is updated
        if (updatePerformanceFeeLastVaultPrice == 0 && vaultPrice != 0) {
            updatePerformanceFeeLastVaultPrice = vaultPrice;
        }
    }

    /* ============ Public View Functions ============ */

    // TODO: Integrate previousEpochsVaultTokenTotalAmount and probably another variable such as withdrawnVaultTokenTotalAmount as well.
    function totalSupply() public view virtual override returns (uint256) {
        if (accumulatedPerformanceFee > 0) {
            return super.totalSupply() + uint(accumulatedManagementFee) + uint(accumulatedPerformanceFee) - withdrawableVaultTokenTotalAmount;
        } else {
            return super.totalSupply() + uint(accumulatedManagementFee) - withdrawableVaultTokenTotalAmount;
        }
    }
    ...
    /**
     * @notice Ends the current epoch and starts a new one.
 
     * @dev Since a new epoch is initiated, redeem() calls ensuing this function's execution will be applied for the new epoch. This effectively means that redemption amounts for the previous epoch are now locked-in, new redeems are not added, existing ones can no longer be modified.
    */

    function rollEpoch() external onlyOwner {
        epochs.push(Epoch({
            vaultTokenTotalAmount: currentEpochVaultTokenTotalAmount,
            vaultTokenPrice: 0,
            redemptionTokenPrice: 0
        }));
        previousEpochsVaultTokenTotalAmount += currentEpochVaultTokenTotalAmount;
        currentEpochVaultTokenTotalAmount = 0;
    }
    
    /**
     * @notice Calculates and transfers the amount of tokens required to fulfill redemptions for the last epoch. 
     *         Afterwards, redeem amounts for that epoch are enabled for withdrawal.
     * @dev Prior to execution of this function, the vault manager must verify that the vault owner account is in possesion of the amount of required redemptionTokens and the vault contract has necessary token allowance from the owner. Otherwise it will revert with TransferFailed().
    */

    function settleLastEpoch() external onlyOwner {
        uint256 epochsLength = epochs.length;
        require(0 < epochsLength);
        Epoch storage lastEpoch = epochs[epochsLength - 1];
        require(lastEpoch.vaultTokenPrice == 0);

        uint256 numerator = lastEpoch.vaultTokenTotalAmount * vaultPrice * (uint256(HUNDRED_PERCENT_BPS) - redemptionFeeBPS);
        uint256 denominator = tokenPrices[redemptionToken] * uint256(HUNDRED_PERCENT_BPS);

        uint256 totalEpochRedemptionTokenAmount = numerator / denominator;

        bool success = redemptionToken.transferFrom(owner(), address(this), totalEpochRedemptionTokenAmount);
        if (!success) {
            revert TransferFailed();
        }

        lastEpoch.vaultTokenPrice = vaultPrice * (uint256(HUNDRED_PERCENT_BPS) - redemptionFeeBPS) / uint256(HUNDRED_PERCENT_BPS);
        lastEpoch.redemptionTokenPrice = tokenPrices[redemptionToken];
        withdrawableVaultTokenTotalAmount += lastEpoch.vaultTokenTotalAmount;
    }
     ...
     *
     * @param nativeState The new state of the whitelist.
     */
    function setNativeDepositState(bool nativeState) external onlyOwner {
        require(nativeDepositState != nativeState);

        nativeDepositState = nativeState;

        emit SetNativeDepositState(nativeState);
    }
     ...
     * @param tokenState The new state of the whitelist.
     */
    function setTokenDepositState(IERC20 tokenAddress, bool tokenState) external onlyOwner {
        require(tokenDepositStates.contains(address(tokenAddress)) != tokenState);
        ...
        emit SetTokenDepositState(tokenAddress, tokenState);
    }
    ...
    function setPrices(uint blockNumber, uint vaultPrice_, uint nativePrice_, IERC20[] calldata tokenAddresses, uint[] calldata tokenPrices_) external onlyOwner {
        ...
        if (vaultPrice != vaultPrice_) {
            // Set the updatePerformanceFeeLastVaultPrice right if the contract is just deployed
            if (updatePerformanceFeeLastVaultPrice == 0 && vaultPrice == 0) {
                updatePerformanceFeeLastVaultPrice = vaultPrice_;
            }
    ...
    function mint(uint maxMints) external onlyOwner returns (uint mintCount) {
        require(state == State.PerformanceFeeUpdated, "Update performance fee first.");
    ...
    function setRedemptionToken(IERC20 _redemptionToken) external onlyOwner {
        redemptionToken = _redemptionToken;
    }

    function setRedemptionFeeBPS(uint256 _redemptionFeeBPS) external onlyOwner {
        redemptionFeeBPS = _redemptionFeeBPS;
    }

    function setPerformanceFeeBPS(int256 _performanceFeeBPS) external onlyOwner {
        if (_performanceFeeBPS > MAX_PERFORMANCE_FEE_BPS) revert InvalidPerformanceFeeBPS();
        performanceFeeBPS = _performanceFeeBPS;
    }

    function setManagementFeeBPS(int256 _managementFeeBPS) external onlyOwner {
        if (_managementFeeBPS > MAX_MANAGEMENT_FEE_BPS) revert InvalidManagementFeeBPS();
        managementFeeBPS = _managementFeeBPS;
    }

    function setPerformanceFeeClaimFirstTimestamp(uint256 timestamp) external onlyOwner {
        performanceFeeClaimFirstTimestamp = timestamp;
    }

    function setPerformanceFeeClaimTimeCooldown(uint256 time) external onlyOwner {
        performanceFeeClaimTimeCooldown = time;
    }

    function setPerformanceFeeClaimTimeWindowLength(uint256 time) external onlyOwner {
        performanceFeeClaimTimeWindowLength = time;
    }

    function claimPerformanceFee(uint256 epoch, address claimAddress, uint256 claimAmount) external onlyOwner {
        uint256 claimPeriodFirstTimestamp = performanceFeeClaimFirstTimestamp + performanceFeeClaimTimeCooldown * epoch;
        if(claimPeriodFirstTimestamp > block.timestamp) revert EarlyClaim(claimPeriodFirstTimestamp, block.timestamp);
        if(block.timestamp > claimPeriodFirstTimestamp + performanceFeeClaimTimeWindowLength) revert LateClaim();
        
        if (int256(claimAmount) <= accumulatedPerformanceFee) {
            accumulatedPerformanceFee -= int256(claimAmount);
            _mint(claimAddress, claimAmount);
        }
    }

    function claimManagementFee(address claimAddress, uint256 claimAmount) external onlyOwner {
        require(int256(claimAmount) <= accumulatedManagementFee, "claimAmount more than accumulated");
        accumulatedManagementFee -= int256(claimAmount);
        _mint(claimAddress, claimAmount);
    }
    ...
    /** 
     * @notice Updates the total performance fee the vault has accumulated.
     *
     * @dev Only updates the storage variable that dictates max. claimable amount by claimPerformanceFees() where fees are actually crystallized by the vault manager.
     *      Should always enforce updating price data before each execution. 
     */

    function updatePerformanceFee() external {
        require(state == State.PricesSet, "Set prices first");
        
        int256 vaultPriceDifference = int(vaultPrice) - int(updatePerformanceFeeLastVaultPrice);
        int256 numerator = int(totalSupply()) * vaultPriceDifference * performanceFeeBPS;
        int256 denominator = int(updatePerformanceFeeLastVaultPrice) * HUNDRED_PERCENT_BPS;
        int256 newPerformanceFee = numerator / denominator;

        if (newPerformanceFee != 0) {
            uint nav = totalSupply() * vaultPrice;
            accumulatedPerformanceFee += newPerformanceFee;
            vaultPrice = nav / totalSupply();
        }

        state = State.PerformanceFeeUpdated;
        updatePerformanceFeeLastVaultPrice = vaultPrice;
    }

    /** 
     * @notice Updates the amount of tokens to be redeemed by the caller in the current epoch.
     *         Redeemed tokens are withdrawable using withdraw() when the next epoch begins and the settlement for the epoch in which redeem() was called is finalized.
     *
     * @dev Can be called more than once during the same epoch to override amount to redeem for that epoch.
     *      The amount that will be redeemed is locked in and can no longer be * modified for the same epoch when manager starts a new epoch by calling rollEpoch() where consecutive redeem() calls will only modify amounts for the new epoch.
     *
     * @param amount    The amount of vault tokens to redeem.
     */

    function redeem(uint256 amount) external {
        if (redemptions[epochs.length][msg.sender] < amount) {
            // _lock has the necessary check for the account balance
            _lock(msg.sender, amount - redemptions[epochs.length][msg.sender]);
        } else if (amount < redemptions[epochs.length][msg.sender]) {
            _unlock(msg.sender, redemptions[epochs.length][msg.sender] - amount);

        }

        unchecked {
            currentEpochVaultTokenTotalAmount += (amount - redemptions[epochs.length][msg.sender]);
        }
        redemptions[epochs.length][msg.sender] = amount;
    }

    /** 
     * @notice Withdraws tokens corresponding to the amount of vault tokens that were redeemed for an epoch and burns the redeemed vault tokens.
     *
     * @dev Can be executed for any past epoch at any point in time in which a user redemption was initialized and locked in.
     *
     * @param account  The address where withdrawn tokens will be transferred.
     * @param epoch    The epoch of the redemption to withdraw.

     */

    function withdraw(address account, uint256 epoch) external {
        require(epoch < epochs.length, "Invalid epoch");
        require(0 < epochs[epoch].vaultTokenPrice, "Settlement for given epoch has not been completed yet.");
        uint256 amount = redemptions[epoch][account];
        require(0 < amount);

        withdrawableVaultTokenTotalAmount -= amount;
        redemptions[epoch][account] = 0;
        _unlock(account, amount);
        _burn(account, amount);
        bool success = redemptionToken.transfer(account, amount * epochs[epoch].vaultTokenPrice / epochs[epoch].redemptionTokenPrice);
        if (!success) {
            revert TransferFailed();
        }
    }
    ...
    /**
     * @notice Checks if settlement is completed for an epoch and withdrawals are available.
     *
     * @param epoch             Epoch to query.
     *
     * @return status      Settlement status of the epoch, true/false.
     */
    function settlementCompleted(uint epoch) external view returns (bool status){
        return epochs[epoch].vaultTokenPrice != 0 ? true : false;
    }

    /**
     * @notice Returns the current active Epoch
     *
     *
     * @return epoch      Current active Epoch.
     */
    function currentEpoch() external view returns (uint epoch){
    ...
    /**
     * @notice Locks the given amount of vault tokens for an account.
    
     * @dev Locked tokens are not transferrable.

     * @param account     The address of the account.
     * @param amount      The amount of tokens to be locked.
     */
    function _lock(address account, uint256 amount) internal {
        require(locks[account] + amount <= balanceOf(account));
        locks[account] += amount;
    }

    /**
     * @notice Unlocks the given amount of vault tokens for an account.
    ...
    function _beforeTokenTransfer(
        ...
        if (from != address(0)) {
            require(amount <= balanceOf(from) - locks[from], "Cannot transfer locked tokens");
        }
    }

    // For updating the management fee on deposits and redemptions
    function _updateManagementFees() internal {
        if (0 < updateManagementFeeTimestamp) {
            int256 numerator = (int256(block.timestamp) - int256(updateManagementFeeTimestamp)) * managementFeeBPS * int(totalSupply());
            // One mean calendar year has 8765.82 hours
            int256 denominator = 8765.82 hours * HUNDRED_PERCENT_BPS;
            int256 newManagementFee = numerator / denominator;

            if (0 < newManagementFee) {
                uint nav = totalSupply() * vaultPrice;
                accumulatedManagementFee += newManagementFee;
                vaultPrice = nav / totalSupply();
            }
        }
        updateManagementFeeTimestamp = block.timestamp;
    }
}
