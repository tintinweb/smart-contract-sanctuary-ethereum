...
contract Lottery is VRFConsumerBase, ReentrancyGuard, AutomationCompatibleInterface {
    ...
    enum LOTTERY_STATE { OPEN, CLOSED, CALCULATING_WINNER, WINNER_PAID }
    ...
    */
    ...
    constructor( address _payments, uint updateInterval) 
        VRFConsumerBase(
    ...
    function getPaidTrue() internal returns (bool) {
         return paid = true;    
    }

    function getPaidFalse() internal returns (bool) {
       return paid = false;    
    }

    
    function checkUpkeep(bytes calldata checkData ) external override returns (bool upkeepNeeded, bytes memory performData ) {
        if((paid = true) && keccak256(checkData) == keccak256(hex'01')){

            upkeepNeeded = true;
            performData = checkData;
            
        }
        if((lottery_state == LOTTERY_STATE.OPEN) && keccak256(checkData) == keccak256(hex'02')) {
   ...
   function performUpkeep(bytes calldata  performData ) external override {
    ...
    function fulfillRandomness(bytes32 requestId, uint randomness) internal override nonReentrant {
