...
contract mQuarkControl is ReentrancyGuard, PullPayment, AccessControl {
  ...
  event FundsWithdrawn(address project, uint256 amount);
  event ProjectFundsWithdrawn(address project, uint256 amount);
  event TemplatePricesSet(uint256[] templateIds, uint256[] prices);
  event CreatorPercentageSet(uint256 percentage);
  event SlotPriceSet(address project, uint256 price);
  event AuthorizedToRegisterWalletSet(address wallet, bool isAuthorized);
  event ProjectRemoved(address project);
  event FundsDeposit(uint256 amount, uint256 creatorPercentage, address project);
  ...
  // event TemplatesSelected(address project, uint256[] templateIds, uint256[] amounts, uint256 globalTokenIdVariable);
  // event TemplatesUnselected(address project, uint256[] templateIds, uint256[] collectionIndexes);
   ...
   * @param id: Unique id
   * @param projectMetadataTemplate: Project default metadata schema
   */
  struct Project {
    address wallet;
    address creator;
    ...
    string name;
    string thumbnail;
    string projectMetadataTemplate;
  }
   ...
   **/
  mapping(address => Project) public registeredProjects;
   ...
   **/
  mapping(address => uint256) public projectSlotAddPrices;
   ...
   *@param project:Pproject wallet address
  ...
  function mint(
    ...
    require(registeredProjects[project].id != 0, "unregistered project");
    require(msg.value == templateMintPrices[templateId], "sent value is wrong");
    require(msg.value != 0, "sent value is zero");
    mQuark.mint(msg.sender, project, templateId, collectionId);

    registeredProjects[project].balance += (msg.value * creatorPercentage) / 100;
    adminBalance[admin] += (msg.value * (adminPercentage)) / 100;
    emit FundsDeposit(msg.value, creatorPercentage, project);
  }
   ...
   *                       address   project: Project wallet address
  ...
  function mintBatch(ParamsLib.CollectionParams calldata collectionParams)
    ...
    nonReentrant
  {
    require(registeredProjects[collectionParams.project].id != 0, "unregistered project");
    ...
    registeredProjects[collectionParams.project].balance += (msg.value * creatorPercentage) / 100;
    adminBalance[admin] += (msg.value * (adminPercentage)) / 100;
    emit FundsDeposit(msg.value, creatorPercentage, collectionParams.project);
  }
  ...
  function mintBatchWithMetadataSlot(
    ...
    require(collectionParams.templateIds.length == collectionParams.amounts.length, "amount mismatch");
    require(registeredProjects[collectionParams.project].id != 0, "unregistered project");
    require(registeredProjects[collectionParams.project].id == _projectId, "project id mismatch");
    require(registeredProjects[collectionParams.project].wallet == collectionParams.project, "wallet mismatch");
    require(
      (this.calculateBatchPrice(collectionParams.templateIds, collectionParams.amounts) +
        this.calculateBatchSlotPrice(collectionParams.amounts, collectionParams.project)) == msg.value,
      "sent value is wrong"
    );
    ...
    mQuark.mintBatchWithMetadataSlot(
      ...
      _projectId,
      collectionParams,
      registeredProjects[collectionParams.project].projectMetadataTemplate
    );
   ...
   *@param projectParams:   struct ProjectParams {
  ...
  function mintWithMetadaSlots(
    uint256 templateId,
    uint256 collectionId,
    ProjectParams calldata projectParams
  ) external payable nonReentrant {
    require(projectParams.projectIds.length < 256, "minting more than limit"); 
    require(projectParams.projectIds.length == projectParams.projects.length, "project ids and wallets mismatch");
    require(templateMintPrices[templateId] > 0, "minting zero value NFT");

    string[] memory _projectMetadataTemplate = new string[](projectParams.projectIds.length);
    uint256 _totalMetadataSlotPriceAmount;
    uint256 _metadataSlotPrice;
    uint256[] memory _projectsMetedataPriceShares = new uint256[](projectParams.projects.length);

    uint256 projectCount = projectParams.projectIds.length;
    for (uint8 i; i < projectCount;) {
      require(registeredProjects[projectParams.projects[i]].id == projectParams.projectIds[i], "unregistered project");
      require(projectSlotAddPrices[projectParams.projects[i]] > 0, "slot value is zero");
      _metadataSlotPrice = projectSlotAddPrices[projectParams.projects[i]];
      _projectMetadataTemplate[i] = (registeredProjects[projectParams.projects[i]].projectMetadataTemplate);
      _totalMetadataSlotPriceAmount += _metadataSlotPrice;
      registeredProjects[projectParams.projects[i]].balance += (_metadataSlotPrice * creatorPercentage) / 100;
    ...
    mQuark.mintWithMetadaSlots(
      ...
      templateId,
      collectionId,
      projectParams.projects,
      projectParams.projectIds,
      _projectMetadataTemplate
    );
    registeredProjects[projectParams.projects[0]].balance += ((_templateMintPrice * creatorPercentage) / 100);
    adminBalance[admin] += ((msg.value * adminPercentage) / 100);

    /** @notice Base mint price should be considered at zero index! */
    emit MintBatchSlotFundsDeposit(msg.value, creatorPercentage, projectParams.projects, _projectsMetedataPriceShares);
  }
   ...
   *@param project:Project wallet address
   */
  function addMetadataSlotToTemplate(
    ...
    require(registeredProjects[project].id == projectId, "unregistered project");
    require(projectSlotAddPrices[project] == msg.value, "sent value is wrong");
    require(msg.value != 0, "sent value is zero");
    ...
    mQuark.addMetadataSlotToTemplate(
      ...
      tokenId,
      projectId,
      registeredProjects[project].projectMetadataTemplate
    );
    registeredProjects[project].balance += (msg.value * creatorPercentage) / 100;
    adminBalance[admin] += (msg.value * adminPercentage) / 100;
   ...
   *@param projectParams:   struct ProjectParams {
   *                                uint256[] projectIds: The number of mints for each IDs.
   *                                address[] projects:Project wallet addresses.
   *                          }
  ...
  function addBatchMetadataSlotsToTemplate(
    ...
    uint256[] memory _projectsShares = new uint256[](projectParams.projects.length);
    uint256 _projects = projectParams.projectIds.length;

    for (uint256 i; i < _projects;) {
      require(registeredProjects[projectParams.projects[i]].id == projectParams.projectIds[i], "unregistered project");
      require(projectSlotAddPrices[projectParams.projects[i]] > 0, "slot value is zero");
      _price = projectSlotAddPrices[projectParams.projects[i]];
      _projectMetadataTemplate[i] = (registeredProjects[projectParams.projects[i]].projectMetadataTemplate);
      _totalAmount += _price;
      registeredProjects[projectParams.projects[i]].balance += (_price * creatorPercentage) / 100;
    ...
    adminBalance[admin] += (msg.value * adminPercentage) / 100;
    mQuark.addBatchMetadataSlotsToTemplate(msg.sender, tokenId, projectParams.projectIds, _projectMetadataTemplate);
    emit BatchSlotFundsDeposit(msg.value, creatorPercentage, projectParams.projects, _projectsShares);
  }
   ...
   *@param project:Project wallet addresses
   */
  function addBatchMetadataSlotToTemplates(
    ...
    require(registeredProjects[project].id == projectId, "unregistered project");
    require((projectSlotAddPrices[project] * tokenIds.length) == msg.value, "sent value is wrong");
    require(msg.value != 0, "sent value is zero");

    mQuark.addBatchMetadataSlotToTemplates(
      msg.sender,
      tokenIds,
      projectId,
      registeredProjects[project].projectMetadataTemplate
  ...
  function updateURI(
    ...
    Project memory _registeredProject = registeredProjects[projectWallet];
  ...
  function resetProjectURI(
    ...
    require(msg.sender == registeredProjects[project].wallet, "unauthorized access");
    require(registeredProjects[project].id == projectId, "unregistered project");

    // mQuark.resetProjectURI(tokenId, projectId, registeredProjects[project].projectMetadataTemplate);
   ...
   *@param companyWallet: Creator wallet of the project
   *@param name: Project Name
   *@param companyName: Company name of the project
   *@param thumbnail: Image URL
   *@param projectMetadataTemplate: Project default metadata.
   */
  function registerProject(
    ...
    require(!projectWallets.contains(wallet), "already registered");

    unchecked {
      uint256 _id = ++projectIndexId;
      registeredProjects[wallet] = Project(
        wallet,
        companyWallet,
        _id,
        registeredProjects[wallet].balance,
        name,
        thumbnail,
        projectMetadataTemplate
      );
      projectWallets.add(wallet);
      emit ProjectRegistered(wallet, companyWallet, _id, name, companyName, thumbnail, projectMetadataTemplate);
    }
  }

  /**
   *Removes registered project from the contract
   *@param wallet: Wallet address of registered project
   */
  function removeProject(address wallet) external onlyRole(DEFAULT_ADMIN_ROLE) {
    require(registeredProjects[wallet].wallet != address(0), "deleting unregistered project");

    registeredProjects[wallet].wallet = address(0);
    registeredProjects[wallet].creator = address(0);
    registeredProjects[wallet].id = 0;
    registeredProjects[wallet].name = "";
    registeredProjects[wallet].thumbnail = "";
    registeredProjects[wallet].projectMetadataTemplate = "";
    projectWallets.remove(wallet);

    emit ProjectRemoved(wallet);
  }
   ...
   */
  function setUnbounded(address _mQuarkAddress) external onlyRole(DEFAULT_ADMIN_ROLE) {
  ...
  */
  ...
  function createCollections(
    ...
    mQuark.createCollections(wallet, registeredProjects[wallet].id, verifier, templateIds, amounts, signatures, uris);
  }
   ...
   *@notice Amount should be between 0-100
   */
  function setSlotAddPrice(address wallet, uint256 price) external {
    require(registeredProjects[wallet].creator == msg.sender || registeredProjects[msg.sender].wallet == msg.sender, "unauthorized access");
    projectSlotAddPrices[wallet] = price;
    emit SlotPriceSet(wallet, price);
  }
   ...
   *@param amount: Amount of funds that will be withdrawn in wei
   */
  function projectTransferFunds(address payable project, uint256 amount) external {
    require(registeredProjects[project].creator == msg.sender || registeredProjects[msg.sender].wallet == msg.sender, "unauthorized access");
    require(amount <= registeredProjects[project].balance, "insufficient balance");
    registeredProjects[project].balance -= amount;
    _asyncTransfer(project, amount);
    emit ProjectFundsWithdrawn(project, amount);
  }
   ...
   *@param project: Project wallet address
   */
  function calculateBatchSlotPrice(uint8[] calldata amounts, address project)
    external
    view
    returns (uint256 totalAmount)
  {
    require(projectSlotAddPrices[project] > 0, "slot value is zero");
    uint256 _amountsLength = amounts.length;
    for (uint8 i; i < _amountsLength;) {
      totalAmount += (projectSlotAddPrices[project] * amounts[i]);
      unchecked {++i;}
    }
  }
}
