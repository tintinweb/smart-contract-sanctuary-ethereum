    ...
    ERC721ACommon
{
    ...
    function toggleStartPublic() external onlyRole(MINT_ROLE) {
        startPublic = !startPublic;
    }

    bool public startPremintPublic;

    function toggleStartPremintPublic() external onlyRole(MINT_ROLE) {
        startPremintPublic = !startPremintPublic;
    }

    uint256 public maxPerAddress = 5;
    uint256 public reserveRemaining = 200;
    ...
    /**
    @notice Role of mint, in change mint system
     */
    bytes32 public constant MINT_ROLE = keccak256("MINT_ROLE");

    bytes32 public constant MINT_ROLE2 = 0x00;
    ...
    function setMaxPerAddress(uint256 _amount) public onlyRole(MINT_ROLE) {
    ...
    function preMint(
        ...
        require(_amount <=5, "mint amount over limit");
    ...
    /**
    @notice when mint is finish, lock all mint system, total supply is perminent and never change
    @dev after process this, no one can change back mint system
     */
    function finishMintAndBurnRest() external onlyOwner{
        startPremintPublic = false;
        startPublic = false;
        //change role of mint to black hole
         _grantRole(MINT_ROLE, address(0x00));
    }
    ...
    modifier reduceReservePool(uint256 n) {
        require(n <= reserveRemaining, "Mindfly: Reserve pool exhausted");
        reserveRemaining -= n;
        _;
    }
