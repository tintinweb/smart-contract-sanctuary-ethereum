...
import {ILayer3Execution} from "./interfaces/ILayer3Execution.sol";
...
import {ExchangeEventsAndErrors} from "./libraries/ExchangeEventsAndErrors.sol";
...
contract Layer3Exchange is
    IStargateReceiver,
    OwnableUpgradeable,
    ExchangeEventsAndErrors,
    UUPSUpgradeable
{
    ...
    /// @notice Domain separator for EIP712 used in signature checking
    bytes32 public DOMAIN_SEPARATOR =
        keccak256(
            abi.encode(
                ...
                keccak256("Layer3Marketplace"), // keccak256("LooksRareExchange")
                keccak256("1"), // keccak256(bytes("1")) for versionId = 1
                block.chainid,
                address(this)
            )
        );
    ...
    uint16 public layerZeroChainId;

    // TODO: pack/optimize
    uint256 public constant intConstant = 10000;
    uint256 public fee;
    ...
    function matchAskWithTakerBid(
        ...
        if (msg.sender != takerBid.taker) revert TakerMustBeSender();

        // requires taker provided price equal to msg.value
        if (msg.value != takerBid.price) revert IncorrectValueSupplied();

        bytes32 askHash = makerAsk.hash();
        _validateOrder(makerAsk, askHash);

        // requires taker provided price equal maker provided price
        (bool isExecutionValid, uint256 tokenId) = ILayer3Execution(
            makerAsk.strategy
        ).canExecuteTakerBid(takerBid, makerAsk);

        if (!isExecutionValid) revert ExecutionInvalid();
    ...
    function _transferFeesAndFunds(
        ...
        if ((protocolFeeRecipient != address(0)) && (protocolFeeAmount != 0)) {
            (bool feeSuccess, ) = address(protocolFeeRecipient).call{
                value: protocolFeeAmount
            }("");

            if (!feeSuccess) revert ProtocolFeeRecieptError();
            finalSellerAmount -= protocolFeeAmount;
        }

        (bool sellerSuccess, ) = address(to).call{value: finalSellerAmount}("");
        if (!sellerSuccess) revert SellerReceiptError();
    }
    ...
    function _calculateProtocolFee(address executionStrategy, uint256 amount)
        ...
        returns (uint256)
    {
        uint256 protocolFee = ILayer3Execution(executionStrategy)
            .viewProtocolFee();
    ...
    function _validateOrder(
        ...
        if (makerOrder.signer == address(0)) revert InvalidSigner();
        if (
    ...
    /// delete these, they are just for testing

    function recover(OrderTypes.MakerOrder memory makerOrder, bytes32 orderHash)
        external
        view
        returns (address)
    {
    ...
    function verify(OrderTypes.MakerOrder memory makerOrder, bytes32 orderHash)
        external
        view
        returns (bool)
    {
...
abstract contract ExchangeEventsAndErrors {
    event CancelMultipleOrders(address indexed user, uint256[] orderNonces);

    error NoOrdersSupplied();
    error NonceLowerThanCurrent();
    error TakerMustBeSender();
    error InvalidSigner();
    error InvalidSignature();
    error IncorrectValueSupplied();
    error ExecutionInvalid();
    error ProtocolFeeRecieptError();
    error SellerReceiptError();
}
