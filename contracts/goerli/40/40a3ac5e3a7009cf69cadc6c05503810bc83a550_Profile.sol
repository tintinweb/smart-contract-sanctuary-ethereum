...
contract Profile is Ownable {
    ...
    function setProfile(
        string memory _name,
        address _promotedAddress,
        string memory _promotedMessage
    ) public {
        require(bytes(_name).length <= 32, "Name too long");
        require(bytes(_promotedMessage).length <= 256, "Message too long");
        UserInfo storage user = userInfo[msg.sender];
        user.name = _name;
        user.promotedAddress = _promotedAddress;
        user.promotedMessage = _promotedMessage;
    }
}