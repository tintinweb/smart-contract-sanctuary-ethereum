...
contract InterestRateModel is IInterestRateModel, InterestRateModelStorage {
  ...
  function calculateRates(
    ...
    vars.newDepositAPY = averageBorrowRate.rayMul(vars.utilizationRate).rayMul(
      Math.RAY - poolFactor
    );
  ...
  function addNewPoolInterestRateModel(
    ...
    if (_interestRateModel[asset].borrowRateMax != 0) revert ModelAlreadyExists();
    if (Math.RAY <= optimalUtilizationRate) revert OptimalUExceedsRAY();
    if (borrowRateOptimal <= borrowRateBase) revert BaseBorrowRateExceedsOptimal();
    if (borrowRateMax <= borrowRateOptimal) revert OptimalBorrowRateExceedsMax();
  ...
  function updateOptimalUtilizationRate(address asset, uint256 optimalUtilizationRate)
    ...
    onlyGovernance
  {
    if (_interestRateModel[asset].borrowRateMax == 0) revert NotExistingAsset();
    if (Math.RAY <= optimalUtilizationRate) revert OptimalUExceedsRAY();
  ...
  {
    if (_interestRateModel[asset].borrowRateMax == 0) revert NotExistingAsset();
    if (_interestRateModel[asset].borrowRateOptimal <= borrowRateBase)
      revert BaseBorrowRateExceedsOptimal();
  ...
  {
    if (_interestRateModel[asset].borrowRateMax == 0) revert NotExistingAsset();
    if (borrowRateOptimal <= _interestRateModel[asset].borrowRateBase)
      revert BaseBorrowRateExceedsOptimal();
  ...
  {
    if (_interestRateModel[asset].borrowRateMax == 0) revert NotExistingAsset();
    if (borrowRateMax <= _interestRateModel[asset].borrowRateOptimal)
      revert OptimalBorrowRateExceedsMax();
