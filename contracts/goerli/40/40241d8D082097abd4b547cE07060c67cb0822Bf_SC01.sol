...
contract SC01 is ERC721, Ownable {
    bytes32 public whitelistMerkleRoot;

    // for constant example
    uint256 public constant MAX_SUPPLY = 1000;
    bool public saleState = false;
    uint256 public publicMintPrice = 0.0089 ether;
    uint256 public wlMintPrice = 0.0069 ether;
    ...
    function setPublicMintPrice(uint256 _publicMintpirce) public onlyOwner {
        publicMintPrice = _publicMintpirce;
    }

    function setWhiteListMintPrice(uint256 _wlMintPrice) public onlyOwner {
    ...
    function publicMint(uint256 tokenQuantity) external payable {
        ...
        require(saleState, "Sale must be active to mint");
        require(tokenQuantity <= maxMint, "Sale would exceed max mint");
        require(
            tokenQuantity * publicMintPrice <= msg.value,
            "Not enough ether sent"
        );
        _mint(tokenQuantity, msg.sender);
    }

    function _mint(uint256 tokenQuantity, address recipient) internal {
    ...
    function whitelistMint(
        ...
        require(
            addressToMinted[_msgSender()] + tokenQuantity <= allowance,
            "Exceeds whitelist supply."
        );
        require(tokenQuantity <= maxMint, "Sale would exceed max mint");
        require(
            tokenQuantity * wlMintPrice == msg.value,
            "Invalid funds provided."
        );
        // record the wl minted count
        addressToMinted[_msgSender()] += tokenQuantity;
        _mint(tokenQuantity, _msgSender());
    }
