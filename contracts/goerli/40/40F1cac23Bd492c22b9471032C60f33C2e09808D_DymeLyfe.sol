...
//@title Dyme Lyfe Club
//@author Satoshigoat (gh:@jcksber, tw:@satoshigoat)
contract DymeLyfe is Satoshigoat {

	string private _previewCID = "QmdURAzLmAK929WiUa5scvdM5unNG3esnu8zsZtahY4YcC";
	bool private _imagesGenerated;
	...
	constructor() Satoshigoat("Dyme Lyfe Club", "", "ipfs://") 
	{
		_contractURI = "ipfs://";
		// _owner = address();
		// payoutAddress = address();
    	purchasePrice = .000000058 ether;//change this!
	}
	...
	{
		// only valid token id's
		if (!_exists(tid))
			revert URIQueryForNonexistentToken();

		// if the images haven't been generated, show the preview metadata
		if (_imagesGenerated)
			return string(abi.encodePacked(_baseURI(), toString(tid)));
		else
			return string(abi.encodePacked(_baseURI(), _previewCID));
	}

	//@dev Mint tokens for free (only owners)
	function mint(address payable to, uint256 qty) 
		external
		payable
		isSquad
		nonReentrant
	...
	//@dev Purchase tokens
	function purchase(address payable to, uint256 qty)
		external
		payable
		nonReentrant
		enoughSupply(qty)
		purchaseArgsOK(to, msg.value, qty)
	{
		_safeMint(to, qty);
	}

	//@dev Toggle whether or not we're showing the preview image
	function toggleImagesGenerated() 
		external isSquad { _imagesGenerated = !_imagesGenerated; }
	...
	//@dev [BACKUP METHOD] Ability to change _previewCID
	function setPreviewCID(string calldata newCID) 
		external isSquad notEqual(_previewCID, newCID) { _previewCID = newCID; }
...
contract Satoshigoat is ERC721A, Pausable, ReentrancyGuard {
	...
	//@dev Sale Data
	uint256 public constant MAX_NUM_TOKENS = 5000;
