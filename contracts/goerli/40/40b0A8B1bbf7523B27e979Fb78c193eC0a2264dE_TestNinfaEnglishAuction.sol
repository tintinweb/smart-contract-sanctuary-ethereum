...
contract TestNinfaEnglishAuction is Ownable, RoyaltyEngineV1, ERC1155Holder {
    ...
    Counters.Counter public _auctionId;

    mapping(address => mapping(uint256 => uint256)) public tokenIdToAuctionId; // The auction configuration for a specific auction id.
    mapping(uint256 => _Auction) public auctions; // The auction id for a specific NFT. This is deleted when an auction is finalized or canceled.
    ...
    // TODO review which of the following constants should not be constant but should have a setter function instead for adminws only (or even per auction)!!
    ...
    modifier isWhitelisted(address _collection) {
        ...
        while (i < collectionsRegistries.length) {
            ...
            (, bytes memory authorized) = collectionsRegistries[i].staticcall(
                abi.encodeWithSelector(0xf6a3d24e, _collection)
            );
            // exists (bytes authorized) contains a boolean returned by hasRole, if `true` break the loop and continue, i.e. the collection is whitelisted
            if (abi.decode(authorized, (bool)) == true) break;
    ...
    function createAuction(
        ...
        tokenIdToAuctionId[_collection][_tokenId] = _auctionId.current();
        auctions[_auctionId.current()] = _Auction(
            _collection,
            _tokenId,
     ...
     */
    function updateAuction(uint256 auctionId_, uint256 _price) external {
        _Auction storage _auction = auctions[auctionId_];
        ...
        auctions[auctionId_].price = _price;

        emit AuctionUpdated(auctionId_, _price);
    }
    ...
    function _trade(_Auction memory _auction) public {
    ...
    function addCollectionsRegistry(address _newCollectionsRegistry)
        external
        onlyOwner
    {
    ...
    function deleteCollectionsRegistry(uint256 _index) external onlyOwner {
