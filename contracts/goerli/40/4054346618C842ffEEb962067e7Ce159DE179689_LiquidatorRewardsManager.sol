...
contract LiquidatorRewardsManager is Owned, MixinSystemSettings {
    ...
    function sendUpdateEntry(
        ...
        address account,
        uint256 amount
    ) external onlyLiquidatorRewards(collateralKey) {
        liquidatorRewardsAggregator().sendUpdateEntry(collateralKey, amount, account);

        emit UpdateEntryMsgToAggregator(collateralKey, account, amount);
    }

    function sendNotifyReward(bytes32 collateralKey, uint256 amount) external onlyLiquidatorRewards(collateralKey) {
        liquidatorRewardsAggregator().sendNotifyReward(collateralKey, amount);

        emit NotifyRewardMsgToAggregator(collateralKey, amount);
    }

    /* ========== MODIFIER FUNCTIONS ========== */
    modifier onlyLiquidatorRewards(bytes32 collateralKey) {
        require(msg.sender == address(liquidateRewards[collateralKey]), "Only liquidatorRewards can call this function");
        _;
    }

    /* ========== EVENTS ========== */
    event UpdateEntryMsgToAggregator(bytes32 collateralKey, address indexed account, uint256 amount);
    event NotifyRewardMsgToAggregator(bytes32 collateralKey, uint256 amount);
