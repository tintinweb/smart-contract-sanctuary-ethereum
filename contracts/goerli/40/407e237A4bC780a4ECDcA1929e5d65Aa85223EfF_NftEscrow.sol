...
UUPSUpgradeable
{
    ...
    external {
        require(
            _addr.length == _tokenId.length,
            "NftEscrow: The two arrays are not equal in length"
        );
        uint256  _len = _addr.length;
        for (uint i=0;i<_len;i++){
            (bool success, ) = _addr[i].call(
                abi.encodeWithSignature("safeTransferFrom(address,address,uint256)", msg.sender,address(this),_tokenId[i])
            );
            require(success,"NftEscrow:transfer error");
        ...
        for (uint i=0;i<addr_len;i++){
            ...
            for (uint j=0;j<_ts.length;j++){
                if(_ts[j].TokenId == _tokenId[i]){
                    ...
                    (bool success,) = _addr[i].call(
                        abi.encodeWithSignature("safeTransferFrom(address,address,uint256)", address(this),msg.sender,_tokenId[i])
                    );
                    require(success,"NftEscrow:transfer error");
