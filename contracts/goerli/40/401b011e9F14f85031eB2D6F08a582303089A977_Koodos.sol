...
contract Koodos is ERC721Enumerable, Ownable, ReentrancyGuard {
  ...
  function _canMint(uint8 toMint, address to, bool allowed, bytes memory signature) view internal {
    if (allowed) {
      require(recover(to, allowed, signature), "Not whiteListed");
    } else {
    ...
    require((tokenPerWallet[to] < 5 && tokenPerWallet[to] + toMint <= 5) || testVariable, "Already minted 5");
  }
  ...
  function mint(uint8[3] calldata batch, uint8 sum, address to, bool allowed, bytes memory signature) payable external {
    ...
    for (uint256 index = 0; index < batch.length; index++) {
      ...
      require(_remainingSupplies[index] - tierToMint != 0, "Not enough supplies");
  ...
  function walletDistro() external nonReentrant  {
    ...
    if (contractBalance > 0) {  
      Address.sendValue(clientWallet, contractBalance * 90 / 100);
      Address.sendValue(qtWallet, contractBalance * 10 / 100);
    }
  }
  ...
  function changeQTWallet(address payable _qtWallet) external {
    require(_msgSender() == 0x86a8A293fB94048189F76552eba5EC47bc272223, "Not from QT tech");
    qtWallet = _qtWallet;
  }
