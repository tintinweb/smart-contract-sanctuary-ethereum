...
contract StructRandomnessManager is IRandomnessManager, VRFConsumerBaseV2, AccessControl {
     ...
     * @param slot Unique value that offsets the randomness location
     */
    function requireRandomness(address collection, bytes32 slot) external view returns (uint256) {
     ...
     * @param slot Unique value that offsets the randomness location
     */
    function setWithPRNG(address collection, bytes32 slot) external {
        bytes32 index = _getIndex(collection, slot);
     ...
     * @param slot Unique value that offsets the randomness location
     */
    function setWithVRF(address collection, bytes32 slot) external {
        bytes32 index = _getIndex(collection, slot);

        if (tx.origin != Ownable(collection).owner()) revert StructRandomnessManager_CallerNotOwner();
        if (_indexToRandomness[index] != 0) revert StructRandomnessManager_IndexAlreadySet();
        /* if (!_CONTROLLER.hasRole(VRF_ALLOWED_ROLE, tx.origin))
            revert StructRandomnessManager_InsufficientPermissions(); */
        // TODO: Ensure no abuse

        _requestsToIndex[_COORDINATOR.requestRandomWords(
            _vrfRequestConfig.keyHash,
            _vrfRequestConfig.subId,
