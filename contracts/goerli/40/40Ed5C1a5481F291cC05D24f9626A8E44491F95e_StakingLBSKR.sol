    ...
    Pausable
{
    ...
    function pauseContract() public onlyRole(MANAGER_ROLE) {
        _pause();
    }

    function unPauseContract() public onlyRole(MANAGER_ROLE) {
        _unpause();
    }
}
...
contract LBSKR is BaseBSKR {
    ...
    event debug(uint256 u1, uint256 u2, uint256 u3);
    ...
    function swapTokensForTokensExt(uint256 tokenAmount)
        external
    ...
    {
        uint8 SECS_IN_AN_HOUR = 1; // TODO to be made constant with value of 3600
        uint256 secsElapsed = (nowTS - _lastDistTS); // miners can manipulate upto 900 secs
        uint256 hoursElapsed = secsElapsed % (SECS_IN_AN_HOUR * 24);
        uint256 daysElapsed = hoursElapsed / 24;

        // uint256 inflation;
        uint8 inflationBips = 2; // divide by bips

        uint256 currBal = _tBalance[_inflationAddress];
        if (daysElapsed > 0) {
            inflation = currBal * (1 + inflationBips / _BIPS) - currBal;
        }
        inflation += (currBal * inflationBips * hoursElapsed) / 24 / _BIPS; // plus hours elapsed
    ...
    function creditInflation() internal {
        ...
        emit debug(inflation, _tBalance[_inflationAddress], _tBalance[address(this)]);
    }
}
...
contract StakingLBSKR is LBSKR {
    using EnumerableSet for EnumerableSet.AddressSet;
    ...
    constructor(
        ...
        address growthAddressA,
        address[] memory sisterOAsA
    ) LBSKR(nameA, symbolA, growthAddressA, sisterOAsA) {
    ...
    function setInitialRatio(
        ...
        uint256 stakeAmount
    ) public isInitialRatioNotSet onlyRole(MANAGER_ROLE) {
    ...
    function getBSKRPerShare() public view returns (uint256) {
        // return (bskrERC20.balanceOf(address(this))) / totalShares;
        return (balanceOf(address(this))) / totalShares;
    }
        ...
        returns (uint256)
    {
        ...
        require(stakeholderStake >= bskrAmount, "Not enough staked!");
    ...
    function getCurrentRewards() public view returns (uint256) {
        ...
        return balanceOf(address(this)) - totalStakes;
    }
    ...
    function refundLockedBSKR(uint256 from, uint256 to)
        public
        onlyRole(MANAGER_ROLE)
    {
    ...
    function removeLockedRewards() public onlyRole(MANAGER_ROLE) {
