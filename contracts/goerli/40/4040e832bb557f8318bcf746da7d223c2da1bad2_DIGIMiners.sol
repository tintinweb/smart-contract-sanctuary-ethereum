...
contract DIGIMiners is ERC721A, Ownable {
  ...
  uint256 public allowListLimitPerwallet = 2;
  ...
  function allowListMint(uint256 quantity, bytes32[] calldata proof) public {
    require(whenAllowlistSaleIsOn() == true, "DIGI: Whitelist Sale Not Started Yet");
    require(totalSupply().add(quantity) <= MAX_SUPPLY, "DIGI: Exceeding Max Limit");
    require(
      isValid(proof, keccak256(abi.encodePacked(_msgSender()))) == true,
    ...
    require(
      ...
      "DIGI: Whitelist Limit Reached "
    );
    _safeMint(msg.sender, quantity);
  }

  function publicMint(bytes calldata signature) public {
    require(whenPublicaleIsOn() == true, "DIGI: Public Sale Not Started Yet");
    require(checkPublicMinted(_msgSender()) == false, "DIGI: You Already minted");
    publicMinted[msg.sender] = true;
    require(
      _signerAddress == checkSign(signature, _msgSender()), "DIGI: Invalid Signature"
    );
    totalPublicMinted += 1;
