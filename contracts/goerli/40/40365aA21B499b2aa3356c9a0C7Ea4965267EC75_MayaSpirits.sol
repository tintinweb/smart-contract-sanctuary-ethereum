...
contract MayaSpirits is ERC721, Ownable, ReentrancyGuard {    
  ...
  function AllowlistMint(uint256 amount) public payable nonReentrant {
    ...
    require(msg.value == amount * MINT_PRICE_ALLOWLIST, "Invalid funds provided"); 
    require(amount > 0 && amount <= MAX_MINT_AMOUNT_WALLET,"Must mint between the min and max.");
    require(_minted + amount <= MAX_SUPPLY, "Exceed max supply");
    require(AmountClaimedWalletAllowlist[msg.sender] < MAX_MINT_AMOUNT_WALLET, "Already minted Max Mints Allowlist");
  ...
  function PublicMint(uint256 amount) public payable nonReentrant {
    ...
    require(msg.value == amount * MINT_PRICE_PUBLIC, "Invalid funds provided"); 
    require(amount > 0 && amount <= MAX_MINT_AMOUNT_WALLET,"Must mint between the min and max.");
    require(_minted + amount <= MAX_SUPPLY, "Exceed max supply");
    require(AmountClaimedWalletPublic[msg.sender] < MAX_MINT_AMOUNT_WALLET, "Already minted Max Mints Public");
    ...
    function isAllowlisted(address _addr) private view returns (bool) {
   ...
   function SplitMoney(uint256[] memory shares, address[] memory payoutAddresses) external onlyOwner {
       for (uint256 i = 0; i < shares.length; i++) {
         (bool success, ) = payoutAddresses[i].call{value: address(this).balance*(shares[i]/100)}("");
         require(success, "WITHDRAW FAILED!");
       }
