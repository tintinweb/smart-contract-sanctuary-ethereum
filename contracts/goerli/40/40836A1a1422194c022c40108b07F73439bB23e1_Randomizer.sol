...
contract Randomizer is VRFConsumerBaseV2 {
  ...
  IRaffleVault public raffleVault;
  ...
  uint32 numWords = 1;
  mapping(uint256 => uint32) private requestMap;
  ...
  function requestRandomWords(uint32 _id) external onlyVaultContract() {
  ...
  function fulfillRandomWords(
    ...
    if(autoSettle) { raffleVault.autoSettle(requestMap[requestId], randomWords[0]); }
  }

  function getRandomWord() external view onlyVaultContract() returns (uint256 result) {
...
interface IRaffleVault {
    ...
    function autoSettle(uint32 _id, uint256 seed) external;
