...
import {RocketStorageInterface as IRocketStorage} from "@rocketpool/interface/RocketStorageInterface.sol";
import {RocketTokenRETHInterface as RETH} from "@rocketpool/interface/token/RocketTokenRETHInterface.sol";
import {RocketDepositPoolInterface as IDepositPool} from "@rocketpool/interface/deposit/RocketDepositPoolInterface.sol";
 ...
 * @author @douglasqian @blackmarkt
 * @notice Implementation of the Spirals ImpactVault on the Rocket Pool liquid
 *   staking protocol. This vault accrues yield through holding rETH.
 *   For now, unstaking on ETH2 isn't supported yet which makes withdrawals
 *   from Rocket Pool challenging. Our vault will try to burn rETH for ETH
 *   to send back to the receiver, but it will likely be reverted until
 *   unstaking on ETH2 is supported.
 */
contract SpiralsETHImpactVault is ImpactVault {
    using SafeERC20 for IERC20;

    event DependenciesUpdated(
        address indexed rocketStorageAddress,
        address indexed wrappedEthAddress,
        address indexed chainlinkEtherOracleAddress
    );

    event Receive(address indexed sender, uint256 indexed amount);

    IRocketStorage public c_rocketPoolStorage;
    IWrappedEther public c_wETH; // 0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2
    IChainlinkEACAggregator public c_ethPriceOracle; // 0x5f4ec3df9cbd43714fe2740f5e3616155c5b8419
     ...
     */
    function initialize(
        address _rocketPoolStorageAddress,
        ...
        setDependencies(
            _rocketPoolStorageAddress,
            _wrappedEthAddress,
            _chainlinkEtherOracleAddress
        );

        __ERC20_init("Spirals ETH Vault Token", "spETH");
        __ImpactVault_init(
            IERC20Upgradeable(address(0)), // ignore, not using "asset"
            IERC20Upgradeable(address(getRETH())),
    ...
    function setDependencies(
        ...
        c_rocketPoolStorage = IRocketStorage(_rocketPoolStorageAddress);
     ...
     * By default should be WETH in order to conform with ImpactVault contract
     */
        ...
        nonReentrant
    {
        ...
        // Need to convert wETH to ETH
        c_wETH.transferFrom(_receiver, address(this), _amount);
        c_wETH.withdraw(_amount); // withdraw ETH to contract
    ...
    /**
     * @dev Override original ImpactVault "deposit" function.
     */
    // function deposit(uint256, address) external virtual override {
    //     revert NotSupported();
    // }

    /**
     * @notice Support for ETH and directly depositing into RocketPool
     */
    ...
    /**
     * @dev Deposit ETH into RP Deposit Pool and receive rETH.
     */
    function _stake(uint256 _amount) internal virtual override {
        getDepositPool().deposit{value: _amount}();
    }

    /**
     * @notice As of 9/14/22 it is not really feasible to withdraw ETH from
     * Deposit Pool
     * @notice Attempts to withdraw ETH from RocketPool by converting rETH back to ETH.
     * @dev This will likely revert on the burn because ETH2 unstaking isn't
     * supported yet and RocketPool might not have enough liquidity. Short-term solution
     * is to use withdrawYieldAsset to return rETH to receiver
     */
        ...
        override
    {
        ...
        withdrawRETH(_receiver, _amountETH);

    }

    /**
     * @notice Attempts to withdraw Excess Balance.
     */

     function _withdrawExcessBalance(uint256 _amount) internal virtual {
        getDepositPool().withdrawExcessBalance(_amount);
    }

    /**
     * @notice Withdraws yield asset from owner balance to receiver.
     */
    function withdrawRETH(address _receiver, uint256 _amount) 
        public
        virtual {
        // Convert ETH -> rETH for receiver
        uint256 amountRETH = convertToYieldAsset(_amount);
        getRETH().transfer(_receiver, amountRETH);
    }
        ...
        returns (uint256 usdAmount)
    {
        ...
        return priceOfEth * _amountETH;
    }

    /**
     * @dev rETH -> ETH
     */
    function convertToAsset(uint256 _amountRETH)
        public
        view
        virtual
        override
        returns (uint256)
    {
        return getRETH().getEthValue(_amountRETH);
    }
    ...
    function convertToYieldAsset(uint256 _amountETH)
        ...
        returns (uint256)
    {
        return getRETH().getRethValue(_amountETH);
    }

    /**
     *  @dev Returns Rocket Pool token contract (rETH).
     */
    function getRETH() public view returns (RETH) {
        address rethAddress = c_rocketPoolStorage.getAddress(
            keccak256(abi.encodePacked("contract.address", "rocketTokenRETH"))
        );
        return RETH(rethAddress);
    }

    /**
     *  @dev Returns Rocket Pool deposit pool contract.
     */
    function getDepositPool() public view returns (IDepositPool) {
        address rocketDepositPoolAddress = c_rocketPoolStorage.getAddress(
            keccak256(abi.encodePacked("contract.address", "rocketDepositPool"))
        );
        return IDepositPool(rocketDepositPoolAddress);
    }
}
