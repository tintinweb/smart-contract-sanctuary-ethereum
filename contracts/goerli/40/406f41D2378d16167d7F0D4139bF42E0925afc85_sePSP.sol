...
error CannotReenter(int256 reqId);
error CannotWithdraw(int256 reqId);
error InvalidRequestWithdrawId(int256 reqId);
error PermitUnsuccessful();
...
contract TimeLockedERC20 is ERC20Permit, Ownable, Pausable {
    ...
    function requestWithdraw(uint256 _unstakingAmount) external {
        ...
        emit RequestedUnlocking(id, msg.sender, _unstakingAmount);
    }
    ...
    function _withdraw(int256 id) internal returns (uint256) {
        if (id < 0) {
            revert InvalidRequestWithdrawId(id);
        }

        WithdrawalRequest storage request = userVsWithdrawals[msg.sender][id];

        if (request.status != WITHDRAW_STATUS.UNLOCKING || request.releaseTime > block.timestamp) {
