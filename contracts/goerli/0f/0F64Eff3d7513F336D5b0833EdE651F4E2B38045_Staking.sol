...
contract Staking {
    ...
    address public immutable admin;   // admin to initiate the pools (Fixed + Flexible)
    ...
    function claimFixStake(uint _poolId) external noReentrant {
        ...
        delete fixedStake[msg.sender][_poolId][index];        
    }
...
// ["Gold", 8000000000000000000, 120, 3]
    ...
    function claimFlexReward(uint _poolId) external noReentrant {
        ...
        IERC20(token).transfer(msg.sender, amount);
        delete flexibleStake[msg.sender][_poolId][index];
    }
