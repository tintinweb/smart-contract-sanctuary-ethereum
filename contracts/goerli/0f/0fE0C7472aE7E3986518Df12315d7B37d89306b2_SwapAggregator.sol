    ...
    ISwapAggregator
{
        ...
        returns (StrategyPathTask memory)
    {
        ...
        return swapAllConnectors(task, target);
    }
    ...
    function swapAllConnectors(StrategyPathTask memory task, address target)
        internal
        returns (StrategyPathTask memory)
    {
        address[] memory tier2connectors = task.connectors.copy(
            task.connectors.length
        );
        address[] memory tier1connectors = new address[](
            task.connectors.length
        );
        uint256 j = 0;

        for (uint256 i; i < tier2connectors.length; ++i) {
            uint256 connectorIdx = task.balances.getIndex(tier2connectors[i]);

            if (
                task.balances[connectorIdx].balance > 1 &&
                tier2connectors[i] != target &&
                !tier1connectors.includes(tier2connectors[i])
            ) {
                (
                    SwapQuote memory currentQuote,
                    address tokenOut,

                ) = findSwapQuote(task, connectorIdx, target, false); // F:[CPF-10,11]

                if (!currentQuote.found)
                    if (!task.force) {
                        revert PathNotFoundException(
                            tier2connectors[i],
                            target
                        ); // F:[CPF-9]
                    } else {
                ...
                if (tokenOut == target) {
                    tier1connectors[j] = tier2connectors[i];
                    ++j;
                } else {
                    if (!tier1connectors.includes(tokenOut)) {
                        tier1connectors[j] = tokenOut;
                        ++j;
                    }

                    task.balances[connectorIdx].balance = 0; // F:[CPF-10,11]
                    task.balances.addBalance(
                        tokenOut,
                        task.amountOutWithSlippage(currentQuote.amount)
                    ); // F:[CPF-10,11]
                    task.calls = task.calls.append(currentQuote.multiCall);
                    task.gasUsage += currentQuote.gasUsage; // F:[CPF-10,11]
                }
            }
        }
        ...
        for (uint256 i = 0; i < j; ++i) {
            address token = tier1connectors[i];
            uint256 connectorIdx = task.balances.getIndex(token);
            ...
            if (!currentQuote.found)
                revert PathNotFoundException(token, target);

            task.balances[connectorIdx].balance = 0;
            task.balances.addBalance(
                target,
                task.amountOutWithSlippage(currentQuote.amount)
            );
            task.calls = task.calls.append(currentQuote.multiCall);
            task.gasUsage += currentQuote.gasUsage;
        }
