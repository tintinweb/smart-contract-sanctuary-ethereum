...
interface IERC20 {
     ...
     * allowed to spend on bedividedBalance of `owner` through {transferFrom}. This is
     * zero by default.
     *
...
contract MyToken is ERC20, Ownable {
    ...
    event updateBuyTax(uint256 buyLiqFee, uint256 buyDevFee);
    event updateSellTax(uint256 sellLiqFee, uint256 sellDevFee);
    event updateMaxTxAmount(uint256 maxTxAmount);
    event updateMaxWalletAmount(uint256 maxWalletAmount);
    event updateFeeReceiver(address devFeeAddress);
    ...
    uint256 private _tokensAmountToSellForLiq = 100000 * 10**_decimals;
    uint256 private _tokensAmountToSellForDevFee = 200000 * 10**_decimals;
    uint256 private swapTokensTrigger = 500000 * 10**_decimals;
    ...
    constructor() ERC20(_name, _symbol) {
        ...
        buyDevFee = 8;
        buyLiqFee = 2;
        buyFeeTotal = buyLiqFee + buyDevFee;

        sellDevFee = 23;
        sellLiqFee = 2;
        sellFeeTotal = sellLiqFee + sellDevFee;
    ...
    function changeFeeReceiver(address newAddress) public onlyOwner {
        devFeeAddress = newAddress;
        emit updateFeeReceiver(devFeeAddress);
    }    
    ...
    function setSwapTriggerAmount(uint256 amountDevFee, uint256 amountLiqFee) public onlyOwner {
        _tokensAmountToSellForDevFee = amountDevFee * (10**_decimals);
    ...
    function setBuyTax(uint256 _buyLiqFee, uint256 _buyDevFee) public onlyOwner {
        require((_buyLiqFee + _buyDevFee) <= 100, "ERC20: total tax must no be greater than 100");
        buyLiqFee = _buyLiqFee;
        buyDevFee = _buyDevFee;
        emit updateBuyTax(buyLiqFee, buyDevFee);
    }


    function setSellTax(uint256 _sellLiqFee, uint256 _sellDevFee) public onlyOwner {
        require((_sellLiqFee + _sellDevFee) <= 100, "ERC20: total tax must no be greater than 100");
        sellLiqFee = _sellLiqFee;
        sellDevFee = _sellDevFee;
        emit updateSellTax(sellLiqFee, sellDevFee);
    }
    ...
    function _transfer(address from, address to, uint256 amount) internal override {
        ...
        if (to == deadAddress) {
            ...
            bool swapForFees = _feeReserves > _tokensAmountToSellForDevFee;


            if ((swapForLiq || swapForFees) && isSelling) {
                swapping = true;
                ...
                if (swapForFees) {
                    _swapTokensForEth(_tokensAmountToSellForDevFee);
                    _feeReserves -= _tokensAmountToSellForDevFee;
                    bool sent = payable(devFeeAddress).send(address(this).balance);
            ...
            if (isSelling) {
                if (devFee > 0 && liqFee > 0) {
                    uint256 devFeeShare = ((amount * devFee) / 100);
                    uint256 liqFeeShare = ((amount * liqFee) / 100);
                    uint256 feeShare = devFeeShare + liqFeeShare;
                    transferAmount = amount - feeShare;
                    _feeReserves += devFeeShare;
                }
            }
