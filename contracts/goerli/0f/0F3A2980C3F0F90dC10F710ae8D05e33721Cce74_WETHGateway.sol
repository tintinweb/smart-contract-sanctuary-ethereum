...
interface IWETHGateway {
  /**
   * @dev deposits WETH into the reserve, using native ETH. A corresponding amount of the overlying asset (ULTokens)
   * is minted.
   * @param onBehalfOf address of the user who will receive the ULTokens representing the deposit
  ...
  /**
   * @dev borrow WETH, unwraps to ETH and send both the ETH and UDTokens to msg.sender, via `approveDelegation` and onBehalf argument in `LendPool.borrow`.
...
contract WETHGateway is IWETHGateway, ERC721HolderUpgradeable, EmergencyTokenRecoveryUpgradeable {
  ...
  function withdrawETH(uint256 amount, address to) external override nonReentrant {
    ...
    ILendPool cachedPool = _getLendPool();
    IULToken bWETH = IULToken(cachedPool.getReserveData(address(WETH)).ULTokenAddress);
