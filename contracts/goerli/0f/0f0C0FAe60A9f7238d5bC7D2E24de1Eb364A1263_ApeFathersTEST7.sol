...
import "@openzeppelin/contracts/token/ERC721/extensions/ERC721Burnable.sol";
    ...
    /**
    * @author gsknnft.eth
    * @notice This smart contract handles burning "The ApeDads(DAPES)" tokens for a free mint of The ApeFathers token, and includes a paid mint for The ApeFathers NFT tokens.
    */
    
    interface ApeDadsContract {
...
contract ApeFathersTEST7 is ERC721A, ERC721AQueryable, Ownable, Pausable, ReentrancyGuard, ERC2981, ERC4907A {
    ...
    ERC721A ApeDadsAddress = ERC721A(0x6468f4243Faa8C3330bAAa0a7a138E2e5628C6f5);
    address burnAddress = address(0);
    address ApeFathersNFT = address(this);
    ...
    ApeDadsContract public apeDads;
    mapping(address => mapping(uint256 => bool)) private approvedTokens;
    ...
    event TokenApproved(address indexed _sender, uint256 tokenId);
    ...
    constructor(address apeDadsIn) ERC721A("The ApeFathers NFT", "DAPES") {
    apeDads = ApeDadsContract(apeDadsIn);
        setHiddenMetadataUri("ipfs://bafybeigqicwgfc2lyro3gkvv6wj6it5epm7kcnlnbnxh4apflwmvnscmwe/hidden.json");
    ...
    function _beforeTokenTransfers(
        ...
        if (from != address(0)) {
            address ADowner = apeDads.ownerOf(tokenId);
            require(ADowner == msg.sender, "Only the owner of NFT can transfer or burn it");
        }
    }
    ...
    */
    function withdraw() external nonReentrant onlyOwner {
        require(ApeFathersNFT.balance > 0, "NO_FUNDS");
        uint256 balance = ApeFathersNFT.balance;
    ...
    function toString(uint256[] memory _tokens) internal pure returns (string memory) {
        bytes memory tokenBytes = abi.encodePacked(_tokens);
        return string(tokenBytes);
    }
    
    function askForConfirmation(string memory message) internal returns (bool) {
        bytes memory data = abi.encodeWithSignature("confirm(string)", message);
        (bool success, bytes memory result) = address(apeDads).call(data);
        require(success, "Failed to show confirmation prompt");
        return abi.decode(result, (bool));
    }


    function approveTokensForBurn(uint256[] calldata _tokenIds) external {
        address owner = apeDads.ownerOf(_tokenIds[0]);
        require(owner == _msgSender(), "TOKENS NOT OWNED BY CALLER OR INVALID TOKEN ID");

        // Inform the user of what they are approving
        string memory warning = "WARNING: You are approving the transfer of all your ApeDad NFTs to be burned by this contract. Please ensure you have selected the correct tokens to burn.";
    ...
    function approveTokens(uint256[] calldata _tokenIds) external {
        uint256 numberOfTokens = _tokenIds.length;
        require(numberOfTokens >= 1, "MUST APPROVE AT LEAST ONE TOKEN");

            // Inform the user of what they are approving
            string memory warning = "WARNING: You are approving the transfer of all your ApeDad NFTs to be burned by this contract. Please ensure you have selected the correct tokens to burn.";
            bytes memory data = abi.encodeWithSignature("warning(string)", warning);
            (bool success, ) = msg.sender.call(data);
            emit ApprovalGranted(_msgSender());
            require(success, "Failed to show Metamask warning");

        for (uint256 i = 0; i < numberOfTokens; i++) {
            uint256 tokenId = _tokenIds[i];
            address owner = apeDads.ownerOf(tokenId);
            require(owner == _msgSender(), "TOKEN NOT OWNED BY CALLER OR INVALID TOKEN ID");

            approvedTokens[_msgSender()][tokenId] = true;

            emit TokenApproved(_msgSender(), tokenId);
        }
    }
    error TokenCountVerificationFailed();
    error BurnClaimIsNotActive();
    error InvalidClaim();
    error CannotClaimTokenYouDontHold();
    function burnClaim(uint256[] calldata _tokenIds, uint256 verifyNumberOfTokens) external nonReentrant {
        ...
        for (uint256 i = 0; i < _tokenIds.length; i++) {
            address owner = apeDads.ownerOf(_tokenIds[i]);
            if (owner != _msgSender()) {
                revert CannotClaimTokenYouDontHold();
            }            
