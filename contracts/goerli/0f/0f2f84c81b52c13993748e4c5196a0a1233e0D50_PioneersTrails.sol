...
contract PioneersTrails is VRFConsumerBaseV2,KeeperCompatibleInterface{
    ...
    function generateInconveniencesForNextTrails() public {
        ...
        if(_nextStage == IPioneers.Stage.Journey1){
            for(uint i = 7; i <= 14; i++){
                ...
                for(uint j = 0; j < 3; j++){
                    uint rand = s_randomWords[j] % (13 - removedInconveniences);
    ...
    {       
        ...
        string memory pickedLetter = string( abi.encodePacked( getAvailableLettersFromWagons()[rand] ) );
    ...
    function getAvailableLettersFromWagons() public view returns (bytes memory letters){
        bytes memory currentLetters = new bytes(25);
        uint length = IPioneers(pioneers).getTokenCount();
