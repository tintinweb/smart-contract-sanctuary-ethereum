...
contract LandRegistration {
    ...
    function getMyLands() public view returns (Land[] memory) {
        ...
        for (uint256 i = 0; i < nextId; i++) {
            if (landRegistry[i].owner == msg.sender) {
    ...
    function getLandIdsAndOwners() public view returns (uint256[] memory) {
        uint256[] memory ids = new uint256[](nextId);
        for (uint256 i = 0; i < nextId; i++) {
            ids[i] = landOwned[landRegistry[i].owner];
        }
        return ids;
    }
