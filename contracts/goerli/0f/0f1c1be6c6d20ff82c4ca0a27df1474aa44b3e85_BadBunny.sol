...
contract BadBunny is ERC721AQueryable, DefaultOperatorFilterer, Ownable, Pausable {
    ...
    constructor(address _BADDIES, address _BTFD) ERC721A("BadBunny", "BBUN") {
    ...
    */
    ...
    /* Helper functions: 
        These functions will return true if an id has been used in a claim
        Will also return the index of the id in the array that was used
    */
    function checkBTFDids(uint256[] memory _ids) external view returns(bool used, uint256 i) {
    ...
    function checkBADids(uint256[] memory _ids) external view returns(bool used, uint256 i) {
        uint256 len = _ids.length;
        for (i = 0; i < len; i++) {
