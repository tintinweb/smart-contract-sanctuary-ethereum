    ...
    SnapshotModule
{
    ...
    mapping(address => bool) localBlacklist;

    event SetLocalWhitelist(address[] wallets, bool value);
    event SetLocalBlacklist(address[] wallets, bool value);
    ...
    function mint(address[] memory to, uint256 amount)
        public
    ...
    function setLocalWhitelist(address[] memory wallets, bool value)
        public
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        for (uint256 i = 0; i < wallets.length; i++) {
            localWhitelist[wallets[i]] = value;
        }
        emit SetLocalWhitelist(wallets, value);
    }

    function setLocalBlacklist(address[] memory wallets, bool value)
        public
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        for (uint256 i = 0; i < wallets.length; i++) {
            localBlacklist[wallets[i]] = value;
        }
        emit SetLocalBlacklist(wallets, value);
    }
    ...
    function _transfer(
        ...
        require(
            isSecurityDLT,
            "CMTAT: Security uncertificated. Transfer not allowed."
        );
...
abstract contract MintModule {
    ...
    event MintMultiple(address[] indexed beneficiaries, uint256 amount);
}
