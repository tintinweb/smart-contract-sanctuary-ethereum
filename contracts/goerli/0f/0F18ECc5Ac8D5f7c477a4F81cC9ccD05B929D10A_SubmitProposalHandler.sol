...
contract SubmitProposalHandler is Ownable {
    ...
    function handleInvokeEchelon(
        ...
        require(!paymentsDisabled, "Payments disabled");
        (string memory _nonce, string memory _hash) = abi.decode(
            _data,
            (string, string)
        );
        require(!proposalPaid[_hash], "Proposal already submitted");
        proposalPaid[_hash] = true;
        paymentCount += 1;
        emit ProposalSubmitted(_from, _nonce, _hash, paymentCount);
    }
    ...
    function setProposalCost(uint256 _val) external onlyOwner {
        proposalCost = _val;
    }
}
