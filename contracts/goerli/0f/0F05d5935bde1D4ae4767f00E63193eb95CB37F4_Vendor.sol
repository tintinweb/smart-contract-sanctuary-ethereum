...
contract Vendor is Ownable {
  ...
  event Withdrawal(address owner, uint256 amountOfETH);
  ...
  // ToDoo: create a payable buyTokens() function:
  function buyTokens() public payable {
    uint256 tokenBuyAmount = msg.value * tokensPerEth;
    uint256 vendorTokenBalance = yourToken.balanceOf(address(this));
    require(vendorTokenBalance >= tokenBuyAmount);
    yourToken.transfer(msg.sender, tokenBuyAmount);
    emit BuyTokens(msg.sender, msg.value, tokenBuyAmount);
  }

  // ToDoo: create a withdraw() function that lets the owner withdraw ETH
  ...
  // ToDoo: create a sellTokens(uint256 _amount) function:
  function sellTokens(uint256 _amount) public {
    ...
    require(userTokenBalance >= _amount, "user has less tokens than amount specified");
    require (ethAmount >= vendorEthBalance, "not enough eth in contract for token amount specified");
