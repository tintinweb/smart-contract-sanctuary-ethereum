...
contract ERC721 is ERC721A, Ownable, ReentrancyGuard, DefaultOperatorFilterer {
    ...
    uint256 public maxWLMinted = 3;

    uint256 public pMintStartTime = 1676455200;
    uint256 public pMintEndTime = 1676887200;
    ...
    event FlipSaleState();
    event SetMaxMint(uint256);
    event SetWLTime(uint256, uint256);
    event setWlPriceEvent(uint256);
    event SetPTime(uint256, uint256);
    event SetPPrice(uint256);
    event SetSuffixUri(string);
    event UpdateWhitelistLimitMinted(uint256);
    ...
    nonReentrant
    {
        ...
        uint256 kachousenOwned = IERC721A(kachousen).balanceOf(msg.sender);
        if(kachousenOwned >= 1) _maxWLMinted = 5;
    ...
    function userNftList(address _contractAddress, address _tokenOwner) external view returns (uint256[] memory) {
        uint256 balance = IERC721Enumerable(_contractAddress).balanceOf(_tokenOwner);
        uint256[] memory tokens = new uint256[](balance);
        for (uint256 i = 0; i < balance; i++) {
            tokens[i] = (IERC721Enumerable(_contractAddress).tokenOfOwnerByIndex(_tokenOwner, i));
        }
        return tokens;
    }

    function kachousenNftList(address owner, address _kachousen) external view returns (uint256[] memory) {
        uint256 balance = IERC721A(_kachousen).balanceOf(owner);
        uint256[] memory tokens = new uint256[](balance);
        unchecked {
            for (uint256 i = 1; i <= 3333; i++) {
            address o = IERC721A(_kachousen).ownerOf(i);
            if (o == owner) {
                tokens[i-1] = i;
            }
            }
