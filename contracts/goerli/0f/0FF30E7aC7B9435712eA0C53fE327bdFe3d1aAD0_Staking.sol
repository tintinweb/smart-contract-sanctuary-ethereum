...
contract Staking is Ownable {
    ...
    /// @dev User info(PoolInfo + All user Stakes info in that pool) to be sent offchain in response to view functions.
    ...
    function getPoolDetailsFrom(uint256 _startingPoolId, uint256 _numberOfPools) external view returns(PoolInfoResponse[] memory) {
    ...
    /**
     * @notice Function to return staker info (Address + Stake id) all stakes created in the pool.
     * @param _poolId Id of the stake pool for which to return StakerInfo of all stakes created.
     */
    function getAllStakerInfo(uint256 _poolId) external view returns(StakerInfo[] memory) {
        StakerInfo[] memory stakersInfoInPool = new StakerInfo[](totalStakesInPool[_poolId]);
        for(uint256 i=0; i<totalStakesInPool[_poolId]; ++i) {
            stakersInfoInPool[i] = stakerInfo[_poolId][i+1];
        }
    ...
    /**
     * @notice Function to return details of all stakes done in given pool by all users.
     * @param _poolId Pool Id for which to return all stake details.
     */
    function getAllStakeDetailsIn(uint256 _poolId) external view returns(StakeInfoResponse[] memory) {
        uint256 totalStakes = totalStakesInPool[_poolId];
        StakeInfoResponse[] memory stakeDetail = new StakeInfoResponse[](totalStakes);
        for(uint256 i=0; i<totalStakes; ++i) {
            StakerInfo memory stake = stakerInfo[_poolId][i+1];
            stakeDetail[i] = createStakeInfoResponse(stake.staker, _poolId, stake.stakeId);
        }
        return stakeDetail;
    }
    ...
    // |==============================================================================================================|
    // |------------------------------------FUNCTIONS RETURNING USER DETAILS------------------------------------------|
    // |==============================================================================================================|

    /**
     * @notice Returns [Pool Details + Details of all stakes created in this pool by user] => For all pools created.
     * @param _address User address for which to return details.
     */
    ...
    /**
     * @notice Returns [Pool Details + Details of all stakes created in this pool by user] => For pools currently live.
     * @param _address User address for which to return details.
     */
    ...
    /**
     * @notice Returns [Pool Details + Details of all stakes created in this pool by user] => For past pools.
     * @param _address User address for which to return details.
     */
    ...
    /**
     * @notice Returns [Pool Details + Details of all stakes created in this pool by user] => For active pools.
     * @param _address User address for which to return details.
     */
    ...
    /**
     * @notice Returns [Pool Details + Details of all stakes created in this pool by user] => For inactive pools.
     * @param _address User address for which to return details.
     */
    ...
    /**
     * @dev Creates and returns user data in PoolInfoResponse + StakeInfoResponse struct formats.
     */
    function createUserInfoResponse(PoolInfoResponse[] memory _poolInfo, address _address) private view returns(UserInfoResponse[] memory) {
        ...
        for(uint256 i=0; i<_poolInfo.length; ++i) {
            userInfo[i] = UserInfoResponse(
                _poolInfo[i],
                getAllStakeDetails(_address, _poolInfo[i].poolId)
            );
        }
        return userInfo;
    }
