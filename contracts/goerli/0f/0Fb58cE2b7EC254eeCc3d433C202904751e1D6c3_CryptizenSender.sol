...
contract CryptizenSender is Runnable, ReentrancyGuard{
    ...
    function mintCids(address nftAddress, address[] memory recepients, uint256[] memory tokenIds) public onlyOwner{
        ICryptizenNFT nft = ICryptizenNFT(nftAddress);
        for(uint256 index = 0; index < recepients.length; index++){
            nft.mintNft(recepients[index], tokenIds[index]);
        }
    }

    function transferCidMultiple(address nftAddress, address[] memory recepients, uint256[] memory tokenIds) public onlyOwner{
...
interface ICryptizenNFT{
    function mintNft(address to, uint256 tokenId) external returns(uint256);
