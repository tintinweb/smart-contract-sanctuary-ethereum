...
contract Core is ICore, CoreStorage {
  ...
  function borrow(
    ...
    IDebtToken(poolData.debtTokenAddress).mint(loanPrincipal, loanInterestRate);
  ...
  function repay(
    ...
    IDebtToken(poolData.debtTokenAddress).burn(repayAmount, loanInterestRate);
  ...
  function addNewPool(
    ...
    if (_poolData[asset].lastUpdateTimestamp != 0) revert DuplicatePool();
...
interface IIncentiveManager {
  ...
  /// @notice Emitted when totalIncentivePersecond is updated.
  event UpdateTotalIncentive(uint256 totalIncentivePerSecond);
  ...
  /// @notice Emitted when the user incentive is claimed.
  event ClaimIncentive(address pool, address receiver, uint256 amount);
...
import './IDebtTokenMetadata.sol';

interface IDebtToken is IDebtTokenMetadata {
  ...
  function burn(uint256 amount, uint256 rate) external;
...
library Factory {
    ...
    returns (address poolAddress, address debtTokenAddress)
  {
    ...
    Pool pool = new Pool{salt: salt}(asset);
    DebtToken debtToken = new DebtToken{salt: salt}(asset);

    return (address(pool), address(debtToken));
  }
}
...
library Rate {
  ...
  function updateRates(
    ...
    uint256 averageBorrowRate = IDebtToken(poolData.debtTokenAddress).getTotalAverageBorrowRate();
...
library Treasury {
  ...
  function accrueLoanFee(CoreStorage.PoolData storage poolData, uint256 interestIndex) internal {
    ...
    uint256 accruedProtocolTreasury = accruedDebt.rayMul(poolData.poolFactor);
...
error OptimalUExceedsRAY();
error BaseBorrowRateExceedsOptimal();
error OptimalBorrowRateExceedsMax();
error NotExistingAsset();
...
interface IInterestRateModel {
  ...
  /// @param asset Underlying asset address
  /// @param averageBorrowRate average borrow rate
...
contract Pool is IPool, PoolToken {
  ...
  constructor(address asset)
    PoolToken(
      string(
        abi.encodePacked(
          'Elyfi_',
          ICore(msg.sender).getERC20NameSafe(asset),
          '_PoolToken'
        )
      ),
      string(
        abi.encodePacked(
          'ELFI_',
          ICore(msg.sender).getERC20SymbolSafe(asset),
          '_PT'
        )
      ),
      asset
  ...
  function transferAsset(address receiver, uint256 amount) external override onlyCore {
    IERC20(_underlyingAsset).safeTransfer(receiver, amount);
  }

  /// @inheritdoc IPool
  /// @custom:check - check that `msg.sender` should be the address of `core`
  /// @custom:effect - mint (amount / index) worth of poolToken to `_protocolTreasury` contract.
  function mintToProtocolTreasury(uint256 amount, uint256 index) external override onlyCore {
    uint256 implicitBalance = amount.rayDiv(index);
    _mint(_protocolTreasury, implicitBalance);
    emit Mint(_protocolTreasury, implicitBalance, index);
  }
...
contract DebtToken is IDebtToken, Context {
  ...
  constructor(address asset) {
    _name = string(
      abi.encodePacked('Elyfi_', ICore(msg.sender).getERC20NameSafe(asset), '_DebtToken')
    );
    _symbol = string(abi.encodePacked('ELFI_', ICore(msg.sender).getERC20SymbolSafe(asset), '_DT'));
  ...
  function updateDebtTokenState() external override {
    ...
    emit UpdateDebtTokenState(_totalSupply, _lastUpdateTimestamp);
  }
  ...
  function mint(
    ...
    uint256 currentAverageBorrowRate = _averageBorrowRate;
    ...
    _averageBorrowRate = currentAverageBorrowRate = Calculation.calculateRateInIncreasingBalance(
      currentAverageBorrowRate,
      previousSupply,
      amount,
      rate
    );

    emit Mint(nextSupply - previousSupply, currentAverageBorrowRate, nextSupply);
  }

  /// @inheritdoc IDebtToken
  function burn(uint256 amount, uint256 rate) external override onlyCore {
    uint256 previousSupply = totalSupply();
    ...
    if (previousSupply <= amount) {
      ...
      uint256 secondTerm = rate.rayMul(amount.wadToRay());
    ...
    if (nextSupply > previousSupply) {
      emit Mint(nextSupply - previousSupply, newAvgStableRate, nextSupply);
    }
    else {
      emit Burn(previousSupply - nextSupply, newAvgStableRate, nextSupply);
    }
  }
...
library Calculation {
  ...
  function calculateRateInIncreasingBalance(
    ...
    uint256 newAverageRate = (weightedAverageRate + weightedAmountRate).rayDiv(
      (totalBalance + amountIn).wadToRay()
    );
