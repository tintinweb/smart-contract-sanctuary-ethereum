  ...
  ReentrancyGuard
{
  ...
  constructor(
    IERC20 token, // the token being claimed
    uint256 total, // the total claimable by all users
    bytes32 merkleRoot, // the merkle root for claim membership
    uint256 start, // vesting clock starts at this time
    uint256 cliff, // claims open at this time
    uint256 end, // vesting clock ends and this time
    uint256 voteWeightBips,
    string memory uri // information on the sale (e.g. merkle proofs)
  )
    ContinuousVesting(start, cliff, end)
    MerkleSet(merkleRoot)
    Distributor(token, total, voteWeightBips, uri)
  {}
  ...
  ) validMerkleProof(index, beneficiary, amount, merkleProof) external {
  ...
  function claim(
    ...
    uint256 amount, // the total claimable by this beneficiary
    bytes32[] calldata merkleProof
  ) external validMerkleProof(index, beneficiary, amount, merkleProof) nonReentrant {
...
abstract contract ContinuousVesting is Distributor {
  uint256 public immutable start; // time vesting clock begins
  uint256 public immutable cliff; // time vesting begins (all tokens vested prior to the cliff are immediately claimable)
  uint256 public immutable end; // time vesting clock ends
  ...
  function _getVestedBips(
	address /*beneficiary*/,
	uint time // time is in seconds past the epoch (e.g. block.timestamp)
	...
	return 10000 * (time - start) / (end - start);
  } 
}
