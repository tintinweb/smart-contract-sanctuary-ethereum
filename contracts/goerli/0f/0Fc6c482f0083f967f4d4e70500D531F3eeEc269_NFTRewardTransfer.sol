...
contract NFTRewardTransfer is Ownable {
    ...
    // function claimNFT(address token, address from, address to, uint256 amount) external onlyOwner {
    ...
    function checkWhaleNFT(address _whaleaddr) external view returns (bool) {
    ...
    function blackBlack(address _address) external onlyOwner {
        require(_address != address(0),"Address should not be 0");
        isBlacklisted[_address] = true;
    }

    function whitelistNFT(address[] memory _wat, bool status) external onlyOwner {
    ...
    function setTokenIsFinishedNFT(bool _isFinished) external onlyOwner {
      isFinished = _isFinished;
    }

    function setLastTimeForTokensNFT() external onlyOwner {
