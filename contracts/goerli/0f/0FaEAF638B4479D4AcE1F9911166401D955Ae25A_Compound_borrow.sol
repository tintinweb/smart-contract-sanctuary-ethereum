...
contract Compound_borrow is AccessControl{
    ...
    mapping(address=>mapping(address=>uint)) public userGetCTokensForSupply;
    ...
    function supplyERC20ToCompound(address _token,uint _numTokenForSupply) external returns(uint,uint,uint){
        ...
        userGetCTokensForSupply[msg.sender][_ctoken] = mintCTokens;
    ...
    function redeemERC20FromCompound(uint amount,bool reddemType,address _cToken)public returns(uint){
        require(usertokensSuppliedToCompound[msg.sender][_cToken] > 0,"Did not supply before using redeem");
        uint redeedmReslt;
        uint suppliedBalance = usertokensSuppliedToCompound[msg.sender][_cToken];
        uint balanceNow = suppliedBalance - amount;
        usertokensSuppliedToCompound[msg.sender][_cToken] = balanceNow;
        //uint balanceCtoken = userGetCTokensForSupply[msg.sender][_cToken];
