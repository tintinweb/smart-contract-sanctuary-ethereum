    ...
    Ownable
{
    ...
    mapping(uint256 => mapping(uint256 => bool)) public claimedId;
    ...
    function publicClaim(uint256 _id, uint256 _quantity) external {
        ...
        uint256 offset = ownedIds.length - mintsAvailable;
        for (uint256 i = offset; i < offset + _quantity; i++) {
            if (!claimedId[ownedIds[i]][_id]) {
                claimedId[ownedIds[i]][_id] = true;
            }
        }
