...
import { ITokenizer } from "./ITokenizer.sol";

//Insipired by https://github.com/beber89/dividend-erc20
contract Tokenizer_ERC20 is Ownable, ReentrancyGuard, ERC20, ITokenizer {
    ...
    /// @notice Holds the Rent metrics
    RentMetric _rentMetric;
    ...
    modifier isUnlocked(uint256 /*id_*/) {
    ...
    function setRentMetric(uint256 /*id_*/, uint256 seconds_, uint256 gwei_) public override {
        uint256 holderBalance = balanceOf(_msgSender());
        uint256 sharePercent = holderBalance * 100 / totalSupply();
        require(sharePercent > 50, "Can only be set by the majority holder");
        _rentMetric._seconds = seconds_;
        _rentMetric._gwei = gwei_;
    }

    function getRentMetrics(uint256 /*id_*/) public override view returns (RentMetric memory) {
        return _rentMetric;
    }

    function toggleLock(uint256 /*id_*/) external override onlyOwner {
        locked = !locked;
    }
    ...
    function withdraw(uint256 id_) external override nonReentrant isUnlocked(id_) {
        uint256 holderBalance = balanceOf(_msgSender());
    ...
    function emergencyWithdraw(uint256 /*id_*/) external override onlyOwner {
    ...
    //=================================== PRIVATE ============================================== 
    function mint(address to_, uint256 amount_) private onlyOwner mintable(amount_) {
...
interface ITokenizer {
    ...
    function setRentMetric(uint256 id_, uint256 seconds_, uint256 gwei_) external;
    
    function getRentMetrics(uint256 id_) external view returns (RentMetric memory);

    function toggleLock(uint256 id_) external;

    function withdraw(uint256 id_) external;
    
    function emergencyWithdraw(uint256 id_) external;
}
