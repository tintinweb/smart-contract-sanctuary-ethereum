...
contract DOEStaking is TokenWrapper, NFTWrapper, Ownable {
	...
	event UpdategDOEAddress(IGDOE indexed gDOE);
    ...
    function withdrawForgDOE(uint256 amount) public updateReward(msg.sender) {
        super.withdrawForgDOE(msg.sender, amount);
    }
	
	function stakeForgDOE(uint256 amount) public updateReward(msg.sender) {
        super.stakeForgDOE(msg.sender, amount);
    }
...
contract TokenWrapper {
    ...
    function stakeForgDOE(address account, uint256 amount) internal {
        require(amount > 0, "Cannot stake 0");

        require(
            gDOE.depositForStaking(account, amount),
            _TransferErrorMessage
        );
        ...
        unchecked {
            totalSupply += amount;
            _balances[account] += amount;
            _gDOEstakers[account] = true;
        }
		
        emit Staked(account, amount);
    }

    function withdrawForgDOE(address account, uint256 amount) internal {
        require(_gDOEstakers[account] == true, "Make have staked for gDOE");
        require(amount <= _balances[account], "withdraw: balance is lower");

        require(
            gDOE.withdrawFromStaking(account, uint256(amount)),
