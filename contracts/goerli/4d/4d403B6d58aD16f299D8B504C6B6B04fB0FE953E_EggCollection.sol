  ...
  DefaultOperatorFilterer
{
  ...
  constructor() ERC721A("Metropolis World Avatar Eggs", "MWAE") {
  ...
  function tokenOfOwnerByIndex(address owner, uint256 index) public view virtual returns (uint256) {
    require(index < ERC721A.balanceOf(owner), "ERC721A: owner index out of bounds");
    uint[] memory _ownedTokens = this.tokensOfOwner(owner);
    return _ownedTokens[index];
  }
  ...
  function internalMint(address to, uint[] calldata eggIds) internal {
    uint currentId = _nextTokenId();
    _safeMint(to, eggIds.length);

    for(uint i; i < eggIds.length; i++) {
      tokenIdToEggId[currentId + i] = eggIds[i];
      tokenIdToRevealed[currentId + i] = false;
    }
  }

  function bulkAirdrop(address[] memory tos, uint[] calldata eggIds) external onlyRole(UPDATER_ROLE) {
    for(uint i; i < tos.length; i++) {
      internalMint(tos[i], eggIds);
    }
  }

  function airdropMint(address to, uint[] calldata eggIds) external onlyRole(UPDATER_ROLE) {
    internalMint(to, eggIds);
  }
  ...
  function attachToAvatar(
    ...
    transferFrom(msg.sender, address(Avatars), _tokenId);
  }
  ...
  function withdraw(address to)external onlyRole(UPDATER_ROLE){
