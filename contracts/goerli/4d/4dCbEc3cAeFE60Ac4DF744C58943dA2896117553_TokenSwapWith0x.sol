...
contract TokenSwapWith0x {
    ...
    function _processBuyToken(
      ...
      if(sellToken == WETH) {
        ...
        require(sellToken.approve(address(this),type(uint256).max),"Approve sellToken to this contract failed");
        require(sellToken.approve(swapTarget,type(uint256).max),"Approve sellToken to swapTarget failed");
        require(sellToken.transfer(address(this), amountInSellToken),"Transfer failed");
      }
