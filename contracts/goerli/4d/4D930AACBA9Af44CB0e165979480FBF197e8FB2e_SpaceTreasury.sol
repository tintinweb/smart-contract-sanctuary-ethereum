...
contract SpaceTreasury is SpaceAccessControlled, ITreasury {
	...
	function create(
		...
		ownerPricingReserves[tokenId] = 250000 * 10**9; // the most flexible constant
		userPricingReserves[tokenId]  = 250000 * 10**9; // for price vs rebase amount
    ...
    function deposit(
        ...
        emit Deposit(token_, id_, amount_, toRemove);
    }
	...
	function buy(
		...
		emit Bought(token_, tokenId_, amount_, toSend, to_);
	}
    ...
    function incurDebt(
        ...
        emit CreateDebt(msg.sender, tokenId_, token_, amount_, debt);
    }
    ...
    function repayDebt(
        ...
        emit RepayDebt(msg.sender, tokenId_, token_, amount_, refund);
    }
	...
	function _rebase(uint256 tokenId, uint256 amount0Out, uint256 amount1Out) internal {
		...
		uint256 circulation = IERC20Rebase(tokenAddress).circulatingSupply();
		uint256 total = IERC20(tokenAddress).totalSupply();
		int256 delta = 0;

		if (circulation != 0) {
			uint256 target = getAmountIn(ownerReserves[tokenId], amount1Out, amount0Out);
			delta = int256(target * total / circulation) - int256(total);
		}
