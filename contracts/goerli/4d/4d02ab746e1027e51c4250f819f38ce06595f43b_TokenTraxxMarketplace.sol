...
interface IERC1155Custom {
    /**
     * @dev Returns the maximum quantity which can be purchased.
     */
    ...
    ErrorCodes
{
    ...
    function initialize(
        ...
        require(
            ...
            throwError(COLLECTION_CONTRACT_CANNOT_BE_ZERO_ADDRESS)
        );
        require(
            _primaryMarketFeeBps <= MAX_BPS && _marketFeeBps <= MAX_BPS, 
            throwError(BPS_EXCEEDS_10000)
        );
        require(_initialCurrency != address(0), throwError(ZERO_ADDRESS));
        ...
        override
    {
        ...
        if(_tokenListings[_params.assetContract][_params.tokenId].length >= 1)
    ...
    function delegatedBuy(
        ...
        else if (listings[_listingId].tokenType == TokenType.ERC1155) {
