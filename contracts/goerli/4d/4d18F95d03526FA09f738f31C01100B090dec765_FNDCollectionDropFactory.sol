 ...
 */
contract FNDCollectionDropFactory is Initializable {
  ...
  /**
   * @notice The address of the template all new BatchMintAndReveal collections will leverage.
   */
   ...
   * @param _marketContract The address of the market contract the factory can trust.
   */
  ...
  function createBatchMintAndRevealCollectionWithPaymentFactory(
    ...
    address payable paymentAddress = paymentAddressFactory.callAndReturnContractAddress(paymentAddressCallData);
    return
      _createBatchMintAndRevealCollection(
        name,
        symbol,
  ...
  function _createBatchMintAndRevealCollection(
    string calldata name,
    string calldata symbol,
    address payable paymentAddress,
    bytes32 baseURIHash,
    ...
    uint32 maxTokenId,
    uint256 nonce
  ) internal returns (address batchMintAndRevealCollectionAddress) {
