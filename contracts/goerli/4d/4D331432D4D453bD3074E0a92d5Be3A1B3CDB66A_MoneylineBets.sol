...
interface IMoneylineBets {
    ...
    struct Bet {
        ...
        mapping(Result => uint256) totalCount;
...
contract MoneylineBets is IMoneylineBets, AccessControl {
    ...
    event InvalidateBet(uint256 indexed id, uint256 indexed fromIdx, uint256 indexed toIdx);
    ...
    function makeBet(uint256 id, Result choice, uint256 ticketCount) external payable {
        ...
        bet.totalCount[choice] += ticketCount;
    ...
    function closeBets(
        ...
        for (uint256 i = 0; i < ids.length; i++) {
            ...
            if (bet.choices[bet.result].length != 0 && result != Result.CANCEL) {
                bet.prizePerTicket = totalPrize / bet.totalCount[bet.result];
            }
    ...
    function invalidateBet(uint256 id, Result choice, uint256 fromIdx, uint256 limit, bool isLast) external onlyRole(OPERATOR_ROLE) {
        Bet storage bet = bets[id];
        require(bet.status != Status.FINALIZED && bet.result == Result.CANCEL, "Already finalized or not canceled");
        ...
        for (uint256 i = fromIdx; i < toIdx; i++) {
            bet.claimable[bet.choices[choice][i]] += bet.pricePerTicket * bet.counts[choice][i];
        }
        ...
        emit InvalidateBet(id, fromIdx, toIdx);
    }
