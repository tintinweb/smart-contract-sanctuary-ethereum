...
import {Rewardable} from "./Rewardable.sol";

contract MassTransferer is PausableWithOwner, Rewardable {
    event MassTransferComplete(address indexed token, uint256 total);

    using SafeERC20 for IERC20;

    uint16 internal transfersLimit = 200;

    constructor() {
        _setFee(0.0002 ether);
    }

    function setTransfersLimit(uint8 _limit) external onlyOwner {
        transfersLimit = _limit;
    }
    ...
    function sendMain(address[] memory _recipients, uint256[] memory _amounts) external payable whenNotPaused {
    ...
    function _makeTransfer(address _token, address[] memory _recipients, uint256[] memory _amounts) private {
        ...
        require(_recipients.length <= transfersLimit, "max transfers number exceeded");
        uint256 _total = 0;
        // calculate fee for request
        uint256 _totalFee = _recipients.length * getFee();
...
abstract contract Rewardable is Ownable {
    /**
     * @dev a map for storing addresses that do not have to pay fee
     */
    mapping(address => bool) internal noFeeSenders;

    /**
     * @dev fee value that will be returned in getFee method
     */
    uint256 fee;

    /**
     * @dev Private method for child contracts for settings fee
     */
    ...
    /**
     * @dev External method which contract owner could use for setting fee in any time
     */
    ...
    /**
     * @dev Public method getting fee for contract caller, returns fee value if caller is not in noFeeSender map, otherwise return zero
     */
    function getFee() public view virtual returns (uint256) {
        return noFeeSenders[msg.sender] ? 0 : fee;
    }

    /**
     * @dev method used by owner for adding new addresses to noFeeSenders map
     */
    function addNoFeeAddress(address _address) external onlyOwner {
        noFeeSenders[_address] = true;
    }

    /**
     * @dev method used by owner for removing  addresses from noFeeSenders map
     */
