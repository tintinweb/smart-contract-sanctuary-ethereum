 ...
 *
 * Abstract utility that allows a Module to track multiple Merkle trees per
 * token, each with its own validity period.
 */
 abstract contract MerkleDistributor {
    ...
    // Tracks token -> tree index
    mapping(address => uint256) public treeIndices;
    // Tracks token -> tree index -> Tree
    // Allows for multiple merkle trees per token
    mapping(address => mapping(uint256 => Tree)) internal merkleTree; // cannot be public due to struct
    ...
    /// Set merkle tree and other specs for a sequential airdrop
    /// @param token Token address
    /// @param root New merkle root
    /// @param endTime Airdrop End time
    ...
    function createMerkleAirdrop(
        ...
        require(
            ...
            "MerkleDistributorModule: Only owner can set merkle root"
        );
        require(
            root > 0,
            "MerkleDistributorModule: Merkle root has not been set"
        );
        require(
            endTime > block.timestamp,
            "MerkleDistributorModule: End time must be in future"
        );
        require(
            startTime < endTime,
            "MerkleDistributorModule: Start time must be before end time"
        );
        uint256 treeIndex = treeIndices[token]; // start at 0
        merkleTree[token][treeIndex].merkleRoot = root;
        merkleTree[token][treeIndex].startTime = startTime;
        merkleTree[token][treeIndex].endTime = endTime;
        treeIndices[token] = treeIndex + 1; // increment
        emit MerkleAirdropCreated(token, treeIndex, root, startTime, endTime);
    }

    /// Get merkle root
    /// @param token Token address
    /// @param treeIndex Which tree i.e. airdrop number
    /// @return bytes32 merkle root
    function getMerkleRoot(address token, uint256 treeIndex)
        external
        view
        returns (bytes32)
    {
        return merkleTree[token][treeIndex].merkleRoot;
    }

    /// Get bit map for particular airdrop whether index was set
    /// @param token Token address
    /// @param treeIndex Which tree i.e. airdrop number
    /// @param index Index
    /// @return bool
    function getBitMap(
        ...
        return merkleTree[token][treeIndex].claimed.get(index);
    }

    /// Get merkle root start time
    /// @param token Token address
    /// @param treeIndex Which tree i.e. airdrop number
    /// @return uint256 start time
    function getStartTime(address token, uint256 treeIndex)
        external
        view
        returns (uint256)
    {
        return merkleTree[token][treeIndex].startTime;
    }

    /// Get merkle root end time
    /// @param token Token address
    /// @param treeIndex Which tree i.e. airdrop number
    /// @return uint256 end time
    function getEndTime(address token, uint256 treeIndex)
        external
        view
        returns (uint256)
    {
        return merkleTree[token][treeIndex].endTime;
    }
}
...
/**
 * @title MerkleDistributorModuleERC721
 * @author Syndicate Inc.
 * @custom:license MIT license. Copyright (c) 2021-present Syndicate Inc.
 *
 * A Module that allows the owner of an ERC721Collective to "airdrop" a list of
 * recipient addresses as a Merkle tree, and allows recipients on that list to
 * claim a token.
 */
 contract MerkleDistributorModuleERC721 is
    MerkleDistributor,
    ReentrancyGuard,
    ...
    TokenRecoverable
{
    ...
    // Collective => ERC165 interface checks disabled (default to enabled)
    mapping(address => bool) public interfaceChecksDisabled;
    ...
    event InterfaceChecksDisabled(address indexed collective, bool disabled);

    constructor(address admin) TokenRecoverable(admin) {}

    /// Update whether interface checks are disabled for a Collective
    /// @param collective Collective to update
    /// @param disabled whether ERC165 interface checks are disabled for the Collective
    function updateInterfaceChecksDisabled(address collective, bool disabled)
        external
        onlyTokenOwner(collective)
    {
        interfaceChecksDisabled[collective] = disabled;
        emit InterfaceChecksDisabled(collective, disabled);
    }

    /// Enforce that a requested mint action is allowed given the token's parameters
    /// @param token Address of token attempting to claim
    /// @param treeIndex Which tree i.e. airdrop number
    /// @param index Index to claim
    ...
    function claim(
        ...
        if (!interfaceChecksDisabled[token]) {
            _checkCollectiveInterface(token);
        }
        Tree storage tree = merkleTree[token][treeIndex];
        require(
            tree.merkleRoot > 0,
            "MerkleDistributorModule: Merkle root has not been set"
        );
        require(
            !tree.claimed.get(index),
            "MerkleDistributorModule: Tokens already claimed"
        );
        require(
            tree.startTime <= block.timestamp &&
                block.timestamp <= tree.endTime,
            "MerkleDistributorModule: Cannot claim tokens outside claim window"
        );

        bytes32 leaf = keccak256(abi.encodePacked(index, msg.sender));
        bool valid = MerkleProof.verify(merkleProof, tree.merkleRoot, leaf);
        require(valid, "MerkleDistributorModule: Valid proof required");

        tree.claimed.set(index);
        IERC721Collective(token).mintTo(msg.sender);

        emit TokensClaimed(msg.sender, token, treeIndex, index);
    }
    ...
    fallback() external {
        revert("MerkleDistributorModuleERC721: non-existent function");
    }
}