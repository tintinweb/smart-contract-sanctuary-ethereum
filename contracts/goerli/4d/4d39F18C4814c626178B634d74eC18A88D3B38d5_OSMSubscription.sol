...
abstract contract Approvable is Ownable, OwnerEnumerable {  
...
abstract contract FlexibleMetadata is Context, ERC165, IERC721, IERC721Metadata {  
...
contract OSMSubscription is TokenReceiver {  
    ...
    function mintForRecipient(address recipient, uint256 numberOfDays) external onlyOwner { 
        _mintTokenFor(recipient,numberOfDays);
    }
    ...
    // function renewOSM(uint256 tokenId, uint256 numberOfDays, string calldata discountCode) external payable {
    //     payOutFees(numberOfDays, discountCode);
    //     renewSubscription(tokenId, numberOfDays, discountCode);
    // }

    function renewOSM(uint256 tokenId, uint256 numberOfDays) external payable {
        payOutFees(numberOfDays, blank);
        renewSubscription(tokenId, numberOfDays, blank);
    }
}
