...
contract DiamondCutFacet is Base, IDiamondCut {
    ...
    /// @dev Starts the upgrade process. Only current governor can propose upgrade.
    ...
    function proposeDiamondCut(Diamond.FacetCut[] calldata _facetCuts, address _initAddress) external onlyGovernor {
        ...
        // That means the governor can call `initAddress` with ANY calldata while executing of the upgrade.
    ...
    function executeDiamondCutProposal(Diamond.DiamondCutData calldata _diamondCut) external onlyGovernor {
        ...
        require(approvedBySecurityCouncil || !diamondStorage.isFreezed, "f3");
    ...
    function emergencyFreezeDiamond() external onlyGovernor {
        Diamond.DiamondStorage storage diamondStorage = Diamond.getDiamondStorage();
        require(!diamondStorage.isFreezed, "a9"); // diamond proxy is freezed already
        _resetProposal();
    ...
    function unfreezeDiamond() external onlyGovernor {
        ...
        require(diamondStorage.isFreezed, "a7"); // diamond proxy is not freezed
