    ...
    IAnomuraEquipment
{
    ...
    /// @dev Emit an event so an offchain source can build the metadata equipment
    ...
    /**
     * @dev Keep track of Equipment from tokenId
     */
    mapping(uint256 => EquipmentMetadata) private _equipmentMetaById;
    ...
    */
    ...
    function getMetadataForToken(uint256 _tokenId)
        external
        view
        returns (EquipmentMetadata memory)
    {
        return _equipmentMetaById[_tokenId];
    }
    ...
    function claim(
        ...
        uint256 quantity,
        string memory nonce
    ) external isValidMerkleProof(_claimProof, _claimMerkleRoot) {
    ...
    **/
    function setMetaDataForToken(bytes calldata performData)
        external
        onlyRole(SET_METADATA_ROLE)
    {
        ...
        _equipmentMetaById[tokenId] = metaData;
    ...
    */
    ...
    function setSignerAddress(address _addr)
        external
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
    ...
    */
    function setClaimMerkleRoot(bytes32 _merkleRoot)
        external
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        _claimMerkleRoot = _merkleRoot;
        // emit UpdatedMerkleRootOfClaim(_merkleRoot, msg.sender);
    }

    function toggleClaimStatus() external onlyRole(DEFAULT_ADMIN_ROLE) {
        claimLive = !claimLive;
    }
