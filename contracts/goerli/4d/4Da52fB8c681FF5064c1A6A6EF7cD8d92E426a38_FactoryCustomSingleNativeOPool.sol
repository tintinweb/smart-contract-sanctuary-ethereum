...
// File: yomiswap-v2/src/OtherPool.sol
...
contract OtherPool is BasePool {
    ...
    //@param totalNFTprice: total price of NFT price
        ...
        returns (uint256 _LP)
    {
        if (totalNFTpoint == 0) {
            ...
            _LP = totalNFTpoint * _totalFee / (totalNFTprice + totalNFTfee);
        }
    }
    ...
    {
        uint256 _tmpPrice = ((totalNFTprice + totalNFTfee) * userInfo[_user].totalNFTpoint / totalNFTpoint);
        if (_tmpPrice > userInfo[_user].totalNFTprice) {
            _userFee = _tmpPrice - userInfo[_user].totalNFTprice;
        } else {
            _userFee = 0;
        }
    }
...
// File: yomiswap-v2/src/NativeOPool.sol


pragma solidity =0.8.17;


contract NativeOPool is OtherPool {
        ...
        returns (uint256 _protocolFee)
    {
        ...
        //check
        require(_itemNum <= poolInfo.buyNum, "Not enough NFT");
    ...
    function swapNFTforFT(
        ...
        //check
        require(_itemNum <= poolInfo.sellNum, "Not enough NFT");
...
// File: yomiswap-v2/pool/CustomSingleNativeOPool.sol
...
contract CustomSingleNativeOPool is NativeOPool {
    constructor(
        address _collection,
        ...
        onlyRouter
    {
        ...
        //effect
        uint256 _totalFee = stakeNFTprice * _itemNum;
        // - NFTpoint
        uint256 _stakeNFTpoint = _calcNFTpoint(_totalFee);
        userInfo[_user].totalNFTpoint += _stakeNFTpoint;
        totalNFTpoint += _stakeNFTpoint;
        // - NFTprice
        userInfo[_user].totalNFTprice += _totalFee;
        totalNFTprice += _totalFee;
        // - BuyNum
    ...
    function withdrawNFT(uint256[] calldata _tokenIds, address _user)
        external
        payable
        onlyRouter returns(uint256 _userFee)
    {
        uint256 _itemNum = _tokenIds.length;
        ...
        //effect
        // - NFTprice
        totalNFTprice -= userInfo[_user].totalNFTprice;
        userInfo[_user].totalNFTprice = 0;
        // - BuyNum
        poolInfo.buyNum -= _itemNum;
    ...
    function withdrawFee(address _user) external payable onlyRouter {
        uint256  _totalStakeFee;
        
        if(userInfo[_user].totalNFTpoint > 0){
            uint256 _totalStakeNFTfee = _calcNFTfee(_user);
            uint256 _totalStakeNFTpoint = totalNFTpoint * _totalStakeNFTfee / (totalNFTprice + totalNFTfee);
            _totalStakeFee += _totalStakeNFTfee;
            _updateNFTpointAndFee(_user, _totalStakeNFTfee, _totalStakeNFTpoint);
        }
    ...
    function _updateNFTpointAndFee(address _user, uint256 _userFee, uint256 _userPoint) internal {
      userInfo[_user].totalNFTpoint -= _userPoint;
      totalNFTpoint -= _userPoint;
      totalNFTfee -= _userFee;
    }
...
// File: yomiswap-v2/factory/FactoryCustomSingleNativeOPool.sol
