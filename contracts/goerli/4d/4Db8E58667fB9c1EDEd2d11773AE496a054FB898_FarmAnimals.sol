...
contract FarmAnimals is ERC721AQueryable, IFarmAnimals, Ownable, ERC2981ContractWideRoyalties, ERC721ACheckpointable {
  ...
  uint16 private maxRoosters = 990; // max number of allowed roosters to be minted

  // HACK: Set to 40% for local dev, should be 2%
  uint256 private twinHenRate = 40; // rate for twin hen mint
  ...
  // used to ensure there are no duplicates traits produced
  ...
  function mint(address recipient, uint256 seed) external onlyController returns (uint16[] memory) {
    ...
    kind = _generateAndStoreTraits(minted, seed, 0, false, 0).kind;

    if (tx.origin != recipient && recipient != address(henHouse)) {
      ...
      if (kind == Kind.HEN) {
        ...
        if (!_applePieTaken) {
          ...
          emit TokenKidnapped(tx.origin, recipient, minted, 'HEN');
          uint256 rescuedChance = ((seed >> 185) % 10);

          // HACK: Set to 60% for dev, should be 30%
          if (rescuedChance < 6) {
            ...
            if (rescuer != address(0x0)) {
              ...
              emit TokenRescued(recipient, finalRecipient, uint16(minted), 'HEN');
            }
          }
        }
        ...
        if (!_applePieTaken) {
          ...
          emit TokenKidnapped(tx.origin, recipient, minted, 'COYOTE');
        }
    ...
    if (twinRate < twinHenRate && kind == Kind.HEN) {
      ...
      _mint(finalRecipient, 1);
      emit Mint('HEN', finalRecipient, minted);
  ...
  function _beforeTokenTransfers(
    ...
    // FIXME: To only movevotes if its a Gen0 token
  ...
  function _setRoosterChance(uint16[] memory _chances) internal {
    require(_chances.length > 0 && _chances.length < 7, 'Wrong length');
  ...
  function _setRoosterSupply(uint16[] memory _supply) internal {
    require(_supply.length > 0 && _supply.length < 7, 'Wrong length');
    ...
    returns (uint256)
  {
    ...
    require(controllers[_msgSender()] || lastWriteAddress[owner].blockNum < block.number, 'LastWrite check');
