...
contract POMGDistributor is ReentrancyGuard, Ownable {
	...
	function addSwapTransaction(bytes32 txhash, address owner, uint256 amount) external onlySigner{		
		...
		//first claim checkpoint
		uint256 claimCheckpoint = distributionStart - claimInterval;
		if(block.timestamp > distributionStart){
			claimCheckpoint = distributionStart + (((block.timestamp - distributionStart) / claimInterval) * claimInterval);
			claimCheckpoint -= claimInterval; 
		}
	...
	function claimById(uint256 _swapId) public {		
		...
		if(pendingClaim > 0){
			...
			totalDistribution += pendingClaim;
	...
	function pendingClaimById(uint256 _swapId) public view returns (uint256) {
		...
		if(userSwap.claimAmount - userSwap.claimedAmount > 0){
			...
			if(pendingClaimDay > 0){
				...
				if(pendingClaim > userSwap.claimAmount - userSwap.claimedAmount){
					pendingClaim = userSwap.claimAmount - userSwap.claimedAmount;
				}
			}
		}
