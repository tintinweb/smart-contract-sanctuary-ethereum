...
contract Random is IRandom, OwnableUpgradeable {
    ...
    function getRandomVariant() external override returns (uint256) {
    ...
    function getRandomClass() external override returns (uint8) {
        uint256 number = random();
        uint numberRandomSplit = (number %= 1000);
        emit RandomSplit(numberRandomSplit);
        return uint8(_getChosenClass(numberRandomSplit, 0));
    }

    event RandomSplit(uint256 number);
    event RandomStage(uint256 randomNumber, uint256 number);
    event RandomBioMeta(uint256 number);
        ...
        returns (uint256)
    {
        ...
        uint256 number = random() % 1000;
        uint256 randomNumber = number;
        number = number + (probabilityIncrease * 10);
        emit RandomStage(randomNumber, number);

        if (number <= 699) {
            _stage = _stage + 1;
        } else if (number >= 700 && number <= 899) {
            _stage = _stage + 2;
        } else if (number >= 900 && number <= 949) {
            _stage = _stage + 3;
        } else if (number >= 950 && number <= 979) {
            _stage = _stage + 4;
        } else if (number >= 980) {
            _stage = _stage + 5;
        }
    ...
    function getGroup(uint256 number) private returns (Group memory group) {
        if (number >= 6000 && number <= 8999) {
            group.startBlock = 10;//74000;
            group.step = 0;//10000;
            group.stages = 9;
        } else if (number >= 9000 && number <= 10000) {
            group.startBlock = 10;//24000;
            group.step = 0;//5000;
            group.stages = 9;
        } else {
            group.startBlock = 10;//174000;
            group.step = 0;//10000;
            group.stages = 10;
        }
        ...
        returns (uint256 amount)
    {
        ...
        uint256 typeAmount = _random % 1000;
        emit RandomBioMeta(typeAmount);
        if (typeAmount <= 899) {
            amount =
                ((_randomByDifficulty - typeAmount * 1000) % 91) +
                10;
        } else if (typeAmount >= 900 && typeAmount <= 969) {
            amount =
                ((_randomByDifficulty - typeAmount * 1000) % 901) +
                100;
        } else {
            amount =
                ((_randomByDifficulty - typeAmount * 1000) % 9001) +
                1000;
        }
    }
...
interface IRandom {
    ...
    function randomEnhancerId(uint256 limit) external returns (uint256 randomId);

    function randomRateSplitBiometaToken() external returns (uint256 amount);
}
