...
import "../interfaces/INash21OffChainData.sol";
import "../utils/UUPSUpgradeableByRole.sol";

/// @title The contract of the Nash21OffChainData contract
/// @notice Stores off chain data about the NFTs
contract Nash21OffChainData is INash21OffChainData, UUPSUpgradeableByRole {
    // Storage
    bytes32 private constant _OFFCHAINDATA_ADMIN_ROLE = keccak256("OFFCHAINDATA_ADMIN_ROLE");
    mapping(uint256 => string) private _contractHashByTokenId;

    /// @inheritdoc INash21OffChainData
    function contractHashByTokenId(uint256 tokenId) external view returns (string memory) {
        string memory baseURI = "ipfs://";
        uint256 originTokenId = _getOriginTokenId(tokenId);
        return string(abi.encodePacked(baseURI, _contractHashByTokenId[originTokenId]));
    }
    ...
    /// @inheritdoc INash21OffChainData
    function setContractHash(uint256 tokenId, string memory cid) external onlyRole(_OFFCHAINDATA_ADMIN_ROLE) {
        uint256 originTokenId = _getOriginTokenId(tokenId);
        _contractHashByTokenId[originTokenId] = cid;
        emit SetContractHash(originTokenId, cid);
    }

    function _getOriginTokenId(uint256 tokenId) internal view returns (uint256) {
        (uint256 originTokenId,,,,,,,) = INash21Factory(INash21Manager(manager).get(keccak256("FACTORY"))).data(tokenId);
