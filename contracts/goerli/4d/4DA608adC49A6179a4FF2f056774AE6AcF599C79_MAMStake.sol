...
contract MAMStake is Ownable, IERC721Receiver, ReentrancyGuard {
    ...
    uint256 public firsttier = 7; //number of days
    ...
    constructor(address _nft, address _token, uint256[] memory _mutantIds, uint256[] memory _topTenIds, uint256[] memory _legendaryIds)  {
        nft = IERC721Enumerable(_nft);
        token = IERC20(_token);

        for(uint16 i; i < _mutantIds.length; i++) {
            mutantIds.push(_mutantIds[i]);
        }
        for(uint16 i; i < _topTenIds.length; i++) {
            topTenIds.push(_topTenIds[i]);
        }
    ...
    function stake(uint256 tokenId) external nonReentrant{
        ...
        for(uint16 i; i<=mutantIds.length; i++) {
            if(tokenId == mutantIds[i]) {
                mMultiplier[tokenId] = true;
            }
        }
        for(uint16 i; i<=topTenIds.length; i++) {
            if(tokenId == topTenIds[i]) {
    ...
    function unstake(uint256 tokenId) external nonReentrant {
        ...
        if (time < firsttier) {
            revert("Not enough time has elapsed to unstake");
        } else {
            if(time >= firsttier && time < secondtier) {

            if(lMultiplier[tokenId] == true) {
                payout = dayRate * firsttier * 20;
            } else if(tMultiplier[tokenId] == true) {
                payout = dayRate * firsttier * 4;
            } else if(mMultiplier[tokenId]==true) {
                payout = dayRate * firsttier * 2;
            } else {
                payout = dayRate * firsttier / 2;
            } 
            }
            else if (time >= secondtier) {
                if(lMultiplier[tokenId] == true) {
                payout = dayRate * secondtier * 40;
            } else if(tMultiplier[tokenId] == true) {
                payout = dayRate * secondtier * 8;
            } else if(mMultiplier[tokenId]==true) {
                payout = dayRate * secondtier * 4;
            } else {
                payout = dayRate * secondtier;
            } 
            }
    ...
    function addmutantIds(uint256[] calldata _newIds) external onlyOwner {
      ...
      mutantIds.push(_newIds[i]);
        }
  }

  function addTopTenIds(uint256[] calldata _newIds) external onlyOwner {
      for(uint8 i; i<_newIds.length; i++) {
      topTenIds.push(_newIds[i]);
      }
  }
  ...
  function removeMutantIds(uint256[] memory _ids) external onlyOwner {
      for(uint8 i; i< _ids.length; i++) {
          for(uint8 j; j<mutantIds.length; j++) {
              if(_ids[i] == mutantIds[j]) {
                  delete mutantIds[j];
              }
          }
  ...
  function removeTopTenIds(uint256[] memory _ids) external onlyOwner {
      for(uint8 i; i< _ids.length; i++) {
          for(uint8 j; j<topTenIds.length; j++) {
              if(_ids[i] == topTenIds[j]) {
                  delete topTenIds[j];
              }
          }
