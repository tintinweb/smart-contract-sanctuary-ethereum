...
contract MosaicSquare is 
    ...
    MSRoyalty
  {
    ...
    function initialize(address payable adminContract) public initializer {
    ...
    function mintToMarketWithUriAndRoyalty(
      ...
      external onlyAdmin 
    {
      require(get1stMarketAddress() != address(0), "MosaicSquare: Need to set the market address");
    ...
    function mintManualTokenIdToMarketWithUriAndRoyalty( 
      ...
      external onlyAdmin 
    {
      require(!_exists(tokenId), "MosaicSquare: Already used");
      require(get1stMarketAddress() != address(0), "MosaicSquare: Need to set the market address");
    ...
    function burn(uint256 tokenId) public override {
        require( isAdmin(_msgSender()) || _isApprovedOrOwner(_msgSender(), tokenId), 
          "MosaicSquare: caller is not admin nor approved");
        _burn(tokenId);
    }

    function transferTo2ndMarket(address from, uint256 tokenId) external {
      require(get2ndMarketAddress() != address(0), "MosaicSquare: Need to set the market address");
      require (msg.sender == get2ndMarketAddress(), "MosaicSquare: caller is not market" );
    ...
    ERC721Upgradeable, MSAdmins
{
    ...
    function _setTokenRoyaltyAddress(uint256 tokenId, address royaltyAddress, uint96 ratio) internal {
      require(royaltyAddress != address(0), "MSRoyalty: Invalid royalty address");
...
abstract contract MSAdmins is Initializable, ContextUpgradeable {
    ...
    event MosaicSquareAdminUpdated(address newAdminContract);

    function __MSAdmin_init(address payable admin_) internal onlyInitializing {
        require (admin_.isContract(),"MosaicSquareAdmin: MosaicSquareAdmin Address must be a Contract");
        _admin = admin_;
        _maker = _msgSender();
    }

    modifier onlyAdmin() {
        require(isAdmin(_msgSender()), "MosaicSquareAdmin: caller does not have the Admin role");
        _;
    }

    modifier adminOrMaker() {
        require(isAdmin(_msgSender())|| _maker == _msgSender(), "MosaicSquareAdmin: caller is not the admin");
        _;
    }
    ...
    function update1stMarketAddress(address payable market) external onlyAdmin {
      require (market.isContract(),  "MosaicSquareAdmin: MosaicSquare 1st Market address must be a Contract");
      _market1 = market;
      emit NFTMarketUpdated(market);
    }

    function update2ndMarketAddress(address payable market) external onlyAdmin {
      require (market.isContract(),  "MosaicSquareAdmin: MosaicSquare 2nd Market address must be a Contract");
      _market2 = market;
      emit NFTMarketUpdated(market);
    }

    function updateAdminAddress(address payable admin) external adminOrMaker {
      require (admin.isContract(),  "MosaicSquareAdmin: MosaicSquare Admin address must be a Contract");
      _admin = admin;
