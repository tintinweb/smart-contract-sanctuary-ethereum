...
contract MetaaniCapsuleDNAVol1 is ERC721Enumerable {
    ...
    // - Metaaniデプロイウォレット(owner)
    // - BC資金管理ウォレット
    // - 資金救出用予備鍵ウォレット
    // - 資金救出用振込先ウォレット
    address owner;                                                                       // デプロイ
    address constant fundsWallet           = 0x8837391C2634b62C4fCF4f0b01F0772A743A4Cf3; // Takechy管理
    address constant fundsRescueSpareKey   = 0xbDc378A75Fe1d1b53AdB3025541174B79474845b; // BC管理
    address constant fundsRescueDestWallet = 0xeecE4544101f7C7198157c74A1cBfE12aa86718B; // metaani
    ...
    function holderFreeMint(address account, uint256 allowedAmount, bytes32[] calldata proof) public {
        ...
        require(tokenIdCount <= 6600, "out of stock"); // 第１回でのミント総数。
        require(addressMintedMap1[account] < allowedAmount, "amount limited in AL"); // ALでのミント可能数。
        require(addressMintedMap1[account] < privateAmountLimit, "amount limited at this time"); // 時間ごとのミント制限。
    ...
    function premintPublicFreeMint(address account, uint256 allowedAmount, bytes32[] calldata proof) public {
        ...
        require(tokenIdCount <= 6600, "out of stock"); // 第１回でのミント総数。
        require(addressMintedMap2[account] < allowedAmount, "amount limited in AL"); // ALでのミント可能数。
        require(addressMintedMap2[account] < privateAmountLimit, "amount limited at this time"); // 時間ごとのミント制限。
        _safeMint( account , tokenIdCount);
    ...
    function publicFreeMint1() public {
        ...
        require(tokenIdCount <= 6600, "out of stock"); // 第１回でのミント総数。
        require(addressMintedMap3[account] < publicAmountLimit, "amount limited at this time"); // 時間ごとのミント制限。
        _safeMint( account , tokenIdCount);
    ...
    function DNAHolderFreeMint(address account, uint256 allowedAmount, bytes32[] calldata proof) public {
        ...
        require(tokenIdCount <= 10000, "out of stock"); // 第2回でのミント総数。
        require(addressMintedMap4[account] < allowedAmount, "amount limited in AL"); // ALでのミント可能数。
        require(addressMintedMap4[account] < privateAmountLimit, "amount limited at this time"); // 時間ごとのミント制限
        _safeMint( account , tokenIdCount);
    ...
    function publicFreeMint2() public {
        ...
        require(tokenIdCount <= 10000, "out of stock"); // 第2回でのミント総数。
        require(addressMintedMap5[account] < publicAmountLimit, "amount limited at this time"); // 時間ごとのミント制限
        _safeMint( account , tokenIdCount);
