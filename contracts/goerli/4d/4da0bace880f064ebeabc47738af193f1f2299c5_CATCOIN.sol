...
contract CATCOIN is Context, Ownable, IERC20, ERC20Detailed {
    ...
    uint256 public numTokensSellToAddToLiquidity = 81000000000 * 10**18;
    ...
    constructor () ERC20Detailed("Catcoin", "CATS", 0) {
   ...
   function setBuyFeePercent(uint256 updatedLiquidityFee) external onlyOwner {
        buyLiquidityFee = updatedLiquidityFee;
        buyTotalFee = buyLiquidityFee;
        require(buyTotalFee <= 2000, "Max. 20%");
        emit buyFeePercentUpdated(buyTotalFee);
    }

    function setSellFeePercent(uint256 updatedLiquidityFee) external onlyOwner {
        sellLiquidityFee = updatedLiquidityFee;
        sellTotalFee = sellLiquidityFee;
        require(sellTotalFee <= 2000, "Max. 20%");
