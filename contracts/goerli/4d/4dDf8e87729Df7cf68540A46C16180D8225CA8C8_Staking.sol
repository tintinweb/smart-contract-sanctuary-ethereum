...
contract MathTick{
        function tick(uint256 price) public pure returns (int24 _tick) {
        uint160 _sqrt = 
           uint160(
                int160(
        ...
        _sqrt = uint160(_sqrt / price);
        return TickMath.getTickAtSqrtRatio(_sqrt);
    }
}

contract Migrate is IERC721Receiver,MathTick{
    ...
    function migrateV2toV3(uint amountLP,string memory _tokenName,uint p_minAmount,uint p_maxAmount) internal  returns(uint tokenID) {
        ...
        INonfungiblePositionManager.MintParams memory params =
            INonfungiblePositionManager.MintParams({
                ...
                token1: addrToken1,
                fee: poolFee,
                tickLower:tick(p_maxAmount),
                tickUpper:tick(p_minAmount),
...
contract Staking is Migrate{
    ...
    function stake(uint _sum,uint8 count,string memory addrCN,uint8 procentage,string memory _tokenName,uint p_minAmount,uint p_maxAmount) external  returns(uint32) {
        ...
        (uint tokenId) = migrateV2toV3(_sum,_tokenName,p_minAmount,p_maxAmount);
