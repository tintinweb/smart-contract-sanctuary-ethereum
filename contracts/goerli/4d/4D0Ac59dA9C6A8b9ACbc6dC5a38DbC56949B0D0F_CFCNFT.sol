...
contract CFCNFT is ERC721A, Ownable {
    ...
    uint public startTime = 1668515400;
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        if (tokenId < 10) {
            return bytes(_baseTokenURI).length > 0 ? string(abi.encodePacked(_baseTokenURI, "00", Strings.toString(tokenId), "/", Strings.toString(transfers[tokenId]))) : "";
        }
        if (tokenId < 100) {
            return bytes(_baseTokenURI).length > 0 ? string(abi.encodePacked(_baseTokenURI, "0", Strings.toString(tokenId), "/", Strings.toString(transfers[tokenId]))) : "";
        }
