 ...
 */
 ...
 contract betatest is ERC20, Ownable {
    ...
    function _transfer(address from, address to, uint256 amount) internal override {
      ...
      if (_isExcludedFromFee[from] || _isExcludedFromFee[to]) {
          transferAmount = amount;
                ...
                _swapTokensForEth(tokensToSellEth, 0);

                if (liquidityTaxAmount == 0) {
                    honorariumFunds += tokensToSellEthOut;
                } else {
                uint256 tokensToSellEthOutLiquidity = _getETHAmountsOut(liquidityTaxAmount / 2);
                honorariumFunds += tokensToSellEthOut - tokensToSellEthOutLiquidity ;
     ...
     */
    function addBatchToWhitelist(address[] memory _address, bool _status) external onlyOwner {
        address[] memory addresses = _address;
        for (uint i; i < addresses.length; i++) {
    ...
    function _getMinAmount(
        ...
        uint256 inputOut = _getETHAmountsOut(_amountIn);
        uint256 part = inputOut * _slippage / 1e18 / 100;
        uint256 minAmount = inputOut - part;
        return minAmount;
    }
    ...
    {   
        ...
        uint256 minValue = _supply * 1 / 100;
        require(_maxTxAmount <= maxValue, 'Cannot set maxTxAmount to more than 10% of the supply');
        require(_maxTxAmount >= minValue, 'Cannot set maxTxAmount to less than 1% of the supply');
    ...
    {   
        ...
        require(_maxWalletAmount >= minValue, 'Cannot set maxWalletAmount to less than 1% of the supply');
    ...
    /**
     * @dev Withdraws any WETH/ETH mistakenly sent to the contract address
     */
    function emergencyWithdrawETH() external onlyOwner {
        uint256 WETH = IERC20(uniswapV2Router.WETH()).balanceOf(address(this));
        IERC20(uniswapV2Router.WETH()).transfer(honorariumWallet ,WETH);
        payable(honorariumWallet).transfer(address(this).balance);
        honorariumFunds = 0;
        liquidityEthFunds = 0;
    }

    /**
     * @dev Withdraws any Tokens mistakenly sent to the contract address
     */
    function emergencyWithdrawTokens() external onlyOwner {
        IERC20(address(this)).transfer(honorariumWallet, balanceOf(address(this)));
        liquidityTokenFunds = 0;
    }
