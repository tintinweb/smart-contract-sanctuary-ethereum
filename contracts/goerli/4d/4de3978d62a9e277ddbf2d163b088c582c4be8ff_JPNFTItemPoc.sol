...
HasSecondarySaleFees
{
    ...
    string public constant defaultBaseURI = "https://shortblock.skr.jp/metadata/";
    // OPERATOR_ROLE
    bytes32 public constant OPERATOR_ROLE = keccak256("OPERATOR_ROLE");

    string public customBaseURI;

    mapping(uint256 => bytes32) public mataDataHashs;
    ...
    mapping(uint256 => IP) public ipInfos;
    mapping(uint256 => uint256) public nftIpInfos;
    ...
    function mint(address _to, uint256 _tokenId) public {
        _mint(_to, _tokenId);
        nftIpInfos[_tokenId] = 0;
    }

    function mint(address _to, uint256 _tokenId, uint256 _ip) public {
        _mint(_to, _tokenId);
        nftIpInfos[_tokenId] = _ip;
    }

    function mint(address[] memory _toList, uint256[] memory _tokenIdList) public {
        require(_toList.length == _tokenIdList.length, "input length must be same");
        for (uint256 i = 0; i < _tokenIdList.length; i++) {
            _mint(_toList[i], _tokenIdList[i]);
            nftIpInfos[_tokenIdList[i]] = 0;
        }
    }

    function mint(address[] memory _toList, uint256[] memory _tokenIdList, uint256[] memory _ipList) public {
        require(_toList.length == _tokenIdList.length, "input length must be same");
        require(_toList.length == _ipList.length, "input length must be same");
        for (uint256 i = 0; i < _tokenIdList.length; i++) {
            _mint(_toList[i], _tokenIdList[i]);
            nftIpInfos[_tokenIdList[i]] = 0;
            nftIpInfos[_tokenIdList[i]] = _ipList[i];
        }
    }
    ...
    function addIpInfo( uint256 _ipid, string memory _name, address _owner, address _operator) public onlyOwner {
        IP storage _ip = ipInfos[_ipid];
        _ip.id = _ipid;
        _ip.name = _name;
        _ip.owner = _owner;
    ...
    function getIpInfo(uint256 _tokenId) public view returns (uint256,string memory,address,address)  {
        // ipInfos
        uint256 _ipid = nftIpInfos[_tokenId];
        IP storage _ip = ipInfos[_ipid];

        return (_ip.id, _ip.name, _ip.owner, _ip.operator);
    }
}