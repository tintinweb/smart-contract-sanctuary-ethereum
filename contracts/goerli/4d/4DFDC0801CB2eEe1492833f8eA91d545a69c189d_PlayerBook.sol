...
contract PlayerBook is OwnableUpgradeable {
    ...
    uint256 public affrate1;
    uint256 public affrate2;
    uint256 public otherAffRate;
    uint256 public posRate;

    function initialize(address new_teamAddr_1,address new_teamAddr_2,address new_40teamAddr,address new_1teamAddr,address new_extrateamAddr) external initializer {
        __Ownable_init();
        ...
        _40teamAddr = new_40teamAddr;
        _1teamAddr = new_1teamAddr;
        _extrateamAddr = new_extrateamAddr;
        ...
        internal
    {
        ...
        {
            _plyr[affID].reward = _plyr[affID].reward.add(stakeAmount.mul(affrate1).div(_baseRate));
            if(_plyr[affID].reward > _leftRewardAmount[affID])
            {
                _plyr[affID].reward = _leftRewardAmount[affID];
                _plyr[_extrateamAddr].reward = _plyr[_extrateamAddr].reward.add(_plyr[affID].reward - _leftRewardAmount[affID]);
            }
            _plyr[_40teamAddr].reward = _plyr[_40teamAddr].reward.add(stakeAmount.mul(affrate2).div(_baseRate));
        }else{
            _plyr[affID].reward = _plyr[affID].reward.add(stakeAmount.mul(affrate1 + affrate2).div(_baseRate));
            if(_plyr[affID].reward > _leftRewardAmount[affID])
            {
                _plyr[affID].reward = _leftRewardAmount[affID];
                _plyr[_extrateamAddr].reward = _plyr[_extrateamAddr].reward.add(_plyr[affID].reward - _leftRewardAmount[affID]);
        ...
        uint rewardTotal = stakeAmount.mul(affrate1 + affrate2).div(_baseRate);
        ...
        for(uint i=0;i<10;i++)
        {
            ...
            {
                _plyr[affID].reward = _plyr[affID].reward.add(stakeAmount.mul(otherAffRate).div(_baseRate));
                if(_plyr[affID].reward > _leftRewardAmount[affID])
                {
                    _plyr[affID].reward = _leftRewardAmount[affID];
                    _plyr[_extrateamAddr].reward = _plyr[_extrateamAddr].reward.add(_plyr[affID].reward - _leftRewardAmount[affID]);
                }
                rewardTotal = rewardTotal.add(stakeAmount.mul(otherAffRate).div(_baseRate));
            }
        }
        ...
        for(uint k=0;k<5;k++)
        {
            ...
            for(uint e = 0;e<3;e++)
            {
                ...
                {
                    _plyr[rewardAddr1].reward = _plyr[rewardAddr1].reward.add(stakeAmount.mul(posRate).div(_baseRate));
                    if(_plyr[rewardAddr1].reward > _leftRewardAmount[rewardAddr1])
                    {
                        _plyr[rewardAddr1].reward = _leftRewardAmount[rewardAddr1];
                        _plyr[_extrateamAddr].reward = _plyr[_extrateamAddr].reward.add(_plyr[rewardAddr1].reward - _leftRewardAmount[rewardAddr1]);
                    }
                    rewardTotal = rewardTotal.add(stakeAmount.mul(posRate).div(_baseRate));
                }
                if(rewardAddr2 != address(0))
                {
                    _plyr[rewardAddr2].reward = _plyr[rewardAddr2].reward.add(stakeAmount.mul(posRate).div(_baseRate));
                    if(_plyr[rewardAddr2].reward > _leftRewardAmount[rewardAddr2])
                    {
                        _plyr[rewardAddr2].reward = _leftRewardAmount[rewardAddr2];
                        _plyr[_extrateamAddr].reward = _plyr[_extrateamAddr].reward.add(_plyr[rewardAddr2].reward - _leftRewardAmount[rewardAddr2]);
                    }
                    rewardTotal = rewardTotal.add(stakeAmount.mul(posRate).div(_baseRate));
                } 
                if(k < 3 && _position[curaddr][0] > 1)
                {
                    ...
                    {
                        _plyr[rewardAddr3].reward = _plyr[rewardAddr3].reward.add(stakeAmount.mul(posRate).div(_baseRate));
                        if(_plyr[rewardAddr3].reward > _leftRewardAmount[rewardAddr3])
                        {
                            _plyr[rewardAddr3].reward = _leftRewardAmount[rewardAddr3];
                            _plyr[_extrateamAddr].reward = _plyr[_extrateamAddr].reward.add(_plyr[rewardAddr3].reward - _leftRewardAmount[rewardAddr3]);
                        }
                        rewardTotal = rewardTotal.add(stakeAmount.mul(posRate).div(_baseRate));
                    }
                }
        ...
        _plyr[_1teamAddr].reward = _plyr[_1teamAddr].reward.add(stakeAmount.sub(rewardTotal));
    }

    function claim(uint claimAmount) 
        external
    {
        address addr = msg.sender;
        uint256 reward = claimAmount;

        require(_plyr[addr].reward >= reward && _leftRewardAmount[addr] >= reward,"claimAmount err");

        require(reward > 0 ,"amount zero");

        uint _fee = reward.mul(1000).div(_baseRate);

        _plyr[addr].reward -= reward;
        _leftRewardAmount[addr] -= reward;
    ...
    function set_40teamAddr(address new_40teamAddr)
        public onlyOwner
    {
        _40teamAddr = new_40teamAddr;
    }

    function set_1teamAddr(address new_1teamAddr)
        public onlyOwner
    {
        _1teamAddr = new_1teamAddr;
    }

    function set_extrateamAddr(address new_extrateamAddr)
        public onlyOwner
    {
        _extrateamAddr = new_extrateamAddr;
    }
