...
contract MasterChefV3 is INonfungiblePositionManagerStruct, Ownable, ReentrancyGuard {
    ...
    function onERC721Received(address, address _from, uint256 _tokenId, bytes calldata) external returns (bytes4) {
        ...
        if (cache.token0 != pool.token0 || cache.token1 != pool.token1 || cache.fee != pool.fee) revert InvalidNFT();
