...
contract KudasaiMinter is Ownable {
    ...
    modifier onlyKudasaiHolder(uint256[] memory _ids) {
        for (uint256 i = 0; i < _ids.length; i++) {
            ...
            require(IERC721(address(_onchainKudasai)).ownerOf(_ids[i]) == tx.origin, "You do not have Kudasai NFTs");
            claimed[_ids[i]] = true;
        }
        _;
    }
    ...
    function kantsu(uint256 _quantity, bytes32 _code) external payable isNotContract {
        ...
        require(_hidden != address(0) && Hidden(_hidden).check(tx.origin, _quantity, _code), "Your address is Blacklisted!");
        require(_kudasaiCounter[tx.origin][proofRound] + _quantity <= _maxWalletPerToken && _kantsuCounter < 10, "No More Kudasai");

        _kudasaiCounter[tx.origin][proofRound] += _quantity;
        _kantsuCounter++;
        Kudasai(_kudasaiNFT).mintKudasai(tx.origin, _quantity);
    }
    ...
    function holderClaim(uint256[] memory _ids, bool _burn) public payable onlyKudasaiHolder(_ids) isNotContract {
        ...
        Kudasai(_kudasaiNFT).mintKudasai(tx.origin, _ids.length);
        if (_burn) {
            for (uint256 i = 0; i < _ids.length; i++) {
                IERC721(address(_onchainKudasai)).safeTransferFrom(tx.origin, address(0x000000000000000000000000000000000000dEaD), _ids[i]);
            }
        }
    }

    function holderClaimAndKudasai(uint256 _quantity, bytes32[] calldata _proof, uint256[] memory _ids, bool _burn) public payable {
        kudasai(_quantity, _proof);
        holderClaim(_ids, _burn);
    }

    function kudasai(uint256 _quantity, bytes32[] calldata _proof) public payable validateKudasaiAddress(_proof) isNotContract {
        require(msg.value == mintCost * _quantity, "Mint cost is insufficient");
        require(_kudasaiCounter[tx.origin][proofRound] + _quantity <= _maxWalletPerToken, "No More Kudasai");
