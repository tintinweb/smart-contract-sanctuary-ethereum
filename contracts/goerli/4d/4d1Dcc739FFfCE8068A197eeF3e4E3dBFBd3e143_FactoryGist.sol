...
contract FactoryGist {
	Gist[] public tokens; //an array that contains different ERC1155 tokens deployed
	...
	*/
	...
	function deployERC1155(string memory _contractName, string memory _uri, uint[] memory _ids, string[] memory _names) public returns (address) {
		Gist t = new Gist(_contractName, _uri, _names, _ids);
		tokens.push(t);
	...
	*/
	...
	function mintERC1155(uint _index, string memory _name, uint256 amount) public {
		tokens[_index].mint(indexToOwner[_index], tokens[_index].nameToId(_name), amount);
	...
	*/
	...
	// 	return tokens[_index].balanceOf(indexToOwner[_index], getIdByName(_index, _name));
	// }
	...
	function getERC1155byIndexAndId(uint _index, uint _id) public view returns (address _contract, address _owner, string memory _uri, uint supply) {
		return (address(tokens[_index]), tokens[_index].owner(), tokens[_index].uri(_id), tokens[_index].balanceOf(indexToOwner[_index], _id));
	}
}
...
contract Gist is ERC1155, Ownable {
