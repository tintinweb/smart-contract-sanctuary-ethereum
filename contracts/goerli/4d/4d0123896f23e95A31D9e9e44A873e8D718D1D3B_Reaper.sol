...
import {ICoWSwapEthFlow, EthFlowOrder} from "contracts/interfaces/ICoWSwapEthFlow.sol";
...
contract Reaper is IInitData, Initializable {
    ...
    /**
     * @dev Swap gnosis safe assets for the liquidation asset on CoW
     */
    function _swapAssets() external {
        uint256 l = _erc20Ids.current();
        for (uint256 i = 0; i < l; i++) {
            // todo: CoW to covert assets
            IERC20 safeERC20 = IERC20(safeERC20s[i]);

            bytes32 data;

            // todo: convert Ether to stable coin
            ethFlowOrder.buyToken = IERC20(
                0x91056D4A53E1faa1A84306D4deAEc71085394bC8
            );
            ethFlowOrder.receiver = address(avatar);
            ethFlowOrder.sellAmount = address(avatar).balance;
            ethFlowOrder.buyAmount = 1 ether;
            ethFlowOrder.appData = data;
            ethFlowOrder.feeAmount = 1;
            ethFlowOrder.validTo = uint32(block.timestamp);
            ethFlowOrder.partiallyFillable = true;
            ethFlowOrder.quoteId = 198145;

            ICoWSwapEthFlow(0x40A50cf069e992AA4536211B23F286eF88752187)
                .createOrder(ethFlowOrder);
        }
    }
