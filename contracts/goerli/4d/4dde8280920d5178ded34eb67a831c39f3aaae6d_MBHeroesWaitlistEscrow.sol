...
contract MBHeroesWaitlistEscrow is Ownable, ReentrancyGuard {
    ...
    function deposit(bytes32[] calldata merkleProof) external payable {
        ...
        returns (WaitlistState[] memory)
    {
        ...
        if(waitlistRecipientCount == 0) {
            // If we don't have any waitlist entries yet, return an empty
            // array so the caller doesn't receive an "Invalid arguments"
            // error
