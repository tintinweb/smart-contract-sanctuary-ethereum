...
// File: contracts/BK/IERC721A.sol
...
// File: contracts/BK/IERC721AQueryable.sol
...
// File: contracts/BK/ERC721AQueryable.sol
    ...
    ReentrancyGuard
{
    ...
    uint256 public constant RIDER = 0;
    uint256 public constant CELESTIAL = 1;
    uint256 public constant SRX = 2;
    uint256 public constant ANCIENT = 3;
    uint256 public dockingPeriod = 1;
    uint256 public dockingPeriodLength = 432000;
    ...
    uint256[4] public supplies = [2500, 390, 90, 20];
    ...
    mapping(address => bool) public walletDocked;
    mapping(address => mapping(uint256 => bool)) public walletClaimed;
    string public baseURI;
    string public altBaseURI;
    bool public isPaused = true;
    bool public canDock = false;

    SpaceRiders public spaceRiders =
        SpaceRiders(0x985ae3528A0313E65dF852195cCA13B90236fE24);
    StarToken public starToken =
        StarToken(0x64a7eAFC13799f95f8D1cd73429881E845294566);
    OgPass public ogPass = OgPass(0xBC854d30B7120D628C0B4C4D7Fda28Fc82Ff29c4);
    ...
    function mint(uint256 _id, bytes32[] calldata proof) public nonReentrant {
        if (_id > supplies.length || _id < 0) revert InvalidTokenType();
        if (isPaused && msg.sender != owner()) revert Paused();
        if (
            !proof.verify(
                merkleRoots[_id],
        ...
        if (minted[_id] + 1 > supplies[_id]) revert SupplyExceeded();
        if (uint64(_getAux(msg.sender)) > 0) revert MaxMintAmountExceed();
        uint256 starCost = (
    ...
    function dockBoard(uint256 tokenId) external {
        if (!canDock) revert BoardDockingClosed();
        if (dockingStarted[tokenId] != 0) revert BoardDocked();
        if (walletDocked[msg.sender]) revert WalletDocked();
        if (rewardClaimed[tokenId][dockingPeriod]) revert TokenClaimed();
        if (walletClaimed[msg.sender][dockingPeriod]) revert WalletClaimed();
        if (spaceRiders.balanceOf(msg.sender) < 1) revert NoSpaceRider();

        toggleBoardDocking(tokenId);
    }
        ...
        returns (string memory)
    {
        ...
        if (dockingStarted[_tokenId] > 0) {
            uint256 day = (block.timestamp - dockingStarted[_tokenId]) /
                86400 +
                1;

            return
                bytes(altBaseURI).length > 0
                    ? string(
                        abi.encodePacked(
                            altBaseURI,
                            _toString(_tokenId),
                            "/",
                            _toString(dockingPeriod),
                            "/",
                            _toString(day)
                        )
                    )
    ...
    function tokenType(uint256 _id) public view returns (uint256) {
        return _tokenTypes[_id];
    }

    function mintedTotalOfTokenType(uint256 _id) public view returns (uint256) {
        return minted[_id];
    }
    ...
    function starCostOfTokenType(uint256 _id) public view returns (uint256) {
        return mintRates[_id];
    }

    function dockingPeriodTime(uint256 tokenId)
        public
        view
    ...
    function rewardClaimedForTokenAndPeriod(uint256 _id, uint256 _period)
        public
        view
        returns (bool)
    {
        return rewardClaimed[_id][_period];
    }

    function rewardClaimedForAddressAndPeriod(address _address, uint256 _period)
        public
        view
        returns (bool)
    {
        return walletClaimed[_address][_period];
    }
    ...
    function setAltBaseURI(string memory _altBaseURI) public onlyOwner {
        altBaseURI = _altBaseURI;
    }
    ...
    function setMintRates(uint256[4] calldata _mintRates) public onlyOwner {
        mintRates = _mintRates;
    }

    function setSupplies(uint256[4] calldata _supplies) public onlyOwner {
    ...
    function mintMultipleToAddress(
        ...
        if (_id > supplies.length || _id < 0) revert InvalidTokenType();
        if (_amount > (supplies[_id] - minted[_id])) revert SupplyExceeded();

        uint256 mintIndex = _nextTokenId();
        _safeMint(_address, _amount);
        for (uint256 i = mintIndex; i < (mintIndex + _amount); i++) {
            _tokenTypes[i] = _id;
        }
        minted[_id] += _amount;
    }

    function mintRemainingSupplyToAddress(address _address) public onlyOwner {
        for (uint256 i = 0; i < supplies.length; i++) {
            if ((supplies[i] - minted[i]) > 0) {
                uint256 mintIndex = _nextTokenId();
                uint256 mintAmount = (supplies[i] - minted[i]);
                _safeMint(_address, mintAmount);
                for (
                    uint256 ii = mintIndex;
                    ii < (mintIndex + mintAmount);
                    ii++
                ) {
                    _tokenTypes[ii] = i;
                }
    ...
    function setDockingPeriodDetails(
        uint256 _dockingPeriod,
        uint256 _dockingPeriodLength
    ) public onlyOwner {
        dockingPeriod = _dockingPeriod;
        dockingPeriodLength = _dockingPeriodLength;
    }

    function toggleCanDock() external onlyOwner {
        canDock = !canDock;
    }
    ...
    */
    function terminateFromDock(uint256[] calldata tokenIds) external onlyOwner {
        for (uint256 i = 0; i < tokenIds.length; ++i) {
            ...
            uint256 start = dockingStarted[tokenId];
            if (start == 0) revert BoardUndocked();

            address owner = _ownershipOf(tokenId).addr;
            if (
                dockingPeriodTime(tokenId) > dockingPeriodLength &&
                !rewardClaimed[tokenId][dockingPeriod] &&
                !walletClaimed[msg.sender][dockingPeriod]
            ) {
                dockingStarted[tokenId] = 0;
                walletDocked[owner] = false;
                walletClaimed[owner][dockingPeriod] = true;
                rewardClaimed[tokenId][dockingPeriod] = true;
                //do reward
                spaceRidersRewards.getReward(msg.sender, dockingPeriod);
            } else {
                dockingStarted[tokenId] = 0;
                walletDocked[owner] = false;
            }
    ...
    function toggleBoardDocking(uint256 tokenId) internal {
        if (_ownershipOf(tokenId).addr != msg.sender) revert OnlyBoardOwner();
        uint256 start = dockingStarted[tokenId];
        if (start == 0) {
            dockingStarted[tokenId] = block.timestamp;
            walletDocked[msg.sender] = true;
            emit Docked(tokenId);
        } else {
            if (dockingPeriodTime(tokenId) > dockingPeriodLength) {
                dockingStarted[tokenId] = 0;
                walletDocked[msg.sender] = false;
                walletClaimed[msg.sender][dockingPeriod] = true;
                rewardClaimed[tokenId][dockingPeriod] = true;
                spaceRidersRewards.getReward(msg.sender, dockingPeriod);
            } else {
                dockingStarted[tokenId] = 0;
