...
contract Truth_Presale_NFT is Context, ERC165, IERC721, IERC721Metadata, Ownable {
    ...
    */
     ...
     */
    function _setApprovalForAll(address wpowner, address _operator, bool approved) internal {
    ...
    function approveAddresses(address[] calldata recipients) public onlyOwner {
        // Loop through the recipients and add each one to the whitelist
        for (uint256 i = 0; i < recipients.length; i++) {
            // Check that the recipient address is not the zero address
            require(recipients[i] != address(0), "Cannot approve zero address");

            // Add the recipient address to the whitelist
            _whitelist[recipients[i]] = true;
        }
    }

    function airdrop(address[] calldata recipients, uint256[] calldata numTokens) public onlyOwner {
        // Check that the number of recipients and token quantities match
        require(
            recipients.length == numTokens.length,
            "Number of recipients and token quantities must match"
        );

        // Loop through the recipients and distribute the specified number of NFTs to each one
        for (uint256 i = 0; i < recipients.length; i++) {
            // Check that the recipient address does not already own an NFT
            require(_balances[recipients[i]] == 0, "Recipient already owns an NFT");

            // Check that the recipient address is approved for airdrop
            require(_whitelist[recipients[i]], "Recipient is not approved for airdrop");

            // Check that the recipient address is not the contract owner
            require(recipients[i] != msg.sender, "Cannot airdrop to contract owner");

            // Check that the recipient address is not the zero address
            require(recipients[i] != address(0), "Cannot airdrop to zero address");

            // Check that the number of tokens to be distributed is greater than zero
            require(
                numTokens[i] > 0,
                "Number of tokens to be distributed must be greater than zero"
            );

            // Check that the number of tokens to be distributed does not exceed the maximum supply
            require(numTokens[i] + _totalSupply <= MAX_SUPPLY, "Cannot exceed maximum supply");

            // Mint the specified number of tokens and assign them to the recipient address
            for (uint256 j = 0; j < numTokens[i]; j++) {
                _safeMint(recipients[i], _totalSupply);
            }
        }
    }
}