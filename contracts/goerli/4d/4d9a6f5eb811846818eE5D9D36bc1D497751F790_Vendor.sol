...
contract Vendor is Ownable{
  ...
  event SellTokens(address sender,uint256 amount);
...
function sellTokens(uint256 _amount) public returns(uint256){
  uint256 allowanceAmount = yourToken.allowance(msg.sender, (address(this)));
  require(allowanceAmount >= _amount, "You should allow use certain amount first!");
  
  uint256 userTokenBalance = yourToken.balanceOf(msg.sender);
  require(userTokenBalance >= _amount, "You don't have enough tokens!");
  
  uint256 amountToTransfer = _amount / tokensPerEth;
