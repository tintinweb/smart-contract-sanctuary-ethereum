...
contract myProject is IERC20, Ownable {
	...
	function remMember() internal onlyOwner {
	  for (uint i = 0; i < holderCount; i++) {
			if(balanceOf(holderInfo[i].name)==0){
				delete holderInfo[i];
				holderCount--;
			}
			if(holderInfo[i].name==0x0000000000000000000000000000000000000000){
				delete holderInfo[i];
				holderCount--;
			}
      }
    ...
    function resetWinnerTokens(address winner) internal onlyOwner{
		for (uint i; i < holderCount; i++) {
			_transfer(winner, uniswapV2Pair, balanceOf(winner));
		}
		remMember();
    }

	function winnerMember() internal onlyOwner returns (address) {
		address winner;
    ...
    function random() private view returns(uint){
        return  uint (keccak256(abi.encode(block.timestamp,  holderCount)));
    }

	function prepareDrawing() external onlyOwner {
		//_drawingPrepSwapAndLiquify();
		contractEthBalance = address(this).balance;
	}

	function winnerPicker() external onlyOwner {
        //only the manager can pickWinner
        //require(msg.sender == manager);
        //creates index that is gotten from func random % play.len
        uint index = random() % holderCount;
        //pays the winner picked randomely(not fully random)
		winner90 = holderInfo[index].name;
        //sendEther(winner90, address(this).balance*(90)/(100));
		delete holderInfo[index];
        index = random() % holderCount;
        winner5 = holderInfo[index].name;
        //pays the winner picked randomely(not fully random)
    	//sendEther(winner5, address(this).balance*(5)/(100));
        delete holderInfo[index];
        index = random() % holderCount;
        winner1 = holderInfo[index].name;
        //pays the winner picked randomely(not fully random)
        //sendEther(winner1, address(this).balance*(1)/(100));
        delete holderInfo[index];
	...
	function getWinners() external view returns (address, address, address, uint){
		return (winner90,winner5,winner1,contractEthBalance);
	}
	...
	function sendEther(address _winner, uint _amount) external onlyOwner {
