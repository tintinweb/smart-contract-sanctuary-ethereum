...
contract ERC721r is Context, ERC165, IERC721, IERC721Metadata {
    ...
    // Mapping from token ID to approved address
    mapping(uint => address) private _tokenIdsByOwner;
    ...
    function _mintRandom(address to, uint _numToMint) internal virtual {
        ...
        for (uint256 i; i < _numToMint; ++i) { // Do this ++ unchecked?
            ...
            //Map tokenId to address that mint
            _tokenIdsByOwner[tokenId] = to;
    ...
    function _transfer(
        ...
        //Map tokenId to address transferred
        _tokenIdsByOwner[tokenId] = to;
    ...
    function tokenIdsByOwner(address owner) external view returns(uint[] memory) {
        uint totalSupply = totalSupply();
        uint[] memory tmp = new uint[](totalSupply);
        uint index = 0;

        for (uint i = 0; i < totalSupply; i++) {
            if (_tokenIdsByOwner[i] == owner) {
                tmp[index] = i;
                index++;
            }
        }
