...
contract ERC721 is Context, ERC165, IERC721, IERC721Metadata {
    ...
    uint256 public cards;     //普通股东卡数量
    uint256 public cardsVip;  //创世股东卡数量
    /**
    ** 初始化代币名称       name_
    ** 初始化代币简称       symbol_
    ** 初始化URL前缀地址    baseURI_
    ...
    */
    function toMint(address to_,uint256 type_) public WhiteList returns (bool){
        if(type_ == 1){
            uint256 tokenId = cards + 10000000;
            cards += 1;
            require(tokenId < 10000050, "Quantity Online");
            _safeMint(to_,tokenId,'');
            _setTokenURI(tokenId, "pt");
        }else if(type_ == 2){
            uint256 tokenId = cardsVip + 20000000;
            cardsVip += 1;
            require(tokenId < 20001000, "Quantity Online");
            _safeMint(to_,tokenId,'');
            _setTokenURI(tokenId, "vip");
        }else{
            require(false, "this cards non-existent");
        }
        return true;
    }
    /**
