...
interface IStickyPool {
    ...
    function setNFTAdmin(address) external;
...
contract StickyPoolRecipe {
    ...
    event StickyPoolDeployed(address pool, address token);
    ...
    */
    ...
    function stickyPoolTemplate() external view returns (address) {
    ...
    */
        ...
        returns (address)
    {
        ...
        address sticky_pool = jellyFactory.deployContract{value: msg.value}(
            POOL_ID,
            payable(jellyVault), 
            "");
        ...
        address pool_descriptor = jellyFactory.deployContract(
            ...
            payable(jellyVault), 
            "");
        IJellyContract(pool_descriptor).initContract(abi.encode(sticky_pool, access_controls)); 

        // Contract config
        IStickyPool(sticky_pool).setDescriptor(pool_descriptor);
        IStickyPool(sticky_pool).setNFTAdmin(_poolAdmin); 
