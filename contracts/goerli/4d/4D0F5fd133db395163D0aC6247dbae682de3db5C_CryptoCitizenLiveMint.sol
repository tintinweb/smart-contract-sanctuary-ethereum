...
contract CryptoCitizenLiveMint is Ownable, Pausable, ReentrancyGuard, CTZNPlugin
{   
    /*-------------------*/
    /*  STATE VARIABLES  */
    /*-------------------*/
    ...
    mapping(uint => mapping(uint => mapping(uint => uint))) public BookingsAvailableArtists;                         // Returns Amount Of Bookings Available For Input TimeSlot
    ...
    { 
        ...
        // ArtistMintPasses[_CurrentCityIndex][7] = 0x40ee4A63f6773D06F70629962AfCF52Af9AB38ed; //mainnet
        ArtistMintPasses[_CurrentCityIndex][7] = 0xd3daeCf84AFA8046A76A9FcC3f319e752A598E6a; //testing goerli
        ...
        // ArtistContracts[_CurrentCityIndex][7] = ;
        // ArtistContracts[_CurrentCityIndex][8] = ;
    ...
    function _LiveMintArtist(uint ArtistID, uint TicketID) public onlyMinter whenNotPaused 
    {
        ...
        if(DelegateStatusArtist[_CurrentCityIndex][ArtistID][TicketID]) { ICTZN(CTZN).IncrementRewards(Recipient, _RewardRates[0]); }
        else { ICTZN(CTZN).IncrementRewards(Recipient, _RewardRates[1]); }
    }
     ...
     * note: Slot is denoted in YYYY-MM-DD with no -'s (20220102) for Jan 2nd
     */
    function __ChangeTimeSlotArtist(uint ArtistID, uint TimeSlot, uint Amount) external onlyOwner
    {
        uint OldAmount = BookingsAvailableArtists[_CurrentCityIndex][ArtistID][TimeSlot];
        BookingsAvailableArtists[_CurrentCityIndex][ArtistID][TimeSlot] = Amount;
        emit TimeSlotChanged(OldAmount, Amount);
    }
...
{
    address public immutable CTZN = 0x7781762c5E9c6875B29D4137f69111D2115B2662;
