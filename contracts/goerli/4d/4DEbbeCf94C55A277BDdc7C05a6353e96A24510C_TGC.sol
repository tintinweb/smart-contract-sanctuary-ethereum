...
contract liquidityProvider is ILiquidityProvider {
	address public TGCToken;
	address public USDC = address(0x07865c6E87B9F70255377e024ace6630C1Eaa37F);
...
contract TGC is Context, IERC20, Ownable {
	...
	address public burnWallet = address(0x000000000000000000000000000000000000dEaD);
	address public USDC = address(0x07865c6E87B9F70255377e024ace6630C1Eaa37F);
	IStaking public stakingContract = IStaking(0x308E61958Cb246a9ff4D898D2673D8b7cbAEd177);
    ...
    string private _name = "The Grays Currency";
	...
	function setStakingContract(IStaking _stakingContract) external onlyOwner{
	   require(address(_stakingContract) != address(0), "Zero address");
	...
	function unlockToken(address user) public {
	   ...
	   lockedAmount[user] = 0;
	   emit UnLockToken(amount, user, msg.sender);
    }
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if (canSwap && !swapping && _automatedMarketMakerPairs[to] && swapAndLiquifyEnabled) 
		{
			...
			if(tokenToSwap >= swapTokensAtAmount) 
			{
				...
				_tokenTransfer(address(this), address(LPProviderAddress), tokenToLiqudity, false, false);
				
				uint256 USDCFinal = IERC20(USDC).balanceOf(address(LPProviderAddress)) - USDCInitial;
				uint256 liqudityPart = USDCFinal.mul(tokenToLiqudity).div(swapTokensAtAmount);
				uint256 marketingPart = USDCFinal - liqudityPart;
				
				if(liqudityPart > 0)
				{
				     uint256 liqudityToken = swapTokensAtAmount.mul(tokenToLiqudity).div(tokenToSwap);
				     LPProvider.provideLiquidity(USDCFinal, tokenToLiqudity);
				     liquidityFeeTotal = liquidityFeeTotal.sub(swapTokensAtAmount.mul(tokenToLiqudity).div(tokenToSwap)).sub(liqudityToken);
				}
				if(marketingPart > 0) 
				{
