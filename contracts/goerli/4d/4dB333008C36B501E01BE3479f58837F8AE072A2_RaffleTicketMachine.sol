...
contract RaffleTicketMachine is ERC721Enumerable, Ownable {
    ...
    function getRaffleQuantity(string memory _raffleName)
        public
        view
    ...
    function getRaffleMaxQuantity(string memory _raffleName)
        public
        view
    ...
    function getRaffleTicketPrice(string memory _raffleName)
        public
        view
        returns (uint256 eth, uint256 randoms)
    {
    ...
    function getRaffleSplitPriceEth(string memory _raffleName)
        public
        view
        returns (uint256 eth)
    {
    ...
    function getRaffleSplitPriceRandoms(string memory _raffleName)
        public
        view
        returns (uint256 randoms)
    {
        ...
        isQuantityPerMintValid(_raffleName, _quantity)
    {
        for (uint256 i = 0; i < raffles.length; i++) {
            if (
                ...
                require(
                    msg.value >= raffles[i].ticketPriceEth * _quantity * 1e18,
        ...
        isQuantityPerMintValid(_raffleName, _quantity)
    {
        for (uint256 i = 0; i < raffles.length; i++) {
            if (
                ...
                require(
                    msg.value >= raffles[i].splitPrice.eth * _quantity * 1e18,
                    "Not enough ETH sent to mint"
                );
                require(
                    IERC20(token).balanceOf(msg.sender) >=
        ...
        returns (string memory)
    {
        ...
        return
            string(
                abi.encodePacked(baseURI, raffleName, _tokenId, baseExtension)
            );
    }
