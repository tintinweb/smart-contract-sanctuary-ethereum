...
interface IERC4908 {
    ...
    function setUser(address collection, uint256 tokenId, address user, uint64 expires) external;

    /// @notice Get the user address of interfaceIdan NFT
    /// @dev The zero address indicates that there is no user or the user is expired
    /// @param tokenId The NFT to get the user address for
    /// @return The user address for this NFT
    function userOf(address collection, uint256 tokenId) external view returns (address);

    /// @notice Get the user expires of an NFT
    /// @dev The zero value indicates that there is no user
    /// @param tokenId The NFT to get the user expires for
    /// @return The user expires for this NFT
    function userExpires(address collection, uint256 tokenId) external view returns (uint256);
}
...
import "../interfaces/IERC4908.sol";
import "../storage/AppStorage.sol";

contract ERC4908 is IERC4908 {
    AppStorage s;
    ...
    function setUser(
        ...
        // );
        UserInfo storage info = s.cs._collection[collection]._users[tokenId];
        info.user = user;
        info.expires = expires;
        emit UpdateUser(collection, tokenId, user, expires);
    }
    ...
    /// @param tokenId The NFT to get the user address for
    /// @return The user address for this NFT
    function userOf(address collection, uint256 tokenId) public view virtual returns (address) {
        if (uint256(s.cs._collection[collection]._users[tokenId].expires) >= block.timestamp) {
            return s.cs._collection[collection]._users[tokenId].user;
        } else {
            return address(0);
        }
    }
    ...
    function userExpires(
        ...
        return s.cs._collection[collection]._users[tokenId].expires;
    }
    ...
    function _safeTransferFrom(
        ...
        if (from != to && s.cs._collection[collection]._users[tokenId].user != address(0)) {
            delete s.cs._collection[collection]._users[tokenId];
            emit UpdateUser(collection, tokenId, address(0), 0);
        }
    }
}
...
import "./libraries/ERC4908.sol";
import "./storage/VaultStorage.sol";
import "@openzeppelin/contracts/token/ERC20/IERC20.sol";

contract RentalMarketVaultFacet is ERC4908 {
