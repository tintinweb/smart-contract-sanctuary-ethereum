...
contract SynthetixDebtShare is Owned, MixinResolver, ISynthetixDebtShare {
    ...
    function mintShare(address account, uint256 amount) external onlyIssuer {
        ...
        if (msg.sender != address(debtShareAggregator())) {
    ...
    function exchangeShare(
        ...
        require(from != address(0) && to != address(0), "ERC20: mint to the zero address");
        ...
        debtShareAggregator().sendExchangeShare(from, to, amount, destChainId);

        emit ExchangeShare(from, to, amount, destChainId);
    }
    ...
    function transferFrom(
        ...
        if (msg.sender != address(debtShareAggregator())) {
            debtShareAggregator().sendTransferFrom(from, to, amount);
        }
