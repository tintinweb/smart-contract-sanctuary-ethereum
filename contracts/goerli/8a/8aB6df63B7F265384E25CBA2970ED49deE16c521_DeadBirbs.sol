 ...
 * @dev Extends ERC721A
 */
contract DeadBirbs is ERC721A, Seller, AttributeStore, ReentrancyGuard {
    ...
    constructor(
        ...
        // allowlist sale 
        setAllowlistSale(1674406997, 1209600, 20000000000, 1, 0xe12e7B6DDA6bC392b8441240459f7960E016D7Cb, false);
        
        // public sale
        setPublicSale(1674406997, 1209600, 20000000000, 1);

        setAttributeSigner(0xF7bFE4DfDE8891a8878D440Ee8139Aeb3cEFdcB0);
    }

    // mint functions

    /**
    @notice mints with verifying the provided mintKey
    *       you can get your mintKey at our site
    */
    function allowlistMint(string calldata attributeHash, bytes calldata attributeSignature, bytes calldata mintKey) external payable nonReentrant {
        verifyAllowlist(1, mintKey);
        verifyAttributeHash(attributeHash, attributeSignature);
        sotreAttributeHash(attributeHash, msg.sender, totalSupply());
    ...
    */
    function publicMint(string calldata attributeHash, bytes calldata attributeSignature) external payable nonReentrant {
        verifyPublicSale(1);
        verifyAttributeHash(attributeHash, attributeSignature);
        sotreAttributeHash(attributeHash, msg.sender, totalSupply());
        _safeMint(msg.sender, 1);
    ...
    */
    function ownerMint(string calldata attributeHash, bytes calldata attributeSignature, address recipient) external onlyOwner nonReentrant {
        verifyAttributeHash(attributeHash, attributeSignature);
        sotreAttributeHash(attributeHash, msg.sender, totalSupply());
...
contract Seller is Ownable {
    ...
    function verifySaleDate(SaleConfig memory sale) internal virtual view {
        ...
        require(sale.startDate != 0 && block.timestamp >= sale.startDate, string.concat("sale has not been started yet, ",sale.saleName));
        require(sale.duration == 0 || block.timestamp < sale.startDate + sale.duration, string.concat("sale has been ended, ",sale.saleName));
    }

    /**
     @dev returns the active sale
     */
    function activeSale() public view returns (string memory) {
        if(allowlistConfig.startDate <= block.timestamp && allowlistConfig.startDate + allowlistConfig.duration > block.timestamp){
            return string.concat("Allowlist mint is live");
        }
        if(waitlistConfig.startDate <= block.timestamp && waitlistConfig.startDate + waitlistConfig.duration > block.timestamp){
            return string.concat("Waitlist mint is live");
        }
        if(publicConfig.startDate <= block.timestamp && publicConfig.startDate + publicConfig.duration > block.timestamp){
            return string.concat("Public mint is live");
        }
    ...
    function verifyMintKey(bytes memory mintKey, SaleConfig memory sale) internal virtual {
        require(sale.isPublicSale || 
                ...
                string.concat("address with this key is not eligible to mint or saleKey has been already used, ",sale.saleName));
...
contract AttributeStore is Ownable {
    ...
    function verifyAttributeHash(string calldata attributeHash, bytes memory attributeSignature) internal virtual view {
        require(checkSignature(attributeSigner, attributeHash, attributeSignature), "invalid attribute hash siganture");
        require(firstMinters[attributeHash] != address(0), "hash has been minted already");
    }

    function checkSignature(address signer, string calldata attributeHash, bytes memory signature) private pure returns (bool) {
        return signer == 
            keccak256(
                abi.encodePacked(
                    "\x19Ethereum Signed Message:\n32",
    ...
    function sotreAttributeHash(string calldata attributeHash, address minter, uint256 tokenId) internal {
        firstMinters[attributeHash] = minter;
        tokenData[tokenId] = attributeHash;
    }
}
