...
contract LicenseToken is ERC1155Upgradeable, NFTCreator, ILicenseToken, IBatchAction {
    ...
    function _afterTokenTransfer(
        ...
        for (uint256 i = 0; i < ids.length; i++) {
            ...
            if (_licenseMetas[ids[i]].expiredAt < block.timestamp && to != address(0))
                revert expiredError({id: ids[i]});

            if (to == address(0) && op != operator())
                require(_licenseMetas[ids[i]].expiredAt > block.timestamp, "should expired");

            if (to == address(0) || from == address(0)) require(op == operator(), "should operator");
        }
    }
