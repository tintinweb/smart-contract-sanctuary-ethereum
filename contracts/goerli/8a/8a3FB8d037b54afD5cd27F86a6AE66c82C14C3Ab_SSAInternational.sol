...
contract SSAInternational is ERC721A, ERC2981, Ownable {
    ...
    external
  {
    ...
    if (!debug) { // todo: remove
      addressHasMinted[msg.sender] = true; // todo: bring back in, don't delete just move out of the block
    }
  ...
  // todo: test this
  function emergencyWithdrawERC721(address _contractAddress, address _address, uint _tokenId)
    external
    onlyOwner
  {
    SevenTwentyOne _interface = SevenTwentyOne(_contractAddress);
    _interface.safeTransferFrom(address(this), _address, _tokenId, "");
  }

  // todo: test this
  function emergencyWithdrawERC1155(address _contractAddress, address _address, uint _tokenId, uint _count)
    external
    onlyOwner
  {
    ElevenFiftyFive _interface = ElevenFiftyFive(_contractAddress);
    _interface.safeTransferFrom(address(this), _address, _tokenId, _count, "");
  }
