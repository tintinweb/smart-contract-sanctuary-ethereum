...
contract MasterContractsRegistry is IMasterContractsRegistry, RoleManagedRegistry {
    ...
    modifier onlyCreatePermission() override {
        require(
            IMasterAccessManagement(getMasterPermissionManager())
    ...
    function getMasterPermissionManager() public view override returns (address) {
...
contract MasterDAOFactory is IMasterDAOFactory, AbstractPoolFactory {
    ...
    string public constant DAO_FACTORY_RESOURCE = "DAO_FACTORY_RESOURCE";

    string public constant MASTER_DAO_REGISTRY_DEP = "MASTER_DAO_REGISTRY";

    MasterAccessManagement internal _masterAccess;

    MasterDAORegistry internal _masterDAORegistry;
    ...
    function setDependencies(address registryAddress_, bytes calldata data_) public override {
        ...
        _masterAccess = MasterAccessManagement(registry_.getMasterPermissionManager());
        _masterDAORegistry = MasterDAORegistry(registry_.getContract(MASTER_DAO_REGISTRY_DEP));
    }

    function deployDAOPanel(
        DAORegistry dao_,
        IDAOVoting.ConstructorParams memory votingParams_
    ) public onlyExecutePermission {
        (
            DAOParameterStorage daoParameterStorage_,
            DAOMemberStorage daoMemberStorage_
        ) = _deployStorages(dao_, votingParams_.panelName);

        IVotingFactory votingFactory_ = IVotingFactory(dao_.getVotingFactory());

        daoParameterStorage_.setDAOParameters(
            [
                votingParams_.votingValues.votingPeriod.encodeUint(
                    votingParams_.votingKeys.votingPeriodKey
                ),
                votingParams_.votingValues.vetoPeriod.encodeUint(
                    votingParams_.votingKeys.vetoPeriodKey
                ),
                votingParams_.votingValues.proposalExecutionPeriod.encodeUint(
                    votingParams_.votingKeys.proposalExecutionPKey
                ),
                votingParams_.votingValues.requiredQuorum.encodeUint(
                    votingParams_.votingKeys.requiredQuorumKey
                )
            ].asArray()
        );

        daoMemberStorage_.addMembers(votingParams_.initialMembers);

        address voting_ = votingFactory_.deployDAOVoting(votingParams_);
        ...
        IRBAC.ResourceWithPermissions[]
            memory resWithPerms_ = _getResourcesWithPermissionForPanels(
                dao_,
                votingContractPermissions_,
                votingParams_.panelName
            );

        // TODO: check if votingParams_.votingName already exists

        manager_.addCombinedPermissionsToRole(
            votingParams_.votingName,
            votingParams_.votingDescription,
            resWithPerms_,
            new IRBAC.ResourceWithPermissions[](0)
        );
        ...
        resWithPerms_ = _getResourcesWithPermissionForPanels(
            dao_,
            memberPermissions_,
            votingParams_.panelName
        );
        ...
        manager_.addCombinedPermissionsToRole(
            string.concat("MemberOf", votingParams_.votingName),
            string.concat("Create role for members of ", votingParams_.votingName),
            resWithPerms_,
            new IRBAC.ResourceWithPermissions[](0)
        );

        manager_.grantRoles(voting_, votingParams_.votingName.asArray());
    }

    function deployDAO(
        IDAORegistry.ConstructorParams calldata params_
    ) external onlyExecutePermission {
        string memory moduleType_ = _masterDAORegistry.DAO_REGISTRY_NAME();

        address daoRegistryProxy_ = _deploy(address(_masterDAORegistry), moduleType_);

        string memory daoRegistryResource_ = _getDAOResource(moduleType_, daoRegistryProxy_);

        address permissionManager_ = _masterDAORegistry.getProxyBeacon(
            _masterDAORegistry.DAO_PERMISSION_MANAGER_NAME()
        );

        DAORegistry dao_ = DAORegistry(daoRegistryProxy_);

        dao_.__DAORegistry_init(
            ProxyBeacon(permissionManager_).implementation(),
            address(this),
            daoRegistryResource_
        );

        _register(address(_masterDAORegistry), moduleType_, daoRegistryProxy_);

        _deployTokenFactoryAndRegistry(dao_, params_.tokenNames, params_.tokenAddresses);
        _deployVotingFactoryAndRegistry(dao_, params_.votingNames, params_.votingAddresses);

        _deployNativeTokenAndVoting(dao_, params_.tokenParams, params_.votingParams);

        emit DeployedDAORegistry(daoRegistryProxy_, params_);
    }

    function _deployTokenFactoryAndRegistry(
        DAORegistry dao_,
        string[] calldata tokenNames_,
        address[] calldata tokenAddresses_
    ) private {
        string memory factoryType_ = _masterDAORegistry.DAO_TOKEN_FACTORY_NAME();
        string memory registryType_ = _masterDAORegistry.DAO_TOKEN_REGISTRY_NAME();

        address tokenFactoryProxy_ = _deploy(address(_masterDAORegistry), factoryType_);
        address tokenRegistryProxy_ = _deploy(address(_masterDAORegistry), registryType_);
        ...
        dao_.addContract(dao_.TOKEN_FACTORY_NAME(), tokenFactoryProxy_);
        dao_.addContract(dao_.TOKEN_REGISTRY_NAME(), tokenRegistryProxy_);

        _register(address(_masterDAORegistry), factoryType_, tokenFactoryProxy_);
        _register(address(_masterDAORegistry), registryType_, tokenRegistryProxy_);

        _injectDAODependencies(address(dao_), address(_masterDAORegistry), tokenFactoryProxy_);
        _injectDAODependencies(address(dao_), address(_masterDAORegistry), tokenRegistryProxy_);

        TokenRegistry tokenRegistry_ = TokenRegistry(tokenRegistryProxy_);
        tokenRegistry_.setNewImplementations(tokenNames_, tokenAddresses_);

        emit DeployedTokenFactory(tokenFactoryProxy_, dao_);
        emit DeployedTokenRegistry(tokenRegistryProxy_, dao_);
    }
    ...
    function _deployVotingFactoryAndRegistry(
        ...
        string memory factoryType_ = _masterDAORegistry.DAO_VOTING_FACTORY_NAME();
        string memory registryType_ = _masterDAORegistry.DAO_VOTING_REGISTRY_NAME();

        address votingFactoryProxy_ = _deploy(address(_masterDAORegistry), factoryType_);
        address votingRegistryProxy_ = _deploy(address(_masterDAORegistry), registryType_);
        ...
        dao_.addContract(dao_.VOTING_FACTORY_NAME(), votingFactoryProxy_);
        dao_.addContract(dao_.VOTING_REGISTRY_NAME(), votingRegistryProxy_);

        _register(address(_masterDAORegistry), factoryType_, votingFactoryProxy_);
        _register(address(_masterDAORegistry), registryType_, votingRegistryProxy_);

        _injectDAODependencies(address(dao_), address(_masterDAORegistry), votingFactoryProxy_);
        _injectDAODependencies(address(dao_), address(_masterDAORegistry), votingRegistryProxy_);
    ...
    function _deployNativeTokenAndVoting(
        DAORegistry dao_,
        ITERC20.ConstructorParams calldata tokenParams_,
        IDAOVoting.ConstructorParams memory votingParams_
    ) private {
        ITokenFactory tokenFactory_ = ITokenFactory(dao_.getTokenFactory());
        address nativeToken_ = tokenFactory_.deployTERC20(tokenParams_);

        votingParams_.votingToken = nativeToken_;
        votingParams_.votingType = VotingType.NON_RESTRICTED;
        votingParams_.panelName = DAO_MAIN_PANEL_NAME;
        votingParams_.votingName = "DAO Voting";
        votingParams_.votingDescription = "Voting to vote for DAO Constitution parameters";

        deployDAOPanel(dao_, votingParams_);
    }

    function _deployStorages(
        DAORegistry dao_,
        string memory panelName_
    ) private returns (DAOParameterStorage, DAOMemberStorage) {
        address daoParameterStorage_ = ProxyBeacon(
            _masterDAORegistry.getProxyBeacon(_masterDAORegistry.DAO_PARAMETER_STORAGE_NAME())
        ).implementation();

        address daoMemberStorage_ = ProxyBeacon(
            _masterDAORegistry.getProxyBeacon(_masterDAORegistry.DAO_MEMBER_STORAGE_NAME())
        ).implementation();

        string memory daoParameterPanelName_ = dao_.getDAOPanelResource(
            dao_.DAO_PARAMETER_STORAGE_NAME(),
            panelName_
        );
        string memory daoMemberPanelName_ = dao_.getDAOPanelResource(
            dao_.DAO_MEMBER_STORAGE_NAME(),
            panelName_
        );

        require(!dao_.hasContract(daoParameterPanelName_), "DAO already has a parameter panel");
        require(!dao_.hasContract(daoMemberPanelName_), "DAO already has a member panel");
        ...
        string memory parameterStorageType_ = _masterDAORegistry.DAO_PARAMETER_STORAGE_NAME();
        string memory memberStorageType_ = _masterDAORegistry.DAO_MEMBER_STORAGE_NAME();

        string memory daoParameterStorageResource_ = _getDAOResource(
            parameterStorageType_,
            address(parameterStorage_)
        );
        string memory daoMemberStorageResource_ = _getDAOResource(
            memberStorageType_,
            address(memberStorage_)
        );

        parameterStorage_.__DAOParameterStorage_init(
            daoParameterStorageResource_,
            VotingType.NON_RESTRICTED
        );
        memberStorage_.__DAOMemberStorage_init(panelName_, daoMemberStorageResource_);

        _register(address(_masterDAORegistry), parameterStorageType_, address(parameterStorage_));
        _register(address(_masterDAORegistry), memberStorageType_, address(memberStorage_));

        _injectDAODependencies(
            address(dao_),
            address(_masterDAORegistry),
            address(parameterStorage_)
        );
        _injectDAODependencies(
            address(dao_),
        ...
        return (parameterStorage_, memberStorage_);
    }
    ...
    function _requirePermission(string memory permission_) private view {
        require(
            _masterAccess.hasPermission(msg.sender, DAO_FACTORY_RESOURCE, permission_),
            "MasterDAOFactory: access denied"
        );
    }
    ...
    function _getResourcesWithPermissionForPanels(
        ...
        string memory panelName_
    ) private view returns (IRBAC.ResourceWithPermissions[] memory) {
        string memory daoParameterPanelResource_ = dao_.getDAOPanelResource(
            dao_.DAO_PARAMETER_STORAGE_NAME(),
            panelName_
        );

        string memory daoMemberPanelResource_ = dao_.getDAOPanelResource(
            dao_.DAO_MEMBER_STORAGE_NAME(),
            panelName_
        );
        ...
        return [daoParameterStorageResource_, daoMemberStorageResource_].asArray();
    }

    function _getDAOResource(
...
contract MasterDAORegistry is IFactoryDAORegistry, AbstractPoolContractsRegistry {
    string public constant MASTER_DAO_REGISTRY_RESOURCE = "MASTER_DAO_REGISTRY_RESOURCE";

    string public constant MASTER_DAO_FACTORY_DEP = "MASTER_DAO_FACTORY";

    string public constant DAO_REGISTRY_NAME = "DAO_REGISTRY";
    string public constant DAO_PERMISSION_MANAGER_NAME = "DAO_PERMISSION_MANAGER";
    ...
    string public constant DAO_TOKEN_FACTORY_NAME = "DAO_TOKEN_FACTORY";
    string public constant DAO_TOKEN_REGISTRY_NAME = "DAO_TOKEN_REGISTRY";
    string public constant DAO_VOTING_FACTORY_NAME = "DAO_VOTING_FACTORY";
    string public constant DAO_VOTING_REGISTRY_NAME = "DAO_VOTING_REGISTRY";

    MasterAccessManagement internal _masterAccess;

    address internal _daoFactory;
    ...
    modifier onlyDAOFactory() {
        require(_daoFactory == msg.sender, "MasterDAORegistry: caller is not a factory");
        _;
    }
    ...
    function setDependencies(address registryAddress_, bytes calldata data_) public override {
        ...
        _masterAccess = MasterAccessManagement(registry_.getMasterPermissionManager());
        _daoFactory = registry_.getContract(MASTER_DAO_FACTORY_DEP);
    }
    ...
    function addProxyPool(string calldata name_, address poolAddress_) external onlyDAOFactory {
        _addProxyPool(name_, poolAddress_);
    }

    function _requirePermission(string memory permission_) internal view {
        require(
            _masterAccess.hasPermission(msg.sender, MASTER_DAO_REGISTRY_RESOURCE, permission_),
...
interface IMasterDAOFactory {
    event DeployedDAORegistry(address daoRegistryProxy_, IDAORegistry.ConstructorParams params_);

    // TODO: add events for storages deployment

    event DeployedPermissionManager(address permissionManagerProxy_, DAORegistry);

    event DeployedTokenFactory(address tokenFactoryProxy_, DAORegistry);

    event DeployedTokenRegistry(address tokenRegistryProxy_, DAORegistry);
    ...
    function deployDAO(IDAORegistry.ConstructorParams calldata params_) external;
}
...
interface IMasterContractsRegistry {
    ...
    function getMasterPermissionManager() external view returns (address);
