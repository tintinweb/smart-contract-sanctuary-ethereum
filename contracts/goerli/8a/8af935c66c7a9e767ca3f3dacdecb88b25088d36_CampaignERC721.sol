...
// File: contracts/CampaignERC721.sol
 ...
 */
contract CampaignERC721 is ERC2771Context, ERC721URIStorage, AccessControlEnumerable {
    ...
    constructor(
        ...
        if(campaignId>bytes12(0) && merkleRoot > bytes32(0) && expireAt > uint64(0)) {
    ...
    function mintNFT(address recipient, string memory tokenUri) external returns (uint256) {
        require(hasRole(MINTER_ROLE, _msgSender()), "DOES_NOT_HAVE_MINTER_ROLE");
        return _mintNFT(recipient, tokenUri);
    }

    function mintNFTCampaign(bytes12 campaignId, bytes32[] memory proof, bytes memory sign, address recipient, string memory tokenUri) external returns (uint256) {
        require(isCampaignActive(campaignId), "campaign is not active");
        (bytes32 userId, bool canMint) = _canUserMintCampaign(campaignId, proof, sign);
    ...
    function createCampaign(bytes12 campaignId, bytes32 merkleRoot, uint64 expireAt) public {
        require(hasRole(MINTER_ROLE, _msgSender()), "DOES_NOT_HAVE_MINTER_ROLE");
    ...
    function isCampaignActive(bytes12 id) public view returns (bool) {
        return campaigns[id].expireAt > block.timestamp && campaigns[id].mintState[BYTE_ONE] == CAN_MINT;
    }
    ...
    function _canUserMintCampaign(bytes12 campaignId, bytes32[] memory proof, bytes memory sign) private view returns (bytes32, bool){
        bytes32 _newHash = ECDSA.toEthSignedMessageHash(keccak256(abi.encodePacked(campaignId)));
        (address _userId,ECDSA.RecoverError error) = ECDSA.tryRecover(_newHash, sign);
        
        if(error != ECDSA.RecoverError.NoError) return (BYTE_ZERO, false);

        bytes32 userId = keccak256(abi.encodePacked(_userId, campaignId));
        return (userId, campaigns[campaignId].mintState[userId] == EMPTY && _isAllowed(campaignId, proof, userId));
    }

    function _isAllowed(bytes12 campaignId, bytes32[] memory proof, bytes32 userId) private view returns (bool) {
    ...
    function _mintNFT(address recipient, string memory tokenUri) private returns (uint256) {
        _tokenIds.increment();
