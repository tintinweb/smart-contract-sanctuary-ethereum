...
contract ShirzanTest is ERC721A, Ownable, ReentrancyGuard {
    using Strings for uint256;

    uint256 private constant MAX_SUPPLY = 7777;
    uint256 private constant MAX_PER_WALLET = 20;
    uint256 private constant PRICE_PER_TOKEN = 1 * 10**14;
    ...
    constructor() ERC721A("Shirzan Test", "SIZ_TST") {
        _unrevealedURI = "ipfs://QmergAZZXbyqXZ1NeMMvTyaZAY41bPVREPXTihL5jjWogb";
        _tokenBaseURI = "ipfs://QmTGNhVMWRpbDjmrb212eYgJK3rmRUEHTVYWnLq2at7yGB/";
    }
    ...
    function mint(uint amount_) public payable nonReentrant {
        require(_mintingIsActive, "minting not active");
        require(totalSupply() + amount_ <= MAX_SUPPLY, "max supply reached");
        require(amount_ <= MAX_PER_WALLET, "max per wallet reached");
        require((PRICE_PER_TOKEN * amount_) == msg.value, "exact value in ETH needed");
    ...
    function reveil() external onlyOwner {
        _reveil();
    }
    ...
    function _reveil() internal {
        require(!_isRevealed, "already reveiled");
        _isRevealed = true;
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        ...
        tokenId = (tokenId + _shift) % MAX_SUPPLY;
