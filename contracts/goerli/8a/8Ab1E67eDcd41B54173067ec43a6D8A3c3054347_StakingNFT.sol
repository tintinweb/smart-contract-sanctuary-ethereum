...
contract StakingNFT is Migrate{
    ...
    function stake(
        ...
        (uint160 _P, ) = _slot0(token0,token1,fee);
        
        
        _staked(count,addrCN,_timeUnlock,procentage,token,liquidity,token0,token1,tickLower,tickUpper,_P);
    ...
    function _getPair(address t0,address t1, uint24 f) private view returns(address _pool){
        return UniswapV3Factory.getPool(t0,t1,f);
    }
    function _slot0 (address _token0,address _token1, uint24 _fee) internal view returns(uint160 _P,int24 _tickNow){
        address pair = _getPair(_token0,_token1,_fee);
        (_P,_tickNow, , , , , ) = IUniswapV3Pool(pair).slot0();
    }
    function _staked(
        uint8 countMonths,
