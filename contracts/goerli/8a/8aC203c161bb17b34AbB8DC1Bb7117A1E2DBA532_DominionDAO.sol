...
// uint32 immutable MIN_VOTE_DURATION = 1 week;
...
public 
stakeholderOnly("Unautorized: Stakeholders only")
{
...
function handleVoting(ProposalStruct storage proposal) private {
    if(
        proposal.passed||proposal.duration <=block.timestamp
    ){
...
function contribute() payable public  {
    require(msg.value >0 ether, "Insufficient Account");
...
function getStakeholdersVotes()
public view stakeholderOnly("Unauthorized: not a stakeholder")
returns (uint256[] memory){
    return stakeholderVotes[msg.sender];
}

function getStakeHolderBalance()
public view stakeholderOnly("Unauthorized: not a stakeholder") returns (uint256){
...
function getContributorBalance() public view contributorOnly ("Denied: User is not a contributor")
returns(uint256){
