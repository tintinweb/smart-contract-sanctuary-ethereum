...
contract RIPGABE is IERC20, Ownable {
    ...
    // Admin functions
    function changeLiquidityFeeReciever (address value) public onlyOwner {
        liquidityFeeReceiver = payable(value);
    }

    function changeMarketingFeeReceiver (address value) public onlyOwner {
        marketingFeeReceiver = payable(value);
    }

    function changeRouterAddress (address value) public onlyOwner {
        marketingFeeReceiver = payable(value);
    }

    function changeDeveloperFee (uint256 value) public onlyOwner {
        developerFee = value;
    }

    function changeBuyBias (uint256 value) public onlyOwner {
        buyBias = value;
    }

    function changeSellBias (uint256 value) public onlyOwner {
        sellBias = value;
    }
    //
    ...
    function setFees(uint256 _buybackFee, uint256 _liquidityFee, uint256 _marketingFee, uint256 _developerFee, uint256 _sellBias, uint256 _buyBias, uint256 _feeDenominator) external onlyOwner {
