...
contract SweepersCompetitionVault is ReentrancyGuard, Ownable, IERC721Receiver, IERC1155Receiver, OpsReady {
    ...
    uint256 public DevFeeBuy = 0.0025 ether;
    uint256 public DevFeeRedeem = 0.0025 ether;
    ...
    uint16 public earningRate = 10;

    modifier holdsSweeper(uint256 _id) {
        require(!comps[_id].mustHoldSweeper || SWEEPERS.balanceOf(msg.sender) > 0, "Must hold a Sweeper");
        _;
    }
    ...
    function setDev(address _dev, uint256 _devFeeBuy, uint256 _devFeeRedeem) external onlyOwner {
        Dev = payable(_dev);
        DevFeeBuy = _devFeeBuy;
        DevFeeRedeem = _devFeeRedeem;
    }
    ...
    function setEarningRate(uint16 _rate) external onlyOwner {
        earningRate = _rate;
    }

    function setMustHold(uint256 _id, bool _flag) external onlyOwner {
        comps[_id].mustHoldSweeper = _flag;
    }
    ...
    function claimRefund(uint256 _id) external nonReentrant {
        ...
        DUST.mint(msg.sender, claimData.dustSpent);
     ...
     */
    function buyEntryDust(uint256 _id, uint256 _priceId, bytes32 _referrer, uint16 _redeemEntries) external payable holdsSweeper(_id) nonReentrant {
        ...
        require(msg.value == DevFeeBuy, 'Fee not covered');
        ...
        uint16 _numEntries = priceStruct.numEntries;
        if(_redeemEntries > 0) {
        ...
        (bool sent,) = Dev.call{value: DevFeeBuy}("");
        require(sent);
     ...
     */
    function buyEntryETH(uint32 _id, uint256 _priceId, bytes32 _referrer, uint16 _redeemEntries) external payable holdsSweeper(_id) nonReentrant {
        require(compStatus(_id) == 1, 'Comp is not Active');
