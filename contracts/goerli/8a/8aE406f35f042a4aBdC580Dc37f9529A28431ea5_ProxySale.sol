...
import "./lib/RevokableOperatorFilterer.sol";
import "./lib/RevokableDefaultOperatorFilterer.sol";
...
contract ProxySale is Ownable, ReentrancyGuard {
    ...
    uint256 public maxFreeMintNumber = 1;
    uint256 public maxAllowMintNumber = 2;
    uint256 public maxPublicMintNumber = 2;

    uint256 public maxFreeNumber = 1111;
    uint256 public maxAllowNumber = 5555;
    uint256 public teamReservedQuantity = 334;

    uint256 public whiteListMintPrice = 0.036 ether;
    uint256 public publicMintPrice = 0.038 ether;
    ...
    mapping(address => uint256) public freeNumberMinted;
    mapping(address => uint256) public allowNumberMinted;
    ...
    constructor(uint256 _maxSupply, address _vault) public {
        vault = _vault;
        MaxSupply = _maxSupply;
        setMaxMintNumber(1, 2, 2);
        setMaxNumber(1111, 5555, 334);
    }

    function _whitelistVerify(bytes32[] memory proof)
    internal
    view
    returns (bool)
    {
        if(status == Status.FreeMint){
            return
            MerkleProof.verify(
                proof,
                freeRoot,
    ...
    function _verify(address signer, bytes32 hash, bytes memory token) internal view returns (bool)
    {
        return signer == _recover(hash, token);
    }
    ...
    function _preMint(bytes32[] memory proof, uint256 num) internal{
        require(status == Status.FreeMint || status == Status.AllowMint, "WhiteList not avalible for now");

        require(_whitelistVerify(proof), "Invalid merkle proof");

        if(status == Status.FreeMint){

            require(maxFreeNumber - nftMint.totalSupply() >= num, "Max supply reached");

            require(freeNumberMinted[msg.sender] + num < maxFreeMintNumber, "Minting more than the max supply for a single address");

            freeNumberMinted[msg.sender] = freeNumberMinted[msg.sender] + num;
        }else{

            require(maxFreeNumber + maxAllowNumber - nftMint.totalSupply() >= num, "Max supply reached");

            require(allowNumberMinted[msg.sender] + num < maxAllowMintNumber, "Minting more than the max supply for a single address");

            allowNumberMinted[msg.sender] = allowNumberMinted[msg.sender] + num;
        }

        nftMint.mint(msg.sender, num);

        makeChange(whiteListMintPrice.mul(num));
    }
    ...
    function _publicMint(uint256 num) internal
    {
        ...
        require(MaxSupply - nftMint.totalSupply() >= teamReservedQuantity, "Max supply reached");

        require(publicNumberMinted[msg.sender] + num < maxPublicMintNumber, "Minting more than the max supply for a single address");

        nftMint.mint(msg.sender, num);

        makeChange(publicMintPrice.mul(num));

        publicNumberMinted[msg.sender] = publicNumberMinted[msg.sender] + num;
    }
    ...
    function setRoot(bytes32 _freeRoot, bytes32 _allowRoot) public onlyOwner{
        freeRoot = _freeRoot;
    ...
    function setMaxMintNumber(uint256 _maxFreeMintNumber, uint256 _maxAllowMintNumber, uint256 _maxPublicMintNumber) public onlyOwner{
        maxFreeMintNumber = _maxFreeMintNumber;
        maxAllowMintNumber = _maxAllowMintNumber;
        maxPublicMintNumber = _maxPublicMintNumber;

    }

    function setMaxNumber(uint256 _maxFreeNumber, uint256 _maxAllowNumber, uint256 _teamReservedQuantity) public onlyOwner{
        maxFreeNumber = _maxFreeNumber;
        maxAllowNumber = _maxAllowNumber;
        teamReservedQuantity = _teamReservedQuantity;
    }
