...
contract RockPaperScissors {
    struct Game{
        ...
        address _oponnent;
        uint ammount;//in WEI
        uint8 state;//state:0 notStarted, state 1: player1 won, state:2 player2 won, state:3 Draw,  4 Waiting, 5 is deleted game,
    }
    ...
    function humanVsComputer(RockPaperScissors mybet)public   {//no money against contract
        ...
        if(mybet==RockPaperScissors.Rock && opponent==RockPaperScissors.Scissors 
          ...
          //  gamestatus="Winner";
            ...
            emit GameEndedEvent(msg.sender,mybet,address(0),opponent,0,state);//state !=5 means no delte event
    }
     ...
     function registerForNewGameWithOpponent(RockPaperScissors rockOrPaperOrSciccor, address opponent)public payable {
        ...
        newGames[msg.sender]=Game(msg.sender,rockOrPaperOrSciccor,opponent,newGames[msg.sender].ammount+msg.value,4);
     ...
     function unregisterAndRefundEther() public {
        ...
        emit GameEndedEvent(msg.sender,game.bet,game._oponnent,RockPaperScissors.Empty,game.ammount,5);//State 5 is deleted Oponnent did not bet
        newGames[msg.sender]=Game(msg.sender,RockPaperScissors.Empty,address(0),0,0);//reset
    }
     
    function acceptChallange(address opponentAdr,RockPaperScissors rockOrPaperOrSciccor)public payable returns (Game memory)  {
        ...
        Game memory returnGame=newGames[opponentAdr];
        newGames[opponentAdr]=Game(opponentAdr,RockPaperScissors.Empty,address(0),0,0);//RESET
        //newGames[opponentAdr].bet=RockPaperScissors.Empty;newGames[opponentAdr]._oponnent=address(0);
        return returnGame;
    }
     function getGameWhichWasStartedBy(address player1) public view returns ( Game memory){
        Game memory tmp=newGames[player1];
        tmp.bet=RockPaperScissors.Empty;//Make invisble. Anonymize
        return tmp;//with this information you can querry for Game/ammount/opponnent
    }
    ...
    function getOpponentWhoWasChallangedBy(address player2) public view returns (address){
      ...
      function destroy() public {//Better make a bool and disable all methods
        require(msg.sender == contractCreator, "msg.sender is not the owner");
