...
contract SwaprFee is BaseGovernanceUpgradeable {
    ...
    function addPaymentToken(IERC20MetadataUpgradeable[] memory _paymentTokens) external onlyAdmin {
    ...
    /// @param tokenPriceUSD Coingecko price of the payment token in USD
    /// @return baseFee base calculated fee
    function getBaseOrderFee(uint subjectAmount, uint tokenPriceUSD) public view returns(uint baseFee) {
        baseFee = _getBaseFee(OrderFee, subjectAmount, tokenPriceUSD);
    ...
    /// @param subjectAmount The amount to be priced for listing subject to the fee in percentage
    /// @param fixedBaseFeeTokens Coingecko price of the payment token in USD
    /// @return baseFee base calculated fee
    function getBaseAuctionFee(uint subjectAmount, uint fixedBaseFeeTokens) public view returns(uint baseFee) {
        baseFee = _getBaseFee(AuctionFee, subjectAmount, fixedBaseFeeTokens);
    ...
    function getFinalOrderFee(uint subjectAmount) public view returns(uint finalFee) {
        finalFee = _getFinalFee(OrderFee, subjectAmount);
        if(finalFee > OrderFee.priceCap){
            finalFee = OrderFee.priceCap;
        }
    }
    ...
    /// @param subjectAmount The amount to be priced for listing subject to the fee in percentage
    /// @return finalFee final calculated fee
    function getFinalAuctionFee(uint subjectAmount) public view returns(uint finalFee) {
        finalFee = _getFinalFee(AuctionFee, subjectAmount);
        if(finalFee > AuctionFee.priceCap){
            finalFee = AuctionFee.priceCap;
        }
    }
    ...
    /// @return baseFee base calculated fee
    function _getBaseFee(FeePricing memory feePricing, uint subjectAmount, uint fixedBaseFeeTokens) internal pure returns(uint baseFee) {
        ...
        if(feePricing.isFixedBaseFee && feePricing.isPercentBaseFee){
            baseFee = fixedBaseFeeTokens + percentBaseFee;
        }
        if(feePricing.isFixedBaseFee && !feePricing.isPercentBaseFee){
            baseFee = fixedBaseFeeTokens;
        }
        if(!feePricing.isFixedBaseFee && feePricing.isPercentBaseFee){
