...
contract ChainWaves is ERC721, Owned {
    ...
    // TODO: Get Merkleroot of SC holders
    ...
    //uint arrays
    uint16[][4] private TIERS;

    constructor()
        ERC721("ChainWaves", "CA")
        Owned(0x9ea04B953640223dbb8098ee89C28E7a3B448858)
    {
        // TODO: Set right address of generator (if generator necessary)
        chainWavesGenerator = new ChainWavesGenerator();

        // TODO: Replace Folded Faces Traits/Rarity %s
        //Palette
        TIERS[0] = [2000, 2000, 2000, 4000];
        //Noise
    ...
    function snowcrashMint(address account, bytes32[] calldata proof)
        external
        payable
    {
        bytes32 node = keccak256(abi.encodePacked(account));
        if (!MerkleProof.verify(proof, snowcrashRoot, node))
            revert NotSnowcrashList();
        if (account != msg.sender) revert SelfMintOnly();
        if (msg.value != SNOWCRASH_PRICE) revert EthFail();
        if (snowcrashMinted[msg.sender]) revert SnowcrashMinted();

        snowcrashMinted[msg.sender] = true;
    ...
    // TODO: everything :kek:
    function hashToSVG(string calldata _hash, uint256 _tokenId)
        external
        view
        disallowIfStateIsChanging
        returns (string memory svgString)
    {
        // From the hash parse out what traits they equal
        // Input the necessary traits into SVG string
        // return SVG string
    }
                    ...
                    bytes(
                        string(
                            abi.encodePacked(
                                ...
                                '","attributes":[{"trait_type":"chain","value":"waves"}]}'
                            )
                        )
                    )
                )
...
contract ChainWavesGenerator {
    ...
    constructor() {
        PALETTES[0] = ["d00000", "370617", "faa307"];
        PALETTES[1] = ["3a0ca3", "f72585", "4cc9f0"];
        PALETTES[2] = ["250902", "38040e", "640d14"];
        PALETTES[3] = ["7067CF", "B7C0EE", "CBF3d2"];
    ...
    function buildEightLines(string memory _chars, uint256 _modStart)
        public
        pure
        returns (string memory lineOut)
    {
        uint8 x = 1;
        uint8 y = 0;
        for (uint256 i; i < 8; ++i) {
        ...
        returns (string memory _svg)
    {
        ...
        string[] memory palette = tokenTraits.palette;
        uint256 noise = tokenTraits.noise;
        string memory chars = tokenTraits.charSet;
        ...
        _svg = string(
            abi.encodePacked(
                ...
                buildEightLines(chars, modStart),
                "<animate attributeName='font-size' attributeType='XML' values='100%;60%;100%' begin='0s' dur='15s' repeatCount='indefinite'/></g><filter id='turbulence'><feTurbulence type='turbulence' baseFrequency='0.",
                AnonymiceLibrary.toString(noise),
                "' numOctaves='5' result='noise' seed='",
                AnonymiceLibrary.toString(_tokenId),
                "'/><feDisplacementMap in='SourceGraphic' in2='noise' scale='3' /></filter></defs><use href='#chars' y='0' x='-3' filter='url(#turbulence)' width='20' height='20' fill='#",
                palette[1],
                "'/> <use href='#chars' y='-4' x='-1' filter='url(#turbulence)' width='20' height='20' fill='#",
                palette[2]
            )
        );
    }
    ...
    {
        uint256[] memory traitArray = new uint256[](4);
