  ...
  UUPSUpgradeable
{
  ...
  struct RoyaltyInfo {
    address[] receivers;
    uint16[] receiversPercentage; // Sum must be always equal to feeDenominator = 10000 = 100%
    uint16 royaltyPercentage;
  }

  // Fired when maxRoyaltyReceiversCount is changed
  event MaxRoyaltyReceiversCountUpdated(uint16 maxRoyaltyReceiversCount);
  // Number of royalty receivers per token, can be changed later
  uint16 public maxRoyaltyReceiversCount;

  event CollectionDefaultRoyaltyInfoSet(address _token);
  event CollectionDefaultRoyaltyInfoDeleted(address _token);
  // Manages Collection's DefaultRoyaltyInfo
  mapping(address => RoyaltyInfo) private _collectionDefaultRoyaltyInfo;

  event TokenRoyaltyInfoSet(address _token, uint256 _tokenId);
  event TokenRoyaltyInfoReset(address _token, uint256 _tokenId);
   ...
   * TODO: check for using initializer or onlyInitializing
   */
   ...
   *
   * @param _newMaxRoyaltyReceiversCount uint16
   */
  function updateMaxRoyaltyReceiversCount(uint16 _newMaxRoyaltyReceiversCount)
    external
    onlyAdmin
  {
  ...
  function royaltyInfo(
  ...
  ) public view virtual override returns (address[] memory, uint256[] memory) {
    ...
    RoyaltyInfo memory royalty = _tokenRoyaltyInfo[_token][_tokenId];
    if (royalty.receivers.length == 0) {
      // We haven't found RoyaltyInfo for respective _token and _tokenId, let's see if DefaultRoyaltyInfo exists
      royalty = _collectionDefaultRoyaltyInfo[_token];
      // EM: RoyaltyRegistry:royaltyInfo no CollectionRoyaltyInfo nor DefaultRoyaltyInfo was found
      require(royalty.receivers.length != 0, "RR:RI:RINF");
    }
    ...
    if (_isSecondarySale == 1) {
      ...
      splitTotalPrice =
        (_salePrice * royalty.royaltyPercentage) /
        feeDenominator();
    }

    uint256[] memory royaltyFees = new uint256[](royalty.receivers.length);
    for (uint256 idx = 0; idx < royalty.receivers.length; idx++) {
      royaltyFees[idx] =
        (splitTotalPrice * royalty.receiversPercentage[idx]) /
        feeDenominator();
    }

    return (royalty.receivers, royaltyFees);
  }
   ...
   * @param _receivers address[] calldata
   * @param _receiversPercentage uint16[] calldata
   * @param _royaltyPercentage uint16
   */
  function _validateRoyaltyInfo(
    address _token,
    address[] calldata _receivers,
    uint16[] calldata _receiversPercentage,
    uint16 _royaltyPercentage
  ) private view {
    // EM: RoyaltyRegistry:_validateRoyalty _token must be a contract address
    require(_token.isContract(), "RR:VR:TNC");
    // EM: RoyaltyRegistry:_validateRoyalty Difference in the number of elements
    require(_receivers.length == _receiversPercentage.length, "RR:VR:DNOE");
    // EM: RoyaltyRegistry:_validateRoyalty Overfrow Max setting elements
    require(_receivers.length <= maxRoyaltyReceiversCount, "RR:VR:RME");

    //Address validation
    // EM: RoyaltyRegistry:_validateRoyalty zero address must not be included in receivers
    require(_validateReceiversAddresses(_receivers), "RR:VR:IA");
    // EM: RoyaltyRegistry:_validateRoyalty receiversPercentage not 100
    require(
      _calcReceiversPercentage(_receiversPercentage) == feeDenominator(),
      "RR:VR:RPN100"
    );
    // EM: RoyaltyRegistry:_validateRoyalty Overfrow royalty value limit or value 0
    require(
      _royaltyPercentage > 0 && _royaltyPercentage <= (feeDenominator() / 2),
      "RR:VR:ROFZ"
    );
  }

  /**
   * @dev Calculate total percentage of royalty
   *
   * @param _receiversPercentage uint16[] calldata
   */
  function _calcReceiversPercentage(uint16[] calldata _receiversPercentage)
    private
    pure
    returns (uint16)
  {
    uint16 total = 0;
    for (uint256 idx = 0; idx < _receiversPercentage.length; idx++) {
      // EM: RoyaltyRegistry:_calcPercentage receiversFractions cannot be zero
      require(_receiversPercentage[idx] > 0, "RR:CP:RZ");
      total += _receiversPercentage[idx];
    }
    return total;
  }

  /**
   * @dev validate Addresses function
   *
   * @param _receivers address[] calldata
   */
  function _validateReceiversAddresses(address[] calldata _receivers)
    private
    pure
    returns (bool)
  {
    for (uint256 idx = 0; idx < _receivers.length; idx++) {
      if (_receivers[idx] == address(0)) {
        return false;
      }
    }
    //trueの場合は使わないので基本0としている
    return true;
  }
  ...
  function setCollectionDefaultRoyalty(
    ...
    _validateRoyaltyInfo(
      _token,
      _receivers,
      _receiversPercentage,
      _royaltyPercentage
    );

    _collectionDefaultRoyaltyInfo[_token] = RoyaltyInfo(
      _receivers,
      _receiversPercentage,
      _royaltyPercentage
    );

    emit CollectionDefaultRoyaltyInfoSet(_token);
  }
   ...
   * Requirements:
   *
   * - `_receivers` cannot be the zero address.
   * - `_receiversPercentage` cannot be greater than the fee denominator.
   */
  function setTokenRoyalty(
    ...
    require(_tokenId != 0, "RR:SCR:TIDNZ");

    _tokenRoyaltyInfo[_token][_tokenId] = RoyaltyInfo(
      _receivers,
      _receiversPercentage,
      _royaltyPercentage
    );

    emit TokenRoyaltyInfoSet(_token, _tokenId);
  }
