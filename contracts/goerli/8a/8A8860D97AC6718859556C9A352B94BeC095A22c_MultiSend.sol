...
contract MultiSend {
    /**
     * @notice msg.sender should have {setApprovalForAll} true of {contractAddresses}
     * @dev SendsERC721 tokens with {tokenIds} to {recipients} addresses
     * @param contracts ERC721 contract addresses
    ...
    function multiSendERC721(
        ...
        IERC721[] memory _contracts = contracts;
        address[] memory _recipients = recipients;
        uint256[] memory _tokenIds = tokenIds;
        uint256 length = _recipients.length;

        for (uint256 i; i < length; ) {
            _contracts[i].safeTransferFrom(msg.sender, _recipients[i], _tokenIds[i]);
    ...
    function multiSendERC1155(
        ...
        IERC1155[] memory _contracts = contracts;
        address[] memory _recipients = recipients;
        uint256[] memory _tokenIds = tokenIds;
        ...
        for (uint256 i; i < length; ) {
            _contracts[i].safeTransferFrom(msg.sender, _recipients[i], _tokenIds[i], _amounts[i], "");
