...
contract PhalanxV3 is Initializable, ERC1155URIStorageUpgradeable, OwnableUpgradeable, IERC1155ReceiverUpgradeable, IERC721ReceiverUpgradeable  {
    
    event TokenMinted(address indexed _by, uint256 indexed _id);

    enum MintType { DAO, OWNER, MEMBER }
    ...
    function uri(
        ...
        return bytes(_uri).length > 0 ? _uri : super.uri(_tokenIdFromType(tokenId >> 8, MintType.DAO));
    }
