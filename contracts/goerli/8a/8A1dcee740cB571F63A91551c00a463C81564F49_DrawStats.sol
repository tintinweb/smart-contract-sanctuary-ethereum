...
import "./PotatozDraw.sol";

contract DrawStats {
    PotatozDraw public drawContract;
    address public owner;
    uint256 public arrayLengthMax = 9999;
    ...
    constructor(address _drawContract) {
        owner = msg.sender;
        drawContract = PotatozDraw(_drawContract);
    }

    function setDrawContract(address _drawContract) external onlyOwner {
        drawContract = PotatozDraw(_drawContract);
    }

    function setArrayLengthMax(uint256 _arrayLengthMax) external onlyOwner {
        arrayLengthMax = _arrayLengthMax;
    }

    function getDrawStats(string calldata _drawID)
        public
        view
        returns (uint256[] memory)
    {
        require(address(drawContract) != address(0), "Address not set");
        uint256[] memory ret = new uint256[](2);

        // dirty binary search to find length of a public array
        uint256 low = 0;
        uint256 high = arrayLengthMax;
        while (low < high) {
            uint256 mid = Math.average(low, high);
            try drawContract.drawEndTicketOrdinals(_drawID, mid) {
                low = mid + 1;
            } catch {
                high = mid;
            }
        ...
        try drawContract.drawEndTicketOrdinals(_drawID, low) returns (
            uint256 r
        ) {
            ret[0] = low + 1; // participants
            ret[1] = r + 1; // ticketsTotal
        } catch {
            try drawContract.drawEndTicketOrdinals(_drawID, low - 1) returns (
                uint256 r
            ) {
                ret[0] = low; // participants, (low - 1) + 1
                ret[1] = r + 1; // ticketsTotal
            } catch {
                // wtf?
            }
