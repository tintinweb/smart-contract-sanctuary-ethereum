...
contract DaoFactory is Pausable {
    ...
    address private _dao_join_address;
    ...
    */
    event daoCreated(string name, string symbol, address owner_of, uint256 presale, uint256 limit, uint256 price, address token_address, bool created);
    ...
    */
    function createDao(string memory name, string memory symbol, uint256 presale, uint256 limit, uint256 price) public notPaused {
        DaoToken token = new DaoToken(name, symbol, msg.sender, presale, limit, price, _dao_join_address);
        ...
        emit daoCreated(name, symbol, msg.sender, presale, limit, price, token_address, true);
    }
    ...
    */
    function addDao(address token_address) public notPaused {
        ...
        _owner_by_dao[token_address] = msg.sender;
        _is_activated[token_address] = true;
        emit daoCreated(token.name(), token.symbol(), msg.sender, presale, limit, price, token_address, false);
    }
    ...
    */
    ...
    function setDaoJoinAddress(address dao_join_address_) public {
        _dao_join_address = dao_join_address_;
    }
}
...
interface IDAOJoin {
    function createNewDAOPool(address token_address_, uint256 price_, address owner_of_) external;
}
...
contract DaoToken is ERC20Burnable, Pausable {
    ...
    constructor(string memory name, string memory symbol, address owner_of, uint256 presale_, uint256 limit_, uint256 price_, address dao_join_address_) ERC20(name, symbol) Pausable(owner_of) {
        if (limit_ > 0) {
            require(limit_ >= presale_, "Limit overrized");
            _mint(dao_join_address_, limit_ - presale_);
            IDAOJoin(dao_join_address_).createNewDAOPool(address(this), price_, owner_of);
        }
        if (presale_ > 0) {
    ...
    function mint(uint256 amount_) public payable notPaused {
