...
contract MGTESTNFT2NFT is
    ERC721AQueryable,
    Pausable,
    AccessControl {

    uint256 public constant TONIC_SUPPLY = 800;
    uint256 public constant MULE_SUPPLY = 800;
    uint256 public constant MARTINI_SUPPLY = 800;
    uint256 public constant RARE_SUPPLY = 100;
    uint256 public constant MAX_SUPPLY = TONIC_SUPPLY + MULE_SUPPLY + MARTINI_SUPPLY + RARE_SUPPLY;

    uint256 public constant DEFAULT_MINT_PRICE = 0.08 ether;
    uint256 public mint_price = DEFAULT_MINT_PRICE;
    
    bool public revealed = false;
    
    string private hiddenMetadataUri;
    
    // please have a trailing slash in the baseURI
    string private baseURI;

    constructor(string memory name, string memory symbol, string memory _hiddenMetadataUri, uint256 _mint_price) ERC721A(name, symbol) {
        _setupRole(DEFAULT_ADMIN_ROLE, _msgSender());
        hiddenMetadataUri = _hiddenMetadataUri;
        mint_price = _mint_price;
    }

    function setMintPrice(uint256 _mint_price) public onlyRole(DEFAULT_ADMIN_ROLE) {
        mint_price = _mint_price;
    }

    /// @notice Set hiddenMetadataUri for UNREVEALED token metadata URI, please have a trailing slash in the baseURI
    ...
    /// @notice Set BaseURI for token metadata URI, please have a trailing slash in the baseURI
    /// @dev The caller must have the `DEFAULT_ADMIN_ROLE`.
    ...
    /// @notice our safeMint takes care of Token Supply
    /// @param _to address to mint to.
    function safeMint(address _to, uint256 _amount) internal {
        require(_totalMinted() + _amount < MAX_SUPPLY, "Not enough NFTs left!");
        _safeMint(_to, _amount);
    }

    /// @notice our external mintToken function that checks for price
    function mintToken(uint256 _amount) external payable {
        require(msg.value == _amount * mint_price, "Not enough ether to purchase NFT.");
        safeMint(_msgSender(), _amount);
    }

    /// @notice our _beforeTokenTransfer that halts if the contract is paused
    function _beforeTokenTransfers(
        address from,
        address to,
        uint256 firstTokenId,
        uint256 batchSize
    ) internal virtual override(ERC721A) whenNotPaused {
        super._beforeTokenTransfers(from, to, firstTokenId, batchSize);
    }

    /// @notice set the reveal status of all NFTs
    /// @param _revealed whether the NFTs are revealed or not.
    ...
    function tokenURI(uint256 tokenId)
        ...
        require(_exists(tokenId), Errors.NOT_EXIST_TOKEN_ID);
        return revealed ? string(abi.encodePacked(baseURI, Strings.toString(tokenId))) : hiddenMetadataUri;
    }
