...
contract Today is IERC721, ERC721Enumerable, Ownable {
	...
	function getPrice() public view returns (uint256) {
		...
		for (uint256 i = 0; i < hour; i++) {
			price = max(PRICE_MIN, (price * PRICE_DECREASE_PERCENTAGE) / 100);
		}
		return price;
	}
	...
	function tokenURI(uint256 tokenId) public view override returns (string memory) {
		...
		string memory dateStr = getDateStr(storedTime);
		string memory svg = renderer.render(dateStr);
	...
	function currentImage() public view returns (string memory) {
		string memory svg = renderer.render(getDateStr(block.timestamp));
	...
	function getYear(uint256 time) internal pure returns (uint256) {
		uint256 secondsAccountedFor = 0;
		uint256 year;
		uint256 numLeapYears;

		// Year
		year = ORIGIN_YEAR + time / YEAR_IN_SECONDS;
...
contract TodayRenderer {
	...
	function getRandomHSL(uint256 seed)
		private
		pure
		returns (
			uint256 hue,
