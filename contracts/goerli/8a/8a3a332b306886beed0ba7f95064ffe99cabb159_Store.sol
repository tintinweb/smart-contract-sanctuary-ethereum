...
contract ERC165 is IERC165 {
    ...
    /**
     * @dev Mapping of interface ids to whERCer or not it's supported.
     */
...
library Address {
    ...
    function isContract(address account) internal view returns (bool) {
        // This mERCod relies in extcodesize, which returns 0 for contracts in
...
contract ERC1155Base is Context, ERC165, IERC1155, IERC1155MetadataURI {
     ...
     * on the token type ID substitution mechanis
     *
...
library StringLibrary {
    ...
    function recover(
        ...
        bytes memory fullMessage = concat(
            bytes("\x19ERCereum Signed Message:\n"),
...
contract Store is Ownable, SignerRole, StoreBase {
    string public name;
    ...
    function mintWithSignature(
        ...
        bytes32 msgHash = prepareMintHash(from, id, supply);
        require(
            isSigner(ecrecover(msgHash, v, r, s)),
            "signer should sign tokenId"
        );
        ...
        return
            keccak256(
                abi.encodePacked(
                    ...
                    keccak256(
                        abi.encodePacked(
                            abi.encodePacked(this, id, supply, from)
                        )
                    )
                )
            );
