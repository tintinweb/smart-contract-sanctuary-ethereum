...
contract Gasoline is ERC721A, VRFConsumerBaseV2, AccessControl {
    ...
    uint256 public constant MAX_NORMAL_TOKEN_SUPPLY = 3000;
    ...
    bytes32 public weirdosMerkleRoot = 0x0;
    ...
    uint256 private _currentIndexNormal;
    ...
    constructor(
        ...
        _currentIndexNormal = _normalStartTokenId();
    ...
    function setWeirdosMerkleRoot(bytes32 _merkleRoot)
        external
        onlyRole(SUPPORT_ROLE)
    {
        weirdosMerkleRoot = _merkleRoot;
    }

    function pause(bool _pause) external onlyRole(SUPPORT_ROLE) {
        paused = _pause;
    }
    ...
    uint256 public constant NORMAL_MINT_WEIRDOS_COST = 5;
    uint256 public constant SUPER_MINT_WEIRDOS_COST = 10;

    /**
     * @notice This function allows for a weirdos holder to mint a number of normal and/or super passes
     *         based on how many weirdos they own at the time of a snapshot.
     */
    function weirdosMint(
        bytes32[] calldata _merkleProof,
        uint256 _numberOfNormalMints,
        ...
        bytes32 leaf = keccak256(
            abi.encodePacked(
                msg.sender,
                _numberOfNormalMints *
                    NORMAL_MINT_WEIRDOS_COST +
                    _numberOfSuperMints *
                    SUPER_MINT_WEIRDOS_COST
            )
        );

        require(
            MerkleProof.verify(_merkleProof, weirdosMerkleRoot, leaf),
            "Invalid Merkle proof."
        );

        if (_numberOfNormalMints > 0) {
            _normalMint(msg.sender, _numberOfNormalMints);
        }
    ...
    function _normalMint(address _to, uint256 _mintAmount) private {
        // mint 1-3000
        require(
            totalMintedNormal() + _mintAmount <= MAX_NORMAL_TOKEN_SUPPLY,
            "Will exceed token supply."
        );

        _safeMint(_to, _mintAmount, _currentIndexNormal);
        _currentIndexNormal = _currentIndexNormal + _mintAmount;
    }
    ...
    function _normalStartTokenId() internal view virtual returns (uint256) {
    ...
    function totalMintedNormal() public view virtual returns (uint256) {
        unchecked {
            return _currentIndexNormal - _normalStartTokenId();
        }
    }
    ...
    function _totalMinted() internal view virtual override returns (uint256) {
        unchecked {
            return totalMintedNormal() + totalMintedSuper();
        }
    }
    ...
    function _unstake(uint256 tokenId, bool _expel) internal {
        if (!_expel) {
            require(ownerOf(tokenId) == msg.sender, "Not owner.");
        }
