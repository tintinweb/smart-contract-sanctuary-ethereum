...
contract NFT721 is ERC721A, Ownable, ReentrancyGuard, DefaultOperatorFilterer {
    ...
    function mint(uint256 _mintAmount, bytes32[] calldata proof) 
        ...
        require(verifyWhitelist(proof), 'Whitelist only!');

        if(freeMint[_msgSender()]) {
            // Free mint used up
            require(msg.value >= _mintAmount * MINT_PRICE, 'Insufficient Funds!');
        } else {
            // Update price include a free mint
            require(msg.value >= (_mintAmount - 1) * MINT_PRICE, 'Insufficient Funds!');
    ...
    modifier mintCompliance(uint256 _mintAmount) {
        require(_mintAmount <= 11, "Max 11 per transaction");
