...
contract Stable is Ownable, ERC721AWithRoyalties, Pausable, PaymentSplitter {
    ...
    function requestVerifier(
        ...
        require(
            Farmer(Index.getAddress("Farm")).ownerOf(farmID) == requester,
    ...
    function destablingVerifier(
        ...
        Horser HorseGovContract = Horser(HorseGovAddress);
        require(
            HorseContract.ownerOf(horseID) == to || ownerOf(farmID) == to,
        ...
        if (block.timestamp < HorseContract.checkStableExp(horseID)) {
            if (msg.sender == HorseContract.ownerOf(horseID)) {
                _addr = payable(ownerOf(farmID));
            } else if (msg.sender == ownerOf(farmID)) {
                if (
                    HorseContract.ownerOf(horseID) ==
                    Index.getAddress("HorsePartnership")
                ) {
                    _addr = payable(HorseGovContract.ownerOf(horseID));
                } else {
                    _addr = payable(HorseContract.ownerOf(horseID));
                }
            }
        }
        return _addr;
