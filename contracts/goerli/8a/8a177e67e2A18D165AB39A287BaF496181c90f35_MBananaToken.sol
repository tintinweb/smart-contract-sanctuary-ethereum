...
///@author 0xBeans
///@notice shout out to 0xBeans for this amazing implementation. 10x better than mine so
///@notice I adopted it kinda last minute
...
abstract contract MULTIDRIP20 {
    ...
    function _transfer(
        ...
        if (fromAccruer.accrualStartBlock != 0) {
            fromAccruer.accrualStartBlock = uint256(block.number);
        }
    ...
    function _startDripping(address addr, uint256 multiplier) internal virtual {
        ...
        _currEmissionBlockNum = uint256(block.number);
        accruer.accrualStartBlock = uint256(block.number);
