...
contract TwapLimitOrder is ITwapLimitOrder {
    ...
    LimitOrders.Data internal orders;
    ...
    function getOrderStatus(uint256 orderId) external view override returns (LimitOrders.LimitOrderStatus) {
    ...
    function getTwapPrice(
        ...
        require(orders.delay != address(0), 'LO02');
        require(orders.factory != address(0), 'LO02');
        (address pairAddress, ) = orders.getPair(token0, token1);
        address uniswapPair = ITwapOracle(ITwapPair(pairAddress).oracle()).uniswapPair();
        int256 decimalsConverter = (10**(18 + IERC20(token0).decimals() - IERC20(token1).decimals())).toInt256();
    ...
    function sell(
        ...
        uint256 currentPrice = getTwapPrice(sellParams.tokenIn, sellParams.tokenOut, twapInterval);
        orderId = orders.sellLimitOrder(sellParams, price, twapInterval, tokenShares);
        (, bool inverted) = orders.getPair(sellParams.tokenIn, sellParams.tokenOut);
        if (inverted && currentPrice <= price) {
            execute(orderId);
        } else if (!inverted && currentPrice >= price) {
            execute(orderId);
        }
    }
    ...
    function buy(
        ...
        uint256 currentPrice = getTwapPrice(buyParams.tokenIn, buyParams.tokenOut, twapInterval);
        orderId = orders.buyLimitOrder(buyParams, price, twapInterval, tokenShares);
        (, bool inverted) = orders.getPair(buyParams.tokenIn, buyParams.tokenOut);
        if (inverted && currentPrice >= price) {
            execute(orderId);
        } else if (!inverted && currentPrice <= price) {
            execute(orderId);
    ...
    function execute(uint256 orderId) public override lock {
        require(isBot[msg.sender] || msg.sender == orders.orders[orderId].submitter, 'LO00');
        uint256 gasStart = gasleft();
        try orders.executeOrder(orderId, tokenShares) {
          (uint256 gasUsed, uint256 leftOver) = refund(
                orders.orders[orderId].gasLimit,
                orders.orders[orderId].gasPrice,
                gasStart,
                orders.orders[orderId].to
            );
            emit OrderExecuted(orderId, gasUsed, leftOver);
        } catch Error(string memory _err) {
            emit OrderExecuteFailed(orderId, _err);
            orders.markOrderFail(orderId);
        } catch (bytes memory reason) {
            emit OrderExecuteFailed(orderId, string(abi.encodePacked(reason)));
            orders.markOrderFail(orderId);
        }
    }
    ...
    function refund(
        ...
        require(refundEth(msg.sender, actualRefund), 'L040');
        refundEth(payable(to), leftOver);
    }

    function retryRefund(uint256 orderId) external override lock {
        if (orders.orders[orderId].status == LimitOrders.LimitOrderStatus.Fail) {
            performRefund(orderId);
            forgetOrder(orderId);
        } else if (orders.orders[orderId].status == LimitOrders.LimitOrderStatus.Submitted) {
            ITwapDelay(orders.delay).retryRefund(orders.delayOrderIdMapping[orderId]);
            forgetOrder(orderId);
        } else {
            revert('order status is not failed');
        }
    }

    function getOrder(uint256 orderId) external view override returns (LimitOrders.StoredOrders memory order) {
        return orders.getOrder(orderId);
    }

    function getOrders(address user) external view override returns (uint256[] memory) {
    ...
    function cancelOrder(uint256 orderId) external override {
        ...
        require(orders.orders[orderId].status != LimitOrders.LimitOrderStatus.NonExistent, 'LO41');
        require(orders.orders[orderId].status != LimitOrders.LimitOrderStatus.Cancelled, 'LO41');
        if (orders.orders[orderId].status == LimitOrders.LimitOrderStatus.Waiting) {
            orders.orders[orderId].status = LimitOrders.LimitOrderStatus.Cancelled;
            performRefund(orderId);
            forgetOrder(orderId);
        } else {
            ITwapDelay(orders.delay).cancelOrder(orders.delayOrderIdMapping[orderId]);
            forgetOrder(orderId);
        }
    }

    function getDelayOrderId(uint256 orderId) external view override returns (uint256) {
        return orders.delayOrderIdMapping[orderId];
    }

    function performRefund(uint256 orderId) internal {
        LimitOrders.StoredOrders memory order = orders.orders[orderId];
        LimitOrders.LimitOrderType orderType = order.orderType;

        if (orderType == LimitOrders.LimitOrderType.Sell) {
            address token0 = order.sellParams.tokenIn;
            address token1 = order.sellParams.tokenOut;
            address to = order.to;
            require(refundToken(order.inverse ? token1 : token0, to, order.share, order.sellParams.wrapUnwrap), 'LO14');
            uint256 value = order.gasPrice.mul(order.sellParams.gasLimit);
            require(refundEth(payable(to), value), 'LO40');
        } else if (orderType == LimitOrders.LimitOrderType.Buy) {
            address token0 = order.buyParams.tokenIn;
            address token1 = order.buyParams.tokenOut;
            address to = order.to;
            require(refundToken(order.inverse ? token1 : token0, to, order.share, order.buyParams.wrapUnwrap), 'LO14');
            uint256 value = order.gasPrice.mul(order.buyParams.gasLimit);
            require(refundEth(payable(to), value), 'LO40');
        }
    }
    ...
    function forgetOrder(uint256 orderId) internal {
        ...
        delete orders.delayOrderIdMapping[orderId];
        uint256[] memory myOrders = orders.userOrders[msg.sender];
        for (uint256 i = myOrders.length - 1; i >= 0; --i) {
            if (myOrders[i] == orderId) {
                delete orders.userOrders[msg.sender][i];
                break;
            }
        }
    }
    ...
    function refundToken(
        ...
        uint256 gas = ITwapDelay(orders.delay).getTransferGasCost(token);
        (bool success, bytes memory data) = address(this).call{ gas: gas }(
...
interface ITwapLimitOrder {
    ...
    event OrderExecuted(uint256 indexed id, uint256 gasSpent, uint256 ethRefunded);
    ...
    function getOrderStatus(uint256 _orderId) external returns (LimitOrders.LimitOrderStatus);
    ...
    function getOrder(uint256 orderId) external view returns (LimitOrders.StoredOrders memory order);

    function getOrders(address user) external view returns (uint256[] memory);
}
