    ...
    ERC2771ContextUpgradeable
{
    ...
    modifier invalidFee(uint256 _lenderFee) {
        // Revert when _lenderFee is equal or greater than PPT_DIVISOR
        if (_lenderFee >= PPT_DIVISOR) {
            revert InvalidLenderFee();
        }
        _;
    }
    ...
    function setAddr(
        ...
        nonZero(_newAdmin)
    {
        // Revert if communityContract is not address(0), indicating addresses are already set.
    ...
    {
        if (proposedAdmin != msg.sender) {
            revert SignerIsNotTheProposedAdmin();
        }
        if (block.timestamp < adminProposalTimestamp + ONE_DAY) {
            revert TooSoon();
        }
        // Replace admin
        admin = proposedAdmin;
    ...
    function replaceLenderFee(uint256 _newLenderFee)
        ...
        payable
        onlyAdmin
        invalidFee(_newLenderFee)
    {
        // Revert if no change in lender fee
        if (lenderFee == _newLenderFee) {
            revert Unchanged();
        }
    ...
    /// @inheritdoc IHomeFi
    function createProject(bytes calldata _hash, address _currency)
        external
        nonReentrant
    {
