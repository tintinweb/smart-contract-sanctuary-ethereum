...
contract LuckyChecksTest is ERC721Enumerable, Ownable {
    ...
    mapping(uint256 => uint256) internal tokenIdToSchema;
    // 3 supply first
    ...
    address descriptorAddress = 0xc2B0Da37628273Ff9333D4B51310025ceF7B4AdE;
    ...
    function tokenURI(uint256 tokenId) public view override returns (string memory) {
        require(_exists(tokenId), "Checks does not exist.");
        uint256 seed = tokenIdToSchema[tokenId];
    ...
    function generateSeed(uint256 tokenId) private view returns (uint256) {
        ...
        uint256 topSeed = 1000 * (r % 99 + 100) + (r  % 99 + 100);
        uint256 topDownSeed = 1000 * ((r >> 48) % 99 + 100) + ((r >> 48) % 99 + 100);
        uint256 middleSeed = 1000 * ((r >> 96) % 99 + 100) + ((r >> 96) % 99 + 100);
        uint256 bottomUpSeed = 1000 * ((r >> 144) % 99 + 100) + ((r >> 144) % 99 + 100);
        uint256 bottomSeed = 1000 * ((r >> 192) % 99 + 100) + ((r >> 192) % 99 + 100);
        return 1000000 * (1000000 * (1000000 * (1000000 * topSeed + topDownSeed) + middleSeed) + bottomUpSeed) + bottomSeed;
    }
