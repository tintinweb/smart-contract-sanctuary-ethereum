...
// internal imports
import {IInfinityExchangeAdmin} from '../interfaces/IInfinityExchangeAdmin.sol';

/**
@title InfinityExchange
@author nneverlander. Twitter @nneverlander
@notice The admin contract for the Infinity Exchange. Used for admin functions like withdrawing fees, adding complications etc
*/
contract InfinityExchangeAdmin is Ownable, IInfinityExchangeAdmin  {
  /// @notice Address of the Infinity Exchange contract
  address public infinityExchange;
  ...
  event NewInfinityExchange(address infinityExchange);
  event CurrencyAdded(address currency);
  event CurrencyRemoved(address currency);
  event ComplicationAdded(address complication);
  event ComplicationRemoved(address complication);
  event ERC20Withdrawn(address destination, address currency, uint256 amount);

  // ===================================================== ADMIN FUNCTIONS ======================================================

  /**
    @param _infinityExchange address of the Infinity Exchange contract
   */
  function setInfinityExchange(address _infinityExchange) external onlyOwner {
    infinityExchange = _infinityExchange;
    emit NewInfinityExchange(_infinityExchange);
  }
  ...
  function withdrawETH(address destination) external onlyOwner {
    (bool sent, ) = destination.call{value: address(infinityExchange).balance}('');
    require(sent, 'failed');
  }
  ...
  function withdrawTokens(
    ...
    IERC20(currency).transferFrom(infinityExchange, destination, amount);
    emit ERC20Withdrawn(destination, currency, amount);
  }

  /// @dev adds a new transaction currency to the exchange
  function addCurrency(address _currency) external onlyOwner {
    require(_currency != address(0), 'address cannot be 0');
    IInfinityExchangeAdmin(infinityExchange).addCurrency(_currency);
  ...
  function addComplication(address _complication) external onlyOwner {
    require(_complication != address(0), 'address cannot be 0');
    IInfinityExchangeAdmin(infinityExchange).addComplication(_complication);
    emit ComplicationAdded(_complication);
  }

  /// @dev removes a transaction currency from the exchange
  function removeCurrency(address _currency) external onlyOwner {
    IInfinityExchangeAdmin(infinityExchange).removeCurrency(_currency);
  ...
  function removeComplication(address _complication) external onlyOwner {
    IInfinityExchangeAdmin(infinityExchange).removeComplication(_complication);
  ...
  function updateMatchExecutor(address _matchExecutor) external onlyOwner {
    require(_matchExecutor != address(0), 'matchExecutor cannot be 0');
    IInfinityExchangeAdmin(infinityExchange).updateMatchExecutor(_matchExecutor);
    emit NewMatchExecutor(_matchExecutor);
  }

  /// @dev updates the gas units required for WETH transfers
  function updateWethTransferGasUnits(uint32 _newWethTransferGasUnits) external onlyOwner {
    require(_newWethTransferGasUnits <= MAX_WETH_TRANSFER_GAS_UNITS, 'weth transfer gas units too high');
    IInfinityExchangeAdmin(infinityExchange).updateWethTransferGasUnits(_newWethTransferGasUnits);
  ...
  function updateProtocolFee(uint32 _newProtocolFeeBps) external onlyOwner {
    ...
    IInfinityExchangeAdmin(infinityExchange).updateProtocolFee(_newProtocolFeeBps);
...
/**
 * @title IInfinityExchangeAdmin
 * @author nneverlander. Twitter @nneverlander
 * @notice Admin interface that contains all admin functions for Infinity Exchange
 */
interface IInfinityExchangeAdmin {
  function setInfinityExchange(address _infinityExchange) external;
