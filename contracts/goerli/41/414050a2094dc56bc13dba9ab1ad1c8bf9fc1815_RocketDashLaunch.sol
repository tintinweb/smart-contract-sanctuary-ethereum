...
contract RocketBase is ERC721Base("Rocket", "RocketDash") {
        ...
        returns (uint)
    {
        ...
        uint256 newrocketId =  rockets.length - 1;
...
contract RocketMinting is AuctionBase {
    ...
    /// @dev we can create promo rockets, up to a limit. Only callable by owner
    /// @param _owner the future owner of the created rockets. Default to contract owner
    function createPromoRocket(address _owner) external onlyOwner {
        address rocketOwner = _owner;
...
contract RocketDashLaunch is RocketMinting{

    constructor (string memory _baseUri) {

        _createRocket(0, 0, 0, address(0));
