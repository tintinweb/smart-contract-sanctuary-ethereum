...
library RentalStorageLib {
    ...
    function getLeftCollateral(
        ...
        if (s.cs._collection[_collection]._users[_tokenId].renting == true) {
            ...
            if (
                rentingexpires + 7200 <= block.timestamp &&
                block.timestamp <= rentingexpires + 604800
            ) {
                uint256 overDueTime = block.timestamp - rentingexpires;
                uint256 leftCollateral = collateral * (1 - (overDueTime / 604800));
                return (leftCollateral, overDueTime, leftCollateral);
    ...
    function getClaimableCollateral(
        ...
        if (s.cs._collection[_collection]._users[_tokenId].renting == true) {
            ...
            if (
                ...
                uint256 overDueTime = block.timestamp - rentingexpires;
                uint256 collateralToClaim = collateral * (overDueTime / 604800);
                return (collateralToClaim - collateralClaimed, collateralToClaim, overDueTime);
            } else if (block.timestamp > rentingexpires + 604800) {
                return (collateral - collateralClaimed, 0, 0);
            } else {
                return (0, 1, 1);
            }
        } else {
