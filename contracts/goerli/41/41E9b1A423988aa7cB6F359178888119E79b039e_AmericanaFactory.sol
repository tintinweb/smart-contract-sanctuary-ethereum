...
contract AmericanaFactory is Initializable, ReentrancyGuard {
    ...
    modifier onlyAuthorized {
      require(isAuthorized[msg.sender], "AmericanaFactory: Unauthorized");
      _;
   }
    modifier notPaused {
      require(paused == false, "AmericanaFactory: Contract currently paused");
      _;
   }
    ...
    struct Listing {
        ...
        uint72 price;// max 4726 ether ~= $10million
        uint24 itemID;// 16,777,215 possibilities
    }

    struct ListingExpensive {
        // 2 slots, used for more expensive item sales
        address payable seller;
        uint256 price;// no max price
        uint24 itemID;//16,777,215 possibilities
    }
    ...
    event NewListingCreated(uint24 _itemId, address _seller, uint256 _amount);
    ...
    function initialize(address payable _feeRecipient, uint8 _maxEscrowDurationInDays, uint8 _marketFee, address _minterAddress) public initializer{
 
        ReentrancyGuard.reentrancyInitialize();
    ...
    function createNewListing(uint24 _itemId, uint72 _price) external notPaused {
        require(listingsByID[_itemId].price == 0, "AmericanaFactory: Item already exists");
        require(_price > 0, "AmericanaFactory: 0 price");
                                                
        listingsByID[_itemId] = Listing(payable(msg.sender), _price, _itemId);
        emit NewListingCreated(_itemId, msg.sender, _price);
    }

    function createEscrow(uint24 _itemid, bool _wantsNFT) external payable notPaused {
        require(listingsByID[_itemid].seller != payable(address(0)), "AmericanaFactory: Item not active.");
        require(escrowByID[_itemid].buyer == payable(address(0)), "AmericanaFactory: Item already in escrow.");
        require(listingsByID[_itemid].price == msg.value, "AmericanaFactory: Improper escrow amount.");
    ...
    function mint(uint24 _itemid) external notPaused {
        require(availableToMint[_itemid] == true, "AmericanaFactory: Escrow not eligible for mint");
    ...
    function withdrawEscrowFunds(uint24 _itemid) external {        
        require(escrowByID[_itemid].buyer == msg.sender,"AmericanaFactory: You're not entitled to indicated escrow funds");
        require(block.timestamp >= escrowByID[_itemid].timestamp + maxEscrowDuration, "AmericanaFactory: Escrow withdrawal not yet elligible");

        address payable buyer = escrowByID[_itemid].buyer;
        escrowByID[_itemid].buyer = payable(address(0));

        (bool success, ) = buyer.call{value: listingsByID[_itemid].price}("");
        require(success, "AmericanaFactory: Failed to withdraw ether");

        emit EscrowFundsWithdrawal(_itemid,listingsByID[_itemid].price, buyer);
    }
    ...
    function approveItem(uint24 _itemid) external onlyAuthorized nonReentrant{
        require(listingsByID[_itemid].seller != payable(address(0)), "AmericanaFactory: Item not active.");
        require(escrowByID[_itemid].buyer != payable(address(0)),"AmericanaFactory: Escrow not active");
        uint256 fee = uint256(marketFee)*uint256(listingsByID[_itemid].price)/1000;
        pendingFees+=fee;

        (bool success, ) = listingsByID[_itemid].seller.call{value: listingsByID[_itemid].price - fee}("");
        require(success, "AmericanaFactory: Failed to withdraw ether after approval");
    ...
    function rejectItem(uint24 _itemid) external onlyAuthorized nonReentrant{
        ...
        require(success, "AmericanaFactory: Failed to withdraw ether after rejection");
    ...
    //struct getters
    function getEscrowByID(uint24 _itemid) public view returns(Escrow memory){
        return escrowByID[_itemid];
    }
    function getListingByID(uint24 _itemid) public view returns(Listing memory){
        return listingsByID[_itemid];
    }
    //fee stuff
    function changeMarketFee(uint8 newFee) external onlyAuthorized{
    ...
    function widthrawFees() external onlyAuthorized{
