...
contract Exchange is Ownable{
    ...
    function fillOrder(uint256 _id) public payable{
        ...
        _Order storage _order = orders[_id];
        address tokenGive = _order.tokenGive;
        uint256 amountGive = _order.amountGive;
        bool isNativeHolder = isNativeTokenHolder(msg.sender);
        uint256 feeAmount = 0;
        if(isNativeHolder) {
            feeAmount = amountGive.mul(feeNum).div(feeDen);
        } else {
            feeAmount = amountGive.mul(feeNativeNum).div(feeDen);
        }
        if(tokenGive == ETHER || tokenGive == WETHER) {
            require(msg.value >= feeAmount, 'insufficient ETH');
            tokenBalances[ETHER][feeAccount] = tokenBalances[ETHER][feeAccount].add(feeAmount);
        } else {
            uint256 ethAmount = countEtherFromToken(tokenGive, feeAmount);
        ...
        _trade(_order.id, _order.user, _order.tokenGet, _order.amountGet, _order.tokenGive, amountGive.sub(feeAmount));
