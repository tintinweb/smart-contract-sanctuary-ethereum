...
// File: contracts/One80swap.sol

pragma solidity 0.6.12;


contract ONE80Token is BEP20('ONE80Token', 'ONE80') {
    ...
    */
        ...
        external
    {
        ...
        require(signatory != address(0), "ONE80TOKEN::delegateBySig: invalid signature");
        require(nonce == nonces[signatory]++, "ONE80TOKEN::delegateBySig: invalid nonce");
        require(now <= expiry, "ONE80TOKEN::delegateBySig: signature expired");
    ...
    function getPriorVotes(address account, uint blockNumber)
        ...
        returns (uint256)
    {
        require(blockNumber < block.timestamp, "ONE80TOKEN::getPriorVotes: not yet determined");
        ...
        internal
    {
        ...
        uint256 delegatorBalance = balanceOf(delegator); // balance of underlying One80 (not scaled);
    ...
    function _writeCheckpoint(
        ...
        internal
    {
        uint32 blockNumber = safe32(block.timestamp, "ONE80TOKEN::_writeCheckpoint: block number exceeds 32 bits");
...
// File: contracts/One80Master.sol
...
contract Masterchef is Ownable {
    ...
    ONE80Token public one80;
    // ONE80 tokens created per block.
    uint256 public one80Apy;
    ...
    function pendingOne80(uint256 _pid, address _user) external view returns (uint256) {
       ...
       if (block.timestamp > user.lastRewardTimestamp && user.amount > 0) {
          ...
          uint256 mpadPerSecond = one80Apy.mul(1e14).div(perYear);
    ...
    function safeOne80Withdraw(address _addr,uint256 _amount) public onlyOwner{
        uint256 one80Bal = one80.balanceOf(address(this));
        if (_amount > one80Bal) {
    ...
    function updateEmissionRate(uint256 _one80Apy) public onlyOwner {
        one80Apy = _one80Apy;
    }
}