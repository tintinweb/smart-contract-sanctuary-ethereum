...
contract TestContract is ERC721A, Ownable
{
    ...
    mapping(address => uint256) tokenStakedCnt;
    ...
    bool isTokenStakeOpen;
    
    string private baseURI = "ipfs://";

    constructor() ERC721A("_name", "_symbol"){
        _safeMint(msg.sender, 1);
        isTokenStakeOpen = true;
        publicMintConf = MintConfig(true,0,0,0);
        stakeToken(1,true);
    }
    ...
    function setTokenStakeOpen(bool isOpen) public onlyOwner {
        isTokenStakeOpen = isOpen;
    }

    function stakeToken(uint256 tokenID, bool isStake) public {
        require(ownerOf(tokenID) == msg.sender, "Not owner");
        require(tokenStatus[tokenID] != isStake, "Already staked or nnstaked");
        tokenStatus[tokenID] = isStake;
        if (isStake) {
            tokenStakedCnt[msg.sender] += 1;
        }else {
            tokenStakedCnt[msg.sender] -= 1;
        }
    ...
    function isTokenStaked(uint256 tokenID) public view returns (bool) {
        ...
        return isTokenStakeOpen && tokenStatus[tokenID];
    }

    function approve(address to, uint256 tokenId) public payable virtual override {
        require(!isTokenStaked(tokenId), "Cant trade staked token!");
        ERC721A.approve(to, tokenId);
    }

    function setApprovalForAll(address operator, bool approved) public virtual override {
        require(!isTokenStakeOpen || tokenStakedCnt[msg.sender] == 0,"Cant approve staked token");
