...
// @creator: g56d
// @dev: bortch, g56d
// @notice: Primes is a token contract that contains methods to properly interact with ERC721SeaDrop

contract Primes is ERC721SeaDrop {
    using Strings for uint256;
    using PrimesUtils for *;

    uint256 private constant MAX_SUPPLY = 1575;
    uint256 private constant RESERVE = 75;
    uint256 private constant MAX_PER_ADDRESS = 5;
    uint public allowlistPrice = 0.069 ether;
    uint public publicPrice = 0.096 ether;
    uint16[] private serialUL;
    ...
    function setSerialUL(uint16[] calldata _serialUL) public onlyOwner {
        delete serialUL;
        serialUL = _serialUL;
    }
    ...
    function tokenURI(
        ...
        require(serialUL.length > 0, "Primes: missing serial");

        uint i = serialUL[_tokenId];
        string
        ...
        string memory dataImage = generateBase64SVG(i);
        uint primesCount = PrimesUtils.getNumberOfPrimeNumbers(i);
        return
            string(
                abi.encodePacked(
                    ...
                    Base64.encode(
                        bytes(
                            abi.encodePacked(
                                '{"name":"',
                                string(
                                    abi.encodePacked("Primes #", i.toString())
                                ),
                                '", "description":"',
                                description,
                                '", "image_data": "',
        ...
        for (y = 0; y <= 800; y += 100) {
            for (x = 0; x <= 800; x += 100) {
                ...
                if (PrimesUtils.isPrime(uint256(count))) {
                    string memory d = PrimesUtils.setPath(x, y);
                    string memory values = PrimesUtils.setValues(x, y);
...
library PrimesUtils {
     ...
     */
    function getNumberOfPrimeNumbers(
        uint256 _i
    ...
    function setPath(
        ...
        string memory path = "";
        uint256 value = y * 9 + x;
        if ((value > 300) && ((value + 100) % 300 == 0)) {
            return path;
        }
        path = concatUintPathValues(x, y, (x + 100), (y + 100), x);
        return path;
    }

    /**
     * @notice Concat uint256 to a Path string
     * @param m M
     * @param h H
     * @param v V
     * @param h2 H
     * @param z Z
     * @return path as string in format "M000 000H000V000H000Z"
     */
    function concatUintPathValues(
        uint256 m,
        uint256 h,
        uint256 v,
        uint256 h2,
    ...
    /**
     * @notice Concat int256 to a Path string (negative values allowed)
     * @param m M
     * @param h H
     * @param v V
     * @param h2 H
    ...
    function concatIntPathValues(
        ...
        // pattern: M000 000H 000V 000H 000Z
        return
            string.concat(
                "M",
                intToString(m),
     ...
     * @param value int256 value
     * @return string negative values are indicated by a leading "-"
     */
    function intToString(int256 value) internal pure returns (string memory) {
        return string.concat(value < 0 ? "-" : "", abs(value).toString());
    }

    function setMinValuesAnimation(
        uint x,
        uint y
    ) internal pure returns (string memory minV) {
        if (x == 200 && y == 0) {
            return concatUintPathValues(x, y, x + 50, y + 50, x);
        } else {
            if (x % 300 == 0) {
                if (y % 200 == 0) {
                    return
                        concatUintPathValues(
                            x + 50,
                            y,
                            x + 100,
                            y + 50,
            ...
            } else if (x == 100 || x == 400 || x == 700) {
                if (y % 200 == 0) {
                    return concatUintPathValues(x, y, x + 50, y + 50, x);
                } else {
                    return concatUintPathValues(x, y + 50, x + 50, y + 100, x);
                }
            }
        }
    }

    function setMaxValuesAnimation(
        uint _x,
        uint _y
    ) internal pure returns (string memory maxV) {
        int256 x = int256(_x);
        int256 y = int256(_y);

        if (x == 200 && y == 0) {
            return concatIntPathValues(x, y, x + 150, y + 150, x);
        } else {
            if (x % 300 == 0) {
                if (y % 200 == 0) {
                    return
                ...
                if (y % 200 == 0) {
                    ...
                    return concatIntPathValues(x, y - 50, x + 150, y + 100, x);
                }
            }
        }
    }
    ...
    function setValues(uint x, uint y) internal pure returns (string memory) {
        string memory mn = setMinValuesAnimation(x, y);
        string memory mx = setMaxValuesAnimation(x, y);
        bytes memory tempMinTest = bytes(mn);
        bytes memory tempMaxTest = bytes(mx);
        if (tempMinTest.length == 0 || tempMaxTest.length == 0) {
            revert("Primes: something wrong with values");
        } else {
            return string(abi.encodePacked(mx, ";", mn, ";", mx));
        }
    }
}
