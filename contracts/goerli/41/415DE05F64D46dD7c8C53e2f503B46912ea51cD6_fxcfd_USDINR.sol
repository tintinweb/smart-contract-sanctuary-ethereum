...
contract fxcfd_USDINR {
    ...
    function complete_trade() public payable returns (bool) {
        require(
            msg.sender == buy_addr || msg.sender == owner,
            "Not Authorized"
        );
        if (buyer_amount > im) {
            payable(buy_addr).transfer(im);
            emit Transfer(address(this), buy_addr, im);
        } else {
            payable(buy_addr).transfer(buyer_amount);
            emit Transfer(address(this), buy_addr, buyer_amount);
            owner_amount = im - buyer_amount;
