    ...
    IERC2981Upgradeable
{
    ...
    function initialize() public initializerERC721A initializer {
        __ERC721A_init("Section21Fos", "FOS");
    ...
    function batchSetTokenURI(uint256[] memory tokenIds, string[] memory uris) external onlyOwner {
        require(tokenIds.length == uris.length, "Not same length.");
        for (uint256 i = 0; i < tokenIds.length; i++) {
            tokenIdToURI[tokenIds[i]] = uris[i];
        }
    }
    ...
    function publicMint(
        uint256 quantity
    ) external payable whenPublicMintNotPaused checkMintLimit(quantity) checkPay(PUBLIC_PRICE, quantity) {
    ...
    uint256 public constant ALLOWLISTED_OWNER_MINT_LIMIT = 2;
    ...
    uint256 public constant ALLOWLIST_BONUS_PER = 1000;
