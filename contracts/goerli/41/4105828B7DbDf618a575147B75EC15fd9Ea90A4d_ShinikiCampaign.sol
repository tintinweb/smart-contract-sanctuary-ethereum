    ...
    PausableUpgradeable
{
    ...
    // Info collection
    mapping(address => mapping(uint256 => uint256)) internal collectionDetail;

    // Info colelction index for each token
    ...
    function claimCampaign(
        ...
        for (uint256 i = 0; i < campaignIds.length; i++) {
            require(
                !claimed[msg.sender][campaignIds[i]],
                "ShinikiCampaign: campaign is claimed"
            );
            claimed[msg.sender][campaignIds[i]] = true;
        }
        for (uint256 i = 0; i < tokens.length; i++) {
            uint256 tokenId = collectionDetail[tokens[i]][
                currentIndex[tokens[i]]
            ];
            while(amounts[i] != 0) {
                if (
                    IERC165Upgradeable(tokens[i]).supportsInterface(IID_IERC721)
                ) {
                    // while(IERC721Upgradeable(tokens[i]).ownerOf(tokenId) != address(this)) {
                    //     currentIndex[tokens[i]]++;
                    //     tokenId = collectionDetail[tokens[i]][currentIndex[tokens[i]]];
                    // }
                    IERC721Upgradeable(tokens[i]).safeTransferFrom(
                        address(this),
                        msg.sender,
                        tokenId,
                        ""
                ...
                if (
                    IERC165Upgradeable(tokens[i]).supportsInterface(
                        IID_IERC1155
                    )
                ) {
                    // while(IERC1155Upgradeable(tokens[i]).balanceOf(address(this), tokenId) == 0) {
                    //     currentIndex[tokens[i]]++;
                    //     tokenId = collectionDetail[tokens[i]][currentIndex[tokens[i]]];
                    // }
                    IERC1155Upgradeable(tokens[i]).safeTransferFrom(
                        address(this),
                        msg.sender,
                        tokenId,
                        amountERC1155s,
        ...
        emit Claimed(msg.sender, tokens, amounts, amountERC1155s, campaignIds, nonce);
    }
        ...
        onlyOwner
    {
        ...
        for (uint256 i = 0; i < tokenIds.length; i++) {
            collectionDetail[token][collectionIndex[token]] = tokenIds[i];
    ...
    function depositeERC1155(
        ...
        require(
            tokenIds.length != 0 && amounts.length != 0,
            "ShinikiCamPaign: input is not zero"
        );
        require(
    ...
    OwnableUpgradeable
{
        ...
        returns (bool)
    {
        address signer = keccak256(
            abi.encode(receiver, tokens, amounts, amountERC1155s, campaignIds, nonce)
        ).ethSignedMessage().recover(signature);
        require(signer == TRUSTED_PARTY, "ShinikiCampaign: Invalid signature claim NFT");
