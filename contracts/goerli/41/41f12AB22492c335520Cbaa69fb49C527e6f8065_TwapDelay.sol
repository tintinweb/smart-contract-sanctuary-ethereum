...
contract TwapDelay is ITwapDelay {
    ...
    function execute(Orders.Order[] calldata _orders) external payable override lock {
        uint256 ordersLen = _orders.length;
        uint256 gasBefore = gasleft();
        bool orderExecuted;
        bool senderCanExecute = isBot[msg.sender] || isBot[address(0)];
        for (uint256 i; i < ordersLen; ++i) {
...
interface ITwapDelay {
    ...
    function retryRefund(Orders.Order calldata orders) external;

    function cancelOrder(Orders.Order calldata orders) external;
}
