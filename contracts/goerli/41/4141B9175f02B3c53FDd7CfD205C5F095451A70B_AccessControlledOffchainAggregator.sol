    ...
    AggregatorV2V3Interface
{
   ...
   */
     ...
     */
    function validator() external view returns (AggregatorValidatorInterface) {
        return s_validator;
    }
   ...
   */
    ...
    /**
     * @notice mocked mannually update answer to the contract
     */
    function mockUpdateAnswer(int192 median) public onlyOwner {
        s_hotVars.latestAggregatorRoundId++;
        s_transmissions[s_hotVars.latestAggregatorRoundId] = Transmission(
            median,
            uint64(block.timestamp)
        );
...
contract OffchainAggregatorBilling is Owned {
    ...
    uint256 private constant maxUint16 = (1 << 16) - 1;
    uint256 internal constant maxUint128 = (1 << 128) - 1;
    ...
    function setBillingAccessControllerInternal(
        AccessControllerInterface _billingAccessController
    ) internal {
    ...
    function withdrawPayment(address _transmitter) external {
        require(
            msg.sender == s_payees[_transmitter],
    ...
    function payOracles() internal {
        Billing memory billing = s_billing;
        uint16[maxNumOracles]
            memory observationsCounts = s_oracleObservationsCounts;
        uint256[maxNumOracles]
            memory gasReimbursementsLinkWei = s_gasReimbursementsLinkWei;
        ...
        for (
            ...
            uint256 reimbursementAmountLinkWei = gasReimbursementsLinkWei[
                transmitteridx
            ] - 1;
            uint256 obsCount = observationsCounts[transmitteridx] - 1;
            uint256 linkWeiAmount = obsCount *
                uint256(billing.linkGweiPerObservation) *
                (1 gwei) +
                reimbursementAmountLinkWei;
            if (linkWeiAmount > 0) {
                address payee = s_payees[transmitters[transmitteridx]];
                // Poses no re-entrancy issues, because LINK.transfer does not yield
                // control flow.
                require(
                    LINK.transfer(payee, linkWeiAmount),
                    "insufficient funds"
                );
    ...
    ) internal pure returns (uint16[maxNumOracles] memory) {
        // reward each observer-participant with the observer reward
        for (uint256 obsIdx = 0; obsIdx < observers.length; obsIdx++) {
            uint8 observer = uint8(observers[obsIdx]);
            observations[observer] = saturatingAddUint16(
    ...
    function transmitterGasCostEthWei(
        ...
        uint256 gasLeft
    ) internal pure returns (uint128 gasCostEthWei) {
        require(initialGas >= gasLeft, "gasLeft cannot exceed initialGas");
        uint256 gasUsed = initialGas - // gas units
            gasLeft + // observed gas usage
            callDataCost +
            accountingGasCost; // estimated gas usage
    ...
    function withdrawFunds(address _recipient, uint256 _amount) external {
        ...
        require(
            LINK.transfer(_recipient, min(linkBalance - linkDue, _amount)),
            "insufficient funds"
        );
    }

    // Total LINK due to participants in past reports.
    function totalLINKDue() internal view returns (uint256 linkDue) {
        ...
        uint16[maxNumOracles]
            memory observationCounts = s_oracleObservationsCounts;
        for (uint256 i = 0; i < maxNumOracles; i++) {
    ...
    function reimburseAndRewardOracles(
        ...
        s_oracleObservationsCounts = oracleRewards(
            observers,
            s_oracleObservationsCounts
        );
        // Reimburse transmitter of the report for gas usage
        require(
            txOracle.role == Role.Transmitter,
        ...
        // uint32 (128+32 < 256!).
        uint256 gasCostLinkWei = (gasCostEthWei * billing.microLinkPerEth) /
            1e6;
        ...
        s_gasReimbursementsLinkWei[txOracle.index] =
            s_gasReimbursementsLinkWei[txOracle.index] +
            gasCostLinkWei +
            uint256(billing.linkGweiPerTransmission) *
            (1 gwei); // convert from linkGwei to linkWei
    ...
    function setPayees(
        ...
        require(
            _transmitters.length == _payees.length,
            "transmitters.size != payees.size"
        );

        for (uint256 i = 0; i < _transmitters.length; i++) {
     ...
     */
    function transferPayeeship(address _transmitter, address _proposed)
        external
    {
        require(
            msg.sender == s_payees[_transmitter],
        ...
        if (previousProposed != _proposed) {
            emit PayeeshipTransferRequested(
                _transmitter,
                msg.sender,
                _proposed
            );
    ...
    function acceptPayeeship(address _transmitter) external {
        require(
            msg.sender == s_proposedPayees[_transmitter],
