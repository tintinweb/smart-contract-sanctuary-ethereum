...
contract FxCustomERC20RootTunnel is FxBaseRootTunnel, Create2 {
    using SafeERC20 for IERC20;
    // maybe DEPOSIT and MAP_TOKEN can be reduced to bytes4
    bytes32 public constant DEPOSIT = keccak256("DEPOSIT");
    bytes32 public constant MAP_TOKEN = keccak256("MAP_TOKEN");
    bytes32 public constant MAP_TOKEN_WITHOUT_DEPLOY = keccak256("MAP_TOKEN_WITHOUT_DEPLOY");
    ...
    function mapTokenWithoutDeploy(address rootToken, address childToken) public {
        // check if token is already mapped
        require(rootToChildTokens[rootToken] == address(0x0), "FxERC20RootTunnel: ALREADY_MAPPED");

        // MAP_TOKEN, encode(rootToken, name, symbol, decimals)
        bytes memory message = abi.encode(MAP_TOKEN_WITHOUT_DEPLOY, abi.encode(rootToken, childToken));
        _sendMessageToChild(message);
