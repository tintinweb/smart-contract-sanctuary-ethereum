...
// File @openzeppelin/contracts-upgradeable/token/ERC721/[email protected].2
...
// File contracts/IERC721JUpgradeable.sol


// !! THIS FILE WAS AUTOGENERATED BY abi-to-sol v0.6.6. !!
// It was then trimmed into parts.


interface IERC721JUpgradeable {
    ...
    function amountOfSongs() external view returns (uint256);

    //Returns the generation / rarity of a tokenId
    function generationOfToken(uint256 tokenId) external view returns (uint256);
    ...
    function mintOriginal1(string memory songURI1, uint256 maxEditions) external;

    //Mints an original using 2 pieces of metadata
...
// File contracts/IERC721JFullUpgradeable.sol
...
interface IERC721JFullUpgradeable is IERC721JUpgradeable {
    ...
    //Returns the staking status for a token
    ...
    //Lets you "stake" and "unstake" a token from the public mint store.
    function togglePublicMint(uint256 tokenId) external;
...
// File contracts/IERC721JFreeClaimsUpgradeable.sol
...
interface IERC721JFreeClaimsUpgradeable is IERC721JUpgradeable {

    //Emits when a free claim is set
    event SetFreeClaim(
    ...
    //Takes a contract address and returns (if one exists) the free claim set for that contract
    function claimCheck(address _contract)
        external
        view
        returns (uint256 _songIdOut, uint256 _generationOut);

    //Takes a contract address, song and generation and returns (if one exists) the free claim set for that specific set of tokens on that contract
    function claimCheckSpecific(
        address _contract,
        uint256 songIdIn,
        uint256 generationIn
    ) external view returns (uint256 _songIdOut, uint256 _generationOut);

    //Mints a free token using the contract wide setting
    function freeClaim(address _contract, uint256 tokenId) external;

    //Mints a free token using the setting that accepts any token of a rarity
    function freeClaimRarityReward(address _contract, uint256 tokenId) external;

    //Mints a free token using the setting that accepts any token of a songId
    function freeClaimSongReward(address _contract, uint256 tokenId) external;

    //Mints a free token using the setting that needs matching songId and generation to be accepted
    function freeClaimSpecific(address _contract, uint256 tokenId) external;

    //Returns if a token from an address has claimed it's free token for the contract wide reward
    function giftClaimCheck(address _contract, uint256 tokenId)
        external
        view
        returns (bool);

    //Returns if a token from an address has claimed it's free token for the specific reward
    function giftClaimCheckSpecific(address _contract, uint256 tokenId)
        external
        view
        returns (bool);

    //Sets a free claim for a contract wide reward.  Any token from an ERC721 contract gets a reward.
    ...
    //Sets a free claim for a ERC721J specific reward.  Any token from an ERC721J contract can get a reward based on it's song, rarity, or both.
...
// File contracts/IERC721JEnumerableUpgradeable.sol
...
//Added recycle to burn 2 songs to mint 1. Gave recycling an opt out option.
...
//IERC721J for the main parts, IERC721JFull for the full feature set essentials, IERC721JFreeClaims for the free claim system, IERC721JEnumerable for the indexes
//
//
//
contract ERC721J is
    ...
    OwnableUpgradeable
{
    ...
    event NameChange(string oldName);

    event SymbolChange(string oldSymbol);
        ...
        returns (bool)
    {
        return
            ...
            interfaceId == type(IERC721JFreeClaimsUpgradeable).interfaceId ||
    ...
    function _setSongURI1(uint256 songId, string memory songURI1)
        internal
        virtual
    {
    ...
    function setSongURIs(
        ...
        emit NewSongURI(songId, songGeneration1);
        emit NewSongURI(songId, songGeneration2);
    }
    ...
    //Returns if recycle minting has been disabled
    function recycleDisabled() public view virtual returns (bool) {
        return _disableRecycle;
    }

    //Toggles recycle mint
    function toggleRecycleMint() public virtual onlyOwner {
        _disableRecycle = !_disableRecycle;
    }
    ...
    function mintOriginal1(string memory songURI1, uint256 maxEditions)
        public
        onlyOwner
    {
    ...
    // Mint Master token and set 2 pieces of metadata
    function mintOriginal2(
        string memory songURI1,
        string memory songURI2,
        uint256 maxEditions
    ...
    function freeClaim(address _contract, uint256 tokenId) public {
        ...
        //requires the current amount of copies for that song to be less than what's set
...
// File contracts/IContractWizard.sol


// !! THIS FILE WAS AUTOGENERATED BY abi-to-sol v0.6.6.
// Then it was trimmed down to this.


interface IContractWizard {
    ...
    //Returns the contract address for a contractId
    function getContractByID(uint256 contractId)
        external
        view
        returns (address);
