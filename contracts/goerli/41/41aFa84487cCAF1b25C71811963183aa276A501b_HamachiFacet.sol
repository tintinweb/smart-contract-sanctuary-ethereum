...
import {RewardToken} from "../types/reward/RewardStorage.sol";
import {LibHamachi} from "../libraries/LibHamachi.sol";
import {WithRoles} from "../libraries/LibAccessControl.sol";
import {DEFAULT_ADMIN_ROLE, EXCLUDED_FROM_MAX_WALLET_ROLE, EXCLUDED_FROM_FEE_ROLE, PROCESS_FEE_ROLE, VESTING_ROLE, EXCLUDED_FROM_REWARD_ROLE} from "../types/access/AccessControlStorage.sol";
...
//  WithReward,
contract HamachiFacet is WithRoles {
  using Strings for uint256;

  // ==================== Management ==================== //

  function setLiquidityWallet(address _liquidityWallet) external onlyRole(DEFAULT_ADMIN_ROLE) {
    LibUniswap.DS().liquidityWallet = _liquidityWallet;
  }
  ...
  {
    LibHamachi.DS().fee.liquidityBuyFee = _liquidityBuyFee;
    LibHamachi.DS().fee.rewardBuyFee = _rewardBuyFee;
  }
  ...
  function setSellFee(uint32 _liquiditySellFee, uint32 _rewardSellFee)
    ...
    onlyRole(DEFAULT_ADMIN_ROLE)
  {
    LibHamachi.DS().fee.liquiditySellFee = _liquiditySellFee;
    LibHamachi.DS().fee.rewardSellFee = _rewardSellFee;
  }

  function setIsLpPool(address _pairAddress, bool _isLp) external onlyRole(DEFAULT_ADMIN_ROLE) {
    LibHamachi.DS().lpPools[_pairAddress] = _isLp;
  }

  function setManualClaim(bool _manual) external {
    LibReward.DS().manualClaim[msg.sender] = _manual;
  }

  function setMinBalanceForReward(uint256 newValue) external onlyRole(DEFAULT_ADMIN_ROLE) {
    LibReward.DS().minRewardBalance = newValue;
  }

  function setNumTokensToSwap(uint256 _amount) external onlyRole(DEFAULT_ADMIN_ROLE) {
    LibHamachi.DS().numTokensToSwap = _amount;
  }

  function setProcessingGas(uint32 _gas) external onlyRole(DEFAULT_ADMIN_ROLE) {
    LibHamachi.DS().processingGas = _gas;
  }
  ...
  function setSwapRouter(address _routerAddress, bool _isRouter)
    ...
    onlyRole(DEFAULT_ADMIN_ROLE)
  {
    LibHamachi.DS().swapRouters[_routerAddress] = _isRouter;
  }

  function setDefaultRouter(address _router) external onlyRole(DEFAULT_ADMIN_ROLE) {
    LibUniswap.DS().defaultRouter = _router;
    LibAccessControl.grantRole(EXCLUDED_FROM_MAX_WALLET_ROLE, _router);
  }
  ...
  function setImplementation(address _newImplementation) external onlyRole(DEFAULT_ADMIN_ROLE) {
    LibHamachi.DS().methodsExposureFacetAddress = _newImplementation;
  }

  function setMaxTokenPerWallet(uint256 _maxTokenPerWallet) external onlyRole(DEFAULT_ADMIN_ROLE) {
    LibHamachi.DS().maxTokenPerWallet = _maxTokenPerWallet;
  }

  function excludeFromFee(address _account) external onlyRole(DEFAULT_ADMIN_ROLE) {
    LibAccessControl.grantRole(EXCLUDED_FROM_FEE_ROLE, _account);
  }

  function excludeFromMaxWallet(address _account) external onlyRole(DEFAULT_ADMIN_ROLE) {
    LibAccessControl.grantRole(EXCLUDED_FROM_MAX_WALLET_ROLE, _account);
  }

  function excludeFromReward(address _account) external onlyRole(DEFAULT_ADMIN_ROLE) {
    LibAccessControl.grantRole(EXCLUDED_FROM_REWARD_ROLE, _account);
    LibReward._setBalance(_account, 0);
    LibReward._remove(_account);
  }

  function setProcessRewards(bool _process) external onlyRole(DEFAULT_ADMIN_ROLE) {
    LibHamachi.DS().processRewards = _process;
  }

  function setProcessFeeRole(address _account) external onlyRole(DEFAULT_ADMIN_ROLE) {
    LibAccessControl.grantRole(PROCESS_FEE_ROLE, _account);
  }

  function setVestingContract(address _vestingContract) external onlyRole(DEFAULT_ADMIN_ROLE) {
    LibHamachi.DS().vestingContract = _vestingContract;
  }

  function claimRewards(bool goHami, uint256 expectedOutput) external {
    LibReward._processAccount(msg.sender, goHami, expectedOutput);
  }

  function processFees() external onlyRole(PROCESS_FEE_ROLE) {
    if (LibERC20.balanceOf(address(this)) >= LibHamachi.DS().numTokensToSwap) {
      (uint256 sellIntoETH, uint256 liquidityTokens) = LibHamachi.calculateLiquidifyAmounts(
        LibHamachi.DS().numTokensToSwap
      );
      ...
      // swap tokens
      if (sellIntoETH > 0) {
        LibUniswap.swapTokensForEth(sellIntoETH);
      }
      ...
      // add liquidity
      uint256 liquidityETH = (newBalance * liquidityTokens) / sellIntoETH;

      if (liquidityETH > 0) {
        LibUniswap.addLiquidity(liquidityTokens, liquidityETH);
      }
  ...
  function updateRewardBalance(address account, uint256 balance) public onlyRole(VESTING_ROLE) {
    LibReward.setRewardBalance(account, balance);
  }
  ...
  function setRewardToken(
    ...
    LibReward.setRewardToken(token, router, path, _useV3, pathV3);
    LibHamachi.DS().swapRouters[router] = true;
  }

  // ==================== Views ==================== //

  function liquidityWallet() external view returns (address) {
    return LibUniswap.DS().liquidityWallet;
  }
  ...
  function sellFees() external view returns (uint256, uint256) {
    return (LibHamachi.DS().fee.liquiditySellFee, LibHamachi.DS().fee.rewardSellFee);
  }

  function defaultRouter() external view returns (address) {
    return LibUniswap.DS().defaultRouter;
  }
  ...
  function numTokensToSwap() external view returns (uint256) {
    return LibHamachi.DS().numTokensToSwap;
  }

  function maxTokenPerWallet() external view returns (uint256) {
    return LibHamachi.DS().maxTokenPerWallet;
  }

  function implementation() external view returns (address) {
    return LibHamachi.DS().methodsExposureFacetAddress;
  }
  ...
  function isExcludedMaxWallet(address account) external view returns (bool) {
    return LibAccessControl.hasRole(EXCLUDED_FROM_MAX_WALLET_ROLE, account);
  }
  ...
  {
    account = _account;
    index = LibReward.getIndexOfKey(account);
    if (index < 0) {
      return (account, -1, 0, 0, 0, 0, false);
    }

    uint256 lastProcessedIndex = LibReward.DS().lastProcessedIndex;
    rewardBalance = LibReward.rewardBalanceOf(account);

    numInQueue = 0;
    if (uint256(index) > lastProcessedIndex) {
      numInQueue = index - int256(lastProcessedIndex);
    } else {
      uint256 holders = LibReward.DS().rewardHolders.keys.length;
    ...
    withdrawableRewards = LibReward.withdrawableDividendOf(account);
    totalRewards = LibReward.accumulativeDividendOf(account);
    manualClaim = LibReward.DS().manualClaim[account];
  }
    ...
    )
  {
    ...
    return (rewardToken.token, rewardToken.router, rewardToken.useV3);
  }
  ...
  function getRewardHolders() external view returns (uint256 numHolders) {
    return LibReward.DS().rewardHolders.keys.length;
  }
}
