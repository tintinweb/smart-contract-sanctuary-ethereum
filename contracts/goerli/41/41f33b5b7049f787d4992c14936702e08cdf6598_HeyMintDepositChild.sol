...
2. Test ordering your structs and passing and not passing the refund/fundingDurations and seeing if that changes the gas for deploy
3. Add a way to pass all the blocklisted contracts to blocklist and a way to make that more gas efficient
4. Test if child contracts are upgradeable if you add a function to change the implementation address stored
5. Add back the set approval for all and approve overrides for the blocklist
*/
...
struct ConfigSettings {
    ...
    // The price of a token in the public sale
    uint64 publicPrice;
    // The price of a token in the presale
    uint64 presalePrice;
...
struct AdditionalConfigSettings {
    // If set, amount of seconds from contract deploy in which the fundingTarget must be met or funds are refundable
    uint32 fundingDuration;
    // The amount of ETH that must be raised in fundingDuration seconds since contract deploy or funds are refundable
 ...
 */
contract HeyMintERC721ABase is
    ERC721AUpgradeable,
    ...
    IERC2981Upgradeable
{
    ...
    function initialize(
        ...
        if (additionalConfig.fundingDuration != 0) {
    ...
    modifier originalUser() {
        require(tx.origin == msg.sender, "OPERATOR_NOT_ALLOWED");
        _;
    }
    ...
    function setBaseURI(string calldata _newBaseURI) external onlyOwner {
        require(!metadataFrozen, "NOT_ACTIVE");
    ...
    {
        require(!isBlocklistedOperator(operator), "OPERATOR_NOT_ALLOWED");
    ...
    function approve(address to, uint256 tokenId)
        ...
        override(ERC721AUpgradeable, IERC721AUpgradeable)
    {
        require(!isBlocklistedOperator(to), "OPERATOR_NOT_ALLOWED");
    ...
    function updatePayoutAddressesAndBasisPoints(
        ...
        require(totalBasisPoints == 10000, "INVALID_ARGUMENTS");
        ...
        onlyOwner
    {
        ...
        require(totalSupply() + totalMint <= config.maxSupply, "MINT_TOO_LARGE");
        ...
        originalUser
    {
        ...
        require(totalSupply() + _numTokens <= config.maxSupply, "MINT_TOO_LARGE");
        require(msg.value == config.publicPrice * _numTokens, "INVALID_PRICE_PAID");

        if (additionalConfig.fundingDuration != 0) {
            uint256 firstTokenIdToMint = _nextTokenId();
            for (uint256 i = 0; i < _numTokens; i++) {
              pricePaid[firstTokenIdToMint + i] = config.publicPrice;
            }
        }
     ...
     */
    function setPresalePrice(uint64 _presalePrice) external onlyOwner {
    ...
    function presaleMint(
        ...
        require(msg.value == config.presalePrice * _numTokens, "INVALID_PRICE_PAID");
        require(
        ...
        if (additionalConfig.fundingDuration != 0) {
            uint256 firstTokenIdToMint = _nextTokenId();
            for (uint256 i = 0; i < _numTokens; i++) {
              pricePaid[firstTokenIdToMint + i] = config.presalePrice;
            }
        }
    ...
    function determineFundingSuccess() external {
        require(additionalConfig.fundingDuration > 0, "NOT_ACTIVE");
        require(address(this).balance > additionalConfig.fundingTarget, "NOT_ACTIVE");
    ...
    function burnToRefund(uint256[] calldata _tokenIds) external nonReentrant {
        ...
        require(additionalConfig.fundingDuration > 0, "NOT_ACTIVE");
        require(block.timestamp >= (contractDeployTime + additionalConfig.fundingDuration + 86400), "NOT_ACTIVE");
        require(!fundingTargetReached, "NOT_ACTIVE");
        require(address(this).balance < additionalConfig.fundingTarget, "NOT_ACTIVE");
        ...
        for (uint256 i = 0; i < _tokenIds.length; i++) {
          ...
          require(pricePaid[_tokenIds[i]] > 0, "INVALID_ARGUMENTS");
    ...
    function setBurnClaimState(bool _burnClaimActive) external onlyOwner {
        if (_burnClaimActive) {
            ...
            require(
                config.mintsPerBurn != 0,
                "INVALID_ARGUMENTS"
            );
            require(
                config.burnToMintContractType != 0,
                "INVALID_ARGUMENTS"
            );
            if (config.burnToMintContractType == 2) {
                require(config.burnToMintTokenId != 0, "INVALID_ARGUMENTS");
            }
        }
    ...
    function setBurnToMintContractType(uint8 _burnToMintContractType) external onlyOwner {
        config.burnToMintContractType = _burnToMintContractType;
    }
    ...
    function updateMintsPerBurn(uint8 _mintsPerBurn) external onlyOwner {
        config.mintsPerBurn = _mintsPerBurn;
    }
        ...
        originalUser
    {
        ...
        require(
            config.burnToMintContractType == 1,
            "NOT_ACTIVE");
        require(
            totalSupply() + (_tokenIDs.length * config.mintsPerBurn) <= config.maxSupply,
            "MINT_TOO_LARGE"
        );
        ...
        _safeMint(msg.sender, _tokenIDs.length * config.mintsPerBurn);
    }
    ...
    function burnERC1155ToMint(uint256 _amount) external nonReentrant originalUser {
        require(burnClaimActive, "NOT_ACTIVE");
        require(_amount > 0);
        require(config.burnToMintContractType == 2, "NOT_ACTIVE");
        require(totalSupply() + (_amount * config.mintsPerBurn) <= config.maxSupply, "MINT_TOO_LARGE");
        IERC1155 ExternalERC1155BurnContract = IERC1155(additionalConfig.burnToMintContractAddress);
        require(ExternalERC1155BurnContract.balanceOf(msg.sender, config.burnToMintTokenId) >= _amount, "NOT_OWNED");

        ExternalERC1155BurnContract.safeTransferFrom(msg.sender, burnAddress, config.burnToMintTokenId, _amount, '');
        _safeMint(msg.sender, _amount * config.mintsPerBurn);
    }
    ...
    function updateMintsPerFreeClaim(uint8 _mintsPerFreeClaim) external onlyOwner {
        config.mintsPerFreeClaim = _mintsPerFreeClaim;
    }
        ...
        originalUser
    {
        ...
        require(
            totalSupply() + (_tokenIDs.length * config.mintsPerFreeClaim) <= config.maxSupply,
            "MINT_TOO_LARGE"
        );
        ...
        for (uint256 i = 0; i < _tokenIDs.length; i++) {
            ...
            require(!freeClaimUsed[_tokenIDs[i]], "INVALID_ARGUMENTS");
            freeClaimUsed[_tokenIDs[i]] = true;
        }
        _safeMint(msg.sender, _tokenIDs.length * config.mintsPerFreeClaim);
    }
    ...
    function _beforeTokenTransfers(
        ...
        require(!isBlocklistedOperator(msg.sender), "OPERATOR_NOT_ALLOWED");
        require(
            !stakingActive ||
    ...
    constructor(
        ...
        (bool success, ) = _nftImplementation.delegatecall(
            abi.encodeWithSignature(
                "initialize(address,string,string,(bool,uint8,uint8,uint8,uint8,uint8,uint16,uint16,uint16,uint16,uint64,uint64,string),(uint32,uint128,address,address))",
                msg.sender,
                _name,
                _symbol,
                _config,
