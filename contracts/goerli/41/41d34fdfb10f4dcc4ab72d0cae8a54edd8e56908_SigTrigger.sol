...
//        signer = _signer;
    ...
    //First Succesfull Prototype @ 05:37 - 22/01/2023
    //Flaw = unprotected, no ownership checks, no inherent upgradability - could explore mapping for future extension without contract change
    function upgradeProxy(uint256 tokenId) external {
    rocket.setNewTokenURI(1, tokenId, URI1);
    }

       function upgradeProxyMulti(uint256 tokenId, uint256 skin) external {
           //code to be executed if skin is 1
           if (skin == 1){
            rocket.setNewTokenURI(1, tokenId, URI1);
            }
            //code to be executed if skin is 1
           else if (skin == 2){
            rocket.setNewTokenURI(1, tokenId, URI2);
            }
            //code to be executed if skin is 1
           else if (skin == 2){
            rocket.setNewTokenURI(1, tokenId, URI3);
            }
