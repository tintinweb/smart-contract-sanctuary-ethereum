...
contract OptimisticRelayer is Guarded, IOptimisticRelayer {
        ...
        returns (bool collybusWasUpdated)
    {
        ...
        proposal.proposer = uint160(msg.sender);
        proposal.disputed = 0;
        proposal.proposedValue = uint128(value_);
        proposal.blockNumber = uint128(blockNumber_);
    ...
    function dispute(
        ...
        if (proposals[encodedId_].disputed == 1) {
            revert OptimisticRelayer__dispute_valueAlreadyDisputed(encodedId_);
        }

        proposals[encodedId_].disputed = uint96(1);
        ...
        if (!isValid) {
            proposals[encodedId_].proposedValue = uint128(verifiedValue);
        ...
        )
    {
        ...
        return (
            uint256(proposals[encodedId_].proposedValue),
            proposalBlockNumber,
            proposalBlockNumber + disputeBlockWindow >= block.number,
            proposals[encodedId_].disputed == 1
        );
    }
        ...
        )
    {
        ...
        updatesCollybus_ = checkDeviation(
            getCollybusValue(encodedId_, rates[encodedId_].rateType),
            value_,
            rates[encodedId_].deviation
        );
    }
    ...
    function updateCollybus(
        ...
        if (rateType_ == uint256(IOptimisticRelayer.Type.DiscountRate)) {
            return updateDiscountRate(uint256(encodedId_), deviation_, value_);
        } else if (rateType_ == uint256(IOptimisticRelayer.Type.SpotPrice)) {
            return
                updateSpot(
