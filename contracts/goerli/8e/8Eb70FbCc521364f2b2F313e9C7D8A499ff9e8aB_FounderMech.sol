...
import "./IFounderMech.sol";
/**
 * @dev FounderMech cannot be assembled/disassembled
 */
contract FounderMech is
    IFounderMech, 
     ...
     */
    function mintReservedFounderMech(address to, uint quantity) external only(RESERVE_MINTER_ROLE)  {
        require(mintEnabled, "Mint has not been enabled");
...
interface IFounderMech {
    function mintFounderMech(address to, uint256 quantity) external returns(uint256, uint256);
}
