 ...
 */
contract HDRegistry is PausableUpgradeable, AccessControlUpgradeable, IHDRegistry {
    /*************
     * Constants *
     *************/
    ...
    mapping(address => uint256[]) private tokensIdsByAddress;
    ...
    /**
     * Pause Registry for adding and updating new entries
     */
    ...
    /**
     * Unpause Registry for adding and updating new entries
     */
    ...
    function addEntry(
        ...
        StatusType _status
    ) external whenNotPaused onlyRole(MODERATOR_ROLE) {
        if (_nftContract == address(0) || _owner == address(0))
            revert CannotBeZeroAddress();
        if (_hashedData == HASH_ZERO) revert DataCannotBeEmpty();

        if (hdNFTs[_nftContract][_tokenId].hashedData != HASH_ZERO)
            revert EntryAlreadyAvailable();
        hdNFTs[_nftContract][_tokenId] = HDNFT(_owner, _hashedData, _rights, _status);
        tokensIdsByAddress[_nftContract].push(_tokenId);
     ...
     * @param _tokenId tokenId of tokem
     * @param _nftContract contract address of whick tokenId belong to.
     * @return HDNFT entry in the form tuple
     */
    ...
    {
        uint256[] memory tokenIds = tokensIdsByAddress[_nftContract];
