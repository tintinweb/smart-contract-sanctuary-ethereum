...
contract PassHelper {
	...
	Diamonds immutable public DIAMONDS;

	constructor(address _pass, address _mint, address _diamonds) {
		PASS = GenesisPass(_pass);
		MINT = GenesisMint(_mint);
		DIAMONDS = Diamonds(_diamonds);
	}
	...
	function getData() external view returns(
		...
		uint256 passPrio,
		uint256 diamondPrio) {
		mintedFromMerkle = MINT.currentMint();
		mintedFromDiamond = DIAMONDS.passMinted();
		maxSupply = 777;
		price = MINT.MINT_PRICE();
		whitelistMint = MINT.whitelistMint();
		publicMint = MINT.publicMint();
		maxStage = DIAMONDS.growthStages();
		passPrio = MINT.currentPrio();
