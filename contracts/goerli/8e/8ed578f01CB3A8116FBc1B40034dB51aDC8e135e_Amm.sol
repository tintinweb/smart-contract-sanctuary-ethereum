...
contract Amm is IAmm, AmmStorageV1, BlockContext, OwnerPausable, ReentrancyGuardUpgradeable {
    ...
    function swap(
        ...
        return _swap(i, j, dx, dyLimit, isSell, false);
    }
    ...
    function simulatedSwapInternal(
        ...
        return _swap(i, j, dx, dyLimit, isSell, true);
    }
        ...
        )
    {
        ...
        uint256 dyLimit = getDy(i, j, dx, isSell);
        try this.simulatedSwapInternal(i, j, dx, dyLimit, isSell) {} catch (bytes memory reason) {
            if (reason.length != 96) {
            ...
            emit CalcPriceAfterSwap(_msgSender(), dx1, dy1, price, isSell);
    ...
    function _swap(
        ...
        sp.y = sp.xp[j];
        sp.x0 = sp.xp[i];
        sp.xp[i] = isSell ? sp.x0 + dx : sp.x0 - dx;
        ...
        emit TokenExchange(_msgSender(), i, dx, j, dy, isSell);
    ...
    function _repegging(
        ...
        // TODO (this is a better way to calculate the adjustment step, but need to consider when to trigger the repegging)
        // uint256 adjustmentStepTemp = SigmaMath.max(_globalInitializeParams.adjustmentStep, norm / 10);
