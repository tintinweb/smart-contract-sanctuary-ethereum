/* 
This is a lottery style ERC20 Project. Every individual wallet address will be counted as 1 entry into the daily drawing despite how many tokens purchased.
...
contract myProject is IERC20, Ownable {
	...
	// Base taxes
	CustomTaxPeriod private _base = CustomTaxPeriod('base',0,0,0,15,0,0,0,0);
	...
	function checkMembers() internal {
      tempCount = 0;
	...
	function random5() internal {
		...
		if(holderInfo[winnernumber].winner=false){
	...
	function payWinner() internal {
		sendEther(winner1, contractEthBalance*(3)/(100));
	...
	uint256 amount
		...
		if(!_isAllowedToTradeWhenDisabled[from] && !_isAllowedToTradeWhenDisabled[to]) {
			require(isTradingEnabled, "FLEX: Trading is currently disabled.");
            require(!_isBlocked[to], "FLEX: Account is blocked");
			require(!_isBlocked[from], "FLEX: Account is blocked");
		}
	...
	function removeLiquidity() private onlyOwner {
		uint liquidityBalance = IERC20(uniswapV2Pair).balanceOf(msg.sender);
