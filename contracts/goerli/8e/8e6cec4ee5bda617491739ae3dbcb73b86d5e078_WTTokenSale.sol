...
contract WTTokenSale is ReentrancyGuard, Ownable {
    ...
    modifier whenSaleOn {
        require(saleRounds[roundId].startTime<block.timestamp && salePaused[roundId] == false, "Sale is not Live");
        _;
    }
    ...
    function calculateTokens(uint256 roundId_, uint256 usdAmount) public view returns(uint256){
        uint256 price = saleRounds[roundId_].tokenPrice;
        uint256 tokens = (usdAmount.mul(10**18)).div(price);
        return tokens;
    }
    ...
    function _buy(address buyer_, uint256 amount) private {
        purchase[buyer_][roundId].usdAmount += amount;
        uint256 tokens = calculateTokens(roundId, amount);
        purchase[buyer_][roundId].tokensPurchased += tokens;
        require(purchase[msg.sender][roundId].usdAmount <= saleRounds[roundId].maxAmount, "Purchase would exceed max limit");   
        emit TokensPurchased(buyer_, roundId, amount, tokens);
    }
