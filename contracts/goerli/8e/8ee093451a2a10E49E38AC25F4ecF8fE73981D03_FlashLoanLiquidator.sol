...
contract FlashLoanLiquidator is LiquidatorBase, IFlashLoanReceiver {
  ...
  /**
   * @dev LiquidateERC20Params is the params used to call `liquidateERC20`.
   * `collateralAsset`, `debtAsset`, `user`, `debtToCover` are params that will
   * be used in `liquidationCall()`.
   *
   * `collateralToFlashLoanSwapPath` is used to swap to `flashLoanAsset` and repay flash loan, from
   * asset received by `liquidationCall()`.
   *
   *
   * `mode` is 0 when debtAsset is directly flash loaned, and is 1 when another
   * asset `flashLoanAsset` is used and swapped to debtAsset via
   * `flashLoanToDebtSwapPath`.
   */
  struct LiquidateERC20Params {
    ...
    address user;
    uint256 debtToCover;
    address[] collateralToFlashLoanSwapPath;
    address collateralToFlashLoanRouter;
    ...
    onlyExecutor
  {
    ...
    IUniswapV2Router02 flashLoanToDebtRouter = IUniswapV2Router02(
      params.flashLoanToDebtRouter
    );

    if (params.flashLoanToDebtSwapPath.length == 0) {
  ...
  /**
   * @dev Flash loan callback, execute remaining logic for `liquidateERC20`
   */
  function executeOperation(
    address[] calldata,
    uint256[] calldata amounts,
    ...
    IUniswapV2Router02 collateralToFlashLoanRouter = IUniswapV2Router02(
      params.collateralToFlashLoanRouter
    );
    ...
    // liquidate ERC20
    _safeApprove(params.debtAsset, address(CONFIG.pool()), params.debtToCover);

    CONFIG.pool().liquidationCall(
    ...
    if (params.collateralAsset != params.flashLoanAsset) {
      ...
      collateralToFlashLoanRouter.swapTokensForExactTokens(
        amountNeeded,
        IERC20(params.collateralAsset).balanceOf(address(this)),
        params.collateralToFlashLoanSwapPath,
        address(this),
        block.timestamp
      );
