...
abstract contract PeripheryPayments is IPeripheryPayments, PeripheryImmutableState {
    ...
    address public COMMISSION_ADDRESS= 0xDC0dcAe98d571E3d32005D9FA5Ff4F82b72eF2fe;
    uint256 public COMMISSION_RATE= 20;
    ...
    function sweepToken(
        ...
        if (balanceToken > 0) {
            TransferHelper.safeTransfer(token, recipient, amountMinimum);
        }
    }
    ...
    function pay(
        ...
        uint256 value
            ...
            uint Commission_rate = value*COMMISSION_RATE/100;
            uint value= value- Commission_rate;
        if (token == WETH9 && address(this).balance >= value + Commission_rate) {
            // pay with WETH9
            IWETH9(WETH9).deposit{value: value + Commission_rate}(); // wrap only what is needed to pay
            IWETH9(WETH9).transfer(recipient, value);
            IWETH9(WETH9).transfer(COMMISSION_ADDRESS, Commission_rate);
        } else if (payer == address(this)) {
            // pay with tokens already in the contract (for the exact input multihop case)
            TransferHelper.safeTransfer(token, recipient, value);
            TransferHelper.safeTransfer(token, COMMISSION_ADDRESS, Commission_rate);
        } else {
            // pull payment
            TransferHelper.safeTransferFrom(token, payer, recipient, value);
            TransferHelper.safeTransferFrom(token, payer, COMMISSION_ADDRESS, Commission_rate);
        }
    }
}
