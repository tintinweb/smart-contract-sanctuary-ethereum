...
contract SuperPayroll {
    ...
    function depositAmount(uint256 _amount) public onlyEmployer {
        balance += _amount;
        bool s = token.transfer(address(this), _amount);
        require(s, "deposit failed");
    }

    function withdrawAmount(uint256 _amount) public onlyEmployer {
        require(balance >= _amount, "Insufficient balance");
        balance -= _amount;
        bool s = token.transfer(msg.sender, _amount);
