...
contract TokenFacet is ERC721AUpgradeable {
    function tokenTransfers(uint tokenNumber) external view returns (uint256) {
        return TokenFacetLib.getState().transferCount[tokenNumber];
    }
    ...
    function mint(uint256 amount) external payable {
        ...
        uint256 _walletCap = 10;
        require(
            amount <= _walletCap,
            string(
                abi.encodePacked(
                    "TokenFacet: maximum tokens per mint is 10 "
                )
            )
        );
    ...
    function _beforeTokenTransfers(
        ...
        TokenFacetLib.getState().transferCount[i] += 1;
        }
    }
    
    function withdraw(uint256 amount) external {
        GlobalState.requireCallerIsAdmin();
        require(amount > 0, "TokenFacet: must withdraw more than 0");
