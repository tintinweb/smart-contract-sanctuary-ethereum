...
contract PayrollNFT is ERC721, Ownable {
    /**
     * @dev
    ...
    /// mint済みのjointedID(userID + SNSID)を記録するMapping
    mapping(string => bool) private _isJointedIdMinted;
    ...
    function mint(
        ...
        require(!_isJointedIdMinted[jointedId], "already minted");
        ...
        // mintした情報をMappingに格納
        _isJointedIdMinted[jointedId] = true;
    ...
    function burn(uint256 tokenId) public virtual {
        ...
        _isJointedIdMinted[_tokenIdtoJointedId[tokenId]] = false;
    ...
    function getMintedTokenId(string calldata jointedId) external view returns (uint256) {
        require(isMintedJointedId(jointedId), "not minted");
        uint256 tokenId = 0;
        for (uint256 i = 1; i <= _tokenIds.current(); i++) {
            if(keccak256(abi.encodePacked(_tokenIdtoJointedId[i])) == keccak256(abi.encodePacked(jointedId))){
                tokenId = i;
                break;
            }
        }
