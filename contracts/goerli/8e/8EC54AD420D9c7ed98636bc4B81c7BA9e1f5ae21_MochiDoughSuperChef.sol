                      ...
                      ██▒▒▒▒  ▒▒▒▒▒▒▒▒▒  ▒▒██                        
                        ...
                        ██▒▒▒▒            ▒▒▒▒██                          
                          ██▒▒                 ██                            
                            ████        ████                              
                                ████████              
                                                  https://github.com/supersorbet/                                                                    
 */
pragma solidity ^0.8.0;
//
library CrackHeadMath64x64 { 
    /*
    ...
    function divi(int256 x, int256 y) internal pure returns (int128) {
        unchecked {
            ...
            uint128 absoluteResult = divuuuuuu(uint256(x), uint256(y));
            if (negativeResult) {
     ...
     * @return signed 64.64-bit fixed point number
     */
    function gayXp(int128 x) internal pure returns (int128) {
        unchecked {
            require(x < 0x400000000000000000); // Overflow
     ...
     */
    function divuuuuuu(uint256 x, uint256 y) private pure returns (uint128) {
        unchecked {
            require(y != 0);
...
abstract contract ReentrancyGuard {
    ...
    modifier nonReentrantStayTheFuckOutBitch() {
        _nonReentrantBefore();
        _;
        _nonReentrantAfter();
    }
 ...
 *
 * NOTE: `SafuMath` is generally not needed starting with Solidity 0.8, since the compiler
 * now has built in overflow checking. so fuck it lmao
 */
library SafuMath {
        ...
        returns (bool, uint256)
    {
        unchecked {
            ...
            // benefit is lost if 'b' is also tested. Honestly just siphon the gas or hack the gas pump at the gas station
...
interface IMochisToken {
    function wakeUp(address to, uint256 amount) external;
    ...
    function fragmentToMochis(uint256 value) external view returns (uint256);

    function mochisToFragment(uint256 mochis) external view returns (uint256);
...
contract MochiDoughSuperChef is Ownable, ReentrancyGuard {
    ...
    // mochis LP address
    IUniswapPair public mochisLp;
    // Uniswap V2 Router
    IUniswapV2Router public router;
    // mochis tokens reward per block.
    ...
    // The block number when mochis mining starts.
    ...
    constructor(
        IMochisToken _mochis, 
        IUniswapPair _mochisLp,
    ...
    function pow(int128 x, uint256 n) public pure returns (int128 r) {
        r = CrackHeadMath64x64.fromUInt(1);
        while (n > 0) {
            if (n % 2 == 1) {
                r = CrackHeadMath64x64.mul(r, x);
                n -= 1;
            } else {
                x = CrackHeadMath64x64.mul(x, x);
                n /= 2;
            }
        }
    }
    ...
    function compound(
        ...
        return
            CrackHeadMath64x64.mulu(
                pow(
                    CrackHeadMath64x64.add(
                        CrackHeadMath64x64.fromUInt(1),
                        CrackHeadMath64x64.divu(ratio, 10**18)
                    ),
                    n
                ),
                principal
    ...
    // XXX DO NOT add the same LP token more than once. Rewards will be FUCKED UP if you do. IM SERIOUS
    function add(
        uint256 _allocPoint,
        IERC20 _lpToken,
        bool _withUpdate
    ...
    // Update the given pool's mochis allocation point. Can only be called by the owner.
    function set(
        uint256 _pid,
        uint256 _allocPoint,
        bool _withUpdate
        ...
        returns (uint256)
    {
        ...
        if (address(pool.lpToken) == address(mochis)) {
            lpSupply = mochis.balanceOfUnderlying(address(this));
        }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 mochisReward = ((block.number - pool.lastRewardBlock) *
            rewardPerBlock *
            pool.allocPoint) / totalAllocPoint;
        pool.accRewardPerShare += (mochisReward * 1e12) / lpSupply;
        pool.lastRewardBlock = block.number;
    ...
    // Gib tokens to teh MochiChef for MOCHIDOUGH allocation.
    function deposit(
        uint256 _pid,
        uint256 _amount,
        address _account
    ) external nonReentrantStayTheFuckOutBitch {
        require(
        ...
        if (address(pool.lpToken) == address(mochis)) {
            _amount = mochis.fragmentToMochis(_amount);
        }
    ...
    // Withdraw tokens from MochiChef.
    function withdraw(uint256 _pid, uint256 _amount) external {
        ...
        user.rewardDebt = (user.amount * pool.accRewardPerShare) / 1e12;
        if (address(pool.lpToken) == address(mochis)) {
            _amount = mochis.mochisToFragment(_amount);
        }
    ...
    // User will be gib MOCHIDOUGH from MochiChef
    function claim(uint256 _pid, address _account)
        external
        nonReentrantStayTheFuckOutBitch
        returns (uint256)
    {
        require(
            msg.sender == _account || msg.sender == address(this),
        ...
        mochis.wakeUp(_account, pending);
    ...
    // autoCompound smol smol
    function compoundAndReplicateSmol() external {
        uint256 rewards = this.claim(1, msg.sender);
        this.deposit(1, rewards, msg.sender);
    }

    // autoCompound thicc
    function compoundAndReplicateThicc(
        uint256 _amountTokenDesired,
        uint256 _amountTokenMin,
        uint256 _amountETHMin
    ) external payable {
        this.claim(0, msg.sender);
        IERC20(address(mochis)).safeTransferFrom(
            msg.sender,
            address(this),
            _amountTokenDesired
        );
        IERC20(address(mochis)).approve(address(router), _amountTokenDesired);
        ...
        IERC20(address(mochis)).safeTransfer(
            msg.sender,
