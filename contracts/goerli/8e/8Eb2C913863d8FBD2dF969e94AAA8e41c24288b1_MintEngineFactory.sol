...
contract ERC721Minter is Ownable, PullPayment, Initializable {
...
contract MintEngineFactory {
    ...
    function getProxyAddressFor(
        ...
        bytes32 _salt = keccak256(abi.encodePacked(owner, nft, price, merkleRoot, nonce));
        minter = address(uint160(uint(keccak256(abi.encodePacked(
            bytes1(0xff),
            address(this),
            _salt,
            keccak256(abi.encodePacked(
                type(ERC721MinterProxy).creationCode,
                abi.encode(minterImplementation)
            ))
        )))));
    }
}