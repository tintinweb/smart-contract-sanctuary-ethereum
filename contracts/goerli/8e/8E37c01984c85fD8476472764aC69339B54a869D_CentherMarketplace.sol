...
interface INFTCollection {
    function MintNFT(string memory _tokenURI, address _to, uint256 _lockPeriod) external returns(uint256);
}
...
contract CentherMarketplace is OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    event CreateItemWithLockTime(address indexed _collection, uint256 indexed _tokenID, string _tokenURI, bool _isAuction, uint256 _price, uint256 _endAt, address _creator, uint256 _lockPeriod, address _receiver);
    ...
    function createCollection(string memory name, string memory symbol, string memory category, string memory uri, uint256 maxSupply) external payable onlyRegisterUser returns(address) {
    ...
    function createItems(address collection, string memory tokenURI, uint32 supply, bool isAuction, uint256 price, uint256 period, uint256 lockPeriod, address receiver) external payable onlyRegisterUser {
        ...
        for(uint32 i = 0; i < supply; i++) {
            uint256 tokenID;
            if (lockPeriod > 0) {
                tokenID = INFTCollection(collection).MintNFT(tokenURI, receiver, lockPeriod);
                emit CreateItemWithLockTime(address(collection), tokenID, tokenURI, isAuction, price, block.timestamp + period, msg.sender, lockPeriod, receiver);
            } else if (lockPeriod == 0) {
                tokenID = INFTCollection(collection).MintNFT(tokenURI, address(this), lockPeriod);
                if(isAuction) {
    ...
    function bidOnNonAuctionItem(address collection, uint256 tokenID, uint256 newPrice) external onlyRegisterUser {
    ...
    function acceptBid(address collection, uint256 tokenID, uint256 index) external onlyRegisterUser nonReentrant {
        require(_nftInfoForAuction[collection][tokenID].state != AuctionState.OPEN, "This is Auction Item");
        BidInfo storage bidInfo = _bidInfo[collection][tokenID];
    ...
    function listItemForSale(address collection, uint256 tokenID, uint256 price) external onlyRegisterUser {
        require(_nftInfoForAuction[collection][tokenID].state != AuctionState.OPEN, "This is Auction Item");
    ...
    function editItemForSale(address collection, uint256 tokenID, uint256 newPrice) external onlyRegisterUser {
        require(_nftInfoForAuction[collection][tokenID].state != AuctionState.OPEN, "This is Auction Item");
    ...
    function cancelItemForSale(address collection, uint256 tokenID) external onlyRegisterUser {
        require(_nftInfoForAuction[collection][tokenID].state != AuctionState.OPEN, "This is Auction Item");
        require(_nftInfoForSale[collection][tokenID].seller == msg.sender, "Not Your NFT");
        _nftInfoForSale[collection][tokenID] = SaleInfo({seller: address(0), price: 0});
    ...
    function buyForListedItem(address collection, uint256 tokenID) external payable onlyRegisterUser nonReentrant {
        require(_nftInfoForAuction[collection][tokenID].state != AuctionState.OPEN, "This is Auction Item");
        SaleInfo memory saleInfo = _nftInfoForSale[collection][tokenID];
        // Verify that the item is for sale
        require(saleInfo.seller != address(0x0) || saleInfo.price != 0, "Incorrect Sale Info");
        // Ensure buyer can't be seller
        require(saleInfo.seller != msg.sender, "Item listed by You");
        // Ensure price
    ...
    function createAuction(address collection, uint256 tokenID, uint256 startPrice, uint256 auctionPeriod) external onlyRegisterUser{
        require(IERC721(collection).ownerOf(tokenID) == msg.sender, "Not Your NFT");
    ...
    function bidOnAuction(address collection, uint256 tokenID) external payable onlyRegisterUser {
    ...
    function endAuction(address collection, uint256 tokenID) external onlyRegisterUser nonReentrant {
        AuctionInfo storage auctionInfo = _nftInfoForAuction[collection][tokenID];
        require(auctionInfo.state == AuctionState.OPEN, "This is non exist Auction");
        ...
        //Transfer money by distribution
        if (auctionInfo.highestBidAddress != address(0x0) && auctionInfo.highestBidPrice > 0) {
            _distributeFunds(
    ...
    function cancelAuction(address collection, uint256 tokenID) external onlyRegisterUser {
        AuctionInfo storage auctionInfo = _nftInfoForAuction[collection][tokenID];
        require(auctionInfo.state == AuctionState.OPEN, "This is non exist Auction");
...
contract NFTCollection is ERC721URIStorage {
    ...
    function MintNFT(string memory _tokenURI, address _to, uint256 _lockPeriod) external onlyMarketplaceContract returns(uint256) {
        require(_tokenIdentifiers.current() < MAX_SUPPLY, "Exceeded max supply.");
        bytes memory stringURI = bytes(_tokenURI);
        require(stringURI.length > 0, "TokenURI can't be empty string!");
        _tokenIdentifiers.increment();
        _mint(_to, _tokenIdentifiers.current());
        _setTokenURI(_tokenIdentifiers.current(), _tokenURI);
        if (_lockPeriod > 0) {
            unlockTime[_tokenIdentifiers.current()] = block.timestamp + _lockPeriod;
        }
        return _tokenIdentifiers.current();
    }

    function _beforeTokenTransfer(address from, address to, uint256 tokenID) internal override (ERC721) {
        require(block.timestamp > unlockTime[tokenID], "Can't transfer until unlockTime");
