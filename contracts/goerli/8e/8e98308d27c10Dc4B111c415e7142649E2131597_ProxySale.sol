...
contract ProxySale is Ownable, ReentrancyGuard {
    ...
    mapping(address => uint256) public reapNumberMinted;
    ...
    function _preMint(bytes32[] memory proof, uint256 num) internal{
        ...
        if(status == Status.ReapMint){
            ...
            require(maxReapNumber - nftMint.totalSupply() >= num, "Max supply reached");

            require(reapNumberMinted[msg.sender] + num <= maxReapMintNumber, "Minting more than the max supply for a single address");

            reapNumberMinted[msg.sender] = reapNumberMinted[msg.sender] + num;

            makeChange(0);
        }else{

            require(maxReapNumber + maxAllowNumber - nftMint.totalSupply() >= num, "Max supply reached");
    ...
    function _publicMint(uint256 num) internal
    {
        ...
        require(MaxSupply - nftMint.totalSupply() - num >= teamReservedQuantity, "Max supply reached");
    ...
    function setAllowListMintPrice(uint256 _allowListMintPrice) public onlyOwner{
        allowListMintPrice = _allowListMintPrice;
    }

    function setMaxMintNumber(uint256 _maxReapMintNumber, uint256 _maxAllowMintNumber, uint256 _maxPublicMintNumber) public onlyOwner{
        maxReapMintNumber = _maxReapMintNumber;
        maxAllowMintNumber = _maxAllowMintNumber;
        maxPublicMintNumber = _maxPublicMintNumber;

    }

    function setMaxNumber(uint256 _maxReapNumber, uint256 _maxAllowNumber, uint256 _teamReservedQuantity) public onlyOwner{
        maxReapNumber = _maxReapNumber;
