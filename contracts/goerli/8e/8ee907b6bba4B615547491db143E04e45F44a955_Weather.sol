    ...
    DefaultOperatorFilterer
{
    ...
    uint256 private constant _TOTAL_SUPPLY = 10;

    address payable private _royaltyWallet;
    uint256 public royaltyBasis;

    mapping(uint256 => string) private _tokenIdToLocation; // token id => location

    constructor() ERC721A("Weather", "WTHR") {
        _isPreSale = false;
        _isPublicSale = false;
        _royaltyWallet = payable(0xD5F47f85cEE5EdDb8Bd94020219464e6d9385b90);
        royaltyBasis = 500; // 5%
    }
    ...
    function setIsPreSale(bool isPreSale) external onlyOwner {
        _isPreSale = isPreSale;
    }

    function setIsPublicSale(bool isPublicSale) external onlyOwner {
        _isPublicSale = isPublicSale;
    }
    ...
    function preMint(uint256 quantity, bytes32[] calldata proof) external payable {
        ...
        for (uint256 i = nextTokenId; i < nextTokenId + quantity; i++) {
            _tokenIdToLocation[i] = generateLocation(i);
        }

        _mint(msg.sender, quantity);
    }

    function publicMint(uint256 quantity, bytes32[] calldata proof) external payable {
        require(_isPublicSale, "PublicSale is not active.");
        require(isAllowlisted(proof), "Invalid proof");
        require(
            totalSupply() + quantity <= _TOTAL_SUPPLY,
            "Reached max supply"
    ...
    function generateLocation(
        ...
        uint8 source = (uint8(pseudorandomness) % 10) + 1;
        if (source == 1) {
            return "New York";
        } else if (source == 2) {
            return "Los Angeles";
