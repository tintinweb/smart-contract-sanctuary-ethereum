...
contract LandNFT_V1 is Initializable, ERC721Upgradeable, OwnableUpgradeable {
    ...
    event TransferTokensEvent(string tokenURI, address to);
    event PurchasedNftPriceEvent(uint256 price, address to);
        ...
        returns(uint256)
    {
        ...
        bytes32 leaf = keccak256(abi.encodePacked(msg.sender));
        if (MerkleProofUpgradeable.verify(_discount5PerMerkleProof, discount2PerMerkleRoot, leaf)) {
        ...
        } else if (MerkleProofUpgradeable.verify(_discount2PerMerkleProof, discount5PerMerkleRoot, leaf)) {
    ...
    function transferTokens(uint256 _mintNum, uint256 _tokenPrice) internal {
        for (uint256 i; i < _mintNum; i++) {
            for (uint256 index; index < MAX_SUPPLY_MINT; index++) {
                ...
                if (!isNonMintTokenIds[tokenId]) {
                    ...
                    tokenIdListIndex++;
                    numberOfPurchases++;
                    emit TransferTokensEvent(tokenURI(tokenId), msg.sender);
                    break;
                } else {
                    tokenIdListIndex++;
                }
        ...
        emit PurchasedNftPriceEvent(_tokenPrice.mul(_mintNum), msg.sender);
    }
}