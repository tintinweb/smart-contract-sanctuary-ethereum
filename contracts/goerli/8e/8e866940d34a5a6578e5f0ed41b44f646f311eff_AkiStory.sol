...
contract AkiStory is ERC721A, Ownable {
  ...
  function mintOG(uint8 mintAmount, bytes32[] calldata _proof) public payable {
    require(sellingStep == Steps.WLMints, "WLMints has not started");
    require(totalSupply().add(mintAmount) <= MaxToken, "Sold Out");
    require(isOGWhitelisted(msg.sender, _proof), "Not Whitelisted");
  ...
  function mintWL(uint8 mintAmount, bytes32[] calldata _proof) public payable {
    require(sellingStep == Steps.WLMints, "WLMints has not started");
    require(totalSupply().add(mintAmount) <= MaxToken, "Sold Out");
    require(isWalletWhiteListed(msg.sender, _proof), "Not Whitelisted");
    require(mintAmount > 0, "At least one should be minted");
  ...
  function mintAllowList(uint8 mintAmount, bytes32[] calldata _proof) public payable {
    require(sellingStep == Steps.Allowlist, "Allow List has not started");
    require(totalSupply().add(mintAmount) <= MaxToken, "Sold Out");
    require(isAllowList(msg.sender, _proof), "Not Whitelisted");
    require(mintAmount > 0, "At least one should be minted");
