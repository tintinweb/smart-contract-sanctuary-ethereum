...
contract UniversityCoursesCert is ERC1155LazyMint, PermissionsEnumerable {

    bytes32 public constant PROFESSOR_ROLE = keccak256("PROFESSOR_ROLE");
    bytes32 public constant WLSTUDENT_ROLE = keccak256("WLSTUDENT_ROLE");
    ...
    function _canLazyMint() internal view override returns (bool) {
        return hasRole(DEFAULT_ADMIN_ROLE,msg.sender) || hasRole(PROFESSOR_ROLE,msg.sender);
    }

    function verifyClaim(address _claimer, uint256 _tokenId, uint256 _quantity) public view override {
        require(_tokenId % 2 == 0, "Only the attempt certification can be claimed!");
        require(_quantity == 1, "Only 1 certification can be claimed!");
    }

    //odds tokenIds represent the passed certifications
    function upgradeToPassed(uint256 tokenId) public {
        require(hasRole(DEFAULT_ADMIN_ROLE,msg.sender) || hasRole(WLSTUDENT_ROLE,msg.sender), "Only students who passed the exam can get this certification!");
        require(tokenId+1 < nextTokenIdToMint(), "Invalid tokenId");
        require(tokenId % 2 == 0, "Only the attempt certifications can be upgraded!");
        _burn(msg.sender, tokenId, 1);
        _transferTokensOnClaim(msg.sender, tokenId+1, 1);
        emit TokensClaimed(msg.sender, msg.sender, tokenId+1, 1);
    }
