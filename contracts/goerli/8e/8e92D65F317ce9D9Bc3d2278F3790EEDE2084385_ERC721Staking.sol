    ...
    UUPSUpgradeable
{
    ...
    function stake(uint256[] calldata tokenIds) external updateReward(_msgSender()) {
        require(tokenIds.length > 0, "Staking: amount must be greater than zero");

        for (uint256 i = 0; i < tokenIds.length; i++) {
            stakingToken.transferFrom(_msgSender(), address(this), tokenIds[i]);
        }
        balanceOf[_msgSender()] += tokenIds.length;
        totalSupply += tokenIds.length;

        emit Staking(_msgSender(), tokenIds.length);
    }
    ...
    function withdraw(uint256[] calldata tokenIds) external updateReward(_msgSender()) {
        ...
        for (uint256 i = 0; i < tokenIds.length; i++) {
            stakingToken.transferFrom(address(this), _msgSender(), tokenIds[i]);
        }
        balanceOf[_msgSender()] -= tokenIds.length;
        totalSupply -= tokenIds.length;

        emit Unstaking(_msgSender(), tokenIds.length);
    }
    ...
    function claim() external updateReward(_msgSender()) {
        ...
        rewardsToken.transferFrom(address(this), _msgSender(), reward);
    ...
    function _setRewardsDuration(uint256 duration_) internal {
        require(finishBlock < block.number, "Staking: reward duration not finished");
        duration = duration_;
    }
