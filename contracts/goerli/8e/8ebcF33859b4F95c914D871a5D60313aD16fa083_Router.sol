...
contract Router is IRouter,AccessControl{
    ...
    */
    ...
    bool public updateIndexDisable; // default:false
    bool public lockDisable;    // default:false
    ...
    function setBeforeRevealURI(string memory newURI) external onlyAdmin {
        beforeRevealURI = newURI;
    }

    function setUpdateIndexDisable(bool _value) external onlyAdmin {
        updateIndexDisable = _value;
    }

    function setlockDisable(bool _value) external onlyAdmin {
        lockDisable = _value;
    }
    ...
    function tokenURI(uint256 _tokenId,uint256 _param) external view returns(string memory URI){
        string memory _updateIndex;
        if(updateIndexDisable == false){
            uint256 _index = _param.getBitValueUint8(BITPOS_UPDATE_INDEX);
            uint256 _current = _param.getBitValueUint8(BITPOS_CURRENT_UPDATE_INDEX);

            //console.log("_index:%s,_current:%s,_param:%s",_index,_current,_param);

            if( (_current != 0) && (_index >= _current) ){
                // before reveal 
                return URI = beforeRevealURI;
            }
            _updateIndex = string.concat(_index.toString(),"/");
        }else{
            _updateIndex = "0/";
        }

        string memory _lockState;
        if(lockDisable == false){
            if(_param.getBitValueBool(BITPOS_LOCK_STATUS) == true){
                _lockState = "lock/";
            }else{
                _lockState = "unlock/";
            }
        ...
        // BASE/updateIndex/lockState/tokenId.json
        URI = string.concat(
            baseURI,
            _updateIndex,
            _lockState,
...
library BitOperator{
    ...
    function setBitValueUint8(uint256 value, uint256 pos, uint8 set) internal pure returns(uint256){
        if (pos > 248) revert(ERROR_OVERFLOW);
        // return (value & ~(0xFF << (pos*8))) | (uint256(set) << (pos*8));
