...
// File: contracts\PixelPronFactory.sol

pragma solidity >=0.7.0 <0.9.0;
contract PixelPronFactory is ERC721, Ownable {
  using Strings for uint256;
...
function _mintIdWithoutBalanceUpdate(address to, uint256 tokenId) private {
    ...
    function _mintRandom(address to, uint _mintAmount) internal virtual {
        require(msg.sender == tx.origin, "Contracts cannot mint");
        require(to != address(0), "ERC721: mint to the zero address");
        require(_mintAmount > 0, "ERC721r: need to mint at least one token");
        
        // TODO: Probably don't need this as it will underflow and revert automatically in this case
        require(_numAvailableTokens >= _mintAmount, "ERC721r: minting more tokens than available");
        
        uint updatedNumAvailableTokens = _numAvailableTokens;
        for (uint256 i; i < _mintAmount; ++i) { // Do this ++ unchecked?
    ...
    for (uint256 i = 0; i < _mintAmount; i++) {
      supply.increment();
      _mintAtIndex(_receiver, _mintAmount);
