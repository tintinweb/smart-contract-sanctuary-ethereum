...
contract D3Trading is D3Funding {
    ...
    /// @return askAmount initial ask token amount maker set
    /// @return bidAmount initial bid VUSD amount maker set
        ...
        )
    {
        ...
        askAmount = Types.parseAskAmount(
            state.tokenMMInfoMap[token].amountInfo
        );
        bidAmount = Types.parseBidAmount(
            state.tokenMMInfoMap[token].amountInfo
        );
...
contract D3Maker is D3Storage {
    ...
    /// @param allPriceSet origin price slot
    /// @param slotInnerIndex token index in slot
    /// @param priceInfo the token info needed to update
    function stickPrice(
        uint256 allPriceSet,
        uint256 slotInnerIndex,
        uint256 priceInfo
    ) public pure returns (uint256 newAllPriceSet) {
        uint256 leftPriceSet = allPriceSet >>
            ((slotInnerIndex + 1) * Types.ONE_PRICE_BIT * 2);
        uint256 rightPriceSet = allPriceSet &
            ((2**(slotInnerIndex * Types.ONE_PRICE_BIT * 2)) - 1);
        newAllPriceSet =
            (leftPriceSet << ((slotInnerIndex + 1) * Types.ONE_PRICE_BIT * 2)) +
            (priceInfo << (slotInnerIndex * Types.ONE_PRICE_BIT * 2)) +
            rightPriceSet;
    }
