    ...
    DefaultOperatorFilterer
{
    ...
    mapping(uint256 => string) public equipmentOnBody;
    mapping(uint256 => uint256) public equipmentCountOnBody;
    ...
    function requestRandomStartingIndex(DiskType diskType_, uint32 numWords_, string[] memory bodies_)
        external
        onlyOwner
        returns (uint256 requestId)
    {
        if (diskType_ == DiskType.Body){
            ...
            if (numWords_ != bodies_.length) revert DifferentCount();

            for (uint256 i = 0; i < bodies_.length; i++) {
                if (bytes(provenanceEquipmentOnBody[i]).length <= 0) revert ProvenanceEquipmentNotSet();
                if (startingIndexEquipmentOnBody[i] > 0) revert StartingIndexExisted();
            }
        ...
        emit RequestSent(requestId, numWords_);
        return requestId;
    }
    ...
    function fulfillRandomWords(
        ...
        if (diskType == DiskType.Body){
            ...
            for (uint256 i = 0; i < _randomWords.length; i++) {
                uint256 startingIndex = _randomWords[i] % equipmentCountOnBody[i];
                // Prevent default sequence
                if (startingIndex == 0 || startingIndex == 1) {
                    startingIndex = 2;
                }

                startingIndexEquipmentOnBody[i] = startingIndex;
            }
    ...
    function setProvenanceEquipmentOnBody(
        ...
        uint256[] memory equipmentCountOnBody_,
        string[] memory equipmentOnBody_
    ) external onlyOwner {
        for (uint256 i = 0; i < provenanceEquipmentOnBody_.length; i++) {
            provenanceEquipmentOnBody[i] = provenanceEquipmentOnBody_[i];
            equipmentCountOnBody[i] = equipmentCountOnBody_[i];
            equipmentOnBody[i] = equipmentOnBody_[i];
        }
    }
