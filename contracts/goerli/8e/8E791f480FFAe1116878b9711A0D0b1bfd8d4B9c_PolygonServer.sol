...
// NEVELLUS POLYGON-IMPLEMENTATION CONTRACT FOR TESTNET
...
contract PolygonServer is BaseServer {
    address public constant polygonBridgeAddr =
        0x79B77456684AEB0bf7c8Ec50FB9c714d52BdA46c;

    event BridgedNvls(address indexed dione, uint256 indexed amount);

    constructor(uint256 _pid, address _dione) BaseServer(_pid, _dione) {}

    function _bridge() internal override {
        uint256 nvlsBalance = nvls.balanceOf(address(this));

        nvls.approve(address(polygonBridgeAddr), nvlsBalance);
        IPolygonBridge(polygonBridgeAddr).deposit(
            address(nvls),
            dione,
            nvlsBalance,
            toBytes(nvlsBalance)
        ...
        emit BridgedNvls(dione, nvlsBalance);
    }
