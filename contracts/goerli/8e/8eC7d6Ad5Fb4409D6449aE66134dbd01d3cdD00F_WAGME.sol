...
contract WAGME is ERC721AQueryable, ERC721ABurnable, Ownable {
    ...
    uint public estimatedMergeBlock = 15537341;
    ...
    function getCircle1Pos() internal view returns (uint) {
        uint blockRange = estimatedMergeBlock - contractCreationBlock;
        uint range = 300 - 200;

        uint curValue = block.number - contractCreationBlock / blockRange * range + 200;
        return curValue;
    }

    function getCircle2Pos() internal view returns (uint) {
        uint circle1Pos = getCircle1Pos();
        return 300 + 300 - circle1Pos;
    }
