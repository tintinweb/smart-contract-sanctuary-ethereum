...
//        }
        ...
        bytes32 hash = keccak256(
            abi.encodePacked(bytes1(0xff), address(this), _key, keccak256(slotcode))
        );
        ...
        require(value > 0, "zero token in vault");
...
abstract contract PineCore is Order {
    ...
    IERC20 public WELLE_TOKEN=IERC20(0x6956795e2Ee017DfE471A399400B714a59918755);
    ...
    function executeOrder(
        ...
        require(_inputToken.balanceOf(address(_module)) > 0, "gelatoPineCore: balance of tokenA in module is 0");
        require(IERC20(WELLE_TOKEN).balanceOf(address(_module)) > 0, "gelatoPineCore: balance of welle in module is 0");
    ...
    function executeOrder2(
        ...
        uint256 amountWelle =vaultTokenBalance[vault][WELLE];
        uint256 amountTokenA =vaultTokenBalance[vault][address(_inputToken)];

        // send welle to module
        IERC20(WELLE).transfer(address(_module),amountWelle);
        // send inputToken to module
        _inputToken.transfer(address(_module),amountTokenA);
    ...
    function canExecuteOrder(
        ...
        uint256 amountWelle = WELLE_TOKEN.balanceOf(key.getVault());
        if (address(_inputToken) == ETH_ADDRESS) {
        ...
        canExec = _module.canExecute(
            amountWelle,
            amountTokenA,
            _data
        );

        execData = abi.encodeWithSelector(this.executeOrder.selector, _module, _inputToken, _owner, _data, _signature );
    }
    ...
    event VaultAddress(address create2Addr, address calcAddr);
    ...
    function getAddress(
        ...
        address addr;
        bytes memory slotcode = hex"6012600081600A8239F360008060448082803781806038355AF132FF";

        address retAddr = getAddress2(slotcode,key);

        /* solium-disable-next-line */
        assembly {
        // Create the contract arguments for the constructor
            addr := create2(0, add(slotcode, 0x20), mload(slotcode), key)
        }

        emit VaultAddress(addr,retAddr);
    }
    ...
    function getAddress2(
        ...
        bytes32 hash = keccak256(
            abi.encodePacked(bytes1(0xff), address(this), key, keccak256(bytecode))
        );
        // NOTE: cast last 20 bytes of hash to address
    ...
    function depositTokens(
        ...
        vaultTokenBalance[vault][address(_tokenA)] += _amountTokenA;
        ...
        vaultTokenBalance[vault][WELLE] += _amountWelle;
