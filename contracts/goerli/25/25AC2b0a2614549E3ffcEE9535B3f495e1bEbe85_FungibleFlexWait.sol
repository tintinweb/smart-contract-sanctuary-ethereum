...
//
     ...
     */
    function createDisplay(string memory _displayName, uint256 _estID, uint256 _rentPer10Mins) public {
        require(isWhitelisted(msg.sender), "The address is not whitelisted");
    ...
    function setImage(
        ...
        //if display is not occupied
        if (!isOccupied(_displayId)) {
    ...
    function getDisplaysOwnedByEstablishment(uint256 _estID) public view returns (uint256[] memory) {
        uint256 len = displayOwnerMapping[establishments[_estID].establishmentOwner].length;
        uint256[] memory displaysArray = new uint256[](len);
        for (uint256 counter = 0; counter < len; counter++)
            displaysArray[counter] = displayOwnerMapping[establishments[_estID].establishmentOwner][counter]
                .dispNum;
        return displaysArray;
    }
