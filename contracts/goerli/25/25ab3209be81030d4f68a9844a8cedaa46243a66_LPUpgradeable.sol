    ...
    PausableUpgradeable
{
    ...
    function validateMintEligibility(
        ...
        if (block.timestamp >= phase3Start) {
            // validating phase 3
            if (block.timestamp > phase3End)
                return "Phase 3 has ended";

            if (quantity > 20)
                return "Max 20 at once";
        } else {
            if (!isMintValid(to, proof, phase2Root))
                return "Not in phase 2 whitelist";
            if (quantity > 2)
                return "Max 2 at once";
            if (userMinted[to] + quantity > 2)
                return "Max 2 per wallet";
        }

        if (totalSupply() + quantity > maxSupply)
            return "Exceeds supply";
