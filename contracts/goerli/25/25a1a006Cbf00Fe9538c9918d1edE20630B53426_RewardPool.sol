 ...
 **/
import {IOwnership, IRewardPool} from "../interfaces/AllInterfaces.sol";
import "../securities/AbstractOwner.sol";
import "../securities/Pausable.sol";
import "@openzeppelin/contracts/token/ERC20/utils/SafeERC20.sol";

contract RewardPool is IRewardPool, AbstractOwner, Pausable {
    ...
    uint256 public ownerWithdrawableTimestamp;

    /**
     * Modifiers
     */
    modifier ownerWithdrawable() {
        if(block.timestamp < initialTimestamp + ownerWithdrawableTimestamp) revert YetWithdrawableTime();
        _;
    }
     ...
     */
    constructor(address _ownership, address _token, address _market) {
        if (_ownership == address(0)) revert ZeroAddress();
        ownership = _ownership;
        token = _token;
        market = _market;
        initialTimestamp = block.timestamp;
        ownerWithdrawableTimestamp = 90 days;
    }

    /// @notice inherit IRewardPool
    function addReward(uint256 _reward, address _account) override external{
        if(market != msg.sender) revert OnlyMarket();
        rewards[_account] += _reward;
        emit RewardAdded(_reward, _account);
    }

    /// @notice inherit IRewardPool
    function withdrawReward(uint256 _reward) override public{
        if(_reward == 0) revert AmountZero();
        if(rewards[msg.sender] < _reward) revert AmountExceeded();
        rewards[msg.sender] -= _reward;
        IERC20(token).safeTransfer(msg.sender, _reward);
        emit RewardWithdrawal(_reward, msg.sender);
    }
 
    /// @notice inherit IRewardPool
    function withdrawAllReward() override external{
        withdrawReward(rewards[msg.sender]);
    }

    /// @notice inherit IRewardPool
    function withdrawRedundant(address _token, address _to) override external onlyOwner ownerWithdrawable{
        IERC20(_token).safeTransfer(_to, IERC20(_token).balanceOf(address(this)));
    }

    /// @notice inherit IRewardPool
    function setOwnerWithdrawableTimestamp(uint256 _ownerWithdrawableTimestamp) override external onlyOwner{
        if(_ownerWithdrawableTimestamp <= ownerWithdrawableTimestamp) revert OnlyUpper();
        ownerWithdrawableTimestamp = _ownerWithdrawableTimestamp;
        emit OwnerWithdrawableTimestamp(_ownerWithdrawableTimestamp);
    }

    /// @notice inherit IRewardPool
    function setMarket(address _market) override external onlyOwner{
        market = _market;
        emit Market(_market);
    }
