...
contract SHECHEMA is ERC721, Ownable {
    using Strings for uint256;
    ...
    constructor() ERC721("SHECHEMA", "SHCH") {
        baseUri = "https://gateway.pinata.cloud/ipfs/Qma4n1r7QtDsLvDr2eVQYN53uormHdDPy2QUtDmm3hHSZR/";
    ...
    function mint(uint256 _amountToMint) external payable {
        ...
        require(_amountToMint <= MAX_MINT_PER_TX, "You can only mint a maximum of 3 NFTs per transaction.");
        require(_amountToMint > 0, "Not 0 mints");
        uint256 curTotalSupply = totalSupply;
        require(curTotalSupply + _amountToMint <= MAX_TOKENS, "Exceeds total supply.");
        require(_amountToMint * price <= msg.value, "Insufficient funds.");

        for(uint256 i = 1; i <= _amountToMint; ++i) {
            _safeMint(msg.sender, curTotalSupply + 1);
        }
        mintedPerWallet[msg.sender] += _amountToMint;
