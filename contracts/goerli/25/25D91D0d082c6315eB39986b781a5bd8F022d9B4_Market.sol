...
contract Market is IMessageRecipient, Ownable {
  ...
  function handle(uint32 _origin, bytes32 _sender, bytes calldata _messageBody) external override onlyResisterdContract(_origin, _sender) {
    ...
    address seller = sellerAddress[keccak256(abi.encodePacked(originDomain, nftContractAddress, tokenId))];
    bytes32 key = keccak256(abi.encodePacked(originDomain, nftContractAddress, tokenId, seller));
