...
contract ClaimGift is Ownable, IERC1155Receiver, ReentrancyGuard, IERC721Receiver, GiftIndexListManager {
    ...
    function rescueGiftToken(uint256 _giftIndex, address _transferTo) external nonReentrant onlyOwner {
        if (giftTokens[_giftIndex].giftCollectionContractType == uint256(GiftContractType.ERC_1155)) {
            ...
            if (!giftTokens[_giftIndex].isGivenAway && !giftTokens[_giftIndex].isRescued) {
                ...
                uint256 giftIndexInGiftListManager = getGiftIndexAt(_giftIndex);
                removeGiftIndex(giftIndexInGiftListManager);
            } else {
    ...
    function sendGift(uint256 _claimRequestId, uint256 _randomSeed) public noContracts {
...
contract GiftIndexListManager {
    ...
    function getGiftIndexAt(uint256 number) public view returns (uint256) {
