...
contract StakingTimelock is TimelockControllerUpgradeable {
  /**
  ...
  function initialize(uint256 minDelay) external virtual initializer {
    __StakingTimelock_init(minDelay);
  }

  function __StakingTimelock_init(uint256 minDelay) internal onlyInitializing {
    address[] memory nobody = new address[](0);
    __TimelockController_init(minDelay, nobody, nobody, address(0));
    __StakingTimelock_init_unchained();
  }

  function __StakingTimelock_init_unchained() internal onlyInitializing {
    /**
     * MVP: msg.sender is governor.
     * When the governor contract is ready, the following roles will set to governor contract AND
     * renounce / revoke these roles from msg.sender.
     */
    address governor = _msgSender();
    _setupRole(TIMELOCK_ADMIN_ROLE, governor);
