    ...
    EIP2981RoyaltyOverrideCore
{
    ...
    function mintBatchMultiAddress(
        address[] memory _to,
        uint256[] memory _assetTypes,
        uint256[] memory _amounts
    ) public onlyRole(MINTER_ROLE) {
        for (uint i = 0; i < _to.length; i++) {
            _mintBatch(_to[i], _assetTypes, _amounts, "");
        }
    }
    ...
    function supportsInterface(bytes4 interfaceId)
        public
        view
        override(ERC1155, EIP2981RoyaltyOverrideCore, AccessControl)
        returns (bool)
    {
...
contract LAGPartsMinter is Ownable, AccessControl {
    using Strings for uint256;

    LAGParts public lagPartsContract;
    uint256[] public allowdTokenId = [1,2,3,4,5,6,7,8];
    mapping(uint256 => uint256) public maxSupply;

    uint256 public preCost = 1.000 ether;
    uint256 public publicCost = 1.000 ether;
    ...
    mapping(uint256 => bytes32) public merkleRootFreeMint;
    mapping(uint256 => bytes32) public merkleRootPreMint1;
    mapping(uint256 => bytes32) public merkleRootPreMint2;
     ...
     */
    function setLagPartsContract(address _lagPartsContract) external onlyRole(DEFAULT_ADMIN_ROLE) {
        lagPartsContract = LAGParts(_lagPartsContract);
    }

    /**
     * @notice Set the merkle root for the FreeMint
     */
    function setAllowdTokenId(uint256[] memory _allowdTokenId) external onlyRole(DEFAULT_ADMIN_ROLE) {
        allowdTokenId = _allowdTokenId;
    }

    /**
     * @notice Set the merkle root for the FreeMint
     */
    function setMaxSupply(uint256[] memory _tokenId, uint256[] memory _maxSupply) external onlyRole(DEFAULT_ADMIN_ROLE) {
        for (uint i = 0; i < _tokenId.length; i++) {
            maxSupply[_tokenId[i]] = _maxSupply[i];
        }
    }
     ...
     * @notice Set the merkle root for the FreeMint
     */
    function setMerkleRootFreeMint(uint256 index, bytes32 _merkleRoot) external onlyRole(DEFAULT_ADMIN_ROLE) {
        merkleRootFreeMint[index] = _merkleRoot;
    }

    /**
     * @notice Set the merkle root for the PreMint1
     */
    function setMerkleRootPreMint1(uint256 index, bytes32 _merkleRoot) external onlyRole(DEFAULT_ADMIN_ROLE) {
        merkleRootPreMint1[index] = _merkleRoot;
    }

    /**
     * @notice Set the merkle root for the PreMint2
     */
    function setMerkleRootPreMint2(uint256 index, bytes32 _merkleRoot) external onlyRole(DEFAULT_ADMIN_ROLE) {
        merkleRootPreMint2[index] = _merkleRoot;
    }

    function freeMint(uint256 index, uint256 _tokenId, uint256 _mintAmount, uint256 _freeMintMax, bytes32[] calldata _merkleProof)
        public
        payable
        whenMintable
        callerIsUser
    {
        uint256 cost = 0;
        mintCheck(_tokenId, _mintAmount,  cost);
        require(salePhase == 1, "FreeMint is not active.");
        bytes32 leaf = keccak256(abi.encodePacked(msg.sender, _freeMintMax));
        require(
            MerkleProof.verify(_merkleProof, merkleRootFreeMint[index], leaf),
            "Invalid Merkle Proof"
        );

        require(
            claimed[index][msg.sender] + _mintAmount <= _freeMintMax,
            "Already claimed max"
        );

        lagPartsContract.mint(msg.sender, _tokenId, _mintAmount);
        claimed[index][msg.sender] += _mintAmount;
    }

    function preMint1(uint256 index, uint256 _tokenId, uint256 _mintAmount, uint256 _preMint1Max, bytes32[] calldata _merkleProof)
        public
        payable
        whenMintable
        callerIsUser
    {
        ...
        require(
            MerkleProof.verify(_merkleProof, merkleRootPreMint1[index], leaf),
            "Invalid Merkle Proof"
        );

        require(
            claimed[index][msg.sender] + _mintAmount <= _preMint1Max,
            "Already claimed max"
        );
    ...
    function preMint2(uint256 index, uint256 _tokenId, uint256 _mintAmount, uint256 _preMint2Max, bytes32[] calldata _merkleProof)
        public
        payable
        whenMintable
        callerIsUser
    {
        ...
        require(
            MerkleProof.verify(_merkleProof, merkleRootPreMint2[index], leaf),
            "Invalid Merkle Proof"
        );
        require(
            claimed[index][msg.sender] + _mintAmount <= _preMint2Max,
            "Already claimed max"
        );
        require(
    ...
    function publicMint(uint256 index, uint256 _tokenId, uint256 _mintAmount) public
        payable
        whenMintable
        callerIsUser
    {
        ...
        require(
            claimed[index][msg.sender] + _mintAmount <= maxMintPerWallet,
            "Already claimed max"
        );
    ...
    function mintCheck(
        ...
        require(
            lagPartsContract.totalSupply(_tokenId) + _mintAmount <= maxSupply[_tokenId],
            "MAXSUPPLY over"
        );
        bool isAllowdTokenId = false;
        for (uint i = 0; i < allowdTokenId.length; i++) {
            if (allowdTokenId[i] == _tokenId) {
                isAllowdTokenId = true;
            }
        }
        require(isAllowdTokenId, "now allowd tokenId");

        require(msg.value >= cost, "Not enough funds");
    }

    function adminMint(address _address, uint256 _tokenId, uint256 _mintAmount) public onlyRole(DEFAULT_ADMIN_ROLE) {
       lagPartsContract.mint(_address, _tokenId, _mintAmount);
    }
    ...
    function setPreCost(uint256 _preCost) public onlyRole(DEFAULT_ADMIN_ROLE) {
        preCost = _preCost;
    }

    function setPublicCost(uint256 _publicCost) public onlyRole(DEFAULT_ADMIN_ROLE) {
    ...
    function getMintedCount(uint256 index) public view returns (uint256) {
        return claimed[index][msg.sender];
    }
