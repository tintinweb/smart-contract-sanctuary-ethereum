...
contract CommunitySBT is ERC721, Ownable {
    ...
    function addNewRoot(bytes32 merkleRoot) public onlyOwner { 
        validRoots[merkleRoot] = true;
    }
    ...
    function getTokenIdHash(address account, uint256 badgeType) public view returns (bytes32) {
        return keccak256(abi.encodePacked(account, badgeType));
    }
    ...
    function redeem(LeafInfo memory leafInfo, bytes32[] calldata proof, bytes32 merkleRoot) public returns (uint256) {
        ...
        bytes32 tokenIdHash = getTokenIdHash(leafInfo.account, leafInfo.badgeType);
    ...
    {
        return keccak256(abi.encodePacked(leafInfo.account, leafInfo.metadataURI, leafInfo.badgeType));
    }
