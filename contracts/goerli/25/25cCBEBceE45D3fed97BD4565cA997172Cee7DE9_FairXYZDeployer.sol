...
contract FairXYZDeployer is ERC721xyzUpgradeable, AccessControlUpgradeable, MulticallUpgradeable, ReentrancyGuardUpgradeable {
    ...
    function _initialize(
        ...
        baseURI = URIs_[1];
        pathURI = URIs_[2];
        globalMintsPerWallet = globalMintsPerWallet_;
    ...
    function _setStages(stageData[] calldata stages, uint256 startId) internal returns(uint256) {
        ...
        // is moved to the future after it has already commenced
        ...
        for (uint256 i = startId; i < startId + length; ) {
            ...
            if(i > 0 && stageMap[i].phaseLimit < stageMap[i-1].phaseLimit) revert LessNFTsOnSaleThanBefore();
            ...
            if(stageMap[i].endTime <= block.timestamp) revert EndTimeInThePast();
    ...
    function mint(
        ...
        if (!signatureReleased) {
            ...
            // Set a time limit of 20 blocks for the signature
