...
contract MarketPlaceScientist is
    TimelockAccess,
    OwnableUpgradeable,
    IMarketPlaceScientist
{
    ...
    function initialize(address _timelock, address _scientistToken)
        external
        initializer
    {
        __Ownable_init();
    ...
    function setScientistToken(address _address)
        external
        override
        onlyTimelock
    {
        ...
        override
    {
        ...
        IERC721Upgradeable(scientistToken).safeTransferFrom(
            _oldOwner,
            msg.sender,
            _tokenId,
            ""
        ...
        override
    {
        ...
        require(
            msg.sender == IERC721Upgradeable(scientistToken).ownerOf(_tokenId),
        ...
        returns (address[] memory owners, uint256[] memory scientists)
    {
        ...
        owners = new address[](scientists.length);

        for (uint256 i = 0; i < scientists.length; i++) {
            owners[i] = IERC721Upgradeable(scientistToken).ownerOf(
                scientists[i]
            );
        }
    ...
    function withdrawTokens(address token, address to) external onlyTimelock {
        ...
        IERC20Upgradeable(token).transfer(to, balance);
    }
}
