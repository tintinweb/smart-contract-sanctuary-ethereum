...
import "../pioneers/PioneerMinter.sol";
import "../interfaces/ILand.sol";

contract LandMinter is Minter {
    LandParcels public land;
    PioneerMinter public pioneerMinter;

    constructor(
        address contractControlListAddr_,
        address landAddress_,
        address pioneerMinterAddress_
    ) Minter(contractControlListAddr_) {
        prevalidateAddresses(
            landAddress_,
            pioneerMinterAddress_
        );

        land = LandParcels(landAddress_);
        pioneerMinter = PioneerMinter(pioneerMinterAddress_);
    }

    function prevalidateAddresses(
        address landAddress_,
        address pioneerMinterAddress_
    ) internal pure {
        address addressZero = address(0);
        require(landAddress_ != addressZero, "LandMinter: landAddress zero");
        require(
            pioneerMinterAddress_ != addressZero,
            "LandMinter: pioneerMinterAddress zero"
        );
    }
            ...
            keccak256(
                abi.encodePacked(
                    ...
                    keccak256(
                        abi.encode(
                            keccak256(
                                "mintLand(address to_, uint256 landId_, uint256 nativePrice_, ILand.MintParams memory mintParams_, bytes memory signature_)"
                            ),
                            to_,
                            landId_,
                            mintParams_,
    ...
    function beforeLandMinting_(address to_, uint256 pioneerId_, ILand.PurchaseType purchaseType_) internal view {
        if (purchaseType_ == ILand.PurchaseType.DirectMint) {
            require(
                !pioneerMinter.activePioneer().exists(pioneerId_),
                "LandMinter: Active Pioneer with particular Id exists"
            );
        } else {
            require(
                pioneerMinter.activePioneer().ownerOf(pioneerId_) == to_,
                "LandMinter: Addr is not owner of the pioneer"
            );
        }
    }

    function _updateFounderStatus(address to_, uint256 pioneerId_, ILand.PurchaseType purchaseType_) internal {
        if (purchaseType_ == ILand.PurchaseType.DirectMint) {
            pioneerMinter.mintFounder(to_, pioneerId_);
        } else {
            pioneerMinter.burnActivePioneerForFounder(pioneerId_);
        }
    }

    /**
     * @dev Payable public function which allows user to buy Land in exchange for ActivePioneer.
    ...
    function mintLand(
        ...
        beforeLandMinting_(to_, mintParams_.founderId, mintParams_.purchaseType);
        ...
        _updateFounderStatus(to_, mintParams_.founderId, mintParams_.purchaseType);
...
interface ILand {
    ...
    function terrain(uint256 tokenId) external view returns(string memory);
}
...
contract Minter {
    ...
    constructor(address contractControlListAddr_) {
        require(contractControlListAddr_ != address(0), "");
        contractControlList = ContractControlList(contractControlListAddr_);
    }
    ...
    function withdraw() external {
        contractControlList.checkRole(0x0, msg.sender);
...
contract LandParcels is DotEarthERC721, ILand {
    ...
    string[] internal terrains = ["Dryland", "Midland", "Highland", "Coastland"];
    ...
    constructor(
        ...
        address payable dotEarthRoyaltyAddr,
        string memory name_,
    ...
    function _mintTo(address to_, uint256 tokenId_, MintParams memory mintParams_) internal {
        require(mintParams_.terrainType < terrains.length, "Land: terrain type incorrect");
    ...
    function mintTo(address to_, uint256 tokenId_, MintParams memory mintParams_) external override onlyMinterRole {
    ...
    function _afterTokenTransfer(
        ...
        if (from == address (0)) { // after mint
            require(founder.ownerOf(params[tokenId].founderId) == to, "Land: Incorrect Founder owner");
        } else { // after transfer
    ...
    function setRoyalty(address payable dotEarthRoyaltyAddress_) external override onlyAdminRole {
        _setRoyalty(dotEarthRoyaltyAddress_);
    }
    ...
    function toggleStaking(uint256 tokenId) external override {
        require(_isApprovedOrOwner(msg.sender, tokenId), "Land: not approved or owner");
    ...
    function getParams(uint256 tokenId) external view override returns(MintParams memory) {
        return params[tokenId];
    }

    function isPremiumLand(uint256 tokenId) external view override returns(bool) {
    ...
    function purchaseType(uint256 tokenId) external view override returns (PurchaseType) {
    ...
    function name(uint256 tokenId) external view override returns(string memory) {
        ...
        if (bytes(tokenName).length == 0) {
            return string(abi.encodePacked("Parcel ", Strings.toString(tokenId)));
        }

        return tokenName;
    }

    function terrain(uint256 tokenId) external view override returns(string memory) {
        return terrains[params[tokenId].terrainType];
    }
}
...
import "../interfaces/IPioneer.sol";
import "../ContractControlList.sol";
import "../Minter.sol";

contract PioneerMinter is Minter {
    IPioneer public activePioneer;
    IPioneer public founder;
    ...
    constructor(
        address contractControlListAddr_,
        address activePioneerAddr_,
        address founderAddr_,
        uint16 maxMintPerWallet_
    ) Minter(contractControlListAddr_) {
        prevalidateAddresses_(activePioneerAddr_, founderAddr_);
        activePioneer = IPioneer(activePioneerAddr_);
        founder = IPioneer(founderAddr_);
        maxMintPerWallet = maxMintPerWallet_;
    }

    function prevalidateAddresses_(
        address activePioneerAddr_,
        address founderPioneerAddr_
    ) internal pure {
        address zeroAddress = address(0);
        require(
            activePioneerAddr_ != zeroAddress,
            "PioneerMinter: ActivePioneer Address zero"
        );
        require(
            founderPioneerAddr_ != zeroAddress,
            "PioneerMinter: Founder Address zero"
        );
    }

    modifier onlyMinterRole() {
        contractControlList.checkPioneerMinterRole(msg.sender);
        _;
    }

    function getMintActivePioneerHash(
        address to_,
        uint256 nativePrice_,
        uint256 pioneerId_
    ) public pure returns (bytes32) {
            ...
            keccak256(
                abi.encodePacked(
                    ...
                    keccak256(
                        abi.encode(
                            keccak256(
                                "mintActivePioneer(address to_, uint256 nativePrice_, uint256 pioneerId_, bytes memory signature_)"
                            ),
                            to_,
                            nativePrice_,
                            pioneerId_
    ...
    function beforeMintActivePioneer_(address to_, uint256 nativePrice_) internal {
        require(
            mintedPerWallet[to_] < maxMintPerWallet,
            "Cannot mint more Pioneers"
        );
        require(msg.value == nativePrice_, "Wrong ETH amount sent");
        mintedPerWallet[to_] += 1;
    }

    function mintActivePioneer(
        address to_,
        uint256 nativePrice_,
        uint256 pioneerId_,
        bytes memory signature_
    ) external payable {
        beforeMintActivePioneer_(to_, nativePrice_);

        bytes32 messageHash = getMintActivePioneerHash(
            to_,
            nativePrice_,
            pioneerId_
        );
        contractControlList.checkPioneerMinterRole(
            _getSigner(messageHash, signature_)
        );

        activePioneer.mintTo(to_, pioneerId_);
    }

    function burnActivePioneerForFounder(uint256 activePioneerId_) external {
        contractControlList.checkLandMinterRole(msg.sender);

        address owner = activePioneer.ownerOf(activePioneerId_);
        activePioneer.burn(activePioneerId_);
        founder.mintTo(owner, activePioneerId_);
    }

    function burnActivePioneer(uint256 activePioneerId_) external {
        contractControlList.checkLandMinterRole(msg.sender);
...
contract ContractControlList is AccessControl {
    ...
    bytes32 public constant PIONEER_ADMIN_ROLE =
        keccak256("PIONEER_ADMIN_ROLE");
    ...
    bytes32 public constant PIONEER_MINTER_ROLE =
        keccak256("PIONEER_MINTER_ROLE");
...
abstract contract DotEarthERC721 is ERC721Royalty, IDotEarth721 {
    ...
    constructor(
        address accessControllerAddr_,
        address payable dotEarthRoyaltyAddr_,
        string memory name_,
        string memory symbol_,
        string memory baseURI_
    ) ERC721(name_, symbol_) {
        prevalidateAddresses_(accessControllerAddr_, dotEarthRoyaltyAddr_);

        accessController = ContractControlList(accessControllerAddr_);
        baseURI = baseURI_;

        _setRoyalty(dotEarthRoyaltyAddr_);
    }
    ...
    function setAccessController(ContractControlList accessController_) external onlyAclAdminRole {
        accessController = accessController_;
    }

    function _setRoyalty(address payable dotEarthRoyaltyAddr_) internal {
        DotEarthRoyalty royalty_ = DotEarthRoyalty(dotEarthRoyaltyAddr_);
        prevalidateDenominators_(royalty_);
        royalty = royalty_;
        _setDefaultRoyalty(dotEarthRoyaltyAddr_, royalty.sumOfNumerators());
    }
    ...
    function prevalidateAddresses_(
        ...
        require(
            accessControllerAddr != address(0),
            "Pioneer: accessControllerAddr zero"
        );
        require(
            dotEarthRoyaltyAddr != address(0),
            "Pioneer: dotEarthRoyaltyAddr zero"
        );
    }
    ...
    function prevalidateDenominators_(DotEarthRoyalty royalty_) internal pure {
        require(
            ...
            "Pioneer: Royalty denominator invalid"
        );
    }
    ...
    function setRoyalty(address payable dotEarthRoyaltyAddress_) external virtual;
}
