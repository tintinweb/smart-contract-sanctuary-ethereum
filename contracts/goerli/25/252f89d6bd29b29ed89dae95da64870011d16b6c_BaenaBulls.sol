...
contract BaenaBulls is ERC721A, Ownable, DefaultOperatorFilterer {
    using Strings for uint256;
    ...
    uint256 public publicCost = 500 ether;


    constructor(string memory _initBaseURI) ERC721A("Baena Bulls", "BB") {
        setBaseURI(_initBaseURI);
    }

    modifier allowMintCompliance(uint256 quantity) {
    ...
    // public mint
    function mint(uint256 quantity) external payable publicMintCompliance(quantity) {
        _safeMint(msg.sender, quantity);
    }

    function crossmint(address _to) public payable {
        if (preSale) {
            require(presaleCost >= msg.value, "Incorrect value sent");
        } else {
            require(publicCost >= msg.value, "Incorrect value sent");
        }
