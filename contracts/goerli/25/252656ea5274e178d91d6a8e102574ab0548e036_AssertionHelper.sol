...
// File: contracts/AssertionHelper.sol
...
contract AssertionHelper{
    function getBlockHash(Assertion calldata assertion) external pure returns(bytes32 hash){
        hash = GlobalStateLib.getBlockHash(assertion.afterState.globalState);
    }
    function getSendRoot(Assertion calldata assertion) external pure returns(bytes32 root){
        root = GlobalStateLib.getSendRoot(assertion.afterState.globalState);
    }
}