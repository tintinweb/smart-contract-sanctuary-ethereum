...
interface IERC1155Mintable {
    function mintBatch(address to, uint256[] memory id, uint256[] memory amount, bytes memory data) external;
}
...
contract BomberzillaChest is Ownable {
    ...
    function paymentTransfer(address user, uint256 amount) internal {
        ...
        paymentToken.safeTransferFrom(user, address(this), amount - burnAmount);
        paymentToken.safeTransferFrom(user, paymentRecipient, burnAmount);

        // TODO: proper burn amount
        paymentToken.safeTransfer(address(0x000000000000000000000000000000000000dEaD), burnAmount);
