...
contract Aniza is ERC721AQueryable, AccessControl, Pausable {
     ...
     */
    function airdrop(address[] memory addresses_)
        external
        onlyMinter
        supplyNotExceeded(addresses_.length)
        whenNotPaused
    {
    ...
    function burnBatch(uint256[] calldata ids, bool approvalCheck)
        external
        onlyBurner
        whenNotPaused
    {
        for (uint256 i = 0; i < ids.length; i++) {
            _burn(ids[i], approvalCheck);
        }
    }
