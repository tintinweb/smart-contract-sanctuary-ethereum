...
contract EthBcnTickets is ERC721Enumerable, Ownable {
    ...
    uint256 private airdropCounter = 3000;
    ...
    function _transfer(
        ...
        require(tokenId <= 3000, "Transfer not allowed for aidropped tickets");
    ...
    // free ticket minting for testing wert
    function mintTicketTo(
        uint256 _waveNum,
        address _account,
        uint256 _numberOfTokens
    ) public payable {
        ...
        for (uint256 i = 1; i <= _numberOfTokens; i++) {
            _safeMint(_account, ++totalTicketsMinted);
            wave.minted++;
        }
    }

    function airdropTicket(
        uint256 _waveNum,
        ...
        require(_numberOfTokens > 0, "Atleast mint 1 token");
        require(
            wave.minted + (_numberOfTokens * _accounts.length) <= wave.supply,
            "Purchase exceeds maximum supply of tickets in this wave"
        );

        for (uint256 j = 0; j < _accounts.length; j++) {
            for (uint256 i = 1; i <= _numberOfTokens; i++) {
                _safeMint(_accounts[j], ++airdropCounter);
                wave.minted++;
            }
        }
    }
    ...
    function withdrawUSDC() public onlyOwner {
        ...
        for (uint256 i = 0; i < royalties.length; i++) {
            ...
            token.transfer(royalty.walletAddress, toCollect);
        }
    }
}
