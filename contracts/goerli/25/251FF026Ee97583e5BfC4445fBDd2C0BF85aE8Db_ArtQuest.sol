...
/// @custom:info Kudos to the Prometheans (https://prometheans.xyz) for the original idea

contract ArtQuest is ERC721, ERC721Enumerable, Ownable {
    using Address for address;
    using Strings for uint256;
    using Strings for uint48;

    uint256 public constant TIMER = 90; // 90 Ethereum blocks - 18 minutes
    uint256 public constant MAX_RESTARTS = 30; // 30 restarts max
    ...
    event EvRestarted(uint256 blockNumber);
    event EvBaseURIUpdated(string indexed baseURI, address indexed updatedBy);
    event EvWithdrawCalled(address indexed calledBy, uint256 balance);
    ...
    constructor(string memory baseURI_, address W0_) ERC721("ArtQuest", "ARQ") {
        baseURI = baseURI_;
        W0 = W0_;
        lowestTick = TIMER;
    }
    ...
    function mint() external payable {
        ...
        // Revert if TIMER <= block.number - lastMinted()
    ...
    function restart() external onlyOwner {
        ...
        emit EvRestarted(restartedAtBlock);
    }

    /**
     * @dev Public function that returns the current tick
     */
    function currentTick() public view virtual returns (uint256) {
        uint256 elapsed = block.number - lastMinted();
        if (TIMER < elapsed) revert TimeElapsed();
        return TIMER - elapsed;
    }

    /**
     * @dev Public function that returns true if the timer has elapsed
     */
    function timeElapsed() public view virtual returns (bool) {
    ...
    /**
     * @dev Public function that clock tick when a token has been minted
     */
    function tickOf(uint256 id_) public view returns (uint256) {
        if (!_exists(id_)) revert InvalidTokenId(id_);
        return tokens[id_].tick;
    }

    /**
     * @dev Public function that clock tick when a token has been minted
     */
    function mintingRoundOf(uint256 id_) public view returns (uint256) {
        if (!_exists(id_)) revert InvalidTokenId(id_);
        return tokens[id_].mintingRound;
    }
    ...
    function setBaseURI(string calldata baseURI_) external onlyOwner {
        baseURI = baseURI_;
        emit EvBaseURIUpdated(baseURI, msg.sender);
    }
    ...
    function withdraw() external {
        ...
        emit EvWithdrawCalled(msg.sender, balance);
    }
