...
contract CToken is CTokenInterface, Exponential, TokenErrorReporter {
    ...
    /**
     * @notice Accrues interest and reduces all reserves by transferring to admin
     * @return uint 0=success, otherwise a failure (see ErrorReporter.sol for details)
     */
    function _reduceReserves() external nonReentrant returns (uint) {
        uint error = accrueInterest();
        if (error != uint(Error.NO_ERROR)) {
            // accrueInterest emits logs on errors, but on top of that we want to log the fact that an attempted reduce reserves failed.
    ...
    function _reduceReservesFresh(uint reduceAmount) internal returns (uint) {
        ...
        // doTransferOut reverts if anything goes wrong, since we can't be sure if side effects occurred.
        doTransferOut(IComptroller(address(comptroller)).treasury(), reduceAmount);
...
contract CTokenInterface is CTokenStorage {
    ...
    function _reduceReserves() external returns (uint);
    function _setInterestRateModel(InterestRateModel newInterestRateModel) public returns (uint);
}
...
interface IComptroller {
    ...
    /// @notice The address of the treasury which collects and distributes reserves
    function treasury() external view returns (address payable);

    /// @notice The address of the governance gNFT token
...
contract ComptrollerV7Storage is ComptrollerV6Storage {
    ...
    /// @notice The address of the Tonpound Participation Index token
