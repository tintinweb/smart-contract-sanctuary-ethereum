...
/**
 * 1) Made mint directly on user
 * 2) Made some code improvement
 * 3) Used OpenZeppelin ERC721Royalty library instead of custom one
 * 4) Removed couponCode and everything connected with it. If it is a necessary function, then it can be returned later
 * 5) Removed duplicate with ipfs fields associated with the product and the NFT
 * 6) Reduced product object size and added some suggestions (in test file) about generating token ids
 * 7) Changed the contract variant to upgradable and added the ability to deploy through a beacon and beaconProxy (this feature can be applied to any contract)
 * 8) Added roles
 */


contract GeerProductsVeryOptimizedWitCombineTokenIdWithProxyWithRoles is
  ERC721Upgradeable,
  ...
  AccessControlUpgradeable
{
  ...
  function initialize(string calldata tokenName, string calldata tokenSymbol) external initializer  {
  ...
  function setRoyalties(address recipient, uint96 royaltyFee)
    ...
    onlyOwner
  {
    require(recipient != address(0), "INVALID_WALLET");
    require(royaltyFee <= _feeDenominator(), "INVALID_FEE");
