...
contract StakingContract is Ownable, ReentrancyGuard {
    ...
    uint256 constant DAY = 30 seconds; // for test
    ...
    function pendingReward(address _user, uint256 _tokenId) public view returns (uint256) {
        ...
        (bool _isStaked, uint256 _startTime, uint256 _createTime) = getStakingItemInfo(_user, _tokenId);
        if(!_isStaked) return 0;
        uint256 _currentTime = block.timestamp;
        
        uint256 claimAmount = rewardAmountFromStakingDuration(_createTime, _startTime);
        uint256 rewardAmount = rewardAmountFromStakingDuration(_createTime, _currentTime);
        rewardAmount = rewardAmount - claimAmount;

        return rewardAmount;
    }

    function rewardAmountFromStakingDuration(uint256 _startTime, uint256 _endTime) public view returns(uint256){
        uint256 period = _endTime - _startTime;
        uint256 rewardAmount = 0;
        if(period < DAY * 30) {
            rewardAmount = period / DAY * RewardTokenPerDay;
        } else if(period < DAY * 100) {
            rewardAmount = 30 * RewardTokenPerDay + (period / DAY - 30) * RewardTokenPerDay * 5 / 4;
        } else if(period < DAY * 180) {
            rewardAmount =  30 * RewardTokenPerDay + 70 * RewardTokenPerDay * 5 / 4 + 
                            RewardTokenPerDay * (period / DAY - 100) * 3 / 2;
        } else if(period < DAY * 365) {
            rewardAmount =  30 * RewardTokenPerDay + 70 * RewardTokenPerDay * 5 / 4 + 
                            RewardTokenPerDay * 80 * 3 / 2 + RewardTokenPerDay * (period / DAY - 180) * 2;
        } else {
            rewardAmount =  30 * RewardTokenPerDay + 70 * RewardTokenPerDay * 5 / 4 + 
                            RewardTokenPerDay * 80 * 3 / 2 + RewardTokenPerDay * 185 * 2 + 
                            RewardTokenPerDay * (period / DAY - 365) * 3;
        }
        return rewardAmount;
    }
    ...
    function getStakingItemInfo(address _user, uint256 _tokenId) public view returns(
        bool _isStaked, uint256 _startTime, uint256 _createTime
    ) {
        for(uint256 i = 0; i < userInfo[_user].length; i++) {
            if(userInfo[_user][i].tokenId == _tokenId) {
                _isStaked = true;
                _startTime = userInfo[_user][i].startTime;
                _createTime = userInfo[_user][i].createTime;
                break;
            }
        }
    }
