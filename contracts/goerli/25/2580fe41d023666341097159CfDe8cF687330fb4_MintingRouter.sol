...
interface INFT {
  ...
  function MAX_RESERVED_SUPPLY() external view returns (uint256);

  function totalSupply() external view returns (uint256);

  function burnTokens(uint256[] calldata tokenIds, address forOwner) external;
}
...
contract MintingRouter is EIP712Whitelisting, ReentrancyGuard, Ownable {
  ...
  // mapping saleRoundIndex to price
  mapping(uint256 => uint256) public salePricePerRound;
  ...
  constructor(
    ...
    _setWhitelistSigningAddress(msg.sender);
  }
  ...
  function createSaleRound(
    ...
    salePricePerRound[currentSaleIndex] = price;
  ...
  function allowedTokenCount(address minter) public view returns (uint256) {
    ...
    uint256 allowedWalletCount = saleRound.limitAmountPerWallet != UNLIMITED_AMOUNT
      ? (saleRound.limitAmountPerWallet > _mintedAmountPerAddress[currentSaleIndex][minter] ? saleRound.limitAmountPerWallet - _mintedAmountPerAddress[currentSaleIndex][minter] : 0)
    ...
    uint256 availableTokenCount = saleRound.totalAmount != UNLIMITED_AMOUNT
      ? (saleRound.totalAmount > mintedAmountPerRound[currentSaleIndex] ? saleRound.totalAmount - mintedAmountPerRound[currentSaleIndex] : 0)
      : _nftContract.MAX_SUPPLY() - _nftContract.totalSupply();
    // Calculate the limit of the number of tokens per single mint.
    uint256 allowedAmountPerMint = saleRound.maxAmountPerMint != UNLIMITED_AMOUNT ? saleRound.maxAmountPerMint : _nftContract.MAX_SUPPLY() - _nftContract.totalSupply();
  ...
  function tokensLeft() public view returns (uint256) {
    ...
    return
      saleRound.totalAmount != UNLIMITED_AMOUNT
        ? (saleRound.totalAmount > mintedAmountPerRound[currentSaleIndex] ? saleRound.totalAmount - mintedAmountPerRound[currentSaleIndex] : 0)
        : _nftContract.MAX_SUPPLY() - _nftContract.totalSupply();
  }
  ...
  function setRefund(bool flag) external onlyOwner {
    shouldRefund = flag;
  }

  function _findSalePriceOfToken(uint256 tokenId) internal view returns (uint256) {
    uint256 maxReservedSupply = _nftContract.MAX_RESERVED_SUPPLY();
    if (tokenId < maxReservedSupply) {
      revert("refund not allowed");
    }

    uint256 supply = maxReservedSupply;
    for (uint256 i = 0; i <= currentSaleIndex; i++) {
      supply += mintedAmountPerRound[i];
      if (tokenId < supply) {
        return salePricePerRound[i];
      }
    }
  ...
  function refund(uint256[] calldata tokenIds) external nonReentrant {
    require(shouldRefund, "Refund is disabled");
    _nftContract.burnTokens(tokenIds, msg.sender);
    // loop through tokenIds array

    uint256 refundAmount = 0;
    for (uint256 i = 0; i < tokenIds.length; i++) {
      uint256 tokenId = tokenIds[i];
      uint256 price = _findSalePriceOfToken(tokenId);
      refundAmount += price;
    }

    // transer refundAmount to msg.sender
