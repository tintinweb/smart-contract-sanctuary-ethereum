...
/// @title  ERC721WithEIP2981
/// @author  Illuststrative Space
...
import "./ERC721WithEIP2981/overrides/RoyaltyOverrideCore.sol";
import "./meta-transactions/NativeMetaTransaction.sol";
import "./meta-transactions/ContentMixin.sol";

/// @notice This contract is a derivative of the ERC721 contract with EIP2981 support.
/// @dev  Support for royalties handled through manifolds.xyz creator-core-solidity https://github.com/manifoldxyz/creator-core-solidity
/// @dev  Support for meta-transactions and open sea approved transactions
...
{
    /// ERRORS ///
    error TokenURIFrozen();

    /// @notice a list of all the frozen token IDs
    mapping(uint256 => bool[]) public frozenTokenIds;
    ...
    /// @dev Only configurable by contract owner.
    ...
    /// @notice public function to mint a token to a given address
    /// @param to The address to send the token to
    /// @param tokenId The unique identifier of the token to mint
    /// @param uri The data to send with the token
    /// @param royaltyRecipient The address to receive royalties
    /// @param bps The number of basis points to assign to the recipient where 1bps = .01%
    function mint(
        address to,
        uint256 tokenId,
        string memory uri,
        ...
        _setTokenRoyalty(tokenId, royaltyRecipient, bps);
    }
    ...
    /// @notice freezes the uri of a given token
    /// @notice The contract owner can freeze the uri of a token
    /// @notice the uri cannot be changed once frozen
    /// @dev Only callable by the token owner
    /// @dev change the token id mapping to true
    /// @param tokenId The unique identifier of the token to freeze
    function freezeTokenURI(uint256 tokenId) public whenNotPaused onlyOwner {
        frozenTokenIds[tokenId].push(true);
    }

    /// @notice change the uri of a given token
    /// @notice Cannot be changed once frozen
    /// @dev Only callable by the contract owner
    /// @param tokenId The unique identifier of the token to set the uri of
    /// @param uri The data to send with the token
    function setTokenURI(uint256 tokenId, string memory uri)
        ...
        onlyOwner
    {
        bool frozen = frozenTokenIds[tokenId].length > 0;
        if (frozen) {
            revert TokenURIFrozen();
        }
        _setTokenURI(tokenId, uri);
    }

    /// @notice returns if a token is frozen
    function isTokenURIFrozen(uint256 tokenId) public view returns (bool) {
        return frozenTokenIds[tokenId].length > 0;
    }
 ...
 */
abstract contract EIP2981RoyaltyOverrideCore is
    IEIP2981,
    ...
    ERC165
{
    ...
    /**
     * @dev Sets token royalties for a single token. When you override this in the implementation contract
     * ensure that you access restrict it to the contract owner or admin
     */
    function _setTokenRoyalty(
        ...
        if (recipient == address(0)) {
            ...
            _tokensWithRoyalties.remove(tokenId);
            emit TokenRoyaltyRemoved(tokenId);
        } else {
            _tokenRoyalties[tokenId] = TokenRoyalty(recipient, bps);
            _tokensWithRoyalties.add(tokenId);
