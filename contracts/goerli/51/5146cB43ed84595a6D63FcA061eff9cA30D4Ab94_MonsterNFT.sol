...
contract MonsterNFT is OwnableInitializable, ERC721Checkpointable {
    ...
    mapping(uint256 => uint256) public capsuleType;
    mapping(uint256 => uint256) public points;
    mapping(uint256 => uint256) shadows;
    mapping(uint256 => RevealDetail) public revealDetails;

    // monsterId => attributeId
    ...
    function _setCategory(uint256 _id, uint256 _cat) public {
        require(enabledMinter[msg.sender], "!minter");
        require(!paused, "paused");

        monsterCategory[_id] = _cat;
    }

    function setRevealDetails(uint256 _id, uint256 _revealTime) external {
        require(enabledMinter[msg.sender], "!minter");
        revealDetails[_id] = RevealDetail({
            id: _id,
    ...
    function setMisc(uint256[] calldata _ids, uint256[] calldata _values)
        public
        onlyOwner
    {
        require(
    ...
    function increasePoints(uint256 id, uint256 val) external {
        require(enabledMinter[msg.sender], "Not minter!");
        points[id] = points[id] + val;
    }

    function decreasePoints(uint256 id, uint256 val) external {
        require(enabledMinter[msg.sender], "Not minter!");
        points[id] = points[id] - val;
    }

    function setShadow(uint256 _id, uint256 _shadow) external {
        require(enabledMinter[msg.sender], "Not allowed!");
        shadows[_id] = _shadow;
    }

    function getRevealDetails(uint256 _id)
        public
        view
        returns (RevealDetail memory)
    {
        return revealDetails[_id];
    }

    function getShadow(uint256 _nft) public view returns (uint256) {
        uint256 revealTime = revealDetails[_nft].revealTime * 1 days;
        require(
            block.timestamp >= revealDetails[_nft].mintDate + revealTime,
            "too soon"
        );
        return shadows[_nft];
    }

    function getPoints(uint256 _id) public view returns (uint256) {
        return points[_id];
    }

    function setMonsterCategory(
    ...
    function setAttributes(uint256 _id, uint256 _attributes) external {
        require(enabledMinter[msg.sender], "Not allowed!");
 ...
 * the owner.
 */
abstract contract OwnableInitializable is Context {
