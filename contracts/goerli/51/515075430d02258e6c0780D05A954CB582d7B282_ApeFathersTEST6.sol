...
contract ApeFathersTEST6 is ERC721A, ERC721AQueryable, Ownable, Pausable, ReentrancyGuard, ERC2981, ERC4907A {
    ...
    returns (bool)
{
    ...
    */
    ...
    function burnClaim(uint256[] calldata _tokenIds, uint256 maxTokensToBurn) external nonReentrant {
        ...
        uint256[] memory approvedTokens = new uint256[](numberOfTokens);
        ...
        for (uint256 i = 0; i < numberOfTokens; i++) {
            ...
            approvedTokens[i] = tokenId;
        }

        for (uint256 i = 0; i < numberOfTokens; i++) {
            uint256 tokenId = approvedTokens[i];
    ...
    function setApprovaltoALLForBurn(uint256[] memory _tokenIds) external {
        uint256 numberOfTokens = _tokenIds.length;
        for (uint256 i = 0; i < numberOfTokens; ++i) {
            uint256 tokenId = _tokenIds[i];
            require(IERC721A(tokensToBurnAddr).ownerOf(tokenId) == _msgSender(), "TOKEN NOT OWNED BY CALLER OR INVALID TOKEN ID");
            IERC721A(tokensToBurnAddr).setApprovalForAll(address(this), true);
        }
        emit ApprovalComplete(_msgSender(), _tokenIds);
    }
