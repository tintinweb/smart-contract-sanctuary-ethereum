    ...
    ERC721DropStorageV1
{
    ...
    modifier canMintPublicSaleTokens(uint256 quantity) {
        if (quantity + _totalMinted() > config.publicSaleEditionSize) {
            revert Mint_SoldOut();
        }
    ...
    ///  @param _publicSaleEditionSize Number of editions that can be minted during public sale. If type(uint64).max, unlimited editions can be minted as an open edition.
    ///  @param _presaleEditionSize Number of editions that can be minted during presale. Presale editions have no impact on publicSaleEditionSize (mints during public sale).
    ...
    function initialize(
        ...
        config.publicSaleEditionSize = _publicSaleEditionSize;
        ...
        returns (IKitsERC721Drop.SaleDetails memory)
    {
        return
            IKitsERC721Drop.SaleDetails({
                ...
                totalPublicSaleMinted: _totalMinted() - presaleMints,
                totalPreSaleMinted: presaleMints,
                maxSupply: config.publicSaleEditionSize + config.presaleEditionSize,
    ...
    function mintedPerAddress(
        address minter
    ) external view override returns (IKitsERC721Drop.AddressMintDetails memory) {
        return
    ...
    function withdraw() external nonReentrant {
        ...
        emit FundsWithdrawn(
            ...
            0 // formerly zoraFee. we are not collecting a fee
        );
    }
    ...
    {
        if (config.publicSaleEditionSize != type(uint64).max) {
            revert Admin_UnableToFinalizeNotOpenEdition();
        }

        config.publicSaleEditionSize = uint64(_totalMinted());
        emit OpenMintFinalized(_msgSender(), config.publicSaleEditionSize);
    }
