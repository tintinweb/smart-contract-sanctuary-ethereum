...
contract UniswapConnector is Ownable {
    ...
    /* function swapTokenForETH(address _tokenIn, uint256 _amount, uint256 _amountOutMin, address _to) external returns(uint256) {
        ...
        if (routers[tokens[_tokenIn].router] != uniswapV3Router02) {
            uint256 amountOut = _swapV2(routers[tokens[_tokenIn].router], _tokenIn, _amount, _to);
            require(amountOut >= _amountOutMin, "ERR: amountOutMin");
            return amountOut;
        } else {
            bytes memory path = abi.encodePacked(_tokenIn, tokens[_tokenIn].fee, wethAddress);
            return _swapV3(path, _tokenIn, _amount, _amountOutMin, _to);
        }
    } */
    ...
    function _swapV3(bytes memory _path, address _tokenIn, uint256 _amount, uint256 _amountOutMin, address _to) private returns(uint256) {
        ...
        return IUniswapRouterV3(uniswapV3Router02).exactInput(params);
    }
}