    ...
    ReentrancyGuardUpgradeable
{
    ...
    /// @notice The minimum price an item can be listed for (equivalent to '0.01' if there are 6 decimal places)
    uint256 public constant MIN_PRICE = 1e4;
    ...
    function isListed(
        ...
        require(
            approvedItemContracts[nftAddress],
            "MachoMarket: Invalid contract address"
        );
        ContractType contractType = contractTypes[nftAddress];
        if (
            contractType == ContractType.ERC721 ||
