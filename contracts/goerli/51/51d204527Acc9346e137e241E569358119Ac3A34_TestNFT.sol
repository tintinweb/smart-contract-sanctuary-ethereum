...
contract TestNFT is ERC721A, ERC2981, Ownable, Pausable {
    using Strings for uint256;
    ...
    uint256 public constant MAX_SALE_SUPPLY = 2702;
    ...
    function firstPreMint(
        uint256 _mintAmount
    ) public payable whenMintable whenNotPaused {
        uint256 cost = preCost * _mintAmount;
        mintCheck(_mintAmount, cost);
        // TODO：本番は時間で制御
        require(presale, "First presale is not active.");
        require(firstPresale, "Second presale is not active.");
        require(
            whiteLists[msg.sender] < 1,
            "Already claimed max or don't have role"
        );
    ...
    function secondPreMint(
        ...
        require(
            whiteLists[msg.sender] >= _mintAmount,
            "Already claimed max or don't have WhiteList"
        );
        _mint(msg.sender, _mintAmount);
        whiteLists[msg.sender] -= _mintAmount;
    }
