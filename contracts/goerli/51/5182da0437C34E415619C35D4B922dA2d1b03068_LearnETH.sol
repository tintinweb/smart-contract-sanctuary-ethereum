...
contract LearnETH is ERC721, ReentrancyGuard, Ownable {
    ...
    event partnerAdded(uint256 partnerTokenId, uint256 referrerTokenId, uint256 level, uint256 position);
    event studentAdded(uint256 studentTokenId, uint256 referrerTokenId, uint256 father);
    event instructorAdded(uint256 instructorTokenId, uint256 referrerTokenId);
    event courseAdded(uint256 courseId, uint256 instructorTokenId);
    ...
    function initiatePartnerNetwork() external onlyOwner {
        ...
        emit partnerAdded(tokenId, 0, 1, 1);
    }
    ...
    function registerAsPartner(uint256 _referrerTokenId) public nonReentrant payable {
        ...
        uint256 instructorSharePerMonth = (msg.value).mul(30).div(100);
        uint256 partnerShare = (msg.value).mul(70).div(100);
        uint256 twelth = partnerShare.div(12);
        partnerCourseDetails[currentPartnerCourseId].balance+=instructorSharePerMonth;        
        nodeDetails[nodeDetails[tokenId].personalReferrer].balance+=twelth.mul(3);
        distributeUplineFee(tokenId, twelth.mul(9));

        emit partnerAdded(tokenId, _referrerTokenId, _level, _position);        
    }
    ...
    function upgradeStudentAsPartner(uint256 _tokenId) public {
        ...
        nodeDetails[_tokenId].father1 = partnerNodeTokenMapping[_level - 1][f_position];
        partnerNodeTokenMapping[_level][_position] = _tokenId;
        
        emit partnerAdded(_tokenId, currentReferrerTokenId, _level, _position);        
    }
    ...
    function registerInstructor(uint256 _referrerTokenId) public {
        ...
        emit instructorAdded(tokenId, _referrerTokenId); 
    }
    ...
    function addCourse(uint256 _tokenId, string memory _courseName, uint256 _courseFee) public {
        ...
        emit courseAdded(courseIndex, _tokenId);
    }
    ...
    function enrolForPartnerCourse(uint256 _tokenId, uint256 _months) public nonReentrant payable {
        ...
        //require(msg.value == _months.mul(partnerCourseFee), "Incorrect fee");
        uint256 instructorSharePerMonth = (msg.value).mul(30).div(100).div(_months);
        uint256 partnerShare = (msg.value).mul(70).div(100);

        for(uint i=0; i< _months; i++) {
            nodeDetails[_tokenId].subscribedTill = block.timestamp.add(monthDuration);
	...
	function estimateFeeDistribution(uint256 _paymentId) public view returns(earnings[] memory studentUpline, earnings[] memory instructorUpline) {
        ...
        earnings[] memory _studentUpline;
        ...
        earnings[] memory _instructorUpline;        
