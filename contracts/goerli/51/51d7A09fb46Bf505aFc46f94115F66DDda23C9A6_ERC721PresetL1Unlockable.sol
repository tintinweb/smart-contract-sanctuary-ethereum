...
contract ERC721PresetL1Unlockable is ERC721PresetL1 {
    uint256 public price;
    mapping(uint256 => bool) public unlocked;

    event SetPrice(uint256 price);
    event Withdraw(address indexed to, uint256 indexed amount);
    event Unlock(address indexed owner, uint256 indexed tokenID);
    ...
    modifier onlyUnlocked(uint256 tokenId) {
        require(unlocked[tokenId], "locked token");
        _;
    }

    /// @notice Sets price for an NFT.
    ///
    /// @notice Requirements:
    /// - It must be called by only admin.
    ///
    /// @notice Emits a {SetPrice} event.
    ...
    /// - It must be called by only owner or approved.
    ///
    /// @notice Emits a {Unlock} event.
    function unlock(uint256 tokenId) external payable {
        require(!unlocked[tokenId], "unlocked already");
    ...
    function _burn(uint256 tokenId) internal override onlyUnlocked(tokenId) {
        super._burn(tokenId);
    }
    ...
    function _transfer(
        ...
        address to,
        uint256 tokenId
    ) internal override onlyUnlocked(tokenId) {
...
contract ERC721PresetL1 is ERC721Settleable, Pausable, Ownable {
    ...
    /// @notice Disables state settlement.
    ///
    /// @notice Requirements:
    /// - It must be called by only admin.
    ///
    /// @notice Emits a {SettlementDisabled} event.
    function disableSettlement() external onlyAdmin {
        super._disableSettlement();
    }
...
abstract contract Settleable {
    ...
    function settle(
        ...
        require(msg.sender == bridge, "Settleable: not from bridge");
