...
contract GoerliSender{
    ...
    function Call(uint256 Age, string calldata Name, address Addy) public{
        Person memory ToEncode = Person(Age, Name, Addy);
        AnyCall(Anycall).anyCall(receiver, abi.encode(ToEncode), TBNBID, 0, '');
    }
...
interface AnyCall {
    function anyCall(address _to, bytes calldata _data, uint256 _toChainID, uint256 _flags, bytes calldata) external;
