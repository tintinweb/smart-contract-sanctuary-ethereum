...
contract TomiStates {
    ...
    /// @notice address of the auctioning contract
    ...
    mapping (uint256 => ProposalAuctionData) internal proposalsAuctionData;
...
contract TomiAuctionDao is TomiEvents, TomiStates, Ownable {
    using SafeMath for uint256;

    constructor(ITomiMainDao _tomiMainDao, address _treasury, IAuctionHouse _auctionHouse) {
    ...
    function getProposalsAuctionData(uint256 proposalId) external view returns (ProposalAuctionData memory) {
        return proposalsAuctionData[proposalId];
    }
    ...
    function proposeChangeAuction(
        ...
        require(9 <= functionId && functionId <= 11, "incorrect function id");
        ...
        ProposalAuctionData storage newProposalData = proposalsAuctionData[tomiMainDao.proposalCount()];
    ...
    function executeChangeAuction(uint256 proposalId) external {
        Proposal storage proposal = proposals[proposalId];

        require(9 <= proposal.functionId && proposal.functionId <= 11, "incorrect function id");
        ...
        if (state(proposalId) == ProposalState.Succeeded) {
            ...
            uint256 proposalChangeCriteria = proposalsAuctionData[proposal.id].proposalChangeCriteria;
            if (proposal.functionId == 9) {
                auctionHouse.setMinBidIncrementPercentage(uint8(proposalChangeCriteria));
            } else if (proposal.functionId == 10) {
                auctionHouse.setReservePrice(proposalChangeCriteria);
            } else if (proposal.functionId == 11) {
                auctionHouse.setTimeBuffer(proposalChangeCriteria);
            }
        }
    ...
    function testUpdateAuctionHouse(address _auctionHouse) external {
