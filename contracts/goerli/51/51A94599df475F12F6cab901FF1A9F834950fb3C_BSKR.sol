...
contract BSKR is BaseBSKR {
    ...
    bool internal addLPEnabled = true;
    ...
    // TODO add description
    function bestowReflection(uint256 tAmount) external {
        ...
        _rBalance[sender] -= response[uint8(Field.rAmount)];
    ...
    // TODO add description
    function setRfiFeePercent(uint16 newRfiBips)
        external
        onlyManager
    {
        // _rfiFee = newRfiBips;
    ...
    function _transferTokens(
        ...
        if (recipientExcluded) {
            _tBalance[recipient] += response[uint8(Field.tTransferAmount)];
        }
        _rBalance[recipient] += response[uint8(Field.rTransferAmount)];
        _reflectFee(
            response[uint8(Field.tRfiFee)],
    ...
    // TODO add description
    function setLBSKRContract(address newLBSKRContract)
        external
        onlyManager
    {
