...
contract BluechipSafe is ERC721Enumerable, Ownable, ReentrancyGuard {
	...
	constructor() ERC721("Bluechip Safe Token", "BCS")  {}
	...
	function mintVerificationTokenToSelf() public payable nonReentrant {
	...
	function verifyOwnershipOfTokens(address contractAddress, address primaryAddress) public view returns (uint256) {
		...
		for (uint256 i; i < verificationTokensOwned.length; i++) {
			address verificationTokenOwner = tokenIdToOwnerMapping[verificationTokensOwned[i]];
	...
	function getAllAssociatedWallets(address primaryAddress) public view returns(address[] memory) {
		uint256[] memory verificationTokensOwned = tokensInWallet(primaryAddress);
		address[] memory associatedWallets = new address[](verificationTokensOwned.length); 
		for (uint256 i; i < verificationTokensOwned.length; i++) {
			associatedWallets[i] = tokenIdToOwnerMapping[verificationTokensOwned[i]];
		}
		return associatedWallets;
	}

	//Returns the associated wallet of a particular BCS token.
	function getAssociatedWallet(uint256 verificationTokenId) public view returns (address) {
		return tokenIdToOwnerMapping[verificationTokenId];
	}

	//Returns the associated BCS token of primaryAddress.
	function getAssociatedVerificationToken(address primaryAddress) public view returns (uint256) {
	...
	function setBurnReplacePrice(uint256 _newBurnReplacePrice) public onlyOwner {
	...
	//Pause self mint.
	function pauseSelfMint(bool val) public onlyOwner {
