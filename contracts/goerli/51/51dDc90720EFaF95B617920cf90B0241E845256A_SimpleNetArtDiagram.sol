...
contract SimpleNetArtDiagram is Ownable, IERC721Metadata, ERC721Royalty {
    ...
    constructor() ERC721(NAME, "SNAD") {
     ...
     /**
     * @dev Contract owner may mint max supply to the contract owner
     */   
     ...
     */
    function tokenURI(uint256 _tokenId) public pure override(ERC721, IERC721Metadata) isValidTokenId(_tokenId) returns (string memory) {
        /* solhint-disable quotes */
        string memory edition = string (abi.encodePacked(tokenIdToString(_tokenId), '/3'));
        string memory metadata = Base64.encode(abi.encodePacked(
            '{"name":"', NAME, ' #', edition,
            '","description":"', DESCRIPTION, 
            '","image":"', SIMPLE_NET_ART_DIAGRAM,
            '","edition":"', edition,
            '","attributes": [{"trait_type":"Artist","value":"MTAA"},{"trait_type":"License","value":"CC0 1.0 ipfs://QmZcU7ZkmVSNfVZjsxoHSoCtw89Az5hmqufLPowZxCURn8"},{"trait_type":"Edition", "value":"', edition, 
        '"}]}'));
     ...
     */
    function tokenIdToString(uint256 _tokenId) internal pure  isValidTokenId(_tokenId) returns (string memory tokenId) {
        if (_tokenId == 1) {
            return "1";
        }
        if (_tokenId == 2) {
