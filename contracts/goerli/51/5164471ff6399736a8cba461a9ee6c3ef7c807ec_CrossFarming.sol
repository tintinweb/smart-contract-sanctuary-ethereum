...
contract CrossFarming is MessageSenderApp, MessageReceiverApp {
    ...
    function sendFarmMessage(
        ...
        uint256 feeBNB = (tx.gasprice * estimateGaslimit(_account, _msgType) * exchangeRate * marginRate) /
            (EXCHANGE_RATE_PRECISION * MARGIN_PRECISION);
        ...
        IMessageBus(messageBus).sendMessage{value: MessageBus(messageBus).calcFee(message) + 100}(_receiver, _dstChainId, message);

        // send message
        // (bool success, ) = messageBus.call{value: MessageBus(messageBus).calcFee(message) + 100}(
