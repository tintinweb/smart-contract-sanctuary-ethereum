...
contract Pioneers is IERC721, IERC721Metadata {
    ...
    function wagonToLetter(uint16 index) external view returns (bytes1) {
        return wagonToFirstLetter[index];
    }

    function getTokenCount() external view returns (uint256) {
        return tokenIndexToWagon.length;
    }
    ...
    function steer(
        ...
        bytes32 leaf = keccak256(
            abi.encodePacked(_wagonId, _items[0], _items[1], firstLetter)
        );
        ...
        require(
            wagonToToken[uint16(_wagonId)] == 0 &&
                items[uint16(_wagonId)][0] == 0 &&
                items[uint16(_wagonId)][1] == 0,
            'steered'
        );
        ...
        items[uint16(_wagonId)] = _items;
        tokenIndexToWagon.push(uint16(_wagonId));

        uint256 _tokenId = tokenIndexToWagon.length;
        balances[msg.sender]++;
        owners[_tokenId] = msg.sender;
        wagonToToken[uint16(_wagonId)] = _tokenId;
        tokenToTrail[_tokenId] = _trail;
        wagonToFirstLetter[uint16(_wagonId)] = firstLetter;

        emit Steer(uint16(_wagonId), _tokenId);
    ...
    function ambush(uint256 _ambusherTokenId, uint256 _victimTokenId) public {
        ...
        (bool _isVulnerable, bytes32 _inconvenienceId) = checkVulnerable(
            _victimTokenId,
            _lastAmbush
        );
        require(_isVulnerable, 'vulnerable');
    ...
    function changeTrail(uint256 _tokenId, uint8 _newTrailId) public payable {
        ...
        (bool _isVulnerable, ) = checkVulnerable(_tokenId, _lastAmbush);
        require(!_isVulnerable);
        ...
        returns (bool, bytes32)
    {
        ...
        if (_lastAmbush == encodeAmbush(_inconvenienceId))
            return (false, _inconvenienceId);
        ...
        returns (string memory)
    {
        ...
        uint256 _wagonId = tokenToWagon(_tokenId);
        ...
        //TODO:requires a suffix in production build
