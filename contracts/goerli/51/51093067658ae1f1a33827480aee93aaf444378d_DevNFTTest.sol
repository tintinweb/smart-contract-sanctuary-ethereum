...
contract DevNFTTest is DevCitiNFT {
    ...
    function initialize(NFTContractInitializer memory _initializer) 
        ...
        public 
    {
        __DevCitiNFT_init("NFTTest", "NFTTest", _initializer);
    }
}
...
contract DevCitiNFT is BaseCitiNFT, ERC721URIStorageUpgradeable {
     ...
     */
    function __DevCitiNFT_init(string memory _name, string memory _symbol, NFTContractInitializer memory _initializer) 
        internal 
        onlyInitializing 
    {
        require(!Utils.isMainnet(), "mainnet not allowed");
        __BaseCitiNFT_init(_name, _symbol, _initializer);
    ...
    function _beforeTokenTransfer(address from, address to, uint256 tokenId)
        ...
        override(BaseCitiNFT, ERC721Upgradeable)
    {
        BaseCitiNFT._beforeTokenTransfer(from, to, tokenId);
    }
    ...
    function supportsInterface(bytes4 interfaceId)
        ...
        returns (bool)
    {
        return BaseCitiNFT.supportsInterface(interfaceId);
    }

    /**
     * @dev See {BaseCitiNFT}, {ERC721URIStorageUpgradeable}
     */
    function _burn(uint256 tokenId)
        internal
        override(BaseCitiNFT, ERC721URIStorageUpgradeable)
    {
        BaseCitiNFT._burn(tokenId);
        // TODO: we're NOT clearing the token URI as calling 
        // ERC721URIStorageUpgradeable._burn reverts, this is because
        // the token is alreadt deleted in BaseCitiNFT._burn
     ...
     * See {ERC721URIStorageUpgradeable}, {BaseCitiNFT}
     */
        ...
        returns (string memory)
    {
        if (bytes(_baseTokenURI).length > 0) {
            return BaseCitiNFT.tokenURI(tokenId);
        } else {
     ...
     */
    function devMint(address to, string calldata uri) 
        public 
        returns (uint256) 
    {
     ...
     * Returns the minted tokenId.
     */
    function devMint2(address to, string calldata uri, uint256 numTokensToBurn) 
        public 
        returns (uint256) 
    {
        if (numTokensToBurn > 0) {
            require(_token != BaseToken(address(0)), "bad token contract");
            _token.burnFrom(_msgSender(), numTokensToBurn);
        }
        uint256 tokenId = _doSafeMint(to);
        _setTokenURI(tokenId, uri);
        return tokenId;
     ...
     * Returns the minted tokenId.
     */
    function devMintWithTokenId(address to, string calldata uri, uint256 tokenId) 
        public 
        returns (uint256) 
    {
     ...
     */
    function devUpdateTokenURI(uint256 tokenId, string calldata newURI) 
        public 
    {
                        ...
                        Versioned 
{
     ...
     */
    function __BaseCitiNFT_init(string memory _name, string memory _symbol,  NFTContractInitializer memory _initializer) 
        internal 
        onlyInitializing 
    {
        ...
        returns (string memory) 
    {
        ...
        if (bytes(_baseTokenURI).length > 0) {
            // returns <base-uri>/<chain-id>/<contract-name>/<contract-address>/<token-id>
    ...
    /**
     * @dev See {ERC721PausableUpgradeable}, {ERC721EnumerableUpgradeable}
     */
    function _beforeTokenTransfer(address from, address to, uint256 tokenId)
        internal
        virtual
    ...
    /**
     * @dev See {ERC721Upgradeable}, {ERC721EnumerableUpgradeable}, {AccessControlEnumerableUpgradeable}
     */
    function supportsInterface(bytes4 interfaceId)
        public
        view
        virtual
        override(ERC721Upgradeable, ERC721EnumerableUpgradeable, ERC721RoyaltyUpgradeable, AccessControlEnumerableUpgradeable)
        returns (bool)
    {
     ...
     */
    function tokenURIOfOwnerByIndex(address account, uint256 index) 
        public 
        view 
        ...
        whenHasValidator 
    {
        ...
        bytes32 messageHash = keccak256(abi.encodePacked(tokenId, details, sender, tokenNonce, numTokensToBurn));
    ...
    {
        address sender = _msgSender();
        bytes32 messageHash = keccak256(abi.encodePacked(to, details, sender, numTokensToBurn));
...
contract BaseToken is ERC20PausableUpgradeable,
     ...
     */
    function __BaseToken_init(string memory _name, string memory _symbol, address admin) 
        internal 
        onlyInitializing 
    {
        require(Utils.isKnownNetwork(), "unknown network");
...
contract Versioned {
    string public constant version = "a69d83e137dba6f78c84712d2308761a7dde1de3";
}
...
contract Validator is PausableUpgradeable, 
     ...
     */
    function splitSignature(bytes memory signature, uint256 offset) 
        internal 
        pure 
