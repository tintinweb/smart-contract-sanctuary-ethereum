...
contract TESTMINT is ERC721Enumerable, Ownable {
    ...
    address public treasury = 0xB91fb18babD1b77cd628BE1841db934480c61Ad7;

    uint256 private constant MAX_ENTRIES = 1000;

    // Set Prices following Tokenomics
    uint256 private WL_PRICE = 0.6 ether;
    uint256 private SALE_PRICE = 0.7 ether;
    ...
    constructor() ERC721("TEST MINT", "TMIN") {
        // setBaseURI(baseURI);
    }
    ...
    function mint(uint256 amount) public payable {
        ...
        if (whitelisted[msg.sender]) price = WL_PRICE * amount;
        else price = SALE_PRICE * amount;

        // Payment value must be larger than the 'price'
        require(msg.value >= price, "TMIN : INCORRECT PRICE!");
        ...
        returns (string memory)
    {
        if (!revealStart) return hiddenURI;
            ...
            uint256 tokenUriId = tokenId % 10 + 1;
            return
                string(
                    abi.encodePacked(_baseTokenURI, tokenUriId.toString(), ".json")
                );
        }
    }
