...
/// @notice The backbone of the Ysd stability system.
contract StabilizerNode is Permissions {
  ...
  function _distributeRewards(uint256 rewarded) internal {
    ...
    rewardToken.approve(address(auction), 0);
    ...
    if (callerCut > 0) {
      rewardToken.transfer(msg.sender, callerCut);
    }

    if (auctionPoolCut > 0) {
      rewardToken.transfer(auctionPool, auctionPoolCut);
    }

    if (swingTraderCut > 0) {
      rewardToken.transfer(address(swingTrader), swingTraderCut);
    }

    if (treasuryCut > 0) {
      rewardToken.transfer(treasuryMultisig, treasuryCut);
    }

    if (daoCut > 0) {
      rewardToken.transfer(address(dao), daoCut);
    }

    if (lpCut > 0) {
      rewardToken.transfer(address(rewardThrottle), lpCut);
      rewardThrottle.handleReward();
    }

    if (buyBurn > 0) {
      rewardToken.transfer(address(buyBurnToken), buyBurn);
    }
  ...
  function _replenishLiquidityExtension(uint256 rewards) internal returns (uint256 remaining) {
    ...
    if (deficit >= maxContrib) {
      rewardToken.transfer(address(liquidityExtension), maxContrib);
      return rewards - maxContrib;
    }

    rewardToken.transfer(address(liquidityExtension), deficit);
  ...
  function emergencyMintYsd(uint256 amount) external {
    ...
    rewardToken.transfer(address(swingTrader), amount);
