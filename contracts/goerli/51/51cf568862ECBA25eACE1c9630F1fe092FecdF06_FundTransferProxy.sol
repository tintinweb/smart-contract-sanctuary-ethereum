...
abstract contract AdminPool is ERC2771ContextUpgradeable {
   ...
   */
  function addAdminBatch(address[] calldata _newAdminList)
    public
    virtual
    onlyAdmin
  {
...
interface IFundTransferProxy is IERC165Upgradeable {
   ...
   */
  function setUzuVaultAddress(address newAddress) external;

  /**
   * @dev Deposit ETH to UZU Vault
   *
   * @param sender address
   */
  function depositEthToVault(address sender) external payable;
   ...
   */
  function depositERC20ToVault(IERC20 tokenAddress, uint256 amount, address sender) external;
  
  /**
   * @dev Proxy the ETH transfer transaction
   *
   * @param to address
   * @param amount uint256
   */
  function transferEthFromVault(address to, uint256 amount) external;
...
interface IUzuVault is IERC165Upgradeable {
  event ETHDeposited(uint256 amount, address depositer);
  event ERC20Deposited(IERC20 erc20token, uint256 amount, address from);

  event ETHWithdrawn(uint256 amount, address recipient);
  event ERC20Withdrawn(IERC20 erc20token, uint256 amount, address recipient);

  /**
   * @dev To withdraw the ETH that this contract has.
   *
   * @param amount uint256
   */
  ...
  /**
   * @dev To withdraw all ETH that this contract has.
   */
   ...
   */
  function depositEth(uint256 amount, address from) external payable;
  ...
  /**
   * @dev Return the current deposited amount of ETH in Wei
   */
  function getCurrentBalanceOfEth() external view returns (uint256);
   ...
   * @param amount uint256
   */
  function withdrawERC20Token(IERC20 erc20Token, uint256 amount) external;
   ...
   * @param erc20Token address
   */
  function withdrawAllERC20Token(IERC20 erc20Token) external;

  /**
   * @dev Transfer ETH to the recipient. Assume that FundTransferProxy has SPENDER_ROLE.
   *
  ...
  /**
   * @dev Approve token transfer for the spender. Assume that FundTransferProxy is approved by this function.
   *
   * @param erc20Token address
...
import "../interfaces/IUzuVault.sol";
import "../access/AdminPool.sol";

contract FundTransferProxy is
  Initializable,
  ERC165Upgradeable,
  IFundTransferProxy,
  EIP712Upgradeable,
  ...
  UUPSUpgradeable
{
   ...
   */
  IUzuVault public uzuVaultAddress;
   ...
   * @dev Initializer
   *
   * @param uzuVaultAddress_ address
   */
  function __FundTransferProxy_init(address uzuVaultAddress_)
    external
    initializer
  {
    __ERC165_init();
    __AdminPool_init();
    __EIP712_init("FundTransferProxy", "1.0");
    __UUPSUpgradeable_init();

    setUzuVaultAddress(uzuVaultAddress_);
  }
  ...
  {
    return
      interfaceId == type(IFundTransferProxy).interfaceId ||
      super.supportsInterface(interfaceId);
  }

  /**
   * @dev See {IFundTransferProxy.setUzuVaultAddress}
   */
  function setUzuVaultAddress(address newAddress)
    public
    virtual
    override
    onlyAdmin
  {
    require(
      IUzuVault(newAddress).supportsInterface(type(IUzuVault).interfaceId),
      "FundTransferProxy: newAddress is not IUzuVault"
    );

    uzuVaultAddress = IUzuVault(newAddress);
  }

  /**
   * @dev See {IFundTransferProxy.depositEthToVault}
   */
  function depositEthToVault(address sender) external payable virtual override {
    // TODO: need to fix sending
    uzuVaultAddress.depositEth(msg.value, sender);
  }

  /**
   * @dev See {IFundTransferProxy.depositERC20ToVault}
   */
  function depositERC20ToVault(
    ...
    uzuVaultAddress.depositERC20(tokenAddress, amount, sender);
  }

  /**
   * @dev See {IFundTransferProxy.transferEthFromVault}
   */
  function transferEthFromVault(address to, uint256 amount)
    external
    virtual
    override
    onlyAdmin
  {
    uzuVaultAddress.transferEth(to, amount);
  }

  /**
   * @dev See {IFundTransferProxy.transferERC20FromVault}
   */
  function transferERC20FromVault(
    IERC20 tokenAddress,
    address to,
    ...
    IERC20(tokenAddress).transferFrom(address(uzuVaultAddress), to, amount);
  }
}