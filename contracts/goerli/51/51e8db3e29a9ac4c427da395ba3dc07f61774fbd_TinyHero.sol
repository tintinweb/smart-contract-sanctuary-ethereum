...
contract TinyHero is ERC721A,Ownable, ReentrancyGuard {
    string public baseURI;
    uint256 public MAX_SUPPLY = 1111;
    bool public curOpenState = false;

    //first free,and then pay 0.007 each. max 2 per wallet.

    // for public
    uint256 public constant publicCost = 0.007 ether;
    ...
    constructor() ERC721A("TinyHero","THT",maxPerWallet,MAX_SUPPLY) {
    }
    ...
    function HeroMint(uint256 quantity) external payable mintCompliance(quantity) {
        require(curOpenState,"not open yet!");
        //require(msg.sender == tx.origin, "must be real hero!");
        require(totalSupply() + quantity <= MAX_SUPPLY, "Purchase would exceed max supply");
        
        uint256 minted = numberMinted(msg.sender);
        require(minted + quantity <= maxPerWallet, "max 2 per wallet");
