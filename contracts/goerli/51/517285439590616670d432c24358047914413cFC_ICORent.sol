...
contract ICORent is Ownable {
    ...
    constructor() {
        ...
        USD = IERC20(0x781A96c0d81A0CB254A4dB68ffCcd10457a58E01);
    ...
    function buyRENT(uint256 amountRENTtoBuy) public {
        require(sellActivated, "Sell: the purchase is not active");

        uint256 amountUSD = (purchasePrice * amountRENTtoBuy)/1e18;

        if (totalSold >= amountToSell) {
            revert ErrorSoldOut(msg.sender, amountRENTtoBuy);
        }

        if (amountUSD < investorMinCap) {
            revert ErrorMinCap(msg.sender, amountRENTtoBuy);
        }

        if (amountRENTtoBuy > amountToSell - totalSold) {
            revert ErrorGreaterThanSellLimit(msg.sender, amountRENTtoBuy);
        }
    ...
    function unPauseTheSale() external onlyOwner {
        require(!sellActivated, "Sell: the purchase is active");
        sellActivated = true;
    }
