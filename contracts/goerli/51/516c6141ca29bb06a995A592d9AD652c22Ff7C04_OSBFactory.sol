...
contract OSBFactory is IOSBFactory, ContextUpgradeable {
    ...
    function create(bool _isSingle, string memory _uri, string memory _name, string memory _symbol, address _defaultReceiverRoyalty, uint96 _defaultPercentageRoyalty) public returns (TokenInfo memory) {
        ...
        if (_isSingle) {
            ...
            _osb721.initialize(_msgSender(), _uri, _name, _symbol, _defaultReceiverRoyalty, _defaultPercentageRoyalty); 
            deployedContract = address(_osb721);
        } else {
            OSB1155 _osb1155 = OSB1155(ClonesUpgradeable.cloneDeterministic(library1155Address, salt));
            _osb1155.initialize(_msgSender(), _uri, _name, _symbol, _defaultReceiverRoyalty, _defaultPercentageRoyalty); 
     ...
     */
    function createAndMintSingleToken(TokenInput memory _tokenInput, string[] memory _tokenUris) external {
        TokenInfo memory tokenInfo = create(true, _tokenInput.uri, _tokenInput.name, _tokenInput.symbol, address(0), 0);
        OSB721(tokenInfo.token).mintBatch(_msgSender(), _tokenUris);
    }
     ...
     */
    function createAndMintSingleTokenWithRoyalty(TokenInput memory _tokenInput, string[] memory _tokenUris, RoyaltyInput memory _defaultRoyaltyInput, RoyaltyInput[] memory _royaltyInputPerTokenMinted) external {
        ...
        OSB721(tokenInfo.token).mintBatchWithRoyalty(_msgSender(), _tokenUris, _royaltyInputPerTokenMinted);
    }
    /**
     ...
     */
    function createAndMintMultiToken(TokenInput memory _tokenInput, string[] memory _tokenUris, uint256[] memory _amounts) external {
        TokenInfo memory tokenInfo = create(false, _tokenInput.uri, _tokenInput.name, _tokenInput.symbol, address(0), 0);
        OSB1155(tokenInfo.token).mintBatch(_msgSender(), _amounts, _tokenUris);
    }
     ...
     * @param _royaltyInputPerTokenMinted Custom royalty information for each token in the batch.
     */
    function createAndMintMultiTokenWithRoyalty(TokenInput memory _tokenInput, string[] memory _tokenUris, uint256[] memory _amounts, RoyaltyInput memory _defaultRoyaltyInput, RoyaltyInput[] memory _royaltyInputPerTokenMinted) external {
        ...
        OSB1155(tokenInfo.token).mintBatchWithRoyalty(_msgSender(), _amounts, _tokenUris, _royaltyInputPerTokenMinted);
    }
}
