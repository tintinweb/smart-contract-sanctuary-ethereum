...
interface INounChecksParams {
  function getCheckPositions() external view returns (string[] memory checkPosititions);
  ...
  function getBgColors() external view returns (string[] memory bgColors);
...
import './Params.sol';
import './interfaces/INounChecksParams.sol';

contract NounChecksRendererV1 is Initializable, OwnableUpgradeable {
    using StringsUpgradeable for uint256;
    using StringsUpgradeable for uint160;
    using StringUtils for string;

    INounChecksParams public params;    
    ...
    event ParamsUpdated(INounChecksParams params);

    function initialize(address owner) public initializer {
        _owner = owner;
        description = string(
            abi.encodePacked(
                "This artwork may or may not be nounable."
            )
        );
        name = string(abi.encodePacked("Noun Checks"));
        staticBlockNum = 16031;
    }
    ...
    function constructTokenURI(
        ...
        // Build seed array
        uint256[] memory checkColors = randomArray(tokenId, params.getCheckColors().length, params.getCheckPositions().length);
        ...
        // 5. check type
        // 6. accessory color
        uint256[7] memory tokenDNA = [
            random(tokenId, 2, staticBlockNum * 33), // bg color
            random(tokenId, 2, staticBlockNum * 55), // theme
            random(tokenId, params.getBodyColors().length - 1, staticBlockNum * 77), // body color
            random(tokenId, params.getCheckColors().length - 1, staticBlockNum * 99), // noggle color
            chooseNoggleType(tokenId), // noggle type
            random(tokenId, 2, staticBlockNum * 555), // check type
            random(tokenId, params.getCheckColors().length - 1, staticBlockNum * 777) // accessory color
        ];
    ...
    function getTokenIdSvg(
        ...
        string memory checks = buildChecks(tokenId);
        string memory nogglesOverlay;
        string memory checkAccessory = params.getCheckPaths()[0];
        string memory nogglesColor = params.getCheckColors()[tokenDNA[3]];

        if (tokenDNA[5] == 1) {
            checkAccessory = params.getCheckPaths()[1];
        }
        // If noggle type is "Full", build noggle overlay
        if (tokenDNA[4] == 1) {
            nogglesOverlay = string(
                abi.encodePacked(buildNogglesOverlay(nogglesColor))
            );
        }
        // For hip noggles, add second row
        if (tokenDNA[4] == 2) {
            nogglesOverlay = string(
                abi.encodePacked(
                    buildNogglesOverlay(nogglesColor), '<path fill="#',
                    nogglesColor,
                    '" d="M80 140h45v10H80v-10Zm45 0h45v10h-45v-10Z"/>'
                )
            );
        }
        ...
        return
            string(
                abi.encodePacked(
                    '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 320 320" width="2000" height="2000" >',
                    '<defs><style>.check{fill: #',
                    params.getCheckColors()[tokenDNA[6]],
                    '}</style></defs>'
                    '<path fill="#',
                    params.getBgColors()[tokenDNA[0]],
                    '" d="M0 0h320v320H0z" />',
                    '<path fill="#',
                    // frame for theme
                    params.getFrameColors()[tokenDNA[1]],
                    '" d="M80 40h180v180H80z"/>',
                    '<path fill="#',
                    // matte for theme
                    params.getMatteColors()[tokenDNA[1]],
                    '" d="M90 50h160v160H90z"/>',
                    checks,
                    '<path fill="#',
                    // body color
                    params.getBodyColors()[tokenDNA[2]],
                    '" d="M90 220h20v100H90V220Zm20 0h10v30h-10v-30Zm10 0h110v100H120V220Z"/>',
                    // check accessory uses noggles color palette
                    checkAccessory,
                    // slightly lighten pixels between noggle frames
                    '<path fill="',
                    params.getMatteColors()[tokenDNA[1]],
                    '" fill-opacity=".6" d="M160 120h10v50h-10z"/>',
                    // eyes
                    '<path fill="#fff" d="M130 120h-20v10h20v-10Zm0 10h-20v10h20v-10Zm0 10h-20v10h20v-10Zm0 10h-20v10h20v-10Z"/><path fill="#000" d="M150 120h-20v10h20v-10Zm0 10h-20v10h20v-10Zm0 10h-20v10h20v-10Zm0 10h-20v10h20v-10Z"/><path fill="#fff" d="M200 120h-20v10h20v-10Zm0 10h-20v10h20v-10Zm0 10h-20v10h20v-10Zm0 10h-20v10h20v-10Z"/><path fill="#000" d="M220 120h-20v10h20v-10Zm0 10h-20v10h20v-10Zm0 10h-20v10h20v-10Zm0 10h-20v10h20v-10Z"/>',
                    // noggles
                    '<path fill="#',
                    params.getCheckColors()[tokenDNA[3]],
    ...
    function buildNogglesOverlay(string memory noggleColor) internal pure returns (string memory) {
        return string( 
            abi.encodePacked(
                '<path fill="#',
                noggleColor,
                '" d="M110 120h40v40h-40z"/><path fill="#',
                noggleColor,
                '" d="M180 120h40v40h-40z"/>',
                '<path fill="#FFFFFF" d="M140 120h10v20h-10z"/><path fill="#FFFFFF" d="M210 120h10v20h-10z"/>'
            )
        );
    }
  
    function buildChecks(uint256 tokenId) internal view returns (string memory) {
        bytes memory list;
        string[] memory checkPosititions = params.getCheckPositions();
        string[] memory colors = params.getCheckColors();
        for (uint i = 0; i < checkPosititions.length; ++i) {
            uint256 colorIndex = random(tokenId, colors.length, i * staticBlockNum);
    ...
    function getTokenIdMetadata(uint256[7] memory tokenDNA) public view returns (string memory metadata) {
        ...
        metadata = string(
            abi.encodePacked(
                ...
                bgColorName,
                '"},',
                '{"trait_type":"Color mode", "value":"',
                params.getThemes()[tokenDNA[1]],
                '"},',
                '{"trait_type":"Body", "value":"',
                params.getBodyColorNames()[tokenDNA[2]],
                '"},',
                '{"trait_type":"Noggle color", "value":"#',
                params.getCheckColors()[tokenDNA[3]],
                '"},',
                '{"trait_type":"Noggle style", "value":"',
                params.getNoggleType()[tokenDNA[4]],
                '"},',
                '{"trait_type":"Check style", "value":"',
                params.getCheckType()[tokenDNA[5]],
                '"}'
            )
        );
    ...
    function setParams(INounChecksParams _params) public {
    ...
    */
    ...
    function randomArray(uint256 input, uint256 max, uint256 count)
        public
        view
        returns (uint256[] memory)
    {
        ...
        for (uint256 i = 0; i < count; i++) {
            output[i] = random(input, max, count);
            input = output[i];
        }
        return output;
    }

    function random(uint256 input, uint256 max, uint256 randomNum) public pure returns (uint256) {
        return (uint256(keccak256(abi.encodePacked((input * randomNum) + (input * max) + input))) % max);
    }

    function chooseNoggleType(uint256 tokenId) public pure returns (uint256) {
