 ...
 * @title CML utilities
 *
 * @notice this library implements utility methods for the CML multiplier
...
library CMLUtils {
    ...
    uint256 public constant CML_PROPORTION = 10**(DECIMALS - 2); // 1%

    /**
     * @notice compute CML bonus as a function of usage ratio, stake amount,
     * and CML spent
     * @param cml number of CML token applied to bonus
    ...
    function cmlBonus(
        ...
        // scale CML amount with respect to proportion
        uint256 portion = (CML_PROPORTION * total) / 10**DECIMALS;
        if (amount > portion) {
            cml = (cml * portion) / amount;
        }

        // 1 + cml / (0.01 + ratio)
        uint256 x = 2**64 + (2**64 * cml) / (10**(DECIMALS - 2) + ratio);
 ...
 * also receive a CML multiplier by spending CML at the time of unstaking.
 *
 */
contract ERC20CompetitiveRewardModule is ERC20BaseRewardModule {
    using SafeERC20 for IERC20;
    using CMLUtils for uint256;
     ...
     * @return spent amount of cml spent
     * @return vested amount of cml vested
     */
    function _unstake(
        address account,
        address user,
        ...
        uint256 bonus = spent.cmlBonus(shares, totalStakingShares, _usage);
        ...
        // compute and apply CML token bonus
        uint256 cmlWeightedShareSeconds = (bonus * timeWeightedShareSeconds) /
            10**DECIMALS;
        ...
        uint256 rewardShares = (unlockedShares * cmlWeightedShareSeconds) /
            (totalStakingShareSeconds + cmlWeightedShareSeconds);
        ...
        if (rewardShares > 0) {
            ...
            if (spent > 0) {
                vested = spent;
                emit CMLSpent(user, spent);
                emit CMLVested(user, vested);
 ...
 * other contracts and specifically the CML multiplier calculation
 *
