...
import {ERC721AUpgradeable} from "./forks/erc721a-upgradeable/contracts/ERC721AUpgradeable.sol";
import {ERC1155Upgradeable} from "./forks/ERC1155/ERC1155Upgradeable.sol";
import {ERC1155Storage} from "./forks/ERC1155/ERC1155Storage.sol";

import {IERC1155721Facet} from "./IERC1155721Facet.sol";
import {ERC1155Impl} from "./ERC1155Impl.sol";
import {ERC721Impl} from "./ERC721Impl.sol";
import {AppModifiers} from "../../AppModifiers.sol";
import {FacetInitializable} from "../../../shared/utils/FacetInitializable.sol";
import {LibUtilities} from "../../../shared/libraries/LibUtilities.sol";
import {ERC1155721FacetStorage, TokenInfo} from "./ERC1155721FacetStorage.sol";

contract ERC1155721Facet is IERC1155721Facet, FacetInitializable, ERC721Impl, ERC1155Impl {
    using LibUtilities for uint256;
    using LibUtilities for string;
    using ERC1155721FacetStorage for TokenInfo;

    error MaxSupplyReached(uint256 _id);
    error NonexistentToken(uint256 _id);

    error MintToBurnAddress();
    error MintUninitializedToken(uint256 _id);

    error InvalidCreateTokenInputs();

    function ERC1155721Facet_Initialize()
        external
        facetInitializer(keccak256("ERC1155721Facet"))
        requireOwnerOrRole(ADMIN_ROLE)
    {
        __ERC721Impl_Initialize();
        __ERC1155Impl_Initialize();
    }
    ...
    function uri(uint256 _id) public view virtual override returns (string memory) {
        TokenInfo memory _tokenInfo = ERC1155721FacetStorage.getTokenInfo(_id);
        if (!_tokenInfo.isInitialized()) {
            revert NonexistentToken(_id);
        }
        // TODO: implement real
        if (_id == 1) {
            // happy pepe?
            return
                "https://dthezntil550i.cloudfront.net/qe/latest/qe2001070444226870012184583/1280_960/f0c37cde-9689-4234-a284-a239110170cf.png";
        }
        // sad pepe
        return "https://www.nicepng.com/png/detail/149-1492157_1501986705579-sad-helper-pepe.png";
    }

    // TODO: what requirement is there to mint?
    function mint(
        address _to,
        uint256 _id,
        uint256 _amount
        ...
        if(_tokenInfo.maxSupply == 1) {
            // mint with erc721a
            _mint(_to, _amount);
            return;
        }
        // mint with erc1155
        // _beforeMint(_to, _id, _amount);
    ...
    function mintBatch(
        ...
        for (uint256 i = 0; i < len; i++) {
            _beforeMint(_to, _ids[i], _amounts[i]);
        }
        _mintBatch(_to, _ids, _amounts, _data);
    }

    function initializeToken(string memory _name, uint256 _maxSupply) public {
        initializeTokens(_name.asSingletonArray(), _maxSupply.asSingletonArray());
    }

    function initializeTokens(string[] memory _names, uint256[] memory _maxSupplies)
        public
        requireOwnerOrRole(ADMIN_ROLE)
    {
        if (_names.length != _maxSupplies.length) revert InvalidCreateTokenInputs();
        uint256 curTokenId = ERC1155721FacetStorage.getState().tokenIdCounter;
        for (uint256 i = 0; i < _names.length; i++) {
            uint256 newTokenID = ++curTokenId;
            ERC1155721FacetStorage.setTokenInfo(newTokenID, TokenInfo({name: _names[i], maxSupply: _maxSupplies[i]}));
        }
        // update storage with potentially new last-used tokenId
        ERC1155721FacetStorage.getState().tokenIdCounter = curTokenId;
    }

    function getTokenInfo(uint256 _id) public view returns (TokenInfo memory tokenInfo_) {
        tokenInfo_ = ERC1155721FacetStorage.getTokenInfo(_id);
        if (!tokenInfo_.isInitialized()) {
            revert NonexistentToken(_id);
        }
    }
    ...
    // Validate inputs and require the contract is not paused to allow minting
    function _beforeMint(
        ...
        if (_to == address(0)) {
            revert MintToBurnAddress();
        }
        TokenInfo memory _tokenInfo = ERC1155721FacetStorage.getTokenInfo(_id);
        if (!_tokenInfo.isInitialized()) {
            revert MintUninitializedToken(_id);
        }
        if (totalSupply(_id) + _amount > _tokenInfo.maxSupply) {
            revert MaxSupplyReached(_id);
        }
    }
    ...
    function setApprovalForAll(address operator, bool approved)
        public
        virtual
        override(ERC1155Upgradeable, ERC721AUpgradeable)
    {
    ...
    function isApprovedForAll(address account, address operator)
        ...
        returns (bool)
    {
        // TODO: look into erc721 storage if its an nft
...
library ERC1155721FacetStorage {
    struct State {
        // The next token ID to be minted / initialized.
        uint256 tokenIdCounter;
        mapping(uint256 => TokenInfo) tokenIdToInfo;
    }

    bytes32 internal constant FACET_STORAGE_POSITION = keccak256("diamond.dapp.facet.erc1155721facetstorage");
    ...
    function isInitialized(TokenInfo memory _tokenInfo) internal pure returns (bool isInitialized_) {
        isInitialized_ = _tokenInfo.maxSupply != 0;
    }

    function setTokenInfo(uint256 _id, TokenInfo memory _tokenInfo) internal {
        getState().tokenIdToInfo[_id] = _tokenInfo;
    }

    function getTokenInfo(uint256 _id) internal view returns (TokenInfo memory tokenInfo_) {
        tokenInfo_ = getState().tokenIdToInfo[_id];
    }
}
...
import {ERC1155SupplyUpgradeable, ERC1155Upgradeable} from "./forks/ERC1155/extensions/ERC1155SupplyUpgradeable.sol";
import {ERC1155BurnableUpgradeable} from "./forks/ERC1155/extensions/ERC1155BurnableUpgradeable.sol";
import {IERC1155721Facet} from "./IERC1155721Facet.sol";
import {AppModifiers} from "../../AppModifiers.sol";
import {FacetInitializable} from "../../../shared/utils/FacetInitializable.sol";
import {ERC1155721FacetStorage, TokenInfo} from "./ERC1155721FacetStorage.sol";

abstract contract ERC1155Impl is
    IERC1155721Facet,
    FacetInitializable,
    ...
    AppModifiers
{
    ...
    function __ERC1155Impl_Initialize() public onlyFacetInitializing requireOwnerOrRole(ADMIN_ROLE) {
        __ERC1155_init("");
...
import {ERC721AUpgradeable} from "./forks/erc721a-upgradeable/contracts/ERC721AUpgradeable.sol";
import {ERC721A__InitializableStorage} from "./forks/erc721a-upgradeable/contracts/ERC721A__InitializableStorage.sol";
import {IERC1155721Facet} from "./IERC1155721Facet.sol";
import {AppModifiers} from "../../AppModifiers.sol";
import {FacetInitializable} from "../../../shared/utils/FacetInitializable.sol";

abstract contract ERC721Impl is IERC1155721Facet, FacetInitializable, ERC721AUpgradeable, AppModifiers {
    function __ERC721Impl_Initialize() public onlyFacetInitializing requireOwnerOrRole(ADMIN_ROLE) {
        ERC721A__InitializableStorage.layout()._initializing = true;
        __ERC721A_init("RUG", "ME"); // TODO: change... lmao
...
library ERC1155Storage {
    ...
    bytes32 internal constant STORAGE_SLOT = keccak256("omnificient.contracts.storage.ERC1155");
...
// import to avoid duplicate event definitions with ERC721AUpgradeable
import {IConflictedEvents} from "../IConflictedEvents.sol";

import "./IERC1155Upgradeable.sol";
import "./IERC1155ReceiverUpgradeable.sol";
import "./extensions/IERC1155MetadataURIUpgradeable.sol";
import "../../../../../transpiled-oz/utils/AddressUpgradeable.sol";
import "../../../../../transpiled-oz/utils/ContextUpgradeable.sol";
import "../../../../../transpiled-oz/utils/introspection/ERC165Upgradeable.sol";
...
import "../../../../../transpiled-oz/utils/introspection/IERC165Upgradeable.sol";
...
interface IERC1155Upgradeable is IERC165Upgradeable {
     ...
     * `approved`.
     */
    // Commented out from interface to remove a conflict when importing erc721 & erc1155
    //  (see ERC1155Upgradeable for emit event)
...
library ERC1155SupplyStorage {
    ...
    bytes32 internal constant STORAGE_SLOT = keccak256("omnificient.contracts.storage.ERC1155Supply");
...
// import to avoid duplicate event definitions with ERC1155Upgradeable
import {IConflictedEvents} from "../../IConflictedEvents.sol";
 ...
 */
contract ERC721AUpgradeable is ERC721A__Initializable, IERC721AUpgradeable, IConflictedEvents {
...
interface IERC721AUpgradeable {
     ...
     */
    // moved to IConflictedEvents to be used with erc1155 to prevent compilation error
...
import {IAccessControlUpgradeable} from "../../../transpiled-oz/access/IAccessControlUpgradeable.sol";
import {UpgradesFacet} from "../../../shared/facets/UpgradesFacet.sol";
import {ERC1155721Facet} from "../erc1155721/ERC1155721Facet.sol";
import {LibUtilities} from "../../../shared/libraries/LibUtilities.sol";
import {LibUpgrades} from "../../../shared/libraries/LibUpgrades.sol";
import {LibDiamond} from "../../../shared/libraries/LibDiamond.sol";

contract InitialDeployFacet is UpgradesFacet {
    // =============================================================
    //                    Upgrade function hashes
    // =============================================================

    // INITIAL_DEPLOY already defined in UpgradesFacet.
    // Future upgrades will contain their hashes here

    // =============================================================
    //                       Upgrade functions
    // =============================================================

    function initialDeploy(LibUpgrades.GrantRoleInfo[] calldata _grants)
        external
        virtual
        override
        onlyRunOnce(INITIAL_DEPLOY)
    {
        _grantRolesBatch(_grants);
        if (!LibUtilities.isFacetInitialized(keccak256("ERC1155721Facet"))) {
            LibDiamond.callDelegateFuncNoArgs(ERC1155721Facet.ERC1155721Facet_Initialize.selector);
        }
    ...
    function _grantRolesBatch(LibUpgrades.GrantRoleInfo[] calldata _grants) private {
        uint256 len = _grants.length;
        for (uint256 i = 0; i < len; i++) {
            LibUpgrades.GrantRoleInfo calldata roleInfo = _grants[i];
            // Will skip roles that have already been granted
            LibDiamond.callDelegateFunc(IAccessControlUpgradeable.grantRole.selector,
                abi.encodeWithSelector(IAccessControlUpgradeable.grantRole.selector, roleInfo.role, roleInfo.account));
        }
    }
}
...
import {IUpgrades} from "../interfaces/IUpgrades.sol";
import {LibUpgrades} from "../libraries/LibUpgrades.sol";

abstract contract UpgradesFacet is IUpgrades {
    // =============================================================
    //                    Upgrade function hashes
    // =============================================================
    ...
    // This will have no affect on using the AppStorage pattern in the future
    //  if desired (storing a struct at storage slot 0)
    bytes32 internal constant INITIAL_DEPLOY = keccak256("initialDeploy()");

    // =============================================================
    //                       Upgrade functions
    // =============================================================

    function initialDeploy(LibUpgrades.GrantRoleInfo[] calldata _grants) external virtual onlyRunOnce(INITIAL_DEPLOY) {}

    // =============================================================
    //                       Helper functions
    // =============================================================

    function hasRan(bytes32 _funcHash) public view returns (bool hasRan_) {
        hasRan_ = LibUpgrades.getUpgradeCompletedStatus(_funcHash);
    }
    ...
    modifier onlyRunOnce(bytes32 _funcHash) {
        if (hasRan(_funcHash)) {
            revert LibUpgrades.HasRan(_funcHash);
        }
        _;
    }
}
...
interface IUpgrades {
    function hasRan(bytes32 _funcHash) external view returns (bool hasRan_);

    function initialDeploy(LibUpgrades.GrantRoleInfo[] calldata _grants) external;
}
...
library LibUpgrades {
    struct GrantRoleInfo {
        bytes32 role;
        address account;
    }

    event UpgradePerformed(bytes32 indexed _funcHash);

    error HasRan(bytes32 _funcHash);

    struct State {
        mapping(bytes32 => bool) upgradeCompleted;
    }

    bytes32 internal constant FACET_STORAGE_POSITION = keccak256("diamond.rugstop.erc721");
    ...
    function setUpgradeCompleted(bytes32 _funcHash) internal {
        getState().upgradeCompleted[_funcHash] = true;
        emit UpgradePerformed(_funcHash);
    }

    function getUpgradeCompletedStatus(bytes32 _funcHash) internal view returns (bool isCompleted_) {
        isCompleted_ = getState().upgradeCompleted[_funcHash];
    }
}
