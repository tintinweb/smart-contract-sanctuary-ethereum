...
// File: contracts/SpaceTedNft.sol
...
contract SpaceTed is Ownable, ERC721A, PaymentSplitter {
    ...
    uint256 private constant MAX_SUPPLY = 6500;
    uint256 private constant MAX_WHITELIST = 6400;
    uint256 private constant MAX_PUBLIC = 6400;
    uint256 private constant MAX_GIFT = 100;

    uint256 public wlSalePrice = 0.02 ether;
    uint256 public publicSalePrice = 0.03 ether;

    bytes32 public merkleRoot;

    uint256 public saleStartTime = 1669834800;
    ...
    constructor(
        ...
        bytes32 _merkleRoot,
        string memory _bURI
    ) ERC721A("Space Ted NFT", "STN") PaymentSplitter(_team, _teamShares) {
    ...
    function whitelistMint(
        ...
        require(
            currentTime() < saleStartTime + 300 minutes,
            "Allowlist Sale is finished"
        );
        require(
            sellingStep == Step.WhitelistSale,
            "Allowlist sale is not activated"
        );
        require(
            isWhiteListed(msg.sender, _proof),
            "You are not on Allowlisted"
        );
        require(
            amountNFTsperWalletWhitelistSale[msg.sender] + _quantity <= 6,
            "You can only get 6 NFT on the Allowlist Sale"
        );
        require(
        ...
        callerIsUser
    {
        ...
        require(
            amountNFTsperWalletPublicSale[msg.sender] + _quantity <= 3,
