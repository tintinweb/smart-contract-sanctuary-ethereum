...
contract Staking is Pausable, Ownable {
  ...
  // public percent denominator to avoid floating point numbers
  uint96 public percentDenominator = 10000;
  ...
  uint256[] private _availablePercents = [2000, 2000, 2000, 2000];
  bytes32[] private _availableConditionsHashes = [
    keccak256(
      abi.encodePacked(_availableExpiresAfter[0], ":", _availablePercents[0])
    ),
    keccak256(
      abi.encodePacked(_availableExpiresAfter[1], ":", _availablePercents[1])
    ),
    keccak256(
      abi.encodePacked(_availableExpiresAfter[2], ":", _availablePercents[2])
    ),
    keccak256(
      abi.encodePacked(_availableExpiresAfter[3], ":", _availablePercents[3])
    )
  ];
    ...
    returns (bool)
  {
    ...
    uint128 interest = uint256(amount)
      .mul(_conditionsMap[conditionHash].percent)
      .mul(_conditionsMap[conditionHash].expiresAfter)
      .div(360 days)
  ...
  {
    return (
      _availableExpiresAfter,
      _availablePercents,
      _availableConditionsHashes
    );
  }
  ...
  function setConditions(
    ...
    for (uint256 i = 0; i < _availableExpiresAfter.length; i++) {
      bytes32 key = keccak256(
        abi.encodePacked(_availableExpiresAfter[i], ":", _availablePercents[i])
      );
      delete _conditionsMap[key];
      _availableExpiresAfter[i] = 0;
      _availablePercents[i] = 0;
    }
    ...
    for (uint256 i = 0; i < expiresAfter.length; i++) {
      bytes32 key = keccak256(
        abi.encodePacked(expiresAfter[i], ":", percents[i])
      );
      _conditionsMap[key] = Condition(expiresAfter[i], percents[i], true);
    ...
    emit SetConditions(expiresAfter, percents, newAvailableConditionsHashes);
  }
