...
// Enable recoverERC20 for eth and non staking tokens
// changeController for Aragon 
// add payer address to deposit_from (security risk for approvals)
// Assert not contract depositor (aura protection)
// Whitelist smart contracts for deposit
// Add current and next period start timestamps
// Add whitelist support
// Change balanceOf/totalSupply to reflect user votes, not nft count
// Decide which funtions are done by operators/admin/jelly
  ...
  /// @notice Deposit `_value` tokens for `_tokenId` and add to the lock
    ...
    function _balanceOfNFT(uint _tokenId, uint _t) internal view returns (uint) {
        ...
        if (_epoch == 0) {
            ...
            uint weighted_supply = uint(uint128(last_point.bias));
    ...
    function _checkpoint(
        ...
        Point memory last_point = Point({bias: 0, slope: 0, ts: block.timestamp, blk: block.number, amt: 0});
        if (_epoch > 0) {
        ...
        if (_tokenId != 0) {
            ...
            if (new_locked.amount > old_locked.amount) {
                last_point.amt += uint256(int256(new_locked.amount - old_locked.amount));
            }
            if (new_locked.amount < old_locked.amount) {
                last_point.amt -= uint256(int256(old_locked.amount - new_locked.amount));

                if (new_locked.amount == 0 && !emergencyUnlockActive) {
        ...
        if (_tokenId != 0) {
            ...
            u_new.amt = uint256(int256(locked[_tokenId].amount));
    ...
    function enableList(bool _useList, bool _useListAmounts) external {
        require(accessControls.hasAdminRole(msg.sender));
