...
contract TTS is Initializable, ERC721Upgradeable, OwnableUpgradeable, ERC2981Upgradeable, SignedAllowance {
    ...
    mapping(address => QuantumStorage) private _quantumStorage;
    ...
    function initialize() initializer public {
        __ERC721_init("Society of Time Travellers", "SoTT");
        __Ownable_init();
        __ERC2981_init();
    ...
    function TimeKeeperMint(address to) external onlyOwner {
        _mint(to, tokenIndex);
        unchecked {
            ++tokenIndex;
        }
    }
    //if the user exists on the whitelist, they're free to mint 1 token, else it's payable for minimum 0.1 ether
    function TimeTravellerMint (bool whitelisted, uint256 nonce, bytes memory signature) external payable {
        require(quantumStorage.SaleOpen, "Sale closed.");
        require(!_quantumStorage[msg.sender].Minted, "Already minted.");
        if (whitelisted) {
            validateSignature(msg.sender, nonce, signature);
            _quantumStorage[msg.sender].Minted = true;
            _mint(msg.sender, tokenIndex);
            unchecked {
                ++tokenIndex;
            }
        } else {
            require(msg.value >= 1000000000000000000, "Minimum 0.1 ether.");
            _quantumStorage[msg.sender].Minted = true;
            _mint(msg.sender, tokenIndex);
            unchecked {
                ++tokenIndex;
    ...
    function lockTransfer(bool locked) external onlyOwner {
    ...
    function setRoyaltyBps(uint16 newRoyaltyBps) external onlyOwner {
        _setDefaultRoyalty(owner(), newRoyaltyBps);
    }
    ...
    function checkLock() external view returns (bool) {
        return quantumStorage.TransferLocked;
    }

    function checkSaleStatus() external view returns (bool) {
        return quantumStorage.SaleOpen;
    }
    ...
    function _beforeTokenTransfer(address from, address to , uint256 tokenId) internal view override {
        require(!quantumStorage.TransferLocked || isOwner(msg.sender) || isZeroAddress(from), "Locked / Unauthorized");
        }
    ...
    function moveToken(address from, address to, uint256 tokenId) external onlyOwner {
        if (to == address(0)) {
            _burn(tokenId);
        } else {
    ...
    function isOwner(address admin) public view returns (bool) {
        return (owner() == admin);
    }
        

    function isZeroAddress(address addr) public view returns (bool) {
...
// SPDX-License-Identifier: MIT
pragma solidity ^0.8.9;
// from @dievardump's https://github.com/dievardump/polygon-dex/blob/main/contracts/Access/OwnerOperatorControl.sol
