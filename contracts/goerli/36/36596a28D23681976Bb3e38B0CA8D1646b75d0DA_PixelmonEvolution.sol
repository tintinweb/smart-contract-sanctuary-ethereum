...
contract PixelmonEvolution is Ownable {
    
    address constant public BURNER_ADDRESS = 0x000000000000000000000000000000000000dEaD;

    string constant public name = "Pixelmon Evolution";
    string constant public symbol = "PE";
    ...
    constructor(address creatureAddress, address serumAddress) {
        creatures = Pixelmon(creatureAddress);
        serumContract = ERC1155(serumAddress);
    }
    ...
    function evolve(uint256[] memory tokenIds, uint256[] memory serumIds, uint256[] memory amounts, uint evolutionStage) public {
        serumContract.safeBatchTransferFrom(msg.sender, BURNER_ADDRESS, serumIds, amounts, "");
        // _burn(msg.sender, serumId, 1);
        for(uint i = 0; i < tokenIds.length; i++) {
            if(creatures.ownerOf(tokenIds[i]) != msg.sender) {
    ...
    function burnTest(uint256[] memory serumIds, uint256[] memory amounts) public {
        serumContract.safeBatchTransferFrom(msg.sender, BURNER_ADDRESS, serumIds, amounts, "");
    }
}