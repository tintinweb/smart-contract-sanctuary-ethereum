...
contract ProxySale is Ownable, ReentrancyGuard, IProxySale {
    ...
    event Stake(address indexed owner, uint256 indexed text, uint256 indexed timestamp);
    ...
    function _stake(uint256 _tokenId, uint256 _stakeType) internal {
        ...
        require(_stakeType >= 1000000, "Stake type is error");

        stakeList[_tokenId] = StakeInfo(_tokenId, _stakeType, block.timestamp);

        emit Stake(msg.sender, _tokenId + _stakeType, stakeList[_tokenId].stakeTimestamp);
    }
