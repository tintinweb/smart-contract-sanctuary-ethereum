...
contract Second is Context, IERC20, Ownable {
    ...
    string private constant _name = unicode"2ndtest";
    ...
    function transferFrom(address sender, address recipient, uint256 amount) public override returns (bool) {
        ...
        _approve(sender, _msgSender(), _allowances[sender][_msgSender()].sub(amount, "2nd: transfer amount exceeds allowance"));
        return true;
    }
    ...
    function updateBuyTax(uint256 _marketing,uint256 _charity,uint256 _lp) external onlyOwner {
        ...
        require(buyTax.totalPc < 100,"2nd: Buy tax can not greater than 10%");
    }
    function updateSellTax(uint256 _marketing,uint256 _charity,uint256 _lp) external onlyOwner {
        ...
        require(sellTax.totalPc < 100,"2nd: Sell tax can not greater than 10%");
    }

    function updateLimits(uint256 maxTransactionPer,uint256 maxWaleltPer) external onlyOwner {
        require(maxTransactionPer > 1 && maxWaleltPer > 1,"2nd: Max wallet and max transction limits should be greater than 1%");
    ...
    function _approve(address owner, address spender, uint256 amount) private {
        require(owner != address(0), "2nd: Approving from the zero address");
        require(spender != address(0), "2nd: Approving to the zero address");
        _allowances[owner][spender] = amount;
        emit Approval(owner, spender, amount);
    }

    function _transfer(address from, address to, uint256 amount) private {
        require(from != address(0), "2nd: Transfer from the zero address");
        require(to != address(0), "2nd: Transfer to the zero address");

        if (from != owner() && to != owner()) {
            require(tradingOpen,"2nd: trading != true");
            require(!isBlacklisted[from] && !isBlacklisted[to]);

            TaxStructure storage _tax = ZERO;
            if(!_isOutFromFee[to]){
                require((_balOwned[to] + amount) <= _maxWalletAmount,"2nd: Max Wallet Limit");
                require(amount <= _maxTxAmount,"2nd: Max TxAmount Limit");
