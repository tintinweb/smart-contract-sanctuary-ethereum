...
contract PARCOShibuyaV2 is ERC1155SupplyUpgradeable, ERC1155BurnableUpgradeable, OwnableUpgradeable, AccessControlUpgradeable {
  ...
  event Attest(address indexed to, uint256[] indexed tokenId, uint256[] indexed amount, bytes data);
  event Revoke(address indexed to, uint256[] indexed tokenId, uint256[] indexed amount, bytes data);
  ...
  function mintBatch(address _to, uint[] memory _ids, uint[] memory _amounts) external onlyRole(ADMIN) {
  ...
  function safeTransferFrom(address from, address to, uint id, uint amount) public {
    require(!locked, "The token cannot transfered");
    safeTransferFrom(from, to, id, amount, "");
  }

  function safeBatchTransferFrom(address from, address to, uint[] memory ids, uint[] memory amount) public {
    require(!locked, "The token cannot transfered");
  ...
  function revoke(uint _id, uint _amount) external onlyRole(BURN) {
  ...
  function burn(uint _id, uint _amount)external {
    require(_ownerOf(_id), "Only Owner can burn it");
     _burn(msg.sender, _id, _amount);
  }
  ...
  function _afterTokenTransfer(
        ...
        // if token address sent from address zero (minted), 
        // if was send to zero address (burn) it was revoked
        data = "";
        if (from == address(0)){
            emit Attest(to, ids, amounts, data);
        } else if (to == address(0)){
            emit Revoke(to, ids, amounts, data);
  ...
  function _beforeTokenTransfer(
        ...
        // if user burning or receiving the token from address 0 meaning issued from
        // and to address 0 meaning burn to address 0
        require(from == address(0) || to == address(0), "Token can't trasferred");
