...
contract RightsDeclarationV1 is ERC2771Context, Ownable {  
  /**
   * @dev Emitted when rights are declared for tokens of contract `contractAddr`
   */
  event RightsDeclaredBatch(address indexed contractAddr, string rightsURI, address indexed declarer);

  // Indicates if allowlist is active or not
  ...
  modifier onlyAllowed() {
    if (_useAllowlist) {
      require(_allowlist[msg.sender] == true || _allowlist[tx.origin] == true, "RightsDeclarationV1: caller is not allowed");
    }
    _;
  }
  ...
  /*
   * Declare Rights for a NFT Smart Contract
   */
  function declare(address contractAddr, string calldata rightsURI_) public onlyAllowed {
    require(bytes(rightsURI_).length > 0, "RightsDeclarationV1: Rights URI can not be empty");
    
    // Emit event
    emit RightsDeclaredBatch(contractAddr, rightsURI_, _msgSender());
  }
}