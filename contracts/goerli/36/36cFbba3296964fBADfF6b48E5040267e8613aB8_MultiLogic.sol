...
contract MultiLogic is LogicUpgradeable {
        ...
        if (sumAvailable > 0) {
            for (index = 0; index < _count; ) {
                tokenAvailableLogic[_token][
                    multiStrategyData[
                        multiStrategyName[index]].logicContract
        ...
        onlyOwnerAndAdmin
    {
        ...
        for (uint256 i = 0; i < multiStrategyLength; ) {
            if (keccak256(abi.encodePacked((multiStrategyName[i]))) ==
    ...
    function setLogicTokenAvailable(
        ...
        for (uint256 i = 0; i < _count; i++) {
            address logicAddress = multiStrategyData[
                multiStrategyName[i]].logicContract;
    ...
    function isExistLogic(address _logicAddress) public view returns (bool) {
        ...
        for (uint256 i; i < _count; ) {
            if (multiStrategyData[
                multiStrategyName[i]].logicContract == _logicAddress) return true;
            unchecked {
                ++i;
            }
        }
