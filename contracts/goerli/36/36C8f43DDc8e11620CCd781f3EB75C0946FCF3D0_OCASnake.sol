...
contract OCASnakeArt is Ownable, ReentrancyGuard {
    ...
    //             ? Strings.toString(
    //                 uint256(generateAppleSeed(tokenId, 128)) % uint256(255)
    //             )
    //             : Strings.toString(
    //                 uint256(generateAppleSeed(tokenId, 128 + i)) % uint256(255)
    //             );
    //         string memory c2 = i == 1
    //             ? Strings.toString(
    //                 uint256(generateAppleSeed(tokenId, i - i)) % uint256(255)
    //             )
    //             : Strings.toString(
    //                 uint256(generateAppleSeed(tokenId, i)) % uint256(255)
    //             );
    //         string memory c3 = i == 1
    //             ? Strings.toString(
    //                 uint256(generateAppleSeed(tokenId, 69)) % uint256(128)
    //             )
    //             : Strings.toString(
    //                 uint256(generateAppleSeed(tokenId, 69 + i)) % uint256(128)
    //             );
    //         uint256 o = uint256(generateAppleSeed(tokenId, i) + i);

    //         if (i == 1) parts[i] = funkToString(c1, c2, c3, 1);
    //         else parts[i] = funkToString(c1, c2, c3, o);
    //     }
        ...
        returns (uint256)
    {
        uint256 seed = uint256(
            keccak256(abi.encodePacked(apple, tokenId, block.timestamp, salt))
        );
        return seed;
    }

    function selectDifficulty() public pure returns (string memory) {
        return Strings.toString(randomNumber(0, 3));
    }
            ...
            bytes(
                string(
                    abi.encodePacked(
                        ...
                        " 4px;background:",
                        selectedColors[
                            generateAppleSeed(tokenId, 1) % uint256(19)
                        ],
                        "'",
                        'onload="df=',
                        selectDifficulty(),
                        ...
                        "G('",
                        selectedColors[
                            generateAppleSeed(tokenId, 4) % uint256(19)
                        ],
                        "')(a);s.map(G('",
                        selectedColors[
    ...
    function retrofy(uint256 tokenId) public view returns (string[20] memory) {
        string[20] memory parts;
        ...
        for (uint256 i = 0; i < parts.length; i++) {
            ...
            if (i == 1) parts[1] = funkToString(c1, c2, c3, 1);
            else parts[i] = funkToString(c1, c2, c3, o);
        }
    ...
    function cloneSnake(uint256 tokenId) private view returns (bytes memory) {
        ...
        return
            abi.encodePacked(
                ...
                selectedColors[generateAppleSeed(tokenId, 3) % uint256(19)],
                " } .Rrrrr { font: italic 40px serif; fill:",
                selectedColors[generateAppleSeed(tokenId, 420) % uint256(19)],
                "; }</style><rect x='25' y='25' width='400' height='400' style='fill:",
                selectedColors[1],
                "; stroke:",
                selectedColors[
                    uint256(generateAppleSeed(tokenId, 69)) % uint256(20)
                ],
                "; stroke-width:5; stroke-opacity:0.9;' /> <rect x='50' y='50' width='12' height='12' style='fill:",
                selectedColors[
                    uint256(generateAppleSeed(tokenId, 4)) % uint256(20)
                ],
                ";'  />",
                string(svgData),
