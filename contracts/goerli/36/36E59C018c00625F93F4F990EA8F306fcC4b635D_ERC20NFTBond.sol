...
error CanNotSetLesserOrEqualZeroInterest();
error CanNotSetLesserOrEqualZeroMaturity();
...
abstract contract InterestParameters is Initializable, IInterestParameters, AccessManagedUpgradeable {
    ...
    internal {
        ...
        for (uint256 i = 0; i < _interests.length; ++i) {
            ...
            if (_interests[i] <= 0) revert CanNotSetLesserOrEqualZeroInterest();
            if (_maturities[i] <= 0) revert CanNotSetLesserOrEqualZeroMaturity();
        }
...
abstract contract LockedBondGranter is BondGranter {
    ...
    function activateCooldown(uint256 tokenId) public virtual {
        return _activateCooldown(tokenId);
    }
    ...
    /**
     * @dev Activates the cooldown period to the bond
     * @param tokenId uint256
     **/
    function _activateCooldown(uint256 tokenId) internal virtual {
        Bond memory bond = bonds[tokenId];
...
contract ERC20NFTBond is NFTBond {
    ...
    external initializer {
        ...
        __LockedBondGranter_init(_cooldownSeconds);
    }
...
bytes32 constant COOLDOWN_SETTER  = keccak256("COOLDOWN_SETTER");
