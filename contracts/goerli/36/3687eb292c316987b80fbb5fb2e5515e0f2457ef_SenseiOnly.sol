...
contract SenseiOnly is ERC1155Supply, Ownable {
    bool public saleIsActive = false;
    uint256 public activeBadgeId = 1;
    uint256 public maxPerTransaction = 1;
    uint256 public maxPerWallet = 1;
    uint256 public maxSupply = 1444;
    uint256 public constant NUMBER_RESERVED_TOKENS = 100;
    uint256 public constant PRICE = 0; //0.0 ETH
    string public name = "Sensei Only";
    ...
    constructor()
        ERC1155(
            "https://ipfs.io/ipfs/QmZ5im37Mkz8k1F6ayM3WN25rcRkXn8Wwur15rnTeN9/{id}.json"
        )
    {}

    address payable private recipient1 =
        payable(0x66Ef959830Ea35160d4325081ee9aFe939B536a8); //partners payout address
    ...
    function mintToken(uint256 amount) external payable {
        ...
        require(
            amount > 0 && amount <= maxPerTransaction,
            "Max per transaction reached, sale not allowed"
        );
        require(
            balanceOf(msg.sender, activeBadgeId) + amount <= maxPerWallet,
            "Limit per wallet reached with this amount, sale not allowed"
        );
        require(
            totalSupply(activeBadgeId) + amount <=
    ...
    function withdraw() external {
        require(
            msg.sender == recipient1 || msg.sender == owner(),
            "Invalid sender"
        );

        uint256 part = (address(this).balance / 100) * 30;
