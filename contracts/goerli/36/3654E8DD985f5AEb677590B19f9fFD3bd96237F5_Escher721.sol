...
contract Escher721Factory is CloneFactory {

    event NewEscher721Contract(address indexed creator, address indexed clone, address indexed uri);

    Creators public immutable creators;
    address public implementation;

    constructor(address _creators) {
    ...
    function createContract(string memory name, string memory symbol, address uri) external returns(address clone){
        require(creators.hasRole(creators.CREATOR_ROLE(), msg.sender), "NOT AUTHORIZED");

        clone = createClone(implementation);
        address uriDelegate = createClone(uri);
        Escher721(clone).initialize(msg.sender, uriDelegate, name, symbol);
        Base(uriDelegate).initialize(msg.sender);

        emit NewEscher721Contract(msg.sender, clone, uriDelegate);
    }
