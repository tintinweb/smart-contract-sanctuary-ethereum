...
contract FUND_WITHDRAW
{
    ...
    function FUND() public payable
    {
        ...
        Search_Amount[msg.sender] = msg.value;
    } 

    address public immutable OWNER; //immutable keyword with variables that dont update or change 
                                // throughout contract but are initialized in a different line or func.
   
    AggregatorV3Interface internal immutable PRICE_FEED; 
    address public PriceFeedAddress; //for testing we have to make it public or it can't be access by another contract if internal
    constructor(address priceFeedaddress)                      
    {
        OWNER = msg.sender;
        PriceFeedAddress = priceFeedaddress; // we initialized this so that we can access it in global scope for testing
    ...
    function CHEAP_WITHDRAW() public payable{
        require( msg.sender == OWNER, "YOU ARE NOT THE OWNER!");

        //HERE WE WILL FIRST LOAD " ARRAY / ANY VARIABLE " INTO MEMEORY BY DECLARING A MEMORY VARIABLE FOR IT

        address[] memory funders = Funders_Address;
        ...
        Funders_Address = new address[](0);  //RESETTING ARRAY

        (bool call_success, ) = payable(OWNER).call{value: address(this).balance}("");
        require(call_success, "ERROR!");
    }
    ...
    {
        FUND();   //fallback() will trigger if the external transaction has input data along vlaue
    }
