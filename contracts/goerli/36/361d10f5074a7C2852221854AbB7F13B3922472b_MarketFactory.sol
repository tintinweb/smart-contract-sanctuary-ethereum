...
contract MarketFactory is IMarketFactory {
        ...
        returns (address newMarketAddr)
    {
        ...
        string memory bSymbol = string(
            abi.encodePacked("b", IERC20Metadata(token).name())
        );
        string memory bName = string(
            abi.encodePacked("BUMPER wrapped ", bSymbol)
        );
        address bMaker = Clones.clone(bNftMakerCloneInstance);
        bNFTMaker(bMaker).initialize(bName, bSymbol, token, address(config));

        address bTaker = Clones.clone(bNftTakerCloneInstance);
        bNFTTaker(bTaker).initialize(bName, bSymbol, token, address(config));
    ...
    BACUpgradable
{
    ...
    function premiumWithPenaulty(uint id) public view virtual override returns (uint);

    /// @notice virtual function to calculate yield with penaulty
    function yieldWithPenaulty(uint id) public view virtual override returns (int);    
...
//        AR += posPremium;
        ...
        uint claimInStableTokens = 0;
        if (pos.floor > uint(_price)) {
            claimInStableTokens = (pos.floor - uint(_price)) / DIVIDER_ORACLE * pos.assetAmount;
            stable.safeTransfer(pos.owner, claimInStableTokens );
        }

        delete allTakerPositions[id];

        emit Canceled(msg.sender, id, amountToReturn, _premium, claimInStableTokens);
    }
