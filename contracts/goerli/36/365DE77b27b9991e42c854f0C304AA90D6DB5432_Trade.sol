...
contract Trade is AccessControl, Ownable {
    ...
    mapping(address => uint256) public royaltyRefTake;
    ...
    */
    ...
    function tradeAsset(
        ...
        if (fee.royaltyFee > 0) {
            if (
                royaltyRefTake[order.nftAddress] > 0 &&
                referral.referrer != address(0)
            ) {
                transferProxy.erc20safeTransferFrom(
                    IERC20(order.erc20Address),
                    order.buyer,
                    ...
                    fee
                        .royaltyFee
                        .mul(1000 - royaltyRefTake[order.nftAddress])
                        .div(1000)
                );
                transferProxy.erc20safeTransferFrom(
                    IERC20(order.erc20Address),
                    order.buyer,
                    referral.referrer,
                    fee.royaltyFee.mul(royaltyRefTake[order.nftAddress]).div(
                        1000
                    )
                );
            } else {
        ...
        if (referral.referrer != address(0) && referral.referralFee > 0) {
            uint256 referralFeeAmnt = referral.referralFee.mul(fee.assetFee);
            addressRefRewards[referral.referrer][
                order.erc20Address
            ] += referralFeeAmnt;
            transferProxy.erc20safeTransferFrom(
    ...
    function set721royaltyRefTake(
        uint256 refSplitAmount,
        address collectionAddress
    ) public {
        require(
            _msgSender() == ISokuNFT721(collectionAddress).owner(),
            "You do not own this 721 NFT."
        );
        require(
            refSplitAmount <= 1000,
            "Cannot give away more than 100 percent split"
        );
        royaltyRefTake[collectionAddress] = refSplitAmount;
    }

    function set1155royaltyRefTake(
        uint256 refSplitAmount,
        address collectionAddress
    ) public {
        require(
            _msgSender() == ISokuNFT1155(collectionAddress).owner(),
            "You do not own this 1155 NFT."
        );
        require(
            refSplitAmount <= 1000,
            "Cannot give away more than 100 percent split"
