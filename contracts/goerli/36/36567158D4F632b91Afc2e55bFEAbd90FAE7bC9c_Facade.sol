...
/// @notice Revert with the error if option's price is too high
/// @param currentPrice of the assest
error FACADE_ERR_OPTION_PRICE_TOO_HIGH(uint256 currentPrice);
...
contract Facade is Ownable {
     ...
     * @param pool The option pol of interest
     * @param timestamp The option expiry time in unix timestamp format
    ...
    function getOptionPrice(
        ...
        total = pool.calculateTotalPremium(timestamp, amount, strike);
    }

    /**
     * @notice Used for approving the pools contracts addresses.
     * @param pool The pool to approve
     **/
    function poolApprove(IOptionsPool pool) external {
     ...
     * @param pool The pool that the user wants to buy option in
     * @param timestamp The option expire timestamp
     * @param amount The option size
     * @param strike The option strike
    ...
    function createOption(
        ...
        uint256 optionPrice = getOptionPrice(pool, timestamp, amount, strike);
        if (optionPrice > acceptablePrice)
            revert FACADE_ERR_OPTION_PRICE_TOO_HIGH({
        ...
        pool.sellOption(buyer, timestamp, amount, strike);
    }

    /**
     * @notice Unlock options for user(s)
     * @param pool the pool that options we want to unlock
     * @param optionErc20 the optionErc20 contract which we would like to unlock
     * @param users the users that we would like to unlock for
     */
    function unlockUserOptions(
        IOptionsPool pool,
        IOptionErc20 optionErc20,
        address[] calldata users
    ) external {
        uint256 length = users.length;
        for (uint256 i = 0; i < length; i++) {
            pool.unlock(optionErc20, users[i]);
        }
    }

    /**
     * @notice Invalidate optionErc20(s) of a pool
     * @param pool The pool that optionErc20s that we want to invalidate
     * @param optionErc20s List of optionErc20s to invalidate
     */
    function unlockExpiredOptionErc20s(
        IOptionsPool pool,
        IOptionErc20[] calldata optionErc20s
    ) external {
        uint256 length = optionErc20s.length;
        for (uint256 i = 0; i < length; i++) {
            pool.unlockOptionErc20(optionErc20s[i]);
        }
    }
