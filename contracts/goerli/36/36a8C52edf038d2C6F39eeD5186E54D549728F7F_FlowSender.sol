...
contract FlowSender {
    ...
    function gainDaiX() external {
        ...
        IFakeDAI fdai = IFakeDAI( superToken.getUnderlyingToken() );
        
        // Mint 10,000 fDAI
        fdai.mint(address(this), 10000e18);

        // Approve fDAIx contract to spend fDAI
        fdai.approve(address(superToken), 20000e18);

        // Wrap the fDAI into fDAIx
        superToken.upgrade(10000e18);
    ...
    function createStream(int96 flowRate, address receiver) external {
        // setup platform fee variable to 5%
        int96 platformFee = int96(5);

        // Calculate the platform fee in wei and account for the flowRate having to be int96
        // int96 platformFeeWei = (flowRate * int96(platformFee)) / 100;
        int96 platformFeeWei = (flowRate * platformFee) / int96(100);

        // Create stream and deduct platform fee
        cfaV1.createFlow(
            receiver,
            superToken,
            flowRate - platformFeeWei,
            new bytes(0)
        );

        //Send the platform fee to the contract's owner
        // payable(msg.sender).transfer(uint256(platformFeeWei));
        payable(msg.sender).transfer(uint256(int256(platformFeeWei)));
    ...
    function updateStream(int96 flowRate, address receiver) external {
        ...
        // Update stream
        cfaV1.updateFlow(receiver, superToken, flowRate);
    ...
    function readFlowRate(address receiver) external view returns (int96 flowRate) {
        ...
        // Get flow rate
        (,flowRate,,) = cfaV1.cfa.getFlow(superToken, address(this), receiver);

    }

    
    /// @dev get the contract balance of fDAIx
    function getBalance() external view returns (uint256 balance) {

        balance = superToken.balanceOf(address(this));
