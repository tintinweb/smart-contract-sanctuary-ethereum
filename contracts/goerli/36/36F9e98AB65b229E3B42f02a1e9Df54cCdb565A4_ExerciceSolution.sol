...
{
    address private myERC721Address = 0xcb509fC7F371cF0580D2221b84920232f595368B;
    address private minterAddress = 0x07de845f9f7B6e304766d44FC782ed30C6bfDb16;

    constructor() public {} 

    function ERC721Address() override external returns(address){

        return myERC721Address;
    }

    function mintATokenForMe() override external returns(uint256){

        return Minter(minterAddress).mintATokenForMe();
    }

    function mintATokenForMeWithASignature(bytes calldata _signature) override external returns(uint256){

        return 0;
    }

	function getAddressFromSignature(bytes32 _hash, bytes calldata _signature) override external returns (address){

        return address(this);
    }

    function signerIsWhitelisted(bytes32 _hash, bytes calldata _signature) override external returns (bool){

        return false;
    }

    function whitelist(address _signer) override external returns (bool){
