...
contract Clifford is ERC721, ERC721Enumerable, ERC721URIStorage, Pausable, Ownable, ERC721Burnable {
    ...
    function setSubscriptionManager(VRFv2Consumer vrf) public onlyOwner {
        _VRF = vrf;
    }
          ...
          '.png", "animation_url": "data:text/html;base64,'
			...
			string memory jsonFinal = Base64.encode(
					bytes(
							string.concat(
									jsonInitial,
									_compose.composeHTML(combineUintArrays(getShell(_tokenId), combineUintArrays(getRWPositions(_tokenId), combineUintArrays(getLWPositions(_tokenId), getFPositions(_tokenId))))),
									'"}'
							)
					)
			);
