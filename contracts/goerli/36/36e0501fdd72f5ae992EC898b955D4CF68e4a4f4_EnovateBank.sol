...
contract EnovateBank {
    ...
    function depositETH() payable public {
        ...
        totalDeposit[msg.sender] = totalDeposit[msg.sender] + msg.value;
    ...
    function withdrawETH() public {
        ...
        uint withdrawalAmount = etherBalanceOf[msg.sender] + interest;

        etherBalanceOf[msg.sender] = 0; //reset user balance
        isDeposited[msg.sender] = false; //reset deposit status
        address payable recipient = payable(msg.sender);
        bool sent = recipient.send(withdrawalAmount);
        require(sent, "Error: failed to send Ether to user.");

        enovateToken.mint(msg.sender, interest); //interest to user
        totalTokensMinted[msg.sender] += interest;

        emit Withdraw(msg.sender, withdrawalAmount, depositTime, interest);
    }

    function borrow(uint _collateralEtherAmount) public {
        require(msg.sender != address(0) && _collateralEtherAmount > 0, 'Error: invalid address or amount');
        require(isBorrowed[msg.sender] == false, 'Error: loan already active');
        uint tokensToMint = _collateralEtherAmount / 2; //Mint 50% of collateral in tokens
        require(enovateToken.balanceOf(address(this)) >= tokensToMint, 'Error: insufficient token balance');
    ...
    function payoff() public {
        ...
        uint fee = collateralEther[msg.sender] / 10; //10% fee on repayment
        uint tokensToBurn = totalTokensMinted[msg.sender]; //Burn of minted tokens
    ...
    function getTotalDeposit() public view  returns (uint) {
        ...
        for (uint i = 0; i < addresses.length; i++) {
            totalDepositAmount += totalDeposit[addresses[i]];
        }
...
contract EnovateToken is ERC20, ERC20Burnable, Ownable {
  ...
  function passMinterRole(address EnovateBank) public returns (bool) {
  	require(msg.sender==minter, 'Error, only owner can change pass minter role');
  	minter = EnovateBank;

    emit MinterChanged(msg.sender, EnovateBank);
    return true;
  }
  ...
  function mint(address account, uint256 amount) public {
    ...
    function burn(address account, uint256 amount) public {
		require(msg.sender==minter, 'Error, msg.sender does not have burner role'); //dBank
		_burn(account, amount);
    }
}