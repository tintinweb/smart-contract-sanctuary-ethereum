...
contract RandomIpfsNft is VRFConsumerBaseV2, ERC721URIStorage, Ownable {
  // when we mint and nft, we will trigger a chain link vrf call to get us a random number
  ...
  ) VRFConsumerBaseV2(vrfCoordinatorV2) ERC721("RANDOM IPS NFT", "RIN") {
    ...
    override
  {
    ...
    uint256 moddedRng = randomWords[0] % MAX_CHANCE_VALUE; // --> gives words b/w 0-99

    Breed dogBreed = getBreedFromModdedRng(moddedRng);
    _safeMint(dogOwner, newTokenId);

    // ERC721URIStorage has a function _setTokenURI
  ...
  function getChanceArray() public pure returns (uint256[3] memory) {
    ...
    // 1st have 10% chance, 2nd has 30% and last dog has 60% chance
  }
