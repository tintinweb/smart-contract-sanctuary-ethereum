...
contract DomainVault is IDomainVault, ERC721Holder, OwnableUpgradeable, PausableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    // voter
    EnumerableSet.AddressSet private _voterList;
    ...
    *  @notice Buy presale fragment using ETH
     ...
     * @param reserveCurrency_ reserveCurrency
    ...
    function initialize(address fragment_,
        ...
        tokenId = tokenId_;
        //erc721 domain
        //domain = IDomainERC721(collection_).getDomainByTokenId(tokenId_);
        //治理设置
        domainSettings = domainSettings_;
        //币种
        reserveCurrency = reserveCurrency_;
        //保底价
    ...
    function afterTokenTransferForDomainERC20(address from, address to, uint256 amount) external {
        ...
        //投票已结束，不允许撤票
        if (voteEndTime > _currentTime()) {
            //判断投票人 from
            if (from != address(0) && _voterList.contains(from)) {
                //销毁或转给抵押库 不减票
                if (to != address(0) && to != address(this)) {
                    votes -= amount;
                    emit Vote(_msgSender(), votes);
                }
                //判断用户票数
                uint256 fragmentBalance = IERC20Metadata(fragment).balanceOf(from);
                if (fragmentBalance == 0) {
                    _voterList.remove(from);
                    emit RemoveVoter(from);
                }
            }
            //判断投票人 to
            if (to != address(0) && _voterList.contains(to)) {
                votes += amount;
                afterVote();
                emit Vote(_msgSender(), votes);
            }
    ...
    function redeem() public whenNotPaused nonReentrant {
        ...
        //投票未开始 或者 投票结束且投票未通过（提示：投票中或投票通过，不能发起买断）
        require(voteEndTime == 0 || (voteEndTime < _currentTime() && !_isVotePassed()), "DomainVault: voting or voted passed, can't redeeming");

        //拍卖未开始（提示：拍卖中，不能赎回）
        require(auctionEndTime < _currentTime(), "DomainVault: auctioning, can't redeeming");

        //退回原竞标者资金
        if (bidPrice > 0 && bidder != address(0)) {
            //IERC20Metadata(reserveCurrency).transfer(bidder, bidPrice);
            if (reserveCurrency == WETH) {
                IWETH(WETH).transfer(bidder, bidPrice);
            } else {
                IERC20Metadata(reserveCurrency).transfer(bidder, bidPrice);
            }
        }
    ...
    function firstBidUsingETHAndWETH(uint256 currentBidPrice) external payable whenNotPaused nonReentrant {
        ...
        uint256 totalBidIncreasePercentage = uint256(10000).add(bidIncreasePercentage);

        //保底价(加价大于1%)（提示：买断价小于保底价）
        require(uint256(currentBidPrice).mul(10000) >= uint256(reservePrice).mul(totalBidIncreasePercentage), "DomainVault: bid price lower than reserve price");
        ...
        //投票未开始 或者 投票结束且投票未通过（提示：投票中或投票通过，不能发起买断）
        require(voteEndTime == 0 || (voteEndTime < _currentTime() && !_isVotePassed()), "DomainVault: voting or voted passed, can't make bid");

        //拍卖结束或未开始（提示：拍卖中，不能发起买断）
        require(auctionEndTime < _currentTime(), "DomainVault: auctioning, can't make bid");
        ...
        //投票结束时间开始计时
        voteEndTime = voteDuration.add(_currentTime());

        //竞拍截止时间
        auctionEndTime = uint256(0);
        ...
        emit FirstBid(_msgSender(), bidPrice);
    }
    ...
    function firstBid(uint256 currentBidPrice) external payable whenNotPaused nonReentrant {
        ...
        //收款
        IERC20Metadata(reserveCurrency).transferFrom(_msgSender(), address(this), currentBidPrice);

        //投票未开始 或者 投票结束且投票未通过（提示：投票中或投票通过，不能发起买断）
        require(voteEndTime == 0 || (voteEndTime < _currentTime() && !_isVotePassed()), "DomainVault: voting or voted passed, can't make offer bid");

        //拍卖结束或未开始（提示：拍卖中，不能发起买断）
        require(auctionEndTime < _currentTime(), "DomainVault: auctioning, can't make offer bid");
    ...
    function vote() external whenNotPaused nonReentrant {
        ...
        //是否已经投票通过
        require(!_isVotePassed(), "DomainVault: vote passed, can't vote");

        //是否重复投票
        require(!_voterList.contains(_msgSender()), "DomainVault: You have voted");

        //投票已开始（提示：投票未开始）
        require(voteEndTime > 0, "DomainVault: voting has not begun");

        //投票未结束（提示：投票已关闭）
        require(voteEndTime > _currentTime(), "DomainVault: vote closed");

        //拍卖结束或未开始（提示：拍卖中，不能投票）
        require(auctionEndTime < _currentTime(), "DomainVault: auctioning, can't vote");

        //判断用户票数
        uint256 fragmentBalance = IERC20Metadata(fragment).balanceOf(_msgSender());

        //用户余额大于0（提示：用户票数不足）
        require(fragmentBalance > 0, "DomainVault: insufficient fragment votes");

        //添加票数
        votes += fragmentBalance;

        //添加投票人
        if (!_voterList.contains(_msgSender())) {
            _voterList.add(_msgSender());
    ...
    function afterVote() internal {
        //如果还未通过，并且投票结束时间小于15分钟，投票结束时间延长15分钟
        if (!_isVotePassed() && voteEndTime.sub(_currentTime()) <= 15 minutes) {
            voteEndTime += 15 minutes;
        }

        //如果投票已经通过，设置拍卖结束时间
        if (_isVotePassed() && auctionEndTime < _currentTime()) {
            auctionEndTime = auctionDuration.add(_currentTime());
            voteEndTime = uint256(0);
        }
    }
    ...
    function bidUsingETHAndWETH(uint256 currentBidPrice) external payable whenNotPaused nonReentrant {
        ...
        //判断出价人是否相同（提示：当前竞拍者）
        require(bidder != _msgSender(), "DomainVault: you already bid");

        //投票结束，投票通过（提示：投票中或投票未通过，不能竞标）
        require(voteEndTime < _currentTime() || _isVotePassed(), "DomainVault: voting or no vote passed, can't bid");

        //拍卖未结束（提示：拍卖关闭，不能竞标）
        require(auctionEndTime > _currentTime(), "DomainVault: auction close, can't bid");
        ...
        //保底价(加价大于1%)（提示：买断价小于保底价）
        require(uint256(currentBidPrice).mul(uint256(10000)) >= uint256(bidPrice).mul(totalBidIncreasePercentage), "DomainVault: bid price lower than reserve price");
        ...
        //拍卖时间延长15分钟
        if (auctionEndTime.sub(_currentTime()) <= 15 minutes) {
    ...
    function withdrawBidFunds() external payable whenNotPaused nonReentrant {
        ...
        //投票已结束（提示：投票中，不能取回买断资金）
        require(voteEndTime < _currentTime(), "DomainVault: voting, can't withdraw bid funds");

        //投票不通过（提示：投票已投过，不能取回买断资金）
        require(!_isVotePassed(), "DomainVault: vote passed, can't withdraw bid funds");

        //当前竞标者（提示：不是竞标者，不能取回买断资金）
        require(_msgSender() == bidder, "DomainVault: you can't withdraw bid funds");

        //当前资金大于0（提示：您已取回竞拍资金）
        require(bidPrice > 0, "DomainVault: You got bid funds back");

        uint256 bidPrice_ = bidPrice;
        ...
        emit WithdrawBidFunds(_msgSender(), bidPrice_);
    }
    ...
    function claimNFT() external whenNotPaused nonReentrant {
        ...
        //投票已结束（提示：投票中）
        require(voteEndTime < _currentTime(), "DomainVault: voting, can't claim NFT");

        //投票通过（提示：投票未通过）
        require(_isVotePassed(), "DomainVault: vote no passed, can't claim NFT");

        //拍卖已结束（提示：拍卖中）
        require(auctionEndTime < _currentTime(), "DomainVault: auctioning, can't claim NFT");

        //当前竞标者（提示：不是竞标者，不能取回erc721）
        require(_msgSender() == bidder, "DomainVault: you can't claim NFT");
    ...
    //碎片持有人取资金
    function claimFundsWithPermit(uint256 withdrawERC20Amount, uint256 deadline, uint8 v, bytes32 r, bytes32 s) external {
        //签名授权
        IDomainERC20(fragment).permit(_msgSender(), address(this), withdrawERC20Amount, deadline, v, r, s);
        //
        claimFunds(withdrawERC20Amount);
    }

    //碎片持有人取资金
    function claimFunds(uint256 withdrawERC20Amount) public whenNotPaused nonReentrant {

        //投票通过（提示：投票未通过）
        require(_isVotePassed(), "DomainVault: vote passed, can't claim funds");

        //拍卖已结束（提示：拍卖中，不能取回资金）
        require(auctionEndTime < _currentTime(), "DomainVault: auctioning, can't claim funds");

        //调用者碎片本次领取额度大于0，小于余额
        uint256 erc20Balance = IERC20Metadata(fragment).balanceOf(_msgSender());
        require(erc20Balance > 0 && withdrawERC20Amount <= erc20Balance, "DomainVault: you have insufficient fragment balance");

        //当前抵押库资金大于0
        uint256 vaultFundsBalance = IERC20Metadata(reserveCurrency).balanceOf(address(this));
        ...
        uint256 fundsShareAmount = (withdrawERC20Amount.mul(vaultFundsBalance)).div(originalTotalSupply_);

        //本次取回
        require(fundsShareAmount <= vaultFundsBalance, "DomainVault: you have insufficient funds");

        //调用者本次取的碎片转入当前
        IERC20Metadata(fragment).transferFrom(_msgSender(), address(this), withdrawERC20Amount);

        //销毁碎片
        IDomainERC20(fragment).burnForDomainVault(withdrawERC20Amount);

        //资金转账给调用者
        IERC20Metadata(reserveCurrency).transfer(_msgSender(), fundsShareAmount);

        emit ClaimFunds(_msgSender(), withdrawERC20Amount, fundsShareAmount);
    }

    //投票人列表
    function getVoterList() external view returns (address[] memory voterList) {
        return _voterList.values();
    }

    //票数是否通过
    function _isVotePassed() internal returns (bool) {
        if (!votePassed) {
            uint256 originalTotalSupply_ = IDomainERC20(fragment).originalTotalSupply();
            votePassed = votes.mul(uint256(10000)) >= originalTotalSupply_.mul(votePercentage);
        }
        return votePassed;
    }

    //清空投票人
    function _clearVoterList() internal {
        uint256 length = _voterList.length();
        if (length > 0) {
            for (uint256 i = 0; i < length; i++) {
                address voter_ = _voterList.at(i);
                _voterList.remove(voter_);
                emit RemoveVoter(voter_);
            }
        }
    }
