...
contract Settings is OwnableUpgradeable {
    ...
    function initialize(address _weth) external initializer {
        ...
        weth = _weth;
        auctionLength = Constants.VAULT_AUCTION_LENGTH;
        auctionExtendLength = Constants.VAULT_AUCTION_EXTEND_LENGTH;
        feeReceiver = payable(msg.sender);
        feePercentage = 700; //70%
        minExitFactor = 200; // 20%
        maxExitFactor = 5000; // 500%
        minBidIncrease = 10; // 1%
    ...
    {
        GovernorSetting memory conf = governorSettings[nftAddr];
        address governor;
        uint256 delayBlock = Constants.VOTING_DEPLAY_BLOCK;
        uint256 periodBlock = Constants.VOTING_PERIOD_BLOCK;
        ...
        return (governor, delayBlock, periodBlock);
    }
    ...
    function setReduceStep(uint256 _reduceStep) external onlyOwner {
        require(reduceStep > 1000, "too high");
    ...
    function setMinBidIncrease(uint256 _min) external onlyOwner {
        require(_min <= 100, "min bid increase too high");
