...
contract Forwarder is Initializable , AccessControlUpgradeable {
    ...
    function initialize(address _vault) initializer public {
        ...
        _grantRole(FLUSH_ROLE, msg.sender);
        vault = _vault;
    }
    function changeVault(address _newVault) public onlyRole(DEFAULT_ADMIN_ROLE) {
        require(_newVault != address(0), "Vault address can't be zero");
        vault = _newVault;
    }
    ...
    */
    function flushTokens(address tokenContractAddress) public onlyRole(FLUSH_ROLE) {
    ...
    function flushETH() public onlyRole(FLUSH_ROLE){
