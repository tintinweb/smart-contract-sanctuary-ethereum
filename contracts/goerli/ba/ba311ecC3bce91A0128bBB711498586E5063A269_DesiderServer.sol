...
contract DesiderServer {
    ...
    function _getRandomNumber(uint _upper) internal returns (uint256) {
    ...
    function send721ToL2(address _addr721, uint256 l2ContractAddress, uint256 l2UserAddress, uint256 tokenId) external {
        address owner = ERC721(_addr721).ownerOf(tokenId);
        ...
        string memory _tokenURI = DesiderNft(_addr721).onlyTokenIdUri(tokenId);
        uint256 lucky = DesiderNft(_addr721).getLucky(tokenId);
        uint256 earn_tm = DesiderNft(_addr721).getLastEarnTm(tokenId);
        
        DesiderNft(_addr721).burnForTransferToL2(tokenId);
    ...
    /// @param tokenId        ID of the token
    function retrieve721(address _addr721, uint256 l2ContractAddress, uint256 tokenId, uint256 lucky, uint256 earn_tm) external {
        ...
        DesiderNft(_addr721).receiveFromL1ReMint(msg.sender, tokenId, lucky, earn_tm);
    }
