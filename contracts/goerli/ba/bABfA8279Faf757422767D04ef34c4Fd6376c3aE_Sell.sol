...
contract Sell is Permission, Ownable, ReentrancyGuard {

    uint public ticketPrice = 0.00005 ether;
    IGGG private gggAddress = IGGG(0xC7083B94cD6348a6a40f130f018122FdF255C0c2);
    IFFF private fffAddress = IFFF(0x28981A3eE7BC2f17dE9F1373A5f40abFd190FD49);
    IFFF2 private fff2Address = IFFF2(0xaAeD89E61a5cDb3C9428e7d17d0A4207DA1c9c63);
    ...
    event Level(uint[] tokenId);

    function _verifySigner(bytes memory _signature) internal view returns (address){
    ...
    function getWhitelist(uint8 _table, address add)public view returns(bool) {
        return fff2Address.whitelist(_table, add);
    }

    function getInvestor(address add)public view returns(uint256) {
        return fff2Address.investor(add);
    }
    ...
    function buyVIP(bytes memory _signature,uint256 _tokenId) external payable nonReentrant{
        require(!_isContract(msg.sender), "It is contracts");
        require(_verifySigner(_signature)==msg.sender,"The signer is error");
        ...
        require(fffAddress.ownerOf(_tokenId) == msg.sender,"It is not your");
        require(!alreadyUsed[_tokenId],"NFT already used!");
        sellAmount = sellAmount + 1;
        alreadyUsed[_tokenId] = true;
        alreadymint[msg.sender] = true;
        gggAddress.mint(msg.sender);
        fffAddress.addLevel(_tokenId);
        uint[] memory level = new uint[](1);
        level[0] = _tokenId;
        emit Level(level);
    }

    function buyfff(bytes memory _signature,uint256 _tokenId) external payable nonReentrant{
        require(!_isContract(msg.sender), "It is contracts");
        require(_verifySigner(_signature)==msg.sender,"The signer is error");
        require(msg.value >= ticketPrice,"Your eth is not enough!");
        require(sellAmount + 1 <= 999,"It is solt out");
    ...
    function buyPublic(bytes memory _signature) external payable nonReentrant{
        require(!_isContract(msg.sender), "It is contracts");
        require(_verifySigner(_signature)==msg.sender,"The signer is error");
        require(msg.value >= ticketPrice,"Your eth is not enough!");
        require(sellAmount + 1 <= 999,"It is solt out");
        ...
        require(!alreadymint[msg.sender],"You are already minted");
        uint256 nftAmount = fffAddress.balanceOf(msg.sender);
        uint256 _tokenId = 0;
        if(nftAmount > 0){
            for(uint16 i = 0; i < nftAmount; i++){
                if(!alreadyUsed[fffAddress.tokenOfOwnerByIndex(msg.sender, i)]){
                    _tokenId = fffAddress.tokenOfOwnerByIndex(msg.sender, i);
                    break;
                }
            }
            if(_tokenId > 0){
    ...
    function getToken(uint256 _amount)private view returns(uint[] memory) {
        uint256 nftAmount = fffAddress.balanceOf(msg.sender);
        uint256 am = 0;
        uint[] memory level = new uint[](_amount);
        for(uint16 i = 0; i < nftAmount; i++){
            uint256 _token = fffAddress.tokenOfOwnerByIndex(msg.sender, i);
            if(!alreadyUsed[_token]){
                level[am] = _token;
                am = am + 1;
            }
            if(am == _amount){
        ...
        require(am == _amount,"Your fff is not enough!");
        return level;
    }

    function buyBatchInvestor(bytes memory _signature) external payable nonReentrant{
        ...
        gggAddress.mintBatch(msg.sender, _amount);
        uint[] memory level = getToken(_amount);
        for(uint16 i = 0; i < _amount; i++){
            uint256 _tokenId = level[i];
            fffAddress.addLevel(_tokenId);
            alreadyUsed[_tokenId] = true;
        }
        emit Level(level);
    ...
    function setFAB(IFFF _fab) external onlyRole(MANAGER_ROLE){
        fffAddress = _fab;
    }

    function setFAB2(IFFF2 _fab) external onlyRole(MANAGER_ROLE){
        fff2Address = _fab;
    }

    function setGIR(IGGG _gir) external onlyRole(MANAGER_ROLE){
        gggAddress = _gir;
    }
