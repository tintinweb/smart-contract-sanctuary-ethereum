...
contract DopeAggregator is SpecialTransferHelper, Ownable, ReentrancyGuard {
    ...
    constructor(
        ...
        affiliates.push(AffiliateDetails(_feeRecipient, true));
    }
    ...
    function batchBuyFromOpenSea(OpenseaTrades[] memory openseaTrades) external payable nonReentrant {
        // execute trades
        for (uint256 i = 0; i < openseaTrades.length; i++) {
            // execute trade
            address(0x7Be8076f4EA4A4AD08075C2508e481d6C946D12b).call{value: openseaTrades[i].value}(
