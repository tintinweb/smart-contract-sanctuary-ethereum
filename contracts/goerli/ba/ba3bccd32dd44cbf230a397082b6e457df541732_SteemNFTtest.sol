...
Contract by @MetaShoba
*/

contract SteemNFTtest is Ownable, ERC721A {
    bytes32 public merkleRootCollabList = 0x0419b031a2b2fe33113ed5bf8c24458dfa3dfc477e579152bfc74254133f12d0;
    ...
    uint256 public collabListMax = 4;
    uint256 public allowListMax = 4;
    uint256 public publicMintMax = 12;
    
    uint256 public COLLAB_LIST_PRICE = 0.001 ether;
    uint256 public ALLOW_LIST_PRICE = 0.0011 ether;
    uint256 public PUBLIC_PRICE = 0.0013 ether;

    uint256 public COLLAB_LIST_MINT_LIMIT_TXN = 2;
    uint256 public ALLOW_LIST_MINT_LIMIT_TXN = 2;
    uint256 public PUBLIC_MINT_LIMIT_TXN = 5;

    uint256 public COLLAB_LIST_MINT_LIMIT = 2;
    uint256 public ALLOW_LIST_MINT_LIMIT = 2;
    uint256 public PUBLIC_MINT_LIMIT = 10;

    uint256 public totalSupplyCollabList;
    uint256 public totalSupplyAllowList;
    uint256 public totalSupplyPublic;

    // Remaining 85% goes to Treasury for future development

    uint256 internal FOUNDER_WITHDRAW_PERCENT = 1500;
    ...
    address constant internal FOUNDER_ADDRESS = 0x1F16fb1CF6E0E62208116d6926D6c06003387a76;
    address constant internal TREASURY_ADDRESS = 0xC83Da0bF969eD23E0926550e4358E6942Ae44Ca3;
    
    mapping(address => uint256) public userMintedCollab;
    mapping(address => bool) public userMintedWL;
    mapping(address => uint256) public numUserMints;

    constructor() ERC721A("SteemNFTtest", "STEEM") { }
    ...
    */
     ...
     function collabListMint(uint256 quantity, bytes32[] memory proof) external payable mintCompliance(quantity) {
        require(_verifyPublicAllowList(proof, merkleRootCollabList), "User not on Collab list");
        
        require(totalSupplyCollabList + quantity <= collabListMax, "Collab supply out");

        uint256 price = COLLAB_LIST_PRICE;
        uint256 currMints = userMintedCollab[msg.sender];

        require(quantity <= COLLAB_LIST_MINT_LIMIT_TXN, "2 mints per tx"); 
        require(currMints + quantity <= COLLAB_LIST_MINT_LIMIT, "User max mint limit");   
    
        refundOverpay(price * quantity);
        
        userMintedCollab[msg.sender] = (currMints + quantity);
        totalSupplyCollabList += quantity;
    ...
    */
    ...
    function setCollabListMerkleRoot(bytes32 _merkleRoot) external onlyOwner {
        merkleRootCollabList = _merkleRoot;
    }
    ...
    function setCollabListMax(uint256 _collabListMax) public onlyOwner {
        collabListMax = _collabListMax;
    }

    function setAllowListMax(uint256 _allowListMax) public onlyOwner {
        allowListMax = _allowListMax;
        publicMintMax = MAX_SUPPLY - collabListMax - _allowListMax;
    }    
    ...
    function setMaxSupplytotal(uint256 _maxSupplyTotal) public onlyOwner {
        MAX_SUPPLY = _maxSupplyTotal;
    }

    function setCollablistprice(uint256 _collablistprice) public onlyOwner {
        COLLAB_LIST_PRICE = _collablistprice;
    }

    function setAllowlistprice(uint256 _allowlistprice) public onlyOwner {
        ALLOW_LIST_PRICE = _allowlistprice;
    }
