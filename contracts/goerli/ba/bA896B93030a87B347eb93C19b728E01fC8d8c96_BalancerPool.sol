...
contract BalancerPool is IJellyPool, IJellyContract, JellyPoolNFT {
     ...
     */
    function claimBal(address _mintr, address _vault) external returns (uint256){
        require(
        ...
        uint256 balance = 0;
        IMinter(_mintr).mint(address(liquidityGauge));
     ...
     */
    function initJellyPool(address _poolToken, address _accessControls) public {
    ...
    function initContract(bytes calldata _data) external override {
        (address _poolToken, address _accessControls) = abi.decode(
            _data,
            (address, address)
        );

        initJellyPool(_poolToken, _accessControls);
    }
}
