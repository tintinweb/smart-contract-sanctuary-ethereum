...
contract GAT is ERC721A, Ownable, ReentrancyGuard {
  ...
  function _setProjectWallet(address _PROJECT_WALLET, uint16 _value ,string calldata _salt, bytes calldata _token, address signer) external onlyOwner {
    if (!verifyTokenForAddress(_salt, _token, msg.sender, _value, signer))
