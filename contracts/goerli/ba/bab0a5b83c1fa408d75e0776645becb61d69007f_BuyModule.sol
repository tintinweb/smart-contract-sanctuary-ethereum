...
library Sealib {
    ...
    address private constant SEAPORT_PAYABLE_PROXY =
    ...
    function buildListOrderFor(
        ...
        // Consideration items
        // We add this since the opensea backend doesn't accept if considerations[0].recipient != offerer
        ConsiderationItem memory offererConsideration;
        offererConsideration.itemType = ItemType.NATIVE;
        offererConsideration.token = address(0);
        offererConsideration.startAmount = 0;
        offererConsideration.endAmount = 0;
        offererConsideration.recipient = payable(offerer);
    ...
    function convertOrderToOrderComponents(Order memory seaportOrder)
        internal
        pure
        returns (OrderComponents memory orderComponents)
    {
