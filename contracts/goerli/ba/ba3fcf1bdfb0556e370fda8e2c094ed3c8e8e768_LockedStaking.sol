...
contract LockedStaking is Initializable, OwnableUpgradeable {
    ...
    event NftMinted(address indexed addr, uint256 index, uint256 tokenId);
    event NftLockUpdated(uint256 indexed tokenId, uint208 amount, uint32 end, uint16 multiplier);
    event UnlockNft(uint256 indexed tokenId, uint256 amount);
    ...
    ILockNFT public lockNFT;
    uint256 public nextTokenId = 1;

    mapping(uint256 => Lock) public nftLocks;
    mapping(uint256 => uint256) public nftLastAccRewardWeight;
    ...
    modifier onlyNftOwner(uint256 tokenId) {
        if (lockNFT.ownerOf(tokenId) != msg.sender) revert NotOwner();
        _;
    }
    ...
    // gets rewards weight & returns nft claimable amount
    function getNftClaimable(uint256 tokenId) external view returns (uint256 claimable) {
        uint256 accRewardsWeight = getRewardsWeight();

        return calculateNftClaimable(tokenId, accRewardsWeight);
    }
    ...
    // returns users claimable amount
    function calculateNftClaimable(uint256 tokenId, uint256 accRewardsWeight_)
        internal
        view
        returns (uint256 claimable)
    {
        Lock storage lock = nftLocks[tokenId];
        return (lock.multiplier * lock.amount * (accRewardsWeight_ - nftLastAccRewardWeight[tokenId])) / precision;
    }
    ...
    function setRewardoor(address addr, bool value) public onlyOwner {
        rewardoors[addr] = value;
    }

    function setLockNFTAddress(address addr) public onlyOwner {
        lockNFT = ILockNFT(addr);
    }

    function mintNFTForLock(uint256 index) public {
        Lock storage lock = locks[msg.sender][index];
        if (lock.amount == 0) revert AmountIsZero();

        uint256 tokenId = nextTokenId++;

        lockNFT.mint(msg.sender, tokenId);

        nftLocks[tokenId] = lock;

        uint256 newAccRewardsWeight = updateRewardsWeight();
        uint256 claimable = calculateUserClaimable(msg.sender, newAccRewardsWeight);

        userLastAccRewardsWeight[msg.sender] = newAccRewardsWeight;
        nftLastAccRewardWeight[tokenId] = newAccRewardsWeight;
        ...
        emit NftMinted(msg.sender, index, tokenId);
    }

    // claims for current NFT lock and adds amount to existing lock, keeping the same end
    function updateNFTLockAmount(uint256 tokenId, uint208 amount) external onlyNftOwner(tokenId) {
        if (amount == 0) revert AmountIsZero();
        ...
        uint256 claimable = calculateNftClaimable(tokenId, newAccRewardsWeight);
        ...
        emit NftLockUpdated(tokenId, newAmount, lock.end, multiplier);
    }

    // claims for current NFT lock and increases duration of existing lock
    function updateNftLockDuration(uint256 tokenId, uint256 duration) external onlyNftOwner(tokenId) {
        uint256 newAccRewardsWeight = updateRewardsWeight();
        ...
        emit NftLockUpdated(tokenId, amount, end, multiplier);
    }

    // adds claimable to current lock, keeping the same end
    function compoundNft(uint256 tokenId) external onlyNftOwner(tokenId) {
        uint256 newAccRewardsWeight = updateRewardsWeight();
    ...
    // returns locked amount to user and deletes NFT lock
    function unlockNFt(uint256 tokenId) external onlyNftOwner(tokenId) {
        uint256 newAccRewardsWeight = updateRewardsWeight();
        Lock storage lock = nftLocks[tokenId];
        ...
        nftLastAccRewardWeight[tokenId] = 0;

        delete nftLocks[tokenId];

        lockNFT.burn(tokenId);
