...
contract CookiCard is ICookiCard, ERC721, Ownable {
    ...
    function mintAndDistribute(address[] memory _recipients) external onlyOwner {
        require(initialised);
        ...
        for (uint256 i = 0; i < length; i++) {
            ...
            uint256 seed = uint256(keccak256(abi.encodePacked(block.number + nonce)));
    ...
    function drawFull(uint256 _tokenId) internal view returns (string memory) {
        ...
        svgHTML = string.concat('{"name": "CookiCard #', Strings.toString(_tokenId), '", "description": "I find the Ourobric memes to be the most satisfying to contemplate.", "image": "data:image/svg+xml;base64,', Base64.encode(bytes(svgHTML)), '"');
