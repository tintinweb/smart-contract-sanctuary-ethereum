...
contract Airdrop is Validatable {
     ...
     */
    function setWhitelist(address[] memory receivers, uint256[] memory amounts) external onlyAdmin {
        require(receivers.length > 0 && amounts.length == receivers.length, "Invalid length");
        for (uint256 i = 0; i < receivers.length; i++) {
            require(receivers[i] != address(0), "Invalid address");
            claimable[receivers[i]] = amounts[i];
        }

        emit SetWhitelist(receivers, amounts);
    }
    ...
    function airdrop() external {
        ...
        require(claimable[_msgSender()] > 0, "User is not in whitelist");

        uint256 claimAmount = claimable[_msgSender()];
        claimable[_msgSender()] = 0;
        // Mint Airdrop NFTs
        //slither-disable-next-line unused-return
        genesis.mintBatch(_msgSender(), claimAmount);

        // Emit events
        emit Airdroped(_msgSender(), claimAmount);
    }
}
