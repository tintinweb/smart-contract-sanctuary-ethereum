...
contract OptionLP is Ownable, ReentrancyGuard, Pausable {
    ...
    mapping(address => LpPosition[]) public getLpPositions;
    ...
    // mapping (user => striken token => LpPosition[])
    mapping(address => mapping(address => LpPosition[]))
        internal userLpPositions;
    ...
    function addToLP(
        ...
        ISSOV ssov = getSsov(tokenVaultRegistry[token][isPut]);
    ...
    function _addLiquidityForStrike(
        ...
        address epochStrikeToken = ssov.epochStrikeTokens(currentEpoch, strike);
        ...
        getLpPositions[epochStrikeToken].push(lpPos);
        userLpPositions[buyer][epochStrikeToken].push(lpPos);
        ...
        emit LiquidityForStrikeAdded(
            epochStrikeToken,
            getLpPositions[epochStrikeToken].length - 1
        );
    ...
    function fillLPPosition(
        ...
        ISSOV ssov = getSsov(getOptionTokenInfo[epochStrikeToken].ssov);
        uint256 currentEpoch = ssov.currentEpoch();
        (, uint256 epochExpiry) = ssov.getEpochTimes(currentEpoch);

        require(
            lpIndex < getLpPositions[epochStrikeToken].length,
    ...
    function _fillLPPosition(
        ...
        // Update LP position
        getLpPositions[epochStrikeToken][lpIndex].liquidity -= premium;
        getLpPositions[epochStrikeToken][lpIndex].liquidityUsed += premium;
        getLpPositions[epochStrikeToken][lpIndex].purchased += amount;
        getOptionTokenInfo[epochStrikeToken].tokenLiquidity -= premium;

        // if liquidity is lower than threshold, kill the position
        if (
            getLpPositions[epochStrikeToken][lpIndex].liquidity < DUST_THRESHOLD
        ) {
    ...
    function _killAndTransfer(
        ...
        getLpPositions[epochStrikeToken][lpIndex].killed = true;
        getOptionTokenInfo[epochStrikeToken].tokenLiquidity -= liquidity;
    ...
    {
        positions = userLpPositions[user][epochStrikeToken];
    }

    function getSsov(address vault) public view returns (ISSOV) {
        return ISSOV(vault);
    }
    ...
    {
        uint256[] memory strikes = getEpochStrikes(vault, epoch);
        tokens = new address[](strikes.length);
        for (uint256 i; i < strikes.length; ++i) {
            tokens[i] = getSsov(vault).epochStrikeTokens(epoch, strikes[i]);
        }
    }

    function getEpochStrikes(address vault, uint256 epoch)
        public
        view
