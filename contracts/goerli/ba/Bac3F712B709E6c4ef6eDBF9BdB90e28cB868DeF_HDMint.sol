...
import "./extensions/HDCryptography.sol";
import "./interfaces/IHDMint.sol";
    ...
    IHDMint
{
    ...
    /**
     * Pause and unpause functions that has `whenNotPaused` modifier
     * @dev Reverts if called by anyone other than the contract admin.
     */
     ...
     */
    function safeMint(address _to, string calldata _uri) public {
    ...
    function gaslessMint(address _redeemer, NFTVoucher calldata _voucher)
        external
        payable
    {
        if (msg.value < _voucher.minPrice) revert InvalidAmount(_voucher.minPrice);
        address signer = _verify(_voucher.minPrice, _voucher.uri, _voucher.signature);
        if (signer != _voucher.signer) revert InvalidSignature();
        safeMint(signer, _voucher.uri);
        _transfer(signer, _redeemer, _currentIndex);
        payable(signer).transfer(msg.value);
        emit GaslessTransfer(signer, _redeemer, _voucher.signature, _currentIndex);
    }
...
contract HDCryptography is EIP712Upgradeable {
    function _hash(uint256 minPrice, string calldata uri)
        internal
        view
        returns (bytes32)
    {
    ...
    function _verify(
        ...
        bytes32 digest = _hash(minPrice, uri);
