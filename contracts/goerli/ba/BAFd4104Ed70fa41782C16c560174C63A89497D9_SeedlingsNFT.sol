...
contract SeedlingsNFT is ERC721, ERC721Enumerable, ERC2981, Ownable, ReentrancyGuard {
    ...
    mapping(address => bool) public alMinted;

    constructor() payable ERC721('Seedlings', 'SEED') {

        mintPrice = 0.000002 ether;
	alMintPrice = 0.000001 ether;
        maxSupply = 10000;
        maxPerWallet = 25;
        maxPerPurchase = 5;
        devReserve = 40;
    ...
    function checkAllowlisted(bytes32[] calldata _merkleProof) public view returns (bool) {
	require(!alMinted[msg.sender], "You have already done an allowlist mint.");
	bytes32 leaf = keccak256(abi.encodePacked(msg.sender));
	return MerkleProof.verify(_merkleProof, alMerkleRoot, leaf);
    }
   ...
   function allowlistMint(bytes32[] calldata _merkleProof) external payable nonReentrant {
   	require(isAllowlistMintEnabled, 'Allowlist minting is not enabled.');
	uint256 totals = totalSupply();
        
	require(totals.add(1) <= maxSupply, 'Sold Out');
	
	require(checkAllowlisted(_merkleProof));

	require(msg.value >= alMintPrice, 'Insufficient funds');

	uint256 refundNeeded = msg.value.sub(alMintPrice);

	alMinted[msg.sender] = true;

	_safeMint(msg.sender, totals + 1 );

	if (refundNeeded > 0) {
		payable(msg.sender).transfer(refundNeeded);
	}
   } 
