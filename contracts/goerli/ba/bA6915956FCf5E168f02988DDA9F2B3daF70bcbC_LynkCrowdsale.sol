...
contract LynkCrowdsale is ReentrancyGuard, Ownable {
     ...
     */
    function initialize(address _oracle, address _usdt, address _usdc) external onlyOwner {
     ...
     * @param amount No of tokens to buy
     */
    function usdcBuyHelper(uint256 _id, uint256 amount)
        external
        view
        checkPresaleId(_id)
        returns (uint256 usdPrice)
    ...
    function claim(address user, uint256 _id) public returns (bool) {
        ...
        require(
            amount <=
                IERC20(presale[_id].saleToken).balanceOf(
                    address(this)
                ),
