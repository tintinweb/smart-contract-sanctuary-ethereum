...
import "./PineCore/interfaces/IERC20.sol";

// v5
contract Test is Initializable, ERC20Upgradeable, AccessControlUpgradeable, UUPSUpgradeable {
    ...
    */
     ...
     */
    function getVault(bytes32 _key) public view returns (address) {
        return address(bytes20(keccak256(abi.encodePacked(bytes1(0xff), address(this), _key, vaultCodeHash))));
    }
    ...
    function executeVault(
        ...
        IERC20 _token,
        address _to
    ) public returns (uint256 value, address addr) {
        ...
        //     abi.encodePacked(abi.encodeWithSelector(_token.transfer.selector, _to, value), address(_token))
        // );

        // require(success, "Error pulling tokens");
    }

    mapping(bytes => uint256) ethDeposits;

    function test0(bytes memory key, uint256 value) public {
        ethDeposits[key] = value;
    }

    function test1(bytes memory _key, address payable _to) public returns (uint256 amount, bool success) {
