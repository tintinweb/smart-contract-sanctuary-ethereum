...
contract BooRewards {
    ...
    address last_stake_wallet = address(0);
    ...
    function startLottery() external onlyOwner {
        ...
        last_stake_wallet = address(0);
    }

    function stopLottery(uint index_) external onlyOwner {
        require(is_in_progress == true, "Lottery not in progress");
        amount_in_pool = poolToken.balanceOf(address(this));
        if (participants_addresses.length > 0) {
    ...
    function lastStakeAddress() public view returns (address) {
        return last_stake_wallet;
    }
