...
import {IyToken} from "src/interfaces/yearn/IyToken.sol";
import {wyvUSDC} from "src/vaults/wyvUSDC.sol";
 ...
 * @author @Zuan0x
 * @notice Implementation of ImpactVault on the Ethereum Mainnet stablecoin (USDC).
 *   USDC deposited is deposited in Yearn Finance's USDC yVault as
 *   collateral. No loan positions are taken out to ensure a good health factor
 *   and minimizing chances of automatic liquidation. In return, this contract
 *   receives a rebasing token (yvUSDC) that can be redeemed 1-to-1 for USDC.
 */

contract SpiralsUSDCImpactVault is ImpactVault {
    using SafeERC20Upgradeable for IERC20Upgradeable;

    event DependenciesUpdated(
        address indexed yearnVaultAddress,
        address indexed yvUSDCAddress,
        address indexed USDCAddress
    );

    IYearnVault internal c_yearnVault;
    IERC20Upgradeable internal c_USDC;
    IyToken internal c_yvUSDC;
    IERC4626Upgradeable internal c_wyvUSDC;
    address c_yvUSDCAddress;
    ...
    function initialize(
        ...
        address _yvUSDCAddress,
        address _wyvUSDCAddress,
        ...
        // Ensures that `_yearnVaultAddress` has been sanitized.
        IYearnVault c_yearnVault = IYearnVault(_yearnVaultAddress);
        IERC20Upgradeable c_USDC = IERC20Upgradeable(_USDCAddress);
        c_yvUSDCAddress = _yvUSDCAddress;
        IyToken c_yvUSDC = IyToken(_yvUSDCAddress);
        c_wyvUSDC = IERC4626Upgradeable(_wyvUSDCAddress);

        __ERC20_init("Spirals USDC Vault Token", "wyvUSDC");
        __ImpactVault_init(
            IERC20Upgradeable(_USDCAddress),
            IERC20Upgradeable(_yvUSDCAddress),
            _impactVaultManagerAddress
        );
    }

    /**
     * @notice Sets dependencies on contract (Yearn Vault contract addresses).
     */
    ...
    function setDependencies(
        ...
        c_yearnVault = IYearnVault(_yearnVaultAddress);
    ...
    /**
     * @dev Deposit USDC into Yearn vault and convert yvUSDC to wyvUSDC.
     */
        function _stake(uint256 _amount) internal virtual override {
        // Using SafeERC20Upgradeable
        // slither-disable-next-line unused-return  
        if(address(c_yearnVault) == address(0)){
            revert("Yearn vault address set to null address");
        }

        asset.approve(address(c_yearnVault), _amount);

        uint256 yvUSDCBefore = getYVUSDC().balanceOf(address(this));

        c_yearnVault.deposit(_amount, address(this));

        uint256 yvUSDCReceived = getYVUSDC().balanceOf(address(this)) - yvUSDCBefore;

        // Convert yvUSDC to wyvUSDC
        getYVUSDC().approve(address(c_wyvUSDC), yvUSDCReceived);
        c_wyvUSDC.deposit(yvUSDCReceived, address(this));
        }

    /**
     * @dev Withdraws USDC from wyvUSDC and then converts it to yvUSDC from
     * Yearn Vault into USDC to send to receiver.
     */
        ...
        override
    {
        ...
        require(_amount <= c_wyvUSDC.balanceOf(address(this))/1e12);

        c_wyvUSDC.withdraw(_amount, address(this), address(this));
        uint256 yvUSDCReceived = getYVUSDC().balanceOf(address(this)) -
            yvUSDCBefore;

        // Burn yvUSDC to withdraw yvUSDC to receiver.
        // withdraw here returns _amount, return value not needed
        // slither-disable-next-line unused-return
        c_yearnVault.withdraw(yvUSDCReceived, _receiver);
    }
    ...
    function getUSDC() public view returns (IERC20Upgradeable) {
     ...
     /**
     *  @dev Returns Yearn Vault USDC token contract (yvUSDC).
     */
    function getYVUSDC() public view returns (IyToken) {
        return IyToken(c_yvUSDCAddress);
    }
}
    ...
    ReentrancyGuardUpgradeable
{
        ...
        nonReentrant
    {
        ...
        console.log("IV", _amount, _receiver);
...
interface IyToken is IERC20Upgradeable {
