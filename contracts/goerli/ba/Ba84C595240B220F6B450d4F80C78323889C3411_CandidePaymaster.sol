...
contract CandidePaymaster is BasePaymaster {
    ...
    public view returns (bytes32) {
        return keccak256(abi.encode(
                ...
                paymasterData.maxTokenCost,
                paymasterData.costOfPost,
    ...
    public pure returns (PaymasterData memory) {
        (IERC20 token, uint160 maxTokenCost, uint160 costOfPost, uint48 validUntil, uint48 validAfter, bytes memory signature) = abi.decode(
            paymasterAndData[PAYMASTER_DATA_OFFSET:],
            (IERC20, uint160, uint160, uint48, uint48, bytes)
        );
        return PaymasterData(token, maxTokenCost, costOfPost, validUntil, validAfter, signature);
    }
    ...
    internal virtual override returns (bytes memory context, uint256 validationData){
        ...
        bytes memory _context = abi.encode(account, paymasterData.token, gasPriceUserOp, paymasterData.maxTokenCost, maxCost, paymasterData.costOfPost);

        return (_context, _packValidationData(false, paymasterData.validUntil, paymasterData.validAfter));
    }
    ...
    function _postOp(PostOpMode mode, bytes calldata context, uint256 actualGasCost) internal override {
        ...
        (address account, IERC20 token, uint256 gasPricePostOp, uint160 maxTokenCost, uint256 maxCost, uint160 costOfPost) 
            = abi.decode(context, (address, IERC20, uint256, uint160, uint256, uint160));
        // use same conversion rate as used for validation.
        // if costOfPost is zero the transaction is sponsored
        if(costOfPost > 0){
            uint256 actualTokenCost = (actualGasCost + costOfPost * gasPricePostOp) * maxTokenCost / maxCost;
