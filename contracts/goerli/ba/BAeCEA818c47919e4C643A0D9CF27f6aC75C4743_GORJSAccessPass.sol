    ...
    DefaultOperatorFiltererUpgradeable
{
    ...
    function transferFrom(
        ...
        require(address(_daoToken) != address(0), "Yielding Token is not set.");
        require(
            !EnumerableSetUpgradeable.contains(userStakeInfo[msg.sender], id),
    ...
    function _verify(address receiver, bytes calldata signature)
        internal
        view
        returns (address)
    {
        bytes32 digest = _hashTypedDataV4(
            keccak256(abi.encode(keccak256("NFT(address receiver)"), receiver))
        );
