...
contract meta_power is owned {
    ...
    uint public defaultROI = 578703703 ; // per second , equal to 0.5% daily 
                             
    uint public div = 10 ** 18; // for roi percent calculation
    ...
    function firstInvest(address _referrer, uint _amount) public returns(bool) {
        ...
        //if booster
        if(block.timestamp - temp.joinTime <= 2592000 && _amount >= temp.investedAmount && _amount > (100 * div) && temp.returnPercent < 14467592592 ) {
            temp.returnPercent = temp.returnPercent + 1157407407; // persecond increase = 0.1 % daily 
            if ( temp.returnPercent > 11574074074 )  temp.returnPercent = 14467592592; // persecond increase = 1.25 %
    ...
    function withdrawROI(address forUser) public returns(bool) {
        ...
        if(temp.totalPaid < temp.investedAmount * maxPayout / 10000 ) {
            uint totalSecond = block.timestamp - temp.lastWithdrawTime;
            if (totalSecond > 0) {
                uint roiAmount = totalSecond *  temp.investedAmount *  temp.returnPercent / div;
                ...
                emit withdrawEv(forUser, roiAmount, totalSecond,temp.returnPercent, block.timestamp );
    ...
    function viewMyROI(address forUser) public view returns(uint) {
        ...
        if(temp.totalPaid < temp.investedAmount *  maxPayout / 10000) {
            uint totalSecond = block.timestamp - temp.lastWithdrawTime;
            if (totalSecond > 0) {
                roiAmount = totalSecond *  temp.investedAmount *  temp.returnPercent / div;
                if ( temp.totalPaid + roiAmount > temp.investedAmount * maxPayout / 10000 ) roiAmount = (temp.investedAmount * maxPayout / 10000) - temp.totalPaid;              
            }
        }
        return roiAmount;
    ...
    function withdrawBonus(address forUser) public returns(bool) {
        ...
        for(uint i=0;i<10;i++) {
            ...
            if (bonus[forUser][i] && bp > 150 * rewardBonus[i]) {
                uint day = ( block.timestamp - lastBonusTime[forUser][i] ) / 300 ;
