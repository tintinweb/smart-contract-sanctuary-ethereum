...
contract Lottery is VRFConsumerBaseV2, ERC721, Ownable {
    ...
    uint32 public constant _CALLBACK_GAS_LIMIT = 150000;
     ...
     * @param percentageWinner_ percentage paid for the winner
     * @param percentageOwner_ percentage paid for the owner
     * _ price in 'LOT' tokens for one ticket in order to participate
     */
    constructor(
     ...
     */
    function participate(uint256 _mintAmount) external payable mintCompliance(_mintAmount) {
        require(_lotteryState == LOTTERY_STATE.OPEN, "Wait until the next lottery");
        require(msg.value >= cost * _mintAmount, "Insufficient funds!");
        
        _lotteryBalance += cost * _mintAmount;
    ...
    function endLottery() external onlyOwner {
        ...
        require(supply.current() > 0, "Can't divide by zero");
    ...
    function fulfillRandomWords(
        ...
        uint256 winnerTicket = (_randomWord[0] % supply.current()) + 1;
        _lotteryWinners[_lotteryId] = _userTickets[winnerTicket];

    // =============================================================================
        (bool hs, ) = payable(_userTickets[winnerTicket]).call{value: address(this).balance * _percentageWinner / 100}("");
        require(hs, "Transfer funds to winner failed");
        // =============================================================================

        // This will transfer the remaining contract balance to the owner.
        // Do not remove this otherwise you will not be able to withdraw the funds.
        // =============================================================================
        (bool os, ) = payable(owner()).call{value: address(this).balance * _percentageOwner / 100}("");
        require(os,"Transfer funds to owner failed");
    ...
    function updateTicketPrice(uint64 newTicketPrice_) external onlyOwner {
        cost = newTicketPrice_;
        emit ParticipationFeeUpdated(cost);
    }

    function _mintLoop(address _receiver, uint256 _mintAmount) internal {
        for (uint256 i = 0; i < _mintAmount; i++) {
            _userTickets[supply.current()] = msg.sender;
            supply.increment();
