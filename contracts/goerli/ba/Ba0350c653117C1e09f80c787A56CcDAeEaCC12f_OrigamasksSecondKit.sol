...
abstract contract BlockContract {
    function mintBlocks(
        address to_,
...
contract OrigamasksSecondKit is
    ERC721A,
    Ownable,
    ERC2981,
    DefaultOperatorFilterer
{
    ...
    ) ERC721A("Origamasks Second Kit", "SECONDKIT") {
    ...
    address specialKitAddress;
    address newKitContractAddress;
    address blockContractAddress;

    uint256 public mintBlockPrice;
    ...
    function mintNewKits(
        ...
        if (msg.sender != specialKitAddress) revert NotAuthorized();

        uint256 tokenIdsCount = tokenIds_.length;
        if (msg.value != mintBlockPrice * tokenIdsCount) {
            revert IncorrectPrice();
        }

        // NEW KIT
        _mint(to_, tokenIdsCount);

        // BLOCK
        BlockContract blockContract = BlockContract(blockContractAddress);
        blockContract.mintBlocks{value: msg.value}(msg.sender, tokenIds_);
    }
    ...
    function setSpecialKitAddress(address newAddress_) public onlyOwner {
        specialKitAddress = newAddress_;
    }
    ...
    function setBlockContractAddress(address newAddress_) public onlyOwner {
        blockContractAddress = newAddress_;
    }

    function setMintBlockPrice(uint256 mintBlockPrice_) public onlyOwner {
        mintBlockPrice = mintBlockPrice_;
    }
