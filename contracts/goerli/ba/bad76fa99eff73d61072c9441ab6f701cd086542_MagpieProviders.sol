  ...
  PausableUpgradeable
{
    ...
    onlyValidLpToken(_nftId, msg.sender)
  {
    ...
    IMagpieLPToken.MagpieLPTokenInfo memory data = IMagpieLPToken.MagpieLPTokenInfo({
      token: token,
      suppliedLiquidity: totalSuppliedLiquidity + _amount,
      shares: shares + mintedSharesAmount
    });
  ...
  function _burnSharesFromNft(
    ...
    magpieLPToken.updateTokenInfo(
      _nftId,
      IMagpieLPToken.MagpieLPTokenInfo({
        token: _tokenAddress,
        suppliedLiquidity: nftSuppliedLiquidity,
        shares: nftShares
      })
    );
  }
