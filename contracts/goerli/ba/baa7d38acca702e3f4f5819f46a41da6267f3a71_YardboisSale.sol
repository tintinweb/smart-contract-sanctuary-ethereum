...
contract YardboisSale is Ownable {
    ...
    function purchase(
        ...
        uint256 _totalSupply = GNOMES.totalSupply();
        if (_totalSupply + _amount > MAX_SUPPLY) revert SaleClosed();

        uint256 _mintedAmount = mintedAmounts[msg.sender];
        if (_amount == 0 || _mintedAmount + _amount > MAX_PER_WALLET)
            revert InvalidAmount();

        if (_amount * MINT_PRICE != msg.value) revert InvalidETHAmount();

        if (_range >= RANGE_LENGTH)
            revert InvalidRange();

        mintedAmounts[msg.sender] = _mintedAmount + _amount;

        _mintedAmount = _tryMintFromRange(msg.sender, _amount, _range);
