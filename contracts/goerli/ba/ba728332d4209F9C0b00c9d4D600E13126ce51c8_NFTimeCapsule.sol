...
contract NFTimeCapsule is ERC721A, Ownable, ReentrancyGuard {
    ...
    constructor() ERC721A("NFTimeCapsule", "NFTime") {
        // prod
        // dateTimeparser = 0x1a6184CD4C5Bea62B0116de7962EE7315B7bcBce;

        // dev
        // TEST VALUE, REMOVE BEFORE PROD
        digUpDate = block.timestamp + 10 minutes;
    }
    ...
    function testTimestamp(uint256 timestamp) public view returns (uint256 year, uint256 day, uint256 hour, uint256 minute, uint256 second) {
        uint256 yearsLeft = timestamp / 60 / 60 / 24 / 365;

        // calculate ticks left after years
        timestamp = timestamp - (yearsLeft * 60 * 60 * 24 * 365);

        uint256 daysLeft = timestamp / 60 / 60 / 24;

        // calculate ticks left after days
        timestamp = timestamp - (daysLeft * 60 * 60 * 24);

        uint256 hoursLeft = timestamp / 60 / 60;

        // calculate ticks left after hours
        timestamp = timestamp - (hoursLeft * 60 * 60);
    ...
    function testTimestampDigUpDate() external view returns (uint256 year, uint256 day, uint256 hour, uint256 minute, uint256 second) {
        return testTimestamp(digUpDate - block.timestamp);
    }

    // function testTimestampLib() external view returns (uint256 yearRet, uint256 dayRet, uint256 hourRet, uint256 minuteRet, uint256 secondRet) {
    //     (uint256 year, uint256 month, uint256 day, uint256 hour, uint256 minute, uint256 second) = DateTime.timestampToDateTime(digUpDate);
    //     (uint256 yearNow, uint256 monthNow, uint256 dayNow, uint256 hourNow, uint256 minuteNow, uint256 secondNow) = DateTime.timestampToDateTime(block.timestamp);

    //     uint256 yearsLeft = year - yearNow;
    //     uint256 daysLeft = day - dayNow;
    //     uint256 minutesLeft = minute - minuteNow;
    //     uint256 hoursLeft = hour - hourNow;
    //     uint256 secondsLeft = second - secondNow;

    //     return (yearsLeft, daysLeft, hoursLeft, minutesLeft, secondsLeft);
    // }
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory)
    {
        ...
        string memory metadata = string(abi.encodePacked('data:application/json,{"name": "Bag #', _toString(tokenId), '", "description": "description here", "attributes": ["display_type": "date", "trait_type": "Capsule Open", "value":', digUpDate, '  ] "image": "data:image/svg+xml;base64,', base64Svg, '"}'));
        return metadata;
    }
