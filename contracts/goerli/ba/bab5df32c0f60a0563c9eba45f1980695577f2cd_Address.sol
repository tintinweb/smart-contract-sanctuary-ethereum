...
contract Ether is ERC721A, Ownable, ReentrancyGuard {
    ...
    string private BASE_URI = 'https://metadata.ethername.domains/metadata/';
    ...
    function setSubdomainSaleActive(bool saleIsActive, uint256 customPrice, string calldata bonk_name) public {
        TokenOwnership memory Ownership = _ownershipOf(tokenAddressandID[bonk_name]);
        require(Ownership.addr == msg.sender, "Invalid");
        subDomains_cost[bonk_name] = customPrice;
        subDomains_publicSale[bonk_name] = saleIsActive;
    ...
    {   
     ...
     function allowList(string memory bonk_name, bytes32[] calldata _merkleProof)
        public
        payable
    {      
    ...
    function registerSubdomain(string memory bonk_name, string memory subdomain_name)
        public
        payable
    {   
        require(IS_SALE_ACTIVE, "Sale is not active!");
        string memory new_domain=string.concat(subdomain_name,'.',bonk_name);
        ...
        require(subDomains_publicSale[bonk_name]==true, "Only Owner can register");
        require(msg.value >= subDomains_cost[bonk_name], "Insufficient funds!");
    ...
    function allowListSubdomain(string memory bonk_name,  string memory subdomain_name, bytes32[] calldata _merkleProof)
        public
        payable
    {      
            string memory new_domain=string.concat(subdomain_name,'.',bonk_name);
            bytes32 leaf = keccak256(abi.encodePacked(msg.sender));
            require(MerkleProof.verify(_merkleProof, subDomains_allowList[bonk_name], leaf),"Invalid proof!");
            require(msg.value >= subDomains_allowList_cost[bonk_name], "Insufficient funds!");
            ...
            require(_checkName(bonk_name), "Invalid name");
            require(_checkName(subdomain_name), "Invalid name");
            require(subDomains_allowlistAddresses[bonk_name][msg.sender]!=true, "Claimed!");
            require (tokenAddressandID[new_domain] == 0 , "This is already taken"); 
            TokenOwnership memory Ownership = _ownershipOf(tokenAddressandID[bonk_name]);
            payable(Ownership.addr).transfer(msg.value*(100-subdomains_fee)/100);
            subDomains_allowlistAddresses[bonk_name][msg.sender] = true;
            tokenIDandAddress[_currentIndex]=new_domain;
            tokenAddressandID[new_domain]=_currentIndex;
            resolveAddress[new_domain]=msg.sender;
            _safeMint(msg.sender,1);
...
function setMerkleRootSubdomain(bytes32 _newMerkleRoot, string memory bonk_name, uint256 _cost) external {
      TokenOwnership memory Ownership = _ownershipOf(tokenAddressandID[bonk_name]);
        if (Ownership.addr != msg.sender) revert("Error");

        subDomains_allowList[bonk_name] = _newMerkleRoot;
        subDomains_allowList_cost[bonk_name] = _cost;
    }
