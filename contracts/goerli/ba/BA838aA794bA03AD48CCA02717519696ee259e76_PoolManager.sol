...
contract PoolManager {
  ...
  uint256 public constant ssvTarget = 30;
  address public constant ssvToken = 0x3a9f01091C446bdE031E39ea8354647AFef091E7;
  address public constant ssvNetwork = 0xb9e155e65B5c4D66df28Da8E9a0957f06F11Bc04;
  ...
  event LaunchPool(uint256 id, address sender, PoolStatus status);
  ...
  function readyToLaunch(uint256 id) public view returns (bool) {
  ...
  function launchPool(
    ...
    require(readyToLaunch(id), 'Pool is not ready to launch');
    IERC20(ssvToken).approve(ssvNetwork, maxInt);
    ISSVNetwork(ssvNetwork).registerValidator(publicKey, operatorIds, sharesPublicKeys, sharesEncrypted, amount);
    pools[id].status = PoolStatus.launched;
    emit LaunchPool(id, msg.sender, PoolStatus.launched);
  }
  ...
  function addEthPool(uint256 poolId) public payable {
    ...
    IERC20Mint(poolToken).mint(msg.sender, msg.value);
