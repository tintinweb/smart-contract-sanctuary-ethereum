...
contract CandaoCoordinator is Ownable, Pausable {
    ...
    struct ProrityPass {
        ...
        // @dev: package value index starting from 0
        uint256 packageValueIndex;

        // @dev: domain lenght in character count
        uint256 domainLenght;
    ...
    // @dev: EVENTS
    event PriorityPassBought(address indexed buyer, string indexed reservationToken, uint256 packageIndex, uint256 packageValueIndex, uint256 price, uint256 domainLenght);
    event DomainBought(address indexed buyer, string indexed reservationToken, uint256 price, uint256 domainLenght);
    event CDOBought(address indexed buyer, uint256 amount);
    ...
    function buy(uint256 packagePrice, string memory domain, string memory reservationToken) external whenNotPaused {
        PriorityPassPackageValue[] memory selectedPackage = _packages[packagePrice];

        uint256 characterCount = bytes(domain).length;
        require(characterCount != 0, "CandaoCoordinator: Incorrect domainLenght.");

        uint256 packageValueIndex = characterCount - 1;
        require(selectedPackage[packageValueIndex].valid, "CandaoCoordinator: package value not found.");

        PriorityPassPackageValue memory domainConfiguration = selectedPackage[packageValueIndex];

        bool success = token.transfer(wallet, domainConfiguration.totalPrice);
        require(success, "CandaoCoordinator: Can't transfer tokens from user.");
        _userInfo[msg.sender] = ProrityPass(packagePrice, packageValueIndex, characterCount, block.number, domainConfiguration.totalPrice, true);
        emit PriorityPassBought(msg.sender, reservationToken, packagePrice, packageValueIndex, domainConfiguration.totalPrice, characterCount);
    }

    function buyCDO(uint256 amount) external {
        bool success = token.transfer(wallet, amount);
        require(success, "CandaoCoordinator: Can't transfer tokens from user.");
        emit CDOBought(msg.sender, amount);
    }

    function buyAddinationalDomain(string memory domain, string memory reservationToken) external {
        ...
        bool success = token.transfer(wallet, domainPrice);
        require(success, "CandaoCoordinator: Can't transfer tokens from user.");
        emit DomainBought(msg.sender, reservationToken, domainPrice, characterCount);
    }

    function addDomainPrice(uint256 domainLenght, uint256 price) external onlyOwner {
        _domainPricing[domainLenght] = price;
    }

    function addPackageOption(uint256 packagePrice, uint256 domainLenght, uint256 domainPrice, uint256 totalPrice) external onlyOwner {
        if (_packages[packagePrice].length == 0) {
            availablePackages.push(packagePrice);
        }
        _packages[packagePrice].push(PriorityPassPackageValue(domainLenght, domainPrice, totalPrice, true));
    }
    ...
    function removePackage(uint256 packagePrice) external onlyOwner {
        ...
        for (uint i = 0; i <= availablePackages.length; i++) {
            if (availablePackages[i] == packagePrice) {
                delete availablePackages[i];
            }
        }
    }

    function removePackageDomainLenght(uint256 packagePrice, uint256 packageValueIndex) external onlyOwner {
