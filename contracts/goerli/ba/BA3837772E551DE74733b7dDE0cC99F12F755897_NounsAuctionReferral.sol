...
contract NounsAuctionReferral {
     ...
     * @dev This function can only be called once.
     */
    function initialize(address socialNounsToken_,uint8 share_,address weth_,uint256 reservePrice_,uint8 minBidIncrementPercentage_) public virtual{
    ...
    function createBid(uint256 nounId_,uint256 referralNounsID_) external payable {
        ...
        require(address(this).balance <= ((LastBid.amount * share) / 100), 'Contract balance to low');
        if(nounId_ > LastBid.nounId && LastBid.nounId != 0){
            if(socialNounsTokenInterface.balanceOf(address(this)) > 0){
                ...
                address payable referral = payable(socialNounsTokenInterface.ownerOf(LastBid.referralnounId));
        ...
        try auction.createBid{value: msg.value}(nounId_) {
            ...
            LastBid.referralnounId = referralNounsID_;
