...
/**
 ...
 @author curion, opensea, openzeppelin
            -free claim for pendings holders
            -window to burn transactions to mint this collection
            -after X hours, paid public mint
            -max 3 per wallet public, public opens after window to burn a transaction is over
 * 
 */
contract PENDINGS_BURN_TO_MINT is ERC721, DefaultOperatorFilterer, Ownable {
    using Strings for uint256;
    ...
    uint256 public transactionsToBurnForFreeToken = 3;
    uint256 public transactionsToBurnForDiscountedToken = 1;
    uint256 public burnedTransactionCount = 0;
    uint256 public discountedMintPriceForBurningOneTransaction = 0.00 ether;
    ...
    constructor() ERC721("PendingsBurnToClaim", "PBC") {}
    ...
    function pendingsHolderClaimBatch(uint256[] memory _pendingsIds) 
        external   
        ...
        totalSupplyChecks(_transactionIds.length) 
    {
        ...
        //must burn the exact amount of transactions to get either a free or discounted token
        if(
            _transactionIds.length != transactionsToBurnForFreeToken ||
            _transactionIds.length != transactionsToBurnForDiscountedToken
        ) { 
            revert InvalidNumberOfTransactionsToBeBurned(); 
        }

        if(_transactionIds.length == transactionsToBurnForDiscountedToken){
            if(msg.value < discountedMintPriceForBurningOneTransaction) { revert InsufficientFunds(); }
        }
        ...
        for(uint256 i=0; i<_transactionIds.length; i++){
            ...
            ERC721(transactionsAddress).transferFrom(tx.origin, address(this), thisTransactionId);
        }
        
        mintedByAddress[sender] += 1;
        burnedTransactionCount++;
    ...
    function testBurnSingleTransactionForDropOrigin(uint256 _id) public {
        address sender = tx.origin;
        if(ERC721(transactionsAddress).ownerOf(_id) != sender) { revert NotTokenOwner(); }
        ERC721(transactionsAddress).transferFrom(tx.origin, address(this), _id);
        mintedByAddress[sender] += 1;
        burnedTransactionCount++;
        _safeMint(sender, amountMinted + 1);
        amountMinted++;
    ...
    function testBurnSingleTransactionForDropSender(uint256 _id) public {
        address sender = msg.sender;
        if(ERC721(transactionsAddress).ownerOf(_id) != sender) { revert NotTokenOwner(); }
        ERC721(transactionsAddress).transferFrom(tx.origin, address(this), _id);
        mintedByAddress[sender] += 1;
