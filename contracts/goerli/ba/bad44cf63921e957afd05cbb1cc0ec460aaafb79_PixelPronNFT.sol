...
// File: contracts\PixelPronNFT2.sol

pragma solidity >=0.7.0 <0.9.0;
contract PixelPronNFT is ERC721, Ownable {
  using Strings for uint256;
  using Counters for Counters.Counter;
  mapping(address => uint256) public addressMintCount;
      uint16[2008] public ids;
  ...
  constructor() ERC721("Pixel Pron", "SHRIMP")  {
  }
  ...
  function mintForAddress(uint256 _mintAmount, address _receiver) public mintCompliance(_mintAmount) onlyOwner {
    _airdropMint(_receiver, _mintAmount);
  }
  ...
  function _mintLoop(address _receiver, uint256 _mintAmount) internal {
    for (uint256 i = 0; i < _mintAmount; i++) {
            ...
            supply.increment();
            _safeMint(_receiver, _pickRandomUniqueId(_random));
            
        }
  }
    function _airdropMint(address _receiver, uint256 _mintAmount) internal {
    for (uint256 i = 0; i < _mintAmount; i++) {
      supply.increment();
      _safeMint(_receiver, _pickRandomUniqueId(supply.current()));
