...
contract MintableController is SystemRole, StandardController {
   ...
   * @param s Signature.
   */
  function burnFrom_withCaller(address caller, address from, uint amount, bytes32 h, bytes memory s)
    public
    guarded(caller)
    onlySystemAccount(caller)
    returns (bool)
  {
    return token.burn(from, amount, h, s);
  }
