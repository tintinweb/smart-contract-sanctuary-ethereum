...
contract AiRein is ERC721A, Ownable, ERC2981, ReentrancyGuard {
    ...
    NFTState public nftState = NFTState.BLIND_BOX;
    ...
    constructor(
        ...
        // Receiver can change to another by setRoyaltyInfo
        _setDefaultRoyalty(payeeAddress, defaultFeeNumerator);
    }
    ...
    function freeMint(uint256 qty, address recipient) external {
        if (_msgSender() != officialAddress) revert InvalidOfficialAddress();
        if (_totalMinted() + qty > supply) revert SoldOut();
    ...
    function setNFTState(string memory baseURI, NFTState state) external onlyAdmin {
    ...
    function setRoyaltyInfo(address receiver, uint96 feeBasisPoints) external onlyAdmin {
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        ...
        if (bytes(baseURI).length == 0) {
            return "";
        } else if (nftState == NFTState.BLIND_BOX) {
            return baseTokenURI;
        } else {
