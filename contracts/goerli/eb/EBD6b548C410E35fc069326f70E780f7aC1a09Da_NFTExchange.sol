...
contract ExchangeCore  is ReentrancyGuarded, Ownable{
   ...
   // NOTE: these hashes are derived and verified in the constructor.
    ...
    mapping( uint8 => bool ) public debug_num;
    bool public DebugTrade = true;
    ...
    constructor () {
       ...
       // require(keccak256("Order(bytes calldata)") == _ORDER_TYPEHASH);
      // require(keccak256("Order(address exchange,address maker,address taker,uint256 makerRelayerFee,uint256 takerRelayerFee,uint256 makerProtocolFee,uint256 takerProtocolFee,address feeRecipient,uint8 feeMethod,uint8 side,uint8 saleKind,address target,uint8 howToCall,bytes calldata,bytes replacementPattern,address staticTarget,bytes staticExtradata,address paymentToken)") == _ORDER_TYPEHASH);
      // require(keccak256("Order(address exchange,address maker,address taker,uint256 makerRelayerFee,uint256 takerRelayerFee,uint256 makerProtocolFee,uint256 takerProtocolFee,address feeRecipient,uint8 feeMethod,uint8 side,uint8 saleKind,address target,uint8 howToCall,bytes calldata,bytes replacementPattern,address staticTarget,bytes staticExtradata,address paymentToken,uint256 basePrice,uint256 extra,uint256 listingTime,uint256 expirationTime,uint256 salt)") == _ORDER_TYPEHASH);
       
       // require(DOMAIN_SEPARATOR == _deriveDomainSeparator());
    }

/*
     function _order_hash1() public view returns (bytes32) {
         return keccak256("Order(bytes calldata)");
     }
    
     function _order_hash12() public view returns (bytes32) {
         return keccak256("Order(address exchange,address maker,address taker,uint256 makerRelayerFee,uint256 takerRelayerFee,uint256 makerProtocolFee,uint256 takerProtocolFee,address feeRecipient,uint8 feeMethod,uint8 side,uint8 saleKind,address target)");
     }

     function _order_hash18() public view returns (bytes32) {
         return keccak256("Order(address exchange,address maker,address taker,uint256 makerRelayerFee,uint256 takerRelayerFee,uint256 makerProtocolFee,uint256 takerProtocolFee,address feeRecipient,uint8 feeMethod,uint8 side,uint8 saleKind,address target,uint8 howToCall,bytes calldata,bytes replacementPattern,address staticTarget,bytes staticExtradata,address paymentToken)");
     }

     function _order_hash23() public view returns (bytes32) {
...
*/
    ...
    */
    ...
    {
        if( DebugTrade )
            return _tradeMatch_debug( buy, buySig, sell, sellSig );
        else 
            return _tradeMatch( buy, buySig, sell, sellSig );    
    }

    function set_debug_num( uint8 num, bool state ) external
    {
        debug_num[num] = state;
    }

    function set_Debug_Trade( bool state ) external
    {
        DebugTrade = state;
    }

    function _tradeMatch_debug(Order memory buy, bytes memory buySig, Order memory sell, bytes memory sellSig)
        internal
        reentrancyGuard
    {
        /* CHECKS */
...
contract OwnableDelegateProxy is OwnedUpgradeabilityProxy {
    ...
    constructor(address owner, address initialImplementation, bytes memory call_data)
    {
        ...
        //require(initialImplementation.delegatecall(call_data));
