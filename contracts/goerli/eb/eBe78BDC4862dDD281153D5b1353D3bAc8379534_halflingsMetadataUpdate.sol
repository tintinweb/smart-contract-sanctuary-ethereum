...
contract halflingsMetadataUpdate is Ownable {
      ...
      address public halflingsItems=0x3E8266c67f4D8a9Dc3fCa4377dfb0C378CB8D691;
      ...
      event _addItems(address user,uint256 nftid,uint256 itemId,uint256 seed,bytes signature);
      event _removeItem(address user,uint256 nftid,uint256 itemId,uint256 seed,bytes signature);
    ...
    function addItems(
        ...
        // bytes32 hash = keccak256(abi.encodePacked(_msgSender(),nftid,itemId,seed));
        ...
        require((ERC1155(halflingsItems).balanceOf(_msgSender(),itemId) >0),"caller is not owner of NFT item");
        ERC1155(halflingsItems).burn(_msgSender(),itemId,1);
        
        emit _addItems(_msgSender(),nftid,itemId,seed,signature);
    }
    ...
    function removeItem(
        ...
        emit _removeItem(_msgSender(),nftid,itemId,seed,signature);
    }
