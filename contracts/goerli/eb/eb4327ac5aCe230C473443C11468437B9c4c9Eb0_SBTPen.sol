...
import "./Interface/ISBTPen.sol";
import "@openzeppelin/contracts/utils/Strings.sol";



contract SBTPen is Initializable,UUPSUpgradeable, ERC1155Upgradeable, PausableUpgradeable, AccessControlUpgradeable, ReentrancyGuardUpgradeable, ISBTPen {
    ...
    function initialize(address usdc_) public initializer {
        ...
        uri_[LOW] = "https://bafybeibaew3m4erd7zb3ldlu6pag7kqqltvzcnaroiyqt3w6jrvvol2f5a.ipfs.w3s.link/common.png";
        uri_[MIDDLE] = "https://bafybeifzjaarwowvwlgc5bur74fiha6wkj65mn4ircdf6dksqbseiozwz4.ipfs.w3s.link/rare.png";
        uri_[HIGH] = "https://bafybeielz3hvmjfd6dh7m5fkwi2uy7za4dnthnwqvdgkzv4tovzhn6i3qa.ipfs.w3s.link/legendary.png";

        // Set the price for each token type
        levelPrice[LOW] = 100 * 10**decimals;
        levelPrice[MIDDLE] = 200 * 10**decimals;
    ...
    /// @notice 通过USDC铸造SBT
    /// @param id_ 1: 低级 2: 中级 3: 高级
    function mint(uint256 id_) external override callerIsUser nonReentrant whenNotPaused {
        require(!minters[msg.sender], "The SBT has been minted");
        require(id_ == LOW || id_ == MIDDLE || id_ == HIGH, "not support level");
        require(IERC20(usdc).balanceOf(msg.sender) >= levelPrice[id_], "not enough usdc");

        IERC20(usdc).transferFrom(msg.sender, address(this), levelPrice[id_]);
    ...
    /// @notice 升级SBT
    /// @param next_ 1: 低级 2: 中级 3: 高级
    function upgrade(uint256 next_) external override callerIsUser nonReentrant whenNotPaused {
        require(minters[msg.sender], "The SBT has not been minted");
        require(next_ == MIDDLE || next_ == HIGH, "not support level");
        uint256 id_ = getId(msg.sender);
        require(id_ != 0 && id_ < next_, "not support upgrade");
        uint256 price = levelPrice[next_] - levelPrice[id_];
        require(IERC20(usdc).balanceOf(msg.sender) >= price, "not enough usdc");
    ...
    /// @notice 获取用户的SBT等级
    /// @param account_ 用户地址
    /// @return 用户的SBT等级 1: 低级 2: 中级 3: 高级 0: 未铸造
    ...
    /// @notice 批量获取用户的SBT数量
    /// @param accounts 用户地址
    /// @param ids 1: 低级 2: 中级 3: 高级 0: 未铸造
    /// @return 用户的SBT数量
    ...
    function batchMint(address[] memory accounts, uint256 id_) external override onlyRole(MARKET_ROLE) nonReentrant {
        ...
        for (uint256 i = 0; i < accounts.length; i++) {
            require(!minters[accounts[i]], "The SBT has been minted");
    ...
    /**
     * @dev 实现 {IERC165-supportsInterface} 接口.
     */
