...
contract Bridge is AccessControl, IBridge {
    ...
    event MessageReceived(
        ...
        address to,
        bytes signature,
        IAssetRouter.CreditObj c,
        IAssetRouter.SwapObj s
    );
    ...
    //     nonBlockingReceive(srcChainId_, srcAddressBytes_, nonce_, payload_);
    // }
    ...
    function handleReceive(uint16 _srcChainId, bytes memory _srcAddress, uint64, bytes memory _payload) internal {
        // TODO remove this for testing
        require(msg.sender == address(layerZeroEndpoint), "!l0");
        require(
        ...
        if (functionType == TYPE_SWAP_REMOTE) {
            (
                ...
                uint16 srcPoolId,
                uint16 dstPoolId,
                IAssetRouter.CreditObj memory c,
                IAssetRouter.SwapObj memory s,
                bytes memory to,
                bytes memory signature
            ) = abi.decode(
                    _payload,
                    (uint8, uint16, uint16, IAssetRouter.CreditObj, IAssetRouter.SwapObj, bytes, bytes)
                );
            address toAddress;
            assembly {
                toAddress := mload(add(to, 20))
            }
            router.receiveVouchers(dstPoolId, _srcChainId, srcPoolId, c);
            router.swapRemote(dstPoolId, _srcChainId, srcPoolId, toAddress, s);
            emit MessageReceived(_srcChainId, srcPoolId, dstPoolId, toAddress, signature, c, s);
        } else if (functionType == TYPE_ADD_LIQUIDITY) {
            (, uint16 srcPoolId, uint16 dstPoolId, IAssetRouter.CreditObj memory c) = abi.decode(
                _payload,
                (uint8, uint16, uint16, IAssetRouter.CreditObj)
            );
            router.receiveVouchers(dstPoolId, _srcChainId, srcPoolId, c);
        }
    }
    ...
    function swap(
        IAssetRouter.SwapParams memory _params,
        ...
        _call(_params.dstChainId, TYPE_SWAP_REMOTE, _params.refundAddress, payload, _params.gasSRCReceiver);
    }
    ...
    function sendVouchers(
        ...
        IAssetRouter.CreditObj memory _c,
        uint256 _gasSRCReceiver
    ) external payable onlyRole(ROUTER_ROLE) {
        bytes memory payload = abi.encode(TYPE_ADD_LIQUIDITY, _srcPoolId, _dstPoolId, _c);
        _call(_chainId, TYPE_ADD_LIQUIDITY, _refundAddress, payload, _gasSRCReceiver);
    }
    ...
    function quoteLayerZeroFee(
        ...
        uint256 gasForSRCReceiver
    ) external view returns (uint256, uint256) {
        bytes memory payload = "";
        IAssetRouter.CreditObj memory c = IAssetRouter.CreditObj(1, 1);
        if (_functionType == TYPE_SWAP_REMOTE) {
            IAssetRouter.SwapObj memory s = IAssetRouter.SwapObj(1, 1, 1);
        ...
        uint16 version = 1;
        bytes memory adapterParams = abi.encodePacked(version, gasForSRCReceiver);
        return layerZeroEndpoint.estimateFees(_chainId, address(this), payload, useLayerZeroToken, adapterParams);
    }
    ...
    function setGasAmount(
        ...
        require(_functionType >= 1 && _functionType <= 4, "invalid target");
    ...
    function _call(
        ...
        uint16 version = 1;
        bytes memory adapterParams = abi.encodePacked(_type, version, gasForSRCReceiver);
