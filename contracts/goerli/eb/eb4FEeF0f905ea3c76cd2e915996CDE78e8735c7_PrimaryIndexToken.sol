...
contract PrimaryIndexToken is Initializable, AccessControlUpgradeable, ReentrancyGuardUpgradeable
{
    ...
    function withdrawFromRelatedContracts(address projectToken, uint256 projectTokenAmount, address user, address beneficiar) public isProjectTokenListed(projectToken) nonReentrant returns(uint256) {
        return withdrawInternal(projectToken, projectTokenAmount, user, beneficiar);
    }

    function withdrawInternal(address projectToken, uint256 projectTokenAmount, address user, address beneficiar) internal returns(uint256){
    ...
    function supplyInternal(address lendingToken, uint256 lendingTokenAmount, address user) internal isLendingTokenListed(lendingToken) {
