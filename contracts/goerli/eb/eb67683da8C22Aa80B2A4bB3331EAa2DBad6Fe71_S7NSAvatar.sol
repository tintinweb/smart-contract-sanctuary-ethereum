...
contract S7NSAvatar is ERC721Enumerable {
	...
	modifier isHalted() {
		if (management.halted()) revert Maintenance();
		_;
	}
	
	modifier onlyMinter() {
		if (!management.hasRole(MINTER_ROLE, _msgSender())) revert OnlyMinter();
        _;
    }

	modifier onlyManager() {
		if (!management.hasRole(MANAGER_ROLE, _msgSender())) revert OnlyManager();
        _;
    }
    ...
    */
	function setManagement(address _management) external onlyManager {
		if (_management == address(0)) revert AddressZero();
    ...
    */
	function burn(uint256[] calldata _ids) external {
		...
		for (uint256 i; i < _amounts; i++) {
			_tokenId = _ids[i];
			if (!isAuthorized && ownerOf(_tokenId) != _operator)
				revert NotAuthorizedNorOwner();
