...
import { IERC165 } from "openzeppelin/utils/introspection/IERC165.sol";
import { IRarementFeeRegistry } from "@core/interfaces/IRarementFeeRegistry.sol";
import { IRangeVolumeMinter, VolumeMintData, MintInfo } from "./interfaces/IRangeVolumeMinter.sol";
import { BaseMinter } from "./BaseMinter.sol";
import { IMinterModule } from "@core/interfaces/IMinterModule.sol";
import { IRarementVolumeV1 } from "@core/interfaces/IRarementVolumeV1.sol";

/*
 * @title RangeVolumeMinter
 * @notice Module for range volume mints of Rarement volumes.
 * @author rmnt.xyz
 */
contract RangeVolumeMinter is IRangeVolumeMinter, BaseMinter {
    // =============================================================
    //                            STORAGE
    // =============================================================
    ...
    function mint(
        ...
        _mint(volume, mintId, quantity, affiliate);
    }
    ...
    function setTimeRange(
        ...
        uint32 cutoffTime,
        uint32 endTime
    ) public onlyVolumeOwnerOrAdmin(volume) onlyValidCombinedTimeRange(startTime, cutoffTime, endTime) {
        ...
        RangeVolumeMinter.setTimeRange(volume, mintId, startTime, endTime);

        emit CutoffTimeSet(volume, mintId, cutoffTime);
    }
    ...
    function setTimeRange(
        ...
        uint32 startTime,
        uint32 endTime
    ) public override(BaseMinter, IMinterModule) onlyVolumeOwnerOrAdmin(volume) {
        VolumeMintData storage data = _volumeMintData[volume][mintId];

        if (!(startTime < data.cutoffTime && data.cutoffTime < endTime)) revert InvalidTimeRange();

        BaseMinter.setTimeRange(volume, mintId, startTime, endTime);
    }
    ...
    function setMaxMintableRange(
        ...
        emit MaxMintableRangeSet(volume, mintId, maxMintableLower, maxMintableUpper);
    }
    ...
    function supportsInterface(bytes4 interfaceId) public view override(IERC165, BaseMinter) returns (bool) {
        return BaseMinter.supportsInterface(interfaceId) || interfaceId == type(IRangeVolumeMinter).interfaceId;
    }
    ...
    function moduleInterfaceId() public pure returns (bytes4) {
        return type(IRangeVolumeMinter).interfaceId;
    }
     ...
     * @param data The volume mint data.
     * @return The computed value.
     */
    function _getMaxMintable(VolumeMintData storage data) internal view returns (uint32) {
        uint32 _maxMintable;
...
/**
 * @dev Data unique to a range volume mint.
 */
...
/**
 * @dev All the information about a range volume mint (combines VolumeMintData with BaseData).
 */
struct MintInfo {
    uint32 startTime;
    uint32 endTime;
...
/**
 * @title IRangeVolumeMinter
 * @dev Interface for the `RangeVolumeMinter` module.
 * @author rmnt.xyz
 */
interface IRangeVolumeMinter is IMinterModule {
    // =============================================================
    //                            EVENTS
    // =============================================================

    /**
     * @dev Emitted when a range volume is created.
    ...
    event CutoffTimeSet(address indexed volume, uint128 indexed mintId, uint32 cutoffTime);
    ...
    /*
     * @dev Mints tokens for a givenvolume.
     * @param volume  Address of the song volume contract we are minting for.
     * @param quantity Token quantity to mint in song `volume`.
     */
    function mint(
        address volume,
        uint128 mintId,
    ...
    /**
     * @dev Returns {IRangeVolumeMinter.MintInfo} instance containing the full minter parameter set.
