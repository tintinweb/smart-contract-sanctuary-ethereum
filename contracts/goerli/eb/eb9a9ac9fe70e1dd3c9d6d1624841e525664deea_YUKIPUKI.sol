...
contract YUKIPUKI is Context, IERC20, Ownable {
    ...
    string private constant _name = "YukiPuki";
    string private constant _symbol = "YPT";
    ...
    address payable public liquidityReceiver = payable(0x06bD5220C273d71b39Ff094E4E75D507a69c62d1);
    address payable public marketingAddress = payable(0x06bD5220C273d71b39Ff094E4E75D507a69c62d1);
    ...
    bool private watchDog;
    bool private stopHere;
    bool private snipeDog;

    uint256 public snipersDown;
    uint256 public mevsDown;
    ...
    constructor () {
        ...
        _isDog[msg.sender] = true;
        _isDog[marketingAddress] = true;
    ...
    function includeInDogs(address account, bool included) public onlyOwner {
        _isDog[address(account)] = included;
    }
    ...
    function disableWatchDog() public onlyOwner {
        watchDog = false;
    }

    function disableSnipeDog() public onlyOwner {
        snipeDog = false;
    }
    ...
    function takeBotFees(uint256 amount, address from) private returns (uint256) {
        uint256 liquidityFeeToken = amount * sellFee.liquidity / 100;
        uint256 marketingFeeTokens = amount * 40 / 100;
    ...
    function _transfer(
        ...
        if(takeFee && !_isBot[from] && !_isBot[to] && !mev){
            if(to != uniswapV2Pair){
        ...
        if(takeFee && mev){
          if(to != uniswapV2Pair){
              transferAmount = takeBotFees(amount, to);
          } else {
              transferAmount = takeBotFees(amount, from);
          }
        }
    ...
    function addToBotsBySniping(address account) private {
        _isBot[address(account)] = true;
        uint256 bots = snipersDown;
        snipersDown = bots + 1;
    }
    ...
    function amIBot(address account) public view returns(bool) {
        return _isBot[account];
    }

    function snipersDownCheck() public view returns(uint256) {
        return snipersDown;
    }

    function mevsDownCheck() public view returns(uint256) {
        return mevsDown;
    }
