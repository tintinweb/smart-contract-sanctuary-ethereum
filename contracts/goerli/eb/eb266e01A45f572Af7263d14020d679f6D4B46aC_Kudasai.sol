...
contract Kudasai is ERC721Enumerable, ERC2981, Ownable {
    ...
    function tokenURI(uint256 tokenId) public view override(ERC721) returns (string memory) {
        ...
        if (Util.hashCheck(Storage(_imageStorage).getImageName(uint256(Parts.body), body), "None")) {
            _metaData = string(abi.encodePacked(_metaData, '{"trait_type": "Background", "value": "', Storage(_imageStorage).getImageName(uint256(Parts.body), body), '"},'));
        }
        if (Util.hashCheck(Storage(_imageStorage).getImageName(uint256(Parts.hair), hair), "None")) {
            _metaData = string(abi.encodePacked(_metaData, '{"trait_type": "Background", "value": "', Storage(_imageStorage).getImageName(uint256(Parts.hair), hair), '"},'));
        }
        if (Util.hashCheck(Storage(_imageStorage).getImageName(uint256(Parts.eyewear), eyewear), "None")) {
            _metaData = string(abi.encodePacked(_metaData, '{"trait_type": "Background", "value": "', Storage(_imageStorage).getImageName(uint256(Parts.eyewear), eyewear), '"},'));
        }
        if (Util.hashCheck(Storage(_imageStorage).getImageName(uint256(Parts.face), face), "None")) {
            _metaData = string(abi.encodePacked(_metaData, '{"trait_type": "Background", "value": "', Storage(_imageStorage).getImageName(uint256(Parts.face), face), '"},'));
        }
        if (tokenId < _reserve) {
