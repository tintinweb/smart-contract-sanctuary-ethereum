...
contract HumblGalleryERC721 is Initializable, ERC721Upgradeable, ERC721EnumerableUpgradeable, ERC721URIStorageUpgradeable,OwnableUpgradeable, ERC721BurnableUpgradeable, PausableUpgradeable,CommissionUpgradeable, ReentrancyGuardUpgradeable {
...
/* Execute once on contract deployment with commission fee, commission fee receiver, royaltyFraction and royalty receiver */
    function initialize(uint96 commissionPercentage, address commissionReceiver) initializer public {
        __ERC721_init("HUMBL Marketplace", "HUMBLNFT");
    ...
    function mint(uint256[3] memory intData, string calldata uri, uint8 ERC20TokenIndex,address seller,address buyer, address[] memory royaltyReceiver,uint96[] memory royaltyPercentage) public whenNotPaused returns(uint256) {
        require(royaltyPercentage.length == royaltyReceiver.length, "H105");
        return _mint(Payload(intData[0],intData[1],intData[2],uri,ERC20TokenIndex,seller, buyer, royaltyReceiver, royaltyPercentage));
    }
    
    function lazyMint(uint256[3] memory intData, string calldata uri, uint8 ERC20TokenIndex,address seller,address buyer,address[] memory royaltyReceiver,uint96[] memory royaltyPercentage,Signature memory sig) public payable whenNotPaused returns(uint256 tokenId)  {
        Payload memory payload = Payload(intData[0],intData[1],intData[2],uri,ERC20TokenIndex,seller,buyer, royaltyReceiver,royaltyPercentage);
    ...
    function transferWithMint(uint256[3] memory intData, string calldata uri, uint8 ERC20TokenIndex,address seller,address buyer, address[] memory royaltyReceiver,uint96[] memory royaltyPercentage) public whenNotPaused returns(uint256 tokenId)  {
        Payload memory payload = Payload(intData[0],intData[1],intData[2],uri,ERC20TokenIndex,seller,buyer,royaltyReceiver,royaltyPercentage);
        payload.tokenId = _mint(payload);
    ...
    function safeBuyNFT(Payload memory payload, Signature memory sig, uint256 tokenId) public payable whenNotPaused nonReentrant {
        ...
        require(nft.salt < payload.salt, "H161");
    ...
    function safeTransferNFT(Payload memory sellerPayload,Payload memory buyerPayload, Signature memory sellerSig, Signature memory buyerSig, uint256 tokenId) public payable {
