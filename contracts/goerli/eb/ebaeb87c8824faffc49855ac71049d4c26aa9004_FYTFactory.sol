...
contract FYTFactory {
    mapping(address => mapping(uint256 => mapping(uint256 => FYT)))
        private futureToFytWithSchedule; // mapping future address to fyt having particular start/end date
    mapping(address => mapping(address => FYT[][])) private userFYTPerBatch; // mapping future address to user to an array of fyts per batch
    mapping(address => mapping(address => uint256[]))
        private userFYTAmountPerBatch; // mapping future address to user to amounts per batch

    /* Events */
    event FYTDeployed(address indexed fyt, uint256 startDate, uint256 endDate);
    event YTTokenized(
        address indexed user,
        address indexed future,
        uint256 amount
    );
    event YTWithdrawn(
        address indexed user,
     ...
     * @param future Future vault of the YT being tokenized
    ...
    function tokenizeYT(
    ...
    ) external returns (FYT[] memory fyts) {
        uint256 scheduleParts = fytSchedule.length;
        uint256 futureExpiry = future.expiry();
        ...
        // gets the YT
        future.yt().transferFrom(msg.sender, address(this), amount);

        // deploying and minting FYTs
        address futureAddress = address(future);
        fyts = new FYT[](scheduleParts);
        FYT fyt;
        for (uint256 i; i < scheduleParts; ++i) {
            fyt = address(
                futureToFytWithSchedule[futureAddress][fytSchedule[i][0]][
                    fytSchedule[i][1]
                ]
            ) != address(0)
                ? futureToFytWithSchedule[futureAddress][fytSchedule[i][0]][
        ...
        userFYTPerBatch[futureAddress][msg.sender].push(fyts);
        userFYTAmountPerBatch[futureAddress][msg.sender].push(amount);

        emit YTTokenized(msg.sender, futureAddress, amount);
    }
    ...
    function tokenizeYTOldBatch(
        ...
        address futureAddress = address(future);
        uint256 userBatchNb = userFYTAmountPerBatch[futureAddress][msg.sender]
            .length;

        require(batchId < userBatchNb, "FYTFactory: Index out of bounds");

        uint256 firstFytStartDate = userFYTPerBatch[futureAddress][msg.sender][
            batchId
        ][0].startDate();

        // checking if the batch has not started yet so first fyt start date should be less than current timestamp
        require(
            firstFytStartDate >= block.timestamp,
            "FYTFactory: The given batch has already started"
        );

        // gets the YT
        future.yt().transferFrom(msg.sender, address(this), amount);

        FYT[] memory batch = userFYTPerBatch[futureAddress][msg.sender][
            batchId
        ];
        ...
        userFYTAmountPerBatch[futureAddress][msg.sender][batchId] += amount;
     ...
     * @param future Future vault of the YT being withdrawn
    ...
    function withdrawUntokenizedYTByBatch(
        ...
        require(
            withdrawnBatchNb > 0,
            "FYTFactory: No batch given to withdraw YT from"
        );

        // further checking (e.g. is it well sorted, is the last FYT expired) + withdrawing YT from batches
        uint256 batchId = batches[0];
        require(batchId < userBatchNb, "FYTFactory: Index out of bounds");
        uint256 batchLength = userFYTPerBatch[futureAddress][msg.sender][
            batchId
        ].length;
        uint256 lastFytEndDate = userFYTPerBatch[futureAddress][msg.sender][
            batchId
        ][batchLength - 1].endDate();
        require(
            lastFytEndDate <= block.timestamp,
            "FYTFactory: Some FYT are left in schedule for the given batches"
        );
        uint256 amount = userFYTAmountPerBatch[futureAddress][msg.sender][
            batchId
        ];
        for (uint256 i = 1; i < withdrawnBatchNb; ++i) {
            ...
            require(
                ...
                "FYTFactory: Batches must be in strictly ascending order"
            );
            require(batchId < userBatchNb, "FYTFactory: Index out of bounds");
            batchLength = userFYTPerBatch[futureAddress][msg.sender][batchId]
                .length;
            lastFytEndDate = userFYTPerBatch[futureAddress][msg.sender][
                batchId
            ][batchLength - 1].endDate();
            require(
                lastFytEndDate <= block.timestamp,
            ...
            amount += userFYTAmountPerBatch[futureAddress][msg.sender][batchId];
        }

        YT yt = future.yt();
        yt.transfer(msg.sender, amount);
        emit YTWithdrawn(msg.sender, futureAddress, amount);
        ...
        for (uint256 i = withdrawnBatchNb; i > 0; i--) {
            ...
            if (batchId < userBatchNb - 1) {
                ...
                userFYTPerBatch[futureAddress][msg.sender][
                    batchId
                ] = userFYTPerBatch[futureAddress][msg.sender][userBatchNb - 1];
                userFYTAmountPerBatch[futureAddress][msg.sender][
                    batchId
                ] = userFYTAmountPerBatch[futureAddress][msg.sender][
                    userBatchNb - 1
                ];
            ...
            // pop the last batch
            userFYTPerBatch[futureAddress][msg.sender].pop();
            userFYTAmountPerBatch[futureAddress][msg.sender].pop();
    ...
    function splitFYT(
    ...
    ) external returns (FYT[] memory newFyts) {
        uint256 fytStartDate = fyt.startDate();
        require(
            fytStartDate > block.timestamp,
            "FYTFactory: Trying to burn an fyt during its period"
        );
        uint256 scheduleParts = newFytSchedule.length;
        uint256 fytEndDate = fyt.endDate();
        FutureVault future = fyt.future();

        // checking correctness of the schedule
        _checkSchedule(newFytSchedule, scheduleParts, fytStartDate, fytEndDate);

        // finding index of the FYT in the given batch and creating the new array of FYTs
        uint256 batchLength = userFYTPerBatch[address(future)][msg.sender][
            batchId
        ].length;
        ...
        FYT curFyt;
        for (uint256 i; i < batchLength; ++i) {
            curFyt = userFYTPerBatch[address(future)][msg.sender][batchId][i];
            if (curFyt == fyt) {
                fytId = i;
                break;
            }
        ...
        require(
            fytId < batchLength,
            "FYTFactory: FYT not found in the given batch"
        );

        // burning the old fyt
        uint256 amount = userFYTAmountPerBatch[address(future)][msg.sender][
            batchId
        ];
        ...
        for (uint256 i; i < scheduleParts; ++i) {
            ...
            FYT newFyt = address(
                futureToFytWithSchedule[address(future)][newFytStartDate][
                    newFytEndDate
                ]
            ) != address(0)
                ? futureToFytWithSchedule[address(future)][newFytStartDate][
        ...
        for (uint256 i = fytId + scheduleParts; i < finalLength; ++i) {
            newFyts[i] = userFYTPerBatch[address(future)][msg.sender][batchId][
                i - scheduleParts + 1
            ];
        }

        // replacing the old fyts by the new ones
        userFYTPerBatch[address(future)][msg.sender][batchId] = newFyts;
    ...
    /**
     * @notice Function to set the mapping of particular fyt with start, end date and futureVault
     * @param future The futureVault address.
    ...
    function setFYTOfFuture(
        ...
        futureToFytWithSchedule[future][startDate][endDate] = fyt;
        emit UpdatedFYTOfFuture(future, startDate, endDate, fyt);
    }

    /**
     * @notice Function to get the mapping of particular fyt with start, end date and futureVault
     * @param future The futureVault address.
     * @param startDate Start date of the fyt.
     * @param endDate End date of the fyt.
    ...
    function getFYTOfFuture(
        ...
        return futureToFytWithSchedule[future][startDate][endDate];
    }
    ...
    function getUserFYT(
        ...
        return userFYTPerBatch[future][user];
    }
    ...
    function getUserFYTAmount(
        ...
        return userFYTAmountPerBatch[future][user];
    }
     ...
     * @param future Future vault of the FYT
     * @param startDate Start date of the FYT
     * @param endDate End date of the FYT
    ...
    function _deployFYT(
        ...
        fyt = new FYT();
        fyt.initialize(name, symbol, this, future, startDate, endDate);
        futureToFytWithSchedule[address(future)][startDate][endDate] = fyt;
    ...
    function _checkSchedule(
        ...
        require(scheduleParts > 0, "FYTFactory: FYT schedule is empty");
        ...
        for (uint256 i; i < scheduleParts - 1; ++i) {
            ...
            require(
                scheduledFytStartDate >= minStartDate,
                "FYTFactory: Start date is incorrect"
            );
            require(
                scheduledFytEndDate <= maxEndDate,
                "FYTFactory: End date is incorrect"
            );
            require(
                scheduledFytEndDate >= scheduledFytStartDate + 1 days,
                "FYTFactory: Duration must be at least 1 day"
            );
            require(
                scheduledFytStartDate <= nextScheduledFytStartDate,
                "FYTFactory: Start dates must be in ascending order"
            );
            require(
                scheduledFytEndDate <= nextScheduledFytStartDate,
                "FYTFactory: FYT schedule is not disjoint"
            );
        }
...
interface IYT {
    ...
    */
    ...
    /** @dev Returns the address of futureVault associated with this YT. */
    function getFutureVault() external view returns (address);
}
...
contract FYT is Initializable, ERC20PermitUpgradeable {
    using SafeMathUpgradeable for uint256;

    FutureVault public future;
     ...
     * @param _future The FutureVault contract associated with this fyt token.
    ...
    function initialize(
        ...
        require(
            ...
            "FYT: Start date must be in the future"
        );
        require(
            _endDate <= _future.expiry(),
            "FYT: Trying to initialize future yield token beyond expiration of the future vault"
        );
        require(
            _endDate >= _startDate + 1 days,
            "FYT: Duration must be at least 1 day"
        );
        require(
            address(
            ...
            "FYT: Future yield token having this start date has already been deployed for this future"
        );
    ...
    function decimals() public view virtual override returns (uint8) {
        return IERC20MetadataUpgradeable(address(future)).decimals();
    }
    ...
    function mint(address to, uint256 amount) external {
        require(
            msg.sender == fytFactory,
            "FYT: Caller should be the FYT factory"
        );
        _mint(to, amount);
    }
    ...
    PausableUpgradeable
{
    ...
    uint256 private ibtRateAtExpiry;
    uint256 private ptRateAtExpiry;
    ...
    uint256 public unclaimedFeesInIBT;
    uint256 public maxProtocolFee;
    ...
    event StoredRatesAtExpiry(
        uint256 indexed ibtRateAtExpiry,
        uint256 indexed ptRateAtExpiry
    );
    ...
    function initialize(
        ...
        require(
            _ytBeacon != address(0),
            "FutureVault: yt beacon address should not be equal to zero"
        );
        require(
            _ibt != address(0),
            "FutureVault: Cannot set ibt to zero address"
        );
        __ERC20_init(_name, _symbol);
        ...
        ibtUnit = 10 ** IERC20MetadataUpgradeable(_ibt).decimals();
        assetUnit = 10 ** _asset.decimals();
        ibt = _ibt;
        yt = _deployYT("Yield Token", "YT", _ytBeacon);
        ibtRate = convertToAssetsOfIBT(ibtUnit);
        ptRate = assetUnit;
        require(
            _max_fees <= 1e18,
            "FutureVault: Max fees should be less than 100%"
        );
    ...
    function mint(
        ...
        returns (uint256)
    {
        require(shares <= maxMint(receiver), "FutureVault: mint more than max");
        ...
        returns (uint256 assets)
    {
        require(
            owner == msg.sender,
            "FutureVault: caller should be the owner of shares"
        );
        require(
            shares <= maxRedeem(owner),
            "FutureVault: redeem more than max"
        );
        ...
        returns (uint256 shares)
    {
        ...
        uint256 feesInIBT = (yieldInIBT * maxProtocolFee) / 1e18; // fees of the yield of owner in IBT
        ...
        if (assetsInIBT > yieldInIBT - feesInIBT) {
            ...
            if (yieldInIBT - feesInIBT == 0) {
                revert(
                    "FutureVault: Net yield is 0 whereas supposed to cover the withdraw"
                );
            }
    ...
    /** @dev See {IFutureVault-withdrawAfterExpiry}. */
    function withdrawAfterExpiry(
        address receiver,
        address owner
    )
        ...
        returns (uint256 withdrawnAssets)
    {
        ...
        // dividing by 10^18 because fees is in terms of 18 decimals for precision.
        uint256 currentFee = ((yieldInIBT * maxProtocolFee) / 1e18);
    ...
    function updateYield(
        ...
        uint256 _ibtRate;
        uint256 _ptRate;
        (_ptRate, _ibtRate) = _updatePTandIBTRates();
    ...
    function claimYieldOfAmount(
        ...
        if (yieldToTransfer != 0 && (_amount != 0 || userYTBalance == 0)) {
            ...
            uint256 currentFee = ((yieldToTransfer * maxProtocolFee) / 1e18); // dividing by 10^18 because fees is in terms of 18 decimals for precision.
            _updateFees(currentFee);
    ...
    function setFeeCollector(address _feeCollector) public {
        ...
        require(
            ...
            "FutureVault: Cannot set fee collector to zero address"
        );
        feeCollector = _feeCollector;
        emit UpdatedFeeCollector(feeCollector);
    }

    /** @dev See {IFutureVault-storeRatesAtExpiry}. */
        ...
        returns (uint256, uint256)
    {
        require(
            !ratesAtExpiryStored,
            "FutureVault: Rates at expiry already stored"
        );
        ratesAtExpiryStored = true;

        (uint256 _ptRate, uint256 _ibtRate) = _getCurrentPTandIBTRates();
        ibtRateAtExpiry = _ibtRate;
        ptRateAtExpiry = _ptRate;
        ibtRate = _ibtRate;
        ptRate = _ptRate;
        return (_ibtRate, _ptRate);
    }

    /**
     * @notice update zap depositor
     */
    function updateZapDepositor() public {
        _checkOwner();
        zapDepositor = registry.getZapDepositor();
    ...
    function previewWithdraw(
        ...
        uint256 _yieldOfUserInIBT = _computeYield(
            msg.sender,
            _oldibtRate,
            _ibtRate,
            _oldptRate,
        ...
        uint256 maxNetYieldOfUserInUnderlying = convertToAssetsOfIBT(
            (_yieldOfUserInIBT * (1e18 - maxProtocolFee)) / 1e18
        );
        ...
        returns (uint256)
    {
        ...
        return
            convertToAssetsOfIBT(
                ((1e18 - maxProtocolFee) * _yieldOfUserInIBT) / 1e18
        ...
        returns (uint256)
    {
        if (ratesAtExpiryStored) {
            ...
            uint256 assetsAtExpiry = convertToAssetsWithRate(
                shares,
                ptRateAtExpiry
            );
            ...
            revert("FutureVault: Rates at expiry not stored");
        }
    }
    ...
    function decimals()
        ...
        returns (uint8)
    {
        return IERC20MetadataUpgradeable(ibt).decimals();
    }
    ...
    /** @dev See {IFutureVault-getIBTRate}. */
    function getIBTRate() public view override returns (uint256) {
        return ibtRate;
    }

    /** @dev See {IFutureVault-getPTRate}. */
    function getPTRate() public view override returns (uint256) {
        return ptRate;
    }

    /** @dev See {IFutureVault-getIBTUnit}. */
    function getIBTUnit() public view override returns (uint256) {
        return ibtUnit;
    }

    /** @dev See {IFutureVault-getAssetUnit}. */
    function getAssetUnit() public view override returns (uint256) {
        return assetUnit;
    }

    /** @dev See {IFutureVault-getIBTRateAtExpiry}. */
    function getIBTRateAtExpiry()
        external
        view
        override
        afterExpiry
        returns (uint256)
    {
        return ibtRateAtExpiry;
    }

    /** @dev See {IFutureVault-getPTRateAtExpiry}. */
    function getPTRateAtExpiry()
        external
        view
        override
        afterExpiry
        ...
        address _ytAddr = address(
            new BeaconProxy(
                ...
                abi.encodeWithSelector(
                    YT(address(0)).initialize.selector,
                    name_,
                    symbol_,
                    address(this)
                )
        ...
        if (_oldPTRate == _ptRate && _ibtRate == _oldIBTRate) {
            ...
            if (_oldPTRate > _ptRate) {
                ...
                if (_ibtRate >= _oldIBTRate) {
                    ...
                    yieldInUnderlying = yieldInUnderlying < 100
                        ? 0
                        : yieldInUnderlying;
                }
                newYieldInIBT = convertAssetsToIBTWithRate(
                ...
                revert(
                    "FutureVault: PT rate increased or no PT depeg but IBT rate decreased!"
                );
            }
        }
    ...
    function _getPTandIBTRates() internal view returns (uint256, uint256) {
        if (block.timestamp >= expiry) {
            if (ratesAtExpiryStored) {
                return (ptRateAtExpiry, ibtRateAtExpiry);
            } else {
                revert("FutureVault: Rates at expiry not stored");
            }
        } else {
...
contract YT is IYT, Initializable, ERC20PermitUpgradeable {
    using SafeMathUpgradeable for uint256;
     ...
     * @param vault The address of the futureVault associated with this yt token.
     */
    function initialize(
        string memory name_,
        string memory symbol_,
        ...
        futureVault = IFutureVault(vault);
    }

    /**
    * @notice checks for msg.sender to be futureVault and then calls _burn of ERC20Upgradeable.
      See {ERC20Upgradeable- _burn}.
     */
    ...
    /**
    * @notice checks for msg.sender to be futureVault and then calls _mint of ERC20Upgradeable.
      See {ERC20Upgradeable- _mint}.
     */
    function mint(address to, uint256 amount) external {
        require(
    ...
    * @notice updates the yield of the caller and then calls _burn of ERC20Upgradeable.
    ...
    function burn(uint256 amount) public {
        futureVault.updateYield(msg.sender);
    ...
    function transfer(
        ...
        futureVault.beforeYtTransfer(msg.sender, to);
    ...
    function transferFrom(
        ...
        futureVault.beforeYtTransfer(from, to);
    ...
    function decimals()
        ...
        view
        virtual
        override(IYT, ERC20Upgradeable)
        returns (uint8)
    {
        return IERC20MetadataUpgradeable(address(futureVault)).decimals();
    }

    /** @dev See {IYT- getFutureVault} */
    function getFutureVault() public view virtual override returns (address) {
