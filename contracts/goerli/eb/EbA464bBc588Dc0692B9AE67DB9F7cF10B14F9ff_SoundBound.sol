...
//    uint256 private whitelistPurchasePrice = 28500000000000000 wei;
    ...
    event LogMusicPassCreated(uint256 indexed tokenId, address indexed ownerWalletAddress, bytes32 ensDomain, uint32 genre);
...
//        require(msg.sender == ens.ownerOf(node),  "You are not the owner of this ENS domain.");

        purchaseStatus[node] = true;
        tokenId.increment();
        uint256 newItemId = tokenId.current();
        _safeMint(msg.sender, newItemId);
        domainNameToTokenId[node] = newItemId;
        emit LogMusicPassCreated(newItemId, msg.sender, node, genreId);
    }
    ...
    function getTokenIdByDomain(bytes32 node) external view returns (uint256) {
        require(purchaseStatus[node] == true,  "The domain hasn't been purchased yet.");
        return domainNameToTokenId[node];
    }
