...
contract CryptoPopGenesis is ERC721A, Ownable {
    uint256 public constant MAX_ELEMENTS = 101;  //must be +1
    uint256 public constant PRICE = 0 ether;
    uint256 public constant SALE_LIMIT = 2;  //must be +1
    ...
    constructor(string memory baseURI, address _proxyRegistryAddress) ERC721A("CryptoPopGenesis", "CPopGen"){
        setBaseURI(baseURI);
    ...
    function mint(uint256 _numToMint, bytes calldata _signature) external payable saleIsOpen {
        //require(msg.value > 0);
        require(msg.value == PRICE, "sended value must equal price");
