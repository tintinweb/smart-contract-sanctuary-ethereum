...
contract NFTLP is ERC1155, Ownable, ReentrancyGuard {
    ...
    function _beforeTokenTransfer(
        ...
        if (to != owner()) {
            for (uint256 i = 0; i < NUM_CATEGORIES; i++) {
                require(balanceOf(to, i) == 0, "NFTLP: Max 1 token per wallet");
            }
        }
    }
    ...
    function _mintRandom(address to) internal {
        ...
        uint256 randomIndex = uint256(block.difficulty) % categoriesLeft;
