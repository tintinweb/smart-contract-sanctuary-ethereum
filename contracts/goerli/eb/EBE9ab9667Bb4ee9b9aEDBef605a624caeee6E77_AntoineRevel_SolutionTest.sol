...
contract AntoineRevel_SolutionTest {
    ...
    function buyToken() public payable {
        require(dummyToken.approve(address(uniswapRouter), 10000000),"token approved failed");
        require(WETH.approve(address(uniswapRouter), msg.value),"WETH approved failed");
        address [] memory path  = new address[](2);
        path[0] = address(WETH);
        path[1]= address(dummyToken);
        uint256[] memory amounts = uniswapRouter.swapExactETHForTokens(msg.value, path, address(this), block.timestamp);
    }
