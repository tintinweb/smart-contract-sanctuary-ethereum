...
// File: EngagementFarmers_flat.sol
...
// File: contracts/EngagementFarmers.sol
...
contract EngagementFarmers is ERC721A, Ownable, ReentrancyGuard {
    ...
    function whitelistMint(uint256 quantity, bytes32[] memory proof) external payable nonReentrant {
        require(whenAllowlistSaleIsOn() == true, "FRMR: Whitelist Sale Not Started Yet");
        require(isValid(proof, keccak256(abi.encodePacked(msg.sender))), "EF: Not a part of Allowlist");
        whitelistCounter[msg.sender] += quantity;
        require(getWhitelistMinted(msg.sender) <= WHITELIST_LIMIT, "EF: Whitelist Limit Exceeded");
    ...
    function publicMint(bytes calldata signature) external payable nonReentrant {
        require(whenPublicaleIsOn() == true, "FRMR: Public Sale Not Started Yet");
        require(checkSign(signature, msg.sender, 1) ==_signerAddress, "EF: Invalid Signature");
        publicCounter[msg.sender] += 1;
        require(getPublicMinted(msg.sender) <= PUBLIC_LIMIT, "EF: Public Limit Exceeded");
        require(totalSupply().add(1) <= MAX_SUPPLY, "EF: Mint would exceed max limit");
        require(msg.value == (MINT_FEES), "EF: Send proper mint fees");
        payable(owner()).transfer(msg.value);
        _mint(msg.sender, 1);
    }
