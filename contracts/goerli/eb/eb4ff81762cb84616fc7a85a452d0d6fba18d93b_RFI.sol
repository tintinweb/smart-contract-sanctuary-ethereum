...
contract RFI is Context, IERC20, Ownable {
    ...
    constructor () {
        ...
        // TODO: I THINK WE NEED TO EXCLUDE OTHER WALLETS AS WELL. NEED TO EXCLUDE STAKING CONTRACT FROM FEES AND EVERYTHING
        ...
        _approve(owner(), address(uniswapV2Router), _tTotal * 10 / 100); //is this helpful at all? or will uniswap ask to grant an allowance anyway?
    ...
    function _transfer(
        ...
        if (!_isExcludedFromFees[from] && !_isExcludedFromFees[to]) {
            ...
            if (automatedMarketMakerPairs[to]) { //sell
                tRewards = amount / 25;                     //4
                tBurn = amount / 25;                        //4
                tRemainingFees = amount * 2 / 25;           //8
                tokensForLiquidity += tRemainingFees / 4;   //2
                tokensForOps += tRemainingFees * 3 / 8;     //3
                tokensForMarketing += tRemainingFees / 4;   //2
                tokensForFarmRewards += tRemainingFees / 8; //1

            } else if (automatedMarketMakerPairs[from]) { //buy
                tRewards = amount * 3 / 100;              //3
                tBurn = amount * 3 / 100;                 //3
                tRemainingFees = amount * 3 / 50;        //6
                tokensForLiquidity += tRemainingFees / 6;    //1
                tokensForOps += tRemainingFees / 3;          //2
                tokensForMarketing += tRemainingFees / 3;    //2
                tokensForFarmRewards += tRemainingFees / 6;  //1
            }
