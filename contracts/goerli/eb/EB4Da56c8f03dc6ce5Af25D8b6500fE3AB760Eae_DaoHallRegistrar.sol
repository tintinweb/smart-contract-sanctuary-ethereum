...
contract DaoHallRegistrar is Ownable{
  ...
  modifier authorised(bytes32 labelhash) {
    address owner = _ownerOfEns(labelhash);
    if(isWrapped(owner)){
      owner = wrapper.ownerOf(uint256(_l2Node(labelhash)));
  ...
  function whitelistClaimed(bytes32 root, address claimer) public view returns (uint256){
    return daoHallRegistry.getWhitelistClaimed(root, claimer);
  }
  ...
  function buySubnode(string memory label, bytes32 parentNode, address resolver, uint64 ttl, uint32 fuses, uint32 duration) external payable {
    (address parentOwner, ,) = wrapper.getData(uint256(parentNode));
    NodeRecord memory record = daoHallRegistry.getRecord(parentNode, parentOwner);
    _checkBuy(record, label, parentNode, duration);
  ...
  function buySubnodeWhitelisted(string memory label, bytes32 parentNode, address resolver, uint64 ttl, uint32 fuses, bytes32[] calldata proof, uint64 data) external payable {
    ...
    (address parentOwner, ,) = wrapper.getData(uint256(parentNode));
    NodeRecord memory record = daoHallRegistry.getRecord(parentNode, parentOwner);
    _checkBuyWhitelisted(record, label, parentNode, duration, proof, limit);
    _buy(label, parentNode, resolver, ttl, fuses, msg.value, duration);
  }
  ...
  function extendSubnode(string memory label, bytes32 parentNode, uint32 duration) external payable {
    ...
    (address owner, , uint64 oldExpiry) = wrapper.getData(uint256(node));
    require(owner != address(0), "No such subnode");

    (address parentOwner, , uint64 parentExpiry) = wrapper.getData(uint256(parentNode));
    NodeRecord memory record = daoHallRegistry.getRecord(parentNode, parentOwner);

    uint256 price = _calculatePrice(record, parentNode, unicodeLength(label), duration);
    ...
    wrapperController.setSubnodeExpiry(parentNode, label, newExpiry);
  ...
  function extendSubnodeEns(string memory label, bytes32 parentHash, uint32 duration) external payable {
    ...
    (, uint256 oldExpiry) = ensController.getNodeData(node);
    require(owner != address(0), "No such subnode");

    NodeRecord memory record = daoHallRegistry.getRecord(parentNode, owner);
    uint256 price = _calculatePrice(record, parentNode, unicodeLength(label), duration);
    address parentOwner = _ownerOfEns(parentHash);
    ...
    ensController.setSubnodeExpiry(parentNode, label, uint64(newExpiry));
    emit SubnodeExtended(label, parentNode, msg.sender, duration, price);
  }
  ...
  function _calculatePrice(NodeRecord memory record, bytes32 node, uint256 length, uint32 duration) internal pure returns (uint256) {
  ...
  function _calculateWhitelistPrice(NodeRecord memory record, bytes32 node, uint256 length, uint32 duration) internal pure returns (uint256) {
    // check discount
    uint8 discount = whitelistDiscount(record.whitelistParam, length);
    require(discount <= 100, "Not available");
    return _calculatePrice(record, node, length, duration) * (100 - discount) / 100;
  }

  function _checkBuy(NodeRecord memory record, string memory label, bytes32 parentNode, uint32 duration) internal {
    ...
    // check amount
    uint256 price = _calculatePrice(record, parentNode, len, duration);
    require(msg.value == price, "Invalid price");
  }

  function _checkBuyWhitelisted(NodeRecord memory record,string memory label, bytes32 parentNode, uint32 duration, bytes32[] calldata proof, uint32 limit) internal {
    ...
    uint256 claimed = daoHallRegistry.getWhitelistClaimed(record.whitelistRoot, msg.sender);
    ...
    // check amount
    uint256 price =  _calculateWhitelistPrice(record, parentNode, unicodeLength(label), duration);
    require(msg.value == price, "Invalid price");
    ...
    daoHallRegistry.setWhitelistClaimed(record.whitelistRoot, msg.sender, claimed + 1);
  }
  ...
  function _buy(string memory label, bytes32 parentNode, address resolver, uint64 ttl, uint32 fuses, uint256 price, uint32 duration) internal{
    ...
    _processFee(price, parentOwner);
    wrapperController.setSubnodeRecord(parentNode, label, msg.sender, resolver, ttl, fuses, childExpiry);
  ...
  function _buyEns(string memory label, bytes32 parentHash, address resolver, uint64 ttl, uint32, uint256 price, uint32 duration) internal{
    ...
    ensController.setSubnodeRecord(parentNode, label, msg.sender, resolver, ttl, 0, childExpiry);
    emit SubnodeSale(label, parentNode, msg.sender, duration, price);
  }

  function isWrapped(address owner) public view returns (bool) {
