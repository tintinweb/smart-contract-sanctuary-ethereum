...
contract Rektguy is ERC721,Ownable,ReentrancyGuard{
    ...
    bytes32 private merkleRoot =
        0xabb537f5b063e17d4eaa4abc45c0328e336d635db57b79545f41bf2ef822b696;
    ...
    function getMerkleProofFor(address sampleAddress, bytes32[] calldata _merkleProof) public view returns (bool){
    ...
    */
    ...
    modifier restrictMint(uint256 numberOfTokens,bool _saleIsActive) {
        ...
        require(tokenCounter.add(numberOfTokens) <= MAX_TOKENS, "Purchase exceeds max supply of tokens");
        _;
    ...
    function mintNFTPreSale(uint256 numberOfTokens, bytes32[] calldata _merkleProof) public nonReentrant() restrictMint(numberOfTokens,preSaleIsActive) {
        require(getPreSaleDuration() < PRESALE_DURATION, "presale one has ended... the main sale will now be initiated");
        if (validNumberOfTokensPerBuyerMap[msg.sender] == 0 && !whitelistClaimed[msg.sender]){
            bytes32 leaf = keccak256(abi.encodePacked(msg.sender));
            require(MerkleProof.verify(_merkleProof, merkleRoot, leaf), "Account not valid for presale");
            whitelistClaimed[msg.sender] = true;
            validNumberOfTokensPerBuyerMap[msg.sender]= 1;
        }
        require(tokenBalancePerOwner[msg.sender].add(numberOfTokens) <= validNumberOfTokensPerBuyerMap[msg.sender], "No more valid tokens, please refer to status page for more info");
        validNumberOfTokensPerBuyerMap[msg.sender]-=numberOfTokens;
    }
    ...
    function mintNFT(uint256 numberOfTokens) public nonReentrant() restrictMint(numberOfTokens,saleIsActive) {
         require(tokenBalancePerOwner[msg.sender].add(numberOfTokens)<= MAX_PURCHASE_TOKENS, "A maximum of 10 tokens can be purchased per account");
    }
