...
contract DPSGameEngine is DPSGameEngineI, Ownable {
    ...
    function getLockedVoyageByOwnerAndId(address _owner, uint256 _voyageId, DPSVoyageIV2 _voyage)
        external
        view
        returns (LockedVoyage memory locked)
    {
        LockedVoyage[] memory cachedVoyages = docks.getLockedVoyagesForOwner(_owner, 0, _voyage.maxMintedId());
        for (uint256 i; i < cachedVoyages.length; ++i) {
            if (cachedVoyages[i].voyageId == _voyageId) return cachedVoyages[i];
        }
    }

    function getFinishedVoyageByOwnerAndId(address _owner, uint256 _voyageId, DPSVoyageIV2 _voyage)
        external
        view
        returns (LockedVoyage memory locked)
    {
        LockedVoyage[] memory cachedVoyages = docks.getFinishedVoyagesForOwner(_owner, 0, _voyage.maxMintedId());
        for (uint256 i; i < cachedVoyages.length; ++i) {
            if (cachedVoyages[i].voyageId == _voyageId) return cachedVoyages[i];
        }
    }
    ...
    function setContract(address _contract, uint8 _target) external onlyOwner {
        ...
        else if (_target == 5) dpsFeatures = DPSPirateFeaturesI(_contract);
        else if (_target == 6) cartographer = DPSCartographerI(_contract);
        else if (_target == 7) chests = DPSChestsIV2(_contract);
        else if (_target == 8) docks = DPSDocksI(_contract);
        else if (_target == 9) doubloons = IERC20MintableBurnable(_contract);
        else if (_target == 10) treasury = _contract;
        else if (_target == 11) plunderers = _contract;
