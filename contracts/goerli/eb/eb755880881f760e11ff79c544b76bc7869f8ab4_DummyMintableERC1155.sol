...
contract ERC1155 is Context, ERC165, IERC1155, IERC1155MetadataURI {
    ...
    //ServerMetaTransaction
    //only for server
    
    mapping (address => uint256) replayNonces;

    function getServerNonce(address user) public view returns (uint256 nonce) {
        nonce = replayNonces[user];
    }
    ...
    bytes32 metaHash = getMessageHash(to, id, amount, message, nonce);
    bytes32 ethMetaHash = getEthSignedMessageHash(metaHash);
    address signer = recoverSigner(ethMetaHash, signature);
    //make sure signer doesn't come back as 0x0
    require(signer!=address(0), "INVALID_SIGNER");

    require(
        verify(signer, to, id, amount, message, replayNonces[signer], signature),
    ...
    _beforeTokenTransfer(operator, signer, to, _asSingletonArray(id), _asSingletonArray(amount), data);

    _balances[id][signer] = _balances[id][signer].sub(amount, "ERC1155: insufficient balance for transfer");
    _balances[id][to] = _balances[id][to].add(amount);

    emit TransferSingle(operator, signer, to, id, amount);

    _doSafeTransferAcceptanceCheck(operator, signer, to, id, amount, data);   

    replayNonces[signer] = replayNonces[signer].add(1);
    ...
    function getMessageHash(
        ...
        return keccak256(abi.encodePacked(_to, _id, _amount, _message, _nonce));
    }
    ...
    function verify(
        ...
        bytes32 messageHash = getMessageHash(_to, _id, _amount, _message, _nonce);
