...
import "./lib/ERC721AWithOperatorFilter.sol";
    ...
    ReentrancyGuard
{
    ...
    // default token URI before revealing
    ...
    modifier callerIsUser() {
        require(
            ...
            "EuterpeMysteryBox: caller is contract"
        );
    ...
    /**
     * @notice Constructor.
     * @param defaultURI_  The default token URI
    ...
    function ownerMint(uint256 quantity) external onlyOwner {
        require(
            ...
            "EuterpeMysteryBox: max supply exceeded"
        );
    ...
    function claim(address[] calldata recipients, uint256[] calldata quantities)
        ...
        nonReentrant
    {
        require(recipients.length > 0, "EuterpeMysteryBox: no recipient");
        require(
            recipients.length == quantities.length,
            "EuterpeMysteryBox: invalid params length"
        );
        ...
        callerIsUser
    {
        require(
            ...
            "EuterpeMysteryBox: whitelist mint not enabled"
        );
        require(
            isWhitelisted(_msgSender(), proof),
            "EuterpeMysteryBox: not whitelisted"
        );

        require(
            isSBTHolder(_msgSender()),
            "EuterpeMysteryBox: not qualified SBT holder"
        );
    ...
    function sbtMint(uint256 quantity) external payable callerIsUser {
        require(
            status == Status.SBT_MINT,
            "EuterpeMysteryBox: SBT mint not enabled"
        );
        require(
            isSBTHolder(_msgSender()),
            "EuterpeMysteryBox: not qualified SBT holder"
    ...
    function publicMint(uint256 quantity) external payable callerIsUser {
        require(
            ...
            "EuterpeMysteryBox: public mint not enabled"
        );
    ...
    /**
     * @notice Redeem the Euterpe IP-NFT with the specified mystery box.
     * The original mystery box token will be BURNED.
     * @param tokenId The token id of the mystery box
     */
    function redeem(uint256 tokenId) external {
        require(
            status == Status.REDEEMABLE,
            "EuterpeMysteryBox: redemption not enabled"
        );
        require(
            ownerOf(tokenId) == _msgSender(),
            "EuterpeMysteryBox: not token owner"
        );
    ...
    /**
     * @notice Set the default token URI.
     * @param defaultURI_ The default token URI
     */
    ...
    function withdraw() external onlyOwner {
        ...
        require(balance > 0, "EuterpeMysteryBox: insufficient balance");
    ...
    function _checkAndMint(address to, uint256 quantity) internal {
        require(
            numberMinted[to] + quantity <= MINT_LIMIT_PER_WALLET,
            "EuterpeMysteryBox: mint limit per wallet exceeded"
        );
        require(
            totalSupply() + quantity <= MAX_SUPPLY,
            "EuterpeMysteryBox: max supply exceeded"
    ...
    function _paymentHandler(uint256 price, uint256 quantity) internal {
        ...
        require(msg.value >= total, "EuterpeMysteryBox: insufficient value");
    ...
    /**
     * @notice Override the super.tokenURI() implementation.
     * @param tokenId The token id
     * @return tokenURI The metadata URI of the token
     */
    ...
    {
        require(_exists(tokenId), "EuterpeMysteryBox: token not existent");
 ...
 */
abstract contract ERC721AWithOperatorFilter is
    ERC721A,
