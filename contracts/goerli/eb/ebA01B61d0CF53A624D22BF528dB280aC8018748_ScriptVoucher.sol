...
/// @title Script Discount Voucher
/// @author @nabeelimran852
contract ScriptVoucher is ERC1155Supply {
  // Address of the SPAY Token
  IScriptPay public immutable spay;

  // Address of the owner of ScriptTV
  address public scriptTV;
  // supply of each voucher
  uint256[3] public supply;
  // price of each type of voucher
  uint256[3] public mintPrice;
  // discount on each type of voucher
  uint256[3] public discountPercent;
  ...
  /**
   * @notice Emitted when the supply is updated
   * @param previousSupply old supply
  ...
  constructor(IScriptPay _spay) ERC1155("") {
    spay = _spay;
    scriptTV = msg.sender;
    // Define the initial supply and mint price for each token type
    supply = [300, 200, 100];
    mintPrice = [250 ether, 500 ether, 800 ether];
    discountPercent = [10 ether, 20 ether, 30 ether];
  }
  ...
  /**
   * @notice mint certain type of token
   * @param tokenType  type of token to mint
   * @param to  the address to which token is minted
   */
  function mint(address to, VoucherType tokenType) external onlyScriptTV {
    if (totalSupply(uint256(tokenType)) >= supply[uint256(tokenType)]) {
      revert InsufficientSupply();
    }
    if (spay.balanceOf(to) < mintPrice[uint256(tokenType)]) {
      revert InsufficientFunds();
    }
    // pay price
    spay.burnFrom( to, mintPrice[uint256(tokenType)]);
    // Mint the tokens and send them to the to
    _mint(to, uint256(tokenType), 1, "");
    //  move the event to script TV
  }

  /**
   * @notice update the supply of given token type
   * @param tokenType  type of token
   * @param newSupply  updated supply
   */
  function updateSupply(VoucherType tokenType, uint256 newSupply) external onlyScriptTV {
    uint256 oldSupply = supply[uint256(tokenType)];
    if (newSupply <= oldSupply) {
      revert SameOrLessSupply();
    }
    // Update the supply of the specified token type
    supply[uint256(tokenType)] = newSupply;

    emit SupplyUpdate(oldSupply, newSupply, tokenType);
  }
   ...
   */
  function updateMintPrice(VoucherType tokenType, uint256 newPrice) external onlyScriptTV {
    uint256 oldPrice = mintPrice[uint256(tokenType)];
    // Update the mint price of the specified token type
    mintPrice[uint256(tokenType)] = newPrice;

    emit PriceUpdate(oldPrice, newPrice, tokenType);
  }

  /**
   * @notice return the discount percent of given voucher type
   * @param voucherType  type of voucher
   */
  function getDiscountPercentage(uint8 voucherType) external view returns (uint256) {
    return discountPercent[voucherType];
  }
}
