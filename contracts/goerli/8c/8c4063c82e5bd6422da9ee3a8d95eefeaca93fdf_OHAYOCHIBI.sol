...
contract OHAYOCHIBI is ERC721A, Ownable {
    ...
    address public teamAddress = 0x2B3CABa988CC1be61948559A3e8f90aC33DDCb1D;
    ...
    function reserveForTeam(uint256 quantity) public onlyOwner {
        require(
            totalSupply() + quantity <= teamReserve,
            "Not enough left for the team :("
        );
        for (uint256 i = 0; i < quantity; i++) {
            _safeMint(teamAddress, maxNumberMint);
        }
    }
    ...
    function mintChibi() external payable callerIsUser {
        ...
        require(numberMinted(msg.sender) < 1, "Only 1 chibi per wallet :)");
