...
contract ToyApeYachtClub is Ownable, ERC721A, ERC2981, ReentrancyGuard {
    ...
    function getOwnedToybirdIds(address holder) external view returns(uint256[] memory) {
    ...
    function holderClaim(uint256[] memory ownedToybirdIds) public nonReentrant {
        ...
        for (uint256 i = 0; i < ownedToybirdIds.length; i++) {
            ...
            if (!isClaimed(ownedToybirdIds[i]) && _toybirds.ownerOf(ownedToybirdIds[i]) == msg.sender) {
