...
contract LiquidatorRewards is ILiquidatorRewards, Owned, MixinSystemSettings, ReentrancyGuard {
    ...
    function resolverAddressesRequired() public view returns (bytes32[] memory addresses) {
        ...
        newAddresses[4] = CONTRACT_LIQUIDATOR_REWARDS_MANAGER;
        newAddresses[5] = CONTRACT_REWARDESCROW_V2_MANAGER;
    ...
    function rewardEscrowV2(bytes32 key) internal view returns (IRewardEscrowV2) {
        return rewardEscrowV2Manager().getRewardEscrow(key);
    }

    function totalLiquidates() external view returns (uint256) {
        if (synthetix().collateralCurrency(collateralKey) == NULL_ADDRESS) {
            return address(this).balance;
        } else {
            return IERC20(synthetix().collateralCurrency(collateralKey)).balanceOf(address(this));
        }
    }
    ...
    function getReward(address account) external nonReentrant {
        require(synthetix().collateralCurrency(collateralKey) != address(0), "Missed collateral currency");
        updateEntry(account);
        ...
        if (reward > 0) {
            ...
            if (collateralCurrency == NULL_ADDRESS) {
                TransferHelper.safeTransferETH(address(rewardEscrowV2(collateralKey)), reward);
            } else {
                IERC20(collateralCurrency).approve(address(rewardEscrowV2(collateralKey)), reward);
            }
            // synthetix().approve(address(rewardEscrowV2()), reward);
            rewardEscrowV2(collateralKey).createEscrowEntry(account, reward, getLiquidationEscrowDuration());
    ...
    function updateEntry(address account) public {
        ...
        liquidatorRewardsManager().sendUpdateEntry(collateralKey, account, synthetixDebtShare().balanceOfCrossChain(account));
    }
    ...
    function rewardRestitution(
        ...
        address collateralCurrency = synthetix().collateralCurrency(_collateralKey);
        if (collateralCurrency == NULL_ADDRESS) {
            require(address(this).balance >= _amount, "Insufficient ETH balance");
            TransferHelper.safeTransferETH(_to, _amount);
        } else {
            require(IERC20(collateralCurrency).balanceOf(address(this)) >= _amount, "Insufficient Collateral balance");
            TransferHelper.safeTransfer(collateralCurrency, _to, _amount);
        }
        emit TransferForRestritution(_to, _amount, _collateralKey);
        return true;
    }
    ...
    function notifyRewardAmount(uint256 reward) external onlySynthetix {
        uint256 sharesSupply = synthetixDebtShare().totalSupply();

        if (sharesSupply > 0) {
            accumulatedRewardsPerShare = accumulatedRewardsPerShare.add(reward.divideDecimal(sharesSupply));
        }
        liquidatorRewardsManager().sendNotifyReward(collateralKey, reward.divideDecimal(sharesSupply));
    }
    ...
    event TransferForRestritution(address indexed _to, uint256 _amount, bytes32 _collateralKey);
}
...
interface ILiquidatorRewardsAggregator {
    ...
    function sendNotifyReward(bytes32 currencyKey, uint256 increasedShare) external;
