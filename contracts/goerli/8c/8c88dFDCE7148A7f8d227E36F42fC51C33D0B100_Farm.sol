...
contract Farm is Initializable, OwnableUpgradeable {
  ...
  event RewardUser(address indexed user, uint256 amount, uint256 fee);
  event CollectFee(
  ...
  function pending(address _user) external view returns (uint256) {
    ...
    if (
      ...
      uint256 nrOfBlocks = lastBlock.sub(lastRewardBlock);
      uint256 erc20Reward = nrOfBlocks.mul(rewardPerBlock);
      _accERC20PerShare = accERC20PerShare.add(
        erc20Reward.mul(1e36).div(totalDeposited)
      );
    }

    return user.amount.mul(_accERC20PerShare).div(1e36).sub(user.rewardDebt);
  }
  ...
  function deposit(uint256 _amount, address _to) external onlyOwner {
    ...
    depositToken.transferFrom(address(_to), address(this), _amount);

    // calculate difference to avoid issues with token fees
    uint256 difference = depositToken.balanceOf(address(this)).sub(
      beforeBalance
    );

    totalDeposited = totalDeposited.add(difference);

    user.depositedAt = block.number;
    user.amount = user.amount.add(difference);
    user.rewardDebt = user.amount.mul(accERC20PerShare).div(1e36);
  ...
  function withdraw(uint256 _amount, address _to) external onlyOwner {
    ...
    user.rewardDebt = user.amount.mul(accERC20PerShare).div(1e36);
    depositToken.transfer(address(_to), _amount);
  ...
  function _rewardUser(address _address) internal {
    UserInfo storage user = userInfo[_address];
    if (user.amount > 0) {
      uint256 pendingAmount = user.amount.mul(accERC20PerShare).div(1e36).sub(
        user.rewardDebt
      );
      ...
      if (hasClaimRewardsFee() && claimRewardsFee() > 0) {
        ...
        rewards -= fee;
        emit CollectFee(_address, feeReceiver(), fee);
      }

      // Check and charge early withdraw fee
      if (
        lockPeriod > 0 && // lock period is set
        earlyWithdrawFee > 0 && // early withdraw fee is set
        pendingAmount > 0 && // there is pending rewards
        (user.depositedAt + lockPeriod) > block.number // user is in lock period
      ) {
        uint256 fee = (rewards * earlyWithdrawFee) / 10**4;
        totalFee += fee;
        erc20Transfer(creator, fee);
        rewards -= fee;
        emit CollectFee(_address, creator, fee);
      }

      // Transfer rewards to user
      erc20Transfer(_address, rewards);
      emit RewardUser(_address, rewards, totalFee);
    }
  }
  ...
  function emergencyWithdraw(address _address) public onlyOwner {
    ...
    require(
      user.amount > 0,
      "Farm.emergencyWithdraw: can't withdraw from user with no tokens deposited"
    );

    depositToken.transfer(_address, user.amount);
    emit EmergencyWithdraw(_address, user.amount);
...
contract FarmsFeeManager is Initializable, OwnableUpgradeable {
  /**
   * Storage
   */
