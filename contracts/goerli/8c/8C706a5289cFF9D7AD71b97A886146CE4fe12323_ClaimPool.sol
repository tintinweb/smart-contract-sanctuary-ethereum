...
contract ClaimPool is IClaimPool, Adminable, ERC165Upgradeable, ReentrancyGuardUpgradeable {
    ...
    event Removed(address indexed collectionAddress);
    event Received(address caller, uint amount);
    event UpdatedBudget(address indexed collectionAddress, uint256 budget);
     ...
     */
    function updateBudget(address _collectionAddress, uint256 _budget) external onlyProject {
        collectionClaimPool[_collectionAddress] = _budget;

        emit UpdatedBudget(_collectionAddress, _budget);
    }

    /**
     * @notice Remove collection
     * @dev    Only admin can call this function
     * @param _collectionAddress address of collection
     * 
     * emit {Removed} events
     */
    function remove(address _collectionAddress) external nonReentrant onlyProject notZeroAddress(_collectionAddress) {
        uint256 totalAmount = collectionClaimPool[_collectionAddress];
        delete collectionClaimPool[_collectionAddress];
        if (totalAmount > 0) {
            TransferHelper._transferToken(paymentToken, totalAmount, address(this), projectOwner);
        }

        emit Removed(_collectionAddress);
    }
    ...
    function withdrawCollection(
        ...
        address _to,
        uint256 _amount
    ) external nonReentrant notZeroAddress(_collectionAddress) notZeroAddress(_to) notZeroAmount(_amount) {
        require(project == _msgSender() || IProject(project).taskManager() == _msgSender(), "Caller is not permitted");
        require(collectionClaimPool[_collectionAddress] >= _amount, "Amount exceeds balance");
    ...
    function withdraw() external nonReentrant {
        require(_msgSender() == project || _msgSender() == owner(), "Caller is not project or owner");
    ...
    function getReward(address _collectionAddress) external view returns (uint256) {
        return collectionClaimPool[_collectionAddress];
    }
}
