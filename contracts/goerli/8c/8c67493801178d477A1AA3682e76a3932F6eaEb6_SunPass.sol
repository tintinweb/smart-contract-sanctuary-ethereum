 ...
 */
abstract contract ERC721VQueryable is ERC721V, IERC721AQueryable {
    /**
...
contract SunPass is ERC721VQueryable, EIP712, Ownable {
    using ECDSA for bytes32;
    using Address for address;
    /*------------------------------------------------------------*/
    /*------------------------EVENT LIST--------------------------*/
    /*------------------------------------------------------------*/
    event AddSaleInformation(
        string saleType,
        uint256 cost,
        uint256 maxSupply,
        uint256 maximumMintCount,
        uint256 duringTime
    );
    event StartSale(string indexed saleType, uint256 startTime);
    event LockUp();
    event Reveal();
    ...
    uint24 public startLockUpTime;
    // mapping(address => bool) private _isLockList;
    ...
    struct SaleInformation {
        ...
        uint256 startTime;
        uint256 duringTime;
        uint256 beforeTokenAmounts;
    }

    mapping(bytes32 => SaleInformation) public saleInformations;
    ...
    bytes32 public constant _DOMAIN_HASH =
        0x8b73c3c69bb8fe3d512ecc4cf759cc79239f7b179b0ffacaa9a75d522b39400f;
    /// @dev value is equal to keccak256("Sale(string saleType,address to,uint256 amounts)")
    bytes32 public constant _SALE_TYPEHASH =
        0x1d83688808bc549fc5c5f4355cd91cd5abff92e70bc9627df16899afc20991e7;

    constructor() ERC721V("SunPass", "SUNPASS") EIP712("SunPass", "1") {}

    /*------------------------------------------------------------*/
    /*------------------------NFT Functions-----------------------*/
    /*------------------------------------------------------------*/
    /**
     * Owner Function List For NFT
     */
    function setLockUpTime(uint24 lockTime) external onlyOwner {
        startLockUpTime = lockTime;
    }
    ...
    function tokenURI(
        uint256 tokenId
    ) public view override(ERC721V, IERC721A) returns (string memory) {
        if (!_exists(tokenId)) revert URIQueryForNonexistentToken();
        ...
        return
            bytes(baseURI).length != 0
                ...
                : string(
                    "https://pass-test.s3.ap-northeast-2.amazonaws.com/0.json"
                );
    }
    ...
    function setSaleInformation(
        ...
        require(
            keccak256(abi.encodePacked(saleType)) ==
                keccak256(abi.encodePacked("PRIVATE")) ||
                keccak256(abi.encodePacked(saleType)) ==
                keccak256(abi.encodePacked("ALLOWLIST")) ||
                keccak256(abi.encodePacked(saleType)) ==
                keccak256(abi.encodePacked("PUBLIC")),
            "SET INFO: Invalid sale type"
        );
        require(
            maxSupply + totalSupply() <= MAX_SUPPLY - TEAM_SUPPLY,
            "SET INFO: Exceed Max Supply"
        );
    ...
    function startSale(string memory saleType) external onlyOwner {
        require(
            keccak256(abi.encodePacked(saleType)) ==
                keccak256(abi.encodePacked("PRIVATE")) ||
                keccak256(abi.encodePacked(saleType)) ==
                ...
                keccak256(abi.encodePacked(saleType)) ==
                keccak256(abi.encodePacked("PUBLIC")),
            "START SALE: Invalid sale type"
        );
        bytes32 typeName = keccak256(abi.encodePacked(saleType));
        saleInformations[typeName].startTime = block.timestamp;

        emit StartSale(saleType, saleInformations[typeName].startTime);
    }
    ...
    function sale(
        ...
        bytes32 saleTypeByte32 = keccak256(abi.encodePacked(saleType));
        require(
            saleTypeByte32 == keccak256(abi.encodePacked("PRIVATE")) ||
                saleTypeByte32 == keccak256(abi.encodePacked("ALLOWLIST")) ||
                saleTypeByte32 == keccak256(abi.encodePacked("PUBLIC")),
            "SALE: Invalid Sale Type"
        );

        SaleInformation memory sales = saleInformations[saleTypeByte32];

        require(sales.startTime > 0, "SALE: Not Start Private Sale");
        require(
            sales.startTime + sales.duringTime >= block.timestamp,
            "SALE: End Sale"
        );
        require(
            balanceOf(msg.sender) + amounts <= sales.maximumMintCount,
            "SALE: Exceed Maximum Mint Amounts"
        );
        require(
            totalSupply() - sales.beforeTokenAmounts + amounts <=
                sales.maxSupply,
            "SALE: Exceed Max Supply"
        );
        if (
            saleTypeByte32 == keccak256(abi.encodePacked("PRIVATE")) ||
            saleTypeByte32 == keccak256(abi.encodePacked("ALLOWLIST"))
            ...
            require(
                checkSignature(saleType, msg.sender, amounts, signature),
                "SALE: Invalid Signer"
            );
        }
        require(msg.value == sales.cost * amounts, "SALE: Not Enough Cost");

        _mint(msg.sender, amounts);

        if (saleTypeByte32 == keccak256(abi.encodePacked("PRIVATE"))) {
            _setExtraDataAt(_nextTokenId(), startLockUpTime);
            emit LockUp();
        }
    }
    ...
    /*------------------------------------------------------------*/
    /**
     * Owner Function List For Verify
     */
    function setSigner(address newSigner) external onlyOwner {
        require(!newSigner.isContract(), "Signer can be only EOA");
        signer = newSigner;
    }

    /**
     * @notice Check Signature
     * Sale(string saleType,address to,uint256 amounts)
     */
    ...
    function _beforeTokenTransfers(
        ...
        require(
            _getExtraDataAt(startTokenId) * 3600 <= block.timestamp,
            "Token is locked"
        );
