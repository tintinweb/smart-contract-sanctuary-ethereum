...
import "./Global/ClubrareMarketplaceGlobal.sol";
 ...
 */
contract ClubrareMarketplace is Initializable, ClubrareMarketplaceGlobal, IMarketplacePayment {
    ...
    function updateEscrow(address _escrowAddress) external onlyOwner {
        escrow = IEscrow(_escrowAddress);
    }

    function updateTradeAddress(address _tradeAddress) external onlyOwner {
    ...
    function buy(
        ...
        (uint256 _tokenId, uint256 earning) = ITrade(tradeAddress).buy{ value: msg.value }(
            contractAddress,
            tokenId,
            amount,
            order,
        ...
        if (order.orderType != Type.Escrow) {
            emit Buy(
                ...
                block.timestamp,
                order.paymentToken,
                0, //escrow id : no need in this as we creating seperate event for that
                order.objId
            );
        }
    }
    ...
    {
        (uint256 _tokenId, uint256 amt) = ITrade(tradeAddress).acceptOffer(order, bid, buyer, _amount, msg.sender);
        if (order.orderType != Type.Escrow) {
            string calldata _objId = order.objId;
    ...
    function claim(Order calldata order) external whenNotPaused nonReentrant isNotBlacklisted(msg.sender) {
        ...
        if (_isLazyMint(_tokenId, _order.contractAddress)) {
            ...
            address to = _order.orderType == Type.Escrow ? address(this) : auction.highestBidder;
            _tokenId = isAdmin
                ? _adminMint(_order.contractAddress, _order.uri, _order.royaltyFee)
                : nftContract.safeMint(to, _order.uri, _order.royaltyReceiver, _order.royaltyFee);
        } else {
        ...
        if (_order.orderType == Type.Escrow) {
            ...
            require(royaltyAmt < auction.currentBid, "invalid royalty fee");
            ...
            _settlement(nftContract, _tokenId, auction.currentBid, auction.highestBidder, _order, true, msg.sender);
            emit Claimed(
                _order.contractAddress,
                _tokenId,
    ...
    function settlement(
        ...
        uint256 sellerEarning = _settlement(nftContract, _tokenId, amount, taker, _order, isClaim, sender);
        return sellerEarning;
    }

    function chargeAndSplit(
        uint256 _amount,
        address user,
        address _erc20Token,
        ...
        require(address(escrow) == msg.sender, "Invalid Caller");
        uint256 sellerEarning = _chargeAndSplit(_amount, user, _erc20Token, royaltyValue, royaltyReceiver, isClaim);
        return sellerEarning;
    }
    ...
    function disburse(
        ...
        require(address(escrow) == msg.sender, "Invalid Caller");
        INFT token = INFT(nftAddress);
        token.safeTransferFrom(address(this), nftTo, nftId);
        if (isToken) {
            IERC20Upgradeable(erc20Address).safeTransfer(tokenTo, amount);
        } else {
            payable(tokenTo).transfer(amount);
        }
        return true;
    }
    ...
    function adminMint(
        ...
        require(msg.sender == tradeAddress, "Invalid Caller");
        tokenId = _adminMint(contractAddress, uri, royality);
    }
...
contract ClubrareMarketplaceGlobal is
    ...
    IClubrareMarketplace
{
    ...
    function getRoyalties(
        ...
        address contractAddress,
        address _sender
    ) external view returns (address creator, uint256 royalty) {
        (creator, royalty) = _getRoyalties(nft, tokenId, amount, contractAddress, _sender);
    }
...
import "./IClubrareMarketPlace.sol";

interface ITrade is IClubrareMarketplace {
    function buy(
        address contractAddress,
        uint256 tokenId,
        uint256 amount,
...
interface IMarketplacePayment is IClubrareMarketplace {
    function tokenTransfer(address erc20Address, address from, address to, uint256 amount) external;
...
interface IClubrareMarketplace {
    ...
    event Delivered(uint256 orderId, uint256 updatedTime);

    event OrderReturned(uint256 orderId, uint256 updatedTime);
}
...
interface IEscrow is IClubrareMarketplace {
