...
contract CyptoPunks3dTest is ERC721, Pausable, Ownable, ERC721Burnable {
    ...
    // constructor(address _ExistingERC1155, address usdt, address usdc, address dai, address busd,uint _currentEthPrice) ERC721("Crypto3d", "C3d") {
    constructor() ERC721("CyptoPunks3d", "CP3d") {
        ...
        // ExistingERC1155 = _ExistingERC1155;
        // CryptoCurrencies[0]=usdt;
        // CryptoCurrencies[1]=usdc;
        // CryptoCurrencies[2]=dai;
        // CryptoCurrencies[3]=busd;
        validators[msg.sender] = true;
        EtherPrice = 500000; // current price of ethereum upto 2 decimals.
        // EtherPrice= _currentEthPrice; // current price of ethereum upto 2 decimals.
        // royaltyFeesInBips = 500;  // 5.0% of the the marketprice will be send to royalityAddress
        // royaltyAddress = owner();  // address to which royality is send.
    }
    ...
    function updateEtherPrice(uint256 price) public {
        require(
    ...
    function pauseUnpauseNFTsTransfer(uint256 _round, bool flip)
        public
        onlyOwner
    {
    ...
    function updatePriceCard(uint256 round, uint256[] memory updatedValues)
        public
    {
        require(
            validators[msg.sender],
    ...
    function AlterPayment(uint256 token) public onlyOwner {
        paymentPermitted[token] = !paymentPermitted[token];
    }

    function batchMint(uint256 _quantity, address _to)
        public
        payable
        whenNotPaused
    {
        ...
        // NOTE: EtherPrice is store upto two decimals i.e. 137890
        if (msg.sender != owner()) {
            if (_quantity == 1) {
                require(
                    msg.value >=
                        ((priceCardInDollar[Round][0] * (10**20)) * _quantity) /
                            EtherPrice,
                    "Not enough ethers"
                );
            } else if (_quantity == 2) {
                require(
                    msg.value >=
                        ((priceCardInDollar[Round][1] * (10**20)) * _quantity) /
                            EtherPrice,
                    "Not enough ethers"
                );
            } else if (_quantity == 3) {
                require(
                    msg.value >=
                        ((priceCardInDollar[Round][2] * (10**20)) * _quantity) /
                            EtherPrice,
                    "Not enough ethers"
                );
            } else if (_quantity == 4) {
                require(
                    msg.value >=
                        ((priceCardInDollar[Round][3] * (10**20)) * _quantity) /
                            EtherPrice,
                    "Not enough ethers"
                );
            } else if (_quantity == 5) {
                require(
                    msg.value >=
                        ((priceCardInDollar[Round][4] * (10**20)) * _quantity) /
                            EtherPrice,
                    "Not enough ethers"
                );
                ...
                require(
                    msg.value >=
                        ((priceCardInDollar[Round][5] * (10**20)) * _quantity) /
                            EtherPrice,
                    "Not enough ethers"
                );
            }
    ...
    function getPriceForARound(
        ...
        for (uint256 i = 0; i < 6; i++) {
            if (isEther) {
                if (i == 5) {
                    uint256 amountinWei = (priceCardInDollar[round][i] *
                        10 *
                        (10**20)) / EtherPrice;
                ...
                if (i == 5) {
                    uint256 amount = (priceCardInDollar[round][i] *
                        10 *
                        (10**decimal));
                    prices[i] = amount;
                } else {
    ...
    function batchMintUsingCryptoCurrency(
        uint8 token,
        uint256 _quantity,
        address _to
        ...
        require(uint256(currentState) == Round, "Please enter correct round");
        require(
            paymentPermitted[token],
            "Payment Stopped for this crypto currency"
        );
        require(
            (_gloablId.current() + _quantity) <= roundCap[Round],
            "quantity exceeded the limit for this round"
        ...
        if (msg.sender != owner()) {
            if (_quantity == 1) {
                uint256 amount = (priceCardInDollar[Round][0] * (10**decimal)) *
                    _quantity;
                require(
                    ERC20(CryptoCurrencies[token]).balanceOf(msg.sender) >=
                        amount,
                    "Not enough Tokens"
                );
                ERC20(CryptoCurrencies[token]).transferFrom(
                    msg.sender,
                    address(this),
                    amount
                );
            } else if (_quantity == 2) {
                uint256 amount = (priceCardInDollar[Round][1] * (10**decimal)) *
                    _quantity;
                require(
                    ERC20(CryptoCurrencies[token]).balanceOf(msg.sender) >=
                        amount,
                ...
                uint256 amount = (priceCardInDollar[Round][2] * (10**decimal)) *
                    _quantity;
                require(
                    ERC20(CryptoCurrencies[token]).balanceOf(msg.sender) >=
                        amount,
                ...
                uint256 amount = (priceCardInDollar[Round][3] * (10**decimal)) *
                    _quantity;
                require(
                    ERC20(CryptoCurrencies[token]).balanceOf(msg.sender) >=
                        amount,
                ...
                uint256 amount = (priceCardInDollar[Round][4] * (10**decimal)) *
                    _quantity;
                require(
                    ERC20(CryptoCurrencies[token]).balanceOf(msg.sender) >=
                        amount,
                ...
                uint256 amount = (priceCardInDollar[Round][5] * (10**decimal)) *
                    _quantity;
                require(
                    ERC20(CryptoCurrencies[token]).balanceOf(msg.sender) >=
                        amount,
    ...
    function reveal(uint256 _round) public onlyOwner {
    ...
    function unpause(bool newRound, uint256 _EtherPrice) public onlyOwner {
    ...
    function withdrawEthers(address to, uint256 amount) public onlyOwner {
    ...
    // [500, 850, 1350, 2100, 3000, 4000, 5000, 6000, 7000, 8000, 9000];

    function _beforeTokenTransfer(
        address from,
        address to,
        uint256 tokenId // whenNotPaused  // or not ???
    ) internal override {
        if (
            from != address(0) &&
            !stopTransfer[0] &&
            (tokenId >= 0 && tokenId <= 499)
        ) {
    ...
    // function updatePriceForCryptoCurrencyForaRoundInDollars(uint256 token,  uint256 round, uint256[] memory updatedValues) public {
    //     require(validators[msg.sender],"Only validator can call this function");
    //     require(round >= 1 && round <= 10);
    //     require(updatedValues.length==6,"Invalid updatedValues array size");
    //     for(uint i=0; i<updatedValues.length; i++){
    //         amountInCryptoCurrency[token][round][i] = updatedValues[i];
    //     }
    // }
