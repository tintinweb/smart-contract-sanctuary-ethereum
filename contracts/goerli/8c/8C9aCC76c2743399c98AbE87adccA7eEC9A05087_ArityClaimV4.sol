...
contract ArityClaimV4 {
    ...
    uint64 private constant MIN_SET_BALANCE = 40000000000000000; // 0.1 ETH
    uint256 private constant BALANCEOF_GOLD = 3; // esto es solo para pruebas
    uint256 private constant BALANCEOF_SILVER = 5; // esto es solo para pruebas
    uint256 private constant FIRST_DROP_DATE = 1682812800;
    ...
    function calcClaim(bool isToPay, address _sender) public view returns(uint256){
        ...
        slot.goldTokens = getGoldTokens(_sender); // esro debe ser reemplazado por la funcion real desde el contrato de arity
        slot.silverTokens = getSilverTokens(_sender); // esro debe ser reemplazado por la funcion real desde el contrato de arity
        ...
        if (getIsAllyBoost() > 0) {
            // esro debe ser reemplazado por la funcion real que determina si tiene Ally Boost
        ...
        //return slot.totalToClaim / 10000000;
        return slot.totalToClaim * 100000000000;
    }
    ...
    function claim() public payable {
        ...
        slot.goldTokens = getGoldTokens(msg.sender); // esro debe ser reemplazado por la funcion real desde el contrato de arity
        slot.silverTokens = getSilverTokens(msg.sender); // esro debe ser reemplazado por la funcion real desde el contrato de arity
        ...
        uint256 valueToPay = (calcClaim(true, msg.sender))/100000;
    ...
    function retrieveBalance() public onlyOwner {
        (bool success, ) = msg.sender.call{value: address(this).balance}(""); /*address(this).balance*/
        if (!success) {
    ...
    function setBalanceClaim() public payable onlyOwner {
        if (msg.value < MIN_SET_BALANCE) {
            revert Arity__NotEnoughValueEntered();
        }
    }
    ...
    function balanceOfGold(address _address) public view returns (uint256) {
        bool found = false;
        for (uint i = 0; i < s_nftOwner.length; i++) {
            if (s_nftOwner[i] == _address) {
                found = true;
                break;
            }
        }
    ...
    function balanceOfSilver(address _address) public view returns (uint256) {
        bool found = false;
        for (uint i = 0; i < s_nftOwner.length; i++) {
            if (s_nftOwner[i] == _address) {
                found = true;
    ...
    function getIsAllyBoost() public pure returns (uint256) {
        return 0;
    }
