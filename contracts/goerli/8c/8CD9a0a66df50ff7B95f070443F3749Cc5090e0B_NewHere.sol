    ...
    Ownable
{
    ...
    */
     ...
     */
    function mintNormal(uint256 quantity) external payable {
        require(
            isPublicSaleActive,
            "Public sale must be active"
        );
        // require(
        //     need to limit how much normal users can buy?
        // );
        require(
     ...
     * @param quantity Quantity to mint.
     */
    function mintAllowlist(bytes32[] calldata proof, int256 quantity) external {
        require(
     ...
     */
    function setAllowlist(address[] calldata addresses, int256[] calldata quantities) external onlyOwner {
        require(addresses.length == quantities.length, "Addresses and quantities lengths do not match up");
        for (uint256 i=0; i < addresses.length; i++) {
            allowlistQuantity[addresses[i]] = (quantities[i] - 1);
        }
    }
