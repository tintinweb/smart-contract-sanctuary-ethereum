...
import "../../governance/DAOVoting.sol";
...
contract VotingFactory is IVotingFactory, AbstractPoolFactory {
    ...
    string public constant VOTING_REGISTRY_DEP = "VOTING_REGISTRY";

    PermissionManager internal _permissionManager;

    VotingRegistry internal _votingRegistry;
    ...
    function setDependencies(address registryAddress_, bytes calldata data_) public override {
        ...
        _permissionManager = PermissionManager(registry_.getPermissionManager());
        _votingRegistry = VotingRegistry(registry_.getContract(VOTING_REGISTRY_DEP));
    }
    ...
    function deployDAOVoting(
        ...
        string memory votingType_ = _votingRegistry.VOTING_NAME();

        address votingProxy_ = _deploy(address(_votingRegistry), votingType_);

        string memory votingResource_ = _getVotingResource(votingType_, votingProxy_);

        DAOVoting(votingProxy_).__DAOVoting_init(params_, votingResource_);

        _register(address(_votingRegistry), votingType_, votingProxy_);
        _injectDependencies(address(_votingRegistry), votingProxy_);
    ...
    function _getVotingResource(
        string memory votingType_,
        address votingProxy_
    ) internal pure returns (string memory) {
        return string.concat(votingType_, ":", uint256(uint160(votingProxy_)).toHexString(20));
    }

    function _requirePermission(string memory permission_) internal view {
        require(
            _permissionManager.hasPermission(msg.sender, VOTING_FACTORY_RESOURCE, permission_),
            "VotingFactory: access denied"
        );
    }
}
...
contract DAOVoting is IDAOVoting, Initializable, AbstractDependant {
    using ParameterCodec for Parameter;
    ...
    DAOVotingKeys public votingKeys;

    uint256 public proposalCount;

    DAOMemberStorage internal _daoMemberStorage;
    DAOParameterStorage internal _daoParameterStorage;
    PermissionManager internal _permissionManager;

    mapping(uint256 => DAOProposal) public proposals;
    mapping(uint256 => mapping(address => bool)) public hasUserVoted;
    mapping(uint256 => mapping(address => bool)) public hasExpertVetoed;

    modifier onlyCreatePermission(address targetContract_) {
        _requirePermission(targetContract_, CREATE_VOTING_PERMISSION, false);
        _;
    }

    modifier onlyVotePermission(address targetContract_) {
        _requirePermission(targetContract_, VOTE_PERMISSION, false);
        _;
    }

    modifier onlyVetoPermission(address targetContract_) {
        _requirePermission(targetContract_, VETO_PERMISSION, true);
        _;
    }
    ...
    function __DAOVoting_init(
        ...
        votingType = params_.votingType;
        votingKeys = params_.votingKeys;
    ...
    function setDependencies(
        ...
        _permissionManager = PermissionManager(registry_.getPermissionManager());
        _daoParameterStorage = DAOParameterStorage(registry_.getDAOParameterStorage(targetPanel));
        _daoMemberStorage = DAOMemberStorage(registry_.getDAOMemberStorage(targetPanel));
    }
    ...
    function createProposal(
    ...
    ) external onlyCreatePermission(target_) returns (uint256) {
        ...
        uint256 endDate_ = block.timestamp +
            _daoParameterStorage.getDAOParameter(votingKeys.votingPeriodKey).decodeUint();

        uint256 vetoEndDate_ = endDate_ +
            _daoParameterStorage.getDAOParameter(votingKeys.vetoPeriodKey).decodeUint();

        newProposal.params.requiredQuorum = _daoParameterStorage
            .getDAOParameter(votingKeys.requiredQuorumKey)
            .decodeUint();

        newProposal.params.votingEndTime = endDate_;

        if (_getExpertsCount() == 0) {
            newProposal.params.vetoEndTime = 0;
        } else {
            newProposal.params.vetoEndTime = vetoEndDate_;
        }

        newProposal.params.proposalExecutionP = _daoParameterStorage
            .getDAOParameter(votingKeys.proposalExecutionPKey)
            .decodeUint();
    ...
    function voteFor(
        uint256 proposalId_
    ) external override onlyVotePermission(proposals[proposalId_].target) {
    ...
    function veto(
        uint256 proposalId_
    ) external override onlyVetoPermission(proposals[proposalId_].target) {
        require(!hasExpertVetoed[proposalId_][msg.sender], "DAOVoting: expert has already vetoed");
        require(
            getProposalStatus(proposalId_) == ProposalStatus.ACCEPTED,
            "DAOVoting: proposal must be accepted to be vetoed"
        );

        hasExpertVetoed[proposalId_][msg.sender] = true;

        ++proposals[proposalId_].counters.vetoesCount;
    }

    function executeProposal(uint256 proposalId_) external override {
        DAOProposal storage proposal_ = proposals[proposalId_];

        require(
            getProposalStatus(proposalId_) == ProposalStatus.PASSED,
            "DAOVoting: proposal must be passed to be executed"
        );

        proposal_.executed = true;

        (bool success_, ) = address(proposal_.target).call(proposal_.callData);
        require(success_, "DAOVoting: proposal execution failed");
    }
    ...
    function getProposalStatus(uint256 proposalId_) public view returns (ProposalStatus) {
        ...
        if (proposal_.params.votingEndTime == 0) {
        ...
        uint256 expertsCount_ = _getExpertsCount();
        uint256 actualQuorum_ = _calculatePercentage(votesCount_, expertsCount_);

        if (actualQuorum_ < proposal_.params.requiredQuorum) {
            return ProposalStatus.REJECTED;
        }

        if (_getVetoesPercentageNumber(proposalId_) > PRECISION / 2) {
            return ProposalStatus.REJECTED;
        }

        if (block.timestamp < proposal_.params.vetoEndTime) {
            return ProposalStatus.ACCEPTED;
        }

        if (block.timestamp > proposal_.params.vetoEndTime + proposal_.params.proposalExecutionP) {
    ...
    function _vote(uint256 proposalId_, VotingOption votingOption_) private {
        require(
            getProposalStatus(proposalId_) == ProposalStatus.PENDING,
            "DAOVoting: proposal must be pending to be voted"
        );

        require(!hasUserVoted[proposalId_][msg.sender], "DAOVoting: user has already voted");

        hasUserVoted[proposalId_][msg.sender] = true;

        if (votingOption_ == VotingOption.FOR) {
            proposals[proposalId_].counters.votedFor += ITERC20(votingToken).balanceOf(msg.sender);
        } else {
            proposals[proposalId_].counters.votedAgainst += ITERC20(votingToken).balanceOf(
                msg.sender
            );
        }
    }

    function _getVetoesPercentageNumber(uint256 proposalId_) private view returns (uint256) {
        return
            _calculatePercentage(proposals[proposalId_].counters.vetoesCount, _getExpertsCount());
    }

    function _getExpertsCount() private view returns (uint256) {
        return _daoMemberStorage.getMembers().length;
    }

    function _requirePermission(
        address targetContract_,
        string memory permission_,
        bool restrictedOnly_
    ) internal view {
        require(
            IDAORestriction(targetContract_).checkPermission(
                msg.sender,
                permission_,
                restrictedOnly_
            ),
            "DAOVoting: access denied"
        );
    }
...
interface IVotingFactory {
    ...
    event DeployedDAOVoting(address votingAddress, IDAOVoting.ConstructorParams);
