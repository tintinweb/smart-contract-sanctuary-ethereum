...
*/	
    function uri(uint256 nfttokenid) external view virtual override returns (string memory) {
    	...
    	// Result uri changed
//        return string(abi.encodePacked(_uri, StringsUpgradeable.toHexString(nfttokenid)));

        bytes memory strBytes = bytes(StringsUpgradeable.toHexString(nfttokenid));
        uint result_len = strBytes.length;
		bytes memory result = new bytes(result_len-2);		
	    for(uint i = 2; i < result_len; i++) {
	        result[i-2] = strBytes[i];
	    }                        
        return string( abi.encodePacked(_uri, result) );
