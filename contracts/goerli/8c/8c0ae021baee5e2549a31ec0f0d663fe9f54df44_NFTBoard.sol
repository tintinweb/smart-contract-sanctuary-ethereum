...
contract NFTBoard is ERC721ACommon, DefaultOperatorFilterer {
    ...
    uint256 public publicSaleResAmount = 800;
    ...
    /// Point Multiplier
    uint256 public pointsMultiplier = 1;
    ...
    bool public allowPointsTransfer = true;
    ...
    ) ERC721ACommon("NFTBoard", "BOARD", royaltiesReceiver, 500) {
    ...
    function boardToken(uint256 _boardableTokenId) external {
        require(boardingState == true, "Boarding is not active");
        require(
            msg.sender == ownerOf(_boardableTokenId),
            "Can't board a token you don't own!"
        );
        BoardedToken memory _token = checkTokenBoardStatus[_boardableTokenId];
        require(!_token.boarded, "Token already boarded!");
        _token.timeBoarded = uint48(block.timestamp);
        _token.boarded = true;
        _token.timeLastBoarded = uint48(block.timestamp);
    ...
    function deboardToken(uint256 _boardableTokenId) external {
        require(
            msg.sender == ownerOf(_boardableTokenId) || msg.sender == owner(),
            "You are not the owner of this token!"
        );
        BoardedToken memory _token = checkTokenBoardStatus[_boardableTokenId];
        require(_token.boarded, "Token has not boarded!");
    ...
    function checkActiveBoardingPoints(
        ...
        if (checkTokenBoardStatus[_boardedTokenId].boarded) {
            return ((block.timestamp -
                checkTokenBoardStatus[_boardedTokenId]
                    .timeLastBoarded) / pointsMultiplier); // Time in minutes
        } else {
            return 0;
        }
    }
    ...
    function checkWalletPointsBalance(
        address _boarderAcc
    ) public view returns (uint256 _points) {
    ...
    /// @param _boarderAcc The address to query for
    function checkWalletBoardedTokens(
        address _boarderAcc
    ...
    function withdrawActiveBoardingPoints(uint256 _boardedToken) public {
        boarderAcc[msg.sender] += checkActiveBoardingPoints(_boardedToken);
        require(checkTokenBoardStatus[_boardedToken].boarded, "Token has not boarded!");
        checkTokenBoardStatus[_boardedToken].timeLastBoarded = uint48(block.timestamp);
        emit activePointsMoved(msg.sender, _boardedToken, checkActiveBoardingPoints(_boardedToken));
    }

    /// @notice Disables Boarding
    function disableBoarding() public onlyOwner {
        for(uint256 i=0; i < totalSupply(); ++i){
            ...
            checkTokenBoardStatus[i].timeLastBoarded = uint48(block.timestamp);
    ...
    function setPointMultiplier(uint256 _time) public onlyOwner {
        pointsMultiplier = _time;
    }

    /// @notice Set Transfer Points state
    /// @param _state true or false
    function setPointTransfersOn(bool _state) public onlyOwner{
    ...
    function pointsTransfer(address _to, uint256 _amount) public {
        uint256 ownerBalance = checkWalletPointsBalance(msg.sender);
        require(allowPointsTransfer == true, "Point transfers not active");
        require(
    ...
    /// @param _to Address
    /// @param _amount Amount of Points
    function pointsAdd(address[] memory _to, uint256 _amount) public delegateOrOwner {
    ...
    /// @param _to Address
    /// @param _amount Amount of Points
    function pointsSubtract(address[] memory _to, uint256 _amount) public delegateOrOwner {
        for (uint256 i=0; i < _to.length; ++i){
            boarderAcc[_to[i]] -= _amount;
    ...
    modifier delegateOrOwner() {
        require(msg.sender == owner() || msg.sender == delegateAddress, "You do not have permission to run this function!");
        _;
    }

    // =============================================================
    //                Prevent Boarded Token Transfers
    // =============================================================
    ...
    function _beforeTokenTransfers(
        ...
        if (checkTokenBoardStatus[startTokenId].boarded){
            ...
            checkTokenBoardStatus[startTokenId].boarded = false;
            checkTokenBoardStatus[startTokenId].timeBoarded = 0;
            checkTokenBoardStatus[startTokenId].timeLastBoarded = uint48(block.timestamp);
