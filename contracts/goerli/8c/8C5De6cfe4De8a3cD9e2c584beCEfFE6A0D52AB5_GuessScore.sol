...
contract GuessScore is ReentrancyGuard, Ownable {
    ...
    function arryToHash(uint256[2] memory _n) public pure returns (bytes32) {
        return keccak256(abi.encodePacked(_n[0]));
    }
    ...
    function deposit(
        ...
        // Share the Rewards
        superiorReward(msg.sender, referrer, reward);
        //
    ...
    function superiorReward(
        ...
        (_active, _superior) = _relation.getUser(_user);
        if (_active) {
            _rewardAddr = _superior;
        } else {
            _relation.bind(_user, _referrer);
            _rewardAddr = _referrer != address(0) ? _referrer : flowAddr;
        }

        require(
            token.transferFrom(_user, _rewardAddr, reward),
...
interface IRelation {
    ...
    function getUser(address account) external view returns (bool, address);
}
