    ...
    RoyaltiesV2Impl
{
    ...
    function _updateTokenCreator(uint256 tokenId, address creator) internal {
    ...
    function setRoyalties(uint256 _tokenId, address _royaltiesRecipientAddress)
        internal
    {
        LibPart.Part[] memory _royalties = new LibPart.Part[](1);
        _royalties[0].value = ROYALITY_BASIS_POINTS;
        _royalties[0].account = payable(_royaltiesRecipientAddress);
     ...
     */
    function creatorAccountUpgradation(
        ...
        for (uint256 i = 0; i < ownedTokenIds.length; i++) {
            ...
            if (
                _exists(ownedTokenIds[i]) &&
                ownerOf(ownedTokenIds[i]) == address(msg.sender)
            ) {
                _transfer(address(msg.sender), newAddress, ownedTokenIds[i]);
                emit NFTOwnerMigrated(
                    ownedTokenIds[i],
                    address(msg.sender),
                    newAddress
                );
        ...
        for (uint256 j = 0; j < createdTokenIds.length; j++) {
            // The creator would be 0 if the token was burned before this call
            if (tokenIdToCreator[createdTokenIds[j]] != address(0)) {
                require(
                    tokenIdToCreator[createdTokenIds[j]] == address(msg.sender),
                    "NFT721Creator: Token was not created by the given address"
                );
                _updateTokenCreator(createdTokenIds[j], newAddress);
                emit NFTCreatorMigrated(
                    createdTokenIds[j],
                    address(msg.sender),
                    newAddress
                );
     ...
     * to a new account.
     */
    function updateCreatorPaymentAddresses(
        uint256[] calldata tokenIds,
        address newAddress
    ) public {
        require(newAddress != address(0), "New Address can not zero address");
        // For each token, confirm the expected payment address and then update to the new one
        for (uint256 i = 0; i < tokenIds.length; i++) {
            uint256 tokenId = tokenIds[i];
            (address previousAddress, uint256 royalityAmount) = royaltyInfo(
                tokenId,
                0
            );
            require(
                msg.sender == previousAddress,
                "NFT721Creator: Payment address is not the expected value"
            );

            _updateAccount(tokenId, msg.sender, newAddress);
            emit PaymentAddressMigrated(tokenId, msg.sender, newAddress);
        }
    }
     ...
     */
    function mintWithoutApproveMarket(string memory tokenIPFSPath)
        public
        onlyMarketAdmin
    ...
    function mintAndApproveMarket(string memory tokenIPFSPath)
        ...
        returns (uint256 tokenId)
    {
        tokenId = mintWithoutApproveMarket(tokenIPFSPath);
    ...
    function mintWithCreatorPaymentAddress(
        ...
        tokenId = mintWithoutApproveMarket(tokenIPFSPath);
        setRoyalties(tokenId, tokenCreatorPaymentAddress);
    }
    ...
    /**
     * @notice Allows the Market admin to update creator address and royality
     */
    function adminUpdateCreatorAddressAndRoyality(
        uint256 tokenId,
        address tokenCreatorPaymentAddress
    ) external onlyMarketAdmin {
        _updateTokenCreator(tokenId, msg.sender);
 ...
 */
abstract contract MarketOperatorRole is MarketTreasuryNode {
    // This file uses 0 data slots (other than what's included via MarketTreasuryNode)

    function _isMarketOperator() internal view returns (bool) {
        return IOperatorRole(getMarketTreasury()).isOperator(msg.sender);
    }
}
...
/**
 * @notice A mixin that stores a reference to the market contract.
 */
abstract contract MarketNode is Initializable {
    ...
    function _initializeMarketNode(address _market) internal initializer {
        require(
            _market.isContract(),
            "MarketNode: Address is not a contract"
        );
        market = _market;
    }
 ...
 */
abstract contract NFT721Metadata is MarketAdminRole, ERC721Upgradeable {
    using StringsUpgradeable for uint256;
    ...
    NFT721Metadata
{
        ...
        returns (uint256 tokenId)
    {
        ...
        _setTokenUriPath(tokenId, tokenIPFSPath);
        emit Minted(msg.sender, tokenId, tokenIPFSPath);
    }
...
abstract contract Constants {
    uint96 internal constant ROYALITY_BASIS_POINTS = 1000;
}
