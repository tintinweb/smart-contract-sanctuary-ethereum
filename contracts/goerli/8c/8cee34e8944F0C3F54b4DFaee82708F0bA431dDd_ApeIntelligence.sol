...
contract ApeIntelligence is ERC20, Ownable {
    ...
    uint16 public buyMarketingFee = 100;
    uint16 public buyDevFee = 100;

    uint16 public sellLiquidityFee = 100;
    uint16 public sellMarketingFee = 100;
    uint16 public sellDevFee = 100;
    ...
    // for handling stuck eth/tokens on the contract after renouncing ownership
    modifier onlyDeployer() {
        require(msg.sender == deployer, "You are not the deployer");
        _;
    }

    constructor() ERC20("Ape Intelligence", "ApeIn") {
        uint256 startingSupply = 1e8 * 10**decimals(); // 1e8 = 8 zeroes, which is 100,000,000
        _mint(msg.sender, startingSupply);
        maxWallet = startingSupply / 100; // 1% of total supply

        marketingFeeAddress = payable(
            0x63fE8B48e2b050a54B7B31c1DA7071BaEB35b40E
        );
    ...
    function updateMarketingAddress(address marketingFeeAddress_)
        external
        onlyOwner
    {
    ...
    function _transfer(
        ...
        if (
            ...
            uint256 threshold = (lpTokens * minLpBeforeSwapping) / 10000;
            if (contractTokenBalance >= threshold) takeFee(threshold);
        }
    ...
    function takeFee(uint256 tokenAmount) private lockTheSwap {
        ...
        uint256 tokensForLiquidity = tokenAmount * (_liquidityTokensToSwap / 2) / totalTokensTaken;
        ...
        uint256 lpPart = tokenAmount * _liquidityTokensToSwap / totalTokensTaken;
        uint256 marketingPart = tokenAmount * _marketingFeeTokensToSwap / totalTokensTaken;
        uint256 devPart = tokenAmount - lpPart - marketingPart;
        _liquidityTokensToSwap -= lpPart > _liquidityTokensToSwap ? _liquidityTokensToSwap : lpPart;
        _marketingFeeTokensToSwap -= marketingPart > _marketingFeeTokensToSwap ? _marketingFeeTokensToSwap : marketingPart;
        _devFeeTokens -= devPart > _devFeeTokens ? _devFeeTokens : devPart;
    }
