...
import "./interfaces/IMagpieHelper.sol";
...
contract MagpieLPToken is
    ...
    Pausable
{
    address public magpieProvidersAddress;
    IMagpieListManager public magpieListManager;
    mapping(uint256 => MagpieLPTokenInfo) public tokenMetadata;
    mapping(address => IMagpieHelper) public magpieHelpers;

    event MagpieProvidersUpdated(address indexed lpm);
    event MagpieListManagerUpdated(address indexed manager);
    event MagpieHelperUpdated(address indexed tokenAddress, IMagpieHelper indexed magpieHelper);
    ...
    modifier onlyMagpiePools() {
        require(_msgSender() == magpieProvidersAddress, "ERR_UNAUTHORIZED");
        _;
    }

    function setMagpieHelper(address _tokenAddress, IMagpieHelper _magpieHelper) public onlyOwner {
        require(_magpieHelper != IMagpieHelper(address(0)), "ERR_INVALID_SVG_HELPER");
        require(_tokenAddress != address(0), "ERR_INVALID_TOKEN_ADDRESS");
        magpieHelpers[_tokenAddress] = _magpieHelper;
        emit MagpieHelperUpdated(_tokenAddress, _magpieHelper);
    }

    function setMagpieProviders(address _liquidityProviders) external onlyOwner {
        require(_liquidityProviders != address(0), "ERR_INVALID_LIQUIDITY_PROVIDERS");
        magpieProvidersAddress = _liquidityProviders;
        emit MagpieProvidersUpdated(_liquidityProviders);
    }

    function setMagpieListManager(address _magpieListManager) external onlyOwner {
        require(_magpieListManager != address(0), "ERR_INVALID_LIST_PERIOD_MANAGER");
        magpieListManager = IMagpieListManager(_magpieListManager);
        emit MagpieListManagerUpdated(_magpieListManager);
    }
    ...
    function mint(address _to) external onlyMagpiePools whenNotPaused nonReentrant returns (uint256) {
        uint256 tokenId = totalSupply() + 1;
        _safeMint(_to, tokenId);
        return tokenId;
    }

    function updateTokenInfo(uint256 _tokenId, MagpieLPTokenInfo calldata _lpTokenMetadata)
        external
        onlyMagpiePools
        whenNotPaused
    {
        ...
        returns (string memory)
    {
        ...
        require(magpieHelpers[tokenAddress] != IMagpieHelper(address(0)), "ERR__SVG_HELPER_NOT_REGISTERED");

        IMagpieHelper magpieHelper = IMagpieHelper(magpieHelpers[tokenAddress]);

        string memory svgData = magpieHelper.getTokenDetails(
            tokenId,
            tokenMetadata[tokenId].suppliedLiquidity,
            IMagpieProviders(magpieProvidersAddress).totalReserve(tokenAddress)
        );

        string memory description = magpieHelper.getDescription(
            tokenMetadata[tokenId].suppliedLiquidity,
            IMagpieProviders(magpieProvidersAddress).totalReserve(tokenAddress)
        );

        string memory attributes = magpieHelper.getAttributes(
            tokenMetadata[tokenId].suppliedLiquidity,
            IMagpieProviders(magpieProvidersAddress).totalReserve(tokenAddress)
        );
    ...
    function _beforeTokenTransfer(
        ...
        // Only call list manager for NFT Transfers, not mint and burns
        if (from != address(0) && to != address(0)) {
            magpieListManager.beforeTransferringLiquidity(
                from,
                to,
...
interface IMagpieHelper {
    function getBackgroundUrl() external view returns (string memory);
