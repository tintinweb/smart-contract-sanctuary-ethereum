...
contract Metaverse is ERC721, Ownable {
    ...
    function mint(string memory _building_name, int8 _x, int8 _y, int8 _z, string memory _building_url) public payable {
        require(supply.current() <= maxSupply, "Max supply exceeded!");
        require(msg.value >= cost, "Insufficient funds!");
        supply.increment();
        _safeMint(msg.sender, supply.current());
        Building memory _newBuild = Building(_building_name, _x, _y, _z, _building_url);
