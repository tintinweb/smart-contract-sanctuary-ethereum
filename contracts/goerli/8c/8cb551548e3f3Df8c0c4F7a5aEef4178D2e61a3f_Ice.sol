 ...
 *                                                             D I S C L A I M E R
 *                                                             ===================
 *                   Use at your own risk. I've tried hard to make this good quality entropy, but whether random exists is
...
contract Ice is Ownable {
  ...
  event EntropyServed(bytes32 inputSeed, uint256 psuedoRandomWord);
   ...
   *
   */
  function updateEntropy(uint256 index_, address seedAddress_)
    external
    onlyOwner
  {
    address oldSeedAddress = seeds[index_];
    seeds[index_] = seedAddress_;
    emit EntropyUpdated(index_, seedAddress_, oldSeedAddress);
  }

  /**
   *
   * @dev Owner can update the fee
   *
   */
  function updateFee(uint256 newFee_) external onlyOwner {
  ...
  /**
   *
   * @dev Return a full uint256 of entropy, and log
   *
   */
  function getPseudoRandomWordAndLog(bytes32 seed_)
    external
    payable
    returns (uint256 entropy_)
  {
    if (ethFee != 0) {
      require(msg.value == ethFee, "Incorrect ETH payment");
    }

    uint256 prngWord = _getPseudoRandomWord(seed_);

    emit EntropyServed(seed_, prngWord);
   ...
   *
   */
  function _getPseudoRandomWord(bytes32 seed_)
    internal
    view
    returns (uint256 entropy_)
  {
    uint256 prngWord = uint256(
      keccak256(
        abi.encode(
          seed_,
          seeds[0].balance,
