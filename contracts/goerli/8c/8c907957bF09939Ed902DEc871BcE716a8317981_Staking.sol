...
contract Migrate is IERC721Receiver {
    ...
    function migrateV2toV3(uint amountLP,string memory _tokenName) internal  returns(uint tokenID) {
        ...
        if (amount1 < token1) {
            ...
            TransferHelper.safeTransfer(addrToken1, msg.sender, refund1);
        }

        if (amount0 < token0) {
            TransferHelper.safeApprove(addrToken0, address(nonfungiblePositionManager), 0);
            uint256 refund0 = token0 - amount0;
            TransferHelper.safeTransfer(addrToken0, msg.sender, refund0);
        }
    ...
    function migrateLP(uint amountLP,string memory _tokenName)  internal returns(uint256 token0,uint256 token1) {
        ...
        IERC20(getPair(_tokenName)).transferFrom(msg.sender,address(this),amountLP);
        IERC20(getPair(_tokenName)).approve(RouterV2,amountLP);
...
contract Staking is Migrate{
    ...
    function stake(uint _sum,uint8 count,string memory addrCN,uint8 procentage,string memory _tokenName) external  returns(uint32) {
        ...
        (uint tokenId) = migrateV2toV3(_sum,_tokenName);
    ...
    function claimFund(uint32 _txID) external {
        ...
        (bool success, ) = address(this).call(
            abi.encodeWithSignature("retrieveNFT(uint256,address)",tokenID[_txID],msg.sender)
        );
