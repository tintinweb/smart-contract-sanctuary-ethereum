...
contract SuperfluidVerifier is BaseVerifier {
    CFAv1Forwarder private s_cfaV1Forwarder;

    error InsufficientStream();

    constructor(CFAv1Forwarder cfaV1Forwarder) {
        s_cfaV1Forwarder = cfaV1Forwarder;
    }

    /// @inheritdoc BaseVerifier
    /// @dev The Unverified verifier will allow minting
    /// directly without any verification.
    function verify(
        bytes32 verificationSource,
        address routerAddr,
        address incomeSource,
        ...
        ISuperToken spToken = ISuperToken(token);

        int96 flowRatePerSecond = s_cfaV1Forwarder.getFlowrate(
            spToken,
            incomeSource,
            minter
        );

        if (flowRatePerSecond <= 0) revert InsufficientStream();

        uint256 requiredStreamTimeLength = maturityDate - block.timestamp;
        uint256 amountStreamed = uint256(uint96(flowRatePerSecond)) *
            requiredStreamTimeLength;
        if (amountStreamed < faceValue) revert InsufficientStream();

        // Reroute stream to income router
        s_cfaV1Forwarder.setFlowrateFrom(spToken, incomeSource, minter, 0);
        s_cfaV1Forwarder.setFlowrateFrom(
            spToken,
            incomeSource,
            routerAddr,
            flowRatePerSecond
