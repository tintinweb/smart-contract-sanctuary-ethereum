	...
	ReentrancyGuard
{
	...
	/**
	 * @notice Mapping token ID => token information
	 */
	...
	constructor(
		...
		uint256 _maxBatch
	) ERC721(_tokenName, _symbol) ReentrancyGuard() {
		maxBatch = _maxBatch;
	...
	function _beforeTokenTransfer(
		...
		TokenStatus status = tokenInfoOf[tokenId].status;
		require(status == TokenStatus.Active || 
			status == TokenStatus.CreditAdded,
			"Invalid status"
		);
	...
	function _checkValidParams(
		...
		require(_params.receiptAddress != address(0), "Invalid address");
		require(_params.price > 0, "Invalid price");
		require(_params.volume > 0, "Invalid purchased volume");
		bytes memory _msg = abi.encodePacked(
			_to,
			_params.receiptAddress,
		...
		require(_verifySignature(_msg, _signature), "Invalid signature");
	}
	...
	function setMaxBatch(uint256 _maxBatch) external onlyOwner {
		require(_maxBatch > 0 && _maxBatch <= maxBatch, "Invalid max batch");
	...
	function setTokenURI(
		...
		bytes memory message = abi.encodePacked(
			_tokenId, _tokenURI, _nonce
        );
		require(_verifySignature(message, _signature), "Invalid signature");

		TokenInfo storage token = tokenInfoOf[_tokenId];
		string memory oldTokenURI = token.uri;
		token.uri = _tokenURI;
	...
	function setStatus(
		...
		isUsedSignature[_signature] = true;
		emit SetStatus(_tokenId, _status, oldStatus);
	}
	...
	function tokenURI(
		...
		require(_exists(_tokenId), "Nonexistent token");
		return tokenInfoOf[_tokenId].uri;
	}
	 ...
	 *  @param  _params.receiptAddress        	Payment receiver address
	 ...
	 *  @param  _params.organizationCode    	Organization code
	 *  @param  _params.projectCode        		Project code
	 *  @param  _params.monitoringCycleCode 	Monitoring cycle Code
	 *  @param  _params.typeToken         		Type of token
	 *  @param  _params.nonce         			Unique param
	 *  @param  _params.nftCode         		Organization code - Project code - Monitoring cycle code
	 *  @param  _signature                  	Signature of transaction
	 *
	...
	function mint(
		...
		if (_params.paymentToken == address(0)) {
			if (_msgSender() == _params.receiptAddress) {
				require(msg.value == 0, "Invalid value");
			} else {
				require(
					msg.value > 0 && 
					msg.value == _params.price, 
					"Invalid price"
				);
			}
		}

		_mintAndPay(_to, _params, _signature);
	}
	 ...
	 *
	 *          Name                        Meaning
	 *  @param  _tos        				List of recipient addresses
	 *  @param  _params[].receiptAddress    Payment receiver address
	...
	function mintBatch(
		...
		require(
			_params.length == _signatures.length && 
			_params.length == _tos.length, 
			"Inconsistent length"
		);
		require(_params.length > 0 && _params.length <= maxBatch, "Invalid length");

		uint256 totalNative = 0;

		for (uint256 i = 0; i < _params.length; i++) {
			if (
				_params[i].paymentToken == address(0) &&
				_msgSender() != _params[i].receiptAddress
			) {
				totalNative += _params[i].price;
			}

			_mintAndPay(_tos[i], _params[i], _signatures[i]);
		}

		require(msg.value == totalNative, "Invalid price");
	}
	...
	/**
	 *  @notice Store token information
	 *
	 * 	@dev 	Private function
	 *
	 *          Name        	Meaning
	...
	function _saveInfo(
		...
		lastId.increment();
		tokenInfoOf[lastId.current()] = TokenInfo({
			uri: _params.tokenURI,
			nftCode: _params.nftCode,
	...
	/**
	 *  @notice Store, mint and transfer money
	 *
	 * 	@dev 	Private function
	 *
	 *          Name        	Meaning
	 ...
	 *
	 *          Name        	Meaning
	 *  @param  _msg  			Message hash
	 *  @param  _signature    	Signature
	 * 
	 *  @return Check if signature is signed by verifier
	 */
	function _verifySignature(bytes memory _msg, bytes memory _signature) private view returns (bool) {
		bytes32 ethSignedMessageHash = _msg.toEthSignedMessageHash();

		return ethSignedMessageHash.recover(_signature) == verifier;
	}
}
...
struct MintParams {
	...
	address receiptAddress; 	// Local government address
	address paymentToken; 		// Payment token address
	uint96 royaltyPercent; 		// Royalty percent
	string tokenURI; 			// Token URI
	string nftCode;				// Organization code - Project code - Monitoring cycle code
	string nonce;				// Unique parameter
	uint256 price; 				// Price of token
	uint256 volume;				// Purchased volume
}
...
struct TokenInfo {
	...
	string nftCode;			// Organization code - Project code - Monitoring cycle code
	TokenStatus status;     // Status of token
	TokenType tokenType;    // Type of token
	uint256 volume;			// Purchased volume
}
