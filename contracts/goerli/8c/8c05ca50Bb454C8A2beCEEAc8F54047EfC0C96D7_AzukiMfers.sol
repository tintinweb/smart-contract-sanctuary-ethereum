...
contract AzukiMfers is ERC721A, Ownable {
    ...
    uint256 public constant maxAmountPerMint = 20;
    ...
    function availableSlotsOf(address owner) view public returns (uint256) {
        return allowlist[owner];
    }

    function allowListMint(uint256 amount) external payable {
        uint256 mintableAmount = amount;

        // check to ensure quantity is not exceeded MAX_SUPPLY
        ...
        // check to ensure quantity is not exceeded available slots
        uint256 availableSlots = allowlist[msg.sender];
        require(availableSlots > 0, "No more slot to mint!"); 
        mintableAmount = Math.min(mintableAmount, availableSlots);
        allowlist[msg.sender] -= mintableAmount;
