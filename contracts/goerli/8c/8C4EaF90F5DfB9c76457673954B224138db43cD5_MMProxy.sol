  ...
  SP1Storage
{
  ...
  /**
   * @notice Check whether a Market Maker address is on the allowlist for exchange operations.
   *
   * @param  mmAddress  The Market Maker address to check.
   *
   * @return Boolean `true` if the Market Maker address is allowed, otherwise `false`.
   */
  function isMMAddressAllowed(
    address mmAddress
  )
    external
    view
    returns (bool)
  {
    return _ALLOWED_MM_ADDRESS_[mmAddress];
  }
  ...
  ReentrancyGuardUpgradeable
{
  ...
  /**
   * @dev Modifier to ensure the MM address is allowed.
   */
  modifier onlyAllowedMMAddress(
    address mmAddress
  ) {
    require(_ALLOWED_MM_ADDRESS_[mmAddress], 'SP1Storage:  MM address is not on the allowlist');
    _;
  }
  ...
  mapping(address => bool) internal _ALLOWED_MM_ADDRESS_;
...
abstract contract SP2Exchange is SP1Balances {
    ...
    // @notice Address to bridge usdt to FXCore.
    bytes32 public MM_CONTRACT;

    // @notice Network to bridge usdt to FXCore.
    bytes32 public TARGET_IBC;

    // ============ Events ============

    event DepositedToMMContract(
        address mmAddress,
        bytes32 mmContract,
        address tokenType,
        bytes32 targetIBC,
    ...
    event UpdateWithdrewFromExchange(
        address mmAddress,
    ...
    /**
     * @notice Deposit funds to the FX-EVM.
     *
     ...
     */
    function depositToMMContract(
        uint256 quantizedAmount
    )
        external
        nonReentrant
        onlyAllowedMMAddress(msg.sender)
        returns (uint256)
    {
        ...
        emit DepositedToMMContract(msg.sender, MM_CONTRACT, address(TOKEN), TARGET_IBC, tokenAmount);
        return tokenAmount;
    }
}
  ...
  SP2Exchange
{
  ...
  /**
   * @notice Allow exchange functions to be called for a particular MM address.
   *
   *  Will revert if the Market maker address key is not whitelisted in this contract.
   *
   * @param  mmAddress  The MM address to allow.
   */
  function allowMMAddress(
    address mmAddress
  )
    external
    nonReentrant
    onlyRole(OWNER_ROLE)
  {
    require(!_ALLOWED_MM_ADDRESS_[mmAddress], 'SP2Owner: MM address already allowed');
    _ALLOWED_MM_ADDRESS_[mmAddress] = true;
    emit UpdatedMMAddress(mmAddress, true);
  }

  /**
   * @notice Remove a MM address from the allowed list.
   *
   * @param  mmAddress  The MM address to disallow.
   */
  function disallowMMAddress(
    address mmAddress
  )
    external
    nonReentrant
    onlyRole(OWNER_ROLE)
  {
    require(_ALLOWED_MM_ADDRESS_[mmAddress], 'SP2Owner: MM address already disallowed');
    _ALLOWED_MM_ADDRESS_[mmAddress] = false;
    emit UpdatedMMAddress(mmAddress, false);
  }
