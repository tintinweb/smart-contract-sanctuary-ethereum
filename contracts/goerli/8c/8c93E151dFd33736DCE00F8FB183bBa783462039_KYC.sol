...
contract KYC {
  ...
  function getPermissionedUserDetails(
    ...
    require(userCount[user] > 0, "User does not exist.");
    return registeredUser[user].getPermissionedUserDetails(getUserCount(msg.sender));
  }

  function sendKYC(address bankAddress) public {
    ...
    registeredUser[msg.sender].addKYC(bankAddress, getUserCount(bankAddress));
  }
...
contract userDetails {
  ...
  function addKYC(address bankAddress, uint bankId) public onlyCreator {
...
contract bankDetails {
  ...
  modifier onlyBank() {
    require(msg.sender == bankPublicAddress, "Only bank can call this function.");
    _;
  }
}
