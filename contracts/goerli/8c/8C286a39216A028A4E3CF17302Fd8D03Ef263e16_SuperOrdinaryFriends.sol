...
contract SuperOrdinaryFriends is ERC721Base, PaymentSplitter, AccessControl, Pausable, ReentrancyGuard, Ownable {
    uint public maxPerTx = 20;
    uint public maxAirDrop = 50;
    uint private currAirDrop = 0;

    // Signer address
    address private whitelistSignerAddr;
    ...
    function whitelistMint(uint256 amount, uint256 maxAmount, bytes memory signature) external payable whenNotPaused nonReentrant {
        uint256 supply = totalSupply();
        uint64 aux = _getAux(msg.sender);
        
        require(isPreMint, "Whitelist mint is not open");
        require(supply + amount <= MAX_SUPPLY, "exceed max supply");
        require(aux + amount <= maxAmount, "Exceed max per wallet");
        require(msg.value == whitelistPrice * amount, "Insufficient Fund");
        require(Signature.verify(maxAmount, msg.sender, signature) == whitelistSignerAddr, "Invalid signature");
        require(tx.origin == msg.sender, "invalid source");
    ...
    function whitelistClaimed(address target) external view returns (uint) {
        return _getAux(target);
    }

    function checkWhitelist(address target, uint256 maxAmount, bytes memory signature) external view returns (bool) {
        return Signature.verify(maxAmount, target, signature) == whitelistSignerAddr;
    }

    function airdrop(address wallet, uint256 amount) external onlyRole(DEFAULT_ADMIN_ROLE) {
        uint256 supply = totalSupply();
        require(supply + amount <= MAX_SUPPLY, "exceed max supply");
        require(currAirDrop + amount <= maxAirDrop, "exceed max airdrop");
        currAirDrop += amount;
        _safeMint(wallet, amount);
    }

    // Minting fee
    function setPublicPrice(uint amount) external onlyRole(DEFAULT_ADMIN_ROLE) {
        publicPrice = amount;
    }

    function setWhitelistPrice(uint amount) external onlyRole(DEFAULT_ADMIN_ROLE) {
    ...
    // Signer
    function setWhitelistSignerAddr(address signer) external onlyRole(DEFAULT_ADMIN_ROLE){
        whitelistSignerAddr = signer;
    }
    ...
    function setPause(bool pause) external onlyRole(DEFAULT_ADMIN_ROLE){
        if (pause) {
            ...
            require(whitelistSignerAddr != address(0), "Signer address is not set");
            _unpause();
        }
    }

    function setPublic(bool _isPublic) external onlyRole(DEFAULT_ADMIN_ROLE){
        isPublic = _isPublic;
    }

    function setPreMint(bool _isPreMint) external onlyRole(DEFAULT_ADMIN_ROLE){
    ...
    function setMaxAirDrop(uint amount) external onlyRole(DEFAULT_ADMIN_ROLE) {
        maxAirDrop = amount;
    }
    ...
    // Set royalty for specific token sale
...
contract ERC721Base is ERC721A, ERC2981 {
