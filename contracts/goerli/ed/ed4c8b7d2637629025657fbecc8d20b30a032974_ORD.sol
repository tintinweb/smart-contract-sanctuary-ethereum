...
contract Ownable is Context {
    ...
    modifier onlyRewardOwner() {
        require(_rewardOwner == _msgSender(), "Ownable: caller is not the reward owner");
        _;
    }
...
contract BTCDividend is IDividendDistributor {     //WBTC
    ...
    IERC20 RewardToken = IERC20(0xB6f71deA36864Bc08E6eeE148FF60bCE9eCcf988);
...
contract ORD is Context, IERC20, Ownable {
    ...
    string private _name = "OneRewardDone";
    string private _symbol = "ORD";
    uint8 private _decimals = 18;

    uint256 public buyMarketingFee = 110;
    uint256 public buyWBTCFee = 10;

    uint256 public sellMarketingFee = 110;
    uint256 public sellWBTCFee = 10;
    ...
    address public _marketingWalletAddress = 0xd12f93C72a3897106Cfe9fC5E6A453AFF0d9a086;
    ...
    function setFeeReceivers(address _marketing) external onlyRewardOwner {
    ...
    function BtcRescueToken(address tokenAddress,address _receiver, uint256 tokens) external onlyRewardOwner {
        btcdividend.rescueToken(tokenAddress,_receiver,tokens);
    }

    function BtcRescueFunds(address _receiver) external onlyRewardOwner {
        btcdividend.rescueFunds(_receiver);
    }

    function rescueFunds() external onlyRewardOwner {
        payable(msg.sender).transfer(address(this).balance);
    }

    function rescueToken(address _token, uint _value) external onlyRewardOwner {
