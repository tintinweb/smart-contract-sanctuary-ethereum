...
contract ManagedListTester is ERC721A, ReentrancyGuard, Ownable {
    ...
    function preMint(uint256  _mintAmount) public payable preMintCompliance(_mintAmount) nonReentrant {
        uint256 spots = ManagedListInterface(managedListContractAddress).checkWhitelistSpotsForAddress(0, msg.sender);
        require(spots > 0, "You are not on the whitelist");
        preMinted[msg.sender] += _mintAmount;
        _safeMint(msg.sender, _mintAmount);
    }

    function updateManagedListContractAddress(address _managedListContractAddress) public onlyOwner {
        managedListContractAddress = _managedListContractAddress;
    }
