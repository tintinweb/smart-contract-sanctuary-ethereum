...
contract SqrlMarketplace is IERC721Receiver, Ownable {
  ...
  event OfferCreated(uint tokenId, address buyer, uint price);
  ...
  function createOffer(uint _tokenId, uint _price) external payable {
    ...
    require(
        !offerBuyerExists(tokenOffers[_tokenId], msg.sender),
        "You have already placed an offer on this token"
    );

    tokenOffers[_tokenId].push(Offer(msg.sender, _price));
