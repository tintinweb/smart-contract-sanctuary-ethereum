...
contract PaidEarthBuddies is ERC721A, Ownable {
    ...
    modifier onlyWhenNotPaused {
        require(!_paused || msg.sender == owner(), "Contract currently paused");
        _;
    }
    ...
    function mint(uint256 quantity, bytes32[] memory proof) external payable onlyWhenNotPaused callerIsUser {
        ...
        if (balanceOf(msg.sender) > 0) {
            require(msg.value > priceAfterFirstMint);
        }
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        ...
        return bytes(baseURI).length > 0 ? string(abi.encodePacked(baseURI, _toString(tokenId), ".json")) : hiddenMetadataUri;
    }
    ...
    function setPriceAfterFirstMint(uint256 price) public onlyOwner {
    ...
    // TODO: Make this function (isValid()) an internal/private function
