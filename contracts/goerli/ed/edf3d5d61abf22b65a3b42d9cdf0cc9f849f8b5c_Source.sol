...
/// @title Source
/// @author https://github.com/connext/xapp-starter
/// @notice Example of a cross-chain txn with calldata using Connext
contract Source {
    ...
    IConnextHandler public immutable theConnext;

    /*//////////////////////////////////////////////////////////////
                               CONSTRUCTOR
    //////////////////////////////////////////////////////////////*/

    /// @notice Creates Transfer contract 
    /// @param aConnext The address of a deployed ConnextHandler.sol contract
    constructor (address aConnext) {
        theConnext = IConnextHandler(aConnext);
    } 
    ...
    /// @notice Updates value on target contract using 'xcall'
    /// @param aTo The address of the target contract
    /// @param aAsset The address of the token on origin domain
    /// @param aOriginDomain The origin domain id (https://docs.nomad.xyz/bridge/domains.html)
    /// @param aDestinationDomain the destination domain id
    /// @param aNewValue The amount to update to
    function updateValue(
        address aTo,
        address aAsset,
        uint32 aOriginDomain,
        uint32 aDestinationDomain,
        uint256 aNewValue
    ) external {
        // Create the calldata
        bytes4 mySelector = bytes4(keccak256("updateValue(uint256)"));
        bytes memory myCallData = abi.encodeWithSelector(mySelector, aNewValue);

        // Construct XCallArgs
        CallParams memory myCallParams = CallParams({
            to: aTo,
            callData: myCallData,
            originDomain: aOriginDomain,
            destinationDomain: aDestinationDomain,
            recovery: aTo,
            callback: address(0),
            callbackFee: 0,
            forceSlow: false,
        ...
        XCallArgs memory myXcallArgs = XCallArgs({
            params: myCallParams,
            transactingAssetId: aAsset,
            amount: 0,
            relayerFee: 0
        });

        emit UpdateInitiated(aTo, aNewValue, false);

        theConnext.xcall(myXcallArgs);
    }
}
