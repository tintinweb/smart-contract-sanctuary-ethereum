...
contract Single_Collection is ERC721 {
    ...
    mapping(uint=>NFTowner) public nftOwner; 
    ...
    function mint(string memory _tokenURI, uint _royalty) ifHashExist(_tokenURI) public {
        tokenId.increment();
        ...
        nftOwner[newTokenId].marketPlace = address(this);
        nftOwner[newTokenId].nft_owner = msg.sender;
        nftOwner[newTokenId].roylaty = _royalty;
        
        _mint(msg.sender,newTokenId);

        emit nft_id(newTokenId);
    }

    function artistOfNFT(uint _tokenId) public view returns(address){
        return nftOwner[_tokenId].nft_owner;
    }

    function getRoyaltyOfNFT(uint _tokenId) public view returns(uint){
        return nftOwner[_tokenId].roylaty;
    }

    function getNFTMarketPlace(uint _tokenId) public view returns(address){
        return nftOwner[_tokenId].marketPlace;
    }
