...
contract DynamicNft is AccessControl, IDynamic {
    ...
    address private genesisContractAddress;
    address private subsContractAddress;
    ...
    event Refunded(address owner, uint256 tokenId);
    ...
    event ProposalApproved(string id, address author, uint256 amount);
    ...
    function mintGenesis(address _to, bytes32[] calldata _merkleProof) public payable {
        ...
        require(GenesisNft.totalSupply() + 1 <= genesisSupply, "GENESIS NFT LIMIT REACHED");
        ...
        if (GenesisNft.totalSupply() == getGenesisSupply()) {
    ...
    function mintSubscription(address _to) public payable {
        ...
        subsHolder[_id] = Subscription({tokenId: _id, owner: _to, expired: false, renewed: false, renewalExpire: 0});

        cap3TreasuryFundShare(msg.value);
        SubscriptionNft.mint(_to);
    ...
    function claimRefund(uint256 tokenId) public {
        ...
        require(GenesisNft.ownerOf(tokenId) == msg.sender, "CLAIMREFUND: NOT A TOKEN HOLDER");
        ITreasury treasury = ITreasury(payable(treasuryAddress));
        GenesisNft.burn(tokenId);
        treasury.payRefund(msg.sender, genesisPrice);

        emit Refunded(msg.sender, tokenId);
    }
    ...
    function addApprovedProposal(
        ...
        emit ProposalApproved(_id, _author, _funds);
    }
    ...
    function cap3TreasuryFundShare(uint256 _amount) internal {
        ...
        uint256 convertedDollarAmount = dollarValueOfEth / 10**8;
        uint256 newLimitInEth = 5000000 / convertedDollarAmount;

        _setTreasuryLimit(newLimitInEth * 10**18);

        if (address(treasuryAddress).balance > treasuryLimit) {
            uint256 extraBalance = address(treasuryAddress).balance - treasuryLimit;
            ...
            require(success, "MINT:ETH TRANSFER FAILED");
        } else if (address(treasuryAddress).balance == treasuryLimit) {
            (bool success, ) = cap3Wallet.call{value: _amount}("");
            require(success, "MINT:ETH TRANSFER FAILED");
        } else if ((address(treasuryAddress).balance + _amount) > treasuryLimit) {
