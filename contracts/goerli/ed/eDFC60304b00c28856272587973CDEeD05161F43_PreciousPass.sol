    ...
    PriceConsumerV3
{
    ...
    uint256 public constant WHITELIST_COST = 0.055 ether;
    ...
    uint256[] private diamondSpots; //TODO: write a getter
    address[] private _team= [0xcA7b37c62740B2706b0c195843ca06a61693Bb06,0xf814B7489003126D3118f86c2Ad1F42BE14F5aA2,0x52eC9DE74755FCb9eDf7D290e677081C597c2408];
    ...
    *
     */
    function getDiamondSpots() external view returns(uint256[] memory){
        return diamondSpots;
    }
    ...
    /**
    * @notice creates an array of 49 numbers from the chainlink oracle ruandom number
     */
    ...
    /**
    * @notice randomly selects the diamond spots from chainlink oracle;
     */
    function revealDiamondSpots() internal {
        getRandomNumber();
        uint256 rn = randomResult;
        uint256[] memory array = expand(rn, 49);

        for (uint256 i = 0; i < array.length; i++) {
            uint256 rnE = 150 + (array[i] % 3350);
            while (_diamondSpots[rnE] == true)
                rnE = (rnE * array[i - 1]) % 3350;
    ...
    function payment() external nonReentrant {
        ...
        require(address(this).balance >= (2000 * 10 ** 8 / uint(_price)), "not enough balance");

        payable(_team[1]).transfer(2 ether);
        payable(_team[2]).transfer(2 ether);
        paymentClaimed = true;
    }
}
