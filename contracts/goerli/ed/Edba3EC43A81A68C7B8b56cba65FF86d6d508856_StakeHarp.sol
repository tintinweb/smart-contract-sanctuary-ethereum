...
contract StakeHarp is Pausable, Ownable, ReentrancyGuard {

    Token HarpToken;

    uint256 public planDuration = 86400;

    uint256 _planExpired = 2592000;
    ...
    event Harped(address indexed from, uint256 amount);
    ...
    function claimReward() external returns (bool){
        ...
        require(stakeInfos[_msgSender()].claimed == 0, "Already Serviced!");
        ...
        HarpToken.transfer(_msgSender(), totalTokens);
    ...
    function stakeHarp(uint256 stakeAmount) external payable whenNotPaused {
        require(stakeAmount >0, "Park your Car correctly");
        require(block.timestamp < planExpired , "Parking Expired");
        require(addressStaked[_msgSender()] == false, "You are already parkked");
        require(HarpToken.balanceOf(_msgSender()) >= stakeAmount, "Insufficient Parking Balance");
        
           HarpToken.transferFrom(_msgSender(), address(this), stakeAmount);
            totalStakers++;
        ...
        emit Harped(_msgSender(), stakeAmount);
    }    
