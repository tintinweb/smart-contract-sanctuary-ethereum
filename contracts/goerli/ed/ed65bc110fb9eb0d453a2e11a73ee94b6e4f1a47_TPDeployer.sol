...
contract TPDeployer {
    ...
    constructor(address owner, address router, address factory, address usdt, address signer) {
        ...
        address didFactory = address(new TPDIDFactory(swapManager, owner));
        manager = address(
            new TPManager(
                owner,
                liquidityManager,
        ...
        TPDIDFactory(didFactory).setManager(manager);
        emit Deploy(manager, liquidityManager, tokenFactory, vaultFactory, swapManager, didFactory);
    }
}
...
import {ITPManager} from "../pool/interfaces/ITPManager.sol";

contract TPDIDFactory is Ownable2Step {
    address public implementation;
    address public manager;
    address public forwardPolicies;

    constructor(address swapManager_, address owner_) {
    ...
    function _createTPDID(
        ...
        ITPManager(manager).didFactoryWhitelist(did);
        return did;
    }
...
contract TPManager is TPWhitelistAndBlacklist, Ownable2Step {
    ...
    event CreateVaultAndToken(address indexed vault, address indexed token, bytes32 indexed symbolHash);
    ...
    function createVaultAndToken(
        ...
        emit CreateVaultAndToken(vault, token, keccak256(abi.encodePacked(symbol)));
    }
...
contract TPVault is Initializable, Ownable2StepUpgradeable, UUPSUpgradeable {
    using SafeTransferLib for ERC20;
    ...
    function initialize(address _manager, address newOwner, bytes32 _merkleRoot, string memory _uri)
        public
        initializer
    {
        __Ownable2Step_init();
...
contract TPVaultFactory is Ownable2Step {
    ...
    constructor(address owner_) {
        implementation = address(new TPVault());
        _transferOwnership(owner_);
    }
    ...
    function _createVault(address newOwner, bytes32 merkleRoot, string memory uri) private returns (address) {
        bytes memory initializeCalldata =
            abi.encodeWithSelector(TPVault.initialize.selector, manager, newOwner, merkleRoot, uri);
        return _createProxy(implementation, initializeCalldata);
    }
