...
import {DAOArguments} from "./Vault.sol";
// import "openzeppelin-contracts-upgradeable/proxy/utils/Initializable.sol";

contract GovernanceToken is ERC20VotesUpgradeable, OwnableUpgradeable {

    function initialize(DAOArguments calldata _govTokenContractArguments) external initializer{
        __Ownable_init();
        __ERC20Permit_init(_govTokenContractArguments._govTokenName);
        __ERC20_init_unchained(_govTokenContractArguments._govTokenName, _govTokenContractArguments._govTokenSymbol);
    }
  ...
  GovernorTimelockControlUpgradeable
{
  ...
  function initialize(
    ...
    DAOArguments memory _governorContractArguments,
    ...
    address _vault
      ...
      __GovernorVotesQuorumFraction_init(_governorContractArguments._quorumPercentage);
...
contract TimeLockContract is TimelockControllerUpgradeable {

  function initialize(DAOArguments calldata _timelockContractArguments) external initializer{
    address[] memory proposers = new address[](0);
    address[] memory executors = new address[](0);
    __TimelockController_init(_timelockContractArguments._minDelay,proposers,executors, address(0));
    // This is revoked after setting the proposer and executor in the Vault.sol file
...
contract Vault {
    ...
    address private immutable timelockImplementation;
    address private immutable governorImplementation;
    //for Gnosis
    address public immutable masterCopyAddress;
    GnosisSafeProxyFactory public immutable proxyFactory;

    constructor() {
        // TODO: Verify below addresses
        ...
        timelockImplementation = address(new TimeLockContract());
        governorImplementation = address(new GovernorContract());
    }


    // setting up a new campaign
    // This returns the campaign address. It is useful for testing - TODO: Can be removed
    function setupCampaign(
        address _nftContractAddress,
        DAOArguments calldata _DAOArguments,
        ...
        require(_DAOArguments._signatories.length > 1 && _DAOArguments._quorum > 0, "Wrong number of signatories or Quorum given");
        ...
        address newGovernanceToken = Clones.clone(tokenImplementation);
        GovernanceToken(newGovernanceToken).initialize(_DAOArguments);

        // Deploy Timelock
        address payable newTimelock = payable(Clones.clone(timelockImplementation));
        TimeLockContract(newTimelock).initialize(_DAOArguments);
        timelocks[newTimelock] = true;

        // Deploy Governor
        address payable newGovernorContract = payable(Clones.clone(governorImplementation));
        GovernorContract(newGovernorContract).initialize(GovernanceToken(newGovernanceToken), TimeLockContract(newTimelock), _DAOArguments, _nftContractAddress, address(this));

        address newMultisig = setupMultisig(_DAOArguments);

        // Grant and revoke as per documentation. Referred PatickCollins Github - https://github.com/PatrickAlphaC/dao-template/blob/main/deploy/04-setup-governance-contracts.ts
        TimeLockContract(newTimelock).grantRole(TimeLockContract(newTimelock).PROPOSER_ROLE(), newGovernorContract);
        TimeLockContract(newTimelock).grantRole(TimeLockContract(newTimelock).EXECUTOR_ROLE(), address(0));
        // //Revoking role here
        TimeLockContract(newTimelock).revokeRole(TimeLockContract(newTimelock).TIMELOCK_ADMIN_ROLE(), address(this));

        // //TODO: Check if this is needed -> making nftContract the owner of GovernanceToken Contract
        // newGovernanceToken.transferOwnership(_nftContractAddress);
        storeCampaign(
            msg.sender,
            _nftContractAddress,
            newGovernanceToken,
    ...
    //creating multisig
    function setupMultisig(DAOArguments calldata _multisigContractArguments) private returns(address){
        ...
        require(multisig != address(0), "Gnosis Safe deployment failed");
        return multisig;
    }
    ...
    function mintGovernanceTokens(address _to, uint256 _amount) external {
        ...
        GovernanceToken govToken = GovernanceToken(allCampaigns[msg.sender].governanceToken);
        uint256 rate = _amount * 10**12;
        govToken.mint(_to, rate);
    }
}