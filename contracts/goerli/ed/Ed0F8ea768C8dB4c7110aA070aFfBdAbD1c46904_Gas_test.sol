...
contract Gas_test is ERC721, EIP712, ERC721Votes{
 using Counters for Counters.Counter;
...
function safeMint(address[] calldata to) public {

uint256 index=to.length;
 
for (uint256 i=0;i<index;i++){
   require(minted[to[i]]+1<=2,"Max number per address is 2");
    require(minted[to[i]]+1<=8000,"Total supply is 8000");
    minted[to[i]]+=1;
