...
contract TokenSwap is Ownable {
    ...
    mapping(address => uint256) tokenToDecimals;

    constructor() {
        tokenToDecimals[tokens[0]] = 6;
        tokenToDecimals[tokens[1]] = 18;
        tokenToDecimals[tokens[2]] = 18;
    }
    ...
    function exchange(uint128 i, uint128 j, uint256 dx, uint256 dy, bool use_eth) external {
        ...
        uint256 p_dx = dx * (10 ** (18 - tokenToDecimals[tokens[i]]));
        require(p_dx > 0, "Amount must be positive");
        require(tokenA.balanceOf(msg.sender) >= p_dx, "Insufficient balance of tokenA");
        require(
            tokenA.allowance(msg.sender, address(this)) >= p_dx,
            "Insufficient allowance of tokenA"
        );

        // Calculate the amount of tokenB to be received
        uint256 amountB = (p_dx * rate) / (10 ** (18 - tokenToDecimals[tokens[j]]));
        ...
        tokenA.transferFrom(msg.sender, address(this), p_dx);

        // Transfer tokenB from the contract to the user
        tokenB.transfer(msg.sender, amountB);

        // Emit the swap event
        emit Swap(msg.sender, p_dx, amountB);
    }
