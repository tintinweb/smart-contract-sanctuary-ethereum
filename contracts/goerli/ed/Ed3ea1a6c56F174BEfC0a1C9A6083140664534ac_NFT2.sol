...
contract NFT2{
    ...
    constructor(address _owner, address nftAddress, address accessControlAddress) {
        owner=_owner;
        impl = NFT(nftAddress);
        accessControl = AccessControlContract(accessControlAddress); 
    }
    ...
    function approve (address to, uint256 tokenId) public{
        impl.approve(to,tokenId);
    }

    function safeTransferFrom(address from, address to, uint256 tokenId) public{
        impl.safeTransferFrom(from,to,tokenId);
    }

    function safeTransferFrom(address from, address to, uint256 tokenId, bytes memory _data) public{
        impl.safeTransferFrom(from,to,tokenId,_data);
    }

    function setApprovalForAll(address operator, bool approved) public {
        impl.setApprovalForAll(operator,approved);
    }

    function transferFrom(address from, address to, uint256 tokenId) public{
        impl.transferFrom(from,to,tokenId);
    }

    function balanceOf(address _owner) public view returns (uint256) {
        return impl.balanceOf(_owner);
    }

    function getApproved(uint256 tokenId) public view returns (address) {
        return impl.getApproved(tokenId);
    }
    function isApprovedForAll(address _owner, address operator) public view returns (bool){
        return impl.isApprovedForAll(_owner,operator);
    }
    ...
    function mint(address mintAddress, string memory _tokenURI) public onlyNftRole returns(uint256) {
        return impl.mint(mintAddress, _tokenURI);
    }
