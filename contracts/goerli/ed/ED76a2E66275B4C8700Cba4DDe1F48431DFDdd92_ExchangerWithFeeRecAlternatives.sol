...
import "./interfaces/IExchangerBridge.sol";
...
contract Exchanger is Owned, MixinSystemSettings, IExchanger {
    ...
    bytes32 private constant CONTRACT_EXCHANGER_BRIDGE = "ExchangerBridge";
    ...
    function resolverAddressesRequired() public view returns (bytes32[] memory addresses) {
        ...
        newAddresses[10] = CONTRACT_EXCHANGER_BRIDGE;
    ...
    function exchangerBridge() internal view returns (IExchangerBridge) {
        return IExchangerBridge(requireAndGetAddress(CONTRACT_EXCHANGER_BRIDGE));
    }
    ...
    function updateDestinationForExchange(
        ...
        dest.issue(recipient, destinationAmount, false);
    }
    ...
    function _exchange(ExchangeArgs memory args) internal returns (uint256 amountReceived, uint256 fee) {
        ...
        uint256 sourceAmountAfterSettlement = _settleAndCalcSourceAmountRemaining(
            args.sourceAmount,
            args.fromAccount,
        ...
        _convert(args, sourceAmountAfterSettlement, amountReceived, fee);
    ...
    function _convert(
        ExchangeArgs memory args,
        uint256 sourceAmountAfterSettlement,
        uint256 amountReceived,
        uint256 fee
    ) internal {
        // Burn the source amount
        issuer().synths(args.sourceCurrencyKey).burn(args.fromAccount, sourceAmountAfterSettlement, false);
        // synthetixDebtShare()

        if (args.destChainId == 0) {
            ISynth dest = issuer().synths(args.destCurrencyKey);

            dest.issue(args.destAccount, amountReceived, false);
        } else {
            exchangerBridge().updateDestination(args.destAccount, args.destCurrencyKey, amountReceived, fee, args.destChainId);
        }
        issuedSynthAggregator().sendExchange(
            args.destCurrencyKey,
            amountReceived,
            args.destAccount,
            args.sourceCurrencyKey,
        ...
        if (fee > 0) {
            ...
            fee = exchangeRates().effectiveValue(args.destCurrencyKey, fee, sUSD);

            // Remit the fee in sUSDs
            issuer().synths(sUSD).issue(feePool().FEE_ADDRESS(), fee, true);
    ...
    function reclaim(
        ...
        issuer().synths(currencyKey).burn(from, amount, true);
    ...
    function refund(
        ...
        issuer().synths(currencyKey).issue(from, amount, true);
    ...
    modifier onlyExchangerBridge() {
        require(msg.sender == address(exchangerBridge()), "Exchanger: Only exchange bridge can call");
        _;
    }
...
contract ExchangerWithFeeRecAlternatives is Exchanger {
    ...
    bytes32 private constant CONTRACT_EXCHANGE_BRIDGE = "ExchangeBridge";
    ...
    function _exchangeAtomically(ExchangeArgs memory _args) internal returns (uint256 amountReceived, uint256 fee) {
        ...
        ) = _getAmountsForAtomicExchangeMinusFees(sourceAmountAfterSettlement, args.sourceCurrencyKey, args.destCurrencyKey);
