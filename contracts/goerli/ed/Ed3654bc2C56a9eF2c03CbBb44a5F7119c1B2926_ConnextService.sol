...
contract ConnextService is Ownable, Pausable, IMdexCrossChainSwap {
    ...
    function swapCommon(
        ...
        mdexController.swap(assetAddress[destinationDomain],tokenOut, amount, routeIndex);
        IERC20(tokenOut).transfer(receiver, IERC20(tokenOut).balanceOf(address(this)));
    }

    function connextSwap(uint256 amount, bytes calldata payload, bytes calldata apiPayload) internal {
        // api payload
        bool isSpiltSwap;
        ...
        if(isSpiltSwap){
            ...
            _swapCommon(amount,payload, apiPayload);
        }
    }

    function _swapCommon(uint256 amount, bytes calldata payload, bytes calldata apiPayload) internal {
        ...
        bytes4 selector = bytes4(keccak256("swapCommon(address,address,uint256,uint256,uint32)"));
        bytes memory callData = abi.encodeWithSelector(selector, sender, tokenDestinationAddress, amount, routeIndex, destinationDomain);
    ...
    function _swap(uint256 amount, bytes calldata payload, bytes calldata apiPayload) internal override {
