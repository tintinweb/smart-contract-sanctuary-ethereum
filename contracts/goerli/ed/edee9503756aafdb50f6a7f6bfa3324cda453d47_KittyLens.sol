...
contract KittyLens {
    ...
    function amountToShares(Kitty kitty, uint256 amount) external view returns (uint256 shares) {
        uint256 totalSupply = kitty.totalSupply();

        ERC20 asset = kitty.asset();
        uint256 totalBorrows = kitty.totalBorrows();
        uint256 inventory = asset.balanceOf(address(kitty)) + totalBorrows;

        return (totalSupply == 0) ? amount : FullMath.mulDiv(amount, totalSupply, inventory);
    }
}
