...
contract ABBLegacy is MintpassValidator, ERC721A, ERC2981, Ownable {
    ...
    // Mint Price
    int256 public immutable mintUSDPrice = 1500000000; // TODO = 1,500 USD +00
    int256 public immutable discountMintUSDPrice = 1300000000; // TODO = 1,300 USD +00
    int256 public redeemUSDPrice = 500000000; // = 5 USD
    // We appreceate new users in web3 so we cover their FIAT and Conversion Fees if paying with FIAT.
    uint256 public complimentaryFeePercentage = 550; // = 5.5%
    // TODO set to production address
    address internal complimentaryAddress =
        0x2Ff0EC69341f43CC462251bD49BB63681adAfCb0; // Wallet that has a reduced Fee for FIAT Payments
    ...
    function mint(address minter, uint256 quantity) public payable {
        ...
        // We welcome new web3 users and cover the FIAT Fees
        uint256 subtractFee;
        if (complimentaryAddress == msg.sender) {
            subtractFee = (mintPrice() / 10000) * complimentaryFeePercentage;
        }
        require(
            msg.value >= ((mintPrice() - subtractFee) * quantity * 199) / 200, // 0.5% treshold due to conversion rate swings
    ...
    function allowlistMint(
        ...
        uint256 finalMintPrice = ((mintPrice() - subtractFee) *
            quantity *
            199) / 200; // 0.5% treshold due to conversion rate swings

        if (mintpass.tier == 2) {
            if (complimentaryAddress == msg.sender) {
                subtractFee =
                    (discountMintPrice() / 10000) *
                    complimentaryFeePercentage;
            }
            finalMintPrice =
                ((discountMintPrice() - subtractFee) * quantity * 199) /
                200; // 0.5% treshold due to conversion rate swings
        }

        require(
            msg.value >= finalMintPrice, 
            "ABB Legacy: Insufficient Amount"
        );
    ...
    /**
     * @dev Function to be called by contract owner to change the FIAT payment address and fees.
     *
     * @param _complimentaryAddress wallet address that is performing the purchases
     * @param _complimentaryFeePercentage 550 = 5.5%; Fee that is subtracted from payment amount.
     */
    function setComplimentaryFee(
        address _complimentaryAddress,
        uint256 _complimentaryFeePercentage
    ) public virtual onlyOwner {
        complimentaryAddress = _complimentaryAddress;
        complimentaryFeePercentage = _complimentaryFeePercentage;
    }
