...
contract BitForestNFTs is ERC721, Ownable, ERC721Enumerable {
    ...
    constructor() payable ERC721('BitPals', 'BP') {
        mintPrice = 0.001 ether;
        totalTokenSupply = 0;
        maxTokenSupply = 5000;
        maxWhiteListSupply = 500;
        maxPerWallet = 4;
    ...
    function publicMint(uint amount) public payable {
        ...
        require(totalTokenSupply + amount < maxTokenSupply, 'Sold out');
        require(walletMints[msg.sender] + amount <= maxPerWallet, 'Max mints per wallet exceeded');
    ...
    function whiteListMint(uint amount) public payable {
        require(isWhiteListMintEnabled, 'Public mint is not enabled');
        require(whiteList[msg.sender], 'You are not on the white list');
        require(totalTokenSupply + amount < maxWhiteListSupply, 'Pre Sale sold out');
        require(walletMints[msg.sender] + amount <= maxPerWallet, 'Max mints per wallet exceeded');
        require(msg.value == amount * mintPrice, 'Insufficient payment');
