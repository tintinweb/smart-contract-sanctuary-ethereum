...
contract staking is OwnableUpgradeable{
    ...
    IERC721Enumerable public SNContract;

    mapping (uint256 => uint256) public tokenLastStakedAt;
    mapping (address => uint256[]) public stakedTokensOfWallet;

    function initialize(address _SNContract) initializer public {
        __Ownable_init();
        SNContract = IERC721Enumerable(_SNContract);
    }
    ...
    function stakeToken(uint256[] calldata tokenIDs) external {
        ...
        for(uint i; i < tokenIDs.length; ){
            ...
            require(SNContract.ownerOf(currentToken) == msg.sender,"You dont own one or more of the tokens");
            SNContract.transferFrom(msg.sender, address(this), currentToken);
    ...
    function unstakeToken(uint256[] calldata tokenIDs) external{
        for(uint i; i < tokenIDs.length;){
            ...
            SNContract.transferFrom(address(this), msg.sender, currentToken);
