...
contract Payments is Ownable {
    ...
    function insertNewProject(
        ...
        // WE NEED TO SPEND WHATEVER WE HAVE TO GET CHAINLINK TOKENS.
        ...
        if (chainLinkPeggedToken != address(0x0)) {
            LinkSwapper swapper = LinkSwapper(chainLinkPeggedSwapContract);
            IERC20(chainLinkToken).approve(
            ...
            swapper.swap(
                CHAINLINK_FEE_FOR_PACK_GEN,
                chainLinkToken,
                chainLinkPeggedToken
            );
            IERC20(chainLinkPeggedToken).transfer(
                parentAddress,
                CHAINLINK_FEE_FOR_PACK_GEN
            );
            // chainLinkToken = LinkTokenInterface(chainLinkPeggedToken);
        } else {
