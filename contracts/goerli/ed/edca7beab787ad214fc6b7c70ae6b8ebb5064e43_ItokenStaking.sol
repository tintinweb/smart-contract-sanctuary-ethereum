...
contract ItokenStaking is Initializable, OwnableUpgradeable {
    using SafeMathUpgradeable for uint256;
    ...
    */
    ...
    function deposit(
        ...
        _deposit(_pid, _amount, _vault, msg.sender, _itokenId);
    }
    ...
    function depositFromOtherContract(
        ...
        _deposit(_pid, _amount, _vault, _userAddress, _itokenId);
    }
    ...
    function _deposit(
        ...
        updateUserDepositDetails(_pid, _userAddress, _amount, _vault, _itokenId);
    ...
    function withdraw(uint256 _pid,uint256 _itokenId, bool _withStake) external {
        UserInfo storage user = userInfo[_pid][msg.sender];
        ...
        if (user.cooldown == false) {
            ...
            return;
                ...
                _withdraw(_pid, _withStake, _itokenId);
        }
    }

    // Withdraw LP tokens from MasterChef.
    function _withdraw(uint256 _pid, bool _withStake, uint256 _itokenId) internal {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
        uint256 _amount = checkEligibleAmount(_pid, msg.sender, _itokenId);
    ...
    */
    function checkEligibleAmount(uint256 _pid, address _user, uint256 _itokenId)
        private
        returns (uint256)
    {
        uint256 eligibleAmount = 0;
    ...
    function emergencyWithdraw(uint256 _pid, uint256 _amount) public onlyOwner {
        itokenInfo[_pid].itoken.safeTransfer(address(msg.sender), _amount);
