...
contract TheyXolo is ERC721A, Ownable, ReentrancyGuard, ProxyRegistry {
  ...
  uint64 public constant costPerToken = 0.022 ether;
  uint64 public constant costPerPresaleToken = 0.011 ether;

  uint8 public constant maxPerWalletPresale = 2;
  uint8 public constant maxPerTx = 11; // Plus one
  uint16 public constant maxTokenSupply = 223; // Plus one
  uint32 private royaltyFeesInBeeps = 5;

  string public baseURI = 'https://test.theyxolo.art/api/tokens/';
  ...
  */
  ...
  modifier inMintPhase(MintPhase _mintPhase) {
    require(mintPhase == _mintPhase, 'TXOL: incorrect mint phase');
    _; // The _; we call the instruction after we check
  }
  ...
  */
   ...
   */
  function allowlistMint(uint8 _amount, bytes32[] calldata _proof)
    external
    payable
    virtual
    nonReentrant
    inMintPhase(MintPhase.ALLOWLIST_SALE)
  {
    ...
    require(
      msg.value == costPerPresaleToken * _amount,
      'TXOL: incorrect payment'
    );
    require(
      _totalMinted() + _amount < maxTokenSupply,
      'TXOL: insufficient supply'
    );

    uint64 allowlistMintRedemptions = _getAux(msg.sender);

    require(
      allowlistMintRedemptions + _amount < maxPerWalletPresale,
      'TXOL: exceeds allowlist max'
    );
  ...
  function mint(uint8 _amount)
    external
    payable
    nonReentrant
    inMintPhase(MintPhase.PUBLIC_SALE)
  {
    require(_amount < maxPerTx, 'TXOL: Max tokens per transaction reached');

    // Check total supply is not exceeded
    require(
      _totalMinted() + _amount < maxTokenSupply,
      'TXOL: Max token supply reached'
    );

    // Check value is exact
    require(msg.value == costPerToken * _amount, 'TXOL: value is not correct');
  ...
  */
   ...
   */
  function incrementRedemptionsAllowlist(uint8 _numToIncrement) private {
    uint64 allowlistMintRedemptions = _getAux(msg.sender);

    allowlistMintRedemptions += uint64(_numToIncrement);
    _setAux(msg.sender, allowlistMintRedemptions);
  }
  ...
  */
  ...
  fallback() external payable {
    require(false, 'TXOL: not implemented');
  }
  ...
  {
    return (owner(), (_salePrice * royaltyFeesInBeeps) / 1000000);
  }
}
