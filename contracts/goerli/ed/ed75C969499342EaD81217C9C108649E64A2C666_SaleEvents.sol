...
contract SaleEvents {
    ...
    modifier onlyManager() {
        if (!management.hasRole(MANAGER_ROLE, msg.sender)) revert OnlyManager();
        _;
    }
    ...
    */
    function setEvent(
        ...
        uint256 _current = block.timestamp;
        if (events[_eventId].end != 0) revert EventExist();
        if (_start >= _end || _current >= _end || _current > _start) revert InvalidSetting();
    ...
    */
    function setPrice(uint256 _eventId, address _token, uint256 _price) external onlyManager checkEvent(_eventId) {
        if (!management.paymentTokens(_token)) revert PaymentNotSupported();
    ...
    */
    function purchase(
        ...
        if (_purchasedAmt > events[_eventId].maxAllocation) revert ExceedAllocation();
    ...
    function _precheck(
        ...
        if (!events[_eventId].isPublic && !whitelist[_eventId][_beneficiary]) 
