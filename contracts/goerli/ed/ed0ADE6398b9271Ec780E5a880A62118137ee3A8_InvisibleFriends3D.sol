...
contract InvisibleFriends3D is ERC721A, ERC2981, Ownable {
  ...
  mapping(uint256 => bool) _alreadyMinted;
  ...
  constructor(
    ...
    signingAddress = initialSigningAddress;
  ...
  modifier verifySignature(
    ...
    bytes32 msgHash = keccak256(abi.encodePacked(sender, originalIds));
    address recovered = msgHash.toEthSignedMessageHash().recover(signature);
    if (signingAddress != recovered) revert BadSignatureError();
    _;
  }

  // Minting

  function alreadyMinted(uint256[] calldata tokenId) public view returns (bool[] memory) {
    bool[] memory states = new bool[](tokenId.length);
    for (uint256 i = 0; i < tokenId.length; i++) {
      states[i] = _alreadyMinted[tokenId[i]];
    }
    return states;
  }

  function verifiedMint(bytes calldata signature, uint256[] calldata originalIds)
    external
    payable
    onlySaleState(SaleState.Private)
    mustMatchPrice(originalIds.length)
    requireSupply(originalIds.length)
    verifySignature(_msgSender(), signature, originalIds)
  {
  ...
  function verifiedMint(
    ...
    mustMatchPrice(originalIds.length)
    requireSupply(originalIds.length)
    verifySignature(onBehalfOf, signature, originalIds)
  {
    _checkOwnershipAndAvailability(originalIds, onBehalfOf);
    _mint(_msgSender(), originalIds.length);
  }
  ...
  function _checkOwnershipAndAvailability(uint256[] calldata originalIds, address proposedOwner) private {
    ...
    for (uint256 i = 0; i < originalIds.length; i++) {
      ...
      if (_alreadyMinted[tokenId]) revert TokenIdAlreadyMintedError(tokenId);

      _alreadyMinted[tokenId] = true;
    }
  }
