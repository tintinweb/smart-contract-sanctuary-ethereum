...
contract FinanceClaim is IFinanceClaim, Ownable {
    ...
    modifier isValidPoolIndex(uint256 poolIndex) {
        ...
        if (poolIndex >= poolsCount) {
            revert Errors.FinanceClaim__PoolIndexNonExistent(poolIndex, poolsCount);
        }
        _;
    }
    ...
    constructor(address fixedSwapContractAddress) {
        require(fixedSwapContractAddress != address(0), "FinanceClaim: FixedSwap contract address is 0x0");
    ...
    function depositFunds(uint256 poolIndex, uint256 amount) external onlyFinOps isValidPoolIndex(poolIndex) {
        ...
        if (pools[poolIndex].reward == address(0)) {
            revert Errors.FinanceClaim__RewardNotInitialized(poolIndex);
        }
    ...
    function addPoolRewards(uint256[] calldata poolsIdxs, address[] calldata rewards) external onlyFinOps {
        if (poolsIdxs.length != rewards.length) {
            revert Errors.FinanceClaim__ArrayLengthsMismatch(poolsIdxs.length, rewards.length);
        }
        ...
        isValidPoolIndex(poolIndex)
    {
        ...
        for (uint256 i = 0; i < users.length; i++) {
            if (users[i] == address(0)) {
                revert Errors.FinanceClaim__InvalidClaimableUser(users[i]);
            }
            pools[poolIndex].userClaimableFunds[users[i]] = amounts[i];
            pools[poolIndex].validUserClaims[users[i]] =
                keccak256(abi.encodePacked(users[i], poolIndex, pools[poolIndex].latestWithdrawalTs));
        }
    ...
    function claimFunds(uint256 poolIndex) external isValidPoolIndex(poolIndex) {
        ...
        if (claimAmount == 0) {
            revert Errors.FinanceClaim__NoFundsToClaim(poolIndex, msg.sender);
        }
        if (pools[poolIndex].reward == address(0)) {
            revert Errors.FinanceClaim__RewardNotInitialized(poolIndex);
        }
        bytes32 userClaimHash = keccak256(abi.encodePacked(msg.sender, poolIndex, pools[poolIndex].latestWithdrawalTs));
        if (userClaimHash != pools[poolIndex].validUserClaims[msg.sender]) {
            revert Errors.FinanceClaim__InvalidClaimHash(msg.sender, poolIndex);
        }
    ...
    function changeFixedSwapAddress(address newFixedSwapAddress) external onlyOwner {
        require(newFixedSwapAddress != address(0), "FinanceClaim: FixedSwap contract address is 0x0");
