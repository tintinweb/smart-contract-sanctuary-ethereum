...
contract StakingFactory is Ownable, IStakingFactory {
    ...
    /**
     * @notice Ufo Pool plasma per block
     */
    uint256 public ufoPoolPlasmaPerBlock;

    /**
     * @notice Lp Pool plasma per block
     */
    uint256 public lpPoolPlasmaPerBlock;
    /**
    ...
    constructor(
        ...
        // uint256 totalBlocksPerYear = uint256(360 days) * (86400) * (10) / ((131); // for mainnet
        uint256 totalBlocksPerYear = 5760; // for local
        // uint256 totalBlocksPerYear = 120000; // for goerli

        uint256 blocksPerEpoch = totalBlocksPerYear / totalPools;

        for (uint256 index = 0; index < totalPools; index += 2) {
            // for ease even pools are ufo pool, odd pools are lp pools
            uint256 lockInBlocks = (blocksPerEpoch * (index)) / (2) + (1);
    ...
    function flushReward(address user, uint256 amount) external override onlyPool {
        if (pools[msg.sender].ufoPool) {
            claimedUfoRewardsForUfoPools = claimedUfoRewardsForUfoPools + (amount);
        } else {
            claimedUfoRewardsForLpPools = claimedUfoRewardsForLpPools + (amount);
        }
    ...
    /**
     * @notice Returns plasma per block for pool multiplied by WEIGHT_SCALE
     */
    function getPlasmaPerBlock() external view override returns (uint256 plasmaPerBlock) {
        Pool memory _pool = pools[msg.sender];
        if (!_pool.isPool) {
            return 0;
        }
        if (_pool.ufoPool) {
            ...
            plasmaPerBlock = (ufoPoolPlasmaPerBlock * WEIGHT_SCALE) / totalUfoLocked;
        } else {
            if (totalLpLocked == 0) {
                // to avoid division overflow when tvl is 0
                return 0;
            }
            plasmaPerBlock = (lpPoolPlasmaPerBlock * WEIGHT_SCALE) / totalLpLocked;
        }
    }
     ...
     * @param newValue new plasma reward per block
     */
    function changeUfoPoolPlasmaPerBlock(uint256 newValue) external onlyOwner {
        ufoPoolPlasmaPerBlock = newValue;
    }
     ...
     * @param newValue new plasma reward per block
     */
    function changeLpPoolPlasmaPerBlock(uint256 newValue) external onlyOwner {
        lpPoolPlasmaPerBlock = newValue;
    }
     ...
     * @param depositNumbers Deposit Numbers to claim
     */
    function claimPlasmaFromPool(uint256 poolIndex, uint256[] calldata depositNumbers) public {
