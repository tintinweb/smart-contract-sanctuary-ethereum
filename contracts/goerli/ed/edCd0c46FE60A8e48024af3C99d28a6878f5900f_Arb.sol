...
interface IExofiCompatiblePair
{
	function token0() external view returns (address);
	function token1() external view returns (address);
}

interface IExofiCompatibleFactory
{
...
interface IArb is IOwnable
{
	...
	function getAllPairsBulk(IExofiCompatibleFactory factory, uint256 startIndex, uint256 count) external view returns (PairData[] memory);
}
...
contract Arb is IArb, Ownable
{
	function trade(MetaPath[] calldata metaPath, uint256 amount) override external onlyOwner returns (uint256)
	{
		...
		unchecked
		{
			for(uint256 i = 0; i < len; ++i)
			{
				...
				// if(IERC20(p.path[0]).allowance(address(this), p.router) < type(uint256).max)
				// {
				// 	IERC20(p.path[0]).approve(p.router, type(uint256).max);
				// }
				// if(p.hasFee)
				// {
	...
	function getAllPairsBulk(IExofiCompatibleFactory factory, uint256 startIndex, uint256 count) override external view returns (PairData[] memory)
	{
		uint256 pairCount = factory.allPairsLength();
		if(startIndex >= pairCount)
		{
			return new PairData[](0);
		}
		if(pairCount > count + startIndex)
		{
			pairCount = count + startIndex;
		}
		PairData[] memory memArray = new PairData[](pairCount - startIndex);
		for (uint256 i = startIndex; i < pairCount; i++)
		{
			IExofiCompatiblePair pair = IExofiCompatiblePair(factory.allPairs(i));
			memArray[i - startIndex].index = i;
			memArray[i - startIndex].pairAddress = address(pair);
			memArray[i - startIndex].token0Address = pair.token0();
			memArray[i - startIndex].token1Address = pair.token1();
		}
		return memArray;
	}
}