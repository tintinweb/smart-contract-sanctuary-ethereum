...
abstract contract Verify is Initializable {
    using EnumerableSet for EnumerableSet.AddressSet;
...
contract Jackpot is Initializable, Verify, UUPSUpgradeable{
    mapping(uint256 => Jackpot) public jackpots;
    mapping(uint256 => User[]) public users;
    mapping(string => bool) private verifyMessage;
    address public monoWallet;

    event CreateJackpot(uint256 jackpotId, User[] users, uint256 amount, address addressToken);

    event UpdateWinner(uint256 jackpotId, User[] users, uint256 amount, uint256 userIdWinner, address addressWinner);
