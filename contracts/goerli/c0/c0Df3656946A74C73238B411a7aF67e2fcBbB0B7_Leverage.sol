...
contract Leverage is Ownable {
    ...
    address[] public reserveList = new address[](0);

    constructor() {
        address _poolProvider = 0x0562c663Ba8145a9fEE6F44b45837FbC8C2E59b3;
        address _curveProvider = 0x0562c663Ba8145a9fEE6F44b45837FbC8C2E59b3;
        address payable _weth9 = payable(
            0x6154B89bEB93530c067508Df27de7369983b1CF3
        );
        address _arthAddress = 0x2f73a674CEB69279EB3044d192A4842e0627d3b0;
        POOL_PROVIDER = _poolProvider;
        POOL = IPoolAddressesProvider(POOL_PROVIDER).getPool();
        CURVE_SWAPS = _curveProvider;
     ...
     **/
    function getAvailableLoanAmount(
        address _user,
        address _loan
    ) public view returns (uint256 availableLoanAmount) {
        for (uint i = 0; i < reserveList.length; i++) {
            address reservedToken = reserveList[i];
            uint256 balance = userBalances[_user][reservedToken];
            if (balance > 0) {
                availableLoanAmount += _calcAvailalbleLoanAmount(
                    reservedToken,
                    _loan,
    ...
    function withdrawLeverage(
        ...
        totalLoanAmount = getAvailableLoanAmount(msg.sender, _loan);
        uint256 interestRateMode = 1;
        uint16 referral = 0;
        IPool(POOL).borrow(
            _loan,
