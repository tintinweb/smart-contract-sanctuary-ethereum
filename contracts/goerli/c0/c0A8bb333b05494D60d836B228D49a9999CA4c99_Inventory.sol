...
contract Inventory is IInventory, Ownable {
  ...
  function confirmBurned(
    ...
    // Trying to confirm more than sent will lead to panic code 0x11 (Arithmetic operation underflowed or overflowed outside of an unchecked block) -- see unit tests
    toBeBurned[_tokenId] -= _amount;
...
interface IInventory {
  ...
  event TotalSupplyUpdated(uint256 _oldSupply, uint256 _newSupply);
...
interface ISoulboundToken is IERC165 {
  ...
  /// @param symbole_ The token symbol
  ...
  //##########################
  //#### CUSTOM FUNCTIONS ####
  function mint(address to, uint256 tokenId, string calldata imageURI) external;
