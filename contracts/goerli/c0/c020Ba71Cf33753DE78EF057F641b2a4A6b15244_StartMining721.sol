...
contract StartMining721 is ERC721, Ownable, ReentrancyGuard {
    ...
    event InitializedMint(uint256 tokenId, uint16 salePrice);

    constructor(string memory _baseURI, IERC20 _usdt)
        ERC721("Start Mining", "SMI")
    {
        // TESTS: usdt à supprimer
        baseURI = _baseURI;
        usdt = _usdt;
    }
    ...
    {
        require(_tokenId > 0, "NFT doesn't exist");
        return
    ...
    function setChainlink(address _address) external onlyOwner {
        priceFeed = AggregatorV3Interface(_address);
        emit ChainlinkUpdated(_address);
    }
    ...
    function initMint(
        ...
        emit InitializedMint(_nextNFT, _salePrice);
    }
    ...
    function mintSaleUSDT(uint16 _amount, address _referral)
        ...
        mintModifier(_amount, _referral)
    {
        _acceptPayment(uint256(_amount) * salePrice * 10**6);
        for (uint16 i = 0; i < _amount; i++) {
            _mintSale(msg.sender, _referral);
        }
    }
