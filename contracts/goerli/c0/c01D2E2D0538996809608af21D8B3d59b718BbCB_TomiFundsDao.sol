...
contract TomiFundsDao is TomiEvents, TomiStates, Initializable, OwnableUpgradeable  {
    ...
    function initialize(ITomiMainDao _tomiMainDao, ITreasury _treasury) public initializer {
        ...
        uniswapRouter = IUniswapV2Router02(0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D); // testnet

        // uniswapRouter = IUniswapV2Router02(0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D); // mainnet

        weth = 0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D; // testnet
        usdt = 0x7c2F82424f9681AFe13E8cc552E254B0B90F18c9; // testnet

        // weth = 0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2; // mainnet
        // usdt = 0xdAC17F958D2ee523a2206206994597C13D831ec7; // mainnet

        priceFeed = prices(0x34c5048a21430fF295e605f766F3f25B3d5916e2); // testnet

        tomiNFT = IERC721(0x4fd80B4e2005AB5ca1ED8d708a6Da5959A989565); // testnet

        // tomiNFT = IERC721(0x18B97FeB170eB6983aE79769B051d35e8103DC08); // mainnet
    ...
    function proposeSendToken(
        ...
        uint256 executionAmountInUSD;
        if (tokenType == 0) {
            require(address(treasury).balance >= executionAmount, "insufficient ETH amount in treasury");
            executionAmountInUSD = getLatestEthPrice(executionAmount);
        }
        else {
            require(IERC20(token).balanceOf(address(treasury)) >= executionAmount, "insufficient token amount in treasury");
            try this.getLatestPrice(token, executionAmount) returns (uint256 latestPrice) {
                executionAmountInUSD = latestPrice;
            } catch {
                executionAmountInUSD = 500_001 * (10 ** 6);
            }
        }
        require(functionId == 16, "incorrect function id");
        if (executionAmountInUSD > 100_000 * (10 ** 6) && executionAmountInUSD <= 500_000 * (10 ** 6)) {
            functionId = 17;
        } else if (executionAmountInUSD > 500_000 * (10 ** 6)) {
            functionId = 18;
        }
    ...
    function getLatestEthPrice(uint256 amountEth) internal view returns (uint256) {
        address[] memory path = new address[](2);
        path[0] = address(weth);
        path[1] = address(usdt);

        uint256[] memory amountsOut = uniswapRouter.getAmountsOut(amountEth, path);

        return amountsOut[1];
    }

    function getLatestPrice(address token, uint256 amountToken) external view returns (uint256) {
        uint256 amountInUSD;

        if (token == address(tomiToken)) {
            uint256 tomiTokenPrice = priceFeed.getTomiPrice();

            amountInUSD = (amountToken * tomiTokenPrice) / (10 ** 20);
        }
        else {
            ...
            uint256[] memory amountsOut = uniswapRouter.getAmountsOut(amountToken, path);

            amountInUSD = amountsOut[2];
        }
