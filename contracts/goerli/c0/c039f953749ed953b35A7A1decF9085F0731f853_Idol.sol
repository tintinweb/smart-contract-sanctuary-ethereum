...
error CannotMintWithinRegisteredRange(uint256 tokenId);
...
contract Idol is RelicsERC721 {
    uint256 private immutable _STARTING_INDEX = 1501;

    address private _relicsTreasury;
    uint256 private _highestIndex;
    ...
    constructor(
        ...
        uint24 royaltyValue
    ) RelicsERC721("Idol", "IDOL", baseTokenURI, contractURI, royaltyRecipient, royaltyValue, _msgSender()) {
        _highestIndex = _STARTING_INDEX;
    }

    function highestIndex() public view returns (uint256) {
        return _highestIndex;
    }

    function setTreasury(address treasury) external isAdmin {
        _relicsTreasury = treasury;
    }

    function getTreasuryAddress() public view returns (address) {
        return _relicsTreasury;
    }
    ...
    ) external isMinter returns (bool) {
        Range storage _range = _ranges[range];

        // verify range identifier is unregistered
        if (_range.startIndex != 0) {
            revert RangePreviouslyRegistered(range);
        }

        unchecked {
            uint256 startIndex = highestIndex();
            uint256 endIndex = startIndex + quantity - 1;
    ...
    function mint(address to, uint256 tokenId) external override isMinter whenNotPaused {
        if (tokenId >= _STARTING_INDEX) {
            revert CannotMintWithinRegisteredRange(tokenId);
        }
        _mint(to, tokenId);
    }
    ...
    function mintRange(
        ...
        if (msg.sender != _range.minter) {
            revert UnuthorizedMinter(range, msg.sender);
        }

        _verifyRangeSupply(quantity, _range.nextIndex, _range.startIndex, _range.endIndex);

        uint256 nextIndex = _range.nextIndex;
    ...
    function withdraw() external isAdmin {
        if (_relicsTreasury == address(0)) revert MustSetTreasury();
        payable(_relicsTreasury).transfer(address(this).balance);
    }

    function withdrawToken(address token) external isAdmin {
        if (_relicsTreasury == address(0)) revert MustSetTreasury();
        IERC20 erc20 = IERC20(token);
        erc20.transfer(_relicsTreasury, erc20.balanceOf(address(this)));
    }
...
abstract contract Administration is AccessControl, Pausable {
    ...
    constructor(address globalAdmin) {
        if (_msgSender() != globalAdmin) _setupRole(DEFAULT_ADMIN_ROLE, _msgSender());
