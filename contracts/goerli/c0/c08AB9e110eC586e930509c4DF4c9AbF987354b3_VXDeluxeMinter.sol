...
contract VXDeluxeMinter is ReentrancyGuard, Ownable {
    ...
    ClaimVXDeluxeI public claimVxDeluxe;
    ...
    function getAvailableBears() external returns (uint16[] memory) {
        uint256 i;
        uint256 counter;
        uint16[] memory allBears = claimVxDeluxe.getAllForfeithIds();
        uint16[] memory bearsAvailable = new uint16[](allBears.length);
        for (i; i < allBears.length; ) {
            unchecked {
                if (vxDeluxe.exists(allBears[i]) == false) {
                    bearsAvailable[counter] = allBears[i];
                    counter ++;
                }
            }
        }
        return bearsAvailable;
    }
    ...
    function setClaimVXBears(ClaimVXDeluxeI _vxDeluxeI) external onlyOwner {
        claimVxDeluxe = _vxDeluxeI;
        emit SetContract(address(_vxDeluxeI), "ClaimVXDeluxe");
    }
