...
contract ParaSwapLiquiditySwapAdapter is BaseParaSwapSellAdapter, ReentrancyGuard {
  ...
  function swapAndDeposit(
    ...
    IERC20WithPermit bToken =
      IERC20WithPermit(_getReserveData(address(assetToSwapFrom)).bTokenAddress);
 ...
 * @dev Contract to keep guarantee
 */
contract SafeGuard is VersionedInitializable, ReentrancyGuard {
  using SafeMath for uint256;
  ...
  uint256 public constant SAFEGUARD_REVISION = 0x1;

  function initialize(ILendingPoolAddressesProvider provider, address _dusd) public initializer {
    lendingPool = ILendingPool(provider.getLendingPool());
    DUSD = _dusd;
  }

  function getRevision() internal pure override returns (uint256) {
    return SAFEGUARD_REVISION;
  }

  /**
   * @notice Deposit token to safeguard
   * @param token The token address to deposit, can only be DUSD
   * @param amount The amount of token to deposit
   */
  function deposit(address token, uint256 amount) external nonReentrant {
    require(amount > 0, "amount = 0");
    require(token == address(DUSD), "Not allowed token deposited");

    IERC20(token).transferFrom(msg.sender, address(this), amount);
    _balanceOf[msg.sender][token] = _balanceOf[msg.sender][token].add(amount);
  }

  /**
   * @notice Withdraw token to safeguard
   * @param token The token address to withdraw, can only be DUSD
   * @param amount The amount of token to withdraw
   */
  function withdraw(address token, uint256 amount) external nonReentrant {
    require(amount > 0, "amount = 0");
    require(token == address(DUSD), "Not allowed token deposited");
    require(_balanceOf[msg.sender][token] >= amount, "Not enough amount to withdraw");

    lendingPool.withdrawValidation(msg.sender, token, amount);

    _balanceOf[msg.sender][token] = _balanceOf[msg.sender][token] - amount;
  ...
  LendingPoolStorage
{
  ...
  struct LiquidationCallLocalVars {
    ...
    uint256 healthFactor;
    uint256 liquidatorPreviousBTokenBalance;
  ...
  function liquidationCall(
    ...
    vars.collateralBtoken = IBToken(collateralReserve.bTokenAddress);

    vars.userCollateralBalance = vars.collateralBtoken.balanceOf(user);
    ...
    if (!receiveBToken) {
      uint256 currentAvailableCollateral =
        IERC20(collateralAsset).balanceOf(address(vars.collateralBtoken));
    ...
    debtReserve.updateInterestRates(
      debtAsset,
      debtReserve.bTokenAddress,
      vars.actualDebtToLiquidate,
      0
    );

    if (receiveBToken) {
      vars.liquidatorPreviousBTokenBalance = IERC20(vars.collateralBtoken).balanceOf(msg.sender);
      vars.collateralBtoken.transferOnLiquidation(user, msg.sender, vars.maxCollateralToLiquidate);

      if (vars.liquidatorPreviousBTokenBalance == 0) {
...
interface ILendingPoolCollateralManager {
   ...
   * @param liquidator The address of the liquidator
   * @param receiveBToken true if the liquidator wants to receive bTokens, false otherwise
   **/
  event LiquidationCall(
   ...
   * @param receiveBToken true if the liquidators wants to receive the bTokens, false if
 ...
 * - An instance of this same contract, can't be used across different Bakery markets, due to the caching
 ...
 * @notice Interface for the Bakery borrow rate oracle. Provides the average market borrow rate to be used as a base for the stable borrow rate calculations
 **/
...
import {ILendingPool} from '../interfaces/ILendingPool.sol';
import {IBakeryOracle} from './interfaces/IBakeryOracle.sol';
import {IBToken} from '../interfaces/IBToken.sol';
import {IVariableDebtToken} from '../interfaces/IVariableDebtToken.sol';
import {IStableDebtToken} from '../interfaces/IStableDebtToken.sol';
...
contract UiPoolDataProviderV2V3 is IUiPoolDataProviderV3 {
  ...
  {
    IBakeryOracle oracle = IBakeryOracle(provider.getPriceOracle());
    ILendingPool lendingPool = ILendingPool(provider.getLendingPool());
...
/**
 * @title IBakeryOracle interface
 * @notice Interface for the Bakery oracle.
 **/

interface IBakeryOracle {
...
/**
 * @title Bakery BToken enabled to delegate voting power of the underlying asset to a different address
...
import {IBakeryIncentivesController} from '../../interfaces/IBakeryIncentivesController.sol';

/**
 * @title Bakery ERC20 BToken
 * @dev Implementation of the interest bearing token for the Bakery protocol
 * @author Bakery Finance
 */
contract BToken is
  VersionedInitializable,
  IncentivizedERC20('BTOKEN_IMPL', 'BTOKEN_IMPL', 0),
  IBToken
{
  using WadRayMath for uint256;
  using SafeERC20 for IERC20;
  ...
  IBakeryIncentivesController internal _incentivesController;
  ...
  /**
   * @dev Returns the address of the Bakery treasury, receiving the fees on this bToken
   **/
   ...
   **/
  function _getIncentivesController() internal view override returns (IBakeryIncentivesController) {
   ...
   * @dev Returns the address of the incentives controller contract
   **/
  function getIncentivesController() external view override returns (IBakeryIncentivesController) {
 ...
 * @author Bakery Finance, inspired by the Openzeppelin ERC20 implementation
 **/
abstract contract IncentivizedERC20 is Context, IERC20, IERC20Detailed {
   ...
   **/
  function _getIncentivesController() internal view virtual returns(IBakeryIncentivesController);
...
contract UiPoolDataProvider is IUiPoolDataProvider {
  ...
  IBakeryIncentivesController public immutable override incentivesController;
  IPriceOracleGetter public immutable oracle;

  constructor(IBakeryIncentivesController _incentivesController, IPriceOracleGetter _oracle) public {
...
interface IUiPoolDataProvider {
  ...
  function incentivesController() external view returns (IBakeryIncentivesController);
...
contract UiIncentiveDataProviderV2V3 is IUiIncentiveDataProviderV3 {
    ...
    returns (AggregatedReserveIncentiveData[] memory)
  {
    ...
    for (uint256 i = 0; i < reserves.length; i++) {
      ...
      try IBToken(baseData.bTokenAddress).getIncentivesController() returns (
        IBakeryIncentivesController bTokenIncentiveController
      ) {
        RewardInfo[] memory aRewardsInformation = new RewardInfo[](1);
        if (address(bTokenIncentiveController) != address(0)) {
          address aRewardToken = bTokenIncentiveController.REWARD_TOKEN();

          try bTokenIncentiveController.getAssetData(baseData.bTokenAddress) returns (
            uint256 bTokenIncentivesIndex,
            uint256 aEmissionPerSecond,
            uint256 aIncentivesLastUpdateTimestamp
          ) {
              ...
              aRewardsInformation[0] = RewardInfo(
                ...
                bTokenIncentivesIndex,
                bTokenIncentiveController.DISTRIBUTION_END(),
                0,
                IERC20Detailed(aRewardToken).decimals(),
                bTokenIncentiveController.PRECISION(),
                0
              );
            ...
            (
              ...
              uint256 aIncentivesLastUpdateTimestamp,
              uint256 bTokenIncentivesIndex
            ) = bTokenIncentiveController.assets(baseData.bTokenAddress);
            aRewardsInformation[0] = RewardInfo(
              getSymbol(aRewardToken),
              aRewardToken,
              address(0),
      ...
      try IStableDebtToken(baseData.stableDebtTokenAddress).getIncentivesController() returns (
        IBakeryIncentivesController sTokenIncentiveController
      ) {
      ...
      try IVariableDebtToken(baseData.variableDebtTokenAddress).getIncentivesController() returns (
        IBakeryIncentivesController vTokenIncentiveController
      ) {
      ...
      try IBToken(baseData.bTokenAddress).getIncentivesController() returns (
        ...
        if (address(bTokenIncentiveController) != address(0)) {
          ...
          aUserRewardsInformation[0] = UserRewardInfo(
            ...
            address(0),
            aRewardToken,
            bTokenIncentiveController.getUserUnclaimedRewards(user),
            bTokenIncentiveController.getUserAssetData(user, baseData.bTokenAddress),
            0,
            0,
            IERC20Detailed(aRewardToken).decimals()
          );
...
contract UiIncentiveDataProviderV2 is IUiIncentiveDataProviderV2 {
    ...
    returns (AggregatedReserveIncentiveData[] memory)
  {
    ...
    for (uint256 i = 0; i < reserves.length; i++) {
      ...
      try IStableDebtToken(baseData.bTokenAddress).getIncentivesController() returns (IBakeryIncentivesController bTokenIncentiveController) {
        if (address(bTokenIncentiveController) != address(0)) {
          address aRewardToken = bTokenIncentiveController.REWARD_TOKEN();
      ...
      try IStableDebtToken(baseData.stableDebtTokenAddress).getIncentivesController() returns (IBakeryIncentivesController sTokenIncentiveController) {
        if (address(sTokenIncentiveController) != address(0)) {
      ...
      try IStableDebtToken(baseData.variableDebtTokenAddress).getIncentivesController() returns (IBakeryIncentivesController vTokenIncentiveController) {
        if (address(vTokenIncentiveController) != address(0)) {
          address vRewardToken = vTokenIncentiveController.REWARD_TOKEN();
    ...
    returns (UserReserveIncentiveData[] memory)
  {
    ...
    for (uint256 i = 0; i < reserves.length; i++) {
      ...
      try IBToken(baseData.bTokenAddress).getIncentivesController() returns (IBakeryIncentivesController bTokenIncentiveController) {
        if (address(bTokenIncentiveController) != address(0)) {
          address aRewardToken = bTokenIncentiveController.REWARD_TOKEN();
          aUserIncentiveData.tokenincentivesUserIndex = bTokenIncentiveController.getUserAssetData(
            user,
            baseData.bTokenAddress
          );
          aUserIncentiveData.userUnclaimedRewards = bTokenIncentiveController.getUserUnclaimedRewards(
            user
          );
          aUserIncentiveData.tokenAddress = baseData.bTokenAddress;
          aUserIncentiveData.rewardTokenAddress = aRewardToken;
          aUserIncentiveData.incentiveControllerAddress = address(bTokenIncentiveController);
      ...
      userReservesIncentivesData[i].bTokenIncentivesUserData = aUserIncentiveData;

      UserIncentiveData memory vUserIncentiveData;

      try IVariableDebtToken(baseData.variableDebtTokenAddress).getIncentivesController() returns(IBakeryIncentivesController vTokenIncentiveController) {
        if (address(vTokenIncentiveController) != address(0)) {
          address vRewardToken = vTokenIncentiveController.REWARD_TOKEN();
...
} from '../libraries/bakery-upgradeability/InitializableImmutableAdminUpgradeabilityProxy.sol';
import {ReserveConfiguration} from '../libraries/configuration/ReserveConfiguration.sol';
import {ILendingPoolAddressesProvider} from '../../interfaces/ILendingPoolAddressesProvider.sol';
import {ILendingPool} from '../../interfaces/ILendingPool.sol';
import {IERC20Detailed} from '../../dependencies/openzeppelin/contracts/IERC20Detailed.sol';
 ...
 * @author Bakery Finance
 * @dev Implements the configuration methods for the Bakery protocol
 **/
...
contract LendingPoolConfigurator is VersionedInitializable, ILendingPoolConfigurator {
    ...
    address bTokenProxyAddress =
      _initTokenWithProxy(
        ...
        abi.encodeWithSelector(
          ...
          IBakeryIncentivesController(input.incentivesController),
  ...
  function _checkNoLiquidity(address asset) internal view {
    ...
    uint256 availableLiquidity = IERC20Detailed(asset).balanceOf(reserveData.bTokenAddress);
...
import {
  VersionedInitializable
} from '../../libraries/bakery-upgradeability/VersionedInitializable.sol';
import {IncentivizedERC20} from '../IncentivizedERC20.sol';
import {Errors} from '../../libraries/helpers/Errors.sol';
...
/// - Owned by the Bakery governance system, allowed to add sources for assets, replace them
///   and change the fallbackOracle
contract BakeryOracle is IPriceOracleGetter, Ownable {
  using SafeERC20 for IERC20;
  ...
  /// @notice External function called by the Bakery governance to set or replace sources of assets
  /// @param assets The addresses of the assets
  /// @param sources The address of the source of each asset
  ...
  /// - Callable only by the Bakery governance
...
contract UniswapRepayAdapter is BaseUniswapAdapter {
  ...
  function swapAndRepay(
    ...
    if (collateralAsset != debtAsset) {
      ...
      _pullBToken(
        collateralAsset,
        collateralReserveData.bTokenAddress,
        msg.sender,
        amounts[0],
        permitSignature
      );
...
abstract contract BaseUniswapAdapter is FlashLoanReceiverBase, IBaseUniswapAdapter, Ownable {
   ...
   * @param permitSignature struct containing the permit signature
   */
  function _pullBToken(
    address reserve,
    address reserveBToken,
    address user,
    uint256 amount,
    PermitSignature memory permitSignature
  ) internal {
    if (_usePermit(permitSignature)) {
      IERC20WithPermit(reserveBToken).permit(
        user,
        address(this),
        permitSignature.amount,
        permitSignature.deadline,
    ...
    // transfer from user to adapter
    IERC20(reserveBToken).safeTransferFrom(user, address(this), amount);
...
contract UniswapLiquiditySwapAdapter is BaseUniswapAdapter {
   ...
   * @dev Swaps the received reserve amount from the flash loan into the asset specified in the params.
   * The received funds from the swap are then deposited into the protocol on behalf of the user.
   * The user should give this contract allowance to pull the BTokens in order to withdraw the underlying asset and
  ...
  struct SwapAndDepositLocalVars {
    uint256 i;
    uint256 bTokenInitiatorBalance;
    uint256 amountToSwap;
    uint256 receivedAmount;
    address bToken;
  }
  ...
  function swapAndDeposit(
    ...
    for (vars.i = 0; vars.i < assetToSwapFromList.length; vars.i++) {
      vars.bToken = _getReserveData(assetToSwapFromList[vars.i]).bTokenAddress;

      vars.bTokenInitiatorBalance = IERC20(vars.bToken).balanceOf(msg.sender);
      vars.amountToSwap = amountToSwapList[vars.i] > vars.bTokenInitiatorBalance
        ? vars.bTokenInitiatorBalance
  ...
  function _swapLiquidity(
    ...
    vars.bToken = _getReserveData(assetFrom).bTokenAddress;

    vars.bTokenInitiatorBalance = IERC20(vars.bToken).balanceOf(initiator);
    vars.amountToSwap = swapAllBalance && vars.bTokenInitiatorBalance.sub(premium) <= amount
      ? vars.bTokenInitiatorBalance.sub(premium)
      : amount;
    ...
    _pullBToken(assetFrom, vars.bToken, initiator, vars.amountToPull, permitSignature);
...
contract BTokensAndRatesHelper is Ownable {
  ...
  event deployedContracts(address bToken, address strategy);
