...
import "./SYRUP Token.sol";

// import "@nomiclabs/buidler/console.sol";
interface IMigratorChef {
    // Perform LP token migration from legacy BullionFX to BullSwap.
    // Take the current LP token address and return the new LP token address.
    // Migrator should have full access to the caller's LP token.
    // Return the new LP token address.
    //
    // XXX Migrator must have allowance access to BullionFX LP tokens.
    // BullSwap must mint EXACTLY the same amount of BullSwap LP tokens or
    // else something bad will happen. Traditional BullionFX does not
...
contract MasterChef is OwnableUpgradeSafe {
    ...
    // Update the given pool's BULL allocation point. Can only be called by the owner.
    function set(
        uint256 _pid,
        uint256 _allocPoint,
        bool _withUpdate
        ...
        returns (uint256)
    {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            accBullPerShare = accBullPerShare.add(
                bullReward.mul(1e12).div(lpSupply)
            );
        }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 bullReward = multiplier
            ...
            .mul(pool.allocPoint)
            .div(totalAllocPoint);
        bull.mint(devaddr, bullReward.div(10));
        bull.mint(address(syrup), bullReward);
    ...
    function deposit(uint256 _pid, uint256 _amount) public {
        require(_pid != 0, "deposit BULL by staking");
    ...
    function withdraw(uint256 _pid, uint256 _amount) public {
        require(_pid != 0, "withdraw BULL by unstaking");

        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
        require(user.amount >= _amount, "withdraw: not good");
        updatePool(_pid);
        uint256 pending = user.amount.mul(pool.accBullPerShare).div(1e12).sub(
            user.rewardDebt
        );
        if (pending > 0) {
            safeBullTransfer(msg.sender, pending);
    ...
    // Stake BULL tokens to MasterChef
    ...
    // Withdraw BULL tokens from STAKING.
    ...
    function safeBullTransfer(address _to, uint256 _amount) internal {
        syrup.safeBullTransfer(_to, _amount);
    }
...
contract SyrupBar is ERC20Upgradeable {
    ...
    function initialize(BullToken _bull) public initializer {
        bull = _bull;
        ERC20Upgradeable.__ERC20_init("SyrupBar Token", "SYRUP");
        OwnableUpgradeSafe.__Ownable_init();
    }

    // Safe bull transfer function, just in case if rounding error causes pool to not have enough BULLs.
    function safeBullTransfer(address _to, uint256 _amount) public onlyOwner {
        uint256 bullBal = bull.balanceOf(address(this));
        if (_amount > bullBal) {
            bull.transfer(_to, bullBal);
        } else {
    ...
    function delegateBySig(
        ...
        require(
            signatory != address(0),
            "BULL::delegateBySig: invalid signature"
        );
        require(
            nonce == nonces[signatory]++,
            "BULL::delegateBySig: invalid nonce"
        );
        ...
        returns (uint256)
    {
        require(
            ...
            "BULL::getPriorVotes: not yet determined"
        );
    ...
    function _writeCheckpoint(
        ...
        uint32 blockNumber = safe32(
            block.number,
            "BULL::_writeCheckpoint: block number exceeds 32 bits"
        );
...
contract BullToken is Initializable, OwnableUpgradeSafe, ERC20Upgradeable {
    function initialize() public initializer {
        ERC20Upgradeable.__ERC20_init("BULL", "BULL");
        OwnableUpgradeSafe.__Ownable_init();
    }
