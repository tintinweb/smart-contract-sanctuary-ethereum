    ...
    Ownable
{
    ...
    IFables2 private fables2Address = IFables2(0x21013cf1b7995daBa4196478026c29F70b2546C4);
    ...
    function setMerkleRoot(uint8 _table, bytes32 _merkleRoot )external onlyRole(MANAGER_ROLE){
            merkleRoot[_table] = _merkleRoot;
    }
    ...
    function mintVIP2(bytes32[] calldata _merkleProof) public virtual{
        require(!alreadymint[msg.sender],"You are already minted");
        require(block.timestamp >= startTime[1] && block.timestamp < endTime[1], "It is not sellTime");
        bytes32 leaf = keccak256(abi.encodePacked(msg.sender));
        require(MerkleProof.verify(_merkleProof, merkleRoot[1], leaf), "invalid proof, you're not on the pregame list.");
        require(_tokenIdTracker + 1 <= 3000,"NFT is already limited");
        alreadymint[msg.sender] = true;
        _tokenIdTracker = _tokenIdTracker + 1;
        _safeMint(msg.sender, 1);
