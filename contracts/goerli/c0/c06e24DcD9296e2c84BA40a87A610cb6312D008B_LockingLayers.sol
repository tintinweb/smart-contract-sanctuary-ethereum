...
contract LockingLayers is ILockingLayers, ERC721, VRFConsumerBase, Ownable {
    ...
    uint16 constant TOTAL_ARTWORK_SUPPLY = 12;
    ...
    uint16 constant BLOCKS_PER_ACTION = 4;
    ...
    uint16[3] totalArtworksInTier = [2, 8, 2];

    // remaining artworks in each tier that can be purchased
    uint16[3] artworksRemainingInTier = [2, 8, 2];

    // CID of the mappings from canvasId to canvas! View on ipfs.
    string constant provinanceRecord =
        "QmZ7Lpf5T4NhawAKsWAmomG5sxkSN6USfRVRW5nMzjrHdD";
    ...
    uint256 public constant AUTOMATIC_START_BLOCK_DELAY = 18;
    ...
    string baseURI =
        "https://su3p5zea28.execute-api.us-west-1.amazonaws.com/prod/metadata/";
     ...
     */
    function currentPrice(ArtworkTier tier)
        public
        view
        override
        returns (uint256)
    {
        ...
        if (tier == ArtworkTier.ENTHUSIAST) {
            ...
            uint256 midwayPrice = collectorMin +
                (collectorMax - collectorMin) /
                2;
            ...
            if (artworksRemainingInTier[1] > midwayArtworks) {
                ...
                numerator =
                    midwayArtworks -
                    (totalArtworksInTier[1] - artworksRemainingInTier[1]);
     ...
     */
    function getTierPurchaseData(ArtworkTier tier)
        public
        view
        override
     ...
     */
    function totalArtworks() public pure override returns (uint16) {
     ...
     */
    function availableArtworks() public view override returns (uint16) {
        return
            artworksRemainingInTier[0] +
            artworksRemainingInTier[1] +
            artworksRemainingInTier[2];
    }

    /**
     * @dev The number of blocks remaining until next layer is revealed.
     */
    function blocksUntilNextLayerRevealed()
        public
        view
        override
        returns (uint256)
    {
        ...
        return
            ((getAction()) * BLOCKS_PER_ACTION + startBlock()) - block.number;
    }
     ...
     */
    function canLock(uint256 artworkId) public view override returns (bool) {
        // check locks
     ...
     */
    function getCanvasIds(uint256 artworkId)
        public
        view
        override
        returns (
    ...
    function getCurrentLayerAndAction() public view returns (uint8, uint8) {
        ...
        uint8 currentActionZeroIndexed = actionZeroIndexed -
            (currentLayer * ACTIONS_PER_LAYER);
     ...
     */
    function purchase(ArtworkTier tier)
        public
        payable
        override
        returns (uint256)
    ...
    function requestStartArtwork() public returns (bytes32) {
        ...
        require(
            availableArtworks() == 0 ||
                (firstPurchaseBlock > 0 &&
                    block.number >
                    firstPurchaseBlock + AUTOMATIC_START_BLOCK_DELAY),
            "Cannot start the artwork before all works are sold out or until after sale period"
        );
        /**
        ADDED FOR MOCK TO HELP TESTING
        */
        bytes32 requestId = requestRandomness(keyHash, vrfFee, block.number);
    ...
    function startArtwork(uint256 randomSeed) internal {
        ...
        require(
            !hasStarted(),
            "Artwork has already started, seed cannot be set twice!"
        );
    ...
    function lockLayer(uint256 artworkId) public override {
        ...
        uint16 currentCanvasId = getCanvasIdForAction(
            artworkId,
            currentLayer,
            currentAction
        );
        ...
        uint8 currLockedValue = lockedLayerHistory[currentCanvasId][
            currentLayer
        ];
        ...
        if (currentLayer < NUM_LAYERS - 1) {
            canvasIdStartOffsets[currentLayer + 1] =
                (block.number + canvasIdStartOffsets[currentLayer]) %
                TOTAL_ARTWORK_SUPPLY;
        }
    ...
    function getCanvasIdForAction(
        ...
        uint16 currCanvasId = uint16(
            ((canvasIdStartOffsets[layer] + artworkId) % (NUM_CANVAS_IDS)) + 1
        );
        ...
        for (uint8 i = 1; i < action; i++) {
            ...
            uint8 canvasLockedOnAction = lockedLayerHistory[currCanvasId][
                layer
            ];
    ...
    function withdrawlFunds() public {
        (bool success, ) = address(0x1Df3260ea86d338404aC49F3D33cec477a46A827)
...
interface ILockingLayers {
    ...
    /**
     * @dev Emit on return of random number
     */
...
abstract contract VRFConsumerBase is VRFRequestIDBase {
    ...
    LinkTokenInterfaceSimplified internal immutable LINK;
