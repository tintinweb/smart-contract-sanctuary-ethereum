...
contract TTS is Initializable, ERC721Upgradeable, OwnableUpgradeable, UUPSUpgradeable, SignedAllowance, TimeWeaverKeeperControl, ReentrancyGuardUpgradeable {
    ...
    function TimeTravellerMint (bool whitelisted, uint256 nonce, bytes memory signature, uint256 quantity) external payable {
        ...
        if (whitelisted) {
            ...
            if (quantity != 0 && msg.value >= quantity * 1000000000000000000) {
    ...
    function moveToken(address from, address to, uint256 tokenId) external onlyTimeKeeper {
        if (to == address(0)) {
            _burn(tokenId);
        } else {
    ...
    function withdrawFunds() external onlyTimeKeeper nonReentrant {
