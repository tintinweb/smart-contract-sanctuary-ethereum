...
/// @author Krishna Sharma - <[email protected]>
contract Enum {
    enum Operation {
        Call,
        DelegateCall
...
abstract contract Modifiers is Validators, Enum {
    //
    //  Modifiers
    //
...
contract PayoutManager is Modifiers, Executor {
  ...
  function processPayouts(
  ...
  
    {
      ...
      checkSignatures(safeAddress, txHash, txHashData, signatures);
      ...
      require(gasleft() >= ((safeTxGas * 64) / 63).max(safeTxGas + 2500) + 500, "GS010");
        // Use scope here to limit variable lifetime and prevent `stack too deep` errors
        {
            ...
            execute(to, 0, data, operation, gasPrice == 0 ? (gasleft() - 2500) : safeTxGas);
  ...
  function checkSignatures(
    ...
    uint256 _threshold = orgs[safeAddress].threshold;
    // Check that a threshold is set
    require(_threshold > 0, "CS001");
    checkNSignatures(safeAddress, dataHash, data, signatures, _threshold);
  }
  ...
  function checkNSignatures(
    ...
    require(signatures.length >= requiredSignatures.mul(65), "CS020");
    ...
    for (i = 0; i < requiredSignatures; i++) {
      ...
      if (v == 0) {
        ...
        require(uint256(s) >= requiredSignatures.mul(65), "CS021");

        // Check that signature data pointer (s) is in bounds (points to the length of data -> 32 bytes)
        require(uint256(s).add(32) <= signatures.length, "CS022");
      ...
      console.log(currentOwner,"owner hu mai");
      require(
        currentOwner > lastOwner &&
          orgs[safeAddress].approvers[currentOwner] != address(0) &&
          currentOwner != SENTINEL_ADDRESS,
        "CS026"
      );
 ...
 * Renamed from SafeMath to SafeMath to avoid conflicts
...
import "./Base/ApproverManager.sol";
import "./Base/PayoutManager.sol";
...
contract OrganizerV2 is ApproverManager, PayoutManager, Pausable {
    //  Events
    //  Org Onboarded
    event OrgOnboarded(
    ...
    //  Onboard an Org
    function onboard(address[] calldata _approvers, uint96 threshold) external {
        ...
        orgs[safeAddress].threshold = threshold;
