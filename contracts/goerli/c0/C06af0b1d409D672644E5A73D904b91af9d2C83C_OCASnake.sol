...
contract OCASnake is Ownable, ERC721A, ERC2981, ReentrancyGuard, OCASnakeArt {
        ...
        returns (uint256)
    {
        ...
        uint256 combined = snakeNameIndex * adjectiveIndex;
    ...
    function recursiveNameCheck(
        uint256 tokenId,
        uint256 seed,
        uint256[] memory foundNames
    ) public view returns (string memory) {
        //for through all IDS until this one to see if they match as well

        uint256 combined = returnName(0 + seed, seed);
        foundNames[combined] = 1;

        string memory name;

        if (seed == tokenId) {
            // if (
            //     keccak256(abi.encodePacked(foundNames[1])) ==
            //     keccak256(abi.encodePacked((foundName)))
            // ) {
            //     return recursiveNameCheck(tokenId, seed + 1, foundNames);
            // }

            if (foundNames[combined] == 1) {
                name = "TERD";
                // if (
                //     keccak256(
                //         abi.encodePacked(foundNames[foundNames.length - 1])
                //     ) == keccak256(abi.encodePacked(foundName))
                // ) {
                //     return "DUPE";
                // } else return recursiveNameCheck(tokenId, seed, foundNames);
            }
        } else name = "TERD23";
        return name;
