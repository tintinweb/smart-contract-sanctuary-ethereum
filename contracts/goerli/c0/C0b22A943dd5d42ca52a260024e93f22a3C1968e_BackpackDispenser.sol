...
contract BackpackDispenser is Administrable, ReentrancyGuard {
  ...
  function swapToken(address[] memory contracts, uint256[] memory tokenIds) external nonReentrant {
    for (uint256 i = 0; i < contracts.length; ) {
      ...
      IYellowBackpack(backpackContract).airdrop(swapRule.newTokenId, msg.sender, 1);
  ...
  function burnBackpack(uint256 tokenId) external nonReentrant {
    address backpackItemContract = burnRules[tokenId];
    if (backpackItemContract == address(0)) revert BurnRuleNotDefined();

    IYellowBackpack(backpackContract).burn(msg.sender, tokenId);
    IBackpackItem(backpackItemContract).fulfill(msg.sender);

    // TODO: Where did we endup with Fire?
...
interface IBackpackItem {
  function fulfill(address recipient) external;
}
