...
// File contracts/BaseStake.sol


contract BaseStake is IBaseStake {
    using SafeERC20 for IERC20;
    // ERC20 basic token contract being held
    IERC20 private immutable m_BaseCoin;

    mapping(address => stakeUnit) m_mpStake;
    ...
    function newStake(
        ...
        for (uint i = 0; i < aryTime.length; i++) {
            m_mpStake[investor].lockTime.push(aryTime[i]);
            m_mpStake[investor].lockAmount.push(aryAmount[i]);
            m_mpStake[investor].lockTag.push(0);
        }

        m_mpStake[investor].nonce = nonce;
        emit evNewStake(investor, nonce, aryTime, aryAmount);
    }

    function claimStake() public {
        uint256 total = 0;
        for (uint256 i = 0; i < m_mpStake[msg.sender].lockTag.length; i++) {
            if (m_mpStake[msg.sender].lockTag[i] == 1) {
                continue;
            }
            if (block.timestamp > m_mpStake[msg.sender].lockTime[i]) {
                m_mpStake[msg.sender].lockTag[i] = 1;
                total += m_mpStake[msg.sender].lockAmount[i];
            }
        }
        m_BaseCoin.transfer(msg.sender, total);
    }

    function queryStake(
        address user
    ) external view returns (stakeUnit memory userStake) {
...
contract HTXCoin is BaseCoin {
    // 1. State Variables & Constructor
    // 1.1 Sepcific Pool

    // /* development, 10000 ArtX$
    uint256[3] private m_specPools = [
        4500 * 10 ** uint256(decimals()), // 0 - Reward
        3400 * 10 ** uint256(decimals()), // 1 - Invest
        2100 * 10 ** uint256(decimals()) // 2 - Mint
    ];
    ...
    /* production
    uint256[3] private m_specPools = [
        450000000 * 10 ** uint256(decimals()), // 0 - Reward
        340000000 * 10 ** uint256(decimals()), // 1 - Invest
        210000000 * 10 ** uint256(decimals()) // 2 - Mint
    ];
    // */
    ...
    function claimHTX(
        ...
        // require claim pool be sufficient
        require((m_specPools[0] - amount) >= 0, "exceed pool's limit");
        m_specPools[0] -= amount;

        _claimCoin(nonce, amount, tag, this.claimHTX.selector, signature);
    }

    // 2.2 consuming
    function consumeHTX(uint256 amount) external {
        _consumeCoin(amount);
    }
    ...
    function mintHTX(
        ...
        // require mint pool be sufficient
        require(m_specPools[2] - amount >= 0, "exceed pool's limit");
        m_specPools[2] -= amount;
    ...
    function stakeHTX(
        ...
        // less than 10 staking
        require(aryTime.length <= 10, "no more 10 arrays");
        // check total amount for staking
        uint256 sum = 0;
        for (uint256 i = 0; i < aryAmount.length; i++) {
            sum += aryAmount[i];
        }
        // require invest pool be sufficient
        require(m_specPools[1] - sum >= 0, "exceed pool's limit");
        m_specPools[1] -= sum;

        _stakeCoin(conBaseStake, staker, nonce, aryTime, aryAmount, signature);
    }

    // 3. Views for Internal Data
    // 3.1 query the specific pool's remaining, return m_specPools
    function querySpecPool() external view returns (uint256[3] memory) {
        return m_specPools;
    }
}