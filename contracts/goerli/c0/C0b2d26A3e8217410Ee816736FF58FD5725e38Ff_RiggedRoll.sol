...
contract RiggedRoll is Ownable {
    ...
    // 收到eth的事件
    event Log(uint amount, address callAddress, bool success, uint nonce);

    error SendNotEnoughValue();
    ...
    function withdraw(address _addr, uint256 _amount) public onlyOwner {
        require(address(this).balance >= _amount, "amount is enough");
    ...
    function riggedRoll() public {
        ...
        if (roll <= 2 ) {
            ...
            emit Log(address(this).balance, address(diceGame), true, diceGame.nonce());
            return;
        }
        revert FailRoll();
    }
