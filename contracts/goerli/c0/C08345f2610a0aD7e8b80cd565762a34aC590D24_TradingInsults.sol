...
error SaleClosed();
error NoTokensDue(address account);
...
contract TradingInsults is ERC721A, IERC2981, Ownable {
    ...
    KingOfTheHill private _kingOfTheHill;
    ...
    function setKingMessage(bytes32 message) external onlyOwner {
        _kingOfTheHill.message = message;
    }
    ...
    function mint(
        ...
        if (SaleType.standardInsult == saleType) {
            ...
            _mintHelper(insultee, 1, 1);
            _config.saleExpiry += _config.timerConfig.extensionSeconds;
            _kingOfTheHill.king = msg.sender;
            _kingOfTheHill.message = kingMessage;
    ...
    function getLastTokenOwner() public view returns (address) {
        return _tradingInsultsPOIs.ownerOf(_tradingInsultsPOIs.totalSupply());
    }
    ...
    function sharesBps(address account) public view returns (uint256) {
        ...
        if (_config.shares.kingShares != 0) {
            if (account == _kingOfTheHill.king)
    ...
    function release(address payable account) external {
        ...
        if (payment == 0) revert NoTokensDue(account);
        if (_config.shares.lastMinterShares != 0) {
            if (account == getLastTokenOwner())
                _config.shares.lastMinterShares = 0;
        }
        if (_config.shares.kingShares != 0) {
            if (account == _kingOfTheHill.king) _config.shares.kingShares = 0;
        }
        if (account == owner()) {
            if (_config.shares.kingShares != 0 || _config.shares.lastMinterShares != 0) revert NoTokensDue(account);
        }
    ...
    function releaseERC20(IERC20 token) public {
        ...
        if (payment == 0) revert NoTokensDue(owner());
