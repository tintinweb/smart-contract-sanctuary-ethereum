...
contract PreSale is Ownable {
    ...
    function newCampaign(
        ...
        require(data.durationDays <= configs.maxduration, "Duration days is large!");
        require(data.durationDays >= configs.minduration, "Duration days is small!");
        require(data.startTime > block.timestamp - 3600000, "Start date too past!");
        ...
        require(
            tokencontract.allowance(campaignowner, address(this)) >= data.supply,
            "Owner allowance not enough!"
        );

        if (configs.maxmarketcap > 0) {
            require(
                totalsupply * data.price < configs.maxmarketcap,
                "Price too high!"
            );
        }
        ...
        emit StartCampaign(token, data.price, data.supply, data.durationDays);
    }

    function buyToken(uint campaignId, uint amount)
        public
        payable
        enable
        returns (bool)
    {
        ...
        require(msg.value == price + wage_saler, "Paid value is incorrect!");
        ...
        payable(c.owner).transfer(price - wage_buyer);
        ...
        campaignFunders[campaignId].push(funderId);
        fundersOwner[buyer].push(funderId);

        emit BuyToken(campaignId, amount, price, wage_saler, wage_buyer);
        return true;
    }
