    ...
    PausableUpgradeable
{
    ...
    function _transfer(
        ...
        if (whitelistedAddress[sender] || whitelistedAddress[recipient]) {
            ...
            if (!inSwap && swapEnabled) {
                ...
                //./revert if greater than 1% total holding
                //./check and change for final
                ...
                if (totalTax > tax) revert TaxLimitExceeded();
                ...
                //./check if this is a trade on dex
                if (
                    (pair == recipient && sender != address(this)) ||
                    (pair == sender && recipient != address(this))
                ) {
                    //./in a trade on dex - handle treasury, liquidity and VB burn

                    //./SELL Transaction: only buy back and burn and add liquidity on sell
                    if (sender != pair) {
                        ...
                        //./transfer to the VBC Treasury
                        super._transfer(
                            sender,
                            treasury,
                            feeAmount.sub(
                                (
                                    _feesOnContract.mul(liquidityTax()).div(
                                        totalTax
                                    )
                                ).add(
                                        (
                                            _feesOnContract.mul(vbTax()).div(
                                                totalTax
                                            )
                                        )
                                    )
    ...
    function swapTokensForEthLiquidity(uint256 tokenAmount)
        private
        lockTheSwap
    {
    ...
    function openTrading() external onlyOwner {
        ...
        IUniswapV2Router _uniswapV2Router = IUniswapV2Router(Liquidity.ROUTER);
        uniswapV2Router = _uniswapV2Router;
        _approve(address(this), address(uniswapV2Router), _maxSupply());
        uniswapV2Pair = IUniswapV2Factory(_uniswapV2Router.factory())
            .createPair(address(this), Liquidity.WETH);
