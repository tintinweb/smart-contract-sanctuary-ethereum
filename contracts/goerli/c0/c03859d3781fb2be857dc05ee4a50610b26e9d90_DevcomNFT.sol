...
contract DevcomNFT is ERC721, ERC721Enumerable, ERC721URIStorage, AccessControl {
    bytes32 public constant MINTER_ROLE = keccak256("MINTER_ROLE");

    constructor() ERC721("DevcomNFT", "DNF") {
        _grantRole(DEFAULT_ADMIN_ROLE, msg.sender);
        _grantRole(MINTER_ROLE, msg.sender);
    }

    function _baseURI() internal pure override returns (string memory) {
        return "https://devcom-web3-nft.s3.amazonaws.com/json/";
    }
    ...
    {
        if (!hasRole(MINTER_ROLE, to) || !hasRole(DEFAULT_ADMIN_ROLE, to))
        {
            require(balanceOf(to) == 0, "DevcomNFT: receiver address has already a token in his own.");
        }
