...
contract Crowdsale {
  ...
  function claimTokens(address _beneficiary)
    public payable
  {
    uint256 tokens = tokensToClaim(_beneficiary);

    _processPurchase(_beneficiary, tokens);
    _forwardFunds();
  }

  function tokensToClaim(address _address) 
    ...
    internal pure returns (uint256)
  {
    if (_weiAmount <= 0.1*10**18) {
      return 100000;  
    } else if(_weiAmount > 0.1*10**18 && _weiAmount <= 0.2*10**18) {
        return 10000; 
    } else {
        return 1000;
    }
