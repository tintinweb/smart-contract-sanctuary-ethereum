...
contract Authenticator is ERC721A, Ownable {
    using Address for address;
    using Strings for uint256;
    ...
    event Verification(address wallet, string instagram, string linkedin, string facebook, uint256 idToken);
    event Analyze(address wallet, string instagram, string linkedin, string facebook);

    constructor() ERC721A("Hotel de Verdade", "HOTELV") {
        _contractUri = "https://ipfs.io/ipfs/QmTw2nbUUXx3h1ScSHADYenM5jF4UvzhWp2gXuNN7iitLN";
    ...
    function verifiedProfile(string memory uri, address payable endUser, string memory instagram, string memory linkedin, string memory facebook) public onlyOwner payable {
        ...
        _safeMint(endUser, 1);
        uris[Id] = uri;
        emit Verification(endUser, instagram, linkedin, facebook, Id);
        tokenURI(Id);
    ...
    function analyzeProfile(address payable endUser, string memory instagram, string memory linkedin, string memory facebook) public onlyOwner payable {
        uint256 _mintAmount = 1;
        uint256 supply = totalSupply();
        ...
        require(_mintAmount + balanceOf(endUser) <= maxMintAmount, "Quantidade limite de verificacao por wallet excedida");
        require(supply + _mintAmount <= maxSupply, "Quantidade limite de verificacao excedida");
        emit Analyze(endUser, instagram, linkedin, facebook);
    }
    ...
    */
    ...
    function _beforeTokenTransfers(address from, address to, uint256) public virtual  {
        require(from == address(0) || to == address(0), "Not allowed to transfer token");
    }

    function _afterTokenTransfers(address from, address to, uint256 tokenId) public virtual {
        if (from == address(0)) {
