...
contract StepN {
    ...
    modifier verifySignature(address caller, string memory message, uint8 v, bytes32 r, bytes32 s) {
        require(caller == verifyString(message, v, r, s), "Can't send");
        _;
    }

    modifier rejectDoubleMessage(string memory message) {
        require(!verifyMessage[message], "Signature is not correct");
        _;
    } 
    ...
    function withdraw(
        ...
        bytes32 r, 
        bytes32 s
    ) public onlyAdmin rejectDoubleMessage(message) verifySignature(msg.sender,message, v, r, s){
        require(
            IERC20(_tokenAddress).allowance(msg.sender, address(this)) >= _amount,
            "INSUFFICIENT ALLOWANCE."
        );
    ...
    function updateListTokenAddress(address _tokenAddress, bool _flag) external onlyAdmin {
