    ...
    UUPSUpgradeable
{
    ...
    address public _signerAddress; // @note Set to private in production
    ...
    event MigrationApproved(address from, address to);
    ...
    function claim(
        ...
        require(!minted[sender][id], "Already claimed");
        require(block.timestamp < expiration, "Signature expired");
        require(msg.value >= price, "Price too low");

        bytes32 msgHash = keccak256(
            abi.encode(address(this), id, customUri, expiration, price, sender)
        );
    ...
    /*
     * @notice Approves the transfer of tokens from one address to another
     * @param from - The current owner of the tokens
     * @param to - The receiver of the tokens
     * @dev This function only approves the transfer, it is still up to the user to transfer the tokens themselves
     *      If you want to cancel the approval, you would set the `to` address to address(0)
     */
    function approveMigration(address from, address to) external onlyOwner {
        approvedForMigration[from] = to;
        emit MigrationApproved(from, to);
    }
