...
contract RevShare {
    ...
    // Mapping of all referals
    mapping(string => address payable) public referrals;
    // Number of Nouns Token requiered
    uint256 socialNounsRequiered;
    // Max amount of referals
    uint256 maxAmountReferrals;
    // Number of current referals
    uint256 currentAmountReferrals;
     ...
     * @dev This function can only be called once.
     */
    function initialize(address socialNounsToken_, uint256 socialNounsRequiered_,uint256 share_,address weth_,uint256 reservePrice_,uint8 minBidIncrementPercentage_) public virtual{
        require(init == false, 'RevShare::initialize: can only initialize once');
        require(msg.sender == admin, 'RevShare::initialize: admin only');
        require(socialNounsToken_ != address(0), 'RevShare::initialize: invalid nouns address');
        require(share_ > 0, 'RevShare::initialize: share cant be 0');
    ...
    /** 
     * @notice Add a referral code.
     * @dev ReferalCodes have to start with "Social".
     */
    function addReferalCode(string memory referralCode) public{
        require(socialNounsTokenInterface.balanceOf(msg.sender) > socialNounsRequiered, 'You need more Social Nouns');
        require(referrals[referralCode] == address(0), 'This code is already used');
        require(currentAmountReferrals <= maxAmountReferrals,'RevShare::addReferalCode: The max amount of referals is reached');
        bytes memory valueAsBytes16 = bytes(referralCode);
        require(valueAsBytes16.length > 1,'RevShare::addReferalCode: The referal code has to start with social');
        referrals[referralCode] = payable(msg.sender);
        currentAmountReferrals ++;
    }

    /**
     * @notice Set the max amount of referals.
     */
    function setMaxAmountofReferalCodes(uint256 newMaxAmountReferals) public{
        require(msg.sender == admin, 'RevShare::setMaxAmountofReferalCodes: admin only');
        maxAmountReferrals = newMaxAmountReferals;
    }
    ...
    function createBid(uint256 nounId_,string memory referalCode_) external payable {
        ...
        if(nounId_ > LastBid.nounId && LastBid.nounId != 0){
            if(socialNounsTokenInterface.balanceOf(address(this)) > 0){
                ...
                address payable referal = referrals[LastBid.referralCode];
        ...
        try auction.createBid{value: msg.value}(nounId_) {
            ...
            LastBid.referralCode = referalCode_;
    ...
    function _setPendingAdmin(address newPendingAdmin) external {
        // Check caller = admin
        require(msg.sender == admin, 'RevShare::_setPendingAdmin: admin only');
    ...
    function _acceptAdmin() external {
        ...
        require(msg.sender == pendingAdmin && msg.sender != address(0), 'RevShare::_acceptAdmin: pending admin only');
