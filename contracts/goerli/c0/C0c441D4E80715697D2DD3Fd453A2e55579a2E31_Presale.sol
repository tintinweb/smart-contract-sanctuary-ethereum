...
contract Presale is Airdrop {
    ...
    function setErc20Token(address token) external onlyAdmin {
        erc20Token = token;
    }
    ...
    function burnAll(address collection) external {
    ...
    function isWhitelist(
        ...
        bytes32 leaf
    ) internal view override checkAddCollection(collection) returns (bool) {
    ...
    function getTokens(
        ...
        bytes32 accountHash = keccak256(abi.encodePacked(msg.sender));
        ...
        bool isWhitelistAccount = isWhitelist(collection, proofs, accountHash);
        uint256 cTotalPrice = getPriceCollection(
            collection,
            isWhitelistAccount
        ) * tokenAmount;
        ...
        checkAddCollection(collection)
    {
        ...
        require(
            address(this).balance >= cTotalFunds,
        ...
        emit ReturningFunds(msg.sender, cTotalFunds);
    }

    function getPriceCollection(address collection, bool isWhite)
        private
        view
        checkAddCollection(collection)
        returns (uint256)
    {
        Collection memory cCollection = collectionInformations[collection];

        return isWhite ? cCollection.whitelistPrice : cCollection.publicPrice;
    }

    event ReturningFunds(address account, uint256 amount);
...
contract Airdrop is OwnableExt {
    ...
    mapping(bytes32 => mapping(address => bool)) public dropTokenAccounts;
    ...
    function getTokens(
        ...
        bytes32 accountHash = keccak256(
            abi.encodePacked(msg.sender, tokenAmount)
        );
        ...
        require(
            !dropTokenAccounts[accountHash][collection],
        ...
        dropTokenAccounts[accountHash][collection] = true;
    ...
    function isWhitelist(
        ...
        bytes32[] calldata proofs,
        bytes32 leaf
    ) internal view virtual checkAddCollection(collection) returns (bool) {
...
contract ERC721Collection is ERC721A, OwnableExt, IERC721Collection {
    bool public isBurnt;
