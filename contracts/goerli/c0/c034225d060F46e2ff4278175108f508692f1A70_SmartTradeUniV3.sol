...
contract SmartTradeUniV3 is IUniswapV3SwapCallback, Pausable, Ownable {
    ...
    function masterSwap(
        ...
        if (priceToken0Usd < 0 || priceToken1Usd < 0 || ethPriceUsd < 0 || bribePercent < 0) {
        ...
        if (bribePercent > 0) {
            ...
            if (msg.value > 0) {
                int256 profitEth = profit  / ethPriceUsd;
                uint256 bribe = uint256(profitEth * bribePercent / 100);
                ...
                uint256 bribeMod = msg.value - bribe;
                if (bribeMod > 0)
                {
                    payable(msg.sender).transfer(bribeMod);
                }
            }
        }
