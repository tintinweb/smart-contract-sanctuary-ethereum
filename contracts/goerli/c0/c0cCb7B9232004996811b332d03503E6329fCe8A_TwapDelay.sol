...
interface ITwapDelay {
    ...
    ) external view returns (Orders.OrderStatus);
...
contract TwapDelay is ITwapDelay {
    ...
    function getOrderStatus(
        ...
        return orders.getOrderStatus(orderId, orderType, validAfterTimestamp);
    }
    ...
    function execute(Orders.Order[] calldata _orders) external payable override lock {
        ...
        emit Execute(msg.sender, ordersLen);
        ...
        for (uint256 i; i < ordersLen; ++i) {
            ...
            Orders.OrderType orderType = Orders.decodeType(_orders[i].orderType);
    ...
    function executeWithdraw(Orders.Order calldata order) internal {
        uint256 gasStart = gasleft();
        Orders.WithdrawOrder memory withdrawOrder = orders.dequeueWithdrawOrder(order);
        (bool executionSuccess, bytes memory data) = address(this).call{
    ...
    function executeSell(Orders.Order calldata order) internal {
        uint256 gasStart = gasleft();
        (Orders.SellOrder memory sellOrder, uint256 amountLimit) = orders.dequeueSellOrder(order);
    ...
    function executeBuy(Orders.Order calldata order) internal {
        uint256 gasStart = gasleft();
        (Orders.BuyOrder memory buyOrder, uint256 amountLimit) = orders.dequeueBuyOrder(order);
    ...
    function performRefund(Orders.Order calldata order, bool shouldRefundEth) internal {
        Orders.OrderType orderType = Orders.decodeType(order.orderType);
        require(orderType != Orders.OrderType.Empty, 'TD41');
        bool canOwnerRefund = uint256(order.validAfterTimestamp).add(365 days) < block.timestamp;

        if (orderType == Orders.OrderType.Deposit) {
            (Orders.DepositOrder memory depositOrder, , ) = Orders.getDepositOrder(order);
            (, address token0, address token1) = orders.getPairInfo(depositOrder.pairId);
            ...
            Orders.WithdrawOrder memory withdrawOrder = Orders.getWithdrawOrder(order);
            (address pair, , ) = orders.getPairInfo(withdrawOrder.pairId);
            ...
            (Orders.SellOrder memory sellOrder, ) = Orders.getSellOrder(order);
            (, address token0, address token1) = orders.getPairInfo(sellOrder.pairId);
            ...
            (Orders.BuyOrder memory buyOrder, ) = Orders.getBuyOrder(order);
            (, address token0, address token1) = orders.getPairInfo(buyOrder.pairId);
    ...
    function retryRefund(Orders.Order calldata order) external override lock {
        ...
        require(orders.isRefundFailed(order.orderId), 'TD21');
    ...
    function cancelOrder(Orders.Order calldata order) external override lock {
        orders.verifyOrder(order);
        require(
            orders.getOrderStatus(order.orderId, Orders.decodeType(order.orderType), order.validAfterTimestamp) ==
                Orders.OrderStatus.EnqueuedReady,
            'TD52'
        );
        require(uint256(order.validAfterTimestamp).sub(orders.delay).add(ORDER_CANCEL_TIME) < block.timestamp, 'TD1C');
