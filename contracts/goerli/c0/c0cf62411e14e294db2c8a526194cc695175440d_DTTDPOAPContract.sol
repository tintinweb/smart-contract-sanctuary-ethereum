...
contract DTTDPOAPContract is ERC1155Supply, Ownable {
    struct Token {
        string name;
        string symbol;
        string tokenURI;
    ...
    function setupPOAP(
        ...
        require(bytes(_symbol).length > 0, "Missing symbol");
    ...
    function airdropBatch(
        ...
        require(_ids.length > 0, "Nothing to airdrop");
        require(_ids.length == _quantities.length, "Parameter length mismatch");
        for (uint256 i = 0; i < _ids.length; ++i) {
            require(_ids[i] < numberOfTokens, "Invalid ID");
    ...
    function burnBatch(
        ...
        require(_ids.length > 0, "Nothing to burn");
