    ...
    AccessControl
{
        ...
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        ...
        emit VaultRegistered(tokenType, vaultAddress);
    }
    ...
    function mapToken(
        ...
        require(
            rootToChildToken[rootToken] == bytes32(0) &&
     ...
     */
    function cleanMapToken(address rootToken, bytes32 childToken)
        external
        override
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
    ...
    function remapToken(
        ...
        // cleanup old mapping
        bytes32 oldChildToken = rootToChildToken[rootToken];
        address oldRootToken = childToRootToken[childToken];
        if (rootToChildToken[oldRootToken] != bytes32(0)) {
            rootToChildToken[oldRootToken] = bytes32(0);
    ...
    function _depositEtherFor(address user, bytes32 ccdUser) private {
        bytes memory depositData = abi.encode(msg.value);
        _depositFor(user, ccdUser, ETHER_ADDRESS, depositData);
        // payable(typeToVault[tokenToType[ETHER_ADDRESS]]).transfer(msg.value);
        // transfer doesn't work as expected when receiving contract is proxified so using call
        (
            bool success, /* bytes memory data */

        ) = typeToVault[tokenToType[ETHER_ADDRESS]].call{value: msg.value}("");
        if (!success) {
    ...
    function _depositFor(
        ...
        require(
            rootToChildToken[rootToken] != bytes32(0) && tokenType != 0,
    ...
    //  * the transaction log is then sent to token vault to handle it accordingly
    //  *
...
interface IRootChainManager {
    ...
    function cleanMapToken(address rootToken, bytes32 childToken) external;
...
abstract contract RootChainManagerStorage {
    ...
    mapping(address => bytes32) public rootToChildToken;
