  ...
  ReentrancyGuard
{
  ...
  /// @notice an array of all the router addresses that are allowed to swap
  ...
  constructor(
    ...
    //exclude routers from tax and cooldown and max wallet balance 
    address[] memory _routers = new address[](2);
    _routers[0] = address(uniswapV2Router);
    _routers[1] = address(uniswapV2Factory);

    for (uint256 i = 0; i < _routers.length; i++) {
      addrsExcludedFromCooldown[_routers[i]] = true;
      addrsExcludedFromTax[_routers[i]] = true;
      addrsExcludedFromMaxWalletBalance[_routers[i]] = true;
  ...
  function addRouterAddress(address[] calldata addrs) external onlyOwner {
    for (uint256 i = 0; i < addrs.length; i++) {
      require(addrs[i] != address(0), "NIBBLE: Address cannot be zero");
      routers.push(addrs[i]);
      isRouter[addrs[i]] = true;
    }
  }

  function removeRouterAddress(address[] calldata addrs) external onlyOwner {
    for (uint256 i = 0; i < addrs.length; i++) {
      require(addrs[i] != address(0), "NIBBLE: Address cannot be zero");
      for (uint256 j = 0; j < routers.length; j++) {
        if (routers[j] == addrs[i]) {
          routers[j] = routers[routers.length - 1];
          routers.pop();
          isRouter[addrs[i]] = false;
          break;
        }
      }
    }
  ...
  function _transfer(
    ...
    require(recipient != address(0), "NIBBLE: Transfer to the zero address");
    require(sender != address(0), "NIBBLE: Transfer from the zero address");
    require(amount > 0, "NIBBLE: Transfer amount must be greater than zero");
    //check if the address is excluded from tax
    if (isRouter[msg.sender] || isRouter[recipient]) {
      if (addrsExcludedFromTax[sender]) {
        super._transfer(sender, recipient, amount);
        cooldowns[sender] = block.timestamp + cooldownTime;
