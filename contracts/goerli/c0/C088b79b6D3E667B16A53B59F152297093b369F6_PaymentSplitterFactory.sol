...
contract PaymentSplitterFactory is Ownable {
    ...
    /**
     * @dev access control not needed, anyone may use this contract in order to deploy a payment splitter
     * @dev this function is not returning the address of the deployed clone, as it is not needed by other smart contracts, frontends should rely on the emitted event.
     */
    function deployClone(
        ...
        // deploying clone, returns clone address
        clone = master.clone();

        // initializing deployed clone
        PaymentSplitter(payable(clone)).initialize(
            _payees,
            _shares,
            _totalShares
        );
    ...
    // function deployCloneDeployERC1155(
    //     address[] calldata _royaltyReceivers,
    //     uint256[] calldata _royaltyShares,
    //     bytes32 _tokenURI,
    //     uint256 _totalShares
     ...
     * @param _commissionShares SHOULD be set shares lower than 100% or 10000 BPS, this is not enforced, as it does not represent a security risk, however should they be >= 10000 BPS the `_trade` function in the marketplace will not pay the commissions at all (the entire amount goes to the seller instead).
     *  Should _commissionShares also be greater than (price - fee - royalties), say 8500 bps on secondary sales (5% fee, 10% royalties), the `trade` function will revert because royalties and fees MUST be paid
     *
     */
    function deployCloneCreateOrder(
        ...
        // TODO what is cheaper in order to get the sum of shares; for loop o external call?
        // uint256 shares = PaymentSplitter(payable(clone)).totalShares();
        uint256 shares;
        for (uint256 i = 0; i < _commissionShares.length; i++) {
            shares += _commissionShares[i];
        }
        ...
        (bool success, ) = ninfaMarketplace.call(
            abi.encodeWithSignature(
                ...
                _price, // _price
                _amount, // _amount
                clone, // _commissionReceiver
                shares, // _commissionBps
                msg.sender // _seller
            )
        );
        require(success);
    ...
    function deployCloneAcceptOffer(
        ...
        // NOTE for now payment splitter does not have any setter functions for changing payees/shares, if this changes, the below check will not be sufficient anymore
        (bool success, ) = ninfaMarketplace.call(
            abi.encodeWithSignature( // todo encodeWithSelector
        ...
        onlyOwner
    {
        ...
        // todo emit event to store marketplace address info
    }
