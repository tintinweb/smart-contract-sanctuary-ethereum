...
contract CollectionStaking is ReentrancyGuard, Ownable {
    ...
    event BatchStaked (uint256[] indexed tokenIds);
    event BatchUnstaked (uint256[] indexed tokenIds);
    event RewardsClaimed (uint256 indexed amount);

    uint256 public rewardsPerHour = 1 ether;
    ...
    function stake(uint256[] calldata _tokenIds) external nonReentrant {
        uint tokenLength = _tokenIds.length;
        uint i;
        for (i; i <  tokenLength; ++i) {
          _stake(_tokenIds[i]);
        }
        totalStaked += tokenLength;
        emit BatchStaked(_tokenIds);
    }
    ...
    function getStakedTokens(address _user) public view returns (StakedToken[] memory) {
        if (stakers[_user].amountStaked > 0) {
            ...
            for (j; j < stakers[_user].stakedTokens.length; ++j) {
    ...
    function changeRewardsPerHour(uint256 _newRperH) external onlyOwner {
        rewardsPerHour = _newRperH;
    }
