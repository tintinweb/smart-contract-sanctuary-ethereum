...
contract BookVesting is Ownable, ReentrancyGuard {
    ...
    uint256 public immutable startTime; // beginning of 36 month vesting window (unix timestamp)
    uint256 public immutable totalAllotments; // Total $BOOK tokens vested
    uint256 public totalWithdrawn;
    bool private allotmentsSet;

    string public constant CONTRACT_DESCRIPTION = "Contributor Vesting";
    uint256 private constant A_FACTOR = 10**18;
    uint256 public constant PERIOD_DIVISOR = 27777777777777777;
    ...
    constructor(uint256 _startTime, address _book, uint256 _totalVested) {
    ...
    function setAllotments(
        ...
        require(IERC20(book).balanceOf(address(this)) == totalAllotments, "Contract does not hold $BOOK");
        uint256 s;
    ...
    function claimTokens() external nonReentrant {
        ...
        if (withdrawable > 0) {
            IERC20(book).safeTransfer(msg.sender, withdrawable);
    ...
    function claimableBook() external view returns (uint256 withdrawable) {
        ...
        uint256 periodAmount = (totalAllotments * PERIOD_DIVISOR) / A_FACTOR; // 1/36th of user's original allotment;

        // uint256 vestedTime = (getElapsedTime() / 30 days) + 1;
        uint256 vestedTime = (getElapsedTime() / 3 minutes) + 1;
    ...
    function _calculateWithdrawableAmounts(address _address) internal view returns (uint256 withdrawable) {
        ...
        uint256 periodAmount = (original * PERIOD_DIVISOR) / A_FACTOR; // 1/36th of user's original allotment;
