...
contract TokenOffRampStorage is ITokenOffRampStorage, Pausable {
    ...
    modifier onlyMultisig() {
        require(msg.sender == protocolOwner, 'Only Multisig wallet can access');
        _;
    }
    ...
    function setPendingMultisig(address _multisig)
        external
        override
        whenPaused
        onlyMultisig
        isValidAddress(protocolOwner, _multisig)
    {
        pendingProtocolOwner = _multisig;
    }

    /// @notice Claim pending ownership
    function approvePendingMultisig() external override whenPaused {
        require(msg.sender == pendingProtocolOwner, 'Only pending wallet can approve');
        emit UpdatedMultisig(msg.sender, protocolOwner, pendingProtocolOwner);
...
interface ITokenOffRampStorage {
    ...
    event UpdatedExpiryPeriod(address indexed operator, uint256 oldPeriod, uint256 newPeriod);
    ...
    function setPendingMultisig(address multisig) external;

    function approvePendingMultisig() external;
