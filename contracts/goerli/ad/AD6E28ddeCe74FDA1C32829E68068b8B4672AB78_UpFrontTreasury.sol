...
contract UpFrontTreasury is Context, Ownable, ReentrancyGuard {
  ...
  address[] public tokenInterfaceList;
  mapping(address => tokenInterfaceDataStruct) public tokenInterfaceData;
  ...
  function addTokenInterface(address _contract) external isOwner {
    ...
    tokenInterfaceData[_contract].tokenInterface = IERC20(_contract);
  }
  ...
  function incrementAsOwner(uint256 i) public isOwner {
    count += i;
  }

  function incrementAsPublic(uint256 i) public {
    count += i;
  }

  function withdraw(address payable _to, uint256 _amount) public payable isOwner nonReEntrant {
  ...
  function withdrawToken(address _contract, address _to, uint256 _amount) public isOwner nonReEntrant {
    ...
    bool success = tokenInterfaceData[_contract].tokenInterface.transfer(_to, _amount);
  ...
  function getTokenBalance(address _contract) public view returns (uint256) {
    ...
    return tokenInterfaceData[_contract].tokenInterface.balanceOf(address(this));
  }
}