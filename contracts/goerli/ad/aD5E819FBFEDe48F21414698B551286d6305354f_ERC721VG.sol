...
contract ERC721VG is ERC721A, Ownable, ERC2981 {
     ...
     * @member royaltyPercent: % of royalties for each sale
     * @member burnable: whether owner can burn tokens or not
     */
    struct SaleSettings {
    ...
    constructor(
        ...
        // mint and transfer to founders
        if (
            saleSettings_.founders.length > 0 &&
            saleSettings_.amountForFounders.length > 0
        ) {
            _airdrops(
                saleSettings_.founders,
                saleSettings_.amountForFounders,
                false
            );
        }
    ...
    function mint1(uint256 roundId, uint256 amount) external payable virtual {
        ...
        require(
            _waveToRounds[waveId][roundId].merkleRoot == bytes32(0),
            "Need merkle validation"
        );
        require(
            balanceOf(msg.sender) + amount <= globalMintCapPerWallet ||
                globalMintCapPerWallet == 0,
     ...
     */
    function setGlobalMintCapPerWallet(uint32 newMaxMints)
        external
        virtual
        onlyOwner
    {
    ...
    */
    ...
    function safeMint(address wallet, uint256 amount) internal virtual {
        ...
        require(
            _totalMinted() + amount <= collectionSize,
            "Collection supply exceeded"
        );
