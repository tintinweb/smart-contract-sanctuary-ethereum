...
// File: Sapphire/MarketplaceContract.sol
...
///         collectable tokens and payments may be at risk.
...
contract SapphireMarketplace is AccessControlEnumerableUpgradeable, PausableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    /// @notice SAPPHIRE_MARKETPLACE_ADMIN_ROLE role hash
    bytes32 public constant SAPPHIRE_MARKETPLACE_ADMIN_ROLE = keccak256("SAPPHIRE_MARKETPLACE_ADMIN_ROLE");
    ...
    function initialize(
        ...
        initializer
    {
        require(address(_initialPaymentToken) != address(0), "SapphireMarketplace: cannot set address(0)");

        __AccessControl_init_unchained();
        __Pausable_init_unchained();
        __ReentrancyGuard_init_unchained();

        _setRoleAdmin(SAPPHIRE_MARKETPLACE_ADMIN_ROLE, SAPPHIRE_MARKETPLACE_ADMIN_ROLE);
        _grantRole(SAPPHIRE_MARKETPLACE_ADMIN_ROLE, msg.sender);
    ...
    function createListing(
        ...
        whenNotPaused
    {
        require(listings[_nftAddress][_tokenId][_msgSender()].quantity == 0, "SapphireMarketplace: already listed");
    ...
    function updateListing(
        ...
        whenNotPaused
    {
        require(listings[_nftAddress][_tokenId][_msgSender()].quantity > 0, "SapphireMarketplace: not listed item");
    ...
    function _createListingWithoutEvent(
        ...
        internal
    {
        require(_expirationTime > block.timestamp, "SapphireMarketplace: invalid expiration time");
        require(_pricePerItem >= MIN_PRICE, "SapphireMarketplace: below min price");

        if (tokenApprovals[_nftAddress] == TokenApprovalStatus.ERC_721_APPROVED) {
            IERC721Upgradeable nft = IERC721Upgradeable(_nftAddress);
            require(nft.ownerOf(_tokenId) == _msgSender(), "SapphireMarketplace: not owning item");
            require(nft.isApprovedForAll(_msgSender(), address(this)), "SapphireMarketplace: item not approved");
            require(_quantity == 1, "SapphireMarketplace: cannot list multiple ERC721");
            ...
            require(nft.balanceOf(_msgSender(), _tokenId) >= _quantity, "SapphireMarketplace: must hold enough nfts");
            require(nft.isApprovedForAll(_msgSender(), address(this)), "SapphireMarketplace: item not approved");
            require(_quantity > 0, "SapphireMarketplace: nothing to list");
        } else {
            revert("SapphireMarketplace: token is not approved for trading");
        }
    ...
    {
        // Validate buy order
        require(_msgSender() != _owner, "SapphireMarketplace: Cannot buy your own item");
        require(_quantity > 0, "SapphireMarketplace: Nothing to buy");

        // Validate listing
        Listing memory listedItem = listings[_nftAddress][_tokenId][_owner];
        require(listedItem.quantity > 0, "SapphireMarketplace: not listed item");
        require(listedItem.expirationTime >= block.timestamp, "SapphireMarketplace: listing expired");
        require(listedItem.pricePerItem > 0, "SapphireMarketplace: listing price invalid");
        require(listedItem.quantity >= _quantity, "SapphireMarketplace: not enough quantity");
        require(listedItem.pricePerItem <= _maxPricePerItem, "SapphireMarketplace: price increased");

        // Transfer NFT to buyer, also validates owner owns it, and token is approved for trading
        if (tokenApprovals[_nftAddress] == TokenApprovalStatus.ERC_721_APPROVED) {
            require(_quantity == 1, "SapphireMarketplace: Cannot buy multiple ERC721");
    ...
    function setFee(uint256 _newFee) public onlyRole(SAPPHIRE_MARKETPLACE_ADMIN_ROLE) {
        require(_newFee <= MAX_FEE, "SapphireMarketplace: max fee");
        fee = _newFee;
    ...
    function setFeeRecipient(address _newFeeRecipient) public onlyRole(SAPPHIRE_MARKETPLACE_ADMIN_ROLE) {
        require(_newFeeRecipient != address(0), "SapphireMarketplace: cannot set 0x0 address");
    ...
    function setTokenApprovalStatus(address _nft, TokenApprovalStatus _status) external onlyRole(SAPPHIRE_MARKETPLACE_ADMIN_ROLE) {
        if (_status == TokenApprovalStatus.ERC_721_APPROVED) {
            require(IERC165Upgradeable(_nft).supportsInterface(INTERFACE_ID_ERC721), "SapphireMarketplace: not an ERC721 contract");
        } else if (_status == TokenApprovalStatus.ERC_1155_APPROVED) {
            require(IERC165Upgradeable(_nft).supportsInterface(INTERFACE_ID_ERC1155), "SapphireMarketplace: not an ERC1155 contract");
        }
    ...
    function pause() external onlyRole(SAPPHIRE_MARKETPLACE_ADMIN_ROLE) {
        _pause();
    }

    /// @notice Unpauses the marketplace, all functionality is restored
    /// @dev    This is callable only by the owner.
    function unpause() external onlyRole(SAPPHIRE_MARKETPLACE_ADMIN_ROLE) {
        _unpause();
    }
}