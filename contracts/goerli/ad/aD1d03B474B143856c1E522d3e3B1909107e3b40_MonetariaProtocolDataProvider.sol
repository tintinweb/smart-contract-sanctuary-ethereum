...
import {WadRayMath} from '../protocol/libs/math/WadRayMath.sol';
...
/**
 * @title MonetariaProtocolDataProvider
 * @author Monetaria
 * @notice Peripheral contract to collect and pre-process information from the Pool.
 */
contract MonetariaProtocolDataProvider is IPoolDataProvider {
  ...
  /**
   * @notice Returns the list of the existing MTokens in the pool.
   * @return The list of MTokens, pairs of symbols and addresses
   */
  ...
  /**
   * @notice Returns the config data of the reserve
   ...
   **/
  function getReserveConfigData(address asset)
    external
    view
    returns (
      uint256 decimals,
  ...
  {
    DataTypes.ReserveConfigMap memory config = IPool(ADDRESSES_PROVIDER.getPool())
      .getConfig(asset);

    (ltv, liquidationThreshold, liquidationBonus, decimals, reserveFactor, ) = config
      .getParams();

    (isActive, isFrozen, borrowingEnabled, stableBorrowRateEnabled, ) = config.getFlags();
  ...
  function getReserveEModeCategory(address asset) external view returns (uint256) {
    DataTypes.ReserveConfigMap memory config = IPool(ADDRESSES_PROVIDER.getPool())
      .getConfig(asset);
    return config.getEModeCategory();
  }
  ...
  {
    (borrowCap, supplyCap) = IPool(ADDRESSES_PROVIDER.getPool()).getConfig(asset).getCaps();
  }
  ...
  function getPaused(address asset) external view returns (bool isPaused) {
    (, , , , isPaused) = IPool(ADDRESSES_PROVIDER.getPool()).getConfig(asset).getFlags();
  }
  ...
  function getSiloedBorrowing(address asset) external view returns (bool) {
    return IPool(ADDRESSES_PROVIDER.getPool()).getConfig(asset).getSiloedBorrowing();
  }
  ...
  function getLiquidationProtocolFee(address asset) external view returns (uint256) {
    return IPool(ADDRESSES_PROVIDER.getPool()).getConfig(asset).getLiquidationProtocolFee();
  }
  ...
  function getUnbackedMintCap(address asset) external view returns (uint256) {
    return IPool(ADDRESSES_PROVIDER.getPool()).getConfig(asset).getUnbackedMintCap();
  }
  ...
  function getDebtCeiling(address asset) external view returns (uint256) {
    return IPool(ADDRESSES_PROVIDER.getPool()).getConfig(asset).getDebtCeiling();
  }
  ...
  function getDebtCeilingDecimals() external pure returns (uint256) {
    return ReserveConfig.DEBT_CEILING_DECIMALS;
  }
   ...
   * @return totalMToken The total supply of the mToken
    ...
    )
  {
    ...
    return (
      ...
      IERC20Detailed(reserve.mTokenAddress).totalSupply(),
  ...
  /**
   * @notice Returns the total supply of mTokens for a given asset
   * @param asset The address of the underlying asset of the reserve
   * @return The total supply of the mToken
   **/
  function getMTokenTotalSupply(address asset) external view override returns (uint256) {
    DataTypes.ReserveData memory reserve = IPool(ADDRESSES_PROVIDER.getPool()).getReserveData(
      asset
    );
    return IERC20Detailed(reserve.mTokenAddress).totalSupply();
  }
   ...
   * @return currentMTokenBalance The current MToken balance of the user
    ...
    )
  {
    ...
    DataTypes.UserConfigMap memory userConfig = IPool(ADDRESSES_PROVIDER.getPool())
      .getUserConfig(user);
   ...
   * @param asset The address of the underlying asset of the reserve
   * @return mTokenAddress The MToken address of the reserve
...
interface IPoolDataProvider {
   ...
   * @return The total supply of the mToken
   **/
  function getMTokenTotalSupply(address asset) external view returns (uint256);
