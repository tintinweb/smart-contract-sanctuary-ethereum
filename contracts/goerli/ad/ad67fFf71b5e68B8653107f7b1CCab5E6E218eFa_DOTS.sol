...
contract DOTS is ERC721AQueryable, AccessControl {
    ...
    /**
     * @notice Admin function to add Shiny traits to lucky dots metadata
     */
    function addShinyDots(
        uint256[] calldata _tokenIds
    ) external onlyRole(USER_ROLE) {
        for (uint256 i; i<_tokenIds.length; ) {
            Metadata storage _data = metadata[_tokenIds[i]];
            if (_data.evoNum == 0) revert TokenDoesNotExist();
            _data.isShiny = true;
            unchecked { i++; }
        }
        emit ShinyDotsAdded(_tokenIds);
    }
