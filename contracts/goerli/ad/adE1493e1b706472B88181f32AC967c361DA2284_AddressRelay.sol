...
contract AddressRelay is IAddressRelay, Ownable {
    ...
    function getImplAddress(
        ...
        implAddress_ = selectorToImplAddress[_functionSelector];
        if (implAddress_ == address(0)) {
            implAddress_ = fallbackImplAddress;
        }
        require(implAddress_ != address(0), "Function does not exist");
    }
        ...
        returns (Implementation[] memory impls_)
    {
        ...
        impls_ = new Implementation[](trueImplAddressCount);
        for (uint256 i = 0; i < trueImplAddressCount; i++) {
            if (implAddresses[i] == address(0)) {
                continue;
            }
            ...
            impls_[i] = Implementation(implAddress, selectors_);
        }
    }
    ...
    function getSelectorsForImplAddress(
        ...
        for (uint256 i = 0; i < selectors.length; i++) {
            if (selectorToImplAddress[selectors[i]] == _implAddress) {
                selectors_[selectorIndex] = selectors[i];
                selectorIndex++;
            }
        }
    }
