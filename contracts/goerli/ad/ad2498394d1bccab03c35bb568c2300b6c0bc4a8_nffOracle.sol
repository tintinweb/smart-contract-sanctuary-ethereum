...
contract nffOracle is ChainlinkClient, ConfirmedOwner {
    ...
    modifier byOwner() {
        require(msg.sender == theOwner, "Sorry you are not the owner");
        _;
    }
    ...
    function withdrawLink() public byOwner {
        LinkTokenInterface link = LinkTokenInterface(chainlinkTokenAddress());
        require(link.transfer(msg.sender, link.balanceOf(address(this))), 'Unable to transfer');
    }

    //Set the nffBank address
    function setBankAddr(address contractAddr) public byOwner{
        bankaddr = contractAddr;
    }

    //Set url and path for api fetch
    function setApi(string memory url, string memory jsPath) public byOwner{
        osURL = url;
        jsonPATH = jsPath;
    }
