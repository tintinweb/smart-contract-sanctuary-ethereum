...
//        vestingStartTime = _tgeTime;
    ...
    function registerMultiplyUsdPurchases(Phase _phase, address[] calldata _investors, uint256[] calldata _usdAmounts) public onlyOwner {
    ...
    function registerMultiplyPurchases(Phase _phase, address[] calldata _investors, uint256[] calldata _amounts) public onlyOwner {
    ...
    function getAvailableAmount() public view returns(uint256) {
        return getAvailableAmountByInvestor(msg.sender);
    }

    function getInvestedAmount() public view returns(uint256) {
        return getInvestedAmountByInvestor(msg.sender);
    }
    ...
    function releaseAvailable() public {
        if(getAvailableAmount() > 0) {
            for(uint _phaseIndex = uint(Phase.SEED); _phaseIndex < uint(Phase.PUBLIC); _phaseIndex++) {
                releaseAvailableByPhaseAndInvestor(Phase(_phaseIndex), msg.sender);
            }
        }
    }

    function releaseAvailableByInvestor(address _investor) public onlyOwner {
    ...
    function releaseAvailableByPhaseAndInvestor(Phase _phase, address _investor) public {
        require(_investor == msg.sender || msg.sender == owner(), "Only investor or owner can release");
        require(_investor != address(0x0), "Invalid beneficiary address");
    ...
    function getWithdrawableAmountByPhase(Phase _phase) public view returns(uint256) {
    ...
    function withdraw(Phase _phase, uint256 _amount) public onlyOwner {
        require(_amount > 0, "Invalid amount");
    ...
    function updatePrice(Phase _phase, uint256 _price) public onlyOwner {
        require(_price > 0, "Invalid price");
    ...
    function buy(uint256 _busdAmount, uint256 _maxPrice) public {
        ...
        uint256 _allowedAmount = busd.allowance(msg.sender, address(this));
        ...
        token.transfer(msg.sender, _phetaAmount);
        busd.transferFrom(msg.sender, owner(), _busdAmount);

        publicSaleRegistry.push(PublicSale(msg.sender, _busdAmount, _phetaAmount));
