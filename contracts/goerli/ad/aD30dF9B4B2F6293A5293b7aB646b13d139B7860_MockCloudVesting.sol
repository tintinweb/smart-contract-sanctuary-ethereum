...
contract MockCloudVesting is CloudVesting {
    ...
    function setStage(uint stage) external{
        if(stage==0){
            uint[3] memory unlockDelts;
            for(uint i=1; i<3; i++){
                unlockDelts[i]=unlockDates[i]-unlockDates[i-1];
            }
            unlockDates[0]=block.timestamp+1000;
            for(uint i=1; i<4; i++){
                unlockDates[i]+=unlockDelts[i-1];
            }
        }
        if(stage >=1 && stage <=4){
            uint delt = block.timestamp - unlockDates[stage-1];
            for(uint i=0; i<4; i++){
                unlockDates[i]-=delt;
            }
        }
        if(stage ==5){
            ...
            unlockDates[3]=block.timestamp-1000;
            for(uint i=4; i>1; i--){
                unlockDates[i-2]-=unlockDelts[i-1];
            }
        }
    }
