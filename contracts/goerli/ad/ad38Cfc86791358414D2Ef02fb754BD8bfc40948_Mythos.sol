  ...
  Pausable
{
  ...
  event SetPublicMintTime(uint256 newMintTime);

  event SetMerkleRoot(bytes32 newRoot);
  ...
  uint256 private constant ABSOLUTE_MAX_SUPPLY = 9966;
  uint256 private constant PRESALE_MAX_SUPPLY = 996;
  ...
  constructor(
    ...
    uint256 mintTime,
    bytes32 root
  ) ERC721A('Mythos', 'P10V2') Ownable() {
    withdrawAddress = _withdrawAddress;
    emit SetWithdrawAddress(_withdrawAddress);

    tokenBaseURI = tokenBaseURI_;
    emit SetBaseUri(tokenBaseURI_);

    publicMintTime = mintTime;
    emit SetPublicMintTime(mintTime);
  ...
  function _verifiedMint(bytes memory signature, uint256 quantity)
    internal
    whenNotPaused
  {
    require(
    ...
    require(
      quantity < maxTokenMint + 1,
      'You can only mint a maximum of 14 tokens at once'
    );
    require(
    ...
    require(
      ...
      'You have not supplied enough eth'
    );
  ...
  function setPublicMintTime(uint256 newPublicMintTime) external onlyOwner {
    ...
    emit SetPublicMintTime(newPublicMintTime);
  }
  ...
  function _afterTokenTransfers(
    ...
    emit BulkMint(to, quantity, startTokenId, from);
  }
