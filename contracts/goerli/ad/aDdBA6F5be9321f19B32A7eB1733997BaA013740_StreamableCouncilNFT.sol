...
interface ISuperfluid {
    ...
    **************************************************************************/
    ...
    function decodeCtx(bytes calldata ctx)
        external pure
        returns (SuperContext memory context);
...
contract CouncilNFT is ERC721Enumerable {
    ...
    // function _beforeTokenTransfer(from, to, tokenId);
}
...
contract StreamableCouncilNFT is SuperAppBase, CouncilNFT {
    ...
    function _updateOutflow(bytes memory ctx, bool useContext) private returns (bytes memory newCtx) {
        ...
        int96 inFlowRate = cfaV1Lib.cfa.getNetFlow(
            _acceptedToken,
            address(this)
        );

        if (inFlowRate < int256(totalSupply())) {
            return newCtx;
        }
        else {
            for (uint256 i = 0; i < totalSupply(); i++) {
                if (useContext) {
                    newCtx = _createFlowWithContext(newCtx, ownerOf(tokenByIndex(i)), inFlowRate / int256(totalSupply()));
                }
                else {
                    _createFlowNoContext(ownerOf(tokenByIndex(i)), inFlowRate / int256(totalSupply()));
                }
            }
        }
    }
        ...
        internal
    {
        ...
        if (outFlowRate > 0) {
            ...
            if (newReceiver != address(0)) {
                _createFlowNoContext(newReceiver, outFlowRate);
            }
        }
    ...
    function afterAgreementCreated(
        ...
        returns (bytes memory newCtx)
    {
        return _updateOutflow(_ctx, true);
    }
