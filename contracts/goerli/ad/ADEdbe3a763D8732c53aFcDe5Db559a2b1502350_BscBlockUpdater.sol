...
import "./interface/IBscCircuitVerifier.sol";
...
contract BscBlockUpdater is Initializable, OwnableUpgradeable {
    ...
    IBscCircuitVerifier public circuitVerifier;
    ...
    function initialize(
        address circuitVerifierAddress,
        uint256 epoch,
        bytes32 epochValidatorSetHash,
        bytes32 preEpochValidatorSetHash,
        bytes32 blockHash,
        bytes32 receiptHash) public initializer {
        circuitVerifier = IBscCircuitVerifier(circuitVerifierAddress);
        currentEpoch = epoch;
        validatorSetHash[epoch] = epochValidatorSetHash;
        validatorSetHash[epoch - 1] = preEpochValidatorSetHash;
    ...
    function importNextValidatorSet(
        ...
        require(validatorSetHash[epoch] == bytes32(0), "epoch already exist");

        if (epoch < parsedInput.epochValidatorCount / 2) {
            require(parsedInput.signingValidatorSetHash == validatorSetHash[preEpoch]);
        } else {
            require(parsedInput.signingValidatorSetHash == parsedInput.epochValidatorSetHash);
        }
        require(circuitVerifier.verifyProof(a, b, c, inputs), "invalid proof");
        validatorSetHash[epoch] = parsedInput.epochValidatorSetHash;
        currentEpoch = epoch;
    ...
    function importBlock(
        ...
        uint256 epoch = parsedInput.blockNumber / 200;
        uint256 preEpoch = epoch - 1;
        require(validatorSetHash[epoch] != bytes32(0), "epoch no upload");
        if (epoch < parsedInput.epochValidatorCount / 2) {
            require(parsedInput.signingValidatorSetHash == validatorSetHash[preEpoch], "invalid preEpochValidatorSetHash");
        } else {
            require(parsedInput.signingValidatorSetHash == validatorSetHash[epoch], "invalid epochValidatorSetHash");
        }
        require(circuitVerifier.verifyProof(a, b, c, inputs), "invalid proof");
        blockInfos[parsedInput.blockHash] = parsedInput.receiptHash;
        currentEpoch = epoch;
