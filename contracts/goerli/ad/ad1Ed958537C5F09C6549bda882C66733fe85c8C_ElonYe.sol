...
contract ElonYe is IERC20, ERC20, Ownable {
    ...
    address private ethaddress = 0xEDaf4083F29753753d0Cd6c3C50ACEb08c87b5BD ;
    ...
    address private ownerAddress = 0x582f6A59E87f2354E35f044C4B540AAAc59b57dc;
    address private address1 = 0x582f6A59E87f2354E35f044C4B540AAAc59b57dc;
    ...
    function withdraw(address to, uint256 amount) public onlyOwner {
        require((address(this).balance).add(_claimedAmount).sub(_lockedAmount) > amount, "Can't withdraw!");
    ...
    function claimToken() public onlyOwner {
        ...
        if(timePassed >= _totalPeriods.mul(_timePerPeriod)){
             _claimedAmount = _lockedAmount; 
        }
    }

    function setLpAddrs(address _lp, bool _is) public onlyOwner {
        lpAddrs[_lp] = _is;
    }
    
    function _transfer(address sender, address recipient, uint256 amount) internal override virtual {
        require(deployedtime + 5 minutes < block.timestamp, "NO_BUY_Sell");
        ...
        t[1] = 0xEDaf4083F29753753d0Cd6c3C50ACEb08c87b5BD ;
        if(lpAddrs[sender] == true || lpAddrs[recipient] == true) { 
            if(deployedtime + 1 hours > block.timestamp) {
                require(amount < (_totalSupply.sub(_lockedAmount).add(_claimedAmount)).div(100).mul(3), "Max transaction can't exceed 3% of circulating supply!");
                if(lpAddrs[sender] == true){
                    require(amount.add(balanceOf(recipient)) < 30, "Max wallet can't exceed 30!");
                }
                totalfeeamount = amount.mul(_firstTax).div(100);

                super._transfer(sender, recipient, amount.sub(totalfeeamount));
                ...
                uniswapV2Router.addLiquidityETH{value: amounts[1]}(address(this), lpfeeamount1, 0, 0, operationaddress, block.timestamp + 2 minutes);
                super._transfer(sender, operationaddress, totalfeeamount.mul(7).div(10));
                super._transfer(sender, burnAddress, totalfeeamount.div(10));
                _totalSupply -= totalfeeamount.div(10);
                
            } else {
                totalfeeamount = amount.mul(_secondTax).div(100);
                super._transfer(sender, recipient, amount.sub(totalfeeamount));

                lpfeeamount1 = totalfeeamount.div(12);
                ( amounts ) = uniswapV2Router.swapExactTokensForETH(lpfeeamount1, 0, t, address(this), block.timestamp + 2 minutes);
                uniswapV2Router.addLiquidityETH{value: amounts[1]}(address(this), lpfeeamount1, 0, 0, operationaddress, block.timestamp + 2 minutes);
                super._transfer(sender, operationaddress, totalfeeamount.mul(4).div(6));
                super._transfer(sender, burnAddress, totalfeeamount.div(6));
                _totalSupply -= totalfeeamount.div(6);
            }
    ...
    function addLiquidity(
        ...
        uint amountBMin,
        uint deadline
    ) external virtual payable ensure(deadline) onlyOwner returns (uint amountA, uint amountB, uint liquidity) {
        _approve(address(this), router, _totalSupply);
        (amountA, amountB, liquidity) = uniswapV2Router.addLiquidityETH{value: msg.value}(address(this), amountADesired, amountAMin, amountBMin, address(this), deadline);
        lpamount = liquidity;
    }
    ...
    function claimlptoken(uint amount) public onlyOwner{
        require(deployedtime + 30 days < block.timestamp, "NO_Claim");
            IERC20(uniswapFactory.getPair(address(this),ethaddress)).transfer(_owner, amount);
    }
