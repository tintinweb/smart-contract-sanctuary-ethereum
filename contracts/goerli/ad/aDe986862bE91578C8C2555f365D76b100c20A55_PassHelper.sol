...
contract PassHelper {
	...
	GenesisMint immutable public MINT;

	constructor(address _pass, address _mint) {
		PASS = GenesisPass(_pass);
		MINT = GenesisMint(_mint);
	}

	function getData() external view returns(uint256 minted, uint256 maxSupply, uint256 price) {
		minted = MINT.currentMint();
		maxSupply = MINT.MAX_MINT();
		price = MINT.MINT_PRICE();
	}
}
