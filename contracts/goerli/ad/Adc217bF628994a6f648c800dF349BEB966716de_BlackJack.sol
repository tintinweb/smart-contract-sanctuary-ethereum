...
contract BlackJack{
    ...
    uint256 private _randNum2;
    uint256 private _randNum3;
    ...
    function RandomNumberGenerator() internal returns (uint randomNumber) {
        uint randNonce = 2;
        _randNum = (uint(keccak256(abi.encodePacked(block.timestamp,msg.sender,randNonce)))%13 + 1);
        randNonce++;
        if(_randNum > 10)
            _randNum = 10;
    ...
    function RandomNumberGenerator2() internal returns (uint randomNumber) {
        uint randNonce = 2;
        _randNum2 = (uint(keccak256(abi.encodePacked(randNonce++)))%13 + 1);
    ...
    function RandomNumberGenerator3() internal returns (uint randomNumber) {
        uint randNonce = 202212;
        _randNum3 = (uint(keccak256(abi.encodePacked(randNonce)))%13 + 1);
        randNonce++;
        if(_randNum3 > 10)
            _randNum3 = 10;
    ...
    function Deal() internal returns (string memory) {
        ...
        _PlayerCard1 = RandomNumberGenerator2();
        if(_PlayerCard1 == 1)
            _PlayerCard1 = 11;             //A=11
        
        _PlayerCard2 = RandomNumberGenerator3();
        ...
        if(_PlayerCardTotal == 21) {
            if(_DealerCard1 == 10) {
                _DealerCard2 = RandomNumberGenerator3();
                if(_DealerCard2 == 1) 
                    _DealerCardTotal = 21;
            }
            
            if(_DealerCard1 == 1) {
                _DealerCard2 = RandomNumberGenerator2();
                if(_DealerCard2 == 10) 
                    _DealerCardTotal = 21;
            }
    ...
    function Stand() public returns (string memory) {
        ...
        while(_DealerCardTotal < 17) {
            _DealerNewCard = RandomNumberGenerator3();
