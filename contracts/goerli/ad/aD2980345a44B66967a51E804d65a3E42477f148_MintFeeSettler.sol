...
contract MintFeeSettler is IMintFeeSettler, Ownable {
    ...
    DoubleEndedQueue.Deque private _unsettle;
    DoubleEndedQueue.Deque private _unsent;
    ...
    function unsettle(uint256 i) external view returns (Settle memory) {
        return _unsettle.at(i);
    }

    function unsettleBack() external view returns (Settle memory) {
        return _unsettle.back();
    }

    function unsettleFront() external view returns (Settle memory) {
        return _unsettle.front();
    }

    function unsent(uint256 i) external view returns (Settle memory) {
        return _unsent.at(i);
    }

    function unsentBack() external view returns (Settle memory) {
        return _unsent.back();
    }

    function unsentFront() external view returns (Settle memory) {
        return _unsent.front();
    }
    ...
    function _settleLastUnclaim(uint256 needDistr, uint32 maxNonce) internal {
        uint32 nonce;
        while (!_unsettle.empty()) {
            Settle memory s = _unsettle.popFront();
        ...
        while (!_unsent.empty()) {
            if (nonce < maxNonce) {
                Settle memory s = _unsent.popFront();
                _sendeSettled(s);
                nonce++;
            } else {
                break;
    ...
    function _sendeSettled(Settle memory settle) internal {
        ...
        if (sendvalue > address(this).balance) IWETH9(_weth).withdraw(IERC20(_weth).balanceOf(address(this)));
        //solhint-disable avoid-low-level-calls
        (bool success, ) = settle.recipient.call{value: sendvalue}("");
        if (!success) {
            _unsent.pushBack(settle);
        }
    }
    ...
    function afterTokenTransfer(
        ...
        //TODO: check msg.sender is deployed by ori.

        //burn or mint
        if (from == address(0) || to == address(0)) {
            return;
        }
        if (!_config.isExchange(op)) {
            return;
        }
    ...
    function afterTokenTransfer(
        ...
        ids[0] = id;
        // TOD0: optimize signle processing.
        afterTokenTransfer(op, from, to, ids);
    }
    ...
    function _processPendingSettlement(uint256[] memory derivativeIds, address to) internal {
        ...
        uint256 maxEarnBP = _config.maxEarnBP();
        ...
        for (uint256 i = 0; i < metas.length; i++) {
            ...
            for (uint256 j = 0; j < licenses.length; j++) {
                ...
                _unsettle.pushBack(settleTemp[i]);
            }
            require(totalEarnPoint <= maxEarnBP, "over");
            //Less than 10% of the earnedPoint will be sent to the recipient
            if (totalEarnPoint < maxEarnBP) {
                _unsettle.pushBack(Settle({recipient: to, value: maxEarnBP - totalEarnPoint, index: index}));
            }
        }
    }
