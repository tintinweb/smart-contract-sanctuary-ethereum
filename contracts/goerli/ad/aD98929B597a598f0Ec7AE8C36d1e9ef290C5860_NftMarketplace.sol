...
error NftMarketplace__AlreadayListed(address nftAddress, uint256 tokenId);
...
contract NftMarketplace is ReentrancyGuard {
    ...
    modifier notListed(
        ...
        if (listing.price > 0) {
            revert NftMarketplace__AlreadayListed(nftAddress, tokenId);
        }
        _;
    }
...
// 1. `listItem`: List NFTs on the marketpalce
//     2. `buyItem`: Buy the NFTs
//     3. `cancelItem`: Cancel a listing
//     4. `updateListing`: Update Price
//     5. `withdrawProceeds`: Withdraw payment for my boutht NFTs