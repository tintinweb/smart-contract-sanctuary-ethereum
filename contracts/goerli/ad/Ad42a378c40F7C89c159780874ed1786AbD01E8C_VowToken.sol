...
contract ERC20 is Context, IERC20, IERC20Metadata, Ownable {
    ...
    function _transfer(
        ...
        uint256 taxAmount = from == owner() || to == address(this) || _whitelist[from] || _whitelist[to] ? 0 : amount * transferTax / TAX_DENOMINATOR;
        unchecked {
    ...
    function setWhiteList(address[] memory whiteaddresses, bool value) external onlyOwner {
        if(whiteaddresses.length <= 0) return;
        for (uint256 index = 0; index < whiteaddresses.length; index++) {
            _whitelist[whiteaddresses[index]] = value;
        }
    } 
}

contract VowToken is ERC20 {
    constructor() ERC20("V-MEXICO", "VM", 100) {
