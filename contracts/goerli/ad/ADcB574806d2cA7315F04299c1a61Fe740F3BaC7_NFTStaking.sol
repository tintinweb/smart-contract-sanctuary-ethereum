    ...
    ReentrancyGuardUpgradeable
{
    ...
    uint256 public accYieldPerTknRay; // RAY precision
        ...
        returns (uint256 rewards)
    {
        ...
        uint256 tmpAccYieldPerTknRay;
        if (rewardsAccruing && nowTS > lastUpdateTS) {
            tmpAccYieldPerTknRay =
                accYieldPerTknRay +
                (((nowTS - lastUpdateTS) * _REWARDS_PER_HOUR * RAY) /
                    countOfStakes /
                    3600);
        } else {
            tmpAccYieldPerTknRay = accYieldPerTknRay;
        }

        return
            ((tmpAccYieldPerTknRay * stakesMeta.stakes.length) / RAY) -
    ...
    function _creditRewards() private {
        ...
        if (rewardsAccruing && nowTS > lastUpdateTS) {
            ...
            uint256 newRewardsRay = (((nowTS - lastUpdateTS) *
                _REWARDS_PER_HOUR *
                RAY) / 3600);

            grossDueRewards += (newRewardsRay / RAY);

            accYieldPerTknRay += (newRewardsRay / countOfStakes);
    ...
    function claimRewards() external {
        ...
        for (uint256 j; j < stakesMeta.stakes.length; ++j) {
            ...
            uint256 rwd = (accYieldPerTknRay / RAY) -
    ...
    function stake(uint256[] calldata tokenIds) external nonReentrant {
        ...
        uint256 nowTS = block.timestamp;
        stakeMap[_msgSender()].totalRewards2Exclude = ((accYieldPerTknRay *
            countOfStakes) / RAY);
        for (uint256 i; i < tokenIds.length; ++i) {
            ...
            stakeMap[_msgSender()].stakes.push(
                Stake(tokenIds[i], nowTS, 0, (accYieldPerTknRay / RAY))
            );
            ++countOfStakes;
        }
    }
