...
contract DreamAFTest is ERC721, Ownable {
    ...
    constructor(uint256 _price) ERC721("DreamAFTest", "DREAMAFTEST") {
        price = _price;
    }
    ...
    function safeMint(string memory phrase) public payable {
        uint256 tokenId = _tokenIdCounter.current();
        require(_tokenIdCounter.current() <= 10000, "I'm sorry we reached the cap");
        require(msg.value >= price, "Ether value sent is not correct");
        _tokenIdCounter.increment();
        _safeMint(_msgSender(), tokenId);

        emit MintEvent(tokenId, phrase);
    }

    /// Returns tokenURI
    function tokenURI(uint256 _tokenId) public view virtual override(ERC721) returns (string memory) {
        require(_exists(_tokenId), "Nonexistent token");
        return string(abi.encodePacked("https://zuck.ai/metadata/", _tokenId.toString(), ".json"));
    }
