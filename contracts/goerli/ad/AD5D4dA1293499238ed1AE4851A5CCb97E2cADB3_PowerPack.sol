...
abstract contract SlotContract {
  function mintToOpenedPack(address to, uint256 packId) external virtual;
}

abstract contract StudioBadgeContract {
  function ownerOf(uint256 tokenId) external virtual returns (address owner);
}
  ...
  VRFConsumerBaseV2
{
  ...
  uint256 public s_price = 0.247 ether;
  ...
  struct SlotThreeInfo {
    uint256 packId;
    address mintedTo;
    uint256 randomNumber;
  }

  struct SlotThreeCounter {
    uint16 readerCount;
    uint8 sketchCount;
    uint8 panelCount;
   ...
   */
  SlotThreeCounter private s_slotCounter = SlotThreeCounter(0, 0, 0, 0);
   ...
   */
  mapping(address => bool) private s_publicMinted;
   ...
   */
  mapping(uint256 => SlotThreeInfo) private s_requestToSlotThree;
  ...
  /**
   * @dev emitted when a random slot3 nft is minted after opening pack
   */
  event SlotThreeMinted(uint256 packId, address to, uint256 rand);
  ...
  constructor(
    ...
    bytes32 gasLane,
    uint32 callbackGasLimit
  ) ERC721A("Power Pack", "PPACK") VRFConsumerBaseV2(vrfCoordinatorV2) {
  ...
  function openPack(uint256 packId) public nonReentrant {
    ...
    s_requestToSlotThree[requestId].packId = packId;
    s_requestToSlotThree[requestId].mintedTo = to;
   ...
   * calls to choose which slot3 to mint.
   * Numbers are hardcoded to save on gas
   */
  function fulfillRandomWords(
    ...
    uint256 packId = s_requestToSlotThree[requestId].packId;
    address to = s_requestToSlotThree[requestId].mintedTo;

    s_requestToSlotThree[requestId].randomNumber = rand;

    if (rand <= 4589) {
      if (s_slotCounter.readerCount < 4589) {
        mintReaderToken(to, packId);
      } else if (s_slotCounter.sketchCount < 168) {
        mintSketchCard(to, packId);
      } else if (s_slotCounter.panelCount < 143) {
        mintPanel(to, packId);
      } else if (s_slotCounter.redeemableCount < 100) {
        mintRedeemable(to, packId);
      } else {
        revert PowerPack__MaxSupplyReached();
      }
    } else if (rand <= 4757) {
      if (s_slotCounter.sketchCount < 168) {
        mintSketchCard(to, packId);
      } else if (s_slotCounter.readerCount < 4589) {
        mintReaderToken(to, packId);
      } else if (s_slotCounter.panelCount < 143) {
        mintPanel(to, packId);
      } else if (s_slotCounter.redeemableCount < 100) {
      ...
      if (s_slotCounter.panelCount < 143) {
        mintPanel(to, packId);
      } else if (s_slotCounter.readerCount < 4589) {
        mintReaderToken(to, packId);
      } else if (s_slotCounter.sketchCount < 168) {
      ...
      if (s_slotCounter.redeemableCount < 100) {
        mintRedeemable(to, packId);
      } else if (s_slotCounter.readerCount < 4589) {
        mintReaderToken(to, packId);
      } else if (s_slotCounter.sketchCount < 168) {
    ...
    emit SlotThreeMinted(packId, to, rand);
  }

  function mintReaderToken(address to, uint256 packId) private {
  ...
  function mintSketchCard(address to, uint256 packId) private {
  ...
  function studioBadgeMint(
    ...
    StudioBadgeContract studioBadge = StudioBadgeContract(
      s_addresses[Addresses.STUDIO_BADGE]
    );
  ...
  function mintlistMint(
    ...
    if (quantity > MAX_MINT_PER_ADDRESS) {
      revert PowerPack__QuantityNotAllowed();
    }
   ...
   */
  function getRequestToSlotThreeInfo(
    uint256 requestId
  ) external view returns (SlotThreeInfo memory) {
    return s_requestToSlotThree[requestId];
  }

  /**
   * @notice Returns slot3 counter of randomized mints
   */
  function getSlotCounter() external view returns (SlotThreeCounter memory) {
