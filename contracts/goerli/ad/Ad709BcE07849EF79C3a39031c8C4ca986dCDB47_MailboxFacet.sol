...
contract MailboxFacet is Base, IMailbox {
    ...
    /// @param _gasPricePerPubdata The maximum amount L2 gas that the operator may charge the user for.
    ...
    function requestL2Transaction(
        ...
        _verifyDepositLimit(msg.sender, _l2Value);
    ...
    function _requestL2Transaction(
        ...
        {
            uint256 baseCost = _isFree ? 0 : l2TransactionBaseCost(tx.gasprice, _l2GasLimit, _gasPricePerPubdata);
        ...
        params.l2GasPricePerPubdata = _gasPricePerPubdata;
    ...
    /// @param _gasPricePerPubdata The price for each byte of pubdata in L2 gas.
    ...
    function _getOverheadForTransaction(
        ...
        uint256 blockOverheadL2Gas = BLOCK_OVERHEAD_L2_GAS + BLOCK_OVERHEAD_PUBDATA * _gasPricePerPubdata;

        // The overhead from taking up the transaction's slot
        uint256 txSlotOverhead = Math.ceilDiv(blockOverheadL2Gas, MAX_TRANSACTIONS_IN_BLOCK);
        ...
        uint256 overheadForLength = Math.ceilDiv(_encodingLength * blockOverheadL2Gas, BOOTLOADER_TX_ENCODING_SPACE);
        ...
        {
            uint256 numerator = (blockOverheadL2Gas * _totalGasLimit + _gasPricePerPubdata * MAX_PUBDATA_PER_BLOCK);
            uint256 denominator = (_gasPricePerPubdata * MAX_PUBDATA_PER_BLOCK + blockOverheadL2Gas);
        ...
        // The overhead for L2 gas that could be used to use single-instance circuits
        uint256 overheadForComputation;
        {
            uint256 numerator = blockOverheadL2Gas * _totalGasLimit + L2_TX_MAX_GAS_LIMIT;
            uint256 denominator = L2_TX_MAX_GAS_LIMIT + blockOverheadL2Gas;

            overheadForComputation = (numerator - 1) / denominator;
        }
        blockOverheadForTransaction = Math.max(blockOverheadForTransaction, overheadForComputation);
    }
