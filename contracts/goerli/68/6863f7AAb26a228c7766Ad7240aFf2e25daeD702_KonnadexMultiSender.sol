...
contract KonnadexMultiSender is Initializable, OwnableUpgradeable {
    ...
    event TokenAdded(bytes8 _symbol, address _tokenAddress);
     ...
     */
    function initialize(
        uint256 _salaryCharge,
        uint256 _feeAmountConverter
    ) public initializer {
        __Ownable_init();
    ...
    function addToken(
        bytes8 _symbol,
        address _tokenAddress
    ) external onlyOwner onlyUnsetToken(_symbol) {
        require(Address.isContract(_tokenAddress), "Invalid token address");
    ...
    function removeToken(
        bytes8 _symbol
    ) external onlyOwner onlySetToken(_symbol) {
        delete (tokens[_symbol]);
    }

    function getTokenBalance(
        bytes8 _symbol
    ) public view onlySetToken(_symbol) returns (uint256) {
    ...
    function distributeToken(
        ...
        if (
            IERC20(tokens[_symbol]).balanceOf(msg.sender) <
    ...
    function distributeNativeCoin(
        ...
        if (msg.value < (totalAmount.add(feeAmount))) {
            revert InsufficientBalance();
        }
    ...
    function moveNativeTokens(
        address payable _account
    ) external onlyOwner onlyValidAddress(_account) returns (bool) {
    ...
    function setFeeAmountConverter(
    ...
    function setSalaryCharge(
        uint256 _newSalaryCharge
        ...
        returns (bool)
    {
        ...
        if (_senderShouldBearCharge && msg.value < _amount.add(feeAmount)) {
            revert InsufficientBalance();
        }
