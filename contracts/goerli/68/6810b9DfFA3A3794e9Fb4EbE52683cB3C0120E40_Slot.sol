...
contract Slot is VRFConsumerBaseV2, Ownable, KeeperCompatibleInterface {
    struct SpinStreakData {
        uint16 spinStreak; // e.g. if you spun 1 3 3 5 2, you would've had streaks of: [[1], [3, 3], [5], [2]] = [1,2,1,1]
        uint16 spinStreakTemp;
        uint16 spinStreakStop; // stops would've been [1, 3, 5, 2] for streaks [1,2,1,1]
        uint16 spinStreakStopTemp;
        uint16 luckySpinStreak;
        uint16 luckySpinStreakTemp;
    }
    ...
    uint16 private constant DIGITS_PER_WORD = 76;
    uint16 private constant NUMBERS_PER_WORD = 38; // VRF gives you a random number with at least 76 digits. From this we can extract 38 unique numbers.

    uint16[] private s_winTypes;
    uint16[] private s_winStops;
    uint256[] private s_payouts;
    uint16[] private s_words;
    ...
    mapping(address => uint16[]) private s_playerWinsArray;
    mapping(address => uint16[]) private s_playerStopsArray;
    mapping(address => uint16[]) private s_playerSpins;
    ...
    function fulfillRandomWords(
        ...
        uint16[] memory winTypes = new uint16[](startingStoredSpinsLength);
        uint16[] memory winStops = new uint16[](startingStoredSpinsLength);
        uint256[] memory payouts = new uint256[](startingStoredSpinsLength);
        uint16[] memory numbers = new uint16[](i_startingStoredNumbers);
        uint16 digitIdx;
    ...
    function buySpins() public payable {
        ...
        uint16[] memory playerWins = new uint16[](SPINS_PER_BUY);
        uint16[] memory playerStops = new uint16[](SPINS_PER_BUY);
        uint256 playerPayout = 0;
        uint16[] memory playerSpins = new uint16[](uint16(NUM_REELS * SPINS_PER_BUY));
    ...
    function getPlayerSpinsFull() public view returns (uint16[] memory) {
        return s_playerSpins[msg.sender];
    }

    function getPlayerWinsArr() public view returns (uint16[] memory) {
        return s_playerWinsArray[msg.sender];
    }

    function getPlayerStopsArr() public view returns (uint16[] memory) {
