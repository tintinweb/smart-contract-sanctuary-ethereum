...
contract GreyMarket is OwnableUpgradeable, ReentrancyGuardUpgradeable, GreyMarketStorage, GreyMarketEvent {
    ...
    bytes32 public constant CREATE_ORDER_TYPEHASH = 
        keccak256(
            "Create(uint256 id,address buyer,address seller,address paymentToken,uint256 orderType,uint256 amount)"
        );

    bytes32 public constant CLAIM_ORDER_TYPEHASH = 
        keccak256(
            "Claim(uint256 id,address buyer,address seller,uint256 orderStatus)"
        );
    
    bytes32 public constant WITHDRAW_ORDER_TYPEHASH = 
        keccak256(
            "Withdraw(uint256 id,address buyer,address seller,uint256 orderStatus)"
        );

    bytes32 public constant RELEASE_DISPUTED_ORDER_TYPEHASH = 
        keccak256(
            "Release(uint256 id,address buyer,address seller,uint256 orderStatus,address winner)"
        );
    ...
    constructor(address _usdc, address _weth, address _uniswap) {
        require(_usdc != address(0) && _weth != address(0) && _uniswap != address(0), "invalid token address");
        USDC_TOKEN_ADDRESS = _usdc;
        WETH_TOKEN_ADDRESS = _weth;
        UNISWAP_ROUTER_ADDRESS = _uniswap;
    }
    ...
    function createOrder(
        ...
        require(validateCreateOrder(v, r, s, id, msg.sender, seller, paymentToken, uint256(orderType), amount), "createOrder: invalid signature");
    ...
    function claimOrder(
        ...
        require(validateClaimOrder(v, r, s, id, buyer, seller, uint256(OrderStatus.ORDER_DELIVERED)), "claimOrder: invalid signature");
        OrderInfo storage orderInfo = orders[id];
        ...
        if (orderInfo.orderType == OrderType.ESCROW) {
            fee = orderInfo.amount * escrowFee / 1e18;
            adminFees[orderInfo.paymentToken] = adminFees[orderInfo.paymentToken] + fee;
        }

        uint256 remainder = orderInfo.amount - fee;
        if (orderInfo.paymentType == PaymentType.PAYMENT_ETH)
            payable(orderInfo.seller).transfer(remainder);
        else if (orderInfo.paymentType == PaymentType.PAYMENT_USDC)
            IERC20(USDC_TOKEN_ADDRESS).safeTransfer(orderInfo.seller, remainder);
     ...
     * @param vs Array of the v value of the signatures
     * @param rs Array of the r value of the signatures
     * @param ss Array of the s value of the signatures
     */
    ...
    function withdrawOrder(
        ...
        require(validateWithdrawOrder(v, r, s, id, buyer, seller, uint256(OrderStatus.ORDER_CANCELLED)), "withdrawOrder: invalid signature");
        OrderInfo storage orderInfo = orders[id];
    ...
    function releaseDisputedOrder(
        ...
        require(validateReleaseDisputedOrder(vs, rs, ss, id, buyer, seller, uint256(OrderStatus.ORDER_DISPUTE), winner), "releaseDisputedOrder: invalid signature");
     ...
     * @param recipient The address that will receive the fees.
     */
    function _withdrawLockedFund(uint256 id, address recipient) external onlyOwner {
        OrderInfo storage orderInfo = orders[id];
    ...
    function validateCreateOrder(
        ...
        return ecrecover(digest, v, r, s) == proofSigner;
    }
     ...
     * @param vs Array of signature v values
     * @param rs Array of signature r values
     * @param ss Array of signature s values
     * @param id Order id
     * @param buyer Buyer address
     * @param seller Seller address
     * @param orderStatus Order status in integer value
    ...
    function validateReleaseDisputedOrder(
        ...
        require(vs.length == REQUIRED_SIGNATURE_COUNT, "invalid signature required count");
        ...
        address signerOne = ecrecover(digest, vs[0], rs[0], ss[0]);
        address signerTwo = ecrecover(digest, vs[1], rs[1], ss[1]);
     ...
     */
    function getOrderInfo(uint256 orderId) public view returns (OrderInfo memory) {
