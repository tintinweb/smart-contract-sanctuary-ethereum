...
import { Potter } from "../dAgoraWizard/Potter.sol";
import '../IdAgoraMembership.sol';

contract dAgoraPotterFactory is Ownable, ReentrancyGuard {
    Potter potter;
    ...
    uint256 public potterCount = 0;
    bool public paused;

    mapping(address => Deploys) private _deployedContracts;

    event HarryPotterCreated(address potterContract, address owner, uint256 potterIndex);
        ...
        nonReentrant
    {
        ...
        potterCount++;
        _deployedContracts[msg.sender].ContractAddress.push(address(potter));
        _deployedContracts[msg.sender].Owner = msg.sender;
        _deployedContracts[msg.sender].potterIndex = potterCount;

        emit HarryPotterCreated(address(potter), msg.sender, potterCount);
    }
...
contract Potter is ERC721A, Ownable {
    ...
    uint8 public maxWhiteListAmount;
    ...
    function reserveTokens(uint256 _amount) public onlyOwner notPaused {
        require(_amount + totalSupply() <= maxTotalSupply, "Soldout");
        _safeMint(msg.sender, _amount);
    }
    ...
    function setBulkBuyLimit(uint8 _newBulkBuyLimit) public onlyOwner {
