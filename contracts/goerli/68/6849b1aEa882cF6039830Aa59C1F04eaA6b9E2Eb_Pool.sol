...
contract Pool is Initializable, OwnableUpgradeable, ReentrancyGuardUpgradeable, IPool, Governor {
  ...
  function getTVL() public returns (uint256) {
    ...
    for (uint256 i = 0; i < tokenWhitelist.length; i++) {
      if (tokenWhitelist[i] == address(0)) {
        ...
        uint256 balance = IERC20Upgradeable(tokenWhitelist[i]).balanceOf(
          address(this)
        );
        if (balance > 0) {
  ...
  modifier onlyProposalGateway() {
    require(msg.sender == service.proposalGateway(), "Not proposal gateway");
    _;
  }

  modifier onlyPool() {
    require(msg.sender == address(this), "Not a pool");
    _;
  }
}
...
abstract contract Governor {
    ...
    function executeBallot(uint256 proposalId) external {
        ...
        require(
            proposals[proposalId].state == ProposalExecutionState.Initialized,
            "Proposal execution is in wrong state"
        );
        ...
        if (success) {
            ...
            proposals[proposalId].state = ProposalExecutionState.Rejected;
        }        
