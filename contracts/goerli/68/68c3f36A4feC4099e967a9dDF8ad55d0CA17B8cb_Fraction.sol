...
contract Fraction is Ownable {
  ...
  event WithdrawnTokens(address user, uint256 poolId, uint256 tokensAmount);
  ...
  function getAssetPiece(uint256 poolId, uint256 piecesAmount) external {
  ...
  function withdraw(uint256 poolId) external {
    ...
    emit WithdrawnTokens(msg.sender, poolId, amoutToWithdraw);
  }

  function buyAsset(uint256 poolId) external onlyOwner {
