...
contract TestTokenVesting is Ownable {
    ...
    uint256 private constant MONTH = 300;
    uint256 private constant DECIMAL_FACTOR = 10 ** 18;

    uint256 public vestingStartTS = 10000000000;
    uint256 slicePeriodSeconds = MONTH;
    uint256 privateRoundTotalAmount;
    ...
    enum VestingScheduleType {
        ...
        Private,
        Public,
        OperationsConsumerRewardsReserve,
        SocialAdvisory,
        MarketingAndTechDevelopment,
        LiquidityExchangeListings,
        FoundersTeam
    }

    VestingSchedule[8] public vestingSchedules;

    mapping(address => PrivateRoundDetails) public privateRoundInvestors;

    constructor(string memory _tokenName, string memory _tokenSymbol) {
        
        vvToken = new VVToken(_tokenName, _tokenSymbol);
        ...
        vestingSchedules[uint256(VestingScheduleType.Public)] = VestingSchedule(
            ...
            0,
            0x9C5304Cf9066a860672BA5cf7f1C4592DCf20f56
        );
        vestingSchedules[uint256(VestingScheduleType.OperationsConsumerRewardsReserve)] = VestingSchedule(
            48 * MONTH,
            456000000 * DECIMAL_FACTOR,
            0,
            0x79c7F1980189D0a52a9e57dE83060DCE98802b20
        );
        vestingSchedules[uint256(VestingScheduleType.SocialAdvisory)] = VestingSchedule(
            12 * MONTH,
            60000000 * DECIMAL_FACTOR,
            0,
            0xf38CFd68632849a9499857411D883B86a4b73bb2
        );
        vestingSchedules[uint256(VestingScheduleType.MarketingAndTechDevelopment)] = VestingSchedule(
            0,
            70000000 * DECIMAL_FACTOR,
            0,
            0xC95cE5A64b5f2d3772Ee6B9adF9AA27d7fF2b68D
        );
        vestingSchedules[uint256(VestingScheduleType.LiquidityExchangeListings)] = VestingSchedule(
            48 * MONTH,
            100000000 * DECIMAL_FACTOR,
            0,
            0x486F08ED7B1e1c80A65E185005E17504b3E97b62
        );
        vestingSchedules[uint256(VestingScheduleType.FoundersTeam)] = VestingSchedule(
            12 * MONTH,
            220000000 * DECIMAL_FACTOR,
            0,
            0x8F8451D858645695C690295125dd8Ad6657a7313
        );
    ...
    function addPrivateVestingScheduleBeneficiary(
        ...
        privateRoundInvestors[_beneficiary] = PrivateRoundDetails(_amount, 0);
        privateRoundTotalAmount += _amount;
    }

    function release(uint256 vestingScheduleId, uint256 amount)
        external
    {
        require(vestingSchedules[vestingScheduleId].amount > 0, "Not correct id");
        ...
        vvToken.safeTransfer(_beneficiary, amount);
    }

    function releaseForPrivateRoundInvestors(uint256 amount) public
    {
        ...
        privateRoundInvestors[_beneficiary].released += amount;
        vvToken.safeTransfer(_beneficiary, amount);
    }
        ...
        returns (uint256)
    {
        ...
        if (currentTime < vestingStartTS) {
            return 0;
        } else if (currentTime >= vestingStartTS + vestingSchedules[1].duration) {
            return
                privateRoundInvestors[_beneficiary].amount -
                privateRoundInvestors[_beneficiary].released;
        } else {
            ...
            uint256 vestedAmount = (privateRoundInvestors[_beneficiary].amount *
                vestedSeconds) / (vestingSchedules[1].duration);
            vestedAmount -= privateRoundInvestors[_beneficiary].released;
            return vestedAmount;
        }
    }
    ...
    function getVestingSchedule(uint256 index)public view returns(VestingSchedule memory){
    ...
    function withdrawToken(address tokenAddress) external onlyOwner {
        require(tokenAddress != address(vvToken), "vvToken are not withdrawable");
    ...
    function getCurrentMonth() public view returns (uint256) {
        ...
        uint256 currentMonth = ((currenttime - vestingStartTS) / 30 days) + 1;
        return currentMonth;
    }
