...
contract MarketHistory is IMarketHistory, AccessHandler {
    ...
    /* CURRENTLY UNUSED - MARKETS ARE STARTING ACTIVE WHEN CREATED
     * @notice Event that fires when a market is activated.
     * @param marketHash is the hash used to identify the market.
    ...
    /**
     * @notice Makes a market for testing.
    ...
    function addMarket()
        ...
        override
        isInitialized()
        onlyRole(AccessHandler.REPORTER_ROLE)
    {
        marketCounter++;
        bytes32 hash = keccak256(abi.encodePacked((marketCounter)));
/*
        // Just debugging
        console.log("Counter:", marketCounter);
        console.log("MarketHash:", Strings.toHexString(uint256(hash), 32));
        IMarketHistory.MarketState state = marketStates[hash];
        console.log("State:", uint256(state));
        marketCounter++;
        hash = keccak256(abi.encodePacked((marketCounter)));
        console.log("Counter:", marketCounter);
        console.log("MarketHash:", Strings.toHexString(uint256(hash), 32));
        state = marketStates[hash];
        console.log("State:", uint256(state));
        marketCounter++;
        hash = keccak256(abi.encodePacked((marketCounter)));
        console.log("Counter:", marketCounter);
...
*/
    ...
    /* CURRENTLY UNUSED - MARKETS ARE STARTING ACTIVE WHEN CREATED
     * @notice Updates a market to Active state.
     * @notice Revert Error is emitted if not exist.
     * @param marketHash is the hash of the market to update.
    function activateMarket(bytes32 marketHash)
        external
        override
        onlyRole(AccessHandler.REPORTER_ROLE)
    {
        // Checks that it is pending
        if (marketStates[marketHash] != IMarketHistory.MarketState.Pending) { // TODO: Should pending <-> activate be an option for pause betting?
            revert InvalidMarketState({
                expected: IMarketHistory.MarketState.Pending,
                current: marketStates[marketHash]
            });
        }
        marketStates[marketHash] = IMarketHistory.MarketState.Active;
        emit MarketActivated(marketHash);
    }
     */
        ...
        onlyRole(AccessHandler.REPORTER_ROLE)
    {
        ...
        if (marketStates[marketHash] != IMarketHistory.MarketState.Active) {  // TODO: Should playing be available in other states like pending?
            revert InvalidMarketState({
    ...
    {
        // Checks that it is Active
        if (marketStates[marketHash] != IMarketHistory.MarketState.Playing) {  // TODO: Should settle be available in other states like Active?
            revert InvalidMarketState({
                expected: IMarketHistory.MarketState.Playing,
                current: marketStates[marketHash]
            });
        }
        // Below is disabled for now, as its not a possible error
        // Checks that it has no outcome yet
//        if (marketOutcomes[marketHash] != MarketData.MarketOutcome.Undefined) {
//            revert InvalidMarketOutcome({
//                expected: MarketData.MarketOutcome.Undefined,
//                current: marketOutcomes[marketHash]
//            });
//        }
    ...
    /*
     * @notice Check if market exists and is playing.
     * @notice Errors are emitted if check fails.
     * @param marketHash is the hash of the market to check.
    function assertMarketIsPlaying(bytes32 marketHash) external view override {
        IMarketHistory.MarketState state = marketStates[marketHash];
        if (state != IMarketHistory.MarketState.Playing) {
            revert InvalidMarketState({
                expected: IMarketHistory.MarketState.Playing,
                current: state
            });
    ...
    function isMarketOutcome(
        ...
        returns (bool)
    {
        IMarketHistory.MarketOutcome storedOutcome = marketOutcomes[marketHash];
        if (storedOutcome == outcome) return true;
        return false;
    }
...
interface IMarketHistory {
    ...
    enum MarketOutcome { Undefined, Win, Loss, Cancel } // Enum
