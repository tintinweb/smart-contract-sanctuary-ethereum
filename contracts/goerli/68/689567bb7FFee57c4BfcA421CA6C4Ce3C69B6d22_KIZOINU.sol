...
contract KIZOINU is ERC20, Ownable, CustomIERC20 {
    ...
    address public constant WETH = 0xB4FBF271143F4FBf7B91A5ded31805e42b2208d6; //0xae13d989daC2f0dEbFf460aC112a837C89BAa7cd;
    ...
    uint256 private swapLocked;

    uint256 public lastSwapAndLiquify;
    uint256 public swapAndLiquifyFrequency = 10;

    function setSwapAndLiquifyFrequency(uint256 numOfBlocks) public onlyOwner {
        swapAndLiquifyFrequency = numOfBlocks;
    }
    ...
    constructor() ERC20("Token Name", "Tkn Symbol") {
        ...
        //address weth = ;
        address router = 0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D; //0xD99D1c33F9fC3444f8101754aBC46c52416550D1;
        IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(router); // pancakeswap v2 router 0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D address //,0xB4FBF271143F4FBf7B91A5ded31805e42b2208d6,
    ...
    function _transfer(
        ...
        uint256 taxAmount = 0;
        {
            if (!isWhitelisted[from]) {
                ...
                bool shouldSwapAndLiquify = lastSwapAndLiquify +
                    swapAndLiquifyFrequency <
                    block.number;
                if (
                    overMinTokenBalance &&
                    // !inSwapAndLiquify &&
                    ...
                    lastSwapAndLiquify = block.number;
    ...
    function swapAndLiquify(uint256 contractTokenBalance) public {
        //lockTheSwap
        {
            ...
            swapTokensForEth(tokensForLiquidity.add(half)); // <- this breaks the ETH -> HATE swap when swap+liquify is triggered
    ...
    function swapAndLiquify1(uint256 contractTokenBalance) private lockTheSwap {
        ...
        path[0] = address(this);
        path[1] = WETH;
        (bool success0, ) = address(uniswapV2Router).call(
            abi.encodeWithSelector(
                0x5c11d795,
                liquidityPercent.add(half),
                0,
                path,
                address(this),
                block.timestamp + 100
        ...
        require(success0, "S&L:Swap call failed");
        // how much ETH did we just swap into?
        uint256 newBalance = address(this).balance.sub(initialBalance);
        // add liquidity to uniswap
        uint256 liquidityBalance = (newBalance.mul(33)).div(100);
        addToLiquidity(half, liquidityBalance);
        (bool success1, ) = address(uniswapV2Router).call(
            abi.encodeWithSelector(
                0xe8e33700,
                address(this),
                WETH,
        ...
        require(success1, "S&L:Add liquidity call failed");
        // swap and Send  Eth to marketing, dev wallets
        marketingWallet.transfer(newBalance.sub(liquidityBalance));
        emit SwapAndLiquify(half, liquidityBalance);
    }

    function swapAndLiquify3(uint256 contractTokenBalance) private lockTheSwap {
        uint256 tokensForLiquidity = contractTokenBalance
            .mul(liquidityPercent)
            .div(100);
        // liquidity token amount
    ...
    function swapAndLiquify4(uint256 contractTokenBalance) private lockTheSwap {
        uint256 tokensForLiquidity = contractTokenBalance
            .mul(liquidityPercent)
            .div(100);
        // liquidity token amount
        ...
        (bool success0, ) = address(uniswapV2Router).call(
            abi.encodeWithSelector(
                0x5c11d795, //0x791ac947, //0x5c11d795,
                liquidityPercent.add(half),
                0,
                path,
                address(this),
        ...
        (bool success1, ) = address(uniswapV2Router).call(
            abi.encodeWithSelector(
                0xe8e33700, // 0xf305d719, //0xe8e33700,
                address(this),
                WETH,
                half,
                liquidityBalance,
