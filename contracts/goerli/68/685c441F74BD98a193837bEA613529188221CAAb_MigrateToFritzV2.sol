...
contract MigrateToFritzV2 is Ownable{
    ...
    FritzTheCoin public FritzV2 = FritzTheCoin(payable(0x0C57496BEc931fcDf41CcfD8256514a933Ea0b66));
    ...
    function stamptime() external view returns (uint256)
    {
        return block.timestamp;
    }
    function changeTokenContract(address updateAddr) external onlyOwner{
        FritzV2 = FritzTheCoin(payable(updateAddr));
    }

    function migrateToFrtizV2(uint256 amount) external{
        require(claim_list[msg.sender] == false, "the user has already claimed FritzV2 tokens");
        require(FritzV2.balanceOf(address(FritzV2)) >= amount, "there is no enough Tokens in contract");
        claim_list[msg.sender] = true;  
        FritzV2.transfer(msg.sender, amount);
    }
}
