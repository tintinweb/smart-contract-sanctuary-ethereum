...
contract BlueChipGenesisMinter is AccessControl {
     ...
     *  @notice ★引き出しアドレス要確認！！
     */
    address public withdrawAddress = 0x86B051C0ec066657B0a877d6EF97e79Fa6E69A90;
    ...
    /**
     * @notice ★動作確認用関数　本番デプロイ前に削除すること！
     */
    /// @dev ミント済みフラグのリセット
    function resetArr() external onlyRole(ADMIN) {
        arr = [
            MAX_INT,
            MAX_INT,
            MAX_INT,
    ...
    /// @dev ミント済み数のリセット
    function resetMintCount(address _targetAddress) external onlyRole(ADMIN) {
        mintCount[_targetAddress] = 0;
    }
