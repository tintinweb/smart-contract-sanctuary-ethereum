...
contract TestIntern is ERC721, Pausable, Ownable {
    ...
    constructor() ERC721("TestIntern", "TI") {
        baseTokenURI = "";
        extensionTokenURI = ".json";
        unrevealedTokenURI = "";
        saleConfig.publicSaleMintPrice = 0.0001 ether;
        saleConfig.publicSaleStartTime = 1655341970;
        maxSupply = 500;
        isRevealed = false;
        saleState = true;
    ...
    function publicSaleMint(uint256 _mintAmount) external payable whenNotLocked callerIsUser {
        ...
        require(block.timestamp > config.publicSaleStartTime, "Public sale is closed.");
        uint256 tokenId = _tokenIdCounter.current();
        require(tokenId.add(_mintAmount) < maxSupply.add(1), "Max supply reached.");
    ...
    /// @dev Dev mint.
    function devMint(uint256 _mintAmount, address _destinationAddress) external onlyOwner {
        uint256 tokenId = _tokenIdCounter.current();
        require(tokenId.add(_mintAmount) < maxSupply.add(1), "Max supply reached.");

        amountMintedPublic[_destinationAddress] += _mintAmount;
