...
// - フロント側
//     - 関数の呼び方： etherjsで`contract["mint(bytes)"]([0x00])`のように書く必要がある。
//     - 引数のエンコード: `const args_bytes = ethers.utils.defaultAbiCoder.encode(["uint", "string"], [888, "Takechy"]);`のように符号化する。
// - 参考：
...
import "./interface/IDNA.sol";
import "./struct/MintInfo.sol";

//
contract MetaaniDNASBT is IConataNFT, ERC1155, Ownable{
    // CHECK:
    string public name = "MetaaniDNA SBT";
    string public ipfs_base = "ipfs://QmTiW6V5AG3tVJuewTV2NX1yqFJzLb28MpS7ctTHnPzKXT/";
    IDNA public dna;
    string[] mintTermNames;
    mapping ( string => MintInfo) mintInfoMap;
    mapping ( address => bool[7] ) claimedTokenIdMap;
    uint private _mintedAmount = 0;
    uint private _burnedAmount = 0;

    //
    constructor() ERC1155(ipfs_base) {
    ...
    // 取得可能なtokenIdリスト
    function getAvailableTokenIds(address account) public view returns(uint[] memory){
        uint mintedDNAAmount = getMintedDNAAmount(account);
        ...
        // Ceramics
        tokenId = 1;
        if(mintedDNAAmount >= 1 && claimedTokenIdMap[account][tokenId - 1] == false){
            ids[count] = tokenId;
            count++;
        }
        // Bronze
        tokenId = 2;
        if(mintedDNAAmount >= 3 && claimedTokenIdMap[account][tokenId - 1] == false){
            ids[count] = tokenId;
            count++;
        }
        // Silver
        tokenId = 3;
        if(mintedDNAAmount >= 5 && claimedTokenIdMap[account][tokenId - 1] == false){
            ids[count] = tokenId;
            count++;
        }
        // Gold
        tokenId = 4;
        if(mintedDNAAmount >= 10 && claimedTokenIdMap[account][tokenId - 1] == false){
            ids[count] = tokenId;
            count++;
        }
        // Platinum
        tokenId = 5;
        if(mintedDNAAmount >= 30 && claimedTokenIdMap[account][tokenId - 1] == false){
            ids[count] = tokenId;
            count++;
        }
        // Diamond
        tokenId = 6;
        if(mintedDNAAmount >= 50 && claimedTokenIdMap[account][tokenId - 1] == false){
            ids[count] = tokenId;
            count++;
        }
        // Alexandrite
        tokenId = 7;
        if(mintedDNAAmount >= 100 && claimedTokenIdMap[account][tokenId - 1] == false){
            ids[count] = tokenId;
            count++;
        }
    ...
    function _minter(bytes memory data) internal{
        ...
        (string memory termName, address to) = abi.decode(data, (string, address));
        // 開いているか？
        require(mintInfoMap[termName].exists, "Not Exist");
        require(mintInfoMap[termName].isOpened, "Not Opened");
        // Claim要件を確認。
        address target_account = _msgSender();
        uint[] memory ids = getAvailableTokenIds(target_account);
        // 取得可能なtokneIdがなければrevertする。
        require(ids.length > 0, "Not Qualified");
        // 指定したアドレスにミントする。
        for(uint i=0; i<ids.length; i++){
            _mint(to, ids[i], 1, "");
            _mintedAmount++;
            emit Minted(to, ids[i]);
            // Claim済みを記録。tokenId - 1をインデックスにする。
            claimedTokenIdMap[target_account][ids[i] - 1] = true;
        }
    }
...
struct MintInfo {
    bool isOpened;
    bool isRestricted;
    mapping( address => uint ) addressMintedMap;
    bool exists;
}