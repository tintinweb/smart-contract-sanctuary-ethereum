...
pragma solidity ^0.8.15;
     ...
     * @notice This contract handles minting Student ID tokens.
     */
    contract LaunchpadSID is ERC721A,ERC721AQueryable,ERC4907A,Ownable,Pausable,ERC2981 {
    ...
    address[] public payoutAddresses = [0x16aD4E68C2E1D312C01098d3E1Cfc633B90dFF46,0x8AD5Ee55A8425a8a4938106050636E0901Df4321,0x52EA5F96f004d174470901Ba3F1984D349f0D3eF];
    ...
    bool public soulboundAdminTransferInProgress;
...
function _directApproveMsgSenderFor(uint256 tokenId) internal {
    ...
    function supportsInterface(
      bytes4 interfaceId
    ) public view virtual override(ERC721A, IERC721A, ERC2981, ERC4907A) returns (bool) {
    ...
    function gift(address[] calldata receivers, uint256 mintNumber) external onlyOwner {
        require((totalSupply() + (receivers.length * mintNumber)) <= MAX_SUPPLY, "MINT_TOO_LARGE");
    ...
    function withdraw() public onlyOwner {
        ...
        for (uint i = 0; i < 3; i++) {
            require(payable(payoutAddresses[i]).send(balance * payoutPercentages[i] / 100));
        }
    }
    ...
    function setSoulbindingActive(bool _soulbindingActive) public onlyOwner {
        require(soulbindingActive != _soulbindingActive, "NEW_STATE_IDENTICAL_TO_OLD_STATE");
     ...
     */
    function setSoulboundAdminAddress(address _adminAddress) external onlyOwner {
        require(!soulbindAdminAddressPermanentlyDisabled, "CHANGING_ADMIN_ADDRESS_PERMANENTLY_DISABLED");
    ...
    function adminTransfer(address from, address to, uint256 tokenId) public {
        require(msg.sender == soulboundAdminAddress, "CAN_ONLY_BE_CALLED_BY_ADMIN_ADDRESS");
  ...
  */
  function setBurnClaimState(bool _burnClaimActive) public onlyOwner {
    require(burnClaimActive != _burnClaimActive, "NEW_STATE_IDENTICAL_TO_OLD_STATE");
    burnClaimActive = _burnClaimActive;
  }

  function burnERC1155ToMint(uint256 amount) public {
      require(burnClaimActive, "BURN_CLAIM_IS_NOT_ACTIVE");
      require(totalSupply() + amount <= MAX_SUPPLY, "MAX_SUPPLY_EXCEEDED");
      ERC1155 ExternalERC1155BurnContract = ERC1155(0xF5621a065F267D382C00839Ae8a04688c93D24b8);
      require(ExternalERC1155BurnContract.balanceOf(msg.sender, 1) >= amount, "NOT_ENOUGH_TOKENS_OWNED");
      ExternalERC1155BurnContract.safeTransferFrom(msg.sender, burnAddress, 1, amount, '');
  ...
  */
  function setFreeClaimState(bool _freeClaimActive) public onlyOwner {
    require(freeClaimActive != _freeClaimActive, "NEW_STATE_IDENTICAL_TO_OLD_STATE");
  ...
  */
   function freeClaim(uint256[] calldata _tokenIDs) public {
    require(freeClaimActive, "FREE_CLAIM_IS_NOT_ACTIVE");
    require(totalSupply() + (_tokenIDs.length * 3) <= MAX_SUPPLY, "MAX_SUPPLY_EXCEEDED");
    ERC721A ExternalERC721FreeClaimContract = ERC721A(0xf37488Af8f71233d75aC69Dc29C45732374D3419);
    for (uint256 i = 0; i < _tokenIDs.length; i++) {
      require(ExternalERC721FreeClaimContract.ownerOf(_tokenIDs[i]) == msg.sender, "DOES_NOT_OWN_TOKEN_ID");
    ...
    function _beforeTokenTransfers(address from, address to, uint256 tokenId, uint256 quantity) internal whenNotPaused override(ERC721A) {
      require(!soulbound[tokenId] || soulboundAdminTransferInProgress, "SOULBOUND_TOKENS_CANNOT_BE_TRANSFERRED");
