...
contract Guild is IGuild, ERC2771Context, AccessControlEnumerable {
    /*///////////////////////////////////////////////////////////////
                               ERRORS
    //////////////////////////////////////////////////////////////*/
    ...
    function guildID_mint(string calldata name, uint256 avatarID) external payable {
        _payGuildFee(_msgSender(), 0);

        IGuildID guildID = IGuildID(s.contracts.guildID);
        guildID.mint(_msgSender(), name, avatarID);
        emit GuildIDMinted(guildID.getID(_msgSender()), _msgSender(), avatarID, name);
    ...
    function guildID_upgradeName(string calldata newName) external payable onlyRole(MEMBER_ROLE) {
        _payGuildFee(_msgSender(), 1);

        IGuildID guildID = IGuildID(s.contracts.guildID);
        guildID.upgradeName(_msgSender(), newName);
        emit GuildIDNameUpgraded(guildID.getID(_msgSender()), _msgSender(), newName);
    }

    function guildID_upgradeRank() external payable onlyRole(MEMBER_ROLE) {
        _payGuildFee(_msgSender(), 2);

        IGuildXP(s.contracts.guildXP).burn(_msgSender(), getRankUpCostXP(_msgSender()));

        IGuildID guildID = IGuildID(s.contracts.guildID);
        guildID.upgradeRank(_msgSender());
        uint256 tokenID = guildID.getID(_msgSender());
        emit GuildIDRankUpgraded(tokenID, _msgSender(), uint256(guildID.getRank(tokenID)));
    }

    function guildID_upgradeAvatar(uint256 avatarID) external payable onlyRole(MEMBER_ROLE) {
        _payGuildFee(_msgSender(), 3);

        IGuildID guildID = IGuildID(s.contracts.guildID);
        guildID.upgradeAvatar(_msgSender(), avatarID);
        uint256 tokenID = guildID.getID(_msgSender());
        emit GuildIDAvatarUpgraded(tokenID, _msgSender(), avatarID);
    }
    ...
    function guildXP_claim(bytes calldata signature, uint256 xpAmount) external onlyRole(MEMBER_ROLE) {
        IGuildXP(s.contracts.guildXP).claim(_msgSender(), signature, xpAmount);
        emit GuildXPClaimed(IGuildID(s.contracts.guildID).getID(_msgSender()), _msgSender(), xpAmount);
    }
    ...
    function guildXP_mint(address to, uint256 amount) external onlyRole(MANAGER_ROLE) {
        ...
        emit GuildXPMintedTo(to, _msgSender(), amount);
    }
    ...
    function guildXP_burn(address from, uint256 amount) external onlyRole(MANAGER_ROLE) {
        ...
        emit GuildXPBurnedFrom(from, _msgSender(), amount);
    }
    ...
    function guildBank_deposit(
        ...
        rentalId_ = IGuildBank(s.contracts.guildBank).deposit(
            _msgSender(),
            collection,
            tokenId,
            share,
    ...
    function guildBank_rent(uint256 rentalId, uint32 amountToRent) external onlyRole(MEMBER_ROLE) {
        ...
        // Get XP Cost
        uint256 xpCost = _getXPCost(_msgSender(), actualUtilizationRate);

        // Burn XP Fee
        uint256 dueAmount = xpCost * uint256(amountToRent) * rental.duration;
        IGuildXP(s.contracts.guildXP).burn(_msgSender(), dueAmount);

        // Call subcontract
        uint256 newRentalId = guildBank.rent(_msgSender(), rentalId, amountToRent);
    ...
    function guildBank_withdraw(uint256 _rentalId) external onlyRole(MEMBER_ROLE) {
        IGuildBank(s.contracts.guildBank).withdraw(_msgSender(), _rentalId);
