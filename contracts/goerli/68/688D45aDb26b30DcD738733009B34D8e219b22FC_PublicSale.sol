    ...
    UUPSUpgradeable
{
    ...
    function getPriceByIdUSDC(uint256 _id) external view returns (uint256,uint256) {
     ...
     function purchaseNftByIdUsingUSDCoin(uint256 _id,uint256 _amountIn) external {
        ...
        require(allowance >= _amountIn, "Public Sale: Not enough allowance");

        // Obtener el precio segun el id
        tpriceNft = _getPriceById(_id);
        uint amountOut = tpriceNft;
        uint amountInMax = _amountIn;
        // obtiene USDC del comprador
        usdcoin.transferFrom(msg.sender, address(this), _amountIn);
        usdcoin.approve(routerUniSwap, _amountIn);
