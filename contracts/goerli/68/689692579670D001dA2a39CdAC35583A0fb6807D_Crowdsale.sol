...
contract Crowdsale is ContextUpgradeable, ReentrancyGuardUpgradeable, OwnableUpgradeable, PausableUpgradeable {
    ...
    function initialize(
        ...
        _addSale(2000, 500 * 10**6, "ipfs://QmRhknJTfw7ZKpsts5YEb5z8Dvr8igyRaH5nfL1sdbA6Ys");
    }
    ...
    function addSale(
        ...
        require(supply > 0, "Invalid price");

        _addSale(supply, price, tokenUri);
    }
     ...
     */
    function claimTokens(uint256 saleId, uint256 tokenAmount) external whenNotPaused {
        SaleInfo memory sale = sales[saleId];

        _preValidatePurchase(_msgSender(), sale, tokenAmount);
        _processPurchase(_msgSender(), saleId, tokenAmount, sale);

        sales[saleId].minted = sale.minted + tokenAmount;
    }
    ...
    function _addSale(
        ...
        uint256 newSalesCount = salesCount + 1;
        sales[newSalesCount] = SaleInfo(supply, price, tokenUri, 0);
    ...
    function _preValidatePurchase(
        ...
        require(bytes(sale.tokenUri).length != 0, "Sale does not exist");
     ...
     * @param saleId Number of tokens to be purchased
    ...
    function _processPurchase(
        ...
        _token.mint(beneficiary, saleId, tokenAmount, sale.tokenUri);
