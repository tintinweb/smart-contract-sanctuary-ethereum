...
contract PreviewMembership is Context, ERC721A, Ownable, ReentrancyGuard {
	...
	function mint(
		...
		require(_verify(signature, tokenAmount, tier), "Wallet is not whitelisted");
		if (whitelisted) {
	...
	function _verify(
		...
		bytes32 freshHash = keccak256(abi.encode(msg.sender, tokenAmount, tier));
