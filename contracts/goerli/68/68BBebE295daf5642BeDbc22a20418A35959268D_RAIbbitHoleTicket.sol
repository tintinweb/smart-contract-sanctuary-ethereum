...
contract RAIbbitHoleTicketStorage {
    ...
    /// Set with #setBurnMintRateForRPFs
    uint256 public burnMintRateForRPFs;
...
contract RAIbbitHoleTicket is 
    ...
    constructor(
        uint256 _burnMintRateForRPFs,
        uint96 _royaltyFee,
        string memory _tokenURI
    )
        ERC1155(_tokenURI)
    {
        // Check initial parameters, revert if it's zero
        if (_burnMintRateForRPFs == 0) {
            revert InvalidInput();
        }
        burnMintRateForRPFs = _burnMintRateForRPFs;
    ...
    /** 
     * @dev User can burn Rug Pull Frens and mint corresponding amount of tickets
     * @param _burnIds List of tokenIds of RPF that callers want to burn
     * @param _quantity Quantity of tickets that callers want to mint
     */
    function mintTicket(
        ...
        mintPublicActive
    {
        uint256 _burnQuantity = _burnIds.length;
        // Check if the burn amount doesn't match the conversion rate
        if (_burnQuantity != _quantity * burnMintRateForRPFs) {
            revert InvalidInput();
        }

        for (uint256 index=0; index<_burnQuantity; index++) {
            ERC721Burnable(rugPullFrensAddress).burn(_burnIds[index]);
        }
        ...
        override
    {
        ...
        if (tokenSupply[tokenId] > maxTicketAmount) {
            revert ExceedAmount();
        }
    ...
    /** 
     * @notice Since ERC1155 doesn't have an exact totalSupply function,
     *          and there is only one item in this collection,
     *          so we use tokenSupply[0] to represent the totalSupply. 
     *  @dev Retrieve the current total supply of the tickets
     */
    ...
    /** 
     * @dev Set the conversion rate for burning Rug Pull Frens and mint tickets
     * @param _rate Convesion rate for burning Rug Pull Frens and mint tickets
     */
    function setBurnMintRateForRPFs(uint256 _rate) 
        external
        override
        onlyOwner
    {
        // If burn & mint rate is zero, it will cause error in the mint ticket feature
        if (_rate == 0) {
            revert InvalidInput();
        }

        burnMintRateForRPFs = _rate;

        emit NumberSet(_rate, "RPF2Tickets");
    }
...
interface IRAIbbitHoleTicket {
  ...
  function mintTicket(uint256[] calldata _burnIds, uint256 _quantity) external;

    ////////////////////////////
   // Info Getters Functions //
  ////////////////////////////

  // Retrieve the current total supply of tickets.
  ...
  // Set the conversion rate between Rug Pull Frens tokens and Galaxy Ticket tokens.
  function setBurnMintRateForRPFs(uint256 _rate) external;
