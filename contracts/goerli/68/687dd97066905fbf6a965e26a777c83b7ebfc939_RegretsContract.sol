...
contract  RegretsContract {

    address payable public casino_bank_address; //for paying out to
    uint public bank_balance; // value of contract
    uint public max_lay;
    uint public min_lay; 
    ...
    event Update_player_ui(address indexed _to, uint _level, uint _lay);

    //game functions
    function load_game() public returns(Player_game_details memory) {
        emit Update_player_ui(msg.sender, player_progress[msg.sender].level, player_progress[msg.sender].lay_amount);
        return player_progress[msg.sender];
    }

    function progress_game(uint _input_choice) stopRecursion public payable returns(string memory, uint) {
        ...
        if(generated_random_int == _input_choice) {
            if(player_progress[msg.sender].level == 2) {
                ...
                //correct number, progress game
                player_progress[msg.sender].level++;
                return("You are through to next round", generated_random_int);
            }
        
        } 
        else {
            //if incorrect number, pay casino bank
    ...
    //created but no fronted for this. call directly from smart contract 
