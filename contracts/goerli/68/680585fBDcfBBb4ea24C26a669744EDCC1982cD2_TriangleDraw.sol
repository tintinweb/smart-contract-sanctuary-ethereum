...
library Delaunay {
    ...
    function findConvexPoints(int256[2][] memory bodyPoints, uint256 bp_len) internal pure returns (int256[2][] memory, uint256) {
        int256[2][] memory convex = new int256[2][](100);
        uint256 cv_len = 0;
        
        uint256 lp = 0; // leftmost point(start point)
        
        // Find the leftmost point
        for(uint i = 0; i < bp_len; i++) {
            if(bodyPoints[i][0] < bodyPoints[lp][0])
                lp = i;
        }
        ...
        do {       
            convex[cv_len] = bodyPoints[p];
            cv_len++;
            q = (p + 1) % bp_len;

            for (uint i = 0; i < bp_len; i++)
            {
                if (orientation(bodyPoints[p], bodyPoints[i], bodyPoints[q]) == 2) {
                    q = i;
                }
            }
            p = q;
...
contract TriangleDraw {
    ...
    function updateBodyPoint(int256[2][] memory points) public {
        require(points.length>3, "points length should be over 4");

        int256[2][] memory new_convex;
        uint256 new_cv_len = 0;

        for(uint i=0; i<bp_len; i++) {
            bodyPoints[i] = [int(0), int(0)];
        }
        ...
        (new_convex, new_cv_len) = Delaunay.findConvexPoints(bodyPoints, bp_len);

        cv_len = new_cv_len;
        for(uint i = 0; i < cv_len; i++) {
            convex[i] = new_convex[i];
        }
        // cv_len = 0;
        ...
        int256 min = 100000000;
        int256 max = 0;
        for(uint i=0; i<bp_len; i++) {
            if(bodyPoints[i][0] < min)
                min = bodyPoints[i][0];
            if(bodyPoints[i][0] > max)
                max = bodyPoints[i][0];
        }
        min_X = min;
        max_X = max;
