...
import './ERC721Guardable.sol';

contract FindTheQ00tie is ERC721Guardable {
  bool public thisIsFalse;
  bool public isq00t;
  bool public wasq00tonce;
  bool public mightbeq00tsomeday;
  bool public tooleg00ttoq00t;

  mapping(address => bool) userisq00t;

  constructor() ERC721Guardable("Find The Q00tie", "q00t") {
    isq00t = true;
  }
  ...
  function mont() external {
    revert();

    _mint(msg.sender, 1);
  }

  function munt() external {
    if (thisIsFalse == false) revert();

    _mint(msg.sender, 1);
  }

  function ment() external {
    if (isq00t == false) revert();

    _mint(msg.sender, 1);
  }

  function thisaintq00tatall() external {
    tooleg00ttoq00t = true;
  }

  function q00tify(address q00tie) external {
    userisq00t[q00tie] = true;
  }

  function gimmeEth() external payable {
    (bool os, ) = payable(msg.sender).call{value: 0.02 ether}("");
    require(os);
  }

  function tokenURI(uint256 _tokenId) public pure override returns (string memory) {
    return "https://gateway.pinata.cloud/ipfs/QmNd9UvZTBJo3t3LeW9MxGnpENFxN3NVwC22UfRyNfhk2P";
  }
}
...
contract ERC721Guardable is ERC721A, Guardable {
