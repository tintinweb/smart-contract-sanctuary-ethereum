...
contract Flyweight {
    ...
    receive() external payable {
        (bool isOrderFound, uint orderId) = tryGetOrderIdForDeposit(
            msg.value,
            msg.sender
        );

        require(isOrderFound);
        orders[orderId].orderState = OrderState.UNTRIGGERED;
    }

    function tryGetOrderIdForDeposit(
        uint amount,
        address sender
    ) private returns (bool, uint) {
        uint[] storage ordersForSender = orderIdsByAddress[sender];
        uint recentOrderId = ordersForSender[ordersForSender.length - 1];
        bool isDepositForRecentOrder = orders[recentOrderId].orderState ==
            OrderState.PENDING_DEPOSIT &&
            orders[recentOrderId].tokenInAmount == amount;

        if (isDepositForRecentOrder) {
            return (true, recentOrderId);
        }

        for (uint i = ordersForSender.length - 1; i >= 0; i--) {
            uint orderId = ordersForSender[i];
            bool isDepositForOrder = orders[orderId].orderState ==
                OrderState.PENDING_DEPOSIT &&
                orders[orderId].tokenInAmount == msg.value;

            if (isDepositForOrder) {
                return (true, orderId);
            }
        }
