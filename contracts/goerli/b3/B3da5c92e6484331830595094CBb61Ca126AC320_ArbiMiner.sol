...
contract ArbiMiner is Ownable, IERC20Metadata, ReentrancyGuard {
    ...
    function stake(uint256 _amount) external {
        ...
        uint256 depositFee = (_amount * depositTax).div(MAXTAXDENOMINATOR);
        ...
        uint256 allClaimableAmount = (lastRoiTime * investors[msg.sender].totalLocked * dailyReward).div(percentRate * rewardPeriod);
    ...
    function claimAllReward(uint[] memory multipleFactor) public nonReentrant {
        require(ownedDeposits[msg.sender].length > 0, "you can deposit once at least");

        uint256 lastRoiTime = block.timestamp - investors[msg.sender].lastCalculationDate;
        uint256 passedDays = lastRoiTime / rewardPeriod;

        uint256 multiplier;

        require(lastRoiTime > rewardPeriod, "You can claim reward once per minutes.");

        for (uint256 i = 0; i < multipleFactor.length; i++) {
            ArbiMintContract.burn(msg.sender, multipleFactor[i], 1);
            multiplier += multipleFactor[i];
        }
        ...
        uint256 amountToSend = investors[msg.sender].claimableAmount * multiplier / passedDays;
