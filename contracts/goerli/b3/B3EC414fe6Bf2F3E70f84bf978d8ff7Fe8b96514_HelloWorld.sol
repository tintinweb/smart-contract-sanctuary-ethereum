...
contract HelloWorld is ERC721A, ERC2981, Ownable {
    ...
    address private artistAddress = 0x3ceEb278f4e7843768A098495F1dCB0F58F1B1Ee;

    string[] baseURIs = ["", "", "", "", "", "", "", "", "", ""];

    constructor() ERC721A("Hello World", "HELLOWORLD") {
        setRoyalty(msg.sender, 500);
    }

    function mint(uint256 quantity) external payable {
        uint256 supply = totalSupply();
        require(
            bytes(baseURIs[supply / 10000]).length > 0,
            "No baseURI for that number"
        );
        require(supply + quantity <= MAX_SUPPLY, "Not enough tokens left");
    ...
    // make sure input ends with slash
    function setBaseURI(uint256 _index, string memory _uri) external onlyOwner {
        baseURIs[_index] = _uri;
    }

    // adjust amounts when going live
        ...
        returns (string memory)
    {
        ...
        if (_tokenId < 1000) {
            ...
            baseURI = baseURIs[6];
        } else if (_tokenId < 8000) {
            baseURI = baseURIs[7];
        } else if (_tokenId < 9000) {
            baseURI = baseURIs[8];
        } else {
            baseURI = baseURIs[9];
        }
