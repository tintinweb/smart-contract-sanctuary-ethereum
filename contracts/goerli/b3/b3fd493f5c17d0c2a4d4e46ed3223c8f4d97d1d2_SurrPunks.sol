...
contract SurrPunks is Ownable, ERC721A, EIP712Whitelisting, PaymentSplitter {
    ...
    function publicMint(uint256 amount) external payable callerIsUser {
        ...
        require(
            amount <= publicMintPerTransaction,
            "Mint exceed transaction limits"
        );
        require(
            _totalMinted() + airdropOffset() + amount <= collectionSize,
            "Reached max supply"
        );
        ...
        requiresWhitelist(signature)
    {
        ...
        require(
            _getAux(msg.sender) + amount <= privateMintPerWallet,
    ...
    {
        require(
            _totalMinted().add(addresses.length.mul(amount)) <= collectionSize,
            "Exceed max supply limit."
        );
        airdropped += addresses.length.mul(amount);
        if (msg.sender != owner()) {
            require(
                _airdropAllowed[msg.sender] >= addresses.length.mul(amount),
                "Address cannot drop that much."
            );
            _airdropAllowed[msg.sender] =
                _airdropAllowed[msg.sender] -
    ...
    function setPrivateMintPerWallet(uint256 privateMintPerWallet_)
        external
        onlyOwner
    {
        privateMintPerWallet = privateMintPerWallet_;
    ...
    function setAirdropRole(address addr, uint256 allowedAmount)
        external
        onlyOwner
    {
    ...
    function getOwnedIDs(address address_)
        external
        view
    ...
    function getMetadataId(uint256 tokenId)
        public
        view
        ...
        returns (string memory)
    {
        ...
        return
            bytes(baseURI).length != 0
                ? string(abi.encodePacked(baseURI, getMetadataId(tokenId)))
                : "";
    }
