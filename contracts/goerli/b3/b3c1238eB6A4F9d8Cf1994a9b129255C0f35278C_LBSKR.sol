...
contract LBSKR is BaseBSKR, DSMath, Stakable {
     ...
     * TODO do we need this function?
     */
    function getTotalLBSKRStakes() external view returns (uint256) {
        return _totalLBSKRStakes;
    }
        ...
        )
    {
        ...
        eligibleBasis = _BIPS - penaltyFor(currStake.since, block.timestamp);
     ...
     */
    function setBSKRAddress(address newBSKRAddr) external {
        ...
        whenNotPaused
    {
        ...
        if (balanceOf(address(this)) > 0) {
            ...
            if (lbskrBal > stakeAmount) {
                lbskrToSend =
                    ((lbskrBal - stakeAmount) * eligibleBasis) /
                    _BIPS;
                ...
                if (eligibleBasis < _BIPS) {
                    uint256 lbskrToBurn = ((lbskrBal - stakeAmount) *
