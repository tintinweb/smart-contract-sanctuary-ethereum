...
contract DinoNFT is ERC721, Ownable {
    ...
    string[] public speciesURIs;
    mapping(uint => string) public speciesBase;
  ...
  function addSpeciesURI(string[] memory _speciesURIs) public onlyOwner
  {
    // species A, B, C, D, E
    require(_speciesURIs.length == 5,"there must only be 5 species");
    for(uint i = 0; i < _speciesURIs.length; i++){
      speciesURIs.push(_speciesURIs[i]);
    }
...
//currently testing -- mint increments -- random number calling from egg -- 

contract DinoEgg is ERC721, ERC721Burnable, Ownable, VRFConsumerBaseV2 {
    ...
    address vrfResolver = 0x2Ca8E0C643bDe4C2E08ab1fA0da3401AdAD7734D;


    mapping (uint => bool) private eggToDino;   

    //interfacing with structure
    ...
    mapping(address => uint) public mintCountPerOwner;
    mapping(uint => bool) public burnedIDs;
    mapping(uint => uint) private eggToDinoID;
    mapping (uint => bool) public indexedEggID;
    ...
    {
        require(currentState != States.publicMint, "cant go forward anymore");
    ...
    {
        require(currentState != States.bigStakeMint,"cant reverse the state to nothing");
    ...
    function hasTenKorMoreDino(address _caller) internal view returns(bool)
    {
        uint256 callerBalance = dinoToken.balanceOf(_caller);
        uint256 holdBarrier = 10000 * 10 ** 18;

        if(callerBalance >= holdBarrier){
            return true;
        } else {
            return false;
        }
    }
    mapping(uint => uint) private eggDino;
    //requires token approval
    function vipAndTeamMint(uint quantity) public
    {
        require(isVIP[msg.sender] == true,"you're up to no good if you're seeing this.");
        require(vipMintCount[msg.sender] <= vipMintLimit, "that's all you get from this mint function");
    ...
    function stakersMint(uint quantity) public {
        ...
        require(mintCountPerOwner[msg.sender] <= allOtherLimits, "you've minted the max amount for this phase");
        require(dinoToken.transferFrom(msg.sender, address(this), (1000 * 10 ** 18) * quantity),"either no approval or not enough dino");

        if(currentState == States.bigStakeMint){
            require(isBigStaker[msg.sender] == true, "youre not a big staker. check back later");
        } else if(currentState == States.littleStakeMint) {
            require(hasTenKorMoreDino(msg.sender) == true || isSmallStaker[msg.sender] == true, "not staking or have enough dino");
        }

        //loop through quantity amount and begin egg minting process
        for(uint i = 0; i < quantity; i++){
            mintCountPerOwner[msg.sender] ++;
    ...
    {   
        ...
        require(publicMintCount[msg.sender] <= pubMintLimit,"you've minted max for public");
    ...
    {   
        require(canHatch == true, "cant hatch yet bubba.");         
        require(ownerOf(_eggID) == msg.sender, "what are u doing?");
        require(burnedIDs[_eggID] == false, "egg already burned");
        
        if(isVIP[msg.sender] == false){
        ...
        emit numberRequested(_requestID, requestBySender[_requestID]);
    ...
    //input link to egg picture -- "ipfs://somehashedstuff/egg.json"
    function _baseURI() internal view virtual override returns (string memory)
    {
        return baseURI;
    }
    ...
    returns (string memory)
  {
    ...
    function burnDinoTokens() public onlyOwner
    {
        ...
        dinoToken.transferFrom(address(this), address(0x000000000000000000000000000000000000dEaD), burnBalance);
    }
    ...
    function turnHatchOn(bool _go) public onlyOwner
    {
        canHatch = _go;
    }
    ...
    {
        require(_randomWords[0] > 0,"promise not fulfilled");   
        
        uint word = _randomWords[0];
        uint range = (word % 5) + 1;
        emit numberResolved(requestID, requestBySender[requestID]);
        // Infinite loop, but will always break eventually (worst case 1/5 on each iteration)
        while(true) {
            if(_base[range - 1].current() < 2000) {
                ...
                dinoHatchery.indexHatcher(requestBySender[requestID], tokenid);
                emit hatchSent(requestBySender[requestID], tokenid);
                break; // Does the same thing as return, actually. 
            }
            else {
