...
contract Infanity is ERC1155, ERC2981, IInfanity, DefaultOperatorFilterer, AccessControl  {
        ...
        private 
    {
        ...
        if (enabledTokenGated) {
            ...
            if (_isValidTokenAddress(tokenGatedAddrs)) {
                revert NonSupportedTokenGatedAddress();
            }
        }
        ...
        onlyExistingItem(_id) 
    {
        if (_enabled) {
            ...
            if (_isValidTokenAddress(_token)) {
                revert NonSupportedTokenGatedAddress();
            }
        }
    ...
    function _isValidTokenAddress(address _addrs) internal view returns (bool) {
        return _addrs.supportsInterface(type(IERC1155).interfaceId) || _addrs.supportsInterface(type(IERC721).interfaceId);
    }
