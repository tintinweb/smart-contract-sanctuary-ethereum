...
/// at commit fffd375659358af54a6e8bbf8c3aa44188894c81
/// Changes from original:
/// - Upgraded to Solidity ^0.8.0
/// - Added custom errors
/// - Updated inline docs
library StateProofVerifier {
    ...
    error StateProofVerifier__verifyStateProof_invalidProofLength();
    error StateProofVerifier__verifyStateProof_lengthMismatch();
    error StateProofVerifier__verifyBlockHeader_blockhashMismatch();
    error StateProofVerifier__extractAccountFromProof_invalidFieldLength();
        ...
        )
    {
        ...
        if (proofs.length != 2)
            revert StateProofVerifier__verifyStateProof_invalidProofLength();
        ...
        if (slotProofs.length != _slotHashes.length)
            revert StateProofVerifier__verifyStateProof_lengthMismatch();
        ...
        returns (BlockHeader memory)
    {
        ...
        if (header.hash != blockhash(header.number))
            revert StateProofVerifier__verifyBlockHeader_blockhashMismatch();
        return header;
    }
    ...
    function extractAccountFromProof(
        ...
        if (acctFields.length != 4)
            revert StateProofVerifier__extractAccountFromProof_invalidFieldLength();
...
library MerklePatriciaProofVerifier {
    ...
    error MerklePatriciaProofVerifier__extractProofValue_notRootHash();
    error MerklePatriciaProofVerifier__extractProofValue_invalidRootHash();
    error MerklePatriciaProofVerifier__extractProofValue_invalidNodeHash();
    error MerklePatriciaProofVerifier__extractProofValue_divNodeNotLast();
    error MerklePatriciaProofVerifier__extractProofValue_leafNodeNotLast();
    error MerklePatriciaProofVerifier__extractProofValue_extensionNodeIsLast();
    error MerklePatriciaProofVerifier__extractProofValue_branchNodeNotNibble();
    error MerklePatriciaProofVerifier__extractProofValue_branchLeafNodeNotLast();
    error MerklePatriciaProofVerifier__extractProofValue_branchNodeNotLast();
    error MerklePatriciaProofVerifier__merklePatriciaCompactDecode_compactIsZero();
    error MerklePatriciaProofVerifier__merklePatriciaCompactDecode_invalidFirstNibble();
    error MerklePatriciaProofVerifier__decodeNibbles_compactIsZero();
    error MerklePatriciaProofVerifier__decodeNibbles_skipNibblesGreaterThanLength();

    /// @notice Validates a Merkle-Patricia-Trie proof.
    /// If the proof proves the inclusion of some key-value pair in the
    /// trie, the value is returned. Otherwise, i.e. if the proof proves
    /// the exclusion of a key from the trie, an empty byte array is
    /// returned.
    /// @param rootHash is the Keccak-256 hash of the root node of the MPT
    /// @param path is the key of the node whose inclusion/exclusion we are proving
    /// @param stack is the stack of MPT nodes (starting with the root) that need to be traversed during verification
    /// @return value value whose inclusion is proved or an empty byte array for a proof of exclusion
    function extractProofValue(
        ...
        if (stack.length == 0) {
            ...
            if (
                ...
                revert MerklePatriciaProofVerifier__extractProofValue_notRootHash();
            }
            return new bytes(0);
        }
        ...
        for (uint256 i = 0; i < stack.length; i++) {
            ...
            if (i == 0 && rootHash != stack[i].rlpBytesKeccak256()) {
                revert MerklePatriciaProofVerifier__extractProofValue_invalidRootHash();
            }
            // ... whereas all other nodes are hashed with the MPT
            // hash function.
            if (i != 0 && nodeHashHash != _mptHashHash(stack[i])) {
                revert MerklePatriciaProofVerifier__extractProofValue_invalidNodeHash();
            }
            ...
            if (node.length == 2) {
                ...
                if (prefixLength < nodeKey.length) {
                    ...
                    if (i < stack.length - 1) {
                        ...
                        revert MerklePatriciaProofVerifier__extractProofValue_divNodeNotLast();
                    }
                ...
                if (isLeaf) {
                    ...
                    if (i < stack.length - 1) {
                        ...
                        revert MerklePatriciaProofVerifier__extractProofValue_leafNodeNotLast();
                    }
                    ...
                    if (i == stack.length - 1) {
                        ...
                        revert MerklePatriciaProofVerifier__extractProofValue_extensionNodeIsLast();
                    }
                ...
                if (mptKeyOffset != mptKey.length) {
                    ...
                    if (nibble >= 16) {
                        ...
                        revert MerklePatriciaProofVerifier__extractProofValue_branchNodeNotNibble();
                    }
                    ...
                    if (_isEmptyBytesequence(node[nibble])) {
                        ...
                        if (i != stack.length - 1) {
                            ...
                            revert MerklePatriciaProofVerifier__extractProofValue_branchLeafNodeNotLast();
                        }
                    ...
                    if (i != stack.length - 1) {
                        ...
                        revert MerklePatriciaProofVerifier__extractProofValue_branchNodeNotLast();
                    }
    ...
    /// @notice Computes the hash of the Merkle-Patricia-Trie hash of the RLP item.
    ...
    /// @return hash Keccak-256(MPT-hash(item))
        ...
        returns (bool isLeaf, bytes memory nibbles)
    {
        ...
        if (firstNibble == 0) {
            ...
            revert MerklePatriciaProofVerifier__merklePatriciaCompactDecode_invalidFirstNibble();
        }
        ...
        returns (bytes memory nibbles)
    {
        ...
        if (skipNibbles > length)
            revert MerklePatriciaProofVerifier__decodeNibbles_skipNibblesGreaterThanLength();
