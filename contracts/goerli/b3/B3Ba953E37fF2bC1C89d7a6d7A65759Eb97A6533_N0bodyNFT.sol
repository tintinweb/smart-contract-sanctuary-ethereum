...
contract N0bodyNFT is ERC721A, Ownable, PaymentSplitter {
    ...
    uint256 public saleStartTime = 1668252900;
    ...
    address[] private _team = [0xAF50D91c1eBC1eF0f7A00d0B66e2bCF2059f3B5E];
    ...
    function getStep() public view returns (Step actualStep) {
        ...
        if (
            ...
            block.timestamp < saleStartTime + 5 minutes
        ) {
            return Step.WhitelistSale;
        }
        if (
            block.timestamp >= saleStartTime + 5 minutes &&
            block.timestamp < saleStartTime + 10 minutes
        ) {
            return Step.PublicSale;
        }
        if (
            block.timestamp >= saleStartTime + 10 minutes &&
            block.timestamp < saleStartTime + 12 minutes
        ) {
