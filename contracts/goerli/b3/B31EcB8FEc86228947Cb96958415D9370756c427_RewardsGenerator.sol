...
contract RewardsGenerator is IRewardsGenerator, OwnableUpgradeable, AbstractDependant {
    ...
    {
        contractTypes = new IPolicyBookFabric.ContractType[](6);
        ...
        contractTypes[5] = IPolicyBookFabric.ContractType.CUSTODIAN;
    }
    ...
    function setRewardPerBlock(uint256 _rewardPerBlock) external onlyOwner {
        ...
        for (uint256 i = 0; i < contractTypes.length; i = uncheckedInc(i)) {
    ...
    function setRewardRatios(uint256[] memory rewardRatios) external onlyOwner {
        ...
        for (uint256 i = 0; i < rewardRatios.length; i = uncheckedInc(i)) {
    ...
    function _updateCumulativeSum(
        ...
        distributionInfo[contractType].toUpdateRatio = contractTypeRewardPerBlock
            .mul(PERCENTAGE_100.tryMul(10**5))
            .tryDiv(totalStaked);
        ...
        returns (uint256)
    {
        ...
        uint256 rewardPerBlockPolicyBook =
            policyBookRewardMultiplier
                ...
                distributionInfo[contractType].totalContractTypeStaked.add(
                    policyBookRewardMultiplier.tryMul(APY_TOKENS)
                )
            );
        ...
        return
            rewardPerBlockPolicyBookSTBL
                .mul(getBlocksPerDay(_currentNetwork).tryMul(365))
                .mul(100)
                .uncheckedDiv(totalStakedPolicyBook);
    }
    ...
    {
        return
            rewardPerBlock.mul(distributionInfo[contractType].rewardRatio).uncheckedDiv(
                PERCENTAGE_100
            );
    }
