...
contract MoonMoonz is Ownable, Pausable, ERC721("Moon Moonz", "MOONZ"), ERC721Enumerable {
  ...
  uint256 public constant MAX_AMOUNT = 3;
  ...
  uint256 public saleState; // 0 -> closed / 1 -> claimFree / 2 -> claimPremint / 3 -> claim

  address public immutable splitter; // -> payment splitter

  bytes32 public whitelist1Root;

  bytes32 public whitelist2Root;

  mapping(address => bool) public whitelist1Bought;

  mapping(address => bool) public whitelist2Bought;

  mapping(address => uint256) public bought; // -> amount bought during sale by address
  ...
  modifier requireWhitelisted(
    bytes32 root,
    bytes32[] calldata proof,
    mapping(address => bool) storage _bought
  ) {
    require(!_bought[msg.sender], "Max amount per address reached");
    require(proof.verify(root, keccak256(abi.encodePacked(msg.sender))), "Invalid whitelist proof");
    _;
    _bought[msg.sender] = true;
  }
  ...
  modifier requireValue(uint256 amount) {
    require(msg.value == PRICE * amount, "Invalid ether amount");
  ...
  modifier requireSupply() {
    _;
    require(super.totalSupply() <= MAX_SUPPLY, "Max supply was reached");
  }
  ...
  constructor(
    ...
    whitelist1Root = root1;
    whitelist2Root = root2;
  ...
  function claimFree(bytes32[] calldata proof)
    external
    requireSaleState(1)
    requireWhitelisted(whitelist1Root, proof, whitelist1Bought)
    requireSupply
  {
    super._safeMint(msg.sender, super.totalSupply());
  }

  function claimPremint(bytes32[] calldata proof)
    external
    payable
    requireSaleState(2)
    requireValue(1)
    requireWhitelisted(whitelist2Root, proof, whitelist2Bought)
    requireSupply
  {
    super._safeMint(msg.sender, super.totalSupply());
  }
  ...
  function claim(uint256 amount)
    ...
    requireSupply
  {
    require((bought[msg.sender] += amount) <= MAX_AMOUNT, "Max amount per address reached");
  ...
  function setRoots(bytes32 _root1, bytes32 _root2) external onlyOwner {
    whitelist1Root = _root1;
    whitelist2Root = _root2;
  }
  ...
  function _beforeTokenTransfer(
    ...
    require(from == address(0) || Calendar.night(timezoneOf[tokenId]), "Still day");
    super._beforeTokenTransfer(from, to, tokenId);
  }
