 ...
 */
contract Vault is TokenHandler, OwnableUpgradeable, VaultStorageV1 {
     ...
     * @param ethValue The payout of native Ether amount to withdraw
     */
    function withdraw(ERC20Payment[] calldata payout, uint256 ethValue) public onlyOwner {
        ...
        if (ethValue > 0) {
            (bool success, ) = owner.call{value: ethValue}("");
     ...
     * @param ethValue The payout of native Ether amount to whitehat
    ...
    function payWhitehat(
        ...
        if (ethValue > 0) {
            (bool success, ) = feeTo.call{value: ethValue / 10}("");
            require(success, "Vault: Failed to send ether to fee receiver");
            (success, ) = wh.call{value: ethValue, gas: gas}("");
     ...
     * @dev Only callable by owner
     * @dev Owner needs to set isPausedOnImmunefi to false before they can stop using the vault on Immunefi
