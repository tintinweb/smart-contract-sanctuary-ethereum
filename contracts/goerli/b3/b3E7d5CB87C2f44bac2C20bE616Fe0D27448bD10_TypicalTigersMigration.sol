...
contract TypicalTigersMigration is ERC721Enumerable, Ownable, OperatorFilterer {
    bool public operatorFilteringEnabled;

    address public ApeContract = 0x8580156166e7ae7c4a050821a8F2a03bcf9d2Ee9;
    address public ApeForge = 0xac4438580BdeA883608A88922dDE0944cf32d837;

    uint256 public maxSupply = 3333;
    bool public claimIsActive = true;

    string public _baseTokenURI =
        "ipfs://Qmd17vfvMGQyBabNVP4J86838i3uryuNYrAYxhFxo4sEVL/";

    // NOTE: Map old token id to new (same) token id
    mapping(uint => uint256) public apeClaimed;


    constructor() ERC721("Typical Tigers", "TIGER") {
    ...
    function claimApes(uint256[] memory theTokenIDs) public {
        ...
        for (uint256 i; i < numTokens; i++) {
            ...
            require(IERC721(ApeContract).ownerOf(theTokenIDs[i]) == msg.sender);
            require(apeClaimed[theTokenIDs[i]] != 1);
        }
        for (uint256 i; i < numTokens; i++) {
                mintApe(theTokenIDs[i]);
            }
        }
    ...
    function claimForgedApes(uint256[] memory theTokenIDs) external {
        ...
        require(numTokens != 0, "Empty token ID array");
        for (uint256 i; i < numTokens; i++) {
            require(ApeForgeInterface(ApeForge)._nftTokenForges(theTokenIDs[i]) == msg.sender,"Token not owned");
            require(apeClaimed[theTokenIDs[i]] != 1,"Tiger already claimed");
        }
        for (uint256 i; i < numTokens; i++) {
                mintApe(theTokenIDs[i]);
