...
contract StartMiningIsERC721 is Ownable, ReentrancyGuard, ERC721Pausable {
    ...
    AggregatorV3Interface internal priceFeed; // Chainlink Goerli ETH/USD
    ...
    */
    constructor(string memory _baseURI, IERC20 _stableCoin, address _recipient, AggregatorV3Interface _priceFeed) ERC721("Startmining", "START") {
        baseURI = _baseURI;
    ...
    */
    modifier mintModifier(uint16 _amount, address _referral) {
        require(!salePaused, "Sale not open");
    ...
    * @param uint256 _nextNFT : Id of the first token minus 1.
    * @param uint256 _limit : Maximum amount of units.
    * @param uint16 _salePrice : Price value of 1 NFT.
    */
    function initMint(uint256 _nextNFT, uint256 _limit, uint16 _salePrice) external onlyOwner {
        require(_nextNFT >= nextNFT, "New nextNFT must be higher than the old one");
