...
// Copyright @ Props Labs LLC. Usage for running test scripts for contracts under modules only.
// initially inspired by thirdweb's registry
pragma solidity ^0.8.11;

import "./PropsRegistry.sol";
import "./PropsAccessRegistry.sol";
...
contract PropsFactory is Multicall, ERC2771Context, AccessControlEnumerable {
    // @dev Only CONTRACT_ADMIN_ROLE holders can setRegistry and setAccessRegistry
    ...
    // @dev only DEPLOYER_ROLE holders can
    bytes32 public constant DEPLOYER_ROLE = keccak256("DEPLOYER_ROLE");

    PropsRegistry public registry;
    PropsAccessRegistry public accessRegistry;
    ...
    constructor(address _trustedForwarder, address _registry, address _accessRegistry) ERC2771Context(_trustedForwarder) {
        _setupRole(DEFAULT_ADMIN_ROLE, _msgSender());

        _setRoleAdmin(CONTRACT_ADMIN_ROLE, DEFAULT_ADMIN_ROLE);
        _setRoleAdmin(FACTORY_ROLE, CONTRACT_ADMIN_ROLE);
        _setRoleAdmin(DEPLOYER_ROLE, FACTORY_ROLE);

        registry = PropsRegistry(_registry);
        accessRegistry = PropsAccessRegistry(_accessRegistry);
    }

    /// @dev Deploys a proxy that points to the latest version of the given contract type.
    function deployProxy(bytes32 _type, bytes memory _data) external minRole(DEPLOYER_ROLE) returns (address) {
    ...
    function deployProxyDeterministic(
        ...
        bytes memory _data,
        bytes32 _salt
    ) public minRole(DEPLOYER_ROLE) returns (address) {
    ...
    function deployProxyByImplementation(
        ...
        bytes memory _data,
        bytes32 _salt
    ) public minRole(DEPLOYER_ROLE) returns (address deployedProxy) {
        ...
        // accessRegistry.add(_msgSender(), deployedProxy);
        accessRegistry.grantRole(keccak256("OPERATOR_ROLE"), deployedProxy);
    ...
    function addImplementation(address _implementation) external minRole(FACTORY_ROLE){
        IPropsContract module = IPropsContract(_implementation);
    ...
    function approveImplementation(address _implementation, bool _toApprove) external minRole(FACTORY_ROLE){
    ...
    // @dev ability to update the registry
    function setRegistry(address _registry) external minRole(CONTRACT_ADMIN_ROLE){
      registry = PropsRegistry(_registry);
    }
    ...
    function revokeRole(bytes32 role, address account) public virtual override(AccessControl, IAccessControl) minRole(CONTRACT_ADMIN_ROLE) {
    ...
    */
    modifier minRole(bytes32 _role) {
        require(_hasMinRole(_role), "Not authorized factory");
        _;
    }
...
contract PropsRegistry is Multicall, ERC2771Context, AccessControlEnumerable {
    ...
    constructor(address _trustedForwarder) ERC2771Context(_trustedForwarder) {
        ...
        _setRoleAdmin(CONTRACT_ADMIN_ROLE, DEFAULT_ADMIN_ROLE);
        _setRoleAdmin(OPERATOR_ROLE, CONTRACT_ADMIN_ROLE);
    }

    // slither-disable-next-line similar-names
    function add(address _deployer, address _deployment) external {
        require(_hasMinRole(OPERATOR_ROLE) || _deployer == _msgSender(), "not operator or deployer.");
    ...
    */
    modifier minRole(bytes32 _role) {
        require(_hasMinRole(_role), "Not authorized reg");
        _;
    }
...
contract PropsAccessRegistry is Multicall, ERC2771Context, AccessControlEnumerable {
    bytes32 public constant OPERATOR_ROLE = keccak256("OPERATOR_ROLE");
    bytes32 private constant CONTRACT_ADMIN_ROLE = keccak256("CONTRACT_ADMIN_ROLE");

    using EnumerableSet for EnumerableSet.AddressSet;

    /// @dev wallet address => [ressource addresses]
    mapping(address => EnumerableSet.AddressSet) private resourceAccess;

    // @dev will we ever need more than 8 bytes for roles aka how many roles can a user have on 1 contract?
    mapping(address => mapping(address => uint8)) private resourceAccessCount;

    event Added(address indexed account, address indexed deployment);
    event Deleted(address indexed account, address indexed deployment);
    event Reduced(address indexed account, address indexed deployment);
    ...
    function add(address _account, address _deployment) external {
        require(_hasMinRole(OPERATOR_ROLE) || _account == _msgSender(), "not operator or account.");

        if(!resourceAccess[_account].contains(_deployment)){
          bool added = resourceAccess[_account].add(_deployment);
          require(added, "failed to add");
        }
        resourceAccessCount[_account][_deployment]++;

        emit Added(_account, _deployment);
    }
    ...
    function remove(address _account, address _deployment) external {
        ...
        if(resourceAccessCount[_account][_deployment] > 0) resourceAccessCount[_account][_deployment]--;

        if(resourceAccessCount[_account][_deployment] == 0 && resourceAccess[_account].contains(_deployment)){
          bool removed = resourceAccess[_account].remove(_deployment);
          resourceAccessCount[_account][_deployment] = 0;
          require(removed, "failed to remove");
          emit Deleted(_account, _deployment);
        }else{
          emit Reduced(_account, _deployment);
        }

    }

    function getAll(address _account) external view returns (address[] memory) {
        return resourceAccess[_account].values();
    }

    function countSpecific(address _account, address _contractAddress) external view returns (uint8) {
        return resourceAccessCount[_account][_contractAddress];
    }

    function count(address _account) external view returns (uint256) {
        return resourceAccess[_account].length();
    }
    ...
    */
    modifier minRole(bytes32 _role) {
        require(_hasMinRole(_role), "Not authorized access reg");
        _;
    }
