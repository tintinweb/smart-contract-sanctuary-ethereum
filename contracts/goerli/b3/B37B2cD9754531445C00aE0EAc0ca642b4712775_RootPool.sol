...
import "./IPolidoAdapter.sol";
import "./IPool.sol";

// todo make it sequential
// expiry  and cancellation
// confirm that only root can send message to child and viceversa
// claim reinterrancy check
// governance fallback
// change tunnel address // goverance
...
contract RootPool {
    ...
    // erc20PredicateProxt 0xdD6596F2029e6233DEFfaCa316e6A95217d4Dc34
    ...
    constructor(
        ...
        IRootchainManagerProxy _rootChainManagerProxy,
    ...
    function processExits(address inputToken) public {
        withdrawManagerProxy.processExits(inputToken);
    }

    function receiveMessageAndStakeTokens(bytes memory messageReceiveData)
        public
    {
        rootTunnel.receiveMessage(messageReceiveData);
        (uint256 batchNumber, uint256 amount) = rootTunnel.readData();

        withdrawManagerProxy.processExits(address(maticToken));
        uint256 balance = maticToken.balanceOf(address(this));

        require(balance >= amount, "Insufficient amount to bridge");

        maticToken.approve(address(polidoAdapter), amount);
        uint256 stMatic = polidoAdapter.depositForAndBridge(
            address(this),
            amount
        );
        rootTunnel.sendMessageToChild(
            abi.encode(batchNumber, stMatic, RootShuttleStatus.PROCESSED)
        );
    }

    function recieveAndCancelShuttle(bytes memory messageReceiveData) public {
        rootTunnel.receiveMessage(messageReceiveData);
        (uint256 batchNumber, uint256 amount) = rootTunnel.readData();
        ...
        maticToken.approve(erc20PredicateProxy, amount);
        ...
        rootTunnel.sendMessageToChild(
            abi.encode(batchNumber, 0, RootShuttleStatus.CANCELLED)
        );
    }
}
