...
#x=========================================================================x
#|    ____       _                ____             __                   __ |
#|   / __ \_____(_)___  ____     / __ \_________  / /_____  _________  / / |
#|  / / / / ___/ / __ \/ __ \   / /_/ / ___/ __ \/ __/ __ \/ ___/ __ \/ /  |
#| / /_/ / /  / / /_/ / / / /  / ____/ /  / /_/ / /_/ /_/ / /__/ /_/ / /   |
#| \____/_/  /_/\____/_/ /_/  /_/   /_/   \____/\__/\____/\___/\____/_/    |
#|                                                                         |
#x=========================================================================x
#================================== veORN ==================================
#===========================================================================
...
# veORN is basically a fork, with the key difference that 1 ORN locked for 1 second would be ~ 1 veORN,
# As opposed to ~ 0 veORN (as it is with veCRV)
...
def toggleEmergencyUnlock():
    """
    @dev Used to allow early withdrawals of veORN back into ORN, in case of an emergency
    """
    assert msg.sender == self.admin  # dev: admin only
...
def recoverERC20(token_addr: address, amount: uint256):
    """
    @dev Used to recover non-ORN ERC20 tokens
    """
    assert msg.sender == self.admin  # dev: admin only
    assert token_addr != self.token  # Cannot recover ORN. Use toggleEmergencyUnlock instead and have users pull theirs out individually
...
# real coins.
# FRAX adds minimal 1-1 ORN/veORN, as well as a voting multiplier
...
@external
@view
def totalORNSupply() -> uint256:
    """
    @notice Calculate ORN supply
    @dev Adheres to the ERC20 `totalSupply` interface for Aragon compatibility
    @return Total ORN supply
    """
    return ERC20(self.token).balanceOf(self)

@external
@view
def totalORNSupplyAt(_block: uint256) -> uint256:
    """
    @notice Calculate total ORN at some point in the past
    @param _block Block to calculate the total voting power at
    @return Total ORN supply at `_block`
    """
    assert _block <= block.number
    _epoch: uint256 = self.epoch
    target_epoch: uint256 = self.find_block_epoch(_block, _epoch)
