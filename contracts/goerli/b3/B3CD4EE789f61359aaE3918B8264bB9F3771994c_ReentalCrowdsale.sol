...
contract ReentalCrowdsale is
  UUPSUpgradeableByRole,
  RefundableUpgradeable
{

  uint256 public minimum;
  mapping(address => bool) public listed;
  mapping(address => uint256) public poolPercent; // not using it anymore
  mapping(address => uint256) public price;

  event ListedToken(address indexed token, uint256 price, uint256 listingTime);
  ...
  function setMinimum(
    ...
    public
    onlyRole(_CROWDSALE_ADMIN_ROLE)
  {
    minimum = min;
  }
    ...
    returns (uint256)
  {
    if(listed[token] && available(token)) {
      ...
      uint256 totalSupply_ = IReentalToken(token).totalSupply();
      return cap_ - totalSupply_;
    } else {
      return 0;
    }
  }

  function getTokensFromAmount (
    address token,
    uint256 amount
  )
    public
    ...
    returns (uint256)
  {
    ...
    require(tokensOut <= getOnSale(token), "ReentalCrowdsale: not enough tokens left");
    ...
    onlyRole(_CROWDSALE_ADMIN_ROLE)
  {
    ...
    emit ListedToken(token, price_, listingOn);
  }
