...
contract FW is ERC721, ERC721Enumerable, Pausable, Ownable {
    ...
    uint256 public constant MAX_TOKENS = 8888; // 8888 FW
    uint256 public maxMint = 20; // Max 20 FW per transaction
    uint256 public price = 0.05 ether; // Each FW cost 0.05 ETH to mint
    ...
    constructor(string memory _baseTokenURI, string memory _contractURI) ERC721("FW Items", "FW") {
        setBaseURI(_baseTokenURI);
        setContractURI(_contractURI);

        // The first 50 FW are minted for the team, giveaways and our community
        mint(msg.sender, 50);
    ...
    function preMint(address _to, uint256 _quantity) public payable {
        ...
        require(hasPresaleAccess(_to), "You are not whitelisted for the FW pre-sale");
        require(!mainSale, "You can't pre-mint FW at the moment");
    ...
    function mint(address _to, uint256 _quantity) public payable {
        ...
        if (msg.sender != owner()) {
            require(mainSale, "The FW main sale is not open");
            require(_quantity > 0 && _quantity <= maxMint, "You can only mint 1 to 20 FW");
