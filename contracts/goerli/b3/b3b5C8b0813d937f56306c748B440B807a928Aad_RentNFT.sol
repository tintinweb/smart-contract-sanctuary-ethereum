...
// holds nft inventory marketplace metadata
...
contract RentNFT is IERC721Receiver, AccessControl {
    event NFTListed(
        uint8 nftID,
        address nftAddress,
    ...
    // nft sale metadata of nftinventory
    ...
    function _addUpdatepackage(
        ...
        uint256 discountinbps,
        uint256 packagageID
    ) external onlyRole(PACKAGE_ROLE) {
...
//TODO
    ...
    //             _itemID,
    //             block.timestamp,
    //             allNFT[_itemID].expiresAt
    //         ),
    //         "already booked for time period"
    //     );
    //     require(msg.sender == allNFT[_itemID].owner, "only owner can unlist");

    //     ERC721(allNFT[_itemID].collectionAddress).safeTransferFrom(
    //         address(this),
    //         msg.sender,
    ...
    function rentnftinternal(
        ...
        require(nftdetail.openforrent == true, "not available for rent");
        ...
        addRentedNFT(add, _startAt, _endAt, _itemID);
    ...
    function rentNFTMultiple(NFTRentParams[] memory param) external {
        for (uint256 index = 0; index < param.length; index++) {
            rentnftinternal(
                param[index]._itemID,
                param[index]._startAt,
                param[index]._endAt,
                param[index]._packageID
            );
        }
    }

    function calculateRent(uint8 _itemID, uint256 duration)
        public
        view
        returns (uint256)
    {
        ...
        )
    {
        ...
        uint256 lastID = inventoryinstance.getLastID();

        if (limit > lastID - offset) {
            limit = lastID - offset;
        }

        inventory = inventoryinstance.getByLimit(limit, offset);

        return (inventory, offset + limit, lastID);
    }
        ...
        )
    {
        ...
        inventory = inventoryinstance.getByLimitAndAddress(limit, offset, _owner);
    ...
    function updateDiscountStatus(uint256 _itemID, bool isAllowed) external {
        Inventory memory inv = inventoryinstance.get(_itemID);


        require(inv.owner == msg.sender, "only owner can chnage this");
        inv.discountAllowed = isAllowed;
        inventoryinstance.update(inv, _itemID);
    }

    function getBookingTimePeriodOfNFT(uint256 _itemID)
        public
        view
        returns (NFTRented[] memory)
    {
        NFTRented[] memory rentperiod = new NFTRented[](
            rentedindex[_itemID].length
        );
        for (uint256 i = 0; i < rentedindex[_itemID].length; i++) {
            rentperiod[i] = (rentedindex[_itemID][i]);
        }
        return rentperiod;
    }

    function _addPackageRoleUser(address user) external onlyOwner {
        _setupRole(PACKAGE_ROLE, user);
    }

    function buyListedNFT(uint256 _itemID) external {
        //TODO handle earned rent
        // TODO check if nft is still with the contract
    ...
    function allNFT(uint256 itemID)external view{
                inventoryinstance.get(itemID);
    }
}
 ...
 * ```
 *
 * Roles can be granted and revoked dynamically via the {} and
...
contract wrapped is wrappedabstract{
    ...
    constructor(uint256 _endAt, uint256 _startAt, string memory _name, string memory _symbol, string memory _tokenURI,address owner, uint256 _tokenID ) ERC721(_name, _symbol) {
...
// TODO access control
    ...
    function getByLimit(uint256 limit, uint256 offset)
        external
        view
...
// TODO access control
    function update(Inventory memory inventory, uint256 _itemID) external {
