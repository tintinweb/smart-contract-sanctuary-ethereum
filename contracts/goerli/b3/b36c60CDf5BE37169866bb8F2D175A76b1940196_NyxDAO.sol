...
contract NyxDAO is ReentrancyGuard, Ownable {
    ...
    mapping(ProposalType => mapping(uint256 => Vote[])) public proposalVotes;
        ...
        returns (ProposalConf memory)
    {
        ...
        VoteConf memory voteConf = VoteConf(false, false, 0, 0, block.timestamp + minimumVotingPeriod, voters);
        ...
        returns (ProposalConf memory)
    {
        ...
        proposalVotes[proposalType][proposalId].push(senderVote);
    ...
    function getStakeholderVotes(ProposalType proposalType, address addr)
        public
        view
        onlyStakeholder("User is not a stakeholder")
        returns (Vote[] memory)
    {
        return stakeholderVotes[proposalType][addr];
    }
    ...
    function getProposalVotes(ProposalType proposalType, uint256 proposalId)
        ...
        returns (Vote[] memory)
    {
        Vote[] memory _proposalVotes = proposalVotes[proposalType][proposalId];
