    ...
    Governor
{
    ...
    */
    ...
    function proposeSingleAction(
        ...
        // proposal type bound delay, cancel proposals get executed immediately
        if (proposalType != ProposalType.CancelProposal)
            execDelay += ballotExecDelay[uint256(proposalType) + 1];
...
abstract contract Governor {
        ...
        returns (ProposalState)
    {
        ...
        // require absolute majority for CancelProposal
        if (proposal.proposalType == ProposalType.CancelProposal) {
    ...
    function _executeBallot(uint256 proposalId) internal {
        Proposal memory proposal = _proposals[proposalId];
