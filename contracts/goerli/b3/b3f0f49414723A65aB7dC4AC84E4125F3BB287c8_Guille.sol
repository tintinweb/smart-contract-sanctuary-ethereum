...
contract Guille is KeeperCompatibleInterface, VRFConsumerBase, Digit {
    uint256 public counter;
    //    uint public immutable updateInterval;
    uint256 public immutable interval;
    uint256 public lastTimeStamp;
    bytes32 public lastrequestId;
    bytes32 public lastmagic;
    ...
    batalla[] public jornadas;
    mapping(uint256 => bytes32) public batallaMap;
    event ReturnedCollectible(uint256 requestId, bytes32 randomNumber);
    ...
    function fulfillRandomness(bytes32 requestId, uint256 randomness)
        ...
        override
    {
        lastmagic = requestRandomness(keyHash, fee);
        lastTimeStamp = block.timestamp;
        batallaMap[counter] = lastmagic;
        uint256 playerA = getDigitAtIndex(uint256(lastmagic), 0);
        uint256 playerB = getDigitAtIndex(uint256(lastmagic), 1);
        jornadas.push(
            batalla(
                0xab830B01B6F3F063E6F3739ca12dB920885d0ade,
                0xab830B01B6F3F063E6F3739ca12dB920885d0ade,
                playerA,
                playerB
            )
        );
        //jornadas.push(batalla(player1, player2, valuea, valueb));
        emit ReturnedCollectible(counter, lastmagic);
        // your logic
    }
    ...
    function performUpkeep(bytes calldata performData) external override {
        bytes32 lastrequestId = requestRandomness(keyHash, fee);
...
contract Digit {
    ...
    function getDigitAtIndex(uint256 n, uint256 index)
        public
        pure
        returns (uint256)
    {
