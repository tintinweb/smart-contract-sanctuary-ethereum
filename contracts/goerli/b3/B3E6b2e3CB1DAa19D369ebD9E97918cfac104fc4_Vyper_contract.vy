...
def newton_D(
    ...
    """
    # Safety checks
    assert ANN > MIN_A - 1 and ANN < MAX_A + 1, "unsafe values A"  # dev: unsafe values A
    assert gamma > MIN_GAMMA - 1 and gamma < MAX_GAMMA + 1, 'unsafe values gamma'  # dev: unsafe values gamma
    ...
    ), 'unsafe values x[0]'  # dev: unsafe values x[0]
    for i in range(1, N_COINS):
        frac: uint256 = x[i] * 10**18 / x[0]
        assert frac > 10**11 - 1, 'unsafe values x[i]'  # dev: unsafe values x[i]
    ...
    for i in range(255):
        ...
        if diff * 10**14 < max(
            ...
            for _x in x:
                ...
                assert (frac > 10**16 - 1) and (frac < 10**20 + 1), 'unsafe values x[i]'  # dev: unsafe values x[i]
            return D
    raise "Did not converge"
