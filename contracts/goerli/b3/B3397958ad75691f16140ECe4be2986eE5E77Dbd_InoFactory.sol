...
contract InoFactory is Ownable, Pausable {
    ...
    event InoPoolCreated(address registeredBy, address indexed pool, uint256 poolId);
    ...
    function registerPool(address _signer) external whenNotPaused returns (address pool) {
        pool = address(new InoPool());
        IPool(pool).initialize(_signer);
        getPools[msg.sender].push(pool);
        allPools.push(pool);

        emit InoPoolCreated(msg.sender, pool, allPools.length - 1);
    }
}
...
contract InoPool is Ownable, ReentrancyGuard, Pausable, InoWhitelist {
    ...
    // Sold quantity of each package
    mapping(uint256 => uint256) public packageSold;
    ...
    function buyNftByEtherWithPermission(InoMessage memory message, bytes memory _signature)
        public
        payable
        whenNotPaused
        nonReentrant
    {
        ...
        require(
            packageSold[message.outPackageId] + message.outAmount <= message.outPackageQuantity,
            "POOL::PACKAGE_SOLD_OUT"
        );
        ...
        packageSold[message.outPackageId] += message.outAmount;

        emit TokenPurchaseByEther(
            msg.sender,
            _inAmount,
            message.outToken,
            message.outAmount,
