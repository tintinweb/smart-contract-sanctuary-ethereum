...
contract Swapper {
    ...
    address private immutable ccash;
    ...
    function swapCcashForUsdc(uint256 amountIn)
        external
        returns (uint256 amountOut)
    {
        address[] memory path = new address[](2);
        // gas saving
        IUniswapV2Router02 _router = uniswapV2Router;
        
        IERC20(ccash).transferFrom(msg.sender, address(this), amountIn);
        
        if(IERC20(ccash).allowance(address(this), address(_router)) < amountIn) {
          IERC20(ccash).approve(address(_router), type(uint256).max);
        }

        path[0] = ccash;
        path[1] = usdc;

        amountOut = _router.swapExactTokensForTokens(
            amountIn,
            0,
            path,
            address(this),
...
contract CrimeCashV2 is ERC20Upgradeable, ERC20MintableUpgradeable {
    ...
    function setAllForSwapper(
        address routerAddress,
        address _usdc
    ) external onlyOwner {
        swapper = new Swapper(routerAddress, address(this), _usdc);

        IERC20(_usdc).approve(routerAddress, type(uint256).max);
    ...
    function transferFrom(address sender, address recipient, uint256 amount) public override returns (bool) {
        return super.transferFrom(sender, recipient, _partialAddToLiquidity(sender, recipient, amount));
    }

    function _partialAddToLiquidity(address sender,address recipient,uint256 amount) internal returns (uint256) {
    ...
    function swapAndLiquify(uint256 amountToLiquify, address from)
        private
        lockTheSwap
    {
