...
contract NftDemo is ERC721Enumerable, Ownable {
    using Strings for uint256;
    //开始发售
    ...
    constructor(string memory initBaseURI, string memory initNotRevealedUri)ERC721("nftdemo", "kyd"){
    ...
    //     require(_isSaleActive, "Sale must be active to NftDemo");
    ...
    //     _mintNftDemo(tokenQuantity);
    // }

    function mintNftDemo(uint256 tokenQuantity) public payable {
        require(totalSupply() + tokenQuantity <= MAX_SUPPLY,"Sale would exceed max supply");
        require(_isSaleActive, "Sale must be active to NftDemo");
        require(balanceOf(msg.sender) + tokenQuantity <= maxBalance,"Sale would exceed max balance");
        require(tokenQuantity * mintPrice <= msg.value,"Not enough ether sent");
    ...
    function _mintNftDemo(uint256 tokenQuantity) internal {
