...
contract xALPACA is Initializable, ReentrancyGuardUpgradeable, OwnableUpgradeable {
  ...
  function withdraw() external nonReentrant {
    ...
    if (_grasshouse != address(0)) {
      IGrassHouse(_grasshouse).claim(msg.sender, 0);
    }
  ...
  function earlyWithdraw(uint256 _amount) external nonReentrant {
    ...
    if (grasshouse != address(0)) {
      uint256 _pendingRewards = IGrassHouse(grasshouse).pendingRewardsOf(msg.sender);
      uint256 _rewardPenalty = (earlyWithdrawBpsPerWeek * remainingWeeks * _pendingRewards) / 10000;
      if (_prevLockAmount == _amount) {
        // if withdraw all, we will claim from grass house
        IGrassHouse(grasshouse).claim(msg.sender, _rewardPenalty);
      } else {
        // ow, record user penalty
        IGrassHouse(grasshouse).increasePenaltyOf(msg.sender, _rewardPenalty);
      }
    }
...
interface IGrassHouse {
  ...
  function claim(address _for, uint256 _penalty) external returns (uint256);

  function pendingRewardsOf(address _user) external returns (uint256);

  function increasePenaltyOf(address _user, uint256 _amount) external;
}
