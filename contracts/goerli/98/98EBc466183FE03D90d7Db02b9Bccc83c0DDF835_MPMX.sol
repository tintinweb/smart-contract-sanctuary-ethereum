...
{    
    struct MintPass
    {
        ...
        uint _DefaultMaxUserPurchaseableAmount; // [2] -> _DefaultMaxUserPurchaseableAmount
        uint _MaximumAvailableForSale;          // [3] -> _MaximumAvailableForSale
        uint _StartingBlockUnixTimestamp;       // [4] -> _StartingBlockUnixTimestamp
        uint _SecondsBetweenPriceDecay;         // [5] -> _SecondsBetweenPriceDecay
        bool _AllowMultiplePurchases;           // [6] -> _AllowMultiplePurchases
        bool _ActivePublic;                     // [7] -> _ActivePublic
        bool _ActiveBrightList;                 // [8] -> _ActiveBrightList
        bool _ActiveRespend;                    // [9] -> _ActiveRespend
        bytes32 _Root;                          // [10] -> _Root
    }
    ...
    struct MintPack
    {
        ...
        bool _AllowMultiplePurchases;           // [8] => _AllowMultiplePurchases
        bool _ActiveRespend;                    // [9] => _ActiveRespend
        bytes32 _Root;                          // [10] => _Root
    }
    struct InternalSale
    {
        ...
        uint _UniqueSales;        // [3] -> _UniqueSales
        uint _CurrentIndex;       // [4] -> _CurrentIndex
        uint _MaxIndex;           // [5] -> _MaxIndex
    }
    struct InternalSaleMintPack
    {
        uint _AmountSold;         // [0] -> _AmountSold
        uint _FinalClearingPrice; // [1] -> _FinalClearingPrice
        uint _MintPackIndex;      // [3] -> _MintPackIndex
        uint _MintPackMaxIndex;   // [4] -> _MintPackMaxIndex
    }
    struct UserSaleInformation
    {
        ...
        uint _MintPackCurrentPrice;        // [5] -> _MintPackCurrentPrice
        uint _MintPackAmountPurchased;     // [6] -> _MintPackAmountPurchased
        uint _MintPackAllocation;          // [7] -> _MintPackAllocation
        uint _MintPackAmountRemaining;     // [8] -> _MintPackAmountRemaining
        bool _MintPackEligible;            // [9] -> _MintPackEligible
    }
    struct SaleInformation
    {
        ...
        // Mint Pack Information 
        uint _MintPackCurrentPrice;  // [0] -> _MintPackCurrentPrice
        uint _MintPackRemaining;     // [1] -> _MintPackRemaining
        uint _MintPackSaleStartTime; // [2] -> _MintPackSaleStartTime
        uint _MintPackStartingPrice; // [3] -> _MintPackStartingPrice
        uint _MintPackEndingPrice;   // [4] -> _MintPackEndingPrice
    }
    MintPass public MintPassSale = MintPass(
        ...
        2,          // [2] -> _DefaultMaxUserPurchaseableAmount
        417,        // [3] -> _MaximumAvailableForSale
        1662286500, // [4] -> _StartingBlockUnixTimestamp
        767,        // [5] -> _SecondsBetweenPriceDecay
        true,       // [6] -> _AllowMultiplePurchases
        false,      // [7] -> _ActivePublic
        true,       // [8] -> _ActiveBrightList
        false,      // [9] -> _ActiveRespend
        0x0         // [10] -> _Root
    ); 
    MintPack public MintPackSale = MintPack(
        ...
        // 5 ether,    // [1] -> _PriceEnd
        0,          // [2] -> _DefaultMaxUserPurchaseableAmount
        10,         // [3] -> _MaximumAvailableForSale
        1662286500, // [4] -> _StartingBlockUnixTimestamp
        767,        // [5] -> _SecondsBetweenPriceDecay
        false,      // [6] -> _ActivePublic
        true,       // [7] -> _ActiveBrightList
        true,       // [8] -> _AllowMultiplePurchases
        false,      // [9] -> _ActiveRespend
        0x0         // [10] -> _Root
    );
    ...
    InternalSaleMintPack public MintPackInternalSale = InternalSaleMintPack(
        0,   // [0] -> _AmountSold
        0,   // [1] -> _FinalClearingPrice
        900, // [3] -> _MintPackIndex
        999  // [4] -> _MintPackMaxIndex
    );
    ...
    mapping(address=>uint) private _UserETHCredit;             // `Wallet` => `Total Purchase Value`
    ...
    event Purchased(address Recipient, uint Amount, uint MessageValue, uint AmountSold);
    ...
    event ETHRespent(address Recipient, uint Amount, uint ETHAmount);

    constructor() ERC721MP("Mint Pass Mexico City: MPMX","MPMX") VRFConsumerBase(_vrfCoordinator, _linkToken) 
    { 
        ...
        _WhitelistedSender[msg.sender] = true;
     ...
     * @dev Purchases NFTs
     */
    function PurchaseMintPass(uint Amount, bytes32[] calldata Proof) public payable nonReentrant
    { 
        ...
        if(MintPassSale._ActiveBrightList) { require(VerifyBrightList(msg.sender, Proof), "Merkle: User Is Not On BrightList"); }
        ...
        require(MintPassInternalSale._AmountSold + Amount <= MintPassSale._MaximumAvailableForSale, "MPMX: Overflow");
        require(
        ...
        if(MintPassSale._ActiveRespend && _UserETHCredit[msg.sender] > 0) { __ActiveRespend(CurrentPurchaseValue); }
        else
        { 
            ...
            _UserETHCredit[msg.sender] += CurrentPurchaseValue;
        }
        ...
        {
            IERC721(address(this)).transferFrom(address(this), msg.sender, MintPassInternalSale._CurrentIndex);
            MintPassInternalSale._CurrentIndex++;
        }
        // ICTZN(CTZN).IncrementCTZN(msg.sender, _RewardRates[0] * Amount); // Increments $CTZN Rewards
        emit Purchased(msg.sender, Amount, msg.value, MintPassInternalSale._AmountSold);
    }

    /**
     * @dev Purchases Mint Pass
     */
    function PurchaseMintPack(uint Amount, bytes32[] calldata Proof) public payable nonReentrant
    {
        require(block.timestamp > MintPackSale._StartingBlockUnixTimestamp, "MPMX: Sale Has Not Started");
        require(MintPackSale._ActivePublic || MintPackSale._ActiveBrightList, "MPMX: Sale Not Active");
        if(MintPackSale._ActiveBrightList) { require(VerifyBrightListMintPack(msg.sender, Proof), "Merkle: User Is Not On BrightList"); }
        ...
        for(uint x; x < Amount; x++)
        {
            for(uint y; y < 10; y++)
            {
                IERC721(address(this)).transferFrom(address(this), msg.sender, MintPackInternalSale._MintPackIndex);
        ...
        emit Purchased(msg.sender, Amount, msg.value, MintPackInternalSale._AmountSold);
    }
    ...
    /**
     * @dev Toggles Active Respend
     */
    function __ToggleActiveRespend() external onlyOwner { _ActiveRespend = !_ActiveRespend; }
     ...
     */
    function __NewLiveMintAddress(address NewAddress) external onlyOwner { _WhitelistedSender[NewAddress] = !_WhitelistedSender[NewAddress]; }
    ...
    // function __NewDefaultUserPurchaseableAmount(uint Amount) external onlyOwner { }
    ...
    function ViewBlockInformation() public view returns (
        ...
        if(block.timestamp >= MintPassSale._StartingBlockUnixTimestamp) 
        { 
            _SecondsUntilSale = 0;
            _SecondsAfterSale = block.timestamp - MintPassSale._StartingBlockUnixTimestamp;
        }
        else
        {
            _SecondsAfterSale = 0;
            _SecondsUntilSale = MintPassSale._StartingBlockUnixTimestamp - block.timestamp;
        }
        return(
            block.timestamp, 
    ...
    function ViewAllWalletRefunds() public view returns (address[] memory, uint[] memory)
    {
        address[] memory Addresses = new address[](MintPassInternalSale._UniqueSales);
        uint[] memory Refunds = new uint[](MintPassInternalSale._UniqueSales);
        uint CurrentPrice = ViewCurrentPriceMintPass();
        for(uint OrderIndex; OrderIndex < MintPassInternalSale._UniqueSales; OrderIndex++)
        {
            // if(UniqueSaleIndexToAddress[OrderIndex])
            // {
    ...
    {
        SaleInformation memory Info = SaleInformation(
        ...
        return(MintPassSale, MintPackSale, Info);
    }
    ...
    function ViewWalletSaleInformation(
        ...
        uint _WalletAllocationMintPass = _WalletAllocationsMintPass[Wallet];
        uint _WalletAllocationMintPack = _WalletAllocationsMintPack[Wallet];
        if(_WalletAllocationMintPass == 0) { _WalletAllocationMintPass = MintPassSale._DefaultMaxUserPurchaseableAmount; }
        if(_WalletAllocationMintPack == 0) { _WalletAllocationMintPack = MintPackSale._DefaultMaxUserPurchaseableAmount; }
        UserSaleInformation memory Info = UserSaleInformation(
            ViewCurrentPriceMintPass(),
            _MintPassPurchasedAmt[Wallet],
            _WalletAllocationMintPass, 
            MintPassSale._MaximumAvailableForSale - MintPassInternalSale._AmountSold,
            VerifyBrightList(Wallet, MintPassProof),
            ViewCurrentPriceMintPack(),
            _MintPackPurchasedAmt[Wallet],
            _WalletAllocationMintPack,
            MintPackSale._MaximumAvailableForSale - MintPackInternalSale._AmountSold,
            VerifyBrightListMintPack(Wallet, MintPackProof)
        );
        return(Info);
    }
    ...
    {
        if(block.timestamp <= MintPackSale._StartingBlockUnixTimestamp) { return MintPassSale._PriceStart; }
    ...
    {
        if(block.timestamp <= MintPackSale._StartingBlockUnixTimestamp) { return MintPackSale._PriceStart; }
     ...
     */
    function VerifyBrightList(address Recipient, bytes32[] calldata Proof) internal view returns(bool Status)
    {
     ...
     */
    function VerifyBrightListMintPack(address Recipient, bytes32[] calldata Proof) internal view returns(bool Status)
    {
        bytes32 Leaf = keccak256(abi.encodePacked(Recipient));
    ...
    /**
     * @dev Internal Active Respend
     */
    function __ActiveRespend(uint CurrentPurchaseValue) internal
    {
        uint MinimumETHRequired = CurrentPurchaseValue - _UserETHCredit[msg.sender];
        require(msg.value >= MinimumETHRequired, "MPMX: Invalid Message Value Sent");
        if(_UserETHCredit[msg.sender] >= CurrentPurchaseValue) 
        {
            _UserETHCredit[msg.sender] -= CurrentPurchaseValue;
            if(msg.value > 0) { __Refund(msg.sender, msg.value); }
        }
        else // Insufficient Lifetime Purchase Value To Cover Entire New Purchase
        {
            uint Rebate;
            if(msg.value > _UserETHCredit[msg.sender] && (_UserETHCredit[msg.sender] + CurrentPurchaseValue) > msg.value)
            {
                Rebate = (_UserETHCredit[msg.sender] + CurrentPurchaseValue) - msg.value;
                // Rebate = (msg.value - _UserETHCredit[msg.sender]) - CurrentPurchaseValue;
                __Refund(msg.sender, Rebate);
            }
            else if(_UserETHCredit[msg.sender] > msg.value)
            {
                Rebate = (_UserETHCredit[msg.sender] + msg.value) - CurrentPurchaseValue;
            }
            __Refund(msg.sender, Rebate);
        }
    }
