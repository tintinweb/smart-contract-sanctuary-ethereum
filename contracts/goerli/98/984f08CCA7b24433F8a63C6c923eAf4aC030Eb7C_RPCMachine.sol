...
contract RPCMachine is Ownable, ReentrancyGuard, IERC721Receiver {
    ...
    uint256 public POOL_TOTAL_SUPPLY = 10000;
    ...
    address public saltSiger=0x501008EE101467FD8C6BF4306d902fF69A2Be8f0;
    ...
    event SetPoolTotalSupply(uint256 totalSupply);
    ...
    error RedeemOverFlow(uint256 exceptPoolSize, uint256 totalSupply);



    modifier onlyAdmin() {
        if(!admins[msg.sender] && msg.sender != owner() && msg.sender != address(this)) {
    ...
    function changeNftMapping(uint256[] calldata indexs, uint256[] calldata tokenIds) external onlyOwner {
        for(uint i = 0; i < indexs.length; i++) {
            poolNftMap[indexs[i]] = tokenIds[i];
        }
...
//############################ Liquid Functions Start #####################
    ...
    //     }
    //     if (poolSize + size > POOL_TOTAL_SUPPLY) {
    //         revert RedeemOverFlow(poolSize + size, POOL_TOTAL_SUPPLY);
    //     }
    ...
    // function removePoolForUpdate(uint256[] calldata _targetTokens, uint256[] calldata _targetPoolIndexs, address receiver) external  onlyOwner nonReentrant {
    ...
    //         _pandaNFT.safeTransferFrom(address(this), receiver, _iTargetTokenId, "");
    //         {
    ...
    //     emit RemovePoolForUpdate(receiver, _targetTokens);
    // }


    // function addPoolForSwap(uint256[] calldata _supplyTokens) external nonReentrant {
...
//########################### Pool Utils Functions Start ####################
    ...
    function _currentPrice() public view returns(uint256 _price) {
        if (IERC721(pandaNFT).balanceOf(address(this)) < POOL_TOTAL_SUPPLY) {
            _price = IERC20(pandaToken).balanceOf(address(this)) / (POOL_TOTAL_SUPPLY - poolSize);
        } else {
...
//########################### Pool Utils Functions End ####################
    ...
    function setPoolTotalSupply(uint256 _poolSupply) external onlyOwner {
        POOL_TOTAL_SUPPLY = _poolSupply;
        emit SetPoolTotalSupply(_poolSupply);
    }
