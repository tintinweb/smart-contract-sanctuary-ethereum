...
contract SongCatNFT is ERC721Psi {
    using Strings for uint256;
    uint256 public mintPrice = 0.005 ether; // 5000000000000000
    uint256 public maxSupply = 10;
    address private owner;
    string private baseURI; // 圖片URI
    bool private _isPublicMint = false; // 是否已經公售;
    bool private isReveal = false; // 是否解盲
    bytes32 private merkleRoot;
    
    
    modifier onlyOwner { // 檢查是否為擁有者
        require(msg.sender == owner, "Owner Required");
        _;
    }
    
    modifier isPublicMint { // 檢查是否公售
        require(_isPublicMint, "You can't mint now!");
        _;
    }
    modifier checkPrice(uint256 quantity) {
        require(msg.value == mintPrice * quantity, "Your price value is not match");
        _;
    }
    modifier soldOut(uint256 newSupplyNum) {
        require (maxSupply <= newSupplyNum + totalSupply() , "Sold Out");
        _;
    }


    constructor() ERC721Psi ("SongCatNFT", "SC"){
        owner = msg.sender;
    }
    ...
    // 一般鑄造
    function mint(uint256 quantity) external payable isPublicMint checkPrice(quantity) {
        // _safeMint's second argument now takes in a quantity, not a tokenId. (same as ERC721A)
        _safeMint(msg.sender, quantity);
    }

    // 管理員鑄造
    function ownerMint(address to,  uint256 quantity) external payable onlyOwner checkPrice(quantity) {
        _safeMint(to, quantity);
    }

    // 白單鑄造
    function whitelistMint(bytes32[] memory _proof, uint256 quantity) external payable checkPrice(quantity) {
    ...
    // 設定圖片路徑＋是否解盲盒
    function setBaseURI(string memory _newBaseURI, bool _activeReveal) external onlyOwner {
        baseURI = _newBaseURI;
        isReveal = _activeReveal;
    }
