  ...
  Payable
{
  ...
  event CollectibleMinted(uint256 botId, address owner);
  ...
  mapping(address => bool) private mint_list;
  bool public mint_list_enabled = true;

  bytes32 public merkle_root;
  uint8 mint_list_limit = 5;
  mapping(address => uint8) public mint_list_claimed;
  ...
  function setMintListLimit(uint8 limit) external onlyController {
    mint_list_limit = limit;
  }

  function enableMintList(bool _enabled) external onlyController {
    mint_list_enabled = _enabled;
  }

  function onMintList(address addr, bytes32[] calldata proof) public view returns (bool) {
    return MerkleProof.verify(proof, merkle_root, keccak256((abi.encodePacked(addr))));
  }
  ...
  function mint(uint256 amount, address to) external payable whenNotPaused {
    require(
      curr_batch_max > curr_batch_total,
    ...
    require(!mint_list_enabled, 'Unable to mint; please verify mint list status or try again later.');
    _mint(amount, to);
  }

  function mintFromList(bytes32[] calldata proof, uint256 amount, address to) external payable whenNotPaused {
    require(mint_list_enabled, "Mint list disabled, please use normal mint function.");
    require(this.onMintList(to, proof), 'Unable to mint');
    require(mint_list_claimed[to] < mint_list_limit, "This address has already claimed the maximum # of tokens.");
    _mint(amount, to);
  }
  ...
  function _mint(uint256 amount, address to) internal {
    ...
    factions[tokenId] = curr_batch_faction_id;
    mint_list_claimed[to]++;
  ...
  function setFactionNames(string[6] calldata names) external onlyController {
  ...
  function setFaction(uint256 tokenId, uint8 factionId)
    external
    onlyController
  {
