...
contract TournamentManager is ChainlinkRandomInterval {
	...
	function initialize(IUniswapV2Router02 _router, address _usdcAddress, address _tokenImplementation) external {
	...
	// Calculate winning/losing tokens based on highest uniswap price
	// remove usdc liquidity from losing pool, reward to winning pool
	function resolveMatch(uint8 tokenA, uint8 tokenB) internal returns (uint8 winningId) {
		// cache variables to save gas costs. could store pairs to reduce external calls
		uint256 tokenAPrice;
		uint256 tokenBPrice;
		ITournamentToken winningToken;
		ITournamentToken losingToken;
		IUniswapV2Pair winningPair;
		IUniswapV2Pair losingPair;
		IUniswapV2Pair tokenAPair;
		IUniswapV2Pair tokenBPair;
		...
		// TODO: calculate amountAMin, amountBMin, amountOutMin correctly
		...
		uint256 matchFee;
		if (fee > 0) {
			uint256 losingUsdc = usdc.balanceOf(address(this)) - usdcBalanceBefore;
			uint256 winningUsdc = usdc.balanceOf(address(winningPair));
			matchFee = ((losingUsdc + winningUsdc) * fee ) / BASIS_PRECISION;
			if (matchFee > losingUsdc) {
				matchFee = losingUsdc;
			}
		}

		pendingFees += matchFee; // check-effects-interaction
		...
		router.swapExactTokensForTokens(
			usdc.balanceOf(address(this)) - usdcBalanceBefore - matchFee, 	// amountIn
			0, 													// amountOutMin
			path,  												// path
			address(this), 										// to
			block.timestamp										// deadline
	...
	function addUniswapLiquidity() internal {
		...
		for (uint8 i = 0; i < 8; i++) {
			...
			IERC20(factory.getPair(address(token), address(usdc)))
    ...
    function getSpotPriceAndPair(address token) public view returns (uint256 spotPrice, IUniswapV2Pair) {
    	// could save gas by returning price and pair
    	IUniswapV2Pair pair = IUniswapV2Pair(factory.getPair(token, usdcAddress));
