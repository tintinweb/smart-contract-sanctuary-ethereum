...
contract Nakashi is ERC721URIStorage, RoyaltiesV2Impl,VRFConsumerBaseV2 {
    ...
    uint256 public returnCost = 0.001 ether;

string[] public  packs=["common","uncommon","rare","epic","legendary" ];
string[] private  newPacks;
...
//
  function getPacks() internal {
packs=["common","uncommon","rare","epic","legendary" ];
 for (uint i = 0; i < packs.length; i++) {
    unityDetails[packs[i]];
                if (  unityDetails[packs[i]].maxSupplyOfNFT != unityDetails[packs[i]].itemsMinted) {
 ...
 function clearPack() internal  {

   while (newPacks.length > 0){
       newPacks.pop();
   }
 }
//
    ...
    function mint() public payable {
           getPacks();
          requestRandomWords();
s_randomWords=(s_randomWords% newPacks.length)+1;
