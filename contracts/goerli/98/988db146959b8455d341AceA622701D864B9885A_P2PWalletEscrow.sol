...
contract P2PWalletEscrow is Ownable {
    ...
    event BuyRequestCreated(address indexed tokenAddress, uint buyRequestId);

    event BuyRequestAccepted(address indexed tokenAddress, uint buyRequestId);

    event BuyRequestCancelled(address indexed tokenAddress, uint buyRequestId);
    ...
    struct User {
        mapping(address => uint) withdrawableTokenBalance;
    ...
    /****************************************************************************
     @notice This Method Is Used To Deposit Or Lock User's Tokens In Contract,
     But User Will Be Able To Withdraw That Tokens.
     @param tokenToDeosit, Address Of Token To Deposit. 
     @param amount, Number Of Tokens To Deposit.
    *****************************************************************************/
    function depositTokens(IERC20 tokenToDeposit, uint amount)
        public
        OnlyWhiteListedUser
    {
        ...
        usersData[msgSender].withdrawableTokenBalance[
    ...
    /********************************************************************************************
     @notice This Method Is Used To Withdraw User's Deposited Or Locked Tokens From Contract.
     @param tokenToWithdraw, Address Of Token To Withdraw. 
     @param amount, Number Of Tokens To Withdraw.
    *********************************************************************************************/

    function withdrawTokens(address tokenToWithdraw, uint amount)
        public
        OnlyWhiteListedUser
    {
        address msgSender = msg.sender;
        require(
            usersData[msgSender].withdrawableTokenBalance[tokenToWithdraw] >=
                amount,
            "Insufficeint Withdrawable Token Balance"
        );
        usersData[msgSender].withdrawableTokenBalance[
            tokenToWithdraw
        ] -= amount;
    ...
    /****************************************************************************************************
     ...
     @param tokenToBuy, Address Of That Token Which User Wants To Buy.
    ...
    *****************************************************************************************************/
    ...
    function createBuyRequest(
    ...
    ) public OnlyWhiteListedUser {
        address msgSender = msg.sender;
        require(
            isWhiteListedUser(sellerAddress) == true,
            "Seller Is Not WhiteListed, Seller Must Be WhiteListed"
        );
        require(
            msgSender != sellerAddress,
            "Buyer Address And Seller Address Should Not Be Same"
        );
        require(
            usersData[sellerAddress].withdrawableTokenBalance[tokenToBuy] >=
                amount,
            "Seller Have Insufficient Tokens To Sell, Please Decrease Token Amount"
        );
        ...
        usersData[sellerAddress].buyRequestsReceived.push(newBuyRequestId);
        usersData[sellerAddress].withdrawableTokenBalance[tokenToBuy] -= amount;
        ...
        emit BuyRequestCreated(tokenToBuy, newBuyRequestId);
    }
    ...
    **************************************************************************/
    ...
    function acceptBuyRequest_Batch(uint[] memory buyRequestIds) external {
        uint totalBetIds = buyRequestIds.length;
        for (uint i = 0; i < totalBetIds; ) {
    ...
    function acceptBuyRequest(uint buyRequestId) public {
        require(
        ...
        require(
            tempBuyRequestData.seller == msg.sender,
            "Only Seller Of Buy Request Can Access This Method"
        );
        require(
        ...
        SafeERC20.safeTransfer(
            IERC20(tempBuyRequestData.tokenToBuy),
            tempBuyRequestData.buyer,
            tempBuyRequestData.tokenAmount
        );
        emit BuyRequestAccepted(tempBuyRequestData.tokenToBuy, buyRequestId);
    }
    ...
    ************************************************************************************/
    ...
    function cancelBuyRequest_Batch(uint[] memory buyRequestIds)
        external
        onlyOwner
    {
        uint totalBetIds = buyRequestIds.length;
        for (uint i = 0; i < totalBetIds; ) {
            cancelBuyRequest(buyRequestIds[i]);
            unchecked {
                i++;
            }
        }
    ...
    function cancelBuyRequest(uint buyRequestId) private {
        require(
            buyRequestId > 0 && buyRequestId <= totalBuyRequests,
            "Invalid BuyRequestId, Please Make Sure That You Are Passing Valid BuyRequestId"
        );
        ...
        require(
            isWhiteListedUser(tempBuyRequestData.seller) == true,
            "Seller Is Not WhiteListed, Seller Must Be WhiteListed"
        );
        require(
            tempBuyRequestData.requestStatus == BuyRequestStatus.Requested,
        ...
        usersData[tempBuyRequestData.seller].withdrawableTokenBalance[
                tempBuyRequestData.tokenToBuy
            ] += tempBuyRequestData.tokenAmount;

        emit BuyRequestCancelled(tempBuyRequestData.tokenToBuy, buyRequestId);
    }
    ...
    ********************************************************************************************************/
    ...
    function getWithdrawableTokenBalance(
        address userAddress,
        address tokenAddress
    ) external view returns (uint) {
        return usersData[userAddress].withdrawableTokenBalance[tokenAddress];
    }
