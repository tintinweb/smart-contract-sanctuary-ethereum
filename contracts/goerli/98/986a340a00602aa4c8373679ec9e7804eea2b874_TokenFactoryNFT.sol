...
contract TokenFactoryNFT is TokenFactoryNFTStorage, ITokenFactoryNFT, AccessControlEnumerableUpgradeable {
    ...
    */
    function enableReserveFunction(address _tokenAddress, bool _useReserveFunction) external onlyRole(DEFAULT_ADMIN_ROLE) {
    ...
    * @param _feesCollectorAddress fees collector address where fees will be sent to be collected
    ...
    * @param _waitToTransferOwner, true if whitelisted
    ...
    */
    function deployTokenNFT(string memory _name,
            ...
            uint256 _startDate,
            bool _waitToTransferOwner) public override onlyRole(DEFAULT_ADMIN_ROLE) returns (address) {
        TokenNFT newToken = new TokenNFT();
        newToken.initialize(_name, _symbol, _baseTokenURI, _maxSupply, _creatorAddress,
                _feesCollectorAddress, _biPercent, _creatorPercent, _baseCollectionPrice, _startDate);
    ...
    */
    function deployTokenWL(address _tokenAddress, address _creator) public override onlyRole(DEFAULT_ADMIN_ROLE) returns (address) {
        require(tokenDeployed[_tokenAddress], "Tank index not allowed");
    ...
    */
    function deployTokenWithWL(string memory _name,
            ...
            uint256 _creatorPercent,
            uint256 _baseCollectionPrice,
            uint256 _startDate) external override onlyRole(DEFAULT_ADMIN_ROLE) returns (address, address) {
        address newTokenAddress = deployTokenNFT(_name, _symbol, _baseTokenURI, _maxSupply, _creatorAddress,
                _feesCollectorAddress, _biPercent, _creatorPercent, _baseCollectionPrice, _startDate, true);
...
contract TokenNFTStorage {
    ...
    uint256 public constant minfeesPercentage = 25000;  // min BI percentage = 2,5%
...
contract TokenNFT is TokenNFTStorage, OwnableUpgradeable, ERC721EnumerableUpgradeable, ERC721PausableUpgradeable, ERC2981Upgradeable, ITokenNFT {
     ...
     * @param _feesPercent fees collector's fees percentage
    ...
    function initialize(string memory _name, 
        ...
        require(_feesPercent >= minfeesPercentage, "BI precentage too low");
        require(_creatorPercent <= PERCENT_DIVIDER - _feesPercent, "creator precentage too high"); 
        require(_startDate >= block.timestamp, "Date not allowed"); 
        OwnableUpgradeable.__Ownable_init();
    ...
    modifier creatorOnly() {
        require(creatorAddress == _msgSender(), "Not a creator");
        _;
    }
    ...
    modifier factoryOnly() {
        require(nftFactoryAddress == _msgSender(), "Not an NFT factory");
        _;
    }
    ...
    function setTokenWLAddress(address _tokenWLAddr) external factoryOnly {
        require(_msgSender() == nftFactoryAddress || _msgSender() == owner(), "Not an NFT factory or an owner");
    ...
    */
    ...
    function setCreatorAddress(address _newCreator) external creatorOnly {
        ...
        // OwnableUpgradeable.transferOwnership(_newCreator);
