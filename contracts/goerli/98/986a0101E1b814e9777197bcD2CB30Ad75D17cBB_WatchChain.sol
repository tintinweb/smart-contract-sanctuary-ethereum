    ...
    EIP712("Watch Chain Limit Order", "1")
{
    ...
    address public immutable wcnFeeAddress;
    address public immutable wcnFundAddress;
    ...
    modifier onlyKnownYourCustomer() {
        require(kycInfo[msg.sender].verification, "please verify KYC first");
        _;
    }
    ...
    function checkOrderError(
        ...
        if (kycInfo[order.maker].verification == false) {
    ...
    function setKYCSigner(address _kycSigner) external onlyOwner {
        require(_kycSigner != address(0), "zero address!");
        kycSigner = _kycSigner;
    }
    ...
    function addWatch(
        ...
        require(shares > 0, "price is too small");
        require(
            kycInfo[watchOwner].verification == true ||
                watchOwner == wcnFundAddress,
            "watchOwner did not pass the KYC verification"
        );
        startTime = (startTime > block.timestamp ? startTime : block.timestamp);
    ...
    function balanceOfNotZero(
        ...
        for (uint256 i = startId; i < endId; ++i) {
            ...
            if (balance > 0 || auction > 0) {
                ...
                notZeroWatches[count].status = watchStatus(i);
                count++;
            }
        }
        assembly {
