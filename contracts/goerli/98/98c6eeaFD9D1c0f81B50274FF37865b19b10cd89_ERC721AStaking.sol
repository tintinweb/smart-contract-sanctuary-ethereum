...
contract ERC721AStaking is ReentrancyGuard {
    ...
    function withdraw(uint256 _tokenId) external nonReentrant {
        ...
        uint256 rewards = calculateRewards(msg.sender);
        stakers[msg.sender].unclaimedRewards += rewards;
        uint256[] storage array=stakedTokens[msg.sender];
        ...
        for (uint256 i = 0; i < array.length; i++) {
             ...
             if(array[i]==_tokenId){
                    require(i < array.length, "index out of bound");

                    for (uint myi = i; myi < array.length - 1; myi++) {
                    array[i] = array[i + 1];
                    }
                    array.pop();
