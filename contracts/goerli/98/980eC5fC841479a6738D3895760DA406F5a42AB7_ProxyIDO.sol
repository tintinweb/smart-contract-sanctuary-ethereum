...
contract ProxyIDO is Ownable {
    ...
    event debug1(
        ...
        address owner,
        address admin,
        IDO.projectInfo data
    );
    
    /*
    event donationCreateLog(
        string name,
        address msg,
        address owner,
        address admin,
        Donation.projectDonation data
    );
    */
    ...
    function createIDO(IDO.projectInfo memory data) external {
        ...
        uint256 timeValue = data.timeList[0];
        require(data.endTime > timeValue, "first unlock time  error");

        //检索上次创建时间
        uint256 timeVlaue = userTime[msg.sender];
        require(block.timestamp > timeVlaue, "Not completed last time");
        ...
        IDO contractAddr = new IDO(data, msg.sender, adminAddr, adminPoint);
    ...
    /*
    function createDonation(Donation.projectDonation memory data) external {
        require(proxyStatus == true, "proxy lock");
        require(data.startTime < data.endTime, "time range error");
        ...
        Donation contractAddr = new Donation(
            data,
            msg.sender,
            adminAddr,
            adminPoint
    ...
    function createDonationTest() external {
        Donation.projectDonation memory data;
        Donation contractAddr = new Donation(
            data,
            msg.sender,
            adminAddr,
...
contract IDO is Ownable {
    ...
    struct projectInfo {
        ...
        uint256 inTokenCapacity; // 项目方充入 tokenB 的总额度
        ...
        uint256 decimalB; // TokenOut 精度 需要放大 1000 倍
    ...
    mapping(uint256 => lockAmount) private itemMapping; //数据查询结构ID
    ...
    function initIdoTest() internal {
        //标记项目已经可以运行
        emit DebugMsg("initIdoTest", address(msg.sender), "", 0);
        statusRun = false;
    }
    ...
    function swap(uint256 _amountIn) public {
        ...
        require(
            Info.inTokenCapacity - amountIn >= 0,
        ...
        Info.outTokenCapacity = Info.inTokenCapacity - amountIn;
        ...
        uint256 amountOut = amountIn / Info.lockNum;
