...
contract DAPP is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    DAPPDividendTracker public dividendTracker;
    ...
    constructor() ERC20("DAPP", "DAPPCASH") {
        marketingWallet = payable(0xEcf49fBc53342c117b697B1C0ecf2E7A0D202723);
        devWallet = payable(0xEcf49fBc53342c117b697B1C0ecf2E7A0D202723);
        ...
        dividendTracker = new DAPPDividendTracker(
            payable(this),
            router,
...
contract DAPPDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor(
        ...
        defaultToken = token;
        DAPPContract = DAPP(mainContract);
    ...
    function _transfer(
        ...
        require(false, "DAPP_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "DAPP_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main DAPP contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(DAPPContract);

            uint256 prevBalance = DAPPContract.balanceOf(address(this));
            ...
            {
                uint256 received = DAPPContract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    DAPPContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
