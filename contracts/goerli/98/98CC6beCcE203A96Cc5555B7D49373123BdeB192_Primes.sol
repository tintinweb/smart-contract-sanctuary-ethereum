...
pragma solidity ^0.8.0;
// @author https://twitter.com/_g56d_
...
contract Primes is ERC721r, Ownable {
    ...
    uint256 private constant MAX_SUPPLY = 1576;
    uint256 public gifts = 36;
    uint256 public maxPerAddress = 5;
    uint256 public price = 0.001 ether;
    uint256 public timestampStart = 1672550283;
    ...
    modifier isUser() {
        require(
            ...
            "Primes: can't be called by another contract"
        );
        _;
    }

    function mint(uint256 _quantity) external payable isUser {
        require(!paused, "Primes: minting is paused");
        require(
            currentTime() >= timestampStart,
            "Primes: sale has not started yet"
        );
        require(_quantity > 0, "Primes: mint at least 1 NFT");
        require(
            addressMintedBalance[msg.sender] <= maxPerAddress,
            "Primes: can't mint that quantity"
        );

        uint256 supply = totalSupply();
        require(
            supply + _quantity <= MAX_SUPPLY - gifts,
            "Primes: not enough PRIMES to mint"
        );

        if (msg.sender != owner()) {
            uint256 mintedCount = addressMintedBalance[msg.sender];
            require(
                mintedCount + _quantity <= maxPerAddress,
                "Primes: maximum per address exceeded"
            );
            require(
                msg.value >= price * _quantity,
                "Primes: insufficient funds"
            );
        }
    ...
    function tokenURI(uint256 _tokenId)
        ...
        returns (string memory)
    {
        require(_exists(_tokenId), "Primes: non-extistent token");

        string memory serial = _tokenId.toString();
        string memory dataImage = generateBase64SVG(_tokenId);
        string
            memory description = "Primes is the first collection of on-chain NFTs of scalable, animated and interactive vector graphics by g56d";
        // string[] memory stats;
                    ...
                    Base64.encode(
                        bytes(
                            abi.encodePacked(
                                ...
                                dataImage,
                                '", "attributes": [',
                                '{"trait_type":"Prime numbers", "value":""},',
        ...
        returns (string memory)
    {
        ...
        return
            string(
                abi.encodePacked(
                    '<svg xmlns="http://www.w3.org/2000/svg" version="1.2" viewBox="0 0 900 900" width="900px" height="900px" shape-rendering="geometricPrecision">',
                    title,
                    "<desc>Primes is the first collection of on-chain NFTs of scalable, animated and interactive vector graphics by g56d</desc>",
    ...
    // function generatePaths(uint256 _tokenId)
    function generatePaths(uint256 _tokenId)
        internal
        view
        returns (string memory)
    {
        uint256 index = _tokenId * 81 - 80;
        uint256 x = 0;
        uint256 y = 0;
                ...
                if (isPrime(uint256(index), 2)) {
                    ...
                    anime = string(
                        abi.encodePacked(
                            ...
                            generateAnimationValues(x, y),
                            // string(values),
                            '" dur="',
                            index.toString(),
                            'ms" repeatCount="indefinite" end="click"/>'
                        )
                    );
                    paths = strConcat(paths, anime);
                    paths = strConcat(paths, "</path>");
                }
                index += 1;
            }
        }
    ...
    function generateAnimationValues(uint256 _x, uint256 _y)
        internal
        pure
        returns (string memory)
    {
        ...
        return string(abi.encodePacked(a, b, a));
        ...
        */
        ...
        string memory values = strConcat(a, b);

        return strConcat(values, a);
        */

        // test ok
        // return string(abi.encodePacked(_x.toString(), _y.toString()));
        //
        string memory a = string(
            abi.encodePacked(
                "M",
        ...
        b = string(abi.encodePacked(b, a));
        return b;
    }

    // from /AvastarBase.sol
    /**
    ...
    // primality test
    function isPrime(uint256 _n, uint256 _o) internal pure returns (bool) {
        if (_n == 2 || _n == 3) {
            return true;
        }
        ...
        uint256[2] memory values = getValues(_n);
        uint256 s = values[0];
        uint256 d = values[1];

        uint256 x = fastModularExponentiation(_o, d, _n);
        ...
        for (uint256 i = s - 1; i > 0; i--) {
            x = fastModularExponentiation(x, 2, _n);
            if (x == 1) {
                return false;
            }
            if (x == _n - 1) {
    ...
    function getValues(uint256 _n) internal pure returns (uint256[2] memory) {
        uint256 s = 0;
        uint256 d = _n - 1;
        while (d % 2 == 0) {
            d = d / 2;
    ...
    function setTimestampStart(uint256 _timestampStart) public onlyOwner {
    ...
    function sendGift(address _address, uint256 _quantity) public onlyOwner {
    ...
    function withdraw() public payable onlyOwner {
        ...
        require(success, "Primes: transfer failed.");
    }
