...
contract CollateralAggregator is BaseAggregator {
    ...
    uint private _depositGas = 200000;
    uint private _withdrawGas = 200000;

    event UpdateCollateral(uint16 indexed packetType, uint16 indexed srcChainId, address indexed account, uint amount, bytes32 collateralKey);
    ...
    function _updateCollateral(
        ...
        (, bytes memory accountBytes, uint amount, bytes32 collateralKey) = abi.decode(_payload, (uint16, bytes, uint, bytes32));
    ...
    function sendDeposit(
        ...
        _collateralByIssuerAggregation[collateralKey][_account] += amount;

        bytes memory lzPayload = abi.encode(PT_DEPOSIT, abi.encodePacked(_account), amount, collateralKey);

        _broadcast(lzPayload, PT_DEPOSIT);
    }
    ...
    /** Gas for LZ fee */
    function depositGas() external view returns (uint) {
        return _depositGas;
    }

    function withdrawGas() external view returns (uint) {
        return _withdrawGas;
    }

    function _lzAdapterParam(uint16 packetType) internal view override returns (bytes memory) {
        uint16 version = 1;
        if (packetType == PT_DEPOSIT) {
            return abi.encodePacked(version, _depositGas);
        }

        return abi.encodePacked(version, _withdrawGas);
    }

    function setDepositGas(uint newGas) external onlyOwner {
        _depositGas = newGas;
    }

    function setWithdrawGas(uint newGas) external onlyOwner {
        _withdrawGas = newGas;
    }
}
