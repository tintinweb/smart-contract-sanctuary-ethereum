    ...
    PaymentSplitter
{
    ...
    uint256 public constant MAX_PRESALE_SUPPLY = 3000;

    uint256 public constant MAX_TOKENS_PER_ADDRESS_PRESALE = 3;
    uint256 public constant MAX_TOKENS_PER_ADDRESS_SALE = 2;
        ...
        isCorrectPayment(PRESALE_PRICE, _quantity)
    {
        ...
        _updateNumberPresaleMinted(_to, _quantity);
    }
    ...
    function mint(address _to, uint256 _quantity)
        ...
        canOwnerMintQuantitySale(_to, _quantity)
        isCorrectPayment(SALE_PRICE, _quantity)
    {
        _safeMint(_to, _quantity);
    }
    ...
    function startSale() public onlyOwner duringPresale {
        stage = Stage.Sale;
        emit SaleStarted();
    }
    ...
    /*******************************************************
     ******* ERC721A AddressData.aux - presale slots *******
     *******************************************************/

    function getNumberPresaleMinted(address _owner) public view returns (uint64) {
        return _getAux(_owner);
    }

    function _updateNumberPresaleMinted(address _owner, uint256 _quantity) internal {
        uint64 updated = getNumberPresaleMinted(_owner) + uint64(_quantity);
        _setAux(_owner, updated);
    }
    ...
    modifier duringPublicSale() {
        require(stage == Stage.Sale, "Only after presale");
        _;
    }
    ...
    modifier canOwnerMintQuantityPresale(
        ...
        require(
            balanceOf(_owner) + _quantity <= MAX_TOKENS_PER_ADDRESS_PRESALE,
            "Maximum tokens per address"
        );
        _;
    }

    modifier canOwnerMintQuantitySale(
        address _owner,
        uint256 _quantity
    ) {
        require(
            balanceOf(_owner) - getNumberPresaleMinted(_owner) + _quantity <= MAX_TOKENS_PER_ADDRESS_SALE,
            "Maximum tokens per address"
        );
        _;
    }
