...
contract PrimaryIndexTokenAtomicRepayment is Initializable, AccessControlUpgradeable, ReentrancyGuardUpgradeable {
    ...
    event CalcDepositPositionWhenAtomicRepay(address usser, uint amountSold, uint amountRecive, uint collateralBalanceLeft, uint amountBackToUser, uint amountRepay);
    ...
    function callApproveByAugutus(address token, address spender, uint amount) public {
        IParaSwapAugustus(augustusParaswap).approveToken(token, spender, amount);
    }
   ...
   function calcDepositPositionWhenAtomicRepay(address prjToken, uint collateralAmount, bytes memory buyCalldata) public returns(uint collateralBalanceLeft, uint amountBackToUser, uint amountRepay) {
       address lendingToken = getLendingToken(msg.sender, prjToken);
        ...
        require(collateralAmount <= remainingDeposit, "PIT: invlid amount");
        ...
        ERC20Upgradeable(prjToken).safeApprove(tokenTransferProxy, collateralAmount);
        ...
        uint256 collateralLeft = collateralAmount - amountSold;
        collateralBalanceLeft = collateralLeft > 0 ? collateralLeft : 0;
        amountBackToUser = amountRecive > totalOutStanding ? amountRecive - totalOutStanding : 0;
        amountRepay = amountRecive > totalOutStanding ? totalOutStanding : amountRecive;
        emit CalcDepositPositionWhenAtomicRepay(msg.sender, amountSold, amountRecive, collateralBalanceLeft, amountBackToUser, amountRepay);
   }

   function depositBack(address prjToken, uint collateralBalanceLeft) public {
   ...
   function transfer(address lendingToken, uint amountRecive, uint totalOutStanding) public {
        if(amountRecive > totalOutStanding) {
            uint amountBackToUser = amountRecive - totalOutStanding;
            ERC20Upgradeable(lendingToken).safeTransfer(msg.sender, amountBackToUser);
        }
   }

   function repay(address prjToken, address lendingToken, uint amountRecive) public {
