...
import "./DigitalTwinStorage.sol";
import "./IDigitalTwin.sol";

import "@openzeppelin/contracts/utils/Strings.sol";

error TokenAlreadyRedeemed(uint256 tokenId);
error TokenNotRedeemed(uint256 tokenId);
    ...
    DigitalTwinStorage
{
    ...
    event BaseURIChanged(string baseTokenURI_);
    event ContractURIChanged(string contractURI_);
    event MintValueChanged(uint256 mintValue);
    event TokenRedeemed(uint256 tokenId);
    event TokenUnredeemed(uint256 tokenId);
    ...
    function mint(address to) external payable returns (uint256 tokenId) {
        require(
            msg.value == _mintValue,
            string.concat("DigitalTwin: mint: value must be ", Strings.toString(_mintValue))
        );
    ...
    function burn(uint256 tokenId) external override onlyOperator whenBurnEnabled {
        _burn(tokenId);
    }

    function redeem(uint256 tokenId) external onlyOperator {
        if (_redeemedTokens[tokenId]) revert TokenAlreadyRedeemed(tokenId);
        _redeemedTokens[tokenId] = true;
        emit TokenRedeemed(tokenId);
    }

    function unredeem(uint256 tokenId) external onlyOperator {
        if (!_redeemedTokens[tokenId]) revert TokenNotRedeemed(tokenId);
        _redeemedTokens[tokenId] = false;
        emit TokenUnredeemed(tokenId);
    }

    function setBaseTokenURI(string calldata baseTokenURI_) external onlyOperator {
        _baseTokenURI = baseTokenURI_;
        emit BaseURIChanged(baseTokenURI_);
    }

    function setContractURI(string calldata contractURI_) external onlyOperator {
        _contractURI = contractURI_;
        emit ContractURIChanged(contractURI_);
    }

    function setMintValue(uint256 mintValue) external onlyOperator {
        _mintValue = mintValue;
        emit MintValueChanged(mintValue);
    }
    ...
    function getMintValue() external view returns (uint256 mintValue) {
        return _mintValue;
    }

    // The following functions are overrides defined for this contract.
    ...
    function tokenURI(uint256 tokenId) public view virtual override(ERC721Upgradeable) returns (string memory) {
        ...
        string memory redeemed = _redeemedTokens[tokenId] ? "/redeemed" : "/unredeemed";
        string memory uri = string(abi.encodePacked(baseURI, Strings.toString(tokenId), redeemed));
        return uri;
    }
...
abstract contract DigitalTwinStorage {
    CountersUpgradeable.Counter internal _tokenIdCounter;

    string internal _baseTokenURI;
    string internal _contractURI;
    uint256 internal _maxTokenSupply;
    uint256 internal _mintValue;
    bool internal _burnEnabled;

    mapping(uint256 => bool) internal _redeemedTokens;
}
...
interface IDigitalTwin is IERC721EnumerableUpgradeable {
    function mint(address to) external payable returns (uint256 tokenId);
    ...
    function setMintValue(uint256 mintValue) external;
