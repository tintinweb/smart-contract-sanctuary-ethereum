...
contract Staker {
  ...
  modifier isOwner(address _addr) {
    require(owner == _addr, "You're not owner");
    _;
  }

  modifier isStaking() {
    require(timeLeft() == 0, "The staking it's still running");
    _;
  }

  modifier isCompleted() {
    require(exampleExternalContract.completed() == false, 'The stake was completed');
    _;
  }

  modifier isOpenForWithdraw() {
    require(openForWithdraw == true, "Withdraw it's not available");
    _;
  }

  // TODO: Collect funds in a payable `stake()` function and track individual `balances` with a mapping:
  //  ( make sure to add a `Stake(address,uint256)` event and emit it for the frontend <List/> display )
  function stake() external payable isCompleted {
  ...
  function execute() public isStaking isCompleted {
  ...
  function withdraw() public isOpenForWithdraw {
