...
contract HarvestableVaultMock is ERC4626, PricePerTokenMixin {
    ...
    constructor(
        ...
        rewardToken.safeIncreaseAllowance(address(assetConverter), type(uint256).max);
    }
        ...
        returns (uint256 shares)
    {
        return (totalAssets() == 0) ?
            (assets * (10 ** decimals()) / (10 ** IERC20Metadata(asset()).decimals()))
            :
            (totalSupply() * assets / totalAssets());
    }
        ...
        returns (uint256 assets)
    {
        return (totalSupply() == 0) ?
            (assets * (10 ** IERC20Metadata(asset()).decimals()) / (10 ** decimals()))
