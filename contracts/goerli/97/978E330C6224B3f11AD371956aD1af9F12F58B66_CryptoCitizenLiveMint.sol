...
{   
    ...
    { 
        ...
        ArtistMintPasses[_CurrentCityIndex][13] = 0xc440F0B749B6CE1D72cEf4eDD61921c9B489B70B; 
        ArtistMintPasses[_CurrentCityIndex][14] = 0xc440F0B749B6CE1D72cEf4eDD61921c9B489B70B; 
        ArtistMintPasses[_CurrentCityIndex][15] = 0xc440F0B749B6CE1D72cEf4eDD61921c9B489B70B; 
        ArtistMintPasses[_CurrentCityIndex][16] = 0xc440F0B749B6CE1D72cEf4eDD61921c9B489B70B; 
        ArtistMintPasses[_CurrentCityIndex][17] = 0xc440F0B749B6CE1D72cEf4eDD61921c9B489B70B; 
        ArtistMintPasses[_CurrentCityIndex][18] = 0xc440F0B749B6CE1D72cEf4eDD61921c9B489B70B; 
    ...
    function RedeemGT(
        ...
        for(uint x; x < TicketIDs.length; x++)
        {
            ...
            ICTZN(CTZN).IncrementCTZN(msg.sender, _RewardRates[4]); 
    ...
    function RedeemArtistPass(
        ...
        for(uint x; x < ArtistIDs.length; x++)
        {
            ...
            ICTZN(CTZN).IncrementCTZN(msg.sender, _RewardRates[5]);
     ...
     */
    function RedeemQR(uint[] calldata TimeSlots, bytes32[] calldata Proof) external nonReentrant 
    {
        ...
        if(!(QRAllocation[_CurrentCityIndex][msg.sender] > 0)) // User Is Able To Redeem Explicitly 1 QR Code
        {
            ...
            _QRIndex++;
            SelectedTimeSlotGoldenTicket[_CurrentCityIndex][_QRIndex] = TimeSlots[0];
            BrightListCitizen[_CurrentCityIndex][_QRIndex] = msg.sender;
            ICTZN(CTZN).IncrementCTZN(msg.sender, _RewardRates[7]);
            emit QRRedeemed(msg.sender, _QRIndex, TimeSlots[0]);
        }
        else // User Is Able To Redeem More Than 1 QR Code Because Their Integer Allocation > 0
        {
            ...
            for(uint x; x < Allocation; x++)
            {
                ...
                SelectedTimeSlotGoldenTicket[_CurrentCityIndex][_QRIndex] = TimeSlots[x];
                BrightListCitizen[_CurrentCityIndex][_QRIndex] = msg.sender;
                ICTZN(CTZN).IncrementCTZN(msg.sender, _RewardRates[7]);
                emit QRRedeemed(msg.sender, _QRIndex, TimeSlots[x]);
            }
        }
    }
     ...
     */
    function _LiveMintArtist(uint ArtistID, uint TicketID) external onlyMinter  
    {
    ...
    function _LiveMintArtistBatch(uint[] calldata ArtistIDs, uint[] calldata TicketIDs) external onlyMinter 
    {
        for(uint ArtistID; ArtistID < ArtistIDs.length; ArtistID++)
        {
            for(uint TicketID; TicketID < TicketIDs.length; TicketID++)
            {
                address Recipient = readBrightListArtist(ArtistIDs[ArtistID], TicketIDs[TicketID]);
                require(!MintedArtist[_CurrentCityIndex][ArtistIDs[ArtistID]][TicketIDs[TicketID]], "LiveMint: Artist Mint Pass Already Minted");
                require(Recipient != address(0), "LiveMint: Mint Pass Entered Is Not Brightlisted");
                MintedArtist[_CurrentCityIndex][ArtistIDs[ArtistID]][TicketIDs[TicketID]] = true;
                address ArtistMintingContractAddress = resolveArtistContract(ArtistIDs[ArtistID]);
                bool ArtBlocksMint = resolveArtistMint(ArtistIDs[ArtistID]);
                uint TokenID = 69420;
                // if(ArtBlocksMint) { TokenID = IArtBlocks(ArtistMintingContractAddress).purchaseTo(Recipient, ArtistIDs[ArtistID]); }
                // else { TokenID = IRandomEdition(ArtistMintingContractAddress).purchaseTo(Recipient); }
                if(!DelegateStatusArtist[_CurrentCityIndex][ArtistIDs[ArtistID]][TicketIDs[TicketID]]) { ICTZN(CTZN).IncrementCTZN(Recipient, _RewardRates[2]); } 
                else { ICTZN(CTZN).IncrementCTZN(Recipient, _RewardRates[3]); }
                emit LiveMintCompleteArtist(
                    Recipient, ArtistIDs[ArtistID], 
                    TicketIDs[TicketID], 
                    TokenID, 
    ...
    {
        if(!resolveArtistMint(ArtistID)) { return IRandomEdition(resolveArtistContract(ArtistID)).tokenURI(TokenID); }
        else if(resolveArtistMint(ArtistID)) { return IArtBlocks(resolveArtistContract(ArtistID)).tokenURI(TokenID); }
        else { return "LiveMint: Artist NFT Corresponding To `TicketID` Not Minted"; }
    }

    /**
     * @dev Returns Citizen Metadata
     */
    function readMetadataCitizen(uint TokenID, address ContractAddress) public view returns (string memory)
    {
        return IArtBlocks(ContractAddress).tokenURI(TokenID);
    }
    ...
    function readUnmintedGoldenTokens() public view returns(uint[] memory)
    {
        ...
        for(uint x; x < 1000; x++)
        {
            if(!MintedCitizen[_CurrentCityIndex][x]) { MintedTokenIDs[Counter] = x; }
        }
    ...
    {
        uint[][] memory Minted = new uint[][](ArtistIDs.length);
        uint Index;
        for(uint ArtistID; ArtistID < ArtistIDs.length; ArtistID++)
        {
            uint[] memory MintedArtistIDs = new uint[](Range);
            uint Counter;
            for(uint TokenID; TokenID < Range; TokenID++)
            {
                if(!MintedArtist[_CurrentCityIndex][ArtistID][TokenID]) 
                { 
                    MintedArtistIDs[Counter] = TokenID; 
                    Counter++;
                }
            }
            Minted[Index] = MintedArtistIDs;
            Index++;
        }
        return Minted;
    }
...
{
    address public immutable CTZN = 0x874795a3077aBb71aA88E328C5fd592EaeC310aE;
    event RewardRatesChanged(uint[] RewardIndexes, uint[] RewardRates);
    mapping(uint=>uint) public _RewardRates;

    /**
     * @dev Modifies The Reward Rate
     */
    ...
    {   
        ...
        for(uint x; x < RewardRates.length; x++)
        {
            _RewardRates[RewardIndexes[x]] = RewardRates[x];
        }
...
interface ICTZN 
{ 
    function IncrementCTZN(address Recipient, uint Amount) external;
}
