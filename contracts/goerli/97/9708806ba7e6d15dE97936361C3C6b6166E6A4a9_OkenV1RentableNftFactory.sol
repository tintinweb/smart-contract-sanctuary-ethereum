...
/// @title Rentable NFT Factory contract to deploy `OkenV1RentableNft` contracts
contract OkenV1RentableNftFactory is Ownable, IOkenV1RentableNftFactory {
    ...
    /// @notice Fee to deploy a `OkenV1RentableNft` contract
    uint256 private _platformFee;

    /// @notice Address the fees are transferred to
    address payable _feeRecipient;

    /// @notice `OkenV1RentableNft` address -> exists
    mapping(address => bool) private _exists;

    /// @notice ERC165 interface id of ERC4907
    ...
    function deployRentableNftContract(
        string memory name_,
        ...
        emit RentableNftAdded(_msgSender(), address(nft));
        return address(nft);
    }

    function addRentableNftContract(address rentableNft) external override onlyOwner {
        // check if already added
        if (_exists[rentableNft]) revert ContractAlreadyExists(rentableNft);

        // check if contract is ERC4907
        if (!IERC165(rentableNft).supportsInterface(INTERFACE_ID_ERC4907)) {
            revert InvalidNftAddress(rentableNft);
        }

        // update exists
        _exists[rentableNft] = true;
        emit RentableNftAdded(_msgSender(), rentableNft);
    }

    function removeRentableNftContract(address rentableNft) external override onlyOwner {
        // check contract has been added
        if (!_exists[rentableNft]) revert ContractNotExists(rentableNft);

        // update exists
        _exists[rentableNft] = false;
        emit RentableNftRemoved(_msgSender(), rentableNft);
    }

    //--------------------------------- accessors

    function getExists(address rentableNft) external view override returns (bool) {
        return _exists[rentableNft];
    }
...
contract OkenV1RentableNft is ERC4907URIStorage, IOkenV1RentableNft, Ownable {
    ...
    function mintRentableNft(address to, string calldata uri) external payable override {
        // check if fees are paid
        if (msg.value < _platformFee) {
            revert InsufficientFunds(_platformFee, msg.value);
        }
        ...
        emit RentableNftMinted(tokenId, to, _msgSender());
    }

    function burnRentableNft(uint256 tokenId) external override {
        ...
        // burn token
        _burn(tokenId);
        emit RentableNftBurned(tokenId, operator);
    }
...
interface IOkenV1RentableNftFactory {
    ...
    /// @dev Emitted when a `OkenV1RentableNft` contract is deployed or added
    event RentableNftAdded(address indexed operator, address indexed rentableNft);

    /// @dev Emitted when a `OkenV1RentableNft` contract is removed
    event RentableNftRemoved(address indexed operator, address indexed rentableNft);
    ...
    /// Emits a `RentableNftAdded` event
    ...
    /// @return Address of the `OkenV1OkenNft` contract deployed
    function deployRentableNftContract(
        string memory name_,
        string memory symbol_,
        uint256 mintFee,
    ...
    /// @dev Reverts if the contract is not ERC4907 or has already been added
    /// Emits a `RentableNftAdded` event
    /// @param rentableNft Address of the ERC4907 contract
    function addRentableNftContract(address rentableNft) external;

    /// @notice Removes a contract from the factory
    /// @dev Reverts if the contract has not already been added
    /// Emits a `RentableNftRemoved` event
    /// @param rentableNft Address of the ERC4907 contract
    function removeRentableNftContract(address rentableNft) external;

    //--------------------------------- accessors

    function getExists(address rentableNft) external view returns (bool);
...
interface IOkenV1RentableNft {
    ...
    event RentableNftMinted(uint256 indexed tokenId, address indexed owner, address indexed minter);

    /// @notice Emitted when a NFT is burned
    event RentableNftBurned(uint256 indexed tokenId, address indexed operator);
    ...
    function mintRentableNft(address to, string calldata uri) external payable;
...
/// @notice Contract storing the deployment addresses of Oken V1 contracts
interface IOkenV1AddressRegistry {
    /// @notice Address of `OkenV1TokenRegistry`
    function tokenRegistry() external view returns (address);

    /// @notice Modifiy the deployment address of `OkenV1TokenRegistry`
