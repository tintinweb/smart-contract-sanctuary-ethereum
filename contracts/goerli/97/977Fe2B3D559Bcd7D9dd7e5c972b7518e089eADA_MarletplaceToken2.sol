...
contract MarletplaceToken2 is ERC1155, Ownable, Pausable, ERC1155Supply, ERC1155Holder {
    ...
    event Minted (uint indexed tokenId,string indexed tokenUri, uint quantity);
    ...
    /***
        @dev An external function to create a new Character.
        @param newCharacter to give a name to the new token/character created associated to an ID
        @param initialSUpply for a new character  
    */
    function mint(string memory tokenURI,uint256 initialSupply, address account) public returns(uint256)  {
        tokenCounter++;
        uint newTokenId = tokenCounter;
        _mint(account, newTokenId, initialSupply, "");
        _setTokenUri(newTokenId, tokenURI);
        emit Minted(newTokenId, tokenURI, initialSupply);
        return newTokenId;
    }
    ...
    function _setTokenUri(uint256 tokenId, string memory newTokenUri) public {
