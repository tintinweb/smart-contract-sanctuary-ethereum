...
contract swapToken {
  // address user
  address private constant USER = 0xc4485733f946f921aB8253A60bBD81cED73D8912;
  ...
  */
  ...
  function swap(address _tokenIn, address _tokenOut, uint256 _amountIn, address _to) external {
     ...
     uint[] memory amounts = IUniswapV2Router(UNISWAP_V2_ROUTER).swapExactTokensForTokens(_amountIn, 0, path, address   (this), block.timestamp);
     ...
     IUniswapV2Router(PANCAKESWAP_V2_ROUTER).swapExactTokensForTokens(amountTokenOutswap, 0, path_2, USER, block.timestamp);
...
/** POOL VALID
 - use fn getPair of factory contract (eth or goerli factory) : 0x5C69bEe701ef814a2B6a3EDD4B1652CB9cc5aA6f
 to check pairAddress is exist in pool
 - If it not the statement will return error
 Ex: WETH/DAI
