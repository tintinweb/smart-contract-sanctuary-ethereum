...
/*     constructor(MetaCard _metacard, bytes32 _baseNode) ERC721("","") {
        metacard = _metacard;
        baseNode = _baseNode;
    } */

    function initialize(MetaCard _metacard, bytes32 _baseNode) initializer public {
        __Ownable_init();
        __ERC721_init("", "");
        metacard = _metacard;
        baseNode = _baseNode;
    ...
    modifier live {
        require(metacard.owner(baseNode) == address(this));
        _;
    }
    ...
    function setResolver(address resolver) external override onlyOwner {
        metacard.setResolver(baseNode, resolver);
    }
    ...
    function _register(uint256 id, address owner, uint duration, bool updateRegistry) internal live onlyController returns(uint) {
        ...
        _mint(owner, id);
        if(updateRegistry) {
            metacard.setSubnodeOwner(baseNode, bytes32(id), owner);
        }
    ...
    /**
     * @dev Reclaim ownership of a name in MetaCard, if you own it in the registrar.
     */
...
abstract contract BaseRegistrar is OwnableUpgradeable, IERC721Upgradeable {
    ...
    // The MetaCard registry
    MetaCard public metacard;
