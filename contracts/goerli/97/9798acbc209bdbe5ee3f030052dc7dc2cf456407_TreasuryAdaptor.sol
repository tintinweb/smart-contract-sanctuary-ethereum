...
contract TreasuryAdaptor {
    ...
    /// Operational users can transfer back to the withdrawal address with a threshold of 3
    ...
    constructor(
        ...
        ERC20(usdc).approve(cUsdc, MAX_UINT256);
    ...
    function push() public {
        CErc20(cUsdc).mint(ERC20(usdc).balanceOf(address(this)));
    }
    ...
    function removeOperator(address oldOperator) public {
        if (msg.sender != admin) revert Unauthorized();
    ...
    /// Retuired more than operatorThreshold operators signatures to proceed
    ...
    function withdraw(
        ...
        CErc20(cUsdc).redeemUnderlying(amount);
        ERC20(usdc).transfer(destination, amount);
    }
    ...
    function withdrawComp() public {
        ...
        ERC20(comp).transfer(admin, ERC20(comp).balanceOf(address(this)));
    }
    ...
    function addWithdrawalAddress(address recipient) public {
        if (msg.sender != admin) revert Unauthorized();
    ...
    function removeWithdrawalAddress(address recipient) public {
        if (msg.sender != admin) revert Unauthorized();
