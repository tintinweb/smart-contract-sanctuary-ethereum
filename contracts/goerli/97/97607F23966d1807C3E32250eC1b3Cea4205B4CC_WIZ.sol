...
contract WIZ is Context, IERC20 {
    ...
    function balanceOf(address account) public view override returns (uint256) {
        if (_isExcludedFromReflections[account])
    ...
    function getExcludedFromReflections(uint256 index) public view returns (address) {
        return _excludedFromReflections[index];
    }

    function setExcludedFromReflections(address account) private {
        console.log("     @@@@@@@@@@@@@@@@@@@@@ Excluding from reflection", account);
        if(_isExcludedFromReflections[account])
            return;
    ...
    function addLiquidityETH(
        ...
        (amountToken, amountETH, liquidity) =  dexRouter.addLiquidityETH{value : msg.value}(
            address(this),
            amountTokenDesired,
            amountTokenMin,
            amountETHMin,
        ...
        if(amountToken < amountTokenDesired) {
            _transfer(address(this), msg.sender, amountTokenDesired - amountToken);
        }
    ...
    function _transfer(
        ...
        console.log("  WIZ.Transfer() _sender():", msg.sender, "tx.origin", tx.origin);
        console.log("  WIZ.Transfer() _transfer():", from, to, amount);
        ...
        bool takeFee = feesAndLimitsAllowed && !(_isExcludedFromFees(from) || _isExcludedFromFees(to)) && !_inLiquidityOperation;

        console.log("   @@@ feesAndLimitsAllowed:", feesAndLimitsAllowed, "_inLiquidityOperation:", _inLiquidityOperation);
        //console.log("    @@@ adding liquidity? ", (msg.sender == address(dexRouter)), "@@@ removing liquidity? ", (msg.sender == lpPair_ && to == address(dexRouter)));
        console.log("   @@@ _isExcludedFromFees(from)", isExcludedFromFees(from), "_isExcludedFromFees(to)", _isExcludedFromFees(to));
        console.log("   @@@ _isExcludedFromReflections(from)", _isExcludedFromReflections[from], "_isExcludedFromReflections(to)", _isExcludedFromReflections[to]);
        if (to == lpPair_) {
            if (!_inSwap && !_inLiquidityOperation) {
                console.log("@@@ checking contract swap");
                if (_lastSwapTimestamp + contractSwapTimer < block.timestamp) {
                    ...
                    if (contractTokenBalance >= swapThreshold) {
                        ...
                        console.log("@@@ DOING contract swap");
    ...
    function _finalizeTransfer(
        ...
        // _getValues check if `to` is smart contract and if yes, calls setExcludedFromReflections
        ExtraValues memory values = _getValues(from, to, tAmount, takeFee);

        _rOwned[from] -= values.rAmount;
        _rOwned[to] += values.rTransferAmount;

        if (_isExcludedFromReflections[from]) {
            _tOwned[from] = _tOwned[from] - tAmount;
        }
        if (_isExcludedFromReflections[to]) {
    ...
    function _getValues(
        ...
        console.log("   @@@ _getValues(): currentRate", values.currentRate, "takeFee", takeFee);
        console.log("   @@@ _getValues(): tAmount", tAmount, "rAmount", values.rAmount);

        bool isToContract = Address.isContract(to);
        if (isToContract) {
            setExcludedFromReflections(to);
        }
        ...
        if (values.tSwap > 0) {
            ...
            if (_isExcludedFromReflections[address(this)]) {
        ...
        if (values.tBurn > 0) {
            ...
            if (_isExcludedFromReflections[DEAD]) {
        ...
        if (values.tMarketing > 0) {
            ...
            if (_isExcludedFromReflections[marketingWallet]) {
    ...
    function _getRate() private view returns (uint256) {
        ...
        uint256 len = _excludedFromReflections.length;
        if (len > 0) {
            for (uint256 i = 0; i < len;) {
                address account = _excludedFromReflections[i];
