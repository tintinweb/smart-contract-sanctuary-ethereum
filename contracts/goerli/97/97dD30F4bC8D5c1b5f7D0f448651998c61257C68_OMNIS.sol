...
error ProvenanceEquipmentNotSet();
error ProvenanceEquipmentOnBodyNotSet();
error ProvenanceAlreadySetOnThisStage();
    ...
    DefaultOperatorFilterer
{
    ...
    uint256 public constant RESERVED_TEAM = 299;
    ...
    string public baseEquipmentURI;
    string public provenanceBody;
    // string public provenanceEquipment;
    ...
    // uint256 public startingIndexEquipment;
    ...
    function publicMint(
        ...
        if (_totalMinted() + quantity_ > (collectionSize - RESERVED_TEAM)) revert SoldOut(); // Minimum of 100 tokens for Public
        if (_numberMinted(msg.sender) + quantity_ > limitPerWallet) revert LimitPerWalletExceeded();
    ...
    function reserve(address receiver_, uint256 quantity_) external onlyOwner {
        ...
        if (_totalMinted() + quantity_ > collectionSize) revert ReservedExceeded();
        ...
        returns (uint256 requestId)
    {
        if (diskType_ == DiskType.Body){
            ...
            // if (provedBodyCount <= 0) revert ProvenanceEquipmentNotSet();
            // if (numWords_ != provedBodyCount) revert DifferentCount();

            // for (uint256 i = 0; i < provedBodyCount; i++) {
            ...
            // }
            // if (bytes(provenanceEquipment).length <= 0) revert ProvenanceEquipmentNotSet();
            // if (startingIndexEquipment > 0) revert StartingIndexExisted();
        }
        else {
            revert NotFound();
        }
    ...
    function fulfillRandomWords(
        ...
        if (diskType == DiskType.Body){
            uint256 startingIndex = _randomWords[0] % collectionSize;
    ...
    function setProvenanceBody(
        string memory provenanceBody_
    ) external onlyOwner {
        provenanceBody = provenanceBody_;
    }

    // function setProvenanceEquipment(
    //     string memory provenanceEquipment_
    // ) external onlyOwner {
    //     provenanceEquipment = provenanceEquipment_;
    // }
    ...
    function setProvenanceEquipmentOnBody(
        ...
        // for (uint256 i = 0; i < provenanceEquipmentOnBody_.length; i++) {
            provenanceEquipmentOnBody[bodyIndex_] = provenanceEquipmentOnBody_;
            bodyCount[bodyIndex_] = bodyCount_;
            bodyName[bodyIndex_] = bodyName_;
        // }
    ...
    function setBaseEquipmentURI(string memory baseEquipmentURI_) public onlyOwner {
        baseEquipmentURI = baseEquipmentURI_;
    }
    ...
    function equipmentURI(uint256 tokenId) public view virtual returns (string memory) {
        return bytes(baseEquipmentURI).length > 0 ? string(abi.encodePacked(baseEquipmentURI, _toString(tokenId))) : "";
    }
