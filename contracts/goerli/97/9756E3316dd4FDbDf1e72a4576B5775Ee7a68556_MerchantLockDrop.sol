...
contract MerchantLockDrop  is OwnerPausable {
    ...
    function merchantInitial( LockDetail memory lockDetail )external    {
        ...
        require(  lockDetail.startTime <  lockDetail.endTime &&  lockDetail.startTime > block.timestamp && lockDetail.lockPeriod>0 ,"time is  not regularly");
        require(lockDetail.inTokenCapacity>0 && lockDetail.outTokenSupply>0,"tokenCapacity need >0");
        require( lockDetail.inToken != address(0) && lockDetail.outToken != address(0) ,"token address wrong");
        require(lockDetail.limit>0 ,"limit need > 0 "  );
        {
            ...
            li.inTokenAmount = 0;
            li.outTokenCapacity = lockDetail.outTokenSupply;
            li.outTokenSupply = lockDetail.outTokenSupply;
    ...
    function lockERC20(uint index,uint initAmount,address merchant)  external  whenNotPaused{
        require( merchantLocksInfo[merchant].LIL.length   > index ,"invalid index");
    ...
    function advanceLock(uint index)external {
        require( merchantLocksInfo[msg.sender].LIL.length   > index ,"invalid index");
        require ( merchantLocksInfo[msg.sender].LIL[index].lockDetail .owner == msg.sender, "not the owner");
        require ( merchantLocksInfo[msg.sender].LIL[index].lockDetail.startTime < block.timestamp &&  merchantLocksInfo[msg.sender].LIL[index].lockDetail.endTime > block.timestamp,
            "time invalid");
    ...
    function unlock(uint index) external   whenNotPaused{
        ...
        require(lInfo.startTime >  block.timestamp, "lock already start");
    ...
    function merchantWithdraw(uint index)external  whenNotPaused{
        require( merchantLocksInfo[msg.sender].LIL.length   > index ,"invalid index");
        require ( merchantLocksInfo[msg.sender].LIL[index].lockDetail.endTime < block.timestamp, "lock not over");
        require(  merchantLocksInfo[msg.sender].LIL[index].lockDetail.inTokenAmount >0 ||   merchantLocksInfo[msg.sender].LIL[index].lockDetail.outTokenCapacity >0 ,"no token to withdraw" );

        LockInfo  storage  lInfoAUser = merchantLocksInfo[msg.sender].LIL[index];
        LockDetail storage lInfo = lInfoAUser.lockDetail;
        uint  wAmount = lInfo.inTokenAmount;
        uint  oAmount = lInfo.outTokenCapacity;

        emit MerchantWithdraw(msg.sender, index);
        if( oAmount >0){
            lInfo.outTokenCapacity = 0;
            bool success = IERC20(lInfo.outToken).transfer(msg.sender,  oAmount);
            require(success, "Call failed");
        }
        if (wAmount >0){
            lInfo.inTokenAmount = 0;
            uint amount = wAmount*adminPoint/10000;
            adminTokenFee[lInfo.inToken] +=amount;
            bool inTokenSuccess = IERC20(lInfo.inToken).transfer(msg.sender,  wAmount - amount);
