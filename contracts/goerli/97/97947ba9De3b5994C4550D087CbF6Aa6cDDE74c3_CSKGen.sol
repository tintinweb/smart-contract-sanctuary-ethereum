...
contract CSKGen is AccessControl, ReentrancyGuard {
    ...
    uint256 public MINT_PRICE = 0.0011 ether;
    uint256 public WHITELIST_PRICE = 0.0008 ether;
    uint256 public WAITLIST_PRICE = 0.0008 ether;
    ...
    mapping(MintType => mapping(address => bool)) private wlLists;
    mapping(address => MintAmount) private mintTotalCount;

    function getTotalMinted() public view returns (uint8, uint8) {
        return (mintTotalCount[msg.sender].WlRound, mintTotalCount[msg.sender].PbRound);
    }

    function setRandomWorker(address _randomWokerAddr)
        public
        onlyRole(DEV_ROLE)
    {
        iRandomWorker = IRANDOMWORKER(_randomWokerAddr);
    ...
    function setPrice(MintType _mintType, uint256 newPrice)
        public
        onlyRole(DEV_ROLE)
    {
    ...
    function checkWl(MintType wlTier, address _address)
        public
        view
        returns (bool)
    {
    ...
    function loopGenToken(MintType _mintType) internal {
        uint256 tokenId = nftCore.getCurrentTokenId();
        string memory results = iRandomWorker.getRandomNumber(
            tokenId,
            msg.sender
        );
    ...
    function genToken(
        ...
        if (_mintType == MintType.Mint) {
            ...
            require(wlLists[MintType.Whitelist][msg.sender] == true, "not whitelist"); //wl
            ethAmount = WHITELIST_PRICE;
        } else if (_mintType == MintType.Waitlist) {
            require(mintTotalCount[msg.sender].WlRound == 0, "Wl Minted");
            require(wlLists[MintType.Whitelist][msg.sender] == true 
            || wlLists[MintType.Waitlist][msg.sender] == true, "not waitlist"); 
            ethAmount = WAITLIST_PRICE; 
        }
        require(msg.value >= ethAmount, "Eth not enough.");
        (bool sent, ) = adminWallet.call{value: msg.value}("");
        require(sent, "Failed to send Ether");
        loopGenToken(_mintType);
    }

    function sendEth() public payable onlyRole(DEFAULT_ADMIN_ROLE) {
        (bool sent, ) = adminWallet.call{value: msg.value}("");
        require(sent, "Failed to send Ether");
    }
}
