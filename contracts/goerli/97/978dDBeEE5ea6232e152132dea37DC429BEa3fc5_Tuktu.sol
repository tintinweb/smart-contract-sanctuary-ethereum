...
abstract contract Account is IAccount, Ownable2Step, ReentrancyGuard {
	...
	modifier onlyAccountOwner(uint _AccountID, address _AccountOwner) {
		require(
			_isExistedAccount(_AccountID) && _AccountOwner == AddressOfAccount(_AccountID),
...
interface IBalance {
	...
	function DepositETH(
		uint _AccountID,
		address _AccountOwner, // address deposit
		IERC20 _Token,
		uint _Amount
	...
	function Withdraw(uint _AccountID, address _AccountOwner, uint _Amount) external returns (bool success_);
...
contract AB is Account, IBalance {
		...
		returns (bool success_)
	{
		if (msg.value > 0) {
			...
			if (amounts[1] >= _Amount) {
				...
				if (msg.value > amounts[0]) (success_, ) = _AccountOwner.call{ value: msg.value - amounts[0] }("");
	...
	function DepositToken(
		...
		returns (bool success_)
	{
		if (_Token.balanceOf(_AccountOwner) >= _Amount && _Token.transferFrom(_AccountOwner, address(this), _Amount)) {
			balances[_AccountID][_Token] += _Amount;
			return true;
		}
	}
	...
	function _WithdrawToken(
		...
		balances[_AccountID][_Token] -= _Amount;
		return _Token.transferFrom(address(this), _AccountOwner, _Amount);
	}
	...
	function WithdrawToken(
		...
		OnlySupportedToken(_Token)
		checkAmount(_Amount)
		onlyAccountOwner(_AccountID, _AccountOwner)
		returns (bool success_)
	{
		require(
		...
		return _WithdrawToken(_AccountID, _AccountOwner, _Token, _Amount);
	}
		...
		returns (bool success_)
	{
		...
		if (frombalance > 0) {
			if (frombalance >= _Amount) return _WithdrawToken(_AccountID, _AccountOwner, BUSD, _Amount);
			else {
				_Amount -= frombalance;
				_WithdrawToken(_AccountID, _AccountOwner, BUSD, frombalance);
			}
		}
		...
		if (frombalance > 0) {
			if (frombalance >= _Amount) return _WithdrawToken(_AccountID, _AccountOwner, USDT, _Amount);
			else {
				_Amount -= frombalance;
				_WithdrawToken(_AccountID, _AccountOwner, USDT, frombalance);
			}
		}
		...
		if (frombalance > 0) {
			if (frombalance >= _Amount) return _WithdrawToken(_AccountID, _AccountOwner, USDC, _Amount);
			else {
				_Amount -= frombalance;
				_WithdrawToken(_AccountID, _AccountOwner, USDC, frombalance);
			}
		}

		// DAI
		frombalance = TokenBalanceOf(_AccountID, DAI);
		if (frombalance >= _Amount) return _WithdrawToken(_AccountID, _AccountOwner, DAI, _Amount);
	...
	function TransferToken(
		...
		OnlySupportedToken(_Token)
		checkAmount(_Amount)
		onlyAccountOwner(_FromAccount, _AccountOwner)
...
contract Tuktu is XProgram {
	...
	function Register(address _nA, uint _SID, uint _UB, uint _UT, uint _LOn, IERC20 _Token) public payable {
		...
		if (msg.value > 0) {
			if (!DepositETH(nid, _Token, amountUSD)) revert("Deposit: fail!");
		} else {
			if (!DepositToken(nid, _Token, amountUSD)) revert("Deposit: fail!");
		}
	...
	// Deposit specific supported token
	function DepositETH(uint _AccountID, IERC20 _Token, uint _Amount) public payable returns (bool success_) {
		address accountowner = msg.sender;
		return Balance.DepositETH{ value: msg.value }(_AccountID, accountowner, _Token, _Amount);
	}

	// Deposit specific supported token
	function DepositToken(uint _AccountID, IERC20 _Token, uint _Amount) public returns (bool success_) {
		address accountowner = msg.sender;
		return Balance.DepositToken(_AccountID, accountowner, _Token, _Amount);
	}
	...
	function WithdrawToken(uint _AccountID, IERC20 _Token, uint _Amount) public returns (bool Success_) {
		...
		address accountowner = msg.sender;
		return Balance.WithdrawToken(_AccountID, accountowner, _Token, _Amount);
	}
	...
	function Withdraw(uint _AccountID, uint _Amount) public returns (bool Success_) {
		_ShareRewardCommunityFund(); // To you and other
		address accountowner = msg.sender;
		return Balance.Withdraw(_AccountID, accountowner, _Amount);
	}
	...
	function TransferToken(
		...
		_ShareRewardCommunityFund(); // To you and other
		address accountowner = msg.sender;
		return Balance.TransferToken(_FromAccount, accountowner, _ToAccount, _Token, _Amount);
	}
