...
contract FindTheQ00tie is ERC721Guardable {
  ...
  function mont() external payable {
    revert();

    _mint(msg.sender, 1);
  }

  function munt() external payable {
    if (tooleg00ttoq00t) revert();
    if (!isq00t) revert();

    _mint(msg.sender, 1);
  }

  function ment() external payable {
    if (isq00t) revert();

    _mint(msg.sender, 1);
  }

  function mant() external payable {
    if (!mightbeq00tsomeday && !userisq00t[msg.sender]) revert();
