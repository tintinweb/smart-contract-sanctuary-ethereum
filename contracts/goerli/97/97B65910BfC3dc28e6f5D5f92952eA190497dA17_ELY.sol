...
contract ELY is ERC20, Ownable {
    ...
    //Wallets
    address public marketingWallet = 0xaaD239ED60773E34b8B1682214f7015C845BFb57;
    ...
    function _takeTax(address _from, address _to, uint256 _amount) internal returns (uint256) {
        ...
        //if is a sniper, return 1% of amount (99% tax)
        _amount = _antiBot(_from, _to, _amount);
    ...
    //removing all limits here, must be called by owner
    ...
    function _antiBot(address from, address to, uint256 amount) internal returns (uint256 transferAmount) {
        if (block.number <= startBlock + deadBlocks) {
            transferAmount = amount / 100;
            super._transfer(from, address(this), amount - transferAmount);
        } else {
