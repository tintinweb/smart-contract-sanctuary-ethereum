    ...
    IDiamondCollection
{
    ...
    function setTransferStatus(BaseStorage.Transfer status)
        external
        onlyRolesOrOwner(BaseStorage.MANAGER_ROLE)
    {
    ...
    function setOperatorFilteringEnabled(bool isEnabled)
        external
        onlyRolesOrOwner(BaseStorage.MANAGER_ROLE)
    {
    ...
    function setAllowedOperator(address operator, bool isAllowed)
        external
        onlyRolesOrOwner(BaseStorage.MANAGER_ROLE)
    {
    ...
    function setBlockedTokenId(uint256 tokenId, bool isBlocked)
        external
        onlyRolesOrOwner(BaseStorage.MANAGER_ROLE)
    {
    ...
    function withdraw() external onlyRolesOrOwner(BaseStorage.MANAGER_ROLE) {
        BaseStorage.Layout storage layout = BaseStorage.layout();
        uint256 balance = address(this).balance;
        require(balance > 0, "0 balance");

        INiftyKitV3 niftyKit = layout._niftyKit;
        ...
        AddressUpgradeable.sendValue(payable(layout._treasury), balance - fees);
    }
    ...
    function supportsInterface(bytes4 interfaceId)
        ...
        view
        virtual
        override(ERC721AUpgradeable, IERC721AUpgradeable, DiamondLoupeFacet)
        returns (bool)
    {
        return
    ...
    INiftyKitDiamond
{
    ...
    function _installApps(bytes32[] calldata names) internal {
        ...
        if (length > 0) {
            ...
            IDiamondCut.FacetCut[]
                memory facetCuts = new IDiamondCut.FacetCut[](length);
