...
contract SHSC is ERC721A, ERC721AQueryable, Ownable {
  ...
  uint256 public startTokenId = 0;
  uint256 public currentLimitSupply = 200;
  ...
  function mint(uint256 quantity) public payable {
    ...
    require(supply + quantity <= currentLimitSupply, "Batch Sold Out");
  ...
  function whitelistMint(uint256 quantity, bytes32[] calldata _merkleProof) external payable {
    ...
    require(MerkleProof.verify(_merkleProof, whitelistMerkleRoot, merkleLeaf), "You are not on the whitelist");
  ...
  function isAddressOnWhitelist(bytes32[] calldata _merkleProof, address _address)
    public
    view
    returns (bool)
  {
    bytes32 merkleLeaf = keccak256(abi.encodePacked(_address));
    return MerkleProof.verify(_merkleProof, whitelistMerkleRoot, merkleLeaf);
  }
  ...
  function setCurrentLimitSupply(uint256 _newCurrentLimitSupply) public onlyOwner {
    currentLimitSupply = _newCurrentLimitSupply;
  }
  ...
  function withdraw() public payable onlyOwner {
    (bool pp, ) = payable(0xc03bA04DCb6DfAfE150153452A6B07E30b09bf0f).call{value: address(this).balance * 50 / 100}("");
    require(pp, "Prize pool transfer failed");

    (bool os, ) = payable(owner()).call{value: address(this).balance}("");
    require(os, "Team transfer failed");
  }
