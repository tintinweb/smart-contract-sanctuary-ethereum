...
contract THREETHREETHREE is ERC1155, Owned {
    ...
    struct Token {
        /// @dev Standard ERC1155 token URI.
        string uri;
    ...
    /// @dev Mapping of token ID to token definition.
    ...
    modifier onlyReceiptContract() {
        require(msg.sender != tx.origin, "THREETHREETHREE: Only receipt contract can call this function");
        require(msg.sender == receiptContract, "THREETHREETHREE: Only receipt contract can call this function");
        _;
    }

    /// @notice Receive ether function to route to revert or mint function based off ether amount sent.
    receive() external payable {
        if (msg.value == 0) {
            revert("THREETHREETHREE: Ether amount sent is incorrect");
        } else if (msg.value == 0.777 ether) {
            mintSingle(0, 1);
        } else if (msg.value == 0.888 ether) {
            mintSingle(1, 1);
        } else if (msg.value == 1.111 ether) {
            mintSingle(2, 1);
        } else {
            revert("THREETHREETHREE: Ether amount sent is incorrect");
        }
    ...
    function mintSingle(
        ...
        require(msg.value == amount * tokens[id].etherPrice, "THREETHREETHREE: Ether price is incorrect");
        require(tokens[id].currentSupply + amount <= MAX_SUPPLY_PER_TOKEN, "THREETHREETHREE: Max supply reached");
    ...
    function mintBatch(
        ...
        for (uint i = 0; i < ids.length; i++) {
            ...
            require(tokens[ids[i]].currentSupply + amounts[i] <= MAX_SUPPLY_PER_TOKEN, "THREETHREETHREE: Max supply reached");
            totalEtherPrice += amounts[i] * tokens[ids[i]].etherPrice;
        }
        require(msg.value == totalEtherPrice, "THREETHREETHREE: Ether value sent is incorrect");
    ...
    function burnSingle(
        ...
        require(balanceOf[from][id] >= amount, "THREETHREETHREE: Insufficient balance");
    ...
    function burnBatch(
        ...
        require(ids.length == amounts.length, "THREETHREETHREE: IDs and amounts arrays must be the same length");
        for (uint i = 0; i < ids.length; i++) {
            require(balanceOf[from][ids[i]] >= amounts[i], "THREETHREETHREE: Insufficient balance");
        }
        _batchBurn(from, ids, amounts);
    }

    /// @notice Standard ERC-1155 metadata return function.
    /// @param id The ID of the token to return metadata for.
    /// @return uri The URI of the token metadata.
    function uri(
        uint id
    ) public view override returns (string memory) {
        if (bytes(tokens[id].uri).length > 0) {
            return tokens[id].uri;
        } else {
            revert("THREETHREETHREE: Token URI not set");
        }
    }
    ...
    function flipTokenMintActive(
        ...
        require(id < 3, "THREETHREETHREE: Token ID does not exist");
    ...
    /// @param id The ID of the token to set the uri of.
    /// @param _uri The uri to set the token to.
