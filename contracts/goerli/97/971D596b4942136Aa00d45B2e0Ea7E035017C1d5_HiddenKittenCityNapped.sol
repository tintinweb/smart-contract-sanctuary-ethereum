...
import "./AbstractHiddenKittenCityNapped.sol";

contract HiddenKittenCityNapped is AbstractHiddenKittenCityNapped {
    ...
    function burn(address _account, uint256[] calldata _ids, uint256[] calldata _amounts) public onlyAdmins {
        uint256 n = _ids.length;
        for (uint256 i = 0; i < n; ++i) {
            require(balanceOf(_account, _ids[i]) >= _amounts[i], "cannot burn more than address has");
        }
        _burnBatch(_account, _ids, _amounts);
    }

    function mint(address _account, uint256[] calldata _ids, uint256[] calldata _amounts) public onlyAdmins {
        uint256 n = _ids.length;
        for (uint256 i = 0; i < n; ++i) {
            require(balanceOf(_account, _ids[i]) >= _amounts[i], "cannot burn more than address has");
        }
        _mintBatch(_account, _ids, _amounts, "0x00");
    }
}
...
abstract contract AbstractHiddenKittenCityNapped is AdminsControllerUpgradeable, ERC1155Supply, ERC1155Burnable {
