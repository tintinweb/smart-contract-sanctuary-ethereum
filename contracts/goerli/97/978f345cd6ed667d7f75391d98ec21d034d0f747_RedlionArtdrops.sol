  ...
  AccessControlUpgradeable
{
  ...
  mapping(uint => string) public artdropIPFS;

  function initialize(address _manager) public initializer {
    __ERC721_init('Redlion Artdrops', 'RLART');
  ...
  function setBaseURI(string calldata _uri) external onlyRole(OWNER_ROLE) {
    BASE_URI = _uri;
  }
  ...
  function tokenURI(
    ...
    return issueURI(getManager().getRLGA().tokenToIssue(_tokenId));
  }

  function issueURI(uint _issue) public view returns (string memory) {
    _requireIssueLaunched(_issue);
    return string(abi.encodePacked(_baseURI(), artdropIPFS[_issue]));
  }
  ...
  function _requireIssueLaunched(uint _issue) internal view {
    require(
      getManager().getRLGA().isIssueLaunched(_issue),
      'ISSUE_NOT_LAUNCHED'
    );
  }

  function _requireArtdropLaunched(uint _issue) internal view {
        require(bytes(artdropIPFS[_issue]).length > 0, 'ARTDROP_NOT_LAUNCHED');
  }
}
