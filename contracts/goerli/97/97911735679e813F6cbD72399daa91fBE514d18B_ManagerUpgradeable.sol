    ...
    function setMomot(
        ...
        require(
            hasRole(WITHDRAWER_ROLE, msg.sender) ||
                ...
                OraclePermission.has(OFFICER, permExpiresAt, signature),
            'no access'
        );
        momot = momot_;
    }
...
interface IManagerUpgradeable is IAccessControlUpgradeable {
    ...
    function requestReceiver(VaultType vaultName) external returns (address);

    function getVaultAddress(VaultType name) external view returns (address);
    ...
    function setProfilePaused(bool) external;

    function setMomot(address, uint256 permExpiresAt, bytes calldata signature) external;
...
library Constants {
    function DEPLOYER_ROLE() internal pure returns (bytes32) {
        return keccak256('DEPLOYER');
    }

    function WITHDRAWER_ROLE() internal pure returns (bytes32) {
        return keccak256('WITHDRAWER');
    }
    ...
    function EXECUTOR_VOLUME_ROLE() internal pure returns (bytes32) {
        return keccak256('EXECUTOR_VOLUME');
    }

    function EXECUTOR_LIMIT_ROLE() internal pure returns (bytes32) {
        return keccak256('EXECUTOR_LIMIT');
    }
}
...
library OraclePermission {
    function has(
        ...
        return
            IOfficerUpgradeable(officer).hasPermission(
                msg.sender,
                address(this),
                permExpiresAt,
                msg.data[0:(msg.data.length - 192)],
                signature
            );
    }
}
