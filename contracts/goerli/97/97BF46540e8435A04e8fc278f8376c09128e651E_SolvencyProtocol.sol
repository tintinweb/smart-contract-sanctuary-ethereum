...
contract SolvencyProtocol {
    // using Pairing for *;
    // using VerifyingKeyA for *;
    // using VerifyingKeyB for *;

    event ProofPublished(uint vKeyId, uint savedProofIdx, bool verificationResult);

    mapping (uint => Pairing.VerifyingKey) public verifyingKeys;
    uint[] public vKeyIds;


    Pairing.PublishedProof[] public publishedProofs;
    Pairing.PublishedProof public mostRecentPublishedProof;
    ...
    function addVerifyingKey(uint[2] memory alpha1,
        ...
        verifyingKeys[vKeyId] = Pairing.VerifyingKey({
            alpha1: _alpha1,
            beta2: _beta2,
            gamma2: _gamma2,
            delta2: _delta2,
    ...
    function publishSolvencyProof(uint[2] memory a,
                                 ...
                                 uint[2] memory c, 
                                 uint[] memory input,
                                 uint vKeyId) public returns (uint, uint, bool) 
    {
        
        Pairing.Proof memory proof = Pairing.Proof({
        ...
        Pairing.PublishedProof memory publishedProof = Pairing.PublishedProof({
            proof:proof,
            publicInputs: Pairing.PublicInputs(input),
        ...
        emit ProofPublished(vKeyId, publishedProofIdx, verified);

        return (vKeyId, publishedProofIdx, verified);
    }

    function verify(uint[] memory input, Pairing.Proof memory proof, Pairing.VerifyingKey memory verifyingKey) internal view returns (bool) {
        uint256 snark_scalar_field = 21888242871839275222246405745257275088548364400416034343698204186575808495617;
        
        require(input.length + 1 == 4,"verifier-bad-input");
    ...
    function verifyProof(
        ...
        Pairing.VerifyingKey memory verifyingKey = verifyingKeys[vKeyId];
