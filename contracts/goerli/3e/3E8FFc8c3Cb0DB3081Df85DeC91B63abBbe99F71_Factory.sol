...
contract Factory is IFactory {
    address immutable basicErc20Implementation;

    constructor(address impl_) {
        basicErc20Implementation = impl_;
    }

    event Deployed(address proxy, address proxyAdmin);

    function deployUpgradableERC20(
        address owner_
    ) external returns (address proxyAddress_, address adminAddress_) {
        ...
        TransparentUpgradeableProxy proxy_ = new TransparentUpgradeableProxy(
            basicErc20Implementation,
            address(admin_),
            ""
        );
        proxyAddress_ = address(proxy_);

        IBasicToken(proxyAddress_).__Token_init("TestToken", "TT");

        IOwnable(proxyAddress_).transferOwnership(owner_);

        adminAddress_ = address(admin_);

        emit Deployed(proxyAddress_, adminAddress_);
    }
}
