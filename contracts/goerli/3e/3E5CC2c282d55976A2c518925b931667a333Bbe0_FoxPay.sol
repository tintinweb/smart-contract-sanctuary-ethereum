    ...
    IFoxPay
{
    ...
    function addBlocklisted(address _account) external onlyRole(DEFAULT_ADMIN_ROLE) {
        _grantRole(BLOCKED_ROLE, _account);
    }

    function removeBlocklisted(address _account) external onlyRole(DEFAULT_ADMIN_ROLE) {
        _revokeRole(BLOCKED_ROLE, _account);
    }

    function initialize(address metadataDescriptor_) public initializer {
        __ERC3525Mintable_init("Fox Pay", "FOX", 18);
        __Context_init();
        __Pausable_init();
    ...
    function tokenURI(
        uint256 tokenId_
    ) public view override(ERC3525Upgradeable, IERC721Metadata) returns (string memory) {
        _requireMinted(tokenId_);
        return metadataDescriptor.constructTokenURI(tokenId_);
    }

    function startDate(uint256 tokenId_) public view returns (uint256) {
        (uint256 expireYear_, uint256 expireMonth_, ) = BokkyPooBahsDateTimeLibrary.timestampToDate(
            expireDate(tokenId_)
        );
        uint256 startYear = (expireMonth_ >= 7) ? expireYear_ : expireYear_ - 1;
        // ex) 1 -> 7, 6 -> 12, 8 -> 2, 12 -> 6
        uint256 startMonth = (expireMonth_ >= 7) ? expireMonth_ - 6 : expireMonth_ + 6;
        return BokkyPooBahsDateTimeLibrary.timestampFromDate(startYear, startMonth, 1);
    }

    function expireDate(uint256 tokenId_) public view returns (uint256) {
        uint256 monthDay = slotOf(tokenId_) % 10000;
        uint256 year = (slotOf(tokenId_) - monthDay) / 10000;
    ...
    function supportsInterface(
        bytes4 interfaceId
    ) public view override(IERC165, ERC3525Upgradeable, AccessControlUpgradeable) returns (bool) {
    ...
    function _generateSVGTexts(
        ...
        return
            string(
                abi.encodePacked(
                    /* solhint-disable */
                    '<text fill="#FF980E" xml:space="preserve" style="white-space: pre" font-family="Arial" font-size="128" letter-spacing="0em"><tspan x="411.75" y="471.875">&#xa5;',
                    value_.toString(),
                    "</tspan></text>",
                    '<text fill="#FF980E" xml:space="preserve" style="white-space: pre" font-family="Arial" font-size="32" letter-spacing="0em"><tspan x="111" y="814.094">#',
                    tokenId_.toString(),
                    "</tspan></text>",
                    '<text fill="#FF980E" xml:space="preserve" style="white-space: pre" font-family="Arial" font-size="56" letter-spacing="0em"><tspan x="637.938" y="180.914">',
