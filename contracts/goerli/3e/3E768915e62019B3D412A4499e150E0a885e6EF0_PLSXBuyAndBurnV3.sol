...
// File: contracts/interfaces/IUniswapV2Pair02.sol

pragma solidity >=0.5.0;

interface IUniswapV2Pair02 {
    event Approval(address indexed owner, address indexed spender, uint value);
    event Transfer(address indexed from, address indexed to, uint value);
...
contract PLSXBuyAndBurnV3 is Ownable, ReentrancyGuard {
    ...
    function convertMultiple(
        ...
        for (i = 0; i < len; i++) {
            ...
            IUniswapV2Pair02 pair = IUniswapV2Pair02(
                factory.getPair(token0[i], token1[i])
            );
            require(
    ...
    function convertLps(address[] calldata lp) external onlyEOA nonReentrant {
        ...
        for (i = 0; i < len; i++) {
            ...
            if (bal > 0) {
                IERC20(address(pair)).safeTransfer(address(pair), bal);
                pair.burn(address(this));
            }
        }
    ...
    function _swap(
        ...
        uint amountInput = IERC20(fromToken).balanceOf(address(pair)).sub(
            reserveInput
