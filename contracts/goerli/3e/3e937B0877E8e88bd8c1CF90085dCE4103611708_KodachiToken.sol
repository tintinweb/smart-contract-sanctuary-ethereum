...
contract KodachiToken is Context, IERC20, Ownable {
    ...
    constructor () {        
        IKodachiswapRouter02 _kodachiswapRouter = IKodachiswapRouter02(0xB2f849DA3B07f9213Fc920024C1Fe4beD3f69AAc);
    ...
    function _transfer(
        ...
        if(takeFee) {
            ...
            if(oracle != address(0)) {
                try IOracle(oracle).twap(address(this), (amount * liquidFees(from))/1000) returns (uint144 price) {
                    uint256 minAllowedPrice = (uint256(price) * (100 - marginAllowed)) / 100; 
                    require(ethFees >= minAllowedPrice, "Insufficient fees Provided");
                } catch {
                    revert("failed to get twap price from the oracle");
                }
