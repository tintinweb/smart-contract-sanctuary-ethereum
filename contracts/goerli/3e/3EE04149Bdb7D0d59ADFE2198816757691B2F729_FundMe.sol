...
error FundMe__NotOwner(); // idea

contract FundMe {
    using PriceConverter for uint256;

    // constant, immutable => state variables
    uint256 public constant MINIMUM_USD = 0.01 * 1e18;
    address public immutable i_contractOwner;
    address[] public s_funders; // s_ stand for storage
    ...
    function withdraw() public payable onlyOwner {
        ...
        // reset an array
        s_funders = new address[](0); // reset all with value of 0
    ...
    function cheaperWithdraw() public payable onlyOwner {
        address[] memory funders = s_funders; // funders save in memory, s_funders save in storage, and load/save from/to memory is much cheaper than storage
        // mapping can't be in memory, just array can
        for (uint256 funderIndex = 0; funderIndex < funders.length; funderIndex++) {
            address funder = funders[funderIndex];
            s_addressToAmountFunded[funder] = 0;
        }
        s_funders = new address[](0); // reset all with value of 0
        (bool callSuccess, ) = i_contractOwner.call{value: address(this).balance}("");
        require(callSuccess, "Call failed!");
    }
