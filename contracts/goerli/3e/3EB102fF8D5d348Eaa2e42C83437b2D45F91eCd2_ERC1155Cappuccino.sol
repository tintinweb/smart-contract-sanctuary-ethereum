...
interface IERC1155Burnable is IERC1155 {
    function burn(address account, uint256 id, uint256 value) external view returns (bool);
}
...
contract ERC1155Marketplace is ERC1155YuzuInternal, ReentrancyGuard {
    ...
    // IERC1155 token to be used for yield
    IERC1155Burnable public erc1155Token;
    ...
    constructor(
        IERC20 _paymentToken, address _owner, address _admin, uint256 _commissionRate, 
    ...
    function setYield(address from, uint256 tokenId, uint256 price, uint256 qty) external nonReentrant {
        // validate
    ...
    function getYield(address from, uint256 tokenId, uint256 qty) external nonReentrant {
        // validate
        require(from != address(0), "Cappuccino: cannot zero address");
        require(tokenId > 0 , "Cappuccino: cannot zero id");
        require(qty > 0 , "Cappuccino: cannot zero qty");
        uint256 balanceOf = erc1155Token.balanceOf(from, tokenId);
        require(balanceOf >= qty, "Cappuccino: cannot transfer qty more than you owned.");
        uint256 remainYieldQty = openYield[tokenId].qty - qty;
        require(remainYieldQty >= 0, "Cappuccino: cannot get yield more than fund quantity.");

        erc1155Token.safeTransferFrom(from, admin, tokenId, qty, "0x00");

        uint256 total = qty * openYield[tokenId].price;
        require(paymentToken.transferFrom(address(this), from, total), "Transfer failed.");

        openYield[tokenId].qty = remainYieldQty;
    }

    function balanceOf1155(address from, uint256 tokenId) external view returns (uint256) {
        // validate
        require(from != address(0), "Cappuccino: cannot zero address");
        require(tokenId > 0 , "Cappuccino: cannot zero id");
    ...
    function distributeFunds(uint256 totalPrice, address from, address to, uint256 tokenId, bool useTransferFrom) internal {
        ...
        uint256 amount4owner = totalPrice - amount4admin;
    ...
    function updateAdmin(address _admin, uint256 _commissionRate, bool _anyoneCanMint, IERC20 _paymentToken) external onlyOwner() {
        require(_commissionRate < 100, "Cappuccino: total commissions should be lower than 100");
        admin = _admin;
        commissionRate = _commissionRate;
    ...
    // update contract fields
    function updateERC1155Address(IERC1155Burnable _erc1155Token) external nonReentrant() {
 ...
 */
contract ERC1155Cappuccino is ERC1155Marketplace, EIP712 {
    string private constant SIGNING_DOMAIN = "ERC1155Cappuccino-Voucher";
    string private constant SIGNATURE_VERSION = "1";

    constructor(IERC20 _paymentToken, address _owner, address _admin, uint256 _commissionRate, 
        string memory name, string memory symbol, bool _anyoneCanMint, string memory uri)
        ERC1155Marketplace(_paymentToken, _owner, _admin, _commissionRate, name, symbol, _anyoneCanMint, uri)
