...
import "./ITreasuryFactory.sol";
import "./Treasury.sol";
import "@openzeppelin/contracts/proxy/Clones.sol";

/**
 * @title TreasuryFactory
 * @notice This contract is used for deployment of Treasury contracts
 */
contract TreasuryFactory is ITreasuryFactory {
  address public treasuryImplementation;

  constructor() {
    treasuryImplementation = address(new Treasury());
  }

  /**
   * @dev See {ITreasuryFactory-createTreasury}
   */
  function createTreasury(
    ...
    address treasuryAddr = Clones.clone(treasuryImplementation);
    Treasury(payable(treasuryAddr)).initialize(name, description, owner);
    emit TreasuryCreated(msg.sender, treasuryAddr, owner);
    return treasuryAddr;
  }
}
...
/// @notice This contract is used for deployment of Treasuries
interface ITreasuryFactory {
  event TreasuryCreated(address indexed createdBy, address treasury, address indexed owner);

  /**
   * @dev Create a treasury with the given owner
   * @param owner The owner of the new Treasury contract
   */
  function createTreasury(
    string memory name,
    string memory description,
