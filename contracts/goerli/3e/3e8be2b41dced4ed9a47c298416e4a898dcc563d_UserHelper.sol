...
contract UserHelper is Initializable, UUPSUpgradeable, AccessControlUpgradeable {
    ...
    struct DAOV1_User_State {
        DAOV1Staking_User_State stakingState;
        DAOV1Governance_User_State governanceState;
    }

    struct DAOV1Staking_User_State {
        uint256 balance;
        uint256 votingPower;
        uint256 lockedUntil;
    ...
    struct DAOV1Governance_User_State {
    ...
    function getDAOV1_UserState(address staking, address governance, address user) public view returns (DAOV1_User_State memory) {
        DAOV1_User_State memory state;

        state.stakingState = getDAOV1Staking_UserState(staking, user);
        state.governanceState = getDAOV1Governance_UserState(governance, user);

        return state;
    }

    function getDAOV1Staking_UserState(address staking, address user) public view returns (DAOV1Staking_User_State memory) {
        IDAOV1Staking d = IDAOV1Staking(staking);

        DAOV1Staking_User_State memory state;

        state.balance = d.balanceOf(user);
        state.votingPower = d.votingPower(user);
        state.lockedUntil = d.userLockedUntil(user);
        state.delegatedPower = d.delegatedPower(user);
        state.delegatedTo = d.userDelegatedTo(user);

        return state;
    }

    function getDAOV1Governance_UserState(address governance, address user) public view returns (DAOV1Governance_User_State memory) {
        IDAOV1Governance d = IDAOV1Governance(governance);

        DAOV1Governance_User_State memory state;

        state.latestProposalId = d.latestProposalIds(user);
