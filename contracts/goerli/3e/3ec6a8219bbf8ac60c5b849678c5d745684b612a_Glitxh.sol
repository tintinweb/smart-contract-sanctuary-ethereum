...
contract Glitxh is ERC721A, ReentrancyGuard, Ownable {
    using Strings for uint256;

   //@todo modify these when deploying
    ...
    uint256 constant GLITXH_SUPPLY = 1000;
    uint256 constant MINT_COST = 0.0001 ether;
    uint256 constant MINT_PER_WALLET = 1000; 
    ...
    Phase public mintingPhase = Phase.AL;
    ...
    // @notice if supply is available
    modifier onlyIfSupplyAvailable() {
        if (_totalMinted() > GLITXH_SUPPLY) {
    ...
    modifier onlyOwnerOf(uint256 tokenId) {
        if (msg.sender != ownerOf(tokenId)) {
            revert NotGlitxhOwner();
        }
        _;
    }

    constructor(string memory _baseTokenURI, bytes32 _merkleRoot) ERC721A("GLITXH", "GLITXH") {
    ...
    function mintAllowList(bytes32[] calldata proof)
        ...
        // onlyIfNotAlreadyClaimedAllowlist @todo: add this when deploying
