...
contract LockFactory is AccessControl, DeployFee {
    ...
    function addToWhiteList(address[] memory whiteListAddresses) external onlyRole(FACTORY_MANAGER) {
    ...
    function removeFromWhiteList(address[] memory removedAddresses) external onlyRole(FACTORY_MANAGER) {
    ...
    function getRequiredMsgValue() external view returns(uint256 fixedRequiredETH, uint256 percentageRequiredETH) {
        (fixedRequiredETH, percentageRequiredETH) = calculateRequiredETH();
    }
    ...
    function deployLock(
        ...
        if(!hasRole(WHITE_LIST, _msgSender()) || !checkTokenOnWhiteList(lockAndInitialBeneficiariesData)){
            verifyFeePayment(
    ...
    function verifyFeePayment(address paymentToken, bytes memory data) internal {
        require(
    ...
    function checkTokenOnWhiteList(bytes memory lockAndInitialBeneficiariesData) internal view returns(bool) {
        (bytes memory lockData, ) = abi.decode(lockAndInitialBeneficiariesData, (bytes, bytes));
        (, , , address token, , , , , ) = abi.decode(
            lockData,
            (
                address,
