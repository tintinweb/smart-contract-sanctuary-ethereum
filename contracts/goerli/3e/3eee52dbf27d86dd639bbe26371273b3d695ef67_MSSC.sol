...
contract MSSC is MSSCBase, MembraneAuth {
    ...
    function registerHybridCycle(
        ...
        _cycles[cycleId].lock.init(cycleId, hashlock, deadline);
    ...
    function withdraw(bytes32 cycleId, bytes32 instructionId)
        ...
        cycleExists(cycleId)
    {
      _cycles[cycleId].lock.assertIsNotLocked();
...
abstract contract MSSCBase {
    ...
    modifier isLocked(bytes32 cycleId) {
        _cycles[cycleId].lock.assertIsLocked();
...
library LockManagerLib {
     ...
     */
    function init(LockInfo storage lock, bytes32 cycleId, bytes32 hashlock_, uint32 deadline) internal {
        if (lock.hashlock != bytes32(0)) revert AlreadyInitialized();
        if (uint32(block.timestamp) >= deadline ) revert InvalidDeadline();
    ...
    function assertIsNotLocked(LockInfo memory lock) internal view {
        if (isLocked(lock)) revert Locked();
    }
  
    function assertIsLocked(LockInfo memory lock) internal view {
