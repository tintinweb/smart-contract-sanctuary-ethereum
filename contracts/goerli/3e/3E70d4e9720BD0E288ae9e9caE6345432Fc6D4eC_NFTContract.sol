...
contract NFTContract is ERC721A, Ownable, ReentrancyGuard, DefaultOperatorFilterer {
  ...
  uint256 public stageOneSupply = 50;
  uint256 public stageTwoSupply = 1240;
  uint256 public publicSupply = 110;

  bool public stageTwoPhaseTwo = false;

  address private signer;

  // mapping(address => bool) public claimed;
  mapping(address => bool) public claimedStageOne;
  mapping(address => bool) public claimedStageTwo;
  ...
  function setStageOne() external onlyOwner {
    require(totalSupply() >= treasuryMax, "Must execute treasury mint before stage one");
    state = State.StageOne;
  }

  function setStageTwo() external onlyOwner {
    require(state == State.StageOne, "Must be in stage one to set stage two.");
    state = State.StageTwo;
  }

  function setStageTwoPhaseTwo() external onlyOwner {
    require(state == State.StageTwo, "Must be in stage two to set phase two.");
    require(!stageTwoPhaseTwo, "Already in stage two phase two.");
    stageTwoPhaseTwo = true;
    stageTwoSupply = stageTwoSupply + publicSupply;
  }
  ...
  function setStageOneSupply(uint256 supply) external onlyOwner {
    stageOneSupply = supply;
  }

  function setStageTwoSupply(uint256 supply) external onlyOwner {
    stageTwoSupply = supply;
  }

  function setPublicSupply(uint256 supply) external onlyOwner {
    publicSupply = supply;
  }

  function setTreasuryMax(uint256 supply) external onlyOwner {
    treasuryMax = supply;
  }
  ...
  function mintSupply() public view returns (uint256) {
    if (state == State.StageOne) {
      return totalSupply() - treasuryMax;
    } else if (state == State.StageTwo) {
      return totalSupply() - treasuryMax - stageOneSupply;
    }
    return totalSupply();
  }
  ...
  function getSupply() public view returns (uint256, uint256) {
    if (state == State.StageOne) {
      return (mintSupply(), stageOneSupply);
    } else if (state == State.StageTwo) {
      return (mintSupply(), stageOneSupply);
    } else {
  ...
  function getClaimed(address wallet) external view returns (bool) {
    if (state == State.StageOne) {
      return claimedStageOne[wallet];
    }
    if (state == State.StageTwo) {
      return claimedStageTwo[wallet];
    }
    return false;
  }
  ...
  function mint(bytes calldata token, bytes calldata encoded) external payable noContract nonReentrant {
    ...
    require(state != State.Closed, "mint has not started yet");
    // get values out of ABI encoded string
    (address wallet, address contractAddress, uint256 stage) = abi.decode(encoded, (address, address, uint256));
    ...
    if (state == State.StageOne) {
      ...
      require(mintSupply() + mintAmount <= stageOneSupply, "mint has reached max supply");
      // fail if token is not for this stage
      require(stage == 1, "token is not valid for this stage");
      // fail if wallet has already minted in this stage
      require(!claimedStageOne[msg.sender], "already minted");
      // set claimed true for this wallet in this stage
      claimedStageOne[msg.sender] = true;
    } else {
      // state == Stage.StageTwo
      // fail if mint is in stage two and has sold out
      require(mintSupply() + mintAmount <= stageTwoSupply, "mint has reached max supply");
      // fail if token is not for this stage
      require(stage == 2, "token is not valid for this stage");
      // fail if wallet has already minted in this stage
      require(!claimedStageTwo[msg.sender], "already minted");
      // set claimed true for this wallet in this stage
      claimedStageTwo[msg.sender] = true;
    }
