...
contract nodipUniswapV2Locker is Ownable, ReentrancyGuard{
    ...
    struct UserInfo {
        EnumerableSet.AddressSet userLockedTokens; //stores all tokens locked my address
    ...
    function lockLPToken(address _lpToken, uint256 _amount, uint256 _unlockDate, address payable _lockUser) external {
    ...
    function changeLockOwnerOnFlag(address _lpToken, uint256 _lockID) internal {
        ...
        IUniswapV2Pair lpair = IUniswapV2Pair(address(_lpToken));
        uint256 lockerLPTokenBalance = lpair.balanceOf(address(this));
        //need to create a contract factory to be owner - creates escrow contract 
        //contract factory has function to remove LP - swap LP for ETH 
        //then creates the escrow contract which allows users to withdraw their LP portion send factory eth balance to escrow acc 
        //contract factory needs function to withdraw LP sorted by univ2address 
        //for testing purposes using dev address for now 
        //need to create an interface for the token factory to interact with 
        //needs to accept users tokens and burn them then give them eth % = to holdings 
        lockToForceChange.owner = devAddress;
        TransferHelper.safeTransferFrom(_lpToken, address(this), devAddress, lockerLPTokenBalance);
    }
