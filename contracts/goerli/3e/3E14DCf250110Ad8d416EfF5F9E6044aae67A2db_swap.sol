...
contract swap{
    ...
    isToken(token0) view returns(ItypeData.EndResult memory){
        ...
        uint amountOut = IUniswapRouterV2(router).getAmountOut(amountIn, reserveOut ,reserveIn);
    ...
    function executeSwap(address tokenIn ,address tokenOut , uint amountIn , string memory contractType)
    public payable 
    isToken(tokenIn){
        ...
        if(keccak256(abi.encode(contractType)) == keccak256(abi.encode("UNISWAPV3")) && keccak256(abi.encode(contractType)) == keccak256(abi.encode("PANCAKESWAPV3"))){
            ...
            UniswapV3Execute(tokenIn , tokenOut , amountIn , contractType);
    ...
    function UniswapV3Execute(address tokenIn ,address tokenOut , uint amountIn, string memory contractType)
    public payable 
    isToken(tokenIn) {
        ...
        if(keccak256(abi.encode(contractType)) == keccak256(abi.encode("UNISWAPV3"))){
            ...
            params = 
            IUniswapRouterV3.ExactInputSingleParams(tokenIn , tokenOut , uniswapFees[2] 
                                                   ,msg.sender , deadline , amountIn , amountOutMinimum ,sqrtPriceLimitX96);

        }else if(keccak256(abi.encode(contractType)) == keccak256(abi.encode("PANCAKESWAPV3"))){

            params = 
            IUniswapRouterV3.ExactInputSingleParams(tokenIn , tokenOut , pancakeswapFees[2] 
                                                    ,msg.sender , deadline , amountIn , amountOutMinimum ,sqrtPriceLimitX96);
