...
contract FixedStaking is AccessControl {
    ...
    function stake(
        ...
        uint256 reward = (_amount * _stake.interest) / 1000;
    ...
    function claim(uint256 _stakeId) public {
        ...
        console.log("amount", _stake.amount, "reward", reward);
    ...
    function unstake(uint256 _stakeId) public {
        ...
        // Full reward if it was staked for full duration
        uint256 reward = (_stake.amount * stakeType.interest) / 1000;
        // scale the reward based on amount of time staked
        reward =
            (reward * (block.timestamp - _stake.stakeAt)) /
            stakeType.duration;
        // reduce rewards by 70%
    ...
    function restake(uint256 _stakeId) public returns (uint256) {
        Stake memory _stake = stakes[_stakeId];
        StakeType memory stakeType = stakeTypes[_stake.stakeType];
        ...
        uint256 newReward = (amount * stakeType.interest) / 1000;
    ...
    function setPenalty(uint256 _penalty) public onlyRole(DEFAULT_ADMIN_ROLE) {
        require(penalty >= 0, "penalty should be greater than 0");
        require(penalty <= 100, "penalty should be less than 100");
        penalty = _penalty;
    }
