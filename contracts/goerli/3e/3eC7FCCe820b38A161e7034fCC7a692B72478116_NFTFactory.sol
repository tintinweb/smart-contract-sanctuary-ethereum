...
contract NFTFactory {
    ...
    function createNFT(string memory _name, string memory _symbol) public returns (address) {
    ...
    function getNFTs(address _owner) public view returns (address[] memory) {
        NFT[] memory ownerNFTs = nfts[_owner];
        address[] memory result = new address[](ownerNFTs.length);
        for (uint i = 0; i < ownerNFTs.length; i++) {
            result[i] = address(ownerNFTs[i]);
        }
        return result;
    }
}
