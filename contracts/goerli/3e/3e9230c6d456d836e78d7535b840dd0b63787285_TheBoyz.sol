...
contract TheBoyz is ERC721Enumerable, Ownable
{
    ...
    mapping(address => uint256) public publicClaimedBy;
     ...
     struct TheBoy {
      ...
      mapping(uint256 => TheBoy) public allTheBoyz;

      // check if token URI exists
      mapping(string => bool) public tokenURIExists;


    constructor() ERC721("The Boyz", "TBZ")  {
            collectionName = name();
  ...
  function buyToken(uint256 _tokenId) public payable {
    ...
    TheBoy memory theboy = allTheBoyz[_tokenId];
    // price sent in to buy should be equal to or more than the token's price
    require(msg.value >= theboy.price);
    // token should be for sale
    require(theboy.forSale);
    // transfer the token from owner to the caller of the function (buyer)
    _transfer(tokenOwner, msg.sender, _tokenId);
    // get owner of the token
    address payable sendTo = theboy.currentOwner;
    // send token's worth of ethers to the owner
    sendTo.transfer(msg.value);
    // update the token's previous owner
    theboy.previousOwner = theboy.currentOwner;
    // update the token's current owner
    theboy.currentOwner = payable(msg.sender);
    // update the how many times this token was transfered
    theboy.numberOfTransfers += 1;
    // set and update that token in the mapping
    allTheBoyz[_tokenId] = theboy;
  }
...
//   switch between set for sale and set not for sale
  function toggleForSale(uint256 _tokenId) public {
    ...
    function mint(uint chosenAmount , uint256 _price) public payable isSaleOpen{
        ...
        for (uint i = 0; i < chosenAmount; i++) {
            _safeMint(msg.sender, theBoyzCounter);
            theBoyzCounter++;

            TheBoy memory newTheBoy = TheBoy(
            theBoyzCounter,
            payable(msg.sender),
            payable(msg.sender),
            payable(address(0)),
            ...
            true);
             ...
             allTheBoyz[theBoyzCounter] = newTheBoy;
