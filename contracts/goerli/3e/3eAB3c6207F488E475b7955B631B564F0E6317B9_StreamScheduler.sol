...
contract StreamScheduler is IStreamScheduler {
    ...
    function executeCreateStream(
        ...
        if(order.startDate == 0 || (order.startDate + order.startDuration > block.timestamp)) revert TimeWindowInvalid();
        // revert if userData was set by user, but caller didn't provide it
        bytes32 userDataHash = userData.length > 0 ? keccak256(userData) : bytes32(0x0);
        if(order.userData.length > 0 && userDataHash != order.userData) revert UserDataInvalid();
    ...
    function executeDeleteStream(
        ...
        // revert if userData was set by user, but caller didn't provide it
        if(order.userData.length > 0 && keccak256(userData) != order.userData) revert UserDataInvalid();
