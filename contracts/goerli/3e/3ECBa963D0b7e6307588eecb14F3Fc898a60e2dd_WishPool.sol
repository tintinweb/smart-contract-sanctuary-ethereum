...
contract WishPool is Initializable, Pausable, Ownable {
    ...
    // useraddr -> tokenid -> redeemgen -> wishid -> grantaddr -> granttokenid
    // -> grantnftgen -> grantwishid -> color -> donatetoken -> donateamount -> blocktime
    event Redeem(address indexed, uint256, uint8, string, address, uint256, uint8, string, string, address, uint256, uint256);
    ...
    function redeem(RedeemData memory redeemData) public 
        whenNotPaused needInit payable {
        require(!appConf.validBlacklist(_msgSender()), "can not redeem");

        // burn redeem nft
        address redeemNftToken = appConf.getRedeemNftToken();
        // IBurnNft(redeemNftToken).burn(redeemData.tokenId);

        // nft info
        (, uint8 redeemNftGen,) = IFactory(appConf.getNftFactoryAddr()).tokenDetail(Model.NFT_TYPE_WISH, redeemData.tokenId);
        
        // handle grant
        (, uint256 grantTokenId, uint8 grantNftGen) = _grant(redeemData.grantAddr);

        // handle donate
        if (redeemData.donateAmount > 0) {
            _donate(redeemData.donateToken, redeemData.donateAmount);
        }

        emit Redeem(_msgSender(), redeemData.tokenId, redeemNftGen, redeemData.wishId, 
            redeemData.grantAddr, grantTokenId, grantNftGen, redeemData.grantWishId, redeemData.grantColor, 
            redeemData.donateToken, redeemData.donateAmount, block.timestamp);
    }
    ...
    function _grant(address grantAddr) private returns(address, uint256, uint8) {
        ...
        if (grantAddr != address(0)) {
            ...
            uint256[] memory grantTokenIds = IFactory(appConf.getNftFactoryAddr()).mintDamo(grantAddr, nftType, grantNftGen, 1);
    ...
    function _donate(address donateToken, uint256 donateAmount) private {
        require(appConf.validDonateToken(donateToken), "invalid donate token");

        if (donateToken == address(0)) {
            require(msg.value == donateAmount, "invalid donateAmount");
            require(_msgSender().balance >= donateAmount, "balance insufficient");
            Address.sendValue(payable(appConf.getDonateAddr()), donateAmount);
        } else {
            uint256 balance = IERC20Ex(donateToken).balanceOf(_msgSender());
            uint256 allowance = IERC20Ex(donateToken).allowance(_msgSender(), address(this));
            require(balance >= donateAmount && allowance >= donateAmount, "balance or allownce insufficient");

            IERC20Ex(donateToken).safeTransferFrom(_msgSender(), appConf.getDonateAddr(), donateAmount);
        }
    }
}
...
interface IFactory {
    function mintDamo(address to,uint8 damoType,uint8 genera,uint8 numbers) external returns(uint256[] memory);
...
interface IAppConf {
    ...
    function validDonateToken(address donateToken) external view returns(bool);
