...
contract FundPG {
    ...
    function getCurrentLiquidityIndex() public view returns (uint256 liquidityIndex) {
        AaveLendingPool aaveContract = AaveLendingPool(strategyAddress);
        (, liquidityIndex, , , , , , , , , , ) = aaveContract.getReserveData(depositToken);
    }

    function getUserDetails() public view returns (uint256 userAllocation, uint256 userPrincipal, uint256 initialLiquidityIndex) {
        userAllocation = users[msg.sender].userAllocation;
        userPrincipal = users[msg.sender].userPrincipal;
        initialLiquidityIndex = users[msg.sender].initialLiquidityIndex;
    }

    function getInitialScaledBalance() public view returns (uint256 userPrincipal, uint256 initialLiquidityIndex, uint256 initialScaledBalance, uint256 totalValue) {
        userPrincipal = users[msg.sender].userPrincipal;
        initialLiquidityIndex = users[msg.sender].initialLiquidityIndex;
        initialScaledBalance = WadRayMath.rayDiv(userPrincipal,initialLiquidityIndex);
        totalValue = WadRayMath.rayMul(initialScaledBalance, getCurrentLiquidityIndex());
    }
    ...
    function getUserBalance(address userAddress) public view returns(uint256 totalValue, uint256 userWithdrawAmount, uint256 donatedYield) {
        ...
        (, uint256 liquidityIndex, , , , , , , , , , ) = aaveContract.getReserveData(depositToken);
