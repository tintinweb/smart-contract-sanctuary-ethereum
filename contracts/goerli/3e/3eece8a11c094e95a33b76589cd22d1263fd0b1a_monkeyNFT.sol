...
contract monkeyNFT is ERC721A, Ownable {
  ...
  function mint(uint256 tokens) public payable {
    require(!paused, "monke: oops contract is paused");
    require(publicSale, "monke: Sale Hasn't started yet");
    uint256 supply = totalSupply();
    uint256 ownerTokenCount = balanceOf(_msgSender());
    require(tokens > 0, "monke: need to mint at least 1 NFT");
    require(tokens <= maxpertx, "monke: max mint amount per tx exceeded");
    require(supply + tokens <= maxSupply, "monke: We Soldout");
    require(ownerTokenCount + tokens <= MaxperWallet, "monke: Max NFT Per Wallet exceeded");
    require(msg.value >= cost * tokens, "monke: insufficient funds");
...
/// @dev presale mint for whitelisted
    ...
    require(!paused, "monke: oops contract is paused");
    require(preSale, "monke: Presale Hasn't started yet");
    require(MerkleProof.verify(merkleProof, merkleRoot, keccak256(abi.encodePacked(msg.sender))), "monke: You are not Whitelisted");
    uint256 supply = totalSupply();
    uint256 ownerTokenCount = balanceOf(_msgSender());
    require(ownerTokenCount + tokens <= MaxperWalletWl, "monke: Max NFT Per Wallet exceeded");
    require(tokens > 0, "monke: need to mint at least 1 NFT");
    require(tokens <= MaxperTxWl, "monke: max mint per Tx exceeded");
    require(supply + tokens <= WlSupply, "monke: Whitelist MaxSupply exceeded");
    require(msg.value >= wlcost * tokens, "monke: insufficient funds");
