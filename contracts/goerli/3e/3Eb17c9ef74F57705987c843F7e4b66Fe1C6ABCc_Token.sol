...
abstract contract ERC2981 is IERC2981, ERC165 {
    ...
    constructor(address _royaltyReceiver, uint96 _royaltyFraction) {
        _setRoyaltyInfo(_royaltyReceiver, _royaltyFraction);
    }
...
contract Token is Ownable, ERC2981, ERC721A {
    ...
    uint256 public constant RESERVED_AIDDROP = 500;
    ...
    function whitelistMint(
        ...
        if (
            maxSupply - _totalMinted() - RESERVED_AIDDROP - RESERVED_TEAM <
            quantity
        ) revert SoldOut();

        if (!verifySignature(signature_)) revert InvalidSignature();
