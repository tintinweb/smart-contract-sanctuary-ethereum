...
contract Garage is Ownable, ReentrancyGuard {
    ...
    function removeStake(uint16 _id) external onlyRemover {
        ...
        if(rewardEnd > 0 && block.timestamp > rewardEnd) {
            penaltyEarnings[msg.sender].earnings += ((((rewardEnd - StakedNFTInfo[_id].lastClaimTimestamp) * dailyDust) / 86400) * multiplier[StakedNFTInfo[_id].NFTMultiplier]) / 10000;
        } else {
            penaltyEarnings[msg.sender].earnings += ((((block.timestamp - StakedNFTInfo[_id].lastClaimTimestamp) * dailyDust) / 86400) * multiplier[StakedNFTInfo[_id].NFTMultiplier]) / 10000;
        }
    ...
    function migrateGarage2(uint256 start, uint256 end) external onlyOwner {
        uint80 _stakedTimestamp;
        uint80 _lastClaimTimestamp; 
        for(uint i = start; i <= end;) {
            if(oldGarage.StakedAndLocked(uint16(i))) {
