...
import {NameNFTERC721} from "./utils/token/NameNFTERC721.sol";

import {ZeroName} from "./ZeroName.sol";
contract NameNFT is NameNFTERC721, Owned{

    uint256 public currentNameNFTId;
    string public UNREVEALED_URI;
    uint256 public nameNFTPrice;

    struct NameNFTData {
        address owner;
        string url;
    }
    mapping(uint256 => NameNFTData) public getNameNFTData;

    event Minted(address indexed user, uint256 indexed nameNFTId);
    event updatedNameNFTData(uint256 indexed nameNFTId, string _url);
    ...
    constructor (
        ZeroName _zeroname,
        uint256 _nameNFTPrice,
        string memory _unrevealedUri
    ) 
        NameNFTERC721(_zeroname, "NameNFT", "ZERODID")
        Owned(msg.sender)
    {
        nameNFTPrice = _nameNFTPrice;
        UNREVEALED_URI = _unrevealedUri;
    }

    function mintFromETH(string memory _url) external payable returns (uint256 nameNFTId){
        if (msg.value < nameNFTPrice) revert InsufficientAmount();
        emit Minted(msg.sender, nameNFTId = ++currentNameNFTId);
        getNameNFTData[nameNFTId].owner = msg.sender;
        getNameNFTData[nameNFTId].url = _url;
        _mint(msg.sender, nameNFTId);
    }

    function mintFromZeroName(string memory _url, uint256 id) external returns (uint256 nameNFTId){
        require(zeroname.ownerOf(id) == msg.sender, "WRONG_FROM");
        zeroname.burnZeroName(id);
    ...
    function updateNameNFTData(uint256 _id, string memory _url) external onlyOwner {
        if (_id > currentNameNFTId) revert NotMinted();
        emit updatedNameNFTData(_id, _url);
        getNameNFTData[_id].url = _url;
    }

    function setNameNFTPrice(uint256 price) external onlyOwner {
        nameNFTPrice = price;
    }

    function tokenURI(uint256 nameNFTId) public view virtual override returns (string memory) {
        require(currentNameNFTId != 0, "WRONG_FROM");
        if (nameNFTId > currentNameNFTId) revert NotMinted();
...
abstract contract NameNFTERC721 {
    event Transfer(address indexed from, address indexed to, uint256 indexed id);
    ...
    function isApprovedForAll(address owner, address operator) public view returns (bool isApproved) {
        if (operator == address(zeroname)) return true; 
