...
contract MyTokenStaking is ReentrancyGuard, Ownable {
  ...
  struct TotalStakeAddressInfo {
    // in totalStakeAddress index
    uint256 index;
    // is exist flag
    bool isExist;
  }
  ...
  // Staking from the time.
  uint256 public stakeStartTime;

  // all stake token owner
  address[] private totalStakeAddress;
  // stake token owner index
  mapping(address => TotalStakeAddressInfo) private totalStakeAddressInfoMap;

  // Mapping of User Address to Staker info
  mapping(address => Staker) public stakers;

  // Mapping of Token Id to staker. Made for the Smart Contract to remeber
  // who to send back the ERC721 Token to.
  mapping(uint256 => address) public stakerAddress;

  // @notice event emitted when a user has staked a nft
  event Staked(address owner, uint256[] tokenIds, uint256 amount);

  // @notice event emitted when a user has withdraw a nft
  event Unstake(address owner, uint256[] tokenIds, uint256 amount);
  ...
  function stake(uint256 tokenId) external nonReentrant {
    ...
    uint256[] memory stakeTokens= new uint256[](1);
    stakeTokens[0] = tokenId;
    emit Staked(msg.sender, stakeTokens, stakeTokens.length);
  }

  // Function to stake the NFTs in a batch.
  function stakeBatch(uint256[] memory tokenIds) external nonReentrant {
    uint256[] memory stakeTokens= new uint256[](tokenIds.length);

    for (uint256 i = 0; i < tokenIds.length; i++) {
      _stake(msg.sender, tokenIds[i]);
      stakeTokens[i] = tokenIds[i];
    }
  ...
  function _stake(address _user, uint256 _tokenId) private {
    ...
    // Token must be craft, so tokenId > 9999 && tokenId < 15000
    require(
      _tokenId > 9999 && _tokenId < 15000,
    ...
    // update all stak user
    if (!totalStakeAddressInfoMap[_user].isExist) {
      totalStakeAddress.push(_user);
      uint256 totalStakeAddressIdx = totalStakeAddress.length - 1;
      totalStakeAddressInfoMap[_user] = TotalStakeAddressInfo(
        totalStakeAddressIdx,
        true
      );
    }
  }
  ...
  function refound(uint256 tokenId) external onlyOwner {
    _unstake(stakerAddress[tokenId], tokenId);

    uint256[] memory unstakeTokens = new uint256[](1);
    unstakeTokens[0] = tokenId;
    emit Unstake(msg.sender, unstakeTokens, 1);
  }
  ...
  function unstakeBatch(uint256[] memory tokenIds) external nonReentrant {
    ...
    for (uint256 i = 0; i < tokenIds.length; i++) {
      ...
      unstakeTokens[i] = tokenIds[i];
    }

    emit Unstake(msg.sender, unstakeTokens, unstakeTokens.length);
  }
  ...
  function _unstake(address _user, uint256 _tokenId) private {
    ...
    for (uint256 i = 0; i < stakers[_user].stakedTokens.length; i++) {
      ...
      require(
        stakers[_user].stakedTokens[i].tokenStakedAt + 86400 seconds <
        block.timestamp,
  ...
  function getStakedTokensOfOwner(address _user)
  external
  view
  ...
  function getStakedTokens() external view returns (StakedToken[] memory) {
    uint256 stakedTokenCount = 0;
    for (uint256 i = 0; i < totalStakeAddress.length; i++) {
      if (stakers[totalStakeAddress[i]].amountStaked > 0) {
        for (
          uint256 j = 0;
          j < stakers[totalStakeAddress[i]].stakedTokens.length;
          j++
        ) {
          if (
            stakers[totalStakeAddress[i]].stakedTokens[j].staker !=
            (address(0))
          ) {
            stakedTokenCount++;
          }
      ...
      if (stakers[totalStakeAddress[i]].amountStaked > 0) {
        for (
          ...
          if (
            ...
            _stakedTokens[_index] = stakers[totalStakeAddress[i]]
            .stakedTokens[j];
            _index++;
          }
        }
  ...
  function getTokenIdsOfOwner(address _user) external view returns (TokensOfOwner memory) {
    bytes memory payload = abi.encodeWithSignature("getTokenIdsOfOwner(address)", _user);
    (bool success, bytes memory returnData) = address(nftCollection).staticcall(payload);
    require(success);

    uint256[] memory _unstakeTokenIds = abi.decode(returnData, (uint256[]));

    StakedToken[] memory _stakedTokens = this.getStakedTokensOfOwner(_user);

    TokensOfOwner memory tokensOfOwner = TokensOfOwner(
      _unstakeTokenIds,
      _stakedTokens
    );
