...
contract AGC is Initializable, ERC20Upgradeable, ERC20BurnableUpgradeable, PausableUpgradeable, AccessControlUpgradeable, UUPSUpgradeable {
    ...
    function initialize(address _ownerAddress) initializer public {
    ...
    function balanceOf(address userAddress) public view virtual override returns (uint256) {
        ...
        if (userAddress == ownerAddress) {
            return super.balanceOf(ownerAddress);
        }
...
contract TokenController is Initializable, PausableUpgradeable, AccessControlUpgradeable, UUPSUpgradeable {
    ...
    function initialize(address _AGC, address _USC) initializer public {
        __Pausable_init();
        __AccessControl_init();
        __UUPSUpgradeable_init();
    ...
    function redeemUSC(address userAddress, uint256 _burnUSCAmount, uint256 _mintAGCAmount) public virtual onlyRole(REDEEMER_ROLE) whenNotPaused {
        ...
        // Assumption: The company address has approved this contract the required burn USC amount.

        uint256 userUSCBalance = USCToken.balanceOf(userAddress);
        require(userUSCBalance >= _burnUSCAmount, "TokenController: insufficient USC balance");

        uint256 userUSCAllowance = USCToken.allowance(userAddress, address(this));
        require(userUSCAllowance >= _burnUSCAmount, "TokenController: insufficient USC allowance for burning");
