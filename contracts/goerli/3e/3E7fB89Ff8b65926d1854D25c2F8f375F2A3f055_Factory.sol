...
contract Buffer is Initializable, ReentrancyGuard {
    ...
    function initialize(
        ...
        for (uint256 i = 0; i < _shares.length - 1; i++) {
            //////////
            totalShares[stage] += _shares[i];
            shareDetails[stage][i] = _shares[i];
            shareDetailLength++;
            //////////
        }
        require(totalShares[stage] > 0, "Sum of share percentages must be greater than 0.");
        require(
        ...
        for (uint256 i = 0; i < _partnerShare.length; i++) {
            totalSharesOfPartners[stage] += _partnerShare[i];
            //////////
            partnerShareDetails[stage][i] = _partnerShare[i];
            //////////
        }
    }
    ...
    function updateRoyaltyPercentage(uint256[] calldata _share, uint256[] calldata _partnerShare) external onlyOwner {
        ...
        totalShares[stage] = totalTmp;
        totalSharesOfPartners[stage] = partnersTmp;

        emit UpdateSharesCheck(stage, _share, _partnerShare);
    }
