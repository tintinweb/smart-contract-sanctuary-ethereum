...
contract iMETA_NFT is Initializable, UUPSUpgradeable, OwnableUpgradeable, ERC1155Upgradeable {
    ...
    function initialize(address marketplaceAddress) public initializer {
        _transferOwnership(_msgSender());
        //__ERC1155_init("https://ipfs.io/ipfs/QmYPSJ5rLeviyDuszcZfJ3rxHa7qJdwH7fRSQH2bnq6HdF/");
    ...
    //NFT 민팅
    function mintQuad(uint256 amount, bytes memory data, bytes32 hash) public onlyOwner returns(uint) {
        require(hash != 0, "hash is zero");
        _tokenIds.increment();
        uint256 quadId = _tokenIds.current();
        _mint(msg.sender, quadId, amount, data);
        _metadataHash[quadId] = hash;
    ...
    //hash data change
    function changehashdata(uint256 id, bytes32 hash) public {
        _metadataHash[id] = hash;
    }

    function uri(uint256 quadId) public view virtual override returns (string memory) {
        require(uint256(_metadataHash[quadId]) != 0, "id already used");
        return
            string(
                abi.encodePacked(
                    "https://ipfs.io/ipfs/bafybei",hash2base32(_metadataHash[quadId])//,
    ...
    function delete_NFT(address from, uint _tokenId, uint256 amount) public onlyOwner{
        //require(ownerOf(_tokenId) == msg.sender, "ERC721URIStorage: URI query for nonexistent token");
        uint index;
        ...
        NFTList.pop();
        super._burn(from, _tokenId, amount);
