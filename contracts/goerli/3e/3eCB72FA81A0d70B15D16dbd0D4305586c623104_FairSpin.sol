...
contract FairSpin is Ownable, GroupGame {
    using ECDSA for bytes32;

    constructor(address _vault) GroupGame(_vault) {
        this;
    }
}
...
contract GroupGame is Ownable, BaseGame {
    using ECDSA for bytes32;
    ...
    function newGame(
        ...
        require(gameId.toEthSignedMessageHash().recover(signature) == verifier, "GROUP: Invalid signature");
    ...
    function endGame(
        ...
        require(_hash.toEthSignedMessageHash().recover(signature) == verifier, "GROUP: Invalid signature");
        ...
        vault.setGroupResult(winner, game.total, game.fee, game.players);

        emit GameOver(gameId, 1, winner, results);
    }

    function emergencyEndGame(bytes32 gameId) external virtual override onlyOwner {
        Game memory game = games[gameId];
        require(game.players.length != 0, "GROUP: Invalid game id");
        require(game.winner == address(0x0), "GROUP: It's a finished game");
