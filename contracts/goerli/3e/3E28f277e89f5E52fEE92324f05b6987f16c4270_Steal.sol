...
interface IAnon {
  ...
  function getLevel(uint256 _anonId) external view returns (uint256);
  ...
  AccessControlUpgradeable
{
  ...
  event StealCreated(uint256 indexed _stealId, uint256 indexed _anonId);
  ...
  bool public burnPaymentsEnabled;
  ...
  function toggleStealEnabled() external onlyRole(DEFAULT_ADMIN_ROLE) {
    stealEnabled = !stealEnabled;
  }

  function toggleEthPaymentsEnabled() external onlyRole(DEFAULT_ADMIN_ROLE) {
    ethPaymentsEnabled = !ethPaymentsEnabled;
  }

  function toggleErc20PaymentsEnabled() external onlyRole(DEFAULT_ADMIN_ROLE) {
    erc20PaymentsEnabled = !erc20PaymentsEnabled;
  }

  function toggleBurnPaymentsEnabled() external onlyRole(DEFAULT_ADMIN_ROLE) {
    burnPaymentsEnabled = !burnPaymentsEnabled;
  }
  ...
  function getStealInfo(uint256 _stealId)
    external
    view
    returns (
      uint256,
    ...
    )
  {
    ...
    return (
      steals[totalSupply].anonId,
      steals[totalSupply].level,
      steals[totalSupply].multiplier,
      steals[totalSupply].gift,
      steals[totalSupply].arweaveId
    );
  }
  ...
  function steal(
    ...
    if (IAnon(anon).getLevel(_anonId) != _level) revert LevelMismatch();
    if (!stealEnabled) revert StealDisabled();
    ...
    steals[totalSupply].anonId = _anonId;
    steals[totalSupply].level = _level;
    steals[totalSupply].multiplier = _multiplier;
    steals[totalSupply].gift = _gift;
    steals[totalSupply].arweaveId = _arweaveId;

    _mint(msg.sender, totalSupply);

    emit StealCreated(totalSupply, _anonId);

    IAnon(anon).addSteal(
      _anonId,
      totalSupply,
      keccak256(
        abi.encodePacked(totalSupply, _level, msg.sender, block.timestamp)
      )
    );
    return totalSupply;
  }
  ...
  function mintStealEth(
    ...
    if (!ethPaymentsEnabled) revert EthDisabled();
    if (IERC721Upgradeable(anon).ownerOf(_anonId) == address(this))
      revert AnonBurned();
    if (msg.value < ((priceEth * _multiplier) + _gift))
    ...
    (bool sentTreasury, ) = treasury.call{value: fee / 3}("");
    (bool sentOwner, ) = payable(IERC721Upgradeable(anon).ownerOf(_anonId))
      .call{value: msg.value - (fee / 3)}("");
  ...
  function mintStealErc20(
    ...
    if (!erc20PaymentsEnabled) revert Erc20Disabled();
    if (IERC721Upgradeable(anon).ownerOf(_anonId) == address(this))
      revert AnonBurned();
    if (
      erc20.allowance(msg.sender, address(this)) <
      (priceErc20 * _multiplier) + _gift
    ) revert InsufficientPayment();
    tokenId = steal(
      _anonId,
      _level,
  ...
  function mintStealBurn(
    ...
    if (!burnPaymentsEnabled) revert BurnDisabled();
    if (IERC721Upgradeable(anon).ownerOf(_anonId) != msg.sender)
      revert NotTheOwner();
    if (msg.value < (priceEth * _multiplier) / 3) revert InsufficientPayment();
    tokenId = steal(
      _anonId,
      _level,
      _multiplier,
    ...
    IERC721Upgradeable(anon).transferFrom(msg.sender, address(this), _anonId);
  }
