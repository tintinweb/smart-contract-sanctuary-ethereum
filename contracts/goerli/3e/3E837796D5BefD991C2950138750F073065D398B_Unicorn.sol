...
abstract contract VerifierUpgradeable is Initializable, ContextUpgradeable {
    ...
    function _verify(address verifier_, bytes32 messageHash_, uint256 deadline_, bytes memory signature_) internal view returns (bool) {
    ...
    ReentrancyGuardUpgradeable
{
    ...
    function buy(uint256 typeNFT_, uint256 quantity_, bytes calldata data_) external payable override nonReentrant {
        (Metadata[] memory metadatas_, uint256 deadline_, address verifier_, bytes memory signature_) = _decode(data_);
    ...
    function _buy(address paymentToken_, uint256 paymentAmount_, uint256 typeNFT_, Metadata[] memory metadatas_) internal {
        ...
        emit Registered(buyer, typeNFT_, tokenId, totalMint, _sold[typeNFT_], paymentToken_, paymentAmount_);
    ...
    function _preValidate(bytes calldata data_) internal view returns (bool) {
        (Metadata[] memory metadatas_, uint256 deadline_, address verifier_, bytes memory signature_) = _decode(data_);
        return _verify(verifier_, _getMessageHash(_msgSender(), deadline_, metadatas_), deadline_, signature_);
    }
    ...
    function _decode(bytes calldata data) private pure returns (Metadata[] memory metadatas, uint256 timestamp, address validator, bytes memory sign) {
        (metadatas, timestamp, validator, sign) = abi.decode(data, (Metadata[], uint256, address, bytes));
    }
