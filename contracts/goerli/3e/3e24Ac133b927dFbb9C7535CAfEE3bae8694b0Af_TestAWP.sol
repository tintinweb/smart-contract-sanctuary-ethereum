...
contract TestAWP is Ownable, ERC721A, ReentrancyGuard {
    ...
    function isUserOG(bytes32[] calldata _ogMerkleProof) public view returns (bool) {
        return MerkleProof.verify(_ogMerkleProof, _ogMerkleRoot, toBytes32(msg.sender));
    }

    function isUserWL(bytes32[] calldata _wlMerkleProof) public view returns (bool) {
        return MerkleProof.verify(_wlMerkleProof, _wlMerkleRoot, toBytes32(msg.sender));
    }
    ...
    function ownerMint(uint256 quantity) external payable callerIsUser {
        ...
        require(msg.sender == _vaultAddress || msg.sender == _ownerAddress1, "You're not an owner. Let's not try that again.");

        _safeMint(msg.sender, quantity);
    }

    function OGMint(uint256 quantity, bytes32[] calldata _ogMerkleProof) external payable callerIsUser {
        require(isUserOG(_ogMerkleProof) == true, "You are not an OG member");
        ...
        require(_ogOwnerTransactionCount[msg.sender] < 1, "each address may only perform 1 transaction in this phase of the mint");
        require(quantity > 0);
        require(quantity <= 6, "you cannot mint more than 6 TestAWP in a single transaction");
        require(
            totalSupply() + quantity <= _collectionSize,
            "sold out"
        );
    ...
    function WLMint(uint256 quantity, bytes32[] calldata _wlMerkleProof) external payable callerIsUser {
        require(isUserWL(_wlMerkleProof) == true, "You are not a WL member");
        require(_isMintActive, "mint is not available at this time");
        require(_mintState == MintStateOptions.WL, "WL mint is not available at this time");
        require(_wlOwnerTransactionCount[msg.sender] < 1, "each address may only perform 1 transaction in this phase of the mint");
        require(quantity > 0);
        require(quantity <= 3, "you cannot mint more than 3 TestAWP in a single transaction");
        require(
            totalSupply() + quantity <= _collectionSize,
            "sold out"
        );
    ...
    function PublicMint(uint256 quantity) external payable callerIsUser {
        ...
        require(quantity > 0);
        require(quantity <= 10, "you cannot mint more than 10 TestAWP in a single transaction");
        require(
            totalSupply() + quantity <= _collectionSize,
            "sold out"
        );
