...
contract LOOTaDOG is ERC3525, Ownable {
    ...
    function mint(
        ...
        _verifySign(Voucher(id, slot, amount, expirationTime), v, r, s);
    ...
    //"Voucher(uint256 id,uint256 slot,uint256 amount,uint256 expirationTime)";
    bytes32 private constant VOUCHER_TYPEHASH = 0xd2e65b454b8b95a3da2d5e5e28e970d5db2ce67c08dce424cd4c41c87ef1450f; //keccak256(bytes(VOUCHER_TYPEHASH));
    ...
    function _encodeSellerVoucher(Voucher memory voucher) internal pure returns (bytes32) {
        return
            keccak256(abi.encode(VOUCHER_TYPEHASH, voucher.id, voucher.slot, voucher.amount, voucher.expirationTime));
    }
