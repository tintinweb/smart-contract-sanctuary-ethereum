...
contract AccessPass is ERC721, AccessControl {
    using Counters for Counters.Counter;

    Counters.Counter public tokenIdCounter;
    uint256 public constant MAX_SUPPLY = 450;
    ...
    event MintCompleted(address owner, uint256 tokenId);
    ...
    /**
     * @dev Allows the admin to freeze the set URI function. 
     */
    ...
    /**
     * @dev Allows the admin to set the owner of the smart contract 
     */
    ...
    /**
     * @dev Allows the admin to set the base URI of the stored NFT content.
    ...
    function mintRequirements() internal view {
        require(tokenIdCounter.current() <= MAX_SUPPLY, "Access pass is sold out");
        require(balanceOf(msg.sender) < 1, "You can mint only one Access Pass");
    }
    ...
    /**
     * @dev Sets the readyToMint variable to true;
     */
    function setReadyToMint() external onlyRole(DEFAULT_ADMIN_ROLE) {
        readyToMint = true;
    }

    /**
     * @dev Mints an NFT if readyToMint is set to true.
     * @notice function name is required by Immutable X.
     */
    function mintFor() public {
        readyToMint ? _mintNft() : revert("Not ready to mint");
    }
