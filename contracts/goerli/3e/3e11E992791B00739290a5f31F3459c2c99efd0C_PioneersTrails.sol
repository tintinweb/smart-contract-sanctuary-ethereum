...
contract PioneersTrails is
    VRFConsumerBaseV2,
    KeeperCompatibleInterface,
    Ownable
{
    uint256 constant INCONVENIENCE_BLOCK_INTERVAL = 5;
    ...
    uint32 private constant NUM_WORDS = 24;
    ...
    constructor(
        ...
        /* Add a consumer to a chainlink subscription */
    ...
    function getTrailIdByStage(
        uint8 _baseTrailId,
        bool debug
    ) public view returns (uint8 _updatedTrailId) {
    ...
    function getEncodedInconveniencesByTrail(
        uint8 _baseTrailId
    ) public view returns (bytes12) {
        return
            trailToEncodedInconveniences[
                getTrailIdByStage(_baseTrailId, false)
            ];
    }

    /** Decode bytes-written inconveniences into array of string (single-char). */
    function getDecodedInconveniencesByTrail(
        uint8 _baseTrailId
    )
        public
        view
        returns (string[] memory _decodedInconveniences, uint256 _size)
    {
        string[] memory inconveniences = new string[](12);

        uint8 updatedTrailId = getTrailIdByStage(_baseTrailId, false);

        bytes12 inconveniencesBytes = trailToEncodedInconveniences[
            updatedTrailId
        ];

        if (inconveniencesBytes.length == 0) revert PIONEERS_EmptyTrail();
    ...
    function getCurrentTrailInconvenience(
        uint8 _baseTrailId,
        int256 _currentHash
    ) public view returns (string memory _inconvenience) {
        string[] memory allTrailInconveniences;
        (allTrailInconveniences, ) = getDecodedInconveniencesByTrail(
            _baseTrailId
        );

        int256 validLength = 0;

        for (
            uint256 i = 0;
            i <= allTrailInconveniences.length &&
                bytes(allTrailInconveniences[i]).length != 0;
            i++
        ) validLength++;

        return allTrailInconveniences[uint256(_currentHash % validLength)];
    }

    /** Debug function for pseudo-random number. */
    function getRandomNumber(
        bool _isDebug
    ) public view returns (uint256 _rand) {
        if (!_isDebug) {
            uint256 eliminationBlock = block.number -
                (block.number % INCONVENIENCE_BLOCK_INTERVAL) +
                1;
            return
                uint256(blockhash(eliminationBlock)) %
    ...
    function removeByteFromBytes16(
        bytes16 data,
        uint256 index
    ...
    function generateInconveniencesForNextTrails() public onlyOwner {
        // pidyjmcwhxbrq
        ...
        if (_nextStage == IPioneers.Stage.Journey1) {
            ...
            for (uint256 i = 7; i <= 14; i++) {
                ...
                uint256 removedInconveniences = 0;
                for (uint256 j = 0; j < 3; j++) {
                    uint256 rand = s_randomWords[k] %
                        (13 - removedInconveniences);
                    temp[j] = availableInconveniences[rand];
                    removedInconveniences++;
                    availableInconveniences = removeByteFromBytes16(
                        availableInconveniences,
                        rand
                    );
                    k++;
                }
                trailToEncodedInconveniences[uint8(i)] = bytes12(
                    abi.encodePacked(temp[0], temp[1], temp[2])
                );
            }
            ...
            for (uint256 i = 3; i <= 6; i++) {
                ...
                for (uint256 j = 0; j < 6; j++) {
                    uint256 rand = getRandomNumber(true) %
                        (13 - removedInconveniences);
                    temp[j] = availableInconveniences[rand];
                    removedInconveniences++;
                    availableInconveniences = removeByteFromBytes16(
    ...
    function currentInconveniences(
        uint8 _baseTrailId
    )
        public
        view
        ...
        )
    {
        ...
        uint256 eliminationBlock = block.number -
            ...
            1;
        int256 hash = int256(
            uint256(blockhash(eliminationBlock)) % uint256(type(int256).max)
        );
        // int hash = int(getRandomNumber(true));

        uint256 rand = uint256(hash) % 25;
        string memory pickedLetter = string(
            abi.encodePacked(alphabet[rand])
        );

        uint256 size;
        string[] memory inconveniences;
        (inconveniences, size) = getDecodedInconveniencesByTrail(_baseTrailId);
        rand = uint256(hash) % size;
        ...
        returns (bool upkeepNeeded, bytes memory /* performData */)
    {
        IPioneers.Stage _nextStage = IPioneers(pioneers).stage();
        require(
            _nextStage != IPioneers.Stage.Initial &&
                _nextStage != IPioneers.Stage.Minting
        );
    ...
    function performUpkeep(bytes calldata /* performData */) external override {
        ...
        if (!upkeepNeeded) revert PIONEERS_NoUpkeepNeeded();
        else {
    ...
    function addSubscriptionConsumer() private onlyOwner {
    ...
    function setRandomWords(uint256[] memory _randomWords) public onlyOwner {
    ...
    //== End Random Numbers Implementation
}
...
interface IPioneers {
    ...
    function wagonToLetter(uint16) external view returns(bytes1);
