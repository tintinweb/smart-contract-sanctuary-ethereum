...
{
    constructor() ERC721A("ChillOrb", "CHILLORB") {}
    ...
    function tokenPreviewURI(uint256 tokenId) private view returns (string memory) {
        return string.concat(previewBaseURI, isColored(tokenId) ? "/colorful/" : "/blackAndWhite/", uintToString(tokenId), ".jpg");
    }
    ...
    function generateHTML(uint256 tokenId) public view returns (string memory) {
        ...
        string memory withTokenId = replaceInString(tokenHtmlSourceCode, "%TOREPLACE.TOKENID%", uintToString(tokenId));
        string memory coloredString = isColored(tokenId) ? "true" : "false";

        return replaceInString(withTokenId, "%TOREPLACE.ISCOLORED%", coloredString);
    }

    function orbBase64(uint256 tokenId) public view returns (string memory) {
    ...
    function generateJSON(uint256 tokenId) internal view returns (string memory) {
        ...
        json = replaceInString(json, "%TOKENID%", uintToString(tokenId));
        json = replaceInString(json, "%PREVIEW%", tokenPreviewURI(tokenId));
        json = replaceInString(json, "%ORBURI%", generateOrbURI(tokenId));
        json = replaceInString(json, "%ORBURI%", generateOrbURI(tokenId));
        json = replaceInString(json, "%COLORFUL%", isColored(tokenId) ? "true" : "false");
        return json;
    }
