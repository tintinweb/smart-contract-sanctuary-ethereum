...
enum OrderExecutionResult {
    /// @notice order fulfilled (no partial filled), should process nextstep
    FILLED,
    /// @notice order should be removed from
    EXPIRED,
    /// @notice not filled, wait to next round
    SKIP
}
...
/// Return result tell the orderbook order should be removed or not
/// If true, order book should force close this order
interface IModule {
    function execute(uint256 indexPrice, Order memory order) external returns (OrderExecutionResult);
...
contract MarketOrderModule is IModule {
    ...
    constructor(uint256 _maxOrderTimeout) {
    ...
    function execute(uint256 indexPrice, Order memory order) external view returns(OrderExecutionResult) {
        uint256 acceptablePrice = abi.decode(order.data, (uint256));
        require(indexPrice > 0, "LimitOrderModule: invalid mark price");

        if (order.submissionTimestamp + maxOrderTimeout < block.timestamp) {
            return OrderExecutionResult.EXPIRED;
        }

        // check if the price is match, if it's not, remove the order as well
        if (order.side == Side.LONG && order.orderType == OrderType.INCREASE) {
            return indexPrice <= acceptablePrice ? OrderExecutionResult.FILLED : OrderExecutionResult.EXPIRED;
        } else {
            return indexPrice >= acceptablePrice ? OrderExecutionResult.FILLED : OrderExecutionResult.EXPIRED;
        }
    }
