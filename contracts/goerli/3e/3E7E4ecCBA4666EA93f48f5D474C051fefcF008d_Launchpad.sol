...
contract Launchpad is Ownable, ReentrancyGuard {
    ...
    event AddCampaign(address contractAddress, address payeeAddress, uint256 price, uint256 maxSupply,
                        uint256 listingTime, uint256 expirationTime, uint256 maxBatch, uint256 maxPerAddress, string collectionURI);
    ...
    function mint(address contractAddress, uint256 batchSize) external payable nonReentrant {
        ...
        require(_campaigns[contractAddress].contractAddress != address(0), "contract not register"); //This line seems sus, might need to check

        // activity check
        Campaign memory campaign = _campaigns[contractAddress];
        require(batchSize <= campaign.maxBatch, "reach max batch size");
        //sus comparison, commented and added more correct code
    ...
    function addCampaign(address contractAddress_, address payeeAddress_, uint256 price_, uint256 listingTime_,
        uint256 expirationDays_, uint256 maxBatch_, uint256 maxPerAddress_, string memory collectionURI_) external onlyOwner {
        ...
        require(block.timestamp < listingTime_, "cannot set listing time earlier than current time");
        //sus code, replaced witha proper one, need to do some test
        //require(_campaigns[contractAddress_].contractAddress == contractAddress_, "campaign already initialized!");
        require(_campaigns[contractAddress_].contractAddress != contractAddress_, "campaign already initialized!");
        require(payeeAddress_ != address(0), "payee address can not be empty"); //owner of the nft contract 
        ...
        require(maxBatch_ <= maxSupply_, "max batch cannot be greater than the max supply!");
        uint256 numberOfdays;
        numberOfdays = (expirationDays_ * 1 days);
        uint256 expirationTime_ =  listingTime_ + numberOfdays;

        emit AddCampaign(contractAddress_, payeeAddress_, price_, maxSupply_, listingTime_,
            expirationTime_, maxBatch_, maxPerAddress_, collectionURI_);
        _campaigns[contractAddress_] = Campaign(contractAddress_, payeeAddress_, price_, maxSupply_, listingTime_,
                                                expirationTime_, maxBatch_, maxPerAddress_, collectionURI_);
    }

    function updateCampaign(address contractAddress_, address payeeAddress_, uint256 price_, uint256 listingTime_,
        uint256 expirationDays_, uint256 maxBatch_, uint256 maxPerAddress_, string memory collectionURI_) external onlyOwner {
        require(contractAddress_ != address(0), "contract address can not be empty");
