...
/// @title ThrllUsernames ERC-721 token
    ...
    AccessControlEnumerable
{
    ...
    /**
    @notice Role of administrative users allowed to expel a ThrllUsernames from the
    rack.
    ...
    function isValidUsername(string memory str) public pure returns (bool){
    ...
    function addUsername(string memory _string) public {
        require(isValidUsername(_string));
        require(!isUnique(_string));
        usernames[_string] = true;
    }

    function isUnique(string memory _string) public view returns (bool) {
        require(isValidUsername(_string));
        return usernames[_string];
    }
     ...
     */
    function mintPublic(address to, string memory username)
        external
        payable
        inMintPhase(MintPhase.PUBLIC_SALE)
    {
        addUsername(username);
        nfts[_nextTokenId()] = username; 
        _purchase(to, 1);
    }
    ...
    /**
    @notice Returns the length of time, in seconds, that the ThrllUsernames has
    racked.
    @dev Racking is tied to a specific ThrllUsernames, not to the owner, so it doesn't
    reset upon sale.
    @return racking Whether the ThrllUsernames is currently racking. MAY be true with
    ...
    @return total Total period of time for which the ThrllUsernames has racked across
    ...
    /**
    @notice Transfer a token between addresses while the ThrllUsernames is minting,
    ...
    function _beforeTokenTransfers(
        ...
        for (uint256 end = tokenId + quantity; tokenId < end; ++tokenId) {
            require(
                ...
                "ThrllUsernames: racking"
            );
        }
    }

    /**
    @dev Emitted when a ThrllUsernames begins racking.
     */
    event Racked(uint256 indexed tokenId); // Racked

    /**
    @dev Emitted when a ThrllUsernames stops racking; either through standard means or
    by expulsion.
     */
    event Unracked(uint256 indexed tokenId); // Unracked

    /**
    @dev Emitted when a ThrllUsernames is expelled.
     */
    ...
    /**
    @notice Changes the ThrllUsernames's racking status.
    */
        ...
        onlyApprovedOrOwner(tokenId)
    {
        uint256 start = rackingStarted[tokenId];
        if (start == 0) {
            require(rackingOpen, "ThrllUsernames: racking closed");
    ...
    /**
    @notice Changes multiple ThrllUsernames's racking status
     */
    ...
    /**
    @notice Admin-only ability to expel a ThrllUsernames from the rack.
    ...
    function expelFromRack(uint256 tokenId) external onlyRole(EXPULSION_ROLE) {
        require(rackingStarted[tokenId] != 0, "ThrllUsernames: not racked");
    ...
    function burn(uint256 tokenId) public virtual onlyApprovedOrOwner(tokenId) {
        require(burningOpen, "ThrllUsernames: burning not enabled");
        _burn(tokenId);
    }
