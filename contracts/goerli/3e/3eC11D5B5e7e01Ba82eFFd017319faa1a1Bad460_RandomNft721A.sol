...
contract RandomNft721A is Token721A, VRFConsumerBaseV2Upgradeable {
    ...
    bool private allowlisted;
    uint256 private publicTokenSold;
    ...
    */
    ...
    function initialize(uint64 _s_subscriptionId, address _vrfCoordinator) public initializerERC721A initializer {
        feeAddress = 0x26BA546b581f859BFeE6821958097E8bA1C24444;
        ...
        __ERC721A_init("RandomToken721A", "721A");
    ...
    function whitelistBuyToken(uint256 quantity) private whenMintable {
        ...
        require(whitelists.maxWhiteListSupply < 100, "Maximum whitelist supply reached");
    ...
    function publicBuyToken(uint256 quantity) private whenMintable {
        require(publicTokenSold < maxPublicSupply, "Maximum supply reached");
        price = publicPrice * quantity;
        require(msg.value == price, "Not enough eth sent");
        for(uint i = 0; i < quantity; i++) {
            addresses.push(_msgSender());
        }
        publicTokenSold += quantity;
        payable(feeAddress).transfer(price);
    }
    ...
    function mint() public adminOnly returns (uint256 requestId) {
        ...
        uint256 mintLength = addresses.length;
        require(mintLength > 0, "No address need to mint");
        for (uint i = 0; i < mintLength; i++) {
            uint256 randomIndex = requestId % (addresses.length);
            address resultNumber = addresses[randomIndex];
            addresses[randomIndex] = addresses[addresses.length - 1];
            addresses.pop();
    ...
    function setPublicMint(uint256 _tokenPrice, uint _maxSupply) public adminOnly {
        publicPrice = _tokenPrice;
        maxPublicSupply = _maxSupply;
        allowlisted = false;
    }

    function setPublicTokenPrice(uint256 _newPublicPrice) public adminOnly {
        publicPrice = _newPublicPrice;
    }

    function setPublicMaxSupply(uint256 _newPublicSupply) public adminOnly {
        maxPublicSupply = _newPublicSupply;
    }
...
contract Token721A is ERC721AUpgradeable, AccessControlUpgradeable {
