...
contract mfMain is Ownable, Pausable, ReentrancyGuard {
    ...
    function calculateLockingRewards(uint256 tokenId) public view returns(uint256 rewards) {
        ...
        if(lockDuration >= MINIMUM_DAYS_TO_BONUS){
        ...
        if(TIER_EMISSIONS_STARTED){
            if(_tier1Tokens[tokenId]) 
                return rewards += lockDuration * DAILY_TIER1_FEWL_RATE / 1 days - myStake.claimedAmount;
            if(_tier2Tokens[tokenId]) 
                return rewards += lockDuration * DAILY_TIER2_FEWL_RATE / 1 days - myStake.claimedAmount;       
        } 

        //if tier emissions have not started all nfts get base rate
        return rewards += lockDuration * DAILY_BASE_FEWL_RATE / 1 days - myStake.claimedAmount;        
