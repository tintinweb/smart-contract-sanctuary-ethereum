...
contract MinerBondingCalculator is IBondingCalculator {
    using FixedPoint for *;
    using SafeMath for uint256;

    IERC20 internal immutable MINER;
    ...
    function markdown(address _pair) external view override returns (uint256) {
        ...
        uint256 reserve;
        if (IUniswapV2Pair(_pair).token0() == address(MINER)) {
            reserve = reserve1;
        } else {
            require(IUniswapV2Pair(_pair).token1() == address(MINER), "Invalid pair");
            reserve = reserve0;
        }
        return reserve.mul(2 * (10**IERC20Metadata(address(MINER)).decimals())).div(getTotalValue(_pair));
    }
}
