 ...
 */
contract ERC721BridgeToken is ERC721, Ownable, IBurnableMintableERC721Token {
    ...
    modifier onlyMediator() {
        require(msg.sender == mediatorContract, "Only mediator contract has access.");
        _;
    }
    ...
    /**
     * @dev Mint new ERC721 tokens.
     * Only bridge contract is authorized to mint tokens.
     * @param _to address of the newly created token owner.
     * @param _tokenIDs an array with unique identifiers of the minted tokens.
     */
    function mint(address _to, uint256[] calldata _tokenIDs) external override onlyMediator {
    ...
    /**
     * @dev Burns some ERC721 tokens.
     * Only bridge contract is authorized to burn tokens.
     * @param _tokenIDs an array with unique identifiers of the burned tokens.
     */
    function burn(uint256[] calldata _tokenIDs) external override onlyMediator {
        for (uint256 i = 0; i < _tokenIDs.length; i++) {
            _burn(_tokenIDs[i]);
        }
    }
