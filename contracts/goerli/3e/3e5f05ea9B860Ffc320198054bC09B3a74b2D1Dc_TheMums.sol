...
interface InterfaceQueen {
    function transferTokens(address _from, address _to) external;
} 

contract TheMums is ERC721A, Ownable {

    InterfaceQueen public Queen;
    ...
    mapping (address => uint256) public mumBalance;
    ...
    function setQueen(address _queen) external onlyOwner { 
         Queen = InterfaceQueen(_queen); 
    }

    function MumMint(uint256 num) public payable  notContract {
        uint256 supply = totalSupply(); 
        require( !_paused,                      "Minting paused" );
        require( num < 31,                      "You can mint a maximum of 30 Mums" );
        require( supply + num < 5001,          "Exceeds maximum supply" );
        require( msg.value >= _price * num,     "Eth sent is not correct" ); 
        
        _safeMint(msg.sender, num);
        mumBalance[msg.sender] += num; 
        
    }

    function giveAway(address _to, uint256 _amount) external onlyOwner() {
        uint256 supply = totalSupply(); 
        require( supply + _amount < 5001,  "Exceeds maximum Mum supply" );
         
        _safeMint(_to, _amount);
        mumBalance[_to] += _amount; 
    ...
    function MumtoMummy(uint256 tokenId) external {
        _burn(tokenId, true); 
    }  
