...
contract ApiensGen2 is Allowable {    
    ...
    uint256 ALLOW_PRICE = (1 ether)*.024206;
    ...
    /// @param stakedApien uint256 id of staked Apien
    /// @param genesisBag uint256 id of Genesis Bag
    /// @param signature bytes trusted signature 
    function freeClaimMint(uint256 stakedApien, uint256 genesisBag, bytes calldata signature) 
    external requiresFreeSig(signature,msg.sender,stakedApien,genesisBag) {        
        
        if (hasBeenClaimed(genesisBag, GENESIS_BAG)) {
            revert BagAlreadyClaimed(genesisBag);
        }
        
        claim(genesisBag, GENESIS_BAG);
        
        if (hasBeenClaimed(stakedApien, APIENS)) {
            revert ApienAlreadyClaimed(stakedApien);
        }
        
        claim(stakedApien, APIENS);

        phasedMint(CLAIM_PHASE, 1, true);        
    } 

    /// @notice minting ALLOW_PHASE, requires trusted signature and GenesisBag
    /// @param genesisBag uint256 id of Genesis Bag
    /// @param signature bytes trusted signature 
    function bagAllowMint(uint256 genesisBag, bytes calldata signature) 
    external payable requiresBagSig(signature,msg.sender,genesisBag) {   
        ...
        isValidPayment(phased.cost, 1);
        
        claim(genesisBag, GENESIS_BAG);

        phasedMint(ALLOW_PHASE, 1, true);        
        
        setAux(msg.sender, getAux(msg.sender)+1);
    }     
...
abstract contract EIP712Allowlisting is EIP712Listable {
    ...
    function recoverBagAddress(bytes calldata sig, address recip, uint256 bag) public view returns (recoveredBag memory) {    
        require(sigKey != address(0), "allowlist not enabled");
        string memory bagged = (uint(uint160(recip))+bag).toString();
        bytes32 digest = keccak256(
            abi.encodePacked(
                "\x19\x01",
                DOM_SEP,
                keccak256(abi.encode(BAG_MINT_TYPE,keccak256(abi.encodePacked(bagged))))
            )
        );
        address recovery = digest.recover(sig);
        return recoveredBag(recip, sig, recovery, sigKey, bagged);
    }
    modifier requiresBagSig(bytes calldata sig, address recip, uint256 bag) {
        require(sigKey != address(0), "allowlist not enabled");
        string memory bagged = (uint(uint160(recip))+bag).toString();
        bytes32 digest = keccak256(
            abi.encodePacked(
    ...
    function recoverFreeAddress(bytes calldata sig, address recip, uint256 bag, uint256 staked) public view returns (recoveredBag memory) {
        require(sigKey != address(0), "allowlist not enabled");
        string memory bagged = (uint(uint160(recip))+bag+staked).toString();
        bytes32 digest = keccak256(
            abi.encodePacked(
                "\x19\x01",
                DOM_SEP,
    ...
    modifier requiresFreeSig(bytes calldata sig, address recip, uint256 bag, uint256 staked) {
        require(sigKey != address(0), "allowlist not enabled");
        string memory bagged = (uint(uint160(recip))+bag+staked).toString();
        bytes32 digest = keccak256(
            abi.encodePacked(
