...
////////////////////////////////////////////////////////////////////////////////////////////////////////
contract TheAIArtGamePhrases is Ownable, ERC2981, ERC721A, ReentrancyGuard {
    //apple string
    TheAIArtGame ai;
    ...
    mapping(uint256 => PhraseInfo) internal phraseInfo;
    ...
    constructor(address _aiAddress)
        ERC721A("On chain AI NFT Game.", "AI_ART_GAME_PHRASES")
    {
    ...
    function grabTokenPhrase(uint256 tokenId)
        public
        view
        returns (string[] memory)
    {
        return phraseInfo[tokenId].words;
    }
    ...
    function combineWords(uint256[] memory tokenIds)
        public
        callerIsSender
        nonReentrant
    {
        require(tokenIds.length >= 4, "Not enough words.");
        string[] memory words;
        for (uint256 index = 0; index < tokenIds.length; index++) {
            require(
                ai.ownerOf(tokenIds[index]) == msg.sender,
                "Must be owner of all words."
            );
            string memory word = ai.grabWord(tokenIds[index]);
            words[index] = word;
            ai.burnToken(tokenIds[index]);
        }
        phraseInfo[_currentIndex].words = words;
        ...
        returns (string memory)
    {
        ...
        string[3] memory metadataVars = [
            string(unhideWord ? "erg" : "Waiting on reveal..."),
            "svgwebsitehre",
            Strings.toString(tokenId)
        ];
        bool freePhrase = generateSeed(tokenId) % 1337 == 420 ||
            generateSeed(tokenId) % 1337 == 69;
        bool freeProtection = generateSeed(tokenId) % 500 == 420 ||
            generateSeed(tokenId) % 1337 == 69;
