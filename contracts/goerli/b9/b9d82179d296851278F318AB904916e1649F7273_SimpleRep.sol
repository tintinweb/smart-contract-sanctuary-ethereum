...
contract SimpleRep {
    ...
    /// @param _domain The reputation domain
    /// @param _refillAmount The amount of the rating token to give to the creator
    function refillDomainCreator(
        uint256 _domain,
        uint256 _refillAmount
    ) external payable {
        Domain memory domain = domains[_domain];
        require(
            msg.sender == domain.creator,
            "Only the creator can refill the domain"
        );
        Actor memory creator = actors[_domain][domain.creator];
        require(
            creator.ratingBalance + _refillAmount <= domain.ratingTokenSupply,
            "Cannot refill more than the total supply"
        );
        actors[_domain][domain.creator].ratingBalance += _refillAmount;
        emit CreatorRefilled(_domain, domain.creator, _refillAmount);
    }
    ...
    function getSelf(
        ...
        return getActor(_domainId, msg.sender);
    }
}
