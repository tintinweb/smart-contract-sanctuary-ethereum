...
contract HybridHumans is ERC721A, Ownable {
    ...
    uint256 public constant MAX_PER_WHITELIST_WALLET = 10;

    bytes32 private merkleRoot;

    bool public publicSaleStarted;

    mapping(address => bool) private whaleMinter;
    ...
    function mintWhitelist(bytes32[] calldata _proof, uint256 amount) external payable {
        if (!whaleMinter[msg.sender] && _numberMinted(msg.sender) + amount > 5) revert ExceedMaxMint();

        uint256 price = amount > 5 ? BULK_MINT_PRICE : MINT_PRICE;
    ...
    function mint(uint256 amount) external payable {
        ...
        if (!whaleMinter[msg.sender] && _numberMinted(msg.sender) + amount > 10) revert ExceedMaxMint();
