...
contract VaultFactory is IVaultFactory {
    ...
    uint public override fee = 25e16; // 25%
    ...
    event VaultCreated(address indexed user, address vault, uint id);
    ...
    function createVault(address _owner) external override returns (address) {
        require(getVault[_owner] == address(0), 'exits');
    ...
    function setFee(uint _fee) external override {
        require(msg.sender == feeToSetter, '!setter');
        require(_fee > 0 && _fee <= 50e16, "fee needs to be > 0 and < 50%");
        fee = _fee;
    }
...
contract Vault is IVault, Ownable {
    ...
    event Claim(uint feeAmount, uint yieldAmount);
    ...
    function pendingYield() public view override returns (uint) {
        return factory.collateral().balanceOf(address(this)) - minted;
    }

    function mint(uint _amount) external override {
        _claim();
        // Avoid potential transfer rounding errors with tokens such as stETH
        ...
        factory.token().mint(msg.sender, actualAmount);
        minted += actualAmount;
        emit Mint(actualAmount);
    }

    function redeem(uint _amount) external override {
        _claim();
        factory.collateral().transfer(owner(), _amount);
        factory.token().burn(msg.sender, _amount);
        minted -= _amount;
        emit Redeem(_amount);
    }
    ...
    function _claim() internal {
        ...
        if (yield > 0) {
            uint fee = yield * factory.fee() / factory.PRECISION();
            yield -= fee;
            factory.collateral().transfer(factory.feeTo(), fee);
            factory.collateral().transfer(owner(), yield);
            emit Claim(fee, yield);
        }
    }
}
