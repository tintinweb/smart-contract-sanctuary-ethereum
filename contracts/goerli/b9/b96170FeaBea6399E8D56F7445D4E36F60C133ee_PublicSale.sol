...
import "@openzeppelin/contracts-upgradeable/token/ERC20/IERC20Upgradeable.sol";//MPRTKN Token
    ...
    UUPSUpgradeable
{
    ...
    // Maximo price NFT
    uint256 constant MAX_PRICE_NFT = 50000;// * 10 ** 18;//En unidades, cálculo de precio considera decimales
    ...
    function purchaseNftById(uint256 _id) external {
        ...
        require(!isSoldToken[_id-1], "Public Sale: id not available");
    ...
    function depositEthForARandomNft() public payable {
        ...
        require(msg.value >= 0.01 ether, "Ether Insuficiente. Debe ser al menos 0.01");

        // 2 - que haya NFTs disponibles para hacer el random
        require(soldTokens < 30, "No quedan Tokens NFT disponibles");

        // Escgoer una id random de la lista de ids disponibles
        uint256 nftId = _getRandomNftId();// 1..30
        uint256 prevNftId = (nftId + 29) % 30;//1..30
        while (isSoldToken[nftId-1] || nftId==prevNftId) // Itera hasta encontrar Token libre a partir del random o sea el anterior al random para prevenir loop infinito
        {
            nftId = (nftId + 1) % 30;
        }
        require(!isSoldToken[nftId-1], "Public Sale: id not available"); //Just double check for attack prevention
    ...
    function getRandomNftId() public view returns (uint256) {
        return _getRandomNftId();
    }
    ...
    function _getPriceById(uint256 _id) internal view returns (uint256) {
        ...
        if (_id > 0 && _id < 11) {
            ...
            nftPrice = (10000 + _id * 1000);
        } else {
            nftPrice = Math.min(MAX_PRICE_NFT,10000 + 1000 * (block.timestamp - startDate) / 3600);
        }
