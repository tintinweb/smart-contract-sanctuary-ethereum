...
import {ONE_DAY} from "./Constants.sol";
import "./Errors.sol";

/**
 * @title HomeFiRegistry Contract for HomeFi v0.2.5

 * @dev This contract provided functionality to update the core HomeFi contracts.
 */
contract HomeFiRegistry is Initializable {
    error NameInUse();
    /*******************************************************************************
     * -------------------------PUBLIC STORED PROPERTIES-------------------------- *
     *******************************************************************************/

    /// @notice Owner of HomeFiRegistry
    address public owner;

    /// @notice address of HomeFiRegistry Proposed Owner
    address public proposedOwner;

    /// @notice timestamp of Last HomeFiRegistry Owner Replacement Proposal
    uint256 public ownerProposalTimestamp;

    /// @notice Address of proxy admin
    ProxyAdmin public proxyAdmin;

    /// @notice bytes2 array of upgradable contract initials
    ...
    /// @dev mapping that maps contract initials with their implementation address
    ...
    event OwnerReplaceProposed(address _newOwner);
    ...
    /// @dev check if new address is same as old
     ...
     * @dev Can only be called by HomeFiRegistry owner.
     ...
     */
    function initiateHomeFi(address _owner, address[] calldata _implementations)
        external
        payable
        initializer
    {
        ...
        if (_length != _implementations.length) {
        ...
        onlyOwner
    {
        ...
        if (contractAddress[_contractName] != address(0)) {
            revert NameInUse();
        }
    ...
    function upgradeMultipleImplementations(
        ...
        if (_length != _contractAddresses.length) {
            revert LengthMismatch();
        }
    ...
    /**
     * @notice Allows HomeFiRegistry owner to change the owner of proxyAdmin contract.
     * This can be useful when trying to deploy new version of HomeFiRegistry

     * @dev modifier onlyOwner
     * @dev modifier nonZero with _newAdmin

     * @param _newAdmin address of new proxyAdmin owner / new version of HomeFiRegistry
     */
    ...
    /**
     * @notice Propose the replacement of the current Owner

     * @dev modifier onlyOwner
     * @dev modifier nonZero with _newOwner
     * @dev modifier noChange with `owner` and `_newOwner`

     * @param _newOwner address - new owner address
     */
    function proposeOwnerReplace(address _newOwner)
        external
        payable
        onlyOwner
        nonZero(_newOwner)
        noChange(proposedOwner, _newOwner)
    {
        // Propose Owner replace
        proposedOwner = _newOwner;
        ownerProposalTimestamp = block.timestamp;

        emit OwnerReplaceProposed(_newOwner);
    }

    /**
     * @notice Execute the proposed replacement of the current owner
     */
    function executeOwnerReplace()
        external
        payable
        nonZero(proposedOwner)
        noChange(proposedOwner, owner)
    {
        if (proposedOwner != msg.sender) {
            revert SignerIsNotTheProposedAdmin();
        }
        if (block.timestamp < ownerProposalTimestamp + ONE_DAY) {
            revert TooSoon();
        }
        // Replace owner
        owner = proposedOwner;
        ...
        delete ownerProposalTimestamp;

        emit OwnerReplaced(owner);
    }
