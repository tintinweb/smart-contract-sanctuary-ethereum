...
contract NounCreepz is ERC721LazyMint {
    ERC1155Base public immutable eggs;
    ...
    {
        // require(
        //     eggs.balanceOf(_claimer, 0) >= _quantity,
        //     "You don't own enough eggs"
        // );

        address[] memory eggBalanceAddresses = new address[](6);
        ...
        eggBalanceAddresses[5] = _claimer;

        uint256[] memory eggBalanceIds = new uint256[](6);
        eggBalanceIds[0] = 1;
        eggBalanceIds[1] = 2;
        eggBalanceIds[2] = 3;
        eggBalanceIds[3] = 4;
    ...
    function _transferTokensOnClaim(address _receiver, uint256 _quantity)
        ...
        returns (uint256)
    {
        // eggs.burn(_receiver, 0, _quantity);
        ...
        eggBalanceAddresses[5] = _receiver;
        ...
        uint256[] memory eggBurnQuantities = new uint256[](6);
        ...
        for (uint256 i = 0; i < 6; i++) {
            eggBurnQuantities[i] = eggBalances[i] >= eggSacrificesRemaining ? eggSacrificesRemaining : eggBalances[i];
            eggSacrificesRemaining = eggSacrificesRemaining - eggBurnQuantities[i];
        }

        eggs.burnBatch(_receiver, eggBalanceIds, eggBurnQuantities);
