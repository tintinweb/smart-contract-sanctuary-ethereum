...
contract Wearables is Ownable, ERC721IWIN, ReentrancyGuard {
    ...
    function publicSaleMint(uint256 quantity, uint256 item) external payable callerIsUser {
        ...
        require(itemsCount[i] + quantity <= collectionSize[i], "reached max supply");
...
contract ERC721IWIN is Context, ERC165, IERC721, IERC721Metadata, IERC721Enumerable {
    ...
    uint256[] internal collectionSize = [1000, 700, 600, 500, 50, 9]; //item 1-> 6 
    uint256[] internal itemsCount = [0, 0, 0, 0, 0, 0];
    ...
    function _setOwnersExplicit(uint256 quantity, uint256 item) internal {
        ...
        if (endIndex > collectionSize[idx] - 1) {
            endIndex = collectionSize[idx] - 1;
        }
