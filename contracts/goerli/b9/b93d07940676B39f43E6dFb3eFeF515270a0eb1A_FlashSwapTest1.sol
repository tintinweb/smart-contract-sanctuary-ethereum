...
contract FlashSwapTest1 is IUniswapV2Callee {
    ...
    function approveToken(address _token, address _toApprove, uint _quantity) public returns (uint amountApproved){
        ...
        amountApproved = token.allowance(address(this), _toApprove);
    }
    ...
    function uniswapV2Call(address _sender, uint _amount0, uint _amount1, bytes calldata _data) override external {
        ...
        // approve the sushiSwapRouter to spend our tokens so the trade can occur             
        token.approve(address(msg.sender), amountTokenBorrowed);
        ...
        uint amountReceived = _swapExactTokensForTokens(amountTokenBorrowed, amountRequired, pathA, address(this), block.timestamp)[1]; 
