...
contract ERC1155Openzeppelin is ERC1155, Ownable, Loyalty {
    ...
    string public baseUri = "https://bsc-api.bullz.com/api/nfts/item/";
    ILoyalty private loyalty;
    constructor(string memory _name, string memory _symbol)  ERC1155("https://eth-api.bullz.com/api/nfts/item/") {
    ...
    function awardItem(uint256 newItemId, uint256 amount, bytes memory data, uint256 loyaltyPercent, bool isResaleAllowed) public returns (uint256) {
        _mint(_msgSender(), newItemId, amount, data);
        addLoyalty(address(this), newItemId, _msgSender(), loyaltyPercent, isResaleAllowed);
        return newItemId;
    }
    function setURI(string memory newUri) onlyOwner() public returns (string memory){
        _setURI(newUri);
        baseUri = newUri;
...
contract Loyalty is ILoyalty {
    ...
    mapping(address => mapping(uint256 => bool)) public resaleProhibited;
    ...
    function addLoyalty(
        ...
        resaleProhibited[collection][assetId] = !resale_allowed;
    ...
    function isResaleAllowed(address collection, uint256 assetId)
        ...
        returns (bool)
    {
        return !_isResaleProhibited(collection, assetId);
    }

    function _isResaleProhibited(address collection, uint256 assetId)
        internal
        view
        returns (bool)
    {
        return resaleProhibited[collection][assetId];
    }
    ...
    ) override external view returns (address creator, uint256 creatorBenif) {     

        if (_isInLoyalty(collection, assetId)) {         
            creator = _getLoyaltyCreator(collection, assetId);
            if (creator != seller) {
