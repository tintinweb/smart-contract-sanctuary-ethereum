...
contract JUNI is ERC721r, Ownable {
  ...
  function revealNFT(uint256[] memory tokenIds) external {
    ...
    for (uint256 i; i < tokenIds.length; i++) {
      address tokenOwner = nft.ownerOf(tokenIds[i]);
      require(tokenOwner == msg.sender, "Not token owner");
    }
    for (uint256 i; i < tokenIds.length; i++) {
      nft.transferFrom(msg.sender, burnAddress, tokenIds[i]);
    }
    _mintRandom(msg.sender, tokenIds.length);
  }

  function revealNFTTwo(uint256[] memory tokenIds) external {
    IERC721 nft = IERC721(unrevealedContract);
    require(nft.isApprovedForAll(msg.sender, address(this)), "No permission to transfer");
    for (uint256 i; i < tokenIds.length; i++) {
      address tokenOwner = nft.ownerOf(tokenIds[i]);
