...
contract TomiFundsDao is TomiEvents, TomiStates, Ownable {
    ...
    function getProposals(uint256 id) external view returns (Proposal memory) {
        return proposals[id];
    }

    function getProposalsSendTokenData(uint256 id) external view returns (ProposalSendTokenData memory) {
        return proposalsSendTokenData[id];
    }

    function getProposalsSendNFTData(uint256 id) external view returns (ProposalSendNFTData memory) {
        return proposalsSendNFTData[id];
    }

    function getReceipts(uint256 id, address voter) external view returns (Receipt memory) {
    ...
    function proposeSendToken(
        ...
        require(16 <= functionId && functionId <= 18, "incorrect function id");
        if (
            functionId == 15 && executionAmount > 100_000 * (10**token.decimals()) ||
            functionId == 16 && executionAmount <= 100_000 * (10**token.decimals()) && executionAmount > 500_000 * (10**token.decimals()) ||
            functionId == 17 && executionAmount <= 500_000 * (10**token.decimals())
        ) {
            revert("fucntion id and amount not matched");
        }
        proposalCount++;
    ...
    function initializeProposal(uint256[] memory nftIds, uint8 functionId, string memory description, string memory title) internal {
        ...
        uint256 quorumVotes =
            tomiNFT.totalSupply().mul(tomiMainDao.getCriteria(functionId).quorumVotes);
        ...
        emit ProposalCreated(proposalCount, _msgSender(), newProposal.startTime, newProposal.endTime, description, title);
    }
    ...
    function executeSendToken(uint256 proposalId) external {
        ...
        if (state(proposalId) == ProposalState.Succeeded) {
            ...
            treasury.withdrawToken(proposalData.token, proposalData.executionTarget, proposalData.executionAmount);
            // SafeERC20.safeTransferFrom(proposalData.token, address(treasury), proposalData.executionTarget, proposalData.executionAmount);
        }
    ...
    function executeSendNFT(uint256 proposalId) external {
        ...
        if (state(proposalId) == ProposalState.Succeeded) {
            ...
            for(uint256 i =0; i< proposal.nftIds.length; i++) {
                ...
                // proposalData.nft.transferFrom(address(treasury), proposalData.executionTargets[i], proposalData.executionNFTIds[i]);
            }
        }
