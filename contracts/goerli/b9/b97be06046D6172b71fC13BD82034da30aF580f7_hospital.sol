...
contract Consultation{
    ...
    */
    ...
    function createMedicalAppointment(bytes32 _secretNumberPatientHash,
    address doctor,address hospital, uint256 consultationNumber) external {
        require(checkDoctor(doctor,hospital),"doctor not valid in this hospital");
        ...
        consultationInVerification[msg.sender][consultationNumber].hospital = hospital;
    ...
    function checkDoctor(address doctor, address hospital) public view returns(bool) {
        return _hospitalContract.checkDoctor(hospital, doctor);
    }

    function validatingPatientConsultation(address _patient, uint256 secretNumber,bool MerkleTree,
    uint256 consultationNumber, bytes32 consultationMerkleTree
    )external{
    ...
    function SetProntuarioMedico(bytes32 PassoZeroDaProvaDeMerkleConsulta, 
    string memory hashDoDocumento,
    address hospital
    ) external {
        require(checkDoctor(msg.sender, hospital));
        _prontuarioMedico[PassoZeroDaProvaDeMerkleConsulta] = hashDoDocumento;
    }
