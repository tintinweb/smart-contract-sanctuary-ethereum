...
import {TurinDistributionVault} from "./TurinDistributionVault.sol";
...
contract TurinDeployer {
    ...
    constructor(uint256 timestamp, address otc, address feed, IERC20Upgradeable wbtc) {
        {
            TurinToken tokenImplementation = new TurinToken();
            bytes memory tokenInitializeData = abi.encodeWithSelector(TurinToken.initialize.selector);
            address tokenAddress = _createProxy(address(tokenImplementation), tokenInitializeData);
            token = IERC20Upgradeable(tokenAddress);
        }

        address vaultImplementationAddress = address(new TurinDistributionVault());
...

        {
            ...
            bytes memory psdaInitializeData = abi.encodeWithSelector(
                TurinDistributionVault.initialize.selector,
                address(token),
                1,
                psdaAmount,
                timestamp + ONE_YEAR,
...

        {
            ...
            bytes memory rewardsInitializeData = abi.encodeWithSelector(
                TurinDistributionVault.initialize.selector, address(token), 1, rewardsAmount, timestamp, timestamp + 1
            );
            rewardsVault = _createProxy(vaultImplementationAddress, rewardsInitializeData);
...

        {
            ...
            bytes memory stakingInitializeData = abi.encodeWithSelector(
                TurinStaking.initialize.selector, incentivesVault, token, 30 days, 0.1 ether, 10 ether
            );
...

        {
            ...
            wbtc.safeTransferFrom(otc, address(this), 2e8);
...

        {
            ...
            uint256 lpBalance = lpToken.balanceOf(otc);
            require(lpBalance == liquidity, "");
            token.safeTransfer(otc, token.balanceOf(address(this)));
        }

        {
            address farmingImplementationAddress = address(new TurinFarming());
            bytes memory farmingInitializeData =
                abi.encodeWithSelector(TurinFarming.initialize.selector, incentivesVault, pool);
...

        {
            ...
            TurinDistributionVault(incentivesVault).addAccount(farming, allocations);
            uint256[] memory allocations2 = new uint256[](uint256(2));
            allocations2[0] = 80 ether;
            allocations2[1] = 20 ether;
            TurinDistributionVault(incentivesVault).addAccount(staking, allocations2);
        }
    }
