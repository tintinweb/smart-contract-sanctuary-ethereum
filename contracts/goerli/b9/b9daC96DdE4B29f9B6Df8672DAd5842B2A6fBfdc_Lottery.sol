...
contract Lottery is RandomnessConsumer, Automated {
     ...
     */
    function enter() external payable lotteryIsOpen entryFeePaid {
...
import {AutomationRegistryInterface, State} from '@chainlink/contracts/src/v0.8/interfaces/AutomationRegistryInterface1_2.sol';
...
abstract contract Automated is AutomationCompatibleInterface {
    error NotEnoughLinkProvidedForInitialFunding();
    error AutomationRegistryAutoApproveDisabled();
    ...
    function fundAutomation(uint96 amount) external {
        if (
            automationUpkeepId == 0 &&
            amount < MINIMUM_INITIAL_AUTOMATION_FUNDING_AMOUNT
        ) {
            revert NotEnoughLinkProvidedForInitialFunding();
        }
    ...
    function registerAndFundUpkeep(string memory name, uint96 amount) private {
        ...
        if (newRegistryState.nonce == initialRegistryState.nonce + 1) {
            ...
            revert AutomationRegistryAutoApproveDisabled();
        }
    }
}
