...
contract B {
    ...
    enum poll_status { voting, pass, dismissal}
    struct poll {
        uint number;
        string title;
        string contents;
    ...
    enum vote_status {
        notYet, agree, disagree
    }

    struct user {
        string name;
        string[] poll_list; // 내가 올린 안건
        mapping(string => vote_status) voted; // 문제
    }

    mapping(address => user) users;

    function setPoll(string memory _title, string memory _contents) public {
        Polls[_title] = poll(index++, _title, _contents, msg.sender, 0,0, poll_status.voting);
    ...
    function getUser2(string memory _a) public view returns(vote_status) {
        return users[msg.sender].voted[_a];
    }

    function vote(string memory _title, bool _b) public {
        require(users[msg.sender].voted[_title] == vote_status.notYet);
        require(Polls[_title].p_status == poll_status.voting);
        ...
        if(keccak256(bytes(Polls[_title].title)) == keccak256(bytes(_title))) { //title과 _title 비교, string은 직접 비교가 아닌 hash값으로
            ...
            if(_b == true) {
                ...
                users[msg.sender].voted[_title] = vote_status.agree;  // voted는 string과 bool로 구성(key -value), _title을 key값으로 넣고 _b를 value 값으로 넣음.
            } else {
                Polls[_title].disag++;
                users[msg.sender].voted[_title] = vote_status.disagree;
            }
        }


        if(Polls[_title].agree+Polls[_title].disag >= 10) {
            if ((Polls[_title].agree * 100/(Polls[_title].agree+Polls[_title].disag)) >= 70 ) {
                Polls[_title].p_status = poll_status.pass;
            } else {
                Polls[_title].p_status = poll_status.dismissal;
            }
        }
