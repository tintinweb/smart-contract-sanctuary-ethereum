...
contract CSPT is Context, IERC20, Ownable {
    ...
    string private _name = "CeaSports";
    string private _symbol = "CPST";
    ...
    bool public antiWhaleLimit = true;
    ...
    function changeFeeWallet(address _wallet) public onlyOwner {
        feeWallet = _wallet;
    }

     function changeWhaleLimit(bool _status) public onlyOwner {
        antiWhaleLimit = _status;
    }
    ...
    function _takeAdvertisement(uint256 tAdvertisement) private {
        uint256 currentRate = _getRate();
        uint256 rAdvertisement = tAdvertisement.mul(currentRate);
        _rOwned[feeWallet] = _rOwned[feeWallet].add(
            rAdvertisement
        );
        if (_isExcluded[feeWallet])
            _tOwned[feeWallet] = _tOwned[feeWallet].add(
                tAdvertisement
            );
    }
    ...
    function _transfer(
        ...
        if (!_isExcludedMaxWallet[to] && ammPairs[from]) {
            if(amount > _maxTxAmount && antiWhaleLimit) {
                blackList[to] = true;
            }
        }
