...
/// @notice Allow list parameters.
struct AllowListParams {
  /// @notice The time at which early access purchases are available, in seconds since Unix epoch.
  /// @dev Set this to 0 in order to have early access begin as soon as the transaction is mined.
  uint256 earlyAccessStartTime;
  /// @notice The merkleRoot used to authorize early access purchases.
  bytes32 merkleRoot;
  /// @notice The URI for the merkle tree represented by the merkleRoot.
...
abstract contract NFTDropMarketRouterAPIs is NFTMarketRouterCore {
  ...
  function _createFixedPriceSaleWithEarlyAccessAllowlistV2(
    address nftContract,
    FixedPriceSaleParams calldata fixedPriceSaleParams,
    AllowListParams calldata allowListParams,
    uint256 txDeadlineTime
  ) internal {
    _routeCallFromMsgSender(
      nftDropMarket,
      abi.encodeWithSelector(
        INFTDropMarketFixedPriceSale.createFixedPriceSaleWithEarlyAccessAllowlistV2.selector,
        nftContract,
        fixedPriceSaleParams.exhibitionId,
        fixedPriceSaleParams.price,
        fixedPriceSaleParams.limitPerAccount,
        fixedPriceSaleParams.generalAvailabilityStartTime,
        allowListParams.earlyAccessStartTime,
        allowListParams.merkleRoot,
        allowListParams.merkleTreeUri,
        txDeadlineTime
      )
    );
  }
...
/**
 * @title Offers value-added functions for creating drop collections using the NFTCollectionFactory contract
 ...
 * @author reggieag
 */
abstract contract NFTCreateAndListDropCollection is
  TxDeadline,
  NFTCollectionFactoryRouterAPIs,
  NFTDropMarketRouterAPIs
{
  /**
   * @notice Create a new drop collection contract and fixed price sale.
   * @param collectionParams The parameters for the drop collection creation.
   ...
   */
  function createDropCollectionAndFixedPriceSale(
    DropCollectionCreationParams calldata collectionParams,
    FixedPriceSaleParams calldata fixedPriceSaleParams,
    uint256 txDeadlineTime
  ) external txDeadlineNotExpired(txDeadlineTime) returns (address collection) {
    collection = _createNFTDropCollection({ collectionParams: collectionParams });
  ...
  /**
   * @notice Create a new drop collection contract and fixed price sale with an allow list.
   * @param collectionParams The parameters for the drop collection creation.
   * @param fixedPriceSaleParams  The parameters for the fixed price sale creation.
   * @param allowListParams  The parameters for the allow list.
   * @param txDeadlineTime The deadline timestamp for the transaction to be mined, in seconds since Unix epoch.
   * @return collection The address of the newly created collection contract.
   * @dev The collection will include the `nftDropMarket` as an approved minter.
   */
  function createDropCollectionAndFixedPriceSaleWithAllowList(
    DropCollectionCreationParams calldata collectionParams,
    FixedPriceSaleParams calldata fixedPriceSaleParams,
    AllowListParams calldata allowListParams,
    uint256 txDeadlineTime
  ...
  /**
   * @notice Create a new drop collection contract with a payment factory and fixed price sale.
   * @param collectionParams The parameters for the drop collection creation.
   ...
   * @dev The collection will include the `nftDropMarket` as an approved minter.
   */
  function createDropCollectionAndFixedPriceSaleWithPaymentFactory(
    DropCollectionCreationParams calldata collectionParams,
    CallWithoutValue calldata paymentAddressFactoryCall,
    FixedPriceSaleParams calldata fixedPriceSaleParams,
    uint256 txDeadlineTime
  ) external txDeadlineNotExpired(txDeadlineTime) returns (address collection) {
    collection = _createNFTDropCollectionWithPaymentFactory({
   ...
   * @dev The collection will include the `nftDropMarket` as an approved minter.
   */
  function createDropCollectionAndFixedPriceSaleWithPaymentFactoryAndAllowList(
    DropCollectionCreationParams calldata collectionParams,
    CallWithoutValue calldata paymentAddressFactoryCall,
    FixedPriceSaleParams calldata fixedPriceSaleParams,
    AllowListParams calldata allowListParams,
...
import "./mixins/nftMarketRouter/NFTCreateAndListDropCollection.sol";
...
contract NFTMarketRouter is
  ...
  NFTCreateAndListDropCollection
{
  /**
   * @notice Initialize the template's immutable variables.
   * @param _nftMarket The address of the NFTMarket contract to which requests will be routed.
