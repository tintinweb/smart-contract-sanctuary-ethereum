...
contract SHIBAFTESTNFT is Ownable, ERC721Enumerable, SalesActivation {
    // total sales
    uint256 public constant TOTAL_MAX_QTY = 2000;
    // nft public sales price
    uint256 public SALES_PRICE = 30000 * 10**9;
    ...
    constructor(address _addressToken) ERC721("ShibaFNft", "ShibaFNft") {
        rarity=[0,0,0,0,0];
    ...
    function setRarityNft (uint256 tokenId) internal returns(bool) {
        ...
        if(rarityRandom <= 50 && rarity[0] <1000){
            _setRarityforNFT(tokenId, 1, 0);
            return true;
        }
        if(rarityRandom > 50 && rarityRandom <= 70 && rarity[1] <400){
            _setRarityforNFT(tokenId, 2, 1);
            return true;
        }
        if(rarityRandom > 70 && rarityRandom <= 85 &&  rarity[2] <300){
            _setRarityforNFT(tokenId, 3, 2);
            return true;
        }
        if(rarityRandom > 85 && rarityRandom <= 95 &&  rarity[3] <200){
            _setRarityforNFT(tokenId, 4, 3);
            return true;
        }
        if(rarityRandom > 95 && rarity[4] <100){
        ...
        if(rarity[4] < 100){
            _setRarityforNFT(tokenId, 5, 4);
            return true;
        }else if(rarity[3] < 200){
            _setRarityforNFT(tokenId, 4, 3);
            return true;
        }else if(rarity[2] < 300){
            _setRarityforNFT(tokenId, 3, 2);
            return true;
        }else if(rarity[1] < 400){
            _setRarityforNFT(tokenId, 2, 1);
            return true;
        }else{
            _setRarityforNFT(tokenId, 1, 0);
