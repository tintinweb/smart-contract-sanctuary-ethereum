...
contract TheCup is ERC1155Supply, ERC2981, Ownable, ReentrancyGuard, QRNG {
    ...
    event AdminMinted(address sender);

    //commenting out the airnode for now
    constructor(address _airnodeRrp,address _royalty, string memory _cupNFT) ERC1155(_cupNFT) QRNG(_airnodeRrp) { 
    ...
    function mint(uint256 amount) external payable {
        ...
        for (uint256 i = 0; i < amount; i++) {
            ...
            uint256 sum = (seed + localRandom) % 495;  
            uint256 _tokenNumber = getTokenId(sum);
    ...
    function airDrop(address[] memory _airdrops) external onlyOwner {
        ...
        for (uint256 i = 0; i < _airdrops.length; i++) {
            ...
            uint256 sum = (seed + localRandom) % 495; 
            uint256 _tokenNumber = getTokenId(sum);
            _mint(_airdrops[i], _tokenNumber, 1, "");
            nftsMinted += 1;
        }
        emit AdminMinted(msg.sender);
    }

    function cupDistribute(address[] memory _winners) external onlyOwner {
        require(!paused, "Contract Paused");
        for (uint256 i = 0; i < _winners.length; i++) {
            _mint(_winners[i], 33, 1, "");
        }    
    }
        ...
        if (chance > 240) {
            ...
            if (chance > 360) {
                ...
                if (chance > 420) {
                    ...
                    if (chance > 450) {
                        // 31 or 32
                        if (chance > 465) return 32;
                        else return 31;
                    } else {
                        // 29 or 30
                        if (chance > 435) return 30;
                        else return 29;
                    }
                } else {
                    // 25 to 28
                    if (chance > 390) {
                        // 27 or 28
                        if (chance > 324) return 28;
                        else return 27;
                    } else {
                        // 25 or 26
                        if (chance > 375) return 26;
                        else return 25;
                    }
                }
            } else {
                ...
                if (chance > 300) {
                    ...
                    if (chance > 330) {
                        // 23 or 24
                        if (chance > 345) return 24;
                        else return 23;
                    } else {
                        // 21 or 22
                        if (chance > 315) return 22;
                        else return 21;
                    }
                } else {
                    // 17 to 20
                    if (chance > 270) {
                        // 19 or 20
                        if (chance > 285) return 20;
                        else return 19;
                    } else {
                        // 17 or 18
                        if (chance > 255) return 18;
                        else return 17;
                    }
                }
            }
            ...
            if (chance > 120) {
                ...
                if (chance > 180) {
                    ...
                    if (chance > 210) {
                        // 15 or 16
                        if (chance > 225) return 16;
                        else return 15;
                    } else {
                        // 13 or 14
                        if (chance > 195) return 14;
                        else return 13;
                    }
                } else {
                    // 9 to 12
                    if (chance > 150) {
                        // 11 or 12
                        if (chance > 165) return 12;
                        else return 11;
                    } else {
                        // 9 or 10
                        if (chance > 135) return 10;
                        else return 9;
                    }
                }
            } else {
