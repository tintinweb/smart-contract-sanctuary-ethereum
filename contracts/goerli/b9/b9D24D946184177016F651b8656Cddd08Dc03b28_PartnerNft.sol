...
contract PartnerNft is ERC1155, Ownable, Pausable/*, ERC1155Supply*/, ERC2981Base {
    RoyaltyInfo private _royalties;
    string public name = "Lore Pass";
...
//        _pause();
    ...
    /// @notice Add merkle root for a token id
    function modifyType(
        uint256 _id,
        bytes32 _root
    ) public onlyOwner {
    ...
    onlyOwner
    {
        require(toAccounts.length > 0, "Must have at least one account");
        require(amount > 0, "Must have at least one token");
        for (uint256 i = 0; i < toAccounts.length; ++i) {
            _mint(toAccounts[i], id, amount, data);
        }
    }

    /// @dev Value is in basis points so 10000 = 100% , 100 = 1% etc
...
//        _mintApprovals[tokenId][to] = false;
    ...
    function mintSelf(uint256 tokenId, bytes32[] calldata proof)
    external
    whenNotPaused
    {
        mint(msg.sender, tokenId, proof);
    }
