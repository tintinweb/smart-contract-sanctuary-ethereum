...
contract DefimonsSkins is AccessControl, ONFT1155 {
    ...
    event Airdrop(address[] addresses, uint256 tokenId_);
    ...
    /**
     * @notice Airdrop one Skin to each address present in the list.
     * @dev This function calls the '_mint()' function inside a recursive loop. Beware of gas spending!
     * @param addrs_    The list of addresses to airdrop to.
     * @param tokenId_  The tokenId of the Skin to airdrop.
     */
    function airdrop(address[] memory addrs_, uint256 tokenId_) external onlyRole(DEFAULT_ADMIN_ROLE) {
        emit Airdrop(addrs_, tokenId_);

        for (uint256 i = 0; i < addrs_.length; ) {
            _mint(addrs_[i], tokenId_, 1, "");
    ...
    function _beforeTokenTransfer(
        ...
        if (from == address(0)) {
            for (uint256 i = 0; i < ids.length; ) {
                require(totalSupply[ids[i]] + amounts[i] <= maxSupply[ids[i]], "ERC1155: Mint surpasses max supply");
        ...
        if (to == address(0)) {
            for (uint256 i = 0; i < ids.length; ) {
                uint256 id = ids[i];
                uint256 amount = amounts[i];
                require(totalSupply[id] >= amount, "ERC1155: Burn amount exceeds totalSupply");
                unchecked {
...
contract SkinSales is AccessControl {
     ...
     * @param bought        How many Skins have already been bought from this Sale.
     ...
     * @param resetBought_  Whether the Admin intends to reset the 'bought' parameter.
     *                      This should be set to true if the Admin intends to reuse this Sale.
     */
    function deactivateSale(uint256 saleId_, bool resetBought_)
        public
        ...
        onlyActive(saleId_)
    {
        ...
        if (resetBought_) _sales[saleId_].bought = 0;
