    ...
    IAssetVault
{
    ...
    modifier onlyAdminOrVaultBuilder() {
        require(
            metawealthMod.isAdmin(_msgSender()) ||
                _msgSender() == _vaultBuilder,
            "MetaWealthAccessControl: Restricted to Admin and vault builder"
        );
        _;
    }
    ...
    {
        require(_totalShares > 0, "AssetVault: Asset not instantiated");
        active = !active;
        ...
        returns (address shareholder)
    {
        ...
        require(
            ...
            "AssetVault: Defractionalization not permitted yet"
        );
...
contract VaultBuilder is IVaultBuilder, Initializable, ContextUpgradeable {
    ...
    function fractionalize(
        ...
        newVault = _createNewAssetVault(payees, shares);

        tokenFractions[collection][tokenId] = newVault;

        /// @dev post-audit: Made the recipient address(this)
        IERC721(collection).transferFrom(_msgSender(), address(this), tokenId);
        emit AssetFractionalized(collection, tokenId, newVault, payees, shares);
    }

    /// @dev post-audit: Added defractionalize()
    function defractionalize(
        address collection,
        uint256 tokenId
    ) public override onlyAdmin {
        IAssetVault vault = IAssetVault(tokenFractions[collection][tokenId]);
        require(
            address(vault) != address(0),
            "VaultBuilder: Already defractionalized"
        );
        address shareholder = IAssetVault(vault).burn();
        IERC721(collection).transferFrom(address(this), shareholder, tokenId);
