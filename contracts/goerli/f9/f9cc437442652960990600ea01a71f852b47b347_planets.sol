...
contract planets is  ERC721A, ERC721AQueryable, IERC721ABurnable, ReentrancyGuard, Ownable, EIP712Whitelisting {
...
//  uint256 public MAX_PREMIUM_PER_WALLET;
  ...
  uint256 public PLANET1_SUPPLY;
  uint256 public PLANET2_SUPPLY;
  uint256 public PLANET3_SUPPLY;
  uint256 public PLANET4_SUPPLY;
  uint256 public PLANET5_SUPPLY;
  uint256 public PLANET6_SUPPLY;
  uint256 public PLANET7_SUPPLY;
  uint256 public PLANET8_SUPPLY;
  uint256 public PLANET9_SUPPLY;
...
// 0x70b235927b619FBEEFE0ACAa5eFa28ac66B0E34E

// ["0x9eBF693deD468fB1F6E68fA0e4E43A11D7Fc7F2c", "0x70b235927b619FBEEFE0ACAa5eFa28ac66B0E34E"]
    ...
    PLANET8_SUPPLY = 500;
    PLANET9_SUPPLY = 500;
    MAX_SUPPLY = PLANET1_SUPPLY + PLANET2_SUPPLY + PLANET3_SUPPLY + PLANET4_SUPPLY + PLANET5_SUPPLY + PLANET6_SUPPLY + PLANET7_SUPPLY + PLANET8_SUPPLY + PLANET9_SUPPLY;
  ...
  Counters.Counter private planet9SupplyCounter;
    ...
    //base template
    //  function totalPlanet1Supply() public view returns (uint256) {
  ...
  function totalPlanet2Supply() public view returns (uint256) {
    return planet2SupplyCounter.current();
  }

  function totalPlanet3Supply() public view returns (uint256) {
    return planet3SupplyCounter.current();
  }

  
  function totalPlanet4Supply() public view returns (uint256) {
    return planet4SupplyCounter.current();
  }

  
  function totalPlanet5Supply() public view returns (uint256) {
    return planet5SupplyCounter.current();
  }

    
  function totalPlanet6Supply() public view returns (uint256) {
    return planet6SupplyCounter.current();
  }

  
  function totalPlanet7Supply() public view returns (uint256) {
    return planet7SupplyCounter.current();
  }

  function totalPlanet8Supply() public view returns (uint256) {
    return planet8SupplyCounter.current();
  }

  function totalPlanet9Supply() public view returns (uint256) {
  ...
  function mintPlanet1(uint256 count) public payable nonReentrant {
    require(saleIsActive, "Sale not active");
    // required function ~ totalPlanet1Supply() ~ required constant PLANET1_SUPPLY
    ...
    for (uint256 i = 0; i < count; i++) {
      ...
      _safeMint(_msgSender(), totalPlanet1Supply());
    }
...
// REF
// MAX_STANDARD_SUPPLY + MAX_PREMIUM_RESERVED_SUPPLY + totalPremiumSupply());
    function mintPlanet2(uint256 count) public payable nonReentrant {
    require(saleIsActive, "Sale not active");
    // required function ~ totalPlanet1Supply() ~ required constant PLANET1_SUPPLY
    ...
    for (uint256 i = 0; i < count; i++) {
      standardSupplyCounter.increment();
      _safeMint(_msgSender(), totalPlanet2Supply() + PLANET1_SUPPLY);
    }

    payable(_splitter).transfer(msg.value);
  }

    function mintPlanet3(uint256 count) public payable nonReentrant {
    require(saleIsActive, "Sale not active");
    // required function ~ totalPlanet1Supply() ~ required constant PLANET1_SUPPLY
    ...
    for (uint256 i = 0; i < count; i++) {
      standardSupplyCounter.increment();
      _safeMint(_msgSender(), totalPlanet3Supply() + PLANET1_SUPPLY + PLANET2_SUPPLY );
    }
