...
contract Staker {
  ...
  event Stake(address sender, uint256 n);
  ...
  function threshold() public pure returns(uint256){
    uint256 target = 1000000000000000000;  
    return (target);
  }
  ...
  function withdraw() public payable notCompleted {
    require (openForWithdraw, "failed to withdraw.");
    require(address(this).balance < threshold(), "You are not able to withdraw your funds!");
  ...
  function timeLeft() public view returns(uint) {
    ...
    else {uint time = deadline - block.timestamp;
    return (time);}
  }
