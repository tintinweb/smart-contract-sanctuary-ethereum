...
contract MarketPlace is ERC721, ERC721URIStorage, Ownable {
    ...
    function BuyNfts(uint256 _tokenId, uint256 amount) public {
        require(amount >= Sale[_tokenId].price);
        uint256 OwnerPercentage = (amount * 200) / 1000;
        amount -= OwnerPercentage;
        require(IERC20(Token).transferFrom(msg.sender, owner(), OwnerPercentage));
        require(IERC20(Token).transferFrom(msg.sender, Sale[_tokenId].Owner, amount));
    ...
    function ChangeNftPrice(uint256 _tokenId, uint256 _price) public {
        require(
            ...
            "Only Owner able to change the price"
        );
