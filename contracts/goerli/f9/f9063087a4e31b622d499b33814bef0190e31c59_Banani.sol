...
contract Banani is ERC721AQueryable, Ownable, ReentrancyGuard, Pausable {
  ...
  function mintOwner(address addr, bytes32 hashedMail) public payable onlyOwner whenNotPaused {
    require(!minted[hashedMail],"Already minted");
    _safeMint(addr, 1);
    minted[hashedMail]=true;
    emit Minted(addr, hashedMail);
  }
