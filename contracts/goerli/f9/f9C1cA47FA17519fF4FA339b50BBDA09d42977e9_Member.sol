    ...
    ReentrancyGuardUpgradeable
{
    ...
    function createMember(string memory uid, address _walletAddress) public {
        ...
        if (
            ...
            member memory _member = member(uid, block.timestamp, _wallets, 0);
            members[uid] = _member;
    ...
    function _storeAssets(
        ...
        if (checkWallet(_backUpWallet, uid) == false) {
            for (uint256 i = 0; i < _backUpWallet.length; i++) {
                addWallet(uid, _backUpWallet[i], false);
            }
        }
