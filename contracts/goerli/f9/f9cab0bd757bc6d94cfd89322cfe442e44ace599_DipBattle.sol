...
// File: contracts/DipBattlePolygon.sol
...
contract DipBattle is Ownable {
    event BattleStatus(uint256 indexed round, bool indexed status);

    bool public battleStatus;
    uint256 public round;
    address private _bowlBound; // = 0xc36cb218848F173148ff55f4dfC18f1540FB7475;
    ...
    constructor(address chipPowers, address bowlBound) {
        _chipPowers = chipPowers;
        _bowlBound = bowlBound;
    }
        ...
        whenBattle
    {
        ...
        for (uint256 i; i < chips.length; i++) {
            require(
                IERC721(_bowlBound).ownerOf(chips[i]) == msg.sender,
                "Chip not owned!"
            );
