...
contract ENSTogether is ReentrancyGuard, Ownable {
    ...
    function propose(address _to) external payable {
        ...
        require(
            unionWith[msg.sender].relationshipStatus == uint8(Status.EMPTY) ||
        ...
        require(
            unionWith[_to].relationshipStatus == uint8(Status.EMPTY) ||
                unionWith[_to].expired == true,
            "This address is already in a relationship"
        );
        //Check if both addresses have an ENS name
        // string memory ensSender = lookupENSName(msg.sender);
        // string memory ensReceiver = lookupENSName(_to);

        // if (bytes(ensSender).length == 0) {
        //     revert("Sender does not have an ENS name");
        // }
        // if (bytes(ensReceiver).length == 0) {
        //     revert("Receiver does not have an ENS name");
        // }
    ...
    function respondToProposal(Proposal response, string memory ens1, string memory ens2) external payable {
        //Response shouldnt be EMPTY or PENDING
        require(
            uint8(response) != uint8(Proposal.EMPTY) &&
