...
contract loan20230114 is Ownable {
    ...
    */
    ...
    function setTerms() internal returns (bool){
        ...
        */
        ...
        eeee=Initial Base Payment="eth"/100
        
        Example:"speed test"            =10006000100020002001686, with 1 ETH loan
        Example2:"unInsured speed test" =10006000100020000001686, with 1 ETH loan
        0006=total designed payments
        0001=payment frequency=enumeration of "speed test"=every 5 min
        0002=apr=2 percent=.02
        0002=insurance divisor
        016="eth"/100=.16 (eth)
        */
    ...
    function makeLoanPayment() public payable {
        ...
        //uint256 currIntrest=calcCurrentIntrest();
        //uint256 totAmountOwed=loanBalance + processFee + insuranceFee + currIntrest;
        /*
        bool finalPaymentReady=false;
        if(address(this).balance >=(totAmountOwed+ 5999999999999999)){ finalPaymentReady=true;}
        //IF "breachStatus==30" AND "msg.sender==underWriter" THEN payments is being made by insurance
        //check paymentDue
        if (finalPaymentReady){
            //check if loan is ending. !!!!!!!!!!change per terms if Early Payoff is allowed.
