...
contract BullRun is Ownable, ReentrancyGuard {
    ...
    address payable public RandomizerContract = payable(0x798654cCbdC18b66eEaE893362135A121236D640); // VRF contract to decide nft stealing
    address payable dev;
    address public HUB;
    address public betContract;
    IERC721 public Genesis = IERC721(0x97A792aCE504c2590C5046A7768A91CAd8aa0971); // Genesis NFT contract
    IERC721 public Alpha = IERC721(0xdf4F2DA93C07d590E16D34d2c19d3Ba93CB13E98); // Alpha NFT contract

    ICoinFlip private CoinFlipInterface = ICoinFlip(0x40a928a84D4b4B62BC8F3aAa8183D6D63695fA0F);
    IRandomizer private RandomizerInterface = IRandomizer(0x798654cCbdC18b66eEaE893362135A121236D640);
    IHUB public HubInterface = IHUB(0xe95861F4dF501C1E11F2dce1BeC1948a06215860);
    ...
    mapping(address => EnumerableSet.UintSet) WastelandMatadorsOfWallet; // list of matador IDs a user has sent to the wastes
    ...
    function sendMatadorToWastelands(uint16[] calldata _ids) external payable notContract() {
        ...
        for (uint16 i = 0; i < numWords;) {
            ...
            if (HubInterface.alienCount() > 0 && (seed[i]) % 100 < 25) { // stolen
                ...
                WastelandMatadorsOfWallet[msg.sender].add(_ids[i]);
