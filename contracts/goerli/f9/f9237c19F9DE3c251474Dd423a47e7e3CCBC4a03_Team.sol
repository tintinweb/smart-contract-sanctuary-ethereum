...
contract Team is Initializable, ITeam { 
    // using CountersUpgradeable for CountersUpgradeable.Counter;

    IAccessRestriction public accessRestriction;

    /** NOTE {isTeam} set inside the initialize to {true} */
    uint32 private _counter;

    /** NOTE {isTeam} set inside the initialize to {true} */
    bool public override isTeam;

    /** NOTE mapping of teamId to TeamData Struct */
    mapping(uint32 => bytes32) public override teams;

    /** NOTE mapping of countryName to boolean is exists Struct */
    mapping(bytes32 => bool) public override countryNames;
    ...
    private
    {
        ...
        teams[17]="Mexico";
        teams[18]="Morocco";
        teams[19]="Netherlands";
        teams[20]="Poland";
        teams[21]="Portugal";
        teams[22]="Qatar";
        teams[23]="Saudi Arabia";
        teams[24]="Senegal";
        teams[25]="Serbia";
        teams[26]="South Korea";
        teams[27]="Spain";
        teams[28]="Switzerland";
        teams[29]="Tunisia";
        teams[30]="United States";
        teams[31]="Uruguay";
        teams[32]="Wales";
        _counter=33;
    }
    ...
    /// @inheritdoc ITeam
    function addTeam(bytes32 _countryName)
        external
        override
        onlyDataManager
    {
        require(_countryName!="","Empty String is not valid");
        require(countryNames[_countryName]!=true,"The countryName used before");
        teams[_counter]=_countryName;
        countryNames[_countryName]=true;
        emit TeamAdded(_counter, _countryName);
        _counter+=1;
    }
    /// @inheritdoc ITeam
    function updateTeam(uint32 _id,bytes32 _countryName)
        external
        override
        onlyDataManager
    {
        require(_countryName!="" ,"Empty String is not valid");
        require(teams[_id]==_countryName ||countryNames[_countryName]!=true,"The countryName used before");
        if(teams[_id]!=_countryName){
            countryNames[teams[_id]]=false;
            countryNames[_countryName]=true;
        }
        teams[_id]=_countryName;
        emit TeamUpdated(_id, _countryName);
    }
    /// @inheritdoc ITeam
    ...
    returns (bool)
    {
        return (countryNames[teams[_teamA]]&& countryNames[teams[_teamB]]);
    }
