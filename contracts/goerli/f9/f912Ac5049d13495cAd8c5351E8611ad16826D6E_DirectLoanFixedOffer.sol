 ...
 * @author
 * @notice Main contract for  Direct Loans Fixed Type. This contract manages the ability to create NFT-backed
 ...
 *   1. the borrower calls nftContract.approveAll(), approving the  contract to move their NFT's on their
 * be1alf.
 *   2. the lender calls erc20Contract.approve(), allowing  to move the lender's ERC20 tokens on their
 * behalf.
 ...
 *  promissory note (in ERC721 form) that represents the rights to either the principal-plus-interest, or the
 ...
 * receives an  promissory note (in ERC721 form) that represents the rights to either the principal-plus-interest,
 ...
 * - First, a borrower may call .payBackLoan() and pay back the loan plus interest at any time, in which case they
 ...
 * .liquidateOverdueLoan(), in which case they receive the underlying NFT collateral and forfeit the rights to the
...
contract DirectLoanFixedOffer is DirectLoanBaseMinimal {
    ...
    ) DirectLoanBaseMinimal(_admin, _Hub, _permittedErc20s) {
    ...
    function acceptOffer(
        ...
        _acceptOffer(
            _setupLoanTerms(_offer, nftWrapper, _signature.signer),
            _setupLoanExtras(
                _borrowerSettings.revenueSharePartner,
                _borrowerSettings.referralFeeInBasisPoints
            ),
            _offer,
            _signature
        );
    ...
    function _acceptOffer(
        ...
        require(
            SigningUtils.isValidLenderSignature(_offer, _signature),
            "Lender signature is invalid"
        );

        address bundle = hub.getContract(ContractKeys._BUNDLER);
        require(
            _loanTerms.nftCollateralContract != bundle,
            "Collateral cannot be bundle"
        );
        ...
        returns (uint256 adminFee, uint256 payoffAmount)
    {
        ...
        uint256 interestDue = _loanTerms.maximumRepaymentAmount -
     ...
     *
     */
    function _loanSanityChecksOffer(LoanData.Offer memory _offer)
        internal
        pure
    {
        require(
 ...
 * @author
 * @notice Main contract for  Direct Loans Type. This contract manages the ability to create NFT-backed
 * peer-to-peer loans.
 *
 * There are two ways to commence an NFT-backed loan:
 *
 ...
 *
 */
abstract contract DirectLoanBaseMinimal is
    IDirectLoanBase,
    IPermittedERC20s,
    BaseLoan,
    NftReceiver,
    LoanData
{
     ...
     * nonces that are used by both the lender and the borrower when they are first signing off-chain  orders.
     *
     * These nonces can be any uint256 value that the user has not previously used to sign an off-chain order. Each
     * nonce can be used at most once per user within , regardless of whether they are the lender or the borrower
     ...
     * .cancelLoanCommitmentBeforeLoanHasBegun(), which marks the nonce as used and prevents any future loan from
     * using the user's off-chain order that contains that nonce.
     */
    mapping(address => mapping(uint256 => bool))
        internal _nonceHasBeenUsedForUser;
    ...
    /**
     * @notice This event is fired whenever a borrower begins a loan by calling .beginLoan(), which can only occur
     * after both the lender and borrower have approved their ERC721 and ERC20 contracts to use , and when they
     ...
     * @param  lender - The address of the lender. The lender can change their address by transferring the  ERC721
     ...
     */
    function updateAdminFee(uint16 _newAdminFeeInBasisPoints)
        external
        onlyOwner
    {
        require(
            _newAdminFeeInBasisPoints <= HUNDRED_PERCENT,
            "basis points > 10000"
        );
     ...
     */
    function drainERC20Airdrop(address _tokenAddress, address _receiver)
        external
        onlyOwner
    {
    ...
    function drainERC721Airdrop(
        ...
        require(
            _escrowTokens[_tokenAddress][_tokenId] == 0,
            "token is collateral"
        );
        require(
     ...
     * off-chain  orders. These nonces can be any uint256 value that the user has not previously used to sign an
     * off-chain order. Each nonce can be used at most once per user within , regardless of whether they are the
     * lender or the borrower in that situation. This serves two purposes:
     * - First, it prevents replay attacks where an attacker would submit a user's off-chain order more than once.
     * - Second, it allows a user to cancel an off-chain order by calling .cancelLoanCommitmentBeforeLoanHasBegun()
    ...
    function wrapCollateral(uint32 _loanId) external nonReentrant {
        ...
        (address instance, uint256 receiverId) = LoanAirdropUtils
            .wrapCollateral(_loanId, loan, hub);
     ...
     * Each nonce can be used at most once per user within , regardless of whether they are the lender or the
     ...
     * instead to nonces that are used by both the lender and the borrower when they are first signing off-chain
     *  orders. These nonces can be any uint256 value that the user has not previously used to sign an off-chain
     * order. Each nonce can be used at most once per user within , regardless of whether they are the lender or
    ...
    function _renegotiateLoan(
        ...
        (address borrower, address lender) = LoanChecksAndCalculations
            .renegotiationChecks(
                loan,
                _loanId,
                _newLoanDuration,
        ...
        require(
            SigningUtils.isValidLenderRenegotiationSignature(
                _loanId,
                _newLoanDuration,
                _newMaximumRepaymentAmount,
                _renegotiationFee,
    ...
    /**
     * @dev Transfer collateral NFT from borrower to this contract and principal from lender to the borrower.
     *
    ...
    /**
     * @dev Transfer principal from lender to the borrower.
     *
     * @param _loanTerms - Struct containing the loan's settings
     * @param _loanExtras - Struct containing some loan's extra settings, needed to avoid stack too deep
     * @param _lender - The address of the lender.
    ...
    function _createLoanNoNftTransfer(
        ...
        _escrowTokens[_loanTerms.nftCollateralContract][
        ...
        if (referralfee > 0) {
            ...
            IERC20(_loanTerms.loanERC20Denomination).safeTransferFrom(
                _lender,
                _referrer,
                referralfee
            );
        ...
        loanIdToLoan[totalNumLoans] = _loanTerms;
        loanIdToLoanExtras[totalNumLoans] = _loanExtras;
    ...
    function _transferNFT(
        ...
        Address.functionDelegateCall(
            ...
            abi.encodeWithSelector(
                INftWrapper(_loanTerms.nftCollateralWrapper)
                    .transferNFT
                    .selector,
                _sender,
                _recipient,
     ...
     *
     */
    function _loanSanityChecks(
        LoanData.Offer memory _offer,
        address _nftWrapper
    ) internal view {
        require(
            getERC20Permit(_offer.loanERC20Denomination),
            "Currency denomination is not permitted"
        );
        require(
            _nftWrapper != address(0),
            "NFT collateral contract is not permitted"
        );
        require(
            uint256(_offer.loanDuration) <= maximumLoanDuration,
            "Loan duration exceeds maximum loan duration"
        );
        require(
            uint256(_offer.loanDuration) != 0,
    ...
    function _setupLoanExtras(
        ...
        return
            LoanExtras({
                ...
                revenueShareInBasisPoints: LoanChecksAndCalculations
                    .getRevenueSharePercent(_revenueSharePartner, hub),
     ...
     */
    function _getWrapper(address _nftCollateralContract)
        internal
        view
        returns (address)
    {
...
library ContractKeys {
    ...
    bytes32 public constant _BUNDLER = bytes32("_BUNDLER");
...
interface LoanData {
     ...
     * @param nftCollateralWrapper - The  wrapper of the NFT collateral contract.
     * @param nftCollateralId - The ID within the NFTCollateralContract for the NFT being used as collateral for this
     * loan. The NFT is stored within this contract during the duration of the loan.
     ...
     * off-chain  orders. These nonce can be any uint256 value that the user has not previously used to sign an
     * off-chain order. Each nonce can be used at most once per user within , regardless of whether they are the
     * lender or the borrower in that situation. This serves two purposes:
     * - First, it prevents replay attacks where an attacker would submit a user's off-chain order more than once.
     * - Second, it allows a user to cancel an off-chain order by calling .cancelLoanCommitmentBeforeLoanHasBegun()
...
library LoanChecksAndCalculations {
    ...
    function payBackChecks(uint32 _loanId) external view {
        ...
        // Depending on how the rest of the code turns out, this check may be unnecessary.
        require(
            !IDirectLoanBase(address(this)).loanRepaidOrLiquidated(_loanId),
            "Loan already repaid/liquidated"
        );
        ...
        ) = IDirectLoanBase(address(this)).loanIdToLoan(_loanId);

        // When a loan exceeds the loan term, it is expired. At this stage the Lender can call Liquidate Loan to resolve
        // the loan.
        require(
            block.timestamp <= (uint256(loanStartTime) + uint256(loanDuration)),
            "Loan is expired"
        );
    }
     ...
     */
    function getRevenueSharePercent(address _revenueSharePartner, IHub _hub)
        external
        view
        returns (uint16)
    {
        ...
        uint16 revenueSharePercent = IPermittedPartners(
            _hub.getContract(ContractKeys.PERMITTED_PARTNERS)
    ...
    function renegotiationChecks(
        ...
        address lender = _loan.lender;

        require(msg.sender == borrower, "Only borrower can initiate");
        require(
            block.timestamp <=
                (uint256(_loan.loanStartTime) + _newLoanDuration),
            "New duration already expired"
        );
        require(
            uint256(_newLoanDuration) <=
                IDirectLoanBase(address(this)).maximumLoanDuration(),
        ...
        require(
            !IDirectLoanBase(address(this)).getWhetherNonceHasBeenUsedForUser(
                lender,
                _lenderNonce
            ),
            "Lender nonce invalid"
    ...
    function bindingTermsSanityChecks(
        ...
        require(
            _offer.loanERC20Denomination == _listingTerms.loanERC20Denomination,
            "Invalid loanERC20Denomination"
        );
        require(
        ...
        uint256 maxRepaymentLimit = _offer.loanPrincipalAmount +
            ...
            HUNDRED_PERCENT;
        require(
            _offer.maximumRepaymentAmount <= maxRepaymentLimit,
            "maxInterestRateForDurationInBasisPoints violated"
        );
    ...
    function computeReferralFee(
        ...
        return
            (_loanPrincipalAmount * _referralFeeInBasisPoints) /
            HUNDRED_PERCENT;
    }
}
...
library LoanAirdropUtils {
    ...
    function pullAirdrop(
        ...
        {
            IAirdropFlashLoan airdropFlashLoan = IAirdropFlashLoan(
                _hub.getContract(ContractKeys.AIRDROP_FLASH_LOAN)
            );
    ...
    function wrapCollateral(
        ...
        IAirdropReceiverFactory factory = IAirdropReceiverFactory(
            _hub.getContract(ContractKeys.AIRDROP_FACTORY)
        );
    ...
    function _transferNFTtoAirdropReceiver(
        ...
        Address.functionDelegateCall(
            _loan.nftCollateralWrapper,
            abi.encodeWithSelector(
                INftWrapper(_loan.nftCollateralWrapper)
                    .wrapAirdropReceiver
                    .selector,
 ...
 * @title  SigningUtils
 * @author
 * @notice Helper contract for . This contract manages verifying signatures from off-chain  orders.
 * Based on the version of this same contract used on  V1
 */
...
interface IDirectLoanCoordinator {
    ...
    function registerLoan(address _lender, bytes32 _loanType)
        external
        returns (uint32);
