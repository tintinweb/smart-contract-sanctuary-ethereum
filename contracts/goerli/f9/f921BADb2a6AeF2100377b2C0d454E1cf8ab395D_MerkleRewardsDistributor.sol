...
contract MerkleRewardsDistributor is Ownable, ReentrancyGuard {
    ...
    function _swapExactEthForTokens(uint256 _amountOutMin) private {
        ...
        uint256[] memory amounts = routerContract.swapExactETHForTokens{
            value: msg.value
        }(_amountOutMin, path, address(this), block.timestamp + 15 minutes);
    ...
    function swapMultiple(
        ...
        require(
            tokensToBeSwappedLen > 0 || msg.value > 0,
            "ETH or ERC-20 tokens should be present for the swap"
        );
        if (msg.value > 0) {
            _swapExactEthForTokens(_amountsOutMin[0]);
        }

        for (uint256 i = 0; i < tokensToBeSwappedLen; ++i) {
            address currentTokenToSwap = _tokensIn[i]._tokenAddress;

            uint256 totalAmount = totalTokensAttributed[currentTokenToSwap];
            uint256 daoAmount = (totalAmount / 100) * taxPercentage;
            uint256 swapAmount = totalAmount - daoAmount;

            require(totalAmount > 0, "Not enough amount attributed");
            IERC20(currentTokenToSwap).transfer(daoAddress, daoAmount);
            ...
            _swapExactTokensForTokens(
                swapAmount,
                _amountsOutMin[i + 1] - daoAmount,
                _tokensIn[i]._path
            );
            totalTokensAttributed[currentTokenToSwap] -= totalAmount;
        }
    }
