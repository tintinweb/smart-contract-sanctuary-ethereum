...
contract ZporeMetadataRenderer is IMetadataRenderer, MetadataRenderAdminCheck {
    ...
    function traitsMetadata(ZporeRemix memory remix) internal view returns (string memory) {
        string memory traits = "\"attributes\": [";
        for (uint256 i=0; i < remix.stemIds.length; i++) {
            traits = string(
                abi.encodePacked(
                    traits,
                    "{",
                    "\"trait_type\": \"Stem #", Conversion.uint2str(remix.stemIds[i]), "\",",
                    "\"value\": ", Conversion.uint2str(remix.stemPercentages[i]),
                    "}"));
            if (i < remix.stemIds.length - 1) {
                traits = string(
                    abi.encodePacked(
                        traits,
                        ", "));
