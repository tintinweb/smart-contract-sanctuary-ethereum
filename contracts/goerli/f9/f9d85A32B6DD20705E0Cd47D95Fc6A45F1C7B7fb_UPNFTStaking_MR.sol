...
contract UPNFTStaking_MR is Ownable, ReentrancyGuard {
    ...
    function accumulatedRewardPerNFT(address token) public view returns (uint256) {
        if (totalStaked == 0) {
            return accRewardPerNFT[token];
        } else {
            return accRewardPerNFT[token] + ((lastTimeRewardApplicable(token) - lastUpdateTime[token]) * rewardRate[token] * 1e18) /
            totalStaked;
        }
    }
    ...
    function earned(address token) public view returns (uint256 reward) {
        for (uint256 i; i < stakedNFTs[msg.sender].length; i++) {
            reward += (accumulatedRewardPerNFT(token) - abortedRewardPerNFT[token][stakedNFTs[msg.sender][i]]) / 1e18;
        }
    }
     ...
     */
    function notifyRewardAmount(address token, uint256 amount) external onlyRewardDistributor {
        bool exist;
        for (uint256 i; i < rewardTokens.length; i++) {
     ...
     */
    function setRewardDuration(address token, uint256 period) external onlyRewardDistributor {
        _updateRewardConditions();
    ...
    function _updateRewardConditions() internal {
        for (uint256 i; i < rewardTokens.length; i++) {
            accRewardPerNFT[rewardTokens[i]] = accumulatedRewardPerNFT(rewardTokens[i]);
    ...
    function _updateAbortedReward(uint256[] memory tokenIds) internal {
        for (uint256 i; i < rewardTokens.length; i++) {
            for (uint256 j; j < tokenIds.length; j++) {
                abortedRewardPerNFT[rewardTokens[i]][tokenIds[j]] = accRewardPerNFT[rewardTokens[i]];
            }
        }
    }
}