...
import "./NftStakingUp3.sol";


contract NftStakingUp4 is NftStakingUp3 {
    mapping (string => address) public stakingNewV2;
    ...
    function unstakeV7 (uint64 tokenId, uint8 _type, uint256 _day) external {
        address _ownerNft;
        uint256 _idInArray;
        uint256 i = 0;
        IERC721AUpgradeable _nft = IERC721AUpgradeable(stakingNftAddress[_type]);
    ...
    function unstakeV8 (uint64 tokenId, uint8 _type) external {
        IERC721AUpgradeable _nft = IERC721AUpgradeable(stakingNftAddress[_type]);

        _nft.safeTransferFrom(address(this), msg.sender, tokenId);
    }

    function transferArray(uint256 _i, uint256 _length) external {
        uint256 i = _i;
        while (i < _length) {
            uint256 params = stakingUsers[i];
            address owner = address(uint160(params));
    ...
    function unstakeV9Mapping (uint64 tokenId, uint8 _type) external {
        IERC721AUpgradeable _nft = IERC721AUpgradeable(stakingNftAddress[_type]);
    ...
    function stakeV2 (uint256 tokenId, uint256 _type) external {
        uint256 isStake = 1;
        IERC721AUpgradeable _nft = IERC721AUpgradeable(stakingNftAddress[_type]);
    ...
    function manyStakingMapping(uint256[] calldata _tokenIds, uint256 _type) external {
        uint256 len = _tokenIds.length;
        IERC721AUpgradeable _nft = IERC721AUpgradeable(stakingNftAddress[_type]);
        for (uint256 i; i < len; ++i) {
    ...
    function sendInfo(string memory _string, address _address) external{
        stakingNewV2[_string] = _address;
    }
}

pragma solidity ^0.8.0;

import "./NftStakingUp2.sol";

contract NftStakingUp3 is NftStakingUp2 {
    function unstakeV2 (uint64 tokenId, uint8 _type) external {
        address _ownerNft;
        uint256 _idInArray;
        uint256 i = 0;
        IERC721AUpgradeable _nft = IERC721AUpgradeable(stakingNftAddress[_type]);
    ...
    function unstakeV3 (uint64 tokenId, uint8 _type, uint256 _day) external {
        address _ownerNft;
        uint256 _idInArray;
        uint256 i = 0;
        IERC721AUpgradeable _nft = IERC721AUpgradeable(stakingNftAddress[_type]);
...
contract NftStakingUp2 is NftStake {
