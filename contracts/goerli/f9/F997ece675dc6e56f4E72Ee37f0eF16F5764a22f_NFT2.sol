...
abstract contract NFT1 is ERC721A, Ownable {
    function burnToken(uint256 tokenId) external {}
}

contract NFT2 is ERC721A, Ownable {
    using Strings for uint256;
    ...
    function InitContract(address _address) 
    external 
    onlyOwner {
        nft1 = NFT1(_address);
    }
    ...
    function mint(uint256[] calldata nft1Ids)
        external
        payable
    {
        for (uint256 i = 0; i < nft1Ids.length; i++) {
            nft1.burnToken(nft1Ids[i]);
        }
        _mint(msg.sender, nft1Ids.length, "", false);
    }
