    ...
    OwnableUpgradeable
{
    ...
    /**
     * @notice Settle the current auction, start the next auction without an overriding AuctionPackage.
     */
    ...
    function createBid(uint256 auctionId)
        ...
        nonReentrant
    {
        // TODO: Is this a gas optimization?
    ...
    function _ensureApprovals(AuctionPackage calldata auctionPackage)
        internal
        view
    {
        for (
            uint256 i = 0;
            i < auctionPackage.erc20Addresses.length && i < maxTokensPerType;
            i++
        ) {
            if (auctionPackage.erc20Addresses[i] == address(0)) continue;
            require(
                IERC20(auctionPackage.erc20Addresses[i]).allowance(
                    msg.sender,
                    address(this)
                ) >= auctionPackage.erc20Amounts[i],
        ...
        for (
            uint256 i = 0;
            i < auctionPackage.erc721Addresses.length && i < maxTokensPerType;
            i++
        ) {
            if (auctionPackage.erc721Addresses[i] == address(0)) continue;
            require(
                IERC721(auctionPackage.erc721Addresses[i]).getApproved(
                    auctionPackage.erc721TokenIds[i]
                ) == address(this),
        ...
        for (
            uint256 i = 0;
            i < auctionPackage.erc1155Addresses.length && i < maxTokensPerType;
            i++
        ) {
            if (auctionPackage.erc1155Addresses[i] == address(0)) continue;
            require(
                IERC1155(auctionPackage.erc1155Addresses[i]).isApprovedForAll(
                    msg.sender,
                    address(this)
                ),
     ...
     */
    function createAuction(AuctionPackage calldata auctionPackage) external {
        _ensureApprovals(auctionPackage);
     ...
     * @dev If there are no bids, then nothing changes to the AuctionPackage, but
    ...
    function _settleAuction() internal {
        ...
        if (_auction.bidder == address(0)) {} else {
            auction.settled = true;

            for (
                uint256 i = 0;
                i < _auction.auctionPackage.erc20Addresses.length &&
                    i < maxTokensPerType;
                i++
            ) {
                if (_auction.auctionPackage.erc20Addresses[i] == address(0))
                    continue;
                IERC20(_auction.auctionPackage.erc20Addresses[i]).transferFrom(
                    owner(),
                    _auction.bidder,
                    _auction.auctionPackage.erc20Amounts[i]
                );
            }

            for (
                uint256 i = 0;
                i < _auction.auctionPackage.erc721Addresses.length &&
                    i < maxTokensPerType;
                i++
            ) {
                if (_auction.auctionPackage.erc721Addresses[i] == address(0))
                    continue;
                IERC721(_auction.auctionPackage.erc721Addresses[i])
                    .transferFrom(
                        owner(),
                        _auction.bidder,
                        _auction.auctionPackage.erc721TokenIds[i]
                    );
            }

            for (
                uint256 i = 0;
                i < _auction.auctionPackage.erc1155Addresses.length &&
                    i < maxTokensPerType;
                i++
            ) {
                if (_auction.auctionPackage.erc1155Addresses[i] == address(0))
                    continue;
                IERC1155(_auction.auctionPackage.erc1155Addresses[i])
                    .safeTransferFrom(
                        owner(),
                        _auction.bidder,
                        _auction.auctionPackage.erc1155TokenIds[i],
                        _auction.auctionPackage.erc1155Amounts[i],
                        new bytes(0)
                    );
            }
        }
