...
contract PancakeSwapLottery is ReentrancyGuard, IPancakeSwapLottery, Ownable {
    ...
    uint256 public constant MIN_LENGTH_LOTTERY = 10 minutes - 5 minutes; // 5 minutes
    ...
    uint256 public feeAfterDrawLottery0 = 2000; // 20%
    // case there NO is a winner
    uint256 public feeAfterDrawLottery1 = 4000; // 20%
    ...
    event IsThereWinner(uint256 indexed lotteryId, bool isThere, uint256 fee);
        ...
        nonReentrant
    {
        ...
        if (_autoInjection) {
            //  Modify fee, if there is winner so fee 20%, else so fee 40%
            uint256 feeAfterDraw;
            // Case there is winner
            if(amountToWithdrawToTreasury < amountToShareToWinners) {
                feeAfterDraw = ( amountToWithdrawToTreasury * feeAfterDrawLottery0 ) / 10000;
                emit IsThereWinner(currentLotteryId, true, feeAfterDraw);
            }
            // Case there is no winner
            if(amountToWithdrawToTreasury >= amountToShareToWinners) {
                feeAfterDraw = ( amountToWithdrawToTreasury * feeAfterDrawLottery1 ) / 10000;
                emit IsThereWinner(currentLotteryId, false, feeAfterDraw);
            }

            pendingInjectionNextLottery = amountToWithdrawToTreasury - feeAfterDraw;
            amountToWithdrawToTreasury = 0;

            // To testing, later u can remove this to reduce gas
            // amountToWithdrawToTreasury = feeAfterDraw;
            // Transfer Fee ( Fee winner ) to treasury address
            cakeToken.safeTransfer(treasuryAddress, feeAfterDraw);
        }
    ...
    function recoverWrongTokens(address _tokenAddress, uint256 _tokenAmount) external onlyOwner {
     ...
     /**
     * @notice Set fee after draw lottery
     * @param _fee0: feeAfterDrawLottery0
     * @param _fee1: feeAfterDrawLottery1
     * @dev Only callable by owner
     */
    function setMaxNumberTicketsPerBuy(uint256 _fee0, uint256 _fee1) external onlyOperator {
        require(_fee0 >= 0 && _fee0 <= 10000, "Must be 0 <= fee0 <= 10000");
        require(_fee1 >= 0 && _fee1 <= 10000, "Must be 0 <= fee1 <= 10000");
        feeAfterDrawLottery0 = _fee0;
        feeAfterDrawLottery1 = _fee1;
    }
