...
contract Hood is Ownable, ERC721A {
    enum Status {
        close,
        whitelist,
        open
    ...
    modifier mintChecker(Status _status, uint256 _price) {
        if (status != _status) revert NotLive();
        if (msg.sender != tx.origin) revert Restricted();
        if (totalSupply() + 1 > maxSupply) revert MaxSupply();
        if (_numberMinted(msg.sender) > 1) revert MintLimit();
        if (msg.value < _price * 1) revert InsufficientFund();
        _;
    }
    ...
    function whitelistMint(bytes32[] calldata _merkleProof)
        external
        payable
        mintChecker(Status.whitelist, whitelistPrice)
    {
    ...
    function publicMint(uint256 _quantity) external payable mintChecker(Status.open, publicPrice) {
