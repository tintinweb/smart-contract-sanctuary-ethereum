...
contract Staking {
    ...
    function claimFund(uint32 _txID,address _token) external {
        ...
        require(_token == timeTokenLock[msg.sender][_txID].token,"You have not staked such a token");
        require(block.timestamp >= testtime[msg.sender],"The time has not yet come");
        require(timeTokenLock[msg.sender][_txID].staked,"There is no steak with this id");
