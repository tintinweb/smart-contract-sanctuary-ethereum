...
contract Huralya is ERC721, ERC721URIStorage, ReentrancyGuard {
  ...
  address public lyaToken = 0xa7fF03610119bDD26d2c98d918b5af78b3ab3329;
  ...
  uint256 private tokensForEachMintPeriod = 1000;
  ...
  uint256 public stakeRewardPeriod = 1 days;
  uint256 public totalStakePeriod = 1000 days;
  ...
  mapping(uint => StakingHistory) private stakingHistory;
  ...
  modifier onlyUnlockedTokens(uint256 _tokenId) {
    require(!lockedTokens[_tokenId], 'Token is locked');
    _;
  }
  ...
  function mint(uint256 _quantity) external nonReentrant {
    ...
    for (uint256 i = 0; i < _quantity; i++) {
      ...
      tokensOfAddress[msg.sender].push(tokenId);
    }
  }

  function lockToken(
    uint256 _tokenId
  ) public onlyOwner onlyExistTokens(_tokenId) {
  ...
  {
    return super._transfer(msg.sender, _to, _tokenId);
  }
  ...
  function calculateStakeRewardPerDay(
    ...
    return tokenMintPrice[_tokenId].mul(100000).div(1000000000000000000);
  }
  ...
  function historyOfTotalStakeDays(
    uint256 _tokenId
  ) public view onlyExistTokens(_tokenId) returns (uint256) {
    StakingHistory storage history = stakingHistory[_tokenId];

    uint256 totalStakedDaysInHumanReadableFormat = SafeMath.div(
      history.totalStakedDays,
      1 days
    );

    return totalStakedDaysInHumanReadableFormat;
  }
  ...
  function passedStakingDays(
    ...
    uint256 passedStakingDaysInHumanReadableFormat = SafeMath.div(
      totalDaysStaked + history.totalStakedDays,
      1 days
    );

    return passedStakingDaysInHumanReadableFormat;
  }
    ...
    returns (uint256)
  {
    ...
    return
      reward.mul(
        passedStakingDays(_tokenId) - historyOfTotalStakeDays(_tokenId)
      );
  }

  function allowedStakingDaysTest(
    uint256 _tokenId
  ) public view onlyExistTokens(_tokenId) returns (uint256) {
    StakingHistory storage history = stakingHistory[_tokenId];

    uint256 totalAllowedStakingDaysInHumanReadableFormat = history
      .totalAllowedStakingDays;

    return totalAllowedStakingDaysInHumanReadableFormat;
  }
  ...
  function allowedStakingDays(
    ...
    uint256 totalAllowedStakingDaysInHumanReadableFormat = SafeMath.div(
      history.totalAllowedStakingDays,
      1 days
    );
    ...
    onlyStakedTokens(_tokenId)
  {
    ...
    require(
      ERC20(lyaToken).transferFrom(owner, ownerOf(_tokenId), profit),
      'LYA Transfer failed'
    );
    stakingHistory[_tokenId].totalAllowedStakingDays = remainingStakingDays(
      _tokenId
  ...
  function getLockedTokens(
    ...
    return lockedTokensByAddress[_address];
  }
}