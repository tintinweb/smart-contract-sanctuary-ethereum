...
/**
    ...
    @title  The Connors
    @author curion (tw: @curi0n), opensea, openzeppelin
    [ ] 1500 x 0.05 / mint
    [ ] max 3 per wallet public, public opens after whitelist (merkle tree, adding manual add as backup)
    [ ] ERC721
    [ ] 6% creator royalties
...
contract CONNORS is ERC721, DefaultOperatorFilterer, Ownable {
    ...
    uint256 public mintLimitPerTxnAndWalletPublic = 1;
    uint256 public mintLimitPerTxnAndWalletWhitelist = 1;

    mapping (address => uint256) public mintedByAddressPublic;
    mapping (address => uint256) public mintedByAddressWhitelist;
    mapping (address => bool) public whitelistStatusManualAdd;
    ...
    constructor() ERC721("The Connors", "CON") {}
    ...
    // if one-stop mint is needed, just provide placeholder proof and leaf values
    function mint(uint256 _amount, bytes32[] memory _proof, bytes32 _leaf) public payable {
        if(whitelistMintIsOpen && isValidMerkle(_proof, _leaf, msg.sender)) { 
            mintWhitelist(_amount, _proof, _leaf); 
        } else { 
            mintPublic(_amount); 
        }
    }

    function mintPublic(uint256 _amount) public payable supplyChecks(_amount) {
        
        if(!publicMintIsOpen && msg.sender != owner()) { revert MintIsClosed(); }
        if(msg.value < publicMintPrice * _amount) { revert InsufficientFunds(); }
        if(mintedByAddressPublic[msg.sender] + _amount > mintLimitPerTxnAndWalletPublic) { revert PublicConditionsNotMet(); }
        mintedByAddressPublic[msg.sender] += _amount;
    ...
    function mintWhitelist(uint256 _amount, bytes32[] memory _proof, bytes32 _leaf) public payable supplyChecks(_amount) {

        if(!whitelistMintIsOpen && msg.sender != owner()) { revert MintIsClosed(); }
        if(!isValidMerkle(_proof, _leaf, msg.sender) && !isOnWhitelistManualAdd(msg.sender) ) { revert NotOnWhitelist(); }
        if(msg.value < whitelistMintPrice * _amount) { revert InsufficientFunds(); }
        if(mintedByAddressWhitelist[msg.sender] + _amount > mintLimitPerTxnAndWalletWhitelist) { revert PublicConditionsNotMet(); }
        mintedByAddressWhitelist[msg.sender] += _amount;
    ...
    function addToWhitelistManually(address[] memory _whitelistAddresses) public onlyOwner {
        for(uint256 i=0; i<_whitelistAddresses.length; i++){
            whitelistStatusManualAdd[_whitelistAddresses[i]]=true;
        } 
    }
    ...
    function setWhitelistMintIsOpen(bool _whitelistMintIsOpen) public onlyOwner {
    ...
    /// @notice backup in case of trouble with merkle tree
    function isOnWhitelistManualAdd(address _address) public view returns (bool) {
        return whitelistStatusManualAdd[_address];
    }
