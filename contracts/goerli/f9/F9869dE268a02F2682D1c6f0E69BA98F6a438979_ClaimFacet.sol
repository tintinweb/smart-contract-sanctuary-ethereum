...
contract ClaimFacet is IClaim, ReentrancyGuard {
    ...
    function batchClaimFee(FeeClaim[] calldata _feeClaims)
        external
        override
        nonReentrant
    {
        ...
        for (uint256 i = 0; i < len; ) {
            _claimFee(
                _feeClaims[i].collateralToken,
                _feeClaims[i].recipient,
                fs
            );
            unchecked {
                ++i;
    ...
    function batchTransferFeeClaim(
        ...
        LibDIVAStorage.FeeClaimStorage storage fs = LibDIVAStorage
            ._feeClaimStorage();
        uint256 len = _feeClaimTransfers.length;
        for (uint256 i = 0; i < len; ) {
            _transferFeeClaim(
                _feeClaimTransfers[i].recipient,
                _feeClaimTransfers[i].collateralToken,
                _feeClaimTransfers[i].amount,
                fs
            );
            unchecked {
                ++i;
...
interface IClaim {
     ...
     * @param _feeClaims Struct array containing collateral token and
     * recipient addresses
     */
    function batchClaimFee(FeeClaim[] calldata _feeClaims) external;
