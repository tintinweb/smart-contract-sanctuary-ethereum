...
/**
 * @title Open3 ERC-721 Smart Contract
 */

contract Open3 is Ownable, ReentrancyGuard, ERC721Enumerable {
    string public OPEN3_PROVENANCE = "";
    ...
    address public SIGNING_ADDRESS;
    ...
    uint256 public constant MAX_TOKENS_PER_TRANSACTION = 10;
    bool public qualifiedIsActive = false;
    mapping(uint256 => bool) public qualifiedNonceList;
    mapping(address => uint256) public qualifiedWalletList;

    constructor(address signingAddress, address withdrawAddress)
        ERC721("Open3", "O3")
    {
    ...
    function flipQualifiedState() external onlyOwner {
        qualifiedIsActive = !qualifiedIsActive;
    }

    function mintQualified(
        uint256 numberOfTokens,
        bytes memory sig,
        uint256 nonce
    ) external payable nonReentrant {
        require(qualifiedIsActive, "Qualified is not active");
        require(
            !qualifiedNonceList[nonce],
        ...
        require(
            qualifiedWalletList[msg.sender] + numberOfTokens <=
                MAX_TOKENS_PER_ADDRESS,
            "You can't mint anymore!"
        );
        require(
        ...
        require(
            ECDSA.recover(message, sig) == SIGNING_ADDRESS,
            "Bad signature"
        );

        qualifiedNonceList[nonce] = true;
        qualifiedWalletList[msg.sender] += numberOfTokens;

        //_safeMint(msg.sender, numTokensMinted + numberOfTokens);
    ...
    function withdraw() external onlyOwner {
        ...
        Address.sendValue(payable(WITHDRAW_ADDRESS), balance);
    }
    ...
    function setSigningAddress(address newSigningAddress) external onlyOwner {
        SIGNING_ADDRESS = newSigningAddress;
    }
    ...
    function setProvenanceHash(string memory provenanceHash)
        ...
        onlyOwner
    {
        OPEN3_PROVENANCE = provenanceHash;
    }
