...
/// @author MetaJuice
/// @notice MetaJuice NFT contract that includes royalties and burning functionality
contract MetaJuiceCollection_Copper is ERC721Royalty, ERC721Burnable, Ownable {
    using Strings for uint256;

    /// @dev IMX Core (StarkEx Bridge) Contract address that is granted access to call mintFor function
    address public imx;

    /// @dev tokenURI will be baseUri/<blueprint>/<tokenId>
    string private baseUri;
    
    /// @dev maps tokenId to bytes that is the onchain metadata, generally the IPFS content hash
    mapping(uint256 => bytes) private blueprints;

    /// @dev Event for when an asset is minted
    event AssetMinted(address indexed to, uint256 indexed tokenId, bytes blueprint);

    /// @dev Only the owner of the contract or IMX is granted access to call mintFor function
    modifier onlyOwnerOrIMX() {
        require(msg.sender == imx || msg.sender == owner(), "Function can only be called by owner or IMX");
        _;
    }

    /// @param owner_ The wallet that is the owner of this minting contract
    /// @param name_ The name of this NFT collection
    /// @param symbol_ The symbol for this NFT collection
    /// @param imx_ is 0x5FDCCA53617f4d2b9134B29090C87D01058e27e9 for mainnet https://github.com/immutable/imx-contracts/blob/4a6dc460292ffa9a352d3043a4d9019d8f316a59/deploy/utils.ts#L1
    /// @param baseUri_ The baseURI for the tokenURI metadata URL
    /// @param royaltyRecipient The wallet that will receive royalties
    /// @param royaltyNumerator The numerator fee for royalties
    constructor(
        address owner_,
        string memory name_,
        ...
        ERC721(name_,symbol_)
    {
        ...
        require(imx_ != address(0), "IMX Address must not be empty");
        transferOwnership(owner_);
        imx = imx_;
        _setDefaultRoyalty(royaltyRecipient, royaltyNumerator);
        baseUri = baseUri_;
    }
    ...
    function mintFor(address to, uint256 quantity, bytes calldata mintingBlob) external onlyOwnerOrIMX {
        require(quantity == 1, "Mintable: invalid quantity");
        
        /// @dev mintingBlob consists of {tokenId:blueprint} 
        ...
        _setTokenURI(tokenId, blueprint);

        emit AssetMinted(to, tokenId, blueprint);
    }
    ...
    /// @dev Will rewrite tokenURI when NFT is withdrawn to L1 Ethereum to be baseURI/<blueprint>/<tokenId>
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        _requireMinted(tokenId);

        string memory _blueprint = string(blueprints[tokenId]);
        ...
        if (bytes(_blueprint).length > 0) {
            return string(abi.encodePacked(base, _blueprint, "/", tokenId.toString()));
        }

        return super.tokenURI(tokenId);
    }

    function _setTokenURI(uint256 tokenId, bytes memory _blueprint) internal virtual {
    ...
    function _burn(uint256 tokenId) internal override(ERC721, ERC721Royalty) {
        ...
        if (bytes(blueprints[tokenId]).length != 0) {
