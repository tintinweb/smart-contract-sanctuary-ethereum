...
contract JosieNftBox is ERC721, Ownable, WithOperatorFilter {
	 ...
	 **/
	function setIsOperatorFilterEnabled(bool _setting) public onlyOwner {
		...
		onlyOwner
	{
		...
		// This is lossy - it automatically truncates to a whole number
		...
		for (uint256 idx; idx < tokensToRoll; idx++) {
			...
			_tokenId = _pullRandomUniqueIndex(currentPrng, leftToRoll, 1, _idSwaps);

			// Generate the revealed token variant
			revealedTokenVariant[_tokenId] = 1 + (_prng(currentPrng, leftToRoll, 42) % (NUM_REVEALED_VARIANTS));

			// Pull the prize index
			_prizeIndex = (leftToRoll == 1) ? 500 : _pullRandomUniqueIndex(_prng(currentPrng, leftToRoll, 69), leftToRoll - 1, 0, _prizeDrawnSwaps);
		...
		returns (uint256)
	{
		...
		// get a random id
		uint256 index = indexOffset + (currentPrng % leftToRoll);
