    ...
    SVGFactory
{
    ...
    event ReceivedRandomness(uint256 tokenId, uint256 randomNumber);
    ...
    function fulfillRandomWords(uint256 requestId, uint256[] memory randomness) internal override {
        ...
        emit ReceivedRandomness(tokenId, randomness[0]);
    }

    function completeMint(uint256 _tokenId) external tokenExists(_tokenId) {
        if (tokenIdToRandomness[_tokenId] == 0) {
            revert SVGNFT__PendingRandomNumber();
        }
