...
contract Base1155 is ERC1155, DefaultOperatorFilterer, OwnableRoyalties, Pausable, ERC1155Burnable, ERC1155Supply {
    ...
    // Max supply of each id
    mapping(uint256 => uint256) public maxSupply;
    // total number of unique ids
    Counters.Counter numOfNFTs;
    // total number of mints
    Counters.Counter totalMints;
    // max number of mints possible total.
    uint256 public maxMints;
    // price (not used currently. on logic contract)
    uint256 public price = 0.001 ether;
    // current number of total mints available. 
    uint256 public totalSupplies;

    // Royality fee BPS (1/100ths of a percent, eg 1000 = 10%)
    uint16 private immutable _feeBps = 500;

    error NotEnoughLeftToMint(uint256 [] ids);

    string public name;

    constructor(string memory initName, uint256[] memory amounts) ERC1155("") {
        name = initName;
        for (uint256 i = 0; i < amounts.length; i++) {
            maxSupply[i] = amounts[i];
            totalSupplies += amounts[i];
            numOfNFTs.increment();
        }
    }
    ...
    function getMintsRemaining() public view returns (uint256) {
        return totalSupplies - totalMints.current();
    }
    
    function getTotalNFTs() public view returns (uint256) {
        return numOfNFTs.current();
    }
    ...
    function getInfo() public view returns (uint256, uint256, uint256) {
        return (getTotalMints(), getMintsRemaining(), getTotalNFTs());
    }

    function setMintContract(address newContract) public onlyOwner {
        require(newContract != address(0), "Invalid address");
        MintLogic = newContract;
    }

    function setForgeContract(address newContract) public onlyOwner {
        require(newContract != address(0), "Invalid address");
        ForgeLogic = newContract;
    }
    ...
    function setMaxSupply(uint256 id, uint256 newMax) public onlyOwner {
        maxSupply[id] = newMax;
    }

    // @notice Helper function for the front end to get a users balance of tokens
    // @param a - users address
    // @return Array of balances of all tokens
    function getAddressTotals(address a) public view returns (uint256[] memory) {
        uint256[] memory result = new uint256[](numOfNFTs.current());
        for (uint256 i = 0; i < numOfNFTs.current(); i++) {
            result[i] = balanceOf(a, i);
        }
        return result;
    }

    // @notice Helper function to get available IDs for minting
    function getSupplies() external view returns (uint256[] memory) {
        uint256[] memory supplies = new uint256[](numOfNFTs.current());
        uint256 idx = 0;
        for(uint256 i = 0; i < numOfNFTs.current(); i++) {
            if (totalSupply(i) < maxSupply[i]) {
                supplies[idx] = i;
                idx++;
            }
        }
        uint256[] memory ids = new uint256[](idx);
        for(uint256 i = 0; i < ids.length; i++) {
            ids[i] = supplies[i];
        }
        return (ids);
    ...
    // @notice burn function used by the forging contract
    function burn(address account, uint256 id, uint256 amount) public virtual override {
        require(ForgeLogic != address(0), "Invalid forging contract address");
        require(ForgeLogic == _msgSender(), "ERC1155: caller is not owner nor approved");
        _burn(account, id, amount);
    }

    // @notice mint function used by the minting contract
    function mint(address account, uint256 id, uint256 amount, bytes memory data)
        public
        returns (bool)
    {
        require(msg.sender == MintLogic, "Must be the master contract");
        // require(totalSupply(id) + amount <= maxSupply[id], "Not enough left of that token.");
