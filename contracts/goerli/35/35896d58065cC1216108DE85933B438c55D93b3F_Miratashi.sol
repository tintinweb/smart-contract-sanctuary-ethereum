...
contract Miratashi is ERC721M, Ownable, ERC2981, ReentrancyGuard {
    ...
    function mintDutchAuction(uint8 _quantity) external payable callerIsUser {
        ...
        // Check is active
        require(isAuctionActive, "Miratashi: Public phase is not active.");
        ...
        // Require max 2 per transaction
        require(
            _quantity <= MAX_MINT_TXN_SIZE,
        ...
        if (totalSupply() + _quantity == MINT_CAPACITY) {
    ...
    function tokenURI(
        ...
        if (!isRevealed) return string(abi.encodePacked(tokenBaseURI));
    ...
    function setWhitelistQuantity(uint256 _quantity) external onlyOwner {
        require(
            (TEAM_TREASURY_SUPPLY + totalSupply() + _quantity) <= MINT_CAPACITY,
