  ...
  IERC721ReceiverUpgradeable
{
  ...
  event DonationPlaced(address from, uint256 grantsId, uint256 amount);
  event DAOWalletAddressSet(address walletAddress);
  event nftAuthorWalletAddressSet(address nftAuthorWallet);
  event NFTsentToWinner(uint256 grantsID, address winner);
  ...
  /**
        @notice sets NFT author wallet address for transfering NFT at the end of grants cycle
  ...
  /**
    ...
    */
  function withdrawDonations(address payable account, uint256 amount)
    public
    onlyRole(DEFAULT_ADMIN_ROLE)
  {
    ...
    if (address(this).balance < amount)
      revert InsufficientAmount("Insufficient balance to withdraw amount");
    ...
    returns (uint256)
  {
    ...
    if (_grants.minimumDonationAmount <= 0)
      revert MinimumDonationCantBeZero("Amout has to be higher than zero");

    grantsCount++;
    // Set the id of the grants in the grants struct
    _grants.grantsID = grantsCount;
  ...
  /**
        ...
        @param grantsID id of grants
    */
  function cancelGrant(uint256 grantsID) public onlyRole(CURATOR_ROLE) {
    if (
      (grants[grantsID].ended == true) ||
      (grants[grantsID].topDonatedAmount > 0 &&
        grants[grantsID].endTime < block.timestamp)
    ) revert GrantHasEnded("Grant already ended", grantsID); // check this logic
    grants[grantsID].cancelled = true;

    address nftContractAddress = grants[grantsID].nftContract;

    // send NFTs back to owner
    IERC721Upgradeable(nftContractAddress).safeTransferFrom(
      address(this),
      grants[grantsID].nftOwner,
      grants[grantsID].tokenID1,
      ""
    );
  ...
  /**
        ...
        @param _grantsId grants ID
        @param _amount amount to donate
    */
  function donate(uint256 _grantsId, uint256 _amount)
    public
    payable
    nonReentrant
  {
    ...
    if (grants[_grantsId].endTime < block.timestamp)
      revert GrantHasEnded("Grant already ended", _grantsId);
    if (_amount < grants[_grantsId].minimumDonationAmount)
      revert DonationTooLow("Donation has to be higher");

    totalDonationPerAddressPerCycle[_grantsId][msg.sender] += _amount;

    if (
      grants[_grantsId].topDonatedAmount <
      totalDonationPerAddressPerCycle[_grantsId][msg.sender]
    ) {
      grants[_grantsId].topDonor = msg.sender;
      grants[_grantsId].topDonatedAmount = totalDonationPerAddressPerCycle[
        _grantsId
      ][msg.sender];
    }

    emit DonationPlaced(msg.sender, _grantsId, _amount);
  }
  ...
  /**
        ...
        @param grantsID id of grants
    */
  function sendRewards(uint256 grantsID) public onlyRole(CURATOR_ROLE) {
    if (grants[grantsID].endTime > block.timestamp)
      revert GrantHasNotEnded("Grant is still running", grantsID);
    if (grants[grantsID].cancelled == true)
      revert GrantCancelled("Grant has been cancelled", grantsID);
    if (grants[grantsID].ended == true)
      revert GrantHasEnded("Grant already ended", grantsID);
    ...
    IERC721Upgradeable(nftContractAddress).safeTransferFrom(
      address(this),
      grants[grantsID].topDonor,
      grants[grantsID].tokenID1,
      ""
    );

    emit NFTsentToWinner(grantsID, grants[grantsID].topDonor);
    ...
    emit NFTsentToWinner(grantsID, DAOWallet);
    ...
    emit NFTsentToWinner(grantsID, nftAuthorWallet);

    // Send Grant total donations to DAOWallet
    DAOWallet.transfer(address(this).balance);
    emit DonationsTransferred(DAOWallet, grantsID, address(this).balance);
  }
  ...
  function getGrant(uint256 grantsID) public view returns (Grant memory) {
    return grants[grantsID];
  }
