...
import "./interfaces/IOrganisationHelper.sol";
...
contract OrganisationHelper is IOrganisationHelper, Initializable, UUPSUpgradeable, AccessControlUpgradeable {
    ...
    function getOrganisationDetails(address orgRegistry, bytes32 orgId) public view returns (OrganisationDetails memory) {
        ...
        IOrganisationRegistry.DAOV1Config memory daoV1,
        IOrganisationRegistry.YieldFarmingV1Config memory yieldFarmingV1,
        IOrganisationRegistry.VestingV1Config memory vestingV1,
        IOrganisationRegistry.AirdropV1Config memory airdropV1
        ) = OrganisationRegistry(orgRegistry).organisations(orgId);

        require(exists, "OrganisationHelper: organisation does not exist");

        OrganisationDetails memory details;
        details.name = name;
        details.owner = owner;
        details.rewardsEscrowV1 = rewardsEscrowV1;

        if (token != address(0)) {
            details.tokenV1 = getTokenDetails(token);
        }

        if (daoV1.staking != address(0)) {
            details.daoV1.staking = getDAOV1_StakingDetails(daoV1.staking);
        }

        if (daoV1.governance != address(0)) {
            details.daoV1.governance = getDAOV1_GovernanceDetails(daoV1.governance);
        }

        details.daoV1.rewards = new DAOV1_RewardsDetails[](daoV1.rewards.length);
        for (uint i = 0; i < daoV1.rewards.length; i++) {
            details.daoV1.rewards[i] = getDAOV1_RewardsDetails(daoV1.rewards[i]);
        }

        if (yieldFarmingV1.staking != address(0)) {
            details.yieldFarmingV1.staking = getYieldFarmingV1_StakingDetails(yieldFarmingV1.staking);

            details.yieldFarmingV1.pools = new YieldFarmingV1_PoolDetails[](yieldFarmingV1.pools.length);
            for (uint256 i = 0; i < yieldFarmingV1.pools.length; i++) {
                details.yieldFarmingV1.pools[i] = getYieldFarmingV1_PoolDetails(yieldFarmingV1.pools[i]);
            }
        }

        details.vestingV1 = new VestingV1Details[](vestingV1.epochs.length + vestingV1.cliffs.length);
        for (uint256 i = 0; i < vestingV1.epochs.length; i++) {
            details.vestingV1[i] = getVestingDetails(vestingV1.epochs[i]);
        }

        for (uint256 i = 0; i < vestingV1.cliffs.length; i++) {
            details.vestingV1[i + vestingV1.epochs.length] = getVestingDetails(vestingV1.cliffs[i]);
        }

        details.airdropV1 = new AirdropV1Details[](airdropV1.simple.length + airdropV1.gamified.length);
        for (uint256 i = 0; i < airdropV1.simple.length; i++) {
            details.airdropV1[i] = getAirdropV1Details(airdropV1.simple[i]);
        }

        for (uint256 i = 0; i < airdropV1.gamified.length; i++) {
            details.airdropV1[i + airdropV1.simple.length] = getAirdropV1Details(airdropV1.gamified[i]);
        }
    ...
    function getYieldFarmingV1_PoolState(address ctr) public view returns (YieldFarmingV1_PoolState memory) {
        ...
        state.effectivePoolSize = pool.getPoolSize(state.currentEpoch);
        state.poolSize = pool.getPoolSize(state.currentEpoch + 1);
        ...
        for (uint256 i = 0; i < poolTokens.length; i++) {
            ...
            stateByToken.effectivePoolSize = pool.getPoolSizeByToken(poolTokens[i], state.currentEpoch);
            stateByToken.poolSize = pool.getPoolSizeByToken(poolTokens[i], state.currentEpoch + 1);
    ...
    function _authorizeUpgrade(address) internal view override {
        require(hasRole(UPGRADER_ROLE, msg.sender), "OrganisationHelper: caller does not have upgrader role");
    }
}
...
contract OrganisationRegistry is IOrganisationRegistry, Initializable, UUPSUpgradeable, AccessControlUpgradeable {
    ...
    function deployYieldFarmingV1Staking(
        ...
        emit OrganisationAddedModule(id, Modules.YF_V1_STAKING, upgrader != address(0), staking);
    }

    function deployYieldFarmingV1Pool(
        bytes32 id,
        IYieldFarmingV1Pool.YieldFarmConfig memory cfg,
        address roleAdmin,
        address upgrader
    ) public payable {
        Organisation storage org = _getOrganisationWithOwnerOrRevert(id, msg.sender);
        ...
        emit OrganisationAddedModule(id, Modules.YF_V1_POOL, upgrader != address(0), pool);
    }
    ...
    function deployVestingV1Epochs(
        ...
        emit OrganisationAddedModule(id, Modules.VESTING_V1, upgrader != address(0), vesting);
    }
    ...
    function deployVestingV1Cliffs(
        ...
        emit OrganisationAddedModule(id, Modules.VESTING_CLIFFS_V1, upgrader != address(0), vestingCliffs);
    }
    ...
    function deployAirdropV1Simple(
        ...
        emit OrganisationAddedModule(id, Modules.AIRDROP_SIMPLE_V1, upgrader != address(0), airdropSimple);
    }
    ...
    function deployAirdropV1Gamified(
        ...
        emit OrganisationAddedModule(id, Modules.AIRDROP_GAMIFIED_V1, upgrader != address(0), airdropGamified);
    }
...
import "./IOrganisationRegistry.sol";
import "./IVestingV1Cliffs.sol";
import "./IDAOV1Rewards.sol";

interface IOrganisationHelper {
