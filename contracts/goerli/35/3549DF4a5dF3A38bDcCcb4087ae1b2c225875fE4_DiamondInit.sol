...
contract DiamondInit {
    ...
    uint64 private constant trainingMissionLength = 2 days;
    ...
    function initTokenFacet() public {
        ...
        s1.baseURI = baseURI;
        s1.trainingMissionLength = trainingMissionLength;
...
library AuctionFacetLib {
    ...
    function encode(address bidder, uint256 quantity, uint256 price) internal pure returns(uint256 result) {
        result = uint256(uint160(bidder));
        result |= quantity<<160;
        result |= price<<168;
    }

    function decode(uint256 result) internal pure returns(address bidder, uint256 quantity, uint256 price) {
        bidder = address(uint160(result));
        quantity = uint8(result>>160);
        price = uint88(result>>168);
    }
}
...
contract AuctionFacet {
    ...
    function bid(uint256 quantity, uint256 price) external payable {
        if(!(msg.value == quantity * price)) revert("Invalid Amount");
        (bool sent,) = AuctionFacetLib.getState().treasureContract.call{value: msg.value}("");
        if(!sent) revert("Bid Failed");
        AuctionFacetLib.getState().bids[Counters.current(counter)] = AuctionFacetLib.encode(msg.sender, quantity, price);
        Counters.increment(counter);
        emit AuctionFacetLib.bid(msg.sender, quantity, price);
    }

    function bid1(uint256 quantity, uint256 price) external payable {
        if(!(msg.value == quantity * price)) revert("Invalid Amount");
        (bool sent,) = AuctionFacetLib.getState().treasureContract.call{value: msg.value}("");
        if(!sent) revert("Bid Failed");
        AuctionFacetLib.getState().bids1.push(AuctionFacetLib.encode(msg.sender, quantity, price));
        emit AuctionFacetLib.bid(msg.sender, quantity, price);
    }

    function getBids() external view returns(bidStruct[] memory _data) {
        _data = new bidStruct[](Counters.current(counter));
        for(uint256 i = 0; i < Counters.current(counter); i++) {
            (address bidder, uint256 quantity, uint256 price) = AuctionFacetLib.decode(AuctionFacetLib.getState().bids[i]);
            _data[i].bidder = bidder;
            _data[i].quantity = quantity;
            _data[i].price = price;
        }
    }

    function getBids1() external view returns(bidStruct[] memory _data) {
        uint256[] memory bids = AuctionFacetLib.getState().bids1;
        _data = new bidStruct[](bids.length);
        for(uint256 i = 0; i < bids.length; i++) {
            (address bidder, uint256 quantity, uint256 price) = AuctionFacetLib.decode(bids[i]);
            _data[i].bidder = bidder;
            _data[i].quantity = quantity;
            _data[i].price = price;
        }
