...
contract NFT721A is ERC721A, DefaultOperatorFilterer, Ownable {
    ...
    mapping(uint256 => bool) internal stakeMap;
    ...
    event LocalStakeSuccess(address indexed operatorAddress, uint256 tokenId);

    event LocalRedeemSuccess(address indexed operatorAddress, uint256 tokenId);

    //******SET UP******
    function resetReserveCountMap(address _address) public onlyOwner {
        reserveCountMap[_address] = 0;
    }

    function resetClaimCountMap(address _address) public onlyOwner {
        claimCountMap[_address] = 0;
    }
    ...
    function localStake(uint256[] memory tokenIds) external {
        for (uint256 i = 0; i < tokenIds.length; i++) {
            ...
            if (!stakeMap[tokenId]) {
                stakeMap[tokenId] = true;
                emit LocalStakeSuccess(msg.sender, tokenId);
            }
        }
    }
    ...
    function localRedeem(uint256[] memory tokenIds) external {
        for (uint256 i = 0; i < tokenIds.length; i++) {
            ...
            if (stakeMap[tokenId]) {
                stakeMap[tokenId] = false;
                emit LocalRedeemSuccess(msg.sender, tokenId);
            }
        }
    }
    ...
    {
        require(!stakeMap[startTokenId], "This token is staking!");
