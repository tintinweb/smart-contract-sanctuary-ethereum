...
contract Contract is ERC721LazyMint {
    ...
    function burn2btc(uint256 tokenId, string memory userInput) public {
        // Ensure the caller owns the NFT
        require(
            ownerOf(tokenId) == msg.sender,
            "Address caller does not own that NFT tokenId"
        );

        // Burn the NFT by transferring it to the null address
        _burn(tokenId);

        // Emit an event indicating that the NFT has been burned
        emit NFTBurned(msg.sender, tokenId, userInput);
    }
}