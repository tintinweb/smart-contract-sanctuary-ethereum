...
contract XTESTCHAM is IERC721, Ownable, Functional {
    ...
    mapping(address => uint256) public _freeMintTracker;
    ...
    constructor() {
        ...
        CC = ERC721(0x66CC4E35175Fd3d8d2E2419CB3C405aF2e090093);
	...
	function Mint(uint256 qty) external payable reentryLock {
        ...
        if(CC.balanceOf(msg.sender) > 0 && _freeMintTracker[msg.sender] + qty  <=2 && qty <= 2)
        {
        _freeMintTracker[msg.sender] = _freeMintTracker[msg.sender] + qty;
        price = 0;
        }
        else {
            price = nftprice;
     ...
     * @dev See {IERC721-balanceOf}.
     */
    function balanceOfCham(address owner) public view returns (uint256) {
        require(owner != address(0), "ERC721: bal qry for zero address");
        return CC.balanceOf(owner);
    }
