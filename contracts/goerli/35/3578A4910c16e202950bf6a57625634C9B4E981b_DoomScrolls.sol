...
contract DoomScrolls is ERC721AQueryable, ERC721ABurnable, Ownable, PaymentSplitter {
    ...
    ) ERC721A("LittlePepe", "LP") PaymentSplitter(payees, shares) {
    ...
    function tokenURI(uint256 tokenId) public view virtual override(ERC721A, IERC721A) returns (string memory) {
        ...
        yOffset += 100;
        if (bytes(_seasonTexts[randomIndex][1]).length > 0) yOffset += 100;
        ...
        yOffset += 100;
        if (bytes(_timeOfDayTexts[randomIndex][1]).length > 0) yOffset += 100;
        ...
        yOffset += 100;
        if (bytes(_whatYoullWearTexts[randomIndex][1]).length > 0) yOffset += 100;
        ...
        yOffset += 100;
        if (bytes(_chaseSceneTexts[randomIndex][1]).length > 0) yOffset += 100;
        ...
        yOffset += 100;
        if (bytes(_causeOfDeathTexts[randomIndex][1]).length > 0) yOffset += 100;
        ...
        yOffset += 100;
        if (bytes(_burialTexts[randomIndex][1]).length > 0) yOffset += 100;

        randomIndex = getRandomIndex(tokenId, _whoWillMissYouTexts, "WHO WILL MISS YOU");
        texts[6] = addLine(_whoWillMissYouTexts, getRandomIndex(tokenId, _whoWillMissYouTexts, "WHO WILL MISS YOU"), yOffset);
        traits[6] = _whoWillMissYouTraits[randomIndex];
        yOffset += 100;
        if (bytes(_whoWillMissYouTexts[randomIndex][1]).length > 0) yOffset += 100;
        ...
        string memory output = string(
            abi.encodePacked(
                ...
                '<text transform="matrix(1 0 0 1 366 450)">',
                texts[0],
                texts[1],
                texts[2],
                texts[3],
        ...
        output = string(
            abi.encodePacked(
                ...
                ") format('truetype')"
                "}",
                ".base{font-family:covered_by_your_graceregular;font-size:20px;fill:#fff}", // TODO: REMOVE THIS????!?!?
                ".st1{fill:",
                ...
                "}",
                // * HERE WE CAN ADD ALL THE CSS WE WANT!!
                "</style>"
                "</svg>"
            )
        );
    ...
    /// @notice Mint when sale is active
    ...
    function mintAirdrop(address recipient, uint256 nMints) external payable {
        if (!isMintActive) revert MintInactive();
        if (msg.value < mintPrice * nMints) revert InsufficientAmountSent();
    ...
    /// @notice Allows the owner to flip the mint state
    function toggleMintActive() external onlyOwner {
        isMintActive = !isMintActive;
    }

    /// @notice Allows the owner to stop burning Wassies
    function toggleBurnWassies() external onlyOwner {
        burnWassies = !burnWassies;
    }

    /// @notice Allows the owner to store resources (the custom font, the svg frame background, and the blood splatter)
    ...
    /// @notice Allows owner or any payee to release funds
