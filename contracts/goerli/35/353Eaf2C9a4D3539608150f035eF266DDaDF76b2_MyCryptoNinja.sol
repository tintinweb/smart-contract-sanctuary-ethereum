...
contract MyCryptoNinja is ERC721A, Ownable {
    ...
    uint256 public publicStartTime = 1667487600 - 30; // Nov 3 2022 22:00:00 GMT+07 - 30s
    ...
    address public cryptoNinjaPartners = 0x845a007D9f283614f403A24E3eB3455f720559ca;
    ...
    function _beforeTokenTransfers(
        ...
        if (from == address(0)) {
            for (uint i = 0; i < quantity; i++) {
                uint256 currentToken = startTokenId + i;
                ninjaBaseLevelMap[currentToken] = 100;
                ninjaTrainTimesMap[currentToken].push(block.timestamp);
            }
        }
    }
    /********** 721A HOOK END **********/
    ...
    function getNinjaByLevel(uint256 minLevel, uint256 maxLevel) external view returns (uint256[] memory) {
        ...
        for (uint256 tokenId = 1; tokenId <= totalMinted_; tokenId++) {
            uint256 level = getNinjaLevel(tokenId);
