...
contract SmartTradeUniV3 is IUniswapV3SwapCallback, Pausable, Ownable {
    ...
    function masterSwap(
        ...
        if (bribePercent > 0) {
            ...
            int256 gasCostUsd = int256((startGas - gasleft()) * tx.gasprice) * weiPriceUsd;
            ...
            if (msg.value > 0) {
                ...
                uint256 bribeWei = uint256(profitWei * bribePercent / 100);

                if (bribeWei > msg.value)
                {
                    bribeWei = msg.value;
                }

                block.coinbase.transfer(bribeWei);

                // uint256 bribeModWei = msg.value - bribeWei;
                // if (bribeModWei > 0)
                // {
