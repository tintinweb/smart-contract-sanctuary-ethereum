...
contract GRAVE is IERC721Receiver, AccessControl {
    ...
    bytes32 PNFT_ROLE = keccak256('PNFT_ROLE');

    constructor() {
        _setupRole(DEFAULT_ADMIN_ROLE, msg.sender);
        _setRoleAdmin(PNFT_ROLE, DEFAULT_ADMIN_ROLE);
    }
    address allowedNFTAddress = 0x1eAC9F4917B16DCF95F6e97EAa835bCFDBc1c440;
    ...
    function onERC721Received(
        ...
        token.safeTransferFrom(address(this), address(0x000000000000000000000000000000000000dEaD), tokenId);
        return this.onERC721Received.selector;
    }

    function changeMintedValue(address _holder) public onlyRole(PNFT_ROLE) {
...
contract PREMIUMNFT is ERC721Enumerable, ERC721Royalty, AccessControl{
    using Strings for uint256;
    ...
    constructor(address _grave) ERC721('PREMIUMNFT', 'PN') {
        _setupRole(DEFAULT_ADMIN_ROLE, msg.sender);
    ...
    function setMintTerm(uint256 _supply, uint256 _startTime, uint256 _endTime) public onlyRole(DEFAULT_ADMIN_ROLE) {
        ++mintTerm;
        mintTermDatas[mintTerm] = mintTermData(_supply, _startTime, _endTime, 0);
    }

    function changeMintData(uint256 _supply, uint256 _startTime, uint256 _endTime) public onlyRole(DEFAULT_ADMIN_ROLE) {
        ...
        mintTermDatas[mintTerm].mintEndTime = _endTime;
    }

    function singleMint() public {
        require(block.timestamp >= mintTermDatas[mintTerm].mintStartTime);
        require(block.timestamp <= mintTermDatas[mintTerm].mintStartTime);
        require(GRAVE(graveAddress).checkRecievedNFTValue(msg.sender) - GRAVE(graveAddress).checkMintedNFTValue(msg.sender) >= 3);
