...
contract Staking is Ownable {
    ...
    uint256 private stakingFeeRate;
     ...
     */
    function getStakingFeeRate() public view returns (uint256) {
     ...
     */
    function setStakingFeeRate(uint256 _rate) public onlyOwner {
    ...
    /**
     * @notice Change Referral Contract
     */
    function setreferralContract(address _referral) public onlyOwner {
        referralContract = Referral(_referral);
    }

    function setReferralIncome(address _userAddress, uint256 _amount) external {
        require(
            msg.sender == address(referralContract),
            "Only referral contract"
        );
     ...
     */
    function updateAccount(address account, bool isCompound) private {
        ...
        if (pendingDivs > 0) {
            ...
            transactionHistory[msg.sender].amount.push(
        ...
        lastClaimedTime[account] = lastDistributionTime;
    }
    ...
    {
        uint256 _lastDistributionTime = getLastDistributionTime();
        uint256 timeDiff = _lastDistributionTime.sub(lastClaimedTime[_holder]);
    ...
    function deposit(uint256 amountToStake, address _referral) public {
        ...
        if (
            ...
            require(
                referralContract.setUserReferral(msg.sender, _referral),
                "Can't set user referral"
            );

            require(
                referralContract.setReferralAddressesOfUsers(
                    msg.sender,
                    _referral
                ),
