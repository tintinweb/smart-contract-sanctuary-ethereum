...
contract WimpyKids is ERC721A, Ownable {using Strings for uint256;
    ...
    // Token Price
    uint256 public tokenPrice = 0.0008 ether;
    ...
    constructor(address _signerVerificationAddress) ERC721A('Wimpy Kids', 'WMP') {
		    signer = msg.sender;
        signerVerification = ISignerVerification(_signerVerificationAddress);
	}

    function mintOnPresale(uint256 quantity, bytes memory signature) public payable {
		require(preSaleActive, "Presale closed");
        require(quantity <= maxPerWallet, "You can mint only 5 NFT");
        require(balanceOf(msg.sender) < maxPerWallet, "You can mint only 5 NFT");
        require(msg.value == tokenPrice * quantity, "Insufficient funds");
        require(totalSupply() + 1 <= _totalSupply, "All NFT already was minted");
        require(signerVerification.isMessageVerified(signer, signature, _addressToString(msg.sender)), 'Presale signature is wrong');


        _mint(msg.sender, quantity);
	}

    function mintOnPublicsale(uint256 quantity) public payable {
		require(publicSaleActive, "Publicsale closed");
        require(quantity <= maxPerWallet, "You can mint only 5 NFT");
        require(balanceOf(msg.sender) < maxPerWallet, "You can mint only 5 NFT");
        require(msg.value == tokenPrice * quantity, "Insufficient funds");
        require(totalSupply() + 1 <= _totalSupply, "All NFT already was minted");
  ...
  function airdrop(address _userAddress) public onlyOwner {
  ...
  function changeTotalSupply(uint256 newTotalSupply) public onlyOwner {
    ...
    function togglePublicSale() public onlyOwner {
		require(!preSaleActive, "Firstly deactivate presale");
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        ...
        if (reveal) {
			...
			return bytes(baseURI).length > 0 ? string(abi.encodePacked(baseURI, _extension)) : '';
    ...
    function changeCountForWallet(uint256 amount) public onlyOwner {
		maxPerWallet = amount;
	}

    function activeReveal() public onlyOwner {
		reveal = true;
	}
    ...
    function _addressToString(address _addr) internal pure returns (string memory) {
        ...
        for (uint256 i = 0; i < 20;) {uint8 leftValue = uint8(addressBytes[i]) / 16;
