...
contract CompoundProtocol is IDefiProtocol, OwnableUpgradeable, AbstractDependant {
    ...
    function setDependencies(IContractsRegistry _contractsRegistry)
        ...
        onlyInjectorOrZero
    {
        stablecoin = IERC20(_contractsRegistry.getUSDTContract());
        ...
        returns (uint256 actualAmountWithdrawn, uint256 accumaltedAmount)
    {
        ...
        if (cTokenBalance > 0) {
            uint256 res = cToken.redeem(cTokenBalance);
            if (res == ERRCODE_OK) {
                ...
                if (actualAmountWithdrawn > totalDeposit) {
                    accumaltedAmount = actualAmountWithdrawn.uncheckedSub(totalDeposit);
                    actualAmountWithdrawn = actualAmountWithdrawn.uncheckedSub(accumaltedAmount);

                    totalRewards = totalRewards.add(accumaltedAmount);
                }

                totalDeposit = totalDeposit.trySub(actualAmountWithdrawn);
            }
        }
    }
    ...
    function claimRewards() external override onlyYieldGenerator {
        ...
        if (_totalStblValue > totalDeposit) {
            uint256 _accumaltedAmount = _totalStblValue.uncheckedSub(totalDeposit);

            uint256 res = cToken.redeemUnderlying(_accumaltedAmount);

            if (res == ERRCODE_OK) {
                stablecoin.safeTransfer(deinTreasury, _accumaltedAmount);
    ...
    function _claimCompRewards() internal {
        // get comp reward on top of farming and send it to dein treasury
        comptroller.claimComp(address(this));

        IERC20 comp = IERC20(comptroller.getCompAddress());
        uint256 compBalance = comp.balanceOf(address(this));
        if (compBalance > 0) {
            comp.safeTransfer(deinTreasury, compBalance);
        }
    }
    ...
    function _totalValue() internal returns (uint256) {
        ...
        uint256 _totalStableValue =
            cTokenBalance.mul(cTokenPrice).uncheckedDiv(COMPOUND_EXCHANGE_RATE_PRECISION);
    ...
    function getOneDayGain() external view override returns (uint256) {
        ...
        return rate.uncheckedDiv(10**18);
    }
