...
contract VoteWin is Pausable, Ownable {
    ...
    function vote(uint _day, uint _matchIndex, uint _teamSide, uint amount) external whenNotPaused {
        ...
        require(block.timestamp > matchDetails[_day][_matchIndex].startTime, 'VoteWin::vote: match Ended');
    ...
    function setMatch(uint _day, uint _matchIndex, uint startTime, uint endTime, uint teamIndexA, uint teamIndexB) external onlyOwner whenPaused {
