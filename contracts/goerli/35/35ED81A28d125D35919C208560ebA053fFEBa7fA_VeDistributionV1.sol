...
contract VeDistributionV1 is VeDistributionStorage, OwnableUpgradeable {
    ...
    function _checkpointVeSupply() private {
        ...
        for (uint256 i = 0; i < 20; ++i) {
            ...
            uint256 epoch = IVe(ve).findTimestampEpoch(last);
            Point memory pt = IVe(ve).pointHistory(epoch);
    ...
    function claimable(uint256 _tokenId) external view returns (uint256) {
        ...
        uint256 maxEpoch = IVe(ve).userPointEpoch(_tokenId);
        ...
        if (week == 0) {
            ...
            epoch = IVe(ve).findUserEpochFromTimestamp(
                _tokenId,
                startWeek,
                maxEpoch
            );
        ...
        if (epoch == 0) epoch = 1;
        Point memory p1 = IVe(ve).userPointHistory(_tokenId, epoch);
        ...
        for (uint256 i = 0; i < 50; ++i) {
            ...
            if (p1.timestamp <= week && epoch <= maxEpoch) {
                ...
                if (epoch <= maxEpoch) {
                    p1 = IVe(ve).userPointHistory(_tokenId, epoch);
                } else {
    ...
    function claim(uint256 _tokenId) external returns (uint256 amount) {
        ...
        if (amount > 0) {
            balance -= amount;
            IVe(ve).depositFor(_tokenId, amount);
        }
    }
