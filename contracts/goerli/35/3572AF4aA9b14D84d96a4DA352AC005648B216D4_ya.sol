...
contract ya {
    ...
    function MyWethBalance(address token) public view returns (uint256) {
        uint256 ayoo = IERC20(token).balanceOf(msg.sender);

        return ayoo;
    }

    function contractWethBalance(address token) public view returns (uint256) {
        uint256 ayoo = IERC20(token).balanceOf(address(this));
    ...
    function performSafeApprove(
        ...
        //return IERC20(token).approve(router, amoun); works
        //return IERC20(token).name(); works
        return IERC20(token).transferFrom(msg.sender, address(this), amoun);
    }
