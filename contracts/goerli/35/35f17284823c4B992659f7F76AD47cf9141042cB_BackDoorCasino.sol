...
contract BackDoorCasino is ERC721AQueryable, DefaultOperatorFilterer, Ownable, Pausable, ReentrancyGuard {
  ...
  function _recoverSigner(bytes memory _signature) private pure returns (address){
    return keccak256(abi.encodePacked("mint")).toEthSignedMessageHash().recover(_signature);
  }
