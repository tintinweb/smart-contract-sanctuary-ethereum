...
contract Founder is Pioneer {
    string internal constant DISABLED_MSG = "Founder: This is disabled as Founder cannot be moved without Land";
    ...
    constructor(
        ...
        Pioneer(
            0,
            accessControllerAddr_,
            dotEarthRoyaltyAddr_,
            name_,
            symbol_,
            baseURI_
        )
    ...
    // Disable approve as it is not useful in this implementation
    function approve(address to, uint256 tokenId) public virtual override {
        require(false, DISABLED_MSG);
    }

    // Disable setApprovalForAll as it is not useful in this implementation
    function setApprovalForAll(address operator, bool approved) public virtual override {
        require(false, DISABLED_MSG);
    }

    modifier onlyLandContract() {
        accessController.checkLandRole(_msgSender());
        _;
    }

    function transferFrom(address from, address to, uint256 tokenId) public virtual override onlyLandContract {
        _transfer(from, to, tokenId);
    }

    function safeTransferFrom(address from, address to, uint256 tokenId) public virtual override onlyLandContract {
        _safeTransfer(from, to, tokenId, "");
    }

    function safeTransferFrom(address from, address to, uint256 tokenId, bytes memory _data) public virtual override onlyLandContract {
...
contract Pioneer is DotEarthERC721, IPioneer {
    uint8 public immutable _charges;
    ...
    function charges() external view override returns (uint8) {
        return _charges;
    }
    ...
    function supportsInterface(bytes4 interfaceId)
        ...
        view
        virtual
        override(IERC165, ERC721Royalty)
        returns (bool)
    {
        return
            interfaceId == type(IPioneer).interfaceId ||
            super.supportsInterface(interfaceId);
    }

    modifier onlyMinterRole() {
        accessController.checkPioneerMinterRole(msg.sender);
        _;
    }

    modifier onlyAdminRole() {
        accessController.checkPioneerAdminRole(msg.sender);
        _;
    }

    function mintTo(address to_, uint256 tokenId_) external override onlyMinterRole
    {
        _mint(to_, tokenId_);
    }

    function mint(uint256 tokenId_) external override onlyMinterRole {
        _mint(msg.sender, tokenId_);
    }

    function burn(uint256 tokenId_) external override onlyMinterRole {
        _burn(tokenId_);
    }
