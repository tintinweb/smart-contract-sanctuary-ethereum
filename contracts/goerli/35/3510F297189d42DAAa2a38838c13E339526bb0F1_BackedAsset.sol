...
contract BackedAsset is Stabilizer {
    ...
    /**
     * @notice Divest.
     * @param _btoken_amount Amount to be divested.
     * @dev Swap from the token to usdx.
     */
    function divest(
        uint256 _btoken_amount
    ) external onlyBorrower validAmount(_btoken_amount) {
        _divest(_btoken_amount);
    }
    ...
    function _divest(uint256 _btoken_amount) internal override {
        uint256 token_balance = token.balanceOf(address(this));
        if(token_balance < _btoken_amount) _btoken_amount = token_balance;

        TransferHelper.safeTransfer(address(token), redeem_address, _btoken_amount);

        emit Divested(_btoken_amount, 0);
    }
}
