...
contract HSCompound is HandlerBase {
  ...
  modifier isDSProxyOwner(address dsProxy) {
    ...
    _requireMsg(IDSProxy(dsProxy).owner() == sender, "General", "Not owner of the DSProxy");
    _;
  }
  ...
  function enterMarket(address dsProxy, address cToken) external payable isDSProxyOwner(dsProxy) {
    _enterMarket(dsProxy, cToken);
  }

  function enterMarkets(address dsProxy, address[] calldata cTokens) external payable isDSProxyOwner(dsProxy) {
    try
      IDSProxy(dsProxy).execute(
        FCOMPOUND_ACTIONS,
        abi.encodeWithSelector(
  ...
  function exitMarket(address dsProxy, address cToken) external payable isDSProxyOwner(dsProxy) {
    try
      IDSProxy(dsProxy).execute(
        FCOMPOUND_ACTIONS,
        abi.encodeWithSelector(
  ...
  function claimComp(address dsProxy) external payable isDSProxyOwner(dsProxy) returns (uint256) {
    try IComptroller(COMPTROLLER).claimComp(dsProxy) {} catch Error(string memory reason) {
  ...
  function _deposit(
    ...
    if (token == NATIVE_TOKEN_ADDRESS) {
      ...
      IERC20(token).transfer(dsProxy, amount);
    }
  }
...
// Mainnet Address......
//   // prettier-ignore
//   address  constant FCOMPOUND_ACTIONS = 0x05EF8eb657027927fAB9b279138f0189CB144976;
//   // prettier-ignore
...
// Goerli Address......
  address  constant FCOMPOUND_ACTIONS = 0xE8Cc240149394702F5eB1Cf17B443711917e0834;
  // prettier-ignore
  address  constant COMPTROLLER = 0x05Df6C772A563FfB37fD3E04C1A279Fb30228621;
  // prettier-ignore
  address  constant CETH_ADDRESS = 0x64078a6189Bf45f80091c6Ff2fCEe1B15Ac8dbde;
  // prettier-ignore
  address  constant COMP_ADDRESS = 0x3587b2F7E0E2D6166d6C14230e7Fe160252B0ba4;
