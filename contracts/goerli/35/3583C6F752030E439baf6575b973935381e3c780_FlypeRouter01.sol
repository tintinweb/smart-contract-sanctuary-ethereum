...
contract FlypeRouter01 is FlypeLibrary {
    ...
    function _addLiquidity(
        ...
        (uint256 reserveLP, uint256 reserveETH) = getReserves(tokenA, tokenB);
        (uint256 priceLP, uint256 priceETH) = getPrices(tokenA, tokenB);
        if (reserveLP == 0 && reserveETH == 0) {
            uint256 amountBOptimal = quotePrice(amountADesired, priceLP, priceETH);
            if (amountBOptimal <= amountBDesired) {
                require(amountBOptimal >= amountBMin, "FlypeRouter: INSUFFICIENT_B_AMOUNT");
                (amountA, amountB) = (amountADesired, amountBOptimal);
            } else {
                uint256 amountAOptimal = quotePrice(amountBDesired, priceETH, priceLP);
            ...
            uint256 amountBOptimal = quote(amountADesired, reserveLP, reserveETH);
            if (amountBOptimal <= amountBDesired && amountBOptimal > 0) {
                require(amountBOptimal >= amountBMin, "FlypeRouter: INSUFFICIENT_B_AMOUNT");
                (amountA, amountB) = (amountADesired, amountBOptimal);
            } else {
                uint256 amountAOptimal = quote(amountBDesired, reserveETH, reserveLP);
                assert(amountAOptimal <= amountADesired);
                require(amountAOptimal >= amountAMin, "FlypeRouter: INSUFFICIENT_A_AMOUNT");
                (amountA, amountB) = (amountAOptimal, amountBDesired);
            }
...
contract Dmm is AccessControl{
    ...
    /// @notice Sets parameters for D calculations
    /// @param _a parameter for formula
    /// @param _b parameter for formula
    /// @param _b_ACC decimals of _b parameter
    ...
    /// @notice calculate how much Eth should be sent to receive exact amount of LP
    function getAmountInForEthToLpSwap(uint256 lpPrice, uint256 ethPrice, uint256 lpReserve, uint256 ethReserve, uint256 amountSwapped) external view liquidityCheck(lpReserve, amountSwapped) returns(uint256 quote){
        ...
        /// @dev ΔX = ΔY = D*Xc*ΔX / (Xc-ΔX)
        quote = lpReserve * amountSwapped * D / (lpReserve - amountSwapped) / 1e18;
    }

    /// @notice calculate how much Eth would be sent in return for exact amount of LP
    function getAmountOutForEthToLpSwap(uint256 lpPrice, uint256 ethPrice, uint256 lpReserve, uint256 ethReserve, uint256 amountSwapped) external view returns(uint256 quote){
        uint D = getD(lpPrice, ethPrice, lpReserve, ethReserve);
        /// @dev ΔX = Xc*ΔY / (Xc*D+ΔY)
        quote = lpReserve * amountSwapped * 1e18 / (lpReserve * D + amountSwapped * 1e18); 
    }     

    /// @notice calculate how much LP would be sent in return for exact amount of Eth
    function getAmountInForLpToEthSwap(uint256 lpPrice, uint256 ethPrice, uint256 lpReserve, uint256 ethReserve, uint256 amountSwapped) external view liquidityCheck(ethReserve, amountSwapped) returns(uint256 quote){
        uint D = getD(lpPrice, ethPrice, lpReserve, ethReserve);
        /// @dev ΔX = Yc*ΔY / (D*(Yc-ΔY)
        quote = ethReserve * amountSwapped * 1e18 / (D * ethReserve - D * amountSwapped);
    }

    /// @notice calculate how much LP should be sent to receive exact amount of Eth
    ...
    function getD(uint256 lpPrice, uint256 ethPrice, uint256 lpReserve, uint256 ethReserve) public view returns(uint){
        ...
        if(k != 1e18){
            if(a%10 > 0){     
                root1 = nthRootWithoutSlice(sqrt(k), 5, 5, 1000) * 1e22 / tenthRootFrom1e18;
                uint sqr = 1e18;
        ...
        //D = Px/Py * (1+b*(1-k^a)/(1+k^a))
        int256 D = int(lpPrice * 1e18 / ethPrice) * int(1e18 + 
...
contract FlypeLibrary is AccessControl {
    ...
    constructor(address factory, address WETH, Dmm _DMM){
        _factory = factory;
        _WETH = WETH;
        DMM = _DMM;
        _grantRole(DEFAULT_ADMIN_ROLE, msg.sender);
    ...
    /// @param reserveA Reserves of first token in pair
    /// @param reserveB Reserves of second token in pair
    ...
    function quotePrice(uint amountA, uint priceA, uint priceB) public pure returns (uint amountB) {
        require(amountA > 0, 'FlypeLibrary: INSUFFICIENT_AMOUNT');
    ...
    function getAmountsOut(uint amountIn, address[] memory path, address user) public view returns (uint[] memory amounts, uint256[] memory fees) {
        ...
        for (uint i; i < path.length - 1; i++) {
            ...
            (amounts[i + 1], fees[i]) = getAmountOut(
                DMM.getAmountOutForEthToLpSwap(priceLp, priceEth, reserveLp, reserveEth, amounts[i]),
                priceEth, user, path.length == 3)
             :
             (amounts[i + 1], fees[i]) = getAmountOut(
                DMM.getAmountOutForLpToEthSwap(priceLp, priceEth, reserveLp, reserveEth, amounts[i]), 
        ...
        for (uint i = path.length - 1; i > 0; i--) {
            ...
            (path[i] == _WETH)?
             (amounts[i - 1], fees[i - 1]) = getAmountIn(
                DMM.getAmountInForLpToEthSwap(priceLp, priceEth, reserveLp, reserveEth, amounts[i]),
                 priceLp, user, path.length == 3)
             :
             (amounts[i - 1], fees[i - 1]) = getAmountIn(
                 DMM.getAmountInForEthToLpSwap(priceLp, priceEth, reserveLp, reserveEth, amounts[i]), 
                 priceEth, user, path.length == 3);
        }
    }
    ...
    function getD(address flypeLp) external view returns(uint){
        ...
        return(DMM.getD(lpPrice, ethPrice, lpReserve, ethReserve));
    }
}
