...
contract EclipseMinter is EclipseMinterBase {
        ...
        onlyAdmin
    {
        ...
        require(params.startTime > block.timestamp, "startTime too early");
     ...
     */
    function _checkAvailableMints(address collection, uint256 amount)
        internal
        view
    {
        uint256 allowedMint = EclipseMintAllocator(mintAllocator)
            .getAllowedMints(collection);

        (, , , , uint256 maxSupply, uint256 totalSupply) = IEclipseERC721(
            collection
        ).getInfo();
        require(
            amount <= allowedMint,
    ...
    function mintOne(address collection) external payable override {
        ...
        _checkAvailableMints(collection, 1);

        IEclipseERC721(collection).mint(msg.sender);
        ...
        override
    {
        ...
        _checkAvailableMints(collection, amount);

        address minter = _msgSender();

        for (uint256 i; i < amount; i++) {
            IEclipseERC721(collection).mint(minter);
        }
        _splitPayment(collection);
    }
    ...
    function _splitPayment(address collection) internal {
        ...
        IEclipsePaymentSplitter(paymentSplitter).splitPayment{
            value: msg.value
        }();
    }
...
contract EclipseMintAllocator is EclipseAccess {
     ...
     */
    function init(address collection, uint8 allowedPerTransaction)
        external
        onlyAdmin
    {
        mintstates[collection].init(allowedPerTransaction);
    }

    /**
     *@dev Get allowed mints for collection
     */
    function getAllowedMints(address collection)
        external
        view
        returns (uint256)
    {
        return mintstates[collection].getAllowedMints();
    }
}
...
library MintAlloc {
    ...
    function getAllowedMints(State storage state)
        internal
        view
        returns (uint256)
    {
        return state.allowedPerTransaction;
    }

    function init(State storage state, uint8 allowedPerTransaction) internal {
