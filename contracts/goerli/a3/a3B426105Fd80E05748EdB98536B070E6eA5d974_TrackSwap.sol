    ...
    function _trade(
        ...
        for (uint256 i = 0; i < _tradeDetails.length; i++) {
            ...
            if(success)
            {
                ...
                {
                    // X2Y2
                    bytes memory cdata = BytesLib.slice(_tradeDetails[i].tradeData, 4, _tradeDetails[i].tradeData.length-4);
                    X2Y2Market.RunInput memory input = abi.decode(cdata, (X2Y2Market.RunInput));
                ...
                {
                    bytes memory cdata = BytesLib.slice(_tradeDetails[i].tradeData, 4, _tradeDetails[i].tradeData.length-4);
                    (OrderTypes.TakerOrder memory takerBid, OrderTypes.MakerOrder memory makerAsk) = abi.decode(cdata, (OrderTypes.TakerOrder, OrderTypes.MakerOrder));
