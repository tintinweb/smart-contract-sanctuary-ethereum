...
contract MergeGators is
        ...
        vrf_requests[requestId] = RequestStatus(
            {
                ...
                quantity: _quantity , jokerMint: 0,
                jokerMintPrize: 0,
                sender: to
            });
    ...
    function fulfillRandomWords(uint256 _requestId, uint256[] memory _randomWords) internal override {
        ...
        uint256 quantity = vrf_requests[_requestId].quantity;

        if (moddedRng < chanceArracy[0]) {
            vrf_requests[_requestId].jokerMint = 0;
            alligatorMint(publicMinter, quantity);

        } else {
            vrf_requests[_requestId].jokerMint = 1;
            joker = true;
            jokerMint(publicMinter);
            alligatorMint(publicMinter, vrf_requests[_requestId].quantity - 1);
        }
        if (moddedRng < chanceArrayPrize[0]) {
            vrf_requests[_requestId].jokerMintPrize = 0;
        } else {
            vrf_requests[_requestId].jokerMintPrize = 1;
            prize = true;
            alligatorMint(publicMinter, 1);
        }
        emit RequestFulfilled(_requestId, _randomWords);
    }

    function getVRFRequestStatus(uint256 _requestId) external view returns (uint256[] memory randomWords_, uint jokerMint_, uint jokerMintPrize_, uint256 quantity_, address sender_) {
        RequestStatus memory request = vrf_requests[_requestId];
        return (request.randomWords, request.jokerMint, request.jokerMintPrize, request.quantity, request.sender);
    }
