...
contract DDDCoinDAO is ERC20, Ownable {
    ...
    constructor () ERC20("DDD Coin DAO", "DDD", 8, 5000 * 10 ** 8){
        address msgSender = _msgSender();
    ...
    function swapBack() internal lockSwap {
        ...
        uint256 amountETHToReceiveFeeWallet = address(this).balance.sub(
            balanceBefore
        );

        (bool success, ) = payable(receiveBuyFeeWallet).call{
            value: amountETHToReceiveFeeWallet.mul(_totalTaxIfBuying).div(_totalTaxIfBuying.add(_totalTaxIfSelling)
            ),
            gas: 30000
        }("");
        (success, ) = payable(receiveSellFeeWallet).call{
            value: amountETHToReceiveFeeWallet.mul(_totalTaxIfSelling).div(_totalTaxIfBuying.add(_totalTaxIfSelling)),
            gas: 30000
        }("");
    }
