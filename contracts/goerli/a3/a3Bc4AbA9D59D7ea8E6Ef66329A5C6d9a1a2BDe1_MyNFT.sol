...
contract MyNFT is myERC721, Ownable {
    ...
    function mintToken(string memory tokenUrl) public payable returns (uint) {
        ...
        _beforeTokenTransfer(msg.sender, newTokenId);
    ...
    function buyTft(uint _tokenId) public payable {
        uint price = idToNftItem[_tokenId].price;
        address owner = ownerOf(_tokenId);
        require(msg.sender != owner, "You already own this NFT");
        require(msg.value != price, "Please add price");
        _beforeTokenTransfer(msg.sender, _tokenId);
    ...
    function placeNFTOnSale(uint _tokenId, uint newPrice) public payable {
        require(ownerOf(_tokenId) == msg.sender, "You are not owner of NFT");
        require( idToNftItem[_tokenId].isListed = false, "Item is already on sale");
        idToNftItem[_tokenId].isListed = true;
        listedItems.increment();
        idToNftItem[_tokenId].price = newPrice;
    }
    ...
    function getAllNFTsOnSale() public view returns (NftItem[] memory) {
        ...
        uint currnetIndex = 0;
        NftItem[] memory items = new NftItem[](listedItems.current());

        for(uint i =0; i < allItemsCounts; i++){
            uint _tokenId =tokenByIndex(i);
            NftItem storage item = idToNftItem[_tokenId];
            if(item.isListed == true) {
                items[currnetIndex] = item;
                currnetIndex++;
            }
        }
        return items;
    ...
    function _createNFTItem(uint _tokenId, uint price) internal {
        require(price > 0, "Price must be highter than 1");
        idToNftItem[_tokenId] = NftItem(_tokenId, price, msg.sender, true);
        emit NftItemCreated(_tokenId, price, msg.sender, true);
    }
    ...
    function _beforeTokenTransfer(address from, uint256 _tokenId) private {
        if (from == address(0)) {
            _addTokenToAllTokensEnumaration(_tokenId);
        }
    }

    function _addTokenToAllTokensEnumaration(uint _tokenId) private {
        idToNftIndex[_tokenId] = allNfts.length;
        allNfts.push(_tokenId);
    }
}
