...
contract AssetTrade is Initializable,Ownable {
    ...
    event BuyAssets(uint256 index, address buyAddr, address paymentToken, uint256 paymentAmount, uint256 fee, uint256 sta, uint256 buyId);
    ...
    {   
        ...
        if(HGBPAddr == paymentToken){
            ...
            _amount = amounts[1].mul(paymentAmount).div(10**18);
        ...
        emit BuyAssets(index, _tradeMsg.buyAddr, paymentToken, paymentAmount, _tradeMsg.fee, 3, buyId);
    }
  
    function voteAssetsTrade(uint256[] calldata indexes,  uint256[] calldata voteStas, string[] memory tokenURIs) external{   
        ...
        for(uint i= 0; i<indexes.length; i++){
            ...
            if(_tradeMsg.agreeAddr.length > num/2 && _tradeMsg.sta ==3){
                ...
                bytes memory tokenURI = bytes(tokenURIs[i]);
                if(bytes7(tokenURI) == "ipfs://"){
                    IERC721(_tradeMsg.nftAddr).setTokenURI(_tradeMsg.nftTokenId, tokenURIs[i]);
                }
