...
contract DZooToken is ERC20Permit {
    // DZooNFT contract identifier for mint/burn
    address dZooNFT;

    // DZooNFT contract modifier for mint/burn
    modifier onlyDZoo() {
        require(
            _msgSender() == dZooNFT,
    ...
    constructor(
        ...
        address admin,
        address dZooNFT_
    ) ERC20("DegenZoo", "DZOO") ERC20Permit("DegenZoo") {
     ...
     */
    function mint(address to, uint256 amount) public onlyDZoo {
        _mint(to, amount);
    }
     ...
     * - the caller must be the DZooNFT contract.
     */
    function burn(address to, uint256 amount) public onlyDZoo {
        _burn(to, amount);
    }
}