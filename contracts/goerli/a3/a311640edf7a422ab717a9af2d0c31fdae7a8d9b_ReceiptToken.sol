...
contract ReceiptToken is ERC721URIStorage, IReceiptToken, Ownable {
    ...
    function mint(
        ...
        _safeMint(owner, currentReceiptId);
        _setTokenURI(currentReceiptId, formatTokenURI(borrower, collection, tokenId, expiration));
        emit ReceiptMinted(owner, collection, currentReceiptId, borrower, tokenId, expiration);
        _receiptId.increment();
        
        return currentReceiptId;
    }

    function burn(uint256 receiptId) external onlyOwner {
        _burn(receiptId);
    ...
    function formatTokenURI(address borrower, address collection, uint256 tokenId, uint256 expiration) internal pure returns (string memory) {
        return string(
                abi.encodePacked(
...
interface IReceiptToken is IERC721Metadata {
