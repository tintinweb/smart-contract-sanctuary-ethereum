...
contract FundManager is SigCheckable, WithAdmin {
    ...
    function addLiquidityTo(address to, address token, uint256 amount) external onlyRouter{
    ...
    function removeLiquidityIfPossibleTo(address to, address token, uint256 amount) external onlyRouter returns (uint256) {
        require(amount != 0, "Amount must be positive");
        require(token != address(0), "Bad token");
        ...
        liquidities[token][to] = liquidities[token][to] - actualLiq;
        if (actualLiq != 0) {
            IERC20Upgradeable(token).safeTransfer(to, actualLiq);
            emit BridgeLiquidityRemoved(to, token, amount);
        }
        return actualLiq;   
    }
