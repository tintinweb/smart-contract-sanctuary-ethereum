...
// File: contracts/extensions/ERC2981.sol
...
// @Dev Add MerkleRoot
// @Dev add burnable - enumerable

  error InvalidProof();
  error Nope();


contract test is  ERC721, ReentrancyGuard, OwnableRoles, DefaultOperatorFilterer, ERC2981{
  ...
  /** Sale States **/
  // @dev switch to false for production

  bool public saleIsActive = true;
  bool public whitelistSaleIsActive = true;
  bool public tester = true;
    ...
    //@dev modifier format: onlyRolesOrOwner(ADMIN_ROLE)
    ...
    /** Planet Sale Status**/
      ...
      bool private Planet1SaleOpen = true;
  ...
  /** **/
      bool private P1RainbowOpen = false;
...
//mapping (uint => bool) public planetSaleStatus;
...
//////////////////////////////////////////
  ...
  uint256 private PLANET3_SUPPLY; // Orab
  ...
  // @dev Define Max mint PER WALLET + PER Planet
  // @idea - max per planet except PLANET1 is consistent
  // 1) MAX_REG_PLANET
  // 2) MAX_SPECIAL_PLANET

  // @dev This maps users mints per planet
  // Planets 1 - 9

  mapping(address => uint256) private Planet1CountMap; // Binary
  mapping(address => uint256) private Planet2CountMap; // Red
  mapping(address => uint256) private Planet3CountMap; // Orab
  ...
  // 0x8F7dA00573C5dE1A433E0fC57c6A4e706EFd3adc
  ...
  constructor (
    ...
    //setRootHash(_rootHash); @dev - for production
...
// Returns on all mint sale status in 1 array.
  ...
  function setMaxRegMint(uint256 newMaxReg) external onlyOwner {
    MAX_REG_PLANET = newMaxReg;
  }

  function setMaxSpecial(uint256 newMaxSpecial) external onlyOwner {
  ...
  function togglePlanetSaleStatus(uint256 planetNumber) public {
    if (planetNumber == 1) {
...
function toggleRainbowListStatus(uint256 planetNumber) public {
    if (planetNumber == 1) {
  ...
  */
  ...
  function setSplitContract(address splitContract)
  external
  onlyOwner {
    _fundingrecipient = splitContract;
    }
  ...
  function adminMint(uint256 planetNumber, address recepient, uint256 count) public  onlyRolesOrOwner(ADMIN_ROLE | DEV_ROLE){
        if (planetNumber == 1) {
      require(totalPlanet1Supply() + count - 1 < PLANET1_SUPPLY, "Exceeds max supply");
      for (uint256 i = 0; i < count; i++) {
       planet1SupplyCounter.increment();
      _safeMint(recepient, totalPlanet1Supply());
    }
    } else if (planetNumber == 2) {
      require(totalPlanet1Supply() + count - 1 < PLANET1_SUPPLY, "Exceeds max supply");
      for (uint256 i = 0; i < count; i++) {
      planet2SupplyCounter.increment();
      _safeMint(recepient, totalPlanet2Supply() + PLANET1_SUPPLY);
      }
    }
}

    function RainbowMintPlanet1(uint256 planetNumber, address recepient, uint256 count, bytes32[] calldata _proof) public payable  {
        if (!_verifyProof(msg.sender, _proof)) {
      revert InvalidProof();
    }
    if (planetNumber == 1) {
      require(P1RainbowOpen, "Sale not active"); //Public sale exlusive 
          P1checks(recepient,count);
    } else if (planetNumber == 2) {
           P1checks(recepient,count);
    }
  ...
  function MasterMint(uint256 planetNumber, address recepient, uint256 count) public payable {
      require(msg.value >= PLANET_PRICE * count, "Insufficient payment");
      
      if (planetNumber == 1) {
        require(Planet1SaleOpen, "Sale not active"); //Public sale exlusive 
          P1checks(recepient,count);
    ...
    //Checks specific to user and purchase
  function P1checks(address recepient, uint256 count) internal {
    require(count - 1 < MAX_MULTIMINT, "Trying to mint too many at a time");
    require(msg.value >= PLANET_PRICE * count, "Insufficient payment");

    if (allowedPlanet1MintCount(recepient) > 0) {
      updatePlanet1MintCount(recepient, count);
    } else {
      revert("Minting limit exceeded");
    }
    P1Mint(recepient,count);
}

    //Checks required for any and all mints. Even admin mints, could abstract supply to the admin call.. 
   function P1Mint(address recepient, uint256 count) internal{
      require(totalPlanet1Supply() + count - 1 < PLANET1_SUPPLY, "Exceeds max supply");
      for (uint256 i = 0; i < count; i++) {
      planet1SupplyCounter.increment();
      _safeMint(recepient, totalPlanet1Supply());
...
//   function MasterMint2(uint256 planetNumber, address recepient, uint256 count) public payable {
//       require(msg.value >= PLANET_PRICE * count, "Insufficient payment");
      
//       if (planetNumber == 1) {
//           require(Planet1SaleOpen, "Sale not active");
//           P1checks(recepient,count);
...
//   function P1checks2(address recepient, uint256 count) internal {
//   //  require(Planet1SaleOpen, "Sale not active");
//     require(count - 1 < MAX_MULTIMINT, "Trying to mint too many at a time");
//     require(msg.value >= PLANET_PRICE * count, "Insufficient payment");
...
// }
  ...
  //Support for Merkleproof


  /** PAYOUT **/
  // Replace with Withdraw for any eth sent to contract.
  // function release(address payable account) public virtual onlyOwner {
  //   _splitter.release(account);
  // }

 
  //////RESERVED incase need to count each users mint total per planet//////
  ...
  // ============================================================= //
    ...
    function mintPlanet1(address to, uint256 count) public payable  {
    require(Planet1SaleOpen, "Sale not active");
    require(totalPlanet1Supply() + count - 1 < PLANET1_SUPPLY, "Exceeds max supply");
    require(count - 1 < MAX_MULTIMINT, "Trying to mint too many at a time");
    require(msg.value >= PLANET_PRICE * count, "Insufficient payment");

    if (allowedPlanet1MintCount(to) > 0) {
      updatePlanet1MintCount(to, count);
    } else {
      revert("Minting limit exceeded");
    }
    ...
    for (uint256 i = 0; i < count; i++) {
      planet1SupplyCounter.increment();
      _safeMint(to, totalPlanet1Supply());
    }
    ...
    function mintPlanet2(address to, uint256 count) public payable  {
    require(Planet2SaleOpen, "Sale not active");
    require(totalPlanet2Supply() + count - 1 < PLANET2_SUPPLY, "Exceeds max supply");
    require(count - 1 < MAX_MULTIMINT, "Trying to mint too many at a time");
    require(msg.value >= PLANET_PRICE * count, "Insufficient payment");

    if (allowedPlanet2MintCount(to) > 0) {
      updatePlanet2MintCount(to, count);
    } else {
      revert("Minting limit exceeded");
    }
    ...
    for (uint256 i = 0; i < count; i++) {
      planet2SupplyCounter.increment();
      _safeMint(to, totalPlanet2Supply() + PLANET1_SUPPLY);
    }
    ...
    function mintPlanet3(address to, uint256 count) public payable  {
    require(Planet3SaleOpen, "Sale not active");
    require(totalPlanet3Supply() + count - 1 < PLANET3_SUPPLY, "Exceeds max supply");
    require(count - 1 < MAX_MULTIMINT, "Trying to mint too many at a time");
    require(msg.value >= PLANET_PRICE * count, "Insufficient payment");

     if (allowedPlanet3MintCount(to) > 0) {
      updatePlanet3MintCount(to, count);
    } else {
      revert("Minting limit exceeded");
    }
    ...
    for (uint256 i = 0; i < count; i++) {
      ...
      _safeMint(to, totalPlanet3Supply() + PLANET1_SUPPLY + PLANET2_SUPPLY );
    }

    payable(_fundingrecipient).transfer(msg.value);
  }

    function mintPlanet4(address to, uint256 count) public payable  {
    require(Planet4SaleOpen, "Sale not active");
    require(totalPlanet4Supply() + count - 1 < PLANET4_SUPPLY, "Exceeds max supply");
    require(count - 1 < MAX_MULTIMINT, "Trying to mint too many at a time");
    require(msg.value >= PLANET_PRICE * count, "Insufficient payment");

    if (allowedPlanet4MintCount(to) > 0) {
      updatePlanet4MintCount(to, count);
    } else {
      revert("Minting limit exceeded");
    }
    ...
    for (uint256 i = 0; i < count; i++) {
      ...
      _safeMint(to, totalPlanet4Supply() + PLANET1_SUPPLY + PLANET2_SUPPLY + PLANET3_SUPPLY );
    }

    payable(_fundingrecipient).transfer(msg.value);
  }

    function mintPlanet5(address to, uint256 count) public payable  {
    require(Planet5SaleOpen, "Sale not active");
    require(totalPlanet5Supply() + count - 1 < PLANET5_SUPPLY, "Exceeds max supply");
    require(count - 1 < MAX_MULTIMINT, "Trying to mint too many at a time");
    require(msg.value >= PLANET_PRICE * count, "Insufficient payment");

    if (allowedPlanet5MintCount(to) > 0) {
      updatePlanet5MintCount(to, count);
    } else {
      revert("Minting limit exceeded");
    }
    ...
    for (uint256 i = 0; i < count; i++) {
      ...
      _safeMint(to, totalPlanet5Supply() + PLANET1_SUPPLY + PLANET2_SUPPLY + PLANET3_SUPPLY + PLANET4_SUPPLY);
    }

    payable(_fundingrecipient).transfer(msg.value);
  }

    function mintPlanet6(address to, uint256 count) public payable  {
    require(Planet6SaleOpen, "Sale not active");
    require(totalPlanet6Supply() + count - 1 < PLANET6_SUPPLY, "Exceeds max supply");
    require(count - 1 < MAX_MULTIMINT, "Trying to mint too many at a time");
    require(msg.value >= PLANET_PRICE * count, "Insufficient payment");

    if (allowedPlanet6MintCount(to) > 0) {
      updatePlanet6MintCount(to, count);
    } else {
      revert("Minting limit exceeded");
    }
    ...
    for (uint256 i = 0; i < count; i++) {
      planet5SupplyCounter.increment();
      _safeMint(to, totalPlanet6Supply() + PLANET1_SUPPLY + PLANET2_SUPPLY + PLANET3_SUPPLY + PLANET4_SUPPLY + PLANET5_SUPPLY);
    }

    payable(_fundingrecipient).transfer(msg.value);
  }

    function mintPlanet7(address to, uint256 count) public payable  {
    require(Planet7SaleOpen, "Sale not active");
    require(totalPlanet7Supply() + count - 1 < PLANET7_SUPPLY, "Exceeds max supply");
    require(count - 1 < MAX_MULTIMINT, "Trying to mint too many at a time");
    require(msg.value >= PLANET_PRICE * count, "Insufficient payment");

    if (allowedPlanet7MintCount(to) > 0) {
      updatePlanet7MintCount(to, count);
    } else {
      revert("Minting limit exceeded");
    }
    ...
    for (uint256 i = 0; i < count; i++) {
      ...
      _safeMint(to, totalPlanet7Supply() + PLANET1_SUPPLY + PLANET2_SUPPLY + PLANET3_SUPPLY + PLANET4_SUPPLY + PLANET5_SUPPLY + PLANET6_SUPPLY);
    }

    payable(_fundingrecipient).transfer(msg.value);
  }

    function mintPlanet8(address to, uint256 count) public payable  {
    require(Planet8SaleOpen, "Sale not active");
    require(totalPlanet8Supply() + count - 1 < PLANET8_SUPPLY, "Exceeds max supply");
    require(count - 1 < MAX_MULTIMINT, "Trying to mint too many at a time");
    require(msg.value >= PLANET_PRICE * count, "Insufficient payment");

    if (allowedPlanet8MintCount(to) > 0) {
      updatePlanet8MintCount(to, count);
    } else {
      revert("Minting limit exceeded");
    }
    ...
    for (uint256 i = 0; i < count; i++) {
      ...
      _safeMint(to, totalPlanet8Supply() + PLANET1_SUPPLY + PLANET2_SUPPLY + PLANET3_SUPPLY + PLANET4_SUPPLY + PLANET5_SUPPLY + PLANET6_SUPPLY + PLANET7_SUPPLY);
    }

    payable(_fundingrecipient).transfer(msg.value);
  }

    function mintPlanet9(address to, uint256 count) public payable  {
    require(Planet9SaleOpen, "Sale not active");
    require(totalPlanet9Supply() + count - 1 < PLANET9_SUPPLY, "Exceeds max supply");
    require(count - 1 < MAX_MULTIMINT, "Trying to mint too many at a time");
    require(msg.value >= PLANET_PRICE * count, "Insufficient payment");

    if (allowedPlanet9MintCount(to) > 0) {
      updatePlanet9MintCount(to, count);
    } else {
      revert("Minting limit exceeded");
    }
    ...
    for (uint256 i = 0; i < count; i++) {
      ...
      _safeMint(to, totalPlanet9Supply() + PLANET1_SUPPLY + PLANET2_SUPPLY + PLANET3_SUPPLY + PLANET4_SUPPLY + PLANET5_SUPPLY + PLANET6_SUPPLY + PLANET7_SUPPLY + PLANET8_SUPPLY);
    }

    payable(_fundingrecipient).transfer(msg.value);
  }

    function mintPlanet10(address to, uint256 count) public payable  {
    require(Planet10SaleOpen, "Sale not active");
    require(totalPlanet10Supply() + count - 1 < PLANET10_SUPPLY, "Exceeds max supply");
    require(count - 1 < MAX_MULTIMINT, "Trying to mint too many at a time");
    require(msg.value >= PLANET_PRICE * count, "Insufficient payment");

    if (allowedPlanet10MintCount(to) > 0) {
      updatePlanet10MintCount(to, count);
    } else {
      revert("Minting limit exceeded");
    }
    ...
    for (uint256 i = 0; i < count; i++) {
      ...
      _safeMint(to, totalPlanet9Supply() + PLANET1_SUPPLY + PLANET2_SUPPLY + PLANET3_SUPPLY + PLANET4_SUPPLY + PLANET5_SUPPLY + PLANET6_SUPPLY + PLANET7_SUPPLY + PLANET8_SUPPLY + PLANET9_SUPPLY);
    }
  ...
  // }
    ...
    function RainbowMintPlanet1(address to, uint256 count,  bytes32[] calldata _proof) public payable  {
        if (!_verifyProof(msg.sender, _proof)) {
      revert InvalidProof();
    }
    require(P1RainbowOpen, "Sale not active");
    require(totalPlanet1Supply() + count - 1 < PLANET1_SUPPLY, "Exceeds max supply");
    require(count - 1 < MAX_MULTIMINT, "Trying to mint too many at a time");
    require(msg.value >= PLANET_PRICE * count, "Insufficient payment");
    ...
    function RainbowMintPlanet2(address to, uint256 count,  bytes32[] calldata _proof) public payable  {
      if (!_verifyProof(msg.sender, _proof)) {
        revert InvalidProof();
      }
    require(P2RainbowOpen, "Sale not active");
    require(totalPlanet2Supply() + count - 1 < PLANET2_SUPPLY, "Exceeds max supply");
    require(count - 1 < MAX_MULTIMINT, "Trying to mint too many at a time");
    require(msg.value >= PLANET_PRICE * count, "Insufficient payment");
    ...
    function RainbowMintPlanet3(address to, uint256 count, bytes32[] calldata _proof) public payable  {
      if (!_verifyProof(msg.sender, _proof)) {
        revert InvalidProof();
      }
    require(P3RainbowOpen, "Sale not active");
    require(totalPlanet3Supply() + count - 1 < PLANET3_SUPPLY, "Exceeds max supply");
    require(count - 1 < MAX_MULTIMINT, "Trying to mint too many at a time");
    require(msg.value >= PLANET_PRICE * count, "Insufficient payment");
    ...
    function RainbowMintPlanet4(address to, uint256 count, bytes32[] calldata _proof) public payable  {
      if (!_verifyProof(msg.sender, _proof)) {
        revert InvalidProof();
      }
    require(P4RainbowOpen, "Sale not active");
    require(totalPlanet4Supply() + count - 1 < PLANET4_SUPPLY, "Exceeds max supply");
    require(count - 1 < MAX_MULTIMINT, "Trying to mint too many at a time");
    require(msg.value >= PLANET_PRICE * count, "Insufficient payment");
    ...
    function RainbowMintPlanet5(address to, uint256 count, bytes32[] calldata _proof) public payable  {
      if (!_verifyProof(msg.sender, _proof)) {
        revert InvalidProof();
      }
    require(P5RainbowOpen, "Sale not active");
    require(totalPlanet5Supply() + count - 1 < PLANET5_SUPPLY, "Exceeds max supply");
    require(count - 1 < MAX_MULTIMINT, "Trying to mint too many at a time");
    require(msg.value >= PLANET_PRICE * count, "Insufficient payment");
    ...
    function RainbowMintPlanet6(address to, uint256 count, bytes32[] calldata _proof) public payable  {
       if (!_verifyProof(msg.sender, _proof)) {
        revert InvalidProof();
      }
    require(P6RainbowOpen, "Sale not active");
    require(totalPlanet6Supply() + count - 1 < PLANET6_SUPPLY, "Exceeds max supply");
    require(count - 1 < MAX_MULTIMINT, "Trying to mint too many at a time");
    require(msg.value >= PLANET_PRICE * count, "Insufficient payment");
    ...
    function RainbowMintPlanet7(address to, uint256 count, bytes32[] calldata _proof) public payable  {
      if (!_verifyProof(msg.sender, _proof)) {
        revert InvalidProof();
      }
    require(P7RainbowOpen, "Sale not active");
    require(totalPlanet7Supply() + count - 1 < PLANET7_SUPPLY, "Exceeds max supply");
    require(count - 1 < MAX_MULTIMINT, "Trying to mint too many at a time");
    require(msg.value >= PLANET_PRICE * count, "Insufficient payment");
    ...
    function RainbowMintPlanet8(address to, uint256 count, bytes32[] calldata _proof) public payable  {
      if (!_verifyProof(msg.sender, _proof)) {
        revert InvalidProof();
      }
    require(P8RainbowOpen, "Sale not active");
    require(totalPlanet8Supply() + count - 1 < PLANET8_SUPPLY, "Exceeds max supply");
    require(count - 1 < MAX_MULTIMINT, "Trying to mint too many at a time");
    require(msg.value >= PLANET_PRICE * count, "Insufficient payment");
    ...
    function RainbowMintPlanet9(address to, uint256 count, bytes32[] calldata _proof) public payable  {
      if (!_verifyProof(msg.sender, _proof)) {
        revert InvalidProof();
      }
    require(P9RainbowOpen, "Sale not active");
    require(totalPlanet9Supply() + count - 1 < PLANET9_SUPPLY, "Exceeds max supply");
    require(count - 1 < MAX_MULTIMINT, "Trying to mint too many at a time");
    require(msg.value >= PLANET_PRICE * count, "Insufficient payment");
    ...
    function RainbowMintPlanet10(address to, uint256 count, bytes32[] calldata _proof) public payable  {
         if (!_verifyProof(msg.sender, _proof)) {
        revert InvalidProof();
      }
    require(P10RainbowOpen, "Sale not active");
    require(totalPlanet10Supply() + count - 1 < PLANET10_SUPPLY, "Exceeds max supply");
    require(count - 1 < MAX_MULTIMINT, "Trying to mint too many at a time");
    require(msg.value >= PLANET_PRICE * count, "Insufficient payment");
...
// @dev @PreProd - can probably reduce this to one public return function.
// PLANET 1
//getter
