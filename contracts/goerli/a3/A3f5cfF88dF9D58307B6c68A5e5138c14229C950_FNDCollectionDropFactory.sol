...
contract FNDCollectionDropFactory is Initializable, ICollectionFactory {
   ...
   */
  function initialize(address _rolesContract) external initializer {
   ...
   * @param admin The address that will have admin privelledges for the drop.
   * @param beneficiary The address that will receive royalties and mint payments.
   * @param saleConfig Terms for the sale.
   * @param referralConfig Terms for the referrals.
  ...
  function createFixedPriceCollectionDrop(
    ...
    IFixedPriceCollectionDropInitializer.SaleConfig memory saleConfig,
    IFixedPriceCollectionDropInitializer.ReferralConfig memory referralConfig,
    uint256 nonce
  ...
  function _updateFixedPriceImplementation(address _implementation) private {
    ...
    require(
      _implementation.supportsERC165Interface(type(IFixedPriceCollectionDropInitializer).interfaceId) == true,
    ...
    IFixedPriceCollectionDropInitializer.SaleConfig memory saleConfig;
    saleConfig.maxTokenId = 1;
    saleConfig.limitPerAccount = 1;
    IFixedPriceCollectionDropInitializer.ReferralConfig memory referralConfig;

    // TODO: should we emit FixedPriceCollectionDropCreated here for consistency?
    ...
    IFixedPriceCollectionDropInitializer(_implementation).initialize(
      ...
      "",
      string(abi.encodePacked("ipfs://foundation.os.token.uri.preveal.content/v-", versionFixedPrice.toString())),
      saleConfig,
      referralConfig
    );
  }
...
interface IFixedPriceCollectionDropInitializer {
  /**
   * @notice Configuration for the drop terms.
   */
  struct SaleConfig {
    /**
     * @notice The start time of the collection drop.
     */
    uint32 startTime;
    /**
  ...
  /**
   * @notice Configuration for the referral fee terms.
   */
  struct ReferralConfig {
    /**
     * @notice Seller referral address.
     * @dev Used to pay referral fees for `listing` the drop.
     */
    address payable sellReferrer;
    /**
     * @notice The max fee for the seller referral in basis point.
     * @dev The value should be between [0, 10000], where 10000 is BASIS_POINTS.
     * e.g. If the `feeInBasisPoints` is set to `1000`,
     * BASIS_POINTS/feeInBasisPoints is used to denote 10% goes to the referral beneficiary.
     */
    uint16 sellReferrerMaxFeeInBasisPoints;
    /**
     * @notice The max fee for the buyer referral in basis point.
     * @dev The value should be between [0, 10000], where 10000 is BASIS_POINTS.
     * e.g. If the `feeInBasisPoints` is set to `1000`,
     * BASIS_POINTS/feeInBasisPoints is used to denote 10% goes to the referral beneficiary.
     */
    uint16 buyReferrerMaxFeeInBasisPoints;
  }
  ...
  function adminUpdateSale(SaleConfig memory _saleConfig, ReferralConfig memory _referralConfig) external;
}
