...
contract NotDeployedHunter is DefaultOperatorFilterer, ERC721A, Ownable, ReentrancyGuard, PaymentSplitter {
    ...
    //limit number
    uint256 public personalPrivateLimit = 10;
    uint public privateTotalLimit = 5; 

    //mintNumber
    uint public privateTotalSupply = 0;
    ...
    function setPrivateTotalLimit (uint _privateTotalLimit) public onlyOwner {
        privateTotalLimit = _privateTotalLimit;
    }
    ...
    function privateMint(uint _amount, bytes32[] calldata _proof ) 
        ...
        {   
            ...
            require(_amount < personalPrivateLimit + 1 , "You can't mint so many token.");
            require(_privateListMintAmount[msg.sender] +_amount < personalPrivateLimit + 1, "amount exceeded limit."); //personal Limit
            require(privateTotalSupply + _amount < privateTotalLimit +1 , "SHTC's max supply exceeded");
            require(_privatePrice * _amount <= msg.value,"Not enough ethers sent");
            _privateListMintAmount[msg.sender] += _amount;
            _safeMint(msg.sender, _amount);
            privateTotalSupply += _amount;
        }
