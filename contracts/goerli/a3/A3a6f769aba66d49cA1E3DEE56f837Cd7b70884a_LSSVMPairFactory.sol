...
// File: src/lib/LSSVMPairCloner.sol
...
// File: src/LSSVMPairMissingEnumerable.sol
...
abstract contract LSSVMPairMissingEnumerable is LSSVMPair {
    ...
    function _sendAnyNFTsToRecipient(
        ...
        for (uint256 i; i < numNFTs; ) {
            ...
            _nft.safeTransferFrom(permissionedIds[nftId], nftRecipient, nftId);
            idSet.remove(nftId);
    ...
    function removeNFTFromPool(uint256[] calldata ids) external {
        for (uint256 i; i < ids.length; i++) {
            address nftOwner = nft().ownerOf(ids[i]);
...
// File: src/LSSVMPairMissingEnumerableERC20.sol
...
contract LSSVMPairFactory is Ownable, ILSSVMPairFactoryLike {
    ...
    function createPairERC20(CreateERC20PairParams calldata params, address payable WETH)
        external payable
        returns (LSSVMPairERC20 pair)
    {
        ...
        emit NewPair(msg.sender, address(params.nft), params.poolType, tx.origin, address(pair));
    }
