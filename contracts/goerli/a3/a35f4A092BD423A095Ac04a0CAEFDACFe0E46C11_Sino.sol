...
import {ERC721URIHardcoded} from '../token/ERC721/extensions/ERC721URIHardcoded.sol';
    ...
    ERC721Renamable
{
    ...
    function mint(ToMint[] memory toMint) external onlyOwner {
        for (uint256 i = 0; i < toMint.length; i++)
            _mint(
                owner(),
                toMint[i].data,
                toMint[i].keyOrHashedPublicKey,
                toMint[i].hashedPinCode
            );
    }
    ...
    function supportsInterface(
        ...
        view
        virtual
        override(ERC721, ERC721Renamable, ERC721Enumerable, ERC721URIHardcoded)
        returns (bool)
    {
    ...
    function tokenURI(
        ...
        view
        virtual
        override(ERC721, ERC721URIHardcoded)
        returns (string memory)
    {
...
abstract contract CustomTemplate is Ownable {
    ...
    function setTemplate(Template memory template_) external onlyOwner {
        template = template_;
    }
...
abstract contract Evidenz is ERC721, Ownable, CustomTemplate {
    ...
    function _mint(
        address to,
        string memory data,
        bytes memory keyOrHashedPublicKey,
        bytes32 hashedPinCode_
    ) internal {
        uint256 tokenId = _tokenIdCounter.current();
        _tokenIdCounter.increment();
        _safeMint(to, tokenId);
        _data[tokenId] = data;
        _hashedPinCodes[tokenId] = hashedPinCode_;
        _addKey(tokenId, keyOrHashedPublicKey);
    }
...
import {Evidenz} from '../../../domain/Evidenz.sol';
import {IERC721URIHardcoded} from './IERC721URIHardcoded.sol';

abstract contract ERC721URIHardcoded is
    ERC721,
    Ownable,
    Evidenz,
    IERC721URIHardcoded
{
    ...
    function setImage(string memory image_) external onlyOwner {
        image = image_;
    }

    function setTermsOfUse(string memory termsOfUse_) external onlyOwner {
    ...
    function supportsInterface(
        ...
        return
            interfaceId == type(IERC721URIHardcoded).interfaceId ||
            super.supportsInterface(interfaceId);
    }
...
import {Issuer} from '../../../domain/Issuer.sol';
...
interface IERC721URIHardcoded is IERC165, IERC721 {
    function setDescription(string memory description_) external;

    function setImage(string memory image_) external;

    function setTermsOfUse(string memory termsOfUse_) external;
}
...
library Environment {
    ...
    function buildURL(
        ...
        string memory url = string(abi.encodePacked(endpoint.url, publicKey));
