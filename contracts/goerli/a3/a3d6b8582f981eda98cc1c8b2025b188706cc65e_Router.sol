...
contract Router {
    ...
    function swap(
        ...
        IERC20 tokenIn = IERC20(path[0].fromToken);
        uint256 len = path.length;
        IERC20 tokenOut = IERC20(path[len - 1].toToken);
        uint256 priorBalance = tokenOut.getBalance(to);

        for (uint256 i = 0; i < path.length; i++) {
            Hop memory hop = path[i];
            if (i == 0) {
                tokenIn.safeTransferFrom(msg.sender, address(hop.pool), amountIn);
            }
            address receiver = i < len - 1 ? address(path[i + 1].pool) : to;
            hop.pool.swap(hop.fromToken, hop.toToken, 0, receiver);
        }
        uint256 amountOut = tokenOut.getBalance(to) - priorBalance;
...
interface IPool {
    ...
    event SetPositionFee(uint256 positionFee, uint256 liquidationFee, uint256 adminFee);
    event SetSwapFee(uint256 baseSwapFee, uint256 taxBasisPoint);
    event AccureInterest(address token, uint256 borrowIndex);
    event MaxLeverageChanged(address token, uint256 maxLeverage);
    event TokenWhitelisted(address token);
    event OracleChanged(address oldOracle, address newOracle);
    event SetInterestRate(uint256 interestRate, uint256);
}
