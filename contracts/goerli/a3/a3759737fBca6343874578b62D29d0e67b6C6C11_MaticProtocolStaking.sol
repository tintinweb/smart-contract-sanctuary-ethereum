...
contract MaticProtocolStaking is Helpers {
    ...
    function delegateMultiple(
        ...
        for (uint256 i = 0; i < validatorAddresses.length; i++) {
            IValidatorShareProxy validatorContractAddress = validatorAddresses[i];
            require(
    ...
    function withdrawRewardsMultiple(
        ...
        uint256 initialBal = getTokenBal(maticToken);
        for (uint256 i = 0; i < validatorContractAddresses.length; i++) {
            IValidatorShareProxy validatorContractAddress = validatorContractAddresses[i];
            require(
                address(validatorContractAddress) != address(0),
                "!Validator"
            );
