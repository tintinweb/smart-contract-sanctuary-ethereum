...
contract EthAllocatorSoloVault is ERC1155Supply, ReentrancyGuard, Ownable {
        ...
        RocketStorageInterface rocketStorage = RocketStorageInterface(0xd8Cd47263414aFEca62d6e2a3917d6600abDceB3);


constructor(
        address _rocketStorageAddress,
        address _blockscapeRocketPoolNode
    )
        ERC1155(
            "https://ipfs.blockscape.network/ipns/k51qzi5uqu5divkfa2vxu2i71yhj3k6rm6bgvgnd4q00h4f80cb4imsg9uy29l/{id}.json"
        )
    {
        rocketStorage = RocketStorageInterface(_rocketStorageAddress); // set the rocketStorage address - 0x1d8f8f00cfa6758d7bE78336684788Fb0ee0Fa46
        blockscapeRocketPoolNode = _blockscapeRocketPoolNode; // set the blockscapeRocketPoolNode address - 0xF6132f532ABc3902EA2DcaE7f8D7FCCdF7Ba4982
    }
    ...
    address public blockscapeRocketPoolNode = address(0); //0xF6132f532ABc3902EA2DcaE7f8D7FCCdF7Ba4982;
    ...
    event SoloVaultOpend(bool _value); // event for when a permanent URI is set
    ...
    function openSoloVault() external onlyOwner {
        require(allowPubDeposit == false, "Solo vault already open");
        allowPubDeposit = true;
        emit SoloVaultOpend(allowPubDeposit);
    }
    ...
    function metadataSoloVaultInternal(
        ...
        tokenIDtoValidator[_tokenID] = "0x00";
        metadata.stakedTimestamp = block.timestamp;
        require(metadata.stakedETH > 0, "please deposit ether");
    ...
    function depositSoloVault() external payable nonReentrant {
        require(
            allowPubDeposit == true,
            "Solo vault is currently closed. Please wait for the next one."
        );
        require(
            msg.value == curETHlimit,
            "You need to deposit the exact amount of ETH for Solo Staking."
        );
        metadataSoloVaultInternal(msg.value, msg.sender, tokenID);

        if (address(this).balance == (curETHlimit)) {
            tokenID++;
    ...
    {
        require(
            compareBytes("0x00", tokenIDtoValidator[_tokenID]),
    ...
    function withdrawBatch() external onlyOwner returns (uint256) {
        require(!allowPubDeposit, "Deposit limit not reached yet.");
        require(address(this).balance == (curETHlimit), "Pool not full");
