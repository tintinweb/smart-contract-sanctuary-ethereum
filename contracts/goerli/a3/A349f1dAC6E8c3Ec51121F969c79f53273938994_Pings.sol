...
contract Pings is ERC721A, ReentrancyGuard, Ownable, IValidatable {
    ...
    uint256 public maxSupply = 555*numClaimsPerParent;
//    uint256 public reserved = 30*numClaimsPerParent;
...
//    bool[555] public claimedByPlanesId;
//    mapping (uint256 => uint16) public childToParentId;
//    uint16[1110] public childToParentId;
...
//    address _adminSigner;
    ...
    constructor() ERC721A("Pings, BlockMachine", "PINGS") {}
    ...
    function getAttsById(uint256 tokenId) external returns (PingAtts memory atts){
...
//    function mintClaim(uint256[] calldata parentIds) external nonReentrant {
//        require(phase >= SalePhase.Presale, 'Not Open');
//        uint numToMint = parentIds.length * numClaimsPerParent;
////        require(totalSupply() + numToMint <= maxSupply - reserved, "Sold out");
//        require(totalSupply() + numToMint <= maxSupply, "Sold out");
//
//        uint childId = totalSupply();
//        for (uint256 i; i < parentIds.length; i++) {
//            require(ERC721(_originAddr).ownerOf(parentIds[i]) == msg.sender, "not owner");
//            require(!claimedByPlanesId[parentIds[i]], "already claimed");
//
//            claimedByPlanesId[parentIds[i]] = true;
//            childToParentId[childId] = uint16(parentIds[i]);
//            childId += numClaimsPerParent;  //save gas - no need to save same prent id for remaining ids, since we can go up to <numClaimsPerParent> indices back and deduce it
//        }
//        _mint(msg.sender, numToMint);
//        emit MintedEvent(numToMint);
//    }

    function mintClaimSpecified(uint256[] calldata parentIds, PingAtts[] calldata paramCombos) external nonReentrant {
        require(phase >= SalePhase.Open, 'Not Open');
        uint numToMint = parentIds.length * numClaimsPerParent;
        require(totalSupply() + numToMint <= maxSupply, "Sold out");
        require(numToMint == paramCombos.length , "num to mint vs num skies mismatched");
        ...
        for (uint i; i < parentIds.length; i++) {
            ...
            claimParamCombo(childId++, paramCombos[count++], count);
            claimParamCombo(childId++, paramCombos[count++], count);
        }
        _mint(msg.sender, numToMint);
        emit MintedEvent(numToMint, totalSupply());
    }
...
//    function checkUnique(uint tokenId, PingAtts calldata atts, uint paramsIndex) internal {
//
//    }
//
//    function hasParentClaimed(uint tokenId, PingAtts calldata atts, uint paramsIndex) internal {
//        claimedByPlanesId[parentIds[i]]
//    }
