...
contract GeoInsuranceProvider is ShambaChainSelector {
    ...
    /**
     * @dev Event to log when a contract is updated
     */
    ...
    function newContract(
        ...
        }(_client, _premium, _payoutValue, _geostatsThreshold, LINK, chainId);
        ...
        i.getShambaGeostatsData(insurer);

        emit contractUpdated(address(i));
     ...
     */
    function updateContractGeostats(address _contract)
        external
        returns (int256)
    {
        GeoInsuranceConsumer i = GeoInsuranceConsumer(_contract);
        return i.getShambaGeostatsData(insurer);
    }

    /**
     * @dev checks whether the given contract being paid or not
     */
    function getContractGeostats(address _contract)
        external
        view
        returns (int256)
    {
...
contract GeoInsuranceConsumer is ShambaGeoConsumer {
    ...
    uint256 premium;
    uint256 payoutValue;
    int256 public GEOSTATS_THRESHOLD;

    bool contractActive; // is the contract currently active, or has it ended
    bool contractPaid = false;
    int256 currentGeostats = 0; // what is the current geostats for the location
    uint256 currentGeostatsDateChecked = block.timestamp; // when the last geostats check was performed
    uint256 requestCount = 0; // how many requests for geostats data have been made so far for this insurance contract
    ...
    event contractPaidOut(
        uint256 _paidTime,
        uint256 _totalPaid,
        int256 _finalGeostats
    );
    event dataReceived(int256 _geostats);
    ...
    constructor(
        ...
        emit contractCreated(insurer, client, premium, payoutValue);
    }
    ...
    function getShambaGeostatsData(address payable _insurer) public returns (int256) {
        currentGeostats = ShambaGeoConsumer.getGeostatsData();

        if (currentGeostats <= GEOSTATS_THRESHOLD && currentGeostats != 0) {
            // threshold has been met
            payOutContract(_insurer);
        }
        else {
     ...
     */
    function payOutContract(address payable _insurer) private onContractActive() {
        //Transfer agreed amount to client
        client.transfer(address(this).balance);

        //Transfer any remaining LINK(s) back to Insurer
        LinkTokenInterface link = LinkTokenInterface(chainlinkTokenAddress());
        require(
            link.transfer(_insurer, link.balanceOf(address(this))),
            "Unable to transfer"
        );
     ...
     */
    function refundInsurer(address payable _insurer) private onContractActive() {
     ...
     */
    function getPayoutValue() external view returns (uint256) {
        return payoutValue;
    }
     ...
     */
    function getCurrentGeostats() external view returns (int256) {
        return currentGeostats;
    }
     ...
     */
    function getCurrentGeostatsDateChecked() external view returns (uint256) {
     ...
     */
    function getContractStartDate() external view returns (uint256) {
        return startDate;
    }
