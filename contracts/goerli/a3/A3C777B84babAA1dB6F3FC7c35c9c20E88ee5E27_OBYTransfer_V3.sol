...
contract OBYTransfer_V3 {
    ...
    constructor(address _blackSquareAddress, address _obyAddress, bool _claimable, address _obyTransferAddress, address _illuminaNFT_V2) {
        ...
        illuminaNFT_v2 = IlluminaNFT_V2(_illuminaNFT_V2);
        owner = msg.sender;
    ...
    function getFirstEditionToBeIlluminated () public view returns (uint256) {
        BlackSquareNFT.Edition[] memory editions = blackSquare.getEditions();
        uint256 firstEdition = blackSquare.getFirstEditionToSetIlluminationDate();

        uint256[] memory illuTimestamps = new uint256[](54);

        if (firstEdition > 0) {
            for (uint256 i = 0; i < editions.length; i++) {
                if (editions[i].id != 0) {
                    illuTimestamps[i - 1] = editions[i].illuminationTimeStamp;
                }
            }

            uint256 l = illuTimestamps.length;
            for(uint i = 0; i < l; i++) {
                for(uint j = i + 1; j < l ;j++) {
                    if(illuTimestamps[i] > illuTimestamps[j]) {
                        uint256 temp = illuTimestamps[i];
                        illuTimestamps[i] = illuTimestamps[j];
                        illuTimestamps[j] = temp;
                    }
                }
            }

            uint256 smallestDate = illuTimestamps[54];

            for (uint256 i = 0; i < editions.length; i++) {
                if (editions[i].illuminationTimeStamp == smallestDate) {
                    firstEdition = editions[i].id;
                    break;
                }
            }
        }
        ...
        if (claimable) {
            for (uint256 i = 0; i < tokens.length; i++) {
                ...
                if (cycle == 1 && !obyTransfer_v2.getAlreadyClaimed(tokens[i]) && illuminationTimeStamp < block.timestamp) {
                    transferableOby += OBY_PER_CYCLE;
                    obyTransfer_v2.setClaimed(false, tokens[i]);
                }
    ...
    function mintIlluminaInBulk (string[] memory _ipfsHashes, uint256[] memory _tokens) external {
        if (bulkMintable && _ipfsHashes.length == _tokens.length && _tokens.length == maxMintable) {
            uint256 editionId = getFirstEditionToBeIlluminated();
            for (uint256 i = 0; i < _tokens.length; i++) {
                illuminaNFT_v2.fulfillRequirements(_tokens[i]);
                illuminaNFT_v2.handleMint(_ipfsHashes[i], editionId);
            }
        }
    }
}
...
contract OBYTransfer_V2 {
    ...
    modifier onlyEligible() {
        require(
            owner == msg.sender || _eligibles[msg.sender] == true,
            "IlluminaNFT: caller is not eligible"
        );
        _;
    }
    ...
    function transferOBYPerToken(uint256 _tokenId) external {
        if (
            getOwnerOf(_tokenId) == msg.sender &&
            !_claimed[_tokenId] &&
            claimable
        ) {
    ...
    function getTransferableOBYPerToken(uint256 _tokenId)
        external
        view
        returns (uint256)
    {
    ...
    function getTransferableOBYForAllTokensHeld()
        external
        view
        returns (uint256)
    {
...
contract IlluminaNFT_V2 is ERC721, DefaultOperatorFilterer, Ownable, ERC2981ContractWideRoyalties, VRFConsumerBaseV2 {
    ...
    function simpleMint(string memory _ipfsHash, uint256 _tokenId) external {
        ...
        fulfillRequirements(_tokenId);
        handleMint(_ipfsHash, 0);
    }

    function fulfillRequirements(uint256 tokenId) public view onlyEligible {
    ...
    function handleMint (string memory ipfsHash, uint256 _editionId) public onlyEligible {
        _tokenIds.increment();
