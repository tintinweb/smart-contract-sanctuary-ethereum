...
struct Participant {
    ...
    /// @notice Indicates whether the participant has been confirmed by an ADMIN_ROLE.
    bool confirmed;
...
contract RegisterManager is Initializable, Pausable, AccessControl {
    ...
    constructor() {
        ...
        _grantRole(ADMIN_ROLE, ASoto);
        _grantRole(ADMIN_ROLE, CRodriguez);
    }
     ...
     * @dev Only the account with the ADMIN_ROLE can confirm a participant, and the contract must be paused.
     ...
     * @dev Only the account with the ADMIN_ROLE can refund a participant's fee, and the contract must be paused.
    ...
    function getConfirmedParticipants() external view returns (Participant[] memory) {
        ...
        for (uint256 i = 0; i < participants.length; i++) {
            if (participants[i].confirmed) {
                confirmedParticipants[confirmedParticipantsCount] = participants[i];
    ...
    function getUnconfirmedParticipants() external view returns (Participant[] memory) {
        ...
        for (uint256 i = 0; i < participants.length; i++) {
            if (!participants[i].confirmed) {
                unconfirmedParticipants[unconfirmedParticipantsCount] = participants[i];
     ...
     * @dev Only the account with the ADMIN_ROLE can reset the participants array.
     */
    function resetParticipants() external whenPaused onlyRole(ADMIN_ROLE) {
        delete participants;
    }
     ...
     * @dev Only the account with the ADMIN_ROLE can pause the contract.
     */
     ...
     * @dev Only the account with the ADMIN_ROLE can unpause the contract.
     */
