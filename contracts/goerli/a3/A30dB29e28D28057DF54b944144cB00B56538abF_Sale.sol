...
contract Sale is ReentrancyGuard, Ownable, Pausable {
    ...
    modifier isMax(uint amount) {
        require(
            amount+_mintCount.current() <= InformationSale.mintMax,
    ...
    function _buy(uint _amount, uint _price) internal whenNotPaused isInformationSet isPresaleStart isPayEnough(_price, _amount) isMax(_amount) nonReentrant{
        for (uint256 i = 0; i < _amount; i++) {
            nft.mintTo(_msgSender());
        }
    ...
    function withdraw() external onlyOwner nonReentrant {
        require(
            gnosisWallet != address(0),
            "Withdraw: Gnosis Wallet has not been set"
        );

        if (address(this).balance > 0) {
            payable(gnosisWallet).transfer(address(this).balance);
        }
    }

    function setGnosisWallet(address _gnosisWallet) external onlyOwner {
        gnosisWallet = _gnosisWallet;
    }
