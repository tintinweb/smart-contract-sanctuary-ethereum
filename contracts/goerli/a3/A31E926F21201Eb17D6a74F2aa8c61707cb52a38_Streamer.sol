...
contract Streamer is Ownable {
    ...
    function fundChannel() public payable {
        ...
        */
        ...
        // there can only be 1 channel open per address
        require(balances[msg.sender] == 0, "Already has an opened channel");
    ...
    function withdrawEarnings(Voucher calldata voucher) public onlyOwner {
        ...
        */
        // recovering the signer address from the message and signature
        ...
        // checks that the recovered address has a channel opened with this smart contract
        require(balances[signer] > voucher.updatedBalance, "Signer does not have a payment channel open");

        // calculate payment amount
        uint256 paymentAmount = balances[signer] - voucher.updatedBalance;
        ...
        // paying the owner
        address owner = owner();
        (bool sent, ) = owner.call{value: paymentAmount}("");
        // checks that payment was successfull
        require(sent, "Payment to owner failed");

        // emit event
        emit Withdrawn(owner, paymentAmount);
    }
    ...
    */
    function challengeChannel() public {
      // checks that a channel exists with msg.sender
      require(balances[msg.sender] > 0, "User does not have a payment channel open");

      // set the deadline for closing the channel
    ...
    */
    ...
    function defundChannel() public {
      // checks that the deadline has been set to close the channel
      require(canCloseAt[msg.sender] > 0, "Channel has not been marked for closing, should first call challengeChannel()");
      // checks that deadline is past
      require(block.timestamp > canCloseAt[msg.sender], "deadline to close the channel is not past yet");

      // send the remaining funds for the user
