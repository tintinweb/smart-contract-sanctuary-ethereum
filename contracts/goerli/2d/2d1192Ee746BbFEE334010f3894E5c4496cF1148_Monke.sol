...
contract ERC1155 is IERC1155 {
    ...
    uint256 public MAX_TOKEN_SUPPLY = (1 << 64) - 1;
    ...
    event TokenMintLimitChanged(uint256 tokenId, string newMintLimit);
    ...
    function _remainingSupply(uint256 tokenId)
        ...
        returns (uint256)
    {
        if (_tokenData[tokenId].tokenMintLimit == 0) {
            return MAX_TOKEN_SUPPLY - _tokenData[tokenId].numMinted;
        }
        return
    ...
    function _burn(
        ...
        if (!_tokenData[id].burnable) revert NotBurnable();
...
contract Monke is ERC1155, Ownable, ERC2981 {
    ...
    /**
     * @dev Allows token owners to burn tokens if burn is enabled for that token.
     */
    ...
    /**
     * @dev Owner can allow for certain addresses to 
     * 
     * If this is an EOA, the burn transaction will revert.
     */
    ...
    /**
     * @dev Allows token owners to burn their tokens through owner-approved burner addresses.
     */
