...
contract NFTingAuction is Ownable, NFTingBase {
    ...
    function claim(uint256 _auctionId) external isExpiredAuction(_auctionId) {
        ...
        if (auction.winner == _msgSender()) {
            ...
            !payable(_msgSender()).send(auction.biddersToAmount[_msgSender()])
        ) {
            revert TransactionError();
        }
    }

    function cancel(uint256 _auctionId) external isRunningAuction(_auctionId) {
        Auction storage auction = auctions[_auctionId];
        if (auction.biddersToAmount[_msgSender()] == 0) {
            revert NotBidder(_auctionId, _msgSender());
        }
        ...
        if (_msgSender() == auction.winner) {
            ...
            for (uint256 i; i < auction.bidders.length; i++) {
                if (auction.bidders[i] != auction.winner) {
                    if (
                        newMaxBidAmount <
                        auction.biddersToAmount[auction.bidders[i]]
                    ) {
                        newWinner = auction.bidders[i];
                        newMaxBidAmount = auction.biddersToAmount[
                            auction.bidders[i]
                        ];
                    }
                } else {
            ...
            auction.maxBidAmount = auction.biddersToAmount[newWinner];
        } else {
            for (uint256 i; i < auction.bidders.length; i++) {
        ...
        if (!payable(_msgSender()).send(bidAmount)) {
            revert TransactionError();
        }
    }
        ...
        )
    {
        ...
        uint256[] memory amounts = new uint256[](curAuction.bidders.length);
...
contract NFTingOffer is NFTingBase {
    ...
    {
        uint256 previousPrice = offers[_offerId].price;
        offers[_offerId].price = _newPrice;

        if (offers[_offerId].buyer != _msgSender()) {
            revert PermissionDenied();
        } else if (_newPrice > previousPrice) {
            if (msg.value < _newPrice - previousPrice) {
            ...
            if (
                !offers[_offerId].buyer.send(previousPrice - _newPrice)
            ) {
                revert TransactionError();
            }
        }
