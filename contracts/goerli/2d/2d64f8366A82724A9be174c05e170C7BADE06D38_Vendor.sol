...
contract Vendor is Ownable {
  ...
  function buyTokens() public payable{
    ...
    require(msg.value > 0, "Dude cmon send some eth!");
    uint256 amountOfETH = msg.value;
    uint256 amountOfTokens = amountOfETH * tokensPerEth; /// 10 ** 18;
    uint256 vendorBalance = yourToken.balanceOf(address(this));
    require(vendorBalance>=amountOfTokens, "Sorry, we cant afford that, though we would like to");

    address buyer = msg.sender;
    (bool sent) = yourToken.transfer(buyer, amountOfTokens);
    require(sent, "didnt work out");
