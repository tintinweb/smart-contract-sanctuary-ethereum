...
contract Asset is ERC721, Mintable {
    constructor(
        ...
        address _starkEx
    ) ERC721(_name, _symbol) Mintable(_owner, _starkEx) {}
...
abstract contract Mintable is Ownable, IMintable {
  ...
  constructor(address _owner, address _starkEx) {
    starkEx = _starkEx;
    require(_owner != address(0), "Owner must not be empty");
    transferOwnership(_owner);
  }

  modifier onlyOwnerOrStarkEx() {
    require(
      msg.sender == starkEx || msg.sender == owner(),
  ...
  ) external override onlyOwnerOrStarkEx {
