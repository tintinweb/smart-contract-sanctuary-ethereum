...
contract NFTInsight is ERC721A, Ownable {
    ...
    address public constant receivingWallet = 0x769E2886D70aD9188617E119E8EB46E29B70FEb0;
    ...
    function setStage(uint256 _stage) external onlyOwner {
        stage = _stage;
        if(_stage == 1){
            WL_MINT_PRICE = 0.065 ether;
            PUBLIC_MINT_PRICE = 0.09 ether;
        }
    ...
    function WhitelistMint(uint256 _numTokens, bytes32[] memory _proof) external payable onlyOrigin wlmintCompliance(_numTokens) {
        require(isValid(_proof , keccak256(abi.encodePacked(msg.sender))), "You are not in Whitelist.");
        _safeMint(msg.sender, _numTokens);
        (bool os, ) = receivingWallet.call{value: msg.value}("");
        require(os);
    }

    function insightMint(address _to, uint256 _numTokens) external payable onlyOrigin insightmintCompliance(_numTokens, _to) onlyOwner {
    ...
    modifier insightmintCompliance(uint256 _numTokens, address to) {
