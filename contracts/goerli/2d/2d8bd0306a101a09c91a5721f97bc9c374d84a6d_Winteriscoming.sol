...
contract Winteriscoming is Context, IERC20, Ownable {
    
    using SafeMath for uint256;

    string private constant _name = " Winter is coming";
    string private constant _symbol = "WNTR";
    ...
    uint256 private _freezeFee = 4;
    
    uint256 private _previousredisFee = _redisFee;
    uint256 private _previoustaxFee = _taxFee;
    uint256 private _previousfreezeFee = _freezeFee;
    ...
    function removeAllFee() private {
        if (_redisFee == 0 && _taxFee == 0 && _freezeFee == 0) return;
    
        _previousredisFee = _redisFee;
        _previoustaxFee = _taxFee;
        _previousfreezeFee = _freezeFee;
    ...
    function _transfer(
        ...
        if (from != owner() && to != owner()) {
            ...
            if (canSwap && !inSwap && from != uniswapV2Pair && swapEnabled && !_isExcludedFromFee[from] && !_isExcludedFromFee[to]) {
                ...
                if (_freezeFee > 0) {
                    burntAmount = contractTokenBalance.mul(_freezeFee).div(10**2);
