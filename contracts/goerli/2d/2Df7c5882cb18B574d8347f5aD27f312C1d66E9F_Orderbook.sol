...
contract Orderbook {
    ...
    function cancelMakerOrder(uint256 _makerOrderID) public {
        ...
        for (uint256 x = 0; x < makerOrdersByID[_makerOrderID].num_legs; x++) {
            feedBidLiquidity[makersLegs[_makerOrderID][x].feedID] =
                feedBidLiquidity[makersLegs[_makerOrderID][x].feedID] -
                ((makerOrdersByID[_makerOrderID].ethPosted * 999) / 1000);
        }
        payable_user.transfer(makerOrdersByID[_makerOrderID].ethPosted);
    }

    function fetchUserMakerOrders(address user) public view returns(HDGXStructs.MakerOrder[] memory){
        HDGXStructs.MakerOrder[] memory returnArray = new HDGXStructs.MakerOrder[](userMakerOrders[user].length);
        for(uint i=0; i<userMakerOrders[user].length; i++){
            returnArray[i] = userMakerOrders[user][i];
        }
        return(returnArray);
    }
    
    function fetchUserTakerOrders(address user) public view returns(HDGXStructs.TakerOrder[] memory){
        HDGXStructs.TakerOrder[] memory returnArray = new HDGXStructs.TakerOrder[](userMakerOrders[user].length);
        for(uint i=0; i<userTakerOrders[user].length; i++){
            returnArray[i] = userTakerOrders[user][i];
        }
        return(returnArray);
    }

    function fetchFeedLiquidity(bytes32[] memory ids) public view returns(uint256[] memory){
    ...
    function fetchMakerOrdersByFeedID(bytes32 feedID) public view returns(HDGXStructs.MakerOrder[] memory){
        HDGXStructs.MakerOrder[] memory return_array = new HDGXStructs.MakerOrder[](makerOrdersByFeedID[feedID].length);
        uint256 count = 0;
        for(uint i = 0; i<makerOrdersByFeedID[feedID].length; i++){
            if(makerOrderIDMatched[makerOrdersByFeedID[feedID][i].order_ID] == false && makerOrderCanceled[makerOrdersByFeedID[feedID][i].order_ID] == false){
                return_array[count] = makerOrdersByFeedID[feedID][i];
                count++;
            }
        }
        HDGXStructs.MakerOrder[] memory shortened_array = new HDGXStructs.MakerOrder[](count);
