...
contract BattleRoyaleNFTOnEthereum is ERC721AQueryable, Ownable, EIP712, DefaultOperatorFilterer {
    uint public constant MINT_PRICE = 0.0077 ether;
    uint public constant TOTAL_SUPPLY = 10000;
    uint public constant FREE_SUPPLY = 1000;

    uint public constant MAX_PER_WALLET = 20;
    
    uint public constant WL_MINT_AMOUNT = 3;
    ...
    function mint(uint amount) external payable {
        require(_numberMinted(msg.sender) + amount <= MAX_PER_WALLET, "mint too many");
        uint finalTokenId = _nextTokenId() + amount;
        uint money = 0;
        if (finalTokenId > FREE_SUPPLY) {
            money = ((finalTokenId - FREE_SUPPLY) > amount ? amount : (finalTokenId - FREE_SUPPLY)) * MINT_PRICE;
        }
        require(msg.value >= money, "not enough money");
        _mintByAmount(amount);
    }

    // free wl mint
    function mintWL(uint8 v, bytes32 r, bytes32 s) external {
        require(_numberMinted(msg.sender) == 0, "minted");
    ...
    function safeTransferFrom(address from, address to, uint256 tokenId, bytes memory data)
        public      
        payable override(ERC721A, IERC721A)
