...
// File: contracts/cbridge/IBridge.sol

// https://github.com/celer-network/sgn-v2-contracts/blob/main/contracts/interfaces/IBridge.sol
...
contract DVFDepositContract is OwnableUpgradeable {
  ...
  event ProxiedBridgedWithdrawalInitiated(address indexed user, address indexed token, uint256 amount, uint64 dstChainId, string withdrawalId);
  ...
  function setCBridgeAddress(address newCBridgeAddress) public
    _isAuthorized
  {
    cBridgeAddress = newCBridgeAddress;
  }

    /**
    * @dev withdraw ERC20 tokens from the contract address to another chain using cBridge
    * NOTE: only for authorized users
    */
  function proxyWithdraw(address token, address to, uint256 amount, uint32 maxSlippage, uint64 dstChainId, uint64 nonce, string calldata withdrawalId) external
    _isAuthorized
    _withUniqueWithdrawalId(withdrawalId)
  {
    require(cBridgeAddress != address(0), 'CBRIDGE_ADDRESS_NOT_SET');
    require(token != address(0), 'BLACKHOLE_NOT_ALLOWED');
    IERC20Upgradeable(token).approve(cBridgeAddress, amount);
    IBridge(cBridgeAddress).send(
      to,
      token,
      amount,
      dstChainId,
    ...
    emit ProxiedBridgedWithdrawalInitiated(to, token, amount, dstChainId, withdrawalId);
  }
