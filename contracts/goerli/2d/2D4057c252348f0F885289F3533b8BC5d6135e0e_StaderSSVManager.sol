...
contract StaderSSVManager is Initializable, AccessControlUpgradeable {
    ETHX public ethX;
    ...
    bytes32 public constant SSV_MANAGER_ADMIN = keccak256('SSV_MANAGER_ADMIN');
    ...
    function initialize(
        ...
        ssvToken = _ssvToken;
        minDeposit = 10;
        maxDeposit = 32 * decimals;
        exchangeRate = 1 * decimals;
        _grantRole(SSV_MANAGER_ADMIN,  msg.sender);
    ...
    ) public onlyRole(SSV_MANAGER_ADMIN) {
        ...
        onlyRole(BOT_ROLE)
    {
        ...
        require(validatorsRegistry[registeredValidatorCount].depositStatus,'deposit did not happen for validator');
    ...
    function depositEthtoDepositContract() external  onlyRole(BOT_ROLE) {
        require(
        ...
        validatorsRegistry[registeredValidatorCount].depositStatus = true;
        emit depositToDepositContract(validatorsRegistry[registeredValidatorCount].pubKey);
    }
        ...
        returns (uint256)
    {
        for (uint256 i = 0; i < validatorCount; i++) {
            if (
                keccak256(_publicKey) == keccak256(validatorsRegistry[i].pubKey)
            ) {
                return i;
            }
        }
     ...
     */
    function updateExchangeRate(uint256 _totalTVL) public  onlyRole(BOT_ROLE) returns(uint256){
        totalTVL = _totalTVL;
    ...
    function updateMinDeposit(uint256 _minDeposit) external  onlyRole(SSV_MANAGER_ADMIN) {
        require(_minDeposit > 0, "minimum deposit should be greater than 0");
        minDeposit = _minDeposit;
    }

    function updateMaxDeposit(uint256 _maxDeposit) external  onlyRole(SSV_MANAGER_ADMIN) {
        require(
...
contract ETHX is ERC20, ERC20Burnable, AccessControl {
    ...
    constructor() ERC20("ETHX-DEMO", "ETHX-DEMO") {
        _grantRole(DEFAULT_ADMIN_ROLE, msg.sender);
