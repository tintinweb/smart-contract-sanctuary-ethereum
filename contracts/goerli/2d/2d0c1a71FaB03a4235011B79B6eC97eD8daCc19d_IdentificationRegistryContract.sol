...
contract IdentificationRegistryContract is Initializable, ERC721Upgradeable, ERC721EnumerableUpgradeable, ERC721VotesUpgradeable, OwnableUpgradeable {
    ...
    event UpdateCensus(bytes32 indexed merkleTree, string ipfsHash);
    ...
    modifier onlyTokenOwner(uint256 tokenId) {
        require(msg.sender == ownerOf(tokenId), "IdentificationRegistryContract: Caller is not the owner of the tokenID");
        _;
    }
    ...
    function updateCensus(
        ...
        _setMerkleRoot(_merkleRoot);
        _setIpfsHash(_ipfsHash);
        emit UpdateCensus(_merkleRoot, _ipfsHash);
    }

    // Return the tokenID at the specific index if it has delegated to himself, 0 otherwise
    function getDelegateeByIndex(uint256 index) external view returns(uint256, bool) {
    ...
    // Return the tokenID at the specific index if it has delegated to himself, 0 otherwise
    function getTokenIdByIndexIsDelegable(uint256 index) external view returns(uint256, bool) {
        uint256 tokenId = tokenByIndex(index);
        uint256 delegate = delegates(tokenId);
        if(tokenId == delegate) {
            return (tokenId, true);
    ...
    function removeFromCensus(uint256 tokenId) external onlyOwner {
        require(_exists(tokenId), "IdentificationRegistryContract: TokenId doesn't exist");
        _burn(tokenId);
    }

    function setUsername(uint256 tokenId, string calldata username) public onlyTokenOwner(tokenId) {
    ...
    function getCensusEntry(uint256 index) external view returns(CensusEntry memory) {
        uint256 tokenId = tokenByIndex(index);
        string memory username = getUsername(tokenId);
        address owner = ownerOf(tokenId);
        uint256 votingPower = getVotes(tokenId);
        uint256 delegatee = delegates(tokenId);
        CensusEntry memory censusEntry = CensusEntry(tokenId, delegatee, votingPower, owner, username);
        return censusEntry;
    }
...
abstract contract VotesUpgradeable is Initializable, IVotesUpgradeable {
    ...
    function _returnDelegate(uint256[] memory array, uint256 oldDelegatee) internal {
        ...
        uint256[] memory updatedDelegators = new uint256[](0);
        _delegators[oldDelegatee] = updatedDelegators;
    }
