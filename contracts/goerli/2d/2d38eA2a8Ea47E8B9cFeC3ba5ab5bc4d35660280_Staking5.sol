...
contract Staking5 is OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    mapping(address => mapping(uint128 => DetailsLibraryUpdated.eachTransaction)) public userStakesData;  //address-->stakeType--->transactionData
    ...
    // removed the stakeTypeExist mapping as a optimizational step. As stake type can be checked through stakeTypesList array length, thus removing the 
    // ambiguity around _stakeType used as key for stakeTypeExist mapping and index for stakeTypesList array
    ...
    function deposit(address user,uint128 _amount, uint128 _stakeType) external payable nonReentrant onlyOwner returns(uint128 emitAmount, uint128 _period, uint128 _totalPoolStaked) {
        ...
        if(fees > 0) 
            ...
            stakedToken.transferFrom(user, ownerWallet, fees);
        ...
        if(stakes.stakeAmount == 0) {
            ...
            // userTotalPerPool[user][_stakeType] +=  realAmount + totalRewardTillNow;
    ...
    function withdraw(address user, uint128 _amount, uint128 _stakeType) external payable nonReentrant onlyOwner returns(uint128, uint128, uint128) {
        ...
        require(_stakeType < uint128(stakeTypesList.length), "INVALID_STAKE_TYPE");
        // require(stakeTypeExist[_stakeType], "STAKE_TYPE_DOES_NOT_EXIST");
        // require(_amount <= userTotalPerPool[user][_stakeType], "CANT_WITHDRAW_MORE_THAN_STAKED");   // --------
        ...
        // userTotalPerPool[user][_stakeType] -=  actualAmount;
    ...
    function rewardCalculation(uint128 _amount, uint128 _stakeType, uint128 _time) public view returns(uint128) {
        ...
        uint128 interest = (_amount * rate * _time)  / (100 * 365 minutes);
        return interest;
    ...
    function deleteStakeType(uint128 _stakeType) external onlyOwner returns(bool) {
        ...
        // stakeTypeExist[_stakeType] = false;
        stakeTypesList[_stakeType].isActive = false;


        // for(uint128 i = _stakeType; i < stakeTypesList.length - 1; i++ ) {
        //     DetailsLibraryUpdated.StakeTypeData memory temp = stakeTypesList[i];
        //     stakeTypesList[i] = stakeTypesList[i + 1];
        //     stakeTypesList[i].stakeType -= 1;
        //     stakeTypesList[i + 1] = temp;
        // }
        // stakeTypesList.pop();
    ...
    //     require(_token != address(stakedToken), "CANT_BE_SAME_ADDRESS");
    //     stakedToken = IStakedToken(_token);
    //     emit UpdateStakeToken(_token);
    // }
    ...
    function emergencyWithdraw(address user, uint128 _stakeType) external payable onlyOwner returns(uint128, uint128, uint128){
        ...
        // userTotalPerPool[user][_stakeType] =  0;
        totalStaked -= amount;
