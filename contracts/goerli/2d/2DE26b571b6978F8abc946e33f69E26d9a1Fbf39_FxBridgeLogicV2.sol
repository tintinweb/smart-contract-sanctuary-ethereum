...
contract FxBridgeLogicV2 is ReentrancyGuardUpgradeable, OwnableUpgradeable, PausableUpgradeable {
    ...
    /**
    * @dev Update params FxBridgeLogicV2
    *  Add convert_decimals
    *  Compatible with the problem that the same token is inconsistent in decimals on different blockchains;
    *  Convert gravity module compatible with fxCore to crosschain module;
    *  Add ETH into bridge token;
    */
    ...
    function init(bytes32 _fxBridgeId, uint256 _powerThreshold, address[] memory _oracles, uint256[] memory _powers) public initializer {
        ...
        require(_oracles.length == _powers.length, "Malformed current validator set");
    ...
    /* =============== MUTATIVE FUNCTIONS  =============== */
    /* Deprecated: after fxcore upgrade v3 */
    function setFxOriginatedToken(address _tokenAddr) public onlyOwner returns (bool)  {
        ...
        emit AddBridgeTokenEvent(_tokenAddr, IERC20MetadataUpgradeable(_tokenAddr).name(), IERC20MetadataUpgradeable(_tokenAddr).symbol(), IERC20MetadataUpgradeable(_tokenAddr).decimals(), state_lastEventNonce, bytes32(0));
        return true;
    }

    function addBridgeToken(address _tokenAddr, bytes32 _channelIBC) public onlyOwner returns (bool)  {
        require(!_isContainToken(bridgeTokens, _tokenAddr), "Token already exists");
        require(_tokenAddr != address(0), "Invalid address");
        if (_tokenAddr == ethDesignAddress()) {
            require(_channelIBC == bytes32(0), "Invalid channel IBC");
            bridgeTokens.push(_tokenAddr);
            state_lastEventNonce = state_lastEventNonce.add(1);
            emit AddBridgeTokenEvent(_tokenAddr, "ethereum", "ETH", 18, state_lastEventNonce, _channelIBC);
        } else {
            require(_tokenAddr != state_fxOriginatedToken, 'Invalid bridge token');
            bridgeTokens.push(_tokenAddr);
            state_lastEventNonce = state_lastEventNonce.add(1);
            ...
            emit AddBridgeTokenEvent(_tokenAddr, IERC20MetadataUpgradeable(_tokenAddr).name(), IERC20MetadataUpgradeable(_tokenAddr).symbol(), decimals, state_lastEventNonce, _channelIBC);
        }
        return true;
    }
    ...
    function updateOracleSet(address[] memory _newOracles, uint256[] memory _newPowers, uint256 _newOracleSetNonce,
        ...
        require(_newOracleSetNonce > _currentOracleSetNonce, "New valset nonce must be greater than the current nonce");

        require(_newOracles.length == _newPowers.length, "Malformed new validator set");
    ...
    function sendToFx(address _tokenContract, bytes32 _destination, bytes32 _targetIBC, uint256 _amount) public payable nonReentrant whenNotPaused {
        ...
        if (_tokenContract == ethDesignAddress()) {
            ...
            if (_tokenContract == state_fxOriginatedToken) {
                ...
                if (targetDecimals > 0) {
                    uint256 decimals = IERC20MetadataUpgradeable(_tokenContract).decimals();
                    targetAmount = convertAmount(_amount, decimals, targetDecimals);
                    require(convertAmount(targetAmount, targetDecimals, decimals) == _amount, "Convert loss of precision");
                }
            }
        }
        state_lastEventNonce = state_lastEventNonce.add(1);
        emit SendToFxEvent(_tokenContract, msg.sender, _destination, _targetIBC, targetAmount, state_lastEventNonce);
    }

    function submitBatch(address[] memory _currentOracles, uint256[] memory _currentPowers,
        uint8[] memory _v, bytes32[] memory _r, bytes32[] memory _s,
        uint256[] memory _amounts, address payable[] memory _destinations, uint256[] memory _fees,
        uint256[2] memory _nonceArray, address _tokenContract, uint256 _batchTimeout, address payable _feeReceive
    ) public payable nonReentrant whenNotPaused {
        {
...

            {
                ...
                if (decimals > 0) {
                    targetDecimals = IERC20MetadataUpgradeable(_tokenContract).decimals();
                }
                uint256 totalFee;
                bool isOriginToken = _tokenContract == state_fxOriginatedToken;
                for (uint256 i = 0; i < _amounts.length; i++) {
                    if (isOriginToken) {
                        ...
                        if (_tokenContract == ethDesignAddress()) {
                            ...
                            IERC20MetadataUpgradeable(_tokenContract).safeTransfer(_destinations[i], convertAmount(_amounts[i], decimals, targetDecimals));
                        }
                    }
                ...
                if (isOriginToken) {
                    ...
                    if (_tokenContract == ethDesignAddress()) {
                        _feeReceive.transfer(totalFee);
                    } else {
                        IERC20MetadataUpgradeable(_tokenContract).safeTransfer(_feeReceive, convertAmount(totalFee, decimals, targetDecimals));
                    }
                }
            }
        }
    ...
    function updateTokenDecimalsConvert(address _token, uint8 _to) public onlyOwner returns (bool) {
    ...
    function ethDesignAddress() public pure returns (address) {
    ...
    function convertAmount(uint256 _amount, uint256 sourceDecimals, uint256 targetDecimals) internal pure returns (uint256) {
        if (targetDecimals == 0) {
            return _amount;
        }
    ...
    function checkOracleSignatures(address[] memory _currentOracles, uint256[] memory _currentPowers,
        ...
        for (uint256 i = 0; i < _currentOracles.length; i++) {
            if (_v[i] != 0) {
                require(verifySig(_currentOracles[i], _theHash, _v[i], _r[i], _s[i]), "Validator signature does not match.");
