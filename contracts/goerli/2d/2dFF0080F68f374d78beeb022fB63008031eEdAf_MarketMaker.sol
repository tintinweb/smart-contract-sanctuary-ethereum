    ...
    function add(ExchangeDomain.Order calldata order) external {
        ...
        require(
            orders[key].selling == 0 &&
                orders[key].buying == 0 &&
                orders[key].sellerFee == 0,
            "Order is already existed. Try to change salt"
        );
        orders[key] = OrderParams(order.selling, order.buying, order.fee);
    }
...
contract MarketMaker is OwnableExt, ExchangeDomain {
    ...
    /// @notice This function is called by contract owner and sets fee receiver address.
    function setErc20TransferProxy(ERC20TransferProxy _erc20TransferProxy)
        external
        onlyOwner
    {
        erc20TransferProxy = _erc20TransferProxy;
    ...
    function validateOrderSig(Order memory order, ECDSASig memory sig)
        public
        view
    {
    ...
    function transferFeeToBeneficiary(
        ...
        (uint256 restValue, uint256 restFee) = subFeeInBp(total, total, fee);

        if (restFee > 0) {
            transfer(asset, fee, from, beneficiary);
        }
...
contract ERC20TransferProxy is OwnableExt {
...
contract TransferProxy is OwnableExt {
