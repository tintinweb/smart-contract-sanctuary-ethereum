...
contract StakeBitcoinxEarnUSD is ReentrancyGuard{
    ...
    uint256 public rewardPerBlock = 2140410000000; //wei in USD Per 1M BTCX (USD 0.00000000214041) 
    uint256 public baseShare = 1000000000000000000000000000; //1B BTCX
    ...
    function doStake(uint256 amount) external nonReentrant{
        ...
        if(rewardBalances[msg.sender] > 0){
            ...
            console.log('Reward sent for restake', rewardBalances[msg.sender]);
        ...
        lockedDurations[msg.sender] = block.timestamp + 1 days;
    ...
    function claimReward(uint256 amount) external payable nonReentrant{
        ...
        _updateReward(msg.sender);
        console.log('User Stake Date ', stakedDates[msg.sender]);
        console.log('current timestamp', block.timestamp);
        console.log('Reward Balance ', rewardBalances[msg.sender]);
        if(rewardBalances[msg.sender] > 0){
            ERC20(rewardToken).transfer(msg.sender, rewardBalances[msg.sender]);
            emit RewardPaid(msg.sender, rewardBalances[msg.sender]);
            rewardPaidBalances[msg.sender] += rewardBalances[msg.sender];
    ...
    function unStakeToken(uint256 amount) external payable nonReentrant{
        require(amount >= baseFee, 'Low Gas Fee');
        _updateReward(msg.sender);
        console.log('Locked: ', lockedDurations[msg.sender]);
        console.log('Current Timestamp: ', block.timestamp);
        if(block.timestamp < lockedDurations[msg.sender]){
            ...
            console.log('Send to Fee Wallet:' , penaltyAmount);
            ERC20(stakeToken).transfer(msg.sender, stakeBalances[msg.sender] - penaltyAmount);
            console.log('Unstake with Penalty:' , stakeBalances[msg.sender] - penaltyAmount);
            emit UnStaked(msg.sender, stakeBalances[msg.sender] - penaltyAmount);
            stakeBalances[msg.sender] = 0;

            ERC20(rewardToken).transfer(msg.sender, rewardBalances[msg.sender]);
            emit RewardPaid(msg.sender, rewardBalances[msg.sender]);
            console.log('Reward Sent' , rewardBalances[msg.sender]);
            ...
            console.log('Unstake with no Penalty:' , stakeBalances[msg.sender]);
            totalStakedAmount -=  stakeBalances[msg.sender]; 
            stakeBalances[msg.sender] = 0;
