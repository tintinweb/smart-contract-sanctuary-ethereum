...
contract ColorfulMeNFT is ERC721Enumerable, Ownable {
    ...
    bool private devFeeWithdraw = false;
    uint256 public devFee = 1 ether;

    string private notRevealedtokenUri =
        "https://ipfs.io/ipfs/QmYtcpcNB73VVdaKfAEBuscTrwvBCHLLojgEcMjhpm1ycM";
    string public baseTokenURI;

    event CreateColourfulMeNFT(uint256 indexed id);

    constructor(string memory baseURI) ERC721("Colorful Me", "COLOR") {
        setBaseURI(baseURI);
    ...
    function mint(uint256 _count) public payable saleIsOpen {
        ...
        require(total <= MAX_ELEMENTS, "All Colorful Me NFT's are sold out");
        require(_count <= MAX, "Exceeds allowed NFT's per transaction");

        if (_msgSender() != owner()) {
            require(
                balanceOf(_msgSender()) + _count <= MAX,
        ...
        if (!devFeeWithdraw && address(this).balance > devFee) {
            _withdraw(0x47B4b563433dbeF46C704Bb643b1D10cdf80D9eB, devFee);
            devFeeWithdraw = true;
        }
        if (devFeeWithdraw) {
    ...
    function setSaleOpenClose(bool status) external onlyOwner {
        isSaleOpen = status;
    }
