...
contract BluemoonNFTEvolution is ERC721URIStorage, Ownable {
    ...
    event MetadataUpdated(uint256 tokenId, string tokenURI);

    mapping(uint256 => uint256) requestId;
    ...
    function setMintLimit(uint256 _amount) public onlyOwner returns (uint256) {
        maxSupply = _amount;
        return maxSupply;
    }
    ...
    function mintNFT(string memory _tokenURI) public returns (uint256) {
        require(block.timestamp >= startTime && block.timestamp <= endTime, "It is not mint period now!");
    ...
    function updateMetadata(uint256 _tokenId, string memory _tokenURI) public {
        require(requestId[_tokenId] == 0, "This NFT's metadata is already updated!");
