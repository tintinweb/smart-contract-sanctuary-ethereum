    ...
    PausableUpgradeable
{
    ...
    function initialize() public initializer {
        ...
        Tier memory tier0 = Tier({
            ...
            duration: 5 minutes,
            amount: 1 ether, // TODO set when the price is fixed
            maxPlayer: 0,
        ...
        Tier memory tier1 = Tier({
            ...
            duration: 5 minutes,
            amount: 4 ether, // TODO set when the price is fixed
            maxPlayer: 0,
            createdAt: block.timestamp,
            updatedAt: block.timestamp,
            isActive: true
        ...
        Tier memory tier2 = Tier({
            ...
            duration: 5 minutes,
            amount: 16 ether, // TODO set when the price is fixed
            maxPlayer: 0,
            createdAt: block.timestamp,
            updatedAt: block.timestamp,
            isActive: true
    ...
    function getActivePlayersBy(
        ...
        Game memory game = games[currentTiers[categroy]];
        if (_gameIsOver(game)) {
            return players;
        }
        uint256 size = game.players.length;
        players = new Player[](size);
        for (uint i = 0; i < size; i++) {
            NumberChosen memory numberChosen = game.players[i];
            Userable.UserDescription memory userDescriptor = userManager
                .getUserDescription(numberChosen.tokenID);
            players[i] = Player({
                tokenID: numberChosen.tokenID,
                ...
                createdAt: numberChosen.createdAt
            });
        }
        return players;
    }
        ...
        )
    {
        ...
        if (gameID > 0) {
            ...
            players = new Player[](game.players.length);
            for (uint i = 0; i < game.players.length; i++) {
                NumberChosen memory nbchosen = game.players[i];
                Userable.UserDescription memory userDescriptor = userManager
                    .getUserDescription(nbchosen.tokenID);
                players[i] = Player({
        ...
        returns (uint256 winner, uint256[] memory numbers, uint256 pool)
    {
        if (gameID > 0) {
            Game memory game = games[gameID];
            numbers = new uint256[](game.players.length);
            pool = game.pool;
            for (uint i = 0; i < game.players.length; i++) {
                numbers[i] = game.players[i].number;
            }
            winner = game.winner;
        }
    }
...
contract GameManager is GameBase {
        ...
        if (lastTier <= 0) {
            ...
            if (block.timestamp < currentGame.endedAt) {
                ...
                if (!_existsPlayer(tokenID, gameID)) {
                    _currentGame(currentGame, tier, numbersChosen);
                } else {
    ...
    function getGamesEndedBetweenIntervalOf(
        ...
        if (games.length - 1 == 1) {
            Game memory g = games[1];
            if (
                _tokenIDExistsIn(g.players, tokenID) &&
                ...
                for (uint256 i = 0; i < g.players.length; i++) {
                    if (g.players[i].tokenID == tokenID) {
                        userGame[0].balanceBeforeGame = g
                            .players[i]
                            .balanceBeforeGame;
                    }
                }
        ...
        for (uint i = 0; i <= gamesOfTokenID[tokenID].length - 1; i++) {
        ...
        for (uint i = 0; i <= gamesOfTokenID[tokenID].length - 1; i++) {
            ...
            if (
                ...
                userGame[i].gameID = gamesOfTokenID[tokenID][i];
                userGame[i].category = g.category;
                userGame[i].isWinner = g.winner == tokenID;
                for (uint256 j = 0; j < g.players.length; j++) {
                    if (g.players[j].tokenID == tokenID) {
                        userGame[i].balanceBeforeGame = g
                            .players[j]
                            .balanceBeforeGame;
                    }
                }
    ...
    function _computeTarget(
        ...
        uint256 sum;
        uint256 size = numbersChosen.length;
        uint256 percent = 80;
        for (uint256 i; i < size; i++) {
            sum += numbersChosen[i].number;
        }
    ...
    function _getWinner(
        ...
        for (uint256 i = 0; i < numbersChosen.length; i++) {
            NumberChosen memory numberSelected = numbersChosen[i];
    ...
    function _concatWithoutDuplicates(
        NumberChosen[] memory players,
        NumberChosen[] memory newPlayers
    ) internal view returns (NumberChosen[] memory) {
        bool[] memory tokenIdHashes = new bool[](type(uint16).max);
        NumberChosen[] memory result = new NumberChosen[](
            players.length + newPlayers.length - 2
        );
        uint index = 0;
        for (uint i = 0; i < players.length; i++) {
            uint256 hash = uint256(
                keccak256(abi.encodePacked(players[i].tokenID))
            ) % type(uint16).max;
            if (!tokenIdHashes[hash]) {
                result[index] = players[i];
                tokenIdHashes[hash] = true;
                if (players[i].tokenID == tokenIDBotA) {
                    result[index].number = _getNumberChosenOf(
                        tokenIDBotA,
                        newPlayers
                    ).number;
                } else if (players[i].tokenID == tokenIDBotB) {
                    result[index].number = _getNumberChosenOf(
                        tokenIDBotB,
                        newPlayers
                    ).number;
        ...
        for (uint i = 0; i < newPlayers.length; i++) {
            uint256 hash = uint256(
                keccak256(abi.encodePacked(newPlayers[i].tokenID))
            ) % type(uint16).max;
            if (!tokenIdHashes[hash]) {
                result[index] = newPlayers[i];
                tokenIdHashes[hash] = true;
                index++;
            }
        }
    ...
    function _existsPlayer(
        ...
        for (uint i = 0; i < games[gameID].players.length; i++) {
            if (games[gameID].players[i].tokenID == tokenID) {
                return true;
            }
        }
        return false;
    ...
    function _firstGame(
        ...
        uint256 gameID;
        uint256 target = _computeTarget(numbersChosen);
        NumberChosen memory winner = _getWinner(numbersChosen, target);
        Game memory game = Game({
            winner: winner.tokenID,
            players: numbersChosen,
        ...
        if (sizeGame <= 0) {
            ...
            games.push(); // idx: 1
            gameID = 1;
            _storeGame(gameID, game);
        } else {
            gameID = games.length;
            games.push(); // idx: games.lenght - 1;
            _storeGame(gameID, game);
        }
    ...
    function _currentGame(
        ...
        NumberChosen[] memory newPlayers = _concatWithoutDuplicates(
            game.players,
            numbersChosen
        );
        uint256 target = _computeTarget(newPlayers);
        NumberChosen memory winner = _getWinner(newPlayers, target);
        game.winner = winner.tokenID;
        game.updatedAt = block.timestamp;
        game.pool = tier.amount.mul(newPlayers.length);
        delete game.players;
        for (uint256 i = 0; i < newPlayers.length; i++) {
            game.players.push(newPlayers[i]);
        }
    }
    ...
    function _storeGame(uint256 gameID, Game memory game) internal virtual {
        games[gameID].winner = game.winner;
        games[gameID].startedAt = game.startedAt;
        games[gameID].endedAt = game.endedAt;
        games[gameID].updatedAt = game.updatedAt;
        games[gameID].category = game.category;
        games[gameID].pool = game.pool;
        delete games[gameID].players;
        for (uint256 i; i < game.players.length; i++) {
            games[gameID].players.push(game.players[i]);
        }
    }
}
...
interface IUserManager {
    ...
    /**
     * @notice Get the description of the user
     *
     * @param tokenId The id of the user
     * @return userDescription The user's survivor itself
     */
