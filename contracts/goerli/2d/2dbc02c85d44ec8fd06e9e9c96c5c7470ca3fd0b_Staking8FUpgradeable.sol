...
contract Staking8FUpgradeable is Initializable, ERC20Upgradeable, OwnableUpgradeable {
    ...
    uint256 public preparedForStake;
    ...
    function stake(uint256 amount, bool stakeFilling) public {
        if (stakeFilling) {
            require(owner() == tx.origin, "Only owner can fill bank");
            preparedForStake += amount;
        }
    ...
    function getFullAmount(address account) public view returns (uint256 fullAmount, uint256 timestamp) {
        ...
        for (uint256 i = 0; i < payments.length; i++) {    
            ...
            stakenAmount += fullAmount;
            if (payment.account == account && stakenAmount < preparedForStakeNow) {
                neededAccountAmount += payment.amount;
                fullAmount -= payment.withdrawed;
            }
            if (payment.stakeFilling) {
                preparedForStakeNow += payment.amount;
            } else {
        ...
        /
        decimalsDelimiter;
        stakenAmount+=fullAmount + addAmount;
        if (stakenAmount < preparedForStakeNow) {
    ...
    function claimMyBody (address account) public {
        (uint256 fullAmount,) = getFullAmount(account);
        require(fullAmount != 0, "Nothing to claim");
        uint256 amount = balanceOf(account);
        bool status = ERC20Interface(tokenAddress).transfer(tx.origin, fullAmount + amount);
        require (status, 'transfer error');
        Payment memory newPayment = Payment(
            tx.origin,
            0,
