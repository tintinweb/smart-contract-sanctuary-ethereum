...
contract IncentiveManager is IIncentiveManager, IncentiveManagerStorage, Ownable, Multicall {
  ...
  modifier checkAsset(address pool) {
    IncentivePlan storage p = _incentivePlans[pool];

    if (p.lastUpdateTimestamp == 0) revert IncentivePlanNotExist();
    _;
  }
  ...
  function updateUserIncentive(address pool, address account) external override checkAsset(pool) {
  ...
  function updatePlanState(address pool) external override checkAsset(pool) {
    _updatePlanState(pool);
  }
  ...
  function claimIncentive(address pool) external override checkAsset(pool) {
    ...
    emit ClaimIncentive(pool, user, incentiveAmount);
  }
  ...
  function setIncentivePlan(address pool, uint256 allocation) external override onlyCore {
    IncentivePlan storage p = _incentivePlans[pool];
  ...
  function updatePlanAllocation(address pool, uint256 newAllocation)
    external
    override
    onlyOwner
    checkAsset(pool)
  ...
  function updateTotalIncentive(uint256 totalIncentivePerSecond) external override onlyOwner {
    ...
    emit UpdateTotalIncentive(_totalIncentivePerSecond);
  }

  /// ************** View Functions ************* ///

  /// @inheritdoc IIncentiveManager
  function getPlanIndex(address pool) public override view checkAsset(pool) returns (uint256) {
  ...
  {
    userIndex = _incentivePlans[pool].userIndex[user];
    userPreviousIncentive = _incentivePlans[pool].userIncentive[user];
    userIncentive = getUserIncentive(pool, user);
  }
  ...
  {
    incentiveAsset = address(_incentiveAsset);
    totalIncentivePerSecond = _totalIncentivePerSecond;
    ...
    )
  {
    ...
    poolAllocatedIncentive = (_totalIncentivePerSecond * p.allocation) / _totalAllocation;

    return (p.lastUpdateTimestamp, p.planIndex, poolAllocatedIncentive, p.allocation);
  }
