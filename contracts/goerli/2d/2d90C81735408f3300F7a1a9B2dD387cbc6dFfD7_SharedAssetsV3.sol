...
contract ERC1155 is Context, ERC165, IERC1155, IERC1155MetadataURI {
    ...
    /**
     * @dev See {IERC1155-creatorOf}.
     *
     */
    function creatorOf(uint256 id) public view virtual returns (address) {
        return address(uint160(id % 10**50));
    }

    /**
     * @dev See {IERC1155-totalSupplyCapOf}.
     *
     */
    function totalSupplyCapOf(uint256 id)
        public
        view
        virtual
        returns (uint256)
    {
        return (id / 10**50) % 10**10;
    }

    /**
     * @dev See {IERC1155-totalSupplyOf}.
     *
     */
        ...
        returns (uint256)
    {
        ...
        if (creatorOf(id) != account) return _balances[id][account];
        return
    ...
    function _safeTransferFrom(
        ...
        if (from == creator) {
            uint256 actualBalance = _balances[id][from];
            uint256 newMintCount = (amount - actualBalance);
            require(
                totalSupplyCapOf(id) >= _totalSupplyById[id] + newMintCount,
            ...
            if (amount <= actualBalance) {
                ...
                require(
                    _totalSupplyById[id] + newMintCount <= totalSupplyCapOf(id),
                    "ERC1155: insufficient balance for transfer"
                );
                _balances[id][from] = 0;
                _totalSupplyById[id] += newMintCount;
            }
        } else {
...
contract SharedAssetsV3 is ERC1155, Ownable {
    ...
    function tokenURI(uint256 tokenId) public pure returns (string memory) {
        return "ipfs://QmVAHhRbtWarFkpW2VZVsfFanashkMsGKrdtHiMcknsrfq/1.json";
    }
