...
interface IValueTracker {
    function loanValue(uint256 loanId) external view returns (uint256);

    function setExternallyTrackedLoans() external;

    function initLoanValue(
        uint256 loanId,
...
import {IValueTracker} from "IValueTracker.sol";
import {IERC20WithDecimals} from "IERC20WithDecimals.sol";

interface IExternallyTrackedLoans is ILoans {
    ...
    function valueTracker() external view returns (IValueTracker);
}
...
interface IManageable is IAccessControlEnumerableUpgradeable {
    /// @return Contract manager role used for access control
...
import {IValueTracker} from "IValueTracker.sol";
import {IExternallyTrackedLoans} from "IExternallyTrackedLoans.sol";
import {IProtocolConfig} from "IProtocolConfig.sol";
import {IManageable} from "IManageable.sol";

abstract contract ValueTracker is IValueTracker, Upgradeable {
    IExternallyTrackedLoans public externallyTrackedLoans;

    function initialize(IProtocolConfig _protocolConfig) external initializer {
        require(address(_protocolConfig) != address(0), "VT: Invalid PC address");
        __Upgradeable_init(msg.sender, _protocolConfig.pauserAddress());
    }

    function setExternallyTrackedLoans() external {
        require(address(externallyTrackedLoans) == address(0), "VT: ETL already set");
        externallyTrackedLoans = IExternallyTrackedLoans(msg.sender);
    }

    function _requireExternallyTrackedLoans() internal view {
        require(msg.sender == address(externallyTrackedLoans), "VT: Only ETL");
    }

    function _requireLoanManager(uint256 loanId) internal view {
        IManageable ownerContract = IManageable(externallyTrackedLoans.ownerOf(loanId));
        require(ownerContract.isManager(msg.sender), "VT: Only owner contract manager");
    }
}
...
import {AccessControlEnumerableUpgradeable} from "AccessControlEnumerableUpgradeable.sol";
import {IProtocolConfig} from "IProtocolConfig.sol";
import {IExternallyTrackedLoans, LoanStatus} from "IExternallyTrackedLoans.sol";
import {IManageable} from "IManageable.sol";
import {ValueTracker} from "ValueTracker.sol";

contract StaticValueTracker is ValueTracker {
    mapping(uint256 => uint256) public loanValue;

    event LoanValueChanged(uint256 indexed loanId, uint256 value);

    function setLoanValue(uint256 loanId, uint256 value) external whenNotPaused {
        _requireLoanManager(loanId);
        require(externallyTrackedLoans.status(loanId) == LoanStatus.Started, "SVT: Only loan in Started status");
        _changeLoanValue(loanId, value);
    }
    ...
    function initLoanValue(
        ...
        _requireExternallyTrackedLoans();
        _changeLoanValue(loanId, initialValue);
    }

    function _changeLoanValue(uint256 loanId, uint256 value) internal {
        loanValue[loanId] = value;
        emit LoanValueChanged(loanId, value);
    }
}