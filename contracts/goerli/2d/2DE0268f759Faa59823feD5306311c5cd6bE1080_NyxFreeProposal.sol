...
contract NyxFreeProposal is Ownable, converter {
    string public constant name = "Nyx Proposal - Free";
    ...
    function parseProposalParams(bytes[] memory params)
        public pure
        returns (FreeProposalParams memory)
    {
        require(params.length == 2, "wrong format");

        return FreeProposalParams(converter.bytesToString(params[0]),
        ...
        returns (FreeProposalParams memory)
    {
        ...
        return FreeProposalParams(converter.bytesToString(param1), converter.bytesToString(param2));
    }
    ...
    function parseProposalParams2(bytes memory params)
        ...
        returns (FreeProposalParams memory)
    {
        (string memory param1, string memory param2) = abi.decode(params, (string, string));
        return FreeProposalParams(param1, param2);
    }

    function proposalToBytes(FreeProposal memory proposal)
        public pure
        returns (bytes memory)
    {
        return abi.encode(proposal.title, proposal.description);
    }
    ...
    function createProposal(uint256 proposalId, bytes[] memory params)
        ...
        returns (bytes memory)
    {
        FreeProposalParams memory parsedParams = parseProposalParams(params);
        FreeProposal memory proposal = FreeProposal(proposalId, parsedParams.title, parsedParams.description);
    ...
    function getProposal(bytes memory proposalBytes)
        ...
        returns (bytes[] memory)
    {
        FreeProposalParams memory params = parseProposalParams2(proposalBytes);
        bytes[] memory prop = new bytes[](2);
        prop[0] = abi.encode(params.title);
        prop[1] = abi.encode(params.description);
        return prop;
    }
        ...
        returns (bytes[2][] memory props)
    {
        ...
        {
            FreeProposalParams memory params = parseProposalParams2(proposalBytesArr[idx]);
            props[idx] = [abi.encode(params.title), abi.encode(params.description)];
        }
    }
}
