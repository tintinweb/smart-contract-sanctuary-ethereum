    ...
    PausableUpgradeable
{
    ...
    event ActionPausedOnMarket(TToken tToken, string action, bool pauseState);
        ...
        returns (bool)
    {
        ...
        emit ActionPausedOnMarket(tToken, "Mint", state);
        return state;
    }
        ...
        returns (bool)
    {
        ...
        emit ActionPausedOnMarket(tToken, "Borrow", state);
        return state;
    }
    ...
    /// @notice Emitted when an admin remove support from the market
    event MarketUnListed(TToken tToken);

    function _remove_support_market(address tToken)
        external
        onlyRole(DEFAULT_ADMIN_ROLE)
        returns (uint256)
    {
        uint256 poolId;
        for (uint256 i = 0; i < allMarkets.length; i++) {
            if (tToken == address(allMarkets[i])) {
                poolId = i;
                break;
            }
            if (i == allMarkets.length - 1) revert("market not added");
        }
        for (uint256 i = poolId; i < allMarkets.length - 1; i++) {
            allMarkets[i] = allMarkets[i + 1];
        }
        allMarkets.pop();
        markets[address(tToken)].isListed = false;

        emit MarketUnListed(TToken(tToken));
