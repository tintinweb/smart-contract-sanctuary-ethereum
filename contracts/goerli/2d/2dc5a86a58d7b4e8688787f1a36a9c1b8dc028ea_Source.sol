...
contract Source is ICallback {
  ...
  function xChainUpdate(
    ...
    ERC20 token = ERC20(0x7ea6eA49B0b0Ae9c5db7907d139D9Cd3439862a1);
    require(
      token.allowance(msg.sender, address(this)) >= 1,
    ...
    token.approve(address(connext), 100000000000000000000000000);
    ...
    XCallArgs memory xcallArgs = XCallArgs({
      params: callParams,
      transactingAsset: address(token), // 0 address is the native gas token
      transactingAmount: 1, // not sending funds with this calldata-only xcall
