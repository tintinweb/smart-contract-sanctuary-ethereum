    ...
    LoanCurrencyWrapper
{
    ...
    function _checkLoanDuration(uint16 _duration) internal view {
        require(
            _duration <= MAX_DURATION && _duration > 0,
            "Duration is in range of 1 to 366"
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (offer.offerType == OfferType.FIXED) {
            return
                ((offer.totalReturn - offer.principalAmount) *
...
contract CollateralWrapper is TermData {
    ...
    function _getNFTApproved(
        ...
        if (_collateralType == CollateralType.ERC721) {
            IERC721 collateralWrapper = IERC721(_collateralAddress);
            return collateralWrapper.isApprovedForAll(_owner, address(this));
        } else if (_collateralType == CollateralType.ERC721A) {
            IERC721A collateralWrapper = IERC721A(_collateralAddress);
            return collateralWrapper.isApprovedForAll(_owner, address(this));
        } else if (_collateralType == CollateralType.ERC1155) {
