...
contract NTStakingWithPools is Ownable {
  ...
  /// address of boxMint S1 Citizen contract
  address immutable public boxMint;
  ...
  */
  ...
  function calculateS1CitizenPoints(Info memory _info) internal view returns(uint256) {
    ...
    uint256 vaultId = IGenericGetter(S1Citizen).getVaultIdOfTokenId(_info.tokenId);
    // if S1 staker have vault he get boosts from it  
    if (vaultId != 0) {
      string memory _vaultCredit = IGenericGetter(vaultContract)
            .getCreditMultiplier(_info.tokenId);
      points = (vaultCredit[_vaultCredit] * boostConfigs[_info.poolId][_info.boostPeriod]) / boostPrecision;
    }
  ...
  function getCreditYield(uint256 _tokenId) internal view returns(string memory) {
    ...
    IGenericGetter dataContract = IGenericGetter(boxMint);
    output = dataContract.getCreditYield(_tokenId);
