 ...
 */
contract Bundler is IBundleBuilder, ERC9981155Extension, ERC998ERC20Extension {
    ...
    /**
     * @dev Stores the Hub, name and symbol
     *
        ...
        returns (bool)
    {
        return
            ...
            _interfaceId == type(IBundler).interfaceId ||
    ...
    function permittedAsset(address _asset) public view returns (bool) {
        IPermittedNFTs permittedNFTs = IPermittedNFTs(
     ...
     * @return true if permitted, false otherwise
     */
    function permittedErc20Asset(address _erc20Contract)
        public
        view
        returns (bool)
    {
        IPermittedERC20s permittedERC20s = IPermittedERC20s(
            hub.getContract(ContractKeys.PERMITTED_BUNDLE_ERC20S)
        );
    ...
    function buildBundle(
        ...
        for (uint256 i = 0; i < _bundleElements.erc721s.length; i++) {
            if (_bundleElements.erc721s[i].safeTransferable) {
                IERC721(_bundleElements.erc721s[i].tokenContract)
                    .safeTransferFrom(
                        _sender,
                        address(this),
                ...
                _getChild(
                    _sender,
                    bundleId,
                    _bundleElements.erc721s[i].tokenContract,
                    _bundleElements.erc721s[i].id
                );
            }
        }
        ...
        for (uint256 i = 0; i < _bundleElements.erc20s.length; i++) {
            _getERC20(
                _sender,
                bundleId,
                _bundleElements.erc20s[i].tokenContract,
                _bundleElements.erc20s[i].amount
            );
        }

        for (uint256 i = 0; i < _bundleElements.erc1155s.length; i++) {
            IERC1155(_bundleElements.erc1155s[i].tokenContract)
     ...
     */
    function decomposeBundle(uint256 _tokenId, address _receiver)
        external
        override
        nonReentrant
    {
        ...
        while (childContracts[_tokenId].length() > 0) {
            ...
            while (childTokens[_tokenId][childContract].length() > 0) {
                ...
                if (balance > 0) {
                    ...
                    IERC1155(childContract).safeTransferFrom(
                        address(this),
                        _receiver,
                        childId,
                        balance,
 ...
 */
abstract contract ERC9981155Extension is
    ERC998TopDown,
    ...
    IERC1155Receiver
{
    ...
    function safeBatchTransferChild(
        ...
        if (_to != address(this)) {
            IERC1155(_childContract).safeBatchTransferFrom(
                address(this),
                _to,
                _childTokenIds,
                _amounts,
    ...
    function onERC1155BatchReceived(
        ...
        require(
            _data.length == 32,
            "data must contain tokenId to transfer the child token to"
        );
 ...
 */
abstract contract ERC998ERC20Extension is
    ERC998TopDown,
    IERC998ERC20TopDown,
    ...
    ReentrancyGuard
{
    ...
    bytes32 internal constant ERC998_MAGIC_MASK =
        ...
        returns (bytes32 rootOwner)
    {
        ...
        if (rootOwnerAddress.isContract()) {
            try
                IERC998ERC721TopDown(rootOwnerAddress).rootOwnerOfChild(
                    address(this),
                    _childTokenId
                )
            returns (bytes32 returnedRootOwner) {
                // Case 2: Token owner is other external top-down composable
                if (
                    returnedRootOwner & ERC998_MAGIC_MASK == ERC998_MAGIC_VALUE
                ) {
    ...
    function _validateChildTransfer(
        ...
        require(
            tokenId != 0,
            "_transferChild _childContract _childTokenId not found"
        );
        require(
