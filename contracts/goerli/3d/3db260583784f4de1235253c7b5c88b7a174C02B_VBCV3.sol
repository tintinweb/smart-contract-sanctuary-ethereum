...
interface ITreasury {
    ...
    //./new
    function vbTax() external view returns (uint256);

    //./new
    function liquidityTax() external view returns (uint256);

    //./new
    function vbcTax() external view returns (uint256);
    ...
    PausableUpgradeable
{
    ...
    address public vbAddress;

    event TreasuryAddressUpdated(address newTreasury);
    event VBAddressUpdated(address newVBAddress);
    ...
    function setTreasuryAddress(address _treasury) external onlyOwner {
        require(_treasury != address(0), "No address");
        treasury = _treasury;
        whitelistedAddress[_treasury] = true;
        emit TreasuryAddressUpdated(_treasury);
    }

    function setVBAddress(address _vbAddress) external onlyOwner {
        require(_vbAddress != address(0), "No address");
        vbAddress = _vbAddress;
        whitelistedAddress[_vbAddress] = true;
        emit VBAddressUpdated(_vbAddress);
    }
    ...
    function _transfer(
        ...
        if (whitelistedAddress[sender] || whitelistedAddress[recipient]) {
            ...
            if (recipient != pair && recipient != address(this)) {
            ...
            if (
                (pair == recipient && sender != address(this)) ||
                (pair == sender && recipient != address(this))
            ) {
                uint256 totalTax = burnTax() +
                    vbcTax() +
                    rewardTax() +
                    vbTax() +
                    liquidityTax();
                uint256 feeAmount = amount.mul(totalTax).div(100); //./(amount * totalTax) / 10000;
                super._transfer(sender, address(this), feeAmount);

                _feesOnContract += feeAmount;

                if (sender != pair) {
                    uint256 swappedVB = _buyAndBurnToken(
                        vbAddress,
                        _feesOnContract.mul(vbTax()).div(totalTax)
                    );

                    uint256 swappedWeth = Liquidity.swap(
                        address(this),
                        Liquidity.WETH,
                        _feesOnContract.mul(liquidityTax()).div(totalTax).div(
                            2
                        ),
                        slippage(),
                        address(this)
                    ...
                    // uint256 burnAmount = (_feesOnContract * burnTax()) /
                    //     totalTax;
                    // super._transfer(
                    //     address(this),
                ...
                super._transfer(
                    ...
                    treasury,
                    feeAmount.sub(
                        (_feesOnContract.mul(liquidityTax()).div(totalTax)).add(
                            (_feesOnContract.mul(vbTax()).div(totalTax))
                        )
                    )
                );
    ...
    function slippage() public view returns (uint256) {
        return ITreasury(treasury).slippage();
    }

    function burnTax() public view returns (uint256) {
        return ITreasury(treasury).burnTax();
    }

    function vbTax() public view returns (uint256) {
        return ITreasury(treasury).vbTax();
    }

    function liquidityTax() public view returns (uint256) {
        return ITreasury(treasury).liquidityTax();
    }

    function rewardTax() public view returns (uint256) {
        return ITreasury(treasury).rewardTax();
    }

    function vbcTax() public view returns (uint256) {
        return ITreasury(treasury).vbcTax();
    }
}