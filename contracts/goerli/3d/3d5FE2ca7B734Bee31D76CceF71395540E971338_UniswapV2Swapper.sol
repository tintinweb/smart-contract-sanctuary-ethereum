...
contract UniswapV2Swapper is ISwapper, GasEstimator {
        ...
        returns (SwapQuote memory quote)
    {
        ...
        quote = getQuoteByPath(swapTask, adapter, path); // F:[PUV2-2,3,4]
    }
    ...
    function getBestConnectorSwap(SwapTask memory swapTask, address adapter)
        ...
        returns (SwapQuote memory quote)
    {
        uint256 len = swapTask.connectors.length; // F:[PUV2-5,6]

        address[] memory path = new address[](3); // F:[PUV2-5,6]
        path[0] = swapTask.tokenIn; // F:[PUV2-5,6]
        path[2] = swapTask.tokenOut; // F:[PUV2-5,6]
        for (uint256 i; i < len; ) {
            path[1] = swapTask.connectors[i]; // F:[PUV2-5,6]

            SwapQuote memory currentQuote = getQuoteByPath(
                swapTask,
                adapter,
                path
            ); // F:[PUV2-5,6]
    ...
    function getQuoteByPath(
        ...
        if (swapTask.isInputTask()) {
            try
                IUniswapV2Router02(adapter).getAmountsOut(swapTask.amount, path) // F:[PUV2-2,3,5]
            returns (uint256[] memory amountsOut) {
                resultAmount = amountsOut[amountsOut.length - 1]; // F:[PUV2-2,3,5]
            } catch {}
        } else {
            try
                IUniswapV2Router02(adapter).getAmountsIn(swapTask.amount, path)
        ...
        // F:[PUV2-2]
        if (resultAmount != 0) {
            quote.amount = resultAmount; // F:[PUV2-3,4,5,6]
            quote.found = true; // F:[PUV2-3,4,5,6]
            quote.multiCall = toMulticall(
                swapTask,
                adapter,
                path,
                resultAmount
            ...
            return
                MultiCall({
                    ...
                    callData: abi.encodeWithSelector(
                        ...
                        swapTask.rateRAYWithSlippage(amount),
                        path,
            ...
            return
                MultiCall({
                    ...
                    callData: abi.encodeWithSelector(
                        ...
                        swapTask.amount,
                        swapTask.amountWithSlippage(amount),
                        path,
