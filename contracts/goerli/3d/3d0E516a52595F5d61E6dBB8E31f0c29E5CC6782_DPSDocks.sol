...
contract DPSDocks is ERC721Holder, ERC1155Holder, Ownable {
     ...
     */
    EnumerableSet.AddressSet private _voyages;

    /**
     * @notice we can have multiple collections acting as pirates
     */
    EnumerableSet.AddressSet private _pirates;

    /**
     * @notice we can have multiple collections acting as flagships
     */
    EnumerableSet.AddressSet private _flagships;
    ...
    struct VoyageData {
        LockedVoyageV2[] lockedVoyages;
        DPSVoyageIV2 voyage;
        IERC721Metadata pirate;
        DPSFlagshipI flagship;
    }

    function lockVoyagesItems(VoyageData[] calldata voyagesParam) external {
        ...
        uint256 i;
        for (i; i < voyagesParam.length; ++i) {
            lockVoyageItems(
                voyagesParam[i].lockedVoyages,
                voyagesParam[i].voyage,
                voyagesParam[i].pirate,
                voyagesParam[i].flagship
            );
        }
    ...
    function lockVoyageItems(
        LockedVoyageV2[] calldata _lockedVoyages,
        DPSVoyageIV2 _voyage,
        IERC721Metadata _pirate,
        DPSFlagshipI _flagship
    ) private {
        if (
            !_voyages.contains(address(_voyage)) ||
            !_pirates.contains(address(_pirate)) ||
            !_flagships.contains(address(_flagship))
        ) revert Unauthorized();
     ...
     */
    function claimRewards(uint256[] calldata _voyageIds) external {
        if (nonReentrant == 2) revert Unauthorized();
        nonReentrant = 2;
        ...
        for (uint256 i; i < _voyageIds.length; ++i) {
            ...
            if (!gameEngine.checkIfViableClaimer(msg.sender, lockedVoyage, owner)) revert NotViableClaimer();
    ...
    function setContract(
        ...
        if (_target == 1) {
            if (_enabled) {
                _voyages.add(address(DPSVoyageIV2(_contract)));
            } else {
                _voyages.remove(address(DPSVoyageIV2(_contract)));
            }
        } else if (_target == 2) {
            random = DPSQRNGI(_contract);
        } else if (_target == 3) {
            if (_enabled) {
                _pirates.add(address(IERC721Metadata(_contract)));
            } else {
                _pirates.remove(address(IERC721Metadata(_contract)));
            }
        } else if (_target == 4) {
            if (_enabled) {
                _flagships.add(address(DPSFlagshipI(_contract)));
            } else {
                _flagships.remove(address(DPSFlagshipI(_contract)));
            }
        } else if (_target == 5) {
    ...
    function voyages() external view returns (address[] memory) {
        return _voyages.values();
    }

    function voyages(address voyageAddress) external view returns (bool) {
        return _pirates.contains(voyageAddress);
    }

    function pirates() external view returns (address[] memory) {
        return _pirates.values();
    }

    function pirates(address piratesAddress) external view returns (bool) {
        return _pirates.contains(piratesAddress);
    }

    function flagships() external view returns (address[] memory) {
        return _flagships.values();
    }

    function flagships(address flagshipAddress) external view returns (bool) {
        return _flagships.contains(flagshipAddress);
    }
}
