...
contract InfinityBitTokenV2 is IERC20, Ownable {
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        {
            uint256 autoswap_amount;

            if(tax_amount > getMaxAutoswap())
            {
                autoswap_amount = getMaxAutoswap();
            }
            else
            {
                autoswap_amount = tax_amount;
            }
    ...
    // Given an amount, calculate the taxes which would be collected.
    function _calcTaxes(uint256 amount) public view returns (uint256 tax_to_collect) {
        return _calcLpTaxAmount(amount) + _calcDevTaxAmount(amount) + _calcMarketingTaxAmount(amount);
    }
