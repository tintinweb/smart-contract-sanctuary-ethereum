...
contract PositionBurnerJob is IPositionBurnerJob, Keep3rJob {
  ...
  function work(IPoolManager _poolManager, IStrategy.Position calldata _position) external upkeep(msg.sender) notPaused {
    if (!POOL_MANAGER_FACTORY.isChild(_poolManager)) {
      revert PositionBurnerJob_InvalidPoolManager(_poolManager);
    }
  ...
  /// @inheritdoc IPositionBurnerJob
  function isWorkable(IPoolManager _poolManager, IStrategy.Position calldata _position)
    external
    returns (bool _workable)
  {
  ...
  /// @inheritdoc IPositionBurnerJob
  function isWorkable(IPoolManager _poolManager, IStrategy.Position calldata _position, address _keeper)
    external
    returns (bool _workable)
  {
    if (!_isValidKeeper(_keeper)) {
   ...
   */
  function _isWorkable(IPoolManager _poolManager, IStrategy.Position calldata _position)
    internal
    returns (bool _workable)
  {
    if (paused) {
