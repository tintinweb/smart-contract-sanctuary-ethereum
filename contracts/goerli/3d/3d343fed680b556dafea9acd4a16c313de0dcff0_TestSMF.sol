...
/*
    function getIdsExcludedChemToken(uint256 [] memory ids) public view returns(uint256 [] memory) {
       uint256 [] memory excludedIds= new uint256[](ids.length);
        uint256 index = 0;
        for(uint256 i = 0; i < ids.length; i++){
            if(excludedChemToken[ids[i]] == true){
                excludedIds[index] = ids[i];
                index++;
            }
        }
        return excludedIds;
