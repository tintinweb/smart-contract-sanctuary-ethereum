...
library Events {
    ...
    /// @param id for which Merkle root was set or updated
    /// @param newMerkleRoot used for validating claims against id
    event MerkleRootChanged(uint256 id, bytes32 newMerkleRoot);
    ...
    /// @param id for which image data was set or updated
    /// @param newImageData used for id
    event ImageDataChanged(uint256 id, string newImageData);

    /// @notice Emitted after name is changed
    /// @param id for which name was set or updated
    /// @param newName used for id
    event NameChanged(uint256 id, string newName);

    /// @notice Emitted after description is changed
    /// @param id for which description was set or updated
    /// @param newDescription used for id
    event DescriptionChanged(uint256 id, string newDescription);
}
...
contract DirtTokenHoldersNFT is Owned, ERC1155 {
    ...
    modifier tokenNotYetClaimed(uint256 id, address minter) {
    ...
    modifier hasToken(uint256 id, address burner) {
        if (balanceOf[burner][id] == 0) {
    ...
    /// @notice Thrown if address has already minted its token for id
    error NotAllowedToMintAgain();

    /// @notice Thrown if address is not part of Merkle tree for id
    error NotInMerkle();
    ...
    /// @notice Creates a new DirtTokenHoldersNFT contract
    ...
    /// @notice Mint a token
    /// @param id of token being minted
    /// @param proof of mint eligibility
    function mint(uint256 id, bytes32[] calldata proof)
        external
        tokenExists(id)
        tokenNotYetClaimed(id, msg.sender)
        mintingEnabled
    {
        bytes32 leaf = keccak256(abi.encodePacked(msg.sender));
        bool isValidLeaf = MerkleProofLib.verify(proof, merkleRoots[id], leaf);
    ...
    function burn(uint256 id) external hasToken(id, msg.sender) {
    ...
    /// @notice Set a new Merkle root for a given id
    /// @param id to get a new or updated Merkle root
    /// @param _merkleRoot to be used for validating claims
    function ownerSetMerkleRoot(uint256 id, bytes32 _merkleRoot)
        public
        onlyOwner
    {
        merkleRoots[id] = _merkleRoot;

        emit Events.MerkleRootChanged(id, _merkleRoot);
    }

    /// @notice Set new image data for a given id
    /// @param id to get new or updated image data
    /// @param _imageData to be used
    function ownerSetImageData(uint256 id, string memory _imageData)
        public
        onlyOwner
    {
        imageData[id] = _imageData;

        emit Events.ImageDataChanged(id, _imageData);
    }

    /// @notice Set new name for a given id
    /// @param id to get new or updated name
    /// @param _name to be used
    function ownerSetName(uint256 id, string memory _name) public onlyOwner {
        names[id] = _name;

        emit Events.NameChanged(id, _name);
    }

    /// @notice Set new description for a given id
    /// @param id to get new or updated description
    /// @param _description to be used
    function ownerSetDescription(uint256 id, string memory _description)
        public
        onlyOwner
    {
        descriptions[id] = _description;

        emit Events.DescriptionChanged(id, _description);
    }
    ...
    function ownerSetEnabled(bool _enabled) public onlyOwner {
        enabled = _enabled;
        emit Events.EnabledUpdated(_enabled);
    }
}
...
/// @title LilStrings
/// @notice Excerpted from Open Zeppelin's strings library
library LilStrings {
