...
contract RelicDrop is ERC1155Receiver, AllowList, Administration {
    //////////////  ERRORS  //////////////
    ...
    constructor(address relicsContract) Administration(_msgSender()) {
    ...
    function getTreasuryAddress() public view returns (address treasury) {
        treasury = _relicsTreasury;
    }
    ...
    function createDrop(
        ...
        (uint256 startIndex, uint256 endIndex) = IERC721(_relicsContract).registerRange(id, quantity, proof);
    ...
    function presaleMint(
        ...
        // Mint
        IERC721(_relicsContract).mintRange(id, _msgSender(), quantity);
    }
    ...
    function onERC721Received(
        ...
        IERC721(msg.sender).burn(tokenId);
        ...
        IERC721(_relicsContract).mintRange(ticket.saleId, from, 1);
    ...
    function onERC1155Received(
        ...
        Ticket memory ticket = _tickets[tokenAddress];
        Sale storage sale = _sales[ticket.saleId];
        // uint256 nextIndex = sale.nextIndex;
        ...
        // Burn the NFTs received
        IERC1155(msg.sender).burn(address(this), id, value);

        // Add mint quantity to drop next index
        unchecked {
            sale.minted += uint128(value);
            // sale.nextIndex = nextIndex + value;
        }

        // Mint the relics
        IERC721(_relicsContract).mintRange(ticket.saleId, from, value);
    ...
    function withdraw1155(address token, uint256 tokenId) external isAdmin {
        if (_relicsTreasury == address(0)) revert MustSetTreasury();
        ...
        erc1155.safeTransferFrom(
            ...
            _relicsTreasury,
            tokenId,
            erc1155.balanceOf(address(this), tokenId),
            ""
        );
    }

    function withdraw721(address token, uint256 tokenId) external isAdmin {
        if (_relicsTreasury == address(0)) revert MustSetTreasury();
        IERC721 erc721 = IERC721(token);
        erc721.transferFrom(address(this), _relicsTreasury, tokenId);
    }
