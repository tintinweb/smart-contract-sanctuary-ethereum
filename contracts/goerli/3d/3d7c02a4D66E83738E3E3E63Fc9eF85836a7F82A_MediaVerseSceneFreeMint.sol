...
/**
 * @title Media Verse Scene Free Mint contract
 * @dev Extends ERC721 Non-Fungible Token Standard basic implementation
 */
contract MediaVerseSceneFreeMint is ERC721Enumerable, Ownable {
    using ECDSA for bytes32;
    using Counters for Counters.Counter;

    address private root; // Root signer
    ...
    string public currentPhase = "Preparing";
    uint256 public currentPhaseMaxMintPerAddress;
    uint256 public currentPhaseMaxSupply;

    constructor() ERC721("Media Verse Scene Free Mint", "MEDSCEFREE") {}

    function setPhase(
        string calldata _newPhase,
    ...
    function setPhaseMaxSupply(uint256 _newPhaseMaxSupply) external onlyOwner {
        require(
            totalClaimedCountForCurrentPhase[currentPhase] < _newPhaseMaxSupply,
            "Supply not less than minted"
        );
        currentPhaseMaxSupply = _newPhaseMaxSupply;
    }
    ...
    function setRootSigner(address _root) external onlyOwner {
        root = _root;
    }
    ...
    modifier onlyBackendVerified(bytes memory _signature) {
        bytes32 msgHash = keccak256(
            abi.encodePacked(name(), msg.sender, currentPhase)
        );
        require(
    ...
    modifier mintCompliance(uint256 quantity) {
        ...
        require(
            addressClaimedCountForCurrentPhase[currentPhase][msg.sender] +
                quantity <=
                currentPhaseMaxMintPerAddress,
            "Over address limit"
        );
        require(
            totalClaimedCountForCurrentPhase[currentPhase] + quantity <=
    ...
    function mintForAddress(uint256 quantity, address receiver)
        ...
        onlyOwner
    {
        _mintForAddress(quantity, receiver);
    }

    function _mint(uint256 quantity) internal onlyEOA mintCompliance(quantity) {
        addressClaimedCountForCurrentPhase[currentPhase][
            msg.sender
        ] += quantity;
        totalClaimedCountForCurrentPhase[currentPhase] += quantity;
        _safeMintLoop(quantity, msg.sender);
    }

    function _mintForAddress(uint256 quantity, address receiver)
        internal
        onlyOwner
    {
        require(
        ...
        addressClaimedCountForCurrentPhase[currentPhase][receiver] += quantity;
        totalClaimedCountForCurrentPhase[currentPhase] += quantity;
        _safeMintLoop(quantity, receiver);
    }

    function _safeMintLoop(uint256 quantity, address to) internal {
