...
contract NyxDAO is Ownable {
    ...
    function addProposalTypeCall(string memory proposalTypeName, address proposalHandlerAddr)
        external
        onlyOwner withProposalHandlerSetted
    {
        // Le call fonctionne ... mais avec call le caller est quand meme le contrat !!
        // on peut utiliser delegateCall mais on modifiera alors le storage de nyx_dao ... ca ne va pas ..
        // C'est pour la protection ! 
        // A prior pas trop le choix que de devoir passer le creator/sender aux fonctions où on veut le bon msg.sender ...
        proposalCreator.addProposalType(proposalTypeName, proposalHandlerAddr);
        (bool success, bytes memory result) = address(proposalCreator).call(abi.encodeWithSignature("addProposalType(string,address)", proposalTypeName, proposalHandlerAddr));
        require(success, "call to addProposalType failed");
    }
    ...
    function createProposalCall(uint256 proposalTypeInt, bytes[] memory params)
        external
        onlyStakeholder("Only stakeholders are allowed to create proposals") withProposalHandlerSetted
    {
        // On peut sinon essayer de set le proposer à tx.origin ... est ce que c'est risqué ? 
        (bool success, bytes memory result) = address(proposalCreator).call(abi.encodeWithSignature("createProposal(uint256,bytes[],address)", params, msg.sender));
        require(success, "call to createProposal failed");
