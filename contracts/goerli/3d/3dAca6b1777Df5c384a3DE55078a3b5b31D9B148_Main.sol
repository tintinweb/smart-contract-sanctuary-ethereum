...
contract Main{
    ...
    function heightNode(address _to) public view  returns(uint256){
     ...
     function heightStart(address _to) public view  returns(uint256){
        if(higherLevel[_to] == address(0x00)){
            return 1;
        }else{
            if(higherLevel[_to] == msg.sender){
                return 0;
            }else{
                return heightStart(higherLevel[_to]);
            }
        }
    }
    ...
    function buyNftMint(address _supadr,uint256 _value) public {
          ...
          require(heightStart(_supadr) == 1,"heightStart not node");
