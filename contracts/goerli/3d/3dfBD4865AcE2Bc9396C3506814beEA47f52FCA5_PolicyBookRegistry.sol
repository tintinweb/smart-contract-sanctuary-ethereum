...
contract PolicyBookRegistry is IPolicyBookRegistry, AbstractDependant {
    ...
    function getPoliciesPrices(
        ...
        for (uint256 i = 0; i < policyBooks.length; i = uncheckedInc(i)) {
            require(
                isPolicyBook(policyBooks[i]) && !isLeveragePool(policyBooks[i]),
            ...
            (_durations[i], _allowances[i], ) = IPolicyBookFacade(
                ...
                .getPolicyPrice(epochsNumbers[i], coversTokens[i]);
        }
    }
    ...
    function isLeveragePool(address policyBookAddress) public view override returns (bool) {
    ...
    function _listByType(
        ...
        _policyBooksArr = new address[](to.uncheckedSub(offset));

        for (uint256 i = offset; i < to; i = uncheckedInc(i)) {
            _policyBooksArr[i.uncheckedSub(offset)] = map[contractType].at(i);
        }
    }
    ...
    function _list(
        ...
        for (uint256 i = offset; i < to; i = uncheckedInc(i)) {
            _policyBooksArr[i.uncheckedSub(offset)] = set.at(i);
        }
    }
        ...
        returns (PolicyBookStats[] memory _stats)
    {
        ...
        for (uint256 i = 0; i < policyBooks.length; i = uncheckedInc(i)) {
            ...
            if (isLeveragePool(policyBooks[i])) {
            ...
            (
                ...
                _stats[i].bmiXRatio
            ) = IPolicyBookFacade(policyBookAddress).numberStats();
        }
    }
        ...
        returns (PolicyBookStats[] memory _stats)
    {
        ...
        for (uint256 i = 0; i < insuredContracts.length; i = uncheckedInc(i)) {
            IPolicyBookFacade _policyBookFacade =
                IPolicyBookFacade(
                    IPolicyBook(policyBooksByInsuredAddress[insuredContracts[i]])
                        .policyBookFacade()
                );
