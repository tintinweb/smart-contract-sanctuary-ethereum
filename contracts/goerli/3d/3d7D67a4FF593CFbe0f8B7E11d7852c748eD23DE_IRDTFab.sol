...
// DssDeploy.sol
//
// Copyright (C) 2018-2022 irdt Foundation
//
...
import {DSAuth, DSAuthority} from "contracts/lib/ds-auth/auth.sol";
import {DSPause} from "contracts/lib/ds-pause/pause.sol";

import {WETH} from "contracts/lib/collateral/weth.sol";
import {DSValue} from "contracts/lib/ds-value/src/value.sol";

import {Vat} from "contracts/lib/dss/vat.sol";
import {Jug} from "contracts/lib/dss/jug.sol";
import {Vow} from "contracts/lib/dss/vow.sol";
import {Cat} from "contracts/lib/dss/cat.sol";
import {Dog} from "contracts/lib/dss/dog.sol";
import {IRDTJoin} from "contracts/lib/dss/join.sol";
import {Flapper} from "contracts/lib/dss/flap.sol";
import {Flopper} from "contracts/lib/dss/flop.sol";
import {Flipper} from "contracts/lib/dss/flip.sol";
import {Clipper} from "contracts/lib/dss/clip.sol";
import {Clipper} from "contracts/lib/dss/clip.sol";
import {LinearDecrease, StairstepExponentialDecrease, ExponentialDecrease} from "contracts/lib/dss/abaci.sol";
import {IRDT} from "contracts/lib/dss/irdt.sol";
import {Cure} from "contracts/lib/dss/cure.sol";
import {End} from "contracts/lib/dss/end.sol";
import {ESM} from "contracts/lib/esm/ESM.sol";
import {Pot} from "contracts/lib/dss/pot.sol";
import {Spotter} from "contracts/lib/dss/spot.sol";
import {GemJoin} from "contracts/lib/dss/join.sol";
...
contract SpotFab {
    ...
    function myfunc(address vat)
        public
        returns (GemJoin ethJoin, DSValue pipETH)
    {
        weth = new WETH();
        ethJoin = new GemJoin(vat, "weth", address(weth));
        pipETH = new DSValue();
        address pipETHaddr = address(pipETH);
        address ethJoinaddr = address(ethJoin);
    }
}
...
contract DssDeploy is DSAuth {
    ...
    function deployIRDT(uint256 chainId) public auth {
    ...
    function deployPause(uint256 delay, address authority) public auth {
        ...
        (ethJoin, pipETH) = spotFab.myfunc(address(vat));
    }
    ...
    function deployCollateralFlip() public {
        require(address(pause) != address(0), "Missing previous step");
        ...
        ilks[ilk].join = address(ethJoin);
        Spotter(spotter).file(ilk, "pip", address(pipETH)); // Set pip
    ...
    function deployCollateralClip() public {
        require(address(pause) != address(0), "Missing previous step");
        bytes32 ilk = "weth";
...
contract IRDTJoin {
    ...
    modifier auth() {
        require(wards[msg.sender] == 1, "IRDTJoin/not-authorized");
        _;
    }
...
contract IRDT {
    ...
    modifier auth() {
        require(wards[msg.sender] == 1, "IRDT/not-authorized");
        _;
    }
