...
//     function validators(uint256 tokenId_) external view returns (bytes calldata, bytes calldata, bytes32, uint64);
// }
...
interface ISenseiStake is IERC721 {
    ...
    function endOperatorServices(uint256 tokenId) external;
    function withdraw(uint256 tokenId_) external;
    function getServiceContractAddress(uint256 tokenId) external view returns (address);
}
...
interface ISenseistakeServicesContract {
    function exitDate() external view returns (uint64);
}
...
import {ISenseiStake} from "./interfaces/ISenseiStake.sol";
import {ISenseistakeServicesContract} from "./interfaces/ISenseistakeServicesContract.sol";
...
contract Vault is ERC4626, Initializable, IERC721Receiver {
    ...
    function afterDeposit(uint256, uint256) internal override {
        ...
        if (totalAssets() == vaultAmount) {
            ...
            stake.createContract{value: vaultAmount}();
            tokenId = stake.tokenIdCounter();
        }
    }
    ...
    function exitStake() external {
        ...
        stake.endOperatorServices(tokenId);
        stake.withdraw(tokenId);
    ...
    function exitDate() external view returns (uint256) {
        address _sscc = stake.getServiceContractAddress(tokenId);
        return ISenseistakeServicesContract(_sscc).exitDate();
    }
    ...
    function canExit() public view returns (bool) {
        address _sscc = stake.getServiceContractAddress(tokenId);
        return ISenseistakeServicesContract(_sscc).exitDate() < block.timestamp;
    }
}
