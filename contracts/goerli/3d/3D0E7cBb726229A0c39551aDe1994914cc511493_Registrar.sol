    ...
    ReentrancyGuardUpgradeable 
{
    ...
    function initialize(IPriceOracle _priceFeed) initializer public {
        __ERC721_init("Bi Decentralized Identity", "BIDID");
        __Ownable_init();
    ...
    function valid(string calldata name) 
        public 
        pure 
        returns (bool) 
    {
    ...
    function priceOf(string calldata name)
        external
        view
        override
        returns (uint256 etherPrice) 
    {
        return _price(name);
    }

    function ownerRegister(address to_, string calldata name) 
        external 
        onlyOwner 
        returns(uint256) 
    {
        require(_available(name),"Bi DID: Name is unavailable.");
        uint256 namePrice = _price(name);
    ...
    function register(
        ...
        require(_available(name),"Bi DID: Name is unavailable.");
        require(block.timestamp <= deadline, "Bi DID: Expired value.");
        require(status != Status.Pending, "Bi DID: It's not time to register yet.");
        require(_verify(_hash(name, msg.sender,deadline), s), "Bi DID: Signature verification failed.");
        uint256 namePrice = _price(name);
        require(
            msg.value >= namePrice,
            "Bi DID: Not enough ether provided."
        );
        ...
        if (msg.value > namePrice) {
            AddressUpgradeable.sendValue(payable(msg.sender), msg.value - namePrice);
            cost = msg.value - namePrice;
        }
    ...
    function withdraw() external onlyOwner nonReentrant {
        require(address(this).balance > 0, "Bi DID: Insufficient balance.");
    ...
    function _price(string calldata name) internal view returns (uint256) {
        return priceOracle.price(name);
    }

    function _available(string calldata name) internal view returns (bool) {
        uint256 tokenId = uint256(keccak256(bytes(name)));
        return (valid(name) && !_exists(tokenId));
    }
    ...
    function _hash(
        ...
        return keccak256(abi.encodePacked(name, registrantAddress, deadline));
    }
    ...
    function _verify(
        ...
        require(s.length == 65, "Bi DID: invalid signature length");
        bytes32 ethMsghash = ECDSA.toEthSignedMessageHash(messagehash);
        return (ECDSA.recover(ethMsghash,s) == _verifier);
    }
