 ...
 */
contract Bridge is Initializable, UUPSUpgradeable, PausableUpgradeable, AdminProxyManager {
     ...
     */
    function init(uint8 domainID, address accessControl) external initializer proxied {
     ...
     */
    function deposit(uint8 destinationDomainID, bytes32 resourceID, bytes memory depositData, bytes calldata feeData) external payable whenNotPaused {
    ...
    function setMPC(address MPCAddress) external {
        require(
            (_accessControl.hasAccess(msg.sig, _msgSender()) || _msgSender() == _MPCAddress) &&
            MPCAddress != address(0) &&
            _MPCAddress != address(0) &&
