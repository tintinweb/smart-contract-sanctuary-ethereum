  ...
  OperatorFilterer
{
  ...
  function ownerAtSlot() public view returns (address) {
    bytes32 OWNER_SLOT = bytes32(
      uint256(keccak256("registrar.proxy.owner")) - 1
    );
  ...
  function transferBeaconOwnership(
    ...
    if (msg.sender != ownerAtSlot()) {
      revert NotOwner();
    }
    OwnableUpgradeable(beaconProxy).transferOwnership(newOwner);
  }
...
import {IOperatorFilterRegistry, OperatorFilterRegistry} from "operator-filter-registry/src/OperatorFilterRegistry.sol";
