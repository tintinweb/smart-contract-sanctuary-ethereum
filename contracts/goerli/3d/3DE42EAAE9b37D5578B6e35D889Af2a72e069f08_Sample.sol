...
contract Sample is ERC1155Supply, Ownable {
  ...
  address payable private _withdrawalWallet = payable(0xF33CFb981Af9a324F8Ec01367BE904807c2B0cCc);
  ...
  function checkPreSaleClaimEligibility(address toAddress, uint256 amount, bool isFree)
    external
    view
  ...
  function _tokenId(address toAddress) private returns (uint256) {
    randNonce++;
    uint256 number = uint256(keccak256(abi.encodePacked(block.timestamp, toAddress, randNonce))) % 200;
    if (number == 0) {
      return 1;
