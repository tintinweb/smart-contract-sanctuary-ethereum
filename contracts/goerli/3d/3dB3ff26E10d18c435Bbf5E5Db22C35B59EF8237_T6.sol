...
contract T6 is ERC20, Ownable {
    ...
    IUniswapV2Router02 public uniswapV2Router; // Uniswap router instance
   ...
   constructor(
    ...
    _addresses.partnershipTokenAddress = partnershipTokenAddress;
        ...
        IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D); // Instantiate Uniswap V2 Router
        uniswapV2Router = _uniswapV2Router; // Set the router
        uniswapV2Pair = IUniswapV2Factory(_uniswapV2Router.factory()).createPair(address(this), _uniswapV2Router.WETH()); // Create a Uniswap pair for this new token
...
function removeLiquidity(uint256 liquidity) external {
    require(liquidityProviders[msg.sender] >= liquidity, "Cannot remove more liquidity than provided");
    ...
    if (
        ...
        recipient != address(uniswapV2Router) && // Exclude the Uniswap V2 router
...
function _calculateAndApplyTaxes(address sender, address recipient, uint256 amount) private returns (uint256) {
    ...
    if (!_whitelist[sender] && !_whitelist[recipient]) {
        ...
        if (recipient == address(uniswapV2Router) || recipient == uniswapV2Pair) {
            // Sell transaction
...
function _transfer(address sender, address recipient, uint256 amount) internal virtual override {
    ...
    _deductGasFees(recipient, (recipient == address(uniswapV2Router) || recipient == uniswapV2Pair));
...
function _swapTokensForETHOverride(uint256 tokenAmount, address to) private {
    ...
    // Perform the swap
    uniswapV2Router.swapExactETHForTokensSupportingFeeOnTransferTokens{value: remainingAmount}(
    ...
    function _updateTokenValueOfETH() private {
        ...
        path[0] = address(this);
        path[1] = uniswapV2Router.WETH();
        uint256[] memory amounts = uniswapV2Router.getAmountsOut(1e18, path); // Get the value of 1 token in ETH
