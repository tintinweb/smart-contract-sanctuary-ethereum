...
// File: contracts/utils/Signature.sol
    ...
    AccessControlEnumerableUpgradeable
{
    ...
    address public mysteryBiz;

    modifier onlyAdmin() {
        require(
            hasRole(DEFAULT_ADMIN_ROLE, _msgSender()),
            "MysteryBox: not admin role"
        );
        _;
    }
    ...
    modifier onlyMiner() {
        require(
            ...
            "MysteryBox: not minter role"
        );
        _;
    }

    function initialize(address _mysteryBiz) external initializer {
        __ERC721_init("Hellven Mystery Box", "HELLBOX");
        ...
        __baseURI = "https://meta.hellven.io/box/";
        mysteryBiz = _mysteryBiz;
    }

    function setMysteryBiz(address _addr) external onlyAdmin {
        mysteryBiz = _addr;
    }

    function setBaseURI(string memory uri) external onlyAdmin {
        require(bytes(uri).length > 0, "MysteryBox: uri invalid");
    ...
    function mint(address to) external virtual onlyMiner {
        _mint(to, ++currentId);
    }

    function mintBatch(address[] memory accounts) external virtual onlyMiner {
        uint256 length = accounts.length;

        require(length > 0, "MysteryBox: array invalid");
    ...
    function open(
        ...
        require(ownerOf(_tokenId) == _sender, "MysteryBox: owner invalid");

        _verifySignerSignature(
            keccak256(
                abi.encodePacked(_sender, _tokenId, _target, _rarity, _totem)
            ),
            _signature
        );

        _transfer(_sender, address(this), _tokenId);

        require(
            MysteryBiz(mysteryBiz).openBox(_tokenId, _target, _rarity, _totem)
        );
    }
    ...
    function _verifySignerSignature(
        ...
        require(hasRole(SIGNER_ROLE, singer), "MysteryBox: signature invalid");
    }
