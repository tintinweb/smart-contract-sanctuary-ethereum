...
contract MarketplaceV7 is Initializable,ReentrancyGuardUpgradeable {
    ...
    struct OffersForListed{
        uint256 newOfferedValue;
        address maker;
        uint256 listingId;
        OfferStatus status;
        uint256 listingOfferId;
        address taker;
    ...
    event ListingOffer(uint256 listingId, address buyer, uint256 amount, uint256 listingOffersLastIndex);

    event ListingOfferCompleted(uint256 listingId, address buyer,uint256 listingOffersLastIndex);
    ...
    event StakingOfferAccepted(uint256 stakingId, address taker, uint256 stakingOfferId);
    ...
    event CancelListingOffer(uint256 listingId,uint256 listingOffersLastIndex);

    event CancelStakingOffer(uint256 stakingId,uint256 stakingOfferId);
    ...
    mapping(uint256 => OffersForListed) _listingOffers;
    ...
    mapping(uint256 => OfferForNotListedToken) _offersForNotListedTokens;
    uint256 public stakingOffersLastIndex;
    uint256 public listingOffersLastIndex;
    ...
    function acceptListingOffer(uint256 offerListingId, uint256 listingId) external {
        Listing memory listing = _listings[listingId];
        OffersForListed memory offer = _listingOffers[offerListingId];
        ...
        buyTokenInternal(listingId, offer.newOfferedValue, offer.newOfferedValue, offer.maker);

        emit ListingOfferCompleted(listingId, offer.maker, offer.listingOfferId);
    }
    ...
    function denyListingOffer(uint256 offerListingId,uint256 listingId) external nonReentrant {
        Listing storage listing = _listings[listingId];
        require(msg.sender == listing.seller, "Only seller can cancel listing");

        OffersForListed memory offer = _listingOffers[offerListingId];
        
        payable(offer.maker).transfer(offer.newOfferedValue);
        offer.newOfferedValue = 0;
        emit CancelListingOffer(listingId,offer.listingOfferId);
    }

    function cancelListingOffer(uint256 offerListingId,uint256 listingId) external nonReentrant {
        OffersForListed memory offer = _listingOffers[offerListingId];
        
        payable(msg.sender).transfer(offer.newOfferedValue);
        offer.newOfferedValue = 0;
        emit CancelListingOffer(listingId,offer.listingOfferId);
    }

    function cancelStakingOffer(uint256 stakingId) external nonReentrant {
        StakingOffer memory offer = _stakingOffers[stakingId][msg.sender];
        payable(msg.sender).transfer(offer.premium + offer.collateral);
        offer.collateral = 0;
        offer.premium = 0;

        emit CancelStakingOffer(stakingId,offer.stakingOfferId);
    }
    ...
    function stakingOffer(
        ...
        offer.stakingOfferId = stakingOffersLastIndex;
        require(
        ...
        emit StakingOffered(
            ...
            offer.collateral,
            offer.premium,
            offer.stakingOfferId 
        );
    }

    function isEnoughValueWasSend(uint messageValue, uint256 collateral, uint256 premium , uint256 _premiumFeePercentage)internal pure returns(bool){ 
    ...
    function acceptStakingOffer(
        ...
        emit StakingOfferAccepted(stakingId, taker, offer.stakingOfferId);
    }
    ...
    function getContractBalanceV2()public view returns(uint){
