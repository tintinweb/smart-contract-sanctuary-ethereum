...
contract ADD is ERC721Enum, Ownable, ReentrancyGuard{
    using Strings for uint256;
	using SafeERC20 for IERC20;
	
    uint256 public SALE_NFT_TIER_ONE   = 500;
	uint256 public SALE_NFT_TIER_TWO   = 500;
	uint256 public SALE_NFT_TIER_THREE = 500;
	...
	uint256 public TOKEN_REQUIRED_FOR_MINT = 10000000 * (10**18);
	
	IERC20 public ADDToken = IERC20(0xf86640900dD2C849a4AFb8E9baEDB09A9Ac3Bf0b);
	...
	mapping(address => mapping(uint256 => loanRequest)) public requestingLoans;
	mapping(uint256 => StakingInfo) public stakingInfo;
	
    constructor() ERC721P('Anarchist Development DAO', 'ADD') {}
	...
	function deposit(uint256[] calldata tokenIds) external{
        for (uint256 i = 0; i < tokenIds.length; i++) {
            ERC721P.safeTransferFrom(msg.sender, address(this), tokenIds[i], "");
			...
			stakingInfo[tokenIds[i]].proposalApproved = false;
			stakingInfo[tokenIds[i]].proposalRejected = false;
	...
	function withdraw(uint256[] calldata tokenIds) external {
        for (uint256 i; i < tokenIds.length; i++) {
		   require(!stakingInfo[tokenIds[i]].proposalSubmitted, "Proposal submitted for this tokenID");
		   require(!stakingInfo[tokenIds[i]].hasLoan, "NFT is available for loan");
           ERC721P.safeTransferFrom(address(this), msg.sender, tokenIds[i], "");
		   stakingInfo[tokenIds[i]].inStaking = false;
        }
    }
	
	function proposalApproved(uint256 tokenId) external onlyOwner {
	   require(stakingInfo[tokenId].proposalSubmitted, "Proposal is not submitted for this tokenID");
	   require(!stakingInfo[tokenId].proposalApproved && !stakingInfo[tokenId].proposalRejected, "Proposal already approved or rejected by owner");
	   
	   if(stakingInfo[tokenId].staker == stakingInfo[tokenId].owner) 
	   {
	        _burn(tokenId);
	   } 
	   else 
	   {
	       ERC721P.safeTransferFrom(address(this), stakingInfo[tokenId].owner, tokenId, "");
	   }
    }
	
	function proposalRejected(uint256 tokenId) external onlyOwner{
	   require(stakingInfo[tokenId].proposalSubmitted, "Proposal submitted for this tokenID");
	   require(!stakingInfo[tokenId].proposalApproved && !stakingInfo[tokenId].proposalRejected, "Proposal already approved or rejected by owner");
	   
	   ERC721P.safeTransferFrom(address(this), stakingInfo[tokenId].owner, tokenId, "");
    }
	
	function proposalSubmited(uint256 tokenId) external{
	   require(!stakingInfo[tokenId].proposalSubmitted, "Proposal already submitted for this tokenID");
	   require(stakingInfo[tokenId].staker == msg.sender, "Owner is not correct");
	   require(stakingInfo[tokenId].inStaking, "Token is not staked");
	   
	   stakingInfo[tokenId].proposalSubmitted = true;
    }
	
	function getLoanOnStake(uint256 tokenID , uint256 returnPerDay, uint256 loanDuration) external {
	   require(stakingInfo[tokenID].inStaking == true, 'Incorrect token id');
	   require(stakingInfo[tokenID].hasLoan == false, 'Already loan request submitted');
	   require(stakingInfo[tokenID].owner == msg.sender, 'Owner is not correct');
	   require(loanDuration >= 86400, 'Minimum 1 day required for loan duration');
	   
	   stakingInfo[tokenID].hasLoan == true;
       requestingLoans[msg.sender][tokenID].loanerAddress = msg.sender;
	   requestingLoans[msg.sender][tokenID].tokenID = tokenID;
	   requestingLoans[msg.sender][tokenID].returnPerDay = returnPerDay;
	   requestingLoans[msg.sender][tokenID].duration = (loanDuration / 86400) * 86400;
	}
	
	function cancelLoanRequest(uint256 tokenID) public {
       require(stakingInfo[tokenID].owner == msg.sender, 'Owner is not correct');
	   require(!requestingLoans[msg.sender][tokenID].loanIsPaid, 'Loan is already paid for this nft');
	   
	   requestingLoans[msg.sender][tokenID].tokenID = 0;
	   stakingInfo[tokenID].hasLoan == false;
    }
	
	function lendOnStake(address loanerAddress , uint256 tokenID) external payable nonReentrant {
	   require(!requestingLoans[loanerAddress][tokenID].loanIsPaid, 'Loan is already paid for this nft');
	   require(msg.value >= requestingLoans[msg.sender][tokenID].returnPerDay * (requestingLoans[msg.sender][tokenID].duration / 86400), 'Return amount is not correct');
	   
	   requestingLoans[msg.sender][tokenID].lenderAddress = msg.sender;
	   requestingLoans[msg.sender][tokenID].loanIsPaid = true;
	   requestingLoans[msg.sender][tokenID].startDay = block.timestamp;
	   requestingLoans[msg.sender][tokenID].endDay = block.timestamp + requestingLoans[msg.sender][tokenID].duration;
	   requestingLoans[msg.sender][tokenID].returnAmount = msg.value;
	}
	
	function withdrawLoanedFunds(uint256 tokenID) external nonReentrant {
       require(requestingLoans[msg.sender][tokenID].loanIsPaid, 'No funds to withdraw');
	   require(requestingLoans[msg.sender][tokenID].endDay <= block.timestamp, 'No funds to withdraw');
	   
	   payable(msg.sender).transfer(requestingLoans[msg.sender][tokenID].returnAmount);
