...
contract RareEvent is ERC721URIStorage, OwnableExtended {
	...
	constructor(string memory _tokenURI, string memory _contractURI) ERC721("RareEvent", "ROE"){
		_setBaseTokenURI(_tokenURI);
	...
	function createEvent(address owner, string memory filmId) external onlyOperator returns (address toOwner, string memory filmID, uint eventId) {
		tokenIds.increment();
		...
		eventIdToFilmId[tokenId] = filmId;

		emit Create(owner, filmId, tokenId);
		return (owner, filmId, tokenId);
	}
