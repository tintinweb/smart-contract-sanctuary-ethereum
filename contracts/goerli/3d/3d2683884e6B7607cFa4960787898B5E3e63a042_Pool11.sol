...
contract Pool11 is Initializable, ERC20UpgradeableFromERC777, IERC721ReceiverUpgradeable {
    ...
    */
    function stake(uint256 amount) public returns (bool) {
        ...
        bool successfulStake = PoolStakingRewards1(poolStakingRewardAddress).stake(msg.sender, amount);
    ...
    */
    ...
    function airdropBHome(address[] memory recepients, uint256[] memory amounts, uint256 length) public {
        require(msg.sender == servicer, "unapproved sender");
        require(!airdropLocked, "Airdrop locked");

        for (uint256 i = 0; i < length; i++) {    
            super._mint(recepients[i], amounts[i]);
        }
    }
