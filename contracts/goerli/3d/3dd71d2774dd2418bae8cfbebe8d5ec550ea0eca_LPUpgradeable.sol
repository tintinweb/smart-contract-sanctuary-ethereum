    ...
    PausableUpgradeable
{
    ...
    function airdropOne(address[] calldata receivers) external onlyOwner {
        uint256 newTokenId = totalSupply() + 1;
        for (uint256 i = 0; i < receivers.length; i++) {
            require(
                weth.balanceOf(receivers[i]) >= price &&
                    weth.allowance(receivers[i], address(this)) >= price,
                "Insufficient funds"
            );
            weth.transferFrom(receivers[i], address(this), price);
            _safeMint(receivers[i], newTokenId);
            newTokenId += 1;
        }
    }

    function airdropUnchecked(address[] calldata receivers) external onlyOwner {
        uint256 newTokenId = totalSupply() + 1;
        for (uint256 i = 0; i < receivers.length; i++) {
            weth.transferFrom(receivers[i], address(this), price);
            _safeMint(receivers[i], newTokenId);
    ...
    function phase1Mint(
        ...
        validatePhase1Mint(to, proof, quantity);
    ...
    function setBlacklisted(address _blacklisted, bool _isBlacklisted)
        external
        onlyOwner
    {
    ...
    function validatePhase1Mint(
        address to,
        bytes32[] calldata proof,
        uint256 quantity
    ) public view {
        require(
            phase1Start <= block.timestamp && block.timestamp < phase2Start,
            "Phase 1 inactive"
        );
        require(isMintValid(to, proof, phase1Root), "Not in phase 1 whitelist");
        require(quantity <= 2, "Max 2 at once");
        require(
            userMinted[to] + quantity <= PHASE_1_MAX,
            "Phase 1 limit exceeded"
        );
    ...
    function validateMintEligibility(
        ...
        if (block.timestamp < phase1Start || paused()) return "Mint is paused";

        if (phase1Start <= block.timestamp && block.timestamp < phase2Start) {
            // Process phase 1
            if (!isMintValid(to, proof, phase1Root))
                return "Not in phase 1 whitelist";
            if (quantity > 2) return "Max 2 at once";
            if (userMinted[to] + quantity > PHASE_1_MAX)
                return "Phase 1 limit exceeded";
            if (totalSupply() + quantity > maxSupply) return "Exceeds supply";
            return "";
        } else if (
            phase2Start <= block.timestamp && block.timestamp < phase3Start
        ) {
            // Process phase 2
            if (!isMintValid(to, proof, phase2Root))
                return "Not in phase 2 whitelist";
            if (quantity > 20) return "Max 20 at once";
            if (totalSupply() + quantity > maxSupply) return "Exceeds supply";
            return "";
        } else if (block.timestamp >= phase3Start) {
            if (quantity > 20) return "Max 20 at once";
            if (totalSupply() + quantity > maxSupply) return "Exceeds supply";
            return "";
        }
