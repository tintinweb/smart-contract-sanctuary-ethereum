...
contract ReactorV2 is Initializable, OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    // // premium stake minimum:
    // uint256 public premiumStake; // if userStake >= premiumStake, then no monthly fee

    // stake lock (min days):
    uint256 public stakeLock;

    // unstake before due date, keep x/10000 amount:
    uint256 public stakeKeep;

    // monthly subscriptions fee token
    IERC20Upgradeable public subsToken;

    // subscriptions fee structure:
    struct Fee {
        uint256 stake; // stake amount
        uint256 subsFee; // monthly subscriptions fee amount
        uint256 actionFee; // action fee amount
    }
    // monthly subscriptions fee count
    uint public feeCount;

    // monthly subscriptions fee count
    mapping (uint => Fee) public fee;

    // last userInfo stake/unstake:
    struct UserDetail {
        uint256 lastStake;
    ...
    // action event:
    event Action(address indexed user, uint indexed projectId, uint indexed actionType, uint256 amount, uint256 actionCount, uint256 actionFee);

    // ---
    event Subscribe(address indexed user, uint256 indexed subsMonth, uint256 indexed amount, uint256 exp);
    //\---
    ////////////////////////////////////////////////////////////////////////////////

    function doAction(uint _projectId, uint _action, address _user, uint256 _actionCount, uint256 _actionFee, uint256 _amount) private {
        nativePayout = nativePayout.add(_amount);
        emit Action(_user, _projectId, _action, _amount, _actionCount, _actionFee);
    }
    ...
    function stake(uint256 _amount) external nonReentrant {
        ...
        uint256 rAmount = userStake[msg.sender].add(_amount);
        userStake[msg.sender] = rAmount;
        userDetail[msg.sender].lastStake = block.timestamp;
        userDetail[msg.sender].lastStakeAmount = rAmount;
    ...
    function unstake(uint256 _amount) external nonReentrant {
        require(userStake[msg.sender] > _amount);
        uint256 _lastStake = userDetail[msg.sender].lastStake;
        uint256 _prevAmount = userDetail[msg.sender].lastStakeAmount;
        if (userDetail[msg.sender].lastStake == 0) {
            userDetail[msg.sender].lastStake = block.timestamp;
            userDetail[msg.sender].lastStakeAmount = userStake[msg.sender];
            _lastStake = block.timestamp;
            _prevAmount = userStake[msg.sender];
        }
        uint256 _keep = 0;
        uint256 rAmount = userStake[msg.sender].sub(_amount);
        if (block.timestamp < _lastStake.add(stakeLock.mul(1 days))) {
            _keep = userStake[msg.sender].mul(stakeKeep).div(10000);
            require(rAmount <= _keep);
        }
    ...
    function action(uint _projectId, uint _action, uint256 _actionCount) payable external {
        require(projectMap[_projectId].maintainers[msg.sender], "Only maintainers can perform actions");
        (, , uint256 actionFee) = getFeeDetail(msg.sender);
        uint256 _amount = actionFee.mul(_actionCount);
        if (actionFee > 0) {
            require(msg.value >= _amount, "value is not enaugh");
        }
        doAction(_projectId, _action, msg.sender, _actionCount, actionFee, _amount);
    }
    ...
    // function setPremiumStake(uint256 _amount) external onlyOwner {
    //     premiumStake = _amount;
    // }

    function setStakeLock(uint256 _days) external onlyOwner {
        stakeLock = _days;
    }

    function setStakeKeep(uint256 _keep) external onlyOwner {
        stakeKeep = _keep;
    }

    function changeSubsToken(IERC20Upgradeable _newSubsToken) external onlyOwner {
        subsToken = _newSubsToken;
    }

    function setFeeCount(uint _feeCount) external onlyOwner {
        feeCount = _feeCount;
    }

    function setFeeDetail(uint _index, uint256 _stake, uint256 _subsFee, uint256 _actionFee) external onlyOwner {
        fee[_index].stake = _stake;
        fee[_index].subsFee = _subsFee;
        fee[_index].actionFee = _actionFee;
    }

    function setPremiumFee(uint256 _stake, uint256 _subsFee, uint256 _actionFee) external onlyOwner {
        fee[feeCount].stake = _stake;
        fee[feeCount].subsFee = _subsFee;
        fee[feeCount].actionFee = _actionFee;
    }

    function getFee(address _user) public view returns(uint256 _stake, uint256 subsFee, uint256 actionFee) {
        // require(userStake[_user] >= minimumStake, "stake is not enough");
        uint _idx = feeCount - 1;
        if (userStake[_user] >= fee[feeCount].stake) {
            _stake = fee[feeCount].stake;
            subsFee = fee[feeCount].subsFee;
            actionFee = fee[feeCount].actionFee;
        } else {
            while ((userStake[_user] < fee[_idx].stake) && (_idx >= 0)) {
                _stake = fee[_idx].stake;
                subsFee = fee[_idx].subsFee;
                actionFee = fee[_idx].actionFee;
                _idx--;
            }
        }
    }
    ...
    function subscribe(uint256 _amount) external nonReentrant {
        require(userStake[msg.sender] >= minimumStake, "stake is not enough");
        // if (fee[feeCount].subsFee == 0) {
        //     require(userStake[msg.sender] < fee[feeCount].stake, "no subscriptions needed");
        // }
        // Fee memory _fee = getFee(msg.sender);
        (, uint256 subsFee, ) = getFeeDetail(msg.sender);
        require(subsFee > 0, "no subscriptions needed");
        // if (_fee.subsFee > 0) {
            uint256 _subMonth = _amount.div(subsFee);
            require(_subMonth > 0, "amount is not enaugh");
            uint256 _usedAmount = subsFee.mul(_subMonth);
            subsToken.safeTransferFrom(msg.sender, address(this), _usedAmount);
            uint256 _prevDate = userDetail[msg.sender].expired;
            if (_prevDate < block.timestamp) {
                _prevDate = block.timestamp;
            }
            uint256 _exp = _subMonth.mul(1 days).add(_prevDate);
            userDetail[msg.sender].expired = _exp;
            emit Subscribe(msg.sender, _subMonth, _usedAmount, _exp);
        // }
    }
    
    function getFeeDetail(address _user) public view returns(uint256 _stake, uint256 subsFee, uint256 actionFee) {
        // require(userStake[_user] >= minimumStake, "stake is not enough");
        uint _idx = feeCount - 1;
        if (userStake[_user] >= fee[feeCount].stake) {
            _stake = fee[feeCount].stake;
