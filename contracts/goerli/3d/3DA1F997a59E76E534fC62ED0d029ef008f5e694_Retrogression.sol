...
contract Retrogression is ERC721A, ERC2981, Ownable {
  ...
  constructor() ERC721A('Retrogression', 'RTGN') {
    mintRate = 100000000000000000; // 0.1eth tbc
    whitelistMintRate = 100000000000000000; // 0.1eth tbc
    whiteListMaxMint = 2;
    totalMaxSupply = 5050;
    ogMaxSupply = 400;
    ...
    _setDefaultRoyalty(0xb8623497431893Fc4820eC708003f27DE086FEF1, 2500);
  }
  ...
  modifier isWithinMintLimit(uint256 _quantity, uint256 _limit) {
    if (_quantity > _limit) revert ExceededLimit();
    _;
  }
  ...
  function sendFee() public payable {
    uint256 fee = msg.value.mul(10).div(10000); // deduct 10% from minting and send to mintable account
    sendValue(feeCollecter, fee);
  }
  ...
  function ogMint(uint256 _quantity)
    ...
    isWithinMintLimit(_quantity, ogMaxSupply)
    isEnoughTokensLeft(_quantity)
  {
    if (totalOgSupply.add(_quantity) > ogMaxSupply) revert ExceededLimit();
  ...
  function whiteListMint(uint256 _quantity, bytes32[] calldata _proof)
    ...
    isWithinMintLimit(_quantity, whiteListMaxMint)
    isEnoughTokensLeft(_quantity)
    isCorrectPayment(_quantity, whitelistMintRate)
  {
    if (totalWhiteListSupply.add(_quantity) > whiteListMaxSupply)
