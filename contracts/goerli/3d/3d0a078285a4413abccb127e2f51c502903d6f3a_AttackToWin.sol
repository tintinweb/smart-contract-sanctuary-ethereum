...
contract FactoryV1 is Ownable {
    ...
    function createContract(bytes calldata input, address newOwner) external onlyOwner returns (address attackToWin) {
        ...
        (bool state1, bytes memory returndata1) = attackToWin.call(abi.encodeWithSignature("changeOwner(address)", newOwner));
        require(state1, "Factory change owner failed");
    }
}