...
contract UniswapV3Swapper is ISwapper, GasEstimator {
    using SwapTaskOps for SwapTask;
    using SwapQuoteOps for SwapQuote;

    mapping(address => address) public routerToQuoter;

    error UnknownFeeIndex();
    error QuoterNotFound(address);

    /// @notice Pathfinder Component ID
    PathFinderComponent public override getComponentId =
        PathFinderComponent.UNISWAP_V3_SWAPPER;

    // Contract version
    uint256 public constant override version = 1;

    constructor() {
        routerToQuoter[UNIV3_ROUTER] = UNIV3_QUOTER;
    }

    function addQuoter(address router, address quoter) external {
        routerToQuoter[router] = quoter;
    }
    ...
    {
        (quote, , ) = _getBestDirectSwapWithAmountAndFee(swapTask, adapter);
    }

    function _getBestDirectSwapWithAmountAndFee(
        ...
        returns (
            ...
            uint256 bestAmount,
            uint256 bestFeeIndex
        )
    {
        quote.amount = swapTask.swapOperation == SwapOperation.EXACT_OUTPUT
            ? type(uint256).max
            : 0;
        address quoter = routerToQuoter[IAdapter(adapter).targetContract()];

        for (uint256 feeIndex; feeIndex < FEE_LENGTH; ) {
            SwapQuote memory currentQuote;
            if (swapTask.isInputTask()) {
                try
                    IQuoter(quoter).quoteExactInputSingle{ gas: 800_000 }(
                        swapTask.tokenIn,
                        swapTask.tokenOut,
                        fees(feeIndex),
                        swapTask.amount,
                ...
                try
                    IQuoter(quoter).quoteExactOutputSingle{ gas: 800_000 }(
                        swapTask.tokenIn,
                        swapTask.tokenOut,
                        fees(feeIndex),
                        swapTask.amount,
            ...
            if (currentQuote.isBetter(swapTask, quote, 0)) {
                quote = currentQuote;
                bestFeeIndex = feeIndex;
                bestAmount = quote.amount;
            }
    ...
    function getSingleQuote(
        ...
        quote.multiCall = exactSingleMulticall(
            swapTask,
            amount,
            adapter,
            feeIndex
        );
        quote.gasUsage = gasUsageByAdapter(
            adapter,
            swapTask.tokenIn,
            swapTask.tokenOut
        );
    ...
    function exactSingleMulticall(
        ...
        if (swapTask.swapOperation == SwapOperation.EXACT_INPUT_ALL) {
            IUniswapV3Adapter.ExactAllInputSingleParams
                memory params = IUniswapV3Adapter.ExactAllInputSingleParams({
                    tokenIn: swapTask.tokenIn,
                    tokenOut: swapTask.tokenOut,
                    fee: fees(feeIndex),
                    deadline: block.timestamp + 3600,
                    rateMinRAY: swapTask.rateRAYWithSlippage(amountOut),
                    sqrtPriceLimitX96: 0
                });
            return
                MultiCall({
        ...
        if (swapTask.swapOperation == SwapOperation.EXACT_INPUT) {
            ISwapRouter.ExactInputSingleParams memory params = ISwapRouter
                .ExactInputSingleParams({
                    ...
                    tokenOut: swapTask.tokenOut,
                    fee: fees(feeIndex),
                    recipient: swapTask.creditAccount,
                    deadline: block.timestamp + 3600,
                    amountIn: swapTask.amount,
                    amountOutMinimum: swapTask.amountWithSlippage(amountOut),
                    sqrtPriceLimitX96: 0
                });
            return
                MultiCall({
        ...
        if (swapTask.swapOperation == SwapOperation.EXACT_OUTPUT) {
            ISwapRouter.ExactOutputSingleParams memory params = ISwapRouter
                .ExactOutputSingleParams({
                    ...
                    recipient: swapTask.creditAccount,
                    deadline: block.timestamp + 3600,
                    amountOut: swapTask.amount,
                    amountInMaximum: swapTask.amountWithSlippage(amountOut),
                    sqrtPriceLimitX96: 0
                });
            return
                MultiCall({
    ...
    function getBestConnectorSwap(SwapTask memory swapTask, address adapter)
        ...
        returns (SwapQuote memory quote)
    {
        uint256 len = swapTask.connectors.length;
        quote.amount = (swapTask.swapOperation == SwapOperation.EXACT_INPUT ||
        ...
        for (uint256 i; i < len; ) {
            ...
            {
                SwapTask memory connectorSwapTask0;

                if (swapTask.isInputTask()) {
                    connectorSwapTask0 = swapTask.makeConnectorInTask(i);
                } else {
                    connectorSwapTask0 = swapTask.makeConnectorOutTask(
                        swapTask.amount,
                        i,
                        false
                    );
                ...
                (
                    ...
                    connectorAmount,
                    feeIndex0
                ) = _getBestDirectSwapWithAmountAndFee(
                    connectorSwapTask0,
                    adapter
                );

                if (
                    swapTask.isInputTask()
                        ? connectorAmount > 0
                        : connectorAmount < type(uint256).max
                ) {
                    SwapTask memory connectorSwapTask1;

                    if (swapTask.isInputTask()) {
                        connectorSwapTask1 = swapTask.makeConnectorOutTask(
                            connectorAmount,
                            i,
                            false
                        );
                    } else {
                        connectorSwapTask1 = swapTask.makeConnectorInTask(
                            connectorAmount,
                            i
                        );
                    }
            ...
            bool betterQuoteFound;

            if (swapTask.isInputTask()) {
                if (currentAmount != 0 && currentAmount > quote.amount) {
                    betterQuoteFound = true;
                }
            } else {
                if (currentAmount != 0 && currentAmount < quote.amount) {
                    betterQuoteFound = true;
            ...
            if (betterQuoteFound) {
                quote.found = true;
                quote.amount = currentAmount;

                bytes memory path = generatePathV3(
                    swapTask.swapOperation,
                    swapTask.tokenIn,
                    swapTask.isInputTask() ? feeIndex0 : feeIndex1,
                    swapTask.connectors[i],
                    swapTask.isInputTask() ? feeIndex1 : feeIndex0,
                    swapTask.tokenOut
                );

                quote.multiCall = exactConnectorMulticall(
                    swapTask,
                    currentAmount,
                    adapter,
                    path
    ...
    function exactConnectorMulticall(
        SwapTask memory swapTask,
        ...
        if (swapTask.swapOperation == SwapOperation.EXACT_INPUT_ALL) {
            IUniswapV3Adapter.ExactAllInputParams
                memory params = IUniswapV3Adapter.ExactAllInputParams({
                    path: path,
                    deadline: block.timestamp + 3600,
                    rateMinRAY: swapTask.rateRAYWithSlippage(resultAmount)
                });
        ...
        if (swapTask.swapOperation == SwapOperation.EXACT_INPUT) {
            ISwapRouter.ExactInputParams memory params = ISwapRouter
                .ExactInputParams({
                    ...
                    deadline: block.timestamp + 3600,
                    amountIn: swapTask.amount,
                    amountOutMinimum: swapTask.amountWithSlippage(resultAmount)
                });
            return
                MultiCall({
                    target: adapter,
        ...
        if (swapTask.swapOperation == SwapOperation.EXACT_OUTPUT) {
            ISwapRouter.ExactOutputParams memory params = ISwapRouter
                .ExactOutputParams({
                    ...
                    deadline: block.timestamp + 3600,
                    amountOut: swapTask.amount,
                    amountInMaximum: swapTask.amountWithSlippage(resultAmount)
                });
            return
                MultiCall({
                    target: adapter,
    ...
    function fees(uint256 index) public pure returns (uint24) {
        if (index >= FEE_LENGTH) revert UnknownFeeIndex();
        //return index == 0 ? 1000 : index == 1 ? 3000 : 5000;
        return index == 0 ? 100 : index == 1 ? 500 : index == 2 ? 3000 : 10000;
        //return index == 0 ? 500 : index == 1 ? 3000 : 10000;
        // return 100;
    }

    function generatePathV3(
        ...
        uint24 fee0 = fees(feeIndex0);
        uint24 fee1 = fees(feeIndex1);
        result = (swapOperation == SwapOperation.EXACT_INPUT ||
            swapOperation == SwapOperation.EXACT_INPUT_ALL)
            ? abi.encodePacked(tokenIn, fee0, connector, fee1, tokenOut)
            : abi.encodePacked(tokenOut, fee1, connector, fee0, tokenIn);
    }

    function getContracts(address adapter)
        internal
        view
        returns (address router, address quoter)
    {
        router = IAdapter(adapter).targetContract();
        quoter = routerToQuoter[router];
    }
}
