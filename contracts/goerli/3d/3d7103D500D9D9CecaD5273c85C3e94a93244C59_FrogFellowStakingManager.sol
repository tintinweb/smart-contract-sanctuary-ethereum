...
import "../interfaces/IFrogFellowReferral.sol";
import "../interfaces/IMiner.sol";
import "@openzeppelin/contracts-upgradeable/access/OwnableUpgradeable.sol";
import "@openzeppelin/contracts-upgradeable/security/ReentrancyGuardUpgradeable.sol";

// FrogFellowStakingManager is the master of frogFellow. He can make Position and he is a fair guy.
//
...
contract FrogFellowStakingManager is
    OwnableUpgradeable,
    ReentrancyGuardUpgradeable
{
    ...
    struct PoolInfo {
        ...
        uint256 accFrogPerShare; // Accumulated Positions per share, times 1e12. See below.
        uint16 depositFeeBP; // Deposit fee in basis points
        uint256 harvestInterval; // Harvest interval in seconds
    }

    // The Frog Fellow TOKEN!
    IERC20 public frogFellow;
    IMiner public frogFellowTreasury;
    // Dev address.
    ...
    IFrogFellowReferral public frogFellowReferral;
    ...
    function initialize(
        IERC20 _frogFellow,
        IMiner _frogFellowTreasury,
        uint256 _startBlock,
        uint256 _frogPerBlock
    ) public initializer {
        __ReentrancyGuard_init();
        __Ownable_init();
        frogFellow = _frogFellow;
        frogFellowTreasury = _frogFellowTreasury;
    ...
    // View function to see pending Frog on frontend.
    function pendingFrog(
        uint256 _pid,
        address _user
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 positionReward = multiplier
                ...
                .mul(pool.allocPoint)
                .div(totalAllocPoint);
            accFrogPerShare = accFrogPerShare.add(
                positionReward.mul(1e12).div(lpSupply)
            );
        }
        uint256 pending = user.amount.mul(accFrogPerShare).div(1e12).sub(
            user.rewardDebt
        );
    ...
    function updatePool(uint256 _pid) public nonReentrant {
        ...
        frogFellowTreasury.mint(address(this), positionReward);
        // transfer 10% to the dev wallet
        frogFellowTreasury.mint(devAddress, positionReward.div(10));
        pool.accFrogPerShare = pool.accFrogPerShare.add(
            positionReward.mul(1e12).div(lpSupply)
        );
        pool.lastRewardBlock = block.number;
    }

    function harvest(uint256 pid) public nonReentrant {
        PoolInfo storage pool = poolInfo[pid];
        UserInfo storage user = userInfo[pid][msg.sender];
        payOrLockupPendingPosition(pid);
        user.rewardDebt = user.amount.mul(pool.accFrogPerShare).div(1e12);
        //        emit Harvest(msg.sender, pid, _amount);
    }

    // Deposit LP tokens to FrogFellowStakingManager for Position allocation.
    function deposit(
        uint256 _pid,
        uint256 _amount,
        address _referrer
        ...
        if (
            _amount > 0 &&
            address(frogFellowReferral) != address(0) &&
            _referrer != address(0) &&
            _referrer != msg.sender
        ) {
            frogFellowReferral.recordReferral(msg.sender, _referrer);
        }
        ...
        if (_amount > 0) {
            ...
            //            if (address(pool.lpToken) == address(frogFellow)) {
            //                uint256 transferTax = _amount
            //                    .mul(frogFellow.transferTaxRate())
            //                    .div(10000);
    ...
    // Withdraw LP tokens from FrogFellowStakingManager.
    ...
    function payOrLockupPendingPosition(uint256 _pid) internal {
        ...
        uint256 pending = user.amount.mul(pool.accFrogPerShare).div(1e12).sub(
            user.rewardDebt
        );
    ...
    function safePositionTransfer(address _to, uint256 _amount) internal {
        uint256 positionBal = frogFellow.balanceOf(address(this));
        if (_amount > positionBal) {
            frogFellow.transfer(_to, positionBal);
        } else {
            frogFellow.transfer(_to, _amount);
        }
    }
    ...
    function updateEmissionRate(uint256 _frogPerBlock) public onlyOwner {
        massUpdatePools();
        emit EmissionRateUpdated(msg.sender, frogPerBlock, _frogPerBlock);
        frogPerBlock = _frogPerBlock;
    }

    // Update the position referral contract address by the owner
    function setFrogFellowReferral(
        IFrogFellowReferral _frogFellowReferral
    ) public onlyOwner {
        frogFellowReferral = _frogFellowReferral;
    }
    ...
    function payReferralCommission(address _user, uint256 _pending) internal {
        if (
            ...
            address referrer = frogFellowReferral.getReferrer(_user);
            uint256 commissionAmount = _pending.mul(referralCommissionRate).div(
                10000
            );

            if (referrer != address(0) && commissionAmount > 0) {
                if (frogFellow.balanceOf(address(this)) < commissionAmount) {
                    frogFellowTreasury.mint(address(this), commissionAmount);
                }
                frogFellow.transfer(referrer, commissionAmount);
                frogFellowReferral.recordReferralCommission(
                    referrer,
                    commissionAmount
                );
