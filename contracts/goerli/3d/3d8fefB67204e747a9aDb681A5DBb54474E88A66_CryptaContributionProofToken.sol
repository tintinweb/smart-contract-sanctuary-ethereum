...
import {ECDSA as ECDSASol} from "solady/src/utils/ECDSA.sol";
import {IERC721A} from "erc721a/contracts/IERC721A.sol";
import {SafeTransferLib} from "solady/src/utils/SafeTransferLib.sol";
import {ICryptaContributionProof} from "./ICryptaContributionProof.sol";
//  Crypta Digital                                                 
//                        .        :kk:        .                                       
//                    .;oo'       cO00Oc       ,ol,.                                   
//                  .lKW0,      .cO0000Oc.      ,0WKl.                                 
...
* 100% Utility token. 
* It can't be bought.
* It can only be earned by contributing to the communities 
* and projects that are part of Crypta network
* 
* CREDITS:
* 
* CRYPTA TEAM:
* @CryptoSeven007, @CrypSaf, @skyyfettiNFT, @MooreRads, 
* @nftboi_, @0xLTR, @r_elstad, @rivara_tyr, @Bxn_W, @TheCryptoOne3
* @WaliTheBot
*
* ARTISTS/DEVS HELPING:
* @Sternweiss_, @__parthpatel__, @Miks1410, @gl0in
*
* EARLY SUPPORTERS:
* @NuggetsNifty, @0xAlphaGems, @PGodjira, @FrensRG, @ColiseumCCCX, @hangoutDao_,
* @TBDKitchen, @0xDEGENio, @eth_apple, @searchfi_eth, @CheetopiaNFT,
* @ether, @Llamaverse_, @DigitalDoka
*
* SPECIAL SHOUTOUT to the amazing beta testers, friends and supporters:
* @JayDTR, @Indegenuity, @DrewBleam, @0xAlpaca_, @microglia_8888, @XabSeb, 
* [email protected]
*/
contract CryptaContributionProofToken is ICryptaContributionProof, ERC20Votes, ERC20Burnable, Ownable, ReentrancyGuard {
    address private signer;
    address public vault;
    uint256 immutable internal MAX_SUPPLY = 300_000_000 * 10 ** 18;

    mapping(string => bool) private _claimed;
    event Received(address indexed sender, uint256 value);

    constructor() 
        ERC20("CryptaContributionProof", "KUDOS") 
        ERC20Permit("CryptaContributionProof") 
        ERC20Burnable()
    {
    }

    modifier requireSignature(uint amount, string calldata nonce, bytes calldata signature) {
        bytes32 hash = keccak256(abi.encodePacked(amount, nonce, msg.sender));
        require(_matchSigner(hash, signature), "INVALID_SIGNER");
        require(!_claimed[nonce], "HAS_USED");
        _claimed[nonce] = true;
        _;
    }
    ...
    function claim(uint quantity, string calldata nonce, bytes calldata signature)
        external
        requireSignature(quantity, nonce, signature)
        nonReentrant
    {
    ...
    function isClaimed(string calldata nonce) external view returns(bool) {
        return _claimed[nonce];
    }
    ...
    function withdrawErc20(address _token) external nonReentrant onlyOwner {
        require(vault != address(0), "INVALID_VAULT");
        IERC20 targetToken = IERC20(_token);
        uint256 balance = targetToken.balanceOf(address(this));
        require(balance != 0, "EMPTY_BALANCE");
        SafeTransferLib.safeTransfer(_token, vault, balance);
    }
    ...
    function _matchSigner(bytes32 _hash, bytes calldata _signature) private view returns(bool) {
        return signer == ECDSASol.recover(ECDSASol.toEthSignedMessageHash(_hash), _signature);
    }
}
...
interface ICryptaContributionProof {

    function setSigner(address _signer) external;

    function claim(uint quantity, string calldata nonce, bytes calldata signature) external;
