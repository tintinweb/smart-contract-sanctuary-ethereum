...
contract CrowdFunding {
    ...
    function stakeInRequest(
        ...
        public
        onlyEligiableContributers(
            s_requestes[requestId].getMinimumContribution(),
            msg.sender
        )
    {
        s_requestes[requestId].stake(myVote);
    }
...
contract Stake {
    ...
    function stake(withdrawLib.vote myVote) public deadlineReached(false) {
        if (myVote == withdrawLib.vote.ACCEPT) {
            s_totalAcceptVote += 1;
        } else if (myVote == withdrawLib.vote.NEUTRAL) {
    ...
    function getMinimumContribution() public view returns (uint256) {
        return i_minContributionToVote;
    }
}