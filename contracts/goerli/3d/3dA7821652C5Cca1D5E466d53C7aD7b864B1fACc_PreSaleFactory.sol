...
contract PreSalePool is Ownable, ReentrancyGuard, Pausable, RedKiteWhitelist {
    ...
    function buyTokenByEtherWithPermission(
        ...
        require(
            tokens >= _minAmount ||
                userPurchased[_candidate].add(tokens) >= _minAmount,
            "POOL::MIN_AMOUNT_UNREACHED"
        );
        require(
            userPurchased[_candidate].add(tokens) <= _maxAmount,
    ...
    function buyTokenByTokenWithPermission(
        ...
        investedAmountOf[_token][_candidate] = investedAmountOf[_token][
            _candidate
        ].add(_amount);
    ...
    */
        ...
        internal
    {
        if(_currency == address(0)){
            ...
            IERC20(_currency).transfer(_beneficiary, _currencyAmount);
        }
    }
    ...
    function _verifyWhitelist(
        ...
        if (useWhitelist) {
            return (
                verify(signer, _candidate, _maxAmount, _minAmount, _signature)
            );
        }
        return true;
    }
