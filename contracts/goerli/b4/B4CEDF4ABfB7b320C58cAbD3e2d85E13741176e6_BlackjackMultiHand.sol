...
contract BlackjackMultiHand is ReentrancyGuard {
    ...
    */
    ...
    mapping(uint256 => Player[]) public playerHands;
    ...
    modifier betLimit(uint8 hands, bool isEth, uint256 amount) {
        require(hands <= 6, 'Player can have maximum of 6 hands');
        if(isEth) {
    ...
    modifier playerChecks(uint256 betId) {
        ...
        require(bet.dealer.hand.length > 0, "Card distribution for this bet id is not completed");
    ...
    */
    function play(uint8 hands, bool isEth, address tokenAddr, uint256 amount) public payable betLimit(hands,isEth, amount) {
        currentBetId++;
        ...
        if(isEth) {
            for(uint i = 0; i < hands; i++){
                Player storage hand = playerHands[currentBetId][i];
            ...
            for(uint i = 0; i < hands; i++){
                bet.playerHands[i].betAmount = amount;
            }
            emit BetPlaced(currentBetId, bet.startTime, msg.sender, amount);
        }

        uint8 numOfCards = (hands * 2) + 1;
        uint256[] memory cards = drawCard(currentBetId, numOfCards);

        for(uint i = 0; i < hands; i++){
            bet.playerHands[i].cards.push(cards[i]);
            bet.playerHands[i].cards.push(cards[i]);
        }
