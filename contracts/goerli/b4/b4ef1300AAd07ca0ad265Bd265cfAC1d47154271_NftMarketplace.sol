...
contract NftMarketplace is ReentrancyGuard {
    ...
    modifier AlreadyListed(address nftAddress, uint tokenId) {
        if (s_listings[nftAddress][tokenId].price > 0) {
    ...
    event ItemCancelled(address indexed nftAddress, uint indexed tokenId, address indexed owner);
    event ItemBought(
        address indexed nftAddress,
        uint indexed tokenId,
    ...
    function getProceedsBalance(address seller) external view returns (uint) {
