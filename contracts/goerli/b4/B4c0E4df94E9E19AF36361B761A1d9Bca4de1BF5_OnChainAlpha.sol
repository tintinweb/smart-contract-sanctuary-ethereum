...
//2 re-rolls for 0.02 eth each, straight to flooredApe treasury
...
contract OnChainAlpha is ERC721A, ReentrancyGuard, Ownable {
    ...
    event AttributesUpdated(uint256 tokenId, string userName, string twitter, string primaryENS, string website, string profileName);
    ...
    string[] private LAYER_NAMES = [unicode"Mouth Special", unicode"Outwear", unicode"Headwear", unicode"Eyewear", unicode"Mouth", unicode"Ears", unicode"Eyes", unicode"Body", unicode"Background"];

    bool private shouldWrapSVG = true;
    string private backgroundColor = "transparent";

    bool public isContractSealed;
    uint public constant maxSupply = 9999;
    uint public publicMintPrice = 0.0005 ether;
    ...
    constructor() ERC721A("On-Chain Alpha", "OCA") {
        TIERS[0] = [100,200,300,500,600,1000,1500,2300,3500]; //4
        TIERS[1] = [3000,2000,1500,1500,900,800,300]; //5
        TIERS[2] = [100,100,100,100,100,100,100,100,100,100,200,200,200,200,200,200,200,200,200,300,300,300,300,300,300,300,500,500,500,500,500,600,2000]; //33
        TIERS[3] = [100,200,300,500,600,650,700,750,800,1000,1100,3300]; //13
        TIERS[4] = [1000,1500,2000,5499]; //4
        TIERS[5] = [500,1000,2000,2500,4000]; //5
        TIERS[6] = [100,200,300,500,600,650,700,750,800,1000,1100,1200,2100]; //13
        TIERS[7] = [200,500,700,900,900,1000,1600,2000,2200]; //body 9
        TIERS[8] = [9999];
    }
    ...
    function reRoll(uint _tokenId) public payable whenUnsealed nonReentrant {
        require(msg.sender == ownerOf(_tokenId), "Only the token owner can re-roll");
        require(msg.value >= 0.02 ether, "Not enough ether");
        Address.sendValue(payable(testnet), msg.value);
        _initializeOwnershipAt(_tokenId);
    ...
    function handleMint(uint256 _count) internal whenMintActive returns (uint256) {
        ...
        if (fAContract.balanceOf(msg.sender) == 0){
    ...
    function tokenURI(uint _tokenId) public view override returns (string memory)
    {
        ...
        if (StringUtils.length(tokenValues[_tokenId].userName) > 0)
        {
            jsonBytes.appendSafe(abi.encodePacked(unicode"{\"name\":\"", tokenValues[_tokenId].userName));
        } else {
            ...
            if (shouldWrapSVG) {
                ...
                svgCode = string(
                    abi.encodePacked(
                        ...
                        Base64.encode(
                            abi.encodePacked(
                                '<svg width="100%" height="100%" viewBox="0 0 1600 1600" version="1.2" xmlns="http://www.w3.org/2000/svg"><image width="1600" height="1600" href="',
                                svgString,
                                '"></image></svg>'
                            )
                        )
    ...
    //profile functions
    function initalizeToken(uint256 _tokenId) internal {
        AlphaToken memory initToken;
        tokenValues[_tokenId] = initToken;

        emit AttributesUpdated(
            _tokenId, 
            tokenValues[_tokenId].userName, 
            tokenValues[_tokenId].twitter, 
            tokenValues[_tokenId].primaryENS, 
            tokenValues[_tokenId].website,
            tokenValues[_tokenId].profileName
        );
    }

    function setProfile(uint256 _tokenId, string memory _username, string memory _twitter, string memory _primaryENS, string memory _website) external {
        require(ownerOf(_tokenId) == msg.sender, "you do not own this token");

        _username = normalize(_username);

        if (keccak256(abi.encodePacked(tokenValues[_tokenId].userName)) != keccak256(abi.encodePacked(_username)))
        { tokenValues[_tokenId].userName = _username; }

        if (keccak256(abi.encodePacked(tokenValues[_tokenId].twitter)) != keccak256(abi.encodePacked(_twitter)))
        { tokenValues[_tokenId].twitter = _twitter; }

        if (keccak256(abi.encodePacked(tokenValues[_tokenId].primaryENS)) != keccak256(abi.encodePacked(_primaryENS)))
        { tokenValues[_tokenId].primaryENS = _primaryENS; }

        if (keccak256(abi.encodePacked(tokenValues[_tokenId].website)) != keccak256(abi.encodePacked(_website)))
        { tokenValues[_tokenId].website = _website; }

        tokenValues[_tokenId].profileName = StringUtils.concat(StringUtils.concat(_username, "#"), _toString(_tokenId));
    ...
    function getProfile(uint256 _tokenId) external view returns(AlphaToken memory result){
        return tokenValues[_tokenId];
    }

    function getProfileName(uint256 _tokenId) external view returns(string memory){
        return tokenValues[_tokenId].profileName;
    }

    function normalize(string memory word) private pure returns (string memory) {
        string memory checkLetter;
        string memory result = "";

        for (uint256 i = 0; i < word.length(); i++) {

            checkLetter = StringUtils._substring(word, 1, int(i));
            if(!StringUtils.compareTo(checkLetter, " ") &&
                !StringUtils.compareTo(checkLetter, "<") &&
                !StringUtils.compareTo(checkLetter, ">") &&
                !StringUtils.compareTo(checkLetter, "\"") &&
                !StringUtils.compareTo(checkLetter, "'") &&
                !StringUtils.compareTo(checkLetter, "%") &&
                ...
                result = StringUtils.concat(result, checkLetter);
            }   
        }
        return result;
    }
