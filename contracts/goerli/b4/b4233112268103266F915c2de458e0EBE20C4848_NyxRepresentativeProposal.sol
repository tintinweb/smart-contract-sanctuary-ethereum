...
contract NyxRepresentativeProposal is Ownable, converter {
        ...
        returns (bytes memory)
    {
        ...
        RepresentativeProposal memory proposal = RepresentativeProposal(proposalId, parsedParams.representativeAddress, parsedParams.action,
            parsedParams.socialNetworkLink, parsedParams.representativeDescription);
        ...
        returns (bytes[] memory)
    {
        ...
        prop[3] = abi.encode(params.representativeDescription);
        return prop;
    }
        ...
        returns (bytes[4][] memory props)
    {
        ...
        for (uint256 idx = 0; idx < proposalBytesArr.length; idx++)
        {
            ...
            props[idx] = [abi.encode(params.representativeAddress), abi.encode(params.action), abi.encode(params.socialNetworkLink), abi.encode(params.representativeDescription)];
        }
    }
}
