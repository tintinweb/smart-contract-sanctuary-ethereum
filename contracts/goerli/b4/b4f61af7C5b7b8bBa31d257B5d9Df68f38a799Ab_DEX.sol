...
contract DEX {
    ...
    function ethToToken() public payable returns (uint256 tokenOutput) {
        require(msg.value > 0, "DEX: ethToToken - no eth sent");
        tokenOutput = price(
            msg.value,
        ...
        require(
            ...
            "DEX: ethToToken - transfer did not transact"
        );
        emit EthToTokenSwap();
    }
    ...
    function tokenToEth(uint256 tokenInput) public returns (uint256 ethOutput) {
        require(tokenInput > 0, "DEX: tokenToEth - no tokens sent");
        require(
    ...
    function deposit() public payable returns (uint256 tokensDeposited) {
        ...
        tokensDeposited = msg.value.mul(
            token.balanceOf(address(this)).div(address(this).balance)
        );
        require(
    ...
    function withdraw(
        ...
        require(amount > 0, "DEX: withdraw - no tokens sent");
        require(
            liquidity[msg.sender] >= amount,
            "DEX: withdraw - not enough liquidity"
        );
        eth_amount = amount.mul(address(this).balance).div(totalLiquidity);
        token_amount = amount.mul(token.balanceOf(address(this))).div(
            totalLiquidity
        );
        liquidity[msg.sender] = liquidity[msg.sender].sub(amount);
        totalLiquidity = totalLiquidity.sub(amount);
        (bool success, ) = msg.sender.call{value: eth_amount}("");
        require(success, "DEX: withdraw - transfer did not transact");
        require(
            token.transfer(msg.sender, token_amount),
            "DEX: withdraw - transfer did not transact"
        );
