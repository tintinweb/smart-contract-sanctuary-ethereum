...
contract ToyApeKennelClub is Ownable, ERC721A, ERC2981, ReentrancyGuard {
    ...
    function getOwnedToyApeIds(address holder) external view returns(uint16[] memory) {
        ...
        uint16[] memory indexMap = new uint16[](totalToyApes);

        uint16 index;
        for(uint16 tokenId; tokenId <= totalToyApes; ++tokenId) {
            if(_toyApeYachtClub.ownerOf(tokenId) == holder && !isClaimed(tokenId)) {
                indexMap[index] = tokenId;
                index++;
            }
        }

        uint16[] memory tokenIds = new uint16[](index);
    ...
    function holderClaim(uint16[] calldata ownedToyApeIds) public nonReentrant {
        ...
        require(CONTRACT_STATUS == TAKCContractStatus.holderClaim, "Claim is not active.");
