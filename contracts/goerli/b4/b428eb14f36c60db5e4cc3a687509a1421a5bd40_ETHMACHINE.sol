...
contract ETHMACHINE {
    ...
    IToken public ETH;
    address erctoken = 0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2; /** WRAPPED ETH **/
    ...
    uint256 public MIN_INVEST_LIMIT = 0 * 1e6; 
    uint256 public WALLET_DEPOSIT_LIMIT = 1 * 1e6; /** 1 ETH  **/
    ...
    function sellCrops() public{
        ...
        ETH.transfer(msg.sender, eggsPayout);
    ...
    function hireFarmers(address ref, uint256 amount) public{
        ...
        ETH.transferFrom(address(msg.sender), address(this), amount);
        ...
        if (user.referrer != address(0)) {
            ...
            if (upline != address(0)) {
                ...
                ETH.transfer(upline, refRewards);
    ...
    function payFees(uint256 eggValue) internal returns(uint256){
        ...
        ETH.transfer(dev1, tax);
        ETH.transfer(dev2, tax);
        ETH.transfer(dev3, tax);
        ETH.transfer(prtnr1, tax);
        ETH.transfer(prtnr2, tax);
        ETH.transfer(mkt, tax);
        return tax.mul(6);
    }
