    ...
    ERC721Royalty
{
    ...
    */
    ...
    function adminMint(address shoeOwner, string memory tokenUri, uint256 numMints)
        public
        returns (uint256, uint256)
    {
        require(
            hasRole(ENDSTATE_ADMIN_ROLE, _msgSender()),
            "EndstateNFT: must have admin role to mint"
        );

        uint256 lastId;
        for (uint256 i = 0; i < numMints; i++) {
            lastId = mint(shoeOwner, tokenUri);
        }
    ...
    function _checkLimit(uint256 numMints) internal view virtual {
        if (maxSupply > 0) {
            require(
                (_tokenIds.current() + numMints <= maxSupply),
                "EndstateNFT: token limit reached"
            );
        }
    }

    function mint(address shoeOwner, string memory tokenUri) internal returns (uint256) {
        _checkLimit(1);
        ...
        _setTokenURI(newShoeTokenId, string.concat(tokenUri, Strings.toString(newShoeTokenId)));
