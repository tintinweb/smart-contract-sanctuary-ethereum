...
// File: main/contracts/unpacker/interfaces/IERC1155.sol
...
// File: main/contracts/unpacker/unpacker.sol
...
contract unpacker is Ownable
{
    ...
    function claim(uint256 requestId)public
    {
        ...
        require(res.owner != address(0), "This request is empty");
        require(owner == res.owner, "Missing authorization of owner");

        if(!res.error)
        {
            require(res.rewards.length > 0, "Rewards is not initialized");
    ...
    function withdrawAssets(uint256 tokenId, uint256 amount, address withdrawTo)public onlyOwner
    {
        parentNFT.safeTransferFrom(address(this), withdrawTo, tokenId, amount, "0x00");
    }
