...
contract NFTauction is Ownable, ReentrancyGuard {
    ...
    modifier OnlyCuratorORItemOwner(address nftContract, uint256 tokenId, uint256 auctionType) {
        if (auctionType == 0) {
            require(curators[nftContract] == msg.sender);
        }
    ...
    function addCurators(address nftContract, address curator) external onlyOwner {
        ...
        if (curators[_nftContract] == _curator) {
            revert AlreadyAdded(_curator);
        }
        curators[_nftContract] = _curator;
        ...
        public nonReentrant
    {
        ...
        IERC20(paymentToken).safeApprove(address(this), _bidPrice);
    ...
    function extendAuction(address nftContract, uint256 tokenId, uint256 extensionDuration, uint256 auctionType) 
        public nonReentrant
        OnlyCuratorORItemOwner(nftContract, tokenId, auctionType)
    {
