...
contract RedeemableCollection is Collection {
    mapping(uint256 => address) public redeemed;

    event Redeemed(uint256 itemID, address redeemer);

    function redeem(uint256 itemID) external {
        address sender = _msgSender();

        require(ownerOf(itemID) == sender, "sender is not owner");
        require(redeemed[itemID] == address(0), "item already redeemed");

        redeemed[itemID] = sender;
        emit Redeemed(itemID, sender);
    }
}
