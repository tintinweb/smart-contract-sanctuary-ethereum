...
contract AllowListFacet is AllowList {
    ...
    string constant testString = "TEST";
    ...
    function allowListEnabled() external view returns (bool enabled) {
        return AllowListStorage.layout().allowLists[0].allowListEnabled;
    }
    ...
    function allowListEnabled(
        ...
        return AllowListStorage.layout().allowLists[_tokenId].allowListEnabled;
    }
    ...
    function allowList(
        ...
        AllowListStorage.AllowListStruct
            storage allowListInfo = AllowListStorage.layout().allowLists[
                _tokenId
            ];
        allowListMap = new AllowListMap[](allowListInfo.allowList.length());
        uint256 allowListLength = allowListInfo.allowList.length();
        uint256 i = 0;
        for (; i < allowListLength; ) {
            address addressAtIndex = allowListInfo.allowList.at(i);
            allowListMap[i].account = addressAtIndex;
            allowListMap[i].allowance = allowListInfo.allowance[addressAtIndex];
            ++i;
        }
    }
}
...
contract AllowList is AllowListInternal, OwnableInternal {
    ...
    function _enableAllowList(uint256 tokenId) internal {
        if (AllowListStorage.layout().allowLists[tokenId].allowListEnabled) {
            revert AllowListEnabled();
        }

        AllowListStorage.layout().allowLists[tokenId].allowListEnabled = true;
    ...
    function _disableAllowList(uint256 tokenId) internal {
        ...
        AllowListStorage.layout().allowLists[tokenId].allowListEnabled = false;
...
contract AllowListInternal is IAllowListInternal {
    ...
    modifier allowListed(uint256 tokenId, address account) {
        if (
            !AllowListStorage.layout().allowLists[tokenId].allowList.contains(
                account
            )
        ) {
    ...
    function _allowListAllowance(
        ...
        return AllowListStorage.layout().allowLists[tokenId].allowance[account];
    }
    ...
    function _addToAllowList(
        ...
        if (allowListInfo.allowList.contains(_account)) {
            revert AccountAlreadyAllowListed();
        }

        allowListInfo.allowList.add(_account);
        allowListInfo.allowance[_account] = _allowance;
        allowListInfo.allowTime[_account] = _allowTime;
    ...
    function _addToAllowList(
        ...
        for (; i < accountsLength; ) {
            if (allowListInfo.allowList.contains(_accounts[i])) {
                revert AccountAlreadyAllowListed();
            }
            allowListInfo.allowList.add(_accounts[i]);
            allowListInfo.allowance[_accounts[i]] = _allowance;
            ++i;
        }
    ...
    function _addToAllowList(
        ...
        for (; i < accountsLength; ) {
            ...
            allowListInfo.allowList.add(_accounts[i]);
            allowListInfo.allowance[_accounts[i]] = _allowance;
            allowListInfo.allowTime[_accounts[i]] = _allowTime;
            ++i;
        }
    ...
    function _removeFromAllowList(uint256 tokenId, address _account) internal {
        ...
        allowListInfo.allowList.remove(_account);
        delete allowListInfo.allowance[_account];
    ...
    function _removeFromAllowList(
        ...
        for (; i < accountsLength; ) {
            ...
            allowListInfo.allowList.remove(_accounts[i]);
            delete allowListInfo.allowance[_accounts[i]];
            ++i;
        }
...
library AllowListStorage {
     ...
     */
    struct Layout {
        bool allowListEnabled; // Deprecated
        EnumerableSet.AddressSet allowList; // Deprecated
        mapping(address => uint256) allowance; // Deprecated
        mapping(address => uint256) allowTime; // Deprecated
        mapping(address => uint256) minted; // Deprecated
