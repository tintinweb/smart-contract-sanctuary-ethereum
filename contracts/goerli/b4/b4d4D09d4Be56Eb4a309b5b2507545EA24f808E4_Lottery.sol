...
contract Lottery is OwnableUpgradeable {
    ...
    function internalBuy(uint256 _price, uint8[2] memory _numbers) internal {
        ...
        ownerOfTicket[newItemId] = address(this);
        _holderTokens[address(this)].add(newItemId);
    ...
    function multiBuy(uint256 _price, uint8[2][] memory _numbers) external inDrawingPhase {
        ...
        uint256 _totalAddresses = totalAddresses;
        ...
        for (uint i = 0; i < _numbers.length; i++) {
            _tokenIds.increment();
            uint256 newItemId = _tokenIds.current();
            lotteryTicketInfo[newItemId] = _numbers[i];
            lotteryTicketAmount[newItemId] = _price;
            issueIndexTicketMap[newItemId] = _issueIndex;
            ownerOfTicket[newItemId] = msg.sender;
            _holderTokens[msg.sender].add(newItemId);

            lotteryInfo[_issueIndex].push(newItemId);
            if (userInfo[msg.sender].length == 0) {
                _totalAddresses = _totalAddresses + 1;
            }
            userInfo[msg.sender].push(newItemId);
            _totalAmount = _totalAmount.add(_price);
            _lastTimestamp = block.timestamp;
            totalPrice = totalPrice.add(_price);

            // 3 means keyLengthForEachBuy
            uint32[3] memory numberIndexKey;
            numberIndexKey[0] = uint32(_numbers[i][0])*256*256 + 1*256 + uint32(_numbers[i][1]);
            numberIndexKey[1] = uint32(_numbers[i][0]);
            numberIndexKey[2] = 1*256 + uint32(_numbers[i][1]);

            userBuyAmountSum[_issueIndex][numberIndexKey[0]]=userBuyAmountSum[_issueIndex][numberIndexKey[0]].add(_price);
            userBuyAmountSum[_issueIndex][numberIndexKey[1]]=userBuyAmountSum[_issueIndex][numberIndexKey[1]].add(_price);
            userBuyAmountSum[_issueIndex][numberIndexKey[2]]=userBuyAmountSum[_issueIndex][numberIndexKey[2]].add(_price);
        }        
