...
contract Freakshow is Ownable, ERC721A, IERC721Receiver {
    using SafeMath for uint256;

    address clownsAddress;
    string public baseTokenURI;
    uint256 public exchangeRate = 5;
    uint256[] customFreakshowTokens;
    mapping(uint256 => bool) public isCustomClown;

    constructor(address _clownsAddress) ERC721A("Freakshow", "Freakshow") {
        ...
        clownsAddress = _clownsAddress;
    }

    function getCustomFreakshowTokens() external view returns (uint256[] memory) {
    ...
    function exchangeCustomMint(uint256[] calldata _tokens) external {
        for (uint256 index; index < _tokens.length; index++) {
            require(isCustomClown[_tokens[index]], "Not custom token");
            IERC721(clownsAddress).safeTransferFrom(
                msg.sender,
                address(this),
                _tokens[index]
            );
    ...
    function exchangeMint(uint256[] calldata _tokens) external {
        require(
