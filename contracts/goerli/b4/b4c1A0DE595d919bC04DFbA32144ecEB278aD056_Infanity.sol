...
contract Infanity is ERC1155, ERC2981, AccessControl  {
    ...
    mapping(uint256 => uint256) brandCollectionTracker;
    ...
    function mint(uint256 _collectionId, uint256 _quantity) external payable {
        // get brand make sure its enabled
        require(infanities[_collectionId].mintPriceInWei * _quantity == msg.value, "Eth not enough");
        _mintCollection(msg.sender, _collectionId, _quantity);
    }

    function mint(address _to, uint256 _collectionId, uint256 _quantity) external onlyHasRole(MINTER_ROLE) {
    ...
    function _mintCollection(address _to, uint256 _collectionId, uint256 _quantity) internal {
        uint256 brandId = brandCollectionTracker[_collectionId];
        require(brands[brandId].enabled, "Brand currently disabled.");

        InfanityItem memory item = infanities[_collectionId];
        require(item.exist, "Collection doesnt exist");
        require(item.minted * _quantity  <= item.cap, "Cannot mint more than max supply");
        

        infanities[_collectionId].minted = item.minted + _quantity;
    ...
    function getCollectionInfo(uint256 collectionId) external view returns (InfanityItem memory) {
        require(infanities[collectionId].exist, "Infanity doesnt exist.");
        return infanities[collectionId];
    }
    ...
    function createCollection(uint256 brandId, uint256 mintPriceInWei, uint256 cap, bool enabled) external onlyHasRole(ADMIN_ROLE) {
        require(brands[brandId].exist, "brand doesnt exist");
        ...
        brandCollectionTracker[collectionId] = brandId;

        infanities[collectionId] = InfanityItem(
            brandId, 
            mintPriceInWei,
            cap,
            0,
    ...
    function updateCollectionMintPrice(uint256 _id, uint256 _mintPriceInWei) external onlyHasRole(ADMIN_ROLE) {
    ...
    function nextCollectionId() public view returns (uint256) {
        return _nextCollectionId.current();
    }
