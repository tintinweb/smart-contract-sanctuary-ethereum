...
///@notice Swap assets cross-chain using 0x for both legs
contract SwapXc is ISwapXc, ZetaInteractor, ZetaReceiver, Pausable {
            ...
            ZetaInterfaces.SendInput({
                ...
                message: abi.encode(
                    XcMessage({
                        ...
                        minSecondLegZetaAmount: _args.minSecondLegZetaAmount,
        ...
        isValidMessageCall(_zetaMessage)
    {
        ...
        if (_zetaMessage.zetaValue < xcMessage.minSecondLegZetaAmount)
        ...
        if (xcMessage.toToken == NATIVE) {
            ...
            (bool success, ) = sZeroEx.call{value: 0}(xcMessage.secondLegCalldata);
            if (!success) revert SellZetaFailed();

            toTokenFinalAmount = address(this).balance - prevBal;

            TransferHelper.safeTransferETH(recipientAddress, toTokenFinalAmount);
        } else {
            IERC20 toToken = IERC20(xcMessage.toToken);
            uint256 prevBal = toToken.balanceOf(address(this));

            (bool success, ) = sZeroEx.call{value: 0}(xcMessage.secondLegCalldata);
            if (!success) revert SellZetaFailed();

            toTokenFinalAmount = toToken.balanceOf(address(this)) - prevBal;

            TransferHelper.safeTransfer(xcMessage.toToken, recipientAddress, toTokenFinalAmount);
        }
