...
//                                                 ................
/*
* $KUDOS - Proof of Contribution Token
* 100% Utility token. 
* It can't be bought.
...
* @JayDTR, @Sternweiss_, @Indegenuity, @DrewBleam, @0xAlpaca_, @microglia_8888, @XabSeb, 
* [email protected]
*/
contract CryptaContributionProofToken is ICryptaContributionProof, ERC20Votes, ERC20Burnable, Ownable, ReentrancyGuard {
    ...
    modifier requireSignature(uint256 _amount, uint256 _nonce, bytes calldata _signature) {
        bytes32 _hash = keccak256(abi.encodePacked(_amount, _nonce, msg.sender));
        if (!_matchSigner(_hash, _signature)) revert InvalidSigner();
        if (!claimed[_nonce]) revert NonceAlreadyClaimed();
       claimed[_nonce] = true;
        _;
    }
    ...
    function burnFrom(address _account, uint256 _amount) public override virtual {
    ...
    function claim(uint _quantity, uint256 _nonce, bytes calldata _signature)
        external
        requireSignature(_quantity, _nonce, _signature)
        nonReentrant
    {
    ...
    function isClaimed(uint256 _nonce) external view returns(bool) {
        return claimed[_nonce];
    }
    ...
    function withdrawErc20(address _token) external nonReentrant onlyOwner {
        ...
        if(_balance == 0) revert InvalidBalance();
        SafeTransferLib.safeTransfer(_token, vault, _balance);
    }
...
interface ICryptaContributionProof {
    ...
    function claim(uint256 _quantity, uint256 _nonce, bytes calldata _signature) external;
