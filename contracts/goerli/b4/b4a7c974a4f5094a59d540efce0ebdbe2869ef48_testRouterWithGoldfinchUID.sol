...
// File: testRouterWithoutUID.sol


pragma solidity ^0.8.1;

import "frigg/SafeERC20.sol";
import "frigg/Ownable.sol";
import "frigg/IERC1155.sol";
import "frigg/IAccessControl.sol";
...
interface IFrigg {
    ...
    */
    ...
    function _burn(address account, uint256 amount) external view;
...
contract testRouterWithGoldfinchUID {

    using SafeERC20 for IERC20;

    event SuccessfulPurchase (address indexed _buyer, uint256 amount);
    event SuccessfulExpiration (address indexed _seller, uint256 amount);
    ...
    */
    ...
    function add(address outputTokenAddress, address _uIdContract, address _issuer, uint256 _issuancePriceInUSDC, uint256 _expiryPriceInUSDC, address _issuanceTokenAddress) external {
        IAccessControl outputToken = IAccessControl(outputTokenAddress);
        bytes32 DEFAULT_ADMIN_ROLE = 0x00;
        require(outputToken.hasRole(DEFAULT_ADMIN_ROLE, msg.sender), "only token admin can add the token to this router");
        tokenData[outputTokenAddress]=TokenData(_issuer, _uIdContract, _issuancePriceInUSDC, _expiryPriceInUSDC, _issuanceTokenAddress);
    }
    ...
    */ 
    ...
    //@param users buy outputToken with inputToken
    ...
    //outputAmount should be in 18 decimals
    function buy(address friggTokenAddress, uint256 inputTokenAmount) external {
            ...
            outputToken.mint(msg.sender,inputTokenAmount/tokenData[friggTokenAddress].issuancePrice);
            
            emit SuccessfulPurchase(msg.sender, inputTokenAmount);
        }

    //@param users sell inputToken for outToken
    //@dev initially users can only sell Frigg-issued asset backed tokens for USDC
    //i.e. inputToken is ABT and outputToken is USDC
    //inputAmount should be in 18 decimals
    //outputAmount should be in 6 decimals if USDC
    function sell(address friggTokenAddress, uint256 inputFriggAmount) external {
            require(inputFriggAmount > 0, "You cannot sell 0 token");
            require(IERC1155(tokenData[friggTokenAddress].uIdContract).balanceOf(msg.sender,0) > 0, "Need a UID token");
            ...
            //Issuer SC address should give approval to router to transfer USDC to msg.sender
            outputToken.transferFrom(tokenData[friggTokenAddress].issuer,msg.sender,inputFriggAmount * tokenData[friggTokenAddress].expiryPrice);
            
            emit SuccessfulExpiration(msg.sender, inputFriggAmount);
        }
}
...
import "frigg/Address.sol";
import "frigg/IERC20.sol";
import "frigg/IERC20Permit.sol";
