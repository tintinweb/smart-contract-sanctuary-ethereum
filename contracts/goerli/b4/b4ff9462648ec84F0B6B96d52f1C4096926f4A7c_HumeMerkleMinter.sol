...
import { IHumeMerkleMinter, EditionMintData, MintInfo } from "./interfaces/IHumeMerkleMinter.sol";
import { IMinterModule } from "./../core/interfaces/IMinterModule.sol";
import { ISoundEditionV1 } from "./../core/interfaces/ISoundEditionV1.sol";

/**
 * @title HumeMerkleMinter
 * @dev Module for minting Sound editions using a merkle tree of whitelisted addresses with approved
 *      allowance and price per account.
 * @author humie devs
 */
contract HumeMerkleMinter is IHumeMerkleMinter, BaseMinter {
    // =============================================================
    //                            STORAGE
    // =============================================================
    ...
    /**
     * @dev minting price info.
     *      Maps `edition` => `mintId` => `minter` => price.
     */
    mapping(address => mapping(uint128 => mapping(address => uint96))) internal _userMintPrice;
    ...
    function mint(
        ...
        bytes32 leaf = keccak256(abi.encodePacked(msg.sender, allowance, price));
        bool valid = MerkleProofLib.verify(merkleProof, data.merkleRootHash, leaf);
        if (!valid) revert InvalidMerkleProof();

        // Store price info after successfully verifying
        _userMintPrice[edition][mintId][msg.sender] = price;
        ...
        unchecked {
            ...
            if (numberMinted + requestedQuantity > allowance) revert ExceedsAccountAllowance();
        }

        // BaseMinter method. TODO: Will need to be overriden to support DelegateBaseMinter
    ...
    function totalPrice(
        ...
        unchecked {
            ...
            return uint128(uint256(_userMintPrice[edition][mintId][minter]) * uint256(quantity));
        }
    }
    ...
    function supportsInterface(bytes4 interfaceId) public view override(IERC165, BaseMinter) returns (bool) {
        return BaseMinter.supportsInterface(interfaceId) || interfaceId == type(IHumeMerkleMinter).interfaceId;
    }
    ...
    function moduleInterfaceId() public pure returns (bytes4) {
        return type(IHumeMerkleMinter).interfaceId;
    }
}
...
/**
 * @dev Data unique to a Hume merkle mint.
 */
...
/**
 * @title IHumeMerkleMinter
 * @dev Interface for the `HumeMerkleMinter` module.
 * @author humie devs
 */
interface IHumeMerkleMinter is IMinterModule {
    // =============================================================
    //                            EVENTS
    // =============================================================
    ...
    /**
     * @dev Returns IHumeMerkleMint.MintInfo instance containing the full minter parameter set.
