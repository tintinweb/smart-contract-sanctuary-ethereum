...
contract VotePoolLayout is Upgradeable {
    event ConfigChanged(VotePoolConfig newcfg);
    ...
    VotePoolConfig public innercfg;
    ...
    function initialize(
        ...
        address asset_,
        VotePoolConfig memory innercfg_
    ...
    function changeInnercfg(VotePoolConfig calldata cfg) public onlyOwner {
        _changeInnercfg(cfg);
    }

    function _changeInnercfg(VotePoolConfig memory cfg) internal {
        require(cfg.minimumVote > 0, "zero amount");
        require(cfg.weightWithoutBidder < 1e5, "overflow");
        require(cfg.weightWithBidder < 1e5, "overflow");
        innercfg = cfg;
...
contract VotePool is VotePoolInterface, VotePoolLayout {
    ...
    function currentShare(bytes32 orderHash, uint256 amount) public view override returns (uint120) {
        VotePoolConfig memory _cfg = innercfg;

        if (amount < _cfg.minimumVote) revert VoteAmoutTooLow();

        if (exchange.getOrderStatus(orderHash) != OrderStatus.Listing) revert OrderIsNotListing();

        uint256 weight = exchange.lastBidder(orderHash) == address(0)
            ? _cfg.weightWithoutBidder
            : _cfg.weightWithBidder;
        ...
        if (share == 0 || share > type(uint120).max) revert InvalidShare();
...
interface ExchangeInterface {
    ...
    /// @param externalData the external data from seller.
    event NewOrder(address indexed seller, bytes32 orderHash, bytes externalData);
    ...
    /// @notice close the order and open new one.
    /// Requirements:
    ///
    ///  1. the order cannot be sold until it has expired.
    ///
    function reopen(bytes32 oldOrderHash, OriginOrder calldata order) external;
    ...
    /**
     * @notice Check if the given order allows voting.
     */
    function allowVote(bytes32 orderHash) external view;
...
error EarnFactorTooSmall();
error EarnFactorTooBig();
error InvalidDeposit();
error OrderEndTimeOverLimit();
error NotFoundVote(uint256 voteId);
error OverflowMaxValue(uint256 max);
error NotSupportedType();
error NotSupportVote();
error OrderNotFound();
...
struct OriginOrder {
    ...
    address token; // The contract address of the nft token.
    uint256 tokenId; // The token id of the nft token.
    bytes externalData; // will be print in SELL Event.
...
struct VotePoolConfig {
    uint120 minimumVote;
    uint32 weightWithoutBidder;
    uint32 weightWithBidder;
}
