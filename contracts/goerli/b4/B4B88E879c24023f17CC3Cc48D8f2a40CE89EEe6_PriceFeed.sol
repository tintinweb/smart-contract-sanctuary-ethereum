...
/*
 * PriceFeed mock contract for Testnet
 * Does not use Chainlink oracles. Use the setter functions instead.
 */
contract PriceFeed is Initializable, Ownable, IPriceFeed {
    ...
    function setParameters(address _adminContract) external initializer {
	...
	function setOracles(
		address _mockETHAddress, 
		address _mockBTCAddress,
		uint256 _priceETH,
		uint256 _priceBTC,
		uint256 _index) 
		external {
		...
		mockBTCAddress = _mockBTCAddress;
		priceETH = _priceETH;
		priceBTC = _priceBTC;
		index = _index;
	...
	function fetchPrice(address _asset) external override returns (uint256) {
		...
		if (_asset == mockETHAddress) {
			emit LastGoodPriceUpdated(_asset, priceETH);
			emit LastGoodIndexUpdated(_asset, priceETH);
			return priceETH.mul(1 ether).div(index);
		}

		if (_asset == mockBTCAddress) {
			emit LastGoodPriceUpdated(_asset, priceBTC);
			emit LastGoodIndexUpdated(_asset, priceBTC);
			return priceBTC.mul(1 ether).div(index);
		}
	}

	function setPriceETH(uint256 _price) external returns (bool) {
		priceETH = _price;
		return true;
	}

	function setPriceBTC(uint256 _price) external returns (bool) {
		priceBTC = _price;
		return true;
	}
