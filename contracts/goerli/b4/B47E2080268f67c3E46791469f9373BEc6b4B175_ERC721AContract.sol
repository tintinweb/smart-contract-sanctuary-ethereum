...
contract ERC721AContract is ERC721A, Ownable, PaymentSplitter {
    ...
    function paperSetup(address _paperAddress, address _paperKey) external onlyOwner {
    ...
    function canMint(address _address, uint16 _quantity, bytes32[] memory _proof) private returns (bool) {
        ...
        require(totalSupply() + _quantity <= _maxSupply, "Insufficient supply");
        require(_saleIsActive, "Sale inactive");
        if(_preSaleIsActive) {
            if (token.salePhase == 1) {
                require(mintpass.balanceOf(_address, mintpassId) >= _quantity, "Invalid quantity");
                mintpass.burnForAddress(mintpassId, _quantity, _address);
            }
            if (token.salePhase == 2) {
                uint16 mintedAmount = hasMinted[_address] + _quantity;
    ...
    function mint(uint16 _quantity, bytes32[] memory _proof) public payable {
        ...
        if (canMint(msg.sender, _quantity, _proof)) {
            _safeMint(msg.sender, _quantity);
        }
    }

    function crossmint(address _address, bytes32[] memory _proof, uint256 _quantity) public payable onlyCrossmint {
        require(price() * _quantity <= msg.value, "ETH incorrect");
        uint16 quantity = uint16(_quantity);
        if (canMint(_address, quantity, _proof)) {
            _safeMint(_address, quantity);
        }
    }

    function paper(address _address, bytes32[] memory _proof, uint256 _quantity, bytes32 _nonce, bytes calldata _signature) public payable 
        onlyPaper(keccak256(abi.encode(_address, _proof, _quantity)), _nonce, _signature)
    {
        require(price() * _quantity <= msg.value, "ETH incorrect");
        uint16 quantity = uint16(_quantity);
        if (canMint(_address, quantity, _proof)) {
