...
contract Lottery {
    ...
    function giveTickets(address wallet, uint256 amount) external onlyOwner {
    ...
    function rng() internal view returns (uint256 result) {
        result = uint256(
            keccak256(abi.encodePacked(ticket_cost, fees, players, pool))
        );
    }

    function removePlayer(uint256 index) internal {
        players[index] = players[players.length - 1];
        players.pop();
    }

    function pickWinner() external onlyOwner notZeroPlayers {
        winners = new address[](0);

        // pick winners
        uint256 numPlayers = players.length;
        for (uint256 i = 0; i < 3 && i < numPlayers; i++) {
            uint256 pickedWinnerIndex = rng() % players.length;
            winners.push(players[pickedWinnerIndex]);
            removePlayer(pickedWinnerIndex);
        }
        uint256 winningAmount = pool / winners.length;

        // Record base prize
        for (uint256 i = 0; i < winners.length; i++) {
            claimableRewards[winners[i]][address(ERC20Token)] = winningAmount;
        }
        // Record bonus prize
        for (uint256 i = 0; i < bonusPoolTokens.length; i++) {
            ...
            for (uint256 i = 0; i < winners.length; i++) {
                claimableRewards[winners[i]][bonusToken] = bonusTokenAmount;
            }
        }
        // Reset info
        pool = 0;
    ...
    function claimRewards(address[] calldata tokenAddresses) external {
        ...
        ERC20Token.approve(address(this), basePrize);
        ERC20Token.transferFrom(address(this), msg.sender, basePrize);
        for (uint256 i = 0; i < tokenAddresses.length; i++) {
            ...
            if (tokenAddresses[i] == address(0)) {
                ...
                // erc20s
                IERC20(tokenAddresses[i]).approve(address(this), tokenAmount);
