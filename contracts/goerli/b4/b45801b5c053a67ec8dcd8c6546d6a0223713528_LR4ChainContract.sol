...
contract LR4ChainContract is LR4Ownable,  LR4Sealed, LR4Common, LR4Killable {
    ...
    //mapping key = parcel code  value = list of  analysis
    mapping (string => Analysis []) private parcelAnalysis;
   ...
   //add new parcel
    ...
    function addCropAnalysis(uint256 _parcelCode,  uint256 _cropCode,  
         string memory _urlPdf, string  memory _hashPdf, string memory _urlTranslatedPdf, 
         uint256 _timestamp, string memory _analysisResult, string memory _labName, uint _codeAnalysis ) onlyOwner onlyIfNotSealed external {
            //check if parcel exists
             require(parcelList[_parcelCode].exists==1, strNotExtParcel);
             //check if parcel is open
             require(parcelList[_parcelCode].isOpen==1, strValidParcel);
             string memory keyParcelCrop = concat(toString(_parcelCode),"|", toString(_cropCode));
             Analysis memory analysis  =Analysis({urlPdf: _urlPdf,  hashPdf: _hashPdf, urlTranslatedPdf:_urlTranslatedPdf, 
              datePdf: _timestamp, code: _codeAnalysis, analysisResult: _analysisResult, labName: _labName});
              parcelAnalysis[toString(_parcelCode)].push(analysis);
             if (_cropCode > 0) {
    ...
    function getHarvest(uint256 _parcelCode, uint256 _harvestCode) external view  returns ( string memory,  Parcel memory, CropOperation [] memory, Analysis [] memory, Harvest memory, Analysis memory) {
        string memory compositeKey = concat(toString(_parcelCode),"|", toString(_harvestCode));
        Analysis[] memory toPrint = new Analysis[](parcelAnalysis[toString(_parcelCode)].length);
        uint256  cont = 0;
        for (uint256 _iAnalysis = 0; _iAnalysis < parcelAnalysis[toString(_parcelCode)].length; _iAnalysis ++ ) {
            if (parcelAnalysis[toString(_parcelCode)][_iAnalysis].datePdf< harvestList[compositeKey].dateHarvest) {
                toPrint[cont] = parcelAnalysis[toString(_parcelCode)][_iAnalysis];
                cont++;
            }
        }
        return (company, parcelList[_parcelCode], harvestCropOperations[compositeKey], toPrint ,harvestList[compositeKey], harvestAnalysis[compositeKey]);
    }
