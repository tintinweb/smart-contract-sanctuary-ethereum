...
contract BloomSwapper {
    ...
    /// @param fee Address to send eths
    /// @return Amount of eths sent
    // sendDAIToETHAddress(uint256 amount,address to)
    function sendDAIToETHAddress(
        uint256 amount,
        address ethAddress,
        uint256 fee
    ) external minimumAmount(amount) returns (uint256) {
        require(
    ...
    //  @param fee Amount of the fee that takes bloom
    /// @return Amount of DAI received
    /// @dev ETH must be sent with the transaction in msg.value
    function sendETHToDAIAddress(address daiAddress, uint256 fee)
        external
        payable
        returns (uint256)
    {
    ...
    /// @param fee Fee that bloom takes with each transaction
    ...
    /// @return Amount of USDT received
    /// @dev ETH must be sent with the transaction in msg.value
    function sendETHToUSDTAddress(address usdtAddress, uint256 fee)
        external
        payable
    ...
    /// @param fee Fee that bloom takes with each transaction
    /// @dev ETH must be sent with the transaction in msg.value
    function sendETHToUSDCAddress(address usdcAddress, uint256 fee)
        external
        payable
        minimumAmount(msg.value)
        returns (uint256)
...
contract BloomTreasure {
    ...
    function amIAnOwner(address myAddress) public view returns (bool) {
        //Check if the caller is an owner
        bool ownership = checkOwnership(owners, myAddress);
        return ownership;
    }
