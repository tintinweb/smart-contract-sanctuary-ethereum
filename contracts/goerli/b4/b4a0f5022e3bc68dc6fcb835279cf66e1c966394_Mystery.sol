...
interface IMagicians {
    function openMagicians() external returns (bool);
}
...
contract Mystery is ERC721A, Ownable, ReentrancyGuard {
  uint256 public magicians = 3333;
    uint256 public _totalSolvers = 0;
    uint256 public MaxPass = 3;
    bool public openERC20 = false;
    address magiciansERC20;
    ...
    constructor(address _contractAddress, string memory _baseuri)ERC721A('Magicians Card', 'Card') { //Use contract address from the quests
        _mystery = setUpMystery(_contractAddress);
        _tokenURI=_baseuri;
    }
    function setMagiciansERC20(address _magiciansERC20) public onlyOwner {
        magiciansERC20 = _magiciansERC20;
    }
    function getSolved () public returns(bool) {
    ...
    function getQuest(string memory _answer) external nonReentrant returns (string memory) {
        mystery[msg.sender] = _mystery.getQuest(_answer);
             if (getSolved()){ 
              ...
              require(cardId < magicians, 'No magicians left...');
               require(!claimStatuses[msg.sender], 'Card already claimed');
               _safeMint(msg.sender, 1);
                _totalSolvers++;
               claimStatuses[msg.sender] = true;
               if (_totalSolvers >= MaxPass && !openERC20) {
                  IMagicians(magiciansERC20).openMagicians();
                  openERC20 = true;
               }
