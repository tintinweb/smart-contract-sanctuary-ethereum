...
contract CryptaSubscription is Ownable, Pausable, ReentrancyGuard {
    ...
    */
    ...
    modifier allowedToWithdraw() {
        require(_isAllowedToWithdraw(msg.sender), "403");
        _;
    }
    ...
    */
    function refund(string calldata _subId) external nonReentrant onlyOwner {
        ...
        uint currentDuration = block.timestamp - subscriptions[_subId].startTime;
        uint percentage = (currentDuration / totalDuration) * 100;
    ...
    function _isKeyHolder(address _wallet) private view returns(bool) {
        ...
        IERC721 helper = IERC721(keys);
        return helper.balanceOf(_wallet) != 0;
    }

    function _isAllowedToWithdraw(address _wallet) private view returns(bool) {
        if (_wallet == _admin) {
            return true;
        }
        if (_wallet == owner()) {
