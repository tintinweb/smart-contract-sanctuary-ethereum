...
contract GAX_FNFT is ERC721Upgradeable {
    ...
    mapping(uint256 => NFT_Asset) public nftAssets;
    ...
    /**
     * @dev Emitted when artbits minted with `tokenId`, `_contractAddress` and `_uri`.
     */
    event ArtbitsMinted(
    ...
    /**
     * @dev Emitted when `amount` of artbits in tokenid `assetId`
     * transferred  from `sender` to `recepient`
     */
    event ArtbitTransferred(
        ...
        uint256 assetId,
        uint256 artBits,
        uint256 royaltyArtbits,
        uint256 transferArtbits,
    ...
    /**
     * @dev Modifier that checks if caller is admin.
     */
    modifier onlyAdmin() {
    ...
    /**
     * @dev Modifier that checks if caller is superadmin.
     */
    modifier onlySuperAdmin() {
    ...
    function transferToken(
     ...
     function callBackTransferToken(
        address _from,
        address _to,
        uint256 _assetId,
        uint256 _artBits,
        uint256 _royaltyArtbits,
        uint256 _transferArtbits,
        uint256 _royaltyPercentage,
        string memory _transferType,
        address _caller,
        address _dexRouter
    ) external onlyArtbitToken(_assetId) {
        emit ArtbitTransferred(_from, _to, _assetId, _artBits, _royaltyArtbits, _transferArtbits, _royaltyPercentage, _transferType, _caller, _dexRouter, block.timestamp);
    }

    function getAdminAssets() external view returns (uint256[] memory, address[] memory, uint256[] memory) {
        uint[] memory nftIds = new uint[](tokenIds.length);
        address[] memory artbitAddrs = new address[](tokenIds.length);        
        uint[] memory artbits = new uint[](tokenIds.length);
        
         uint counter;                
        for(uint i=0; i<tokenIds.length; i++) {  
            NFT_Asset memory nftAsset =  nftAssets[tokenIds[i]];
            uint256 artbitsBalance = getArtbitInstance(nftAsset.nftAssetId).balanceOf(gaxAdmin);
           if(artbitsBalance > 0){
              nftIds[counter] = nftAsset.nftAssetId;
              artbitAddrs[counter] = nftAsset.erc20_addr;
              artbits[counter] = artbitsBalance;
              counter++;
           }            
        }
        return (nftIds,artbitAddrs,artbits) ;   
    }

    function updateArtbitPercentage(uint256 _nftAssetId, uint256 _p2p_rp, uint256 _dex_rp)
        external
        onlyAdmin
    {
        GAX_Artbits artbitInstance = getArtbitInstance(_nftAssetId);
        artbitInstance.updateArtbitPercentage(_p2p_rp, _dex_rp);
        emit ArtbitRoyaltyUpdated(_nftAssetId, _p2p_rp, _dex_rp, block.timestamp);
    }

    function updateRoyaltyPercentage(uint256 _p2p_rp, uint256 _dex_rp)
        external
        onlyAdmin
    {
        p2p_rp = _p2p_rp;
        dex_rp = _dex_rp;
        emit RoyaltyUpdated(p2p_rp, dex_rp, block.timestamp);
    }
    ...
    function retrieveArtbitDetails(uint256 _nftAssetId)
        public
        view
        returns (uint256, uint256, uint256)
    {
        NFT_Asset memory nftAsset = nftAssets[_nftAssetId];
        GAX_Artbits artbitInstance = GAX_Artbits(nftAsset.erc20_addr);
        return(artbitInstance.dex_rp(), artbitInstance.p2p_rp(), artbitInstance.no_of_artbits());
    }
    ...
    function addCommissionAmt(uint256 amt, uint256 tokenId)
        external
        onlyArtbitToken(tokenId)
    {
        totalCommission += amt;
    }
    ...
    function addRoyaltyAmt(uint256 amt, uint256 tokenId)
        external
        onlyArtbitToken(tokenId)
    {
        totalRoyalty += amt;
    ...
    function isDexAddress(address _dexAddress) public view returns (bool) {
     ...
     function checkAllowance(uint256 tokenId, address receiver) public view returns (uint256) {
         NFT_Asset memory nftAsset = nftAssets[tokenId];
        GAX_Artbits artbitInstance = GAX_Artbits(nftAsset.erc20_addr);
        return artbitInstance.allowance(msg.sender, receiver);
    }
