...
contract TolPass is DefaultOperatorFilterer, EIP2981RoyaltyOverrideCore, ITolPass, ERC721AntiScam, AccessControl {
    ...
    // Mint
    function mint(address _address, uint256 _amount) external payable override onlyRole(MINTER) {
        _mint(_address, _amount);
    }

    // Burn
    function burn(uint256 _tokenId) external payable onlyRole(BURNER) {
        _burn(_tokenId);
    }
...
interface ITolPass {
    function mint(address _address, uint256 _amount) payable external;
    function burn(uint256 _tokenId) payable external;
    function isTokenOwner(address _owner, uint256 _tokenId) view external returns (bool);
