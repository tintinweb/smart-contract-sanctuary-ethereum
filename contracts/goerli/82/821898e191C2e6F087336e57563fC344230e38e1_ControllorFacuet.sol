...
contract ControllorFacuet {
    ...
    function controllorCreateSignetor() external {
        ...
        LibSignetStorage.controllorCreateSignetor(
            msg.sender,
            newContractAddress
        );
    ...
    function getOwnerContractForSignetor(
    ...
    function getOwnerNumContractOfSignetor(
        address contractOwner
...
library LibSignetStorage {
    ...
    function findPfpId(string memory _pfp) internal view returns (uint256 id) {
        ...
        for (uint256 i = 0; i < s.ps.totalpfp + 1; i++) {
            if (
                keccak256(abi.encodePacked(s.pm.pfp[i].pfp)) ==
                keccak256(abi.encodePacked(_pfp))
            ) {
                return (i);
            }
        }
    ...
    function unfollow(address msgSender, address signetor) internal {
        ...
        if (totalFollower == i) {
            ...
            s.fm.follower[signetor].followerNum -= 1;
            s.fm.follower[signetor].whoFollowed[i - 1] = s
                .fm
                .follower[signetor]
                .whoFollowed[s.fm.follower[signetor].whoFollowed.length - 1];
            s.fm.follower[signetor].whoFollowed.pop();
        }
    ...
    function checklikeable(
        ...
        if (SignetId <= s.fss.signetId) {
            if (s.fss.signetId != 0) {
                if (SignetId != 0) {
                    if (
                        s.fm.signetState[SignetId].SignetIdOwner ==
                        SignetIdOwner
                    ) {
                        return (true);
                    }
    ...
    function checkliked(
        ...
        for (
            i = 0;
            i < s.fm.signetState[signetID].likeContributors.length;
            i++
        ) {
            if (
                s.fm.signetState[signetID].likeContributors[i] == likedAddress
            ) {
                return (true);
            }
        }
    ...
    function findfollowerId(
        ...
        for (i = 1; i < s.fm.follower[signetor].whoFollowed.length + 1; i++) {
            if (
                s.fm.follower[signetor].whoFollowed[i - 1] == followersaddress
            ) {
                return i;
            }
        }
    ...
    function findFollwingId(
        ...
        for (i = 1; i < s.fm.following[signetor].followedWho.length + 1; i++) {
            if (
                s.fm.following[signetor].followedWho[i - 1] == followingAaddress
            ) {
                return i;
            }
        }
    ...
    function findLikeId(
        ...
        for (
            i = 1;
            i < s.fm.signetState[signetID].likeContributors.length + 1;
            i++
        ) {
            if (
                s.fm.signetState[signetID].likeContributors[i - 1] ==
                likedAddress
            ) {
                return i;
            }
    ...
    function getStaredNumForSignetor(
