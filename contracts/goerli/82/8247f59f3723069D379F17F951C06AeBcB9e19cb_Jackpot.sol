...
interface IMonoWallet {
    function sendTokenToJackpotContract(uint256 _amount, address _tokenAddress) external ;
}
...
import ".././Interface/IMonoWallet.sol";
import ".././Library/UUPSUpgradeable.sol";
import ".././Library/Initializable.sol";


contract Jackpot is Initializable, VerifyInitializable, UUPSUpgradeable{
    mapping(uint256 => Jackpot) public jackpots;
    mapping(uint256 => User[]) public users;
    mapping(string => bool) private verifyMessage;
    address public monoWallet;

    event CreateJackpot(uint256 jackpotId, uint256 amount, address addressToken);

    event UpdateWinner(uint256 jackpotId, uint256 amount, uint256 userIdWinner, address addressWinner);

    event ClaimToken(address caller, uint256 amount, uint256 jackpotId, uint256 userId);

    event Test(address caller, uint256 jackpotId, uint256 userId);
    ...
    function createJackpot(
        ...
        IMonoWallet(monoWallet).sendTokenToJackpotContract(
            _amount,
            _addressToken
        );
        ...
        emit CreateJackpot(_jackpotId, _amount, _addressToken);
    }
    ...
    function updateWinner(
        ...
        emit UpdateWinner(jackpotId, _amount, _userIdWinner, users[jackpotId][indexWinner].userAddress);
    }
    ...
    function setMonoWallet(address _monoWaletAddress) public onlyOperator {
        require(_monoWaletAddress != address(0), "INVALID ADMIN ADDRESS.");
        monoWallet = _monoWaletAddress;
    }
