...
contract NYXSoulmateAI is ERC721Enumerable, Ownable, ReentrancyGuard {
    ...
    uint256 public PUBLIC_MINT_COST = 88000000000000000; // 0.088 ETH
    uint256 public ALLOWLIST_MINT_COST = 70000000000000000; // 0.07 ETH
    uint256 public TMP_MAX_SUPPLY = 5000; // can never be higher than MAX_SOUL_SUPPLY
    ...
    function mint(uint256 count) external payable nonReentrant {
        ...
        if (hasFreeMint[msg.sender]) {
            require(
                mintedCount[msg.sender] + count <= MAX_PER_WALLET + 1,
                "Exceeds your allowance"
            );
        } else {
            require(
                mintedCount[msg.sender] + count <= MAX_PER_WALLET,
                "Exceeds your allowance"
            );
        }
        ...
        require(supply + count <= TMP_MAX_SUPPLY, "Max supply reached");

        require(
            PUBLIC_MINT_COST * count == msg.value,
    ...
    function allowListMint(
        ...
        require(
            ALLOWLIST_MINT_COST * payedCount == msg.value,
            "Invalid funds provided"
        );
    ...
    function setTmpMaxSupply(uint256 maxSupply) external onlyOwner {
        ...
        require(
            maxSupply > TMP_MAX_SUPPLY,
            "New max supply must be higher than TMP_MAX_SUPPLY"
        );
        TMP_MAX_SUPPLY = maxSupply;
    }

    function setPublicMintCost(uint256 mintCost) external onlyOwner {
        PUBLIC_MINT_COST = mintCost;
    }

    function setAllowListMintCost(uint256 mintCost) external onlyOwner {
        ALLOWLIST_MINT_COST = mintCost;
    }
