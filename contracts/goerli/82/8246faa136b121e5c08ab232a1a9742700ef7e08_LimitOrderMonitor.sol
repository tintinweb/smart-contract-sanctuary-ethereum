...
contract LimitOrderMonitor is
    ...
    function initialize (IOrderManager _orderManager,
        ...
        require(ERGO.approve(address(orderManager), MAX_INT));
    }
    ...
    function _transferFees(uint256 _amount, address _owner) internal virtual {
        if (_amount > 0) {
            TransferHelper.safeTransfer(address(ERGO), _owner, _amount);
        }
    }
...
interface IOrderManager {
    ...
    function quoteERGO(uint256 weiAmount) external returns (uint256 quote);
