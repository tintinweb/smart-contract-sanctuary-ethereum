...
contract MockBuyModule is Module, IBuyEvents, IBuyActions {
...
contract MockShipDeployer is IDeployerEvents, IDeployerActions {
        ...
        returns (SetupReturnVars memory rv)
    {
        ...
        rv.buyModuleAddress = address(
            new MockBuyModule(rv.safeAddress, rv.safeAddress, SEAPORT)
        );
        ...
        GnosisSafe(payable(rv.safeAddress)).setup(
            ...
            abi.encodeCall(
                this.initSafe,
                (rv.listModuleAddress, rv.buyModuleAddress)
            ), // data
    ...
    function initSafe(address listModule, address buyModule) external {
        ModuleManager(address(this)).enableModule(listModule);
        ModuleManager(address(this)).enableModule(buyModule);
    }
}
