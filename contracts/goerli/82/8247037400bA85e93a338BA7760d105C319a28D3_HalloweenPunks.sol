    ...
    Ownable
{
    ...
    function publicAuthMint(uint256 qty, bytes calldata signature) external payable {
    ...
    function adminMint(uint256 qty, address to) external onlyOwner {
        if (totalSupply() + qty > maxSupply) revert InvalidQuantity();
        if (teamMinted + qty > teamSupply) revert InvalidQuantity();
        _mint(to, qty);
    }

    function setBlueChipContracts(
        BlueChip[] calldata _blueChips,
