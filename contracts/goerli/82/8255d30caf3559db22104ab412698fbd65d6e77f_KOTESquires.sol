...
contract KOTESquires is ERC721Enumerable, Ownable {
	...
	constructor() ERC721("KOTESQUIRES", "KOTESQUIRES") {
		setBaseURI(baseURI);
		}

	  function airdropSquire(address[] calldata addr, uint squireType, uint genesis, uint setStength, uint setWisdom, uint setLuck, uint setFaith) external onlyOwner { 
	...
	//returns upgrade Amount
	function upgradeAmountByTokenId(uint256 tokenId) public view returns (string memory) {
	  require(_exists(tokenId), "Token has not been minted yet");
   	  return tokenIdUpgradeAmount[tokenId];
 	 } 
