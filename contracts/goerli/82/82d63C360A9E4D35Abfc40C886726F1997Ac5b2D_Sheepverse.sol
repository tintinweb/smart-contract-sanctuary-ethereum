...
contract Sheepverse is ERC721A, Ownable, ReentrancyGuard {
    ...
    MintPhaseConfig[] public _mintPhases;
    ...
    // The root hash of the whitelist merkle tree
    // Make sure to prepend 0x
    bytes32 private _whitelistMerkleRoot = 0x9d97282d96ff0057fec2757f09642f3c50d88b5572042d6f0843bd142029c4de;
    mapping(address => bool) _freeMint;

    // Token URI (reveal)
    mapping(uint256 => string) _baseTokenURIs;
    string constant private _unrevealedUri = 'ipfs://QmZCGmeQT8w7YPQKPgHsxwSNmwWuXKRkFHByGs5tz9ubPB/reveal.json';

    /**
     * Modifier | Only resolves if the sale has started
     */
    modifier onlyIfSale() {
        require(block.timestamp > SALE_START - 1, 'Sale has not started yet');
        _;
    }

    /**
     * Modifier | Only resolves if the whitelist sale has started
     */
    modifier onlyIfWhitelistSale() {
        require(block.timestamp > WHITELIST_SALE_START - 1, 'Whitelist Sale has not started yet');
        require(block.timestamp < SALE_START + 1, 'Public Sale has started');
        _;
    }
     ...
     * Constructor
     */
    constructor(string memory name, string memory symbol, uint256 saleStart, uint256 whitelistSaleStart, address receiver) ERC721A(name, symbol) {
    ...
    function addFreeMintAddress(address[] calldata addresses) external onlyOwner {
        for (uint i = 0; i < addresses.length; i++) {
            _freeMint[addresses[i]] = true;
        }
    }

    // Use custom function for mint require to prevent
    // code duplication. We don't use modifiers to not bloat
    // the smart contract
    ...
    function getMintPhase(uint256 tokenId) public view returns(MintPhaseConfig memory) {
        require(_mintPhases.length > 0, 'No MintPhases defined');

        for (uint256 i = 0; i < _mintPhases.length; i++) {
            // Add +1 to amount of mintPhase to prevent usage of <=
            if (tokenId < _mintPhases[i].amount + 1) {
                return _mintPhases[i];
            }
        }

        return _mintPhases[_mintPhases.length - 1];
    }

    function addMintPhase(uint256 amount, uint256 tokenPrice) external onlyOwner {
        if (_mintPhases.length != 0) {
            MintPhaseConfig memory lastEl = _mintPhases[_mintPhases.length - 1];
            amount = lastEl.amount + amount;
        }
        _mintPhases.push(MintPhaseConfig(amount, tokenPrice, false, ''));
    }

    function revealMintPhase(uint256 mintPhaseIndex, string calldata baseTokenURI) external onlyOwner {
        require(_mintPhases[mintPhaseIndex].amount > 0, 'MintPhases not found');
        _mintPhases[mintPhaseIndex].baseTokenURI = baseTokenURI;
        _mintPhases[mintPhaseIndex].revealed = true;
    }
