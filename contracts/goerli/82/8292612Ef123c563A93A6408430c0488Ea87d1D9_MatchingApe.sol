...
contract ApeCoinStaking is Ownable {
    ...
    function claimApeCoin(address _recipient) public {
        ...
        uint256 rewardsToBeClaimed = _claim(
            APECOIN_POOL_ID,
            position,
            _recipient
        );
    ...
    function addTimeRange(
        ...
        if (_startTimestamp >= _endTimeStamp)
            revert StartMustBeGreaterThanEnd();
        ...
        if (length > 0) {
            if (_startTimestamp != pool.timeRanges[length - 1].endTimestampHour)
    ...
    function getTimeRangeBy(
        uint256 _poolId,
        uint256 _index
    ) public view returns (TimeRange memory) {
    ...
    function rewardsBy(
        ...
        if (_to < pool.timeRanges[0].startTimestampHour)
        ...
        for (uint256 i = currentIndex; i < length; ) {
            ...
            startTimestampHour = _from <= current.startTimestampHour
                ? current.startTimestampHour
                : _from;
            endTimestampHour = _to <= current.endTimestampHour
                ? _to
                : current.endTimestampHour;

            rewards =
                rewards +
                ((endTimestampHour - startTimestampHour) *
                    current.rewardsPerHour) /
                SECONDS_PER_HOUR;
    ...
    function updatePool(uint256 _poolId) public {
        ...
        uint48 lastTimestampHour = pool
            .timeRanges[pool.timeRanges.length - 1]
            .endTimestampHour;
        uint48 previousTimestampHour = getPreviousTimestampHour().toUint48();

        if (pool.stakedAmount == 0) {
            pool.lastRewardedTimestampHour = previousTimestampHour >
                lastTimestampHour
                ? lastTimestampHour
        ...
        (uint256 rewards, uint256 index) = rewardsBy(
            _poolId,
    ...
    function getCurrentTimeRangeIndex(
        ...
        if (block.timestamp < pool.timeRanges[current].startTimestampHour)
            return current;
        for (
            current = pool.lastRewardsRangeIndex;
            current < pool.timeRanges.length;
            ++current
        ) {
    ...
    function getPoolsUI()
        public
        view
        returns (PoolUI memory, PoolUI memory, PoolUI memory, PoolUI memory)
    {
        ...
        return (
            PoolUI(
                0,
                apeCoinPool.stakedAmount,
                apeCoinPool.timeRanges[current]
            ),
    ...
    function _stakedTotal(
        ...
        for (uint256 i = 0; i < nftCount; ++i) {
            uint256 tokenId = nftContracts[_poolId].tokenOfOwnerByIndex(
                _addr,
                i
            );
    ...
    function _stakedTotalPair(address _addr) private view returns (uint256) {
        ...
        for (uint256 i = 0; i < nftCount; ++i) {
            uint256 baycTokenId = nftContracts[BAYC_POOL_ID]
                .tokenOfOwnerByIndex(_addr, i);
            if (mainToBakc[BAYC_POOL_ID][baycTokenId].isPaired) {
                uint256 bakcTokenId = mainToBakc[BAYC_POOL_ID][baycTokenId]
                    .tokenId;
        ...
        for (uint256 i = 0; i < nftCount; ++i) {
            uint256 maycTokenId = nftContracts[MAYC_POOL_ID]
                .tokenOfOwnerByIndex(_addr, i);
            if (mainToBakc[MAYC_POOL_ID][maycTokenId].isPaired) {
                uint256 bakcTokenId = mainToBakc[MAYC_POOL_ID][maycTokenId]
                    .tokenId;
                total += nftPosition[BAKC_POOL_ID][bakcTokenId].stakedAmount;
            }
        }
    ...
    function getAllStakes(
        address _address
    ) public view returns (DashboardStake[] memory) {
        ...
        uint256 count = (baycStakes.length +
            maycStakes.length +
            bakcStakes.length +
            splitStakes.length +
            1);
    ...
    function getSplitStakes(
        ...
        DashboardStake[] memory baycSplitStakes = _getSplitStakes(
            baycSplits,
            _address,
            BAYC_POOL_ID
        );
        DashboardStake[] memory maycSplitStakes = _getSplitStakes(
            maycSplits,
            _address,
            MAYC_POOL_ID
        );
    ...
    function _getSplitStakes(
        ...
        for (
            uint256 i = 0;
            i < nftContracts[_mainPoolId].balanceOf(_address);
            ++i
        ) {
            uint256 mainTokenId = nftContracts[_mainPoolId].tokenOfOwnerByIndex(
                _address,
                i
            );
            if (mainToBakc[_mainPoolId][mainTokenId].isPaired) {
                uint256 bakcTokenId = mainToBakc[_mainPoolId][mainTokenId]
                    .tokenId;
                address currentOwner = nftContracts[BAKC_POOL_ID].ownerOf(
                    bakcTokenId
                );

                /* Split Pair Check*/
                if (currentOwner != _address) {
                    uint256 deposited = nftPosition[BAKC_POOL_ID][bakcTokenId]
                        .stakedAmount;
                    uint256 unclaimed = deposited > 0
                        ? this.pendingRewards(
                            BAKC_POOL_ID,
    ...
    function _getStakes(
        ...
        for (uint256 i = 0; i < nftCount; ++i) {
            ...
            uint256 unclaimed = deposited > 0
                ? this.pendingRewards(_poolId, _address, tokenId)
                : 0;
            uint256 rewards24Hrs = deposited > 0
                ? _estimate24HourRewards(_poolId, _address, tokenId)
                : 0;
            ...
            if (_poolId == BAKC_POOL_ID) {
                if (bakcToMain[tokenId][BAYC_POOL_ID].isPaired) {
                    pair = DashboardPair(
                        bakcToMain[tokenId][BAYC_POOL_ID].tokenId,
                        BAYC_POOL_ID
                    );
    ...
    function _estimate24HourRewards(
        ...
        Pool memory pool = pools[_poolId];
        Position memory position = _poolId == 0
            ? addressPosition[_address]
            : nftPosition[_poolId][_tokenId];

        TimeRange memory rewards = getTimeRangeBy(
            _poolId,
            pool.lastRewardsRangeIndex
        );
        return
            (position.stakedAmount * uint256(rewards.rewardsPerHour) * 24) /
            uint256(pool.stakedAmount);
    }
    ...
    function pendingRewards(
        ...
        if (
            block.timestamp >
            pool.lastRewardedTimestampHour + SECONDS_PER_HOUR &&
            pool.stakedAmount != 0
        ) {
        ...
        return
            ((position.stakedAmount * accumulatedRewardsPerShare).toInt256() -
    ...
    function getPreviousTimestampHour() internal view returns (uint256) {
        return
            block.timestamp -
            (getMinute(block.timestamp) * 60 + getSecond(block.timestamp));
    }
    ...
    function _deposit(
        ...
        _position.rewardsDebt += (_amount * pool.accumulatedRewardsPerShare)
            .toInt256();
    }
    ...
    function _depositPairNft(
        uint256 mainTypePoolId,
        PairNftDepositWithAmount[] calldata _nfts
    ) private {
        uint256 length = _nfts.length;
        uint256 totalDeposit;
        ...
        for (uint256 i; i < length; ) {
            ...
            if (position.stakedAmount == 0) {
                ...
                mainToBakc[mainTypePoolId][pair.mainTokenId] = PairingStatus(
                    pair.bakcTokenId,
                    true
                );
                bakcToMain[pair.bakcTokenId][mainTypePoolId] = PairingStatus(
                    pair.mainTokenId,
                    true
                );
            } else if (
    ...
    function _claim(
        ...
        int256 accumulatedApeCoins = (_position.stakedAmount *
            uint256(pool.accumulatedRewardsPerShare)).toInt256();
        rewardsToBeClaimed =
            (accumulatedApeCoins - _position.rewardsDebt).toUint256() /
            APE_COIN_PRECISION;
    ...
    function _claimPairNft(
        ...
        for (uint256 i; i < length; ) {
            ...
            if (nftContracts[mainTypePoolId].ownerOf(mainTokenId) != msg.sender)
                revert NotOwnerOfMain();

            bakcTokenId = _pairs[i].bakcTokenId;
            if (nftContracts[BAKC_POOL_ID].ownerOf(bakcTokenId) != msg.sender)
    ...
    function _withdrawPairNft(
        uint256 mainTypePoolId,
        PairNftWithdrawWithAmount[] calldata _nfts
    ) private {
        ...
        for (uint256 i; i < length; ) {
            pair = _nfts[i];
            mainTokenOwner = nftContracts[mainTypePoolId].ownerOf(
                pair.mainTokenId
            );
            bakcOwner = nftContracts[BAKC_POOL_ID].ownerOf(pair.bakcTokenId);

            if (mainTokenOwner != msg.sender) {
                if (bakcOwner != msg.sender)
                    revert NeitherTokenInPairOwnedByCaller();
            }
            ...
            uint256 finalAmountToWithdraw = pair.isUncommit
...
contract MatchingApe is IERC721Receiver {
    // declare the interfaces for the APE token, NFTs, and staking contract
    IERC20 public apeToken;
    IERC721 public baycToken;
    IERC721 public maycToken;
    IERC721 public bakcToken;
    ApeCoinStaking public apeStaking;
    ...
    // create a struct to be used to pass as argument when calling functions from ApeCoinStaking contract
    // some of those functions require a SingleNft struct argument for depositing and withdrawing
    struct SingleNft {
        uint256 tokenId;
        uint256 amount;
    }
    ...
    constructor() {
        ...
        apeStaking = ApeCoinStaking(0xa6d86470da4709d2FF38521AaE18c47B4162311F);
    }
    ...
    function depositNft(
        ...
        if (nftType == NftType.BAYC) {
            ...
            // BAKC deposit not implemented yet
    ...
    function fulfillNftDeposit(
        ...
        uint32 tokenId = request.tokenId;

        // SingleNft[] memory nftArray = new SingleNft[](1);
        // nftArray[0] = SingleNft(tokenId, apeAmount);

        ApeCoinStaking.SingleNft[]
            memory nftArray = new ApeCoinStaking.SingleNft[](1);
        nftArray[0] = ApeCoinStaking.SingleNft(tokenId, apeAmount);

        if (request.nftType == NftType.BAYC) {
            apeStaking.depositBAYC(nftArray); // depositBAYC commits 1 or more BAYC NFTs, each with an ApeCoin amount to the BAYC pool
        } else if (request.nftType == NftType.MAYC) {
            apeStaking.depositMAYC(nftArray);
        } else if (request.nftType == NftType.BAKC) {
            // BAKC deposit not implemented yet
            // apeStaking.depositBAKC(nftArray);
        }
    ...
    function claimRewardsAndUnstake(uint256 depositRequestId) external {
        ...
        // uint256 rewards = apeStaking.pendingRewards(
        //     request.poolId,
        //     address(this)
        // );
        ...
        // if (request.poolId == BAYC_POOL_ID) {
        //     apeStaking.withdrawBAYC(nfts, address(this));
        // } else if (request.poolId == MAYC_POOL_ID) {
        //     apeStaking.withdrawMAYC(nfts, address(this));
        // } else if (request.poolId == BAKC_POOL_ID) {
