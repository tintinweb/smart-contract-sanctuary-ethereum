...
contract Whitelist is Ownable, EIP712 {
    bytes32 public constant WHITELIST_TYPEHASH =
        keccak256("Whitelist(uint256 amountIn, uint256 amountOut)"); //, address tokenIn, address tokenOut, string totalAmountInEth, string totalAmountOutEth, bytes32 swapId, uint256 timeout)");
...
contract Swapsy is ReentrancyGuard, Ownable, Whitelist {
    ...
    ) public payable nonReentrant isSenderWhitelisted(amountIn, amountOut,/* tokenIn, tokenOut, totalAmountInEth, totalAmountOutEth, swapId, timeout,*/ _signature)
    {
        ...
        _sell(msg.sender, amountIn, amountOut, address(0), tokenOut, _totalAmountInEth, _totalAmountOutEth, sellerFeeAmt, swapId, timeout, deadline);
    }
    ...
    ) public payable nonReentrant isSenderWhitelisted(amountIn, amountOut, /* tokenIn, tokenOut, totalAmountInEth, totalAmountOutEth, swapId, timeout, */_signature)
    {
        ...
        _sell(msg.sender, amountIn, amountOut, tokenIn, tokenOut, _totalAmountInEth, _totalAmountOutEth, sellerFeeAmt, swapId, timeout, deadline);
    }
