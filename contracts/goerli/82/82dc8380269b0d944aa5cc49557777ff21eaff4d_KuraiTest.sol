...
contract KuraiTest is Ownable, ERC721A, ReentrancyGuard, DefaultOperatorFilterer {
  using Strings for uint256;
  ...
  uint256 public HOLDERS_ALLOWED_TO_MINT = 1; // maximam amount that user can mint in Free holder


  uint256 public PRICE = 0.01 ether; 
  uint256 public PRICE_FOR_HOLDERS = 0 ether;
  ...
  mapping(address => uint256) public holdersMintClaimedBy;

  constructor() ERC721A("Kurai Test","KRT") {
  ...
  function mint(uint256 quantity) external payable callerIsUser {
   ...
   function mintForHolders(uint256 quantity) external payable callerIsUser {
    require(status == 3 , "Sale is not Active");
    require(totalSupply() + quantity <= TotalCollectionSize_, "reached max supply");
    require(balanceOf(msg.sender) >= 1, "You are not a Holder");
    require(quantity <= HOLDERS_ALLOWED_TO_MINT,"can not mint this many");
    require(msg.value >= PRICE_FOR_HOLDERS * quantity, "Need to send more ETH.");

    holdersMintClaimedBy[msg.sender] += quantity;
    require(holdersMintClaimedBy[msg.sender] <= HOLDERS_ALLOWED_TO_MINT, "Purchase exceeds max allowed");
  ...
  function changeMintPrice(uint256 _newPrice) external onlyOwner
  {
   ...
   function changeHoldersMintPrice(uint256 _newPrice) external onlyOwner
  {
      PRICE_FOR_HOLDERS = _newPrice;
  }
     ...
     function changeMAX_PER_WALLET(uint256 q) external onlyOwner
  {
    ...
    function change_HOLDERS_ALLOWED_TO_MINT(uint256 q) external onlyOwner
  {
      HOLDERS_ALLOWED_TO_MINT = q;
  }
