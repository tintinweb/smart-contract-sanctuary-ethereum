...
contract CentBaseMarketPlaceBETA is CentBaseTimedAuctionBETA, ReentrancyGuard {
    ...
    function createMarketItem(address nftContract, uint tokenId, address payable seller) external onlyRole(ITEM_CREATOR_ROLE) returns (bytes32 itemId) {
        if (seller != IERC721(nftContract).ownerOf(tokenId)) revert NotTokenOwner();

        uint _index = _itemsIndex.current();
        uint _salt = block.timestamp;
    ...
    /// @param one Requires the param to be uint value 1.
    /// @param orderId The id of the MarketOrder to purchase.
    function executeOrder(uint one, bytes32 orderId) 
        external 
        payable 
        isActiveOrder(orderId) nonReentrant 
        costs(one, orderId) 
    {
        if (one != 1) revert ErrorMessage("Requires uint value: 1");
        ...
        (, uint _toSellerAmount, uint _totalFeeAmount) = _calculateFees(_order.priceInWei);
        ...
        IERC721(_item.nftContract).safeTransferFrom(address(this), _msgSender(), _item.tokenId);
...
/// --------------------------------- PRIVATE METHODS ---------------------------------
    ...
    function _itemRemove(bytes32 itemId) private {
        if (itemsMapping[itemId].active) {
            revert IsActive(itemId, itemsMapping[itemId].status);
        }
    ...
    function _orderRemove(bytes32 orderId) private {
        if (ordersMapping[orderId].isOrder) {
            revert ActiveOrder(orderId);
        }
...
/// --------------------------------- ADMIN METHODS ---------------------------------
    ...
    function releaseStuckTokens(address erc20Token) external onlyRole(ADMIN_ROLE) {
        uint balance = IERC20(erc20Token).balanceOf(address(this));
...
contract CentBaseStorageBETA is Context, AccessControlEnumerable {
    ...
    /// @notice Maps token address to amount in pendingReturns.
...
contract CentBaseTimedAuctionBETA is CentBaseStorageBETA {
        ...
        minBid(auctionId)
    {
        ...
        if (two != 2) revert ErrorMessage("Requires uint value: 2");
    ...
    function isHodler(address account, address collection) external view returns (uint balance){
    ...
    function balanceInEscrow(address payable seller)external view  isAuth(seller) returns (uint256 balance) {
    ...
    function _transferRoyaltiesAndServiceFee(
        ...
        uint _totalFeeAmount, 
        uint _toSellerAmount
    ) 
        internal 
    {

        ( address _royaltyReceiver, uint _royaltyAmount ) = 
            IERC2981(_item.nftContract)
                .royaltyInfo(_item.tokenId, _toSellerAmount);

        uint _toEscrow = (_toSellerAmount - _royaltyAmount); 
        ...
        if (!payable(_royaltyReceiver).send(_royaltyAmount)) 
            revert FailedTransaction("Royalty");
    ...
    /// @notice Internal method used to the nft asset to the new token Owner.
    ...
    function _sendAsset(bytes32 itemId, address receiver) internal {
        ...
        emit AssetSent(receiver, _item.nftContract, _item.tokenId);
    }
