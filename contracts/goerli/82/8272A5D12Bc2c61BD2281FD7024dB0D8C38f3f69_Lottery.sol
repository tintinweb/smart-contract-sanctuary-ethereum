...
contract Lottery is LotteryBase, PullPayment {
    constructor(
        ...
        if (vrfCoordinatorAddress_ == address(0)) revert InvalidAddress(vrfCoordinatorAddress_);
    ...
    receive() external payable override nonReentrant whenNotPaused {
        if (!_isReadyToAcceptPlayer(msg.sender)) revert InvalidState("lottery-not-ready-for-enter");
...
abstract contract LotteryBase is Ownable, Pausable, ReentrancyGuard, VRFConsumerBaseV2, KeeperCompatibleInterface {
    ...
    fallback() external payable nonReentrant whenNotPaused { revert InvalidState("fallback-not-allowed"); }
    ...
    function failSafeMode() external nonReentrant onlyOwner {
        lotteryState = LotteryState.FailSafeMode;
    }

    function close() external nonReentrant onlyOwner {
        if (lotteryState == LotteryState.Closed) revert InvalidState("already-closed");
        _close();
    }
    ...
    function initialize(
        ...
        if (!_isReadyForChangeSettings()) revert InvalidState("not-ready-for-change-settings");
        if (feeDestination_ == address(0)) revert InvalidAddress(feeDestination_);
        if (charityDestination_ == address(0)) revert InvalidAddress(feeDestination_);
    ...
    function _open() internal {
        if (!_isReadyForOpen()) revert InvalidState("not-ready-for-open");
    ...
    function _enter(address player_) internal {
        ...
        deposits[player_] += ticketPrice;
        balances[feeDestination] += fee;
        balances[charityDestination] += charity;
    ...
    function fulfillRandomWords(uint256 requestId, uint256[] memory randomWords) internal override nonReentrant {
        if (lotteryState != LotteryState.Draw) revert InvalidState("not-in-draw-state");
    ...
    function _draw() private {
        if (!(_isReadyForDraw())) revert InvalidState("not-ready-for-draw");
    ...
    function _preparePayouts(uint256 startIndex) internal view returns (address[] memory, uint256[] memory) {
        ...
        uint256 winnerPayout = (jackpot * winnerPayoutPercentage) / 100;
    ...
    function _payouts(address[] memory _addresses, uint256[] memory _balances) internal {
        if (!(_isReadyForPayouts())) revert InvalidState("not-ready-for-payouts");
