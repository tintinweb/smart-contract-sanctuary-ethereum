...
import "./interfaces/IPioneersTrails.sol";
...
contract Pioneers is IERC721, IERC721Metadata {
	...
	//GAME VARS
	uint256 constant MAX_WAGONS = 38611; //from table

	uint256 constant MINT_COST = 0.000001 ether; //TODO:

	uint256 constant MINT_PERCENT_PRIZE = 85; //TODO:
	uint256 constant MINT_PERCENT_LOOT = 1; //TODO:
	uint256 constant MINT_PERCENT_CREATOR = 14;

	uint256 constant SUPPLY_PERCENT_PRIZE = 90;
	uint256 constant SUPPLY_PERCENT_CREATOR = 10;

	uint256 constant CHANGE_TRAIL_BASE_PRICE = 0.02 ether; //TODO:
	uint256 constant CHANGE_TRAIL_PERCENT_PRIZE = 90;
	uint256 constant CHANGE_TRAIL_PERCENT_CREATOR = 10;

	uint256 public startTime;
	uint256 SALE_TIME = 10 seconds;
	// uint EARLY_ACCESS_TIME = 2 days;

	uint256 public boughtSupplies;
	uint256 public changedTrails;
	uint256 public destroyed;
	uint256 public settledFunds;
	...
	Counters.Counter private _wagonId;
	mapping(uint16 => uint256) public wagonToToken;
	mapping(uint16 => uint8[2]) items;
	mapping(uint16 => bytes1) wagonToFirstLetter;
	mapping(uint256 => address) owners;

	mapping(uint256 => bytes32) structuralData;
	mapping(uint256 => uint8) tokenToTrail; /** Associa un tokenId al percorso su cui viaggia. */
	mapping(address => uint256) lastAmbushes; /** Associa un account al block.number nel momento dell'ultimo Ambush effettuato. */

	function getStructuralData(uint256 _tokenId)
		public
		view
		returns (
			uint8 supplies,
		...
		)
	{
		...
		supplies = uint8(uint256(((_data << 248) >> 248)));
		ambushes = uint8(uint256(((_data << 240) >> 240) >> 8));
		lastAmbush = (_data >> 16);

		return (supplies, ambushes, lastAmbush);
	}

	function setStructuralData(
		uint256 _tokenId,
		uint8 supplies,
		uint8 ambushes,
		bytes32 lastAmbush
	) internal {
		bytes32 _supplies = bytes32(uint256(supplies));
		bytes32 _ambushes = bytes32(uint256(ambushes)) << 8;
	...
	function tokenToWagon(uint256 _tokenId) public view returns (uint16) {
	...
	function stage() public view returns (Stage) {
		// TODO: Enable block.timestamp check
		if (startTime == 0) {
			return Stage.Initial;
		} else if (
			!_startJourney /*&& block.timestamp < startTime + SALE_TIME && tokenIndexToWagon.length < MAX_WAGONS*/
		) {
			return Stage.Minting;
		} else if (
			totalSupply() > ((3 * tokenIndexToWagon.length) / 4) &&
			totalSupply() <= tokenIndexToWagon.length
		) {
			return Stage.Journey1;
		} else if (
			totalSupply() > tokenIndexToWagon.length / 2 &&
			totalSupply() <= ((3 * tokenIndexToWagon.length) / 4)
		) {
			return Stage.Journey2;
		} else if (
			totalSupply() > tokenIndexToWagon.length / 4 &&
			totalSupply() <= tokenIndexToWagon.length / 2
		) {
			return Stage.Journey3;
		} else if (
			totalSupply() > 1 && totalSupply() <= tokenIndexToWagon.length / 4
		) {
	...
	function steer(
		...
		bytes32 leaf = keccak256(
			abi.encodePacked(_wagonId.current(), _items[0], _items[1], firstLetter)
		);
		...
		require(
			wagonToToken[uint16(_wagonId.current())] == 0 &&
				items[uint16(_wagonId.current())][0] == 0 &&
				items[uint16( _wagonId.current())][1] == 0,
			"steered"
		);
		...
		items[uint16(_wagonId.current())] = _items;
		tokenIndexToWagon.push(uint16(_wagonId.current()));
		
		uint256 _tokenId = tokenIndexToWagon.length;
		balances[msg.sender]++;
		owners[_tokenId] = msg.sender;
		wagonToToken[uint16(_wagonId.current())] = _tokenId;
		tokenToTrail[_tokenId] = _trail;
		wagonToFirstLetter[uint16(_wagonId.current())] = firstLetter;

		emit Steer(uint16(_wagonId.current()), _tokenId);
		emit Transfer(address(0), msg.sender, _tokenId);
	}

	function ambush(uint256 _ambusherTokenId, uint256 _victimTokenId) public {
		...
		(
			...
			uint8 _ambushes,
			bytes32 _lastAmbush
		) = getStructuralData(_victimTokenId);

		//  covered by isValidToken
		//      require(_damage <= _reinforcement,"eliminated" );

		require(
			tokenToTrail[_victimTokenId] == tokenToTrail[_ambusherTokenId],
			"sameTrail"
		);

		require(checkVulnerable(_victimTokenId, _lastAmbush), "vulnerable");

		(, , bytes32 _inconvenienceId) = IPioneersTrails(trails)
			.currentInconveniences(tokenToTrail[_victimTokenId]);
	...
	function changeTrail(uint256 _tokenId, uint8 _newTrailId) public payable {
		Stage _stage = stage();

		require(
			_stage == Stage.Journey1 ||
				_stage == Stage.Journey2 ||
				_stage == Stage.Journey3,
			"stage"
		);
		...
		changedTrails +=
			msg.value -
			((CHANGE_TRAIL_BASE_PRICE * CHANGE_TRAIL_BASE_PRICE) / 100);

		emit Trail(_tokenId, _newTrailId);
	}

	function buySupply(uint256 _tokenId) public payable {
		Stage _stage = stage();
	...
	function settleDown(uint256 _tokenId) public {
		Stage _stage = stage();
		bool isInJourney = isInJourneyStage();
		require(isInJourney || _stage == Stage.Minting, "stage");
		...
		if (isInJourney) require(!isVulnerable(_tokenId), "vulnerable");

		uint256 wagonCount = tokenIndexToWagon.length;

		uint256 fromPool = ((((MINT_COST * wagonCount * MINT_PERCENT_PRIZE) / //Winner fee from mints less evacuated funds
				100 -
				settledFunds) / totalSupply()) *
				(10000000000 + ((29000000000 * destroyed) / wagonCount))) /
				//Divided by remaining tokens
				//Multiplied by (3+(0.9 * (destroyed / cities)) /4
				40000000000;

		//Also give them the admin fee
		uint256 toWithdraw = fromPool + getSettlementRebate(_tokenId);
	...
	function winnerWithdraw(uint256 _winnerTokenId) public {
		...
		uint256 toWithdraw = winnerPrize(_winnerTokenId);
	...
	function ownerWithdraw() public {
		...
		uint256 toWithdraw = (MINT_COST * wagonCount * (MINT_PERCENT_CREATOR)) /
			100 +
	...
	function currentPrize() public view returns (uint256) {
		...
		return
			(MINT_COST * wagonCount * MINT_PERCENT_PRIZE) /
			100 -
			//minus fees removed
			settledFunds +
			//plus supplies * 90%
	...
	function winnerPrize(uint256 _tokenId) public view returns (uint256) {
		return currentPrize() + getSettlementRebate(_tokenId);
	}

	function getSettlementRebate(uint256 _tokenId)
		internal
		view
		returns (uint256)
	{
		...
		_lastAmbush;
		return
			(MINT_COST * (1 + _supplies - _ambushes) * MINT_PERCENT_PRIZE) /
			100;
	}
	...
	function isInJourneyStage() public view returns (bool) {
		return (stage() == Stage.Journey1 ||
			stage() == Stage.Journey2 ||
			stage() == Stage.Journey3 ||
			stage() == Stage.Journey4);
	}

	function checkVulnerable(uint256 _tokenId, bytes32 _lastAmbush)
		internal
		view
		returns (bool)
	{
		(
			string memory _pickedLetter,
			string memory _pickedInconvenience,
			bytes32 _inconvenienceId
		) = IPioneersTrails(trails).currentInconveniences(
				tokenToTrail[uint8(_tokenId)]
			);
		...
		return
			wagonToFirstLetter[_wagonId] ==
			bytes1(abi.encodePacked(_pickedLetter)) &&
			convertItem(items[_wagonId][0]) !=
			bytes1(abi.encodePacked(_pickedInconvenience)) &&
			convertItem(items[_wagonId][1]) !=
			bytes1(abi.encodePacked(_pickedInconvenience)); // Protetto dagli item?
	}

	function isVulnerable(uint256 _tokenId) public view returns (bool) {
		(
			uint8 _supplies,
			uint8 _ambushes,
			bytes32 _lastAmbush
	...
	function approve(address _approved, uint256 _tokenId) external override {
		...
		require(
			_owner == msg.sender || //Require Sender Owns Token
...
interface IPioneersTrails {

    function currentInconveniences(uint8 _baseTrailId) external view returns (string memory _pickedLetter, string memory _pickedInconvenience, bytes32 _inconvenienceId);
