...
/// @author @okwme / okw.me
/// @dev moving tokens and permissions for Minting and Metadata as controlled by external contracts

contract Cable is ERC721Enumerable {
...
contract Metadata is Ownable {
  ...
  function getSVG(uint256 tokenId) public view existsModifier(tokenId) returns (string memory) {
    string memory x = tokenId != 335 ? "50" : "10";
    return
      string(
        abi.encodePacked(
          ...
          Base64.encode(
            abi.encodePacked(
              '<?xml version="1.0" encoding="utf-8"?><svg xmlns="http://www.w3.org/2000/svg"  height="100%" width="100%" viewBox="0 0 600 600"  preserveAspectRatio="meet"><style>.name {fill: #000000;font-size: 14px;font-family: "Verdana",sans-serif;}path{fill: none;stroke: #000000;stroke-width: 100;stroke-dasharray: 1;animation: dash 1s linear infinite;}@keyframes dash { to {stroke-dashoffset: 100;}}</style>',
              getPath(tokenId),
              '<text x="',
              x,
              '" y="550" class="name">',
  ...
  function updateCable(uint256 tokenId, uint256 regionIndex, string memory path, string memory name) public onlyOwner {
