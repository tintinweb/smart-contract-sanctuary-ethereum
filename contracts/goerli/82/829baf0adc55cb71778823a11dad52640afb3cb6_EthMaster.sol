...
contract EthMaster is Ownable{
    ...
    function createVault(uint64 _timePeriod, uint256 _amount) external returns(address){
        ...
        bool success = USDCContract.transferFrom(msg.sender, address(this), _amount);
    ...
    function withdrawFees(uint256 _amount) external onlyOwner{
        bool success = USDCContract.transfer(msg.sender, _amount);
        require(success,"Transfer Failed");
    }
...
contract EthVault is AutomationCompatibleInterface {
    ...
    {
        if(!paused){
            for (uint256 i; i < totalEmployees; ++i) {
                if(block.timestamp >= employeeDetails[employees[i]].nextPayTimestamp){
                    upkeepNeeded = true;
