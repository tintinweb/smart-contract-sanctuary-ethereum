...
contract GenesisINO is Ownable, Pausable {
    ...
    mapping (uint256 => uint256) public issuesQuanity; // stage_id 1,2 = issue 1 || stage_id 3,4 = issue 2 || stage_id 5 = issue 3
    mapping (uint256 => mapping (address => bool)) public stageWhiteList; // stage_id => address => whitelist?
    mapping (uint256 => uint256) public nIssueJoined; // stage_id => joined quantity
    ...
    function joinINO(uint256 _quantity) external isNotContract(_msgSender()) whenNotPaused openToBuy payable {
        ...
        /* CHECK */
        require (_quantity >= 1 && _quantity <= 10, "GenesisINO::invalid NFT quantity");
        ...
        nIssueJoined[currentIssue()] += _quantity;
    ...
    function setStage(uint stageId, uint256 amount, uint256 maxBuy, uint256 startTime, uint256 endTime, bool isWhitelist) external onlyOwner {
        stages[stageId] = Stage(amount, maxBuy, startTime, endTime, isWhitelist);
	}

    function setIssueQuantity(uint _issueId, uint256 quantity) external onlyOwner {
        issuesQuanity[_issueId] = quantity;
	}
    ...
    function isJoinable(address buyer, uint256 _quantity) public view returns (bool) {
        ...
        uint256 _curIssue = currentIssue();
        // check NFT quantity for Issue
        if(nIssueJoined[_curIssue] + _quantity > issuesQuanity[_curIssue]) {
            return false;
        }
    ...
    function currentIssue() public view returns (uint256) {
        if(currentStage == 0) {
            return 0;
    ...
    function checkStage(address buyer) external view returns (uint256) {
        if(currentStage == 0) {
            return 0;
        } else if(currentStage == 1) {
            return stageWhiteList[1][buyer] ? 1 : stageWhiteList[2][buyer] ? 2 : 0;
        } else if(currentStage == 2) {
            return stageWhiteList[2][buyer] ? 2 : 0;
        } else if(currentStage == 3) {
            return stageWhiteList[3][buyer] ? 3 : 0;
        } else {
            return 0;
        }
    }
