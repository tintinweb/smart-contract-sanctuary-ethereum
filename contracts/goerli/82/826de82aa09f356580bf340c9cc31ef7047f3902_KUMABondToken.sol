 ...
 * @dev Allows accounts to be blacklisted by a "BLACKLIST_ROLE" role
 */
contract Blacklistable is IBlacklistable {
    ...
    modifier onlyBlacklister() {
        if (!accessController.hasRole(Roles.MCAG_BLACKLIST_ROLE, msg.sender)) {
            revert Errors.BLACKLISTABLE_CALLER_IS_NOT_BLACKLISTER();
        }
        _;
    }
    ...
    modifier notBlacklisted(address account) {
        if (_blacklisted[account]) {
            revert Errors.BLACKLISTABLE_ACCOUNT_IS_BLACKLISTED(account);
        }
        _;
    }
...
contract KUMABondToken is ERC721, Pausable, Blacklistable, IKUMABondToken {
    ...
    constructor(IAccessControl accessController) ERC721("KUMABondToken", "KUMA") Blacklistable(accessController) {}
