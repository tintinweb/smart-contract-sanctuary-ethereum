...
contract CentherPresale is OwnableUpgradeable {

    IERC20 public centher;
    IERC20 public ntr;
    IERC20 public busd;
    ...
    event SetRefRewardBUSD(address indexed referrer, address refererAddress, uint8 round, uint256 amount);
    event SetRefRewardNTR(address indexed referrer, address refererAddress, uint8 round, uint256 amount);
    ...
    function setRateForCoreTeam(uint8 _rate) public onlyOwner {
        rateForCoreTeam = _rate;
    }
    ...
    function tokenPurchaseWithBUSD(uint256 _busdAmount) public {
        ...
        uint256 busdForCoreTeam = _busdAmount * rateForCoreTeam / MULTIPLER;

        busdAmountForCoreTeam += busdForCoreTeam;

        uint256 busdForOwner = _busdAmount - busdForCoreTeam; 

        uint256 tokenAmount = _busdAmount * info.rateForBusd / MULTIPLER;
        ...
        for(uint8 i = 0; i < referralDeep; i++) {
            ...
            uint256 bonus = _busdAmount * referralRate[i] / 100;
            refRewardByBUSD[referrers[i]] += bonus;
            busdForOwner -= bonus;
            emit SetRefRewardBUSD(referrers[i], msg.sender, uint8(_round), bonus);
        }

        busdAmountForOwner += busdForOwner;

        emit TokenPurchaseWithBUSD(msg.sender, uint8(_round), _busdAmount);
    }
    ...
    function tokenPurchaseWithNtr(uint256 _ntrAmount) public {
        ...
        uint256 ntrForCoreTeam = _ntrAmount * rateForCoreTeam / MULTIPLER;

        ntrAmountForCoreTeam += ntrForCoreTeam;

        uint256 ntrForOwner = _ntrAmount - ntrForCoreTeam;

        uint256 tokenAmount = _ntrAmount * info.rateForNtr / MULTIPLER;
        ...
        for(uint8 i = 0; i < referralDeep; i++) {
            ...
            uint256 bonus = _ntrAmount * referralRate[i] / 100;
            refRewardByNTR[referrers[i]] += bonus;
            ntrForOwner -= bonus;
            emit SetRefRewardNTR(referrers[i], msg.sender, uint8(_round), bonus);
        }

        ntrAmountForOwner += ntrForOwner;

        emit TokenPurchaseWithNTR(msg.sender, uint8(_round), _ntrAmount);
    }
    ...
    function claimTokensFromBusd(uint8 _round) external {
        ...
        centher.transfer(msg.sender, tokenAmount);
    ...
    function claimRefRewardBUSD() external {
        require(_hasEnded(), "Presale is not over");
        busd.transfer(msg.sender, refRewardByBUSD[msg.sender]);
        emit RefRewardClaimBUSD(msg.sender, refRewardByBUSD[msg.sender]);
    }

    function claimRefRewardNTR() external {
        require(_hasEnded(), "Presale is not over");
        ntr.transfer(msg.sender, refRewardByBUSD[msg.sender]);
        emit RefRewardClaimNTR(msg.sender, refRewardByNTR[msg.sender]);
    }
    
    function withdrawBusdForCoreTeam() external {
        require(_hasEnded(), "Presale is not over");
        require(coreTeamAddress == msg.sender, "This is not address of CoreTeam wallet");
        busd.transfer(msg.sender, busdAmountForCoreTeam);
    }

    function withdrawNtrForCoreTeam() external {
        require(_hasEnded(), "Presale is not over");
        require(coreTeamAddress == msg.sender, "This is not address of CoreTeam wallet");
        ntr.transfer(msg.sender, ntrAmountForCoreTeam);
    }

    function withdrawBUSD() external onlyOwner {
        require(_hasEnded(), "Presale is not over");
        busd.transfer(msg.sender, busdAmountForOwner);
    }

    function withdrawCenther() public onlyOwner {
        require(_hasEnded(), "Presale is not over");
        uint256 tokens = centher.balanceOf(address(this));
        centher.transfer(msg.sender, tokens);
    }

    function withdrawNtr() public onlyOwner {
        require(_hasEnded(), "Presale is not over");
        // uint256 tokens = ntr.balanceOf(address(this));
        ntr.transfer(msg.sender, ntrAmountForOwner);
    }
    ...
    function getClaimableTokenAmountFromBusd(uint8 _round, address _user) public view returns (uint256) {
        ...
        if ((block.timestamp - contribution.purchaseTimeForBusd) / MONTH > info.lockMonths) {
            ...
            uint256 tokenAmount = months * contribution.totalClaimableTokenAmountForBusd / 10 - contribution.claimedTokenAmountForBusd;
            return tokenAmount;
        } else {
            return 0;
        }
    ...
    function getClaimableTokenAmountFromNtr(uint8 _round, address _user) public view returns (uint256) {
        ...
        if ((block.timestamp - contribution.purchaseTimeForNtr) / MONTH > info.lockMonths) {
            ...
            if(months > 10) months = 10;
            uint256 tokenAmount = months * contribution.totalClaimableTokenAmountForNtr / 10 - contribution.claimedTokenAmountForNtr;
            return tokenAmount;
        } else {
            return 0;
        }
