...
contract ThreeWiseMen is ERC721, Ownable, ReentrancyGuard, PaymentSplitter {
    using Strings for uint256;
    ...
    uint256 vipAmountLimit = 2;
    
    mapping(address => uint256) public _presaleClaimed; 
    mapping(address => uint256) public _vipSaleClaimed;

    uint256 _price = 10000000000000000; // 0.01 ETH
    uint256 _vipPrice = 0; 
     uint256 _publicPrice = 20000000000000000; // 0.02 ETH

    Counters.Counter private _tokenIds;

    uint256[] private _teamShares = [25, 35, 40]; // 3 PEOPLE IN THE TEAM
    address[] private _team = [0xe57Fb01EE88290026523BA594D7352E3E6903a64, 0xA2fC250ea84cf142bB4291fe987FAc0F82b9Df34, 0xf39Fd6e51aad88F6F4ce6aB8827279cffFb92266 // VIP Account gets 40% of the total revenue
    ];

    constructor(string memory uri, bytes32 merkleroot)
        ERC721("ThreeWiseMen", "TWM")
 ...
 function togglePresale() public onlyOwner {
    ...
    function VIPsaleMint(address account, uint256 _amount, bytes32[] calldata _proof)
    external
    payable
    isValidMerkleProof(_proof)
    onlyAccounts
    {
        require(msg.sender == account, "Not allowed");
        require(vip,"VIPsale is OFF");
        
        require(_amount <= vipAmountLimit,"You can't mint so much tokens");
        require(
            _vipSaleClaimed[msg.sender] + _amount <= vipAmountLimit,  "You can't mint so much tokens");  
        _vipSaleClaimed[msg.sender] += _amount;
        for (uint i = 0; i < _amount; i++) {
            mintInternal();
        }
    }
