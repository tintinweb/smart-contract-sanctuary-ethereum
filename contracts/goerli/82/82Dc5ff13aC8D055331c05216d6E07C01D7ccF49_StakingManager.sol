...
contract StakingManager is Ownable {
    ...
    uint256 private rewardTokensPerSecond; // Number of reward tokens minted per block
    ...
    // Constructor
    constructor(address _rewardTokenAddress, uint256 _rewardTokensPerSecond) {
     ...
     */
    function createPool(IERC20 _stakeToken, uint256 allocPoint)
        external
        onlyOwner
    {
        Pool memory pool;
    ...
    /// @notice View function to see pending PULSEDOGE on frontend.
    /// @param _poolId The index of the pool. See `poolInfo`.
    /// @param _account Address of user.
    /// @return pending PULSEDOGE reward for a given user.
    function pendingRewards(uint256 _poolId, address _account)
        external
        view
        returns (uint256)
    {
        ...
        uint256 elapsedTimeSinceLastReward = block.timestamp - pool.lastRewardedTimestamp;
        uint256 rewards = elapsedTimeSinceLastReward * rewardTokensPerSecond;
        ...
        rewardsToHarvest =
            (rewardsToHarvest * pool.allocPoint) /
            totalAllocPoint;

        return rewardsToHarvest / 1e18;
    }
    ...
    function harvestRewards(uint256 _poolId) public {
        ...
        emit HarvestRewards(msg.sender, _poolId, rewardsToHarvest / 1e18);
        IERC20(rewardToken).safeTransfer(address(msg.sender), rewardsToHarvest / 1e18);
    }
