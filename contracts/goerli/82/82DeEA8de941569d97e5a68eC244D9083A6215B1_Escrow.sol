...
contract NFTCOLLECTION is ERC721URIStorage, Ownable {
    ...
    modifier tradable(uint _tokenId) {
        require(true);
        _;
    }

    function mint(address recipient, string memory tokenURI,uint tokenId)
        public
    {
        _mint(recipient, tokenId);
        _setTokenURI(tokenId, tokenURI);
    }
    function transferFrom(address from, address to, uint tokeId) override public tradable(tokeId){
        super.transferFrom(from,to,tokeId);
    }
    function safeTransferFrom(address from, address to, uint tokeId) override public tradable(tokeId){
        super.transferFrom(from,to,tokeId);
    }
}
...
contract Escrow is Ownable{
    ...
    */
    constructor(address _oracle,NFTCOLLECTION _NFTCOLLECTION,uint _disputFee){
        oracle = _oracle;
    ...
    */
    function createContract(uint _uid,uint _amount, address payable _buyer, address payable _seller, address _currency) public payable{
        ...
        agreements[_uid].isNewMint = false;
    ...
    @param _isNewMint True or False. Sent by oracle after validation.
    @custom:events this emits "StatusChange" event on success.
    @custom:events this emits "Delete" event if validation fails.
    */
    function validateContract(uint _uid, bool _validationStatus, uint _fee, bool _isNewMint)public onlyOracle{
        if(!_validationStatus){
            delete agreements[_uid];
            emit Delete(_uid);
        }else{
            agreements[_uid].validated = _validationStatus;
            agreements[_uid].isNewMint = _isNewMint;
            agreements[_uid].fee = _fee;
            agreements[_uid].lastTime = block.timestamp+ 0 days;
    ...
    */
    ...
    /**  
    @notice Function to finish a successfull contract/agreement with mint enabled. It will also release the money to seller. and mint token for buyer.
    @param _uid Predefined unique identifier for contract/agreement.
    @custom:route This will coduct calling resolveContract function.
    @custom:events this emits "Complete" event.
    */
    function mintAndMarkComplete(uint _uid, string memory _tokenURI) public onlySeller(_uid) {
        require(agreements[_uid].stage==Stage.productReceived);
        NFT.mint(agreements[_uid].buyer,_tokenURI,_uid);
        markContractCompleted(_uid);
    }
