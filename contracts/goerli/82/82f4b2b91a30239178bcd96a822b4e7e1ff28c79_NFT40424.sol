...
contract NFT40424 is Mirror721, SVGUtil {
...
abstract contract Mirror721 is ERC721 {
    ...
    uint8 immutable finality;

    uint256 immutable chains;
    uint16 immutable prevChain;
    uint16 immutable nextChain;

    mapping(uint256 => address) processing;
    mapping(uint256 => address) prevAddr;
    mapping(uint256 => bool) passed;
    mapping(uint256 => uint256) timestampLock;
    ...
    function validateTransfer(bytes calldata encodedVM) external {
        ...
        if (processing[id] == address(0)) {
            ++counter;
            prevAddr[id] = from;
            _emitTransfer(from, to, id, counter);
    ...
    function validateMint(bytes calldata encodedVM) external {
        bytes memory payload = _validateVM(encodedVM, prevChain);
    ...
    function rectify(uint256 id) public virtual {
        require(prevAddr[id] != address(0));
        IWormhole(wormhole).publishMessage(
            1,
            abi.encodePacked(prevAddr[id], id),
            finality
        );
        super.transferFrom(_ownerOf[id], prevAddr[id], id);
        delete prevAddr[id];
    }

    function rectifyMint(uint256 id) public virtual {
        require(passed[id]);
        IWormhole(wormhole).publishMessage(
            1,
            abi.encodePacked(_ownerOf[id], id),
            finality
        );
        _burn(id);
        delete passed[id];
    ...
    function rectificationReciever(bytes calldata encodedVM) external {
        ...
        if (processing[id] == address(0)) {
            ...
            require(processing[id] == to);
            require(timestampLock[id] == 0);
            delete processing[id];
        }
    }

    function rectificationMintReciever(bytes calldata encodedVM) external {
        bytes memory payload = _validateVM(encodedVM, nextChain);
    ...
    function proveTimestampLock(uint256 id) external {
        require(processing[id] != address(0));
        IWormhole(wormhole).publishMessage(
            1,
            abi.encodePacked(id, timestampLock[id]),
            finality
        );
    }
    ...
    function removeTimestampLock(bytes calldata encodedVM) external {
        ...
        require(timestampLock[id] > timestamp);
        delete timestampLock[id];
    }
    ...
    function _checkProcessing(
        ...
        require(processing[id] == address(0));
        processing[id] = to;
        timestampLock[id] = block.timestamp;
    }
}
