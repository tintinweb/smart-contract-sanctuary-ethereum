...
contract motionNFTG is ERC721, Ownable, ReentrancyGuard {
    ...
    // uint256 public ownerNFTs = 15;
    address payable public withdrawWallet;
    mapping(address => uint256) public walletMints;

    constructor()
        payable
        ERC721("MOTION NFT GOLD", "MOTNGOL")
        ReentrancyGuard()
    {
        Price = 0.0003 ether;
        totalSupply = 0;
    ...
    // function sendNFT(
    //     address[] calldata _sendTo,
    //     uint256 _qty
    // ) external onlyOwner {
    //     ownerNFTs -= _sendTo.length * _qty;

    //     for (uint256 i = 0; i < _sendTo.length; i++)
    //         _safeMint(_sendTo[i], _qty);
    // }
