...
interface IScorefamConsumer {
    ...
    function requestId(uint256 _leagueId, uint256 _date) external view returns (bytes32);
    function requestIdGames(bytes32 _requestId, uint256 _idx) external view returns (bytes memory);
    function getRequestIdGamesLength(bytes32 _requestId) external view returns (uint256);
    function requestSchedule(bytes32 _specId, uint256 _payment, uint256 _market, uint256 _leagueId, uint256 _date) external;
    ...
    UUPSUpgradeable
{
    ...
    //Mapping a game to the token used to play 
    mapping(uint256 => address) private gameToken;

    IBEP20Upgradeable private IBEP20;
    ...
    function setRewardToken(address _stablecoin) external onlyRole(DEFAULT_ADMIN_ROLE) {
        IBEP20 = IBEP20Upgradeable(_stablecoin);    
    }
    ...
    function getHalvedReward(uint256 _amount) internal view returns (uint256) {
        ...
        if (block.timestamp <= _deploymentTime + 26 weeks) {
            ...
            if(_amount < 500000000000){
                reward = 3;
            } else if(_amount >= 500000000000 && _amount < 2000000000000){
                reward = 6;
            } else if(_amount >= 2000000000000 && _amount < 5000000000000){
                reward = 9;
            } else if(_amount >= 5000000000000){
                reward = 12;
            } else {
                reward = 0;
            }
        ...
        returns (uint256)
    {
        ...
        else if ((block.timestamp > _gaming[_msgSender()][game_].gameStartTime) && (block.timestamp - _gaming[_msgSender()][game_].gameStartTime) < 1 days) {
        ...
        returns (uint256)
    {
        ...
        require(gameType == 3);
            ...
            if(isGameResultResolved(_gaming[_msgSender()][game_].fixtureID)){
                uint256 result;
            ...
            // else if (gameResult[_gaming[_msgSender()][game_].fixtureID].retrieved){
            //     return amount;
            // }
    ...
    function playSlip(
        ...
        if(gameType_ == 1){
            ...
            require(accOddPoint_.mul(amount_).div(100) <= 100000 * 10**8);
            ...
            for(uint256 i = 0; i < _slipCuts.length; i++){
                ...
                require(_slipCuts[i].gameStartTime > 0);
        ...
        //Multi-flex Game
            ...
            require(_slipCuts.length <= 15);
            require(amount_ >= 10 * 10**18);
            require(accOddPoint_.mul(amount_).div(100) <= 5000 * 10**18);
            ...
            IBEP20.transferFrom(_msgSender(), address(this), amount_);
            gameToken[_slipsCount[_msgSender()]] = _token;
    ...
    // FOR GAMES 
    function requestOracleSchedule(uint256 _leagueId, uint256 _date) external {
        sc.requestSchedule(0x6431313062356334623833643432646361323065343130616335333763643934, 100000000000000000, 1, _leagueId, _date);
    }

    function isGameResultResolved(uint256 _fixtureID) public view returns(bool){
        return gameResult[_fixtureID].retrieved && (gameResult[_fixtureID].status.equal(hex"66696e6973686564") || gameResult[_fixtureID].status.equal(hex"696e746572727570746564")) ? true : false;
    }

    function updateGameResult(uint256 _leagueId, uint256 _date) internal {
        //Update Game Status, Home and Away Scores from the Oracle
        for(uint8 i = 0; i < sc.getRequestIdGamesLength(sc.requestId(_leagueId,_date)); i++){
            gameResult[sc.getGameResolve(sc.requestId(_leagueId,_date),i).gameId] = GameResult(bytes(sc.getGameResolve(sc.requestId(_leagueId,_date),i).status), sc.getGameResolve(sc.requestId(_leagueId,_date),i).homeScore, sc.getGameResolve(sc.requestId(_leagueId,_date),i).awayScore, true);
        }
    }
    ...
    function claimGameReward(uint256 game_, uint256 _leagueId, uint256 _date) external returns (bool){
        ...
        if(!isGameResultResolved(_gaming[_msgSender()][game_].fixtureID)){
            updateGameResult(_leagueId, _date);
        }
        require(isGameResultResolved(_gaming[_msgSender()][game_].fixtureID));
    ...
    function claimSlipReward(uint256 game_) external returns (bool){
        ...
        for(uint256 i = 0; i < _gamingSlip[_msgSender()][game_].slips.length; i++){
            require(isGameResultResolved(_gamingSlip[_msgSender()][game_].slips[i].fixtureID));
        }
        ...
        if(_gamingSlip[_msgSender()][game_].gameType == 2){
            IBEP20.transfer(_msgSender(), amount_);
        } else {
            _mint(fee, onePercent); //Charging 1% Fee
            _mint(_msgSender(), amount_.sub(onePercent));
        }
    ...
    //Retrieve Rewards
    function retrieveRevenue() external onlyRole(MINTER_ROLE){
        require(IBEP20.transfer(fee, IBEP20.balanceOf(address(this))), "Unable to retrieve");
    }
