...
contract OCASnake is Ownable, ERC2981, ERC721A, ReentrancyGuard {
    ...
    mapping(string => string) private difficultyNames;
    ...
    function checkCoinInflationAndMintNumber(uint256 amount) private view {
        uint256 totalCoins;
    ...
    function midnightRelease(
        ...
        bytes32 _r,
        bytes32 _s
    ) public payable onlyValidAccess(date, "whitelist", _v, _r, _s) {
        require(
            midnightReleaseIsOn,
            "You might have shown up a tad too early."
        );
        bool Neo = isNeo();
        if (!Neo) checkCoinInflationAndMintNumber(amount);
        ...
        mustBeOpen(amount)
    {
        ...
        checkCoinInflationAndMintNumber(amount);
        ERC721A._safeMint(msg.sender, amount);
    }
