...
contract EasygoldToken is ERC1400, IExtensionTypes {

    bytes32 public constant EASYGOLD_DEFAULT_PARTITION = keccak256(abi.encodePacked("EASYGOLD_DEFAULT_PARTITION"));
    bytes32[] public EASYGOLD_DEFAULT_PARTITIONS = [bytes32(EASYGOLD_DEFAULT_PARTITION)];
    uint256 public constant GRANULARITY = 1;
    uint256 public constant ISSUED_TOKENS = 1_924_583_085 * 10 ** 18;
    uint256 public constant FOR_SALE_TOKENS = 538_888_890 * 10 ** 18;
   ...
   */
    ...
    ERC1400("Easygold", "EASG", GRANULARITY, EASYGOLD_DEFAULT_PARTITIONS)
    {
        require(newOwner != address(0), "EasygoldToken: wrong address");
        require(seller != address(0), "EasygoldToken: wrong address");

        transferOwnership(newOwner);

        _issueByPartition(EASYGOLD_DEFAULT_PARTITION, msg.sender, newOwner, ISSUED_TOKENS, "");
        _isIssuable = false;
