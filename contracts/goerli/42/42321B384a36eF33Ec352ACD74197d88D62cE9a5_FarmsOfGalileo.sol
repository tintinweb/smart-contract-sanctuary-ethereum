    ...
    AccessControlMixin
{
    ...
    mapping(address => uint8) numMintedForUser;
    ...
    function initialize() public initializer {
        __ERC721_init("Farms of Galileo", "FoG");
        __Ownable_init();
        __Pausable_init();
        ...
        _setupRole(PREDICATE_ROLE, 0xE6F45376f64e1F568BD1404C155e5fFD2F80F7AD);
        ...
        nonReentrant
    {
        ...
        if (
            numMintedForUser[_msgSender()] + amount >
        ...
        if (tokenSupply + amount > maxTokens) revert NoTokensLeft();

        for (uint256 i = 0; i < amount; i++) {
            uint256 tokenId = tokenSupply;
            tokenSupply++;
            numMintedForUser[_msgSender()]++;
...
contract AccessControlMixin is Initializable, AccessControlUpgradeable {
    string private _revertMsg;

    function __AccessControlMixin_init(string memory contractId) public initializer {
      _revertMsg = string(
