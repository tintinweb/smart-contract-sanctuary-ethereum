...
import "./VortexPacks.sol";
import "@openzeppelin/contracts/utils/cryptography/MerkleProof.sol";
import "@openzeppelin/contracts/access/Ownable.sol";

//  __      __        _              ______                    _                 _____           _
//  \ \    / /       | |            |  ____|                  | |               |  __ \         | |
//   \ \  / /__  _ __| |_ _____  __ | |__ ___  _   _ _ __   __| | ___ _ __ ___  | |__) |_ _  ___| | _____
//    \ \/ / _ \| '__| __/ _ \ \/ / |  __/ _ \| | | | '_ \ / _` |/ _ \ '__/ __| |  ___/ _` |/ __| |/ / __|
//     \  / (_) | |  | ||  __/>  <  | | | (_) | |_| | | | | (_| |  __/ |  \__ \ | |  | (_| | (__|   <\__ \
//      \/ \___/|_|   \__\___/_/\_\ |_|  \___/ \__,_|_| |_|\__,_|\___|_|  |___/ |_|   \__,_|\___|_|\_\___/

contract VortexFoundersPacks is Ownable {
    ...
    uint256 private constant _BITPOS_PUBLIC_MINTS = 128;

    uint256 private constant _BITMASK_VORTEX = (1 << 128) - 1;
    uint256 private constant _BITMASK_PUBLIC = _BITMASK_VORTEX << 128;
    ...
    uint256 public vortexPrice = 0.04 ether;
    uint256 public publicPrice = 0.06 ether;

    bytes32 public merkleRoot;
    uint256 public totalSupply;

    VortexPacks public packContract;
    ...
    event NewSaleStage(SaleStage newStage);

    constructor(bytes32 _merkleRoot, VortexPacks _packContract) {
    ...
    modifier verifyProof(
        ...
        bytes32 leaf = keccak256(abi.encode(msg.sender, _merkleAmount));
    ...
    function setVortexPackContract(VortexPacks _packContract)
        external
        onlyOwner
    {
        packContract = _packContract;
    ...
    function setVortexPrice(uint256 _vortexPrice) external onlyOwner {
        vortexPrice = _vortexPrice;
    }
    ...
    function getVortexPointData(address[] memory possibleMinters)
        public
        view
        returns (uint256[] memory actualMints)
    {
        actualMints = new uint256[](possibleMinters.length);

        for (uint256 i; i < possibleMinters.length; i++) {
            actualMints[i] = getVortexMints(possibleMinters[i]);
        }
    }

    function getVortexMints(address _minter) public view returns (uint256) {
        return mints[_minter] & _BITMASK_VORTEX;
    }

    function getPublicMints(address _minter) public view returns (uint256) {
        uint256 publicMints = mints[_minter] & _BITMASK_PUBLIC;
        return publicMints >> _BITPOS_PUBLIC_MINTS;
    }

    function getMints(address _minter) public view returns (uint256) {
        return getVortexMints(_minter) + getPublicMints(_minter);
    }
    ...
    function mintVortex(
        ...
        external
        payable
        verifySaleStage(SaleStage.Vortex)
        verifyAmount(_amount, vortexPrice)
        verifyProof(_merkleProof, _merkleAmount)
        verifyAvailableSupply(_amount)
    {
        if (getVortexMints(msg.sender) + _amount > _merkleAmount)
    ...
    function mintPublic(uint256 _amount)
        ...
        verifyAmount(_amount, publicPrice)
        verifyAvailableSupply(_amount)
    {
        if (getMints(msg.sender) + _amount > MAX_MINTS)
            revert ExceedsWalletSupplyError();

        unchecked {
            mints[msg.sender] += _amount << _BITPOS_PUBLIC_MINTS;
        }
    ...
    ERC2981
{
    ...
    constructor(string memory uri_) ERC1155(uri_) {
        ...
        // TODO: This needs more thought, we could just register _registerForOperatorFiltering(address(0), false) so we dont get the default block list..?
        // See: https://github.com/ProjectOpenSea/operator-filter-registry
        _registerForOperatorFiltering();
        operatorFilteringEnabled = true;

        _grantRole(MODERATOR, _msgSender());
    ...
    // TODO: This needs consideration if we want our own marketplace...
