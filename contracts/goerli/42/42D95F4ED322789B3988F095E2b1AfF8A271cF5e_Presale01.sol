...
contract Presale01 is ReentrancyGuard, Verify {
    ...
    event AlertRefundTokens (
        address indexed buyerAddress,
        uint256 amountRefundTokenForBuyer,
        uint256 amountRefundTokenForPresaleOwner
    );
    ...
    function userRefundBaseTokens() external nonReentrant declineWhenTheSaleStops {
        require(REFUND_INFO.isRefund, "PRESALE IS NOT ALLOWED TO REFUND.");
        BuyerInfo storage buyer = BUYERS[msg.sender];
        
        // require(presaleStatus() == 2, "NOT SUCCESS."); 
        require(buyer.totalTokenWithdraw == 0 || REFUND_INFO.refundTime <= block.timestamp, "CANNOT REFUND OR NOT TIME TO REFUND BASE TOKEN.");
        require(!BUYER_REFUND[msg.sender], "NOTHING TO REFUND.");

        uint256 whitelistDeposited = buyer.baseDeposited - USER_FEES[msg.sender];
        uint256 refundAmount = (whitelistDeposited * (1000 - REFUND_INFO.refundFee)) / 1000;
        uint256 remainingAmount = whitelistDeposited - refundAmount;
        ...
        // Transfer amount base token for use 
        ...
        // Transfer fee refund token for presale owner
        TransferHelper.safeTransferBaseToken(
            address(PRESALE_INFO.B_TOKEN),
        ...
        STATUS.TOTAL_BASE_WITHDRAWN += whitelistDeposited;

        emit AlertRefundTokens(
            msg.sender,
            refundAmount,
            remainingAmount
        );
    ...
    function userWithdrawBaseTokens() external nonReentrant declineWhenTheSaleStops {
        ...
        BuyerInfo storage buyer = BUYERS[msg.sender];
        require(!buyer.isWithdrawnBase, "NOTHING TO REFUND BASE TOKEN WHEN SALE FAILED.");
    ...
    function updatePresaleTime(uint256 _startTime, uint256 _endTime) external onlyOperator declineWhenTheSaleStops{
        require(
    ...
    function updateRefundInfor(bool _isRefund, uint256 _refundTime, uint256 _refundFee) external onlyOperator {
        require(
            _refundTime > block.timestamp && 
            _refundTime > PRESALE_INFO.END_TIME &&
            PRESALE_INFO.END_TIME > block.timestamp,
            "REFUND TIME HAS PASSED."
        );

        require(
            _refundFee > 0 && _refundFee < 1000, 
            "INVALID REFUND FEE."
        );
        REFUND_INFO.isRefund = _isRefund;
        REFUND_INFO.refundTime = _refundTime;
        REFUND_INFO.refundFee = _refundFee;
    }

    function updatePresaleInfor(
        address payable _presaleOwner, 
        IERC20 _presaleToken,
        IERC20 _baseToken,
        uint256 _amount,
        ...
        PRESALE_INFO.HARDCAP = _hardcap;
        PRESALE_INFO.SOFTCAP = _softcap;
        STATUS.WHITELIST_ONLY = _is_white_list;
    }
