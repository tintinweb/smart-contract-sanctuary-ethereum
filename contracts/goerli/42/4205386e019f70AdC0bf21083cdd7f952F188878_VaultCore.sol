...
import "./ERC721/interfaces/IOnChainNft.sol";
import "./ERC20/interfaces/IRewardBaseCoin.sol";
import "@openzeppelin/contracts/token/ERC721/IERC721Receiver.sol";
import "@openzeppelin/contracts/access/Ownable.sol";

contract VaultCore is Ownable, IERC721Receiver {
    //--------------------------------------------------------------------
    // VARIABLES
    ...
    error PikaBounchingVault__ItemAlreadyStaked();
    error PikaBounchingVault__NotItemOwner();

    //--------------------------------------------------------------------
    // CONSTRUCTOR

    constructor(address _nftAddress, address _tokenAddress) {
        nft = IOnChainNft(_nftAddress);
        rewardCoin = IRewardBaseCoin(_tokenAddress);
    }
    ...
    function stake(uint256[] calldata tokenIds) external {
        ...
        for (uint256 i; i < len; ) {
            ...
            if (vault[tokenId].owner != address(0)) {
                revert PikaBounchingVault__ItemAlreadyStaked();
            }
            if (nft.ownerOf(tokenId) != msg.sender) {
                revert PikaBounchingVault__NotItemOwner();
            }
