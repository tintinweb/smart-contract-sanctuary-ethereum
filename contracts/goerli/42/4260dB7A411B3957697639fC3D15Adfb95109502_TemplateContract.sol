...
contract TemplateContract is ERC721A, Ownable, ReentrancyGuard {
    ...
    uint256 public whitelistStartTime = 1695863786;
    uint256 public publicStartTime = 1705863786;
    uint256 public maxMint = 3;

    uint256 public maxWhitelistMintPerAccount = 2;
    uint256 public maxSupply = 10;
    uint256 public freeMint = 1;
    uint256 public whitelistCost = 0.0099 ether;
    uint256 public cost = 0.099 ether;

    mapping(address => uint256) public whitelistMintCount;

    constructor() ERC721A("Bra", "BRA") {}

    function verifyWhitelist(bytes32[] calldata proof)
        public
        view
        returns (bool)
    {
    ...
    function mint(uint256 _count) public payable nonReentrant {
        ...
        require(balanceOf(msg.sender) + _count <= maxMint, "Max mint reached");
        require(totalSupply() + _count <= maxSupply, "NFT supply not enough");
        require(msg.value >= cost * _count, "ether amount not correct");
    ...
    function whitelistMint(uint256 _count, bytes32[] calldata _proof)
        ...
        nonReentrant
    {
        require(verifyWhitelist(_proof), "You are not whitelisted, wait.");
        require(!paused, "Mint is Paused");
        require(
            block.timestamp >= whitelistStartTime,
            "Patient. Mint Not Open Yet"
        );
        require(_count > 0, "Mint at least 1");
        require(
        ...
        require(
            msg.value >=
                whitelistCost * (balanceOf(msg.sender) + _count - freeMint),
    ...
    function reveal(string memory _revealBaseURI) external onlyOwner {
        _postRevealBaseURI = _revealBaseURI;
        _preRevealURI = "";
        _isRevealed = true;
    }

    function conceal(string memory _uri) external onlyOwner {
        _preRevealURI = _uri;
        _postRevealBaseURI = "";
        _isRevealed = false;
    }
    ...
    function setMintPaused(bool _pause) external onlyOwner {
        paused = _pause;
    }
