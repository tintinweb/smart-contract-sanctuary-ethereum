...
contract RentalMarketManagerFacet {
    ...
    function returnRental(
        ...
        if (tokenType == 721) {
            (address renter, uint256 rentingexpires) = RentalStorageLib.getRenterDetails(
                collection,
                tokenId
            );
            ...
            if (rentingexpires + 7200 <= block.timestamp) {
                uint256 overDueTime = block.timestamp - rentingexpires;
                uint256 leftCollateral = (collateral * overDueTime) / 604800;
                (bool callSuccess, ) = payable(msg.sender).call{value: leftCollateral}("");
                require(callSuccess, "Call failed");
            } else {
                (bool callSuccess, ) = payable(msg.sender).call{value: collateral}("");
                require(callSuccess, "Call failed");
            }
