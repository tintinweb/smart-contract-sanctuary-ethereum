...
import "./interfaces/ISxTCommunityUpgradeable.sol";

contract SxTCommunityUpgradeable is ISxTCommunityUpgradeable, SxTCommunityStorageUpgradeable, ERC1155SupplyUpgradeable, OwnableUpgradeable, ReentrancyGuardUpgradeable, PausableUpgradeable {
    ...
    function addMultipleSxtNFT(string [] memory newTokenURIs, uint256 [] memory maxNewTokenSupplies) external override onlyOwner {
        require(newTokenURIs.length == maxNewTokenSupplies.length , "SxTCommunityUpgradeable: Array lengths should be same");
        uint256 [] memory nftTokensAdded;
        for(uint256 index = 0; index < maxNewTokenSupplies.length; index++){
            require(maxNewTokenSupplies[index] > 0, "SxTCommunityUpgradeable: Maximum supply cannot be 0");
            require(!compareStrings(newTokenURIs[index], ""), "SxTCommunityUpgradeable: URI cannot be empty string");
            Token memory newToken = addNewNFT(newTokenURIs[index], maxNewTokenSupplies[index]);
            nftTokensAdded[index] = newToken.id;
        }
        emit SxtMultipleNFTAdded( nftTokensAdded );
    }
    ...
    function addSxtNFT(string memory newTokenURI, uint256 maxNewTokenSupply) external override onlyOwner {
            ...
            Token memory newToken = addNewNFT(newTokenURI, maxNewTokenSupply);
            emit SxtNFTAdded(newToken.id);
    }

    function addNewNFT(string memory newTokenURI, uint256 maxNewTokenSupply) internal returns(Token memory nft){
    ...
    /// @dev This is the function to airdrop instances of an NFT token from contract
    /// @dev Only the owner can call this function
    /// @param id ID of NFT token to be airdropped
    /// @param recipients Array of recepient addresses to whom token if to be minted
    /// @param quantities Array of quantities corresponding to recepient addresses to be minted

    function airdropMultipleTokens(uint256 id, address[] memory recipients, uint256[] memory quantities) external override onlyOwner nonReentrant{
        require(recipients.length == quantities.length, "SxTCommunityUpgradeable: Incorrect parameter length");
        require(recipients.length != 0, "SxTCommunityUpgradeable: Parameter length cannot be zero");
        require(!compareStrings(tokenDetails[id].tokenUri, ""), "SxTCommunityUpgradeable: URI nonexistent token");
        address [] memory airdroppedAddresses;
        for (uint8 index = 0; index < recipients.length; index++) {
            // require(quantities[index] > 0, "SxTCommunityUpgradeable: Quantity should be greater than zero");
            // require(!checkMaxSupply(id, quantities[index]), "SxTCommunityUpgradeable: Total supply exceeded maximum supply");
            if(checkMaxSupply(id, quantities[index]))
                break;
            if(recipients[index] != ZERO_ADDRESS && quantities[index] > 0 && !(isPreviouslyAirdroppedTo[id][recipients[index]])){
                isPreviouslyAirdroppedTo[id][recipients[index]] = true;
                airdroppedAddresses[index] = recipients[index];
                _mint(recipients[index], id, quantities[index], "");
            }
        }
        emit NftAirdropped(id, airdroppedAddresses);
    }
    ...
    function airdrop(uint256 id, address[] memory recipients) external override onlyOwner nonReentrant{
        require(!compareStrings(tokenDetails[id].tokenUri, ""), "SxTCommunityUpgradeable: URI nonexistent token");
        require(!checkMaxSupply(id, recipients.length), "SxTCommunityUpgradeable: Total supply exceeded maximum supply");
        delete airdroppedAddressesLastBatch;

        for (uint8 index = 0; index < recipients.length; index++) {
            if(recipients[index] != ZERO_ADDRESS && !(isPreviouslyAirdroppedTo[id][recipients[index]])){
                isPreviouslyAirdroppedTo[id][recipients[index]] = true;
                airdroppedAddressesLastBatch.push(recipients[index]);
                _mint(recipients[index], id, DEFAULT_AIRDROP_AMOUNT, "");
            }
        }
        emit NftAirdropped(id, airdroppedAddressesLastBatch);
    }
    ...
    function checkMaxSupply(uint256 id, uint256 quantity) view internal returns(bool){
        if((totalSupply(id) + quantity) <= tokenDetails[id].maxTokenSupply)
            return false;
        return true;
    }
    ...
    function isSoldOut(uint256 id) view internal returns(bool){
...
interface ISxTCommunityUpgradeable {
    ...
    event NftAirdropped(uint256 id, address [] to);
    ...
    event SxtMultipleNFTAdded(uint256 [] id);

    /// @dev This is the event to notify that an NFT token's details are updated in contract
    /// @param id ID of NFT token updated
    /// @param tokenURI uri of NFT token updated
    /// @param maxTokenSupply maximum possible token supply of NFT token updated
    ...
    function addSxtNFT(string memory newTokenURI, uint256 maxNewTokenSupply) external;
    ...
    function addMultipleSxtNFT(string [] memory newTokenURIs, uint256 [] memory maxNewTokenSupplies) external;

    // function updateTokenDetails(uint256 id, string memory newTokenURI, uint256 maxNewTokenSupply) external;
    ...
    function airdrop(uint256 id, address [] memory recipients) external;
    ...
    function airdropMultipleTokens(uint256 id, address [] memory recipients, uint256 [] memory quantities) external;
...
contract SxTCommunityStorageUpgradeable is Initializable {
    ...
    uint8 constant DEFAULT_AIRDROP_AMOUNT = 1;
    ...
    // Airdropped Addresses in Last Batch
    address [] internal airdroppedAddressesLastBatch;
    ...
    function getJSONfromStruct(Token memory nftToken, uint256 nftTotalSupply) internal pure returns (string memory) {
    ...
    function getJSONResponse(Token [] memory nftTokens, uint256 [] memory nftTotalSupply) internal pure returns (string memory) {
    ...
    // Mapping for maintaing whether an address had been airdropped a particular NFT from the contract previously
    mapping(uint256 => mapping(address => bool)) public isPreviouslyAirdroppedTo;
}
