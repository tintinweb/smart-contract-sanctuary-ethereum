...
contract ZeroLiquidDevelopmentMarketingVesting is Ownable {
    ...
    constructor(address beneficiaryAddress, uint256 cliffDuration, uint256 vestingDuration) {
        ...
        require(beneficiaryAddress != address(0), "DevelopmentMarketingVesting:: beneficiary can not be zero address");
        require(cliffDuration <= vestingDuration, "DevelopmentMarketingVesting:: cliff is longer than duration");
        require(vestingDuration > 0, "DevelopmentMarketingVesting:: duration is 0");
        require(
            startTimestamp.add(vestingDuration) > block.timestamp,
            "DevelopmentMarketingVesting:: final time is before current time"
        );
    ...
    function release(IERC20 token) public {
        ...
        require(unreleased > 0, "DevelopmentMarketingVesting:: no tokens are due");
    ...
    function revoke(IERC20 token) public onlyOwner {
        require(_revocable, "DevelopmentMarketingVesting:: cannot revoke");
        require(!_revoked[address(token)], "DevelopmentMarketingVesting:: vesting already revoked");
