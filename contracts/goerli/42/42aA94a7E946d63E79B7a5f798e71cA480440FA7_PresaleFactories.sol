...
contract PresaleFactories is Ownable {
    ...
    function createNewPresale(
        ...
        _token.transferFrom(msg.sender, address (presale), (initialInfo[5] * initialInfo[3]* 10 ** _tokenDecimals)/ (10 ** 18));
...
//  Site info
  ...
  function setSiteInfo(string[] calldata siteInfo) public onlyTokenOwner {
      logoLink = siteInfo[0];
      webiste = siteInfo[1];
      facebook = siteInfo[2];
      twitter = siteInfo[3];
  ...
  function getTokenAmount(uint256 weiAmount) public view returns (uint256) {
    return weiAmount.mul(presaleRate).mul(10** _tokenDecimals).div(10**18);
  }
  ...
  modifier onlyTokenOwner() {
    require(msg.sender == _wallet || msg.sender == owner(), "Admin can withdraw only");
    _;
  }
