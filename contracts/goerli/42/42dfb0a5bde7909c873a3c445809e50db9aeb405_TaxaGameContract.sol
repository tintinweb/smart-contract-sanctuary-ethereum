...
contract TaxaGameContract {
    ...
    function getPlayerInfo(bytes memory taxaPublicKey)
        public
        view
        returns (uint seedNumber, string memory gameId, address playAddress, bool isWinner, bool resultSubmitted, bool tokensClaimed)
    {
        require(checkIfPubKeyExists(taxaPublicKey), "Public key does not exist");
        ...
        playAddress = currentPlayer.playAddress;
        isWinner = currentPlayer.isWinner;
        resultSubmitted = currentPlayer.resultSubmitted;
        tokensClaimed = currentPlayer.tokensClaimed;
    }

    function getJoinedAwaitingPlayers() public view returns (uint numberOfPlayers) {
        numberOfPlayers = allPlayerInfos.length - startIndex;
    }
    ...
    function verifyPeer(bytes memory taxaPublicKey, uint amountToLock)
        public
        view
        returns(bool)
    {
