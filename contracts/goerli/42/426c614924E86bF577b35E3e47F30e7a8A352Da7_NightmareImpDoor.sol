...
contract NightmareImpDoor is EIP712, Ownablearama {
    ...
    mapping(address => mapping(uint256 => uint256))
        public partnerTokenClaimedCounts;
    ...
    function requireSenderIsTokenOwnerAndAllocationNotClaimed(
        ...
        if (partnerContract == souls) {
            trickOrTreatAllocation = 2;
        } else if (partnerContract == beasts) {
            trickOrTreatAllocation = 10;
        }

        require(
            partnerTokenClaimedCounts[partnerContract][partnerTokenId] <
    ...
    function mintTricksAndBoxes(
        ...
        require(
            partnerContracts.length == partnerTokenIds.length &&
                ...
                partnerContracts.length == tokenIds.length,
        ...
        bytes32 digest = _hashTypedDataV4(
            keccak256(
                abi.encode(
                    keccak256(
                        "TricksAndBoxes(address to,address[] partnerContracts,uint256[] partnerTokenIds,bool[] isBox,uint256[] tokenIds)"
                    ),
                    msg.sender,
        ...
        for (uint256 i = 0; i < partnerContracts.length; i++) {
            ...
            if (isBox[i]) {
                treasureBox.mint(msg.sender);
            } else {
                tricks.mint(msg.sender, tokenIds[i], 1, "");
            }
        }
    }
    ...
    function mintTreats(
        ...
        for (uint256 i = 0; i < tokenIds.length; i++) {
            ...
            require(
                treasureBox.burnersMap(boxId) == msg.sender,
...
contract NightmareImpTreasureBox is ERC721Burnable, Ownablearama {
    uint256 public constant MAX_MINTABLE_SUPPLY = 21000;

    uint256 public numMinted = 0;

    address public minter;

    mapping(uint256 => address) public burnersMap;

    string public baseTokenURI;

    constructor() ERC721("NightmareImpTreasureBox", "IMPTREASURE") {}
...
abstract contract ForgottenRunesTricksOrTreats is ERC1155Supply, IForgottenRunesTricksOrTreats, Ownablearama {
    ...
    function totalSupply(uint256 id) public view virtual override(ERC1155Supply, IForgottenRunesTricksOrTreats) returns (uint256) {
    ...
    function setMinter2(address _minter2) external onlyOwner {
        minter2 = _minter2;
    }

    function setBaseURI(string memory _baseUri) internal virtual {
        baseUri = _baseUri;
    }

    function uri(uint256 tokenId) public view virtual override returns (string memory) {
        // TODO: check per token URI override before returning generic...
        return baseUri;
    }
}
