...
contract DexAggregator {
    ...
    ISwapRouter public immutable swapRouter = ISwapRouter(univ3router);
    ...
    //function to calculate the time weighted average amount of token out based on the amount of token in
    function uniswapv3rate(
        address tokenIn,
        address tokenOut,
        uint128 amountIn,
        uint32 secondsAgo
    ) public view returns (uint256, uint24) {
        ...
        for (uint i; i < 4; ++i) {
            ...
            if (poolandamountcheck(pool, tokenIn, amountIn)) {
                ...
                uint feeamount = (amountOut * feearray[i]) / 1000000;
                uint swapamount;
    ...
    //function to calculate the amount of token out in uniswap v2 , given the amount in at a particular time
    function uniswapv2rate(
        address tokenIn,
        address tokenOut,
        uint256 amountIn
    ) public view returns (uint256, uint24) {
        address pool = IUniswapV2Factory(univ2factory).getPair(
            tokenIn,
            tokenOut
        );
        ...
        tokens[0] = tokenIn;
        tokens[1] = tokenOut;
        uint256[] memory prices = IUniswapV2Router02(univ2router).getAmountsOut(
            amountIn,
            tokens
        );
    ...
    //function to calculate the amount of token out in sushiswap , given the amount in at a particular time
    function sushiswaprate(
        address tokenIn,
        address tokenOut,
        uint256 amountIn
    ) public view returns (uint256, uint24) {
        address pool = IUniswapV2Factory(sushifactory).getPair(
            tokenIn,
            tokenOut
        );
        ...
        tokens[0] = tokenIn;
        tokens[1] = tokenOut;
        uint256[] memory prices = IUniswapV2Router02(sushirouter).getAmountsOut(
            amountIn,
            tokens
        );
        return (prices[1], 3000);
    ...
    // function will return all the prices if 0 is passed as flag and will return best value if flag is passed as 1
    function getRates(
        address tokenIn,
        address tokenOut,
        uint256 amountIn,
    ...
    //function to check the balance of pool address for a particular token
    ...
    function bestrateswap(
        ...
        uint24 fees;
        uint[] memory expectedamountout;
        //getting the flag and fees for the exchange
        //notice- The fees will only be used if the flag is 1, which represents uniswapv3
        (expectedamountout, flag, fees) = getRates(tokenIn, tokenOut, amountIn, 2);

        uint perc=100-slippageallowed;
        amountOutMin=(expectedamountout[0]*perc)/100;

        console.log(expectedamountout[0],flag,fees);
