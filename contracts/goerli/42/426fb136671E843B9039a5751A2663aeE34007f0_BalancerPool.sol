...
interface IVeBonus {
    function stakeRewards(address, uint256) external returns(bool);
}

contract BalancerPool is IJellyPool, IJellyContract, JellyPoolNFT {
    ...
    constructor(address _jelly, address _bal) {
        jelly = _jelly;
        bal = _bal;
    }
    ...
    /**
     * @notice Stake rewards for ve.
     */
    function stakeRewards() public {
        require(poolSettings.tokensClaimable == true, "Not yet claimable");
        uint256[] memory tokenIds = getOwnerTokens(msg.sender);
        uint256 jellyAmount = _claimRewards(tokenIds, jelly, msg.sender);
        // OZIERC20(poolToken).safeApprove(address(liquidityGauge), 0);
        // OZIERC20(poolToken).safeApprove(address(liquidityGauge), jellyAmount);
        veBonus.stakeRewards(msg.sender, jellyAmount);
    }
    ...
    function _unstake(
        ...
        // GM: not sure if it needs to check poolToken balance after the withdraw
        // GM: mb it's possible to withdraw entire amount at once?
    ...
    function claimBal(address _vault) external returns (uint256){
        ...
        // uint256 balance = 0;
        IMinter(minter).mint(address(liquidityGauge));
        uint256 balance = OZIERC20(bal).balanceOf(address(this));
     ...
     * @param _minter address of Balancer Minter.

     */
    function initJellyPool(address _poolToken, address _accessControls, address _minter) public {
    ...
    function initContract(bytes calldata _data) external override {
        (address _poolToken, address _accessControls, address _minter) = abi.decode(
            _data,
            (address, address, address)
        );

        initJellyPool(_poolToken, _accessControls, _minter);
    }
}
