...
contract SimpleFight {
    ...
    modifier checkAllowance() {
        require(controltoken.allowance(msg.sender, address(this)) >= betamount, "Error");
        _;
    }

    function depositTokens(uint256 _amount) public checkAllowance() {
        controltoken.transferFrom(msg.sender, address(this), _amount);
    }

    function stake(address to, uint256 amount) public {
        controltoken.transfer(to, amount);
    }
    ...
    function enterroom(uint256 _roomnum) public payable {
        ...
        depositTokens(2000000000000000000);
    ...
    function fight(uint256 _roomnum) public payable {
        ...
        if(firstrandom > secondrandom) {
            stake(roominfo[_roomnum].fighters[0], 4000000000000000000);
        } else {
            stake(roominfo[_roomnum].fighters[1], 4000000000000000000);
        }
