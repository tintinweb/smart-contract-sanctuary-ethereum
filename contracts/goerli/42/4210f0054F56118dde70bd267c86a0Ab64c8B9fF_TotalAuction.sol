...
import "./CellSlots.sol";
import "./VRFv2DirectFundingConsumer.sol";

interface ISlot{
    function mint(address slotOwner, uint256 rank) external;
    function burn(uint256 tokenId) external ;
}

contract TotalAuction is VRFv2DirectFundingConsumer {
    ...
    midTimestamp = startTimestamp + 100;
  }

    ISlot public Slot = ISlot(0x2411c2c0487f851eFd1b497A042AE0Ca060aA03b);
    ...
    function startAuction() external onlyOwner {
        ...
        totalBids = 0;
        startTimestamp = block.timestamp;
        midTimestamp = startTimestamp + 250;
        auctionState = 1;
    }
   ...
   */
    function finishAuction() external onlyOwner {
        endTimestamp = block.timestamp + 500;
        auctionState = 2;
        reqId = requestRandomWords();
    ...
    function finalAuction() external onlyOwner {
        (,,uint256[] memory randomW) = getRequestStatus(reqId);
        finalTimestamp = midTimestamp + (randomW[0] % (endTimestamp - midTimestamp));
        finalizeAuction();
    ...
    */
    ...
    function getClaimFunds() public view returns (TokenFund[] memory) {
        return claimFunds;
    }

    function getLockFunds() public view returns (TokenFund[] memory) {
        return lockingFunds;
    }
}
...
contract CellSlots is ERC721Enumerable {

  using Strings for uint256;

  string _baseTokenURI;
  address  totalAuction;
  address owner;
  ...
  constructor() 
    ...
    require(msg.sender == totalAuction, "Only Pr imary type contract can do this");
    _;
  }
  ...
  function mint(address slotOwner, uint256 rank) external  onlyPrimary {
  ...
  function burn(uint256 tokenId) external  onlyPrimary {
    require(_exists(tokenId) == true, "Slot NFT does not exist");
    _burn(tokenId);
    delete rankOfToken[tokenId];
  }

  function setAddressPrimary(address _primary) external onlyOwner{
    totalAuction = _primary;
  }
