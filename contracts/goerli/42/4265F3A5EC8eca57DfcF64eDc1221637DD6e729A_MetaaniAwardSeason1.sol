...
// 特筆
// # 汎用性のある引数: `bytes calldata data`
// - 必須ではない引数に対して、バイナリでパラメータを受けるために使用する。
// - bytesのデコード: (uint tokenId, string memory text) = abi.decode(data, (uint, string));
// - フロント側
//     - Overload関数の呼び方:
...
// - 参考：
//     - (ethers-v5からv6の変更)[https://docs.ethers.org/v6-beta/migrating/]
//     - (オーバーロード関数)[https://docs.ethers.org/v5/migration/web3/#migration-from-web3-js--contracts--overloaded-functions]
//     - (エンコード)[https://docs.ethers.org/v5/api/utils/abi/coder/#AbiCoder-encode]
//     - (デコード)[https://solidity-by-example.org/abi-decode/]
...
import "./interface/IConataNFT.sol";
