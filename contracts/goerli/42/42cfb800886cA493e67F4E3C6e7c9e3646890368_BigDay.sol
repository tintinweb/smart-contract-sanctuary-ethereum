...
contract BoundCurve {
        ...
        )
    {
        ...
        secondsPassed = lastMintTime > 0 ? (currentTime - lastMintTime) : 0;
        currentPrice = getPrice(_getPriceK(secondsPassed));
        currentSlope = slope;
        //timeStepSecondsLeft - for testing purposes
    ...
    function mintRegistration(uint256 tokenId) public {
        ...
        // //Slope related
        // uint256 timeDiff = lastMintTime > 0 ? (block.timestamp - lastMintTime) : 0;

        // lastPrice = this.getPrice();
        // slope = _getNextSlope(timeDiff);
        _priceRecalc();
    }

    function _priceRecalc() internal {
        uint256 currentTimeStep = block.timestamp / TIMESTEP_PERIOD;
        if (priveTimeStep != currentTimeStep) {
            priveTimeStep = currentTimeStep;
            //recalc K and slope
            nextK = 1;
            uint256 currentTime = block.timestamp;
            uint256 secondsPassed = lastMintTime > 0 ? (currentTime - lastMintTime) : 0;

            lastPrice = getPrice(currentK);
            slope = _getNextSlope(secondsPassed);
            currentK = _getPriceK(secondsPassed);
        } else {
            // multiply nextK
            nextK = (nextK * currentK) / PERCENT_PREC;
        }
    ...
    function _getNextSlope(uint256 secondsPass) internal view returns (uint256) {
        uint256 spStepped = (secondsPass / TIMESTEP_PERIOD) * TIMESTEP_PERIOD;
        uint256 daysPass = (spStepped * PERCENT_PREC) / (24 * 60 * 60);
        uint256 k = daysPass / ((MIDDLE_POINT_HOURS / 12)); // twice far from middlepoint
        return ((minSlope * PERCENT_PREC + (maxSlope - minSlope) * k) + (PERCENT_PREC / 2)) / PERCENT_PREC;
    }

    function _getPriceK(uint256 secondsPass) internal view returns (uint256) {
        uint256 spStepped = (secondsPass / TIMESTEP_PERIOD) * TIMESTEP_PERIOD;
        uint256 daysPass = (spStepped * PERCENT_PREC) / (24 * 60 * 60);
        uint256 a = (24 * slope * PERCENT_PREC) / MIDDLE_POINT_HOURS;
        uint256 _currentK = (a * PERCENT_PREC < daysPass * a)
            ? 0
            : (a * PERCENT_PREC - daysPass * a) / PERCENT_PREC / PERCENT_PREC;
        return _currentK;
    }
}
