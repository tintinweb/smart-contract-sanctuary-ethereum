...
contract ERC721SellerV1 is Initializable, UUPSUpgradeable, AdminableUpgradable {
  ...
  event InvokeContractWithAbi(bool success, bytes data);
  event SendNFT(address tokenAddress, address to, uint256 tokenId);
  event WithdrawCoin(uint256 amount);
  event WithdrawToken(address tokenAddress, uint256 amount);
  ...
  function callContractWithSelector(address tokenAddress, bytes memory tokenCallAbi) external payable onlyOwner {
    (bool success, bytes memory data) = tokenAddress.call{value: msg.value}(tokenCallAbi);
    emit InvokeContractWithAbi(success, data);
  ...
  function withdrawCoin(uint256 amount) external onlyAdmin {
  ...
  function transferTokenOwnership(address tokenAddress, address newOwner) external onlyAdmin {
    OwnableERC721 token = OwnableERC721(tokenAddress);
