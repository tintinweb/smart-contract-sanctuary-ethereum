...
contract EscrowProducer is Utilities, Ownable, AssetType{
    ...
    // address constant usdtAddress = 0xd9145CCE52D386f254917e481eB44e9943F39138; // Test net
    TetherToken usdtToken = TetherToken(address(usdtAddress));  

    function addNewProducerUSDT(uint[] memory _postedAssets,string memory _escrowname, uint _assetTypeID, uint _amount) public returns(ProducerStruct memory){
    ...
    function findMatchedAssets(uint _escrowID, uint[] memory _collectedAssets) internal returns(string memory,address, uint[] memory,uint,uint){
...
contract EscrowProvider is Utilities, Ownable{
    ...
    function addNewProvider(uint _escrowID, string memory _escrowName, address _producer,address _provider,uint[] memory _collectedAssets, uint _redeemedAmount, uint _rebateReceivedAmount) internal returns(ProviderStruct memory){
...
contract REscrow_N is EscrowProducer, EscrowProvider {
  
    
    function addProviderUSDT(uint[] memory _collectedAssets) public returns(string memory){
        string memory result;
        result = "No Match Found";
        uint matchedAssetsCount ;
        for (uint i = 0; i < EscrowProducer.currentEscrowCount; i++)
        {
            (string memory producerEscrowName,address producerAddress, uint[] memory matchedAssets, uint rebateReceivedAmount,uint redeemedAmount) = EscrowProducer.findMatchedAssets(i, _collectedAssets);
            ProviderStruct memory newProvider;
