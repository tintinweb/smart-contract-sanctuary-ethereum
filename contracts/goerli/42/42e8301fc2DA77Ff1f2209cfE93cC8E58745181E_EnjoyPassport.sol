...
contract EnjoyPassport is IEnjoyPassport, Ownable, AccessControl, ERC721A, IERC4906{
    ...
    // external contract
    // ==================================================================
    function mint(address to, uint256 quantity) external onlyRole(ADMIN) {
        _safeMint(to, quantity);
    }

    function minterMint(address _address, uint256 _amount) external onlyRole(MINTER) {
        _safeMint(_address, _amount);
    }

    function burnerBurn(address _address, uint256[] calldata tokenIds) external onlyRole(BURNER) {
    ...
    // onlyAdimin Setting
    // ==================================================================
    function setRouter(IRouter _router) external onlyRole(ADMIN) {
        router = _router;
    }

    function setWalletfamily(IWalletFamily _walletFamily) external onlyRole(ADMIN) {
        walletfamily = _walletFamily;
    }

    function setTargetId(uint256 _value) external onlyRole(ADMIN){
        targetId = _value;
    }
    ...
    // ERC-4906
    ///////////////////////////////////////////////////////////////////////////
	function refreshMetadata(uint256 _tokenId) external onlyRole(UPDATER) {
		emit MetadataUpdate(_tokenId);
	}

	function refreshMetadata(uint256 _fromTokenId, uint256 _toTokenId) external onlyRole(UPDATER) {
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        ...
        return router.tokenURI(tokenId, targetId);
    } 
