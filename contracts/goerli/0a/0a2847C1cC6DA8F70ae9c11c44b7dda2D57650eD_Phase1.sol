...
// File contracts/interfaces/IPhase1.sol
...
contract Phase1 is IPhase1, ReentrancyGuard, Ownable {
    ...
    mapping(address => uint256) public _haloLPAmountsByPairs;

    /// @notice Represents the pair to liquidity positions before uniswap burn
    mapping(address => uint256) public _uniswapAmountsByPairs;

    /// @notice Represents the total weight of locked positions in phase 1
    mapping(address => uint256) public _totalWeightOfLockedPositions;
    ...
    function setPhase2(IPhase2 _phase2) external onlyOwner {
    ...
    function migrateLiquidityToHaloAMM() external onlyOwner notStopped {
        ...
        for (uint256 i = 0; i < supportedPairsAddresses.length; ++i) {
            ...
            if (_token0 == WETH || _token1 == WETH) {
                ...
                uint256 ethUtilized;
                (, ethUtilized, liquidity) = IRouter(haloRouter)
                ...
                (token0Used, token1Used, liquidity) = IRouter(haloRouter)
                    .addLiquidity(
                        _token0,
                        _token1,
    ...
    function deposit(
        ...
        if (_token0 == WETH || _token1 == WETH) {
            ...
            if (amount1Used < amount1)
                IERC20(_token1).transfer(msg.sender, amount1 - amount1Used);
        }
    ...
    function claimLP(address pair) external lock notStopped {
        ...
        address haloPair = IRouter(haloRouter).getPair(
            token0,
            token1,
            isStablePair[pair]
        );
