...
contract Pixellations is ERC721, Ownable {
    ...
    Counters.Counter private tokenCount; //use instead of tokenCount
    ...
    function tokenURI(uint256 tokenId) public view override returns (string memory) {
        ...
        Metadata memory metadata = Metadata(0, 0, 0, 0, random(tokenCount.current(), 2, 7), smallStarOptions[random(tokenCount.current(), 0, 4)]);
        
        uint256 bigStarRefX = random(tokenCount.current() * metadata.bigStars, 0, 17) * 2;
        uint256 bigStarRefY = random(tokenCount.current() * metadata.smallStars, 0, 17) * 2;
        ...
        for (uint256 i = 0; i < metadata.bigStars; i++) {
            svg = string(
                abi.encodePacked(
                    svg, '<rect x="', uint2str(random((tokenCount.current() + i), bigStarRefX, bigStarMaxX)), '" y="', uint2str(random((tokenCount.current() + i * 2), bigStarRefY, bigStarMaxY)), '" width="2" height="2" fill="#FFF"></rect>'
                    )
                );
        }
        ...
        for (uint256 i = 0; i < metadata.smallStars; i++) {
            ...
            svg = string(
                abi.encodePacked(
                    svg, '<rect x="', uint2str(random((i), 0, 64)), '" y="', uint2str(random((tokenCount.current() + i), 0, 64)), '" width="1" height="1" fill="', getColor(i), '" opacity="', currentOpacity, '"></rect>' 
                    )
                );
        }
        ...
        return string(
            abi.encodePacked(
                ...
                Base64.encode(
                    bytes(
                        abi.encodePacked(
                            '{"name": "Pixellation #', uint2str(tokenCount.current()), '",',
