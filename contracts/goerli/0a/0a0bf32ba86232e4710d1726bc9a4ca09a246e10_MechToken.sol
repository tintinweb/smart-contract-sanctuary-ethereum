    ...
    AccessControlUpgradeable
{
    ...
    mapping(address => uint256[]) public ownersTokenList;
    ...
    */
    ...
    modifier onlyWhitelisted() {
        require(
            ...
            "onlyWhitelisted: only whitelisted address can access"
        );
        _;
    }
    ...
    function addAddressesToWhitelist(address[] memory _addrs)
        ...
        returns (bool success)
    {
        for (uint256 i = 0; i < _addrs.length; ) {
            if (addAddressToWhitelist(_addrs[i])) {
                emit WhitelistedAddressAdded(_addrs[i]);
                success = true;
            }
            unchecked {
                ++i;
        ...
        returns (bool success)
    {
        for (uint256 i = 0; i < _addrs.length; ) {
            if (removeAddressFromWhitelist(_addrs[i])) {
                emit WhitelistedAddressRemoved(_addrs[i]);
                success = true;
            }
            unchecked {
                ++i;
    ...
    */
    function mintMechIndividually(
        ...
        emit Mint(_tokenId, _reciever, _amount, _uri);
