...
contract AssetLock is Ownable, Pausable, ReentrancyGuard {
    ...
    function setUnit(uint32 _unit) external onlyOwner {
        unit = _unit;
    }
    ...
    function claim(uint256[] calldata tokenIds) external whenNotPaused nonReentrant {
        ...
        for (uint256 i = 0; i < tokenIds.length; i++) {
            ...
            if (!isNFTClaimed[tokenIds[i]]) {
                ...
                unusedTokenIdsOf[msg.sender].push(tokenIds[i]);
            }
        }

        if (diff <= unit) {
            for (uint256 i = 0; i < diff; i++) {
                isNFTClaimed[unusedTokenIdsOf[msg.sender][i]] = true;
                usedTokenIdsOf[msg.sender].push(unusedTokenIdsOf[msg.sender][i]);
                delete unusedTokenIdsOf[msg.sender][i];
            }
            _claim(diff);
        } else {
            for (uint256 i = 0; i < unit; i++) {
                isNFTClaimed[unusedTokenIdsOf[msg.sender][i]] = true;
                usedTokenIdsOf[msg.sender].push(unusedTokenIdsOf[msg.sender][i]);
                delete unusedTokenIdsOf[msg.sender];
            }
            _claim(unit);
        }
    }
