...
// File: contracts\NftExchangeV2.sol

// solium-disable-next-line linebreak-style
pragma solidity >=0.4.22 <0.9.0;
contract NftExchangeV2 is
    Initializable,
...
{
    address owner;
    // This field is unused but is retained for storage compatibility.
    ...
    // This variable represents the payout address for creator fees. It has
    // not been renamed to avoid variable storage clashes during contract
    // upgrade, but we should consider rennaming before going live.
    ...
    uint16 private constant EXCHANGE_RATE_NUMERATOR = 25;
    ...
    event NewNftFixedPriceVoucherRedeemedV3(
        address seller,
        address buyer,
        uint256 tokenId,
        uint256 price,
    ...
    event NftFixedPriceVoucherRedeemedV3(
        address seller,
        address buyer,
        uint256 tokenId,
        uint256 price,
    ...
    event NewNftAuctionVoucherRedeemedV3(
        address seller,
        address buyer,
        uint256 tokenId,
        uint256 price,
    ...
    event NftAuctionVoucherRedeemedV3(
        address seller,
        address buyer,
        uint256 tokenId,
        uint256 price,
    ...
    event NewNftAuctionWithFixedPriceVoucherRedeemedV3(
        address seller,
        address buyer,
        uint256 tokenId,
        uint256 price,
    ...
    event NftAuctionWithFixedPriceVoucherRedeemedV3(
        address seller,
        address buyer,
        uint256 tokenId,
        uint256 price,
        ...
        if (
            ...
            if (sellerVoucher.sellVoucherType == SellVoucherType.FIXED_PRICE) {
                if (sellerVoucher.saleMarketType == SaleMarketType.PRIMARY) {
                    emit NewNftFixedPriceVoucherRedeemedV3(
                        seller,
                        buyer,
                        sellerVoucher.tokenId,
                        sellerVoucher.minPriceWei,
                    ...
                    emit NftFixedPriceVoucherRedeemedV3(
                        seller,
                        buyer,
                        sellerVoucher.tokenId,
                        sellerVoucher.minPriceWei,
                ...
                if (sellerVoucher.saleMarketType == SaleMarketType.PRIMARY) {
                    emit NewNftAuctionWithFixedPriceVoucherRedeemedV3(
                        seller,
                        buyer,
                        sellerVoucher.tokenId,
                        sellerVoucher.minPriceWei,
                    ...
                    emit NftAuctionWithFixedPriceVoucherRedeemedV3(
                        seller,
                        buyer,
                        sellerVoucher.tokenId,
                        sellerVoucher.minPriceWei,
            ...
            if (sellerVoucher.sellVoucherType == SellVoucherType.AUCTION) {
                if (sellerVoucher.saleMarketType == SaleMarketType.PRIMARY) {
                    emit NewNftAuctionVoucherRedeemedV3(
                        seller,
                        buyer,
                        sellerVoucher.tokenId,
                        buyerVoucher.offerWei,
                    ...
                    emit NftAuctionVoucherRedeemedV3(
                        seller,
                        buyer,
                        sellerVoucher.tokenId,
                        buyerVoucher.offerWei,
    ...
    function _redeemVoucherForNftFixedPrice(
        ...
        // past the end date.
        require(
            !params.hasEndDate || (params.hasEndDate && block.timestamp <= params.endDate + 5 * 60),
        ...
        uint256 weiToExchange = (params.minPriceWei * EXCHANGE_RATE_NUMERATOR) /
        ...
        payable(tokenCreator[params.tokenId]).transfer(weiToCreator);
        ...
        if (saleMarketType == SaleMarketType.PRIMARY) {
            ...
            tokenCreator[params.tokenId] = params.creatorFeePayoutWalletAddress;
        }
    }
    ...
    function _acceptOfferForSale(
        ...
        // yet passed. Allow a voucher to be redeemed for an additional 5
        // minutes past the end date.
        require(
            !params.buyVoucherHasEndDate ||
                (params.buyVoucherHasEndDate &&
                    block.timestamp <= params.buyVoucherEndDate + 5 * 60),
        ...
        require(
            !params.sellVoucherHasEndDate ||
                (params.sellVoucherHasEndDate &&
                    block.timestamp <= params.sellVoucherEndDate + 5 * 60),
            SELL_VOUCHER_END_DATE_IN_PAST
        );
        ...
        uint256 weiToExchange = (params.buyVoucherOfferWei * EXCHANGE_RATE_NUMERATOR) /
            EXCHANGE_RATE_DENOMINATOR;
        uint256 weiToCreator;
        if (saleMarketType == SaleMarketType.SECONDARY) {
        ...
        IWETH9(payable(params.sellVoucherWethSmartContractAddress)).transferFrom(
            params.buyer,
            tokenCreator[params.buyVoucherTokenId],
            weiToCreator
        );
        ...
        if (saleMarketType == SaleMarketType.PRIMARY) {
            ...
            tokenCreator[params.sellVoucherTokenId] = params.creatorFeePayoutWalletAddress;
        }
    }
