...
contract BitsWallet is ERC165, IERC1271, SimpleWallet {
    constructor(IEntryPoint anEntryPoint, address anOwner)
    ...
    function setRecoveryConfirmationTime(uint256 recoveryConfirmationTime_)
        public
        onlyOwner
    {
    ...
    function initRecovery(address newOwner) public {
        require(msg.sender == guardian, "BitsWallet: msg sender invalid");
    ...
    function cancelRecovery() public {
        require(msg.sender == owner, "BitsWallet: msg sender invalid");
        require(
            recoveryRequest.newOwner != address(0x0),
            "BitsWallet: request invalid"
        );
    ...
    function executeRecovery() public {
        require(
            ...
            "BitsWallet: msg sender invalid"
        );
        require(
            recoveryRequest.requestedAt + recoveryConfirmationTime <
                block.timestamp,
            "BitsWallet: recovery confirmation time not passed"
        );
...
import "./BitsWallet.sol";

import "@openzeppelin/contracts/utils/Create2.sol";

contract BitsWalletDeployer {
    function deployWallet(
        IEntryPoint entryPoint,
        address owner,
        uint256 salt
    ) public returns (BitsWallet) {
        return new BitsWallet{salt: bytes32(salt)}(entryPoint, owner);
    }
    ...
    function getCreate2Address(
        ...
        bytes memory creationCode = type(BitsWallet).creationCode;
