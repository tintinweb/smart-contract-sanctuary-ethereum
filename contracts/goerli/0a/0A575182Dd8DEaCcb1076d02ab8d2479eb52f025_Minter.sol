...
contract Minter is Ownable {
    ...
    function createAddress(uint256 _quantity) public onlyOwner {
        for (uint256 i = 0; i < _quantity; i++) {
            (bool success, bytes memory result) = address(_cloneContract).call(abi.encodeWithSignature("createClone(address)", address(_childContract)));
            require(success, "Cannot create clone");
            child[msg.sender][childCount[msg.sender]] = bytesToAddress (result);
