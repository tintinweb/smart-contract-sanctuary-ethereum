 ...
 */
 ...
 //modify stake/unstake to set the timesStaked array to the current block.timestamp - the block.timestamp when they staked
...
contract CONNORS_STAKING is Ownable, ReentrancyGuard, Pausable {
    ...
    struct Staker {
        ...
        uint256[] timesStaked;
        uint256 numSetbacks;
        bool hasClaimedOrdinal; //necessary?
    }
    ...
    function stake(uint256[] calldata _tokenIds) external whenNotPaused {
        ...
        if (staker.stakedTokenIds.length > 0) {
            for (uint256 i; i < staker.stakedTokenIds.length; ++i) {
                staker.timesStaked[i] += block.timestamp - staker.timesStaked[i];
            }
        } else {
            stakersArray.push(msg.sender);
            stakerToArrayIndex[msg.sender] = stakersArray.length - 1;
            staker.timesStaked.push(block.timestamp);
        }
    ...
    function userStakeInfo(address _user)
        public
        view
        returns (uint256[] memory _stakedTokenIds, uint256[] memory _timesStaked)
    {
        return (stakers[_user].stakedTokenIds, stakers[_user].timesStaked);
    }
