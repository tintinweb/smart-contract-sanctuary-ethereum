...
contract Simple is Context, IERC20, Ownable {
    ...
    string private constant _symbol = unicode"SMPL";
    ...
    uint256 private _maxTxAmount = 13333334 * 10**8;
    uint256 private _maxWalletSize = 26666668 * 10**8;
...
contract NFTStaking is Ownable, IERC721Receiver {
  ...
  mapping(uint256 => Stake) public vault; 
   ...
   constructor(Collection _nft, Simple _token) { 
    nft = _nft;
    token = _token;
  }
  ...
  function _claim( address account, uint256[] calldata tokenIds, bool _unstake) internal {
    ...
    for (uint i = 0; i < tokenIds.length; i++) {
      ...
      earned += 100 gwei  * (block.timestamp - stakedAt) / 1 days;
      vault[tokenId] = Stake({
        owner: account,
        tokenId: uint24(tokenId),
        timestamp: uint48(block.timestamp)
    ...
    if (earned > 0) {
      token.transferFrom(address(this),account, earned);
    }
    if (_unstake) {
      _unstakeMany(account, tokenIds);
    }
    emit Transfer(address(this),account, earned);
  }
