...
contract Kenomi is Ownable, Utils {
    ...
    // 7 days
    uint256 public itemsRemoveTime = 120;
    ...
    function checkUpkeep(
        ...
        for (uint256 i = 0; i < auctionItems.length; i++) {
            ...
            if (timePassed) {
                ...
                // 0 => Auction Time End
                return (upkeepNeeded, abi.encodePacked(i), abi.encodePacked(uint256(0)));
            }
            if (removeTimePassed) {
                upkeepNeeded = (removeTimePassed);
                // 1 => Auction Remove
                return (upkeepNeeded, abi.encodePacked(i), abi.encodePacked(uint256(1)));
            }
        }
        for (uint256 i = 0; i < whiteListItems.length; i++) {
            ...
            if (timePassed) {
                upkeepNeeded = (timePassed);
                // 2 => WhiteList remove
                return (upkeepNeeded, abi.encodePacked(i), abi.encodePacked(uint256(2)));
            }
        }
    }
