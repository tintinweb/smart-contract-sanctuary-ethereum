...
contract Mint is VRFV2WrapperConsumerBase, ERC1155, Ownable, ERC1155Supply {
        ...
        override
    {
        ...
        for (uint256 i = 0; i < 5; i++) {
            ...
            statuses[requestId].randomIds[i] = ((randomWords[i] % 250) +1) * randomRarity((randomWords[i+8] % 500)+1);
            // statuses[requestId].randomIntrinsicRarity[i] = randomWords[i] % 250; // 0 -> 250 // 
            // statuses[requestId].randomRarityRank[i] = randomWords[i+8] % 4; // 0 -> 250 // 
        }

        uint256[] memory amounts = new uint256[](5);
        for (uint256 i = 0; i < 5; i++) amounts[i] = 1;
        _mintBatch(statuses[requestId].sender, statuses[requestId].randomIds, amounts, '');
    }
