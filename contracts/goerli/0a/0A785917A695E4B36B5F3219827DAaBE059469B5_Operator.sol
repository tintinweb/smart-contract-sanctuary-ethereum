...
contract Operator is AuthorizedReceiver, ConfirmedOwner, SXTTokenReceiver, OperatorInterface, WithdrawalInterface {
  ...
  /**
   * @notice Deploy with the address of the SXT token
   * @dev Sets the SxtToken address for the imported SxtTokenInterface
  ...
  function oracleRequest(
    ...
    uint256 dataVersion,
    bytes calldata data
  ) external override validateFromSXT {
  ...
  {
    assert(sxtToken.transfer(recipient, amount));
  }

  /**
   * @notice Displays the amount of SXT that is available for the node operator to withdraw
   * @dev We use `ONE_FOR_CONSISTENT_GAS_COST` in place of 0 in storage
   * @return The amount of withdrawable SXT on the contract
   */
   ...
   */
  function ownerForward(address to, bytes calldata data) external onlyOwner validateNotToSXT(to) {
  ...
  /**
   * @notice Interact with other SxtTokenReceiver contracts by calling transferAndCall
  ...
  function ownerTransferAndCall(
    ...
    return sxtToken.transferAndCall(to, value, data);
  }
   ...
   * Will transfer the SXT sent for the request back to the recipient address.
  ...
  function cancelOracleRequest(
    ...
    sxtToken.transfer(msg.sender, payment);
  }
  ...
  function _verifyAndProcessOracleRequest(
    ...
    uint256 nonce,
    uint256 dataVersion
  ) private validateNotToSXT(callbackAddress) returns (bytes32 requestId, uint256 expiration) {
  ...
  /**
   * @notice Returns the SXT available in this contract, not locked in escrow
   * @return uint256 SXT tokens available
   */
  function _fundsAvailable() private view returns (uint256) {
    uint256 inEscrow = s_tokensInEscrow.sub(ONE_FOR_CONSISTENT_GAS_COST);
    return sxtToken.balanceOf(address(this)).sub(inEscrow);
  }
  ...
  /**
   * @dev Reverts if the callback address is the SXT token
   * @param to The callback address
   */
  modifier validateNotToSXT(address to) {
    require(to != address(sxtToken), "Cannot call to SXT");
    _;
  }
