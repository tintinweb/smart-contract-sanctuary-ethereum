...
import "./IRainiLpv3StakingPoolViews.sol";
...
contract RainiLpv3StakingPoolFunctions is AccessControl, ReentrancyGuard {
    ...
    function getRewardByDuration(
        ...
        return calculateReward(_owner, _amount, _duration, lpStakingPool.rewardRate());
    }
...
interface IRainiLpv3StakingPoolViews {
    ...
    function getStaked(address _owner) external view returns (uint256);
    function balanceOf(address _owner) external view returns (uint256);
    function getCurrentBonus(address _owner) external view returns (uint256);
    function getCurrentAvgBonus(address _owner, uint256 _duration)
        external
        view
        returns (uint256);
    ...
    function photonRewardPerToken() external view returns (uint256);
    function photonEarned(address account) external view returns (uint256);
    function balanceOfXphoton(address _owner) external view returns (uint256);
}
