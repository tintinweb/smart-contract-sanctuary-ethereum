    ...
    IErc721CountDealPointsController
{
        ...
        override
    {
        ...
        uint256 cnt = _count[pointId];
        uint256[] storage items = _tokensId[pointId];
        for (uint256 i = 0; i < cnt; ++i) {
            uint256 tokenId = items[i];
            token.safeTransferFrom(address(this), withdrawAddr, tokenId);
        }
    }
}
...
interface IDealsController is IFeeSettings, IHasFactories {
    ...
    function getDeal(uint256 dealId) external view returns (Deal memory, DealPointRef[] memory);

    /// @dev dets deal by index for particular acctount
    function getDealByIndex(address account, uint256 dealIndex)
        external
        view
        returns (Deal memory, DealPointRef[] memory);
