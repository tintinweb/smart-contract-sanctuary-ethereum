...
import "./IERC721ABulkTransfer.sol";
import "@openzeppelin/contracts/interfaces/IERC721.sol";
import "@openzeppelin/contracts/token/ERC1155/ERC1155.sol";

contract ERC721ABulkTransfer is IERC721ABulkTransfer {
    
    function transferToSingle(address collection, address recipient, uint256[] calldata tokenIds) external override{
        require(tokenIds.length > 0, "Invalid token ids amount");

        for(uint256 i; i < tokenIds.length;) {
            uint256 tokenId = tokenIds[i];
            IERC721(collection).safeTransferFrom(msg.sender, recipient, tokenId);
            unchecked {
                i++;
            }
        }
    ...
    function transferToMultiple(address collection, ToRecipient[] calldata recipientsData) external override {
        require(recipientsData.length > 0, "Empty recipients data");

        for(uint256 i; i < recipientsData.length;) {
    ...
    function transferMultipleCollectionToSingle(FromCollection[] calldata collectionsData, address recipient) external override{
        require(collectionsData.length > 0, "Empty collections data");

        for (uint256 i; i < collectionsData.length;) {
    ...
    function transferMultipleCollectionToMultiple(FromCollectionToRecipient[] calldata transferData) external override{
        require(transferData.length > 0, "Empty transfer data");

        for (uint256 i; i < transferData.length;) {
    ...
    function transferToSingle1155(address collection, address recipient, uint256[] calldata tokenIds, uint256[] calldata amounts) external {
        require(tokenIds.length > 0, "Invalid token ids amount");
        require(tokenIds.length == amounts.length, "Tokenids and amounts must match");

        IERC1155(collection).safeBatchTransferFrom(msg.sender, recipient, tokenIds, amounts, "");
    }
...
/// @title ERC721A bulk transfer interface
interface IERC721ABulkTransfer {
    struct ToRecipient {
        address recipient;
        uint256 tokenId;
    }
    ...
    function transferToMultiple(address collection, ToRecipient[] calldata recipientsData)
        external;
    ...
    /// @param recipient The recipient of the tokens
    function transferMultipleCollectionToSingle(
    ...
    function transferMultipleCollectionToMultiple(FromCollectionToRecipient[] calldata transferData)
        external;
}
