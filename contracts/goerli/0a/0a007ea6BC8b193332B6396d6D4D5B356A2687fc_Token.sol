...
// File: rarible/royalties/contracts/RoyaltiesV2.sol
...
// File: rarible/royalties/contracts/impl/AbstractRoyalties.sol
...
abstract contract RoyaltiesV2Impl is AbstractRoyalties, RoyaltiesV2, IERC2981 {
...
contract Token is ERC721A, StartTokenIdHelper, Ownable, ReentrancyGuard, RoyaltiesV2Impl {
    ...
    event PresalePaused(uint256 indexed _totalSupply);
    ...
    constructor(
        string memory name_, string memory symbol_, string memory baseURI_, uint256 limitSupply_, uint256 startTokenId_
    ...
    // for feeNumerator 1% = 100, so 100% = 10000
    function setDefaultRoyalty(address payable _royaltiesReceipientAddress, uint96 _percentageBasisPoints) public onlyOwner {
        require(_limitSupply > 0, "No tokens to update");
        LibPart.Part[] memory _royalties = new LibPart.Part[](0);
        _royalties[0].value = _percentageBasisPoints;
        _royalties[0].account = _royaltiesReceipientAddress;
        for (uint256 tokenId = startTokenId; tokenId <= _limitSupply; tokenId++) {
            _saveRoyalties(tokenId, _royalties);
        }
    }

    // for feeNumerator 1% = 100, so 100% = 10000
    function setRoyaltiesForToken(uint _tokenId, address payable _royaltiesReceipientAddress, uint96 _percentageBasisPoints) public onlyOwner {
    ...
    function startPresale(
        uint256 presalePrice_, uint8 presaleLimit_, bytes32 merkleRoot_
        ...
        presaleActive = true;
        emit PresaleStart(presalePrice, presaleLimit);
    }

    function pausePresale() external onlyOwner whenPresaleActive {
        presaleActive = false;
        emit PresalePaused(totalSupply());
    }
    ...
    function _preValidatePurchase(uint256 tokensAmount) internal view {
        ...
        require(tokensAmount > 0, "TT: Must mint at least one token");
        require(totalSupply() + tokensAmount <= limitSupply(), "TT: Minting would exceed max supply");
    }


    function presaleMint(uint256 tokensAmount, bytes32[] memory merkleProof) external payable whenPresaleActive nonReentrant {
        _preValidatePurchase(tokensAmount);

        require(tokensAmount + presalePurchasedAmount[msg.sender] <= presaleLimit, "TT: Presale, limited amount of tokens");
        require(presalePrice * tokensAmount <= msg.value, "TT: Presale, insufficient funds");

        bytes32 leaf = keccak256(abi.encodePacked(msg.sender));
        require(MerkleProof.verify(merkleProof, _merkleRoot, leaf), "TT: Address isn't in the allow list");

        presalePurchasedAmount[msg.sender] += tokensAmount;
        _safeMint(msg.sender, tokensAmount);
    }

    function publicMint(uint256 tokensAmount) external payable whenSaleActive nonReentrant {
        _preValidatePurchase(tokensAmount);

        require(tokensAmount <= saleTransactionLimit, "TT: Limited amount of tokens");
        require(salePrice * tokensAmount <= msg.value, "TT: Insufficient funds");
