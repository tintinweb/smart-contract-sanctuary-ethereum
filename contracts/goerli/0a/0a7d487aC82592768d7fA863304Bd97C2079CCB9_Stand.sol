...
contract Stand is ERC721V, Ownable, IERC2981 {
    ...
    uint256 private _royaltyBPS = 0;

    // Address royalties get sent to
    address private _royaltyWallet = 0x0000000000000000000000000000000000000000;

    address public operatorFilterRegistryAddress = 0x0000000000000000000000000000000000000000;
    address public filterRegistrant = 0x0000000000000000000000000000000000000000;
     ...
     */
    uint24 public firstUnlockTime = 0;
    uint24 public secondUnlockTime = 0;
    uint24 public thirdUnlockTime = 0;
    ...
    address public auctioneer = 0x0000000000000000000000000000000000000000;
    ...
    function setUnlockTimes(uint24 first, uint24 second, uint24 third) external onlyOwner {
    ...
    function mintPresale(address[] memory presaleUsers) external onlyOwner {
        ...
        while (supplyLeft > 0) {
            ...
            uint256 randomIndex = uint256(keccak256(abi.encodePacked(block.timestamp, supplyLeft))) % supplyLeft;
            ...
            unchecked {
                ...
                presaleUsers[randomIndex] = presaleUsers[supplyLeft - 1];
                nextId += tokensOwed;
                --supplyLeft;
            }
        }
    ...
    function _mintPresaleAddress(address presale, uint256 nextId, uint256 amount) internal {
    ...
    function mintPublic(uint256 amount) external payable {
        ...
        if (amount > 5) revert OverMintLimit();

        uint256 totalPrice = amount * publicMintPrice;
        if (msg.value < totalPrice) revert InsufficientPayment();
    ...
    /**
     * Allows the owner to set a new auctioneer address. 
     */
    ...
    function endAuction() external {
        ...
        for(uint256 i = 0; i < auctionTokens; ) {
