...
import {ERC721Voucher} from "voucher/common/ERC721Voucher.sol";
import {BasicSingleRecordedRedeemer} from
    "voucher/BasicSingleRecordedRedeemer.sol";
import {AddressBook} from "constants/AddressBook.sol";

contract DevERC721Voucher is ERC721Voucher {
    constructor(address admin, address royalties)
        ERC721Voucher(
            admin,
            msg.sender,
            "DevERC721Voucher",
...
contract DeployTestnet is Script {
    DevERC721Voucher public voucher;
    ...
    function run() public {
        ...
        if (address(voucher) == address(0)) {
            voucher =
            new DevERC721Voucher(AddressBook.engineering(), AddressBook.treasury());
            voucher.devmint(1);
        }

        if (address(redeemer) == address(0)) {
            redeemer = new BasicSingleRecordedRedeemer();
            voucher.approveRedeemer(address(redeemer), true);
            // redeemer.redeem(voucher, 0);
        }
    }
}
...
abstract contract BaseVoucherToken is IVoucherToken {
     ...
     */
    mapping(address => bool) private _isRedeemerApproved;
    ...
    function redeem(address sender, uint256 tokenId) external {
        if (!_isSenderAllowedToSpend(sender, tokenId)) {
            revert IVoucherToken.CallerNotOwnerNorApprovedForVoucher(tokenId);
        }

        if (!_isRedeemerApproved[msg.sender]) {
    ...
    function _approveRedeemer(address redeemer, bool toggle) internal {
        _isRedeemerApproved[redeemer] = toggle;
    }
...
} from "ethier/contracts/erc721/BaseTokenURI.sol";
 ...
 */
contract ERC721Voucher is BaseVoucherToken, ERC721ACommonBaseTokenURI {
    // =========================================================================
    //                           Constructor
    // =========================================================================
     ...
     */
    function approveRedeemer(address redeemer, bool toggle)
        external
        onlyRole(DEFAULT_STEERING_ROLE)
    {
        _approveRedeemer(redeemer, toggle);
    }
        ...
        returns (bool result)
    {
        ...
        return (sender == tokenOwner) || (sender == getApproved(tokenId))
