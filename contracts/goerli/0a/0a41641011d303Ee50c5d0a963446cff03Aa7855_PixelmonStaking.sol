...
error TokenLocked();
/// @notice Thrown when insufficient or excess Ether provided with transaction
error InsufficientOrExcessPaymentAmount();
/// @notice Thrown when corresponding range #2 token reward already claimed for specified range #1 token
...
/// @dev Interface to interact with PixelmonTrainer NFT contract
interface IPixelmonTrainer is IERC721 {
    function mintRangeTwo(address _to, uint256 _tokensAmount) external returns (uint256);
}
...
contract PixelmonStaking is IERC721Receiver, Ownable, ReentrancyGuard, EIP712, AccessControl {
    ...
    /// @notice Max amount of tokens to claim in single transaction
    uint256 public constant MAX_TOKENS_PER_CLAIM = 50;
    ...
    /// @notice Receiver of collected funds
    address private fundsCollector;

    /// @notice Ether price per token for claim
    uint256 public rangeTwoClaimPrice;
    ...
    /// @notice Range #1 tokenId to range #2 tokenId mapping
    mapping(uint256 => uint256) private rangeTwoRewardClaimed;
    ...
    event SetRangeTwoClaimPrice(uint256 _price);
    event SetFundsCollectorAddress(address indexed _address);
    event Withdraw(address indexed _to, uint256 amount);
    event RangeTwoTrainerClaimed(
        address indexed user,
        uint256 indexed rangeOneTrainerId,
        uint256 indexed rangeTwoTrainerId
    );
    ...
    constructor(
        ...
        rangeTwoClaimPrice = _claimPrice;
    ...
    /// @notice Function to set ETH claim price of range #2 tokens. Callable only by contract owner
    /// @param _claimPrice token claim price in wei
    function setRangeTwoClaimPrice(uint256 _claimPrice) external onlyOwner {
        rangeTwoClaimPrice = _claimPrice;

        emit SetRangeTwoClaimPrice(_claimPrice);
    }

    /// @notice Function to set Ether collected by claimRangeTwoTrainer. Callable only by contract owner
    /// @param _address funds collector address
    function setFundsCollectorAddress(address _address) external onlyOwner validAddress(_address) {
        fundsCollector = _address;

        emit SetFundsCollectorAddress(_address);
    }
    ...
    /// @notice Withdraw ETH to funds collector address
    function withdraw() external nonReentrant onlyOwner {
        uint256 amount = address(this).balance;

        // solhint-disable-next-line avoid-low-level-calls
        (bool success, ) = fundsCollector.call{value: amount}("");

        require(success);

        emit Withdraw(fundsCollector, amount);
    }
        ...
        validInputLength(_tokenIds)
    {
        ...
        for (uint256 i = 0; i < inputLength; i = _uncheckedInc(i)) {
            ...
            IPixelmonTrainer(pixelmonTrainerContract).safeTransferFrom(
                address(this),
                msg.sender,
                _rewardTokenId
            );
    ...
    /// @notice Gives PixelmonTrainer from range #2 as a reward of holding range #1 trainer
    /// Cannot claim reward twice for same range #1 trainer
    /// @dev Mints range #2 tokens to user
    /// @param rangeOneTrainers array of range #1 trainers to claim rewards for
    function claimRangeTwoTrainer(uint256[] calldata rangeOneTrainers)
        external
        payable
        nonReentrant
        whenNotPaused
    {
        uint256 inputLength = rangeOneTrainers.length;
        uint256 paymentAmount = inputLength * rangeTwoClaimPrice;

        if (inputLength == 0 || inputLength > MAX_TOKENS_PER_CLAIM) {
            revert InvalidInputArrayLength();
        }

        if (msg.value != paymentAmount) {
            revert InsufficientOrExcessPaymentAmount();
        }

        for (uint256 i = 0; i < inputLength; i = _uncheckedInc(i)) {
            uint256 rangeOneTrainer = rangeOneTrainers[i];

            if (IPixelmonTrainer(pixelmonTrainerContract).ownerOf(rangeOneTrainer) != msg.sender) {
                revert NotAnOwner();
            }

            if (rangeTwoRewardClaimed[rangeOneTrainer] != 0) {
                revert RewardAlreadyClaimed();
            }
        }

        uint256 rangeTwoTrainerId = IPixelmonTrainer(pixelmonTrainerContract).mintRangeTwo(
            msg.sender,
            inputLength
        );
        ...
        for (uint256 i = 0; i < inputLength; i = _uncheckedInc(i)) {
            ...
            rangeTwoRewardClaimed[rangeOneTrainer] = rangeTwoTrainerId;

            emit RangeTwoTrainerClaimed(msg.sender, rangeOneTrainer, rangeTwoTrainerId);
            rangeTwoTrainerId = _uncheckedInc(rangeTwoTrainerId);
        }
    }
    ...
    /// @notice Get range #2 trainer token id claimed as a reward for range #1 trainer token
    /// Returns 0 if reward is not yet claimed
    function getRangeTwoRewardTrainerTokenId(uint256 rangeOneTrainerId)
        external
        view
        returns (uint256)
    {
        return rangeTwoRewardClaimed[rangeOneTrainerId];
    }
