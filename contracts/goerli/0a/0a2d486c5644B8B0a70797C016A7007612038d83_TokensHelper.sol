...
contract TokensHelper {
        ...
        )
    {
        ...
        for (uint256 index = 0; index < length; index++) {
            ...
            tokenIds[index] = _getTokenIds(tokens[index], account);
        }
    }
    ...
    function _getTokenIds(address token, address account)
        internal
        view
        returns (uint256[] memory tokenIds)
    {
        uint256 balance = IERC721Enumerable(token).balanceOf(account);
        tokenIds = new uint256[](balance);
        for (uint256 index = 0; index < balance; index++) {
            tokenIds[index] = IERC721Enumerable(token).tokenOfOwnerByIndex(account, index);
        }
    }
...
interface IERC721Registry {
    ...
    function tokens(bytes32 key) external view returns (TokenInfo memory);
