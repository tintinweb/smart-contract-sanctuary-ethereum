...
contract Guns is ERC721AQueryable, ERC721ABurnable, EIP712, Ownable {
    ...
    constructor() ERC721A("Guns", "gu") EIP712("Guns", "1.0.0") {
    }

    function mint(uint amount, bytes calldata signature) external payable {
        require(_recoverAddress(msg.sender, signature) == _signerPublicKey, "invalid signature");
    ...
    function _hash(address account) private view returns (bytes32) {
        return _hashTypedDataV4(
            keccak256(
                abi.encode(
                    keccak256("Guns(address account)"),
                    account
                )
            )
        );
    ...
    function isMessageValid(bytes memory _signature, uint amount) public view returns (address, bool){
        ...
        if(signer == _signerPublicKey)return (signer, true);
