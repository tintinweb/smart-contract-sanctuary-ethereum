...
contract BullBear is ERC721, ERC721Enumerable, ERC721URIStorage, Ownable, KeeperCompatibleInterface, VRFConsumerBaseV2 {
    ...
    function requestRandomnessForNftUris() internal {
        require(s_subscriptionId != 0, "Cannot fund the VRF request");
    ...
    function fulfillRandomWords(
        ...
        string[] memory nftUri = currentMarketTrend == MarketTrend.BULL ? bullUrisIpfs : bearUrisIpfs;
        uint256 idx = s_randomWords[0] % nftUri.length;

        for (uint256 i = 0; i < _tokenIdCounter.current(); i++) {
            _setTokenURI(i, nftUri[idx]);
        }
    ...
    function setCallbackGastLimit(uint32 _limit) public onlyOwner {
