...
contract DerivedLiquidity is  ReentrancyGuard {
  ...
  constructor(
    ...
    public
  {
    require(_governance != ZERO_ADDRESS, "DerivedLiquidity: governance address cannot be 0x0.");
    require(_sentinel != ZERO_ADDRESS, "DerivedLiquidity: sentinel address cannot be 0x0.");

    token = _token;
    xtoken = _xtoken;
    governance = _governance;
    sentinel = _sentinel;
    flushActivator = 1000 ether;// change for non 18 digit tokens
  ...
  function setPendingGovernance(address _pendingGovernance) external onlyGov {
    require(_pendingGovernance != ZERO_ADDRESS, "DerivedLiquidity: governance address cannot be 0x0.");
  ...
  function setTransmuter(address _transmuter) external onlyGov {
    ...
    require(_transmuter != ZERO_ADDRESS, "DerivedLiquidity: transmuter address cannot be 0x0.");
  ...
  function setRewards(address _rewards) external onlyGov {
    ...
    // transfers to the address because of `safeTransfer` checks.
    require(_rewards != ZERO_ADDRESS, "DerivedLiquidity: rewards address cannot be 0x0.");
  ...
  function setHarvestFee(uint256 _harvestFee) external onlyGov {
    ...
    require(_harvestFee <= PERCENT_RESOLUTION, "DerivedLiquidity: harvest fee above maximum.");
  ...
  function setCollateralizationLimit(uint256 _limit) external onlyGov {
    ...
    require(_limit >= MINIMUM_COLLATERALIZATION_LIMIT, "DerivedLiquidity: collateralization limit below minimum.");
    require(_limit <= MAXIMUM_COLLATERALIZATION_LIMIT, "DerivedLiquidity: collateralization limit above maximum.");
  ...
  /// @dev Withdraw from vault's stake pool without caring about rewards. EMERGENCY ONLY.
  ///
  ...
  function emergencyWithdraw(uint256 _vaultId) external {
    ...
    _vault.adapter.emergencyWithdraw();
 
    emit EmergencyWithdrawn(_vaultId);
  }
  ...
  function initialize(IVaultAdapter _adapter) external onlyGov {
    ...
    require(!initialized, "DerivedLiquidity: already initialized");

    require(transmuter != ZERO_ADDRESS, "DerivedLiquidity: cannot initialize transmuter address to 0x0");
    require(rewards != ZERO_ADDRESS, "DerivedLiquidity: cannot initialize rewards address to 0x0");
  ...
  function mint(uint256 _amount) external nonReentrant noContractAllowed onLinkCheck expectInitialized {
    ...
    if (_totalCredit < _amount) {
      ...
      _cdp.checkHealth(_ctx, "DerivedLiquidity: Loan-to-value ratio breached");
    } else {
  ...
  modifier expectInitialized() {
    require(initialized, "DerivedLiquidity: not initialized.");
    _;
  }
  ...
  ///
  modifier onlyGov() {
    require(msg.sender == governance, "DerivedLiquidity: only governance.");
    _;
  }
  ...
  function _updateActiveVault(IVaultAdapter _adapter) internal {
    require(_adapter != IVaultAdapter(ZERO_ADDRESS), "DerivedLiquidity: active vault address cannot be 0x0.");
    require(_adapter.token() == token, "DerivedLiquidity: token mismatch.");
  ...
  function _recallFunds(uint256 _vaultId, uint256 _amount) internal returns (uint256, uint256) {
    require(emergencyExit || msg.sender == governance || _vaultId != _vaults.lastIndex(), "DerivedLiquidity: not an emergency, not governance, and user does not have permission to recall funds from active vault");
...
interface IVaultAdapter {
  ...
  /// @param _amount    the amount of funds to withdraw.
  function withdraw(address _recipient, uint256 _amount, bool _harvest) external;
...
library Vault {
  ...
  function withdraw(Data storage _self, address _recipient, uint256 _amount, bool _harvest) internal returns (uint256, uint256) {
    (uint256 _withdrawnAmount, uint256 _decreasedValue) = _self.directWithdraw(_recipient, _amount, _harvest);
  ...
  /// @param _recipient the account to withdraw the tokens to.
  /// @param _amount    the amount of tokens to withdraw.
  function directWithdraw(Data storage _self, address _recipient, uint256 _amount, bool _harvest) internal returns (uint256, uint256) {
    IDetailedERC20 _token = _self.token();

    uint256 _startingBalance = _token.balanceOf(_recipient);
    uint256 _startingTotalValue = _self.totalValue();

    _self.adapter.withdraw(_recipient, _amount, _harvest);
