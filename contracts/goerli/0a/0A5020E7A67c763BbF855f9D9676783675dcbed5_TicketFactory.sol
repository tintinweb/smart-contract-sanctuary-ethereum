...
contract TicketFactory{
    ...
    // Can keep who owns what is the database
    // name of collection => contract address
    ...
    function newTicketCollection(uint256 totalTickets, string memory name, string memory ticker, string memory baseURI) external onlyOwner {
        TicketCollection ticketCollection = new TicketCollection(totalTickets, name, ticker, msg.sender, baseURI);
...
contract TicketCollection is ERC1155URIStorage {
    ...
    bytes public msgHash;
    bytes public b;
    uint256 public c;
    string public ok;

    constructor(uint256 _maxTickets, string memory _name, string memory _ticker, address _owner, string memory _baseURI) ERC1155(_baseURI) {
    ...
    function mint(string memory tokenURI, address to) external onlyOwner returns (uint256) {
        ...
        require(maxTickets-1 >= ticketCount, "Sold out");
    ...
    function transferWithSignedMessage(address from, address to, uint256 tokenId, bytes32 messageHash, uint8 v, bytes32 r, bytes32 s) external onlyOwner {
        bytes memory _uri = bytes(uri(tokenId));
        b = abi.encodePacked(Prefix, bytes(Strings.toString(_uri.length)), bytes(_uri));
        address recoveredAddress = _validateSignature(b, messageHash, v, r, s);
