    ...
    DefaultOperatorFilterer
{
    ...
    uint256 public provedCountBody;
    ...
    function requestRandomStartingIndex(DiskType diskType_, uint32 numWords_)
        external
        onlyOwner
        returns (uint256 requestId)
    {
        if (diskType_ == DiskType.Body){
            ...
            if (provedCountBody <= 0) revert ProvenanceEquipmentNotSet();

            for (uint256 i = 0; i < provedCountBody; i++) {
                if (bytes(provenanceEquipmentOnBody[i]).length <= 0) revert ProvenanceEquipmentOnBodyNotSet();
    ...
    function setProvenanceEquipmentOnBody(
        ...
        provedCountBody = provenanceEquipmentOnBody_.length;
    }
