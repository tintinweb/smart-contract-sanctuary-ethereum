...
interface IMarginTrading {
    ...
    event SendERC20(address indexed ad, address indexed tokenAddress, uint256 indexed tokenAmount);
    ...
    function sendERC20(address _address, address _tokenAddress, uint256 _tokenAmount) external;
...
interface IMarginTradingFactory {
    ...
    event SendMarginTradingERC20(address _marginTradingAddress, address _tokenAddress, address _to, uint256 _amount);

    event SendMarginTradingETH(address _marginTradingAddress, address _to, uint256 _amount);
    ...
    function sendMarginTradingETH(address _marginTradingAddress, address _to, uint256 _amount) external payable;
...
contract MarginTrading is OwnableUpgradeable, IMarginTrading, IFlashLoanReceiver {
    ...
    modifier onlyUser() {
        require(_USER == msg.sender || owner() == msg.sender, "caller is not the user");
        _;
    }
    ...
    /// @param _amount Asset token Amount
    /// @param _flag Operation flag
    function lendingPoolDeposit(address _asset, uint256 _amount, uint8 _flag) external onlyUser {
    ...
    /// @notice Owner clean ETH
    /// @param _address To address
    /// @param _amount send ETH amount
    function sendETH(address _address, uint256 _amount) external payable onlyOwner {
        _safeTransferETH(_address, _amount);
        emit SendETH(_address, _amount);
    }

    /// @notice Owner clean ERC20 token
    /// @param _address To address
    /// @param _tokenAmount send ERC20 token amount
    /// @param _tokenAddress send ERC20 token address
    function sendERC20(address _address, address _tokenAddress, uint256 _tokenAmount) external onlyOwner {
...
contract MarginTradingFactory is Ownable, IMarginTradingFactory {
    ...
    /// @notice Owner clean marginTrading contract ETH.
    ...
    /// @notice Owner clean marginTrading contract ERC20 token
    /// @param _marginTradingAddress MarginTrading contract address
    /// @param _tokenAddress send ERC20 token address
    /// @param _to To address
    /// @param _amount send ERC20 token amount
    function sendMarginTradingERC20(
        address _marginTradingAddress,
        address _tokenAddress,
        address _to,
        ...
        IMarginTrading(_marginTradingAddress).sendERC20(_to, _tokenAddress, _amount);
        emit SendMarginTradingERC20(_marginTradingAddress, _tokenAddress, _to, _amount);
    }
