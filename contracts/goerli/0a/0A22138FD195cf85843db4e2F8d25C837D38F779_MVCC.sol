...
contract MVCC is ERC721A, Ownable, ReentrancyGuard {
        ...
        supplyLimit(quantity)
    {
        ...
        if (!_validSum(quantity, skills)) revert InvalidStats();
    ...
    function airdrop(
        ...
        uint256 _quantity,
        uint256[6][] calldata skills
    ) external onlyOwner supplyLimit(_quantity) {
        if (_quantity != skills.length) revert LengthMismatch();
        if (!_validSum(_quantity, skills)) revert InvalidStats();
...
contract MVCCOnChainMetadata is Ownable {
        ...
        returns (string memory)
    {
        ...
        string memory imageLink = string(
            abi.encodePacked(_uriPrefix, tokenId.toString(), ".png", '", ')
        );
    ...
    function getIndexes(uint256 _dna)
        public
        view
        returns (string memory indexes)
    {
        (, , indexes) = getTokenData(_dna);
    }
