...
struct CerfInfo{
  ...
  bool Valid;
    ...
    function generateCertificate(uint256 tokenId) internal view returns(string memory){
...
'<text x="50%" y="81%" class="base" dominredant-baseline="middle" text-anchor="middle" font-size="8px" font-weight="bold">', "Validity: ",getValidity(tokenId),'</text>',
 '</svg>'
    ...
    function updateValidity(uint256 tokenId)public {
        if(block.timestamp>tokenidtoCerf[tokenId].ValidTill)
           {
                 tokenidtoCerf[tokenId].Valid=false;
           }
    }
    function getValidity(uint256 tokenId) public view returns(string memory) {
