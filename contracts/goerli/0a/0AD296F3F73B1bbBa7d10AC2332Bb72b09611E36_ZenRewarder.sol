...
contract ZenRewarder is IJellyRewarder, IJellyContract {
    ...
    bytes32 public constant override TEMPLATE_ID = keccak256("ZEN_REWARDER");
    ...
    function poolRewards(address _poolAddress, address _rewardToken, uint256 _from, uint256 _to) public override view returns (uint256 rewards) {
        ...
        if (_poolAddress == poolAddresses[0]) {
            if (fromPeriod == toPeriod) {
                return periodRewardsPerSecond[fromPeriod] * (_to - _from);
            }
            uint256 initialRemander = startTime + ((fromPeriod+1) * periodSeconds) - _from;
            rewards = periodRewardsPerSecond[fromPeriod] * initialRemander;
            for (uint256 i = fromPeriod+1; i < toPeriod; i++) {
                rewards = rewards + (periodRewardsPerSecond[i] * periodSeconds);
            }
            uint256 finalRemander =  _to - (startTime + (toPeriod * periodSeconds)) ;
            rewards = rewards + (periodRewardsPerSecond[toPeriod] * finalRemander);
            return rewards;
        } else {
            return 0;
        }
