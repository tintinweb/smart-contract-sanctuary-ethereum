...
contract ProxyOFT is OFTCore {
     ...
     *         This way users do not have to approve proxyOFT to spend PRIME and bridge their tokens.
     *         PRIME will be sent to a primeDestinationAddress that has already approved ProxyOFT and
     *         ethDestinationAddress will be the proxyOFT contract itself so that the nativeFees required for
     ...
     *                                 of the user.
     * @param _ethValue amount of eth sent to this contract to cover the nativeFees on the lzSend call
    ...
    function _handleInvokeEchelon(
        ...
        ) = abi.decode(_data, (uint16, uint256, bool, bytes, bytes));
        // debit from _primeDestinationAddress that has already approved ProxyOFT
        if (isCreditTo) {
            // PRIME must be transferred to the _primeDestinationAddress by user _from
            if (_primeValue != amount) {
                revert InvalidPrimePayment();
            }
            _debitFrom(_primeDestinationAddress, dstChainId, "", _primeValue);
        }

        bytes memory toAddress = abi.encodePacked(_from);
        bytes memory payload = abi.encode(
            _from,
            toAddress,
            amount,
        ...
        _lzSend(
            ...
            address(0x0),
            adapterParams,
            _ethValue // Typically msg.value but forwarding send call from invokeEchelon
        );

        emit SendToChain(dstChainId, _from, toAddress, _primeValue);
    }
    ...
    function _debitFrom(
        ...
        if (_from != _msgSender() && address(PRIME) != _msgSender()) {
            revert InvalidCaller();
        }
