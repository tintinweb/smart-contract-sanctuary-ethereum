...
import {BeaconImpl} from "../proxy/BeaconImpl.sol";
import {IBeaconCache} from "../interfaces/proxy/IBeacon.sol";
import {TokenHandler} from "../utils/TokenHandler.sol";
import {VaultStorageV1} from "./VaultStorage.sol";
import {IVaultFactory} from "../interfaces/IVaultFactory.sol";
import {IWETH} from "../interfaces/IWETH.sol";

contract Vault is VaultStorageV1, TokenHandler, OwnableUpgradeable, BeaconImpl {
    using SafeERC20 for IERC20;

    /**
     * @dev The _beacon factory has permission to self destruct this
     *      implementation if it's the cache contract and not active.
     * @param _beacon Address of the beacon factory
     */
    constructor(IBeaconCache _beacon) BeaconImpl(_beacon) {}
    ...
    /**
     * @notice Initializes the vault with a specified owner
     * @dev Requires WETH to be set up in the vault factory
     * @param _owner The address which will own the vault
     */
    function initialize(address _owner) public onlyProxy initializer {
        __Ownable_init();
    ...
    function payWhitehat(address wh, ERC20Payment[] calldata payout) public onlyOwner {
        address immunefi = vaultFactory.immunefi();
        uint256 length = payout.length;
        for (uint256 i; i < uint256(length); i++) {
            IERC20(payout[i].token).safeTransfer(immunefi, payout[i].amount / 10);
    ...
    function supportsInterface(bytes4 interfaceId)
        ...
        view
        virtual
        override(BeaconImpl, TokenHandler)
        returns (bool)
    {
...
import {IBeaconCache, IBeaconImpl} from "../interfaces/proxy/IBeacon.sol";

abstract contract BeaconImpl is IBeaconImpl, ERC165 {
    IBeaconCache public immutable override beacon;
    IBeaconImpl public immutable override cache;
    IBeaconImpl internal immutable _thisCopy;

    /**
     * @param _beacon The address of the beacon
     */
    constructor(IBeaconCache _beacon) {
        beacon = _beacon;
        cache = _beacon.cache();
        _thisCopy = this;
        assert(type(IBeaconImpl).interfaceId == 0x93075e13);
    }
    ...
    function supportsInterface(bytes4 interfaceId) public view virtual override(ERC165, IERC165) returns (bool) {
        return interfaceId == type(IBeaconImpl).interfaceId || super.supportsInterface(interfaceId);
    }

    /**
     * @notice Getter for implementation address
     */
    function implementation() public view override returns (IBeaconImpl) {
        return IBeaconImpl(beacon.implementation());
    }

    function _requireProxy() internal view {
        require(implementation() == _thisCopy && this != _thisCopy && this != cache, "BeaconImpl: only proxy");
    }
    ...
    /**
     * @notice Function to self destruct implementation contracts
     * @dev Sender must be beacon contract
     * @dev Only inactive implementations or caches can be selfdestructed
     */
    function cleanUp() external override {
        require(msg.sender == address(beacon), "BeaconImpl: not beacon");
        if (this == _thisCopy) {
            require(this != implementation(), "BeaconImpl: active implementation");
        } else {
            require(this == cache, "BeaconImpl: only cache");
        }
...
contract TokenHandler is ERC1363Receiver, ERC777Receiver, ERC677Receiver {
    ...
    {
        return
            interfaceId == type(ERC677Receiver).interfaceId ||
            interfaceId == type(ERC777Receiver).interfaceId ||
            interfaceId == type(ERC1363Receiver).interfaceId ||
