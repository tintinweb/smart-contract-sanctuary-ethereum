...
contract OzuPaymaster is BasePaymaster {
  bool public useRejectOnRecipientRevert;

  // only whitelist specific methods from specific contracts
  mapping(address => mapping(bytes4 => bool)) public methodWhitelist;

  constructor() {
    useRejectOnRecipientRevert = true;
  }
  ...
  /**
   * @dev Whitelist the target contract's method
   */
  ...
  /**
   * @dev Set if method should revert when recipient reverts
   */
  function setConfiguration(bool _useRejectOnRecipientRevert) public onlyOwner {
    useRejectOnRecipientRevert = _useRejectOnRecipientRevert;
  }

  /**
   * @dev Returns if contract method is whitelisted
   */
  function checkWhitelistMethod(
    address target,
    bytes4 method
  ) public view returns (bool) {
    return methodWhitelist[target][method];
  }
