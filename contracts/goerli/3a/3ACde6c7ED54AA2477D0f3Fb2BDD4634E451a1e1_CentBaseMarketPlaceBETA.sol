...
/// --------------------------------- ADMIN METHODS ---------------------------------
    ...
    function listAndSell(address collection, uint256 tokenId, address payable seller, uint256 priceInWei) 
        external 
        onlyRole(ITEM_CREATOR_ROLE) 
        returns (bytes32 itemId, bytes32 orderId) 
    {
        itemId = createMarketItem(collection, tokenId, seller);
        orderId = createMarketOrder(itemId, priceInWei);
        emit ListAndSell(collection, seller, tokenId, priceInWei, itemId, orderId);
    }
}
