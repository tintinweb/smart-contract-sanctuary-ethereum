...
contract Djin is ERC721A, Ownable, ReentrancyGuard {
    ...
    function mint(uint256 amount_, bytes32[] calldata proof_) external payable nonReentrant stockCount(amount_) {
        uint256 cost = mintPrice;

        require(whitelistPhase || publicPhase, "Sale has not commenced");
        ...
        if (publicPhase) require(publicMints[msg.sender] + amount_ <= maxPerWallet, "You have minted max amount per wallet");
