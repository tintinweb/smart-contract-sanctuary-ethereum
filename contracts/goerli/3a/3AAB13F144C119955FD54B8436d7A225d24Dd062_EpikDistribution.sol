...
contract EpikDistribution is Ownable, ReentrancyGuard {
    ...
    function changeGroupStatus(uint256 groupIndex_, bool isPaused_) external onlyManager {
...
contract RollingGroup is BaseGroup {
        ...
        returns (uint256 lastPaidDate, uint256 claimable)
    {
        ...
        uint256 totalClaimedAmount = userClaimedAmount[receiverAddress];

        // If the user already get paid all amount, no need to pay
        if (totalClaimedAmount >= amount) {
            return (lastPaidDate, claimable);
        }

        uint256 restAmount = amount - totalClaimedAmount;

        uint256 shouldPaidCnt = 0;
        if (lastPaidDate == 0) {
            shouldPaidCnt = (currentTime - startDate) / period + 1;
        } else {
            shouldPaidCnt = (currentTime - lastPaidDate) / period;
        }
  
        claimable = (shouldPaidCnt * percentage * amount) / PERCENT_DENOMINATOR;
        if (claimable >= restAmount) {
  	        claimable = restAmount;
        }
  
        uint256 totalPaidCnt = ceil(((claimable + totalClaimedAmount) * PERCENT_DENOMINATOR), amount * percentage) / (amount * percentage);
        lastPaidDate = (totalPaidCnt - 1) * period + startDate;
    }
}