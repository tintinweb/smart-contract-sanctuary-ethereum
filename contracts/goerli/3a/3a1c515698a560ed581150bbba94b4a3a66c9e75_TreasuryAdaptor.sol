...
contract TreasuryAdaptor {
    ...
    function addOperator(address newOperator) public {
        ...
        if (isActiveOperator(newOperator)) revert DuplicatedAddress();
        operators[newOperator] = block.timestamp + delay;
        // When succesfully add, remove it from blocklist
        operatorsBlocklist[newOperator] = 0;
    }
    ...
    function removeOperator(address oldOperator) public {
        if (msg.sender != admin) revert Unauthorized();
        if (!isActiveOperator(oldOperator)) revert DuplicatedAddress();
        operatorsBlocklist[oldOperator] = block.timestamp + delay;
    }
    ...
    function withdraw(
        ...
        if (!isActiveWithdrawalAddress(destination))
        ...
        for (uint256 i = 0; i < signatures.length; i++) {
            ...
            if (!isActiveOperator(recoveredSigner)) revert NotActiveOperator();
    ...
    function addWithdrawalAddress(address recipient) public {
        if (msg.sender != admin) revert Unauthorized();
        if (isActiveWithdrawalAddress(recipient)) revert DuplicatedAddress();
        withdrawalAddresses[recipient] = block.timestamp + delay;
        withdrawalAddressesBlocklist[recipient] = 0;
    }
    ...
    function removeWithdrawalAddress(address recipient) public {
        if (msg.sender != admin) revert Unauthorized();
        if (!isActiveWithdrawalAddress(recipient)) revert DuplicatedAddress();
        withdrawalAddressesBlocklist[recipient] = block.timestamp + delay;
    }

    /// @dev Check if the operator is active or not
    /// Active means when the address has been added to the operators list and has not been blocked yet
    /// @param addr address to check if it is active operator or not
    /// @return boolean to indicate if the address is active
    function isActiveOperator(address addr) public view returns (bool) {
        return
            completedTimelock(operators[addr]) &&
            !completedTimelock(operatorsBlocklist[addr]);
    }

    /// @dev Check if the withdrawal address is active or not
    /// Active means when the address has been added to the withdrawal address and has not been blocked yet
    /// @param addr address to check if it is active withdrawal address or not
    /// @return bool to indicate if the address is active or not
    function isActiveWithdrawalAddress(address addr)
        public
        view
        returns (bool)
    {
        return
            completedTimelock(withdrawalAddresses[addr]) &&
            !completedTimelock(withdrawalAddressesBlocklist[addr]);
    }
