    ...
    ERC2981
{
    ...
    constructor(address payable _withdrawAddress)
    {
     ...
     */
    function setPrices(uint256 _price, uint256 _discountedPrice)
        external
        onlyRole(SUPPORT_ROLE)
    {
        _setPrices(_price, _discountedPrice);
    }
    ...
    */
        ...
        finalPriceSet
    {
        ...
        require( contractState == ContractState.WHITELIST, "Private mint closed");
        require(_verify(_merkleProof, sender), "Not in whitelist");
        require( _quantity <= MAX_PER_PRIVATE_WALLET - _alreadyMinted[sender],"Wallet max exceeded");

        _alreadyMinted[sender] += _quantity;
        _internalMint(sender, _quantity, discountedPrice);
    }

    function publicMint(uint256 _quantity) external payable nonReentrant finalPriceSet {
...
contract InetBid is InetMint{
    ...
    struct Bidder {
        uint232 contribution; // cumulative sum of ETH bids
        uint16 tokensClaimed; // tracker for claimed tokens
    ...
    function _processBidder(address _address) internal finalPriceSet{
        ...
        //only airdrop if the bidder has a winning bid.
        //if user has a winning bid and but doesn't have it in time then they will not get a token
        //we know this because refundValue will be the same as contribution
        if (refundValue != bidder.contribution){

            //airdrop tokens
            uint256 totalWon = bidder.contribution / price;

            if (totalWon > 0) {
                require(bidder.tokensClaimed == 0, "Already airdroped");
     ...
     *   has been set.
     * @dev helper function. if max supply is already used then refund amount should equal contribution
     * @param _bidderAddress address to query.
     */
    function _refundAmount(address _bidderAddress) internal view
        returns (uint256)
    {
        if (maxBidSupply <= _totalMinted()){
            return bidderData[_bidderAddress].contribution;
        }
        else{
     ...
     * @dev helper function
     * @param _address address of the caller.
     */
    function _totalWonFromBidding(address _address) internal view
        returns (uint256)
    {
        return bidderData[_address].contribution / price;
    }
}
...
* @dev v.00.90.00
*/
contract InetMint is InetBase, ERC721A {
    ...
    function _verify(bytes32[] calldata _merkleProof, address _sender)
        internal
        view
        returns (bool)
    {
        bytes32 leaf = keccak256(abi.encodePacked(_sender));
        return MerkleProof.processProof(_merkleProof, leaf) == merkleRoot;
    }
...
contract InetBase is ReentrancyGuard, Ownable {
     ...
     */
    function _setPrices(uint256 _price, uint256 _discountedPrice) internal 
    {
