...
import "../sale/CloudVesting.sol";

contract MockCloudVesting is CloudVesting {

    constructor(address _presale, address _cloud, uint[4] memory dates, uint[4] memory percents){
        initialize(_presale, _cloud, dates, percents);
    }

    function setStage(uint stage) external{
        uint delt = block.timestamp - unlockDates[stage];
        for(uint i=0; i<4; i++){
            unlockDates[i]-=delt;
        }
    }
    ...
    function clear() external{
        for(uint i=0; i< 4; i++){
            _claimed[_msgSender()][i]=0;
        }
    }

    function addMeToWhitelist() external{
        whitelist[_msgSender()] = true;
    }

     function removeMeFromWhitelist() external{
...
import "../interfaces/ICLOUDPresale.sol";
...
contract CloudVesting is OwnableUpgradeable {
        ...
        for (uint256 i = 0; i < STAGES; i++) {
            if (unlockDates[i] < block.timestamp) {
                if (_claimed[_msgSender()][i] == 0) {
                    ...
                    IERC20Upgradeable(CLOUDToken).safeTransfer(_msgSender(), payout);
     ...
     * @return _totalClaimed The amount of the token claimed.
     */
