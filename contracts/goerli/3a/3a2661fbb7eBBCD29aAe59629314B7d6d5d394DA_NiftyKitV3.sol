...
contract NiftyKitV3 is INiftyKitV3, Initializable, OwnableUpgradeable {
    ...
    EnumerableSetUpgradeable.AddressSet private _collections;
    mapping(address => INiftyKitV3.Entry) private _rateOverrideByCollection;
    mapping(address => INiftyKitV3.Entry) private _rateOverrideByUser;
    ...
    function initialize(address appRegistry_, address diamondImplementation_)
        public
        initializer
    {
    ...
    function commission(address collection, uint256 amount)
        ...
        returns (uint256)
    {
        uint256 rate = _rateOverrideByCollection[collection].isValue
            ? _rateOverrideByCollection[collection].value
            : _rate;
        ...
        onlyOwner
    {
        ...
        _rateOverrideByCollection[collection].isValue = true;
        _rateOverrideByCollection[collection].value = rate;
    }
    ...
    function createDiamond(
        ...
        if (address(_dropKitPass) != address(0)) {
            _rateOverrideByCollection[deployed].isValue = true;
            _rateOverrideByCollection[deployed].value = _dropKitPass
                .getFeeRateOf(_msgSender());
        }

        if (_rateOverrideByUser[_msgSender()].isValue) {
            _rateOverrideByCollection[deployed].isValue = true;
            _rateOverrideByCollection[deployed].value = _rateOverrideByUser[
                _msgSender()
            ].value;
        }
