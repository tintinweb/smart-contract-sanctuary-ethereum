...
contract Elemxnts is ERC1155Supply, Ownable {
    string public website = "https://ghxsts.com";
    string public name_ = "Elemxnts";
    string public symbol_ = "ELEMXNTS";
    ...
    function createElemxnts(Elemxnt[] memory els) internal {
        for (uint256 i; i < els.length; i++) {
            ELEMXNTS[i] = Elemxnt({
                metadataURI: els[i].metadataURI,
                maxSupply: els[i].maxSupply
            });
        }
    }
    ...
    function ownerMint(address _to, MINT[] calldata mintData)
        external
        onlyOwner
    {
    ...
    function allowlistMint(
        ...
        for (uint256 i; i < mintData.length; i++) {
            ...
            require(
                mintData[i].qty + totalSupply(mintData[i].id) <=
                    ELEMXNTS[mintData[i].id].maxSupply,
                "Max supply reached"
            );
    ...
    function uri(uint256 id) public view override returns (string memory) {
        ...
        return string(abi.encodePacked(ELEMXNTS[id].metadataURI));
    }

    function getElemxnt(uint256 id) external view returns (Elemxnt memory) {
        return ELEMXNTS[id];
    }

    function getMintedQty(address addr) external view returns (bool) {
        return minted[addr];
    }
