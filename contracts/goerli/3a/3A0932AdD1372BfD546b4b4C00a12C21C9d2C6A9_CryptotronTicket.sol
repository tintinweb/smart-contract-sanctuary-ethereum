...
contract CryptotronTicket is ERC721, ERC721Enumerable, ERC721Burnable {
    ...
    /**
     * @dev Function that's being used by lottery contract to change the URI of the winning ticket
     */
    function _resetWinningURI(uint256 tokenId) external virtual onlyLottery {
    ...
    function getWinStatus(uint256 ticketId) public view returns (string memory) {
        if (isLotteryOver) {
            if (ticketId == winner) {
                return "Won";
            } else {
    ...
    function getLotteryStatus(uint256 ticketId) public view returns (string memory) {
        if (isLotteryOver) {
            if (ticketId == winner) {
                return "Win";
            } else {
    ...
    /**
     * @dev used for preparing new URI to get set by lottery contract in case of winning
     */
