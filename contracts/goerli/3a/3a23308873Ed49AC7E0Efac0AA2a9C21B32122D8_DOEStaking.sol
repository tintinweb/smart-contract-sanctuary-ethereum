...
contract DOEStaking is TokenWrapper, NFTWrapper, Ownable {
	...
	function setMaxNftCap(uint8 _maxNftCap) external onlyOwner {
        maxNFTCap = _maxNftCap;
    }
    ...
    function nftReward(uint128 nftBalance) public view returns (uint128) {
        if (multiNftReward) {
            uint128 multiNft = nftBalance > maxNFTCap ? maxNFTCap : nftBalance;
            return
                uint128((rewardPerToken() * rewardPerNFTStored) * multiNft);
        } else {
            return uint128(rewardPerToken() * rewardPerNFTStored);
        }
    }
