...
contract PublicSales is Ownable {
    ...
    // coin price
    uint256 public constant coinPriceUSD = 2 * priceDecimal;

    // public sale enable flags
    ...
    */
    ...
    /**
     * @dev Buy the coins in public sales Coins supply with USDC coin
     * @param amount USDC coins to be transferred
     * @param splitNFTCount nft count
     * @param coinType type of coin: USDC, USDT, BUSD
    */

    function buy(uint256 amount, uint256 splitNFTCount, Coin coinType) external {
        require(publicSalesEnabled, "PublicSales: sale is not enabled");
        require(splitNFTCount <= getMaxSplitCount(amount), "PublicSales: split count exceed it's limit");
        require(amount >= priceLimit, "PublicSales: amount should be greater than price limit ");
        ...
        seasideCoin.transfer(address(seasideNFT), amount);
        seasideNFT.mint(
            msg.sender, 
            seasideCoins, 
            splitNFTCount, 
    ...
    /**
     * @dev airdroping the coins to user from public sales Coins supply, with USDC/BUSD/USDT coin
     * @param amount USDC coins to be transferred
     * @param splitNFTCount nft count
     * @param coinType type of coin: USDC, USDT, BUSD
    */

    function airdrop(address to, uint256 amount, uint256 splitNFTCount, Coin coinType) external {
        require(publicSalesEnabled, "PublicSales: sale is not enabled");
        require(splitNFTCount <= getMaxSplitCount(amount), "PublicSales: split count exceed it's limit");
        require(amount >= priceLimit, "PublicSales: amount should be greater than price limit ");
