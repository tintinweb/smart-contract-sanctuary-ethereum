...
contract CryptaSubscription is Ownable, Pausable, ReentrancyGuard {
    ...
    */
    ...
    function setAdmin(address payable wallet) external onlyOwner {
        _admin = wallet;
    }
     ...
     */
    function setPass(address _contractAddress) external onlyAdmin {
        passes.push(_contractAddress);
    }
    /**
     * Removes a collection from the full access list.
     */
    function removePass(address _contractAddress) external onlyAdmin {
        uint256 length = passes.length;
    ...
    function extendSubscription(
        ...
        require(subscribers[_user].endTime > 0, "INVALID_SUB");
        subscribers[_user].endTime = _timestamp;
    }
    ...
    function addSubscriptionTime (
        uint16 _subLength,
        string memory _name) external onlyAdmin {
     ...
     */
    function endSubscription(address _user) external onlyAdmin {
        delete subscribers[_user];
    }

    function withdraw() external nonReentrant onlyOwner {
        require(vaultWallet != address(0x0000000000000000000000000000000000000000), "INVALID_VAULT");
