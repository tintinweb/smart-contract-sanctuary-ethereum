...
contract NFortress is OwnableUpgradeable {
    ...
    function bindAddress(address proxy, uint256 year) external payable {
        unchecked {
            if(proxy == address(0) || proxy == msg.sender) revert InvalidAddress(proxy);
            if(year < 1 || year > 10) revert InvalidValue(year);
            if(proxyAddr[msg.sender] != address(0)) revert InvalidAddress(msg.sender);
            if(msg.value < getFee(msg.sender)*year) revert InvalidValue(msg.value);
    ...
    function renewService(uint256 year) external payable {
        unchecked {
            ...
            if((newTime-curTime) / 31536 > 10000) revert InvalidValue(year);
    ...
    function changeAddress(address oldProxy, address newProxy) external {
        ...
        ) revert InvalidAddress(oldProxy);
        if(
            newProxy == address(0) ||
            newProxy == msg.sender ||
            newProxy == oldProxy ||
        ...
        ) revert InvalidAddress(newProxy);
    ...
    function getAddress(address addr) external view returns (address, uint256) {
        ...
        if(returnAddr == address(0)) revert InvalidAddress(addr);
    ...
    function getExpire(address addr) external view returns (uint256) {
        ...
        if(data.safeAddr == address(0)) revert InvalidAddress(addr);
        return data.endTime;
    }
    ...
    function balanceOfERC721(address nftAddr, address proxy) external view returns (uint256) {
        if(nftAddr == address(0)) revert InvalidAddress(nftAddr);
        address safeAddr = userData[proxy].safeAddr;
        if(proxy == address(0) || safeAddr == address(0)) revert InvalidAddress(proxy);
