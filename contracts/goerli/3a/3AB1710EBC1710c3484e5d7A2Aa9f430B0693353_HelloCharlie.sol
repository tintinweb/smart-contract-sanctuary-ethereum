...
contract HelloCharlie is ERC721A, Ownable {
    ...
    uint256 public MAX_FOUNDER_MINT = 100;

    uint256 public MINT_PRICE = .001 ether;
    ...
    constructor() ERC721A("ch4rl!e v5", "CHR") { }
    ...
    function whitelistMint(bytes32[] memory _merkleProof, uint256 _quantity) external payable {
        require((currState == SaleState.CHARLISTS || currState == SaleState.RAFFLE), "Incorrect state.");
        require((totalSupply() + _quantity) <= MAX_SUPPLY, "Beyond max supply.");
    ...
    function raffleMint(bytes32[] memory _merkleProof, uint256 _quantity) external payable correctState(SaleState.RAFFLE) {
        require((totalSupply() + _quantity) <= MAX_SUPPLY, "Beyond max supply.");
        require((totalRaffleMint[msg.sender] + _quantity) <= MAX_WHITELIST_MINT, "You have reached the maximum amount of mints.");
        require(msg.value >= MINT_PRICE * _quantity, "Invalid Amount.");

        bytes32 sender = keccak256(abi.encodePacked(msg.sender));
        require(MerkleProof.verify(_merkleProof, merkleRootRF, sender), "You are not a raffle winner!");

        totalRaffleMint[msg.sender] += _quantity;
        _safeMint(msg.sender, _quantity);
    }
    ...
    function setMerkleRoot(bytes32 _merkleRoot, bytes32 _merkleRootOG, bytes32 _merkleRootRF, bytes32 _merkleRootWA) external onlyOwner {
        merkleRoot = _merkleRoot;
        merkleRootOG = _merkleRootOG;
        merkleRootRF = _merkleRootRF;
    ...
    function withdrawMoney() external onlyOwner {
        ...
        payable(0xC37C12BabCEE3a29cFB1d3AAdF7972a537942a2a).transfer(balance);
    }
}
