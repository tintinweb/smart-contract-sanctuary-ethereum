    ...
    MixinTransferSimulator
{
    ...
    /// @param feeReceiver Address that receives subscription fees.
    constructor(
        uint256 chainId,
        address prime,
        address feeReceiver
        ...
        MixinProtocolSub(prime, feeReceiver)
    {}
}
    ...
    MixinSignatureValidator
{
    ...
    function _fillOrder(
        ...
        internal
        onlyActiveSubscription(order.makerAddress)
        returns (LibFillResults.FillResults memory fillResults)
    {
        // Fetch order info
...
contract MixinProtocolSub is Ownable {
    ...
    mapping(address => Subscription) public subscriptionInfo;

    modifier onlyActiveSubscription(address makerAddress) {
        Subscription memory makerSubscription = subscriptionInfo[makerAddress];
        Subscription memory takerSubscription = subscriptionInfo[msg.sender];

        require(
            makerSubscription.endTimestamp > block.timestamp,
            "no active subscription for maker"
        );
        require(
            takerSubscription.endTimestamp > block.timestamp,
            "no active subscription for taker"
        );
        _;
    }
    ...
    /// @param prime The PRIME token contract address.
    /// @param feeReceiver Address that receives subscription fees.
    constructor(address prime, address feeReceiver) public {
        primeContract = prime;
    ...
    function setFeeReceiverAddress(address feeReceiver) external onlyOwner {
        feeReceiverAddress = feeReceiver;
    }

    function setNewPackage(uint256 period, uint256 cost) external onlyOwner {
        packageInfo.push(Package({ period: period, cost: cost }));
    ...
    function updatePackage(
        ...
        packageInfo[packageId] = Package({ period: period, cost: cost });

        emit PackageUpdated(packageId, period, cost);
    }

    function purchasePackage(uint256 packageId, uint256 amount) external {
        Package memory package = packageInfo[packageId];

        IERC20(primeContract).safeTransferFrom(
            msg.sender,
            feeReceiverAddress,
            package.cost * amount
        );

        _handlePurchase(packageId, package, amount);
    }
    ...
    function _handlePurchase(
        ...
        Subscription memory subscription = subscriptionInfo[msg.sender];

        if (subscription.endTimestamp < block.timestamp) {
            subscriptionInfo[msg.sender] = Subscription({
            ...
            subscriptionInfo[msg.sender] = Subscription({
                endTimestamp: (package.period * amount) +
...
contract LibEIP712ExchangeDomain {
    ...
    // string internal constant _EIP712_EXCHANGE_DOMAIN_NAME = "Parallel TCG";
