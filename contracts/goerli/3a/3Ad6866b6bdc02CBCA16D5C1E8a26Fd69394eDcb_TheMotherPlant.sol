...
contract TheMotherPlant is ERC721A, Ownable, DefaultOperatorFilterer {
    ...
    function whitelistMint(
        ...
        require(wlActive, "Whitelist is not active or has already ended");
        ...
        // require(
        //     balanceOf(msg.sender) < maxPerWalletPreSale,
        //     "Exceeds wallet limit for presale"
        // ); // usar el mapping
        require(
            quantity <= maxMintAmountPerTxPreSale,
            "Exceeds presale txn limit"
        );
        require(price * quantity <= msg.value, "ETH sent is incorrect.");
        require(
            whitelistClaimed[msg.sender] + quantity <= maxPerWalletPreSale,
            "Max amount minted for whitelist"
        );
        unchecked {
