...
contract EnsanEditions is GsERC1155Merkle, DefaultOperatorFilterer {
    ...
    constructor() GsERC1155Merkle("Ensan Editions", "EES", "https://www.geeks.solutions", msg.sender, 1000) DefaultOperatorFilterer() {} 
    ...
    function ownerMint(uint256 _token_id, address _receiver, uint256 _amount) external onlyOwner {
        ...
        super.elligible_mint(_token_id, _amount, false, getTokenData(_token_id).max_supply, 0);
...
contract GsERC1155Merkle is GsERC1155 {
    ...
    /// @param _wl_max_mints_per_wallet The maximum amount of tokens one can mint using this whitelist
    ...
    function addNewWhitelist(uint256 _token_id, uint256 _wl_price_in_wei, bytes32 _merkle_root, 
        uint8 _wl_max_mints_per_wallet, bool _active, bool _use_dynamic_price) public editors tokenExist(_token_id)
    returns(uint256){
        whitelist_datas[_token_id].push(
            WhitelistData({
                ...
                max_mints_per_wallet: _wl_max_mints_per_wallet,
    ...
    /// @param _wl_max_mints_per_wallet The maximum amount of tokens one can mint using this whitelist
    /// @param _use_dynamic_price a boolean to indicate if we should use the dynamic price or not
    function editWhitelistFull(uint256 _whitelist_index, uint256 _token_id, uint256 _wl_price_in_wei, bytes32 _merkle_root, 
        uint8 _wl_max_mints_per_wallet, bool _active, bool _use_dynamic_price) public editors whitelistExist(_whitelist_index, _token_id) {
        editWhitelist(whitelist_datas[_token_id][_whitelist_index], _token_id, _whitelist_index, _merkle_root, _active, _wl_price_in_wei, _wl_max_mints_per_wallet, _use_dynamic_price);
    ...
    function editWhitelistPartial(uint256 _whitelist_index, uint256 _token_id, bytes32 _merkle_root, bool _active, bool _use_dynamic_price) public editors whitelistExist(_whitelist_index, _token_id) {
        WhitelistData memory whitelist = whitelist_datas[_token_id][_whitelist_index];
        editWhitelist(whitelist, _token_id, _whitelist_index, _merkle_root, _active, whitelist.token_price, whitelist.max_mints_per_wallet, _use_dynamic_price);
    }
    ...
    function editWhitelist(WhitelistData memory _whitelist, uint256 _token_id, uint256 _whitelist_index, bytes32 _merkle_root, bool _active,
        uint256 _wl_price, uint8 _wl_max_mints_per_wallet, bool _use_dynamic_price) internal {            
        ...
        _whitelist.max_mints_per_wallet = _wl_max_mints_per_wallet;
    ...
    /// @return length
    function getWhiteListLengthForToken(uint256 _token_id) public view returns(uint256) {
    ...
    ///
    /// @return price
    function getMintTotalPrice(address account, uint256 token_id, uint256 whitelist_index, uint256 amount) public view whitelistExist(whitelist_index, token_id) 
    returns(uint256 price) {
        WhitelistData memory _whitelist = getWhiteListAtIndex(token_id, whitelist_index); 
        if (_whitelist.use_dynamic_price) {
            return super.extractTotalPrice(token_id, _whitelist.token_price, totalSupply(token_id), mintsOf(account, token_id), amount);
        }
    ...
    returns(TokenData memory td, bool isPaused, uint256 supply, uint256 mintsCount, WhitelistData memory wl){
       (td, isPaused, supply, mintsCount) = super.initFront(account, token_id);
       wl = getWhiteListAtIndex(token_id, whitelist_index);
    }
    ...
    returns(uint256 total) {
        ...
        return super.elligible_mint(_token_id, _amount, whitelist.use_dynamic_price, whitelist.max_mints_per_wallet, whitelist.token_price);
    }
...
import "./extensions/ERC1155MintCount.sol";
...
/// token SUPPLY or wallet MINTS count or a combination of both
...
/// https://sections.geeks.solutions to get a ready frontend solution to run
/// your mint page that connects seamlessly to this contract
contract GsERC1155 is ERC1155MintCount, ERC1155Supply, ERC1155Burnable, Ownable, Pausable, IERC2981 {
    ...
    constructor(string memory _name, string memory _symbol, string memory _contractURI, address _recipient, uint16 _royaltyShare) ERC1155("https://geeks.solutions") {
        ...
        _default.max_mints_per_wallet = 1;
        _default.exist = true;
        _default.royalty_recipient = _recipient;
        _default.use_dynamic_price = true;
    ...
    function getDecreaseMintCountOnBurn() public view returns (bool) {
        return _decreaseOnBurn;
    }

    /**
     * @dev When set to true, burning a token will decrease the mints count for a wallet
     * @param _decrease the boolean to activate or deactive decreasing mint counts when burning a token
     */
    function setDecreaseMintCountOnBurn(bool _decrease) public onlyOwner {
        _decreaseOnBurn = _decrease;
    }
    ...
    function enableEditor(address editor) public onlyOwner {
        _editors[editor] = true;
    }

    function disableEditor(address editor) public onlyOwner {
    ...
    */
    ...
    function setDefaultTokenMeta(uint32 _maxSupply, uint8 _maxMintsPerWallet, uint256 _price_in_wei) public onlyOwner {
        _default.max_supply = _maxSupply;
        _default.max_mints_per_wallet = _maxMintsPerWallet;
    ...
    function setDefaults(address _recipient, uint16 _share,
        ...
        uint32 _maxSupply, uint8 _maxMintsPerWallet, uint256 _price_in_wei) external onlyOwner {
    ...
    /// @param _tokenMaxMintsPerWallet The maximum amount of token a wallet can mint
    ...
    function addNewTokenLight(uint256 _tokenId, string calldata _tokenUri) public editors {
            addToken(_tokenId, _tokenUri, _default.public_sale, _default.burnable, _default.max_supply, _default.max_mints_per_wallet, 
    ...
    function addToken(uint256 _tokenId, 
        ...
        bool _use_dynamic_price) internal {
            if(exists(_tokenId)) revert ExistingToken();
            token_datas[_tokenId] = TokenData({
                ...
                max_mints_per_wallet: _tokenMaxMintsPerWallet,
                exist: true,
    ...
    /// @param _maxMintsPerWallet The maximum number of token a wallet can mint
    /// @param _price_in_wei The new price in GWEI (https://eth-converter.com/)
    function editTokenMeta(uint256 _tokenId, uint32 _maxSupply, uint8 _maxMintsPerWallet, uint256 _price_in_wei) public editors {
        if(exists(_tokenId)) revert TokenLockedBySupply();
        token_datas[_tokenId].max_supply = _maxSupply;
        token_datas[_tokenId].max_mints_per_wallet = _maxMintsPerWallet;
    ...
    /// @param token_id the token id to get the price for
    /// @return price 
    // function getTokenPrice(address account, uint256 token_id) public view tokenExist(token_id) 
    // returns(bool isDynamicPrice, uint256 price){
    //     return extractDynamicPrice(token_id, token_datas[token_id], totalSupply(token_id), mintsOf(account, token_id));
    // }

    function getMintTotalPrice(address account, uint256 token_id, uint256 amount) public view tokenExist(token_id) 
    returns(uint256 price) {
        TokenData storage td = token_datas[token_id];
        if (td.use_dynamic_price) {
            return extractTotalPrice(token_id, td.base_price, totalSupply(token_id), mintsOf(account, token_id), amount);
        }
        return td.base_price * amount;
    }

        function initFront(address account, uint256 token_id) public view virtual tokenExist(token_id)
        returns(TokenData memory td, bool isPaused, uint256 supply, uint256 mintsCount) {
        return (token_datas[token_id], super.paused(), totalSupply(token_id), mintsOf(account, token_id));
    }
    ...
    function addPriceRules(uint256 _token_id, GsDynamicPrice.PriceRule[] calldata _ordered_rules) public editors tokenExist(_token_id) {
    ...
    function updatePriceRules(uint256 _token_id, uint8 _priority_index, GsDynamicPrice.PriceRule[] calldata _ordered_rules) public editors tokenExist(_token_id) {
        if(_priority_index >= price_conditions[_token_id].length) revert IndexOutOfRange();
        if(!GsDynamicPrice.checkValidRulesList(_ordered_rules)) revert GsDynamicPrice.InvalidRule();
    ...
    */
    function burn(address account, uint256 id, uint256 amount) public whenNotPaused override(ERC1155Burnable){
    ...
    */
    function burnBatch(address account, uint256[] memory ids, uint256[] memory amounts) public whenNotPaused override(ERC1155Burnable){
    ...
    function _beforeTokenTransfer(
        ...
        uint256[] memory amounts,
        bytes memory data
    ) internal whenNotPaused virtual override(ERC1155, ERC1155Supply, ERC1155MintCount) {
    ...
    returns(uint256 dynamicPrice) {
        ...
        return elligible_mint(id, amount, td.use_dynamic_price, td.max_mints_per_wallet, td.base_price);
    }
    ...
    /// @param _wallet_max_mints the max_mints_per_wallet condition to check (could come from a whitelist entry)
    /// @param _default_price the price to apply in case no dynamic price were found or it is disabled
    function elligible_mint(uint256 _token_id, uint256 _amount, bool _load_dynamic_price, uint _wallet_max_mints, uint _default_price) view internal tokenExist(_token_id)
    returns(uint256 dynamicPrice) {
        //TokenData storage td = token_datas[_token_id];
        uint256 _totalSupply = totalSupply(_token_id);
        uint256 mintsCount = mintsOf(_msgSender(), _token_id);
        if(tx.origin != _msgSender()) revert ContractMint();
        if(_totalSupply + _amount > token_datas[_token_id].max_supply) revert MaxSupply();
        if(mintsCount + _amount > _wallet_max_mints) revert MaxMintsCount();
    ...
    function extractTotalPrice(uint256 _token_id, uint256 _base_price, uint256 total_supply, uint256 mintsCount, uint256 amount) view internal 
    returns(uint256) {
        uint256 value;
...
library GsDynamicPrice {
    ...
    returns (bool triggered, uint price){
        ...
        for (uint32 i = 0; i < length; i++) {
            ...
            if (!value_found) {
                if (rule.trigger < value && i == length - 1) {
            ...
            if (rule.trigger < value + amount  && 
                (i == length - 1 || (i < length - 1 && ordered_rules[i + 1].trigger > value + amount))) {
...
/**
 * @author www.geeks.solutions
 * @dev Extension of ERC1155 that adds tracking of wallet mints per id.
 *
 * Useful for scenarios where mints have to be tracked per wallet
 */
abstract contract ERC1155MintCount is ERC1155 {
    // Mapping from token ID to account mints
    mapping(uint256 => mapping(address => uint256)) private _mints;
    bool internal _decreaseOnBurn;

    /**
     * @dev Number of mints performed by account on token id
     *
     * Requirements:
     *
     * - `account` cannot be the zero address.
     */
    function mintsOf(address account, uint256 id) public view virtual returns (uint256) {
        if(account == address(0)) revert ZeroAddress();
        return _mints[id][account];
    }

    /**
     * @dev Batch number of mints performed by account on token id
     *
     * Requirements:
     *
     * - `accounts` and `ids` must have the same length.
     */
    function mintsOfBatch(address[] memory accounts, uint256[] memory ids)
        public
        view
        virtual
        returns (uint256[] memory)
    {
        ...
        uint256[] memory batchMints = new uint256[](accounts.length);

        for (uint256 i = 0; i < accounts.length; ++i) {
            batchMints[i] = mintsOf(accounts[i], ids[i]);
        }
    ...
    function _beforeTokenTransfer(
        ...
        if (from == address(0)) {
            for (uint256 i = 0; i < ids.length; ++i) {
                _mints[ids[i]][to] += amounts[i];
            }
        }


        if (_decreaseOnBurn && to == address(0)) {
            for (uint256 i = 0; i < ids.length; ++i) {
                uint256 id = ids[i];
                uint256 amount = amounts[i];
                uint256 mints = _mints[id][from];
                if (mints <= amount) {
                    unchecked {
                        _mints[id][from] = 0;
                    }
