...
contract TheFryingPan is Ownable, IERC721Receiver, ReentrancyGuard {
	...
	uint256 public MINIMUM_TO_EXIT = 0;
	 ...
	 */
	function stake(uint256[] calldata tokenIds) external notContract nonReentrant {
	 ...
	 */
	function claim(uint256[] calldata tokenIds, bool unstake) external notContract _updateEarnings nonReentrant {
		uint256 owed = 0;
		for (uint256 i = 0; i < tokenIds.length; i++) {
			if (!isCop(tokenIds[i])) {
				//Bacon pay Bacon tax
	 ...
	 * @param tokenIds the IDs of the tokens to claim earnings from
	 */
	function rescue(uint256[] calldata tokenIds) external notContract nonReentrant {
