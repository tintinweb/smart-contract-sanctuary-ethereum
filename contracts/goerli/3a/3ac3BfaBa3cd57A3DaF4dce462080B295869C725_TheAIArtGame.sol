...
contract TheAIArtGame is Ownable, ERC2981, ERC721A, ReentrancyGuard {
    ...
    function loopCssSplit(
        ...
        for (uint256 i = 0; i < vars.length; i++) {
            ...
            string memory str = strings;
            if (i == 1) {
                str = "";
                for (uint256 j = 0; j < letters.length; j++) {
                    uint256 random = generateNumber((_seed + 69), 3);
                    if (random == 1) {
                        str = string(abi.encodePacked(str, "_"));
                    } else str = string(abi.encodePacked(str, letters[j]));
                }
            }
            newCss = abi.encodePacked(newCss, "{", vars[i], str, "\\''},");
            if (i == 2) {
                //generate random 0 - 360 degrees
                str = Strings.toString(generateNumber((_seed + 69), 360));
                newCss = abi.encodePacked(newCss, "{", vars[i], str, "deg'},");
            }
            if (i == 3) {
                //generate random 0 - 360 degrees
                str = Strings.toString(generateNumber((_seed + 69), 360) + 90);
                newCss = abi.encodePacked(newCss, "{", vars[i], str, "deg'},");
            }
        }
        return newCss;
    ...
    function animationCreator(
        ...
        string[] memory others = new string[](4);
