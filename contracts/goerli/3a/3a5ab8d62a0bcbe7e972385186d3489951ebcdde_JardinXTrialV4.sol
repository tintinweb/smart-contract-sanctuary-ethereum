    ...
    ReentrancyGuardUpgradeable
{
        ...
        whenPauseSA
    {
        ...
        emit TokenWithdrawn(from, amount, block.timestamp);
    }
    ...
    function trade(
        ...
        if (from.length > 1) {
            require(
                bidStatus(to[0], cid, sumAmountBid) == 1,
                "insufficient bid balance"
            );
            for (uint256 i = 0; i < from.length; i++) {
                require(
                    askStatus(from[i], 0, amountAsk[i]) == 1,
            ...
            require(
                askStatus(from[0], 0, sumAmountAsk) == 1,
                "insufficient ask balance"
            );
            for (uint256 i = 0; i < from.length; i++) {
                require(
                    bidStatus(to[i], cid, amountBid[i]) == 1,
                    "insufficient bid balance"
                );
