...
contract CoCreateLaunch is ICoCreateLaunch, Initializable, OwnableUpgradeable, UUPSUpgradeable {
  ...
  address[] private createInstances;
  ...
  function initialize(
    ...
    address _owner,
    address _createInstanceFactory,
    address _weth9
  ) public initializer {
    require(_upgradeGate != address(0), "Invalid Upgrade Gate address");
    require(_owner != address(0), "Invalid Owner address");
    require(_createInstanceFactory != address(0), "Invalid CreateInstance Factory address");
    ...
    projectFactory = CoCreateProjectFactory(_createInstanceFactory);
    weth9 = IWETH9(_weth9);
  }
  ...
  function deployCoCreateProject(
    ...
    CoCreateProject instance = projectFactory.deployCoCreateProject(this, admin, _name, _description);
    createInstances.push(address(instance));
    emit CoCreateProjectDeployed(address(instance), _name, _description, admin);
    return instance;
  }

  function getCoCreateProjects() external view override returns (address[] memory) {
