...
/**
 * @dev All the information about a merkle drop mint (combines CollectionMintData with BaseData).
 */
struct MintInfo {
    uint32 startTime;
    uint32 endTime;
...
interface IMerkleDropMinter is IMinterModule {
    ...
    /**
     * @dev Emitted when the `merkleRootHash` is changed for (`collection`, `mintId`).
     * @param collection        Address of the song collection contract we are minting for.
     * @param mintId         The mint ID.
     * @param merkleRootHash The merkle root hash of the entries.
     */
    event MerkleRootHashSet(address indexed collection, uint128 indexed mintId, bytes32 merkleRootHash);
    ...
    /*
     * @dev Sets the `merkleRootHash` for (`collection`, `mintId`).
     * @param collection        Address of the song collection contract we are minting for.
     * @param mintId         The mint ID.
     * @param merkleRootHash The merkle root hash of the entries.
     */
