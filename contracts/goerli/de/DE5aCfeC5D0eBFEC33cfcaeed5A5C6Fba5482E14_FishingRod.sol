    ...
    UUPSUpgradeable
{
    ...
    function mint(address to) external whenNotPaused noContract {
        ...
        if (_booty.balanceOf(sender) < tier.mintPrice) {
            revert InsufficientFunds();
        }

        _booty.burnFrom(sender, tier.mintPrice);
    ...
    function upgrade(uint256[] calldata tokenIds, uint256[] calldata tiers) public virtual whenNotPaused noContract {
        ...
        for (uint256 i; i < tokenIds.length; i++) {
            ...
            totalPrice += _tiers[tier].mintPrice;
        ...
        if (_booty.balanceOf(sender) < totalPrice) {
            revert InsufficientFunds();
        }

        _booty.burnFrom(sender, totalPrice);
    }
