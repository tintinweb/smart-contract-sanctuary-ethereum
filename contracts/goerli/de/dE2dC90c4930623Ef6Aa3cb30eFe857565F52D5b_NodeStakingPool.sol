...
interface INodeValidatorRegistry {
    ...
    function getValidatorByIndex(uint256 _index)
        external
        view
    ...
    AccessControlUpgradeable
{
    ...
    uint256 public constant validatorsETHBalance = 0;
    uint256 public constant totalETHStaked = 0;
    uint256 public constant totalPoolETHRewards = 3000 gwei;

    event ValidatorAddedToRegistry(uint256 validatorIndex);
    ...
    function initialize(address _nvr, address _ssvNetwork) public initializer {
        __Pausable_init();
    ...
    // function to add new validator to the registry
    function addValidatorToRegistry(
        ...
        uint256 validatorIndex = INodeValidatorRegistry(NVR).addValidator(
            _validator,
            _minStake,
            _maxStake,
            _commissionRate,
        ...
        emit ValidatorAddedToRegistry(validatorIndex);
    }

    // register SSV validator
    function registerValidator(uint256 _index, uint256 _amount)
        public
    ...
    AccessControlUpgradeable
{
    ...
    function initialize(NodePoolManager npm_) public initializer {
        __ERC20_init("01NodeEth", "noETH");
    ...
    function totalETHWaitingToBeStaked() public view returns (uint256) {
        return address(this).balance;
    }

    function totalETHStaked() public view returns (uint256) {
        return totalETHWaitingToBeStaked() + _npm.totalETHStaked();
    }

    function totalETHRewards() public view returns (uint256) {
        return _npm.totalPoolETHRewards();
    }
    ...
    function userAssets(address _user) public view returns (uint256) {
        return _convertToAssets(balanceOf(_user), Math.Rounding.Down);
    }

    function userRewards(address _user) public view returns (uint256) {
        uint256 totalPoolRewards = _npm.totalPoolETHRewards();

        return (balanceOf(_user) * totalPoolRewards) / totalSupply();
    }
    ...
    function previewStake(uint256 amount)
        public
        view
        virtual
        returns (uint256)
    ...
    function stake() public payable whenNotPaused returns (uint256) {
        require(msg.value >= MIN_DEPOSIT_AMOUNT, "Stake amount too low");

        uint256 shares = _convertToSharesStake(msg.value, Math.Rounding.Down);
    ...
    function _convertToSharesStake(uint256 assets, Math.Rounding rounding)
        internal
        view
        virtual
        returns (uint256)
    {
        uint256 totalAssets = totalETHStaked() - assets;
        uint256 result = assets.mulDiv(
            totalSupply() + 10**_decimalsOffset(),
            totalAssets + 1,
            rounding
