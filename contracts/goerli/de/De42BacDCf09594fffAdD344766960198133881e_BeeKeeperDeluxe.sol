 ...
 */
contract BeeKeeperDeluxe is OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    mapping(uint16 => HoneyCombDetails) claimedCombsPerHive;

    struct HoneyCombDetails {
        uint8 claimedTimes;
        uint256 lastClaimed;
    }

    // solhint-disable-next-line
    uint256 public CLAIM_EPOCH;
    ...
    event ClaimEpochChanged(uint256 _newEpoch);
    ...
    function initialize() public initializer {
        ...
        CLAIM_EPOCH = 10 days;
        honeyAmountPerCombRarity[HONEY_COMB_RARITY.COMMON] = 1 * 1e18;
        honeyAmountPerCombRarity[HONEY_COMB_RARITY.UNCOMMON] = 2 * 1e18;
        honeyAmountPerCombRarity[HONEY_COMB_RARITY.RARE] = 3 * 1e18;
        honeyAmountPerCombRarity[HONEY_COMB_RARITY.EPIC] = 4 * 1e18;
        honeyAmountPerCombRarity[HONEY_COMB_RARITY.LEGENDARY] = 5 * 1e18;
    }
    ...
    function claimHoneyComb(
        ...
        for (uint256 i; i < _hiveIds.length; i++) {
            ...
            HoneyCombDetails memory honeyCombDetails = claimedCombsPerHive[_hiveId];
            if (
                honeyCombDetails.claimedTimes >= 2 ||
                hives.ownerOf(_hiveId) != msg.sender ||
                (honeyCombDetails.lastClaimed > 0 && block.timestamp - honeyCombDetails.lastClaimed < CLAIM_EPOCH)
            ) continue;

            amountOfHoney += honeyAmountPerCombRarity[_rarity];
            require(honey.balanceOf(msg.sender) >= amountOfHoney, "Not enough Honey");

            honeyCombDetails.claimedTimes++;
            honeyCombDetails.lastClaimed = block.timestamp;
            claimedCombsPerHive[_hiveId] = honeyCombDetails;
            amountOfCombs++;
        ...
        honeyCombs.safeTransferFrom(owner(), msg.sender, uint256(_rarity), amountOfCombs, "");
    }
    ...
    /**
     * @notice sets the new claim epoch for claiming a Comb for the same hive
     * @param _claimEpoch the new Claim Epoch
     */
    function setClaimEpoch(uint256 _claimEpoch) external onlyOwner {
        CLAIM_EPOCH = _claimEpoch;
        emit ClaimEpochChanged(_claimEpoch);
    }
}
