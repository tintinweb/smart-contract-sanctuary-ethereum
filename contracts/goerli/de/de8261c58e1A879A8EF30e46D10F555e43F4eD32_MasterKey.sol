...
import "openzeppelin-contracts/contracts/mocks/EnumerableMapMock.sol";
...
contract MasterKey is ERC721A, ERC2981, AccessControl {
    ...
    uint256 public constant HARD_SUPPLY_CAP = 10_000;

    // TODO: set these final values
    ...
    // default is CLOSED
    Phase public phase;
    uint256 public CURRENT_MAX_SUPPLY;
    uint256 public MAX_TO_COLLABORATORS;
    ...
    string private baseTokenURI = "/path/to/base/token/uri";
    string private collectionContractURI = "/path/to/collection/contract/uri.json";
    ...
    constructor() ERC721A("Master Key", "MKEY") {
        ...
        _setDefaultRoyalty(creatorsAddress, 500);
        ...
        // this cap is across all rounds. There will never be more than 100 tokens minted to collaborators
        MAX_TO_COLLABORATORS = 100;
    }
    ...
    function allowedMintsLeft(address wallet) public view returns (uint256) {
        ...
        uint256 leftForSupply = CURRENT_MAX_SUPPLY - totalSupply();
    ...
    // Withdraws all funds from the contract to the caller
    function withdrawAllFunds() external onlyRole(DEFAULT_ADMIN_ROLE) {
    ...
    function mintTo(address[] calldata wallets, uint256[] calldata amounts) external payable onlyRole(MINTER_ROLE) {
        ...
        if (totalSupply() + totalMinted > CURRENT_MAX_SUPPLY) revert ExceedsMaxSupply();
    }

    // TODO: make this function accept arrays of addresses and amounts
    function mintToCollaborators(address to, uint256 amount) external onlyRole(MINTER_ROLE) {
        ...
        if (phase == Phase.CLOSED) revert PrivateSaleIsClosed();
        // this function does not require value, but its supply is capped.
        // We don't check the total supply or the max per wallet in this case
        if (collaboratorsSupply + amount > MAX_TO_COLLABORATORS) revert ExceedsCollaboratorsSupply();
    ...
    function setRoundCaps(uint256 newSupplyCap, uint256 newMaxPerWallet) external onlyRole(CONTRACT_OPERATOR_ROLE) {
        ...
        if ((newSupplyCap < CURRENT_MAX_SUPPLY) || (newSupplyCap > HARD_SUPPLY_CAP)) revert InvalidNewSupplyCap();
        if (newMaxPerWallet < MAX_PER_WALLET) revert InvalidNewMaxPerWallet();
        CURRENT_MAX_SUPPLY = newSupplyCap;
    ...
    function _batchMint(address to, uint256 amount) internal {
        ...
        if (totalSupply() + amount > CURRENT_MAX_SUPPLY) revert ExceedsMaxSupply();
