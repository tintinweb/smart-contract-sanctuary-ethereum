...
contract ARTECONFTUPGRADEABLE is Initializable, ERC721EnumerableUpgradeable, ERC2981Upgradeable, OwnableUpgradeable {
    ...
    */
    ...
    function initialize() public initializer {
        __ERC721_init("ARTECONFT", "ARTECONFT");
        __Ownable_init();
    }
    ...
    /*
        functions
    */
    function mintToAddress(uint256 tokenId, address _address) external onlyOwner {
        _safeMint(_address, tokenId);
    }
    
    function mintBySignature(bytes memory signature, uint256 tokenId) external payable {
    ...
    function initializeNft(uint256 tokenId, uint256 _priceInWei, uint128 _projectId, bytes32 _hash) external onlyOwner {
        tokenIdToToken[tokenId].priceInWei = _priceInWei;
        tokenIdToToken[tokenId].projectId = _projectId;
        tokenIdToToken[tokenId].metadatafileHash[0] = _hash;
    }
    function _setDefaultRoyalty(uint96 defaultRoyaltyFractionInBps) external virtual onlyOwner {
        _setDefaultRoyalty(this.owner(), defaultRoyaltyFractionInBps);
    }
    function _setTokenRoyalty(uint256 tokenId, uint96 defaultRoyaltyFractionInBps) external virtual onlyOwner {
        _setTokenRoyalty(tokenId, this.owner(), defaultRoyaltyFractionInBps);
    }
    function appendHashes(uint256 tokenId, bytes32 _hash) external onlyOwner {
        (tokenIdToToken[tokenId].metadatafileHash).push(_hash);
    }
    function getHashes(uint256 tokenId) external view returns (bytes32[] memory) {
        return tokenIdToToken[tokenId].metadatafileHash;
    }
    ...
    function setIindividualUrl(uint256 tokenId, string memory _individualUri) external {
        require(ownerOf(tokenId) == msg.sender, "ARTECONFT: Only NFT owner can set individual URI");
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        require(_exists(tokenId), "ARTECONFT: URI query for nonexistent token");
