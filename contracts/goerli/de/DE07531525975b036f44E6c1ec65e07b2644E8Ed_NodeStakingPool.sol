    ...
    PausableUpgradeable
{
    ...
    uint256 public totalETHRewards; // totalValidatorsBalance - totalETHStaked
    ...
    // Add oracle role to address
    function addOracleRole(
        address _address
    ) public onlyRole(DEFAULT_ADMIN_ROLE) {
        grantRole(ORACLE_ROLE, _address);
    }

    // Remove oracle Role from address
    function removeOracleRole(
        address _address
    ) public onlyRole(DEFAULT_ADMIN_ROLE) {
        revokeRole(ORACLE_ROLE, _address);
    }
    ...
    function oracleUpdate(
        ...
        totalETHRewards = totalValidatorsBalance - totalETHStaked;
    }
    ...
    function balancenLETHTokensOf(address _user) public view returns (uint256) {
        return INodeLiquidETH(nLETH).balanceOf(_user);
    }

    function balanceETHOf(address _user) public view returns (uint256) {
        uint256 shares = INodeLiquidETH(nLETH).balanceOf(_user);
        return
            (shares * totalValidatorsBalance) /
            INodeLiquidETH(nLETH).totalSupply();
    }

    function pendingETHToStake() public view returns (uint256) {
        return nLETH.balance;
    }
