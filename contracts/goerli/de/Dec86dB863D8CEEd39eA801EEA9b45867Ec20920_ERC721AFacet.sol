...
contract Diamond is IERC165, IDiamondCut, IDiamondLoupe, Initializable, IERC173 {
    ...
    function setEventReportingContract(address _contract,  address[] memory _owners) external  {
        LibDiamond.enforceIsContractOwner();
        AppStorage storage ds = LibAppStorage.diamondStorage();
        ds.eventReporter = _contract;
        IEventReporter(ds.eventReporter).register(_owners);
    } 

    function setupMetadata(
        AppStorage storage appStorage, 
        address,
        DiamondInitParameters memory params) private {
        DiamondLib.setupMetadata(appStorage, params);
    }

    function initialize(
        address _owner, 
        DiamondInitParameters memory params,
        IDiamondCut.FacetCut[] memory _facets,
        address diamondInit,
        address eventReporter_,
    ...
    function _eventReporter() internal view returns (address){
        AppStorage storage ds = LibAppStorage.diamondStorage();
        return ds.eventReporter;
    }
...
contract ERC721AFacet is Context, Initializable, Modifiers {
    ...
    function totalSupply() external view returns (uint256) {
        return LibAppStorage.diamondStorage().erc721Contracts.totalSupply();
    }
    ...
    function balanceOf(address owner) external view returns (uint256) {
        return LibAppStorage.diamondStorage().erc721Contracts.balanceOf(owner);
    }
    ...
    function ownershipOf(uint256 tokenId) internal view returns (TokenOwnership memory) {
        return LibAppStorage.diamondStorage().erc721Contracts.ownershipOf(tokenId);
    }
    ...
    function getApproved(uint256 tokenId) external view returns (address) {
        return LibAppStorage.diamondStorage().erc721Contracts.getApproved(tokenId);
    }
    ...
    function setApprovalForAll(address operator, bool approved) external {
        ...
        LibAppStorage.diamondStorage().erc721Contracts.
    ...
    function isApprovedForAll(address owner, address operator) public view virtual returns (bool) {
        return LibAppStorage.diamondStorage().erc721Contracts.isApprovedForAll(owner, operator);
    }
    ...
    function _mint(
        ...
        LibAppStorage.diamondStorage().erc721Contracts._mint(msg.sender, to, quantity, _data, safe);
    }
    ...
    function _transfer(
        ...
        LibAppStorage.diamondStorage().erc721Contracts._transfer(msg.sender, from, to, tokenId);
    }
    ...
    function _burn(uint256 tokenId) internal virtual {
        LibAppStorage.diamondStorage().erc721Contracts._burn(tokenId);
    }
    ...
    function _approve(
        ...
        LibAppStorage.diamondStorage().erc721Contracts._tokenApprovals[tokenId] = to;
...
library DiamondLib{
    ...
    function setupMetadata(
        AppStorage storage appStorage, 
        DiamondInitParameters memory params) internal {

        // initialize the metadata
        appStorage.metadata = MetadataContract(
            params.settings.tokenDefinition.name,
            params.settings.tokenDefinition.symbol,
            params.settings.tokenDefinition.description,
            params.settings.tokenDefinition.imageName,
            params.settings.tokenDefinition.imagePalette,
            params.settings.tokenDefinition.externalUrl
        );
    ...
    function initialize(
        ...
        appStorage.eventReporter = eventReporter;

        LibDiamond.setContractOwner(_owner);
        setupInterfaces();

        // set up the gem metadata
        setupMetadata(appStorage, params);

        // get the bitgem storage struct
        DiamondContract storage bg = appStorage.diamondContract;
        
        // create the bitgem data struct
        DiamondSettings memory bitgemSettings = DiamondSettings(
            params.owner,
            params.settings.tokenDefinition
        );

        // initialize the bitgem
        bg.settings = bitgemSettings;
        bg.settings.tokenDefinition.token = tokenAddress;

        // initialize the diamond
        LibDiamond.diamondCut(_facets, diamondInit, _calldata);

        // generate the bitgem token created event
        EventReportingContract(LibAppStorage.diamondStorage().eventReporter).dispatchEvent(
            ...
            "DiamondCreated", 
                ...
                tx.origin, // owner/sender
                bitgemSettings // settings struct
            )
            )
        );        
    }

    function createDiamondInit(DiamondInitParameters memory, address) internal returns (address payable diamondInitAddress) {
        diamondInitAddress = payable ( 
            address(new DiamondInit())
        );
    }

    function mintToken(ERC721AContract storage erc721a, address fromAddress, address toAddress) internal {
        erc721a._mint(fromAddress, toAddress, 1, "", true);
    }
}
...
contract EventReportingContract is Initializable {  
    ...
    event ApplicationEvent(address indexed account, address indexed contractAdddress, bytes32 indexed selector, string name, bytes params);
