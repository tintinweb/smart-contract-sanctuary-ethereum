...
contract Test_VitoshisCastle is ERC721Enumerable, Ownable {

  bytes32 immutable public merkleRoot;
  uint256 public constant TOTAL_SUPPLY = 6302;
  uint256 public constant MINT_PRICE = 0.000000008 ether;
  ...
  constructor(bytes32 _merkleRoot, string memory _baseURI) ERC721("Test_Vitoshis Castle", "T_VITC") {
  ...
  function mintFromWhitelist(bytes32[] calldata _merkleProof) CheckSupply public payable {
    ...
    require(amountClaimed[msg.sender] < WHITELIST_LIMIT_PER_WALLET, "Already Claimed Maximum Per Wallet");
    require(msg.value == MINT_PRICE, "Wrong payment");
    
    amountClaimed[msg.sender] = amountClaimed[msg.sender] + 1;
  ...
  function mintFromSale(uint contestantsToMint) CheckSupply public payable {
    require(publicMintingOpen == true, "Public sale hasn't started yet");
	  require(contestantsToMint <= 6, "Only up to 5 contestants can be minted at once");
