...
contract FarmStaking is IFarmStaking, ERC721Holder, ERC1155Holder, ReentrancyGuard, Initializable, Pausable, Recoverable {
    ...
    function _unstake(uint256 stakingIndex) private {
        require(stakingRecordMap[_msgSender()].length > stakingIndex, "invalid index");
        require(stakingRecordMap[_msgSender()][stakingIndex].status == STAKING_RECORD_STATUS_STAKED, "invalid staking status");

        // claim reward
        farmReward.proxyClaim(_msgSender(), stakingIndex);

        // update status
        Model.StakingRecord storage record = stakingRecordMap[_msgSender()][stakingIndex];
        ...
        // stat staking count
        currentStakingCountMap[_msgSender()] -= record.tokenIds.length;
