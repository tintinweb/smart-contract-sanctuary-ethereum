...
contract BetDex is AccessControl, ReentrancyGuard, Pausable {
    ...
    mapping(uint256 => mapping(uint8 => EnumerableSet.Bytes32Set))
        private gameBets;
    ...
    function createRoomWithToken(
        ...
        address mainOwner,
        address contractAddress
    ) external nonReentrant whenNotPaused onlyRole(ROLE_MANAGER) {
        Room storage room = rooms[roomId];
        require(room.id == 0, "ROOM_ALREADY_EXISTS");
    ...
    function updateRoomOwners(bytes32 roomId, RoomOwner[] calldata owners)
        external
        nonReentrant
        whenNotPaused
        onlyRoomManager(roomId)
    {
        require(rooms[roomId].id != 0, "ROOM_NOT_EXISTS");
        require(
            owners[0].ownerAddress == rooms[roomId].owners[0],
            "MAIN_OWNER_NOT_MATCH"
        );
    ...
    function changeRoomMainOwner(bytes32 roomId, address newOwner)
        external
        nonReentrant
        whenNotPaused
        onlyAdmin
    ...
    function roomSupportedContractAddresses(bytes32 roomId)
        public
        view
    ...
    modifier onlyRoomManager(bytes32 roomId) {
        require(
            rooms[roomId].owners[0] == msg.sender ||
                hasRole(ROLE_MANAGER, msg.sender),
            "PERMISSION_DENIED"
        );
        _;
    ...
    function depositReserves(
        ...
        address contractAddress,
        uint256 _amount
    ) external nonReentrant whenNotPaused onlyOwners(roomId) {
        require(
            rooms[roomId].availableContracts[contractAddress],
            "TOKEN_NOT_EXISTS"
        );
    ...
    function withdrawReserves(
        ...
        // require(
        //     roomBalance > rooms[roomId].totalOwnerDeposit,
        //     "ROOM_NOT_PROFIT"
        // );
        // require(
        //     (roomBalance - rooms[roomId].totalOwnerDeposit) *
        //         rooms[roomId].shares[msg.sender] >=
        //         _amount,
        //     "ROOM_NOT_PROFIT"
        // );
        lockRoomReverse(roomId, contractAddress);
    ...
    function signedBet(
        ...
        uint8 _target,
        uint32 odds,
        BettingAddress calldata addresses,
        uint256 _amount,
        uint32 timestamp,
        uint32 nonce,
        ...
        require(
            (timestamp - timestampThreshold) <= block.timestamp &&
                (timestamp + timestampThreshold) >= block.timestamp,
            "INVALID_TIMESTAMP"
        );
    ...
    function bet(
        ...
        rooms[myBet.roomId].reverses[myBet.contractAddress] += transferAmount;
    ...
    function batchBet(ClientBet[] calldata clientBets)
        external
        nonReentrant
        whenNotPaused
    {
        ...
        for (uint256 i = 0; i < clientBets.length; i++) {
            bet(
                clientBets[i].matchInfo,
                clientBets[i].gameType,
                clientBets[i].target,
                clientBets[i].odds,
                clientBets[i].contractAddress,
                clientBets[i].bettorAddress,
                clientBets[i].amount
            );
        }
    }
