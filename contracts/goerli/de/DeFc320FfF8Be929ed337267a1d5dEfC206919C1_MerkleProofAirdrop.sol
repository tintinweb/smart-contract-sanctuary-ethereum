...
contract MerkleProofAirdrop {
  ...
  mapping(bytes32 => Airdrop) public airdrops;
    ...
    function isClaimed(bytes32 _ipfsHash, address _recipient) public view returns (bool) {
      Airdrop storage airdrop = airdrops[_ipfsHash];
      return airdrop.claimedRecipients[_recipient];
   }

  function createNewAirdrop(bytes32 _root, address _tokenAddress, uint _total, string memory _ipfs) public {
  ...
  function drop(bytes32[] memory proof, address _recipient, uint256 _amount, string memory _ipfs) public {
    ...
    require(!isClaimed(ipfsHash, _recipient), "double spend");
