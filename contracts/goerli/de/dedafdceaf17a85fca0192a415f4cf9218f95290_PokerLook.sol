...
contract PokerLook is ERC721A, Auth {
    using SafeMath for uint256;

    string baseTokenURI;
    bool public isPublic = false;
    uint256 public maxMintNumber = 1500;

    event TokenMint(address indexed to, uint256 tokenId);
    event MaxMintNumber(uint256 _value);

    constructor() ERC721A("PokerLook", "PKL", 1500) Auth(msg.sender) {}

    function setMaxMintNumber(uint256 _maxMintNumber) external authorized {
        require(_maxMintNumber > 1500, "Mint number should be more than initial supply!");
    ...
    function mint(uint256 number, address to) public authorized {
        require(totalSupply().add(number) <= maxMintNumber, "Too many tokens to mint.");
        require(isPublic, "Minting is paused!");
        _safeMint(to, number);
        emit TokenMint(to, number);
    }
