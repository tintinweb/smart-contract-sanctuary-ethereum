...
contract PhiMap is MultiOwner, ERC1155Receiver {
    ...
    function writeObjectToLand(string calldata name, Object calldata objectData,IObject _object) public {
    ...
    function batchWriteObjectToLand(string calldata name, Object[] calldata objectData,IObject _object) external {
        address owner = ownerOfPhiland(name);
        if (owner == address(0)) {
            revert NotReadyPhiland({ sender: msg.sender, owner: owner });
        }
    ...
    function deposit(address _contractAddress,uint256 _tokenId, uint256 _amount,IObject _object) public {
    ...
    function batchDeposit(address _contractAddresses,uint256[] memory _tokenIds, uint256[] memory _amounts,IObject _object) public {
    ...
    function checkDepositStatus(address sender, address _contractAddresses,uint256 _tokenId) public view returns (Deposit memory) {
        return depositInfo[sender][_contractAddresses][_tokenId];
    }

    function undeposit(address _contractAddresses,uint256 _tokenId,IObject _object) public {
        _object.safeTransferFrom(address(this), msg.sender, _tokenId, depositInfo[msg.sender][_contractAddresses][_tokenId].amount, "0x00");
        depositTime[msg.sender][_contractAddresses][_tokenId] += (block.timestamp - depositInfo[msg.sender][_contractAddresses][_tokenId].timestamp);
