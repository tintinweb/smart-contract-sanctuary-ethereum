    ...
    */
    ...
    function buyTickets( bytes calldata signature,  uint256 quantity,  uint256 tokenIdHolding,  uint256 miningLevel) external payable nonReentrant {
        ...
        for(uint256 i= 0;i<quantity; i++){
            ...
            TicketKey[roundId][ticketId] = Ticket(_msgSender(), ticketKey, miningLevel, tokenIdHolding, false, winningOre);
            ticketHolding[msg.sender][roundId].push(ticketId);
            nftTicketHolding[msg.sender][roundId][tokenIdHolding].push(ticketId);
            emit TicketPurchased(ticketId, roundId, _msgSender());
        }
    }
