...
import "../token/bNFTMaker.sol";
import "../token/bNFTTaker.sol";
...
contract MarketFactory is IMarketFactory {
    ...
    address public marketCloneInstance;
    address public bNftMakerCloneInstance;
    address public bNftTakerCloneInstance;

    constructor(
        address _marketCloneInstance,
        address _bNftMakerCloneInstance,
        address _bNftTakerCloneInstance,
        address _config
    ) {
        config = ProtocolConfig(_config);
        bac = IGlobalAccessControl(config.getGAC());

        marketCloneInstance = _marketCloneInstance;
        bNftMakerCloneInstance = _bNftMakerCloneInstance;
        bNftTakerCloneInstance = _bNftTakerCloneInstance;
    }

    function setMarketOriginal(address _marketCloneInstance) external {
        require(
    ...
    function createMarket(address token, address feed)
        external
        override
        returns (address newMarketAddr)
    {
        require(
            bac.userHasRole(bac.GLOBAL_GOVERNANCE_ROLE(), msg.sender),
            "!mf-ggov"
        ...
        Market newMarket = Market(Clones.clone(marketCloneInstance));
        newMarketAddr = address(newMarket);

        newMarket.initialize(token, config.getStable(), address(config), feed);
        newMarket.grantRole(newMarket.DEFAULT_ADMIN_ROLE(), msg.sender);

        config.setMarket(token, newMarketAddr);
        // create new b-NFT-Tokens
        string memory bSymbolTaker = string(
            abi.encodePacked("bNFTt", IERC20Metadata(token).name())
        );
        string memory bNameTaker = string(
            abi.encodePacked(
                "BUMPER wrapped ",
        ...
        string memory bSymbolMaker = string(
            abi.encodePacked("bNFTm", IERC20Metadata(token).name())
        );
        string memory bNameMaker = string(
            abi.encodePacked(
                "BUMPER wrapped ",
                IERC20Metadata(token).name(),
                " maker market NFT"
        ...
        address bMaker = Clones.clone(bNftMakerCloneInstance);
        bNFTMaker(bMaker).initialize(
            bNameMaker,
            bSymbolMaker,
            token,
            address(config)
        ...
        address bTaker = Clones.clone(bNftTakerCloneInstance);
        bNFTTaker(bTaker).initialize(
            bNameTaker,
            bSymbolTaker,
            token,
            address(config)
        ...
        config.setNFT(token, bTaker, bMaker);

        emit Created(token, newMarketAddr, bMaker, bTaker);
    }

    /// @notice Create new ETH market
    function createWrappedNativeMarket(address feed)
        external
        override
        returns (address)
    {
...
interface IMarketFactory {
    ...
    function createWrappedNativeMarket(address feed) external returns (address);
}
...
contract ProtocolConfig is IProtocolConfig, Initializable, BACUpgradable {
    ...
    function getPremiumMultiplier(
        ...
        uint16 periodIndex = getPeriodIndex(term);
        uint16 riskIndex = getRiskIndex(risk);
        return marketConfigs[token].Premium_Mul[riskIndex][periodIndex];
    }

    /// @notice Get yield multiplier to convert base yield to individual maker yield
    function getYieldMultiplier(
        ...
        uint16 periodIndex = getPeriodIndex(term);
        return marketConfigs[token].Yield_Mul[riskIndex - 1][periodIndex];
    }
    ...
    /// @notice Set market configuration parameter
    function setPremiumMultipliers(address token, int128[5][5] calldata mul)
        external
        onlyGovernance
    {
...
{    
    ...
    /// @dev amount of gas, that can be spended
    /// on protect transction and will not be refunded
    uint public constant PROTECT_NO_REFUND_GAS = 200_000;

    /// @dev how much gas should be additionally refunded due
    /// to eth transfer
    uint public constant REFUND_GAS = 30_000;

    event GasRefund(address refundedTo, uint gasToRefund, uint ethRefunded);

    modifier refundExtraGas(uint noRefundGas) { 
    ...
    function premiumWithPenaulty(uint id) public view override returns (uint) {
        TakerPosition memory pos = allTakerPositions[id];
        uint p = calc.premium( address(token), pos ) * 150 / 100; // use 50%     
        (int price,) = price();
        return DIVIDER * p * pos.assetAmount / AP / uint(price);        
    }
    ...
    function yieldWithPenaulty(uint id) public view override returns (int) {
        MakerPosition memory pos = allMakerPositions[id];
        int _yield = (calc.yield(address(token), pos) *
                int(uint(pos.term)) *
                int(pos.stableAmount)) / int(CP);
        if (_yield > 0)
            _yield -= _yield * 50/100;
        else 
            _yield -= (-_yield) * 50/100;
        return _yield;
    }
    ...
    function protect(uint amount, uint16 risk, uint16 term, bool autorenew) 
        public virtual override refundExtraGas(PROTECT_NO_REFUND_GAS) returns (uint id) {
            
        // gas saving
        address _token = address(token);

        // lock BUMP tokens using Bond contract
        uint bumpAmount = bond.takerLock(msg.sender, _token, amount, risk, term );            
        // open position and transfer tokens
        uint id = super.protect( amount, risk, term, autorenew );
        console.log("id: %s, bumpAmount: %s", id, bumpAmount );
    ...
    function protectNative(
        ...
        // update bumpAmount for position
        console.log("bumpAmount:", bumpAmount );
        allTakerPositions[id].bumpAmount = bumpAmount;
        // mint token for position
    ...
    function cancel(uint id) public override {
        // get locked BUMP token amount for this position
        uint bumpAmount = allTakerPositions[id].bumpAmount; 
        // unlock BUMP tokens
        if (bumpAmount > 0) {
    ...
    function deposit(uint amount, uint16 risk, uint16 term, bool autorenew) public virtual override returns (uint id) {
        // lock BUMP tokens using Bond contract
        uint bumpAmount = bond.makerLock(msg.sender, address(token), amount, risk, term );    
        // deposit tokens and open position
        uint id = super.deposit(amount,risk,term, autorenew);
    ...
    /// @notice cancel position (override with NFT burn)
    function abandon(uint id) public override {
        // get locked BUMP token amount for this position
        uint bumpAmount = allTakerPositions[id].bumpAmount; 
        // unlock BUMP tokens
        if (bumpAmount > 0) {
    ...
    function _refundExtraGasFee(
        uint noRefundGas,
        uint totalGasUsed,
        address refundTo
    ) internal {
        console.log("noRefundGas: ", noRefundGas);
        console.log("totalGasUsed: ", totalGasUsed);

        if (noRefundGas >= totalGasUsed) {
            console.log("Nothing to refund");
            return;
        }

        uint gasToRefund = (totalGasUsed - noRefundGas) + REFUND_GAS;
        uint ethRefundNeeded = gasToRefund * tx.gasprice;
        uint currentBalance = address(this).balance;
        uint ethToRefund = currentBalance >= ethRefundNeeded
            ? ethRefundNeeded
            : currentBalance;

        payable(address(refundTo)).transfer(ethToRefund);

        console.log("Tx gas refunded: ", gasToRefund);
        console.log("ETH to refund: ", ethRefundNeeded);
        console.log("Tx ETH refunded: ", ethToRefund);

        emit GasRefund(refundTo, gasToRefund, ethToRefund);
    }
}
...
abstract contract ManagedMarketBase is IMarketManagement, MarketBase {
    ...
    function rebalance() external virtual override onlyArbitrager {
        (int quantity, uint discount) = IRebalancer(rebalancer).calculate(
            address(token)
        );

        address stable = config.getStable();
        address weth = ISwap(swap).WETH();
        address _token = address(token);

        if (_token == weth) {
            if (quantity > 0) {
                ISwap(swap).swapExactTokensForTokens(
                    _token,
                    uint(quantity),
                    stable,
                    0,
                    ISwap(swap).poolFee(_token, stable)
                );
            } else if (quantity < 0) {
                ISwap(swap).swapExactTokensForTokens(
                    stable,
            ...
            ISwap(swap).swapExactTokensForTokensMultRawPath(path, amount, 0);
        }
...
interface IMarket is IMakerPosition, ITakerPosition {
    ...
    BACUpgradable
{
    ...
    function close(uint id) public virtual override takerTermEnds(id) {
        ...
        if (pos.floor > uint(_price)) {
            claimInStableTokens =
                (((DIVIDER_STABLE * (pos.floor - uint(_price))) /
                    DIVIDER_ORACLE) * pos.assetAmount) /
                DIVIDER;
    ...
    function claim(uint id) external virtual override takerTermEnds(id) {
        ...
        uint claimAmount = pos.floor > posSizeInStable
            ? (pos.floor - posSizeInStable)
            : 0;
        if (claimAmount > 0) {
            ...
            stable.safeTransfer(
                pos.owner,
                (DIVIDER_STABLE * claimAmount) / DIVIDER_ORACLE
            );
        }

        emit Claimed(address(this), msg.sender, id, claimAmount, pos.floor);
    }
    ...
    function cancel(uint id) public virtual override takerTermEnds(id) {
        ...
        if (pos.floor > uint(_price)) {
            claimInStableTokens =
                ((pos.floor - uint(_price)) / DIVIDER_ORACLE) *
                pos.assetAmount;
            stable.safeTransfer(pos.owner, claimInStableTokens);
        }
        ...
        )
    {
        return (
            AP,
            token.balanceOf(address(this)) - AP,
            CP,
            stable.balanceOf(address(this)) - CP,
            B,
            L,
            D
        );
...
contract MarketStorage is IMarketStorage {
    ...
    uint public override B; // Book
    uint public override L; // Liability
