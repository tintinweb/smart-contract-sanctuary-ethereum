...
contract ExchangeChallenge is IExchangeChallenge, Ownable, ERC1155Holder{
    ...
    constructor(address token, address loyaltyAddr){
        require(token != address(0), "Challenge Exchange: Not a valid address");
        require(loyaltyAddr != address(0), "Challenge Exchange: Not a valid address");
    ...
    function addLoyaltyChallenge(        
        address collection, 
        uint256 assetId, 
        uint256 amount, 
        uint256 airdropStartAt,
        uint256 airdropEndAt,
        uint256 _loyaltyPercent)external override{
            _addChallenge(collection, assetId, amount, airdropStartAt, airdropEndAt);
            loyalty.addLoyalty(collection, assetId, _msgSender(), _loyaltyPercent);
    }
    /**
...
interface IExchangeChallenge {
    ...
    function addLoyaltyChallenge(        
        ...
        uint256 airdropStartAt,
        uint256 airdropEndAt,
        uint256 _loyaltyPercent) external;
    /**
