...
contract LynkCrowdsale is ReentrancyGuard, Ownable {
        ...
        returns (bool)
    {
        ...
        addOrUpdateUserVesting(_id, amount, _msgSender());
     ...
     * @param amount No of tokens to buy
     */
    function buyWithUSDC(uint256 _id, uint256 amount)
        external
        checkPresaleId(_id)
        checkSaleState(_id, amount)
        returns (bool)
    ...
    /**
     * @dev To buy into a presale using DAI
     * @param _id Presale id
     * @param amount No of tokens to buy
     */
    function buyWithDAI(uint256 _id, uint256 amount)
        external
        checkPresaleId(_id)
        checkSaleState(_id, amount)
        returns (bool)
    {
        ...
        uint256 ourAllowance = DAIInterface.allowance(
            _msgSender(),
            address(this)
        );
        require(usdPrice <= ourAllowance, "Make sure to add enough allowance");
        ...
        require(success, "DAI payment failed");
        emit TokensBought(
            _msgSender(),
            _id,
            address(DAIInterface),
            amount,
            usdPrice,
            block.timestamp
        );
    ...
    function addOrUpdateUserVesting(
        uint256 _id,
        uint256 amount,
        address _user
    ) internal {
    ...
    /**
     * @dev Helper funtion to get USDT/USDC/DAI price for given amount
     * @param _id Presale id
     * @param amount No of tokens to buy
     */
    function stablecoinBuyHelper(uint256 _id, uint256 amount)
        external
        view
        checkPresaleId(_id)
