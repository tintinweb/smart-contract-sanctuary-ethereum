...
contract UFOs is ERC721Enumerable, IERC2981, ReentrancyGuard, Ownable {
    ...
    constructor(IUFOIdentifier newIdentifier, address _vault) ERC721("Prime UFOs", "PUFO") {
        identifier = newIdentifier;
        vault = _vault;
    }

   function mint(uint32 _mintAmount) external payable nonReentrant {
    require(saleIsActive, "Sale needs to be active to mint");
    require(_mintAmount > 0 && _mintAmount <= 100, "Invalid quantity");
    ...
    function generateSeed(uint256 tokenId) private view returns (uint256) {
        ...
        uint256 domeSeed = 100 * (r % 7 + 10) + ((r >> 48) % 20 + 10);
        uint256 hullSeed = 100 * ((r >> 96) % 6 + 10) + ((r >> 96) % 20 + 10);
        uint256 powercoreSeed = 100 * ((r >> 144) % 7 + 10) + ((r >> 144) % 20 + 10);
        uint256 propulsionSeed = 100 * ((r >> 192) % 2 + 10) + ((r >> 192) % 20 + 10);
