 ...
 *          Most operational logics are implemented here while RainCollateral is mainly used to keep collateral.
 *  @dev This is an upgradable contract
 */
contract RainCollateralController is OwnableUpgradeable {
    /// @notice Elliptic Curve Digital Signature Algorithm Used to validate signature
    using ECDSA for bytes32;

    /// @notice address that runs admin functions. Signature should be created by this address.
    address public controllerAdmin;

    /// @notice treasury contract address where Rain Company keeps its treasury.
    address public treasury;

    /// @notice a counter to prevent duplicate withdrawals with same signature
    mapping(address => uint256) public withdrawalNonce;

    /// @notice a counter to prevent duplicate liquidation with same signature
     ...
     */
    modifier isCollateralAdmin(address _collateral) {
        require(
            IRainCollateral(_collateral).isAdmin(address(msg.sender)),
            "Unauthorized"
        );
        _;
    }
     ...
     * @param _controllerAdmin controller admin address to operate collaterals
     * @param _treasury Rain Company's treasury contract address
     */
    function initialize(address _controllerAdmin, address _treasury)
        public
        initializer
    {
        __Ownable_init();
     ...
     * @param _collateral targeting RainCollateral address
     ...
     *       see {_verifyWithdrawSignature} function
     * Requirements:
    ...
    function withdrawAsset(
    ...
    ) external isCollateralAdmin(_collateral) {
        _verifyWithdrawSignature(
            _collateral,
            _asset,
            _amount,
            _expiresAt,
        ...
        IRainCollateral(_collateral).withdrawAsset(_asset, msg.sender, _amount);
    }
    ...
    function makePayment(
        ...
        for (uint256 i = 0; i < _assets.length; i++) {
            _transferToTreasury(_collateral, _assets[i], _amounts[i]);
        }
    }
    ...
    function _transferToTreasury(
        ...
        IRainCollateral(_collateral).withdrawAsset(_asset, treasury, _amount);
    }

    /**
     * @notice Used to verify withdraw signature
     * @dev Uses the ECDSA and check if the signing address = controllerAdmin and increments its nonce
     ...
     *       and withdraw function parameters(collateral,asset,amount,nonce,expiresAt) to generate messageHash,
    ...
    function _verifyWithdrawSignature(
        ...
        bytes32 messageHash = keccak256(
            abi.encodePacked(
                ...
                _asset,
                _amount,
                withdrawalNonce[_collateral],
                _expiresAt
            )
        );
        bytes32 messageDigest = messageHash.toEthSignedMessageHash();
        address signingAddress = ECDSA.recover(messageDigest, _signature);

        // verify that the signature was generated by controllerAdmin address
        require(signingAddress == controllerAdmin, "Invalid signature");

        // update nonce
        withdrawalNonce[_collateral] += 1;
    }

    /**
     * @notice Used to verify payment signature
     * @dev Uses ECDSA and check if signing address = controllerAdmin and increments paymentNonce
     * @param _collateral targeting RainCollateral address
     * @param _assets array of asset's contract addresses
     * @param _amounts array of amounts corresponding to _assets
     * @param _expiresAt timestamp when signature expires as unix seconds
     ...
     *       and parameters (user,collateral,assets,amounts,nonce) to generate messageHash,
     *		 that will be signed using {ethers-signMessage} function.
     */
    function _verifyPaymentSignature(
        ...
        bytes32 messageHash = keccak256(
            bytes.concat(
                ...
                abi.encodePacked(paymentNonce[_collateral], _expiresAt)
            )
        );
        bytes32 messageDigest = messageHash.toEthSignedMessageHash();
        address signingAddress = ECDSA.recover(messageDigest, _signature);
        ...
        // update
        paymentNonce[_collateral] += 1;
    }

    /**
     * @notice Used to liquidate assets owned by RainCollateral contract
     * @dev loop to all assets and transfer them to treasury
     * Requirements:
     * - only controllerAdmin can call this function.
     * @param _collateral targeting RainCollateral contract address
     * @param _assets array of asset's contract addresses
     * @param _amounts array of amounts corresponding to _assets
     */
