...
contract ETHPool is AccessControl {
    ...
    uint256 totalStaked;
    bool depositReward;
    uint256 rewardInterval = 86400 / 24 / 60; // 86400 = 1 day
    uint8 rewardFactor = 1;

    uint256 periodStake = 30 minutes;
    ...
    function calculateReward(address _sender) public view returns (uint256) {
        ...
        uint256 rewardPriceUsd = (((userInfos[_sender].amount *
            getLatestEthPrice(priceFeed)) / 1e8 / 100) *
            rewardFactor *
            (block.timestamp - userInfos[_sender].startTimestamp)) /
            100 /
            rewardInterval;
    ...
    function checkWeekTime() public {
        require(
            block.timestamp - lastRun > periodStake,
