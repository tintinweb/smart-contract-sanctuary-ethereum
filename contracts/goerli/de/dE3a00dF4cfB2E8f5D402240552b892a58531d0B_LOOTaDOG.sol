...
contract LOOTaDOG is ERC3525, Ownable {
    ...
    function mintToken(
        ...
        _verifySign(Voucher(id, tokenId, slot, amount, expirationTime, owner), v, r, s);
        if (tokenId > 0) {
            _mint(msg.sender, tokenId, slot, amount);
        } else {
    ...
    //"Voucher(uint256 id,uint256 tokenId,uint256 slot,uint256 amount,uint256 expirationTime,address owner)";
    bytes32 private constant VOUCHER_TYPEHASH = 0x7f2844ce661b9a5c013d20692ac628dc6edb856ad179a61a36f345a5df283dd0; //keccak256(bytes(VOUCHER_TYPEHASH));
