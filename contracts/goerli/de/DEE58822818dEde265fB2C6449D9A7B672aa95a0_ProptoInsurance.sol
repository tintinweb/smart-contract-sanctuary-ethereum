...
contract ProptoInsurance is Ownable {
    using SafeMath for uint256;

    using Counters for Counters.Counter;

    address public IPTokenContract;
    ...
    uint256 public incomingReserves;

    mapping(uint256 => mapping(address => incomingReservesDetail))
        public incomingReservesMap;

    mapping(uint256 => uint256) public incomingIPTokenAmountMap;

    uint256 public outgoingIPTokens;

    mapping(uint256 => mapping(address => outgoingIPTokenDetail))
        public outgoingIPTokensMap;

    mapping(uint256 => uint256) public outgoingReservesAmountMap;

    address public standardPVRF;

    uint256 public nextResizeReservesVRFIndex;
    ...
    constructor(
        ...
        IPTokenContract = address(ipToken);
    ...
    function initPool() public onlyOwner {
        ...
        IPToken(IPTokenContract).mint(msg.sender, 10000000000000);
        totalReserves = 10000000000000;
    }

    function getIPTokenValue() public view returns (uint256) {
        return
            IERC20(reservesContract).balanceOf(address(this)) /
            IPToken(IPTokenContract).totalSupply();
    }
    ...
    function addInsuranceShare(uint256 amount) public {
        uint256 presentIndex = IPVRF(standardPVRF).getPresentIndex();
        uint256 previousIndex = presentIndex - 1;
        (, uint8 VRFStatus) = IPVRF(standardPVRF).getVRFInfo(previousIndex);
        require(VRFStatus == 0, "Previous round not finished");
        incomingReserves += amount;
        incomingReservesMap[presentIndex][msg.sender].amount += amount;
    }

    function collectIncomingIPToken(uint256 VRFIndex) public {
        require(incomingIPTokenAmountMap[VRFIndex] > 0, "nothing to collect");
        require(
            incomingReservesMap[VRFIndex][msg.sender].amount > 0,
            "nothing to collect for you"
        );
        uint256 amount = (incomingIPTokenAmountMap[VRFIndex] *
            incomingReservesMap[VRFIndex][msg.sender].amount) /
            incomingReserves;
        incomingReservesMap[VRFIndex][msg.sender].collected = true;
        IPToken(IPTokenContract).transferFrom(
            address(this),
            msg.sender,
            amount
        );
    ...
    function removeInsuranceShare(uint256 amount) public {
        require(
            IPToken(IPTokenContract).balanceOf(msg.sender) >= amount,
        ...
        (, uint8 VRFStatus) = IPVRF(standardPVRF).getVRFInfo(previousIndex);
        require(VRFStatus == 0, "Previous round not finished");
        IPToken(IPTokenContract).transfer(address(this), amount);
        IPToken(IPTokenContract).burn(amount);
        outgoingIPTokens += amount;
        outgoingIPTokensMap[presentIndex][msg.sender].amount += amount;
    }

    function collectOutgoingReserves(uint256 VRFIndex) public {
        require(outgoingReservesAmountMap[VRFIndex] > 0, "nothing to collect");
        require(
            outgoingIPTokensMap[VRFIndex][msg.sender].amount > 0,
            "nothing to collect for you"
        );
        uint256 amount = (outgoingReservesAmountMap[VRFIndex] *
            outgoingIPTokensMap[VRFIndex][msg.sender].amount) /
            outgoingIPTokens;
        outgoingIPTokensMap[VRFIndex][msg.sender].collected = true;
        IERC20(reservesContract).transferFrom(
            address(this),
            msg.sender,
            amount
    ...
    function buyInsurance(
        ...
        require(
            ...
            "none validate Propto Predict NFT"
        );
        ...
        require(PVRFContractIndex > 0, "PVRF not in white list");
        ...
        require(ppi.PVRFIndex == presentIndex, "keychain not in present");
        ...
        uint256 payment = calcBuyInsurancePaymeny(
            ppi.PVRFContract,
            ppi.PVRFIndex,
            ppi.amount,
            valuation
        ...
        setRoundLockReserves(ppi, valuation, direction);
        ...
        uint256 proptoPredictCustomIdGroupsIndex = proptoPredictCustomIdGroupsMap[
                PVRFContractIndex
            ][ppi.PVRFIndex][proptoPredictContract][ppi.customId];
        ...
        IProptoPredict(proptoPredictContract).updatePredictInsuranceIndex(
            tokenId,
            index
        );
    }
    ...
    function setRoundLockReserves(
        ...
        for (
            ...
            console.log("fill spot batch");
            console.log(batchInfoIndex);
            console.log("======");
            if (
            ...
            for (uint256 i = 0; i < ranges.length; i++) {
                ...
                console.log(ranges[i].rangeStart);
                console.log(ranges[i].rangeAmount);
                console.log("======");
                (
                ...
                console.log("rangesTemp.length");
                console.log(rangesTemp.length);
                console.log("======");

                for (uint256 j = 0; j < rangesTemp.length; j++) {
                    console.log("unit notinsert start");
                    console.log(rangesTemp[j].rangeStart);
                    console.log(rangesTemp[j].rangeAmount);
                    console.log("======");
                    assembly {
                ...
                console.log("rangesNotInsert len");
                console.log(rangesNotInsert.length);
    ...
    function insertRoundLockRangeUnits(
        ...
        console.log(unit.rangeStart);
        console.log(unit.rangeAmount);
        ...
        for (uint256 k = 0; k < items.length; k++) {
            ...
            console.log(k);
            console.log(items[k].rangeStart);
            console.log(items[k].rangeAmount);
        }
        roundLockReserveses[VRFMachineIndex][VRFIndex]
            .batchInfos[batchInfoIndex]
            .batchRangesUnitNum++;
        ...
        )
    {
        ...
        for (uint256 i = 0; i < info.batchRangesUnitNum; i++) {
            ...
            console.log(item.rangeStart);
            console.log(item.rangeAmount);
            console.log("======");
    ...
    function calcBuyInsurancePaymeny(
        ...
        require(
            valuation <= (totalReserves - totalLockedReserves),
            "insurance reserves not enough"
        );
        uint256 a = valuation;
        ...
        uint256 floatingIntrerest = bignumber
            .mul(a)
            .mul(5)
            .mul(hour)
            .div(totalReserves)
            .div(10000);
        uint256 payment2 = a.mul(floatingIntrerest).div(10000);
        uint256 payment3 = a.mul(5).div(10000);
        console.log("payment");
        console.log(hour);
    ...
    function resizeReserves() public {
        calcVRFRound();
        uint256 presentIndex = IPVRF(standardPVRF).getPresentIndex();
        if (nextResizeReservesVRFIndex == presentIndex) return;
        (, uint8 VRFStatus) = IPVRF(standardPVRF).getVRFInfo(
            nextResizeReservesVRFIndex
        );
        if (VRFStatus != 0) return;

        uint256 outgoingReservesAmount = (totalReserves * outgoingIPTokens) /
            IPToken(IPTokenContract).totalSupply();
        outgoingReservesAmountMap[
            nextResizeReservesVRFIndex
        ] = outgoingReservesAmount;
        totalReserves -= outgoingReservesAmount;
        outgoingReservesAmount = 0;

        uint256 incomingIPTokenAmount = incomingReserves /
            totalReserves /
            IPToken(IPTokenContract).totalSupply();
        incomingIPTokenAmountMap[
            nextResizeReservesVRFIndex
        ] = incomingIPTokenAmount;
        IPToken(IPTokenContract).mint(address(this), incomingIPTokenAmount);
        totalReserves += incomingReserves;
        incomingReserves = 0;

        nextResizeReservesVRFIndex = presentIndex;
    }
    ...
    function calcVRFRound() public {
        ...
        for (uint256 i = 0; i <= _PVRFIndexCounter.current(); i++) {
            ...
            for (
                ...
                (uint8 VStatus, uint256 VerifiedTokenId) = IProptoPredict(
    ...
    function getPVRFS() public view returns (address[] memory PVRFSAddresses) {
        uint256 j = 0;
        for (uint256 i = 1; i <= _PVRFIndexCounter.current(); i++) {
...
contract IPToken is ERC20, ERC20Burnable, Ownable {
    constructor() ERC20("Insurance Providers Token", "IP") {}
...
interface IProptoPredict is IERC721 {
    function safeMint(
        address to,
        address PVRFContract,
        uint256 PVRFIndex,
    ...
    function updatePredictInsuranceIndex(
        uint256 tokenId,
