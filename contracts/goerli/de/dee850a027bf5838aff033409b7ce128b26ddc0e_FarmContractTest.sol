...
/** 
 *  @dev YouWant. Team
 *  Deposit LP tokens to earn rewards in the form of tokens.
 *  This contract mints new tokens everytime a new user deposits LP tokens to the pool
 */
 
contract FarmContractTest is Ownable {
   ...
   struct DepositInfo {
    ...
    bool public enableContract = true;
    ...
    event RetrieveEther(address indexed user, uint256 amount);
    ...
    function claim(uint256 _pid) public validatePoolByPid(_pid) {
        ...
        require(address(this).balance > 0,"claim: The reward ether balance of this contract is zero"); 
        ...
        safeEtherTransfer(msg.sender, deservedReward);
    ...
    function withdraw(uint256 _pid) public validatePoolByPid(_pid) {
        ...
        updatePool(_pid);
        if(pendingReward(_pid, msg.sender) > 0 && address(this).balance > 0){
            uint256 deservedReward = myPendingReward(_pid, msg.sender);
            user.mintAmount = user.mintAmount.add(deservedReward);
            safeEtherTransfer(msg.sender, deservedReward);
            emit Claim(msg.sender, _pid, deservedReward);
    ...
    function safeEtherTransfer(address _to, uint256 _amount) internal {
        uint256 rewardTokenBalance = address(this).balance;
        if (_amount > rewardTokenBalance) {
            payable(_to).transfer(rewardTokenBalance);
        } else {
            payable(_to).transfer(_amount);
        }
    }

    function retrieveForeignTokens(address token) public onlyOwner {
        uint256 rewardTokenBalance = IERC20Contract(token).balanceOf(address(this));
        IERC20(token).transfer(msg.sender, rewardTokenBalance);
    }

    function retrieveEther() public onlyOwner {
        uint256 rewardTokenBalance = address(this).balance;
        safeEtherTransfer(msg.sender, rewardTokenBalance);
        emit RetrieveEther(msg.sender, rewardTokenBalance);
    }
