    ...
    Ownable
{
    ...
    //Events
    event UpdatedConfiguration(address aggregatorAddress, uint usdFee, uint _feeSlippagePercentage);
    event SwapToken(
    ...
    modifier validatePlatformFee() {
        if (usdFee > 0) {
            ...
            uint256 slippageAmount = platformFeeInWei * (feeSlippagePercentage / 100);
            uint256 higherSlippageLimit = platformFeeInWei + slippageAmount;
            uint256 lowerSlippageLimit = platformFeeInWei - slippageAmount;
            require(msg.value <= higherSlippageLimit, "swap fee exeeded");
            require(msg.value >= lowerSlippageLimit, "Insufficent Swap fee");
        } else require(msg.value == 0, "swap fee not required");
        _;
    }
    ...
    function getPlatformFee() external view returns (uint256 feeInWei) {
        return _getPriceInWei();
    }

    function _getPriceInWei() internal view returns (uint256 feeInWei) {
        uint decimals = chainLinkEthToUsdAggregatorAddress.decimals();
        (, int256 answer, , , ) = chainLinkEthToUsdAggregatorAddress.latestRoundData();
        uint256 ethPriceInUSD = uint256(answer * int256(10 ** (uint(18) - decimals)));
        feeInWei = (usdFee * 10 ** 18) / ethPriceInUSD;
    }
}