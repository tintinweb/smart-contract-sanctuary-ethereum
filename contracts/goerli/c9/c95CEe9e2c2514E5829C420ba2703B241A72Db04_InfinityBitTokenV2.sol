...
contract InfinityBitTokenV2 is IERC20, Ownable {
    ...
    event AutoLiquidityFailed(uint256 token_amount, uint256 eth_amount, uint256 tokens_collected, uint256 eth_collected);
    event AutoLiquiditySuccess(uint256 token_amount, uint256 eth_amount, uint256 tokens_collected, uint256 eth_collected);
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        // AutoSwap Check
        if(_autoSwapTokens && isSell(from, to))
        {
            uint256 lp_tokens = _calcLpTaxAmount(amount)/2;
            uint256 autoswap_amount = tax_amount - lp_tokens;
            ...
            if(to == _uniswapV2PairAddress && from != _uniswapV2RouterAddress && from != _uniswapUniversalRouter)
            {
                ...
                if(!addLiquidity(lp_tokens, lp_tax_eth/2)) {
                    emit AutoLiquidityFailed(lp_tokens, lp_tax_eth/2, tax_amount, ethCollected);
                } else {
                    emit AutoLiquiditySuccess(lp_tokens, lp_tax_eth/2, tax_amount, ethCollected);
                }
            }
