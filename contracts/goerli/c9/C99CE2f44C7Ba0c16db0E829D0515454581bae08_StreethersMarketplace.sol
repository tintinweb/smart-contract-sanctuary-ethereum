...
contract StreethersMarketplace is ReentrancyGuard, Ownable {
    ...
    public
    {
        ...
        Streethers(nftContract).approve(address(this), _id);
        Streethers(nftContract).transferFrom(msg.sender, address(this), _id);
        listedMap[_id] = true;
        price[_id] = _price;
    }
    ...
    public
    {
        ...
        Streethers(nftContract).transferFrom(address(this), msg.sender, _id);
        listedMap[_id] = false;
    }
    ...
    function buy(uint[] memory _ids) external payable {
        for (uint i = 0; i < _ids.length; i ++) {
            ...
            Streethers(nftContract).transferFrom(address(this), _newOwner, _ids[i]);
    ...
    function _validate(uint _id) internal view {
        ...
        require(msg.sender != Streethers(nftContract).ownerOf(_id), "Can not buy what you own");
    ...
    function updateListingStatus(uint _tokenId, bool shouldBeListed) public returns (bool) {
        require(msg.sender == Streethers(nftContract).ownerOf(_tokenId), "Error, you are not the owner");
    ...
    function createAuction(
        ...
        //     Streethers(nftContract).mint(tokenId, msg.sender, _tokenUri);
        // }
        ...
        onlyCreatorOrWinner(tokenId)
    {
        ...
        if(winner == address(0)) {
            Streethers(nftContract).transferFrom(address(this), Creator, tokenId);
            ownerMap[tokenId] = Creator;
        } else {
            Streethers(nftContract).transferFrom(address(this), winner, tokenId);
    ...
    function mint(string memory _tokenURI, uint256 _price, bool _isListOnMarketplace, uint256 _royalty) public {
        ...
        Streethers(nftContract).mint(newTokenId, msg.sender, _tokenURI);
    ...
    //     Streethers(nftContract).transferFrom(
...
contract Streethers is ERC721URIStorage, Ownable {

    constructor() ERC721("Streethers NFT", "StreethersNFT") {}
