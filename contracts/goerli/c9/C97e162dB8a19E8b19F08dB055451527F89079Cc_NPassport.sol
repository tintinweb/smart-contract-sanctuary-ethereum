...
contract NPassport is AccessControl, ERC721, Ownable, Pausable {
    ...
    uint256 public subscribeTermSec = 370 * 24 * 60 * 60;
    ...
    mapping(uint8 => uint256) planCosts;
    ...
    // Getter
    function getMintCostByPlanNo(uint8 planNo) public view returns(uint256) {
        return planCosts[planNo];
    }
    ...
    // Subscribe
    function subscribe(address owner) external payable
        existsPassportInfo(owner)
    {
        if (passportInfoByAddress[owner].expiredTimestamp > block.timestamp) {
            require(msg.value >= planCosts[passportInfoByAddress[owner].planNo] , 'Not Enough Eth');
            passportInfoByAddress[owner].expiredTimestamp += subscribeTermSec;
        } else {
            require(msg.value >= planCosts[planNoForPublic] , 'Not Enough Eth');
            passportInfoByAddress[owner].expiredTimestamp = block.timestamp + subscribeTermSec;
            passportInfoByAddress[owner].planNo = planNoForPublic;
        }
    }
