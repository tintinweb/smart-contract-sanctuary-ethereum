...
/// @dev 
///
contract EmrinEthBank2 is ERC721, Ownable, ReentrancyGuard {
  ...
  constructor() ERC721("EmrinEthBank2","EMRIM2"){}
  ...
  function formingDiamondHands(uint _readyTime) external payable {
    ...
    tokenURI(tokenId);
    //need to put an emit in here so the front end can listen for when this is successful
  }

  function deposit(uint _tokenId) external payable {
    require(accounts[_tokenId].accountBalances > 0, "This account is closed and is no longer accepting deposits");
  ...
  // this funciton for somereason does not work calling through etherscan but it does work when interacting with the front end - just leave it?
  ...
  // metadata URI
  // this tokenURI will point to the metadata json
  // the metadata json will have an image tag that has the url for the image in it
  function addTokenURI (uint tokenId) internal {
