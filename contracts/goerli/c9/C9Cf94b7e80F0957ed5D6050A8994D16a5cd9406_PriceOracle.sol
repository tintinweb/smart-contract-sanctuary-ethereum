...
contract PriceOracle is BasePriceOracle {
        ...
        returns (uint256, uint8)
    {
        ...
        console.log("try to get from chainlinkOracle!");
        try chainlinkOracle.getTokenEthPrice(token) returns (uint256 retPrice, uint8 retDecimals) {
                if (retPrice != 0) return (retPrice, retDecimals);
        } catch {
            console.log("catch in getTokenEthPrice chainlinkOracle");
        }

        // Try to get UniswapV3 price
        console.log("try to get from uniswapV3Oracle!");
        try uniswapV3Oracle.getTokenEthPrice(token) returns (uint256 retPrice, uint8 retDecimals) {
            if (retPrice != 0) return (retPrice, retDecimals);
        } catch {
            console.log("catch in getTokenEthPrice uniswapV3Oracle");
        }

        console.log("will use user customize price");
        return (0, 0); // will use user customized token price
    }
    ...
    function getEthUsdPrice() external view returns (uint256, uint8) {
        try chainlinkOracle.getEthUsdPrice() returns (
            uint256 price,
            uint8 decimals
        ) {
            return (price, decimals);
        } catch {
            return (150000000000, 8); // TODO for test only
        }
    }
    ...
    //         console.log("getEthUsdPrice.reason:", reason);
    //         return (150000000000, 8); // TODO for test only
    //     }
    // }
}
