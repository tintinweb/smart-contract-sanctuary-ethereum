...
contract Coordinator {
  ...
  function commitRequest(bool _agreement) public onlyBeforeEndOfCommitRequest {  
    if(_agreement) { 
      commitCounter = add(commitCounter, 1); //no integer overflow possible 
    } else {
      abortCounter = add(abortCounter, 1); //no integer overflow possible
    }  
  }
  ...
  function commit(address payable _receiver, uint256 _amount) public onlyAfterCommitPhase { 
    ...
    if(abortCounter == 0 && commitCounter > 0) { // continue if no participant voted against the transaction
    ...
    } else if(abortCounter == 0 && commitCounter == 0) {
        revert("No participant voted");
