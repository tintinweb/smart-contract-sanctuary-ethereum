...
contract Bridge is AccessControl, IBridge {
    ...
    mapping(uint16 => mapping(bytes => IAssetRouter.SwapMessage)) private receivedSwapMessages;
    mapping(uint16 => mapping(bytes => IAssetRouter.LiquidityMessage)) private receivedLiquidityMessages;
    ...
    function dispatchMessage(
        ...
        if (estimatedFee > msg.value) {
            console.log("Estimated fee: %s", estimatedFee);
    ...
    function lzReceive(
        ...
        require(msg.sender == address(layerZeroEndpoint), "!l0");
    ...
    function nonBlockingReceive(uint16 srcChainId_, bytes memory srcAddressBytes_, bytes memory payload_) public {
        // TODO remove this for testing
        require(msg.sender == address(this) || hasRole(DEFAULT_ADMIN_ROLE, msg.sender), "!address(this)");
        require(
            srcAddressBytes_.length == bridgeLookup[srcChainId_].length &&
                keccak256(srcAddressBytes_) == keccak256(bridgeLookup[srcChainId_]),
            "Bridge does not match"
        );
        handleReceive(srcChainId_, srcAddressBytes_, payload_);
    }

    function handleReceive(uint16 _srcChainId, bytes memory _srcAddress, bytes memory _payload) internal {
        uint8 functionType;

        assembly {
            functionType := mload(add(_payload, 32))
        }
        if (IAssetRouter.MESSAGE_TYPE(functionType) == IAssetRouter.MESSAGE_TYPE.SWAP) {
            (, IAssetRouter.SwapMessage memory receivedMessage) = abi.decode(
                _payload,
                (uint8, IAssetRouter.SwapMessage)
            );
            receivedSwapMessages[_srcChainId][_srcAddress] = receivedMessage;

            router.receiveVouchers(
                receivedMessage.chainData.srcPoolId,
                receivedMessage.chainData.srcChainId,
                receivedMessage.chainData.dstPoolId,
                receivedMessage.vouchers
            );
            router.swapRemote(receivedMessage);
            emit SwapMessageReceived(receivedMessage);
        } else if (IAssetRouter.MESSAGE_TYPE(functionType) == IAssetRouter.MESSAGE_TYPE.ADD_LIQUIDITY) {
            (, IAssetRouter.LiquidityMessage memory receivedMessage) = abi.decode(
                _payload,
                (uint8, IAssetRouter.LiquidityMessage)
            );

            receivedLiquidityMessages[_srcChainId][_srcAddress] = receivedMessage;
            ...
            emit LiquidityMessageReceived(receivedMessage);
        }
    }
    ...
    function quoteLayerZeroFee(
        ...
        if (_type == IAssetRouter.MESSAGE_TYPE.SWAP) {
            IAssetRouter.SwapMessage memory swapMessage = IAssetRouter.SwapMessage({
                chainData: IAssetRouter.ChainData(1, 1, 1, 1),
                receiver: address(this),
                vouchers: IAssetRouter.VoucherObject(1, 1, false),
                amount: 1,
                fee: 1,
                kbpRemove: 1,
                id: keccak256(abi.encode(address(this), 1, 1, IAssetRouter.MESSAGE_TYPE.SWAP)),
                payload: _payload
            });
            encodedMessage = abi.encode(IAssetRouter.MESSAGE_TYPE.SWAP, swapMessage);
        } else if (_type == IAssetRouter.MESSAGE_TYPE.ADD_LIQUIDITY) {
            IAssetRouter.LiquidityMessage memory liquidityMessage = IAssetRouter.LiquidityMessage({
                chainData: IAssetRouter.ChainData({ srcPoolId: 1, srcChainId: 1, dstPoolId: 1, dstChainId: 1 }),
                vouchers: IAssetRouter.VoucherObject(1, 1, false),
                id: keccak256(abi.encode(address(this), 1, 1, IAssetRouter.MESSAGE_TYPE.ADD_LIQUIDITY)),
                payload: _payload
            });
            encodedMessage = abi.encode(IAssetRouter.MESSAGE_TYPE.ADD_LIQUIDITY, liquidityMessage);
        } else {
...
interface IAssetRouter {
    ...
    function receiveVouchers(
        ...
        uint16 _dstChainId,
        uint16 _dstPoolId,
        VoucherObject memory _c
    ) external;

    function swapRemote(SwapMessage memory receivedMessage) external returns (SwapMessage memory);
