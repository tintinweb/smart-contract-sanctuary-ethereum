...
interface IBADD {
   function getUserInfo(address voter) external view returns (uint256, uint256, uint256);
}
...
contract Governance is Initializable, UUPSUpgradeable, OwnableUpgradeable {
    ...
    function propose(uint256 tokenID, string memory description) public returns (uint256) {
		...
		IStaking(NFTstaking).proposalSubmited(tokenID, msg.sender);
    ...
    function cancel(uint256 proposalId) external {
        require(state(proposalId) != ProposalState.Approved, "Governor::cancel: cannot cancel approved proposal");
		
        Proposal storage proposal = proposals[proposalId];
        require(msg.sender == proposal.proposer, "Governor::cancel: Other user cannot cancel proposal");
        proposal.canceled = true;
		
		IStaking(NFTstaking).proposalCancel(proposal.NFT, msg.sender);
        emit ProposalCanceled(proposalId);
    }
	
	function approved(uint256 proposalId) external onlyOwner{
        require(state(proposalId) != ProposalState.Succeeded, "Governor::approved: only succeeded proposal can approved");
		
        Proposal storage proposal = proposals[proposalId];
        proposal.approved = true;
		IStaking(NFTstaking).proposalApproved(proposal.NFT);
    ...
    function state(uint256 proposalId) public view returns (ProposalState) {
        ...
        } else if (proposal.forVotes > proposal.againstVotes && block.timestamp > proposal.endTime) {
    ...
    function castVote(uint256 proposalId, uint256 support) external {
	    (uint256 amount, , ) = IBADD(token).getUserInfo(msg.sender);
		require(amount > 0, "Governor::castVote: not eligible for voting");
	...
	function setbADDAddress(address newAddress) public onlyOwner {
		require(newAddress != address(0), "INFLIV: Zero address");
		token = newAddress;
	}

	function setNFTstaking(address newAddress) public onlyOwner {
		require(newAddress != address(0), "INFLIV: Zero address");
		NFTstaking = newAddress;
	}
	
	function _authorizeUpgrade(address) internal view override {
        require(owner() == msg.sender, "Only admin can upgrade implementation");
    }
}
