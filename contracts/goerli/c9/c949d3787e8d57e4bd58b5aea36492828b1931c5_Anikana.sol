...
contract Anikana is ValidatorSmartContractInterface, Ownable {
    ...
    */
        ...
        returns (address[] memory, uint256[] memory)
    {
        ...
        for (
            uint256 i = 1;
            i <= indexValidatorCandidateRequest.current();
            i++
        ) {
            if (validatorCandidateRequestList[i].status == Status.Requested)
                amountRequestPending++;
        }

        address[] memory addressPendingRequestList = new address[](
            amountRequestPending
        );
        uint256[] memory startPendingRequestList = new uint256[](
            amountRequestPending
        );
        ...
        for (
            ...
            if (validatorCandidateRequestList[i].status == Status.Requested) {
                addressPendingRequestList[
                    indexPendingRequest
                ] = validatorCandidateRequestList[i].requester;
                startPendingRequestList[
                    indexPendingRequest
                ] = validatorCandidateRequestList[i].createdBlock;
                indexPendingRequest++;
            }
        }
    ...
    function getApproveRequestList()
        public
        view
        returns (address[] memory, uint256[] memory)
    {
        uint256 amountRequestApprove;
        for (
            uint256 i = 1;
            i <= indexValidatorCandidateRequest.current();
            i++
        ) {
            if (validatorCandidateRequestList[i].status == Status.Approved)
                amountRequestApprove++;
        }

        address[] memory addressApproveRequestList = new address[](
            amountRequestApprove
        );
        uint256[] memory startApproveRequestList = new uint256[](
            amountRequestApprove
        );
        uint256 indexPendingRequest;
        for (
            ...
            if (validatorCandidateRequestList[i].status == Status.Approved) {
                addressApproveRequestList[
                    indexPendingRequest
                ] = validatorCandidateRequestList[i].requester;
                startApproveRequestList[
                    indexPendingRequest
                ] = validatorCandidateRequestList[i].createdBlock;
                indexPendingRequest++;
            }
        }
        return (addressApproveRequestList, startApproveRequestList);
    }

    function getRejectRequestList()
        public
        view
        returns (address[] memory, uint256[] memory)
    {
        uint256 amountRequestReject;
        for (
            uint256 i = 1;
            i <= indexValidatorCandidateRequest.current();
            i++
        ) {
            if (validatorCandidateRequestList[i].status == Status.Rejected)
                amountRequestReject++;
        }

        address[] memory addressRejectRequestList = new address[](
            amountRequestReject
        );
        uint256[] memory startRejectRequestList = new uint256[](
            amountRequestReject
        );
        uint256 indexPendingRequest;
        for (
            ...
            if (validatorCandidateRequestList[i].status == Status.Rejected) {
                addressRejectRequestList[
                    indexPendingRequest
                ] = validatorCandidateRequestList[i].requester;
                startRejectRequestList[
                    indexPendingRequest
                ] = validatorCandidateRequestList[i].createdBlock;
                indexPendingRequest++;
            }
        }
        return (addressRejectRequestList, startRejectRequestList);
    }
    ...
    {
        uint256 amountAppointedValidatorCandidateListOfKnight = currentKnightList[
                _idKnight
            ].appointedValidatorCandidateList.length;
        uint256[]
            memory dateRequestListBecomeValidatorCandidate = new uint256[](
                amountAppointedValidatorCandidateListOfKnight
            );
        uint256[]
            memory dateApproveListBecomeValidatorCandidate = new uint256[](
                amountAppointedValidatorCandidateListOfKnight
            );
        uint256[] memory knightNoOfValidatorCandidate = new uint256[](
            amountAppointedValidatorCandidateListOfKnight
        );

        for (
            uint256 i = 0;
            i < amountAppointedValidatorCandidateListOfKnight;
            i++
        ) {
            for (
                uint256 j = 1;
                j <= indexValidatorCandidateRequest.current();
                j++
            ) {
                if (
                    currentKnightList[_idKnight]
                        .appointedValidatorCandidateList[i] ==
                    validatorCandidateRequestList[j].requester
                ) {
                    dateRequestListBecomeValidatorCandidate[
                        i
                    ] = validatorCandidateRequestList[j].createdBlock;
                    dateApproveListBecomeValidatorCandidate[
                        i
                    ] = validatorCandidateRequestList[j].endBlock;
                    knightNoOfValidatorCandidate[
                        i
                    ] = validatorCandidateRequestList[j].knightNo;
                    break;
                }
            }
        }
