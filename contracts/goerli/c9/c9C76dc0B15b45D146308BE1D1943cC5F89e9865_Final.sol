...
contract Final is ERC20, Ownable{
    ...
    mapping (address => bool) public isTraderBot;
    ...
    Fees public buyFees = Fees(1, 2, 2);
    Fees public sellFees = Fees(1, 2, 2);
    uint256 public totalSellFee = 5;
    uint256 public totalBuyFee = 5; // when high slip, just more 5% -- start listing
    ...
    function setTaxWallet(address _lpRecipient, address _marketingWallet, address _stakingPoolWallet) external onlyOwner{
    ...
    function setMoAdrr(address[] calldata addr, bool _isTraderBot) external onlyOwner{
        for (uint256 i = 0; i < addr.length; i++) {
            isTraderBot[addr[i]] = _isTraderBot; 
        }
    }

    function _transfer(address from, address to, uint256 amount) internal override {
        require(amount > 0, "Transfer amount must be greater than zero");
        require(!isTraderBot[from], "From cannot be BOT");
    ...
    function SetShutdownBotsAndWhales() external onlyOwner {
        antiBot = false;
    }
