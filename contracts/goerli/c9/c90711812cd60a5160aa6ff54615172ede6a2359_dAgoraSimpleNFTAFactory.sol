...
contract dAgoraSimpleNFTAFactory is Ownable, ReentrancyGuard {
    ...
    mapping(address => Deploys) public _deployedContracts;
    event FactoryContractCreated(address factory);
    ...
    modifier isPaused() {
        if(paused) revert ContractsPaused(paused);
        _;
    }
    ...
    {
        if(!canCreate()) {
            revert CantCreateFactory(canCreate());
        }

        if(_bulkBuyLimit > _maxTotalSupply) {
            revert BulkBuyOverMaxSupply(_bulkBuyLimit, _maxTotalSupply);
        }

        if(_maxTotalSupply == 0) {
            revert ZeroMaxSupply(_maxTotalSupply);
        }
        ...
        emit FactoryContractCreated(address(nfta));
    }
