...
/**
 * @title DebtToken
 * @author Taker, inspired by AAVE variable debt token implementation
 * @notice Implementation of debt token
 **/
contract DebtToken is ScaledERC20, IDebtToken {
  using WadRayMath for uint256;

  mapping(address => mapping(address => uint256)) internal _delegateAllowance;
  ...
  function initialize(
    ...
    string memory name = string.concat(namePrefix, "_DEBT_TOKEN");
    string memory symbol = string.concat(symbolPrefix, "_DEBT_TOKEN");
  ...
  function mint(
    ...
    if (initiator != to) {
      require(
        _delegateAllowance[to][initiator] >= amount,
        Errors.genErrMsg(symbol(), Errors.DEBT_ALLOWANCE_OVERREACH)
      );
      uint256 newAllowance = _delegateAllowance[to][initiator] - amount;
      _delegateAllowance[to][initiator] = newAllowance;
      emit Delegated(to, initiator, UNDERLYING_ASSET, newAllowance);
    }
  ...
  function burn(
    ...
    uint256 amount,
    uint256 scaleFactor
  ) external override(ScaledERC20, IScaledERC20) onlyLendingPool {
    _burnWithScaling(from, amount, scaleFactor);
  }

  /// @inheritdoc IDebtToken
  function delegate(address to, uint256 amount) external override {
    _delegateAllowance[msg.sender][to] = amount;
    emit Delegated(msg.sender, to, UNDERLYING_ASSET, amount);
  }

  /// @inheritdoc IScaledERC20
  function compoundedBalanceOf(address account) public view override returns (uint256) {
    return balanceOf(account).rayMul(LENDING_POOL.getReserveNormalizedDebtScale(UNDERLYING_ASSET));
  }

  /// @inheritdoc IScaledERC20
  function compoundedTotalSupply() external view override returns (uint256) {
    return super.totalSupply().rayMul(LENDING_POOL.getReserveNormalizedDebtScale(UNDERLYING_ASSET));
  }

  /// @inheritdoc IDebtToken
  function delegateAllowance(address from, address to) external view override returns (uint256) {
    return _delegateAllowance[from][to];
  }

  // @dev Debt token is not transferrable
  function transfer(
    address,
    uint256
  ) public view override(ERC20Upgradeable, IERC20Upgradeable) returns (bool) {
    revert(Errors.genErrMsg(symbol(), Errors.TOKEN_NOT_TRANSFERRABLE));
  }
