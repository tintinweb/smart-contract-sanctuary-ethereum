...
contract Royalty_GAT is ERC721A, Ownable, ReentrancyGuard {
  ...
  constructor() ERC721A("Gods & Titans - Royalty NFT", "ROYALTYGAT") {
    setHiddenMetadataUri("ipfs://HIDDENIPFSURLHERE/hidden.json");
  }  
  modifier onlyUser(){
  ...
  function verifyTokenForAddress(string calldata _salt, bytes calldata _token, address _address , address signer) internal pure returns (bool) {
      return _verify(keccak256(abi.encode(_salt, _address)), _token, signer);
  }

  function airdrop(uint256 _mintAmount, address _receiver, string calldata _salt, bytes calldata _token) external onlyOwner  {
