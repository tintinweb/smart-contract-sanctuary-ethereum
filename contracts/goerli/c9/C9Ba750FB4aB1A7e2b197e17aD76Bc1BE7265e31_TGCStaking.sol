...
contract TGCStaking is Ownable{
	...
	event NewMinStakingToken(uint256 minStaking);
    ...
    constructor () {
	   unlockFee = 100;
	   topStakerNumber = 30;
	   minStakingToken = 10 * 10**18;
    }
	
	function deposit(uint256 amount) external {
		require(IERC20(stakedToken).balanceOf(msg.sender) - IStaking(stakedToken).lockedAmount(msg.sender) >= amount, "balance not available for staking");
		...
		if(mapUserInfo[msg.sender].amount > 0) {
            ...
            if (pending > 0)
			{
                ...
                IERC20(stakedToken).safeTransfer(address(msg.sender), pending - fee);
			    _updatePool(fee);
            }
        }
		...
		mapUserInfo[msg.sender].rewardDebt = (mapUserInfo[msg.sender].amount * accTokenPerShare);
		
		totalStaked += amount;
		IStaking(stakedToken).lockToken(amount, msg.sender);
	...
	function withdraw() external {
	    if(mapUserInfo[msg.sender].amount > 0) 
		{
			...
			IStaking(stakedToken).unlockToken(msg.sender);
	...
	function updatePool(uint256 amount) external {
	   require(msg.sender == address(stakedToken), "sender not allowed");
       uint256 newReward = amount;
       accTokenPerShare += newReward / totalStaked;
	   emit PoolUpdated(amount);
    }
	...
	function pendingReward(address user) public view returns (uint256) {
		...
		{
            uint256 pending = (mapUserInfo[user].amount * accTokenPerShare) - mapUserInfo[user].rewardDebt;
			return pending;
        } 
		else 
		{
	...
	function checkHighestStaker(address user) public view returns (bool){
        HighestAstaStaker[] storage highestStaker = highestStakerInPool[0];
        uint256 i = 0;
	...
	function SetMinStakingToken(uint256 minStaking) external onlyOwner {
	    require(minStaking > 0, "Incorrect `Min Staking` value");
		
		minStakingToken = minStaking;
		emit NewMinStakingToken(minStaking);
    }
}
