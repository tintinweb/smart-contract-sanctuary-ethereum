...
contract Marketplace is Ownable, Pausable, IERC1155Receiver, ERC165 {
    ...
    uint256 defaultOfferExp = 5 * offerTimeIncrement; // 72 hours

    uint256 listingTimeIncrement = (1 minutes); // 1 day

    ContractGlossary Index;

    modifier expireOffer(
        address nftContract,
        uint256 tokenID,
        uint256 index
    ) {
        if (block.timestamp >= offers[nftContract][tokenID][index].exp) {
            emit expireOff(
                nftContract,
                tokenID,
                offers[nftContract][tokenID][index].exp
            );
    ...
    event expireOff(address nftContract, uint256 tokenID, uint256 expDate);
    ...
    function setDefaultOfferExp(uint256 exp) public onlyOwner {
        defaultOfferExp = exp * offerTimeIncrement;
    }
    ...
    function _createMarketSale(
        ...
        marketItems[itemID].quantity -= quantity;
        marketItems[itemID].owner = payable(to);
        _itemsSold.increment();
        marketItems[itemID].seller.transfer(
    ...
    function makeOffer(
        ...
        require(
            msg.value >=
                (floorPrices[SilksContractsbyAddress[nftContract]] * quantity),
            "Price must be greater than floor price for it's type"
        );
        require(quantity >= 1, "QUANTITY MUST BE AT LEAST 1");
        ...
        offers[nftContract][tokenID].push(
            Offer(
                ...
                (msg.value / quantity),
                quantity,
                block.timestamp + (exp * offerTimeIncrement)
            )
        );
        emit OfferMade(
            indexID,
    ...
    function acceptOffer(
        ...
        uint256 index,
        uint256 quantity
    ) public whenNotPaused expireOffer(nftContract, tokenID, index) {
        refreshContracts();
        require(quantity >= 1, "QUANTITY MUST BE AT LEAST 1");
    ...
    function deleteOffer(
        ...
        if (msg.sender == offers[nftContract][tokenID][index].user) {} else if (
            is1155[nftContract] != true &&
            IERC721(nftContract).ownerOf(tokenID) == msg.sender
        ) {} else if (is1155[nftContract]) {
            require(
                is1155[nftContract] != true,
                "CAN'T DECLINE OFFER FOR 1155"
            );
        } else {
            revert("MUST BE OFFERER OR (OWNER OF TOKEN AND NO 115)");
        }
        delete offers[nftContract][tokenID][index];
        payable(offers[nftContract][tokenID][index].user).transfer(
            offers[nftContract][tokenID][index].price
    ...
    function getAllOffers(address nftContract, uint256 tokenID)
        public
        view
        ...
        returns (MarketItem[] memory)
    {
        ...
        for (uint256 i = 0; i <= fullMarketSize; i++) {
            if (
                marketItems[i].nftContract == tokenAddress &&
                marketItems[i].exp > block.timestamp
            ) {
                items[j] = (marketItems[i]);
                j++;
    ...
    function fetchMarketSizebyAddress(address tokenAddress)
        public
        view
        returns (uint256)
    {
