...
//  ============ Custom imports ============
import "./Sbcc.sol";
import "./SkullBombs.sol";

contract BombHeadz is ERC721Drop, ReentrancyGuard {
    uint256 public constant MAX_SUPPLY = 2000;
    uint256 public privateSalePrice = 0.045 ether;
    uint256 public publicSalePrice = 0.069 ether;

    // ============ Sale Params ===========
    uint256 public publicStateDuration;
    uint256 public burnForMintDuration;

    // ============ Sales State ===========
    bool public publicSaleActive;
    bool public burnForMintActive;

    uint256 public publicSaleDuration;
    uint256 public publicSaleStartingPrice;
    uint256 public publicSaleStartTime;
    uint256 public burnForMintStartTime;
    uint256 public collectionStartingIndexBlock;
    uint256 public mintedBombHeadzStartingIndex;

    string private baseURI;
    SkullBombs private immutable skullBombs;
    Sbcc private immutable sbcc;

    event BombHeadzPublicSaleStart(
        uint256 indexed _saleDuration,
        uint256 indexed _saleStartTime
    );

    event BombHeadzPublicSalePaused(
        uint256 indexed _saleDuration,
        uint256 indexed _saleStartTime
    );

    modifier whenPublicSaleActive() {
        require(publicSaleActive, "Contract: Public sale is not active");
        _;
    }
    ...
    {
        skullBombs = SkullBombs(skullBombsAddress);
        sbcc = Sbcc(sbccAddress);
    }
    ...
    function startPublicSale(uint256 saleDuration, uint256 saleStartPrice)
        ...
        onlyOwner
    {
        require(!publicSaleActive, "Public sale is still active");
        publicSaleDuration = saleDuration;
        publicSaleStartingPrice = saleStartPrice;
        publicSaleStartTime = block.timestamp;
        publicSaleActive = true;
        emit BombHeadzPublicSaleStart(saleDuration, block.timestamp);
    }
    ...
    function burnForMint(uint256 skullBombsId) external nonReentrant {
        require(burnForMintActive, "Contract: Burn for mint is not active");
        require(
            skullBombs.ownerOf(skullBombsId) == msg.sender,
            "Contract: Not the owner of the skull bomb"
        );
        require(
            sbcc.balanceOf(msg.sender, skullBombsId) == 5,
            "Contract: Not enough SBCC"
        );

        uint256 bombHeadzId;

        if (totalSupply() < MAX_SUPPLY) {
            bombHeadzId = totalSupply() + 1;
            _safeMint(msg.sender, bombHeadzId);
    ...
    function toggleBurnForMintActive() external onlyOwner {
        burnForMintActive = !burnForMintActive;
    }
}
