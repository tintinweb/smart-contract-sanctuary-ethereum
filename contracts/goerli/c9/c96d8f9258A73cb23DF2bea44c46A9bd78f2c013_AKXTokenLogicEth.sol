    ...
    AKXMath
{
    ...
    /// @notice maximum supply allowed to be minted or token total supply (3 billion AKX)
    ...
    /// @notice time presale was started (usually same time as this contract is deployed to mainnet)
    /// @dev this is setup in the initializer to avoid upgradability issues
    ...
    /// @notice true if presale is started
    ...
    function __AKXTokenLogic_init(
        ...
        feeWallet = _fw;
        treasury = _tw;
        emit PresaleStarted(presaleStartedOn);
    }
    ...
    function temporaryPausePresale() public onlyRole(AKX_OPERATOR_ROLE) {
        isPresale = false;
    }

    function restart() public onlyRole(AKX_OPERATOR_ROLE) {
        isPresale = true;
    }
    ...
    function buy() public payable {
        ...
        if (mintOverflowCheck(akxToMint + fee)) {
            pendingTransferToTreasury += msg.value;
            safeMint(_msgSender(), akxToMint);
        }
        emit Buy(msg.sender, msg.value, akxToMint - fee, _isPresale);
    }

    function mintOverflowCheck(uint256 amount)
        internal
        view
        returns (bool success)
    {
        if (isPresale && presaleMaxDuration < block.timestamp + 25 seconds) {
