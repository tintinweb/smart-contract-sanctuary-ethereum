...
contract EsDDY is MintableBaseToken {
    constructor() MintableBaseToken("Escrowed DDY", "esDDY", 0) {
    }
...
contract BaseToken is IERC20, IBaseToken, Context {
    ...
    function transferFrom(
        ...
        uint256 nextAllowance = allowances[_sender][_msgSender()].sub(
            _amount,
            "BaseToken: transfer amount exceeds allowance"
        );
        _approve(_sender, _msgSender(), nextAllowance);
        _transfer(_sender, _recipient, _amount);
        return true;
    }
