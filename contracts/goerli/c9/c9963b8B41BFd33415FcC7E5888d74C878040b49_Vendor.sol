...
contract Vendor is Ownable {
  ...
  function withdraw() public payable onlyOwner {
    ...
    // Below is how we want to xfer balance of contract nowadays
    uint256 ownerBalance = address(this).balance;
    require(ownerBalance > 0, "ERRFUN");
    (bool sent, bytes memory data) = owner().call{value: ownerBalance}("");
    require(sent, "Failed to withdraw Ether");
    emit WithdrawTokens(msg.sender, msg.value);

    // Old styles that are bad:
  ...
  function sellTokens(uint256 amount) public payable {
    // Approve an xfer from sender
    yourToken.approve(msg.sender, amount);

    // Now sell the tokens
    yourToken.transferFrom(msg.sender, address(this), amount);
    uint256 value = amount / tokensPerEth;
    (bool sent, bytes memory data) = payable(msg.sender).call{value: value}("");
    require(sent, "Failed to withdraw Ether");
    emit WithdrawTokens(msg.sender, msg.value);
  }
...
contract YourToken is ERC20 {
  constructor() ERC20('GoatGold', 'GGLD') {
