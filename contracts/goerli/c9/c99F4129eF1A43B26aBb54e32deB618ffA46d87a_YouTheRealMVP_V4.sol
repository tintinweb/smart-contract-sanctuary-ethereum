...
contract YouTheRealMVP_V4 is ERC721x, DefaultOperatorFiltererUpgradeable {
    ...
    event Unstake(uint256 tokenId, address by, uint256 stakedAt, uint256 unstakedAt);
    ...
    event BoostStarted(uint256 indexed tokenId, uint256 boostStartedAt, uint256[] captainz);
    event BoostEdited(uint256 indexed tokenId, uint256 boostStartedAt, uint256[] captainz, uint256 boostEditedAt);
    event BoostStopped(uint256 indexed tokenId, uint256 boostStartedAt, uint256 boostStoppedAt);
    ...
    function initialize(string memory baseURI) public initializer {
        ERC721x.__ERC721x_init("YOU THE REAL MVP", "MVP");
    ...
    function airdrop(address receiver, uint256 tokenAmount) external onlyOwner {
        require(totalSupply() + tokenAmount <= MAX_SUPPLY, "would exceed MAX_SUPPLY");
    ...
    function giveaway(address[] memory receivers) external onlyOwner {
        ...
        require(totalSupply() + receivers.length <= MAX_SUPPLY, "would exceed MAX_SUPPLY");
    ...
    function giveawayWithAmounts(address[] memory receivers, uint256[] memory amounts) external onlyOwner {
        require(receivers.length >= 1, "at least 1 receiver");
        require(receivers.length == amounts.length, "receivers.length must equal amounts.length");
        uint256 total = 0;
    ...
    function burnSupply(uint256 maxSupplyNew) external onlyOwner {
        ...
        require(maxSupplyNew >= totalSupply(), "cannot burn more than current supply");
    ...
    // V3
    function transferFrom(address from, address to, uint256 tokenId) public override(ERC721x) onlyAllowedOperator(from) {
        require(tokensLastStakedAt[tokenId] == 0, "Cannot transfer staked token");
        super.transferFrom(from, to, tokenId);
    }

    // V3
    function safeTransferFrom(address from, address to, uint256 tokenId, bytes memory _data) public override(ERC721x) onlyAllowedOperator(from) {
        require(tokensLastStakedAt[tokenId] == 0, "Cannot transfer staked token");
    ...
    function stake(uint256 tokenId) public {
        ...
        require(tokensLastStakedAt[tokenId] == 0, "boosting/staking already started for [MVP tokenId]");

        tokensLastStakedAt[tokenId] = block.timestamp;
        emit Stake(tokenId, msg.sender, tokensLastStakedAt[tokenId]);
        uint256[] memory emptyCaptainzToken = new uint256[](0);
        emit BoostStarted(tokenId, block.timestamp, emptyCaptainzToken);
    }
    ...
    function checkGuardianOrMarketplace(address operator) internal view {
        ...
        require(!(marketplaceRestriction == 1 && blacklistedMarketplaces[operator]), "Marketplace blacklisted");
        require(!(marketplaceRestriction == 2 && !whitelistedMarketplaces[operator]), "Marketplace not whitelisted");
        return;
    }

    function approve(address to, uint256 tokenId) public virtual override(ERC721AUpgradeable) {
    ...
    function whitelistMarketplaces(address[] calldata markets, bool whitelisted) external onlyOwner {
    ...
    function blacklistMarketplaces(address[] calldata markets, bool blacklisted) external onlyOwner {
        for (uint256 i = 0; i < markets.length; i++) {
            address market = markets[i];
    ...
    function _mayTransfer(address operator, uint256 tokenId) private view returns (bool) {
    ...
    function _beforeTokenTransfers(
        ...
        for (uint256 tokenId = startTokenId; tokenId < startTokenId + quantity; tokenId += 1) {
            if (from != address(0) && to != address(0) && !_mayTransfer(msg.sender, tokenId)) {
    ...
    function batchEditBoost(BoostInfo[] calldata boostInfos) external {
        ...
        for (uint256 i = 0; i < batch; ) {
            uint256 tokenId = boostInfos[i].tokenId;
            ...
            _resetQuestingCaptainz(tokenId);
            unchecked {
                ++i;
            }
        }
        ...
        for (uint256 i = 0; i < batch; ) {
            uint256 tokenId = boostInfos[i].tokenId;
            uint256[] calldata captainzTokenIds = boostInfos[i].captainzTokenIds;

            require(captainzTokenIds.length <= MAX_QUESTING_CAPTAINZ, "too many questingCaptainz [captainzTokenIds]");

            _addQuestingCaptainz(tokenId, captainzTokenIds);
            emit BoostEdited(tokenId, tokensLastStakedAt[tokenId], captainzTokenIds, block.timestamp);
            unchecked {
                ++i;
            }
        }
    ...
    function startBoost(uint256 tokenId, uint256[] calldata captainzTokenIds) public {
        ...
        _addQuestingCaptainz(tokenId, captainzTokenIds);
        emit Stake(tokenId, msg.sender, tokensLastStakedAt[tokenId]);
        emit BoostStarted(tokenId, block.timestamp, captainzTokenIds);
    }
    ...
    function _addQuestingCaptainz(uint256 tokenId, uint256[] calldata captainzTokenIds) private {
        if (captainzTokenIds.length >= 1) {
            ...
            for (uint256 i = 0; i < batch; ) {
                ...
                require(existCheck.length == 0, "Duplicate [captainzTokenIds]");
                questingCaptainzToMVP[cTokenId] = wrapper;
                captainzLastBoostedAt[cTokenId] = block.timestamp;
                unchecked {
                    ++i;
                }
            }
    ...
    function _resetQuestingCaptainz(uint256 tokenId) private {
    ...
    function stopBoost(uint256 tokenId) public {
        require(msg.sender == ownerOf(tokenId) || msg.sender == owner(), "caller must be owner of token or contract owner");
        require(tokensLastStakedAt[tokenId] > 0, "boosting/staking not started for [MVP tokenId]");

        _resetQuestingCaptainz(tokenId);

        uint256 tlsa = tokensLastStakedAt[tokenId];
        tokensLastStakedAt[tokenId] = 0;
        emit Unstake(tokenId, msg.sender, block.timestamp, tlsa);
        emit BoostStopped(tokenId, tlsa, block.timestamp);
    }
    ...
    function getCaptainzInfo(uint256 captainzTokenId) external view returns (uint256 tokenId, uint256 lastBoostedAt) {
        require(address(captainzContract) != address(0), "captainzContract not set");
        uint256[] storage existCheck = questingCaptainzToMVP[captainzTokenId];
        if (existCheck.length == 0) return (0, 0);
        tokenId = existCheck[0];
        if (captainzContract.ownerOf(captainzTokenId) != ownerOf(tokenId)) return (0, 0);
        return (tokenId, captainzLastBoostedAt[captainzTokenId]);
    }

    function getActiveBoostedQuestingCaptainz(uint256 tokenId) external view returns (uint256[] memory) {
        require(address(captainzContract) != address(0), "captainzContract not set");
        address owner = ownerOf(tokenId);

        uint256[] memory cTokenIds = boostedQuestingCaptainz[tokenId];
        uint256 activeLength = cTokenIds.length;
        for (uint256 i = 0; i < activeLength; ) {
            uint256 cTokenId = cTokenIds[i];
            if (captainzContract.ownerOf(cTokenId) != owner || captainzContract.tokensLastQuestedAt(cTokenId) == 0) {
                cTokenIds[i] = 0;
                activeLength--;
            }
            unchecked {
        ...
        uint256[] memory activeBoostedQuestingCaptainz = new uint256[](activeLength);
        uint256 activeIdx;
        for (uint256 i = 0; i < activeLength; ) {
            if (cTokenIds[i] != 0) {
                activeBoostedQuestingCaptainz[activeIdx++] = cTokenIds[i];
            }
            unchecked {
                ++i;
            }
        ...
        return activeBoostedQuestingCaptainz;
    }
