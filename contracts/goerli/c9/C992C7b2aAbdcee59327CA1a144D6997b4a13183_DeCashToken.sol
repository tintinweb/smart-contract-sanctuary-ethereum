...
// File contracts/lib/DeCashSignature.sol
...
library Signature {
  ...
  function requireSignature(
    ...
    if (_sigStd == Std.typed) {
      ...
      require(
        _signer == ecrecover(keccak256(abi.encodePacked(dest, _data)), v, r, s),
      ...
      require(
        _signer ==
          ecrecover(
            keccak256(abi.encodePacked(ETH_SIGNED_MESSAGE_PREFIX, "32", _data)),
            v,
            r,
            s
          ) || // Geth-adopted
          ...
          ecrecover(
            keccak256(
              abi.encodePacked(ETH_SIGNED_MESSAGE_PREFIX, "\x20", _data)
            ),
            v,
            r,
            s
...
// File contracts/contract/DeCashMultisignature.sol
...
abstract contract DeCashMultisignature {
  ...
  event RequiredSignerChanged(uint256 newRequiredSignature, uint256 generation);
...
// File contracts/contract/DeCashToken.sol
...
contract DeCashToken is DeCashBase, DeCashMultisignature, ERC20 {
  ...
  modifier onlyLastest() {
    require(
      address(this) ==
        _getAddress(keccak256(abi.encodePacked("contract.address", "token"))) ||
        address(this) ==
  ...
  function initialize(
    ...
    if (currentVersion == 0) {
      ...
      _setString(keccak256(abi.encodePacked("token.symbol", _name)), _symbol);
      _setUint(keccak256(abi.encodePacked("token.decimals", _name)), _decimals);
    ...
    if (currentVersion != version) {
      _setUint(keccak256(abi.encodePacked("token.version", _name)), version);
    }
  }
  ...
  function _getTotalSupply() internal view returns (uint256) {
    return _getUint(keccak256(abi.encodePacked("token.totalSupply", _name)));
  }

  function _setTotalSupply(uint256 _supply) internal {
    _setUint(keccak256(abi.encodePacked("token.totalSupply", _name)), _supply);
  }
    ...
    returns (uint256)
  {
    return
      _getUint(
        keccak256(abi.encodePacked("token.allowed", _name, _owner, _spender))
      );
  }
  ...
  function _addAllowed(
    ...
    _setAllowed(_owner, _spender, _getAllowed(_owner, _spender).add(_balance));
  }
  ...
  function _subAllowed(
    ...
    _setAllowed(_owner, _spender, _getAllowed(_owner, _spender).sub(_balance));
  }

  // Balance Handling
  function _getBalance(address _owner) internal view returns (uint256) {
    return
      _getUint(keccak256(abi.encodePacked("token.balance", _name, _owner)));
  }
    ...
    returns (bool)
  {
    return
      _getBool(
        keccak256(abi.encodePacked("sign.generation", _name, _signer, _sigId))
      );
  }
...
// File contracts/currencies/EURD/EURDToken.sol
