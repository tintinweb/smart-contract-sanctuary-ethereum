    ...
    ReentrancyGuardUpgradeable
{
    ...
    /// @notice Event emitted when bid withdraws or outbids are processed
    /// @param pool Address of the pool
    /// @param bidder Account who's bid was processed
    /// @param amount Amount of the bid in pool's currency
    event BidReturned(address pool, address bidder, uint256 amount);

    /// @notice Event emitted when bid withdraws are processed
    /// @param pool Address of the pool
    /// @param bidder Account who's bid was processed
    /// @param amount Amount of the bid in pool's currency
    event BidWithdrawn(address pool, address bidder, uint256 amount);
    ...
    function initialize(
        ...
        require(
            (incrementInfo_.percent >= 1e16 &&
                incrementInfo_.percent <= 1e18) &&
    ...
    function bid(
        ...
        if (currentAuction.lastBidder == address(0)) {
            ...
            require(
                amount >=
                    currentAuction.lastBid + currentAuction.incrementalAmount,
                "LBA"
            );
        }
        ...
        (currentAuction.lastBid, currentAuction.lastBlock) = (
            amount,
            block.number
        );
    ...
    /// @param pool Address of a pool
    function resolveAuctionWithoutGoverment(
        address pool
    ...
    function setBidIncrementalPercent(
        ...
        require(
            bidIncrementalPercent_ >= 1e16 && bidIncrementalPercent_ <= 1e18,
            "IVL"
        ); // 1% - 100%
    ...
    function _startAuction(address poolAddress, uint256 amount) private {
        ...
        uint256 incrementalAmount = weiPoolSize.mulDecimal(
            bidIncrementInfo.percent
        );
