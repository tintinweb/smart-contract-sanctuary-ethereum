...
contract Claim is AccessControl {
    ...
    function checkAmount() private view returns (uint _claimAmount) {
            ...
            if (claimSettings.isNFT) {
                _claimAmount =
                    (claimSettings.claimAmountDetails.tokenIds.length *
                        IERC721(claimSettings.daoToken).balanceOf(msg.sender)) /
                    claimSettings.nftTotalSupply;
            } else {
                _claimAmount =
                    (claimSettings.claimAmountDetails.totalClaimAmount *
                        decimalConversion(
                            IERC20(claimSettings.daoToken).balanceOf(msg.sender)
                        )) /
                    decimalConversion(
    ...
    function airdropTokens(address _user) private {
        ...
        if (claimSettings.isNFT) {
            ...
            require(claimBalance >= _claimAmount, "Insufficient balance");
            if (claimSettings.hasAllowanceMechanism) {
    ...
    function rollbackTokens(uint _amount) external onlyRole(ADMIN) {
    ...
    function changeRoot(bytes32 _newRoot) external onlyRole(ADMIN) {
    ...
    function toggleClaim() external onlyRole(ADMIN) {
