...
contract Staking is Initializable{
    ...
    uint64 public rewardRate; // 18% per annum  
    ...
    function stake(uint256 _amount) external{
        ...
        if(userStake[msg.sender].amount==0){
            userStake[msg.sender].unlockTime = uint64(block.timestamp + lockInPeriod);
        }
    ...
    function withdraw(uint256 _amount) external {
        ...
        if(_amount == userStakeData.amount){
            ...
            userStake[msg.sender] = Stake(0,0,0,userStake[msg.sender].claimedReward + uint128(reward + pendingRewardAmount));
            totalStakes -= _amount;
            ...
            userStake[msg.sender].claimedReward += uint128(reward + pendingRewardAmount); 
            totalStakes -= _amount;
            pendingReward[msg.sender] = 0;
            IERC20(stakeToken).transfer(msg.sender,_amount);
            IERC20(stakeToken).transferFrom(rewardWallet,msg.sender, reward + pendingRewardAmount);
