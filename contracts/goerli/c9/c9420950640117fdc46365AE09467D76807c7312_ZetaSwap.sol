...
contract ZetaSwap  {
    ...
    // address private ZETA_TOKEN = 0xe1110F89Ef01E45D5D3d4DEE5AeA63E95817a774;
    // address private ZETA_MPI_ADDRESS = 0x97000a2D910DdaB15EE5687D55Ef4b7da443F658;
    ...
    function xcSwap(address _tokenIn, address _tokenOut, uint _amountIn, uint _amountOutMin, address _toAddress, string calldata _toChain, string calldata _destContract, uint _gasLimit) external {
        ...
        console.log("balance of ZETA of MPI contract %s", bal); 
        ZetaMPI(ZETA_MPI_ADDRESS).zetaMessageSend(msg.sender, _toChain, _destContract, amounts[1], _gasLimit, message, "");
    
    }

    function uponZetaMessage(address sender, string calldata destChainID, address destContract, uint zetaAmount,
		uint gasLimit, bytes calldata message, bytes32 messageID) external {
        ...
        console.log("toAddress %s amount %s  balance %s", toAddress, amounts[1],  IERC20(tokenOut).balanceOf(toAddress)); 
    }
}