...
contract OCASnakeArt is Ownable, ReentrancyGuard {
    ...
    uint8[20] private difficultyAndSnakeSize = [
        1,
        2,
        3,
        4,
    ...
    function retrofy(uint256 tokenId) private view returns (string[20] memory) {
        ...
        for (uint256 i = 0; i < 5; i++) {
            uint256 seed = generateAppleSeed(tokenId, i);
    ...
    function selectDifficulty(uint256 tokenId) private view returns (uint256) {
        uint256 seed = generateAppleSeed(tokenId, 0);
        return difficultyAndSnakeSize[uint256(seed) % uint256(3)];
    }
