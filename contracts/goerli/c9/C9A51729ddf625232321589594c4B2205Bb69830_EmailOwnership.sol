...
contract EmailOwnership is ERC1155, Ownable, ERC2771Recipient {
  mapping(uint256 => bool) public nullifiers;
  mapping(string => mapping(address => uint256)) private emailBalances;
  ...
  function balanceOf(address account, string memory email)
    public
    view
    returns (uint256)
  {
    ...
    return emailBalances[email][account];
  }
  ...
  function mint(EmailProof memory proof) external {
    ...
    // Check community attestor
    require(
      proof.input[91] != proof.input[92],
      "You should have at least two attestors"
    );
    // Check proof
    require(
    ...
    // Mint
    string memory email = extractDomain(proof.input, 0, 90);
    emailBalances[email][_msgSender()] = 1;
    nullifiers[nullifier] = true;
  }

  function extractDomain(
    uint256[93] memory input,
    uint256 startIndex,
    uint256 length
