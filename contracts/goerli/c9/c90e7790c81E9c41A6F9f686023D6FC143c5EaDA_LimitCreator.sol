    ...
    LimitOrdersStorage
{
        ...
        returns (uint256)
    {
        ...
        address poolAddress = PoolAddress.computeAddress(
            ...
            PoolAddress.getPoolKey(_params.token0, _params.token1, _params.fee)
        );
        require(whiteListFactory.spotMarketWhiteList(poolAddress), "CREO21");
        require(
            _params.orderType == OrderType.TAKE_PROFIT ||
                _params.orderType == OrderType.BUY_LIMIT,
            "CREO2"
        );
        if (_msgSender() != address(positionRouter))
            require(
                _msgSender() ==
                    ecrecover(
                        keccak256(
        ...
        if (_params.orderType == OrderType.TAKE_PROFIT) {
            ...
            require(
                _params.tickLower <= TickMath.MAX_TICK - tickSpacing,
                "CREO6"
            );
            require(_params.amountOfToken0 > 0, "CREO7");
            require(
                _params.amountOfToken0 >=
                    whiteListFactory.limitMinAmountToTrade(_params.token0),
                "CREO19"
            );
            require(_params.amountOfToken1 == 0, "CREO8");
            require(_params.receivedAmountOfToken0 == 0, "CREO9");
            require(_params.receivedAmountOfToken1 > 0, "CREO10");
            require(_params.tickUpper == 0, "CREO11");
            require(
                whiteListFactory.limitMinAmountToTrade(_params.token0) >= 1 &&
                    keccak256(whiteListFactory.tokenToWETH(_params.token0)) !=
                    keccak256(bytes("")),
                "Unsupported token0"
            );
            require(
                whiteListFactory.limitMinAmountToTrade(_params.token1) >= 1 &&
                    keccak256(whiteListFactory.tokenToWETH(_params.token0)) !=
                    keccak256(bytes("")),
                "Unsupported token1"
            );
            ...
            IERC20Upgradeable(_params.token0).safeTransferFrom(
                _msgSender(),
                address(this),
                _params.amountOfToken0
            );
            IERC20Upgradeable(_params.token0).safeIncreaseAllowance(
            ...
            require(
                _params.amountOfToken1 >=
                    whiteListFactory.limitMinAmountToTrade(_params.token1),
                "CREO20"
            );
            require(_params.amountOfToken1 > 0, "CREO14");
            require(_params.amountOfToken0 == 0, "CREO15");
            require(_params.receivedAmountOfToken1 == 0, "CREO16");
            require(_params.receivedAmountOfToken0 > 0, "CREO17");
            ...
            require(
                whiteListFactory.limitMinAmountToTrade(_params.token1) >= 1 &&
                    keccak256(whiteListFactory.tokenToWETH(_params.token1)) !=
                    keccak256(bytes("")),
                "Unsupported token1"
            );
            require(
            ...
            // Transfer tokens from user to us.
            IERC20Upgradeable(_params.token1).safeTransferFrom(
                _msgSender(),
                address(this),
                _params.amountOfToken1
            );
            IERC20Upgradeable(_params.token1).safeIncreaseAllowance(
                address(nftPositionManager),
                _params.amountOfToken1
            );
        ...
        orders[poolAddress][tokenId] = Order({
            ...
            receivedAmount: _params.receivedAmountOfToken1 == 0
                ? _params.receivedAmountOfToken0
                : _params.receivedAmountOfToken1,
            liquidity: liquidity,
            orderType: _params.orderType,
            transferedToken0: 0,
            transferedToken1: 0,
            signature: SignatureData({
                v: _params.signature.v,
                r: _params.signature.r,
                s: _params.signature.s
            }),
            executionCost: 0
        });
    ...
    function checkOrder(address _poolAddress, uint256 _tokenId)
        public
        view
        returns (
            bool isClosable,
    ...
    {
        (baseAmount, quoteAmount) = _getBaseTokenAmount(
            _tokenId,
            _poolAddress
        );
        ...
        if (
            ...
            quoteToken = order.poolData.token0;
            baseToken = order.poolData.token1;
        } else if (
            order.orderType == OrderType.TAKE_PROFIT ||
            order.orderType == OrderType.TAKE_PROFIT_FILLED ||
            order.orderType == OrderType.TAKE_PROFIT_CANCELLED
        ) {
            quoteToken = order.poolData.token1;
            baseToken = order.poolData.token0;
        }
    }
    ...
    function _getBaseTokenAmount(uint256 _tokenId, address _poolAddress)
        private
        view
        returns (uint256 baseAmount, uint256 quoteAmount)
    {
        Order memory order = orders[_poolAddress][_tokenId];
        (uint160 sqrtPriceCurrentX96, , , , , , ) = IUniswapV3Pool(
            _poolAddress
        ).slot0();
        uint160 sqrtPriceLowerX96 = TickMath.getSqrtRatioAtTick(
            order.poolData.tickLower
        );
        uint160 sqrtPriceUpperX96 = TickMath.getSqrtRatioAtTick(
            order.poolData.tickUpper
        );
...
import "./interfaces/ITheTradeWETH.sol";
import "./interfaces/ITFactory.sol";
...
contract LimitOrdersStorage {
    ...
    /// @notice whitelist factory.
    ITFactory public whiteListFactory;
    ...
    /// @notice Address of position router
    ...
    address public createDelegator;
    address public editDelegator;
    address public executeDelegator;
    address public cancellDelegator;
    address public repayerDelegator;
    ...
    mapping(address => PoolRequirement) public pathTokenToWETH;
    ...
    event SetNewPositionRouter(address newRouter);
    event SetNewCreateDelegator(address newDelegator);
    event SetNewEditDelegator(address newDelegator);
    event SetNewExecuteDelegator(address newDelegator);
    event SetNewCancelDelegator(address newDelegator);
    event SetNewRepayDelegator(address newDelegator);
    event SetNewSlippageValue(
        uint256 _newSlippage,
        uint256 _newSlippageAccuracy
    );
    event SetNewGASValue(uint256 _newGAS, uint256 _newGASAccuracy);
    event SetNewPriceLimit(
        address _token,
        address[] _path,
        uint24[] _pathFee,
        uint256 _minAmount
...
/// @notice TheTradeutes sqrt price for ticks of size 1.0001, i.e. sqrt(1.0001^tick) as fixed point Q64.96 numbers. Supports
...
library TickMath {
        ...
        returns (uint160 sqrtPriceX96)
    {
        unchecked {
            ...
            require(absTick < uint256(int256(MAX_TICK)), "tick > MAX_TICK");
...
library LiquidityAmounts {
    ...
    /// @notice TheTradeutes the amount of liquidity received for a given amount of token0 and price range
    ...
    /// @notice TheTradeutes the amount of liquidity received for a given amount of token1 and price range
    ...
    /// @notice TheTradeutes the maximum amount of liquidity received for a given amount of token0, token1, the current
    ...
    /// @notice TheTradeutes the amount of token0 for a given amount of liquidity and a price range
    /// @param sqrtRatioAX96 A sqrt price representing the first tick boundary
    /// @param sqrtRatioBX96 A sqrt price representing the second tick boundary
    ...
    /// @notice TheTradeutes the amount of token1 for a given amount of liquidity and a price range
    /// @param sqrtRatioAX96 A sqrt price representing the first tick boundary
    /// @param sqrtRatioBX96 A sqrt price representing the second tick boundary
    /// @param liquidity The liquidity being valued
    ...
    /// @notice TheTradeutes the token0 and token1 value for a given amount of liquidity, the current
    /// pool prices and the prices at the tick boundaries
    /// @param sqrtRatioX96 A sqrt price representing the current pool prices
    /// @param sqrtRatioAX96 A sqrt price representing the first tick boundary
    /// @param sqrtRatioBX96 A sqrt price representing the second tick boundary
...
// SPDX-License-Identifier: MIT
pragma solidity 0.8.9;
import "../PositionRouterStorage.sol";
import "@uniswap/v3-core/contracts/interfaces/IUniswapV3Pool.sol";
import "@openzeppelin/contracts-upgradeable/access/AccessControlUpgradeable.sol";
import "@openzeppelin/contracts-upgradeable/proxy/utils/Initializable.sol";
import "@openzeppelin/contracts-upgradeable/proxy/utils/UUPSUpgradeable.sol";
...
interface IPositionRouter {
    ...
    function executionComplite(uint256 orderId, uint256 startGas) external;
}
...
import "./interfaces/ILimitOrdersController.sol";
import "./interfaces/IMarketOrdersController.sol";
import "./interfaces/ITheTradeWETH.sol";
...
library FullMath {
    ...
    function mulDiv(
        ...
        unchecked {
            ...
            // TheTradeute the product mod 2**256 and mod 2**256 - 1
            ...
            // TheTradeute remainder using mulmod
            uint256 remainder;
            assembly {
            ...
            // TheTradeute largest power of two divisor of denominator.
            // Always >= 1.
            ...
            // TheTradeute the inverse by starting with a seed that is correct
