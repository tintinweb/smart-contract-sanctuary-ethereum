...
contract AbNFT is IAbNFT, ERC721Enumerable, Ownable, Configurable {
    ...
    MintingStatus public mintingStatus = MintingStatus.BLIND;
    ...
    error InvalidMintingStatus(MintingStatus current, MintingStatus expected);
    error ClaimNotPossible(uint256 amount, uint256 available);
    ...
    modifier onlyInMintingStatus(MintingStatus _status) {

        // check minting status
        if (mintingStatus != _status) revert InvalidMintingStatus(mintingStatus, _status);
        _;
    ...
    returns (string memory) {
        ...
        // check if revealed
        if (mintingStatus == MintingStatus.REVEALED) {
    ...
    function mint(uint256[] memory _nftIds, address _owner) external override
    onlyInState(State.CONFIGURED)
    onlyInMintingStatus(MintingStatus.BLIND)
    onlyMinter {
    ...
    function claim(uint256[] memory _nftIds, address _owner) external
    onlyInState(State.CONFIGURED)
    onlyInMintingStatus(MintingStatus.REVEALED)
    onlyMinter {

        // amount to claim
        uint256 amount_ = _nftIds.length;

        // check if claim possible
        if (toClaim < amount_) {
            revert ClaimNotPossible(amount_, toClaim);
        }

        // decrement available to claim
        toClaim -= amount_;

        // claim / mint after reveal
        __mint(_nftIds, _owner);
    ...
    function __mint(uint256[] memory _nftIds, address _owner) internal {
    ...
    onlyMinter {
        ...
        toClaim = _toClaim;
        mintingStatus = MintingStatus.REVEALED;
    ...
    function isRevealed() external view returns (bool) {
        ...
        // return
        return mintingStatus == MintingStatus.REVEALED;
...
interface IAbNFT is IERC721Enumerable {
    ...
    function mint(uint256[] memory _nftIds, address _owner) external;
    function reveal(string memory _revealedURI, uint256 _toClaim) external;
    function isRevealed() external view returns (bool);
    function claim(uint256[] memory _nftIds, address _owner) external;
