...
contract KunAgueroRaffle is GenericRaffle {
    uint256 public constant MAX_SUPPLY = 6_333;
...
/// @title EchidnaTester
/// @author Some Author
/// @notice You can use this contract as simple echidna testing example
contract EchidnaTester {
    //Instead of having the invariant contract inherit from the contract to test, it works better to link them together
    Greeter public greeter;

    /// @notice EchidnaGreeter constructor
    constructor() {
        greeter = new Greeter("hello world");
    }

    /// @notice it should always keep the same owner
    function echidna_testOwner() external view returns (bool) {
        return (greeter.contractOwner() == address(this));
    }

    /// @notice it should always return true
    function echidna_alwaysFoo() external pure returns (bool) {
        return true;
    }

    /// @notice it should always revert
    function echidna_alwaysRevert() external pure returns (bool) {
        revert();
    }

    /// @dev some examples using assertions, checkAsserts in default.yalm must be true

    /// @notice it should set new greeting message
    /// @param newGreeting The greeting message
    function setGreeting_(string memory newGreeting) external {
        greeter.setGreeting(newGreeting);
        string memory _greeting = greeter.greet();
        assert(keccak256(abi.encodePacked((_greeting))) == keccak256(abi.encodePacked((newGreeting))));
    }
}