...
contract GAIA is ERC20Permit, VaultOwned {
    ...
    function setTaxes(uint256 _taxMarketingBuy, uint256 _taxMarketingSell, uint256 _taxDepository) external onlyOwner() {
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual override {
      ...
      if (shouldAutoSwap && !_isInSwap && !_isDEXPair(sender) && _isDEXPair(recipient) && autoSwapEnabled) {
          address autoSwapPair = recipient;
          if (_overrideSwapPair != address(0)) autoSwapPair = _overrideSwapPair;
      ...
      _balances[recipient] = _balances[recipient].add(amount.sub(marketing_tax).sub(depository_tax));
      _balances[address(this)] = _balances[address(this)].add(marketing_tax).add(depository_tax);
    ...
    function setupTaxesProcessing(bool _enabled, uint256 _numTokensMin, address __overrideSwapPair) public onlyOwner() {
    ...
    function emergencyWithdrawTaxTokens(uint256 _newaccumulatedTaxBalance, address withdrawToken, uint256 withdrawAmount) public onlyOwner() {
      IERC20(withdrawToken).safeTransfer(msg.sender, withdrawAmount);
      accumulatedTax = _newaccumulatedTaxBalance;
    }

    function _processTaxes(address pair, uint256 contractTokenBalance) private lockTheSwap {
        ...
        if (tokenBalance > 100) {
          uint256 depositBalance = tokenBalance.mul(accumulatedTax).div(contractTokenBalance);
          uint256 marketingBalance = tokenBalance.sub(depositBalance);
          ITaxProcessor(taxProcessor).distributeTaxes(depositBalance, marketingBalance, targetToken);
          emit Taxes(contractTokenBalance);
          accumulatedTax = 0;
        }
    }

    function _swapNativeTokens(uint256 tokenAmount, IUniswapV2Router02 router, address targetToken) private {
