...
contract OCASnakeArt is Ownable, ReentrancyGuard {
    ...
    function returnSvgVarablesWithBuild(string[7] memory vars)
        public
        view
        returns (bytes memory)
    {
        ...
        for (uint256 i = 0; i < vars.length; i++) {
            bytes memory prevBytes = svgBytes;
            svgBytes = abi.encodePacked(prevBytes, svgBuild[i], vars[i]);
            if (i == vars.length - 1) {
                svgBytes = abi.encodePacked(prevBytes, svgBuild[i + 1]);
            }
    ...
    function returnSVG(uint256 count) internal view returns (string memory) {
        return svgBuild[count];
    }
