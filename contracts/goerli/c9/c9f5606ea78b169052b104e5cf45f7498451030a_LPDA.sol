...
interface ILPDA {
    ...
    function refundOwed(address _vault, address _minter)
        external
...
library LibLPDAInfo {
    function getAuctionState(LPDAInfo memory _info)
        internal
        view
        returns (LPDAState)
    {
    ...
    function isNotSuccessful(LPDAInfo memory _info)
        internal
        view
        returns (bool)
    {
        return (block.timestamp > _info.endTime &&
            _info.numSold < _info.supply);
    }
    ...
    function remainingSupply(LPDAInfo memory _info)
        internal
        pure
        returns (uint256)
    {
    ...
    NFTReceiver
{
    ...
    function settleAddress(address _vault, address _minter)
        external
        nonReentrant
    {
    ...
    function settleCurator(address _vault) external nonReentrant {
        ...
        _sendEthOrWeth(lpda.curator, total - fee);
        emit CuratorClaimed(_vault, lpda.curator, total - fee);
    }
    ...
    function currentPrice(address _vault) public view returns (uint256 price) {
        LPDAInfo memory lpda = vaultLPDAInfo[_vault];
        uint256 deduction = (block.timestamp - lpda.startTime) *
            lpda.dropPerSecond;
