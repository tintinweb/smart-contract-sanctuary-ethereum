...
contract Buns is ERC721A, Ownable, ReentrancyGuard, DefaultOperatorFilterer {
    ...
    uint256 public per_wallet = 100;
    bool public minting = false;
    bool public canUpdateSeed = true;

    constructor(IBunDescriptor newDescriptor) ERC721A("Buns", "BUNS") {
        descriptor = newDescriptor;
    }

    function mint(uint32 count) external payable nonReentrant{
        require(minting, "Minting needs to be enabled to start minting.");
        require(count < per_wallet, "Exceeds max 100 per transaction.");
        ...
        for (uint32 i; i < count;) {
            seeds[nextTokenId] = generateSeed(nextTokenId);
                _safeMint(_msgSenderERC721A(), nextTokenId);
    ...
    function burn(uint256 tokenId) public {
        require(_ownershipAt(tokenId).burned, "Not approved to burn.");
