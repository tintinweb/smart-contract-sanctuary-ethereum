...
contract Example9 is ERC721URIStorage, Ownable, EIP712, AccessControl 
, WNRoyalties 
...
, GelatoRelayContext
{
  ...
  string private constant SIGNING_DOMAIN = "WNLazyNFT-Voucher";
  string private constant SIGNATURE_VERSION = "1";

  uint256 pendingWithdrawals;
  uint256 minimumFundToKeep;

  mapping(uint256 => bool) private _reavealedTokens;

  constructor(
     uint256 bps
    )
    ERC721('Ex9 Token', 'EX9') 
     WNRoyalties(bps)
  ...
  function redeem(address redeemer, NFTVoucher calldata voucher) public payable returns (uint256) {
    ...
    increasePendingWithdrawals(msg.value, signer);
  ...
  function increasePendingWithdrawals(uint256 val, address signer) internal {
  ...
  , WNRoyalties
    ...
    || WNRoyalties.supportsInterface(interfaceId)
    ;
  }

  //Update the URI when the contract is revealed (only once by token id)
  function setRevealedURI(uint256 tokenId, string memory _tokenURI) public  {
      require(msg.sender == owner(), "WNERC721: only owner can reveal URI");
      require(_exists(tokenId), "can't reveal nonexistent token");
      require(_reavealedTokens[tokenId] != true, "WNERC721: already revealed");
  
      _reavealedTokens[tokenId] = true;
      _setTokenURI(tokenId, _tokenURI);
  }
