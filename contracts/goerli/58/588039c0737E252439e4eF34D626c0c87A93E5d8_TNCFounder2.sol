...
error SaleNotStarted(string msg);
error FailedProof(string msg);

contract TNCFounder2 is ERC721A, Ownable, ReentrancyGuard {
    string  public _baseTokenURI = "";
    bool    public _paused = false;            
    uint256 public _maxTokens = 628;
    bytes32 public _merkleRoot = 0x3e7a339c1ccd4942172db6da2924941d3e2cadfb5092a64db812c34fac49f4d1;
    
    mapping(address => uint256) private _walletMints;

    constructor() ERC721A("TNCFounder2", "TNC2") {}
    ...
    function mint(uint256 _amount, uint256 _walletLimit, bytes32[] calldata merkleProof) public
    {
        ...
        if(!MerkleProof.verify(merkleProof, _merkleRoot, node))revert FailedProof({msg:"Invalid proof"});
        ...
        if(!_paused)revert SaleNotStarted({msg:"Mint paused"});
