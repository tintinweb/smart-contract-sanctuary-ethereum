...
contract Vault {
  ...
  function sendToken(address target, uint amount, address token) public OnlyDexOrOperator {
  ...
  function cancelBooking(address user) public OnlyDexOrOperator {
    ...
    bookedBalance = bookedBalance.sub(bookings[user]);
  ...
  function emitBookingsReady() public OnlyOperator {
    ...
    for (uint256 i = 0; i < usersBookings.length ; i += 1) {
      ...
      (bool success, bytes memory message) = ownDexContract.call(
        abi.encodeWithSignature('emitBookingReady(address,uint256)', msg.sender, amount)
      );

      if (!success) {
        emit ExchangeError(user, string(message));
      } else {
        balance = balance.sub(amount);
      }

      usersToBookingCancel.push(user);
    }

    uint256 bookingsToCancelCount = usersToBookingCancel.length;

    if (bookingsToCancelCount > 0) {
      for (uint256 i = bookingsToCancelCount - 1; i > 0; i -= 1) {
        cancelBooking(usersToBookingCancel[i]);
        delete usersToBookingCancel[i];
      }
    }
  }
}
