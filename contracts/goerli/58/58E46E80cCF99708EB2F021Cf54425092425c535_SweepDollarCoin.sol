...
contract SweepDollarCoin is BaseSweep {
    ...
    function refreshInterestRate() public {
        ...
        if (current_amm_price > current_target_price + price_band) { // decrease interest rate
            yearly_interest_rate = yearly_interest_rate - sweep_step;
        } else if (current_amm_price < current_target_price - price_band) { // increase interest rate
            yearly_interest_rate = yearly_interest_rate + sweep_step;
        }
    ...
    function setPriceTarget(uint256 _new_target_price) public onlyByOwnGov {
        ...
        int256 price_ratio = int256(PRICE_PRECISION + yearly_interest_rate).fromInt();
        int256 price_precision = int256(PRICE_PRECISION).fromInt();
        int256 price_unit = price_ratio.pow(time_ratio).div(price_precision.pow(time_ratio));

        if (yearly_interest_rate > 0) {
            next_target_price = current_target_price * uint256(price_unit) / PRICE_PRECISION;
        } else {
