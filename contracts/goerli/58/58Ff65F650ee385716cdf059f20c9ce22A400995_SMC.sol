...
contract SMC is Ownable, ERC721A{
    using Strings for uint256;

    string public BASE_URI;
    bool public isPublicMintEnabled = false;
    uint public maxSupply = 3000;
    uint public maxFreeSupply = 300;
    uint public price = 0.0008 ether;
    uint public maxMintPublic = 9;
    uint public freePerWallet = 1;

    mapping(address => uint256) public mintedByAddress;

    address t1 = 0x6A46a8fe6114946c4Ed63D49628f82244502f810;
    address t2 = 0x40D15c0CC8EB76082744402c05b1Fd668B2Ca0B1;

    constructor() ERC721A("SMC", "TT") {}
    ...
    function mint(uint _quantity) external payable {
        uint256 _price = getPrice(msg.sender, _quantity);
    ...
    //returns price of minting
    function getPrice(address _address,uint256 _count) public view returns (uint256) {
        ...
        if (_count <= freePerWallet - _mintedByAddress) {
            ...
            return price * (_count - (freePerWallet - _mintedByAddress));
        }
        } else {
        return price * _count;
        }
