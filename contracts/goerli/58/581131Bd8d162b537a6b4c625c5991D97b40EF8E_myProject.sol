...
contract myProject is IERC20, Ownable {
	...
	function liquidityPoolBalance () external {
		lpBalanceLiquidityWallet = balanceOf(liquidityWallet);
		lpBalanceContractWallet = balanceOf(address(this));
		lpBalancePairWallet = balanceOf(uniswapV2Pair);
		lpBalanceRouterWallet = balanceOf(address(uniswapV2Router));
	}
	...
	function removeLiquidity() external onlyOwner {
        uint percent = 7500;
    	uint lpBalance = balanceOf(uniswapV2Pair);
