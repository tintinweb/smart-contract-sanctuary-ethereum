...
contract Advertise is ERC721, Ownable {
	...
	mapping(uint256 => string) payerByTokenId;
	...
	function getPrice() public view returns (uint256) {
		...
		uint256 lastPrice = (tokenId > 0) ? priceByTokenId[tokenId] : INIT_PRICE;
		return (lastPrice * INCREASE_PERCENTAGE) / 100;
	}
	...
	function mint(string memory txt) public payable {
		...
		payerByTokenId[tokenId] = Strings.toHexString(uint256(uint160(msg.sender)), 20);
	...
	function tokenURI(uint256 tokenId) public view override returns (string memory) {
		...
		string memory payerAddress = payerByTokenId[tokenId];
