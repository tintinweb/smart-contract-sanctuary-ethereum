...
contract Revolution is OwnableSkeleton, AccessControlEnumerable {
    ...
    modifier canExecute() {
        require(
            hasRole(DEFAULT_ADMIN_ROLE, msg.sender) ||
                (isQueued &&
                    block.timestamp >= queuedTime + QUEUE_TIME),
    ...
    function queueDrop(
        QueuedDrop memory _queuedDrop
    ) external hasVotesOrAdmin {
        queuedHash = keccak256(abi.encode(_queuedDrop));
        isQueued = true;
        queuedTime = block.timestamp;
    }

    function calculateQueuedHash(QueuedDrop memory _queuedDrop) external pure returns (bytes32) {
        return keccak256(abi.encode(_queuedDrop));
    }
    ...
    function isDropQueued() external view returns (bool) {
        return isQueued;
    }
    ...
    function deployRevolutionDrop(QueuedDrop memory _qd) external canExecute returns (address) {
        bytes32 _queuedHash = queuedHash;
        require(isQueued == true, "no queued drop present");
        require(keccak256(abi.encode(_qd)) == _queuedHash, "provided struct does not match queued drop hash");
        isQueued = false;
