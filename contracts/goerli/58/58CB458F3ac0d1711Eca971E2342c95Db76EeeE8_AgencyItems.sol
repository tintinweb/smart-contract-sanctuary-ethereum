...
contract AgencyItems is ERC1155, Ownable { // todo: rename contract, constructor, file
  ...
  modifier onlySystem() {
    if (msg.sender != systemAddress || msg.sender != mainContractAddress) revert SystemMustCall();
    _;
  }

  constructor(address _mainContract) ERC1155("") {
  ...
  function addNewItem(string calldata _name)
    ...
    onlyOwner
  {
    if (inventory[_name].id != 0) revert ItemExists(); // todo: test this
  ...
  // todo: what are these? it's the slot token, and 2 other things
  ...
  function destroyAgentItems(address _from, uint _itemId, uint _count) // can I destroy someone's items?
    external
    onlySystem
  {
  ...
  function setSystemAddress(address _systemAddress)
    external
    onlyOwner
  {
