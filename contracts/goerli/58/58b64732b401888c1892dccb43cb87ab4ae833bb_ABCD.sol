    ...
    ERC721AOpensea
{
    ...
    function bid() external payable isAuctionActive {
        ...
        uint256 minimumContribution = getAuctionPrice();
    ...
    /**
     * @dev get the number of tokens purchased by an address, after the
     *   clearing price has been set.
    ...
    /**
     * @dev get the refund amount for an account, after the clearing price
     *   has been set.
     * @param a address to query.
     */
    ...
    /**
     * @dev handles multiple refund methods.
     * @param to address to refund to.
     */
    function _sendRefund(address to) internal {
        uint256 price_ = price; // storage to memory
        if (price_ == 0) revert PriceHasNotBeenSet();
     ...
     */
    function sendRefund(address to)
        external
     ...
     */
    function sendRefundBatch(address[] calldata addresses)
        external
        onlyRole(REFUND_ROLE)
        nonReentrant
    {
        for (uint256 i; i < addresses.length; i++) {
            _sendRefund(addresses[i]);
        }
    }
     ...
     */
    function sendTokensBatch(address[] calldata addresses)
        external
        onlyRole(REFUND_ROLE)
    {
        for (uint256 i; i < addresses.length; i++) {
     ...
     * @param to address to send tokens and refund.
     */
     ...
     * @param to the address to refund.
     */
    function sendTokensAndRefund(address to)
        external
        onlyRole(REFUND_ROLE)
        nonReentrant
    {
    ...
    {
        for (uint256 i; i < addresses.length; i++) {
            _sendTokensAndRefund(addresses[i]);
        }
    }
...
contract SteppedDutchAuction {
    ...
    function _createNewAuction(
        ...
        duration =
            ((startPrice_ - finalPrice_) * timeStepSeconds_) /
            priceStep_;
    }
