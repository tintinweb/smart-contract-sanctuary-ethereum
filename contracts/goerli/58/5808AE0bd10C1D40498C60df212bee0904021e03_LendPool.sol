  ...
  LendPoolStorageExt
{
   ...
   * @param from The user from which the UDToken are transferred
  ...
  function finalizeTransfer(
    ...
    require(_msgSender() == reserve.UDTokenAddress, Errors.LP_CALLER_MUST_BE_AN_UDToken);
  ...
  /**
   * @dev Initializes a reserve, activating it, assigning an UDToken and nft loan and an
   * interest rate strategy
   * - Only callable by the LendPoolConfigurator contract
   * @param asset The address of the underlying asset of the reserve
   * @param UDTokenAddress The address of the UDToken that will be assigned to the reserve
   * @param UBTokenAddress The address of the UBToken that will be assigned to the reserve
  ...
  function initReserve(
    ...
    _reserves[asset].init(UDTokenAddress, UBTokenAddress, interestRateAddress);
...
library BorrowLogic {
  ...
  function _borrow(
    ...
    IUBToken(reserveData.UBTokenAddress).mint(
      vars.initiator,
      params.onBehalfOf,
      params.amount,
      reserveData.variableBorrowIndex
    ...
    reserveData.updateInterestRates(params.asset, reserveData.UDTokenAddress, 0, params.amount);

    IUDToken(reserveData.UDTokenAddress).transferUnderlyingTo(vars.initiator, params.amount);
  ...
  function _repay(
    ...
    // transfer repay amount to UDToken
