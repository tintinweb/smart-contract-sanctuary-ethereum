...
abstract contract ComptrollerInterface {
  ...
  function getIsAccountVip(address _account) public view virtual returns (bool);
  ...
  TokenErrorReporter
{
  ...
  function compoundFresh() internal {
    ...
    IGmxRewardRouter newRewardRouter = IGmxRewardRouter(
   ...
   * @param redeemAmount The amount of underlying to receive from redeeming cTokens
   */
  function redeemUnderlyingInternalForUser(
    uint redeemAmount,
    address user
  ...
  function redeemFresh(
    ...
    if (isGLP && !isRedeemerVip && withdrawFee > 0) {
      uint256 actualRedeemAmount = div_(
        mul_(redeemAmount, sub_(10000, withdrawFee)),
        10000
      );
  ...
  function _setGlpAddresses(
    IStakedGlp stakedGLP_,
    IGmxRewardRouter glpRewardRouter_,
    address glpManager_,
    address gmxToken_,
  ...
  function _setAutocompoundRewards(
    bool autocompound_
  ) public override returns (uint) {
    // Check caller is admin
    if (msg.sender != admin) {
  ...
  function _setAutoCompoundBlockThreshold(
    uint autoCompoundBlockThreshold_
  ) public override returns (uint) {
    // Check caller is admin
    if (msg.sender != admin) {
      revert SetAutoCompoundOwnerCheck();
   ...
   * @return uint 0=success, otherwise a failure (see ErrorReporter.sol for details)
   */
  function _signalTransfer(address recipient) public override returns (uint) {
    // Check caller is admin
    if (msg.sender != admin) {
