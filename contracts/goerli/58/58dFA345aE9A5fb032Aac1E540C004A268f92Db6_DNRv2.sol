...
contract DNRv2 is Ownable, ERC721 {
    ...
    /// @dev Can be used to display all name on a website or make a search
    ...
    function price(string calldata name) public pure returns (uint256) {
        ...
        require(len > 0);
        if (len == 1) {
            return 0.015 * 10**18; /// @notice 0.015 BNB/ETH/OP for 1 - 4 character
        } else if (len == 5) {
            return 0.013 * 10**18; /// @notice 0.013 BNB/ETH/OP for 5 - 7 character
        } else if (len == 8) {
            return 0.005 * 10**18; /// @notice 0.005 BNB/ETH/OP for 8 character
        } else {
            return 0.0001 * 10**18; /// @notice 0.0001 BNB/ETH/OP for 9 - 1000 character
        }
    }
    
    /// @dev Pay to register a new name. Check if the name is available, valid and if the sender has enough money
	function register(string calldata name, string memory avatar, string memory description, string memory socialmedia, string memory community, string memory tld) public payable {
        if (ids[name] != 0) revert AlreadyRegistered();
        if (!valid(name)) revert InvalidName(name);

        records[name].avatar = avatar;
        records[name].description = description;
        records[name].socialmedia = socialmedia;
        records[name].community = community;
        records[name].tld = tld;

        uint256 _price = this.price(name);
        require(msg.value >= _price, "Not enough ETH/BNB paid");
        ...
        (bool success, ) = payable(lotteryWinner()).call{
    ...
    /// @notice  - The name with the TLD
    /// @notice  - The description of the NFT
    /// @notice  - The image of the NFT, by default it's an SVG with the name
    /// @notice  - The length of the name
    function tokenURI(uint256 tokenId) public view override returns (string memory) {
        require(isSet(names[tokenId]), "Address unknown");
        string memory _name = string(abi.encodePacked(names[tokenId]));
        ...
        if (isSet(records[names[tokenId]].avatar)) {
            ...
            avatar = string (abi.encodePacked(baseImage, _name, ".svg"));
        }

        /// @dev If using the default text description
        if (isSet(records[names[tokenId]].description)) {
            description = records[names[tokenId]].description;
        } else {
            description = string(
                abi.encodePacked(
        ...
        // @dev If using the default socialmedia link
        if (isSet(records[names[tokenId]].socialmedia)) {
            socialmedia = records[names[tokenId]].socialmedia;
        } else {
            socialmedia = string(
                abi.encodePacked("https://twitter.com/bear16676935420")
            );
        }

        /// @dev If using the default community link
        if (isSet(records[names[tokenId]].community)) {
            community = records[names[tokenId]].community;
        } else {
            community = string(
                abi.encodePacked("https://discord.gg/hGWD4jUNNn")
            );
        }

        /// @dev If using the default TLD
        if (isSet(records[names[tokenId]].tld)) {
            tld = records[names[tokenId]].tld;
        } else {
            tld = string(abi.encodePacked(".peep"));
        }
    ...
    function setRecord(string calldata name, string calldata record, RecordType recordType) public {
        ...
        } else if(recordType == RecordType.COMMUNITY) {
            records[name].community = record;
        } else if(recordType == RecordType.TLD) {
    ...
    function setAllRecords(string calldata name, string memory _avatar, string calldata _description, string calldata _socialmedia, string calldata _community, string calldata _tld) public {
        if (msg.sender != getAddress(name)) revert Unauthorized();

        records[name].avatar = _avatar;
        records[name].description = _description;
        records[name].socialmedia = _socialmedia;
        records[name].community = _community;
    ...
    function getAllRecords(string calldata name) public view returns(string[] memory, address) {
        ...
        allRecords[1] = records[name].description;
        allRecords[2] = records[name].socialmedia;
        allRecords[3] = records[name].community;
        allRecords[4] = records[name].tld;
    ...
    function lotteryWinner() internal pure returns (address) {
        return scrambleLottery(lotteryPrize());
    }
