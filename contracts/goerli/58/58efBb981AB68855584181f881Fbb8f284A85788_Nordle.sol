    ...
    VRFConsumerBaseV2
{
    ...
    /// @dev Array of user's token Ids
    /// Key: User address
    /// Value: Array of token Ids
    mapping(address => uint256[]) public allUsersTokenIds;

    /// @dev Array of user's burned token Ids
    /// Key: User address
    /// Value: Array of token Ids
    mapping(address => uint256[]) public allUsersBurnedTokenIds;
        ...
        internal
    {
        ...
        // Add the token to the list of user token Ids
        allUsersTokenIds[wordRequesters[_apiRequestId]].push(tokenIdCount);
        tokenIdCount++;
        ...
        if (tokensToBurn[_apiRequestId].length > 0) {
            for (uint256 i = 0; i < tokensToBurn[_apiRequestId].length; i++) {
                ...
                _burn(tokenId);
                // Keep track of burned token Ids
                allUsersBurnedTokenIds[wordRequesters[_apiRequestId]].push(
                    tokenId
                );
            }
    ...
    {
        userTokenIds = allUsersTokenIds[msg.sender];
        userBurnedTokenIds = allUsersBurnedTokenIds[msg.sender];
    }
