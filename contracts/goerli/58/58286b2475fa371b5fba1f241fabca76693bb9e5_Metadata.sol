...
// File: contracts/DigiGhostsRenderer.sol
...
contract Metadata is Ownable {
    ...
    mapping(uint256 => address) onChainGhost1;
    mapping(uint256 => address) onChainGhost2;
    ...
    function buildMetadata(uint256 tokenId) public view returns(string memory) {
        if (onChainGhost1[tokenId] == address(0)) {
            return string(abi.encodePacked(
                'data:application/json;base64,', Base64.encode(bytes(abi.encodePacked(
                            '{"name": "Ghost ', tokenId.toString(), 
                            '", "description":"', 
                            "This Ghost is off the chain.",
                            '", "image": "', 
                            'https://storage.googleapis.com/nounpepes123/', 
                            tokenId.toString(), '.gif',
                            '"}')))));
        } else {
            return string(abi.encodePacked(
                'data:application/json;base64,', Base64.encode(bytes(abi.encodePacked(
                            '{"name": "Ghost ', tokenId.toString(), 
                            '", "description":"', 
                            "This Ghost is fully on-chain.", 
                            '", "image":', 
                            '"data:image/gif;base64,',
                            SSTORE2.read(onChainGhost1[tokenId]),
                            SSTORE2.read(onChainGhost2[tokenId]),
                            '"}')))));
        }
    }
    ...
    function putGhostOnChain1of2(uint256 tokenId, string calldata svg) external onlyFren {
        onChainGhost1[tokenId] = SSTORE2.write(bytes(svg));
    }
    function putGhostOnChain2of2(uint256 tokenId, string calldata svg) external onlyFren {
        onChainGhost2[tokenId] = SSTORE2.write(bytes(svg));
    }
    function takeNounPepeOffChain(uint256 tokenId) external onlyFren {
        onChainGhost1[tokenId] = address(0);
        onChainGhost2[tokenId] = address(0);
    }
