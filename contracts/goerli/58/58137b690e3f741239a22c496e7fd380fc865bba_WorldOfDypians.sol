...
/**
 * @title WorldOfDypians contract
 * @dev Extends ERC721 Non-Fungible Token Standard basic implementation
 */
contract WorldOfDypians is ERC721A, Ownable {
    using SafeMath for uint256;
    ...
    address cawsContractAddress;

    ERC721A cawsContract;

    uint256 public constant landPrice = 6500000000000000; //0.0065 ETH
    
    uint256 public constant LandPriceDiscount = 5000000000000000; //0.005 ETH
    //650000000000000000; //0.65 ETH
    uint public constant maxLandPurchase = 100;
    ...
    constructor(string memory name, string memory symbol, uint256 maxNftSupply, uint256 saleStart, address _cawsContractAddress) ERC721A(name, symbol, maxLandPurchase, maxNftSupply) {
        MAX_WOD = maxNftSupply;
        REVEAL_TIMESTAMP = saleStart + (86400 * 13);
        cawsContractAddress = _cawsContractAddress;
        cawsContract = ERC721A(cawsContractAddress);
    }
    ...
    // Mapping to keep track of CAWS token IDs that have been used to mint a new NFT
        mapping (uint256 => bool) public usedCAWS;


    /**
     * Mint the rest of WOD
     */
    function reserveWod(uint numberOfTokens) public onlyOwner {
        require(numberOfTokens > 0, "Mint at least 1 NFT");
        require(totalSupply() >= 1000, "Genesis Mint is not Finished");
    ...
    * Mints WOD
    */
    function mintWodGenesis(uint numberOfTokens) public payable {
...
//
    function mintWodGenesisWithDiscount(uint numberOfTokens, uint256 cawsTokenId) public payable {
        require(cawsContract.ownerOf(cawsTokenId) == msg.sender);
        ...
        require(usedCAWS[cawsTokenId] == false, "This CAWS NFT has already been used to mint a WoD Genesis NFT with Discount");
        usedCAWS[cawsTokenId] = true;
