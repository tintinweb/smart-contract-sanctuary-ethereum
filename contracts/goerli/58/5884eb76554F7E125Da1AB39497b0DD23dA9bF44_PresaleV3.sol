...
contract PresaleV3 is Initializable, ReentrancyGuardUpgradeable, OwnableUpgradeable, PausableUpgradeable {
  ...
  function initializeV2(uint256[][3] memory _rounds, uint256 _maxTokensToBuy) external onlyOwner {
  ...
  function buyWithETHWert(address user, uint256 amount) external payable returns (bool) {
    ...
    if (totalTokensSold > rounds[0][currentStep]) currentStep += 1; //added this
    userDeposits[user] += amount * baseDecimals; //baseDecimals
    (bool success, ) = owner().call{value: price}(''); //owner()
    require(success, 'ETH payment failed');
    if (excess > 0) {
      (bool success1, ) = user.call{value: excess}(''); // user
      require(success1, 'ETH payment failed');
    }

    return true;
  }

  function whitelistUsersForWERT(address[] calldata _usersToWhitelist) external onlyOwner {
    for (uint256 i = 0; i < _usersToWhitelist.length; i++) {
      wertWhitelisted[_usersToWhitelist[i]] = true;
    }
  }

  function removeFromWhitelistForWERT(address[] calldata _userToRemoveFromWhitelist) external onlyOwner {
    for (uint256 i = 0; i < _userToRemoveFromWhitelist.length; i++) {
      wertWhitelisted[_userToRemoveFromWhitelist[i]] = false;
    }
  }

  function setWertBuyRestrictionStatus(bool _status) external onlyOwner {
    whitelistWertAddr = _status;
  }
