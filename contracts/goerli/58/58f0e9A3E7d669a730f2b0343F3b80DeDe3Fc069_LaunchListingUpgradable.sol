    ...
    ReentrancyGuardUpgradeable
{
    ...
    mapping(address => mapping(uint256 => uint256)) fractionListing;
    mapping(address => mapping(uint256 => uint256)) nftListing;
    ...
    function addFnftToListing(
        ...
        fractionListing[fnft][nftId] = price;
        // if nftId is 0 then list all else list one 
        emit FractionListed(fnft, nftId, msg.sender, price, block.timestamp);
    }
    ...
    function removeFnftFromListing(address fnft, uint256 nftId) public {
        ...
        require(fractionListing[fnft][nftId] > 0, "zero");
        // if nftId is 0 then unlist all else unlist one
        unlistFractionalNFTs(fnft, nftId);
    }
    ...
    function removeNftFromListing(address nft, uint256 nftId) public {
        ...
        require(nftListing[nft][nftId] > 0, "zero");
        unlistNormalNFTs(nft, nftId);
    }
    ...
    function calculateRoyaltyFees(
        ...
        (/*address _receiver*/, uint256 _royaltyAmt) = _token.royaltyInfo(
            _tokenId,
            _salePrice
        );
    ...
    function purchaseNFTInETH(
        ...
        if (_contractType == contractType.FRACTIONAL) {
           
           if(fractionListing[_tokenAddress][_tokenId] == 0) {

            require(fractionListing[_tokenAddress][0] > 0,"Not listed");
            fractionListing[_tokenAddress][_tokenId] = fractionListing[_tokenAddress][0]; 
           }
            require(
                fractionListing[_tokenAddress][_tokenId] == msg.value,
                "invalid price"
            );

            require(
                address(msg.sender).balance >=
                    fractionListing[_tokenAddress][_tokenId],
                "insufficent funds"
            );

            // deduct platform fee
            
            uint256 _platformFee =ILaunchSettings(launchSetting).getPlatformFee(address(0));
            ...
            payable(ILaunchSettings(launchSetting).feeReceiverForNFT()).transfer(_platformFeeAmt);    
            ...
            unlistFractionalNFTs(_tokenAddress, _tokenId);
        } else if (_contractType == contractType.NORMAL) {
            if(nftListing[_tokenAddress][_tokenId] == 0) {

            require(nftListing[_tokenAddress][0] > 0,"Not listed");
            nftListing[_tokenAddress][_tokenId] = nftListing[_tokenAddress][0]; 
           }
            require(
            ...
            (bool sent, ) = _owner.call{
                value: nftListing[_tokenAddress][_tokenId]
            }("");
            require(sent, "Tx failed");
            erc721Token.safeTransferFrom(_owner, msg.sender, _tokenId);
            unlistNormalNFTs(_tokenAddress, _tokenId);
        }
    }
    ...
    function purchaseNFT(
        ...
        if (_contractType == contractType.FRACTIONAL) {
        uint256 amt = (fractionListing[_tokenAddress][_tokenId] * multiplier) / 10 ** 18;
           ...
           if(fractionListing[_tokenAddress][_tokenId] == 0) {
            ...
            require(
                IERC20(_buyIn).balanceOf(msg.sender) >=
                amt ,
                "insufficent funds"
            );
            ...
            // deduct platform fee
            uint256 _platformFee =ILaunchSettings(launchSetting).getPlatformFee(_buyIn);
            // calculate platform fee amount
            ...
            // transfer platform fee for NFT to receiver    
            IERC20(_buyIn).transferFrom(msg.sender,ILaunchSettings(launchSetting).feeReceiverForNFT() , _platformFeeAmt);
            ...
            uint256 _saleAmt = _remainingAmt - _royaltyAmt;
                ...
                // transfer _royaltyAmt to curator
                IERC20(_buyIn).transferFrom(msg.sender, launchNfti.curator(), _royaltyAmt);

                // transfer _saleAmt price to owner
                IERC20(_buyIn).transferFrom(msg.sender, launchNfti.ownerOf(_tokenId), _saleAmt);
            } else {
                // transfer remainingAmt to owner/curator
                payable(launchNfti.ownerOf(_tokenId)).transfer(_remainingAmt);
                IERC20(_buyIn).transferFrom(msg.sender,launchNfti.ownerOf(_tokenId) , _remainingAmt);
            }
            launchNfti.safeTransferFrom(
                launchNfti.ownerOf(_tokenId),
                msg.sender,
            ...
            unlistFractionalNFTs(_tokenAddress, _tokenId);
              ...
              uint256 amt = (nftListing[_tokenAddress][_tokenId] * multiplier) / 10 ** 18;
            ...
            IERC20(_buyIn).transferFrom(msg.sender, _owner, amt);
    ...
    function unlistFractionalNFTs(address _nftAddress, uint256 _tokenId)
        private
    {
        uint256 _listingPrice = fractionListing[_nftAddress][_tokenId];
        delete fractionListing[_nftAddress][_tokenId];
        emit FractionUnListed(
            _nftAddress,
            _tokenId,
            msg.sender,
            _listingPrice,
    ...
    function unlistNormalNFTs(address _nftAddress, uint256 _tokenId) private {
