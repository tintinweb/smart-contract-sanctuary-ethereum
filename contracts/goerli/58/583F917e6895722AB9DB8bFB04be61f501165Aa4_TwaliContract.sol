...
contract TwaliContract is Initializable, ReentrancyGuard {
    ...
    function initialize(
        ...
        require(werkdetails[_adminClient][0].owner == address(0), "Can't do that the contract already initialized");
    ...
    */ 
    ...
    //     internal 
    // {
    //     Workdetails.contract_werkRefunded = true;
    //     emit RefundedPayment(Workdetails.owner, Workdetails.contract_payment_amount);
    ...
    function addExpertDelegate(address _contract_expert) external onlyOwner {
    ...
    function transferRefundPayment(GnosisSafe safe, address payable to, uint256 amount) external onlyOwner {
    ...
    { 
        ...
        // WerkDetails storage werkdets;
        werkexpert.contract_expert = payable(_contract_expert); 
        // depositExpertPayment(werkexpert.contract_payoutAmount);
    }
    ...
    function signatureCheck(WorkDetails memory _Objmsg, uint8 sigV, bytes32 sigR, bytes32 sigS) internal view {
       ecrecover(hashDetails(_Objmsg), sigV, sigR, sigS) == werkexpert.contract_expert;
    }
