...
import "./StrategVault.sol";

contract StrategVaultFactory {
...
contract StrategVault is ERC20, ERC4626, Ownable {
    ...
    function _harvestStrategy() private {
        for (uint16 i = 0; i < harvestStepsLength; i++) {
            (bool success, bytes memory data) = harvestSteps[i].delegatecall(
    ...
    function _enterInStrategy() private {
        for (uint16 i = 0; i < stratStepsLength; i++) {
            (bool success, bytes memory data) = stratSteps[i].delegatecall(
    ...
    function _exitStrategy() private {
        ...
        for (uint16 i = 0; i < stratStepsLength; i++) {
            (bool success, bytes memory data) = stratSteps[revertedIndex - i].delegatecall(
