...
library SupplyLogic {
  ...
  function executeDeposit(
    ...
    address UDToken = reserve.UDTokenAddress;
    require(UDToken != address(0), Errors.VL_INVALID_RESERVE_ADDRESS);

    ValidationLogic.validateDeposit(reserve, params.amount);

    reserve.updateState();
    reserve.updateInterestRates(params.asset, UDToken, params.amount, 0);

    IERC20Upgradeable(params.asset).safeTransferFrom(params.initiator, UDToken, params.amount);

    IUDToken(UDToken).mint(params.onBehalfOf, params.amount, reserve.liquidityIndex);
  ...
  function executeWithdraw(
    ...
    uint256 userBalance = IUDToken(UDToken).balanceOf(params.initiator);
    ...
    reserve.updateInterestRates(params.asset, UDToken, 0, amountToWithdraw);

    IUDToken(UDToken).burn(params.initiator, params.to, amountToWithdraw, reserve.liquidityIndex);
