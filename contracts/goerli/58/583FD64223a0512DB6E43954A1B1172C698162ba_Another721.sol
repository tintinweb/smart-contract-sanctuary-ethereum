...
contract Another721 is ERC721ABv2, ERC721ABErrors {
    // Base Token URI
    string private baseTokenURI;

    // Phase definition per drop ID
    Phase[4] public phasesPerDrop;

    // Stores the amounts of tokens minted per address and per drop per phase
    mapping(address => mapping(uint256 => uint256)) public mintedPerPhase;

    // Anotherblock PFP NFT contract interface
    ...
    function initialize(
        ...
        __ERC721ABv2_init(_dropManager, _name, _symbol);
    ...
    function mintPfpHolder(
        ...
        IABDropManager.Drop memory drop = _verifyEligibility(
            dropId,
            _quantity,
            msg.value
        );
        ...
        if (block.timestamp > phasesPerDrop[1].phaseStart)
            revert PhaseCompleted();

        // Check that the user is eligible for this phase (holds at least one PFP NFT & is included in the merkle root)
        ...
        if (mintedPerPhase[_to][0] + _quantity > maxMint)
            revert MaxMintPerAddress();

        mintedPerPhase[_to][0] += _quantity;
    ...
    /**
     * @notice
     *  Let a AB NFT (Season 1 or Season 2) holder mint `_quantity` token(s) of the given `_dropId`
     *
     * @param _to recipient address
     * @param _quantity amount of tokens to be minted
     * @param _proof merkle tree proof used to verify whitelisted user
     */
    function mintSeasonHolder(
        address _to,
        uint256 _quantity,
        bytes32[] memory _proof
    ) external payable {
        ...
        // Check that the first phase has started (revert otherwise)
        if (block.timestamp < phase.phaseStart) revert SaleNotStarted();
        if (block.timestamp > phasesPerDrop[2].phaseStart)
            revert PhaseCompleted();
        ...
        // Check that user did not mint the maximum amount per address for the current phase
        if (mintedPerPhase[_to][1] + _quantity > phase.maxMint)
            revert MaxMintPerAddress();

        mintedPerPhase[_to][1] += _quantity;
    ...
    function mintAllowlist(
        ...
        // Check that the first phase has started (revert otherwise)
        if (block.timestamp < phase.phaseStart) revert SaleNotStarted();
        if (block.timestamp > phasesPerDrop[3].phaseStart)
            revert PhaseCompleted();

        // Check that user did not mint the maximum amount per address for the current phase
        if (mintedPerPhase[_to][2] + _quantity > phase.maxMint)
            revert MaxMintPerAddress();
        ...
        mintedPerPhase[_to][2] += _quantity;
     ...
     * @param _quantity : amount of tokens to be minted
     */
    function mintPublic(address _to, uint256 _quantity) external payable {
        IABDropManager.Drop memory drop = _verifyEligibility(
            dropId,
            _quantity,
            msg.value
        ...
        Phase memory phase = phasesPerDrop[3];

        // Check that the first phase has started (revert otherwise)
        if (block.timestamp < phase.phaseStart) revert SaleNotStarted();

        // Check that user did not mint the maximum amount per address for the current phase
        if (mintedPerPhase[_to][3] + _quantity > phase.maxMint)
            revert MaxMintPerAddress();

        mintedPerPhase[_to][3] += _quantity;
        _safeMint(_to, _quantity);
        IABDropManager(dropManager).updateDropCounter(dropId, _quantity);
    }
    ...
    // TODO : Get the rightholder address from the drop (?)
    /**
     * @notice
     *  Withdraw mint proceeds to the right holder address
     *
     * @param _rightholder : address of the rightholder
     * @param _amount : amount to be transferred
     */
    function withdrawToRightholder(
        address _rightholder,
        uint256 _amount
    ) external onlyOwner {
        if (_rightholder == address(0)) revert ZeroAddress();
    ...
    function _verifyEligibility(
        ...
        uint256 _quantity,
        uint256 _amount
    ) internal view returns (IABDropManager.Drop memory) {
    ...
    OwnableUpgradeable
{
    ...
    // If dropId can be changed
    bool public locked;

    // If opensea registry filter is enabled
    ...
    function _beforeTokenTransfers(
        ...
        for (uint256 i = 0; i < quantity; ++i) {
            IABDropManager(dropManager).updateOnTransfer(from, to, dropId);
        }
    }
}
...
contract ERC721ABErrors {
    ...
    **/
    ...
    /**
    @notice Error returned if the phase is completed
    **/
    error PhaseCompleted();
