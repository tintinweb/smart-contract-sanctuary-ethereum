...
/******************************************************************************\
/// @title Contract Facet for Native Token Transaction
/// @author Ather Labs
/******************************************************************************/
...
contract TxNativeFacet is IOperations {
    /* ------ TYPE & VARIABLES ------ */
    ...
    /* ------ EXTERNAL FUNTIONS ------ */
    /// @notice deposit native token to the portal under atherId account
    function depositNative() external payable {
        LibStatusControl._requireDepositEnable();
        require(msg.value > 0, "TN:invalid deposit amount");
        LibTokenManagement._requireUnpaused(0);

        Deposit memory deposit = Deposit({
            from: msg.sender,
            listingId: 0,
            tokenId: 0,
            amount: LibSafeCast.toUint88(msg.value)
        });
        bytes32 hashDepositRequest = keccak256(
            abi.encode(s.domainSeparator, LibOperations._hashDepositRequest(deposit))
        );
        uint256 newRequestId = LibOperations._addPendingRequest(hashDepositRequest);

        emit NewDeposit(msg.sender, deposit.listingId, deposit.tokenId, deposit.amount, s.currentBatchId, newRequestId);
    }
...
/******************************************************************************\
/// @title Library Operations - support operation features
/// @author Ather Labs
...
library LibOperations {
    ...
    uint256 internal constant REQUEST_EXPIRATION_PERIOD = 30 days;

    /// @dev Max deposit of ERC20 token that is possible to deposit
    uint256 internal constant MAX_DEPOSIT_AMOUNT = 309485009821345068724781056;

    /// @dev Max deposit of ERC20 token that is possible to deposit
    uint256 internal constant MAX_DEPOSIT_NFT_AMOUNT = 4294967295;

    /// @notice Read bytes input to reconstruct the Deposit request data struct
    /// @param _data Data input in bytes
    function _readDepositRequest(bytes memory _data, uint256 _offset)
        internal
        pure
        returns (IOperations.Deposit memory deposit)
    {
        require(_data.length == 44, "LOps:invalid data length");
        ...
        (offset, deposit.amount) = LibBytes.readUInt88(_data, offset); //43 bytes
    }

    /// @notice Read bytes input to reconstruct the DepositBatch request data struct
    /// @param _data Data input in bytes
    function _readDepositBatchRequest(bytes memory _data, uint256 _offset)
        internal
        pure
        returns (IOperations.DepositBatch memory depositBatch)
    {
        require(_data.length > 66 && (_data.length - 23) % 11 == 0, "LOps:invalid data length");
        uint256 offset = _offset; //offset start from 1
        ...
        require(depositBatch.arrayLength > 1 && depositBatch.arrayLength < 11, "LOps:invalid batch length");
        ...
        //min 66 + (n-2)*22 bytes| n>=2;n<=10
    }

    /// @notice Read bytes input to reconstruct the Withdraw request data struct
    /// @param _data Data input in bytes
    function _readWithdrawRequest(bytes memory _data, uint256 _offset)
        internal
        pure
        returns (IOperations.Withdraw memory withdraw)
    {
        require(_data.length == 44, "LOps:invalid data length");
        uint256 offset = _offset; //offset start from 1
    ...
    /// @param _data Data input in bytes
    function _readWithdrawBatchRequest(bytes memory _data, uint256 _offset)
        internal
        pure
        returns (IOperations.WithdrawBatch memory withdrawBatch)
    {
        require(_data.length > 66 && (_data.length - 23) % 11 == 0, "LOps:invalid data length");
        uint256 offset = _offset; //offset start from 1
        (offset, withdrawBatch.to) = LibBytes.readAddress(_data, offset);
        (offset, withdrawBatch.listingId) = LibBytes.readUInt8(_data, offset);
        (offset, withdrawBatch.arrayLength) = LibBytes.readUInt8(_data, offset);
        require(withdrawBatch.arrayLength > 1 && withdrawBatch.arrayLength < 11, "LOps:invalid batch length");
        (offset, withdrawBatch.tokenIds) = LibBytes.readUInt88Array(_data, offset, withdrawBatch.arrayLength);
        (offset, withdrawBatch.amounts) = LibBytes.readUInt88Array(_data, offset, withdrawBatch.arrayLength);
        //min 66 + (n-2)*16 bytes| n>=2;n<=10
    }

    /// @notice Read bytes input to reconstruct the Withdraw1155C request data struct
    /// @param _data Data input in bytes
    function _readWithdraw1155CRequest(bytes memory _data, uint256 _offset)
        internal
        pure
        returns (IOperations.Withdraw1155C memory withdraw1155C)
    {
        require(_data.length > 112 && (_data.length - 91) % 11 == 0, "LOps:invalid data length");
        uint256 offset = _offset; //offset start from 1
        ...
        require(withdraw1155C.arrayLength > 0 && withdraw1155C.arrayLength < 11, "LOps:invalid batch length");
        ...
        //min 112 + (n-2)*16 bytes| n>=2;n<=10
    }

    /// @notice Add new pending request
    /// @param _hashedRequestData hashed bytes32 of deposit data
    /// @return newRequestId new id of the request
    function _addPendingRequest(bytes32 _hashedRequestData) internal returns (uint256 newRequestId) {
        AppStorage storage s = LibAppStorage.diamondStorage();
        uint256 expirationTimestamp_ = block.timestamp + REQUEST_EXPIRATION_PERIOD;
        newRequestId = s.currentPendingRequestId + s.numberOfPendingRequest;

        s.pendingDepositRecord[newRequestId] = IOperations.PendingDepositRecord({
    ...
    function _minU128(uint128 a, uint128 b) internal pure returns (uint128) {
        return a < b ? a : b;
    }
    ...
    function _verifyRoot(
        ...
        require(_stageRoot != bytes32(0), "invalid root");
        bytes32 computedHash = keccak256(abi.encode(_defaultReceiver, _listingId, _tokenIds, _amounts));
        ...
        return computedHash == _stageRoot;
    }
}
...
library LibStatusControl {
    ...
    function _requireDepositEnable() internal view {
        AppStorage storage s = LibAppStorage.diamondStorage();
        require(s.status == 1, "LSta:deposit disabled");
    }

    /// @notice Checks if operations features are enabled
    function _requireOperationsEnable() internal view {
        AppStorage storage s = LibAppStorage.diamondStorage();
        require(s.status == 1 || s.status == 2, "LSta:operations disabled");
    }

    /// @notice Checks if withdraw features are enabled
    function _requireWithdrawEnable() internal view {
        AppStorage storage s = LibAppStorage.diamondStorage();
        require(
            s.status == 1 || (s.status == 0 && (block.timestamp < s.maintenanceExpirationTime / 2)),
            "LSta:withdraw disabled"
        );
    }

    /// @notice Checks that current state is Maintenance
    function _requireExitEnable() internal view {
        AppStorage storage s = LibAppStorage.diamondStorage();
        require(s.status == 3, "LSta: Exis disabled. Contract is not in mass exit mode");
    }
}
...
interface IOperations {
    ...
    /// @notice Struct data for WithdrawBatch request, support: ERC1155, ERC1155M
...
library LibTokenManagement {
    ...
    function _validateTokenAddress(address _tokenAddr) internal view returns (uint8) {
        require(_tokenAddr != address(0), "Gov:invalid token's address");
        AppStorage storage s = LibAppStorage.diamondStorage();

        uint8 listingId = s.listingIds[_tokenAddr];
        require(listingId != 0, "Gov:invalid listingId");
        return listingId;
    }

    /// @notice Validate if listingId is valid, except for native token which has listingId = 0 by default
    /// @param _lisitngId listingId to validate
    /// @return tokenAddr address of valid listingId
    function _validateLisitingId(uint8 _lisitngId) internal view returns (address) {
        require(_lisitngId != 0, "Gov:invalid listingId");
        AppStorage storage s = LibAppStorage.diamondStorage();

        address tokenAddr = s.tokenAddresses[_lisitngId];
        require(tokenAddr != address(0), "Gov:invalid token's address");
        return tokenAddr;
    }
    ...
    function _requireUnpaused(uint8 _listingId) internal view {
        AppStorage storage s = LibAppStorage.diamondStorage();
        require(!s.pausedTokens[_listingId], "Gov:token is paused");
    }
}
...
/******************************************************************************\
/// @title Library Safe Cast - convert data type
/// @author Ather Labs
...
/******************************************************************************\
/// @title Library Bytes - features to read and convert data in bytes
/// @author Ather Labs
/******************************************************************************/
