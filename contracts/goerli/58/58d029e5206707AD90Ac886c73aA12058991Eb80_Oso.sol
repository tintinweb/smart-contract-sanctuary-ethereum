...
contract Oso is ERC721, ERC721URIStorage, Pausable, Ownable, ERC721Burnable {
    ...
    function _beforeTokenTransfer(
        ...
        // if user is burning token or receiving token
        // if from is address 0 then token  is being issues
        // is to is address 0 then token is being burned
        require(
            from == address(0) || to == address(0),
            "TOKEN_TRANSFER_NOT_POSSIBLE"
        );
    ...
    function _afterTokenTransfer(
        ...
        // if from address(0) emit event attest

        if (from == address(0)) {
            emit Transfer(from, to, tokenId);
        } else if (to == address(0)) {
            emit Revoke(from, to, tokenId);
        }
    }
    ...
    function unequip(uint256 tokenId) external {
        require(ownerOf(tokenId) == msg.sender, "ONLY_OWNER_CAN_BURN");
        _burn(tokenId);
    }
