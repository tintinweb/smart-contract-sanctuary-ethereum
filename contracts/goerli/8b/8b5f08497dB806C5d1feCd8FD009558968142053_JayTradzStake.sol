...
uint256 public totalCapReachedReward;
uint256 public totalCapReachedReferral;
uint256 public totalCapReachedRoyalty;
...
function payout(uint256 listId, address _addr, uint256 _amount, bool cashPayout, uint8 _prize) external {
    ...
    if(listIdToQuarterly[id].isQuarterly == true) {
        if(block.timestamp <= qTimers[id].timeToNextClaim) {
            ...
            if(royalCap){
        totalCapReachedRoyalty+= listIdToQuarterly[id].amountPaid;
            }
            if(referralCap){
                totalCapReachedReferral += listIdToQuarterly[id].amountPaid;
            }
            if(rewardCap){
                totalCapReachedReward += listIdToQuarterly[id].amountPaid;
            }
        }
        ...
        if(block.timestamp <= sTimers[id].timeToNextClaim) {
            ...
            if(gradBonus) {
             ...
             if(royalCap){
        totalCapReachedRoyalty+= listIdToSemiAnnually[id].amountPaid;
            }
            if(referralCap){
                totalCapReachedReferral += listIdToSemiAnnually[id].amountPaid;
            }
            if(rewardCap){
                totalCapReachedReward += listIdToSemiAnnually[id].amountPaid;
            }
        }
            ...
            if(royalCap){
        totalCapReachedRoyalty+= listIdToMonthly[id].amountPaid;
            }
            if(referralCap){
                totalCapReachedReferral += listIdToMonthly[id].amountPaid;
            }
            if(rewardCap){
                totalCapReachedReward += listIdToMonthly[id].amountPaid;
            }
    }
    ...
    if(amount <= totalAdminPoints) {
        ...
        qTimers[id].termStart = block.timestamp;
            ...
            if(amount <= totalAdminPoints){
                ...
                require(remainder <= referralPoints + royaltyPoints, "Insufficient royalty and referral points.");
                remainder -= royaltyPoints;
                listIdToQuarterly[id].previousRoyaltyPoints += royaltyPoints;
                listIdToQuarterly[id].currentRoyaltyPoints = 0;
                listIdToQuarterly[id].previousReferralPoints += remainder;
