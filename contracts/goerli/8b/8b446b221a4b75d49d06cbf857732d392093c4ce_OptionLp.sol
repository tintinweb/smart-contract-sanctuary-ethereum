...
contract OptionLp is BaseOptionLp {
    ...
    function fillLpPosition(
        ...
        _fillLpPosition(isPut, ssov, strikeToken, lpIndex, amount);
        return true;
    }
    ...
    function multiFillLpPosition(
        ...
        for (uint256 i; i < lpIndices.length; ) {
            _fillLpPosition(isPut, ssov, strikeToken, lpIndices[i], amount[i]);
            unchecked {
                ++i;
            }
        }
        ...
        returns (bool)
    {
        ...
        if (!_killLpPosition(strikeToken, lpIndex)) {
    ...
    function multiKillLpPosition(
        ...
        for (uint256 i; i < lpIndices.length; ) {
            if (!_killLpPosition(strikeToken, lpIndices[i])) {
                revert LpPositionFailedToKill();
            }
            unchecked {
                ++i;
...
contract BaseOptionLp is Ownable, ReentrancyGuard, Pausable, ContractWhitelist {
    ...
    uint256 internal constant SSOV_OPTIONS_PRECISION = 1e18;
    ...
    function getSsovCollateralPrice(address vault)
        public
        view
        returns (uint256)
    {
        return getSsov(vault).getCollateralPrice();
    }
