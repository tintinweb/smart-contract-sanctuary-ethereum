...
contract TestKeep3rHelper is IKeep3rHelper {
  address public keep3rV2;

  constructor(address _kp3r, address _KP3R_WETH_POOL) {
    KP3R = _kp3r;
    KP3R_WETH_POOL = _KP3R_WETH_POOL;
  }

  /// @inheritdoc IKeep3rHelper
  address public override KP3R;

  /// @inheritdoc IKeep3rHelper
  address public override KP3R_WETH_POOL;
  ...
  function setKP3RAddress(address _kp3rAddress) public {
    KP3R = _kp3rAddress;
  }

  function setKP3RWethPoolAddress(address _keep3rV2Address) public {
    keep3rV2 = _keep3rV2Address;
  }

  function setKP3RV2Address(address _keep3rV2Address) public {
    keep3rV2 = _keep3rV2Address;
  }
  ...
  function getPoolTokens(address _pool) public view override returns (address _token0, address _token1) {
    return (ITestUniV3Pool(_pool).token0(), ITestUniV3Pool(_pool).token1());
  }
  ...
  {
    int56 _currentTick = IKeep3r(keep3rV2).observeLiquidity(_pool).current;
    ...
    if (_secondsAgo.length == 1) {
      ...
      try ITestUniV3Pool(_pool).observe(_secondsAgo) {
        _tickCumulative1 = 0;
