...
import { BaseConfig, AdvancedConfig, AddressConfig, BurnToken, LaunchpadStorage } from '../libraries/LaunchpadStorage.sol';
    ...
    RevokableOperatorFiltererUpgradeable
{
     ...
     * @param _advancedConfig Optional advanced configuration settings
     * @param _addressConfig Optional address configuration settings
     * @param _burnTokens Optional array of burn token configuration settings
     */
    function initialize(
        address _owner,
        string memory _name,
        ...
        for (uint256 i = 0; i < _burnTokens.length; i++) {
            LaunchpadStorage.state().burnTokens.push(_burnTokens[i]);
        }
        ...
        __OperatorFilterer_init(
            ...
            true // only subscribe if a filter is provided
        );
    }
    ...
    function supportsInterface(bytes4 interfaceId)
        ...
        view
        virtual
        override(ERC721AUpgradeable, IERC721AUpgradeable, ERC4907AUpgradeable, IERC165Upgradeable)
        returns (bool)
    {
    ...
    function reduceMaxSupply(uint16 _newMaxSupply) external onlyOwner {
        require(_newMaxSupply < LaunchpadStorage.state().cfg.maxSupply, "NEW_MAX_SUPPLY_TOO_HIGH");
    ...
    function _baseURI() internal view virtual override returns (string memory) {
        return bytes(LaunchpadStorage.state().cfg.uriBase).length == 0
            ? string(
                abi.encodePacked(
    ...
    function setApprovalForAll(address operator, bool approved) public override(ERC721AUpgradeable, IERC721AUpgradeable) onlyAllowedOperatorApproval(operator) {
        require(!LaunchpadStorage.state().cfg.soulbindingActive, "TOKENS_ARE_SOULBOUND");
    ...
    function withdraw() external nonReentrant onlyOwner {
        ...
        if (LaunchpadStorage.state().addrCfg.payoutAddresses.length == 0) {
            ...
            for (
                ...
                uint256 amount = balance * LaunchpadStorage.state().addrCfg.payoutBasisPoints[i] / 10000;
                (bool success, ) = LaunchpadStorage.state().addrCfg.payoutAddresses[i].call{value:amount}("");
    ...
    function gift(address[] calldata _receivers, uint256[] calldata _mintNumber)
        ...
        onlyOwner
    {
        require(_receivers.length == _mintNumber.length, 'ARRAY_LENGTHS_MUST_MATCH');
        uint256 totalMint = 0;
    ...
    //  */
    // function setPublicSaleEndTime(uint256 _publicSaleEndTime) external onlyOwner {
    ...
    //  */
    // function setUsePublicSaleTimes(bool _usePublicSaleTimes) external onlyOwner {
    //     require(usePublicSaleTimes != _usePublicSaleTimes, "NEW_STATE_IDENTICAL_TO_OLD_STATE");
    ...
    function _generateRandomHash(uint256 _tokenId) internal {
        if (LaunchpadStorage.state().data.randomHashStore[_tokenId] == bytes32(0)) {
