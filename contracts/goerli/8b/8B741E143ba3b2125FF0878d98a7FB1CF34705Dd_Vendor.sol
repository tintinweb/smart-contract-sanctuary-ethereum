...
contract Vendor is Ownable {
    ...
    function buyTokens() public payable {
        // Use the msg.value and tokensPerEth to calculate the number of tokens to buy.
        uint256 amountOfEth = msg.value;
        require(amountOfEth > 0, "You must send some ETH");

        // Calculate the amountOfTokens we are buying.
        ...
        // Next, transfer the tokens to msg.sender
    ...
    function withdraw() public onlyOwner {
        ...
        // Check that there is some ETH to withdraw
        uint256 vendorEthBalance = address(this).balance;
        require(vendorEthBalance > 0, "Vendor must have some ETH to withdraw");

        // Transfer the ETH to the owner
        (bool sent, ) = msg.sender.call{ value: vendorEthBalance}("");
        require(sent, "Failed to withdraw ETH");
    }

    // create a sellTokens(uint256 _amount) function:
    function sellTokens(uint256 _amount) public {
        // Validate that the seller is selling more than 0 tokens
        require(_amount > 0, "You must sell some tokens");

        // Validate that the user has tokens to sell
        address seller = msg.sender;
        uint256 sellerBalance = yourToken.balanceOf(seller);
        require(sellerBalance >= _amount, "You do not have enough tokens");
        ...
        // Transfer the tokens
        (bool sent) = yourToken.transferFrom(seller, vendor, _amount);
        require(sent, "Could not transfer tokens");

        // Transfer the ETH
        (bool ethSent, ) = seller.call{ value: amountOfEth }("");
        require(ethSent, "Failed to send ETH back to seller");


        // Finally, emit the SellTokens event
        emit SellTokens(seller, amountOfEth, _amount);
    }
    /* <---- EVENTS ------> */
