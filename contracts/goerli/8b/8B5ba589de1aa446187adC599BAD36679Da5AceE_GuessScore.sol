...
contract GuessScore is ReentrancyGuard, Ownable {
    ...
    mapping(address => bool) private opWhitelist;
    ...
    modifier onlyOP() {
        require(
            opWhitelist[msg.sender] || msg.sender == owner(),
            "unauthorized"
        );
        _;
    }
    ...
    function getTeamUser(bytes32 teamId)
        public
        view
    ...
    function getTeamUserDepoistAmount(bytes32 teamId, address user)
        public
        view
        returns (uint256)
    {
    ...
    function getTeamUserscoreAmount(
        ...
        return teamsData[teamId].usersScore[user][score];
    }

    function getTeamTotalAmount(bytes32 teamId) public view returns (uint256) {
        return teamsData[teamId].totalAmount;
    }

    function getTeamTotalReward(bytes32 teamId) public view returns (uint256) {
        return teamsData[teamId].totalReward;
    }

    function getTeamTurnOn(bytes32 teamId) public view returns (bool) {
        return teamsData[teamId].turnOn;
    }

    function getTeamlat(bytes32 teamId) public view returns (bytes32) {
        return teamsData[teamId].score;
    }
    ...
    function withdrawal(bytes32 teamId) public nonReentrant {
        ...
        require(
            token.balanceOf(address(this)) >= teamsData[teamId].totalAmount,
            "Lack of balance"
        );

        uint256 share = getTeamShare(msg.sender, teamId, score);

        uint256 reward = (teamsData[teamId].totalAmount / 10**18) * share; // 实际奖励
    ...
    function getTeamShare(
        ...
        uint256 total = teamsData[teamId].totalAmount;
    ...
    function openRewards(bytes32 teamId, bytes32 score)
        public
        onlyOP
        nonReentrant
    {
    ...
    function setStopDeposit(bytes32 teamId, bool b) public onlyOP nonReentrant {
    ...
    function setOPWhitelist(address to, bool b) public onlyOwner {
        opWhitelist[to] = b;
    }
    ...
    constructor() Ownable() {
        token = ERC20(0x643C565D11Ec7975018Bc9f81B69E00cA11A59D2);
        feeAddr = 0x284CE33481f205A8250a487CEe031B82429413F1;
        poolAddr = 0x85CDC44b0fb92090AA0C3Bf0e66D4217d963A8fd;
    }
}
