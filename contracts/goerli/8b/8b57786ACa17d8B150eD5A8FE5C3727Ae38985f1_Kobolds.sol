...
contract Kobolds is ERC721A,Ownable, ReentrancyGuard {
    ...
    // for public
    uint256 public constant publicCost = 0.02 ether;
    ...
    bytes32 private _devMerkleRoot;
    uint256 public constant maxDevMintPerWallet = 10;

    constructor() ERC721A("Kobolds","KBT",maxPerWallet,MAX_SUPPLY) {
    }

    // public function  
    function freeMint() external{
        require(curOpenState,"not open yet!");
        require(freeMinted < MAX_FREE_SUPPLY, "free mint out!");
        require(freeMintRela[msg.sender] < maxFreePerWallet, "already free minted!");

        uint256 addr_freeminted = freeMintRela[msg.sender];
        freeMintRela[msg.sender] = addr_freeminted + 1;
        freeMinted = freeMinted + 1;
        _safeMint(msg.sender,1);
    }

    function setDevMintMerkleRoot(bytes32 newMerkleRoot) external onlyOwner{
        _devMerkleRoot = newMerkleRoot;
    }

    function devteamMint(bytes32[] memory proof) 
            external 
            payable 
            isValidMerkleProof(proof,_devMerkleRoot) {
        require(curOpenState,"not open yet!");
        _safeMint(msg.sender,maxDevMintPerWallet);
    }

    function Mint(uint256 quantity) external payable mintCompliance(quantity) {
        require(curOpenState,"not open yet!");
        require(quantity <= maxPerWallet,"max 10 once time !");
    ...
    function canProof(bytes32[] calldata merkleProof) view external returns (bool res){
        require(_devMerkleRoot != "", "root not set");
        if (MerkleProof.verify(merkleProof,_devMerkleRoot,keccak256(abi.encodePacked(msg.sender))))
        {
            return true;
        }
        return false;
