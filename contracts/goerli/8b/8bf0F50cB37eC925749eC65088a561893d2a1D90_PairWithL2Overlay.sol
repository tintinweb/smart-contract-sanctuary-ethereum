...
contract PairWithL2Overlay is UniswapV2Pair, EmergencyWithdrawal, StarkWareAssetData, Initializable {
  ...
  */
  ...
  function orderFundingArgs (
    FundingArgs memory fundingArgs
  ...
  ) external operatorOnly l2Only returns(bool) {
  ...
  function _burnStarkware(
    ...
    amount0 = lpQuantisedAmount.mul(balance0) / _totalSupply; // using balances ensures pro-rata distribution
    amount1 = lpQuantisedAmount.mul(balance1) / _totalSupply; // using balances ensures pro-rata distribution
    
    finalBalance0 = balance0 - amount0;
    finalBalance1 = balance1 - amount1;
    ...
    lpAssetId, amount0, lpAmountA, 0, vault, vault, vault, localNonce, unsignedInt22);

    starkEx.registerLimitOrder(exchangeAddress, token1AssetId, lpAssetId,
    lpAssetId, amount1, lpAmountB, 0, vault, vault, vault, localNonce, unsignedInt22);
  ...
  function settleStarkWare() external operatorOnly returns(bool) {
    ...
    { // block to avoid stack limit exceptions
      ...
      balance1 = token0Quanatum.toQuantizedUnsafe(balance1);
  ...
  function _withdrawAllFromVault() private {
    IStarkEx starkEx = getStarkEx();
    withdrawAllFromVaultIn(starkEx, lpAssetId);
    withdrawAllFromVaultIn(starkEx, token0AssetId);
    withdrawAllFromVaultIn(starkEx, token1AssetId);
  }
