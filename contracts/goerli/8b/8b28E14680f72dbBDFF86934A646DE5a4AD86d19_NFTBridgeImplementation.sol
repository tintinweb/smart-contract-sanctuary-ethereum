...
contract NFTBridge is NFTBridgeGovernance {
    ...
    function completeTransfer(bytes memory proofBlob,uint16 proofIndex, bytes32[] memory merkleProof) public {
        _completeTransfer(proofBlob,proofIndex,merkleProof);
    }

    // Execute a Transfer message
    function _completeTransfer(bytes memory proofBlob, uint16 proofIndex,bytes32[] memory merkleProof) internal {
