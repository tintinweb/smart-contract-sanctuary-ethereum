    ...
    ReentrancyGuardUpgradeable
{
    ...
    function _creditRewards() private {
        ...
        if (countOfStakes > 0 && rewardsAccruing && nowTS > lastUpdateTS) {
            uint256 newRewardsWei = (((nowTS - lastUpdateTS) *
                _REWARDS_PER_HOUR *
                _WEI) / 3600);

            grossDueRewards += (newRewardsWei / _WEI);

            accYieldPerTknWei += (newRewardsWei / countOfStakes);
    ...
    function claimRewards() external {
        ...
        if (lbskrBal > 0) {
            grossDueRewards -= rewards; // this may be inaccurate, if funds deplete unexpectedly
    ...
    function withdraw(uint256[] calldata tokenIds) external nonReentrant {
        ...
        uint256 nowTS = block.timestamp;
        for (uint256 i; i < tokenIds.length; ++i) {
            require(tokenStakerMap[tokenIds[i]] == _msgSender());
...
abstract contract ReentrancyGuardUpgradeable is Initializable {
    ...
    function _preNonReentrant() private view {
