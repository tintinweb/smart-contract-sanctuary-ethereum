...
import "./generated/deploy/BaseAspenDeployerV2.sol";
    ...
    CoreRegistryEnabled
{
    ...
    event DeploymentFeeDetailsUpdated(uint256 newDeploymentFee, address newFeeReceiver);
    ...
    function supportsInterface(bytes4 interfaceId)
        public
        view
        override(BaseAspenDeployerV2, CoreRegistryEnabled, AccessControlUpgradeable)
        returns (bool)
    {
        return
            super.supportsInterface(interfaceId) ||
            BaseAspenDeployerV2.supportsInterface(interfaceId) ||
        ...
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        ...
        emit DeploymentFeeDetailsUpdated(_newDeploymentFee, _newFeeReceiver);
    }
    ...
    function disableDeploymentFee() public onlyRole(DEFAULT_ADMIN_ROLE) {
        deploymentFee = 0;
        emit DeploymentFeeDetailsUpdated(0, feeReceiver);
    }
    ...
    function deployAspenERC721Drop(
    ...
    ) external payable override isRegisterdOnCoreRegistry returns (IAspenERC721DropV2) {
        IGlobalConfigV0 aspenConfig = IGlobalConfigV0(CORE_REGISTRY);
        ...
        IDropFactoryDataTypesV0.DropConfig memory dropConfig = IDropFactoryDataTypesV0.DropConfig(
        ...
        return IAspenERC721DropV2(address(newContract));
    }
    ...
    function deployAspenERC1155Drop(
        ...
        IDropFactoryDataTypesV0.FeeDetails memory _feeDetails,
        bytes32 _operatorFiltererId
    ) external payable override isRegisterdOnCoreRegistry returns (IAspenERC1155DropV2) {
        IGlobalConfigV0 aspenConfig = IGlobalConfigV0(CORE_REGISTRY);
        /// Note: If operator id does NOT exist, it will throw an error here.
        /// Even in case we don't want to set an operator, we need to specify the NO_OPERATOR filterer id
        IOperatorFiltererDataTypesV0.OperatorFilterer memory filterer = aspenConfig.getOperatorFiltererOrDie(
        ...
        _payDeploymentFee();
        emit AspenInterfaceDeployed(address(newContract), major, minor, patch, interfaceId);
        return IAspenERC1155DropV2(address(newContract));
    }
...
import "@openzeppelin/contracts-upgradeable/utils/introspection/ERC165CheckerUpgradeable.sol";
import "../api/errors/IUUPSUpgradeableErrors.sol";
import "../generated/impl/BaseAspenCoreRegistryV0.sol";
import "./config/PlatformFeeConfigUpgradeable.sol";
import "./config/OperatorFiltererConfig.sol";
...
contract AspenERC1155DropFactory is Ownable, IDropFactoryEventsV1, ICedarImplementationVersionedV0 {
    ...
    constructor() {
        ...
        emit AspenImplementationDeployed(address(implementation), major, minor, patch, "IAspenERC1155DropV2");
    }
...
// Filename: './BaseAspenDeployerV2.sol'
...
abstract contract BaseAspenDeployerV2 is IAspenDeployerV2 {
    ...
    function implementationInterfaceId() virtual override public pure returns (string memory interfaceId) {
        interfaceId = "deploy/IAspenDeployer.sol:IAspenDeployerV2";
    }

    function supportsInterface(bytes4 interfaceID) virtual override public view returns (bool) {
        return (interfaceID != 0x0) && ((interfaceID != 0xffffffff) && ((interfaceID == 0x01ffc9a7) || ((interfaceID == type(IAspenDeployerV2).interfaceId) || ((interfaceID == type(IAspenDeployerOwnEventsV1).interfaceId) || (interfaceID == type(IAspenVersionedV2).interfaceId)))));
    }
}
...
abstract contract BaseAspenPaymentsNotaryV1 is IAspenFeaturesV0, IAspenVersionedV2, IPaymentNotaryV1 {
    ...
    function supportsInterface(bytes4 interfaceID) virtual override public view returns (bool) {
        return (interfaceID != 0x0) && ((interfaceID != 0xffffffff) && ((interfaceID == 0x01ffc9a7) || ((interfaceID == type(IAspenFeaturesV0).interfaceId) || ((interfaceID == type(IAspenVersionedV2).interfaceId) || ((interfaceID == type(IPaymentNotaryV1).interfaceId) || (interfaceID == type(IAspenPaymentsNotaryV1).interfaceId))))));
    }
    ...
    PaymentsNotary
{
    ...
    {
        return
            BaseAspenPaymentsNotaryV1.supportsInterface(interfaceId) ||
            AccessControlUpgradeable.supportsInterface(interfaceId);
    }
...
contract PaymentsNotary is Initializable, ContextUpgradeable, AccessControlUpgradeable, IPaymentNotaryV1 {
    ...
    function __PaymentsNotary_init(address _feeReceiver, uint256 _feeBPS)
        internal
        validFeeDetails(_feeReceiver, _feeBPS)
        onlyInitializing
    {
        __PaymentsNotary_init_unchained(_feeReceiver, _feeBPS);
    }

    function __PaymentsNotary_init_unchained(address _feeReceiver, uint256 _feeBPS)
        internal
        validFeeDetails(_feeReceiver, _feeBPS)
        onlyInitializing
    {
