...
contract TokenOffRamp is ITokenOffRamp, Pausable {
    ...
    uint256 private expiryTimePeriod; // Time in seconds
    uint256 private createTradeExpiryPeriod;
    ...
    {
        require(
            submittedTimestamp + createTradeExpiryPeriod > block.timestamp,
        ...
        Trade memory trade = Trade(
            ...
            toCurrency,
            fromAmount,
            block.timestamp + expiryTimePeriod
        );
        tradeForId[id] = trade;
        emit CreateTrade(id);
    }
        ...
        tradeNotProcessed(id)
    {
        ...
        tradeForId[id].status = TradeStatus.REFUNDED;
        require(token.transfer(tradeForId[id].from, tradeForId[id].fromAmount));
        emit RefundTrade(id);
    }
    ...
    function setExpiryTimePeriod(uint256 _time) public override whenPaused onlyMultisig {
        require(expiryTimePeriod != _time, 'Time period is already in use');
        require(_time >= 300, 'Time period cannot be less than 5 Minutes');
        uint256 tempTime = expiryTimePeriod;
        expiryTimePeriod = _time;
        emit UpdatedExpiryPeriod(msg.sender, tempTime, expiryTimePeriod);
    }

    /// @notice Set a new time period before the created trade expires if left in tx pool for too long
    /// @param _time time period in seconds
    function setCreateTradeTimePeriod(uint256 _time) public override whenPaused onlyMultisig {
        require(createTradeExpiryPeriod != _time, 'Time period is already in use');
        require(_time >= 60, 'Time period cannot be less than 60 seconds');
        uint256 tempTime = createTradeExpiryPeriod;
        createTradeExpiryPeriod = _time;
        emit UpdatedCreateTradeExpiryPeriod(msg.sender, tempTime, createTradeExpiryPeriod);
    }
...
interface ITokenOffRamp {
    ...
    function setExpiryTimePeriod(uint256 _time) external;

    function setCreateTradeTimePeriod(uint256 _time) external;
}
