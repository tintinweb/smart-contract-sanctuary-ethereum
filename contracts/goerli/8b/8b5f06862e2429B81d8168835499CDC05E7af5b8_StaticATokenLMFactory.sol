...
import "./StaticATokenLM.sol";
import "./AToken.sol";

contract StaticATokenLMFactory {
    
    event GotData(address aToken, string name, string symbol);
    event CreatedWrapper(address aToken, address aTokenWrapper);

    mapping(address => address) aTokenToWrapper;

    function create(address aTokenAddress) public {
        
        require(aTokenToWrapper[aTokenAddress] == address(0), "Wrapper exists");
        
        ILendingPool lendingPool =  AToken(aTokenAddress).POOL();
        string memory name =        ERC20(aTokenAddress).name();
        string memory symbol =      ERC20(aTokenAddress).symbol();
        name =      string(abi.encodePacked("Wrapped ", name));
        symbol =    string(abi.encodePacked("W", symbol));

        emit GotData(aTokenAddress, name, symbol);
        StaticATokenLM staticATokenLM = new StaticATokenLM();
        emit CreatedWrapper(aTokenAddress,  address(staticATokenLM));
        ...
        aTokenToWrapper[aTokenAddress] = address(staticATokenLM);
    }
    function getWrapperForAToken(address aTokenAddress) public view returns (address) {
        return aTokenToWrapper[aTokenAddress];
    }
}
