    ...
    ReentrancyGuardUpgradeable
{
    ...
    event TokenDeposited(address to, uint256 amount, uint256 timestamp);
    event TokenWithdrawn(address from, uint256 amount, uint256 timestamp);
    ...
    // revert
    error invalidTransaction(uint256 sent, uint256 amountRequired);
    // modifiers
    ...
    function withdraw(address payable from, uint256 amount)
        external
        nonReentrant
        whenPauseSA
    {
        require(from != address(0), "not valid owner");
        require(msg.sender == from, "not owner");
        require(
            balanceOf(msg.sender, 0) >= amount + _txFee,
            "not enough balance"
        );

        bool sent = from.send(amount);
        require(sent, "Failed to send Ether");
        burn(from, 0, amount + _txFee);
    }
    ...
    function trade(
        ...
        uint256 cid,
        uint256[] calldata amountBid,
        uint256[] calldata amountAsk,
        uint256 sumAmountBid,
        uint256 sumAmountAsk
    ) public onlySAnA {
        require(_pause == 0 || owner() == msg.sender, "Pausable: paused");
        if (from.length > 1) {
            if (bidStatus(to[0], cid, sumAmountBid) == 0) {
                revert invalidTransaction(sumAmountBid, balanceOf(to[0], cid));
            }
            for (uint256 i = 0; i < from.length; i++) {
                if (askStatus(from[i], 0, amountAsk[i]) == 0) {
                    revert invalidTransaction(
                        amountAsk[i],
                        balanceOf(from[i], cid)
                    );
                }
                transferTrade(to[0], from[i], cid, amountBid[i], "0x");
                transferTrade(from[i], to[0], 0, amountAsk[i], "0x");
            }
        } else if (to.length > 1) {
            if (askStatus(from[0], 0, sumAmountAsk) == 0) {
                revert invalidTransaction(
                    sumAmountAsk,
                    balanceOf(from[0], cid)
                );
            }
            for (uint256 i = 0; i < from.length; i++) {
                if (bidStatus(to[i], cid, amountBid[i]) == 0) {
                    revert invalidTransaction(
                        amountBid[i],
                        balanceOf(to[i], cid)
                    );
                }
                transferTrade(to[i], from[0], cid, amountBid[i], "0x");
                transferTrade(from[0], to[i], 0, amountAsk[i], "0x");
            }
        } else {
            if (askStatus(from[0], 0, sumAmountAsk) == 0) {
                revert invalidTransaction(
            ...
            transferTrade(to[0], from[0], cid, sumAmountBid, "0x");
            transferTrade(from[0], to[0], 0, sumAmountAsk, "0x");
        }

        emit TradeSucceed(from, to, cid, amountBid, amountAsk, block.timestamp);
    }
    ...
    function safeTransferFrom(
        ...
        uint256 amount,
        bytes memory data
    ) public payable override nonReentrant whenPauseSA {
        uint256 sentWei = msg.value;
