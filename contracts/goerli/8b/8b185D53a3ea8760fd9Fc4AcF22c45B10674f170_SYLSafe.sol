...
contract SYLSafe {
    ...
    uint256 immutable private _interestRate;
    ...
    function withdraw() external {
        require(_amount(msg.sender) != 0, "saving nonexists");
        require(_done(msg.sender), "cannot withdraw");

        _marketing.release();
        _erc20.transfer(msg.sender, _amount(msg.sender));
        _erc20.transferFrom(_marketing.beneficiary(), msg.sender, _interest(msg.sender, block.timestamp));

        delete _savings[msg.sender];
        emit Withdraw(msg.sender, _amount(msg.sender) + _interest(msg.sender, block.timestamp));
    }

    /**
     * @dev Returns duration, interest rate, total allocation, current allocation, minimum deposit, maximum deposit amount of safe.
     */
    function safeStatus() external view returns (uint256[] memory) {
        uint256[] memory result = new uint256[](6);
        result[0] = _duration;
        result[1] = _interestRate;
    ...
    /**
     * @dev Returns start, end, amount, accumulated amount, closing amount of saving
     */
    ...
    function _interest(address addr, uint256 timestamp) private view returns (uint256) {
        ...
        if (timestamp - _start(addr) < _duration) {
            return _amount(addr) * _interestRate * (timestamp - _start(addr)) / _duration / 100;
        }
        return _amount(addr) * _interestRate / 100;
    }
