...
/*
    Original Dutch Auction contract written by mousedev.eth
    Source: https://github.com/mouse-dev-1/MDDA
*/

contract DutchAuction is Allowlist, Ownable {
    uint256 private constant ONE_WEEK = 604800;
    uint256 private constant ONE_DAY = 86400;
    uint256 private constant ALLOW_LIST_DISCOUNT = 777;
    ...
    uint256 public requestMax;
    uint256 public maxSupply;
    uint256 public allowlistSupply;

    uint256 public decrement;
    uint256 public decrementFrequency;

    uint256 private _allowlistClaims;

    bool public initialized;
    bool public soldOut;
    bool public finalized;

    // struct for storing successful payments
    struct Receipt {
        uint128 totalPaid;
    ...
    function dropState() external view returns (string memory) {
        if (soldOut == true) return "SOLDOUT";
        if (endTimestamp > 0 && block.timestamp > endTimestamp + ONE_DAY) return "PUBLIC";
        if (endTimestamp > 0 && block.timestamp < endTimestamp + ONE_DAY) return "ALLOWLIST";
        if (block.timestamp >= startTimestamp) return "AUCTION";
        return "COUNTDOWN";
    }
    ...
    function initializeAuction(
        ...
        decrementFrequency = _decrementFrequency;
    ...
    function allowlistPrice() public view returns (uint256) {
        if (endTimestamp == 0) revert AllowlistNotStarted();
        return _allowlistPrice();
    }

    function payments(address user) public view returns (Receipt[] memory) {
        return _receipts[user];
    }

    function refundOwed(address user) public view returns (uint256) {
        if (finalPrice == 0) revert NotEnded();
        uint256 totalRefund;

        for (uint256 i = _receipts[user].length; i > 0; ) {
            Receipt memory r = _receipts[user][i - 1];

            uint256 expectedPrice;
            uint256 refund;

            unchecked {
                // expected price to pay based on final price point
                expectedPrice = r.quantityMinted * finalPrice;
                // expected refund amount
                refund = r.totalPaid - expectedPrice;
                // add to total refund amount
                totalRefund = totalRefund + refund;

                // go to next in array of receipts
                i--;
            }
        }
        return totalRefund;
    ...
    function currentPrice() public view returns (uint256) {
        ...
        if (block.timestamp < startTimestamp) return startPrice;

        uint256 epochs;

        unchecked {
            // epochs since start
            epochs = (block.timestamp - startTimestamp) / decrementFrequency;

            // verify decrement amount is greater than end price
            if (epochs * decrement > startPrice - endPrice) return endPrice;

            // return the new price
            return startPrice - (epochs * decrement);
        }
    }
    ...
    function processRefund() public {
        ...
        for (uint256 i = _receipts[msg.sender].length; i > 0; ) {
            Receipt memory r = _receipts[msg.sender][i - 1];

            // remove the receipt being processed
        ...
        // final check to verify receipts for sender is empty
        if (_receipts[msg.sender].length > 0) revert RefundFailed();
    ...
    function withdrawFunds() public onlyOwner {
        if (!soldOut) revert NotEnded();
        if (finalized) revert FundsAlreadyWithdrawn();

        finalized = true;

        // expected payout based on minted supply + allow list supply * final price
        uint256 expectedPayout = (maxSupply + allowlistSupply - _allowlistClaims) * finalPrice;
        expectedPayout = expectedPayout + (_allowlistClaims * _allowlistPrice());

        (bool success, ) = payable(msg.sender).call{value: expectedPayout}("");
        if (!success) revert TransferFailed();
    }

    function safeWithdrawFunds() public onlyOwner {
        // require this is 1 week after auction start
        if (block.timestamp < startTimestamp + ONE_WEEK) revert NotLongEnough();

        uint256 finalFunds = address(this).balance;

        (bool success, ) = payable(msg.sender).call{value: finalFunds}("");
        if (!success) revert TransferFailed();
    }

    function _allowlistHook(bytes32[] calldata proof, uint256 _totalSupply) internal {
        if (endTimestamp == 0) revert AllowlistNotStarted();

        uint256 _price = _allowlistPrice();
        uint256 _allowedSupply;
        uint256 _newSupply;

        unchecked {
            _allowedSupply = maxSupply + allowlistSupply;
            _newSupply = _totalSupply + 1;
            _allowlistClaims = _allowlistClaims + 1;
        }

        _verifyProofOrRevert(msg.sender, proof);

        if (_totalSupply == _allowedSupply) {
            revert MaxSupplyReached(_allowedSupply);
        }

        if (_newSupply > _allowedSupply) {
            revert ExceedsMaxSupply(_allowedSupply);
        }

        if (msg.value != _price) {
            revert InvalidPaymentAmount(_price, msg.value);
        }

        // set the sale to sold out
        if (_newSupply == _allowedSupply) soldOut = true;
    }

    function _publicSaleHook(uint128 _mintQuantity, uint256 _totalSupply) internal {
        if (endTimestamp == 0) revert AllowlistNotStarted();
        ...
        unchecked {
            _allowedSupply = maxSupply + allowlistSupply;
            _requiredEther = _mintQuantity * finalPrice;
            _newSupply = _totalSupply + 1;
            _startTime = endTimestamp + ONE_DAY;
        }

        if (block.timestamp < _startTime) {
            revert PublicSaleNotStarted(_startTime);
        }
        ...
        // do not add receipt, user is purchasing at final price
    }

    function _auctionHook(uint128 _mintQuantity, uint256 _totalSupply) internal {
        ...
        unchecked {
            _requiredEther = _mintQuantity * _currentPrice;
            _newSupply = _mintQuantity + _totalSupply;
        }

        if (block.timestamp < startTimestamp) revert NotStarted(startTimestamp);
        if (_totalSupply == maxSupply) revert MaxSupplyReached(maxSupply);
        if (_newSupply > maxSupply) revert ExceedsMaxSupply(maxSupply);
        if (_mintQuantity == 0 || _mintQuantity > requestMax) {
            revert ExceedsTransactionMaxOrMin(requestMax);
        }
        if (msg.value != _requiredEther) {
        ...
        // add the payment to receipts
        _receipts[msg.sender].push(Receipt(uint128(msg.value), _mintQuantity));
    }

    function _allowlistPrice() internal view returns (uint256) {
        unchecked {
            return finalPrice - ((finalPrice * ALLOW_LIST_DISCOUNT) / 10000);
        }
    }
}
...
contract Lunatics is
    ...
    DutchAuction
{
    constructor(address _recipient, uint256 _royalty, uint256 _initial) ERC721A("Lunatics", "L") {
    ...
    function dutchAuctionMint(uint8 _quantity) public payable {
        _auctionHook(_quantity, totalSupply());
    ...
    function allowlistMint(bytes32[] calldata proof) public payable {
        _allowlistHook(proof, totalSupply());
    ...
    function _beforeTokenTransfers(
        ...
        uint256 quantity
    ) internal virtual override(ERC721A, PassiveStaking) {
...
abstract contract PassiveStaking is ERC721A {
    ...
    function total(uint256 tokenId) public view virtual returns (uint256) {
        ...
        return earned(tokenId).add(spent(tokenId));
    }
    ...
    function earned(uint256 tokenId) public view virtual returns (uint256) {
        ...
        uint256 base = _blockTime().sub(ownershipStartTimestamp).mul(EARN_RATE);

        return base.sub(_spent[tokenId]);
    }
...
contract Allowlist {
    ...
    function _verifyProofOrRevert(address from, bytes32[] calldata proof) internal returns (bool) {
        if (_claimed[from]) revert AlreadyClaimed(from);
