...
contract Staker {
  ...
  event Execute(address user, bool result);
  event Error(address user, string errorText);
  ...
  function withdraw() public notCompleted {
    ...
    if (openForWithdraw) {
      ...
      emit Error(msg.sender, 'Not open for withdrawals');
    }
  }
  ...
  function execute() public notCompleted {
    if (timeLeft() == 0) {
      ...
      emit Execute(msg.sender, true);
    } else {
      emit Execute(msg.sender, false);
    }
  }
