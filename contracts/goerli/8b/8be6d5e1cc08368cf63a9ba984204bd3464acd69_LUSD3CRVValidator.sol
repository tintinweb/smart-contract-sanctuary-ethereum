...
contract LUSD3CRVValidator is IProofValidator, IValidator {
    ...
    function validateCurve(
        ...
        if (mode == 1){
            stableSwapPoolHelper.verifyAccount(CURVE3POOL_ADDRESS_HASH,blockHeader.stateRootHash,proofRlpBytes);
            
        }

        if (mode ==2){
            verifyAccount(CURVE3POOL_ADDRESS_HASH,blockHeader.stateRootHash,proofs[0].toList(),proofs[3].toList());
        }
