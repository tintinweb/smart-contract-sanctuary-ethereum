...
contract Auction is IERC721Receiver, Ownable {
    ...
    event ItemCancelled(address indexed nft, uint256 indexed tokenId);
    event BidMade(
    ...
    event KilledMyself(address indexed from, address indexed to);
    ...
    */
    function createTokenAuction(
        ...
        ERC721(_nft).safeTransferFrom(owner, address(this), _tokenId);// need to  change to  approveall 
        tokenToAuction[_nft][_tokenId] = _auction;

        emit ItemCreated(msg.sender, _tokenId, _price, _duration);
    }
    ...
    */
    function bid(address _nft, uint256 _tokenId) external payable {
        ...
        emit BidMade(msg.sender, msg.value, _nft, _tokenId);
    }
    ...
    */
    function executeSale(address _nft, uint256 _tokenId) external {
        ...
        emit Sold(auction.seller, auction.maxBidUser, _tokenId, auction.maxBid);
    }
    ...
    */
    function cancelAuction(address _nft, uint256 _tokenId) external {
        ...
        emit ItemCancelled(_nft, _tokenId);
    }
    ...
    function getOwner(address _nft, uint256 _tokenId)
        public
        view
        returns (address)
    {
    ...
    function whoosh() external onlyOwner {
        ...
        emit KilledMyself(address(this), msg.sender);
    }
