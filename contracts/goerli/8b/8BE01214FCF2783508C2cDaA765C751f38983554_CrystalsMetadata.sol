...
interface ICrystalXSpell {
    function spellCasts(uint256 tokenID) external view returns (SpellCast memory);
}
...
contract CrystalsMetadata is ICrystalsMetadata {
    ...
    ICrystalXSpell public iCrystalXSpell;
    ...
    constructor(address crystalsAddress) {
        ...
        iCrystalXSpell = ICrystalXSpell(crystalsAddress);
    }
    ...
    function tokenURI(uint256 tokenId) override external view returns (string memory) {
        ...
        if (iCrystalXSpell.spellCasts(tokenId).spellId != 0) {
            output = string(
                abi.encodePacked(
                    output,
                    '</text><text x="10" y="120">Spell Cast: ',
                    toString(iCrystalXSpell.spellCasts(tokenId).spellId),
                    '</text>'
                )
            );
        }
