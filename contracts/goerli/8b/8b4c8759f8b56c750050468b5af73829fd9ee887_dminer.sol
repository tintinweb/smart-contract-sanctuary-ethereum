...
contract dminer is Ownable, ReentrancyGuard {
    ...
    */
    ...
    /* Mapping of Round Id to Jackpot Claim */
    mapping(uint256=>JackpotClaim) public jackpotClaimed;
    ...
    function buyTickets( uint256 quantity, uint256 tokenIdHolding, uint256 miningLevel) external payable nonReentrant {
       ...
       // require(checkSign(signature, quantity, tokenIdHolding, miningLevel, _msgSender(), roundId)==signerWallet, "DIGI: Fake signature");
        ...
        for(uint256 i= 0;i<quantity; i++){
            round.ticketsSold += 1;
            TicketKey[roundId][round.ticketsSold] = Ticket(_msgSender(), getTicketKey(round.ticketsSold), miningLevel, false);
            emit TicketPurchased(round.ticketsSold, roundId, _msgSender());
        }
    }

    function getJackpotPrice(uint256 roundId) public view returns(uint256){
        LotteryRound memory previosRound = RoundDetails[roundId];
        uint256 jackpotPrice;
        if((previosRound.ticketsSold.mul(ticketPrice)).div(2) > previosRound.priceClaimed){
            jackpotPrice = (previosRound.ticketsSold.mul(ticketPrice)).sub(previosRound.priceClaimed);
        }
        return jackpotPrice;
    }

    function getJackpotWinner(uint256 roundId) public view returns(uint256){
        LotteryRound memory round = RoundDetails[roundId];
        return getWinningKey(currentRound()).div(round.ticketsSold.add(1));
    }
    ...
    function announceResult() public onlyOwner {
        uint256 roundId = currentRound();
        require(RoundDetails[roundId].endTime < block.timestamp, "DIGI: Lottery not ended");
    ...
    function getTicketResult(uint256 ticketId, uint256 roundId) public view returns(uint256){
    ...
    function getMultiplier(uint256 ticketId, uint256 roundId) public view returns(uint256){
    ...
    function getWinningOre(uint256 ticketId, uint256 roundId) public view returns(Ores Ore){
        uint256 TicketKey_ = TicketKey[roundId][ticketId].ticketKey;
        uint256 winningKey_ = getWinningKey(roundId);
        uint256 result_ = TicketKey_.mul(winningKey_);
        Ores previousOre;
        uint256 previousWinning;
        uint256 multipler = getMultiplier(ticketId,roundId);
        for(uint256 i = 1; i<=multipler; i++) {
        Ores ore;
        uint256 multiplierResult= (result_.mul(i)) % 1000 ;
           ore = getOre(multiplierResult);
           uint256 winning = getPrice(ore);
            if (previousWinning < winning) {
                previousOre = ore;
            }
        }
       return previousOre;
    ...
    function claimWinning(uint256 ticketId) public nonReentrant {
        ...
        Ores winningOre = getWinningOre(ticketId, roundId);
        uint256 winning = getPrice(winningOre);
        round.priceClaimed += winning;
        TransferHelper.safeTransferETH(msg.sender, winning);
        if(ticketId == getJackpotWinner(roundId)){
            claimJackpot();
        }
        emit WinningClaimed(ticketId, roundId, _msgSender(), winning);
    }


    function claimJackpot() internal {
        uint256 roundId = currentRound();
        uint256 winner = getJackpotWinner(roundId);
        Ticket memory ticket = TicketKey[roundId][winner];
        require(ticket.holder == _msgSender(), "DIGI: You are not winner");
        JackpotClaim storage jackpot = jackpotClaimed[roundId];
        require(jackpot.claimed == false, "DIGI: Already claimed");
        jackpot.claimed = true;
        jackpot.wallet = _msgSender();
        uint256 _winning = getJackpotPrice(roundId);
        if(_winning > 0){
            payable(_msgSender()).transfer(_winning);
        }
        emit JackpotClaimed(winner, roundId, _msgSender(), _winning);
    }
