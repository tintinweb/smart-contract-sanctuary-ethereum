...
contract RFOXNFTPresale is RFOXNFTSale, BaseRFOXNFTPresale
{
     ...
     */
    function buyNFTsPresale(uint256 tokensNumber, uint256 maxUserLimitPresale, bytes32[] calldata proof)
        external
        payable
        whenNotPaused
        callerIsUser
        authorizePresale(maxUserLimitPresale, proof)
    {
        _buyNFTsPresale(tokensNumber, maxUserLimitPresale);
    }
}
...
contract BaseRFOXNFTPresale is BaseRFOXNFT
{
     ...
     */
    modifier authorizePresale(uint256 maxUserLimitPresale, bytes32[] calldata proof) {
        if (isWhitelistActivated) {
            require(
                (checkWhitelisted(msg.sender, maxUserLimitPresale, proof) && block.timestamp >= saleStartTime),
    ...
    /// @param tokensNumber How many NFTs for buying this round
    function _buyNFTsPresale(uint256 tokensNumber, uint256 maxUserLimitPresale) internal tokenInSupply(tokensNumber) {
        require(totalPresaleMintedPerAddress[msg.sender].add(tokensNumber) <= maxUserLimitPresale, "Exceed the limit");
     ...
     */
    function _leaf(address account, uint256 maxUserLimitPresale) internal pure returns (bytes32) {
        return keccak256(abi.encodePacked(account, maxUserLimitPresale));
    }
     ...
     * @return true / false.
     */
    function checkWhitelisted(address account, uint256 maxUserLimitPresale, bytes32[] memory proof)
        public
        view
        returns (bool)
    {
        bytes32 leaf = _leaf(account, maxUserLimitPresale);
