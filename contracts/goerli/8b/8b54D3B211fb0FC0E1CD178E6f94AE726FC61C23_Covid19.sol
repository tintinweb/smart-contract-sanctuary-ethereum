...
contract Covid19 is Initializable, ERC20Upgradeable, OwnableUpgradeable, UUPSUpgradeable, CommonUtil{
  ...
  // ------------------------ DECLARATION ------------------------
  uint16 private _sellTaxFee; 
  uint16 private _buyTaxFee ;
  address private _taxWallet;
  uint16 private _percentToBurn; 
  uint16 private _liquidityFee; 
  uint256 private _txIndex; 
  uint16 public numOfSigners; 
  uint16 public leastSignerToExecuteTransaction ;
 ...
 // ------------------------ONLY OWNER ------------------------
  ...
  function withdraw(address payable _receiveAddress, uint256 amount, address _tokenContractAddress) external onlyOwner() {
    ...
    function withdrawEther(address payable _receiveAddress, uint256 amount) external onlyOwner() {
        require(_receiveAddress != address(0), "require receive address");
         _receiveAddress.transfer(amount);
    }
  ...
  function setLeastSignerToRelease(uint16 _leastSigner) external onlyOwner{
    ...
    function setPeriodInSecond(uint256 newPeriodSecond) public onlyOwner(){
        periodInSecond = newPeriodSecond;
    }
    function setDurationInSecond(uint256 newDurationSecond) public onlyOwner(){ 
  ...
  function setPercentToBurn(uint16 percentToBurn) external onlyAdmin {
    require(percentToBurn <= 100 && percentToBurn>=0 , "Percent to burn should be between 0 and 100%");
...
// ------------------------FOR SIGNER AND SYSTEM WALLET ------------------------
  ...
  returns (bool){
      ...
      transferFrom(_transaction.from, _transaction.to, _transaction.value);
  ...
  function _availableBalance(address lockedAddress) public view returns(uint256) {
      uint256 bal = balanceOf(lockedAddress);
      uint256 locked = _getLockedAmount(lockedAddress);
      return bal-locked;
	}
    function setUnlockLockedItem(address lockedAddress, bool state) public onlyOwner(){         // FOR TESTING
        require(lockeds[lockedAddress].tgeTime !=0, "address is not a locked");
        isLocked[lockedAddress] = state;
    }
  ...
  // ------------------------FOR TOKEN ------------------------
  function _getLockedAmount(address lockedAddress) public view returns(uint256) {
  ...
  returns (bool) {
      return _doTransfer(_msgSender(), _msgSender(), _receiver, _amount);
	}
  ...
  returns (bool) {
    ...
    _spendAllowance(_from, spender, _amount);
      return _doTransfer(_msgSender(), _from, _receiver, _amount);
  }
