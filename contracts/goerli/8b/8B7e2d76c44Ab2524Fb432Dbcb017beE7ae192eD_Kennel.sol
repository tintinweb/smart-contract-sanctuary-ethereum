...
contract Kennel is Ownable, ReentrancyGuard {
    ...
    // withdrawable values
    uint256 public remainingETHFees = 0;
    ...
    function lockTokens(
        ...
        require(!_feeInETH || msg.value > ETHFee, 'ETH fee not provided');
        ...
        if (_feeInETH) {
            ...
            remainingETHFees = remainingETHFees.add(msg.value);
        } else {
    ...
    function withdrawFees(address payable withdrawalAddress) external onlyOwner {
        if (remainingETHFees > 0) {
            withdrawalAddress.transfer(remainingETHFees);
            remainingETHFees = 0;
        }
