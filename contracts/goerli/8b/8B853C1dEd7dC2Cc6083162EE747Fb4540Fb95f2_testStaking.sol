...
contract testStaking is Ownable, ReentrancyGuard {
    ...
    // The block number when test reward ends.
    uint256 public bonusEndBlock;

    // The block number when test reward starts.
    ...
    function pendingReward(address _user) external view returns (uint256) {
        ...
        if (block.number > lastRewardBlock && tokenStaked != 0) {
            ...
            uint256 testReward = multiplier.mul(rewardPerBlock);
            uint256 adjustedTokenPerShare =
                accTokenPerShare.add(testReward.mul(PRECISION_FACTOR).div(tokenStaked));
    ...
    function _updatePool() internal {
        ...
        uint256 multiplier = _getMultiplier(lastRewardBlock, block.number);
        uint256 testReward = multiplier.mul(rewardPerBlock);
        accTokenPerShare = accTokenPerShare.add(testReward.mul(PRECISION_FACTOR).div(tokenStaked));
        lastRewardBlock = block.number;
    }
