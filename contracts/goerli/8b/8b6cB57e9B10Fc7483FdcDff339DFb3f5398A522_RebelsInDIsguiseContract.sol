...
// File: contracts/rebels.sol


pragma solidity ^0.8.7;

/**
 * @title RebelsInDIsguiseContract contract
 * @dev Extends ERC721 Non-Fungible Token Standard basic implementation
 */
contract RebelsInDIsguiseContract is Ownable, ERC721 {
    using SafeMath for uint256;

  uint public supplyLimit = 5555;
  uint256 public totalSupply;
  string public baseURI;
  uint256 public tokenPrice = 55000000000000000;
...
//Presale config
  uint public maxPerTransactionOnPresale = 2;
  ...
  function publicMintRebels(uint _count) public payable{
   ...
   require((totalSupply + _count) <= supplyLimit, "Supply has been reached");
  ...
  function whitelistMintRebels(bytes32[] calldata _merkleProof, uint256 _count) public payable {
    bytes32 leaf = keccak256(abi.encodePacked(msg.sender));

    require(presaleActive == true, "Sale is paused.");
    require(_count <= maxPerTransactionOnPresale, "Invalid token count");
    require((totalSupply + _count) <= supplyLimit, "Supply has been reached");
    require(tokenPrice.mul(_count) <= msg.value,"Ether value sent is not correct");
    require(!presaleParticipants[msg.sender], 'Whitelist already used!');
