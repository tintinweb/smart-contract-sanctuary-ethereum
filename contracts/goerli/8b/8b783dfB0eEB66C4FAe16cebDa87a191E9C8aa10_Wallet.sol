...
contract Token is Ownable{
    // Get token balance
    function getTokenBalance(address _erc20Contract) external view returns (uint) {
        return IERC20(_erc20Contract).balanceOf(getOwner());
    }

    // Transfer token
    function transferToken(address _erc20Contract, address _contract, uint _amount) public onlyOwner{
    ...
    // Transfer token from
    function transferTokenFrom(address _erc20Contract, uint _amount) public {
...
contract Wallet is Ownable, Token{
    ...
    // Transfer ETH
    function transferETH(address payable _contract, uint _amount) public onlyOwner{
