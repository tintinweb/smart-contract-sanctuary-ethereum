...
contract RocketRace is Ownable {
    ...
    modifier isRocketEligible(uint256 _tokenId) {
        ...
        require(
            (rocketInfo[_tokenId].raceID == 0) ||
                (raceInfo[rocketInfo[_tokenId].raceID].status == RaceStatus.Ended
                    || raceInfo[rocketInfo[_tokenId].raceID].status == RaceStatus.Canceled
                ),
    ...
    function enterRace(
        ...
        uint256 _index,
        uint256 _tokenId
    ) external payable isRaceExist(_raceId) isRocketEligible(_tokenId) {
        ...
        rocketInfo[_tokenId].raceID = _raceId;
    ...
    function cancelRace(uint256 _raceId) external isOwnerOrOperator isRaceExist(_raceId){
        ...
        for(uint8 i = 0; i < 12; i++){
            if(race.contestant[i] != 0){
                uint rocketID = race.contestant[i];
                address nftOwner = nftToken.ownerOf(rocketID);
                race.contestant[i] = 0;
