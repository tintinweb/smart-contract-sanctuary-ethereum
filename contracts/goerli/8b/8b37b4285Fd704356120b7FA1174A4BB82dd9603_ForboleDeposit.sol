...
// Deposit contract implementation

contract ForboleDeposit is Pausable, Ownable {
    using SafeMath for uint256;

    address depositContract;
    uint256 private _fee;
    mapping(address => bytes[]) public linkedValidators;
    ...
    function batchDeposit(
        ...
        require(
            withdrawal_credentials.length == count * CREDENTIALS_LENGTH,
        ...
        //bytes[] memory validatorsPublicKeys = new bytes[](count);
        for (uint256 i = 0; i < count; ++i) {
            bytes memory pubkey = bytes(
                pubkeys[i * PUBKEY_LENGTH:(i + 1) * PUBKEY_LENGTH]
            );
            //validatorsPublicKeys[i] = pubkey;
            linkedValidators[msg.sender].push(pubkey);
        ...
        // bytes[] storage validators = linkedValidators[msg.sender];
        // for (uint256 i = 0; i < validatorsPublicKeys.length; i++) {
        //     validators.push(validatorsPublicKeys[i]);
        // }
        // linkedValidators[msg.sender]= validatorsPublicKeys;
        emit Deposit(msg.sender, msg.value, linkedValidators[msg.sender]);
    }

    /**
     * @dev get Linked Validators
     * @param user The address of the user
     */
    function getLinkedValidators(address user)
        public
        view
        returns (bytes[] memory)
    {
        return linkedValidators[user];
    }
