...
interface InterfaceTheMums {
    function mumBalance(address owner) external view returns(uint256);
}

contract Queen is ERC20, Ownable, Pausable {

    InterfaceTheMums public TheMums;
    ...
    modifier onlyFromTheMums() {
        require(msg.sender == address(TheMums));
        _;
    } 

    constructor(address theMumsAddress) ERC20("QUEEN", "Queen") {

        // Set who the evofoxes address.
        TheMums = InterfaceTheMums(theMumsAddress);
    ...
    function setMumContractAddress(address _theMums) public onlyOwner {
        TheMums = InterfaceTheMums(_theMums);
    }

     // Set the address for the contract.
    function setTicketContractAddress(address _theMums) public onlyOwner {
        ticketContract = _theMums;
    }
    ...
    function transferTokens(address _from, address _to) onlyFromTheMums whenNotPaused external {
    ...
    function getPendingReward(address user) internal view returns(uint256) {
        return TheMums.mumBalance(user) *
               rate *
