...
import { ISporkProxy } from './interfaces/ISporkProxy.sol';
...
contract ERC1155AssetGovernedProxy is NonReentrant, IGovernedContract, IGovernedProxy {
    ...
    ISporkProxy public spork_proxy;
    ...
    modifier senderOrigin() {
        ...
        require(
            ...
            'ERC1155AssetGovernedProxy: Only direct calls are allowed!'
        );
        _;
    }
    ...
    modifier onlyImplementation() {
        require(
            ...
            'ERC1155AssetGovernedProxy: Only calls from implementation are allowed!'
        );
        _;
    }
    ...
    function emitURI(string calldata _uri, uint256 _id) external onlyImplementation {
        emit URI(_uri, _id);
    }
    ...
    function owner() external view returns (address) {
        return IERC1155Asset(address(uint160(address(implementation)))).owner();
    }
    ...
    function uri(uint256 _id) external view returns (string memory) {
        return IERC1155Asset(address(uint160(address(implementation)))).uri(_id);
    }
    ...
    function setSporkProxy(address payable _sporkProxy) external onlyImplementation {
        spork_proxy = ISporkProxy(_sporkProxy);
    }
    ...
    {
        require(_newImplementation != implementation, 'ERC1155AssetGovernedProxy: Already active!');
        require(
            _newImplementation.proxy() == address(this),
            'ERC1155AssetGovernedProxy: Wrong proxy!'
        );
    ...
    function upgrade(IUpgradeProposal _proposal) external noReentry {
        ...
        require(newImplementation != implementation, 'ERC1155AssetGovernedProxy: Already active!');
        // in case it changes in the flight
        require(
            address(newImplementation) != address(0),
            'ERC1155AssetGovernedProxy: Not registered!'
        );
        require(_proposal.isAccepted(), 'ERC1155AssetGovernedProxy: Not accepted!');
    ...
    function safeTransferETH(address to, uint256 amount) external noReentry onlyImplementation {
        ...
        require(
            ...
            'ERC1155AssetGovernedProxy: failed to transfer ETH'
        );
    }
    ...
    function migrate(IGovernedContract) external {
        revert('ERC1155AssetGovernedProxy: Good try');
    }
