...
contract EarlyAccessOsPass is ERC721A, Ownable, DefaultOperatorFilterer {
    ...
    mintCompliance(tokenQuantity,_type)
    {
    ...
    external 
    payable
    mintCompliance(tokenQuantity,1) 
    OnlyIfHumanAndAuthorised
    {
    ...
    modifier mintCompliance(uint256 _mintAmount, uint128 _wlType)
    {
        require((balanceOf(msg.sender)+_mintAmount) <= (_wlType == 0 ? 1 : MINT_PER_WALLET), "Max Mint per wallet reached"); 
