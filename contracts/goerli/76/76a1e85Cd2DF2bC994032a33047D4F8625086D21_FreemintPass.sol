...
contract OldFreemintPass is ERC721A, Ownable {
...
contract FreemintPass is ERC721A, Ownable {
    ...
    OldFreemintPass OldContract = OldFreemintPass(0xca0Cd1f948C3829E9095C3a728267036F6786927);
    
    mapping(address => uint8) private _mintedCount;
    mapping(address => uint8) private _whitelistMintedCount;
    ...
    function redeem(bytes32[] calldata merkleProof, uint256 benefitTokenId, uint8 count) external payable {
        ...
        if (saleStatus == SaleStatus.HOLDERS_ONLY) {
            require(OldContract.ownerOf(benefitTokenId) == msg.sender, "You must own the token");
