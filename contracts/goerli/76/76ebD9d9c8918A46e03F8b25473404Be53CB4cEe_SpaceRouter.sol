...
contract SpaceRouter {
    ...
    error NotEnoughSpc();
    error SpcSwappedLowerThanMin();
    error EthSwappedLowerThanMin();
    error CannotProvideEth();
    error CannotProvideSpc();
    error CannotSendEth();
    error CannotSendSpc();
    ...
    function addLiquidity(uint256 spc) external payable {
        (uint256 _ethReserve, uint256 _spaceCoinReserve) = spaceLP
            .getReserves();

        uint256 requiredSpc = spc;
        if (_ethReserve != 0 && _spaceCoinReserve != 0) {
            requiredSpc = (msg.value * _spaceCoinReserve) / _ethReserve;

            if (spaceCoin.hasTax()) {
                requiredSpc = requiredSpc * 100 / (100 - spaceCoin.taxRate());
            }
        ...
        if (!success) {
            revert CannotProvideEth();
        }
        success = spaceCoin.transferFrom(
            msg.sender,
            address(spaceLP),
            requiredSpc
        );
        if (!success) {
            revert CannotProvideSpc();
        }
    ...
    function swapETHForSPC(uint256 spcOutMin) external payable {
        ...
        uint256 spcAmountOut = spaceLP.swap(msg.sender);

        if (spcOutMin > spcAmountOut) {
            revert SpcSwappedLowerThanMin();
        }
    }
    ...
    function swapSPCForETH(uint256 spcIn, uint256 ethOutMin) external {
        ...
        if (!success) {
            revert CannotSendSpc();
        }

        uint256 ethAmountOut = spaceLP.swap(msg.sender);

        if (ethOutMin > ethAmountOut) {
            revert EthSwappedLowerThanMin();
        }
    }
}
