...
contract GainERC20Tokenv3test012 is ERC20Upgradeable, OwnableUpgradeable {
    using SafeMathUpgradeable for uint256;
    using SafeMathInt for int256;
    ...
    /* MrKawdy renames Gain_Usd to GAIN_Dai for clarification purposes */ 
    ...
    * TargetRatio: allows for GAIN to have the rebase target a backing of greater than 100%.
    * Stretch: allows for softening of each rebase (a stretch of 10 = total supply +/- 1/10th the amount it would with a stretch = 1)
    * DaiAddress: contract address of DAI.
    * Whitelist/Whitelisted: allows for whitelisting of addresses or contracts to execute the rebase for automation purposes  
    *
    ...
    */
    ...
    return totalReserves().div(_totalSupply);
    }
    ...
    function initialize() public initializer {
        __ERC20_init("Gainv3t012","GAINv3t012");
        __Ownable_init();
    ...
    require(
        ...
        if (CurrentRatio() > TargetRatio) {
            uint256 percentage = ((CurrentRatio() - TargetRatio) * 100000000000000000000);
            ...
            _supplyDelta = int256((_data / Stretch) / 100000000000000000000);
            if (_supplyDelta != 0) {
                _totalSupply = _totalSupply.add(uint256(_supplyDelta));
            }
        }
        if (CurrentRatio() < TargetRatio) {
            uint256 percentage = ((TargetRatio - CurrentRatio()) * 100000000000000000000);
