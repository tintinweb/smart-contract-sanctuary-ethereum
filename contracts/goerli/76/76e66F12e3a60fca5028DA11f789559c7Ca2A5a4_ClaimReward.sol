...
contract ClaimReward is Pausable, Ownable {
    ...
    event ItemRewarded(address recipient, address tokenAddress, uint256 tokenId, uint256 lootboxId);
    ...
    function openLootBox(
        ...
        address nftAddress = request.nftAddress;
        uint256 tokenId = request.tokenId;

        IERC1155Burnable(lootBoxAddress).burn(msg.sender, lootboxId, 1);
        // IScroll(nftAddress).mint(msg.sender, 1, 1);
        emit ItemRewarded(msg.sender, nftAddress, tokenId, lootboxId);
    }
    ...
    function setLootboxAddress(address _lootboxAddress) external onlyOwner {
