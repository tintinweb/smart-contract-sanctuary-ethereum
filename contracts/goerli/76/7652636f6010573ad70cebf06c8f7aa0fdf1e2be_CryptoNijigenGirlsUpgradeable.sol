...
contract CryptoNijigenGirlsUpgradeable is IChildToken, Initializable, ERC721Upgradeable, OwnableUpgradeable {
    ...
    function setTokenPrice(uint256 start, uint256 end, uint256 price) public virtual onlyOwner{
    ...
    function mint(address user, uint256 _tokenId) external {
        ...
        require(_tokenId < 17778, "CNGE010");
    ...
    function withdraw(uint256 tokenId) external {
        ...
        if(index < _tokenIds.length){
            uint256 lastTokenIndex = _tokenIds.length - 1;
            uint256 lastTokenId = _tokenIds[lastTokenIndex];
            _tokenIds[index] = lastTokenId;
            _tokenIds[lastTokenIndex] = tokenId;
            delete _tokenIds[lastTokenIndex];
            _tokenIds.pop();
        }
        _burn(tokenId);
    }
