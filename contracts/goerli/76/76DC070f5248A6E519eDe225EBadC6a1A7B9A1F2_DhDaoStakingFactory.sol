...
interface IDHTOKEN {
    function setUp(string memory name_, string memory symbol_) external;
...
@notice Vote on grants powered by conviction voting off-chain by staking your dao staking token.
*/
contract DhDaoStaking is Initializable {
    event VoteCasted(
        uint56 voteId,
    ...
    /**
    @dev initializer.
    @param tokenAddress dao staking token address.
    */
    function setUp(address tokenAddress)
    public
    initializer
    {
        sToken = IERC20(tokenAddress);
    }
    ...
    */
    function releaseTokens(uint256[] calldata _voteIds) external {
        for (uint256 i = 0; i < _voteIds.length; i++) {
            ...
            sToken.transfer(msg.sender, votes[_voteIds[i]].amount);
...
contract DhDaoStakingFactory is CloneFactory {

    address immutable public _template;
    address immutable public _tokenTemplate;

    event SummonDaoStake(address indexed staker, address indexed token);

    constructor(address template, address tokenTemplate) {
        _template = template;
        _tokenTemplate = tokenTemplate;
    }

    function summonDaoStaker(address[] memory members, string memory name, string memory symbol) external returns (address) {
        DhDaoStaking staker = DhDaoStaking(createClone(_template));
        IDHTOKEN token = IDHTOKEN(createClone(_tokenTemplate));

        token.setUp(name, symbol);
        
        for (uint256 i = 0; i < members.length; i++) {
            token.mint(members[i], 100);
        }

        // token.transferOwnership(address(0));

        staker.setUp(address(token));

        emit SummonDaoStake(address(staker), address(token));
