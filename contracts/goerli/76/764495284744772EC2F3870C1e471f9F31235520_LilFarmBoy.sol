...
contract LilFarmBoy is 
    ...
    * Modifier
    */
   modifier mintMod(uint256 _qty, bytes32[] memory _proof) {
      require(lilFarmBoy.sale, 'LFB :: Minting is close!');
      require((totalSupply() + _qty) <= lilFarmBoy.maxSupply, 'LFB :: Not enough supply!');
      if(mintPhaseData[currMintPhase].maxWallet > 0){
         require(userData[msg.sender].mintCount[currMintPhase] <= mintPhaseData[currMintPhase].maxWallet, 'LFB :: Wallet minted out!');
      }
      if(mintPhaseData[currMintPhase].price > 0){
         require(msg.value >= (mintPhaseData[currMintPhase].price * _qty), 'LFB :: Not enough payment!');
      }
      if(currMintPhase == Phase.WHITELIST){
         bytes32 sender = keccak256(abi.encodePacked(msg.sender));
         require(MerkleProofUpgradeable.verify(_proof, mintPhaseData[currMintPhase].merkleRoot, sender), "LFB :: You're not in the list!");
      }
      _;
   }

   modifier mintOgMod(bytes32[] memory _proof) {
      bytes32 sender = keccak256(abi.encodePacked(msg.sender));
      require(MerkleProofUpgradeable.verify(_proof, mintPhaseData[currMintPhase].merkleRoot, sender), "LFB :: You're not in the list!");

      if(currMintPhase == Phase.FARMER){
         require(!userData[msg.sender].farmerPhaseMinted, "LFB :: Already minted!");
   ...
   modifier onlyApprovedAddress() {
      require(approvedAddress[msg.sender], 'LFB :: Address is not approved');
      _;
   }

   modifier teamMint() {
      require(allowedTeamAddress[msg.sender], 'LFB :: You are not included in team');
      _;
   }
   ...
   function mint(uint256 _qty, bytes32[] memory _proof) external payable mintMod(_qty, _proof) returns(string[] memory) {
      IProofOfFunds pof = IProofOfFunds(lilFarmBoy.treasury);
      pof.depositFund(IProofOfFunds.Tokens.ETH, msg.value);
      _mint(msg.sender, _qty);
      emit mintLilFarmBoy(msg.sender, _qty);
      return viewMintedNFTs(_qty);
   }

   function mintOG(bytes32[] memory _proof) external mintOgMod(_proof) returns(string[] memory) {
      uint256 qty;
      if(currMintPhase == Phase.FARMER){
         qty = userData[msg.sender].farmerPhaseAllocation;
   ...
   function tokenURI(uint256 _tokenID) public view virtual override(ERC721AUpgradeable, IERC721AUpgradeable) returns (string memory) {
      ...
      if(!lilFarmBoy.reveal){
         return lilFarmBoy.placeholderUri;
      }
      //string memory baseURI = _baseURI();
      return bytes(lilFarmBoy.baseUri).length > 0 ? string(abi.encodePacked(lilFarmBoy.baseUri, trueId.toString(), ".json")) : "";
   }

   function batchTokenURI(uint256[] memory _tokenIDs) external view returns(string[] memory){
   ...
   function viewUserMintCount(address _user, Phase _phase) external view returns(uint256) {
   ...
   function setCurrentMintPhase(Phase _phase) external onlyApprovedAddress {
      currMintPhase = _phase;
   }

   function setMintPhaseData(Phase _phase, uint256 _price, uint256 _maxSupply, uint256 _maxWallet) external onlyOwner {
   ...
   function setReveal() external onlyOwner {
      lilFarmBoy.reveal = !lilFarmBoy.reveal; 
   }
   ...
   function setAllocation(address _user, uint256 _allocation) external onlyOwner {
   ...
   function setPlaceholderUri(string memory _placeholderUri) external onlyOwner {
      lilFarmBoy.placeholderUri = _placeholderUri;
   }

   function setTeamAddress(address _team, bool _isAllowed) external onlyOwner {
      allowedTeamAddress[_team] = _isAllowed;
   }
...
interface IProofOfFunds {
  ...
  function depositFund(Tokens _token, uint256 _amount) external;
}
