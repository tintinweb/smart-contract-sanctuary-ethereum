...
import "./interfaces/IFundUVault.sol";
...
contract FundU is ReentrancyGuard, FundUtils {
    ...
    IFundUVault private immutable vault;
    ...
    function newStream(
        ...
        vault.depositOnVault(amountToDeposit, tokenAddress, beneficiary, msg.sender);
    ...
    function _withdrawPauseAndResume(uint256 _id, address _who) private {
        ...
        if (_balance > 0) {
            vault.withdrawFromVault(_balance, stream.tokenAddress, stream.beneficiary, _who);
...
interface IFundUVault {
    ...
    event VaultWithdraw(uint256 amount, address indexed tokenAddress, address indexed user);

    function depositOnVault(
        uint256 amount,
        address tokenAddress,
        address beneficiary,
        address owner
