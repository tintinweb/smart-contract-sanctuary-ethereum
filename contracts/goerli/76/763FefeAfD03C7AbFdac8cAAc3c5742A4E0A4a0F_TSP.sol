...
contract TSP is TinyERC721, TokenSale {
    uint256 public constant MAX_SUPPLY = 100;

    // third constructor argument is maximum batch size, 0 for no limit
    constructor() TinyERC721("Training Shelter Pass", "TSP", 0) {
      console.log("constructor contract deployed");
    }

    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        require(_exists(tokenId), "Token does not exist");
        return "https://www.jsonkeeper.com/b/EFPA";
    }

    function _mintTokens(address to, uint256 quantity) internal virtual override {
        _mint(to, quantity);
        console.log("%s NFTs have been minted to %s", quantity, to);
        console.log("Mint count is %s", totalSupply());
    }
