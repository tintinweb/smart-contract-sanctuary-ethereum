...
import "./IRift.sol";
import "./IHXP.sol";
import "./helpers/common/ActionsSet.sol";
import "./helpers/ERC5050/IERC5050.sol";
    ...
    IERC5050Receiver
{
    ...
    // SPELLS 
    mapping(uint256 => SpellCast) public spellCasts;
    address spellsContract;
    ...
    ITransferableMana iTransferableMana; // remove for mainnet
    bool public isHyperEventOn;
    address public hyperChest;  // remove for mainnet
    address payable public hyperChestA;
        ...
        nonReentrant
    {
        ...
        if (isHyperEventOn) {
            payable(hyperChestA).transfer(msg.value - (msg.value / 4));
        }
    }
    ...
    function claimableMana(uint256 crystalId) public view returns (uint32) {
        ...
        if (spellCasts[crystalId].spellId != 0) {
            // crystal has a spell cast on it
            manaToProduce = manaToProduce / 2;
        }
    ...
    function _castWithCharacter(uint256 spellId, uint256 crystalId, address collection, uint256 characterId)
        internal 
        canCast(crystalId, collection, characterId)
    {
        iMana.burn(tx.origin, 50 * crystalsMap[crystalId].focus * crystalsMap[crystalId].attunement);
        hyperXP.useAP(collection, characterId, 1, address(this));
        spellCasts[crystalId] = SpellCast({
            from: tx.origin,
            spellId: uint64(spellId),
            time: uint64(block.timestamp)
        });
        hyperXP.awardXP(collection, characterId, 50);
    }

    function clearSpell(uint256 crystalId, address collection, uint256 characterId)
        public
        nonReentrant
        ownsCrystal(crystalId) {
        require(spellCasts[crystalId].spellId != 0, "No spell");
        // this contract can't transfer Mana, so we have to burn and mint instead
        iMana.burn(tx.origin, 20 * crystalsMap[crystalId].focus * crystalsMap[crystalId].attunement);
        iMana.ccMintTo(spellCasts[crystalId].from, 20 * crystalsMap[crystalId].focus * crystalsMap[crystalId].attunement);
        spellCasts[crystalId].spellId = 0;
        spellCasts[crystalId].time = uint64(block.timestamp);
        hyperXP.awardXP(collection, characterId, 50);
    }

    function onActionReceived(Action calldata action, uint256)
        external
        payable
        override
    {
        require(msg.sender == spellsContract, "sender not spells");
        require(action.selector == _CAST_SELECTOR, "not cast");
        require(_exists(action.to._tokenId), "Crystal doesn't exist");
       
        (address collection, uint256 characterId) = abi.decode(action.data, (address, uint256));   
        _castWithCharacter(action.from._tokenId, action.to._tokenId, collection, characterId);
    }
    ...
    function ownerSetHyperEvent(bool _isOn, address payable _hyperChest) external onlyOwner {
        isHyperEventOn = _isOn;
        hyperChestA = _hyperChest;
    }

    function ownerRegisterCast() external onlyOwner {
        _receivableActions.add("cast");
    }
    ...
    function ownerSetHXPSpells(address hxp, address spells) external onlyOwner {
        hyperXP = IHXP(hxp);
        spellsContract = spells;
    }
    ...
    function ownerSetPaused(bool _paused) external onlyOwner {
        if (_paused) _pause();
        else _unpause();
    }
    ...
    modifier canCast(uint256 crystalId, address collection, uint256 characterId) {
        require(spellCasts[crystalId].spellId == 0, "Crystal already has a spell");
        require(crystalsMap[crystalId].focus == 1 || crystalsMap[crystalId].focus < hyperXP.getLevel(collection, characterId), "Character level too low");
        require(diffDays(spellCasts[crystalId].time, block.timestamp) >= 1, "Crystal recently dispelled");
        _;
    }
}

interface ITransferableMana {} // remove for mainnet
...
interface IMana {
    ...
    function transfer(address recipient, uint256 amount) external; // remove for mainnet
}
