...
import {IFixedInterestOnlyLoans, FixedInterestOnlyLoanStatus} from "IFixedInterestOnlyLoans.sol";
import {ITrancheVault} from "ITrancheVault.sol";
import {IStructuredPortfolio, Status, TrancheData, TrancheInitData, PortfolioParams, TranchesCheckpoint} from "IStructuredPortfolio.sol";
import {LoansManager, AddLoanParams} from "LoansManager.sol";

contract StructuredPortfolio is IStructuredPortfolio, LoansManager {
    using SafeERC20 for IERC20WithDecimals;

    /// @inheritdoc IStructuredPortfolio
    bytes32 public constant TRANCHE_STRATEGY_OWNER_ROLE = keccak256("TRANCHE_STRATEGY_OWNER_ROLE");
    ...
    TranchesCheckpoint public tranchesCheckpoint;

    event PortfolioInitialized(ITrancheVault[] tranches);
    event PortfolioStatusChanged(Status newStatus);
    event CheckpointUpdated(uint256[] values, uint256 timestamp);
    ...
    function initialize(
        ...
        _grantRole(TRANCHE_STRATEGY_OWNER_ROLE, manager);

        require(portfolioParams.duration > 0, "StructuredPortfolio: Duration cannot be zero");
        ...
        tranchesCheckpoint.totalAssets = new uint256[](tranchesCount);
    ...
    function getTranchesCheckpoint() external view returns (TranchesCheckpoint memory) {
        return tranchesCheckpoint;
    }
    ...
    function getMaxValueOnCloseForTranche(uint256 trancheIdx) external view returns (uint256) {
        return tranchesData[trancheIdx].maxValueOnClose;
    }

    function updateCheckpoint(uint256 trancheIdx, int256 delta) external {
        require(msg.sender == address(tranches[trancheIdx]), "StructuredPortfolio: Caller must be a tranche");

        TranchesCheckpoint memory _checkpoint = TranchesCheckpoint({
            totalAssets: calculateWaterfall(),
            timestamp: _min(block.timestamp, endDate)
        });

        if (delta > 0) {
            _checkpoint.totalAssets[trancheIdx] += uint256(delta);
        } else {
            _checkpoint.totalAssets[trancheIdx] -= uint256(-delta);
        }

        tranchesCheckpoint = _checkpoint;

        emit CheckpointUpdated(_checkpoint.totalAssets, _checkpoint.timestamp);
    }
    ...
    function start() external {
        ...
        _checkTranchesMinSizes();
        _changePortfolioStatus(Status.Live);

        startDate = block.timestamp;
        endDate = block.timestamp + portfolioDuration;
        tranchesCheckpoint.timestamp = block.timestamp;

        for (uint256 i = 0; i < tranchesCount; i++) {
            tranchesCheckpoint.totalAssets[i] = tranches[i].transferToPortfolio();
        }
    }
    ...
    function _checkTranchesRatios() internal view {
        ...
        for (uint256 i = 1; i < tranchesCount; i++) {
            ...
            bool isMinRatioRequired = trancheData.minSubordinateRatio != 0;
            if (isMinRatioRequired) {
                bool isMinRatioSatisfied = _isMinRatioSatisfied(trancheValue, subordinateValue, trancheData.minSubordinateRatio);
                require(
                    isMinRatioSatisfied,
                    string(
                        abi.encodePacked(
                            "StructuredPortfolio: Tranche ",
                            StringsUpgradeable.toString(i),
                            " minimum subordinate ratio not met"
                        )
                    )
                );
            }
    ...
    function _isMinRatioSatisfied(
        uint256 trancheValue,
        uint256 subordinateValue,
        uint256 minSubordinateRatio
    ) internal pure returns (bool) {
        uint256 subordinateValueInBps = subordinateValue * BASIS_PRECISION;
        uint256 lowerBound = trancheValue * minSubordinateRatio;
        return subordinateValueInBps >= lowerBound;
    }

    function _checkTranchesMinSizes() internal view {
        for (uint256 i = 0; i < tranchesCount; i++) {
            ITrancheVault tranche = tranches[i];
            require(
                tranche.totalAssets() >= tranche.minSize(),
                string(abi.encodePacked("StructuredPortfolio: Tranche ", StringsUpgradeable.toString(i), " minimum size not met"))
            );
        }
    }
    ...
    function close() external {
        ...
        if (status == Status.Live) {
            require(isManager || isAfterEndDate, "StructuredPortfolio: Only manager can close before end date");
            _close();
        } else {
            require(isManager || block.timestamp >= startDeadline, "StructuredPortfolio: Only manager can close before deadline");
        }
    ...
    function _close() internal {
        ...
        for (uint256 i = 0; i < tranchesCount; i++) {
            ...
            trancheData.maxValueOnClose = _assumedTrancheValue(i, timePassed);
    ...
    function calculateWaterfall() public view returns (uint256[] memory) {
        ...
        uint256 timePassed = _getTimePassedSinceCheckpoint();

        for (uint256 i = tranchesCount - 1; i > 0; i--) {
            uint256 assumedTrancheValue = _assumedTrancheValue(i, timePassed);

            if (assumedTrancheValue + totalWaterfallSum > portfolioValue) {
    ...
    function _assumedTrancheValue(uint256 trancheIdx, uint256 timePassed) internal view returns (uint256) {
        if (trancheIdx == 0) {
            return 0;
        }
        TrancheData memory _trancheData = tranchesData[trancheIdx];

        uint256 initialValue = tranchesCheckpoint.totalAssets[trancheIdx];
        uint256 interest = (initialValue * _trancheData.targetApy * timePassed) / YEAR / BASIS_PRECISION;
    ...
    function _getTimePassedSinceCheckpoint() internal view returns (uint256) {
        return _min(block.timestamp, endDate) - tranchesCheckpoint.timestamp;
    }
