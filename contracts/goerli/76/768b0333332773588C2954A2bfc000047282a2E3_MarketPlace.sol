...
contract MarketPlace is TimelockAccess, IMarketPlace, IModuleBox, OwnableUpgradeable {
    ...
    CountersUpgradeable.Counter private enhancerIds;
    EnumerableSetUpgradeable.UintSet private nanoCells;
    EnumerableSetUpgradeable.UintSet private modules;
        ...
        returns (address[] memory owners, uint256[] memory metaCells)
    {
        ...
        for (uint256 i = 0; i < metaCells.length; i++) {
            owners[i] = IERC721Upgradeable(metaCellToken).ownerOf(
                metaCells[i]
            );
        }
        ...
        override
    {
        ...
        ICellRepository(metaCellToken).updateMetaCell(
            cell,
            IERC721Upgradeable(metaCellToken).ownerOf(_tokenId)
        );
        ...
        IERC721Upgradeable(metaCellToken).safeTransferFrom(
            _oldOwner,
            msg.sender,
            _tokenId,
            ""
        ...
        override
    {
        ...
        require(
            msg.sender == IERC721Upgradeable(metaCellToken).ownerOf(_tokenId),
    ...
    function buyModule(uint256 id) external override {
        address oldOwnerOf = IERC721Upgradeable(moduleToken).ownerOf(id);
        uint256 price = modulesOnSale[id];
        require(modulesOnSale[id] != 0, "Empty price");
        require(oldOwnerOf != msg.sender, "You are the owner");
        require(
            IERC20Upgradeable(biometaToken).balanceOf(msg.sender) > price,
            "Not enough funds"
        );
        IERC20Upgradeable(biometaToken).transferFrom(msg.sender, address(this), price);
        uint256 feeAmount = getFeeAmount(price);
        IERC20Upgradeable(biometaToken).approve(address(this), price.sub(feeAmount));
        ...
        // NOTE: marketplace(this contract) should be approved by token owner to transfer cell
        IERC721Upgradeable(moduleToken).safeTransferFrom(oldOwnerOf, msg.sender, id, "");
        delete modulesOnSale[id];
        EnumerableSetUpgradeable.remove(modules, id);
        emit ModuleSold(
            oldOwnerOf,
            id,
            msg.sender,
    ...
    function sellModule(uint256 id, uint256 price) external override {
        require(!EnumerableSetUpgradeable.contains(modules, id), "Module is on sale");
        require(price != 0, "Invalid price");
        require(
            IERC721Upgradeable(moduleToken).ownerOf(id) == msg.sender,
            "Invalid owner"
        );
        modulesOnSale[id] = price;
        EnumerableSetUpgradeable.add(modules, id);
    ...
    function updateModulesPrice(uint256 id, uint256 price) external override {
        ...
        require(
            EnumerableSetUpgradeable.contains(modules, id),
    ...
    function buyNanoCell(uint256 id) external override {
        address oldOwnerOf = IERC721Upgradeable(nanoCellToken).ownerOf(id);
        ...
        // NOTE: marketplace(this contract) should be approved by token owner to transfer cell
        IERC721Upgradeable(nanoCellToken).safeTransferFrom(oldOwnerOf, msg.sender, id, "");
        delete nanoCellOnSale[id];
        EnumerableSetUpgradeable.remove(nanoCells, id);
        emit NanoCellSold(
            oldOwnerOf,
            id,
            msg.sender,
    ...
    function sellNanoCell(uint256 id, uint256 price) external override {
        require(price != 0, "Invalid price");
        require(
            IERC721Upgradeable(nanoCellToken).ownerOf(id) == msg.sender,
            "Invalid owner"
        );
        require(!EnumerableSetUpgradeable.contains(nanoCells, id), "NanoCell is on sale");
        nanoCellOnSale[id] = price;
        EnumerableSetUpgradeable.add(nanoCells, id);
    ...
    function getNanoCellsOnSale()
        ...
        returns (uint256[] memory)
    {
        return EnumerableSetUpgradeable.values(nanoCells);
    }
    ...
    function updateNanoCellPrice(uint256 id, uint256 price) external override {
        ...
        require(
            EnumerableSetUpgradeable.contains(nanoCells, id),
    ...
    function buyBox(address owner) external override {
        require(
            IERC20Upgradeable(biometaToken).balanceOf(msg.sender) >= pricePerBox,
            "Not enough funds"
        );

        //TODO: ideally we should send mad directly to feeWallet
        IERC20Upgradeable(biometaToken).transferFrom(msg.sender, address(this), pricePerBox);
        ...
        boxes[box.id] = box;
        EnumerableSetUpgradeable.add(boxIds[owner], box.id);
    ...
    function openBox(uint256 id) external override {
        ...
        EnumerableSetUpgradeable.remove(boxIds[msg.sender], id);
        delete boxes[id];
    ...
    function getUserBoxes(address owner)
        ...
        returns (uint256[] memory)
    {
        return EnumerableSetUpgradeable.values(boxIds[owner]);
    }
...
abstract contract IModuleBox {
    using CountersUpgradeable for CountersUpgradeable.Counter;
    CountersUpgradeable.Counter internal boxCounter;
    mapping(address => EnumerableSetUpgradeable.UintSet) internal boxIds;
