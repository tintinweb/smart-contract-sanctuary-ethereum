...
contract YouTubePerformanceBounty is ChainlinkClient, ConfirmedOwner {
    ...
    uint256 public requestMetricsFulfillCount;
    ...
    function funderWithdrawBounty() public {
        require(msg.sender == funder, "Unauthorized");
        if (
            deadline < block.timestamp &&
            0 < requestMetricsFulfillCount &&
            views < desiredViewsCount &&
            comments < desiredCommentsCount
        ) {
            funder.transfer(address(this).balance);
        }
    }

    function receiverWithdrawBounty() public {
