...
contract Payload is Ownable {
    ...
    PayloadPrice public payloadPrice =
        PayloadPrice(0x76BE3b62873462d2142405439777e971754E8E77);
    ...
    event SetRedeemableCardCount(
        address indexed account,
        uint256 indexed cardCount
    );
    event SetPayloadPrice(address indexed payloadPrice);
    ...
    function handleInvokeEchelon(
        ...
        uint256 cardPrice = payloadPrice.getCardPrice(cardCount);

        // price can only deduce over time so we can have loose check here
        ...
        onlyOwner
    {
        ...
        emit SetCardCountOperatorAddress(_cardCountOperator);
    }
    ...
    /** @notice Set the payload price address
     *  @param _payloadPrice payload price address
     */
    function setPayloadPrice(PayloadPrice _payloadPrice) external onlyOwner {
        payloadPrice = _payloadPrice;
        emit SetPayloadPrice(address(_payloadPrice));
    }
...
contract PayloadPrice is Ownable {
    uint256 public constant PRECISION = 10000000000;

    uint256 public price = 1 ether;
    uint256 public lastGrowthPrice = 1 ether;
    uint256 public lastDecayPrice = 1 ether;

    uint256 public linearIncreaseRate = 10000000;
    uint256 public linearDecreaseRate = 10000000;
    uint256 public growthDoubleTime = 3600;
    uint256 public decayHalfLife = 3600;
    uint256 public growthFactor = 1;
    uint256 public decayFactor = 1;
    uint256 public ceilingPrice = 10000 ether;
    uint256 public floorPrice = 0 ether;
    uint256 public numHitsPerIncrease = 2000;
    uint256 public secondsPerDecrease = 300;

    uint256 public countSinceLastPriceDecrease = 0;
    uint256 public lastTimestamp;
    Payload public payload =
        Payload(0x76BE3b62873462d2142405439777e971754E8E77);

    event LinearIncreaseRateSet(uint256 indexed linearDecreaseRate);
    event LinearDecreaseRateSet(uint256 indexed linearDecreaseRate);
    event GrowthDoubleTimeSet(uint256 indexed growthDoubleTime);
    event DecayHalfLifeSet(uint256 indexed decayHalfLife);
    event GrowthFactorSet(uint256 indexed growthFactor);
    event DecayFactorSet(uint256 indexed decayFactor);
    event CeilingPriceSet(uint256 indexed ceilingPrice);
    event FloorPriceSet(uint256 indexed floorPrice);
    event PrimeAddressSet(uint256 indexed numHitsPerIncrease);
    event PayloadSet(address indexed payload);
    event SecondsPerDecreaseSet(uint256 indexed secondsPerDecrease);
    ...
    function getExpDecay(
        ...
        value >>= (t / halfLife);
        t %= halfLife;
        exp = value - (value * t) / halfLife / 2;
    }

    function getExpGrowth(
        uint256 value,
        uint256 t,
        uint256 halfLife
    ) public pure returns (uint256 exp) {
    ...
    function calculatePrice()
        public
        view
        returns (uint256 timeIntervals, uint256 newPrice)
    {
        timeIntervals = (block.timestamp - lastTimestamp) / secondsPerDecrease;

        if (timeIntervals > 0 && lastTimestamp != 0) {
            uint256 linear = ((linearDecreaseRate * timeIntervals * 1 ether) /
                PRECISION);
            newPrice =
                decayFactor *
                getExpDecay(price, timeIntervals, decayHalfLife);
            ...
            uint256 numTicks = countSinceLastPriceDecrease / numHitsPerIncrease;
            uint256 exp = growthFactor *
                getExpGrowth(price, numTicks, growthDoubleTime);
            uint256 linear = ((linearIncreaseRate * numTicks * 1 ether) /
                PRECISION);

            if (exp + linear > ceilingPrice) {
    ...
    function getCardPrice(uint256 _cardCount) external returns (uint256) {
        (uint256 timeIntervals, uint256 newPrice) = calculatePrice();

        if (timeIntervals > 0 && lastTimestamp != 0) {
            countSinceLastPriceDecrease = _cardCount;
            lastDecayPrice = newPrice;
        } else {
            countSinceLastPriceDecrease += _cardCount;
            lastGrowthPrice = newPrice;
        }
    ...
    /** @notice Set the payload address
     *  @param _payload payload address
     */
    function setPayload(Payload _payload) external onlyOwner {
        payload = _payload;
        emit PayloadSet(address(_payload));
    }

    /** @notice Set the linear % each tick should increase by
     *  @param _linearIncreaseRate % to set that is relative to PRECISION
     */
    function setLinearIncreaseRate(uint256 _linearIncreaseRate)
        external
        onlyOwner
    {
        linearIncreaseRate = _linearIncreaseRate;
        emit LinearIncreaseRateSet(_linearIncreaseRate);
    }
     ...
     */
    function setLinearDecreaseRate(uint256 _linearDecreaseRate)
        external
        onlyOwner
    {
        linearDecreaseRate = _linearDecreaseRate;
        emit LinearDecreaseRateSet(_linearDecreaseRate);
    }

    /** @notice Set the growth double time
     *  @param _growthDoubleTime New growth double time
     */
    function setGrowthDoubleTime(uint256 _growthDoubleTime) external onlyOwner {
        growthDoubleTime = _growthDoubleTime;
        emit GrowthDoubleTimeSet(_growthDoubleTime);
    }

    /** @notice Set the decay half life
     *  @param _decayHalfLife New decay half life
     */
    function setDecayHalfLife(uint256 _decayHalfLife) external onlyOwner {
        decayHalfLife = _decayHalfLife;
        emit DecayHalfLifeSet(_decayHalfLife);
    }

    /** @notice Set the growth factor relative to exponential growth
     *  @param _growthFactor New growth factor
     */
    function setGrowthFactor(uint256 _growthFactor) external onlyOwner {
    ...
    function setDecayFactor(uint256 _decayFactor) external onlyOwner {
        ...
        emit DecayFactorSet(_decayFactor);
    }

    /** @notice The max price each card can be worth
     *  @param _ceilingPrice Ceiling price to set
     */
    function setCeilingPrice(uint256 _ceilingPrice) external onlyOwner {
        ceilingPrice = _ceilingPrice;
        emit CeilingPriceSet(_ceilingPrice);
    }
     ...
     */
    function setNumHitsPerIncrease(uint256 _numHitsPerIncrease)
        external
        onlyOwner
    {
        numHitsPerIncrease = _numHitsPerIncrease;
        emit PrimeAddressSet(_numHitsPerIncrease);
    }

    /** @notice Set how many seconds it takes before price is decreased
     *  @param _secondsPerDecrease number of seconds per decrease
     */
    function setSecondsPerDecrease(uint256 _secondsPerDecrease)
        external
        onlyOwner
    {
        secondsPerDecrease = _secondsPerDecrease;
        emit SecondsPerDecreaseSet(_secondsPerDecrease);
    }
}