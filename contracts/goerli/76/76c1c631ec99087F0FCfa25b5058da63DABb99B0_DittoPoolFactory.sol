...
contract DittoPoolFactory is IDittoPoolFactory, OwnerTwoStep, ReentrancyGuard {
    ...
    uint96 internal constant MAX_PROTOCOL_FEE = 0.10e18;
    ...
    LpNft private immutable _lpNft;
    ...
    constructor(
        ...
        _lpNft = new LpNft(msg.sender, metadataGenerator_);
        ...
        returns (IDittoPool dittoPool, uint256 lpId, IPoolManager poolManager, IPermitter permitter)
    {
        ...
        if (permitterTemplate_.templateIndex < _permitterTemplates.length) {
            permitter = IPermitter(
                address(_permitterTemplates[permitterTemplate_.templateIndex]).clone()
            );
            permitter.initialize(permitterTemplate_.templateInitData);
        }
        ...
        if (poolManagerTemplate_.templateIndex < _poolManagerTemplates.length) {
            poolManager = IPoolManager(
                address(_poolManagerTemplates[poolManagerTemplate_.templateIndex]).clone()
            );
    ...
    ///@inheritdoc IDittoPoolFactory
    function addPoolManagerTemplates(IPoolManager[] calldata poolManagerTemplates_)
        external
        onlyOwner
    {
        _addPoolManagerTemplates(poolManagerTemplates_);
    ...
    function setMetadataGenerator(IMetadataGenerator metadataGenerator_) external onlyOwner {
        _lpNft.setMetadataGenerator(metadataGenerator_);
    }
    ...
    function _addLiquidityIfRequested(
        ...
        if (areErc20sToAdd || areNftsToAdd) {
            if (areErc20sToAdd) {
                ...
                ERC20(poolTemplate_.token).safeApprove(
                    address(dittoPool_), poolTemplate_.initialTokenBalance
                );
            }
            if (areNftsToAdd) {
     ...
     */
    function _transferNftsToFactory(
        IERC721 nft_,
        address from_,
