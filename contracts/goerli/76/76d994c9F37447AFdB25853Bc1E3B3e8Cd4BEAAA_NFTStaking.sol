...
contract NFTStaking is Ownable, IERC721Receiver,ReentrancyGuard {
    ...
    //Rewards Per Level 
    uint256 private levelZeroRewardsPerSecond = 1000000000000000000;
    uint256 private levelOneRewardsPerSecond = 3000000000000000000;
    uint256 private levelTwoRewardsPerSecond = 9000000000000000000;
    uint256 private levelThreeRewardsPerSecond = 27000000000000000000;


    //Price For Upgrades
    uint256 private levelOneUpgradePrice = 33000000000000000000;
    uint256 private levelTwoUpgradePrice = 99000000000000000000;
    ...
    function upgradeNFT(uint256 tokenID) external {
        ...
        require(currentLevel < 2,"You Have Reached Maximum Level");   
    ...
    function upgradeToMythic(uint256 _tokenToUpdate, uint256[2] memory _tokensToSacrifice ) external {
        //require(nft.ownerOf(_tokenToUpdate) == msg.sender, "You are not the owner,If it's staked, unstake and upgrade.");
        vault memory _sacrificeone = Vault[_tokensToSacrifice[0]];
        vault memory _sacrificetwo = Vault[_tokensToSacrifice[1]];
        if(_sacrificeone.isStaked){
            require(_sacrificeone.owner == msg.sender, "You are not the owner,If it's staked, unstake and upgrade.");
            _claim(_tokensToSacrifice[0], true); // Claim and Unstake if the token is staked , so that they get updated rewards from going on
        }
        else{
            require(nft.ownerOf(_tokensToSacrifice[0]) == msg.sender, "You are not the owner,If it's staked, unstake and upgrade.");
        }
        if(_sacrificetwo.isStaked){
            require(_sacrificetwo.owner == msg.sender, "You are not the owner,If it's staked, unstake and upgrade.");
            _claim(_tokensToSacrifice[1], true); // Claim and Unstake if the token is staked , so that they get updated rewards from going on
        }
        else{
            require(nft.ownerOf(_tokensToSacrifice[1]) == msg.sender, "You are not the owner,If it's staked, unstake and upgrade.");
        }
        
        require(NFTLevels[_tokensToSacrifice[0]] == 2,"You need to burn level 2 nft for mythic" );
        require(NFTLevels[_tokensToSacrifice[1]] == 2,"You need to burn level 2 nft for mythic" );

        uint256 currentLevel = NFTLevels[_tokenToUpdate];
        require(currentLevel == 2,"Please Select Level 2 NFT to Upgrade.");  
        token.transferFrom(msg.sender, address(this), getUpgradePrice(currentLevel));
        nft.burn(_tokensToSacrifice[0]);
        nft.burn(_tokensToSacrifice[1]);
        NFTLevels[_tokenToUpdate] = currentLevel +1;
        emit NFTUpgraded(msg.sender,_tokenToUpdate);
    }
