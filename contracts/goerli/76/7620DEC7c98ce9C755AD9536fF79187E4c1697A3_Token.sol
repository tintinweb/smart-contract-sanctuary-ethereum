    ...
    UUPSUpgradeable
{
    ...
    /**
     * Indicates that an account has approved operators to spend tokens on its behalf
     */
    event Approval(
    ...
    /**
     * Indicates that a new checkpoint is created which results from a newly created vote
     */
    ...
    /**
     * Voting weight checkpoints, which are updated everytime a vote is created and governance tokens minted,
     * transferred and burned.
     * {checkpointIds} is an array of past checkpoint IDs
    ...
    /**
     * Total supply checkpoints, which are updated everytime governance tokens are minted and burned.
     * {checkpointIds} is an array of past checkpoint IDs
     * {votingWegights} is an array of total voting weights corresponding to the checkpoint id at the same index
     * The total voting weight is the total supply of governance tokens at the corresponding checkpoint.
     */
    struct TotalVotingWeightCheckpoints {
        // Current checkpoint ID is a counter, which can only be incremented when a vote is called on
        uint256 currentCheckpointId;
        uint256[] checkpointIds;
        uint256[] totalVotingWeights;
    }
     ...
     * account => voting weight checkpoints
     */
    mapping(address => VotingWeightCheckpoints)
        private _accountVotingWeightCheckpoints;

    /**
     * Total voting weight checkpoints, same as the total supply of governance tokens at each checkpoint
     */
    TotalVotingWeightCheckpoints private _totalVotingWeightCheckpoints;

    mapping(uint256 => uint256) private _totalSupplies;

    // Indicates if token transfer (including minting and burning) is prohibited
     ...
     */
    function getCurrentCheckpointId() public view returns (uint256) {
        return _totalVotingWeightCheckpoints.currentCheckpointId;
    }

    /**
     * Create a new checkpoint and increment it by 1
     * Access is limited to {VOTE_REGISTER_ROLE} role
     * Designed to be invoked when a vote/poll is created
     */
    function createCheckpoint()
        ...
        returns (uint256)
    {
        uint256 newCheckpointId = _totalVotingWeightCheckpoints
            .currentCheckpointId++;
        emit CheckpointCreated(newCheckpointId);
    ...
    /**
     * Obtain the total voting weight (total governance supply) at checkpoint {checkpointId}
     */
    function checkpointedTotalVotingWeight(uint256 checkpointId)
        public
        view
        returns (uint256)
    {
        return
            _checkpointedValue(
                checkpointId,
                _totalVotingWeightCheckpoints.checkpointIds,
                _totalVotingWeightCheckpoints.totalVotingWeights
            );
    }
    ...
    function delegateeOf(address account) public view returns (address) {
        return _accountVotingWeightCheckpoints[account].delegatee;
    }
     ...
     *
     * Delegated voting weight is not forwarded through delegation, only the current
        ...
        onlyRole(VOTE_REGISTER_ROLE)
    {
        VotingWeightCheckpoints
            storage delegatorCkpts = _accountVotingWeightCheckpoints[delegator];
        VotingWeightCheckpoints
            storage delegateeCkpts = _accountVotingWeightCheckpoints[delegatee];
        address oldDelegatee = delegatorCkpts.delegatee;
        VotingWeightCheckpoints
            storage oldDelegateeCkpts = _accountVotingWeightCheckpoints[
                oldDelegatee
            ];
        uint256 balance = balanceOf(delegator, 0);
        if (oldDelegatee != address(0)) {
            _modifyCheckpointedValues(
                oldDelegateeCkpts.checkpointIds,
                oldDelegateeCkpts.votingWeights,
                balance,
                _subtract
            );
            if (delegatee == address(0)) {
                _modifyCheckpointedValues(
                    delegatorCkpts.checkpointIds,
                    delegatorCkpts.votingWeights,
                    balance,
                    _add
                );
            }
        ...
        if (delegatee != address(0)) {
            _modifyCheckpointedValues(
                delegateeCkpts.checkpointIds,
                delegateeCkpts.votingWeights,
                balance,
                _add
            );
            if (oldDelegatee == address(0)) {
        ...
        emit DelegationChanged(delegator, oldDelegatee, delegatee);
        delegatorCkpts.delegatee = delegatee;
    }

    /**
     * When a governance token transfer happens (including minting and burning), the voting weights have to
     ...
     */
    function _targetVotingWeightCheckpoints(address account)
        private
        view
        returns (VotingWeightCheckpoints storage)
    {
        VotingWeightCheckpoints
            storage accountVotingWeightCkpts = _accountVotingWeightCheckpoints[
                account
            ];
        address delegatee = accountVotingWeightCkpts.delegatee;
        return
            delegatee == address(0)
                ? accountVotingWeightCkpts
                : _accountVotingWeightCheckpoints[delegatee];
    }
    ...
    function _modifyCheckpointedValues(
        ...
        if (
            ids.length == 0 ||
            ids[ids.length - 1] <
            _totalVotingWeightCheckpoints.currentCheckpointId
        ) {
            values.push(newValue);
            ids.push(_totalVotingWeightCheckpoints.currentCheckpointId);
        } else {
    ...
    /**
     * Obtain the voting weight of the {account} at the {checkpointId}
     */
    function checkpointedVotingWeight(address account, uint256 checkpointId)
        public
        view
        returns (uint256)
    {
        return
            _checkpointedValue(
                checkpointId,
                _accountVotingWeightCheckpoints[account].checkpointIds,
                _accountVotingWeightCheckpoints[account].votingWeights
            );
    }

    /**
     * Core token transfer function including minting and burning tokens
     * Need to update the voting weights of the accounts (delegatees if appointed) when
     * transferring goernance tokens
     */
    function _tokenTransfer(
        ...
        if (from != address(0)) {
            ...
            if (tokenId == 0) {
                VotingWeightCheckpoints
                    storage target = _targetVotingWeightCheckpoints(from);
                _modifyCheckpointedValues(
                    target.checkpointIds,
        ...
        if (to != address(0)) {
            ...
            if (tokenId == 0) {
                VotingWeightCheckpoints
                    storage target = _targetVotingWeightCheckpoints(to);
                _modifyCheckpointedValues(
                    target.checkpointIds,
                    target.votingWeights,
                    amount,
    ...
    /**
     * Total supply is updated when minting tokens
     */
    function mintBatch(
        address to,
        uint256[] calldata tokenIds,
...
contract AccessControl is Initializable, ContextUpgradeable, UUPSUpgradeable {
