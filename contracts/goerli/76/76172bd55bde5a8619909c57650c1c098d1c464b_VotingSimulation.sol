...
contract VotingSimulation {
    ...
    function createVoters() public {
        ...
        uint256 prevNumber = uint(keccak256(abi.encodePacked(block.timestamp ))) % 1000;
        uint256 multiplier = block.timestamp % 31;
        uint256 increment = block.timestamp % 27;
        uint256 modulus =  increment + multiplier + block.timestamp % 53; 
        ...
        for (uint256 i;i<5;i++){
         ...
         for (uint256 i; i< 100; i++){
            s_votesArray[i] = 0;
            randomNumber = (prevNumber*multiplier+increment)%modulus % 10;
    ...
    function startVoting() public{
       ...
       for (uint256 i; i<10; i++){
        ...
        uint256 randomNumber = (prevNumber*multiplier+increment)%modulus % 5;
