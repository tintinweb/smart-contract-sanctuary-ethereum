...
import "./GameCreditSaleZ.sol";

contract GameCreditSaleZTest is GameCreditSaleZ {
    ...
    GnosisSafeUpgradeable
{
    ...
    uint oneToken;
    uint public creditPrice; // The price of credit in ZONE.
    ...
    function getTokenAmount(uint _credits) public view returns (uint) {
        return  _credits * creditPrice;
    }

    ///@param _amount is amount of ZONE token
    function buy(uint _amount) external {
        uint credits = _amount / creditPrice;
        uint adjustedAmount = credits * creditPrice;
