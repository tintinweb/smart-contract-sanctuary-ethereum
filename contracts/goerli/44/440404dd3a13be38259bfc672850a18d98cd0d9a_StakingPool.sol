...
contract StakingPool is Ownable, Pausable, ReentrancyGuard{
    ...
    mapping(uint256 => uint256[2]) public totalStaked; 
    mapping(uint256 => uint256[2]) public totalShares;
    ...
    function stake(uint256 index, uint256 amount) public {
        ...
        DegenIDToken.safeTransfer(address(this), amount);
        data.tokenStaked[index] = data.tokenStaked[index] + amount;
        data.actualShare[index] = data.actualShare[index] + weightedShare;

        totalStaked[currentRound][index] = totalStaked[currentRound][index] + amount;
        totalShares[currentRound][index] = totalShares[currentRound][index] + weightedShare;
    ...
    function _calculateShare(address user, uint256 round, uint256 index) internal view returns(uint256){
        ...
        return RewardAmount*data.actualShare[index]/(totalShares[round][index]);
    }
    ...
    function getCompounderPercent() public view returns(uint256){
        return 10000*totalShares[currentRound][0]/(totalShares[currentRound][0]+totalShares[currentRound][1]);
    }
