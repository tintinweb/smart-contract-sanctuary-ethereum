...
contract Pool is Initializable, BasePool {
    ...
    function setRedeemCooldown(uint256 _redeemCooldown) external onlyOwner {
        redeemCooldown = _redeemCooldown;
        emit SetRedeemCooldown(_redeemCooldown);
    }
    ...
    function calcAdjustedFee(
        ...
        if (nextDiff < initDiff) {
            ...
            uint256 feeAdjust = (targetValue == 0 || avgDiff > targetValue)
                ? taxBasisPoint
                : (taxBasisPoint * avgDiff) / targetValue;
    ...
    event SetRedeemCooldown(uint256 redeemCooldown);
}
