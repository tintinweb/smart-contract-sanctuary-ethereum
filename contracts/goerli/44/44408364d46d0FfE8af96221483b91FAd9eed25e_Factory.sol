...
contract NFTcontract is ERC721Enumerable, ReentrancyGuard {
    ...
    function updateMultiBaseURIs(
        ...
        uint256 i;
        uint256 tmp = 0;
        for (i = 0; i < _numOfTokens.length; i++) {
            tmp += _numOfTokens[i];
        }
        ...
        for (i = 0; i < _numOfTokens.length; i++) {
            ...
            arrBaseURIs[totalCntOfContent] = _baseURIs[i];
        }
    }

    // ============ FUNCTION TO UPDATE SINGLE BASEURI FOR EXISTING ITEM ============
    function updateSingleBaseURIExisting(
        uint256 _tokenId,
        string calldata _baseURI
    ) external onlyOwner {
        require(
            (_tokenId <= totalCntOfContent) && (_tokenId > 0),
            "Please input the valid tokenId within the available range."
        );
        if (_tokenId < smallestCnt) {
            smallestCnt = _tokenId;
        }
        if (keccak256(abi.encodePacked(arrBaseURIs[_tokenId - 1])) == keccak256(abi.encodePacked(""))) {
            uint256 i;
            for (i = _tokenId; i <= maxPublicMint; i++) {
                if (keccak256(abi.encodePacked(arrBaseURIs[i])) != keccak256(abi.encodePacked(""))) {
                    break;
                }
            }
            arrBaseURIs[_tokenId - 1] = arrBaseURIs[i];
        }
        arrBaseURIs[_tokenId] = _baseURI;
    }
