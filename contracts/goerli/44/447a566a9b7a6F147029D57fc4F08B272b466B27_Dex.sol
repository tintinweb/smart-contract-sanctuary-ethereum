...
contract Dex {
  ...
  modifier OnlyVault() {
    require(vaultContract == msg.sender, 'Permission denied');
    _;
  }
  ...
  function withdrawTo(uint256 maxAmountToSell, uint256 amountToBuy, address _recepient) public OnlyVault returns (bool) {
  ...
  function withdraw(uint256 maxAmountToSell, uint256 amountToBuy) public returns (bool) {
    return withdrawTo(maxAmountToSell, amountToBuy, msg.sender);
  }
  ...
  function emitBookingReady(address _recepient, uint256 amountToBuy) public OnlyVault {
    ...
    require(withdrawTo(amountToSell, amountToBuy, _recepient), 'Failed to withdraw');
  }
