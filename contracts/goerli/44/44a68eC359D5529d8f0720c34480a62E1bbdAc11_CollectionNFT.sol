...
contract CollectionNFT is ERC1155, ReentrancyGuard{
    ...
    // mint
    function mint(uint256 tokenId,uint256 amt, address mintAddress)
    external
    onlyStarted
    onlyAdminOrOwner(msg.sender)
    {
        if(!isAdmin[mintAddress]){
            require(
                mintedPerAddress[mintAddress] + amt <= maxMintAmount,
                "Exception: Reached the limit for each user. You can't mint no more"
            );
        }
        _mint(mintAddress, tokenId, amt, "");
        mintedPerAddress[mintAddress] = mintedPerAddress[mintAddress] + amt;
        mintedAmount[tokenId] = mintedAmount[tokenId] + amt;
    }
