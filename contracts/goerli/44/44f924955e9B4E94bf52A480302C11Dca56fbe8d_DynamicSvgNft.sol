...
contract DynamicSvgNft is ERC721 {
    ...
    function mintNft(int256 _highValue) public {
        ...
        emit NftMinted(s_tokenCounter, _highValue);
        s_tokenCounter = s_tokenCounter + 1;
    }

    function tokenURI(uint256 _tokenId) public view override returns (string memory) {
        require(_exists(_tokenId), "DynamicSvgNft: Query for non existing token");
    ...
    function svgToImageURI(string memory _svg) public pure returns (string memory) {
        string memory svgBase64Encoded = Base64.encode(bytes(abi.encodePacked(_svg)));
