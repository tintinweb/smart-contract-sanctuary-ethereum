...
contract AvarikSaga is EIP712, ERC721Enumerable, Ownable {
    ...
    uint256 public constant AVARIK_PRICE = 0.000000000001 ether;
    ...
    constructor(string memory tokenBaseURI_, address _whitelistSigner)
        ERC721("Avarik Saga Test", "AVARIK-TST")
    ...
    function _hash(address _buyer, uint256 _maxCount)
        internal
        view
        returns (bytes32 hash)
    {
        hash = _hashTypedDataV4(
            keccak256(abi.encode(PRESALE_TYPEHASH, _buyer, _maxCount))
        );
    }
    ...
    function presaleMod(address _user, uint256 tokenQuantity) external  {
        require(whitelistSigner != address(0), "Signer is default address!");
        require(totalSupply() < AVARIK_MAX, "All Avariks are minted");
        require(
            privateAmountMinted + tokenQuantity <= AVARIK_PRIVATE,
            "Minting would exceed the presale allocation"
        );

        presalerListPurchases[_user] += tokenQuantity;
    ...
    function presaleBuy(
        ...
        require(whitelistSigner != address(0), "Signer is default address!");
        require(
            _verify(_hash(msg.sender, maxCount), signature),
        ...
        require(
            presalerListPurchases[msg.sender] + tokenQuantity <= maxCount,
            "You can not mint exceeds maximum NFT"
        );
        require(
            AVARIK_PRICE * tokenQuantity <= msg.value,
