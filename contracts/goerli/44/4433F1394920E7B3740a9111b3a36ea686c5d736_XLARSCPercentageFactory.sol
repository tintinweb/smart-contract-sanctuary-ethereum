...
import "contracts/RSCPercentage.sol";


contract XLARSCPercentageFactory {
    address payable immutable contractImplementation;

    event RSCGCreated(address contractAddress, address owner);

    constructor() {
        contractImplementation = payable(new XLARSCPercentage());
    }

    function createRSCPercentage() external returns(address) {
        address payable clone = payable(Clones.clone(contractImplementation));
        XLARSCPercentage(clone).initialize(msg.sender);
        emit RSCGCreated(clone, msg.sender);
        return clone;
    }
}
...
contract XLARSCPercentage is Initializable, OwnableUpgradeable {
