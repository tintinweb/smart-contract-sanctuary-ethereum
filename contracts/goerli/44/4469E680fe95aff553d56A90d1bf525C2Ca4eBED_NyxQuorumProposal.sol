...
import "./interfaces/INyxQuorumProposal.sol";

contract NyxQuorumProposal is NyxProposal, INyxQuorumProposal {
    string public constant name = "NyxQuorumProposal";
        ...
        returns (QuorumProposalParams memory)
    {
        ...
        return QuorumProposalParams(bytesToUint(params[0]),
    ...
    function getProposal(bytes memory proposalBytes)
        ...
        returns (bytes[] memory)
    {
        QuorumProposalParams memory params = parseProposalParams(proposalBytes);
        ...
        returns (bytes[nparams][] memory props)
    {
        ...
        {
            QuorumProposalParams memory params = parseProposalParams(proposalBytesArr[idx]);
...
// @title INyxQuorumProposal for Nyx DAO
// @author TheV
...
interface INyxQuorumProposal {
    ...
    function parseProposalParams(bytes[] memory params) external pure returns (QuorumProposalParams memory);
    function parseProposalParams(bytes memory params) external pure returns (QuorumProposalParams memory);
    function proposalToBytes(QuorumProposal memory proposal) external pure returns (bytes memory);
