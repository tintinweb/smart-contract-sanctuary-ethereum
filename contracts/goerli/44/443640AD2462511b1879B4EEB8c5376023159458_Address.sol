...
function generateCommonValues(uint256 _tokenId) internal pure returns (CommonValues memory) {
    ...
    uint256 petalCount = uint256(keccak256(abi.encodePacked(_tokenId, "petalCount"))) % 10 + 5;

    uint256[] memory radius = new uint256[](petalCount);
    uint256[] memory strokeWidth = new uint256[](petalCount);
    ...
    return CommonValues(hue, petalCount, radius, strokeWidth);
}
...
function generateSVG(uint256 _tokenId) internal pure returns (string memory) {
    ...
    for (uint256 i = 0; i < commonValues.petalCount; i++) {
        uint256 hueStep = 360 / commonValues.petalCount;
        uint256 hue = (commonValues.hue + (i * hueStep)) % 360;
        ...
        string memory strokeColor = string(abi.encodePacked("hsl(", Strings.toString(hue), ",", Strings.toString(sat), "%,64%)"));

        uint256 petalX = uint256(keccak256(abi.encodePacked(_tokenId, "petalX", i))) % 300 + 10;
        uint256 petalY = uint256(keccak256(abi.encodePacked(_tokenId, "petalY", i))) % 300 + 10;
        uint256 radius = commonValues.radius[i];
        uint256 strokeWidth = commonValues.strokeWidth[i];

        string memory petal = string(
            abi.encodePacked(
                '<circle cx="', Strings.toString(petalX), '" cy="', Strings.toString(petalY), '" r="', Strings.toString(radius), '" fill="', fillColor, '" stroke="', strokeColor, '" stroke-width="', Strings.toString(strokeWidth), '"/>'
            )
        );
...
function generateAttributes(uint256 _tokenId) internal pure returns (string memory) {
    ...
    string memory attributes = string(
        abi.encodePacked(
            '{"trait_type": "hue", "value": "', Strings.toString(commonValues.hue), '"},',
            '{"trait_type": "petal_count", "value": "', Strings.toString(commonValues.petalCount), '"},'
        )
    );
    ...
    for (uint256 i = 0; i < commonValues.petalCount; i++) {
        attributes = string(
            abi.encodePacked(
                attributes,
                '{"trait_type": "petalradius', Strings.toString(i), '", "value": "', Strings.toString(commonValues.radius[i]), ' pixels"},',
                '{"trait_type": "petal_strokewidth', Strings.toString(i), '", "value": "', Strings.toString(commonValues.strokeWidth[i]), ' pixels"}'
            )
        );
        if (i < commonValues.petalCount - 1) {
...
function tokenURI(uint256 _tokenId) public view override returns (string memory) {
    ...
    string memory name = string(abi.encodePacked("Flower #", Strings.toString(_tokenId)));
    string memory description = "Flowers blossoming in different color hues and sizes.";
