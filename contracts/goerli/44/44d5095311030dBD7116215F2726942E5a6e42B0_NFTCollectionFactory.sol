 ...
 */
abstract contract NFTCollectionFactoryACL {
  using AddressUpgradeable for address;
  ...
  modifier onlyAdmin() {
    if (!rolesManager.isAdmin(msg.sender)) {
...
abstract contract NFTCollectionFactoryNFTCollections is NFTCollectionFactorySharedTemplates {
   ...
   */
  function createNFTCollection(string calldata name, string calldata symbol, uint96 nonce)
    external
    returns (address collection)
  {
    collection = _createCollection(CollectionType.NFTCollection, symbol, nonce);
    INFTCollectionInitializer(collection).initialize(payable(msg.sender), name, symbol);

    emit NFTCollectionCreated(
      collection,
      msg.sender,
      _getTemplateVersion(CollectionType.NFTCollection),
      name,
      symbol,
      nonce
    );
...
abstract contract NFTCollectionFactoryNFTDropCollections is NFTCollectionFactorySharedTemplates {
  /**
  ...
  function _createNFTDropCollection(
    ...
    collection = _createCollection(CollectionType.NFTDropCollection, symbol, nonce);
    ...
    emit NFTDropCollectionCreated(
      ...
      maxTokenId,
      paymentAddress,
      _getTemplateVersion(CollectionType.NFTDropCollection),
      nonce
    );
  }
}
...
error NFTCollectionFactorySharedTemplates_Initialize_Invalid_Version();
...
// Structs for calldata
struct InitialCollectionTemplate {
  ...
  NFTCollectionFactoryTemplateInitializer
{
  ...
  mapping(CollectionType => CollectionTemplate) private collectionTypeToTemplate;
   ...
   */
  function initialize(InitialCollectionTemplate[] calldata templates) external reinitializer(2) {
    for (uint i = 0; i < templates.length; ) {
      InitialCollectionTemplate calldata template = templates[i];
      // The version number provided should be the current version on mainnet.
      if (template.version == 0) {
        revert NFTCollectionFactorySharedTemplates_Initialize_Invalid_Version();
      }
      _setCollectionTemplate(template.collectionType, template.implementation, template.version);
  ...
  function adminUpdateCollectionTemplates(CollectionTemplateUpgrade[] calldata newTemplates) external onlyAdmin {
    for (uint i = 0; i < newTemplates.length; ) {
      _upgradeCollectionTemplate(newTemplates[i]);
   ...
   */
  function _createCollection(CollectionType collectionType, string calldata symbol, uint96 nonce)
    internal
    returns (address collection)
  {
    ...
    collection = collectionTypeToTemplate[collectionType].implementation.cloneDeterministic(
      _getSalt(msg.sender, nonce)
    );
  }

  function _upgradeCollectionTemplate(CollectionTemplateUpgrade calldata newTemplate) private {
    CollectionTemplate storage template = collectionTypeToTemplate[newTemplate.collectionType];
    uint32 version;
    unchecked {
      version = ++template.version;
    }

    _setCollectionTemplate(newTemplate.collectionType, newTemplate.implementation, version);
  }
  ...
  function _setCollectionTemplate(CollectionType collectionType, address implementation, uint32 version) private {
    ...
    collectionTypeToTemplate[collectionType] = CollectionTemplate(implementation, version);
   ...
   */
  function getCollectionTemplate(CollectionType collectionType)
    external
    view
    returns (address implementation, uint version)
  {
    CollectionTemplate memory template = collectionTypeToTemplate[collectionType];
   ...
   */
  function predictCollectionAddress(CollectionType collectionType, address creator, uint96 nonce)
    public
    view
    returns (address collection)
  {
    collection = collectionTypeToTemplate[collectionType].implementation.predictDeterministicAddress(
      _getSalt(creator, nonce)
    );
  }

  /**
   * [DEPRECATED] use `predictCollectionAddress` instead.
  ...
  /**
   * @notice A helper to get the current template version.
   * @dev Currently a separate function instead of part of `_createCollection` to avoid stack too deep errors.
   */
  function _getTemplateVersion(CollectionType collectionType) internal view returns (uint version) {
    version = collectionTypeToTemplate[collectionType].version;
  }
...
abstract contract NFTCollectionFactoryTemplateInitializer is NFTCollectionFactoryTypes {
   ...
   * @dev This call will revert if the template has already been initialized.
   */
  function _initializeTemplate(CollectionType collectionType, address implementation, uint version) internal {
    ...
    if (collectionType == CollectionType.NFTCollection) {
      ...
      INFTCollectionInitializer(implementation).initialize(payable(msg.sender), name, symbol);
    } else {
      /* if (collectionType == CollectionType.NFTDropCollection) */
...
abstract contract NFTCollectionFactoryTypes {
   ...
   * @return maxIndex The maximum value of the CollectionType enum.
   * @dev Templates are indexed from 0 to this value.
   */
  function getCollectionMaxTypeIndex() public pure returns (uint8 maxIndex) {
    maxIndex = uint8(type(CollectionType).max);
  }
  ...
  function getCollectionTypeName(CollectionType collectionType) public pure returns (string memory typeName) {
    if (collectionType == CollectionType.NFTCollection) {
      typeName = "NFT Collection";
    } else {
      /* if (collectionType == CollectionType.NFTDropCollection) */
      typeName = "NFT Drop Collection";
    }
  }
}
...
import "./mixins/shared/Gap10002.sol";

import "./mixins/nftCollectionFactory/NFTCollectionFactoryACL.sol";
import "./mixins/nftCollectionFactory/NFTCollectionFactoryNFTCollections.sol";
import "./mixins/nftCollectionFactory/NFTCollectionFactoryNFTDropCollections.sol";
  ...
  NFTCollectionFactoryNFTDropCollections
{
   ...
   * @dev
   */
  constructor(address _rolesManager) NFTCollectionFactoryACL(_rolesManager) {
...
import "../../../../contracts/mixins/nftCollectionFactory/NFTCollectionFactoryACL.sol";

contract $NFTCollectionFactoryACL is NFTCollectionFactoryACL {
    bytes32 public __hh_exposed_bytecode_marker = "hardhat-exposed";
...
import "../../../../contracts/mixins/nftCollectionFactory/NFTCollectionFactoryNFTCollections.sol";

contract $NFTCollectionFactoryNFTCollections is NFTCollectionFactoryNFTCollections {
    bytes32 public __hh_exposed_bytecode_marker = "hardhat-exposed";

    event $_createCollection_Returned(address arg0);

    constructor(address _rolesManager) NFTCollectionFactoryACL(_rolesManager) {}

    function $_createCollection(CollectionType collectionType,string calldata symbol,uint96 nonce) external returns (address) {
        (address ret0) = super._createCollection(collectionType,symbol,nonce);
        emit $_createCollection_Returned(ret0);
        return (ret0);
    }

    function $_getTemplateVersion(CollectionType collectionType) external view returns (uint256) {
        return super._getTemplateVersion(collectionType);
    }

    function $_initializeTemplate(CollectionType collectionType,address implementation,uint256 version) external {
        return super._initializeTemplate(collectionType,implementation,version);
    }
...
import "../../../../contracts/mixins/nftCollectionFactory/NFTCollectionFactoryNFTDropCollections.sol";

contract $NFTCollectionFactoryNFTDropCollections is NFTCollectionFactoryNFTDropCollections {
    bytes32 public __hh_exposed_bytecode_marker = "hardhat-exposed";
...
import "../../../../contracts/mixins/nftCollectionFactory/NFTCollectionFactorySharedTemplates.sol";

contract $NFTCollectionFactorySharedTemplates is NFTCollectionFactorySharedTemplates {
    bytes32 public __hh_exposed_bytecode_marker = "hardhat-exposed";
...
import "../../../../contracts/mixins/nftCollectionFactory/NFTCollectionFactoryTemplateInitializer.sol";

contract $NFTCollectionFactoryTemplateInitializer is NFTCollectionFactoryTemplateInitializer {
    bytes32 public __hh_exposed_bytecode_marker = "hardhat-exposed";
...
import "../../../../contracts/mixins/nftCollectionFactory/NFTCollectionFactoryTypes.sol";

contract $NFTCollectionFactoryTypes is NFTCollectionFactoryTypes {
    bytes32 public __hh_exposed_bytecode_marker = "hardhat-exposed";
...
import "../../../../contracts/mixins/shared/Gap10002.sol";

contract $Gap10002 is Gap10002 {
    bytes32 public __hh_exposed_bytecode_marker = "hardhat-exposed";
