...
contract SusuStaking is Ownable, ReentrancyGuard {
    ...
    // Stake reward time default 2025.01.01
    ...
    // Lock info
    struct LockInfo {
        // ERC721 Tokens unlock time and lock time
        uint256 unlockTime;
        uint256 lockTime;
    }
    ...
    // value of now.
    function stake(uint256[] calldata _tokenIds, uint8 _lock)
        external
        nonReentrant
    {
        ...
        for (uint256 i; i < len; ++i) {
            ...
            stakerLockInfo[_tokenIds[i]].lockTime = getStakeLockTime(_lock);
            stakerLockInfo[_tokenIds[i]].unlockTime =
                block.timestamp +
                getStakeLockTime(_lock);
    ...
    function isGoldPass(uint256 _tokenId) internal view returns (bool) {
        string memory level = getNFTLevel(_tokenId);
        if (
    ...
    function getStakeLockTime(uint8 _lock) internal pure returns (uint256) {
        if (_lock == 1) {
    ...
    function getRewardBips(uint256 _tokenId) public view returns (uint256) {
        uint256 lockTime = stakerLockInfo[_tokenId].lockTime;
        uint256 rewardBips = 0;
        if (lockTime == LOCK_TIME_ZERO) {
            rewardBips =
                (rewardsPerHour * BIPS_ZERO) /
                PERCENT_BASE_BIPS /
                3600;
        } else if (lockTime == LOCK_TIME_ONE) {
            rewardBips = (rewardsPerHour * BIPS_ONE) / PERCENT_BASE_BIPS / 3600;
        } else if (lockTime == LOCK_TIME_TWO) {
            rewardBips = (rewardsPerHour * BIPS_TWO) / PERCENT_BASE_BIPS / 3600;
        } else if (lockTime == LOCK_TIME_THREE) {
            rewardBips =
    ...
    function getTotalBips(uint256[] calldata _tokenIds)
        public
        view
        returns (uint256)
    {
        ...
        for (uint256 i; i < _tokenIds.length; ++i) {
            totalBips += getRewardBips(_tokenIds[i]);
        }
        return totalBips;
    }
    ...
    // Check unlock time of ERC721 Tokens
