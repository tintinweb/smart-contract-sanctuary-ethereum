...
contract DOTS is ERC721AQueryable, AccessControl {
    ...
    function mint(uint256 _evoNum, uint256 _numDots) external {
        ...
        for (uint i=0; i<_numDots; ) {
            ...
            unchecked { _rand = uint256(keccak256(abi.encode(keccak256(abi.encodePacked(msg.sender, tx.origin, gasleft(), block.timestamp, block.number, blockhash(block.number), blockhash(block.number-100)))))); }

            while (true) {
                _gene = _normalize(_rand);
    ...
    function upgrade(
        ...
        uint256 _diff;
        uint256 _valueFromDots = evoData[_oldEvoNum].numTokensNeeded;
        ...
        emit DOTUpgraded(_primaryTokenId, _oldEvoNum, _targetEvoNum, _tokenIds);
    }
    ...
    function upgradeMultiple(
        ...
        for (uint i=0; i<_upgrades.length; ) {
            ...
            upgrade(_upgrade.primaryTokenId, _targetEvoNum, _upgrade.tokenIds);
            unchecked { i++; }
        }
    }
    ...
    function _normalize(
        ...
        if (_traitNumSelector < 1638) {
            ...
            if (_traitNumSelector < 62169) _accessoryId = 0;
            else if (_traitNumSelector < 63033) _clothingId = 0;
            else _headgearId = 0;
        }
