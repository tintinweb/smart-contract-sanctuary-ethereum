...
// WELCOME TO CONTRACT OF VR ACCOUNT SERVICE.
...
contract ERC721A is Context, ERC165, IERC721A {
    ...
    mapping(uint => string) public vrIDtoName;
    mapping(string => uint) public vrNametoID;
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        ...
        return bytes(baseURI).length != 0 ? string(abi.encodePacked(baseURI, vrIDtoName[tokenId])) : '';
    }
...
contract zkVR is ERC721A, Ownable, ReentrancyGuard {
    using Strings for uint256;
    ...
    uint8 private ref_discount = 10;
    string private BASE_URI = 'https://zkvr.io/metadata/';
    bytes _allowChars = "0123456789-_abcdefghijklmnopqrstuvwxyz";

    constructor() ERC721A("zkVR", ".vr") {}
    ...
    function setCreatePrice(uint256 Character1and2,uint256 Character3,uint256 Character4more) external onlyOwner {
    ...
    function Create(address ref_address, string memory name)
        public
        payable
    {
        ...
        require(charlength>0,"Write a zkVR Name");
        if(charlength<3) {price=Cost1_2Character;} else if(charlength==3){price=Cost3Character;} else {price=Cost4mCharacter;}
        require (vrNametoID[name] == 0 , "This is already taken");
        require(msg.value >= price, "Insufficient funds!");
        vrIDtoName[_currentIndex]=name;
        vrNametoID[name]=_currentIndex;
        if(ref_address!= 0x0000000000000000000000000000000000000000) {ref_cost=price*ref/100;payable(ref_address).transfer(ref_cost);}
...
function walletOfvr(address _owner)
    public
    view
    returns (string[] memory)
  {
    ...
    while (ownedTokenIndex < ownerTokenCount) {
      ...
      if (currentTokenOwner == _owner) {
        ownedTokenIds[ownedTokenIndex] = vrIDtoName[currentTokenId];
        ownedTokenIndex++;
      }
...
function LastVRs(uint256 count)
    public
    view
    returns (string[] memory)
  {
    ...
    while (total > currentId) {
        lastAddr[ownedTokenIndex] = string.concat(vrIDtoName[total],"?",Strings.toString(total),"?",Strings.toHexString(uint160(ownerOf(total)), 20));
        ownedTokenIndex++;
      total--;
    }
 ...
 function checkCharacters(string memory name) public view returns(bool){
        uint allowedChars =0;
