...
@param randomContract_ The address of the random contract
@param actorContract_ The address of the actor contract
@param boxContract_ The address of the MysteryBox contract
...
@param woman_ The address of the woman calculator contract
@param childs_ The maximum number of the childs (for woman actors only)
*/
    constructor(
        string memory name_,
        string memory symbol_,
        address randomContract_,
        address actorContract_,
        ...
        _random = IRandomizer(randomContract_);
        _actor = IActors(actorContract_);
        _woman = ICalculator(woman_);
        _box = boxContract_;
        _childs = childs_;
...
*/
    ...
    function _create(address owner_, uint256 level) private returns (uint256) {
        uint256 newId = _tokenIds.current();
        _issuedLevels[newId] = level;
        _mint(owner_, newId);
        _tokenIds.increment();
        emit Created(owner_, newId, level);
        return newId;
    }
...
*/
    function setChilds(uint256 childs_) external override onlyOwner {
        _childs = childs_;
...
*/
    function setBox(address box_) external override onlyOwner {
        _box = box_;
...
*/
    function setWoman(address woman_) external override onlyOwner {
        _woman = ICalculator(woman_);
        emit WomanDefined(woman_);
    }
...
*/
    function getWoman() external view override returns (address) {
        return address(_woman);
    }

    function _getLimits(uint256 level) private view returns (uint256, uint256) {
        require(level < _limits.length, "Potion: wrong level");
        if (level == 0) {
            return (1000, _limits[0]);
        }
        return (_limits[level - 1], _limits[level]);
    }
...
*/
    ...
    function getLimits(uint256 level) external view returns (uint256, uint256) {        
        return _getLimits(level);
    }

    function womanPromille(uint256 level) external returns (uint256) {        
        (uint256 minRange, uint256 maxRange) = _getLimits(level);
        uint16[10] memory props;
        uint256 range = maxRange - minRange;
        uint256 power = 0;
        for (uint256 i = 0; i < 10; i++) {
            props[i] = uint16(_random.randomize(range) + minRange);
            power = power + props[i];
        }
        return  _woman.compute(abi.encode(power));
    }
...
*/
    function open(uint256 id_) external override returns (uint256) {
        require(ownerOf(id_) == _msgSender(), "Potion: wrong owner");
        uint256 level = _issuedLevels[id_];
        (uint256 minRange, uint256 maxRange) = _getLimits(level);
        uint16[10] memory props;
        uint256 range = maxRange - minRange;
        uint256 power = 0;
 ...
 /*       uint256 beWomanPromille = 0;// _woman.compute(abi.encode(power));
        uint256[] memory weights;
        weights[0] = 1000 - beWomanPromille;
        weights[1] = beWomanPromille;
 /*       uint256 sexPos = _random.distributeRandom(1000, weights);
        /*bool sex = (sexPos == 0);
        uint8 childs = uint8(_random.randomize(_childs + 1));
        uint256 newId = _actor.mint(_msgSender(), props, sex, childs); 
        _burn(id_);
        delete _issuedLevels[id_];
        emit Opened(_msgSender(), newId); */
        return 1111; //newId;
    }
...
*/
    function create(address target, bool rare)
        external
        override
        onlyBox
        returns (uint256)
    {
        uint256 level = _amounts.length - 1;
        if (!rare) {
            level = _random.distributeRandom(_unissued, _amounts);
            require(_amounts[level] > 0, "Potion: try again");
            _amounts[level] = _amounts[level] - 1;
        }
        require(_amounts[level] > 0, "Potion: sold out");
        return _create(target, level);
    }
...
@return The new pack id
*/
    function createPotion(address target, uint256 level)
        external
        override
        onlyOwner
        returns (uint256)
    {
        require(_unissued > 0, "Potion: sold out");
        require(_amounts[level] > 0, "Potion: sold out");
        _amounts[level] = _amounts[level] - 1;
        _unissued = _unissued - 1;
        uint256 created = _create(target, level);
        _last[target] = created;
        return created;
    }

    function getLast(address target) external returns(uint256) {
        return _last[target];
    }
...
*/
        ...
        returns (bool)
    {
        ...
        if (rare) {
            uint256 level = _amounts.length - 1;
            require(_amounts[level] > 0, "Potion: not enough tokens");
            _amounts[level] = _amounts[level] - 1;
        }
        return true;
    }
...
*/
    function withdraw(address payable target, uint256 amount)
        external
        override
        onlyOwner
    {
        require(target.send(amount), "Potion: Failed to send Ether");
    }

    /**
@notice Receive the funds 
*/
