...
contract Flowers is Ownable, ERC1155 {
    ...
    uint256 public constant TIME_TO_STAKE = 5 minutes;
    ...
    function getClaimableCount(uint256 tokenId) public view returns (uint256) {
        (, uint256 stakedAt, , ) = staking.tokenStakedInfo(tokenId);
        NyoInfo memory nyoInfo = nyoInfos[tokenId];

        if (nyoInfo.lastFlowerId == MAX_FLOWER_ID) return 0;
        ...
        return timeCount > MAX_FLOWER_ID ? MAX_FLOWER_ID + 1 : timeCount;
    }
    ...
    function claim(uint256[] memory nyolingIds) public {
        for (uint256 i = 0; i < nyolingIds.length; i++) {
            ...
            require(owner != address(0), "Nyo is not staked");
            ...
            for (uint256 j = 1 + lastFlowerId; j <= count + lastFlowerId; j++) {
                uint256 flowerId = j % (MAX_FLOWER_ID + 1);
                _mint(msg.sender, flowerId, 1, "");
            }
    ...
    function burnUltimate(uint256[] memory nyolingIds) public {
        _burn(msg.sender, ULTIMATE_FLOWER_ID, nyolingIds.length);

        for (uint256 i = 0; i < nyolingIds.length; i++) {
            (address stakedOwner, , , ) = staking.tokenStakedInfo(
                nyolingIds[i]
            );
            require(
