...
contract Water is ERC20, Ownable {
    ...
    uint256 public partnershipsBalance;
    uint256 public autoBurnBalance;
    uint256 public maintenanceBalance;
    ...
    constructor () ERC20("Water", "H2O") {
        ...
        // Set the initial tax structures here.
        buyTax = Tax({
            devFee: 10,       // % taken for devFee
            partnerships: 10, // % taken for partnerships
            maintenance: 10,  // % taken for maintenance
            autoBurn: 10      // % taken for autoBurn
        });
    ...
    function setBuyTax(uint256 devFee, uint256 partnerships, uint256 maintenance) external onlyOwner {
        buyTax = Tax(devFee, partnerships, 0, maintenance);
    }

    function setSellTax(uint256 devFee, uint256 partnerships, uint256 autoBurn) external onlyOwner {
        sellTax = Tax(devFee, partnerships, autoBurn, 0);
    }
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal override {
        // Apply maxWalletAmount check, unless the operation involves the contract owner, the contract itself, or Uniswap V2 pair
        if (
            recipient != owner() &&
            ...
            revert("Transfer amount would exceed the maxWalletAmount");
        }

        // Check if sender or recipient is the contract owner, the contract itself, or Uniswap V2 pair
        bool isExcluded = sender == owner() || sender == address(this) || sender == uniswapV2Pair ||
                          recipient == owner() || recipient == address(this) || recipient == uniswapV2Pair;

        if (!isExcluded) {
            uint256 totalTaxPercentage;
            uint256 tax;
            Tax memory currentTax;

            if (recipient == uniswapV2Pair) { // When selling
                currentTax = sellTax;
            } else if (sender == uniswapV2Pair) { // When buying
                currentTax = buyTax;
            } 

            totalTaxPercentage = currentTax.devFee + currentTax.partnerships + currentTax.autoBurn + currentTax.maintenance;
            tax = amount * totalTaxPercentage / 100;
            uint256 tokensToTransfer = amount - tax;

            // update balances
            devFeeBalance += tax * currentTax.devFee / totalTaxPercentage;
            partnershipsBalance += tax * currentTax.partnerships / totalTaxPercentage;
            autoBurnBalance += tax * currentTax.autoBurn / totalTaxPercentage;
            maintenanceBalance += tax * currentTax.maintenance / totalTaxPercentage;
