...
contract StakeManager is AccessControl, ReentrancyGuard, EIP712 {
    ...
    bytes32 public constant MULTIPLE_CLAIM_TYPEHASH =
    ...
    bytes32 public constant LOCK_REWARD_CLAIM_TYPEHASH =
    ...
    function getOldTier(address addr) public view returns (Tier) {
        return oldContract.getTier(addr);
    }
    ...
    function recoverFund(address _token, address _receiver) external onlyRole(BIG_GUARDIAN_ROLE) {
    ...
    function _paramsValidate(address _token, address _receiver) internal pure {
        require(_token != address(0) && _receiver != address(0), "Invalid address!");
    }
    ...
    function stake(uint256 amount) external payable nonReentrant {
        ...
        if (getTier(msg.sender) == Tier.NORANK && getOldTier(msg.sender) == Tier.NORANK) {
            require(gpoolInUSDC(oldStake.amount + amount) >= USDC_THRESHOLD, "minimum stake does not match");
        }
    ...
    function unstake(uint256 amount) external nonReentrant {
        ...
        oldStake.amount -= amount;
        if (oldStake.amount == 0 || gpoolInUSDC(oldStake.amount) < USDC_THRESHOLD) {
    ...
    function _verifyMultipleClaimProof(
        ...
        require(_currentTier <= MAX_TIER, "Invalid Tier");
