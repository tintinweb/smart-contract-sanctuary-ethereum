...
contract NormalPoolDNFTFactory is NoDelegateCall {
  ...
  function getPoolInfoFromAddress(
    ...
    for (uint256 i = 0; i < deployedLicenseSC.length; i++) {
      if (
        ISpanningERC721(deployedLicenseSC[i]).balanceOf(
  ...
  function getOffersFromBidder(
    address nftContractAddress,
    address bidder
  ) public view returns (NormalPoolNFT.Offer memory) {
    NormalPoolNFT nftContract = NormalPoolNFT(nftContractAddress);
    return nftContract.getOffersFromBidder(bidder);
  }
}
...
contract NormalPoolNFT is PoolBaseDerivativeNFT {
  ...
  function downgrade() public payable {
    for (uint256 i = 0; i < upgradedTokenList.length; i++) {
      if (block.timestamp > upgradeValidUntil[upgradedTokenList[i]]) {
        ...
        upgradedTokenList[i] = upgradedTokenList[upgradedTokenList.length - 1];
  ...
  function _unstake(address _user, uint256 _tokenId) internal {
    ...
    require(stakerAddress[_tokenId] == _user, "Not Owner");
    ...
    if (stakers[_user].amountStaked == 0) {
      ...
      require(index != stakerArray.length + 1, "failed.");
  ...
  function mintItem() public payable returns (uint256 tokenId) {
    require(!ownerToTokenIdMap[msg.sender].hasLicense, "Already own license.");
  ...
  function acceptOffer() public payable returns (uint256 offerId) {
    ...
    require(ownerToTokenIdMap[msg.sender].hasLicense, "No License Found");
  ...
  function _locateOffer(
    ...
    require(indexToFind != activeOfferIds.length + 1, "no offer");
  ...
  function getOffersFromBidder(
    ...
    for (uint i = 0; i < activeOfferIds.length; i++) {
      Offer memory offer = getOfferById[activeOfferIds[i]];
...
/*, IERC721Receiver*/
abstract contract PoolBaseDerivativeNFT is
