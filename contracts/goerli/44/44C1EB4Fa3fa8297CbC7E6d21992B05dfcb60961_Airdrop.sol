...
contract Airdrop is Ownable {
    ...
    function signatureClaim(bytes calldata signature, address _to, uint256 _amount ) external {
        ...
        bytes32 typedDataHash = toTypedDataHash(_to, _amount);
        (bytes32 r, bytes32 s, uint8 v) = _splitSignature(signature);

        // Revert if a malleable signature
        require(uint256(s) <= 0x7FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF5D576E7357A4501DDFE92F46681B20A0, "INVALID_SIGNATURE");
    ...
    function merkleClaim(bytes32[] calldata _proof, address _to, uint256 _amount) external {
        ...
        require(computedHash == merkleRoot, "INVALID_MERKLE_PROOF");

        claim(_to, _amount);
    }

    /// @dev Internal function to perfom claim action for the given recipient and amount
    /// @param _to The address which will receive PARA from a successful claim
    /// @param _amount The amount of PARA to be claimed
    function claim(address _to, uint256 _amount) internal {
        require(alreadyClaimed[_to] == false, "ALREADY_CLAIMED");
     ...
     * @param sig Signature that we are wish to decompose
     * @return r
     * @return s
     * @return v
     */
    ...
    /**
     * @notice Calculate a hash of two tree nodes in a more efficient way
     * @param a Tree node
     * @param b Tree node
     * @return value Hash of two nodes
     */
