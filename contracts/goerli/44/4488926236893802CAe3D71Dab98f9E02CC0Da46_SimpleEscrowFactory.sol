...
contract SimpleEscrowFactory {
    ...
    function createEscrow(address _payee, address _token, uint _amount) external returns(address simpleEscrowContract) {
        parameters = [msg.sender, _payee, llama, _token];
        simpleEscrowContract = address(new SimpleEscrowContract());
...
interface Factory {
    function parameters() external view returns(address[4] calldata);
}
...
contract SimpleEscrowContract {
    ...
    constructor() {
        factory = msg.sender;
        payer = Factory(msg.sender).parameters()[0];
        payee = Factory(msg.sender).parameters()[1];
        llama = Factory(msg.sender).parameters()[2];
        token = IERC20(Factory(msg.sender).parameters()[3]);
    }
