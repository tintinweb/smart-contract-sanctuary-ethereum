...
contract MiraOnChain is ERC721A, Ownable {
    ...
    modifier canMint(uint256 _quantity) {
        require(_quantity > 0, "Invalid input amount");
    ...
    function miralist_Mint(uint256 _quantity, bytes32[] calldata proof) external canMint(_quantity) {
        require(mintStatus == MintStatus.WHITELIST, "Whitelist mint is not live");
        require(!hasClaimedWhitelist[msg.sender], "You have claimed your free mint");
    ...
    function publicMint(uint256 _quantity) external payable canMint(_quantity) {
        require(mintStatus == MintStatus.PUBLIC, "Public mint is not live");
        require(msg.value >= (_quantity * price), "Incorrect eth amount");
    ...
    function createNewBlock(uint256 _quantity) private {
        for(uint256 i = 0; i < _quantity; i++) {
            ...
            Block memory newBlock = Block(
                ...
                "Colourful Mira bars, 100% uniquely generated on chain. Live on the Ethereum Blockchain!",
    ...
    function getHeight() private view returns(string memory, string memory, string memory) {
        ...
        uint256 value = uint256(keccak256(abi.encodePacked(block.timestamp, block.number, msg.sender))) % 4;
