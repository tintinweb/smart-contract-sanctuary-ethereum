...
contract ERC1155AssetStorage is StorageBase, IERC1155AssetStorage {
 //asdfasdf
    address private eRC1155AssetOwner; // owner of token contract; can withdraw minting fees from the proxy
    ...
    constructor(
        address _eRC1155AssetOwner,
        address _eRC1155AssetMinter,
        address _operations,
        uint256 _mintFee
    ) {
        eRC1155AssetOwner = _eRC1155AssetOwner;
    ...
    function getERC1155AssetOwner() external view override returns (address) {
        return eRC1155AssetOwner;
    }
    ...
    function setERC1155AssetOwner(address _newERC1155AssetOwner) external override requireOwner {
        eRC1155AssetOwner = _newERC1155AssetOwner;
    }
...
contract ERC1155Asset is ERC1155AssetAutoProxy, IERC1155Asset {
 //asdfasdf
    ...
    modifier onlyERC1155AssetOwner() {
        require(
            msg.sender == eRC1155AssetStorage.getERC1155AssetOwner(),
    ...
    modifier onlyOwnerOrOperations() {
        require(
            msg.sender == eRC1155AssetStorage.getERC1155AssetOwner() ||
                msg.sender == eRC1155AssetStorage.getOperations(),
            'ERC1155Asset: FORBIDDEN'
        );
        _;
    }
    ...
    constructor(
        ...
        address _operations,
        uint256 _mintFee
    ) ERC1155AssetAutoProxy(_proxy, address(this)) {
        eRC1155AssetStorage = new ERC1155AssetStorage(_owner, _minter, _operations, _mintFee);
    }

    // Governance functions

    // This function allows to set sporkProxy address after deployment in order to enable upgrades
    function setSporkProxy(address payable _sporkProxy) external onlyERC1155AssetOwner {
    ...
    function implMint(
        ...
        require(
            caller == eRC1155AssetStorage.getERC1155AssetOwner() || // owner is allowed
    ...
    function implMintBatch(
        ...
        for (uint256 i = 0; i < ids.length; ++i) {
            require(
                caller == eRC1155AssetStorage.getERC1155AssetOwner() || // owner is authorized
    ...
    function eRC1155AssetOwner() external view override returns (address) {
        return eRC1155AssetStorage.getERC1155AssetOwner();
    }
    ...
    function setERC1155AssetOwner(address _newERC1155AssetOwner) external override onlyOwner {
        eRC1155AssetStorage.setERC1155AssetOwner(_newERC1155AssetOwner);
    }
...
interface IERC1155AssetStorage {
    function getERC1155AssetOwner() external view returns (address);
    ...
    function setERC1155AssetOwner(address _newERC1155AssetOwner) external;
...
contract GovernedERC1155 is Pausable, NonReentrant, GovernedContract, ERC2981, IGovernedERC1155 {
 //asdfasdf
    ...
    constructor(address _proxy) GovernedContract(_proxy) {
...
contract ERC1155AssetAutoProxy is GovernedERC1155 {
    constructor(address _proxy, address _implementation) GovernedERC1155(_proxy) {
        proxy = _proxy;
