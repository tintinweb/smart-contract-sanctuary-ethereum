...
contract RaffleWorldNft is Ownable, VRFConsumerBaseV2 {
    ...
    function changeRaffleWorldStorageAddress(address _newAddress) external onlyOwner {
        require(_newAddress != address(0), "Address cannot be address 0!");
        raffleStorage = IRaffleWorldStorage(_newAddress);
    }
    ...
    function withdrawOwnerFunds(uint256 _amount)
        external
        onlyOwner
    {
        _giveFunds(
...
interface IRaffleWorldStorage {
    ...
    /**
     * @dev used to set the value of all the prizes in the raffle expressed
     *  in the native currency of the blockchain
     */
    function SetRaffleTotalAmount(uint256 _raffleId, uint256 _totalAmount)
        external;

    /**
     * @dev returns the total value of all the prizes in the raffle expressed
     *  in the native currency of the blockchain
     */ 
    function GetRaffleTotalAmount(uint256 _raffleId)
        external
        view
        returns (uint256);
    ...
    /**
     * @dev returns the number of raffles 
     */
    function GetAllRafflesLength() external view returns(uint256);
