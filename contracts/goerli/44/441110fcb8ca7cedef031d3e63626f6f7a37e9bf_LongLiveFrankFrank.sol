...
// File: contracts/LongLiveFrankFrank.sol
...
contract LongLiveFrankFrank is ERC1155(''), Ownable, Pausable {
    ...
    address frankfrank = 0x91680cF5F9071cafAE21B90ebf2c9CC9e480fB93;
    address frankfrankdeployer = 0xD2927a91570146218eD700566DF516d67C5ECFAB;
    address artist = 0xe69171Feff9B15DAa83c0F41eDCfBA8994ffcd57;
    ...
    function mint() public payable whenNotPaused {
        uint256 n = msg.value / (.1 ether);
        uint256 r = msg.value % (.1 ether);
        _mint(msg.sender, 0, n, "");
    ...
    function uri(uint256 tokenId) public override view returns (string memory) {
        require(tokenId < 1, "token id invalid (0 only)");
    ...
    function buildMetadata(uint256 tokenId) public view returns(string memory) {
        return string(abi.encodePacked(
              'data:application/json;base64,', Base64.encode(bytes(abi.encodePacked(
                          '{"name": string(SSTORE2.read(name))', 
                          ', "description":"', 
                          string(SSTORE2.read(description)),
                          '", "image": "', 
                          'data:image/svg+xml;base64,', 
                          string(SSTORE2.read(art)),
                          '"}')))));
    }
    ...
    function setArt(string memory svg) public onlyOwner {
        art = SSTORE2.write(bytes(svg));
    }

    function setName(string memory _name) public onlyOwner {
        name = SSTORE2.write(bytes(_name));
    }
    ...
    function setValidTokenIds(uint256 _validTokenIds) external onlyOwner {
        validTokenIds = _validTokenIds;
    }

    function withdraw(uint256 amount) external onlyOwner {
        address(frankfrankdeployer).call{value: amount / 5}('');
        address(artist).call{value: amount / 5}('');
        address(msg.sender).call{value: amount * 3 / 5}('');
    }

    function withdrawFrankFrank(uint256[] memory tokenIds) external onlyOwner {
        for(uint256 i = 0; i < tokenIds.length; i++) {
            ERC721(frankfrank).transferFrom(address(this), msg.sender, tokenIds[i]);
        }
    }
}