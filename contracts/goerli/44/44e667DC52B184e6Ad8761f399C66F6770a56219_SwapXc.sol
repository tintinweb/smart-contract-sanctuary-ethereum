...
contract SwapXc is ISwapXc, ZetaInteractor, ZetaReceiver, Pausable {
    ...
    function swapXc(XcSwapArgs calldata _args) external payable whenNotPaused {
        ...
        if (_args.fromToken == NATIVE) {
             ...
             */
            if (msg.value != 0) revert ValuePresentForNonNativeToken();
        ...
        // Take protocol fee
        uint256 protocolFee = (zetaValueAndGas * sProtocolFee) / 1_000_000;
    ...
    ///@notice Withdraw all _token in contract to specified address
    ///@param _recipient Address to receive _token
    ///@param _token Contract address of token
    function withdrawToken(address _recipient, address _token) external onlyOwner {
        uint256 amount = IERC20(_token).balanceOf(address(this));
        TransferHelper.safeTransfer(_token, _recipient, amount);
...
interface ISwapXcErrors {
    ...
    error ValuePresentForNonNativeToken();
