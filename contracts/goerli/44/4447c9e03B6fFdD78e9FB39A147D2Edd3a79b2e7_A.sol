...
6.  .env와 상호작용 후, interact.js 파일(크기 비교 함수 1번, setA() 1번[4,5,6 넣기], getA()1번 결과물 - 터미널), env 파일  + setA()에 대한 캡쳐 - 거래내역 7. 터미널에서 2번 지갑으로 돈 보내기 후, 터미널과 etherscan 결과 캡쳐
...
contract A {
    ...
    function setArray(uint _number) public returns(uint [] memory){
        array = new uint[](_number);
    ...
    function getArrayLength(uint[] memory _array) public view returns (uint) {
        _array = array;
