...
contract NumberGods is ERC721A, Ownable {
    ...
    function tokenURI(uint256 _tokenId) public view virtual override(ERC721A) returns (string memory) {
        ...
        string memory json = string(
            abi.encodePacked(
                '{"name": "NUMGOD #', Strings.toString(_tokenId), '",',
        ...
        return string(abi.encodePacked("data:application/json;base64,", jsonBase64Encoded));
    }
        ...
        returns (string memory)
    {
        ...
        string memory style = '';
        {
            style = string(abi.encodePacked('text{font-size:', fontSize, 'px;font-family:"a",sans-serif;fill:#fff;text-shadow:4px 4px 60px #000;}'));
        }
        ...
        //     Base64.encode(bytes(
                abi.encodePacked(
                    '<svg viewBox="0 0 1024 1024" xmlns="http://www.w3.org/2000/svg" style="background:#000">',
                    '<defs><style>',
                    '@font-face {',
                    'font-family: "a";',
                    ...
                    '</style></defs>',
                    '<style>', style, '</style>',
                    '<rect width="1024" height="1024" fill="url(#bg-image)" />',
                    '<pattern id="bg-image" patternUnits="userSpaceOnUse" width="1024" height="1024">',
                    '<image xlink:href="https://picsum.photos/1024/1024" x="0" y="0" width="1024" height="1024" />',
                    '</pattern>',
    ...
    function formatScientificNotation(uint64 num) public pure returns (string memory) {
        uint64 mantissa = num;
        uint64 exponent = 0;
        while(mantissa >= 10) {
            exponent++;
