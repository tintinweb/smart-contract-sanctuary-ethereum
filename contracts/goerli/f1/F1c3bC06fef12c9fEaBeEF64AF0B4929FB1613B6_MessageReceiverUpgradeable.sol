...
contract MessageReceiverUpgradeable is OwnerWithdrawable {
	...
	event MessageWithTransferFailed(address sender, IERC20Upgradeable token, uint256 amount, uint64 srcChainId, bytes message, address executor);
	...
	function executeMessageWithTransfer(
		...
		try dstChainPayment.payFromSourceChain(token, amount, message) {
			...
			emit MessageWithTransferFailed(sender, token, amount, srcChainId, message, _executor);
		}
