...
contract Swap is IStargateReceiver {
    ...
    event Swapped(address swapTokenIn, uint256 swapTokenInAmount, address swapTokenOut, uint256 swapTokenOutAmount);
    ...
    constructor(address _stargateRouter, address _bridgeToken) {
    ...
    function swap(
        ...
        emit Swapped(_tokenIn, _tokenInAmount, bridgeTokenAddress, bridgeTokenOutAmount);

        return bridgeTokenOutAmount;
    }

    /// @param _bridgeTokenAmount The amount of bridgeToken to send accross the bridge
    /// @param _dstChainId The destination chain id
    /// @param _srcPoolId The source chain Stargate poolId
    /// @param _dstPoolId The destination chain Stargate poolId
    /// @param _bridgeTxnFee The fee for the bridge transaction
    /// @param _dstChainTo The bytes containing the toAddress
    /// @param _dstStargateComposedAddress The destination chain stargate contract address
    function bridge(
        uint256 _bridgeTokenAmount,
        uint16 _dstChainId,
        uint16 _srcPoolId, 
        uint16 _dstPoolId,
        uint64 _bridgeTxnFee,
        address _dstChainTo,
        address _dstStargateComposedAddress
    ) internal {
        // encode payload data to send to destination contract, which it will handle with sgReceive()
        bytes memory data = abi.encode(_dstChainTo);
        ...
        IStargateRouter(stargateRouterAddress).swap{value:_bridgeTxnFee}(
            ...
            abi.encodePacked(_dstStargateComposedAddress),    // destination address, the sgReceive() implementer
            data                                              // bytes payload
        );
    }
    ...
    function transfer(
        ...
        bridge(
            ...
            _bridgeTxn.srcPoolId, 
            _bridgeTxn.dstPoolId, 
            _bridgeTxn.fee, 
            _bridgeTxn.dstChainTo, 
            _bridgeTxn.dstStargateComposed
        );
    }
