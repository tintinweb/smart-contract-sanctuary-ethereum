...
contract Bridge is UUPSUpgradeable, Ownable, Initializable {
    ...
    function getUserRequests(address userAddress) external view returns(bytes32[] memory) {
        return users[userAddress].requests;
    }

    function getUserReqCount(address userAddress) external view returns(uint256) {
