...
contract test244 is ERC721A, ReentrancyGuard { 
...
event newkeeperset(address _newowner);
  ...
  constructor(address _owner) ERC721A("Test", "Test2") {
       owner = _owner;
  }
  ...
  function getOwnershipData(uint256 tokenId) public view returns (TokenOwnership memory)
  {
    ...
    function getRank(uint256 _tokenId) public view returns (string memory) { 
        uint256 current_unix = block.timestamp; //cache to save gas in the loop.
        TokenOwnership memory ownership = getOwnershipData(_tokenId);
        uint256 Time_held_for = current_unix - ownership.startTimestamp;
        string memory rank = '';
            if (Time_held_for <= 10) { //  < 6months
                rank = "Bronze";
            } 
            //  if (Time_held_for <= 15768096) { //  < 6months
            //     Ranking[i] = Rank.Bronze; 
            // } 
            if (Time_held_for > 10 && Time_held_for <= 31536192) { // 6m-1yr
               rank = "Silver"; 
            }
            //   if (Time_held_for > 15768096 && Time_held_for <= 31536192) { // 6m-1yr
            //     Ranking[i] = Rank.Silver; 
            // } // replace silver with this once finished test 
  ...
  // Note: Inital supply is 100 however max supply is not capped, owner can mint additional denizen. 
