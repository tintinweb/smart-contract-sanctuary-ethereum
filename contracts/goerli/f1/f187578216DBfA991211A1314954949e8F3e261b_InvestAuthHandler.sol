...
contract InvestAuthHandler is IXReceiver {
    ...
    event LogInvest(uint256 investAmount);
    ...
    constructor(address _connext, address _token, address _admin) {
        connext = IConnext(_connext);
        token = IERC20(_token);
        admin = _admin;
    }

    function setSourceChain(address _source, uint32 _originDomain) external {
        source = _source;
    ...
    function xReceive(
        ...
        (address depositContract, uint256 relayerFee) = abi.decode(
            _callData,
            (address, uint256)
        );
        ...
        doInvest(_origin, depositContract, _amount, relayerFee);
        investAmount += _amount;
        emit LogInvest(_amount);
    }
    ...
    function doInvest(
        ...
        bytes memory callData = abi.encode(investAmount);
