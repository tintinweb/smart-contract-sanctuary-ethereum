...
contract SingleAuction is OwnableUpgradeable, ERC721HolderUpgradeable, Signature {
     ...
     */
    function createAuction(bytes calldata _signature, address _collectionId, uint256 _tokenId, uint256 _startPrice, uint256 _startTime, uint256 _endTime) 
    ...
    {   
        ...
        IInvestNFT nft = IInvestNFT(_collectionId);

        // filter floor Price
        uint256 floorPrice = nft.getFloorPrice();
        require(floorPrice <= _startPrice, "startPrice have to be greater than floorPrice");
     ...
     */
    function finalizeAuction(bytes calldata _signature, uint256 _auctionId) public {
        confirmSignature(
            address(this), 
            msg.sender, 
            "finalize", 
        ...
        if(bidsLength == 0) {
            IInvestNFT(myAuction.collectionId).safeTransferFrom(address(this), myAuction.owner, myAuction.tokenId);
            ...
            uint256 totalAmount = lastBid.bidPrice;
            uint256 floorPrice = IInvestNFT(myAuction.collectionId).getFloorPrice();        
            if (floorPrice > totalAmount) {
            // cancel auction   

                // send token to last bidder
                if (myAuction.tokenAdr == address(0x0)) {
                    (bool result, ) = payable(lastBid.from).call{value: totalAmount}("");
                    require(result, "Failed to send coin to last bidder");
                } else {
                    IERC20 governanceToken = IERC20(myAuction.tokenAdr);            
                    require(governanceToken.transfer(lastBid.from, totalAmount), "refund to last bidder failed");
                } 
                ...
                uint256 _feeValue = totalAmount.mul(swapFee).div(PERCENTS_DIVIDER); 
                uint256 _sellerValue = totalAmount.sub(_feeValue);
                ...
                IInvestNFT(myAuction.collectionId).safeTransferFrom(address(this), lastBid.from, myAuction.tokenId);		
                auctions[_auctionId].active = false;

                emit AuctionFinalized(lastBid.from,totalAmount, myAuction);
            }
     ...
     * @param _auctionId uint256 ID of the created auction
     */
    function bidOnAuction(bytes calldata _signature, uint256 _auctionId, uint256 amount) external payable {
        ...
        if (myAuction.tokenAdr == address(0x0)) {
            require(amount >= floorPrice, "bid amount have to be grater than floor price");
            require(amount >= tempAmount, "bid amount have to be grater than previous price");
            ...
            // check if amount is greater than floor price  
            require(amount >= floorPrice, "bid amount have to be grater than floor price");
    ...
    modifier onlyTokenOwner(address _collectionId, uint256 _tokenId) {
        address tokenOwner = IInvestNFT(_collectionId).ownerOf(_tokenId);
