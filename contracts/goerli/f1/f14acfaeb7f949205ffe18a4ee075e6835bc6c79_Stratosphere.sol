    ...
    AccessControlUpgradeable
{
    ...
    CountersUpgradeable.Counter private _tokenIds; // SIMPLIFY!!!
    ...
    function mint() external returns (uint256) {
        ...
        if (balanceOf(_msgSender()) != 0) revert MemberAlreadyExists();
    ...
    function tokenIdOf(address _account) external view returns (uint256) {
        if (balanceOf(_account) != 0) {
            return tokenOfOwnerByIndex(_account, 0);
        }
        return
            IAuthenticationController(authenticationController)
                .associatedTokenOf(_account);
    }
    ...
    function setAuthenticationController(address _authenticationController)
        external
