...
interface IRandomGenerator {
    function sales(uint) external view returns (address);

    function seeds(address) external view returns (uint);

    function requestRandom(uint32 _callbackGasLimit)
        external
    ...
    VRFConsumerBaseV2
{
    ...
    {
        require(
            accessRegistry.hasRole(ADD_SEED_ROLE, msg.sender),
            "RG: FORBIDDEN"
        );
        require(seeds[msg.sender] == 0, "RG: ALREADY_REQUESTED");
    ...
    function fulfillRandomWords(uint _requestId, uint[] memory _randomWords)
        ...
        override
    {
        require(_randomWords.length == 1 && _randomWords[0] != 0, "RG: ZERO");
        address sale = sales[_requestId];
        require(seeds[sale] == 0, "RG: EXISTS");
        seeds[sale] = _randomWords[0];
    }
