...
contract MGLTH is ERC721A, Owned {
    ...
    function vandalize(uint token, string calldata media) external payable {
        require(msg.value >= vandalCost, "Insufficient funds");
        require(broadcastActive, "Broadcast is not active");
        require(ownerOf(token) == msg.sender, "You are not the owner of this token");
    ...
    function _isPrime(uint n) public pure returns (bool) {
        if (n <= 1) {
            return false;
        }
    ...
    /// @notice Function called by owner to update the vandalCost uint variable
    /// @param newVandalCost The new value for the vandalCost uint variable
