...
contract SeasonPassNFT is ERC721URIStorage, ERC721Royalty, Ownable {
    ...
    mapping (uint256 => EnumerableMap.AddressToUintMap) referees;
    ...
    {
        if (!referees[_referralId].contains(recipient)) {
            referees[_referralId].set(recipient, _count);
        } else {
            uint256 prevReferralMint = referees[_referralId].get(recipient);
            referees[_referralId].set(recipient, prevReferralMint + _count);
        }
        mintSeasonPassNFT(recipient, _count);
    }

    // CALLED FROM PUBLIC SITE
    function getReferees(uint256 _referralId) public view returns (Referee[] memory) {
        uint256 refereesLength = referees[_referralId].length();
        Referee[] memory _referees = new Referee[](refereesLength);
        for (uint256 i=0; i<refereesLength; i++) {
            address referee;
            uint256 count;
            (referee, count) = referees[_referralId].at(i);
            _referees[i] = Referee(referee, count);
        }
        return _referees;
    }
 ...
 * ```
 *
 * As of v3.0.0, only maps of type `uint256 -> address` (`AddressToUintMap`) are
 * supported.
 */
