...
contract MergeGators is
   ...
   /**
    ...
    function VRFMint(address to, uint256 _quantity) internal {
        ...
        uint moddedRng = random(to, _quantity) % MAX_CHANCE_VALUE;
    ...
    function random(address to, uint256 _quantity) internal view returns(uint256){
        uint256[] memory _randomWords = vrf_requests[lastRequestId].randomWords;
        return uint256(keccak256(abi.encodePacked(to, _quantity, _randomWords[0])));
    }
    ...
    function setMergeHub(address mergeHub_) external onlyOwner {
        if (mergeHub_ == address(0)) revert MintToZeroAddress();
        mergeHub = mergeHub_;
     ...
     */
     function approve(address to, uint256 tokenId) public override {
        ...
        if (_msgSender() != owner_)
            if (!isApprovedForAll(owner_, _msgSender())) {
