...
contract StakingContract is OwnableUpgradeable, ERC721HolderUpgradeable {
    ...
    mapping (address => mapping ( uint256 => uint256 ) ) private NappingStarted;
    mapping (address => mapping ( uint256 => uint256 ) ) private NappingTotal;
    ...
    function stakeKitties(address _collectionAddress, uint256[] memory kitties) external {
        ...
        for (uint256 i =0; i<arrayLength;) {
            ...
            // Add time track
            if (NappingTotal[_collectionAddress][localKitties[i]] > 0){
                if (lastStakedWallet[_collectionAddress][localKitties[i]] != msg.sender) {
                    NappingTotal[_collectionAddress][localKitties[i]] = 0;    
                }
            }
            NappingStarted[_collectionAddress][localKitties[i]] = block.timestamp;
        unchecked {
            i++;
        }
        }
    ...
    function unstakeKitties(address _collectionAddress, uint256[] memory kitties) external{
        ...
        for (uint256 j = 0; j <arrayLength;) {
            ...
            // time tracker
            NappingTotal[_collectionAddress][localKitties[j]]+= block.timestamp - NappingStarted[_collectionAddress][localKitties[j]];
            lastStakedWallet[_collectionAddress][localKitties[j]] = msg.sender;
    ...
    function getNappingTime(address[] calldata _collectionAddress, uint256[] calldata _tokenId) external view returns (uint256[] memory) {
        uint256[] memory tokenStakingTime = new uint256[](_tokenId.length);
        for (uint256 i=0; i<_collectionAddress.length; i++) {
            tokenStakingTime[tokenStakingTime.length] = NappingTotal[_collectionAddress[i]][_tokenId[i]];
        }
        return tokenStakingTime;
    }

    function napTotalTime(address _collectionAddress, uint256 tokenId)
        external
        view
        returns (
            uint256 current,
    ...
    {
        uint256 start = NappingStarted[_collectionAddress][tokenId];
        if (start != 0) {
            current = block.timestamp - start;
        }
        total = current + NappingTotal[_collectionAddress][tokenId];
    }
