...
contract Treasury is Ownable, ReentrancyGuard {
    ...
    function transferERC20(address from, address to, address currency, uint256 amount)
        external 
        notPaused
        nonReentrant
        currencySupported(currency)
    ...
    function _addCurrency(address currency) external onlyOwner notPaused {
        supportCurrency[currency] = true;
        emit AddCurrency(currency);
    }

    function _removeCurrency(address currency) external onlyOwner notPaused {
