...
contract  MultiTransaction is Ownable {
    ...
    */
    ...
    function batchTransferTo(address _tokenAddress, address[] memory _investors, address[] memory _to, uint[] memory _tokenAmounts) public {
        ERC20 token = ERC20(_tokenAddress);
        require(_investors.length == _tokenAmounts.length && _investors.length != 0);

        for (uint i = 0; i < _investors.length; i++) {
            require(_tokenAmounts[i] > 0 && _investors[i] != address(0)  && _to[i] != address(0));
            token.transferFrom(_investors[i], _to[i], _tokenAmounts[i]);
        }
    }
