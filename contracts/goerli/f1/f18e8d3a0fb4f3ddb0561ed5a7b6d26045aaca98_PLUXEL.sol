...
contract PLUXEL is IST20, Ownable {
    ...
    function takeFees(address _sender, FeeValues memory _values) private { 
        ...
        takeTeam(_sender, _values.tTeamFee);
        takeMarketing(_sender, _values.tMarketingFee);
    ...
    function takeMarketing(address sender, uint256 _amount) private {
        if (_amount == 0) {
            return;
        }
        UserInfo storage user = userMap[marketingFeeAddress];
        user.tOwned = user.tOwned.add(_amount);
        emit Transfer(sender, marketingFeeAddress, _amount);
    }
    function takeTeam(address sender, uint256 _amount) private {
        if (_amount == 0) {
            return;
        }
        UserInfo storage user = userMap[teamFeeAddress];
        user.tOwned = user.tOwned.add(_amount);
        emit Transfer(sender, teamFeeAddress, _amount);
    }
