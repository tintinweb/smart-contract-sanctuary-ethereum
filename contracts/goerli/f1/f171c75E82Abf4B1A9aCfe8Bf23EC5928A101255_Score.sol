...
contract Score
{
	...
	event Check(string checkType, bool valid, string reason);
    ...
    constructor(IVerifier _verifier, IListHash _listhash, iProver _prover) {
			verifier = _verifier;
			listhash = _listhash;
			prover = _prover;
    }
	...
	function syncScore(uint256 permalink, 	// permalink on our records
					   uint128 version,		// version on our records)
					   MPT.MerkleProof memory accountProof,
        			   MPT.MerkleProof memory storageProof) public
	{
		(bool valid, string memory reason) = prover.verifyStorage(accountProof, storageProof);
		emit Check("verifyStorage", valid, reason);
		if( valid ) 
		{
...
library RLPDecode {
    ...
    */
    ...
    function toAddress(RLPItem memory item) internal pure returns (address) {
		...
		uint160 adr = uint160(toUint(item));
        return address(adr);
    }
