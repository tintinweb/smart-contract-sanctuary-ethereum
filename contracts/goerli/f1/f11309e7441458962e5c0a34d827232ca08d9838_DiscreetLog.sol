...
contract DiscreetLog {
    ...
    event CloseDLC(string uuid, string asset, uint256 actualClosingTime);
    ...
    function closeDlc(string calldata _uuid) external {
        ...
        dlc.closingPrice = 1000.00;
        dlc.actualClosingTime = block.timestamp;
        removeClosedDLC(findIndex(_uuid));
        emit CloseDLC(_uuid, dlc.asset, block.timestamp);
    }
