...
interface IMarketplaceCore {
    function getBids(uint40 listingId) external view returns(Bid[] memory);
}

contract Frames is AdminControl, ICreatorExtensionTokenURI, ILazyDelivery, ILazyDeliveryMetadata {
    ...
    // Initialize refund floor to the max value of uint256
    uint256 private _refundFloor = MAX_UINT_256;
    ...
    function deliver(uint40 listingId, address to, uint256, uint24, uint256 payableAmount, address, uint256) external override returns(uint256 refund) {
        require(msg.sender == _marketplace && listingId == _listingId, "Invalid call data");

        // Set refund floor to minimum bid if uninitialized
        if (_refundFloor == MAX_UINT_256) {
            Bid[] memory bids = IMarketplaceCore(_marketplace).getBids(_listingId);
            for (uint256 i = bids.length; i > 0; i--) {
                if (bids[i - 1].amount < _refundFloor) {
                    _refundFloor = bids[i - 1].amount;
                }
            }
        }
        ...
        // Refund the difference between the minimum bid and the payable amount
        refund= payableAmount - _refundFloor;
    }
