...
interface ITuktu {
	...
	function InitializeTuktu(IXProgram _XProgram, IMatrix _Matrix, API _API) external;
...
abstract contract Account is IAccount, ReentrancyGuard, Ownable2Step {
	...
	address public MultiSig; // CF address and gov recover unsupported

	IXProgram public XPrograms;
	IMatrix public Matrixes;
	API public TAPI; // Tuktu API contract
	...
	function InitializeAccount() external notAccountInitialized onlyOwner {
		_InitAccount(CFID, MultiSig); // Init community fund
		XPrograms.InitializeXProgram(IAccount(address(this)), IBalance(address(this)), ITuktu(address(this)), TAPI);
	...
	modifier OnlyAccountOwner(uint _AID) {
		require(_isExisted(_AID) && msg.sender == AddressOfAccount(_AID), "account: not existed or owner");
		_;
	}
...
abstract contract Balance is IBalance, Account {
	...
	function InitializeBalance() external notBalanceInitialized onlyOwner {
	...
	function _DepositToken(uint _AID, IERC20 _Token, uint _Amount) internal returns (bool success_) {
	...
	function Withdraw(
		...
		require(AvailableToWithdrawn(_AID) >= _Amount, "Withdraw amount exceeds balance");
...
contract Tuktu is ITuktu, Balance {
	...
	function InitializeTuktu(IXProgram _XProgram, IMatrix _Matrix, API _API) external notTuktuInitialized onlyOwner {
		XPrograms = _XProgram;
		Matrixes = _Matrix;
		TAPI = _API;
		...
		returns (bool success_)
	{
		if (msg.value > 0) {
			...
			if (amounts[1] >= _Amount && _DepositToken(_AID, _Token, _Amount)) {
				// refund dust eth, if any -> uniswap.swapETHForExactTokens
	...
	function TransferToken(
		...
		Matrixes._ShareRewardCommunityFund(); // To you and other
		return super.TransferToken(_FromAccount, _ToAccount, _Token, _Amount);
	}
}