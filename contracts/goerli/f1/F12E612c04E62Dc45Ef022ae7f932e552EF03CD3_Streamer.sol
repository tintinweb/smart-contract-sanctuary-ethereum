...
contract Streamer is Ownable {
    ...
    function fundChannel() public payable {
        if (balances[msg.sender] != 0) revert channelRunning();
        balances[msg.sender] += msg.value;
        emit Opened(address(msg.sender), msg.value);
    }

    function timeLeft(address channel) public view returns (uint256) {
        require(canCloseAt[channel] != 0, "channel is not closing");
        return canCloseAt[channel] - block.timestamp;
        //return canCloseAt[channel] == 0 ? 0 : canCloseAt[channel] - block.timestamp;
    }
    ...
    function withdrawEarnings(Voucher calldata voucher) public {
        ...
        // require that the recovered signer has a running channel with balances[signer] > voucher.updatedBalance
        require(
            balances[signer] > voucher.updatedBalance,
            "Signer balance below voucher balance, no channel"
        );

        // calculate the payment when reducing balances[signer] to voucher.updatedBalance
        uint256 payment = balances[signer] - voucher.updatedBalance;

        // adjust the channel balance, and pay the contract owner. (Get the owner address with the `owner()` function)
        balances[signer] -= voucher.updatedBalance;
