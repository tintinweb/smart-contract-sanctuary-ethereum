...
contract InvoiceFactory is Invoice, VerifySignature, AdminOnly {
    ...
    mapping(uint256 => InvoiceStruct) public invoices; // To store the details of each invoice
    ...
    modifier paid(uint256 _id) {
        require(!invoices[_id].paid, "InvoiceStruct already paid");
        _;
    }
...
function updateValidatorContract(IValidatorNFT _validatorContract) public onlyAdmin {
    ...
    function mintTheInvoice(uint256 _id, string memory uri)
        ...
        onlyInvoicer(_id)
    {
            require(!_exists(_id), "Token with the given ID already exists");
        InvoiceStruct storage invoice = invoices[_id];
        require(
    ...
    function closeDeal(uint256 _id) public onlyInvoicer(_id) validID(_id) {
        InvoiceStruct storage invoice = invoices[_id];
        require(!invoice.paid, "InvoiceStruct paid");
        require(verifySignatures(_id) == true, "Need 2 signatures");
