...
contract Erc1155{
    ...
    function safeBatchTransferFrom(address from, address to, uint256[] memory ids, uint256[] memory amounts, bytes calldata _data) external {
        require(msg.sender == from || isApprovedForAll(from, msg.sender), "msg.sender is not the operator or owner");
        require(to != address(0), "Address is 0");
        require(ids.length == amounts.length, "IDs an amount should be of the same length");
...
contract Leadguitars is Erc1155 {
    string public name;
    ...
    function mint(uint256 amount, string memory _uri) public {
        require(msg.sender != address(0), "you are minting to the Zero Address");
        tokenCount += 1;
