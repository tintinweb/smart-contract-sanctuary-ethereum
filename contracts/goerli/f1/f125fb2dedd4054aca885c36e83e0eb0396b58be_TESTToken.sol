...
contract TESTToken is ERC20, ERC20Burnable, Ownable {
    ...
    event Airdrop(address[] _recipients, uint256[] _amount);
    ...
    constructor(uint256 initialSupply) ERC20("Test", "TST") {
        ...
        _mint(address(this), airdropbalance);
    }
        ...
        returns (bool)
    {
        ...
        require(total <= balanceOf(address(this)), "Insufficient balance!");
        require(_recipients.length == _amount.length, "Lists not equal!");

        for (uint i = 0; i < _recipients.length; i++) {
            transfer(_recipients[i], _amount[i]);
        }

        airdropbalance -= total;
        emit Airdrop(_recipients, _amount);
        return true;
    }
}
