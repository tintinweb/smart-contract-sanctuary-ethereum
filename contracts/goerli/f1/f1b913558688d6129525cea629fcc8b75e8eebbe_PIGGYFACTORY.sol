...
contract PIGGYFACTORY is Ownable {
    ...
    function makePiggy(
        ...
        PIGGY piglet = new PIGGY(contract_name, contract_description, contract_image, external_url, msg.sender, amount, name, unlockTime, targetBalance);
        emit ContractCreated(address(piglet));
        contracts.push(address(piglet));
        return address(piglet);
    }
}

contract PIGGY is ERC1155, ERC1155Burnable, Ownable {
    ...
    ContractAttr public contract_attributes;
    uint public supply;
    ...
    constructor(
        ...
        string memory _name,
        uint unlockTime,
        uint256 _targetBalance) payable ERC1155("") {
            require(
            ...
            contract_attributes = ContractAttr(_contract_name, _contract_description, _image_url, _external_url);

            supply = amount;
            _mint(receiver, 0, amount, "");
            attributes = Attr(_name, unlockTime, _targetBalance, address(this));
    }

    receive() external payable {
        emit Received(msg.sender, msg.value);
        //attributes.current_balance = address(this).balance;
    }
    ...
    function payout() public {
        ...
        uint thisOwnerBalance = balanceOf(msg.sender, 0);
        ...
        uint payoutAmount = address(this).balance / supply * thisOwnerBalance;
        ...
        string memory json = Base64.encode(
            bytes(string( 
                abi.encodePacked(
                    ...
                    '{"trait_type": "Receive Address", "value": ', Utils.toAsciiString(attributes.receive_address), '},',
                    ']}'
                )
            ))
        );
    ...
    function _getSvg() internal view returns (string memory) {
        ...
        {
            markup = string(abi.encodePacked(markup, '</tspan></text><text class="b" x="180" y="260"><tspan>Piggy needs </tspan><tspan>'));
        }
           
        {
            // eth to go
            markup = string(abi.encodePacked(markup, Utils.decimalString((attributes.target_balance - address(this).balance) * 100 / 1 ether, 2, false )));
        }

        {
            markup = string(abi.encodePacked(markup, ' ETH</tspan></text><text class="b" x="180" y="280"><tspan>Piggy has </tspan><tspan>'));
        }
        
        {
            markup = string(abi.encodePacked(markup, Utils.decimalString(address(this).balance * 100 / 1 ether, 2, false)));
        }

        {
            markup = string(abi.encodePacked(markup, ' ETH</tspan></text><text class="b" x="165" y="335">Piggy needs ETH! Send to:</text><text class="c" x="165" y="355">'));
        }

        {
            markup = string(abi.encodePacked(markup, Utils.toAsciiString(attributes.receive_address)));
        }

        {
           markup = string(abi.encodePacked(markup, '</text></svg>')); 
        }
    ...
    function _createContractURI() internal view virtual returns (string memory) {
        return string(abi.encodePacked('data:application/json;base64,', Base64.encode(abi.encodePacked(
          '{"name": "', contract_attributes.name, '",',
              '"description": "', contract_attributes.description, '",',
              '"image": "', contract_attributes.image_url, '",',
              '"external_link": "', contract_attributes.external_url, '"',
          '}'
      ))));
    }
}
