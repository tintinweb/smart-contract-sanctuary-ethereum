...
contract Neverland{
    ...
    function call_proxy(uint256 begin,uint256 end,address destination,bytes memory data,uint256 value) external payable onlyowner{
        ...
        for (i; i <= end; ++i) {
            ...
            payable(proxy_address).transfer(value);
            assembly {
                let succeeded := call(
                    gas(),
                    proxy_address,
    ...
    function withdrawETH_from_proxy(uint256 begin,uint256 end)public {
        uint256 i=begin;
        for (i; i <= end; ++i){
            address proxy_address=contract_address[i-1];
            if (proxy_address.balance>0){
                payable(address(this)).transfer(proxy_address.balance);
            }  
        }
    }
