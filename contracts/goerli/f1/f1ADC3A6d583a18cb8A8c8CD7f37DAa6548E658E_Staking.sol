...
contract Staking is IStaking, IVotingWeightSource, AccessControlEnumerable {
    ...
    uint256 public constant MIN_LOCK = 10 minutes; // minimum staking lock
    ...
    constructor(
        ...
        /// @dev test purpose
        emission.rate = uint128(25000000 ether) / 52 weeks; // 2.5 % of total supply, and another 2.5% will be used for boosting
        emission.expiration = SafeCast.toUint128(block.timestamp + 52 weeks); // 1 year
    }
     ...
     *
     * @dev Removed access role for test purpose
     */
    function updateEmission(uint256 rate, uint256 expiration) external syncd {
     ...
     */
    function turnOffBoost(bool newBoostOn) external {
        require(boostOn != newBoostOn, "Staking: Already off");
        boostOn = newBoostOn;
