...
contract ETMUpgradeableV1 is Initializable, DefaultOperatorFiltererUpgradeable, ERC721Upgradeable, ERC721RoyaltyUpgradeable, ERC721EnumerableUpgradeable, OwnableUpgradeable  {
    ...
    mapping (address => ETMint) public allowList;

    uint256[24] __gap;

    function initialize() initializer public {
        __ERC721_init("Enter The Machine", "ETM");
        __Ownable_init();
        _setDefaultRoyalty(msg.sender, 750);

        ipfsHash = "QmPNWUFNpSHku4zgqrFGcGYhtdymfSgrhUAMoomqBdb6JE";
        bucket = "v1";
    ...
    function updateBucket(string memory _bucket) public onlyOwner {
        bucket = _bucket;
    }

    function mint(address _to) public payable returns (uint256) {
        uint256 price = allowList[_to].price;
        require(allowList[_to].qty>=1, "Not on allow list");
    ...
    function updateAllowList(bytes calldata data) external onlyOwner {
        address[] memory addresses;
        uint256[] memory prices;
        (addresses, prices) = abi.decode(data,(address[],uint256[]));

        for (uint256 i = 0; i < addresses.length; i++) {
            allowList[addresses[i]].qty = 1;
            allowList[addresses[i]].price = prices[i];
        }
    }
    ...
    function tokenURI(uint256 tokenId) public view virtual override (ERC721Upgradeable) returns (string memory) {
        _requireMinted(tokenId);
        string memory baseURI = _baseURI();
        return bytes(baseURI).length > 0 ? string(abi.encodePacked(baseURI, ipfsHash,"/", bucket,"/", tokenId.toString(),".json")) : "";
    }

    function contractURI() public pure returns (string memory) {
        return string(abi.encodePacked("data:application/json;base64,", Base64Upgradeable.encode(abi.encodePacked(
                '{',
                '"name": "Enter The Machine",',
                '"description": "*Enter The Machine* is a crowd-sourced, digital file portraiture project that aims to provide a new way of seeing our digital files, one that does justice to their uniqueness, the diversity of the data they contain, and the complexity by which they are structured.",',
                '"image": "https://enter-the-machine.nyc3.digitaloceanspaces.com/v1/v1/cover/_collection/ETM.png",',
                '"external_link": "https://enterthemachine.io",',
                '"seller_fee_basis_points": "750",',
                '"fee_recipient": 0xaC37e4f10F876D4Da96936531aE39403e6917D66"',
                '}'
            ))));
    }

    function getLastMintedTokenId() view external returns (uint256){
    ...
    function _beforeTokenTransfer(address from, address to, uint256 firstTokenId, uint256 batchSize) internal override(ERC721Upgradeable, ERC721EnumerableUpgradeable) {
        super._beforeTokenTransfer(from, to, firstTokenId, batchSize);
    }
    ...
    function supportsInterface(bytes4 interfaceId) public view override(ERC721RoyaltyUpgradeable, ERC721Upgradeable, ERC721EnumerableUpgradeable) returns (bool){
        return super.supportsInterface(interfaceId);
    }
