...
contract Indelible is ERC721AX, DefaultOperatorFilterer, ReentrancyGuard, Ownable {
    ...
    string[] private layerNames = ["Hat","Face","Arms","Torso","Legs","Max Life"];
    ...
    function tokenIdToHash(
        ...
        for (uint i = 0; i < tiers.length; i++) {
            ...
            uint traitIndex = rarityGen(traitRangePosition, i);
            hash[i] = traitIndex;
        }
        ...
        for (uint i = 0; i < tiers.length - 1; i++) {
            ...
            svgBytes.appendSafe(
                abi.encodePacked(
                    ...
                    positions[i],
                    '" x="160" fill="#964B00">',
                    values[i][thisTraitIndex],
                    '</tspan>'
                )
            );
        }
            
        svgBytes.appendSafe(abi.encodePacked("</text></svg>"));
    ...
    function setGameContractAddress(address contractAddress) external onlyOwner {
