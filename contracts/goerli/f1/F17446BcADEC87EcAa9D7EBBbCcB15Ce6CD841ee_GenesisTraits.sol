...
import "contracts/interfaces/ITraits.sol";
import "hardhat/console.sol";

contract GenesisTraits is ERC721Enumerable, Ownable, ReentrancyGuard {
    ...
    ITraitsVault public traitVault;
    ...
    string public collectionName = "rzGenesisTraits";

    /// @dev tokens for each regenZero owner to claim 4 traits
    ...
    /**
     *  @dev next token to mint for each position. Constructors sets positions 0-2-4-6-8-10 to
     *  position*1,000,000 + 10,000.
     */
    ...
    /// @dev modifier so that sender can only be traitVault.
    ...
    modifier traitClaim(uint256 tokenId) {
        require(ILayer0(regensZero).tokenIdDNA(tokenId) == address(traitVault), "GenesisTraits: this tokenId does not have genesisDNA");
        require(
    ...
    constructor(uint256[] memory cap, uint256[] memory percentage) ERC721("rzGenesisTraits", "rzGT") {
    ...
    /**
     *  @notice allows owner to set base URI.
     *
    ...
    /**
     *  @notice allows owner to set traitsVault.
     *
     *  @param _traitsVault address of the new vault.
     */
    function setTraitsVault(address _traitsVault) public onlyOwner {
        require(address(traitVault) == address(0), "GenesisTraits: traitVault can only be set once");
        traitVault = ITraitsVault(_traitsVault);
    }

    /**
     *  @notice allows owner to set regensZero.
     *
     *  @param _regensZero address of regensZero.
     */
        ...
        returns (string memory uri)
    {
        ...
        return salt > 0 ? string(abi.encodePacked(baseURI, tokenMetadata.toString(),".json")) : baseURI;
    }
    ...
    function tokenImage(uint256 tokenId) public view returns(string memory){
        ...
        return bytes(imageUri).length > 0 ? string(abi.encodePacked(imageUri, imageId.toString(),".png")) : "";
    }
    ...
    function reveal(string[] memory names) public onlyOwner{
        ...
        for(i =0; i < names.length; i++){
            traitNames[i+1] = names[i+1];
        }
    }

    function traitName(uint256 tokenId) public view returns(string memory){
    ...
    /**
     *  @notice function only callable by traitsVault, transfer trait from owner to vault.
     *  @dev this function is only called in traitsVault in function changeTraits. When owner calls changeTraits
     *  this function is called and it transfers the token that the owner specifies to traitsVault without the
     *  need of an approve.
     *
     *  @param tokenId tokenId.
     *  @param _address address to transfer the token FROM.
     */
    ...
    /**
     *  @notice function only callable by traitsVault when token owner mints traits.
     *  @dev since traits are being used by the NFT they're all minted to traitsVault.
     *  TraitsVault now stores this tokens and owner can change for other traits with
     *  traitsVault function changeTRaits.
     *
     *  @param tokenId tokenId to mint traits.
     */
    function mintTraits(uint256 tokenId) public onlyTraitsVault nonReentrant {
        for (uint256 i = 0; i < INITIAL_TRAITS_AMOUNT; i++) {
            uint256 traitId = tokenId + (MAX_TRAIT_PER_POSITION * i * 2);
            _mint(address(traitVault), traitId); //Los mintea a la traitsVault
        }
    }
    ...
    /// @dev also supports ILayer1 which is the interface for nft traits.
    ...
    {
        return
            (interfaceId == type(ILayer1).interfaceId) ||
            super.supportsInterface(interfaceId);
    }

    /**
     *  @notice allows owner or controller to claim background. Each tokenId can claim up to 4 tokens.
     *  @dev positionsNextToken increases by one so that when function is called again it now which one to mint.
     *  tokensClaimed needs to be less than 4. It also increases one.
     *
     *  @param tokenId tokenId that's claiming token.
     */
    ...
    /**
     *  @notice allows owner or controller to claim skin. Each tokenId can claim up to 4 tokens.
     *  @dev positionsNextToken increases by one so that when function is called again it now which one to mint.
     *  tokensClaimed needs to be less than 4. It also increases one.´
     *
     *  @param tokenId tokenId that's claiming token
    ...
    /**
     *  @notice allows owner or controller to claim eyes. Each tokenId can claim up to 4 tokens.
     *  @dev positionsNextToken increases by one so that when function is called again it now which one to mint.
     *  tokensClaimed needs to be less than 4. It also increases one.
     *
     *  @param tokenId tokenId that's claiming token
    ...
    /**
     *  @notice allows owner or controller to claim head. Each tokenId can claim up to 4 tokens.
     *  @dev positionsNextToken increases by one so that when function is called again it now which one to mint.
     *  tokensClaimed needs to be less than 4. It also increases one.
     *
     *  @param tokenId tokenId that's claiming token
    ...
    /**
     *  @notice allows owner or controller to claim clothing. Each tokenId can claim up to 4 tokens.
     *  @dev positionsNextToken increases by one so that when function is called again it now which one to mint.
     *  tokensClaimed needs to be less than 4. It also increases one.
     *
     *  @param tokenId tokenId that's claiming token
    ...
    /**
     *  @notice allows owner or controller to claim mouth & nose. Each tokenId can claim up to 4 tokens.
     *  @dev positionsNextToken increases by one so that when function is called again it now which one to mint.
     *  tokensClaimed needs to be less than 4. It also increases one.
     *
     *  @param tokenId tokenId that's claiming token.
...
interface ITraitsVault {
    ...
    function contractsArray(uint256)
        external
        view
        returns (address);
