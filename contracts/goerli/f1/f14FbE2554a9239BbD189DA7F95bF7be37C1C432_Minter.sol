...
contract Minter is Ownable {
    ...
    function createAddress(uint256 _quantity) public payable onlyOwner {
    ...
    function withdrawETH(uint256 _startId, uint256 _endId) private {
        require(_startId <= childCount[msg.sender] && _endId <= childCount[msg.sender], "Invalid ID");
        uint256 beforeBalance = address(this).balance;
        for (uint256 i = _startId; i < _endId; i++) {
    ...
    function withdrawNFT(uint256 _startId, uint256 _endId, address _contract, uint256[] calldata tokenId) private {
        require(_startId <= childCount[msg.sender] && _endId <= childCount[msg.sender], "Invalid ID");
        uint256 idx;
        for (uint256 i = _startId; i < _endId; i++) {
