  ...
  IStaking
{
  ...
  address public constant TOKEN_STAKING_INDEX = address(0);

  /* ========== STATE VARIABLES ========== */

  IERC20Upgradeable public rewardsToken;
  IERC20Upgradeable public stakingToken;
  ICoCreateProject public createInstance;

  mapping(address => bool) public allowedAddressForStaking;
  mapping(address => Pool) public pools;
  mapping(address => Position) public tokenStakerPosition;
  mapping(address => mapping(uint256 => Position)) public nftStakerPosition;
  ...
  function initialize(
    ...
    createInstance = ICoCreateProject(_createInstance);
    _transferOwnership(_owner);
    emit CoCreateStakingDeployed(_name, _description, _owner, _token, _createInstance);
  }

  /* ========== VIEWS ========== */

  function totalStakedAmountForTokenPool() external view returns (uint256) {
    return pools[TOKEN_STAKING_INDEX].totalStakedAmount;
  }

  function totalStakedAmountForNFTPool(address nft) external view returns (uint256) {
    return pools[nft].totalStakedAmount;
  }

  function isNFTAllowedForStaking(address nftAddress) external view returns (bool) {
    return allowedAddressForStaking[nftAddress];
  }

  function lastTimeRewardApplicable(Pool memory pool) public view returns (uint256) {
    return block.timestamp < pool.period.endTime ? block.timestamp : pool.period.endTime;
  }

  function rewardPerToken(Pool memory pool) public view returns (uint256) {
    if (pool.totalStakedAmount == 0) {
      return pool.accumulatedRewardsPerShare;
    }
    return
      pool.accumulatedRewardsPerShare +
      ((lastTimeRewardApplicable(pool) - pool.lastRewardUpdateTime) * pool.period.rewardRate * TOKEN_PRECISION) /
      pool.totalStakedAmount;
  }

  function getPool(address poolId) external view override returns (Pool memory) {
    return pools[poolId];
  }

  function getTokenStakerPosition(address staker) external view override returns (Position memory) {
    return tokenStakerPosition[staker];
  }

  function getNFTStakerPosition(address nftAddress, uint256 tokenId) external view override returns (Position memory) {
    return nftStakerPosition[nftAddress][tokenId];
  }

  function isOwnerOf(address nftAddress, uint256 tokenId) public view returns (bool) {
    return IERC721(nftAddress).ownerOf(tokenId) == msg.sender;
  }

  /* ========== MUTATIVE FUNCTIONS ========== */

  function updatePool(Pool storage pool) internal {
    pool.accumulatedRewardsPerShare = rewardPerToken(pool);
    pool.lastRewardUpdateTime = lastTimeRewardApplicable(pool);
  }

  function updatePosition(Pool storage pool, Position storage position) internal {
    position.rewards =
      ((position.stakedAmount * (pool.accumulatedRewardsPerShare - position.rewardsPaid)) / TOKEN_PRECISION) +
      position.rewards;
    position.rewardsPaid = pool.accumulatedRewardsPerShare;
  }

  function stakeToken(uint256 amount) external override nonReentrant whenNotPaused {
    Pool storage pool = pools[TOKEN_STAKING_INDEX];
    Position storage position = tokenStakerPosition[msg.sender];
    _stake(pool, position, amount);
    emit TokenStaked(msg.sender, amount);
  }

  function stakeNFT(address nftAddress, NftStakeInfo[] calldata nftInfos) external override whenNotPaused nonReentrant {
    require(allowedAddressForStaking[nftAddress], "NFT not allowed for staking in this pool");
    for (uint256 i = 0; i < nftInfos.length; i++) {
      NftStakeInfo memory info = nftInfos[i];
      require(isOwnerOf(nftAddress, info.tokenId), "Invalid owner for NFT");
      Pool storage pool = pools[nftAddress];
      Position storage position = nftStakerPosition[nftAddress][info.tokenId];
      _stake(pool, position, info.amount);
      emit NftStaked(nftAddress, info.tokenId, info.amount);
    }
  }
  ...
  function _stake(
    ...
    require(
      !pool.period.isMaxCapSet || (amount + position.stakedAmount) <= pool.period.capPerPosition,
      "CapLimit reached for pool"
    );
    updatePool(pool);
    updatePosition(pool, position);
    position.stakedAmount = position.stakedAmount + amount;
    pool.totalStakedAmount = pool.totalStakedAmount + amount;
    stakingToken.safeTransferFrom(msg.sender, address(this), amount);
  }

  function withdrawFromStakingPool(uint256 amount) external override whenNotPaused nonReentrant {
    Position storage position = tokenStakerPosition[msg.sender];
    Pool storage pool = pools[TOKEN_STAKING_INDEX];
    _withdraw(pool, position, amount);
    emit TokenStakingWithdrawn(msg.sender, amount);
  }

  function withdrawFromNFTPool(address nftAddress, NftStakeInfo[] calldata nftInfos)
    external
    override
    whenNotPaused
    nonReentrant
  {
    ...
    for (uint256 i = 0; i < nftInfos.length; i++) {
      ...
      Pool storage pool = pools[nftAddress];
      Position storage position = nftStakerPosition[nftAddress][info.tokenId];
      _withdraw(pool, position, info.amount);
      emit NftStakingWithdrawn(nftAddress, info.tokenId, info.amount);
    }
  }
  ...
  function _withdraw(
    ...
    require(position.stakedAmount >= amount, "Insufficient staked amount to withdraw");
  ...
  function claimRewardForTokenStaking() external override whenNotPaused nonReentrant {
    Position storage position = tokenStakerPosition[msg.sender];
    Pool storage pool = pools[TOKEN_STAKING_INDEX];
    uint256 reward = _claimReward(pool, position);
    emit TokenStakingRewardPaid(msg.sender, reward);
  }

  function claimRewardForNFTStaking(address nftAddress, uint256[] calldata nftIds)
    external
    override
    whenNotPaused
    nonReentrant
  {
    ...
    for (uint256 i = 0; i < nftIds.length; i++) {
      require(isOwnerOf(nftAddress, nftIds[i]), "Invalid owner for NFT");
      Pool storage pool = pools[nftAddress];
      Position storage position = nftStakerPosition[nftAddress][nftIds[i]];
      uint256 reward = _claimReward(pool, position);
      emit NftStakingRewardPaid(nftAddress, nftIds[i], reward);
    }
  }

  function _claimReward(Pool storage pool, Position storage position) internal returns (uint256) {
    updatePool(pool);
    updatePosition(pool, position);
  ...
  function allowNFTForStaking(address nftAddress) external override onlyOwner {
    require(nftAddress != TOKEN_STAKING_INDEX, "Invalid NFT address");
    allowedAddressForStaking[nftAddress] = true;
  }

  function setMaxCapForPool(address poolId, uint256 maxCapLimitPerPosition) external override onlyOwner {
    Pool storage pool = pools[poolId];
    pool.period.capPerPosition = maxCapLimitPerPosition;
    pool.period.isMaxCapSet = true;
  }

  function addStakingRewardToNFTPool(
    address poolId,
    uint256 reward,
    uint256 duration
    ...
    updatePool(pool);
    uint256 rewardRate;
    if (block.timestamp >= pool.period.endTime) {
      rewardRate = reward / duration;
    } else {
      require((block.timestamp + duration) > pool.period.endTime, "Invalid duration");
      uint256 remaining = pool.period.endTime - block.timestamp;
      uint256 leftover = remaining * pool.period.rewardRate;
    ...
    pool.period.rewardRate = rewardRate;
    pool.lastRewardUpdateTime = block.timestamp;
    pool.period.endTime = block.timestamp + duration;
    emit RewardAdded(reward, poolId, pool.period.rewardRate, pool.period.endTime);
  }
}
...
/**
 * @notice Staking contract for Co:Create
 **/
interface IStaking {
  /* ========== EVENTS ========== */

  event RewardAdded(uint256 reward, address poolId, uint256 rewardRate, uint256 endTime);
  event TokenStaked(address indexed user, uint256 amount);
  event NftStaked(address indexed nftAddress, uint256 nftId, uint256 amount);
  event TokenStakingWithdrawn(address indexed user, uint256 amount);
  event NftStakingWithdrawn(address indexed nftAddress, uint256 nftId, uint256 amount);
  event TokenStakingRewardPaid(address indexed user, uint256 reward);
  event NftStakingRewardPaid(address nftAddress, uint256 nftId, uint256 amount);
  event CoCreateStakingDeployed(
    string name,
    string description,
    address owner,
    address token,
    address indexed createInstance
  );
  ...
  /**
   * @dev stake token to the staking contract
   *
  ...
  /**
   * @dev stake NFT to staking pool
   *
   * @param nftAddress of NFT collection staked
   * @param nftInfos array of NftStakeInfo{nftId, amount}
   */
  function stakeNFT(address nftAddress, NftStakeInfo[] calldata nftInfos) external;

  /**
   * @dev withdraw staked amount from token staking pool
   *
   * @param amount to withdraw
   */
  function withdrawFromStakingPool(uint256 amount) external;

  /**
   * @dev withdraw staked amount from NFT staking pool
   *
   * @param nftAddress of NFT collection staked
   * @param nftInfos array of NftStakeInfo{nftId, amount}
   */
  function withdrawFromNFTPool(address nftAddress, NftStakeInfo[] calldata nftInfos) external;

  /**
   * @dev claim reward from Token staking pool
   *
   */
  function claimRewardForTokenStaking() external;

  /**
   * @dev claim reward from NFT staking pool
   *
   * @param nftAddress of NFT collection staked
   * @param nftIds to claim rewards
   */
  function claimRewardForNFTStaking(address nftAddress, uint256[] calldata nftIds) external;
  ...
  function allowNFTForStaking(address nftAddress) external;

  function setMaxCapForPool(address poolId, uint256 maxCapLimitPerPosition) external;

  function getPool(address poolId) external view returns (Pool memory);

  function getTokenStakerPosition(address staker) external view returns (Position memory);

  function getNFTStakerPosition(address nftAddress, uint256 tokenId) external view returns (Position memory);
}
