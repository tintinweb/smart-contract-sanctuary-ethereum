...
contract PLZTST is ERC721URIStorage {
    ...
    address private teamWallet = 0xA6d7c9964225a80e188180f2fb0e0BCD0b670D71;
    bytes32 public merkleRoot = 0x8ef4d49a6ef0b4c800cdfd29ba21c6a8ad33258413181b2992e864155a1afb7a;
    

    uint256 currentInternalIndex = 0;
    event Log(string lmsg);

    constructor() ERC721("Pillz Test Token Name 4", "PLZTST4"){
    ...
    function mint(bytes32[] calldata _merkle) public returns(string memory){
        ...
        if (supplyCount > 0) {
            if (mintedCount < supplyCount) {
                ...
                // uint rndNum = rnd(0, 8);
                // bool met = false;
                // bool erroredst = false;
                // uint256 scope_iterator = 0;
                uint target_index = ipfslinks.length-1;

                // while (scope_iterator <= supplyCount) {
                //     if (bytes(indexminted_to_ipfs_map[scope_iterator]).length == 0) {
                //         met = true;
                //         target_index = scope_iterator;
                //     }
                //     scope_iterator = scope_iterator + 1;

                //     if (scope_iterator >= supplyCount) {
                //         met = true;
                //         erroredst = true;
                //     }
                // }
                ...
                // if (erroredst) {
                //     emit Log("Not found nft candidate");
                // }
