...
import "./externals/chainlink/AggregatorV2V3Interface.sol";
// FlagsInterface from Chainlink addresses SIP-76
import "./externals/chainlink/FlagsInterface.sol";
import "./interfaces/IExchangeCircuitBreaker.sol";

// https://docs.synthetix.io/contracts/source/contracts/exchangerates
contract ExchangeRates is Owned, MixinSystemSettings {
    ...
    function rateAndUpdatedTime(bytes32 currencyKey) external view returns (uint256 rate, uint256 time) {
        IExchangeRates.RateAndUpdatedTime memory rateAndTime = _getRateAndUpdatedTime(currencyKey);
    ...
    function lastRateUpdateTimesForCurrencies(bytes32[] calldata currencyKeys) external view returns (uint256[] memory) {
    ...
    // function effectiveAtomicValueAndRates(bytes32, uint256, bytes32) external view returns (uint256, uint256, uint256, uint256) {
    //     _notImplemented();
    // }
    ...
    /// @return rates a list of rates
    /// @return times a list of times
        ...
        returns (uint256[] memory rates, bool anyRateInvalid)
    {
        ...
        for (uint256 i = 0; i < currencyKeys.length; i++) {
            ...
            IExchangeRates.RateAndUpdatedTime memory rateEntry = _getRateAndUpdatedTime(currencyKeys[i]);
    ...
    function getFlagsForRates(bytes32[] memory currencyKeys) internal view returns (bool[] memory flagList) {
        ...
        if (_flags != FlagsInterface(address(0))) {
            ...
            for (uint256 i = 0; i < currencyKeys.length; i++) {
                _aggregators[i] = aggregators[currencyKeys[i]];
            }
    ...
    function _formatAggregatorAnswer(bytes32 currencyKey, int256 rate) internal view returns (uint256) {
        require(rate >= 0, "Negative rate not supported");
        if (currencyKeyDecimals[currencyKey] > 0) {
            uint256 multiplier = 10**uint256(SafeMath.sub(18, currencyKeyDecimals[currencyKey]));
            return uint256(uint256(rate).mul(multiplier));
        }
        return uint256(rate);
    }

    function _getRateAndUpdatedTime(bytes32 currencyKey) internal view returns (IExchangeRates.RateAndUpdatedTime memory) {
        // sUSD rate is 1.0
        if (currencyKey == sUSD) {
            return IExchangeRates.RateAndUpdatedTime({rate: uint216(SafeDecimalMath.unit()), time: 0});
        } else {
    ...
    function _getCurrentRoundId(bytes32 currencyKey) internal view returns (uint256) {
        if (currencyKey == sUSD) {
            return 0;
        }
        AggregatorV2V3Interface aggregator = AggregatorV2V3Interface(aggregators[currencyKey]);
    ...
    function _rateIsStale(bytes32 currencyKey, uint256 _rateStalePeriod) internal view returns (bool) {
    ...
    function _rateIsStaleAtRound(
        bytes32 currencyKey,
        uint256 roundId,
        uint256 _rateStalePeriod
    ) internal view returns (bool) {
        // sUSD is a special case and is never stale (check before an SLOAD of getRateAndUpdatedTime)
        if (currencyKey == sUSD) {
            return false;
        }
        (, uint256 time) = _getRateAndTimestampAtRound(currencyKey, roundId);
        return _rateIsStaleWithTime(_rateStalePeriod, time);
    }

    function _rateIsStaleWithTime(uint256 _rateStalePeriod, uint256 _time) internal view returns (bool) {
...
contract ExchangeRatesWithDexPricing is ExchangeRates {
    ...
    // bytes32 public constant override CONTRACT_NAME = "ExchangeRatesWithDexPricing";

    bytes32 internal constant SETTING_DEX_PRICE_AGGREGATOR = "dexPriceAggregator";

    constructor(address _owner, address _resolver) ExchangeRates(_owner, _resolver) {}
    ...
    function dexPriceAggregator() public view returns (IDexPriceAggregator) {
        return IDexPriceAggregator(flexibleStorage().getAddressValue(ExchangeRates.CONTRACT_NAME, SETTING_DEX_PRICE_AGGREGATOR));
    }
    ...
    function _getMinValue(uint256 x, uint256 y) internal pure returns (uint256) {
        return x < y ? x : y;
    }

    function _getMaxValue(uint256 x, uint256 y) internal pure returns (uint256) {
        return x > y ? x : y;
    }
    ...
    function _getPriceFromDexAggregator(
        ...
        uint256 result = _dexPriceDestinationValue(sourceEquivalent, destEquivalent, amount).mul(SafeDecimalMath.unit()).div(
            amount
        );
    ...
    function _dexPriceDestinationValue(
        ...
        uint256 sourceAmountInEquivalent = (sourceAmount.mul(10**uint256(sourceEquivalent.decimals()))).div(
            SafeDecimalMath.unit()
        );

        uint256 twapWindow = getAtomicTwapWindow();
        require(twapWindow != 0, "Uninitialized atomic twap window");
        uint256 twapValueInEquivalent = dexPriceAggregator().assetToAsset(
        ...
        return (twapValueInEquivalent.mul(SafeDecimalMath.unit())).div(10**uint256(destEquivalent.decimals()));
    }
    ...
    function synthTooVolatileForAtomicExchange(bytes32 currencyKey) external view returns (bool) {
        ...
        uint256 considerationWindow = getAtomicVolatilityConsiderationWindow(currencyKey);
        uint256 updateThreshold = getAtomicVolatilityUpdateThreshold(currencyKey);
        ...
        uint256 considerationWindowStart = block.timestamp.sub(considerationWindow);
        uint256 roundId = _getCurrentRoundId(currencyKey);
        for (updateThreshold; updateThreshold > 0; updateThreshold--) {
            (uint256 rate, uint256 time) = _getRateAndTimestampAtRound(currencyKey, roundId);
