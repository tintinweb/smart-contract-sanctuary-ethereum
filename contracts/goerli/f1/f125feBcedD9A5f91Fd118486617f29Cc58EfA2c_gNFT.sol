    ...
    IgNFT
{
    ...
    function mint(uint256 tokenId) external whenNotPaused nonReentrant {
    ...
    function _checkUserMarkets(address[] memory markets) internal view {
        IComptroller comptroller = TONPOUND_COMPTROLLER;
        for (uint256 i; i < markets.length; ) {
            if (!comptroller.checkMembership(msg.sender, markets[i])) revert InvalidMarket();
            unchecked {
                i++;
            }
        }
    ...
    function _checkLiquidity(address[] memory markets, TokenType tokenType) internal view {
        if (markets.length == 0) {
        ...
        for (uint256 i; i < markets.length; ) {
            totalValue += _quoteLiquiditySingle(
                markets[i],
                oracle,
                IERC20Upgradeable(markets[i]).balanceOf(msg.sender),
                false
            );
...
interface IgNFT {
    ...
    /// @param tokenId       tokenId to be minted, must be non-existing
