...
contract MoonFlip is Ownable, ReentrancyGuard {
    ...
    uint256[] betAmounts = [0.05 ether, 0.1 ether, 0.5 ether, 1 ether, 2.5 ether, 5.0 ether];
    ...
    function checkForFee(uint256 num) public pure returns (uint256) {
        uint256 value = num * numerator;
        value /= denominator;
        return num + value;
    }

    function getFee(uint256 num) public pure returns (uint256) {
        uint256 value = num * numerator;
        value /= denominator;
        return value;
    }
