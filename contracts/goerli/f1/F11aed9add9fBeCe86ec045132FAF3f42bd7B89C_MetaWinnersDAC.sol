 ...
 * thus preventing token holders from selling their NFTs on non-trusted marketplaces.
 * In this implementation, it must be used in conjunction with another smart-contract that handles the addresses list.
 */
abstract contract ERC721RestrictedApprovals is ERC721Timestamp, Ownable {
    ...
    modifier approvalRestrictions(address approvee) virtual {
        require(
            IAddressList(approveeListAddress).isAllowed(approvee) // The check is outsourced to {AddressList}.
    ...
    /**
     * @dev Sets the address of the contract handling the list of allowed/denied approvees
     */
    function setApproveeListAddress(address contractAddress) external virtual onlyOwner {
     ...
     */
    function freezeApproveeListAddress() external virtual onlyOwner {
