...
contract Create2ContractFactory {
    ...
    */
    function deployLargeContract(
        ...
        bytes memory data = toBytes(bytecode, salt, anticipatedAddress, nonredeployable);
        bytes memory reason = interaction(payable(address(this)), data);
        assembly {
            deployedAddress := reason // Convert reason to address.
        }
        return deployedAddress;
    }
    ...
    */
    function interaction(
        ...
        if (data.length > 0 && !isContract(account)) revert(); // Must be a contract.
        ...
        if (!success) { // If not successful.
            if (reason.length == 0) { // If no reason.
                revert UNABLE_TO_TRANSFER_ETHER_OR_INTERACT_WITH_CONTRACT(account); // Generic error.
            } else {
                assembly {
                    revert(add(32, reason), mload(reason)) // Error from contract.
                }
            }
        }
        return _reason;
...
/// @notice Unable to transfer ether or interact with contract.
error UNABLE_TO_TRANSFER_ETHER_OR_INTERACT_WITH_CONTRACT(address account);
