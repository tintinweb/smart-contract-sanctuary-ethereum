...
contract VoxToVFXCollectionFactory is ICollectionFactory {
  ...
  modifier onlyAdmin() {
    require(rolesContract.isAdmin(msg.sender), "VTX: Caller does not have the Admin role");
    _;
  }
  ...
  function createCollection(
    ...
    require(bytes(symbol).length > 0, "VTX: Symbol is required");
  ...
  function _updateRolesContract(address _rolesContract) private {
    require(_rolesContract.isContract(), "VTX: RolesContract is not a contract");
  ...
  function _updateImplementation(address _implementation) private {
    require(_implementation.isContract(), "VTX: Implementation is not a contract");
  ...
  function _updateProxyCallContract(address _proxyCallContract) private {
    require(_proxyCallContract.isContract(), "VTX: Proxy call address is not a contract");
