...
contract NyxGovernanceProposal is NyxProposal, INyxGovernanceProposal {
    ...
    uint256 public constant nparams = 3;
        ...
        returns (GovernanceProposalParams memory)
    {
        ...
        {
            (address param1, uint256 param2, string memory param3) = abi.decode(params, (address, uint256, string));
            return GovernanceProposalParams(param1, GovernanceAddressAction(param2), param3);
        }
        else
        {
    ...
    {
        return abi.encode(proposal.ambassadorAddress, proposal.action, proposal.socialNetworkLink);
    }
        ...
        returns (bytes memory)
    {
        ...
        GovernanceProposal memory proposal = GovernanceProposal(proposalId, parsedParams.ambassadorAddress, parsedParams.action,
            parsedParams.socialNetworkLink);
        ...
        returns (bytes[nparams][] memory props)
    {
        ...
        for (uint256 idx = 0; idx < proposalBytesArr.length; idx++)
        {
            ...
            props[idx] = [abi.encode(params.ambassadorAddress), abi.encode(params.action), abi.encode(params.socialNetworkLink)];
        }
    }
