...
contract WaterMan is ERC721A, Ownable {
    using Strings for uint256;

    // States
    bool public whiteListIsSaleActive = false;
    bool public publicIsSaleActive = false;
    bool public _revealed = false;

    // Sales Parameters
    uint256 public constant MAX_SUPPLY = 10000;
    uint256 public whiteListMintPrice = 100000000000000;
    ...
    function whiteListMint(uint256 quantity, bytes32[] calldata proof)
        ...
        payable
    {
        require(whiteListIsSaleActive, "White list sale not start!");
        _quantityVerify(quantity);
        require(
            balanceOf(_msgSender()) + quantity <= maxBalance,
            "Sale would exceed max balance"
        );
        require(verify(_msgSender(), proof), "caller is not in whitelist");
        require(
            quantity * whiteListMintPrice <= msg.value,
            "Not enough ether sent"
        );
        require(quantity <= maxMint, "Exceeded max per mint");

        _purchaseMint(_msgSender(), quantity);
    }

    function mint(uint256 quantity) external payable {
        require(publicIsSaleActive, "Public sale not start!");
        _quantityVerify(quantity);
        require(
            balanceOf(_msgSender()) + quantity <= maxBalance,
            "Sale would exceed max balance"
    ...
    function airdrop(address to, uint256 quantity) external onlyOwner {
        _quantityVerify(quantity);
        _purchaseMint(to, quantity);
    }

    function _quantityVerify(uint256 quantity) internal view {
    ...
    function _purchaseMint(address to, uint256 quantity) internal {
    ...
    function flipWhiteListSaleActive() external onlyOwner {
        whiteListIsSaleActive = !whiteListIsSaleActive;
    }

    function flipSaleActive() external onlyOwner {
        publicIsSaleActive = !publicIsSaleActive;
    }
