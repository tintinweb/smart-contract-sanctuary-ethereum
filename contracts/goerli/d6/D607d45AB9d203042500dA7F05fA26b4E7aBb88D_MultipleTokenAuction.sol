    ...
    function resolve(uint256 uid) public notPaused nonReentrant {
        ...
        if (item.highest_bidder != address(0)) {
            ...
            if (_royalty_receivers[item.token_address]) {
                ...
                payable(creator).send((item.highest_bid * royalty) / 1000);
                summ += royalty;
            }
            payable(_owner_of).send((item.highest_bid * 25) / 1000);
            summ += 25;
            payable(item.curator_address).send((item.highest_bid * item.curator_fee) / 1000);
            summ += item.curator_fee;
            payable(item.owner_of).send((item.highest_bid - ((item.highest_bid * summ) / 1000)));
