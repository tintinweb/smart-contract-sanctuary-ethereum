...
contract HyperbolicProtocol is IHyperbolicProtocol, UniswapV3FeeERC20 {
    ...
    returns (uint256 lendPoolETHBal, uint256 marketCapETH)
  {
    ...
    marketCapETH = _token0 == address(this)
      ? (totalSupply() * _priceX96) / 2 ** 96
      : (totalSupply() * FixedPoint96.Q96 ** 2) / _priceX96 / 2 ** 96;
  }
  ...
  function lpCreatePool(
    ...
    // create UniswapV3Pool instance
    (address token0, address token1) = _getToken0AndToken1();
    // if the token placements are different than what we expect,
    // invert the price since it's expected token1 to be WETH9
    if (token0 != address(this)) {
      _initialPriceX96 = (FixedPoint96.Q96 ** 2) / _initialPriceX96;
    }
  ...
  PeripheryImmutableState
{
  ...
  function _createLiquidityPool(
    ...
    for (uint256 _i = 0; _i < _lpPoolFees.length; _i++) {
      require(_lpPoolFees[_i] != _fee, 'CREATEPOOL: already created');
    }
    _lpPoolFees.push(_fee);
