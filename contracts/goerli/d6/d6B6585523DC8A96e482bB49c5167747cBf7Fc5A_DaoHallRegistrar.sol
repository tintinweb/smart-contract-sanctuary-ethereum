...
contract DaoHallRegistrar is Ownable{
  ...
  function buySubnodeWhitelisted(string memory label, bytes32 parentHash, address resolver, uint64 ttl, uint32 fuses, uint64 data) external payable {
    uint32 duration = uint32(data);    
    // _checkBuyWhitelisted(label, parentHash, duration, uint32(data >> 32));
    _buy(label, parentHash, resolver, ttl, fuses, msg.value, duration);
  }
