...
contract Nakashi is ERC721URIStorage, RoyaltiesV2Impl,VRFConsumerBaseV2 {
    using Address for address;
 ...
 //   address public contractAddress;
    address public OWNER =  0xDb3360F0a406Aa9fBbBd332Fdf64ADb688e9a769;
    bytes4 private constant _INTERFACE_ID_ERC2981 = 0x2a55205a;
...
//
     ...
     ERC721("Nakashi Manga", "NM")
    {
        ...
        // Setting unityName, maxMintAmount, tokenId to start from
        setUnityDetails("common", 225, 275); // Black
        setUnityDetails("uncommon", 150, 125); // Purple
        setUnityDetails("rare", 75, 50); // White
        setUnityDetails("epic", 35, 15); // Red
        setUnityDetails("legendary", 15, 0); // Yellow
...
//
    ...
    modifier check(string memory unityName) {
        ...
        uint256 currentBalance = IERC721(address(this)).balanceOf(_msgSender());

        require(!paused, "Check: NFT contract is paused");
        require(
            currentBalance < maxMintAmount,
            "Check: Cannot mint more NFT's than MaxMintAmount"
        );
        require(
        ...
        require(msg.sender.balance >= cost, "Check: Low Balance!");
        require(
            msg.value == cost,
...
uint8 count=0;
 for (uint i = 0; i < packs.length; i++) {
                if (   unityDetails[packs[i]].itemsMinted <= unityDetails[packs[i]].maxSupplyOfNFT ) {
               count++;
            }
        }
...
//
    ...
    function mint() public payable {
          ...
          requestRandomWords();
s_randomWords=(s_randomWords% packs.length)+1;
       _Newmint(packs[s_randomWords]);
    ...
    function returnComic() external payable {
        require(msg.sender.balance >= returnCost, "Return Comic: Low Balance!");
        require(
            msg.value == returnCost,
            " Return Comic: You need to pay ether to pay for return fees!"
        );
        emit ReturnComic(msg.sender);
    }
    ...
    function setUnityDetails(
        ...
        uint16 id = unityDetails[_unityName].tokenId;
