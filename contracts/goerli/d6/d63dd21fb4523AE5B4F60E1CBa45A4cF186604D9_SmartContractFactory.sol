...
contract ERC1155MinterMultiple is ERC1155, ReentrancyGuard{
    bool isInitialized = false;
    using Strings for uint256;
    ...
    function uri(uint256 typeId) public view override returns (string memory) {
        ...
        require(bytes(baseURI).length > 0, "TokenUri: base URI is not set");
        return
        string(abi.encodePacked(baseURI,metaDataExt,typeId.toString()));
    }
}
...
import './ERC1155MinterMultiple.sol';

contract SmartContractFactory {

    CollectionNFT private CollectionNFTTemplate;
    ERC1155MinterMultiple private ERC1155MinterMultipleTemplate;

    event Created(address addr);

    constructor(){
        CollectionNFTTemplate = new CollectionNFT();
    }
    ...
    function createCollectionNFT(
        ...
        address payable clone = createClone(address(CollectionNFTTemplate));
        CollectionNFT(clone).initialise(_multisig,_owner, _baseURI, _name, _symbol);
        emit Created(clone);
    }

    function createERC1155MinterMultiple(
        address _multisig,
        address _owner,
        string memory _baseURI,
        string memory _name,
        ...
        address payable clone = createClone(address(ERC1155MinterMultipleTemplate));
        ERC1155MinterMultiple(clone).initialise(_multisig,_owner, _baseURI, _name, _symbol);
        emit Created(clone);
    }
