...
contract Wallet is ProxyUtils {
    ...
    function initialize(
        ...
        bool _subscriptionPaymentsRelayGuardianPermission
    ) external {
        ...
        for (uint256 i = 0; i < _signerConfigs.length; i++) {
            ...
            require(signer != address(0), "At least one signer is the zero address.");
            signerConfigs[signer] = _signerConfigs[i];
            emit SignerConfigChanged(signer, _signerConfigs[i]);
        }

        threshold = _threshold;
        relayerWhitelistTimelock = _relayerWhitelistTimelock;
        subscriptionPaymentsEnabled = _subscriptionPaymentsEnabled;
        subscriptionPaymentsRelayGuardianPermission = _subscriptionPaymentsRelayGuardianPermission;
    ...
    /// @param signatures Array of signatures sorted by address (ascending).
    function _validateSignatures(Signature[] calldata signatures, bytes32 signedDataHash, bool requireRelayGuardian) internal {
        // Input validation
        require(signatures.length > 0, "No signatures supplied.");

        // Append "Ethereum Signed Message" message prefix (for forwards compatibility/potential future security)
        signedDataHash = ECDSAUpgradeable.toEthSignedMessageHash(signedDataHash);
        
        // Loop through signers to tally votes (keeping track of last signer checked to look for duplicates)
        uint256 _votes = 0;
        address lastSigner;
        ...
        for (uint256 i = 0; i < signatures.length; i++) {
            ...
            // Check for duplicates in sorted array by ensuring each element is greater than the last
            require(signer > lastSigner, "Duplicate signer in signatures array (or signers array is not sorted correctly in ascending order by address).");
            lastSigner = signer;
            ...
            if (config.signingTimelock > 0) {
                bytes32 key = keccak256(abi.encode(sig));
                uint256 timestamp = pendingSignatures[key];
                pendingSignatures[key] = 0; // Get a gas refund
    ...
    ///     const A = 18700; // Maximum base gas recognized by `gasleft()` (i.e., excluding paymaster incentive) in `functionCall` with one signature (note that it does not matter if `value` > 0 or if the external call's target function is payable; also note that this estimate assumes that we do call `walletFactory.checkRelayGuardian()` in `_validateSignatures`).
    ///     const B = 6350; // Maximum gas used by `_validateSignatures` per additional signature (i.e, other than the first one), without `signingTimelock`s.
    ///     const C = 5700; // Additional gas used by `_validateSignatures` per signature due to presence of `signingTimelock > 0`.
    ///     const E = 0.198; // Additional gas used per byte of calldata in the external function call.
    ///     let sigGas = 0;
    ...
    ///     const X = 38000; // Maximum base gas unrecognized by `gasleft()` in `functionCall` with one signature.
    ///     const Y = 1190; // Maximum gas used per additional signature--i.e, other than the first one (in terms of built-in ABI decoding of `functionCall`'s data into its `signatures` parameter).
    ///     const V = -4750; // Additional gas used (note: value should be negative, so this is actually a deduction, not an addition) per signature due to presence of `signingTimelock > 0`.
    ///     const W = 16.5; // Maximum gas used per byte of calldata in the external function call.
    ///     let sigGas = 0;
    ///     for (const sig of signatures) sigGas += Y + (signerConfig.signingTimelock > 0 ? V : 0);
    ///     return X + sigGas + W * data.length;
    ...
    function functionCall(
        ...
        if (!success) emit FunctionCallReversion(nonce, 0, 0, getTruncatedRevertReasonString(ret));
    ...
    /// @dev If possible, returns the revert reason string from an unsuccessful call's return data.
    function getTruncatedRevertReasonString(bytes memory returnData) internal pure returns (string memory) {
        string memory truncatedReason;
        if (returnData.length > 68) {
            assembly {
                returnData := add(returnData, 68)
            }
            bytes32 first32Bytes = bytes32(returnData);
            assembly {
                returnData := add(returnData, 32)
            }
            truncatedReason = string(abi.encode(first32Bytes, bytes32(returnData)));
        }
    ...
    function simulateFunctionCall(
        ...
        if (!success) {
            if (ret.length > 68) {
                ...
                revert(string(abi.encodePacked("Call reverted: ", abi.decode(ret, (string)))));
            } else {
                revert("Call reverted without a reason string.");
            }
        }

        // Revert with gas used encoded as a decimal string
        revert(string(abi.encodePacked("WALLET_SIMULATE_FUNCTION_CALL_GAS_USAGE=", StringsUpgradeable.toString(initialGas - gasleft()))));
    }

    /// @notice Blacklist for scheduled function calls that have already been dispatched or that have been revoked.
    mapping(uint256 => bool) public functionCallScheduledUniqueIdBlacklist;

    /// @notice Scheduled function calls--i.e, calls a function with any nonce and an (optional) expiration timestamp.
    /// @param deadline Function call must be executed on on before this timestamp. Set to `type(uint256).max)` to prevent expiration.
    /// @param uniqueId Unique ID that can be blacklisted/revoked. Does not have to be unique across all function calls, just across all function call groups--i.e., unique IDs be reused across scheduled function calls so multiple scheduled function calls can be blacklisted/revoked at once.
    function functionCallScheduled(
        Signature[] calldata signatures,
        address target,
        bytes calldata data,
        uint256 value,
        ...
        // Scheduled call validation
        require(!functionCallScheduledUniqueIdBlacklist[uniqueId], "Function call unique ID has been blacklisted.");
        require(block.timestamp <= deadline, "Function call deadline has passed.");

        // Get message hash (include chain ID, wallet contract address, and nonce) and validate signatures
        ++nonce;
        bytes32 dataHash = keccak256(abi.encode(block.chainid, address(this), type(uint256).max, this.functionCallScheduled.selector, target, data, value, feeData, deadline, uniqueId));
        _validateSignatures(signatures, dataHash, true);
        ...
        functionCallScheduledUniqueIdBlacklist[uniqueId] = true;
    ...
    /// @notice Blacklists a scheduled function call unique ID.
    function blacklistScheduledFunctionCall(uint256 uniqueId) external onlySelf {
        functionCallScheduledUniqueIdBlacklist[uniqueId] = true;
    }
    ...
    /// Relayer should confirm the function call does not revert due to bad signatures or low gas limit by either `eth_call` or `eth_estimateGas` or by simulating signature validation off-chain.
    /// Predict user-specified gas limit (i.e., `feeData[0]`) using the following pseudocode:
    ///     const A; // Maximum base gas recognized by `gasleft()` (i.e., excluding paymaster incentive) in `functionCallMulti` with one signature (note that it does not matter if `value` > 0 or if the external call's target function is payable; also note that this estimate assumes that we do call `walletFactory.checkRelayGuardian()` in `_validateSignatures`).
    ///     const B; // Maximum gas used by `_validateSignatures` per additional signature (i.e, other than the first one), without `signingTimelock`s.
    ...
    ///     const Y; // Maximum gas used per additional signature--i.e, other than the first one (in terms of built-in ABI decoding of `functionCallMulti`'s data into its `signatures` parameter).
    ...
    function functionCallMulti(
        ...
        for (uint256 i = 0; i < targets.length; i++) {
            ...
            if (!success) emit FunctionCallReversion(nonce, 0, i, getTruncatedRevertReasonString(ret));
        }
        ...
        for (uint256 i = 0; i < targets.length; i++) {
            ...
            if (!success) {
                if (ret.length > 68) {
                    ...
                    revert(string(abi.encodePacked("Call #", StringsUpgradeable.toString(i), " reverted: ", abi.decode(ret, (string)))));
                } else {
                    revert(string(abi.encodePacked("Call #", StringsUpgradeable.toString(i), " reverted without a reason string.")));
                }
            }
        }
        ...
        for (uint256 i = 0; i < multi.length; i++) {
            ...
            if (multi[i]) {
                ...
                for (uint256 j = 0; j < targets.length; j++) {
                    ...
                    // If call reverted:
                    emit FunctionCallReversion(nonce, i, j, getTruncatedRevertReasonString(ret));
                }
                ...
                // Call contract
                (bool success, bytes memory ret) = target.call{ value: value, gas: gasleft() - 30000 }(data);
                if (!success) emit FunctionCallReversion(nonce, i, 0, getTruncatedRevertReasonString(ret));
                require(minEndingGas <= gasleft(), "Gas limit not satisfied.");
            }
        }
    ...
    /// WARNING: Does not validate that all signers have >= threshold votes.
    function modifySigner(address signer, uint16 votes, uint32 signingTimelock) external onlySelf {
    ...
    /// No unqueue function because transaction nonces can be overwritten (or, in the case of scheduled function calls, unique batch IDs can be blacklisted) and signers can be removed.
    ...
    /// @dev Whether or not the relay guardian is allowed to begin subscription payments.
    bool public subscriptionPaymentsRelayGuardianPermission;

    /// @dev Payment amount per cycle.
    /// If this variable is set to zero, the `subscriptionPayments` function falls back to `DEFAULT_SUBSCRIPTION_PAYMENT_AMOUNT`.
    uint256 internal _subscriptionPaymentAmount;

    /// @dev Current subscription payment amount on this wallet.
    function subscriptionPaymentAmount() public view returns (uint256) {
        uint256 __subscriptionPaymentAmount = _subscriptionPaymentAmount;
        return __subscriptionPaymentAmount > 0 ? __subscriptionPaymentAmount : DEFAULT_SUBSCRIPTION_PAYMENT_AMOUNT;
    }

    /// @dev Max payment amount per cycle (note that monthly payment amount can only be increased with the user's permission).
    uint256 constant MAX_SUBSCRIPTION_PAYMENT_AMOUNT = 0.328e18; // Approximately 4 ETH per year

    /// @dev Default payment amount per cycle.
    uint256 constant DEFAULT_SUBSCRIPTION_PAYMENT_AMOUNT = 0.164e18; // Approximately 2 ETH per year
    ...
    /// @dev Last recurring payment timestamp.
    uint256 public pendingSubscriptionPayments;

    /// @dev Recurring payments transfer function.
    /// Payments roll over if there is not enough ETH in the wallet.
    function subscriptionPayments() external {
        // Validate payments enabled and cycles have passed
        ...
        require(cycles > 0 || pendingSubscriptionPayments > 0, "No cycles have passed.");

        // Get payment amount
        uint256 amount = cycles > 0 ? (subscriptionPaymentAmount() * cycles) + pendingSubscriptionPayments : pendingSubscriptionPayments;

        // Check if there is enough in the wallet to cover the payment
        if (amount > address(this).balance) {
            // If there is not enough, roll the remaining payment over to the next call
            pendingSubscriptionPayments = amount - address(this).balance;
            amount = address(this).balance;
        }
    
        // Set lastSubscriptionPaymentTimestamp to now
    ...
    /// @dev Sets the relay guardian's permission to begin recurring payments.
    /// @param canStart If true, allows the relay guardian to begin recurring payments (must be called by the user). If false, prevents the relay guardian from enabling recurring payments (must be called by the relay guardian).
    function setSubscriptionPaymentsRelayGuardianPermission(bool canStart) external {
        // Validate sender
        if (canStart) {
            require(msg.sender == address(this), "Sender is not user.");
        } else {
            address relayGuardian = walletFactory.relayGuardian();
            require(
                (msg.sender == relayGuardian || msg.sender == walletFactory.secondaryRelayGuardian()) ||
                    (msg.sender == address(this) && (relayerWhitelistTimelock > 0 && relayGuardian != address(0))),
                "Failed attempt to disallow relay guardian's permission to begin subscription payments: sender is not user (via relay guardian) or relay guardian."
            );
        }

        // Set permissions
        subscriptionPaymentsRelayGuardianPermission = canStart;
    }

    /// @dev Enable/disable recurring payments.
    /// Should be enabled on the user's primary wallet if they are not paying via a credit/debit card.
    /// @param enabled If enabling, sender can be user or relay guardian (if `subscriptionPaymentsRelayGuardianPermission`). If disabling, sender can be user (via relay guardian) or relay guardian.
    function setSubscriptionPaymentsEnabled(bool enabled, uint256 secondsPaidForAlready, uint256 newSubscriptionPaymentAmount) external {
        // Validate input
        require(subscriptionPaymentsEnabled != enabled, "Subscription payments status already set to desired status.");

        // Validate sender
        address relayGuardian = walletFactory.relayGuardian();
        require(
            (msg.sender == address(this) && (relayerWhitelistTimelock > 0 && relayGuardian != address(0))) ||
                ((!enabled || subscriptionPaymentsRelayGuardianPermission) && (msg.sender == relayGuardian || msg.sender == walletFactory.secondaryRelayGuardian())),
            "Failed attempt to modify subscription payments: sender is not user (via relay guardian) or relay guardian (with subscriptionPaymentsRelayGuardianPermission if enabling)."
        );

        // Set subscriptionPaymentsEnabled
        subscriptionPaymentsEnabled = enabled;

        // If enabling:
        if (enabled) {
            // Calculate new lastSubscriptionPaymentTimestamp (using secondsPaidForAlready)
            uint256 newLastSubscriptionPaymentTimestamp = block.timestamp - SUBSCRIPTION_PAYMENT_INTERVAL_SECONDS + secondsPaidForAlready;

            // Validate new lastSubscriptionPaymentTimestamp and future subscriptionPaymentAmount
            require(newLastSubscriptionPaymentTimestamp >= lastSubscriptionPaymentTimestamp, "Cannot decrease last subscription payment timestamp.");
            if (msg.sender != address(this)) require(newSubscriptionPaymentAmount <= subscriptionPaymentAmount(), "Cannot increase subscription payment amount.");
            else require(newSubscriptionPaymentAmount <= MAX_SUBSCRIPTION_PAYMENT_AMOUNT, "New subscription payment amount is higher than the maximum.");

            // If amount is zero, disable payments and relay guardian's permission to enable payments
            if (newSubscriptionPaymentAmount == 0) {
                subscriptionPaymentsEnabled = false;
                subscriptionPaymentsRelayGuardianPermission = false;
                return;
            }

            // Set lastSubscriptionPaymentTimestamp and subscriptionPaymentAmount
            lastSubscriptionPaymentTimestamp = newLastSubscriptionPaymentTimestamp;
            _subscriptionPaymentAmount = newSubscriptionPaymentAmount;
        }
    }

    /// @notice Message hashes that `isValidSignature` will confirm as valid (with any signature).
    mapping(bytes32 => bool) internal validSignedMessageHashes;
    ...
    /// See https://eips.ethereum.org/EIPS/eip-1271 for more info.
    function isValidSignature(bytes32 _hash, bytes memory _signature) external view returns (bytes4) {
        return bytes4(validSignedMessageHashes[_hash] ? 0x1626ba7e : 0xffffffff);
    }
    
    /// @notice Sets a message hash as valid so that `isValidSignature` will confirm it as valid (with any signature).
    function setValidSignedMessageHash(bytes32 _hash, bool isValid) external onlySelf {
        validSignedMessageHashes[_hash] = isValid;
    }
}
...
contract WalletFactory {
    ...
    /// @notice The primary relay guardian.
    ...
    /// The (primary or secondary) relay guardian relays all transactions for a `Wallet`, unless the relay guardian whitelist is deactivated on a `Wallet` or if the primary `relayGuardian` is set to the zero address here, in which case any address can relay transactions.
    /// The (primary or secondary) relay guardian can act as off-chain transaction policy node(s) permitting realtime/AI-based fraud detection, symmetric/access-token-based authentication mechanisms, and/or instant onboarding to the Waymont chain.
    /// The (primary or secondary) relay guardian whitelist can be disabled/enabled via a user-specified timelock on the `Wallet`.
    ...
    /// @notice Event emitted when the primary relay guardian is changed.
    ...
    /// Only callable by the (primary or secondary) relay guardian so nonces used on other chains can be kept unused on this chain until the same user deploys to this chain.
    ...
    /// @param subscriptionPaymentsRelayGuardianPermission Whether or not the relay guardian is allowed to begin subscription payments (only enabled if this is the user's primary wallet).
    ...
    function createWallet(
        ...
        bool subscriptionPaymentsRelayGuardianPermission,
        bool upgradeable
        ...
        instance.initialize(signers, signerConfigs, threshold, relayerWhitelistTimelock, subscriptionPaymentsEnabled, subscriptionPaymentsRelayGuardianPermission);
        return instance;
    }
    ...
    /// @notice Sets the primary relay guardian.
    /// WARNING: If this variable is set to the zero address, the relay guardian whitelist will NOT be validated on wallets--so do NOT set this variable to the zero address unless you are sure you want to allow anyone to relay transactions.
