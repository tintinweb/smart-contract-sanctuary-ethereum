    ...
    VRFV2WrapperConsumerBase
{
    ...
    mapping(address => uint256[]) mintedTokenIds;
    mapping(address => uint256) private publicMints;
    mapping(address => uint256) private walletMints;
    mapping(uint256 => address) private tokensMinted;
    mapping(uint256 => address) private tokensClaimed;

    uint256 public collectionSize = 5000; //DEV

    uint256 public vipMinted = 0;
    uint256 public publicMinted = 0;
    uint256 public buddyMinted = 0;
    uint256 public waitMinted = 0;
    uint256 public reserveMinted = 0; //DEV

    uint256 public vipSupply;
    uint256 public immutable publicSupply;
    uint256 public immutable buddyListSupply;
    ...
    uint256 public constant PUBLIC_LIMIT_PER_WALLET = 5;
    uint256 public constant BUDDY_LIST_LIMIT_PER_WALLET = 1;
    uint256 public constant BUDDY_LIST_PH2_LIMIT_PER_WALLET = 5;
    uint256 public RESERVED_TOKENS = 300; //DEV
        ...
        VRFV2WrapperConsumerBase(linkAddress_, wrapperAddress_)
    {
        ...
        buddyListSupply = buddyListSupply_;
        ...
        isSaleState(SaleState.VIP)
    {
        ...
        if (vipMinted + 1 > vipSupply) revert ExceedVIPSpots();
        if (msg.value != mintPrice) revert IncorrectPrice();
        if (walletMints[msg.sender] >= VIP_LIMIT_PER_WALLET)
        ...
        isSaleState(SaleState.Public)
    {
        ...
        if (publicMinted + 1 > publicSupply) revert PublicSoldOut();
        if (msg.value != publicMintPrice) revert IncorrectPrice();
        if (publicMints[msg.sender] >= PUBLIC_LIMIT_PER_WALLET)
            revert LimitPerWalletExceeded();
        ...
        isSaleState(SaleState.BuddyList)
    {
        ...
        if (buddyMinted + 1 > getBuddyListSupply()) revert BuddyListSoldOut();
        if (msg.value != mintPrice) revert IncorrectPrice();
        if (walletMints[msg.sender] >= BUDDY_LIST_LIMIT_PER_WALLET)
            revert LimitPerWalletExceeded();
    ...
    /**
     * @dev handle Additional Buddy List minting
     * @param signature_ validate
     * @param tokenId_ chosen token id
     */
    function additionalBuddyListMint(
        bytes calldata signature_,
        uint256 tokenId_
    ) external payable isSaleState(SaleState.BuddyListPh2) {
        // General validation
        if (tokenId_ < 1 || tokenId_ > collectionSize) revert TokenOutOfRange();
        if (tokensMinted[tokenId_] != address(0x0)) revert TokenNotAvailable();

        // Specific per sale state validation
        if (!verifySignature(signature_, "BuddyListPh2"))
            revert InvalidSignature();
        if (buddyMinted + 1 > getBuddyListSupply()) revert BuddyListSoldOut();
        if (msg.value != mintPrice) revert IncorrectPrice();
        if (walletMints[msg.sender] >= BUDDY_LIST_PH2_LIMIT_PER_WALLET)
            revert LimitPerWalletExceeded();
        ...
        emit Minted(msg.sender, SaleState.BuddyListPh2, msg.value, tokenId_);
    }

    /**
     * @dev handle Wait List minting
     * @param signature_ validate
     * @param tokenId_ chosen token id
     */
        ...
        isSaleState(SaleState.WaitList)
    {
        ...
        if (getTotalMinted() + 1 > getWaitListSupply()) revert SoldOut();
        if (msg.value != mintPrice) revert IncorrectPrice();
        if (publicMints[msg.sender] >= PUBLIC_LIMIT_PER_WALLET)
            revert LimitPerWalletExceeded();
    ...
    function getMintedTokenIds(address owner_)
        public
        view
        returns (uint[] memory)
    {
        return mintedTokenIds[owner_];
    }

    function getTotalMinted() public view returns (uint256) {
        return
            vipMinted + publicMinted + buddyMinted + waitMinted + reserveMinted;
    }

    function getBuddyListSupply() public view returns (uint256) {
        return buddyListSupply + (vipSupply - vipMinted);
    }

    function getWaitListSupply() public view returns (uint256) {
        return collectionSize - RESERVED_TOKENS;
    }

    function numberPublicMinted(address address_)
        external
        view
        returns (uint256)
    {
    ...
    function numberMinted(address address_) external view returns (uint256) {
        return walletMints[address_];
    }

    function batchMints(address[] memory tos_) external onlyOwner {
        // Loop through owners' addresses
        for (uint256 i = 0; i < tos_.length; i++) {
            uint256[] memory _tokensMinted = mintedTokenIds[tos_[i]]; //get minted token ids for each owner

            // Loop through every token ids
            for (uint256 j = 0; j < _tokensMinted.length; j++) {
                _mint(tos_[i], _tokensMinted[j]);
            }
        }
    }
    ...
    */
    function setSaleState(uint256 saleState_) external onlyOwner {
        if (saleState_ > uint256(SaleState.WaitList)) revert InvalidSaleState();
    ...
    /* total per-token fieldTrip, excluding current period */
    // mapping(uint256 => uint256) private fieldTripTotal;

    function startFieldTrip(uint256 tokenId_, uint256 level_)
        internal
        onlyApprovedOrOwner(tokenId_)
    {
        // uint256 nextLevel = getLevel(tokenId_) + 1;
        ...
        // if (level_ > maxLevel) revert LevelNotAvailable();
        // if (level_ - nextLevel != 1) revert WrongFieldTripLevel();
    ...
    return "current" how long already fieldTrip, in seconds
    return "total" total period of time fieldTrip, throught its entire life
    return "" current level fieldTrip time
    */
    ...
    /* (HOLD to EARN) EXPEL - with help manually from admin role */
        ...
        onlyRole(DISMISS_ROLE)
    {
        ...
        // fieldTripTotal[tokenId] += block.timestamp - fieldTripStarted[tokenId]; // add fieldTrip total
        emit StoppedFieldTrip(tokenId_); // emit Unnested
        emit Dismissed(tokenId_); // emit Expelled
    }

    /* 
        (HOLD to EARN) TOGGLE HOLD */
    // function toggleFieldTrip(uint256 tokenId)
    //     internal
    //     onlyApprovedOrOwner(tokenId)
    // {
    //     uint256 start = fieldTripStarted[tokenId];
    //     if (start == 0) {
    //         require(fieldTripOpen == true, "fieldTrip not available yet"); //require nesting open
    //         fieldTripStarted[tokenId] = block.timestamp; //change time
    //         emit StartedFieldTrip(tokenId); //emit
    //     } else {
    //         fieldTripStarted[tokenId] = 0; //change time
    //         emit StoppedFieldTrip(tokenId); //emit
    //     }
    // }

    // function toggleManyFieldTrips(uint256[] calldata tokenIds) external {
    //     uint256 count = tokenIds.length;
    //     for (uint256 i = 0; i < count; i++) {
    //         toggleFieldTrip(tokenIds[i]);
    //     }
    // }

    /* 
        (HOLD to EARN) BLOCK TRANSFER WHEN LISTING : _beforeTokenTransfer
    */
    function _beforeTokenTransfer(
        address,
        address,
    ...
    function claimReward(uint256 level_, uint256 tokenId_) public payable {
        ...
        if (rewardContractAddress[level_] == address(0x0)) revert RewardAddressNotSetYet();

        rewardAlreadyClaimed[tokenId_] = true;
        RewardContract rewardContract = RewardContract(rewardContractAddress[level_]);
    ...
    function setRewardOpenToClaim(uint256 level_, bool open_)
        public
        onlyOwner
    {
        if (rewardContractAddress[level_] == address(0x0)) revert RewardAddressNotSetYet();
    ...
    function withdraw() external onlyOwner {
        (bool success, ) = treasuryJarAddress.call{
    ...
    function reserve(uint256[] memory tokenIds) external onlyOwner {
        if (getTotalMinted() + tokenIds.length > collectionSize)
            revert ExceedReservedSpots();

        // Loop through owners' addresses
        for (uint256 i = 0; i < tokenIds.length; i++) {
            _mint(msg.sender, tokenIds[i]);
            // tokensMinted[tokenIds[i]] = msg.sender;
            reserveMinted++;
        }
    }
