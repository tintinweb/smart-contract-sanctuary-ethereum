...
contract fullflash {
    ...
    function returnPath(address token11, address token22)
        public
        pure
    ...
    function whatthefuck(
        ...
        address pairaddress = IUniswapV2Factory(factor).getPair(tokenA, tokenB);
        ...
        uint256 amountout = IUniswapV2Router01(uniswapR).getAmountsOut(
            amm,
            path1
        )[1];
        uint256 amounttorepay = IUniswapV2Router01(sushiswapR).getAmountsOut(
            amountout,
            path2
        )[1];
