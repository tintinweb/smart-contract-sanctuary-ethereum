...
contract MBOERC721 is ERC721, Ownable {
        ...
        returns (string memory)
    {
        ...
        if (_exists(tokenId)) {
            _tokenURI = string(
                abi.encodePacked(baseUri, tokenId.toString(), BASE_EXTENSION)
            );
        }
        return _tokenURI;
    }
    ...
    function whitelistClaim(bytes32[] calldata proof) external onlyEOA {
        require(claimState[msg.sender] != true, "already claim");
        require(verifyWhitelist(proof, merkleRoot), "Not whitelisted");
