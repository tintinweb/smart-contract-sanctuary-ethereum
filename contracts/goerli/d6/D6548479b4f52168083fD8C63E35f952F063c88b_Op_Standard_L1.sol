...
contract Op_Standard_L1 is IBridgeRouterBase {

    IL1StandardBridge public opBridge ;
    ...
    function initialize(address _singularityRouterAddress, address _nativeWrappedCurrencyAddress, address[] calldata _adminAddresses, address _opL1BridgeAddress) public initializer {
        IBridgeRouterBase_init(_singularityRouterAddress, _nativeWrappedCurrencyAddress, _adminAddresses);
        opBridge = IL1StandardBridge(_opL1BridgeAddress);
    }
...
*/
    ...
    function performBridgeAction(
        ...
        require(_msgSender() == singularityRouterAddress, "Invalid Router");

        if(_tokenAddress == nativeWrappedCurrencyAddress) {
            require(msg.value >= _amount, "Insufficient amount");
            (uint32 _l2Gas, address _l2TokenAddress,  bytes memory _bridgeData) = abi.decode(_data, (uint32, address, bytes));
            _bridgeEthCall(_sender, _receiver, _destChainId, _amount, _l2Gas, _bridgeData);
        }else {
            (uint32 _l2Gas, address _l2TokenAddress,  bytes memory _bridgeData) = abi.decode(_data, (uint32, address, bytes));
            _bridgeErc20Call(_sender, _receiver, _tokenAddress, _l2TokenAddress, _destChainId, _amount, _l2Gas, _bridgeData);
        }
    }


    function _bridgeErc20Call(
        address _sender,
        address _receiver,
        ...
        require(_destChainId == 10 || _destChainId == 420, "Invalid ChainId");
        _giveTokenApproval(address(opBridge), _l1TokenAddress, _amount);
        opBridge.depositERC20To(_l1TokenAddress, _l2TokenAddress, _receiver, _amount, _l2Gas, _bridgeData);
    }


    function _bridgeEthCall(
        address _sender,
        address _receiver,
        uint256 _destChainId,
        uint256 _amount,
        ...
        require(_destChainId == 10 || _destChainId == 420, "Invalid ChainId");
        opBridge.depositETHTo{value: _amount}(_receiver, _l2Gas, _bridgeData);
    }
    ...
    function setOpBridge(address _opBridgeAddress) public onlySuperAdmin {
        opBridge = IL1StandardBridge(_opBridgeAddress);
    }
