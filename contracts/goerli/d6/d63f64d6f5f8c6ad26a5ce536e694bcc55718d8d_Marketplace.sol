...
interface IMarketplace {
    ...
    event OfferAccepted(bytes32 id);
    event OfferCancelled(bytes32 id);
...
contract FeeManager is Ownable {
     ...
     */
    function setOwnerCommissionFee(uint256 _cutPerMillion) external onlyOwner {
        require(
...
contract Marketplace is Ownable, Pausable, FeeManager, IMarketplace, ERC721Holder {
    ...
    // AllNft[] public allNft;

    // From ERC721 registry assetId to Order (to avoid asset collision)
    mapping(address => mapping(uint256 => Order)) public orderByAssetId;
    mapping(address => mapping(uint256 => Offer)) public offerByOrderId;
    mapping(uint256 => AllNft) public listNFt;
    ...
    function _createOrder(address _nftAddress, uint256 _assetId, uint256 _priceInWei, uint256 _expiresAt) internal {
        ...
        listNFt[indexCount] = AllNft(_nftAddress, _assetId, _priceInWei); 
        indexCount++;
    ...
    function makeOffer(address _nftAddress, uint256 _assetId, uint256 _offerPrice) public whenNotPaused{
        Order memory order = _getValidOrder(_nftAddress, _assetId);
        Offer memory off = offerByOrderId[_nftAddress][_assetId];

        require(order.id != 0, "Marketplace: asset not published");
        // require(order.expiresAt >= block.timestamp, "Marketplace: order expired");
        // require(_offerprice > 0,"Offer price should be greater than O");


        if (off.id != 0) {
            if (order.expiresAt >= block.timestamp) {
                require(
                    _offerPrice > off.price,
                    "Marketplace: offer price should be higher than last offer"
                );

            } else {
                require(_offerPrice > 0, "Marketplace: Offer price should be > 0");
            }

            _cancelOffer(off.id, _nftAddress, _assetId, off.user, off.price);
        ...
        acceptedToken.safeTransferFrom(msg.sender, address(this), _offerPrice);

        // Create bid
        bytes32 offerId = keccak256(abi.encodePacked(block.timestamp, msg.sender, order.id, _offerPrice));

        // Save Bid for this order
        offerByOrderId[_nftAddress][_assetId] = Offer({
            id: offerId,
            user: msg.sender,
            price: _offerPrice
        });

        emit offerCreated(offerId, _nftAddress, _assetId, msg.sender, _offerPrice);

    }

    function acceptOffer( address _nftAddress, uint256 _assetId) public whenNotPaused{
        ...
        // require(off.price == _priceInWei, "Marketplace: invalid offer price");
        // require(bid.expiresAt >= block.timestamp, "Marketplace: the bid expired");

        // remove offer
        delete offerByOrderId[_nftAddress][_assetId];

        emit OfferAccepted(off.id);
        ...
        if (FeeManager.cutPerMillion > 0) {
            // Calculate sale share
            saleShareAmount = off.price.mul(FeeManager.cutPerMillion).div(1e6);

            // Transfer share amount for marketplace Owner
            acceptedToken.safeTransfer(owner(), saleShareAmount);
        }

        // transfer escrowed offer amount minus market fee to seller
        acceptedToken.safeTransfer(order.seller,off.price.sub(saleShareAmount));

        _executeOrder(order.id, off.user, _nftAddress, _assetId, off.price);
           
    }

    function cancelOffer(address _nftAddress, uint256 _assetId) public whenNotPaused {
        Offer memory off = offerByOrderId[_nftAddress][_assetId];

        require(off.user == msg.sender || msg.sender == owner(),"Marketplace: Unauthorized sender");

        _cancelOffer(off.id, _nftAddress, _assetId, off.user, off.price);
    }


    function _cancelOffer(bytes32 _offerId, address _nftAddress, uint256 _assetId, address _user, uint256 _escrowAmount) internal {
        delete offerByOrderId[_nftAddress][_assetId];

        // return escrow to canceled user
        acceptedToken.safeTransfer(_user, _escrowAmount);

        emit OfferCancelled(_offerId);
    }

    function allListing(uint8 skip, uint8 limit)public view returns(AllNft[] memory){
      AllNft[] memory lists = new AllNft[](indexCount);
        for(uint8 i = skip; i<= limit; i++){
            AllNft storage list = listNFt[i];
            lists[i] = list;
        }
        return lists;
