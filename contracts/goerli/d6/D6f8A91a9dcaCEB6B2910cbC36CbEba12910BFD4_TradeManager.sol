...
contract TradeManager is Validator, Ownable {
    ...
    /* Fees */
    uint256 public tradeFeeBasisPoints = 10; // 0.1%, only collected on increase position
    ...
    function setAssetSettings(
        ...
        require(_differenceForMaxFundingRate > 0, "TradeManager: Difference for max funding rate must be greater than 0");
    ...
    function _increasePosition(address _account, bytes32 _asset, bool _isLong, uint256 _sizeIncrease, uint256 _collateralIncrease, uint256 _assetPrice) internal {
        ...
        position.averagePrice = getNextAveragePrice(position.size, _sizeIncrease, position.averagePrice, _isLong, _assetPrice);
     ...
     */
    function getNextAveragePrice(uint256 _size, uint256 _sizeDelta, uint256 _oldAveragePrice, bool _isLong, uint256 _currentPrice) internal pure returns (uint256) {
