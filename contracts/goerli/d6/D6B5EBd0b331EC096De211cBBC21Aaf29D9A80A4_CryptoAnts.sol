...
contract CryptoAnts is ERC721, ICryptoAnts, Ownable {
  ...
  /// @notice Buy '_amount' of eggs, reverted if not enough eth sent, reimbursed if more eth is sent
  /// @param _amount amount of eggs to mint
  function buyEggs(uint256 _amount) external payable override lock {
    // CHECKS
    uint256 _totalPrice = _amount * eggPrice;
    if (msg.value < _totalPrice) revert NotEnoughETH();

    // INTERACTIONS
    eggs.mint(msg.sender, _amount);
    uint256 _toReimburse = msg.value - _totalPrice;
    if (_toReimburse != 0) payable(msg.sender).transfer(_toReimburse);

    emit EggsBought(msg.sender, _amount, _totalPrice, _toReimburse);
  }

  /// @notice Create an ant spending an egg
  ...
  /// @notice Burn an ant in exchange for eth
  ...
  function sellAnt(uint256 _antId) external override lock {
    ...
    emit AntSold(_antId, msg.sender);
  }

  /// @notice Change the price of an egg (Only governance)
  /// @param _newPrice New price to set
  function changeEggPrice(uint256 _newPrice) external override onlyOwner {
    // EFFECTS
    uint256 _oldPrice = eggPrice;
    eggPrice = _newPrice;

    emit EggPriceChanged(_oldPrice, _newPrice);
  }
  ...
  /// @notice Return last ant id created
  /// @return antsCreated last id
  function getAntsCreated() public view override returns (uint256) {
    return antsCreated;
  }

  modifier lock() {
    if (locked == true) revert Locked();
...
interface ICryptoAnts is IERC721 {
  event EggsBought(address _buyer, uint256 _amount, uint256 _totalCost, uint256 _reimbursed);
  event AntSold(uint256 _antId, address _seller);
  event AntCreated(address _creator, uint256 _antId);
  event EggPriceChanged(uint256 _oldPrice, uint256 _newPrice);
  ...
  function changeEggPrice(uint256 _newPrice) external;
