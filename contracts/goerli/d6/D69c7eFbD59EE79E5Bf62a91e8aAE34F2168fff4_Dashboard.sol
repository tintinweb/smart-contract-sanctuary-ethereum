...
contract Dashboard is Ownable2Step {
	...
	function InitializeDashboard(address _T, address _X) public onlyOwner {
		Accounts = IAccount(_T);
		Balances = IBalance(_T);
		Tuktus = ITuktu(_T);
		XPrograms = IXProgram(_X);
		Matrixes = IMatrix(_X);
	}
	...
	function AccountInfo(uint _AccountID) public view returns (Account memory accountInfo_) {
	...
	function AccountSponsorLevel(uint _AccountID) public view returns (SponsorLevel memory sponsorLevel_) {
	...
	function AccountBalance(uint _AccountID) public view returns (Balance memory balance_) {
		balance_ = Balance({
	...
	function PirceOfLevel(uint _Index) external view returns (uint pirceOfLevel_) {
		return XPrograms.PirceOfLevel(_Index);
	}

	function PirceOfLevelOn(uint _LevelOn) public view returns (uint pirceOfLevelOn_) {
		return XPrograms.PirceOfLevelOn(_LevelOn);
	}
