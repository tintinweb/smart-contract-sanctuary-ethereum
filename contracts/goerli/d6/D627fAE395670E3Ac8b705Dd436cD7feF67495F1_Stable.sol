...
contract Stable is Ownable, ERC721AWithRoyalties, Pausable, PaymentSplitter {
    ...
    function requestRemovalVerifier(
        ...
        Horser HorseContract = Horser(HorseAddress);
        Horser HorseGovContract = Horser(HorseGovAddress);
        if (to != HorseContract.ownerOf(horseID) && ownerOf(farmID) != to) {
            require(
                HorseGovContract.ownerOf(horseID) == to,
                "MUST OWN FARM, HORSE OR HORSE GOVERNANCE TOKEN"
            );
        }
        require(
            ownerOf(farmID) == to || HorseContract.ownerOf(horseID) == to,
