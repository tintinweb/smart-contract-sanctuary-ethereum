    ...
    function processPayment(ProcessPaymentParams memory params) external payable override onlyRole(PAYMENT_ADMIN_ROLE) {
        ...
        for (uint256 i = 0; i < params.nonFungibleTokenDeposits.length; i++) {
            ...
            if (params.nonFungibleTokenDeposits[i].tokenAddress == price.v3LpTokens[i].tokenAddress) {
                ...
                console.log("Removing liquidity from V3 LP position", price.v3LpTokens[i].liquidityToRemove);
                require(
                ...
                console.log("Sending token 0 to LP Token Processor", balance0);
                console.log("Sending token 1 to LP Token Processor", balance1);
        ...
        uint256 toRefund = processFee(price.usdtAmount, payable(params.user), params.referrer);
        (bool success, ) = payable(params.user).call{ value: toRefund }("");
    ...
    function processFee(
        ...
        if (usdtAmount > 0) {
            ...
            if (msg.value > 0) {
                ...
                toRefund = address(this).balance - oldEthBalance;
            } else {
