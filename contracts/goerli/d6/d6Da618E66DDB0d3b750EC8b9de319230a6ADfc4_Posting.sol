...
import "./ERC5018/IERC5018.sol";
import "./ERC6150/interfaces/IERC6150.sol";

contract Posting is ERC721, IPosting {
    address public immutable override erc6150;
    address public immutable override erc5018;
    uint256 public override postedCount;
    mapping(uint256 => Item) public override getItem;

    constructor(address erc6150_, address erc5018_) ERC721("Posting", "POST") {
        erc6150 = erc6150_;
        erc5018 = erc5018_;
    }
    ...
    function post(
        ...
        require(IERC6150(erc6150).isLeaf(category), "invalid category");
        uint256 itemId = postedCount;
        postedCount += 1;

        getItem[itemId] = Item(category, title, description);
        _safeMint(msg.sender, itemId);
        emit ItemAdded(msg.sender, itemId, category, title, description);

        bytes memory name = abi.encodePacked(category, itemId);
        bytes memory data = abi.encodePacked(title, description);
        IERC5018(erc5018).write(name, data);
    }
    ...
    function update(
        ...
        require(ownerOf(itemId) == msg.sender, "not owner of this item");
        emit ItemUpdated(itemId, title, description);

        // Update in PostStorage //
        bytes memory name = abi.encodePacked(getItem[itemId].category, itemId);
        bytes memory data = abi.encodePacked(title, description);
        IERC5018(erc5018).write(name, data);
    }
    ...
    function remove(uint256 itemId) external override {
        ...
        // Delete from PostStorage //
        bytes memory name = abi.encodePacked(getItem[itemId].category, itemId);
        IERC5018(erc5018).remove(name);
    }
}