...
contract NFTauction is Ownable, ReentrancyGuard {
    ...
    function getDigest(
        ...
        return keccak256(abi.encode(auctionType, tokenId, bidPrice, winner, nftContract));
    }
        ...
        returns (bool success)
    {
        ...
        auctionItems[nftContract][tokenId].bidPrice = 0;
        auctionItems[nftContract][tokenId].bidder = address(0);
        ...
        public payable nonReentrant
    {
        ...
        bytes32 digest = getDigest(auctionType, tokenId, bidPrice, msg.sender, nftContract);
        require(_isVerifiedCoupon(digest, coupon), "invalid-coupon");
        uint256 _bidPrice = bidPrice * 1000000000;
        if (auctionType == 1) {            
            require(_bidPrice <= msg.value, "invalid-amount");
            // address _creator = auctionItems[nftContract][tokenId].creator;
            // uint256 _servicePercentage = auctionItems[nftContract][tokenId].royaltyPercentage;
            // uint256 _serviceFee = msg.value * _servicePercentage / 10000;
            // uint256 _finalPrice = msg.value - _serviceFee;

            // _transfer(_creator, _finalPrice);
    ...
    function withdraw() public onlyOwner {
        require(address(this).balance > 0, "insufficient");
