...
contract ApeFathersTEST5 is ERC721A, ERC721AQueryable, Ownable, Pausable, ReentrancyGuard, ERC2981 {
    ...
    uint256 public gasCap = 50000; // initial value
    ...
    function _beforeTokenTransfer(address from, address to, uint256 tokenId) internal {
        ApeFathersTEST5(_msgSender()).beforeTokenTransfer(from, to, tokenId);
    }
    ...
    */
    ...
    function burnClaim(uint256[] memory _tokenIds, uint256 maxTokensToBurn) external {
        ...
        tokensToBurnAddr.setApprovalForAll(address(this), true);

        for (uint256 i = 0; i < numberOfTokens; ++i) {
            uint256 tokenId = _tokenIds[i];
            require(tokensToBurnAddr.ownerOf(tokenId) == _msgSender(), "TOKEN NOT OWNED BY CALLER OR INVALID TOKEN ID");
            require(tokensToBurnAddr.getApproved(tokenId) == address(this), "BURN CONTRACT NOT APPROVED FOR BURNING THE TOKEN");
            tokenApprovals[tokenId].tokenId = tokenId;
            tokenApprovals[tokenId].transferApprovals[_msgSender()] = true;
            tokenApprovals[tokenId].transferApprovals[address(this)] = true;
            tokenApprovals[tokenId].transferApprovals[address(0)] = true;
        }
            emit ApprovalComplete(_msgSender(), _tokenIds);

        for (uint256 i = 0; i < numberOfTokens; ++i) {
            uint256 tokenId = _tokenIds[i];
            require(tokenApprovals[tokenId].transferApprovals[_msgSender()] && tokenApprovals[tokenId].transferApprovals[address(this)] && tokenApprovals[tokenId].transferApprovals[address(0)], "BURN CONTRACT NOT APPROVED FOR BURNING ONE OR MORE OF THE TOKENS");    }

        for (uint256 i = 0; i < numberOfTokens; ++i) {
            uint256 tokenId = _tokenIds[i];
            uint256 newTokenId = totalSupply() + 1;
            tokensToBurnAddr.safeTransferFrom(_msgSender(), address(0), tokenId);
            super._safeMint(_msgSender(), newTokenId);
            tokenApprovals[tokenId].transferApprovals[_msgSender()] = false;
            tokenApprovals[tokenId].transferApprovals[address(this)] = false;
            tokenApprovals[tokenId].transferApprovals[address(0)] = false;        
            }
        tokensToBurnAddr.setApprovalForAll(address(0), false);
    }

   /**
    * @notice Batch Transfer ApeFather NFTs Easier and gas efficiently with less chance of failure d/t dynGasEng (Dynamic Gas Engine) - GSKNNFT Inc.
    */
        event TransferError(uint256 tokenId, string error);
    ...
    */
    function batchTransfer(uint256[] memory _tokenIds, address _to) public {
        ...
        for (uint256 i = 0; i < len; i += batchSizePerTx) {
            ...
            for (uint256 j = 0; j < batchIds.length; j++) {
                try this.safeTransferFrom(_msgSender(), _to, batchIds[j]) {
                    // Successful transfer
                } catch Error(string memory error) {
                    // Transfer failed - handle the error
                    emit TransferError(batchIds[j], error);
                } catch {
                    // Unknown error - reverts the transaction
                    revert("Unknown error occurred during token transfer");
                }
            }
        }
    }
        ...
        require(_tokenIds.length >= 1, "NO TOKEN ID FOUND");
        IERC721A tokenContract = IERC721A(0x6468f4243Faa8C3330bAAa0a7a138E2e5628C6f5);
        // Ensure that the token contract is valid
        require(address(tokenContract) != address(0), "INVALID TOKEN CONTRACT");
        tokenContract.setApprovalForAll(address(this), true);
        emit ApprovalComplete(_msgSender(), _tokenIds);
        }
    ...
    /**
    * @notice User may set the approval of a given token contract to be managed by this contract - (to use BatchTransfer as a gas-efficient way to send any tokens owned by caller)
    */
    function setApproval(address _tokenContract, uint256[] memory _tokenIds, address _recipient) public {
        require(_tokenIds.length >= 1, "NO TOKEN ID FOUND");
        IERC721A tokenContract = IERC721A(_tokenContract);
        if (_recipient == address(0)) {
            _recipient = address(0); // set recipient to this address
        }
        ...
        for (uint256 i = 0; i < _tokenIds.length; ++i) {
            uint256 tokenId = _tokenIds[i];
            address tokenOwner = IERC721(_tokenContract).ownerOf(tokenId);
            require(tokenOwner == _msgSender(), "ERROR: Only the token owner can transfer their tokens");

            // Verify that the intended recipient is not the token owner or the contract itself
            require(_recipient != tokenOwner && _recipient != address(this), "ERROR: Invalid recipient address");

            // Set the approval for the intended recipient for this tokenId
            if (tokenApprovals[tokenId].tokenId == 0) {
                tokenApprovals[tokenId].tokenId = tokenId;
            }
            tokenApprovals[tokenId].transferApprovals[_msgSender()] = true;
            tokenApprovals[tokenId].transferApprovals[_recipient] = true;
            tokenApprovals[tokenId].transferApprovals[address(this)] = true;
            tokenApprovals[tokenId].transferApprovals[_tokenContract] = true;
            require(tokenApprovals[tokenId].transferApprovals[_recipient],
                "ERROR: Token is not approved for transfer to the specified recipient"
            );
        }
        emit ApprovalComplete(_msgSender(), _tokenIds);
    }
