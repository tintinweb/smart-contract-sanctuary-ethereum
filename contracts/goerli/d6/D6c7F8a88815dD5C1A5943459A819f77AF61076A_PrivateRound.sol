...
contract PrivateRound{
    ...
    */
    ...
    function tokenStatus(uint _roundId, address _founder, address _investor) public view returns(uint unlockedAmount, int lockedAmount, uint withdrawnTokensByFounder){
        uint unlockedTokens;
        int lockedTokens;
        ...
        for(uint i = 0; i < _milestone[_investor][_roundId].length; i++){
            ...
            if(milestoneApprovalStatus[_roundId][id] == 1 && milestoneWithdrawalStatus[_roundId][id] == false){
                unlockedTokens = _milestone[_investor][_roundId][i]._percent;
                ...
                lockedTokens = milestoneApprovalStatus[_roundId][id];
