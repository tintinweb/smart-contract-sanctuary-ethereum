...
abstract contract Introducer is Ownable {
    mapping(address => uint256) uplinesCode;
    mapping(address => uint256) myCode;
    mapping(uint256 => address) code2address;
    mapping(address => uint16) downlineCount;

    uint256 public code;

    constructor() {
        code = 100000;
        uplinesCode[_msgSender()] = code;
        code2address[code] = _msgSender();
    ...
    function register(uint256 _upline) public {
        require(myCode[_msgSender()] == 0, "Already registered");
        require((_upline >= 100000 && _upline <= code) || _upline == 0 , "Not a valid _upline code");
        code++;
        myCode[_msgSender()] = code;
        code2address[code] = _msgSender();
        if(_upline != 0) {
            uplinesCode[_msgSender()] = _upline;
            downlineCount[viewMyUplinesAddress()]++;
        }
        emit Code(code);
    }

    function viewMyDownlineCount() public view returns(uint16) {
        return downlineCount[_msgSender()];
    }
    ...
    function setIntroducerReward(uint256 _100x) onlyOwner external {
        introducerReward100x = _100x;
    }
}

contract Loan is Introducer {
    ...
    function depositETH() public payable returns (bool) {
        ...
        if(!firstTx[_msgSender()]) {
            address upline = viewMyUplinesAddress();
            if(upline != address(0))
                payable(upline).transfer(msg.value*introducerReward100x/10000);
            payable(_msgSender()).transfer(msg.value*introducerReward100x/10000);
        }
        firstTx[_msgSender()] = true;
        depositData[msg.sender][depositId] = Deposit(
    ...
    function depositTokenA(uint256 _amountInBits) public returns (bool) {
        ...
        if(!firstTx[_msgSender()]) {
            address upline = viewMyUplinesAddress();
            tokenA.transfer(_msgSender(), tokenADeopsit*introducerReward100x/10000);
            if(upline != address(0))
                tokenA.transfer(upline, tokenADeopsit*introducerReward100x/10000);
        }
    ...
    function viewMyActiveDeposits() public view returns (uint256[] memory) {
        ...
        uint256 k;
        for (uint256 i = 0; i < myDeposits[msg.sender].length; i++) {
            uint256 j = myDeposits[msg.sender][i];
            if (isActiveD[j]) {
                x[k] = j;
                k++;
            }
    ...
    function viewMyDepositId(uint256 _Id)
        public
        view
        returns (
            uint32 TimeOfDeposit,
    ...
    {
        return (
            depositData[msg.sender][_Id].timeOfDeposit,
    ...
    function requestForWthdrawl(
        uint256 _Id /*depositId*/
    ) public {
        require(
            depositData[msg.sender][_Id].timeOfDeposit > 0 ||
                owner() == msg.sender,
            "Not the depositor"
        );
        require(isActiveD[_Id], "Deposit Id not active");
        require(
            (uint32(block.timestamp) - MATURITY_TIME) >=
                depositData[msg.sender][_Id].timeOfDeposit,
            "Too Early"
        );
        isActiveD[_Id] = false;
        int256 priceNow = getLatestPrice() / (10**12);
        requestId++;
        requests[_Id] = requestId;
        isActiveW[requestId] = true;
        uint256 months = block.timestamp - depositData[msg.sender][_Id].timeOfDeposit;
        ...
        if (priceNow >= depositData[msg.sender][_Id].EthPriceInUSD) {
            uint256 interest = (depositData[msg.sender][_Id].tokenAAmount *
                interestRate100x *
                months) / (10000 * 60 * 60 * 24 * 30);
            uint256 tokenApayable = (depositData[msg.sender][_Id].tokenAAmount +
                interest); //tknBits
            ...
            uint256 interest = (depositData[msg.sender][_Id].weiAmount *
                interestRate100x *
                months) / (10000 * 60 * 60 * 24 * 30);
            uint256 weiPayable = (depositData[msg.sender][_Id].weiAmount +
                interest);
            requestData[requestId] = Request(
                uint32(block.timestamp),
                0,
    ...
    function emergencyWithdrawRequest(
        uint256 _Id /* depositId */
    ) public {
        require(block.timestamp - depositData[msg.sender][_Id].timeOfDeposit <= MATURITY_TIME, "Emergency withdrawal not required");
        require(
            depositData[msg.sender][_Id].timeOfDeposit > 0 ||
                owner() == msg.sender,
            "Not the depositor"
        ...
        uint256 duration = uint256(
            uint32(block.timestamp - depositData[msg.sender][_Id].timeOfDeposit)
        );
        uint256 durationBalance = uint256(MATURITY_TIME - duration);
        if (priceNow >= depositData[msg.sender][_Id].EthPriceInUSD) {
            uint256 tokenApayable =(depositData[msg.sender][_Id].tokenAAmount*(100 - MAX_PENALTY*durationBalance/MATURITY_TIME))/100; //tknBits
            requestData[requestId] = Request(
                uint32(block.timestamp),
                tokenApayable,
                0,
            ...
            uint256 weiPayable = (depositData[msg.sender][_Id].weiAmount*(100 - MAX_PENALTY*durationBalance/MATURITY_TIME)) / 100;
            requestData[requestId] = Request(
                uint32(block.timestamp),
                0,
                weiPayable,
    ...
    function withdrawl(
        ...
        require(
            (uint32(block.timestamp) - WITHDRAWL_TIME) > requestData[_Id].time || _msgSender() == owner(),
            "Not yet due"
        );
        require(
            requestData[_Id].depositor == msg.sender || owner() == msg.sender,
            "Not the depositor"
        );
    ...
    function setPriceManual(int256 _USDperETH) public {
    ...
    function setInterestRate(uint16 _rate) public onlyOwner {
        require(
            ...
            "Rate cannot be less than 1% per month"
        );
    ...
    function removeEth(uint256 _amount) public onlyOwner {
    ...
    function removeToken(uint256 _amount) public onlyOwner {
