...
contract ChubbyKaijuDAOStakingV2 is IChubbyKaijuDAOStakingV2, Ownable, IERC721Receiver, Pausable, ReentrancyGuard {
  ...
  mapping(address => uint256[40]) public tokensByAddressBitMap;
  mapping(uint256 => uint256) public timeStakeByToken;

  uint256[40] public migratedTokensBitmap;
  ...
  function test() external pure returns (uint256 x) {
    x = 1 << 255;
  }
  ...
  function GEN2depositsOf(address account) external view returns (uint256[40] memory) {
  ...
  function hasMigrated(uint256 tokenId) external view returns (bool) {
    ...
    uint256 bitIndex = tokenId - (256 * index);
  ...
  function _setMap(uint256[40] storage map, uint256 tokenId, bool stake) internal {
    uint256 index = tokenId / 256;
    uint256 bitIndex = tokenId - (256 * index);
    uint256 bitToChange = 1 << bitIndex;
    uint256 bitAtIndex = map[index] & bitToChange;
    uint256 storageBits = 1 << 17;
    ...
    if (stake) {
      ...
      require(bitAtIndex == 1, "not owner of token");
      map[index] ^= bitToChange;
      // remove one from storage if there at least 1
      uint16 tokenNum = uint16(map[39] >> 17);
      if (tokenNum >= 1) {
        map[39] -= storageBits;
      } else {
  ...
  function _getTokenNum(uint256[40] storage map) internal view returns (uint16 tokenNum) {
    tokenNum = uint16(map[39] >> 17);
  }
