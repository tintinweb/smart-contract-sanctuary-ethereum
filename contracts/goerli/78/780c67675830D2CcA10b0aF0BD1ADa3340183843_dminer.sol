...
contract dminer is Ownable, ReentrancyGuard {
    ...
    */
    ...
    function test1(uint256 value1, uint256 value2)
        public
        pure
        returns (uint256)
    {
    ...
    function getTicketDetails(uint256 roundId, uint256 ticketId)
        public
        view
    ...
    function buyTickets(
        ...
        require(
            nftUsed[roundId][tokenIdHolding] <= 10,
            "DIGI: NFT used enough"
        );
        LotteryRound storage round = RoundDetails[roundId];
        require(
            round.endTime > block.timestamp,
            "DIGI: Lottery has been ended"
        );
        require(round.ticketsSold < 100000, "DIGI: All tickets are sold");
        for (uint256 i = 0; i < quantity; i++) {
            round.ticketsSold += 1;
            TicketKey[roundId][round.ticketsSold] = Ticket(
                _msgSender(),
                getTicketKey(round.ticketsSold),
                miningLevel,
                false
            );
    ...
    function getJackpotPrice(uint256 roundId) public view returns (uint256) {
        ...
        uint256 jackpotPrice;
        if (
            (previosRound.ticketsSold.mul(ticketPrice)).div(2) >
            previosRound.priceClaimed
        ) {
            jackpotPrice = (previosRound.ticketsSold.mul(ticketPrice)).sub(
                previosRound.priceClaimed
            );
        }
        return jackpotPrice;
    ...
    function announceResult() public onlyOwner {
        uint256 roundId = currentRound();
        require(
            RoundDetails[roundId].endTime < block.timestamp,
            "DIGI: Lottery not ended"
        );
    ...
    function getTicketResult(uint256 ticketId, uint256 roundId)
        public
        view
        returns (uint256)
    {
    ...
    function getMultiplier(uint256 ticketId, uint256 roundId)
        public
        view
        returns (uint256)
    {
    ...
    function getWinningOre(uint256 ticketId, uint256 roundId)
        public
        view
        returns (Ores Ore)
    {
        uint256 result_ = getTicketResult(ticketId, roundId);
        Ores ore;
        Ores previousOre;
        uint256 multipler = getMultiplier(ticketId, roundId);
        for (uint256 i = 0; i < multipler; i++) {
            uint256 result = (result_.mul(i)) % 1000;
