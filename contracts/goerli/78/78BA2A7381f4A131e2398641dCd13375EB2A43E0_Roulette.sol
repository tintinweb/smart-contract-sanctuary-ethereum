...
contract Roulette is Initializable, OwnableUpgradeable, UUPSUpgradeable {
    ...
    uint80 public minimumBet;
    uint80 public maximumBet;

    mapping(address => Bet[]) internal betsByUser;

    uint8 internal constant ROULETTE_MAX_NUMBER = 36;

    event BetPlaced(address user, uint betIndex, Bet bet);
    event RewardClaimed(address user, Winning winning);
    ...
    function initialize(uint80 _minimumBet, uint80 _maximumBet) initializer public {
    ...
    function placeBet(uint8 rouletteNumber) external payable {
        ...
        placeBetUnsafe(msg.value, block.number, rouletteNumber);
    }

    function claimRewardByIndex(uint index) external {
        Bet memory bet = userBetByIndex(msg.sender, index);
        uint reward = betReward(bet);

        betsByUser[msg.sender][index].claimed = true;
        ...
        emit RewardClaimed(msg.sender, winning);
    }
    ...
    function userWinningBets(address user) external view returns (Winning[] memory) {
        ...
        Winning[] memory unclaimedWinnings = new Winning[](bets.length);
    ...
    function userTotalRewards(address user) external view returns (uint) {
        Bet[] memory bets = userBets(user);
        ...
        for (uint i = 0; i < bets.length; i++) {
            ...
            totalRewards += winningBetReward(bet);
        }
    ...
    function blockRouletteNumber(uint blockNumber) external view returns (uint)
    {
        ...
        return blockRouletteNumberUnsafe(blockNumber);
    }
    ...
    function userBets(address user) public view returns (Bet[] memory) {
        return betsByUser[user];
    }

    function userRewardByIndex(address user, uint index) public view returns (uint) {
        return betReward(userBetByIndex(user, index));
    }

    function userBetByIndex(address user, uint index) public view returns (Bet memory) {
        require(betsByUser[user].length > index, "The requested bet does not exist.");

        return betsByUser[user][index];
    }

    function placeBetUnsafe(uint value, uint blockNumber, uint rouletteNumber) internal {
        Bet memory newBet = Bet({
            value: uint80(value),
            blockNumber: uint160(blockNumber),
            rouletteNumber: uint8(rouletteNumber),
            claimed: false
        });

        Bet[] storage bets = betsByUser[msg.sender];

        bets.push(newBet);
        uint newBetIndex = bets.length - 1;

        emit BetPlaced(msg.sender, newBetIndex, newBet);
    }

    function betReward(Bet memory bet) internal view returns (uint) {
    ...
    function isWinningBet(Bet memory bet) internal view returns (bool) {
        return bet.rouletteNumber == blockRouletteNumberUnsafe(bet.blockNumber);
    }

    function blockRouletteNumberUnsafe(uint blockNumber) internal view returns (uint)
    {
        return uint(blockhash(blockNumber + 1)) % (ROULETTE_MAX_NUMBER + 1);
    }

    function winningBetReward(Bet memory bet) internal pure returns (uint) {
    ...
    function sendFunds(uint amount, address recipient) private {
        require(amount <= contractBalance(), "Amount to send exceeds contract balance.");
