...
contract VegasVickieLegend is ERC721, ERC721Enumerable, ERC2981, AdminControl, Ownable, ReentrancyGuard, Pausable {
    ...
    ) ERC721("Vegas Vickie Legend", "VVLEGEND") {
    ...
    */
    function mintAllowlist(uint256 _quantity, bytes32[] calldata proof) public payable nonReentrant whenNotPaused {
        require(_quantity > 0, "Quantity must be greater than 0");
        require(isAllowlistStage(), "Allowlist is not required for this mint window");

        require(_verify(_leaf(_msgSender()), proof), "Invalid Merkle Tree proof supplied.");
