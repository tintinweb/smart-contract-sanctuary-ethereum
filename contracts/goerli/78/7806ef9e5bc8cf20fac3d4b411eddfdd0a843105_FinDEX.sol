...
contract FinDEX is protected {
    ...
    struct ACCOUNT {
        ...
        address[] owned_erc20;
        mapping(address => uint) erc20_index;
    ...
    function deposit_erc20(address token, uint amount) public {
        ...
        if (accounts[msg.sender].balances_erc20[token] == amount) {
            accounts[msg.sender].owned_erc20.push(token);
        }
    }
    ...
    function withdraw_erc20(address token, uint amount) public {
        ...
        if (accounts[msg.sender].balances_erc20[token] == 0) {
            uint index = accounts[msg.sender].erc20_index[token];
            accounts[msg.sender].owned_erc20[index] = accounts[msg.sender].owned_erc20[accounts[msg.sender].owned_erc20.length - 1];
            accounts[msg.sender].owned_erc20.pop();
            accounts[msg.sender].erc20_index[accounts[msg.sender].owned_erc20[index]] = index;
        }
    }
    ...
    function transfer_erc20(address token, uint amount, address to) public {
        ...
        if (accounts[to].balances_erc20[token] == amount) {
            accounts[to].owned_erc20.push(token);
            accounts[to].erc20_index[token] = accounts[to].owned_erc20.length - 1;
        }
    }
    ...
    function get_erc20_owned() public view returns (address[] memory) {
        return accounts[msg.sender].owned_erc20;
    }
