    ...
    ERC1363Payable
{
     ...
     * 30000 translates into 0.003% reward per hour for each staked token
     */
        ...
        internal whenNotPaused
    {
        ...
        assert(token.balanceOf(address(this)) == totalTokensStaked + amount);
        ...
        assert(token.balanceOf(address(this)) == totalTokensStaked);
    ...
    function getStakingStatus(address stakeholder)
        public view validAddress(stakeholder) returns(StakingStatus memory)
    {
        uint totalAmount = 0;
     ...
     * @param index Index of the stake is the users stake counter, starting at 0 for the first stake
     */
    function unstakeWithoutPayout(uint amount, uint index)
        public nonReentrant
    {
        require(amount > 0,
            "ExenoTokenStaking: amount cannot be zero");
        ...
        address stakeholder = msg.sender;
        _unstake(stakeholder, amount, index);
        ...
        public nonReentrant
    {
        ...
        while (amountLeft > 0 && index < stakes.length) {
            if (stakes[index].amount > 0) {
                uint bite = Math.min(stakes[index].amount, amountLeft);
                _unstake(stakeholder, bite, index);
                amountLeft -= bite;
            }
            index++;
        }
