...
contract CrownTokenClaim is VucaOwnable {
  ...
  address public cwtAddress = 0xF3Bb9F16677F2B86EfD1DFca1c141A99783Fde58;
  ...
  function claim(
    ...
    require(startBlock <= block.number && block.number <= endBlock, "Inactive");
    require(_expiredAt >= block.timestamp, "Signature expired");
    require(validSignature(keccak256(abi.encodePacked("vuca-cwt-claim", msg.sender, _maxAmount, _expiredAt)), _signature), "Invalid signature");
    require(claimed[msg.sender] + _amount <= _maxAmount, "Exceeds max");

    IERC20(cwtAddress).safeTransfer(msg.sender, _amount);
    claimed[msg.sender] += _amount;
  }

  /* Admin */
  // verified
  function setCwtAddress(address _contract) external onlyOwner {
    cwtAddress = _contract;
  }
  ...
  // verified
  function setActiveTime(uint256 _startBlock, uint256 _endBlock) external onlyOwner {
    require(_endBlock > _startBlock, "Invalid input");
  ...
  function withdrawCwt(address _to) public onlyOwner {
    require(block.number > endBlock, "Claim active");

    uint256 balance = IERC20(cwtAddress).balanceOf(address(this));
    IERC20(cwtAddress).safeTransfer(_to, balance);
  }
