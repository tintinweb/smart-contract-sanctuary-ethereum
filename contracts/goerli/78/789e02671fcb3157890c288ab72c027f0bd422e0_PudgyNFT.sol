...
contract PudgyNFT is ERC721Enumerable, Ownable {
  using Strings for uint256;
  ...
  uint256 public minmiumEthBalance=200000000000000000;
  ...
  function mint() public  {
    ...
    require(newMintedPerAddress <= maxTokensMintedPerAddress,"Exceeded maxTokensMintedPerAddress");
    //require(balanceOf(msg.sender) + 1 <= maxTokensMintedPerAddress,"Exceeded maxTokensMintedPerAddress");
    require(!paused,"Minting is paused.");
    require(supply + 1 <= maxSupply,"Exceeded maxSupply");
    require(msg.sender.balance >= minmiumEthBalance,"min eth balance required.");
    
    _safeMint(msg.sender, supply + 1);
  }

  function giveAway(address _to,uint256 _quantity) public onlyOwner {
     uint256 supply = totalSupply();
      require(supply + _quantity <= maxSupply, "exceed MAX_SUPPLY");
