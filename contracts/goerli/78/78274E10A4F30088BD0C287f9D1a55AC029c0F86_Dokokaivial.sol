...
contract Dokokaivial is DefaultOperatorFilterer, ERC1155, Ownable, ERC1155Burnable {
    ...
    function mint(uint256 amountToMint) public payable {
        ...
        require(amount + amountMinted <= MAX_SUPPLY, "Limit reached");
    ...
    function airdropGiveaway(address[] calldata to, uint256[] calldata amountToMint) public onlyOwner {
        for(uint256 i = 0; i < to.length; i++) {
            require(amountToMint[i] + amountMinted <= MAX_SUPPLY, "Limit reached");
    ...
    // Set the merkle root for the allow list mint
