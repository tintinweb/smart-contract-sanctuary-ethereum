...
contract NotJustALock is ERC721A, Ownable {
    ...
    mapping(address => bool) public publicSalePurchased;
    ...
    function processMint(uint256 _quantity) internal {
        require(msg.value == PRICE_LOCKS * _quantity, "INCORRECT ETH SENT");
        require(
            totalSupply() + _quantity <= MAX_LOCKS,
            "MAX CAP OF LOCKS EXCEEDED"
        );
    ...
    function publicSale() external payable callerIsUser {
        require(
            saleStatus == SaleStatus.PUBLIC,
            "PUBLIC SALE IS NOT ACTIVE"
        );
        require(
            !publicSalePurchased[msg.sender],
            "PUBLIC SALE LOCK ALREADY PURCHASED"
        );
