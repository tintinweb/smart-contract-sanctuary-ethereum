...
contract CollectivePortraitRegistry is ERC1155, ERC1155Supply, Ownable {
    ...
    */
    ...
    /*
    Creating human readable names for Collective Portraits such that users can register unique names for their Portraits. This will likely be a paid element on mainnet.
    ...
    */
    mapping(uint256 => string) namesByTokenId;
    ...
    */
    ...
    function mintPortrait(
        ...
        // If getStatus(_name) returns 0, the name is available.
        require(getStatus(_name) == 0, "Name is not available");

        // Name can not be longer than 64 characters.
        require(bytes(_name).length <= 64, "Name is too long");

        // Name has to be at least 3 char
        require(bytes(_name).length >= 3, "Name is too short");

        // Name can not start with 0x
        require(bytes(_name)[0] != 0x30, "Name can not start with 0x");
        ...
        _mint(msg.sender, (portraitCount + 1), _shares, "");
    ...
    This is needed such that we can prevent a scenario in which no user has less than the minimumControlShares through the mapping tokenBalances.
        ...
        for (uint256 i = 0; i < ids.length; i++) {
            ...
            if (from != address(0)) {
                ...
                require(
                    ...
                    "Insufficient token balance to reach minimumControlShares"
                );
            }
        }
    }
    ...
    */
    ...
    /*
    Update the minimumControlShares. This function can only be called by the highest shareholder(s).
    */
    ...
    */
    ...
    /*
    This function returns the latest portraitHash for a Collective Portrait.
    */
    function getPortraitHash(string memory _name)
        public
        view
        returns (string memory)
    {
        // Require a portraitHash to be set
        require(
            keccak256(abi.encodePacked(records[_name].portraitHash)) != 0,
            "No portraitHash is set for this Collective Portrait"
        );
    ...
    If the status is 2, the Collective Portrait is in the 6 month grace period to renew.
    */
    function getStatus(string memory _name) public view returns (uint256) {
        // Require validUntil to be in the future, use block.timestamp
        if (records[_name].validUntil > block.timestamp) {
            // If the validUntil is in the future, the Collective Portrait is active
            return 1;
        } else if (
            records[_name].validUntil + 15768000 > block.timestamp &&
            records[_name].validUntil < block.timestamp
        ) {
            // If the validUntil is in the past, but less than 6 months ago, the Collective Portrait is in the grace period
            return 2;
        } else {
            // If the validUntil is more than 6 months ago, the Collective Portrait has expired
            return 0;
        }
    }
    ...
    */
    ...
    /* TO-DO: Create endpoint */
    string baseUri = "http://localhost:3000/";

    /* Returns the URI for a given token ID */
    function uri(uint256 _id) public view override returns (string memory) {
        return string(abi.encodePacked(baseUri, namesByTokenId[_id]));
    }
    ...
    */
        ...
        returns (uint256)
    {
        ...
        if (_length == 3) {
            return usdToWei(640) * _years;
        }
        if (_length == 4) {
            return usdToWei(160) * _years;
        }
        // if (_length >= 5)
        return usdToWei(5) * _years;
    }

    /* Function to renew a Collective Portrait
    Anyone can pay for a Collective Portrait's renewal. Even non-shareholders.
    */
    function renewPortrait(string memory _name, uint256 _years)
        public
        payable
        returns (bool)
    {
        // Require validUntil to be in the past, use block.timestamp
        require(
            records[_name].validUntil < block.timestamp,
            "This Portrait is not expired"
        );

        // Require validUntil to be in the past, use block.timestamp
        require(
            records[_name].validUntil + 6 * 30 days > block.timestamp,
            "This Portrait is not in the grace period"
        );
    ...
    /* 
    The auction will be a Dutch auction, where the price starts at 100\*yearly renewal fee, and decreases by 1% every 10 minutes.
    If the auction price is lower than the yearly renewal fee, the shares will be burned and the name will be released.
    If the auction is won, the shares of the largest shareholder is transfered to the winner. */
        ...
        returns (uint256)
    {
        ...
        uint256 _timeSinceAuctionStart = block.timestamp -
            records[_name].validUntil;

        // Get the auction price
        uint256 _auctionPrice = (calculatePrice(bytes(_name).length, 1) * 100) /
            (100 + _timeSinceAuctionStart / 10 minutes);

        return _auctionPrice;
    }

    /* Function to see if a name is still in the auction period
    This can be done by checking if validUntil + 6 months is in the past
