 ...
 * Fuck you, build your own or call this one
 * This thing calculates slippage and max txn/wallet if possible  
 * You need to call it as a callStatic in ethers.js or the equivalent elsewhere
*/
pragma solidity >0.8.15;

interface ISlippageDetector {


    // This must be called by the wallet that's selling/buying tokens and with an approval to us - it must also be non-state 
    
        // Run a swap over token path with amountIn
        // But basically we need to execute a swap and read how much we actually get out of it. 
    
    function detectv2Slippage(address[] calldata tokenPath, uint256 amountIn) external returns (uint32 slippage);
...
import "./IWETH.sol";
import "./ISlippageDetector.sol";
import "@openzeppelin/contracts/access/Ownable.sol";


/**
 * I've lifted a fair bit of code out of the original Uniswap v2 - as such, it's licensed as v2 router is on github - GPL 3.0
 * Modified to use in-built SafeMath and the meta-transaction standard
 */
contract ToadRouter01 is IToadRouter01, Ownable {
    mapping(address => bool) allowedBots;

    modifier ensure(uint deadline) {
        require(deadline >= block.timestamp, 'LimitBot: EXPIRED');
        _;
    }


    modifier onlyBot() {
        require(allowedBots[msg.sender], "LimitBot: UNTRUSTED");
        _;
    }
    ...
    function swapExactTokensForTokens(
        ...
        require(amounts[amounts.length - 1] >= amountOutMin, 'LimitBot: INSUFFICIENT_OUTPUT_AMOUNT');
    ...
    function swapExactTokensForTokensSupportingFeeOnTransferTokens(
        ...
        require(
            ...
            'LimitBot: INSUFFICIENT_OUTPUT_AMOUNT'
        );
    ...
    function swapExactWETHforTokensSupportingFeeOnTransferTokens(uint amountIn, uint amountOutMin, address[] calldata path, address to, uint deadline, uint256 gasReturn) external virtual override ensure(deadline) onlyBot() returns(uint256 outputAmount) {
        require(path[0] == WETH, 'LimitBot: INVALID_PATH');
        // Send us gas first
        if(gasReturn > 0) {
    ...
    {
        require(path[path.length - 1] == WETH, 'LimitBot: INVALID_PATH');
        ...
        ensure(deadline) onlyBot() returns(uint256 outputAmount)
    {
        ...
        require(outputAmount >= amountOutMin, 'LimitBot: INSUFFICIENT_OUTPUT_AMOUNT');
