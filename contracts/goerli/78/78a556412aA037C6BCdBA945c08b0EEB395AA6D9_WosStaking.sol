...
contract WosStaking is OnlyAuthorized, CollateralWrapper {
    ...
    function distribute() external onlyAuthorized {
        ...
        if (availableCollateral < minBalance) {
            emit StakeResult(
                "There are not enough funds to make a distribution among stakers", 0
            );
            return;
        }
        ...
        for (uint256 index = 0; index < stakers.length; index++) {
            ...
            uint256 toTransfer = percentageDistribute.mul(availableCollateral).div(100);
            
            uint256 remaining = _collateralBalanceOf(address(this));
            _ctSign(string.concat("WS_RC_T_LSTEP_", Strings.toString(index), ";toTransfer=", Strings.toString(toTransfer), ";remaining=", Strings.toString(remaining)));
            _collateralTransfer(
                staker,
                toTransfer
            );

            collateralSent[staker] = collateralSent[staker].add(toTransfer);
            distributed = distributed.add(toTransfer);
            emit sentToTheStaker(staker, toTransfer);
        }

        emit StakeResult("Balances have been distributed to stakers", distributed);
    }
