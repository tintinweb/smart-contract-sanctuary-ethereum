...
contract StakeHT is Pausable, Ownable, ReentrancyGuard {

    Token HTToken;

    // 30 Days (30 * 24 * 60 * 60)
    uint256 public planDuration = 3600;

    // 180 Days (180 * 24 * 60 * 60)
    uint256 _planExpired = 36000;
    ...
    function claimReward() external returns (bool){
        ...
        HTToken.transfer(_msgSender(), totalTokens);
    ...
    function stakeToken(uint256 stakeAmount) external payable whenNotPaused {
        ...
        require(HTToken.balanceOf(_msgSender()) >= stakeAmount, "Insufficient Balance");
        
           HTToken.transferFrom(_msgSender(), address(this), stakeAmount);
            totalStakers++;
