...
contract BoostNFTMulti_v3 is ERC721A, ERC721AQueryable, Ownable, ReentrancyGuard {
    ...
    constructor() ERC721A("go", "go") {
        allowedCrypto[addedTokens] = TokenInfo("ETH", address(0),  mintRate, true);
        addedTokens++;
    }
    ...
    function toggleStatus(bool _specialsaleEnable, bool _bypricesaleEnable, bool _multisaleEnable) public onlyOwner {
  ...
  function rescueERC20(address tokenAdd, uint256 amount) external onlyOwner {
    ...
    */
    ...
    function multiMint(uint256 _quantity, uint256 _id) external payable {
        ...
        if (_id == 0) {
            //ETH Payment
            require(msg.value >= allowedCrypto[0].price * _quantity, "Not enough ETH send");
            _handleEthTransfer(owner(), msg.value);
        } else {
            // ERC20 Payment
            require(allowedCrypto[_id].enabled, "This Erc20 Token is disabled");
            IERC20 tokenAddress = IERC20(allowedCrypto[_id].addr);
            uint256 totalPrice = allowedCrypto[_id].price * _quantity;
    ...
    function multiMintbyref(
        ...
        if (_id == 0) {
            //ETH Payment
            require(msg.value >= allowedCrypto[0].price * _quantity, "Not enough ETH sent");
 ...
 require(
        ...
        require(getSignerWithPrice(_toAddress, _quantity, _price, signature) == signerAddress, "ECDSA check failed");
         ...
         if (_id == 0) {
            ...
            IERC20 tokenAddress = IERC20(allowedCrypto[_id].addr);
            uint256 totalPrice = _price * _quantity;
            require(tokenAddress.transferFrom(msg.sender, owner(), totalPrice), "transfer to owner failed");
        }
        _safeMint(_msgSender(), _quantity);
        emit MintedNft(_quantity, _msgSender());
    }
    ...
    function transferFrom(
        ...
        require(!isBlackListed[from], "Black Listed");
