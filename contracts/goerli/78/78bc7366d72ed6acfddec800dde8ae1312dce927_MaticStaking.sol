    ...
    OwnableUpgradeable
{
    ...
    // IRootChain private _rootChain;
    IRootChainManager private _rootChainManager;

    // REAL VARIABLES

    PolygonERC20Predicate private _maticPredicate;

    bool public isPolygon;
    ...
    mapping(address => uint256) private _bondRequests;
    mapping(address => uint256) private _certRequests;

    address[] private _bondQueue;
    ...
    modifier isPolygonChain() {
        require(isPolygon, "Access: your chain is not Polygon");
        _;
    }
    ...
    function initialize(
        ...
        _matic = maticAddress;
        _maticPredicate = PolygonERC20Predicate(maticPredicate);
    ...
    /// @param data Proof of a Pos burn event
    function startExist(
        address receiver,
        uint256 amount,
        bool isRebasing,
        ...
        // add to queue like a Bond if isRebasing == true
        Request memory request = Request(receiver, amount, isRebasing);
        _requests.push(request);
        emit StartExist(receiver, amount, isRebasing);
    }

    function delegateLast() external override {
        require(_requests.length > 0, "have not got any requests");
        Request memory request = _requests[_requests.length - 1];
        _rootChainManager.processExits(_matic); // @TODO to add processExitsBatch
        // // delegate via StakeFi PolygonPool
        // if (request.isRebasing) {
        //     _pool.stakeAndClaimBondsFor(request.account, request.amount);
        // } else {
        //     _pool.stakeAndClaimCertsFor(request.account, request.amount);
        // }
        _requests.pop();

        emit Delegated(request.account, request.amount, request.isRebasing);
    }
...
interface IMaticStaking {
    ...
    event StartExist(address indexed receiver, uint256 amount, bool isRebasing);

    event Delegated(address indexed staker, uint256 amount, bool isRebasing);
