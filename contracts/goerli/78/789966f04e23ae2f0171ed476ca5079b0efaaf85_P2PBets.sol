...
contract P2PBets is Ownable {
  ...
  function rollbackBet(uint betId) onlyOwner public {
    ...
    balance[poolBets[betId].gambler] += poolBets[betId].totalBet;
    if (poolBets[betId].status == 1) {
        balance[poolBets[betId].accepter] += poolBets[betId].totalBet;
    }
  }
  ...
  function unstuck() onlyOwner public {
    bool sent = tokenPlay.transferFrom(address(this), msg.sender, tokenPlay.balanceOf(address(this)));
    require(sent, "funds has to be sent");
  }
