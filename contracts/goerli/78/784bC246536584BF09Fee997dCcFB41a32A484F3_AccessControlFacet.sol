...
import {AccessControlEnumerableUpgradeable} from "../../transpiled-oz/access/AccessControlEnumerableUpgradeable.sol";
import {LibDiamond} from "../../shared/libraries/LibDiamond.sol";
import {Modifiers} from "../Modifiers.sol";

contract AccessControlFacet is AccessControlEnumerableUpgradeable, Modifiers {
    // =============================================================
    //                           Overloads
    // =============================================================

    function grantRole(bytes32 _role, address _account) public override requireOwnerOrRole(ROLE_GRANTER_ROLE) {
        _grantRole(_role, _account);
    }

    function revokeRole(bytes32 _role, address _account) public override requireOwnerOrRole(ROLE_GRANTER_ROLE) {
        _revokeRole(_role, _account);
    }

    function addSupportedInterface(bytes4 _interfaceId) external requireOwnerOrRole(ADMIN_ROLE) {
        LibDiamond.diamondStorage().supportedInterfaces[_interfaceId] = true;
    }

    function removeSupportedInterface(bytes4 _interfaceId) external requireOwnerOrRole(ADMIN_ROLE) {
...
library AccessControlEnumerableStorage {
    ...
    bytes32 internal constant STORAGE_SLOT = keccak256("omnificient.contracts.storage.AccessControlEnumerable");
