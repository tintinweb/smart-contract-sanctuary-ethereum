...
// File contracts/interface/IGovernanceToken.sol
...
pragma solidity ^0.8.4;
/**
 * @dev This token serves as an index token, which adjust its value to staking index. 
 * At the moment governing function is disabled, until the protocol large enough to
 * be governed by public investors.
 */

contract GovernanceToken is IGovernanceToken, ERC20 {
    ...
    modifier onlyStakingContract() {
        require(msg.sender == stakingContract, "Only approved");
        _;
    }
    ...
    constructor( address _stakingToken) ERC20("Governance Script", "gSCRIPT") {
        require(_stakingToken != address(0), "Zero address: Staking Token");
        stakingToken = IStakingToken(_stakingToken);
        
    }

    function pushStakingContract(address _staking) external onlyInitializing {
        require(_staking != address(0), "Zero address: Staking Contract");
    ...
    function mint(address _to, uint256 _amount) external override onlyStakingContract {
        _mint(_to, _amount);
    }

    function burn(address _from, uint256 _amount) external override onlyStakingContract {
        _burn(_from, _amount);
    }
