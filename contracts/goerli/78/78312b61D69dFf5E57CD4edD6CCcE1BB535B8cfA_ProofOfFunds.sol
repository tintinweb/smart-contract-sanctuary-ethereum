...
contract ProofOfFunds is IProofOfFunds, OwnableUpgradeable {
   ...
   */
   // Funds events
   event fundsDeposited(address indexed depositor, string indexed token, uint256 amount, string indexed reason);
   event fundsRequested(address indexed requestor, string indexed name, Roles receiver, uint256 amount, string indexed reason);
   ...
   //Signer events
   event signerRequested(uint256 transactionID, address signer, string name, string kyc);
   ...
   modifier onlyAcceptedTransaction(TransactionType _transaction) {
      ...
      if(_transaction == TransactionType.WITHDRAW) {
         require(withdrawTransaction[withdrawID].signList[true].length > (pof.noOfSigners - withdrawTransaction[withdrawID].signList[true].length), 'PoF :: Not enough signers that accepted');
      }
      else {
         require(signerTransaction[signerID].signList[true].length > (pof.noOfSigners - signerTransaction[signerID].signList[true].length), 'PoF :: Not enough signers that accepted');
      }
      _;
   }
   ...
   modifier onlyDeclinedTransaction(TransactionType _transaction) {
      ...
      if(_transaction == TransactionType.WITHDRAW) {
      require(
        withdrawTransaction[withdrawID].signList[false].length > (pof.noOfSigners - withdrawTransaction[withdrawID].signList[false].length) ||
        block.timestamp > (withdrawTransaction[withdrawID].timeCreated + pof.timeToSign),
        'POF :: Not enough signers that declined'
      );
      }
      else {
         require(
         signerTransaction[signerID].signList[false].length > (pof.noOfSigners - signerTransaction[signerID].signList[false].length) ||
         block.timestamp > (signerTransaction[signerID].timeCreated + pof.timeToSign),
         'POF :: Not enough signers that declined'
         );
      }
   ...
   modifier onlyOneVote(TransactionType _transaction, uint256 _transactionID) {
      if(_transaction == TransactionType.WITHDRAW){
        require(!withdrawTransaction[_transactionID].hasSigned[msg.sender], 'PoF :: You already signed.');
      }
      else {
         require(!signerTransaction[_transactionID].hasSigned[msg.sender], 'PoF :: You already signed.');
      }
      _;
   }
  ...
  function requestAddSigner(
    ...
    require(!signerTransaction[pof.tsTransaction].active, 'PoF :: There is still a pending transaction');
  ...
  function signSignerRequest(uint256 _transactionID, bool _isApprove) external onlyRegisteredSigner onlyOneVote(TransactionType.SIGNER, _transactionID){
    require(signerTransaction[_transactionID].active, 'PoF :: Transaction is not active');
  ...
  function processSignerRequest(uint256 _transactionID) external onlyRegisteredSigner {
  ...
  function depositFund(string memory _token, uint256 _amount, string memory _reason) external {
      IERC20Upgradeable token = IERC20Upgradeable(tokenData[_token].contractAddress);
  ...
  function requestFundWithdraw(
    ...
    require(!withdrawTransaction[pof.twTransaction].active, 'PoF :: There is still a pending transaction');
    pof.twTransaction++;
    ...
    emit fundsRequested(msg.sender, signerData[msg.sender].name, _receiver, _amount, _reason);
  }

  function signFundRequest(uint256 _transactionID, bool _isApprove) external onlyRegisteredSigner {
    require(withdrawTransaction[_transactionID].active, 'PoF :: Transaction is not active');
  ...
  function processFundRequest(uint256 _transactionID) external onlyRegisteredSigner {
    WithdrawTransaction storage transaction = withdrawTransaction[_transactionID];

    if(keccak256(abi.encodePacked(transaction.token)) == keccak256(abi.encodePacked('eth'))) {
    ...
    IERC20Upgradeable token = IERC20Upgradeable(tokenData[transaction.token].contractAddress);
    token.transfer(roleAddress[transaction.receiver], transaction.amount);
    withdrawTransaction[_transactionID].active = false;

    emit fundsWithdrawn(roleAddress[transaction.receiver], transaction.amount);
  }
  ...
  function checkSignerDecision(uint256 _transactionID) public view returns(bool) {
    if(
      signerTransaction[_transactionID].signList[false].length > (pof.noOfSigners - signerTransaction[_transactionID].signList[false].length) ||
         block.timestamp > (signerTransaction[_transactionID].timeCreated + pof.timeToSign)
    ) {
      return false;
    }
    else if(signerTransaction[_transactionID].signList[true].length > (pof.noOfSigners - signerTransaction[_transactionID].signList[true].length)){
      return true;
    }
  ...
  function checkWithdrawDecision(uint256 _transactionID) public view returns(bool) {
    if(
      withdrawTransaction[_transactionID].signList[false].length > (pof.noOfSigners - withdrawTransaction[_transactionID].signList[false].length) ||
         block.timestamp > (withdrawTransaction[_transactionID].timeCreated + pof.timeToSign)
    ) {
      return false;
    }
    else if(withdrawTransaction[_transactionID].signList[true].length > (pof.noOfSigners - withdrawTransaction[_transactionID].signList[true].length)){
      return true;
    }
  ...
  function setTokenAddress(string memory _token, address _address) external onlyOwner {
