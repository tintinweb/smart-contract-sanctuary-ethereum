    ...
    DefaultOperatorFilterer
{
    ...
    uint256 private constant MAX_TEAMMEMBER_MINT = 50;
    ...
    function teamMemberMint(uint256 mintNumber) external {
        bytes32 currentStage = getCurrentStage();
        if (currentStage == WARM_UP) {
            revert Errors.MintNotStarted();
        }
        ...
        _verifyMintNumber(mintNumber, MAX_TEAMMEMBER_MINT);
...
contract IdentityManage is IIdentityManage, APEXAccessControl {
    ...
    mapping(address => bool) private _isTeamMember;
    ...
    function setTeamMembers(
        address addr,
        bool setAsTeamMember
    ) external override onlyRole(BUSINESS_MANAGER) {
        _isTeamMember[addr] = setAsTeamMember;
    }
    ...
    function isTeamMember(address user) public view override returns (bool) {
        return _isTeamMember[user];
    }
}
...
interface IIdentityManage {
    ...
    /// @dev set if the address is team member
    /// @param addr the address to set
    /// @param isTeamMember if `addr` is team member
    function setTeamMembers(address addr, bool isTeamMember) external;
    ...
    /// @dev if `user` is team member
    /// @param user the `address` to be verified
    function isTeamMember(address user) external view returns (bool);
}
