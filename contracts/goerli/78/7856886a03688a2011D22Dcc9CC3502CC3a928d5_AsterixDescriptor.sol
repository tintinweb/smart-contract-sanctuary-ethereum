...
// File: contracts/IAsterixDescriptor.sol
...
// File: contracts/AsterixDescriptor.sol
...
contract AsterixDescriptor is IAsterixDescriptor {
    struct Color {
        string value;
        string name;
    }
    ...
    function tokenURI(uint256 tokenId, uint256 seed) external pure override returns (string memory) {
        ...
        Trait memory row1 = getrow1(seed / 100000000000000, colors[0]);
        Trait memory row2 = getrow2(seed % 1000000000000 / 10000000000, colors[1]);
        Trait memory row3 = getrow3(seed % 100000000 / 1000000, colors[2]);
        Trait memory row4 = getrow4(seed % 10000 / 100, colors[3]);
            ...
            abi.encodePacked(
                ...
                Base64.encode(
                    bytes(
                        abi.encodePacked(
                            '{',
                            '"name":"Asterix #', tokenId.toString(), '",',
                            '"description":"', description, '",',
                            '"image": "', 'data:image/svg+xml;base64,', encodedSvg, '",',
                            '"attributes": [{"trait_type": "row1", "value": "', row1.name,' (',row1.color.name,')', '"},',
                            '{"trait_type": "row2", "value": "', row2.name,' (',row2.color.name,')', '"},',
                            '{"trait_type": "row3", "value": "', row3.name,' (',row3.color.name,')', '"},',
                            '{"trait_type": "row4", "value": "', row4.name,' (',row4.color.name,')', '"},',
    ...
    function getrow1(uint256 seed, uint256 colorSeed) private pure returns (Trait memory) {
    ...
    function getrow2(uint256 seed, uint256 colorSeed) private pure returns (Trait memory) {
        Color memory color = getColor(colorSeed);
        string memory content;
        string memory name;
        if (seed == 10) {
    ...
    function getrow3(uint256 seed, uint256 colorSeed) private pure returns (Trait memory) {
        Color memory color = getColor(colorSeed);
        string memory content;
        string memory name;
        if (seed == 10) {
    ...
    function getrow4(uint256 seed, uint256 colorSeed) private pure returns (Trait memory) {
        Color memory color = getColor(colorSeed);
        string memory content;
        string memory name;
        uint256 y;
