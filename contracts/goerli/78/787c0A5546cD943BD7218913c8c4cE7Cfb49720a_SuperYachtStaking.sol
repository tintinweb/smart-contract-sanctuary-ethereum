...
contract SuperYachtStaking is Ownable, ReentrancyGuard {
    using SafeERC20 for IERC20Metadata;

    uint256 private constant PERCENT_PRECISION = 10000;

    uint256 public constant BLOCKS_PER_DAY = 7160;
    ...
    // The earned token
    IERC20Metadata public earnedToken;
    ...
    function deposit(uint256 _amount, uint8 _stakeType) external payable nonReentrant {
        ...
        if(whitelist != ZERO) {
            require(IWhitelist(whitelist).whitelisted(msg.sender), "not whitelisted");
        }
    ...
    function compoundDividend(uint8 _stakeType) external payable nonReentrant {
        ...
        for(uint256 j = 0; j < stakes.length; j++) {
            ...
            if(address(stakingToken) != address(dividendToken) && _pending > 0) {
                if(address(dividendToken) == ZERO) {
                    address wethAddress = IUniswapV2Router02(uniRouterAddress).WETH();
    ...
    function emergencyWithdraw(uint8 _stakeType) external nonReentrant {
        require(activeEmergencyWithdraw, "Emergency withdraw not enabled");
        if(_stakeType >= lockups.length) return;
    ...
    function availableDividendTokens() public view returns (uint256) {
        ...
        uint256 _amount = IERC20Metadata(dividendToken).balanceOf(address(this));
    ...
    *************************/
    function harvest() external onlyOwner {
        ...
        if(reflections > 0) {
            _transferToken(dividendToken, feeReceiver, reflections);
    ...
    function setSettings(
        ...
        require(_feeAddr != ZERO, "Invalid address");

        slippageFactor = _slippageFactor;
        uniRouterAddress = _uniRouter;
        reflectionToStakedPath = _reflectionToStakedPath;
        earnedToStakedPath = _earnedToStakedPath;
        feeReceiver = _feeAddr;
    ...
    *************************/
    ...
    function _safeSwap(
        ...
        IERC20Metadata(_path[0]).safeApprove(uniRouterAddress, _amountIn);
