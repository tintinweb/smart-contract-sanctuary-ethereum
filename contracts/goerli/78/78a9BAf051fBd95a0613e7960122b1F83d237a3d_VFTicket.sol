 ...
 */
contract ERC721VFT is Context, ERC165, IERC721, IERC721Metadata, IERC721VF {
    ...
    // Mapping from token ID to holders address
    ...
    function balanceOfHolder(
        ...
        return _holderBalances[holder] + _balances[holder];
    }
    ...
    function holderOf(uint256 tokenId) public view virtual returns (address) {
        ...
        require(
            holder != address(0),
            "ERC721: holder query for nonexistent token"
        );
        return holder;
    }
    ...
    function approve(address to, uint256 tokenId) public virtual override {
        address owner = ERC721VFT.ownerOf(tokenId);
    ...
    function assignHolder(address to, uint256 tokenId) public virtual {
        require(
            ERC721VFT.ownerOf(tokenId) == _msgSender(),
            "ERC721VF: assign holder from incorrect owner"
        );
        _assignHolder(to, tokenId);
    }

    function clearHolder(uint256 tokenId) public virtual {
        require(
            ERC721VFT.ownerOf(tokenId) == _msgSender(),
            "ERC721VF: clear holder from incorrect owner"
        );
        _clearHolder(tokenId);
    }
    ...
    /**
     * @dev See {IERC721VF-tokensOfHolder}.
     */
    function tokensOfHolder(
        address holder
    ) public view returns (uint256[] memory ownerTokens) {
        address currentHolderAddress;
        uint256 tokenCount = balanceOfHolder(holder);
        ...
        if (tokenCount == 0) {
            ...
            uint256 index;
            for (index = 0; resultIndex != tokenCount; index++) {
                currentHolderAddress = _holders[index];
                if (holder == address(0)) {
                    currentHolderAddress = _owners[index];
                }
                if (currentHolderAddress == holder) {
                    result[resultIndex++] = index;
                }
            }
    ...
    /**
     * @dev See {IERC721VF-tokensOfHolderIn}.
     */
    function tokensOfHolderIn(
        address holder,
        uint256 startIndex,
        uint256 endIndex
    ) public view returns (uint256[] memory ownerTokens) {
    ...
    function _transfer(
        ...
        require(
            ERC721VFT.ownerOf(tokenId) == from,
        ...
        // Clear holder from the previous owner
        address holder = _holders[tokenId];
        if (holder != address(0)) {
            delete _holders[tokenId];
            _holderBalances[holder] -= 1;
        }
    ...
    function _assignHolder(address to, uint256 tokenId) internal virtual {
        require(
            to != address(0),
            "ERC721VF: assign holder to the zero address"
        );
        require(
            to != _msgSender(),
            "ERC721VF: assign holder to the same address"
        );

        _holderBalances[to] += 1;
        _holders[tokenId] = to;
    }

    function _clearHolder(uint256 tokenId) internal virtual {
        address holder = _holders[tokenId];
        require(holder != address(0), "ERC721VF: holder already cleared");
    ...
    function _approve(address to, uint256 tokenId) internal virtual {
        ...
        emit Approval(ERC721VFT.ownerOf(tokenId), to, tokenId);
    }
...
import "./erc721vf/contracts/ERC721VFT.sol";
import "./VFAccessControl.sol";
import "./IVFAccessControl.sol";
...
contract VFTicket is ERC721VFT, IERC2981, DefaultOperatorFilterer {
    //Token base URI
    ...
    error MissingRequiredRole();
    error MintingPermanentlyLocked();
    error MintIsNotActive();
    error BurnIsNotActive();
    error ContractDoesNotSupportInterface();
    error AddressQuantitiesNeedToBeEqualLength();
    ...
    modifier notLocked() {
        if (mintingPermanentlyLocked) {
            revert MintingPermanentlyLocked();
        }
        _;
    }
    ...
    ) public onlyRole(_controlContract.getAdminRole()) {
        _baseUri = baseUri;
    }
    ...
    function setControlContract(
        ...
        if (
            ...
            revert ContractDoesNotSupportInterface();
        }
        _controlContract = VFAccessControl(controlContractAddress);
    }
        ...
        mintActive
    {
        if (addresses.length != quantities.length) {
            revert AddressQuantitiesNeedToBeEqualLength();
        }
    ...
    function burn(
        address from,
        uint256 tokenId
    ) external onlyRole(_controlContract.getBurnerRole()) burnActive {
        _burn(from, tokenId);
    }
