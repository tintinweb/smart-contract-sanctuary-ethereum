...
contract test12312321321 is ERC721A, Ownable {
    ...
    constructor(string memory initBaseURI) ERC721A("testt", "GG") {
        baseURI = initBaseURI;
    }

    function mintGobblerGoblins(uint256 count) external payable {
    ...
    function claimGobblerGoblins(address _to, uint256 count) external payable {
        require(mintStarted, "Minting is not live yet.");
        uint256 cost = (msg.value == 0 && !isFreeClaimed_[msg.sender]) && count == maxFreeMintPerWallet ? 0 : mintPrice;
        require(_mintedPerWallet[msg.sender] + count <= maxMintPerTx, "Max per wallet reached.");
        require(msg.value >= count * cost, "Please send the exact amount.");
