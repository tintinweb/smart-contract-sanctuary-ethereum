...
contract HTest is Ownable, ERC721A, ReentrancyGuard {
    ...
    /// @notice Owner has claimed auction proceeds
    ...
    event AuctionProceedsClaimed(address indexed owner, uint256 amount);

    event NewBaseURI(string newURI, address updatedBy);
    event UpdatedAuctionStartTime(uint256 period, address updatedBy);
    event UpdatedAuctionEndTime(uint256 period, address updatedBy);

    constructor(
        string memory _NFT_NAME,
        string memory _NFT_SYMBOL,
        uint256 _MAX_BATCH_SIZE,
        uint256 _MAX_SUPPLY,
        uint256 _MIN_BID_PRICE,
        uint256 _AUCTION_START_TIME,
        uint256 _AUCTION_END_TIME
    ) 
        ERC721A(
            _NFT_NAME,
            _NFT_SYMBOL,
        ...
        )
    {
        ...
        AUCTION_START_TIME = _AUCTION_START_TIME;
    ...
    function enterBid(uint256 quantity, uint256 price) external payable callerIsUser {
        ...
        uint256 remainingSupply = MAX_SUPPLY - numBids;
    ...
    ///@notice mints NFTs for winning bids
    function mint(address addr, uint256 quantity) private {
        require(quantity <= MAX_BATCH_SIZE, "Too many batch mint");
        _safeMint(addr, quantity);
    }

    ///@notice mints NFTs for winning bids, and refunds all remaining contributions
    function claim() public nonReentrant callerIsUser {
        require(balanceContributed[msg.sender] > 0, "Have nothing to claim");
        ...
        for(uint256 i = 0; i < winningBidIds.length; i++) {
            ...
            balanceContributed[msg.sender] -= qty * clearingPrice;
            curBid.quantity = 0;
            claimedNFTCount += qty;
        }

        require(claimedNFTCount <= MAX_BATCH_SIZE, "Too many batch mint");
        require(totalSupply() + claimedNFTCount <= MAX_SUPPLY, "Would exceed max supply");
        
        mint(msg.sender, claimedNFTCount);

        //refund any contributions not spent on mint
        (bool sent, ) = payable(msg.sender).call{value: balanceContributed[msg.sender]}("");
        require(sent, "Unsuccessful in refund");

        //set contribution to zero to avoid double claiming
        balanceContributed[msg.sender] = 0;

        emit Claimed(msg.sender, claimedNFTCount);
    }
    ...
    ///@notice Set the base URI of NFT token by the owner.
    function setBaseURI(string calldata baseURI) external onlyOwner {
        _baseTokenURI = baseURI;
        emit NewBaseURI(baseURI, msg.sender);
    }

    ///@notice Set the not revealed URI of the NFT collection by the owner.
    function setNotRevealedURI(string memory _notRevealedURI) external onlyOwner {
        notRevealedUri = _notRevealedURI;
    }

    ///@notice Reveal/Hide the real URI of NFT token by the owner.
    function reveal() external onlyOwner() {
        revealed = !revealed;
    }

    function setAuctionStartTime(uint256 _startTime) external onlyOwner {
        require(_startTime < AUCTION_END_TIME, "The start time must be less than end time");
        AUCTION_START_TIME = _startTime;
        emit UpdatedAuctionStartTime(_startTime, msg.sender);
    }

    function setAuctionEndTime(uint256 _endTime) external onlyOwner {
        require(_endTime > AUCTION_START_TIME, "The end time must be greater than start time");
        AUCTION_END_TIME = _endTime;
        emit UpdatedAuctionEndTime(_endTime, msg.sender);
    }
    ...
    function withdrawProceeds() external onlyOwner nonReentrant {
        ...
        emit AuctionProceedsClaimed(msg.sender, proceeds);
...
///@notice a min priority queue implementation
library MinPriorityQueue {
    ...
    ///to the one with the smaller quantity 
