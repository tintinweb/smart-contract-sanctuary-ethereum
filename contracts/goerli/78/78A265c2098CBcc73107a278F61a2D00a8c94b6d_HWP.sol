...
contract HWP is ERC721A, Ownable, ReentrancyGuard {
...
//    uint256 public constant mintPrice = 0.01 ether;
    ...
    uint256 public publicSaleSupply = 1000;
//    uint256 public reservedSupply = 500;
    uint256 public reservedSupply = 5;
    uint256 public VIPSaleStartTime = 1667055600; //2022-10-30 00:00:00 +09:00
    uint256 public whiteListSaleStartTime = 1667206800; //2022-10-31 18:00:00 +09:00
    uint256 public publicSaleStartTime = 1667221200;//2022-10-31 22:00:00 +09:00
    bytes32 public whiteListRoot;
    string public baseExtension = '.json';
    string public baseURI;
    mapping(address => uint256) public VIPList;

    constructor() ERC721A("HWP", "HWP") {
        _safeMint(msg.sender, reservedSupply);
    }

    function wlMint(uint256 _mintCount, uint256 _maxCount, bytes32[] calldata _merkleProof)
        external
        payable
        nonReentrant
    {
        require(whiteListSaleStartTime <= block.timestamp , "can not mint, is not white list sale");
        require(_mintCount > 0, 'can not mint, invalid value');
        require(totalSupply() + _mintCount <= maxSupply - publicSaleSupply , 'can not mint, over max size');
        require(tx.origin == msg.sender, "not eoa");
        require(msg.value == mintPrice * _mintCount, "not enough ETH");

        uint64 mintedCount = _getMintedCount(msg.sender,0);
        ...
        _setMintedCount(msg.sender, 0, mintedCount + uint64(_mintCount));
        ...
        nonReentrant
    {
        ...
        uint64 mintedCount = _getMintedCount(msg.sender,1);
        ...
        _setMintedCount(msg.sender, 1, mintedCount + uint64(_mintCount));
        _safeMint(msg.sender, _mintCount);
    }
    ...
    function setPublicSaleSupply(uint256 _supply)
        external
        onlyOwner
    {
    ...
    function setWhiteListRoot(bytes32 _root)
        external
        onlyOwner
    {
        whiteListRoot = _root;
    ...
    function setWhiteListSaleStartTime(uint256 _time)
        external
        onlyOwner
    {
    ...
    function setPublicSaleStartTime(uint256 _time)
        external
        onlyOwner
    {
    ...
    function setVIPSaleStartTime(uint256 _time)
        external
        onlyOwner
    {
    ...
    function setVIPList(address[] memory _address, uint256[] memory _value)
        external
        onlyOwner
    {
    ...
    // 00..15:WL 16..31:VIP 32..63:unused
