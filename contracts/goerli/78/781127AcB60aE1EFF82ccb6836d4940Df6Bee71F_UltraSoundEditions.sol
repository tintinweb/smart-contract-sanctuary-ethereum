...
contract UltraSoundEditions is
    ...
    Ownable
{
    IERC721Burn public proofOfStake;
    ...
    constructor(IUltraSoundDescriptor _descriptor, IERC721Burn _proofOfStake)
        ERC721("Ultra Sound Editions", unicode"Ξ")
    {
        descriptor = _descriptor;
        proofOfStake = _proofOfStake;
    ...
    function setProofOfStake(IERC721Burn _proofOfStake) external onlyOwner {
        emit ProofOfStakeUpdated(address(proofOfStake), address(_proofOfStake));
        proofOfStake = _proofOfStake;
    }
    ...
    function mint(uint256 tokenId) external override whenNotPaused {
        if (!proofOfStake.isApprovedForAll(msg.sender, address(this))) {
            revert ContractNotOperator();
        }

        if (proofOfStake.ownerOf(tokenId) != msg.sender) {
            revert MustBeTokenOwner(address(proofOfStake), tokenId);
        }
        ...
        whenNotPaused
    {
        ...
        for (uint256 i = 0; i < tokenIds.length; ) {
            if (proofOfStake.ownerOf(tokenIds[i]) != msg.sender) {
                revert MustBeTokenOwner(address(proofOfStake), tokenIds[i]);
            }
    ...
    function swapPalette(uint256 powToBurn, uint256 tokenToSwap)
        ...
        whenNotPaused
    {
        address powOwner = proofOfStake.ownerOf(powToBurn);
        ...
        if (powOwner != msg.sender) {
            revert MustBeTokenOwner(address(proofOfStake), powToBurn);
        }
    ...
    function onERC721Received(
        ...
        if (
            tokenAddress != address(proofOfStake) &&
            tokenAddress != address(this)
        ) {
            revert OnReceivedRequestFailure();
        }

        if (tokenAddress == address(proofOfStake) && data.length == 0) {
            action = 0;
...
interface IUltraSoundEditions {
    ...
    event ProofOfStakeUpdated(address orignal, address replaced);
