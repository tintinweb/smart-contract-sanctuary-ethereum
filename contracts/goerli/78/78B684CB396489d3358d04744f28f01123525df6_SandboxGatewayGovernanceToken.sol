...
library BalanceQueue {
     ...
     * @param queue QueueStorage struct from contract.
     */
    function _length(QueueStorage storage queue) internal view returns (uint256) {
     ...
     * @param queue QueueStorage struct from contract.
     */
    function _dequeue(QueueStorage storage queue) internal isNotEmpty(queue) returns (BalanceData memory data) {
     ...
     * @param queue QueueStorage struct from contract.
     */
    function _peek(QueueStorage storage queue) internal view isNotEmpty(queue) returns (BalanceData storage) {
     ...
     * @param queue QueueStorage struct from contract.
     */
    function _peekLast(QueueStorage storage queue) internal view isNotEmpty(queue) returns (BalanceData storage) {
...
abstract contract ERC20ETHless is Initializable, AccessControlEnumerableUpgradeable, ERC20Upgradeable {
    ...
    function __ERC20ETHless_init(string memory name, string memory symbol) internal initializer {
...
abstract contract ERC20Reservable is Initializable, ERC20Upgradeable {
    ...
    function __ERC20Reservable_init(string memory name, string memory symbol) internal initializer {
        __Context_init_unchained();
        __ERC20_init_unchained(name, symbol);
    ...
    function reclaim(address sender, uint256 nonce) external returns (bool success) {
        ...
        require(
            reservation._expiryBlockNum <= block.number || _msgSender() == reservation._executor,
...
abstract contract ERC20Wrapper is Initializable, AccessControlEnumerableUpgradeable, ERC20Upgradeable {
    ...
    function __ERC20Wrapper_init_unchained(IERC20Upgradeable baseToken) internal virtual onlyInitializing {
    ...
    function mint(
        ...
        bytes32 hash = keccak256(
            abi.encodePacked(GluwacoinModel.SigDomain.Mint, block.chainid, address(this), minter, amount, fee, nonce)
        );
    ...
    function burn(
        ...
        bytes32 hash = keccak256(
            abi.encodePacked(GluwacoinModel.SigDomain.Burn, block.chainid, address(this), burner, amount, fee, nonce)
        );
 ...
 * Holder of this token can retrieve the wrapped token by burning this token.
 */
contract ERC20WrapperGluwacoin is Initializable, ContextUpgradeable, ERC20Wrapper, ERC20ETHless, ERC20Reservable {
    ...
    function __ERC20WrapperGluwacoin_init_unchained(uint8 decimals_, address admin) internal onlyInitializing {
        _decimals = decimals_;
    ...
    ERC20PermitUpgradeable
{
    ...
    bytes32 private constant _STAKE_TYPEHASH =
        keccak256('stake(uint256 amount,uint256 fee,uint256 nonce,uint256 expiry)');

    bytes32 private constant _UNSTAKE_TYPEHASH =
        keccak256('unstake(uint256 amount,uint256 fee,uint256 nonce,uint256 expiry)');

    bytes32 private constant _MINTTOSTAKE_TYPEHASH =
        keccak256('mintoStake(uint256 amount,uint256 fee,uint256 nonce,uint256 expiry)');

    bytes32 private constant _DELEGATION_TYPEHASH =
        keccak256('Delegation(address delegatee,uint256 fee,uint256 nonce,uint256 expiry');
    ...
    mapping(address => BalanceQueue.QueueStorage) private _tokenDelayedBalanceCheckpoints;
    ...
    modifier onlyAdmin() {
        require(hasRole(DEFAULT_ADMIN_ROLE, msg.sender), 'ERC20StakedVotesUpgradeable: Restricted to Admin.');
        _;
    }

    function updateSupportedDAOContract(address daoContract_) external onlyAdmin returns (bool) {
    ...
    {
        return (_processingCap, _wrappingRate, _stakingLockup, _unstakingLockup);
    }
     ...
     */
    function numStakedCheckpoints(address account) public view virtual returns (uint32) {
        return SafeCastUpgradeable.toUint32(_shareholderStakedCheckpoints[account].length);
    }

    /**
     * @dev Get number of checkpoints of for `account` balance.
     */
    function numDelayedBalanceCheckpoints(address account) public view virtual returns (uint32) {
        return SafeCastUpgradeable.toUint32(_tokenDelayedBalanceCheckpoints[account]._length());
    }
     ...
     * - `blockNumber` must have been already mined
     */
    function getPastTotalStaked(uint256 blockNumber) public view virtual override returns (uint256) {
        require(blockNumber < block.number, 'ERC20Votes: block not yet mined');
    ...
    function mintToStakeBySig(
        ...
        require(expiry >= block.timestamp, 'ERC20StakedVotesUpgradeable: Sig is expired');
        address stakeholder = ECDSAUpgradeable.recover(
            _hashTypedDataV4(keccak256(abi.encode(_MINTTOSTAKE_TYPEHASH, amount, fee, nonce, expiry))),
            v,
            r,
            s
        );
    ...
    function _stake(address stakeholder, uint256 amount) internal returns (bool) {
        _transfer(stakeholder, daoContract, amount);
        _writeCheckpoint(_totalStakedCheckpoints, _add, amount, block.number);
        _writeCheckpoint(_shareholderStakedCheckpoints[stakeholder], _add, amount, block.number);
    ...
    function _unstake(address stakeholder, uint256 amount) internal returns (bool) {
        /// @dev amount must be > 0 to reduce queue item
        require(stakeOf(stakeholder) >= amount && amount > 0, 'ERC20StakedVotesUpgradeable: Invalid amount');
        _transfer(daoContract, stakeholder, amount);
        _writeCheckpoint(_totalStakedCheckpoints, _subtract, amount, block.number);
        _writeCheckpoint(_shareholderStakedCheckpoints[stakeholder], _subtract, amount, block.number);
        BalanceQueue.QueueStorage storage balanceQueue = _tokenDelayedBalanceCheckpoints[stakeholder];
    ...
    function stakeBySig(
        ...
        require(expiry >= block.timestamp, 'ERC20StakedVotesUpgradeable: Sig is expired');
        address stakeholder = ECDSAUpgradeable.recover(
            _hashTypedDataV4(keccak256(abi.encode(_STAKE_TYPEHASH, amount, fee, nonce, expiry))),
            v,
            r,
            s
        );
    ...
    function unstakeBySig(
        ...
        require(expiry >= block.timestamp, 'ERC20StakedVotesUpgradeable: Sig is expired');
        address stakeholder = ECDSAUpgradeable.recover(
            _hashTypedDataV4(keccak256(abi.encode(_UNSTAKE_TYPEHASH, amount, fee, nonce, expiry))),
            v,
            r,
            s
        );
    ...
    function stakeOf(address stakeholder) public view override returns (uint256) {
        uint256 pos = _shareholderStakedCheckpoints[stakeholder].length;
        return pos == 0 ? 0 : _shareholderStakedCheckpoints[stakeholder][pos - 1].balance;
    }
     ...
     * - `blockNumber` must have been already mined
     */
    function stakeOf(address stakeholder, uint256 blockNumber) public view override returns (uint256) {
        require(blockNumber < block.number, 'ERC20Votes: block not yet mined');
        return _checkpointsLookup(_shareholderStakedCheckpoints[stakeholder], blockNumber);
    }
    ...
    function delegateBySig(
        ...
        address signer = ECDSAUpgradeable.recover(
            _hashTypedDataV4(keccak256(abi.encode(_DELEGATION_TYPEHASH, delegatee, fee, nonce, expiry))),
            v,
            r,
            s
        );
    ...
    /// @notice Return a number of decimals of the token
    function decimals() public view override(ERC20Upgradeable, ERC20WrapperGluwacoin) returns (uint8) {
    ...
    function _writeCheckpoint(
        ...
        if (pos > 0 && (ckpts[pos - 1].fromBlock == effectiveBlockNumber)) {
            ...
            ckpts.push(
                Checkpoint({
                    fromBlock: SafeCastUpgradeable.toUint32(effectiveBlockNumber),
     ...
     */
    function getDelayedBalance(address account) external view returns (uint256) {
     ...
     */
    function _getDelayedBalance(address account) private view returns (uint256) {
        BalanceQueue.QueueStorage storage balanceQueue = _tokenDelayedBalanceCheckpoints[account];
    ...
    function _beforeTokenTransfer(
        ...
        require(from == address(0) || balanceOf(from) >= amount, 'ERC20StakedVotesUpgradeable: Insufficient balance');
...
contract GatewayGovernanceToken is ERC20StakedVotesUpgradeable {
    ...
    function __GatewayGovernanceToken_init_unchained() internal onlyInitializing {}
