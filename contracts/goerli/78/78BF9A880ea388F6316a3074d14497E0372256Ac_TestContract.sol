...
contract ERC721x is ERC721A, LockRegistry {
	...
	function transferFrom(address _from, address _to, uint256 _tokenId) public override payable virtual {
		require(isUnlocked(_tokenId), "Token is locked");
		ERC721A.transferFrom(_from, _to, _tokenId);
	}

	function safeTransferFrom(address _from, address _to, uint256 _tokenId, bytes memory _data) public override payable virtual {
		require(isUnlocked(_tokenId), "Token is locked");
...
contract TestContract is ERC721x
{
    ...
    // mapping(address => mapping(uint256 => uint256)) holderMinted;
    ...
    // // HOLDER MINT
    // function setContractA(address addr) public onlyOwner {
    //     CONTRACT_A_ADDR = addr;
    // }

    // function holderMintA(address to, uint256 tokenID, uint16 amount) public payable {
    //     require(holderMintConf.isOpen, "Holder A Mint is not available!");
    //     require(totalSupply() + amount <= maxSupply, "No more NFT!");
    //     require(CONTRACT_A_ADDR != address(0), "Contract A addr not set");
    //     require(holderMintConf.price == 0 || msg.value >= holderMintConf.price, "Insufficient value!"); 

    //     address toAddr = to == address(0) ? msg.sender : to;
    //     require(IERC721A(CONTRACT_A_ADDR).ownerOf(tokenID) == toAddr, "Don't have this nft");
    //     require(holderMintConf.maxPerTx == 0 || holderMinted[CONTRACT_A_ADDR][tokenID] + amount <= holderMintConf.maxPerTx, "TokenID is used");

    //     holderMinted[CONTRACT_A_ADDR][tokenID] += amount;
    //     _safeMint(toAddr, amount);
    // }

    // function checkHolderTokenMinted(address addr, uint16 tokenID) public view returns (uint256) {
    //     return holderMinted[addr][tokenID];
    // }
    ...
    function setIsSendMessageOpen(bool isOpen) public onlyOwner {
        isSendMessageOpen = isOpen;
    }

    function sendMessage(uint256 tokenID, string memory content) public {
        require(isSendMessageOpen, "SendMessage not open");
        Message memory m = Message(msg.sender, content);
        tokenMessage[tokenID].push(m);
    }

    function deleteMessage(uint256 tokenID, uint256 index) public {
    ...
    // function stakeToken(uint256 tokenID, bool isStake) public {
    //     require(ownerOf(tokenID) == msg.sender, "Not owner");
    //     tokenStatus[tokenID] = isStake;
    // }
    ...
    //     if (from != address(0)) {
    //         require(!tokenStatus[startTokenId], "Token Staked");
    //     }
    // }
