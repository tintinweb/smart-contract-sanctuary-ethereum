...
contract LinearVesting is ReentrancyGuard {
    ...
    uint256 public TGEDistibutePercentage = 2000;
    ...
    /**
     * @notice Update a existing vesting schedule
     * @notice A transfer is used to bring tokens into the VestingDepositAccount so pre-approval is required
     * @param _beneficiary beneficiary of the vested tokens
     * @param _amount amount of tokens (in wei)
     */
    function updateVestingSchedule(address _beneficiary, uint256 _amount) external returns (bool) {
        require(msg.sender == owner, "VestingContract::updateVestingSchedule: Only Owner");
    ...
    function drawDown() nonReentrant external returns (bool) {
        if (_getNow() <= start.add(cliffDuration.mul(4))) {
    ...
    /**
     * @notice Set TGE distribution percentage
     */
    function setTGEDistributionPercentage(uint _percentage) external {
        require(msg.sender == owner, "VestingContract::setTGEDistributionPercentage: Only Owner");
        TGEDistibutePercentage = _percentage;
    }
    ...
    function _createVestingSchedule(address _beneficiary, uint256 _amount) internal returns (bool) {
        ...
        if(vestedAmount[_beneficiary] > 0){
    ...
    function _drawDown(address _beneficiary) internal returns (bool) {
        ...
        require(block.timestamp > lastDrawnAt[_beneficiary].add(cliffDuration), "VestingContract::_drawDown: You cannot withdraw before locktime");
        ...
        uint256 lastDrawn =  lastDrawnAt[_beneficiary] == 0 ? start : lastDrawnAt[_beneficiary];

        // Update last drawn to now
        uint256 iterator = _getNow() > lastDrawn.add(cliffDuration) ?  _getNow().sub(lastDrawn).div(cliffDuration) : 0;
        lastDrawnAt[_beneficiary] = lastDrawn.add(cliffDuration.mul(iterator));
    ...
    function _drawDownBeforeLock(address _beneficiary) internal returns (bool) {
        ...
        // Update last drawn to start time
        lastDrawnAt[_beneficiary] = start;
    ...
    function _availableDrawDownAmountBeforeLock(address _beneficiary) internal view returns (uint256 _amount) {
        ...
        if (_getNow() <= start.add(cliffDuration)) {
            ...
            _amount = vestedAmount[_beneficiary].mul(TGEDistibutePercentage).div(1e4);
            return _amount;
        }
    }
    ...
    function _availableDrawDownAmount(address _beneficiary) internal view returns (uint256 _amount) {
        ...
        // Schedule is active
        uint fee = vestedAmount[_beneficiary].mul(TGEDistibutePercentage).div(1e4);
        ...
        uint256 iterator = _getNow() > timeLastDrawnOrStart.add(cliffDuration) ?  _getNow().sub(timeLastDrawnOrStart).div(cliffDuration) : 0;
        uint256 currentTime = _getNow() > timeLastDrawnOrStart.add(cliffDuration) ? timeLastDrawnOrStart.add(cliffDuration.mul(iterator)) : _getNow();
        uint256 timePassedSinceLastInvocation = currentTime.sub(timeLastDrawnOrStart);

        // Work out how many due tokens - time passed * rate per second
        uint256 drawDownRate = availableAmount.div(end.sub(start));
