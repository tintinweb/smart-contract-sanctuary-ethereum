...
interface IJembutToken {
    ...
    function fragmentToJembut(uint256 value) external view returns (uint256);

    function jembutToFragment(uint256 jembut) external view returns (uint256);
...
contract JembutChefV2 is Ownable, ReentrancyGuard {
    ...
    // JEMBUT
    IJembutToken public jembut;
    // JEMBUT LP address
    IUniswapPair public jembutLp;
    // Uniswap V2 Router
    IUniswapV2Router public router;
    // JEMBUT tokens reward per block.
    ...
    // The block number when JEMBUT mining starts.
    ...
    constructor(
        IJembutToken _jembut,
        IUniswapPair _jembutLp,
    ...
    // Update the given pool's JEMBUT allocation point. Can only be called by the owner.
    function set(
        uint256 _pid,
        uint256 _allocPoint,
        bool _withUpdate
    ...
    // View function to see pending Jembut on frontend.
        ...
        returns (uint256)
    {
        ...
        if (address(pool.lpToken) == address(jembut)) {
            lpSupply = jembut.balanceOfUnderlying(address(this));
        }
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            uint256 jembutReward = ((block.number - pool.lastRewardBlock) *
                rewardPerBlock *
                pool.allocPoint) / totalAllocPoint;
            accRewardPerShare += (jembutReward * 1e12) / lpSupply;
        }
        return
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 jembutReward = ((block.number - pool.lastRewardBlock) *
            rewardPerBlock *
            pool.allocPoint) / totalAllocPoint;
        pool.accRewardPerShare += (jembutReward * 1e12) / lpSupply;
        pool.lastRewardBlock = block.number;
    ...
    // Deposit tokens to JembutChef for JEMBUT allocation.
    function deposit(
        uint256 _pid,
        uint256 _amount,
        address _account
        ...
        if (address(pool.lpToken) == address(jembut)) {
            _amount = jembut.fragmentToJembut(_amount);
        }
    ...
    // Withdraw tokens from JembutChef.
    function withdraw(uint256 _pid, uint256 _amount) external {
        ...
        user.rewardDebt = (user.amount * pool.accRewardPerShare) / 1e12;
        if (address(pool.lpToken) == address(jembut)) {
            _amount = jembut.jembutToFragment(_amount);
        }
    ...
    function compoundBig(
        ...
        IERC20(address(jembut)).safeTransferFrom(
            msg.sender,
            address(this),
            _amountTokenDesired
        );
        IERC20(address(jembut)).approve(address(router), _amountTokenDesired);
        ...
        IERC20(address(jembut)).safeTransfer(
            msg.sender,
