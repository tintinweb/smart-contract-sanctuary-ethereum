...
interface IOFF {
    ...
    event JoinedToOFF(address to, uint256 tokenId);

    function approveToJoin(address member) external;

    function join() external;

    function leave(address member) external;
}
...
import "./interface/IOFF.sol";

contract OFF is IOFF, Ownable, ERC721Enumerable {
    using Strings for uint256;

    uint256 public numResident;
    uint256 public moveIn;
    uint256 public moveOut;

    string public baseURI;

    mapping(address => bool) residence;

    modifier isResident(address member) {
        require(residence[member], 'not a resident');
        _;
    }

    constructor() ERC721('OFFMembers', 'OFF') {}

    function approveToJoin(address member) public onlyOwner {
        residence[member] = true;
    }

    function join() public isResident(msg.sender) {
        moveIn++;
        numResident++;
        uint256 tokenId = addressToUint256(msg.sender);
        _mintTo(msg.sender, tokenId);
    }

    function leave(address member) public onlyOwner isResident(member) {
        residence[member] = false;
        moveOut--;
        numResident--;
        uint256 tokenId = addressToUint256(member);
        _burn(tokenId);
    }

    function _mintTo(address to, uint256 tokenId) internal returns (uint256) {
        _mint(owner(), to, tokenId);
        emit JoinedToOFF(to, tokenId);
        return tokenId;
    }

    function tokenURI(uint256 tokenId) public view override returns (string memory) {
        require(_exists(tokenId), 'Not a resident');
        return string(abi.encodePacked(baseURI, tokenId.toString()));
    }

    function tokenURIFromAddress(address member) public view returns (string memory) {
        uint256 tokenId = addressToUint256(member);
    ...
    function addressToUint256(address _address) internal view returns (uint256) {
