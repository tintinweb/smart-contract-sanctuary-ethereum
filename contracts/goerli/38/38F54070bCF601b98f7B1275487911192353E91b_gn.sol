...
contract gn is ERC721("say gn", "SAYGN"), IERC2981, Ownable, DefaultOperatorFilterer {
    ...
    function mint(uint8 quantity) payable public {
        ...
        if(hasMintedCount[msg.sender] > 0){
            if (msg.value < PRICE*quantity) {
                revert IncorrectAmountForMint();
            }
            if (hasMintedCount[msg.sender] != 0 && msg.value < PRICE*quantity) {
                revert IncorrectAmountForMint();
            }
            
            hasMintedCount[msg.sender] += quantity;
            setParams(quantity);
        } else if (hasMintedCount[msg.sender] == 1){
            hasMintedCount[msg.sender] += quantity;
            setParams(quantity);

        } else if (hasMintedCount[msg.sender] > 1){
            if (quantity > 1 && msg.value < PRICE*quantity) {
                revert IncorrectAmountForMint();
            }
            quantity++;
            hasMintedCount[msg.sender] += quantity;
    ...
    function setParams(uint8 quantity) internal {
