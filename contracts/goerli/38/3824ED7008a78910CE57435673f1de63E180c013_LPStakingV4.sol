...
contract LPStakingV4 is Initializable, ReentrancyGuardUpgradeable, OwnableUpgradeable {
    ...
    function _getReward(
        ...
        uint256 apr = dynamic
            ...
            : stakeDetail.APR;
        reward = dynamic
            ? stakeAmt.mul(apr).mul(_timestamp.sub(stakeDetail.lastRewardCalculated)).div(
    ...
    function _getRewards(uint stakeAmt,uint apr,uint time,uint _ethUSD) public view returns (uint256 reward) {
       
        reward =  stakeAmt.mul(apr).mul(block.timestamp.sub(time)).div(
                APRTime.mul(100).mul(1000)
            )*10**18/_ethUSD;
