...
// File: contracts/Web3Chick.sol

pragma solidity ^0.8.0;

contract Web3Chick is ERC721A, Ownable, DefaultOperatorFilterer {
  using Strings for uint256;
  ...
  mapping(address => uint256) public _totalW3CPresale;
  mapping(address => uint256) public _totalW3CMainsale;
  
  constructor() ERC721A("Web3Chick", "W3C", maxSupply, maxSupply) {
    setBaseURI("ipfs://QmNfrHjyWA8rLLtBGTJXSK8zavks5gM7JKTVAkmCDYMfCr/");
    setNotRevealedURI("ipfs://QmNYYoKiD5kFrJfFVezjDL8mChzarupQtAqs4nCNedy7fg");
  }
  ...
  // internal
    ...
    function preSaleMint(uint256 _mintAmount) public payable {
        if (msg.sender != owner()){
            ...
            require(_mintAmount <= maxW3CPerWallet, "Max Mint amount per session exceeded");
        }
        require(_mintAmount > 0, "Need to mint at least 1 W3C");
        ...
        require(_totalW3CPresale[msg.sender] + _mintAmount <= maxW3CPerWallet,
        ...
        // minting presale NFT
        _totalW3CPresale[msg.sender] = _totalW3CPresale[msg.sender] + _mintAmount;
  ...
  function saleMint(uint256 _mintAmount) public payable {
        if (msg.sender != owner()){
            require(!paused, "Public sale is not live, can't mint");
            require(_mintAmount <= setMaxW3CPerTxn, "Max mint amount exceeded");
        ...
        require(
            _totalW3CMainsale[msg.sender] + _mintAmount <= maxW3CPerWallet,
            "exceeded presale total mints per wallet"
            );
        if (msg.sender != owner()) {
            if (_giftFreeMint[msg.sender]==false){
  ...
  function setMaxW3CPerWallet(uint256 _limit) public onlyOwner() {
    maxW3CPerWallet = _limit;
  }
...
//   function to update the presale NFT amount
  ...
  function setMaxW3CPerTransaction(uint256 _newmaxMintAmount) public onlyOwner() {
    setMaxW3CPerTxn = _newmaxMintAmount;
  }
