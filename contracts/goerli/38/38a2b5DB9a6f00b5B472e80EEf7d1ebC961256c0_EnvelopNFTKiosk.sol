...
contract EnvelopNFTKiosk is TokenServiceExtended, DefaultPriceModel {
    ...
    {
        // We need two checks. 
        // 1. Only item with Default place (display and index) can be added to exact display
        // 2. Item has been transfered to this contract
        KTypes.Place memory p = assetAtDisplay[_assetItem.asset.contractAddress][_assetItem.tokenId]; 
        require(
            p.display == DEFAULT_DISPLAY && p.index == DEFAULT_INDEX, 
            "Already at display"
        );
    ...
    ) external payable
    {
        ...
        // 2.Receive payment
        ETypes.AssetItem memory payWithItem = ETypes.AssetItem(
            ETypes.Asset(
        ...
        address beneficiary = displays[p.display].items[p.index].owner;
        ...
        // We dont need remove records from data structures if
        // nft was not added with addItemToDisplay() method
        if (displays[p.display].items[p.index].prices.length > 0 
            && displays[p.display].items[p.index].nft.asset.contractAddress 
                == _assetItem.asset.contractAddress
        ...
        returns(KTypes.Price[] memory, KTypes.Discount[] memory) 
    {
        ...
        if (pl.display == DEFAULT_DISPLAY && pl.index == DEFAULT_INDEX){
            // There are two cases whith this. 
            // 1. If item was just transfered to this contract adress and
            // NOT assigned to any display 
            // THEN suppose thet item belong to DEFAULT display
            // and DEFAULT DISPLAY PRICE RULE  must applied
            if (displays[pl.display].items.length == 0 
                || (
                    displays[pl.display].items[pl.index].nft.asset.contractAddress != _assetItem.asset.contractAddress
                    &&  displays[pl.display].items[pl.index].nft.tokenId != _assetItem.tokenId
                    )
               ) 
            {
                return (
        ...
        // 2. The second case when pl.display == DEFAULT_DISPLAY && pl.index == DEFAULT_INDEX
        // is case when item was added to DEFAULT_DISPLAY at first (array index=0) place
        // Check individual price, if exist - return it
        if (displays[pl.display].items[pl.index].prices.length > 0 
            && displays[pl.display].items[pl.index].nft.asset.contractAddress 
                == _assetItem.asset.contractAddress
            && displays[pl.display].items[pl.index].nft.tokenId
                == _assetItem.tokenId     
            ) 
        {
            return (
...
/// @author Envelop Team
/// @notice This model operate sellings of erc20 collateral inside wNFTS
/// @dev ..
contract DefaultPriceModel is IDisplayPriceModel {
    ...
    ) external view virtual returns (KTypes.Price[] memory)
    {
        ...
        try wnftContract.wnftInfo(_assetItem.tokenId) returns (ETypes.WNFT memory wnft){
             KTypes.Place memory pl = kiosk.getAssetItemPlace(_assetItem);
