...
contract PrivateContribution is ReentrancyGuard, Pausable, Ownable {
    ...
    uint256 public maxContributions = 100000000000000000 wei;
    ...
    // address constant chainLinkPriceAddress = 0x639Fe6ab55C921f74e7fac1ee960C0B6293ba612; // eth price arbitrum
    address constant chainLinkPriceAddress = 0xD4a33860578De61DBAbDc8BFdb98FD742fA7028e; // eth price goerli
    ...
    function getUsdDeposit(uint256 _ethDeposit) internal view returns (uint256) {
        AggregatorV3Interface priceFeed = AggregatorV3Interface(chainLinkPriceAddress);
        (, int256 ethPriceUsd, , , ) = priceFeed.latestRoundData();
        // ETH/USD rate in 18 digit
        return (uint256(ethPriceUsd * 10000000000) * _ethDeposit) / 1e18;
    }
    ...
    function contribute(bytes32[] calldata _proof) external payable nonReentrant {
        ...
        uint256 usdDeposited = getUsdDeposit(msg.value);
        contributions.push(Contribution(msg.sender, msg.value, block.timestamp, usdDeposited));
