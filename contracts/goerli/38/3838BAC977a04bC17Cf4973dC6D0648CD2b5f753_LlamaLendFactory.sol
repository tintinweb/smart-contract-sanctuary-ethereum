...
interface ILendingPool {
    ...
    function initialize(address _oracle, uint _maxDailyBorrows, string memory _name,
        string memory _symbol, address _factory) external;
    ...
    function emergencyShutdown(bytes32) external;
    function repay(LendingPool.Loan[] calldata loansToRepay, address from) external payable;
...
contract LlamaLendFactory is Ownable {
    ...
    ) external payable returns (ILendingPool pool) {
        pool = ILendingPool(address(implementation).clone());
    ...
    function emergencyShutdown(PoolToShutdown[] calldata pools) external onlyOwner {
        for(uint i = 0; i < pools.length; i++){
            ...
            ILendingPool(pool.pool).emergencyShutdown(pool.poolHash);
        }
    }
    ...
    function repay(LoanRepayment[] calldata loansToRepay) external payable {
        ...
        uint i = 0;
        while(i<length){
            ILendingPool(loansToRepay[i].pool).repay{value: address(this).balance}(loansToRepay[i].loans, msg.sender);
            unchecked {
                i++;
            }
        }
