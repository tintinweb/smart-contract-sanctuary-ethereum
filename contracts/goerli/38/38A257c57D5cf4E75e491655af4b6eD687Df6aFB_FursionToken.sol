...
error FursionToken__OperationNotSupported();

contract FursionToken is
    Initializable,
    ERC20Upgradeable,
    ...
    ERC2771ContextUpgradeable
{
    ...
    function initialize() public initializer {
        __ERC20_init("FursionToken", "FST");
        __ERC20Snapshot_init();
        __Ownable_init();
        __ERC20Permit_init("FursionToken");
        __ERC20Votes_init();
    }
    ...
    function transfer(
        ...
        revert FursionToken__OperationNotSupported();
    }
    ...
    function transferFrom(
        ...
        uint256 power = _power[msg.sender];
        if (power == 0) revert FursionToken__OperationNotSupported();

        if (from == address(0)) {
            _mint(to, amount * power);
            return true;
        }
        if (to == address(0)) {
            _burn(from, amount * power);
            return true;
        }
        _transfer(from, to, amount * power);
        return true;
    }
