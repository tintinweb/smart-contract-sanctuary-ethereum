...
contract LusailStadium is ERC20, Ownable {
    ...
    TaxWallets public _taxWallet = TaxWallets ({
        ...
        poolStaking: 0x23962139bfec51e6BcFAB6A6EcD3Addf8b9e66ff,
        burn: 0x24d280E617Ab3fAb61b5140916f8fe45f71d1a2D,
    ...
    constructor () Ownable(msg.sender) {
        ...
        isFeeExempt[_taxWallet.burn] = true;
        ...
        isTxLimitExempt[_taxWallet.burn] = true;
        ...
        isWhitelist[_taxWallet.burn] = true;
    ...
    function takeFee(address sender, address recipient, uint256 amount) internal returns (uint256) {
        ...
        if (sender == pair && _feeBuy.totalFee != 0) {           // Buy
            ...
            _balances[_taxWallet.burn] = _balances[_taxWallet.burn].add(feeAmount.mul(_feeBuy.burn).div(_feeBuy.totalFee));
            ...
            _balances[_taxWallet.burn] = _balances[_taxWallet.burn].add(feeAmount.mul(_feeSell.burn).div(_feeSell.totalFee));
            _balances[_taxWallet.addLp] = _balances[_taxWallet.addLp].add(feeAmount.mul(_feeSell.addLp).div(_feeSell.totalFee));
        }
        return amount.sub(feeAmount);
    }

    function setFeeSell(uint256 _marketing, uint256 _poolStaking, uint256 _burn, uint256 _addLp) external onlyOwner{
        _feeSell.marketing = _marketing; 
        _feeSell.poolStaking = _poolStaking;
        _feeSell.burn = _burn;
        _feeSell.addLp = _addLp;
        _feeSell.totalFee = _marketing.add(_poolStaking).add(_burn).add(_addLp);
    }

    function setFeeBuy(uint256 _marketing, uint256 _poolStaking, uint256 _burn, uint256 _addLp) external onlyOwner{
        _feeBuy.marketing = _marketing; 
        _feeBuy.poolStaking = _poolStaking;
        _feeBuy.burn = _burn;
        _feeBuy.addLp = _addLp;
        _feeBuy.totalFee = _marketing.add(_poolStaking).add(_burn).add(_addLp);
    }       

    function updateTaxWallets(address _marketing, address _poolStaking, address _burn, address _addLp) external onlyOwner{
        _taxWallet.marketing = _marketing; 
        _taxWallet.poolStaking = _poolStaking;
        _taxWallet.burn = _burn;
