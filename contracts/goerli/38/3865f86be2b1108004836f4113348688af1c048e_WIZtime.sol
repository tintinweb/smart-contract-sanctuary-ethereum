...
contract WIZtime is Context, IERC20 {
    ...
    string private constant _name = "WIZtime";
    string private constant _symbol = "WIZtime";
    ...
    uint _feesAndLimitsStartTimestamp;
    ...
    function _transfer(
        ...
        bool feesAndLimitsAllowedAllowed = block.timestamp >= _feesAndLimitsStartTimestamp;

        if (feesAndLimitsAllowedAllowed) {
            require(
                amount <= _maxTxAmount,
                "Transfer amount exceeds the maxTxAmount."
            );
        }
        bool takeFee = feesAndLimitsAllowedAllowed;
        if (_isExcludedFromFees(from) || _isExcludedFromFees(to)) {
            takeFee = false;
        }
        if (msg.sender == address(dexRouter) || (msg.sender == lpPair && to == address(dexRouter))) {
            takeFee = false;
        }
        if (to == lpPair) {
            if (!inSwap) {
