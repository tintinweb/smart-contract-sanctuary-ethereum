...
contract NotDeployedHunter is DefaultOperatorFilterer, ERC721, Ownable, ReentrancyGuard, PaymentSplitter {
    ...
    function verifySignature(uint256 _nonce, bytes memory _signature) internal {
        ...
        bytes32 hashedSignature = keccak256(abi.encodePacked(msg.sender, nonce)).toEthSignedMessageHash();
        require(!usedSignature[signature],"Used signature");
        require(hashedSignature.recover(signature) == publicMintSigner, "invalid access");
