...
abstract contract BaseBSKR_VX is
    Utils,
    OwnableUpgradeable,
    UUPSUpgradeable,
    PausableUpgradeable,
    ...
    Manageable
{
    ...
    uint256 public _numOfBlocksGap;
    mapping(address => uint256) public _blocks;

    function __BaseBSKR_VX_init(
        uint8 version,
    ...
    function __BaseBSKR_VX_init_unchained(
        string calldata nameA,
        string calldata symbolA,
        address growth1AddressA,
        address growth2AddressA,
    ...
    /*
     * @notice Sets the gap between buy and sell blocks
     */
    function setNumOfBlocksGap(uint256 gap) external onlyOwner {
        _numOfBlocksGap = gap;
    }
...
contract LBSKR_VX is BaseBSKR_VX, DSMath, Stakable, ReentrancyGuardUpgradeable {
    ...
    function __LBSKR_VX_init(
        ...
        // __LBSKR_VX_init_unchained(inflationAddressA);
    }

    function __LBSKR_VX_init_unchained(address inflationAddressA)
        internal
        onlyInitializing
    {
    ...
    //     uint256[] memory amounts = _dexRouterV2.getAmountsOut(
    //         tokenAmount,
    //         path
    //     );
    ...
    //     uint256 feeComp;
    //     if (amounts[2] != 0) {
    //         if (_paysNoFee[_msgSender()]) {
    //             _BSKR.stakeTransfer(owner(), _inflationAddress, (amounts[2]));
    //         } else {
    //             feeComp = (amounts[2] * 25) / 10000;
    //             _BSKR.stakeTransfer(owner(), _growth1Address, feeComp * 2);
    //             _BSKR.stakeTransfer(owner(), _growth2Address, feeComp);
    //             _BSKR.stakeTransfer(owner(), address(_BSKR), feeComp);
    //             _BSKR.stakeTransfer(owner(), address(0), feeComp * 3);
    //             _BSKR.giftReflection(feeComp * 4);
    //             _BSKR.stakeTransfer(
    //                 owner(),
    //                 _inflationAddress,
    //                 (amounts[2] - 11 * feeComp)
    //             );
    //         }
    //     }
    ...
    // function _newswapTokensForTokens(
    //     uint256 tokenAmount
    // ) private returns (uint256 bskrAmount) {
    //     address[] memory path = new address[](3);
    //     path[0] = address(this);
    ...
    function _oldswapTokensForTokens(
        ...
        // //_approve(swapper, swapper, tokenAmount); // allow owner to spend his/her tokens TODO - can we do away with this statement
        _approve(swapper, address(_dexRouterV2), tokenAmount); // allow router to spend owner's tokens
    ...
    function stakeNew1(
        ...
        // uint256 amountBSKR = _swapTokensForTokens(amountLBSKR);
        ...
        if (amounts[2] != 0) {
            if (_paysNoFee[_msgSender()]) {
                amountBSKR = amounts[2];
            } else {
                feeComp = (amounts[2] * 25) / 10000;
                amountBSKR = amounts[2] - 11 * feeComp;
                _BSKR.stakeTransfer(owner(), _growth1Address, feeComp * 2);
                _BSKR.stakeTransfer(owner(), _growth2Address, feeComp);
                _BSKR.stakeTransfer(owner(), address(_BSKR), feeComp);
                _BSKR.stakeTransfer(owner(), address(0), feeComp * 3);
            ...
            _BSKR.stakeTransfer(owner(), _inflationAddress, amountBSKR);
        }
    ...
    function stakeNew2(
        ...
        _BSKR.stakeSwap(_msgSender(), _inflationAddress, amounts[2]);
        _balances[_msgSender()] -= amountLBSKR;
        _balances[owner()] += amountLBSKR;

        uint256 amountBSKR = _BSKR.balanceOf(_inflationAddress) -
            bskrBalBeforeSwap;
        uint256 sharesBSKR = (amountBSKR * totalBSKRShares) / bskrBalBeforeSwap;
    ...
    function stakeOld(uint256 amountLBSKR) external whenNotPaused nonReentrant {
        require(amountLBSKR != 0, "L: Cannot stake nothing");
        require(_balances[_msgSender()] >= amountLBSKR, "L: Too much staking");
        ...
        uint256 bskrBalBeforeSwap = _BSKR.balanceOf(_inflationAddress);
        uint256 amountBSKR = _oldswapTokensForTokens(
            address(this),
            amountLBSKR
        );
        uint256 sharesBSKR = (amountBSKR * totalBSKRShares) / bskrBalBeforeSwap;
