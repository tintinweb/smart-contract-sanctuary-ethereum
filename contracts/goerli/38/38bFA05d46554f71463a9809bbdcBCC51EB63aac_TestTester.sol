...
contract TestTester is ERC721, Ownable {
    ...
    IShitDescriptor public descriptor;

    uint public totalSupply = 10000;
    uint public maxTokensToMintPerAddress = 50;

    // Prices
    uint private price = 0.0 ether;
    ...
    constructor (IShitDescriptor newDescriptor) ERC721 ("Test Tester", "TESTER"){
		descriptor = newDescriptor;
    }

	function setDescriptor(IShitDescriptor newDescriptor) external onlyOwner {
        descriptor = newDescriptor;
    }

    function setMintingAllowed(bool value) public onlyOwner {
        mintingAllowed = value;
    }
    ...
    function mint(uint256 _amount) public payable {
        require(mintingAllowed, "Minting is not active");
        require(_amount + mintedPerWallet[msg.sender] <= maxTokensToMintPerAddress, "Not allowed to mint more than 50 tokens per wallet");

		require(_amount > 0, "Need to mint more than 1 token");
		require(totalSupply >= _amount + tokenIds.current(), "Not enough tokens left");

        for(uint256 i = 0; i < _amount; i++) {
            tokenIds.increment();
            uint256 newItemId = tokenIds.current();
			seeds[newItemId] = generateSeed(newItemId);
	...
	function getSeed(uint256 tokenId) public view returns (uint256) {
        require(_exists(tokenId), "Shit does not exist.");
