...
contract City is Ownable, IERC721Receiver, ReentrancyGuard {
    ...
    IThugCityNFT public citizen = IThugCityNFT(0x5C081e52260eF01E6654d8d7c7d807380ec022AC);
    ...
    IAssets public assets = IAssets(0x038f5C2Dac1dDd9524A46aBAa16A0e31F964e6Ae);
    ...
    function withdrawSome(uint256 amount) public nonReentrant whenNotPaused {
        require(tx.origin == msg.sender);
        Stake[] storage thugs = thugStake[msg.sender];
        Stake[] storage cops = copStake[msg.sender];
        require(amount > 0);
        ...
        if(deposited[msg.sender] >= amount){
            deposited[msg.sender] -= spending;
            spending = 0;
        }else{
            spending -= deposited[msg.sender];
        ...
        if(spending == 0){
            bills.mint(msg.sender, amount);
        }
        emit Withdraw(msg.sender, amount); // emits output of withdrawal
    }
    ...
    function crime(uint256 tokenId, uint256 location) external whenNotPaused  {
        ...
        updateRandomIndex();
        uint256 chances = getResult(location);
    ...
    function getResult(uint256 location) public virtual view returns (uint256) {
        ...
        uint256 ratio = ((locationStaked*100)/(totalCopsStaked));
        uint256 chances;
    ...
    function getAllUnclaimedMedallions(address addr) external view virtual returns (uint256 medallions) {
