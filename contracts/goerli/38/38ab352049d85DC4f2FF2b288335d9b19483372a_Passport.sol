...
/// @title Outsiders ERC-721A "Passport" contract
/// @author Kacy Fortner
...
import "../contracts/interfaces/IOutsiders.sol";
...
contract Passport is
    ...
    AccessControlEnumerable
 {
    ...
    // Events
    event OutsidersContractUpdated(address addr);
    event PassportsCreated();
    event PassportBurned(uint256 tokenId);
    event AllowListEnabledUpdated(bool isEnabled);

    // Properties
    Monotonic.Increaser public tokenIdCounter;
    IOutsiders public outsidersContract;

    constructor(
        address[] memory grounders,
        address payable beneficiary,
        address payable royaltyReceiver,
        string memory _onChainTokenURI
    ) ERC721ACommon("Passport", "PASSPORT")
      BaseTokenURI("")
      FixedPriceSeller(
            0.15 ether,
            Seller.SellerConfig({
                ...
                maxPerTx: 0,
                freeQuota: 1_112,
                lockFreeQuota: false,  // adjustable
                reserveFreeQuota: true // prevent erosion from regular purchases
            }),
            beneficiary
      )
    {
        ...
        if (bytes(_onChainTokenURI).length > 0) {
            onChainTokenURI = _onChainTokenURI;
        }
    }

    /**
     * @notice Set the Outsiders contract address.
     * @dev Only callable by the grounders role.
     */
    function setOutsidersContract(address outsidersContractAddress) external onlyRole(GROUNDERS_ROLE) {
        outsidersContract = IOutsiders(outsidersContractAddress);
        emit OutsidersContractUpdated(outsidersContractAddress);
    }

    bool public isPublicPurchaseActive = false;
 
    /**
     * @notice Set the public purchase access for 1155 tokens.
     * @dev Only callable by the grounders.
     */
    function setPublicPurchaseActive(bool active) external onlyRole(GROUNDERS_ROLE) {
        isPublicPurchaseActive = active;
    }

    bool public isMigrationEnabled = false;

    /**
     * @notice Enable public access migration of 1155 to 721 tokens.
     * @dev Only callable by the grounders.
     */
    function setMigrationEnabled(bool enabled) external onlyRole(GROUNDERS_ROLE) {
        isMigrationEnabled = enabled;
    }

    string private onChainTokenURI;

    /**
     * @notice On chain token URI storage.
     * @dev Only callable by the grounders.
     */
    function setOnChainTokenURI(string memory _onChainTokenURI) external onlyRole(GROUNDERS_ROLE) {
        onChainTokenURI = _onChainTokenURI;
    }
    ...
    /**
     * @notice Determines if an address is on the allow list.
     */
    ...
    /**
     * @notice Mint tokens purchased via the Seller.
     * @dev See {Seller-_handlePurchase}
     */
    ...
    /**
     * @notice Mint and airdrop passports to a set of addresses.
     * @dev Only callable by the grounders.
     */
    ...
    {
        for (uint256 i = 0; i < addresses.length; i++) {
            purchaseFreeOfCharge(addresses[i], 1);
        }
    }

    /**
     * @notice Mint a passport.
     */
    function mintPassport(uint16 numberToMint) external payable whenNotPaused {
        require(numberToMint > 0, "Must purchase at least 1 token");
        require(
            totalSupply() + numberToMint <= 10_000,
            "Purchase exceeds supply limit"
        );

        if (isPublicPurchaseActive == false) {
            require(
        ...
        _purchase(msg.sender, numberToMint);
        emit PassportsCreated();
    }

    // Migrate

    /**
     * @notice Migrate a Passport token to an Outsider token.
     */
    function migrateToken(uint256 tokenId) external {
        require(isMigrationEnabled, "Migration is not active");
        require(balanceOf(msg.sender) != 0, "Doesn't own a token");
        require(ownerOf(tokenId) == msg.sender, "Doesn't own thee token");
        outsidersContract.mintTransfer(msg.sender); // Mint the Outsider ERC721A token
        _burn(tokenId, true);
    ...
    */
        ...
        returns (string memory)
    {
        ...
        return string(
            abi.encodePacked(
                bytes('data:application/json;utf8,'),
                bytes('{"image":"ipfs://QmduuJtDzLBxygoqY1Fjudt8M858QdLJxE5qFsz3jsMV4a","animation_url":"ipfs://QmUnU6J2ZYfC7bFqpQZqtCXZ4q2WKH5kxapAmy1U3kXCtJ","object_url":"ipfs://QmcV2HtadVZyj1GTSEKnZGxj7uxPcCycLEy7cc7PH9eGgF","external_url":"https://outsiders.xyz","description":"Use this Passport to mint one Outsiders PFP by Gowalla.","name":"OUTSIDERS PASSPORT","attributes":[{"trait_type":"Item","value":"Passport"}]}')
            )
        );
    }
...
/**
* @dev A minimal interface for interaction with the Outsiders contract.
 */
interface IOutsiders is IERC721 {
    function mintTransfer(address to) external;
}
