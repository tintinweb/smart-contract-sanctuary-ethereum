...
contract Matrix is IMatrix, Ownable, IERC1155Receiver {
    ...
    // for test only
    IEggBuilder.ComposeCondition public lastComposeCondition;
...
*/
    function setMetadataHash(string calldata metadataHash_) external onlyOwner {
...
*/
    ...
    function findShard(
        ...
        uint256 criteria,
        uint256 skipIdx
    ) internal view returns (PickedShard memory) {
        uint256 index = 0;
        while (index < numOfGenes) {
            IShardToken.Balance[] memory balances = shardToken
                .tokenOfOwnerByIndexBatch(address(this), index, index + 20);
            for (uint256 j = 0; j < balances.length; j++) {
                if (index + j != skipIdx && balances[j].balance >= criteria) {
                    return
                        PickedShard(
                            index + j,
                            balances[j].id,
        ...
        revert("Matrix: insufficient shard amounts");
    }
        ...
        returns (IEggBuilder.ComposeCondition memory)
    {
        ...
        require(numOfGenes >= 2, "Matrix: insufficient shard ids");

        PickedShard memory shard1 = findShard(numOfGenes, 50, numOfGenes + 1);
        uint256 criteria = shard1.balance >= 99 ? 1 : 100 - shard1.balance;
        PickedShard memory shard2 = findShard(numOfGenes, criteria, shard1.idx);

        // now, we will find combination of number of shards
        uint256 offset = shard2.balance >= 99 ? 1 : 100 - shard2.balance;
        uint256[] memory ids = new uint256[](2);
        ids[0] = shard1.id;
        ids[1] = shard2.id;
        uint256[] memory amounts = new uint256[](2);
        //        uint256 numShards;
        if (offset < shard1.balance) {
            uint256 max = shard1.balance < 100 ? shard1.balance : 100;
            amounts[0] =
                (uint256(
        ...
        amounts[1] = 100 - amounts[0];
        lastComposeCondition = IEggBuilder.ComposeCondition(
            ids,
            amounts,
            _metadataHash
        );
        return lastComposeCondition;
    }

    function getLastComposeCondition()
        public
        view
        returns (IEggBuilder.ComposeCondition memory)
    {
    ...
    function _getShardPairRandomly() internal returns (uint256, uint256)
    {
        ...
        uint256 tokenId1 = shardToken.tokenOfOwnerByIndex(address(this), index1).id;
        uint256 tokenId2 = shardToken.tokenOfOwnerByIndex(address(this), index2).id;
    ...
    */
    ...
    function iMatrixInterfaceId() external pure returns (bytes4) {
        return type(IMatrix).interfaceId;
    }
