...
contract Staker is StakerStorage, OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    function sharePrice() public view returns (uint256 price) {
        ...
        price = ((totalStaked() + claimedRewards + (phiPrecision - phi) * totalRewards()) * 1e18) / (totalShares * phiPrecision);
    ...
    function _deposit(address _user, uint256 _amount) private {
        ...
        uint256 shareIncreaseUser = sharesFromAmount(_amount);
        uint256 shareIncreaseTsy = sharesFromAmount(phi * totalRewards());
    ...
    function withdrawRequest(uint256 _amount) external onlyWhitelist nonReentrant {
        ...
        uint256 shareDecreaseUser = sharesFromAmount(_amount);
        uint256 shareIncreaseTsy = sharesFromAmount(phi * totalRewards());

        // adjust global variables
        userShares[msg.sender] -= shareDecreaseUser;
        userShares[treasuryAddress] += shareIncreaseTsy;
        totalShares += shareIncreaseTsy - shareDecreaseUser;
    ...
    function compoundRewards() external nonReentrant {
        ...
        uint256 shareIncrease = sharesFromAmount(totalStaked() + amountRestaked) - totalShares; // todo is this skewed by claimedRewards?
