...
contract YakuzaElite is ERC721A, Ownable, ReentrancyGuard {
    ...
    IERC721 public constant YAKUZA = IERC721(0xf52ae754AE9aaAC2f3A6C8730871d980389a424d);
    ITempura public Tempura = ITempura(0xeb751d42213B1f0B64467416662DF43b42070Ee5);
...
*/
    ...
    function mintElite(uint256[] calldata tokens) public SaleActive nonReentrant {
        require(_totalMinted() < 334, "All Elites have been claimed");
    ...
    // function mintElite(uint256[] calldata tokens) public nonReentrant {
    //     require(_totalMinted() < MAX_SUPPLY, "All Elites have been claimed");
    //     require(tokens.length == 3 || tokens.length == 6);
    //     if (tokens.length == 3) {
    //         for (uint256 i; i < 3; i++) {
    //             YAKUZA.transferFrom(msg.sender, BURN_ADDRESS, tokens[i]);
    //         }
    ...
    function mintLegendary(uint256 legendary, uint256[] calldata tokens) external SaleActive nonReentrant {
        require(_totalMinted() < 334, "All Elites have been claimed");
    ...
    function lockTokens(uint256[] calldata tokenIds) external nonReentrant {
        ...
        for (uint256 i; i < tokenIds.length; i++) {
            ...
            if (tierByToken[tokenIds[i]] != 0) value += 20;
            else value += 10;
        }
