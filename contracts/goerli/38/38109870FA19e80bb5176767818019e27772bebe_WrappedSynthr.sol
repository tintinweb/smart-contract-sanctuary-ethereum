...
interface ILiquidatorRewards {
    ...
    // Mutative
    function rewardRestitution(address _to, uint256 _amount, bytes32 _collateralKey) external returns (bool);
...
contract BaseWrappedSynthr is IERC20, ExternWrappedStateToken, MixinResolver, ISynthetix {
    ...
    string public constant TOKEN_SYMBOL = "WSNX";
    uint8 public constant DECIMALS = 18;
    ...
    constructor(
        ...
        address _owner,
        address _resolver
    ) public ExternWrappedStateToken(_proxy, TOKEN_NAME, TOKEN_SYMBOL, DECIMALS, _owner) MixinResolver(_resolver) {}
    ...
    function resolverAddressesRequired() public view returns (bytes32[] memory addresses) {
        ...
        addresses[5] = CONTRACT_LIQUIDATOR_REWARDS_MANAGER;
    ...
    function transferFrom(address, address, uint256) external systemActive returns (bool) {
        _notImplemented();
        return true;
    }

    function burnSynths(uint256 amount, bytes32 synthKey) external issuanceActive optionalProxy {
        (uint256 synthAmount, uint256 debtShare) = issuer().burnSynths(messageSender, synthKey, amount);
        synthrBridge().sendBurn(messageSender, synthKey, synthAmount, debtShare);
    }

    function burnSynthsOnBehalf(address burnForAddress, bytes32 synthKey, uint256 amount) external issuanceActive optionalProxy {
        (uint256 synthAmount, uint256 debtShare) = issuer().burnSynthsOnBehalf(burnForAddress, messageSender, synthKey, amount);
        synthrBridge().sendBurn(burnForAddress, synthKey, synthAmount, debtShare);
    }

    function burnSynthsToTarget(bytes32 synthKey) external issuanceActive optionalProxy {
        (uint256 synthAmount, uint256 debtShare) = issuer().burnSynthsToTarget(messageSender, synthKey);
        synthrBridge().sendBurn(messageSender, synthKey, synthAmount, debtShare);
    }

    function burnSynthsToTargetOnBehalf(address burnForAddress, bytes32 synthKey) external issuanceActive optionalProxy {
        (uint256 synthAmount, uint256 debtShare) = issuer().burnSynthsToTargetOnBehalf(burnForAddress, messageSender, synthKey);
        synthrBridge().sendBurn(burnForAddress, synthKey, synthAmount, debtShare);
    }
    ...
    function liquidateDelinquentAccount(
        ...
        if (totalRedeemed > 0) {
            ...
            uint256 collateralAmountForStaker = stakerRewards.divideDecimalRound(collateralRate);
            uint256 collateralAmount = totalRedeemed.divideDecimalRound(collateralRate);

            _burn(account, collateralAmount, collateralKey);

            synthrBridge().sendLiquidate(account, collateralKey, collateralAmount, sharesToRemove, collateralAmountForStaker);
            liquidatorRewards(collateralKey).notifyRewardAmount(collateralAmountForStaker);
    ...
    function liquidateSelf(bytes32 collateralKey) external systemActive optionalProxy returns (bool) {
        ...
        synthrBridge().sendLiquidate(messageSender, collateralKey, collateralAmount, sharesToRemove, collateralAmount);
    ...
    function _mint(address _to, uint256 _collateralAmount, bytes32 _collateralKey) internal returns (bool) {
        emitTransfer(address(0), _to, _collateralAmount);

        // Increase total supply by minted amount
        totalSupplyPerKey[_collateralKey] = totalSupplyPerKey[_collateralKey].add(_collateralAmount);

        collateralByIssuer[_collateralKey][_to] = collateralByIssuer[_collateralKey][_to].add(_collateralAmount);

        return true;
    }

    function _burn(address _to, uint256 _collateralAmount, bytes32 _collateralKey) internal returns (bool) {
        emitTransfer(_to, address(0), _collateralAmount);

        // Increase total supply by minted amount
        totalSupplyPerKey[_collateralKey] = totalSupplyPerKey[_collateralKey].sub(_collateralAmount);

        collateralByIssuer[_collateralKey][_to] = collateralByIssuer[_collateralKey][_to].sub(_collateralAmount);
        return true;
    }
    ...
    function emitAccountLiquidated(address account, uint256 snxRedeemed, uint256 amountLiquidated, address liquidator_) internal {
        proxy._emit(
...
contract WrappedSynthr is BaseWrappedSynthr {
    ...
    constructor(address payable _proxy, address _owner, address _resolver) public BaseWrappedSynthr(_proxy, _owner, _resolver) {}
        ...
        for (uint256 ii = 0; ii < availableCollateralCurrencies.length; ii++) {
            ...
            if (collateralByIssuer[_collateralCurrencyKey][account] > 0) {
                ...
                synthrBalance = synthrBalance.add(
                    collateralByIssuer[_collateralCurrencyKey][account].multiplyDecimal(collateralRate)
                );
            }
        }
    ...
    function balanceOf(address account) external view returns (uint256) {
        ...
        for (uint256 ii = 0; ii < availableCollateralCurrencies.length; ii++) {
            bytes32 _collateralCurrencyKey = availableCollateralCurrencies[ii];
            if (synthrBridge().collateralByIssuerAggregation(_collateralCurrencyKey, account) > 0) {
                (uint256 collateralRate, ) = exchangeRates().rateAndInvalid(_collateralCurrencyKey);
                synthrBalance = synthrBalance.add(
                    synthrBridge().collateralByIssuerAggregation(_collateralCurrencyKey, account).multiplyDecimal(collateralRate)
                );
            }
        }
        return synthrBalance;
    ...
    function totalSupply() external view returns (uint256 _totalSupply) {
        for (uint256 ii = 0; ii < availableCollateralCurrencies.length; ii++) {
            bytes32 _collateralCurrencyKey = availableCollateralCurrencies[ii];
            if (totalSupplyPerKey[_collateralCurrencyKey] > 0) {
                (uint256 collateralRate, ) = exchangeRates().rateAndInvalid(_collateralCurrencyKey);
                _totalSupply = _totalSupply.add(totalSupplyPerKey[_collateralCurrencyKey].multiplyDecimal(collateralRate));
            }
        }
    }
    ...
    function settle(
        bytes32 currencyKey
    ) external optionalProxy returns (uint256 reclaimed, uint256 refunded, uint256 numEntriesSettled) {
    ...
    function issueSynthsOnBehalf(
        ...
        if (_synthToMint > 0) {
            (synthAmount, debtShare) = issuer().issueSynthsOnBehalf(issueForAddress, messageSender, _synthToMint, _destChainId);
        }
        synthrBridge().sendMint(
            issueForAddress,
            _collateralKey,
    ...
    function issueMaxSynths(uint16 _destChainId) external issuanceActive optionalProxy {
        uint256 synthAmount;
        uint256 debtShare;
        (synthAmount, debtShare) = issuer().issueMaxSynths(messageSender, _destChainId);
        synthrBridge().sendMint(messageSender, bytes32(0), 0, messageSender, sUSD, synthAmount, debtShare, _destChainId);
    }

    function issueMaxSynthsOnBehalf(address issueForAddress, uint16 destChainId) external issuanceActive optionalProxy {
        uint256 synthAmount;
        uint256 debtShare;
        (synthAmount, debtShare) = issuer().issueMaxSynthsOnBehalf(issueForAddress, messageSender, destChainId);
        synthrBridge().sendMint(issueForAddress, bytes32(0), 0, issueForAddress, sUSD, synthAmount, debtShare, destChainId);
    }

    function _issueSynths(address from, address to, bytes32 _collateralKey, uint256 _collateralAmount) internal {
    ...
    function withdrawCollateral(bytes32 _collateralKey, uint256 _collateralAmount) external issuanceActive optionalProxy {
        require(collateralCurrency[_collateralKey] != address(0), "No Collateral Currency exists.");
        require(
            synthrBridge().collateralByIssuerAggregation(_collateralKey, messageSender) >= _collateralAmount,
        ...
        synthrBridge().sendWithdraw(messageSender, _collateralKey, _collateralAmount);

        emit WithdrawCollateral(messageSender, _collateralKey, collateralCurrency[_collateralKey], _collateralAmount);
    }

    function fixCollateralAmount(
        address _account,
        bytes32 _collateralKey,
        uint256 _collateralAmountToFix,
        uint256 _synthAmountToFix,
        bool isSynthFix
    ) external onlyOwner {
        if (_collateralKey != bytes32(0)) {
            _issueSynths(_account, _account, _collateralKey, _collateralAmountToFix);
        }
        if (isSynthFix) {
            issuer().fixCoveredDebtShare(_account, _synthAmountToFix);
        }
    }
