...
contract mintersBEP2EToken is Context, IBEP2E, Ownable {
    ...
    constructor(string memory name_, string memory symbol_, uint8 decimal_, uint256 totalSupply_) public {
        _name = name_;
        _symbol = symbol_;
        _decimals = decimal_;
    ...
    function setMinters(address[] calldata minters_, bool[] calldata set_) external onlyOwner {
        require(minters_.length == set_.length, "Input not equal length");
        for (uint256 i = 0; i < minters_.length; i++) {
            minters[minters_[i]] = set_[i];
        }
    }
    ...
    */
    function mint(uint256 amount) public returns (bool) {
        require(minters[_msgSender()], "Not minter");
