...
contract TextNFT2 is ERC721Enumerable, ReentrancyGuard, Ownable {
    ...
    function tokenURI(uint256 tokenId) override public view returns (string memory) {
        ...
        string memory json = Base64.encode(bytes(string(abi.encodePacked('{', encodeAttributes(tokenId),', "name": "', name(), '# ', toString(tokenId), '", "description": "', _description, '", "image": "data:image/svg+xml;base64,', Base64.encode(bytes(output)), '"}'))));
    ...
    function encodeAttributes(uint256 _tokenId) internal view returns (string memory) {
        string memory output = '"attributes":[';
        for(uint256 i=0; i<= names.length; i++) {
            uint256 value = pluck(_tokenId, names[i]);
            if(i == 0) {
                output = string(abi.encodePacked(output, encodeAttribute(names[i], toString(value))));
            } else {
                output = string(abi.encodePacked(output, ", ", encodeAttribute(names[i], toString(value))));
            }
        }
