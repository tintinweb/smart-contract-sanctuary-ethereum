 ...
 *
 */
contract MintableToken is ERC20Pausable, AccessControl {
    /**
     * @dev Throws if called by any account with no admin role.
     */
    modifier onlyAdmin() {
        require(hasRole(DEFAULT_ADMIN_ROLE, _msgSender()), "Err missing default admin role");
        _;
    }
    ...
    constructor(
        ...
        require(decimals_ > 0, "Err Decimals");
        require(bytes(name_).length >= 4, "Err name");
        require(bytes(symbol_).length >= 3, "Err symbol");
    ...
    /// @param _amount qty of assetTokens to be minted
