...
// TODO: Add logic to allow users to withdraw RADAR which they wrongly sent to this contract. Don't allow them to withdraw more than they have staked ofc. :-p
...
contract RadarStake is iRadarStake, Ownable, Pausable, ReentrancyGuard {
    ...
    /** MODIFIERS */
    modifier onlyAdmin() {
        require(_admins[_msgSender()], "RadarStake: Only admins can call this");
        _;
    }
    ...
    /** ONLY ADMIN */
    // TODO: Test it well
    function addToStake(uint256 amount, address addr, uint256 apr, uint256 cooldownSeconds) external whenNotPaused onlyAdmin  {
        ...
        if (myStake.totalStaked > 0 && _totalStaked >= myStake.totalStaked) {
            ...
            // set to 0 if my stake is bigger than the amount of total staked amount (which should never happen)
            _totalStaked = 0;
        }
    ...
    // TODO: check the subtraction in this function as it can lead to issues...
    function removeFromStake(uint256 amount, address addr, uint256 apr, uint256 cooldownSeconds) external whenNotPaused onlyAdmin {
        // get current stake
        Stake memory myStake = _stakedTokens[addr];
    ...
    function setContracts(address radarTokenContractAddr) external onlyOwner {
    ...
    function allowTokensTransfers(address approvedAddress, uint256 amount) external onlyOwner {
        radarTokenContract.approve(approvedAddress, amount);
    }

    // TODO: Decide on this. Shall we keep it or not?
    // TODO: Test this well
    ...
    function withdraw() external onlyOwner {
        ...
        require(sent, "RadarStake: Failed to send funds to projectWallet");
    }
