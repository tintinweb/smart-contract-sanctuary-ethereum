...
interface INFTcontract {
    function mintWithSig(uint256 auctionType, uint256 tokenId, uint256 bidPrice, address winner, bytes32 r, bytes32 s, uint8 v) external;
}
...
contract NFTauction is Ownable, ReentrancyGuard {
    ...
    modifier OnlyCuratorORItemOwner(address nftContract, uint256 tokenId) {
        require(IERC721(nftContract).ownerOf(tokenId) == msg.sender || curators[nftContract][msg.sender] == true);
        _;
    }
        ...
        public nonReentrant
    {
        ...
        IERC20(paymentToken).safeTransferFrom(msg.sender, address(this), _bidPrice);
        IERC20(paymentToken).safeTransfer(_creator, _finalPrice);

        if (auctionType == 0) {
            INFTcontract aucNFT = INFTcontract(nftContract);
            aucNFT.mintWithSig(auctionType, tokenId, bidPrice, msg.sender, coupon.r, coupon.s, coupon.v);
        }
