...
contract RoyaltySplitterFactory is ERC2771Context {
    address public impl;
   
    event SplitterCreated(address _clone);
    ...
    function cloneContract(
        Config.PaymentSplitter memory _config
    ) external returns(address instance) {
        instance = Clones.clone(impl);
        address payable splitter = payable(address(uint160(instance)));
        RoyaltySplitter(splitter).initialize(_config);
...
contract RoyaltySplitter is ERC2771Context
{
    ...
    ERC721URIStorage membershipToken;

    constructor(address _minimalForwarder) ERC2771Context(address(_minimalForwarder)) { }
        ...
        public
    {
        ...
        membershipToken = ERC721URIStorage(
            paymentSplitterConfig.membershipToken
        );

        // Update collection contract from here
    }
    ...
    // TODO: Should be updated only by admin
    function addPayee(address account_, uint256 shares_) public {
        _addPayee(account_, shares_);
    }

    function updateMembershipToken(address membershipToken_) public {
        membershipToken = ERC721URIStorage(membershipToken_);

        emit MembershipTokenAdded(membershipToken_);
    }
}
