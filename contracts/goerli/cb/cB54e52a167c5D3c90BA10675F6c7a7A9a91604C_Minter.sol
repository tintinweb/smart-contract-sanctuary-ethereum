...
import "../interface/ILagoStable.sol";
import "../interface/IMinter.sol";
import "./SignerClient.sol";
import "openzeppelin-contracts-upgradeable/access/OwnableUpgradeable.sol";

/// @param id mint ID
/// @dev error if trying to mint the same ID twice
error AlreadyMinted(uint256 id);

/// Enables minting LagoStable tokens
contract Minter is OwnableUpgradeable, SignerClient, IMinter {
    /// @param token token minting for
    /// @param to the recipient of the mint
    /// @param amount amount of tokens minted
    /// @param id mint reference ID
    /// @dev logged when minting tokens
    event Minted(
    ...
    /// @dev stores record of mints (mint id => minted)
    mapping(uint256 => bool) public mints;

    /// @param owner_ contract owner
    /// @dev initializes Minter
    function initialize(address owner_, address signer_) external initializer {
    ...
    /// check if an ID has been minted already
    /// @param id mint ID
    /// @dev reverts if `mints[id]` is true
    modifier notMinted(uint256 id) {
        if (mints[id]) {
    ...
    /// @inheritdoc IMinter
    /// @dev calls `token.mint()` to mint tokens for the recipient
    function mint(
        address token,
        address to,
        uint256 amount,
        uint256 id
    ) external onlySigner notMinted(id) {
        mints[id] = true;
        emit Minted(token, to, amount, id);
        ILagoStable(token).lagoMint(to, amount);
    }
...
/// Helper methods for access controls.
/// Used by the Minter/Burner contracts to gate access to the
/// appropriate `Signer` contract.
contract SignerClient is ContextUpgradeable {
    /// @param current newly set signer
    /// @param previous previous signer
    event SignerUpdated(address current, address previous);

    /// @dev the contract that can call mint
    address public signer;

    // @dev upgrade gap
    uint256[50] private gap_;

    // @dev revert unless sender is signer
    modifier onlySigner() {
    ...
    function _setSigner(address signer_) internal {
        emit SignerUpdated(signer_, signer);
        signer = signer_;
    }
}
...
interface ILagoStable is IERC20Upgradeable {
    /// mint tokens for recipient
    /// @param to token recipient
    /// @param amount amount of tokens
    function lagoMint(address to, uint256 amount) external;

    /// burn tokens
    /// @param amount amount to burn
    function lagoBurn(uint256 amount) external;
}
