...
contract NftBox is VRFConsumerBaseV2, ERC721URIStorage {
    //TokenCounter
    uint256 private s_tokenCounter;
    //3 Types Of Boxes with their TokenUri
    string[] private NftBoxesUri;
    //Card Uris
    string[] private normalCardUris;
    string[] private rareCardUris;
    string[] private legendaryCardUris;
    //Nft
    struct Nft {
        uint256 tokenId;
        string tokenUri;
    ...
    constructor(
        ...
        string[] memory normalUris,
        string[] memory rareUris,
        string[] memory legendaryUris
    ) ERC721("CardBoxes", "CABO") VRFConsumerBaseV2(vrfCoordinatorV2) {
        ...
        s_tokenCounter = 0;
        legendaryCardUris = legendaryUris;
        rareCardUris = rareUris;
        normalCardUris = normalUris;
    }
    ...
    function fulfillRandomWords(
        ...
        if (nft.boxType == 0) {
            uint256 randomIndex = randomWords[0] % normalCardUris.length;
            newUri = normalCardUris[randomIndex];
        } else if (nft.boxType == 1) {
            uint256 randomIndex = randomWords[0] % rareCardUris.length;
            newUri = rareCardUris[randomIndex];
        } else if (nft.boxType == 2) {
            uint256 randomIndex = randomWords[0] % legendaryCardUris.length;
            newUri = legendaryCardUris[randomIndex];
        } else {
            revert NftBox__AlreadyOpened();
        }
        mappingNfts[tokenId] = Nft(tokenId, newUri, nft.owner, true, 3);
        _setTokenURI(tokenId, newUri);
        emit BoxOpened(nft.owner, tokenId, newUri);
    }

    function getNormalCard(uint256 index) public view returns (string memory) {
        return normalCardUris[index];
    }

    function getRareCard(uint256 index) public view returns (string memory) {
        return rareCardUris[index];
    }

    function getLegendaryCard(
        uint256 index
    ) public view returns (string memory) {
        return legendaryCardUris[index];
    }

    function getNormalCardUrisLength() public view returns (uint256) {
        return normalCardUris.length;
    }

    function getRareCardUrisLength() public view returns (uint256) {
        return rareCardUris.length;
    }

    function getLegendaryCardUrisLength() public view returns (uint256) {
        return legendaryCardUris.length;
    }
    ...
    /*  function priceFeedEthToUsd(uint256 value) public view returns (uint256) {
        (, int256 Price, , , ) = i_priceFeed.latestRoundData();
        uint256 price = ((uint256(Price) * 1e10) * value) / 1e18;
