...
contract MPBRT is ERC721MP, Ownable, ReentrancyGuard
{
    ...
    address _ArtBlocksMintingContract;
    uint public _ArtBlocksProjectID;
    uint public _Price = 0.18 ether;
    uint public _TotalAvailableForPurchase = 10;
    bytes32 public _Root;

    mapping(address=>bool) public _Admin;
    mapping(uint=>bool) public _Minted;
    mapping(uint=>bool) private _DelegateStatus;

    event LiveMintCompleteArtist(
        address Recipient, 
        uint ArtBlocksProjectID, 
        uint TicketID, 
        uint MintedWorkNumber, 
        bool DelegateStatus
    );
    ...
    ************/
     ...
     */
    function Purchase(uint Amount) public payable nonReentrant
    {
        if(_totalMinted() + Amount > _TotalAvailableForPurchase)
        {
            Amount = _TotalAvailableForPurchase - _totalMinted();
            require(Amount > 0, "MP: Sold Out");
        }
        require(_totalMinted() + Amount <= _TotalAvailableForPurchase, "MP: Overflow");
        require(msg.value == Amount * _Price, "MP: Invalid Price");
     ...
     */
    function PurchaseBrightList(uint Amount, bytes32[] calldata Proof) public payable nonReentrant
    {
        if(_totalMinted() + Amount > _TotalAvailableForPurchase)
        {
            Amount = _TotalAvailableForPurchase - _totalMinted();
    ...
    *******************/
     ...
     */
    function _LiveMintArtist(uint[] calldata TicketIDs) external onlyAdmin
    {
        for(uint TicketID; TicketID < TicketIDs.length; TicketID++)
        {
            require(!_Minted[TicketIDs[TicketID]], "MP: TicketID Minted");
            _Minted[TicketIDs[TicketID]] = true;
            address Recipient = ownerOf(TicketIDs[TicketID]);
            _burn(TicketIDs[TicketID]);
            uint _MintedWorkNumber = IArtBlocks(_ArtBlocksMintingContract).purchaseTo(Recipient, _ArtBlocksProjectID);
            emit LiveMintCompleteArtist(
                Recipient, 
                _ArtBlocksProjectID, 
                TicketIDs[TicketID], 
                _MintedWorkNumber, 
                _DelegateStatus[TicketID]
            );
        }
    }
    ...
    *******************/
     ...
     */
    function __NewArtBlocksMintingContract(address NewContract) external onlyOwner { _ArtBlocksMintingContract = NewContract; }

    /**
     * @dev Changes Price Of Sale
     */
    function __NewPrice(uint NewPrice) external onlyOwner { _Price = NewPrice; }

    /**
     * @dev Changes Available Amount To Purchase
     */
    function __NewAvailableToPurchase(uint NewAmount) external onlyOwner { _TotalAvailableForPurchase = NewAmount; }

    /**
     * @dev Changes Merkle Roothash
     */
    function __NewRoot(bytes32 NewRoot) external onlyOwner { _Root = NewRoot; }

    /**
     * @dev Flips An Admin's State
     */
    function __FlipAdminState(address Admin) external onlyOwner { _Admin[Admin] = !_Admin[Admin]; }
    ...
    *****************/
     ...
     */
    function viewBrightListAllocation(address Recipient, bytes32[] memory Proof) public view returns(bool)
    { 
    ...
    **************/
    ...
    modifier onlyAdmin
    {
        require(_Admin[msg.sender], "`msg.sender` Must Be Admin");
        _;
    }
}
