...
/// @title A liquid staking derivative for veAUXO
/// @author alexintosh
/// @notice Tokens are stakingManager in perpetuity, no coming back
/// @dev Make sure you understand how veAUXO work and the stakingManager contract does.
contract xAUXO is ERC20, ERC20Permit {
    ...
    modifier onlyGovernance() {
        require(msg.sender == governor, "not governance");
        _;
    }
    ...
    constructor(address _auxo, address _stakingManager, address _governor, uint256 _entryFee, address _feeBeneficiary)
        ...
        ERC20Permit("xAUXO")
    {
        require(_stakingManager != address(0), "ZERO_ADDR");
        AUXO = _auxo;
        governor = _governor;
        ...
        /**
         * @dev We mint the first xAUXO here to msg.sender
         ...
         * @notice we assume you sent AUXO to stakingManager first
         */

        _mint(msg.sender, IERC20(AUXO).balanceOf(stakingManager));
        IStakingManager(stakingManager).stake();
    }

    function depositFor(address account, uint256 amount) external {
        uint256 amountMinFee = _chargeFee(amount);
        _depositAndStake(account, amountMinFee);
    }

    function _chargeFee(uint256 amount) internal returns (uint256) {
        uint256 feeAmount = amount * entryFee / (10 ** 18);
        uint256 amountMinFee = amount - feeAmount;

        if (feeAmount > 0 && feeBeneficiary != address(0)) {
            IERC20(AUXO).safeTransferFrom(msg.sender, feeBeneficiary, feeAmount);
        }
    ...
    function _depositAndStake(address account, uint256 amount) internal {
        IERC20(AUXO).safeTransferFrom(msg.sender, stakingManager, amount);
        IStakingManager(stakingManager).increase();
