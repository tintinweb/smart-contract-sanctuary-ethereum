...
contract halflingsMetadataUpdate is Ownable {
      ...
      address public halflingsItems=0xC48D131a4eF38798e775490dDe54f1d6d518A702;
      ...
      mapping(uint256 =>mapping(uint256 => bool)) public _isItemAdd;
      ...
      event WithdrawERC20Token(address tokenContractAddress,uint256 amount);
      event _addItems(address user,uint256 nftid,uint256[] itemId,uint256 seed);
      event _removeItem(address user,uint256 nftid,uint256 itemId,uint256 seed);
    ...
    function addItems(
        ...
        if(_isItemAdd[nftid][attributeId[itemcount]])
        {
        require((_isItemRemoved[nftid][itemId[itemcount]]== false),"need to remove item from NFT");
        }
        _isItemAdd[nftid][attributeId[itemcount]]=true;
        ERC1155(halflingsItems).burn(_msgSender(),itemId[itemcount],1);
        }
        
        emit _addItems(_msgSender(),nftid,itemId,seed);
    }
    ...
    function removeItem(
        ...
        isSeedUsed[seed]=true;
        require((IERC721(halflingsNft).ownerOf(nftid) == _msgSender()),"caller is not owner of NFT");
        require((_isItemRemoved[nftid][itemId] == false),"item already removed from NFT");
        _isItemRemoved[nftid][itemId] = true;
        ERC1155(halflingsItems).mint(_msgSender(),itemId,1);
        emit _removeItem(_msgSender(),nftid,itemId,seed);
    }
