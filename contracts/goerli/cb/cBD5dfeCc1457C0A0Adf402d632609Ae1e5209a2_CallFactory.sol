...
abstract contract CallPoolDeployer is ICallPoolDeployer {
    ...
    function deploy(address factory, address erc721token, address oracle, address premium) internal returns (address pool) {
        ...
        NToken(ntoken).transferOwnership(msg.sender);
        CallToken(calltoken).transferOwnership(msg.sender);
        delete parameters;
    }
}
...
contract NToken is ERC721, INToken, Ownable, IERC721Receiver {
    ...
    modifier onlyPool() {
        require(_msgSender() == _pool, Errors.CP_CALLER_IS_NOT_POOL);
        _;
    }
    ...
    function initialize() external onlyOwner{
        ...
        address poolAddress = ICallFactory(factory).getPool(nft);
        _pool = poolAddress;
        emit Initialize(poolAddress);
    }
    ...
    function mint(address user, uint256 tokenId) public override onlyPool{
        _safeMint(user, tokenId);
        emit Mint(user, tokenId);
    }

    function burn(address user, address receiverOfUnderlying, uint256 tokenId) public override onlyPool{
        _burn(tokenId);
...
import {ICallFactory} from "./interfaces/ICallFactory.sol";
import {Strings} from "@openzeppelin/contracts/utils/Strings.sol";
...
contract CallToken is ERC721, IERC721Enumerable, ICallToken, Ownable {
    ...
    function tokenURI(uint256 tokenId) public view override returns (string memory) {
        require(block.timestamp <= ICallPoolState(_pool).getEndTime(tokenId), "token is expired");
    ...
    function balanceOf(address owner) public view virtual override(IERC721, ERC721) returns(uint256) {
        ...
        for(uint256 i = 0; i < _allTokens.length; ++i){
            ...
            if(ERC721.ownerOf(tokenId) == owner && currentTime <= ICallPoolState(_pool).getEndTime(tokenId)){
                balance += 1;
            }
        }
        return balance;
    ...
    function transferFrom(
        ...
        uint256 endTime = ICallPoolState(_pool).getEndTime(tokenId);
    ...
    function totalSupply() public view virtual override returns (uint256) {
        ...
        for(uint256 i = 0; i < _allTokens.length; ++i){
            uint256 tokenId = _allTokens[i];
            if(currentTime <= ICallPoolState(_pool).getEndTime(tokenId)){
                supply += 1;
            }
        }
        return supply;
    ...
    function mint(address user, uint256 tokenId) external override onlyPool {
        _safeMint(user, tokenId);
        emit Mint(user, tokenId);
    }
    ...
    function open( address user, uint256 tokenId) external override onlyPool {
...
library Errors {
    ...
    string public constant CP_CALLER_IS_NOT_POOL = "24";
}
...
library ErrorCodes {
    ...
    uint256 public constant CP_CALLER_IS_NOT_POOL = 24;
}
