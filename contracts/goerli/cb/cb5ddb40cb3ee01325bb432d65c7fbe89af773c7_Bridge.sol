...
contract Bridge is IBridge, Ownable {
    ...
    function relayBlock(
        ...
        verifyBlockHeader(multiStore, merkleParts, commonEncodedVotePart, signatures);
    ...
    function relayAndVerifyModern(bytes calldata data) public view returns(Result memory) {
        (bytes32 vs, bytes32[] memory rs, bytes32[] memory ss, Result memory result) = abi.decode(
            data,
            (bytes32, bytes32[], bytes32[], Result)
        );

        uint256 sigsCount = rs.length;
        uint256 vsInt = uint256(vs);
        require(sigsCount == ss.length && sigsCount == vsInt >> 248, "INVALID_SIGS_SIZE");

        // Counts the total number of valid signatures signed by active validators.
        address lastSigner = address(0);
        uint256 sumVotingPower = 0;
        for (uint256 idx = 0; idx < sigsCount; ++idx) {
            address signer = ecrecover(
                keccak256(abi.encode(result)), uint8((vsInt & 1) + 27), rs[idx], ss[idx]
            );
            vsInt >>= 1;
        ...
        // Verifies that sufficient validators signed the block and saves the oracle state.
        require(sumVotingPower * 3 > sigsCount >> 10, "INSUFFICIENT_VALIDATOR_SIGNATURES");
