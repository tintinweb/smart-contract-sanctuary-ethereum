...
contract MergeKingdom is ERC721Enumerable, Ownable, Pausable {
    ...
    uint32[] public chanceModifier = [1000, 500, 330];               // -0%, -50%,  -75%
    ...
    //
    //
    // SECTION CONSTRUCTOR _________________________________________________________________________________________ ____
    ...
    );  //DETAILS ⋯ "sender"      → Indicates who is sending the transaction
        //        ⋯ "firstToken"  → Indicates which is the first token to be choosed
    ...
    function prepareMultipleMerges (
        ...
        for (uint i; i < firstTokens.length; i++) {
            ...
            require (typeOf(token1) == typeOf(token2),
            ...
            emit awaitingMerge(msg.sender, token1, token2, uint32(block.number));
    ...
    function claimMultipleMerges (
        ...
        for (uint i; i < firstTokens.length; i++) {
            ...
            uint seed = uint(
                keccak256(
                    abi.encodePacked(
                        token1, 
                        token2,
                        _hashCalc(mergedTo[token1].onBlock)
                    )
                )
            );
            ...
            uint successRate = perCalcMerge(baseChance, chanceModifier[typeOf(token1)]);
            
            if (seed % 1000 < successRate) {
                ...
                emit mergeClaimed(token1, token2, successRate, seed % 1000, true);
            ...
            //DETAILS ⋯ If not
                ...
                emit mergeClaimed(token1, token2, successRate, seed % 1000, false);
            }
        }
    }
    ...
    function checkMergeChances (uint tokenId)
    external
    view
    returns (uint) {
        return perCalc(baseChance, chanceModifier[typeOf(tokenId)]);
    }
    ...
    function perCalcMerge (uint input, uint per)
    internal
    pure
    returns (uint) {
