    ...
    Ownable
{
        ...
        )
    {
        ...
        if (address(WETHLINKPair.token0()) == linkToken) {
            (LINKReserve, ETHReserve, ) = WETHLINKPair.getReserves();
        } else {
            (ETHReserve, LINKReserve, ) = WETHLINKPair.getReserves();
        }

        uint256 priceForEachLink = ETHReserve / LINKReserve;

        return ((priceForEachLink / 2) * 10**18, ETHReserve, LINKReserve);
    }
