  ...
  MarketFees
{
  ...
  function cancelBuyPrice(address nftContract, uint256 tokenId) external nonReentrant {
    ...
    _transferFromEscrowIfAvailable(nftContract, tokenId, _msgSender());
  ...
  function _buy(address nftContract, uint256 tokenId, address payable referrer) private nonReentrant {
    ...
    _transferFromEscrow(nftContract, tokenId, _msgSender(), address(0));
    ...
    emit BuyPriceAccepted(nftContract, tokenId, buyPrice.seller, _msgSender(), totalFees, creatorRev, sellerRev);
  }
...
abstract contract NFTMarketExhibition is Context {
  ...
  function _addNftToExhibition(address nftContract, uint256 tokenId, uint256 exhibitionId) internal {
    if (exhibitionId != 0) {
      ...
      if (!exhibitionIdToSellerToIsAllowed[exhibitionId][_msgSender()] && exhibition.curator != _msgSender()) {
  ...
  MarketFees
{
    ...
    returns (uint256 expiration)
  {
    ...
    if (offer.expiration.hasExpired()) {
      ...
      expiration = feth.marketLockupFor{ value: msg.value }(_msgSender(), amount);
    } else {
    ...
    emit OfferMade(nftContract, tokenId, _msgSender(), amount, expiration);
  }
  ...
  function _acceptOffer(address nftContract, uint256 tokenId) private {
    ...
    // Transfer the NFT to the buyer.
    try
      IERC721(nftContract).transferFrom(_msgSender(), offer.buyer, tokenId) // solhint-disable-next-line no-empty-blocks
    {
      ...
      _transferFromEscrow(nftContract, tokenId, offer.buyer, _msgSender());
    }
    ...
    emit OfferAccepted(nftContract, tokenId, offer.buyer, _msgSender(), totalFees, creatorRev, sellerRev);
  }
  ...
  NFTMarketAuction
{
  ...
  function placeBidV2(uint256 auctionId, uint256 amount, address payable referrer) public payable nonReentrant {
    ...
    if (endTime == 0) {
      ...
      } else if (auction.bidder == _msgSender()) {
    ...
    emit ReserveAuctionBidPlaced({ auctionId: auctionId, bidder: _msgSender(), amount: amount, endTime: endTime });
  }
...
abstract contract RouterContext is Context {
   ...
   * @dev If the msg.sender is the trusted router contract, then the last 20 bytes of the calldata is the original
   * sender.
   */
...
import "../../../../contracts/mixins/shared/RouterContext.sol";

contract $RouterContext is RouterContext {
    bytes32 public __hh_exposed_bytecode_marker = "hardhat-exposed";

    constructor(address router) RouterContext(router) {}
...
contract $NFTMarket is NFTMarket {
    ...
    constructor(address payable treasury, address feth, address royaltyRegistry, uint256 duration, address router) NFTMarket(treasury, feth, royaltyRegistry, duration, router) {}
