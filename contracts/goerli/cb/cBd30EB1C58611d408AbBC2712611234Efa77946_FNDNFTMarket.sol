...
error NFTMarketFixedPriceCollectionDrop_Referral_Sum_Must_Be_Less_Than_Basis_Points();
error NFTMarketFixedPriceCollectionDrop_Sell_Referral_Args_Must_Both_Be_Set();
...
abstract contract NFTMarketFixedPriceCollectionDrop is NFTMarketFees {
  ...
  struct SaleConfig {
    ...
    /**
     * @notice The fee for the seller referral in basis point.
     * @dev The value should be between [0, 10000], where 10000 is BASIS_POINTS.
     * e.g. If the `sellReferrerFeeInBasisPoints` is set to `1000`,
     * BASIS_POINTS/sellReferrerFeeInBasisPoints is used to calculate 10% goes to the referral beneficiary.
     */
    uint16 sellReferrerFeeInBasisPoints;
    /**
     * @notice The max fee for the buyer referral in basis point.
     * @dev The value should be between [0, 10000], where 10000 is BASIS_POINTS.
     * e.g. If the `buyReferrerMaxFeeInBasisPoints` is set to `1000`,
     * BASIS_POINTS/buyReferrerMaxFeeInBasisPoints is used to calculate 10% goes to the referral beneficiary.
     */
   ...
   * @param sellReferrerBeneficiary The address for the seller referral.
   * @param price The price per item in the drop.
   * @param limitPerAccount The limit of NFTs that can be bought per account.
   * @param buyReferrerMaxFeeInBasisPoints The fee for the buyer referral in basis point.
   * @param sellReferrerFeeInBasisPoints The fee for the seller referral in basis point.
   */
  event CreateFixedPriceSale(
    ...
    uint256 sellReferrerFeeInBasisPoints
  );
   ...
   * @param sellReferrer The address for the seller referral.
   * @param sellReferrerFeeInBasisPoints The fee for the seller referral in basis point.
   * @param buyReferrerMaxFeeInBasisPoints The fee for the buyer referral in basis point.
   * @dev Notes:
  ...
  function createFixedPriceSale(
    ...
    if (buyReferrerMaxFeeInBasisPoints + sellReferrerFeeInBasisPoints > BASIS_POINTS) {
      revert NFTMarketFixedPriceCollectionDrop_Referral_Sum_Must_Be_Less_Than_Basis_Points();
    }
    if (
      !(sellReferrerFeeInBasisPoints != 0 && sellReferrer != address(0)) &&
      !(sellReferrerFeeInBasisPoints == 0 && sellReferrer == payable(0))
    ) {
      revert NFTMarketFixedPriceCollectionDrop_Sell_Referral_Args_Must_Both_Be_Set();
    }
    ...
    saleConfig.buyReferrerMaxFeeInBasisPoints = buyReferrerMaxFeeInBasisPoints;
    saleConfig.sellReferrer = sellReferrer;
    saleConfig.sellReferrerFeeInBasisPoints = sellReferrerFeeInBasisPoints;
    emit CreateFixedPriceSale(
      nftContract,
      saleConfig.seller,
      saleConfig.sellReferrer,
      saleConfig.price,
      saleConfig.limitPerAccount,
      saleConfig.buyReferrerMaxFeeInBasisPoints,
      saleConfig.sellReferrerFeeInBasisPoints
    );
  }
  ...
  function mintFromFixedPriceSale(
    ...
    return _mintFromFixedPriceSaleAndApprove(nftContract, count, totalBuyPrice, buyReferrer, address(0));
  }
   ...
   * @dev Can only be called if the drop has begun.
   */
  function mintFromFixedPriceSaleAndApprove(
    address nftContract,
    uint16 count,
    uint256 totalBuyPrice,
    address payable buyReferrer
  ) public payable returns (uint256 firstTokenId) {
    return _mintFromFixedPriceSaleAndApprove(nftContract, count, totalBuyPrice, buyReferrer, address(this));
  }

  /* solhint-disable-next-line code-complexity */
  function _mintFromFixedPriceSaleAndApprove(
    address nftContract,
    uint16 count,
    uint256 totalBuyPrice,
    address payable buyReferrer,
    ...
    firstTokenId = ICollectionMint(nftContract).mintCountFor(count, msg.sender);
    if (marketToApprove != address(0)) {
      IERC721(nftContract).setApprovalForAll(marketToApprove, true);
    }

    if (mintCost > msg.value) {
      // Withdraw additional ETH required from their available FETH balance.
      unchecked {
        // The if above ensures delta will not underflow.
        uint256 delta = mintCost - msg.value;
        // Withdraw ETH from the buyer's account in the FETH token contract,
        // making the ETH available for `_distributeFunds` below.
        feth.marketWithdrawFrom(msg.sender, delta);
      }
    } else if (mintCost < msg.value) {
      // Return any surplus funds to the buyer.
      unchecked {
        // The if above ensures this will not underflow
        payable(msg.sender).sendValue(msg.value - mintCost);
      }
    }
    ...
    )
  {
    try ICollectionMint(nftContract).isSoldOut() returns (bool soldOut) {
      if (!soldOut) {
        ...
        sellReferrer = saleConfig.sellReferrer;
        sellReferrerFeeInBasisPoints = saleConfig.sellReferrerFeeInBasisPoints;
        buyReferrerMaxFeeInBasisPoints = saleConfig.buyReferrerMaxFeeInBasisPoints;
      }
    } catch // solhint-disable-next-line no-empty-blocks
    {
