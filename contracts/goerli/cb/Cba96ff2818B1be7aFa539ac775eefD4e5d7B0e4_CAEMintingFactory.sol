...
contract CAEMintingFactory is Pausable, AccessControl {
  ...
  /// @notice emitted when a batch of elementals are airdropped to an address
  event AirdropBatch(
    address to,
    uint32 number
  );

  /// @notice emitted when an address on a list claims a batch of elementals
  event ListClaimBatch(
    address to,
    uint32 number
  );

  /// @notice emitted when a batch of elementals is minted in the public mint
  event PublicMintBatch(
    address to,
    uint32 number
  );
  ...
  function _blocktimeBetween(uint start, uint duration) public view returns (bool) {
  ...
  function listClaimAndMint(ListClaimEnum listChoice, uint8 numToMint, bytes32[] calldata proof) external
  {
    ...
    emit ListClaimBatch(msg.sender, numToMint);
    _doMint(msg.sender, numToMint, MintSourceEnum.CLAIM_LIST);
  }
  ...
  function publicMint(uint8 numToMint) external {
    ...
    emit PublicMintBatch(msg.sender, numToMint);
   ...
   */
  function airdrop(address to, uint32 numberToMint) external onlyRole(AIRDROP_ROLE) {
    emit AirdropBatch(to, numberToMint);
    _doMint(to, numberToMint, MintSourceEnum.AIRDROP);
  }
...
contract ChampionsAscensionElemental is ERC721, Pausable, AccessControl, ERC721Burnable {
    ...
    // FIXME, remove?: bytes32 public constant GAME_ROLE = keccak256("GAME_ROLE"); // if game can change anything, e.g. "burn" a dead elemental
