...
contract KiKiStaking is IKiKiStaking, KiKiStakingToken, IsContract, Ownable, Initializable
{
    ...
    bytes32 internal constant TREASURY_POSITION = keccak256("KKS.KKS.treasury");
    bytes32 internal constant RECOVERY_VAULT_POSITION = keccak256("KKS.KKS.recoveryVault");

    /// @dev storage slot position of the staking rate limit structure
    bytes32 internal constant STAKING_STATE_POSITION = keccak256("KKS.KKS.stakeLimit");
    ...
    bytes32 internal constant SWAP_BUFFER_PERCENT_POSITION = keccak256("KKS.KKS.bufferPercent");
    /// @dev Swap min fee, based on 10000 point
    bytes32 internal constant SWAP_MIN_FEE_POSITION = keccak256("KKS.KKS.swapMinFee");
    /// @dev Swap max fee, based on 10000 point
    bytes32 internal constant SWAP_MAX_FEE_POSITION = keccak256("KKS.KKS.swapMaxFee");
    ...
    */
    ...
    function setLiquidityPoolPercent(uint16 points) external onlyOwner {        
        if (_readBPValue(SWAP_BUFFER_PERCENT_POSITION) != points) {
            ...
            uint256 liquidity = _getLiquidityPoolEther();
            uint256 liquidityPoolSize = _getLiquidityPoolSize();
            if (liquidity > liquidityPoolSize)
            {
                _updateSwapPool(liquidity - liquidityPoolSize);
            }
        }
    }
    ...
    /**
      ...
      */
    function setSwapFeePercent(uint16 minPoints, uint16 maxPoints) external onlyOwner {
    ...
    */
    function getSwapFeePointsByETH(uint256 etherAmount) public view returns (uint16) {
        ...
        uint256 swapPoolSize = _getLiquidityPoolSize();
        if (0 == swapPoolSize) {
                return 0;
        }
        if (maxFee == minFee) {
            ...
            uint256 swapPoolLiquidity = _getLiquidityPoolEther();
    ...
    function getLiquidityPoolNewStatus(uint256 _value) internal view returns (uint256 swapPoolLiquidity, uint256 bufferedEther) {
        uint256 oldSwapPoolSize = _getLiquidityPoolSize();
        uint256 liquidity = _getLiquidityPoolEther();
        // if decreased liquidity pool size, must move the superfluous ether to buffered pool
        if (_value.add(liquidity) <= oldSwapPoolSize) {
            swapPoolLiquidity = liquidity.add(_value);
            bufferedEther = _getBufferedEther();
        } else {
            uint16 swapPoolPercentPoints = _readBPValue(SWAP_BUFFER_PERCENT_POSITION);
            uint256 depositedAndBufferedEthers = getDepositedAndBufferedEthers();
            ...
            bufferedEther = etherToBuffered.add(_getBufferedEther());
        }
    }



    function _setLastTotalSwapFee(uint256 etherAmount) internal {
        TOTAL_SWAP_FEE_POSITION.setStorageUint256(etherAmount);
    }


    function _getLastTotalSwapFee() internal view returns (uint256) {
    ...
    */
    function _setProtocolContracts(address _oracle, address _treasury, address _masterChef) internal {
        ...
        require(_masterChef != address(0), "MASTERCHEF_ZERO_ADDRESS");
        ...
        emit ProtocolContactsSet(_oracle, _treasury, _masterChef);
    }
    ...
    */
    ...
    function getLiquidityPoolEther() public view returns (uint256) {
    ...
    */
    ...
    function _getLiquidityPoolSize() internal view returns (uint256) {
        uint16 swapPoolPercentPoints = getLiquidityPoolPercent();
        uint256 depositedAndBufferedEthers = _getDepositedAndBufferedEthers();
        if (swapPoolPercentPoints == uint16(10000)) {
            return uint256(-1);
        }
    ...
    function _updateSwapPool(uint256 _value) internal {
        (uint256 swapPoolLiquidity, uint256 bufferedEther) = getLiquidityPoolNewStatus(_value);
    ...
    function _swap(address from, address recipient, uint256 ethAmount, uint256 kETHAmount) internal whenNotStopped returns (uint256) {
        uint256 ethFeeAmount = getSwapFeeByETH(ethAmount);
        ...
        uint256 outEthWithFee = ethAmount.sub(ethFeeAmount);
        emit Swap(from, recipient, outEthWithFee, kETHAmount);
        return outEthWithFee;
    }
