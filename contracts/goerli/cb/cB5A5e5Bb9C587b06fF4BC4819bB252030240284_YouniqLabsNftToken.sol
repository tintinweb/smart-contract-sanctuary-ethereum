...
contract YouniqLabsNftToken is
    ERC721,
    ERC721URIStorage,
    ERC721Enumerable,
    ERC2981,
    ...
    AccessControl
{
    ...
    Counters.Counter private _tokenIdCounter; // token Ids counter
    ...
    event UpdateBaseURI(string indexed baseURI);

    constructor(
        uint96 fee_, // royalty fee percent
        address feeRecipient_, // fee recipient address
        uint256 maxTotalSupply_, // max total supply
        string memory baseURI_ // base URI
    ) ERC721("BoneBox", "BB") {
        if (feeRecipient_ == address(0))
            revert ZeroAddress("YouniqLabs: Zero address");
        if (maxTotalSupply_ == 0)
            revert ZeroTotalSupply("YouniqLabs: Zero total supply");
    ...
    /**
    @dev The modifier checks whether the number of minted tokens 
    * does not exceed the permissible amount.
    */
    modifier onlyLimitedSupply() {
        if (totalSupply() == maxTotalSupply) {
            revert SupplyLimit("YouniqLabs: Mintable limit exhausted");
        }
        _;
    }
    ...
    */
    ...
    /** 
    @dev The function batch mints the nft tokens and sets metadata. Only owner can call it.
    @param to - array of recipient wallet addresses
    @param uris - array of token uris
     */
    function batchSafeMintWithUri(address[] memory to, string[] memory uris)
        external
        onlyOwner
    {
        if (to.length != uris.length) {
            revert InvalidArrayLengths("YouniqLabs: Invalid array lengths");
        }
        for (uint256 i; i < to.length; i++) {
            _mintWithUri(to[i], uris[i]);
        }
    }

    /** 
    @dev The function mints the nft token. Only owner can call it.
    @param to - recipient wallet address
     */
    ...
    /** 
    @dev The function batch mints the nft tokens. Only owner can call it.
    @param to - array of recipient wallet addresses
     */
    function batchSafeMint(address[] memory to) external onlyRole(MINTER_ROLE) {
        for (uint256 i; i < to.length; i++) {
            _mint(to[i]);
        }
    }
     ...
     * @param uris - array of token uris
     */
    function updateUriBatch(uint256[] memory tokenIds, string[] memory uris)
        public
        onlyOwner
    {
     ...
     */
    function _mintWithUri(address to, string memory uri)
        internal
        onlyLimitedSupply
    {
     ...
     * @dev See {IERC721-_safeMint}.
     */
    function _mint(address to) internal onlyLimitedSupply {
        uint256 tokenId = _tokenIdCounter.current();
        _tokenIdCounter.increment();
        _safeMint(to, tokenId);
    }
    ...
    {
        if (user == address(0)) revert ZeroAddress("YouniqLabs: Zero address");
