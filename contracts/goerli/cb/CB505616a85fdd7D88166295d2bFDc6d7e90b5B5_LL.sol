...
contract LL is LLRoles {
    ...
    {
        TransferHelper.safeApprove(WETH, address(swapRouter), amountUSDC);
        ...
        )
    {
        ...
        if (amountETH < amountToMintETH) {
            TransferHelper.safeApprove(
                WETH,
                address(nonfungiblePositionManager),
                0
            );
            uint256 refundETH = amountToMintETH - amountETH;
            TransferHelper.safeTransfer(WETH, msg.sender, refundETH);
        }

        if (amountUSDC < amountToMintUSDC) {
            TransferHelper.safeApprove(
                USDC,
                address(nonfungiblePositionManager),
                0
            );
            uint256 refundUSDC = amountToMintUSDC - amountUSDC;
            TransferHelper.safeTransfer(USDC, msg.sender, refundUSDC);
        }
    }
    ...
    function decreaseLiquidity(uint256 tokenId, uint128 newLiquidity)
        external
        onlyOwnerOrTrader
        returns (uint256 amountETH, uint256 amountUSDC)
    {
