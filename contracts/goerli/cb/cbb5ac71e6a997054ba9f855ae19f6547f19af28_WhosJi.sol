...
contract WhosJi is ERC721A, Owned(msg.sender), ReentrancyGuard {
    ...
    modifier requireMintablePublic(
        ...
        unchecked {
            if (!LibBitmap.get(bitmap, _PAUSE_PUBLIC_INDEX))
                revert MintPaused();
            if (quantity + _numberMinted(addy) > maxMint) revert MaxMint();
            if (_totalMinted() + quantity + _AVAILABLE_WHITELIST > _MAX_SUPPLY)
                revert MaxSupplyPublic();
        }
        _;
    }
    ...
    modifier requireMintableOji(address addy, uint256 quantity) {
        unchecked {
            if (!LibBitmap.get(bitmap, _PAUSE_OG_INDEX)) revert MintPaused();
            if (quantity + _numberMinted(addy) > _MAX_MINT_OG) revert MaxMint();
            if (_totalMinted() + quantity + _AVAILABLE_WHITELIST > _MAX_SUPPLY)
                revert MaxSupplyPublic();
        }
        _;
    ...
    modifier requireMintableWhitelist(address addy, uint256 amount) {
        ...
        if (_getAux(addy) != 0) revert MaxMint();
        if (!LibBitmap.get(bitmap, _PAUSE_WHITELIST_INDEX)) revert MintPaused();
        unchecked {
            if (_totalMinted() + amount > _MAX_SUPPLY) revert MaxSupply();
        }
        _;
    }
    ...
    modifier requireExactPrice(uint256 quantity) {
        if (msg.value != _PRICE * quantity) revert InvalidPrice();
        _;
    }
    ...
    function adminMint(uint256 amount) external payable onlyOwner {
        unchecked {
            if (amount + _totalMinted() > _MAX_SUPPLY) revert MaxSupply();
        }
