...
contract nodipUniswapV2Locker is Ownable, ReentrancyGuard{
    ...
    mapping(address => UserFlagInfo) private userFlags;
    ...
    function flagToken(address _lpToken, uint256 _lockID) public {
        UserFlagInfo storage userFlagging = userFlags[msg.sender]; 
        require(!userFlagging.userFlaggedTokens.contains(_lpToken));
        ...
        if(tokenLock.currentFlaggedAmount > erc20TokenTotalSupply / 2) {
            userFlagging.userFlaggedTokens.add(_lpToken);
