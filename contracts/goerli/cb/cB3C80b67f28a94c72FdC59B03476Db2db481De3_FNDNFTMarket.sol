  ...
  NFTMarketOffer
{
  ...
  /**
   * @dev Returns whether the NFT is currently available and in escrow.
   */
  function _isAvailableInEscrow(address nftContract, uint256 tokenId)
    internal
    view
    override(NFTMarketCore, NFTMarketReserveAuction, NFTMarketBuyNow)
    returns (bool)
  {
    return super._isAvailableInEscrow(nftContract, tokenId);
  }

  /**
   * @dev Transfer the NFT from escrow to the to address.
   */
  function _transferFromAvailableEscrow(
    address to,
    address nftContract,
    uint256 tokenId
  ) internal override(NFTMarketCore, NFTMarketReserveAuction, NFTMarketBuyNow) {
    return super._transferFromAvailableEscrow(to, nftContract, tokenId);
  }
}
...
abstract contract NFTMarketCore {
  ...
  /**
   * @dev If the NFT is not escrowed or is unavailable in escrow falls back to is not available in escrow.
   */
  function _isAvailableInEscrow(
    address, /*nftContract*/
    uint256 /*tokenId*/
  ...
  /**
   * Transfer the NFT from escrow to the to address.
   * @dev Implementors of this fn should clear internal state on NFT being removed from escrow.
   */
  function _transferFromAvailableEscrow(
    address to,
    address nftContract,
  ...
  /**
   * @dev Implementors of this interface should return whether a buy now price has been set.
   */
  ...
  NFTMarketAuction
{
  ...
  function createReserveAuction(
    ...
    if (!_isAvailableInEscrow(nftContract, tokenId)) {
      IERC721(nftContract).transferFrom(msg.sender, address(this), tokenId);
    } else {
      require(
        getReserveAuctionIdFor(nftContract, tokenId) == 0,
        "NFTMarketReserveAuction: There is already a created auction"
      );
    }
  ...
  function cancelReserveAuction(uint256 auctionId) public nonReentrant {
    ...
    if (!_hasBuyNowPrice(auction.nftContract, auction.tokenId)) {
      _transferFromAvailableEscrow(msg.sender, auction.nftContract, auction.tokenId);
    }
  ...
  /**
   * @dev Returns whether the NFT is currently available in escrow.
   */
  function _isAvailableInEscrow(address nftContract, uint256 tokenId)
    internal
    view
    virtual
    override(NFTMarketCore)
    returns (bool)
  {
    uint256 auctionId = getReserveAuctionIdFor(nftContract, tokenId);
    if (auctionId != 0 && getReserveAuction(auctionId).endTime == 0) {
      return true;
    }
    return super._isAvailableInEscrow(nftContract, tokenId);
  }
  ...
  function _transferFromAvailableEscrow(
    ...
    address nftContract,
    uint256 tokenId
  ) internal virtual override(NFTMarketCore) {
    ...
    super._transferFromAvailableEscrow(to, nftContract, tokenId);
  }
...
abstract contract NFTMarketOffer is FoundationAdminRole, ReentrancyGuardUpgradeable, NFTMarketFees {
  ...
  FETH public feth;
  mapping(address => mapping(uint256 => mapping(address => Offer))) private nftContractToIdToAddressToOffer;
  ...
  /**
   * @notice Allows Foundation to set the FETH address.
   TODO move into update config
   */
  function initializeFeth(address _feth) public onlyFoundationAdmin {
    require(_feth.isContract(), "FETH must be a contract");
    feth = FETH(_feth);
  }

  receive() external payable {
    require(msg.sender == address(feth), "Can only receive from FETH");
  }

  /**
   * @notice Place an offer for any NFT, funds will be locked in FETH.
  ...
  function placeOffer(
    ...
    require(nftContract.isContract(), "NFT must be a contract");
    Offer storage offer = nftContractToIdToAddressToOffer[nftContract][tokenId][msg.sender];

    // If increasing a valid offer, remove the old one from locked escrow
    if (offer.expiration >= block.timestamp) {

      require(amount > offer.offerAmount, "Offer amount must be greater than current offer amount");

      feth.marketUnlockFor(msg.sender, offer.expiration, offer.offerAmount);
    }

    // Add offer to escrow
    uint256 expiration = feth.marketDepositFor{ value: msg.value }(msg.sender, amount);

    // Record offer details
    offer.expiration = expiration;
    offer.offerAmount = amount;
    emit OfferCreated(nftContract, tokenId, msg.sender, amount, expiration);
  }

  // TODO maybe add offerFor(destination) payable
  // TODO use escrow to place auction bids

  /**
   * @notice Accept an offer for an NFT.
   * @dev The offer must not be expired and the NFT owned + approved by the seller.
   */
  function acceptOffer(
    ...
    Offer memory offer = nftContractToIdToAddressToOffer[nftContract][tokenId][buyer];
    require(offer.expiration >= block.timestamp, "Offer expired");
    require(offer.offerAmount >= minAmount, "NFTMarketOffer: Amount below min");

    // Remove offer from escrow
    delete nftContractToIdToAddressToOffer[nftContract][tokenId][buyer];
    feth.marketWithdrawFrom(buyer, offer.expiration, offer.offerAmount);

    // Settle sale
    // TODO support accepting offers if listed without a bid? yes.
    (uint256 f8nFee, uint256 creatorFee, uint256 ownerRev) = _distributeFunds(
      nftContract,
      tokenId,
      payable(msg.sender),
    ...
    emit OfferAccepted(nftContract, tokenId, buyer, msg.sender, f8nFee, creatorFee, ownerRev);
  }
  ...
  function adminCancelOffers(
    ...
    require(bytes(reason).length > 0, "NFTMarketOffer: Include a reason for this cancellation");

    for (uint256 i = 0; i < nftContracts.length; i++) {
      Offer memory offer = nftContractToIdToAddressToOffer[nftContracts[i]][tokenIds[i]][buyers[i]];
      delete nftContractToIdToAddressToOffer[nftContracts[i]][tokenIds[i]][buyers[i]];

      if (offer.expiration >= block.timestamp) {
        feth.marketUnlockFor(buyers[i], offer.expiration, offer.offerAmount);
        emit OfferCanceledByAdmin(nftContracts[i], tokenIds[i], buyers[i], reason);
      }
    }
  }
}
 ...
 */
abstract contract NFTMarketBuyNow is NFTMarketFees, NFTMarketReserveAuction {
  struct BuyNow {
    address payable seller;
    uint96 amount;
  }
  ...
  event BuyNowPriceSet(address indexed nftContract, uint256 indexed tokenId, address indexed seller, uint256 amount);
  event BuyNowPriceChanged(
    address indexed nftContract,
    uint256 indexed tokenId,
    address indexed seller,
    uint256 amount
  );
  event BuyNowPriceCancelled(address indexed nftContract, uint256 indexed tokenId);
  event BuyNowAccepted(
    address indexed nftContract,
    uint256 indexed tokenId,
    address indexed seller,
    address buyer,
  ...
  // /**
  //  * @notice Sets the buy now price for an NFT when available from the msg.sender.
  //  * Setting a 0x0 address for the seller and 0 for amount will clear the buy now of the NFT.
  //  * @dev If the NFT has an active auction, on the first bid the buy now price is cancelled.
  //  */
  function setBuyNowPrice(
    ...
    if (seller != address(0)) {
      // require seller to match if buy now price set.
      require(seller == msg.sender, "NFTMarketBuyNow: set price of NFT that is not own");
      emit BuyNowPriceChanged(nftContract, tokenId, msg.sender, amount);
    } else {
      emit BuyNowPriceSet(nftContract, tokenId, msg.sender, amount);
    }
    nftContractToTokenIdToBuyNow[nftContract][tokenId] = BuyNow(payable(msg.sender), uint96(amount));
    if (seller == address(0) && !super._isAvailableInEscrow(nftContract, tokenId)) {
      // Escrow the NFT onto the FND Market, if not already escrowed.
      IERC721(nftContract).transferFrom(msg.sender, address(this), tokenId);
    }
  }

  function buyNow(address nftContract, uint256 tokenId) external payable nonReentrant {
    address payable seller = nftContractToTokenIdToBuyNow[nftContract][tokenId].seller;
    uint96 amount = nftContractToTokenIdToBuyNow[nftContract][tokenId].amount;

    require(seller != address(0), "NFTMarketBuyNow: BuyNowPrice not set");
    require(msg.value == amount, "NFTMarketBuyNow: BuyNowPrice does not match");

    // clear internal state.
    delete nftContractToTokenIdToBuyNow[nftContract][tokenId];
    // transfer NFT from escrow to msg.sender.
    super._transferFromAvailableEscrow(msg.sender, nftContract, tokenId);
    ...
    emit BuyNowAccepted(nftContract, tokenId, seller, msg.sender, f8nFee, creatorFee, ownerRev);
  }
   ...
   */
  function cancelBuyNow(address nftContract, uint256 tokenId) public nonReentrant {
    address seller = nftContractToTokenIdToBuyNow[nftContract][tokenId].seller;
    require(seller != address(0), "NFTMarketBuyNow: cancel buy now of NFT that is not set");
    require(seller == msg.sender, "NFTMarketBuyNow: cancel buy now of NFT that is not own");

    // Transfer back NFT to owner.
    // No need to worry about *live* auctions if we got this far
    // since buy now state should be cleared if there is a *live* auction.
    delete nftContractToTokenIdToBuyNow[nftContract][tokenId];
    if (!super._isAvailableInEscrow(nftContract, tokenId)) {
      // NFT is not *listed* in auction, we can call transferFrom directly.
      IERC721(nftContract).transferFrom(address(this), msg.sender, tokenId);
    }

    emit BuyNowPriceCancelled(nftContract, tokenId);
  }

  /**
   * @notice Returns the buy now price for an NFT if one is available.
   */
  function getBuyNowPrice(address nftContract, uint256 tokenId) external view returns (uint256 amount) {
  ...
  function _hasBuyNowPrice(address nftContract, uint256 tokenId) internal view override(NFTMarketCore) returns (bool) {
  ...
  /**
   * @dev Clears the set buy now from storage and delegates to super to perform transfer.
   * Make sure _hasBuyNowPrice() returns false before calling this fn,
   * if you'd like the buy now price to not be cleared and still be active.
   */
  function _transferFromAvailableEscrow(
    address to,
    address nftContract,
    uint256 tokenId
  ) internal virtual override(NFTMarketCore, NFTMarketReserveAuction) {
    delete nftContractToTokenIdToBuyNow[nftContract][tokenId];
    emit BuyNowPriceCancelled(nftContract, tokenId);
    super._transferFromAvailableEscrow(to, nftContract, tokenId);
  }
  ...
  /**
   * @dev Clears buy now price set and emits a BuyNowPriceCancelled on a auction start.
   */
