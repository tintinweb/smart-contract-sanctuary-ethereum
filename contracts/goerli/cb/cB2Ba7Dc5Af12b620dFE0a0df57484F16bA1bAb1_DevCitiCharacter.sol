                       ...
                       ERC721URIStorageUpgradeable 
{
    ...
    /**
     * @dev Emitted when `tokenId` is dev-minted to `recipient` with `details`.
     */
    event DevMintWithDetails(uint256 indexed tokenId, address indexed recipient, string details);
    ...
    /**
     * @dev Mints a token for `to` with `details`.
     *
     * Returns the minted tokenId.
     */
    function devMintWithDetails(address to, string calldata details) 
        public 
        returns (uint256) 
    {
        require(bytes(details).length > 0, "details must not be empty");
        uint256 tokenId = _doSafeMint(to);
        emit DevMintWithDetails(tokenId, to, details);
        return tokenId;
    }
    ...
    function approve(address operator, uint256 tokenId) 
        ...
        override(BaseCitiNFT, ERC721Upgradeable) 
    {
        BaseCitiNFT.approve(operator, tokenId);
    }
    ...
    function safeTransferFrom(address from, address to, uint256 tokenId) 
        ...
        override(BaseCitiNFT, ERC721Upgradeable) 
    {
        BaseCitiNFT.safeTransferFrom(from, to, tokenId);
    }
    ...
    function safeTransferFrom(address from, address to, uint256 tokenId, bytes memory data)
        ...
        override(BaseCitiNFT, ERC721Upgradeable) 
    {
        BaseCitiNFT.safeTransferFrom(from, to, tokenId, data);
    }
    ...
    function setApprovalForAll(address operator, bool approved) 
        ...
        override(BaseCitiNFT, ERC721Upgradeable) 
    {
        BaseCitiNFT.setApprovalForAll(operator, approved);
    }
    ...
    function transferFrom(address from, address to, uint256 tokenId) 
        ...
        override(BaseCitiNFT, ERC721Upgradeable) 
    {
        BaseCitiNFT.transferFrom(from, to, tokenId);
    }
}
                        ...
                        Versioned 
{
    ...
    /**
     * @dev Toggle whether to register to the operator filter registry and subscribe to the OS curated subscription.
     *
     * Emits {RegistrationUpdated} and {SubscriptionUpdated} on OPERATOR_FILTER_REGISTRY
     * https://github.com/ProjectOpenSea/operator-filter-registry/blob/main/src/OperatorFilterRegistryErrorsAndEvents.sol
     * To check if this contract is registered, call OPERATOR_FILTER_REGISTRY.isRegistered
     * To check if an operator is allowed, call OPERATOR_FILTER_REGISTRY.isOperatorAllowed
     * To get the subscription of this contract, call OPERATOR_FILTER_REGISTRY.subscriptionOf
     */
    function adminToggleOperatorFilter(bool registerAndSubscribe)
        public 
        onlyRole(DEFAULT_ADMIN_ROLE) 
        whenNotPaused 
    {
        if (registerAndSubscribe) {
...
contract Versioned {
  string public constant version = "0x62e26bd8d99cf86d17f7ce8f4232f1a3c7b229ec";
}
