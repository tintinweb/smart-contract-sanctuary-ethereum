...
interface IClaimPool is IERC165Upgradeable {
    ...
    function addBudget(address _collectionAddress, uint256 _budget) external;
 ...
 *  @author IHeart Team
 *
 *  @notice This smart contract manager NFT.
 */
...
contract TaskManager is Validatable, ITaskManager, ReentrancyGuardUpgradeable {
    ...
    event CancelledTask(uint256 indexed taskId);
    ...
    function createTask(
        ...
        uint256 endTime,
        uint256 budget
    ) external nonReentrant notZero(budget) {
        ...
        require(budget <= IClaimPool(projectInfo.claimPool).getReward(collection), "Over collection budget");
    ...
    function updateTask(
        ...
        uint256 endTime,
        uint256 budget
    ) external nonReentrant validTaskId(taskId) notZero(budget) {
        ...
        if (taskInfo.collection != collection) {
            ...
            if (budget > taskInfo.budget) {
                require(
                    budget - taskInfo.budget <= IClaimPool(projectInfo.claimPool).getReward(collection),
     ...
     */
    function cancelTask(uint256 taskId) external nonReentrant validTaskId(taskId) {
        TaskInfo storage taskInfo = tasks[taskId];
        require(taskInfo.status == StatusTask.ACTIVE, "Task was done or cancel");

        ProjectInfo memory projectInfo = checkValidProject(taskInfo.projectId, tasks[taskId].collection);
        require(projectInfo.projectOwner == _msgSender() || admin.owner() == _msgSender(), "Caller is not owner or project owner");

        taskInfo.status = StatusTask.CANCEL;
        //slither-disable-next-line unused-return
        _taskOfCollection[taskInfo.collection].remove(taskId);
        if (taskInfo.budget > taskInfo.totalSpent) {
            IClaimPool(projectInfo.claimPool).reduceBudgetUse(
                taskInfo.collection,
                taskInfo.budget - taskInfo.totalSpent
            );
     ...
     */
    function completeTask(uint256 taskId) external nonReentrant validTaskId(taskId) {
        TaskInfo storage taskInfo = tasks[taskId];
        require(taskInfo.status == StatusTask.ACTIVE, "Task was done or cancel");
        ...
        taskInfo.status = StatusTask.DONE;
        //slither-disable-next-line unused-return
        _taskOfCollection[taskInfo.collection].remove(taskId);
     ...
     *  @dev    All caller can call this function.
     */
    function checkValidTaskId(uint256 taskId) public view returns (bool) {
