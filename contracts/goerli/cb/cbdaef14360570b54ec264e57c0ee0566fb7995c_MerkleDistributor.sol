...
abstract contract DelegationRegistry is Ownable {
    ...
    modifier onlyWhitelisted(address _userFor) {
        if (!isRewardsDelegate(_userFor, msg.sender)) {
            revert NotWhitelistedForUser(_userFor, msg.sender);
        }
        _;
    }
...
interface IMerkleDistributorCore {
    /// @notice groups a reward token and quantity into a single entry.
    struct Reward {
        uint256 amount;
        address token;
    }
     ...
     * @param rewards the reward token and amount owed to the user
     ...
     * @param totalRewards total rewards and address for that token
 ...
 */
contract MerkleDistributor is Ownable, DelegationRegistry, IMerkleDistributorCore {
     ...
     * @param _totalRewards reward token with associated total rewards for all claimants for this window
     * @dev Caller must have approved this contract to transfer all rewards amounts for each token in totalRewards or this call will fail.
     ...
     */
    function setWindow(Reward calldata _totalRewards, bytes32 _merkleRoot, string memory _ipfsHash)
        external
        onlyOwner
    {
        uint256 currentWindowIndex = nextCreatedIndex;
        nextCreatedIndex += 1;
        Window storage window = merkleWindows[currentWindowIndex];
        window.merkleRoot = _merkleRoot;
        window.ipfsHash = _ipfsHash;

        // save totals by token and transfer from the sender
        window.totalRewards = _totalRewards;
        IERC20(_totalRewards.token).safeTransferFrom(msg.sender, address(this), _totalRewards.amount);

        emit CreatedWindow(currentWindowIndex, msg.sender, _totalRewards);
    }
    ...
    function deleteWindow(uint256 _windowIndex) external onlyOwner {
        ...
        emit DeleteWindow(_windowIndex, msg.sender);
    }
    ...
    function withdrawRewards(address _rewardToken, uint256 _amount) external onlyOwner {
        ...
        emit WithdrawRewards(msg.sender, _amount, _rewardToken);
    }
     ...
     */
    function claim(Claim memory _claim) external notLocked {
        _processClaim(_claim, _claim.account);
        emit Claimed(msg.sender, _claim.windowIndex, _claim.account, _claim.accountIndex, _claim.rewards);
    }
    ...
    function claimMulti(Claim[] memory claims) external notLocked {
        ...
        for (uint256 i = 0; i < claimCount; i++) {
            ...
            batchedAmount += _claim.rewards.amount;
    ...
    function claimMultiDelegated(Claim[] memory claims) external notLocked {
        uint256 batchedAmount = 0;
        uint256 claimCount = claims.length;
        for (uint256 i = 0; i < claimCount; i++) {
            Claim memory _claim = claims[i];

            if (!isRewardsDelegate(_claim.account, msg.sender)) {
                revert NotWhitelistedForUser(_claim.account, msg.sender);
            }
            ...
            if (
                ...
                IERC20(currentRewardToken).safeTransfer(msg.sender, batchedAmount);
                batchedAmount = 0;
                /// @dev we need to figure out a way to efficiently track claims for large numbers of delegated claims
                /// otherwise we risk emitting events that aren't useful OR are very expensive
                // emit ClaimDelegated(msg.sender, _claim.windowIndex, _claim.account, _claim.accountIndex, _claim.rewards);
            }
        }
    }

    function claimDelegated(Claim memory _claim) external notLocked onlyWhitelisted(_claim.account) {
        _processClaim(_claim, msg.sender);
        emit ClaimDelegated(msg.sender, _claim.windowIndex, _claim.account, _claim.accountIndex, _claim.rewards);
    }
    ...
    function _processClaim(Claim memory _claim, address _receiver) internal {
        ...
        require(_claim.rewards.amount > 0, "Nothing to Claim");
        IERC20(_claim.rewards.token).safeTransfer(_receiver, _claim.rewards.amount);
    }
    ...
    function verifyClaim(Claim memory _claim) public view returns (bool valid) {
        bytes32 leaf = keccak256(
            bytes.concat(
                keccak256(abi.encode(_claim.account, _claim.accountIndex, _claim.windowIndex, _claim.rewards.amount))
            )
        );
    ...
    /**
     * @notice fetch the total quantity of rewards for a given token as a struct
     */
    function getTotalRewardsForWindow(uint256 _windowIndex) external view returns (Reward memory) {
        return merkleWindows[_windowIndex].totalRewards;
    }
}