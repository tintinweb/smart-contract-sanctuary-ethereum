...
contract MultipleTokenInstantSale is Pausable {
    struct Item { uint256 uid; address token_address; uint256 token_id; uint256 amount; uint256 sold; address owner_of; uint256 price; }
    Item[] private _items;
    ...
    function sale(address token_address, uint256 token_id, uint256 price, uint256 amount) public notPaused {
        ...
        _items.push(Item(newItemId, token_address, token_id, amount, 0, msg.sender, price));
    ...
    function buy(uint256 uid, uint256 amount) public payable notPaused {
        ...
        IMetaUnit(_meta_unit_address).increaseLimit(msg.sender, item.price * amount);
