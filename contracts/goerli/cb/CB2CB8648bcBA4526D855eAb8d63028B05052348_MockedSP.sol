...
contract MockedSP {
    ...
    mapping(string => uint256) private cardsUUIDs; // uuid -> tokenId (cardId)
    ...
    function issueTransportCard(IssueTransportCardPayload calldata payload) 
        ...
        returns (uint256)
    {
        require(cardsUUIDs[payload.uuid] == 0, "this uuid is already existed");
        cardId ++;
        emit IssueTransportCard(cardId, payload.recipient, payload.tokenURI);
        cardsUUIDs[payload.uuid] = cardId;
        return cardId;
    }
