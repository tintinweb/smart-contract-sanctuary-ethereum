...
contract ICO is Ownable, ICOEvents, ReentrancyGuard {
    ...
    uint32 public price = 1500; //in 10**5
    ...
    mapping(uint8 => uint32) timeMapping;
    ...
    constructor(address _tokenAddress, address _usdtAddress) {
        ...
        expirationTime = startTime + 2 minutes; //for testing 2 minutes
        percentageArray = [
            700,
            700,
            700,
        ...
        timeMapping[0] = uint32(2 minutes);
    }
        ...
        nonReentrant
    {
        ...
        require(
            _amountInUSD <= (25_000 * 10**8),
            "You can't buy above the upper cap of 25K USD"
        );

        require(
            _tokenAmount <= tokenInstance.balanceOf(address(this)),
    ...
    function claimTokens() public nonReentrant {
        require(
            block.timestamp >= (expirationTime + timeMapping[0]),
        ...
        emit ClaimedToken(amount, claimCount, msg.sender);
    }
        ...
        returns (uint256 _toBeTransfer, uint8 _claimCount)
    {
        ...
        uint8 claimCount = ((uint8(time / 1 minutes)) + 1); // take 1 minutes for testing
        ...
        if (
            user.totalAmount == user.claimedAmount || claimCount == user.claims
        ) {
            return (0, claimCount);
        }
...
interface ICOEvents {
    ...
    event ClaimedToken(uint256 toBeTransfer,uint8 claimCount,address addr);
}
