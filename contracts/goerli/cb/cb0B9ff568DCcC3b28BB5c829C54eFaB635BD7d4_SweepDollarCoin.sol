...
import "./Pools/SweepPool.sol";
import "../Oracle/UniswapPairOracle.sol";
import "../Oracle/ChainlinkETHUSDPriceConsumer.sol";
import "../Governance/AccessControl.sol";

contract SweepDollarCoin is ERC20Custom, AccessControl, Owned {
    ...
    uint256 public constant genesis_supply = 2000000e18; // 2M SWEEP (only for testing, genesis supply will be 5k on Mainnet). This is to help with establishing the Uniswap pools, as they need liquidity

    // The addresses in this array are added by the oracle and these contracts are able to mint sweep
    address[] public sweep_pools_array;

    // Mapping is also used for faster verification
    mapping(address => bool) public sweep_pools; 
    ...
    /* ========== MODIFIERS ========== */
    modifier onlyPools() {
       require(sweep_pools[msg.sender] == true, "Only frax pools can call this function");
        _;
    } 
    ...
    // Iterate through all sweep pools and calculate all value of collateral in all pools globally 
    function globalCollateralValue() public view returns (uint256) {
        uint256 total_collateral_value_d18 = 0; 

        for (uint i = 0; i < sweep_pools_array.length; i++){ 
            // Exclude null addresses
            if (sweep_pools_array[i] != address(0)){
                total_collateral_value_d18 = total_collateral_value_d18.add(SweepPool(sweep_pools_array[i]).collatDollarBalance());
            }
    ...
    function pool_burn_from(address b_address, uint256 b_amount) public onlyPools {
        ...
        emit SweepBurned(b_address, msg.sender, b_amount);
    }

    // This function is what other sweep pools will call to mint new SWEEP 
    function pool_mint(address m_address, uint256 m_amount) public onlyPools {
        super._mint(m_address, m_amount);
        emit SweepMinted(msg.sender, m_address, m_amount);
    }
    ...
    function addPool(address pool_address) public onlyByOwnerGovernanceOrController {
        ...
        require(sweep_pools[pool_address] == false, "Address already exists");
        sweep_pools[pool_address] = true; 
        sweep_pools_array.push(pool_address);
    ...
    function removePool(address pool_address) public onlyByOwnerGovernanceOrController {
        require(pool_address != address(0), "Zero address detected");
        require(sweep_pools[pool_address] == true, "Address nonexistant");
        
        // Delete from the mapping
        delete sweep_pools[pool_address];

        // 'Delete' from the array by setting the address to 0x0
        for (uint i = 0; i < sweep_pools_array.length; i++){ 
            if (sweep_pools_array[i] == pool_address) {
                sweep_pools_array[i] = address(0); // This will leave a null in the array and keep the indices the same
                break;
            }
        }
    ...
    function setMintPrice (uint256 _new_mint_price) public onlyByOwnerGovernanceOrController {
        mint_price = _new_mint_price;

        emit MintPriceSet(_new_mint_price);
    }

    function setRedeemPrice (uint256 _new_redeem_price) public onlyByOwnerGovernanceOrController {
    ...
    // Track SWEEP burned
    event SweepBurned(address indexed from, address indexed to, uint256 amount);

    // Track SWEEP minted
    event SweepMinted(address indexed from, address indexed to, uint256 amount);
...
contract SweepPool is AccessControl, Owned {
    using SafeMath for uint256;
    ...
    address private sweep_contract_address;
    ...
    bool private borrowingPaused = false;
    ...
    constructor (
        ...
        require(
            (_sweep_contract_address != address(0))
            && (_collateral_address != address(0))
            && (_creator_address != address(0))
        , "Zero address detected"); 
        SWEEP = ISweep(_sweep_contract_address);
        sweep_contract_address = _sweep_contract_address;
    ...
    // Returns dollar value of collateral held in this Sweep pool
    function collatDollarBalance() public view returns (uint256) {
        if(collateralPricePaused == true){
            return (collateral_token.balanceOf(address(this))).mul(10 ** missing_decimals).mul(pausedPrice).div(PRICE_PRECISION);
        } else {
            uint256 eth_usd_price = SWEEP.eth_usd_price();
            uint256 eth_collat_price = collatEthOracle.consult(weth_address, (PRICE_PRECISION * (10 ** missing_decimals)));

            uint256 collat_usd_price = eth_usd_price.mul(PRICE_PRECISION).div(eth_collat_price);
            return (collateral_token.balanceOf(address(this))).mul(10 ** missing_decimals).mul(collat_usd_price).div(PRICE_PRECISION); //.mul(getCollateralPrice()).div(1e6);    
        }
    }
    ...
    function mintSWEEP(uint256 collateral_amount) external notMintPaused {
        uint256 collateral_amount_d18 = collateral_amount * (10 ** missing_decimals);
        
        (uint256 sweep_amount_d18) = calcMintSWEEPAmount(
            SWEEP.mintPrice(),
            collateral_amount_d18
        );

        sweep_amount_d18 = (sweep_amount_d18.mul(uint(1e6).sub(minting_fee))).div(1e6); //remove precision at the end

        TransferHelper.safeTransferFrom(address(collateral_token), msg.sender, address(this), collateral_amount);
        SWEEP.pool_mint(msg.sender, sweep_amount_d18);
    }

    function redeemSWEEP(uint256 SWEEP_amount) external notRedeemPaused {
        // Need to adjust for decimals of collateral
        uint256 SWEEP_amount_precision = SWEEP_amount.div(10 ** missing_decimals);
        (uint256 collateral_needed) = calcRedeemSWEEP(
            SWEEP.redeemPrice(),
            SWEEP_amount_precision
        );
        ...
        TransferHelper.safeTransfer(address(collateral_token), msg.sender, collateral_needed);
        SWEEP.pool_burn_from(msg.sender, SWEEP_amount);
    }

    function calcMintSWEEPAmount(uint256 mint_price, uint256 collateral_amount_d18) public pure returns (uint256) {
        return (collateral_amount_d18.mul(mint_price)).div(PRICE_PRECISION);
    }

    function calcRedeemSWEEP(uint256 redeem_price, uint256 SWEEP_amount) public pure returns (uint256) {
        return SWEEP_amount.mul(1e6).div(redeem_price);
    }
    ...
    function amoMinterBorrow(uint256 collateral_amount) external onlyAMOMinters {
        ...
        require(borrowingPaused == false, "Borrowing is paused");
    ...
    function addAMOMinter(address amo_minter_addr) external onlyByOwnGov {
        ...
        uint256 collat_val_e18 = ISweepAMOMinter(amo_minter_addr).collatDollarBalance();
    ...
    function toggleMinting() external onlyByOwnGov {
        mintPaused = !mintPaused;

        emit MintingToggled(mintPaused);
    }

    function toggleRedeeming() external onlyByOwnGov {
        redeemPaused = !redeemPaused;

        emit RedeemingToggled(redeemPaused);
    }

    function toggleBorrowing() external onlyByOwnGov{
        borrowingPaused = !borrowingPaused;

        emit BorrowingToggled(borrowingPaused);
    }

    function toggleCollateralPrice(uint256 _new_price) external onlyByOwnGov {
    ...
    function setPoolParameters(uint256 new_mint_fee, uint256 new_redeem_fee) external onlyByOwnGov {
        minting_fee = new_mint_fee;
        redemption_fee = new_redeem_fee;

        emit PoolParametersSet(new_mint_fee, new_redeem_fee);
    }
    ...
    event PoolParametersSet(uint256 new_mint_fee, uint256 new_redeem_fee);
    event TimelockSet(address new_timelock);
    event MintingToggled(bool toggled);
    event RedeemingToggled(bool toggled);
    event BorrowingToggled(bool borrowingPaused);
...
interface ISweep {
  ...
  function sweep_info() external view returns (uint256, uint256, uint256, uint256, uint256, uint256, uint256, uint256);
  function sweep_pools(address ) external view returns (bool);
  function sweep_pools_array(uint256 ) external view returns (address);
  function sweep_price() external view returns (uint256);
  ...
  function setSweepEthOracle(address _sweep_oracle_addr, address _weth_address ) external;
