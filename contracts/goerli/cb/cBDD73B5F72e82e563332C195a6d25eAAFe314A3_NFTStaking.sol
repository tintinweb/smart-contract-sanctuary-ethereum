...
contract NFTStaking is ERC721Holder, AtticcAccessControl {
    ...
    function unstake(uint256 tokenId) public {
        require(eapPass.ownerOf(tokenId) == address(this), "token is not staked!!");
        uint256 id = stakingPass.underlyingToken(tokenId);
        // Transfer the NFT back to the user
        eapPass.safeTransferFrom(address(this), msg.sender,  id);
        stakerBalances[msg.sender] -= 1;
    ...
    function distributeTokens() public {
        ...
        for(uint i = 0 ; i< stakers.length; i++) {
            ...
            if (stakerBalances[staker] > 0) {
                stakerCount += stakerBalances[staker];
            }
        }

        // Calculate the amount of ERC20 tokens to distribute to each staker
        uint256 tokensPerStaker = 2277 / stakerCount;
