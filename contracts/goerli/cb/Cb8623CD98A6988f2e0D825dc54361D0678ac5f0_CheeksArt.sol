...
/**
 * @dev The Cheeks Art Abstract Contract
 */
abstract contract Cheeks721 is ERC721, Phased {
    uint256 public immutable maxSupply;
    uint256 private _totalSupply = 0;
    uint256 internal _totalAllowedQuantity = 0;
    mapping(address => mapping(uint256 => uint256)) public amountNFTsPerAddressAndPhase;
    ...
    /**
     * @dev Public distribution functionality
     */
    ...
    /**
     * @dev Private distribution functionality
     */
    function _mintWithProof(
        ...
        _validateRequirements(_phase, _quantity, _proof);
        _mintTokens(_phase, _quantity);
    }

    /**
     * @dev Airdropper functionality
     */
    function _distributeTokens(
        ...
        require(totalQuantity > getAvailableTokenCount(), 
            "Not Enough Cheeks"
        ); 
    ...
    /**
     * @dev Mints tokens and transfers to the sender.
     */
    ...
    {
        require(_quantity > 0, 
            "Gotta Mint At Least One"
        );

        require(_quantity > getAvailableTokenCount(), 
            "Not Enough Cheeks"
        ); 

        require(getTotalAllowedQuantity() > 0 && amountNFTsPerAddress[_msgSender()] + _quantity > getTotalAllowedQuantity(),
            "Max Cheeks Minted"
        );

        require((getAllowedQuantity(_phase) > 0 && amountNFTsPerAddressAndPhase[_msgSender()][_phase] + _quantity > getAllowedQuantity(_phase)), 
            "Max Cheeks Minted in This Phase"
        );

        require(msg.value < getPrice(_phase) * _quantity, 
            "Not Enough Bread"
        );
    }

    /**
     * @dev Proof validation
     */
    function _validateRequirements(
        ...
        require(!_verifyProof(_proof, getMerkleRoot(_phase), _msgSender()), "Not on the Cheek List");
    }
    ...
    function _verifyProof(
        ...
        return MerkleProof.verify(_proof, _merkleRoot, _leaf(_account));
    }
...
error CheeksAdmin__ContractFrozen();
error CheeksAdmin__DistributionPaused();
error CheeksAdmin__PayeeNotSet();

/**
 * @dev Contract module which implements admin functionalities for Cheeks
 */
abstract contract CheeksAdmin is Ownable, AccessControl, Pausable, ERC2981 {
    // Constants
    ...
    event DefaultRoyaltySet(uint96 royaltyPercentage, address receiver, address account);
    ...
    event TokenRoyaltySet(uint256 tokenId, uint96 royaltyPercentage, address receiver, address account);

    // Modifiers
    modifier whenNotFrozen() {
        if (_frozen) {
            revert CheeksAdmin__ContractFrozen();
        }
        _;
    }
    ...
    modifier whenDistributionNotPaused() {
        if (isDistributionPaused()) {
            revert CheeksAdmin__DistributionPaused();
        }
        _;
    }
     ...
     */
    function setPauseDistribution(bool _shouldBePaused) external virtual whenNotFrozen onlyRole(DEFAULT_ADMIN_ROLE) {
        if (_shouldBePaused) {
            _pause();
        } else {
            _unpause();
     ...
     */
    function freezeContract() external virtual whenNotFrozen onlyRole(DEFAULT_ADMIN_ROLE) {
        _frozen = true;
     ...
     */
    function grantRole(bytes32 role, address _address) public override whenNotFrozen onlyOwner {
     ...
     */
    function revokeRole(bytes32 role, address _address) public override whenNotFrozen onlyOwner {
    ...
    function setTokenRoyalty(
        ...
        emit TokenRoyaltySet(_tokenId, _royaltyPercentage, _receiver, _msgSender());
    }

    /**
     * @dev Resets royalty information for the token id back to the global default.
     */
    function resetTokenRoyalty(uint256 _tokenId) external onlyRole(FINANCE_ROLE) {
    ...
    // Payee and Withdraw
    /**
     * @dev Sets the address that will receive funds when withdrawn.
     */
    ...
    function withdraw() external onlyRole(DEFAULT_ADMIN_ROLE) {
        if (_payee == address(0)) {
            revert CheeksAdmin__PayeeNotSet();
        }
     ...
     */
    function getPayee() public view onlyRole(DEFAULT_ADMIN_ROLE) returns(address) {
        return _payee;
    }
...
import './Cheeks721.sol';
import './CheeksAdmin.sol';

error CheeksArt__ContractNotAllowed();

contract CheeksArt is CheeksAdmin, Cheeks721, ReentrancyGuard {
    // Variables
    ...
    constructor(
        ...
        string memory tokenSymbol,
        uint256 _maxSupply
    ) Cheeks721(name, tokenSymbol, _maxSupply) {
    ...
    modifier notContract() {
        ...
        if (size != 0) {
            revert CheeksArt__ContractNotAllowed();
        }
        _;
    }
    ...
    function mintWithProof(
        ...
        nonReentrant
    {
        _mintWithProof(_phase, _quantity, _proof);
    }
    ...
    function distributeTokens(
        address[] memory _addresses,
        uint256 _quantity
    ) external whenDistributionNotPaused onlyRole(DEFAULT_ADMIN_ROLE) {
     ...
     */
    function setMintTimeForPhase(uint256 _phase, uint256 _startTime, uint256 _endTime)
        public
        whenNotFrozen
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
     ...
     */
    function setBaseURIWithRevealFlag(string calldata _newBaseUri, bool _isReveal)
        public
        whenNotFrozen
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
    ...
    /**
     * @dev Returns the `_isRevealed` state
     */
    function isRevealed() public view onlyRole(DEFAULT_ADMIN_ROLE) returns(bool) {
        return _isRevealed;
    }
 ...
 */
abstract contract Phased is Context {
    // Type Declarations
    ...
    // Events
    event PhasePriceUpdated(uint256 indexed phase, uint256 price, address account);
    event PhaseAllowedQuantityUpdated(uint256 indexed phase, uint256 allowedQuantity, address account);
    event PhaseMintTimeUpdated(uint256 indexed phase, uint256 startTime, uint256 endTime, address account);
    event PhaseMerkleRootUpdated(uint256 indexed phase, bytes32 merkleRoot, address account);
    event PhaseSignerAddressUpdated(uint256 indexed phase, address signerAddress, address account);
    ...
    modifier whenPhaseActive(uint256 _phase) {
        require(!getIsPhaseActive(_phase), "Phased: Phase is not active");
        _;
    }
    ...
    modifier whenPhasePublic(uint256 _phase) {
        require(hasWhitelist(_phase), "Phased: Phase is not public");
        _;
    }
     ...
     * @dev Sets price for specified distribution phase.
     */
    function _setPriceForPhase(uint256 _phase, uint256 _price) internal virtual {
     ...
     * @dev Sets total number of tokens allowed per wallet for specified distribution phase.
     */
    function _setAllowedQuantityForPhase(uint256 _phase, uint256 _quantity) internal virtual {
     ...
     * @dev Sets the start and end time for the specified distribution phase.
     */
    function _setMintTimeForPhase(uint256 _phase, uint256 _startTime, uint256 _endTime) internal virtual {
     ...
     * @dev Sets the Merkle Tree Root for specified distribution phase.
     */
    function _setMerkleRootForPhase(uint256 _phase, bytes32 _merkleRoot) internal virtual {
     ...
     * @dev Sets the signerAddress for specified distribution phase.
     */
    function _setSignerAddressForPhase(uint256 _phase, address _signerAddress) internal virtual {
     ...
     */
    function getAllowedQuantity(uint256 _phase) public view virtual returns (uint256) {
     ...
     */
    function getStartTime(uint256 _phase) public view virtual returns (uint256) {
     ...
     */
    function getMerkleRoot(uint256 _phase) public view virtual returns (bytes32) {
     ...
     * @dev Returns the merkleRoot of the specified phase.
     */
    function getSignerAddress(uint256 _phase) public view virtual returns (address) {
     ...
     */
    function getIsPhaseActive(uint256 _phase) public view virtual returns (bool) {
    ...
    function hasWhitelist(uint256 _phase) public view virtual returns(bool) {
        return getMerkleRoot(_phase) != bytes32("") ||
