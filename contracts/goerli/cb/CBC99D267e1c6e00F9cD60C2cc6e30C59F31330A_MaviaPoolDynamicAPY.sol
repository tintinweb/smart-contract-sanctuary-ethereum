...
contract MaviaPoolDynamicAPY is ReentrancyGuardUpgradeable, AccessControlUpgradeable, EIP712Upgradeable {
  ...
  /// @dev Bonus accumulation
  mapping(address => uint256) private _bonusAccumulation;

  event EDepositLog(address indexed staker, uint256 dId, uint256 amount, uint256 creditAmount);
  ...
  /**
   * @notice Return bonus accumulation value
   */
  function fGetBonusAcc(address _pStaker) external view returns (uint256) {
    return _bonusAccumulation[_pStaker];
  }
   ...
   * @param _pMaxBonus: Max Bonus amount
  ...
  function fDepositWithBonus(
    ...
    if (_pBonus > 0) {
      ...
      require(
        _fVerify(_fHash(_msgSender(), _pAmount, _pBonus, _pMaxBonus, _pAppId, _pSignatureTime), _pSignature),
        "fDepositWithBonus:05"
      );
      require(_bonusAccumulation[_msgSender()] + _pBonus <= _pMaxBonus, "fDepositWithBonus:06");
      _bonusAccumulation[_msgSender()] += _pBonus;
    ...
    return
      _hashTypedDataV4(
        keccak256(
          abi.encode(
            keccak256(
              "MaviaPoolDynamicAPY(address _pWallet,uint256 _pAmount,uint256 _pBonus,uint256 _pMaxBonus,uint256 _pAppId,uint256 _pSignatureTime)"
            ),
            _pWallet,
            _pAmount,
            _pBonus,
