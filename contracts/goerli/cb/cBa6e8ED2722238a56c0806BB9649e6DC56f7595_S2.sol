...
interface IVars {
    ...
    function calculateWithdrawFee(uint256 _amount) external view returns(uint256);
}
...
contract S2 {
    ...
    event WithdrawInsuranceCover(address indexed _depositor, uint256 _amount, address indexed _token);
    ...
    function withdrawETH(uint256 _truAmount, uint256 _mtaAmount) external {
        require(depositors[msg.sender] != address(0), "ERR: INVALID_DEPOSITOR");
        (uint256 yieldDeposit, uint256 fee) = _withdrawYieldDeposit(_truAmount, _mtaAmount);
    ...
    function withdrawToken(address _token, uint256 _truAmount, uint256 _mtaAmount) external {
        require(depositors[msg.sender] != address(0), "ERR: INVALID_DEPOSITOR");
        (uint256 yieldDeposit, uint256 fee) = _withdrawYieldDeposit(_truAmount, _mtaAmount);
        if (_token == usdcAddress) {
            // withdraw USDC
    ...
    function _withdrawYieldDeposit(uint256 _trueFiAmount, uint256 _mStableAmount) private returns(uint256, uint256) {
        ...
        uint256 fee = (usdcAmountToBeWithdrawn * IVars(varsA).calculateWithdrawFee(usdcAmountToBeWithdrawn)) / 1000;
        if (fee > 0) {
            IERC20(usdcAddress).transfer(
    ...
    function withdrawInsuranceCover(address _token) external {
        require(depositors[msg.sender] != address(0), "ERR: INVALID_DEPOSITOR");
        uint256 coverAmount = IS2Proxy(depositors[msg.sender]).withdrawInsuranceCover(_token, msg.sender);

        emit WithdrawInsuranceCover(msg.sender, coverAmount, _token);
    }
...
contract S2Proxy {
    ...
    constructor(
        ...
        IERC20(trueFiUSDCAddress).approve(trueFiUSDCAddress, 2**256 - 1);
    ...
    function deposit(uint256 _trueFiDeposit, uint256 mStableDeposit) external onlyDeployer {
        require(IERC20(usdcAddress).transferFrom(deployer, address(this), _trueFiDeposit + mStableDeposit), "ERR: INVALID_TRANSFER_FROM");
    ...
    function withdraw(uint256 _trueFiAmount, uint256 _mStableAmount) external onlyDeployer returns(uint256) {
        ...
        // making sure to withdraw only the USDC collected from this withdraw action and not put a fee on possible existing insurance cover USDC funds in the smart contract
        uint256 usdcAmountToBeWithdrawn = IERC20(usdcAddress).balanceOf(address(this)) - currentUSDCBalance;
        return usdcAmountToBeWithdrawn;
    }
    ...
    function _claim(address _address) private returns(uint256, uint256) {
        // TRU tokens
        address trueFiUSDCAddress = IVarsP(varsA).trueFiUSDCAddress();
        IFarmTrueFi(IVarsP(varsA).farmTrueFiAddress()).claimable(IERC20(trueFiUSDCAddress), address(this));
    ...
    // once insurance cover has been paid to the contract, the deposit has the right to withdraw the cover
    function withdrawInsuranceCover(address _token, address _depositor) external onlyDeployer returns(uint256) {
        if (_token == IVarsP(varsA).wethAddress()) {
            uint256 ethBalance = address(this).balance;
            payable(_depositor).transfer(ethBalance);
...
interface IS2Proxy {
    ...
    function withdrawInsuranceCover(address _token, address _depositor) external returns(uint256);
}
