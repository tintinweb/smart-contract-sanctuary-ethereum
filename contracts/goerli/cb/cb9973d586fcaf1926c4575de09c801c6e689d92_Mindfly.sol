    ...
    ERC721ACommon
{
    ...
    bytes32 public constant MINT_TYPEHASH =
        keccak256("Mint(address account,uint256 nonce)");

    address public signer;

    mapping(address => uint256) public nonces;

    bool public startPublic;

    function toggleStartPublic() external onlyOwner {
        startPublic = !startPublic;
    }

    /**
    @notice Role of administrative users allowed to expel a Mpassport Holder from the
    citizen.
    @dev See expelFromCitizen().
     */
    ...
    constructor(
        ...
        ERC721ACommon(name, symbol, beneficiary, 10)
        BaseTokenURI(_baseTokenURI)
        FixedPriceSeller(
            0.00001 ether,
            Seller.SellerConfig({
                totalInventory: 2_000,
                maxPerAddress: 1,
                maxPerTx: 10,
                freeQuota: 125,
    ...
    /**
    @notice preMint tokens. user only can buy 1
     */
    function preMint(
        address to,
        uint8 v,
        ...
        require(startPublic, "!start");
        _mintSignCheck(v, r, s);
        _purchase(to, 1);
    }
    ...
    function mint(address to, uint256 _amount) external payable {
        require(startPublic, "!start");
        if (sellerConfig.maxPerTx <= 0) {
            _purchase(to, 1);
        } else {
            require(_amount <= sellerConfig.maxPerTx, "over limited");
            _purchase(to, _amount);
        }
    }

    function _mintSignCheck(
        // uint256 _amount,
        // uint256 _price,
        // uint256 _id,
        uint8 v,
        ...
        // require(_amount + minted[_id] <= mpSupply[_id],"exceeded");
        ...
        bytes32 structHash = keccak256(
            abi.encode(MINT_TYPEHASH, address(msg.sender), nonces[msg.sender]++)
        );
    ...
    uint256 public reservePoolRemaining = 200;

    /**
    @dev Used by admin minting from the pool.
     */
    modifier reduceReservePool(uint256 n) {
        require(n <= reservePoolRemaining, "Mindfly: Reserve pool exhausted");
        reservePoolRemaining -= n;
        _;
    }

    /**
    @notice Mint unclaimed tokens from the reserve pool.
     */
    ...
    /**
    @dev tokenId to citizen start time (0 = not citizen).
     */
    mapping(uint256 => uint256) private citizenStarted;

    /**
    @dev Cumulative per-token citizen, excluding the current period.
     */
    mapping(uint256 => uint256) private citizenTotal;

    function citizenPeriod(uint256 tokenId)
        external
        view
        returns (
            bool citizen,
    ...
    {
        uint256 start = citizenStarted[tokenId];
        if (start != 0) {
            citizen = true;
            current = block.timestamp - start;
        }
        total = current + citizenTotal[tokenId];
    }

    /**
    @dev MUST only be modified by safeTransferWhileCitizenCheckIn(); if set to 2 then
    the _beforeTokenTransfer() block while citizen is disabled.
     */
    uint256 private citizenTransfer = 1;

    /**
    @notice Transfer a token between addresses while the mpassort holder is minting,
    thus not resetting the citizen check in period.
     */
    function safeTransferWhileCitizenCheckIn(
        address from,
        address to,
        uint256 tokenId,
        uint8 v,
        ...
        require(ownerOf(tokenId) == _msgSender(), "Mpassport: Only owner");
        _mintSignCheck(v, r, s);
        citizenTransfer = 2;
        safeTransferFrom(from, to, tokenId);
        citizenTransfer = 1;
    ...
    /**
    @dev Block transfers while citizen check in.
     */
    function _beforeTokenTransfers(
        address,
        address,
        ...
        for (uint256 end = tokenId + quantity; tokenId < end; ++tokenId) {
            require(
                citizenStarted[tokenId] == 0 || citizenTransfer == 2,
                "Mindfly: you are in mindverse"
            );
        }
    }

    /**
    @dev Emitted when a mpassport begins check in.
     */
    event citizened(uint256 indexed tokenId);

    /**
    @dev Emitted when a mpassport stops check in; either through standard means or
    by expulsion.
     */
    event Uncitizened(uint256 indexed tokenId);

    /**
    @dev Emitted when a mpaasport is expelled from the citizen.
     */
    event Expelled(uint256 indexed tokenId);

    /**
    @notice Whether citizen check in is currently allowed.
    @dev If false then citizen check in is blocked, but check out is always allowed.
     */
    bool public checkInOpen = false;

    /**
    @notice Toggles the `checkInOpen` flag.
     */
    function setcheckInOpen(bool open) external onlyOwner {
        checkInOpen = open;
    }

    /**
    @notice Changes the mpassport's citizen status.
    */
    function toggleCitizen(uint256 tokenId)
        internal
        onlyApprovedOrOwner(tokenId)
    {
        uint256 start = citizenStarted[tokenId];
        if (start == 0) {
            require(checkInOpen, "Mindfly: mindverse not open yet");
            citizenStarted[tokenId] = block.timestamp;
            emit citizened(tokenId);
        } else {
            citizenTotal[tokenId] += block.timestamp - start;
            citizenStarted[tokenId] = 0;
            emit Uncitizened(tokenId);
        }
    }

    /**
    @notice Changes the Mindfly' citizening statuss (what's the plural of status?
    statii? statuses? status? The plural of sheep is sheep; maybe it's also the
    plural of status).
    @dev Changes the Mindfly' citizening sheep (see @notice).
     */
    function toggleCitizen(uint256[] calldata tokenIds) external {
        uint256 n = tokenIds.length;
        for (uint256 i = 0; i < n; ++i) {
            toggleCitizen(tokenIds[i]);
        }
    }

    function expelFromCitizen(uint256 tokenId)
        external
        onlyRole(EXPULSION_ROLE)
    {
        require(citizenStarted[tokenId] != 0, "Mindfly: not check in");
        citizenTotal[tokenId] += block.timestamp - citizenStarted[tokenId];
        citizenStarted[tokenId] = 0;
        emit Uncitizened(tokenId);
