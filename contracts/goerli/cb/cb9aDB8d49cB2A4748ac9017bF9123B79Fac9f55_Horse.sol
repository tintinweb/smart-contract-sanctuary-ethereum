...
contract Horse is Ownable, ERC721AWithRoyalties, Pausable, PaymentSplitter {
    ...
    function _reqSeed() internal {
        VRF VRFV2 = VRF(Index.getAddress("VRFV2"));
        VRFV2.rollDice();
    }

    function mintreq(uint256[] memory ids) public whenNotPaused {
    ...
    function mintTransfer(uint256[] memory ids) public whenNotPaused {
        ...
        uint256 randnum = (randSeed * block.timestamp) / tokenIDs[randSeed];
        ...
        for (uint256 i = 0; i < ids.length; i++) {
            ...
            _transferFrom(address(this), msg.sender, tokenIDs[randnum]);
