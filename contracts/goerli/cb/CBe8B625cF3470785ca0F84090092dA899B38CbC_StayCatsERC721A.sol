...
contract StayCatsERC721A is Ownable, ERC721A, PaymentSplitter {
    ...
    uint private constant MAX_WHITELIST2 = 2022;
    uint private constant MAX_PUBLIC = 100;
    uint private constant MAX_SUPPLY_MINUS_GIFT = MAX_SUPPLY - MAX_GIFT;

    uint public wlSalePrice = 0.0006 ether;
    uint public wlSalePrice2 = 0.0008 ether;
    uint public publicSalePrice = 0.001 ether;

    uint public saleStartTime = 1678057211;
    ...
    address[] private _team = [
        0x6B2d9460dbe1f35267424122adeDfCfbB5Dd5586,
        0x705E39a84746eDdCF17140Cf48D7E31a1E4bc433,
        0x523B9A19590750FBe8f644540635346d713cFbf6
    ];
 ...
 /**
    ...
    **/
    function whitelistMint2(address _account, uint _quantity, bytes32[] calldata _proof) external payable callerIsUser{
        require(!isPaused, "Contract is paused");
        require(currentTime() >= saleStartTime + 6 hours , "Sale has not started yet b");
        require(currentTime() < saleStartTime + 12 hours, "Sale is finished b");
        uint price = wlSalePrice2;
        require(price != 0, "Price is 0");
        require(sellingStep == Step.WhitelistSale2, "Whitelist sale is not activated b");
        require(isWhitelisted(msg.sender, _proof), "Not whitelisted b");
...
function isWhitelisted1(address _account, bytes32[] calldata _proof) internal view returns(bool) {
        return MerkleProof.verify(_proof, merkleRoot1, keccak256(abi.encodePacked((_account))));
    }

function isWhitelisted2(address _account, bytes32[] calldata _proof) internal view returns(bool) {
        return MerkleProof.verify(_proof, merkleRoot2, keccak256(abi.encodePacked((_account))));
    }
