...
contract Staking is IModule, IYieldFarmingV1Staking, Initializable, ReentrancyGuardUpgradeable, AccessControlUpgradeable {
    ...
    function enforcePrerequisites(address registryAddr, bytes32 orgId) external view {
        ...
        require(!registry.hasModule(orgId, Modules.YIELDFARMING_V1_STAKING), "Prerequisites: module already exists");
    }

    function enforceSafeRemove(address registryAddr, bytes32 orgId) external view {
        IRegistry registry = IRegistry(registryAddr);

        require(!registry.hasModule(orgId, Modules.YIELDFARMING_V1_POOL), "SafeRemove: module still required");
    }
