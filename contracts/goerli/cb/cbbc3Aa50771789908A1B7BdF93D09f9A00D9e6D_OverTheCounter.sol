...
contract OverTheCounter is IERC721Receiver, Ownable {
  ...
  function poolCreate(
    ...
    uint256 _amount0LessFee = _amount0 - (_amount0 * poolAddFeePerc) / PERC_DEN;
    ...
    if (serviceFeeETH > 0) {
      (bool _success, ) = payable(_getTreasury()).call{ value: serviceFeeETH }(
        ''
      );
      require(_success, 'POOLCR: service fee sent');
    }
  ...
  function poolRemove(uint256 _tokenId) external {
    ...
    require(msg.sender == _user, 'POOLRM: owner');
  ...
  function packageCreate(
    ...
    if (serviceFeeETH > 0) {
      ...
      require(_success, 'PKGCR: service fee sent');
    }
  ...
  function addPackageOffer(
    ...
    if (addOfferFee > 0) {
      ...
      require(_success, 'PKGOFF: could not send offer fee');
    }
