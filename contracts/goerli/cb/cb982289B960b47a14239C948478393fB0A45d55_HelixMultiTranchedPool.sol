...
contract HelixMultiTranchedPool is BaseUpgradeablePausable, IHelixMultiTranchedPool, IRequiresUID, IVersioned {
  ...
  function withdrawWithPermit(
    ...
    IERC20PermitUpgradeable(trancheInfo.tranche).permit(msg.sender, address(this), shareBalance, deadline, v, r, s);
  ...
  function withdrawMultiple(
    ...
    for (uint256 i = 0; i < tranches.length; i++) {
      withdrawWithPermit(tranches[i], deadline[i], v[i], r[i], s[i]);
    }
  }
...
//   }
  ...
  function poolFundingEvaluate() public view returns(bool) {
    ...
    if (block.timestamp > fundingDate && !poolRequirementsFulfilled()) {
      return false;
    }
