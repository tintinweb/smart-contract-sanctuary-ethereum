...
contract SomaStaking is ISomaStaking, ReentrancyGuardUpgradeable, TokenRecoveryUpgradeable, GuardableUpgradeable {
    ...
    function addRewardToken(address _asset) external override onlyAdmin nonReentrant {
        ...
        require(_rewardTokens.length() <= 64, 'SomaStaking: LIMIT');
    ...
    function _createRequest(address _sender, address _asset, uint256 _amount, RequestType _type) private returns (uint256 _id) {
        require(_amount > 0, 'Staking: INVALID_AMOUNT');
    ...
    function _getRequest(uint256 _id, address _asset, address _sender, RequestType _type) private view returns (Request memory _request) {
    ...
    function _useRequest(uint256 _id, address _asset, address _sender, uint256 _requiredDuration, RequestType _type) private returns (uint256 _amount) {
    ...
    function _cancelRequest(uint256 _id, address _asset, address _sender, RequestType _type) private returns (uint256 _amount) {
...
interface ISomaStaking {
     ...
     */
    event RequestCreated(uint256 indexed id, address indexed asset, uint256 amount, address indexed sender, RequestType requestType);
