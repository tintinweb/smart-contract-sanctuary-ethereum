...
library LibNDF {
    ...
    /// @dev Uses Uniswap OracleLibrary.getQuoteAtTick() function to get the value of Nayms and each of the external tokens in the NDF. Works out ratio of Nayms value to total value
    function _getNaymsValueRatio() internal view returns (uint256) {
        ...
        uint256 naymsValue = OracleLibrary.getQuoteAtTick(getMaxTick(60), uint128(naymsBalance), naymsAddress, WETH);
        ...
        for (uint256 i = 0; i < LibAdmin._getSupportedExternalTokens().length; i++) {
            ...
            digitalAssetValue += OracleLibrary.getQuoteAtTick(getMaxTick(60), uint128(balanceOfAsset), s.supportedExternalTokens[i], WETH);
        }
