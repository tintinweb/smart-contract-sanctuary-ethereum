...
contract Raffle is VRFConsumerBaseV2, AutomationCompatibleInterface {
    ...
    event RaffleRandomWinner(uint256 requestId);
        ...
        )
    {
        ...
        bool timePassed = ((block.timestamp - s_lastBlockTimeStamp) > interval);
    ...
    function performUpkeep(bytes memory) external override {
        ...
        emit RaffleRandomWinner(requestId);
    }
    ...
    function fulfillRandomWords(uint256 requestId, uint256[] memory randomWords)
        ...
        override
    {
        uint256 indexValue = randomWords[0] % players.length;
        address payable recentWinner = players[indexValue];
...
interface VRFCoordinatorV2Interface {
   ...
   * ceilings, so you can select sa specific one to bound your maximum per request cost.
