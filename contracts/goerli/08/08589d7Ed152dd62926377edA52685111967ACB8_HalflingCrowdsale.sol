...
contract HalflingCrowdsale is AccessControl, PullPayment, Ownable {
    ...
    uint256 public mLimit = 2750; //total mining limit
    ...
    modifier onlyTokenOwner(address sender) {
        ...
        require(
            ...
            "HalflingCrowdsale:Mining conditions are not satisfied"
        );
        _;
    }
    ...
    isState(State.ALLOW) {
        require(state==State.ALLOW, "HalflingCrowdsale:sale has not started");
        require(
            _amount <= sLimit,
            "HalflingCrowdsale:Exceeded the single purchase limit"
        );
        require(
            mintNum[msg.sender].add(_amount) <= oLimit,
            "HalflingCrowdsale:Exceeded the own limit"
        );
        require(
            msg.value == _amount.mul(salePrice),
            "HalflingCrowdsale:Payment declined"
        );
        mAmount = mAmount.add(_amount);
        require(
            mAmount <= mLimit,
            "HalflingCrowdsale:Exceeded the total amount of mining"
        );
    ...
    function setNft(address _nft) public onlyRole(CROWD_ROLE) {
        require(_nft != address(0), "HalflingCrowdsale:Invalid address");
        token = _nft;
    }

    function setState(State _state) external onlyRole(CROWD_ROLE) {
        state = _state;
    ...
    function saleState() external  view returns (State,uint256) {
    ...
    function setCollector(address _collector) external onlyOwner {
        require(_collector != address(0), "HalflingCrowdsale:Invalid address");
        collector = _collector;
        ...
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        require(
            ...
            "HalflingCrowdsale:Invalid address"
        );
