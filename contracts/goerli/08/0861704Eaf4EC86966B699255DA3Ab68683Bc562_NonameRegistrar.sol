...
import { IRegistrarController, ISpaceController } from "./IRegistrarController.sol";
...
contract NonameRegistrar is Ownable {
    ...
    function getRentPrice(
        ...
        if (block.chainid == 56 || block.chainid == 97) {
            IPriceOracle.Price memory price = ISpaceController(address(_registrarController)).rentPrice(name, duration);
            return price.base + price.premium;
        } else {
            return _registrarController.rentPrice(name, duration);
        }
    }
