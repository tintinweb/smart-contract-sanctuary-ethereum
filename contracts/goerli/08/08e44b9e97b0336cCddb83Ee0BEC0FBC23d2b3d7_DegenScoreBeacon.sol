    ...
    PausableUpgradeable
{
    ...
    function submitTraits(UserPayload calldata payload, bytes memory signature) external payable whenNotPaused {
        ...
        for (uint256 i = 0; i < payload.traits.length; i++) {
            ...
            uint64 oldTraitValue = isFirstSubmission ? 0 : _traits[payload.account][payload.traits[i].id].value;
    ...
    function getTraitURI(uint256 traitId) public view returns (string memory) {
        return string.concat(primaryTraitURI, StringsUpgradeable.toString(traitId), ".json");
    }

    function getBeaconURI(uint256 beaconId) public view returns (string memory) {
    ...
    function uri(uint256 id) public view override returns (string memory) {
        if (beaconIds[id] == address(0)) {
            return getTraitURI(id);
        } else {
            return getBeaconURI(id);
        }
    }
    ...
    function ownerOfBeacon(uint128 beaconId) public view returns (address owner) {
