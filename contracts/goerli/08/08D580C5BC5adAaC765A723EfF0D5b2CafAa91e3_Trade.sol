...
contract Trade is AccessControl, Ownable {
    ...
    mapping(address => address) private addressRefBy;
    ...
    */
    ...
    function buyAsset(Order memory order, Sign calldata sign) external {
    ...
    */
    ...
    function executeBid(Order memory order, Sign calldata sign) external {
        require(!usedNonce[sign.nonce], "Nonce : Invalid Nonce");
        usedNonce[sign.nonce] = true;
        Fee memory fee = getFees(order);
        verifyBuyerSign(
    ...
    */
    ...
    function mintAndBuyAssetWithRef(
        Order memory order,
        Sign calldata sign,
        Sign calldata ownerSign,
        address refAddress
    ...
    */
    ...
    function mintAndExecuteBidWithRef(
        Order memory order,
        Sign calldata sign,
        Sign calldata ownerSign,
        address refAddress
    ...
    */
    ...
    function tradeAssetWithRef(
        ...
        require(
            order.buyer != refAddress,
            "order.buyer cannot be equal to refAddress"
        );
        if (fee.platformFee > 0) {
            if (addressRefBy[order.buyer] == address(0)) {
                if (refAddress == address(0)) {
                    ...
                    if (
                        order.buyer != addressRefBy[refAddress] ||
                        order.buyer != addressRefBy[addressRefBy[refAddress]] ||
                        order.buyer !=
                        addressRefBy[addressRefBy[addressRefBy[refAddress]]]
                    ) {
                        addressRefBy[order.buyer] = refAddress;
                    }
                    addressRefRewards[refAddress][order.erc20Address] += fee
                        .platformFee
                        .div(10);
                    transferProxy.erc20safeTransferFrom(
                        IERC20(order.erc20Address),
            ...
            if (royaltyRefTake[order.nftAddress] > 0) {
                if (refAddress != address(0)) {
                    ...
                    transferProxy.erc20safeTransferFrom(
                        ...
                        fee
                            .royaltyFee
                            .mul(royaltyRefTake[order.nftAddress])
                            .div(1000)
                    );
                } else {
                    transferProxy.erc20safeTransferFrom(
    ...
    function set721royaltyRefTake(
        ...
        require(
            _msgSender() == IERC721(collectionAddress).ownerOf(tokenId),
    ...
    function set1155royaltyRefTake(
        ...
        uint256 userBalance = IERC1155(collectionAddress).balanceOf(msg.sender, tokenId);
        require(
            userBalance > 0,
            "You do not own items in this contract.");
        require(
            refSplitAmount <= 1000,
            "Cannot give away more than 100 percent split"
        );
