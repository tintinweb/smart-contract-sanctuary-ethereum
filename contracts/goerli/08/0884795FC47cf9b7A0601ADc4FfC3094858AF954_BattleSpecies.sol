...
contract BattleSpecies is ERC721Namable, ERC721Enumerable, BattlespeciesWithdrawer {
    ...
    function _mint(uint numberOfTokens, bool autoManifest, uint256 _mintPrice) internal {
        ...
        require(balanceOf(_msgSender()) + numberOfTokens <= MAX_MINTS_PER_WALLET, "Reached max token allotment");
    ...
    function hyperwarp(uint256 _jumpClone, uint256 _assist) external {
        require(ownerOf(_jumpClone) == _msgSender() && ownerOf(_assist) == _msgSender(), "Must own both NFTs");
        require(hyperwarpManager.tryHyperwarp(_jumpClone, _assist));
        ...
        battlers[mintIndex] = Battler(block.timestamp, false);
        require(!battlers[mintIndex].isManifested, "Battler has already manifested.");
        _safeMint(_msgSender(), mintIndex);
    ...
    function manifest(uint256 _tokenId) external {
        ...
        bool canManifest = hyperwarpManager.tryManifest(_tokenId);
        require(canManifest, "Battler cannot manifest.");
        battler.isManifested = true;
