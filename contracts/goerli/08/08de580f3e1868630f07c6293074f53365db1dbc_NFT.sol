...
contract NFT is ERC721Enumerable, Ownable {
    ...
    function mint(uint256 _mintAmount) public payable {
        // pricing conditon (Amended By ReservedSnow [ Developer - ReservedSnow (https://linktr.ee/reservedsnow)])
        uint256 price;
        if (_mintAmount == 1) {
            price = 1 ether;
        } else if (_mintAmount == 2) {
            price = 0.75 ether;
        } else if (_mintAmount >= 3 && _mintAmount<6) {
            price = 0.667 ether;
        }
        else{
            price = 0.5 ether;
