...
contract ControllerFreedom is Initializable, UUPSUpgradeable, OwnableUpgradeable {
    ...
    */
    ...
    /**
    *   @notice create group in community througn frontend and return array of id of the group
     */
    function createCommunityGroupFn(uint256 daoId, Group[] memory group) external returns(uint256[] memory id) { 
        uint256[] memory groupIdsArr = new uint256[](group.length);
        for (uint256 index = 0; index < group.length; index++) {
            _Group.increment();
            uint256 groupId =  _Group.current();
