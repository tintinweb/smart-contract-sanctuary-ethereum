...
/**
 *  Aurthor: KronicLabz
 *  Create paid mints for already minted "secondary" nfts
 *  great for communities to sell NFTs from their vault for
 *  eth.
 *
 *  RandomNFTMinter supports secondary royalties on mint.
 *  Meaning when a paid mint is conducted, the original
 *  collection still receives their secondary royalties.
 *
 *  Created for the Rug Rescue🛟 migration contract to allow
 *  community to gain extra revenue for blocked tokenIds.
 *
 */
...
contract RandomNFTMinter is Ownable, IERC721Receiver {
    ...
    uint256 public paidMintLimit = 3;
    mapping(address => uint256) public freeMintsUsed;
    mapping(address => uint256) public paidMintsUsed;
...
*/
    ...
    function mintNFT(address recipient) external payable {
        uint256 freeMints = freeMintsUsed[recipient];
        uint256 paidMints = paidMintsUsed[recipient];

        require(
            freeMints < freeMintLimit || paidMints < paidMintLimit,
            "No more mints allowed."
        );
        require(mintedCount < maxSupply, "All NFTs have been minted.");

        if (freeMints < freeMintLimit) {
            require(msg.value == 0, "No payment required for a free mint.");
            freeMintsUsed[recipient]++;
        } else {
            require(msg.value >= mintPrice, "Insufficient funds to mint NFT.");
            paidMintsUsed[recipient]++;
        }
...
*/
    function addAvailableTokens(uint256[] memory tokenIds) external onlyOwner {
