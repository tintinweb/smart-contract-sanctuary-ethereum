...
contract NTRewardSystem is Ownable, ReentrancyGuard {
    ...
    address public vault = 0xb668beB1Fa440F6cF2Da0399f8C28caB993Bdd65;
    ICitizen public citizenContract =
        ICitizen(0xb668beB1Fa440F6cF2Da0399f8C28caB993Bdd65);
    IERC20 public BYTES = IERC20(0x7d647b1A0dcD5525e9C6B3D14BE58f27674f8c95);
    IOuterCitizen public outerCitizenContract =
        IOuterCitizen(0x9b091d2E0Bb88acE4fe8f0faB87b93D8bA932EC4);

    bool public vaultActive;
    bool public splitActiveForSpecific;
    ...
    function setOuterCitizenContract(IOuterCitizen _outerCitizenContract)
        external
        onlyOwner
    {
    ...
    function setCitizenContract(ICitizen _citizenContract) external onlyOwner {
        citizenContract = _citizenContract;
    }

    function setBYTESContract(IERC20 _BYTES) external onlyOwner {
        BYTES = _BYTES;
    }

    function toggleVaultActive() external onlyOwner {
        vaultActive = !vaultActive;
    }

    function setVaultTax(uint256 _newTax) external onlyOwner {
        vaultTax = _newTax;
    }
    ...
    function rewardBytesSpecificWithCustomSplit(
        ...
        if (!vaultActive) {
            if (_recipient.length == 3) {
                if (
                    citizenContract.balanceOf(_recipient[0]) == 0 &&
                    outerCitizenContract.balanceOf(_recipient[0]) == 0 &&
                    citizenContract.balanceOf(_recipient[1]) == 0 &&
                    outerCitizenContract.balanceOf(_recipient[1]) == 0 &&
                    citizenContract.balanceOf(_recipient[2]) == 0 &&
                    outerCitizenContract.balanceOf(_recipient[2]) == 0
    ...
    function rewardBytesPeerToPeerHandleTwoSplit(
        ...
        if (_split[0] + _split[1] > 100 * 1 ether)
        ...
        if (100 * 1 ether - _split[0] + _split[1] > 0) {
            BYTES.transferFrom(
                msg.sender,
                _recipient[0],
                100 * 1 ether - _split[0] + _split[1]
            );
        }
    ...
    function rewardBytesPeerToPeerHandleThreeSplit(
        ...
        if (_split[0] + _split[1] + _split[2] > 100 * 1 ether)
            revert SplitPercentageHasToEqualOneHundred();
        BYTES.transferFrom(
            msg.sender,
            _recipient[0],
        ...
        if (100 * 1 ether - _split[0] + _split[1] + _split[2] > 0) {
            BYTES.transferFrom(
                msg.sender,
                _recipient[0],
                100 * 1 ether - _split[0] + _split[1]
    ...
    function rewardBytesSpecifcWithoutCustomSplit(
        address[] calldata _recipient,
        uint256 _amount
    ) external {
        if (!vaultActive) {
    ...
    function rewardBytesPeerToPeer(
        ...
        for (uint256 j = 0; j < _recipient.length; j++) {
            if (
                citizenContract.balanceOf(_recipient[j]) == 0 &&
                outerCitizenContract.balanceOf(_recipient[j]) == 0
            ) revert RecipientIsNotACitizen();
    ...
    function emitReward(address[] calldata _recipient, uint256 _amount)
        private
    {
    ...
    function rewardBytesPeerToPeerOne(address _recipient, uint256 _amount)
        private
    {
        if (
    ...
    function whichReward(uint256 _amount)
        external
        view
