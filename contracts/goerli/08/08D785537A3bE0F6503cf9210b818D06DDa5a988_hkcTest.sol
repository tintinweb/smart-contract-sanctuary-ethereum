...
contract hkcTest is
    ...
    uint256 public MAX_RESERVED_MINTS = 1;
    uint256 public MAX_PRESALE_MINTS = 2;
    uint256 public MAX_PRESALE_MINTS_PLUS_ONE = 3;   
    uint256 public reservedMeowListSupply = 1000;
    uint256 public generalMeowListSuppy = 3000;
    uint256 public TOTAL_MEOWLIST_SUPPLY = 4000;
    uint256 public TOTAL_MEOWLIST_SUPPLY_PLUS_ONE = 4001;
    ...
    event MintMeowListReservedlHash(address claimer, uint256 amount);
    ...
    /**
        CrossMint for Reserved MeowList (Tier 1)
     */   
    function crossmintMeowListReserved(
        uint32 qty,
        address to,
        ...
        _mintMeowListGeneralMerkle( qty, to, proof );
    }
     ...
     */
    function mintMeowListReservedlMerkle(
        uint256 amount, 
        bytes32[] calldata proof
    ) external payable {
        _mintMeowListReservedMerkle( amount, msg.sender, proof );
    }
     ...
     */
    function _mintMeowListReservedMerkle(
        uint256 amount, 
        address to,
        bytes32[] calldata proof
    ) internal {
        ...
        //Just checking against total meowlist numbers. Necessary?
        ...
        uint256 generalAmount = premintedAmount[msg.sender] + amount - 1;
        require(generalMeowMinted + generalAmount < generalMeowListSuppy, "Exceeds General Meowlist");

        uint256 cost = amount * price;
        if (msg.value < cost) revert InsufficientPayment();

        require( 
            MerkleProof.verify(proof, merkleRootTier1, _generateMerkleLeaf(to)), "User not in WL" 
        );

        premintedAmount[to] += amount;
        _safeMint(to, amount);

        generalMeowMinted += generalAmount;

        emit MintMeowListGeneralMerkle(to, amount);
    }
    ...
    /**
        CrossMint for General MeowList
     */   
    function crossmintMeowListGeneral(
        uint32 qty,
        address to,
        bytes32[] calldata proof
    ) external payable {
    ...
    function mintMeowListGeneralMerkle(
        ...
        _mintMeowListGeneralMerkle( amount, msg.sender, proof );
    }
    ...
    /**
        New Presale with Merkle Method to as msg.sender
     */
    function _mintMeowListGeneralMerkle(
        uint256 amount, 
        address to,
        bytes32[] calldata proof
    ) internal {
        require(presaleOn, "Presale Not Live");
        require(premintedAmount[to] + amount < MAX_PRESALE_MINTS_PLUS_ONE, "Over max presale allowed");
        require(generalMeowMinted + amount < generalMeowListSuppy, "Exceeds General Meowlist");

        uint256 cost = amount * price;
        if (msg.value < cost) revert InsufficientPayment();

        require( 
            MerkleProof.verify(proof, merkleRootTier2, _generateMerkleLeaf(to)), "User not in WL" 
        );
    ...
    function mintMeowListCockadoodleHash(
        ...
        uint256 generalAmount = premintedAmount[msg.sender] + amount - _passAmount;
        require(generalMeowMinted + generalAmount < generalMeowListSuppy );

        // deciding if we keep verification
        ...
        emit MintMeowListReservedlHash(msg.sender, amount);
    ...
    function crossmintPublic(
        ...
        _mintPublicMerkle( qty, to, proof );
    }
    ...
    function mintPublicMerkle(
        ...
        _mintPublicMerkle( amount, msg.sender, proof );
    }

    /**
        Internal Public Sale with Merkle Tree
     */
    function _mintPublicMerkle(
        uint256 amount, 
        address to,
        bytes32[] calldata proof
    ) internal {
        ...
        require( 
            MerkleProof.verify(proof, merkleRootPublic, _generateMerkleLeaf(to)), "User not in WL" 
        );
