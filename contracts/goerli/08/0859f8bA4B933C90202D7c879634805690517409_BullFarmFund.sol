...
contract BullFarmFund is Ownable {
    uint public constant MIN_ETH_LIQUIDITY = 2 ether;
    ...
    function initLiquidityPool() external onlyOwner {
        require(!initialLiquidityAdded, "BullFarmFund: liquidity is already added");
        require(address(this).balance >= MIN_ETH_LIQUIDITY, "BullFarmFund: not enough ETH liquidity");
        require(address(token) != address(0), "BullFarmFund: token is not set");
    ...
    function buy(uint ethValue) external {
    ...
    function sendTokens(address to, uint ethAmount) external {
        ...
        uint[] memory outs = uniswapV2Router.getAmountsOut(ethAmount, path);
        require(outs[0] <= token.balanceOf(address(this)), "BullFarmFund: not enough tokens");
        token.transfer(to, outs[0]);
    }

    function setTokenAddress(IERC20 _token) external onlyOwner {
        require(address(token) == address(0), "BullFarmFund: token is already set");
        token = _token;
    }
