...
interface IMimeticMetadata {
    ...
    function setRevealed(uint256 _layerId, uint256 _tokenId) external;

    function getGenerationToken(uint256 _offset, uint256 _tokenId)
        external
        view
        returns (uint256 generationTokenId);
...
// File contracts/Mimetics/MimeticMetadata.sol
...
contract MimeticMetadata is IMimeticMetadata, Ownable {
     ...
     */
    function _getOffset(uint256 _layerId) internal view returns (uint256) {
        return
            (uint256(
                keccak256(
                    abi.encodePacked(
     ...
     */
    function setRevealed(uint256 _layerId, uint256 _topTokenId)
        public
        virtual
        override
        onlyOwner
    {
        ...
        if (!generation.loaded || !generation.enabled)
            revert GenerationNotEnabled();
     ...
     *         token holders.
     */
    function toggleGeneration(uint256 _layerId)
        public
        virtual
        override
        onlyOwner
    {
        ...
        if (generation.enabled && generation.locked)
            revert GenerationNotToggleable();
     ...
     */
    function _getTokenGeneration(uint256 _tokenId)
        internal
        view
        virtual
        returns (uint256)
        ...
        returns (bool)
    {
        ...
        if (generation.evolutionClosure != 0)
            return block.timestamp < generation.evolutionClosure;
        return true;
    }
     ...
     */
    function _focusGeneration(uint256 _layerId, uint256 _tokenId)
        internal
        virtual
    {
        uint256 activeGenerationLayer = tokenToGeneration[_tokenId];
        ...
        Generation memory activeGeneration = generations[activeGenerationLayer];
        if (activeGeneration.sticky && _layerId < activeGenerationLayer)
            revert GenerationNotDowngradable();

        // Make sure they've supplied the right amount of money to unlock access
        bytes32 tokenIdGeneration = keccak256(
            abi.encodePacked(_tokenId, _layerId)
        );
        if (
            msg.value + tokenGenerationToFunded[tokenIdGeneration] !=
            generation.cost
        ) revert GenerationCostMismatch();
...
contract Doodles is ERC721, ERC721Enumerable, Ownable {
    ...
    uint256 public constant PRICE_PER_TOKEN = 0;
