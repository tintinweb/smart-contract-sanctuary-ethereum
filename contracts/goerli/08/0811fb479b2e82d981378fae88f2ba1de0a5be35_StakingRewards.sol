...
contract StakingRewards is Initializable {
  ...
  /**
   * @dev To change USE_BAL_REWARD_POOL boolean value
   * @param _useBalRewardPool boolean value to use bal reward pool
   */
  function changeUseBalRewardPool(
    bool _useBalRewardPool
  ) external onlyOperator {
    USE_BAL_REWARD_POOL = _useBalRewardPool;
    emit UseBalRewardPoolUpdated(_useBalRewardPool);
  }
  ...
  function stake(uint256 _amount) external {
    ...
    if (useBalRewardPool()) {
      ...
      require(address(balancerGauge) != address(0), 'INVALID_BALANCER_GAUGE');
    }
