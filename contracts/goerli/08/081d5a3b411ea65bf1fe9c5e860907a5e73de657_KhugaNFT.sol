    ...
    DefaultOperatorFilterer
{
	...
	struct mintStage{
		...
		mapping(address => uint8) 	amountClaimed;
		uint256						cost;
		uint16						reservedAmount;
		uint8						maxAmountPerAddress;
		uint8						maxAmountPerTx;
	}

	uint16 public maxSupply = 5555;

    uint8 currMintStage = 255; // Placeholder for disable all mints
    ...
    constructor(
		...
		miawlistMintStage.cost = 1000;
		miawlistMintStage.reservedAmount = 1000;
		...
		allowlistMintStage.cost = 2000;
		allowlistMintStage.reservedAmount = 1000;
		...
		publicMintStage.cost = 3000;
		publicMintStage.reservedAmount = maxSupply;
		publicMintStage.maxAmountPerAddress = 2;
		publicMintStage.maxAmountPerTx = 2;
	}

    modifier mintCompliance(uint8 _stage, uint8 _mintAmount) {
        require(
		...
		require(
			mintStages[_stage].reservedAmount - _mintAmount >= 0,
			"Insufficient reserved amount!"
		);
    ...
    function whitelistMint(uint8 _whitelistLevel, uint8 _mintAmount, bytes32[] calldata _merkleProof)
        public
        payable
    ...
    // Reserve tokens
    function reserveTokens(address _receiver, uint8 _mintAmount)
        public
        onlyOwner
    {
        require(
    ...
    function setCost(uint8 _stage, uint256 _cost) public onlyOwner {
        mintStages[_stage].cost = _cost;
    }

    // Set Max Mint Amount
    function setMaxMintAmountPerTx(uint8 _stage, uint8 _maxMintAmountPerTx)
        public
        onlyOwner
    {
    ...
    function setMerkleRoot(uint8 _whitelistLevel, bytes32 _merkleRoot) public onlyOwner {
		require(_whitelistLevel <= 1, "Invalid whitelist level");
        mintStages[_whitelistLevel].merkleRoot = _merkleRoot;
    }

	function getMerkleRoot(uint8 _whitelistLevel) public onlyOwner view returns (bytes32){
		require(_whitelistLevel <= 1, "Invalid whitelist level");
        return mintStages[_whitelistLevel].merkleRoot;
    }

    // Set Whitelist Status
    function setMintStage(uint8 _stage) public onlyOwner {
		require(_stage <= 2, "Invalid Mint Stage");
        currMintStage = _stage;
    }

	function getMintStage() public view returns (uint8) {
        return currMintStage;
    }

    function getMintCost(uint8 _stage) public view returns (uint256) {
    ...
    function withdrawFund() external onlyOwner nonReentrant {
        (bool os, ) = payable(0x8626f6940E2eb28930eFb4CeF49B2d1F2C9C1199).call{
