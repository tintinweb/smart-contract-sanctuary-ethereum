...
contract ReentalFactory is UUPSUpgradeableByRole, BeaconFactory, RefundableUpgradeable {
    ...
    mapping(address => bytes32) public hashIdOf;

    event NewToken(bytes32 indexed hashId, address indexed proxy, string indexed name, string symbol, uint256 totalSupply);

    function initialize() public initializer {
        __AccessControlProxyPausable_init(msg.sender);
    }

    function createToken(bytes32 hashId, bytes calldata initializeCalldata) public onlyRole(_FACTORY_ADMIN_ROLE) returns(address) {
        address proxy = _createProxy(initializeCalldata);
        addressOf[hashId] = proxy;
        hashIdOf[proxy] = hashId;
        (string memory name, string memory symbol, uint256 totalSupply) = abi.decode(initializeCalldata[4:], (string, string, uint256));

        emit NewToken(hashId, proxy, name, symbol, totalSupply);
