...
contract SilverGerege is ERC721Enumerable, ERC2981, Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    using SafeMath for uint8;

    event transferEvent(uint256 tokenId, address from, address to, address sender);
    ...
    constructor(string memory baseURI) ERC721("Silver Gereges", "SILVERGEREGE") {
        setBaseURI(baseURI);
    ...
    function _calMintCost(uint256 num) private view returns (uint256 cost) {
        ...
        if (step == nextStep) {
            ...
            cost = (lowStepCount.mul(price)).add(highStepCount.mul(price.mul(125).div(100)));
        }
    }
    ...
    function mint(uint256 num) public payable nonReentrant {
        ...
        require(supply + num <= maxSupply, "Exceeds maximum Gerege supply");

        uint256 cost = _calMintCost(num);
        ...
        uint256 new_supply = totalSupply();
        if (supply.div(500) != new_supply.div(500)) {
            price = price.mul(125).div(100);
        }
    }
    ...
    function updateXp(uint256 tokenId, uint256 _newXp) external onlyManager(msg.sender) {
        require(_exists(tokenId), "ERC721Metadata: URI query for nonexistent token");
        NFTInfo memory info = NFTInfos[tokenId];
        info.xp = _newXp;
        NFTInfos[tokenId] = info;
    }
    ...
    function setPublicMint(bool _public_mint) external onlyOwner {
        publicMint = _public_mint;
    }

    function setMintWhitelist(address _mint_address, bool _isWhitelist) external onlyOwner {
        require(_mint_address != address(0), "Zero address");
        mintWhitelist[_mint_address] = _isWhitelist;
    }
    ...
    function updateMaxSupply(uint256 _max_supply) external onlyOwner {
        require(_max_supply > 0, "max supply can not be zero");
