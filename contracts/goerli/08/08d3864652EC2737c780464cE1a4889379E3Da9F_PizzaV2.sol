...
//Openzepplin libraries for controlling upgradability and access.
...
contract Pizza is Initializable, UUPSUpgradeable, OwnableUpgradeable {
    ...
    //upgradeable 계약은 반드시 initialize 메소드를 constructor에 갖는다.abi그리고 initalizer키워드는 초기화하는데 오직 한번만 작동한다. 

    /**
     * @dev required by the OZ UUPS module 
     */
    function _authorizeUpgrade(address) internal override onlyOwner {}
    // transparent proxy pattern이 proxy contract를 경유해서 이루어지는 반면,
    // UUPS 패턴은 계약의 실행으로 실행되기 때문에 승인되지 않은 업그레이드로부터 보호하기 위해 필요하다. (onlyOwner통해서)
