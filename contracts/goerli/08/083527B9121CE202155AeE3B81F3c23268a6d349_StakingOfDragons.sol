...
contract StakingOfDragons is ERC721Holder {
    ...
    function getStakedNFTs(address _address) public view returns (uint256[] memory) {
        uint256[] memory ownedNFTs = denOfDragons.walletOfOwner(_address);
        uint256[] memory stakedNFTs = new uint256[](stakedNFTCount[_address]);
        uint256 matches = 0;
        uint256 counter = 0;
        while (matches < stakedNFTCount[_address]) {
    ...
    function viewTrueTokenBalance(address _address) public view returns (uint256) {
        ...
        uint256[] memory stakedNFTs = getStakedNFTs(_address);
        uint combinedSecsStaked = 0;
        uint256 counter = 0;
        while (counter < stakedNFTs.length) {
            uint256 id = stakedNFTs[counter];
