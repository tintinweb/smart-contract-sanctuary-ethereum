...
contract NFTMarket is ReentrancyGuard {
    ...
    mapping (address => mapping(uint256 => _Offer)) public offers;
    mapping (string  => mapping(address => uint256)) public userFunds;
    mapping(address => mapping(uint256 => Seller)) public sellers;
    ...
    event Offer(uint256 offerId, address nftcontract, uint256 id, address user, uint256 price, bool fulfilled, bool cancelled);
  ...
  function createMarketItem(address nftContract,uint256 tokenId,uint256 price) public nonReentrant {
    ...
    offerCount++;
    offers[nftContract][offerCount] = _Offer(offerCount, nftContract, tokenId, msg.sender, price, false, false);

    IERC721(nftContract).transferFrom(msg.sender, address(this), tokenId);

    emit Offer(offerCount, nftContract, tokenId, msg.sender, price, false, false);
  }

  /* Cancel Listed item */
  function cancelMarketItem(address nftContract, uint256 _offerId) public nonReentrant {
    _Offer storage _offer = offers[nftContract][_offerId];
    ...
    IERC721(nftContract).transferFrom(address(this), msg.sender, _offer.id);
  ...
  function createMarketSale(address nftContract,uint256 _offerId) public payable nonReentrant {
    _Offer storage _offer = offers[nftContract][_offerId];
    require(_offer.offerId == _offerId, "The offer must exist");
    ...
    // Marketing fee 2%
    uint256 marketingfee = msg.value * 2 / 100; // marketing fee 2 %
    payable(treasury).transfer(marketingfee);
    // Pay buyer from seller
    payable(_offer.user).transfer(msg.value * 98 / 100);
    
    sellers[nftContract][count].userAddres = _offer.user;
    sellers[nftContract][count].balance = msg.value;
    count++;
