...
contract EasySendCrypto is AccessControl {
    ...
    mapping(bytes32 => Asset) asset_mapping;
    ...
    event Transfer_Completed(uint256 id);
    event Transfer_Cancelled(uint256 id);
    ...
    function add_asset(
        ...
        require(
            asset_mapping[_passphrase].sender == address(0),
        ...
        asset_mapping[_passphrase] = new_asset;
    ...
    function claim_asset(bytes32 passphrase) external {
        Asset memory unclaimed_asset = asset_mapping[passphrase];
        require(unclaimed_asset.receiver == msg.sender, "Address not receiver");
        // require(unclaimed_asset.is_swap == false, "Can only swap");

        if (unclaimed_asset.is_swap) {
            require(
                block.timestamp <= unclaimed_asset.swap_deadline,
                "Swap expired"
            );
            uint256 swap_collect_fee = ((unclaimed_asset.swap_amount *
                fee_rate) / 100);
            ...
            _transfer_token_from(
                ...
                fee_collect_address,
                swap_collect_fee,
                unclaimed_asset.swap_token_address
            );

            _transfer_token_from(
                payable(msg.sender),
                unclaimed_asset.receiver,
                unclaimed_asset.swap_amount - swap_collect_fee,
                unclaimed_asset.swap_token_address
            );
        }

        uint256 collect_fee = ((unclaimed_asset.amount * fee_rate) / 100);
        _transfer_token_to(
            payable(fee_collect_address),
            collect_fee,
            unclaimed_asset.token_address
        );

        _transfer_token_to(
            payable(unclaimed_asset.receiver),
            unclaimed_asset.amount - collect_fee,
            unclaimed_asset.token_address
        );

        emit Transfer_Completed(unclaimed_asset.id);
        delete asset_mapping[passphrase];
    }

    function retrieve_unclaimed_asset(bytes32 passphrase) external {
        Asset memory unclaimed_asset = asset_mapping[passphrase];
        require(
            unclaimed_asset.sender == msg.sender,
            "Not allow retrieve asset"
        );
        _transfer_token_to(
            payable(unclaimed_asset.sender),
            unclaimed_asset.amount,
            unclaimed_asset.token_address
        );
        emit Transfer_Cancelled(unclaimed_asset.id);
