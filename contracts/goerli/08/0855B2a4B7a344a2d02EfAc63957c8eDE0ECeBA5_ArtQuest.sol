...
contract ArtQuest is ERC721, ERC721Enumerable, Ownable {
     ...
     *
     * - return 0 if the mint has ended - game over!
     * - return 1 if the timer has not started
     * - return 2 if the timer has ended
     * - return 3 if the timer is running
     */
    function mintStatus() public view returns (uint256) {
        if (restartCount == MAX_RESTARTS && timerEnded()) return 0;
        if (currentId == 0 || restartedAtBlock > tokens[currentId].mintingBlock)
            return 1;
        if (timerEnded()) return 2;
        return 3;
    }
    ...
    function mint() external payable {
        ...
        if (mintStatus() == 0) revert MintEndedForever();
        ...
        emit EvMinted(++currentId, msg.sender, block.number, tick);
