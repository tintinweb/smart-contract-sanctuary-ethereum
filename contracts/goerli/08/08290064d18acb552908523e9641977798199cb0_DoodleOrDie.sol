...
contract DoodleOrDie is DoodleProxyStorage, Context, ERC165, IERC721, IERC721Metadata {
     ...
     */
    function initialize(string memory name_, string memory symbol_, uint256 chainID_) public virtual{
        require(msg.sender == admin, 'Doodle::initialize: admin only');
        _name = name_;
        _symbol = symbol_;
        INITIAL_CHAIN_ID = chainID_;
    ...
    function approve(address to, uint256 tokenId) public virtual override {
        address owner = DoodleOrDie.ownerOf(tokenId);
    ...
    function _safeMint(
        ...
        _mint(creator, to, tokenId,owner_,deadline, v, r, s);
        require(
    ...
    function _mint(
        ...
        checkMintApproval(owner_, creator, deadline, v, r, s);
    ...
    function _transfer(
        ...
        require(DoodleOrDie.ownerOf(tokenId) == from, 'ERC721: transfer of token that is not own');
    ...
    function _approve(address to, uint256 tokenId) internal virtual {
        ...
        emit Approval(DoodleOrDie.ownerOf(tokenId), to, tokenId);
    }
    ...
    function checkMintApproval(
        ...
        require(deadline >= block.timestamp, "PERMISSION_DEADLINE_EXPIRED");
        ...
        unchecked {
                ...
                keccak256(
                    abi.encodePacked(
                        ...
                        keccak256(
                            abi.encode(
                                keccak256(
                                    "Permit(address owner,address user,uint256 nonce,uint256 deadline)"
                                ),
                                owner,
                                user,
                                nonces[owner]++,
            ...
            require(recoveredAddress != address(0) && recoveredAddress == admin, "INVALID_SIGNER");
        }
    }
