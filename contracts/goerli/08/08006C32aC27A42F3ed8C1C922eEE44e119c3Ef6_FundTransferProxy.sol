  ...
  UUPSUpgradeable
{
   ...
   * @dev Initializer
   *
   * @param uzuVaultAddress_ IUzuVault
   */
  function __FundTransferProxy_init(
    IUzuVault uzuVaultAddress_
   ...
   */
  function setUzuVaultAddress(
    IUzuVault newAddress
    ...
    require(
      uzuVault != newAddress,
      "FundTransferProxy: existing same UzuVault"
    );

    uzuVault = IUzuVault(newAddress);
  }

  /**
   * @dev See {IFundTransferProxy.depositNativeTokenToVault}
   */
  function depositNativeTokenToVault(
    address sender
  ) external payable virtual override {
    require(
      sender != address(0),
      "FundTransferProxy:depositETHToVault invalid sender"
    );

    uzuVault.depositNativeToken{value: msg.value}(sender);
  }
  ...
  function depositERC20ToVault(
    ...
    require(amount > 0, "FundTransferProxy:depositERC20ToVault invalid amount");

    tokenAddress.transferFrom(sender, address(uzuVault), amount);

    uzuVault.depositERC20(tokenAddress, amount, sender);
  }

  /**
   * @dev See {IFundTransferProxy.transferNativeTokenFromVault}
   */
  function transferNativeTokenFromVault(
    address to,
    uint256 amount
  ) external virtual override onlyAdmin {
    uzuVault.transferNativeToken(to, amount);
  }
  ...
  function transferERC20FromVault(
    ...
    uzuVault.transferERC20(tokenAddress, to, amount);
  }
}