...
contract TrpzStake is IERC721Receiver, AccessControl, ReentrancyGuard {
    ...
    // Change this to a view function --> add the storage change to external
    ...
    // Change this to a view function --> update historic rewards then done in functions
    ...
    function getNonHistoricRewards() external view returns (uint256) {
        return _updateRewards(msg.sender);
    }

    function getHistoricRewards() external view returns (uint256) {
        return _updateHistoricRewards(msg.sender);
    }
}
