...
contract ERC20 is IERC20,Ownable{
    ...
    uint256 public _exchangeRate = 1000000000; //当前兑换比例
    uint256 public _tokenRaised = 0;
    uint256 private _allPresale = formatDecimals(2500000000);
    ...
    function currentExchageRate() public returns (uint256) {
        //预售总量减去已经售卖数量，计算剩余TOKEN
        uint256 leftPresale = _allPresale.sub(_tokenRaised);
        uint256 rate = leftPresale.div(_allPresale).mul(100);
        if(rate <= 100 && rate >=90){
            _exchangeRate;
        }else if(rate >= 50 && rate < 90){
            _exchangeRate = 500000000;
        }else if(rate >=25 && rate < 50){
            _exchangeRate = 250000000;
        }else{
            _exchangeRate = 125000000;
        }
