...
contract TestSimpleTransfer is MessageSenderApp, MessageReceiverApp {
    ...
    struct TestSimpleTransferStatus {
        bytes32 h; // hash(receiver, dstChainId)
        TransferStatus status;
    }
    mapping(uint64 => TestSimpleTransferStatus) public status; // nonce -> TestSimpleTransferStatus
    ...
    function batchTransfer(
        ...
        nonce += 1;
        status[nonce] = TestSimpleTransferStatus({
    ...
    function executeMessageWithTransfer(
        ...
        IERC20(_token).safeTransfer(transfer.account, _amount);
