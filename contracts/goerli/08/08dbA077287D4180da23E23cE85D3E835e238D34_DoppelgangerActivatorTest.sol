...
/// @title Doppelganger Activator Test
...
/// @notice contract where each owner can set their metadata from an array of choices, but onyl if they own a token for an activator collection
...
contract DoppelgangerActivator is ERC1967Proxy {
    ...
    function setActivatorContract(address activatorContractAddress) external {
        ...
        require(!store.isSet, "Other contract has already been set");
    
        store.contractInstance = IERC721(activatorContractAddress);
        store.isSet = true;
    ...
    function doppelgang(uint256 tokenId, uint256 tokenUriIndex) external {
        ...
        if (store.contractInstance.balanceOf(msg.sender) <= 0) revert Unauthorized();
    ...
    function viewActivatorContract() external view returns (address) {
        ...
        return address(store.contractInstance);
    }
}

contract DoppelgangerActivatorTest is DoppelgangerActivator {
