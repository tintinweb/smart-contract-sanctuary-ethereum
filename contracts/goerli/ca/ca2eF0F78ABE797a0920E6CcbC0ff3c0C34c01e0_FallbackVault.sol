    ...
    ERC165Upgradeable
{
    ...
    function registerIncomingFunds(address user) external payable {
        require(
            ...
            'FallbackVault: Registrar not set'
        );
        require(
            msg.sender == registrarAddress,
            'FallbackVault: Only Registrar can register incoming funds'
        );
        balances[user] += msg.value;
    }

    function setRegistrarAddress(address _registrarAddress) external onlyOwner {
        require(
            _registrarAddress != address(0),
            'FallbackVault: registrarAddress is the address zero'
        );
        address previousAddress = registrarAddress;
        registrarAddress = _registrarAddress;

        emit RegistrarAddressChanged(previousAddress, _registrarAddress);
    }
