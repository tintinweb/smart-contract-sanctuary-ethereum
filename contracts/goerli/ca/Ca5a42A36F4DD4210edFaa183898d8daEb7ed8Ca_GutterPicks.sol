...
contract GutterPicks is Ownable {
    ...
    mapping (string => Contest) public contests;
    uint256 public contestCount;

    bool public isOnlyWhitelistAllowed;
    ...
    event NewLineup(string contestId, address participant, uint256 lineupId, uint256 lineup, uint256[] backupBets);
    event EditLineup(string contestId, address participant, uint256 lineupId, uint256 lineup, uint256[] backupBets);
    ...
    */
    ...
    function createLineup(string calldata contestId, uint256 lineup, uint256[] calldata backupBets) external {
        ...
        emit NewLineup(contestId, msg.sender, entryId, lineup, backupBets);
    }

    function editLineup(string calldata contestId, uint256 lineupId, uint256 lineup, uint256[] calldata backupBets) external {
        Contest storage contest = contests[contestId];
        require(!contest.isCanceledOrFinished, "GutterPicks: contest is canceled or finished");
        require(contest.creator != address(0), "GutterPicks: contest doesn't exist");
        require(!contest.excludedParticipants[msg.sender], "GutterPicks: participant excluded");
        ...
        emit EditLineup(contestId, msg.sender, lineupId, lineup, backupBets);
    }
