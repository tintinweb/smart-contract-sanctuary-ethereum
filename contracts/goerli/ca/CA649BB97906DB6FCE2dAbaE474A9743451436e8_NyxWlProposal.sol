...
import "./interfaces/INyxWlProposal.sol";

contract NyxWlProposal is NyxProposal, INyxWlProposal {
    string public constant name = "NyxWlProposal";
        ...
        returns (WLProposalParams memory)
    {
        ...
        return WLProposalParams(WLAddressAction(bytesToUint(params[0])),
            bytesToAddress(params[1]),
    ...
    function getProposal(bytes memory proposalBytes)
        ...
        returns (bytes[] memory)
    {
        WLProposalParams memory params = parseProposalParams(proposalBytes);
        ...
        returns (bytes[nparams][] memory props)
    {
        ...
        {
            WLProposalParams memory params = parseProposalParams(proposalBytesArr[idx]);
...
// @title INyxWlProposal for Nyx DAO
// @author TheV
...
interface INyxWlProposal {
    ...
    function parseProposalParams(bytes[] memory params) external pure returns (WLProposalParams memory);
    function parseProposalParams(bytes memory params) external pure returns (WLProposalParams memory);
    function proposalToBytes(WLProposal memory proposal) external pure returns (bytes memory);
