...
contract Plugman is IPlugman, ERC721A, Ownable, ERC2981, ReentrancyGuard, Pausable {
	...
	function _verifyMint(
		...
		if (mintPhase == MintPhase.PUBLIC) {
			...
			bytes32 root = mintPhase == MintPhase.ALLOWLIST ? allowlistRoot : waitlistRoot;

			bytes32 leaf = _getLeaf(_msgSenderERC721A(), freeAllowance, paidAllowance);
			...
			uint256 claimed = mintPhase == MintPhase.ALLOWLIST ? allowClaims : waitClaims;

			emit Claimed(allowClaims, waitClaims);

			quantity = Math.min(quantity, freeAllowance + paidAllowance - claimed);
			...
			uint256 freeSurplus = freeAllowance > claimed ? freeAllowance - claimed : 0;
			uint256 costQuantity = quantity < freeSurplus ? 0 : quantity - freeSurplus;
	 ...
	 */
	function getClaimed(address by) public view returns (uint16 allowClaims, uint16 waitClaims) {
	...
	function _getLeaf(
		...
		return keccak256(abi.encodePacked(account, freeAllowance, paidAllowance));
	}
	...
	function _mint(uint256 quantity, uint256 costQuantity) private nonReentrant {
