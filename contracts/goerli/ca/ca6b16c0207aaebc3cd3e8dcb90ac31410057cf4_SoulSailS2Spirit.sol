...
contract SoulSailS2Spirit is ERC2981, ERC721A, Ownable, ReentrancyGuard, DefaultOperatorFilterer {
    ...
    function holderClaim(uint256 amount,bytes memory messageSignature,bytes32[] calldata holderSignature,bytes32[] calldata claimAmountSignature) external payable {
        ...
        if (!isValidClaimAmount( string.concat( string(abi.encodePacked(msg.sender)), ",", Strings.toString(amount)) ,claimAmountSignature)) revert ErrorNotValidClaimAmount();
    ...
    function isValidClaimAmount(string memory claimAmountString, bytes32[] calldata signature) public view returns (bool) {
        bytes32 leaf = keccak256(abi.encodePacked(claimAmountString));
