...
contract NounsSeeder is INounsSeeder {
    ...
    function generateSeed(uint256 nounId, INounsDescriptor descriptor) external view override returns (Seed memory) {
        ...
        uint256 partyGlassesCount = descriptor.partyGlassesCount();
        ...
        return Seed({
            ...
            partyGlasses: uint48(
                uint48(pseudorandomness >> 240) % partyGlassesCount
            )
        });
    }
}
...
interface INounsDescriptor {
    ...
    function partyGlasses(uint256 index) external view returns (bytes memory);
    ...
    function partyGlassesCount() external view returns (uint256);
    ...
    function addManyPartyGlasses(bytes[] calldata partyGlasses) external;
    ...
    function addPartyGlasses(bytes calldata partyGlasses) external;
