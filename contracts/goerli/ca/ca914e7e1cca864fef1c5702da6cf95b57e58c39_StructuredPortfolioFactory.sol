...
interface ITrancheVault is IERC4626Upgradeable {
    ...
    function withdrawStrategy() external view returns (IWithdrawStrategy);
    ...
    function setDepositStrategy(IDepositStrategy newStrategy) external;

    function setWithdrawStrategy(IWithdrawStrategy newStrategy) external;

    function setPortfolio(IStructuredPortfolio _portfolio) external;

    function transferToPortfolio() external returns (uint256);
...
interface ILoansManager is IAccessControlUpgradeable {
     ...
     * @return role Portfolio manager role
     */
    function MANAGER_ROLE() external view returns (bytes32 role);
}
...
import {ILoansManager} from "ILoansManager.sol";
...
struct TrancheData {
    uint240 initialValue;
    uint16 targetApy;
    uint128 minSubordinateRatio;
    uint128 maxSubordinateRatio;
    uint256 distributedAssets;
    uint256 optimisticWaterfallValue;
}
...
interface IStructuredPortfolio is ILoansManager {
...
import {IDepositStrategy} from "IDepositStrategy.sol";
import {IWithdrawStrategy} from "IWithdrawStrategy.sol";
import {IStructuredPortfolio, TrancheInitData} from "IStructuredPortfolio.sol";
import {ITrancheVault} from "ITrancheVault.sol";
...
struct TrancheData {
    string name;
    string symbol;
    /// @dev The strategy applied when calling deposit-related functions
    IDepositStrategy depositStrategy;
    /// @dev The strategy applied when calling withdraw-related functions
    IWithdrawStrategy withdrawStrategy;
    /// @dev The minimum amount of funds obtained in a tranche vault required to start the portfolio
    uint256 minSize;
    /// @dev The maximum amount of funds obtained in a tranche vault allowing to start the portfolio
    uint256 maxSize;
    ...
    uint128 minSubordinateRatio;
    /// @dev The maximum ratio of funds obtained in a tranche vault to its subordinate tranches
    uint128 maxSubordinateRatio;
}
 ...
 */
contract StructuredPortfolioFactory is AccessControl {
    bytes32 public constant WHITELISTED_MANAGER_ROLE = keccak256("WHITELISTED_MANAGER_ROLE");
    uint8 constant TRANCHES_COUNT = 3;
     ...
     */
    constructor(address _portfolioImplementation, address _trancheImplementation) {
     ...
     * @param duration Portfolio duration in seconds
     * @param startDeadline The date before which the portfolio should be started by the manager
    ...
    {
        trancheInitData = new TrancheInitData[](TRANCHES_COUNT);
        tranches = new ITrancheVault[](TRANCHES_COUNT);

        for (uint256 i = 0; i < TRANCHES_COUNT; i++) {
                ...
                address(
                    new ProxyWrapper(
                        ...
                        abi.encodeWithSelector(
                            ...
                            trancheData.symbol,
                            underlyingToken,
                            trancheData.depositStrategy,
                            trancheData.withdrawStrategy,
                            trancheData.minSize,
                            trancheData.maxSize,
                            i
                        )
            ...
            trancheInitData[i] = TrancheInitData(
                tranche,
                trancheData.targetApy,
                trancheData.minSubordinateRatio,
                trancheData.maxSubordinateRatio
            );
    ...
    /// @dev Reverts when message sender doesn't have a whitelisted manager role
    function requireWhitelistedManager() internal view {
        _checkRole(WHITELISTED_MANAGER_ROLE, msg.sender);
    }
}