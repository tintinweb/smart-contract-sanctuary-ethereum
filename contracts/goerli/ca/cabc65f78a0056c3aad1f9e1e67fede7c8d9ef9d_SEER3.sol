...
contract SEER3 is ERC721, ERC721Enumerable, Ownable {
    using SafeMath for uint256;


    uint256 private _unitPrice = 0.375 ether;
    ...
    modifier onlyCrossmint() {
        require(crossmintAddress == _msgSender(), "This function is for Crossmint only");
        _;
    }
    ...
    function batchMint(address _to, uint256 _tokens) private {
        uint256 supply = totalSupply();

        require(saleStarted, "Sale not started.");
        require(_tokens > 0);
        require(_tokens < 21,"You cannot mint more than 20 Tokens at once.");
        require( supply + _tokens <= maxSupply - _reserved, "Not enough Tokens left.");

        for (uint256 i=1; i <= _tokens; i++) {
            _safeMint(_to, supply + i);
        }
    }

    function crossmint(address _to, uint256 _tokens) public payable onlyCrossmint {
        uint256 _totalPrice = _tokens * _unitPrice;
        require(msg.value >= _totalPrice, "Ether value sent is not enough.");
        batchMint(_to, _tokens);
    }

    function mint(uint256 _tokens) external payable {
        uint256 _totalPrice = _tokens * _unitPrice;
        require(msg.value >= _totalPrice, "Ether value sent is not enough.");
        batchMint(msg.sender, _tokens);
    }

    function tokenURI(uint256 tokenId) public view override returns (string memory) {
        string memory _totkenURI = super.tokenURI(tokenId);
        return string(abi.encodePacked(_totkenURI, ".json"));
    }
