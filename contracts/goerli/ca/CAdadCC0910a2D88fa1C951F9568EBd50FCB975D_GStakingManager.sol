...
import "./interfaces/IGStakingManager.sol";
...
contract GStakingManager is AccessControl, ReentrancyGuard {
    ...
    uint256 public gpoolRewardPercent = 0;

    uint256 public firstStakingFee; //eth
    address payable public feeTo;

    bool upgradeFromOld = true;
    ...
    event UpgradeFromOld(bool upgradeFromOld);
    ...
    constructor(
        ...
        feeTo = _feeTo;
        oldContract = IGStakingManager(_oldContract);
    ...
    function getStakeInfoData(address user) public view returns (StakeInfo memory) {
        StakeInfo memory newStakeInfo = stakeInfo[user];
        if (upgradeFromOld) {
            IGStakingManager.StakeInfo memory oldStakeInfo = oldContract.stakeInfo(user);
            newStakeInfo.startStake = oldStakeInfo.startStake;
        }
        return newStakeInfo;
    }

    function _getAndSyncStakeInfo(address user) private returns (StakeInfo storage) {
        StakeInfo memory info = getStakeInfoData(user);
        stakeInfo[user] = info;
    ...
    function stake(uint256 amount) external payable nonReentrant {
        ...
        StakeInfo storage staker = _getAndSyncStakeInfo(msg.sender);
        require(
    ...
    function pendingReward(uint256 pid, address userAddress) public view validatePoolById(pid) returns (uint256) {
        ...
        StakeInfo memory staker = getStakeInfoData(userAddress);
    ...
    function getTier(address user) public view returns (Tier) {
        StakeInfo memory staker = getStakeInfoData(user);
        if (staker.startStake == 0) {
            return Tier.NORANK;
        }
    ...
    function setUpgrade(bool _upgradeFromOld) external onlyRole(GUARDIAN_ROLE) {
        upgradeFromOld = _upgradeFromOld;
        emit UpgradeFromOld(upgradeFromOld);
    }

    function setTiers(SetTierParam[] calldata params) external onlyRole(GUARDIAN_ROLE) {
        require(!upgradeFromOld, "Error: Only update from old contract");
...
interface IGStakingManager {
    ...
    function stakeInfo(address) view external returns (StakeInfo memory);
    function hadStake(address) view external returns (bool);
}
