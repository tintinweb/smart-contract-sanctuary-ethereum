    ...
    Ownable
{
    ...
    function transferShares(voucher.offer memory offerDetails, bool _isGlobal)
        external
    {
        require(balanceOf(msg.sender, offerDetails.tokenId) > 0, "OBI"); //Offeree Balance Insufficient

        uint256 shareTransferAmount;
        uint256 transferAmount;

        if (_isGlobal) {
            shareTransferAmount = balanceOf(msg.sender, offerDetails.tokenId);
            transferAmount = (shareTransferAmount * offerDetails.offeredPrice) / tokenSupply[offerDetails.tokenId];
        } else {
            ...
            shareTransferAmount = offerDetails.quantityAsked;
            transferAmount = offerDetails.offeredPrice;
        }
        uint256 fee = (transferAmount * platformFee) / 10000;
        IERC20(usdc).transfer(admin, fee);
        IERC20(usdc).transfer(msg.sender, (transferAmount - fee));
        _safeTransferFrom(
            msg.sender,
            offerDetails.offerer,
            offerDetails.tokenId,
     ...
     * @param   _tokenId  .
     */
    function acceptRedemption(address from,uint256 _tokenId) external {
        require(tokenSupply[_tokenId] > 0, "IT"); //Invalid Token
        require(balanceOf(from, _tokenId) == tokenSupply[_tokenId], "NE"); //Not Eligible
        _safeTransferFrom(
            from,
            admin,
            _tokenId,
            balanceOf(from, _tokenId),
            ""
        );
