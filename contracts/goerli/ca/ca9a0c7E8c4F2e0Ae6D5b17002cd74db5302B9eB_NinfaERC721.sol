...
/// @title Ninfa Communal ERC721 Collection v2
contract NinfaERC721 is AccessControl, ERC2981Communal, ERC721Enumerable {
    ...
    /**
     * @notice Grants `DEFAULT_ADMIN_ROLE` to the account that deploys the contract.
     */
    constructor() {
     ...
     */
    function mint(bytes32 _tokenURI) public returns (uint256 tokenId) {
        // MINTER_ROLE is assigned to contracts such as the marketplace, AND artists.
        // staticcall is not needed because NinfaMarketplace is a trusted contract. todo find out if staticcall or call cost less gas.
        (, bytes memory data) = ninfaMarketplace.call(
            abi.encodeWithSignature(
                "hasRole(bytes32,address)", // TODO encode with selector
                0x9f2df0fed2c77648de5860a4cc508cd0818c85b8b8a1ab4ceeef8d981c8956a6, // hardcoded because (for now) it is only used by this function, thus avoiding reading from storage.
                msg.sender
            )
        );
        ...
        royaltyReceivers[tokenId] = payable(msg.sender); // set royaltyReceivers[tokenId] before minting otherwise tokenId (i.e. total supply) will get increamented

        _mint(msg.sender, tokenId); // Since we don’t support minting to smart contracts, we can avoid using OpenZeppelin's `_safeMint()`, if you expecting smart contracts to be minting Ninfa NFTs, should use `_safeMint()` instead.
     ...
     * - the caller must have the `MINTER_ROLE` role
     */
    function mintAndTransfer(bytes32 _tokenURI, address _to) external {
        uint256 tokenId = mint(_tokenURI); // calling public mint function
...
contract ERC2981Communal {
    ...
    mapping(uint256 => address payable) internal royaltyReceivers; // internal as used by child contract
    mapping(uint256 => address) private artists; // tokenId to original creator address
...
contract ERC721 {
    ...
    // Token name
    string public constant name = "NINFA";

    // Token symbol
    string public constant symbol = "NINFA";
