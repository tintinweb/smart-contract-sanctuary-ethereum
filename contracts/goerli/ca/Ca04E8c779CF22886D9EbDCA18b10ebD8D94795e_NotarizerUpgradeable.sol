...
interface INotarizerUpgradeable {
    // Functions
    function initialize(address _feeColector, uint _feeInWei) external;

    function whitelistNotary(address _appAddress) external;

    function updateFee(uint _newFeeInWei) external;
    ...
    function getIpfsHash(uint _contentId) external view returns (string memory);
...
import "./interfaces/INotarizerUpgradeable.sol";
...
contract NotarizerUpgradeable is
    Initializable,
    OwnableUpgradeable,
    INotarizerUpgradeable
{
    // State Variables
    ...
    modifier onlyFeeCollector() {
        require(msg.sender == feeCollector, "This address is not allowed");
        _;
    }
    ...
    function initialize(
        ...
        __Ownable_init();
        feeCollector = _feeColector;
        currentFee = _feeInWei;
    }
    ...
    function withdrawAllCollectedFees() external onlyFeeCollector {
        (bool sent, ) = feeCollector.call{value: (address(this).balance)}("");
        require(sent, "Failed to send Ether");
    }

    function updateFee(uint _newFeeInWei) external onlyFeeCollector {
        currentFee = _newFeeInWei;
    }

    function updateFeeCollector(address _newFeeCollector) external {
        require(msg.sender == feeCollector, "You can not change feeCollector");
    ...
    function notarizeCID(
        ...
        // Notarization logic
        require(msg.value >= currentFee, "Must cover fee value");
