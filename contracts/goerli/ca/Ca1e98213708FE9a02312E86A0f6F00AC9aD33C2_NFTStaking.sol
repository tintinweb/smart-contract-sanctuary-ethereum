...
contract NFTStaking is Ownable, Pausable{
    ...
    uint256 public rewardCycle = 30 days;

    struct UserInfo {
        mapping(uint256=>uint256) staked;
        mapping(uint256=>uint256) rewardPeriod;
    ...
    address public WETH = 0xB4FBF271143F4FBf7B91A5ded31805e42b2208d6; // depends on network
    IFactory public factory = IFactory(0x5C69bEe701ef814a2B6a3EDD4B1652CB9cc5aA6f);
    IERC20 public rewardToken;
    Yonko public yonkoNFT;
    IFeed public feed;
    uint256 public minTokenUSD = 100000000;
    uint256 public perNFT = 1;
    uint256 public rewardPeriod = 1 minutes;
    mapping(address=>bool) public blackList;
    uint256 public bonusRate = 1000; // 10 = 1%
    uint256 public bonusPeriod = 3; // per 3 rewardPeriod, bonus 1%
    ...
    function setBlackList(address user, bool toBlock) external onlyOwner{
        blackList[user] = toBlock;
        emit blacklisted(user, toBlock);
    }

    function updateDepositCondition(uint256 _minTokenUSD, uint256 _perNFT) external onlyOwner {
        require(_minTokenUSD > 0 && _perNFT > 0, "Invalide setting");
    ...
    function updateReward(uint256[4] memory _rewardForRarity, uint256 _rewardCycle) external onlyOwner {
        require(rewardCycle > 0, "Invalide setting");
        rewardForRarity = _rewardForRarity;
        rewardCycle = _rewardCycle;
    }

    function updateRewardPeriod(uint256 _rewardPeriod) external onlyOwner {
        require(_rewardPeriod > 0, "Invalide setting");
        rewardPeriod = _rewardPeriod;
    }

    function updateBonusRate(uint256 _bonusRate, uint256 _bonusPeriod) external onlyOwner {
        require(_bonusRate >= 0 && _bonusPeriod > 0, "Invalide setting");
        bonusRate = _bonusRate;
        bonusPeriod = _bonusPeriod;
        emit bonusRateUpdated(_bonusRate, _bonusPeriod);
    }

    function depositAction(uint256 tokenID) internal {
        ...
        info.depositedCount++;
        info.staked[tokenID] = block.timestamp;
        info.rewardPeriod[tokenID] = rewardPeriod;
    ...
    function withdrawAll() external whenNotPaused{
        ...
        for(uint256 i = 0; i < stakedIDs.length; i++) {
            withdraw(stakedIDs[i]);
        }
    }

    function emergencyWithdraw(uint256 tokenID) public whenNotPaused{
        require(!blackList[msg.sender], "You are in blacklist");
        UserInfo storage info = userInfos[msg.sender];
        require(info.staked[tokenID] > 0, "You are not the staker of the token.");
        yonkoNFT.safeTransferFrom(address(this), msg.sender, tokenID);
    ...
    function pendingRewardInUSD(address user) public view returns(uint256) {
        ...
        return pendingReward - info.totalWithdrawnInUSD;
    }

    function rewardForTokenInUSD(address user, uint256 tokenID) public view returns(uint256) {
        UserInfo storage info = userInfos[user];
        uint256 stakedPeriod = (block.timestamp-info.staked[tokenID])/info.rewardPeriod[tokenID];
        uint256 rewardPerCycle = rewardForRarity[rarity[tokenID-1]-1];
        uint256 bonus = stakedPeriod/bonusPeriod;
        bonus = 10000 + (bonus*bonusRate);
        return (stakedPeriod*info.rewardPeriod[tokenID]*rewardPerCycle*bonus/(rewardCycle*10000));
    }
    ...
    function gettingRemainingTime(address user, uint256 tokenID) public view returns(uint256) {
        UserInfo storage info = userInfos[user];
        uint256 restDays = rewardPeriod - ((block.timestamp-info.staked[tokenID])%rewardPeriod);
        return restDays;
    }
