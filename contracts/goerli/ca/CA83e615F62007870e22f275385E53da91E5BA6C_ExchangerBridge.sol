...
/**
 * This smart contract is for cross chain exchange
 */
...
contract ExchangerBridge is BaseAggregator {
    using BytesLib for bytes;
    using EnumerableSet for EnumerableSet.Uint16Set;

    uint private _exchangeGas = 200000; // default gas of LayerZero

    event Exchange(uint16 indexed srcChainId, address recipient, uint amount, uint fee, bytes32 currencyKey);
    ...
    function _nonblockingLzReceive(
        ...
        (bytes memory accountBytes, bytes32 currencyKey, uint amountReceived, uint fee) = abi.decode(_payload, (bytes, bytes32, uint, uint));

        address recipient = accountBytes.toAddress(0);

        IExchanger(_srcContract).updateDestinationForExchange(recipient, currencyKey, amountReceived, fee);

        emit Exchange(_srcChainId, recipient, amountReceived, fee, currencyKey);
    }
    ...
    function updateDestination(
        ...
        uint fee,
        uint16 destChainId
    ) external onlySrcContract {
        require(_supportedChains.contains(destChainId), "EB_LZ_UA: Destination chain is not supported.");
        bytes memory lzPayload = abi.encode(abi.encodePacked(recipient), destinationCurrencyKey, amountReceived, fee);
        bytes memory adapterParam = _lzAdapterParam(0);

        (uint lzFee, ) = ILayerZeroEndpoint(lzEndpoint).estimateFees(destChainId, address(this), lzPayload, false, adapterParam);
        _lzSend(destChainId, lzPayload, payable(address(this)), address(0x0), adapterParam, lzFee);
    }

    /** Controlling LZ fee */

    function exchangeGas() external view returns (uint) {
        return _exchangeGas;
    }

    function setExchangeGas(uint newGas) external onlyOwner {
        _exchangeGas = newGas;
    }

    function _lzAdapterParam(uint16) internal view override returns (bytes memory) {
        uint16 version = 1;
        return abi.encodePacked(version, _exchangeGas);
    }
}
...
abstract contract BaseAggregator is Ownable, NonblockingLzApp {
    using EnumerableSet for EnumerableSet.Uint16Set;
