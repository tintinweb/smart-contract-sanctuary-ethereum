...
contract Laboratory {
    event TeamWalledDeployed(address indexed walletCreator, address indexed newContract);
    ...
    function NewStreamWallet(TeamWalletLibrary.Payee[] calldata team, bytes32 root) external returns (address) {
        require(team.length < 101, "Too Many Beneficiaries is prone to ran out of gas");
        address payable clone = payable(Clones.clone(implementation));
        StreamTeamWallet teamWallet = StreamTeamWallet(clone);
        teamWallet.initialize(team, root);
        emit TeamWalledDeployed(msg.sender, clone);
        return clone;
    }
}
