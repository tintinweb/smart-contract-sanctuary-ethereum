...
contract Vendor is Ownable {
  ...
  */
  function buyCrown(uint256 usdcToSpend) public {
    // Check that the requested amount of tokens to sell is more than 0
    require(usdcToSpend > 0, "Specify an amount of Crown greater than zero");

    uint256 crownSent = (usdcToSpend * CrownPerUSDC);///10**18;
    ...
    // Check that the Vendor's balance is enough to do the swap
    uint256 vendorBalance = crownToken.balanceOf(address(this));
    require(vendorBalance >= crownSent, "Vendor contract has not enough Crown");

    (bool recieved) = usdcToken.transferFrom(msg.sender, address(this), usdcToSpend);
    require(recieved, "Failed to transfer USDC from vendor to user");

    (bool sent) = crownToken.transferFrom(address(this), msg.sender, crownSent);
    require(sent, "Failed to transfer Crown from  to vendor");
  }
  ...
  */
  function withdrawCrown() external onlyOwner {
    ...
    (bool sent) = crownToken.transferFrom(address(this), msg.sender, vendorBalance);
