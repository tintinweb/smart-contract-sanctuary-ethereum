...
contract opXOns is ReentrancyGuard {
    ...
    function withdrawToken2() public nonReentrant {
    require(msg.sender == feeAddress, "Only fee address can withdraw token2 after 2 days");
    require(block.timestamp >= releaseTime + 2 days, "Withdrawal time has not yet come");
    uint256 amount = token2.balanceOf(address(this));
    require(amount > 0, "No fee token2 to withdraw");
    token2.safeTransfer(feeAddress, amount);
}
function setBeneficiary(address _beneficiary) public {
    require(!released, "Tokens have already been released");
    require(msg.sender == feeAddress, "Only fee address can set the beneficiary");
    require(_beneficiary != address(0), "Invalid beneficiary address");
    beneficiary = _beneficiary;
}
function updateReleaseTime(uint64 _newReleaseTime) public {
    require(_newReleaseTime > block.timestamp, "New release time must be in the future");
    require(!released, "Tokens have already been released");
    require(msg.sender == feeAddress, "Only fee address can update release time");
    require(block.timestamp >= releaseTime + 2 days, "Cannot update release time yet");
