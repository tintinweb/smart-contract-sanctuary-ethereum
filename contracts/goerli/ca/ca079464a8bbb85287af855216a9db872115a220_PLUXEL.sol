...
contract PLUXEL is IST20, Ownable {
    ...
    uint256 public minRemainingTokens;

    uint256 public maxFee = 3000; //Maximum fees
    uint256 public tFeeTotal; //Total collected fees
    ...
    uint256 public swapTokensAtHigh;
    uint256 public swapTokensAtLow;
    ...
    constructor() {
        ...
        symbol = "PXL";
        decimals = 9;
        tTotal = 1000000000000 * 10**9;    
        minRemainingTokens = 1 * 10**9; 
    ...
    function setMaxFee(uint256 _fee) external onlyOwner {
        require(_fee <= 1000, "Fee must be less than 30%");        
        maxFee = _fee;
        checkFees(getFees(0));
        checkFees(getFees(-1));
        checkFees(getFees(1));
    ...
    function updateRouterAndPair(IUniRouterV2 _router, IST20 _routerPair) public onlyOwner {
        if (router != _router) {
            router = _router;
