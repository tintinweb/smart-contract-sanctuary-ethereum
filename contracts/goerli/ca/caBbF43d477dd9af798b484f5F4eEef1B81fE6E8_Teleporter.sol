...
abstract contract IERC2309 {
  /**
  * @notice Emitted instead of {ERC721.Transfer} when several consecutive tokens are being transferred.
...
import "@lambdalf-dev/ethereum-contracts/contracts/interfaces/IERC2309.sol";
import "@lambdalf-dev/ethereum-contracts/contracts/utils/ERC173.sol";
import "@lambdalf-dev/ethereum-contracts/contracts/interfaces/IERC2981.sol";
...
INFTSupplyErrors, IERC721Errors, IERC721, IERC721Enumerable, IERC721Metadata, IERC2309,
IERC165, ERC173, IERC2981, UpdatableOperatorFilterer {
  // **************************************
  // *****    BYTECODE  VARIABLES     *****
  // **************************************
    ...
    string public constant name = "LL Teleporters"; // solhint-disable-line
    string public constant symbol = "LLTP"; // solhint-disable-line
  // **************************************
  ...
  // **************************************
    ...
    */
    function _exists(uint256 tokenId_) internal view returns (bool) {
      ...
      return tokenId_ < _nextId && ! BitMaps.get(_burned, tokenId_);
    }
    /**
    ...
    */
    function _restore(uint256 tokenId_, address to_) internal {
      if (! BitMaps.get(_burned, tokenId_)) {
        revert NOT_BURNED(tokenId_);
      }
      BitMaps.unset(_burned, tokenId_);
  ...
  // **************************************
    ...
    // ***********
      ...
      *   Throws unless `msg.sender` is the current NFT owner, or an authorized operator of the current owner.
      * 
      * @param operator_ the address approved to manage the token
      * @param tokenId_ identifier of the NFT being approved
      */
      function approve(address operator_, uint256 tokenId_)
      public
      virtual
      exists(tokenId_)
      onlyAllowedOperatorApproval(msg.sender) {
        address _tokenOwner_ = _ownerOf(tokenId_);
        if (operator_ == _tokenOwner_) {
        ...
        _approvals[ tokenId_ ] = operator_;
        emit Approval(_tokenOwner_, operator_, tokenId_);
      }
      /**
      * @notice Transfers the ownership of an NFT from one address to another address
  ...
  // **************************************
    ...
    * @param account_ the account that will receive the tokens
    * 
    * Requirements:
    * 
    * - Caller must be `delegate`.
    */
    function claimFor(uint256 qty_, address account_) public onlyDelegate {
      uint256 _remainingSupply_ = MAX_SUPPLY - supplyMinted();
      if (qty_ > _remainingSupply_) {
        revert NFT_MAX_SUPPLY(qty_, _remainingSupply_);
      }
      _mint2309(account_, qty_);
    }
    /**
    ...
    // *********************
      ...
      */
      function tokenOfOwnerByIndex(address tokenOwner_, uint256 index_) public view virtual override returns (uint256) {
        ...
        uint256 _index_ = 1;
        while (_index_ < _nextId) {
          if (_exists(_index_) && tokenOwner_ == _ownerOf(_index_)) {
            if (index_ == _count_) {
              return _index_;
            }
            unchecked {
