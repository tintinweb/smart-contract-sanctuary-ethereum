...
contract BaseRewardEscrowV2 is Owned, IRewardEscrowV2, LimitedSetup(8 weeks), MixinResolver {
    ...
    bytes32 private constant CONTRACT_REWARDESCROW_V2_AGGREGATOR = "RewardEscrowV2Aggregator";
    ...
    function rewardEscrowV2Aggregator() internal view returns (IRewardEscrowV2Aggregator) {
        return IRewardEscrowV2Aggregator(requireAndGetAddress(CONTRACT_REWARDESCROW_V2_AGGREGATOR));
    }
    ...
    function _transferVestedTokens(address _account, uint256 _amount) internal {
        ...
        require(collateralCurrency != address(0), "Missed collateral currency");
        if (collateralCurrency == NULL_ADDRESS) {
            _account.call.value(_amount)(new bytes(0));
        } else {
            IERC20(collateralCurrency).transfer(_account, _amount);
        }

        rewardEscrowV2Manager().sendVest(collateralKey, _account, _amount);
    ...
    function _appendVestingEntry(
        ...
        rewardEscrowV2Manager().sendAppend(collateralKey, account, quantity);
...
contract RewardEscrowV2 is BaseRewardEscrowV2 {
    ...
    constructor(
        ...
        address _resolver,
        bytes32 _collateralKey
    ) public BaseRewardEscrowV2(_owner, _resolver, _collateralKey) {}
...
interface IRewardEscrowV2Aggregator {
    ...
    function balanceOf(bytes32 currencyKey, address account) external view returns (uint256);
}
