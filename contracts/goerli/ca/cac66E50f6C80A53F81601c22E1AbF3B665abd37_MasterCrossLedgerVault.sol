...
contract MasterCrossLedgerVault is CrossLedgerVault, ERC20Burnable {
    ...
    function _addOperationToQueue(
        ...
        if (canNewOperationBeProcessedInstantly()) {
            startNextOperation();
        }
    }
    ...
    function _startDeposit(
        ...
        totalAssetsBeforeDeposit[opId] += rootVault.convertToAssets(
            rootVault.balanceOf(address(this))
        );
        ...
        totalAssetsAfterDeposit[opId] += rootVault.convertToAssets(
            rootVault.balanceOf(address(this))
        );
    ...
    function _startRedeem(
        ...
        uint256 amountToRedeem = (shares * rootVault.balanceOf(address(this))) /
            _totalSupply;
        if (amountToRedeem > 0) {
            uint256 assets = rootVaultZap.redeem(asset, amountToRedeem);
    ...
    function _startRebalance(
        ...
        if (srcChainId == block.chainid) {
            uint256 amountToRedeem = (shareToRebalance *
                rootVault.balanceOf(address(this))) / 1000;
            uint256 value = (amountToRedeem > 0)
                ? rootVaultZap.redeem(asset, amountToRedeem)
                : 0;
            IBridgeAdapter adapter = bridgeAdapters[asset][dstChainId];
            bytes32 transferId = adapter.sendAssets(value, asset, address(0));
            transferIdToOpId[transferId] = opId;
