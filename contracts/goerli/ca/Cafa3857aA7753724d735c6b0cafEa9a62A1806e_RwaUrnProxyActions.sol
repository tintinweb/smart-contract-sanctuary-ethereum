...
// hevm: flattened sources of src/RwaUrnProxyActions.sol
...
contract RwaUrnProxyActions {
    ...
    function close(address urn) external {
        ...
        require(RwaUrnLike_1(urn).can(msg.sender) == 1, "RwaUrnProxyActions/not-operator");

        RwaUrnLike_1(urn).wipe(wad);

        GemJoinAbstract gemJoin = RwaUrnLike_1(urn).gemJoin();

        VatAbstract vat = RwaUrnLike_1(urn).vat();
        bytes32 ilk = gemJoin.ilk();
        (uint256 ink, ) = vat.urns(ilk, urn);
        RwaUrnLike_1(urn).free(ink);
        ...
        try RwaUrnLike_1(urn).quit() {} catch {}
    }
    ...
    function _estimateWipeAllWad(address urn, uint256 when) internal view returns (uint256 wad) {
        ...
        bytes32 ilk = RwaUrnLike_1(urn).gemJoin().ilk();
        VatAbstract vat = RwaUrnLike_1(urn).vat();
        JugAbstract jug = RwaUrnLike_1(urn).jug();
