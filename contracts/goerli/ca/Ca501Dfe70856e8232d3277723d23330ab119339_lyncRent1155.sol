...
error NoSellersForThisAddressTokenId(address nftAddress, uint256 tokenId);
...
contract lyncRent1155 is ReentrancyGuard {
    ...
    modifier notAlreadyRentedSeller(
        ...
        if (listings.length > 0) {
            ...
            if (
                ...
                //revert ItemAlreadyRented(_nftAddress, _tokenId, _sellerAddress);
            } else if (
                tokenSellerStorage.tokenAmount -
                    tokenSellerStorage.tokenAmountAlreadyRented <
                _tokenAmount
        ...
        )
    {
        ...
        if (msg.value < tokenSellerStorage.pricePerDay * _duration) {
            revert PriceNotMet(
                _nftContractAddress,
                _tokenId,
        ...
        renterDetails.tokenAmountRented = _tokenAmount;
        ...
        (bool success, ) = payable(tokenSellerStorage.sellerAddress).call{
            value: msg.value
        }("");
