...
contract PrimaryIndexToken is Initializable, AccessControlUpgradeable, ReentrancyGuardUpgradeable
{
    ...
    function supply(address lendingToken, uint256 lendingTokenAmount, address account, address onBehalfOf) public isLendingTokenListed(lendingToken) {
        ...
        (uint256 mintError, uint256 mintedAmount) = lendingTokenInfo[lendingToken].bLendingToken.mintTo(account, lendingTokenAmount);
        require(mintError == 0,"PIT: mintError!=0");
        require(mintedAmount > 0, "PIT: mintedAmount==0");

        emit Supply(onBehalfOf, lendingToken, lendingTokenAmount, address(lendingTokenInfo[lendingToken].bLendingToken), mintedAmount);
    }

    function redeem(address lendingToken, uint256 bLendingTokenAmount, address account, address onBehalfOf) public isLendingTokenListed(lendingToken) {
        require(!lendingTokenInfo[lendingToken].isPaused, "PIT: lending token is paused");
        require(bLendingTokenAmount > 0, "PIT: bLendingTokenAmount==0");

        uint256 redeemError = lendingTokenInfo[lendingToken].bLendingToken.redeemTo(account, bLendingTokenAmount);
        require(redeemError == 0,"PIT: redeemError!=0. redeem>=supply.");

        emit Redeem(onBehalfOf, lendingToken, address(lendingTokenInfo[lendingToken].bLendingToken), bLendingTokenAmount);
    }
    ...
    function borrow(address projectToken, address lendingToken, uint256 lendingTokenAmount, address account, address onBehalfOf) public isProjectTokenListed(projectToken) isLendingTokenListed(lendingToken) {
        require(!primaryIndexTokenLeverage.isLeveragePosition(onBehalfOf, projectToken), "PIT: invalid position");
        address _lendingToken = lendingTokenPerCollateral[onBehalfOf][projectToken];
        uint borrowedUSD = totalOutstanding(onBehalfOf, projectToken, usdcToken);
        if (borrowedUSD > 0) {
        ...
        updateInterestInBorrowPositions(onBehalfOf, lendingToken);
        if (lendingTokenAmount == type(uint256).max) {
            lendingTokenAmount = convertPitRemaining(onBehalfOf, projectToken, lendingToken);
        }
        require(getPriceConvert(lendingToken, lendingTokenAmount) <= pitRemaining(onBehalfOf, projectToken, lendingToken), "PIT: lendingTokenAmount exceeds pitRemaining");
        ...
        calcBorrowPosition(account, onBehalfOf, projectToken, lendingToken, lendingTokenAmount, _lendingToken);
        
        emit Borrow(onBehalfOf, lendingToken, lendingTokenAmount, projectToken, depositPosition[onBehalfOf][projectToken][usdcToken].depositedProjectTokenAmount);
    }

    function calcBorrowPosition(address account, address borrower, address projectToken, address lendingToken, uint256 lendingTokenAmount, address currentLendingToken) public {
        ...
        info.bLendingToken.borrowTo(account, lendingTokenAmount);
