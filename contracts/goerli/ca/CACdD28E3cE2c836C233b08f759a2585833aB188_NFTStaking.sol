...
contract NFTStaking is Ownable, IERC721Receiver, Pausable {
  ...
  uint256 public totalSpaceSoilderStaked = 0;
  uint256 public totalAlienStaked = 0;
  uint256 public unaccountedRewards = 0; 
  uint256 public rewardPerSoilder = 0; 
  uint256 public constant SPACE_CLAIM_TAX_PERCENTAGE = 10;
  uint256 public spaceSoilderSteals = 0;
  ...
  function addVault(ERC721Enumerable _nft,Erc20 _token,string calldata _name) public onlyOwner {
        VaultInfo.push(
            vaultInfo({
                nft: _nft,
                token: _token,
  ...
  function stake(uint256 _pid, uint256[] calldata tokenIds) external {
    ...
    if (_pid == 0){ 
      totalAlienStaked += tokenIds.length;
    }
    else if (_pid == 1){
      totalSpaceSoilderStaked += tokenIds.length;
    }
  ...
  function _unstakeMany(address account, uint256[] calldata tokenIds, uint256 _pid) internal {
    ...
    //only claim route
    if(_pid == 0 && earned > 0) {
        _paySpaceTax(earned * SPACE_CLAIM_TAX_PERCENTAGE / 100);
        earned = earned * (100 - SPACE_CLAIM_TAX_PERCENTAGE) / 100;
    ...
    */
    ...
    function _paySpaceTax(uint256 amount) internal {
    if (totalSpaceSoilderStaked == 0) { // if there's no staked soilders
      unaccountedRewards += amount; // keep track of $WOOL due to soilders
      return;
    }
    // makes sure to include any unaccounted $WOOL 
    rewardPerSoilder += (amount + unaccountedRewards) / totalSpaceSoilderStaked;
