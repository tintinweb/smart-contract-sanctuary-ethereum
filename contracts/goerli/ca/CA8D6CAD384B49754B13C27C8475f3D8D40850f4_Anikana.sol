...
contract Anikana is ValidatorSmartContractInterface, IKnightsOfTheRoundTable, Ownable {
    ...
    uint256 private END_TERN_QUEEN = 21024000;//3s per block
    ...
    // @title Mapping to RewardHistory
    mapping(address => RewardHistory[]) private rewardHistoryList;

    
    StatusAppointAndDismiss[] public statusAppointAndDismiss;

    mapping(address => QueenDismissHistory[]) public queenDismissHistory;
    ...
    */
    enum StatusRequest {
        ValidatorCandidateApproved, //0
        ValidatorCandidateRejected, //1
        KnightAppoint, //2
        KnightDismiss, //3
        QueenAppoint, //4--
        QueenDismiss //5--
    ...
    */
    ...
    function cancelVoteQueen() external onlyKnight{
        ...
        onlyKnight
    {
        ...
        // add informastion for queen dismiss history
        queenDismissHistory[_msgSender()].push(
            QueenDismissHistory(
                currentQueen.queenAddr,
        ...
        onlyQueen
    {
        require(
            ...
            "ONLY VALIDATOR CANDIDATE CAN REPLACE NEW KNIGHT"
        );
    ...
    */
    ...
    // @param Address of Validator Candidate
    function getCurrentListAddressProposedList(address _validatorCandidaterAddress)
        public
        view
        returns (address[] memory)
    {
    ...
    function getAwardList() public view returns(RewardHistory[] memory){
        return rewardHistoryList[_msgSender()];
    }
    ...
    function getStatusAppointAndDismiss() public view returns(StatusAppointAndDismiss[] memory) {
        ...
        )
    {
        ...
        for(uint256 i = 0; i < NUMBER_KNIGHT; i++){
            knightAddress[i] = currentKnightList[i+1].knightAddr;
            knightIndex[i] = currentKnightList[i+1].index;
            knightStartTermBlock[i] = currentKnightList[i+1].startTermBlock;
        }
    ...
    // @param index of Knight
    function getApproveRequestsKnight(uint256 _idKnight)
        public
        view
        returns (
            address[] memory,
    ...
    {
        uint256 amountApproveRequestsKnight = 
        currentKnightList[_idKnight].appointedValidatorCandidateList.length;

        uint256[] memory dateRequestsBecomeValidatorCandidate = 
            new uint256[](amountApproveRequestsKnight);
        uint256[] memory dateBecomeValidatorCandidate = 
            new uint256[](amountApproveRequestsKnight);
    ...
    // get the knight's past voting information

    function getKnightVoteHistory(address _knightAddress) 
       public 
       view 
       returns (QueenDismissHistory[] memory)  
    {
        return queenDismissHistory[_msgSender()];
    }
    ...
    function updateTotalRewardOfValidator(
        ...
        rewardHistoryList[_validatorAddr].push(RewardHistory(block.number, valueTransferForKnight));
        ...
        rewardHistoryList[currentQueen.queenAddr].push(RewardHistory(block.number, valueTransferForQueen));
