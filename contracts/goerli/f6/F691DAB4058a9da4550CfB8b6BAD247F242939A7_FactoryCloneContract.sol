...
contract FactoryCloneContract is Ownable {
    ...
    constructor (
        ...
        //Registrar Address
        registrarAddress = address(new Registrar(_link,_registrar,_registry,address(0)));
    ...
    function createDAO(DAO calldata _params)external returns(address) {
        ...
        Registrar(registrarAddress).addAirDropCreaterRole(proxy);
        return proxy;
    }
...
contract Emitter is Initializable,AccessControl {
    ...
    event CreateDao(address indexed ownerAddress,address indexed proxy,string name, string symbol, uint256 totalDeposit,uint256 minDeposit, uint256 maxDeposit, uint256 ownerFee, uint256 _days, bool feeUSDC,uint256 quorum,uint256 threshold,address emitter,address[] daoAdmins,address registrar);   
...
contract ERC20NonTransferable is ERC20Upgradeable, AccessControl {
    ...
    function PerformAirdrop(address[] memory _users,uint256[] memory _amount,address _tokenAddress,uint256 _executionTime,string memory _airDropName) external {
        require(hasRole(ADMIN, msg.sender), "Only Admin");
        require(_users.length > 0, "Invalid Parameters");
        require(_users.length == _amount.length, "Invalid Parameters");
        require(_executionTime > (block.timestamp + 300), "Invalid Parameters");
        ...
        require(IERC20(_tokenAddress).balanceOf(address(this)) >= _totalAmount,"InsufficientFunds");

        IERC20(_tokenAddress).approve(registrar, _totalAmount);

        Registrar(registrar).registerAirdrop(_airDropName,_users,_amount,_tokenAddress,_executionTime,999999);
    }
...
contract Registrar is AccessControl {
  ...
  constructor(
    ...
    //address of the factory contrat
    manager = _manager;
  }

  /// @dev onlyOwner modifier to allow only Owner access to functions
  modifier onlyOwner() {
    require(manager == msg.sender, "Only Owner");
    _;
  }

  function addAirDropCreaterRole(address _implementation) public onlyOwner{
    _grantRole(AIRDROP, _implementation);
  }
