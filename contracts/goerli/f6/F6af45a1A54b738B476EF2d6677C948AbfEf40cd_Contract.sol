...
contract Contract is ERC721Drop, DefaultOperatorFilterer {
    ...
    function tokenURI(uint256 _tokenId) public view override returns (string memory) {
        if (isCustomURI) {
            return isReveal ? string(abi.encodePacked(ERC721A.tokenURI(_tokenId), baseExtension)) : unrevealTokenURI;
        } else {
            return ERC721Drop.tokenURI(_tokenId);
        }
    }

    //It is intentionally left out of OVERRIDE because it could be confusing at thirdweb dashboard etc.
    ...
    //For OpenSea(operator-filter-registry)
    function setApprovalForAll(address operator, bool approved) public override onlyAllowedOperatorApproval(operator) {
        super.setApprovalForAll(operator, approved);
    }

    //can't override approve
