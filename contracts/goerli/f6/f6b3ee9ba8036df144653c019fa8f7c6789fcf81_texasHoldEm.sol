...
contract texasHoldEm {
    ...
    constructor() {
        ...
        totalRoomNumber = 0;
        maxRoomNumber = 1;
        exchangeRate = 10000; //1ETH = 10000 BETCOIN
    }
    ...
    function withdraw(address _claimer, uint256 _BETCOIN) public onlyOwner {
        ...
        uint256 _claim = _BETCOIN * _ETH / exchangeRate;
    ...
    function withdrawList(address[] memory _claimers, uint256[] memory _BETCOINs) public onlyOwner {
        require(_claimers.length == _BETCOINs.length, "Claimers number not equal to fees number");
        uint256[] memory _claimFees = new uint256[](_claimers.length);
        uint256 _ETH = 1 * 10 ** 18;
        for(uint i = 0; i < _claimers.length; i++) {
            uint256 _claim = _BETCOINs[i] * _ETH / exchangeRate;
