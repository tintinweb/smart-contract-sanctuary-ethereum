    ...
    PausableUpgradeable
{
    ...
    mapping(address => uint256) public purchasedAmountETH;
    ...
    event PurchaseLimitsUpdated(uint256 min, uint256 max, uint256 timestamp);
     ...
     * @param _newClaimVest claim vest time in seconds
     * @param _minBuy min ETH payment per wallet
     * @param _maxBuy max ETH payment per wallet
     * @param _initialClaimPercent initialClaimPercent
     */
    function initialize(
        address _oracle,
        uint256 _startTime,
    ...
    function changeMinAndMaxBuy(
        uint256 _minBuy,
        uint256 _maxBuy
    ) external onlyOwner {
        minBuy = _minBuy;
        maxBuy = _maxBuy;
        emit PurchaseLimitsUpdated(_minBuy, _maxBuy, block.timestamp);
    }
        ...
        returns (bool)
    {
        ...
        if (purchasedAmountETH[_msgSender()] == 0) {
            require(
                ethAmount >= minBuy && ethAmount <= maxBuy,
                "Invalid purchase amount"
            );
        } else {
            require(
                purchasedAmountETH[_msgSender()] + ethAmount <= maxBuy,
                "Exceeds max limit"
            );
        }

        uint256 excess = msg.value - ethAmount;
        update(_msgSender(), amount);
        usdRaised += usdPrice;
        purchasedAmountETH[_msgSender()] += ethAmount;
    ...
    function update(address user, uint256 amount) internal {
        ...
        userDeposits[user].amount += (amount * baseDecimals);
        userDeposits[user].initialClaim =
            (userDeposits[user].amount * initialClaimPercent) /
            10000;
    }

    /**
     * @dev To buy into presale with ETH using Wert
        ...
        returns (bool)
    {
        ...
        if (purchasedAmountETH[_user] == 0) {
            ...
            require(
                purchasedAmountETH[_user] + ethAmount <= maxBuy,
                "Exceeds max limit"
            );
        }
        uint256 excess = msg.value - ethAmount;
        update(_user, _amount);
        usdRaised += usdPrice;
        purchasedAmountETH[_user] += ethAmount;
        sendValue(payable(paymentWallet), ethAmount);
    ...
    function claim() external whenNotPaused returns (bool) {
        ...
        userDeposits[_msgSender()].claimedAmount += tokens;
    ...
    function getClaimableAmount(
        ...
        UserDeposits memory deposit = userDeposits[_user];
        ...
        if (block.timestamp < (claimStart + claimVest)) {
            uint256 timePassedRatio = ((block.timestamp - claimStart) *
                baseDecimals) / ((claimVest));
    ...
    function changeClaimVest(uint256 _newClaimVest) external onlyOwner {
        require(block.timestamp < claimStart, "Claim already started");
        claimVest = _newClaimVest;
    }
}
