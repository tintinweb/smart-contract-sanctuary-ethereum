...
contract Campaign {
    ...
    function setParams(
        ...
        require(_endAt > _startAt, "End date should be after than Start date.");
        require(block.timestamp < _startAt, "Start date should be future.");
        name = _name;
        goal = _goal;
        startAt = _startAt;
        endAt = _endAt;
    ...
    function claimDrops() external onlyRecipient onlyIfClosed {
        ...
        require(goal <= balance, "Not enough balance.");
    ...
    function refundDrops() external onlyAdmin {
        require(!checkGoalIsReached(), "Goal is reached.");
        require(block.timestamp > endAt, "Campaign is open.");
        isRefunded = true;
    ...
    modifier onlyRecipient() {
        require(msg.sender == recipient, "Recipient is wrong.");
        _;
    }

    modifier onlyIfOpen() {
        require(isOpen, "Campaign is close");
        require(block.timestamp < endAt, "Campaign is ended.");
        _;
    }

    modifier onlyIfClosed() {
        require(!isOpen, "Campaign is open.");
        _;
    }
    ...
    modifier onlyAdmin() {
        require(msg.sender == admin, "Only contract admin.");
        _;
    }
