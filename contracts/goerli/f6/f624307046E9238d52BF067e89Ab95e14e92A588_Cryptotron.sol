...
// Copyright 2022, Andrew Novikov, All rights reserved.

pragma solidity ^0.8.7;

/*____________________________________________CRYPTOTRON____________________________________________*/
...
error Cryptotron__UpkeepNotNeeded(uint256 currentBalance, uint256 numPlayers, uint256 cryptotronState);
error Cryptotron__TransferFailed();
error Cryptotron__CryptotronNotOpen();
...
/*____________________________________________CRYPTOTRON____________________________________________*/
    ...
    bool private everythingIsBad = false;
...
/*____________________________________________CRYPTOTRON____________________________________________*/
    ...
    constructor(
        ...
        currentContract = 0xA284D7BC6C8cdC4F1Af352C1aCBba7Cb8f89b95a;
    }

/*____________________________________________CRYPTOTRON____________________________________________*/

    function enterCryptotron() internal {
        if (s_cryptotronState != cryptotronState.OPEN) {
            revert Cryptotron__CryptotronNotOpen();
        }
...
/*____________________________________________CRYPTOTRON____________________________________________*/
    ...
    function performUpkeep(bytes calldata) external override {
        require(everythingIsBad == false, "Everything is really bad, find 'emergencyWithdraw()'");
        enterCryptotron();
        (bool upkeepNeeded, ) = checkUpkeep("");
        if (!upkeepNeeded) {
            everythingIsBad = true;
            revert Cryptotron__UpkeepNotNeeded(
...
/*____________________________________________CRYPTOTRON____________________________________________*/
    ...
    function cryptotronBalance() public view returns (uint256) {
    ...
    function changeAddress(address newAddress) public onlyOwner {
        require(currentContract == nullAddress, "Cryptotron is not over yet");
        currentContract = newAddress;
    }

/*____________________________________________CRYPTOTRON____________________________________________*/

    function fundCryptotron() public payable {}

/*____________________________________________CRYPTOTRON____________________________________________*/

    function emergencyWithdraw() public {
        require(everythingIsBad == true, "It's not that bad");
        for (uint i = 0; i < s_players.length; i++) {
            s_players[i].transfer(0.01 ether);
        }
