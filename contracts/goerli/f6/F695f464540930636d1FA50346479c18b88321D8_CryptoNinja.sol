...
contract CryptoNinja is Ownable, IXReceiver {
    IConnext public immutable connext =
        IConnext(0xb35937ce4fFB5f72E90eAD83c10D33097a4F18D2);

    IERC20 public token = IERC20(0x7ea6eA49B0b0Ae9c5db7907d139D9Cd3439862a1);

    uint256 public bid = 5 * 10**18;
    ...
    event FinishGame(address winner, uint256 amount);

    constructor() {
        game = Game(address(0), address(0));
    }

    function finishGame(address _winner, uint32 destinationDomain)
        external
        payable
        onlyOwner
    {
        require(
            game.player1 == _winner || game.player2 == _winner,
            "address is not a player"
        );

        uint256 amount = ((bid * 2) * 9) / 10;

        emit FinishGame(_winner, amount);
    ...
    function xReceive(
        ...
        require(_asset == address(token), "Must pay in TEST tokens");
        address sender = abi.decode(_callData, (address));

        if (game.player1 == address(0)) {
            game.player1 = sender;
            emit GameStarted(game.player1, game.player2);
        } else if (game.player2 == address(0)) {
            game.player2 = sender;
            emit GameStarted(game.player1, game.player2);
        } else {
            revert("The game is already in progress");
        }
    }
...
/******************************************************************************\
* Author: Nick Mudge <[email protected]ectabstractions.com> (https://twitter.com/mudgen)
* EIP-2535 Diamonds: https://eips.ethereum.org/EIPS/eip-2535
/******************************************************************************/
