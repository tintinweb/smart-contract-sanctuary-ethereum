...
contract DappCampNFT is ERC721Enumerable, Ownable {
    ...
    string[] private collection = ["hair", "height", "eyes", "lips"];

    function random(string memory input) internal pure returns (uint256) {
        return uint(keccak256(abi.encodePacked(input)))%4; 
    }

    function toBytes(uint256 value) public returns (bytes memory b){
        b = new bytes(32);
    ...
    function pluck(
        ...
        require(sourceArray.length != 0, "source array should be greater than zero");
        // require(keyPrefix != "", "KeyPrefix should not be empty");
        // collection = sourceArray;

        uint rand_number = random(bytes32ToString(bytes32(abi.encodePacked(keyPrefix, tokenId))));
        require(rand_number < sourceArray.length, "random number error");
        return sourceArray[rand_number];
    }
        ...
        returns (string memory)
    {
        ...
        string memory imageURI = getImageURI(tokenId);
                ...
                abi.encodePacked(
                    ...
                    Base64.encode(
                        bytes(
                            abi.encodePacked(
                                '{"name":"',
                                "Dapp Camp NFT", // You can add whatever name here
                                '", "description":"NFT showing your best feature", "attributes":"", "image":"',
                                imageURI,
                                '"}'
                            )
                        )
