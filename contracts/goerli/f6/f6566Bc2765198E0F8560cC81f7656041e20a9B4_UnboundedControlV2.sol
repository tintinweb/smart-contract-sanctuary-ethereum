...
contract UnboundedControlV2 is ReentrancyGuard, PullPayment {
  ...
  //Stores minting price info for each metaverses for each base ids(templates)
  //given metaverse address and baseId, returns minting price
  mapping(address => mapping(uint256 => uint256)) public metaverseSelectedTemplates;
   ...
   *Connects NFT contract to mint single NFT.
  ...
  function mint(uint256 _baseId, address payable _metaverse) external payable nonReentrant {
    ...
    require(msg.value == metaverseSelectedTemplates[_metaverse][_baseId], "Please send the required amount!");
  ...
  /**
   *Checks the validity of given parameters and whether paid ETH amount is valid
   *Connects NFT contract to mint multiple NFT.
  ...
  /**
   *Checks the validity of given parameters and whether paid ETH amount is valid
   *Connects NFT contract to mint multiple NFTs with a single specified metadata slot
   *@param _baseIds: IDs of Templates which are categorized NFTs
   *@param _amounts: Mint amount for each ID
  ...
  /**
   *Checks the validity of given parameters and whether paid ETH amount is valid
   *Connects NFT contract to mint single NFT with multiple specified metadata slots
  ...
  function mintWithMetadaSlots(
    ...
    for (uint256 i; i < _metaverseIds.length; i++) {
      ...
      _metadataSlotPrice = metaverseMetadataAddPrice[_metaverseWallets[i]];
      ...
      registeredMetaverses[_metaverseWallets[i]].balance += _metadataSlotPrice;
    }
    require(msg.value == (_totalMetadataSlotPriceAmount + metaverseSelectedTemplates[_metaverseWallets[0]][_baseId]), "Please send the required amount!");
    unbounded.mintWithMetadaSlots(msg.sender, _baseId, _metaverseIds, _metaverseMetadataTemplate);
  }

  /**
   *Checks the validity of given parameters and whether paid ETH amount is valid
   *Connects NFT contract to add single NFT metadata to single NFT
   ...
   *Checks the validity of given parameters and whether paid ETH amount is valid
   *Connects NFT contract to add multiple metadata slots to single NFT
   *@param _tokenId: Token ID of the NFT
   *@param _metaverseIds: Number of mint from each IDs
  ...
  function addBatchMetaverseSlotsToTemplate(
    ...
    for (uint256 i; i < _metaverseIds.length; i++) {
      require(registeredMetaverses[_metaverseWallets[i]].id == _metaverseIds[i], "Metaverse wallet and ID mismatch!");
      _price = metaverseMetadataAddPrice[_metaverseWallets[i]];
      _metaverseMetadataTemplate[i] = (registeredMetaverses[_metaverseWallets[i]].metaverseMetadataTemplate);
      _totalAmount += _price;
      registeredMetaverses[_metaverseWallets[i]].balance += _price;
    }
   ...
   *Checks the validity of given parameters and whether paid ETH amount is valid
   *Connects NFT contract to add the same single metadata slot to multiple NFTs
  ...
  function registerMetaverse(
    ...
    unchecked {
      ...
      registeredMetaverses[_wallet] = Metaverse(_wallet,_companyWallet, id, registeredMetaverses[_wallet].balance, _name, _metaverseMetadataTemplate);
  ...
  */
  function selectTemplates(uint256[] calldata _baseIds) external {
    ...
    for (uint256 i; i < _baseIds.length; i++) {
      metaverseSelectedTemplates[msg.sender][_baseIds[i]] = templateMintPrice[_baseIds[i]];
    }
  ...
  */
  function unSelectTemplates(uint256[] calldata _baseIds) external {
    require(registeredMetaverses[msg.sender].id != 0, "Unregistered metaverse wallet!");
    for (uint256 i; i < _baseIds.length; i++) {
      delete metaverseSelectedTemplates[msg.sender][_baseIds[i]];
    }
    emit TemplatesSelected(_baseIds, msg.sender);
  }
  ...
  function calculateBatchPrice(
    ...
    for (uint256 i; i < _baseIds.length; i++) _totalAmount += (metaverseSelectedTemplates[_metaverse][_baseIds[i]] * _amounts[i]);
