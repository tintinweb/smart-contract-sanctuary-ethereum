...
contract PublicGuard is Enum, Guard {
    ...
    mapping(address => mapping(address => bool)) whitelist; //白名单, 仅能白名单内的地址继续执行.
    ...
    event MasterCopyChange(address newMasterCopy);
    event whitelistUpdate(address safe, address[] to, bool on);
    event OnceTransferNumberLimit(address safe, address token, uint256 number);
    event DayTransferFrequencyLimit(address safe, address token, uint256 frequency);
    event DayTransferNumberLimit(address safe, address token, uint256 number);
    event Pass(address safe, address to, uint256 amount, bytes data);
    ...
    function setWhitelist(address safe, address[] memory to, bool on) public OnlyWhallet(safe) {
        ...
        emit whitelistUpdate(safe, to, on);
    } 
    //设置每笔转账Token限额.
    function setTokenTransferNumberLimit(address safe, address token, uint256 number) public OnlyWhallet(safe) {
        ...
        emit OnceTransferNumberLimit(safe, token, number);
    }
    //设置Token每天允许转账次数.
    function setEveryDayTransferFrequency(address safe, address token, uint8 frequency) public OnlyWhallet(safe) {
        ...
        emit DayTransferFrequencyLimit(safe, token, frequency);
    }
    //设置Token每天允许转账额度.
    function setEveryDayTransferNumber(address safe, address token, uint256 number) public OnlyWhallet(safe) {
        //正常来说, 每天允许(最大)转账额度 = 每天允许转账次数 * 每笔转账限额.
        //为防止设置无意义额度, 此处额外做了限制.
        require(number <= tokenEveryDayTransferLimit[safe][token].frequency * tokenEveryTimeTransferNumberLimit[safe][token], "Invalid");
        tokenEveryDayTransferLimit[safe][token].number = number;
        emit DayTransferNumberLimit(safe, token, number);
    }
    //更改masterCopy合约.
    function setMasterCopy(address newMasterCopy) public {
        require(msg.sender == Creater);
        masterCopy = newMasterCopy;
        emit MasterCopyChange(newMasterCopy);
    }
    //查询是否为白名单成员.
    ...
    function checkTransaction(
        ...
        if(data.length > 0) {
            ...
            if(magic == bytes4(keccak256("transfer(address,uint256)"))) {
                ...
                //获取转移代币数.
                address _to;
                (_to, amount_) = abi.decode(meta, (address, uint256));
                //ERC20 转账目标地址需要在白名单中.
                require(whitelist[msg.sender][_to] == true, "ER102");
                transfer_ = true;
            } 
        }
        address to_ = to;
        ...
        emit Pass(msg.sender, to, value, data);
    }
