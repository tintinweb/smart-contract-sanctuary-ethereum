...
interface IRareStake is IERC20Upgradeable {
  /*//////////////////////////////////////////////////////////////////////////
                              Events
  //////////////////////////////////////////////////////////////////////////*/
  ...
  /// @notice Error emitted via {stake} if synthetic RARE mint is zero.
  error NoSyntheticRareForPricePrice();
...
contract RareStake is IRareStake, ERC20SnapshotUpgradeable, ReentrancyGuard {
  ...
  mapping(uint256 => uint256) private roundSnapshotId;
  ...
  modifier claimWindowActive() {
    if (_claimWindowActive()) {
      revert IRareStake.ClaimNotActive();
    }
    _;
  }
  ...
  modifier updatePeriod() {
    uint256 currentRound = block.timestamp / periodLength - roundOffset;
    if (currentRound > lastRound) {
      lastRound = currentRound;
      _snapshot();
      roundSnapshotId[lastRound] = _getCurrentSnapshotId();
    }
    _;
  }
  ...
  function initialize(
    ...
    roundOffset = periodStart / periodLength;
    lastRound = 0;
  }
  ...
  function stake(uint256 _amount) external nonReentrant updatePeriod {
    ...
    // if (amountSRare == 0) revert IRareStake.NoSyntheticRareForPricePrice();
  ...
  function unstake(uint256 _amount) external nonReentrant updatePeriod {
    ...
    uint256 burnAmount = (amountRareReturned * deflationaryPercentage) / 10_000;
    ...
    stakingRegistry.decreaseAmountStaked(msg.sender, userStakedTo, amountRareReturned);
  ...
  function claimReward(address _user) external nonReentrant updatePeriod claimWindowActive {
    uint256 claimRound = getClaimRound();
    if (stakerClaimedRound[claimRound][_user]) revert IRareStake.RewardAlreadyClaimed();

    uint256 totalSRareSupply = totalSupplyAt(roundSnapshotId[claimRound]);
    uint256 senderBalance = balanceOfAt(_user, roundSnapshotId[claimRound]);

    uint256 percentOfTotal = (senderBalance * 1_000) / totalSRareSupply;

    uint256 amountDue = (percentOfTotal * getTotalRewards()) / 1_000;

    rare.transfer(_user, amountDue);

    stakerClaimedRound[claimRound][_user] = true;

    emit RewardClaimed(msg.sender, _user, address(rare), amountDue);
  }
  ...
  function getTotalRewards() public view returns (uint256) {
    return rare.balanceOf(address(this)) - stakingRegistry.getTotalAmountStakedOnUser(userStakedTo);
  }
  ...
  function getRewardsForUser(address _userAddress) external view returns (uint256) {
    uint256 claimRound = getClaimRound();
    uint256 totalSRareSupply = claimRound == lastRound && roundSnapshotId[claimRound] != 0
      ? totalSupplyAt(roundSnapshotId[claimRound])
      : totalSupply();
    uint256 senderBalance = claimRound == lastRound && roundSnapshotId[claimRound] != 0
      ? balanceOfAt(_userAddress, roundSnapshotId[claimRound])
      : balanceOf(_userAddress);
    if (totalSRareSupply == 0) {
      return 0;
    }
    uint256 percentOfTotal = (senderBalance * 1_000) / totalSRareSupply;
    uint256 latestTotalRewards = getTotalRewards();
    return (percentOfTotal * latestTotalRewards) / 1_000;
  }

  /// @notice Calculate the current round
  /// @return uint256 Current round
  function getClaimRound() public view returns (uint256) {
    return block.timestamp / periodLength - roundOffset;
  }
  ...
  /// @param _stakedAmount Amount of RARE being staked.
  /// @return uint256 Amount of synthetic tokens one would get for staking {_stakedAmount} given a totalSupply of {_totalSRare}.
  /// @dev Calculated based on a sqrt token bonding curve. Curve follows that the price of 100 Synthetic
  function calculatePurchaseReturn(uint256 _totalSRare, uint256 _stakedAmount) public pure returns (uint256) {
    uint256 totalSRare = _totalSRare / 1e19;
    uint256 stakedAmount = _stakedAmount / 1e10;
    return (Math.sqrt(((2 * stakedAmount) + totalSRare**2)) - totalSRare) * Math.sqrt(stakedAmount) * 1e11;
  }
  ...
  function _claimWindowActive() internal view returns (bool) {
    uint256 secondsUntilRoundOver = block.timestamp % periodLength;
    return claimWindow < secondsUntilRoundOver;
  }
