...
/// @notice Throw when attempt to use an invalid address
error ICO__InvalidAddress();
...
contract ICO {
    ...
    // SpaceCoin treasury address
    address public immutable treasury;

    // token contract
    SpaceCoin public immutable spaceCoin;

    // allowlist of private investors that can participate on the Seed phase
    ...
    constructor(address _owner, address _treasury) {
        ...
        if (_isAddressZero(_owner) || _isAddressZero(_treasury)) {
            revert ICO__InvalidAddress();
        }
     ...
     */
    function togglePrivateInvestor(
        address _investor,
        bool _state
    ) external onlyOwner {
    ...
    function _validateLimits(address _investor, uint _newAmount) private view {
        ...
        uint _maxTotalLimit = phase == Phase.Seed
            ? SEED_MAX_TOTAL_LIMIT
            : MAX_TOTAL_LIMIT;
        ...
        uint _maxIndividualLimit = phase == Phase.Seed
            ? SEED_MAX_INDIVIDUAL_LIMIT
            : MAX_INDIVIDUAL_LIMIT;
