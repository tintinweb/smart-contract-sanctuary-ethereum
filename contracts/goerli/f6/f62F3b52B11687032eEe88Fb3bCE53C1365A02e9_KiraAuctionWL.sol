...
contract KiraAuctionWL is Ownable {
    ...
    function newBidSingle(bytes calldata signature) payable external notPaused state1 {
    ...
    function newBidMultiple(uint8 amount, uint256[] calldata values, bytes calldata signature) payable external notPaused state1 {
        require(_validateData(msg.sender, signature), "Invalid signature");
        address sender = msg.sender;
        uint8 bidsAmount = uint8(bids[sender].length);
