...
contract Staker {
  ...
  modifier notCompleted() {
    if (exampleExternalContract.completed()) revert Staker__AlredyCompleted();
    _;
  }
  ...
  function stake() public payable {
    if (block.timestamp >= deadline) revert Staker__DeadlinePassed();
  ...
  function execute() public notCompleted {
    if (executed) revert Staker__AlreadyExecuted();
    if (block.timestamp < deadline) revert Staker__DeadlineNotPassed();
  ...
  function withdraw() public notCompleted {
    ...
    if (toWithdraw == 0) revert Staker__NothingToWithdraw();
    balances[msg.sender] = 0;
    (bool success, ) = withdrawer.call{value: toWithdraw}('');
    if (!success) revert Staker__WithdrawFailed();
  }
