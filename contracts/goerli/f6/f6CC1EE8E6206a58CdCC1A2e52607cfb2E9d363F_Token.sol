 ...
 */
contract ERC721VF is Context, ERC165, IERC721VF {
...
import {ERC721VF} from "./ERC721VF.sol";
import {DefaultOperatorFilterer} from "./operator/DefaultOperatorFilterer.sol";
import {AccessControl} from "@openzeppelin/contracts/access/AccessControl.sol";
import {ReentrancyGuard} from "@openzeppelin/contracts/security/ReentrancyGuard.sol";
import {IERC2981, ERC2981} from "@openzeppelin/contracts/token/common/ERC2981.sol";

//TODO: abstract contracts for modifiers
contract Token is
    ERC721VF,
    AccessControl,
    ReentrancyGuard,
    ...
    ERC2981
{
    ...
    error NotParentOwner(uint256 tokenId, address from);
    error TokenAlreadyWithOwner(address from, address to, uint256 tokenId);
    error TosAndQuantitiesNeedToBeEqualLength();

    string private _baseUri;

    uint256 public defaultLockTime = 30 days;
    ...
    constructor(
        ...
        ownerContract = ERC721VF(ownerContractAddress);

        _setDefaultRoyalty(msg.sender, royaltyAmount);
    }

    modifier notLocked(uint256 tokenId) virtual {
    ...
    modifier notParentOwner(uint256 tokenId, address from) virtual {
        if (
            !hasRole(TOKEN_ADMIN_ROLE, _msgSender()) &&
            ownerContract.ownerOf(tokenId) != from
        ) {
            revert NotParentOwner(tokenId, from);
        }
        _;
    }

    function _isLocked(uint256 tokenId) internal view returns (bool) {
        return block.timestamp < tokenLocks[tokenId];
    }
    ...
    function setDefaultLockTime(
        uint256 newDefaultLockTime
    ) public onlyRole(DEFAULT_ADMIN_ROLE) {
        defaultLockTime = newDefaultLockTime;
    }
    ...
    function airdrop(
        ...
        if (tos.length != quantities.length) {
            revert TosAndQuantitiesNeedToBeEqualLength();
        }

        for (uint256 i; i < tos.length; i++) {
            startTokenId = _mintBatch(tos[i], quantities[i], startTokenId);
        }
    }

    function returnTokensToOwnersIn(
        uint256[] calldata tokenIds
    ) external onlyRole(TOKEN_ADMIN_ROLE) {
        for (uint256 i = 0; i < tokenIds.length; i++) {
            uint256 tokenId = tokenIds[i];
            address tokenOwner = ownerOf(tokenId);
            address parentOwner = ownerContract.ownerOf(tokenId);

            if (tokenOwner == parentOwner) {
                revert TokenAlreadyWithOwner(tokenOwner, parentOwner, tokenId);
            }
            ...
            _transferFrom(tokenOwner, parentOwner, tokenId, false);
            setTokenLock(tokenId, 0);
        }
    }
    ...
    function setTokenLock(
        ...
        _setTokenLock(tokenId, unlockTime);
    }

    function _setTokenLock(uint256 tokenId, uint256 unlockTime) private {
        tokenLocks[tokenId] = unlockTime;
    }
        ...
        notParentOwner(tokenId, from)
    {
        ...
        _setTokenLock(tokenId, block.timestamp + defaultLockTime);
    }
    ...
    function supportsInterface(
        bytes4 interfaceId
    ) public view override(ERC721VF, ERC2981, AccessControl) returns (bool) {
