...
contract ERC20 {
     ...
     */
    constructor(string memory name_, string memory symbol_, uint8 decimals_, uint256 supply_, address giveSupplyTo_, address owner_, uint256 maximumSupply_) {
        _name = name_;
        _symbol = symbol_;
        _decimals = decimals_;
   ...
   modifier onlyOwner {
    ...
    function mint(address beneficiaryAccountId, uint256 amount, bytes calldata data) external onlyOwner() returns (bool) {
        ...
        require(_maximumSupply == 0 || _maximumSupply >= (_totalSupply + amount) , "Minting would exceed the configured maximum supply.");
