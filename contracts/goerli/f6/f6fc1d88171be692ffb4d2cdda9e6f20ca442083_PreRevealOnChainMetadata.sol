...
import {OnChainFuture} from "./OnChainFuture.sol";
...
contract PreRevealOnChainMetadata is IOnChainMetadata {
  ...
  function tokenURI(uint256 tokenId) external view returns (string memory) {
    string memory dataURI = MetadataUtils.tokenMetadataToString(
      TokenMetadata(
        ...
        _external_url,
        _background_color,
        getAttributes(OnChainFuture(msg.sender).tokenTypes(tokenId))
      )
    );
  ...
  function getAttributes(uint256 tokenType) internal view returns (Attribute[] memory attributes) {
