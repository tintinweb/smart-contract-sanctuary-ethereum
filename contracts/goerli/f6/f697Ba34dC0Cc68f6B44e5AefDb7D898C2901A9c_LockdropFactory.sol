...
contract AtlendisLockdrop is BaseLockdrop {
    ...
    function _lock(
        ...
        uint256 rewards = _getRewardsAmount(
            baseAllocation,
            lockingDuration,
            tokenId
        );
        require(
            ERC20(token).balanceOf(address(this)) > (pendingRewards + rewards),
            "Not enough rewards left to distribute"
        );
        pendingRewards += rewards;
        ERC721(nft).transferFrom(msg.sender, address(this), tokenId);
        currentLocks[tokenId] = LockDrop({
            owner: msg.sender,
            rewards: rewards,
            endDate: block.timestamp + lockingDuration
        });
        emit Lock(
            tokenId,
            lockingDuration,
    ...
    function _getRewardsAmount(
        ...
        uint256 lockingDuration,
        uint256 tokenId
    ) internal view override returns (uint256 rewardsAmount) {
        (
            uint128 bondsQuantity,
            uint128 normalizedDepositedAmount
        ) = IPositionManager(nft).getPositionRepartition(uint128(tokenId));
        ...
        return
            ((baseAmount +
                ((uint256(bondsQuantity + normalizedDepositedAmount)) *
                    rate *
                    lockingDuration)) * multiplier) / baseMultiplier;
    }
    ...
    {
        // tokenId can be used to compute multiplier depending on position characteristics
 ...
 * for a given time with a corresponding amount of Atlendis tokens
 **/
abstract contract BaseLockdrop {
    ////////////
    // EVENTS //
    ////////////
    ...
    event Quit(uint256 tokenId, uint256 lockEnd, address from, uint256 rewards);
    event Claim(
        uint256 tokenId,
        uint256 lockEnd,
        address from,
    ...
    // contract parameters set at deployment time
    address public immutable nft;
    address public immutable token;
    bytes32 public immutable root; // merkle tree root used for base token allocations
    uint256 public immutable rate; // base tokens per second of locking
    ...
    // contract state
    uint256 public pendingRewards; // rewards attributed to currently locked positions
    struct LockDrop {
        address owner;
        uint256 rewards;
        uint256 endDate;
     ...
     **/
    function lock(uint256 tokenId, uint256 lockingDuration) external {
        require(
    ...
    function quit(uint256 tokenId) external {
        _verifyLockOwnerShip(tokenId);
        require(
            block.timestamp < currentLocks[tokenId].endDate,
            "Quit too late"
        );
        (uint256 endDate, uint256 rewards) = _withdraw(tokenId);
        emit Quit(tokenId, endDate, msg.sender, rewards);
    }
    ...
    function claim(uint256 tokenId) external {
        ...
        emit Claim(tokenId, endDate, msg.sender, rewards);
    }

    /**
     * @notice Internal position lock ownership verification logic
     **/
    function _verifyLockOwnerShip(uint256 tokenId) internal view {
        require(
    ...
    /**
     * @notice Internal position withdrawing and rewards claiming logic
     **/
    function _withdraw(uint256 tokenId)
        internal
        returns (uint256 endDate, uint256 rewards)
    {
        pendingRewards -= currentLocks[tokenId].rewards;
        endDate = currentLocks[tokenId].endDate;
        rewards = currentLocks[tokenId].rewards;
        delete currentLocks[tokenId]; // a position can be locked several times
...
import "./AtlendisLockdrop.sol";
import "../token/AtlendisToken.sol";

/**
 * @title LockdropFactory
 * @notice Deploys and configure new lockdrops
 **/
contract LockdropFactory {
    ////////////
    // EVENTS //
    ////////////
    ...
    AtlendisToken immutable public token;
    address immutable public  poolsController;
    address immutable public positionManager;
    ...
    /**
     * @notice Deploys a new lockdrop contract
     * Grants the priviledged transfer role to allow for lock claims to work even when transfers are paused
     * Mints new tokens to be used by the lockdrop contract
     **/
    function deployNewLockdrop(
        uint256 amount,
        bytes32 poolHash,
        bytes32 root,
        uint256 rate,
        ...
        address deployedLockdrop = address(
            new AtlendisLockdrop(
                poolsController,
                positionManager,
                address(token),
                poolHash,
        ...
        token.mint(deployedLockdrop, amount);

        // allow vesting wallet to transfer tokens otherwise claim would fail
        token.grantRole(token.PRIVILEDGED_TRANSFER_ROLE(), deployedLockdrop);
