...
interface YeanV2VaultAPI
{
	function deposit(uint256 amount) external returns (uint256);

	function withdraw(uint256 maxShares, address receiver) external returns (uint256);
...
contract WrappedERC4626YearnV2Vault is ERC4626
{
	YeanV2VaultAPI public vault;

	constructor (IERC20Metadata asset_, string memory name, string memory symbol, YeanV2VaultAPI _vault) ERC4626(asset_) ERC20(name, symbol)
	{
		vault = _vault;
	...
	{
		_burn(owner, shares);
		vault.withdraw(shares, receiver);
