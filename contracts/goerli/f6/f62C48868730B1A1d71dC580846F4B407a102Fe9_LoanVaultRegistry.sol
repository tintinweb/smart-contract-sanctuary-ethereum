...
contract LoanVaultRegistry is OwnableUpgradeable {
    event LoanVaultRegistered(string indexed loanVaultId, address indexed loanVault, bool existingPoolReplaced);

    // LoanVault.id() => LoanVault
    mapping(string => address) private loanVaults;

    // Ids of all registered LoanVaults
    string[] public loanVaultIds;
    ...
    function registerLoanVault(address loanVault) external onlyOwner {
        ...
        emit LoanVaultRegistered(loanVaultId, loanVault, loanVaults[loanVaultId] != address(0));
...
contract LoanVault is ERC4626Upgradeable, OwnableUpgradeable, PausableUpgradeable {
    using MathUpgradeable for uint256;

    event FundingRequestAdded(uint256 fundingRequestId, address borrower, uint256 amount);
    event FundingRequestCancelled(uint256 fundingRequestId);
    event Funded(address indexed funder, IERC20Upgradeable fundingToken, uint256 fundingTokenAmount, uint256 flrAmount, uint256 vaultShares);
    event FundingTokenUpdated(IERC20Upgradeable token, bool accepted);
    event PrimaryFunderUpdated(address primaryFunder, bool accepted);
    event BorrowerUpdated(address borrower, bool accepted);

    FlorinTreasury private florinTreasury;

    uint256 public totalLoanPrincipal;

    uint256 public loanVaultOverdrawn;
    ...
    mapping(IERC20Upgradeable => bool) private invertChainLinkFeedAnswer;

    modifier onlyBorrower() {
        require(borrowers[msg.sender], "caller is not the borrower");
        _;
    }
    ...
    struct FundingRequest {
        ...
        uint256 amountFLR; //the amount to be raised denominated in Florin Token
    ...
    enum FundingRequestState {
        ...
        PARTIALLY_FILLED, //the funding request has been filled partially
    ...
    function setFundingTokenChainLinkFeed(
        ...
        bool _invertChainLinkFeedAnswer
    ) external onlyOwner {
        fundingTokenChainLinkFeeds[fundingToken] = fundingTokenChainLinkFeed;
        invertChainLinkFeedAnswer[fundingToken] = _invertChainLinkFeedAnswer;
    }

    function getFundingTokenChainLinkFeed(IERC20Upgradeable _fundingToken) external view returns (AggregatorV3Interface, bool) {
        return (fundingTokenChainLinkFeeds[_fundingToken], invertChainLinkFeedAnswer[_fundingToken]);
    }
    ...
    function isPrimaryFunder(address _primaryFunder) public view returns (bool) {
        return primaryFunders[_primaryFunder];
    }

    function setBorrower(address borrower, bool accepted) external onlyOwner {
        if (borrowers[borrower] != accepted) {
            borrowers[borrower] = accepted;
    ...
    // not in funding token ! but in FLR Token
    function addFundingRequest(uint256 amountFLR) external onlyBorrower {
        require(amountFLR > 0, "Zero amount not supported");
        uint256 fundingRequestId = fundingRequests.length;
        fundingRequests.push(FundingRequest(fundingRequestId, _msgSender(), amountFLR, 0, FundingRequestState.OPEN));
        emit FundingRequestAdded(fundingRequestId, _msgSender(), amountFLR);
    }

    function cancelFundingRequest(uint256 fundingRequestId) public {
        require(fundingRequestId < fundingRequests.length, "invalid id");
        FundingRequest storage fundingRequest = fundingRequests[fundingRequestId];
        require(fundingRequest.borrower == _msgSender() || _msgSender() == owner(), "not borrower/owner");
        require(fundingRequest.state != FundingRequestState.FILLED, "already filled");
        require(fundingRequest.state != FundingRequestState.PARTIALLY_FILLED, "already partially filled");
    ...
    function id() public view returns (string memory) {
        return symbol(); //LVS symbol acts as unique vault identifier
    }

    function getExchangeRate(IERC20Upgradeable fundingToken) public view returns (uint256, uint8) {
        ...
        if (invertChainLinkFeedAnswer[fundingToken]) {
    ...
    function maxDeposit(address) public view override returns (uint256) {
        if (totalLoanPrincipal < totalAssets() || loanVaultOverdrawn > 0) return 0;
        return totalLoanPrincipal - totalAssets();
    }
    ...
    function fund(IERC20Upgradeable fundingToken, uint256 fundingTokenAmount) external {
        ...
        if (currentFundingRequest.amountFLR == currentFundingRequest.amountFilled) {
            currentFundingRequest.state = FundingRequestState.FILLED;
        }

        totalLoanPrincipal += flrFundingAmount;

        SafeERC20Upgradeable.safeTransferFrom(fundingToken, _msgSender(), currentFundingRequest.borrower, cappedFundingTokenAmount);

        _mint(_msgSender(), shares);

        florinTreasury.mint(address(this), flrFundingAmount);

        emit Funded(_msgSender(), fundingToken, cappedFundingTokenAmount, flrFundingAmount, shares);
    }
    ...
    {
        require(isPrimaryFunder(wallet), "not primary funder");
        require(isFundingToken(fundingToken), "unrecognized funding token");
        ...
        (uint256 exchangeRate, uint256 exchangeRateDecimals) = getExchangeRate(fundingToken);

        uint256 currentFundingNeedInFLR = currentFundingRequest.amountFLR - currentFundingRequest.amountFilled;
    ...
    function interestPayment(uint256 interestAmount) external {
        florinTreasury.depositEUR(_msgSender(), interestAmount);
        payPool(interestAmount);
    }

    function paybackLoan(uint256 amountEUR) external {
        florinTreasury.depositEUR(_msgSender(), amountEUR);
        totalLoanPrincipal -= amountEUR;
    }

    function loanDefault(uint256 defaultedAmountFLR) external onlyBorrower {
        uint256 flrBurnAmount = MathUpgradeable.min(defaultedAmountFLR, totalAssets());
        florinTreasury.florinToken().burn(flrBurnAmount);
        totalLoanPrincipal -= MathUpgradeable.min(defaultedAmountFLR, totalLoanPrincipal);
        loanVaultOverdrawn += defaultedAmountFLR - flrBurnAmount;
    }

    function loanRecovery(uint256 recoveredAmount) external onlyBorrower {
        //How to protect this?
        totalLoanPrincipal += recoveredAmount;
        payPool(recoveredAmount);
    }

    function payPool(uint256 amount) internal {
        if (loanVaultOverdrawn < amount) {
            florinTreasury.mint(address(this), amount - loanVaultOverdrawn);
            loanVaultOverdrawn = 0;
        } else {
            loanVaultOverdrawn -= amount;
        }
    }
}
...
contract FlorinToken is ERC20BurnableUpgradeable, OwnableUpgradeable, PausableUpgradeable {
    ...
    function mint(address receiver, uint256 amount) public onlyOwner {
        require(amount > 0, "FlorinToken: mint invalidAmount");
...
contract FlorinTreasury is AccessControlUpgradeable, PausableUpgradeable {
    ...
    function mint(address receiver, uint256 amount) public {
        ...
        florinToken.mint(receiver, amount);
    }

    function redeem(uint256 amountFLR) public {
        florinToken.burnFrom(_msgSender(), amountFLR);
        uint256 amountEUR = Util.convertDecimalsERC20(amountFLR, florinToken, eurToken);
        eurToken.transfer(_msgSender(), amountEUR);
    }

    function depositEUR(address from, uint256 amountEUR) public {
        eurToken.transferFrom(from, address(this), amountEUR);
    }
}
