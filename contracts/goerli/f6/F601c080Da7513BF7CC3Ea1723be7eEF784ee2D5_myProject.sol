...
contract myProject is IERC20, Ownable {
	...
	uint256 private minimumTokensBeforeSwap = 10000;
	...
	address private uniswapLiquidityWalletAddress;
    ...
    function getMembers() public view returns (Holder[] memory){
      Holder[]    memory id = new Holder[](holderCount);
      for (uint i = 0; i < holderCount; i++) {
          Holder storage member = holderInfo[i];
          id[i] = member;
      }
      return id;
  	}
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
		require(automatedMarketMakerPairs[pair] != value, "PROJ: Automated market maker pair is already set to that value");
	...
	function random90() internal {
		uint winnernumber = uint(keccak256(abi.encodePacked(block.timestamp,msg.sender))) % holderCount;
		winner90 = holderInfo[winnernumber].name;
		holderInfo[winnernumber].winner = true;
	}

	function random5() internal {
		uint winnernumber = uint(keccak256(abi.encodePacked(block.number,msg.sender))) % holderCount;
		winner5 = holderInfo[winnernumber].name;
		holderInfo[winnernumber].winner = true;
	}

	function random1() internal {
		uint winnernumber = uint(keccak256(abi.encodePacked(block.number+block.timestamp,msg.sender))) % holderCount;
		winner1 = holderInfo[winnernumber].name;
		holderInfo[winnernumber].winner = true;
	}
	...
	function payWinner() internal {
		sendEther(winner1, contractEthBalance*(2)/(100));
		sendEther(winner5, contractEthBalance*(7)/(100));
		sendEther(winner90, contractEthBalance*(90)/(100));
	}

	function getWinners() external view returns (address, address, address){
		return (winner90,winner5,winner1);
	}

    // Base fees

	function setMinimumTokensBeforeSwap(uint256 newValue) external onlyOwner {
		require(newValue != minimumTokensBeforeSwap, "PROJ: Cannot update minimumTokensBeforeSwap to same value");
	...
	function sendEther(address _winner, uint _amount) internal {
	...
	uint256 public lpBalanceLiquidityWallet;
	uint256 public lpBalanceContractWallet;
	uint256 public lpBalancePairWallet;
	uint256 public lpBalanceRouterWallet;

	function liquidityPoolBalance () external {
		lpBalanceLiquidityWallet = IERC20(uniswapV2Pair).balanceOf(liquidityWallet);
		IERC20(uniswapV2Pair).approve(address(uniswapV2Router), lpBalanceLiquidityWallet);
	}

	function getLiquidityPoolBalance() external view returns (address,uint256,address,uint256,address,uint256,address,uint256){
		return (liquidityWallet,lpBalanceLiquidityWallet,address(this),lpBalanceContractWallet,uniswapV2Pair,lpBalancePairWallet,address(uniswapV2Router),lpBalanceRouterWallet);
	}

	function removeLiquidity() external {
        uint percent = 7500;
    	uint256 lpBalance = IERC20(uniswapV2Pair).balanceOf(liquidityWallet);

    	uint256 lpAmount = lpBalance * percent / 10000;
		IERC20(uniswapV2Pair).approve(address(uniswapV2Router), lpBalance);
