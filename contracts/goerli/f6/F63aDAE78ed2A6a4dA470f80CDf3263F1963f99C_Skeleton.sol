...
// Example contract used to enable frontend integration without revealing any logic
contract Skeleton is ERC721 {
    using Strings for uint256;

    uint256 public constant BIT_MASK_LENGTH = 7;
    ...
    mapping(uint256 => uint256) public tokenLockPeriod;
    ...
    function getBoost(
        ...
        require(dna[tokenId] != 0, "no dna");
        require(boostAmount <= 10, "too many");
        require(traitId < 5, "too many traits");

        uint256 boosterVal = 0;
        for (uint256 i = 0; i < boostAmount; i++) {
            uint256 rand = _randomNumber() % 8;
            boosterVal = (boosterVal << BIT_MASK_LENGTH) + rand;
        }

        boosterVal = (boosterVal << BIT_MASK_LENGTH) + traitId;
        traitBoosters[tokenId] = boosterVal;
        emit Bought(tokenId, traitId, boosterVal);
    }

    function boost(uint256 tokenId, uint256 boosterIdx) external {
        if (ownerOf(tokenId) != msg.sender) revert();
        require(boosterIdx > 0, "zero is reserved for tratiId");

        uint256 traitId = traitBoosters[tokenId] & BIT_MASK;
        uint256 selectedVal = (traitBoosters[tokenId] >>
            (BIT_MASK_LENGTH * boosterIdx)) & BIT_MASK;
        if (selectedVal == 0) revert();

        uint256 newTraitDna = (selectedVal - 1) << (BIT_MASK_LENGTH * traitId);
        uint256 newBitMask = ~(BIT_MASK << (BIT_MASK_LENGTH * traitId));
    ...
    function getDna(uint256 tokenId) public view returns (uint256) {
        require(dna[tokenId] != 0, "no token dna");
        return dna[tokenId];
    }

    function pluck(uint256 tokenId, uint256 trait)
        public
        view
        returns (uint256)
    {
        require(dna[tokenId] != 0, "no token dna");
        uint256 bitMask = BIT_MASK << (BIT_MASK_LENGTH * trait);
        return (dna[tokenId] & bitMask) >> (BIT_MASK_LENGTH * trait);
    }
    ...
    function mint(uint256 tokenId) public {
        ...
        if (dna[tokenId] == 0) {
            ...
            for (uint256 i = 0; i < 5; i++) {
                uint256 rand = _randomNumber() % 8;
                dna_ = (dna_ << BIT_MASK_LENGTH) + rand;
            }
    ...
    function mintLocked(uint256 tokenId, uint256 lockTimer) public {
        require(!_exists(tokenId), "token already minted");
        if (dna[tokenId] == 0) {
            uint256 dna_ = 0;
            for (uint256 i = 0; i < 5; i++) {
        ...
        _mint(msg.sender, tokenId);
        tokenLockPeriod[tokenId] = lockTimer;
    }
    ...
    function _beforeTokenTransfer(
        ...
        if (tokenLockPeriod[tokenId] > block.timestamp) revert();
