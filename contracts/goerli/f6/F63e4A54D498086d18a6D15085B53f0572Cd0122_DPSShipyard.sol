...
contract DPSShipyard is Ownable, ReentrancyGuard {
     ...
     *                needs to corespond with the index of the part from flagship's _parts[]
     */
    function upgradeFlagship(uint256 _flagshipId, uint8[6] calldata _levels) external nonReentrant {
    ...
    function setContract(address _contract, uint256 _target) external onlyOwner {
        if (_target == 1) {
