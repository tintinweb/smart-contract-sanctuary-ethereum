...
contract Lottery is Administrated, RandomizerClient {
    ...
    address payable private robot;
    uint256 public robotThreshold;

    constructor(address payable _robot, address _randomizer)
    ...
    function setToken(address _token) external onlyAdmin {
        token = _token;
        entryPrice = _calculateEntryPrice();
    }
    ...
    function setRobotThreshold(uint256 threshold) external onlyAdmin {
        robotThreshold = threshold;
    }

    function _calculateEntryPrice() internal view returns (uint256) {
        ...
        override
    {
        ...
        if (robot.balance < robotThreshold && winningAmount > robotThreshold - robot.balance) {
            uint delta = robotThreshold - robot.balance;
            Address.sendValue(robot, delta);
            winningAmount -= delta;
        }
