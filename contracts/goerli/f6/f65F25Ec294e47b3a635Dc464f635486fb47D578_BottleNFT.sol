...
contract ERC721Common is Context, ERC721Pausable, DefaultOperatorFilterer, OwnerPausable {
...
abstract contract BottleSeller is OwnerPausable, ReentrancyGuard {
    ...
    whenNotPaused
    {
        ...
        require(sellerConfig.reserveFreeQuota, "BottleSeller: reserveFreeQuota is false.");

        uint256 remain = config.airdropQuota - airdrop.current();
        n = Math.min(n, remain);
        require(n > 0, "BottleSeller: airdrop quota exceeded");

        n = Math.min(n, config.totalInventory - _totalSold.current());
        require(n > 0, "BottleSeller: Sold out");
    ...
    whenNotPaused
    {
        ...
        require(remain > 0, "BottleSeller: reserver quota exceeded");
...
contract BottleNFT is ERC721Common, BottleSeller {
    using Strings for uint256;
        ...
        )
    {
        operateConfig = OperateConfig({
            whiteListSaleBegin: 1668744000,
            whiteListSaleEnd: 1669780800,
            merkleRoot: '',
            publicSaleBegin: 1701388800
        });
    }

    function airdrop(address to, uint256 requested) external onlyOwner {
        BottleSeller._airdrop(to, requested);
    }

    function publicBuy(uint256 requested) external {
        require(block.timestamp >= operateConfig.publicSaleBegin, "BottleNFT: Public sale not start!");

        BottleSeller._purchase(msg.sender, requested);
    }
    ...
    function whitelistBuy(uint256 requested, bytes32[] calldata signature) external {
        require(block.timestamp >= operateConfig.whiteListSaleBegin
                ...
                "BottleNFT: White list sale not start!");
    ...
    function hadBought(address addr) external view returns(bool) {
        return BottleSeller._hadBought(addr);
    }
}
