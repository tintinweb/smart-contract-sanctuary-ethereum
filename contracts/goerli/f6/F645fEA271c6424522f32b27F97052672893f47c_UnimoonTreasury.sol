...
contract UnimoonTreasury is AccessControl, ReentrancyGuardUpgradeable {
    using EnumerableSet for EnumerableSet.AddressSet;

    bytes32 public constant BACKEND_ROLE = keccak256("BACKEND_ROLE");
    uint8 public constant DENOMINATOR = 100;

    address public UNIMOON_TOKEN;
    address public USDC;
     ...
     * @param _profitDistribution profit distribution configuration
     * @param _feeDistribution an array of fee distributoin configurations
     * @param _addresses [0] - owner, [1] - backend, [2] - unimoon token, [3] - usdc, [4] - router, [5] - staking
     */
    function initialize(
        ProfitDistribution memory _profitDistribution,
     ...
     */
    function getAllAltcoinPurchases(
        address altcoin
        ...
        returns (uint256)
    {
        ...
        if (amounts[1] > oldValue) {
            ...
            if (
                alloc > 0 &&
                weight > 0 &&
                profitDistribution.stakingProfitPerc > 0
            ) {
                TransferHelper.safeTransfer(
                    USDC,
                    address(STAKING),
                    (profit * profitDistribution.stakingProfitPerc) /
                        DENOMINATOR
                );
            ...
            if (profitDistribution.toBurn > 0) {
                TransferHelper.safeApprove(
                    USDC,
                    address(ROUTER),
                    (profit * profitDistribution.toBurn) / DENOMINATOR
                );
                address[] memory _path = new address[](2);
                _path[0] = USDC;
                _path[1] = UNIMOON_TOKEN;
                ...
                IToken(UNIMOON_TOKEN).burn(_amounts[1]);
            }
        }
     ...
     */
    function changeFeeDistribution(
        FeeDistribution[2] memory _feeDistribution
    ...
    /** @dev Function to change staking and toBurn profit distribution percents
     * @notice available for owner only
     * @param _profitDistribution new profit distribution percents
     */
    function changeProfitDistribution(
        ProfitDistribution memory _profitDistribution
    ) external onlyRole(DEFAULT_ADMIN_ROLE) {
        require(
            _profitDistribution.stakingProfitPerc +
                _profitDistribution.toBurn <=
                DENOMINATOR,
