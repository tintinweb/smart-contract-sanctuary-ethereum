...
contract HashedTimelockERC20 {
    ...
    * @param _preimage string(_preimage) should equal the contract hashlock.
    * @return bool true on success
     */
    function withdraw(bytes32 _contractId, string memory _preimage)
        external
        contractExists(_contractId)
        hashlockMatches(_contractId, keccak256(abi.encodePacked(_preimage)) )
        withdrawable(_contractId)
        returns (bool)
    {
        LockContract storage c = contracts[_contractId];
        c.preimage = keccak256(abi.encodePacked(_preimage));
        c.withdrawn = true;
