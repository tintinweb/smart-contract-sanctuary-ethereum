    ...
    ERC2981Upgradeable
{
    ...
    address public designatedSigner; // signer used for signature verification of the mint
    ...
    event ToggleTraitRegistration(address traitContract, bool isRegistered);
    ...
    function linkTrait(
        ...
        require(linkedTraits[avatarId][traitContract] == 0, "Trait already linked");
    ...
    function unlinkTrait(
        ...
        require(linkedTraits[avatarId][traitContract] == traitId, "Trait not linked");
    ...
    function mint(
        ...
        isValidSignature(signature, designatedSigner, to);
    ...
    // Owner Functions
    /// @notice Change the designated signer
    /// @dev Used in case the signer is compromised
    /// @param _signer The new signer
    function changeDesignatedSigner(address _signer) external onlyOwner {
    ...
    /// @notice Register/Un-register a trait
    /// @dev Should be used to register/un-register a trait
    /// @param traitContract Address of the trait contract
    function toggleTraitRegistration(address traitContract) external onlyOwner {
        registeredTraits[traitContract] = !registeredTraits[traitContract];
        emit ToggleTraitRegistration(traitContract, registeredTraits[traitContract]);
    }
...
contract PlutoSigner is EIP712Upgradeable {
    string private constant SIGNING_DOMAIN = "PlutoSigner";
        ...
        return
            _hashTypedDataV4(
                keccak256(
                    abi.encode(
                        keccak256(
                            "Signature(uint32 nonce,address userAddress)"
                        ),
                        signature.nonce,
    ...
    function isValidSignature(
        ...
        if (signature.userAddress != to) {
            revert InvalidSigner();
        }

        if (signature.nonce + 5 minutes < block.timestamp) {
            revert NonceExpired();
        }
