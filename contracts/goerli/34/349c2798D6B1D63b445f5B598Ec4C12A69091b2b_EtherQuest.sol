...
contract TestToken is ERC20, ERC20Burnable {
    constructor() ERC20("EtherQuest Experience", "EXP") {}
...
contract EtherQuest is ERC721Enumerable {
...
constructor() ERC721("EtherQuest", "ETHQ") {
  ...
  function generateRandomColorFace() internal view returns (string memory) {
    ...
    string memory face = string(abi.encodePacked("hsl(", randomNum1.toString(), ", 90%, 85%)"));
    return face;
}

  function generateRandomColorDecor() internal view returns (string memory) {
    uint256 randomNum2 = randomNum(360, 3, 9); // generate a random number between 0 and 359
    string memory decor = string(abi.encodePacked("hsl(", randomNum2.toString(), ", 90%, 25%)"));
    return decor;
}
...
function claimTokens(uint experience) public {
    uint tokens = experience * (10**18);
...
function startQuest(uint _questIndex) public {
    ...
    if (won) {
        ...
        updateExperience(); // update experience based on test token balance
    } else {
...
function updateExperience() internal {
    uint tokens = testToken.balanceOf(msg.sender);
    uint currentLevel = characters[msg.sender].level;
    uint experienceNeeded = (BASE_EXPERIENCE) * (EXPERIENCE_MULTIPLIER ** currentLevel);
  ...
  function tokenURI(uint256 tokenId) public view override returns (string memory) {
      ...
      string memory description = string(abi.encodePacked('EtherQuest Hero'));
                ...
                Base64.encode(
                    bytes(
                          abi.encodePacked(
                              ...
                              '", "description":"',
                              description,
                              '", "external_url":"https://pub-gmn.com',
                              tokenId.toString(),
                              '", "image": "',
...
function renderTokenById() public view returns (string memory) {
string memory face = characters[msg.sender].face;
string memory decor = characters[msg.sender].decor;
string memory render = string(abi.encodePacked(
    ...
    '<rect id="nosePiece" fill="hsl(',randomNum(361,3,3).toString(),',90%,25%)" x="211.33936" y="69.56966" width="89.3213" height="332.0422"/>',
    '<rect fill="hsl(',randomNum(361,3,3).toString(),',90%,25%)" x="162.91666" y="0" width="186.1667" height="73.78716"/>',
