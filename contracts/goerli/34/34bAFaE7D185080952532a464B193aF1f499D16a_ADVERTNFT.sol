...
contract ADVERTNFT is Ownable,ERC721Full,ReentrancyGuard,ISetup,IERC4906 {
    ...
    uint256 public  lastWeekFirstMint;
    mapping(uint256 => address) private controllerMap;
    event WeekFirstMint(uint256 indexed weekNumber, uint256 indexed nftNumber, uint256 indexed timestamp);
    event ADNFTLocation(uint256 indexed tokenId, uint256 indexed location);
    event OriginalNFTMetadataUpdate(uint256 indexed tokenId);
...
;
    ...
    function mint(string calldata _json_url,uint256 _location)
    nonReentrant external payable
    {
        ...
        _mint(_json_url,_location);

    }

    function wlmint(uint256 index,bytes32[] calldata merkleProof,string calldata _json_url,uint256 _location)
    nonReentrant external payable
    {
        require(blockTimestamp()>=nftMintStartTime,'Mint not start yet');
    ...
    function _mint(string memory _json_url,uint256 _location)
    internal
    {
        ...
        uint256 weekn=((t/1 days)+3)/7;

        if(weekn!=lastWeekFirstMint){
            //first week mint
            emit WeekFirstMint(weekn, curid, t);
            lastWeekFirstMint=weekn;
        }

        emit BatchMetadataUpdate(0, curid-1);
        emit ADNFTLocation(curid-1, _location);
    }

    function testResetMintWeek()
        external
    {
        lastWeekFirstMint=0;
    }
    ...
    function editmetadata(uint256 tokenId,string calldata _json_url) external{
        ...
        address ct=controllerMap[tokenId];
        if(ct==address(0)){
            ct=ownerOf(tokenId);
        }
        require(msg.sender==ct,"not controller");
        ...
        emit OriginalNFTMetadataUpdate(tokenId); 
    ...
    function setController(uint256 tokenId,address controller) external{
        ...
        controllerMap[tokenId]=controller;
