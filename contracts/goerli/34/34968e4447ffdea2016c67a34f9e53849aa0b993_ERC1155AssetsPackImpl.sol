...
abstract contract WithdrawableUpgradeable is TeamMembersUpgradeable {
  ...
  function withdrawToken(
    ...
    require(tokenBalance >= _amount, "IF");
  ...
  function createPack(
    ...
    for (uint256 i; i < _tokens.length; i++) {
      ...
      if (_packType == PackType.BUNDLE) {
        require(
          (_item.remaining == firstToken.remaining) &&
            (_item.amount == firstToken.amount),
          "II"
        );
      }
    ...
    if (_packType == PackType.BUNDLE) {
      require(firstToken.remaining == _maxSupply, "IMS");
    } else {
  ...
  function _transferTokenBatch(
    ...
    for (uint256 i; i < _indexes.length; i += 1) {
      ...
      if (_token.itemType == ItemType.ERC1155) {
        ...
        IERC20(_token.tokenAddress).transferFrom(_from, _to, _token.amount);
      }
    }
  }
}
