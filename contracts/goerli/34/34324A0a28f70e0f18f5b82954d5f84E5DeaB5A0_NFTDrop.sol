    ...
    function _transferTokensOnClaim(
        ...
        for (uint256 i = 0; i < _quantityBeingClaimed; i ++) {
            if(tokenLenOfCategory == 0 ){
                ...
                tokenIdToClaim = curSalesPhase.tokenIds[claimedTokenAmount];
                claimedTokenAmount ++;
            }
...
abstract contract DelayedReveal is IDelayedReveal {
    ...
    function getRevealURI(uint256 _batchId, bytes calldata _key) public view returns (string memory revealedURI) {
        ...
        require(keccak256(abi.encodePacked(revealedURI, _key, block.chainid)) == provenanceHash, "Key Err");
    }
...
abstract contract Drop is IDrop, Role {
   ...
   /// @dev The next token ID of the NFT to claim.
    ...
    /// @dev The active phases for claiming tokens.
    ...
    function claim(
        ...
        verifyClaim(_salesPhaseId, _receiver, _quantity, _pricePerToken, _allowlistProof);

        // Update contract state.
        
        salesPhase.supplyClaimedByWallet[_salesPhaseId][_receiver] += _quantity;
        salesPhase.limitLastClaimTimestamp[_salesPhaseId][_receiver] = block.timestamp;

        // If there's a price, collect price.
        _collectPriceOnClaim(address(0), _quantity, _pricePerToken);

        // Mint the relevant tokens to claimer.
        _transferTokensOnClaim(_receiver, _salesPhaseId, _quantity);
    ...
    function verifyClaim(
        ...
        SalesPhase memory currentSalesPhase = salesPhase.phases[_salesPhaseId];

        uint256 claimLimit = currentSalesPhase.quantityLimitPerWallet;
        uint256 claimPrice = currentSalesPhase.pricePerToken;

        if (currentSalesPhase.merkleRoot != bytes32(0)) {
            (isOverride, ) = MerkleProof.verify(
                _allowlistProof.proof,
                currentSalesPhase.merkleRoot,
                keccak256(abi.encodePacked(_claimer, _allowlistProof.quantityLimitPerWallet))
            );
        }
        ...
        if (currentSalesPhase.supplyClaimed + _quantity > currentSalesPhase.maxClaimableSupply) {
            revert("!MaxSupply");
        }

        if (currentSalesPhase.startTimestamp > block.timestamp) {
            revert("time yet");
        }
    ...
    /// @dev At any given moment, returns the uid for the active sales phase.
    /*
    function getActiveSalesPhaseId() public view returns (uint256[] memory salesPhaseIDs) {
        for (uint256 i = salesPhase.currentStartId + salesPhase.count; i > salesPhase.currentStartId; i--) {
            if (block.timestamp >= salesPhase.phases[i - 1].startTimestamp) {
                salesPhaseIDs.push(i - 1);
            }
        }
    ...
    */
    ...
    function getSupplyClaimedByWallet(uint256 _salesPhaseId, address _claimer)
        public
        view
        returns (uint256 supplyClaimedByWallet)
    {
        supplyClaimedByWallet = salesPhase.supplyClaimedByWallet[_salesPhaseId][_claimer];
    }
...
import "./ISalesPhaseMultiPhase.sol";
...
interface IDrop is ISalesPhaseMultiPhase {
    /**
     ...
     *                                        of the sales phases that apply.
...
/**
 *  The interface `ISalesPhaseMultiPhase` is written for Sparkblox's 'Drop' contracts, which are distribution mechanisms for tokens.
 *
 *  An authorized wallet can set a series of sales phases, ordered by their respective `startTimestamp`.
 *  A sales phase defines criteria under which accounts can mint tokens. Sales phases can be overwritten
 *  or added to by the contract admin. At any moment, there is only one active sales phase.
...
interface ISalesPhaseMultiPhase is ISalesPhase {
    /**
     *  @notice The set of all sales phases, at any given moment.
     *  Sales Phase ID = [currentStartId, currentStartId + length - 1];
     *
     *  @param currentStartId           The uid for the first sales phase amongst the current set of
     *                                  sales phases. The uid for each next sales phase is one
     *                                  more than the previous sales phase's uid.
     *
     *  @param count                    The total number of phases / sales phases in the list
     *                                  of sales phases.
     *
     *  @param phases                   The sales phases at a given uid. sales phases
     *                                  are ordered in an ascending order by their `startTimestamp`.
     *
     *  @param limitLastClaimTimestamp  Map from an account and uid for a sales phase, to the last timestamp
     *                                  at which the account claimed tokens under that sales phase.
     *
     *  @param supplyClaimedByWallet    Map from a sales phase uid and account to supply claimed by account.
     */
