...
contract AirdropSimple is IAirdropV1Simple, Initializable {
    using SafeERC20Upgradeable for IERC20Upgradeable;

    uint8 public constant AIRDROP_TYPE = 1;
    ...
    function initialize(AirdropConfig memory cfg) initializer public {
        __AirdropSimple_init(cfg);
    }

    function __AirdropSimple_init(AirdropConfig memory cfg) internal onlyInitializing {
        __AirdropSimple_init_unchained(cfg);
    }

    function __AirdropSimple_init_unchained(AirdropConfig memory cfg) internal onlyInitializing {
        token = cfg.token;
...
interface IAirdropV1Simple {
    ...
    function initialize(AirdropConfig memory cfg) external;
