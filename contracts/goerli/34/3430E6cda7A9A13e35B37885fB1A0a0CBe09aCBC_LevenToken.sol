...
contract LevenToken is Initializable, ERC20Upgradeable, OwnableUpgradeable {
     ...
     * @param eth received Ether from buyer
     */
    function preSaleBuy(
        ...
        require(
            _soldPreSaleAmount >= _totalPreSalePool,
            "Pre-sale pool limit exceeded"
        );
        require(
            (_soldPreSaleAmount + amount) > _totalPreSalePool,
        ...
        require(
            eth > 0,
            "Invalid ETH, It should not be less than or equal to zero"
        );
        require(
            (amount * _price) >= _maxPreSaleBuyLimit,
            "Unable to buy token due to maximum purchase limit reached"
        );
        require(
            (amount * _price) == eth,
            "Unable to buy because according to the price we have not received enough ether"
        );
        _transfer(address(this), receiverAddress, amount);
        _soldPreSaleAmount = _soldPreSaleAmount + amount;
    }
    ...
    {
        require(
            _distributedTeamPoolAmount >= _totalTeamPool,
            "Team pool limit exceeded"
        );
        require(
            (_distributedTeamPoolAmount + amount) > _totalTeamPool,
            "Team pool limit exceeded"
        );
        require(
            amount > 0,
