...
contract halflingsMetadataUpdate is Ownable {
      ...
      mapping(uint256 =>mapping(address => bool)) public _isBuy;
    ...
    )external payable
    {
        ...
        bytes32 hash = keccak256(abi.encodePacked(nftid,time,amount,seed));
        bytes32 message = ECDSA.toEthSignedMessageHash(hash);
        address receivedAddress = ECDSA.recover(message, signature);
        require(receivedAddress == owner(), "Invalid signature");
        require(!_isBuy[nftid][_msgSender()], "already buy");
        _isBuy[nftid][_msgSender()]=true;
