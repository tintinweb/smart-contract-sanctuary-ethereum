...
contract FANATICO3 is ERC20, ERC20Burnable, ERC20FlashMint, ERC1363, AccessControl {
    ...
    function blackListAccount(address account_, bool state_) external onlyRole(DEFAULT_ADMIN_ROLE) {
        require(blackList[account_] != state_, 'Already set');
    ...
    function recover(
        ...
        if (token_ != address(0)) {
			ERC20(token_).transfer(recipient_, amount_);
		} else {
    ...
    event Blacklisted(address account_, bool state_);
}