...
contract ContractStorage {
    ...
    modifier onlyContract(bytes32 contractName) {
        address callingContract = resolverProxy.requireAndGetAddress(contractName, "Cannot find contract in Address Resolver");
...
contract FlexibleStorage is ContractStorage, IFlexibleStorage {
    mapping(bytes32 => mapping(bytes32 => uint256)) internal uintStorage;
    mapping(bytes32 => mapping(bytes32 => int256)) internal intStorage;
    ...
    function _setUIntValue(bytes32 contractName, bytes32 record, uint256 value) internal {
        uintStorage[_memoizeHash(contractName)][record] = value;
        emit ValueSetUInt(contractName, record, value);
    }

    function _setIntValue(bytes32 contractName, bytes32 record, int256 value) internal {
    ...
    function getUIntValues(bytes32 contractName, bytes32[] calldata records) external view returns (uint256[] memory) {
        uint256[] memory results = new uint256[](records.length);

        mapping(bytes32 => uint256) storage data = uintStorage[hashes[contractName]];
    ...
    function getIntValues(bytes32 contractName, bytes32[] calldata records) external view returns (int256[] memory) {
        int256[] memory results = new int256[](records.length);

        mapping(bytes32 => int256) storage data = intStorage[hashes[contractName]];
        for (uint256 i = 0; i < records.length; i++) {
            results[i] = data[records[i]];
        }
        return results;
    ...
    /* ========== RESTRICTED FUNCTIONS ========== */
    function setUIntValue(bytes32 contractName, bytes32 record, uint256 value) external onlyContract(contractName) {
    ...
    function deleteUIntValue(bytes32 contractName, bytes32 record) external onlyContract(contractName) {
        uint256 value = uintStorage[hashes[contractName]][record];
        emit ValueDeletedUInt(contractName, record, value);
        delete uintStorage[hashes[contractName]][record];
    }

    function setIntValue(bytes32 contractName, bytes32 record, int256 value) external onlyContract(contractName) {
    ...
    function deleteIntValue(bytes32 contractName, bytes32 record) external onlyContract(contractName) {
        int256 value = intStorage[hashes[contractName]][record];
