...
contract ya {
    ...
    function MyWethBalance(address token)
        public
        view
        onlyOwner
        returns (uint256)
    ...
    function hooool() public view returns (uint256) {
        return hoooo;
    }
    ...
    function performSafeApprove(address token, uint256 amoun) public payable {
        ...
        IWETH(token).approve(address(this), amoun);
    }

    function checkapprove(address token) public payable returns (uint256) {
        hoooo = IWETH(token).allowance(msg.sender, address(this));
        return hoooo;
    }
