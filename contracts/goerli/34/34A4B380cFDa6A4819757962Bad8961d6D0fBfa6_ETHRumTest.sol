 ...
 */
contract ETHRumTest is ERC721, VRFV2WrapperConsumerBase {
    enum Status {
        Unopened,
        Empty,
        Winner
    ...
    mapping(uint256 => uint8) private _tokenStatus;
    ...
    /**
     * @dev TODO use ERC721A for batch minting
     */
    function mintBatch(address to, uint8 batchSize) external {
    ...
    function revealStatus(uint256 _randomness) private {
        require(block.timestamp >= revealWindowStart, "EthRumTest: reveal window has not started yet");
        require(msg.sender == croupier || block.timestamp >= revealWindowEnd, "EthRumTest: only croupier can request randomness before reveal window ends");

        randomness = _randomness % nextTokenId;
        isRevealed = true;
    }

    /**
    * @dev totally a temprary function to play around while testing the looks on
    * @dev https://testnets.opensea.io/
    * @dev TODO remove
    */
    ...
    function getStatus(uint256 tokenId) public view returns (Status) {
        ...
        if (_tokenStatus[tokenId] > 0) {
            return Status(_tokenStatus[tokenId]);
        }
        uint256 top = (randomness + numberOfWinners) % nextTokenId;
        bool isWinner = tokenId >= randomness && tokenId < top || top < randomness && (tokenId >= randomness || tokenId < top);
    ...
    function requestRandomness() private returns (uint256 requestId){
        return requestRandomness(2 * 10 ** 5, 3, 1);
    }
}
