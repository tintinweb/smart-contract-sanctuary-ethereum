...
contract IndigodzNft is
    ERC721Enumerable,
    ERC721URIStorage,
    ERC721Royalty,
    InterfaceIndigodzNft,
    AccessControl
{
     ...
     */
    modifier onlyMinter() {
        require(msg.sender == minter, 'IndigodzNft: caller is not the minter');
        _;
    }

    /*
     * @dev Constructs a new IndigodzNft
     * @param _minter IndigodzMinter address
     * @param _reveal IndigodzReveal address
     * @param _baseURI Base URI
     */
    constructor(address _minter, address _reveal, string memory _uri) ERC721('Indigodz Nft', 'IGDZ') {
        require(_minter != address(0), 'IndigodzNft: invalid minter address');
        require(_reveal != address(0), 'IndigodzNft: invalid reveal address');

        minter = _minter;
        _setupRole(DEFAULT_ADMIN_ROLE, msg.sender);
        _setupRole(REVEAL_ROLE, _reveal);
        _setBaseURI(_uri);
    }
    ...
    function _setBaseURI(string memory _uri) internal {
        require(bytes(_uri).length != 0, 'IndigodzNft: invalid uri');
        baseURI = _uri;
    }
    ...
    /*
     * @dev reveal uris for token ids
     * @param list of _tokenIds as uint256[]
     * @param list of _hashes as string[]
     */
    function reveal(uint256[] calldata _tokenIds, string[] calldata _hashes) public onlyRole(REVEAL_ROLE) {
        require(_tokenIds.length == _hashes.length, 'IndigodzNft: tokens number not equal to hash number');
        for (uint i = 0; i < _tokenIds.length; i++) {
            require(_exists(_tokenIds[i]), 'IndigodzNft: try to reveal for nonexistent token');
            _setTokenURI(_tokenIds[i], _hashes[i]);
        }

        emit Reveal(_tokenIds, _hashes);
    }
