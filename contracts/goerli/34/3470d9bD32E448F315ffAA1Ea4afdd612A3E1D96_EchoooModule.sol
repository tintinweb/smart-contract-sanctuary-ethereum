...
import "../../trustlists_contracts/interfaces/IAuthoriser.sol";
...
// Copyright (C) 2022  Echooo Labs Ltd. <https://echooo.xyz>
...
/**
 * @title EchoooModule
 * @notice Single module for the Echooo wallet.
 * @author Skywalker Bob - <[email protected]>
 */
contract EchoooModule is BaseModule, RelayerManager, SecurityManager, TransactionManager {

    bytes32 constant public NAME = "EchoooModule";
    ...
    */
    ...
    function getRequiredSignatures(address _wallet, bytes calldata _data) public view override returns (uint256, OwnerSignature) {
        ...
        if (methodId == TransactionManager.multiCall.selector ||
            ...
            methodId == EchoooModule.addModule.selector ||
...
abstract contract SecurityManager is BaseModule {
    ...
    event GuardianAdditionRequested(address indexed wallet, address[] indexed guardians, uint256 executeAfter);
    ...
    */
     ...
     * @param _wallet The target wallet.
     * @param _guardians The _guardians to add.
     */
    function addGuardian(address _wallet, address[] calldata _guardians) external onlyWalletOwnerOrSelf(_wallet) onlyWhenUnlocked(_wallet) {
        ...
        for (uint i; i< _guardians.length; i++){
            ...
            require(success, "SM: must be EOA/Echooo wallet");
        ...
        emit GuardianAdditionRequested(_wallet, _guardians, block.timestamp + securityPeriod);
    }
     ...
     * @param _wallet The target wallet.
     * @param _guardians The _guardians.
     */
    function confirmGuardianAddition(address _wallet, address[] calldata _guardians) external onlyWhenUnlocked(_wallet) {
        GuardianManagerConfig storage config = guardianConfigs[_wallet];
        for (uint i; i< _guardians.length; i++){
            address _guardian = _guardians[i];
            bytes32 id = keccak256(abi.encodePacked(_wallet, _guardian, "addition"));
        ...
        emit GuardianAdded(_wallet, _guardians);
    }
