...
// this proposal is auto deployed when emergency suspend Occurs
contract AutoDeployContractWhileEmergencySuspend is IProposal, ERC165 {
    //solhint-disable var-name-mixedcase
    string public override NAME;
    string public override ABSTRACTION;
    string public override IPFS;
    //solhint-enable var-name-mixedcase

    IGovernance public immutable override governanceContract;
    address public suspended;
    // if true consortium else proposal is suspended
    bool public isConsortium;

    error SuspendingZeroAddress();
    ...
    constructor(
        ...
        if (_suspended == address(0)) revert SuspendingZeroAddress();
        suspended = _suspended;
        NAME = _name;
        ABSTRACTION = _abstraction;
        IPFS = _ipfs;
        isConsortium = _isConsortium;
    }
    ...
    function execute() external override {
        ProposalMethods[] memory methods = new ProposalMethods[](1);

        if (isConsortium) {
            methods[0] = ProposalMethods({
                method: abi.encodeWithSignature(
                    "reinstateConsortium(address)",
                    suspended
                ),
