...
contract Factory is
    ...
    UUPSUpgradeable
{
    //indexInstanceArray will store the instance of the created index fund contract
    Index[] public indexInstanceArray;
    ...
    // can be used to pause the function
    function pause() public onlyOwner {
        _pause();
    }

    // can be used to unpause the function
    ...
    function createIndexFund(
        ...
        Index index = new Index(_name, _percentages, _tokens);
        indexInstanceArray.push(index);
        indexIdByAddress[address(index)] = indexInstanceArray.length - 1;

        emit CreateIndexFund(
            msg.sender,
            indexInstanceArray.length - 1,
            _name,
            _percentages,
            _tokens
        );
    ...
    function updateIndexFund(
        ...
        Index(indexInstanceArray[id]).udpateindex(_name, _percentages, _tokens);
        ...
        )
    {
        ...
        ) = Index(indexInstanceArray[id]).getIndexInfo();
    ...
    function getNumberofindex() public view returns (uint256) {
        return indexInstanceArray.length;
    }

    //Will be used to upgrade the implementation contract
...
{   
    ...
    // structure that stores the name,token addresses and percentages to correspoding tokens
    struct IndexInfo
    {    string name;
        address[] tokens;
        uint[] percentages; 
...
// will initialize the index contract with name, token addresses and their corresponding percentages
    constructor(string memory _name,uint[] memory _percentages, address[] memory _tokens)
    {
  ...
  // will update the index with the supplied inputs, can only be called by factory contract
    function udpateindex(string memory _name,uint[] memory _percentages, address[] memory _tokens)external onlyOwner
    {
       indexinfo.name=_name;
       indexinfo.tokens=_tokens;
       indexinfo.percentages=_percentages;
    ...
    function getIndexInfo() public view returns (string memory,address[] memory,uint[] memory){
