...
abstract contract BaseBSKR is IERC20, Ownable, Manageable, Pausable, Utils {
    ...
    event Log03B(address from, address to, uint256 amount); // TODO remove this
...
contract Stakable is Ownable {
     ...
     */
    function _addStakeholder(address staker) internal returns (uint32) {
        // Push a empty item to the Array to make space for our new stakeholder
        _stakeholders.push();
        // Calculate the index of the last item in the array by Len-1
        uint32 userIndex = uint32(_stakeholders.length - 1);
    ...
    function _getCurrStake(uint32 userIndex, uint16 stakeIndex)
        internal
        view
    ...
    function _stake(
        ...
        uint32 userIndex = uint32(_stakes[_msgSender()]);
        ...
        emit Staked(
            ...
            sharesLBSKR,
            sharesBSKR,
            uint16(_stakeholders[userIndex].userStakes.length - 1),
            sinceBlock
        );
    }

    function _withdrawStake(uint16 stakeIndex, uint256 unstakeAmount)
        internal
        returns (
        ...
        )
    {
        ...
        if ((currStake.amountLBSKR - unstakeAmount) == 0) {
            ...
            emit Debug05L_(
                "after",
                _msgSender(),
                updatedStake.amountLBSKR,
                updatedStake.amountBSKR,
                updatedStake.sharesLBSKR,
                updatedStake.sharesBSKR,
                updatedStake.sinceBlock
            );
        }
    ...
    {
        uint32 userIndex = uint32(_stakes[stakeholder]);
...
contract LBSKR is BaseBSKR, DSMath, Stakable {
    ...
    event Debug04L(uint256 lbskrSent, uint256 bskrSent, uint16 eligibleBasis);
    event Debug05L(
        string msg,
        address user,
        uint256 amountLBSKR,
    ...
    event Debug07L(
        uint256 balInfAddrBefore,
        uint256 rfiInfAddrBefore,
        uint256 balInfAddrAfter,
        uint256 rfiInfAddrAfter
    );
    ...
    event Debug09L(uint256 bskrShares2Deduct, uint256 _totalBSKRShares);
        ...
        returns (uint256 bskrAmount)
    {
        ...
        uint256 rfiInfAddrBefore = _BSKR.totalReflection();
        ...
        uint256 rfiInfAddrAfter = _BSKR.totalReflection();
        ...
        // Need to deduct the reflection fee as balance includes the reflection
        return
            _BSKR.balanceOf(_inflationAddress) -
            balInfAddrBefore -
            (rfiInfAddrAfter - rfiInfAddrBefore);
    }
    ...
    function isBSKRLBSKRV3Pool(address target)
        private
        view
        isInitialRatioSet
        returns (bool)
    ...
    function penaltyFor(uint256 fromTimestamp, uint256 toTimestamp)
        public
        pure
        returns (uint16 penaltyBasis)
    {
        penaltyBasis = 0;
        if (fromTimestamp + 52 weeks > toTimestamp) {
            uint16 fourWeeksElapsed = uint16(
                (toTimestamp - fromTimestamp) / _SECS_IN_FOUR_WEEKS
            );
    ...
    function penaltyIfUnstakedNow(
        ...
        //uint256 amount,
        uint16 stakeIndex
    ) external view returns (uint16 penaltyBasis) {
        uint32 userIndex = uint32(_stakes[account]);
        Stake memory currStake = _getCurrStake(userIndex, stakeIndex);

        return penaltyFor(currStake.sinceBlock, block.timestamp);
    }
     ...
     */
    function rewardsOf(address stakeholder, uint16 stakeIndex)
        external
        view
        returns (
            uint256 lbskrRewards,
            uint256 bskrRewards,
            uint16 eligibleBasis
        )
    {
        uint256 inflation = 0;
        if (_lastDistTS > 0) {
        ...
        eligibleBasis =
            _BIPS -
            penaltyFor(currStake.sinceBlock, block.timestamp);

        lbskrRewards =
            (((((balanceOf(address(this)) + _totalLBSKRStakes + inflation) *
                currStake.sharesLBSKR) / _totalLBSKRShares) -
                currStake.amountLBSKR) * eligibleBasis) /
            _BIPS; // stakeAmount never existed - it's notional
        bskrRewards =
            ((((_BSKR.balanceOf(_inflationAddress) * currStake.sharesBSKR) /
                _totalBSKRShares) - currStake.amountBSKR) * eligibleBasis) /
            _BIPS;
     ...
     */
    function unstake(uint256 stakeAmount, uint16 stakeIndex)
        external
        isInitialRatioSet
        whenNotPaused
    {
        ...
        emit Debug09L(bskrShares2Deduct, _totalBSKRShares);
        ...
        emit Debug04L(lbskrToSend, bskrToSend, eligibleBasis);
    }
}
