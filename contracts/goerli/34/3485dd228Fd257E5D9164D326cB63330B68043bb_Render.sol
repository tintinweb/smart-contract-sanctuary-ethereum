...
contract Render is Initializable, OwnableUpgradeable, SharedStructs {
    ...
    function _getOneSVG(Trait memory _currentTrait) internal view returns (bytes memory) {
        bytes memory buffer = DynamicBuffer.allocate(2**18);
        _renderImg(_currentTrait, LayerStruct(0,0,0,0), buffer);
        buffer.appendSafe('<style>#pixel {image-rendering: pixelated; image-rendering: -moz-crisp-edges; image-rendering: -webkit-crisp-edges; -ms-interpolation-mode: nearest-neighbor;}</style></svg>');
    ...
    function getSVGForBytes(bytes memory data) public view returns(string memory) {
        ...
        console.log(height);
        console.log(width);
        buffer.appendSafe(bytes(string.concat('<svg xmlns="http://www.w3.org/2000/svg" shape-rendering="crispEdges" id="pixel"  version="1.1" viewBox="0 0 ', Strings.toString(width), ' ', Strings.toString(height),'" width="',Strings.toString(uint256(width)*10),'" height="',Strings.toString(uint256(height)*10),'"> ')));
        bytes3[] memory out = new bytes3[](0);
        console.log("made it past dimensions");
