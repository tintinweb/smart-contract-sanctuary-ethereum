...
function mintWhitelist(bytes32[] calldata _merkleProof, uint256 _amount) public payable {
    ...
    require(id <= totalSupply, "Error: max supply reached");
    require(_amount <= maxNFTWhitelist && _amount > 0, "Error: you can't mint more than max whitelist supply and less than one");
    require((id + _amount) <= totalSupply, "Error: more than max supply will be reached");
    require(WLClaimed[msg.sender] + _amount <= maxNFTWhitelist, "Error: more than max whitelist supply will be reached");
...
function setSeed(bytes32 _seed) virtual public {
    require(msg.sender == relayer || msg.sender == admin);
...
function setRelayer(address _relayer) virtual public onlyAdmin {
    require(_relayer != relayer, "relayer already setted");
    relayer = _relayer;
}
