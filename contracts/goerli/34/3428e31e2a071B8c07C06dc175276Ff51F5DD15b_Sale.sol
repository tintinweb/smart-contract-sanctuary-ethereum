...
contract Sale is ReentrancyGuard, Ownable {
        ...
        returns (bool)
    {
        ...
        require(presale[presaleId].HardcapAchived + usdAmount <= presale[presaleId].UsdtHardcap,
        ...
        presale[presaleId].HardcapAchived += usdAmount;
    ...
    function buyWithEth()
        ...
        returns (bool)
    {
        uint256 usdAmount = (msg.value * getLatestPrice() * USDT_MULTIPLIER) / (ETH_MULTIPLIER * 2);
        require(presale[presaleId].HardcapAchived + usdAmount <= presale[presaleId].UsdtHardcap,
        "Amount should be less than leftHardcap");
    ...
    {
        uint256 usdPrice = (amount * presale[_id].price) / (10**18);
        ethAmount = (usdPrice * ETH_MULTIPLIER) / getLatestPrice();
    }
        ...
        returns (uint256 _tokens)
    {
        ...
        _tokens = (usdAmount * ETH_MULTIPLIER) / presale[_id].price;
    }
    ...
    function usdtToTokens(uint256 _id, uint256 amount)
        ...
        returns (uint256 _tokens)
    {
        _tokens = (amount * 10**IERC20Metadata(SaleToken).decimals()) / presale[_id].price;
    }
