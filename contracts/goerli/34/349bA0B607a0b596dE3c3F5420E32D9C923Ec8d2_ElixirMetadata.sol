...
contract ElixirMetadata is Ownable {
    using Strings for uint256;

    address elixirBottleContract;
    ...
    // contract addresses & names for ingredients
    uint256 public numIngredients;
    mapping(uint256 => address) ingredientContracts;
    mapping(uint256 => string) ingredientNames;
    // mapping from contract addresses back to uints to facilitate easy insertion/deletion/editing
    mapping(address => uint256) private _ingredientContractsRevMap;

    // how many charges does each elixir hold
    mapping(uint256 => uint256) private _charges;

    /**
     * @dev 0: fire, 1: water, 2: earth, 3: air (for frontend parsing)
    **/
    mapping(uint256 => uint256) private _elements;
    ...
    mapping(uint256 => string) private _attributeNames;
    ...
    function registerIngredient(address ingredientContract, string memory ingredientName) public onlyOwner {
        ingredientContracts[numIngredients] = ingredientContract;
        ingredientNames[numIngredients] = ingredientName;
        _ingredientContractsRevMap[ingredientContract] = numIngredients;
        numIngredients++;
    }

    function getIngredientContract(uint256 ingredientType) public view returns (address) {
        return ingredientContracts[ingredientType];
    }

    function getCharges(uint256 elixirId) public view returns (uint256) {
        uint256 elixirBlock = elixirId/128;
        return (_charges[elixirBlock] >> ((elixirId - (elixirBlock*128))*2))%4;
    }

    function getElement(uint256 elixirId) public view returns (uint256) {
        uint256 elixirBlock = elixirId/128;
        return (_elements[elixirBlock] >> ((elixirId - (elixirBlock*128))*2))%4;
    }

    function getAttributes(uint256 elixirId) public view returns (uint256[] memory) {
        uint256 attrCt = uint256(_attributes[elixirId].count);
        uint256 attrs = _attributes[elixirId].attrs;

        uint256[] memory toReturn = new uint256[](attrCt);

        uint256 writePos = 0;
        for (uint256 i = 0; writePos < attrCt; i++) {
            if ((attrs >> i)%2 == 1) {
                toReturn[writePos] = i;
                writePos++;
            }
    ...
    function getIngredientList() public view returns (string[] memory) {
        string[] memory toReturn = new string[](numIngredients);

        for (uint256 i = 0; i < numIngredients; i++) {
            toReturn[i] = ingredientNames[i];
        }
    ...
    **/
    function charge(
        ...
        require(_charges[elixirId] == 0, "z");

        uint248 chargeAttributes = 1;
        unchecked {
            for (uint256 i = 0; i < ingredientTypes.length; i++) {
                chargeAttributes *= uint248((ingredientTypes[i] + 1)*(ingredientIDs[i] + 1));
            }
        }

        uint8 numAttrs = 0;
        for (uint256 i = 0; i < 248; i++) {
            if ((chargeAttributes >> i)%2 == 1) {
                numAttrs++;
            }
        }

        _attributes[elixirId] = Attributes(numAttrs, chargeAttributes);
        _charges[elixirId] = ((elixirId*ingredientTypes[0]*ingredientIDs[0])%3) + 1;
    }
    ...
    **/
    function drink(uint256 elixirId) public onlyBottleContract {
        require(_charges[elixirId] != 0, "nz");
        _charges[elixirId] -= 1;
    }
