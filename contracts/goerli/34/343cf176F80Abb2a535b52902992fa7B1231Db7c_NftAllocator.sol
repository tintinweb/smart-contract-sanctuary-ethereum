...
contract NftAllocator is Ownable, Initializable, ERC721A__IERC721Receiver {
    ...
    struct TokensAllocated {
        /// @dev the account that claimed the tokens
        address account;
        /// @dev the amount of tokens allocation claimed
        uint128 claimed;
        /// @dev the amount of tokens allocation unclaimed
        uint128 unclaimed;
    }

    /// @dev The address where Jubi fees will be sent
    address public jubiFundsAddress;

    /// @dev The Jubi fee amount
    uint256 public jubiFee;

    /// @dev The funds a venture will get per investment
    uint256 public ventureFunds;

    /// @dev which nft investors will be claiming
    IERC721A public nft;

    /// @dev the Venture that manages this contract
    Venture public venture;

    /// @notice The name used to identify this Allocator in Dapps
    string public name;

    /// @dev Is the allocator open
    bool public isOpen;

    /// @dev price to pay for nft
    uint256 public nftPrice;

    /// @dev max number of nft to sale
    ...
    /// @dev maps invitesCodes => TokensAllocated
    mapping(bytes => TokensAllocated) public invitesAllocation;

    /// @dev all address in this mapping have invested in this allocator, No is the number of active claims in this allocator
    mapping(address => uint256) public pendingClaims;

    /// @dev number of investors
    uint256 public investmentsCount;

    /// @dev number of investors which have NOT collected their nft
    ...
    */
    function initialize(address _jubiFundsAddress, Types.Fraction memory _jubiFeePercent, Types.NftAllocatorConfig memory _config) external initializer {
        ...
        jubiFee = nftPrice.mul(_jubiFeePercent.num).div(_jubiFeePercent.den);
        ventureFunds = nftPrice.sub(jubiFee);
        isOpen = true;
    }
    ...
    *
    * @dev _tokenIds, if not tokensIds available at the time of the investment
    * send an empty array []
    *
    ...
    */
    function invest(uint256 numTokens, uint256 minAllowedTokens, uint256 maxAllowedTokens, bytes calldata inviteCode, bytes32[] calldata merkleProof) external {
        ...
        require(tokensForAllocation >= investmentsCount + numTokens, "NftAllocator: Investment will excede tokens allocation");
        ...
        totalUnclaimed = totalUnclaimed.add(numTokens);
        ...
        if (address(nft) != address(0) && nft.balanceOf(address(this)) >= totalUnclaimed && (tokenIdsLength - nextTokenId) >= numTokens) {
            ...
            totalUnclaimed = totalUnclaimed.sub(numTokens);
            invitesAllocation[inviteCode].claimed += uint128(numTokens);
            SafeERC20.safeTransferFrom(venture.treasuryToken(), msg.sender, venture.fundsAddress(), ventureFunds.mul(numTokens));
        } else {
            pendingClaims[msg.sender] = pendingClaims[msg.sender].add(numTokens);
            invitesAllocation[inviteCode].unclaimed += uint128(numTokens);
            SafeERC20.safeTransferFrom(venture.treasuryToken(), msg.sender, address(this), ventureFunds.mul(numTokens));
            emit PendingClaim(msg.sender, ventureFunds, numTokens);
        }

        invitesAllocation[inviteCode].account = msg.sender;
        investmentsCount = investmentsCount.add(numTokens);

        SafeERC20.safeTransferFrom(venture.treasuryToken(), msg.sender, jubiFundsAddress, jubiFee.mul(numTokens));
        emit Investment(msg.sender, nftPrice.mul(numTokens));
    }
    ...
    */
    function claim(uint256 numToClaim) external {
        ...
        SafeERC20.safeTransfer(venture.treasuryToken(), venture.fundsAddress(), ventureFunds.mul(numToClaim));
        totalUnclaimed = totalUnclaimed.sub(numToClaim);
        pendingClaims[msg.sender] = pendingClaims[msg.sender].sub(numToClaim);
    }

    /**
    * @notice Closes this allocator, as consequence investments are DISABLED and claims are ENABLED
    */
