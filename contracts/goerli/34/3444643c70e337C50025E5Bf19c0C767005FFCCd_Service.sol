    ...
    IService
{
    ...
    */
    ...
    /**
    @param ballotExecDelay_ executeBallot delay settings
    First value corresponds to ballotExecDelayThresholdAmountUSDT
    Rest of values correspond to ballotExecDelay
    */
    function createPool(
        IPool pool,
    ...
    function addEvent(IDirectory.EventType eventType, uint256 proposalId, string memory description)
        ...
        whenNotPaused
    {
        directory.addEventRecord(msg.sender, eventType, proposalId, description);
    }
    ...
    /**
    @param ballotExecDelay_ block delay for executeBallot
    [0] - ballot value in USDT after which delay kicks in
    [1] - base delay applied to all ballots to mitigate FlashLoan attacks.
    [2:] - ballot type bound delays to allow pool shareholders prevent bugged/hacked ballot execution.
    */
    ...
    function isManagerWhitelisted(address account) public view override returns (bool) {
...
interface IDirectory {
    ...
    function addEventRecord(address pool, EventType eventType, uint256 proposalId, string memory description)
        external
        returns (uint256 index);
