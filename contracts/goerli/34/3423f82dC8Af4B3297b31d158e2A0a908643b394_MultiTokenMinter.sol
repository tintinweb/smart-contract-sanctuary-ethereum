...
contract MultiTokenMinter is Ownable {
    mapping(uint256 => Nft) public nftMetaData;
    ...
    constructor(IMultiToken _multiToken, IToken _erc20Token) {
    ...
    function setNftMetaData(
        ...
        for (uint256 i = 0; i < ids.length; i++) {
            nftMetaData[ids[i]] = Nft(prices[i], maxSupply[i], uris[i]);
        }
    }

    function claim(uint256 id, uint256 amount) external allowed {
        Nft storage nft = nftMetaData[id];        
        require(nft.price > 0 && nft.maxSupply > 0 , "token doesn't exist");
        require(multiToken.totalSupply(id) < nft.maxSupply, "token max supply reached");

        uint256 totalPrice = amount * nft.price;

        erc20Token.burnFrom(_msgSender(), totalPrice);

        multiToken.mint(_msgSender(), id, amount, bytes(""), nft.uri);

        emit NFTClaimed(_msgSender(), id, amount);
    }

    function setCollectionURI(string memory newURI) external onlyOwner {
        multiToken.setURI(newURI);
    }

    function setTokenURI(uint256 id, string memory newURI) external onlyOwner {
        multiToken.setTokenURI(id, newURI);
    }
