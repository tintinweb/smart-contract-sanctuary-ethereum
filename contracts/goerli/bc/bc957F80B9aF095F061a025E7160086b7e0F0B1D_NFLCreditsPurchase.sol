...
import "./interfaces/INFLCredits.sol";
...
contract NFLCreditsPurchase is Ownable {
  ...
  event NFLCreditsUpdated(address indexed newAddress);
  ...
  constructor(IUniswapV2Router02 _uniswapRouter, INFLCredits _nflCredits) {
    uniswapRouter = _uniswapRouter;
    nflCredits = _nflCredits;
  }
  ...
  function setUSDC(IERC20 _address) external onlyOwner {
    usdc = _address;
    // bytes4(keccak256(bytes('decimals()'))) -> 0x313ce567;
  ...
  /* @dev: Updates NFLCredits contract referenced
   * @param: NFL Credits address
   */
  function setNFLCredits(INFLCredits _nflCredits) external onlyOwner {
    require(address(_nflCredits) != address(0), "cant set nfl credits to zero address");
    nflCredits = _nflCredits;
    emit NFLCreditsUpdated({newAddress: address(_nflCredits)});
  }
  ...
  function purchaseTokens(uint256 astoAmount) external {
    ...
    nflCredits.mint(msg.sender, credits);
...
interface INFLCredits {
  function BURNER_ROLE (  ) external view returns ( bytes32 );
  function DEFAULT_ADMIN_ROLE (  ) external view returns ( bytes32 );
  function MINTER_ROLE (  ) external view returns ( bytes32 );
  function TRANSFERRER_ROLE (  ) external view returns ( bytes32 );
