...
contract BonusDistributor is Ownable, EIP712Upgradable {
    ...
    //------- Events ----------------
    //-------------------------------
    event RemoveDust(address token, address to, uint256 amount);
    ...
    function withdrawDust(
        ...
        TransferHelper.safeTransfer(_token, _to, _amounts);
        emit RemoveDust(_token, _to, _amounts);
    }
    ...
    function validateSig(bytes32 message, bytes memory signature) public returns (bool) {
        ...
        // address signerRecovered =  ECDSA.recover(digest, operatorSigR, operatorSigVs) ;
    ...
    function getFirstByte20(bytes32 input) internal pure returns (bytes20) {
...
abstract contract InvestBase {
    ...
    uint256 constant DOUBLE_WAD = 1E36;


    /// @dev Universal method to get balance
    ...
    /// @dev Universal method to transfer token
    function _transfer(address token, address to, uint256 amount) internal {
        if (amount > 0) {
            if (token == ZERO_ADDRESS) {
                (bool success, ) = payable(to).call{ value: amount, gas: 2300 }("");
                require(success, MetaXInvestErrors.TRANSFER_NATIVE_FAILED);
            } else {
                //TRON
    ...
    /// @dev Universal method to approve token
...
library MetaXInvestErrors {

    // ============ Quantity ============

    // Quantity Over Limit
    string constant OVER_LIMIT = 'OL';
    // Quantity not enough
    string constant NOT_ENOUGH = 'NE';
    // Quantity not equal
    string constant QUANTITY_MISMATCH ='QM';
    // Quantity is Zero
    string constant ZERO_AMOUNT = 'ZAM';
    // Length is Wrong
    string constant LENGTH_MISMATCH = 'LM';

    // ============ Address ============

    // Zero Address
    string constant ZERO_ADDRESS = 'ZAD';
    // Address not Register
    string constant NOT_REGISTER = 'NR';
    // Untrusted lender
    string constant UNTRUSTED_LENDER = 'UL';
    // Address not match
    string constant ADDRESS_MISMATCH ='AM';

    // ============ Authority ============

    string constant ONLY_ENTRANCE = 'OE';

    string constant ONLY_OPERATOR = 'OO';

    string constant ONLY_FLASH_ADAPTER = 'OFA';

    string constant ONLY_ADAPTER = 'OA';

    // ============ Signature ============

    string constant HASH_USED = 'HU';

    string constant INVALID_S = 'IS';

    string constant INVALID_V = 'IV';

    string constant VALIDATION_FAILED = 'VF';

    // ============ Other ============

    string constant CALL_FAILED = 'CF';

    string constant TRANSFER_NATIVE_FAILED = 'TNF';

    string constant PERMIT_LENGTH_WRONG = 'PLW';
