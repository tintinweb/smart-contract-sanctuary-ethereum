...
contract AtlendisLockdrop is BaseLockdrop {
     ...
     * Owner can get back remaining rewards if the target pool is either closed or defaulted
     * Rescueing rewards lets users claim their pending rewards
     * Users won't accrue more rewards after this function was called
     **/
...
abstract contract BaseLockdrop is Ownable {
     ...
     **/
    function withdraw(uint256 tokenId) external virtual isLockOwner(tokenId) {
        require(
            block.timestamp >= currentLocks[tokenId].endDate,
            "Withdraw too early"
        );
     ...
     * Logic can be modified by inheriting contracts
     **/
    function claim(uint256 tokenId) external virtual isLockOwner(tokenId) {
        require(
            block.timestamp >= currentLocks[tokenId].endDate,
            "Claim too early"
        );
