...
contract CRT37 is IERC20, Auth {

    address public WETH = 0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2;
    address public CHEWY = 0xde637d4C445cA2aae8F782FFAc8d2971b93A4998;
    string public reflectionTicker = "USDC";

    string private constant _name = "CRT37";
    string private constant _symbol = "CRT37";
    uint8 private constant _decimals = 18;
    
    uint256 _totalSupply = 100 * 10**6 * (10 ** _decimals);
    uint256 public totalSupplyId = _totalSupply / (10 ** _decimals); //Id for "Intelligible display"
    uint256 _maxTx = 1 * 10**6 * (10 ** _decimals);
    uint256 public maxTxAmountId = _maxTx / (10 ** _decimals); //Id for "Intelligible display"
    uint256 _maxWallet = 2 * 10**6 * (10 ** _decimals);
    uint256 public maxWalletId = _maxWallet / (10 ** _decimals); //Id for "Intelligible display"
 ...
 */
    ...
    function changeTxLimit(uint256 newLimit) external onlyOwner {
        _maxTx = newLimit * (10 * _decimals);
    }

    function changeWalletLimit(uint256 newLimit) external onlyOwner {
        _maxWallet  = newLimit * (10 * _decimals);
    }
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        {
            require(_balances[recipient] + (amount) <= _maxWallet || isWltExempt[sender], "wallet");
        }
    ...
    function swapBack() internal lockTheSwap {
        ...
        uint256 tokensForLiquidity = tokenBalance * toLiquidity / 100 / 2;     
        uint256 amountToSwap = tokenBalance - tokensForLiquidity;

        swapTokensForEth(amountToSwap);

        uint256 totalEthBalance = address(this).balance;
        uint256 ethForCHEWY = totalEthBalance * toReflections / 100;
        uint256 ethForCampaignWallet = totalEthBalance * toMarketing / 100;
        uint256 ethForLiquidity = totalEthBalance * toLiquidity / 100 / 2;
