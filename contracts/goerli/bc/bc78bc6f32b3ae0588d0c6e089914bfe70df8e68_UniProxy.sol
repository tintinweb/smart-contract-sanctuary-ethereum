...
contract UniProxy is IERC721Receiver {
    ...
    function initialize(address uniNFTManager, address swapRouterAddress, bool requireWhitelist) external {
        nonfungiblePositionManager = INonfungiblePositionManager(uniNFTManager);
    ...
    function onERC721Received(
        ...
        _updateDeposit(operator, tokenId);

        return this.onERC721Received.selector;
    }

    function _updateDeposit(address owner, uint256 tokenId) internal {
        require(!isPaused && (!isPermissioned || isWhitelisted[msg.sender]), 'Not Whitelisted');
        ...
        )
    {
        ...
        // Transfer underlying from user to UniProxy
        ...
        // Approve the Uni Position Manager to use underlying
        ...
        // Mints liquidity based on the above parameters
        (tokenId, liquidity, amount0, amount1) = nonfungiblePositionManager
            .mint(params);

        // Create a deposit
        _updateDeposit(msg.sender, tokenId);
    ...
    function decreaseLiquidity (uint256 tokenId, uint256 amount0Min, uint256 amount1Min) external returns (uint256 amount0, uint256 amount1) {
        require(!isPaused && (!isPermissioned || isWhitelisted[msg.sender]), 'Not Whitelisted');
    ...
    function swapExactInputSingle(address tokenIn, address tokenOut, uint24 poolFee, uint256 amountIn, uint256 amountOutMinimum, uint160 sqrtPriceLimitX96 ) external returns (uint256 amountOut) {
       require(!isPaused && (!isPermissioned || isWhitelisted[msg.sender]), 'Not Whitelisted');
