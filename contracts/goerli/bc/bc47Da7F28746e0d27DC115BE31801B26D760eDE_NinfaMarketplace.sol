...
// TODO is this import needed? i.e. Will creating orders by sending tokens with data bytes as function parameters be cheaper than the old function (including the fact that approving is not needed anymore)
// this however requires sending an nft using safeTransferFrom method, and not all nft implement it, or do they?
    ...
    RoyaltyEngineV1
{
    ...
    address private ninfaERC1155Factory;
    ...
    address private constant ZERO_ADDRESS =
        0x0000000000000000000000000000000000000000; // placeholder for address(0)
    address payable private feeAccount; // EOA or contract where to send trading fees generated by the marketplace

    mapping(address => uint256) private marketplaceFees; // public so that it may be called by other contracts, such as auctions. In order to be traded on the Ninfa marketplace, collections need to be manually approved by a multisig contract.
    mapping(address => bool) private primarySaleCollections; // stores collections that are considered valid vor primary sales, such as NinfaMarketplaceV1 and V2, used in order to efficiently check whether a sale is primary or secondary, only settable by admins.
    mapping(address => mapping(uint256 => bool)) private primarySales; // map token Id to bool indicating wether it has been sold before, only applies to the Ninfa collection NFTs, all other collections are considered as secondary sales.
    ...
    struct _Order {
        ...
        uint256 price;
        uint256 amount;
        address payable commissionReceiver; // receiver of commission on sales (primary AND secondary)
        uint256 commissionBps; // commission amount, expressed in basis points 0 - 10000
    }
    ...
    /**
     * @notice ERC721 and ERC1155 must be whitelisted.
     * using logical && because they both must be false or else it means it is whitelisted, i.e. of both are false the collection is not whitelisted, opposite of require statement that would use a logical ||
     */
    modifier isWhitelisted(address _collection) {
        (, bytes memory authorized) = ninfaERC1155Factory.staticcall(
            abi.encodeWithSelector(0xf6a3d24e, _collection) // function selector: 0xf6a3d24e or bytes4(keccak256("exists(address)"));
        );

        if (
            abi.decode(authorized, (bool)) == false &&
            marketplaceFees[_collection] == 0
        ) revert Unauthorized(); // exists (bytes data) contains a boolean returned by `hasRole()`
        _;
    }
     ...
     * @param _collection address of the ERC721 contract. Doesn't require any access control besides the collection being whitelisted, i.e. msg.sender may be any address.
     ...
     * @param _seller is needed only so that the seller is known in case the address creating the order is a gallery, in which case the gallery's address will be _commissionReceiver
    ...
    function createOrder(
        ...
        _transferNFT(_collection, msg.sender, address(this), _tokenId, _amount);
     ...
     *      using _msgSender does not represent a security risk, on the other hand, it is possible for the buyer to use this parameter simply in order to transfer the NFT to an address other than their own, this can be useful for external contract buying NFTs.
     */
    function fillOrder(uint256 _orderId, address _buyer) external payable {
        _Order memory _order = orders[_orderId];
        ...
        _Trade memory trade = _Trade(
            _buyer, // address buyer
            payable(_order.from), // address seller
            _order.price, // uint256 price
            _order.tokenId, // uint256 tokenId
            _order.collection, // address collection
            _order.amount, // uint256 amount
            _order.commissionReceiver, // address commissionReceiver
            _order.commissionBps // uint256 commissionBps
        );
     ...
     * @param _seller is only passed down to the _trade function so that it can get paid (it may be any address), it is needed in case the offer is accepted by a gallery that needs to specify the seller/artist address so that it can get paid the principal amount.
     *  the information about the owner must be stored in the offer if the commission receiver is the seller (e.g. a gallery), otherwise there is no way to trace the original owner of the token, who might not even be the artist (e.g a gallery selling a piece for a collector)
     * Required:
     *
    ...
    function acceptOffer(
        ...
        // todo what if it is an erc1155, offers may take any token amount; what order id would I have to delete then if the order and offer amounts differ!?
        if (_orderId > 0) {
            ...
            // todo what if by mistake one deletes the wrong orderId corresponding to the offer
        } else {
            ...
            // if there is an order then the sender can easily prove they own the tokens, but if there is only an offer then the owner is not the marketplace so the msg.sender address will be passed to _transferNFT(),
            _transferNFT(
                _offer.collection,
                msg.sender,
                _offer.from,
    ...
    function _transferNFT(
        ...
        if (_amount == 0)
            IERC721Ninfa(_collection).transferFrom(_from, _to, _tokenId);
        else
     ...
     * Requires:
     *
     * - Requires that `(royaltyAmount +  marketplaceAmount + commissionAmount) < trade.price`.
     *
     */
    function _trade(
        _Trade memory trade // address trade.buyer, // address trade.seller, // uint256 trade.price, // uint256 trade.tokenId, // address trade.collection, // uint256 trade.amount, // address commissionReceiver, // uint256 trade.commissionBps
    ) private {
        // declare `royaltyReceiver` and `royaltyAmount` outside of if/else block in order for the variables to be visible inside the rest of the function
        address payable royaltyReceiver;
        uint256 royaltyAmount;
        // declare `marketplaceAmount`, will be assigned a value depending on whether it is a primary or secondary sale
        uint256 marketplaceAmount;
        // sellerAmount is set equal to price and reduced at each step by subtracting fees, royalties and commissions, if any.
        uint256 sellerAmount = trade.price;

        if (
            primarySaleCollections[trade.collection] == true && // collection must be whitelisted; not all collections can be sold as primary sales.
            primarySales[trade.collection][trade.tokenId] == false // not only must it be whitelisted, but also that the tokenId has not been sold yet.
        ) {
            /******************************************************
             * If Primary Sale ( Ninfa ERC721 communal Contract ) *
             *****************************************************/
            ...
            // Primary sales exist only for Ninfa ERC721 collections, ERC1155 contracts cannot be a primary sale because it is impossible to track each fungible tokenId supply
            // There are no royalties being calculated, because it is a primary sale, commissions on the other hand may apply to both primary and secondary sales, therefore are calculated after of this if/else block.
            // Since fees are 0% unly until 2024, an if statement might save a little gas until then but cost more aftereards as fees will always be non-zero. It is likely a v3 will be deployed by 2025 or that this contract is upgraded, hence we keep the if check for now.
            if (ninfaPrimaryFee > 0)
                marketplaceAmount = (trade.price * ninfaPrimaryFee) / 10000;

            // Set `primarySales` boolean to `true` after each primary sale on NinfaMarketplace.
            // this is a nested mapping in order to allow more than one collection to be sold as primary, namely the v1 NinfaERC721
            primarySales[trade.collection][trade.tokenId] = true;
        } else {
            /****************************************************
            ...
            /**
             *  `marketplaceFees[address(NinfaERC721)]` is used as a fees BPS placeholder for both NinfaERC721 and for all Ninfa self sovereign erc1155 collections.
             *  Since any collection being traded has already been whitelisted (enforced when creating a new order or offer), if it can't be found in the marketplaceFees (whitelist) then it must be an external ninfa collection clone (721 or 1155).
             *  then it reads the fee amount corresponding to the NinfaERC721 placeholder address in the `marketplaceFees` mapping (5% or 500bps currently), this has not been hardcoded in case Ninfa wants to change this at any point.
             */
            marketplaceAmount =
                (trade.price *
                    (
                        marketplaceFees[trade.collection] > 0
                            ? marketplaceFees[trade.collection] // NinfaERC721 or any external whitelisted collection
                            : marketplaceFees[address(NinfaERC721)] // Ninfa factory clones, defaults to whatever bps stored at `marketplaceFees[address(NinfaERC721)]`, currently 500 or 5%.
                    )) /
                10000;
        ...
        // subtracting primary or secondary fee amount from seller amount.
        // This is a security check as well as a variable assignment.
        // Since royalties (if any) may have been subtracted from `sellerAmount` at this point, if they exceeded price + marketplaceAmount the following variable assignment wil revert due to an underflow.
        ...
        _sendValue(feeAccount, marketplaceAmount);

        /***********************************
         * Pay seller commissions (if any) *
         **********************************/

        // calculate commission amount, only if commission shares/bps have been added in order/offer. This is both a security check and for saving gas (only calculates commission amount if bps > 0)
        if (trade.commissionBps > 0) {
            uint256 commissionAmount = (trade.price * trade.commissionBps) /
                10000;
             ...
             * this is a security check in order to avoid loss of funds, i.e. hackers stealing funds by setting commissions amount equal equal the marketplace's balance, for example (the marketplace does not enfore a maximum amount for commissions when creating an order or accepting an offer).
             * The following `if`statement is better than a `require` statement, because should the royalties or commissions be set too high be mistake, the NFT and funds would be stuck until the order is deleted; the seller would never know however because the error would appear to the buyer, whereas the buyer would be confused about the revert.
             */
            if (sellerAmount - commissionAmount > 0) {
                // must be subtracted inside of this if check or else if `sellerAmount == commissionAmount` neither the seller nor the commission receiver would get paid
                sellerAmount -= commissionAmount;
                _sendValue(trade.commissionReceiver, commissionAmount); /*(_price - marketplaceAmount - royaltyAmount)*/
         ...
         * Pay seller *
         *************/
        _sendValue(trade.seller, sellerAmount); /*(trade.price - marketplaceAmount - royaltyAmount)*/

        emit Trade(
            trade.collection, // address indexed collection
            trade.tokenId, // uint256 indexed tokenId
            trade.seller, // address indexed seller
            trade.buyer, // address buyer
            trade.price, // uint256 price
            trade.amount // uint256 amount
        );
    ...
    function _sendValue(address payable _receiver, uint256 _amount) private {
        ...
        // todo it is unlikely that the reason of this revert is the invalid amount; use another error without any parameters
        if (success == false) revert InvalidAmount(_amount);
    }
     ...
     * @param _ninfaERC721V2 is Ninfa's second version of the ERC721 collection.
     */
    function initAddresses(
        ...
        ninfaERC1155Factory = _ninfaErc1155Factory;
        ...
        NinfaERC721 = IERC721Ninfa(_ninfaERC721V2);
        ...
        marketplaceFees[_ninfaERC721V2] = NINFA_SECONDARY_FEE;
        // todo set address of NinfaERC721v1 or add it to whitelist for trading?
        // todo emit event to store this info
    }

    /// @notice whitelisting Ninfa's ERC721 collections in order to enable primary sale fees to be paid each first time a tokenId is sold.
    /// @dev this function must be called once per collection (at a minimum NinfaERC721V1 and v2) from the deployment scripts, immediately after deploying the marketplace.
    /// todo uncomment the next two functions that were commented in order to reduce contract size in order to deploy it.
    // function setPrimarySaleCollections(address _collection)
    //     external
    //     onlyRole(DEFAULT_ADMIN_ROLE)
    // {
    //     primarySaleCollections[_collection] = true;
    // }

    // function setPrimarySales(address _collection, uint256[] calldata _tokenIds)
    //     external
    //     onlyRole(DEFAULT_ADMIN_ROLE)
    // {
    //     for (uint256 i = 0; i < _tokenIds.length; i++) {
    //         primarySales[_collection][_tokenIds[i]] = true;
    //     }
    // }
    ...
    function whitelistCollection(address _collection, uint256 _collectionFees)
        ...
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        // todo require that it supportsInterface? downside is those that don't won't work; add a boolean parameter to force?
    ...
    function exists(address _collection) external view returns (bool) {
        return marketplaceFees[_collection] > 0; // if more than 0 it is whitelisted
    }
        ...
        returns (bool)
    {
        return
            ...
            // todo Smart contracts MUST implement all of the functions in the ERC1155TokenReceiver interface to accept transfers. See “Safe Transfer Rules” for further detail." - https://eips.ethereum.org/EIPS/eip-1155
            //  bytes4 for `this.onERC1155Received.selector`, normally it would be `type(IERC1155Receiver).interfaceId` which includes the selector for `onERC1155BatchReceived`, but the marketplace doesn't support that function.
            //  regardless, if the marketplace calls .safeTransferFrom on any erc115 then it must implement both interfaces, and simply revert when onERC1155BatchReceived is called
            //  see example fix https://github.com/PWNFinance/TokenBundler/commit/d4ca081d4e585149c6f21556fd07f5fa683ffa68
