 ...
 */
contract UAP is Initializable, ERC20BurnableUpgradeable, AccessControlUpgradeable {
    ...
    function addMinterRole(address _newMinter) external onlyRole(DEFAULT_ADMIN_ROLE) {
        grantRole(MINTER_ROLE, _newMinter);
    }

    function removeMinterRole(address minter) external onlyRole(DEFAULT_ADMIN_ROLE) {
        revokeRole(MINTER_ROLE, minter);
    }

    function addMultipleMinters(address[] calldata newMinters) external onlyRole(DEFAULT_ADMIN_ROLE) {
    ...
    function removeMultipleMinters(address[] calldata mintersToRemove) external onlyRole(DEFAULT_ADMIN_ROLE) {
