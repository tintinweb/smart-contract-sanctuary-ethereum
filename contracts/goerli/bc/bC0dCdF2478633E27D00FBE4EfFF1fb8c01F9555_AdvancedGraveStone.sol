...
abstract contract GraveStoneAbstract is ERC721, ERC721Enumerable, ERC721URIStorage, Pausable, AccessControl {
    ...
    function safeMintWithLockBatch(address[] memory to, uint256 expTime)public onlyRole(MINTER_ROLE) returns (bool){
        require(to.length > 0);
        for (uint i = 0; i < to.length; i++) {
            _mintWithLock(to[i], expTime);
        }
        return true;
    }

    function safeMintBatch(address[] memory to)public onlyRole(MINTER_ROLE) returns (bool){
        require(to.length > 0);
        for (uint i = 0; i < to.length; i++) {
             _mint(to[i]);
        }
