...
contract ERC721r is Context, ERC165, IERC721, IERC721Metadata {
     ...
     */
    constructor(string memory name_, string memory symbol_, uint32 maxSupply_) {
        _name = name_;
        _symbol = symbol_;
...
contract Rarement is ERC721r, Ownable {
    // baseURI of metadata
    ...
    function mint (uint quantity) external payable {
        ...
        require(msg.value >= _price * quantity, "Must send enough to purchase the rarement.");
        // Don"t allow purchases before the start time
        require(_startTime < block.timestamp, "Auction hasn't started");
        // Don"t allow purchases after the end time
        require(_endTime > block.timestamp, "Auction has ended");
