...
contract Chronicles is ERC721A, ERC721AQueryable, Ownable {
    ...
    */
    ...
    struct ClbBurnPreference {
        uint16 department;        // This is set for yellow (rare) and purple (uncommon) CLBs.  See above comment for ID mapping.
        bool isRare;              // TRUE for yellow (rare) CLBs
    ...
    uint256 public tier1Price = 0.05 ether; // CLB pricing
    uint256 public tier2Price = 0.10 ether; // Public pricing
    ...
    // This is a 2-party authenticated purchase: the contract owner and minter.  Signature lasts for 30 seconds.
    // Supports 3 price tiers.
    // Purchaser pays gas fees.
    // This normally won't be called.  Most purchasers will burn the CoiwLootbox to receive the Chronicles NFT.
    function purchase(uint256 num, uint256 priceTier, uint256 _timestamp, bytes memory _signature) external payable {
        ...
        address signerOwner = signatureWallet(wallet, num, priceTier, _timestamp,  _signature);
    ...
    function mintFromBurn(address recipient, uint256 clbTokenId, bool isRare, uint16 department) external payable returns (uint256) {
        ...
        require( msg.value >= tier1Price, "Ether sent is not correct" );
    ...
    function mintFromBurnBatch(address recipient, uint256[] memory clbTokenIds, bool[] memory isRares, uint16[] memory departments) external payable returns (uint256[] memory) {
        ...
        require( msg.value >= tier1Price * clbTokenIds.length, "Ether sent is not correct" );
    ...
    function signatureWallet(address wallet, uint256 num, uint256 priceTier, uint256 timestamp, bytes memory _signature) internal pure returns (address){
        return ECDSA.recover(ethSignedMessage(keccak256(abi.encode(wallet, num, priceTier, timestamp))), _signature);
    }
    ...
    // Contract owner pays gas fee and awards the `receipient` `num` NFT(s).
    ...
    function setTierPrice(uint256 priceTier, uint256 newPrice) external onlyOwner {
        if (priceTier == 0) {
            tier0Price = newPrice;
        }
        else if (priceTier == 1) {
    ...
    function setReserved(uint256 val) external onlyOwner {
        reserved = val;
    }
