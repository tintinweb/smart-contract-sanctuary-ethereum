...
contract StakefishNFTManager is IStakefishNFTManager, ERC721Enumerable, ReentrancyGuard {
    ...
    function burn(address newManager, uint256 tokenId) external override isAuthorizedForToken(tokenId) nonReentrant {
    ...
    function multicall(uint256[] calldata tokenIds, bytes[] calldata data) external nonReentrant override returns (bytes[] memory results) {
...
interface IStakefishNFTManager is IERC721Metadata, IERC721Enumerable {
    ...
    /// @notice Burn NFT
    /// @notice burn NFT does not destroy validator contract, which freely allow
    /// them to associate to a new NFT Issuer contract for migration
    function burn(address newManager, uint256 tokenId) external;
...
import "IStakefishValidatorImmutables.sol";
...
interface IStakefishValidatorWallet {
    ...
    /// @param implementation address implementation version of the contract
...
interface IStakefishValidatorImmutables {
    function factory() external view returns (address);
}