...
contract HyperAppHtml {
    ...
    function registHTML(
        ...
        string[] calldata _navigationItems,
        string[] calldata _navigationItemLinks
    ) external payable  {
    ...
    function addNavItems(
        ...
        for (uint i = 0; i < _navigationItems.length; i++) {
            htmls[id].navigationItems.push(_navigationItems[i]);
            htmls[id].navigationItemLinks.push(_navigationItemLinks[i]);
            emit HtmlNavigationChanged(
                id,
                _navigationItems[i],
                _navigationItemLinks[i]
            );
        }
    }
    ...
    function initNavItem(uint id) external payable  {
