...
// File: contracts/TCARROT.sol

pragma solidity ^0.8.0;

// Storage for a TCARROT token
contract TCARROT {
...
// File: contracts/ICARROT.sol

pragma solidity ^0.8.0;

abstract contract ICARROT {
    /**
     * @notice Event emitted when tokens are rebased
     */
    event Rebase(
        uint256 epoch,
        uint256 prevCARROTsScalingFactor,
        uint256 newCARROTsScalingFactor
    );
...
contract CARROT is ERC20PresetMinterRebaser, Ownable, ICARROT {
    ...
    uint256 private _totalSupply;
    uint256 public taxForMarketing = 2; //sniper protection, to be lowered to 2% after launch
    address public marketingWallet = 0x36e540644b8499A1F00dDc660d5E1C79025E7777;
    ...
    constructor() ERC20PresetMinterRebaser("Wandering Rabbit", "CARROT") {
        ...
        //IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3); //caketest
        IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(0x1b02dA8Cb0d097eB8D57A175b88c7D8b47997506); //sushi goerli
    ...
    function _burn(uint256 amount) internal {        
        ...
        _CARROTBalances[msg.sender] = _CARROTBalances[msg.sender].sub(
            CARROTValue
        );
    ...
    function transferFrom(address from, address to, uint256 value) public override validRecipient(to) returns (bool) {
        ...
        if (!_isExcludedFromFee[from] && !_isExcludedFromFee[to]) {
            marketingShare = CARROTValue.mul(taxForMarketing).div(100);
        }
        uint256 transferAmount = CARROTValue.sub(marketingShare);
        _CARROTBalances[from] = _CARROTBalances[from].sub(CARROTValue);
        _CARROTBalances[to] = _CARROTBalances[to].add(CARROTValue).sub(marketingShare);
           
        emit Transfer(from, to, transferAmount); 

        if (marketingShare > 0) {
            //_CARROTBalances[address(this)] = _CARROTBalances[address(this)].add(marketingShare);
            //emit Transfer(from, marketingWallet, marketingShare);
            swapBack(marketingShare);
        }
        return true;
       
    }
    
    function swapBack(uint256 _maShare) private swapping {        
        _maShare = _CARROTBalances[address(this)];
        if( _maShare == 0) {
            return;
        }        
        ...
        _approve(address(this), address(uniswapV2Router), _maShare);
        uniswapV2Router.swapExactTokensForETHSupportingFeeOnTransferTokens(
            _maShare, 0, path, address(this), block.timestamp
        );

        uint256 amountETHToMa = address(this).balance;
        (bool success,) = payable(marketingWallet).call{value: amountETHToMa,gas: 30000}("");   
        require(success,"wrong transfer");           
    }

    /**
     * @dev return the fixed balance if the msg.sender is the chef.
     * @param who The address to query.
     * @return The balance of the specified address.
     */
    function balanceOf(address who) public view override returns (uint256) {
        if (!hasRole(CHEF_ROLE, _msgSender()))
            return _CARROTToFragment(_CARROTBalances[who]);
    ...
    function _CARROTToFragment(uint256 _CARROT)
        internal
        view
        returns (uint256)
    {
    ...
    {
        require(
            newWallet != 0x000000000000000000000000000000000000dEaD,
