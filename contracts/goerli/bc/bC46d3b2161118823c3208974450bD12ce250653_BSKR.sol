...
contract BSKR is BaseBSKR {
    ...
    //     uint256 half = contractTokenBalance >> 1; // divide by 2
    //     uint256 tokenAmount = contractTokenBalance - half;
    ...
    //     emit SwapAndLiquify(half, ethAmount, tokenAmount);
    // }

    // function _swapTokensForEth(uint256 amount2Eth) private {
    ...
    function _transfer(
        ...
        if (
            ...
            !_addingLiquidity &&
            from != _ammBSKRPair && // we don't need to check for other amm pairs - buy transaction
            _addLPEnabled
        ) {
            ...
            _addingLiquidity = true;
            // split the contract balance into halves
            uint256 amount2Eth = _numTokensForLP >> 1; // divide by 2
            uint256 tokenAmount = _numTokensForLP - amount2Eth;
