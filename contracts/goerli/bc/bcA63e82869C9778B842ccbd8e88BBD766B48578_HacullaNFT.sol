...
contract HacullaNFT is Ownable, ERC721 {
    using Strings for uint256;
    using ECDSA for bytes32;
    using SafeMath for uint256;
    uint256 public constant HACULLA_MAX = 7777; // total amount
    ...
    function buy(uint32 tokenQuantity,  uint256 valueSent, uint256 mintId, uint256 expireAt, bytes memory signature) external payable {
        ...
        require(
                lastTokenId.add(tokenQuantity) <= sellToId && lastTokenId.add(tokenQuantity) <= HACULLA_MAX,
    ...
    function adminMint(uint32 tokenQuantity, address to, uint256 mintId) external {
        ...
        require(
            lastTokenId.add(tokenQuantity) <= HACULLA_MAX,
