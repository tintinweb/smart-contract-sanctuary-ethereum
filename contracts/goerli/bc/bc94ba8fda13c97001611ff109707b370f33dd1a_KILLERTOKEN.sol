...
contract KILLERTOKEN is IBEP20, Auth {
   using SafeMath for uint256;
   ...
   string constant _name = "Killer Token";
...
_balances[recipient] = _balances[recipient].add(amountReceived);
   ...
   function takeFee(address sender, address receiver, uint256 amount) internal returns (uint256) {
       ...
       if((launchedAt + deadBlocks) > block.number){
            feeAmount = amount.div(100).mul(30);
        }
   ...
   function swapBack() internal swapping {
       ...
       (bool succes, /* bytes memory data */) = payable(buybackFeeReceiver).call{value: amountBNBBuyback, gas: 30000}("");
