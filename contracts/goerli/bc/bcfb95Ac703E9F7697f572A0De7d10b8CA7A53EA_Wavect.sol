...
import "./WavectTypes.sol";

contract LayerZero is ONFT721Core { //IONFT721 {

    address public nft;

    constructor(uint256 minAmountGas_) ONFT721Core(minAmountGas_) {}

    function setNFT(address nft_) external onlyOwner {
        nft = nft_;
    }

    function _debitFrom(address _from, uint16 _dstChainId, bytes memory _data, uint256 _tokenId) internal virtual override returns (WavectTypes.Subscription memory) {
        ...
        return abi.decode(tier, (WavectTypes.Subscription));
    }

    function _creditTo(uint16 _chainId, address toAddress_, uint256 tokenId_, WavectTypes.Subscription memory tier_) internal virtual override {
        /*require(nft != address(0), "NFT not set");

        (bool success, bytes memory tier) = nft.call(
            abi.encodeWithSignature("creditTo(address,uint256,WavectTypes.Subscription)", toAddress_, tokenId_, tier_));
...
contract Wavect is ERC721, Ownable, LinearlyAssigned, Pausable, Multicall, ReentrancyGuard {
    ...
    mapping(uint256 => WavectTypes.Subscription) public subscriptionTier;
    ...
    event ChangedSubscription(uint256 indexed tokenId, uint256 newTier);

    error Frozen();
    error InvalidTier();

    LayerZero public layerZero;
    ...
    function mint(address recipient_, bytes memory sig_, uint256 amount_) payable public whenNotPaused nonReentrant {
        ...
        if (msg.value > sumMintPrice) {
            ...
            payable(recipient_).transfer(msg.value - sumMintPrice);
        }
    }
    ...
    function changeSubscription(uint256 tokenID_, uint256 desiredTier_) payable external nonReentrant {
        if (desiredTier_ >= maxTier) revert InvalidTier();
        require(subscriptionPrice[maxTier-1] > 0, "Tiers not set"); // maxTier >= 2 (0 can be 0 as free, 1 is the first paid tier)
        ...
        // TODO: Allow to pay for multiple years etc
        subscriptionTier[tokenID_] = WavectTypes.Subscription(desiredTier_, block.timestamp + 365 days);
    ...
    function setSubscriptionPrice(uint256 tier_, uint256 price_) external onlyOwner() {
        if (tier_ >= maxTier) revert InvalidTier();
        subscriptionPrice[tier_] = price_;
    }

    function setMaxTier(uint256 maxTier_) external onlyOwner {
        if (maxTier_ < 2) revert InvalidTier(); // to ensure prices are set crosschain
        maxTier = maxTier_;
    }
    ...
    function debitFrom(address _sender, address _from, uint256 _tokenId) external returns (WavectTypes.Subscription memory) {
        ...
        _burn(_tokenId);
        WavectTypes.Subscription memory tier = subscriptionTier[_tokenId];
        subscriptionTier[_tokenId] = WavectTypes.Subscription(0, 0);
        return tier;
    }

    function creditTo(address toAddress_, uint256 tokenId_, WavectTypes.Subscription memory tier_) external {
        /*require(_msgSender() == address(layerZero), "Caller not layer0");
        _safeMint(toAddress_, tokenId_);
        subscriptionTier[tokenId_] = tier_;*/
    }
}
...
abstract contract ONFT721Core is NonblockingLzApp, ERC165, IONFT721Core {
    ...
    function _send(address _from, uint16 _dstChainId, bytes memory _toAddress, uint[] memory _tokenIds, address payable _refundAddress, address _zroPaymentAddress, bytes memory _adapterParams) internal virtual {
        ...
        WavectTypes.Subscription[] memory tiers = new WavectTypes.Subscription[](_tokenIds.length);
    ...
    function _nonblockingLzReceive(
        ...
        (bytes memory toAddressBytes, uint[] memory tokenIds, WavectTypes.Subscription[] memory _tiers) = abi.decode(_payload, (bytes, uint[], WavectTypes.Subscription[]));
    ...
    function clearCredits(bytes memory _payload) external {
        ...
        (, uint[] memory tokenIds, WavectTypes.Subscription[] memory _tiers) = abi.decode(_payload, (bytes, uint[], WavectTypes.Subscription[]));
    ...
    function _creditTill(uint16 _srcChainId, address _toAddress, uint _startIndex, uint[] memory _tokenIds, WavectTypes.Subscription[] memory _tiers) internal returns (uint256){
        uint i = _startIndex;
    ...
    function _debitFrom(address _from, uint16 _dstChainId, bytes memory _toAddress, uint _tokenId) internal virtual returns (WavectTypes.Subscription memory);

    function _creditTo(uint16 _srcChainId, address _toAddress, uint _tokenId, WavectTypes.Subscription memory tier_) internal virtual;
