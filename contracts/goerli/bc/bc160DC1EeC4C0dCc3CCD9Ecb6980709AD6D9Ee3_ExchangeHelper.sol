...
contract ExchangeHelper is Ownable, ExchangeAutoProxy, IExchangeHelper {
    ...
    // LibBp
    function bp(uint value, uint bpValue) external override pure returns (uint) {
    ...
    function fillOrder(
        ...
        uint leftOrderTakeAssetFill,
        uint rightOrderTakeAssetFill
    ) external override pure returns (LibFillTypes.FillResult memory) {
        return LibFill.fillOrder(
            leftOrder,
            rightOrder,
    ...
    // LibOrder
    function hashKey(LibOrderTypes.Order calldata order) external override pure returns (bytes32) {
        return LibOrder.hashKey(order);
    }

    function validate(LibOrderTypes.Order calldata order) external override view {
    ...
    function validateMatch(
        ...
        uint matchLeftBeforeBlock,
        uint matchRightBeforeBlock,
        bytes memory orderBookSignatureLeft,
        bytes memory orderBookSignatureRight,
        address verifyingContractProxy,
        address orderBook
    ) external override view returns (bytes32 leftOrderKeyHash, bytes32 rightOrderKeyHash) {
    ...
    function subFeeInBp(uint rest, uint total, uint feeInBp) external override pure returns (uint, uint) {
    ...
    ) external override view returns (LibPartTypes.Part[] memory) {
        return LibExchange.getRoyaltiesByAssetType(assetType, royaltiesRegistry);
    }

    // LibOrderData
    function parse(LibOrderTypes.Order memory order) external override pure returns (LibOrderDataV1Types.DataV1 memory) {
    ...
    ) external override pure returns (LibFeeSideTypes.FeeSide) {
...
interface IRoyaltiesRegistry {
    ...
    // Royalties getter
    function getRoyalties(address token, uint tokenId) external view returns (LibPartTypes.Part[] memory);
}
...
library LibOrderData {
    ...
    function parse(LibOrderTypes.Order memory order) pure internal returns (LibOrderDataV1Types.DataV1 memory dataOrder) {
    ...
    ) pure internal returns (LibOrderDataV1Types.DataV1 memory ) {
...
library LibOrder {
    ...
    bytes32 constant MATCH_ALLOWANCE_TYPEHASH = keccak256(
        'MatchAllowance(bytes32 orderKeyHash,uint256 matchBeforeBlock)'
    );
    ...
    function hash(bytes32 orderKeyHash, uint matchBeforeBlock) internal pure returns (bytes32) {
        return keccak256(abi.encode(
    ...
    function validate(LibOrderTypes.Order memory order) internal view {
        ...
        require(order.start == 0 || order.start < block.number, 'LibOrder: Order start block validation failed');
        require(order.end == 0 || order.end > block.number, 'LibOrder: Order end block validation failed');
        // Check order assets types
        // We only allow trading of ETH and some select ERC20 tokens for ERC721 and ERC1155 assets
        if (order.makeAsset.assetType.assetClass == LibAssetClasses.ETH_ASSET_CLASS
            ...
            require(
                ...
                'LibOrder: Order assets types validation failed as makeAsset is ETH or ERC20 therefore takeAsset should be ERC721 or ERC1155'
            );
        }
        if (order.takeAsset.assetType.assetClass == LibAssetClasses.ETH_ASSET_CLASS
            ...
            require(
                ...
                'LibOrder: Order assets types validation failed as takeAsset is ETH or ERC20 therefore makeAsset should be ERC721 or ERC1155'
            );
        }
        // We disallow trading of ERC721(or ERC1155) for ERC721(or ERC1155)
        if (order.makeAsset.assetType.assetClass == LibAssetClasses.ERC721_ASSET_CLASS
            || order.makeAsset.assetType.assetClass == LibAssetClasses.ERC1155_ASSET_CLASS
        ) {
            require(
                order.takeAsset.assetType.assetClass != LibAssetClasses.ERC721_ASSET_CLASS
                && order.takeAsset.assetType.assetClass != LibAssetClasses.ERC1155_ASSET_CLASS,
                'LibOrder: Order assets types validation failed as makeAsset is ERC721 or ERC1155 therefore takeAsset should NOT be ERC721 or ERC1155'
            );
        }
    }
}
...
library LibMath {
    ...
    function safeGetPartialAmountFloor(
        ...
        if (isRoundingErrorFloor(numerator, denominator, target)) {
            revert('rounding error'); // Reverts if rounding error is >= 0.1%
        }
...
library LibFillTypes {
    struct FillResult {
        uint rightOrderTakeValue;
        uint leftOrderTakeValue;
    }
}
...
library LibFill {
    ...
    function fillOrder(
        ...
        (uint leftMakeValue, uint leftTakeValue) = LibOrder.calculateRemaining(leftOrder, leftOrderTakeAssetFill);
        (uint rightMakeValue, uint rightTakeValue) = LibOrder.calculateRemaining(rightOrder, rightOrderTakeAssetFill);
    ...
    function fillRight(
        ...
        //
        uint leftTake = LibMath.safeGetPartialAmountFloor(rightTakeValue, leftMakeValue, leftTakeValue);
...
library LibFeeSide {
    ...
    function getFeeSide(
        ...
        if (makerAssetClass == LibAssetClasses.WETH_ASSET_CLASS
            || makerAssetClass == LibAssetClasses.PROXY_WETH_ASSET_CLASS
        ) {
            return LibFeeSideTypes.FeeSide.TAKE;
        }
        if (takerAssetClass == LibAssetClasses.WETH_ASSET_CLASS
...
library LibExchange {
    ...
    function validateOrder(
        ...
        if (_order.salt == 0) {
            ...
            if (_order.maker != address(0)) {
                ...
                require(
                    _callerAddress == _order.maker,
                    'LibExchange: order maker is not caller'
                );
            } else {
    ...
    function validateMatch(
        ...
        uint _matchLeftBeforeBlock,
        uint _matchRightBeforeBlock,
        ...
        if (_orderLeft.salt > 0) {
            ...
            require(_matchLeftBeforeBlock > block.number, 'LibExchange: matchLeftBeforeBlock has already been mined');
        ...
        if (_orderRight.salt > 0) {
            ...
            require(_matchRightBeforeBlock > block.number, 'LibExchange: matchRightBeforeBlock has already been mined');
            // OrderBook must be the signer
            if (
                recoverMatchAllowanceSigner(
                    _rightOrderKeyHash,
    ...
    // Helper functions
    function subFee(uint _value, uint _fee) internal pure returns (uint _newValue, uint _realFee) {
        if (_value > _fee) {
    ...
    function subFeeInBp(uint _rest, uint _total, uint _feeInBp) internal pure returns (uint _newRest, uint _realFee) {
        uint _fee = LibBp.bp(_total, _feeInBp); // Calculate fee
    ...
    ) internal pure returns (uint _total){
        _total = _amount;
    ...
    function getRoyaltiesByAssetType(
        ...
        if (
            ...
            (address token, uint tokenId) = abi.decode(_assetType.data, (address, uint));
...
library LibAssetClasses {
    ...
    bytes4 constant public WETH_ASSET_CLASS = bytes4(keccak256('WETH'));
    bytes4 constant public PROXY_WETH_ASSET_CLASS = bytes4(keccak256('PROXY_WETH'));
...
interface IExchangeHelper {
    ...
    function matchAssets(
        ...
        LibAssetTypes.AssetType memory,
        LibAssetTypes.AssetType memory
    );
    ...
    function subFeeInBp(uint _rest, uint _total, uint _feeInBp) external pure returns (uint, uint);
    ...
    function getFeeSide(
        bytes4 makerAssetClass,
        bytes4 takerAssetClass
    ) external pure returns (LibFeeSideTypes.FeeSide);
}
