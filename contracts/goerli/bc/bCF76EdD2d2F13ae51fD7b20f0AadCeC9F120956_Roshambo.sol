...
contract Roshambo is IRoshambo, ERC721, ERC721Holder, Ownable {
    ...
    /// @notice Number of blocks to submit commit and reveal
    uint256 public constant DURATION = 6969;
    ...
    constructor() payable ERC721("Roshambo", "ROSHAMBO") {
        beneficiary = owner();
    ...
    function reveal(uint256 _gameId, Choice _choice, string memory _secret) external {
        ...
        if (block.number > game.reveal) revert TimeElapsed();
        ...
        emit Reveal(_gameId, msg.sender, _choice, game.stage);
    }
    ...
    function settle(uint256 _gameId) external {
        ...
        if (
            game.stage != Stage.SETTLE ||
            (game.stage == Stage.COMMIT && block.number <= game.commit) ||
            (game.stage == Stage.REVEAL && block.number <= game.reveal)
        ) revert InvalidStage();
        ...
        if (
            ...
            _settle(game, player1, pot, fee);
            _record(player1, _gameId, true, false);
            _record(player2, _gameId, false, true);
            if (c2 != Choice.NONE) _transfer(_gameId, player1);
        } else if (
            ...
            _settle(game, player2, pot, fee);
            _record(player2, _gameId, true, false);
            _record(player1, _gameId, false, true);
            if (c1 != Choice.NONE) _transfer(_gameId, player2);
        } else {
            game.stage = Stage.COMMIT;
            game.commit = block.number + DURATION;
            p1.commitment = bytes32(0);
            p2.commitment = bytes32(0);
            p1.choice = Choice.NONE;
            p2.choice = Choice.NONE;
        }

        emit Settle(_gameId, player1, player2, c1, c2, game.stage);
    }
    ...
    function getCommit(
        ...
        if (_choice != Choice.ROCK && _choice != Choice.PAPER && _choice != Choice.SCISSORS)
    ...
    function _commit(Game storage _game, uint256 _gameId, bytes32 _commitment) internal {
        ...
        if (p1.commitment != bytes32(0) && p2.commitment != bytes32(0)) {
            ...
            _game.reveal = block.number + DURATION;
        }

        emit Commit(_gameId, msg.sender, _commitment, _game.stage);
    }

    function _settle(Game storage _game, address _player, uint256 _pot, uint256 _fee) internal {
        _game.stage = Stage.SUCCESS;
        _game.winner = _player;
        balances[_player] += (_pot - _fee);
        balances[beneficiary] += _fee;
    }

    function _record(address _player, uint256 _gameId, bool _win, bool _loss) internal {
        records.setRecord(_player, _gameId, _win, _loss);
    }

    function _transfer(uint256 _gameId, address _player) internal {
        _burn(_gameId);
        _safeMint(_player, _gameId);
    }

    function _verify(uint256 _gameId) internal view returns (Game storage game) {
    ...
    function tokenURI(uint256 _tokenId) public view virtual override returns (string memory) {
        ...
        string memory name = string.concat("Game #", _tokenId.toString());
        ...
        string memory image = Base64.encode(
            abi.encodePacked(generator.generateSVG(game.pot, p1, p2))
        );
