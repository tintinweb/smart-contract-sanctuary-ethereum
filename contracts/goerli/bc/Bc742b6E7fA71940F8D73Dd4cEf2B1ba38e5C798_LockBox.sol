 ...
 */
contract LockBox is ILockBox, AccessHandler {
    ...
    /**
     * @notice Constructor that injects tokenbank contract.
     */
    constructor() {
    ...
    function lockAmount(
        ...
        address token,
        uint256 amount
    ) public override onlyRole(LOCKBOX_ROLE) {
    ...
    function unlockAmount(
        ...
        address token,
        uint256 amount
    ) public override onlyRole(LOCKBOX_ROLE) returns (bool) {
        if (amount == 0)
            return false;
    ...
    /**
     * @notice Decreases the users locked amount and transfer tokens back.
     * @param owner The owner to update.
     * @param token The token type to unlock and transfer.
     * @param amount The amount to unlock and transfer.
     * @return bool True if the unlock succeeded, false if not.
     */
    function unlockAmountAndTransfer(
        address owner,
        address token,
        uint256 amount
    ) public override onlyRole(LOCKBOX_ROLE) returns (bool) {
    ...
    function unlockAmountTo(
        ...
        lockedTokens[owner][token] = lockedTokens[owner][token].sub(amount);
        // TODO: Check for actual token balance of the lockbox?
        uint256 allowance = IERC20(token).allowance(address(this), to);
        bool success = IERC20(token).approve(to, amount + allowance);
        console.log("Allowance (proxy):", amount + allowance / 1 ether);
    ...
    /**
     * @notice Transfer owned tokens to other. Same tokens remains locked.
     * @param to The recepient of the tokens.
     * @param token The token type to transfer.
     * @param amount The amount to transfer.
     * @return bool True if the transfer succeeded, false if not.
     */
    function transferAmountTo(
        address to,
        address token,
        uint256 amount
    ) public override onlyRole(LOCKBOX_ROLE) returns (bool) {
...
abstract contract ILockBox {
    function lockAmount(address, address, uint256) public virtual;
 ...
 * @notice This is a util contract for the BetMatchupWithLP app.
 */
