...
contract FlypeTokenSale is AccessControl, ReentrancyGuard {
        ...
        poolExist(pid)
    {
        ...
        require(msg.value >= totalPaymentInEth, "Insufficient funds sent");
        aFLYP.mintFor(_msgSender(), TotalRewards);
        if (msg.value > totalPaymentInEth)
            safeTransferETH(_msgSender(), msg.value - totalPaymentInEth);
    }
    ...
    function getTicketPriceInEth(uint256 pid)
        ...
        returns (uint256)
    {
        return poolInfo[pid].ticketPrice / getEthPriceInUSD();
    }
}
