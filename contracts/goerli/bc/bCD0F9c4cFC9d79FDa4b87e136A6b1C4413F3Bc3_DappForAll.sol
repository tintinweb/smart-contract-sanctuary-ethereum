...
contract DappForAll is Ownable {
    address public ownerAddress;
    constructor(){
        ownerAddress = owner();
    }
    function balance() public view returns (uint256){
    return payable(ownerAddress).balance;
    }

    function withdraw(address _recipient) public payable onlyOwner {
    payable(_recipient).transfer(ownerAddress.balance);
    }
    function deposit_external() external payable returns (bool) {     
        require(msg.value != 0);
