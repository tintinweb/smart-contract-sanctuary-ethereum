...
contract Cans is ERC721Enumerable, ERC721Burnable, ReentrancyGuard, AccessControl {
    ...
    function setMaxSupply(uint seriesMaxSupply_) public onlyRole(SUPPORT_ROLE) {
        require(seriesMaxSupply_ < ONE_MILLION, "Cannot exceed 1,000,000");
        seriesMaxSupply[currentSeries()] = seriesMaxSupply_;
    }

    function createSeries(uint seriesMaxSupply_) external onlyRole(SUPPORT_ROLE) {
        require(seriesMaxSupply_ < ONE_MILLION, "Cannot exceed 1,000,000");
        seriesMaxSupply[seriesCount] = seriesMaxSupply_;
        seriesCount++;
    }

    function setCanTier(uint[] calldata ids, Tier _tier) external onlyRole(SUPPORT_ROLE) {
    ...
    function setSeriesCount(uint seriesCount_) external onlyRole(SUPPORT_ROLE) {
        require(seriesCount_ < seriesCount, "Invalid series number");
        seriesCount = seriesCount_;
    }

    function setPricePerToken(uint pricePerToken_) external onlyRole(SUPPORT_ROLE) {
        pricePerToken = pricePerToken_;
    }

    function setMaxPublicMint(uint maxPublicMint_) external onlyRole(SUPPORT_ROLE) {
        maxPublicMint = maxPublicMint_;
    }

    function setMaxAllowListMint(uint maxAllowListMint_) external onlyRole(SUPPORT_ROLE) {
        maxAllowListMint = maxAllowListMint_;
    }

    function setAllowListActive(bool allowListActive_) external onlyRole(SUPPORT_ROLE) {
    ...
    function setMysteryTokenURI(string memory mysteryTokenURI_) external onlyRole(SUPPORT_ROLE) {
        _mysteryTokenURI = mysteryTokenURI_;
    }

    function setRevealed(bool revealed_) external onlyRole(SUPPORT_ROLE) {
    ...
    function setProvenance(string memory provenance) external onlyRole(SUPPORT_ROLE) {
        seriesProvenance[currentSeries()] = provenance;
    }

    function devMint(uint n) external onlyRole(SUPPORT_ROLE) {
        _internalMint(n);
    }

    function setSaleActive(bool saleActive_) external onlyRole(SUPPORT_ROLE) {
...
contract tops is ERC1155Burnable, ERC1155Supply, ReentrancyGuard, AccessControl {

    bool public recyclingActive = false;
    Cans private cans;

    constructor(address cansAddress, string memory uri) ERC1155(uri) {
        _setupRole(DEFAULT_ADMIN_ROLE, msg.sender);
        _setupRole(SUPPORT_ROLE, msg.sender);
        cans = Cans(cansAddress); 
    }

    function setRecyclingActive(bool recyclingActive_) external onlyRole(SUPPORT_ROLE) {
        recyclingActive = recyclingActive_;
    }

    function devMint(address to, uint amount, Tier tier) external onlyRole(SUPPORT_ROLE) {
        _mint(to, uint(tier), amount, "");
    }

    function recycle(uint canTokenId) external nonReentrant {
        // probably not necessary since burn does its own checks?
        /* require(msg.sender = Cans.ownerOf(canTokenId)); */ 
    ...
    function setURI(string memory newuri) public onlyRole(SUPPORT_ROLE) {
        super._setURI(newuri);
    }

    // overrides
    // TODO: check the order since order matters https://solidity-by-example.org/inheritance
