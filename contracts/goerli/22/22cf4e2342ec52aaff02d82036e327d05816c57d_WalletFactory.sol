...
contract Wallet is AccessControl, Initializable {
    ...
    // Events
    event TxExecuted(bytes32 transaction);
    ...
    // Modifiers
    modifier onlyOwner {
      require(hasRole(OWNER_ROLE, msg.sender), "Unauthorized");
      _;
    }
    ...
    function execute(Transaction calldata transaction, uint256 nonce, bytes memory signature) external {
        bytes32 txHash = keccak256(abi.encode(nonce, transaction.gasLimit, transaction.target, 
                            transaction.value, transaction.data));
        // check nonce
        require(!executed[txHash], "Transaction was already executed");
        executed[txHash] = true;

        // check signature
        require(_verify(txHash, signature), "Invalid Signature");

        bool success;
        bytes memory result;

        require(gasleft() >= transaction.gasLimit, "NOT_ENOUGH_GAS");
    ...
    // Execute (scope: public?)
    // function execute(Transaction[] calldata txns, uint256 nonce, bytes memory signature) external {
    //     bytes32 txSig = keccak256(abi.encode(nonce, signature));
    //     // check nonce
    //     require(!executed[txSig]);
    //     executed[txSig] = true;
    //     // check signature
    //     require(_verify(bytes32(txns), signature), "Invalid Signature");
