...
contract RiggedRoll is Ownable {
  ...
  function withdraw(address _addr, uint256 _amount) public onlyOwner {
    require(address(this).balance >= _amount, 'You can`t withdraw over balance');
  ...
  function riggedRoll() public {
    require(address(this).balance >= 0.002 ether, 'Account  Balance Is Low');

    bytes32 prevHash = blockhash(block.number - 1);
    bytes32 hash = keccak256(abi.encodePacked(prevHash, s_diceAddress, diceGame.nonce()));
    uint256 roll = uint256(hash) % 16;
    if (roll <= 2) {
