...
contract CrawlerPlayer is Ownable {
	...
	event Give(address indexed to, Crawl.Gem indexed gem, uint16 indexed coins);
	event Take(address indexed from, Crawl.Gem indexed gem, uint16 indexed coins);
	...
	function transferChamberHoard(address from, address to, Crawl.Hoard memory hoard) public {
		if(address(_query) != address(0) && msg.sender == address(_query.getChambersContract())) {
			// Take from
			emit Take(from, hoard.gemType, hoard.coins);
			...
			// Give to
			emit Give(from, hoard.gemType, hoard.coins);
