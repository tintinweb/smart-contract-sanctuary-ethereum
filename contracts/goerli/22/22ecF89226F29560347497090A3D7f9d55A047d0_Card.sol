...
contract Card is ERC721, ERC721Enumerable, ERC721URIStorage, ERC721Burnable, ERC2981, Pausable, Ownable, ReentrancyGuard {
    /* ==================== VARIABLES SECTION ==================== */
    /**
    ...
    */
    ...
    /**
    * @notice Single NFT Quantity
    *
    * @dev use for safeMint or single mint
    */
    uint16 constant SINGLE_QUANTITY  = 1;
    ...
    */
    ...
    /* ==================== EVENTS SECTION ==================== */
    /**
    * @dev Called in `mint()` when we minted a token
    * 
    ...
    */
    modifier isExactPayment(uint256 _tier) {
        uint256 _correctPayment = mintPrices[_tier];
    ...
    modifier isSameLength(uint256 _id, uint256 _tier) {
        ...
        require(_isCorrect == true, "ER: NSL"); // Not Same Length
        _;
    }
    ...
    */
    ...
    /**
    * @dev Reverts if the index is not found on the mintPrice's index
    */
    modifier isValidMintPriceIndex(uint256 _index) {
        require(_index >= 0 && _index <= (mintPrices.length - 1), "ER: ITP"); // Incorrect Tier Price
        _;
    }
    /* ==================== MODIFIERS SECTION ==================== */
    ...
    */
    ...
    /* ==================== PAUSABLE SECTION ==================== */
    /**
    * @dev Pause the contract. This will disallowed to execute `mintSingle()` and `mintBatch()`    
    */
    ...
    /**
    * @dev Unpause the contract. This will allow to execute `mintSingle()` and `mintBatch()`
    */
    ...
    */
    ...
    /**
    * @dev Call the `safeMint()`
    * @dev Mint single token
    *
    * @param _id token id of the token
    * @param _tier which tier they want to use
    */
    function mintSingle(uint256 _id, uint256 _tier)
        external
        payable
        whenNotPaused
        isCallerAllowed
        isMaxSupplyAllowed(SINGLE_QUANTITY)
        isValidMintPriceIndex(_tier)
        isExactPayment(_tier)     
    {
        safeMint(msg.sender, _id, _tier);
    }
    ...
    */
    function safeMint(address _to, uint256 _tokenId, uint256 _tier)
        private
    {
    ...
    */
    function safeBatchMint(address _to, uint256[] calldata _ids, uint256[] calldata _tiers)
        private
    {
        for(uint256 x; x < _ids.length; x++) {
            uint256 _id = _ids[x];
            uint256 _tier = _tiers[x];
            safeMint(_to, _id, _tier);
        }
    }
