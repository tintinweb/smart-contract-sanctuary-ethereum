...
// File contracts/TrackTheMythPass.sol
    ...
    ERC721Royalty
{
    ...
    function setBaseURI(string memory uri) external {
        require(
            ...
            "TrackTheMythPass: must have admin role to mint"
        );
    ...
    function mint(address to) public virtual {
        require(
            ...
            "TrackTheMythPass: must have minter role to mint"
        );
    ...
    function pause() public virtual {
        require(
            ...
            "TrackTheMythPass: must have pauser role to pause"
        );
        _pause();
    }
    ...
    function unpause() public virtual {
        require(
            hasRole(PAUSER_ROLE, _msgSender()),
            "TrackTheMythPass: must have pauser role to unpause"
        );
        _unpause();
    }
...
// File contracts/TrackTheMythPassSale.sol
...
contract TrackTheMythPassSale is Ownable, Pausable, ReentrancyGuard {
    ...
    uint256 private marketingMints;
    uint256 private tokensSold;

    constructor(
        TrackTheMythPass _token,
        ...
        phases[1] = PHASE(
            ...
            2,
            0.065 ether,
            phases[0].startsAt + 20 minutes,
            merkleRoots[1]
        );
        phases[2] = PHASE(2, 5, 0.095 ether, phases[1].startsAt + 20 minutes, 0);

        token = _token;

        transferOwnership(newOwner);
        royaltyWallet = newOwner;
    }

    /**
     * @dev mints a single NFT to the wallet calling this function
     * @param proof, merkleProof
     */
    function mint(bytes32[] memory proof)
        public
        payable
        whenNotPaused
        nonReentrant
    {
        require(totalSold() < saleReserve(), 'Cap for Sale reached');
        ...
        if (_currentPhase.index == 2) {
            ...
            require(
                isWhitelisted(_currentPhase.index, proof),
    ...
    /**
     * @dev mints a single NFT to the wallet calling this function
     * @param _reciever, address that will recieve the NFT
     */
    function marketingMint(address _reciever)
        public
        payable
        whenNotPaused
        nonReentrant
    ...
    {
        require(
            getMarketingMintCount() < marketingReserve(),
            "Marketing Mint Reserve reached"
        );
        marketingMints += 1;
        token.mint(_reciever);
    }
    ...
    /**
     * @dev sets wallet to be set as royalty wallet
     * @param wallet, wallet to be set as royalty wallet
     */
    ...
    /**
     * @dev sets merkle root of designated phase
     * @param _phase, phase id of which merkle root will be updated
     * @param _root, merkle root value
     */
    ...
    /**
     * @dev returns the current phase of sale
     */
    ...
    function marketingReserve() public view returns (uint256) {
        return ((token.cap() * 5) / 100);
    }

    function getMarketingMintCount() public view returns (uint256) {
        return marketingMints;
    }

    function saleReserve() public view returns (uint256) {
        return token.cap() - marketingReserve();
    }
    ...
    function isWhitelisted(uint8 phase, bytes32[] memory proof)
        public
        view
        returns (bool)
    {
    ...
    function _mint(address receiver, PHASE memory _currentPhase)
        internal
        whenNotPaused
    {
        ...
        require(
            totalMinted[receiver][_currentPhase.index] <
                _currentPhase.maxAllowedMint,
            "Wallet mint quota of this phase reached"
        );
    ...
    function _transferFunds() internal {
        Address.sendValue(payable(royaltyWallet), msg.value);
    }
