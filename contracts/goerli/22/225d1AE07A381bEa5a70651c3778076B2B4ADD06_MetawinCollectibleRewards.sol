...
import "./lib/env/Roles.sol";
import "./lib/interfaces/IMetawinCollectibleRewards.sol";

/**
 * @dev Metawin {ERC1155} collectibles.
 */
contract MetawinCollectibleRewards is
    Context,
    Roles,
    ERC1155,
    IMetawinCollectibleRewards
{
    // uint to string conversion
    using Strings for uint256;

    // Locks baseUri when set to true
    ...
    /**
     * @dev Disables functions when {_baseUri_frozen} is set to true.
     */
    modifier onlyIfBaseUriNotFrozen() {
        require(!_baseUri_frozen, "Base URI is frozen");
        _;
    }

    /**
     * @dev Returns the URI by concatenating baseUri and a id.
     */
        ...
        returns (string memory)
    {
        ...
        return
            string(
                abi.encodePacked(baseUri, id.toString())
            );
    }

    /**
     * @dev [Admin] Set the base URI (if not frozen).
     */
    function setBaseUri(string calldata _baseUri)
        external
        onlyRole(METAWIN_ROLE)
        onlyIfBaseUriNotFrozen
    {
        _setURI(_baseUri);
    }

    /**
     * @dev [Admin] Freeze the base URI.
     */
    function freezeBaseUri()
        ...
        onlyIfBaseUriNotFrozen
    {
        _baseUri_frozen = true;
    }

    /**
     * @dev Creates `amount` new tokens for `to`, of token type `typeId`.
     * See {ERC1155-_mint}.
     * Requirements:
     * - the caller must have the `MINTER_SALE_ROLE`.
     */
    function mint(
        address to,
        uint256 typeId,
        uint256 amount
    ) external virtual onlyRole(MINTER_SALE_ROLE) {
        _mint(to, typeId, amount, "");
    }

    /**
     * @dev Batch-operations[Batched] variant of {mint}.
     */
    function mintBatch(
        address to,
    ...
    /**
     * @dev Check {ERC1155-balanceOf}.
     */
    function balanceOf(address account, uint256 typeId)
        public
        view
        virtual
        override(ERC1155, IMetawinCollectibleRewards)
        returns (uint256)
    {
        return ERC1155.balanceOf(account, typeId);
    }

    /**
     * @dev Check {balanceOf} and {ERC1155-balanceOfBatch}.
     */
    function balanceOfBatch(address[] memory accounts, uint256[] memory typeIds)
        public
        view
        virtual
        override(ERC1155, IMetawinCollectibleRewards)
    ...
    {
        return ERC1155.balanceOfBatch(accounts, typeIds);
    }
