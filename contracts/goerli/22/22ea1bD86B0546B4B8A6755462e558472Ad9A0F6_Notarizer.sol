...
contract Notarizer is Ownable {
    ...
    modifier onlyFeeAddress() {
        require(msg.sender == feeAddress, 'This address is not allowed');
        _;
    }
    ...
    function notarizeCID(
        ...
        string memory _tag
    ) public payable onlyWhitelistedNotaries onlyValidHashed(_ipfsHash) {
        // Notarization logic
    ...
    function notarizeBatch(
        ...
        string memory _tag
    ) public payable onlyWhitelistedNotaries {
    ...
    function setFeeAddress(address newFeeAddress) external {
        require(feeAddress == address(0) || msg.sender == feeAddress, 'Only feeAddress can set a new one');
        feeAddress = newFeeAddress;
    }

    function withdrawAllCollectedFees() external onlyFeeAddress {
        (bool sent, ) = feeAddress.call{value: (address(this).balance)}(
            ""
        );
