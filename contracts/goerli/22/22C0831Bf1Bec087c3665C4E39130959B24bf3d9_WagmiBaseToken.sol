...
import "./WagmiMerchClaimToken.sol";
...
contract WagmiBaseToken is ERC721A, VRFConsumerBaseV2, ContractMetadata {
    ...
    uint private _decayLimit = 3;

    // Decay probability
    uint256 private _decayProbability = 10;

    // map tokenIds to decay state
    mapping(uint256 => bool) private _decayInProgress;
    ...
    WagmiMerchClaimToken public merchClaimToken;
    ...
    constructor(
        ...
        address erc1155
    ) ERC721A(_name, _symbol) VRFConsumerBaseV2(vrfCoordinator) {
        COORDINATOR = VRFCoordinatorV2Interface(vrfCoordinator);
        s_owner = msg.sender;
        s_subscriptionId = _subscriptionId;
        deployer = msg.sender;
        src = IERC1155(erc1155);
        merchClaimToken = new WagmiMerchClaimToken("Wagmi Token", "WGMI", address(this));
    }
    ...
    function setDecayLimit(uint decayLimit) public onlyOwner {
    ...
    function burnAndMint(uint256 id, uint256 value) external {
        ...
        uint256 quantity = src.balanceOf(msg.sender, id);
        require(quantity >= value, "Insufficeint tokens.");

        // 1155 Burn value of Token id
        src.burn(msg.sender, id, value);
        ...
        returns (string memory)
    {
        ...
        uint256 decay = _decay[tokenId];
        string memory base = _baseURI();

        // If there is no base URI, return the token URI and decay.
        if (bytes(base).length == 0) {
            return string(abi.encodePacked(_tokenURI, decay));
        }
        // If both are set, concatenate the baseURI, tokenURI, and decay (via abi.encodePacked).
        if (bytes(_tokenURI).length > 0) {
            return string(abi.encodePacked(base, _tokenURI, decay));
        }
        ...
        returns (string memory, string memory, uint256)
    {
        ...
        return (_tokenURI, _base, decay);
    }

    function claimMerch(uint256 tokenId) public {
        _burn(tokenId);
        merchClaimToken.claimMint(msg.sender, 1);
    }
        ...
        virtual
    {
        require(
            _exists(tokenId),
            "WagmiUnitedToken: URI set of nonexistent token"
        );
    ...
    function _burn(uint256 tokenId) internal virtual override {
        ...
        delete _decay[tokenId];
        delete _decayInProgress[tokenId];
    }

    function _decayToken(uint256 tokenId) private returns (uint256 requestId) {
        require(!_decayInProgress[tokenId], "Decay in progress");
        ...
        override
    {
        ...
        bool shouldDecay = value < _decayProbability;
        if (shouldDecay) {
            uint256 decayAmount = 1;
            if (_decay[tokenId] == 0) {
                //Initialize value
                _decay[tokenId] = 10;
            } else if (_decay[tokenId] == _decayLimit || _decay[tokenId] == 1) {
                decayAmount = 0;
            }
            _decay[tokenId] = _decay[tokenId] - decayAmount;
        }
        _decayInProgress[tokenId] = false;
    }
...
contract WagmiMerchClaimToken is ERC721A {
    address _minter;
    ...
    function claimMint(address to, uint256 quantity) public {
        require(msg.sender == _minter, "Sender can not mint tokens");
