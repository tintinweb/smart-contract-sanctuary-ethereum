...
contract WillFactory is AccessControl{
    ...
    // show [0]
    function getInformationsWill() public view returns(string []memory, string  []memory,string []memory,address []memory ){
        uint256 i = 0;
        string []memory names;
        string []memory descs;
        string []memory ipfss;
        address []memory willsAddress;
        uint256 length = tokenIdOwner[msg.sender].length;
        for(i = 0;i<length ; i++){
            uint256 _tokenId = tokenIdOwner[msg.sender][i];
       ...
       return (names,descs , ipfss, willsAddress);
    }
