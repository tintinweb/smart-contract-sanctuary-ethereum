...
contract KiKiStakingHelper is IKiKiStakingHelper, RoleManager {
    ...
    /// @dev Last update APR total swap fee
    bytes32 internal constant TOTAL_SWAP_FEE_POSITION = keccak256("KKS.KKSH.totalSwapFee");
        ...
        onlyMainManager
    {
        ...
        _setAPR(defaultAPR);
        _setLastAPRTimestamp(timestamp);
        initialized();
    }
    ...
    function updateAPR(uint256 rewards, uint256 timestamp) public onlyKiKiStaking {
        IKiKiStaking kikiStaking = IKiKiStaking(KIKISTAKING_POSITION.getStorageAddress());
        uint256 totalSwapFee = kikiStaking.getTotalSwapFee();
        uint256 timeElapsed = timestamp.sub(_getLastBlockTime());
        uint256 increaseRewards = rewards.add(totalSwapFee.sub(_getLastTotalSwapFee()));
        uint256 _APR = getNewAPR(increaseRewards, timeElapsed);
        _setAPR(_APR);
        _setLastTotalSwapFee(totalSwapFee);
        _setLastAPRTimestamp(timestamp);
    }
    ...
    */
    ...
    function getNewAPR(uint256 increaseRewards, uint256 timeElapsed) public view returns (uint256) {
        IKiKiStaking kikiStaking = IKiKiStaking(KIKISTAKING_POSITION.getStorageAddress());
        uint256 currentTotalPooledEther = kikiStaking.getTotalPooledEther();
        uint16 feePoints = kikiStaking.getFee();
        return increaseRewards.mul(365 days).mul(10000-feePoints).div(currentTotalPooledEther.sub(increaseRewards)).div(timeElapsed);
    }
    ...
    function _setLastAPRTimestamp(uint256 timestamp) internal {
