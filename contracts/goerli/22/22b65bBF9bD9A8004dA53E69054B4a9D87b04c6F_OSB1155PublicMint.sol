...
interface IOSBFactory {
    function create(bool _isSingle, address _owner, address _controller, string memory _uri, string memory _name, string memory _symbol, address _defaultReceiverRoyalty, uint96 _defaultPercentageRoyalty) external returns (TokenInfo memory);
}
