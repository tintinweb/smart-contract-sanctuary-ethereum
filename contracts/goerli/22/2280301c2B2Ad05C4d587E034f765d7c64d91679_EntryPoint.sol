...
contract EntryPoint {
    ...
    function handleOps(UserOperation[] calldata userOps) external returns(string memory destination, uint amount){
        ...
        for (uint i = 0; i < userOps.length; i++) {
            ...
            try IAAWallet(sender).executionFromEntryPoint(callData) returns(string memory _destination, uint _amount) {
...
interface IAAWallet {
    ...
    function executionFromEntryPoint(bytes memory callData) external returns(string memory destination, uint amount);
