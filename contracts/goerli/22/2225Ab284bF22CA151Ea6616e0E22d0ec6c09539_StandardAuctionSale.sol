...
error DropEnded(uint128 dropId);
error DropNotEnded(uint128 dropId);
error NoBid();
    ...
    UUPSUpgradeable
{
    ...
    event StandardAuctionDropCancelled(uint128 indexed dropId);
    ...
    /// @param dropId The id of the drop.
    function remainingItems(uint128 dropId) public view returns (uint32) {
        StandardAuctionDrop memory stdAuction = StandardAuctionSaleStorage
            .layout()
            .drops[dropId];
    ...
    /// @param dropId The id of the drop.
    function highestBidder(uint128 dropId, uint128 itemId)
        public
        view
        returns (address)
    {
    ...
    /// @param dropId The id of the drop.
    function highestBid(uint128 dropId, uint128 itemId)
        public
        view
        returns (uint256)
    {
    ...
    /// @notice returns the auction's highest bid in wei
    /// @param dropId The id of the drop.
    function minimumBid(uint128 dropId, uint128 itemId)
        public
        view
        returns (uint256)
    {
    ...
    function getBid(uint128 dropId, uint128 itemId)
        public
        view
        returns (
            address,
    ...
    function isItemInDrop(uint128 dropId, uint128 itemId)
        internal
        view
        returns (bool)
    {
        uint128[] memory itemIds = StandardAuctionSaleStorage
            .layout()
            .drops[dropId]
            .itemIds;
    ...
    function dropEnded(uint128 dropId) internal view returns (bool) {
        StandardAuctionDrop memory stdAuction = StandardAuctionSaleStorage
            .layout()
            .drops[dropId];
    ...
    function dropExists(uint128 dropId) internal view returns (bool) {
        return
    ...
    /// @param dropId The id of the drop.
    function cancelDrop(uint128 dropId)
        public
        ...
        notPaused
    {
        ...
        for (uint16 i; i < stdAuction.itemIds.length; i++) {
            uint128 itemId = stdAuction.itemIds[i];
...
interface IStandardAuctionSale {
    ...
    function cancelDrop(uint128 dropId) external;
    ...
    function remainingItems(uint128 dropId) external view returns (uint32);
...
// Drop Errors
error DropCancelled(uint128 dropId);
error DropNotFound(uint128 dropId);
error DropNotStarted(uint128 dropId);
error DropSoldOut(uint128 dropId);
error DropStarted(uint128 dropId);
error DuplicateDrop(uint128 dropId);
error ItemsRequired();
error InvalidAddress();
error InvalidDropType();
error InvalidDropStart();
error ItemNotFound(uint128 dropId, uint128 itemId);
error ItemMinted(uint128 itemId);
...
library StandardAuctionSaleStorage {
    struct Layout {
        ...
        mapping(uint128 => StandardAuctionDrop) drops; // dropId --> drop
        mapping(uint128 => mapping(uint128 => StandardAuctionItemBid)) bids; // dropId --> itemId --> bid
    }
