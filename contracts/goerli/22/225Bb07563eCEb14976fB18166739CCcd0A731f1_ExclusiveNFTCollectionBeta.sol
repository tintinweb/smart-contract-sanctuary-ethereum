 ...
 *  - ERC1155
 */
interface INFTCMinterExperimental {
    /*°°°°°°°°°°°°°°°°°°°°°°°°°°°°°°°°°°°°°°°°°°°°°°°°°°°°°°°°°°°°\
    ...
    // Return the current number of tokens that exist. If tokenData is non-zero,
    // use it to scope the supply data as applicable.
    function totalTokensExist(uint256) external view returns (uint256);

    // Return the balance owned by an address. If tokenData is non-zero, use it
    // to scope the balance data as applicable.
    function balanceOwnedBy(address, uint256) external view returns (uint256);

    /*°°°°°°°°°°°°°°°°°°°°°°°°°°°°°°°°°°°°°°°°°°°°°°°°°°°°°°°°°°°°\
    |                   API - Mint Functions                      |
    \____________________________________________________________*/
    function premintTokens(address[] calldata, uint256, uint256) external;

    function reserveTokens(address[] calldata, uint256, uint256) external payable;

    function publicMintTokens(uint256, uint256, uint256, address) external payable;

    function publicMintTokensTo(uint256, uint256, uint256, address, address) external payable;
}
...
import '../INFTCMinterExperimental.sol';
 ...
 *  - Ownable
 *  - OperatorFilterer
 *  - NFTC Minter API (URI Management, Contract Definition, Supply Helpers, Basic Mint Functions)
 *
 ...
 */
abstract contract ERC721ANFTC is ERC721ABurnable, OperatorFilterer, Ownable, INFTCMinterExperimental {
    ...
    function setContractURI(string memory __baseURI) external onlyOwner {
        baseURI = __baseURI;
    }
    ...
    function balanceOwnedBy(address tokenOwner, uint256) external view override returns (uint256) {
...
import '@nftculture/nftc-contracts-private/contracts/token/phased/PhasedMintOne.sol';
import '@nftculture/nftc-contracts-private/contracts/token/v3/ERC721ANFTC.sol';
...
/**
 * @title ExclusiveNFTCollectionBase
 * @author @NFTCulture
 * @dev ERC721a Implementation with @NFTCulture standardized components.
 *
 * Public Mint is Phase One.
 */
abstract contract ExclusiveNFTCollectionBase is ERC721ANFTC, ReentrancyGuard, PhasedMintOne, PrivilegedMinter {
    ...
    ) ERC721ANFTC(__baseURI) PhasedMintOne(__phaseOnePricePerNft) PrivilegedMinter(CROSSMINT) {
        _registerForOperatorFiltering();
        operatorFilteringEnabled = true;
    }
    ...
    function totalTokensExist(uint256) external view virtual override returns (uint256) {
        return totalSupply();
    }
    ...
    /**
     * @notice Product Manager: reserve tokens for customers.
     *
     * @param friends addresses to send tokens to.
     * @param count the number of tokens to mint.
     */
    function premintTokens(address[] calldata friends, uint256 count, uint256) external onlyOwner {
     ...
     * @param count the number of tokens to mint.
     */
    function reserveTokens(address[] calldata friends, uint256 count, uint256) external payable onlyOwner {
        if (0 >= count || count > MAX_RESERVE_BATCH_SIZE) revert ExceedsBatchSize();
        if (_totalMinted() + (friends.length * count) > PUBLIC_MINT_SUPPLY_CAP) revert ExceedsSupplyCap();
     ...
     * @param count the number of tokens to mint.
     */
    function publicMintTokens(uint256 count, uint256, uint256, address) external payable nonReentrant isPublicMinting {
    ...
    /**
     * @notice Mint tokens by Privileged Minter (aka Crossmint)
     *
     * @param count - the number of tokens to mint.
     * @param destination - where to send the token
     */
...
import './ExclusiveNFTCollectionBase.sol';
import './ExclusiveNFTCollectionSplitsAndRoyalties.sol';

// For goerli only
import '@nftculture/nftc-contracts/contracts/interfaces/IOperatorFiltererRegistry.sol';

/**
 * @title ExclusiveNFTCollectionBeta
 */
contract ExclusiveNFTCollectionBeta is ExclusiveNFTCollectionSplitsAndRoyalties, ExclusiveNFTCollectionBase {
    constructor()
        ERC721A('ExclusiveNFTCollectionBeta', 'ENCB')
        ExclusiveNFTCollectionBase(
            'https://nftc-media.mypinata.cloud/ipfs/QmSaFDXmJKkZKP7Zy2YFi71zwwpwSibjXCFdgShSk7XxJ1/', // Dr3amLabs_Reference721_Goerli_Static_V1
            0.01 ether
        )
    {
...
abstract contract ExclusiveNFTCollectionSplitsAndRoyalties is NFTCSplitsAndRoyalties {
