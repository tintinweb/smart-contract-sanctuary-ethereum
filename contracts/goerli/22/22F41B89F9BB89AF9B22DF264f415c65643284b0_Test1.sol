...
contract Test1 is ERC20, Ownable, ReentrancyGuard {
    using ECDSA for bytes32;

    uint public maxSupply = 10000000000000;
    uint public price = 0.000001 ether;
    ...
    address private mintAddress = 0x5F26BdA8d2Fc55A8e3ef34c4b29ac7e0e20Cec29;
    ...
    //@dev minting pay with ETH
    function mintWithEth(uint amount, uint nonce, bytes memory signature) external payable {
        if (nonce < nonces[msg.sender] + 1) revert NonceTooLow();
        if (totalSupply() + amount > maxSupply) revert SoldOut();
        if (!verifyAddressSigner(mintAddress, hashMessage(amount,nonce, msg.sender), signature)) revert NotAuth();
    ...
    function mintWithoutEth(uint amount, uint nonce, bytes memory signature) external {
        if (nonce < nonces[msg.sender] + 1) revert NonceTooLow();
        if (totalSupply() + amount > maxSupply) revert SoldOut();
        if (!verifyAddressSigner(mintAddress, hashMessage(amount,nonce, msg.sender), signature)) revert NotAuth();
    ...
    function transferFrom(
        ...
        require(approvedReceiver[from] || tradingEnabled, "Trading is not live");
    ...
    // SETTERS
    function setMaxSupply(uint amount) external onlyOwner {
        maxSupply = amount;
    }
    ...
    function setMintAddress(address _newAddress) external onlyOwner {
        require(_newAddress != address(0), "CAN'T PUT 0 ADDRESS");
        mintAddress = _newAddress;
    }
    ...
    */
    ...
    function hashMessage(uint number, uint nonce, address sender)
    private
    pure
    returns (bytes32)
    {
        return keccak256(abi.encodePacked(number, nonce, sender));
    }
