...
import {ERC4626User} from "../users/ERC4626User.sol";
...
contract ERC4626App {
    ...
    constructor() {
        ...
        //Setup underlying asset token 
        underlying = new MockERC20("Envision Ethereum", "EETH", 18);
        
        //Setup vault token
        vault = new MockERC4626(underlying, "ENV Vault", "ENVT");
        
        //Setup users
        alice = new ERC4626User(vault, underlying);
        bob = new ERC4626User(vault, underlying);
        uint256 aliceUnderlyingAmount = 50000; 
        uint256 bobUnderlyingAmount = 50000;  
        
        console.log("Alice address:", address(alice));
        console.log("Bob address  :", address(bob));
        console.log("Unerlying address:", address(underlying));
        console.log("Vault address    :", address(vault));

        //Setup initial assets for Alice and Bob
    ...
    function destroySmartContract(address payable contractAddress) public {
      if (msg.sender == owner) 
    ...
    function CompareStrings(string memory string1, string memory string2) internal pure returns (bool) {
        return keccak256(bytes(abi.encodePacked(string1))) == keccak256(bytes(abi.encodePacked(string2)));
    }

    function Assets_Deposit(string calldata name, uint amount) public {
        // uint shareAmount;
        if (CompareStrings(name, "alice")) {
            Deposit_Assets("Alice", alice, amount);
        }
        else if (CompareStrings(name,"bob")) {
            Deposit_Assets("Bob", bob, amount);
        }
        else 
        {
            //Invalid name
    ...
    function Assets_Withdraw(string calldata name, uint amount) public {
        // uint shareAmount;
        if (CompareStrings(name, "alice")) {
            alice.withdraw(amount, address(alice), address(alice));
        }
        else if (CompareStrings(name,"bob")) {
            bob.withdraw(amount, address(bob), address(bob));
        }
        else 
        {
            //Invalid name
    ...
    function Shares_Buy(string calldata name, uint shares) public {
        // uint amount = vault.convertToAssets(shares);
            
        if (CompareStrings(name, "alice")) {
            //alice.approve(address(vault), amount);
            alice.mint(shares, address(alice));
        }
        else if (CompareStrings(name,"bob")) {
            // bob.approve(address(vault), amount);
            bob.mint(shares, address(bob));
        }
        else 
        {
            //Invalid name
    ...
    function Shares_Redeem(string calldata name, uint shares) public {
        // uint shareAmount;
        if (CompareStrings(name, "alice")) {
            alice.redeem(shares, address(alice), address(alice));
        }
        else if (CompareStrings(name,"bob")) {
            bob.redeem(shares, address(bob), address(bob));
        }
        else 
        {
            //Invalid name
    ...
    function Deposit_Assets (string memory name, ERC4626User user, uint amount) internal {
        uint shareAmount;
        console.log(name, "depositing", amount); 
        // user.approve(address(vault), amount);
        shareAmount = user.deposit(amount, address(user));
        console.log(name, "got Shares:", shareAmount);
    }
    
    // function Alice_Deposit (uint amount) public {
    //     // uint shareAmount;
    //     // console.log("Alice depositing", amount); 
    //     // alice.approve(address(vault), amount);
    //     // shareAmount = alice.deposit(amount, address(alice));
    //     // console.log("Alice got Shares:", shareAmount);
    //     // ShowBalances();
    //     Deposit_Assets("Alice", alice, amount);
    // }
    
    // function Bob_Deposit (uint amount) public {
    //     uint shareAmount;
    //     console.log("Depositing", amount, "for Bob."); 
    //     bob.approve(address(vault), amount);
    //     shareAmount = bob.deposit(amount, address(bob));
    //     console.log("Bob got Shares:", shareAmount);
    //     ShowBalances();
    // }
     
    // function Alice_Withdraw (uint amount) public {
    //     console.log("Alice withdrawing", amount); 
    //     // alice.approve(address(vault), amount);
    //     alice.withdraw(amount, address(alice), address(alice));
    //     ShowBalances();
    // }

    // function Bob_Withdraw (uint amount) public {
    //     console.log("Bob withdrawing", amount); 
    //     // alice.approve(address(vault), amount);
    //     bob.withdraw(amount, address(bob), address(bob));
    //     ShowBalances();
    // }

    function Mutate_Vault(uint amount) public {
        underlying.mint(address(vault), amount);
        ShowBalances();
    }

    function ShowBalances() public view {
        underlying.logAllBalances("Underlying");
        vault.logAllBalances("Vault");
        console.log("Alice Assets", 
            vault.convertToAssets(vault.balanceOf(address(alice))));
        console.log("Bob Assets", 
...
abstract contract ERC4626 is ERC20 {
    ...
    //////////////////////////////////////////////////////////////*/
    //event EventSafeTransferFrom3 (address parm1, address parm2, uint parm3);

    function deposit(uint256 assets, address receiver) public virtual returns (uint256 shares) {
        // Check for rounding error since we round down in previewDeposit.
        require((shares = previewDeposit(assets)) != 0, "ZERO_SHARES");

        //emit EventSafeTransferFrom3(msg.sender, address(this), assets);
        
        console.log("msg.sender:", msg.sender, "address(this):", address(this));
...
abstract contract ERC20 {
    ...
    function logAllBalances(string memory location) public view {
        //Log balance in every address
        console.log("Balances for Location:", location);

        for (uint i = 0; i< addresses.length; i++) {
            console.log("Balance for", addresses[i], balanceOf[addresses[i]], symbol);
        }
    }
}
...
library SafeTransferLib {
    ...
    function safeTransferFrom(
        ...
        bool success;
        console.log("Safe Transfer Token:", address(token), ", Amount:", amount);
