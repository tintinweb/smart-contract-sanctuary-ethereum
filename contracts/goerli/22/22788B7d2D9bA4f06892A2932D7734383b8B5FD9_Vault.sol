...
contract Vault is Ownable, ReentrancyGuard{
    ...
    function returnNfts(uint256[] memory _tokenIds) external nonReentrant() {
        ...
        if (_tokenIds.length == 1) {
            ...
            for (uint i = 0; i <= 1; i++) {
                ...
                uint256 tokenPrice = Collection(s_collectionContract).getTokenIdToPrice(_id);
    ...
    function getCollectionContract() view public returns (address) {
