...
import {ShipModule2} from "szns/zodiac/modules/ShipModule2.sol";
 ...
 */
contract ShipDeployer2 is Ownable {
    address constant GNOSIS_SAFE_FALLBACK_HANDLER =
        0xf48f2B2d2a534e402487b3ee7C18c33Aec0Fe5e4;
    address constant GNOSIS_SAFE_PROXY_FACTORY =
        0xC22834581EbC8527d974F8a1c97E1bEA4EF910BC;
    address constant GNOSIS_SAFE_TEMPLATE_ADDRESS =
    ...
    function createAndSetup(
        ...
        ShipModule2 module = new ShipModule2();

        // initialze safe and add buy module
        address[] memory owners = new address[](1);
        owners[0] = msg.sender;

        GnosisSafe(payable(safeAddress)).setup(
            owners, // owners
            1, // threshold
            address(this), // to
            abi.encodeCall(this.initSafe, (address(module))), // data
...
import {Order, ConsiderationInterface} from "seaport/interfaces/ConsiderationInterface.sol";
...
import {OrderComponents, ConsiderationItem, ItemType} from "seaport/lib/ConsiderationStructs.sol";

contract ShipModule2 is Module {
    address constant SEAPORT = 0x00000000000001ad428e4906aE43D8F9852d0dD6;
    SignMessageLib constant GNOSIS_SIGN_MSG_LIB =
     ...
     */
    function buy(Order calldata seaportOrder) public {
        bytes memory callData = abi.encodeCall(
            ConsiderationInterface(SEAPORT).fulfillOrder,
    ...
    function getListData(bytes calldata _data) external returns (bytes memory) {
        bytes memory callData = abi.encodeCall(
            GNOSIS_SIGN_MSG_LIB.signMessage,
            (_data)
        );
