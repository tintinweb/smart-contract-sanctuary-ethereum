...
contract Engine is ERC721Upgradeable, IEngineEvents {
    ...
    Order[] public opens; // Trader open orders queue
    ...
    // Open Interest Cap
    uint256 public totalOpenedAmount;
    uint256 public openInterestCap; // In % [6 decimal]
    ...
    uint256 public minMakerFee; // minimum fee protocol collects from maker
    uint256 public premiumFeePerc; // % of the premium that protocol collects
    ...
    function initialize(
        ...
        decimals.amount = 18; // TODO: amount is too abstract to be a decimal spec
        decimals.fr = 18;
        decimals.premium = 18;
        decimals.percent = 6;
        ...
        premiumFeePerc = 5e3; // default value
        minMakerFee = 1 * decimals.amount; // 1 USDC
        ...
        traderFeesPerc = 5e3; // = 0.5%, once 5000/1000000=0.005
    ...
    function setOpenInterestCap(uint256 x) external {
        openInterestCap = x;
    }
    ...
    function setMinMakerFeeUSDC(uint256 x) external {
        minMakerFee = x;
    }

    function setPremiumFeePerc(uint256 x) external {
        premiumFeePerc = x;
    }
    ...
    function open(
        ...
        m.amount = amount;
        m.premium = premium; // TODO: rename to premiumPerc  [also in PnLMath.sol]
    ...
    function decreaseCollateral(uint256 matchId, uint256 amount) external {
        ...
        uint256 makerRealizedPnL = m.accruedFR(priceData, decimals);
        // increase maker collateral
        if (msg.sender == _ownerOf(m.maker)) {
    ...
    function join(
        ...
        // --- Trader Fees ---
        uint256 traderFees = order.traderFees(
            priceData,
            decimals,
            traderFeesPerc
        );

        uint256 makerFees = order.makerFees(
            priceData,
            decimals,
            m.premium,
            premiumFeePerc
        ...
        int256 premium = order.computePremium(priceData, decimals, m.premium);

        if (premium > 0) {
            vault.lock(uint256(premium), order.owner);
            vault.unlock(uint256(premium), maker);
            order.collateral -= uint256(premium);
            m.collateralM += uint256(premium);
        } else {
            vault.unlock(uint256(-premium), order.owner);
            vault.lock(uint256(-premium), maker);
            order.collateral += uint256(-premium);
            m.collateralM -= uint256(-premium);
        }

        if (order.collateral <= traderFees) {
    ...
    function unjoin(uint256 matchId, PriceData memory priceData) internal {
        ...
        _settlePnL(maker, true, int256(m.collateralM) + PnL);
        _settlePnL(trader, false, int256(m.collateralT) - PnL);
        totalOpenedAmount -= m.amount;
    ...
    //     makerRealizedPnL = m.accruedFR(priceData, decimals);
    // }

    function isLiquidatable(uint256 matchId, uint256 tokenId)
        public
        view
        returns (bool res, int256 _uPnL)
    {
        Match storage m = matches[matchId];
        require(
            (tokenId == m.maker) || (tokenId == m.trader),
        ...
        PriceData memory priceData = oracle.getPrice();
        _uPnL = m.pnl(tokenId, priceData.timestamp, priceData.price, decimals);
        ...
        if (tokenId == m.maker) {
            ...
            res =
                LeverageMath.getLeverage(
                    ...
                    m.collateralM,
                    priceData.price,
                    int256(m.accruedFR(priceData, decimals)),
                    decimals
                ) >=
                leverage.maxLeverageOngoing;
        }
    ...
    function pnl(
        ...
        return matches[matchId].pnl(tokenId, timestamp, exitPrice, decimals);
    }
    ...
    function liquidate(
        ...
        for (uint256 i = 0; i < matchIds.length; i++) {
            ...
            if (tokenId == m.maker) {
                ...
                address trader = _ownerOf(tokenId);
                _settlePnL(trader, false, _uPnL);
                // a fix fee of 1$ is always paid off to the liquidator
                uint256 fee = 1 * 10**decimals.collateral;
                // take 0.01% of the collateral as fee
                fee += m.collateralT / 10000;
                int256 remainingCollateral = int256(m.collateralT) + _uPnL;
                if (fee < uint256(remainingCollateral)) {
                    ...
                    // fees is above remaining collateral, the pool pay the liquidator fees and the missing collateral
                    lpPoolMakeUpTrader += uint256(
                        int256(fee) - remainingCollateral
                    );
                    vault.transfer(fee, recipient);
                }
                // settle profitable pnl for the maker
                vault.assign(
                    uint256(int256(m.collateralM) - _uPnL),
                    _ownerOf(m.maker)
                );
                totalOpenedAmount -= m.amount;
                emit MatchLiquidated(matchId, -_uPnL, oracle.getPrice().price);
                delete matches[matchId].maker;
            }
        }
    }
    ...
    function runOpens(uint256 n) external {
        ...
        while (openOrderToExecute < n) {
            ...
            uint256 poolBalance = vault.token().balanceOf(address(pool));
            if (
                poolBalance == 0 ||
                (((totalOpenedAmount * priceData.price) / 10**decimals.amount) *
                    10**decimals.percent) /
                    poolBalance >
                openInterestCap
            ) break;
            join(order, priceData, openOrderToExecute);
            totalOpenedAmount += order.amount;
