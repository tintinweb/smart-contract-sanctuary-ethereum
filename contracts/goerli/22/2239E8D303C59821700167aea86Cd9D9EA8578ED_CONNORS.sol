...
contract CONNORS is ERC721, DefaultOperatorFilterer, Ownable {
    ...
    uint256 public whitelistMintPrice = 0.069 ether;
    ...
    constructor() ERC721("The Connors", "CNRS") {
        //initial mint to deployer to create collection on marketplaces
        ++mintedByAddressPublic[msg.sender];
        ++amountMinted;
    ...
    /// @dev if one-stop mint is needed, just provide placeholder proof and leaf values
    function mint(bytes32[] memory _proof, bytes32 _leaf) external payable {
        if(whitelistMintIsOpen) { 
            mintWhitelist(_proof, _leaf); 
        } else if(publicMintIsOpen) { 
            mintPublic(); 
        } else {
            revert MintIsClosed();
        }
    ...
    function mintPublic() public payable supplyChecks(1) {
        address sender = msg.sender;
        if(!publicMintIsOpen && sender != owner()) { revert MintIsClosed(); }
        if(msg.value < publicMintPrice) { revert InsufficientFunds(); }
        if(mintedByAddressPublic[sender] >= mintLimitPerWalletPublic) { revert PublicConditionsNotMet(); }
        
        ++mintedByAddressPublic[sender];
        ++amountMinted;
         _safeMint(sender,  amountMinted);
    }

    function mintWhitelist(bytes32[] memory _proof, bytes32 _leaf) public payable supplyChecks(1) {
        address sender = msg.sender;
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        ...
        if(revealed){
            ...
            return bytes(unrevealedBaseURI).length > 0 ? string(abi.encodePacked(unrevealedBaseURI)) : "";
        }
    }
