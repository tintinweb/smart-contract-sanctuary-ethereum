...
contract Marketplace is Ownable {
	...
	*/
    ...
    function addProvider(string memory name, string memory description, string memory logo, string memory banner, address addr, uint8 commission) public onlyOwner {
	...
	*/
    function setIsProviderEnabled(bytes32 id, bool value) public onlyOwner {
    ...
    * @param commission The marketplace commission for each sale
	*/
    function editProvider(bytes32 id, string memory name, string memory description, string memory logo, string memory banner, address addr, uint8 commission) public { 
        Provider memory p = providerByID[id];
    ...
    */
    ...
    function addArtwork(string memory name, string memory description, string memory uri, string memory metadataUri, uint256 price, uint16 totalSupply) public {
        require(bytes(name).length > 0, "The name can't be empty");
        require(bytes(description).length > 0, "The description can't be empty");
...
contract NFTToken is ERC721URIStorage, Ownable {
    ...
    constructor() Ownable() ERC721("ScaleUPNFT-Token", "SUT")  {
    }
