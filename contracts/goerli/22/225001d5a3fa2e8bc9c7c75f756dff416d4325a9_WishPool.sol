...
contract WishPool is Initializable, Pausable, Ownable {
    ...
    function redeem(RedeemData memory redeemData) public whenNotPaused needInit payable {
        ...
        if (incenseMinAmount > 0) {
            require(redeemData.incenseAmount >= incenseMinAmount, "invalid incense amount");
        }

        if (incenseMaxAmount > 0) {
            require(redeemData.incenseAmount <= incenseMaxAmount, "invalid incense amount");
        }
        ...
        IBurnNft(burnToken).burn(redeemData.tokenId);
        string memory burnData = string(abi.encodePacked(burnToken, "|", redeemData.tokenId, "|", redeemNftGen, "|", redeemData.wishId));
        
        // handle grant
        (address grantToken, uint256 grantTokenId, uint8 grantNftGen) = _grant(redeemData.grantAddr);
        string memory grantData = string(abi.encodePacked(grantToken, "|", grantTokenId, "|", grantNftGen, "|", redeemData.grantWishId, "|", redeemData.grantColor));

        // handle donate
        if (redeemData.donateAmount > 0) {
            _donate(redeemData.redeemToken, redeemData.donateAmount);
        }

        // handle incense
        if (redeemData.incenseAmount > 0) {
            _incense(redeemData.redeemToken, redeemData.incenseAmount);
        }
    ...
    function _grant(address grantAddr) private returns(address, uint256, uint8) {
        ...
        if (grantAddr != address(0)) {
            ...
            require(grantTokenId > 0, "grant damo failure");
        }
    ...
    function _incense(address incenseToken, uint256 incenseAmount) private {
        if (incenseToken == address(0)) {
            Address.sendValue(payable(appConf.getIncenseAddr()), incenseAmount);
        } else {
            IERC20Ex(incenseToken).safeTransferFrom(_msgSender(), appConf.getIncenseAddr(), incenseAmount);
        }
    }
}
