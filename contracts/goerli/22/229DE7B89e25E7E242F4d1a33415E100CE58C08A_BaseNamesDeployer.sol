...
contract BaseNamesDeployer {
  bytes32 public constant TLD_LABEL = keccak256("base");
  ...
  BaseRegistrarController public controller;
  ...
  constructor(
    ...
    // Create a registrar for the TLD
    registrar = new BaseRegistrarImplementation(
      ens,
      namehash(bytes32(0), TLD_LABEL)
    );

    ens.setSubnodeOwner(bytes32(0), TLD_LABEL, address(registrar));

    // Set up the controller
    controller = new BaseRegistrarController(
      registrar,
      feeToken,
      feePerSecond,
      treasury
    ...
    // Transfer ownerships to the caller
 ...
 */
contract BaseRegistrarController is Ownable, FeeBase {
