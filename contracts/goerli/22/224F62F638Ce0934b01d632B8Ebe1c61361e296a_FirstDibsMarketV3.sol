...
error InvalidAssetClass(uint256 listingId);
    ...
    FirstDibsFeeSupport
{
    ...
    uint256 private listingIdsCounter;

    /// @notice The listing for a given listing ID
    /// @dev Listing ID  => Listing
    ...
    /**
     * @notice This event is emitted when a Listing quantity is updated
     * @param listingId The ID of the listing
     * @param quantity the number up tokens to update to
     */
    event ListingAmountUpdated(uint256 listingId, uint256 quantity);
    ...
    /**
     * @notice This event is emitted when a Listing is purchased
     * @param listingId The ID of the listing
     * @param buyer the buyer of the listing
     ...
     * @param _assetClass The type of token for sale
    ...
    function createListing(
        ...
        address tokenSeller;
        uint256 quantity;
        if (_assetClass != ERC721_ASSET_CLASS && _assetClass != ERC1155_ASSET_CLASS) {
            revert InvalidAssetClass({ listingId: 0 });
        }
        if (_startTime > 0 && block.timestamp > _startTime) {
            revert InvalidTime({ timestamp: _startTime });
        }
        if (_endTime > 0 && block.timestamp > _endTime) {
            revert InvalidTime({ timestamp: _endTime });
        }
        ...
        if (_assetClass == ERC721_ASSET_CLASS) {
            ...
            if (
                ...
                !IERC721(_tokenContract).isApprovedForAll(tokenOwner, _msgSender())
            ) {
                revert UnauthorizedCaller({ listingId: 0 });
            }
            // Store the sender as the seller
            tokenSeller = _msgSender();
            quantity = 1;
        } else {
            if (_quantity <= 0) {
                revert InvalidSupply({ listingId: 0 });
            }
            if (IERC1155(_tokenContract).balanceOf(_msgSender(), _tokenId) < _quantity) {
                revert InvalidSupply({ listingId: 0 });
            }
            // store the sender as the seller
            tokenSeller = _msgSender();
        ...
        address fundsRecipient = tokenSeller;
        ...
        listings[listingIdsCounter].tokenContract = _tokenContract;
        listings[listingIdsCounter].tokenId = _tokenId;
        listings[listingIdsCounter].seller = tokenSeller;
        listings[listingIdsCounter].fundsRecipient = fundsRecipient;
        listings[listingIdsCounter].price = _price;
        listings[listingIdsCounter].quantity = quantity;
        listings[listingIdsCounter].buyer = _buyer;
        listings[listingIdsCounter].assetClass = _assetClass;
        listings[listingIdsCounter].startTime = _startTime;
        listings[listingIdsCounter].endTime = _endTime;
     ...
     * @param _price The listing price to set
     */
    function setListingPrice(uint256 _listingId, uint256 _price)
        external
        nonReentrant
        listingExists(_listingId)
    {
        // Ensure the caller is seller
        if (_msgSender() != listings[_listingId].seller) {
    ...
    /**
     * @notice Updates the quantity of tokens for sale on a listing
     * @param _listingId The id of the listing to update
     * @param _quantity The quantity of tokens to list for sale
     */
    function setListingAmount(uint256 _listingId, uint256 _quantity)
        external
        nonReentrant
        listingExists(_listingId)
    {
        ...
        // Ensure the token is ERC1155
        if (listing.assetClass != ERC1155_ASSET_CLASS) {
            revert InvalidAssetClass({ listingId: _listingId });
        }
        ...
        uint256 balanceOfSender = IERC1155(listing.tokenContract).balanceOf(
            _msgSender(),
            listing.tokenId
        );
        ...
        emit ListingAmountUpdated(_listingId, _quantity);
    }
    ...
    function cancelListing(uint256 _listingId) external nonReentrant listingExists(_listingId) {
        ...
        if (listing.assetClass == ERC721_ASSET_CLASS) {
            ...
            if (_msgSender() != seller && _msgSender() != tokenOwner) {
                revert UnauthorizedCaller({ listingId: _listingId });
            }
        } else if (_msgSender() != seller) {
            revert UnauthorizedCaller({ listingId: _listingId });
        }
    ...
    function purchaseFromListing(
        ...
        if (listing.buyer != address(0) && _msgSender() != listing.buyer) {
            revert UnauthorizedCaller({ listingId: _listingId });
        }
        ...
        FirstDibsFeeSupport._handlePayouts(
        ...
        if (listing.assetClass == ERC721_ASSET_CLASS) {
            ...
            // Remove the listing from storage
            delete listings[_listingId];
        } else if (listing.assetClass == ERC1155_ASSET_CLASS) {
            // Transfer the 1155s to the buyer
            // Reverts if the seller did not approve the FirstDibsTokenManager or no longer owns enough tokens to transfer
            ...
            if (listings[_listingId].quantity == 0) {
                // Remove the listing from storage if there are no more tokens left for sale
                delete listings[_listingId];
            }
        }

        emit ListingPurchased(_listingId, _recipient, _quantity, listings[_listingId].quantity);
    }
}
...
contract FirstDibsFeeSupport is Initializable, AccessControlUpgradeable, PullPaymentUpgradeable {
    ...
    function _handlePayouts(
        ...
        uint256 commissionFee = _handleCommissionFeePayout(_sentFunds, _platformFee);
        ...
        uint256 sellerFee = _sentFunds - royaltyAmount - commissionFee;
    ...
    function _handleRoyaltyPayout(
        ...
        try
            this._handleRoyaltyEnginePayout{ gas: gas }(_tokenContract, _tokenId, _sentFunds)
