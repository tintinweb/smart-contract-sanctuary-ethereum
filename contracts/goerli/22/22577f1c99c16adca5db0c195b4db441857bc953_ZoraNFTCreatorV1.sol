  ...
  ERC721DropStorageV1
{
  ...
  modifier senderHoldsAllowlistToken() {
    uint balance = IERC721AUpgradeable(salesConfig.allowlistContractAddress).balanceOf(
      _msgSender()
    );
    if (balance < 1) {
  ...
  function _publicSaleActive() internal view returns (bool) {
    return
      salesConfig.publicSaleStart <= block.timestamp && salesConfig.publicSaleEnd > block.timestamp;
  }
  ...
  function saleDetails() external view returns (IKitsERC721Drop.SaleDetails memory) {
    return
  ...
  function updateRarities(
    IKitsERC721Drop.RarityConfiguration[NUMBER_OF_RARITY_CONFIGS] memory _rarityConfigs
  ) external onlyAdmin {
  ...
  function adminMint(
    address recipient,
    uint256 quantity
  ) external onlyRoleOrAdmin(MINTER_ROLE) canMintPublicSaleTokens(quantity) returns (uint256) {
  ...
  function owner() public view override(OwnableSkeleton, IKitsERC721Drop) returns (address) {
    return super.owner();
  }
...
import { RarityMetadataRenderer } from './metadata/RarityMetadataRenderer.sol';
import { IKitsERC721Drop } from '../interfaces/IKitsERC721Drop.sol';
import { DropMetadataRenderer } from './metadata/DropMetadataRenderer.sol';
import { IMetadataRenderer } from '../interfaces/IMetadataRenderer.sol';
import { KitsERC721Drop } from './KitsERC721Drop.sol';
...
contract ZoraNFTCreatorV1 is OwnableUpgradeable, UUPSUpgradeable, Version(4) {
  ...
  /// @notice Rarity metadata renderer
  RarityMetadataRenderer public immutable rarityMetadataRenderer;
  ...
  constructor(
    ...
    RarityMetadataRenderer _rarityMetadataRenderer
  ) {
  ...
  //        /|\
  //         |                    ,----------------.              ,----------.
  //        / \                   |ZoraNFTCreatorV1|              |KitsERC721Drop|
  //      Caller                  `-------+--------'              `----+-----'
  //        |                       createDrop()                       |
  //        | --------------------------------------------------------->
  //        |                             |                            |
  ...
  /// @param publicSaleEditionSize The max size of the media contract allowed
  /// @param presaleEditionSize Max number of editions available durin presale
  ...
  function setupDropsContract(
    ...
    IKitsERC721Drop.SalesConfiguration memory saleConfig,
    ...
    KitsERC721Drop(newDropAddress).initialize(
      name,
      symbol,
      defaultAdmin,
      fundsRecipient,
      publicSaleEditionSize,
      presaleEditionSize,
      royaltyBPS,
      saleConfig,
      metadataRenderer,
    ...
    emit CreatedDrop({
      creator: msg.sender,
      publicSaleEditionSize: publicSaleEditionSize,
  ...
  function createDrop(
    ...
    return
      setupDropsContract({
        ...
        publicSaleEditionSize: publicSaleEditionSize,
        presaleEditionSize: presaleEditionSize,
  ...
  /// @param defaultAdmin Default admin address
  /// @param publicSaleEditionSize Total size of the edition (number of possible editions)
  ...
  function createEdition(
    ...
    return
      setupDropsContract({
        ...
        saleConfig: saleConfig,
        fundsRecipient: fundsRecipient,
        metadataRenderer: rarityMetadataRenderer,
        metadataInitializer: metadataInitializer
      });
  }
}
...
interface DropConfigGetter {
    function config()
        external
        view
        returns (IKitsERC721Drop.Configuration memory config);
}
    ...
    MetadataRenderAdminCheck
{
    ...
    function contractURI() external view override returns (string memory) {
        ...
        IKitsERC721Drop.Configuration memory config = DropConfigGetter(target)
            .config();
    ...
    function tokenURI(
        ...
        IKitsERC721Drop media = IKitsERC721Drop(target);
        ...
        return
            NFTMetadataRenderer.createMetadataEdition({
                ...
                publicSaleEditionSize: maxSupply
            });
    }
}
...
interface KitsConfigGetter {
  function config() external view virtual returns (IKitsERC721Drop.Configuration memory config);

  function getRarityMapping(uint256 tokenId) external view returns (uint256);

  function rarityConfigs(
    uint256 rarityId
  ) external view virtual returns (IKitsERC721Drop.RarityConfiguration calldata rarityConfig);
}

/// @notice EditionMetadataRenderer for editions support
contract RarityMetadataRenderer is IMetadataRenderer, MetadataRenderAdminCheck {
  /// @notice Storage for token edition information
  struct TokenEditionInfo {
    string description;
    string imageURI;
  ...
  function contractURI() external view override returns (string memory) {
    address target = msg.sender;
    TokenEditionInfo storage editionInfo = tokenInfos[target];
    IKitsERC721Drop.Configuration memory config = KitsConfigGetter(target).config();
  ...
  function tokenURI(uint256 tokenId) external view override returns (string memory) {
    ...
    TokenEditionInfo memory info = tokenInfos[target];
    // rarity stuff
    KitsConfigGetter kitsContract = KitsConfigGetter(target);
    uint256 rarityId = kitsContract.getRarityMapping(tokenId);
    IKitsERC721Drop.RarityConfiguration memory rarityConfig = kitsContract.rarityConfigs(rarityId);

    return
      NFTMetadataRenderer.createMetadataRarity({
        name: IERC721MetadataUpgradeable(target).name(),
        description: rarityConfig.rarityDescription,
        imageURI: rarityConfig.coverImageURI,
        animationURI: info.animationURI,
        tokenOfEdition: tokenId,
        tierName: rarityConfig.name
      });
  }
}
...
contract ERC721DropStorageV1 {
  ...
  mapping(uint256 => IKitsERC721Drop.RarityConfiguration) public rarityConfigs;
...
library NFTMetadataRenderer {
  ...
  /// @param publicSaleEditionSize Size of entire edition to show
  ...
  /// @param animationURI URI of animation to render for edition
  /// @param tokenOfEdition Token ID for specific token
  /// @param tierName Name of the tier to which this token belongs
  function createMetadataRarity(
    string memory name,
    string memory description,
    string memory imageURI,
    string memory animationURI,
  ...
  function createMetadataJSON(
    ...
    if (publicSaleEditionSize > 0) {
      editionSizeText = abi.encodePacked('/', Strings.toString(publicSaleEditionSize));
    }
    return
      abi.encodePacked(
        '{"name": "',
  ...
  function tokenMediaData(
    ...
    if (hasImage && hasAnimation) {
      return
        string(
          abi.encodePacked('image": "', imageUrl, '", "animation_url": "', animationUrl, '", "')
        );
    }
    if (hasImage) {
