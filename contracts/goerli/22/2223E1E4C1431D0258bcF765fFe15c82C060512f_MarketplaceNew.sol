...
import "@openzeppelin/contracts-upgradeable/interfaces/IERC2981Upgradeable.sol"; //Royalty
...
import "../openzeppelin-presets/metatx/ERC2771ContextUpgradeable.sol"; // meta-transactions (gasless)
import "../lib/CurrencyTransferLib.sol";
import "../lib/FeeType.sol";

//import "hardhat/console.sol";

contract MarketplaceNew is
    Initializable,
    IMarketplace,
    ...
    //UUPSUpgradeable
{
    ...
    uint64 public constant MAX_BPS = 10_000_000;
    ...
    */
    ...
    modifier onlyListingCreator(uint256 _listingId) {
        require(listings[_listingId].tokenSeller == _msgSender(), "!OWNER");
        _;
    }
    ...
    function initialize(
        ...
        nativeTokenWrapper = address(this); // _nativeTokenWrapper;
    ...
    */
    ...
    //    require(marketOwner == msg.sender, "!Owner");
    ...
    */
    ...
    function get_LISTER_ROLE() external pure returns (bytes32) {
        return LISTER_ROLE;
    }

    function get_ASSET_ROLE() external pure returns (bytes32) {
        return ASSET_ROLE;
    }
    ...
    function createListing(ListingParameters memory _params) external override {
        ...
        //console.log("tokenTypeOfListing %s", tokenTypeOfListing);
        ...
        //console.log("startTime %s block.timestamp %s", startTime, block.timestamp); // block.timestamp 는 모두 10자리. 즉 밀리세컨드는 없음.
        ...
        if (newListing.listingType == ListingType.Auction) {
            ...
            transferListingTokens(tokenSeller, address(this), tokenAmountToList, newListing);
        }

        emit ListingAdded(listingId, _params.assetContract, tokenSeller, newListing);
    }
    ...
    function updateListing(
        ...
        if (targetListing.quantity != safeNewQuantity) {
            ...
            if (isAuction) {
                transferListingTokens(address(this), targetListing.tokenSeller, targetListing.quantity, targetListing);
            }

            validateOwnershipAndApproval(
                targetListing.tokenSeller,
            ...
            if (isAuction) {
                transferListingTokens(targetListing.tokenSeller, address(this), safeNewQuantity, targetListing);
            }
        }

        emit ListingUpdated(_listingId, targetListing.tokenSeller);
    }
    ...
    function cancelDirectListing(uint256 _listingId) external onlyListingCreator(_listingId) {
        ...
        listings[_listingId].state = State.Inactivated;
        targetListing.state = State.Inactivated;

        emit ListingRemoved(_listingId, targetListing.tokenSeller, targetListing);
    }
    ...
    function buy(
        ...
        require(_currency == targetListing.currency && _totalPrice == (targetListing.buyoutPricePerToken * _quantityToBuy), "!PRICE");
    ...
    /// @dev Let the listing's creator accept an offer for their direct listing.
    ...
    function executeSale(
        ...
        //console.log("validateDirectListingSale passed");

        _targetListing.quantity -= _listingTokenAmountToTransfer;
        listings[_targetListing.listingId] = _targetListing;

        // 상태 변경 추가
        listings[_targetListing.listingId].state = State.Sold;

        // 소유주 변경 추가
        listings[_targetListing.listingId].tokenBuyer = _payer;

        //console.log("%s %s",_listingTokenAmountToTransfer, _targetListing.quantity);
        //console.log("%s %s %s",_payer, _currency, _currencyAmountToTransfer);

        payout(_payer, _targetListing.tokenSeller, _currency, _currencyAmountToTransfer, _targetListing);
        //console.log("payout succeeded");

        transferListingTokens(_targetListing.tokenSeller, _receiver, _listingTokenAmountToTransfer, _targetListing);

        emit NewSale(
            _targetListing.listingId,
            _targetListing.assetContract,
            _targetListing.tokenSeller,
            _receiver,
            _listingTokenAmountToTransfer,
            _currencyAmountToTransfer
        );
    ...
    function offer(
        ...
        //console.log("offer: %s %s %s", _listingId, _quantityWanted, _currency);
        //console.log("offer: %s %s", _pricePerToken, _expirationTimestamp);
        Listing memory targetListing = listings[_listingId];

        //console.log("block.timestamp: %s", block.timestamp);
        require(targetListing.endTime >= block.timestamp && targetListing.startTime <= block.timestamp, "Inactive!");
        ...
        if (targetListing.listingType == ListingType.Auction) {
            ...
            require(newOffer.currency == targetListing.currency, "!Approved Currency");
            ...
            //console.log("Direct Offer: %s", msg.value);
            // Prevent potentially lost/locked native token.
            require(msg.value == 0, "!Value"); // Direct에서는 WETH로만 주문 가능

            // Offers to direct listings cannot be made directly in native tokens.
            //console.log("same? %s == %s", _currency, CurrencyTransferLib.NATIVE_TOKEN);
    ...
    function handleOffer(Listing memory _targetListing, Offer memory _newOffer) internal {
        //console.log("handleOffer starts");
        require(_newOffer.quantityWanted <= _targetListing.quantity && _targetListing.quantity > 0, "Insufficient Tokens");
        //console.log("validateERC20BalAndAllowance starts");
        ...
        //console.log("validateERC20BalAndAllowance ends");
    ...
    function handleBid(Listing memory _targetListing, Offer memory _incomingBid) internal {
        ...
        if (
            ...
            //console.log("incomingOfferAmount: %s", incomingOfferAmount);
        ...
        //console.log("line 745 emit NewOffer");
    }
        ...
        onlyExistingListing(_listingId)
    {
        ...
        if (toCancel) {
            ...
            require(targetListing.endTime < block.timestamp, "!Ended.");

            // No `else if` to let auction close in 1 tx when targetListing.tokenSeller == targetBid.offeror.
            if (_closeFor == targetListing.tokenSeller) {
    ...
    function _cancelAuction(Listing memory _targetListing) internal {
        require(listings[_targetListing.listingId].tokenSeller == _msgSender(), "!listing creator");

        delete listings[_targetListing.listingId];

        transferListingTokens(address(this), _targetListing.tokenSeller, _targetListing.quantity, _targetListing);

        emit AuctionClosed(_targetListing.listingId, _msgSender(), true, _targetListing.tokenSeller, address(0));
    }
    ...
    function _closeAuctionForAuctionCreator(Listing memory _targetListing, Offer memory _winningBid) internal {
        ...
        payout(address(this), _targetListing.tokenSeller, _targetListing.currency, payoutAmount, _targetListing);
    ...
    function _closeAuctionForBidder(Listing memory _targetListing, Offer memory _winningBid) internal {
        ...
        //console.log("_closeAuctionForBidder");
        transferListingTokens(address(this), _winningBid.offeror, quantityToSend, _targetListing);
        //console.log("transferListingTokens");

        //address tokenSeller = _targetListing.tokenSeller;
        //console.log("tokenSeller: %", tokenSeller);

        //console.log("_closeAuctionForBidder ends");
        ...
        //console.log("AuctionClosed emitted");
    }
    ...
    function transferListingTokens(
        ...
        if (_listing.tokenType == TokenType.ERC1155) {
            ...
            //console.log("%s %s %s", _from, _to, _listing.tokenId);
    ...
    function payout(
        ...
        //console.log("_totalPayoutAmount %s, platformFeeCut %s", _totalPayoutAmount, platformFeeCut);
        ...
        try IERC2981Upgradeable(_listing.assetContract).royaltyInfo(_listing.tokenId, _totalPayoutAmount) returns (
            ...
            if (royaltyFeeRecipient != address(0) && royaltyFeeAmount > 0) {
                require(royaltyFeeAmount + platformFeeCut <= _totalPayoutAmount, "fees > the price");
        ...
        //console.log("royalty: %s %s %s", royaltyRecipient, royaltyCut, _nativeTokenWrapper);
    ...
    function validateERC20BalAndAllowance(
        ...
        //("validateERC20BalAndAllowance: _currency: %s _addrToCheck: %s %s", _currency, _addrToCheck, _currencyAmountToCheckAgainst);
        // https://stackoverflow.com/questions/71091866/solidity-check-balance-of-a-user-address-using-openzeppelin
        //uint256 balanceOf = IERC20Upgradeable(_currency).balanceOf(_addrToCheck);
        //console.log("balanceOf %s : %s", balanceOf, _currencyAmountToCheckAgainst);
        //uint256 allowance = IERC20Upgradeable(_currency).allowance(_addrToCheck, address(this));
        //console.log("allowance %s : %s", allowance, _currencyAmountToCheckAgainst);
        require(
    ...
    /// @dev Validates that `_tokenSeller` owns and has approved Market to transfer NFTs.
    function validateOwnershipAndApproval(
        ...
        //console.log("validateOwnershipAndApproval");

        address market = address(this);
        bool isValid;

        //console.log("_tokenType : %s", _tokenType);

        if (_tokenType == TokenType.ERC1155) {
            //console.log("_tokenType: ERC1155");
            isValid =
                IERC1155Upgradeable(_assetContract).balanceOf(_tokenSeller, _tokenId) >= _quantity &&
                IERC1155Upgradeable(_assetContract).isApprovedForAll(_tokenSeller, market);
        } else if (_tokenType == TokenType.ERC721) {
            //console.log("_tokenType: ERC721");
            //console.log("OwnerOf: %s _tokenSeller: %s", IERC721Upgradeable(_assetContract).ownerOf(_tokenId), _tokenSeller);
            //console.log("getApproved: %s", IERC721Upgradeable(_assetContract).getApproved(_tokenId));
            //console.log("tokenSeller: %s, market: %s approvedForAll: %s",_tokenSeller, market, IERC721Upgradeable(_assetContract).isApprovedForAll(_tokenSeller, market));

            isValid =
                IERC721Upgradeable(_assetContract).ownerOf(_tokenId) == _tokenSeller &&
                (IERC721Upgradeable(_assetContract).getApproved(_tokenId) == market ||
                    IERC721Upgradeable(_assetContract).isApprovedForAll(_tokenSeller, market));
        }
    ...
    function validateDirectListingSale(
        ...
        require(_listing.listingType == ListingType.Direct, "!listingType");
        ...
        //console.log("%s : %s - %s", block.timestamp, _listing.startTime, _listing.endTime);
        require(block.timestamp < _listing.endTime && block.timestamp > _listing.startTime, "!STW");

        // Check: buyer owns and has approved sufficient currency for sale.
        //console.log("%s, %s", _currency, CurrencyTransferLib.NATIVE_TOKEN);
        if (_currency == CurrencyTransferLib.NATIVE_TOKEN) {
            //console.log("%s : %s", msg.value, settledTotalPrice);
        ...
        //console.log("Ok");
        //console.log("%s, %s, %s", _listing.tokenSeller, _listing.assetContract, _listing.tokenId);
        //console.log("%s", _quantityToBuy);
        //console.log("%s", _listing.tokenType);

        // Check whether token owner owns and has approved `quantityToBuy` amount of listing tokens from the listing.
        validateOwnershipAndApproval(
            _listing.tokenSeller,
    ...
    function getTokenType(address _assetContract) internal view returns (TokenType tokenType) {
        if (IERC165Upgradeable(_assetContract).supportsInterface(type(IERC1155Upgradeable).interfaceId)) {
            ...
            //console.log("token must b ERC1155 or ERC721");
    ...
    /// @dev Let a contract admin update platform fee recipient and bps.
    ...
    /// @dev Let a contract admin set auction buffers.
    ...
    /// @dev Let a contract admin set the URI for the contract-level metadata.
    ...
    * todo pagination   
    */
    function listingsCountByType(FetchOperator _op) public view returns (uint256) {
        uint256 total = totalListings; 

        uint256 itemCount = 0;
        for (uint256 i = 1; i <= total; i++) {
            if (isCondition(listings[i], _op)) {
                itemCount ++;
            }
        }
    ...
    */
    function isCondition(Listing memory item, FetchOperator _op) private view returns (bool){
        if(_op == FetchOperator.MyEnlistedItems){ 
            return (item.tokenSeller == _msgSender() && item.state == State.Enlisted)? true : false;
        } else if(_op == FetchOperator.MyPurchasedItems){
            return (item.tokenBuyer ==  _msgSender() && item.state == State.Sold) ? true: false;
    ...
    function fetchItems(FetchOperator _op, uint256 from, uint256 to) public view returns (Listing[] memory) {     
        uint total = totalListings; //_itemCounter.current();
        
        uint pageSize = 0;
        if ( to == 0 ) {
            pageSize = listingsCountByType(_op);
        } else {
            pageSize = to - from + 1;    
        }
        
        Listing[] memory items = new Listing[](pageSize);
        
        //if (to >= itemCount) {
        //    to = itemCount;
        //}

        //console.log("%s %s %s", total, from, to);
    ...
    function getItemById(uint256 id) public view returns (Listing memory) {
        //Listing[] memory item = new Listing[](1);
        return listings[id];
    }
...
import "./IUporicContract.sol";
import "./IPlatformFee.sol";

interface IMarketplace is IUporicContract, IPlatformFee {
    ...
    **/
    ...
    struct Listing {
        uint256 listingId;
        address tokenSeller; //address tokenOwner; // Modified
        address tokenBuyer; // Added
        address assetContract;
        uint256 tokenId;
        uint256 startTime;
        uint256 endTime;
