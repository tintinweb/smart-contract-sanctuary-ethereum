...
// Team: DLotto
// Members:
//1. Phatchara Narinrat    2130731303002  
//2. Piyathida Malayasakul 2130731303006
//3. Worada Limpreedeekul. 2130731303007
...
contract DLotto is Ownable,Pausable{
    ...
    event Payreward(address payable receiver, uint256 reward);
    ...
    function buyDlotto(uint256 no,uint256 amt,uint256 tp) public payable {
        ...
        //require(msg.value == payableAmount, "Depend on user input");
    ...
    function getAnnouncement() view public returns (uint256[] memory){
        return announceReward;
    }
    ...
    function payReward() public payable onlyOwner{
        ...
        for (uint256 i=0;i<all_Buyer.length;i++){
            ...
            for (uint256 j=0;j<announceReward.length;j++)               
                ...
                uint256 rw = (buyer[receiver].amount/10**18)*(rate[j]/10**18)*10**18;    
                receiver.transfer(rw);       
                emit Payreward(receiver, rw);
