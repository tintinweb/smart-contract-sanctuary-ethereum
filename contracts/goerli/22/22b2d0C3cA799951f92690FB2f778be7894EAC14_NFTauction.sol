...
contract NFTauction is Ownable, ReentrancyGuard {
    ...
    // uint256 public servicePercentage = 200;
    ...
    function addSupportedContract(address nftContract) external onlyOwner {
        address _nftCotract = nftContract;
        if (supportedContracts[_nftCotract] == true) {
            revert AlreadyAdded(_nftCotract);
        }
        supportedContracts[_nftCotract] = true;
        emit AddedContract(_nftCotract);
    }

    function removeSupportedContract(address nftContract) external onlyOwner {
        address _nftCotract = nftContract;
        if (supportedContracts[_nftCotract] == false) {
            revert AlreadyRemoved(_nftCotract);
        }
        supportedContracts[_nftCotract] = false;
        emit RemovedContract(_nftCotract);
    }
        ...
        OnlySupportedContract(nftContract)
    {
        ...
        if (startPrice == 0) {
            revert InvalidBidPrice(startPrice);
        }

        uint256 start = block.timestamp;
        // servicePercentage = royaltyPercentage;
    ...
    function cancelAuction(address nftContract, uint256 tokenId) 
        ...
        onlyBeforeEnd(nftContract, tokenId)
    ...
    function placeBid(address nftContract, uint256 tokenId, uint256 bidPrice) 
        external nonReentrant
        // onlyNotCancelled(nftContract, tokenId)
        onlyAfterStart(nftContract, tokenId)
        onlyBeforeEnd(nftContract, tokenId)
        validBidCheck(nftContract, tokenId, bidPrice)
    {
        ...
        // auctionItems[nftContract][tokenId].option = option;
        ...
        onlyNotCancelled(nftContract, tokenId)
    {
        ...
        // uint256 _option = auctionItems[nftContract][tokenId].option;
