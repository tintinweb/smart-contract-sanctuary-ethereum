...
function validateMessage(string memory message_) public view returns(bool) {
        ...
        require(messageBytes.length <= 12, "Message too long");
  ...
  function mint(string memory message_) public payable {
    // Validate the message
    require(validateMessage(message_), "Invalid message");

    // Check if the sender has already sent a message
    require(balanceOf(msg.sender) == 0, "You have already sent a message");

    uint256 supply = totalSupply();
    require(supply + 1 <= 999999999999);

    // Create a new Word NFT with the provided message
    Word memory newWord = Word(
        string(abi.encodePacked('ID#', uint256(supply + 1).toString())), 
        "My Web 3 Username",
        randomNum(361, block.difficulty, supply).toString(),
        randomNum(361, block.timestamp, supply).toString(),
        message_);

    // Check if the sender is the contract owner, or if they have sent the required amount of Ether
    if (msg.sender != owner()) {
        require(msg.value >= 0.005 ether, "Insufficient Ether amount");
    }
...
function buildImage(uint256 _tokenId) public view returns(string memory) {
      ...
      return Base64.encode(bytes(
          abi.encodePacked(
              ...
              '<text x="50%" y="50%" dominant-baseline="middle" fill="white" font-family="Lucida Sans Unicode" text-anchor="middle" font-size="100">','0x',currentWord.value,'</text>',
              '</svg>'
          )
      ));
  }
