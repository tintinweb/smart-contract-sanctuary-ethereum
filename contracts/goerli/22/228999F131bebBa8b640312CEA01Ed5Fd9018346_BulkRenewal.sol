...
import "./IRootRegistrarController.sol";
...
contract BulkRenewal is IBulkRenewal {
    bytes32 private constant ROOT_NAMEHASH = 0;

    BDNS public immutable bdns;

    constructor(BDNS _bdns) {
        bdns = _bdns;
    }

    function getController() internal view returns (RootRegistrarController) {
        Resolver r = Resolver(bdns.resolver(ROOT_NAMEHASH));
        address _interfaceImplementer = r.interfaceImplementer(ROOT_NAMEHASH, type(IRootRegistrarController).interfaceId);
        return RootRegistrarController(payable(_interfaceImplementer));
    }
