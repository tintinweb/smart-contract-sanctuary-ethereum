...
contract Lottery is VRFConsumerBaseV2, ConfirmedOwner {
  ...
  function randomBalls() internal view returns (uint8[5] memory) {
    uint8 ball1 = uint8(s_requests[lastVRF].randomWords[0] % maxBall);
    uint8 ball2 = uint8(s_requests[lastVRF].randomWords[1] % maxBall);
    uint8 ball3 = uint8(s_requests[lastVRF].randomWords[2] % maxBall);
    uint8 ball4 = uint8(s_requests[lastVRF].randomWords[3] % maxBall);
    uint8 ball5 = uint8(s_requests[lastVRF].randomWords[4] % maxBall);
  ...
  function validateBalls(uint8 ball1, uint8 ball2, uint8 ball3, uint8 ball4, uint8 ball5) internal view {
    ...
    require(ball5<maxBall, 'Balls must be less than maxBall');
  }
