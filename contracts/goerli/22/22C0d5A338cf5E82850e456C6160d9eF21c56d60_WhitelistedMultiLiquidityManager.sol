    ...
    IUniswapV3SwapCallback
{
    ...
    /// @dev Can collect Steer fees (Governance)
    bytes32 internal constant STEER_ROLE = keccak256("STEER_ROLE");
    /// @dev Can call update function
    ...
    uint24 public constant TOTAL_FEE = 1500;
    /// @dev Total fraction of fees going towards Steer (as opposed to going towards strategist)
    uint24 public constant STEER_FRACTION_OF_FEE = 6667;
        ...
        )
    {
        ...
        require(totalSupply() <= maxTotalSupply, "TOT");
...
contract MultiPositionLiquidityManager is BaseLiquidityManager {
    ...
    /// @param balance0 The vault's current balance of token0 ready to be deposited
    ///                 (excluding steer and strategist fees)
    /// @param balance1 The vault's current balance of token1 ready to be deposited
    ///                 (excluding steer and strategist fees)
    ...
    function _setBins(
        ...
        if (swapAmount > 0) {
            ...
            require(_getBalance1() < (balance1 * 95) / 100, "swap");
        } else if (swapAmount != 0) {
            // Require that at least 95% of t0 has been deposited
            // (ensuring that swap amount wasn't too great)
            require(_getBalance0() < (balance0 * 95) / 100, "swap");
        }
    }
