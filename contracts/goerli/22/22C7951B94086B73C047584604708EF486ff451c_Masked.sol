...
contract Masked is Ownable, ERC721A, PaymentSplitter, ReentrancyGuard {
    using ECDSA for bytes32;

    string public baseURI;

    uint public maxSupply = 10000;
    uint public reservedTokens = 300;
    uint public publicPrice = 0.25 ether;
    uint public wlPrice = 0.125 ether;

    uint public walletMax = 20;
    uint public wlMax = 5;
    ...
    uint[] private _shares = [6,4,90];
    address[] private _shareholders = [
        0x14425f1CD1482687816429b4E6485CCE8992C150,
        0xD87b1E3F99B4e389B35f47eE4539224d4cc30fE5,
        0x847889fD63E8eF99F42ad77A9B1b53decb482800
    ];
    ...
    modifier onlyEOA() {
        require(tx.origin == msg.sender,"Masked: Only EOA can mint!");
        _;
    }

    modifier enoughSupply(uint256 _amount) {
        require(totalSupply() + _amount <= maxSupply - reservedTokens, "Masked: Minting would exceed max supply!");
        _;
    }
    ...
    function mint(uint _amount) external payable enoughSupply(_amount) onlyEOA() nonReentrant {
        require(publicSale, "Masked: Public sale isn't active at the moment!");
        require(msg.value == publicPrice *_amount, "Masked: Not enough ETH!");
        require(_minted[msg.sender] + _amount <= walletMax, "Masked: You can't mint more than 20 tokens!");
    ...
    function whitelistMint(uint _amount, bytes memory _signature) external payable enoughSupply(_amount) onlyEOA nonReentrant {
        require(wlSale, "Masked: WL sale isn't active at the moment!");
        require(msg.value == wlPrice * _amount, "Masked: Not enough ETH!");
        require(isAllowedToMint(_signature), "Masked: You aren't whitelisted for presale!");
        require(_minted[msg.sender] + _amount <= wlMax, "Masked: You can't mint more than 5 tokens!");
        _minted[msg.sender] += _amount;
        _safeMint(msg.sender, _amount);
    }

    function reserveMint(uint _amount, address _to) external onlyOwner() {
        require(maxSupply >= totalSupply() + _amount, "Masked: Minting would exceed max supply!");
        _safeMint(_to, _amount);
        reservedTokens -=_amount;
    }

    function price() public view returns(uint) {
        return publicSale ? publicPrice : wlPrice;
    }

    function mintedByAddress(address _address) public view returns(uint) {
        return _minted[_address];
    }

    function setMaxSupply(uint _maxSupply) external onlyOwner {
        //FIXME Uncomment for production
        //require(_maxSupply < maxSupply, "Masked: The maximum supply can only be lowered!");
