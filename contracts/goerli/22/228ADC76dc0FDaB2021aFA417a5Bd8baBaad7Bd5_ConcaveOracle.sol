...
contract ConcaveOracle is IBaseOracle, Governable {
    ...
    mapping(address => uint8) public primarySourceCount; // Mapping from token to number of sources
    mapping(address => mapping(uint8 => IBaseOracle)) public primarySources; // Mapping from token to (mapping from index to oracle source)
    ...
    function getETHPx(address token) public view override returns (uint256) {
        ...
        for (uint8 i = 0; i < candidateSourceCount; ) {
            try primarySources[token][i].getETHPx(token) returns (uint256 px) {
        ...
        for (uint8 i = 0; i < validSourceCount - 1;) {
            for (uint8 j = 0; j < validSourceCount - i - 1;) {
        ...
        returns (uint256, uint256)
    {
        if (unitToken == WETH) {
            ...
            uint256 ethPxForUnit = getETHPx(unitToken);
    ...
    function addPrimarySource(
        ...
        IBaseOracle[][] calldata oracles
    ) external onlyGov {
        uint256 tokensLength = tokens.length;
        require(
            tokensLength == maxPriceDeviationList.length &&
        ...
        for (uint256 i = 0; i < tokensLength; ) {
            ...
            require(MAX_SOURCE_COUNT >= oracles[i].length, "too many oracles");
            assembly {
                i := add(i, 1)
            }
        }
        ...
        for (uint256 i = 0; i < tokensLength; ) {
            _addPrimarySource(tokens[i], maxPriceDeviationList[i], oracles[i]);
            assembly {
                i := add(i, 1)
            }
        }
    ...
    function _addPrimarySource(
        ...
        primarySourceCount[token] = oracles.length.toUint8();
        maxPriceDeviations[token] = maxPriceDeviation;
        uint256 oraclesLength = oracles.length;
        for (uint8 i = 0; i < oraclesLength; ) {
            primarySources[token][i] = oracles[i];
            assembly {
                i := add(i, 1)
            }
        }
