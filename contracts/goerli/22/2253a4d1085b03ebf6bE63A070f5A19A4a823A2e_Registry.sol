 ...
 */
contract Registry is IRegistry, Ownable2StepUpgradeable {
    using EnumerableSetUpgradeable for EnumerableSetUpgradeable.AddressSet;

    /* Addresses */
    address private principalTokenFactory;
    EnumerableSetUpgradeable.AddressSet private principalTokens;
    address private zapDepositor;
    address private lpVaultFactory;
    ...
    event LPVaultFactoryUpdated(address indexed _old, address indexed _new);
    event ZapDepositorUpdated(address indexed zapDepositor);
     ...
     */
    function setLPVaultFactory(address _lpVaultFactory) external override {
        _checkOwner();
        if ((_lpVaultFactory == address(0))) {
            revert ZeroAddressError();
        }
        emit LPVaultFactoryUpdated(lpVaultFactory, _lpVaultFactory);
        lpVaultFactory = _lpVaultFactory;
    }
    ...
    function setPrincipalTokenFactory(
        ...
        _checkOwner();
        if ((_newPrincipalTokenFactory == address(0))) {
            revert ZeroAddressError();
        }
        emit PrincipalTokenFactoryUpdated(
            principalTokenFactory,
            _newPrincipalTokenFactory
        );
        principalTokenFactory = _newPrincipalTokenFactory;
    }
     ...
     */
    function addPrincipalToken(address _principalToken) external override {
        _checkOwner();
        if (!(principalTokens.add(_principalToken))) {
            revert PTListUpdateFailed();
        }
        emit PrincipalTokenAdded(_principalToken);
    }
     ...
     */
    function removePrincipalToken(address _principalToken) external override {
        _checkOwner();
        if (!(principalTokens.remove(_principalToken))) {
            revert PTListUpdateFailed();
        }
        emit PrincipalTokenRemoved(_principalToken);
    }
    ...
    function isRegisteredPrincipalToken(
        ...
        return principalTokens.contains(_principalToken);
    }
    ...
    function getPrincipalTokenAt(
        ...
        return principalTokens.at(_index);
    }
     ...
     */
    function principalTokenCount() external view override returns (uint256) {
        return principalTokens.length();
    }
     ...
     */
    function getPrincipalTokenFactoryAddress()
        external
        view
        override
        returns (address)
    {
        return principalTokenFactory;
    }
}
