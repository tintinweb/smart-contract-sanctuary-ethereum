...
contract VanillaDNFTFactory is NoDelegateCall {
    ...
    function setNewDeployerUpdater(address newDeployerUpdater) public {
        require(msg.sender == DNFTDeployerUpdater, "Only the assigned updater can update DNFT Deployer.");
    ...
    function createDerivativeContract(
        ...
        require(getContractAddress[tokenAddress][tokenId] == address(0), "already exist");
        contractAddress = IVanillaDNFTDeployer(DNFTDeployerAddress).deploy(address(this), tokenAddress, tokenId);
    ...
    function getAllMintedLicenses(address originalNftOwner) public view returns(VanillaDerivativeNFT.TokenInfo[] memory) {
        uint256 totalCount;
        for(uint i = 0; i < deployedLicenseSC.length; i++) {
            ...
            if(originalNftOwner == derivativeContract.getContractOwner()) {
...
contract VanillaDerivativeNFT is BaseDerivativeNFT {
    ...
    event PayableReceived(address indexed to, uint256 indexed tokenId, uint256 indexed value);
    event NewMintType(
    ...
    function receivePayable(uint256 tokenId) onlyOriginalNFTHolder external payable {
        require(block.timestamp > tokenInfoMap[tokenId].expTime, "Can not collect payable before license expire");
        require(_exists(tokenId), "License already burned");
        _burn(tokenId);
        uint256 toBeCollected = tokenInfoMap[tokenId].mintPrice;
        payable(IERC721(originNFTAddress).ownerOf(originNFTTokenID)).transfer(toBeCollected);
        emit PayableReceived(msg.sender, tokenId, toBeCollected);
    }

    function receiveAllPayable() onlyOriginalNFTHolder public payable {
    ...
    function getTokenInfoList() public view returns (TokenInfo[] memory tokenInfoList) {
    ...
    function getContractOwner() public view returns(address) {
        return IERC721(originNFTAddress).ownerOf(originNFTTokenID);
    }
}
...
abstract contract BaseDerivativeNFT is ERC721Enumerable {
    ...
    function _beforeTokenTransfer(
        ...
        require(msg.sender == owner || to != address(0), "License is not transferable");
    }
}
