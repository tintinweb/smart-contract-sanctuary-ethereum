...
contract Coliseum is ERC721, Ownable, ReentrancyGuard {
    ...
    uint256 public constant MAX_SUPPLY = 310;
    uint256 public constant MINT_PRICE_TIER_ONE = 0;
    uint256 public constant MINT_PRICE_TIER_TWO = 3;
    uint256 public constant MINT_PRICE_TIER_THREE = 5;
    uint256 public constant MAX_MINT_PER_TX = 1;
    uint256 public totalSupply = 0;

    // @dev this is the USDC Goerli address
    address public constant USDCAdress = 0x07865c6E87B9F70255377e024ace6630C1Eaa37F;
    IERC20 USDC = IERC20(USDCAdress);
    ...
    event Minted(address indexed from);
    ...
    function approveSpending(uint8 _tier) external {
        require(tiers[_tier].id == _tier, "INVALID_TIER");
        require(USDC.balanceOf(msg.sender) >= tiers[_tier].basePrice, "NOT_ENOUGH_FUNDS");
        USDC.approve(address(this), tiers[_tier].basePrice);
    }

    function mint(bytes memory _sig, uint8 _tier)
        external
        nonReentrant
        returns (uint256)
    {
        require(saleStarted, "MINT_NOT_STARTED");
        require(totalSupply + MAX_MINT_PER_TX <= MAX_SUPPLY, "MAX_SUPPLY_REACHED");
        require(tiers[_tier].id == _tier, "INVALID_TIER");

        bytes32 hash = keccak256(abi.encodePacked(_msgSender(), _tier));
        require(!_usedHashes[hash], "HASH_ALREADY_USED");
        require(_matchSigner(hash, _sig), "INVALID_SIGNER");

        require(USDC.transferFrom(msg.sender, address(this), tiers[_tier].basePrice));

        _usedHashes[hash] = true;
        _safeMint(msg.sender, MAX_MINT_PER_TX);
