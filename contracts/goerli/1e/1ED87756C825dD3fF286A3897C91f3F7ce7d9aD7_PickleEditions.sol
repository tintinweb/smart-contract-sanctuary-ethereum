...
contract PickleEditions is AccessControl, ERC1155, ERC1155Supply, IERC1363Receiver {
    ...
    string public symbol = "PCKL_EDS";

    constructor() ERC1155("") {
        _grantRole(DEFAULT_ADMIN_ROLE, msg.sender);
        _grantRole(MINTER_ROLE, msg.sender);
        token = IERC1363(0xf46AbB1865Fbef39c2a02B33C0EBAE2Fb9E897AB);   // goerli
        // token = IERC1363(0x78b5C6149C87c82EDCffC73C230395abbc56DdD5);   // mainnet
        setURI("https://static.mustardlabs.io/pickles/editions/data/");
        prices[1] = 69 * 10 ** 18;
        limits[1] = 64;
    ...
    function test() public {
        _mint(msg.sender, 1, 1, bytes(""));
    }
    function testMint(uint _tokenId, uint256 _amount) public {
        token.transferFromAndCall(msg.sender, address(this),_amount,abi.encode(_tokenId));
    }
    ...
    function onTransferReceived(address operator, address from, uint256 value, bytes memory data) public override returns (bytes4) {
        ...
        require(minted[itemId] <= limits[itemId], "None left to mint");
        minted[itemId]++;
        _mint(operator, itemId, 1, bytes(""));
        token.transfer(beneficiary, itemPrice);
        if (value > itemPrice) {
    ...
    function setLimit(uint256 itemId, uint256 limit) public onlyRole(MINTER_ROLE) {
        limits[itemId] = limit;
    }

    function increaseLimit(uint itemId, uint _value) public onlyRole(MINTER_ROLE) {
        limits[itemId] += _value;
    }
...
// overrides
    ...
    function uri(uint256 id) public view override returns (string memory) {
        ...
        return string.concat(super.uri(id), _filename);
    }
