...
library LibOrder {
    ...
    bytes32 constant INPUT_TYPEHASH =
        keccak256(
            "Order(address maker,Asset makeAssetNormal,Asset makeAssetSigned,address taker,Asset takeAsset,uint256 salt,uint256 start,uint256 end,bytes4 dataType,bytes data,bytes dataMint)Asset(AssetType assetType,uint256 value,address token,uint256 tokenId)AssetType(bytes4 assetClass,bytes data)"
        );
