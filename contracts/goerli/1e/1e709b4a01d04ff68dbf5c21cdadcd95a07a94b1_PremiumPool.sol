    ...
    Ownable
{
    ...
    PoolDeposit public immutable poolDeposit; // ticket instance
    ...
    Ownable
{
     ...
     * @notice allows users to deposit usdc. Users must manually approve transfer by contract beforehand
     */
    function deposit(address pool, address sender, uint256 _usdcAmount, address _usdc, address _ticket, address _aPool) public {
        IERC20 usdc = IERC20(_usdc);
        PremiumPoolTicket ticket = PremiumPoolTicket(_ticket);
        ILendingPool aPool = ILendingPool(_aPool);
        usdc.transferFrom(sender, address(this), _usdcAmount);
        //aPool.deposit(address(usdc), _usdcAmount, pool, 0);
