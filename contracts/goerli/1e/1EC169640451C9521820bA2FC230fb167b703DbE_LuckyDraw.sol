...
contract LuckyDraw is Ownable {
    ...
    event TransferEvent(string _event, uint256 _round ,address indexed _from, address indexed _to, uint256 _value);
    ...
    function register(string memory accountName) public payable {
        require(msg.value == 0.01 ether, "Register sizes are restricted to 0.01 ether");
    ...
    function userCount() public view returns (uint256 count) {
        count = accounts[currentRound].length;
    }

    function adminTransfer(uint256 acctIndex, bool isStart) 
        public
        onlyOwner 
    {
        require(isStart, "Can't transfer because the game hasn't started yet");
        ...
        emit TransferEvent("Transfer reward", currentRound, address(this), toAccount, reward);
        payable(owner()).transfer(fee);
        emit TransferEvent("Transfer fee", currentRound, address(this), owner(), fee);
        adminReset();
    }
