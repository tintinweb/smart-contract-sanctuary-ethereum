...
contract NounsDescriptor is INounsDescriptor, Ownable {
    ...
    // Noun Glasses (Custom RLE)
    bytes[] public override partyGlasses;
    ...
    /**
     * @notice Get the number of available Noun `partyGlasses`.
     */
    function partyGlassesCount() external view override returns (uint256) {
        return partyGlasses.length;
    }
    ...
    /**
     * @notice Batch add Noun partyGlasses.
     * @dev This function can only be called by the owner when not locked.
     */
    function addManyPartyGlasses(bytes[] calldata _partyGlasses) external override onlyOwner whenPartsNotLocked {
        for (uint256 i = 0; i < _partyGlasses.length; i++) {
            _addPartyGlasses(_partyGlasses[i]);
        }
    }
    ...
    /**
     * @notice Add Noun partyGlasses.
     * @dev This function can only be called by the owner when not locked.
     */
    function addPartyGlasses(bytes calldata _partyGlasses) external override onlyOwner whenPartsNotLocked {
        _addPartyGlasses(_partyGlasses);
    }
     ...
     * @notice Add Noun partyGlasses.
     */
    function _addPartyGlasses(bytes calldata _partyGlasses) internal {
        partyGlasses.push(_partyGlasses);
    }
    ...
    function _getPartsForSeed(INounsSeeder.Seed memory seed) internal view returns (bytes[] memory) {
        ...
        _parts[4] = partyGlasses[seed.partyGlasses];
        return _parts;
    }
}
