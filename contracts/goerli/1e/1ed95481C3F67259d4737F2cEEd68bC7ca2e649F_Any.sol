...
contract Any is ERC721, IERC2981, Ownable, Pausable, ReentrancyGuard {
  ...
  modifier isSale {
    require(_isSale, 'Sorry. Not yet on sale.');
    _;
  }
  ...
  constructor(address payable withdrawWallet_, address payable royaltyWallet_, string memory baseTokenURI_, uint256 listingPrice_) ERC721(COLLECTION_NAME, COLLECTION_SYMBOL) {
  ...
  function mintToken(uint256 _count) external payable nonReentrant whenNotPaused() isSale isEnoughNFTs(_count) isEnoughCount(_count) isAmountSufficient(msg.value, _count, listingPrice) returns(uint256) {
    _mintNFT(msg.sender, _count);
