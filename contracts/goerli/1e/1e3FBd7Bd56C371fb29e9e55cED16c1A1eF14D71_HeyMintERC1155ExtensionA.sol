...
import {HeyMintStorage} from "../libraries/HeyMintStorage.sol";
import {ERC1155UDS} from "./ERC1155UDS.sol";

import {ECDSAUpgradeable} from "@openzeppelin/contracts-upgradeable/utils/cryptography/ECDSAUpgradeable.sol";
import {IERC2981Upgradeable, IERC165Upgradeable} from "@openzeppelin/contracts-upgradeable/interfaces/IERC2981Upgradeable.sol";

contract HeyMintERC1155ExtensionA is HeyMintERC1155Upgradeable {
    ...
    function publicPriceInWei(uint16 _tokenId) public view returns (uint256) {
        ...
        return state.tokens[_tokenId].publicPrice * 10 ** 13;
    }

    /**
     * @notice To be updated by contract owner to allow public sale minting for a given token
     */
    function setTokenPublicSaleState(uint16 _tokenId, bool _saleActiveState) external onlyOwner {
        HeyMintStorage.State storage state = HeyMintStorage.state();
        require(state.tokens[_tokenId].publicSaleActive != _saleActiveState, "NEW_STATE_IDENTICAL_TO_OLD_STATE");
  ...
  */
    function setTokenPublicPrice(uint16 _tokenId, uint32 _publicPrice) external onlyOwner {
        HeyMintStorage.State storage state = HeyMintStorage.state();
        require(state.tokens[_tokenId].publicPrice != _publicPrice, "NEW_STATE_IDENTICAL_TO_OLD_STATE");
  ...
  */
    function setTokenPublicMintsAllowedPerAddress(uint16 _tokenId, uint8 _mintsAllowed) external onlyOwner {
        HeyMintStorage.State storage state = HeyMintStorage.state();
        require(state.tokens[_tokenId].publicMintsAllowedPerAddress != _mintsAllowed, "NEW_STATE_IDENTICAL_TO_OLD_STATE");
  ...
  */
    function setTokenPublicSaleStartTime(uint16 _tokenId, uint32 _publicSaleStartTime) external onlyOwner {
        HeyMintStorage.State storage state = HeyMintStorage.state();
  ...
  */
    function setTokenPublicSaleEndTime(uint16 _tokenId, uint32 _publicSaleEndTime) external onlyOwner {
        HeyMintStorage.State storage state = HeyMintStorage.state();
  ...
  */
    function setTokenUsePublicSaleTimes(uint16 _tokenId, bool _usePublicSaleTimes) external onlyOwner {
        HeyMintStorage.State storage state = HeyMintStorage.state();
        require(state.tokens[_tokenId].usePublicSaleTimes != _usePublicSaleTimes, "NEW_STATE_IDENTICAL_TO_OLD_STATE");
  ...
  */
    function tokenPublicSaleTimeIsActive(uint16 _tokenId) public view returns (bool) {
        HeyMintStorage.State storage state = HeyMintStorage.state();
        if (state.tokens[_tokenId].usePublicSaleTimes == false) {
            return true;
        }
        return block.timestamp >= state.tokens[_tokenId].publicSaleStartTime && block.timestamp <= state.tokens[_tokenId].publicSaleEndTime;
    }
...
* @notice Allow for public minting of tokens for a given token
  */
    function mintToken(
        ...
        require(state.tokens[_tokenId].publicSaleActive, "PUBLIC_SALE_IS_NOT_ACTIVE");
        require(this.tokenPublicSaleTimeIsActive(_tokenId), "PUBLIC_SALE_TIME_IS_NOT_ACTIVE");
        ...
        require(state.tokens[_tokenId].maxSupply == 0 || totalSupply + _numTokens <= state.tokens[_tokenId].maxSupply, "MAX_SUPPLY_EXCEEDED");
        require(msg.value == publicPrice * _numTokens + heymintFee, "PAYMENT_INCORRECT");
        require(!state.data.tokenMintingPermanentlyDisabled[_tokenId], "MINTING_PERMANENTLY_DISABLED");
        ...
        if (state.tokens[_tokenId].maxSupply != 0 && totalSupply >= state.tokens[_tokenId].maxSupply) {
...
pragma solidity 0.8.18;
    ...
    struct Data {
        ...
        // TODO: actually need map of maps?
