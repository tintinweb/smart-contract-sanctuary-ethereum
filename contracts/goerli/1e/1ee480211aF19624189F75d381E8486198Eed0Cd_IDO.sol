...
contract IDO is Ownable {
    ...
    function buy() external payable {
        ...
        emit UserBuy(msg.sender, amount);
    }
    ...
    function userAmount(address account_) public view returns (uint256) {
        ...
        uint256 tokenAmount = (userTotal[account_] * tokenSupply) / ETHTotal;
    ...
    function getToken() external {
        ...
        if (diff > 0) {
            ...
            if (balance < diff) {
                ...
                TransferHelper.safeTransferETH(msg.sender, diff);
            }
        }
    ...
    function getEth() external onlyOwner {
        ...
        uint256 sellETHAmount = (tokenSupply * tokenPrice) / 10**tokenDecimals;

        if (ETHTotal > sellETHAmount) {
            uint256 balance = address(this).balance;
            if (balance < sellETHAmount) {
                TransferHelper.safeTransferETH(owner(), balance);
            } else {
                TransferHelper.safeTransferETH(owner(), sellETHAmount);
            }
        } else {
            TransferHelper.safeTransferETH(owner(), ETHTotal);
        }
    ...
    function ownerETH(uint256 amount_) external onlyOwner {
        TransferHelper.safeTransferETH(owner(), amount_);
    }
}