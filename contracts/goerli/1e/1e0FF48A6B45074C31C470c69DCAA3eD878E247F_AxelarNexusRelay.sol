...
abstract contract AbstractNexusRelay is Ownable, INexusRelay {
	mapping(uint256 => address) externalNexusRelayPartners;
	address deployer;
...
contract AxelarNexusRelay is IAxelarExecutable, AbstractNexusRelay {
	...
	uint8 public constant providerId = 1;

	constructor(address _owner) AbstractNexusRelay(_owner) {
	...
	function setRemoteValue(
		...
		_gateway.callContract(destinationChain, destinationAddress, payload);
	}
		...
		if (amount > 0) {
			...
			if (msg.value > 0)
				gasReceiver.payNativeGasForContractCallWithToken{
					...
					AddressToString.toString(targetContract),
					message,
