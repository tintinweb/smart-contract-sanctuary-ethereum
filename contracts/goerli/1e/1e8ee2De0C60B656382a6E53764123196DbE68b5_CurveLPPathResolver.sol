...
import { IWithdrawerOptions } from "../interfaces/IWithdrawer.sol";
...
import { TokenType, PathNotFoundException } from "../interfaces/IPathFinder.sol";
import { PathFinderComponent } from "../data/PathFinderComponent.sol";

contract CurveLPPathResolver is IPathResolver, ResolverConfigurator {
    ...
    PathFinderComponent public override getComponentId =
        PathFinderComponent.CURVE_LP_PATH_RESOLVER;
    ...
    function findOneTokenPath(
        ...
        if (
            ...
            return normal2CurveToken(tokenIn, amount, task);
        }
        ...
        if (
            ...
            return curve2normalToken(tokenIn, amount, task);
        }
        ...
        if (
            ...
            return curve2curveToken(tokenIn, amount, task);
        }
    ...
    /// @dev NORMAL TOKEN -> CURVE TOKEN
    function normal2CurveToken(
        address tokenIn,
        uint256 amount,
        StrategyPathTask memory task
    ) public returns (StrategyPathTask memory) {
        ...
        unchecked {
            for (uint256 i; i < len; ++i) {
                ...
                address tokenMiddle = inputs[i];

                bool isAllMiddle = task.balances.getBalance(tokenMiddle) < 2;

                if (tokenIn != tokenMiddle) {
    ...
    /// @dev CURVE TOKEN -> NORMAL TOKEN
    function curve2normalToken(
        address tokenIn,
        uint256 amount,
        StrategyPathTask memory task
    ) public returns (StrategyPathTask memory) {
        address[] memory inputs;
        (inputs, task) = curveWithdrawer.getUnderlyings(
            TokenType.NORMAL_TOKEN,
            tokenIn,
            task
        );
    ...
    /// @dev CURVE TOKEN -> CURVE TOKEN
    function curve2curveToken(
        address tokenIn,
        uint256 amount,
        StrategyPathTask memory task
    ) public view returns (StrategyPathTask memory) {
        address[] memory inputsIn;
        address[] memory inputsOut;
        unchecked {
            /// Checking deposit
            try
                curveDepositor.getUnderlyings(
                    TokenType.CURVE_LP_TOKEN,
                    task.target,
                    task
                )
            returns (address[] memory inputs, StrategyPathTask memory newTask) {
                inputsOut = inputs;
                task = newTask;
            } catch {}

            uint256 len = inputsOut.length;
            for (uint256 i; i < len; ++i) {
                if (inputsOut[i] == tokenIn) {
                    (, task) = curveDepositor.deposit(
                        TokenType.CURVE_LP_TOKEN,
                        tokenIn,
                        amount,
                        task.target,
