...
contract DiceGame {
    ...
    function rollTheDice() public payable {
        ...
        console.log('block number dice', block.number);
        bytes32 hash = keccak256(abi.encodePacked(prevHash, address(this), nonce));
        console.log('address dice', address(this));
        console.log('nonce dice', nonce);
...
contract RiggedRoll is Ownable {
    ...
    function riggedRoll() external {
        ...
        bytes32 prevHash = blockhash(block.number - 1);
        console.log('block number rig', block.number);
        uint256 nonce = diceGame.nonce();
        bytes32 hash = keccak256(abi.encodePacked(prevHash, address(diceGame), nonce));
        console.log('address rig', address(diceGame));
        console.log('nonce rig', nonce);
        uint256 roll = uint256(hash) % 16;
