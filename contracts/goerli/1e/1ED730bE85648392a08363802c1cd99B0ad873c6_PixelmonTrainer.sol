...
contract PixelmonTrainer is ERC721, Ownable {
    ...
    /// @notice Mint new tokens to address from range #2 [10001, 20000]. Callable only by whitelisted addresses
    /// @dev Minting process uses incremental token ids, starting from 10001
    /// @param _to receiver address
    /// @param _tokensAmount amount of tokens
    /// @return startId minted tokens starting id
    function mintRangeTwo(address _to, uint256 _tokensAmount)
        external
        onlyWhitelisted
        returns (uint256)
    {
        if (secondRangeSupply + _tokensAmount > SECOND_RANGE_MAX_SUPPLY) {
            revert MintAmountExceeded();
        }

        uint256 mintStartId = FIRST_RANGE_MAX_SUPPLY + secondRangeSupply + 1;

        _mintTokens(_to, mintStartId, _tokensAmount);

        unchecked {
            secondRangeSupply += _tokensAmount;
        }
    ...
    /// @dev token ids range #3 - [20001, 20020]
    ...
    function airdropGoldTrainers(address[] calldata _to) external onlyOwner {
        ...
        uint256 thirdRangeTokenId = FIRST_RANGE_MAX_SUPPLY + SECOND_RANGE_MAX_SUPPLY + 1;

        for (uint256 i = 0; i < _to.length; i = _uncheckedInc(i)) {
            _mint(_to[i], thirdRangeTokenId);
            emit Mint(_to[i], thirdRangeTokenId);
    ...
    function _mintTokens(address _to, uint256 _mintStartId, uint256 _tokensAmount) private {
        if (_tokensAmount > 1) {
