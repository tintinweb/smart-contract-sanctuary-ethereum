...
contract PENDINGS is ERC721Enumerable, Ownable {
    ...
    function mintPending(uint256 _qty) payable external {
        address sender = msg.sender;
        if(mintPhase == 0){ revert MintIsClosed(); }
        if(mintPhase == 1 && (!isOnWhitelist(sender) || msg.value < whitelistMintCost * _qty)) { revert WhitelistConditionsNotMet(); }
        if(mintPhase == 2 && (msg.value < publicMintCost * _qty)) { revert PublicConditionsNotMet(); }
        if(currentTokenId == maxSupply){revert MaxSupplyReached(); }
        if(_qty == 0){ revert TryingToMintZero(); }
        if(currentTokenId + _qty > maxSupply){ revert TxWillExceedMaxSupply(); }
        if(_qty > publicMintLimit) { revert YouGreedyMinter(); }
    ...
    function withdrawEthFromContract(address _to, uint256 _amount) external onlyOwner  {
