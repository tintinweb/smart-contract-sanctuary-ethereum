...
contract TestTransferFromVRFConsumer is IVRFCoordinatorConsumer {
    ...
    function approveLink(uint96 amount) external {
        LINKTOKEN.approve(address(i_coordinator), amount);
    }
    ...
    function getReceivedRandomnessByRequestID(RequestID requestID) public returns (uint256[] memory) {
        return s_ReceivedRandomnessByRequestID[requestID];
    }
