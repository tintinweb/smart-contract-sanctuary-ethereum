...
interface IChubbyKaijuDAOStakingV2Old {
  ...
  function gen2StakeByToken(uint16) external view returns (TimeStake memory);
  function gen2Hierarchy(uint16) external view returns (uint16);
}
...
contract ChubbyKaijuDAOStakingV2 is IChubbyKaijuDAOStakingV2, OwnableUpgradeable, IERC721ReceiverUpgradeable, PausableUpgradeable, ReentrancyGuardUpgradeable {
  ...
  function setTimeStake( address account, uint16 tokenId, uint48 time, uint16 rewardType ) external onlyOwner {
  ...
  function stakeTokens(address account, uint16[] calldata tokenIds, bytes[] memory signatures) external whenNotPaused nonReentrant _updateEarnings {
    ...
    for (uint16 i = 0; i < tokenIds.length; i++) {
      ...
      _stakeGEN2(account, tokenIds[i], uint48(block.timestamp), signatures[i]);
  ...
  function _stakeGEN2(address account, uint16 tokenId, uint48 time, bytes memory signature) internal {
  ...
  function claimNonMigratedRewardsAndUnstake(uint16[] calldata tokenIds, bool unstake, bytes[] memory signatures) external whenNotPaused nonReentrant _updateEarnings {
    ...
    uint48 time = uint48(block.timestamp);
    for (uint8 i = 0; i < tokenIds.length; i++) {
      IChubbyKaijuDAOStakingV2Old.TimeStake memory stake = chubbykaijuStakingOld.gen2StakeByToken(tokenIds[i]);
      _stakeGEN2(msg.sender, tokenIds[i], stake.time, signatures[i]);
  ...
  function _claimGen2(uint16 tokenId, bool unstake, uint48 time) internal returns (uint128 reward) {
    ...
    require(!(unstake && time - stake.time < MINIMUM_TO_EXIT), "need 1 day to unstake");

    reward = _calculateGEN2Rewards(tokenId, stake.reward_type, stake.time);
  ...
  function _calculateGEN2Rewards(uint16  tokenId, uint16 rewardType, uint48 stakeTime) internal view returns (uint128 reward) {
    require(tx.origin == msg.sender, "eos only");
    ...
    if (totalCrunchEarned < MAXIMUM_CRUNCH) {
      uint128 time_weight = time-stakeTime > 3600*24*120? 2*TIME_BASE_RATE: 20*(time-stakeTime);
      if(rewardType == 0){
      ...
      uint128 time_weight = lastClaimTimestamp-stakeTime > 3600*24*120? 2*TIME_BASE_RATE: 20*(lastClaimTimestamp-stakeTime);
      if(rewardType == 0){
  ...
  function calculateGEN2Rewards(uint16  tokenId) external view returns (uint128) {
    TimeStake memory stake = gen2StakeByToken[gen2Hierarchy[tokenId]];
    return _calculateGEN2Rewards(tokenId, stake.reward_type, stake.time);
  }

  function calculateNonMigratedGEN2Rewards(uint16 tokenId, uint16 rewardType) external view returns (uint128) {
    IChubbyKaijuDAOStakingV2Old.TimeStake memory stake = chubbykaijuStakingOld.gen2StakeByToken(chubbykaijuStakingOld.gen2Hierarchy(tokenId));
    return _calculateGEN2Rewards(tokenId, rewardType, stake.time);
  }
