...
contract NFTBridge is NFTBridgeGovernance {
    ...
    function transferNFT(address token, uint256 tokenID, uint16 recipientChain, bytes32 recipient) public payable returns (uint64 sequence) {
        ...
        emit TransferNFT(sequence, token, tokenID, recipientChain, msg.sender, address(uint160(uint256(recipient))));
    }
    ...
    function transferNFTBatch(address[] memory tokenList, uint256[] memory tokenIDList, uint16 recipientChain, bytes32 recipient) public payable returns (uint64 sequence)  {
        ...
        for (uint256 i = 0; i < tokenList.length; i++) {
            emit TransferNFT(sequence, tokenList[i], tokenIDList[i], recipientChain, msg.sender, address(uint160(uint256(recipient))));
        }
    }
    ...
    function logTransfer(NFTBridgeStructs.Transfer memory transfer, uint256 callValue) internal returns (uint64 sequence) {
        ...
        address dstContractAddress = address(uint160(uint256(bridgeContracts(transfer.toChain))));
        sequence = zkBridge().publishMessage{value : callValue}(transfer.toChain, dstContractAddress, encoded);
    }
    ...
    function logTransferBatch(NFTBridgeStructs.TransferBatch memory transfer, uint256 callValue) internal returns (uint64 sequence) {
        ...
        address dstAddress = address(uint160(uint256(bridgeContracts(transfer.toChain))));
        sequence = zkBridge().publishMessage{value : callValue}(transfer.toChain, dstAddress, encoded);


    }

    function completeTransfer(bytes memory proofBlob) public {
        _completeTransfer(proofBlob);
    }

    function completeTransferBatch(bytes memory proofBlob) public {
        _completeTransferBatch(proofBlob);
    }

    // Execute a Transfer message
    function _completeTransfer(bytes memory proofBlob) internal {
        (IZKBridge.VM memory vm, bool valid, string memory reason) = zkBridge().validateTransactionProof(proofBlob);

        require(valid, reason);
        require(bridgeContracts(vm.srcChainId) == vm.srcAddress, "invalid emitter");
        require(vm.dstAddress == address(this), "invalid destination");
        ...
        require(vm.dstChainId == chainId(), "invalid target chain");
        ...
        emit ReceiveNFT(vm.sequence, address(uint160(uint256(transfer.tokenAddress))), address(transferToken), transfer.tokenID, transfer.tokenChain, vm.srcChainId, transferRecipient);
    }

    function _completeTransferBatch(bytes memory proofBlob) internal {
        (IZKBridge.VM memory vm, bool valid, string memory reason) = zkBridge().validateTransactionProof(proofBlob);
        ...
        NFTBridgeStructs.TransferBatch memory transfer = parseTransferBatch(vm.payload);
        ...
        for (uint256 i = 0; i < transfer.tokenAddress.length; i++) {
            ...
            if (transfer.tokenChain[i] == chainId()) {
                transferToken = IERC721(address(uint160(uint256(transfer.tokenAddress[i]))));
            } else {
            ...
            emit ReceiveNFT(vm.sequence, address(uint160(uint256(transfer.tokenAddress[i]))), address(transferToken), transfer.tokenID[i], transfer.tokenChain[i], vm.srcChainId, transferRecipient);
        }
    ...
    function parseTransferBatch(bytes memory encoded) public pure returns (NFTBridgeStructs.TransferBatch memory transfer) {
        (transfer.tokenAddress,
        transfer.tokenChain,
        transfer.symbol,
        transfer.name,
...
//    }
    ...
    function setZKBridge(address h) internal {
    ...
    function setPendingImplementation(address pendingImplementation) internal {
        _state.pendingImplementation = pendingImplementation;
    }

    function setToUpdateTime(uint256 toUpdateTime) internal {
        _state.toUpdateTime = toUpdateTime;
    }

    function setLockTime(uint256 lockTime) public {
        require(_state.owner == msg.sender, "Ownable: caller is not the owner");
...
contract NFTBridgeGovernance is NFTBridgeGetters, NFTBridgeSetters, ERC1967Upgrade {
    ...
    function registerChain(uint16 chainId, bytes32 contractAddress) public onlyOwner {
        setBridgeImplementation(chainId, contractAddress);
    }
    ...
    function submitContractUpgrade(address newImplementation) public onlyOwner {
        ...
        setPendingImplementation(newImplementation);
        setToUpdateTime(block.timestamp + lockTime());
    ...
    function confirmContractUpgrade() public onlyOwner {
        ...
        setPendingImplementation(address(0));
...
interface IZKBridge is Structs {

    event LogMessagePublished(address indexed sender, uint16 indexed dstChainId, address dstAddress, uint64 sequence, bytes payload);

    function publishMessage(uint16 dstChainId, address destination, bytes memory payload) external payable returns (uint64 sequence);

    function validateTransactionProof(bytes memory proofBlob) external view returns (Structs.VM memory vm, bool valid, string memory reason);
}
