...
contract AddingData is AccessControl {
    ...
    mapping(uint16 => PropertyAttributes) public defaultProperties;
    ...
    * @param ids the key element is this array which matches with other half 
    ...
    function addNftData(
        ...
        for (uint16 i = 0; i < ids.length; i++) {
            ...
            x.properties.feature_5 = fets5[i];
            x.properties.feature_6 = fets6[i];
    ...
    *@notice called by the extension contract 
    @param nftId the id of the property NFT 
    @param extId the id of the extension NFT 
     */
    function addExtensionToMetadata(uint16 nftId, uint extId)external onlyRole(CONTRACT_ROLE){
    ...
    *@param extId the ID of the extension NFT 
     */
    function removeExtensionFromMetadata(uint16 nftId, uint extId)external onlyRole(CONTRACT_ROLE){
        for (uint16 i; i<defaultProperties[nftId].extensionIds.length; i++){
    ...
    function checkOwnershipOfExtension(uint16 propId, uint extTokenId)external onlyRole(CONTRACT_ROLE) view returns(bool){
        for (uint16 i; i<defaultProperties[propId].extensionIds.length; i++){
    ...
    function extensionNames(uint16 propId)internal view returns(bytes memory){
        if(defaultProperties[propId].extensionCount>0){
            return ExtContract.extensionNameList(defaultProperties[propId].extensionIds);
        }else{
            bytes memory st = abi.encodePacked(
            '"},{"trait_type": "Extensions", "value": "',"None");
            return st;
        }
    }
    ...
    function sectionFour(uint16 propId)external view returns(bytes memory){
        ...
        bytes memory dataURI = abi.encodePacked(
            ...
            '"},{"trait_type": "Feature 5", "value": "',
            defaultProperties[propId].properties.feature_5,
...
contract MetropolisWorldGenesis is ERC721, AccessControl {
    ...
    constructor(address payable paymentSplit ) ERC721("test contract", "METC") {
    ...
    *@param passportContract the contract address of the passport contract. 
     */
    function setInterfaceContracts(address passportContract, address wlContract, address tokContract, address conAdd)public onlyRole(UPDATER_ROLE){
    ...
    *@param extAddress the contract address of the extensions contract. 
     */
    function setContractRole(address extAddress)public onlyRole(UPDATER_ROLE){
    ...
    *@param nftId the property Id of the nft you want to get the price for. 
     */
    function priceOfNft(uint16 nftId) public view returns (uint256) {
    ...
    *@param nftId the property ID of the speific NFT you want the metadata for. 
     */
    function metaData(uint16 nftId)
        public
        view
        returns (PropertyAttributes memory)
    {
    ...
    *@dev the internal function used to process mints 
     */
    function _internalMint(uint passId, uint16 nftId)internal {
        ...
        _tokenIds.increment();
        if (passId != 0){
            PassContract.removeCityWlSpot(passId, "city1", nftId);
        }
    ...
    */
    function paidMint(uint passId, uint16 nftId)public payable{
        //check you owwn this passport. 
        require(PassContract.ownerOf(passId)==msg.sender, 'You must be the owner of this passport');
        require(passId > 0, "Not a valid passport ID");
        require(msg.value >= DataContract.price(nftId), "Not paid enough, sorry");
        _internalMint(passId, nftId);
    ...
    * @param nftId  the id of the property the user is trying to mint 
    */
    function freeMint(uint16 passId, uint16 nftId)public onlyRole(UPDATER_ROLE){
    ...
    /**
        ...
        *@param propId the token id of the property the extension is attached to 
    ...
    */
    function attachExtension(uint propId, uint extTokenId, address owner)external onlyRole(CONTRACT_ROLE){
    ...
    *@param extTokenId the id of the extension id to be checked 
     */
    function checkExtensionOwnership(uint16 propId, uint extTokenId)external view onlyRole(CONTRACT_ROLE) returns(bool){
    ...
    */
    function withdraw() public onlyRole(BALANCE_ROLE) {
        uint256 balance = address(this).balance;
        _paymentSplit.transfer(balance);
    }
}
