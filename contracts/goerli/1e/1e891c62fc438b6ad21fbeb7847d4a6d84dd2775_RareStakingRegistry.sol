...
import {IRareStake} from "../token/IRareStake.sol";
...
contract RareStakingRegistry is IRareStakingRegistry, AccessControlUpgradeable, UUPSUpgradeable {
  ...
  //////////////////////////////////////////////////////////////////////////*/
  mapping(address => IRareStakingRegistry.Info) private userToStakingInfo;
  ...
  EnumerableMapUpgradeable.AddressToUintMap private amountStakedOnUser;
  ...
  function _authorizeUpgrade(address) internal view override {
    if (!hasRole(DEFAULT_ADMIN_ROLE, msg.sender)) revert IRareStakingRegistry.Unauthorized();
  }
  ...
  function setStakingAddress(address _user, address _stakingAddress) external {
    if (!hasRole(STAKING_INFO_SETTER_ROLE, msg.sender)) revert IRareStakingRegistry.Unauthorized();
    if (userToStakingInfo[_user].stakingAddress != address(0))
      revert IRareStakingRegistry.StakingContractAlreadyExists();
    userToStakingInfo[_user] = IRareStakingRegistry.Info("", "", _stakingAddress);
  ...
  function increaseAmountStaked(
    ...
    if (!hasRole(STAKING_STAT_SETTER_ROLE, msg.sender)) revert IRareStakingRegistry.Unauthorized();
    (, uint256 amtStaked) = amountStakedByUser.tryGet(_staker);
    amountStakedByUser.set(_staker, amtStaked + _amount);

    (, uint256 amtStakedOn) = amountStakedOnUser.tryGet(_stakedOn);
    amountStakedOnUser.set(_stakedOn, amtStakedOn + _amount);
  }
  ...
  function decreaseAmountStaked(
    ...
    if (amtStakedOn - _amount == 0) {
      amountStakedOnUser.remove(_stakedOn);
    } else {
      amountStakedOnUser.set(_stakedOn, amtStakedOn - _amount);
    }
  }
  ...
  /// @inheritdoc IRareStakingRegistry
  function setStakingInfo(string calldata _name, string calldata _symbol) external {
    userToStakingInfo[msg.sender].name = _name;
    userToStakingInfo[msg.sender].symbol = _symbol;
    emit IRareStakingRegistry.StakingInfoUpdated(msg.sender, _name, _symbol);
  }
  ...
  /// @inheritdoc IRareStakingRegistry
  function getStakingInfoForUser(address _user) external view returns (IRareStakingRegistry.Info memory) {
    return userToStakingInfo[_user];
  }
  ...
  function getTotalAmountStakedOnUser(address _user) external view returns (uint256 amount) {
    (, amount) = amountStakedOnUser.tryGet(_user);
  }
  ...
  /// @dev This function is intended to be called off chain.
  function getAllStakedOn() external view returns (address[] memory) {
    uint256 length = amountStakedOnUser.length();

    address[] memory stakedOn = new address[](length);

    for (uint256 i = 0; i < length; i++) {
      (address staker, ) = amountStakedOnUser.at(i);
      stakedOn[i] = staker;
    }
  ...
  function getUsersForStakingAddresses(address[] calldata _stakingAddrs) external view returns (address[] memory) {
    ...
    for (uint256 i = 0; i < _stakingAddrs.length; i++) {
      try IRareStake(_stakingAddrs[i]).getUserBeingStakedOn() returns (address user) {
        users[i] = user;
      } catch {
        users[i] = address(0);
      }
...
interface IRareStake {
  ...
  /// @notice Total rewards available this round.
  /// @return uint256 Amount of $RARE tokens rewarded this round.
  function getTotalRewards() external view returns (uint256);

  /// @notice Query rewards for this user this round.
  /// @param _userAddress Address of the user to get rewards.
  /// @return uint256 Amount of $RARE tokens rewarded this round.
  function getRewardsForUser(address _userAddress) external view returns (uint256);
