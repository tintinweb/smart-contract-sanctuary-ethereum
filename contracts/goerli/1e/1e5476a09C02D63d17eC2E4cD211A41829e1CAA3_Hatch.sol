...
contract Hatch is ERC721A, Ownable, ReentrancyGuard {
    ...
    constructor(string memory _initBaseURI) ERC721A("Hatch", "HTC") {
        setBaseURI(_initBaseURI);
    }

    function mint(uint256 _mintAmount) external payable {
        require(mintIsActive, "Paused");
        require(_mintAmount <= maxPublicMints, "Wrong amount");
        require(_totalMinted() + _mintAmount <= maxSupply, "Soldout");
    ...
    function giveAway(
        ...
        require(_totalMinted() + totalQuantity <= maxSupply, "Too many");
        for (uint256 i = 0; i < addressList.length; ++i) {
            _mint(addressList[i], quantityList[i]);
        }
    }
