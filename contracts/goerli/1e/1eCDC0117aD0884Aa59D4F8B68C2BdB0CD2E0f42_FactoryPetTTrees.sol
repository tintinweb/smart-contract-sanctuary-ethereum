...
import {Context} from "./access/Context.sol";
import {Ownable} from "./access/Ownable.sol";

contract FactoryPetTTrees is IFactoryERC721, OpenSeaDefaultOperatorFilterer, Context, Ownable {
    uint currentTokenId;
    uint immutable supplyCap;

    uint256 NUM_OPTIONS = 2;
    uint256 SINGLE_OPTION = 0;
    uint256 SOME_OPTION = 1;
    uint256 MULTIPLE_OPTION = 2;
    uint256 MANY_OPTION = 3;
    uint256 NUM_IN_MULTIPLE_OPTION = 5;
    ...
    function canMint(uint256 _optionId) external override view returns (bool) {
        return currentTokenId < supplyCap - (_optionId * NUM_IN_MULTIPLE_OPTION);
    }
    ...
    function tokenURI(uint256 _optionId) external view override returns (string memory) {
        string memory _baseURI = "ipfs://bafkreig7cg7wshoiew2sk3fg7fswljpcbbijrnhh5lftihmepf4folhtz4/";
    ...
    function mint(uint256 _optionId, address _toAddress) 
        external override 
        onlyOwner onlyAllowedOperator(_toAddress) 
    {
        PetTTrees nftree = PetTTrees(nftAddress);        
        
        if (_optionId == SOME_OPTION) {
            nftree.mintTo(_toAddress, 1);
        } else if (_optionId == SOME_OPTION) {
            nftree.mintTo(_toAddress, SOME_OPTION * NUM_IN_MULTIPLE_OPTION);
        } else if (_optionId == MULTIPLE_OPTION) {
            nftree.mintTo(_toAddress, MULTIPLE_OPTION * NUM_IN_MULTIPLE_OPTION);
        } else if (_optionId == MANY_OPTION) {
            nftree.mintTo(_toAddress, MANY_OPTION * NUM_IN_MULTIPLE_OPTION);
        }
