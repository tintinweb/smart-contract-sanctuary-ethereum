    ...
    LaunchpadSignature
{
    ...
    IMatrix public matrixToken;
    IERC20 public feeToken;
    uint256 private feeBalance;
    uint256 public matrixPrice;
    Counters.Counter private _tokenIds;
    EnumerableSet.AddressSet private whitelistedTokens20;
    EnumerableSet.AddressSet private whitelistedTokens721;

    mapping(address => uint256) private coinNonce; // user => nonceCouner;
    mapping(address => uint256) private matrixNonce; // user => nonceCouner;
    ...
    event CreatedMatrix(address sender, string uri);
    ...
    constructor(string memory _name, string memory _version) {
        ...
        _setupRole(OWNER_LAUNCHPAD_ROLE, msg.sender);
        _setRoleAdmin(OWNER_LAUNCHPAD_ROLE, OWNER_LAUNCHPAD_ROLE);
    }

    function setPlatformToken(address _nftToken721) public onlyOwner {
        nftToken721 = IERC721(_nftToken721);
    }

    function setMatrixToken(address _matrixToken) public onlyOwner {
    ...
    function setMatrixPrice(uint256 _matrixPrice) public onlyOwner {
        matrixPrice = _matrixPrice;
    }

    function addTokensToWhitelist(
        address[] memory tokens,
        bool[] memory tokensType
    ) external onlyOwner {
    ...
    function getWhitelistedTokens(bool tokenType)
        public
        view
    ...
    function createCoin(
        ...
        require(coinNonce[msg.sender] < nonce, "Wrong nonce.");
        require(
            matrixToken.ownerOf(matrixId) == msg.sender,
            "You are not the owner of the matrix."
        );

        coinNonce[msg.sender] = nonce;

        freezingBalances[_tokenIds.current()] = TokenData(amount, tokenAddress);

        if (tokenAddress == address(0)) {
            require(amount == msg.value, "Value is not equal.");
        } else {
            require(
                whitelistedTokens20.contains(tokenAddress),
        ...
        matrixToken.burn(matrixId);
        nftToken721.mint(msg.sender, _tokenIds.current(), uri);
        _tokenIds.increment();
        emit CreateCoin(msg.sender, tokenAddress, amount, uri);
    }

    function unfreezeCoin(uint256 tokenId) external nonReentrant {
        require(
            nftToken721.ownerOf(tokenId) == msg.sender,
    ...
    function createMatrix(
        ...
        require(
            hasRole(
                OWNER_LAUNCHPAD_ROLE,
                _getCreateMatrixSigner(msg.sender, uri, nonce, v, r, s)
            ),
            "Action is inconsistent."
        );
        _createMatrix(msg.sender, nonce, uri);
        emit CreatedMatrix(msg.sender, uri);
    }
    ...
    function _createMatrix(
        ...
        feeBalance += matrixPrice;
        feeToken.safeTransferFrom(receiver, address(this), matrixPrice);
        matrixToken.mint(receiver, uri);
    }
    ...
    /// @dev Withdraw Fee token balance from contract address

    function unlockTokens() external onlyOwner {
        uint256 balance = feeBalance;
        require(balance > 0, "balance must be bigger than 0");
        feeBalance = 0;
...
contract LaunchpadSignature {
    ...
    bytes32 private constant SIGNDATA_TYPEHASH =
        keccak256(
            "CreateCoinData(uint256 amount,uint256 nonce,uint256 matrixId,address buyer,address tokenAddress,string uri)"
        );
    bytes32 private constant MATRIXDATA_TYPEHASH =
        keccak256("CreateMatrixData(uint256 nonce,address buyer,string uri)");
    bytes32 private constant COMBINEDATA_TYPEHASH =
        keccak256(
            "CombineMatrixData(uint256[] matrixIds,uint256 nonce,address buyer,string uri)"
        );
    ...
    function _hash(CreateCoinData memory signData)
        internal
        pure
        returns (bytes32)
    {
    ...
    function _hash(CreateMatrixData memory signData)
        internal
        pure
        returns (bytes32)
    {
    ...
    function _hash(CombineMatrixData memory signData)
        internal
        pure
        returns (bytes32)
    {
    ...
    function _getCreateMatrixSigner(
        ...
        bytes32 digest = keccak256(
            abi.encodePacked(
                "\x19\x01",
                eip712DomainSeparator,
                _hash(CreateMatrixData({nonce: nonce, buyer: buyer, uri: uri}))
            )
        );
        return ecrecover(digest, v, r, s);
    }
