...
contract ExchangeChallenge is IExchangeChallenge, Ownable, ERC1155Holder{
    ...
    uint256 public PRIMARY_TOKEN_FEE_PERCENT = 5e18;
    uint256 public SECONDARY_TOKEN_FEE_PERCENT = 7e18;
    ...
    * @param eventIdAirDropChallenge a tracking id used to sync db
    ...
    function getAirdropFeePercent(address token) external returns (uint256){
       return _getAirdropFeePercent(token);
    }

    function _getAirdropFeePercent(address token) internal returns (uint256){
    ...
    function _getTokenAirdropFee(address token, uint256 tokenAmount) internal returns (uint256){
