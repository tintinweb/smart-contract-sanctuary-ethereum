...
contract StakeContract is Pausable, Ownable, ReentrancyGuard {
    ...
    function transferToken(address to,uint256 amount, string calldata _tokenType) external onlyOwner{
        if(keccak256(bytes(_tokenType)) == keccak256(bytes("usdc"))){
            require(USDCToken.balanceOf(address(this)) >= amount, "insufficient token balance");
            require(USDCToken.transfer(to, amount), "Token transfer failed!");  
        }else{
            require(USDTToken.balanceOf(address(this)) >= amount, "insufficient token balance");
    ...
    function approveClaimAmount(uint256 _indexNum, string calldata _tokenType) public onlyOwner {
        if(keccak256(bytes(_tokenType)) == keccak256(bytes("usdc"))){
            uint256 claimAmount = USDCstakeInfos[_msgSender()].claimAmount - usdcRequerstList[_indexNum]._amount;
            uint256 approvedAmount = USDCstakeInfos[_msgSender()].approvedAmount + usdcRequerstList[_indexNum]._amount;

            delete usdcRequerstList[_indexNum];
            
            USDCstakeInfos[_msgSender()].claimAmount = claimAmount;
            USDCstakeInfos[_msgSender()].approvedAmount = approvedAmount;
        }else{

            uint256 usdtclaimAmount = USDTstakeInfos[_msgSender()].claimAmount - usdtRequerstList[_indexNum]._amount;
            uint256 usdtapprovedAmount = USDTstakeInfos[_msgSender()].approvedAmount + usdtRequerstList[_indexNum]._amount;

            delete usdtRequerstList[_indexNum];
            
            USDTstakeInfos[_msgSender()].claimAmount = usdtclaimAmount;
            USDTstakeInfos[_msgSender()].approvedAmount = usdtapprovedAmount;
        }
    }
    ...
    function unstakeToken(uint256 _unstakeAmount, string calldata _tokenType) external whenNotPaused amountGreaterThenZero(_unstakeAmount) nonReentrant returns (bool){
        ...
        if(keccak256(bytes(_tokenType)) == keccak256(bytes("usdc"))){
            require(USDCstakeInfos[_msgSender()].rewards+USDCstakeInfos[_msgSender()].amount >= _unstakeAmount, "Withdraw amount is greater than stake amount");
        }else{
            require(USDTstakeInfos[_msgSender()].rewards+USDTstakeInfos[_msgSender()].amount >= _unstakeAmount, "Withdraw amount is greater than stake amount");
        }
