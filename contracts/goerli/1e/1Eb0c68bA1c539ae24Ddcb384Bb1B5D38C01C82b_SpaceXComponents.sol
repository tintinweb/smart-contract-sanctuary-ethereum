...
contract SpaceXComponents is ERC721, Ownable {
    ...
    function random() public view returns (uint) {
        ...
        return randomNum.mul(805).mul(10**4).div(100000);
    }

    function viewDividend(address account) public view returns (uint) {

        if (dividendRecord[todayZero()][_msgSender()] != 0) {
            return 0;
        }
        ...
        uint baseDividend = 155_000_000;
        return baseDividend
                .add(random())
                .mul( accountIndex[account].length );
    }
