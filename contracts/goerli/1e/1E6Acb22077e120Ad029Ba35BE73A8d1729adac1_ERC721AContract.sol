...
contract ERC721AContract is ERC721A, Ownable, PaymentSplitter, DefaultOperatorFilterer, ReentrancyGuard, Referable {
    ...
    mapping(MerkleRoot => bytes32) public merkleRoots;
    Token public token;
    ...
    function setMerkleRoot(MerkleRoot _type, bytes32 _root) external onlyOwner {
        require(_type == MerkleRoot.Claim || _type == MerkleRoot.Sale, "Bad type");
        merkleRoots[_type] = _root;
    }
    ...
    function updateSaleState(
        ...
        if (_preSaleIsActive && _preSalePhase == 2) require(merkleRoots[MerkleRoot.Sale] != "", "Root undefined");
        if (_claimIsActive) require(merkleRoots[MerkleRoot.Claim] != "", "Root undefined");
    ...
    function isEligible(
        ...
        if(token.claimIsActive) {
                _isEligibleClaim = _hasSupply && (_quantity <= (_maxMint - _hasClaimed)) &&
                    MerkleProof.verify(_proof, merkleRoots[MerkleRoot.Claim], keccak256(abi.encode(msg.sender, _maxMint)));
        }
        if(token.preSaleIsActive) {
            ...
            if (token.preSalePhase == 2) {
                _isEligiblePreSale = _hasSupply && (_quantity <= (_maxMint - _hasMinted)) &&
                    MerkleProof.verify(_proof, merkleRoots[MerkleRoot.Sale], keccak256(abi.encode(msg.sender, _maxMint)));
            }
        }
        if (token.pubSaleIsActive) {
            if (token.pubMintType == PublicMintType.PerWallet) {
                _isEligiblePubSale = _hasSupply && _quantity <= token.pubMaxMint && (_quantity <= (_maxMint - _hasMinted));
            } else {
    ...
    function _mintTo(
        ...
        (bool _isEligibleClaim, bool _isEligiblePreSale, bool _isEligiblePubSale) = isEligible(_address, uint16(_quantity), _maxMint, _proof);
        require(_isEligibleClaim || _isEligiblePreSale || _isEligiblePubSale, "Not eligible");
        if (_isEligibleClaim) {
            require(_value == 0, "ETH incorrect");
            hasClaimed[_address] += uint16(_quantity);
        }
        ...
        if (!_isEligibleClaim && !_isEligiblePreSale && _isEligiblePubSale) {
            ...
            if (token.pubMintType == PublicMintType.PerWallet) hasMinted[_address] += uint16(_quantity);
        }
    ...
    function mint(
        ...
        _mintTo(msg.sender, _quantity, _maxMint, _proof, _referrer, msg.value);
    }
    ...
    function reserve(address[] memory _addresses, uint16[] memory _quantities) external onlyOwner {
