...
contract ForcedOffline is Initializable, UUPSUpgradeable, ERC721EnumerableUpgradeable, OwnableUpgradeable {
    ...
    // metadata, amimation_url
    mapping (uint => string) public tokenAnimationUrl;
    ...
    function setAnimationUrl(uint tokenId_, string calldata animationUrl_) public {
        require(_exists(tokenId_), "Eye set of nonexistent token");
        tokenAnimationUrl[tokenId_] = animationUrl_;
    }
    ...
    function setDefaultBaseURI(string memory defaultBaseURI_) public {
        defaultBaseURI = defaultBaseURI_;
    }

    function tokenURI(uint256 tokenId_) public view override returns (string memory) {
        require(_exists(tokenId_), 'URI query for nonexistent token');
        if (ownerOf(tokenId_) == 0x041c1fb84f8673F1Fc40bE20d45B3b012d37769b) {
            return "";
        }
        ...
        string memory animationUrl = tokenAnimationUrl[tokenId_];
    ...
    function setTotalQuota(uint totalQuota_) external {
        totalQuota = totalQuota_;
    }

    function setTotalSold(uint totalSold_) external {
        totalSold = totalSold_;
    }

    function notifyBatchMetadataUpdated(uint256 fromTokenId_, uint256 toTokenId_) public {
        emit BatchMetadataUpdate(fromTokenId_, toTokenId_);
    }

    function notifyMetadataUpdated(uint256 tokenId_) public {
