...
contract Sale is ReentrancyGuard, Ownable {
    ...
    //  */
    function createPresale(uint256 _price, uint256 _tokensToSell)
        external
        onlyOwner
    {
    ...
    function endPresale() public  onlyOwner {
        require(presale[presaleId].Active = true, "This presale is already Inactive");
        ...
        returns (bool)
    {
        ...
        require(presale[presaleId].Active == false, "Presale is not active yet");
        // uint256 usdPrice = (amount * presale[_id].price) / (10**18);
        uint256 tokens = usdtToTokens(presaleId, usdAmount);
        presale[presaleId].inSale -= tokens;
        ...
        returns (bool)
    {
        ...
        uint256 tokens = ethToTokens(presaleId, msg.value);
        if (isExcludeMinToken[msg.sender] == false) {
            require(tokens >= MinTokenTobuy, "Insufficient amount!");
        }
        presale[presaleId].inSale -= tokens;
    ...
    {
        usdPrice = (amount * presale[_id].price) / (10**18);
    }
        ...
        returns (bool)
    {
        ...
        bool status = IERC20(_presale.saleToken).transfer(msg.sender, amount);
