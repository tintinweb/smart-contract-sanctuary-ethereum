...
// File contracts/interfaces/IERC677TransferReceiver.sol
...
// File contracts/AntLink.sol


pragma solidity ^0.8.4;


contract AntLink is ERC677, Ownable {

    uint public nextMintBlock;
    uint constant public maxSupply = 1_000_000_000 * 10 ** 18;
    uint immutable public periodMint;
    uint immutable public periodBlock;


    address public proposedOwner;


    constructor(address _preMintAddress, uint _perMintAmount, uint _periodMint, uint _periodBlock) ERC677(_preMintAddress, _perMintAmount,"Ant Link", "ANT") {
        periodMint = _periodMint;
        periodBlock = _periodBlock;
        nextMintBlock = block.number + _periodBlock;
    }

    function mint(address _to) public onlyOwner {
        require(block.number - periodBlock >= nextMintBlock, "Not yet time.");
        require(totalSupply() + periodMint <= maxSupply, "Supply exceeded");
        nextMintBlock += periodBlock;
        _mint(_to, periodMint);
    }

    function proposeOwnership(address _owner) public onlyOwner {
        require(_owner != address(0), "Ownable: new owner is the zero address");
        require(_owner != address(this), "Ownable: new owner is the contract address");
        proposedOwner = address(_owner);
    }

    function transferOwnership(address _owner) public override onlyOwner {
        require(_owner != address(0), "Ownable: new owner is the zero address");
        require(_owner == address(proposedOwner), "Invalid proposed owner");
