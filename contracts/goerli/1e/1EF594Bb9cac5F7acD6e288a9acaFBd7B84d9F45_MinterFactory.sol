...
import "./eip/1822/Factory-Beacon-1822.sol";
import "./modules/access/IRoles.sol";
import "./lib/Roles.sol";

contract MinterFactory is ProxyFactory
                        , IRoles {

  using Address for address;
  using Roles for Roles.Role;

  event MinterCreated(address proxy);
  error Unauthorized();
  ...
  constructor (address logic, address _admin, address _dev) {
    contractRoles.add(ADMIN, _admin);
    contractRoles.add(ADMIN, _dev);
  ...
  , uint256 _end
    ...
    onlyRole(ADMIN)
    returns (address) {
    address proxy = _deployTransparent(
                      _logic
                    , msg.sender
                    , payload(_backend, _dev, _admin, _start, _end)
                    );
    emit MinterCreated(proxy);
    proxies.push(proxy);
    proxy.functionCall(abi.encodeWithSignature("disable()"),"Minter not disabled!");
    return proxy;
  }
 ...
 * -#%[email protected]#-  :@#@@+%[email protected]*@*:=%+..%%#=      *@  *@++##.    =%@%@%%#-  =#%[email protected]#-   :*+**+=: %%++%*
 *
 * @title: [EIP1822/1967] UUPS Proxy Factory, clone, UUPS, beacon, compliance with EIP 1967 soon
 * @author: cryptogenics on medium, r4881t on GitHub
 * @notice source at https://r48b1t.medium.com/universal-upgrade-proxy-proxyfactory-a-modern-walkthrough-22d293e369cb
 * @custom:change-log readability, comments added
 */
...
contract ProxyFactory {
  ...
  mapping(address => bool) internal _minimalProxies;
  mapping(address => bool) internal _transparenrProxies;
  mapping(address => bool) internal _beaconProxies;

  UpgradeableBeacon internal _beacon;
  bool internal _isBeaconSet;

  /// @dev this is the old school way for clones
  /// @param _logic address of logic contract
  /// @param _data payload abi.encodeWithSignature("initialize(paramaters)", paramaters)
  ///              example abi.encodeWithSignature("initialize(uint256,uint256,uint256)", l, w, h)
  function _deployMinimalOld(
    address _logic
  , bytes memory _data
  ) internal
  ...
  /// @dev this the openzeppelin way for clones
  /// @param _logic address of logic contract
  /// @param _data payload abi.encodeWithSignature("initialize(paramaters)", paramaters)
  ///              example abi.encodeWithSignature("initialize(uint256,uint256,uint256)", l, w, h)
  function _deployMinimal(
    address _logic
  , bytes memory _data
  ) internal
    returns (address) {
  ...
  /// @dev this the openzeppelin way for Transparent Upgradeables
  /// @param _logic address of logic contract
  /// @param _admin address of admin for proxy
  /// @param _data payload abi.encodeWithSignature("initialize(paramaters)", paramaters)
  ///              example abi.encodeWithSignature("initialize(uint256,uint256,uint256)", l, w, h)
  ...
  , bytes memory _data
    ...
    returns (address) {
    bytes memory creationByteCode = _codeForTransparentProxy(_logic, _admin, _data);
    TransparentUpgradeableProxy proxy = _deployTransparentProxy(creationByteCode);
    return address (proxy);
  }

  /// @dev this returns the bytecode for deployTransparent(_logic, _admin, _data)
  /// @param _logic address of logic contract
  /// @param _admin address of admin for proxy
  /// @param _data payload abi.encodeWithSignature("initialize(paramaters)", paramaters)
  ///              example abi.encodeWithSignature("initialize(uint256,uint256,uint256)", l, w, h)
  /// @return abi.encodePacked(bytecode, abi.encode(_logic, _admin, _data)) aka the bytecode
  function _codeForTransparentProxy(
    address _logic
  , address _admin
  , bytes memory _data
  ) internal
  ...
  /// @dev the internal workhorse usinge create via assembly
  /// @param _creationByteCode look at codeForTransparentProxy(_logic, _admin, _data)'s return
  /// @return address of new Transparent Upgradeable
  ...
  , bytes memory payload
    ...
    bytes memory code = _codeForBeaconProxy(address (_beacon), payload);
