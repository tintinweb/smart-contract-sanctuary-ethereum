...
/**
 * @title BaseDAO1155
 * @dev only NFT holders of specific ERC-1155 project can create and vote on proposals
 */

interface IBaseDAOContract1155 {
    function balanceOf(address, uint256) external view returns (uint256);

    function supportsInterface(bytes4 interfaceId) external view returns (bool);
}

contract BaseDAO1155 is Initializable {
    ...
    /// @notice valid token IDs that can create proposals
    uint256[] public validTokensToCreateProposal;

    /// @notice valid token IDs that can vote on proposals
    uint256[] public validTokensToVoteonProposal;

    /// @notice NFT contract address of which holders can interact with this contract
    IBaseDAOContract1155 BaseDAOContract;
    ...
    modifier onlyOwner() {
        require(msg.sender == owner, "BaseDAO1155: only owner");
        _;
    }
        ...
        initializer
    {
        require(
            isBase == false,
            "BaseDAO1155: this is the base contract,cannot initialize"
        );

        require(
            owner == address(0),
            "BaseDAO1155: contract already initialized"
        );

        require(owner_ != address(0), "BaseDAO1155: Owner address cannot be 0");

        (
            string memory name_,
            string memory symbol_,
            address _nftContract,
            uint256[] memory _tokenIDsEligibleToCreateProposal,
            uint256[] memory _tokenIDsEligibleToVoteonProposal
        ) = abi.decode(data, (string, string, address, uint256[], uint256[]));

        require(
            _exists(_nftContract),
            "BaseDAO1155: Address must be a contract"
        );
        ...
        BaseDAOContract = IBaseDAOContract1155(_nftContract);

        validTokensToCreateProposal = _tokenIDsEligibleToCreateProposal;

        validTokensToVoteonProposal = _tokenIDsEligibleToVoteonProposal;

        if (!BaseDAOContract.supportsInterface(0xd9b67a26)) {
            revert("BaseDAO1155: Contract does not support ERC1155");
        }
    }
    ...
    function changeAccess(bool _isOpen) public onlyOwner {
        require(
            _isOpen != isOpen,
            "BaseDAO1155: Access already set to this value"
        );
     ...
     * @param _tokenIDsEligibleToCreateProposal array of token IDs that can create proposals
     * @param _tokenIDsEligibleToVoteonProposal array of token IDs that can vote on proposals
     **/
    function change1155Contract(
        address _newAddr,
        uint256[] memory _tokenIDsEligibleToCreateProposal,
        uint256[] memory _tokenIDsEligibleToVoteonProposal
    ) public onlyOwner {
        require(_exists(_newAddr), "BaseDAO1155: Address must be a contract");

        require(
            _newAddr != address(BaseDAOContract),
            "BaseDAO1155: Address already set"
        );

        BaseDAOContract = IBaseDAOContract1155(_newAddr);
    ...
    /**
     * @notice function that allows the DAO owner to change valid token IDs
     * @param _tokenIDsEligibleToCreateProposal array of token IDs that can create proposals
     * @param _tokenIDsEligibleToVoteonProposal array of token IDs that can vote on proposals
     **/
    function changeValidTokenIds(
        uint256[] memory _tokenIDsEligibleToCreateProposal,
        uint256[] memory _tokenIDsEligibleToVoteonProposal
    ) public onlyOwner {
        validTokensToCreateProposal = _tokenIDsEligibleToCreateProposal;
    ...
    function checkProposalEligibility(address _proposer)
        ...
        returns (bool)
    {
        for (uint256 i = 0; i < validTokensToCreateProposal.length; i++) {
            if (
                BaseDAOContract.balanceOf(
                    _proposer,
                    validTokensToCreateProposal[i]
                ) >= 1
            ) {
                return true;
            }
    ...
    function checkVoteEligibility(address _voter) private view returns (bool) {
        for (uint256 i = 0; i < validTokensToVoteonProposal.length; i++) {
            if (
                BaseDAOContract.balanceOf(
                    _voter,
                    validTokensToVoteonProposal[i]
                ) >= 1
            ) {
                return true;
            }
        ...
        public
    {
        if (!isOpen) {
            require(
                ...
                "BaseDAO1155: Only NFT holders can put forth Proposals"
            );
        }
    ...
    function voteOnProposal(uint256 _id, bool _vote) public {
        require(
            ...
            "BaseDAO1155: This Proposal does not exist"
        );
        if (!isOpen) {
            require(
                checkVoteEligibility(msg.sender),
                "BaseDAO1155: You can not vote on this Proposal"
            );
        }
        require(
            !Proposals[_id].voteStatus[msg.sender],
            "BaseDAO1155: You have already voted on this Proposal"
        );
        require(
            block.number <= Proposals[_id].deadline,
            "BaseDAO1155: The deadline has passed for this Proposal"
        );
    ...
    function countVotes(uint256 _id) public onlyOwner {
        ...
        require(
            ...
            "BaseDAO1155: Voting has not concluded"
        );
        require(
            !Proposals[_id].countConducted,
            "BaseDAO1155: Count already conducted"
        );
