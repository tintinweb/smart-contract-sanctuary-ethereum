...
contract teste is ERC721AQueryable, ERC721ABurnable, Ownable {
    ...
    uint public maxFreeMintPerAccount = 1;
    ...
    function mint(uint _amount) external payable {
        ...
        require((_amount - _mintFreeAmount(msg.sender , _amount)) + _totalNormalMintPerAccount[msg.sender] <= maxNormalMintPerAccount, "max tokens per account reached");
    ...
    function whitelistMint(uint _amount) external payable {
        ...
        require(_amount + totalSupply() <= maxSupply, "amount exceeds max supply");
        require((_amount - _mintFreeAmount(msg.sender , _amount)) + _totalWhitelistMintPerAccount[msg.sender] <= maxWhitelistMintPerAccount, "max tokens per account reached");
        uint _free = _mintFreeAmount(msg.sender, _amount);
        if(_free>0) _totalFreeMintPerAccount[msg.sender] += _free;
