...
contract Payload is Ownable {
    error Payload__ContractDisabled();
    error Payload__InvalidSig();
    error Payload__UsedSig();
    error Payload__InvalidCaller();
    error Payload__InvalidPayment();
    error Payload__InvalidTimestamp();
    error Payload__InvalidCardCount();
    ...
    function handleInvokeEchelon(
        ...
        if (msg.sender != address(PRIME)) {
            revert Payload__InvalidCaller();
        }
        uint256 cardCount = abi.decode(_data, (uint256));
        if (cardPrice * cardCount != _primeValue) {
            revert Payload__InvalidPayment();
        }
        if (disabled) {
            revert Payload__ContractDisabled();
        }
        totalCardCount += cardCount;
        totalAddressCardCount[_from] += cardCount;
        emit PrimePayment(
            _from,
            _id,
            cardCount,
    ...
    function redeem(
        ...
        if (isClaimed[_signature]) {
            revert Payload__UsedSig();
        }
        if (block.timestamp > _timestamp + signatureWindow) {
            revert Payload__InvalidTimestamp();
        }
        if (
            !verify(
                trustedSigner,
        ...
        if (addressCardCount > totalAddressCardCount[msg.sender]) {
            revert Payload__InvalidCardCount();
        }
        unchecked {
            totalAddressCardCount[msg.sender] -= addressCardCount;
        }
    ...
    function verify(
        ...
        bytes32 messageHash = keccak256(
            abi.encodePacked(_to, _cardIds, _cardQuantities, _timestamp)
        );
