...
contract Contract is ERC721SignatureMint, LazyMint {
    address public deployer;
    ERC1155LazyMint public pinchRigCollection;
    ...
    {
        deployer = msg.sender;
        pinchRigCollection = ERC1155LazyMint(_pinchRigCollection);
    }

    /**
     *  This function returns who is authorized to lazy mint NFTs on this contract.
     *  We'll only allow the contract deployer to lazy mint NFTs.
     */
    function _canLazyMint() internal view virtual override returns (bool) {
    ...
    /**
     *  This function mint a NFT based in a signed request.
     *  Only 1 NFT can be minted at time.
     */
    function mintWithSignature(
        MintRequest calldata _req,
        bytes calldata _signature
    ) external payable virtual override returns (address signer) {
        // Check if the claimer has pinch rig collection
        require(
            pinchRigCollection.balanceOf(_req.to, 0) >= 1,
            "Rejected: Claimer does not have pinch rig collection."
        );

        // Check if the contract has only one unit requested
        require(
            _req.quantity == 1,
            "Rejected: Claimer can only mint one at the time."
        );
        ...
        // Burn one unit of the pinch rig
        pinchRigCollection.burn(receiver, 0, 1);
    }

    /**
     *  This function generate a signature to the verify the request.
     *  Only the contract owner can generate signatures.
     */
