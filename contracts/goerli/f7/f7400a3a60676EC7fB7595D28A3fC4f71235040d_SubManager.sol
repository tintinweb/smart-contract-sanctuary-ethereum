...
contract SubManager is ISubManager, OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    event MerchantCreated(address indexed owner, uint256 tokenId, string name, bool isSBT);
    ...
    event MerchantManagerTokenSet(address merchantToken);
    ...
    event PlatformFeeManagerSet(address platformFeeManager);

    address public merchantTokenManager;
    address public planManager;
    address public subInfoManager;
    address public subscriptionTokenManager;

    uint256 public ChargeAheadTime;
    ...
    function initialize() public initializer {
        ...
        ChargeAheadTime = 43200; // 12 hours
    }
    ...
    function setMerchantTokenManager(address _merchantToken)
    external
    override
    onlyOwner
    {
        require(_merchantToken != address(0), "merchantToken address invalid");
        merchantTokenManager = _merchantToken;
        emit MerchantManagerTokenSet(_merchantToken);
    }
    ...
    onlyOwner
    {
        ...
        subscriptionTokenManager = _subTokenManager;
    ...
    function setPlatformFeeManager(address _platformFeeManager) external override onlyOwner {
        require(_platformFeeManager != address(0), "platformFeeManager address invalid");
        platformFeeManager = _platformFeeManager;
        emit PlatformFeeManagerSet(_platformFeeManager);
    }

    function createMerchant(string memory name, bool isSBT)
    external
    override
    returns (uint256)
    {
        uint256 merchantTokenId = IMerchantTokenManager(merchantTokenManager)
        .createMerchant(name, msg.sender, isSBT);
        emit MerchantCreated(msg.sender, merchantTokenId, name, isSBT);
    ...
    function updateMerchant(
        ...
        address merchantOwner = IERC721(merchantTokenManager).ownerOf(
            merchantTokenId
        );
        require(msg.sender == merchantOwner, "must call by merchant owner");
        IMerchantTokenManager(merchantTokenManager).updateMerchant(
            merchantTokenId,
            name
        );
    ...
    function createPlan(uint256[] memory uints, address[] memory addresses, string[] memory strings) external override {
        // avoid stack depth limit
        {
            ...
            require(bytes(strings[1]).length <= 1000, "invalid description length");
        }
    ...
    function updatePlan(
        ...
        // avoid stack depth limit
        {
            ...
            require(bytes(description).length <= 1000, "invalid description length");
        }
    ...
    function disablePlan(uint256 merchantTokenId, uint256 planIndex) external override {
        ...
        address merchantOwner = IMerchantTokenManager(merchantTokenManager).ownerOf(
            merchantTokenId
        );
        require(msg.sender == merchantOwner, "must call by merchant owner");
        plan.enabled = false;
    ...
    external override nonReentrant
    {
        ...
        uint256 subscriptionTokenId = ISubTokenManager(subscriptionTokenManager).mintSubToken(msg.sender);
        ...
        if (plan.pricePerBillingPeriod == 0) {
            ...
            _charge(merchantTokenId, planIndex, subscriptionTokenId, msg.sender, plan.payeeAddress, plan.paymentToken, plan.pricePerBillingPeriod, block.timestamp);
        }
    ...
    function charge(uint256 subscriptionTokenId) external override nonReentrant {
        ...
        address subOwner = IERC721(subscriptionTokenManager).ownerOf(subscriptionTokenId);
        ...
        _charge(subInfo.merchantTokenId, subInfo.planIndex, subscriptionTokenId, subOwner, plan.payeeAddress, plan.paymentToken, plan.pricePerBillingPeriod, billingTime);
    }

    function  _charge(uint merchantTokenId, uint planIndex, uint subscriptionTokenId, address subOwner, address payeeAddress, address paymentToken, uint pricePerBillingPeriod, uint billingTime) internal {
        uint256 platformFee = _getPlatformFee(paymentToken, pricePerBillingPeriod);

        uint256 adjustedPrice = pricePerBillingPeriod - platformFee;

        if (adjustedPrice > 0) {
            SafeERC20.safeTransferFrom(IERC20(paymentToken), subOwner, payeeAddress, adjustedPrice);
        }

        if (platformFee > 0) {
            SafeERC20.safeTransferFrom(IERC20(paymentToken), subOwner, platformFeeAddress, platformFee);
        }
    ...
    function cancelSubscription(uint256 subscriptionTokenId) external override {
        ISubInfoManager.SubInfo memory subInfo = ISubInfoManager(subInfoManager).getSubInfo(subscriptionTokenId);
        require(subInfo.subEndTime > subInfo.nextBillingTime, "sub closed");
        address merchantOwner = IMerchantTokenManager(merchantTokenManager).ownerOf(subInfo.merchantTokenId);
        address subOwner = IERC721(subscriptionTokenManager).ownerOf(subscriptionTokenId);
        require(
    ...
    // get platform fee
    // if have fee config and more than minAmount, use fixed fee
    function _getPlatformFee(address paymentToken, uint pricePerBillingPeriod)
    internal
    view
    returns (uint256)
    {
        IPlatformFeeManager.FeeConfig memory feeConfig = IPlatformFeeManager(
            platformFeeManager
        ).getFeeConfig(paymentToken);
        if (feeConfig.fee > 0 && pricePerBillingPeriod >= feeConfig.minAmount) {
...
interface ISubManager {
    ...
    function setMerchantTokenManager(address merchantTokenManager) external;
    ...
    function setPlatformFeeManager(address platformFeeManager) external;
}
...
interface IPlatformFeeManager {
    ...
    function setFeeConfig(address token, uint256 fee, uint256 minAmount) external;
    function getFeeConfig(address token) external view returns (FeeConfig memory feeConfig);
