...
contract Staker {
  ...
  function stake() payable public {
    address st = msg.sender;
    require(!executed, "cannot stake after execute() is called");
    balances[st] += msg.value;
    emit Stake(st, msg.value);
  }

  function execute() public {
    uint256 bal = address(this).balance;
    require(!executed, "Cannot execute() twice");
    if (bal >= threshold) {
      executed = true;
  ...
  function withdraw() public {
    require(!executed, "Cannot withdraw after execute()");
    require(block.timestamp > deadline, "Cannot withdraw before deadline");
    require(address(this).balance < threshold, "cannot withdraw after threshold was reached");
