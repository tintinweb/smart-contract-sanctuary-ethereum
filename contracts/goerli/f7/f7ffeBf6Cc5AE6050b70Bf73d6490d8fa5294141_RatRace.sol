...
contract RatRace is ERC721, Ownable {
  ...
  uint256 public whitelistedCost = 0.001 ether;
  ...
  // hidden image URI
  string public hiddenImageURI = "ipfs://bafybeidpi26kgn75wm7yv3yl4antkcuqq3fayiyzjxxd5tgtmjye7jnd7y/hidden.json";
  ...
  uint public whiteListedMintPeriod = 30 minutes; // 30 minutes
  uint public onGoingWhiteListedMintPeriod = 0;
  ...
  * @param _mintAmount The number of NFTs to mint
  ...
  function mint(uint256 _mintAmount) public payable {
    ...
    // if white listed then mint with less price
    uint _cost = 0;

    if(whiteListed[msg.sender] && isWhitelistminting && block.timestamp < onGoingWhiteListedMintPeriod){
        _cost = whitelistedCost;
    }else{
      _cost = cost;
    }
    ...
    if (allFreeMinted<maxFreeMint && freeMinted[msg.sender] ==false) {
        
        require(msg.value >= _cost * (_mintAmount - 1));
        allFreeMinted +=1;
  ...
  /* 
    -> setting whitelist minting period.
    @params _whiteListedMintPeriod: time in seconds
    @devs only owner can call this function.

  */

  function setWhiteListedMintPeriod(uint _whiteListedMintPeriod) public onlyOwner{
      whiteListedMintPeriod = _whiteListedMintPeriod;
  }

  /* 

    -> setting whitelist minting status.
    @params _isWhitelistminting: true/false
    @devs only owner can call this function.

  */

  function setWhitelistMinting(bool _isWhitelistminting) public onlyOwner{
      isWhitelistminting = _isWhitelistminting;
      onGoingWhiteListedMintPeriod = block.timestamp + whiteListedMintPeriod;
   ...
   */
  function setWhiteListed(address _address, bool _value) public onlyOwner{
      whiteListed[_address] = _value;
  }
  ...
  function setBatchWhiteListed(address[] memory _addresses, bool _value) public onlyOwner{
      for(uint i=0;i<_addresses.length;i++){
          whiteListed[_addresses[i]] = _value;
      }
  }
  ...
  function setWhiteListedCost(uint256 _cost) public onlyOwner{
      whitelistedCost = _cost;
  }
