...
contract RealEven is ERC721, ERC2981, Ownable, UpdatableOperatorFilterer {
  ...
  constructor(
    ...
    UpdatableOperatorFilterer(MYOPERATOR_FILTER_REGISTRY, MYDEFAULT_SUBSCRIPTION, true)
    {
      ...
      baseURI = _uri;
      treasure = _treasure;
      setDefaultRoyalty(_owner, 1000); // 10% fees
  ...
  function mint(address to, uint16[] memory _families, uint16[] memory _count, address stableToken) external {
    ...
    require(tokenAuthorization.authorized == true, "Invalid token");
    ...
    IERC20Metadata(stableToken).transferFrom(msg.sender, treasure, price);
