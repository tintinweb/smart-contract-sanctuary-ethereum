    ...
    ReentrancyGuardUpgradeable
{
    ...
    function batchSeed(uint16 _commitId, uint256 seed) external {
        require(_msgSender() == seeder, "ONLY_SEEDER");

        require(commits[_commitId].seed == 0, "Already seeded");
        commits[_commitId].seed = seed;

        emit MintSeeded(_commitId);
    }
    ...
    function batchUserRevealPending(address recipient) external whenNotPaused nonReentrant {
        require(recipient == tx.origin, "Only EOA");
        (uint16[] memory commitIds,) = getPendingUserCommits(recipient);
        for (uint16 index = 0; index < commitIds.length; index++) {
            batchReveal(commitIds[index]);
        }
    }

    function getPendingUserCommits(address recipient) public view returns (uint16[] memory commitIds, uint16[] memory amounts) {
        uint256 pendingCount = 0;

        uint16[] memory pending = new uint16[](userCommits[recipient].length);
        for (uint256 index = 0; index < userCommits[recipient].length; index++) {
            uint16 _commitId = userCommits[recipient][index];
            MintCommit storage c = commits[_commitId];
            // need to be seed and not reveal
            if (c.seed == 0 || c.revealed) continue;
            pending[pendingCount++] = _commitId;
        }

        commitIds = new uint16[](pendingCount);
        amounts = new uint16[](pendingCount);
        for (uint16 index = 0; index < pendingCount; index++) {
            commitIds[index] = pending[index];
            MintCommit storage c = commits[pending[index]];
            amounts[index] = c.amount;
        }

        return (commitIds, amounts);
    }
