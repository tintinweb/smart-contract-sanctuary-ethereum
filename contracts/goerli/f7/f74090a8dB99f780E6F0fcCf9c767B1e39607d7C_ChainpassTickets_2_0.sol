...
/*
 *  ChainpassTickets V2.0
 */
contract ChainpassTickets_2_0 is ERC721, MetaTxMixin, Ownable {
    using Strings for uint256;

    event ConsecutiveTransfer(uint256 indexed fromTokenId, uint256 toTokenId, address indexed fromAddress, address indexed toAddress);

    constructor() ERC721("Chainpass Tickets", "CPT") MetaTxMixin() Ownable() {}

    function tokenURI(uint256 tokenId) public view override returns (string memory) {
        return string(abi.encodePacked(baseTokenUri, "/ticket/", tokenId.toString(), "/nft_metadata"));
    }
    ...
    function setTokenOwners(uint256[] calldata tokenIds, address owner) public onlyOwner {
        for (uint256 i; i < tokenIds.length; i++) {
            ...
            if (currentOwner == address(0)) {
                ...
                internalTransferFrom(currentOwner, owner, tokenId);
            }
        }
    }
...
abstract contract ERC721 {
    ...
    function internalTransferFrom(address from, address to, uint256 id) internal virtual {
    ...
    function transferFrom(
        ...
        internalTransferFrom(from, to, id);
    }
