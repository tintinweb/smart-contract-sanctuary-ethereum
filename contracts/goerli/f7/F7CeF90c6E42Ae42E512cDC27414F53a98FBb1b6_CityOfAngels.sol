...
contract CityOfAngels is Ownable, ERC721A, PaymentSplitter, UUPSUpgradeable {
    // Contract Constants
    ...
    uint256 public allowlistTimeLock;
    ...
    constructor(
        ...
        uint256 _allowlistTimeLock,
        uint64 _maxMintsPerAddress,
        address[] memory _payees,
        uint256[] memory _shares,
        uint256 _teamReserve
    ) ERC721A("CityOfAngels", "CoA") PaymentSplitter(_payees, _shares) {
        require(
            _allowlistTimeLock > _allowlistStartTime,
            "Start time greater than end time"
        );
        maxMintsPerAddress = _maxMintsPerAddress;
        allowlistStartTime = uint64(_allowlistStartTime);
        allowlistTimeLock = _allowlistTimeLock;
    ...
    modifier mintable(uint64 amount) {
        ...
        require(
            ...
            "You are sending an incorrect amount of eth"
        );
        _;
    }

    function isWhitelisted(address _addr, bytes32[] calldata _merkleProof)
        external
        view
        returns (bool)
    {
    ...
    function allowlistMint(uint64 amount, bytes32[] calldata _merkleProof)
        external
        payable
        mintable(amount)
    {
    ...
    function mint(uint64 amount) external payable mintable(amount) {
        require(
            block.timestamp > allowlistTimeLock,
    ...
    function mintAngel(uint64 amount) internal {
        _mint(msg.sender, amount);
        _setAux(msg.sender, amount);
    ...
    {
        require(
            _allowlistStartTime < allowlistTimeLock,
            "Start time greater than end time"
        );
        allowlistStartTime = _allowlistStartTime;
    }

    function setAllowlistTimeLock(uint256 _allowlistTimeLock)
        external
        onlyOwner
    {
        require(
            _allowlistTimeLock >= allowlistStartTime,
            "Start time greater than end time"
        );
        allowlistTimeLock = _allowlistTimeLock;
    }
    ...
    function setAllowlistTime(
        ...
        require(
            _allowlistStartTime < _allowlistTimeLock,
            "Start time greater than end time"
        );
        allowlistStartTime = _allowlistStartTime;
        allowlistTimeLock = _allowlistTimeLock;
        ...
        onlyOwner
    {
        ...
        require(newImplementation.code.length > 0, "Address is not a contract");
    }
}
