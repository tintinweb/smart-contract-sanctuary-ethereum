...
contract Sloth is Initializable, ERC721AQueryableUpgradeable, OwnableUpgradeable, ISloth, IERC2981, RevokableOperatorFiltererUpgradeable {
  ...
  address private slothMintAddr;

  event SetItem (
    uint256 indexed _tokenId,
    uint256 _itemId,
    address _itemAddr,
    ISlothItem.ItemType indexed _itemType,
    uint256 _setAt
  );

  bool private itemAvailable;
  uint8 private constant itemNum = 5;
  address payable private _royaltyWallet;
  uint256 public _royaltyBasis;
  ...
  function setItemAddr(address _itemAddr) external onlyOwner {
    itemAddr = _itemAddr;
  }

  function setSlothMintAddr(address _slothMintAddr) external onlyOwner {
    slothMintAddr = _slothMintAddr;
  }

  function setItemAvailable(bool _itemAvailable) external onlyOwner {
    itemAvailable = _itemAvailable;
  }

  function getEquipments(uint256 _tokenId) external view returns (Equipment[itemNum] memory) {
    Equipment[itemNum] memory equipments;
    for (uint8 i = 0; i < itemNum; i++) {
      equipments[i] = items[uint256(ISlothItem.ItemType(i))][_tokenId];
    }
    return equipments;
  }

  function setItems(uint256 _tokenId, uint256[] memory _itemIds) external {
    require(itemAvailable, "item not available");
    require(_exists(_tokenId), "not exist");
    require(ownerOf(_tokenId) == msg.sender, "not owner");
    require(_itemIds.length == itemNum, "invalid itemIds length");

    for (uint8 i = 0; i < itemNum; i++) {
      _setItem(_tokenId, _itemIds[i], ISlothItem.ItemType(i));
      emit SetItem(_tokenId, _itemIds[i], itemAddr, ISlothItem.ItemType(i), block.timestamp);
    }
  }

  function _setItem(uint256 _tokenId, uint256 _itemId, ISlothItem.ItemType _targetItemType) internal {
    ISlothItem item = ISlothItem(itemAddr);
    Equipment memory equipment = items[uint256(_targetItemType)][_tokenId];
    if (_itemId == 0) {
      if (equipment.itemId != 0 && equipment.itemAddr != address(0)) {
        item.transferFrom(address(this), msg.sender, equipment.itemId);
      }
      items[uint256(_targetItemType)][_tokenId] = Equipment(0, address(0));
      return;
    }

    if (items[uint256(_targetItemType)][_tokenId].itemId == _itemId && items[uint256(_targetItemType)][_tokenId].itemAddr == itemAddr) {
      return;
    }
    require(item.exists(_itemId), "not exist");
    require(item.ownerOf(_itemId) == msg.sender, "not owner");
    require(item.getItemType(_itemId) == _targetItemType, "invalid itemType");
    ...
    item.transferFrom(msg.sender, address(this), _itemId);

    items[uint256(_targetItemType)][_tokenId] = Equipment(_itemId, itemAddr);
  }
  ...
  function mint(address sender) external {
    require(msg.sender == slothMintAddr, "not slothMintAddr");
