...
contract ForeignIssuer is Ownable, ITelepathyHandler {
    ...
    /// @param _receiver the address of the telepathy receiver contract on this chain that can call `handleTelepathy`
    // create a token erc20 contract, with one owner which is this contract's owner and can only be minted from by this contract
    constructor(address _depositAddress, address _broadcaster, address _receiver, uint16 _sourceChainId, uint16 _foreignChainId) {
        depositAddress = _depositAddress;
        broadcaster = ITelepathyBroadcaster(_broadcaster);
        receiver = _receiver;
        sourceChainId = _sourceChainId;
        foreignChainId = _foreignChainId;
        token =new ERC20PresetMinterPauser("Wrapped Goerli Eth", "WGETH");
    }

    function depositAndBurnWrappedToken(address recipient, uint256 amount) external payable virtual{
        token.transferFrom(msg.sender, address(this), amount);
        token.burn(amount);
        broadcaster.send(sourceChainId, depositAddress, abi.encode(recipient, amount));
        emit DepositEvent(msg.sender, recipient, amount, address(token), sourceChainId);
    }

    // function depositGasFee(address recipient) payable { 
    // TODO use gas price oracle here to get the latest gas price on Ethereum * 250k gas for a withdrawal transaction
    function handleTelepathy(uint16 _sourceChainId, address _senderAddress, bytes calldata _data)
        external
    {
        require(msg.sender == address(receiver), "Only Telepathy Foreign Receiver can call this function");
        require(_sourceChainId == sourceChainId, "Source Chain Ids don't match");
        require(_senderAddress == depositAddress,"Only deposit address can trigger a message call to this contract.");
