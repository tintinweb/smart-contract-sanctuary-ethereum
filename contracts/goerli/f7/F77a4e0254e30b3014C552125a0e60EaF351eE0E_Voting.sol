    ...
    UUPSUpgradeable
{
    ...
    function _countVote(
        ...
        if (choice == uint8(VoteChoice.Nay)) {
            vote.count.nay += weight;
        } else if (choice == uint8(VoteChoice.Yea)) {
    ...
    function state(uint256 voteId) public view returns (VoteState) {
        ...
        if (
            (block.timestamp > vote.schedule.end &&
                ...
                )) || vote.count.yea == totalVotingWeight
        ) {
    ...
    UUPSUpgradeable
{
     ...
     * the following state can also only be explicitly set.
     *
    ...
    function propose(
        ...
        return _propose(execution, proposalSchedule, votingCriteria);
    }
    ...
    function propose(
        ...
        return _propose(execution, proposalSchedule, getVotingCriteria());
    }
    ...
    function update(
        ...
        require(
            ...
            "Governance: only pending proposals can be updated"
        );
        Proposal storage proposal = _proposals[proposalId];
        require(
        ...
        proposal.executionHash = hashProposal(execution, _nonce);
        proposal.nonce = _nonce++;
        proposal.executionTime = proposalSchedule.executionTime;
