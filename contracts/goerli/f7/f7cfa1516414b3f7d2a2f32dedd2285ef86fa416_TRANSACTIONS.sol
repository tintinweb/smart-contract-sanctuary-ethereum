...
/** 
    ...
    * @dev NFT contract for THE TRANSACTIONS, uses OpenSea's Operator Filter Registry (DefaultOperatorFilterer)

    CHECKLIST
    [ ] 2010 Items total - Airdrop to those affect by the Great Blur Bid Fiasco of 2022
    [ ] Rarity of holders transaction corresponds to rarity of the Pendings they hold at the time of mint
        --> Line up the IDs of Pendings and Blocks/Transactions IDs 1-998.
    [ ] Make sure that the metadata and URIs are opensea compatible
    [ ] ERC1155 OZ for now. Upgrade to ERC1155U/D/Solmate Later (https://github.com/nftchef/erc721-gas-compare)
    [ ] anything that mints burns or transfers writes balance-tracking variables (updates both sender and receiver)
    [ ] 
*/
...
contract TRANSACTIONS is ERC1155, Ownable, RrpRequesterV0, DefaultOperatorFilterer {
    ...
    uint256 public phaseOneDuration = 1 days;
    uint256 public phaseTwoDuration = 1 days;
    ...
    uint256 public maxSupply = nominalMaxSupply + blurVictimSupply;

    uint256 public mintPhase = 0;
    uint256 public mintPrice = 0.05 ether;
    uint256 public mintOpenTime; //block.timestamp when mintPhase is set from 0 to 1
    ...
    function batchMintBlocksPendingsOwnerFree(uint256[] memory _pendingsIds) public {
        for(uint256 i = 0; i < _pendingsIds.length; i++){
            mintSingleBlockPendingsOwnerFreeMint(_pendingsIds[i]);
        }
    }

    function batchMintBlocksPendingsOwnerPaid(uint256[] memory _pendingsIds) public payable {
        if(msg.value < mintPrice * _pendingsIds.length){ revert InsufficientFunds(); }
        for(uint256 i = 0; i < _pendingsIds.length; i++){
    ...
    function mintSingleAuto(uint256 _pendingsId) public payable {  
        if(mintPhase == 1 && !usedInFreeMintPendingIds[_pendingsId]){ 
            mintSingleBlockPendingsOwnerFreeMint(_pendingsId);
        } else if(mintPhase == 1 && usedInFreeMintPendingIds[_pendingsId] && !usedInPaidMintPendingIds[_pendingsId]){
            mintSingleBlockPendingsHolderWhitelist(_pendingsId);
        } else if(mintPhase ==2){
            mintSingleBlockPublic();
        } else {
            revert MintIsClosed();
        }
    }
    ...
    function mintSingleBlockPendingsOwnerFreeMint(uint256 _pendingsId) public {
        ...
        totalMinted++;
        ownedIds[sender].push(_pendingsId);

        _mint(sender, _pendingsId, 1, "");
    ...
    function mintSingleBlockPendingsHolderWhitelist(uint256 _pendingsId) public payable {
        ...
        requestRandomTransactionOutcome(sender);  
    ...
    function requestRandomTransactionOutcome(address _minter) private {
    ...
    function fulfillMint(bytes32 _requestId, bytes calldata data) external onlyAirnodeRrp {
        ...
        uint256 thisId = getIdFromQrnAndManageSupply(qrngUint256Array[0]);
        
        totalMinted++;
        ownedIds[sender].push(totalMinted);
    ...
    */ 
    function getIdFromQrnAndManageSupply(uint256 _QRN) private returns (uint256) {
        uint256 thisRandomIndex = (_QRN % remainingIds.length); //includes 0 because this gets an index not an ID.
        uint256 thisRandomId = remainingIds[thisRandomIndex];

        //remove this ID from the remainingIds array
        removeIdFromArray(remainingIds, thisRandomId);
        return thisRandomId;
    }
    ...
    //find desired ID, move 
    function removeIdFromArray(uint256[] storage _array, uint256 _id) private {
        for(uint256 i=0; i < _array.length; i++){
            if(_array[i] == _id){
    ...
    function uri(uint256 _id) public view override returns (string memory) {
        if(_id > maxSupply) { return "Id is beyond max";}
        else {
            if(revealed){
    ...
    // // is this needed? safeguard if some site is looking for tokenURI instead of uri
    ...
    function setIdArray() public onlyOwner {
        for(uint256 i=pendingsSupply+1; i <= maxSupply; i++){
            remainingIds.push(i);
        }
    }
    ...
    function setBlurVictimSupply(uint256 _blurVictimSupply) public onlyOwner {
        ...
        maxSupply = nominalMaxSupply + blurVictimSupply;
    }
        ...
        onlyAllowedOperator(from)
    {
        ...
        if(from != address(0)) {
            removeIdFromArray(ownedIds[from], tokenId);
    ...
    function safeBatchTransferFrom(
        ...
        if(from != address(0)) {
            for(uint256 i=0; i < ids.length; i++) {
                removeIdFromArray(ownedIds[from], ids[i]);
