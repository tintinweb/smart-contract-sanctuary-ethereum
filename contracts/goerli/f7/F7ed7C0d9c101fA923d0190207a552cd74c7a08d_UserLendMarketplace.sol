...
contract UserLendMarketplace is UserMarketplace {
    ...
    function userOffersRental(string memory registryTokenName_, uint256 rentTokenId_, uint64 validityTime_,
        uint64 rentDuration_, address paymentAddress_, uint256 paymentAmount_, bool rentExtensionAllowed_
        ...
        address[] memory allowedToRent = new address[](1);
        allowedToRent[0] = msg.sender;

        // locks funds offered by user
        if (paymentAddress_ == address(0)) {
            if (msg.value != paymentAmount_) {
                revert WrongNativeAmount({sent: msg.value, shouldSend: paymentAmount_});
            }
        } else if(!IERC20(paymentAddress_).transferFrom(msg.sender, address(this), paymentAmount_)) {
        ...
        rentOffers[++lastRentId] = RentOffer ({
            ...
            allowedToRent: allowedToRent,
            validity: validity,
            duration: rentDuration_,
            ownerCreated: false,
            status: RentStatus.CREATED,
            paymentOptions: _createPaymentInterface(paymentAddress_, paymentAmount_, true, rentExtensionAllowed_),
            rewardConfiguration: _createRewardConfig()
        });

        emit UserOfferRental(address(rentTokenAddress_), rentTokenId_, lastRentId, msg.sender, validity);
        return lastRentId;
    }

    function userCancelsRentalOffer(uint256 rentId_) external {
        RentOffer memory offer = _getValidRental(rentId_, RentStatus.CREATED);
        if (!offer.paymentOptions.upfrontPayment()) {
            revert WrongOrderStatus();
        }
        assert(offer.allowedToRent.length == 1);
        if (offer.allowedToRent[0] != msg.sender) {
        ...
        // unlock user funds
        address paymentAddress_ = offer.paymentOptions.payAddress();
        if (paymentAddress_ == address(0)) {
            payable(msg.sender).transfer(offer.paymentOptions.payAmount());
        } else if(!IERC20(paymentAddress_).transferFrom(address(this), msg.sender, offer.paymentOptions.payAmount())) {
            revert ERC20TransferFailed();
        }
    }

    function ownerAcceptUserRentalOffer(uint256 rentId_) external whenNotPaused {
        RentOffer memory offer = _getValidRental(rentId_, RentStatus.CREATED);
        ...
        assert(offer.allowedToRent.length == 1);
        rentInterface.setUser(offer.tokenId, offer.allowedToRent[0], uint64(offer.duration + block.timestamp));
        IContractRegistry.FeeSettings[] memory fees = registry.getFeeFromAddress(rentInterface);
        _forwardPayment(offer.paymentOptions, fees, msg.sender);

        emit OwnerApprovesRental(address(offer.tokenAddress), offer.tokenId);
    }
}
