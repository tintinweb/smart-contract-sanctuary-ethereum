...
abstract contract ERC20 is Access, IERC20 {
    ...
    event Shrink(Mint.Bucket indexed bucket, uint256 amount);
    ...
    function balanceOf(address _owner) public view override returns (uint256) {
        ...
        for (uint256 i = 0; i < state.cliffVesting.length; i++) {
        ...
        for (uint256 i = 0; i < state.linearVesting.length; i++) {
    ...
    function mint1(
        ...
        address recipient,
        uint256 amount
    ) public accessible(_selectRole(bucket)) {
    ...
    function mint2(
        ...
        emit MintType2(bucket, recipient, deadline, amount);
    }
    ...
    function batchMint2(
        ...
        require(buckets.length == recipients.length, "ERC20: Unmatched array length");
        require(recipients.length == deadlines.length, "ERC20: Unmatched array length");
        require(deadlines.length == amounts.length, "ERC20: Unmatched array length");

        for (uint256 i = 0; i < recipients.length; i++) {
            mint2(buckets[i], recipients[i], deadlines[i], amounts[i]);
        }
    }
    ...
    function mint3(
        ...
        require(end.sub(start).mod(interval) == 0, "ERC20: Interval is invalid");
        require(start < end, "ERC20: Start is greater than end");
        require(amount > 0, "ERC20: Amount is zero");
        _decreaseShare(bucket, amount);
    ...
    function recallTeamVesting(address account) external accessible(TEAM_ROLE) {
        ...
        for (uint256 i = 0; i < state.cliffVesting.length; i++) {
            ...
            if (vesting.active && vesting.bucket == Mint.Bucket.Team) {
                ...
                recycling = recycling.add(vesting.amount);
            }
        }
    ...
    function _decreaseShare(Mint.Bucket bucket, uint256 amount) internal {
        if (bucket == Mint.Bucket.EcoGrowth) {
            ...
            _ecoGrowthShare = _ecoGrowthShare.sub(amount);
        } else if (bucket == Mint.Bucket.Team) {
    ...
    function _updateAccountState(AccountState storage state) internal {
        for (uint256 i = 0; i < state.cliffVesting.length; i++) {
            Vesting.Cliff storage vesting = state.cliffVesting[i];
            // solhint-disable-next-line not-rely-on-time
            uint256 released = vesting.updateCliff(block.timestamp);
        ...
        for (uint256 i = 0; i < state.linearVesting.length; i++) {
            Vesting.Linear storage vesting = state.linearVesting[i];
            // solhint-disable-next-line not-rely-on-time
            uint256 released = vesting.updateLinear(block.timestamp);
            state.available = state.available.add(released);
        }
    }
...
library Vesting {
    ...
    /// Cliff Vesting defines `amount` tokens will be fully unlocked after `deadline`.
    struct Cliff {
        bool            active;
        Mint.Bucket     bucket;
        uint64          deadline;
    ...
    function getLinear(Linear storage vesting, uint256 timestamp) internal view returns (uint256, uint256) {
        ...
        if (timestamp <= vesting.start) {
            ...
            uint256 iTimes = timestamp.sub(vesting.start).div(vesting.interval);
            uint256 nTimes = uint256(vesting.end).sub(vesting.start).div(vesting.interval);
            uint256 released = vesting.amount.mul(iTimes).div(nTimes);
            return (vesting.amount.sub(released), released);
        } else {
            return (0, vesting.amount);
        }
    }
    ...
    function updateCliff(Cliff storage vesting, uint256 timestamp) internal returns (uint256) {
        if (!vesting.active || timestamp < vesting.deadline) {
            return 0;
        }
    ...
    function updateLinear(Linear storage vesting, uint256 timestamp) internal returns (uint256) {
        if (!vesting.active || timestamp < uint256(vesting.start).add(vesting.interval)) {
            return 0;
        }
        ...
        if (timestamp < vesting.end) {
            ...
            uint256 nTimes = uint256(vesting.end).sub(vesting.start).div(vesting.interval);
            uint256 released = vesting.amount.mul(iTimes).div(nTimes);
            vesting.start = uint256(vesting.start).add(iTimes.mul(vesting.interval)).toUint64();
            vesting.amount = vesting.amount.sub(released);
            return released;
        } else {
            vesting.active = false;
            return vesting.amount;
