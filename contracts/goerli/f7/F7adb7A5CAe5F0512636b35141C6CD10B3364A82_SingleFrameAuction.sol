...
contract SingleFrameAuction is Ownable, ERC721Holder {
    ...
    function onCreateAuction(address _collectionId, uint256 _tokenId, address _tokenAdr, uint256 _startPrice, uint256 _startTime, uint256 _endTime, address sender) internal {
    ...
    function lazySingleBid(LazyBid memory lazyBid) external payable{
		...
		onCreateAuction(lazyBid.collectionId, tokenId, lazyBid.tokenAdr, lazyBid.startPrice, lazyBid.startTime, lazyBid.endTime, lazyBid.sender);
        onBidOnAuction(auctions.length , lazyBid.amount);
