...
contract Water is ERC20, Ownable {
    ...
    uint256 private _comparisonValue;
    ...
    address private _partnershipTokenAddress = 0x96c0ca1a8E9d5903D9d748533A737079308C70A6;
    ...
    constructor () ERC20("Water", "H2O") {
        ...
        _comparisonValue = 2 * 1e16; // Set comparison value to 2%
    ...
    //sets the comparisonValue % in the isSwap() to determine if swapNow() should be called
    function setIsSwap(uint8 percentage) public onlyOwner {
        require(percentage >= 1 && percentage <= 100, "Percentage must be between 1 and 100");
        _comparisonValue = percentage * 1e16;
    }

    function isSwapPercent() public view returns (uint256) {
        return _comparisonValue;
    }

    //Checks to see if swapping event needs to be triggered based on the the value of tokens 
    //held on the contract compared to their % value of ETH/BNB & returns current %
    function shouldSwapNow() public view returns (bool) {
        if (devFeeBalance == 0 || partnershipsBalance == 0 || autoBurnBalance == 0 || maintenanceBalance == 0) {
            return false;
        }
        if (balanceOf(address(this)) < autoBurnBalance) {
            return false;
        }
    ...
    function isSwap() public view onlyOwnerOrContract returns (bool) {
        uint256 tokensHeld = balanceOf(address(this));
        address[] memory path = new address[](2);
        path[0] = uniswapV2Router.WETH();
        path[1] = address(this);
        uint256[] memory amounts = uniswapV2Router.getAmountsOut(tokensHeld, path);

        if(amounts[0] >= _comparisonValue){
        if (!shouldSwapNow()) {
            return false;
        }
            return true;
    ...
    //can be used to double check the swapping calculations for % calculations
    ...
    //Amount of tokens it takes to equal 1 ETH/BNB
   ...
   function addToTaxBalance(uint256 amount) external onlyOwner {
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal override {
        ...
        if (!isExcluded) {
            ...
            if (recipient == uniswapV2Pair) { // After a sell
                if (this.isSwap()) {
                    swapNow();
                        // The swap operation was successful, continue as normal.
                }
            }
