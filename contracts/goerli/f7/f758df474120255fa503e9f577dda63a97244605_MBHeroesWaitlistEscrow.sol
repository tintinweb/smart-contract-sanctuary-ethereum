...
contract MBHeroesWaitlistEscrow is Ownable, ReentrancyGuard {
    ...
    address payable private constant _TREASURY =
        payable(0x150A156f1937F6c9EBd9547c13e108d43332CB4a); // @todo check this

    address payable private constant _PAPER_HAND_LABS =
        payable(0x338E0421b72dA8ADc371f3247255A8c7dc4fa69d); // @todo check this
        ...
        onlyOwner
    {
        for (uint16 i = 0; i < airdropRecipients.length; i++) {
            ...
            if (!exists) {
                // If we're recording an airdrop for an address that hasn't made a deposit, we
                // want to bring that to light by failing hard
                revert InvalidRecipient(recipient);
            }

            if (airdropCount > 0) {
                // If the recipient has already received an airdrop, it could be that more
                ...
                // it can be investigated, but also to prevent `allowedRefundCount` unflowing
     ...
     */
    function close() external onlyOwner {
        // Ensure that all pending refunds have been processed.
        // allowedRefundCount = number of depositors - airdrop receivers - depositors who have been refunded
    ...
    function beneficiaryWithdraw() external nonReentrant {
        ...
        uint256 phlPayment = (currentBalance * 9) / 100;
    ...
    {
        return getPagableWaitlistDetails(0, 0);
    }

    function getPagableWaitlistDetails(uint256 startIndex, uint256 count)
        public
        view
        returns (WaitlistState[] memory)
    {
    ...
    {
        if (_escrow.depositsOf(payee) == 0) {
            return (false, "No deposits made or already refunded");
        }

        (bool exists, uint256 value) = _waitlistAirdropRecipients.tryGet(payee);
    ...
    function withdraw(address payable payee) external nonReentrant {
        (bool allowed, string memory reason) = withdrawalAllowed(payee);
