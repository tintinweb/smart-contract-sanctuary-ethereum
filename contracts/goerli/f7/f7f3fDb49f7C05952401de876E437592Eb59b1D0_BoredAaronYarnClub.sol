...
import "./IBoredAaronYarnClub.sol";
import "./token/ERC721Enumerable.sol";
import "@openzeppelin/contracts/access/Ownable.sol";

/**
 * @title Bored Aaron Yarn Club
 * @author Aaron Hanson <[email protected]> @CoffeeConverter
 */
contract BoredAaronYarnClub is IBoredAaronYarnClub, ERC721Enumerable, Ownable {
    uint256 public constant PRICE = 0.00001337 ether;
        ...
        payable
    {
        ...
        if (_mintAmount == 0) revert CannotMintZero();
        unchecked {
            if (msg.value != _mintAmount * PRICE) revert IncorrectPaymentAmount();
        }

        _mint(_mintAmount);

        if (_throughLaunchpadId > 0)
            emit LaunchpadMint(_msgSender(), _throughLaunchpadId, _mintAmount);
    }
    ...
    function _mint(
        ...
        internal
    {
        uint256 nextID = _startingTokenID + _owners.length;
        if (_mintAmount == 1) {
