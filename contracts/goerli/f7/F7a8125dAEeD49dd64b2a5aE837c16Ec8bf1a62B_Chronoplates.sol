...
// Chronophotography by 0xDEAFBEEF 
// Apr 2023
...
contract Chronoplates is ERC721, AccessControl {
    uint256 constant MAX_TIME_LOCK = 1314000000; //500 years, assuming ~12 seconds per block
    ...
    uint256 pid_low;
    uint256 pid_high;
    event eTriggerShutter(uint256 token_id,uint256 parent_id);
    
    struct TokenParamsStruct {
      uint256 block_number; //block number used as seed hash seed for random generation
      uint256 parent_id; //the deafbeef2 token ID that this chronophotograph corresponds to
      uint256 seq;
    ...
    constructor() ERC721("Choronophotography Plates", "CHRONOPLATES") {
      _grantRole(DEFAULT_ADMIN_ROLE, msg.sender);

      address _deafbeef2_contract_address = 0x23506Cb98577F479c88FEAC974B7723a1d841F95;
      ...
      _owner = msg.sender;
      setBaseURI("https://deafbeef.com/plates/");
    }
    ...
    function getTokenParams(uint256 i) public view returns (uint256 block_number, bytes32 codeLocation0, uint256 parent_id, uint256 seq, uint256 codeversion) {
    ...
    //trigger the shutter, create a new choronophotograph
    // Only owner of parent token can do this
    function TriggerShutter(uint256 pid) public virtual pidInRange(pid) returns (uint256 _tokenId) {
      ...
      uint256 tokenID =  _mintInternal(pid,msg.sender);
      emit eTriggerShutter(tokenID,pid);
      return tokenID;
    }
    ...
    function _mintInternal(uint256 pid, address to) internal virtual returns (uint256 _tokenId) {
	 ...
	 for the PRNG in the generative code that produces the photographic plate.
      ...
      */
      ...
      tokenParams[tokenID].seq = par[pid].epoch;
      tokenParams[tokenID].code_version = code_version; // store the current code version use to produce this plate
	...
	//time lock remains unchanged past max_time_lock
	// becomes linear when is 500 years.
      } else {
