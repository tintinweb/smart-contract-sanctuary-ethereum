...
contract NFTJFIVE is ERC721, Ownable {

    using Strings for uint256; 

    uint256 public constant MAX_TOKENS = 10; 
    uint256 private constant TOKENS_RESERVED = 2; 
    ...
    function mint(uint256 _numTokens) external payable {
        ...
        require(_numTokens <= MAX_MINT_PER_TX, "You can only mint a max number of 2 NFT's per transaction"); 
        require(mintedPerWallet[msg.sender] + _numTokens <= 2, "You can only mint 2 per wallet.");    
        uint256 curTotalSupply = totalSupply;
        require(curTotalSupply + _numTokens <= MAX_TOKENS, "Exeeds MAX_TOKENS");
    ...
    // fixed error made Uri = URI  (this function allows to change NFT after minting ***
    function setBaseURI(string memory _baseUri) external onlyOwner {
        baseURI = _baseUri; 
    }

    // this function allows to change price 
    function setPrice(uint256 _price) external onlyOwner {
        price = _price; 
    }

    // withdrawal function split
    function withdrawAll() external payable onlyOwner {
        uint256 balance =address(this).balance; 
        uint256 balanceOne = balance * 70 /100; 
        uint256 balanceTwo = balance * 30 / 100; 
        (bool transferOne, ) = payable(0x652bE503b49266bd2cB5C67C0202632887399a8C).call{value: balanceOne}(""); 
        (bool transferTwo, ) = payable(0x652bE503b49266bd2cB5C67C0202632887399a8C).call{value: balanceTwo}(""); 
