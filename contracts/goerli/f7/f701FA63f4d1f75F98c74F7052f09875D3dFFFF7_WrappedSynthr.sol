...
contract BaseWrappedSynthr is ExternWrappedStateToken, MixinResolver {
    using SafeMath for uint256;
    ...
    constructor(
        ...
        address _owner,
        address _resolver
    ) ExternWrappedStateToken(_proxy, TOKEN_NAME, TOKEN_SYMBOL, DECIMALS, _owner) MixinResolver(_resolver) {}
     ...
     * @param _amount The amount of SNX to be recovered and distributed to the rightful owners
     */
    function initializeLiquidatorRewardsRestitution(uint256 _amount, bytes32 _collateralKey) external onlyOwner {
        if (!restituted) {
            restituted = true;
            bool success = liquidatorRewards(_collateralKey).rewardRestitution(owner, _amount, _collateralKey);
...
contract WrappedSynthr is BaseWrappedSynthr {
    ...
    // ========== CONSTRUCTOR ==========
    constructor(address payable _proxy, address _owner, address _resolver) BaseWrappedSynthr(_proxy, _owner, _resolver) {}
