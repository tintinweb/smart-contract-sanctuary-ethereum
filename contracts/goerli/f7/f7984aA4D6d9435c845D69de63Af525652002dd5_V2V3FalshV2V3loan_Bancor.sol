...
/*    function UniV3SwapByOut(
    ...
    private returns (uint256 amountIn){
        ...
        amountIn = uint256((zeroForOne ? amount0 : amount1));
                ...
                //   } else if (exchange ==203) {
            //         (bool zero2one,address poolAdd,bytes memory data)
            //                 = abi.decode(data2Dec, (bool,address,bytes));
            //         UniV3SwapByOut(amountIn2payback,zero2one,poolAdd,msg.sender,data);
*/

    // 这个可以去掉
    // function quoteBancor1(
    //     uint256 amountIn, 
    //     address sourceToken,
    //     address targetToken 
    // )public view returns (            
    ...
    //         address[] memory BancPath) { 
    //     if(sourceToken==weth)sourceToken=ethBancor;
    //     if(targetToken==weth)targetToken=ethBancor;        
    //     BancPath=BancorExchange.conversionPath(sourceToken, targetToken) ;
    //     amountOut=BancorExchange.rateByPath(BancPath, amountIn);         
    // }
