    ...
    VRFConsumerBaseV2
{
    ...
    uint16 internal lastIndex;
    uint16 internal mintedTokens;
    ...
    constructor(
        ...
        string memory _baseUri, // TODO: memory vs storage
        ...
        ERC721("Pink Flamingo Social Club", "PFSC")
    {
        ...
        // initialize range of possbile tokenId's
        for (uint16 i = 0; i < maxTokenId; i++) {
            nextTokenId.push(i + 1);
        }

        lastIndex = maxTokenId - 1;
    ...
    function setBaseURI(string memory uri) external onlyOwner {
        _baseURIextended = uri; // TODO: external vs public
    }
     ...
     */
    modifier availableToMint() {
        require(mintedTokens + 1 <= maxTokenId, "Minted concluded");
        _;
    }
     ...
     */
    modifier isWhitelistPrice() {
        require(msg.value >= whitelistPriceInWei, "Insuffcient Wei");
        _;
    }
     ...
     */
    modifier isPublicPrice() {
        require(msg.value >= publicPriceInWei, "Insuffcient Wei");
        _;
    }
    ...
    function fulfillRandomWords(uint256 requestId, uint256[] memory randomWords)
        ...
        override
    {
        fulfillMint(requestId, randomWords);
    }

    function fulfillMint(uint256 requestId, uint256[] memory randomWords)
        internal
    {
        uint256 tokenId;

        if (lastIndex != 0) {
            uint16 randItem = uint16(randomWords[0] % uint256(lastIndex));
            uint16 swapItem = nextTokenId[lastIndex];
            nextTokenId[lastIndex] = nextTokenId[randItem];
            tokenId = uint256(nextTokenId[randItem]);
            nextTokenId[randItem] = swapItem;
            lastIndex--;
        } else {
            tokenId = uint256(nextTokenId[lastIndex]);
        }

        _safeMint(requestIdToSender[requestId], tokenId);
        _setTokenURI(tokenId, Strings.toString(tokenId));

        emit Mint(requestIdToSender[requestId], tokenId);
    }
    ...
    function _baseURI() internal view virtual override returns (string memory) {
        return _baseURIextended; //todo: view vs pure (& virtual?)
    }

    // function _baseURI() internal pure override returns (string memory) {
    //     return "ipfs://QmU5up9QnGuy78GuneCsLfkUoFDk5vzuYSMx9apYeKYnGt/";
    // }
...
abstract contract Minters is Ownable {
     ...
     */
    modifier withinPublicLimit() {
        require(
            minters[msg.sender].publicMints + 1 <= publicMintLimit,
            "Reached Public Limit"
        );
        _;
    }
}