...
contract Poopsie is ERC721A, Ownable, ERC2981 {
    ...
    function teamMint() external onlyOwner{
        ...
        teamMinted = true;
        _safeMint(0xC3E1DAf7B8C8716e07970844C9bD76d456ae4fD7, 10);
    }

    modifier callerIsUser() {
        require(
            tx.origin == msg.sender,
            "Poopsie :: Cannot be called by a contract"
        );
        _;
    }
    ...
    function publicMint(uint256 _quantity) external callerIsUser {
        ...
        require(
            ...
            "Poopsie :: Beyond Max Supply"
        );
        require(
            (totalPublicMint[msg.sender] + _quantity) <= MAX_PUBLIC_MINT,
            "Poopsie :: Already minted 3 times!"
        );
        ...
        callerIsUser
    {
        ...
        require(isValidMerkleProof(_merkleProof, msg.sender), "Poopsie :: You are not whitelisted");
        require(
            (totalSupply() + _quantity) <= MAX_SUPPLY,
            "Poopsie :: Cannot mint beyond max supply"
        );
        require(
            (totalWhitelistMint[msg.sender] + _quantity) <= MAX_WHITELIST_MINT,
            "Poopsie :: Cannot mint beyond whitelist max mint!"
        );
