...
contract StakingSBT is ERC721Psi, IERC4906, Ownable {
    ...
    uint256 public verifyInterval = 10000;
    uint256 public verifyIntervalModifier = 10000;
    uint256 public verifyIntervalModifierRange = 10000;
    uint256 public previousVerificationTimestamp;

    mapping (address => uint256) public userRankVerifications;

    IStakingContract public stakingContract;

    error QueryForNonexistentToken();

    constructor(string memory _baseURI) ERC721Psi("StakingReceipt", "STKR") {
        baseURI = _baseURI;
    }
    ...
    function connorDoesntTrustHeVerifies() external {
        require(balanceOf(msg.sender) > 0, "You don't own any tokens");
        require(block.timestamp > previousVerificationTimestamp + verifyInterval + verifyIntervalModifier, "You can't verify yet");
        previousVerificationTimestamp = block.timestamp;
        verifyIntervalModifier = uint256(keccak256(abi.encodePacked(block.timestamp, msg.sender))) % verifyIntervalModifierRange;
        ++userRankVerifications[msg.sender];
    ...
    function setVerifyInterval(uint256 newVerifyInterval) public onlyOwner {
        verifyInterval = newVerifyInterval;
    }

    function setVerifyIntervalModifierRange(uint256 newVerifyIntervalModifierRange) public onlyOwner {
        verifyIntervalModifierRange = newVerifyIntervalModifierRange;
    }
