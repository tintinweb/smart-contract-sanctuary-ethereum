    ...
    UUPSUpgradeable
{
    ...
    function initialize(address _tokenA, address _tokenB, address _multiSig, uint256 _evmChainId) public initializer {
        __Pausable_init();
        __Ownable_init();
    ...
    function _getExchangeRate() private view returns (uint256) {
        ...
        Oracle.ExchangeRate memory data = Oracle(oracle).getExchangeRate(evmChainId, fromToken, evmChainId, toToken);
        return data.rate;
    }

    function setOracle(address _exchangeRateOracle) external onlyOwner {
        oracle = _exchangeRateOracle;
    }
