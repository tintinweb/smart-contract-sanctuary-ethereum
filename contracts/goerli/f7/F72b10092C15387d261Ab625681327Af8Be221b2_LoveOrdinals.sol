...
abstract contract HoppiCarrotFactory {
    function feedCarrot(
    ...
    ERC2981Upgradeable
{
    ...
    address public hoppiCarrotContract;
    // address private hoppiCarrotContract;
    ...
    */
    ...
    function feedCarrot(uint256 typeId, uint256 amount)
        external
        payable
        canMint(PUBLIC_SALE_ID, _msgSenderERC721A(), 1)
    {
        HoppiCarrotFactory factory = HoppiCarrotFactory(hoppiCarrotContract);

        factory.feedCarrot(_msgSenderERC721A(), typeId, amount);

        uint256 startTokenId = _nextTokenId();
        _safeMint(_msgSenderERC721A(), 1);

        emit NFTMinted(_msgSenderERC721A(), 1, startTokenId);
    }

    function setHoppiCarrotContractAddress(address hoppiCarrotContractAddress)
        external
        onlyOwner
    {
        hoppiCarrotContract = hoppiCarrotContractAddress;
    }
}
