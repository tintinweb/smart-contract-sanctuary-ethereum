...
contract OVRRegister is ERC721Full, AccessControl, PriceCalculator {
    ...
    // Price in OVR
    uint256 public price; //in USD wei 1$ = 1e18 wei
    // xwTreasury
    ...
    /* ========== EVENTS ========== */
    // prettier-ignore
    event LandChanged(string indexed _subdomain, address indexed _owner, string indexed _newLand, uint256 timestamp);
    ...
    // prettier-ignore
    event NameRegistered(string indexed _subdomain, address indexed _caller, address indexed _beneficiary, bytes32 _labelHash, uint256 _createdDate);
    ...
    modifier _requireBalance(address _user) {
        ...
        // prettier-ignore
        require(acceptedToken.allowance(_user, address(this)) >= price, "Insufficient allowance");
        _;
    }
    ...
    modifier requireNameValid(string memory _name) {
        ...
        // prettier-ignore
        require(tempName.length >= 2 && tempName.length <= 20, "Required from 2 to 20 characters");
        for (uint256 i = 0; i < tempName.length; i++) {
            require(_isLetterOrNumber(tempName[i]), "Invalid characters");
        }
        _;
    }
    ...
    function register(
        ...
        string memory _land
    ) external _requireBalance(_beneficiary) requireNameValid(_subdomain) {
        ...
        //subdomain price in USD / price of 1 token = amount of tokens to be sent
        //uint256 toPay = price.div(valueOfAsset(1));
        uint256 toPay = price.div(1);
        require(
    ...
    function changeLand(string memory _subdomain, string memory _landId)
        public
    {
        bytes32 subdomainLabelHash = keccak256(abi.encodePacked(_subdomain));
        require(
        ...
        changeNotice(subdomainLabelHash, _landId);
        _transfer(address(this), _msgSender(), uint256(subdomainLabelHash));
