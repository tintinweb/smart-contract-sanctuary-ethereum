...
contract BucketAuction is IBucketAuction, ERC721M {
    ...
    function generateTestBids(uint256 numBids) public {
        for (uint256 i; i < numBids; i++) {
            address newUser =
                address(
                    uint160(
            ...
            uint256 bidAmount = 40000000000000000 + uint256(keccak256(abi.encodePacked(block.timestamp, i)))%150000000000000000000;

            _users.add(newUser);
            emit Bid(newUser, bidAmount, 0, address(this).balance);
        }
    }
