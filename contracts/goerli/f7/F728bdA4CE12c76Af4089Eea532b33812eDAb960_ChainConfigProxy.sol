...
abstract contract ManageableProxy is ERC1967Proxy {
    ...
    constructor(IChainConfig chainConfig, address defaultVersion, bytes memory inputData) ERC1967Proxy(defaultVersion, inputData) {
        _changeAdmin(address(chainConfig));
    }
    ...
    modifier onlyFromGovernance() {
        require(msg.sender == IChainConfig(_getAdmin()).getGovernanceAddress(), "Proxy: only governance");
        _;
    }
...
import "../libs/ManageableProxy.sol";
import "./ChainConfig.sol";

contract ChainConfigProxy is ManageableProxy {
    ...
    constructor(
        ...
        address treasuryAddress,
        uint16 swapFeeRatio
    ) ManageableProxy(IChainConfig(address(this)), _deployDefault(), abi.encodeWithSelector(ChainConfig.initialize.selector,
