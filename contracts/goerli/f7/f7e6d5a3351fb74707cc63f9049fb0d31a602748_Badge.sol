...
contract Badge is ERC1155, Ownable {
    ...
    // charge of creating badge set by owner
    function setBadgeFee(uint256 _badgeFee) external onlyOwner {
        badgeFee = _badgeFee;
    }

    // fee charged to sponsor while creating a badge
    function getBadgeFee() external view returns (uint256) {
        return badgeFee;
    }

    // creating badge by sponsor
    ...
    //student can apply on sponsor created badge
    function applyBadge(uint256 badgeId) external onlyStudent {
        require(
            claimed[msg.sender][badgeId] != true,
            "ERROR: Claimed Applied"
        );
    ...
    function transferBadge(address to, uint256 badgeIds) external onlySponsor {
        require(
        ...
        _safeTransferFrom(msg.sender, to, badgeIds, quantity, " ");
        ...
        onlySponsor
    {
        ...
        require(
            ...
            "no quantity available on badge to reset time"
        );
    ...
    // checking the sponsor of the id
