...
interface ISpreadVault {
    ...
    function MFEE() external view returns (uint256);
    ...
    function StrikeHighX1e6(uint256) external view returns (uint256);

    function StrikeLowX1e6(uint256) external view returns (uint256);
    ...
    function WFEE() external view returns (uint256);
    ...
    function initNewRound(
        uint256 roundStrikeLowX1e6,
        uint256 roundStrikeHighX1e6,
        uint256 _spotPrice,
        uint256 _minSizeOfVault
    ) external;

    function initWithdraw() external returns (uint256);
    ...
    function settlementRoundID(uint256) external view returns (uint80);

    function spotPrice(uint256) external view returns (uint256);

    function startRoundID(uint256) external view returns (uint80);
...
import "./Interfaces/ISpreadVault.sol";
...
contract UI_Reader is Initializable, UUPSUpgradeable {
    struct Vault {
        ...
        uint256 vaultTotalRequestedWithdrawals;
    ...
    struct SpreadVault {
        ...
        // the premium payed by the maker in the start of current round
        uint256 premium;
        // the High strike price for current round
        uint256 strike_high_price;
        // the High strike price for current round
        uint256 strike_low_price;
        // the spotPrice for the current round
        uint256 spot_price;
        // the expiry date of the current round, no new rounds can be started befor this date
        uint256 time_on_expiry;
    ...
    function getSpreadVaultInfo(
        address _vault
    ) public view returns (SpreadVault memory) {
        ISpreadVault vault = ISpreadVault(_vault);
        uint256 round = vault.round();
        uint256 totDeposit = 0;
        if (vault.vaultTotalDeposits() > vault.totalDeposit(round + 1)) {
            totDeposit =
                vault.vaultTotalDeposits() -
                vault.totalDeposit(round + 1);
        }
        if (vault.vaultTotalClaimableWithdrawals() >= totDeposit)
            totDeposit = 0;
        else {
        ...
        return
            SpreadVault(
                ...
                vault.StrikeHighX1e6(round),
                vault.StrikeLowX1e6(round),
    ...
    function myVaultPosition(
            ...
            if (vault.queuedRound(walletAddress) > 0) {
                balance =
                    Math.mulDiv(
                        balance,
                        vault.assetScaleX1e36(
                            vault.queuedRound(walletAddress) - 1
                        ),
                        userScaleX1e36
                    ) +
                ...
                balance = Math.mulDiv(
                    balance,
                    vault.assetScaleX1e36(vault.round()),
        ...
        uint256 profit = 0;
        if (
            (balance + WithdrawAmount) > vault.userTotalDeposit(walletAddress)
        ) {
            profit =
        ...
        if (claimableWithdraw > 0) {
            if (totDepo > vault.withdrawQueueAmt(walletAddress)) {
                totDepo -= vault.withdrawQueueAmt(walletAddress);
            } else {
                totDepo = 0;
            }
        }
