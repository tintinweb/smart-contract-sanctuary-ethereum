...
contract NftStakingPosition is ERC721, Ownable
{
	...
	function batchClaimIncentiveStakerReward(uint256[] calldata stakingPositionIds, address beneficiary) external
	{
		for (uint256 i = 0; i < stakingPositionIds.length; i++)
		{
			require(ownerOf(stakingPositionIds[i]) == msg.sender, "Not the owner!");             // Requires to be owner of position.

			uint256 reward = nftBattleArena.calculateIncentiveRewardForStaker(stakingPositionIds[i]);
