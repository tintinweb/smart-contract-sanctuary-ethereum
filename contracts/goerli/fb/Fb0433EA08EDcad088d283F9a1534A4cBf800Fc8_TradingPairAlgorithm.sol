...
contract TradingPairAlgorithm is PositionAlgorithm, ITradingPairAlgorithm {
    ...
    function _swap(
        ...
        // count the old price
        data.lastPrice = (data.inputlastCount * priceDecimals) / output.count();
        ...
        // count the old price
        data.newPrice = (input.count() * priceDecimals) / output.count();
        if (data.newPrice == 0) data.newPrice = 1;

        // count the snapshot price
        data.snapPrice = (snapshot.input * priceDecimals) / snapshot.output;
        if (data.snapPrice == 0) data.snapPrice = 1;
        // slippage limiter
        if (data.newPrice >= data.snapPrice)
            data.slippage = (data.newPrice * priceDecimals) / data.snapPrice;
        else data.slippage = (data.snapPrice * priceDecimals) / data.newPrice;
        ...
        data.priceImpact = (data.newPrice * priceDecimals) / data.lastPrice;
        require(
            data.priceImpact <= priceDecimals + priceDecimals / 2, // 150% of priceDecimals
            'too large price impact'
        );
