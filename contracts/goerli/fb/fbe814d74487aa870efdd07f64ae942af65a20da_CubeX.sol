...
contract CubeX is Ownable, ERC721A {
    ...
    // Dutch auction starting time
    uint256 public DA_STARTING_TIMESTAMP;

    //Starting at 2 ether
    uint256 public constant DA_STARTING_PRICE = 0.77 ether;

    //Ending at 0.1 ether
    uint256 public constant DA_ENDING_PRICE = 0.37 ether;

    //Decrease by 0.05 every frequency.
    uint256 public constant DA_DECREMENT = 0.025 ether;

    //decrement price every 900 seconds (15 minutes).
    uint256 public constant DA_DECREMENT_FREQUENCY = 900;

    //The final dutch auction price.
    uint256 public DA_FINAL_PRICE;

    //WL Price
    uint256 public WLprice;

    uint256 public constant RESERVED_QUANTITY = 60;

    uint16 public RESERVED_MINTED;
    ...
    //The quantity for WL.
    uint256 public WL_QUANTITY = 3717;
    ...
    function mintDutchAuction(uint8 quantity) public payable callerIsUser {
        ...
        //Max supply check
        require(
            DA_MINTED + quantity <= DA_QUANTITY,
        ...
        require(block.timestamp <= WL_STARTING_TIMESTAMP, "DA has ended");
        //Require max 3 per tx
        require(quantity <= 3, "Can only mint max 3 NFTs at once!");
        require(
            _numberMinted(msg.sender) + quantity <= 3,
            "User can only mint max 3 NFTs!"
        );
    ...
    function mintWL(uint8 quantity) public payable callerIsUser {
        require(DA_FINAL_PRICE > 0, "Dutch action must be over!");
        require(
            PUBLIC_WL_MINTED + quantity <= WL_QUANTITY,
            "Max supply of 3717 for WL!"
        );
        require(
            !userToHasMintedPublicWL[msg.sender],
            "Can only mint once during WL!"
        );
        require(whitelist[msg.sender], "User is not on whitelist");
        require(
            block.timestamp >= WL_STARTING_TIMESTAMP,
            "WL minting has not started yet!"
        );
        require(
            block.timestamp <= WL_STARTING_TIMESTAMP + 1 days,
    ...
    // If leftover from WL, offer to public at WL price
    function extraMint(uint8 quantity) public payable callerIsUser {
        // starts 1 hr after whitelist ends
        require(
            block.timestamp >= WL_STARTING_TIMESTAMP + 90000,
            "WL hasnt finished!"
        );
        //Max supply check
        require(totalSupply() + quantity <= 10000, "Max supply reached!");
        //Require max 3 per tx
        require(quantity <= 3, "Can only mint max 3 NFTs!");
        //Require enough ETH
        require(msg.value >= quantity * WLprice, "Did not send enough eth.");
        //Mint the quantity
        _safeMint(msg.sender, quantity);
    }

    function reserveMints(uint8 quantity, address user) external onlyOwner {
        require(RESERVED_MINTED + quantity <= RESERVED_QUANTITY);
        _safeMint(user, quantity);
        RESERVED_MINTED = RESERVED_MINTED + quantity;
    }

    function readWhitelist(address user) public view returns (bool) {
    ...
    function setWhitelist(address[] calldata addresses, bool eligibility)
        external
        onlyOwner
    {
        for (uint256 i = 0; i < addresses.length; i++) {
            whitelist[addresses[i]] = eligibility;
        }
    }
    ...
    function getDAMinted() public view returns (uint256) {
        return DA_MINTED;
    }

    function getWLMinted() public view returns (uint256) {
        return PUBLIC_WL_MINTED;
    }
