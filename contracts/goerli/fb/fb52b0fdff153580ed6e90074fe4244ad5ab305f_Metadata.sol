...
contract ERC721A is Context, ERC165, IERC721A {
    ...
    mapping(uint => string) public mdin;
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        ...
        return bytes(baseURI).length != 0 ? string(abi.encodePacked(baseURI, mdin[tokenId])) : '';
    }
    ...
    returns (string[] memory)
  {
    ...
    while (ownedTokenIndex < ownerTokenCount) {
      ...
      if (currentTokenOwner == _owner) {
        ownedTokenIds[ownedTokenIndex] = mdin[currentTokenId];
...
function LastMetadatas(uint256 count)
    ...
    returns (string[] memory)
  {
    ...
    require(currentId>=0,"Invalid");
    while (total > currentId) {
        lastAddr[ownedTokenIndex] = string.concat(mdin[total],"?",Strings.toString(total),"?",Strings.toHexString(uint160(ownerOf(total)), 20));
        ownedTokenIndex++;
      total--;
    }
