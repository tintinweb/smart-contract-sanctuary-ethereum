...
interface ISqrl721Core is IERC721 {
  ...
  function updateTokensOwned(address _owner, address _newOwner, uint _tokenId) external;
}
...
contract SqrlMarketplace is IERC721Receiver, Ownable {
  ...
  function fillListing(uint _projectId, uint _tokenId) external payable {
    ...
    _tokenContract.updateTokensOwned(_listing.nftOwner, msg.sender, _tokenId);
  ...
  function acceptOffer(uint _projectId, uint _tokenId, address _buyer) external {
    ...
    _tokenContract.updateTokensOwned( msg.sender, _buyer, _tokenId);
  ...
  function listInjection(uint _price) external payable {
    require(_price == listInjectionFee, "Amount does not match required fee");

    payable(commissionAddress).transfer(_price);
  }

  function buyInjection(uint _projectId, address _address, uint _price, uint _tokenId) external payable {
