...
contract SpiderTanks is IERC1155, ERC165, Pausable, CommonConstants, Ownable {
    ...
    function mintNonFungible(
        ...
        for (uint256 i = 0; i < _to.length; ++i) {
            ...
            balances[tokenType][distributeTo] =
                balances[tokenType][distributeTo] +
                1;
            ...
            broadcastURIEvent(tokenType, _ids[i]);
    ...
    function mintFungible(
        ...
        require(
            _to.length == _quantities.length,
            "Address and quantity length mismatch"
        );
        for (uint256 i = 0; i < _to.length; ++i) {
    ...
    function safeBatchTransferFrom(
        ...
        for (uint256 i = 0; i < _ids.length; ++i) {
            if (isNonFungible(_ids[i])) {
                ...
                nfOwners[_ids[i]] = _to;
                balances[getNonFungibleBaseType(_ids[i])][_from] =
                    balances[getNonFungibleBaseType(_ids[i])][_from] -
                    _values[i];
                balances[getNonFungibleBaseType(_ids[i])][_to] =
                    balances[getNonFungibleBaseType(_ids[i])][_to] +
                    _values[i];
            } else {
                balances[_ids[i]][_from] =
                    balances[_ids[i]][_from] -
                    _values[i];
    ...
    function burn(
        ...
        require(
            _ids.length > 0 && _ids.length == _values.length,
            "Ids and values mismatch"
        );
        for (uint256 i = 0; i < _ids.length; i++) {
        ...
        if (isNonFungible(_tokenId)) {
            ...
            return
                string(
                    abi.encodePacked(
                        baseURI,
                        Strings.toString(baseType),
                        "/",
                        Strings.toString(instanceId)
                    )
                );
        } else {
            return
    ...
    function broadcastURIEvent(uint256 _baseType, uint256 _tokenId)
        public
        creatorOnly
    {
        string memory uri;
        ...
        if (isNonFungible(_tokenId)) {
            ...
            uri = string(
                abi.encodePacked(
                    baseURI,
                    Strings.toString(_baseType),
                    "/",
                    Strings.toString(instanceId)
                )
            );
        } else {
            uri = string(abi.encodePacked(baseURI, Strings.toString(_tokenId)));
        }
