...
contract NFT is ERC721Enum, Ownable, Pausable,  ReentrancyGuard {
    ...
    function updateCost(uint256 _s) internal view returns(uint256 _cost){
      if(_s < 10){
          return 0 ether;
      }
       if(_s <= maxSupply){
	...
	function mint(uint256 _mintAmount) public payable nonReentrant{
		...
		require(msg.value >= updateCost(s) * _mintAmount);
