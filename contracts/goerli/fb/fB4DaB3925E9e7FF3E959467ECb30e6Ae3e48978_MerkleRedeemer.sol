...
contract MerkleRedeemer is Ownable {
    ...
    function claim(
        ...
        require(canClaim(proof, week, amount), "redeemer/cannot-claim");
        hasClaimed[msg.sender][week] = true;
        emit Claimed(msg.sender, week, amount);
        require(oopsieDAI.transfer(msg.sender, amount), "redeemer/transfer-failed");
    }
    ...
    function claimMultiple(
        ...
        for (uint16 i = 0; i < _weeks.length; i++) {
            require(canClaim(proofs[i], _weeks[i], amounts[i]), "redeemer/cannot-claim");
            hasClaimed[msg.sender][_weeks[i]] = true;
            total += amounts[i];
    ...
    function canClaim(
        ...
        require(!hasClaimed[msg.sender][week], "redeemer/already-claimed");
