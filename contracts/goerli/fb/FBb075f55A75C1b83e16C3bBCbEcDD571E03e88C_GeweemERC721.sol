...
contract GeweemERC721 is Initializable, ERC721Upgradeable, ERC721EnumerableUpgradeable,ERC721RoyaltyUpgradeable, ERC721URIStorageUpgradeable, PausableUpgradeable, AccessControlUpgradeable, ERC721BurnableUpgradeable, UUPSUpgradeable,NativeMetaTransactionUpgradeable,ContextMixin {
    ...
    function initialize(string memory name, string memory symbol,uint256 mintPrice, uint saleStartsIn ,string memory baseURI,uint initReleaseCount,uint maxSupply,uint getFreeBuyCount,uint getFree,uint promoBefore,address royaltyReceiver,uint96 royaltyFeeBasePoint) initializer public {
        ...
        _mintWhitelistedPrice = 0.00001 ether;
    ...
    function addWhitelistedWallet(address _addressToWhitelist) public onlyRole(MINTER_ROLE) {
    ...
    function mintAssetWhitelisted(uint256 _mintAmount) public isWhitelisted(_msgSender()) payable {
        require(!_isContractPaused, "the contract is paused");        
        require(_mintAmount > 0, "need to mint at least 1 NFT");
    ...
    modifier isWhitelisted(address _address) {
        require(_whitelistedAddresses[_address], "You need to be whitelisted");
        _;
    }
}
