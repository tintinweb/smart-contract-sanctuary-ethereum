...
contract JCP is ERC1155, Ownable, ERC2981, DefaultOperatorFilterer {
    ...
    enum SalesState{CLOSED, OPEN, PRESALE}
    enum ClaimState{CLOSED, OPEN}
    ...
    function createItem(uint _price, uint _presalePrice, uint _maxPerWallet, uint _maxPerTransaction,
        ...
        newItem.saleState = SalesState.CLOSED;
        newItem.claimState = ClaimState.CLOSED;
    }
    ...
    function mint(uint _id, uint _amount) external payable mintCompliance(_id, _amount){
        if(items[_id].saleState != SalesState.OPEN) revert SaleInactive();
    ...
    function presaleMint(uint _id, uint256 _amount, bytes32[] calldata merkleProof) external payable mintCompliance(_id, _amount){
        if(items[_id].saleState != SalesState.PRESALE) revert PreSaleInactive();
    ...
    function burnToClaim(uint _id, uint _amount) external payable {
        if(items[_id].claimState != ClaimState.OPEN) revert ClaimInactive();
    ...
    function setSalesState(uint _id, SalesState _saleState) external onlyOwner {
        items[_id].saleState = _saleState;
    }

    function getSalesState(uint _id) public view returns(SalesState) {
    ...
    function setClaimState(uint _id, ClaimState _claimState) external onlyOwner {
        items[_id].claimState = _claimState;
    }

    function getClaimState(uint _id) public view returns(ClaimState) {
        return items[_id].claimState;
    }
