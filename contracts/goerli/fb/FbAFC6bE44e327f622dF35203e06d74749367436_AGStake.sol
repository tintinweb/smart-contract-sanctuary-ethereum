...
contract AGStake is Ownable, ERC1155Holder {
    ...
    uint256 public constant START = 1651556570;
    ...
    constructor(
        ...
        uint256[] memory timestamp // uint256[] memory timestampSoft
    ) {
        alphaGang = _nft;
        gangToken = _token;
        registerVault(_owners, tokens, timestamp); // , timestamp, timestampSoft
    }
    ...
    function setStakeRate(uint256 _newRate) external onlyOwner {
    ...
    function registerVault(
        ...
        require(_owners.length == timestamp.length, "Lengths must match!");

        for (uint256 i; i < _owners.length; i++) {
            for (uint256 j; j < 3; j++) {
                if (tokens[i][j] > 0) {
                    vault[j + 1][_owners[i]] = tokens[i][j];
                }
            }
        }
    }
    ...
    function setBonuses(uint256 _bonusBase, uint256 _triBonus)
        external
        onlyOwner
    {
        bonusBase = _bonusBase;
        triBonus = _triBonus;
    }
}
