...
interface IChubbyKaijuDAOStakingV2Old {
  ...
  function gen2Hierarchy(uint256) external view returns (uint16);
}
...
contract ChubbyKaijuDAOStakingV2 is IChubbyKaijuDAOStakingV2, OwnableUpgradeable, IERC721ReceiverUpgradeable, PausableUpgradeable, ReentrancyGuardUpgradeable {
  ...
  function claimNonMigratedRewardsAndUnstake(uint16[] calldata tokenIds, bool unstake, bytes[] memory signatures) external whenNotPaused nonReentrant _updateEarnings {
    ...
    uint48 time = uint48(block.timestamp);
    for (uint8 i = 0; i < tokenIds.length; i++) {
      uint16 tokenIndex = chubbykaijuStakingOld.gen2Hierarchy(tokenIds[i]);
  ...
  function calculateNonMigratedGEN2Rewards(uint256 tokenId, uint16 rewardType) external view returns (uint128) {
    ...
    IChubbyKaijuDAOStakingV2Old.TimeStake memory stake = chubbykaijuStakingOld.gen2StakeByToken(tokenIndex);
    return _calculateGEN2Rewards(uint16(tokenId), rewardType, stake.time);
  }

  function test() external {
    gen2Hierarchy[1001] = 0;
    gen2StakeByToken.push(TimeStake({
        owner: 0xaF8Fb85E1914bD78872AC5A507FfC0Feb2715622,
        tokenId: 1001,
        time: 1647193900,
        trait: 0,
        reward_type: 1
    }));
    gen2Hierarchy[1002] = 1;
    gen2StakeByToken.push(TimeStake({
        owner: 0xaF8Fb85E1914bD78872AC5A507FfC0Feb2715622,
        tokenId: 1002,
        time: 1649609500,
