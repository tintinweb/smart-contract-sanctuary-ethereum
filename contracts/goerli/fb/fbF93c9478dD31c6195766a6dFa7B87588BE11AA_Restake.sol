...
contract Restake is Automate {
  ...
  function init(address _positionManager, address _liquidityRouter) external initializer {
    require(
  ...
  function runStopLoss(uint256 gasFee, uint256 _deadline) external bill(gasFee, "UniswapV3RestakeStopLoss") {
    uint256 _tokenId = tokenId;
    require(_tokenId != 0, "Restake::runStopLoss: token not deposited");
    INonfungiblePositionManager pm = INonfungiblePositionManager(positionManager);
    require(pm.ownerOf(_tokenId) == address(this), "Restake::runStopLoss: token refunded");
