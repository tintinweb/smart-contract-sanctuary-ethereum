  ...
  OwnableUpgradeable
{
  ...
  mapping (uint16 => uint256) private _heros;
  ...
  function mint(uint16 _heroId,uint256 count)
    external
    preMintChecks(_heroId,count, msg.sender)
    postMintChecks
  {
    require(open == true, "Mint not open");
    IERC20(_Dai).transferFrom(msg.sender,erc20Recipient,mintCostOfOwner(count));
    uint256 endTokenId = _currentIndex + count - 1;
     unchecked {
            heroInfos[endTokenId].heroType = _heroId;
        }
    _heros[_heroId] = _heros[_heroId] - count;
  ...
  // ------ Read ------
      function _heroOf(uint256 tokenId) public view returns (heroInfo memory) {
        uint256 curr = tokenId;

        unchecked {
             if (curr < _currentIndex) {
                heroInfo memory _heroInfo = heroInfos[curr];
                    while (true) {
                        if (_heroInfo.heroType != 0) {
                            return _heroInfo;
                        }
                        curr++;
                         _heroInfo = heroInfos[curr];
    ...
    function hero(uint16 _heroId) public view returns (uint256) {
    return _heros[_heroId];
  }
  ...
  modifier preMintChecks(uint16 _heroId, uint256 count, address to) {
    require(count > 0, "Mint at least one.");
    require(count <= maxPerMint, "Max mint reached.");
    require(count <= _heros[_heroId], "reached max sale num");
    _;
  }
