...
contract BigDay is Ownable, ERC721Royalty, Pausable, ReentrancyGuard, SignatureValidator, TokenAsDate, BoundCurve {
    ...
    event EventAdded(uint256 indexed tokenId, uint256 eventIndex, address collection, uint256 coverTokenId);
    // event EventUpdated(uint256 indexed tokenId, uint256 eventIndex);
    event ActiveEventChanged(uint256 indexed tokenId, uint256 oldIndex, uint256 newIndex);
    event EventsStatesUpdated(uint256 indexed tokenId);
    ...
    uint8 constant INITIAL_EVENT_ID = 0;
    string constant INITIAL_EVENT_LABEL = "";

    string private _contractMetadataURI;
    bool public isPublicSalesOpen = false;
    string public wepAppBaseURL = "ipfs://contract.metadata";
    uint256 public maxEventsAmount = 24;
        ...
        whenNotPaused
    {
        ...
        require(block.timestamp - voucher.ts < 600, "Voucher is expired");
        ...
        token.tokenActiveEvent = 0;
        token.attributes = BigDayTokens.Attributes(voucher.year, voucher.month, voucher.day, voucher.genId);

        // bounding curve related
        mintRegistration(voucher.tokenId);

        emit EventAdded(voucher.tokenId, 0, address(this), voucher.tokenId);
    }

    function privateMint(PrivateMintPayload calldata payload, PrivateMintEvent calldata _event)
        external
        _dateBounds(payload.tokenId)
        onlyOwner
    {
        require(!_exists(payload.tokenId), "The token already taken");
        _safeMint(msg.sender, payload.tokenId);

        BigDayTokens.BigDayToken storage token = _tokens[payload.tokenId];
        token.attributes = BigDayTokens.Attributes(payload.year, payload.month, payload.day, payload.genId);
        token.events.push(
            Events.Event(
                address(this),
                payload.tokenId,
        ...
        if (_event.collection != address(0)) {
            //TODO require if onwer of event-cover is this contract
            token.events.push(
                Events.Event(
                    _event.collection,
                    _event.coverTokenId,
            ...
            token.tokenActiveEvent = 1;
        } else {
            token.tokenActiveEvent = 0;
        }
        // emit EventAdded(payload.tokenId, 0);
    }
    ...
    function addEvent(
        ...
        _tokens[tokenId].addEvent(maxEventsAmount, collection, coverTokenId, align, label, desc, msg.sender);
        emit EventAdded(tokenId, _tokens[tokenId].tokenActiveEvent, collection, coverTokenId);
    }

    function getEvents(uint256 tokenId, bool showHidden) public view returns (Events.Event[] memory) {
        return _tokens[tokenId].getEvents(showHidden);
    }

    function getEvent(uint256 tokenId, uint256 eventIndex) public view returns (Events.Event memory) {
        return _tokens[tokenId].getEvent(eventIndex);
    }
    ...
    function updateImageUrl(
        ...
        _tokens[tokenId].updateMediaUrl(eventIndex, _imageUrl);
        // emit EventUpdated(tokenId, eventIndex);
    }

    function setEventsStates(uint256 tokenId, bool[] calldata _visibility) external _isTokenOwner(tokenId) {
        _tokens[tokenId].setEventsStates(_visibility);
        emit EventsStatesUpdated(tokenId);
    }

    function changeActiveEvent(uint256 tokenId, uint256 newEventIndex) external _isTokenOwner(tokenId) {
        uint256 oldEventIndex = _tokens[tokenId].tokenActiveEvent;
        _tokens[tokenId].changeActiveEvent(newEventIndex);
        emit ActiveEventChanged(tokenId, oldEventIndex, newEventIndex);
    }
    ...
    function setMaxEventsAmount(uint256 _maxEventsAmount) external onlyOwner {
        maxEventsAmount = _maxEventsAmount;
    }
...
contract BoundCurve {
    ...
    uint256 constant PERCENT_PREC = 10000;

    uint256 constant MIN_MINT_PRICE = 10**14; // 0.0001
    uint256 MIDDLE_POINT_HOURS = 24;

    uint256 minSlope = 10100; //+1%
    uint256 maxSlope = 30000; //+200%
    uint256 slopeWidth = maxSlope - minSlope;
    uint256 TIMESTEP_PERIOD = 600; // 600 seconds = 10 min

    uint256 slope = minSlope; // from 1% to 50%;
    mapping(uint256 => uint256) private _shareCoinsByTokenId;
    mapping(uint256 => uint256) private _fullReleaseMintIdByTokenId;
    mapping(address => uint256[]) private _tokenIdsByUser;

    uint256 private _shareCoinsTotalSupply = 0;
    uint256 private _feeShareCoins = 0;
    uint256 private _mintsCounter = 0; //TODO increment

    uint256 lastMintTime = 0;
    uint256 lastPrice = 0;
    uint256 prevTimeStep = 0;
    uint256 nextGrowK = 10100;
    uint256 currentGrowK = 10100;
    uint256 currentPrice = MIN_MINT_PRICE;
    uint256 holdPrice = MIN_MINT_PRICE;
    ...
    function getContractInfo()
        ...
        view
        returns (
            uint256 secondsPassed, //from last mint
            uint256 currentTimestep,
            uint256 timestepProgress,
            //
            uint256 currentMintPriceMultiplier,
            uint256 currentMintPrice,
            // uint256 currentSlope,
            //
            uint256 nextMintPriceMultiplier,
        ...
        )
    {
        ...
        secondsPassed = lastMintTime > 0 ? (block.timestamp - lastMintTime) : 0;
        bool ifMintsWereInCurrentTimestep = secondsPassed < TIMESTEP_PERIOD;
        currentTimestep = block.timestamp / TIMESTEP_PERIOD;
        timestepProgress = block.timestamp % TIMESTEP_PERIOD;

        uint256 secondPassedStepped = (secondsPassed / TIMESTEP_PERIOD) * TIMESTEP_PERIOD;
        uint256 daysPass = (secondPassedStepped * PERCENT_PREC) / (24 * 60 * 60);
        // console.log("::LL", ifMintsWereInCurrentTimestep, currentGrowK);
        // console.log("::KK", _getPriceK(daysPass), "::", daysPass);
        currentMintPriceMultiplier = ifMintsWereInCurrentTimestep ? currentGrowK : _getPriceK(daysPass, slope);
        currentMintPrice = ifMintsWereInCurrentTimestep ? holdPrice : getPrice(currentMintPriceMultiplier);
        // currentSlope = slope;

        nextMintPriceMultiplier = _getNextSlope(daysPass);
        nextMintPrice = (currentPrice * nextGrowK) / PERCENT_PREC;

        totalMints = _mintsCounter;
        totalShares = _shareCoinsTotalSupply;
        totalEths = address(this).balance;
    }

    function getUserInfo() public view returns (uint256 availableShares, uint256 availableEths) {
        availableShares = _getAvailableShares();
        availableEths = (availableShares * address(this).balance) / _shareCoinsTotalSupply;
    }

    function getPrice(uint256 _currentK) public view returns (uint256) {
        uint256 newPrice = (lastPrice * _currentK) / PERCENT_PREC;
        return newPrice < MIN_MINT_PRICE ? MIN_MINT_PRICE : newPrice;
    }

    function getCurrentShareCoinPrice() public view returns (uint256) {
        return address(this).balance / _shareCoinsTotalSupply;
    }

    function mintRegistration(uint256 tokenId) public {
        //buyer ballance
        _shareCoinsByTokenId[tokenId] = 100;

        // //service fee 20%
        _feeShareCoins += 25;

        //total supply
        _shareCoinsTotalSupply += 125; // 25/125 = 20%

        //full release "date"
        _fullReleaseMintIdByTokenId[tokenId] = _mintsCounter + PENDING_PERIOD;

        // mint increment
        _mintsCounter += 1;

        //
        _tokenIdsByUser[msg.sender].push(tokenId);

        _priceRecalc();
    }

    function _priceRecalc() internal {
        uint256 currentTimeStep = block.timestamp / TIMESTEP_PERIOD;
        if (prevTimeStep != currentTimeStep) {
            prevTimeStep = currentTimeStep;

            //recalc K and slope
            uint256 currentTime = block.timestamp;
            uint256 secondsPassed = lastMintTime > 0 ? (currentTime - lastMintTime) : 0;
            uint256 secondPassedStepped = (secondsPassed / TIMESTEP_PERIOD) * TIMESTEP_PERIOD;
            uint256 daysPass = (secondPassedStepped * PERCENT_PREC) / (24 * 60 * 60);

            lastPrice = getPrice(currentGrowK);
            holdPrice = lastPrice;

            slope = _getNextSlope(daysPass);

            currentPrice = (currentPrice * nextGrowK) / PERCENT_PREC;
            currentGrowK = getGrowK(daysPass, slope);
            nextGrowK = currentGrowK;
        } else {
            // multiply nextK
            nextGrowK = (nextGrowK * currentGrowK) / PERCENT_PREC;
        }
    ...
    function _getAvailableShares() internal view returns (uint256) {
        if (msg.sender == feeReceiver) {
            return _feeShareCoins;
        }
        // uint256[] userTokens = _tokenIdsByUser[msg.sender];
        uint256 coinsTotal = 0;
        for (uint256 i = 0; i < _tokenIdsByUser[msg.sender].length; i++) {
            uint256 tokenId = _tokenIdsByUser[msg.sender][i];
            // full amount released
            if (_fullReleaseMintIdByTokenId[tokenId] <= _mintsCounter) {
                coinsTotal = coinsTotal + _shareCoinsByTokenId[tokenId];
            } else {
                uint256 mintsToFullRelease = _shareCoinsByTokenId[tokenId] -
                    (_fullReleaseMintIdByTokenId[tokenId] - _mintsCounter);
                coinsTotal = coinsTotal + mintsToFullRelease;
            }
        }
    ...
    function withdrawShare(uint256 amount) external {
        ...
        if (msg.sender == feeReceiver) {
            // require(_feeShareCoins >= amount, NotEnough);
            if (_feeShareCoins < amount) {
                revert NotEnoughCoins();
            }
            _feeShareCoins -= amount;
        } else {
            if (_getAvailableShares() < amount) {
                revert NotEnoughCoins();
            }
            uint256 amountLeft = amount;
            uint256[] memory tokenslist = _tokenIdsByUser[msg.sender];
            uint256 len = tokenslist.length;
            for (uint32 i = 0; i < len; i++) {
                uint256 tokenId = tokenslist[i];
                uint256 coinsAvailable = (_fullReleaseMintIdByTokenId[tokenId] <= _mintsCounter)
                    ? _shareCoinsByTokenId[tokenId]
                    : _shareCoinsByTokenId[tokenId] - (_fullReleaseMintIdByTokenId[tokenId] - _mintsCounter);
                if (amountLeft >= coinsAvailable) {
                    //get full amount from the token
                    amountLeft -= coinsAvailable;
                    delete _shareCoinsByTokenId[tokenId];
                    delete _fullReleaseMintIdByTokenId[tokenId];
                    if (amountLeft == 0) {
                        break;
                    }
                } else {
                    _shareCoinsByTokenId[tokenId] -= amountLeft;
                    amountLeft = 0;
                    break;
                }
            }
        }
        _shareCoinsTotalSupply -= amount;
        (bool sent, ) = msg.sender.call{ value: amount * getCurrentShareCoinPrice(), gas: 20317 }("");
        require(sent, "Exchange: Failed to send Ether");
    }

    function _getNextSlope(uint256 daysPass) internal view returns (uint256) {
        return daysPass >= PERCENT_PREC ? maxSlope : (minSlope + (slopeWidth * daysPass) / PERCENT_PREC);
    }

    function getGrowK(uint256 daysPass, uint256 _slope) internal view returns (uint256 growK) {
        uint256 _slopeWidth = _slope - minSlope;
        growK = daysPass >= PERCENT_PREC
            ? minSlope + _slopeWidth
            : (minSlope + (_slopeWidth * daysPass) / PERCENT_PREC);
    }

    function getDiscountK(uint256 daysPass) internal pure returns (uint256 discountK) {
        discountK = daysPass > 10000 ? 0 : 10000 - daysPass;
    }

    function _getPriceK(uint256 daysPass, uint256 _slope) public pure returns (uint256) {
        uint256 k = (_slope * getDiscountK(daysPass)) / PERCENT_PREC;
        return k;
    }
}
...
abstract contract TokenAsDate {
    ...
    uint256 private _lowTimeWindow = 4_280_838; // 1 jan 0001
    uint256 private _highTimeWindow = 6_106_684; // 31 dec 4999
    uint256 private constant _HALF_MAX_DAYS = 5_000_000; // 2740 years

    modifier _dateBounds(uint256 day) {
        // todo bounds by epoch
        require(day >= _lowTimeWindow && day <= _highTimeWindow, "Out of date bounds");
        _;
    }

    function setTimeWindow(uint256 ltw, uint256 htw) public {
        require(
            ltw > 1 && ltw < _HALF_MAX_DAYS && htw > _HALF_MAX_DAYS && htw < _HALF_MAX_DAYS * 2,
            "Time window should fit half-days"
        );
        _lowTimeWindow = ltw;
        _highTimeWindow = htw;
    }
