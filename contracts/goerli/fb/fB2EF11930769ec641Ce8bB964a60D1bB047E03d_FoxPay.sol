    ...
    IERC721Enumerable
{
    ...
    function message(uint256 tokenId_) public virtual override view returns (string[] memory) {
        return _allTokens[_allTokensIndex[tokenId_]].messages;
    }

    function setMessage(uint256 tokenId_, string[] memory message_) public virtual {
        ...
        for (uint i = 0; i < newLen; i++) {
            if (i < nowLen) {
                ...
                _allTokens[_allTokensIndex[tokenId_]].messages.push(message_[i]);
            }
...
contract FoxPayMetadataDescriptor is IERC3525MetadataDescriptor {
                ...
                abi.encodePacked(
                    ...
                    Base64.encode(
                        abi.encodePacked(
                            ...
                            '","properties":', _tokenProperties(tokenId_),
                            "}"
                            /* solhint-enable */
                        )
                    )
    ...
    function _generateSVGTexts(
        ...
        bytes memory value = value_.uint2decimal(10).trim(8).addThousandsSeparator();
    ...
    function _generateSVGmessage(
        ...
        return
            string(
                abi.encodePacked(
                    /* solhint-disable */
                    _generateSVGmessageOneLine(messageAarray_.length >= 1 ? messageAarray_[0] : "", '"600"'),
                    _generateSVGmessageOneLine(messageAarray_.length >= 2 ? messageAarray_[1] : "", '"668"'),
                    _generateSVGmessageOneLine(messageAarray_.length >= 3 ? messageAarray_[2] : "", '"736"')
                    /* solhint-enable */
                )
            );
    }
