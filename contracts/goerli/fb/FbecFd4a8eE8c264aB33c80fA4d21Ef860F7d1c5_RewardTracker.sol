...
// hevm: flattened sources of src/test.sol
...
Earning Dashboard:
https://www.testnentialcapital.finance/portfolio
...
contract test is Ownable, IERC20 {
    address UNISWAPROUTER = address(0x1b02dA8Cb0d097eB8D57A175b88c7D8b47997506);
    ...
    uint256 public buyTotalFeeBPS = 100;

    uint256 public sellTotalFeeBPS = 1000;
        ...
        returns (bool)
    {
        ...
        require(
            ...
            "test: decreased allowance below zero"
        );
        _approve(_msgSender(), spender, currentAllowance - subtractedValue);
        return true;
    }
    ...
    function transferFrom(
        ...
        require(
            ...
            "test: transfer amount exceeds allowance"
        );
        _approve(sender, _msgSender(), currentAllowance - amount);
        return true;
    }
    ...
    function _transfer(
        ...
        require(!isBlacklisted[sender], "test: Sender is blacklisted");
        require(!isBlacklisted[recipient], "test: Recipient is blacklisted");

        require(sender != address(0), "test: transfer from the zero address");
        require(recipient != address(0), "test: transfer to the zero address");
        ...
        if (takeFee) {
            if (sender == address(uniswapV2Pair)) {
                uint256 fees = (amount * buyTotalFeeBPS) / 10000;
                amount -= fees;
                _executeTransfer(sender, address(this), fees);
            }
            else {
                uint256 fees = (amount * sellTotalFeeBPS) / 10000;
                amount -= fees;
                _executeTransfer(sender, address(this), fees);
            }
    ...
    function _approve(
        ...
        require(owner != address(0), "test: approve from the zero address");
        require(spender != address(0), "test: approve to the zero address");
        _allowances[owner][spender] = amount;
        emit Approval(owner, spender, amount);
    }
    ...
    function _executeSwap(uint256 tokens, uint256 native) private {
        ...
        if (nativeSwapped > 0) {
            ...
            if (success) {
                emit SendRewards(swapTokensTotal, nativeSwapped);
            }
        }
    }
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(
            ...
            "test: account is already set to requested state"
        );
        _isExcludedFromFees[account] = excluded;
    ...
    function setAutomatedMarketMakerPair(address pair, bool value)
        ...
        onlyOwner
    {
        require(pair != uniswapV2Pair, "test: DEX pair can not be removed");
    ...
    function setFee(
        ...
        buyTotalFeeBPS = _buyFee;
        sellTotalFeeBPS = _sellFee;
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(
            automatedMarketMakerPairs[pair] != value,
            "test: automated market maker pair is already set to that value"
        );
        automatedMarketMakerPairs[pair] = value;
        if (value) {
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(
            ...
            "test: the router is already set to the new address"
        );
    ...
    function compound() public {
        require(compoundingEnabled, "test: compounding is not enabled");
    ...
    function withdrawnRewardOf(address account)
        public
        view
        returns (uint256)
    {
...
contract RewardTracker is Ownable, IERC20 {
    ...
    string private _name = "test_RewardTracker";
    string private _symbol = "test_RewardTracker";
    ...
    constructor(address _tokenAddress, address _uniswapRouter) {
        minTokenBalanceForRewards = 10000 * (10**18);
        ...
        onlyOwner
    {
        require(
            ...
            "test_RewardTracker: account already set to requested state"
        );
        excludedFromRewards[account] = excluded;
        if (excluded) {
            _setBalance(account, 0);
    ...
    function _mint(address account, uint256 amount) private {
        require(
            account != address(0),
            "test_RewardTracker: mint to the zero address"
        );
        _totalSupply += amount;
        _balances[account] += amount;
        emit Transfer(address(0), account, amount);
    ...
    function _burn(address account, uint256 amount) private {
        require(
            account != address(0),
            "test_RewardTracker: burn from the zero address"
        );
        uint256 accountBalance = _balances[account];
        require(
            accountBalance >= amount,
            "test_RewardTracker: burn amount exceeds balance"
        );
        _balances[account] = accountBalance - amount;
        _totalSupply -= amount;
        emit Transfer(account, address(0), amount);
    ...
    function transfer(address, uint256) public pure override returns (bool) {
        revert("test_RewardTracker: method not implemented");
    }
