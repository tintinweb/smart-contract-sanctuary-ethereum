...
contract ChainWavesGenerator {
    ...
    {
        // get traits from id
        string memory chars = selectChars(_hash);
        uint256 modStart = 13;
        _svg = string(
            abi.encodePacked(
                "<svg viewBox='0 0 20 20' width='600' height='600' xmlns='http://www.w3.org/2000/svg' preserveAspectRatio='xMidYMin'><rect width='20' height='20' fill='#7067CF'/><defs><g id='chars' font-family='monospace'>",
                buildEightLines(chars, modStart),
                "<animate attributeName='font-size' attributeType='XML' values='100%;60%;100%' begin='0s' dur='5s' repeatCount='indefinite'/></g><filter id='turbulence'><feTurbulence type='turbulence' baseFrequency='0.38' numOctaves='5' result='noise' seed='",
                AnonymiceLibrary.toString(_tokenId),
                "'/><feDisplacementMap in='SourceGraphic' in2='noise' scale='3' /></filter></defs><use href='#chars' y='0' x='-3' filter='url(#turbulence)' width='20' height='20' fill='#B7C0EE'/> <use href='#chars' y='-4' x='-1' filter='url(#turbulence)' width='20' height='20' fill='#CBF3D2'/></svg>"
            )
        );
    }

    function selectChars(string memory _hash)
        public
        view
        returns (string memory _chars)
    {
        uint256 thisTrait = AnonymiceLibrary.parseInt(
            AnonymiceLibrary.substring(_hash, 3, 4)
        );
        _chars = CHARS[thisTrait];
