...
contract MyContract is ERC721URIStorage, Ownable {
    ...
    // function giftNFT(address nftContract, address _to, uint256 _tokenId) external payable {
    //     require(nftContract.ownerOf(_tokenId) == msg.sender, "You do not own this NFT.");
    //     nftContract.approve(address(this), _tokenId);
    //     transferNFT(_to, _tokenId);
    // }

    // function transferNFT(address _to, uint256 _tokenId) external payable {
    //     ERC721(address(this)).transferFrom(msg.sender, _to, _tokenId);
    // }

    function returnOwner(uint256 _tokenId) public view returns (address) {
