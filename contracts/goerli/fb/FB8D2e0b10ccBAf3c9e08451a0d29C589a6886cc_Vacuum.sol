...
contract Vacuum is Context, IERC20, Ownable {
    ...
    address payable public marketingAddress =
        payable(0xE4C52C48ae7e121B66F1b009d6F10D60F26f2A58); // Marketing Address
    ...
    uint256 private constant _tTotal = 2997924580 * 10**9;
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _whaleLimit = _tTotal;
    ...
    uint256 public _liqSwapFee = 0;
    uint256 public _marketingFee = 200;

    uint256 public _feeBuy = 100;
    ...
    mapping(address => bool) public _isInBlacklist;
    address[] private _blacklist;

    bool _antiWhale = false;
    ...
    uint256 private constant numTokensSellToAddToLiquidity = 29979245 * 10**9;
    ...
    event ReflectionFeeUpdated(uint256 newReflectionFee);
    event AntiWhaleEnabled(uint256 _whaleLimit);
    ...
    constructor() {
        ...
        excludeFromReflection(uniswapV2Pair);
    ...
    function setReflectionFee(uint256 refFee) external onlyOwner {
        ...
        _taxFee = refFee * 10**2;
        emit ReflectionFeeUpdated(_taxFee);
    }

    function setAntiWhale(uint256 whaleLimit, bool enabled) external onlyOwner {
        _whaleLimit = whaleLimit * 10**9;
        _antiWhale = enabled;
        emit AntiWhaleEnabled(_whaleLimit);
    }
    ...
    function swapAndFee(uint256 contractTokenBalance) private lockTheSwap {
        ...
        if (_liqSwapFee + _marketingFee == 0) {
            liqAmount = 0;
            marketingAmount = 0;
        } else {
            liqAmount = contractTokenBalance.mul(_liqSwapFee).div(
                _liqSwapFee + _marketingFee
            );
            marketingAmount = contractTokenBalance.mul(_marketingFee).div(
                _liqSwapFee + _marketingFee
            );
        }
    ...
    function _tokenTransfer(
        ...
        if (!_isExcludedFromFee[sender] && !_isExcludedFromFee[recipient]) {
            ...
            if (uniswapV2Pair == sender) {
                ...
                _liquidityFee =
                    _liqSwapFee.mul(_feeBuy).div(100) +
                    _marketingFee.mul(_feeBuy).div(100);
                if (_liquidityFee > 0) {
                    buyOrSellSwitch = BUY;
                }
            }
            // Sell
                ...
                _taxFee = _taxFee.mul(_feeSell).div(100);
                _liquidityFee =
                    _liqSwapFee.mul(_feeSell).div(100) +
                    _marketingFee.mul(_feeSell).div(100);
                if (_liquidityFee > 0) {
    ...
    function setBuyFee(uint8 feeBuy) external onlyOwner {
        require(feeBuy < 100, "Fee cannot over 100");
        _feeBuy = feeBuy;
        emit BuyFeeUpdated(_feeBuy);
    }

    function setSellFee(uint8 feeSell) external onlyOwner {
        require(feeSell < 100, "Fee cannot over 100");
        _feeSell = feeSell;
        emit SellFeeUpdated(_feeSell);
    }

    function setBlacklists(address[] calldata accounts, bool enabled)
        public
        onlyOwner
    {
    ...
    function addToBlacklist(address account) public onlyOwner {
        require(!_isInBlacklist[account], "already added");
        _isInBlacklist[account] = true;
    ...
    function enableTrading(uint256 _deadBlocks) external onlyOwner {
        ...
        emit TradingEnabled(deadBlocks);
    }
