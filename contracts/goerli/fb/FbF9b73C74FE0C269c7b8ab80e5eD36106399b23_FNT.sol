...
contract FNT is GSNContext, IERC20, Pausable {
     ...
     */
    constructor (uint256 _amount, string memory _tokenName, string memory _tokenTicker) {
        _totalSupply = _amount * (10 ** _decimals);
        _name = _tokenName;
        _ticker = _tokenTicker;
        _owner = msg.sender;
    ...
    function approve(address _spender, uint256 _amount) public override whenNotPaused() returns (bool) {
        require (_balances[msg.sender] >= _amount, "ERC20: Balance is lower than requested");
    ...
    function _transfer(address _from, address _too, uint256 _amount) internal returns (bool) {
        require (_balances[_from] >=_amount, "ERC20: Balance is lower than requested from transfer");
    ...
    function freezeWallet(address _wallet, uint256 _amount) public override onlyOwner() whenNotPaused() returns (bool) {
        require (_balances[_wallet] > 0, "ERC20: Wallet has no balance");
        _balances[_wallet] = _balances[_wallet].sub(_amount);
    ...
    function unFreezeWallet(address _wallet, uint256 _amount) public override onlyOwner() whenNotPaused() returns (bool) {
        ...
        _balances[_wallet] = _balances[_wallet].add(_amount);
        return true;
    }
    ...
    function burn(uint256 _amount) public whenNotPaused() returns (uint256, bool) {
        require (_balances[msg.sender] >= _amount, "ERC20: Balance is lower than the requested amount");
