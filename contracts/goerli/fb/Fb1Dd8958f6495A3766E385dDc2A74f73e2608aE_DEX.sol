...
contract DEX {
     ...
     */
    event LiquidityRemoved(address, uint, uint, uint);
    ...
    function ethToToken() public payable returns (uint256 tokenOutput) {
        ...
        uint resultTokens = price(
            msg.value,
            address(this).balance - msg.value,
            token.balanceOf(address(this))
        );
        token.transfer(msg.sender, resultTokens);
        emit EthToTokenSwap(
            msg.sender,
            "Eth to Balloons",
            msg.value,
    ...
    function tokenToEth(uint256 tokenInput) public returns (uint256 ethOutput) {
        require(tokenInput > 0, "Tokens should be greater than 0");
        uint resultTokens = price(
            tokenInput,
            token.balanceOf(address(this)),
        ...
        payable(msg.sender).transfer(resultTokens);
        emit TokenToEthSwap(
            msg.sender,
            "Balloons to ETH",
            ethOutput,
    ...
    function withdraw(
        ...
        require(amount <= liquidity[msg.sender], "Not enough balance");
