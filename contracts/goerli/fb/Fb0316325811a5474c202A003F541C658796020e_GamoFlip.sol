...
contract GamoFlip is VRFConsumerBaseV2, ReentrancyGuard {
    ...
    mapping (uint256 => preFlip) public PreFlips;
    ...
    event RequestFulfilled(uint256 requestId, uint flipId, uint randomWord);
    ...
    function fulfillRandomWords(
        ...
        preFlip storage getPreFlip = PreFlips[requestId];
        //distribute(getPreFlip.degen, getPreFlip.choice, getPreFlip.ethBetted, randomWords[0], getPreFlip.flipId);
        emit RequestFulfilled(requestId, getPreFlip.flipId, randomWords[0]);
    }
    ...
    function flipEth(bool expectation) payable external nonReentrant {
        ...
        preFlip storage newPreFlip = PreFlips[s_requestId];
        newPreFlip.requestId = s_requestId;
    ...
    function getResults(address degenToPay, uint choice, uint ethBetted, uint randomNumber, uint flipId, uint requestId) external nonReentrant {
        require(PreFlips[requestId].requestId == requestId, "Not correct flip");
        require(PreFlips[requestId].degen == degenToPay, "Not correct degen");
        ...
        degen storage newDegen = Degens[degenToPay];

        if (newDegen.exists == false) {
            newDegen.id = nextDegen;
            newDegen.exists = true;
            newDegen.degenAddress = degenToPay;

            IdsToDegens[nextDegen] = degenToPay;
            nextDegen++;
        }
