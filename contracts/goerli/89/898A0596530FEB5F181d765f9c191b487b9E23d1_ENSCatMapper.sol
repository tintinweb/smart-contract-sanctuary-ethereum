...
contract ENSCatMapper is Ownable {
    ...
    string public domainLabel = "testcat";
    string public nftImageBaseUri =
        "https://ipfs.io/ipfs/QmUFt1jgxwZ2XSReyStLvEJ4Jou38AKDCZAxJueTS5Rhno/";
    ...
    uint256 public reset_period = 300; // 5 minutes

    bool public publicClaimOpen = true;
    ...
    constructor() {
        ...
        nft = IERC1155(0x888EB6a48587F8bff04a827f28985b186FfeDd58);
        ...
        returns (string memory)
    {
        ...
        require(
            token_id > 0 && tokenHashmap[token_id] != 0x0,
            "Invalid address"
        );
        ...
        } else if (keccak256(abi.encodePacked(key)) == keccak256("url")) {
    ...
    function addr(bytes32 nodeID) public view returns (address) {
        ...
        return tokenIdToClaimer[token_id];
    }
        ...
        returns (string memory uri)
    {
        ...
        uri = string(
            abi.encodePacked(
                hashToDomainMap[tokenHashmap[token_id]],
                ".",
                domainLabel,
                ".eth"
            )
    ...
    function setDomain(
        ...
        uint256 token_id
    ) public isAuthorised(tokenOwner, token_id) {
        require(publicClaimOpen || owner() == msg.sender, "Not authorised");
        require(
            block.timestamp > nextRegisterTimestamp[tokenOwner],
            "Wallet must wait more time to register"
        );

        if (tokenHashmap[token_id] != 0x0) {
            _reset(token_id);
        }
        ...
        tokenIdToClaimer[token_id] = tokenOwner;

        if (owner() != msg.sender) {
            nextRegisterTimestamp[tokenOwner] = block.timestamp + reset_period;
        }

        emit RegisterSubdomain(tokenOwner, token_id, label);
        emit AddrChanged(big_hash, tokenOwner);
        emit AddressChanged(
            big_hash,
            60,
            abi.encodePacked(tokenOwner));
    }
    ...
    function setText(
    ...
    ) external isAuthorised(tokenOwner, hashToIdMap[node]) {
        uint256 token_id = hashToIdMap[node];
        require(
            token_id > 0 && tokenHashmap[token_id] != 0x0,
            "Invalid address"
        ...
        require(
            keccak256(abi.encodePacked(key)) != keccak256("avatar"),
            "cannot set avatar"
        );
    ...
    function resetHash(address tokenOwner, uint256 token_id)
        public
        isAuthorised(tokenOwner, token_id)
    {
        _reset(token_id);

        //allow sender to reclaim (if public == true)
        if (
            nextRegisterTimestamp[tokenOwner] > block.timestamp &&
            msg.sender != owner()
        ) {
            nextRegisterTimestamp[tokenOwner] = block.timestamp + (60 * 30); //30 minute cooldown
        }
    }

    function _reset(uint256 token_id)
        internal
    {
        ...
        tokenHashmap[token_id] = 0x0;
        tokenIdToClaimer[token_id] = address(0);
    }
    ...
    modifier isAuthorised(address tokenOwner, uint256 tokenId) {
        require(
            owner() == msg.sender ||
