...
contract MyNFT is ERC721, Ownable {
    ...
    bool internal allowBuyForWhitelist;
    bool internal allowBuyForPublic;
    ...
    constructor(
        ...
        totalTokenCountTarget = tokenAmount;
        NFTprice = price;
    ...
    function isBuyAllowed(address account) public view returns (bool) {
        return
            isInWhitelist(account) ? allowBuyForWhitelist : allowBuyForPublic;
    }
    ...
    function buyNFTs(uint256 amount) external payable {
        uint256 price = priceNFT() * amount;

        require(
            lastSoldID.current() + amount <= totalTokenCountTarget,
            "Not enough tokens for sale"
        );
        require(msg.value >= price, "Not enough ETH to buy tokens");
        require(isBuyAllowed(msg.sender), "Buy is currently not allowed");
    ...
    function priceNFT() public view returns (uint256) {
        return isInWhitelist(msg.sender) ? whitelistPrice : NFTprice;
    }
    ...
    function hasShares(address account) public view returns (bool) {
