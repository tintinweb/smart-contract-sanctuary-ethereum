...
contract y00tsAnchorClubTest is ERC721A, Ownable, ReentrancyGuard {
  ...
  uint   public price = 0.0 ether;
    ...
    constructor(address _oldContract) ERC721A("y00ts Anchor Club Test", "YACT") {    
	...
	function _getNextUnusedID(address _owner, uint256[] memory _ownedTokens) internal view returns (uint256) {

		for (uint256 i = 0; i < _ownedTokens.length; i++) {
			uint256 index = _ownedTokens[i];
