...
contract GachaDraw is Ownable {
    ...
    function updateMaximumRewardSupply(
        ...
        for (uint256 index = 0; index < _maxRewardSupply.length; index++) {
            if (
                _maxRewardSupply[index] < activity.totalGivenRewardSupply[index]
            ) {
                revert InvalidInput(
        ...
        validActivity(_activityId)
    {
        ...
        uint256 remainingSupplyRunning = activities[_activityId].remainingRewardSupply;
        ...
        for (
            ...
            for (
                ...
                rewardTokenIndex < activities[_activityId].rewardTokenIds.length;
                rewardTokenIndex = _uncheckedIncOne(rewardTokenIndex)
            ) {
                uint256 tokenAmountYetToBeMinted = activities[_activityId].maximumRewardSupply[
                    rewardTokenIndex
                ] -
                    activities[_activityId].totalGivenRewardSupply[
                        rewardTokenIndex
                    ];
                indexCount = _uncheckedIncDelta(
                    indexCount,
                    tokenAmountYetToBeMinted
                );
                if (tokenAmountYetToBeMinted > 0 && indexCount >= randomIndex) {
                    selectedTokenId = activities[_activityId].rewardTokenIds[rewardTokenIndex];
                    activities[_activityId].totalGivenRewardSupply[
                            rewardTokenIndex
                        ] = _uncheckedIncOne(
                        activities[_activityId].totalGivenRewardSupply[
        ...
        activities[_activityId].remainingRewardSupply = remainingSupplyRunning;
