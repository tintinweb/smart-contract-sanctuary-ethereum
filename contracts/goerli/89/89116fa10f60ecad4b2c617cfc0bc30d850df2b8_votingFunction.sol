...
contract votingFunction is myDao {
    ...
    function voteAdmin(uint id) public verifyMember {
    ...
    function resultAdmin() public returns(address,uint){
        ...
        require(block.number-submittedProposals[submittedProposals.length-1].blockNumber>50,"Time still remaining");
        uint temp;
        uint winner;
         for(uint i=0;i<memberCount;i++) {
    ...
    function viewCandidates(uint id) public view returns(address){
        require(newAdmin,"No voting session exist");
        return candidates[id].candidateAddress;
    }
}