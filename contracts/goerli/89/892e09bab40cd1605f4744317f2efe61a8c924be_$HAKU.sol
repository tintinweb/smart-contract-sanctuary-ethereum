...
contract $HAKU is Context, IBEP20, Ownable { // contract name
    ...
    address payable private _marketingAddress = payable(0x000000000000000000000000000000000000dEaD); // Marketing Address
    address payable private _burnAddress = payable(0x000000000000000000000000000000000000dEaD); // Burn Address

    string private _name = "Nigihayami Kohakunushi"; // token name
    string private _symbol = "$HAKU"; // token symbol
    ...
    uint256 private _tokenFee = 0; // marketing fee 0% to be sent as tokens
    uint256 private _previousTokenFee = _tokenFee; // marketing tokens fee
    
    uint256 private _buyBackFee = 0; // buyback fee 0%
    uint256 private _previousBuyBackFee = _buyBackFee; // buyback fee

    uint256 private _marketingBNBFee = 0; // marketing BNB fee
    uint256 private _previousMarketinBNBFee = _marketingBNBFee; // marketing BNB fee

    uint256 public _liquidityFee = 5; // liquidity fee 5%
    ...
    uint256 public _maxTxAmount =  1000000 * 10**9; // max allowed tokens tranfer per transaction
    uint256 private _minTokensSwapToAndTransferTo = 10000 * 10**9; // min token liquidity fee collected before swapandLiquify
    uint256 public _maxWalletToken          = 3000000 * 10**9; // Max number of tokens that an address can hold 3% of total supply
     ...
     */
    function setMarketingTokenFeePercent(uint256 Fee) private onlyOwner {
        _tokenFee = Fee;
    }
    ...
    function setMaxTokenPerAddress(uint256 maxTokens) external onlyOwner {
        _maxWalletToken = maxTokens.mul( 10**_decimals );
    }
     ...
     * @dev set's  address
     */
    function setMarketingAddress(address payable marketingAddress) private onlyOwner {
	...
	*/
    ...
    function _transfer(
        ...
        require(_isExcludedFromAntiWhale[to] || balanceOf(to) + amount <= _maxWalletToken,
