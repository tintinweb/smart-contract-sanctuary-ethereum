 ...
 * {ERC721Enumerable}.
 */
contract Bound2 is Context, ERC165, IERC721, IERC721Metadata, AdminControl {
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        ...
        return _hasUnbound ? _splitTokenURI : _tokenURI;
    }
    ...
    function lock() public adminRequired {
        _locked = !_locked;
    }

    function createNew() public {
        require(_hasUnbound, "Can't create new");
        require(msg.sender == ownerOf(prevToken1 == 0 ? prevToken2 : prevToken1), "Not owner");
        _hasUnbound = false;
        _highestToken++;
        _mint(msg.sender, _highestToken);
        if (prevToken1 == 0) {
            prevToken1 = _highestToken;
        } else {
            prevToken2 = _highestToken;
        }
    }
    ...
    function _afterTokenTransfer(
        ...
        if (_locked && !_hasUnbound && from != address(0x0)) {
            _hasUnbound = true;
            if (firstTokenId == prevToken1) {
                prevOwner1 = from;
            } else {
                prevOwner2 = from;
            }
