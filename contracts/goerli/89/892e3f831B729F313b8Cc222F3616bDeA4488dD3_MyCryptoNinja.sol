...
contract MyCryptoNinja is ERC721A, Ownable {
    ...
    function whitelistMint(uint256 quantity, bytes32[] calldata _merkleProof) external callerIsUser payable {
        require(mintStatus == MintStatus.WHITELIST && block.timestamp >= whitelistStartTime, "Not in whitelist stage");
    ...
    function publicMint(uint256 quantity) external callerIsUser payable {
        require(
            (mintStatus == MintStatus.PUBLIC && block.timestamp >= publicStartTime) || whitelistMinted >= 2000,
            "Not in public stage"
        );
    ...
    function _beforeTokenTransfers(
        ...
        if (from == address(0)) {
            for (uint i = 0; i < quantity; i++) {
                ninjaTrainTimesMap[startTokenId + i].push(block.timestamp);
            }
        }
    }
    /********** 721A HOOK END **********/
