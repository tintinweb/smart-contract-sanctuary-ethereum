...
contract Vault1155 is Vault721 {
  ...
  function redeem(uint256 _lockId) external override onlyMarket {
    _redeem(_lockId);
  }
  ...
  function _mintWNft(address _renter, uint256 _lockId) internal override {
    IMarket.Lend memory _lend = IMarket(marketContract).getLendRent(_lockId).lend;
    IWrap1155(wrapContract).emitTransfer(
      address(this),
      _renter,
      _lend.tokenId,
      _lend.amount,
  ...
  //TODO: function flashloan(address _tokenAddress, uint256 _tokenId, uint256 _amount, address _receiver,bytes memory _params) external override;
...
contract Vault721 is ERC721, Pausable {
  ...
  mapping(uint256 => bool) private tokenIdAllowed;
  mapping(address => uint256) public minPrices;
  address[] public paymentTokens;
  bool public allTokenIdAllowed;
  ...
  function _setCollectionOwnerFeeRatio(uint256 _collectionOwnerFeeRatio) internal {
    require(_collectionOwnerFeeRatio <= 90 * 1000, 'collectionOwnerFeeRatio>80%');
  ...
  function _setCollectionOwnerFeeRatio(uint256 _collectionOwnerFeeRatio) internal {
    ...
    require(_protocolAdminFeeRatio + _collectionOwnerFeeRatio <= 100 * 1000, 'TotalFeeIsOver100%');
    collectionOwnerFeeRatio = _collectionOwnerFeeRatio;
  }
  */
  ...
  function _setMinPrices(uint256[] memory _minPrices, address[] memory _paymentTokens) internal {
    ...
    require(_minPrices.length == _paymentTokens.length, 'MustBeSameLength');
    unchecked {
  ...
  function getTokenIdAllowed(uint256 tokenId) external view returns (bool) {
    if (allTokenIdAllowed) return true;
    return tokenIdAllowed[tokenId];
  }

  function setTokenIdAllowed(uint256[] calldata _tokenIds, bool[] calldata _allowed)
    external
    onlyCollectionOwner
  {
    require(_tokenIds.length == _allowed.length, 'MustBeSameLength');
    unchecked {
    ...
    allTokenIdAllowed = false;
  }

  function setAllTokenIdAllowed(bool _bool) external onlyCollectionOwner {
...
contract Wrap1155 is ERC1155 {
    ...
    constructor(string memory _name, string memory _symbol, address marketAddress) ERC1155('') {
        name = _name;
        symbol = _symbol;
        _vault = msg.sender;
    ...
    function balanceOf(address account, uint256 id) public view override returns (uint256) {
        ...
        for (uint256 x = 0; x < _userToLocks[id][account].length; x++) {
            IMarket.Rent[] memory _rents = IMarket(_market).getLendRent(_userToLocks[id][account][x]).rent;
            for (uint256 i = 0; i < _rents.length; i++) {
                if (_rents[i].renterAddress == account && _rents[i].rentalExpireTime > block.timestamp)
    ...
    function emitTransfer(
        ...
        bool _exist = false;
        for (uint256 i = 0; i < _tokenToLocks[id].length; i++) if (_tokenToLocks[id][i] == lockId) _exist = true;
        if (!_exist) _tokenToLocks[id].push(lockId);
        _exist = false;
        for (uint256 i = 0; i < _userToLocks[id][to].length; i++) if (_userToLocks[id][to][i] == lockId) _exist = true;
        if (!_exist) _userToLocks[id][to].push(lockId);
        emit TransferSingle(_vault, from, to, id, amount);
    }
    ...
    string memory svg = string(
      abi.encodePacked(
        abi.encodePacked(
          ...
          _tokenSymbol,
          "</tspan></text><text fill='#5A6480' font-family='Inter' font-size='24' font-weight='900'><tspan x='28' y='150'>Claimable Funds</tspan></text><text fill='#5A6480' font-family='Inter' font-size='24' font-weight='900'><tspan x='385' y='150'>",
          parsed,
...
library Detector {
  ...
  function availability(
    ...
    IMarket.Rent[] memory _rents,
    IMarket.Lend memory _lend,
    ...
    if (is1155(_originalNftAddress)) {
      ...
      // Check for rental availability
      return 0 < _lend.amount - _rentaled;
    }
