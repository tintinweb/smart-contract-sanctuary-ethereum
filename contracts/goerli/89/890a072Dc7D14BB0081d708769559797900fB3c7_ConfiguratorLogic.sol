...
library ConfiguratorLogic {
   ...
   **/
  event CTokenUpgraded(address indexed asset, address indexed proxy, address indexed implementation);
  ...
  function executeInitReserve(
    ...
    cachePool.initReserve(input.underlyingAsset, cTokenProxyAddress, debtTokenProxyAddress, input.interestRateAddress);
  ...
  function executeInitNft(
    ...
    (address cNFTProxy, ) = registry_.getCNFTAddresses(input.underlyingAsset);
  ...
  function executeUpdateCToken(ILendPool cachedPool, ConfigTypes.UpdateCTokenInput calldata input) external {
    DataTypes.ReserveData memory reserveData = cachedPool.getReserveData(input.asset);

    _upgradeTokenImplementation(reserveData.cTokenAddress, input.implementation, input.encodedCallData);

    emit CTokenUpgraded(input.asset, reserveData.cTokenAddress, input.implementation);
  }
  ...
  function getTokenImplementation(address proxyAddress) external view returns (address) {
    ConfettiUpgradeableProxy proxy = ConfettiUpgradeableProxy(payable(proxyAddress));
  ...
  function _initTokenWithProxy(address implementation, bytes memory initParams) internal returns (address) {
    ConfettiUpgradeableProxy proxy = new ConfettiUpgradeableProxy(implementation, address(this), initParams);
...
interface ICNFT is IERC721MetadataUpgradeable, IERC721ReceiverUpgradeable, IERC721EnumerableUpgradeable {
  /**
...
interface ICNFTRegistry {
  ...
  event CNFTCreated(address indexed nftAsset, address cNFTImpl, address cNFTProxy, uint256 totals);
  event CNFTUpgraded(address indexed nftAsset, address cNFTImpl, address cNFTProxy, uint256 totals);

  function getCNFTAddresses(address nftAsset) external view returns (address cNFTProxy, address cNFTImpl);

  function getCNFTAddressesByIndex(uint16 index) external view returns (address cNFTProxy, address cNFTImpl);
   ...
   **/
  function createCNFTWithImpl(address nftAsset, address cNFTImpl) external returns (address cNFTProxy);
...
contract ConfettiUpgradeableProxy is TransparentUpgradeableProxy {
  constructor(
    address _logic,
    address admin_,
    bytes memory _data
