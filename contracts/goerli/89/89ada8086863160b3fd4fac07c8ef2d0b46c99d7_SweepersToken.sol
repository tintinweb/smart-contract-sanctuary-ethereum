...
contract SweepersToken is ISweepersToken, OwnableUpgradeable, ERC721CheckpointableUpgradeable, IERC721ReceiverUpgradeable, ReentrancyGuardUpgradeable {
    ...
    mapping(uint8 => uint16) private multiplier;
    mapping(address => stakedNFT) private StakedNFTInfo; // address to struct
    mapping(address => uint256) public userStakedSweepers;

    mapping(address => bool) private remover; // address which will call to unstake if NFT is listed on OpenSea while staked
    ...
    // function setSingleMultiplier(uint8 _index, uint16 _mult) external override onlyOwner {
    ...
    function stakeAndLock(uint16[] calldata _ids) external override nonReentrant {
        ...
        for (uint16 i = 0; i < length; i++) {
            ...
            require(msg.sender == ownerOf(_ids[i]), "Not owner of Sweeper");
    ...
    function getUnclaimedDust(address account) external view override returns (uint256 owed, uint256[] memory ownedSweepers, uint256[] memory dustPerNFTList, uint256[] memory multipliers, bool[] memory isStaked) {
        ...
        for (uint i = 0; i < length; i++) {
            ...
            if(isStakedAndLocked[_id]) {
                ...
                tokenDustValue = (_owed * _multiplier) / 10000; 
    ...
    function removeStake(uint16 _id) external override onlyRemover {
        ...
        require(isStakedAndLocked[_id], "NFT is not staked");
