    ...
    ReentrancyGuard
{
    ...
    address private tokenContractAddress =
        0xB3089Dd9c6fAb1eB5dB6f65dcb8E0BD79bE11cEe;
    ...
    function mintTo(address _to, uint256 _amount) public payable {
        ...
        require(
            ...
            "Cannot mint over supply cap of 1000"
        );
        require(
            msg.sender == tx.origin,
            "Recipient must not be a smartcontract"
        );
        ...
        require(
            tempContract.getBurnedBy(_to) - burnedTokensSpent[_to] >
                (burnFee * _amount),
            "Contract does not own burned tokens"
        );

        uint256 burnedTokenUpdate = burnedTokensSpent[_to] +
            (burnFee * _amount);
        burnedTokensSpent[_to] = burnedTokenUpdate;
        _safeMint(_to, _amount, false);
    }

    // Getter function for the burnedTokensSpent mapping
    function getBurnedTokensSpent(address _address)
        public
        view
        returns (uint256)
    {
    ...
    function setBurnFee(uint256 _value) public {
        ...
        require(
            msg.sender == owner(),
            "Only the owner can set the burnFee variable"
        );
        burnFee = _value;
    }
