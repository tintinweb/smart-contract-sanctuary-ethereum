...
contract GiftNFT is ERC721URIStorage, Ownable {
    ...
    function mint(address fromAddress, string memory _fromName, address _to, string memory message, address nftContractAddress, uint256 nftTokenId) public payable {
        require(msg.value >= COST, "Insufficient funds");
        require(MAX_SUPPLY >= _tokenIds.current(), "Max number has been minted");

        //increment tokenId - started at 0//
        _tokenIds.increment();
        _gifts[_tokenIds.current()] = Gift(message, _fromName, fromAddress, nftContractAddress, nftTokenId);
    ...
    function canExchange(uint tokenId) public returns (bool) {
    ...
    function exchangeGift(uint256 tokenId) public payable onlyOwnerOf(tokenId){
        Gift memory gift = _gifts[tokenId];
        IERC721 nftAddress = IERC721(gift.nftContractAddress);
        nftAddress.safeTransferFrom(gift.fromAddress, msg.sender, gift.nftTokenId);
    }
            ...
            Base64.encode(
                bytes(
                    abi.encodePacked(
                        ...
                        '<text class="h1" x="80" y="70">Gift NFT</text>',
                        unicode'<text x="80" y="305">From: ',
                        gift.fromName,
                        "</text>",
                        "</svg>"
                    )
