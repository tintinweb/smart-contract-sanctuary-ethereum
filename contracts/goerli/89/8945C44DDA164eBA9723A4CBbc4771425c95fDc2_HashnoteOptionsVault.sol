...
contract MockHashnoteOptionsVault is HashnoteOptionsVault {
    constructor(address _vaultShare, address _marginEngine) HashnoteOptionsVault(_vaultShare, _marginEngine) { }
...
import { SafeMath } from "../../../../lib/openzeppelin/contracts/utils/math/SafeMath.sol";
...
contract HashnoteVault is ERC1155TokenReceiver, OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    IVaultShare public immutable vaultShare;
     ...
     */
    constructor(address _vaultShare) {
        if (_vaultShare == address(0)) revert HV_BadAddress();

        vaultShare = IVaultShare(_vaultShare);
    }

    /**
     * @notice Initializes the Vault contract with storage variables.
     */
    function baseInitialize(Vault.InitParams calldata _initParams, Vault.VaultParams calldata _vaultParams) internal {
    ...
    function requestWithdraw(uint256 numShares) external {
        ...
        vaultShare.transferFrom(msg.sender, address(this), address(this), numShares, "");
    ...
    function redeem(uint256 numShares, bool isMax) public nonReentrant {
        ...
        vaultShare.transferFrom(address(this), msg.sender, address(this), numShares, "");
    }
    ...
    function _closeRound() internal {
        ...
        uint256[] memory perforamceFees;
        {
            ...
            params.currentShareSupply = vaultShare.totalSupply(address(this));
        ...
        vaultShare.mint(address(this), address(this), mintShares);
    ...
    function _completeWithdraw() internal {
        ...
        if (withdrawShares != 0) {
            ...
            uint256[] memory withdrawAmounts =
                VaultUtil.withdrawWithShares(vaultPauser, withdrawShares, vaultShare.totalSupply(address(this)), collaterals);
            ...
            vaultShare.burn(address(this), address(this), withdrawShares);
    ...
    function shareBalances(address account) public view returns (uint256 heldByAccount, uint256 heldByVault) {
        ...
        if (depositReceipt.round < Vault.PLACEHOLDER_UINT) {
            return (vaultShare.getBalanceOf(account, address(this)), 0);
        }
        ...
        heldByAccount = vaultShare.getBalanceOf(account, address(this));
    }
...
contract HashnoteOptionsVault is HashnoteVault, HashnoteOptionsVaultStorage, IBatchAuctionSeller {
    ...
    /**
     * @notice Initializes the contract with immutable variables
     * @param _vaultShare is the erc1155 contract that issues shares
     * @param _marginEngine is the margin engine used for Grappa (options protocol)
     */
    constructor(address _vaultShare, address _marginEngine) HashnoteVault(_vaultShare) {
    ...
    function startAuction() external {
        ...
        auctionId = AuctionUtil.startAuction(params);
