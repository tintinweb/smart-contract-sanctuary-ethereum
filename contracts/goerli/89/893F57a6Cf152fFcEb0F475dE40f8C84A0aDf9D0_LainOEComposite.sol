...
// Do we need signature for composite lain?
import "@openzeppelin/contracts-upgradeable/utils/cryptography/ECDSAUpgradeable.sol";

contract LainOEComposite is
    ...
    UUPSUpgradeable
{
    ...
    // Variables
    ILainOE public lainOE;
    mapping(uint256 => uint256) lainLevel;

    // Errors
    error NotAllowed();
    error MintCap();
    error LevelDifference();
    error CompositeLevelCap();
    error CompositeBurnNumbersDifferent();
    error InvalidDeificationCount();
    error InvalidDeificationLevel();
    error InsufficientSoulmatePoints();
    error CardNoStock();

    // Events
    event Composite(uint256 tokenId, uint256 burnedId, uint level);
    event Deification(uint256 godId, uint256[] burnIds);
    event TradingCardRedemption(uint256[] tokenIds);
    ...
    function initialize(
        ...
        __ERC721_init(_contractName, _contractSymbol);
        __ERC721Burnable_init();
        __Ownable_init();
        __ERC2981_init();
        __ReentrancyGuard_init();
        lainOE = ILainOE(_OEAddress);
    }
    ...
    function mint(uint256[] calldata tokenIds) external {
        ...
        for (uint256 i; i < count; ) {
            ...
            address owner = lainOE.ownerOf(id);

            // Check whether we're allowed to migrate this Edition.
            if (
                owner != msg.sender &&
                (!lainOE.isApprovedForAll(owner, msg.sender)) &&
                lainOE.getApproved(id) != msg.sender
            ) {
                revert NotAllowed();
            }
            ...
            // Initialize our Check -> for storing our 'levels'
            ...
            // }
            // Map the minted nft to the level
            lainLevel[id] = 1;
            unchecked {
                ++i;
            }
    ...
    */
    ...
    mapping(address => uint256) whitelistPoints;

    function uploadSoulmatesWhiteList(
        SoulmatesWhiteListData[] calldata soulmateHolderList
    ) external onlyOwner {
        for (uint256 i = 0; i < soulmateHolderList.length; ) {
            whitelistPoints[
                soulmateHolderList[i].SOULMATE_HOLDER
            ] = soulmateHolderList[i].POINTS;
    ...
    // Can we do the exponential function on the frontend?
    // function soulmateRedeemMint(uint256 points, uint256 quantity) external {
    //     uint256 pointsRequired = points * quantity;
    //     if (whitelistPoints[msg.sender] < pointsRequired) {
    //        revert InsufficientSoulmatePoints();
    //     }
    //     // lainOE.mint(quantity);

    //     // _safeMint(msg.sender, );
    // }

    /// @notice Composite one token with another. This evolves the token to the next level by burning one token of the same level. Retains the index number of the token
    /// @param tokenId The token ID of the NFT to evolve
    /// @param burnId The token ID of the NFT to be burned
    function _composite(uint256 tokenId, uint256 burnId) internal {
        // Check if both tokens are same level
        if (lainLevel[tokenId] != lainLevel[burnId]) {
            revert LevelDifference();
        }
        // Check if token level is under 8
        // separate if function because || cannot be used for uint8
        if (lainLevel[tokenId] > 8) {
            revert CompositeLevelCap();
        }
        ...
        // Do we need a composite event like Checks? Yes
        emit Composite(tokenId, burnId, lainLevel[tokenId]);
    }

    // Question: will there be two inputs or just a selection of i.e. 64 Lains v3. then we keep the lower indexes?
    function composite(
        uint256[] calldata tokenIds,
        uint256[] calldata burnIds
    ) external {
        // Check that array lengths are equal
        if (tokenIds.length != burnIds.length) {
            revert CompositeBurnNumbersDifferent();
        }
    ...
    function deification(uint256[] calldata tokenIds) external {
        if (tokenIds.length != 8) {
            revert InvalidDeificationCount();
        }

        // single out the god id
        uint256 godId = tokenIds[0];
        if (lainLevel[godId] != 8) {
            revert InvalidDeificationLevel();
        }

        // burning the rest
        for (uint256 i = 1; i < tokenIds.length; ) {
            if (lainLevel[tokenIds[i]] != 8) {
                revert InvalidDeificationLevel();
            }
            _burn(tokenIds[i]);
            unchecked {
        ...
        emit Deification(godId, tokenIds[1:]);
    }

    // should use mapping or struct for this?
    mapping(uint256 => uint256) tradingCardCounter;

    function redeemTradingCard(uint256[] calldata tokenIds) external {
        uint256 redeemLevel = lainLevel[tokenIds[0]];

        for(uint256 i; i < tokenIds.length;){
            if(lainLevel[tokenIds[i]] != redeemLevel) revert LevelDifference();
            if(tradingCardCounter[redeemLevel] >= 100) revert CardNoStock();
            _burn(tokenIds[i]);
            unchecked { ++tradingCardCounter[redeemLevel]; }
            unchecked { ++i; }
        }
        emit TradingCardRedemption(tokenIds);
    }
    ...
    // function tradingCardExchange(uint256[] calldata tokenIds) external {
    //     uint version = lainLevel[tokenIds[0]];
    //     for (uint256 i=0; i<tokenIds.length;){
    //         if(lainLevel[tokenIds[i]] != version) revert LevelDifference();
    //         _burn(tokenIds[i]);
    //         unchecked { ++i; }
    //     }
    // }
    ...
    // Operator Filterer Override - question: why need ierc?
    ...
    /*
        * minting *
        mint from mutedLain burn - WORKS
        soulmates free redeem function (point system) - BLOCKER
        - don't need to override mint; no public mint

        * metadata *
        we need a struct to save the data of the level - WORKS

        * burning *
        composite function - WORKS
        redeem trading card function
        update tokenURI function (for "level up")
        
        * misc *
        how to store claim history??
    */
}
