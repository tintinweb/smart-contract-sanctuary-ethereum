...
contract Leakz is ERC721A, ERC721ABurnable, ERC721AQueryable, Ownable, ReentrancyGuard {
    ...
    // There is no particular usecase for these at the moment. But I
    // bet we can build some interesting mechanic around it in the future.

    event Leaking(uint256 id, address owner);
    event Revealing(uint256 id, address owner);  
    event Patching(uint256 id, address owner);  
    ...
    function leak(string calldata message) external payable leakable() {
      ...
      emit Leaking(currentIndex, _msgSender());
    ...
    function reveal(uint256 tokenId) external returns(string memory) {
        if (_msgSender() != ownerOf(tokenId)) revert NotYourLeak();
        emit Revealing(tokenId, _msgSender());
    ...
    function burn(uint256 tokenId) public override {
        if (_msgSender() !=  ownerOf(tokenId)) revert NotYourLeak();
        emit Patching(tokenId, _msgSender());
        _burn(tokenId);
    }
