    ...
    function _bondTokenURI(uint256 tokenId) private view returns (string memory) {
        ...
        string memory dataJSON = string.concat(
            '{'
                ...
                '"atributes": [',
                _setAtribute('Principal', string.concat((bond.principal/1 ether).toString(), ' USD')),',',
                _setAtribute('Collateral', string.concat((calculateCollateralBondAmount(bond.principal)/1 ether).toString(), ' Ethix')),',',
                _setAtribute('APY', string.concat((bond.interest*366 days/1 ether).toString(), '%')),',',
                _setAtribute('Maturity', string.concat((bond.maturity/30 days).toString(), ' Months')),',',
