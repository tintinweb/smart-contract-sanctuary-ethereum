...
contract UniswapCrossFlash {
    ...
    function startSquareArbitrage(address _fromToken,address _toToken, uint256 _amount) external {
        IERC20(_fromToken).safeApprove(address(UNISWAP_ROUTER), MAX_INT);
        IERC20(_toToken).safeApprove(address(UNISWAP_ROUTER), MAX_INT);
       ...
       // IERC20(LINK).safeApprove(address(SUSHI_ROUTER), MAX_INT);
        ...
        uint256 amount0Out = _fromToken == token0 ? _amount : 0;
        uint256 amount1Out = _fromToken == token1 ? _amount : 0;

        // Passing data as bytes so that the 'swap' function knows it is a flashloan
        bytes memory data = abi.encode(_fromToken, _amount, msg.sender);
    ...
    function uniswapV2Call(
        ...
        bool reuslt = checkSquarePossibleTrade(
            token0,
            token1,
            loanAmount,
            UNISWAP_FACTORY,
