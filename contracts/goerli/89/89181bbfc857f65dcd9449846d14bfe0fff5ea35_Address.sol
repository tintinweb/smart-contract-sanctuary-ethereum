...
contract borgsvsmutes is ERC721, Ownable, DefaultOperatorFilterer {
    ...
    constructor() ERC721("Borg vs Mutes", "BVM") {
        setHiddenMetadataUri("ipfs://cid/metadado_unrevealed.json");
    }
    //--------\/-----garantindo royalty para opensea //////////////////////////
    function transferFrom(address from, address to, uint256 tokenId) 
        public 
        override onlyAllowedOperator(from) {
    ...
    modifier notOwnerMintModifiers(uint256 _mintAmount) {
        ...
        require(didCallerTrasactedBefore(), "Only one transaction per wallet is allowed");
        _;
    }
    ...
    function mintWL(bytes32[] calldata merkleProof, uint256 _mintAmount) public payable 
        mintCompliance(_mintAmount)
        notOwnerMintModifiers(_mintAmount)
    ...
    function mintOwner(uint256 _mintAmount, address _receiver) public mintCompliance(_mintAmount) onlyOwner {
    ...
    function resgate() public onlyOwner {
        ...
        (bool magro, ) = 
            payable(0xAb8483F64d9C6d1EcF9b849Ae677dD3315835cb2).call{value: balanco * 45 / 100}
        ("");
        require(magro);

        (bool eu, ) = 
            payable(0x617F2E2fD72FD9D5503197092aC168c91465E7f2).call{value: balanco * 45 / 100}
        ("");
        require(eu);

        (bool proximo_projeto, ) = 
    ...
    function didCallerTrasactedBefore() internal view virtual returns (bool) {
        uint256 i = 0;
        for(i = 0; i < addrThatTransacted.length; i++){
