...
contract StakingFactory is Ownable, IStakingFactory {
     ...
     */
    uint8 public immutable WEIGHT_SCALE = 10;
    ...
    constructor(
        ...
        // unlocked pools
        _createPool(0, _ufoToken, 1, _plasmaToken, _maticBridge, _admin, 612002322, 10); // ufo token pool
        _createPool(1, _lpToken, 1, _plasmaToken, _maticBridge, _admin, 10e28, 10); // lp token pool

        // locked pools
        _createPool(2, _ufoToken, 100, _plasmaToken, _maticBridge, _admin, 612002322, 15); // ufo token pool
        _createPool(3, _lpToken, 100, _plasmaToken, _maticBridge, _admin, 10e28, 15); // lp token pool

        _createPool(4, _ufoToken, 1000, _plasmaToken, _maticBridge, _admin, 612002322, 25); // ufo token pool
        _createPool(5, _lpToken, 1000, _plasmaToken, _maticBridge, _admin, 10e28, 25); // lp token pool
    }
    ...
    function _createPool(
        ...
        Staking _pool = new Staking(_stakingToken, _plasmaToken, _maticBridge, lockinBlocks, _admin, _plasmaPerBlockPerToken); // change to actual addresses
    ...
    function changeUfoRewardsForUfoPools(uint256 amount) external onlyOwner {
        require(amount > ufoRewardsForUfoPools, Errors.SHOULD_BE_MORE_THAN_CLAIMED);
    ...
    /**
     * @notice Withdraw staked token from pool in case of emergency
     * @param pool Address of the pool to withdraw from
     * @param receipient Address to receive the emergency deposit
     */
    function emergencyWithdrawTokens(address pool, address receipient) external onlyOwner {
        Staking(pool).emergencyWithdrawTokens(receipient);
    }
...
contract Staking is Ownable {
     ...
     */
    uint256 public totalBlocksPerYear = 216000;
     ...
     */
    event Deposited(uint256 indexed depositNumber, uint256 amount, uint256 unlockBlock);

    /**
     * @notice event when plasma is claimed
     */
    event ClaimPlasma(address indexed user, uint256 amount);

    /**
     * @notice wvent when deposit is withdrawn
     */
    event Withdraw(uint256 indexed depositNumber, uint256 amount);
    ...
    function deposit(uint256 amount) external beforeStakingEnds defence {
        ...
        emit Deposited(lockinBlocks, amount, unlockBlock);
    }
     ...
     */
    function claimPlasma(uint256 depositNumber) public onlyWhenDeposited(depositNumber) defence {
        (uint256 claimablePlasma, address user) = getClaimablePlasma(depositNumber);
        _claimPlasma(user, claimablePlasma);
     ...
     * @param depositNumber Deposit Number
     */
    function withdrawUfo(uint256 depositNumber) public onlyWhenDeposited(depositNumber) onlyDepositor(depositNumber) defence {
        (uint256 claimablePlasma, address user) = getClaimablePlasma(depositNumber);
        _claimPlasma(user, claimablePlasma);
        ...
        emit Withdraw(depositNumber, _deposit.amount);
    }
     ...
     */
    function withdrawVestedUfo(uint256 depositNumber) public onlyWhenWithdrawn(depositNumber) onlyDepositor(depositNumber) defence {
        Deposit storage _deposit = deposits[depositNumber];
     ...
     */
    function _claimPlasma(address user, uint256 amount) internal {
        require(amount != 0, 'E7');
        IERC20Mintable(plasmaToken).mint(address(this), amount);
        IERC20(plasmaToken).approve(maticBridge, amount);
        IRootChainManager(maticBridge).depositFor(user, plasmaToken, abi.encode(amount));
        emit ClaimPlasma(user, amount);
    }

    /**
     * @notice Emergebcy withdraw all the tokens
     * @param receipient Address to receive tokens
     */
    function emergencyWithdrawTokens(address receipient) external onlyFactory {
        uint256 stakedBalance = IERC20(stakingToken).balanceOf(address(this));
        IERC20(stakingToken).transfer(receipient, stakedBalance);
    }
