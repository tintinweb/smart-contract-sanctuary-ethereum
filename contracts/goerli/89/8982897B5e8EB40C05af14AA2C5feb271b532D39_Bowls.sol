...
contract Bowls is ERC721A, Ownable, Pausable, ReentrancyGuard {
    ...
    *****************************************/
    ...
    modifier whenNotContract() {
        require( msg.sender == tx.origin, "Bowls: Transactions from smart contracts not allowed" );
        _;
    }

    modifier whenWithdrawAddressSet() {
        require( address(withdrawAddress) != address(0), "Bowls: withdrawAddress not set" );
        _;
    }

    modifier whenNotSoldOut() {
        require( totalSupply() < collectionSize, "Bowls: Sold out" );
        _;
    }

    modifier whenBurningPaused() {
        require( burningPaused, "Bowls: Burning not paused" );
        _;
    }

    modifier whenBurningNotPaused() {
        require( !burningPaused, "Bowls: Burning is paused" );
        _;
    }
    ...
    *****************************************/
    ...
    uint64 public freeMintsRemaining;
    ...
    *****************************************/
    ...
    constructor() ERC721A( "Bowls", "BWLS" ) {
        collectionSize = 6666;
        freeMintsRemaining = 666;
        ...
        metadataBase = "https://metadata.bowlsnft.io/pre/";
    ...
    *****************************************/
    ...
    function setFreeMintsRemaining( uint64 newAmount_ ) public onlyOwner {
        uint256 remainingTokens = collectionSize - totalSupply();
        require( newAmount_ <= remainingTokens, "Bowls: New amount exceeds remaining tokens" );
        freeMintsRemaining = newAmount_;
    }
    ...
    function setCollectionSize( uint256 newSize_ ) external onlyOwner {
        require( newSize_ >= totalSupply(), "Bowls: New collection size can't be lower than current supply" );
    ...
    function setMetadataBase( string memory newMetadataBase_ ) external onlyOwner {
        require( !metadataLocked, "Bowls: Metadata locked" );
        require( bytes(newMetadataBase_).length != 0, "Bowls: Metadata base can't be empty" );
        metadataBase = newMetadataBase_;
    }

    function setMerkleRoot( bytes32 newRoot_ ) external onlyOwner {
        require( newRoot_.length != 0, "Bowls: Merkle root can't be empty" );
        merkleRoot = newRoot_;
    }

    function revealMetadata( string memory newMetadataBase_ ) external onlyOwner {
        require( !isRevealed, "Bowls: Already revealed" );
        require( bytes(newMetadataBase_).length != 0, "Bowls: Metadata base can't be empty" );
        metadataBase = newMetadataBase_;
        isRevealed = true;
    ...
    function lockMetadata() external onlyOwner {
        require( !metadataLocked, "Bowls: Already locked" );
        require( isRevealed, "Bowls: Can't lock before reveal" );
        require( totalSupply() == collectionSize, "Bowls: Can't lock before sellout" );
        metadataLocked = true;
        emit MetadataLocked();
    }
    ...
    *****************************************/
    ...
    function amountExceedsReserve( uint64 numberOfTokens_ ) public view returns (bool) {
        return numberOfTokens_ > totalReserved;
    }
    ...
    *****************************************/
    ...
    function canFreeMint( address account_, bytes32[] calldata merkleProof_ ) public view returns (bool) {
        bytes32 leaf = keccak256( abi.encodePacked(account_) );
        return MerkleProof.verify(merkleProof_, merkleRoot, leaf);
    }

    function freeMint( uint64 numberOfTokens_, bytes32[] calldata merkleProof_ ) nonReentrant external whenNotContract whenNotPaused whenNotSoldOut {
        require( canFreeMint(msg.sender, merkleProof_), "Bowls: Invalid proof" );

        require( !amountExceedsFreeMintLimit(msg.sender, numberOfTokens_), "Bowls: Amount exceeds free mint limit" );
        require( !amountExceedsSupply(numberOfTokens_), "Bowls: Not enough tokens left" );
        require( freeMintsRemaining - numberOfTokens_ >= 0, "Bowls: Not enough free tokens left" );

        _safeMint( msg.sender, numberOfTokens_ );

        freeMintsRemaining -= numberOfTokens_;

        uint64 walletFreeMints = numberMintedFree( msg.sender );
        walletFreeMints += numberOfTokens_;
        _setAux( msg.sender, walletFreeMints );
    }

    function mint( uint64 numberOfTokens_ ) nonReentrant external whenNotContract whenNotPaused whenNotSoldOut payable {
        require( publicMintEnabled, "Bowls: Public mint not enabled" );

        require( !amountExceedsTransactionLimit(numberOfTokens_), "Bowls: Amount exceeds limit per transaction" );
        require( !amountExceedsSupply(numberOfTokens_), "Bowls: Not enough tokens left" );
        require( msg.value == mintPrice * numberOfTokens_, "Bowls: Payment amount is incorrect" );
    ...
    *****************************************/
    ...
    function mintTo( address to_, uint64 numberOfTokens_ ) nonReentrant external onlyOwner whenNotSoldOut {
        ...
        require( totalReserved > 0, "Bowls: No tokens left in reserve" );
        require( !amountExceedsReserve(numberOfTokens_), "Bowls: Not enough reserve tokens left" );
        require( ( totalSupply() + numberOfTokens_ ) <= collectionSize, "Bowls: Not enough tokens left" );
        require( numberOfTokens_ <= totalReserved, "Bowls: Exceeds reserved amount" );
    ...
    *****************************************/
    ...
    function setWithdrawAddress( address newAddress_ ) external onlyOwner {
        require( address(newAddress_) != address(0), "Bowls: Withdraw address can't be null" );
    ...
    function withdraw() external whenWithdrawAddressSet onlyOwner {
        ...
        require( balance > 0, "Bowls: Insufficient balance" );
