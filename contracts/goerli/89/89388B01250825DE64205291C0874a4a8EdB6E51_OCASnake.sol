...
contract OCASnake is Ownable, ERC721A, ERC2981, ReentrancyGuard, OCASnakeArt {
    ...
    function snakeNameSave(uint256 tokenId) private returns (bytes32) {
        ...
        if (isTaken[name]) return snakeNameSave(tokenId + block.timestamp);
        else {
            //     register(name);
            isTaken[name] = true;
            return name;
    ...
    function returnSnakeByte32AsString(uint256 tokenId)
        private
        view
        returns (string memory)
    {
        string memory nameString = string(
            abi.encodePacked(tokenInfoList[tokenId].name)
        );
        return nameString;
    }
