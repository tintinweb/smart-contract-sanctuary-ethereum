...
import {OperatorFilterer721, DefaultOperatorFilterer721} from "./DefaultOperatorFilterer721.sol";
...
// Renaissance minting contract
// Currently supports whitelist minting, public minting and free mints
// Supports stating metadata
contract Renaissance is ERC721A, Ownable, ReentrancyGuard, ERC2981, OperatorFilterer721 {

    using ECDSA for bytes32;

    // Maximum supply of tokens, hardcoded value
    ...
    // Config
    uint256 FREEMINT_SUPPLY = 100;
    uint256 ADVISOR_SUPPLY = 300;
    uint256 TEAM_SUPPLY = 200;
    uint256 ADVISOR_MAX_MINT = 20;
    uint256 TEAM_MINTED = 0;
    uint256 WL_SUPPLY = 3377;
    uint256 MAX_MINT_WL = 3;
    uint256 MAX_MINT_PUBLIC = 20;

    uint256 whitelistMintPrice = 0.0004 ether;
    uint256 publicMintPrice = 0.0005 ether;

    string private _baseTokenURI = "https://red-keen-lamprey-650.mypinata.cloud/ipfs/QmaF67pvfzF3TxAmao4u6RzXgEQagfRSscsLQap6GXzUVK/";
    ...
    // Constructor
    constructor () ERC721A("0xAI Genesis", "0XAI") OperatorFilterer721(SUBSCRIPTION, false) {
    ...
    function teamMint(uint256 amount) external onlyOwner {
        if (_totalMinted() + amount > MAX_SUPPLY) revert MaxMintExceeded();
        require(totalSupply() + amount <= MAX_SUPPLY, "too many already minted before dev mint");
        require(TEAM_MINTED + amount <= TEAM_SUPPLY, "mint exceeds maximum possible team supply");

        TEAM_MINTED = TEAM_MINTED+amount;
        _mint(msg.sender, amount);
  }

    function mintAdvisor(uint256 amount) external {
        require(advisorMint[msg.sender] == 1, "Caller not advisor");
        require(advisorMinted[msg.sender] + amount <= ADVISOR_MAX_MINT, "Advisor already minted allocation");
        require(_totalMinted() + amount <= ADVISOR_SUPPLY, "Advisor supply exceeds maximum");
        if (tx.origin != msg.sender) revert NoContractMints();
        if (_totalMinted() + amount > MAX_SUPPLY) revert MaxMintExceeded();

        if (advisorMinted[msg.sender] + amount == ADVISOR_MAX_MINT) {
            advisorMint[msg.sender] = 0;
        }

        advisorMinted[msg.sender] =  advisorMinted[msg.sender] + amount;
        _mint(msg.sender, amount);
    }
    ...
    function mintWhitelist (bytes calldata signature, uint256 amount) external payable {
        ...
        if (tx.origin != msg.sender) revert NoContractMints();
        if (_totalMinted() + amount > MAX_SUPPLY) revert MaxMintExceeded();
        require(tokensMinted[msg.sender]+amount <= MAX_MINT_WL);
        ...
        _refundOverPayment(whitelistMintPrice*amount);
        _mint(msg.sender, amount);
    ...
    function mintPublic (uint256 amount) external payable {
        ...
        if (tx.origin != msg.sender) revert NoContractMints();
        if (_totalMinted() + amount > MAX_SUPPLY) revert MaxMintExceeded();
        require(tokensMinted[msg.sender]+amount <= MAX_MINT_PUBLIC);

        tokensMinted[msg.sender] += amount;
        _refundOverPayment(publicMintPrice*amount);
        _mint(msg.sender, amount);
    }

    // Mint free tokens if caller is eligible
    function mintFree () external {
        require(freeMintOpen, "Free minting not open");
        if (tx.origin != msg.sender) revert NoContractMints();
        if (_totalMinted() + 1 > MAX_SUPPLY) revert MaxMintExceeded();

        require(freeMints[msg.sender] > 0, "Caller is not eligible for a free mint");
    ...
    // Refund callers if they send too much ETH
