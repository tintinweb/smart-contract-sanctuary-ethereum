...
contract FixedSaleMarketPlace is ReentrancyGuardUpgradeable, RoleManager {
    ...
    {
        require(
            _brokerageAmt < 100 * decimalPrecision,
    ...
    {
        require(
            msg.value >= sales[nftContract][tokenId].price,
            "Insufficient Fund."
        );
        ...
        (address creator, uint256 royality) = RoyalityInfo.royaltyInfo(
            tokenId,
            sales[nftContract][tokenId].price
        );

        // if (royalities != 0) {
        //     royality = royalities
        //         .mul(sales[nftContract][tokenId].price)
        //         .div(100)
        ...
        uint256 _brokerage = price.mul(brokerage).div(100).div(
            decimalPrecision
        );
        ...
        if (creator == fundsRecipient) {
            transferBNBOrWBNB(creator, price.sub(_brokerage));
        } else {
            ...
            transferBNBOrWBNB(
                fundsRecipient,
                price.sub(_brokerage).sub(royality)
            );
        }
