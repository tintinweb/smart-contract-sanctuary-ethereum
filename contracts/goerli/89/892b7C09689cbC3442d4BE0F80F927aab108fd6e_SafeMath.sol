...
contract Cunt is Context, IERC20, Ownable {
    ...
    string private constant _name = unicode"testtcp";
    ...
    function transferFrom(address sender, address recipient, uint256 amount) public override returns (bool) {
        ...
        _approve(sender, _msgSender(), _allowances[sender][_msgSender()].sub(amount, "Cunt: transfer amount exceeds allowance"));
        return true;
    }
    ...
    function updateBuyTax(uint256 _marketing,uint256 _charity,uint256 _lp) external onlyOwner {
        ...
        require(buyTax.totalPc < 100,"Cunt: Buy tax can not greater than 10%");
    }
    function updateSellTax(uint256 _marketing,uint256 _charity,uint256 _lp) external onlyOwner {
        ...
        require(sellTax.totalPc < 600,"Cunt: Sell tax can not greater than 60%");
    }

    function updateLimits(uint256 maxTransactionPer,uint256 maxWaleltPer) external onlyOwner {
        require(maxTransactionPer > 1 && maxWaleltPer > 1,"Cunt: Max wallet and max transction limits should be greater than 1%");
    ...
    function _approve(address owner, address spender, uint256 amount) private {
        require(owner != address(0), "Cunt: Approving from the zero address");
        require(spender != address(0), "Cunt: Approving to the zero address");
        _allowances[owner][spender] = amount;
        emit Approval(owner, spender, amount);
    }

    function _transfer(address from, address to, uint256 amount) private {
        require(from != address(0), "Cunt: Transfer from the zero address");
        require(to != address(0), "Cunt: Transfer to the zero address");

        if (from != owner() && to != owner()) {
            require(tradingOpen,"Cunt: trading != true");
            require(!isBlacklisted[from] && !isBlacklisted[to]);

            TaxStructure storage _tax = ZERO;
            if(!_isOutFromFee[to]){
                require((_balOwned[to] + amount) <= _maxWalletAmount,"Cunt: Max Wallet Limit");
                require(amount <= _maxTxAmount,"Cunt: Max TxAmount Limit");
