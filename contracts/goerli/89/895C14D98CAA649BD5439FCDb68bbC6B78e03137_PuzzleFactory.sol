...
contract PuzzleFactory is Ownable {
    struct Level {
        address puzzleVerifierContract;
        string ipfsCid;
    }
    ...
    mapping(uint256 => Level) internal levels;
    mapping(address => uint256) internal playerLevel;
    ...
    function addNewLevel(address puzzleVerifierContract, string memory ipfsCid)
        public
        onlyOwner
    {
        _levelCounter++;

        levels[_levelCounter] = Level({
            puzzleVerifierContract: puzzleVerifierContract,
            ipfsCid: ipfsCid
        });

        emit NewLevelCreated(_levelCounter);
    }

    function solvePuzzle(bytes memory proof, uint256[] memory pubSignals)
        public
    {
        uint256 currentLevel = getLevel(msg.sender);

        address puzzle = levels[currentLevel].puzzleVerifierContract;
        if (puzzle == address(0)) revert PuzzleFacotry__InvalidLevel();

        bool isSolved = IPuzzleVerifier(puzzle).verifyProof(proof, pubSignals);
        if (!isSolved) revert PuzzleFactory__WrongSolution();

        posp.mintAward(msg.sender, levels[currentLevel].ipfsCid);
    ...
    function getLevel(address player) public view returns (uint256) {
        return playerLevel[player] + 1;
    }

    function getLevelDetails(uint256 level) public view returns (Level memory) {
        return levels[level];
    }
}
