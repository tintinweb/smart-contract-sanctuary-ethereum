...
contract BondingCalculator is IBondingCalculator {
    ...
    function getKValue(address _pair) public view returns (uint256) {
        ...
        uint256 k_ = reserve0.mul(reserve1).div(10**decimals);
        return k_;
    }

    function getTotalValue(address _pair) public view returns (uint256) {
        uint256 value_ = getKValue(_pair).sqrt().mul(2);
        return value_;
    }
    ...
    function valuation(address _pair, uint256 _amount) external view override returns (uint256) {
        ...
        uint256 value_ = totalValue.mul(PRBMathUD60x18.div(_amount, totalSupply)).div(1e18);
        return value_;
    }
    ...
    function markdown(address _pair) external view override returns (uint256) {
        ...
        uint256 reserve;
        if (IUniswapV2Pair(_pair).token0() == address(mainToken)) {
            reserve = reserve1;
        } else {
            require(IUniswapV2Pair(_pair).token1() == address(mainToken), "Invalid pair");
            reserve = reserve0;
        }
        
        return reserve.mul(2*(10**IERC20Metadata(address(mainToken)).decimals())).div(getTotalValue(_pair)); 
    }
}
