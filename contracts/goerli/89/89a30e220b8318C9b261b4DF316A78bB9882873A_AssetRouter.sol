...
contract AssetRouter is IAssetRouter, AccessControl, ReentrancyGuard {
    ...
    event CrossChainSwapInitiated(SwapMessage _message);
    event CrossChainSwapPerformed(SwapMessage _message);
    event CrossChainLiquidityInitiated(LiquidityMessage _message);
    ...
    function swap(SwapParams memory swapParams) external payable nonReentrant returns (SwapMessage memory) {
        ...
        (uint256 actualAmount, uint256 fee) = feeHandler.getFees(
        ...
        require(actualAmount >= swapParams.minAmount, "Slippage too high");

        require(cp.bandwidth >= swapParams.amount, "Src bandwidth too low");
        require(cp.kbp + cp.vouchers >= actualAmount, "Dst bandwidth too low");
        require(cp.bandwidth >= actualAmount, "Dst bandwidth too low");
        cp.bandwidth -= actualAmount;
        cp.actualBandwidth -= actualAmount;

        cp.actualKbp += actualAmount;
        ...
        VoucherObject memory vouchers = _sendVouchers(
            swapParams.srcPoolId,
        ...
        SwapMessage memory swapMessage = SwapMessage({
            chainData: ChainData({
                srcPoolId: swapParams.srcPoolId,
                srcChainId: chainId,
                dstPoolId: swapParams.dstPoolId,
                dstChainId: swapParams.dstChainId
            }),
            receiver: swapParams.to,
            vouchers: vouchers,
            amount: actualAmount,
            fee: fee,
            kbpRemove: actualAmount,
            id: keccak256(
                abi.encode(bridge, bridge.nextNonce(swapParams.dstChainId), block.timestamp, MESSAGE_TYPE.SWAP)
            ),
            payload: swapParams.payload
        });

        bridge.dispatchMessage{ value: msg.value }(
            swapMessage.chainData.dstChainId,
            MESSAGE_TYPE.SWAP,
            swapParams.refundAddress,
            abi.encode(MESSAGE_TYPE.SWAP, swapMessage)
        );
        emit CrossChainSwapInitiated(swapMessage);
    ...
    function swapRemote(
        ...
        PoolObject storage pool = poolLookup[receivedMessage.chainData.dstPoolId];
        cp.kbp -= receivedMessage.kbpRemove;
        cp.actualKbp -= receivedMessage.kbpRemove;
    ...
    ) public payable nonReentrant returns (LiquidityMessage memory liquidityMessage) {
        liquidityMessage = LiquidityMessage({
            chainData: ChainData({
            ...
            vouchers: _sendVouchers(_srcPoolId, _dstChainId, _dstPoolId, false),
            id: keccak256(
                abi.encode(bridge, bridge.nextNonce(_dstChainId), block.timestamp, MESSAGE_TYPE.ADD_LIQUIDITY)
            ),
            payload: ""
        });
        bridge.dispatchMessage{ value: msg.value }(
            ...
            _refundAddress,
            abi.encode(MESSAGE_TYPE.ADD_LIQUIDITY, liquidityMessage)
        );
        emit CrossChainLiquidityInitiated(liquidityMessage);
    }
    ...
    function _sendVouchers(
        ...
        uint16 _dstPoolId,
        bool _swap
    ) internal returns (VoucherObject memory voucherObject) {
        ...
        voucherObject.optimalDstBandwidth = (pool.totalLiquidity * cp.weight) / pool.totalWeight;
        voucherObject.vouchers = cp.vouchers;
        cp.vouchers = 0;
        voucherObject.swap = _swap;
    }
    ...
    function receiveVouchers(
        ...
        uint16 _srcChainId,
        uint16 _srcPoolId,
        VoucherObject memory _voucherObject
        ...
        cp.bandwidth += _voucherObject.vouchers;
        if (!_voucherObject.swap) {
            cp.actualBandwidth += _voucherObject.vouchers;
        }
        if (cp.optimalDstBandwidth != _voucherObject.optimalDstBandwidth) {
            cp.optimalDstBandwidth = _voucherObject.optimalDstBandwidth;
        }
        emit VouchersReceived(_srcChainId, _srcPoolId, _voucherObject.vouchers, _voucherObject.optimalDstBandwidth);
    }
