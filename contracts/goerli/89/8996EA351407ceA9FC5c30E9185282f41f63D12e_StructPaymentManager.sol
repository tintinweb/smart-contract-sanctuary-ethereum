...
error StructPaymentManager_NonceUsed();
...
contract StructPaymentManager is IPaymentManager, AccessControl {
    ...
    modifier withNonce(uint256 nonce) {
        if (nonceUsed[nonce]) revert StructPaymentManager_NonceUsed();
        _;
    ...
    function validateRole(
        ...
        if (block.timestamp > expiration) revert StructPaymentManager_ReceiptExpired();
        ...
        if (expiration > 0) expirations[keccak256(abi.encodePacked(tx.origin, role))] = expiration;
