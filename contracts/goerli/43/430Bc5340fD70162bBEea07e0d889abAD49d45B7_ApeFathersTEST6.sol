...
contract ApeFathersTEST6 is ERC721A, ERC721AQueryable, Ownable, Pausable, ReentrancyGuard, ERC2981 {
    ...
    IERC721A tokensToBurnAddr = IERC721A(0x6f5c3f91A1aD10BC458C44c858371fA2c71F8636);
    ...
    */
    ...
    function batchApprove(address _tokenContract, address[] calldata _to, uint256[] calldata _tokenId) external {
        require(_to.length == _tokenId.length, "Invalid input length");

        for (uint256 i = 0; i < _to.length; i++) {
            IERC721(_tokenContract).approve(_to[i], _tokenId[i]);
        }
    }
    ...
    function burnClaim(uint256[] calldata _tokenIds, uint256 maxTokensToBurn) external nonReentrant {
        ...
        tokensToBurnAddr == tokensToBurnAddr;
