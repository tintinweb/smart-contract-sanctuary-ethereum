...
contract Kitchen is Ownable {
    ...
    uint256 public constant YIELD_TPS = 16666667; // tacos geted per second per unit of yield
    uint256 public constant CLAIM_TACO_TAX_PERCENTAGE = 20;
    ...
    constructor(Chef _chef, Taco _taco, address _vaultAddress) {
        chef = _chef;
        taco = _taco;
    ...
    function getTacosAccruedForMany(uint256[] calldata _tokenIds) external view returns (uint256[] memory) {
        uint256[] memory tacoAmounts = new uint256[](_tokenIds.length);
        for (uint256 i = 0; i < _tokenIds.length; i++) {
            tacoAmounts[i] = _getTacosAccruedFor(_tokenIds[i], false);
        }
        return tacoAmounts;
    }

    function _getTacosAccruedFor(uint256 _tokenId, bool checkOwnership) internal view returns (uint256) {
        ...
        return (block.timestamp - stake.startTimestamp) * chef.tokenYield(_tokenId) * YIELD_TPS;
    }

    /* Mutators */

    function stakeMany(uint256[] calldata _tokenIds) external {
        require(chef.gameStarted(), "The game has not started");

        for (uint256 i = 0; i < _tokenIds.length; i++) {
            uint256 tokenId = _tokenIds[i];
            require(chef.ownerOf(tokenId) == _msgSender(), "You don't own this token");
            require(chef.tokenRevealed(tokenId), "Token not yet revealed");

            chef.transferFrom(_msgSender(), address(this), tokenId);
            _addChefToKitchen(tokenId, _msgSender());
        }
    }

    function claimTacosAndMaybeUnstake(uint256[] calldata _tokenIds, bool unstake) external {
        uint256 totalClaimed = 0;
        uint256 totalTaxed = 0;

        for (uint256 i = 0; i < _tokenIds.length; i++) {
            uint256 tokenId = _tokenIds[i];
            uint256 tacos = _getTacosAccruedFor(tokenId, true); // also checks that msg.sender owns this token
            uint256 taxAmount = (tacos * CLAIM_TACO_TAX_PERCENTAGE + 99) / 100; // +99 to round the division up

            totalClaimed += tacos - taxAmount;
            totalTaxed += taxAmount;
            stakes[tokenId].startTimestamp = block.timestamp;

            if (unstake) {
                _moveChefToCooldown(tokenId);
            }
        }

        taco.mint(_msgSender(), totalClaimed);
        taco.mint(vaultAddress, totalTaxed);
    }

    function withdrawChefs(uint256[] calldata _tokenIds) external {
        for (uint256 i = 0; i < _tokenIds.length; i++) {
            uint256 tokenId = _tokenIds[i];
            ...
            chef.transferFrom(address(this), _msgSender(), tokenId);
            _removeChefFromCooldown(tokenId);
        }
    }

    function _addChefToKitchen(uint256 _tokenId, address _owner) internal {
    ...
    function _moveChefToCooldown(uint256 _tokenId) internal {
    ...
    function _removeChefFromCooldown(uint256 _tokenId) internal {
    ...
    function batchedStakesOfOwner(
        ...
        for (uint256 i = 0; i < outputSize; i++) {
            ...
            outputs[i] = OwnedStakeInfo({
                tokenId: tokenId,
                level: chef.revealedTokenLevel(tokenId),
                accrual: _getTacosAccruedFor(tokenId, false)
            });
        }
