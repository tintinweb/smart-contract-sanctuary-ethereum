...
contract ocPunks is Ownable, ERC721A {
    enum PunkAttributeType {SEX, HAIR, EYES, BEARD, EARS, LIPS, MOUTH, FACE, EMOTION, NECK, NOSE, CHEEKS, TEETH}
    ...
    bytes private constant tokenDescription = "Some Goerli OC CryptoPunks";
    ...
    function bulkTransfer(uint256[] memory tokenIds, address _to) external onlyOwner {
    ...
    function constructTokenURI(uint16 tokenId) private view returns (string memory) {
        bytes memory title = abi.encodePacked("Punk #", tokenId.toString());
                    ...
                    Base64.encode(
                        bytes(
                            abi.encodePacked(
                                ...
                                '"image_data":" ',punkDataContract.punkImageSvg(tokenId), '",'
                                '"attributes": ',
    ...
    address constant phunksTreasury = 0x61f874551c69f0E40c9f55219107B408C989aDEc;

    function withdraw() external {
        require(address(this).balance > 0, "Nothing to withdraw");
        
        uint total = address(this).balance;
        uint quarter = (total * 25) / 100; 
        
        Address.sendValue(payable(claireAddress), quarter);
        Address.sendValue(payable(kenobiAddress), quarter);
        Address.sendValue(payable(chopperAddress), quarter);
        Address.sendValue(payable(phunksTreasury), total - (quarter * 3));
    }
