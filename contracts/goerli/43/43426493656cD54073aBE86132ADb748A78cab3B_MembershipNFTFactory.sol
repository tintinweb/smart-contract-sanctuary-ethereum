...
/// @notice  Factory contract that creates a ERC721
contract MembershipNFTFactory {
    ...
    /// @notice Error for if address is not Kalder Factory
    error NotKalderFactory();
    ...
    /// @notice                          Create `membershipNFT_`
    /// @param maxSupply_                Max supply of `membershipNFT_`
    /// @param pricePerToken_            Price to mint each `membershipNFT_`
    /// @param kalderFee_                Percent Fee kalder takes
    /// @param kalderWallet_             Address of where kalder fees get sent
    ...
    function createNFT(
        ...
        if (msg.sender != kalderFactory) revert NotKalderFactory();
...
contract MembershipNFT is ERC721, ERC721Enumerable {
    ...
    /// @notice Stores if address has already purchased
    ...
    /// @param kalderFee_        Percent fee kalder takes
    /// @param kalderWallet_     Address where kalder fees get sent to
    ...
    function withdraw() external {
        ...
        payable(msg.sender).transfer(balance * (100 - kalderFee) / 100 );
        payable(kalderWallet).transfer(balance * kalderFee / 100);
    }
    ...
    function mint() external payable {
        uint256 ts = totalSupply();
        if (ts + 1 > maxSupply) revert MaxSupplyExceeded();
        ...
        membershipTiers.renew(address(this), 1, ts);
    }
