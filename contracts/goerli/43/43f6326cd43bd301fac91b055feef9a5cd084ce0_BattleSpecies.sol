...
contract ERC721Profile is ERC721 {
    ...
    function reserveName(string memory newName, string memory oldName) internal {
        ...
        if (sha256(bytes(newName)) == sha256(bytes(oldName))) revert NameMustBeDifferent(); 
...
contract BattleSpecies is ERC721Profile, ERC721Enumerable, BattlespeciesWithdrawer, OperatorFilterer {
    ...
    function _baseMint(uint8 _numberOfTokens, bool _autoManifest, uint256 _mintPrice) internal {
        ...
        if (hyperwarpManager.battlerHasUtility(_totalSupply + _numberOfTokens, 1)) {
            if (balanceOf(_msgSender()) + _numberOfTokens > maxMintsPerWalletBeforeTranscendance) revert MaxTokenAllotment();
        }
        else {
            if (balanceOf(_msgSender()) + _numberOfTokens > maxMintsPerWalletAfterTranscendance) revert MaxTokenAllotment();
        }

        //price checks
        if (_autoManifest) {
            if ((_mintPrice * _numberOfTokens) + (autoManifestPrice * _numberOfTokens) > msg.value) revert InsuffientEthPaid();
        }
        else {
            if (_mintPrice * _numberOfTokens > msg.value) revert InsuffientEthPaid();
        }
        ...
        if (_totalSupply + _numberOfTokens > maxBattlers) revert MaxTokenAllotment();
        ...
        assert(totalSupply() == _totalSupply + _numberOfTokens);
    }

    function _remoteMint(uint8 _numberOfTokens, bool _autoManifest, uint256 _mintPrice) external {
        require(msg.sender == remoteMintManager, "!mint mgr");
        _baseMint(_numberOfTokens, _autoManifest, _mintPrice);
    }
    ...
    function mint(uint8 _numberOfTokens, bool _autoManifest) public payable {
        _baseMint(_numberOfTokens, _autoManifest, mintPrice);
    }
    ...
    function mintWhitelist(uint8 _numberOfTokens, bool _autoManifest, bytes32[] calldata _wlProof) public payable {
        ...
        _baseMint(_numberOfTokens, _autoManifest, mintWhitelistPrice);
    }
    ...
    function changeName(uint256 _tokenId, string memory _newName) public {
        if (ownerOf(_tokenId) != _msgSender()) revert InvalidOwner();
    ...
    function _beforeTokenTransfer(address from, address to, uint256 tokenId, uint256 batchSize) internal override(ERC721, ERC721Enumerable) {
        ...
        if (tokenId > uint(30) && hyperwarpManager.battlerHasUtility(tokenId, 2)) {
