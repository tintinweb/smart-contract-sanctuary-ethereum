...
import "./bWalletProxy.sol";
import "./bWalletPolygon.sol";

contract bWalletFactory is Initializable, OwnableUpgradeable {
    ...
    function createProxyContract(address _owner) public onlyOwner {
        bWalletPolygon token = new bWalletPolygon();
         address logic = address(token);

        bWalletProxy newProxy = new bWalletProxy(logic, abi.encodeWithSelector(bWalletPolygon.initialize.selector, _owner));
...
contract bWalletProxy is ERC1967Proxy {
