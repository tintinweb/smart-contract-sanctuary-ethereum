...
contract UniforgeDomains {
  ...
  /* Events */
  event DomainCreated(uint256 id, string name, address owner);
  event DomainUpdated(uint256 id, string name);
  ...
  function createDomain(string memory name) public payable {
    ...
    emit DomainCreated(domainId, name, msg.sender);
  }

  /* updates an existing domain */
  function updateDomain(uint256 domainId, string memory name) public {
    (bool domainExists, ) = getDomainByName(name);
    require(!domainExists, "Domain already exists");
    Domain storage domain = idToDomain[domainId];
    ...
    emit DomainUpdated(domain.id, name);
  }
