...
contract Exchange is ERC20 { //LP토큰 발행, 소각하려면 ERC20 상속 받아야 함
    ...
    constructor (address _token) ERC20("Gray Uniswap V2", "GUNI-V2") { //이더리움과 페어를 맺을 토큰 주소
        token = IERC20(_token);
    ...
    //     //transferFrom을 호출하는 주체는 내가 아니라 Exchange 컨트랙트임
    //     token.transferFrom(msg.sender, address(this), _tokenAmount); 
    //     //msg.sender(함수 호출하는 나)로 부터 address(this)(Exchange 컨트랙트 주소)로 토큰 전송
    // }
    ...
    function addLiquidity(uint256 _maxTokens) public payable {
        ...
        if(totalLiquidity > 0){ //(1)이미 유동성(풀)이 있는 상태에서 유동성(풀) 추가 
            ...
            uint256 tokenReserve = token.balanceOf(address(this));     //기존 풀에 존재하는 그레이 토큰 총량
            uint256 tokenAmount = msg.value * (tokenReserve/etherReserve); //내가 풀에 넣는 그레이 토큰양 = 내가 넣는 이더양 * (토큰/이더 풀의 비율)

            require(_maxTokens >= tokenAmount); //내가 실제 풀에 넣는 그레이 토큰양은 내가 입력한 그레이 토큰양보단 적어야 함

            token.transferFrom(msg.sender, address(this), tokenAmount); //컨트랙트가 내가 공급한 그레이 토큰을 가져감
            uint256 lpTokenMinted = totalLiquidity * (msg.value/etherReserve); //내가 넣은 이더양이 풀 안의 이더양에서 차지하는 비율만큼 LP토큰 민팅
            _mint(msg.sender, lpTokenMinted);

        }else{ //(2) 유동성이 없는 상태에서 처음으로 공급
            uint256 tokenAmount = _maxTokens; 
            uint256 initialLiquidity = address(this).balance;           //내가 풀에 넣는 그레이 토큰양은 내가 입력한 유동성 총량 (1:1이므로) 
            _mint(msg.sender, initialLiquidity);                        //공급자에게 LP토큰 민팅
            token.transferFrom(msg.sender, address(this), tokenAmount); //컨트랙트가 내가 공급한 그레이토큰을 가져감
        }
    }

    //유동성 철회 함수
    function removeLiquidity(uint256 _lpTokenAmount) public payable{
        require(_lpTokenAmount > 0);

        uint256 totalLiquidity = totalSupply();  //전체 LP토큰
        uint256 ethAmount = _lpTokenAmount * (address(this).balance/totalLiquidity); //받아야할 이더양 = 반납한 lp토큰 * (이더가 전체 유동성에서 차지하는 비율)
        uint256 tokenAmount = _lpTokenAmount * (token.balanceOf(address(this))/totalLiquidity); //받아야할 그레이토큰양 = 반납한 lp토큰 * (그레이 토큰이 전체 유동성에서 차지하는 비율) 

        _burn(msg.sender, _lpTokenAmount); //내가 반납한 lp토큰 소각
        payable(msg.sender).transfer(ethAmount); //컨트랙트가 소각자에게 계산된 이더 전송
        token.transfer(msg.sender, tokenAmount); //컨츠랙트가 소각자에게 계산된 토큰 전송
    }
    ...
    function ethToToken(uint256 _minTokens, address _recipient) private {
        ...
        uint256 outputAmount = getOutputAmountWithFee(
            inputAmount,                       //사용자가 넣은 이더양
            address(this).balance-inputAmount, //컨트랙트가 가진 이더양 (address(this).balance는 사용자가 입력한 이더양도 포함한 값이므로 inputAmount 빼줘야함)
            token.balanceOf(address(this))     //컨트랙트가 가진 토큰양
    ...
    function ethToTokenSwap(uint256 _minTokens) public payable { 
        ethToToken(_minTokens, msg.sender); //ethToTokenSwap의 경우 recipient = msg.sender 그레이토큰 받는 사람의 주소
    }
    //이더리움 -> 패스트토큰 스왑으로 전달하기 위해 매개변수 그대로 전달
    ...
    function tokenToEthSwap(uint256 _tokenSold, uint256 _minEth) public {
        ...
        payable(msg.sender).transfer(_minEth); //컨트랙트가 스왑하는 사랍에게 계산된 이더 전송
    ...
    function tokenToTokenSwap(uint256 _tokenSold, uint256 _minTokenBought, uint256 _minEthBought, address _tokenAddress) public payable{
        address toTokenExchangeAddress = factory.getExchange(_tokenAddress); //패스트토큰과 이더리움 스왑하는 Exchange 컨트랙트의 주소

        //그레이토큰->이더리움 스왑하면 받게 될 이더양 계산
