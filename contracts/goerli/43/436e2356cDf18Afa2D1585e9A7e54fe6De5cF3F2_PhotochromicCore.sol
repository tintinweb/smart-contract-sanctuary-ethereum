...
contract PhotochromicCore is Ownable {
    ...
    /**
     * Removes any open tickets and the expiry of the userId of the sender.
     * Also burns the userId at the registrar.
     */
    function burn(string memory userId) external {
        bytes32 baseNode = registrar.baseNode();
        bytes32 node = namehash(baseNode, userId);
    ...
    function _burn(bytes32 node, bytes32 baseNode, string memory label, address owner) internal {
        if (baseNode == registrar.baseNode()) registrar.burn(label);
        _burnTicket(node, owner);
...
contract PhotochromicRegistrar is ERC721Enumerable, Controllable {
    ...
    function burn(string calldata userId) public onlyController {
        bytes32 label = keccak256(abi.encodePacked(userId));
        uint256 tokenId = uint256(keccak256(abi.encodePacked(baseNode, label)));
        _burn(tokenId);
    ...
    function _transfer(address, address, uint256) internal override(ERC721) {
...
contract PhotochromicResolver is Resolver {
    ...
    function setValidatedTextRecords(bytes32 node, ValidatedTextRecord[] calldata list) external onlyOwner {
    ...
    function setValidatedAddrRecords(bytes32 node, ValidatedAddrRecord[] calldata list) external onlyOwner {
