...
contract RoyaltyReceiverMinterCollect is OwnableUpgradeable, UUPSUpgradeable {
    ...
    */
    ...
    //     uint256 minterAmount = _minterBalances[_msgSender()];
    //     if (minterAmount > 0) {
    //         _minterBalances[_msgSender()] = 0;
    //         //
    //         emit MinterCollectedRoyaltyPaid(_msgSender(), minterAmount);
    ...
    */
    ...
    /*
            if (i == 0) {
                ...
                lastMinterSum = _ERC20minterBalances[erc20][minter] + minterPart;
                lastBeneficearySum = _ERC20beneficearyBalances[erc20] + contractPart;
            } else {
                if (lastTokenID == txData.tokenId) {
                    // if tokenId same as previous then just increase value
                    lastMinterSum += minterPart;
                    lastBeneficearySum += contractPart;
                } else {
                    // different tokenID, re-fetch data

                    address _minter = Minter(_bex).getMinter(txData.tokenId);
                    if (minter != _minter) {
                        _ERC20minterBalances[erc20][minter] = lastMinterSum;
                        minter = _minter;
                        lastMinterSum = _ERC20minterBalances[erc20][minter] + minterPart;
                    } else {
                        lastMinterSum += minterPart;
    ...
    */
    ...
    function combinedWithdraw(
        ...
        } else if (erc20payloadHolder.length > 0 || payloadHolder.length > 0) {
