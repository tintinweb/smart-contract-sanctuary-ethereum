...
contract GenerativeCollectionToken is ERC721, ERC2981 {
    ...
    constructor(string[3] memory metadata_, address owner_,uint96 fee_, uint256[5] memory uints, address platform_address_, address[] memory access_tokens_, address[] memory profit_split_addresses_, uint256[] memory profit_split_amount_) ERC721(metadata_[0], metadata_[1]) {
    ...
    function mint(uint256 quantity_) external payable  {
        ...
        require(_limit >= tokenId + quantity_, "Limit exceeded");
        ...
        if (_free_mint > tokenId + quantity_) {
        ...
        if (_free_mint > tokenId + quantity_ && has_access_token) require(msg.value >= _price * quantity_, "Not enough funds send");
        ...
        if (_free_mint > tokenId + quantity_ && has_access_token) {
            uint256 owner_fee = (msg.value * 975) / 1000;
            ...
            for (uint256 i = 0; i < _profit_split_amount.length; i++) {
                payable(_profit_split_addresses[i]).transfer(_profit_split_amount[i]);
            }
        }
    }
