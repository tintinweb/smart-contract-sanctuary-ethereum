    ...
    UUPSUpgradeable
{
    ...
    event StandardAuctionDropCancelled(uint256 indexed dropId);
    ...
    {
        StandardAuctionDrop memory stdAuction = StandardAuctionSaleStorage
            .layout()
            .drops[dropId];
    ...
    function remainingItems(uint256 dropId) public view returns (uint32) {
        ...
        return uint32(stdAuction.itemIds.length) - stdAuction.numMinted;
    }
        ...
        returns (uint256)
    {
        ...
        uint256 currentBid = layout.bids[dropId][itemId].highestBid;
        uint256 startingPrice = layout.drops[dropId].startingPrice;
        ...
        return
            currentBid > 0 // override minimum price if there is any bid. any change to minimum price will not affect existing bids
                ...
                : startingPrice +
                    ((startingPrice * minBidIncrementPercent) / 100);
    }
    ...
    {
        StandardAuctionItemBid memory currentBid = StandardAuctionSaleStorage
            .layout()
            .bids[dropId][itemId];
    ...
    function isItemInDrop(uint256 dropId, uint256 itemId)
        ...
        returns (bool)
    {
        uint256[] memory itemIds = StandardAuctionSaleStorage
            .layout()
            .drops[dropId]
            .itemIds;

        for (uint16 i; i < itemIds.length; i++) {
    ...
    function dropExists(uint256 dropId) internal view returns (bool) {
        return
            StandardAuctionSaleStorage.layout().drops[dropId].itemIds.length >
            0;
    }
    ...
    /// @param itemIds The ids of the auctioned items
    ...
    function publishDrop(
        ...
        layout.drops[dropId] = StandardAuctionDrop(
            collection,
            itemIds,
            startingPrice,
            start,
            ...
            false, // not cancelled
            0, // no item sold
            DEFAULT_BID_INCREMENT_PERCENT // minBidIncrementPercent
        );
    ...
    function rePublishDrop(
        ...
        StandardAuctionDrop memory stdAuction = layout.drops[dropId];
        ...
        notPaused
    {
        ...
        emit StandardAuctionDropCancelled(dropId);

        for (uint16 i; i < stdAuction.itemIds.length; i++) {
            uint256 itemId = stdAuction.itemIds[i];
            StandardAuctionItemBid memory itemBid = layout.bids[dropId][itemId];

            if (
                !itemBid.refunded &&
                !itemBid.minted &&
                itemBid.highestBid > 0 &&
                itemBid.highestBidder != address(0)
            ) {
                // `refunded` could be redundant wrt. drop.cancelled, but it's an extra check against malicious refunds + allows to query on-chain if a bid has been refunded
                layout.bids[dropId][itemId].refunded = true;
                payable(itemBid.highestBidder).transfer(itemBid.highestBid);

                emit BidRefunded(
                    dropId,
                    itemId,
                    itemBid.highestBidder,
                    itemBid.highestBid
                );
            }
        }
    ...
    function _bid(
        ...
        StandardAuctionItemBid memory currentBid = layout.bids[dropId][itemId];
        ...
        layout.bids[dropId][itemId].highestBid = msg.value;
        ...
        validateBidAuth(bidAuth)
    {
        ...
        _bid(bidAuth.dropId, bidAuth.itemId, bidAuth.bidder);
    }
    ...
    /// @param to The address of the recipient of the item to be minted.
    ...
    function _mint(
        ...
        if (block.timestamp < stdAuction.start + stdAuction.auctionPeriod)
            revert DropNotEnded(dropId);
        if (!isItemInDrop(dropId, itemId)) revert ItemNotFound(dropId, itemId);

        StandardAuctionItemBid memory currentBid = layout.bids[dropId][itemId];

        if (currentBid.minted) revert ItemMinted(itemId);

        if (
            (currentBid.highestBid <= 0) ||
            (currentBid.highestBidder == address(0))
        ) revert NoBid();
        if (to != currentBid.highestBidder) revert NotHighestBidder();

        if (currentBid.refunded) revert AlreadyRefunded();

        layout.drops[dropId].numMinted++;
        layout.bids[dropId][itemId].minted = true;
