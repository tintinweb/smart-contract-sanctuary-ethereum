...
contract Vault is Ownable {
    ...
    function deposit(bytes32 operationId, address nftAddress, uint256 tokenId) external OpIdNotSeen(operationId) {
        // @dev disabled this now for event reading and testing, enable it later
...
// CHAIN 2 FUNCS START
    ...
    function mint(bytes32 operationId, address syntheticNFT, address user, bytes32 uri) external onlyOwner OpIdNotSeen(operationId) {
        FaucetERC721(syntheticNFT).mintWithURI(uri);
        uint256 tokenIndex = FaucetERC721(syntheticNFT).tokenIndex();
        FaucetERC721(syntheticNFT).transferFrom(address(this), user, tokenIndex);
