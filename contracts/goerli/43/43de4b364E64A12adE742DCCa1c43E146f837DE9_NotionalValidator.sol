...
contract NotionalValidator{
    ...
    bytes32 constant public DATA_ORACLERATE_POS_1 = keccak256(abi.encodePacked(uint256(1)));
    bytes32 constant DATA_ORACLERATE_POS_HASH_1 = keccak256(abi.encodePacked(DATA_ORACLERATE_POS_1));
    ...
    function submitState(bytes memory _blockHeaderRlpBytes, bytes memory _proofRlpBytes) external{
        ...
        StateProofVerifier.SlotValue memory rate = StateProofVerifier.extractSlotValueFromProof(
            DATA_ORACLERATE_POS_HASH,
            accountData.storageRoot,
            proofs[1].toList()
        );

        require(rate.exists, "oracleRate");
    }
