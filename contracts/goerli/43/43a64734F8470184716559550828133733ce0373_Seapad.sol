...
contract Seapad is VRFV2WrapperConsumerBase, ChainlinkClient {
    ...
    address packBordersAddress;
    // address ELF; //ETH_LINK_FEEDBACK_ADDRESS
    ...
    address public peggedLinkToken;
    address public chainLinkPeggedSwapContract;
    ...
    constructor(
        ...
        address _peggedLinkAddress,
        address _chainLinkPeggedSwapContract,
        address _routerAddress
    )
        VRFV2WrapperConsumerBase(
            _peggedLinkAddress != address(0)
                ? _peggedLinkAddress
                : _linkTokenAddress,
            _vrfWrapper
        )
    {
        owner = msg.sender;
        peggedLinkToken = _peggedLinkAddress;
        chainLinkPeggedSwapContract = _chainLinkPeggedSwapContract;
    ...
    function emergencyGetRandomNumber(
        uint256 projectid,
        uint256 packSoldID,
        uint32 callbackGasFee
        ...
        uint256 amountToPay = VRF_V2_WRAPPER.calculateRequestPrice(
            totalCombined
        );
        if (peggedLinkToken != address(0)) {
            require(
                IERC20(peggedLinkToken).balanceOf(address(this)) >= amountToPay
            );
        } else {
            require(LINK.balanceOf(address(this)) >= amountToPay);
        }
        return
            getRandomNumber(projectid, packSoldID, callbackGasFee, amountToPay);
    }
    ...
    function openPack(uint256 projectid, uint256 packSoldID_) public payable {
        isValidProject(projectid);
        Projects storage project = projects[projectid];
        PacksBought storage pb = project.packsBought[packSoldID_];
        ...
        uint256 ethToPay = Payments.getEstimatedETHforToken(
            linkTokenAddress, //link
            amountToPay //amount of tokens we need to receive from this purchase.
        )[0];
        require(
            msg.value >= ethToPay,
            string(abi.encodePacked("PN: ", SeapadHelper.uint2str(ethToPay))) //PAYMENT NEEDED: VALUE
        ); //notEnoughETHProvided
        uniswapV2Router.swapExactETHForTokens{value: ethToPay}(
            amountToPay,
            SeapadHelper.getPathForToken(linkTokenAddress, WETHADDRESS),
            address(this),
            block.timestamp + 150
        );
        //we received LINK tokens.
        //we need to check if this is pegged, and if it is, then we need to swap to original LINK token.
        if (peggedLinkToken != address(0x0)) {
            LinkSwapper swapper = LinkSwapper(chainLinkPeggedSwapContract);
            IERC20(linkTokenAddress).approve(
                chainLinkPeggedSwapContract,
                amountToPay
            );
            swapper.swap(amountToPay, linkTokenAddress, peggedLinkToken);
        }
        //now we can request the randomness
        ...
        emit OpenPackRequest(pb.projectID, packSoldID_, tx.origin, requestid);
    }
    ...
    function getRandomNumber(
        ...
        pb.requestStatus = RequestStatus({
            requestId: requestId,
            paid: LINKTokenPaid,
    ...
    function getPackSoldRandomnessDetails(
        uint256 projectid,
        uint256 packSoldID_
    ) public view returns (RequestStatus memory status) {
        isValidProject(projectid);
        Projects storage project = projects[projectid];
        PacksBought storage pb = project.packsBought[packSoldID_];
    ...
    /* 
        Used by admins in Testing.
    */
    function fulfillRandomWordsByAdmin(
        uint256 requestId,
        ...
        pb.requestStatus.fulfilled = true;
        pb.requestStatus.randomWords = randomWords;
    ...
    function createTokenPart2(
        ...
        uint256[] calldata paymentMethods_,
        uint256 basePayment,
        uint256 slippage // uint16 gasLane
    ) public payable {
...
contract Minter is Ownable {
    ...
    constructor(address parentContract, uint256 _networkid) {
...
contract Pack is ERC721, Ownable {
    ...
    // collections/' + _this.networkID + "/" + final.pid + '/storefront.json
    // string storefrontURI =
    ...
    function _beforeTokenTransfer(
        ...
        if (pack.requestStatus.requestId != 0) {
            revert("Pack is not transferable");
        }
        // if (pack.requestStatus.requestId != bytes32(0)) {
    ...
    constructor(
        ...
        seapadOwner = _seapadOwner;
        projectid = _projectid;
        transferOwnership(_seapadOwner);
        networkid = _networkid;
        // _uri = thisUri;
    ...
    function openPack(uint256 packid) public payable {
        Parent parentContract = Parent(seapadAddress);

        parentContract.openPack{value: msg.value}(projectid, packid);
    }
    ...
    function contractURI() public view returns (string memory) {
        return
            string(
                abi.encodePacked(
                    "https://cdn.seapad.io/collections/",
                    uint2str(networkid),
                    "/",
                    uint2str(projectid),
                    "/storefront.json"
