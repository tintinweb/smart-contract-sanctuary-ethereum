...
contract ProfileFacet {
    ...
    function checkPfp(address signetUserAddress) public view returns (string memory) {
        (string memory a, address b, uint256 c) = LibSignetStorage.checkPfp(signetUserAddress);
        if (b != address(0)) {
            if (IERC721(b).ownerOf(c) == msg.sender || IERC1155(b).balanceOf(msg.sender, c) > 0) {
                return a;
            } else {
                return
    ...
    function modifyPfpForUser(
        ...
        if (
            ...
            LibSignetStorage.modifyPfpForUser(_pfp, msg.sender, _tokenId, _collection);
...
library LibSignetStorage {
    ...
    ) internal view returns (string memory, address, uint256) {
        AppStorage storage s = getStorage();
        for (uint256 i = 0; i < s.ps.totalpfp + 1; i++) {
            if (s.pm.pfp[i].owner == _signetUserAddress) {
                return (s.pm.pfp[i].pfp, s.pm.pfp[i].collection, s.pm.pfp[i].tokenId);
            }
        }
        return (
            "You seeing this message is becuase this address don't have any pfp created!",
