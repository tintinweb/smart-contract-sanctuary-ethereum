...
//
    function initCode() public view returns (bytes memory) {
        ...
        assembly {
           ...
           let res := mload(0x40)       // get free memory pointer
            ...
            mstore(add(res, 0x20), 74) // length: 64 bytes
            mstore(add(res, 0x40), mload(pos))    // first 32 bytes
            mstore(add(res, 0x60), mload(add(pos,0x20)))    // second 32 bytes
            mstore(add(res, 0x80), mload(add(pos,0x40)))    // second 32 bytes
            mstore(0x40, add(res, 0xa0)) // update free memory pointer
            return (res, 138)           // return 128 bytes (32 bytes data offset + 32 bytes length + 64 bytes data)
        
        }
    }

    function clone() internal returns (address instance) {
        /// @solidity memory-safe-assembly
        address payable implementation = mainAddress;
        assembly {
            let ptr := mload(0x40)
    ...
    function createEip1167() external returns (address newAddress) {
        newAddress = clone();
    }


    function predictDeployAddress1(bytes32 _salt) public view returns(address){
    ...
    function predictDeployAddress2(bytes32 _salt) public view returns(address){
        ...
        return address(uint(keccak256(abi.encodePacked(
            ...
            address(this),
            _salt,
            keccak256(abi.encodePacked(type(LogicContract).creationCode, abi.encode(mainAddress)))
        ))));
    }

    function getDeployNew() public view returns (bytes memory) {
        bytes memory bytecode = abi.encodePacked(type(LogicContract).creationCode, abi.encode(mainAddress));

        return bytecode;
    }

    function getDeployNewNopara() public view returns (bytes memory) {

        return type(LogicContract).creationCode;
    }

    function deployNewContractInstance(bytes32 _salt) public {
        bytes memory bytecode = abi.encodePacked(type(LogicContract).creationCode, abi.encode(mainAddress));
...
//
     ...
     */
    function deployNewCreate2(bytes32 salt) external returns (address dst) {
...
contract LogicContract {
    ...
    fallback() external payable {
        if(msg.data.length == 0){
            (bool success, ) = main.call{ value: msg.value }('');
            require(success, 'Forwarding funds failed');
        }else{
            (bool success, ) = main.delegatecall(msg.data);
