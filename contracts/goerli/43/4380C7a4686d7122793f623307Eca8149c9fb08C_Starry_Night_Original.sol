...
contract NFTClaimRegistry {
    mapping(address => bool) public hasClaimedContractA;
    mapping(address => bool) public hasClaimedContractB;

    function hasClaimed(address _address) public view returns (bool) {
        return hasClaimedContractA[_address] || hasClaimedContractB[_address];
    }

    function setHasClaimedContractA(address _address) public {
        hasClaimedContractA[_address] = true;
    }

    function setHasClaimedContractB(address _address) public {
        hasClaimedContractB[_address] = true;
    }
}
...
contract Starry_Night_Original is ERC721A, ERC2981, OperatorFilterer, Ownable {
    ...
    uint256 public publicMintSupply = 109;
    uint256 public claimSupply = 150;
    ...
    NFTClaimRegistry public claimRegistry;

    constructor(
        address _royaltyReceiver,
        uint96 _royaltyFraction
    )
        ERC721A("Starry Night Original", "Starry Night")
    {
        ...
        claimRegistry = NFTClaimRegistry(0xfd2492D529BBDa7220896288269EbdF20C97836b);
    }
    ...
    function mintForAll(uint256 amount, bool _isUK) external payable {
        ...
        require(publicMintCount < publicMintSupply, "Sold Out.");
    ...
    function claim(uint256 amount) external {
        ...
        require(!claimRegistry.hasClaimed(msg.sender), "You have already claimed an NFT.");
        require(claimCount < claimSupply,"All NFTs claimed from this Collection, try second Collection.");
        require(userClaimCount[msg.sender] + amount < MAX_CLAIM_PER_ACCOUNT, "You can not claim more than MAX_CLAIM_PER_ACCOUNT");
        _mint(msg.sender, amount);
        claimCount += amount;
        //claimed[msg.sender] = true;
        claimRegistry.setHasClaimedContractA(msg.sender);
    }
