...
interface IChubbyKaijuDAOStakingV2 {
  function stakeTokens(uint16[] calldata) external;
}
// File: Strings.sol
...
contract ChubbyKaijuDAOStakingV2 is IChubbyKaijuDAOStakingV2, OwnableUpgradeable, IERC721ReceiverUpgradeable, PausableUpgradeable {
  ...
  mapping(address => mapping(uint16 => uint48)) private _gen2StakedTimes;

  mapping(uint16 => uint16) public gen2RewardTypes; 
  ...
  function setRewardTypes(uint16[] calldata tokenIds, uint16 rewardType) public onlyOwner{
    for (uint16 i = 0; i < tokenIds.length; i++) {
      gen2RewardTypes[tokenIds[i]] = rewardType;
    }
  }

  function stakeTokens(uint16[] calldata tokenIds) external whenNotPaused _updateEarnings {
    
    for (uint16 i = 0; i < tokenIds.length; i++) {
      _stakeGEN2(msg.sender, tokenIds[i]);
      chubbykaijuGen2.safeTransferFrom(msg.sender, address(this), tokenIds[i], '');
    }
  
  }

  function _stakeGEN2(address account, uint16 tokenId) internal {
    ...
    _gen2StakedTimes[account][tokenId] = uint48(block.timestamp);

    emit TokenStaked("CHUBBYKAIJUGen2", tokenId, account);
  }

  function claimRewardsAndUnstake(uint16[] calldata tokenIds, bool unstake) external whenNotPaused _updateEarnings {
