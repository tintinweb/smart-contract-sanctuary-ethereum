    ...
    OperatorFilterer
{
    ...
    uint256 private constant _TOTAL_SHARES_ALLOC = 10_000;
    uint256 private constant _STRUCT_SHARES_ALLOC = 500;
    ...
    function initialize(
        ...
        public
    {
        if (config.maxSupply > _MAX_SUPPLY_LIMIT) revert StructERC721_InvalidConfiguration();
    ...
    function _validatePrepaid(bytes calldata _receipt, uint256 _nonce) internal returns (bool) {
        ...
        bool validated =
            _RECEIPT_ISSUER != ECDSA.recover(
                ECDSA.toEthSignedMessageHash(keccak256(abi.encodePacked(_nonce, msg.sender))),
                _receipt
            );
    ...
    function setProvenanceHash(string calldata newProvenanceHash) public payable lockable onlyOwner {
        if (config.offsetEnabled && _RANDOMNESS_MANAGER.randomness(address(this), _TOKEN_OFFSET_SLOT) != 0)
            revert StructERC721_InvalidMetadataState();
