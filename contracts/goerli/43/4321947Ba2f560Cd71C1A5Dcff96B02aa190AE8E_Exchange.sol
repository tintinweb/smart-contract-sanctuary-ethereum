...
contract Exchange is Ownable, ReentrancyGuard {
    ...
    function fillOrder(uint256 _id, uint256 _getAmount) public payable nonReentrant {
        ...
        _trade( _order.user, _order.tokenGet, _getAmount, _order.tokenGive, _giveAmount);
        ...
        if(_getAmount < _order.amountGet) {
            ...
            emit Subtrade(_order.id,  _order.user, _order.tokenGet, _getAmount, _order.tokenGive, _giveAmount, msg.sender, block.timestamp);
        } else {
            orderFilled[_order.id] = true;
            emit Trade(_order.id,  _order.user, _order.tokenGet, _order.amountGet, _order.tokenGive, _order.amountGive, msg.sender, block.timestamp);
        }
