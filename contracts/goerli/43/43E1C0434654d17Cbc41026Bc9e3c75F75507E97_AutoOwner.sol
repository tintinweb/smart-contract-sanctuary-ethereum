...
contract AutoOwner is DefaultCallbackHandler, ISignatureValidator {
  address owner;
  ...
  function approveHash(IGnosisSafe safe, bytes32 hashToApprove) external {
  ...
  function getOwner() external view returns (address) {
    ...
    function isValidSignature(bytes memory _data, bytes memory _signature) public view override returns (bytes4) {
      require(keccak256(_data).toEthSignedMessageHash().recover(_signature) == owner, "Not owner");
...
interface IGnosisSafe {
    ...
    function signedMessages(bytes32 _data) view external returns(uint256);
