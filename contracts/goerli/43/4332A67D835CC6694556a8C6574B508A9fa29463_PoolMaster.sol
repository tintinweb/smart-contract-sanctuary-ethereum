...
abstract contract PoolBase is PoolBaseInfo {
    ...
    function borrow(
        uint256 amount,
        address receiver
    ) external onlyManager onlyActiveAccrual {
    ...
    function close() external {
        ...
        bool inactiveOverMax = _info.enteredZeroUtilization != 0 &&
            block.timestamp >
            (_info.enteredZeroUtilization + maxInactivePeriod);
    ...
    function accountOlderReserveTransfers(
        address to,
        uint256 amount
    ...
    function _availableToProviders(
        BorrowInfo memory info
    ...
    /// @param info Borrow info struct
    /// @return Pool size for given info
    function _availableProvisionalDefault(
        BorrowInfo memory info
    ) internal view returns (uint256) {
    ...
    function _entranceOfProvisionalDefault(
    ...
    function _accrueInterestVirtual()
        internal
        view
        returns (BorrowInfo memory newInfo)
    {
...
abstract contract PoolBaseInfo is ERC20Upgradeable {
    ...
    function __PoolBaseInfo_init(
        ...
        nonZeroAddress(address(currency_))
    {
        manager = manager_;
        currency = currency_;
        ...
        (_info.enteredZeroUtilization, _info.lastAccrual) = (
            block.timestamp,
            block.timestamp
        );
    }
...
abstract contract PoolConfiguration is PoolBase {
    ...
    function setManager(
        address manager_
    ) external onlyFactory nonZeroAddress(manager_) {
        manager = manager_;
    }
...
abstract contract PoolRewards is PoolBase {
    ...
    function withdrawReward(
        address account
    ) external onlyFactory returns (uint256 withdrawable) {
        _accrueInterest();
        _distributeReward();
