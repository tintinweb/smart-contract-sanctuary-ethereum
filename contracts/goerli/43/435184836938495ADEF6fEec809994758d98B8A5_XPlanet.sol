...
contract XPlanet is ERC721Enumerable, Ownable {
   ...
   // address public constant USDC_CONTRACT = 0x07865c6E87B9F70255377e024ace6630C1Eaa37F;
    ...
    uint256 public constant MAX_WHITELIST2_MINT = 1; 
    ...
    function updateReservedlistMerkleRoot(bytes32 _newMerkleRoot)
        external
        onlyOwner
    {
    ...
    function reservedMint(uint256 _numberOfTokens, bytes32[] calldata merkleProof ) external  {
        ...
        bool isWhitelisted = MerkleProof.verify(
            merkleProof, reservedlistMerkleRoot,
            keccak256(abi.encodePacked(_user))
        );
    ...
    function mint( bytes32[] calldata merkleProof ) external  {
        ...
        require( !isContract(_user), "mint-via-contract" );
        require( totalSupply() + _numberOfTokens <= MAX_SUPPLY, "max-supply-reached" );
        require( maxMintsPerAddress[_user] + _numberOfTokens <= MAX_WHITELIST2_MINT, "max-mint-limit" ); 

        bool isWhitelisted = MerkleProof.verify(
            merkleProof,
            whitelist2MerkleRoot,
            keccak256(abi.encodePacked(_user))
        );
