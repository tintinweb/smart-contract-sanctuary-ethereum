...
****************************************************************************************************/
  function returnNFT(address nftContract, uint256 tokenId, address seller) external onlyAdmin {
    _checkRegisteredMarket(nftContract, tokenId); // 마켓에 등록되어있다면 에러를 발생시킨다.
...
* @dev 주석 주석
****************************************************************************************************/
abstract contract MarketAuction is ErrorMessage, Constants, AdminTreasury, MarketCore, ReentrancyGuardUpgradeable, SupportsInterfaces, SendValueWithFallbackWithdraw, MarketFees {
...
****************************************************************************************************/
abstract contract MarketFixedPrice is ErrorMessage, MarketCore, SupportsInterfaces, MarketFees {
  using AddressUpgradeable for address payable;
...
****************************************************************************************************/
  struct FixedPrice {
    ...
    uint256 startTime;      // buy 가능 시작 시간 Unix timestamp 형태로 저장. 0이면 사용 안함
...
****************************************************************************************************/
  function _buy(address nftContract, uint256 tokenId) private nonReentrant {
    ...
    // Distribute revenue for this sale.
    (uint256 mssFee, uint256 creatorFee) = _distributeFunds(nftContract, tokenId, fixedPrice.price, fixedPrice.marketFee);

    emit FixedPriceSold(nftContract, tokenId, fixedPrice.seller, msg.sender, fixedPrice.price, mssFee, creatorFee);
  }
...
****************************************************************************************************/
  function _distributeFunds(address nftContract, uint256 tokenId, uint256 price, uint16 sendMarketFee) internal returns (uint256 marketFee, uint256 creatorFee)
  {
...
****************************************************************************************************/
  function _getInterfacesCreatorPaymentInfo(address nftContract, uint256 tokenId) internal view returns (address payable recipient, uint256 royaltyPoints) {
    ...
    if (nftContract.supportsERC165Interface(type(IRoyaltyInfo).interfaceId)) {
      (address receiver, uint256 points) = IRoyaltyInfo(nftContract).royaltyInfo{ gas: READ_ONLY_GAS_LIMIT }(tokenId, BASIS_POINTS);
