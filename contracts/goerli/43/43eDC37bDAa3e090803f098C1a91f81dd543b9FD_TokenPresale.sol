...
contract TokenPresale is ITokenPresale, PaymentToken, Ownable, ReentrancyGuard, ChainlinkPriceOracle {
    ...
    modifier onlyFinalized() {
        require(isFinalized == true, "Presale: not finalize yet!");
        _;
    }
    ...
    constructor(
        ...
        require(presaleToken_ != address(0), "Presale: zero address");
    ...
    function setBeneficiary(address beneficiary_) external onlyOwner {
        require(beneficiary_ != address(0), "Presale: zero address");
        beneficiary = beneficiary_;
        emit SetBeneficiary(beneficiary_);
    }

    function setMinPursechase(uint256 minPurchase_) external onlyOwner {
        require(minPurchase_ > 0, "Presale: zero price");
        minPurchase = minPurchase_;
        emit SetMinPurchase(minPurchase_);
    }

    function setTokenPrice(uint256 newPrice_) external onlyOwner {
        require(newPrice_ > 0, "Presale: zero price");
        tokenPrice = newPrice_;
        emit SetPrice(newPrice_);
    }

    function buy(address paymentToken_, uint256 paymentAmount_) external payable onlyAcceptedToken(paymentToken_) nonReentrant {
        uint256 usdAmount;
        uint256 tokenAmount;
        address sender = _msgSender();

        if (paymentToken_ == address(0)) {
            require(msg.value >= paymentAmount_, "Presale: insufficient balance");
            usdAmount = _valueToUsd(paymentAmount_);
            __transfer(address(0), beneficiary, paymentAmount_);
        } else {
            usdAmount = paymentAmount_;
            IERC20(paymentToken_).safeTransferFrom(sender, beneficiary, paymentAmount_);
        }

        require(usdAmount >= minPurchase, "Presale: minPurchase");

        tokenAmount = _getTokenAmount(usdAmount);
        tokensSold += tokenAmount;
        contributions[sender] += tokenAmount;
        tokenRaised[paymentToken_] += paymentAmount_;
        tokenBought[sender][paymentToken_] += paymentAmount_;

        emit TokensPurchased(sender, beneficiary, paymentToken_, paymentAmount_, tokenAmount);
    }

    function buyExactTokens(address paymentToken_, uint256 tokenAmount_) external payable onlyAcceptedToken(paymentToken_) nonReentrant {
        address sender = _msgSender();
        uint256 paymentAmount;
        uint256 usdAmount = (tokenAmount_ / tokenPrice) * 1 ether;

        if (paymentToken_ == address(0)) {
            require(msg.value >= paymentAmount, "Presale: insufficient balance");
            paymentAmount = _usdToValue(usdAmount);
            __transfer(address(0), beneficiary, paymentAmount);
        } else {
            paymentAmount = usdAmount;
            IERC20(paymentToken_).safeTransferFrom(sender, beneficiary, paymentAmount);
        }
        ...
        tokenRaised[paymentToken_] += paymentAmount;
        tokenBought[sender][paymentToken_] += paymentAmount;
    ...
    function claim() external nonReentrant onlyFinalized {
        ...
        require(!hasClaimed[sender], "Presale: already claimed");
        hasClaimed[sender] = true;
        contributions[sender] = 0;
        IERC20(presaleToken).safeTransfer(sender, contributionAmount);
    ...
    function ownerClaim(address[] calldata beneficiaries_) external nonReentrant onlyFinalized onlyOwner {
        ...
        for (uint256 i = 0; i < length; ) {
            address recipient = beneficiaries_[i];
            uint256 contributionAmount = contributions[recipient];
            require(!hasClaimed[recipient], "Presale: already claimed");
            hasClaimed[recipient] = true;
            contributions[recipient] = 0;
            IERC20(presaleToken).safeTransfer(recipient, contributionAmount);
            unchecked {
                ++i;
            }
    ...
    function withdrawToken(address token_, uint256 amount_) external onlyOwner {
        if (token_ == presaleToken) {
            require(this.getWithdrawableAmount() >= amount_, "Presale: not enough withdrawable funds");
        }
    ...
    function totalRaised() external view returns (uint256) {
        uint256 length = viewPaymentTokensCount();
        uint256 total = 0;
        for (uint256 i = 0; i < length; ) {
            address paymentToken = _viewTokenByIndex(i);
            if (paymentToken == address(0)) {
                total += tokenRaised[paymentToken] * getLatestPrice();
            } else {
                total += tokenRaised[paymentToken];
            }
            unchecked {
                ++i;
            }
    ...
    function _getTokenAmount(uint256 usdAmount) internal view returns (uint256) {
        return (usdAmount * 1 ether) / tokenPrice;
    }

    function _valueToUsd(uint256 weiAmount) internal view returns (uint256) {
        uint256 ethPrice = getLatestPrice();
        return (weiAmount * ethPrice) / 1 ether;
    }

    function _usdToValue(uint256 weiAmount) internal view returns (uint256) {
        return (weiAmount * 1 ether) / getLatestPrice();
    }
...
interface ITokenPresale {
    ...
    event TokensPurchased(address indexed purchaser, address indexed beneficiary, address paymentToken, uint256 usdAmount, uint256 tokensAmount);
...
contract ChainlinkPriceOracle {
    ...
    constructor() {
        priceFeed = IAggregatorV3(0xD4a33860578De61DBAbDc8BFdb98FD742fA7028e);
    }
