...
// File contracts/bridge/common/tokens/ITRC1155Receiver.sol
...
// File contracts/bridge/TokenPredicates/MintableERC1155Predicate.sol
    ...
    Initializable
{
    ...
    TokenType public constant TOKEN_TYPE = TokenType.MintableERC1155;
 ...
 * @notice rejects single transfer
    ...
    function onTRC1155BatchReceived(
        ...
        return TRC1155TokenReceiver.onTRC1155BatchReceived.selector;
    }
    ...
    function calculateAmountsToBeMinted(
        ...
        uint256[] memory toBeTransferAmounts = new uint256[](
            tokenBalances.length
        );
        bool needMintStep;
        ...
        for (uint256 i = 0; i < tokenBalances.length; i++) {
            ...
            if (toBeMintedAmounts[i] < amountsToBeExited[i]) {
                toBeTransferAmounts[i] = amountsToBeExited[i] - toBeMintedAmounts[i];
                needTransferStep = true;
            }
        }

        return (toBeMintedAmounts, toBeTransferAmounts, needMintStep, needTransferStep);
    }
     ...
     * @param localToken Token which gets withdrawn
     * @param withdrawData Valid ERC1155 TransferSingle burn or TransferBatch burn log from child chain
     */
    function exitTokens(
        address sender,
        address localToken,
        ...
        (
            ...
            uint256[] memory amounts,
            bytes memory data
        ) = abi.decode(withdrawData, (uint256[], uint256[], bytes));
        emit ExitedBatchMintableERC1155(
            withdrawer,
            localToken,
            ids,
            amounts);
        IMintableERC1155 token = IMintableERC1155(localToken);

        uint256[] memory balances = token.balanceOfBatch(makeArrayWithAddress(address(this), ids.length), ids);
        (uint256[] memory toBeMinted, uint256[] memory toBeTransfer, bool needMintStep, bool needTransferStep) = calculateAmountsToBeMinted(balances, amounts);
