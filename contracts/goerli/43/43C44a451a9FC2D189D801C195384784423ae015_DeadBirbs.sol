...
contract DeadBirbs is ERC721A, Seller, AttributeStore, ReentrancyGuard {
    ...
    */
    ...
    function publicMint2(string calldata attributes, bytes calldata signature) external payable nonReentrant {
        verifyPublicSale(1);
        verifyAttributeHash(attributes, signature);
        sotreAttributeHash(attributes, msg.sender, totalSupply());
        _safeMint(msg.sender, 1);
    }

    function publicMint3(string calldata attributeList, bytes calldata signature) external payable nonReentrant {
        verifyPublicSale(1);
        verifyAttributeHash(attributeList, signature);
        sotreAttributeHash(attributeList, msg.sender, totalSupply());
        _safeMint(msg.sender, 1);
    }
