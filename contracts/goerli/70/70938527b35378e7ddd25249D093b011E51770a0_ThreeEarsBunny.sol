...
contract ThreeEarsBunny is AccessControl, ERC721ALockable {
    ...
    uint96 public constant MAX_WHITE_LIST_SIZE = 2;

    uint256 public publicPrice = 2e16;
    uint256 public whiteListPrice = 1e16;
    string public baseImageUri = "https://nft.3eb.io/"; //https://ipfs.virium.io/three-ears-bunny/image/

    address private constant SIGNER = 0x0Ac584A240fbae9e6403c569A7cE29fC5C4d8912;
    mapping(address => uint256) private mintedTokenCount;
    ...
    Stage public stage = Stage.WhiteList;
    ...
    function mintForPublic(uint256 quantity) external payable {
        ...
        require(stage == Stage.Public, "TEB: Stage mismatch");
    ...
    function mintForWhiteList(uint256 quantity, uint256 nonce, bytes memory sign) payable external {
        require(Address.isContract(msg.sender) == false, "TEB: Prohibit contract calls");
        require(quantity <= 2, "TEB: Mint would exceed max batch size");
        require(stage == Stage.WhiteList, "TEB: Stage mismatch");
        require(publicPrice * quantity <= msg.value, "TEB: Ether value sent is not correct");
        require(mintedTokenCount[msg.sender] + quantity <= MAX_WHITE_LIST_SIZE, "TEB: Mint would exceed max white list size");
        require(_totalMinted() + quantity <= MAX_TOTAL_SUPPLY, "TEB: Mint would exceed max total supply");
        ...
        _safeMint(msg.sender, quantity, "");
        mintedTokenCount[msg.sender] += quantity;
    }
    ...
    function setConfig(uint256 whiteListPrice_, uint publicPrice_, Stage stage_) external onlyRole(MAINTAIN_ROLE) {
        whiteListPrice = whiteListPrice_;
        publicPrice = publicPrice_;
        stage = stage_;
    }

    function getConfig() external view returns (uint256, uint256, Stage){
        return (whiteListPrice, publicPrice, stage);
    }
