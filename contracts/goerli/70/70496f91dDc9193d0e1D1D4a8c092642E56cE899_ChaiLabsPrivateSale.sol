...
contract ChaiLabsPrivateSale is Ownable {
    ...
    function createSale(Sale memory sale, SaleRound[] memory rounds) external {
        require(sale.admin != address(0), "ChaiLabs: invlaid admin");
        require(sale.paymentRecipient != address(0), "ChaiLabs: invlaid paymentRecipient");
    ...
    function _addSaleRound(uint256 saleId, SaleRound[] memory rounds) private {
        ...
        for (uint256 i = 0; i < rounds.length; i++) {
            ...
            uint256 roundEndTime = rounds[i].startTime + rounds[i].saleDuration;
            require(lastRoundEndTime < roundEndTime, "ChaiLabs: invalid round start time or duration");
            lastRoundEndTime = roundEndTime;
    ...
    function contribute(
        ...
        require(saleRoundRaisedAmounts[saleId][roundId] <= saleRound.hardCap, "ChaiLabs: round hardcap reached");
        require(
            saleRound.maxContribution <= userContributions[msg.sender][saleId][roundId] + amount,
            "ChaiLabs: contribution too high"
        );
        require(saleRound.hardCap <= totalContributions[saleId][roundId] + amount, "ChaiLabs: hardcap reached");
    ...
    //     require(saleRound.startTime + saleRound.saleDuration <= block.timestamp, "ChaiLabs: round has ended");
    //     require(saleRoundRaisedAmounts[saleId][roundId] >= saleRound.hardCap, "ChaiLabs: hardCap not reached yet");
    ...
    function claim(uint256 saleId) external validateSale(saleId) {
        ...
        require(sale.finalized, "ChaiLabs: sale not finalized");
        ...
        for (uint256 i = 0; i < rounds.length; i++) {
            ...
            // if softcap is reached and user have some contribution, allow him to claim tokens
            if (claimAmount > 0 && totalContributions[saleId][i] >= rounds[i].softCap) {
    ...
    function finalizeSale(uint256 saleId, uint256[] memory saleTokenAmount) external validateSale(saleId) {
        Sale memory sale = sales[saleId];
        ...
        for (uint256 i = 0; i < rounds.length; i++) {
            // if softcap is reached, finalize this round
            ...
            IERC20(sale.saleToken).safeTransferFrom(msg.sender, address(this), saleTokenAmount[i]);
            uint256 amountDeposited = IERC20(sale.saleToken).balanceOf(address(this)) - balanceBefore;
            require(
                amountDeposited >= totalContributions[saleId][i] * rounds[i].price,
                "ChaiLabs: sale token amount too low"
            );
        }
