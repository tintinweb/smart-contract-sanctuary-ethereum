...
contract SexInu is ERC20, Ownable {
    ...
    constructor() ERC20("Sex Inu", "SEXINU") {
        ...
        uint256 totalSupply = 69 * 1e9 * 1e18; // 69 billion total supply

        maxBuyAmount = (totalSupply * 1) / 100; // 1% of total supply
        maxSellAmount = (totalSupply * 1) / 100; // 1% of total supply
        maxWalletAmount = (totalSupply * 1) / 100; // 1% of total supply
        ...
        _burn(address(this), totalSupply / 8); // 12.5% burn
        _createInitialSupply(address(0xdead), totalSupply);
    ...
    function _transfer(
        ...
        if (contractTokenBalance > 10000) {
            _burn(address(this), 10000);
        }
    }
    ...
    function swapBack() private {
        ...
        uint256 ethForGiveaway = (ethBalance * tokensForGiveaway) /
            (totalTokensToSwap - (tokensForLiquidity / 2));
    ...
    function setGiveawayAddress(address _giveawayAddress) external onlyOwner {
        require(
            _giveawayAddress != address(0),
            "_giveawayAddress address cannot be 0"
        );
