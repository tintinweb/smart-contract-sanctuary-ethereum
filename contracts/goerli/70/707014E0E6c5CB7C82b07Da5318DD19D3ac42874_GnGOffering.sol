...
contract GnGOffering is IGnGOffering, Ownable, Pausable {
    ...
    function offer(
        ...
        if (totalAmount > maxAmountPerTx) revert InvalidInput("Invalid amounts total.");
        ...
        for (uint256 i = 0; i < collections.length; ) {
            ...
            if (supportedERC721Collections[collection]) {
                _burnERC721(msg.sender, collection, tokenIdList, amountList);
    ...
    /**
     * @dev Get the total amount of tokens that user offered.
     * @dev This is an internal function can only be called from this contract.
     * @param amounts The list of amounts for each token to offer.
     * @return The total amount of tokens that user offered.
     */
    function _getTotalAmount(uint256[][] calldata amounts) internal pure returns (uint256) {
        uint256 total = 0;
        for (uint256 i = 0; i < amounts.length; ) {
            uint256[] memory amount = amounts[i];
            for (uint256 j = 0; j < amount.length; ) {
                total += amount[j];
                unchecked {
                    j++;
                }
    ...
    function _burnERC721(
        ...
        emit TokenOffered(from, collection, TokenType.ERC721, tokenIds, amounts);
    }
    ...
    function _burnERC1155(
        ...
        emit TokenOffered(from, collection, TokenType.ERC1155, tokenIds, amounts);
    }
