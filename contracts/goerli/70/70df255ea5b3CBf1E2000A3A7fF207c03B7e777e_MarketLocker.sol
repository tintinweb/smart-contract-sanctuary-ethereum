...
contract MarketLocker {
    ...
    IKIKIVault public immutable vault;
    ...
    function claim() external {
        ...
        uint shouldReleaseAmount = vaultReleaseAmount
            .mul(MARKET_REEASE_RATIO)
            .div(VAULT_RELEASE_RATIO);
        ...
        if (balance < shouldReleaseAmount) {
            IERC20Mintable(address(token)).mint(
                address(this),
                shouldReleaseAmount.sub(balance)
            );
