    ...
    ReentrancyGuardUpgradeable
{
    ...
    function batchSeed(uint256 _commitId, uint256[] calldata seeds) external {
        ...
        require(commitSeeds[_commitId].length != c.amount, "Already seeded");
        require(c.amount == seeds.length, "Wrong number of seeds");
        commitSeeds[_commitId] = seeds;
    ...
    function batchReveal(uint256 _commitId) internal {
        ...
        require(commitSeeds[_commitId].length == c.amount, "Not yet seeded");

        uint16 tokenId = uint16(_totalMinted());
        uint16[] memory tokenIds = new uint16[](c.amount);
        for (uint256 index = 0; index < commitSeeds[_commitId].length; index++) {
            tokenId++;
            generate(tokenId, commitSeeds[_commitId][index]);
    ...
    function batchUserRevealPending(address recipient) external whenNotPaused nonReentrant {
        for (uint256 index = userCommits[recipient].length; index > 0; index--) {
            ...
            if (commitSeeds[_commitId].length != c.amount) return; // when we reach the next seeded one, stop
            batchReveal(_commitId);
        }
    }

    function getPendingUserCommits(address recipient) external view returns (MintCommit[] memory result) {
        uint256 pendingCount = 0;

        uint256[] memory pending = new uint256[](userCommits[recipient].length);
        for (uint256 index = userCommits[recipient].length; index > 0; index--) {
            uint256 _commitId = userCommits[recipient][index - 1];
            MintCommit memory c = commits[_commitId];
            if (c.revealed || c.amount == 0) break; // when we reach the next revealed one, stop
            if (commitSeeds[_commitId].length != c.amount) break; // when we reach the next seeded one, stop
            pending[pendingCount++] = _commitId;
        }

        result = new MintCommit[](pendingCount);
        for (uint256 j = 0; j < pendingCount; j++) {
            result[j] = commits[pending[j]];
        }
