...
contract PrimaryIndexToken is Initializable, AccessControlUpgradeable, ReentrancyGuardUpgradeable
{
    ...
    function setUSDCToken(address usdc) public onlyAdmin {
    ...
    function deposit(address projectToken, uint256 projectTokenAmount, address user) public isProjectTokenListed(projectToken) nonReentrant() {
        ...
        emit Deposit(user, projectToken, usdcToken, projectTokenAmount, user);
    }

    function calcAndTransferDepositPosition(address projectToken, uint256 projectTokenAmount, address user, address receiver) external isProjectTokenListed(projectToken) onlyModerator {
    ...
    function calcDepositPosition(address projectToken, uint256 projectTokenAmount, address user) public isProjectTokenListed(projectToken) onlyModerator {
        calcDepositPositionInternal(projectToken, projectTokenAmount, user);
    }

    function calcDepositPositionInternal(address projectToken, uint256 projectTokenAmount, address user) internal {
        DepositPosition storage _depositPosition = depositPosition[user][projectToken][usdcToken]; //user = msg.sender
        _depositPosition.depositedProjectTokenAmount += projectTokenAmount;
        totalDepositedProjectToken[projectToken] += projectTokenAmount;
    }
    ...
    function borrow(address projectToken, address lendingToken, uint256 lendingTokenAmount) public isProjectTokenListed(projectToken) isLendingTokenListed(lendingToken) {
        require(!primaryIndexTokenLeverage.isLeveragePosition(msg.sender, projectToken), "PIT: invalid position");
        ...
        calcBorrowPositionInternal(msg.sender, projectToken, lendingToken, lendingTokenAmount, _lendingToken); 
    ...
    function calcBorrowPosition(address borrower, address projectToken, address lendingToken, uint256 lendingTokenAmount, address currentLendingToken) external onlyModerator {
    ...
    function repay(address projectToken, address lendingToken, uint256 lendingTokenAmount, address repairer, address borrower) public isProjectTokenListed(projectToken) isLendingTokenListed(lendingToken) returns (uint256) {
        require(lendingTokenAmount > 0, "PIT: lendingTokenAmount==0");
    ...
    function repayInternal(address repairer, address borrower, address projectToken, address lendingToken, uint256 lendingTokenAmount) internal returns (uint256) {
        ...
        if (borrowPositionsAmount == 1) {
            if (lendingTokenAmount > info.bLendingToken.borrowBalanceStored(borrower) || 
                ...
                isPositionFullyRepaid = repayFullyInternal(borrower, projectToken, lendingToken);
            } else {
    ...
    function repayFullyInternal(address borrower, address projectToken, address lendingToken) internal returns(bool) {
        BorrowPosition storage _borrowPosition = borrowPosition[borrower][projectToken][lendingToken];
