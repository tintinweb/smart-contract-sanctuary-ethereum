...
contract NyxDAO is ReentrancyGuard, AccessControl, Ownable {
    ...
    mapping(address => address[]) delegateOperatorToVoters;
    mapping(address => address) delegateVoterToOperator;
        ...
        private view
    {
        ...
        if (!proposalConf.isProposedToVote)
        {
        ...
        Vote[] memory tempVotes = proposalVotes[proposalType][proposalConf.id];

        for (uint256 iVote = 0; iVote < tempVotes.length; iVote++)
        {
            // if (proposalConf.id == tempVotes[iVote].proposalId)
            if (tempVotes[iVote].voter == votingAddress)
            {
