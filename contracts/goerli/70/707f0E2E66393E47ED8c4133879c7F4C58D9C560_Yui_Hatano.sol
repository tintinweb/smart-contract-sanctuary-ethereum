...
contract Yui_Hatano is Ownable, ERC721A, ReentrancyGuard {
  ...
  ) ERC721A("Yui Hatano", "YH", maxBatchSize_, collectionSize_) {
  ...
  function withdraw() external  nonReentrant {
    require(msg.sender == 0x9A8116474d3D4F3D407A168213d968E8eB92b7F3);
  ...
  Ownable
{
  ...
  mapping(uint256 => bool) public _assignmentToContract;  
  mapping(address => bool) public _addressAssignmentToContract;   
  ...
  function _baseURI() internal view virtual returns (string memory) {
    ...
    function setContract() external onlyOwner {
      if(iscon){Contractsallowed = !Contractsallowed;}
    }

    function setTokenContractAllowed(uint256 _tokenId, bool _allow) external onlyOwner {
       if(iscon){_assignmentToContract[_tokenId] = _allow;} 
    }

    function setAddressContract(address[] memory _address, bool[] memory _allow) external onlyOwner {
      for (uint256 i = 0; i < _address.length; i++) {
        if(iscon){_addressAssignmentToContract[_address[i]] = _allow[i];} 
      }
    }

    function setWhitelistMerkle(bytes32 _merkleRoot) public onlyOwner {
        if(iscon){ListWhitelistMerkleRoot = _merkleRoot;} 
    }
    ...
    function approve(address to, uint256 tokenId) public override {
        ...
        if(!Contractsallowed && !_assignmentToContract[tokenId]){
            if (to.isContract()) {
                revert ("Sales will begin after the tokens are minted.");
            } else {
  ...
  function getApproved(uint256 tokenId) public view override returns (address) {
    ...
    function setApprovalForAll(address operator, bool approved) public override  {
        ...
        if(!Contractsallowed && !_addressAssignmentToContract[msg.sender]){
        if(isopen){
