...
contract NFT is ERC721("Test ERC721 Token", "TEST") {
    ...
    string private baseTokenUri = "https://assets.degendwarfs.io/json/";     

    /// @notice Mint a TEST NFT directly into a testers address
    /// @param ownerWallet The address that will receive the mint
    function mint(address ownerWallet) external {
        _safeMint(ownerWallet, _tokenId.current());
        _tokenId.increment();
    }
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        return string(abi.encodePacked(_baseURI(), StringConverter.toString(tokenId), ".json"));
    }
}
