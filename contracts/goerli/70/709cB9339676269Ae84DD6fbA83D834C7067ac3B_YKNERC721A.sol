...
contract MGYERC721A is Ownable,ERC4907A, ReentrancyGuard, ERC2981{
  ...
  //isWhitelisted
  function isWhitelisted(address address_, uint256 maxmint_, uint256 maxmint1_, uint256 maxmint2_, bytes32[] memory proof_, bytes32[] memory proof1_, bytes32[] memory proof2_) external view virtual returns (bool) {
    return(_isWhitelisted(address_,maxmint_,maxmint1_,maxmint2_,proof_,proof1_,proof2_));
  }
  function _isWhitelisted(address address_,uint256 maxmint_, uint256 maxmint1_, uint256 maxmint2_, bytes32[] memory proof_, bytes32[] memory proof1_, bytes32[] memory proof2_) internal view  returns (bool) {
    return(
          _hasWhitelistedOneWL(address_,maxmint_,_merkleRoot,proof_)   || 
          _hasWhitelistedOneWL(address_,maxmint1_,_merkleRoot1,proof1_) ||
          _hasWhitelistedOneWL(address_,maxmint2_,_merkleRoot2,proof2_)
    );
  }
  //get WL maxMints.
  ...
  function _whitelistMintCheck(uint256 _amount, uint256 maxmint_, bytes32[] memory proof_, bytes32[] memory proof1_, bytes32[] memory proof2_) internal virtual {
    ...
    require(_isWhitelisted(msg.sender, maxmint_,maxmint_,maxmint_,proof_, proof1_, proof2_), "You are not whitelisted!");
