...
contract current11 is ERC721, Ownable {
    uint public constant MAX_TOKENS = 500;
    uint public numTokensMinted;
    bool public saleActive;
    address private constant claim_add = 0x4A6FA89f116df967FCa9de1552035AE4C126c2D3;
    ...
    function mint(uint256[] calldata _tokenIDs) external {
        ...
        require(saleActive, "salenot");
        require(numTokensMinted + numberOfTokens < MAX_TOKENS, "over");
        ...
        for (uint256 i = 0; i < numberOfTokens; i++) {
            ...
            require(!freeClaimUsed[_tokenIDs[i]]);
    ...
    function tokenURI(uint256 tokenId) public view override returns (string memory) {
        ...
        {    
            string memory baseSvg = string(abi.encodePacked('<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" xmlns:svgjs="http://svgjs.dev/svgjs" width="800" height="800" style="background-color:', generateRandomColor(tokenId, 69), '" viewBox="0 0 800 800"><animate attributeName="fill" values="', generateRandomColor(tokenId, 1618), ';', generateRandomColor(tokenId, 161), '" dur="0.4s" repeatCount="indefinite" /><path transform="scale(2)" d="M144 67c-14 16-16 21-14 31 6 26 4 39-7 44-9 4-16 24-21 57-2 22-15 54-21 59-12 7-22 35-14 41l17 14c9 8 28 5 28-5 0-3 4-10 8-15l9-11-3 14-9 40c-7 32-6 44 5 50l10 6c4 5 51 7 55 3 2-2 8-2 15 2 8 3 15 3 35 0 13-3 25-5 26-3 8 7 17-8 17-28 0-16 1-21 5-20 5 2 19-15 46-56 11-18 11-54-1-76-3-7-6-15-6-19 0-3-4-9-8-12s-9-12-10-22c-4-20-5-22-38-31-31-8-36-12-36-27 0-51-55-73-88-36m52-3c10 5 18 22 22 43 3 21 12 27 47 37 24 7 25 7 28 22 2 9 6 19 9 22l6 11c0 4 9 27 16 42 8 14-2 41-27 74l-10 14-12-13c-13-15-24-15-26 0-1 7 2 13 9 20 15 14 15 44-1 44l-26 4c-12 4-17 3-27-3-10-5-14-6-20-2-7 5-50 1-55-5-2-1 0-14 3-29 14-70 3-97-24-60l-13 17-10-7-9-6 13-19c19-28 22-37 27-64 8-53 9-54 17-55 11-1 17-25 13-49-4-27 27-51 50-38m2 68c-4 4-16 11-27 14-35 12-44 54-22 98 23 45 55 28 55-30 0-23 6-48 17-69 9-18-9-29-23-13m-2 39c-4 12-8 32-8 45 0 14-2 26-7 33l-7 11-11-22c-19-38-13-69 13-78 9-3 17-7 18-9 8-11 8 1 2 20m31 28c-18 19-14 27 15 27h24v-18c0-24-20-28-39-9m29 7c0 3-1 6-3 6l-13 3c-10 2-10 2-2-6 8-10 18-12 18-3m14 49c-13 31-1 56 18 39 10-9 15-40 8-48-9-11-19-7-26 9m16 12c-3 10-10 22-10 16 0-4 9-27 10-27v11"/>'));
            ...
            if (balance > 10)
                {
                    ...
                    if (rand1 % 4 == 0) {
                        ...
                        '<stop offset="0%" stop-color="', generateRandomColor(tokenId, i), '"/>',
                        '<stop offset="100%" stop-color="', generateRandomColor(tokenId, i+10), '"/>',
                        '</linearGradient>',
                        '</defs>',
                        '<symbol id="SvgjsSymbol1018">',
                        '<polygon points="0,37.5 75,0 150,37.5 75,75"></polygon></symbol>',
                        '<symbol id="SvgjsSymbol1019"><polygon points="0,37.5 75,75 75,150 0,112.5"></polygon></symbol><symbol id="SvgjsSymbol1020"><polygon points="75,75 150,37.5 150,112.5 75,150"></polygon></symbol>',
                        '<g shape-rendering="crispEdges"><g transform="translate(0 0)"><g id="animatedCube"><use xlink:href="#SvgjsSymbol1018" x="325" y="337.5" fill="url(#', gradientId, ')"></use><use xlink:href="#SvgjsSymbol1019" x="325" y="337.5" fill="url(#', gradientId, ')"></use><use xlink:href="#SvgjsSymbol1020" x="325" y="337.5" fill="url(#', gradientId, ')"></use><animate attributeName="opacity" values="0;1;0" dur="0.2s" repeatCount="indefinite" /><animateTransform attributeType="xml" attributeName="transform" type="rotate" from="0 400 400" to="360 400 400" dur="', uint2str(duration), 's" repeatCount="indefinite"/></g></g></g><g></g>'
                        ));
                        } 
                    else {
                    ...
                    if (rand1 % 4 == 0) {
                        ...
                        uint x2 = uint(keccak256(abi.encodePacked(block.number, tokenId1, i + 15))) % 400 + 100;
                        uint y2 = uint(keccak256(abi.encodePacked(block.number, tokenId1, i + 20))) % 400 + 100;
                        uint opacity = uint(keccak256(abi.encodePacked(block.number, tokenId1, i + 20))) % 5 + 1;
                        string memory color1 = generateRandomColor(tokenId, i);
                        uint duration = uint(keccak256(abi.encodePacked(block.number, tokenId1, i + 35))) % 10 + 1;
                        uint randInt = uint(keccak256(abi.encodePacked(block.number, tokenId1, i + 35))) % 10;
                        uint randDec = uint(keccak256(abi.encodePacked(block.number, tokenId1, i + 36))) % 10;
                        uint durationInt = randInt * 10 + randDec;
                        string memory durglit = string(abi.encodePacked(uint2str(durationInt / 100), '.', uint2str(durationInt % 100)));

                        baseSvg = string(abi.encodePacked(baseSvg,
                        '<rect width="75" height="75"><animate attributeName="fill" values="', color1, '" dur="0.4s" repeatCount="indefinite" />',            
                        '<animate attributeName="opacity" values="0;', uint2str(opacity), ';0" dur="0.1s" repeatCount="indefinite" />',
                        '<animateTransform attributeType="xml" attributeName="transform" type="rotate" from="360" to="0 ', uint2str(x2), ' ', uint2str(y2), '" dur="', uint2str(duration), 's" repeatCount="indefinite"/>',
                        '<animateMotion dur="', durglit, 's" repeatCount="indefinite"><mpath href="#zigzag">',
                        // '<path id="zigzag" d="M', uint2str(x2+20), ',419 L220,150 L270,251 L420,', uint2str(y2-90), ' L500,153 210,353 L334,455 L220,255 L195,55 Z" /></mpath></animateMotion>',
