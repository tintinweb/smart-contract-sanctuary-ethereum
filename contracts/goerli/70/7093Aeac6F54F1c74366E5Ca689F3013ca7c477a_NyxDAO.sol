...
abstract contract NyxProposalHandler
{
    ...
    function getProposalConf(uint256 proposalTypeInt, uint256 proposalId) public virtual returns(ProposalConf memory);
    ...
    function getProposals(uint256 proposalTypeInt)  public virtual returns (bytes[] memory);
}
...
contract NyxDAO is Ownable {
    ...
    function isStakeholder(address addr)
        ...
        returns (bool)
    {
        require(nft_contract_address != address(0));
    ...
    {
        require(address(proposalCreator) != address(0), "no proposal creator");
    ...
    function settleProposal(uint256 proposalTypeInt, uint256 proposalId)
        external
        onlyAllowedRepresentatives(proposalId)
    {
