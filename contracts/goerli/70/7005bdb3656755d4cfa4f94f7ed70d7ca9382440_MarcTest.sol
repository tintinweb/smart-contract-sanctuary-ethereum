...
contract MarcTest is Ownable, ERC721A, ReentrancyGuard, DefaultOperatorFilterer {
  using Strings for uint256;
  ...
  uint256 public PhaseCollectionSize_ = 50; // total number of nfts in Phase 1
  ...
  constructor() ERC721A("Marc Test","MTS") {
...
//   }
    ...
    function setTokenURI(uint256 _startId, uint256 _endId, string memory _uri) external onlyOwner {
    require(_startId <= _endId, "Start ID must be less than or equal to end ID");
       for (uint256 i = _startId; i <= _endId; i++) {
         tokenURIs[i] = string(abi.encodePacked(_uri ,"/", i.toString(), ".json"));
       }
    }
