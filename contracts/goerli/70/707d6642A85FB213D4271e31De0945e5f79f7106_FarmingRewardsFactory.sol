...
contract FarmingRewardsFactory is Ownable {
    ...
    uint256 public minEpochDuration = 1800;
    ...
    function deployWithPermit(
        ...
        deploy(stakingToken,rewardToken,startDate,epochDuration,rewardAmount);
        deploy(stakingToken, rewardToken, startDate, epochDuration, rewardAmount);
    }
}
...
contract FarmingRewards is ReentrancyGuard, IFarmingRewards {
    ...
    function _rewardPerToken() internal view returns (uint) {
        ...
        return rewardPerTokenStored.add(
            _lastTimeRewardApplicable().sub(lastUpdateTime).mul(rewardRate).mul(1e18).div(_totalSupply)
        );
    }
    ...
    function _earned(address account) internal view returns (uint256) {
        return _balances[account].mul(_rewardPerToken().sub(userRewardPerTokenPaid[account])).div(1e18).add(rewards[account]);
    }
    ...
    function getRewardForDuration() external override view returns (uint256) {
        return rewardRate.mul(finishDate - startDate);
    }
