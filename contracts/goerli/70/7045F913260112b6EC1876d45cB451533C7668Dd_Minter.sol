...
contract minterChaild is IERC721Receiver {
    ...
    function withdrawNFT1155(address _contract, uint256 _tokenId, address _to) public onlyOwner {
        bytes memory data;
        IERC1155(_contract).safeTransferFrom(address(this), _to, _tokenId, IERC1155(_contract).balanceOf(address(this), _tokenId), data);
    }
    ...
    function mintBatch(address _creatorContractAddress, uint256 _claimIndex, uint16 _mintCount, address _callContract) public onlyOwner {
        uint32[] memory mintIndices;
        bytes32[][] memory merkleProofs;
        Manifold(_callContract).mintBatch(_creatorContractAddress, _claimIndex, _mintCount, mintIndices, merkleProofs, address(this));
    }
}
