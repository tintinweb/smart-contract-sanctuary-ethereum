...
contract DoodMarketplace is ReentrancyGuard, Ownable {
    ...
    function createMarketSaleWithToken(
        ...
        require(
            IERC20(priceAsset).balanceOf(msg.sender) >= price,
        ...
        require(
            !idToMarketItem[itemId].isSGB,
            "User could buy this item by SGB"
        );

        require(!idToMarketItem[itemId].isSold, "This item is already sold!");

        IERC20(priceAsset).transferFrom(msg.sender, address(this), price);
        IERC20(priceAsset).transfer(
            idToMarketItem[itemId].seller,
            (price * (100 - feePercent)) / 100
        );

        IERC20(priceAsset).transfer(ownerAddr, (price * feePercent) / 100);
    ...
    function createMarketSaleWithSGB(address nftContract, uint256 itemId)
        public
        payable
        nonReentrant
    {
        ...
        require(msg.value == price, "Balance of buyer is less than price");
        ...
        require(
            idToMarketItem[itemId].isSGB,
            "User could buy this item by Token"
        );

        require(!idToMarketItem[itemId].isSold, "This item is already sold!");

        payable(idToMarketItem[itemId].seller).sendValue(
            (price * (100 - feePercent)) / 100
        );

        payable(ownerAddr).sendValue((price * feePercent) / 100);
