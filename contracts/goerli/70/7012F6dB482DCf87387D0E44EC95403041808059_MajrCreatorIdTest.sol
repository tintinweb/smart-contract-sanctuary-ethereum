...
/*
 * @title MajrCreatorIdTest
 * @author MAJR, Inc.
 * @notice This contract uses ERC721PartnerSeaDrop, an ERC721A token contract that is compatible with SeaDrop.
 */
contract MajrCreatorIdTest is ERC721PartnerSeaDrop {
  /**
   * @notice Deploy the NFT contract with its name, symbol, administrator, and allowed SeaDrop addresses.
   */
  constructor(
    string memory name,
...
import { TwoStepAdministered } from "./TwoStepAdministered.sol";
...
contract ERC721PartnerSeaDrop is ERC721SeaDrop, TwoStepAdministered {
  ...
  function mintSeaDrop(
    address minter,
    uint256 quantity
  ) external payable override onlyAllowedSeaDrop(msg.sender) {
    ...
    onlyAllowedSeaDrop(seaDropImpl)
  {
    ...
    if (msg.sender != administrator) {
      ...
      uint16 maxTotalMintableByWallet = retrieved.maxTotalMintableByWallet;
    ...
    onlyAllowedSeaDrop(seaDropImpl)
  {
    ...
    if (msg.sender != administrator) {
      ...
      retrieved.maxMaxTotalMintableByWallet = maxMaxTotalMintableByWallet > 0
        ? maxMaxTotalMintableByWallet
        : 1;
...
contract TwoStepAdministered is TwoStepOwnable {
  ...
  function _transferAdministration(address newAdministrator) internal virtual {
