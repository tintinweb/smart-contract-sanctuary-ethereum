...
contract NFTMarketplace is ReentrancyGuard, Ownable {
    ...
    function createBulkMarketSale(
        ...
        require(_conductKeys.length == _itemIds.length, "Please input the same length's info");
        uint256 index;
        uint256 sum = 0;
        ...
        for (index = 0; index < _nftContracts.length; index++) {
            createMarketSale(_nftContracts[index], _tokenIds[index], _itemIds[index], _status[index], _sellers[index], _prices[index], _conductKeys[index]);
        }
    }
