...
contract ShibnobiStakingNFTs is Ownable, IERC721Receiver {
    ...
    mapping(uint256 => address) public nftStakes;
    ...
    function deposit(uint256[] calldata _nftIds) external {
        ...
        if (_nftIds.length > 0) {
            for (uint256 i = 0; i < _nftIds.length; i++) {
                require(nftStakes[_nftIds[i]] == address(0), "NFT already staked");
                stakingToken.transferFrom(address(msg.sender), address(this), _nftIds[i]);
                nftStakes[_nftIds[i]] = msg.sender;
                userNFTs[msg.sender].push(_nftIds[i]);
            }
            ...
            //update balances
            user.amount += _nftIds.length;
            totalStaked += _nftIds.length;
            emit Deposit(msg.sender, _nftIds.length);
        }
    ...
    function withdraw(uint256[] calldata _nftIds) external {
        ...
        if (_nftIds.length > 0) {
            user.amount -= _nftIds.length;
            for (uint256 i = 0; i < _nftIds.length; i++) {
                require(nftStakes[_nftIds[i]] == msg.sender, "Not your NFT");
                stakingToken.transferFrom(address(this), address(msg.sender), _nftIds[i]);
                nftStakes[_nftIds[i]] = address(0);
                for (uint256 j = 0; j < userNFTs[msg.sender].length; j++) {
                    if (userNFTs[msg.sender][j] == _nftIds[i]) {
                        userNFTs[msg.sender][j] = userNFTs[msg.sender][userNFTs[msg.sender].length - 1];
            ...
            totalStaked -= _nftIds.length;
            emit Withdraw(msg.sender, _nftIds.length);
    ...
    function apy() public view returns (uint256) {
        ...
        return rewardPerSecond * 86400 * 365 / totalStaked;
    }

    function info() external view returns (uint256, uint256, uint256, address, address, uint8) {
        return (
            totalStaked,
            usersStaking,
            apy(),
