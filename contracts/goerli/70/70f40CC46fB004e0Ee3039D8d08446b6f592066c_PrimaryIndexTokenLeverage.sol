...
contract PrimaryIndexTokenLeverage is Initializable, AccessControlUpgradeable, ReentrancyGuardUpgradeable
{
    ...
    function deferLiquidityCheck(address user, address projectToken, address lendingToken) internal view {
    ...
    function nakedBorrow(address user, address lendingToken, uint lendingTokenAmount, address projectToken, address currentLendingToken) internal {
    ...
    function approve(address token, address tokenTransferProxy, uint tokenAmount) internal {
    ...
    function collateraliseLoan(address user, address projectToken, uint collateralTokenCount, uint marginCollateralCount) internal returns(uint totalCollateral, uint addingAmount){
    ...
    function checkIsValidPosition(address user, address projectToken, address lendingToken, uint marginCollateralAmount) internal view {
    ...
    function checkSlippage(uint collateralTokenCount, uint maxSlippage, uint amountRecive) internal pure {
    ...
    function leveragedBorrow(address projectToken, address lendingToken, uint notionalExposure, uint marginCollateralAmount, bytes memory buyCalldata, uint8 leverageType) public nonReentrant{
        leveragedBorrowInternal(projectToken, lendingToken, notionalExposure, marginCollateralAmount, buyCalldata, msg.sender, leverageType);
    }
    ...
    function leveragedBorrowFromRelatedContract(address projectToken, address lendingToken, uint notionalExposure, uint marginCollateralAmount, bytes memory buyCalldata, address borrower, uint8 leverageType) public nonReentrant onlyRelatedContracts() {
        leveragedBorrowInternal(projectToken, lendingToken, notionalExposure, marginCollateralAmount, buyCalldata, borrower, leverageType);
    }

    function leveragedBorrowInternal(address projectToken, address lendingToken, uint notionalExposure, uint marginCollateralAmount, bytes memory buyCalldata, address borrower, uint8 leverageType) public {
        ...
        checkIsValidPosition(borrower, projectToken, lendingToken, marginCollateralAmount);
        ...
        nakedBorrow(borrower, lendingToken, lendingTokenCount, projectToken, currentLendingToken);

        approve(lendingToken, tokenTransferProxy, lendingTokenCount);
        ...
        (uint totalCollateral, uint addingAmount) = collateraliseLoan(borrower, projectToken, amountRecive, marginCollateralAmount);

        deferLiquidityCheck(borrower, projectToken, lendingToken);
