...
import {IGovernanceCore, IL1VotingStrategy, CrossChainMandateUtils} from '../interfaces/IGovernanceCore.sol';
  ...
  OwnableWithGuardian
{
  ...
  ///      with a message too big. It has been put at 10 because its a limit that will not be reached in
  ///      the near future, as we currently have AAVE, stkAAVE, stkABPT, aAAVE, and provably some others
  ///      from balancer, curve, etc
  ...
  /// @inheritdoc IGovernanceCore
  function addVotingPortals(address[] calldata votingPortals)
    external
    onlyOwner
  {
    _updateVotingPortals(votingPortals, true);
  ...
  /// @inheritdoc IGovernanceCore
  function removeVotingPortals(address[] calldata votingPortals)
    external
    onlyOwner
  {
  ...
  function voteViaBridge(
    ...
    Proposal storage proposal = _proposals[proposalId];
    require(_getProposalState(proposal) == State.Active, 'NOT_IN_ACTIVE_STATE');

    require(
      votingTokens.length < VOTING_TOKENS_CAP,
      'TO_MANY_TOKENS_FOR_VOTING'
    );

    // TODO: Think if there should be a cap also on times you can bridge. to not drain.
    // for this makes sense to be sure on the payment system of the bridges to see vulnerabilities
    // on sybil attack
    ...
    emit VoteForwarded(proposalId, msg.sender, support, votingTokens);
  }
  ...
  function queueProposal(
    ...
    if (
      ...
      _isPassingQuorum(votingConfig, forVotes) &&
  ...
  // TODO: add external method _getProposalState
   ...
   */
  function _isPassingQuorum(VotingConfig memory votingConfig, uint256 forVotes)
    internal
    pure
    returns (bool)
  {
    return forVotes > votingConfig.quorum * PRECISION_DIVIDER;
  }
   ...
   */
  function _updateVotingPortals(address[] calldata votingPortals, bool state)
    internal
  {
