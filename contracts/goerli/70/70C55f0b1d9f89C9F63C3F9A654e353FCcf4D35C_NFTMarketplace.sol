...
contract NFTMarketplace is ReentrancyGuard, Ownable {
    ...
    function fetchMarketItems(address _sellerAddress, uint256 _status) public view returns (MarketItem[] memory) {
        ...
        for (uint256 i = 0; i < itemCount; i++) {
            if (keccak256(abi.encodePacked(_sellerAddress)) == keccak256(abi.encodePacked(""))) {
                ...
                if (uint256(idToMarketItem[i + 1].status) == _status && keccak256(abi.encodePacked(idToMarketItem[i + 1].seller)) == keccak256(abi.encodePacked(_sellerAddress))) {
                    uint256 currentId = i + 1;
                    MarketItem storage currentItem = idToMarketItem[currentId];
                    items[currentIndex] = currentItem;
                    currentIndex += 1;
