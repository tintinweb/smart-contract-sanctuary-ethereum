...
contract SpaceRander is ERC721, ERC721Burnable, Ownable {
    IStarknetCore starknetCore;

    uint private _seed = 100;
    ...
    uint256 constant SEND_BACK = 20230223;
    ...
    function _mintNftMeta(uint256 tokenQuantity) internal {
        for (uint256 i = 0; i < tokenQuantity; i++) {
            ...
            if (totalSupply() < MAX_SUPPLY) {
                ...
                _token_earn_tm[mintIndex] = block.timestamp;
    ...
    function send721ToL2(uint256 l2ContractAddress, uint256 l2UserAddress, uint256 tokenId) external {
        ...
        uint256 lucky = getLucky(tokenId);
        uint256 earn_tm = getLastEarnTm(tokenId);
        
        _burn(tokenId);

        // Prepare the payload
        uint256[] memory registerPayload = new uint256[](7);
        ...
        registerPayload[4] = lucky % (2**128);
        registerPayload[5] = lucky / (2**128);
        registerPayload[6] = earn_tm;
    ...
    /// @param tokenId        ID of the token
    function retrieve721(uint256 l2ContractAddress, uint256 tokenId, uint256 lucky, uint256 earn_tm) external {
        uint256[] memory rcvPayload = new uint256[](7);
        rcvPayload[0] = SEND_BACK;
        rcvPayload[1] = tokenId % (2**128);
        rcvPayload[2] = tokenId / (2**128);
        rcvPayload[3] = uint256(uint160(msg.sender));
        rcvPayload[4] = lucky % (2**128);
        rcvPayload[5] = lucky / (2**128);
        rcvPayload[6] = earn_tm;
        ...
        _receiveFromL1ReMint(msg.sender, tokenId, lucky, earn_tm);
    }

    function _receiveFromL1ReMint(address to, uint256 tokenId, uint256 lucky, uint256 earn_tm) internal virtual {
