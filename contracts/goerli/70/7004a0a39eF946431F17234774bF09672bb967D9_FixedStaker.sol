    ...
    Ownable, ReentrancyGuard
{
  ...
  */
  ...
  /// A mapping of potential stakers to their ban status.
  ...
  */
  ...
  /**
    Construct a new fixed-emissions staker by providing it a name and the token
    to disburse.
  ...
  */
  function getPosition (
    ...
    Position memory position = positions[_id][_staker];
    return (position.stakedItems, position.tokenPaid);
  }
  ...
  */
  ...
  /**
    Claim all of the `_claimant`'s pending tokens from the specified pools.

    @param _claimant The address to claim all pending tokens for.
    @param _poolIds The IDs of the pools to claim pending token rewards from.
  */
  function _claim (
    address _claimant,
    uint256[] memory _poolIds
  ) private nonReentrant returns (uint256 totalClaimAmount) {
    //uint256 totalClaimAmount;
    for (uint256 poolIndex; poolIndex < _poolIds.length; ++poolIndex) {
      uint256 poolId = _poolIds[poolIndex];
      Pool storage pool = pools[poolId];
      Position storage position = positions[poolId][_claimant];
      ...
      */
      for (uint256 i; i < position.stakedItems.length; ++i) {
        ...
        /*
          ...
          that has been earned by each staked item in this pool.
        */
        ItemStatus storage status = itemStatuses[pool.item][
            stakedItemId
        ];
        uint256 lockEnds = status.stakedAt + pool.lockDuration;
        uint256 totalEarnings;
    ...
    //IERC20(token).safeTransfer(_claimant, totalClaimAmount);

    // Emit an event.
    emit Claim(block.timestamp, _claimant, _poolIds, totalClaimAmount);
  }
  ...
  */
  function claim (
    ...
    uint256 claimed = _claim(_msgSender(), _poolIds);
    if (claimed > 0) {
      IERC20(token).safeTransfer(_msgSender(), claimed);
    }
  }
  ...
  */
  ...
  //       // Reject banned callers.
  //       if (bans[_msgSender()]) {
  //           revert CannotStakeIfBanned();
  //       }

  //       // Reject empty claim attempts.
  ...
  //       // Claim pending tokens from the pool being staked into.
  ...
  //           Pool storage oldPool = pools[status.stakedPool];
  //           uint256 lockEnds = status.stakedAt + oldPool.lockDuration;
  //           bool itemUnlocked = block.timestamp > lockEnds;
  //           /*
  //             We must claim pending tokens before removing any item from the staked
  //             items array in its current position, lest claiming for that pool no
  //             longer properly account for this item.
  //           */
  //           claim(_asSingletonArray(status.stakedPool));

  //           // Remove the token from the staked item array of its old position.
  //           Position storage oldPosition = positions[status.stakedPool][
  //               _msgSender()
  //           ];
  //           for (
  //               uint256 stakedIndex;
  //               stakedIndex < oldPosition.stakedItems.length;
  //               ++stakedIndex
  //           ) {
  //               uint256 stakedId = oldPosition.stakedItems[stakedIndex];
  //               if (tokenId == stakedId) {
  //                   // Remove the element at the matching index.
  //                   for (
  //                       uint256 r = stakedIndex;
  //                       r < oldPosition.stakedItems.length - 1;
  //                       ++r
  //                   ) {
  //                       oldPosition.stakedItems[r] = oldPosition.stakedItems[
  //                           r + 1
  //                       ];
  //                   }
  //                   oldPosition.stakedItems.pop();
  //                   break;
  //               }
  //           }
  ...
  //           /*
  //             If the item being staked is not already transfer-locked, then lock
  ...
  */
  function stake (
    ...
    // Claim pending tokens from the pool being staked into.
    totalClaimed += _claim(_msgSender(), _asSingletonArray(_poolId));
    // Initialize helper array for store pool ids at which we already claimed rewards.
    uint256[] memory poolIdsClaimed = new uint256[](_tokenIds.length + 1);
    uint256 claimesCounter;
    poolIdsClaimed[0] = _poolId;
    ++claimesCounter;
    ...
    for (uint256 i; i < _tokenIds.length; ++i) {
      ...
      */
      ...
      uint256 oldPoolId = status.stakedPool;
      ...
      */
      // Remove the token from the staked item array of its old position.
      Position storage oldPosition = positions[oldPoolId][_msgSender()];
      ...
      */
      ...
      if (_poolId != oldPoolId) {
        position.stakedItems.push(tokenId);
        // Check that tokenId is already staked in some pool.
        if (oldPoolId != 0) {
          bool claimed;
          // Check for prevent double claiming from same pool.
          for (uint256 j; j < claimesCounter; ++j) {
            if (poolIdsClaimed[j + 1] == oldPoolId) {
              claimed = true;
              break;
            }
            if (poolIdsClaimed[j + 1] == 0 && !claimed) {
              poolIdsClaimed[claimesCounter] = oldPoolId;
              ++claimesCounter;
              break;
            }
          }
          ...
          for (
            ...
            // uint256 stakedId = oldPosition.stakedItems[stakedIndex];
            if (tokenId == oldPosition.stakedItems[stakedIndex]) {
              // Remove the element at the matching index.
              if (stakedIndex != oldPosition.stakedItems.length - 1) {
                oldPosition.stakedItems[stakedIndex] = oldPosition
                  .stakedItems[oldPosition.stakedItems.length - 1];
              }
              oldPosition.stakedItems.pop();
              break;
            }
    ...
    if (totalClaimed > 0) {
      IERC20(token).safeTransfer(_msgSender(), totalClaimed);
    }
    ...
    for (uint256 i = 0; i < _tokenIds.length; ++i) {
      ...
      */
      ...
      for (
        ...
        //uint256 stakedId = position.stakedItems[stakedIndex];
        if (tokenId == position.stakedItems[stakedIndex]) {
          // Remove the element at the matching index.
          if (stakedIndex != position.stakedItems.length - 1) {
            position.stakedItems[stakedIndex] = position
              .stakedItems[position.stakedItems.length - 1];
          }
  ...
  */
  function evict (
    ...
    Pool storage pool = pools[_poolId];
    Position storage position = positions[_poolId][_evictee];

    // Set the ban status of the evictee.
    bans[_evictee] = _ban;

    // Claim pending tokens.
    if (!_skipClaim) {
      _claim(_evictee, _asSingletonArray(_poolId));
    }
    ...
    for (uint256 i = 0; i < _tokenIds.length; ++i) {
      uint256 tokenId = _tokenIds[i];
      // Verify that the evictee owns the token being unlocked.
      if (item.ownerOf(tokenId) != _evictee) {
        revert CannotWithdrawUnownedItem();
      }
    ...
    // Emit an event.
    emit Withdraw(block.timestamp, _evictee, _poolId, pool.item, _tokenIds);
  }
