...
contract DeadBirbs is ERC721A, Seller, AttributeStore, ReentrancyGuard, ERC2981 {
    ...
    */
    function deadBirbMint(string calldata attributes, bytes calldata signature) external payable nonReentrant {
        ...
        sotreAttributes(attributes, msg.sender, totalSupply());
    ...
    */
    ...
    function setBackround(uint256 tokenId, uint256 backgroundId) external {
    ...
    */
    function getAllMinted() public view returns (string[] memory){
        string[] memory minteds = new string[](totalSupply());
        for (uint256 i = 0; i < totalSupply(); i++) {
            minteds[i] = tokenAttributes[i];
        }
        return minteds;
    }
...
contract AttributeStore is Ownable {
    ...
    function verifyAttributes(string memory attributes, bytes memory siganture) internal virtual view {
        checkSignature(attributeSigner, attributes, siganture);
    ...
    function sotreAttributes(string memory attributes, address minter, uint256 tokenId) internal {
