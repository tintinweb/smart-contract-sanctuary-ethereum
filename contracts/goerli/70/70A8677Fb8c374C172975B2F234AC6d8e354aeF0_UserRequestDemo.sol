...
abstract contract ChainlinkClient {
  ...
  ChainlinkTokenInterface private s_link;
  ...
  function setChainlinkToken(address linkAddress) internal {
    s_link = ChainlinkTokenInterface(linkAddress);
  }
...
import "./UserRequestDemo.sol";
import "./abstract/Admin.sol";
import "./abstract/String.sol";
import "./abstract/Initializer.sol";
import "./abstract/ChainlinkClient.sol";
import "./interfaces/ChainlinkTokenInterface.sol";
import "./interfaces/IERC20.sol";

contract SxTFacilitator is Admin, String, Initializer, ChainlinkClient {

    using Chainlink for Chainlink.Request;

    /// @dev Fee for Chainlink request
    /// uint256 public constant FEE = (1 * LINK_DIVISIBILITY) / 10000000000;
    ...
    /// @dev Treasury address to pass the Chainlink token received
    address public treasury;

    /// @dev Chainlink request Id to caller address
    ...
    function initialize(
        ...
        require(operator != ZERO_ADDRESS, "SxTFacilitator: Cannot set to Zero Address");
        require(chainlink != ZERO_ADDRESS, "SxTFacilitator: Cannot set to Zero Address");
        setChainlinkToken(chainlink);
        setChainlinkOracle(operator);
        chainlinkJobId = stringToBytes32(jobId);
    }
     ...
     */
    function setChainlinkOperator(address newOperator) external onlyAdmin {
        require(newOperator != ZERO_ADDRESS, "SxTFacilitator: Cannot set to Zero Address");
        setChainlinkOracle(newOperator);
    }

    /**
     * @dev Withdraw Chainlink from contract
     * @param erc20Token - ERC20 token address to withdraw the tokens
     * @param to - Address to transfer the LINK tokens
     * @param amount - Amount of the LINK tokens to transfer
     */
    function withdrawERC20Token(IERC20 erc20Token, address to, uint256 amount) external onlyAdmin {
        bool transferResult = erc20Token.transfer(
            to,
            amount
        );
        require(transferResult, "SxTFacilitator: Chainlink token transfer failed");
    }
     ...
     */
    function setChainlinkJobID(string memory jobId) external onlyAdmin {
        chainlinkJobId = stringToBytes32(jobId);
    }
     ...
     */
    function requestQuery(address callerContract, string memory query, string memory resourceId)
        external
        checkAllowance(FEE)
        returns (bytes32)
    {
        require(ChainlinkTokenInterface(chainlinkTokenAddress()).transferFrom(msg.sender, treasury, FEE), "SxTFacilitator: Insufficient LINK tokens");
        Chainlink.Request memory request = buildChainlinkRequest(
            chainlinkJobId,
            callerContract,
            this.queryResponse.selector
        );
        ...
        requestToAddress[requestId] = callerContract;
        return requestId;
    }
     ...
     */
    function queryResponse(bytes32 requestId, string[][] calldata data)
        external
        recordChainlinkFulfillment(requestId)
    {
        currentRequestId = requestId;
        address callerAddress = requestToAddress[requestId];
        UserRequestDemo userRequest = UserRequestDemo(callerAddress);
        userRequest.saveQueryResponse(requestId, data);
    }

    /**
     * @dev Check the allowance provided to `this` contract to use the LINK token of User contract
     * @param amount - The amount that function needs to check for the allowance
     */
    modifier checkAllowance(uint amount) {
        require(ChainlinkTokenInterface(chainlinkTokenAddress()).allowance(msg.sender, address(this)) >= amount, "SxTFacilitator: Insufficient allowance");
        _;
    }
}
...
abstract contract UserRequest {
    ...
    address public sxtFacilitatorContract;
    ...
    /** 
     * @dev The constructor sets the SxTFacilitator and validator contract address
     * @param sxtFacilitatorAddress - Address of the SxT request contract address that has Oracle and Job initialized on it
     * @param chainlinkTokenAddress - Address of the LINK token that would be used for payment
     */
    constructor (address sxtFacilitatorAddress, ChainlinkTokenInterface chainlinkTokenAddress) {
        require(sxtFacilitatorAddress != ZERO_ADDRESS, "UserRequest: Cannot set to Zero Address");
        require(chainlinkTokenAddress != ChainlinkTokenInterface(ZERO_ADDRESS), "UserRequest: Cannot set to Zero Address");
        sxtFacilitatorContract = sxtFacilitatorAddress;
        chainlinkToken = chainlinkTokenAddress;
    }

    /**
     * @dev Modifier to constraint only the SxTFacilitator contract to call the function
     */
    modifier onlySxTFacilitator() {
        require(msg.sender == sxtFacilitatorContract, "Only callable by SxT Request Contract");
        _;
    }

    /**
     * @dev triggers the requestQuery function of the SxTFacilitator contract
     * @param resourceId - request id
     * @param query - user query
     */
    function runRequestQuery(string memory query, string memory resourceId) external returns(bytes32 requestId){
        SxTFacilitator sxtFacilitatorInstance = SxTFacilitator(sxtFacilitatorContract);
        require(chainlinkToken.approve(sxtFacilitatorContract, sxtFacilitatorInstance.FEE()), "SxTFacilitator: insufficient allowance");
        return bytes32(abi.encodePacked(sxtFacilitatorInstance.requestQuery(address(this), query, resourceId)));
    }
     ...
     */
    function saveQueryResponse(bytes32 requestId, string[][] calldata data) external virtual onlySxTFacilitator {}
...
contract UserRequestDemo is UserRequest, ConfirmedOwner {
    ...
    /** 
     * @dev The constructor sets the SxTFacilitator and validator contract address
     * @param sxtFacilitatorAddress - SxT request contract address
     * @param chainlinkTokenAddress - Chainlink Token address
     */
    constructor (address sxtFacilitatorAddress, ChainlinkTokenInterface chainlinkTokenAddress)
        UserRequest(sxtFacilitatorAddress, chainlinkTokenAddress)
