...
contract RoseInvation is ERC721Enumerable, Ownable, AccessControl, ReentrancyGuard {
    ...
    uint256 public immutable maxWhitelistSupply;
    uint256 public whitelistMinted;
    bytes32 public merkleRoot;

    uint256 public immutable maxOneAccountMint;
    mapping(address => uint256) public amountNFTsPerMint;
    ...
    constructor(string memory name_, string memory symbol_, string memory revealingURI_) ERC721(name_, symbol_) {

        maxSupply = 2222;
        maxWhitelistSupply = 2000;
        maxOneAccountMint = 1;
    ...
    function whitelistMint(bytes32[] calldata proof_) external payable phaseRequired(Phase.WHITE_MINT) notContract nonReentrant {
        require(isWhiteListed(proof_, merkleRoot, msg.sender), "not whitelisted");
        require(whitelistMinted < maxWhitelistSupply && totalSupply() < maxSupply, "full");
        require(amountNFTsPerMint[msg.sender] <= maxOneAccountMint, "you can only get 1 NFT");
        require(msg.value >= whitelistSalePrice, "insufficient funds");
        whitelistMinted++;
        amountNFTsPerMint[msg.sender] += 1;

        _internalMint(msg.sender);

        refundIfOver(whitelistSalePrice);
    }

    function publicMint() external payable phaseRequired(Phase.PUBLIC_MINT) notContract nonReentrant {
        require(totalSupply() < maxSupply, "full");
        require(msg.value >= publicSalePrice, "insufficient funds");
        require(amountNFTsPerMint[msg.sender] <= maxOneAccountMint, "you can only get 1 NFT");
        amountNFTsPerMint[msg.sender] += 1;

        _internalMint(msg.sender);

        refundIfOver(publicSalePrice);
    }

    function _internalMint(address to) internal {
        super._safeMint(to, _lastTokenId);
        emit Mint(to, _lastTokenId);
        unchecked {
            _lastTokenId++;
        }
    }
    ...
    function setPhase(Phase phase_) external onlyRole(DEFAULT_ADMIN_ROLE) {
        _phase = phase_;
    }
    ...
    modifier phaseRequired(Phase phase_) {
        require(phase_ == _phase, "mint not available");
        _;
    }
