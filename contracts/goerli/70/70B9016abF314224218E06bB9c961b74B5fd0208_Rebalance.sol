...
contract Test is Ownable {
  ...
  constructor(address _positionManager, address _liquidityRouter) {
    positionManager = _positionManager;
    liquidityRouter = _liquidityRouter;
  }
  ...
  function deposit(uint256 _tokenId) external onlyOwner {
    require(tokenId == 0, "Test.deposit: token already deposited");
    INonfungiblePositionManager(positionManager).safeTransferFrom(msg.sender, address(this), _tokenId);
    tokenId = _tokenId;
    emit Deposit(_tokenId);
  }
  ...
  function refund() external onlyOwner {
    uint256 _tokenId = tokenId;
    require(_tokenId != 0, "Test.refund: token already refunded");
    INonfungiblePositionManager pm = INonfungiblePositionManager(positionManager);

    pm.safeTransferFrom(address(this), msg.sender, _tokenId);
    (, , address token0, address token1, , , , , , , , ) = pm.positions(_tokenId);
    uint256 balance0 = IERC20(token0).balanceOf(address(this));
    if (balance0 > 0) {
  ...
  function decreaseLiquidity() public onlyOwner returns (uint256 amount0, uint256 amount1) {
    uint256 _tokenId = tokenId;
    require(_tokenId != 0, "Test.decreaseLiquidity: token already refunded");
    INonfungiblePositionManager pm = INonfungiblePositionManager(positionManager);

    (, , , , , , , uint128 liquidity, , , , ) = pm.positions(_tokenId);
    if (liquidity > 0) {
  ...
  {
    uint256 _tokenId = tokenId;
    require(_tokenId != 0, "Test.increaseLiquidity: token already refunded");
    INonfungiblePositionManager pm = INonfungiblePositionManager(positionManager);
    ...
    uint256 balance1 = IERC20(token1).balanceOf(address(this));
    IERC20(token1).safeApprove(address(pm), balance1);
    (liquidity, amount0, amount1) = pm.increaseLiquidity(
      INonfungiblePositionManager.IncreaseLiquidityParams({
        tokenId: tokenId,
        amount0Desired: balance0,
        amount1Desired: balance1,
  ...
  function rebalance(int24 tickLower, int24 tickUpper) external onlyOwner {
    ...
    (, , address token0, address token1, uint24 fee, , , , , , , ) = pm.positions(_tokenId);
    uint256 amountIn1 = IERC20(token1).balanceOf(address(this));
    if (amountIn1 > 0) {
      _swap(fee, token1, token0, amountIn1);
      _swap(fee, token0, token1, IERC20(token0).balanceOf(address(this)) / 2);
    }
