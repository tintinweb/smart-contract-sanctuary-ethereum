...
contract ClaimableUpgradable is Initializable, AdminManagerUpgradable {
    bytes32 private _merkleTreeRoot;
    mapping(address => uint256) internal _claimed;

    function __Claimable_init() internal onlyInitializing {
        __AdminManager_init_unchained();
        __Claimable_init_unchained();
    }

    function __Claimable_init_unchained() internal onlyInitializing {}
    ...
    function _claim(
        ...
        internal 
        onlyClaimable(msg.sender, amount_, proof_)
    {
        _claimed[msg.sender] += amount_;
        _claimMint(msg.sender, amount_);
    }

    function _claimMint(address to_, uint256 count_) internal virtual {}
    ...
    function isClaimable(
        ...
        return MerkleProofUpgradeable.verify(
            proof_, _merkleTreeRoot, keccak256(abi.encodePacked(account_, amount_))
        ) && _claimed[account_] < amount_;
    }
    ...
    modifier onlyClaimable(
        ...
        require(isClaimable(account_, amount_, proof_), "Not claimable");
        _;
    }
...
import '@chocolate-factory/contracts/whitelist/ClaimableUpgradeable.sol';
import "./NFTClaimableUpgradeable.sol";

contract Metaversal is MultiStage, NFTClaimableUpgradeable, ClaimableUpgradable {
   function initialize(
    ...
    updateMerkleTreeRoot(3, phaseOneMerkleRoot_);

    // Guaranteed
    setPrice(4, 0.057 ether);
    updateBalanceLimit(4, 1);
    updateMerkleTreeRoot(4, phaseTwoMerkleRoot_);
    updateMerkleTreeRoot(claimMerkleRoot_);
  }

  function nftClaim(uint256[] memory ids) external {
    require(stage == 2, "NFT Claim not live");
    uint256 total = _nftClaim(ids, msg.sender);
    require(total > 0, "Non claimable");
    _callMint(msg.sender, total);
  }

  function claim(uint256 amount_, bytes32[] calldata proof_) external {
    require(stage == 5, "Claim not live");
    _claim(amount_, proof_);
  }

  function _claimMint(address to_, uint256 count_) internal override(ClaimableUpgradable) {
    _callMint(to_, count_);
  }
}
...
contract NFTClaimableUpgradeable is
    Initializable,
    AdminManagerUpgradable
{
    IERC721Upgradeable public nft;
    mapping (uint256 => bool) private _nftClaimed;

    function __NFTClaimable_init(IERC721Upgradeable _nft) internal onlyInitializing {
        __AdminManager_init_unchained();
        __NFTClaimable_init_unchained(_nft);
    }

    function __NFTClaimable_init_unchained(IERC721Upgradeable _nft)
        internal
        onlyInitializing
    {
        nft = _nft;
    ...
    function _nftClaim(uint256[] memory ids, address to) internal returns (uint256) {
        uint256 total;
        uint256 length = ids.length;
        for(uint256 i; i < length; i++) {
            uint256 id = ids[i];
            if(!_nftClaimed[id] && nft.ownerOf(id) == to) {
                total++;
                _nftClaimed[id] = true;
            }
        }
    ...
    function nftClaimed(uint256 id) public view returns (bool) {
        return _nftClaimed[id];
    }

    function nftClaimable(uint256[] memory ids, address to) public view returns (uint256) {
        uint256 total;
        uint256 length = ids.length;
        for(uint256 i; i < length; i++) {
            uint256 id = ids[i];
