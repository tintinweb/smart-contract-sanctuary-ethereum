...
contract TestWithYourEyes is ERC721URIStorage, Ownable {
    uint256 public normalMintPrice = 0.0033 ether;
    uint256 public maxMintPerAccount = 99;
    ...
    constructor(string memory baseURI_)
        ERC721("Test With Your Eyes", "TESTEYES")
    {
        _tokenBaseURI = baseURI_;
        _tokenIdCounter = 0;
    }

    // Define a custom event for when the name of a token is changed
    ...
    function mint(uint256 amount) external payable {
        ...
        // SELECT ONE OF 6000 METADATAS RANDOM
        uint256 randomIndex = uint256(
            keccak256(
                abi.encodePacked(block.timestamp, block.timestamp, tokenId)
            )
        ) % 6000;
        ...
        // Add the token ID to the name of the NFT
        string memory name = string(
            abi.encodePacked("Checks Test #", tokenId.toString())
        );

        // Set the token name by updating the metadata JSON file
        bytes memory metadataBytes = bytes(tokenURI(tokenId));
        uint256 startIndex = indexOf(metadataBytes, bytes('"name":'), 0) + 8;
        uint256 endIndex = indexOf(metadataBytes, bytes(","), startIndex);
        bytes memory nameBytes = bytes(name);
        require(nameBytes.length <= endIndex - startIndex, "Name is too long");
        for (uint256 i = 0; i < nameBytes.length; i++) {
            metadataBytes[startIndex + i] = nameBytes[i];
        }
        emit NameChange(tokenId, name);
        _setTokenURI(tokenId, string(metadataBytes));
    ...
    function indexOf(
        ...
        uint256 limit = haystack.length - needleLen + 1;
