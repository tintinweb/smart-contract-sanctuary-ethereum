...
abstract contract DeterministicAddress {
    function getMetamorphicContractAddress(bytes32 _salt, address _factory)
        ...
        returns (address)
    {
        // hash of the creation byte code for the MetamorphicDeployer with solidity 0.8.16 and 20000 optimizer runs
        bytes32 metamorphicContractByteCodeHash_ = 0x59e7909057e70589432c192cf9a281d34f50f7d679974d0fdfae3fa0020b43b7;
        return
            address(
                uint160(
                    uint256(
...
// File contracts/libraries/factory/MetamorphicDeployer.sol
...
contract MetamorphicDeployer {
    constructor() {
        assembly {
            let ptr := mload(0x40)
            let ok := staticcall(gas(), caller(), 0, 0, 0, 0)
            returndatacopy(ptr, 0, returndatasize())
            if iszero(ok) {
                revert(ptr, returndatasize())
            }
            ok := delegatecall(gas(), mload(ptr), 0, 0, 0, 0)
            returndatacopy(ptr, 0, returndatasize())
            if iszero(ok) {
                revert(ptr, returndatasize())
            }
...
abstract contract AliceNetFactoryBase is DeterministicAddress, ProxyUpgrader {
    ...
    */
    ...
    function _deployProxy(bytes32 salt_) internal returns (address contractAddr) {
        ...
        bytes memory creationCode = type(MetamorphicDeployer).creationCode;
        assembly {
            // store proxy template address as implementation,
            sstore(_implementation.slot, proxyTemplate)
            contractAddr := create2(0, add(creationCode, 0x20), mload(creationCode), salt_)
