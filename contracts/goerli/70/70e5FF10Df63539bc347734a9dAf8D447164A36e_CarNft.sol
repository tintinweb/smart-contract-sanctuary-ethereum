...
///
contract CarNft is Ownable, Pausable, ERC721, ERC721Enumerable {
    ...
    uint256 private _mintActiveAt = 1668436800; // Nov 14th, 2:40:00 PM GMT
    ...
    /// @notice User mint nfts
    function mintCar(uint256 amount_) external payable whenNotPaused {
        ...
        if (!_whitelist[_msgSender()]) {
            require(
                _mintCount[_msgSender()] + amount_ <= _maxPerWallet,
                "Exceeded wallet max"
            );
            require(amount_ <= _maxMintPerTx, "Exceeded tx max");
        }

        _mintCount[_msgSender()] = _mintCount[_msgSender()] + amount_;
    ...
    function whalesConf() external view returns (uint256, uint256) {
        return (_maxMintPerTx, _maxPerWallet);
    }
    ...
    function mintActiveTime() external view returns (uint256) {
