...
contract testMap is ERC721A, ReentrancyGuard, Ownable, DefaultOperatorFilterer {
	...
	mintedClasses[_to][_class] += _count;
	classCounter[_class] += _count;
	...
	require( _mintedAmount + 1 <= LIMIT_PER_WALLET, "Exceeds max mints per wallet" );
	uint256 _freeAmount = mintedFree[msg.sender];
	require( _freeAmount == 0, "Exceeds claim free amount" );

	_mint(msg.sender, 1);
	mintedFree[msg.sender] = 1;

	mintedAmount[msg.sender] = _mintedAmount + 1;
	addItems( msg.sender, 1, totalSupply(), _class );
    }
