...
abstract contract ERC1155_NFTCExtended_Expandable is ExpandableExtensionWithAccessControlV2, ERC1155_NFTCExtended {
    ...
    function _validateMint(
        ...
        // Execute the validation scheme.
        (bool validationResult, string memory message) = IFlavorInfoValidatorV2(tokenFlavor.externalValidator).validateMint(
            caller,
            holder,
            count,
            tokenFlavor,
        ...
        // Trigger a revert if the validation did not succeed.
        require(validationResult, message);
    }
...
import './IFlavorInfoValidatorV2.sol';
...
abstract contract ExpandableExtensionV2 is FlavorInfoManagerV2 {
    ...
    function injectFlavorsFromProvider(address __externalProviderAddress) external {
        ...
        FlavorInfoV2[] memory flavors = IFlavorInfoProviderV2(__externalProviderAddress).provideFlavorInfos();
    ...
    function _checkValueIsValid(FlavorInfoV2 memory tokenFlavor, uint256 count, uint256 valueSent) internal pure {
        if (valueSent != tokenFlavor.price * count) revert InvalidValuePayment();
    }
...
import './BannerCollectionGoerliBase.sol';
import './BannerCollectionGoerliSplitsAndRoyalties.sol';

/**
 * @title BannerCollectionGoerli
 * @author @NFTCulture
 * @dev NFTC Reference ERC1155 Implementation.
 */
contract BannerCollectionGoerli is BannerCollectionGoerliBase, BannerCollectionGoerliSplitsAndRoyalties {
    constructor()
        BannerCollectionGoerliBase(
            'https://nftc-outland.mypinata.cloud/ipfs/Qmc9KYakbvmQ4gBxPoQG1FfZFLgntEDCUge8v6WqbNXgcf/{id}.json' // Outland_Banner_Goerli_Metadata_V1
        )
    {
    ...
    PrivilegedMinter
{
    ...
    function _getPackedPurchasesAs64(
        ...
        require(wallet == address(0), 'ERC1155 contract does not yet support allowlist.');
        return 0;
    }
     ...
     * @param count the number of tokens to mint.
     * @param flavorId the type of tokens to mint.
     * @param validationData extra information required to pass validation.
     * @param holder a cold wallet, if a delegation scheme is being used, or zero.
     */
    function publicMintTokens(
        ...
        if (updatedFlavor.externalValidator != address(0)) {
            _validateMint(msg.sender, holder, count, updatedFlavor, validationData);
        }
     ...
     * @param validationData extra information required to pass validation.
     * @param holder a cold wallet, if a delegation scheme is being used.
     * @param destination address where the new token should be sent.
     */
    function publicMintTokensTo(
        ...
        if (updatedFlavor.externalValidator != address(0)) {
            // Note: Privileged minter address is being used as caller.
            // For this to work with a delegation scheme, coldWallet needs to
            // delegate to privileged minter.
            _validateMint(msg.sender, holder, count, updatedFlavor, validationData);
        }

        _saveFlavorInfo(updatedFlavor);

        _internalMintTokensOfFlavor(destination, count, flavorId);
    }
...
abstract contract BannerCollectionGoerliSplitsAndRoyalties is NFTCSplitsAndRoyalties {
