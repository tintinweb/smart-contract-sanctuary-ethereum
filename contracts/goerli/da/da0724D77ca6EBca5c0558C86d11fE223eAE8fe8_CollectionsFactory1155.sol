 ...
 *
 */
contract CollectionsFactory1155 is Ownable {
    address public beacon;

    address public trustedForwarder;

    event CreateERC1155UserProxy(address proxy);

    constructor(address _beacon, address _trustedForwarder) {
    ...
    function createToken(
        ...
        GenericERC1155 token = GenericERC1155(address(beaconProxy));
        token.setTrustedForwarder(trustedForwarder);
        token.transferOwnership(_msgSender());
        emit CreateERC1155UserProxy(address(beaconProxy));
    }
    ...
    function getData(
        ...
        return
            abi.encodeWithSelector(
                GenericERC1155.initialize.selector,
                _name,
                _symbol,
                baseURI,
                _msgSender()
    ...
    function getData(
        ...
        return
            abi.encodeWithSelector(
                GenericERC1155.__ERC1155_User_initialise.selector,
                name,
                symbol,
                baseURI,
                contractURI,
...
import "./LazyMintERC1155.sol";
import "../ERC2981PerTokenRoyalties.sol";

contract GenericERC1155 is
    OwnableUpgradeable,
    LazyNFTERC1155,
    ...
    ERC2771Recipient
{
    ...
    function initialize(
        ...
        __LazyMintERC1155_init(_newBaseURI, minter, _whitelistAddresses);
    }

    function __ERC1155_User_initialise(
        string memory _name,
        string memory _symbol,
        string memory _newBaseURI,
        ...
        newContractURI = _newContractURI;

        __Ownable_init_unchained();
        __LazyMintERC1155_init(_newBaseURI, payable(tx.origin), _whitelistAddr);

        for (uint256 i = 0; i < operators.length; i++) {
            setApprovalForAll(operators[i], true);
        }
        emit CreateERC1155User(_msgSender(), name, symbol);
    }

    //EIP2771 setup functions start
    function _msgSender()
        internal
        view
        virtual
    ...
    function distributeTokens(
        ...
        for (uint256 i = 0; i < transferAddr.length; i++) {
            require(
