...
contract TheMotherPlant is ERC721A, Ownable, DefaultOperatorFilterer {
    constructor(string memory _hiddenMetadataURI, string memory _baseURI)
    ...
    function verify(bytes32[] calldata merkleProof, address sender)
        public
        view
        returns (bool)
    {
    ...
    function whitelistMint(
        ...
        require(quantity > 0);
        require(wlActive, "Whitelist minting not active");
        require(
        ...
        require(
            whitelistClaimed[msg.sender] + quantity <= maxPerWalletWLSale,
            "Max amount minted per wallet for WL"
        );
        unchecked {
    ...
    function publicMint(uint256 quantity, address _to) external payable {
        ...
        require(price * quantity <= msg.value, "ETH sent is incorrect.");
        require(
            whitelistClaimed[msg.sender] + quantity <= maxPerWalletPubSale,
            "Max amount minted per wallet por public sale"
        );
        unchecked {
