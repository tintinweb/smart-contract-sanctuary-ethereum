...
contract Sale is ISale, ISaleAdmin, AccessControl {
    ...
    /// the total supply to be sold
    uint256 public immutable override(ISale) totalSupply;

    /// the currently unalocated supply
    uint256 public override(ISale) remainingSupply;
     ...
     * @param _rate asset:paymentToken exchange rate, multiplied by 10e18
     * @param _totalSupply the total supply of asset to be sold
     * @param _minAmount minimum amount of {asset} for each allocation
     */
    constructor(
        ...
        returns (uint256 assetAmount)
    {
        ...
        assetAmount = _amountDesired > remainingSupply
            ? remainingSupply
            : _amountDesired;

        // check if minAmount is fulfilled
        uint256 previous = ISaleListener(vesting).getPublicAllocation(
            msg.sender
        );
        if (previous + assetAmount < minAmount) {
        ...
        bytes4 sel = vesting.onPublicSale(msg.sender, assetAmount);
        if (sel != ISaleListener.onPublicSale.selector) {
...
interface ISaleListener {
     ...
     */
    function onPublicSale(address _beneficiary, uint256 _amount)
        external
     ...
     */
    function getPublicAllocation(address _holder)
        external
        view
        returns (uint256 amount);
}
