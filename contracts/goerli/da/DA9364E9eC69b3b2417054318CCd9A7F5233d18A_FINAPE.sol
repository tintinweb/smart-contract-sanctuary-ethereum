...
contract FINAPE is ERC721Enumerable, Ownable{
    using SafeMath for uint256;

    uint256 public MAX_ELEMENTS = 10000;
    uint256 public PRICE = 10 * 10 ** 18;
    address public PAY_TOKEN;
    enum STAGE { PRIVATE_SALE, PUBLIC_SALE }

    // state variable
    STAGE public CURRENT_STAGE = STAGE.PRIVATE_SALE;
    ...
    constructor() ERC721("FINAPE", "FAPE"){
    }
    ...
    function privateSale(bytes32[] calldata merkleProof) external {
        require(!MINTING_PAUSED, "Minting is not active");
        require(CURRENT_STAGE == STAGE.PRIVATE_SALE, "Current stage should be PRIVATE_SALE");
        require(MerkleProof.verify(merkleProof, merkleRoot, keccak256(abi.encodePacked(msg.sender))), "You are not on the white list");
        require(totalSupply() < MAX_ELEMENTS, 'All tokens have been minted');

        SafeERC20.safeTransfer(IERC20(PAY_TOKEN), owner(), PRICE);
