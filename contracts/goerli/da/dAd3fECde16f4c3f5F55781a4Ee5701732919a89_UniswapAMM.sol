...
contract UniswapAMM is Owned {
    ...
    // Stabilizers
    address[] public stabilizers_array; // Allowed to swap
    mapping(address => bool) public stabilizers; // Mapping is also used for faster verification
    ...
    modifier onlyByOwnStabilizers() {
       require(stabilizers[msg.sender] == true || msg.sender == owner, "Only owner or stabilizers");
        _;
    }
    ...
    */
    ...
    // Uni V3 only
    function swapExactInput(address _tokenA, address _tokenB, uint24 _fee_tier, uint256 _amountIn, uint256 _amountOutMinimum)  public onlyByOwnStabilizers returns (uint256 amountOut) {
        // Approval
    ...
    function swapExactOutput(address _tokenA, address _tokenB, uint24 _fee_tier, uint256 amountOut, uint256 amountInMaximum) public onlyByOwnStabilizers returns(uint256 amountIn) {
        TransferHelper.safeApprove(_tokenA, address(univ3_router), amountInMaximum);
    ...
    // Adds whitelisted stabilizers
    function addStabilizer(address stabilizer_address) public onlyByOwnGov {
        require(stabilizer_address != address(0), "Zero address detected");

        require(stabilizers[stabilizer_address] == false, "Address already exists");
        stabilizers[stabilizer_address] = true;
        stabilizers_array.push(stabilizer_address);

        emit StabilizerAdded(stabilizer_address);
    }

    // Remove a minter
    function removeStabilizer(address stabilizer_address) public onlyByOwnGov {
        require(stabilizer_address != address(0), "Zero address detected");
        require(stabilizers[stabilizer_address] == true, "Address nonexistant");

        // Delete from the mapping
        delete stabilizers[stabilizer_address];

        // 'Delete' from the array by setting the address to 0x0
        for (uint i = 0; i < stabilizers_array.length; i++){
            if (stabilizers_array[i] == stabilizer_address) {
                stabilizers_array[i] = address(0); // This will leave a null in the array and keep the indices the same
                break;
            }
        }

        emit StabilizerRemoved(stabilizer_address);
    }
    ...
    event StabilizerAdded(address minter_address);
    event StabilizerRemoved(address minter_address);
