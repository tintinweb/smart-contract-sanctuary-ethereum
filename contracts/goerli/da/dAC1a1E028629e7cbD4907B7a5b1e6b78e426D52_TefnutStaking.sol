...
contract TefnutStaking is ReentrancyGuard {
    ...
    modifier checkAllowance(uint256 amount) {
        require(s_stakingToken.allowance(msg.sender, address(this)) >= amount, "Error");
        _;
    }
    ...
    function stake(uint256 amount)  external checkAllowance(amount) updateReward(msg.sender) moreThanZero(amount) {
