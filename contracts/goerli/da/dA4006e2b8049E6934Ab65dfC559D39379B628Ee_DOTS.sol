...
contract DOTS is ERC721AQueryable, AccessControl, VRFConsumerBaseV2, OperatorFilterer {
    ...
    mapping(uint256 => uint256[]) public currentEvoDots;
    ...
    function fulfillRandomWords(
        ...
        while (_numAssigned < anomalyRoll.anomalyAmount) {
            ...
            if(!metadata[_tempAnomalyToken].isAnomaly) {
                metadata[_tempAnomalyToken].isAnomaly = true;

                unchecked { _numAssigned++; }

                emit AnomalyRolled(_tempAnomalyToken);
            }
    ...
    function _internalMint(
        ...
        for (uint i=0; i<_numDots; ) {
            ...
            metadata[_tokenId] = Metadata(uint8(_evoNum), uint64(_genes), false);
    ...
    function _normalize(
        ...
        uint256[8] memory _traitIds = [
            _getId(_rand, uint256(16), uint256(65529), NUM_BACKGROUND),
        ...
        _normalized =
            (_traitIds[0] << 0 | // background
            _traitIds[1] << 8 | // back
            _traitIds[2] << 16 | // paintjob
            _traitIds[3] << 24 | // eyewear
            _traitIds[4] << 32 | // mouthgear
            _traitIds[5] << 40 | // headgear
            _traitIds[6] << 48 | // clothing
            _traitIds[7] << 56) // eargear
    ...
    function _generateMask(uint256 _selector) private pure returns (uint256 _mask) {
        ...
        _mask = uint256(4294967295); // first 32 bits always on
        if (_selector > 1637) {
            if (_selector < 24575) { // 5 trait dot (35%)
                if (_selector < 9322) _mask = _mask | _eightBitMask << 48; // 33.5% clothing selected
                else if (_selector < 16891) _mask = _mask | _eightBitMask << 40; // 33% headgear selected
                else if (_selector < 24460) _mask = _mask | _eightBitMask << 32; // 33% mouthgear selected
                else _mask = _mask | _eightBitMask << 56; // 0.5% eargear selected
            } else if (_selector < 57343) { // 6 trait dot (50%)
                if (_selector < 37551) _mask = _mask | _eightBitMask << 40 | _eightBitMask << 48; // 39.6% no mouthgear, eargear
                else if (_selector < 53771) _mask = _mask | _eightBitMask << 32 | _eightBitMask << 48; // 49.5% no headgear, eargear
                else if (_selector < 54066)  _mask = _mask | _eightBitMask << 48 | _eightBitMask << 56; // 0.9% no headgear, mouthgear
                else if (_selector < 57277) _mask = _mask | _eightBitMask << 32 | _eightBitMask << 40; // 9.8% no clothing, eargear
                else if (_selector < 57306) _mask = _mask | _eightBitMask << 40 | _eightBitMask << 56; // 0.089% no clothing, mouthgear
                else _mask = _mask | _eightBitMask << 32 | _eightBitMask << 56; // 0.111% no clothing, headgear
            } else if (_selector < 63897) { // 7 trait dot (10%)
                if (_selector < 62974) _mask = _mask | _eightBitMask << 32 | _eightBitMask << 40 | _eightBitMask << 48; // 85.93% no eargear
                else if (_selector < 63257) _mask = _mask | _eightBitMask << 40 | _eightBitMask << 48 | _eightBitMask << 56; // 4.32% no mouthgear
                else if (_selector < 63568) _mask = _mask | _eightBitMask << 32 | _eightBitMask << 48 | _eightBitMask << 56; // 4.75% no headgear
                else _mask = _mask | _eightBitMask << 32 | _eightBitMask << 40 | _eightBitMask << 56; // 5% no clothing
            } else _mask = _mask | _mask << 32; // 8 trait dot
        }
    }
