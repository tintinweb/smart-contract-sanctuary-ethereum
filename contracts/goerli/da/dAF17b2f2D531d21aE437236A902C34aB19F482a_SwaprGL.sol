...
contract SwaprFee is BaseGovernanceUpgradeable {
    ...
    /// @notice add new payment accepted token
    ...
    function getBaseOrderFee(uint subjectAmount, uint tokenPriceUSD, address paymentToken) public view returns(uint baseFee) {
        baseFee = _getBaseFee(OrderFee, subjectAmount, tokenPriceUSD, paymentToken);
    ...
    /// @param paymentToken token used to pay the fee
    /// @return baseFee base calculated fee
    function getBaseAuctionFee(uint subjectAmount, uint tokenPriceUSD, address paymentToken) public view returns(uint baseFee) {
        baseFee = _getBaseFee(AuctionFee, subjectAmount, tokenPriceUSD, paymentToken);
    ...
    function getFinalOrderFee(uint subjectAmount, address paymentToken) public view returns(uint finalFee) {
        finalFee = _getFinalFee(OrderFee, subjectAmount, paymentToken);
    ...
    /// @param paymentToken token used to pay the fee
    /// @return finalFee final calculated fee
    function getFinalAuctionFee(uint subjectAmount, address paymentToken) public view returns(uint finalFee) {
        finalFee = _getFinalFee(AuctionFee, subjectAmount, paymentToken);
    ...
    /// @notice Internal function to help calculate the exp of given token
    /// @param _paymentToken payment token or 0 address in case of native currency
    function _getEXP(address _paymentToken) internal view returns(uint exp){
        if(_paymentToken == address(0)){
            exp = EXP;
        }else{
            exp = 10 ** IERC20MetadataUpgradeable(_paymentToken).decimals();
        }
    }
    ...
    /// @return baseFee base calculated fee
    function _getBaseFee(FeePricing memory feePricing, uint subjectAmount, uint tokenPrice, address paymentToken) internal view returns(uint baseFee) {
        require(feePricing.percentBaseFee <= EXP, "SHOULD_NOT_EXCEED_1e18");
        if(isTokenSupported(paymentToken)){
            uint iEXP = _getEXP(paymentToken);
            uint percentBaseFee = (subjectAmount * feePricing.percentBaseFee) / iEXP;
            uint fixedBaseFee = (feePricing.fixedBaseFee / tokenPrice) * iEXP;
    ...
    /// @param feePricing refer to type FeePricing type
    /// @return finalFee final calculated fee
    function _getFinalFee(FeePricing memory feePricing, uint subjectAmount, address paymentToken) internal view returns(uint finalFee) {
        require(feePricing.finalFeePercentage <= EXP, "SHOULD_NOT_EXCEED_1e18");
        uint iEXP = _getEXP(paymentToken);
        if(feePricing.isFinalFeePercentage){
            finalFee = (subjectAmount * feePricing.finalFeePercentage) / iEXP;
        }
    }
...
/// @notice Allows only signature based listings
/// @dev Can only be interacted from a recognised marketplace EOA
contract SwaprGL is BaseGovernanceUpgradeable, Helper {

    using SafeERC20Upgradeable for IERC20MetadataUpgradeable;

    event Purchased(bool isSplit, Order order);
    ...
    function buyNowOrder(uint buyerPurchasedAmount, bytes memory data, uint split) external onlyMarketplace(data) {
        ...
        uint iEXP = getEXP(order.acceptedToken);
        ...
        //MakePayment
        uint price = (order.fixedPrice * split) / iEXP;
        ...
        if(split < order.remainingPart){
            ...
            //seller's part
            uint splitAdjust = split / order.remainingPart * EXP;
            splitParts[0] = EXP - splitAdjust;//where remainingPart should be unsold part of the NFT
            ...
            order.remainingPart = order.remainingPart - split;
...
contract SwaprWallet is BaseGovernanceUpgradeable, ERC721HolderUpgradeable {
    ...
    function splitReleaseNFT(address lock, uint nftId, uint[] calldata splitParts, address[] calldata addresses) external onlySwapr returns(uint256[] memory newIDs) {
        newIDs = ILock(lock).split(nftId, splitParts, addresses);
    }
