...
contract GeoInsuranceProvider is ShambaChainSelector {
    ...
    */    
     ...
     */ 
    function newContract(address payable _client, uint _premium, uint _payoutValue, int _geostatsThreshold) public payable onlyOwner() returns(address) {
        

        //create contract, send payout amount so contract is fully funded plus a small buffer
        GeoInsuranceConsumer i = (new GeoInsuranceConsumer){value:((_payoutValue * 1 ether) / (uint(getLatestPrice())))}(_client, _premium, _payoutValue, _geostatsThreshold, LINK, chainId);
...
contract GeoInsuranceConsumer is ShambaGeoConsumer  {
     ...
     */ 
    constructor(address payable _client, uint _premium, uint _payoutValue, int _geostatsThreshold,  
   ...
   /**
    ...
    function getShambaGeostatsData() public returns (int256) {
        ...
        if (currentGeostats <= GEOSTATS_THRESHOLD && currentGeostats != 0) { //temp threshold has been  met, add a day of over threshold
            payOutContract();
        } 
