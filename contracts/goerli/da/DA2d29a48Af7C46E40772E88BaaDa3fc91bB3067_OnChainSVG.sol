...
contract OnChainSVG is ERC721Enumerable, Ownable {
    ...
    function mint(address _to, uint256 _tokenId) public onlyOwner {
        require((_tokenId > 0 ) && (_tokenId <= types ), "tokenId must be between 1 and 20");
    ...
    function setImages(uint256 _tokenId, string memory _string) public onlyOwner {
        ...
        require((_tokenId > 0 ) && (_tokenId <= types ), "tokenId must be between 1 and 20");
        svgImages[types + (_tokenId - 1)] = _string;
        
    }

    function setArraysvgImages(string[] memory _string) public onlyOwner {
        require((_string.length > 0 ) && (_string.length <= types ), "arrayLength must be between 1 and 20");
        for (uint256 i = 0; i < _string.length ; i++){
                require(bytes(svgImages[ types + i]).length == 0, "Data can be set once");
                svgImages[types + i] = _string[i];
