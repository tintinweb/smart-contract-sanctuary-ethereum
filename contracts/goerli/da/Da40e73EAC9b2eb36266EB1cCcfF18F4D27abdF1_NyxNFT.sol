...
contract NyxNFT is ERC1155Supply, Ownable
{
        ...
        private
    {
        ...
        if (totalSupply(currentTokenId) + nMint > MAX_SUPPLY)
        {
            revert("No more NFT to mint");
        }
        if (waveCurrentSupply[currentTokenId] + nMint > waveMintableSupply[currentTokenId])
        {
            revert("No more NFT to mint for this wave");
        }
        if (!waveIsRunning[currentTokenId])
        {
            revert("Wave has ended");
        }
        ...
        public payable
    {
        ...
        // require(msg.value >= wavePrice[currentTokenId] * nMint, "Not enough ETH to mint");
        if (!isWhitelist(v, r, s, nonce, deadline))
        {
            revert("You are not Whitelisted");
        }
        if (waveMints[currentTokenId][recipient] + nMint > max_holding_whitelist)
        {
            revert("Max holded NFTs is reached");
        }
        if (msg.value < wavePrice[currentTokenId] * nMint)
        {
            revert("Not enough ETH to mint");
        }
        return _mintNFT(nMint, recipient);
    }
