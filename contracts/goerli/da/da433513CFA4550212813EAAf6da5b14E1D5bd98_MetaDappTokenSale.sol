...
contract MetaDappTokenSale {
  ...
  function buy(uint _tokens) public payable {
    require(msg.value / phase(currentPhaseIndex).price == _tokens, "Error value not match");
    require(phase(currentPhaseIndex).total <= __amount(_tokens), "Error low balance");
    require(token.balanceOf(address(this)) >= __amount(_tokens), "Sold out");
    require(token.transfer(msg.sender, __amount(_tokens)), "Error tranfering tokens");

    sold += _tokens;
    phases[currentPhaseIndex].total -= _tokens;
  ...
  function endSale() public isOwner {
    require(token.transfer(owner, token.balanceOf(address(this))), "Error transfering tokens");
  ...
  function phase(uint _phaseId) public view returns(Phase memory) {
