 ...
 * The winners will be picked from the list of token that have chosen
 ...
 * - Randomly pick 7 numbers from 1 to max species for each species.
...
contract WondrousXRaffle is Ownable {
    ...
    // Attach the csv token index from IPFS to this contract
    ...
    function _random(uint256 index) public view returns (uint256) {
        return
            uint256(
                keccak256(
                    abi.encodePacked(
