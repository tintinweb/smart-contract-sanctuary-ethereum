...
contract TEST is ERC721Enumerable, Ownable, ReentrancyGuard {
    ...
    function mint(bytes32[] calldata proof) external payable {
        ...
        require(minted < MAX_SUPPLY, "All NFTs have been minted");
        if (currentPhase == MintPhase.PublicMint) {
            uint256 toMint = msg.value / PUBLIC_MINT_PRICE;
            require(
                toMint > 0 && toMint <= 10,
                "Can only mint between 1 and 10 tokens per transaction"
            );
            require(minted + toMint <= MAX_SUPPLY, "Not enough NFTs left");
            ...
            earlyAdopterMintCount[msg.sender] < 2
        ) {
            require(msg.value == WHITELIST_MINT_PRICE, "Incorrect payment");
            earlyAdopterMintCount[msg.sender]++;
            earlyAdopterMinted++;
            _mint(msg.sender, minted + 1);
            ...
            require(
                whitelistMintCount[msg.sender] < 2,
                "Already minted 2 NFTs during whitelist mint"
            );
            require(msg.value == WHITELIST_MINT_PRICE, "Incorrect payment");
    ...
    function mintRemainingFreeToTreasury(
        ...
        require(
            currentPhase == MintPhase.WhitelistMint,
            "In whitelist mint phase"
        );

        uint256 remainingFreeMint = FREE_MINT_SUPPLY - freeMinted;
        for (uint256 i = 0; i < Math.min(remainingFreeMint, max_amount); i++) {
            uint256 minted = totalSupply();
            _mint(owner(), minted + 1);
            freeMinted++;
        }
    }
