...
contract WallStBullsOptions is ERC721Holder, Ownable {
    enum State{ FREE, MOON, HOMELESS, PRISON, DEAD }
    struct TokenState {
        State state;
        address owner;
    }
    ...
    uint256 private optionCount = 0;

    TokenState[MAX_TOKENS] public states;
    ...
    uint256 public deadOdds;
    uint256 public prisonOdds;
    uint256 public moonOdds;

    event TokenStateChange(uint256 tokenId, State state);
    ...
    function _setTokenState(uint256 _tokenId, address _owner, State _state) internal {
        states[_tokenIdToIndex(_tokenId)] = TokenState(_state, _owner);
    ...
    function option(uint256 _tokenId) external {
        ...
        require(erc1155.balanceOf(msg.sender, erc1155TokenId) >= PRICE, "ERC1155: Address has no tokens");
        require(erc1155.isApprovedForAll(msg.sender, address(this)), "ERC1155: Contract is not approved operator");

        require(erc721.ownerOf(_tokenId) == msg.sender, "ERC721: Address does not own token");
        require(erc721.isApprovedForAll(msg.sender, address(this)), "ERC721: Contract is not approved operator");

        erc1155.burn(msg.sender, erc1155TokenId, PRICE);
        _option(msg.sender, _tokenId);
    }

    function _option(address _owner, uint256 _tokenId) internal {
        State state = _rollToState(_roll(_tokenId));
        if(state == State.DEAD || state == State.PRISON) {
            erc721.safeTransferFrom(_owner, address(this), _tokenId);
        }
        _setTokenState(_tokenId, _owner, state);
        optionCount += 1;
    }
    ...
    function _rollToState(uint256 roll) internal view returns (State) {
        if (roll < deadOdds) {
            return State.DEAD;
        } else if (roll < prisonOdds) {
            return State.PRISON;
        } else if(roll < moonOdds) {
            return State.MOON;
        }
