 ...
 * @notice The PrizeFlush contract helps capture interest from the PrizePool and move collected funds
           to a designated PrizeDistributor contract. When deployed, the destination, reserve
           addresses are set and used as static parameters during every "flush" execution. The parameters can be
           reset by the Owner if necessary. The protocol can charge a fee during the movement of the funds.
 */
contract PrizeFlush is Initializable, IPrizeFlush, Manageable {
    ...
    /// @notice Representation of 100% with 3 decimal places, used to split the obtained funds.
    ...
    function flush() external override onlyManagerOrOwner returns (bool) {
        ...
        // After funds are distributed, we EXPECT funds to be located in the Reserve.
        IReserve _reserve = reserve;
        IERC20Upgradeable _token = _reserve.getToken();
        uint256 _amount = _token.balanceOf(address(_reserve));

        // IF the tokens were successfully moved to the Reserve, now move them to the destination (PrizeDistributor) address.
        if (_amount > 0) {
    ...
    /**
     * @notice Distribute ticket tokens to prize to the reserve and protocol fee recipient.
     * @dev Transfers the minted ticket tokens to the reserve and protocol fee recipient.
     */
