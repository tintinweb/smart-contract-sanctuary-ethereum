...
contract AAOptionMarket is AccessControl, IAAOptionMarket {
    ...
    mapping(uint256 => uint256) public override spotPrices;
    ...
    function checkSign(AAOptionMsg memory optionMsg, uint256 optionId, TradeType tradeType, bool open) public override {
        ...
        require(SignatureChecker.isValidSignatureNow(
                ...
                ECDSA.toEthSignedMessageHash(
                    keccak256(abi.encodePacked(
                        address(this), optionId, optionMsg.price, optionMsg.signAmount, tradeType, optionMsg.signTime, block.chainid, makerNonce[optionMsg.maker], open
                    )
                    )
                ),
   ...
   */
    function openPosition(
        ...
        require(optionDetail.expiryTime > block.timestamp + freezeTradeBeforeSettle, "option expired");
        optionLockPool[optionId].poolId = optionId;
        uint256 step;
        uint256 checkAmount;
        while (step < optionMsg.length) {
            checkSign(optionMsg[step], optionId, tradeType, true);
   ...
   */
    function closePosition(
        ...
        uint256 checkAmount;
        while (step < optionMsg.length) {
            checkSign(optionMsg[step], optionId, tradeType, false);
            if (tradeType == TradeType.LONG) {
                uint256 quoteAmount;
    ...
    function settleOptions(uint256 optionId) external override returns (uint256 profit, uint256 amount){
        ...
        uint256 spotPrice = spotPrices[optionDetail.expiryTime];
        require(spotPrice > uint256(0), "spotPrice zero");
        ...
        if (optionDetail.optionType) {
            ...
            // call
            // in the time
            if (optionDetail.strikePrice < spotPrice) {
                if (optionBalance > 0) {
                    profit = (spotPrice - optionDetail.strikePrice) * optionBalance / spotPrice;
                    optionToken.burn(msg.sender, optionId, optionBalance);
                    require(baseAsset.transfer(msg.sender, profit), "BaseTransferFailed");
                }
                if (lockBalance > 0) {
                    amount = lockBalance - lockBalance * (spotPrice - optionDetail.strikePrice) / spotPrice;
            ...
            if (optionDetail.strikePrice > spotPrice) {
                if (optionBalance > 0) {
                    profit = (optionDetail.strikePrice - spotPrice) * optionBalance / (10 ** ITokenDecimals(address(baseAsset)).decimals());
                    optionToken.burn(msg.sender, optionId, optionBalance);
                    require(quoteAsset.transfer(msg.sender, profit), "QuoteTransferFailed");
                }
                if (lockBalance > 0) {
                    amount = lockBalance - lockBalance * (optionDetail.strikePrice - spotPrice) / optionDetail.strikePrice;
                    optionProof.burn(msg.sender, optionId, lockBalance);
  ...
  function storeSpotPrice(uint256 expiryTime,uint80 roundId) external override onlyRole(DEFAULT_ADMIN_ROLE) returns (uint256){
      require(expiryTime < block.timestamp, "not store period");
      uint256 spotPrice = spotPrices[expiryTime];
      if (spotPrice <= uint256(0)) {
          uint256 price = getPriceForBase(roundId);
          spotPrices[expiryTime] = price;
      }
      return spotPrices[expiryTime];
  }

    function getPriceForBase(uint80 _roundId) internal view returns (uint256 price){
        (
        uint80 roundId,
        int256 answer,
        ,
    ...
    function emergencySpotPrice(uint256 expiryTime, uint256 price) public onlyRole(DEFAULT_ADMIN_ROLE) returns (uint256){
        require(expiryTime < block.timestamp, "not emergency store period");
        uint256 spotPrice = spotPrices[expiryTime];
        if (spotPrice <= uint256(0)) {
            spotPrices[expiryTime] = price;
        }
