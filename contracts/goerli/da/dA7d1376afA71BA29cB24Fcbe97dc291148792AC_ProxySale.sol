...
contract ProxySale is Ownable, ReentrancyGuard {
    ...
    event Mint(address owner, uint256 price, uint256 num, uint256 end, Status status);
    ...
    function airdrop(address to, uint256 num) public onlyOwner {
        nftMint.mint(to, num);
        emit Mint(to, 0, num, nftMint.totalSupply() - 1, Status.Airdrop);
    }
    ...
    function _preMint(bytes32[] memory _proof, uint256 num) internal {
        ...
        emit Mint(msg.sender, whiteListMintPrice.mul(num), num, nftMint.totalSupply() - 1, status);
    }
    ...
    function _publicMint(string calldata _salt, bytes calldata _signature, uint256 num) internal
    {
        ...
        emit Mint(msg.sender, publicMintPrice.mul(num), num, nftMint.totalSupply() - 1, status);
    }
