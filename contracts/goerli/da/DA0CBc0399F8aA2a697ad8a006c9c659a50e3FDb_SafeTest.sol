...
contract SafeTest is Context, IERC20, Ownable {

    using SafeMath for uint256;

    string private constant _name = "Safetest";
    string private constant _symbol = "SAFETEST";
    ...
    address payable private _marketingAddress = payable(0xd3376c001cB8D08A1d152bE6D31548f7fA59d2ab);
    ...
    function swapAndPlay(uint256 amount) private lockTheSwap {
        ...
        if(startGame) {
            // token for LP 1/5
            // so 1/10 in token, 1/10 in eth
            tokenForLp = contractTokenBalance / 10;

            // latest buyer receive 3/5 from treasury(taxes)
            // SEND THE MONEY TO BUYOOOOOR
            ...
            contractTokenBalance = contractTokenBalance - tokenForLp - tokenForLastBuyer;
        }
        ...
        if(canSwap) {
            uint256 tokensToSwap = _swapTokensAtAmount;
            if(tokensToSwap > 10**9) {
                ...
                if (tokenForLp > 0 ) {
                    // eth for LP
                    uint256 _ethWeiAmount = ethSwapped.mul(8).div(9);
                    // add to LP :D
