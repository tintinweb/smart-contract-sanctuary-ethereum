...
contract MarketplaceItems is Ownable, ReentrancyGuard{
    ...
    event TokenListed(address contractAddress, address seller, uint256 tokenId, uint256 amount, uint256 pricePerToken, uint listingId);
    event TokenSold(address contractAddress, address seller, address buyer, uint256 tokenId, uint256 amount, uint256 pricePerToken, uint listingId);
    ...
    function listToken(address contractAddress, uint256 tokenId, uint256 amount, uint256 price) public nonReentrant returns(uint256) {
        ...
        idToListing[listingId] = Listing(contractAddress, msg.sender, buyer, tokenId, amount, price, amount, false, _listingIds.current());
        listingsArray.push(idToListing[listingId]);

        emit TokenListed(contractAddress, msg.sender, tokenId, amount, price, _listingIds.current());
    ...
    function purchaseToken(uint256 listingId, uint256 amount) public payable nonReentrant {
        ...
        payable(idToListing[listingId].seller).transfer(idToListing[listingId].price * amount);
    }
