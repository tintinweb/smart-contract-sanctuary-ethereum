...
contract TradFiLinesRenderer {
    ...
    function renderFromSeed(uint256 seed, uint256 extraCount, uint256 tokenId) external view returns(string memory svg) {
        ...
        //overall definition        
        svg = string(abi.encodePacked(svg,'{"image_data": "', "<svg width='", Strings.toString(props.frameWidth), "' height='", Strings.toString(props.frameHeight), "' viewBox='0 0 ", Strings.toString(props.viewbox),"' fill='none' xmlns='http://www.w3.org/2000/svg'>"));
        ...
        svg = string(abi.encodePacked(svg, '</svg>",', descriptionSvg));
        return svg;
    }
    ...
    function attributes(Attributes memory attr, uint256 tokenId) public view returns(string memory descriptionJson) {
        ...
        descriptionJson = string(abi.encodePacked(descriptionJson,'"attributes": [ { "trait_type": "Start Type", "value":"', attr.startStatus, '"},{"trait_type": "Ticker Repetition":"',attr.tickerRepetition));
