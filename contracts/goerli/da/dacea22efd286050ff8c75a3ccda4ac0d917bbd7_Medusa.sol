...
contract Medusa is ERC721EnumerableCustom, Ownable {
  ...
  bool public presaleOpen = isGoerli;
  ...
  string private _baseTokenURI = "ipfs://123/";
  ...
  {
    uint256 supply = totalSupply();
    transferUsdcFrom(msg.sender, address(this), presalePrice * quantity);
  ...
  function mintOwner(uint256 quantity, address recipient) external onlyOwner {
    uint256 supply = totalSupply();
    transferUsdcFrom(msg.sender, address(this), publicPrice * quantity);
  ...
  function setPaperKeys(address _paperKey)
    external
    onlyOwner
  {
