...
contract Ferzan is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    FerzanDividendTracker public dividendTracker;
    ...
    uint256 public _BuyBackFerzanFee = 0;
  
    uint256 public _BuyTotalFees = _BuyETHRewardsFee.add(_BuyLiquidityFee).add(_BuyMarketingFee).add(_BuyBackFerzanFee);
    
    uint256 public _SellETHRewardsFee = 4;
    uint256 public _SellLiquidityFee = 1;
    uint256 public _SellMarketingFee = 5;
    uint256 public _SellBackFerzanFee = 0;
    uint256 public _SellTotalFees = _SellETHRewardsFee.add(_SellLiquidityFee).add(_SellMarketingFee).add(_SellBackFerzanFee);
    ...
    address public _FerzanWalletAddress = 0xEeFeDe36a61d32113F5fB19fDFE9b5aE9C4633dE;
    ...
    constructor() ERC20("Ferzan Token", "FRT") {

    	dividendTracker = new FerzanDividendTracker();
    ...
    //    IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3);//testNet
        ...
        dividendTracker.excludeFromDividends(_FerzanWalletAddress);
        ...
        excludeOrIncludeFromFees(_FerzanWalletAddress, true);
        ...
        excludeOrIncludeFromMaxTx(_FerzanWalletAddress, true);
        ...
        excludeOrIncludeFromMaxWallet(_FerzanWalletAddress, true);
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "Ferzan: The dividend tracker already has that address");

        FerzanDividendTracker newDividendTracker = FerzanDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "Ferzan: The new dividend tracker must be owned by the Ferzan token contract");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "Ferzan: The router already has that address");
    ...
    function excludeOrIncludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "Ferzan: Account is already the value of 'excluded'");
    ...
    {
        require(_isExcludedFromMaxTx[account] != excluded, "Ferzan: Account is already the value of 'excluded'");
    ...
    {
        require(_isExcludedFromMaxWallet[account] != excluded, "Ferzan: Account is already the value of 'excluded'");
    ...
    function setFerzanWallet(address payable wallet) external onlyOwner{
         require(wallet != address(0), "Invalid address");
        _FerzanWalletAddress = wallet;
    }

      function setBuyFees(uint256 buyEthRewardFee, uint256 buyLiquidityFee, uint256 buyMarketingFee, uint256 buyBackFerzanFee) external onlyOwner
    {
        _BuyETHRewardsFee = buyEthRewardFee;
        _BuyLiquidityFee = buyLiquidityFee;
        _BuyMarketingFee = buyMarketingFee;
        _BuyBackFerzanFee = buyBackFerzanFee;
        _BuyTotalFees = _BuyETHRewardsFee.add(_BuyLiquidityFee).add(_BuyMarketingFee).add(_BuyBackFerzanFee);

    }

    function setSellFees(uint256 sellEthRewardFee, uint256 sellLiquidityFee, uint256 sellMarketingFee, uint256 sellBackFerzanFee) external onlyOwner
    {
        _SellETHRewardsFee = sellEthRewardFee;
        _SellLiquidityFee = sellLiquidityFee;
        _SellMarketingFee = sellMarketingFee;
        _SellBackFerzanFee = sellBackFerzanFee;
        _SellTotalFees = _SellETHRewardsFee.add(_SellLiquidityFee).add(_SellMarketingFee).add(_SellBackFerzanFee);

    }
      function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "Ferzan: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 800000, "Ferzan: gasForProcessing must be between 200,000 and 800,000");
        require(newValue != gasForProcessing, "Ferzan: Cannot update gasForProcessing to same value");
    ...
    function _transfer(
        ...
        
        {
            ...
            uint256 walletTokens = contractTokenBalance.mul(_SellMarketingFee.add(_SellBackFerzanFee)).div(_SellTotalFees);
            uint256 contractBalance = address(this).balance;
            swapTokensForEth(walletTokens);
            uint256 newBalance = address(this).balance.sub(contractBalance);
            uint256 marketingShare = newBalance.mul(_SellMarketingFee).div(_SellBackFerzanFee.add(_SellMarketingFee));
            uint256 developmentShare = newBalance.sub(marketingShare);
            payable(_marketingWalletAddress).transfer(marketingShare);
            payable(_FerzanWalletAddress).transfer(developmentShare);
...
contract FerzanDividendTracker is Ownable, DividendPayingToken {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("Ferzan_Dividend_Tracker", "Ferzan_Dividend_Tracker") {
    	claimWait = 3600;
        minimumTokenBalanceForDividends = 200000 * (10**18); //must hold 200000+ tokens
    }

    function _transfer(address, address, uint256) internal pure override {
        require(false, "Ferzan_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "Ferzan_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main Ferzan contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "Ferzan_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "Ferzan_Dividend_Tracker: Cannot update claimWait to same value");
