...
contract FriendshipPassport is AccessControl, ReentrancyGuard {
  ...
  event FriendshipMinted(
    ...
    bytes32 indexed friendshipHash
  );
  ...
  mapping(uint256 => bytes32) public tokenIdToFriendshipHash;

  constructor(
    uint256 _fee,
    address payable _treasury,
    address _passportAddress,
    address _hashingModuleAddress
  ) {
    if (_treasury == address(0) || _hashingModuleAddress == address(0))
      revert AddressNotSet();
    _grantRole(DEFAULT_ADMIN_ROLE, msg.sender);
    fee = _fee;
    treasury = _treasury;
    passport = IPassport(_passportAddress);
    hashingModule = HashingModule(_hashingModuleAddress);
  ...
  function setHashingModule(address _newHashingModuleAddress)
    external
    onlyRole(DEFAULT_ADMIN_ROLE)
  {
    if (_newHashingModuleAddress == address(0)) revert AddressNotSet();
    hashingModule = HashingModule(_newHashingModuleAddress);
    emit HashingModuleSet(_newHashingModuleAddress);
  }
  ...
  function mintFriendshipPassport(address _friend)
    external
    payable
    nonReentrant
    ...
    returns (uint256 tokenId1, uint256 tokenId2)
  {
    ...
    if (minted[friendshipHash]) revert FriendshipAlreadyExists();
    minted[friendshipHash] = true;

    tokenId1 = passport.mintPassport(msg.sender);
    tokenId2 = passport.mintPassport(_friend);

    tokenIdToFriendshipHash[tokenId1] = friendshipHash;
    tokenIdToFriendshipHash[tokenId2] = friendshipHash;

    emit FriendshipMinted(
      msg.sender,
      tokenId1,
      _friend,
      tokenId2,
    ...
    hashingModule.storeTokenHash(tokenId1);
    hashingModule.storeTokenHash(tokenId2);
  }
}