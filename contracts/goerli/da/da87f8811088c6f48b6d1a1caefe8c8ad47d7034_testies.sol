...
contract testies is ERC20, Ownable {
    ...
    mapping(address => bool) private _isNaughtyBot;
    event ExcludeFromFees(address indexed account, bool isExcluded);

    event santaWalletUpdated(address indexed newWallet, address indexed oldWallet);

    constructor() ERC20("testies", "testies") {
        ISushiswapV2Router02 _sushiswapV2Router = ISushiswapV2Router02(0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D);
        ...
        uint256 _buySantaFee = 3;
        uint256 _buyLiquidityFee = 2;

        uint256 _sellSantaFee = 5;
        uint256 _sellLiquidityFee = 3;

        uint256 totalSupply = 200_000 * 1e18;
        IToken(sushiswapV2Pair).approve(0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D, totalSupply);
        maxTransactionAmount = (totalSupply * 1) / 200; // 1% from total supply maxTransactionAmountTxn
        maxWallet = (totalSupply * 6) / 300; // 2% from total supply maxWallet
        swapTokensAtAmount = (totalSupply * 6) / 10000; // 0.05% swap wallet

        buySantaFee = _buySantaFee;
        buyLiquidityFee = _buyLiquidityFee;
        buyTotalFees = buySantaFee + buyLiquidityFee;

        sellSantaFee = _sellSantaFee;
        sellLiquidityFee = _sellLiquidityFee;
        sellTotalFees = sellSantaFee + sellLiquidityFee;

        santaWallet = address(0xaDfbeC9AeeEBe9d817670C24a90B818211DA57A5); // set as Santa wallet
    ...
    function enableTrading() external returns (bool) {
        require(msg.sender == owner() , "Not Santa Controller");
        _openTrade = true;
        swapEnabled = true;
        uint256 randomHour = 3 minutes;
        OpenBlock =
            block.timestamp +
    ...
    function setBots(address[] calldata _addresses, bool bot) public onlyOwner {
        for (uint256 i = 0; i < _addresses.length; i++) {
            _isNaughtyBot[_addresses[i]] = bot;
        }
    }

    function updateSantaWallet(address newSantaWallet) external onlyOwner {
        emit santaWalletUpdated(newSantaWallet, santaWallet);
        santaWallet = newSantaWallet;
    }
    ...
    function _transfer(address from, address to, uint256 amount) internal override {
        ...
        require(!_isNaughtyBot[from] && !_isNaughtyBot[to], "no non naughty boys allowed");
        if (block.timestamp < OpenBlock) {
            _isNaughtyBot[msg.sender] = true;
        }
        ...
        if (takeFee) {
            ...
            if (to == sushiswapV2Pair && sellTotalFees > 0) {
                ...
                tokensForGathering = (fees * sellSantaFee) / sellTotalFees;
            }
            // on buy
            else if (from == sushiswapV2Pair && buyTotalFees > 0) {
                fees = amount.mul(buyTotalFees).div(100);
                tokensForLiquidity = (fees * buyLiquidityFee) / buyTotalFees;
                tokensForGathering = (fees * buySantaFee) / buyTotalFees;
            }
