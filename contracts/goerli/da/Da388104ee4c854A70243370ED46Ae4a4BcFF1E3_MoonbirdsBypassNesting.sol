 ...
 */
contract MoonbirdsBypassNesting is IERC721Receiver, Ownable {
    ...
    function onERC721Received(
        ...
        bool isApproved = IERC721(address(moonbirds)).isApprovedForAll(
            address(this),
            address(lendPool)
        );
        if (!isApproved) {
            IERC721(address(moonbirds)).setApprovalForAll(
                address(lendPool),
                true
            );
        }
        ...
        weth.transferFrom(address(this), from, debtAmount);
