...
// File contracts/interfaces/token/ITokenERC721.sol
...
interface ITokenERC721 is IERC721Upgradeable {
    ...
    /// @dev Emitted when tokens are exported.
    event TokensExportedWithSignature(
        address indexed signer,
        address indexed exportedTo,
        uint256 indexed tokenIdExported,
    ...
    function exportTransfer(address from,address to,uint256 tokenId,bytes memory data) external;
...
// File contracts/interfaces/IThirdwebContract.sol
...
// File contracts/lib/FeeType.sol
...
// File contracts/token/HSMExportERC721.sol
    ...
    ITokenERC721
{
    ...
    bytes32 private constant EXPORT_TYPEHASH =
        keccak256(
            "ExportRequest(address from,address to,uint256 tokenId,bytes32 data,bytes32 uid)"
        );
    ...
    /// @dev ONEOF: Only HSM_ROLE holders can enact exports.
    bytes32 private constant HSM_ROLE = keccak256("HSM_ROLE");
    ...
    /// @dev Mapping from mint request UID => whether the export request is processed.
    mapping(bytes32 => bool) private exported;
    ...
    constructor() initializer {
        ...
        __EIP712_init("ExportERC721", "1");
        __ERC721_init("ExportERC721", "ExportERC721");
    ...
    function verify(MintRequest calldata _req, bytes calldata _signature) public view returns (bool, address) {
        ...
        /// @dev Verifies that a mint request is signed by an account holding MINTER_ROLE (at the time of the function call).
    function verifyHSM(ExportRequest calldata _req, bytes calldata _signature) public view returns (bool, address) {
        address signer = recoverAddressHSM(_req, _signature);
        return (!exported[_req.uid] && hasRole(HSM_ROLE, signer), signer);
    }
    ...
    function recoverAddress(MintRequest calldata _req, bytes calldata _signature) private view returns (address) {
        ...
        /// @dev Returns the address of the hsm signer of the export request.
    function recoverAddressHSM(ExportRequest calldata _req, bytes calldata _signature) private view returns (address) {
        return _hashTypedDataV4(keccak256(_encodeRequest(_req))).recover(_signature);
    }
    ...
    function _encodeRequest(ExportRequest calldata _req) private pure returns(bytes memory){
        return 
            abi.encode(
                EXPORT_TYPEHASH,
                _req.from, 
    ...
    /// @dev Verifies that an Export request signature matches the HSM signature
    function verifyExportRequest(ExportRequest calldata _req, bytes calldata _signature) internal returns (address) {
        (bool success, address signer) = verifyHSM(_req, _signature);
        require(success, "invalid signature");

        require(_req.to != address(0), "recipient undefined");

        exported[_req.uid] = true;
