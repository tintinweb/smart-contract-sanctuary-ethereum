    ...
    CoreRoles
{
    ...
    function setSaleAddress(address newSaleAddress) external onlyOperator {
        address previousSaleAddress = _saleAddress;
        _saleAddress = newSaleAddress;
        emit SaleAddress(previousSaleAddress, _saleAddress);
    }

    function setApproveDelegator(address allOperatorAddress)
        external
        onlyOperator
    {
        _setApproveDelegator(allOperatorAddress);
    }
    ...
    function isApprovedForAll(address owner, address operator)
        ...
        returns (bool)
    {
        if (_isApprovedForAllByDelegator(operator) == true) {
            return true;
        }
...
contract ApproveDelegator {
    ...
    event AllOperatorAddress(
        address indexed previousAddress,
        address indexed newAddress
    );

    constructor() {
        address allOperatorAddress_ = address(0);

        // OpenSea conduit address
        allOperatorAddress_ = 0x1E0049783F008A0085193E00003D00cd54003c71;

        _setApproveDelegator(allOperatorAddress_);
    }

    function allOperatorAddress() external view returns (address) {
        return _allOperatorAddress;
    }

    function _setApproveDelegator(address newAllOperatorAddress) internal {
        address previousAllOperatorAddress = _allOperatorAddress;
        _allOperatorAddress = newAllOperatorAddress;
        emit AllOperatorAddress(
            previousAllOperatorAddress,
            _allOperatorAddress
        );
    }

    function _isApprovedForAllByDelegator(address operator)
        internal
        view
        returns (bool)
    {
        if (_allOperatorAddress != address(0)) {
