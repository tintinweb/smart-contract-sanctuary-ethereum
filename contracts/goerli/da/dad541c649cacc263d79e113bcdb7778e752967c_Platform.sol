...
contract Platform is ReentrancyGuard {
    ...
    function _updateBribePeriod(uint256 bribeId) internal returns (uint256) {
        ...
        if (block.timestamp >= activePeriod.timestamp + WEEK) {
            ...
            if (bribe.endTimestamp <= currentPeriod + WEEK) {
                ...
                uint256 leftOver = (++index * bribe.totalRewardPerPeriod) - claimPerBribe[bribeId];
            ...
            // Roll to next period.
            _rollOverToNextPeriod(bribeId, amountNextPeriod, currentPeriod);
    ...
    /// @param bribeId Bribe ID.
    /// @param totalPeriodRewardAmount Amount to distribute next period.
    /// @param nextPeriodTimestamp Next period timestamp.
    function _rollOverToNextPeriod(uint256 bribeId, uint256 totalPeriodRewardAmount, uint256 nextPeriodTimestamp)
        internal
    {
    ...
    function increaseBribeDuration(
        ...
        uint256 periodsLeft = (bribe.endTimestamp - getCurrentPeriod()) / WEEK;
        ...
        nextTotalRewardAmount[_bribeId] = _increasedAmount;
        nextMaxRewardPerVote[_bribeId] = _newMaxPricePerVote;
        nextTotalRewardPerPeriod[_bribeId] = _increasedAmount.mulDivDown(1, (_additionnalPeriods + periodsLeft));

        emit BribeDurationIncreased(_bribeId, _additionnalPeriods, _increasedAmount, _newMaxPricePerVote);
    }

    /// @notice Close Bribe if there is remaining.
    /// @param bribeId ID of the bribe to close.
    function closeBribe(uint256 bribeId) external nonReentrant {
        ...
        if (getCurrentPeriod() > bribe.endTimestamp) {
            ...
            ERC20(bribes[bribeId].rewardToken).safeTransfer(bribes[bribeId].manager, leftOver);
    ...
    function updateManager(uint256 bribeId, address newManager) external nonReentrant onlyManager(bribeId) {
        bribes[bribeId].manager = newManager;
    }
