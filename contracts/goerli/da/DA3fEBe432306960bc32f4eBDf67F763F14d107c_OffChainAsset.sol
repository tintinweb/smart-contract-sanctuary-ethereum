 ...
 */
contract OffChainAsset is Owned, IAsset {
     ...
     * @param sweep_amount The amount of sweep to deposit in the asset.
     * @param usdx_amount The amount of usdx to deposit in the asset.
     * @dev tracks the time when current_value was updated.
     */
    function deposit(uint256 sweep_amount, uint256 usdx_amount) public onlyStabilizer {
        if (wallet == address(0)) { revert WalletNotSet(); }

        TransferHelper.safeTransferFrom( address(sweep), stabilizer, wallet, sweep_amount);
        TransferHelper.safeTransferFrom(
            address(usdx), stabilizer, wallet, usdx_amount);

        uint256 sweep_in_usdx = SWEEPinUSDX(
            sweep_amount,
            sweep.current_target_price()
        );

        current_value += sweep_in_usdx + usdx_amount;
     ...
     */
    function SWEEPinUSDX(uint256 amount, uint256 price) internal view returns (uint256) {
        return
            (amount * price * (10**usdx.decimals())) /
...
interface IAsset {
    ...
    function deposit(uint256 sweep_amount, uint256 usdx_amount) external;
...
contract Asset {
     ...
     * @param usdx_amount Amount to be changed in the AMM.
     * @param sweep_price Price of Sweep per USDX from AMM.
     * @dev Increases the sweep balance and decrease usdx balance.
     */
    function buy(uint256 usdx_amount, uint256 sweep_price) internal {
        uint256 usdx_balance = usdx.balanceOf(address(this));
        if (usdx_balance < usdx_amount) { revert NotEnoughBalance(); }

        TransferHelper.safeApprove(address(usdx), address(amm), usdx_balance);
        amm.swapExactInput(address(usdx), address(sweep), usdx_balance);

        sweep.refreshTargetPrice(sweep_price);
        emit Bought(usdx_amount);
    }
     ...
     */
    function sell(uint256 sweep_amount) internal {
        ...
        amm.swapExactInput(address(sweep), address(usdx), sweep_amount);
        sweep.refreshTargetPrice(sweep_price);
