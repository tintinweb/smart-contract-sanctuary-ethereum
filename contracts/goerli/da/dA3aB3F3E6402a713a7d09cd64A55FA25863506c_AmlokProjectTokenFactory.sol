...
contract AmlokProjectTokenFactory {
    ...
    event SetCancelTime(address project, uint256 time, string document);
    ...
    function createProject(
        ...
        string memory tokenTypesUri, uint256[] memory tokenTypesQty, IERC20 token, uint256 price,
        ...
        AmlokProjectTokenUpgradable(clone).initialize(contractUri, name, symbol,
            tokenTypesUri, tokenTypesQty, token, price, maxQty, cancelTime, msg.sender, _royaltyReceiver, this);
    ...
    function setCancelTime(uint time, string memory document) onlyProject public {
        emit SetCancelTime(msg.sender, time, document);
    }
...
ERC721EnumerableUpgradeable, OwnableUpgradeable, ERC721Holder, ERC721BurnableUpgradeable, ReentrancyGuard {
    ...
    function initialize(string memory contractUri, string memory name, string memory symbol,
        string memory tokenTypesUri, uint256[] memory tokenTypesQty, IERC20 token, uint256 price, uint256 maxQty,
        uint256 cancelTime, address owner, address royaltyReceiver, AmlokProjectTokenFactory factory) initializer public {
        ...
        _factory.setCancelTime(_cancelTime, "");
    }
    ...
    function buy(uint[] memory tokenTypes) nonReentrant external {
        ...
        for (uint256 i = 0; i < tokenTypes.length; i++) {
            ...
            nftSold += _tokenTypeQty[tokenTypeIndex];
        }
        _totalSell += nftSold;
        require(_totalSell <= _maxQty, "buy: buy more then limit");
        _token.safeTransferFrom(msg.sender, address(this), paymentRequire);
        _factory.investorAction(msg.sender, AmlokProjectTokenFactory.InvestorActionType.Buy, nftSold);
        if (_totalSell == _maxQty) {
    ...
    function reFound() nonReentrant external {
        ...
        if (reFoundAmount > 0) {
            _token.safeTransfer(msg.sender, reFoundAmount * _price);
    ...
    function claimDistributions(uint256[] memory distributions) nonReentrant external {
        ...
        uint256 claimedAmount = totalAmount * notClaimedTokens / _totalSell;
    ...
    function manualTransfer(uint[] memory tokenTypes, address to, string memory documentUri) external onlyOwner {
        ...
        _factory.investorAction(to, AmlokProjectTokenFactory.InvestorActionType.Buy, nftSold);
        _factory.manualTransfer(to, nftSold, documentUri);
    }
    ...
    function setCancelTime(uint256 cancelTime, string memory documentUri) external onlyOwner {
        ...
        _cancelTime = cancelTime;
        _factory.setCancelTime(_cancelTime, documentUri);
    }
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        ...
        return string(abi.encodePacked(_tokenTypesUri, getQtyByTokenId(tokenId).toString(), '.json'));
    }
    ...
    function getTotalSell() public view returns (uint256) {
        return _totalSell;
    }
    ...
    function getTokenTypeUri() external view virtual returns (string memory) {
        return _tokenTypesUri;
    }
