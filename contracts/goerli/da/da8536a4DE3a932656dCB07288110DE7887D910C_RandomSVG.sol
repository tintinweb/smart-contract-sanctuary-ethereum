...
contract RandomSVG is ERC721URIStorage, VRFConsumerBase, Ownable {
    ...
    //number of lines to draw, i.e the paths
    uint256 public maxNumberOfPaths;
    //number of specifications for each path
    uint256 public maxNumberOfPathCommands;
    //for height, width = 500, 500
    uint256 public size;
    ...
    constructor(
        ...
        size = 500;
        //we just chose these 2 cmds, but to create even more randomness we can have additional cmds
        pathCommands = ["M", "L"];
    ...
    function fulfillRandomness(bytes32 requestId, uint256 randomNumber) internal override {
        // as this method wil be called by the chainlink nodes, we cant use the msg.sender to know the owner
        //hence we created a mapping tokenId to Sender!
        address nftOwner = requestIdToSender[requestId];
        uint256 tokenId = requestIdToTokenId[requestId];

        //we are just minting the nft, bu attaching the URI is the main task
        _safeMint(nftOwner, tokenId);
        tokenIdToRandomNumber[tokenId] = randomNumber;
        emit CreatedUnfinishedRandomSVG(tokenId, randomNumber);
    }

    //example of an SVG we are generating, here xmlns is mandatory and w3 means worldwide web consortium standards
    //<svg xmlns="http://www.w3.org/2000/svg" height='500' width='500'><path d='T 454 148 M 88 395' fill='black' stroke='blue'/><path d='M 337 305' fill='black' stroke='black'/><path d='L 213 373M 409 313' fill='black' stroke='blue'/><path d='M 220 490M 133 162L 467 365 M 369 468' fill='black' stroke='red'/></svg>

    // we will use single quotes everywhere inside the svg string
    function generateSVG(uint256 _randomness) public view returns (string memory finalSvg) {
        ...
        for (uint256 i = 0; i < numberOfPaths; i++) {
            ...
            // its bcoz, we just dont want both num of paths and each path length to be same
    ...
    function generatePath(uint256 _randomness) public view returns (string memory pathSvg) {
        ...
        for (uint256 i = 0; i < numberOfPathCommands; i++) {
            ...
            //even here, we are just generating a diffrnt new random num
