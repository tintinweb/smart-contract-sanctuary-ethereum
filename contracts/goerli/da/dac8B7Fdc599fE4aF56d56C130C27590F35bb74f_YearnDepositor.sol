...
import { IYVault } from "@gearbox-protocol/integrations-v2/contracts/integrations/yearn/IYVault.sol";
...
import { YearnV2_Calls, YearnV2_Multicaller } from "@gearbox-protocol/integrations-v2/contracts/multicall/yearn/YearnV2_Calls.sol";

contract YearnDepositor is YearnHelper, IDepositor {
    ...
    RouterComponent public override getComponentId =
        RouterComponent.YEARN_DEPOSITOR;

    // Contract version
    uint256 public constant override version = 1;

    constructor(address _router) YearnHelper(_router) {}

    /// @dev Deposits amountIn into desired LP token
    function deposit(
        uint256 amountIn,
        address tokenOut,
    ...
    {
        address adapter = findYearnAdapter(tokenOut, task);
        address tokenIn = IYVault(adapter).token();
        return _deposit(tokenIn, tokenOut, amountIn, task, adapter, false);
    }

    /// @dev Deposits all tokens into desired LP one
        ...
        returns (uint256 amountOut, StrategyPathTask memory)
    {
        ...
        return
            _deposit(
                tokenIn,
                tokenOut,
                task.balances.getBalance(tokenIn),
                task,
                adapter,
                true
            );
    ...
    function _deposit(
        ...
        amountOut = estimateDeposit(adapter, amountIn);
        amountOut = task.amountOutWithSlippage(amountOut);

        task.balances.subBalance(tokenIn, amountIn);
        task.balances.addBalance(tokenOut, amountOut);

        MultiCall memory call = isAll
            ? YearnV2_Multicaller(adapter).deposit()
            : YearnV2_Multicaller(adapter).deposit(amountIn);
        task.calls = task.calls.append(call);
        task.gasUsage += gasUsageByAdapterAndTokens(adapter, tokenIn, tokenOut);

        return (amountOut, task);
    }

    function estimateDeposit(address yVault, uint256 amount)
        public
        view
        returns (uint256)
    {
        return
            (amount * (10**IYVault(yVault).decimals())) /
            IYVault(yVault).pricePerShare();
    }
}
...
import { TokenType } from "./IRouter.sol";
...
import { TokenType } from "@gearbox-protocol/integrations-v2/contracts/integrations/TokenType.sol";
...
import { IAdapter, AdapterType } from "@gearbox-protocol/core-v2/contracts/interfaces/adapters/IAdapter.sol";
import { ICurveV1Adapter } from "@gearbox-protocol/integrations-v2/contracts/interfaces/curve/ICurveV1Adapter.sol";
...
import { ILPWorker } from "../interfaces/ILPWorker.sol";

abstract contract YearnHelper is GasConsumer, ILPWorker {
    using AddressList for address[];
    using StrategyPathTaskOps for StrategyPathTask;

    constructor(address router) GasConsumer(router) {}
    ...
    {
        address adapter = findYearnAdapter(tokenOut, task);
        tokenIn = IYVault(adapter).token();
        return (tokenIn, task);
    }

    function filterYearnAdapters(address[] memory adapters)
        public
        pure
        returns (address[] memory result)
    {
        uint256 len = adapters.length; // F: [YPF-1]
        result = new address[](len); // F: [YPF-1]
        uint256 found;
        unchecked {
            for (uint256 i; i < len; ++i) {
                address adapter = adapters[i]; // F: [YPF-1]
                if (adapter == address(0)) continue;
                if (
                    IAdapter(adapter)._gearboxAdapterType() ==
                    AdapterType.YEARN_V2
                ) {
                    result[found] = adapter; // F: [YPF-1]
                    found++; // F: [YPF-1]
                }
            }

            if (found < len) {
                return result.copy(found); // F: [YPF-1]
            }
        }
    }

    function findYearnAdapter(address token, StrategyPathTask memory task)
        internal
        view
        returns (address)
    {
        address vaultAdapter = task.getTokenDepositAdapter(token);
        ...
        unchecked {
            for (uint256 i; i < len; ++i) {
                address adapter = task.adapters[i]; //
                if (adapter == address(0)) continue;
                if (
                    (IAdapter(adapter)._gearboxAdapterType() ==
                        AdapterType.YEARN_V2) &&
                    ((IAdapter(adapter).targetContract() == token))
                ) {
                    task.addTokenDepositAdapter(token, adapter);
        ...
        revert("YearnHelper: Adapter not found");
    }
}
...
interface ILPWorker is IRouterComponent {
    function getUnderlying(address tokenOut, StrategyPathTask memory task)
        external
        view
        returns (address tokenIn, StrategyPathTask memory);
...
interface ILPWorkerOptions is IRouterComponent {
