...
contract AggregatePool is ERC20Permit, TransferNFT, IERC721Receiver, IAggregatePool {
     ...
     */
    function release(address _nft, uint256 _id) external onlyOwner releasable(_nft, _id) {
        bytes32 fId = getFurionId(_nft, _id);
    ...
    function _collectFee(uint256 _amount) private {
   	...
   	function _store(address _nft, uint256 _id, bool _updateNow) private {
        uint256 nftPrice = oracle.getNFTPrice(_nft, _id);
        uint256 fftPrice = refPricePerFFT();
   	...
   	function _lock(address _nft, uint256 _id, bool _updateNow) private returns (uint256) {
   		_transferInNFT(_nft, _id);
   	...
   	function _buy(address _nft, uint256 _id, bool _updateNow) private {
   		if (_updateNow) {
   			_collectFee(buyFee);
