...
abstract contract Lander {
    ...
    function farmConditionVerifier(
        ...
        address to,
        uint256 minTerm,
        uint256 minStableTerm,
        uint256 maxTerm,
        uint256 maxStableTerm,
        uint256 maxFarmSize
    ) external virtual;
...
contract Farm is Ownable, ERC721AWithRoyalties, Pausable, PaymentSplitter {
    ...
    uint256 private minDestablingFee;
    ...
    function wrapLandtoFarmReq(
        ...
        require(
            destablingFee >= minDestablingFee,
            "DESTABLING FEE MUST BE HIGHER THAN THE MINIMUM"
        );

        if (stableID == 0) {
            require(
                LandContract.FreeStable(ids[0]) == true,
                "LAND MUST HAVE STABLE FUNCTIONALITY"
            );
        } else {
            Lander StableContract = Lander(StableAddress);
            require(
                StableContract.ownerOf(stableID) == msg.sender,
                "MUST OWN STABLE TOKEN"
            );
            StableContract.transferFrom(msg.sender, address(this), stableID);
        }
    ...
    function unwrapFarmtoLand(uint256 farmID) external {
        ...
        transferFrom(msg.sender, address(this), farmID);
        Lander LandContract = Lander(LandAddress);
    ...
    function stableHorse(
        ...
        refreshContracts();
        require(ownerOf(farmID) != address(this), "FARM MUST EXIST");
        require(
        ...
        require(
            HorseContract.checkStable(horseID) == 0,
    ...
    function _removeRequest(
        ...
        stableRequestsByHorseID[horseID][reqnum] = stableRequestsByHorseID[
            horseID
        ][stableRequestsByHorseID[horseID].length - 1];
        stableRequestsByHorseID[farmID].pop;
    }
    ...
    function removeRequest(
        ...
        require(
            ownerOf(farmID) == msg.sender ||
                HorseContract.ownerOf(horseID) == msg.sender,
            "MUST OWN FARM OR HORSE TO REMOVE REQUESTS"
        );
        _removeRequest(horseID, farmID, reqnum);
    }
