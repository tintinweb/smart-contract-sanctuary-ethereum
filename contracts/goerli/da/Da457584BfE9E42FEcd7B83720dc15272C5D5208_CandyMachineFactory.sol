...
contract CandyMachineFactory is ICandyMachineFactory, Initializable, UUPSUpgradeable, OwnableUpgradeable {
  ...
  function newCandyMachine(string[] calldata metadataURIs) external override returns(address) {
    CandyMachine candyMachine = new CandyMachine(metadataURIs, msg.sender);
...
contract CandyMachineStorage {
  ...
  // uint256 internal requestsOutstanding;
...
contract CandyMachine is CandyMachineStorage, ERC1155URIStorage, VRFV2WrapperConsumerBase {
   ...
   * Initalizes the state variables.
   */
  constructor(string[] memory metadataURIs, address ownerArg)
    ERC1155('')
    VRFV2WrapperConsumerBase(linkAddress, 0x708701a1DfF4f478de54383E49a627eD4852C816)
  {
  ...
  /*
   * @notice Generates a pseudo-random number between 0 and the difference of the numURIsExisting and nonce state variable.
   * @returns uint256 the pseudo-randmly generated number.
   */
  // function _randomNumber() internal view onlyOwner returns(uint256) {
  //   return uint(keccak256(abi.encodePacked(block.timestamp, msg.sender, nonce))) % (numURIsExisting - nonce);
  // }
  ...
  //   require(nonce < numURIsExisting, 'CM depleted or cancelled');
  ...
  {
    // require(nonce/* + requestsOutstanding*/ < numURIsExisting, 'CM depleted or cancelled');
    ...
    //   1
    // );
    s_requests[requestRandomness(
      100000,
      3,
      1
    )] = recipient;/*RequestStatus({
      // paid: VRF_V2_WRAPPER.calculateRequestPrice(100000),
      fulfilled: false,
      recipient: recipient
    });*/
  ...
  function fulfillRandomWords(
    ...
    // address requestStatus = s_requests[_requestId];
    require(s_requests[_requestId] != address(0) && nonce < numURIsExisting, "no request or CM empty");
    // require(nonce < numURIsExisting, 'CM depleted or cancelled');
    ...
    _mint(s_requests[_requestId], nonce, 1, "0x00");
  ...
  //   external
  //   view
  //   returns (uint256 paid, bool fulfilled, address recipient)
  // {
  //   require(requestStatus.paid > 0, "request not found");
  //   RequestStatus memory request = requestStatus;
  //   return (request.paid, request.fulfilled, request.recipient);
  // }
