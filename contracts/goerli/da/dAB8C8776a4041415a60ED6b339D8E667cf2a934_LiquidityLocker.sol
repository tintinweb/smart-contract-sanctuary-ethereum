...
contract BaseLocker is OwnableUpgradeable {
    ...
    function hasLockEnded(
        uint256 idx,
        uint256 tokenID
    ) public view returns (bool) {
        Lock memory lock = locks[idx];
        if (block.timestamp > lock.unlockTime && lock.tokenID == tokenID)
            return true;
        return false;
    }
    ...
    function _contain(
        ...
        for (uint256 i = 0; i < locksOf.length; i++) {
            if (locks[locksOf[i]].tokenID == tokenID) {
                return true;
            }
        }
        return false;
...
contract BuyerLocker is BaseLocker {
    ...
    function _payment() internal {
        require(tokenToBuy != address(0), "specify the token to buy");
...
contract LiquidityLocker is BuyerLocker {
    ...
    function startLock(
        ...
        require(msg.value >= fee, "sender must send minimum :fee");
        require(
            INonfungiblePositionManager(uniswapNFT).getApproved(tokenID) ==
                address(this),
            "approve locker is not approved"
        );
        _payment();
    ...
    function updateLock(
        ...
        require(msg.value >= fee, "sender must send minimum :fee");
        require(
            _isWithdrawalAddress(msg.sender),
            "the withdrawalAddress dont have nft"
        );
        require(
            _contain(msg.sender, tokenID),
            "the address is not authorized to edit"
        );
        _payment();
    ...
    function transfertNFT(
        ...
        require(
            _contain(msg.sender, tokenID),
            "the address is not authorized to transfert"
        );
        _payment();
        Lock storage lock = locks[idx];
        _updateLock(idx, newWithdrawalAddress, lock.unlockTime);
        emit TransferNFT(msg.sender, newWithdrawalAddress, tokenID);
    }

    function sendBackNFT(uint256 idx, uint256 tokenID) external {
        require(
            _isWithdrawalAddress(msg.sender),
            "the withdrawalAddress dont have nft"
        );
        require(
            _contain(msg.sender, tokenID),
            "the address is not authorized to retrieve this nft"
        );
        require(hasLockEnded(idx, tokenID), "the lock is still in progress");
        address withdrawalAddress = msg.sender;
        ...
        emit TransferNFT(address(this), withdrawalAddress, tokenID);   
    }

    function collectFees(uint256 idx) public returns (uint256 amount0, uint256 amount1){
        require(
            _isWithdrawalAddress(msg.sender),
            "the withdrawalAddress dont have nft"
        );
        Lock memory lock = locks[idx];
        require(
            _contain(msg.sender, lock.tokenID),
            "the address is not authorized to collect fee"
        );
        (
            ,
            ,
