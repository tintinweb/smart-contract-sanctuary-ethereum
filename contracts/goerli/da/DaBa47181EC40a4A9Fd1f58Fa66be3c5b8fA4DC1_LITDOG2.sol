...
contract LITDOG2 is ERC20, Ownable {
    ...
    uint256 public totalFees = _fundingFee + _liquidityFee + _BurningFee + utilityFee + _MarketingFee;
    ...
    function addSwapTreshold(uint256 _percent) public onlyOwner {
        swapAt = (totalSupply() * _percent) / 1000;
     ...
     */
     ...
     function setTaxWallets(address fundingWallet,address LPAddress, address MarketingAddr) public onlyOwner{
    ...
    function updateFees(
        ...
        require(fundingFee + liquidityFee + BurningFee + marketingFees <= 10);
        require(utilityFees < 5);
        utilityFee = utilityFees;
        _fundingFee = fundingFee;
        _liquidityFee = liquidityFee;
        _BurningFee = BurningFee;
        _MarketingFee = marketingFees;
        totalFees = fundingFee + liquidityFee + BurningFee + utilityFees + marketingFees;
    }
    ...
    function _transfer(
        ...
        if (
            ...
            if (!tradingLive)
                require(
                    _isExcludedFromFees[from] || _isExcludedFromFees[to] || whiteListed[tx.origin],
        ...
        if (takeFee) {
            ...
            if(to == uniswapV2Pair){
                ...
                feePercent = sellFee + utilityFee + _MarketingFee;
            }else if(from == uniswapV2Pair){
                require(buyStatus,"Buy status is closed");
                feePercent = buyFee + utilityFee + _MarketingFee;
            }
            ...
            _tokensForBurning += (fees.mul(_BurningFee)).div(totalFees);
    ...
    function swapBack() public {
        ...
        _swapTokensForETH(_tokensForMarketing);
        ...
        uint256 remainingBalance = balanceOf(address(this)) - _tokensForBurning;
