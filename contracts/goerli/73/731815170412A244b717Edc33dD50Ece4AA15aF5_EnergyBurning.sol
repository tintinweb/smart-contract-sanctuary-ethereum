...
contract EnergyBurning is Ownable, Pausable, ReentrancyGuard {
    ...
    function claim(
        ...
        require(maxClaimsPerTx >= amount, "Exceeded max claim limit");

        uint256 periodId = energyConverter.getCurrentPeriodId();
        uint256 availableEnergy = energyConverter.getEnergy(msg.sender, periodId);
        uint256 energyToBurn = amount * energyAmountPerClaim;
        uint256 rewardToSend = amount * lpTokenAmountPerClaim;
        require(availableEnergy >= energyToBurn, "Insufficient asto energy");
        require(claimedLP[voidId] + rewardToSend <= maxLPAmountPerVoid, "Exceeded max LP limit");
        require(lpToken.balanceOf(address(this)) >= rewardToSend, "Insufficient LP balance");

        nonces[msg.sender]++;
        claimedLP[voidId] += rewardToSend;

        energyConverter.useEnergy(msg.sender, periodId, energyToBurn);
     ...
     */
    function getMaxClaimPerVoid() external returns (uint256) {
     ...
     */
    function getClaimedCount(uint256 voidId) external returns (uint256) {
     ...
     */
    function isFilled(uint256 voidId) external returns (bool) {
