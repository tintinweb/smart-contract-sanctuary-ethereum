...
contract fullflash {
    ...
    function transferWethtocontract(uint256 am) public {
        IWETH(weth_token).transferFrom(msg.sender, address(this), am);
    }

    function performWithdraw(address token, uint256 am)
        public
        onlyOwner
        returns (bool)
    {
        return IERC20(token).transferFrom(address(this), msg.sender, am);
    }
