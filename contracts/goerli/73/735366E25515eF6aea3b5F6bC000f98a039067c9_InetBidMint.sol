...
contract InetBidMint is InetBid, AccessControl, ERC2981 {
    ...
    */
        ...
        onlyRole(SUPPORT_ROLE)
    {
        ...
        for (uint256 i = 0; i < _addresses.length; ) {
            _processBidderRefunds(_addresses[i], _refundValues.length);
            unchecked {
                i += 1;
            }
        }
    ...
    function sendEth(address _address, uint256 _sendEthAmount) external onlyRole(SUPPORT_ROLE){
        (bool success, ) = _address.call{value: _sendEthAmount}("");
