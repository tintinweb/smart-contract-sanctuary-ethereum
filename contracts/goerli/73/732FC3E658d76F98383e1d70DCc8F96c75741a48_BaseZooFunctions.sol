...
contract BaseZooFunctions is Ownable, VRFConsumerBase
{
	...
	{
		require(randomResult == 0 && battles.getCurrentStage() == Stage.FifthStage, "Random was reset");
...
contract NftBattleArena
{
	...
	function chooseWinnerInPair(uint256 pairIndex) external
	{
		...
		require(zooFunctions.randomFulfilled() == true, "Random wasn't generated");
	...
	function calculateIncentiveRewardForVoter(uint256 votingPositionId) external only(nftVotingPosition) returns (uint256)
	{
		...
		uint256 start = veZoo.getEpochNumber(epochsStarts[votingPosition.startEpoch]);
		uint256 reward = 0;
	...
	function calculateIncentiveRewardForStaker(uint256 stakingPositionId) external only(nftStakingPosition) returns (uint256)
	{
		...
		uint256 reward = 0;
		uint256 start = veZoo.getEpochNumber(epochsStarts[stakingPosition.startEpoch]);
	...
	{
		for (uint256 i = lastUpdatesOfStakedNumbers[collection]; i <= currentEpoch; i++)
		{
