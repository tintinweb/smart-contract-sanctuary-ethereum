...
contract SmartVault is ERC721PresetMinterPauserAutoId, Ownable {
    ...
    function createSmartVault(
        ...
        // wrap ETH
        IWETH9(wETH).deposit{value: msg.value}(); // wrap only what is needed to pay
        ...
        vaultBalance[msg.sender][_vaultId] += ISwapRouter(_swapRouter).exactInputSingle(params);
    ...
    function cancelSmartVault(uint256 _vaultId) public onlyVaultOwner(_vaultId) onlyVaultReady(_vaultId) {        
        uint256 _returnDeposit = vaultBalance[msg.sender][_vaultId]*(1-processingFee/100);
