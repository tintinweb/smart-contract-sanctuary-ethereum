...
contract Index is Ownable, Initializable {
    ...
    //the addresses of the current tokens
    address[] public tokenscurrent;
    //the percentage fo the current tokens
    uint16[] public percentagescurrent;
    //the addresses of the previous tokens
    address[] public tokensprevious;
    //the percentages of the previous tokens
    //mapping to store staking state of the tokens
    ...
    //To add reward token address in the rewardtokens address array
    function addrewardtokens(address token) external onlyOwner {
        rewardtokens.push(token);
    }

    //To transfer reward tokens from this contract address to the rewardgetter address
    function transferReward(uint256 percent, address getter)
        internal
        onlyOwner
    {   
        console.log("transferrewardinside", percent);
        ...
        uint256 leng = rewardPercent.length;
        uint256[] memory _rewardperc = new uint256[](leng);
        _rewardperc= rewardPercent;

        uint256 lengt = depositers.length;
        address[] memory _depositers = new address[](lengt);
        ...
        for (uint256 i; i < len; i++) {
            ...
            SafeERC20.safeTransfer(
                IERC20(_rewardtokens[i]),
                getter,
                (percent * rewardbalance) / 100
            );
            ...
            for (uint256 j = 0; j < leng; j++) {
                SafeERC20.safeTransfer(
                    IERC20(_rewardtokens[i]),
                    _depositers[j],
                    (_rewardperc[j] * rewardbalance) / 100
                );
            }
        }
    }

    //To get the state of the index contract
    ...
    //returns index current tokens info
    ...
    //returns index previous tokens info
    function getPreviousTokensInfo() external view returns (address[] memory) {
        return (tokensprevious);
    }

    //returns balance of the caller address given token address
    function checkbalancebytoken(address token, address whose)
        internal
        view
        returns (uint256)
    {
        return IERC(token).balanceOf(whose);
    }

    //will be used to deposit the amount of ptoken in the contract
    function deposit(uint256 amount, address depositer) external onlyOwner {
        require(block.timestamp <= depositendtime, "depost time limit passed");

        require(amount >= mindeposit, "amount should be greater than minimlum");
    ...
    //will return the balance of cuurent tokens for the index
    ...
    // internal function to purchase the current tokens
    function _purchase(uint256 _amount, uint256[] calldata _allowedslippage)
        internal
    {
    ...
    //internal function to sell the tokens for getting ptokens
    function _sell(
        address[] memory _tokens,
        uint256[] calldata amounts,
        uint256[] calldata _allowedslippage
    ) internal {
        ...
        for (uint256 i; i < numoftokens; ) {
            SafeERC20.safeIncreaseAllowance(IERC20(tokens[i]), dex, amounts[i]);
    ...
    //main purchase function with required checks for purchase
    function purchase(uint256 amount, uint256[] calldata allowedslippage)
        external
        onlyOwner
    {
        require(
    ...
    //main sell function with required checks for selling tokens
    function sell(
    ...
    // function to distribute ptokens and reward tokens
    function distributeamount() external onlyOwner soldcheck {
        ...
        console.log("inside dist ,,  total depoist",_totaldeposit,"totalbalance",_totalbalance);
        bool rewardtransferwithfeecheck;

        if (_totalbalance > _totaldeposit) {
            uint256 interest = _totalbalance - _totaldeposit;
            uint256 fees = (interest * 10) / 100;
            SafeERC20.safeTransfer(IERC20(ptoken), performancefeegetter, fees);
            rewardtransferwithfeecheck = true;
            _totalbalance -= fees;
        }

        uint256 l = depositers.length;
        address[] memory _depositers = new address[](l);
        uint256 rewardperc;
        _depositers = depositers;
        ...
        for (uint256 i; i < l; ) {
            ...
            SafeERC20.safeTransfer(
                IERC20(ptoken),
                _depositers[i],
                (rewardperc * _totalbalance) / 100
            );

            rewardPercent.push(rewardperc);
        ...
        if (rewardtransferwithfeecheck) {
            transferReward(10, performancefeegetter);
        } else {
            transferReward(0, performancefeegetter);
        }
    }

    //function to sell the tokens before the update
    function rebalancesell(
        ...
        for (uint256 i; i < numoftokens; ) {
            console.log(i, checkbalancebytoken(tokens[i], address(this)));
    ...
    //function to purchase the new tokens after the update
    function rebalancepurchase(
        ...
        _purchase(amount, allowedslippage);
        console.log("re", checkbalancebytoken(ptoken, address(this)));
    ...
    //function to initialize the index contract
    function initialize(
        ...
        dex = _dex;
        ptoken = _ptoken;
        managementfeegetter = _managementfeegetter;
        performancefeegetter = _performancefeegetter;
        _transferOwnership(msg.sender);
    }

    // will return the purchase token for the index contract
    ...
    //function to  update the owner of the index contract
    function updateindexowner(address newowner) external onlyOwner {
        transferOwnership(newowner);
    }

    //function to approve token for staking
    ...
    //function to call any contract function given target address and data
    ...
    //function to approve and stake the purchased token
    ...
    //function to unstake the purchased tokens
    function unstake(
        address token,
        address target,
        bytes memory data
