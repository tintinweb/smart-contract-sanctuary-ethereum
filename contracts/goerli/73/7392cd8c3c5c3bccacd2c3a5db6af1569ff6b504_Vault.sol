...
contract TimeLockContract is TimelockControllerUpgradeable {
  ...
  function initialize(DAOArguments calldata _timelockContractArguments) external initializer{
    __TimelockController_init(_timelockContractArguments._minDelay,_timelockContractArguments._proposers,_timelockContractArguments._executors, address(0));
  }
...
contract Vault {
    ...
    function setupCampaign(
        ...
        // //Revoking role here
        TimeLockContract(newTimelock).revokeRole(TimeLockContract(newTimelock).TIMELOCK_ADMIN_ROLE(), msg.sender);
