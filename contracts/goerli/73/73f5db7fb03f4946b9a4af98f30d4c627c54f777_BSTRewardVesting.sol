...
// File contracts/BSTRewardVesting.sol
...
contract BSTRewardVesting is OwnableUpgradeable {
    ...
    event BurnedReward(address property, address user, uint256 index, uint256 reward);
    ...
    function addVestingInfo(
        ...
        if (reward > 0) {
            require(_bstContract.transferFrom(_bsptStakingContract, address(this), reward), "BSTRewardVesting: Couldn't transfer BST");
    ...
    function enableRewardForProperty(
        address property
    ) external onlyOwner {
        _canWithdrawReward[property] = true;
    }

    function burnReward(address property, address user, uint256 index) external onlyOwner {
        require(!_canWithdrawReward[property], "BSTRewardVesting: Can't burn because reward can be claimed");
        ...
        require(_bstContract.transfer(address(0x000000000000000000000000000000000000dEaD), toBurn), "BSTRewardVesting: Couldn't transfer BST to zero address");
    ...
    function claimReward(address property) external {
        require(_canWithdrawReward[property], "BSTRewardVesting: Reward claiming is not yet enabled");
    ...
    function _calculateRemainingReward(VestInfo memory vestInfo) private view returns (uint256) {
        return _calculateUnlockedRewardAtTimestamp(vestInfo, block.timestamp) - vestInfo.claimedAmount;
    }

    function _calculateUnlockedRewardAtTimestamp(VestInfo memory vestInfo, uint256 time) private pure returns (uint256) {
        if (vestInfo.endVest >= time) {
            return vestInfo.vestAmount;
        }
        return (vestInfo.vestAmount * (time - vestInfo.startVest)) / (vestInfo.endVest - vestInfo.startVest);
    }
    ...
    function calculateRemainingRewardForProperty(address property, address user) external view returns (uint256) {
        uint256 totalAmount = 0;
        for (
            uint256 i = 0;
            i < _vestingInfo[property][user].length;
            i++
        ) {
    ...
    function numberOfEntries(address property, address user) external view returns (uint256) {
        return _vestingInfo[property][user].length;
    }

    function getVestingInfo(address property, address user, uint256 index) external view returns (VestInfo memory) {
        return _vestingInfo[property][user][index];
    }

    function getVestingPeriod() external returns(uint256) {
        return _vestPeriod;
    }


    function calculateTotalReward(address property, address user) external view returns(uint256) {
        uint256 totalAmount = 0;
        for (
            uint256 i = 0;
            i < _vestingInfo[property][user].length;
    ...
    function calculateAlreadyClaimedReward(address property, address user) external view returns(uint256) {
        uint256 totalAmount = 0;
        for (
            uint256 i = 0;
            i < _vestingInfo[property][user].length;
