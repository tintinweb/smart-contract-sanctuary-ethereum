    ...
    VRFConsumerBaseV2
{
    ...
    /// - verifySignature If true, verify that the buyer is in the merkle proof. Otherwise not verify.
    ...
    /// - endAt The time when the raffle is created.
    ...
    /// - Drawed: The raffle is drawed, the winner is determined.
    ...
    /// - beforeSoldCount The number of tickets sold.
    /// - afterSoldCount The number of tickets sold.
    ...
    struct TicketSale {
        bool isRefunded;
        address buyer;
        uint32 beforeSoldCount;
        uint32 afterSoldCount;
        uint256 soldAmount;
    }
    ...
    /// - requested: The request has been sent to the VRF coordinator.
    /// - fulfilled: The request has been fulfilled by the VRF coordinator.
    ...
    /// @notice Return the global config of the contract.
    /// - cancelFee The fee to cancel a raffle.
    /// - refundFee The fee to refund a raffle.
    /// - drawFee The fee to draw a raffle.
    /// - bonusRate The commission rate of a raffle.
    /// - commissionRate The commission rate of a raffle.
    /// - platformWallet The wallet to receive the commission.
    /// - recycleDays The days after drawed the winner to recycle the raffle.
    ...
    /// @param tokenType_ see {TokenType}
    /// @param tokenAddress_ The address of the token contract.
    /// @param tokenId_ The id of the token. If the token is ERC20, the value is 0.
    /// @param tokenAmount_ The amount of the token. If the token is ERC721, the value is 1.
    /// @param pricePerTicket_ The price of each ticket.
    /// @param maxTicketsPerBuyer_ The maximum number of tickets that a buyer can buy.
    /// @param maxTicketsNum_ The maximum number of tickets that can be sold.
    /// @param releaseAt_ The estimated time to draw the winner.
    /// @param verifyBuyerHolderToken_ Verify that the buyer holds the token if the value is not address(0).
    /// @param verifySignature_ If true, verify that the buyer is in the merkle proof. Otherwise not verify.
    /// @param ticketDiscounts_ Sorted array of {TicketDiscount}
    /// @return The id of the raffle.
    /// @dev Create a raffle struct and store it in the raffles array. Then the sell transfer the token to the contract.
    function create(
    ...
    modifier ableToEdit(uint32 id) {
        ...
        require(_raffles[id].totalSoldCount == 0, "Raffle: Already sale");
        _;
    }
    ...
    /// @param id The id of the raffle.
    /// @param ticketNum The number of tickets to buy.
    /// @param signature The signature of the buyer.
    /// @param expireAt The timestamp of the request expired.
    function buyTicket(
        uint32 id,
        uint32 ticketNum,
        uint256 expireAt,
        ...
        if (raffle.verifySignature) {
            bytes32 digest = _hashTypedDataV4(
                keccak256(abi.encode(BUY_TICKET_HASH, id, ticketNum, expireAt))
            );
        ...
        if (raffle.maxTicketsNum > 0) {
            require(
                raffle.totalSoldCount + ticketNum <= raffle.maxTicketsNum,
        ...
        if (raffle.verifyBuyerHolderToken != address(0)) {
            require(
                raffle.verifyBuyerHolderToken._ownerOf(msg.sender),
        ...
        _ticketSales[id].push(
            TicketSale({
                isRefunded: false,
                buyer: msg.sender,
                beforeSoldCount: raffle.totalSoldCount,
                afterSoldCount: raffle.totalSoldCount + ticketNum,
                soldAmount: msg.value
            })
        );

        /// Increase the sold count and amount of the raffle.
        _raffles[id].totalSoldCount += ticketNum;
        _raffles[id].totalSoldAmount += msg.value;
        _ticketCounter[id][msg.sender].totalSoldCount += ticketNum;
    ...
    /// @notice Request random numbers from the VRF coordinator.
    /// @param id The id of the raffle.
        ...
        onlyOnStatus(id, Status.Created)
    {
        ...
        bool isSoldOut = raffle.maxTicketsNum > 0 &&
            raffle.maxTicketsNum <= raffle.totalSoldCount;
        ...
        override
    {
        ...
        uint256 totalSoldCount = _raffles[id].totalSoldCount;
        uint256 winnerNumber = (drawInfo.rawRandomNumber % totalSoldCount) + 1;
        ...
        returns (uint256)
    {
        ...
        while (low < high) {
            ...
            if (array[mid].afterSoldCount > element) {
                high = mid;
            } else {
                low = mid + 1;
            }
        ...
        if (low > 0 && array[low - 1].afterSoldCount == element) {
            return low - 1;
        } else {
            return low;
        }
    ...
    /// 1. The caller need to pay the draw fee to the drawer.
        ...
        onlyOnStatus(id, Status.Drawed)
    {
        ...
        // settlement fees
        uint256 payPlatform = (_raffles[id].totalSoldAmount *
            _globalConfig.commissionRate) / 10000;
        uint256 paySeller = _raffles[id].totalSoldAmount - payPlatform;
    ...
    function _tokenTransferOut(
        ...
        if (tokenType == TokenType.ERC20) {
            tokenAddress._safeApproveErc20(tokenAmount);
        }
    ...
    {
        require(
            config_.bonusRate >= 0 && config_.bonusRate <= 10000,
            "Raffle: Illegal bonusRate"
        );
        require(
            config_.commissionRate >= 0 && config_.commissionRate <= 10000,
    ...
    /// @notice An emergency mechanism to change the information of a specific raffle.
    /// @dev only admin has the permission.
        ...
        for (uint32 i = 0; i < freeTickets.length; i++) {
            _ticketSales[id].push(
                TicketSale({
                    ...
                    beforeSoldCount: _raffles[id].totalSoldCount,
                    afterSoldCount: _raffles[id].totalSoldCount +
            ...
            _raffles[id].totalSoldCount += freeTickets[i].ticketNum;
    ...
    function recycleNoCliamToken(uint32 id, address payable receiver)
        external
        onlyRole(ADMIN)
        onlyOnStatus(id, Status.Drawed)
    {
        ...
        uint256 payPlatform = (_raffles[id].totalSoldAmount *
            _globalConfig.commissionRate) / 10000;
        uint256 remainFunds = _raffles[id].totalSoldAmount - payPlatform;
    ...
    string private constant CONTRACT_NAME = "raffle";
    ...
    bytes32 private constant BUY_TICKET_HASH =
        keccak256("BuyTicket(uint32 id,uint32 ticketNum,uint256 expireAt)");
...
library TokenHelper {
    ...
    function _ownerOf(address tokenAddress, address account)
        internal
        view
        returns (bool)
    {
