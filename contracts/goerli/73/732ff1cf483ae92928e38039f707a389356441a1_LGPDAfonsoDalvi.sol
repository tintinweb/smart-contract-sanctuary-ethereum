...
//Founder Omnes Blockchain e CTO Web3Club and Web3Tech
contract LGPDAfonsoDalvi is ERC20{
...
uint256 public numeroTitulares;
    ...
    //address public Operadorend;
    bool acessardados; //booliana
    ...
    constructor(string memory _Empresa, uint _CNPJ) ERC20("[email protected]", "AFH"){// contrução do contrato pela empresa controladora e ela que define quem será o operador
    ...
    string memory _sobrenome, uint _CPF, address endereco) 
     public { //SomenteAutorizados -- inserir esse modifier se quiser que só os endereços permitidos insiram os dados
        ...
        numeroTitulares++;
        _transfer(address(this), msg.sender, 5000); //quem cadastra ganha tokens
        emit solicitante(msg.sender);
    }
    
    function permitirSolicitacao() public { //função que o titular permite a solicitação dos dados pelos operadores ou controladores
    ...
    function Solcitardados(address _titular) view public 
     returns(string memory, string memory, uint){ //SomenteAutorizados
        require(ProtecaoDados == Dados.solicitados || ProtecaoDados == Dados.soliexclu);
