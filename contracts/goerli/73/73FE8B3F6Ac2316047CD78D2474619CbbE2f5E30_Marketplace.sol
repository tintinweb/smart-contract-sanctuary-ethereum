...
contract Marketplace is Ownable {
	...
	*/    
    ...
    event ArtCreated(bytes32 id, bytes32 providerID, string name, string description, string uri, uint256 price, uint256 totalSupply, uint dateCreated);
    event ArtMinted(bytes32 id, bytes32 providerID, string name, string description, string uri, uint256 price, uint256 totalSupply, uint dateCreated, address owner);
    ...
    */
    ...
    function addArtwork(string memory name, string memory description, string memory uri, uint256 price, uint256 totalSupply) public {
        ...
        bytes32 id = keccak256(abi.encodePacked(msg.sender, name, block.timestamp));
        Art memory art = Art(id, providerID, name, description, uri, price, totalSupply, 0, block.timestamp);

        artsByProvider[providerID].push(id);
        artByID[id] = art;

        emit ArtCreated(art.id, art.providerID, art.name, art.description, art.uri, art.price, art.totalSupply, block.timestamp);
    }
    ...
    function buyArtwork(bytes32 aID) public payable {
        ...
        emit ArtMinted(a.id, a.providerID, a.name, a.description, a.uri, a.totalSupply, a.price, a.dateCreated, msg.sender);
    }
}
