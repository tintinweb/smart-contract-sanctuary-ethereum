...
contract APNFT is ERC721 {
    ...
    constructor(address _acoinAddress) ERC721("New NFT", "NNFT") {
        acoin = ACoin(_acoinAddress);
        uint8 tokenDecimals = acoin.decimals();
    ...
    function createImageNFT(string memory _tokenURI) public returns (bytes32) {
        require(
            tokenCounter <= maxLogo,
            "Maximum number of images has been emitted"
        );
        require(tokenFee <= acoin.balanceOf(address(this)));
