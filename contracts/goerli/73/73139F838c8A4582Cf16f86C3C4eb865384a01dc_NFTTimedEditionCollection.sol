  ...
  TimeLimitedCollection
{
  ...
  function initialize(
    ...
    _initializeLazyMintedCollection(_creator, _approvedMinter, _paymentAddress);
 ...
 * @dev This implements the INFTDropCollectionMint ERC-165 interface.
  ...
  CollectionRoyalties
{
  ...
  function _initializeLazyMintedCollection(
    address payable _creator,
    address _approvedMinter,
    address payable _paymentAddress
  ...
  function supportsInterface(
    ...
    view
    virtual
    override(AccessControlUpgradeable, ERC721Upgradeable, CollectionRoyalties)
    returns (bool isSupported)
  {
    isSupported = interfaceId == type(INFTDropCollectionMint).interfaceId || super.supportsInterface(interfaceId);
  }
}
...
error TimedEditionCollection_Minting_Has_Ended(uint256 mintEndTime);

/**
 * @title Defines an upper limit on the number of tokens which may be minted by this collection.
 * @author HardlyDifficult
 */
abstract contract TimeLimitedCollection is Initializable, LazyMintedCollection {
  ...
  function _initializeTimeLimitedCollection(uint256 _mintEndTime) internal onlyInitializing {
    if (_mintEndTime.hasBeenReached()) {
      revert TimeLimitedCollection_Mint_End_Time_Must_Be_In_The_Future();
    }

    if (_mintEndTime > block.timestamp + (2 * 365 days)) {
  ...
  function mintCountTo(uint16 count, address to) public virtual override returns (uint256 firstTokenId) {
    if (mintEndTime.hasExpired()) {
      revert TimedEditionCollection_Minting_Has_Ended(mintEndTime);
    }
    firstTokenId = super.mintCountTo(count, to);
  }
