...
contract BuyHandler is ReentrancyGuard, BuySignatureUtils {
    ...
    constructor(address proxy, uint256 _privatePrice, uint256 _publicPrice, uint256 _privateStartTime, uint256 _publicStartTime) {
        require(_publicStartTime > _privateStartTime, "BuyHandler: the public time need to be bigger than the private time");
        PROXY = proxy;
    ...
    */
    function setPrivateStartTime(uint256 _privateStartTime) public {
        ...
        require(
            _privateStartTime < publicStartTime,
    ...
    */
    ...
    /**
     * @dev Set start time for mint NFT
     * @param _publicStartTime the time that the owner wants to set is the public start time 
    */
    function setPublicStartTime(uint256 _publicStartTime) public {
        require(
            msg.sender == IAviumNFT(PROXY).owner(),
            "BuyHandler: only AviumNFT's owner"
        );
        require(
            _publicStartTime > privateStartTime,
            "BuyHandler: the public time need to be bigger than the private time"
    ...
    /**
     @dev Set price
     @param _publicPrice the price that the owner want to set is the nft public price
    */
    function setPublicPrice(uint256 _publicPrice) public {
        require(
            msg.sender == IAviumNFT(PROXY).owner(),
            "BuyHandler: only AviumNFT's owner"
        );
    ...
    */
    function buy(
        ...
        if(block.timestamp < privateStartTime) {
            revert("BuyHandler: it haven't been time to start buy nft");
        } else if(block.timestamp >= privateStartTime && block.timestamp < publicStartTime) {
            if(msg.value != _quantity * privatePrice) revert("BuyHandler: your price that you paid is wrong");
            if(addressMintAmount[msg.sender]>=1) revert("BuyHandler: you bought exceed the allowed amount");
        } else if(msg.value != _quantity * publicPrice) revert("BuyHandler: your price that you paid is wrong");
        ...
        addressMintAmount[msg.sender]+=_quantity;
