...
contract JungleJaguars is ERC721, Ownable, ERC2981 {
	using Counters for Counters.Counter;

	// These are important changes so we emit events
	event NewURI(string _newURI);
	event URIFrozen();
	event NewSupply(uint256 _newSupply);
	event Withdrawal(uint256 _amount);
	event NewRoyalty(address indexed _receiver, uint96 _royalty);
	 ...
	 * @param _teamAllocation Number of mints to the project team
	 * @param _whitelist      Addresses to whitelist
	 */
	constructor(
		bytes32 _provenanceHash,
		uint256 _teamAllocation,
		address[] memory _whitelist
	) ERC721("Jungle Jaguars","JAGUAR") {
		...
		for (uint256 i; i < _teamAllocation; i++) {
			_mint(msg.sender, nextId.current());
			nextId.increment();
		}
	...
	/// @notice Mint an NFT to the sender if whitelisted and they have not already minted
	function mintWhitelist() external {
		// Make sure they're whitelisted
		require(whitelisted[msg.sender],"Not whitelisted!");
	
		// Consume their whitelist spot
	...
	/**
	 * @notice Reduces the total supply
	...
	* @param _newRoyalty new royalty in basis points, may not exceed 10% (1,000 BPS)
	*/
	function setRoyalty(uint96 _newRoyalty) external onlyOwner {
		require(_newRoyalty <= 1000,"Royalty > max");
		_setDefaultRoyalty(owner(),_newRoyalty);
		emit NewRoyalty(owner(),_newRoyalty);
	}
	...
	*/
	function supportsInterface(bytes4 interfaceId) public view virtual override(ERC721, ERC2981) returns (bool) {
		return
			interfaceId == type(ERC2981).interfaceId || // Replace this with anything you want to support
