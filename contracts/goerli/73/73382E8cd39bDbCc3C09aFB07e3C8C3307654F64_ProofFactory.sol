...
contract ProofFactoryTokenCutter is Context, IERC20, IERC20Metadata {
    ...
    constructor(
        ...
        totalFee = devFee.add(lpFee).add(reflectionFee).add(revenueFee);
        totalFeeIfSelling = devFeeOnSell
            .add(lpFeeOnSell)
            .add(reflectionFeeOnSell)
            .add(revenueFee);
    ...
    function delBot(address notbot) external onlyProofAdmin {
        bots[notbot] = false;
    }
    ...
    function swapBack() internal lockTheSwap {
        ...
        uint256 revenueBalance = amountETH.mul(revenueFee).div(totalFee);
