    ...
    PausableUpgradeable
{
        ...
        initializer
    {
        ...
        collateralToken = IERC20(_grtAddress);
    }
    ...
    function pause() external whenNotPaused {
        ...
        if (msg.sender != owner()) {
            require(collateralToken.balanceOf(msg.sender) >= collateralAmountToPause, "Not enough collateral");
            collateralToken.transferFrom(msg.sender, address(this), collateralAmountToPause);
        }
        _pause();
    }
    ...
    function rescueERC721(address _token, address _to, uint256 _tokenId) external onlyOwner {
    ...
    /**
     * @dev Emmited when new collateral token address is set to `newCollateralToken`.
     */
    event CollateralTokenChanged(address newCollateralToken);
