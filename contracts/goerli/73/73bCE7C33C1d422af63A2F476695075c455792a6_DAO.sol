...
contract DAO {
    ...
    constructor() {
        god = msg.sender;
        deciders[msg.sender] = true;
        members[msg.sender] = true;
    }

    address public immutable god;
    mapping(address => bool) public deciders;
    ...
    function decideDecider(address _decider) public {
        if (!(msg.sender == god)) {
            revert NotCreater();
        }
        members[_decider] = true;
        deciders[_decider] = true;
    }

    function decideMember(address _member) public {
        if (!deciders[_member]) {
            revert NotDecider();
        }
