...
contract DesupTokenVesting is Ownable, ReentrancyGuard {
    ...
    function investWithEther() public payable {
        ...
        holders[msg.sender].histories.push(
            History(
                block.timestamp,
    ...
    function investWithUSDT(uint256 amount) public {
        ...
        holders[msg.sender].histories.push(
            History(block.timestamp, amount / token_price_for_seedA, 0)
        );
     ...
     */
    function withdraw(uint256 amount, uint16 index) public {
        bool isHolder = isInHolderlists(msg.sender);
        require(isHolder, "Only holders can withdraw");

        uint256 withdrawable_amount = getWithdrawableAmount(msg.sender, index);

        require(amount <= withdrawable_amount, "Withdraw limitation");

        _token.transferFrom(master, msg.sender, amount.mul(10**18));

        holders[msg.sender].histories[index].withdrawed_amount += amount;
    ...
    function getTotalMintedDesupTokenAmount() public view returns (uint256) {
        ...
        for (uint16 i = 0; i < holder_lists.length; i++) {
            for (
                uint16 j = 0;
                j < holders[holder_lists[i]].histories.length;
                j++
            ) {
                total_amount += holders[holder_lists[i]]
                    .histories[j]
                    .purchased_amount;
            }
        }
    ...
    {
        uint256 total_amount;
        for (uint16 i = 0; i < holders[holder_address].histories.length; i++) {
            total_amount += holders[holder_address].histories[i].purchase_date;
        }
        return total_amount;
    }
     ...
     * @param holder_address token holder's address
     */
    function getAllowedAmount(address holder_address, uint16 index)
        public
        view
        returns (uint256)
    {
        ...
        // exceptional case that investor bought our token after tge
        uint256 purchase_date = holders[holder_address]
            .histories[index]
            .purchase_date;

        if (purchase_date >= published_date) {
            // count the time flowed from the moment he bought our token
            passed_time = block.timestamp - purchase_date;
        }
        ...
        if (
            ...
            return holders[holder_address].histories[index].purchased_amount;
        }
        ...
        uint256 allowed_amount = (holders[holder_address]
            .histories[index]
            .purchased_amount * allowed_percentage).div(100);
     ...
     * @param holder_address token holder's address
     */
    function getWithdrawableAmount(address holder_address, uint16 index)
        public
        view
        returns (uint256)
    {
        ...
        uint256 allowed_amount = getAllowedAmount(holder_address, index);
        return
            allowed_amount -
            holders[holder_address].histories[index].withdrawed_amount;
    }
     ...
     * @param holder_address token holder's address
     */
    function getWithdrawedAmount(address holder_address, uint16 index)
        public
        view
        returns (uint256)
    {
        require(isInHolderlists(holder_address), "Not a token holder");
        return holders[holder_address].histories[index].withdrawed_amount;
    }
