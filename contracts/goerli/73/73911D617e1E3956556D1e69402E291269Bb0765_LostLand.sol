...
contract LostLand is ERC721Enumerable, Ownable, Pausable {
    ...
    function mintBatch(address to, uint256[] memory tokenIds_) public onlyMinter {
        require(tokenIds_.length > 0, "array length unequal");
        for (uint i = 0; i < tokenIds_.length; i++) {
            ERC721._safeMint(to, tokenIds_[i]);
        }
    }
