    ...
    ERC721Votes
{
    ...
    error UnWantedMarketplace();
    error UnapprovedContract();

    ///@dev The internal token ID tracker
    uint256 private _currentTokenId;

    /// @notice Contract URI used by OpenSea to get contract details (owner, royalties...)
    string private _contractUriIpfsHash =
        "bafkreicg65lyczoa3imrjoavgwtq7ziyeldooqxkumqemw5bez5ud2xcue";

    /// @notice Control growth or decline of the trees, controlled by DAO for future uses
    /// @dev Was initially planned to be block.difficulty > 2**64 || block.difficulty == 0;
    /// https://blog.ethereum.org/2021/11/29/how-the-merge-impacts-app-layer/
    /// But DAO control provides more future uses cases.
    bool public grow;
    ...
    uint32 internal constant MAX_LENGTH = 250; // 138m for real trees
    uint32 internal constant MIN_LENGTH = 20;
    ...
    // uint256 public colors2;
    // mapping(uint256 => uint256) public initials;

    // An address who has permissions to mint Trees
    address public minter;

    // The Merge Tree token seeder
    ITreeSeeder public seeder;

    // The Merge trees DAO address
    address public theMergeTreesDAO;

    ///@notice Game Over text from AuctionHouse
    string private gameOverText;
    ...
    /// @notice Minimum price to cut down a tree
    uint256 public minCutPrice = 0.01 ether;

    /// @notice Number for select color picker priviledge each % NFTs
    uint256 public COLOR_PICKER_SELECTOR = 12;

    /// @notice List of unwanted marketplaces to trade NFT e.g. not respecting royalties
    mapping(address => bool) private _unwantedMarketplaces;
    ...
    constructor(
        ...
        address _minter,
        ITreeSeeder _seeder
    ) ERC721("TheMergeTree", "MTREE") EIP712("TheMergeTree", "1") {
        if (_owner != address(0)) {
            _transferOwnership(_owner);
        }
        if (_dao != address(0)) {
            theMergeTreesDAO = _dao;
        }
        minter = _minter;
        seeder = _seeder;
    ...
    /// @notice Mint a Merge Tree to the minter, along with a possible DAO contributor reward token.
    /// DAO contributors reward tokens are minted every 27 tokens, starting at 0 and will stop when supply is reached.
    /// @dev Call _mintTo with the to address.
    function mint() public override returns (uint256) {
        if (msg.sender != minter) {
            revert NotMinter();
        }
        if (_currentTokenId % 27 == 0) {
            _mintTo(theMergeTreesDAO, _currentTokenId++);
        }
    ...
    function gameOver(string memory text) public override {
        if (msg.sender != minter) {
            revert NotMinter();
        }
        if (bytes(gameOverText).length > 0) {
        ...
        returns (string memory)
    {
        ...
        string memory image = Base64.encode(
            bytes(_generateSVGofTokenById(tokenId, tokenName))
        );
        ...
        string memory dynamicTraits = string(
            abi.encodePacked(
                ...
                '},{"trait_type": "cuts", "value": ',
                StringUtils.uint2str(tree.cuts)
            )
        );
    ...
    /// @notice Generate whole SVG image from tokenId
    /// @param tokenId token ID of the SVG to render
    /// @param tokenName for SVG title a11y
    /// @return full SVG
    function _generateSVGofTokenById(uint256 tokenId, string memory tokenName)
        internal
        view
        returns (string memory)
    {
    ...
    /// @return a SVG rectangle green if grow, red otherwise
    function _renderGrass() internal view returns (string memory) {
        if (bytes(gameOverText).length > 0) {
            return
                string(
                    abi.encodePacked(
                        '<g><rect width="1024" height="24" y="1000" style="fill:black"/><text y="1021" x="50%" text-anchor="middle" fill="white" font-size="24px">',
                        gameOverText,
                        "</text></g>"
                    )
                );
            ...
            return
                string(
                    abi.encodePacked(
                        '<rect width="1024" height="24" y="1000" style="fill:',
                        grow ? "#2e8b57" : "#660000",
                        '"/>'
                    )
                );
    ...
    /// @param tokenId token ID of the SVG to render
    /// @return on-chain SVG NFT from another contract
    function _renderMarker(uint256 tokenId)
        internal
        view
        returns (string memory)
    {
        ...
        if (markerToken.tokenAddress != address(0)) {
            if (
                ...
                return
                    string(
                        abi.encodePacked(
                            '<defs><marker id="leaf" markerWidth="12" markerHeight="12" preserveAspectRatio="xMidYMid meet" orient="auto" refX="512" refY="512" viewBox="0 0 1024 1024">',
                            IERC4883(markerToken.tokenAddress).renderTokenById(
                                markerToken.tokenId
                            ),
                            "</marker></defs>"
                        )
        ...
        return TheMergeTreesRenderer.renderDefaultMarker(colors);
    }

    /// @notice Render the fractal tree or text from tokenId
    /// @param tokenId token ID of the tree to render
    /// @dev Public visibility to allow calls by other contracts for composition EIP-4883.
    /// Length is computed at each block
    /// @return Tree SVG (defs + groups + line), must be encapsulated in <svg>
    function _renderTokenById(uint256 tokenId)
        internal
        view
        returns (string memory)
    {
        ...
        uint32 lengthAfterBlocksMined = tree.initLength +
            uint32(blocksMined / growthDivider);
        ...
        if (!grow) {
            // avoid panic code due to negative lengthAfterBlocksMined
            if (blocksMined < tree.initLength) {
                lengthAfterBlocksMined =
                    tree.initLength -
                    uint32(blocksMined / growthDivider);
                // uint32 min(lengthAfterBlocksMined,MIN_LENGTH)
                length = lengthAfterBlocksMined < MIN_LENGTH
                    ? MIN_LENGTH
                    : lengthAfterBlocksMined;
            } else {
                length = MIN_LENGTH;
        ...
        if (tree.segments < 2) {
            if (grow) {
                return
                    string(
                        abi.encodePacked(
                            '<text dominant-baseline="middle" text-anchor="middle" x="50%" y="50%" fill="#006616" font-size="500%">Spes messis in semine</text>'
                        )
                    );
            } else {
                return
                    string(
                        abi.encodePacked(
                            '<text dominant-baseline="middle" text-anchor="middle" x="50%" y="50%" fill="#660000" font-size="500%">Erysichthon was here</text>'
                        )
                    );
            }
        }

        return TheMergeTreesRenderer.render(tree, length, colors, grow);
    }
    ...
    function _transfer(
        ...
        if (grow) {
            uint8 newSegments = trees[tokenId].segments + 1;
            trees[tokenId].segments = newSegments > MAX_SEGMENTS
                ? MAX_SEGMENTS
                : newSegments;
        } else {
    ...
    {
        if (_unwantedMarketplaces[to]) {
            revert UnWantedMarketplace();
        }
    ...
    function setApprovalForAll(address operator, bool approved)
        ...
        override(ERC721, IERC721)
    {
        if (_unwantedMarketplaces[operator]) {
            revert UnWantedMarketplace();
        }
    ...
    /// @notice Toggle color bit if you are the owner of `tokenId` or DAO. This will impact all trees' colors.
    /// Trees share the same colors each encoded in 32 bits to encourage beneficial cooperation.
    /// @param tokenId id bit to toggle in color
    function toggleColor(uint256 tokenId) external override {
        if (!_exists(tokenId)) {
            revert NotFound();
        }
        if (msg.sender != theMergeTreesDAO && msg.sender != ownerOf(tokenId)) {
            revert NotNFTOwnerNorDAO();
        }
    ...
    /// @notice Cut a tree to 0 segments if you are the owner of `tokenId` or DAO, allowing it to grow again.
    /// This will have immediate & future consequences on the tree life.
    /// @param tokenId tree to cut
    function cut(uint256 tokenId) external payable override {
        if (!_exists(tokenId)) {
            revert NotFound();
        }
        if (msg.sender != theMergeTreesDAO && msg.sender != ownerOf(tokenId)) {
        ...
        if (msg.value < minCutPrice) {
            revert BelowMinPrice(minCutPrice, msg.value);
        }
        trees[tokenId].cuts++;
        trees[tokenId].segments = 0;
        emit MergeTreeCut(tokenId);
    }
    ...
    {
        address recipient = theMergeTreesDAO;
        if (recipient == address(0)) {
            recipient = owner();
        }

        // (theMergeTreesDAO || owner), 7.5%
    ...
    /// @param newContractUriIpfsHash the new IPFS hash e.g. Qm...
    function setContractURIHash(string memory newContractUriIpfsHash)
        external
        onlyOwner
    {
    ...
    /// @notice Set an unwanted marketplace address, only by owner
    function setUnwantedMarketplace(address market, bool unwanted)
        external
        onlyOwner
    {
        _unwantedMarketplaces[market] = unwanted;
    }

    /// @notice Approve or not a token as a composable marker, only by owner
    function setComposableMarkerApproval(address tokenContract, bool approved)
        external
        onlyOwner
    {
        approvedComposableTokenForMarker[tokenContract] = approved;
    }
    ...
    /// @notice Allow DAO to set the TheMergeTreesDAO address
    /// @param newTheMergeTreesDAO the new DAO address
    function setTheMergeTreesDAO(address newTheMergeTreesDAO)
        external
        override
    {
        if (msg.sender != theMergeTreesDAO) {
            revert NotDAO();
        }
        theMergeTreesDAO = newTheMergeTreesDAO;
        emit TheMergeTreesDAOUpdated(newTheMergeTreesDAO);
    }

    /// @notice Allow DAO to update the minimum cut price of a tree
    function setMinCutPrice(uint256 newPrice) external override {
        if (msg.sender != theMergeTreesDAO) {
            revert NotDAO();
        }
        minCutPrice = newPrice;
    }

    /// @notice Set growth or decline of trees, only by DAO
    function setGrow(bool newValue) external {
        if (msg.sender != theMergeTreesDAO) {
            revert NotDAO();
        }
        grow = newValue;
    ...
    /// @notice Set growth divider, only by DAO
...
library StringUtils {
    ...
    function uint2strFixedLength(uint256 _i, uint256 _length)
        internal
        pure
        returns (string memory _uintAsString)
    {
...
library TheMergeTreesRenderer {
    ...
    function render(
        ...
        treeRender = string(
            abi.encodePacked(
                ...
                _tree.animated
                    ? grow
                        ? '<animate attributeName="stroke-dashoffset" dur="12s" repeatCount="indefinite" values="1;0.1;0.07;0.05;0"/>'
                        : '<animate attributeName="stroke-dashoffset" dur="12s" repeatCount="indefinite" values="0;0.05;0.07;0.1;1"/>'
                    : "",
                "</line></g>"
            )
        );
        ...
        if (_tree.animated) {
            treeRender = string(
                abi.encodePacked(
                    treeRender,
                    grow
                        ? '<animate href="#leaf" xlink:href="#leaf" attributeType="CSS" attributeName="opacity" values="0;0.9;1" dur="12s" repeatCount="indefinite"/>'
                        : '<animate href="#leaf" xlink:href="#leaf" attributeType="CSS" attributeName="opacity" values="1;0.9;0" dur="12s" repeatCount="indefinite"/>'
                )
            );
        }
        treeRender = string(
    ...
    function drawSegment(Segment memory _segment)
        internal
        pure
        returns (string memory segmentBranchesRender)
    {
    ...
    function renderDefaultMarker(uint256 colors)
        public
        pure
        returns (string memory markerRender)
    {
        markerRender = string(
            abi.encodePacked(
                '<defs><marker id="leaf" viewBox="0 0 12 12" refX="6" refY="6" markerWidth="12" markerHeight="12"><circle cx="6" cy="6" r="6" fill="#',
                getColor(colors, 0),
                '"/></marker></defs>'
            )
        );
    ...
    function computeScale(uint8 _level, uint8 index)
        internal
        pure
        returns (uint32 scale)
    {
        ...
        // precision left = 42949 close to 16 bits
        return
            MULTIPLIER /
            uint32((precomputedValue >> (32 * (_level - 1))) & 0xFFFFFFFF);
    }

    function getColor(uint256 colors, uint8 index)
        internal
        pure
        returns (string memory)
    {
