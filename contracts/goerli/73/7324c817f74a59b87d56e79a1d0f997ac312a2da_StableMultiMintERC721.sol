...
import "../generic/ReentrancyGuard.sol";
import "../generic/Signable.sol";
import "../Helpers.sol";
import "../generic/Errors.sol";

// TODO: Idea to take more ETH, but return unused
contract StableMultiMintERC721 is
    ERC721,
    ReentrancyGuard,
    Signable,
    AccessControl
{
    ...
    // Approximate ETH in USD value should be sent with mint (owner mint is free), stableMintPrice is in USD (for example $100.55 = 100550000)
    uint256 public stableMintPrice;
    ...
    // Price aggregator for ETH/USD
    AggregatorV3Interface public priceAggregator;

    // Stable coin contract
    StableCoinInterface public stableCoin;
    ...
    // What is time window of acceptable eth price (so if price was different and acceptable 1.5 hours ago, but now it's not ok, it still will be accepted)
    uint256 public priceAggregatorStep = 2 hours;
    ...
    modifier costs(uint256 amount) {
        if (!isAcceptableEthValue(msg.value, amount))
    ...
    // MINTER_ROLE address can call this function to mint `amount` of tokens into account with the address `to`
    ...
    function mintSign(
        ...
        payable
        costs(amount)
        featureRequired(Feature.ETH_IN_USD_WITH_SIGN)
        lock
    {
    ...
    function mint(
        ...
        payable
        costs(amount)
        featureRequired(Feature.ETH_IN_USD_PUBLIC)
        lock
    {
    ...
    function stableMintSign(
        ...
        bytes32 r,
        bytes32 s
    ) external featureRequired(Feature.USDC_WITH_SIGN) lock {
        if (!_verify(signer(), _hash(msg.sender, maxAmount), signature))
            revert Errors.InvalidSignature();
        ...
        _stableMintLogic(amount, owner, value, deadline, v, r, s);
    }
    ...
    function stableMint(
        ...
        bytes32 r,
        bytes32 s
    ) external featureRequired(Feature.USDC_PUBLIC) lock {
        if (minted[msg.sender] + amount > mintsPerAccountOnPublicSale)
            revert Errors.AccountAlreadyMintedMax();
    ...
    function withdrawalAllUSDC() external onlyOwner {
        ...
        for (uint256 i; i < length; ) {
            ...
            try stableCoin.transfer(withdrawalAddress, value) {} catch {}
        ...
        if (balance > 0) {
            try
                stableCoin.transfer(withdrawalAddresses[0].account, balance)
            {} catch {}
        }
    }
    ...
    // Contract owner can call this function to set minting price (in USD) on pre-sale and main-sale, stableMintPrice is in USD (for example $100.55 = 100550000)
    function setStableMintPrice(
        uint256 stableMintPrice_
    ) external onlyOwner returns (string memory) {
        stableMintPrice = stableMintPrice_;
        return readableStablePrice();
    }

    // Contract owner can call this function to change price aggregator
    function setPriceAggregator(
        AggregatorV3Interface priceAggregator_
    ) external onlyOwner {
        priceAggregator = priceAggregator_;
    }

    // Contract owner can call this function to change stable coins address
    function setStableCoin(StableCoinInterface stableCoin_) external onlyOwner {
        stableCoin = stableCoin_;
    }

    // Contract owner can call this function to change uint256 price aggregator step,
    function setPriceAggregatorStep(
        uint256 priceAggregatorStep_
    ) external onlyOwner {
        priceAggregatorStep = priceAggregatorStep_;
    }
    ...
    // The contract owner can call this function to change the max supply. It can't be smaller than the amount of already minted tokens
    ...
    function enabledFeatures() external view returns (Feature[] memory) {
        uint256 count = 0;

        for (uint256 i = 0; i < 5; i++) {
            if (isFeatureEnabled(Feature(i))) {
                count++;
            }
        }

        Feature[] memory features = new Feature[](count);
        uint256 index = 0;
    ...
    function minimalEthPrice() public view returns (uint256) {
        if (stableMintPrice == 0) return 0;
        (, int256 price, , , ) = priceAggregator.latestRoundData();
        uint256 ethPrice = uint256(price);
        return (1 ether * stableMintPrice * 100) / ethPrice;
    }

    function minimalEthPrice(uint256 amount) public view returns (uint256) {
        return minimalEthPrice() * amount;
    }

    function isAcceptableEthValue(
        uint256 value,
        uint256 amount
        ...
        (roundId, price, startedAt, , ) = priceAggregator.latestRoundData();
        while (true) {
            uint256 ethToUSDPrice = uint256(price);
            uint256 ethPrice = (1 ether * stableMintPrice * 100) /
                ethToUSDPrice;
            if (value >= ethPrice * amount) {
                return true;
            }
            if (block.timestamp - priceAggregatorStep > startedAt) {
                break;
            }
            (roundId, price, startedAt, , ) = priceAggregator.getRoundData(
                roundId - 1
            );
        }
    ...
    function stablePrice(uint256 amount) public view returns (uint256) {
        return stableMintPrice * amount;
    }

    function readableStablePrice() public view returns (string memory) {
    ...
    function _stableMintLogic(
        ...
        uint256 usdcAmount = stablePrice(amount);
        if (value < usdcAmount) revert Errors.InsufficientFunds();

        address spender = address(this);
        stableCoin.permit(owner, spender, value, deadline, v, r, s);
        stableCoin.transferFrom(owner, address(this), value);
