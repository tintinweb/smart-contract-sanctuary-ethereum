...
contract TreasuryAdaptor {
    ...
    /// @dev Push fund that this contract holds to compound protocol
    ...
    /// @dev Add oeprator to the list by adding the address to operators list
    function addOperator(address newOperator) public {
        if (msg.sender != admin) revert Unauthorized();
        operators[newOperator] = block.timestamp + delay;
    }

    /// @dev Remove operator from the list by adding the address to operatorsBlocklist
    function removeOperator(address oldOperator) public {
        if (msg.sender != admin) revert Unauthorized();
        operatorsBlocklist[oldOperator] = block.timestamp + delay;
    }

    /// @dev Withdraw fund from compound protocol 
    ...
    /// @dev Withdraw comp to admin wallet
    ...
    /// @dev Add withdrawal address to allow list with delay activation timestamp
