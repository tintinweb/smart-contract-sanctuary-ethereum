...
contract Part is 
    ...
    function mintPart(
        address to,
        uint256 quantity) external only(MINTER_ROLE) returns (uint256, uint256) {        
     ...
     * The company will announce its schedule before conducting `mintReserved`
     * 
     * Requirements
     * - the caller must have the `RESERVE_MINTER_ROLE`
     */
    function mintReservedPart(address to, uint quantity) external only(RESERVE_MINTER_ROLE) {
        require(mintEnabled, "Mint has not been enabled");
    ...
    function supportsInterface(bytes4 interfaceId) 
        public view virtual 
        override(ERC721AUpgradeable, IERC721AUpgradeable, AccessControlUpgradeable, IERC165) returns (bool) {
