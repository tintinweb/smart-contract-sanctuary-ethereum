...
contract ControllerFreedom is Initializable, UUPSUpgradeable, OwnableUpgradeable {
    ...
    */
    ...
    function transferBounty(uint256 _amount, address _targetAcc, uint256 commId) external onlyProxy returns (bool){
        ...
        ERC20(tokenAddress).approve(_targetAcc, _amount);
        ERC20(tokenAddress).transferFrom(address(this), _targetAcc, _amount);
        return true;
    }
     ...
     */
    function createBounty(string memory bountyAmount, string memory availableClaims, string memory daystoComplete, uint256 commId) external onlyProxy returns (bool){
        require(assertVotingContractAddr() == true, "Unauthorized user.");
        Community storage comm = fetchAssertCommunity(commId, "Comm: Doesn't exist");
     ...
     * this is will be done through web3 from frontside using abi
     */
    function fnCall(address contractAddr, string memory name, string memory code, uint256 commId) external onlyProxy returns (bool){
        require(assertVotingContractAddr() == true, "Unauthorized user.");
    ...
    */
    function changeBondAndDeadline(string memory bond, string memory expiry, string memory bountyClaim, string memory timeToUnclaimFree, uint256 commId) external onlyProxy returns (bool) {
        require(assertVotingContractAddr() == true, "Unauthorized user.");
...
contract Voting {
	...
	*/
	function communityPollCreate(uint256 daoId, uint256 pollType, ProposeChanges memory proposedChanges, uint256 startTime, uint256 endTime
		...
		polls.push(pollId);
		_polls[daoId] = polls;
		emit EvtCommunityPollCreate(pollId, block.timestamp, block.timestamp + 10);
		return true;
	}
