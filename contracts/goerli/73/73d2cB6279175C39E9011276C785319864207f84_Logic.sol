...
contract Logic is ILogic, AccessControl, ReentrancyGuard {
    ...
    constructor(address dai, address token_, address investor_) {
    ...
    function addMatch(
        ...
        require(matches[matchId].timeStart == 0 &&
    ...
    returns (uint256)
    {
        ...
        if (matches[matchId].matchResult == bet.bet) {
            uint256 withdrawable = calculateTotalStake(matchId) * bet.stake / calculateWinningStake(matchId);
    ...
    function calculateWinningStake(uint256 matchId) public view returns (uint256 total) {
        total = 0;
    ...
    function betExists(uint256 matchId, address user) public view returns (bool){
        for (uint256 i = 0; i < bets[matchId].length; i++) {
            if (bets[matchId][i].user == user) {
                return true;
            }
