    ...
    IERC721LA
{
    ...
    function mintEditionTokens(
        ...
        if (edition.maxMintPerWallet != 0) {
            uint256 mintedCountKey = uint256(keccak256(abi.encodePacked(_editionId, msg.sender)));
            if ( state._mintedPerWallet[mintedCountKey] + _quantity > edition.maxMintPerWallet) {
                revert CustomErrors.MaximumMintAmountReached();
            }
