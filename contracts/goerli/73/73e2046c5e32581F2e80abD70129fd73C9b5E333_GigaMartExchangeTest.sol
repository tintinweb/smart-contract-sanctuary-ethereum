...
/**
    ...
    ⠄⠄⠄⠄⠄⠄⠄⠄⠄⠄⠄⠄⠄⢰⣶⣶⣶⣾⣿⣿⣿⣿⣷⡄⠄⢠⣾⣿⣿⣿
*/
contract GigaMartExchangeTest is GigaMartExecutor {
    /**
...
import {Utils, ArrayUtils} from "../../../utils/Utils.sol";
import {EIP712} from "../../../libraries/EIP712.sol";
import {NativeTransfer} from "../../../libraries/NativeTransfer.sol";
import {GigaMartEntities, Sales, AuthenticatedProxy} from "./GigaMartEntities.sol";
...
error InvalidFeeManagerAddress(); /** Thrown at changeFeeManager() function, if argument is zero address. */
 ...
 */
abstract contract GigaMartExecutor is PermitControl, EIP712, ReentrancyGuard {
    ...
    /**
        @dev Changes address of the Fee Manager contract.
        @param _feeManager new address of Fee Manager.
     */
    function changeFeeManager(address _feeManager) external hasValidPermit(UNIVERSAL, FEE_MANAGER_SETTER) {
        if (_feeManager == address(0)) revert InvalidFeeManagerAddress();
        feeManager = _feeManager;
    }
    ...
    function pay(
        ...
        uint256 fee;
        /** Fetch fees. */
        uint256[] memory fees = IFeeManager(feeManager).getFees(sell.outline.target);
...
library GigaMartEntities {
    ...
    bytes32 public constant ORDER_TYPEHASH =
        keccak256(
            "Order(uint256 nonce,Outline outline,uint256[] extra,bytes data,bytes replacementPattern)Outline(uint256 basePrice,uint256 listingTime,uint256 expirationTime,address exchange,address maker,uint8 side,address taker,uint8 saleKind,address target,uint8 callType,address paymentToken)"
        );
...
library Sales {
        ...
        returns (bool)
    {
        ...
        if(saleKind == SaleKind.Auction) return expirationTime > 0;
        return uint8(saleKind) < 5;
    }
