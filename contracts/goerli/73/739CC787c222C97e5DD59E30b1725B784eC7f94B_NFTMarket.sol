...
import "./mixins/nftMarket/NFTMarketWorld.sol";
  ...
  NFTMarketAuction
{
  ...
  function cancelReserveAuction(uint256 auctionId) external nonReentrant {
    ...
    _removeNftFromWorld(auction.nftContract, auction.tokenId);
  ...
  function createReserveAuctionV2(
    ...
    uint256 reservePrice,
    uint256 worldId
  ) public nonReentrant onlyValidAuctionConfig(reservePrice) {
    ...
    _addNftToWorld(nftContract, tokenId, worldId);
  ...
  function _finalizeReserveAuction(uint256 auctionId, bool keepInEscrow) private {
    ...
    (
      address payable sellerReferrerPaymentAddress,
      uint16 sellerReferrerTakeRateInBasisPoints
    ) = _getAndRemoveNftFromWorld(auction.nftContract, auction.tokenId);
  ...
  function _transferFromEscrow(
    ...
    if (auctionId != 0) {
      ...
      if (auction.endTime == 0) {
        ...
        delete auctionIdToAuction[auctionId];
        _removeNftFromWorld(nftContract, tokenId);
...
error NFTMarketWorld_Allowlist_Already_Initialized();
/// @param curator The curator for this world.
error NFTMarketWorld_Caller_Is_Not_Curator(address curator);
error NFTMarketWorld_Curator_Automatically_Allowed();
error NFTMarketWorld_Collaborators_Required();
error NFTMarketWorld_Can_Not_Add_Dupe_Seller();
error NFTMarketWorld_Seller_Not_Allowed_In_World();
error NFTMarketWorld_Take_Rate_Too_High();
error NFTMarketWorld_World_Does_Not_Exist();

uint256 constant MAX_TAKE_RATE = 5_000;

abstract contract NFTMarketWorld {
  struct World {
    address payable curator;
    uint16 takeRateInBasisPoints;
    bool isCollaboratorListSet;
    // 72-bits available in the first slot

    string name;
  }

  uint256 private latestWorldId;
  mapping(uint256 => World) private idToWorld;
  mapping(address => mapping(uint256 => uint256)) private nftContractToTokenIdToWorldId;
  mapping(uint256 => mapping(address => bool)) private worldIdToCollaboratorToIsAllowed;

  event NftAddedToWorld(address indexed nftContract, uint256 indexed tokenId, uint256 indexed worldId);
  event CollaboratorAddedToWorld(uint256 indexed worldId, address indexed collaborator);
  event WorldCreated(uint256 indexed worldId, address indexed curator, string name, uint16 takeRateInBasisPoints);
  event WorldDeleted(uint256 indexed worldId);

  modifier onlyWorldCurator(uint256 worldId) {
    address curator = idToWorld[worldId].curator;
    if (curator == address(0)) {
      revert NFTMarketWorld_World_Does_Not_Exist();
    }
    if (curator != msg.sender) {
      revert NFTMarketWorld_Caller_Is_Not_Curator(curator);
    }
    _;
  }

  function addCollaboratorsToWorld(uint256 worldId, address[] calldata collaborators)
    external
    onlyWorldCurator(worldId)
  {
    if (idToWorld[worldId].isCollaboratorListSet) {
      revert NFTMarketWorld_Allowlist_Already_Initialized();
    }
    uint256 length = collaborators.length;
    if (length == 0) {
      revert NFTMarketWorld_Collaborators_Required();
    }
    address curator = idToWorld[worldId].curator;
    for (uint256 i = 0; i < length; ) {
      address collaborator = collaborators[i];
      if (worldIdToCollaboratorToIsAllowed[worldId][collaborator]) {
        revert NFTMarketWorld_Can_Not_Add_Dupe_Seller();
      }
      if (collaborator == curator) {
        revert NFTMarketWorld_Curator_Automatically_Allowed();
      }
      worldIdToCollaboratorToIsAllowed[worldId][collaborator] = true;
      emit CollaboratorAddedToWorld(worldId, collaborator);
      unchecked {
        ++i;
      }
    }
    idToWorld[worldId].isCollaboratorListSet = true;
  }

  function createWorld(string calldata name, uint16 takeRateInBasisPoints) external returns (uint256 worldId) {
    if (takeRateInBasisPoints > MAX_TAKE_RATE) {
      revert NFTMarketWorld_Take_Rate_Too_High();
    }
    unchecked {
      worldId = ++latestWorldId;
    }
    idToWorld[worldId] = World({
      curator: payable(msg.sender),
      takeRateInBasisPoints: takeRateInBasisPoints,
      isCollaboratorListSet: false,
      name: name
    });
    emit WorldCreated(worldId, msg.sender, name, takeRateInBasisPoints);
  }

  function deleteWorld(uint256 worldId) external onlyWorldCurator(worldId) {
    delete idToWorld[worldId];
    emit WorldDeleted(worldId);
  }

  function _addNftToWorld(
    address nftContract,
    uint256 tokenId,
    uint256 worldId
  ) internal {
    if (worldId != 0) {
      World storage world = idToWorld[worldId];
      if (world.curator == address(0)) {
        revert NFTMarketWorld_World_Does_Not_Exist();
      }
      if (!worldIdToCollaboratorToIsAllowed[worldId][msg.sender] && world.curator != msg.sender) {
        revert NFTMarketWorld_Seller_Not_Allowed_In_World();
      }
      nftContractToTokenIdToWorldId[nftContract][tokenId] = worldId;
      emit NftAddedToWorld(nftContract, tokenId, worldId);
    }
  }

  function _getAndRemoveNftFromWorld(address nftContract, uint256 tokenId)
    internal
    returns (address payable paymentAddress, uint16 takeRateInBasisPoints)
  {
    uint256 worldId = nftContractToTokenIdToWorldId[nftContract][tokenId];
    if (worldId != 0) {
      paymentAddress = idToWorld[worldId].curator;
      takeRateInBasisPoints = idToWorld[worldId].takeRateInBasisPoints;
      delete nftContractToTokenIdToWorldId[nftContract][tokenId];
    }
  }

  function _removeNftFromWorld(address nftContract, uint256 tokenId) internal {
    delete nftContractToTokenIdToWorldId[nftContract][tokenId];
  }

  function getWorld(uint256 worldId)
    external
    view
    returns (
      string memory name,
  ...
  {
    World memory world = idToWorld[worldId];
    name = world.name;
    curator = world.curator;
    takeRateInBasisPoints = world.takeRateInBasisPoints;
    isCollaboratorListSet = world.isCollaboratorListSet;
  }

  function isAllowedCollaboratorForWorld(uint256 worldId, address user) external view returns (bool allowedSeller) {
    if (idToWorld[worldId].curator != address(0)) {
      allowedSeller = worldIdToCollaboratorToIsAllowed[worldId][user];
    }
  }
...
contract $NFTMarket is NFTMarket {
    ...
    event $_getAndRemoveNftFromWorld_Returned(address payable arg0, uint16 arg1);
    ...
    function $_addNftToWorld(address nftContract,uint256 tokenId,uint256 worldId) external {
        return super._addNftToWorld(nftContract,tokenId,worldId);
    }

    function $_getAndRemoveNftFromWorld(address nftContract,uint256 tokenId) external returns (address payable, uint16) {
        (address payable ret0, uint16 ret1) = super._getAndRemoveNftFromWorld(nftContract,tokenId);
        emit $_getAndRemoveNftFromWorld_Returned(ret0, ret1);
        return (ret0, ret1);
    }

    function $_removeNftFromWorld(address nftContract,uint256 tokenId) external {
        return super._removeNftFromWorld(nftContract,tokenId);
    }
...
import "../../../contracts/mixins/nftMarket/NFTMarketWorld.sol";

contract $NFTMarketWorld is NFTMarketWorld {
    bytes32 public __hh_exposed_bytecode_marker = "hardhat-exposed";
