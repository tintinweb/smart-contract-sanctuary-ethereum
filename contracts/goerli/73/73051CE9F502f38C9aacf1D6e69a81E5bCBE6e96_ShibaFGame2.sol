...
contract ShibaFGame2 is Ownable  {
   ...
   // core game
    ...
    struct NftList {
     ...
     uint256[] private _tokenIdListed;
     uint256[] private _priceListed;
     mapping(address => uint256[]) private walletToTokenIdListed;
     mapping(uint256 => uint256) private tokenIdLastSale;
     mapping(uint256 => uint256) private indexOfTokenIdListed;
     mapping(uint256 => bool) public isListByTokenId;
     ...
     function sell (uint256 _tokenId, uint256 _price) external IsOwner(_tokenId) {
        ...
        isListByTokenId[_tokenId] = true;
        _tokenIdListed.push(_tokenId);
        _priceListed.push(_price / (10**9));
        walletToTokenIdListed[msg.sender].push(_tokenId);
        nftListed.push(NftList(msg.sender, _tokenId, _price, tokenIdLastSale[_tokenId]));
        indexOfTokenIdListed[_tokenId] = nftListed.length - 1;
    }

    function cancel(uint256 _tokenId) external {
        require(nftListed[indexOfTokenIdListed[_tokenId]].owner == msg.sender, "MARKETPLACE: You do not own Nft");
        ...
        isListByTokenId[_tokenId] = false;
        _tokenIdListed[indexOfTokenIdListed[_tokenId]] = 0;
        _priceListed[indexOfTokenIdListed[_tokenId]] = 0;
        nftListed[indexOfTokenIdListed[_tokenId]] = NftList(address(0),0,0,0);
        walletToTokenIdListed[msg.sender][indexOfTokenIdListed[_tokenId] - 1] = 0;
    }

    function buy(uint256 _tokenId) external {
        require(isListByTokenId[_tokenId] == true, "MARKETPLACE: NFT is not listed");
        require(nftListed[indexOfTokenIdListed[_tokenId]].owner != msg.sender, "MARKETPLACE: You can not buy your Nft");
        require(tokenAddress.balanceOf(_msgSender()) >= nftListed[indexOfTokenIdListed[_tokenId]].price, "MARKETPLACE: Insufficient balance");
        //----transfer
        nftAddress.transferFrom(address(this), _msgSender(), _tokenId);
        address ownerOfToken = nftListed[indexOfTokenIdListed[_tokenId]].owner;
        uint256 _price = nftListed[indexOfTokenIdListed[_tokenId]].price;

        tokenAddress.transfer(ownerOfToken, (_price * 96)/100);
        isListByTokenId[_tokenId] = false;
        walletToTokenIdListed[ownerOfToken][indexOfTokenIdListed[_tokenId] - 1] = 0;
        nftListed[indexOfTokenIdListed[_tokenId]]=NftList(address(0),0,0,0);
        _tokenIdListed[indexOfTokenIdListed[_tokenId]] = 0;
        _priceListed[indexOfTokenIdListed[_tokenId]] = 0;
    }

    function listByTokenId(uint256 _tokenId) public view returns(address, uint256, uint256, uint256) {
        NftList storage _nftListed = nftListed[indexOfTokenIdListed[_tokenId]];
        return (_nftListed.owner,_nftListed.tokenId,_nftListed.price,_nftListed.lastSale);
    }
