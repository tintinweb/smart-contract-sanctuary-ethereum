...
contract IP3 {
    ...
    function purchase(
        ...
        authroizeRecordMap[hashedAuthorizeNFT].totalTransactionRevenue +=termedPrice;
    ...
    function hashTerm(Term memory _term) private returns (bytes32) {
        return
            keccak256(
    ...
    function getAuthroizeRecordMap(bytes32 _hashedAuthorizeNFT) external view returns (AuthorizeRecord memory) {
        return authroizeRecordMap[_hashedAuthorizeNFT];
    }

    function getAuthroizeCertificateMap(bytes32 _hashedCertificate) external view returns (AuthorizeCertificate memory) {
