...
interface IMetasharesStakingEth {
    /**
    ...
    /**
     * @notice Claim rewards for Metashare by sending a message to Polygon contract
     * @param relayerFee The fee to be paid to the relayer
     */
    function claimOnPolygon(uint256 relayerFee) external;

    /**
     * @notice Stake Metashare
     * @param _amount The amount of tokens to stake
     */
    function stake(uint256 _amount) external;

    /**
     * @notice Unstake Metashares
     * @param _amount The amount of tokens to unstake
     */
    function unstake(uint256 _amount, uint256 relayerFee) external;
     ...
     */
    function rewardPerToken(uint256 _untilTimestamp) external view returns (uint256);
     ...
     */
    function lastTimeRewardApplicable(uint256 _untilTimestamp) external view returns (uint256);
     ...
     */
    function earned(address _walletAddress) external view returns (uint256);
     ...
     */
    function earnedUntilTimestamp(address _walletAddress, uint256 _untilTimestamp) external view returns (uint256 reward);
     ...
     */
    function initializePools(uint256 _poolRewards, uint256 _epochDuration) external;
}
...
contract MetashareStakingEth is IMetasharesStakingEth, OwnableUpgradeable, ERC1155HolderUpgradeable, ReentrancyGuardUpgradeable, IXReceiver {
    ...
    function claimOnPolygon(uint256 relayerFee) public override {
        _claimOnPolygon(msg.sender, relayerFee);
    }

    /// @notice sends message to polygon to claim tokens
    function _claimOnPolygon(address recipient, uint256 relayerFee) internal nonReentrant whenNotPaused {
        uint256 reward = _claim(recipient);
        bytes memory _callData = abi.encode(recipient, reward, address(this), relayerFee);
        connext.xcall{value: relayerFee}(
            137, // _destination: Domain ID of the destination chain
    ...
    function xReceive(
        ...
        require(_originSender == polygonStaking, "MetashareStaking: INVALID_ORIGIN");
        require(_origin == 137, "MetashareStaking: INVALID_ORIGIN");
    ...
    /// @inheritdoc IMetasharesStakingEth
    function unstake(uint256 _amount, uint256 relayerFee) external nonReentrant whenNotPaused {
