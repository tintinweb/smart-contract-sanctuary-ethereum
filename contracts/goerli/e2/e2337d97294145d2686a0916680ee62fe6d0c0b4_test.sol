...
contract test is  ERC721, ReentrancyGuard, Ownable {
  ...
  function mintPlanet1(uint256 count) public payable nonReentrant {
    ...
    for (uint256 i = 0; i < count; i++) {
      ...
      _safeMint(msg.sender, totalPlanet1Supply());
    }
 ...
 // require(count - 1 < MAX_MULTIMINT, "Trying to mint too many at a time");
    ...
    for (uint256 i = 0; i < count; i++) {
      ...
      _safeMint(msg.sender, totalPlanet2Supply() + PLANET1_SUPPLY);
    }
 ...
 //   require(count - 1 < MAX_MULTIMINT, "Trying to mint too many at a time");
    ...
    for (uint256 i = 0; i < count; i++) {
      ...
      _safeMint(msg.sender, totalPlanet3Supply() + PLANET1_SUPPLY + PLANET2_SUPPLY );
    }

    payable(_splitter).transfer(msg.value);
  }

  function mintAllPlanets(uint256 count1, uint256 count2, uint256 count3) public payable nonReentrant {
    require(saleIsActive, "Sale not active");
    require(msg.value >= (PLANET_PRICE * count1) + (PLANET_PRICE * count2) + (PLANET_PRICE * count3), "Insufficient payment");
