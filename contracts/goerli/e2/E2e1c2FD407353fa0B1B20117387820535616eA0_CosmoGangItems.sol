...
contract CosmoGangItems is ERC1155Supply, Ownable {
    ...
    function getFaction(uint256 tokenId, uint8 v, bytes32 r, bytes32 s, uint256 deadline)
        public view
        returns (Faction)
    {
        if (ecrecover(sha256(abi.encodePacked(msg.sender, tokenId, uint(Faction.None), deadline)), v, r, s) == signer_address)
        {
            return Faction.None;
        }
        else if (ecrecover(sha256(abi.encodePacked(msg.sender, tokenId, uint(Faction.Jahjahrion), deadline)), v, r, s) == signer_address)
        {
            return Faction.Jahjahrion;
        }
        else if (ecrecover(sha256(abi.encodePacked(msg.sender, tokenId, uint(Faction.Breedorok), deadline)), v, r, s) == signer_address)
        {
            return Faction.Breedorok;
        }
        else if (ecrecover(sha256(abi.encodePacked(msg.sender, tokenId, uint(Faction.Foodrak), deadline)), v, r, s) == signer_address)
        {
            return Faction.Foodrak;
        }
        else if (ecrecover(sha256(abi.encodePacked(msg.sender, tokenId, uint(Faction.Pimpmyridian), deadline)), v, r, s) == signer_address)
        {
            return Faction.Pimpmyridian;
        }
        else if (ecrecover(sha256(abi.encodePacked(msg.sender, tokenId, uint(Faction.Muskarion), deadline)), v, r, s) == signer_address)
        {
            return Faction.Muskarion;
        }
        else if (ecrecover(sha256(abi.encodePacked(msg.sender, tokenId, uint(Faction.Lamborgardoz), deadline)), v, r, s) == signer_address)
        {
            return Faction.Lamborgardoz;
        }
        else if (ecrecover(sha256(abi.encodePacked(msg.sender, tokenId, uint(Faction.Schumarian), deadline)), v, r, s) == signer_address)
        {
            return Faction.Schumarian;
        }
        else if (ecrecover(sha256(abi.encodePacked(msg.sender, tokenId, uint(Faction.Creatron), deadline)), v, r, s) == signer_address)
        {
    ...
    function getFactionBatch(uint256[] calldata tokenIds, uint8 v, bytes32 r, bytes32 s, uint256 deadline)
        public view
        returns (Faction)
    {
    ...
    function claimRole(uint256 tokenId, uint8 v, bytes32 r, bytes32 s, uint256 deadline)
        public
        returns (Faction)
    {
        Faction faction = getFaction(tokenId, v, r, s, deadline);
    ...
    function claimRoleBatch(uint256[] calldata tokenIds, uint8 v, bytes32 r, bytes32 s, uint256 deadline)
        public
        returns (Faction)
    {
        Faction faction = getFactionBatch(tokenIds, v, r, s, deadline);
        for (uint256 idx = 0; idx < tokenIds.length; idx++)
        {
            if (nft_contract.tokenFactions(tokenIds[idx]) == Faction.None)
            {   
                require(faction != Faction.None, "Couldn't set a faction to None");
                nft_contract.setFaction(tokenIds[idx], faction);
            }
        }
        return faction;
    }

    function getTimeBeforeNextMint(uint256[] memory tokenIds)
        public view
        returns (uint256)
    {
        uint256 minNextMintTimestamp = 0;
        uint256[] memory _tokenIdLastMint = tokenIdLastMint;
        for (uint256 idx; idx < tokenIds.length; idx++)
        {
            uint256 tokenId = tokenIds[idx];
            uint256 oneNextMintTimestamp = (_tokenIdLastMint[tokenId] + 60 * 60 * 24 * daysBetweenMints) - block.timestamp;

            if (minNextMintTimestamp == 0 || oneNextMintTimestamp < minNextMintTimestamp)
            {
                minNextMintTimestamp = oneNextMintTimestamp;
            }
        }
        return minNextMintTimestamp;
    }
    ...
    function canMint(uint256 tokenId)
        ...
        returns (bool)
    {
        // return (nft_contract.ownerOf(tokenId) == msg.sender || msg.sender == owner()) && 
        //     block.timestamp >= tokenIdLastMint[tokenId] + 60 * 60 * 24 * daysBetweenMints;
        return block.timestamp >= tokenIdLastMint[tokenId] + 60 * 60 * 24 * daysBetweenMints;
    }

    function canMintBatchOLD(uint256[] calldata tokenIds)
        public view
        returns (bool)
    {
        // uint256[] memory _tokenIdLastMint = tokenIdLastMint;
        for (uint256 idx = 0; idx < tokenIds.length; idx++)
        {
            ...
            // if ((nft_contract.ownerOf(tokenIds[idx]) == msg.sender || msg.sender == owner()) &&  
            //     block.timestamp >= _tokenIdLastMint[tokenIds[idx]] + 60 * 60 * 24 * daysBetweenMints)
            {
                return false;
            }
        }
        ...
        returns (bool)
    {
        ...
        {
            // if (!canMint(tokenIds[idx]))
            if (block.timestamp >= _tokenIdLastMint[tokenIds[idx]] + 60 * 60 * 24 * daysBetweenMints)
            {
                return false;
            }
        }
    ...
    // Mint Logic
    function _mintNFT(uint256 nMint, address recipient, uint8 v, bytes32 r, bytes32 s, uint256 fromTokenId, uint256 deadline, bool useProba)
        private
        returns (bool)
    {
        Faction faction = claimRole(fromTokenId, v, r, s, deadline);
    ...
    // Mint Logic Batch
    function _mintNFTBatch(address recipient, uint8 v, bytes32 r, bytes32 s, uint256[] calldata fromTokenIds, uint256 deadline, bool useProba)
        private
        returns (bool)
    {
        Faction faction = claimRoleBatch(fromTokenIds, v, r, s, deadline);
        require(nft_contract.owner() == address(this), "CosmoItem contract needs to be the owner of Cosmo Gang NFT");
        ...
        for (uint256 idx = 0; idx < fromTokenIds.length; idx++)
        {
            ...
            // _tokenIdLastMint[tokenId] = block.timestamp;
        } 
        // tokenIdLastMint = _tokenIdLastMint;
        // 
        // Not needed since we check it in canMintBatch ????
        // 
        // require(nft_contract.isHolder(recipient), "you have to be a Cosmo Gang holder to mint Cosmo Gang Item");
        require(faction != Faction.None, "you have to claimRole before beeing able to mint your Cosmo Gang Item");
    ...
    // Mint Logic Batch
    // DO NOT SET role in the cosmo gang nft
    function _mintNFTBatch2(address recipient, uint8 v, bytes32 r, bytes32 s, uint256[] calldata fromTokenIds, uint256 deadline, bool useProba)
        private
        returns (bool)
    {
        // Faction faction = claimRoleBatch(fromTokenIds, v, r, s);
        Faction faction = getFactionBatch(fromTokenIds, v, r, s, deadline);
        // Not needed anymore if we dont set the rol in the cosmo gang nft 
        // require(nft_contract.owner() == address(this), "CosmoItem contract needs to be the owner of Cosmo Gang NFT");
        require(canMintBatch(fromTokenIds), "tokenId already minted his periodic Cosmo Gang Item");
        // uint256[] memory _tokenIdLastMint = tokenIdLastMint;
        for (uint256 idx = 0; idx < fromTokenIds.length; idx++)
    ...
    // Normal Mint
    function mintNFT(uint256 nMint, address recipient, uint8 v, bytes32 r, bytes32 s, uint256 fromTokenId, uint256 deadline)
        public payable
        returns (bool)
    {
        ...
        return _mintNFT(nMint, recipient, v, r, s, fromTokenId, deadline, true);
    }

    // Normal Mint Bacth
    function mintNFTBatch(address recipient, uint8 v, bytes32 r, bytes32 s, uint256[] calldata fromTokenIds, uint256 deadline)
        public payable
        returns (bool)
    {
        uint256 nMint = fromTokenIds.length;
       ...
       return _mintNFTBatch(recipient, v, r, s, fromTokenIds, deadline, true);
    }

    // Normal Mint Bacth
    function mintNFTBatch2(address recipient, uint8 v, bytes32 r, bytes32 s, uint256[] calldata fromTokenIds, uint256 deadline)
        public payable
        returns (bool)
    {
        uint256 nMint = fromTokenIds.length;
       ...
       return _mintNFTBatch2(recipient, v, r, s, fromTokenIds, deadline, true);
    }
