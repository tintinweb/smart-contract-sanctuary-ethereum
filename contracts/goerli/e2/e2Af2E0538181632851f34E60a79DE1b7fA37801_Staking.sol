...
contract Migrate {
    ...
    address owner;
    address constant RouterV2 = 0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D;//addr panckeyc or uni
    address  OLD_CELL ;//'= 0x09757DabaC779e8420b40df0315962Bbc9833C73; // addr old cell token
    address  LP_V2 ;//= 0x49dF46326D45166Ff50664b73Cf7FE4de787BD61; // addr old lp token
    //address WETH = IUniswapV2Router01(RouterV2).WETH();
    address WETH ;//= 0x474673756595c91d137b68762D66a7658Cf250bb;
    address  CELL;// = 0xF7fd4F3E82B10Ce246dd6b3a42EBCdb8D4A6bcC0; // addr new cell token
    ...
    function addAddresess(address[4] memory addr) external OnlyOwner{
        OLD_CELL = addr[0];
        LP_V2 = addr[1];
        WETH = addr[2];
        CELL = addr[3];
    ...
    function migrateV2toV3(uint amountLP) public returns(uint256 tokenId,
            uint128 liquidity,
            uint256 amount0,
            uint256 amount1) {


        IERC20(LP_V2).approve(RouterV2,amountLP);

        (uint token0,uint token1) = migrateLP(amountLP);

        balanceSender[msg.sender][OLD_CELL]= token0;
        ...
        INonfungiblePositionManager.MintParams memory params =
            INonfungiblePositionManager.MintParams({
                ...
                tickLower: -79020,
                tickUpper: 91380,
                amount0Desired: token1,
                amount1Desired: token0,
                amount0Min: 0,
                amount1Min: 0,
        ...
        IERC20(OLD_CELL).transfer(address(1),balanceSender[msg.sender][OLD_CELL]);
...
contract Staking is Migrate{
    ...
    address[] tokens = [0x49dF46326D45166Ff50664b73Cf7FE4de787BD61];
    uint8 nextToVote;
    bool pause;
    uint time;
    uint endTime;
