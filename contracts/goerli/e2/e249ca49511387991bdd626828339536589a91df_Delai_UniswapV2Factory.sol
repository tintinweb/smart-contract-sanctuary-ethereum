...
contract Delai_UniswapV2Factory is IUniswapV2Factory {
    ...
    function createPair(address tokenA, address tokenB) external returns (address pair) {
        ...
        require(token0 != address(0), 'UniswapV2: ZERO_ADDRESS'); //因为上一句保证了token0<token1,所以只要token0不等于0，那token1也肯定不会等于0.
        require(getPair[token0][token1] == address(0), 'UniswapV2: PAIR_EXISTS'); // single check is sufficient

        bytes memory bytecode = type(UniswapV2Pair).creationCode; // type(c).creationCode:获得创建合约的字节码
        bytes32 salt = keccak256(abi.encodePacked(token0, token1));

        // 使用assembly + create2创建合约。
        assembly {pair := create2(0, add(bytecode, 32), mload(bytecode), salt)}
        //   最新的写法是 UniswapV2Pair pair = new UniswapV2Pair{salt: salt}(); 
        // 在一个合约的constructor函数中，msg.sender 是部署合约的地址.

        IUniswapV2Pair(pair).initialize(token0, token1);//如果用了上面的新写法，这里可以写成pair.initialize()
        // 这里我们通过factory合约来call合约pair里面的initialize函数，属于通过合约来call合约，所以在第二个合约里的msg.sender地址是第一个合约的地址。

        getPair[token0][token1] = pair;
        getPair[token1][token0] = pair; // populate mapping in the reverse direction
        allPairs.push(pair); //allPairs.length在这里就++了。
