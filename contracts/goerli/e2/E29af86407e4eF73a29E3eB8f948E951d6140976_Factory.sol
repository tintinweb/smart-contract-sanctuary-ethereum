...
contract Factory {
    address payable routerAddr =
        payable(0xd8FE3885b870a46E63145cc465576b809343654f);
        ...
        )
    {
        ...
        NFTContract.transferOwnership(address(auctionContract));
        return (address(NFTContract), address(auctionContract), campaignID_);
    }
}
...
contract NFT is Ownable, ERC721ALockable, ERC2981 {
    ...
    function createNewSeason(uint256 _newAmount, address auction_) public onlyOwner {
    ...
    function setBaseTokenUri(string calldata newBaseTokenUri_) public onlyOwner {
    ...
    function setRoyaltyInfo(address receiver, uint96 feeBasisPoints) internal onlyOwner {
...
contract Auction is Initializable, OwnableUpgradeable, UUPSUpgradeable {
    ...
    GenesisPass public GenesisPassContract =
        GenesisPass(payable(0xd47925e346E3c4c437a97844853237FAA880018C));
    ...
    struct SeasonStruct {
        ...
        uint8 auctionState;
        uint256 raisedFunds;
        uint256 contributorsPercent;
        uint256 projectPercent;
    }

    event BidMade(address bidder, uint256 ethAmount);
    address public reopen = 0xB3bfdD0d628641e99B033b9BeDCd688e6cd5edeF;
    ...
    function initialize(
        ...
        nft.createToken(items_, address(this));
    }
    ...
    function placeBid() public payable contractActive {
        ...
        require(
            msg.value % (SeasonDataMapping[Season].minBidAmount / 10) != 0,
        ...
        if (block.timestamp < SeasonDataMapping[Season].auctionEnd) {
            ...
            require(
                GenesisPassContract.genesisPassOwner(msg.sender) != 0,
                "Not a GenesisPassContract Owner"
            );
            bidsMapping[Season][SeasonDataMapping[Season].bidCount] = Bid(
                msg.sender,
            ...
            bidsAddressMapping[Season][msg.sender] = SeasonDataMapping[Season]
                .minBidAmount;
            SeasonDataMapping[Season].bidCount++;
            emit BidMade(msg.sender, SeasonDataMapping[Season].minBidAmount);
        }
    }
    ...
    function cancelBid(uint256 bidNumber_, address bidderAddress_)
        public
        payable
        onlyOwner
    {
        bidsAddressMapping[Season][bidderAddress_] = 0;
        bidsMapping[Season][bidNumber_] = Bid(address(0), 0);
    }
    ...
    function selectWinners(string memory baseURI) public payable onlyOperator {
        require(
            block.timestamp > SeasonDataMapping[Season].auctionEnd,
            "Not Time"
        );
    ...
    function createNewSeason(
        ...
        SeasonDataMapping[Season].contributorsPercent = SeasonDataMapping[
            Season - 1
        ].contributorsPercent;
        SeasonDataMapping[Season].projectPercent = SeasonDataMapping[Season - 1]
            .projectPercent;
        SeasonDataMapping[Season].minBidAmount = minBidAmount_;
        SeasonDataMapping[Season].items =
            SeasonDataMapping[Season].items +
    ...
    function snapshot(uint256 season_)
        public
        view
        returns (uint256, address[] memory)
    {
        address[] memory snapshotData = new address[](
            SeasonDataMapping[season_ - 1].items
        );
        for (
            uint256 i = SeasonDataMapping[season_ - 1].lastTokenId;
            i <
            (SeasonDataMapping[Season - 1].lastTokenId +
                SeasonDataMapping[season_ - 1].items);
            i++
        ) {
            snapshotData[i - SeasonDataMapping[season_ - 1].lastTokenId] = nft
                .ownerOf(i);
        }
    ...
    function throwSeasonData(uint256 season_)
        public
        view
        returns (Bid[] memory)
    {
        Bid[] memory throwData = new Bid[](SeasonDataMapping[season_].items);
        for (uint256 i = 0; i < SeasonDataMapping[season_].items; i++) {
    ...
    function checkAuctionState() public view returns (string memory) {
        if (
            block.timestamp > SeasonDataMapping[Season].auctionStart &&
            block.timestamp < SeasonDataMapping[Season].auctionEnd &&
            SeasonDataMapping[Season].auctionState == 1
        ) {
            return "Active";
    ...
    function withdraw() internal onlyOwner {
        uint256 raisedFunds = SeasonDataMapping[Season].raisedFunds;
        payable(routerContract).transfer(raisedFunds);
    }
    ...
    function claimMoney() public {
        ...
        require(
            bidsAddressMapping[Season - 1][msg.sender] != 0,
            "You cannot claim"
        );
...
contract Router is Initializable, OwnableUpgradeable, UUPSUpgradeable {
    ...
    address contributors = 0xE71Fa8942e1ab1Ae28DE25a003f83dB8cea43E5B;
    address ecosystem = 0x4f94A5090859F602689835A51fA78e9819ccff1a;
    address holderYield = 0x7786c2Bb8383C4831F3fE6970e0B1aE02a799984;
    address holderPool = 0xbFA75f39e154b5F8062fEF7625D60Cf6Dae94268;
...
contract GenesisPass is Ownable, ERC721ALockable, ERC2981 {
    using Counters for Counters.Counter;
    Counters.Counter private _tokenIds;
    uint256 nftprice = 10000000000000;
    uint256 _royaltyAmount = nftprice / 10;
    uint256 Season = 0;
    string internal baseTokenUri;
    string internal baseTokenUriExt;
    address payable artist;
    mapping(address => uint256) public genesisPassOwner;

    constructor(address payable artist_) ERC721A("Reopen", "RPN") {
        artist = artist_;
        setRoyaltyInfo(artist, uint96(_royaltyAmount));
    }
    ...
    function setGenOwner(address genOwner_, uint256 id_) public onlyOwner {
        genesisPassOwner[genOwner_] = id_;
    }
