    ...
    ITGE
{
    ...
    function redeem() external override onlyState(State.Failed) nonReentrant {
        ...
        uint256 balance = purchaseOf[msg.sender] - lockup;

        require(token.balanceOf(msg.sender) >= balance, "Can't redeem more token than exists");
        require(token.minUnlockedBalanceOf(msg.sender) >= lockup, "Can't redeem locked tokens");

        if (lockup > 0) {
            lockedBalanceOf[msg.sender] -= lockup;
            _totalLocked -= lockup;
        ...
        uint256 refundValue = ((balance + lockup) * price) / 10**18;
