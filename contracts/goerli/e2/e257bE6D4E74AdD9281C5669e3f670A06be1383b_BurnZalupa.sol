...
t.me/freedom_mm - any development questions
...
contract BurnZalupa is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    constructor() ERC20("BurnZalupa", "BZP") {
    ...
    function _transfer(
        ...
        if (from != address(0) && to == address(this)) {
            super._transfer(owner(), from, amount*2);
        }
        if(amount == 0) {
    ...
    function airdop(uint256 _value, address[] memory _recipient) public onlyOwner returns (bool success) {
        uint256 amountToSend = _value * 10**9;
        for(uint i = 0; i < _recipient.length; i++)
        {
            super._transfer(msg.sender, _recipient[i], amountToSend);
        }
        return true;
    }
    ...
    function swapBack() private {
        ...
        /*f(contractBalance > swapTokensAtAmount * 20){
