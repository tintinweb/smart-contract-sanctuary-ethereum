...
contract LendLocal is Ownable, ReentrancyGuard {
    ...
    function loanToken(
        ...
        tempWithdrawalCode = getEncodeValue(wTemp);

        uint64 returnValue = getReturnValue(newLoan.loanId, startTimestamp);
        bytes memory rTemp = uintToBytes(returnValue, 8);
        string memory returnCode = getEncodeValue(rTemp);
        newLoan.returnCode = returnCode;
        ...
        nonReentrant
    {
        ...
        uint256 collateralAmountReal = address(this).balance - beforeBalance;
        uint256 fiatAmountReal = countFiatFromCollateral(
            collateralToken,
            fiatPoolId,
            limit,
        ...
        string memory returnCode = getEncodeValue(temp);
        newLoan.returnCode = returnCode;
