...
pragma solidity ^0.8.0;
  ...
  // CHANGE ME: Contact Name
  contract yv2 is ERC721A, Ownable, ReentrancyGuard {
      // declares the maximum amount of tokens that can be minted
      uint256 public constant MAX_TOTAL_TOKENS = 10000;
  
      // max number of mints per wallet/transaction
      uint256 public free_mint_max = 4;
      uint256 public allowlist_mint_max = 4;
      ...
      // supply caps for each mint phase
      uint256 public freeMintSupplyCap = 10;
      uint256 public allowlistSupplyCap = 100;
  
      // mint phase counters
      uint256 public freeMintsOccurred = 0;
      uint256 public allowlistMintsOccurred = 0;
      ...
      // metadata
      string private baseURI = "asd";
      string private unrevealedURI = "asd";
      ...
      mapping (address => uint256) public reserved_mints;
      ...
      error StakingClosed();
      error FreeMintCapHasBeenReached();
      error AllowlistCapHasBeenReached();
      ...
      constructor (bytes32 _root_al, bytes32 _root_free) ERC721A("yv2", "tv2") {
          root_al = _root_al;
          root_free = _root_free;
          agency = 0xfc0153A4337B994dD4D487b79E477A4017239BDD; // Davin: burn address for now, will check for agencyAddress and set it here on front end
  
          // CHANGE ME: Update reserve tokens if necessary
          // don't forget to update total_reserved
          reserved_mints[msg.sender] = 250; // company wallet
      }
      ...
      function freeMint(bytes32[] calldata _proof, uint256 _amt) external payable nonReentrant {
          ...
          if(freeMintsOccurred + _amt > freeMintSupplyCap) revert FreeMintCapHasBeenReached();
      ...
      function allowlistMint(bytes32[] calldata _proof, uint256 _amt) external payable nonReentrant {
          ...
          if(allowlistMintsOccurred + _amt > allowlistSupplyCap) revert AllowlistCapHasBeenReached();
      ...
      function setAgency(address _newAgency) external onlyOwner {
          agency = _newAgency;
      }

      function setAllowListSupplyCap(uint256 _newAllowListSupplyCap) external onlyOwner {
          allowlistSupplyCap = _newAllowListSupplyCap;
      }

      function setFreeMintSupplyCap(uint256 _newFreeMintSupplyCap) external onlyOwner {
          freeMintSupplyCap = _newFreeMintSupplyCap;
      }
      ...
      // CHANGE ME: Update payout addresses if necessary
      function withdrawEth() public onlyOwner nonReentrant {
          uint256 total = address(this).balance;
          // CHANGE ME: this needs to be updated to the correct mintly fee collection address, should probably be stored in NaaS db and interpolated
          require(payable(0x452A89F1316798fDdC9D03f9af38b0586F8142e5).send((total * 5) / 100)); // Mintly fee
          if(agency != address(0)){
              require(payable(agency).send((total * 5) / 100)); // brand/agency fee
              require(payable(msg.sender).send((total * 90) / 100)); // deployer/owner
          } else {
              require(payable(msg.sender).send((total * 95) / 100)); // deployer/owner
          }
