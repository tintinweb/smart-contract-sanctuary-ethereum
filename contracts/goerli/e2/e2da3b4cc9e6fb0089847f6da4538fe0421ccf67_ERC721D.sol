...
contract ERC721 {
    ...
    function safeMint(address owner, uint256 tokenId) public {
        ...
        ++balanceOf[owner];
        tokenByIndex[totalSupply++] = tokenId;
        tokenOfOwnerByIndex[owner].push(tokenId);
    ...
    function transferFrom(address owner, address to, uint256 tokenId) public {
        ...
        require(_ownerOf[tokenId] == owner, 'wrong owner');
        {
            ...
            for (uint256 i; i < n; ++i) {
                if (tokenOfOwnerByIndex[owner][i] == tokenId) {
                    tokenOfOwnerByIndex[owner][i] = tokenOfOwnerByIndex[owner][n - 1];
