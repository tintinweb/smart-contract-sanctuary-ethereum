...
interface IWeRent is IERC721Receiver, IERC1155Receiver {
    ...
    // function isRenting(address account) external returns (bool);
}
...
contract WeRent is WerentSignature, IWeRent, ERC721Holder, ERC1155Receiver, ERC1155Holder, Initializable {
    ...
    function handleRent(CallData memory _cd) private {
        ...
        for (uint256 i = _cd.left; i < _cd.right; i++) {
            ...
            // update lendRentArr
            // lendRentArr.push(keccak256(abi.encodePacked(_cd.nfts[_cd.left], _cd.tokenIds[i], _cd.lendingIds[i])));
    ...
    // function isRenting(address account) public view override returns (bool) {
    //     for (uint i = 0; i < lendRentArr.length; i++) {
    //         LendingRenting memory item = lendingRenting[lendRentArr[i]];
    //         if (
    //             item.renting.renterAddress == account &&
    //             item.renting.rentedAt + item.renting.rentDuration <= block.timestamp
    //         ) {
    //             return true;
    //         }
    //     }
