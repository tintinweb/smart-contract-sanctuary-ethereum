...
contract PixelPrismatica is StandardContract, IERC1155MetadataURI, IERC2981 {
    ...
    */
    ...
    mapping(uint256 => string) private map_colorMode2ColorModeName;
    ...
    */
    ...
    constructor() StandardContract() payable {
        ...
        // Initialize NFT color mode names.
        map_colorMode2ColorModeName[0] = "Rainbow Light";
        map_colorMode2ColorModeName[1] = "Rainbow Dark";
        map_colorMode2ColorModeName[2] = "Monochrome";
        map_colorMode2ColorModeName[3] = "Red";
        map_colorMode2ColorModeName[4] = "Green";
        map_colorMode2ColorModeName[5] = "Blue";
        map_colorMode2ColorModeName[6] = "Green & Blue";
        map_colorMode2ColorModeName[7] = "Red & Green";
        map_colorMode2ColorModeName[8] = "Red & Blue";
        map_colorMode2ColorModeName[9] = "Cyan";
        map_colorMode2ColorModeName[10] = "Yellow";
        map_colorMode2ColorModeName[11] = "Magenta";
    }
    ...
    */
    ...
    function mint(address _address, uint256 _colorMode, uint256 _imageSize, bytes memory _data) private {
        ...
        // Set NFT config defaults here.
        map_id2NFTConfig[currentID] = NFTConfig(0, 10, 5, 18, 18, 18, 18);
        map_id2SizeString[currentID] = "Medium";

        // Every time an NFT is minted, increase the minting cost for the next one.
        setMintFee((getMintFee() * 105) / 100);

        // Apply user's initial preferences.
        setColorMode(currentID, _colorMode);
        setImageSize(currentID, _imageSize);
    ...
    */
    ...
    function createDescription(uint256 _id) private view returns (string memory) {
        if(_id == 0 || _id > currentID) {
            ...
            return string.concat("Configuration: ", map_colorMode2ColorModeName[getColorMode(_id)], " ", map_id2SizeString[_id]);
        }
    }
    ...
    */
    ...
    function getColorMode(uint256 _id) private view returns (uint256) {
        return map_id2NFTConfig[_id].colorMode;
    }

    function getDuration(uint256 _id) private view returns (uint256) {
        return map_id2NFTConfig[_id].duration;
    }
    ...
    */
    ...
    function setDuration(uint256 _id, uint256 _duration) private {
        map_id2NFTConfig[_id].duration = _duration;
    }

    function setImageSize(uint256 _id, uint256 _imageSize) private {
        if(_imageSize == 0) {
            map_id2SizeString[currentID] = "Small";
            ...
            map_id2SizeString[currentID] = "Large";
    ...
    */
    ...
    function action_mint(uint256 _colorMode, uint256 _imageSize, bytes memory _data) external payable {
        lock();

        requireRemainingMints();
        requireMintFee(msg.value);

        mint(msg.sender, _colorMode, _imageSize, _data);
    ...
    /// @param _imageSize The initial image size.
    /// @param _data Additional data with no specified format.
    function action_mintOther(address _address, uint256 _colorMode, uint256 _imageSize, bytes memory _data) external payable {
        lock();
        ...
        mint(_address, _colorMode, _imageSize, _data);
    ...
    */
    ...
    /// @notice Returns the color mode of the NFT.
    /// @param _id The ID of the NFT.
    /// @return The color mode of the NFT.
    function get_colorMode(uint256 _id) external view returns (uint256) {
        return getColorMode(_id);
    }
