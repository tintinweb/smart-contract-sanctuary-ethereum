...
contract SalePioneer is Ownable {
    ...
    uint public maxPurchaseInitial;
    ...
    function initialMint(uint paySelect, uint256 quantity, bytes32[] calldata _merkleProof, bool marketingSelect) external payable {
    ...
    function whiteListClaimedPaid( uint quantity, bytes32[] calldata _merkleProof, bool marketingSelect ) internal { // uint mintAllowed, // uint quantity
        
        require ( addressWhitelisted[msg.sender] == false, "Already minted/Not whiteListed" );
        uint phase = 1;
        bytes32 leaf = keccak256(abi.encodePacked(msg.sender, phase, quantity, marketingSelect) );
    ...
    function setTomiToken(IERC20 _tomiToken) public onlyOwner {
        tomi = _tomiToken;
    }
