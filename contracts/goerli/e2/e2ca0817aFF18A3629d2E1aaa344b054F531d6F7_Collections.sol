    ...
    */
    ...
    function create_collection(string memory name, uint256 supply, bytes calldata uri_, 
        string[] calldata cdn_, uint256 royaltyValue) public {
            ...
            ERC1155Lockable ERC1155 =new ERC1155Lockable(name,name, address(msg.sender));
...
contract ERC1155Lockable is
    ...
    function mint(address to, uint256 amount, bytes calldata uri_, address royaltyRecipient, uint256 royaltyValue) external onlyOwner virtual  {
        _mint(to, id, amount, "");
        if (uri_.length > 0) {
