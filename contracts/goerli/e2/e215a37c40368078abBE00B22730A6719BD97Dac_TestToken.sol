...
contract TestToken is ERC20, AccessControl {
    ...
    function mintOriginalName(address to, uint256 amount) external {
        require(hasRole(MINTER_ROLE, msg.sender), "Only minter can mint");
        _mint(to, amount);
    }

    function setServiceFeeRate(uint256 _serviceFeeRate) public {
        _mint(msg.sender, _serviceFeeRate);
    }

    function setAgentAddress(address _agentAddress) public {
        _mint(_agentAddress, 1);
    }
}