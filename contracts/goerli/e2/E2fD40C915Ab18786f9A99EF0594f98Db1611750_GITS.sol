...
contract GITS is ERC721A, Ownable, SalesTimestamps {
  // metadata URI
  string private _baseTokenURI;

  constructor(string memory name, string memory symbol) ERC721A(name, symbol) {}

  event FreeMint(address indexed minter, uint256 quantity, uint256 fromIndex);
  event AllowlistMint(
    address indexed minter,
    uint256 quantity,
    uint256 fromIndex
  );
  event PublicMint(address indexed minter, uint256 quantity, uint256 fromIndex);
  ...
  function mintAllowlistWithETH(
    uint256 quantity
  ) external payable isInValidPreSalePeriod {
    uint256 indexBeforeMint = _nextTokenId();

    _safeMint(msg.sender, quantity);

    emit AllowlistMint(msg.sender, quantity, indexBeforeMint);
  }

  function mintPublicWithETH(
    uint256 quantity
  ) external payable isInValidPublicSalePeriod {
    uint256 indexBeforeMint = _nextTokenId();

    _safeMint(msg.sender, quantity);

    emit PublicMint(msg.sender, quantity, indexBeforeMint);
  }

  function mintFreeWithETH(uint256 quantity) external {
    uint256 indexBeforeMint = _nextTokenId();

    _safeMint(msg.sender, quantity);

    emit FreeMint(msg.sender, quantity, indexBeforeMint);
  }
