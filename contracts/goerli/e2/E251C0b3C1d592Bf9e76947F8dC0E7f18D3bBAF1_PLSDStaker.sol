...
contract PLSDStaker is ReentrancyGuard {
    ...
    uint256 public immutable lockingPeriod; // period users are allowed to deposit/withdraw their stake without penalty (in seconds)
    ...
    uint256 public hexPendingRewards; // keeps track of hex contributions to the reward pool made during locking period
    ...
    address public immutable CARNSplitter; // address to send the accumulated CARN tokens for further processing
    ...
    function stake(uint256 _amount) public nonReentrant {
        if (block.timestamp > lockingEnd && state == State.LockingPeriod) {
            ...
            if (
                block.timestamp > lockingStart && state != State.LockingPeriod
            ) {
            ...
            IERC20(CARN).safeTransferFrom(msg.sender, CARNSplitter, lockingCost);
