...
contract FlexibleStaking is Pausable, Ownable {
  ...
  event SetConditions(uint256[] expiresAfter);
  ...
  mapping(uint256 => bool) private _expiresAfterToPercent;
    ...
    returns (bool)
  {
    ...
    require(_expiresAfterToPercent[expiresAt] == true, "Invalid expiresAt");
  ...
  function getConditions()
    ...
    view
    virtual
    returns (uint256[] memory availableExpiresAfter)
  {
    return (_availableExpiresAfter);
  }
  ...
  function setConditions(uint256[] memory expiresAfter) public onlyOwner {
    ...
    for (uint256 i = 0; i < _availableExpiresAfter.length; i++) {
      _expiresAfterToPercent[_availableExpiresAfter[i]] = false;
    }

    // # set new values
    for (uint256 i = 0; i < expiresAfter.length; i++) {
      _expiresAfterToPercent[expiresAfter[i]] = true;
    }

    _availableExpiresAfter = expiresAfter;

    emit SetConditions(expiresAfter);
  }
