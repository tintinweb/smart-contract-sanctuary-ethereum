    ...
    ReentrancyGuardUpgradeable
{
    ...
    function mintPublic(uint256 quantity) external payable {
        _checkPublicMintConditions(msg.sender, quantity, msg.value);

        _mint(msg.sender, quantity);
    }

    /**
     * @dev Mint tokens in all possible phases (including public sale)
     */
    function mintBatch(
        ...
        if (phaseLength > saleState.numPhases) {
            revert InvalidPhase();
        }
        ...
        for (uint256 i = 0; i < phaseLength; ) {
            ...
            // balance underflow is checked in _checkPhaseMintConditions & _checkAllowlistPhaseMintConditions
            unchecked {
                saleState.phases[phaseIndex].amountMinted += uint64(quantity);
        ...
        if (publicQuantity > 0) {
            _checkPublicMintConditions(msg.sender, publicQuantity, balance);
        }

        _mint(msg.sender, quantityToMint + publicQuantity);
    }
     ...
     * The burn method will revert unless this is enabled
     */
    ...
    function getDataForPhase(uint256 phaseIndex)
        external
        view
    ...
    function getAmountMintedForPhase(uint256 phaseIndex, address wallet)
        external
        view
        returns (uint64)
    {
    ...
    function getAmountMintedForOwner(address wallet)
        external
        view
