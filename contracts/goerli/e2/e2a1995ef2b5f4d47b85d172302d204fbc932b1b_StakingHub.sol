    ...
    PausableUpgradeable
{
     ...
     * @param claimTime The time at which the withdrawa was made
     */
    event Claim(
        address indexed staker,
        ...
        returns (uint256 amountClaimed)
    {
        ...
        amountClaimed = stakingPosition.unclaimedReward;
        if (amountClaimed <= 0) {
    ...
    function getStakingPosition(
        uint256 tokenId
    ) public view onlyExist(tokenId) override returns (StakingPosition memory stakingPosition) {
        ...
        for (uint256 j = 1; j <= totalStakingPositions; j++) {
            ...
            if (currentStakingPosition.amount > 0) {
                ...
                if (currentStakingPosition.createdAt >= stakingTimeThreshold) {
                    rewardingAmount = amountToDispatch  * currentStakingPosition.amount / totalTokenStaked  * LOW_TIER_RATIO / 100;
                } else {
                    rewardingAmount =
                        (amountToDispatch * currentStakingPosition.amount / totalTokenStaked * LOW_TIER_RATIO / 100) +
                        (amountToDispatch * currentStakingPosition.amount / fullRewardTokenBalance * HIGH_TIER_RATIO / 100);
                }
