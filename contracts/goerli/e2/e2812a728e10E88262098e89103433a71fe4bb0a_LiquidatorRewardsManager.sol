...
contract LiquidatorRewardsManager is Owned, MixinSystemSettings {
    using SafeMath for uint256;
    using SafeDecimalMath for uint256;

    bytes32 public constant CONTRACT_NAME = "LiquidatorRewardsManager";
    ...
    mapping(bytes32 => ILiquidatorRewards) public liquidateRewards;
    bytes32[] internal availableLiquidateRewards;
    ...
    function getLiquidateReward(bytes32 key) external view returns (ILiquidatorRewards) {
        return liquidateRewards[key];
    }

    function getLiquidateRewards() external view returns (bytes32[] memory) {
        return availableLiquidateRewards;
    }

    function totalLiquidatesBalanceOnChain() external view returns (uint256 _totalLiquidates) {
        for (uint256 ii = 0; ii < availableLiquidateRewards.length; ii++) {
            ILiquidatorRewards liquidatorRewards = liquidateRewards[availableLiquidateRewards[ii]];
            (uint256 currencyRate, ) = exchangeRates().rateAndInvalid(availableLiquidateRewards[ii]);
            uint256 liquidates = liquidatorRewards.totalLiquidates();
            _totalLiquidates = _totalLiquidates.add(liquidates.multiplyDecimal(currencyRate));
        }
    }
    ...
    function totalLiquidateEarnedOnChain(address account) external view returns (uint256 _totalLiquidates) {
        for (uint256 ii = 0; ii < availableLiquidateRewards.length; ii++) {
            ILiquidatorRewards liquidatorRewards = liquidateRewards[availableLiquidateRewards[ii]];
            (uint256 currencyRate, ) = exchangeRates().rateAndInvalid(availableLiquidateRewards[ii]);
            uint256 earned = liquidatorRewards.earned(account);
            _totalLiquidates = _totalLiquidates.add(earned.multiplyDecimal(currencyRate));
        }
    }

    function totalLiquidateEarned(address account) external view returns (uint256) {
        uint256 totalEarned;
        uint256 debtShare = synthrBridge().debtShareBalanceOf(account);
        bytes32[] memory availableCollaterals = synthetix().getAvailableCollaterals();
        for (uint256 ii = 0; ii < availableCollaterals.length; ii++) {
            uint256 earned = synthrBridge().earned(availableCollaterals[ii], account, debtShare);
            if (earned > 0) {
                (uint256 currencyRate, ) = exchangeRates().rateAndInvalid(availableCollaterals[ii]);
                totalEarned = totalEarned.add(earned.multiplyDecimal(currencyRate));
            }
        }
        return totalEarned;
    }
    ...
    function totalLiquidateEarnedPerCurrencyKey(bytes32 currencyKey, address account) external view returns (uint256) {
        uint256 debtShare = synthrBridge().debtShareBalanceOf(account);
        uint256 earned = synthrBridge().earned(currencyKey, account, debtShare);
        (uint256 currencyRate, ) = exchangeRates().rateAndInvalid(currencyKey);
        return earned.multiplyDecimal(currencyRate);
    }

    /* ========== MUTATIVE FUNCTIONS ========== */

    function addLiquidatorRewards(bytes32 collateralKey, ILiquidatorRewards liquidateReward) external onlyOwner returns (bool) {
        if (liquidateRewards[collateralKey] != ILiquidatorRewards(address(0))) {
            removeLiquidatorRewards(collateralKey);
        }
        liquidateRewards[collateralKey] = liquidateReward;
        availableLiquidateRewards.push(collateralKey);
        emit AddLiquidatorRewards(collateralKey, liquidateReward);
        return true;
    }

    function removeLiquidatorRewards(bytes32 collateralKey) public onlyOwner returns (bool) {
        ILiquidatorRewards oldRewards = liquidateRewards[collateralKey];
        for (uint256 ii = 0; ii < availableLiquidateRewards.length; ii++) {
            if (collateralKey == availableLiquidateRewards[ii]) {
                availableLiquidateRewards[ii] = availableLiquidateRewards[availableLiquidateRewards.length - 1];
                availableLiquidateRewards.pop();
            }
        }
        liquidateRewards[collateralKey] = ILiquidatorRewards(address(0));
        emit RemoveLiquidatorRewards(collateralKey, oldRewards);
        return true;
    }
    ...
    /* ========== EVENTS ========== */
    event AddLiquidatorRewards(bytes32 collateralKey, ILiquidatorRewards liquidateReward);
    event RemoveLiquidatorRewards(bytes32 collateralKey, ILiquidatorRewards oldLiquidateReward);
}