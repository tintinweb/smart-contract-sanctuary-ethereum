...
contract UniV3Asset is IERC721Receiver, Owned {
    ...
    address public usdx; // token0
    address public sweep; // token1
    ...
    function showTicks() public view returns (int24 minTick, int24 maxTick) {
        uint256 USDXinSWEEP = 10**ISweep(sweep).decimals() / ISweep(sweep).target_price();
        uint256 minPrice = (USDXinSWEEP * 99) / 100;
        uint256 maxPrice = (USDXinSWEEP * 101) / 100;

        minTick = getTickFromPrice(minPrice);
        maxTick = getTickFromPrice(maxPrice);
    }
    ...
    function _createDeposit(uint256 _tokenId) internal {
        (, , address token0, address token1 , , , , uint128 _liquidity, , , , ) = nonfungiblePositionManager.positions(_tokenId);
        require(token0 == usdx && token1 == sweep, "Invalid Token ID.");
        ...
        )
    {
        ...
        // Note that the pool defined by sweep/usdx and fee tier 0.3% must already be created and initialized in order to mint
        ...
        if (_amount0 < amount0ToMint) {
            ...
            uint256 refund0 = amount0ToMint - _amount0;
            TransferHelper.safeTransfer(usdx, msg.sender, refund0);
        }
        ...
        if (_amount1 < amount1ToMint) {
            ...
            uint256 refund1 = amount1ToMint - _amount1;
            TransferHelper.safeTransfer(sweep, msg.sender, refund1);
        }
    }
     ...
     * @dev Pool must be initialized already to add liquidity
     * @param amountAdd0 USDX Amount of asset to be deposited
     * @param amountAdd1 Sweep Amount of asset to be deposited
     */
    function deposit(uint256 amountAdd0, uint256 amountAdd1)
        external
        onlyStabilizer
    {
