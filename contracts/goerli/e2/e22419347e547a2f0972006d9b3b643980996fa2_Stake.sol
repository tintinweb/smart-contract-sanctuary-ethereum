...
contract Stake is Ownable {
    ...
    event Staked(address owner, uint stakeId, uint amount);
    function createStake(uint256 amount,uint8 lockChoice,address refereeAddress) public returns(uint256 stakeId){
        ...
        stakesList[stakeCount++] = 
            UserStake(msg.sender,amount,block.timestamp,lockTimes[lockChoice],lockChoice,block.timestamp+lockTimes[lockChoice],block.timestamp,0,refereeAddress,true);
