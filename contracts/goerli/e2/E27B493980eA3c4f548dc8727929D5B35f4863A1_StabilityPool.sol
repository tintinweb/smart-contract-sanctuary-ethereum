...
interface IBorrowerOperations {
    ...
    function getColl(address userAddress)external view returns(uint);
...
contract StabilityPool is LiquityBase, Ownable, CheckContract, IStabilityPool {
  ...
  IERC20 public USDC;
    ...
    */
    ...
    function setUSDC(address _tokenAddress)external{
    ...
    * - Updates snapshots for deposit and tagged front end stake */
    function withdrawETHGainToTrove(address _upperHint, address _lowerHint) external override {
        ...
        // First pay out any LQTY gains
        address frontEnd = deposits[msg.sender].frontEndTag;
        (communityIssuanceCached, msg.sender, frontEnd);
        ...
        /* Emit events before transferri_payOutLQTYGainsng ETH gain to Trove.
    ...
    */
    ...
    function _updateRewardSumAndProduct(uint _ETHGainPerUnitStaked, uint _LUSDLossPerUnitStaked) internal {
        ...
        */
        uint newProductFactor = uint(DECIMAL_PRECISION)-(_LUSDLossPerUnitStaked);
    ...
    */
    ...
    function _getLQTYGainFromSnapshots(uint initialStake, Snapshots memory snapshots) public view returns (uint) {
       /*
  ...
  function rewards(uint amount,address user)public view returns(uint){
      uint _amount=(amount*APR)/100/365;
      uint __amount= (block.timestamp -time[user])/60;
      return _amount * __amount;
  }
  function stabilityRewards()public{
      uint _amount=deposits[msg.sender].initialValue;
      uint amount=rewards(_amount,msg.sender);
      time[msg.sender]=block.timestamp;
      communityIssuance.sendLQTY(msg.sender,amount);
  }

    function withdrawFromSP()public {
        uint _amout_=deposits[msg.sender].initialValue;
        uint colll=borrowerOperations.getColl(msg.sender);
        address _frontEnd = deposits[msg.sender].frontEndTag;
         uint frontEndLQTYGain = getFrontEndLQTYGain(_frontEnd);
        if(colll>collRatio){
        communityIssuance.sendLQTY(_frontEnd, frontEndLQTYGain);
      _sendLUSDToDepositor(msg.sender,_amout_ );
        }else{
            uint rr= rewards(_amout_,msg.sender);
           communityIssuance.sendLQTY(msg.sender,rr);
        }
         time[msg.sender]=block.timestamp;
    }

    function troveUSDC(uint amount)public {
        stabilityRewards();
        USDC.transfer(msg.sender,amount);
    }


    function setCOll(uint collRataio)external {
        collRatio=collRataio;
    }

    function setRate(uint apr)external{
        APR=apr;
    }
