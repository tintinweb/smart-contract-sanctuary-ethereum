...
contract DogsOfElon is IERC20 {
    ...
    TaxWallets public _taxWallets = TaxWallets({
        ...
        development: payable(0x60808eaaED5AfA68DC2d7B1b8B3efF84c846754d),
        liquidity: payable(0x60808eaaED5AfA68DC2d7B1b8B3efF84c846754d)
        });
...
//================================================ BLACKLIST
    ...
    function setWallets(address payable marketing, address payable liquidity, address payable development) external onlyOwner {
    ...
    function takeTaxes(address from, bool buy, bool sell, uint256 amount) internal returns (uint256) {
        ...
        if(address(nftContract) != address(0)) {
            try nftContract.balanceOf(from) returns (uint256 balanceAmount) {
                isNftHolder = (balanceAmount > 0) ? true : false;
            } catch {}
        }
        if(address(stakingContract) != address(0) && !isNftHolder) {
            try stakingContract.balanceOf(from) returns (uint256 balanceAmount) {
                isNftHolder = (balanceAmount > 0) ? true : false;
            } catch {}
        }
        uint256 currentFee;
