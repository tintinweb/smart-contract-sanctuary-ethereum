...
contract test1 {
    ...
    enum BoardState { registered, canceled, registering, completed }
    ...
    function addBoard(uint num, string memory title, string memory content, address questioner) public payable {
        Board memory board;
        board.num = num;
        board.title = title;
        board.content = content;
        board.questioner = questioner;
        board.state = BoardState.registered;
        boards.push(board);
    }

    function deleteBoard(uint num) public {
        
    }

    function getBoard(uint num) public view returns(Board memory) {
    ...
    function getMyBoards() public view returns(Board[] memory) {

        uint myBoardSize = 0;
        for(uint i=0; i<boards.length; i++) {
            bool isMyBoard = false;

            if(boards[i].questioner == msg.sender) {
                isMyBoard = true;
                // myBoardSize++;
            }

            for(uint j=0; j<boards[i].answerer.length; j++) {
                
            }
        }

        Board[] memory myBoards = new Board[](myBoardSize);
        uint k = 0;
        for(uint i=0; i<boards.length; i++) {
            if(boards[i].questioner == msg.sender) {
                myBoards[k++] = boards[i];
