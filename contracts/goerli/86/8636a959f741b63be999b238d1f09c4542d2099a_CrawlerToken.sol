...
library Crawl {
  ...
  //
	...
	uint256 private constant mask_Dir = uint256(type(uint64).max);
	uint256 internal constant mask_North = (mask_Dir << 192);
	uint256 internal constant mask_East = (mask_Dir << 128);
	uint256 internal constant mask_West = (mask_Dir << 64);
	uint256 internal constant mask_South = mask_Dir;
	uint256 internal constant mask_NorthSouth = mask_North + mask_South;
	uint256 internal constant mask_EastWest = mask_East + mask_West;
	uint256 internal constant mask_NorthInv = ~mask_North;
	uint256 internal constant mask_EastInv = ~mask_East;
	uint256 internal constant mask_WestInv = ~mask_West;
	uint256 internal constant mask_SouthInv = ~mask_South;
	...
	function offsetCoord(uint256 coord, Crawl.Dir dir) internal pure returns (uint256) {
		if(dir == Crawl.Dir.North) {
			...
			if(coord & mask_North != mask_North) return (coord & mask_SouthInv) + (1 << 192); // ++North
		} else if(dir == Crawl.Dir.East) {
			if(coord & mask_West > (1 << 64)) return coord - (1 << 64); // --West
			if(coord & mask_East != mask_East) return (coord & mask_WestInv) + (1 << 128); // ++East
		} else if(dir == Crawl.Dir.West) {
			if(coord & mask_East > (1 << 128)) return coord - (1 << 128); // --East
			if(coord & mask_West != mask_West) return (coord & mask_EastInv) + (1 << 64); // ++West
		} else { //if(dir == Crawl.Dir.South) {
			if(coord & mask_North > (1 << 192)) return coord - (1 << 192); // --North
			if(coord & mask_South != mask_South) return (coord & mask_NorthInv) + 1; // ++South
		}
		return coord;
	}
...
// File: contracts/extras/ECDSA.sol
...
// File: contracts/extras/IERC721Enumerable.sol
...
// File: contracts/extras/ERC721Enumerable.sol
...
// File: contracts/ICrawlerGenerator.sol
...
// File: contracts/ICrawlerToken.sol
...
// File: contracts/ICrawlerChamberGenerator.sol
...
// File: contracts/ICrawlerMapper.sol
...
// File: contracts/ICrawlerRenderer.sol
...
// File: contracts/ICrawlerIndex.sol
...
// File: contracts/ICrawlerPlayer.sol
...
// File: contracts/CrawlerToken.sol
...
contract CrawlerToken is ERC721, ERC721Enumerable, Ownable, ICrawlerToken {

	error MintingIsPaused();
	error InvalidFromChamber();
	error InvalidValue(uint256 sent, uint256 minimum);
	error InvalidSignature();
	error DoorAlreadyUnlocked();
	error InvalidTokenId(uint256 tokenId);

	ICrawlerIndex public _index;
  ICrawlerPlayer public _player;
	address private _signerAddress;
	uint256 private _mintedCount;
	uint256 private _priceInPwei = 10;
	uint256 private _maxPriceInPwei = 30;
	...
	/// @notice Admin function
	function setPrice(uint256 priceInPwei_, uint256 maxPriceInPwei_) public onlyOwner {
		_priceInPwei = priceInPwei_;
		_maxPriceInPwei = maxPriceInPwei_;
	}
	...
	/// @return prices Prices in PWEI (1 pwei = ETH/1000): single token price, max token price
	function getPrices() public view override returns (uint256, uint256) {
		return (_priceInPwei, _maxPriceInPwei);
	}

	/// @notice Return the current mint prices
	/// Price is FREE for the first token
	/// Price is FREE when minted in-game, provided signature
	/// Otherwise, price is _priceInPwei per owned token, capped at _maxPriceInPwei
	...
	function calculateMintPrice(address to) public view override returns (uint256) {
		return to == owner() ? 0 : Crawl.min(balanceOf(to) * _priceInPwei, _maxPriceInPwei) * 1_000_000_000_000_000;
	}
	...
	function mintDoor(uint256 fromCoord, Crawl.Dir dir, bytes calldata signature) public payable returns (uint256) {
		...
		if(_coordToSeed[newCoord].tokenId > 0) revert DoorAlreadyUnlocked();
		...
		if(signature.length > 0) {
			...
			// Validate price
			uint256 mintPrice = calculateMintPrice(msg.sender);
			if(msg.value < mintPrice) revert InvalidValue(msg.value, mintPrice);
		}

		// Chamber type is defined by a super simple cellular automata
		...
		Crawl.Terrain terrain =
			fromChamber.terrain != _coordToSeed[Crawl.offsetCoord(fromCoord, entryDir)].terrain
			? Crawl.Terrain(fromChamber.terrain)
			: _index.getChamberGenerator().generateTerrainType(fromChamber.seed+uint256(dir), Crawl.Terrain(fromChamber.terrain));
