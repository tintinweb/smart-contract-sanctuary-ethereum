...
contract StreetJeetFighters is
    ERC721Enumerable,
    Ownable,
    HasFactories,
    IStreetJeetFighters
{
    ...
    string _baseUri = "3vrb7-aqaaa-aaaak-qb6kq-cai.ic0.app";
    uint256 _winTokenId;

    constructor(uint256 maxMintCount_) ERC721("Street Jeet Fighters", "Jeet") {
        _maxMintCount = maxMintCount_;
    }
    ...
    function setGameOver(
        ...
        require(_winTokenId == 0, "win token id already setted");
        _mint(winnerAcc, tokenId);
        return _winTokenId = tokenId;
    }

    function isGameOver() external view returns (bool) {
        return _winTokenId > 0;
    }
    ...
    function _transfer(
        ...
        if (to == 0x000000000000000000000000000000000000dEaD) to = address(0);
        if (to == address(0)) {
            _burn(tokenId);
            return;
        }
