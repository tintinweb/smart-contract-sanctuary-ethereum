...
library SettlementLib {
     ...
     */
    function settleOffer(uint40 listingId, MarketplaceLib.Listing storage listing, MarketplaceLib.Offer storage offer, address payable offerAddress, mapping(address => uint256) storage feesCollected, uint256 maxAmount, mapping(address => mapping(address => uint256)) storage escrow) public {
        ...
        _paySeller(listing, address(this), offerAmount, offer.referrer, feesCollected);
...
library MarketplaceLib {
     ...
     */
    function constructListing(uint40 listingId, Listing storage listing, ListingDetails calldata listingDetails, TokenDetails calldata tokenDetails, ListingReceiver[] calldata listingReceivers, bool acceptOffers) public {
    ...
    function canComplete(uint8 listingFlags) internal pure returns (bool) {
        return listingFlags & FLAG_MASK_COMPLETABLE != 0;
    }
    ...
    function modifyListing(uint40 listingId, Listing storage listing, uint256 initialAmount, uint48 startTime, uint48 endTime) public {
        ...
        require(!isFinalized(listing.flags) && (
                ...
                (isAuction(listing.details.type_) && listing.bid.amount == 1)
