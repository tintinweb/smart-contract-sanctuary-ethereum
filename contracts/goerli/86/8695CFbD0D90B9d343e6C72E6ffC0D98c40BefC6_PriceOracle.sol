...
contract PriceOracle {
    ...
    mapping(string => priceFeedData) internal mappingRate; 
    
    function setPrice(string memory currencyPair, string memory price) internal {   
        mappingRate[currencyPair].exchangeRate = price;
        mappingRate[currencyPair].updateTimeStamp = block.timestamp;
    }
    
    function getPrice(string memory currencyPair) public view returns(string memory, uint256){
       return (mappingRate[currencyPair].exchangeRate, mappingRate[currencyPair].updateTimeStamp);
    }

    function storeFeed(string memory _feeds) public {
