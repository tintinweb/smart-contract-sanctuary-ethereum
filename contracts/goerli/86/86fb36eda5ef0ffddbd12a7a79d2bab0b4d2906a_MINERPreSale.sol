...
contract MINERPreSale is Ownable {
    ...
    mapping(address => bool) public boughtMINER;
    ...
    function blackListBuyers(address[] memory _buyers) external onlyOwner {
        totalWhiteListed = totalWhiteListed.sub(_buyers.length);

        for (uint256 i; i < _buyers.length; i++) {
            whiteListed[_buyers[i]] = false;
        }
    }

    function getAllotmentPerBuyer() public view returns (uint256) {
        return IERC20(aMINER).balanceOf(address(this)).div(totalWhiteListed);
    }

    function purchaseaMINER(uint256 _token, uint256 _amount) external returns (bool) {
        require(saleStarted, "Not started");
        require(whiteListed[msg.sender], "Not whitelisted");
        require(!boughtMINER[msg.sender], "Already participated");
        require(block.timestamp < endOfSale, "Sale over");

        boughtMINER[msg.sender] = true;
        ...
        soldTokens = soldTokens.add(_purchaseAmount);

        IERC20(srcTokenAddr[_token]).safeTransferFrom(msg.sender, addressToSend, _amount);
        IERC20(aMINER).safeTransfer(msg.sender, _purchaseAmount);
