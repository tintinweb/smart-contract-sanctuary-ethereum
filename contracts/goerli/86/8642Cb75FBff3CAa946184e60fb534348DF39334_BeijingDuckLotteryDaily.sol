...
contract BeijingDuckLotteryDaily is ReentrancyGuard, Ownable {
    ...
    uint256 public maxNumberTicketsPersonBuyOne = 100;
    uint256 public maxNumberTicketsPersonBuy = 1000;
    ...
    mapping(uint256 => uint256) public NONCE_TICKET;
    mapping(uint256 => mapping(uint256 => Ticket)) public tickets; //tickets[lotteryID][ticketID] = { uint32 number; address user; uint256 purchaseDate; }
    
    mapping(uint256 => mapping(uint32 => address)) public ticketUsers; //tickets[lotteryID][numberTicket] = user address
    mapping(uint256 => mapping(address => EnumerableSet.UintSet)) private userTickets; //tickets[lotteryID][address] = [ uint32, uint32, ...]

    mapping(uint256 => EnumerableSet.UintSet) private allNumberTickets; //allNumberTickets[lotteryID] = [uint32, uint32, ...]

    mapping(address => EnumerableSet.UintSet) private winTickets; //winTickets[address] = [lotteryID, lotteryID, ...]
    ...
    event StopLottery(uint256 lotteryID);
    event UnStopLottery(uint256 lotteryID);
    event BuyTickets(address indexed user, uint256 indexed lotteryID, uint256 amountDuckToTransfer, uint32[] numberTickets);
    event PickWinner(uint256 indexed lotteryID, uint32 numberWinner, address winner);
    event TicketsClaim(address user, uint256 rewardInDuckToTransfer);
    event AdminTokenRecovery(address token, uint256 amount);

    constructor() {
        duckToken = IERC20(0x551e6978240b11B07B1eF3402Add944c1C3C9d32);
    }
    ...
    function createLottery(uint256 _startTime, uint256 _endTime, uint256 _priceTicketInDuck) external onlyOwner nonReentrant {
        ...
        NONCE_LOTTERY ++;
        emit CreateLottery(_lottery.lotteryID, _lottery.startTime, _lottery.endTime, _lottery.priceTicketInDuck);
    }

    function stopLottery(uint256 _lotteryID) external onlyOwner nonReentrant {
        Lottery storage _lottery = lotteries[_lotteryID];
        _lottery.stop = true;
        emit StopLottery(_lottery.lotteryID);
    }

    function unStopLottery(uint256 _lotteryID) external onlyOwner nonReentrant {
        Lottery storage _lottery = lotteries[_lotteryID];
        _lottery.stop = false;
        emit UnStopLottery(_lottery.lotteryID);
    }

    function buyTickets(uint256 _lotteryID, uint32[] calldata _ticketNumbers) external notContract nonReentrant {
        require(_ticketNumbers.length > 0, "No ticket specified");
        require(_ticketNumbers.length <= maxNumberTicketsPersonBuyOne, "Too many tickets one buy");
        require(_ticketNumbers.length + userTickets[_lotteryID][msg.sender].length() <= maxNumberTicketsPersonBuy, "Over quantity many tickets can buy");

        require(lotteries[_lotteryID].stop == false, "Lottery is not open");
        require(block.timestamp >= lotteries[_lotteryID].startTime && block.timestamp <= lotteries[_lotteryID].endTime, "Not the time to buy");

        uint256 amountDuckToTransfer = _calculateTotalPriceForBuyTickets(lotteries[_lotteryID].priceTicketInDuck, _ticketNumbers.length);
        ...
        for (uint256 i = 0; i < _ticketNumbers.length; i++) {
            ...
            require((thisTicketNumber >= 100000) && (thisTicketNumber <= 999999), "Outside range");
            require(!allNumberTickets[_lotteryID].contains(thisTicketNumber), "Ticket already exists");

            tickets[_lotteryID][NONCE_TICKET[_lotteryID]] = Ticket({number: thisTicketNumber, user: msg.sender, purchaseDate: block.timestamp});

            NONCE_TICKET[_lotteryID] ++;
    ...
    function pickWinner(uint256 _lotteryID) external onlyOwner nonReentrant {
        ...
        lotteries[_lotteryID].winner = ticketUsers[_lotteryID][_numberWinner];

        winTickets[lotteries[_lotteryID].winner].add(_lotteryID);

        uint256 amountBurnDuck = lotteries[_lotteryID].totalDuck * 10 / 100;
        duckToken.burn(amountBurnDuck);

        emit PickWinner(_lotteryID, lotteries[_lotteryID].numberWinner, lotteries[_lotteryID].winner);
    }
    ...
    function claimTickets() external notContract nonReentrant {
        ...
        for (uint256 i = 0; i < winTickets[msg.sender].length(); i++) {
            uint256 _lotteryID = winTickets[msg.sender].at(i);
    ...
    function _calculateTotalDuckForClaimTickets(address winner) private view returns (uint256) {
        uint256 reward = 0;
        for (uint256 i = 0; i < winTickets[winner].length(); i++) {
