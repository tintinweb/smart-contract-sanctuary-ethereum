...
contract InkePassExtra is ERC721 {
    address owner;
    ...
    struct tokenIdsInfo {
        uint256 total;
        mapping(uint256 => bool) _tokenIdsBoolMap;
    }

    mapping(address => tokenIdsInfo) private _tokenIdsMap;

    constructor() ERC721("InkePassExtra", "IPET") {
        owner = msg.sender;
    }
    ...
    function appendTokenIdsMapValue(address user_, uint256[] calldata tokenIds_) public atLeastManager {
        require(tokenIds_.length > 0, "Length of tokenIds_ must be bigger than zero");

        for (uint256 i = 0; i < tokenIds_.length; i++) {
            if (_tokenIdsMap[user_]._tokenIdsBoolMap[tokenIds_[i]] == false) {
                _tokenIdsMap[user_]._tokenIdsBoolMap[tokenIds_[i]] = true;
                _tokenIdsMap[user_].total++;
            }
        }
    }

    function deleteTokenIdsMapValue(address user_, uint256[] calldata tokenIds_) public atLeastManager {
        require(tokenIds_.length > 0, "Length of tokenIds_ must be bigger than zero");

        for (uint256 i = 0; i < tokenIds_.length; i++) {
            if (_tokenIdsMap[user_]._tokenIdsBoolMap[tokenIds_[i]] == true) {
                delete _tokenIdsMap[user_]._tokenIdsBoolMap[tokenIds_[i]];
                _tokenIdsMap[user_].total--;
            }
        }
    }

    function validateTokenIds(address user_, uint256[] calldata tokenIds_) public view returns (bool[] memory) {
        require(tokenIds_.length > 0, "Length of tokenIds_ must be bigger than zero");
        bool[] memory validateResults;
        for (uint256 i = 0; i < tokenIds_.length; i++) {
            validateResults[i] = _tokenIdsMap[user_]._tokenIdsBoolMap[tokenIds_[i]];
        }
    ...
    function balanceOf(address user_) public view virtual override returns (uint256) {
        return _tokenIdsMap[user_].total;
    }
    ...
    modifier atLeastManager() {
        uint8 permission = _permissions[msg.sender];
        require(
            msg.sender == owner || permission == MANAGER,
            "not authorized"
        );
        _;
    }
