...
interface IMoxieAddressRegistry {
...
interface IMoxieTokenRegistry {
    function enabled(address) external returns (bool);
}

contract MoxieBundleMarketplace is Ownable, ReentrancyGuard {
    ...
    IMoxieAddressRegistry public addressRegistry;
        ...
        require(
            _payToken == address(0) ||
                (tokenRegistry != address(0) &&
                    IMoxieTokenRegistry(tokenRegistry).enabled(_payToken)),
            "invalid pay token"
        );
    ...
    function _buyItem(string memory _bundleID, address _payToken) private {
        ...
        if (_payToken == address(0)) {
            ...
            require(
                feeTransferSuccess,
                "MoxieMarketplace: Fee transfer failed"
            );
            ...
            require(
                ...
                "MoxieMarketplace: Owner transfer failed"
            );
        } else {
        ...
        for (uint256 i; i < listing.nfts.length; i++) {
            ...
            IMoxieMarketplace(addressRegistry.marketplace()).validateItemSold(
                listing.nfts[i],
                listing.tokenIds[i],
                owner,
                _msgSender()
        ...
        emit ItemSold(
            ...
            _bundleID,
            _payToken,
            IMoxieMarketplace(addressRegistry.marketplace()).getPrice(
                _payToken
            ),
            price
        );
    ...
    /**
     @notice Update MoxieAddressRegistry contract
     @dev Only admin
     */
    function updateAddressRegistry(address _registry) external onlyOwner {
        addressRegistry = IMoxieAddressRegistry(_registry);
    }
