...
contract CapsuleFactory is Initializable, Governable, CapsuleFactoryStorageV2 {
    ...
    event CapsuleCollectionBurnerUpdated(
        address indexed capsuleCollection,
    ...
    /**
     * @notice Collection burner can burn NFT from given collection.
     * Burner still have to own the NFT before they can burn unless burner is added
     * as whitelistedCaller in CapsuleMinter.
     * @notice An address is collectionBurner
     * - if collectionBurner is equal to address(0) aka not set.
     * - or given account is set as collectionBurner
     */
    function isCollectionBurner(address _capsule, address _account) external view returns (bool) {
        return collectionBurner[_capsule] == address(0) || collectionBurner[_capsule] == _account;
    }
    ...
    /******************************************************************************
     *                           Capsule Owner function                           *
     *****************************************************************************/
    /**
     * @notice Caller can only update burner if
     *      - Caller is the owner of collection and
     *      - Caller is a contract and
     *      - new burner is address(0) or it is a contract.
     * @param _capsule Capsule collection address
     * @param _newBurner New address of collection burner. Address(0) is valid input.
     */
    function updateCapsuleCollectionBurner(address _capsule, address _newBurner) external {
        address _caller = _msgSender();
        require(ICapsule(_capsule).isCollectionPrivate(), Errors.NOT_PRIVATE_COLLECTION);
        require(
            _caller == Capsule(_capsule).owner() &&
                Address.isContract(_caller) &&
                (_newBurner == address(0) || Address.isContract(_newBurner)),
            Errors.UNAUTHORIZED
        );
        emit CapsuleCollectionBurnerUpdated(_capsule, collectionBurner[_capsule], _newBurner);
        collectionBurner[_capsule] = _newBurner;
    }
...
abstract contract CapsuleFactoryStorageV2 is CapsuleFactoryStorage {
    /**
     * @notice An address which can call burn() in CapsuleMinter. It is default to 0 address.
     * By default only NFT owners and whitelistedCallers can call burn() in CapsuleMinter.
     * Capsule collection address -> collection burner mapping
     */
    mapping(address => address) public collectionBurner;
}
