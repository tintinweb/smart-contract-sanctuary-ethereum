...
contract objectron is Ownable, ERC721A, ReentrancyGuard, AccessControl {
    constructor(
        ...
        setBaseURI("ipfs://QmUszUmLAaEgZN4UpwjWJgmZTzs75drU3y5LFkKjNs9dbj/");
        //　ERC721Aの_safeMint関数を実行し、指定のアドレスに1つmintする(dev1)
        _safeMint(0x8bfbaCC9A8e7fc2e9735B3B7982408F5ABFD1a38, 1); 
    }
    ...
    **/
    ...
    function mint(uint256 _mintAmount ) public payable callerIsUser{
        ...
        if(mintCount == true){
                // 入力されたmint数が、アドレスあたりの最大mint量(デフォルト300) - tx実行のアドレス（msg.sender）によってパブリックセールで発行されたトークン量を超えていないか確認
                require(_mintAmount <= maxPerAddressDuringMint - publicSaleMintedAmount[msg.sender] , "max NFT per address exceeded");
    ...
    function airdropMintOne(address _airdropAddresses , uint256 _UserMintAmount) public {
        // AccessControl.solのhasRole関数を実行
        // txアドレス（msg.sender）がAIRDROP_ROLEを持っているかの確認
        require(hasRole(AIRDROP_ROLE, msg.sender), "Caller is not a air dropper");
        // mint量を初期化
