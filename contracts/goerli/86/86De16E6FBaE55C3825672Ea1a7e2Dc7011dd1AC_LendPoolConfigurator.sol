...
contract LendPoolConfigurator is Initializable, ILendPoolConfigurator {
   ...
   **/
  function updateCToken(ConfigTypes.UpdateCTokenInput[] calldata inputs) external onlyPoolAdmin {
    ILendPool cachedPool = _getLendPool();
  ...
  function _checkReserveNoLiquidity(address asset) internal view {
    ...
    uint256 availableLiquidity = IERC20Upgradeable(asset).balanceOf(reserveData.cTokenAddress);
