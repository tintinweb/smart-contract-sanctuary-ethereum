...
contract LazyMintParticipant is
  Initializable,
  OwnableUpgradeable,
  ERC1155Upgradeable,
  ...
  PausableUpgradeable
{
  ...
  function mintBatch(
    ...
    require(newMinted + _totalMinted <= _maxSupply, "supply has run out");
    _mintBatch(to, id, amounts, data);
  ...
  function redeemBatch(
    ...
    require(
      msg.sender != signer && msg.sender == voucher.buyer,
      "Signature is invalid for this voucher"
    );
    require(
      voucher.amount <= _woop.balanceOf(msg.sender),
      "Redeemer role: Insufficient funds to redeem"
    );
    _woop.transferFrom(msg.sender, voucher.owner, voucher.amount);
    mintBatch(to, ids, amounts, data);
  }

  function mint(address account, bytes memory data) private {
    uint256 newMinted = _totalMinted + 1;
    ...
    return
      _hashTypedDataV4(
        keccak256(
          abi.encode(
            keccak256(
              "Voucher(uint256 price,address owner,address buyer,uint256 amount)"
            ),
            voucher.price,
            voucher.owner,
            voucher.buyer,
  ...
  function _verify(Voucher calldata voucher) public view returns (address) {
    ...
    return ECDSAUpgradeable.recover(digest, bytes(voucher.signature));
  }
}
