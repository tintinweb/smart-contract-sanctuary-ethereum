...
// File: contracts/LazyMintNFT.sol
...
contract LazyNFT is ERC721URIStorage, EIP712, Ownable {
  ...
  event Minted(uint nftId,address nftCreator, uint time );
  ...
  function redeem(
    ...
    require(_minter == signer, "unauthorized signer"); //_minter will be fetched from backedn throug IPFS minter

    require(msg.value >= voucher.minPrice, "Insufficient funds to redeem");

    // _mintNft(signer, voucher.royaltyPercentage);
        uint256 tokenId = _tokenIdCounter.current();
        _tokenIdCounter.increment();

        mintedByUser[signer].push(tokenId);
        royalty[tokenId] = voucher.royaltyPercentage;
        minter[tokenId] = signer;

        _safeMint(signer, tokenId);
        _setTokenURI(tokenId, voucher.uri);
        
        emit Minted (tokenId,signer,block.timestamp);
...
function withdraw() public {
  ...
  function _hash(NFTVoucher calldata voucher) internal view returns (bytes32) {
    return _hashTypedDataV4(keccak256(abi.encode(
      keccak256("NFTVoucheruint256 tokenId,uint256 minPrice,string uri,uint256 royaltyPercentage)"),
      voucher.minPrice,
  ...
  function _verify(NFTVoucher calldata voucher) public view returns (address) {
    ...
    bytes32 digest = _hashTypedDataV4(keccak256(abi.encode(
            keccak256("NFTVoucher(uint256 minPrice,string uri,uint256 royaltyPercentage)"),
            voucher.minPrice,
            keccak256(bytes(voucher.uri)),
            voucher.royaltyPercentage
        )));
