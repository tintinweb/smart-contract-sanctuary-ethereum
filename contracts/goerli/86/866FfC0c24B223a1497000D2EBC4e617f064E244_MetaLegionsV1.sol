...
import "./interface/IMetaLegions.sol";

contract MetaLegionsV1 is ERC721Drop, MetaLegions {
    // Permits modifications only by the token owner.
    modifier onlyTokenOwner(uint256 tokenId) {
        require(
            msg.sender == ownerOf(tokenId),
            "Only the token owner can set a nickname for it."
        );
        _;
    }

    uint256 private constant _BASE = 36;
    uint256 private constant _VALUE = 36;

    // Mapping from tokenId to Metadata struct
    mapping(uint256 => data) public metaData;

    // Mapping of token ID to nickname string
    mapping(uint256 => string) public nicknames;

    function random(uint256 number, uint256 randNonce)
        private
        view
        returns (uint256)
    {
    ...
    // Generate five phases for token
    function _generateData(uint256 tokenId, uint256 quanity) private {
        uint256 nonce = uint256(blockhash(block.timestamp + block.difficulty));
        for (uint256 i = 0; i < quanity; i += 1) {
            uint256 _id = tokenId + i;

            // calc wood
            uint256 value = _BASE + random(_VALUE, nonce++);
            // save wood
            metaData[_id].wood = value;

            // calc fire
            value = _BASE + random(_VALUE, nonce += value);
            // save fire
            metaData[_id].fire = value;

            // calc earth
            value = _BASE + random(_VALUE, nonce += value);
            // save earth
            metaData[_id].earth = value;

            // calc metal
            value = _BASE + random(_VALUE, nonce += value);
            // save metal
            metaData[_id].metal = value;

            // calc water
            value = _BASE + random(_VALUE, nonce += value);
            // save water
            metaData[_id].water = value;
        }
    }

    // Set new nickname for a token
    function setNickname(uint256 _tokenId, string calldata _nickname)
        public
        onlyTokenOwner(_tokenId)
    {
        nicknames[_tokenId] = _nickname;
        emit onNickname(_tokenId, msg.sender, _nickname);
    }
    ...
    function _beforeClaim(
        ...
        require(msg.sender == _receiver, "Only wallet signer can claim.");
        require(balanceOf(_receiver) < 1, "Only one token per wallet.");
        super._beforeClaim(
            _receiver,
            _quantity,
            _currency,
    ...
    function _beforeTokenTransfers(
        ...
        require(balanceOf(to) < 1, "Only one token per wallet.");
        super._beforeTokenTransfers(from, to, startTokenId, quantity);
    }

    // run this function when an NFT gets minted
    function _transferTokensOnClaim(address _to, uint256 _quantityBeingClaimed)
        internal
        virtual
        override
        returns (uint256 startTokenId)
    {
        ...
        _generateData(startTokenId, _quantityBeingClaimed);
        _safeMint(_to, _quantityBeingClaimed);
    }

    function setTokenURI(uint256 _tokenId, string memory _baseURI)
        public
        onlyOwner
    {
        (uint256 batchId, ) = _getBatchId(_tokenId);
        ...
        if (isEncryptedBatch(batchId)) {
            revert("Not for delayed tokens.");
        } else {
            _setBaseURI(batchId, _baseURI);
            emit onURIChanged(_tokenId, msg.sender, _baseURI);
        }
    }
...
interface MetaLegions {
    ...
    // Logged when token owner change nickname for it.
    event onNickname(uint256 tokenId, address owner, string newName);

    // Logged when token owner change nickname for it.
    event onURIChanged(uint256 tokenId, address owner, string newUri);
}