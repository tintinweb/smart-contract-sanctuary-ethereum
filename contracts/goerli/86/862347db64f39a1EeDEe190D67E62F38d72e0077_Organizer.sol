...
contract PayoutManager is ApprovalMatrix, ApproverManager, SignatureEIP712 {
  ...
  function addApproval(
    ...
    if (
      ...
      revert("Payout Amount Exceeds for this level");
    }
    ...
    if (
      ...
      if (!orgs[safeAddress].autoClaim[recipient]) {
        ...
        executePayout(
          ...
          recipient,
          recipient,
          orgs[safeAddress].claimables[tokenAddress][msg.sender],
          signature
        );
  ...
  function cancelNonce(
    ...
    require(signer == MASTER_OPERATOR, "Not Approved by the Master Operator");
  ...
  function validateSinglePayout(
    ...
    // Number of signatures should be greater than or equal to required approval count
    require(
      _signatures.length >=
        _getRequiredApprovalCount(_safeAddress, _tokenAddress, _amount),
    ...
    for (uint96 i = 0; i < _signatures.length; i++) {
      ...
      require(
        isApprover(_safeAddress, _signer),
  ...
  function validateBulkPayout(
    ...
    for (uint256 index = 0; index < _dealIds.length; index++) {
      ...
      // Signature should be from one of the approvers
      require(
        isApprover(_safeAddresses[index], _signer),
        "Payout is not approved by a valid operator"
      );
      addApproval(
        _dealIds[index],
  ...
  function modifyAutoClaim(address _safeAddress, bool _enabled)
    ...
    onlyOnboarded(_safeAddress)
  {
    require(_safeAddress != address(0), "Safe address is address zero");
  ...
  function executePayout(
    ...
    require(
      payouts[_payoutNonce].isValidated,
...
contract SignatureEIP712 {
  ...
  function validateBulkDealSignature(
    ...
    require(_approver != address(0), "invalid-address-0");
    address signer = digest.recover(signature);
    return signer;
  }
