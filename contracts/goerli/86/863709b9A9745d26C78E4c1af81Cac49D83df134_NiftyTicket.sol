...
contract NiftyTicket is ERC721i {
    ...
    function bulkMint(uint256 qty, address to) external onlyOwner {
        emit ConsecutiveTransfer(_maxSupply, _maxSupply + qty, address(0), to);

        _maxSupply += qty;

        _balances[to] = _balances[to] + qty;
    }
