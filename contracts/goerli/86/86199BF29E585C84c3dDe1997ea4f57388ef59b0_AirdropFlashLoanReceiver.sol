...
import "../../interfaces/IUNFTRegistry.sol";
...
contract AirdropFlashLoanReceiver is IFlashLoanReceiver, ReentrancyGuard, Ownable, ERC721Holder, ERC1155Holder {
  address public immutable unftRegistry;
  mapping(bytes32 => bool) public airdropClaimRecords;
  uint256 public deployType;

  constructor(
    address owner_,
    address unftRegistry_,
    uint256 deployType_
  ) {
    require(owner_ != address(0), "zero owner address");
    require(unftRegistry_ != address(0), "zero registry address");

    unftRegistry = unftRegistry_;
  ...
  function executeOperation(
    ...
    (address unftProxy, ) = IUNFTRegistry(unftRegistry).getUNFTAddresses(nftAsset);
    require(unftProxy == msg.sender, "caller not unft");
    ...
    // allow operator transfer borrowed nfts back to unft
