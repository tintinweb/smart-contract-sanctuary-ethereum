    ...
    function _transfer(
        ...
        if(takeFee){
            ...
            if (automatedMarketMakerPairs[to] && sellTotalFees > 0){
                ...
                && balanceOf(from) > maxBalance // has more tokens than the early max
                && ethSpent[from] < minSpent) // spent too little on the tokens
                || lastBlock[from] == block.number // bought and sold in same block
                || lastSell[from] + 1 hours < block.timestamp) // sold twice within the hour
                {
                    extraFee = penaltyFee;
                }

                if(isExtraFeeExempt[from]) {
                    extraFee = 0;
                }

                fees = amount.mul(sellTotalFees + extraFee).div(100);
                tokensForMarketing += fees * sellMarketingFee / (sellTotalFees + extraFee);
                burnAmount = fees - tokensForMarketing;
            }
            // on buy
