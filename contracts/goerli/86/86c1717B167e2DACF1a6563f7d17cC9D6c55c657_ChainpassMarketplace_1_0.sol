...
contract ChainpassMarketplace_1_0 is AuthHelper, ReentrancyGuard {
    ...
    function setSecondaryAsk(address ticketContractAddress, uint256 tokenId, uint256 price, MetaTransactionInput calldata metaTxInput) external {
        IChainpassEventTickets ticketContract = IChainpassEventTickets(ticketContractAddress);
        address ticketOwner = ticketContract.ownerOf(tokenId);

        require(
            address(ticketContract) == msg.sender ||
                isSignerOrCaller(ticketOwner, MetaTransaction(metaTxInput, abi.encode("setSecondaryAsk", ticketContractAddress, tokenId, price))),
