...
import { LpStakingManager } from "./LpStakingManager.sol";
import { StakingDeployment, StakingConfig } from "../../struct/LpStakingStructs.sol";

/**
 * @title LpStakingFactory
 * @notice This is a helper contract that uses the 
 * [EIP-1167](https://eips.ethereum.org/EIPS/eip-1167) 
 * minimal proxy standard to deploy LpStaking setups for 
 * organizations that want to create Lp Staking yield farms.
 */
contract LpStakingFactory {
    using Clones for address;

    address public immutable _lpStakingTemplate; //making these public as part of the update...
    address public immutable _lpStakingTokenTemplate;
    address public immutable _lpStakingManagerTemplate;
    address public immutable _lpNft;

    event LpStakingFactoryFullDeployment(
        address lpStaking,
        address lpStakingToken,
        address lpStakingManager
    );
    event LpStakingFactoryDeployment(address lpStaking);

    constructor(address lpNft_) {
        _lpNft = lpNft_;
        _lpStakingTemplate = address(new LpStaking());
        _lpStakingTokenTemplate = address(new LpStakingToken());
        _lpStakingManagerTemplate = address(new LpStakingManager());
    }

    /**
     * @notice creates an LpStaking contract, LpStakingManager, and LpStaking token
     * on behalf of a user. This helper allows you to create a full deployment of an
     * LpStaking setup in one easy transaction.
     * 
     * The owner is set for the LpStakingManager, and through the manager, you can set
     * or administer the LpStakingToken and LpStaking contracts.
     * 
     * @param owner_ owner address for the LpStakingManager and through it, the other contracts.
     * @param deployment_ setup config includes new token info, and which Lp Positions are allowed.
     * @return lpStaking the address of the new LpStaking contract
     * @return lpStakingToken the address of the new LpStakingToken contract
     * @return lpStakingManager the address of the new LpStakingManager contract
     */
    function createLpStakingDeployment(
        address owner_,
        StakingDeployment calldata deployment_ 
    ) external returns(
        LpStaking lpStaking,
        LpStakingToken lpStakingToken,
        LpStakingManager lpStakingManager
    ) {
        lpStakingManager = LpStakingManager(_lpStakingManagerTemplate.clone());
        lpStakingToken = LpStakingToken(_lpStakingTokenTemplate.clone());
        lpStaking = LpStaking(_lpStakingTemplate.clone());

        lpStakingManager.initLpStakingManager(owner_, lpStakingToken, lpStaking);
        lpStakingToken.initLpStakingToken(
            address(lpStakingManager),
            deployment_.stakingTokenName,
            deployment_.stakingTokenSymbol
        );
        lpStaking.initLpStaking(
            address(lpStakingManager),
            deployment_.stakingConfig,
            address(lpStakingManager),
            _lpNft,
            deployment_.initialRewardPerBlock
        );

        emit LpStakingFactoryFullDeployment(address(lpStaking), address(lpStakingToken), address(lpStakingManager));
    }

    /**
     * @notice Advanced functionality for creators that wish to BYOT (Bring Your Own Token)
     * this functionality is not recommended for novice users. You will need to ensure that
     * you have a contract that implements the ILpStakingDistributor interface and has appropriate
     * authorization checks on its distribute function for the "rewardDistributor" parameter.
     * 
     * Reason to use this over just deploying the contract directly: cheaper gas using EIP-1167
     * 
     * @param owner_ owner address able to call admin functions on the LpStaking contract
     * @param config_ staking config struct with parameters for which LP positions to allow Staking
     * @param rewardDistributor_ address of the contract that will distribute new tokens for the LpStaking contract
     * @param initialRewardPerBlock_ initial amount of tokens to distribute each block for the LpStaking contract
     */
    function createLpStaking(
        address owner_,
        StakingConfig memory config_, 
        address rewardDistributor_,
        uint256 initialRewardPerBlock_
    ) external returns(LpStaking lpStaking) {
        lpStaking = LpStaking(_lpStakingTemplate.clone());
        lpStaking.initLpStaking(
            owner_,
            config_,
            rewardDistributor_,
            _lpNft,
            initialRewardPerBlock_
        );
        emit LpStakingFactoryDeployment(address(lpStaking));
    }
