...
contract CitizensOfOverworld is ERC721A, Ownable {
    ...
    */
    ...
    // tracks how many Citizens a wallet has minted (to determine if its a wallet's first mint, i.e. free)
    mapping(address => uint256) public mintCount;

    // Used for converting bytes to hex strings.
    ...
    string private constant _SVG_PRE_STYLE_ATTRIBUTE =
        '<svg xmlns="http://www.w3.org/2000/svg" id="citizen" viewBox="-3 -1 38 38" width="100%" height="100%" style="';
    ...
    */
    ...
    constructor() ERC721A("Citizen of Overworld Test", "OVRWRLD") {}
    ...
    */
    ...
    function mint(uint256 quantity) external payable {
        require(isMintStarted, "Mint not started");
        uint256 totalminted = _totalMinted();
        uint256 newSupply = totalminted + quantity;
        require(
            newSupply <= (MAX_SUPPLY - 30),
            "SoldOut"
        );
        require(
        ...
        require(balanceOf(msg.sender) < 4, "MaxPerWallet");
        require(
            balanceOf(msg.sender) < 5 - quantity,
            "MaxSupplyReached"
        );
        uint256 totalFee;
        if (mintCount[msg.sender] == 0 && quantity > 1) {
            totalFee = (quantity - 1) * PRICE_AFTER_FIRST_MINT;
        } else if (mintCount[msg.sender] == 0 && quantity == 1) {
            totalFee = 0;
        } 
        else {
    ...
    */
     ...
     */
    function generateDNA(uint256 tokenId) private {
        ...
        uint256 randomEnough = uint256(
            keccak256(abi.encodePacked(blockhash(block.number - 1), tokenId, msg.sender))
        );
        ...
        extractedRandomNum = randomEnough & mask;
        ...
        // get accessory
        uint256[7] memory accessoryRarity = [
            uint256(10),
            4,
            20,
            3,
            12,
            150,
            1 // ~75% chance no accessory, 0.5% chance rainbow
        ];
    ...
    */
        ...
        unchecked {
            ...
            if (tokenDna.Legendary > 0) {
                metadataString = string(
                    abi.encodePacked(
                        '{"trait_type":"',
                        bytes32ToString(compressedTraitMetadata[8][0]),
                        '","value":"',
                        bytes32ToString(
                ...
                for (uint256 i; i < 9; ++i) {
                    ...
                    string memory traitName = bytes32ToString(
                        compressedTraitMetadata[i][0]
                    );
                    string memory traitValue = bytes32ToString(
    ...
    function getChunk(
        ...
        for (uint256 i = 0; i < cursor; i += 4) {
            bool isRectBlackColor = (keccak256(abi.encodePacked((buffer[i + 3]))) ==
    ...
    */
     ...
     * This function will be locked upon validation that data was loaded properly.
     */
    ...
    /**
     * The owner (0xMongoon) is allowed to mint up to 25 custom Citizens.
     * (in return for a free mint and no royalties/creator fees)
     */
    function ownerMint(uint256[] calldata traits, uint256 quantity)
        external
        onlyOwner
    {
        unchecked {
     ...
     * No going back now!
     */
    function beginMint() external onlyOwner {
        isMintStarted = true;
    }
    ...
    */
    ...
    /**
     * Converts a uint256 into a string.
     */
