...
contract GlobalVerisartManifoldExtension is ICreatorExtensionRoyalties  {
    ...
    mapping(address => mapping(uint256 => address payable[])) private _royaltyAddresses;
    mapping(address => mapping(uint256 => uint256[])) private _royaltyBps;
    ...
    function grantMinting(address creatorContract, address creator) public {
        ...
        require(creatorCon.isAdmin(msg.sender), "Must be admin");
    ...
    function mint(address creatorContract, address to, string calldata uri, address payable[] memory royaltyAddresses, uint256[] memory royaltyBps) public {
        ERC721Creator creatorCon = ERC721Creator(creatorContract);
        require(hasMintingPermission(creatorContract, msg.sender) || creatorCon.isAdmin(msg.sender),"Not allowed to mint");
        uint256 tokenId = creatorCon.mintExtension(to, uri);

        _setRoyalties(creatorContract, tokenId, royaltyAddresses, royaltyBps);
    }
    ...
    function mintBatch(address creatorContract, address to, string[] calldata uris, address payable[][] memory royaltyAddressesPerToken, uint256[][] memory royaltyBpsPerToken) public {
        require(royaltyAddressesPerToken.length == royaltyBpsPerToken.length);
        ERC721Creator creatorCon = ERC721Creator(creatorContract);
        require(hasMintingPermission(creatorContract, msg.sender) || creatorCon.isAdmin(msg.sender), "Not allowed to mint");
        ...
        if (royaltyAddressesPerToken.length != 0) {
            for (uint256 i = 0; i < tokenIds.length; i++) {
                _setRoyalties(creatorContract, tokenIds[i], royaltyAddressesPerToken[i], royaltyBpsPerToken[i]);
            }
        }
    }

    function _setRoyalties(address creatorContract, uint256 tokenId, address payable[] memory royaltyAddresses, uint256[] memory royaltyBps) private {
        require(royaltyAddresses.length == royaltyBps.length, "Royalty arrays not same length");
        if (royaltyAddresses.length != 0) {
            _royaltyAddresses[creatorContract][tokenId] = royaltyAddresses;
            _royaltyBps[creatorContract][tokenId] = royaltyBps;
        }
    }
    ...
    function getRoyalties(address creator, uint256 tokenId) external virtual view override returns (address payable[] memory, uint256[] memory) {
        return (_royaltyAddresses[creator][tokenId], _royaltyBps[creator][tokenId]);
    }
}
