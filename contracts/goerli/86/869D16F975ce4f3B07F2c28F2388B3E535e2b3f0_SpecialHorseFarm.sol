...
contract SpecialHorseFarm is ERC721Holder, OwnerOperatorUpgradeSafe, PauseUpgradeSafe {
    ...
    function withdrawToken(address token) external virtual onlyOwner {
        ...
        require(amount > 0, "HorseFarm: zero out amount");
    ...
    function withdrawNFT(address token, uint256 tokenId) external virtual onlyOwner {
        require(token != horseNFT || ownerOf[tokenId] == address(0), "HorseFarm: horse belong to contract");
    ...
    function lease(LeaseData memory data) external virtual whenNotPaused {
        ...
        for (uint256 i = 0; i < data.horseId.length; i++) {
            require(msg.sender == IERC721(horseNFT).ownerOf(data.horseId[i]), "HorseFarm: sender is not owner");
    ...
    function _verifyLease(LeaseData memory data) internal view returns (bytes32, address) {
        ...
        require(operators[signer], "HorseFarm: lease not verify");
        require(block.number < data.blockExpired, "HorseFarm: lease expired");
        require(!leaseCompleted[message], "HorseFarm: lease completed");
        require(leaseData[data.nonce].owner == address(0), "HorseFarm: nonce existed");
    ...
    function HourseRefund(RefundData memory data) private whenNotPaused onlyOwner {
        require(block.number < data.blockExpired, "HorseFarm: Refund expired");
        require(refundData[data.nonce].blockExpired == 0, "HorseFarm: nonce existed");
        for (uint256 i = 0; i < data.horseId.length; i++) {
            require(ownerOf[data.horseId[i]] != address(0), "HorseFarm: horse Refunded");
            require(!refundCompleted[data.horseId[i]], "HorseFarm: Refund completed");
    ...
    function withdraw(WithdrawData memory data) external virtual whenNotPaused {
        require(msg.sender == data.owner, "HorseFarm: sender is not owner");

        (bytes32 message, address signer) = _verifyWithdraw(data);
        for (uint256 i = 0; i < data.horseId.length; i++) {
            require(msg.sender == ownerOf[data.horseId[i]], "HorseFarm: sender is not horse owner");
            require(ownerOf[data.horseId[i]] != address(0), "HorseFarm: horse Refunded or withdrawed");
            require(!refundCompleted[data.horseId[i]], "HorseFarm: Refund completed");
    ...
    function _verifyWithdraw(WithdrawData memory data) internal view returns (bytes32, address) {
        ...
        address signer = message.toString().recover(data.v, data.r, data.s);
        require(operators[signer], "HorseFarm: withdraw not verify");
        require(block.number < data.blockExpired, "HorseFarm: withdraw expired");
        require(!withdrawCompleted[message], "HorseFarm: withdraw completed");
        require(withdrawData[data.nonce].blockExpired == 0, "HorseFarm: nonce existed");
        return (message, signer);
    }
}