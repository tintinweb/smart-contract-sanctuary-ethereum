...
contract Doges is ERC721Enumerable, Ownable {
    event SeedUpdated(uint256 indexed tokenId, uint256 seed);

    mapping(uint256 => uint256) internal seeds;
    IDogeDescriptor public descriptor;
    uint256 public maxSupply = 10000;
    bool public minting = false;
    bool public canUpdateSeed = true;

    constructor(IDogeDescriptor newDescriptor) ERC721("Doge", "DOGE") {
    ...
    function setDescriptor(IDogeDescriptor newDescriptor) external onlyOwner {
        descriptor = newDescriptor;
    }
    ...
    function getSeed(uint256 tokenId) public view returns (uint256) {
        require(_exists(tokenId), "Doge does not exist.");
