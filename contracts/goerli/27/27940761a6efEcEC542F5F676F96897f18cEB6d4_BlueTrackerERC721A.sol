...
contract BlueTrackerERC721A is Ownable, ERC721A, PaymentSplitter {
    ...
    **/
    function publicSaleMint(uint _quantity) external payable callerIsUser {
        ...
        require(currentPublic + _quantity <= maxPublic, "Max supply exceeded");
    ...
    **/
    function gift(address[] memory _to, uint[] memory _quantity) external onlyOwner {
        ...
        {
            require(currentGift + _quantity[i] <= maxGift, "Max supply for gift exceed");
            _safeMint(_to[i], _quantity[i]);
            currentGift += _quantity[i];
        }
    }
