...
contract GaspackAppImplementation is
    ...
    Ownable
{
    ...
    bytes32 public constant PRIVATE_SALE_TYPEHASH =
        keccak256(
            "PrivateSale(uint256 price,uint256 quantity,uint256 txLimit,uint256 walletLimit,uint256 deadline,address recipient)"
        );
    string public baseURI;
    Stage public stage;
    address public signer;
    address private WALLET_A;
    address private WALLET_B;
    address private minter;
    PublicSale private publicSale;

    mapping(address => uint256) public PrivateSaleMinter;
    mapping(address => uint256) public PublicSaleMinter;
    mapping(address => uint256) public userNonce;
    mapping(uint256 => PrivateSale) private privateSales;
    ...
    function mintTo(address[] calldata _to, uint256[] calldata _amount)
        external
        onlyOwnerOrMinter
    {
    ...
    function privateSaleMint(
        ...
        require(
            signer == verifyPrivateSale(_privateSale, msg.sender, _signature),
            "INVALID_SIGNATURE"
        );
        ...
        require(
            block.timestamp <= privateSale.deadline,
        ...
        require(
            totalSupply() + _privateSale.quantity <= MAX_SUPPLY,
            "SUPPLY_EXCEDEED"
        );
        require(
    ...
    function publicSaleMint(uint256 _quantity) external payable {
        ...
        require(totalSupply() + _quantity <= MAX_SUPPLY, "SUPPLY_EXCEDEED");
        require(
        ...
        bytes32 digest = _hashTypedDataV4(
            keccak256(
                abi.encode(
                    ...
                    keccak256(abi.encode(_privateSale, _sender))
                )
            )
        );
    ...
    function withdrawAll() external onlyOwner {
        ...
        uint256 balance = (address(this).balance * 50) / 100;

        sendValue(payable(WALLET_A), balance);
        sendValue(payable(WALLET_B), balance);
    }
