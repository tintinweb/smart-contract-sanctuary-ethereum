...
interface iConatus {
  ...
  function mintForVendingMachine(address to) external;
...
contract Conatus is iConatus, ERC721, ReentrancyGuard, Ownable {
  uint256 private constant TYPE_NUM = 4;
  //  uint256[] private _prices = [1 ether, 0.5 ether, 0.5 ether, 0.5 ether];
  ...
  uint256[] private _totalAmounts = [3, 3, 100, 3];
  uint256[] private _mintedAmounts = [0, 0, 0, 0];
  ...
  function mint(uint256 _type) external payable override nonReentrant {
    require(isOnSale, "Conatus: Not on sale");
    require(_type >= 1 && _type <= TYPE_NUM, "Conatus: Invalid value");

    uint256 index = _type - 1;

    require(msg.value == _prices[index], "Conatus: Invalid value");
  ...
  function mintForVendingMachine(address to) external override onlyOwner {
    uint256 _type = 2;
    uint256 totalAmount = _totalAmounts[_type];
    require(_mintedAmounts[_type] < totalAmount, "Conatus: Already minted all");
    for (uint256 i = 1; i <= totalAmount; i++) {
      uint256 tokenId = toTokenId(_type, i);
      mintAndTransfer(to, tokenId);
    }
    _mintedAmounts[_type] = totalAmount;
  }
