...
contract SubscriptionManager {
    ...
    function pauseSubscription(uint256 subscriptionId) onlyCreator(subscriptionId) public {
        subscriptions[subscriptionCounter].active = false;
        emit SubscriptionPaused(subscriptionId);
    }

    function resumeSubsription(uint256 subscriptionId) onlyCreator(subscriptionId) public {
        subscriptions[subscriptionCounter].active = true;
        emit SubscriptionActivated(subscriptionId);
    }

    function payoutSubscription(uint256 subscriptionId) public {
        require(subscriptions[subscriptionId].active, 'subscription inactive');
