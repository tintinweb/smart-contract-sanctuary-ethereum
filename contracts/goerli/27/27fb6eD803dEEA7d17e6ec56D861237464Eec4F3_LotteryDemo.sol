...
// File: @chainlink/contracts/src/v0.8/interfaces/AutomationRegistryInterface1_2.sol
...
contract LotteryDemo is ERC1155, ERC1155Burnable {
    ...
    bytes32 private root; // for WL MerkelProof
    ...
    mapping(uint256 => bool) tokenEligible; // saves list of eligible tokens on round start
    ...
    uint256 interval;
    uint256[2] triggerIDs;
    mapping(uint256 => uint256) public triggerToUpkeepID;
    ...
    constructor(
        ...
        ) payable ERC1155("ipfs://QmXGf54iDZTDWSFJS1G3RFNnRf48dJ2uMHBNvDNrXU27oT/{id}.json") {

        i_link = _link; // 0x326C977E6efc84E512bB9C30f76E30c160eD06FB
        registrar = _registrar; // 0x9806cf6fBc89aBF286e8140C42174B94836e36F2
        i_registry = _registry; // 0x02777053d6764996e594c3E88AF1D58D5363a2e6
    ...
    function setRewards() internal {
        require(lotteryRound < 1, "Lottery already started, no allowed to change rewards");
        require(minted > 0, "No income from mint, impossible to calculate rewards");

        uint256 salesIncome = address(this).balance;
        rewards[0] = (salesIncome/100*20);
        rewards[1] = (salesIncome/100*25);
        rewards[2] = (salesIncome/100*30);
        chunk = salesIncome/100*10/minted;
    }
    ...
    function setLotteryRound(uint256 _round, uint256[] memory tokens) external {
       ...
       for (uint256 i = 1; i <= mintSupply; i++) if (tokenEligible[i] == true) tokenEligible[i] = false;
       for (uint256 i = 1; i <= tokens.length; i++) tokenEligible[i] = true;
    ...
    function wlMint(address caller, bytes32[] memory proof) external payable {
        ...
        require(isValid(proof, keccak256(abi.encodePacked(caller))), "You're not allowed to mint");
        require(minted + 1 <= mintSupply, "Exceeds total supply");
        require(walletMints[caller] + 1 <= maxPerWallet[0], "Exceeds max tokens per wallet");
        require(msg.value == mintPrices[0], "Wrong price");

        walletMints[caller]++;
        minted++;
        uint256 tokenId = minted;
        _mint(caller, tokenId, 1, "0x00");

        emit Minted(tokenId);
        if (walletMints[caller] == maxPerWallet[0]) emit MintedMax(caller);
    }

    function publicMint(address caller, uint256 quantity_) public payable {
        require(stage > 0, "Mint isn't enabled yet");
        require(stage < 2, "Mint has already ended");
        require(isPublic == true, "Public Mint isn't enabled yet");
        require(minted + quantity_ <= mintSupply, "Exceeds total supply");
        require(walletMints[caller] + quantity_ <= maxPerWallet[1], "Exceeds max tokens per wallet");
        ...
        for (uint256 i = 0; i < quantity_; i++) {
            ...
            emit Minted(tokenId);
            if (walletMints[caller] == maxPerWallet[1]) emit MintedMax(caller);
        }
    }

    // lottery
    function playLottery(address caller, uint256 tokenId_, uint256 tokenRarity_) public payable {
        ...
        require(balanceOf(caller, tokenId_) > 0, "You don't own this token");
        require(tokenRarity_ == lotteryRound, "This lottery stage for different rarity tokens");

        if (tokenId_ == winnerTokenId) {
            _burn(caller, tokenId_, 1);
            burned++;
            _safeTransferFrom(ownerWallet, caller, BURNED, 1, "0x00");
            roundWon[lotteryRound - 1] = tokenId_;
            payable(caller).transfer(rewards[lotteryRound - 1]);
    ...
    function createNewTrigger() public returns (uint256) {
        uint256 triggerID;
        if (triggerIDs[triggerIDs.length - 1] >= 0) {
            triggerID = triggerIDs[triggerIDs.length - 1];
        } else {
            triggerID = 0;
        }
    ...
    function giveData(uint256 _data) external pure returns (bytes memory) {
    ...
    function registerAndPredictID(
        ...
        uint256 triggerID = createNewTrigger();
        (State memory state, Config memory _c, address[] memory _k) = i_registry.getState();
        uint256 oldNonce = state.nonce;
        
        bytes memory checkData = abi.encodePacked(triggerID);
        ...
        triggerToUpkeepID[triggerID] = upkeepID;
        } else {
    ...
    external view returns(bool upkeepNeeded, bytes memory performData) {
        ...
        if (triggerID == 0) {
            ...
            require(stage == 2, "Lottery hasn't started yet");
            require(roundWon[2] == 0, "Lottery has already ended");
            require(lotteryRound > 0, "First Round hasn't started yet");
            require(roundWon[lotteryRound - 1] == 0, "This round already won");
    ...
    function performUpkeep(bytes calldata performData) external payable{
        ...
        if (triggerID == 0) {
            if ((block.timestamp - triggerToLastTimeStamp[triggerID]) > interval) {
                ...
                uint256 amount = getRandomNumber() % 3;
                
                uint256 botIndex = getRandomNumber() % botAccounts.length;
                address botAccount = botAccounts[botIndex];

                publicMint(botAccount, amount);
            }
        } else if (triggerID == 1) {
            if ((block.timestamp - triggerToLastTimeStamp[triggerID]) > interval) {
                triggerToLastTimeStamp[triggerID] = block.timestamp;
                ...
                for (uint256 i = 1; i <= mintSupply; i++) {
                        uint256 botBalance = balanceOf(botAccount, i);
                        if (botBalance > 0 && tokenEligible[i]) {
                            tokenIDs[idsCounter] = i;
                            idsCounter++;
                        } else {
                            revert("no eligible tokens");
                        }
                }

                uint256 tokenIndex = getRandomNumber() % tokenIDs.length;

                playLottery(botAccount, tokenIDs[tokenIndex], lotteryRound);
            }
        } else {
            revert("wrong trigger");
        }
