...
contract TestFarmContract is Ownable {
   ...
   struct DepositInfo {
    ...
    IERC20Contract public rewardToken;
    uint256 public rewardTokenAccuracy;
    uint256 public lpTokenAccuracy = 10 ** 18;
    ...
    function claim(uint256 _pid) public validatePoolByPid(_pid) {
        ...
        require(IERC20Contract(rewardToken).balanceOf(address(this)) > 0,"claim: The reward token balance of this contract is zero"); 
        ...
        uint256 actualReward = deservedReward.mul(1e12).div(lpTokenAccuracy).mul(rewardTokenAccuracy).div(1e12);
    ...
    function withdraw(uint256 _pid) public validatePoolByPid(_pid) {
        ...
        updatePool(_pid);
        if(pendingReward(_pid, msg.sender) > 0 && IERC20Contract(rewardToken).balanceOf(address(this)) > 0){
            uint256 deservedReward = myPendingReward(_pid, msg.sender);
            user.mintAmount = user.mintAmount.add(deservedReward);
            uint256 actualReward = deservedReward.mul(1e12).div(lpTokenAccuracy).mul(rewardTokenAccuracy).div(1e12);
            safeTokenTransfer(msg.sender, actualReward);
    ...
    function safeTokenTransfer(address _to, uint256 _amount) internal {
        uint256 rewardTokenBalance = IERC20Contract(rewardToken).balanceOf(address(this));
        if (_amount > rewardTokenBalance) {
            IERC20Contract(rewardToken).transfer(_to, rewardTokenBalance);
        } else {
            IERC20Contract(rewardToken).transfer( _to, _amount);
        }
    }
    ...
    function setRewardTokenContract(address _token) public onlyOwner {
        rewardToken = IERC20Contract(_token);
    }
