...
contract ChubbyKaijuDAOMarketplace is Ownable {
    ...
    function addMarketplaceItem(uint _id, uint _common_price, uint _holders_price, uint _max_supply) public onlyOwner {
    ...
    function setActive(uint _id, bool _active) public onlyOwner {
    ...
    function getPrice(uint _id, address _to) public view returns(uint256){
        require(getMaxSupply(_id) != 0, "Token does not exist");
        uint256 _price;
        if (crunchContract.balanceOf(_to) != 0){
    ...
    function whitelist(address _to, uint _id) public {
        require(getTotalSupply(_id) <= getMaxSupply(_id), "Total supply limit reached or token does not exist");
        require(gen1Contract.balanceOf(_to) > 0 || stakingContract.GEN1depositsOf(_to).length > 0, "You need atleast one GEN1 token");
        require(isActive(_id), "Sale has not started");
        require(hasWhitelist(_to, _id) == false, "Address already whitelisted");
        require(crunchContract.balanceOf(msg.sender) >= getPrice(_id, _to), "Not enough balance");
        require(crunchContract.allowance(msg.sender, address(this)) >= getPrice(_id, _to), "Low allowance");
        crunchContract.burn(msg.sender, getPrice(_id, _to));
        whitelists[_to].add(_id);
        items[_id].total_supply += 1;
    }

    function giftWhitelist(address _to, uint _id) public onlyOwner {
        require(getTotalSupply(_id) <= getMaxSupply(_id), "Total supply limit reached or token does not exist");
        whitelists[_to].add(_id);
        items[_id].total_supply += 1;
    }
