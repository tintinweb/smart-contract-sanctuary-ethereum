...
contract DebtCache is BaseDebtCache {
    ...
    function takeDebtSnapshot() external requireSystemActiveIfNotOwner {
        ...
        (uint256[] memory values, uint256 futuresDebt, uint256 excludedDebt, bool isInvalid) = _currentSynthDebts(currencyKeys);
    ...
    function updateCachedSynthDebts(bytes32[] calldata currencyKeys) external requireSystemActiveIfNotOwner {
        (uint256[] memory rates, bool anyRateInvalid) = exchangeRates().ratesAndInvalidForCurrencies(currencyKeys);
        _updateCachedSynthDebtsWithRates(currencyKeys, rates, anyRateInvalid);
    }

    function updateCachedSynthDebtWithRate(bytes32 currencyKey, uint256 currencyRate) external onlyIssuer {
    ...
    function updateCachedSynthDebtsWithRates(bytes32[] calldata currencyKeys, uint256[] calldata currencyRates)
        external
    ...
    function _updateCachedSynthDebtsWithRates(
        ...
        uint256 cachedSum;
        uint256 currentSum;
        uint256[] memory currentValues = _issuedSynthValues(currencyKeys, currentRates);
...
contract BaseDebtCache is Owned, MixinSystemSettings, IDebtCache {
    ...
    function _issuedSynthValues(bytes32[] memory currencyKeys, uint256[] memory rates)
        internal
        view
        ...
        )
    {
        ...
        (uint256 excludedDebt, bool isAnyNonSnxDebtRateInvalid) = _totalNonSnxBackedDebt(currencyKeys, rates, isInvalid);
        (uint256 futuresDebt, bool futuresDebtIsInvalid) = futuresMarketManager().totalDebt();
    ...
    function _cachedSynthDebts(bytes32[] memory currencyKeys) internal view returns (uint256[] memory) {
        uint256 numKeys = currencyKeys.length;
    ...
    function cachedSynthDebts(bytes32[] calldata currencyKeys) external view returns (uint256[] memory snxIssuedDebts) {
        return _cachedSynthDebts(currencyKeys);
    }

    function _excludedIssuedDebts(bytes32[] memory currencyKeys) internal view returns (uint256[] memory) {
        uint256 numKeys = currencyKeys.length;
        uint256[] memory debts = new uint256[](numKeys);
        for (uint256 i = 0; i < numKeys; i++) {
    ...
    function totalNonSnxBackedDebt() external view returns (uint256 excludedDebt, bool isInvalid) {
        bytes32[] memory currencyKeys = issuer().availableCurrencyKeys();
        (uint256[] memory rates, bool ratesAreInvalid) = exchangeRates().ratesAndInvalidForCurrencies(currencyKeys);
    ...
    function _totalNonSnxBackedDebt(
        ...
        (uint256 longValue, bool anyTotalLongRateIsInvalid) = collateralManager().totalLong();
        (uint256 shortValue, bool anyTotalShortRateIsInvalid) = collateralManager().totalShort();
    ...
    function _currentDebt() internal view returns (uint256 debt, bool anyRateIsInvalid) {
        bytes32[] memory currencyKeys = issuer().availableCurrencyKeys();
        (uint256[] memory rates, bool isInvalid) = exchangeRates().ratesAndInvalidForCurrencies(currencyKeys);
