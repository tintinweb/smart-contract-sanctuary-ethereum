  ...
  UUPSUpgradeable
{
  ...
  function depositNativeToken(
    ...
    emit NativeTokenDeposited(msg.value, depositor);
  }
  ...
  /**
   * @dev See {IUzuVault-transferNativeToken}
   */
  function transferNativeToken(
    address recipient,
    uint256 amountInWei
  ) external virtual override onlyRole(SPENDER_ROLE) {
    require(
      recipient != address(0),
      "UzuVault:transferNativeToken invalid recipient"
    );

    require(
      amountInWei > 0,
      "UzuVault:transferNativeToken invalid amountInWei"
    );

    require(
      amountInWei <= address(this).balance,
      "UzuVault:transferNativeToken The NativeToken balance is not enough"
    );

    payable(recipient).transfer(amountInWei);

    emit NativeTokenTransferred(recipient, amountInWei, address(this).balance);
  }

  /**
   * @dev See {IUzuVault-transferERC20}
   */
  function transferERC20(
    IERC20 erc20Token,
    address recipient,
    ...
    require(
      address(erc20Token).code.length > 0,
      "UzuVault:transferERC20 invalid erc20Token"
    );

    require(
      recipient != address(0),
      "UzuVault:transferERC20 invalid recipient"
    );

    require(
      amount > 0,
      "UzuVault:transferERC20 amount should be greater then zero"
    );

    erc20Token.transfer(recipient, amount);

    uint256 remainingBalance = erc20Token.balanceOf(address(this));
    emit ERC20TokenTransferred(erc20Token, recipient, amount, remainingBalance);
  }
  ...
  function withdrawNativeToken(
    uint256 amount
  ) external virtual override onlyRole(WITHDRAWER_ROLE) {
    require(
      amount > 0,
      "UzuVault:withdrawNativeToken withdraw amount should be greater than zero"
    );

    require(
      address(this).balance >= amount,
      "UzuVault:withdrawNativeToken The NativeToken balance is not enough"
    );

    address msgSender = _msgSender();
    payable(msgSender).transfer(amount);

    emit NativeTokenWithdrawn(amount, msgSender);
  }

  /**
   * @dev See {IUzuVault-withdrawAllNativeToken}
   */
  function withdrawAllNativeToken()
    external
    virtual
    override
    onlyRole(WITHDRAWER_ROLE)
  {
    ...
    emit NativeTokenWithdrawn(balanceInWei, msgSender);
  }

  /**
   * @dev See {IUzuVault-withdrawERC20}
   */
  function withdrawERC20(
    IERC20 erc20Token,
    uint256 amount
  ) public virtual override onlyRole(WITHDRAWER_ROLE) {
    require(
      amount > 0,
      "UzuVault:withdrawERC20 withdraw amount should be greater than zero"
    );

    require(
      address(erc20Token).code.length > 0,
      "UzuVault:withdrawERC20 invalid erc20Token"
    );
  ...
  /**
   * @dev See {IUzuVault-withdrawAllERC20}
   */
  function withdrawAllERC20(
    ...
    require(
      address(erc20Token).code.length > 0,
      "UzuVault:withdrawAllERC20 invalid erc20Token"
    );

    withdrawERC20(erc20Token, erc20Token.balanceOf(address(this)));
  }
}