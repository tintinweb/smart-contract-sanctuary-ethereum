 ...
 * @title PowerToken
 */
contract PowerToken is ERC20, Ownable, Authorizable {
    ...
    // Ensurance that transfer is only called once a day
    mapping(address => uint256) public transferAllTracker;
    uint256 public transferAllInterval = 259200;

    // Prevent trading before launch.
    ...
    /**
     * @dev Updates the transferAll interval.
     */
    function transferAllIntervalUpdate(uint256 _transferAllInterval) public onlyAuthorized {
        transferAllInterval = _transferAllInterval;
    }

    // Update the lockFromTime
    function lockFromUpdate(uint256 _lockFromTime) public onlyAuthorized {
        lockFromTime = _lockFromTime;
    }

    // Update the lockToTime
    function lockToUpdate(uint256 _lockToTime) public onlyAuthorized {
        lockToTime = _lockToTime;
    }
    ...
    function _beforeTokenTransfer(
        ...
        if (_excludedFromAntiWhale[to] == false) {
            // Prevent people from trading before launch.
            require(block.timestamp >= globalStartTime, "Not launched yet");
        }
    }
    ...
    function lock(address _holder, uint256 _amount) public onlyAuthorized {
        ...
        if (lastUnlockTime[_holder] < lockFromTime) {
            lastUnlockTime[_holder] = lockFromTime;
        }
    ...
    function canUnlockAmount(address _holder) public view returns (uint256) {
        if (block.timestamp < lockFromTime) {
            ...
            uint256 releasedDuration = block.timestamp.sub(lastUnlockTime[_holder]);
            uint256 remainingLockedDuration = lockToTime.sub(lastUnlockTime[_holder]);
            return (_locks[_holder].mul(releasedDuration)).div(remainingLockedDuration);
        }
    }
    ...
    function transferAll(address _to) public {
        // Track transfers to restrict multiwalleting locked jewel
        require(transferAllTracker[msg.sender] < block.timestamp - transferAllInterval, "transfer all cool down");
        require(transferAllTracker[_to] < block.timestamp - transferAllInterval, "transfer all cool down");
        transferAllTracker[msg.sender] = block.timestamp;
        transferAllTracker[_to] = block.timestamp;

        require(msg.sender != _to, "self transfer");
        _locks[_to] = _locks[_to] + _locks[msg.sender];

        if (lastUnlockTime[_to] < lockFromTime) {
            lastUnlockTime[_to] = lockFromTime;
        }

        if (lastUnlockTime[_to] < lastUnlockTime[msg.sender]) {
            lastUnlockTime[_to] = lastUnlockTime[msg.sender];
        }

        _locks[msg.sender] = 0;
        lastUnlockTime[msg.sender] = 0;
    ...
    /// @dev Set the global start time.
    function setGlobalStartTime(uint256 _startTime) public onlyAuthorized {
        require(globalStartTime > block.timestamp || globalStartTime == 0, "Already started");
        globalStartTime = _startTime;
        emit GlobalStartTimeSet(_startTime);
    }
}
