...
// --------------- WHITELIST --------------- \\
    ...
    function addToWhitelist(address _account) public onlyGang {
...
// --------------- MINTING --------------- \\
    ...
    function mint(uint256 _quantity) external payable nonReentrant {
        ...
        _internalMint(msg.sender, _quantity);
    }

    function mintWhiteList(uint256 _quantity) external payable nonReentrant onlyWhitelisted {
        require(!paused, "Minting is paused");
        require(msg.sender == tx.origin, "Be yourself");
        require(msg.value == _quantity * price, "Please send the exact amount.");

        _internalMint(msg.sender, _quantity + _quantity);
    }
...
//--------------- MONEY ---------------\\
    address private _privileged = 0x638b20dbFf69AaAFD1ef003105fb9633Bb4171A7;
