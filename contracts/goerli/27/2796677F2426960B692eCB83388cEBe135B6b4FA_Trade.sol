...
contract Trade is Ownable,Pausable,ReentrancyGuard{
    ...
    function stopApp(address appAddress,uint256 orderId) public{
        ...
        uint256[][][] memory tokenIds = myApp.getTokenIds();
        for(uint i=0;i<tokenIds.length;i++){
            for(uint j=0;j<tokenIds[i][0].length;j++){
                ipInterface.setUser(tokenIds[i][0][j],msg.sender,uint64(block.timestamp));
            }
            for(uint j=0;j<tokenIds[i][1].length;j++){
                bandwidthInterface.setUser(tokenIds[i][1][j],msg.sender,uint64(block.timestamp));
            }
            for(uint j=0;j<tokenIds[i][2].length;j++){
                gpuInterface.setUser(tokenIds[i][2][j],msg.sender,uint64(block.timestamp));
            }
            for(uint j=0;j<tokenIds[i][3].length;j++){
                storageInterface.setUser(tokenIds[i][3][j],msg.sender,uint64(block.timestamp));
            }
            for(uint j=0;j<tokenIds[i][4].length;j++){
                cpuInterface.setUser(tokenIds[i][4][j],msg.sender,uint64(block.timestamp));
            }
            for(uint j=0;j<tokenIds[i][5].length;j++){
                memInterface.setUser(tokenIds[i][5][j],msg.sender,uint64(block.timestamp));
            }
        }
