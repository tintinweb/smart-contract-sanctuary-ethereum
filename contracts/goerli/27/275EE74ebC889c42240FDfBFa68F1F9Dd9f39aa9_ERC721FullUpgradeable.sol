...
import { NonFungibleTypeUpgradeable } from "./internal-upgradeable/NonFungibleTypeUpgradeable.sol";
import { PaymentTokenUpgradeable } from "./internal-upgradeable/PaymentTokenUpgradeable.sol";
import { PriceOracleUpgradeable } from "./internal-upgradeable/PriceOracleUpgradeable.sol";
import { PrimarySaleUpgradeable } from "./internal-upgradeable/PrimarySaleUpgradeable.sol";

import { ReentrancyGuard } from "./utils/ReentrancyGuard.sol";

contract ERC721FullUpgradeable is
    IERC721Full,
    ReentrancyGuard,
    UUPSUpgradeable,
    ...
    NonFungibleTypeUpgradeable,
    PaymentTokenUpgradeable,
    PriceOracleUpgradeable,
    PrimarySaleUpgradeable,
    OwnableUpgradeable
{
    uint256 private _idCounter;
    mapping(uint256 => uint256) private _metadata;
    ...
    function initialize(string calldata name_, string calldata symbol_) external initializer {
        ...
        __Ownable_init_unchained();
        __NonFungibleType_init_unchained();
        __ReentrancyGuard_init_unchained();
        _setupPrimarySaleRecipient(_msgSender());
        _setPriceFeeds(address(0), 0xD4a33860578De61DBAbDc8BFdb98FD742fA7028e); // Native - USD
        _setForexPriceFeed(0x890701c8Ea61440C2533dC1Fb84F70028599D464); // JPY - USD
        _setType(1, 300 ether, 100, 1); // 300 jpy - limit 100 - minting 1
        _idCounter = 1;
    }
    ...
    function setForexPriceFeeds(address forexPriceFeed_) external onlyOwner {
        _setForexPriceFeed(forexPriceFeed_);
    }
    ...
    function acceptPaymentTokens(address[] calldata addresses_) external onlyOwner {
        _acceptPaymentTokens(addresses_);
    }

    function cancelPaymentTokens(address[] calldata addresses_) external onlyOwner {
        _cancelPaymentTokens(addresses_);
    }
    ...
    function setType(uint256 type_, uint256 price_, uint256 limit_, uint256 quantity_) external override onlyOwner {
        _setType(type_, price_, limit_, quantity_);
    }

    function buy(uint256 typeNFT_, uint256 quantity_, address paymentToken_) external payable override nonReentrant {
        if (!_acceptedToken(paymentToken_)) revert PaymentToken__NotExist();
        ...
        uint256 paymentAmount = _getTokenAmountDown(paymentToken_, total);
        if (paymentToken_ == address(0)) _receiveNative(paymentAmount);
        _transferCurrency(paymentToken_, sender, _recipient, paymentAmount);

        uint256 totalMint = quantity_ * typeInfo.quantity;
        uint256 tokenId = _idCounter;
        _batchMint(sender, typeNFT_, tokenId, totalMint);

        emit Registered(sender, typeNFT_, tokenId, totalMint, _sold[typeNFT_], paymentAmount);
    }

    function recoverToken(address currency_, uint256 amount_) external onlyOwner {
        _transferCurrency(currency_, address(this), owner(), amount_);
    }

    function metadata(uint256 tokenId_) external view override returns (uint256) {
        return _metadata[tokenId_];
    }

    function getTypeNFT(uint256 typeNFT) external view returns (uint256, uint256, uint256) {
        return (_typeInfo[typeNFT].price, _typeInfo[typeNFT].limit, _typeInfo[typeNFT].quantity);
    }

    function getTokenAmount(address paymentToken, uint256 fiatAmount) external view returns (uint256) {
        return _getTokenAmountUp(paymentToken, fiatAmount);
    }
    ...
    function _batchMint(address account_, uint256 typeNFT_, uint256 tokenId_, uint256 totalMint_) private {
        for (uint256 i; i < totalMint_; ) {
            unchecked {
                _metadata[tokenId_] = typeNFT_;
    ...
    function supportsInterface(bytes4 interfaceId) public view override(ERC721Upgradeable, ERC721EnumerableUpgradeable, ERC721WithPermitUpgradable) returns (bool) {
        return super.supportsInterface(interfaceId);
    }

    function _baseURI() internal view virtual override(ERC721Upgradeable, ERC721URIStorageUpgradeable) returns (string memory) {
        return _baseUri;
    }
...
interface IERC721Full {
    ...
    event Registered(address indexed user, uint256 indexed typeNFT, uint256 tokenId, uint256 totalMint, uint256 soldByType, uint256 paymentAmount);

    function setBaseTokenURI(string calldata baseTokenURI_) external;

    function buy(uint256 typeNFT_, uint256 quantity_, address paymentToken_) external payable;

    // function setLockUser(address account, bool status) external;

    function setupPrimarySaleRecipient(address recipient_) external;

    function setType(uint256 type_, uint256 price_, uint256 limit_, uint256 quantity_) external;

    function metadata(uint256 tokenId_) external view returns (uint256);

    function getTypeNFT(uint256 typeNFT) external view returns (uint256 price, uint256 limit, uint256 unboxQuantity);
}
...
contract CurrencyManagerUpgradeable is Initializable {
...
abstract contract EIP712Upgradeable is Initializable {
    ...
    function __EIP712_init_unchained(string memory name, string memory version) internal onlyInitializing {
        ...
        _domainSeparator = keccak256(abi.encode(_TYPE_HASH, keccak256(bytes(name)), keccak256(bytes(version)), block.chainid, address(this)));
    }
...
abstract contract ERC721URIStorageUpgradeable is Initializable, ERC721Upgradeable {
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        ...
        return bytes(baseUri).length > 0 ? string(abi.encodePacked(baseUri, "/", address(this).toHexString(), "/", tokenId.toString())) : "";
    }
...
abstract contract ERC721WithPermitUpgradable is IERC4494Upgradeable, Initializable, EIP712Upgradeable, ERC721Upgradeable {
    ...
    function __ERC721WithPermitUpgradable_init(string calldata name_, string calldata symbol_) internal onlyInitializing {
    ...
    function permit(address spender_, uint256 tokenId_, uint256 deadline_, bytes memory signature_) external override {
        require(deadline_ >= block.timestamp, "EXP");
...
interface INonFungibleTypeUpgradeable {
    ...
    event SetTypeNFT(uint256 typeNFT, uint256 price, uint256 limit, uint256 quantity);
}
...
interface IPaymentTokenUpgradeable {
    error PaymentToken__Existed();
    error PaymentToken__NotExist();

    event PaymentTokensAdded(address[] tokens);
    event PaymentTokensCancel(address[] tokens);
}
...
interface IPriceOracleUpgradeable {
    ...
    function getForexPriceFeed() external view returns (address);

    event SetPriceFeeds(address token, address priceFeed);
    event SetForexPriceFeed(address forexPriceFeed);
...
import { Initializable } from "@openzeppelin/contracts-upgradeable/proxy/utils/Initializable.sol";
import { INonFungibleTypeUpgradeable } from "./interfaces/INonFungibleTypeUpgradeable.sol";

abstract contract NonFungibleTypeUpgradeable is INonFungibleTypeUpgradeable, Initializable {
    mapping(uint256 => TypeInfo) internal _typeInfo;
    mapping(uint256 => uint256) internal _sold;

    function __NonFungibleType_init() internal onlyInitializing {
        __NonFungibleType_init_unchained();
    }

    function __NonFungibleType_init_unchained() internal onlyInitializing {}
    ...
    function _setType(uint256 type_, uint256 price_, uint256 limit_, uint256 quantity_) internal {
        if (_sold[type_] > limit_) revert NonFungibleType__ExceedLimit(type_);
        _typeInfo[type_] = TypeInfo(price_, limit_, quantity_);
        emit SetTypeNFT(type_, price_, limit_, quantity_);
    }
...
import { Initializable } from "@openzeppelin/contracts-upgradeable/proxy/utils/Initializable.sol";
import { IPaymentTokenUpgradeable } from "./interfaces/IPaymentTokenUpgradeable.sol";
import { EnumerableSet } from "../libraries/EnumerableSet.sol";

contract PaymentTokenUpgradeable is IPaymentTokenUpgradeable, Initializable {
    using EnumerableSet for EnumerableSet.AddressSet;
    EnumerableSet.AddressSet internal _paymentTokenAddressSet;

    function _acceptPaymentTokens(address[] calldata addresses_) internal {
        uint256 length = addresses_.length;
        for (uint256 i = 0; i < length; ) {
            if (!_paymentTokenAddressSet.add(addresses_[i])) revert PaymentToken__Existed();
    ...
    function _cancelPaymentTokens(address[] calldata addresses_) internal {
        uint256 length = addresses_.length;
        for (uint256 i = 0; i < length; ) {
            if (_paymentTokenAddressSet.remove(addresses_[i])) revert PaymentToken__NotExist();
            unchecked {
                ++i;
            }
        }
    ...
    function viewPaymenTokens() external view returns (address[] memory, uint256) {
        return (_paymentTokenAddressSet.values(), _paymentTokenAddressSet.length());
    }
...
import { IPriceOracleUpgradeable } from "./interfaces/IPriceOracleUpgradeable.sol";
import { FixedPointMathLib } from "../libraries/FixedPointMathLib.sol";

abstract contract PriceOracleUpgradeable is Initializable, IPriceOracleUpgradeable {
    ...
    function getForexPriceFeed() external view returns (address) {
        return address(_forexPriceFeed);
    }

    function _getTokenAmountDown(address token, uint256 fiatAmount) internal view returns (uint256) {
        if (_forexPriceFeed == IAggregatorV3(address(0))) return fiatAmount.mulDivDown(1 ether, _getUsdPrice(token));
        return ((fiatAmount * _getFiatPrice()) / 1 ether).mulDivDown(1 ether, _getUsdPrice(token));
    }

    function _getTokenAmountUp(address token, uint256 fiatAmount) internal view returns (uint256) {
        if (_forexPriceFeed == IAggregatorV3(address(0))) return fiatAmount.mulDivUp(1 ether, _getUsdPrice(token));
        return ((fiatAmount * _getFiatPrice()) / 1 ether).mulDivUp(1 ether, _getUsdPrice(token));
    }

    function _getFiatPrice() private view returns (uint256) {
        (, int256 price, , , ) = _forexPriceFeed.latestRoundData();
        return uint256(price * 1e10);
    }

    function _getUsdPrice(address token) private view returns (uint256) {
    ...
    function _setForexPriceFeed(address forexPriceFeed_) internal {
        _forexPriceFeed = IAggregatorV3(forexPriceFeed_);
        emit SetForexPriceFeed(address(forexPriceFeed_));
    }
...
abstract contract PrimarySaleUpgradeable is IPrimarySaleUpgradeable {
...
library EnumerableSet {
    ...
    /**
     * @dev Returns the index of the value in the set. O(1).
     */
