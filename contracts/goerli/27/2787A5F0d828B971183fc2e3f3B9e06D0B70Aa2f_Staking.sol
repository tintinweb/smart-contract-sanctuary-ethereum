...
contract Staking is ReentrancyGuardUpgradeable, OwnableUpgradeable {
    bool public paused;
    ...
    function initialize(address[] memory _initContracts) external initializer {
        __Ownable_init();
        __ReentrancyGuard_init();

        for (uint256 i = 0; i < _initContracts.length; i++) {
            addCollection(_initContracts[i]);
        }
    ...
    function deposit(address _nftAddress, uint256[] memory tokenIds)
        public
        nonReentrant
        whenNotPaused
    {
        ...
        for (uint256 i; i < tokenIds.length; i++) {
            require(
                IERC721(_nftAddress).ownerOf(tokenIds[i]) == _msgSender(),
                "Not the owner"
            );
    ...
    function withdraw(address _nftAddress, uint256[] memory tokenIds)
        public
        nonReentrant
        whenNotPaused
    {
        ...
        for (uint256 i; i < tokenIds.length; i++) {
            require(
                IERC721(_nftAddress).ownerOf(tokenIds[i]) == address(this),
                "Token not staked"
            );
            require(
                _ownerOfToken[_nftAddress][tokenIds[i]] == _msgSender(),
    ...
    function addCollection(address _collectionAddress) public onlyOwner {
        require(
            !_tokenAddressExist[_collectionAddress],
