...
contract Median is LibNote {
    ...
    // --- Auth ---
    mapping (address => uint) public wards; // Auth mechanisms.
    ...
    uint128        val; // price/value, only read it from the two functions read and peek
    uint32  public age;
    bytes32 public wat = "ethusd";// the price oracles type (ex: ETHUSD) 
    uint256 public bar = 1; // the minimum number of prices necessary to accept a new median value, the Minimum writers quorum for poke / min number of valid messages you need to have to update the price. => quorum

    // Authorized oracles, set by an auth
    mapping (address => uint256) public orcl; // writers whitelist/signers of the prices (whitelisted via governance / the authorized parties).

    // Whitelisted contracts, set by an auth
    mapping (address => uint256) public bud; // readers whitelist. 
