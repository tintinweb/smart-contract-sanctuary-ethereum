...
import {LibConnextStorage, CallParams, XCallArgs} from 'nxtp-libs/LibConnextStorage.sol';
import {IConnextHandler} from 'nxtp-interfaces/IConnextHandler.sol';

contract ConnextSenderAdapter {
  IConnextHandler public connext;
  uint32 public originDomainId;
  address public asset;

  constructor(IConnextHandler _connext, uint32 _originDomainId, address _asset) {
    connext = _connext;
  ...
  function bridgeObservations(
    ...
    bytes4 _selector = bytes4(keccak256('setMessage(string)'));
    bytes memory _callData = abi.encodeWithSelector(_selector, _message);
    ...
    XCallArgs memory _xcallArgs = XCallArgs({params: _callParams, transactingAssetId: asset, amount: 0});

    connext.xcall(_xcallArgs);
  }

  function setConnext(IConnextHandler _connext) external {
    connext = _connext;
  }

  function setOriginDomainId(uint32 _originDomainId) external {
    originDomainId = _originDomainId;
  }
