...
contract OmniCounter is BaseAggregator {
    ...
    uint16 public constant PT_FIRST = 1;
    uint16 public constant PT_SECOND = 2;
    ...
    function _nonblockingLzReceive(uint16 _srcChainId, bytes memory, uint64, bytes memory _payload) internal override {
        ...
        if (packetType == PT_FIRST) {
            counter += 1;

            bytes memory lzPayload = abi.encode(PT_SECOND, counter);
            (uint256 lzFee, ) = ILayerZeroEndpoint(lzEndpoint).estimateFees(_srcChainId, address(this), lzPayload, false, bytes(""));
            _lzSend(_srcChainId, lzPayload, payable(address(this)), address(0x0), bytes(""), lzFee + 200000);
        } else if (packetType == PT_SECOND) {
            (, uint256 delta) = abi.decode(_payload, (uint16, uint256));
            counter += delta;
        }
    }

    function incrementCounter(uint16 _dstChainId) public payable {
        bytes memory payload = abi.encode(PT_FIRST);
