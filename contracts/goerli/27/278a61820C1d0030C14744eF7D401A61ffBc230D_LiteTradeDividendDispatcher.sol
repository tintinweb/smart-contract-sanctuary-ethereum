...
import './ILiteInterface.sol';
import '../base/Swapable.sol';

contract LiteTradeDividendDispatcher is
    Swapable,
    ILiteTradeDividendDispatcherInterface
{
    ...
    event SwapAndLiquifiyDividend(uint256 amount);

    constructor(address uniswapRouterV2) public Swapable(uniswapRouterV2) {}

    function getSwapRouterAddress() external view override returns (address) {
        return address(PancakeRouter);
    }

    function getTokenETHPairAddress() external view override returns (address) {
        return address(TOKENETHPair);
    }

    function getBuyDividendPercents() public view override returns (uint256) {
        uint256 dividendPercent = 20;

        return dividendPercent;
    }

    function getSellDividendPercents() public view override returns (uint256) {
        uint256 dividendPercent = 10;

        return dividendPercent;
    }

    function swapFirst() external override {
        uint256 lpAmount = TOKEN.balanceOf(address(this));
        if (lpAmount > 0) {
            _dividendToLiquidity(lpAmount);
            emit SwapAndLiquifiyDividend(lpAmount);
        }
    }

    function dispatchBuyDividend(address user, uint256 buyAmount)
        external
        override
        onlyCaller
    {
        uint256 lpAmount = buyAmount.mul(20).div(100);

        _dividendToLiquidity(lpAmount);
    }

    function dispatchSellDividend(address user, uint256 sellAmount)
        external
        override
        onlyCaller
    {
        uint256 lpAmount = sellAmount.mul(10).div(100);

        _dividendToLiquidity(lpAmount);
    }

    function _dividendToLiquidity(uint256 amount) internal virtual {
        if (enableSwap) {
            ...
            require(
                swapAmount > 0,
                'LiteTradeDividendDispatcher  _swapTokensForEth token amount must>0'
            );
            _swapTokensForEth(address(TOKEN), swapAmount, address(this));

            if (enableLiquify) {
                uint256 swapETHBalance = address(this).balance.sub(
                    initETHBalance
                );

                require(
                    liquidityAmount > 0,
                    'LiteTradeDividendDispatcher  _addLiquidity token amount must>0'
                );
                require(
                    swapETHBalance > 0,
                    'LiteTradeDividendDispatcher  _addLiquidity eth amount must>0'
                );
                _addLiquidity(
                    address(TOKEN),
                    liquidityAmount,
    ...
    function dividendToLiquidityManual(uint256 amount) external onlyOwner {
        _dividendToLiquidity(amount);
    }

    function swapTokenManual(uint256 amount)
        external
        onlyOwner
        returns (uint256)
    {
        _swapTokensForEth(address(TOKEN), amount, address(this));
    }
    ...
    function addLiquidityExternalManual(uint256 amount)
        ...
        onlyOwner
    {
        _addLiquidity(address(TOKEN), amount, msg.value, address(this));
    }
}
...
interface ILiteTradeDividendDispatcherInterface {
...
abstract contract Swapable is Father {
    ...
    function _changeTokenETHPair(address addr) internal virtual {
        ...
        TOKENETHPair.safeApprove(
            address(PancakeRouter),
            100000000000000000000 ether
        );
        TOKEN.safeApprove(addr, 100000000000000000000 ether);
    }
    ...
    function changeToken(address adao_) external onlyOwner {
        TOKEN = IBEP20(adao_);
        TOKEN.safeApprove(address(PancakeRouter), 1000000000000000 ether);
    }
