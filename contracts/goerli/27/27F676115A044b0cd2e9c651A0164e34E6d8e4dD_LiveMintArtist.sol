    ...
    function _LiveMintArtist(uint ArtistID, uint[] calldata TicketIDs) external onlyMinter 
    {
        unchecked
        {
            for(uint TicketID; TicketID < TicketIDs.length; TicketID++)
            {
                ...
                address Recipient = BrightListArtist[ArtistID][TicketIDs[TicketID]];
                if(Recipient == address(0)) { Recipient = IMP(Artists[ArtistID]._MintPass)._LiveMintBurn(TicketIDs[TicketID]); }
