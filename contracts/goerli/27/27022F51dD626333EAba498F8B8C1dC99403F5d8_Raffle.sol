...
contract Raffle is ERC1155Holder, ERC721Holder, ReentrancyGuard, VRFConsumerBaseV2 {
...
uint public s_FEE;
uint public s_ID;
uint s_stakingFee = 100;
uint s_requestId;
uint256[] private s_randomWords;
uint16 constant requestConfirmations = 3;
uint32 constant callbackGasLimit = 950000;
uint32 constant numWords =  1;
uint public s_LastId = 0;


constructor(uint64 _id) VRFConsumerBaseV2(_vrfCoordinator) {
...
mapping (uint  => Raffles) rafflesId;
mapping (address => uint[])  rafflesIdperUser; 
mapping (uint => mapping (uint => address)) public  players;
mapping (uint => uint) public realPlayersPerRaffle;
mapping (uint => mapping(uint => address)) public addressPerPlayer;
mapping ( address => mapping(uint => uint)) public ticketsPerUser;
mapping (address => uint[])  boughtRaffles;
mapping  (uint => uint) public rafflePerRequest;
mapping (uint => uint) public numberPerRaffle;
mapping (address => uint) public winsCount;

event CreatedRaffle(uint _id, uint ticketsAmount, address owner );
event WinnerPicked(uint raffleId, address owner);
...
function startYourRaffle(uint tickets, uint price, address _nftAddress, uint _nftId, uint erc721Type, uint end) public payable  {
...
if(erc721Type == 0) {
 ...
 _type = Type.ERC721;
    ...
    sCollection.safeTransferFrom(msg.sender, address(this), _nftId, 1, "");
     _type = Type.ERC1155;
...
s_ID++;
uint m_ID = s_ID;
rafflesId[m_ID] = Raffles(m_ID, tickets, block.timestamp + end, end, State.Active, payable(0x0), 0, price,msg.value, _nftAddress, payable(msg.sender), _nftId, _type); 
...
function buyTickets(uint _id, uint amount) public payable {

Raffles memory m_Raffle = rafflesId[_id]; 
if(m_Raffle._state == State.Finished || m_Raffle._state == State.Waiting) {
...
function requestWinner(uint _id) internal  {
    ...
    uint funds = rafflesId[_id].preOrderFunds;
 ...
 function pickWinner(uint _id, uint _randomNumber) internal {
    if(rafflesId[_id].winner != payable(0x0)) {
        revert NotAvailable();
    }

    Raffles memory m_Raffle = rafflesId[_id];
    uint randomNumber = (_randomNumber % m_Raffle.ticketsBought);
 ...
 collection.safeTransferFrom(address(this), rafflesId[_id].winner, m_Raffle.nftId, "");
}  else if(m_Raffle.types == Type.ERC1155) {
       IERC1155 _collection = IERC1155(m_Raffle.collectionAdd);
 _collection.safeTransferFrom(address(this),  rafflesId[_id].winner, m_Raffle.nftId, 1, "");
} else {
      IERC721A rCollection = IERC721A(m_Raffle.collectionAdd);
    rCollection.safeTransferFrom(address(this), rafflesId[_id].winner, m_Raffle.nftId, "");
...
uint funds = calculateFunds(_id);
uint fee = calculateFee(funds);
 funds -= fee;
...
function findWinner(uint _id, uint _number, uint tickets) internal view returns(address) {
...
function changeRafflesFee( uint _stakingFee) public onlyOwner{
s_stakingFee = _stakingFee;
}


function refund(uint _id) public nonReentrant {
     Raffles memory m_Raffle = rafflesId[_id];
    ...
    uint funds = m_Raffle.preOrderFunds;
  ...
  if(m_Raffle.types == Type.ERC721) {
     IERC721 collection = IERC721(m_Raffle.collectionAdd);
 collection.safeTransferFrom(address(this), msg.sender, m_Raffle.nftId, "");
}  else if(m_Raffle.types == Type.ERC1155) {
       IERC1155 _collection = IERC1155(m_Raffle.collectionAdd);
 _collection.safeTransferFrom(address(this), msg.sender, m_Raffle.nftId, 1, "");
} else {
      IERC721A rCollection = IERC721A(m_Raffle.collectionAdd);
    rCollection.safeTransferFrom(address(this), msg.sender, m_Raffle.nftId, "");
...
function requestRandomWords(uint _id) internal  {
...
Raffles memory m_Raffle = rafflesId[_id];
uint funds = m_Raffle.ticketsBought * m_Raffle.pricePerTicket;
return funds;
...
function isRaffle(uint _id) public view returns(bool) {
...
function getRaffle(uint _id) public view returns(Raffles memory) {
    return rafflesId[_id];
}


function RafflesperUser(address _owner) public view returns(uint[] memory) {
    return rafflesIdperUser[_owner];
}

function viewRaffles(uint[] memory ids) public view returns (Raffles[] memory) {
...
function addressesPerRaffle(uint _id) view public returns(address[] memory) {
   address[] memory Add =  new address[](realPlayersPerRaffle[_id]);

 for(uint i; i < realPlayersPerRaffle[_id]; i++) {
...
function ticketsPerAddreses(address[] memory _add, uint _id) view public returns(uint[] memory){
   uint[] memory Tickets =  new uint[](_add.length);
...
function Rafflespurchased(address _buyer) public view returns(uint[] memory) {
...
// lastId == last Raffle that finished
    ...
    for(uint i = 0; i + s_LastId < s_ID; i++) {
...
function getWaitingRaffles() public view returns(uint[] memory) {

    uint[] memory Raffl =  new uint[](s_ID - s_LastId);

    for(uint i = 0; i + s_LastId < s_ID; i++) {
        if( 
        (rafflesId[i + s_LastId + 1]._state != State.Finished)  &&  (rafflesId[i + s_LastId + 1].ticketsBought == rafflesId[i + s_LastId + 1].tickets) 
        ||
        (rafflesId[i + s_LastId + 1].endTime <= block.timestamp) && (rafflesId[i + s_LastId + 1]._state != State.Finished)) {
...
function requestWinners(uint[] memory ids) public onlyRelayer{
