...
error TokenAlreadyCompletedQuest(uint32 tokenId);
...
contract ChibimonQuestsTest is Ownable, ReentrancyGuard {
    ...
    mapping(uint32 => mapping(uint32 => bool)) public tokenHistory;
    mapping(uint32 => Quest) public quests;
    mapping(uint32 => QuestNftReward[]) public questNftRewards;
    uint32[] public questIds;
    uint32 public nextQuestId;

    IERC20 public immutable apeCoin;

    constructor(address apeCoinAddress, address signerAddress, uint32 startQuestId) {
        treasury = msg.sender;
    ...
    function claim(bytes calldata signature, uint32 questId, uint32[] calldata tokenIds) external nonReentrant {
    ...
    function claimForHolder(address holder, uint32 questId) external onlyOwner {
        if( quests[questId].amount <= 0 ) revert NoQuestAmountAvailable();

        _claim(holder, questId);
    }

    function setTokenQuestHistory(uint32 tokenId, uint32 questId, bool status) external onlyOwner {
        tokenHistory[tokenId][questId] = status;
    }

    function createQuest(uint32 amount, uint32 activeFrom, uint32 activeUntil, uint32 apeCoinRewards, uint32[] calldata nftRewardContractTypes, address[] calldata nftRewardContracts, uint32[] calldata nftRewardTokenIds, uint32[] calldata nftRewardTokenAmounts) external onlyOwner {
        uint32 questId = _createQuest(amount, activeFrom, activeUntil, apeCoinRewards);
    ...
    function editQuestAmount(uint32 id, uint32 amount) external onlyOwner {
        quests[id].amount = amount;
    }

    function editQuestActiveTimespan(uint32 id, uint32 activeFrom, uint32 activeUntil) external onlyOwner {
        quests[id].activeFrom = activeFrom;
        quests[id].activeUntil = activeUntil;
    }

    function editQuestApeCoinRewards(uint32 id, uint32 apeCoinRewards) external onlyOwner {
        quests[id].apeCoinRewards = apeCoinRewards;
    }

    function editQuestNftRewards(uint32 id, uint32[] calldata nftRewardContractTypes, address[] calldata nftRewardContracts, uint32[] calldata nftRewardTokenIds, uint32[] calldata nftRewardTokenAmounts) external onlyOwner {
    ...
    function deleteQuest(uint32 id) external onlyOwner {
        delete quests[id];
    ...
    function getQuestNftRewards(uint32 questId) public view returns(QuestNftReward[] memory) {
        return _getQuestNftRewards(questId);
    }

    function getTokenQuestHistory(uint32 tokenId) public view returns(uint32[] memory) {
        uint32[] memory tokenQuests = new uint32[](_getTokenQuestHistoryCount(tokenId));
    ...
    function _createQuest(uint32 amount, uint32 activeFrom, uint32 activeUntil, uint32 apeCoinRewards) internal returns(uint32) {

        uint32 questId = nextQuestId;
    ...
    function _addNftReward(uint32 questId, uint32 nftContractType, address nftRewardContract, uint32 nftRewardTokenId, uint32 nftRewardTokenAmount) internal {
    ...
    function _claim(address sender, uint32 questId) internal {
        ...
        if(questNftRewards[questId].length > 0) {
            ...
            for( uint32 i; i < currentRewards.length; i++) {
                ...
                } else if(currentRewards[i].nftContractType == 1) {
    ...
    function _getTokenQuestHistoryCount(uint32 tokenId) internal view returns(uint32) {
    ...
    function _getQuestNftRewards(uint32 id) internal view returns(QuestNftReward[] memory) {
    ...
    function _verifySig(address sender, uint32 questId, uint32[] calldata tokenIds, bytes memory signature) internal view returns(bool) {
