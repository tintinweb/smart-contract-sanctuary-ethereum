...
contract Dr1ver is Ownable, Pausable, ReentrancyGuard, ERC721Enumerable {
    ...
    string private _baseTokenURI =
        "https://storage.googleapis.com/bucket-cultivate/dr1ver/dr1ver-metadata/";

    address sorcevialAddress; // Approved mintvial contract
    address unpoweredSorcevialAddress; // Something ~ ~ ~
    ...
    function mintTransfer(address to) public payable {
        require(msg.sender == sorcevialAddress, "Not authorized");
        require(price <= msg.value, "KD: Insufficient eth");

        // @n0x - here we are minting the unpowered Sorce
        MintableInterface unpoweredSorceContract = MintableInterface(
            unpoweredSorcevialAddress
        );
        unpoweredSorceContract.mintTransfer(to);

        currentTokenId++;

        // @n0x - and then minting the dr1ver
    ...
    function enhanceToken(uint256 tokenId) public {
        require(
        ...
        enhancementMap[tokenId + enhancementShift] =
            enhancementMap[tokenId] +
            1;
    ...
    function resetToken(uint256 tokenId) public {
        ...
        _safeMint(
            _msgSender(),
            tokenId - enhancementShift * enhancementMap[tokenId]
        );
    ...
    function setSorcevialAddress(address newAddress)
        public
        onlyOwnerOrApproved
    {
        sorcevialAddress = newAddress;
    }

    function setUnpoweredSorcevialAddress(address newAddress)
        public
        onlyOwnerOrApproved
    {
        unpoweredSorcevialAddress = newAddress;
    }

    function contractURI() public pure returns (string memory) {
        return "ipfs://QmSpdTYZxj7crNyF7wC95eRxFmux9ec1UAXZZv1KLocJd9";
    }
