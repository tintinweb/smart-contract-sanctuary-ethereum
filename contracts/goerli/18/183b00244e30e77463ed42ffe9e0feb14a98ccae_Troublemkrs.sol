...
contract Troublemkrs is ERC721A, Ownable {
	...
	uint public whitelistPrice = 0.01 ether;
    ...
    function depositPublic(bytes32[] memory proof) public payable {
		uint count = 1;
	...
	function awardTokensFromDeposits() public onlyOwner {
        ...
        for (uint i = 0; i < awardableWallets.length; i++) {

            if (_totalMinted() + 1 > maxSupply) revert("Exceeds max supply");

            // Remove address from awardableWallets list
            awardableWallets[i] = awardableWallets[awardableWallets.length - 1];
            awardableWallets.pop();

            // Mint to address
            _safeMint(awardableWallets[i], 1);
