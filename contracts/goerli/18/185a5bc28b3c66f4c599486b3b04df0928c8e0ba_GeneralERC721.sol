...
contract GeneralERC721 is 
    ...
    function redeem(address redeemer, NFTVoucher calldata voucher) public payable returns (uint256) {
        ...
        uint256 current = uint256(keccak256(abi.encodePacked(block.number, msg.sender, block.timestamp)));
        ...
        _safeMint(signer, current);

        // first assign the token to the signer, to establish provenance on-chain
        _setTokenURI(current, voucher.uri);

        // transfer the token to the redeemer
        _transfer(signer, redeemer, current);
    ...
    function _hash(NFTVoucher calldata voucher) internal view returns (bytes32) {
        return EIP712Upgradeable._hashTypedDataV4(keccak256(abi.encode(
            keccak256("NFTVoucher(uint256 minPrice,address signer,string uri)"),
            voucher.minPrice,
            voucher.signer,
     ...
     *
     */
    function mint(address to, string memory uri) public payable virtual {
