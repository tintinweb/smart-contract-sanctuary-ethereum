...
contract MetaMaster {
    ...
    function deposit(uint256 _USDTAmount, address _referrer) public{
        require(_USDTAmount % 50 == 0 && _USDTAmount >= 50 && _USDTAmount <= 5000 , "Invalid USDT Amount");
    ...
    function getUserTotalReturns(address _userAddress, uint256 _days) public view returns(uint256){
        uint256 dailyRewardPercentage = 70;
        uint256 percentageDivider = 100;
        uint256 totalReward = 0;
        for(uint8 i = 0; i < users[_userAddress].deposits.length; i++) {
            uint256 reward;
            uint256 numberOfDays = _days;
            // uint256 numberOfDays = block.timestamp.sub(users[_userAddress].deposits[i].depositedTimeStamp).div(1 days);
            uint256 dailyrewardAmount = users[_userAddress].deposits[i].depositAmount%((dailyRewardPercentage).div(percentageDivider));
    ...
    function getUserPendingWithdrawalAmount(address _userAddress, uint256 _days) public view returns(uint256) {
        return getUserTotalReturns(_userAddress, _days).sub(amountWithdrawn[_userAddress]);
    }
 
    // withdraw from the daily returns
    function withdraw(address _userAddress, uint256 _amount, uint256 _days) public  {
        require(msg.sender == _userAddress, "Caller should be the owner of withdraw account");
        require(getUserPendingWithdrawalAmount(_userAddress, _days) >= _amount, "Invalid Amount");
        amountWithdrawn[_userAddress] = amountWithdrawn[_userAddress] + _amount;
