...
contract UserLendMarketplace is UserMarketplace {
  ...
  function userOffersRental(
    ...
    rentOffers[++lastRentId] = RentOffer({
      ...
      paymentOptions: _createPaymentInterface(paymentAddress_, paymentAmount_, true, false),
  ...
  function offerCreatorCancelsRental(uint256 rentId_) external {
    ...
    } else if (!IERC20(paymentAddress_).transfer(msg.sender, offer.paymentOptions.payAmount())) {
      revert ERC20TransferFailed();
    }
  ...
  function ownerAcceptsUserRentalOffer(uint256 rentId_) external whenNotPaused {
    RentOffer memory offer = _getValidRental(rentId_, RentStatus.CREATED);
