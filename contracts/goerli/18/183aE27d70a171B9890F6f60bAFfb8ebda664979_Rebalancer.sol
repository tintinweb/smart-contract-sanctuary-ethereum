...
contract Rebalancer is IRebalancer {
    ...
    function calculate( address token) public view override returns (int quantity, uint discount)         
    {
        address market = IProtocolConfig(config).getMarket(token);
        (,uint AP, uint AR, uint CP, uint CR, , , ) = IMarket(market).getState();
        (int price,) = IMarket(market).price();        
        quantity = int(AR * DIVIDER  / uint(price) );
        discount = 0;
    }
