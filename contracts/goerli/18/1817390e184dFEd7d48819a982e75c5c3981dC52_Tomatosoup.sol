...
contract Tomatosoup is ERC721Enumerable, Ownable, ReentrancyGuard {
  ...
  function pubMint(uint256 _quantity) external payable callerIsUser nonReentrant {
    ...
    require(block.timestamp >= startTime, 'PubMint close now.');
  ...
  //set token url
  function setIpfsURI(string memory _newIpfsURI) public onlyOwner {
    ipfsURI = _newIpfsURI;
    reveal = true;
  }
  ...
  function tokenURI(uint256 tokenId) public view virtual override returns (string memory url) {
    _requireMinted(tokenId);
    string memory baseURI = _baseURI();
    string memory isreveal = string(abi.encodePacked(baseURI, Strings.toString(tokenId), '.png'));
    if (bytes(baseURI).length > 0) {
      url = reveal ? isreveal : baseURI;
    } else {
      url = '';
    }
  }
