...
contract NFTNFTWalletProxyFactory {
    event ProxyCreation(NFTNFTWalletProxy proxy);

    address public singleton;
    address public policy;

    mapping(address => address) private approvedWallets;
    address internal constant APPROVED_WALLET_HEAD = address(0x1);

    // bytes4(keccak256("isWalletApproved(address)")
    bytes4 internal constant APPROVED_WALLET_MAGIC_VALUE = 0x3657e851;

    constructor(address _singleton, address _policy) {
        singleton = _singleton;
        policy = _policy;
        approvedWallets[APPROVED_WALLET_HEAD] = APPROVED_WALLET_HEAD;
    }

    function isWalletApproved(address proxy) external view returns (bytes4) {
        require(proxy != address(0), "Invalid wallet address");
        if (proxy != APPROVED_WALLET_HEAD && approvedWallets[proxy] != address(0)) {
    ...
    function deployProxyWithNonce(
        ...
        require(address(proxy) != APPROVED_WALLET_HEAD && approvedWallets[address(proxy)] == address(0), "Same proxy already exists");
        approvedWallets[address(proxy)] = approvedWallets[APPROVED_WALLET_HEAD];
        approvedWallets[APPROVED_WALLET_HEAD] = address(proxy);

        bytes memory initializer = abi.encodeWithSelector(NFTNFTWallet.initialize.selector, msg.sender, policy);
        assembly {
...
contract NFTNFTWallet is CallbackHandler, ISignatureValidator {
    ...
    function initialize(address _owner, address _policy) public {
