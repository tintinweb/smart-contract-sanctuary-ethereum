...
/**
 * @title Binary Vault for trading
    ...
    Ownable
{
    ...
    struct WithdrawalRequest {
        uint256 id; // sequence number
        address user; // withdrawal to
        uint256 shareAmount; // share amount 
        bool fromPosition; // Specific token id
        uint256 tokenId; // nft id which will burnt. (This is available if fromPosition is true)
        uint256 timestamp; // request block time
        WithdrawalRequestStatus status; // This request is executed or not?
    }

    WithdrawalRequest[] private withdrawalRequests;

    uint256 public constant MINIMUM_LIQUIDITY = 10**3;
    uint256 public WITHDRAWAL_DELAY_TIME = 24 hours;
    ...
    /// @dev TVL of vault. This should be updated when deposit(+), withdraw(-), trader lose (+), trader win (-), trading fees(+)
    uint256 public totalDepositedAmount;
    /// @dev Watermark for risk management. This should be updated when deposit(+), withdraw(-), trader lose (+), trading fees(+)
    ...
    event WithdrawalRequested(address indexed requester, address indexed user, uint256 shareAmount, uint256 tokenId, bool fromPosition);
    ...
    */
        ...
        nonReentrant
    {
        ...
        if (totalShareSupply > 0) {
            ...
            newShares = amount;
            require(newShares > MINIMUM_LIQUIDITY, "INSUFFICIENT_AMOUNT");
        }
    ...
    */
        ...
        nonReentrant
    {
        ...
        require(user == msg.sender || isApprovedForAll(user, msg.sender), "NOT_APPROVED_OR_OWNER");
    ...
    */
    function _removeLiquidity(address user, uint256 shareAmount)
        internal
        whenNotPaused
    {
        require(shareAmount > 0, "ZERO_AMOUNT");
        require(msg.sender == user || isApprovedForAll(user, msg.sender), "NOT_APPROVED");
        ...
        require(shareAmount <= shareAmounts, "INSUFFICIENT_AMOUNT");
        ...
        uint256 _underlyingTokenAmount = shareAmount * totalDepositedAmount / totalShareSupply;
        underlyingToken.safeTransfer(user, _underlyingTokenAmount);

        totalShareSupply -= shareAmount;
        totalDepositedAmount -= _underlyingTokenAmount;
        watermark -= _underlyingTokenAmount;
    ...
    */
    function _removeLiquidityPosition(address user, uint256 tokenId, uint256 shareAmount)
        internal 
        whenNotPaused
    {
        require(shareAmount > 0, "ZERO_AMOUNT");
    ...
    /**
    * @dev Request withdrawal (This request will be delayed for WITHDRAWAL_DELAY_TIME)
    * @param user Request user
    * @param shareAmount share amount to be burnt
    * @param fromPosition whether burn share from specific token id 
    * @param tokenId This is available when fromPosition is true
    */
    function requestWithdrawal(address user, uint256 shareAmount, bool fromPosition, uint256 tokenId)
        external
        whenNotPaused
        nonReentrant
    {
        ...
        if (fromPosition) {
            ...
            for (uint256 i; i < tokenIds.length; i = i + 1) {
                ...
                shareAmounts += shareBalances[_tokenId];
            }
        ...
        uint256 _id = withdrawalRequests.length;
        ...
        WithdrawalRequest memory _request = WithdrawalRequest (
            ...
            tokenId,
            block.timestamp,
            WithdrawalRequestStatus.NONE
        );

        withdrawalRequests.push(_request);

        emit WithdrawalRequested(msg.sender, user, shareAmount, tokenId, fromPosition);
    }

    /**
    * @dev Execute withdrawal request if it passed enough time.
    * @param requestId withdrawal request id to be executed.
    */
    function executeWithdrawalRequest(uint256 requestId) 
        external 
        whenNotPaused 
        nonReentrant
    {
        require(requestId < withdrawalRequests.length, "INVALID_ID");
        WithdrawalRequest storage _request = withdrawalRequests[requestId];
        // Check if current sender is eligible to execute request.
        require(msg.sender == _request.user, "NOT_REQUEST_OWNER");
        // Check if time is passed enough
        require(block.timestamp >= _request.timestamp + WITHDRAWAL_DELAY_TIME, "TOO_EARLY");
        // Check if request is already executed
        require(_request.status == WithdrawalRequestStatus.NONE, "ALREADY_EXECUTED_OR_CANCELLED");

        _request.status = WithdrawalRequestStatus.EXECUTED;

        if (_request.fromPosition) {
            _removeLiquidityPosition(_request.user, _request.tokenId, _request.shareAmount);
        } else {
            _removeLiquidity(_request.user, _request.shareAmount);
        }
    }

    /**
    * @dev Get all withdrawal requests from certain user
    */
    function getAvailableWithdrawalRequests(address user) external view returns (WithdrawalRequest[] memory requests) {
        requests = new WithdrawalRequest[](withdrawalRequests.length);
        uint256 count;

        for(uint256 i; i < withdrawalRequests.length; i ++) {
            WithdrawalRequest memory _request = withdrawalRequests[i];
            if (_request.user == user && _request.status == WithdrawalRequestStatus.NONE) {
                requests[count] = _request;
                count ++;
            }
        }

        uint256 toDrop = withdrawalRequests.length - count;

        if (toDrop > 0) {
            assembly {
                mstore(requests, sub(mload(requests), toDrop))
            }
        }
    ...
    /**
    * @dev current whole amount of pending withdrwal request.
    */
    function getCurrentPendingWithdrawalAmount() public view returns(uint256 shareAmount, uint256 underlyingTokenAmount) {
        for (uint256 i; i < withdrawalRequests.length; i ++) {
            WithdrawalRequest memory _request = withdrawalRequests[i];
            if (_request.status == WithdrawalRequestStatus.NONE) {
                shareAmount += _request.shareAmount;
            }
        }
        underlyingTokenAmount = totalDepositedAmount * shareAmount / totalShareSupply;
    }

    /**
    * @dev Check if future betting is available based on current pending withdrawal request amount
    */
    function isFutureBettingAvailable() external view returns(bool) {
        (, uint256 currentPendingUnderlyingTokenAmount) = getCurrentPendingWithdrawalAmount();
        if (currentPendingUnderlyingTokenAmount >= totalDepositedAmount * config.maxWithdrawalBipsForFutureBettingAvailable() / config.FEE_BASE()) {
            return false;
        } else {
            return true;
        }
    ...
    function _cutTradingFee(uint256 amount) internal returns (uint256) {
        ...
        uint256 treasury_fee = fee * config.treasury_bips() / config.FEE_BASE();

        underlyingToken.safeTransfer(config.treasury(), treasury_fee);
        feeAccrued += fee;

        totalDepositedAmount += fee;
        watermark += fee;
    ...
    */
        ...
        returns(uint256 shares, uint256 underlyingTokenAmount)
    {
        ...
        underlyingTokenAmount = shares * totalDepositedAmount / totalShareSupply; 
    }
    ...
    * @dev set config
    */
    function setConfig(IBinaryConfig _config) external onlyOwner {
    ...
    */
    function increaseDepositedAmount(uint256 amount) external override onlyMarket {
        totalDepositedAmount += amount;
    ...
    * @dev This function is used to update total deposited amount from user betting
    */
    function decreaseDepositedAmount(uint256 amount) external override onlyMarket {
        if (totalDepositedAmount >= amount) {
            totalDepositedAmount -= amount;        
        } else {
    ...
    /**
    * @dev Set withdrawal delay time
    */
    function setWithdrawalDelayTime(uint256 _time) external onlyOwner {
        require(_time > 0, "INVALID_TIME");
        WITHDRAWAL_DELAY_TIME = _time;
    }

    /**
    * @dev Get vault risk
    */
    function getVaultRiskBips() internal view returns (uint256) {
        return (watermark - totalDepositedAmount) * config.FEE_BASE() / totalDepositedAmount;
    }

    /**
    * @dev Get max hourly vault exposure based on current risk. if current risk is high, hourly vault exposure should be decreased.
    */
    function getMaxHourlyExposure() external view returns(uint256) {
        ...
        uint256 exposureBips = _maxHourlyExposureBips * (_maxVaultRiskBips - currentRiskBips) / _maxVaultRiskBips;

        return exposureBips * totalDepositedAmount / config.FEE_BASE();
    }
}
...
interface IBinaryVault {
    ...
    function increaseDepositedAmount(uint256 amount) external;

    function decreaseDepositedAmount(uint256 amount) external;

    function totalDepositedAmount() external view returns(uint256);

    function getMaxHourlyExposure() external view returns(uint256);

    function getCurrentPendingWithdrawalAmount() external view returns(uint256 shareAmount, uint256 underlyingTokenAmount);
