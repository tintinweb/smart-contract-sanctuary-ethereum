...
import {IOptimisticBBAUSDValue} from "./interfaces/IOptimisticBBAUSDValue.sol";
import {AggregatorV3Interface} from "./interfaces/AggregatorV3Interface.sol";

/// @title OptimisticBBAUSDOracle
/// @notice Implementation of the OptimisticOracle for bb-a-usd principal token.
/// The oracle uses the chainlink feeds of USDC, DAI and USDT to fetch prices and
...
contract OptimisticBBAUSDOracle is OptimisticOracle, IOptimisticBBAUSDValue {
    /// ======== Custom Errors ======== ///

    error OptimisticBBAUSDOracle__fetchLatestValue_invalidTimestamp();
    error OptimisticBBAUSDOracle__encodeNonce_staleProposal();
    error OptimisticBBAUSDOracle__encodeNonce_activeDisputeWindow();
    error OptimisticBBAUSDOracle__push_inactiveRateId();
    error OptimisticBBAUSDOracle__encodeNonce_invalidTimestamp();

    /// ======== Storage ======== ///

    uint256 public constant TOKEN_DECIMALS_SCALE = 1e8;

    // @notice Chainlink Feeds
    address public immutable usdcAggregatorFeed;
    address public immutable daiAggregatorFeed;
    address public immutable usdtAggregatorFeed;
    ...
    /// @param usdcAggregatorFeed_ Address of the USDC chainlink aggregator feed
    /// @param daiAggregatorFeed_ Address of the DAI chainlink aggregator feed
    /// @param usdtAggregatorFeed_ Address of the USDT chainlink aggregator feed
    constructor(
        address target,
        bytes32 oracleType,
        IERC20 bondToken,
        ...
        address usdcAggregatorFeed_,
        address daiAggregatorFeed_,
        address usdtAggregatorFeed_
    ) OptimisticOracle(target, oracleType, bondToken, bondSize, disputeWindow) {
        usdcAggregatorFeed = usdcAggregatorFeed_;
        daiAggregatorFeed = daiAggregatorFeed_;
        usdtAggregatorFeed = usdtAggregatorFeed_;
    }

    /// ======== Chainlink Oracle Implementation ======== ///

    /// @notice Retrieves the latest spot prices for USDC, DAI and USDT from Chainlink
    ...
    /// @return data Latest round ids and round timestamps for the USDC, DAI and
    /// USDT feeds[uint80,uint64,uint80,uint64,uint80,uint64]
    function value()
        public
        view
        override
        returns (uint256 value_, bytes memory data)
    {
        ...
        ) = _fetchLatestValue(usdcAggregatorFeed);
        (
            uint256 daiValue,
            uint80 daiRoundId,
            uint64 daiTimestamp
        ) = _fetchLatestValue(daiAggregatorFeed);
        (
            uint256 usdtValue,
            uint80 usdtRoundId,
            uint64 usdtTimestamp
        ) = _fetchLatestValue(usdtAggregatorFeed);

        value_ = min(usdcValue, min(daiValue, usdtValue));
        ...
        )
    {
        ...
        if (data.length != 192) {
            result = uint256(ValidateResult.InvalidDataLength);
        } else {
            (
                uint256 usdcRoundId,
                ...
                uint256 usdtRoundId,
                uint256 usdtTimestamp
            ) = abi.decode(
                    data,
                    (uint256, uint256, uint256, uint256, uint256, uint256)
                );

            // validate the usdc chainlink round
            ...
            // validate the dai chainlink round, skip if validation failed previously
            if (validValue != 0) {
                validValue = min(
                    validValue,
                    _fetchAndValidate(
            ...
            // validate the usdt chainlink round, skip if validation failed previously
            if (validValue != 0) {
                validValue = min(
                    validValue,
                    _fetchAndValidate(
        ...
        )
    {
        ...
        if (roundTimestamp > type(uint64).max)
            revert OptimisticBBAUSDOracle__fetchLatestValue_invalidTimestamp();
        ...
        unchecked {
            // scale to WAD
            value_ = wdiv(uint256(feedValue), TOKEN_DECIMALS_SCALE);
        }
    }
    ...
    function push(bytes32 rateId) public override(OptimisticOracle) {
        if (!activeRateIds[rateId])
            revert OptimisticBBAUSDOracle__push_inactiveRateId();

        // fetch the latest value from the Chainlink Aggregators
        (uint256 usdcValue, , uint64 usdcTimestamp) = _fetchLatestValue(
            usdcAggregatorFeed
        );
        (uint256 daiValue, , uint64 daiTimestamp) = _fetchLatestValue(
            daiAggregatorFeed
        );
        (uint256 usdtValue, , uint64 usdtTimestamp) = _fetchLatestValue(
            usdtAggregatorFeed
        );

        // compute the min value
        uint256 value_ = min(usdcValue, min(daiValue, usdtValue));
        // compute the min round timestamp
        uint64 minTimestamp = uint64(
            min(usdcTimestamp, min(daiTimestamp, usdtTimestamp))
        );
    ...
    /// @param data Encoded round ids and round timestamps for usdc, dai
    /// and usdt chainlink rounds [uint80,uint64,uint80,uint64,uint80,uint64]
    ...
    {
        // decode the timestamp of each round
        (
            ,
            uint64 usdcTimestamp,
            ,
        ...
        if (prevNonce != 0) {
            ...
            if (prevTimestamp >= uint64(minTimestamp)) {
                revert OptimisticBBAUSDOracle__encodeNonce_staleProposal();
            }

            // revert if prev. proposal is still within `disputeWindow`
            if (sub(block.timestamp, prevProposeTimestamp) <= disputeWindow) {
                revert OptimisticBBAUSDOracle__encodeNonce_activeDisputeWindow();
            }
        }
...
interface IOptimisticBBAUSDValue {
