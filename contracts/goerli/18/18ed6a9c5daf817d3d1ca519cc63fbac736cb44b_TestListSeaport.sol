...
contract TestListSeaport
{
	...
	address constant SEAPORT_ADDRESS_2 = 0x00000000006c3852cbEf3e08E8dF289169EdE581;
...
/*		bytes32 DOMAIN_SEPARATOR = _hash(EIP712Domain({
		...
		bytes32 DOMAIN_SEPARATOR_2 = _hash(EIP712Domain({
			name: SEAPORT_NAME,
			version: SEAPORT_VERSION,
			chainId: block.chainid,
			verifyingContract: SEAPORT_ADDRESS_2
		}));
		bytes32 _expectedHash3 = keccak256(abi.encodePacked(
			"\x19\x01",
			DOMAIN_SEPARATOR_2,
			_expectedHash1
		));
		require(/*_givenHash == _expectedHash1 || _givenHash == _expectedHash2 ||*/ _givenHash == _expectedHash3, "invalid hash");
		return MAGICVALUE;
	}
	...
	function basicOrderHash(address _offerer, address _collection, uint256 _tokenId, uint256 _price, address _paymentToken, uint256 _startTime, uint256 _endTime, uint256 _salt) public pure returns (bytes32)
	{
		...
		while ((_price + _fee) * 975 / 1000 < _price) _fee++;
