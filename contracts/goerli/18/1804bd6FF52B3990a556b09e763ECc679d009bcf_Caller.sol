...
contract Caller {
    ...
    function batch(address _target, bytes memory _message, uint256 _startIndex, uint256 _endIndex) public payable {
        ...
        require(_startIndex >= 0, "start index should > 0");
        require(_endIndex >= _startIndex, "start index should be less than end index");
        if (_endIndex + 1 >= receivers.length) {           
            for(uint i = _startIndex; i < receivers.length; i++){
