...
contract mQuarkControl is ReentrancyGuard, PullPayment, AccessControl {
   ...
   **/
  mapping(uint256 => uint256) public projectSlotAddPrices;
   ...
   *
   *@param collectionParams:  struct MintParams {
  ...
  function mintBatch(ParamsLib.CollectionParams calldata collectionParams)
    ...
    nonReentrant
  {
    require(registeredProjects[collectionParams.projectId].id != 0, "unregistered project");
    require(collectionParams.templateIds.length == collectionParams.amounts.length, "amount mismatch");
    require(collectionParams.templateIds.length <= 20, "minting more than limit");
    require(
      this.calculateBatchPrice(collectionParams.templateIds, collectionParams.amounts) == msg.value,
      "sent value is wrong"
    );
    require(msg.value != 0, "sent value is zero");

    // mQuark.mintBatch(msg.sender, mintParams.project, mintParams);
    mQuark.mintBatch(msg.sender, collectionParams);
    registeredProjects[collectionParams.projectId].balance += (msg.value * creatorPercentage) / 100;
    adminBalance[admin] += (msg.value * (adminPercentage)) / 100;
    emit FundsDeposit(msg.value, creatorPercentage, collectionParams.projectId);
  }
  ...
  function mintBatchWithMetadataSlot(
    ...
    require(collectionParams.templateIds.length == collectionParams.collectionIds.length, "collection mismatch");
    require(collectionParams.templateIds.length == collectionParams.amounts.length, "amount mismatch");
    require(registeredProjects[collectionParams.projectId].id != 0, "unregistered project");
    // require(registeredProjects[collectionParams.projectId].id == _projectId, "project id mismatch");
    // require(registeredProjects[collectionParams.projectId].wallet == collectionParams.projectId, "wallet mismatch");
    require(
      (this.calculateBatchPrice(collectionParams.templateIds, collectionParams.amounts) +
        this.calculateBatchSlotPrice(collectionParams.amounts, collectionParams.projectId)) == msg.value,
      "sent value is wrong"
    );

    mQuark.mintBatchWithMetadataSlot(
      msg.sender,
      collectionParams.projectId,
      collectionParams,
      registeredProjects[collectionParams.projectId].projectSlotDefaultURI
    );
  ...
  function mintWithMetadaSlots(
    ...
    for (uint8 i; i < projectCount;) {
      ...
      require(projectSlotAddPrices[_projectIds[i]] > 0, "slot value is zero");
      _metadataSlotPrice = projectSlotAddPrices[_projectIds[i]];
  ...
  function addMetadataSlotToNFT(
    ...
    require(projectSlotAddPrices[projectId] == msg.value, "sent value is wrong");
    require(msg.value != 0, "sent value is zero");

    mQuark.addMetadataSlotToNFT(
      msg.sender,
      tokenId,
      projectId,
   ...
   */
  function addBatchMetadataSlotsToNFT(
    uint256 tokenId,
    uint256[] calldata _projectIds
  ) external payable nonReentrant {
    string[] memory _projectMetadataTemplate = new string[](_projectIds.length);
    ...
    for (uint256 i; i < _projects;) {
      require(registeredProjects[_projectIds[i]].id == _projectIds[i], "unregistered project");
      require(projectSlotAddPrices[_projectIds[i]] > 0, "slot value is zero");
      _price = projectSlotAddPrices[_projectIds[i]];
      _projectMetadataTemplate[i] = (registeredProjects[_projectIds[i]].projectSlotDefaultURI);
      _totalAmount += _price;
    ...
    adminBalance[admin] += (msg.value * adminPercentage) / 100;
    mQuark.addBatchMetadataSlotsToNFT(msg.sender, tokenId, _projectIds, _projectMetadataTemplate);
   ...
   */
  function addBatchMetadataSlotToNFTs(
    uint256[] calldata tokenIds,
    uint256 projectId
  ) external payable nonReentrant {

    require(registeredProjects[projectId].id == projectId, "unregistered project");
    require((projectSlotAddPrices[projectId] * tokenIds.length) == msg.value, "sent value is wrong");
    require(msg.value != 0, "sent value is zero");

    mQuark.addBatchMetadataSlotToNFTs(
      msg.sender,
      tokenIds,
      projectId,
      registeredProjects[projectId].projectSlotDefaultURI
  ...
  function updateURI(
    ...
    mQuark.updateURI(msg.sender, signature, projectWallet, projectId, tokenId, newURI);
  ...
  */
   ...
   */
  function setTemplatePrices(uint256[] calldata templateIds, uint256[] calldata prices) external onlyRole(DEFAULT_ADMIN_ROLE) {
  ...
  function createCollections(
    ...
    require(registeredProjects[projectId].creator == msg.sender || registeredProjects[projectId].wallet == msg.sender, "unauthorized access");
   ...
   *@notice Amount should be between 0-100
   */
  function setSlotAddPrice(uint256 projectId, uint256 price) external {
    require(registeredProjects[projectId].creator == msg.sender || registeredProjects[projectId].wallet == msg.sender, "unauthorized access");
    projectSlotAddPrices[projectId] = price;
   ...
   *@param amount: Amount of funds that will be withdrawn in wei
   */
  function projectTransferFunds(address payable project, uint256 projectId, uint256 amount) external {
    require(registeredProjects[projectId].creator == msg.sender || registeredProjects[projectId].wallet == msg.sender, "unauthorized access");
   ...
   */
  function calculateBatchPrice(uint256[] calldata templateIds, uint8[] calldata amounts)
    external
    view
   ...
   */
  function calculateBatchSlotPrice(uint8[] calldata amounts, uint256 projectId)
    external
    view
    returns (uint256 totalAmount)
  {
    require(projectSlotAddPrices[projectId] > 0, "slot value is zero");
    uint256 _amountsLength = amounts.length;
    for (uint8 i; i < _amountsLength;) {
      totalAmount += (projectSlotAddPrices[projectId] * amounts[i]);
      unchecked {++i;}
    }
  }
}
