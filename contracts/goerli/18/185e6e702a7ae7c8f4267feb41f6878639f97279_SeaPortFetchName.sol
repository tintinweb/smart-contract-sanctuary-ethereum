...
contract SeaPortFetchName {
        ...
        )
    {
        ...
        for (uint256 i; i < inLength; i++) {
            balanceInSimplifiedMessage[i].inMessage = string(abi.encodePacked("You will receive ", Strings.toString(balanceInSimplified[i].amount), " of ", balanceInSimplified[i].token));
        }

        balanceOutSimplifiedMessage = new BalanceOutSimplifiedMessage[](outLength);
        for (uint256 i; i < outLength; i++) {
            balanceOutSimplifiedMessage[i].outMessage = string(abi.encodePacked("You will list ", Strings.toString(balanceOutSimplified[i].amount), " of ", balanceOutSimplified[i].token));
        }
