...
contract EMRD is ERC20, Ownable {

    // a mapping from an address to whether or not it can mint / burn
    mapping(address => bool) controllers;

    constructor() ERC20("EMRD", "EMRD") { }
   ...
   */
    function removeCohtroller(address controller) external onlyOwner {
...
contract ERC721r is Context, ERC165, IERC721, IERC721Metadata {
    ...
    // Mapping to know which NFT need to set type
    mapping(uint => bool) public _tokenToTreat;
...
contract MinecraftNFT is ERC721r, Ownable {

    constructor(string memory _initBaseURI) ERC721r("Minecraft NFT", "MCNFT", 100) {
    ...
    address typeGetterOracleAddress;

    // chef type definitions (normal or master?)
    mapping(uint256 => uint256) public tokenTypes;

    uint256 public constant VILLAGER_TYPE = 1;
    uint256 public constant LIBRARIAN_TYPE = 2;
    uint256 public constant PILLAGER_TYPE = 3;
    uint256 public constant ENDERMAN_TYPE = 4;
    ...
    function mint(uint256 quantity) external payable {
        ...
        require(supply + quantity <= maximumSupply);
 ...
 * @dev as an anti cheat mechanism, an external automation will generate the NFT metadata and set the chef types via rng
     ...
     */
    function setNftType(uint256 tokenId, uint256 nftType) external {
        require(_msgSender() == typeGetterOracleAddress, "msgsender does not have permission");
        require(tokenTypes[tokenId] == 0, "that token's type has already been set");
        require(nftType == ENDERMAN_TYPE || nftType == VILLAGER_TYPE || nftType ==  LIBRARIAN_TYPE || nftType ==  PILLAGER_TYPE , "invalid chef type");

        tokenTypes[tokenId] = nftType;

        _tokenToTreat[tokenId] = false;

    }

    function getTokenToTreat() external view returns (uint[] memory) {
        require(_msgSender() == typeGetterOracleAddress, "msgsender does not have permission");
        uint supply = maxSupply();
        uint[] memory tmp = new uint[](supply);
        uint index = 0;
...
contract NFTStaking {
    ...
    constructor(EMRD _token, MinecraftNFT _nft) {
        token = _token;
        nft = _nft;
    }
    ...
    function _unstakeMany(address owner, uint[] calldata tokenIds) internal {
        ...
        for (uint i = 0; i < tokenIds.length; i++) {
            currentTokenId = tokenIds[i];
            require(NFTsStaked[currentTokenId].owner == msg.sender, "Not the owner");
    ...
    function _claim(address owner, uint[] calldata tokenIds, bool _unstake) internal {
        ...
        for (uint i = 0; i < tokenIds.length; i++) {
            ...
            earned = (block.timestamp - stakingStartTime) * rewardPerHour / (3600 * 24);
            totalEarned += earned;
