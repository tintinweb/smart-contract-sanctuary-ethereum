...
contract XLARevenueShareContract is Ownable {
    ...
    function _redistributeEth() internal {
        ...
        for (uint256 i = 0; i < recipientsLength;) {
            ...
            if (msg.value > 0) {
                ...
                (bool success, bytes memory data) = payable(recipient).call{value: amountToReceive}("");
     ...
     * @return valid boolean indicating whether sum of percentage is <= 100
     */
     ...
     */
    function addRecipient(address payable _recipient, uint256 _percentage) public onlyOwner {
     ...
     */
    function updateRecipient(address payable _recipient, uint256 _percentage) public onlyOwner {
     ...
     * @param _recipient Address to be removed
     */
    function removeRecipient(address payable _recipient) public onlyOwner {
        require(recipientsPercentage[_recipient] != 0, "Recipient doesnt exists");
        recipientsPercentage[_recipient] = 0;
     ...
     */
    function setRecipients(address[] calldata _newRecipients, uint256[] calldata _percentages) external onlyOwner {
        uint256 newRecipientsLength = _newRecipients.length;
        uint256 currentRecipientsLength = recipients.length;

        require(newRecipientsLength == _percentages.length, "Recipients length does not much percentages");

        for (uint256 i = 0; i < currentRecipientsLength;) {
            removeRecipient(recipients[0]);
            unchecked{i++;}
        }

        for (uint256 x = 0; x < newRecipientsLength;) {
            addRecipient(payable(_newRecipients[x]), _percentages[x]);
            unchecked{x++;}
        }
