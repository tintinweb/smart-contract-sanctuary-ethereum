...
# Modified from: https://github.com/vyperlang/vyper/blob/a0215992d28229cf79bd0c55a162a2b0f2af2a0a/examples/tokens/ERC721.vy
...
def mint(_to: address) -> bool:
    currentTokenIdTracker : uint256 = self.getTokenIdTracker()
    assert currentTokenIdTracker <= self.totalSupply
...
def tokenURI(tokenId: uint256) -> String[335]:
    assert tokenId < self.totalSupply, "ERC721: URI query for nonexistent token"
    assert tokenId < self.tokenIdTracker, "ERC721: URI query for nonexistent token"
