...
contract Presale01 is ReentrancyGuard, Proxiable{
    ...
    struct BuyerInfo {
        ...
        uint256[] listBaseDeposit; // storate list base each other buy sale token
        uint256 countNumberPurchase; // is count number purchase token
        uint256 countNumberClaim; // is count number claim token
        bool isClaim; // storate status claim each other buy sale token
        uint256 valueClaimAfterPurchase; // value cliam after purchase first
    }
      ...
      rejectDoubleMessage(_message)
    {
        ...
        buyer.listBaseDeposit.push(_amount);
        buyer.isClaim = true;
    ...
    function userClaimSaleTokens() external nonReentrant {
        ...
        uint256 tokensOwed;
        uint256 amountNotYetClaim;
        
        if(buyer.isClaim && currentTime < LIST_VESTING_PERIOD[1].distributionTime) {
            tokensOwed = (buyer.listBaseDeposit[(buyer.listBaseDeposit.length).sub(1)])
                         .mul(PRESALE_INFO.TOKEN_PRICE)
                         .div((10**uint256(PRESALE_INFO.B_TOKEN.decimals())));
            rateWithdrawAfter = LIST_VESTING_PERIOD[0].unlockRate;
            buyer.valueClaimAfterPurchase += (tokensOwed * rateWithdrawAfter) / 100;
            buyer.countNumberClaim++;
        } else {
            uint256 index = (buyer.countNumberPurchase).sub(buyer.countNumberClaim);
            if(index > 0) {
                for(uint256 i = 0; i < index; i++) {
                    amountNotYetClaim += (buyer.listBaseDeposit[(buyer.listBaseDeposit.length -1).sub(i)])
                        .mul(PRESALE_INFO.TOKEN_PRICE)
                        .div((10**uint256(PRESALE_INFO.B_TOKEN.decimals())));
                }
                amountNotYetClaim = (amountNotYetClaim.mul(LIST_VESTING_PERIOD[0].unlockRate)).div(100);       
            }
            
            tokensOwed += buyer.tokensOwed;
        ...
        require(
            rateWithdrawAfter > 0,
            "USER WITHDRAW ALL TOKEN SUCCESS OR IT'S NOT TIME TO CLIAM TOKEN YET."
        );

        buyer.lastWithdraw = currentTime;
        uint256 amountWithdraw = (tokensOwed * rateWithdrawAfter) / 100 + amountNotYetClaim; 
    ...
    function getStatusClaimSaleToken(address _buyerAddress) public view returns(bool) {
        BuyerInfo storage buyer = BUYERS[_buyerAddress];
        uint256 rateWithdrawAfter;
        for(uint i = 0 ; i < LIST_VESTING_PERIOD.length ; i++) {
            if(block.timestamp >= LIST_VESTING_PERIOD[i].distributionTime &&
            buyer.lastWithdraw < LIST_VESTING_PERIOD[i].distributionTime
            ){
                rateWithdrawAfter += LIST_VESTING_PERIOD[i].unlockRate;
            }
        ...
        if(buyer.isClaim && block.timestamp < LIST_VESTING_PERIOD[1].distributionTime || rateWithdrawAfter > 0) {
            return true;
        }
