...
contract BlueCheckmark is ERC721A, Ownable {
  ...
  // ($20 @ $1150 ETH/USD) / 30 days per month = 20 / 1150 / 30 = 0.0005797 ETH per day
  uint256 public pricePerDay = 0.0005797 ether;
  ...
  function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
    ...
    if(!_exists(tokenId)) {
      ...
      uint numDaysRemaining = isExpired(tokenId) ? 1 : ((tokenExpirationDates[tokenId] - block.timestamp) / (24 * 60 * 60)) + 1;
      svg = string(
        abi.encodePacked(
          'data:application/svg+xml;base64,',
          Base64.encode(
  ...
  function threatenToBeGoneLikeEnron() external returns (string memory) {
    ...
    // ($8 @ $1150 ETH/USD) / 30 days per month = 8 / 1150 / 30 = 0.0002319 ETH per day
    pricePerDay = 0.0002319 ether;
    return "How about $8?";
  }
