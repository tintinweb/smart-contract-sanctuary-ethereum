...
///         their NFT and executing either the sell function. After an NFT has been sold to this contract
...
contract TrashBin is Ownable, IERC721Receiver, IERC1155Receiver, ReentrancyGuard {
    ...
    ///         if it is an ERC1155), and the block that the NFT was sold so to the contract.
    struct nft {
        address collection;
        uint256 id;
        bool isERC721;
    ...
    /// @notice The price, in wei, any user will recieve for selling an NFT to this contract.
    uint256 public sellPrice;

    /// @notice The multiplier discount, in basis points, that a standard hashes NFT holder will recieve
    ///         when purchasing an NFT from this contract.
    uint256 public standardHashDiscount;

    /// @notice The multiplier discount, in basis points, that a DAO hashes NFT holder will recieve when
    ...
    /// @notice The precentage, in basis points, of profits retained by the owner multi-sig.
    uint256 public ethPercentage;
    ...
    constructor(IHashes _hashes, IHashesDAO _hashesDAO, address _owner) {
        ...
        sellPrice = 100000000 wei;
        standardHashDiscount = 7500;
        daoHashDiscount = 2500;
        buyableDelay = 20000;
        minEthBalance = 0.01e18 wei;
        ethPercentage = 2000;
    }
    ...
    /// @notice This function allows anyone to purchase multiple NFTs that has been sold to this contract.
    ...
    /// @param hashId  An array that allows the purchaser to enter a Hashes NFT ID that they own in order to recieve
    ...
    ///                purchaser the function will not succeed. DAO Hashes NFT owners will recieve the DAO hash
    ///                discount while standard Hashes NFT owners will recieve the standard hash discount.
    function buy(uint256[] memory indexes, uint256[] memory hashId) external payable nonReentrant {
        require(
        ...
        //deletes all of the storage entries
    ...
    /// @notice This function allows anyone to sell any number of NFTs (either ERC721 or ERC1155) to this contract. Each NFT
    ...
    /// @param amounts    An array of the amounts of each token Id to be sold. When the NFT is an ERC721 the ammount is
    ///                   inconsequential becuase each token Id only has one token associated with it. For ERC1155s however,
    ...
    ///         Hashes DAO and owner multi-sig. The owner multi-sig will recieve their percentage of the revenue 
    ...
    function withdrawETH() external nonReentrant {
        require(
            address(this).balance > minEthBalance,
            "TrashBin: contract balance is less than minimum balance amount."
        );

        uint256 totalWithdrawAmount = address(this).balance - minEthBalance;

        uint256 ownerFee = (totalWithdrawAmount * ethPercentage) / 10000;
    ...
    function removeNFT(uint256 index, uint256 hashId) external nonReentrant {
    ...
    /// @notice The function allows the owner multi-sig to delete an Index from the nftStorage array. Ideally, this 
    ///         function will never be needed, but in the event that an NFT is incorrectly withdrawn and a nftStorage
    ///         array entry persists this function can be used to remove the unnecessary index.
    /// @param index The index to be deleted.
    function deleteIndex(uint256 index) external onlyOwner {
        require(index < nftStorage.length, "TrashBin: index out of bounds.");
    ...
    function _sellERC721(address collection, uint256 tokenId) internal nonReentrant {
    ...
    function _sellERC1155(address collection, uint256 tokenId) internal nonReentrant {
    ...
    /// @param newEthPercentage        The updated ETH percentage of revenue that is distributed to the owner multi-sig (in basis points).
    function updateAllSettings(
        ...
        _updateEthPercentage(newEthPercentage);
    }
    ...
    /// @notice This function allows the owner to change the percentage of ETH revenue they recieve when ETH is withdrawn
    ///         from the contract.
    /// @param newEthPercentage The updated owner percentage variable in basis points.
    function updateEthPercentage(uint256 newEthPercentage) external onlyOwner {
        _updateEthPercentage(newEthPercentage);
    }
    ...
    function _updateEthPercentage(uint256 newEthPercentage) internal {
        require(
            newEthPercentage <= 10000,
