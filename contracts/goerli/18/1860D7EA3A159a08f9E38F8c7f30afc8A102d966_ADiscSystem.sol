...
contract ADiscSystem is ERC721, ReentrancyGuard, Ownable {
  ...
  constructor(string memory baseTokenURI) ERC721("A Disc System", "DISC") {
  ...
  {
    require(_exists(tokenId), "ADiscSystem: URI query for nonexistent token");
    return
      string(
        bytes.concat(
          bytes(_baseTokenURI),
  ...
  function setImage(uint256 tokenId, string memory image) external {
    require(
      ...
      "ADiscSystem: caller is not token owner"
    );
  ...
  function mintAndTransfer(address to, uint256 quantity) internal {
    ...
    require(
      ...
      "ADiscSystem: Sold out or invalid amount"
    );
  ...
  function mint(uint256 quantity) external payable nonReentrant {
    require(isOnSale, "ADiscSystem: Not on sale");
    require(msg.value == PRICE * quantity, "ADiscSystem: Invalid value");
  ...
  function setIsMetadataFrozen(bool _isMetadataFrozen) external onlyOwner {
    require(
      !isMetadataFrozen,
      "ADiscSystem: isMetadataFrozen cannot be changed"
    );
    isMetadataFrozen = _isMetadataFrozen;
  }

  function setBaseTokenURI(string memory baseTokenURI) external onlyOwner {
    require(!isMetadataFrozen, "ADiscSystem: Metadata is already frozen");
    _baseTokenURI = baseTokenURI;
  }
