...
contract StitchedETHv2 is ERC2981, ERC721, Ownable {
  ...
  */
  ...
  function listChains() external view returns(string[] memory) {
    string[] memory chs = new string[](_nextSubjectWorkSeriesId - 1);
  ...
  function tokenURI(uint256 tokenId) public view virtual override(ERC721) returns (string memory) {
      ...
      string memory description = string.concat(
        string.concat(
          ...
          ") and the complete set of authorized derivative works. The owner of each of the NFTs in this series is granted a limited exclusive copyright license to produce clothing featuring this artwork, subject to the Non-fungible Token Agreement, limited to the Current Allocation of clothing applications. REVIEW THIS LICENSE CAREFULLY prior to acquiring the NFT.\\n\\n The use of the copyright embedded in this NFT is subject to other important limitations, terms, and conditions of this [Non-fungible Token Agreement]("
          ),
          string.concat(
          IPFS_GATEWAY,
          sw.details.copyrightLicenseAgreementIPFS,
          "). Acquiring or minting this NFT or attempting to invoke any of the rights granted by the exclusive license contained in the metadata constitutes the NFT owner's electronic signature and consent to this agreement.\\n\\n The number of fashion articles authorized by the license agreement increases each year by the Yearly Allocation amount. The Total Allocation number reflects the number of garments that the NFT has been allocated since the mint date. When the NFT holder produces garments, they are reported on-chain and the Number Produced increases and is subtracted from the Current Allocation. The Current Allocation reflects the remaining balance of the number of garments that the current holder may produce.\\n\\n You can find more information about the production history, including records you can use for garment authentication on the [Subject Work Page](https://stitched.eth.limo/",
