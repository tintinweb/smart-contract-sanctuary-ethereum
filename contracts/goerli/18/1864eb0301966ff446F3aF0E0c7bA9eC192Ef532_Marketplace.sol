...
contract Marketplace is ReentrancyGuard, Ownable {
    ...
    function isOwned(address _contractAddress, uint _tokenId) public view returns (bool) {
        ...
        if (isERC721(_contractAddress)) {
            return IERC721(_contractAddress).ownerOf(_tokenId) == address(msg.sender);
        } else if (isERC1155(_contractAddress)) {
            return IERC1155(_contractAddress).balanceOf(address(msg.sender), _tokenId) > 0;
        } else {
            return false;
        }
    }

    function checkMsgSenderAddress() public view returns(address) {
        return msg.sender;
    }
