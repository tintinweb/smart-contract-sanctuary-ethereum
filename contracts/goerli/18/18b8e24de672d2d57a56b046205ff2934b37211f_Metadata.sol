...
contract ERC4907Rent is ERC4907 {
...
contract Metadata is ERC721A, Ownable, ReentrancyGuard {
    using Strings for uint256;
    uint256 public cost = 50000000000000000;
    uint256 public ref = 20;
    uint256 public rent_fee = 10;
    ...
    uint public rent_id;
    mapping(uint => mapping(address => string)) public rents;
    ...
    function setRent(string calldata seller) public payable {
            address sellerAddress = ownerOf(tokenAddressandID[seller]);
            payable(sellerAddress).transfer(msg.value*(100-rent_fee)/100);
            uint newLast=rent_id+1;
            rents[newLast][msg.sender]=string.concat("?",seller,Strings.toString(block.timestamp),Strings.toString(msg.value));
    }

    function getRent(uint256 count)    public view
    returns (string[] memory)
  {
    uint256 total = rent_id;
    ...
    function setRefSettings(uint ref_) external onlyOwner {
        ref = ref_;
    }
