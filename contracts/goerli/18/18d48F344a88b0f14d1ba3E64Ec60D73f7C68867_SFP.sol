...
contract SFP is Initializable, ERC721Upgradeable, ERC721URIStorageUpgradeable, PausableUpgradeable, OwnableUpgradeable, ERC721BurnableUpgradeable, UUPSUpgradeable {
    ...
    mapping(uint256 => countryData) private countryInfo;
    ...
    event NftSale(uint256[] countryId,uint256[] tokenId,address buyer,uint256 quantity,uint256 totalPrice);
    ...
    function initialize(string memory _name, string memory _symbol,address _paymentWallet) initializer public {
        ...
        setAdmin(_msgSender());
        nftPrice_ = 10000000000000000;
        paymentWallet = _paymentWallet;
        collectionSize = 6400;
        countryCount = 0;

        for(uint256 j=1;j<33;j++){
            countryInfo[j] = countryData ({
                countryId: j,
                countryTokenIndex:(j-1)* 200
            });
        }
        totalMinted = 0;
        walletLimit = 10;
    ...
    function setWalletLimitAndMaxLimitPerMint(uint256 _nftLimit,uint256 _maxLimitPerMint) public onlyOwnerAndAdmin{
        walletLimit = _nftLimit;
        maxLimitPerMint = _maxLimitPerMint;
    }

    function setNftPrice(uint256 _priceInWei) public onlyOwnerAndAdmin{
        nftPrice_ = _priceInWei;
    }
    ...
    function buyNFT(uint256 _quantity)public payable whenNotPaused{
        ...
        require(msg.value >= _quantity*nftPrice_, "Insufficient funds");
        userNftsCount[_msgSender()] += _quantity;
        require(userNftsCount[_msgSender()] <= walletLimit,"Exceeded the max mint limit");
        ...
        for (uint256 i = 0; i < _quantity; i++) {
            ...
            countryInfo[countryCount].countryTokenIndex = countryInfo[countryCount].countryTokenIndex+1;
            tokenId = countryInfo[countryCount].countryTokenIndex;
        ...
        // address payable _paymentWallet= payable(paymentWallet);
        payable(paymentWallet).transfer(msg.value);
        emit NftSale(_countryIds,_nftIds,_msgSender(),_quantity,_quantity*nftPrice_);
    }
