...
contract NFTcontract is ERC721Enumerable, ReentrancyGuard {
    ...
    modifier isCorrectPayment(uint256 stage, uint256 numberOfTokens) {
        uint256 tmpId = publicMintId + numberOfTokens;
        uint256 payAmount = 0;

        if (stage == 0) {
            require(PRE_MINT_PRICES[tmpId] > 0, "Pre-Mint of these tokens not available yet.");
            for (uint256 i = 0; i < numberOfTokens; i++) {
                payAmount += PRE_MINT_PRICES[i];
            }
        } else if (stage == 1) {
            require(PUBLIC_MINT_PRICES[tmpId] > 0, "Mint of these tokens not available yet.");
            for (uint256 i = 0; i < numberOfTokens; i++) {
