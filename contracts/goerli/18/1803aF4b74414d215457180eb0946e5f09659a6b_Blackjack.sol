...
contract Blackjack is ReentrancyGuard {
    ...
    mapping(uint256 => bool) public concludeGame;
    mapping(uint256 => bool) public isEther;
    mapping(uint256 => address) public tokenAddress;
    mapping(uint256 => uint256) private pSeed;
    mapping(uint256 => uint256) private spSeed;
    ...
    event Cards(uint256 betId, uint256 card1, uint256 card2, uint256 dCard);
    event SplitPlayer(uint256 betId, uint256 pCard1, uint256 pCard2, uint256 spCard1, uint256 spCard2);
    event PlayerHit(uint256 betId, uint256 newCard);
    event DealerSecondCard(uint256 betId, uint256 secondCard);
    event DealerHit(uint256 betId, uint256 newCard);
    ...
    modifier playerChecks(uint256 betId) {
        ...
        require(bet.player.hand.length > 0, "Plase wait for card distribution");
        require(!bet.player.bust, 'Player is out of the game');
        require(!concludeGame[betId], 'Game is completed');
        _;
    }
    ...
    function distributeCards(uint256 betId, uint256 seed1, uint256 seed2, uint256 dSeed, uint256 playerSeed, uint256 splitPlayerSeed) public onlyAdmins {
        uint256 card1 = drawCard(betId, seed1);
        uint256 card2 = drawCard(betId, seed2);
        uint256 dealerCard1 = drawCard(betId, dSeed);
        setSeed(betId, playerSeed, splitPlayerSeed);
        Game storage bet = GameId[betId];
        bet.player.hand.push(cardValues[card1]);
        bet.player.hand.push(cardValues[card2]);
        bet.dealer.hand.push(cardValues[dealerCard1]);
        calculate(bet.player);
        checkBlackjack(betId);
        emit Cards(betId, bet.player.hand[0], bet.player.hand[1], bet.dealer.hand[0]);
    }

    function drawCard(uint256 betId, uint256 seed) internal view returns(uint256) {
      uint256 card = (uint256(keccak256(abi.encodePacked(block.timestamp,block.difficulty,block.number,betId,seed))) + 1) % 13 ; 
      return card;
    }

    function setSeed(uint256 betId, uint256 playerSeed, uint256 splitPlayerSeed) internal {
        pSeed[betId] = playerSeed;
        spSeed[betId] = splitPlayerSeed;
    }

    function checkBlackjack(uint256 betId) internal {
        Game storage bet = GameId[betId];
        if(bet.player.score == 21) {
            concludeGame[betId] = true;
            uint256 winAmount = (25 * bet.player.betAmount)/10;
            winningsInEther[bet.player.bettor] += winAmount;
            bet.player.winningAmount = winAmount;
        }
    }
    ...
    function split(uint256 betId) public payable playerChecks(betId) {
        ...
        require(bet.splitPlayer.hand.length == 0, 'can only split once');
        ...
        uint256 pCard2 =  drawCard(betId, pSeed[betId]);
        uint256 spCard2 = drawCard(betId, spSeed[betId]);
        concludeGame[betId] = true;
        ...
        emit SplitPlayer(betId, bet.player.hand[0], bet.player.hand[1], bet.splitPlayer.hand[0], bet.splitPlayer.hand[1]);
    }
    ...
    function doubleDown(uint256 betId, uint256 amount) public payable playerChecks(betId) {
        ...
        require(!bet.player.doubleDown, 'Already doubled down');
        if(isEther[betId]) {
        ...
        concludeGame[betId] = true;
        drawCard(betId, pSeed[betId]);
        bet.player.doubleDown = true;
    }

    function hit(uint256 betId) public playerChecks(betId){
        Game storage bet = GameId[betId];
        uint256 card = drawCard(betId, pSeed[betId]);
        bet.player.hand.push(cardValues[card]);
        calculate(bet.player);
        emit PlayerHit(betId, cardValues[card]);
    }
    ...
    function revealDealerCard(uint256 betId) internal {
        Game storage bet = GameId[betId];
        uint256 dealerCard2 = drawCard(betId, pSeed[betId]);
        bet.dealer.hand.push(cardValues[dealerCard2]);
        calculate(bet.dealer);
        while(bet.dealer.score < 17) {
            dealerHit(betId);
        }
        checkWinner(betId);
        emit DealerSecondCard(betId, cardValues[dealerCard2]);
    }

    function dealerHit(uint256 betId) internal {
        Game storage bet = GameId[betId];
        uint256 card = drawCard(betId, pSeed[betId]);
        bet.dealer.hand.push(cardValues[card]);
        calculate(bet.dealer);
        emit DealerHit(betId, cardValues[card]);
    }
    ...
    function checkWinner(uint256 betId) internal {
        ...
        if(isEther[betId]){
            ...
            winningsInToken[tokenAddress[betId]][bet.player.bettor] += winAmount;
        }
    ...
    function calculatePayOut(uint256 betId, Player storage player) internal {
        Game storage bet = GameId[betId];
        uint256 winAmount;
        if(player.score > bet.dealer.score || bet.dealer.bust) {
             winAmount = 2 * player.betAmount;
            player.winningAmount = winAmount;
    ...
    function setBetLimit(uint256 min, uint256 max) external onlyAdmins {
        minBet = min;
        maxBet = max;
    }
    ...
    function withdrawEtherWinnings(uint256 amount) external nonReentrant {
        ...
        require(ReserveInEther() >= amount,'Contract does not have enough reserve');
    ...
    function withdrawTokenWinnings(address ERC20Address, uint256 amount) external nonReentrant {
        ...
        require(ReserveInToken(ERC20Address) >= amount,'Contract does not have enough reserve');
