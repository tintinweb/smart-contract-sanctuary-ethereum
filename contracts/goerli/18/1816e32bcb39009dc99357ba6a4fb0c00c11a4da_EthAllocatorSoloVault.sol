...
contract EthAllocatorSoloVault is ERC1155Supply, ReentrancyGuard, Ownable {
    // RocketStorageInterface
    RocketStorageInterface constant rocketStorage =
       // mainnet: RocketStorageInterface(0x1d8f8f00cfa6758d7bE78336684788Fb0ee0Fa46);
        RocketStorageInterface(0xd8Cd47263414aFEca62d6e2a3917d6600abDceB3); // goerli

    // Contract name
    string constant name = "Blockscape Solo Staking";

    // Contract symbol
    string constant symbol = "BLCS SOLO";

     // Blockscape Rocket Pool Node Address
    address blockscapeRocketPoolNode =
    0x50ff9aFAf4ED6Bc64b52B8BEfc38c4e5837bc203; // goerli
        // mainnet: 0xF6132f532ABc3902EA2DcaE7f8D7FCCdF7Ba4982;

    // Current inital Withdraw Fee
    uint256 initWithdrawFee = 20 * 1e18;

    // Current Rocketpool Minipool Limit
    uint256 curETHlimit = 16 * 1e18;

    // state of the Solo Vault Pool
    bool allowPubDeposit;

    // initial tokenID - Note: the tokenID is the same as the tokenID of the pool (poolID)
    uint256 tokenID = 0;
    ...
    // Mappings of tokenID to Metadata, Staker, Validator
    ...
    event SoloVaultClosed(bool _value); // event for when a permanent URI is set
    ...
    constructor()
        ERC1155(
            "https://ipfs.blockscape.network/ipns/k51qzi5uqu5divkfa2vxu2i71yhj3k6rm6bgvgnd4q00h4f80cb4imsg9uy29l/{id}.json"
        )
    {}

    // Custom Errors for higher gas efficiency
    error VaultClosed(bool _state);
    ...
    function totalSupply() external view returns (uint256) {
        uint256 amount = 0;
        for (uint256 i = 0; i <= this.getTokenID(); i++) {
    ...
    function openSoloVault() external onlyOwner {
        assembly {
    ...
    function closeSoloVault() external onlyOwner {
        assembly {
            sstore(allowPubDeposit.slot,0)
        }
        emit SoloVaultClosed(allowPubDeposit);
    }

    function openSoloVaultInternal() internal {
         assembly {
            if sload(allowPubDeposit.slot) {
                revert(0, 0)
            }
    ...
    function closeSoloVaultInternal() internal {
        assembly {
            sstore(allowPubDeposit.slot,0)
        }
        emit SoloVaultClosed(allowPubDeposit);
    ...
    function metadataSoloVaultInternal(
        uint256 _stakedETH,
        address _staker,
        uint256 _tokenID
    ) internal {
    ...
    function depositSoloVault() external payable nonReentrant {
        //gas cost: 343598 -> 326118 // 17480 gas saved - 0.86 USD
        assembly {
        ...
        metadataSoloVaultInternal(msg.value, msg.sender, tokenID);
        ...
        // close the vault
        closeSoloVaultInternal();
    }

    function testDepositSoloVault() external payable nonReentrant {
        //gas cost: 343598 -> 326118 // 17480 gas saved - 0.86 USD
        assembly {
            // load the curETHlimit from storage
            let cel := sload(curETHlimit.slot)
    ...
    function updateValidator(uint256 _tokenID, bytes memory _vali)
        ...
        onlyOwner
    {
        if (!compareBytes('', tokenIDtoValidator[_tokenID])) {
            revert ValidatorAlreadySet(tokenIDtoValidator[_tokenID]);
        }
        tokenIDtoValidator[_tokenID] = _vali;
        emit ValidatorSet(_vali);
    }

    function withdrawBatch() external onlyOwner returns (uint256) {
        assembly {
        ...
        payable(owner()).transfer(curETHlimit);
        uint256 availableRPL = getAvailableRPLStake();
        uint256 reqRPL = getReqRPLStake();
        if (availableRPL >= reqRPL) {
            openSoloVaultInternal();
            return (tokenID - 1);
        } else {
    ...
    function setWithdrawFee(uint256 _amount) external onlyOwner {
        initWithdrawFee = _amount * 1e18;
    }

    // func without event - tbd!
    function userRequestWithdraw(uint256 _tokenID)
        external
        returns (uint256 _amount)
    {
        uint256 curWithdrawFee = initWithdrawFee;
        if (tokenIDtoStaker[_tokenID] == msg.sender) {
            uint256 secFee = (initWithdrawFee / 31536000); // 20%
            uint256 timePassed = block.timestamp -
                (tokenIDtoMetadata[_tokenID].stakedTimestamp);
            if (timePassed >= 30747600) {
                curWithdrawFee = 5 * 1e17; // fixed minium fee 0,5%
            } else {
    ...
    function getAvailableRPLStake() public view returns (uint256) {
        ...
        uint256 minimumReqRPL = (minimumRPLStake / minipoolLimit);
...
// gas cost: 326209
// assembly {
//     let allowPubDeposit_ptr := mload(0x40)
//     mstore(allowPubDeposit_ptr, sload(allowPubDeposit.slot))
//     let allowPubDeposit_value := mload(allowPubDeposit_ptr)
//     if iszero(allowPubDeposit_value) {
//         revert(0, 0)
//     }
// }
...
// gas cost: 326197
// assembly {
//     if iszero(localPubDeposit) {
//         revert(0, 0)
//     }
// }

// gas cost: 326226
// bool localPubDeposit = allowPubDeposit;
// assembly {
//     let allowPubDeposit_ptr := mload(0x40)
//     mstore(allowPubDeposit_ptr, localPubDeposit)
//     let allowPubDeposit_value := mload(allowPubDeposit_ptr)
//     if iszero(allowPubDeposit_value) {
//         revert(0, 0)
//     }
...
//gas cost: 343598 with optimizer -> 326192
// if (!allowPubDeposit) {
//     revert VaultClosed(allowPubDeposit);
// }

// if (msg.value != curETHlimit) {
//     revert InvalidDeposit(msg.value);
// }

// assembly {
//     if  lt(callvalue(),sload(curETHlimit.slot)) {
//         revert(0, 0)
//     }
//     if  gt(callvalue(),sload(curETHlimit.slot)) {
//         revert(0, 0)
//     }
// }
