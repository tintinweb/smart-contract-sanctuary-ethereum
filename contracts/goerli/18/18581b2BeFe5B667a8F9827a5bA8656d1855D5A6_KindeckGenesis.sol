    ...
    ERC1155Upgradeable
{
        ...
        payable
    {
        ...
        require(mintPrice * _numberOfTokens == msg.value, "Incorrect ETH value sent" );
    ...
    */
    ...
    function gift(address[] calldata receivers, uint8 _level) external onlyOwner {
        ...
        require(receivers.length < tierLevel[_level].tierSupply - tierLevel[_level].tokensMinted, "Receivers are more than the current supply");

        tierLevel[_level].tokensMinted += receivers.length;
        for (uint256 i = 0; i < receivers.length; i++) {
            _mint(receivers[i], _level, 1, "");
            emit Gift(receivers[i], 1, _level, block.timestamp);
        }
    }
