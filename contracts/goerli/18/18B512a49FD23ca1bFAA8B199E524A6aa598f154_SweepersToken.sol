...
contract SweepersToken is ISweepersToken, Ownable, ERC721Checkpointable {
    ...
    function migrateMany(uint256[] calldata sweeperIds) external {
        for(uint i = 0; i < sweeperIds.length;) {
            require(sweepersV1.ownerOf(sweeperIds[i]) == msg.sender, 'caller is not owner of sweeper');
    ...
    function migrateManyByOwner() external {
        uint256 length = sweepersV1.balanceOf(msg.sender);
        for(uint i = 0; i < length;) {
            uint256 sweeperId = sweepersV1.tokenOfOwnerByIndex(msg.sender, i);
    ...
    function migrateManyByOwnerDead() external {
        uint256 length = sweepersV1.balanceOf(msg.sender);
        for(uint i = 0; i < length;) {
            uint256 sweeperId = sweepersV1.tokenOfOwnerByIndex(msg.sender, i);
            _mintExistingTo(msg.sender, sweeperId);
            sweepersV1.safeTransferFrom(msg.sender, 0x000000000000000000000000000000000000dEaD, sweeperId);
            unchecked{i++;}
        }
    }

    function migrateWithOwnerAndSeed(uint256 sweeperId, address sweeperOwner, ISweepersSeeder.Seed memory seed) external onlyOwner {
        _mintExistingTo(sweeperOwner, sweeperId, seed);
    }

    function migrateManyWithOwnerAndSeed(uint256[] calldata sweeperIds, address[] calldata sweeperOwners, ISweepersSeeder.Seed[] calldata _seeds) external onlyOwner {
        for(uint i = 0; i < sweeperIds.length;) {
            _mintExistingTo(sweeperOwners[i], sweeperIds[i], _seeds[i]);
            unchecked{i++;}
        }
    }
     ...
     * @notice Mint a Sweeper with `sweeperId` to the provided `to` address.
     */
    function _mintExistingTo(address to, uint256 sweeperId, ISweepersSeeder.Seed memory seed) internal {
        seeds[sweeperId] = seed;
