...
contract ShangHaiQingNFT is ERC721A, Ownable, ReentrancyGuard {
    ...
    constructor(string memory initURI, address signer) ERC721A("ShangHai Qing", "SHQ", 1) {
        _uri = initURI;
        maxTotalSupply = 1000;
        maxMint = 1;
        _signer = signer;
    ...
    function mint(string calldata salt, bytes calldata token) external{
        require(status == Status.PublicSale || status == Status.PreSale, "SHQE006");
        require(_verify(_hash(salt, msg.sender), token), "SHQE001");
        require(numberMinted(msg.sender) + maxMint <= maxMint, "SHQE008");
