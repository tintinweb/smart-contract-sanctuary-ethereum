...
contract AppWorks is ERC721Upgradeable, OwnableUpgradeable{
    ...
    modifier checkMintableAmount(uint256 _mintAmount) {
        //Check how many NFTs are available to be minted
        require(totalSupply().add(_mintAmount) <= maxSupply, "No sufficient AWs left, try lower amount");
        //Check user has sufficient funds
        require(
            msg.value == _mintAmount.mul(price),
            "ether value need to match total price, it's 0.01 ether per AW");
        require(_mintAmount.add(addressMintedBalance[_msgSender()]) <= maxSupplyOf(_msgSender()));
        _;
    }
    // Public mint function - week 8
    function mint(uint256 _mintAmount) public payable
    checkMintableAmount(_mintAmount)
    {
        //Please make sure you check the following things:
        //Current state is available for Public Mint
        require(mintActive, "Not available for mint now, try again later");
        

        // 開始 call 底層的 _safeMint
        ...
        /*
        一枚 AW 是 0.01 ether
        我帶著 0.01 ether 來 call 這個 mint function，理論上會全部花在 nft 上面，那 gas fee 誰付啊？
        */
        // 目前 mint 多付的錢，合約會自己收著
    }
    ...
    modifier checkEarlyMint() {
        require(earlyMintActive, 'Inactive Early Mint');
        _;
    }
    modifier checkMerkleProof(bytes32[] calldata _merkleProof) {
        bool proofSuccess = _checkInWhitelist(_merkleProof);
        require(proofSuccess, "You're not in whitelist");
        _;
    }
    function _checkInWhitelist(bytes32[] calldata _merkleProof) private view returns(bool) {
    ...
    function tokenURI(uint256 _tokenId) public view virtual override returns (string memory) {
		_requireMinted(_tokenId); //ERC721Upgradeable
		string memory memoryBaseURI = _baseURI();
        bool baseURIset = bytes(memoryBaseURI).length > 0;

		if (!revealed) {
			return baseURIset ? string(abi.encodePacked(memoryBaseURI, 'blind')) : '';
		}
		return baseURIset ? string(abi.encodePacked(memoryBaseURI, _tokenId.toString())) : '';
	}
}
