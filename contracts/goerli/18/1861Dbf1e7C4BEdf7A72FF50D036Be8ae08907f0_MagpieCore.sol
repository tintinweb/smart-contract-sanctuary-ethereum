...
contract MagpieCore is ReentrancyGuard, Ownable, Pausable, IMagpieCore {
  ...
  function updateAmms(Amm[] calldata _amms) external override onlyOwner {
    require(_amms.length > 0, 'MagpieCore: invalid amms');
    ...
    )
  {
    ...
    if (useTokenBridge) {
      args.swapArgs.getToAssetId().increaseAllowance(config.tokenBridgeAddress, amountOut);
      ...
      args.swapArgs.getToAssetId().increaseAllowance(config.hyphenLiquidityPool, amountOut);
      tokenSequence = 0;
...
library LibSwap {
  ...
  function getToAssetId(IMagpieCore.SwapArgs memory self)
    internal
    pure
    returns (address)
  {
    IMagpieCore.Hop memory hop = self.routes[0].hops[
