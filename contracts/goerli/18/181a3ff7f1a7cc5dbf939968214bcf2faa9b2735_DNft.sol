...
contract DNft is ERC721Enumerable, ReentrancyGuard {
  ...
  event XpIncreased (uint indexed id, uint totalXp);
  ...
  function sync(uint id) external isActive(id) returns (int) {
      ...
      _addXp(nft, id, _calcXpReward(XP_SYNC_REWARD + priceChangeAbs));
      idToNft[id]      = nft;
      emit Synced(id);
      return dyadDelta;
  }
  ...
  function snipe(
      ...
      if (prevDyadDelta > 0) {         
        ...
        _addXp(to, _to, _calcXpReward(XP_SNIPE_MINT_REWARD));
      } else {                        
        uint xp;  
        (share, xp)   = _calcNftBurn(prevDyadDelta, from);
        from.deposit += share;      
        _addXp(from, _from, xp);
        _addXp(to, _to, _calcXpReward(XP_SNIPE_BURN_REWARD));
      }
      totalDeposit += share;
      idToNft[_from] = from;
      idToNft[_to]   = to;
  ...
  function liquidate(
      ...
      _addXp(nft, id, _calcXpReward(XP_LIQUIDATION_REWARD));
      nft.deposit  += newDyad;
      totalDeposit += newDyad;
      idToNft[id]   = nft;
  ...
  function _addXp(Nft memory nft, uint id, uint xp) private {
      nft.xp  += xp;
      if (nft.xp > maxXp) { maxXp = nft.xp; } // TODO: is this just to make querying easier? we can eliminate with the graph. same with totalXp i think
      totalXp += xp;
      // TODO: emit this inline wherever _addXp is called, don't need to add a param to this function OR return shit from here. i thought i needed totalXp from in here but i don't, i just need id and xp, which are always present when calling this function
      emit XpIncreased(id, xp);
  }
