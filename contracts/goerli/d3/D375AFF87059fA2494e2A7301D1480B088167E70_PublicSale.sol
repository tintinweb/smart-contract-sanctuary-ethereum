...
contract PublicSale is Validatable {
    using SafeERC20Upgradeable for IERC20Upgradeable;
    using CountersUpgradeable for CountersUpgradeable.Counter;

    /**
     *  @notice genesis is address of genesis NFT
     */
    ...
    /**
     *  @notice startTime is time to start this round
     */
    uint256 public startTime;

    /**
     *  @notice endTIme is time to finish this round
     */
    uint256 public endTime;

    /**
     *  @notice paymentToken is address of payment token to pay
     */
    address public paymentToken;

    /**
     *  @notice price is cost of genesis NFT
     */
    uint256 public price;

    /**
     *  @notice mapping user address => amount of each user bought
     */
    mapping(address => uint256) public boughtAmountOf;

    event SetPrice(uint256 oldValue, uint256 newValue);
    event SetGenesisNFT(IGenesis indexed oldValue, IGenesis indexed newValue);
    event Bought(address indexed user, uint256 times);
     ...
     * @param _genesis Address of genesis contract
     * @param _paymentToken Address of payment token
     * @param _maxPerUser Max of nft that one user can buy
     * @param _startTime Time to start
     * @param _endTime Time to end
    ...
    function initialize(
        ...
        require(admin.isPermittedPaymentToken(_paymentToken), "Invalid payment token");
    ...
    /**
     * @notice Set price for NFT
     * @dev    Only admin can call this function
     * @param _price New price of NFT
     *
     * emit {SetPrice} events
     */
    function setPrice(uint256 _price) external onlyAdmin notZero(_price) {
        uint256 oldValue = price;
        price = _price;
        emit SetPrice(oldValue, price);
    }

    /**
     * @notice Set genesis address
     * @dev    Only admin can call this function
     * @param _genesis Address of genesis
     *
     * emit {SetGenesisNFT} events
     */
    function setGenesisNFT(address _genesis) external onlyAdmin notZeroAddress(_genesis) {
        IGenesis oldValue = genesis;
        genesis = IGenesis(_genesis);
        emit SetGenesisNFT(oldValue, genesis);
    }
    ...
    function setPaymentToken(address _paymentToken) external onlyAdmin {
        require(admin.isPermittedPaymentToken(_paymentToken), "Payment token is not allowed");
        paymentToken = _paymentToken;
        emit SetPaymentToken(_paymentToken);
    }

    /**
     * @notice Set time to start and end
     * @dev    Only admin can call this function
     * @param _startTime Time to start
     * @param _endTime Time to end
     *
    ...
    function setTime(uint256 _startTime, uint256 _endTime) external onlyAdmin {
        require(_startTime > 0 && _startTime < _endTime, "Invalid time");
        startTime = _startTime;
        endTime = _endTime;
        emit SetTime(_startTime, _endTime);
    }
     ...
     */
    function setMaxPerUser(uint256 _maxPerUser) external onlyAdmin notZero(_maxPerUser) {
     ...
     * emit {Bought} events
     */
    function buy(uint256 times) external notZero(times) {
        require(
            (startTime <= block.timestamp && block.timestamp <= endTime) || endTime == 0,
            "Can not buy at this time"
        );
        require(boughtAmountOf[_msgSender()] + times <= maxPerUser, "Limit times each user");

        boughtAmountOf[_msgSender()] += times;
        // Payment
        TransferHelper._transferToken(paymentToken, times * price, _msgSender(), treasury);

        // Mint
        for (uint256 i = 0; i < times; i++) {
            //slither-disable-next-line unused-return
            genesis.mint(_msgSender());
        }

        // Emit events
        emit Bought(_msgSender(), times);
    }
}
