...
import "./Interfaces/ISpellBook.sol";
import "./Interfaces/ICaster.sol";
import "./Curse.sol";


contract SpellBook is AccessControl, ISpellBook, Pausable {
    ...
    function setFeeWallet(address addr) external onlyRole(ADMIN_ROLE) {
        setPermanentProtection(_feeWallet, false);
        setPermanentProtection(addr, true);
        _feeWallet = addr;
    ...
    function setCurseBeneficiary(address addr) external onlyRole(ADMIN_ROLE) {
        setPermanentProtection(_curseBeneficiary, false);
        setPermanentProtection(addr, true);
...
contract Curse is CERC20 {
    ...
    function _autoSwapBack() private {
        ...
        uint256 amountToEth = tokens - tokensForLiquidity;
        ...
        uint256 ethForLiquidity = (ethBalance * _feeCollection.tokensForLiquidity) / tokens;
    ...
    function _collectFee(uint256 amount, uint8 selector, uint8 level) private returns (uint256) {
        uint256 totalFee = _fees[selector].marketingFee + 
                _fees[selector].liquidityFee + 
                _fees[selector].grandMasterFee - 
                ( uint256(level) * _fees[selector].percentageDiscountPerLvl );
                
        uint256 fees = (amount * totalFee) / FEE_DENOMINATOR;

        _feeCollection.tokensForLiquidity += (fees * _fees[selector].liquidityFee) / totalFee;
        _feeCollection.tokensForMarketing += (fees * _fees[selector].marketingFee) / totalFee;
        _feeCollection.tokensForGrandMaster += (fees * _fees[selector].grandMasterFee) / totalFee;
