...
contract insurance20230118 {
    ...
    uint256 public repayAccum; //holds amount that is REPAYed to ins' for "claims payed" de-authorized

    ////define: nftContrAddress, tokenId in constructor
    address public nftContrAddress;
    ...
    function getMinPayment(uint256 _fundsPromised, uint256 _terms) public returns (uint256){
        uint256 _minPayment=99999999999999999999999999999999999999; //set high for safe
        if(_terms==1){
            //set premium payment terms to "monthly" at "5/10000" of initial funding amount
            //minPayment=(fundsPromised*5)/10000;
            _minPayment=(_fundsPromised*12)/1000;
            _minPayment=_minPayment/12;
    ...
    function setTerms(uint256 _terms) internal returns (uint256 newTerms) {
        ...
        if(_terms==1){
            ...
            minPayment=getMinPayment(fundsPromised, _terms);
            paymentInterval=(60*60*24*30); //payment needed every 'month'
            newTerms=1;
        }
        if(_terms==2){
    ...
    function resolveBreach(uint256 _move) public{
        ...
        if(breachStatus==10){
            //non-paymnet preach
            require(msg.sender==getNftOwner()); //underWriter);  //!!!!!!!!!!verify "owner" keyword
            if(_move==10){
                //see _move enums
                breachStatus=0;
    ...
    function claimPaymentRepay() external payable returns (bool){
        require(claimsPayed>0);
        bool claimRepaid=false;
        repayAccum=repayAccum + msg.value;  //accumulate "Repay"
        
        if(repayAccum>=maxBeniPaym){
            //decrement claimsAuthed AND claimsPayed
            if(claimsAuthorized!=0){ claimsAuthorized=claimsAuthorized-1;}
            if(claimsPayed!=0){ claimsPayed=claimsPayed-1;}
            //subtract from "Repay Accumulator"
            repayAccum=repayAccum-maxBeniPaym; 
            claimRepaid=true;
        }
    ...
    function collectPayment(uint256 _paymRequest) external {
        address _underWriter=getNftOwner();
        require(msg.sender==_underWriter);  //underWriter);
        if(insEnds>block.timestamp){
            ...
            if((address(this).balance-fundsPromised)>_paymRequest){ 
                (bool success, ) = _underWriter.call{value: _paymRequest}("");
                require(success, "Failed to send Ether");
            }
        }else{
            //insurance term is over pay all
            (bool success, ) = _underWriter.call{value: address(this).balance}("");
            require(success, "Failed to send Ether");
    ...
    */
    ...
    function reNameFundMngr(address newFundMngr) external{
        require(msg.sender==getNftOwner()); //underWriter);
