...
contract ErthlingMinter is Minter {
    ...
    function _beforeMintActiveErthling_(address to_, uint256 nativePrice_) internal
    assertMaxMints(to_)
    assertTotalSupply(erthling, maxErthlingSupply)
    {
        require(msg.value == nativePrice_, "Wrong ETH amount sent");

        unchecked {
            mintsPerWallet[to_] += 1;
        }
    }
    ...
    function mintActiveErthling(
        ...
        _beforeMintActiveErthling_(to_, nativePrice_);
        ...
        erthling.mintTo(to_, erthlingId_);

        _forwardPayment();
    }

    function setMaxErthlingsLimit(uint256 maxErthlingSupply_)
        external
        onlyAdmin
    {
        maxErthlingSupply = maxErthlingSupply_;
...
interface IErthling is IERC721, IDotEarth721 {
