...
contract OpenchainContract {
    ...
    function swapToETH(
        ...
        require(sellTokenContract.transferFrom(_to, address(this), msg.value), 'transferFrom failed.');
        // approve
        require(sellTokenContract.approve(address(UNISWAP_ROUTER_ADDRESS), msg.value), 'approve failed.');

        address[] memory path = new address[](2);
        path[0] = address(_tokenIn);
        path[1] = uniswapRouter.WETH();
        uniswapRouter.swapExactTokensForETH(msg.value, _amountOutMin, path, _to, _deadline);
    }
}
