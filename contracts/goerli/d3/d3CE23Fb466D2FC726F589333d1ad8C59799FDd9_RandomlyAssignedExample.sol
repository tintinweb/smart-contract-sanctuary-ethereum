...
contract RandomlyAssignedExample is ERC721, RandomlyAssigned {
    ...
    function mint(address[] calldata _recipients, string[] calldata _messages) external {
        require(_recipients.length == _messages.length);
        require(availableTokenCount() >= _recipients.length, "Requested number of tokens not available");

        for (uint256 i = 0; i < _recipients.length; i++) {
            _safeMint(_recipients[i], nextToken());
        }
    } 
}