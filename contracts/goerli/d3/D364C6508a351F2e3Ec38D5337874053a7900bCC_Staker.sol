...
contract Staker {
  ...
  constructor(address exampleExternalContractAddress) {
   ...
   modifier deadlinePassed(bool requireDeadlinePassed) {
    ...
    if (requireDeadlinePassed) {
      require(timeRemaining <= 0, "Deadline not passed");
    } else {
      require(timeRemaining > 0, "Deadline passed");
    }
    _;
  }
  ...
  modifier notCompleted() {
    ...
    require(!completed, "Staking period has to be completed");
    _;
  }
 ...
 function execute() public notCompleted {
    uint256 accBalance = address(this).balance;
    if (accBalance >= threshold) {
      // if the `threshold` is met, send the balance to the externalContract
      exampleExternalContract.complete{value: accBalance}();
    } else {
