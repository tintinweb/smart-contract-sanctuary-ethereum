...
// File: Interfaces/IEnergyToken.sol
...
// File: bicoStaking.sol


contract bicoNFTStaking  is Ownable, ReentrancyGuard, BasicMetaTransaction {
    ...
    // increment the amountStaked and map msgSender() to the Token Id of the staked
    // Token to later send back on withdrawal. Finally give timeOfLastUpdate the
    // value of now.
    function stake(uint256[] calldata _tokenIds) external nonReentrant {
        if (stakers[msgSender()].amountStaked > 0) {
            uint256 rewards = calculateRewards(msgSender());
            stakers[msgSender()].unclaimedRewards += rewards;
        } else {
            stakersArray.push(msgSender());
        }

        uint256 len = _tokenIds.length;
        for (uint256 i; i < len; ++i) {
            require(
                nftContract.ownerOf(_tokenIds[i]) == msgSender(),
                "Can't stake tokens you don't own!"
            );
            nftContract.transferFrom(msgSender(), address(this), _tokenIds[i]);
            stakerAddress[_tokenIds[i]] = msgSender();
        }
        stakers[msgSender()].amountStaked += len;    //_tokenIds.length;
        stakers[msgSender()].timeOfLastUpdate = block.timestamp;
    }
    ...
    // ERC721 Token in param: check if msgSender() is the original staker, decrement
    // the amountStaked of the user and transfer the ERC721 token back to them
    function withdraw(uint256[] calldata _tokenIds) external nonReentrant {
        require(
            stakers[msgSender()].amountStaked > 0,
        ...
        uint256 len = _tokenIds.length;
        for (uint256 i; i < len; ++i) {
            require(stakerAddress[_tokenIds[i]] == msgSender());
            stakerAddress[_tokenIds[i]] = address(0);
            nftContract.transferFrom(address(this), msgSender(), _tokenIds[i]);
        }
        stakers[msgSender()].amountStaked -= len;
        stakers[msgSender()].timeOfLastUpdate = block.timestamp;
        if (stakers[msgSender()].amountStaked == 0) {
            for (uint256 i; i < stakersArray.length; ++i) {
                if (stakersArray[i] == msgSender()) {
    ...
    // Calculate rewards for the msgSender(), check if there are any rewards
    // claim, set unclaimedRewards to 0 and transfer the ERC20 Reward token
    // to the user.
    function claimRewards() external {
        uint256 rewards = calculateRewards(msgSender()) +
        stakers[msgSender()].unclaimedRewards;
        require(rewards > 0, "You have no rewards to claim");
        stakers[msgSender()].timeOfLastUpdate = block.timestamp;
        stakers[msgSender()].unclaimedRewards = 0;
        energyToken.mint(address(this),rewards);
        energyToken.transfer(msgSender(), rewards);
    }
