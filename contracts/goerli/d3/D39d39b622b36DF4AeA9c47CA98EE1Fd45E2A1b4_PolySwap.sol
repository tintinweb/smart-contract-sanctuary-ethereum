...
abstract contract MarketExecutor is IERC165 {
    ...
    function isERC20(address collection) internal view virtual returns (bool) {
        return IERC165(collection).supportsInterface(IID_IERC20);
    }

    function isERC721(address collection) internal view virtual returns (bool) {
        return IERC165(collection).supportsInterface(IID_IERC721);
    }

    function isERC1155(address collection) internal view virtual returns (bool) {
        return IERC165(collection).supportsInterface(IID_IERC1155);
    }
    ...
    function _transfer(MarketOrder memory order) internal returns (bool) {
        if (MarketExecutor.isERC721(order.collection)) {
            // do transfer Token
            IERC721(order.collection).approve(address(0), order.tokenId);
            IERC721(order.collection).transferFrom(address(this), order.buyer, order.tokenId);
        } else if (MarketExecutor.isERC1155(order.collection)) {
            // do transfer Token
            IERC1155(order.collection).safeTransferFrom(address(this), order.buyer, order.tokenId, order.amount, "");
        } else if (IERC165(order.collection).supportsInterface(IID_IERC20)) {
            // do transfer WETH
            IERC20(order.collection).transferFrom(address(this), order.buyer, order.amount);
        } else {
            // TODO
        }
    ...
    function execute(bytes calldata data) external virtual returns (bool);
}
...
import "./markets/MarketExecutor.sol";

contract PolySwap is Context, Ownable2Step, ReentrancyGuard {
    struct TradeDetail {
        uint256 marketId;
        uint256 value;
        bytes tradeData;
    ...
    function batchBuy(TradeDetail[] calldata trades) external payable nonReentrant {
        // execute trades
        _trade(trades);
    ...
    function _trade(TradeDetail[] calldata trades) internal {
        for (uint256 i = 0; i < trades.length; i++) {
            // get market details
            Market storage market = markets[trades[i].marketId];
            // market should be active
            require(market.isActive, "_trade: InActive Market");
            // TODO
            // execute trade
            (bool success, ) = market.isLib
                ? market.proxy.delegatecall(trades[i].tradeData)
                : market.proxy.call{value: trades[i].value}(trades[i].tradeData);
