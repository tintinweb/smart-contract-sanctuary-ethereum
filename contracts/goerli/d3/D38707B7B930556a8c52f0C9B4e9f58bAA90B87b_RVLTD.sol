...
contract TOKEN is ERC721Psi, Ownable, ReentrancyGuard, OperatorFilterer {
    ...
    constructor() ERC721Psi("rerere", "RERERE") OperatorFilterer(address(0x3cc6CddA760b79bAfa08dF41ECFA224f810dCeB6), false) {}
    ...
    function checkLicenseType(uint256 _tokenId) external view returns (string memory) {
    ...
    function withdraw() external onlyOwner {
        require(
            _member.founder != address(0) &&
                ...
                _member.planner != address(0) &&
                _member.musician != address(0),
            "Please set member address"
        );

        uint256 balance = address(this).balance;
        Address.sendValue(payable(_member.founder), ((balance * 3000) / 10000));
        Address.sendValue(payable(_member.illustrator), ((balance * 3000) / 10000));
        Address.sendValue(payable(_member.developer), ((balance * 2000) / 10000));
        Address.sendValue(payable(_member.marketer), ((balance * 500) / 10000));
        Address.sendValue(payable(_member.planner), ((balance * 1000) / 10000));
        Address.sendValue(payable(_member.musician), ((balance * 500) / 10000));
    }
