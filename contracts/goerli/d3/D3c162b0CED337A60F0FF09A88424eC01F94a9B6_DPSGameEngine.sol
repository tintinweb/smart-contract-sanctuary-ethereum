...
contract DPSGameEngine is DPSGameEngineI, Ownable {
    ...
    {
        LockedVoyage[] memory cachedVoyages = docks.getLockedVoyagesForOwner(_owner, 0, voyages.maxMintedId());
    ...
    function getFinishedVoyageByOwnerAndId(address _owner, uint256 _voyageId)
        ...
        returns (LockedVoyage memory locked)
    {
        LockedVoyage[] memory cachedVoyages = docks.getFinishedVoyagesForOwner(_owner, 0, voyages.maxMintedId());
        for (uint256 i; i < cachedVoyages.length; ++i) {
            if (cachedVoyages[i].voyageId == _voyageId) return cachedVoyages[i];
        }
    }
    ...
    function setContract(address _contract, uint8 _target) external onlyOwner {
        ...
        else if (_target == 2) dps = IERC721(_contract);
        else if (_target == 3) supportShip = DPSSupportShipI(_contract);
        else if (_target == 4) artifact = MintableBurnableIERC1155(_contract);
        else if (_target == 5) gameSettings = DPSGameSettingsI(_contract);
        else if (_target == 6) dpsFeatures = DPSPirateFeaturesI(_contract);
        else if (_target == 7) cartographer = DPSCartographerI(_contract);
        else if (_target == 8) chests = DPSChestsIV2(_contract);
        else if (_target == 9) docks = DPSDocksI(_contract);
        else if (_target == 10) doubloons = IERC20MintableBurnable(_contract);
        else if (_target == 11) treasury = _contract;
        else if (_target == 12) plunderers = _contract;
        else if (_target == 13) voyages = DPSVoyageIV2(_contract);
