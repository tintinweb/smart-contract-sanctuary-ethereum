...
abstract contract NFTMarketExhibition {
   ...
   * @param tokenId The ID of the NFT.
   */
  event NftRemovedFromExhibition(address indexed nftContract, uint256 indexed tokenId);
  ...
  function _removeNftFromExhibition(address nftContract, uint256 tokenId) internal {
    if (nftContractToTokenIdToExhibitionId[nftContract][tokenId] != 0) {
      delete nftContractToTokenIdToExhibitionId[nftContract][tokenId];
      emit NftRemovedFromExhibition(nftContract, tokenId);
    }
  }
