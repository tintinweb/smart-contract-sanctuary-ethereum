...
contract SocialScore is ISocialScore,SocialScoreStorage,IDGenerator{
    ...
    function auditUser(address user,Emotion emotion,uint score,string memory metadata) override userMustBeInitialized(user) isNotSelfAuditing(user) isValidEmotion(emotion) isAppropriateScore(score) public {
