...
contract RoyalFoolsHalloween is ERC721, Ownable {
   ...
   mapping(address => bool) public verifiedClaimers;
   mapping(address => mapping(uint256 => bool)) claimedTokens;
   event Pause(bool pause);
   event ClaimedSuccessfully(address to, uint256 indexed id);

   error Soldout();
   error AlresdyClaimed();
   ...
   error ContractIsPaused();
    ...
    constructor(
        ...
        uint256 _maxElements) ERC721("FoolHalloween", "FH") {
            ...
            token = IRoyalFools(0x93aD1d082C12Dce9Bf5FF1bbE695D1B841f06e46);
    }
    ...
    function claim(address wallet, uint256 yourFool) public claimIsOpen {
        require(_exists(yourFool), "ERC721: invalid token ID");
        uint256 tokenId = totalSupply();

        if (verifiedClaimers[wallet] != true) {
            revert IncorrectOwner();
        }
        if (claimedTokens[wallet][yourFool] != false) {
            revert AlresdyClaimed();
        }

        _safeMint(wallet, tokenId);
        supply.increment();
        claimedTokens[wallet][yourFool] = true;
    }
    ...
    function addClaimers(uint256[] memory tokens) public onlyOwner {
        uint256 elements = tokens.length;

        for (uint256 i = 0; i < elements; i = uncheckedIncrement(i)) {

            uint256 tokenId = tokens[i];
            address owner = token.ownerOf(tokenId);
            verifiedClaimers[owner] = true;
        }
    }
