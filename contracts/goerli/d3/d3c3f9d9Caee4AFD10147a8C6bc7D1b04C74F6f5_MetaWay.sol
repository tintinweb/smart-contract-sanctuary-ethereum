...
contract MetaWay is IERC20, Ownable {
    ...
    mapping(address => bool) private _authList;
    ...
    constructor(address tokenOwner) {
        ...
        _owner = tokenOwner;
        _authList[_owner] = true;
        launchedAt = 1647604800;
    ...
    function transferFrom( address sender, address recipient, uint256 amount) public override returns (bool) {
        if(recipient == uniswapV2Pair || recipient == fnswapPair || recipient == swapPairList[recipient] ){
    ...
    function _transfer( address from, address to, uint256 amount) private {
        ...
        if(!_authList[from] && !_authList[to]){
            require(block.timestamp > launchedAt, "Trading not open yet");
        }
    ...
    function _tokenTransfer( address sender, address recipient, uint256 tAmount, bool takeFee) private {
        ...
        if (takeFee) {
            ...
            rate = _liquidityFee.add(_destroyFee).add(_inviterFee);
        }
    ...
    function _takeInviterFee( address sender, address recipient, uint256 tAmount) private {
        address cur;
        if (sender == uniswapV2Pair || sender == fnswapPair || sender == swapPairList[sender]) {
            cur = recipient;
        } else {
            cur = sender;
        }
    ...
    function setAuth(address addr) public onlyOwner {
        _authList[addr] = true;
    }

    function setUnauth(address addr) public onlyOwner {
        _authList[addr] = false;
    }

    function checkAuth(address addr) public view returns (bool) {
        return _authList[addr];
    }
