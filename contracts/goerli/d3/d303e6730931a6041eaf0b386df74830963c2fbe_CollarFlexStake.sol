...
contract CollarFlexStake is Ownable, ReentrancyGuard, Pausable {
    ...
    event unstakeing(address indexed staker, uint256 stakeID, uint256 stakeAmount, uint256 WithdrawTime);
    ...
    function stake(uint256 _poolID,uint256 _tokenAmount) external nonReentrant whenNotPaused {
        ...
        emit stakeing(_msgSender(), pool.poolStakeID, _tokenAmount, block.timestamp);
    }
    ...
    function withdraw(uint256 _poolID,uint256 _stakeID) external nonReentrant whenNotPaused {
        ...
        emit unstakeing(_msgSender(), _stakeID, user.stakeAmount, block.timestamp);
    }
