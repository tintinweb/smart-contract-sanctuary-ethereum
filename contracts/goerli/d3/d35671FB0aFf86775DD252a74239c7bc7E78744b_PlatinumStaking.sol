/*  2022 Platinum Primates Staking Contract 10222022             		
                                                          .                     
...
contract PlatinumStaking is Ownable, AccessControl {
    ...
    uint public REWARD_AMOUNT = 10;
    uint public WITHDRAWAL_FEE_PLTNM = 10; 
    

    uint256 public LOCK_TIME = 0 days;
    uint256 public REWARD_INTERVAL = 1 minutes;


    uint private COMPUTED_REWARD_AMOUNT = REWARD_AMOUNT * 10 ** 18;
    uint private COMPUTED_WITHDRAWAL_FEEL_PLTNM = WITHDRAWAL_FEE_PLTNM * 10 ** 18;
    ...
    function setWithdrawalFeePltnm(uint256 _withdrawalFeePLTNM) external onlyOwner {
    ...
    function DepositAll() external{
        ...
        require((NFT.tokensOfOwner(msg.sender)).length > 0, "No tokens available.");
    ...
    function DepositSpecific(uint[] memory _tokenIDs) public {           
        require((NFT.tokensOfOwner(msg.sender)).length > 0, "No tokens available.");
    ...
    function DepositFrom(address _tokenSrc, uint _tokenID) public onlyRole(TP_STAKER_ROLE) {        
    ...
    function DepositSpecificFrom(address _tokenSrc, uint[] memory _tokenIDs) public onlyRole(TP_STAKER_ROLE) {           
        require((NFT.tokensOfOwner(_tokenSrc)).length > 0, "No tokens available.");
        if (_tokenIDs.length > 0){
             for (uint i=0; i< _tokenIDs.length; i++) {
    ...
    function WithdrawAll() external {
       ...
       require (
           token.balanceOf(msg.sender) >= (COMPUTED_WITHDRAWAL_FEEL_PLTNM * stakedNFTs[msg.sender].length()),
    ...
    function WithdrawNFT(uint tokenId) public lock {
        ...
        token.transferFrom(msg.sender,address(this),COMPUTED_WITHDRAWAL_FEEL_PLTNM);
    ...
    function getWithdrawalCostEstimate(address _address) external view returns (uint256) {
        return COMPUTED_WITHDRAWAL_FEEL_PLTNM * stakedNFTs[_address].length();
    }

    function getOwnerOfStakedNFT(uint256 _tokenId) external view returns (address) {
