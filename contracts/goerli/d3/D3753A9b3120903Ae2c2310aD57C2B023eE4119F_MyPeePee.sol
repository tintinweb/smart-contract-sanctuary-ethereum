    ...
    DefaultOperatorFilterer
{
    ...
    function whitelistMint(
        ...
        if (_totalMinted() + quantity_ > whitelistSupply)
            revert WhitelistSoldOut();
        if (quantity_ > whitelistLimit) revert WrongLimit();
        if (_numberMinted(msg.sender) + quantity_ > whitelistLimit)
            revert LimitExceed();
        if (_numberMinted(msg.sender) >= 1) {
            if (msg.value != quantity_ * whitelistMintPrice)
                revert WrongPrice();
        } else {
            if (msg.value != (quantity_ - 1) * whitelistMintPrice)
                revert WrongPrice();
        }
    ...
    function setMintPrice(uint256 whitelistMintPrice_) public onlyOwner {
