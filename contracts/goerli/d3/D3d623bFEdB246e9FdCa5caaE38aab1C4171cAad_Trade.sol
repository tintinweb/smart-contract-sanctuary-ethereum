...
contract Trade {
    ...
    function getFees(uint paymentAmt, BuyingAssetType buyingAssetType, address buyingAssetAddress, uint tokenId) public view returns(Fee memory){
        ...
        if(buyingAssetType == BuyingAssetType.ERC721) {
            (tokenCreator, royaltyPermille) = ((IERC721(buyingAssetAddress).royaltyInfo(tokenId, price)));
            tokenCreator = IERC721(buyingAssetAddress).ownerOf(tokenId);
        }
        if(buyingAssetType == BuyingAssetType.ERC1155)  {
            (tokenCreator, royaltyPermille) = ((IERC1155(buyingAssetAddress).royaltyInfo(tokenId, price)));
            tokenCreator = IERC1155(buyingAssetAddress).owner();
        }
    ...
    function set721royaltyRefTake(uint refSplitAmount, address collectionAddress, uint Id) public {
        require(msg.sender == IERC721(collectionAddress).ownerOf(Id), "You do not own this 721 NFT.");
    ...
    function set1155royaltyRefTake(uint refSplitAmount, address collectionAddress) public {
        // require(msg.sender == IERC1155(collectionAddress).ownerOf(Id), "You do not own this 1155 Token.");
