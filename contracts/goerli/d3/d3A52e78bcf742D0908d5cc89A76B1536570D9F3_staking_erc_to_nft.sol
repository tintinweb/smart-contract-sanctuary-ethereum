...
interface Imytoken{
   ...
   function _transfer(address _from,address _to, uint256 amount) external;
   
}
contract staking_erc_to_nft is Initializable,UUPSUpgradeable,OwnableUpgradeable {
    //Stake information
    struct Stake{
        uint amount;
    ...
    function initialize() initializer public   {
        //Add address of the newly deployed smart contract  
        token =Imytoken(0x925947cB4dcDd71676D9a50d77720A1441460e37) ;
        __Ownable_init();
        __UUPSUpgradeable_init();
        //minting tokens for the address
        token.mintfortoken(_msgSender(),10**24);//1 million 
    }
    //UUPS
    ...
    //function to mint
    function create_stake(address _to,uint _rate,int _stakedDuration,uint _amount) public {
        require(token.balanceOf(_msgSender())>=_amount,"Account balance is less than the requested stake amount ");
        Stake storage instance=addToStake[_to];
        instance.current_duration=int(block.timestamp);
        instance.rate=_rate;
        instance.amount=_amount;
        instance.stakedDuration=int(block.timestamp)+_stakedDuration;
        instance.owner=_to;
    ...
    function checkReward(address _person) public view returns(uint){
        require(hasStaked(_person)==true,"User has not staked any thing yet");
    ...
    //calculating reward 
    function reward_per_person(address _person) public {
        require(hasStaked(_person)==true,"User has not staked any thing yet");
        Stake memory rewardOf=addToStake[_person];
        int difference=rewardOf.stakedDuration-int(block.timestamp);
        if(difference>0){
    ...
    function timeleft(address _person) public view  returns(int){
        require(hasStaked(_person)==true,"User has not staked any thing yet");
        Stake memory rewardOf=addToStake[_person];        
        int difference=rewardOf.stakedDuration-int(block.timestamp);
        return difference;
    ...
    //withdraw per person
    function withdrawReward(address _person) internal  onlyOwner{
        require(addTOreward[_person]!=0,"Reward not calculated");
        Stake storage rewardOf=addToStake[_person];
        token.mintfortoken(_person,addTOreward[_person]);
        ...
        changehasStaked(_person);

    }
    function changehasStaked(address _person) internal {
        for(uint i=0;i<stakeholder.length;i++){
            if(stakeholder[i]==_person){
                delete stakeholder[i];
            }
        }    
    }
