...
contract Solo2 is ERC721, Ownable {

    address public receivingAddress = 0x30d0c38359eF6465240eB88C9Df1CF52B1b3Fb50;
    using Strings for uint256;

    constructor() ERC721("Solo2","SO") {
        unchecked {
        balanceOf[receivingAddress] += 50;
        totalSupply += 50;
    ...
    enum MintStatus { PAUSED, PHASE1, PHASE2 }
    ...
    function changeStatus(uint _status) public onlyOwner {
        ...
        else if (_status == 1) { theStatus = MintStatus.PHASE1; }
        else if (_status == 2) { theStatus = MintStatus.PHASE2; }
    }

    function mintNFT (uint256 amount) external payable {
        
        require(theStatus == MintStatus.PHASE2, "Not the Public Phase yet");
    ...
    function mintNFT2 (bytes32[] calldata _merkleProof , uint256 amount) external payable {
        
        require(theStatus == MintStatus.PHASE1, "Not the Whitelist Phase yet");
        require(amount > 0, "Unable to mint 0 NFTs");
        require (amount <= maxMintAmount, "You are unable to mint this amount of NFTs");
