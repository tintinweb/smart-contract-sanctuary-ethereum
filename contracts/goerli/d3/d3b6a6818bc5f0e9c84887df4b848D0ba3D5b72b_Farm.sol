...
contract Farm is Ownable, ERC721AWithRoyalties, Pausable, PaymentSplitter {
    ...
    function wrapLandtoFarmReq(
        ...
        LandContract.farmConditionVerifier(
            ids,
            msg.sender,
            [minTerm, maxTerm, destablingFee, ownerFee]
        );
        if (stableID == 0) {
            require(
    ...
    function manualfulfill() public {
        ...
        emit FarmMinted(
            _tokenIdCounter,
            _requests[msg.sender].land_ids,
            _requests[msg.sender].name
        );
    ...
    function upgradeFarmReq(
        ...
        CLClient APIConsContract = CLClient(APIConsAddress);
        require(ownerOf(farmID) == msg.sender, "MUST OWN FARM");
        for (uint256 i = 0; i < land_ids.length; i++) {
            farms[farmID].push(farms[farmID][i]);
        }
        bytes32 reqid = APIConsContract.checkcontiguity(
            msg.sender,
            land_ids,
    ...
    function stableHorse(
        ...
        Stabler StableContract = Stabler(StableAddress);
        uint256 farmExpDate = StableContract.stablingVerifier(
            horseID,
            stableTerm,
            [
                minStableTerm,
    ...
    function destable(uint256 horseID) public payable {
        ...
        if (block.timestamp < farmExpDate) {
            if (farms[farmID].length >= publicNumber) {
                require(
                    msg.value >= farmParams[farmID].destablingFee,
                    "MUST SEND >= ETH AMOUNT TO BREAK CONTRACT"
                );
            }
            if (msg.sender == HorseContract.ownerOf(horseID)) {
                payable(ownerOf(farmID)).transfer(msg.value);
            } else if (msg.sender == ownerOf(farmID)) {
                payable(HorseContract.ownerOf(horseID)).transfer(msg.value);
            }
        }
        _destable(farmID, horseID);
    }
    ...
    function _destable(uint256 farmID, uint256 horseID) internal {
        ...
        Horser(HorseAddress).setStable(horseID, 0, 0);
    }
    ...
    function _removeRequest(
        ...
        delete stableRequests[farmID][stableRequests[farmID].length - 1];

        delete stableRequestsByHorseID[horseID][
            stableRequestsByHorseID[horseID].length - 1
        ];
    }
    ...
    function approveRequest(
        ...
        uint256 farmID,
        uint256 reqnum
    ) public expireHorse(farmID, horseID) {
        refreshContracts();
