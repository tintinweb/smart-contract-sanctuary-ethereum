...
contract GXAGToken is OwnableUpgradeable, ERC20BurnableUpgradeable {
    ...
    function initialize(
        ...
        __Ownable_init();
        __ERC20_init("GXAG", "GXAG");
        emit Initialized(msg.sender, block.number);
    }

    modifier onlyActive() {
        require(toggleStatus, "GXAGToken: logic in not available now");
        _;
    }

    modifier onlyAdmin() {
        require(msg.sender == adminAddress || msg.sender == owner(), "GXAGToken: only admin");
        _;
    }

    modifier onlyASWM() {
        require(msg.sender == aswmAddress, "GXAGToken: only aswm");
        _;
    }
    ...
    function buy(address _token, uint256 _amount) external {
        require(
            ...
            "GXAGToken: Invalid balance"
        );
        require(
            IUniswapV2Factory(factoryAddress).getPair(_token, usdcAddress) != address(0),
            "GXAGToken: Pair with this token not exist"
        );
        _buyToken(msg.sender, _token, _amount);
    }
    ...
    function addReserve(address _holder, uint256 _amount) external onlyASWM {
        require(reserves[_holder] == 0, "GXAGToken: already have reserve with this address");
        require(
            super.balanceOf(_holder) >= _amount && minReserveAmount < _amount,
            "GXAGToken: Invalid amount"
        );
    ...
    function releaseReserve(address _holder) external onlyASWM {
        require(reserves[_holder] != 0, "GXAGToken: invalid reserves");
    ...
    /**
     * Returns the latest price GXAGToken
     */
    ...
    function _buyToken(address _sender, address _token, uint256 _amount) private {
        uint256 _gxagUsdPrice = getTokenPrice();
        uint256 erc20UsdPrice = _getERC20Price(_token);

        require(
            ((_amount * erc20UsdPrice) / _gxagUsdPrice) >= minBuyAmount,
            "GXAGToken: Invalid amount for buy"
        );

        uint256 balanceUsdBefore = IERC20Upgradeable(usdcAddress).balanceOf(address(this));
        ...
        uint256 usdcAmount = IERC20Upgradeable(usdcAddress).balanceOf(address(this)) -
            balanceUsdBefore;

        _transfer(address(this), _sender, (usdcAmount / _gxagUsdPrice));
    }

    /**
     * Returns the latest price ERC20Token
     */
    function _getERC20Price(address _token) private view returns (uint256) {
        address[] memory path = new address[](2);
        path[0] = _token;
        path[1] = usdcAddress;

        uint256[] memory GXADTokenPriceInUSD = IUniswapRouterV2(routerAddress).getAmountsOut(
            1 ether,
            path
        );
        return GXADTokenPriceInUSD[1];
    }
}
