  ...
  UUPSUpgradeable
{
    ...
    setupExecution
  {
    ...
    for (uint8 i = 0; i < executionsLength; i++) {
      assembly {
        ...
        default {
          let next_order_location := calldataload(
    ...
    reentrancyGuard // move re-entrancy check for clarity
  {
    ...
    require(
      _validateOrderParameters(sell.order, sellHash),
      "Sell has invalid parameters"
    );
    require(
      _validateOrderParameters(buy.order, buyHash),
  ...
  {
    if (
      order.order.extraParams.length > 0 && order.order.extraParams[0] == 0x01
    ) {
      /* Check oracle authorization. */
      require(
        block.number - order.blockNumber < blockRange,
  ...
  function _executeFundsTransfer(
    ...
    /* Take fee. */
    uint256 sellerFeesPaid = _transferFees(
      sellerFees,
      paymentToken,
      buyer,
      price,
    ...
    uint256 buyerFeesPaid = _transferFees(
      buyerFees,
      paymentToken,
      buyer,
      price,
