...
contract USC is ERC721I, Ownable, Security, CollectionOwnership {
    ...
    function claim(uint256[] calldata tokenIds_) external onlySender publicClaimEnabled {
        ...
        for (uint256 i = 0; i < _len; i++) {
            ...
            require(!_claimedTokenIds[_tokenId], string(abi.encodePacked("Token ", _toString(i), " already claimed")));
            require(_collectionAddress.ownerOf(_tokenId) == msg.sender, string(abi.encodePacked("Token ", _toString(i), " does not belong to sender")));
