...
contract DefiOTCSwap is ReentrancyGuard {
	...
	function withdrawAllOpenOrders() external nonReentrant {
		...
		for (uint256 i; i < orderIds.length; i++) {
			Order storage currOrder = orderIdMapping[orderIds[i]];
			currOrderMaker = currOrder.maker;
			if (currOrderMaker == address(0)) continue; //already withdrawn or fulfilled
	...
	function readOrdersForMaker(address maker, uint256 cursor, uint256 depth) external view returns (Order[] memory _orders) {
		uint96[] memory orderIds = orderIdsByMaker[maker];
		if (depth > orderIds.length - cursor) depth = orderIds.length - cursor;
		_orders = new Order[](depth);
		for (uint256 i; i < depth && cursor != 0; i++) {
			_orders[i] = orderIdMapping[orderIds[cursor+i]];
		}
	}
