...
contract MockLockManager is Ownable2Step {
    bool public locked;
...
contract MockLockManagerScript is Script {
    function run() external {
        uint256 deployerPrivateKey = vm.envUint("PRIVATE_KEY");
        vm.startBroadcast(deployerPrivateKey);

        bytes32 salt = keccak256("MockLockManager");
        address owner = msg.sender;

        // The contract's creating code and its constructor arguments
        bytes memory creationCode = abi.encodePacked(
            type(MockLockManager).creationCode,
            abi.encode(owner)
        );
        ...
        assembly {
            ...
            // Skip the first 32 bytes of the creation code containing its length
            deployedContract := create2(0, add(creationCode, 0x20), mload(creationCode), salt)
        }
        require(deployedContract != address(0), "DEPLOYMENT_FAILED");

        MockLockManager mockLockManager = MockLockManager(deployedContract);
        require(mockLockManager.owner() == owner, "WRONG_OWNER");
