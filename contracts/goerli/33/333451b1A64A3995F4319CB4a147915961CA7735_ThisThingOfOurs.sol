...
contract ThisThingOfOurs is ERC721, ERC2981, Ownable {
    ...
    // TokenIds 1-155 are reserved for whitelisted Capos
    uint256 public nextId = 156;
    uint256 public nextCapo = 1;
    uint256 public constant maxCapoSupply = 155;
     ...
     * @param   _capoMerkleRoot Merkle whitelist root for special users
    ...
    constructor(
        ...
        address _USDC
    ) ERC721("This Thing Of Ours", "OURS") {
    ...
    /**
     * @notice  Mint an NFT
     * @dev     User must have approved this contract for mintPrice
                Transmit a proof for the regular tree to mint regular, or a proof for the Capo tree to mint Capo
                It is implicit that Capo-WL users should not be WL for regular mint also
     */
    function mint(bytes32[] calldata _proof) external {
        require(!publicSaleActive, "!phase");
        require(!claimed[msg.sender], "!qty");
        ...
        if (MerkleProof.verify(_proof, merkleRoot, leaf)) {
            tokenId = nextId;
            nextId++;
        } else if (MerkleProof.verify(_proof, capoMerkleRoot, leaf)) {
            tokenId = nextCapo;
            nextCapo++;
        } else {
            revert("!proof");
        ...
        // If proof is valid, take payment
        require(
    ...
    function purchase() external {
        require(publicSaleActive, "!phase");
        require(publicSupplyAvailable > 0, "!supply");

        // We have already checked this will not underflow
        unchecked {
            publicSupplyAvailable--;
        }
        ...
        // Mint a Capo if any are left, otherwise mint a regular token
        uint256 tokenId;
        if (nextCapo > maxCapoSupply) {
            tokenId = nextId;
            nextId++;
            _mint(msg.sender, tokenId);
        } else {
    ...
    function startPublicSale() external onlyOwner {
        ...
        publicSupplyAvailable = (totalSupply + 155 - nextId - nextCapo + 2);
        // ----- Example Math -----
        // Mint 1000 regular + 100 Capos = 900 available
        //      2000 + 155 - 1156 - 101 + 2 = 900
        // Mint 1845 regular + 155 Capo = 0 available
        //      2000 + 155 - 2001 - 156 + 2 = 0
        // ------------------------
    ...
    /**
     * @notice Stop a public sale and enable whitelist minting
     */
