...
interface ITokenLocker {
  ...
  function firstUnlockAmount(uint256 _lockAmount)
    external
    view
    returns (uint256);
    ...
    returns (uint256)
  {
    ...
    if (timestamp < startReleaseTimestamp) {
      ...
      uint256 _totalVestingTime =
        endReleaseTimestamp.sub(startReleaseTimestamp);
      return
        _locks[_account].mul(_releasedTime).div(_totalVestingTime).sub(
          _released[_account]
        );
    }
  }
