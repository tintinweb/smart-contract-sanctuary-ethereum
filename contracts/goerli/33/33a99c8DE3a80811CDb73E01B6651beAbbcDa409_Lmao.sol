...
contract Lmao {
    ...
    modifier noReentrancyOnExecuteBridgeOrigin() {
...
//        uint256 _originTokenAmount
        ...
        external
        payable
        noReentrancyOnExecuteBridgeOrigin()
        returns (uint256 sequence)
    {
        ...
        emit BridgeSentLog({
            ...
            targetContractAddress: bytes32ToAddress(_targetContractAddress),
            amount: msg.value,
...
//        }
    ...
    function receiveBridge(bytes memory encodedVm) external {
        ...
        require(vaa.toChain == WORMHOLE_CHAIN_ID, 'Wrong Wormhole chain ID');
        ...
        require(IERC20(wrapped).balanceOf(address(this)) >= vaa.amount * 10 ** DECIMALS, 'Insufficient amount of token to transfer');
        require(IERC20(wrapped).transfer(receiver, vaa.amount * 10 ** DECIMALS), 'Transfer of token failed');

        emit TokenTransferred({
            receiver: receiver,
            amount: vaa.amount * 10 ** DECIMALS
        });
