...
interface IOldCoolPets {
    function _currentPetId() external view returns (uint256);

    function _merkleRoot() external view returns (bytes32);

    function _price() external view returns (uint256);

    function _reserved() external view returns (uint256);
    ...
    OwnableUpgradeable
{
    ...
    modifier burningEnabled() {
        require(_burnEnabled, "Burning disabled");
        _;
    }
    ...
    function initialize(
        ...
        _merkleRoot = IOldCoolPets(oldContract)._merkleRoot();
        _price = IOldCoolPets(oldContract)._price();
        _claimsPaused = true;
        _merkleLocked = false;
        _burnEnabled = true;
    ...
    function migrate(uint256[] memory tokenIds) external {
        for (uint256 i; i < tokenIds.length; i++) {
            IOldCoolPets(_oldContract).transferFrom(
                msg.sender,
                burnAddr,
                tokenIds[i]
            );
    ...
    function burn(uint256[] memory tokenIds) external burningEnabled {
        for (uint256 i; i < tokenIds.length; i++) {
            transferFrom(msg.sender, burnAddr, tokenIds[i]);
        }
    }
    ...
    function setBurningEnabled(bool val) external onlyOwner {
        _burnEnabled = val;
    }
    ...
    /// @notice Se the Old Cool Pets contract address
    /// @param oldAddress Address of the Old Cool Pets contract
    function setOldCoolPetsAddress(address oldAddress) external onlyOwner {
        _oldContract = oldAddress;
    }
