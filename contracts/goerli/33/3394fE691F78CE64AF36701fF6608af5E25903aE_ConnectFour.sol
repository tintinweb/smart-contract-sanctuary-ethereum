...
contract ConnectFour {
    ...
    //This event was added (although not in the video) so that we can use this contract with the CLI-frontend and with future Sprints.
    event RewardClaimed(
        uint16 gameId,
        address winner,
        address recipient,
    ...
    function initializeGame() external payable returns (uint16) {
        require(
            ...
            "ConnectFour: msg.value out of bounds"
        );
    ...
    function startGame(uint16 _gameId) external payable {
        ...
        require(
            ...
            "ConnectFour: msg.value (betAmount) incorrect"
        );
    ...
    function playMove(uint16 _gameId, uint8 _col) external {
        ...
        require(
            ...
            "ConnectFour: invalid player for this turn"
        );
    ...
    function claimReward(
        ...
        require(
            games[_gameId].status == Status.Started,
            "ConnectFour: can't claim reward, game hasn't started, doesn't exist, or is already claimed"
        );
        ...
        for (uint8 i = 0; i < 3; i++) {
            ...
            require(
                ...
                "ConnectFour: board indexes must only contain caller's discs"
            );
    ...
    function boardIndex(uint8 _col, uint8 _row) public pure returns (uint8) {
        require(
            ...
            "ConnectFour: board index out of range"
        );
        return _row * 7 + _col;
    }
}