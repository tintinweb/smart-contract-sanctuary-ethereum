...
contract BrewlabsStakingPool is Ownable, ReentrancyGuard {
    ...
    struct Stake {
        ...
        uint256 duration;   // the stakingPool duration of the stake
    ...
    event StakingPoolUpdated(uint8 _type, uint256 _duration, uint256 _fee0, uint256 _fee1, uint256 _rate);
    event RewardsStop(uint256 blockNumber);
    event EndBlockUpdated(uint256 blockNumber);
    event UpdatePoolLimit(uint256 poolLimitPerUser, bool hasLimit);

    event ServiceInfoUpadted(address _addr);
    ...
    function deposit(uint256 _amount, uint8 _stakeType) external payable nonReentrant {
        ...
        require(_stakeType < pools.length, "Invalid stake type");
        ...
        StakingPool storage stakingPool = pools[_stakeType];

        if(stakingPool.totalStakedLimit > 0) {
            require(stakingPool.totalStaked < stakingPool.totalStakedLimit, "Total staked limit exceeded");

            if(stakingPool.totalStaked + _amount > stakingPool.totalStakedLimit) {
                _amount = stakingPool.totalStakedLimit - stakingPool.totalStaked;
            }
        }
        ...
        for(uint256 j = 0; j < stakes.length; j++) {
            ...
            uint256 _pending = stake.amount * stakingPool.accTokenPerShare / PRECISION_FACTOR - stake.rewardDebt;
            pending = pending + _pending;

            stake.rewardDebt = stake.amount * stakingPool.accTokenPerShare / PRECISION_FACTOR;
        ...
        if (stakingPool.depositFee > 0) {
            uint256 fee = realAmount * stakingPool.depositFee / 10000;
            if (fee > 0) {
        ...
        _addStake(_stakeType, msg.sender, stakingPool.duration, realAmount);

        user.amount = user.amount + realAmount;
        stakingPool.totalStaked = stakingPool.totalStaked + realAmount;
    ...
    function _addStake(uint8 _stakeType, address _account, uint256 _duration, uint256 _amount) internal {
        ...
        newStake.rewardDebt = newStake.amount * stakingPool.accTokenPerShare / PRECISION_FACTOR;
    ...
    function withdraw(uint256 _amount, uint8 _stakeType) external payable nonReentrant {
        ...
        if(realAmount > 0) {
            if (stakingPool.withdrawFee > 0) {
                uint256 fee = realAmount * stakingPool.withdrawFee / 10000;
                stakingToken.safeTransfer(walletA, fee);
                realAmount = realAmount - fee;
            }
    ...
    function claimReward(uint8 _stakeType) external payable nonReentrant {
        if(_stakeType >= pools.length) return;
    ...
    function compoundReward(uint8 _stakeType) external payable nonReentrant {
        ...
        if (pending > 0) {
            ...
            stakingPool.totalStaked = stakingPool.totalStaked + compounded;
    ...
    function compoundDividend(uint8 _stakeType) external payable nonReentrant {
        ...
        for(uint256 j = 0; j < stakes.length; j++) {
            ...
            compounded = compounded + _pending;
            stake.amount = stake.amount + _pending;
            stake.rewardDebt = stake.rewardDebt + _pending * stakingPool.accTokenPerShare / PRECISION_FACTOR;
            stake.reflectionDebt = stake.amount * accDividendPerShare / PRECISION_FACTOR_REFLECTION;
        }
    ...
    function emergencyWithdraw(uint8 _stakeType) external nonReentrant {
        ...
        if (amountToTransfer > 0) {
            ...
            stakingPool.totalStaked = stakingPool.totalStaked - amountToTransfer;
    ...
    function rewardPerBlock(uint8 _stakeType) external view returns (uint256) {
        if(_stakeType >= pools.length) return 0;

        return pools[_stakeType].rate;
    }
    ...
    function insufficientRewards() external view returns (uint256) {
        ...
        for(uint i = 0; i < pools.length; i++) {
            if(startBlock == 0) {
                adjustedShouldTotalPaid = adjustedShouldTotalPaid + pools[i].rate * duration * 28800;
            } else {
                uint256 remainBlocks = _getMultiplier(pools[i].lastRewardBlock, bonusEndBlock);
                adjustedShouldTotalPaid = adjustedShouldTotalPaid + pools[i].rate * remainBlocks;
            }
        }
    ...
    function pendingReward(address _account, uint8 _stakeType) external view returns (uint256) {
        if(_stakeType >= pools.length || startBlock == 0) return 0;

        Stake[] memory stakes = userStakes[_account];
        StakingPool memory stakingPool = pools[_stakeType];

        if(stakingPool.totalStaked == 0) return 0;
        
        uint256 adjustedTokenPerShare = stakingPool.accTokenPerShare;
        if (block.number > stakingPool.lastRewardBlock && stakingPool.totalStaked != 0 && stakingPool.lastRewardBlock > 0) {
            uint256 multiplier = _getMultiplier(stakingPool.lastRewardBlock, block.number);
            uint256 reward = multiplier * stakingPool.rate;

            adjustedTokenPerShare = stakingPool.accTokenPerShare + reward * PRECISION_FACTOR / stakingPool.totalStaked;
        }
    ...
    *************************/
    ...
    function increaseEmissionRate(uint8 _stakeType, uint256 _amount) external onlyOwner {
        ...
        if(remainRewards > shouldTotalPaid) {
            ...
            pools[_stakeType].rate = remainRewards / remainBlocks;
            emit StakingPoolUpdated(_stakeType, pools[_stakeType].duration, pools[_stakeType].depositFee, pools[_stakeType].withdrawFee, pools[_stakeType].rate);
        }
    }
    ...
    function startReward() external onlyOwner {
        ...
        for(uint256 i = 0; i < pools.length; i++) {
            pools[i].lastRewardBlock = startBlock;
        }
    ...
    function updateStakingPool(uint8 _stakeType, uint256 _duration, uint256 _depositFee, uint256 _withdrawFee, uint256 _rate, uint256 _totalStakedLimit) external onlyOwner {
        // require(block.number < startBlock, "Pool was already started");
        require(_stakeType < pools.length, "StakingPool Not found");
        require(_depositFee < 2000, "Invalid deposit fee");
        require(_withdrawFee < 2000, "Invalid withdraw fee");

        _updatePool(_stakeType);

        StakingPool storage _lockup = pools[_stakeType];
        ...
        emit StakingPoolUpdated(_stakeType, _duration, _depositFee, _withdrawFee, _rate);
    }

    function addStakingPool(uint256 _duration, uint256 _depositFee, uint256 _withdrawFee, uint256 _rate, uint256 _totalStakedLimit) external onlyOwner {
        require(_depositFee < 2000, "Invalid deposit fee");
        require(_withdrawFee < 2000, "Invalid withdraw fee");
        
        pools.push();
        
        StakingPool storage _lockup = pools[pools.length - 1];
        _lockup.stakeType = uint8(pools.length - 1);
        _lockup.duration = _duration;
        _lockup.depositFee = _depositFee;
        _lockup.withdrawFee = _withdrawFee;
        _lockup.rate = _rate;
        ...
        emit StakingPoolUpdated(uint8(pools.length - 1), _duration, _depositFee, _withdrawFee, _rate);
    }

    function setServiceInfo(address _addr) external {
        require(msg.sender == buyBackWallet, "setServiceInfo: FORBIDDEN");
        require(_addr != address(0x0), "Invalid address");

        buyBackWallet = _addr;

        emit ServiceInfoUpadted(_addr);
    }
    ...
    *************************/
    ...
    function _updatePool(uint8 _stakeType) internal {
        ...
        StakingPool storage stakingPool = pools[_stakeType];
        if (block.number <= stakingPool.lastRewardBlock || stakingPool.lastRewardBlock == 0) return;
        ...
        uint256 multiplier = _getMultiplier(stakingPool.lastRewardBlock, block.number);
        uint256 _reward = multiplier * stakingPool.rate;
        stakingPool.accTokenPerShare = stakingPool.accTokenPerShare + (
            _reward * PRECISION_FACTOR / stakingPool.totalStaked
        );
        stakingPool.lastRewardBlock = block.number;
