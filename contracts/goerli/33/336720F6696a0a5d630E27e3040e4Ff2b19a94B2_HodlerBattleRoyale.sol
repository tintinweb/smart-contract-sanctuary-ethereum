...
contract HodlerBattleRoyale is ERC1155Supply, Ownable  {
    ...
    uint constant STRIKE_PRICE = 0.01 ether;
    //Reference to aHodler Smart Contract
    ...
    constructor(string memory uri) ERC1155(uri) {
        //set owner addy and create deadline blockstamp -> current time + 1year
    ...
    // Set state of battle Royale
    function setBrState(bool _bool) public onlyOwner {
        battleRoyaleIsActive = _bool;
    }
    //Set Health status + or - 
    //If hodler gets "killed" mints an erc1155 Dog tag
    //sets last throw and concludes Battle Royale if condition is met
    function setHealthStatus(uint256 _tokenId, int256 _amount) public payable{
        //interface declaration for hodler smart Contract
        interfaceAhodler hodler = interfaceAhodler(battleField);
        uint priceMod = 10;
        uint256 amount;
        require(!battleRoyaleConcluded, "Battle Royale has concluded");
        require(battleRoyaleIsActive, "Battle Royale must be active to start attacks/mints");
        require(_amount != 0, "No Shield or Grenade selected");
        require(totalSupply(DOG_TAG) < MAX_SUPPLY, "You can't kill the last hodler standing");
        //calculate price for setHealthStatus function
        ...
        require((STRIKE_PRICE / 10 * priceMod * amount) <= msg.value, "Not enough Ether");

        //Set health in hodler smart Contract
        hodler.setHealth(_tokenId, _amount);

        //if amount negative, then increment msg.sender highscore for bought grenades
        if (_amount < 0){
            walletHighscore[msg.sender] += amount;
            //keep track of addy with most grenades bought
        ...
        //check if hodler got killed by setHealth and if true, mint a dog tag
        ...
        //if all but one dog tag got minted out, conclude the game
        if (totalSupply(DOG_TAG)>= MAX_SUPPLY-1){
            battleRoyaleConcluded = true;
            //function for setting last throw address
            lastThrow = msg.sender;
            //function for setting last id alive
            lastHodlerStanding = hodler.ownerOf(hodler.checkLastSurvivor());
            //most grenades bought
        }
    }
    //Withdraw function for when Deadline is met
    function withdraw() public onlyOwner {
        require(block.timestamp >= deadLine, "The war is not over yet!");
        uint balance = address(this).balance;
        payable(msg.sender).transfer(balance);
    }
    //Withdraw function for when Battle Royale has concluded
    function distributeToWinners() public{
        require(battleRoyaleConcluded, "Still too many Hodlers alive");
        require(!payout, "already paid out");
        // get balance of SmartContract
        uint256 balance = address(this).balance;
        // split balance for each winner type
        uint256 mgSplit = balance/100*30;
        uint256 lhsSplit = balance/100*15;
        uint256 ltSplit = balance/100*15;
        // send cut to each winner
...
contract Ahodler is ERC721A, Ownable {
    address private constant TEAMWALLET = 0xCd7ca0B93A5e447Def0f5D55C2247a81b2d4F1fa;
    address public contractCreator;
    ...
    address private battleRoyaleContract = 0xAb8483F64d9C6d1EcF9b849Ae677dD3315835cb2;
    //battle royale br started?
    ...
    //set battle royale to true or false
    ...
    //Check how many hodlers are still in the game/alive
