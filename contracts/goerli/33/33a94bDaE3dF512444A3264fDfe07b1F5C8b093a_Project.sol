...
contract Project is ContextUpgradeable, ReentrancyGuardUpgradeable {
    ...
    uint256 public createProjectFee;
    uint256 public activeProjectFee;
    ...
    function initialize(InitializeInput memory _input) external initializer {
        require(_input.setting != address(0), "Invalid setting address");
        require(_input.nftChecker != address(0), "Invalid nftChecker address");
        require(_input.osbFactory != address(0), "Invalid osbFactory address");
        require(_input.opFundReceiver != address(0), "Invalid opFundReceiver address");
    ...
    modifier onlyManager(uint256 projectId) {
        checkManager(projectId, _msgSender());
        _;
    }
    ...
    event CloseProject(uint256 indexed projectId, bool isGive);
    ...
    event ActiveProject(uint256 indexed projectId, IDOInput idoInput, uint256 indexed profitShare);
    ...
    function setManager(uint256 _projectId, address _account) external projectExists(_projectId) onlyAdmin {
        require(_account != address(0), "Invalid manager address");
        require(_account != projects[_projectId].manager, "Manager already exists");
    ...
    function updateSold(uint256 _projectId, uint256 _quantity) external projectExists(_projectId) onlySale {
        projects[_projectId].sold = _quantity;
    }
 
    function closeProject(uint256 _projectId, bool _isGive) external nonReentrant onlyManager(_projectId) {
        ...
        SaleInfo[] memory saleInfos = sale.getSalesProject(_projectId);
        for (uint256 i; i < saleInfos.length; i++) {
            SaleInfo memory saleInfo = saleInfos[i];

            address[] memory accountBuyers = sale.getBuyers(saleInfo.id);
            ...
            if (!_project.isSingle && saleInfo.amount > 0) IOSB1155(_project.token).safeTransferFrom(address(sale), _project.manager, saleInfo.tokenId, saleInfo.amount, "");
            sale.close(_project, saleInfo, accountBuyers, _isGive);
        }
        projects[_projectId].status = ProjectStatus.ENDED;
        emit CloseProject(_projectId, _isGive);
    }

    function checkManager(uint256 _projectId, address _account) public view {
        if (projects[_projectId].isCreatedByAdmin) {
            require(setting.isSuperAdmin(_account) || setting.isAdmin(_account) || _account == projects[_projectId].manager, "Caller is not the manager");
        } else {
            require(_account == projects[_projectId].manager, "Caller is not the manager");
        }
    }

    function publish(ProjectInput memory _projectInput, SaleInput[] memory _saleInputs) external payable {
        ...
        require(msg.value == createFee, "Invalid createProjectFee");
        if (isCreatedByAdmin) {
            require(_projectInput.joinStart >= block.timestamp && _projectInput.joinStart < _projectInput.joinEnd, "Invalid join start");
            require(_projectInput.saleStart > _projectInput.joinEnd && _projectInput.saleStart < _projectInput.saleEnd, "Invalid sale start");
        }
        ...
        project.isRaise          = _projectInput.isRaise;
        project.isInstantPayment = _projectInput.isInstantPayment;
        project.joinStart        = _projectInput.joinStart;
        project.joinEnd          = _projectInput.joinEnd;
        ...
        project.amount           = sale.creates(_msgSender(), isCreateNewToken, project, _saleInputs);
        ...
        if (address(opFundReceiver).balance < opFundLimit) {
            Helper.safeTransferNative(opFundReceiver, msg.value);
        }
    ...
    function activeProject(uint256 _projectId, IDOInput memory _idoInput, uint256 _profitShare) external onlyAdmin {
        ProjectInfo storage project = projects[_projectId];
        require(!project.isCreatedByAdmin, "Cannot set approve project created by admin");
        require(project.status == ProjectStatus.INACTIVE, "Project must be inactive");
        require(_idoInput.joinStart >= block.timestamp && _idoInput.joinStart < _idoInput.joinEnd, "Invalid join start");
        require(_idoInput.saleStart > _idoInput.joinEnd && _idoInput.saleStart < _idoInput.saleEnd, "Invalid sale start");
        project.joinStart   = _idoInput.joinStart;
        project.joinEnd     = _idoInput.joinEnd;
        project.saleStart   = _idoInput.saleStart;
        project.saleEnd     = _idoInput.saleEnd;
        project.profitShare = _profitShare;
        project.status      = ProjectStatus.STARTED;

        emit ActiveProject(_projectId, _idoInput, _profitShare);
    }

    function end(uint256 _projectId) external projectExists(_projectId) onlySale {
        projects[_projectId].status = ProjectStatus.ENDED;
    }
    ...
    function setCreateProjectFee(uint256 _fee) external onlySuperAdmin {
        require(_fee > 0, "Invalid create project fee");
    ...
    function setActiveProjectFee(uint256 _fee) external onlySuperAdmin {
        require(_fee > 0, "Invalid active fee");
