...
//DATA
import { TokenType } from "../interfaces/IPathFinder.sol";
...
/// @title ClosePathResolver
/// @notice Special resolver designed for finding path to withdraw and swap all assets into underlying one
contract ClosePathResolver is ResolverConfigurator, IClosePathResolver {
    using PathOptionOps for PathOption[];
    using StrategyPathTaskOps for StrategyPathTask;

    SwapAggregator public swapAggregator;
    IWithdrawerOptions public curveLPWithdarwer;
    IWithdrawer public convexWithdrawer;
    IWithdrawer public yearnWithdrawer;

    /// @notice Pathfinder Component ID
    PathFinderComponent public override getComponentId =
        PathFinderComponent.CLOSE_PATH_RESOLVER;
    ...
    function findBestClosePath(
        ...
        uint256 cycles
    ) public override returns (StrategyPathTask memory result) {
        result = task;

        task = fixedWithdrawals(task);

        bool hasNext;

        for (uint256 i; i < cycles; ) {
            StrategyPathTask memory current = withdrawalsOptions(
                task,
                pathOptions
            );

            if (current.isBetter(result)) {
                result = current;
            }

            if (hasNext) {
                (pathOptions, hasNext) = pathOptions.nextOption();
            } else {
                break;
            }
    ...
    function findClosePath(
        ...
        task = fixedWithdrawals(task);
        return withdrawalsOptions(task, pathOptions);
    }

    function fixedWithdrawals(StrategyPathTask memory task)
        internal
        view
        returns (StrategyPathTask memory)
    {
        task = convexWithdrawer.withdrawAllTokens(task);
        task = yearnWithdrawer.withdrawAllTokens(task);
        return task;
    }

    function withdrawalsOptions(
        StrategyPathTask memory task,
        PathOption[] memory pathOptions
    ) internal returns (StrategyPathTask memory) {
        task = curveLPWithdarwer.withdrawAllTokens(task, pathOptions);
        task = swapAggregator.swapAllNormalTokens(task, task.target);
        return task;
    }
}
