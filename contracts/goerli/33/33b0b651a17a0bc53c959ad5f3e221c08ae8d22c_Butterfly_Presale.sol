...
contract Butterfly_Presale is Ownable {
    using SafeMath for uint256;

    IBEP20 public token;
    IBEP20 public USDT;
    uint256 public TokenPriceETH = 120000000000000000000000; // 120000 tokens per eth
    uint256 public TokenPriceUSDT = 100000000000000000000;  //100  tokens per USDT
    event Deposited(address indexed user, uint256 amount);
    event Recovered(address token, uint256 amount);
    bool public presaleStatus;
    mapping(address => uint256) public deposits;

    constructor(IBEP20 _token,IBEP20 _USDT)  {
        token = _token;
        USDT = _USDT;
        presaleStatus = true;
    }
    ...
    function getTokenUSDT(uint256 value) public view returns(uint256)
    {
        return (TokenPriceUSDT*value)/1e6;
    }
    ...
    {
        require(presaleStatus == true, "Presale : Presale is finished");
        require(_amount > 0, "Presale : Unsuitable Amount");
        USDT.transferFrom(msg.sender, address(this), _amount);
        uint256 tokenAmount = getTokenUSDT(_amount);
        token.transfer(msg.sender, tokenAmount);
    ...
    function setRewardTokenPriceforETH(uint256 _count) external onlyOwner {
     ...
     function setRewardTokenPriceforUSDT(uint256 _count) external onlyOwner {
        TokenPriceUSDT = _count;
    }
...
// Better logics with 18 decimals 0x9dc634397CF9EBfc88678B9f8c869C9160Ccf3dD
// Tether usdt with 6 decimals 0x53c3E08192ECFdb441487249e320694f4F1818e4