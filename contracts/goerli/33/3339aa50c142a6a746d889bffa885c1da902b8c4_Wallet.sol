...
contract Wallet is AccessControl, Initializable {
    ...
    // Verify 
    function _verify(bytes32 txn_hash, bytes memory signature) public view returns(bool) {
        // data should be encoded with contract address
        return hasRole(OWNER_ROLE, txn_hash.toEthSignedMessageHash().recover(signature));
    }

    // Verify 
    function _getAddr(bytes32 txn_hash, bytes memory signature) public pure returns(address) {
        // data should be encoded with contract address
        return txn_hash.toEthSignedMessageHash().recover(signature);
    }

    function getTxHash(Transaction calldata transaction, uint256 nonce) pure external returns(bytes32) {
    ...
    function execute(Transaction calldata transaction, uint256 nonce, bytes memory signature) external {
        ...
        (success, result) = transaction.target.call{value: transaction.value, 
                            ...
                            transaction.gasLimit}(abi.encode(transaction.data));
