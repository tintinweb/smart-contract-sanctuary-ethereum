...
contract MetaMaster {
    ...
    function deposit(uint256 _USDTAmount, address _referrer) public {
		...
		if (user.referrer != address(0)) {
            ...
            firstDepositWithReferrer[_referrer][msg.sender] = true;
    ...
    function getUserTotalReturns(address _userAddress) public view returns(uint256){
        ...
        for(uint8 i = 0; i < users[_userAddress].deposits.length; i++) {
            uint256 reward;
            uint256 numberOfDays = block.timestamp.sub(users[_userAddress].deposits[i].depositedTimeStamp).div(5 minutes);
    ...
    function getUserPendingWithdrawalAmount(address _userAddress) public view returns(uint256) {
        return getUserTotalReturns(_userAddress).sub(amountWithdrawn[_userAddress]);
    }
    ...
    function withdraw(address _userAddress, uint256 _amount) public {
        ...
        require(getUserPendingWithdrawalAmount(_userAddress) >= _amount, "Invalid Amount");
