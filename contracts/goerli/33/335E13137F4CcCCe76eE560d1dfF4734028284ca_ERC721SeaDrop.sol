...
abstract contract TwoStepOwnable is Initializable {
    ...
    function initializeTwoStepOwnable() internal virtual onlyInitializing
    {
...
import "../solmate/utils/TwoStepOwnable.sol";
import "@openzeppelin/contracts-upgradeable/proxy/utils/Initializable.sol";
import "@openzeppelin/contracts/interfaces/IERC165.sol";

contract ERC721ContractMetadata is ERC721AUpgradeable, TwoStepOwnable, ISeaDropTokenContractMetadata {
    uint256 _maxSupply;
    ...
    function initializeERC721MetaData(string memory name, string memory symbol) initializerERC721A onlyInitializing internal virtual {
        __ERC721A_init(name, symbol);
        initializeTwoStepOwnable();
    }

    function _onlyOwnerOrSelf() internal virtual view {
        if (
    ...
    function setBaseURI(string calldata newBaseURI) external override virtual {
        _onlyOwnerOrSelf();
    ...
    function setContractURI(string calldata newContractURI) external override virtual {
        _onlyOwnerOrSelf();
    ...
    function provenanceHash() external view virtual override returns (bytes32) {
        return _provenanceHash;
    }


    function royaltyAddress() external view virtual returns (address) {
        return _royaltyInfo.royaltyAddress;
    }


    function royaltyBasisPoints() external view virtual returns (uint256) {
    ...
    function _cast(bool b) internal pure virtual returns (uint256 u) {
        assembly {
            u := b
        }
    }
...
ISeaDropTokenContractMetadata
} from "./ERC721aMetaDataUpgradable.sol";
...
ReentrancyGuardUpgradeable
{
    ...
    function initialize(string memory name, string memory symbol, address []memory allowedSeaDrop) initializer public {
        initializeERC721MetaData(name, symbol);
    ...
    function _onlyAllowedSeaDrop(address seaDrop) internal virtual view {
    ...
    function _updateAllowedSeaDrop(address[] calldata allowedSeaDrop) internal virtual {
    ...
    function updateCreatorPayoutAddress(address seaDropImpl, address payoutAddress) external virtual {
        _onlyOwnerOrSelf();
        _onlyAllowedSeaDrop(seaDropImpl);
    ...
    function updateSignedMintValidationParams(address seaDropImpl, address signer,
        SignedMintValidationParams memory signedMintValidationParams
    ) external virtual override {
        _onlyOwnerOrSelf();
        _onlyAllowedSeaDrop(seaDropImpl);
    ...
    function safeTransferFrom(address from, address to, uint256 tokenId, bytes memory data) public payable override virtual onlyAllowedOperator(from) {
