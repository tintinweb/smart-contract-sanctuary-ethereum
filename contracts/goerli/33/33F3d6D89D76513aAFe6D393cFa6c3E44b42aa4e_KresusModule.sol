    ...
    */
        ...
        returns (bool, bool, bool, Signature)
    {
        ...
        if (methodId == SecurityManager.revokeGuardian.selector){
            return((VotingEnabled == false) ? (true, true, false, Signature.Owner):(true, true, false, Signature.OwnerandGuardianOrOwnerandKWG)); 
        }
        ...
        if (methodId == TransactionManager.multiCall.selector){ 
            return( (VotingEnabled == false) ? (!_locked, true, true, Signature.Owner):(!_locked, false, true, Signature.OwnerandGuardian) );    
        }
...
abstract contract KresusRelayer is BaseModule {
    ...
    */
    function removeQueue(address _vault, bytes32 _actionHash) internal returns(bool) {
        ...
        _relayer.queue[len - 1] = _relayer.queue[index];
        _relayer.queue.pop();
