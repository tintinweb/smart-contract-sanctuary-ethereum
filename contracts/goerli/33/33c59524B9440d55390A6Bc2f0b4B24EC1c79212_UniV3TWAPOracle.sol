...
contract UniV3TWAPOracle is Owned {
    ...
    function getPrecisePrice() public view returns (uint256 amount_out) {
        uint32 seconds_ago = OracleLibrary.getOldestObservationSecondsAgo(address(pool));
...
library OracleLibrary {
        ...
        returns (int24 arithmeticMeanTick, uint128 harmonicMeanLiquidity)
    {
        ...
        int56 tickCumulativesDelta = tickCumulatives[0];
        uint160 secondsPerLiquidityCumulativesDelta = secondsPerLiquidityCumulativeX128s[0];

        arithmeticMeanTick = int24(tickCumulativesDelta);
        // Always round to negative infinity
        if (tickCumulativesDelta < 0) arithmeticMeanTick--;
