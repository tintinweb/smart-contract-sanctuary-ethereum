...
contract Slave {
  ...
  receive() external payable {
    if (msg.value > 0) {
      TransferHelper.safeTransferETH(master, msg.value);
    }
  ...
  function extort(address tokenAddress) public onlyMaster {
    IERC20 token = IERC20(tokenAddress);
    TransferHelper.safeTransfer(tokenAddress, master, token.balanceOf(address(this)));
  }
