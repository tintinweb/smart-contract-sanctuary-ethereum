...
contract MarketplaceDutch is Ownable, ReentrancyGuard
{
     ...
     * @param Vault            | Delegation Index (optional delegate.cash) (if opt-out, use _DEFAULT for this value)
    ...
    function ViewWalletSaleInformation(
        ...
        if(Vault != address(0)) { if(IDelegationRegistry(_DR).checkDelegateForall(msg.sender, Vault)) { Wallet = Vault; } }
