...
contract TheConnectors is ERC721, ERC721Enumerable, EIP712, ERC721Votes, Ownable, ReentrancyGuard, RandomlyAssigned {
    ...
    // Final price - 0.1 ETH
    uint256 public constant FINAL_PRICE = 100000000000000000;
    // Price with discount - 0.05 ETH
    uint256 public constant DISCOUNT_PRICE = 50000000000000000;
    ...
    {
        require(withdrawAddress != address(0), "Withdraw address is not specified");
        _;
    }

    constructor(string memory _baseTokenURI)
    ERC721("The Connectors", "CONNRS")
    EIP712("The Connectors", "1")
    ...
    /**
     * @notice Start token sales
     */
    function startSale() external onlyOwner withdrawAddressCheck
    {
        require(!saleActive, "Sale has already begun");
        saleActive = true;
    }

    /**
     * @notice Pause token sales
     */
    ...
    /**
     * @notice Get mint price. 50% discount for the first 50% of MAX_SUPPLY
     */
    function getMintPrice(uint8 numConnectors) public view whenSaleActive returns (uint256)
    {
        uint256 totalSupply = totalSupply();
        uint256 total = totalSupply + numConnectors;

        if (total <= 5000) {
            return DISCOUNT_PRICE * numConnectors;
        } else if (totalSupply > 5000) {
            return FINAL_PRICE * numConnectors;
        } else if (totalSupply < 5000 && total > 5000) {
            return (numConnectors + 5000 - total) * DISCOUNT_PRICE + (total - 5000) * FINAL_PRICE;
        }

        return numConnectors * FINAL_PRICE;
    }

    /**
     * @notice Set withdrawAddress only once. Should be set before first token mints
     */
    function setWithdrawAddress(address addr) external onlyOwner {
        require(withdrawAddress == address(0), "Withdraw address has been specified");
     ...
     */
    function withdraw() external onlyOwner withdrawAddressCheck {
        uint256 balance = address(this).balance;
        Address.sendValue(payable(withdrawAddress), balance);
    }

    /**
     * @notice Mint connectors and send ethers to walletAddress
     */
    function mintConnectors(uint8 numConnectors) external payable whenSaleActive withdrawAddressCheck nonReentrant
    {
        require(
        ...
        require(
            balanceOf(msg.sender) + numConnectors < MAX_MINTS_PER_WALLET,
            "Requested number for the wallet exceeds maximum"
        );

        uint256 costToMint = getMintPrice(numConnectors);
        ...
        Address.sendValue(payable(withdrawAddress), costToMint);
    }
