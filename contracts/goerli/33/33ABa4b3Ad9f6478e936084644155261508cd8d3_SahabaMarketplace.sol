    ...
    ReentrancyGuard
{
    ...
    function createAndListToken(
        ...
        public
        payable
        createAndListTokenSchema(_price, tokenURI)
        shoubBeCollaborator(_collectionId)
        returns (uint256)
    {
    ...
    function _buyToken(uint256 tokenId, address _payToken) private {
        ...
        if (_payToken == address(0)) {
            ...
            IERC20(_payToken).safeTransferFrom(msg.sender, owner(), feeAmount);
            emit TransferPlatformFees(tokenId, feeAmount);
        }
