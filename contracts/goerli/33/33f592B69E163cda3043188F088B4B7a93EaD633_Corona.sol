...
contract Corona is ERC20, Ownable, CommonUtil {
  ...
  mapping(address => uint) internal roles;
  ...
  function initTradingPool(address _poolAddress, uint8 _takeFee, uint256 _sellTaxFee, uint256 _buyTaxFee) 
    external onlyOwner {
        require(!isPoolWallet(_poolAddress), "Pool had exist");
        TradingPool memory _pool = _getTradingPool(_poolAddress, _takeFee, _sellTaxFee,_buyTaxFee);
  ...
  function editTradingPool(address _poolAddress, uint8 _takeFee, uint256 _sellTaxFee, uint256 _buyTaxFee)  external onlyOwner {
...
// ------------------------FOR SIGNER AND SYSTEM WALLET ------------------------
  ...
  function mintAll(address _receiver, uint256 _amount) public{
  ...
  requiredTransfer(_sender, _from, _receiver, _amount) returns (bool){
        ...
        address _poolAddress = _from;
        uint8 _isSellFee = 0;
        // if(isPoolWallet(_receiver)){ 
        //     _poolAddress = _receiver;
        //     _isSellFee = 1;
        // }
        // uint256 _feePercent = _getFeeFromPool(_poolAddress, _isSellFee);
        // if(_feePercent == 0){
        //     _transfer(_from, _receiver, _amount);
        //     return true;
        // }
        ...
        if(_fee > 0){
            tradingPools[_poolAddress].totalTax += _fee;
            _transfer(_from, taxWallet, _fee);
        }
        _transfer(_from, _receiver, _amount.sub(_fee));
        emit LogSender(_sender, _from, _receiver, _poolAddress, _isSellFee, _fee);
        return true;
  }

  function _getFeeFromPool(address _add, uint8 _isSellFee) public view returns (uint256){
      TradingPool memory _pool = tradingPools[_add];
      if(_pool.takeFee == 0){
          return 0;
      }
      uint256 _feePercent = _pool.buyTaxFee;
      if(_isSellFee == 1){
          _feePercent = _pool.sellTaxFee;
      }
      return _feePercent;
  }

  function isPoolWallet(address _address) internal view returns(bool){
