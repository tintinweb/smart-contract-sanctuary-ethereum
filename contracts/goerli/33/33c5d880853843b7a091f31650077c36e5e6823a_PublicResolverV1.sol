...
// File contracts/ResolverInterface.sol
...
// File contracts/resolvers/PublicResolverV1.sol
...
contract PublicResolverV1 is ResolverInterface {
  ...
  function _canWrite(uint256 name) internal view {
    ...
    address owner = domain.ownerOf(name);
    require(owner == msg.sender, "ResolverV1: not owner");
    require(!domain.isSuspended(name), "ResolverRegistry: domain suspended");
    uint256 expiresAt = domain.getDomainExpiry(name);
    require(expiresAt > block.timestamp, "ResolverV1: domain expired");
  }
