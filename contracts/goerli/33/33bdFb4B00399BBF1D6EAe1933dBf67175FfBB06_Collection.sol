...
contract Collection is Ownable, ERC721Consecutive, ERC2981 {
    ...
    function updateStatus(uint256 _tokenId, string memory _status) external onlyOwner {
        uint256 _id = _tokenId - 1;
        nfts[_id].status = _status;
        emit StatusUpdated(_id, _status);
    }

    function _mintConsecutive(address to, uint96 batchSize) internal virtual override returns (uint96) {
        uint96 first = uint96(_tokenIds.current());
