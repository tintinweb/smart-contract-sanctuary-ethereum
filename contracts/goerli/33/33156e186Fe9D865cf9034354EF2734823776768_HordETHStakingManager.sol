...
interface IStakingConfiguration {
    ...
    function permissionAddress() external view returns(address);
...
contract HordETHStakingManager is HETH {
    ...
    uint256 public amountETHInValidators;
    ...
    onlyMaintainer
    {
        ...
        // this should initially be 32 ETH until we can withdraw, then it will be 64 ETH (after we add another 32ETH)
        require(liquidationReserves >= amountToWithdraw, "conditions are not met");

        // Update pool information
        liquidationReserves = liquidationReserves.sub(amountToWithdraw);
        amountETHInValidators = amountETHInValidators.add(amountToWithdraw);
    ...
    whenNotPaused
    {
        ...
        uint256 amountForMint = getAmountOfHETHforETH(msg.value);
     ...
     */
    function getAmountOfHETHforETH(uint256 amountETH) public view returns (uint256) {
        if(totalHETHMinted == 0) {
            return amountETH;
        }
        uint256 totalETHInPossession = totalRewardsCollected //0.1
            .add(amountETHInValidators) //0
            .add(liquidationReserves); //2
     ...
     */
    function setTotalRewardsCollected(uint256 newRewardsAmount, uint256 newTotalETHStakedInValidators) external onlyMaintainer {
        uint256 rewardsBoundary = totalRewardsCollected.mul(20).div(100);
        uint256 ethInValidatorsBoundary = amountETHInValidators.mul(20).div(100);

        totalRewardsCollected = newRewardsAmount;
        amountETHInValidators = newTotalETHStakedInValidators;

        if (
            (totalRewardsCollected > 0 &&
            newRewardsAmount > totalRewardsCollected.add(rewardsBoundary) &&
            newRewardsAmount < totalRewardsCollected.sub(rewardsBoundary)) ||
            (amountETHInValidators > 0 &&
            newTotalETHStakedInValidators > amountETHInValidators &&
            newTotalETHStakedInValidators < amountETHInValidators.sub(ethInValidatorsBoundary))
        )
        {
            _pause();
        }
