...
contract AnyCallBase is Initializable {
  ...
  event ProcessRequest(uint requestId);
  event MyLog2(bytes);
  ...
  function getLastSentRequestId() public view returns(uint) {
    return _requestId.current();
  }


  function getSentRequestIds() public view returns(uint[] memory) {
    uint n = _sentRequestIds.length();
    uint[] memory requestIds = new uint[](n);
    for (uint i=0; i<n; i++) {
      requestIds[i] = _sentRequestIds.at(i);
    }
    return requestIds;
  }
    ...
    external returns (bool success, bytes memory result) 
  {
    ...
    ) = abi.decode(data, (uint, uint, bytes)); 
    
    if (_sentRequestIds.contains(requestId)) {
      _processResponse(requestId, requestCode, payload);
    } else {
      _processRequest(requestId, requestCode, payload);
    }
    success = true;
    result = '';
  }
  ...
  ) onlyOwner payable external
  {
    ...
    bytes memory message = abi.encode(
      // requestId is for tracking requests in capital; this request is generated by capital and 
      // sent in request; the response contains the request id to connect between response
      // and request
      requestId, 
      // requestCode is for action definition: what peheriphy should do?
      requestCode, 
      // payload is function (action) arguments
      payload
    ); 
    CallProxy(_anyCallProxy).anyCall{value: msg.value}(
      callee,
      message,
      chainId,
      0, // flags
