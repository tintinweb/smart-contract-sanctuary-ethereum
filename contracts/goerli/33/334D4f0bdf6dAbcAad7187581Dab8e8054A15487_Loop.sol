...
contract Mint is IERC721Receiver {
    ...
    address goerliAzukus = 0x10B8b56D53bFA5e374f38e6C0830BAd4ebeE33E6;

    uint256[] tokenIds;

    function mintApes() external payable {
        ApesInterface(goerliAzukus).mint{value: msg.value}(2);

        for (uint256 i; i < tokenIds.length; i++) {
            // transfer to transferAddress
            ApesInterface(goerliAzukus).safeTransferFrom(
                address(this),
                transferAddress,
                tokenIds[i]
            );
