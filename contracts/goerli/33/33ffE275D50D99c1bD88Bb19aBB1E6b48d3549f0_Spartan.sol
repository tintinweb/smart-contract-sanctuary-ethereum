...
/**s
 * @title ERC1155 Token Contract for Spartan drop
 * @notice This contract is meant to serve for both Semi- and Non-fungible tokens as part of the Spartan drop
 * @author Cryptoware.eth
 **/
contract Spartan is Ownable, ERC1155Pausable, ERC1155Supply, ERC1155Burnable, PaymentSplitter {
    ...
    /// @notice Supply  tokenId
    mapping(uint256 => uint256) public supplyPerTokenId;

    /// @notice The rate of minting per tokenId
    mapping(uint256 => uint256) public mintPrice;

    /// @notice Indicates if the salt has been already used
    mapping(bytes16 => bool) public usedSalt;
    ...
    /// @notice Event that indicates which salt has been used during minting
    event SaltUsed
    (
        bytes16 indexed salt
    );
    ...
    constructor(
        ...
        uint256[] memory supplyPerTokenId_,
        bytes32 root_,
    ...
    PaymentSplitter(payees_, shares_) 
    {
        name = name_;
        symbol = symbol_;
        _initialAddTokens(tokenIds_, mintCostPerTokenId_, supplyPerTokenId_);
        _merkleRoot = root_;
    }
    ...
    /// Create a new token with its own supply and mint price
    function incrementMaxId(uint256 mintPrice_, uint256 supply_)
        external
        onlyOwner
    {
        maxId++;
        mintPrice[maxId] = mintPrice_;
        supplyPerTokenId[maxId] = supply_;
    }
        ...
        onlyOwner
    {
        require(
            ...
            "SpartanNFT: Mint Price should be different than the previous price"
        );
     ...
     * @param tokenId_ id of token
     * @param tokenId_ new supply of specified token ID
     **/
    function changeMaxSupplyOfTokenId(uint256 tokenId_, uint256 newSupply)
        public
        onlyOwner
    {
        require(
            newSupply != supplyPerTokenId[tokenId_],
            "SpartanNFT: Max Supply should be different than the previous supply"
        );
        supplyPerTokenId[tokenId_] = newSupply;
    }
    ...
    function mint(
        ...
        (
            to != address(0),
            "SpartanNFT: Address cannot be 0"
        );
        require
        (
            received == mintPrice[tokenId] * amount,
            "SpartanNFT: Ether sent is not the right amount"
        );
        require
        (
            tokenId <= maxId,
            "SpartanNFT: Token id mismatch"
        );
        require(
            amount + totalSupply(tokenId) <= supplyPerTokenId[tokenId],
            "SpartanNFT: Amount mismatch"
        );
        require
        (
            !usedSalt[salt_],
            "SpartanNFT: Salt already used"
        );

        _merkleRoot > bytes32(0) && isAllowedToMint(proof_, salt_);
    ...
    function adminMint(
        ...
        require
        (
            amount + totalSupply(id) <= supplyPerTokenId[id],
            "SpartanNFT: Amount mismatch"
        );
     ...
     **/
    function isAllowedToMint(bytes32[] memory proof_, bytes16 salt_)
        internal
        view
        returns (bool)
    {
        require(
            MerkleProof.verify(
                proof_,
                _merkleRoot,
                keccak256(abi.encodePacked(salt_))
            ),
            "SpartanNFT: Please register before minting"
        );
        return true;
    }
    ...
    function changeMerkleRoot(bytes32 merkleRoot_) external onlyOwner {
        require(
            ...
            "SpartanNFT: Merkle root cannot be same as previous"
        );
    ...
    function _initialAddTokens(
        ...
        (
            ...
            "SpartanNFT: IDs/MintCost arity mismatch"
        );

        require
        (
            tokenIds_.length == supplyPerTokenId_.length,
            "SpartanNFT: IDs/Supply arity mismatch"
        );

        require
        (
            tokenIds_.length > 0,
            "SpartanNFT: Please add tokens"
        );

        for (uint256 i = 0; i < tokenIds_.length; i++) {
            mintPrice[tokenIds_[i]] = mintCostPerTokenId_[i];
            supplyPerTokenId[tokenIds_[i]] = supplyPerTokenId_[i];
        }
    }
