...
contract SeaGoldTrade is Initializable, OwnableUpgradeable {
    ...
    function acceptBId(
        ...
        require(
            covertToken + netamount <= salestruct.amount,
            "Amount is not equal"
        );
        t.transferFrom(salestruct.user, address(this), covertToken);
        t.transferFrom(salestruct.user, salestruct.seller, netamount);
        t.withdraw(covertToken);
        (bool sent, ) = buyBackAddress.call{value: covertToken}("");
    ...
    function transferNft(
        ...
        if (nftType == 721) {
            address ownerAddr = IERC721Upgradeable(_conAddr).ownerOf(tokenId);
    ...
    function calculaterewardsAndRoyaltyBid(
        ...
        uint256 royaltyPercent = betaLaunched == false ? royaltyPer : 0;
        uint256 buyBackPercent = betaLaunched == false ? buyBackFee : 0;
        ...
        require(netamount + covertToken <= amount, "Invalid calc");
        if (betaLaunched == false) {
            ...
            if (
                blackListedAddresses[salestruct.royaddr] == false &&
                royaltyRewards > 0 &&
                salestruct.royaddr != 0x0000000000000000000000000000000000000000
            ) {
    ...
    function calculaterewardsAndRoyalty(
        ...
        bytes32 message = prefixed(
            keccak256(abi.encodePacked(seller, royaltyPer, nonce))
        );
