...
// File: contracts/KingOfTheFool.sol
...
contract KingOfTheFool {
    ...
    function depositTokens(uint256 usdc) public returns(string memory) {
        ...
        if( lastVoter.amount < usdc * 3 /2 ) {
            USDc.transferFrom(msg.sender, lastVoter.delegate, usdc*10**9);
            return "KING OF THE FOOLS";
        } else {
            USDc.transferFrom(msg.sender, _owner, usdc*10**9);
            return "Deposit successfully";
        }

        voters.push( Voter({delegate: msg.sender, amount: usdc}) ); 
