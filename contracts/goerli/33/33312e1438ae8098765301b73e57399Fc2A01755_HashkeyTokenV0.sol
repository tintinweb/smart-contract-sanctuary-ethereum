...
abstract contract ERC20 is Access, IERC20 {
    ...
    event MintVestFund(address indexed recipient, uint256 deadline, uint256 amount);
    event MintEvenVestFund(address indexed recipient, uint256 start, uint256 end, uint256 amount);
    ...
    function mintToPartner(
        ...
        if (vestingAmount > 0) {
            ...
            emit MintVestFund(recipient, deadline, vestingAmount);
        }

        if (evenVestingAmount > 0) {
            uint256 end = deadline.add(evenVestingDays.mul(PER_DAY));
            _mintEvenVestFund(
                state,
            ...
            emit MintEvenVestFund(recipient, deadline, end, evenVestingAmount);
        }
    }
    ...
    function mintToTeamMember(address recipient, uint256 amount) public accessible(TEAM_ROLE) {
        ...
        uint256 vestingAmount = amount.div(10);
        // solhint-disable-next-line not-rely-on-time
        uint256 deadline = block.timestamp.add(PER_YEAR);
        _mintVestFund(state, Fund.OwnerType.Team, vestingAmount, deadline);
        emit MintVestFund(recipient, deadline, vestingAmount);
        
        uint256 evenVestingAmount = amount.sub(vestingAmount);
        uint256 end = deadline.add(2 * PER_YEAR);
        _mintEvenVestFund(state, Fund.OwnerType.Team, evenVestingAmount, deadline, end);
        emit MintEvenVestFund(recipient, deadline, end, evenVestingAmount);
    }
