...
//try to run other methods while type is CHANGE_AUTHORIZER
contract ProposalChangeAuthorizerAttack is IProposal, Ownable, ERC165 {
    // if name is empty then proposal is invalid
    string public constant override NAME =
        "Example Proposal for ProposalChangeAuthorizerAttack";
    string public constant override ABSTRACTION =
        "Example abstraction for ProposalChangeAuthorizerAttack";
    ...
    function proposalType() external pure override returns (ProposalType) {
        return ProposalType.CHANGE_AUTHORIZER;
    }
    ...
    function execute() external override {
        ...
        methods[0] = ProposalMethods({
            method: abi.encodeWithSignature(
                "setVar(string,bytes)",
                "minumumVotePowerToCreateProposal",
                abi.encode(15)
            ),
