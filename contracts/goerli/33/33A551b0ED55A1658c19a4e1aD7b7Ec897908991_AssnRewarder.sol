...
interface IAssnMasterChef {
    function _lpTokens(uint256 pid) external view returns (IERC20 _lpToken);

    function _pools(uint256 pid)
        external
        view
        returns (
            uint256 lpSupply,
...
contract AssnRewarder is IRewarder, Ownable {
    ...
    address public immutable ASSN_MC;
    ...
    constructor(
        ...
        IERC20 masterLpToken_
    ) {
        require(address(rewardToken_) != address(0), "Rearder: bad token");
        ...
        emit LogInit(rewardToken_, owner_, rewardPerSecond_, masterLpToken_);
    }
    ...
    function onReward(
    ...
    ) external override onlyAssnMC lock {
        require(IAssnMasterChef(ASSN_MC)._lpTokens(pid) == masterLpToken);
        ...
        if (user.amount > 0) {
            pending =
                user.unpaidRewards +
                (user.amount * pool.accToken1PerShare) /
                ACC_TOKEN_PRECISION -
                user.rewardDebt;
        ...
        user.rewardDebt =
            (lpTokenAmount * pool.accToken1PerShare) /
            ACC_TOKEN_PRECISION;
    ...
    modifier onlyAssnMC() {
        require(msg.sender == ASSN_MC, "Only MCV2 can call this function.");
        _;
    }
        ...
        returns (uint256 pending)
    {
        ...
        (uint256 lpSupply, , , ) = IAssnMasterChef(ASSN_MC)._pools(_pid);
    ...
    function updatePool(uint256 pid) public returns (PoolInfo memory pool) {
        ...
        if (block.timestamp > pool.lastRewardTime) {
            (uint256 lpSupply, , , ) = IAssnMasterChef(ASSN_MC)._pools(pid);
            if (lpSupply > 0) {
                uint256 time = block.timestamp - pool.lastRewardTime;
                uint256 rewardAmount = time * rewardPerSecond;
