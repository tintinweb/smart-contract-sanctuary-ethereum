...
contract ActivePool is OwnableUpgradeable, IActivePool {
    ...
    struct DefaultTracker {
        uint256 lpTokenAmount; // deposited lpToken tracker
        uint256 CUSDDebt; // debt
    }

    //lpToknAddress=>DefaultTracker
    mapping(address => DefaultTracker) public trackerData;
    ...
    function getLPTokenAmount(address _lpTokenAddress) external view override returns (uint) {
        return trackerData[_lpTokenAddress].lpTokenAmount;
    }

    function getCUSDDebt(address _lpTokenAddress) external view override returns (uint) {
        return trackerData[_lpTokenAddress].CUSDDebt;
    }


    // --- Pool functionality ---

    function sendLPToken(address _lpTokenAddress, address _account, uint _amount) external override {
        _requireCallerIsBOorTroveMorSP(_lpTokenAddress);
        DefaultTracker storage tracker = trackerData[_lpTokenAddress];
        tracker.lpTokenAmount = tracker.lpTokenAmount.sub(_amount);
        emit ActivePoolLPTokenBalanceUpdated(_lpTokenAddress, tracker.lpTokenAmount);
        emit LPTokenSent(_lpTokenAddress, _account, _amount);

        // (bool success, ) = _account.call{ value: _amount }("");
        // require(success, "ActivePool: sending ETH failed");
        IERC20Upgradeable(_lpTokenAddress).transfer(_account, _amount);
    }

    function increaseCUSDDebt(address _lpTokenAddress, uint _amount) external override {
        _requireCallerIsBOorTroveM();
        DefaultTracker storage tracker = trackerData[_lpTokenAddress];
        tracker.CUSDDebt = tracker.CUSDDebt.add(_amount);
        emit ActivePoolCUSDDebtUpdated(_lpTokenAddress, tracker.CUSDDebt);
    }

    function decreaseCUSDDebt(address _lpTokenAddress, uint _amount) external override {
        _requireCallerIsBOorTroveMorSP(_lpTokenAddress);
        DefaultTracker storage tracker = trackerData[_lpTokenAddress];
        tracker.CUSDDebt = tracker.CUSDDebt.sub(_amount);
        emit ActivePoolCUSDDebtUpdated(_lpTokenAddress, tracker.CUSDDebt);
    }
    ...
    function _requireCallerIsBOorTroveMorSP(address _lpTokenAddr) internal view {
        address _stabilityPoolAddress = collateralManager.find(_lpTokenAddr).stabilityPoolAddr;
        require(
            msg.sender == borrowerOperationsAddress ||
            msg.sender == troveManagerAddress ||
            msg.sender == _stabilityPoolAddress,
    ...
    function increaseLPToken(address _lpTokenAddress, uint _amount) override external {
        _requireCallerIsBorrowerOperationsOrDefaultPool();
        DefaultTracker storage tracker = trackerData[_lpTokenAddress];
        tracker.lpTokenAmount = tracker.lpTokenAmount.add(_amount);
        emit ActivePoolLPTokenBalanceUpdated(_lpTokenAddress, tracker.lpTokenAmount);
    }

    /**
     * only for test,fix the bug
     */
    function modifyTracker(address _lpTokenAddress, uint _lpAmount, uint _CUSDDebtAmount) public onlyOwner {
        DefaultTracker storage tracker = trackerData[_lpTokenAddress];
        if(_lpAmount != 0) {
            tracker.lpTokenAmount = _lpAmount;
        }
        if(_CUSDDebtAmount != 0) {
            tracker.CUSDDebt = _CUSDDebtAmount;
        }
