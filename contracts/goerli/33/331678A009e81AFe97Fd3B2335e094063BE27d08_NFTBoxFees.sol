...
interface INFTBoxFeeDiscount {
    ...
    function getTotalDiscount(address) external view returns(uint256);
}
...
import "./interface/INFTBoxFeeDiscount.sol";

interface getNFTOwner {
    function owner() external view returns (address);
}

contract NFTBoxFees is INFTBox,AccessControl {
    ...
    mapping(address => uint256) private NFTSwapFee;
    mapping(address => uint256) private ERC20SwapFee;
    mapping(address => RoyaltyFee) private NFTRoyaltyFee;
    mapping(address => uint96) private NFTRoyaltyMaxFee;

    PaymentTokenFee[] public paymentTokenFee;
    uint256 private nativePaymentFee;
    ...
    function setNFTRoyaltyMaxFee(address nftAddress, uint256 maxFee) public {
        require(hasRole(ADMIN, msg.sender) || hasRole(MANAGER, msg.sender), "Not member");
        require(maxFee >= 0, "fee must be not smaller than 0");
        NFTRoyaltyMaxFee[nftAddress] = uint96(maxFee);
    }

    function setNFTOwnerRoyaltyFee(address nftAddress, uint256 fee) public {
        require((getNFTOwner(nftAddress).owner() == msg.sender), "Not Owner");
        require(fee >= 0, "fee must be not smaller than 0");
        require(uint256(NFTRoyaltyMaxFee[nftAddress]) > fee, "fee must be smaller than max fee");
        NFTRoyaltyFee[nftAddress].feeAmount = uint96(fee);
        NFTRoyaltyFee[nftAddress].reciever = msg.sender;
    }
    ...
    /**
        Add payment token fee
    */
    function addPaymentTokenFee(address _tokenAddr, uint256 _percentage) external {
        require(hasRole(ADMIN, msg.sender) || hasRole(MANAGER, msg.sender), "Not member");
        require(_percentage > 100 && _percentage <= 10000, "fee must be greate than 0");
        PaymentTokenFee memory tokenFee;
        tokenFee.tokenAddr = _tokenAddr;
        tokenFee.percentage = uint96(_percentage);
        paymentTokenFee.push(tokenFee);
    }

    /**
        Set Native TokenFe percentage
    */
    function setNativePaymentFee(uint256 _percetage) external {
        require(hasRole(ADMIN, msg.sender) || hasRole(MANAGER, msg.sender), "Not member");
        require(_percetage > 100 && _percetage <= 10000, "fee must be greate than 0");
        nativePaymentFee = _percetage;
    }

    function getNativeTokenFee() public view returns(uint256) {
        return nativePaymentFee;
    }
    ...
    function getRoyaltyMaxFee(address nftAddress) public view returns(uint256) {
        return uint256(NFTRoyaltyMaxFee[nftAddress]);
    }

    function getNFTOwnerRoyaltyFee(address nftAddress) public view returns(uint256) {
        return uint256(NFTRoyaltyFee[nftAddress].feeAmount);
    }
    ...
    function getWhiteListTokenFeeByAddress(address _tokenAddr) public view returns(uint256) {
        for(uint256 i ; i < paymentTokenFee.length; ++i){
            if(paymentTokenFee[i].tokenAddr == _tokenAddr)
                return paymentTokenFee[i].percentage;
        }
        return 0;
    }

    function getPaymentTokenList() public view returns(PaymentTokenFee[] memory) {
        PaymentTokenFee [] memory _paymentToken = new PaymentTokenFee[](paymentTokenFee.length);
        for(uint256 i ; i < paymentTokenFee.length; ++i) {
            _paymentToken[i] = paymentTokenFee[i];
        }
        return _paymentToken;
    }
    /**
        Check Payment Token Fee for buying nfts as a fixed price
    */

    function _checkWhiteListToken(address _tokenAddr) external view returns(bool) {
        for(uint256 i ; i < paymentTokenFee.length; ++i) {
            if(paymentTokenFee[i].tokenAddr == _tokenAddr)
                return true;
        }
    ...
    function _whiteListTokenFee(address _tokenAddr) internal view returns(uint256) {
        for(uint256 i ; i < paymentTokenFee.length ; ++i) {
            if(paymentTokenFee[i].tokenAddr == _tokenAddr)
                return paymentTokenFee[i].percentage;
        }
    ...
    function _checkPaymentTokenFee(
        ...
        uint256 totalDiscount = INFTBoxFeeDiscount(nftSwapFeeDiscount).getTotalDiscount(buyer);
        if(_fixedPrice.tokenAddr == address(0)) {
            return _fixedPrice.amount * nativePaymentFee / 10000 - _fixedPrice.amount * nativePaymentFee * totalDiscount / 100000000;
        } else {
            return _fixedPrice.amount * _whiteListTokenFee(_fixedPrice.tokenAddr)/10000 - _fixedPrice.amount * _whiteListTokenFee(_fixedPrice.tokenAddr) * totalDiscount / 100000000;
        }
    }
    ...
    function _checkerc20Fees(
        ...
        uint256 totalDiscount = INFTBoxFeeDiscount(nftSwapFeeDiscount).getTotalDiscount(boxOwner);
        ...
        for(uint256 i ; i < _erc20Details.length ; ++i) {
            ...
            erc20fee -= erc20fee * totalDiscount / 10000;
    ...
    function _checknftgasfee(
        ...
        for(uint256 i ; i < _erc721Details.length ; ++i) {
            // uint256 nftDiscount = INFTBoxFeeDiscount(nftSwapFeeDiscount).getNFTDiscount(_erc721Details[i].tokenAddr);
        ...
        for(uint256 i ; i < _erc1155Details.length ; ++i) {
            // uint256 nftDiscount = INFTBoxFeeDiscount(nftSwapFeeDiscount).getNFTDiscount(_erc1155Details[i].tokenAddr);
        ...
        return (erc721Fee + erc1155Fee + gasFee) * (10000-totalDiscount)/10000;
    }
    ...
    function _checkRoyaltyFee(
        ...
        for(uint256 i ; i < _erc721Details.length ; ++i) {
            ...
            royalty[nftIndex].feeAmount -= uint96(royalty[nftIndex].feeAmount * totalDiscount / 10000);
