...
contract CaffeinatedCreatures is ERC721AQueryable, Ownable {
    ...
    uint256 public              reserveSupply = 467; // Reserved tokens
     ...
     */
    function mintAllowlist(uint256 qty_, Coupon memory coupon_, uint256 allotted_) external {
        ...
        require(qty_ + addressToMinted[msg.sender]._allowlistMintsByAddress < allotted_ + 1, "Exceeds Max Allottment");

        // Verify quantity does not cause _totalMinted() to exceed maxSupply (excluding reserveSupply)
        require(_totalMinted() + qty_ < maxSupply - reserveSupply + 1, "Exceeds Max Supply");
    ...
    function mint(uint256 qty_) external {
        ...
        // Limit max per transaction to 1
        require(qty_ == 1, "Exceeded Max Per Transaction");

        // Limit max per wallet to 3
        require(_numberMinted(msg.sender) - addressToMinted[msg.sender]._allowlistMintsByAddress - addressToMinted[msg.sender]._reserveMintsByAddress < 3, "Exceeded Max Per Wallet");
