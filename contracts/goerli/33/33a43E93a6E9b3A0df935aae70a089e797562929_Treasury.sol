...
interface ITreasury {
    function receiveFromUserWithETH(address user) external payable;
...
contract Treasury is ITreasury {
    ...
    function convertEthToUSDC(address user) internal {
        uint256 deadline = block.timestamp + 15000; // using 'now' for convenience, for mainnet pass deadline from frontend!
        uint256[] memory amounts = getEstimatedETHforUSDC(msg.value * 95 / 100);
        uniswapRouter.swapETHForExactTokens{value: msg.value * 95 / 100}(
            amounts[1],
            getPathForETHtoUSDC(),
            user,
            deadline
    ...
    function getEstimatedETHforUSDC(uint256 ethAmount)
        public
        view
        returns (uint256[] memory)
    {
        return uniswapRouter.getAmountsOut(ethAmount, getPathForETHtoUSDC());
    }
