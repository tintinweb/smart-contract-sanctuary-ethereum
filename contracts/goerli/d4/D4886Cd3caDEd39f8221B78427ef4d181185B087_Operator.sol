...
import {PaygetNFT} from "./PaygetNFT.sol";
...
contract Operator is Ownable {
    ...
    mapping(address => uint256) public allowedToClaim;

    PaygetNFT public nftContract;
    ...
    error CantComputeRewards();

    function init(address nftContractAddress_) external {
        nftContract = PaygetNFT(payable(nftContractAddress_));
        fee = 0.00001 ether;
    }
    ...
    /**
     * @dev установить комиссию, которую взимаем с пользователя за рассчёт суммы вывода
     */
    ...
    /**
     * @dev узнать владельца токена по ид
     */
    ...
    /**
     * @dev узнать сколько токенов было сминчено
     */
    ...
    /**
     * @dev зачислить юзеру награду за нфт. это может делать только бэкэнд
     */
    function spendRewards(address to, uint256 amount) external onlyOwner {
        allowedToClaim[to] += amount;
    }

    /**
     * @dev запросить вывод. пользователь вызывает эту функцию чтобы рассчитать вознаграждение
     */
    function requestReward() external payable {
        if (msg.value != fee) {
            revert IncorrectFeeValue();
        }
        (bool success, ) = payable(owner()).call{value: msg.value}(""); // комиссия отправляется владельцу
        if (!success) {
    ...
    function withdrawRewards() external {
        uint256 reward = allowedToClaim[msg.sender];
        if (reward == 0) {
            revert NoRewards();
        }
    ...
    /**
     * @dev пополнить контракт and рассчитать сколько дивов сейчас полагается
     */
    receive() external payable {
        fixedBalance += msg.value;
        if (nftContract.totalSupply() == 0 || msg.value < 0.001 ether) {
            revert CantComputeRewards();
        }
        dividendPerShare = fixedBalance / nftContract.totalSupply();
        emit DividendsComputed(block.timestamp, dividendPerShare);
    }
}
...
contract PaygetNFT is BaseERC721A, Pausable {
    ...
    uint256 mintToOwnerCount;

    /**
     * @dev Поскольку надо изначально наминтить владельцу 50_000 токенов, нужно вызвать эту функцию 5 раз чтобы уложиться в лимит блока
     */
    function mintToOwner() external {
        if (mintToOwnerCount < 5) {
            _mint(owner(), 10000);
            mintToOwnerCount++;
        }
    }

    function mintToOwnerFORTest() external {
        _mint(owner(), 1000);
    }
        ...
        returns (string memory)
    {
        ...
        return
            bytes(baseURI).length != 0
                ? string(abi.encodePacked(baseURI, FILEHASH))
                : "";
    }
