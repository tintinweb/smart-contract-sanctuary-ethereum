...
contract DEINNFTStaking is IDEINNFTStaking, ERC1155HolderUpgradeable, AbstractDependant {
    using SafeMath for uint256;
    using Math for uint256;
    using EnumerableSet for EnumerableSet.UintSet;

    uint256 public constant MAX_NFT_LOCKED = 30;

    address public deinNFT;
    IClaimVoting public claimVoting;

    mapping(address => EnumerableSet.UintSet) internal _lockedNFTs; // voter -> tokenIDs

    mapping(address => uint256) internal _totalStakedUser; // voter -> stakedAmount

    uint256 public override totalLocked;

    event Locked(address indexed user, uint256 indexed tokenID);
    event Unlocked(address indexed user, uint256 indexed tokenID);
    event PenaltyApplied(address indexed user, uint256 penalty);

    modifier onlyClaimVoting() {
        require(msg.sender == address(claimVoting), "DEINNFTStaking: Not a CV");
        _;
    }

    function __DEINNFTStaking_init() external initializer {
    ...
    function setDependencies(IContractsRegistry _contractsRegistry)
        ...
        onlyInjectorOrZero
    {
        deinNFT = _contractsRegistry.getDEINStakingContract();
        ...
        returns (bool _isLocked)
    {
        ...
        for (uint256 i = 0; i < tokenIDs.length; i = uncheckedInc(i)) {
            if (!_lockedNFTs[user].contains(tokenIDs[i])) {
                _isLocked = false;
            }
        }
    }

    function lockNFT(uint256 tokenID) external override {
        address user = msg.sender;
        require(IStaking(deinNFT).ownerOf(tokenID) == user, "DEINNFTStaking: Not token owner");
        require(_lockedNFTs[user].length() < MAX_NFT_LOCKED, "DEINNFTStaking: Too many locked");
        _lockedNFTs[user].add(tokenID);
        uint256 staked = IStaking(deinNFT).getFullStakedAmount(tokenID);
        _totalStakedUser[user] = _totalStakedUser[user].add(staked);
        totalLocked = totalLocked.add(staked);
        IERC1155Upgradeable(deinNFT).safeTransferFrom(user, address(this), tokenID, 1, "");
        emit Locked(user, tokenID);
    }

    function unlockNFT(uint256 tokenID) external override {
        address user = msg.sender;
        require(_lockedNFTs[user].contains(tokenID), "DEINNFTStaking: Not locked token");
        require(claimVoting.canUnlock(tokenID, user), "DEINNFTStaking: Used in voting");
        _lockedNFTs[user].remove(tokenID);
        uint256 staked = IStaking(deinNFT).getFullStakedAmount(tokenID);
        _totalStakedUser[user] = _totalStakedUser[user].sub(staked);
        totalLocked = totalLocked.sub(staked);
        IERC1155Upgradeable(deinNFT).safeTransferFrom(address(this), user, tokenID, 1, "");
        emit Unlocked(user, tokenID);
    }

    function applyPenalty(address user, uint256 penalty) external override onlyClaimVoting {
        uint256 totalStaked = _totalStakedUser[user];
        uint256 length = _lockedNFTs[user].length();

        uint256[] memory tokenIDs = new uint256[](length);
        uint256[] memory penalties = new uint256[](length);

        penalty = Math.min(penalty, totalStaked);

        for (uint256 i = 0; i < length; i = uncheckedInc(i)) {
            tokenIDs[i] = _lockedNFTs[user].at(i);
            uint256 staked = IStaking(deinNFT).getFullStakedAmount(tokenIDs[i]);
            penalties[i] = penalty.mul(staked).uncheckedDiv(totalStaked);

            _totalStakedUser[user] = _totalStakedUser[user].sub(penalties[i]);
            totalLocked = totalLocked.sub(penalties[i]);
        }

        IDEINStaking(deinNFT).applyVoterPenalty(tokenIDs, penalties, user);
        emit PenaltyApplied(msg.sender, penalty);
    }

    function removeLockedNFT(address user, uint256 tokenID) external override {
        require(msg.sender == deinNFT, "DEINNFTStaking: Not allowed");
        _lockedNFTs[user].remove(tokenID);
    }
}
