  ...
  StashMarketLender
{
  ...
  constructor(address payable treasury, uint16 feeBasisPoints) StashMarketFees(treasury, feeBasisPoints) {}

  function supportsInterface(bytes4 interfaceId) public view override returns (bool) {
    return interfaceId == type(IStashMarket).interfaceId || super.supportsInterface(interfaceId);
  }
}
 ...
 * @author batu-inal & HardlyDifficult
 */
abstract contract StashMarketBuy is IStashMarket, StashMarketFees, StashMarketTerms {
  ...
  function buy(uint256 rentalId) external payable {
    ...
    terms.paymentToken.transferFunds(msg.sender, terms.seller, amount);
 ...
 * @author batu-inal & HardlyDifficult
 */
abstract contract StashMarketFees {
  using TokenTransferLibrary for address;

  address payable private treasury;
  uint16 private feeBasisPoints;
  ...
  function _payFees(address paymentToken, uint256 totalTransactionAmount) internal returns (uint256 feeAmount) {
    ...
    paymentToken.transferFunds(msg.sender, treasury, feeAmount);
  }
}
 ...
 * @author batu-inal & HardlyDifficult
 */
abstract contract StashMarketRenter is IStashMarket, ERC165, StashMarketCore, StashMarketFees, StashMarketTerms {
  using TokenTransferLibrary for address;
...
abstract contract StashMarketLender is IStashMarket, ERC165, StashMarketCore, StashMarketTerms {
  ...
  function setRentalTerms(
    ...
    if (nftType == NFTType.ERC721) {
      // require amount == 0 (or == 1 or just ignore it)?
    ...
    _emitRentalTermsSet(rentalId);
  }

  function _emitRentalTermsSet(uint256 rentalId) private {
    RentalTerms memory rentalTerms = getRentalTerms(rentalId);
    emit RentalTermsSet(
      rentalId,
      rentalTerms.nftContract,
      rentalTerms.tokenId,
      uint256(rentalTerms.amount),
      rentalTerms.nftType,
      msg.sender,
      rentalTerms.expiry,
      rentalTerms.pricePerDay,
      rentalTerms.lenderRevShareBasisPoints,
      rentalTerms.buyPrice,
      rentalTerms.paymentToken,
      uint256(rentalTerms.maxRentalDays)
    );
  }
...
contract $StashMarket is StashMarket {
    ...
    constructor(address payable treasury, uint16 feeBasisPoints) StashMarket(treasury, feeBasisPoints) {}
...
abstract contract $StashMarketBuy is StashMarketBuy {
    ...
    constructor(address payable _treasury, uint16 _feeBasisPoints) StashMarketFees(_treasury, _feeBasisPoints) {}
