...
contract IpfsRentFileManager {
    ...
    //State Variables
    mapping(string => mapping(string => File)) private s_files;
    ...
    function uploadFile(
        ...
        if (s_files[_houseId][_hashFile].category != FileCategory.Empty) {
        ...
        s_files[_houseId][_hashFile] = fileInformation;
     ...
     */
    function deleteFile(string memory _hashFile, string memory _houseId) external {
        if (s_files[_houseId][_hashFile].category == FileCategory.Empty) {
            revert FileDoesNotExist(msg.sender, _hashFile, 'A file with this hash does not exist');
        }

        if (msg.sender != s_files[_houseId][_hashFile].fileOwner) {
    ...
    function getFileOwner(
        ...
        return s_files[_houseId][_hashFile].fileOwner;
    }
    ...
    function getFileSize(
        ...
        return s_files[_houseId][_hashFile].size;
    }
    ...
    function getFileCategory(
        ...
        return uint256(s_files[_houseId][_hashFile].category);
    }
