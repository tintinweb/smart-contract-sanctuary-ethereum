...
import "./IBloomCollectionInitializer.sol";

contract BloomCollectionFactory is Ownable {
	...
	function createCollection(
		...
		require(bytes(symbol_).length > 0, "BloomCollectionFactory: Symbol is required");
		...
		IBloomCollectionInitializer(proxy).initialize(payable(msg.sender), name_, symbol_, maxSupply_, maxPerWallet_, price_, baseURI_);
	...
	function _updateImplementation(address _implementation) private {
		require(_implementation.isContract(), "BloomCollectionFactory: Implementation is not a contract");
...
interface IBloomCollectionInitializer {
	function initialize(
