...
contract KodachiToken is Context, IERC20, Ownable {
    ...
    constructor () {        
        IKodachiswapRouter02 _kodachiswapRouter = IKodachiswapRouter02(0x8CA59877F5e6c1F3d56Af2f24305fD21Ac21eBdD);
    ...
    function liquidFee(address account) public view returns (uint256) {
    ...
    function _transfer(
        ...
        if(!oracleDisabled && takeFee) {
            ...
            if(oracle != address(0)) {
                try IOracle(oracle).consult(address(this), (amount * liquidFee(from))/1000) returns (uint144 price) {
