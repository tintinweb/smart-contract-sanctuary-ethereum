    ...
    PausableUpgradeable
{
    ...
    event ClaimStartUpdated(
     ...
     struct lockInStruct{
        uint256 lockInBonus;
        uint256 lockInTimeStartedAt;
        uint256 lockInTimeEndsAt;
        uint256 lockInAmount;
        bool lockInIsLocked;
    }

    mapping(address => lockInStruct) lockIN;
    ...
    function initialize(
        ...
        maxTokensToBuy = 10_000_000;
        baseDecimals = (10**18);
        token_amount = [10_000_000, 20_000_000, 30_000_000];
        token_price = [
            10000000000000000, // 0.01 USD
            200000000000000, // 0.02 USD
            300000000000000 // 0.03 USD
        ];
    ...
    /**
     * @dev To blacklist users from buying.
     * @param users user addresses array
     */
    function addToBlacklist(address[] calldata users) external onlyOwner {
        require(users.length > 0, "Invalid users length");
    ...
    modifier checkSaleState(uint256 amount) {
        ...
        require(!blackListed[_msgSender()], "User blacklisted");
        _;
    }
    ...
    function claim() external whenNotPaused returns (bool) {
     ...
     function lockFunds(uint256 _timePeriod, uint256 _amount) public payable returns(bool){
        require(lockIN[msg.sender].lockInIsLocked == false ,"Already Locked");
        require(_amount > 0, "Nothing to Lock");
        bool success = IERC20Upgradeable(saleToken).transfer(address(this),_amount);
        require(success, "Token transfer failed");
        uint256 bonus;
        ...
        else if(_timePeriod == 6){
            bonus = (_amount /100) * 3;
        }
         else  if(_timePeriod == 9){
            bonus = (_amount /100) * 6;
        }
          else if(_timePeriod == 12){
            bonus = (_amount /100) * 9;
        }
         else  if(_timePeriod == 15){
            bonus = (_amount /100) * 12;
        }
         else  if(_timePeriod == 18){
            bonus = (_amount /100) * 16;
        }
         else  if(_timePeriod == 21){
            bonus = (_amount /100) * 20;
        }
          else if(_timePeriod == 24){
            bonus = (_amount /100) * 25;
        }
        else{
            bonus = 0;
            revert("Invalid LockIn Period");
        }
        lockIN[msg.sender].lockInIsLocked = true;
        lockIN[msg.sender].lockInAmount = _amount;
        lockIN[msg.sender].lockInBonus = bonus;
        lockIN[msg.sender].lockInTimeStartedAt = block.timestamp;
        // 2630000 = 1 months in seconds
        lockIN[msg.sender].lockInTimeEndsAt = lockIN[msg.sender].lockInTimeStartedAt + (_timePeriod * 2630000);
       
        return true;
    }

    function bonusStatus()public view returns(bool lockInIsLocked_,uint256 lockInAmount_, uint256 lockInBonus_, uint256 lockInTimeStartedAt_, uint256 lockInTimeEndsAt_) {

           return (lockIN[msg.sender].lockInIsLocked,lockIN[msg.sender].lockInAmount,
                    lockIN[msg.sender].lockInBonus,lockIN[msg.sender].lockInTimeStartedAt,lockIN[msg.sender].lockInTimeEndsAt );
    }
