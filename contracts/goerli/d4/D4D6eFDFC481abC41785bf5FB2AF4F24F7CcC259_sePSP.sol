...
contract TimeLockedERC20 is ERC20Permit, Ownable, Pausable {
    ...
    constructor(
        ...
        require(
            _timeLockDuration >= _minTimeLockDuration && _timeLockDuration <= _maxTimeLockDuration,
    ...
    function changeTimeLock(uint256 newTimeLockDuration) external onlyOwner {
        require(
            newTimeLockDuration >= minTimeLockDuration && newTimeLockDuration <= maxTimeLockDuration,
            "timelock out of range"
        );
    ...
    function requestWithdraw(uint256 _unstakingAmount) external {
        int256 id = userVsNextID[msg.sender]++;

        _burn(msg.sender, _unstakingAmount);

        WithdrawalRequest storage request = userVsWithdrawals[msg.sender][id];

        request.amount = _unstakingAmount;
        request.releaseTime = block.timestamp + timeLockDuration;
        request.status = WITHDRAW_STATUS.UNLOCKING;

        unlockingAssets += _unstakingAmount;

        emit Unstaked(id, msg.sender, _unstakingAmount);
    }
    ...
    function deposit(uint256 _assetAmount) public whenNotPaused {
    ...
    function _deposit(uint256 _assetAmount) internal whenNotPaused {
        ...
        emit Entered(msg.sender, _assetAmount);
    }
    ...
    function _withdraw(int256 id) internal returns (uint256) {
        ...
        WithdrawalRequest storage request = userVsWithdrawals[msg.sender][id];
        require(
            request.status == WITHDRAW_STATUS.UNLOCKING && request.releaseTime <= block.timestamp,
            "Cannot withdraw"
        );
    ...
    function reenter(int256 id) public whenNotPaused {
        ...
        WithdrawalRequest storage request = userVsWithdrawals[msg.sender][id];
        require(request.status == WITHDRAW_STATUS.UNLOCKING, "Cannot reenter");
        ...
        emit Reentered(id, msg.sender, _assetAmount);
    }
    ...
    function _permit(bytes memory permit) private {
        (bool success, ) = address(asset).call(abi.encodePacked(IERC20Permit.permit.selector, permit));
