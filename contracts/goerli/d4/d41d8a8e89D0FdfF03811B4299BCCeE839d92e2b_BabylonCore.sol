...
contract BabylonCore is Initializable, IBabylonCore, OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    function participateAllowlist(
        ...
        require(
            MerkleProof.verify(allowlistProof, restrictions.allowlistRoot, leaf),
            "BabylonCore: Not in the allowlist"
        );
    ...
    function isInAllowlist(uint256 id, address user, bytes32[] calldata allowlistProof) external view returns (bool) {
        bytes32 leaf = keccak256(abi.encodePacked(user));
        return MerkleProof.verify(allowlistProof, _listingRestrictions[id].allowlistRoot, leaf);
    }
}
