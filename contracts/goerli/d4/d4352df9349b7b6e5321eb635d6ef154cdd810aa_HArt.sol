...
contract HArt is RandomlyAssigned, Ownable {
    ...
    constructor(uint256 amount, uint256 startFrom, string memory baseURI) ERC721("Hart", "HART") RandomlyAssigned(amount, startFrom) {
        _BASE_URI_ = baseURI;
    }

    function mint(address[] calldata _recipients, string[] calldata _messages) external payable {
        require(block.timestamp >= 0 && block.timestamp <= 9999999999999);
        require(_recipients.length <= 10, "maximum 10 harts");
        require(availableTokenCount() >= _recipients.length);
        require(_recipients.length == _messages.length);

        uint cost = 0;
        for (uint i = 0; i < _messages.length; i++) {
            bytes memory m = bytes(_messages[i]);
            if (m.length > 0) {
