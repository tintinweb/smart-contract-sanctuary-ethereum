...
contract huangfeiStaking is ERC20Burnable, Ownable {
  uint256 public constant NFT_BASE_RATE = 10 * 1e18;
  address public constant NFT_ADDRESS = 0xCeF40d5B7637CD5748832d0eE9C16E98022E1aC5;

  constructor() ERC20('HuangfeiToken', 'HFT') {}

  IERC721Enumerable private constant _NftIERC721 = IERC721Enumerable(NFT_ADDRESS);

  bool public STAKING_LIVE = false;
  bool public RETURN_LOCK = false;
  ...
  function stakeNftsByTokenIds(uint256[] memory tokenIds) public {
    require(STAKING_LIVE, 'STAKING NOT LIVE');

    for (uint256 i = 0; i < tokenIds.length; i++) {
      uint256 id = tokenIds[i];
      require(
        _NftIERC721.ownerOf(id) == msg.sender && NftTokenIdToStaker[id] == address(0),
        'TOKEN NOT YOURS'
      );
  ...
  function toggleStakingLive() external onlyOwner {
    STAKING_LIVE = !STAKING_LIVE;
  }
  ...
  function returnAllStakedNFTs() external payable onlyOwner {
    require(RETURN_LOCK == true, 'Staking locked');
  ...
  function sell(uint256 amount) public {
    require(amount > 0, "You need at least some tokens");
