...
contract SkyFarm is IERC721Receiver, FxBaseRootTunnel, Ownable {
    ...
    function balanceOf(address owner) external view returns (uint256) {
        ...
        if(skyverses.length + skyverseContract.balanceOf(owner) <= 0) return 0;
        return 1;
    }
    ...
    function stake(uint256 tokenId) external notPaused {
        require(skyverseContract.ownerOf(tokenId) == msg.sender, "Not owner");
    ...
    function unstake(uint256 tokenId) external notPaused {
        uint256 index = _verifyOwnership(stakedIds[msg.sender], tokenId);
        _clearIndex(msg.sender, index);
        skyverseContract.transferFrom(
            address(this),
            msg.sender,
            tokenId
    ...
    function stakeMultiple(uint256[] memory tokenIds) external notPaused {
        for (uint256 i = 0; i < tokenIds.length; i++) {
            require(skyverseContract.ownerOf(tokenIds[i]) == msg.sender, "Not owner");
    ...
    function unstakeMultiple(uint256[] memory tokenIds) external notPaused {
        for (uint256 i = 0; i < tokenIds.length; i++) {
            uint256 index = _verifyOwnership(stakedIds[msg.sender], tokenIds[i]);
            _clearIndex(msg.sender, index);
            skyverseContract.transferFrom(
                address(this),
                msg.sender,
    ...
    function _clearIndex(address user, uint256 _index) internal {
        if (stakedIds[user].length > 1) {
            unchecked {
                for (uint256 i = _index; i < stakedIds[user].length - 1; i++) {
