...
contract EthericeStaking is Ownable, ReentrancyGuard {
    ...
    */
    function receiveDivs() external payable {
        ...
        uint256 _day = ( _tokenContract.calcDay() - 1);
        require(_day > 0);
        ...
        for (uint256 i = 1; i <= _daysToSplitRewardsOver; i++) {
    ...
    */
    ...
    function newStake(uint256 _amount, uint256 _days) external {
        ...
        require(
            _tokenContract.balanceOf(msg.sender) >= _amount,
            "Not enough balance"
        );
        
        uint256 allowance =  _tokenContract.allowance(msg.sender, address(this));
        require(
           allowance >= _amount, "not enough allowance"
        );
     ...
     */
    function collectStake(uint256 _stakeId) external {
    ...
    function cancelLoanRequest(uint256 _stakeId) public {
    ...
    function buyStake(uint256 _stakeId) external payable {
        ...
        dayDivendPool[_currentDay] += (_stake.forSalePrice / 100) * 5;
        devFees += (_stake.forSalePrice / 100) * 5;

        payable(_stake.owner).transfer(_stake.forSalePrice);
     ...
     */
    function flushDevTaxes() external {
        address _devWallet = _tokenContract.dev_addr();
        payable(_devWallet).transfer(devFees);
    }
    ...
    function fillLoan(uint256 _stakeId) external payable {
        ...
        // Deduct fees
        uint256 _loanAmount = (_loan.loanAmount / 100) * 98;
