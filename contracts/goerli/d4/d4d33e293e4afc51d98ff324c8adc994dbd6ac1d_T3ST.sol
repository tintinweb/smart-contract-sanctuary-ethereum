...
contract T3ST is ERC721A, Ownable {
    ...
    constructor( uint256 newMaxSupply, address newSignerAddress, string memory newBaseURI, address newAuthorWallet, uint256 newAuthorMaxPayment ) ERC721A("T3ST", "T3ST") {
    ...
    function authorPaid() external view onlyOwner returns(uint256) {
        return _authorAlreadyPaid;
    }

    function authorMaxPayment() external view onlyOwner returns(uint256) {
        return _authorMaxPayment;
    }
