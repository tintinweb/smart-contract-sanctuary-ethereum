...
contract RentNFT is IERC721Receiver, AccessControl {
    ...
    address immutable erc721tokenImplementation;
    ...
    function rentnftinternal(
        ...
        uint256 commision = (rent * rentCommision) / 100;
        uint256 totalRent = rent - commision;
        nftdetail.rentcollected += totalRent;
        rentCommisionFunds = rentCommisionFunds + commision;
        inventoryinstance.update(nftdetail, _itemID);

        addRentedNFT(wrapped(clone), _startAt, _endAt, _itemID, totalRent);
    }
        ...
        )
    {
        ...
        inventory = inventoryinstance.getByLimitAndAddress(
            limit,
            offset,
            _owner
        );
        ...
        returns (NFTRented[] memory)
    {
        ...
        for (uint256 i = 0; i < rentedindex[_itemID].length; ) {
    ...
    function buyListedNFT(uint256 _itemID) external {
        ...
        inventoryinstance.updateRentWithdrawn(
            _itemID,
            inventoryinstance.getRentWithdrawn(_itemID) + nftOwnerRemain
        );

        IERC20(feetoken).transferFrom(msg.sender, owner, ownerCommision);
    ...
    function inventoryCanWithdraw(uint256 _itemID)
        public
        view
        returns (uint256)
    {
        ...
        uint256 rentLeft = rent - inventoryinstance.getRentWithdrawn(_itemID);
        return rentLeft;
    }

    function withDrawFundsInventory(uint256 _itemID) external {
        Inventory memory nftinventory = inventoryinstance.get(_itemID);
        require(
            msg.sender == nftinventory.owner,
            "only owner can do this action"
        ...
        uint256 rentLeft = inventoryCanWithdraw(_itemID);

        inventoryinstance.updateRentWithdrawn(
            _itemID,
            inventoryinstance.getRentWithdrawn(_itemID) + rentLeft
        );

        IERC20(feetoken).transfer(msg.sender, rentLeft);
        emit Withdrawn(rentLeft, address(msg.sender), _itemID);
    }

    // Only package  roles functions
    ...
    function _deletePackage(uint256 packagageID)
        external
        onlyRole(PACKAGE_ROLE)
    {
        require(packagageID > 0, "zero package id is reserved");
        delete packages[packagageID];
        emit addUpdatepackage(packagageID, 0, 0);
    }

    // Only Admin functions

    function changeFeetoken(address _feeToken) external onlyOwner {
        feetoken = _feeToken;
    }
    ...
    function changeBuySellCommision(uint256 _buySellCommision)
        external
        onlyOwner
    {
    ...
    function withDrawFundsOwner() external onlyOwner {
        ...
        IERC20(feetoken).transfer(msg.sender, ownerFund);
    ...
    function unlistedNFTCheck(uint8 _itemID) public view returns (bool) {
        bool unlist = isavailableforrent(_itemID, block.timestamp, 2**256 - 1);
        return unlist;
    }
...
function initialize(uint256 _endAt, uint256 _startAt, string memory _name, string memory _symbol, string memory _tokenURI,address owner, uint256 _tokenID ) external {
...
contract NFTInventory is Ownable {
    ...
    function update(Inventory memory inventory, uint256 _itemID)
        external
        onlyOwner
    {
    ...
    function updateRentWithdrawn(uint256 _itemID, uint256 _rentWithdrawn)
        external
        onlyOwner
    {
...
abstract contract wrappedabstract is ERC721Upgradeable, OwnableUpgradeable {
   ...
   function _initialize(string memory _name,string memory _symbol) initializer public {
