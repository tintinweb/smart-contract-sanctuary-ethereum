...
contract MochiMo is ERC721A, Ownable, PaymentSplitter {
    ...
    uint256 public whitelistSupply = 3111;
    ...
    function mintWhitelist(uint256 amount, bytes calldata signature) external payable mintCompliance(amount) {
        ...
        require(totalSupply() + amount <= whitelistSupply + VIP_SUPPLY, "Whitelist sale is sold out!");
    ...
    function mintTeam(uint256[] calldata amount, address[] calldata recipient) external onlyOwner {
        require(amount.length == recipient.length, "Provide equal amount and recipients!");

        for (uint256 i = 0; i < recipient.length; ++i) {
            require(totalSupply() + amount[i] <= MAX_SUPPLY, "Can't mint that many via team mint!");
