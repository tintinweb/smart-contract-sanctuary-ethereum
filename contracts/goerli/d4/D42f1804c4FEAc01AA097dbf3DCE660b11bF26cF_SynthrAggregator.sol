...
contract LiquidatorRewardsAggregator {
    ...
    function _updateAccumulatedShare(bytes32 currencyKey, uint _increasedShare) internal {
...
contract SynthrAggregator is Ownable, CollateralAggregator, DebtShareAggregator, LiquidatorRewardsAggregator, RewardEscrowV2Aggregator, SynthAggregator {
    ...
    function mintSynth(
        ...
        if (_synthKey != bytes32(0) && _synthAmount != 0) {
            ...
            _updateEntry(collateralKeys, _debtShareBalanceOf[_accountForCollateral], _accountForCollateral);
        }
    }
    ...
    function burnSynth(
        ...
        bytes32[] memory collateralKeys = IWrappedSynthr(_wrappedSynthr).getAvailableCollaterals();
        _updateEntry(collateralKeys, _debtShareBalanceOf[accountForSynth], accountForSynth);
    }
    ...
    function liquidate(
        ...
        bytes32[] memory collateralKeys = IWrappedSynthr(_wrappedSynthr).getAvailableCollaterals();
        _updateEntry(collateralKeys, currentDebtShareBalance, account);

        _burnDebtShare(account, debtShare);

        if (currentDebtShareBalance > debtShare) {
            _updateAccumulatedShare(collateralKey, notifyAmount / (currentDebtShareBalance - debtShare));
        }
