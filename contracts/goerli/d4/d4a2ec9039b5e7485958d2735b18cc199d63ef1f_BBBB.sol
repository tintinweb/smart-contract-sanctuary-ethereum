...
contract BBBB is ERC20, Ownable {
    ...
    uint256 private _numTokensSellToAddToETH = 1250 * 10 ** _decimals;
    ...
    function _transfer(address from, address to, uint256 amount) internal override {
        ...
        if ((from == uniswapV2Pair || to == uniswapV2Pair) && !inContractLiquify) {
            // Determine if contract token balance needs to be swapped and added to liquidity
            if (from != uniswapV2Pair) {
                ...
                if (contractLiquidityBalance >= _numTokensSellToAddToLiquidity) {
                    contractLiquify(_numTokensSellToAddToLiquidity);
                }

                if ((_marketingReserves) >= _numTokensSellToAddToETH) {
                    contractSwapTokensForEth(_numTokensSellToAddToETH);
            ...
            if (_isExcludedFromFee[from] || _isExcludedFromFee[to]) {
                ...
                if (from == uniswapV2Pair) {
                    require((amount + balanceOf(to)) <= maximumHold, "Transfer exceeds maximum amount");
                }

                uint256 marketingShare = ((amount * developmentFee) / 100);
