...
contract Farm is ERC721AWithRoyalties, Pausable, PaymentSplitter {
    ...
    event HorseStableChange(uint256 horseID, uint256 farmID, uint256 oldFarmID);
    ...
    function wrapLandtoFarmReq(
        ...
        if (
            ...
            Stabler(StableAddress).transferFrom(
                msg.sender,
                address(this),
                stableID
            );
        ...
        _requests[msg.sender] = FarmRequest(
            ids,
            false,
            farmName,
            _tokenIdCounter
    ...
    function wrapLandtoFarmFul(address to) external {
        ...
        if (_requests[to].upgrade == false) {
            ...
            for (uint256 i = 0; i < ids.length; i++) {
                ...
                LandContract.transferFrom(to, address(this), ids[i]);
            }
    ...
    function upgradeFarmReq(
        ...
        _requests[msg.sender] = FarmRequest(farms[farmID], true, "U", farmID);
    }
    ...
    function _stableHorse(
        ...
        uint256 blockExp = (block.timestamp + (stableTerm * 1 minutes));
        Horser HorseContract = Horser(HorseAddress);
    ...
    function _stableRequest(
        ...
        stableRequests[farmID].push(
            StableRequest(
                ...
                farmID,
                msg.sender,
                block.timestamp + (stableTerm * 1 minutes)
            )
        );
    ...
    function _destable(uint256 farmID, uint256 horseID) internal {
        ...
        Horser HorseContract = Horser(HorseAddress);
        emit HorseStableChange(horseID, 0, HorseContract.checkStable(horseID));
    ...
    function changeFarmParams(
        ...
        require(ownerOf(farmID) == msg.sender, "MUST OWN FARM TO MAKE CHANGES");
        require(
            horses[farmID].length == 0,
    ...
    function _removeRequest(
        ...
        for (uint256 i = 0; i < stableRequestsByHorse[horseID].length; i++) {
            if (stableRequestsByHorse[horseID][i] == farmID) {
