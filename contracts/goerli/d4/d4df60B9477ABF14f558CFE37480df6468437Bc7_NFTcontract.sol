...
contract NFTcontract is ERC721Enumerable, ReentrancyGuard {
    ...
    modifier isCorrectPayment(string calldata _valid, uint256 _stage) {
        require(keccak256(abi.encodePacked(_valid)) == keccak256(abi.encodePacked(validKey)), "Wrong validation Key.");
        require(
            stageTime[_stage] <= block.timestamp,
            "Mint is not allowed yet."
        );
        if (_stage == 0) {
            require(
                stageTime[_stage] <= block.timestamp &&
                    stageTime[_stage + 1] > block.timestamp,
        ...
        returns (string memory)
    {
        ...
        if (_tokenId < smallestCnt) {
            ...
            for (index = _tokenId; index <= maxPublicMint; index++) {
