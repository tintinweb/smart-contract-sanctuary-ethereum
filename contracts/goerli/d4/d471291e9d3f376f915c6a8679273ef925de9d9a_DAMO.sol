...
contract DAMO is ERC721A, ERC4907A, ERC721AQueryable, ERC721ABurnable, ERC2981, Ownable, Pausable, AccessControl, DefaultOperatorFilterer {
    using Strings for uint256;

    string private baseURI = "ar://P4-OkT0mlPjPlGmp4XzoQiYLT44CemqDeG250lXI6qo/";

    bool public publicSale = false;
    uint256 public publicCost = 0.03 ether;
    bool public mintable = false;
    uint8 publicTicket = 1;
    ...
    mapping(uint8 => uint256) public presaleCost;
    mapping(uint8 => bool) public presalePhase;
    mapping(uint8 => bytes32) public merkleRoot;
    mapping(uint256 => uint8) public tokenTicket;
    mapping(uint8 => mapping(address => uint256)) public whiteListClaimed;

    mapping(uint256 => string) metadataURI;

    constructor() ERC721A("DAMOKEY", "DAMO") {
    ...
    function tokenURI(uint256 tokenId) public view virtual override(ERC721A, IERC721A) returns (string memory) {
        if (bytes(metadataURI[tokenId]).length == 0) {
            return string(abi.encodePacked(baseURI, _toString(tokenTicket[tokenId]), BASE_EXTENSION));
        } else {
            return metadataURI[tokenTicket[tokenId]];
        }
    }
     ...
     */
    function setMerkleRoot(bytes32 _merkleRoot, uint8 ticket) external onlyOwner {
    ...
    function setPublicTicket(uint8 ticket) external onlyRole(MINTER_ROLE) {
        publicTicket = ticket;
    }
    ...
    function setPresalePhase(bool _state, uint8 ticket) external onlyOwner {
        presalePhase[ticket] = _state;
    }

    function setPresaleCost(uint256 _preCost, uint8 ticket) external onlyOwner {
    ...
    function withdraw() external {
        Address.sendValue(payable(0xEc77367447d202f76d282695c2673Ca8fB69D2fd), address(this).balance);
    }
