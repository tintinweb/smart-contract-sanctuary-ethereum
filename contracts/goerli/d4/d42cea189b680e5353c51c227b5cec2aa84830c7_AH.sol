...
//        return ((((_tokenData[heartId].auxData)>>3)%1024) + 1);
    ...
    */
    ...
    function topUp(uint256 heartId, uint256 amt) public payable {
        require(msg.sender == ownerOf(heartId), "o");
        require(msg.value == amt*minSpendIncrement*getBurnMultiplier(heartId), "poor");
        _topUp(heartId, amt);
    }
    ...
    function printNew(uint256 burnMultiplier, HeartColor color) public payable {
        require(balanceOf(msg.sender) > 0, "need <3's");
        require(msg.value == baseBurnRate*burnMultiplier, "poor");

        uint256 justMinted = StorageLayerProto(STORAGE_LAYER_ADDRESS).mint(msg.sender, color);
...
//        require(getExpiryTime(heartId) < block.timestamp, "exp_liq");
        require((getExpiryTime(heartId) - 98*secondsPerIncrement) < block.timestamp, "exp_liq");
...
//        _tokenData[tokenId].auxData += newBurnMultiplier<<3;
    ...
    function _initExpiryTime(uint256 heartId) private {
        ...
        _heartsInfo[heartId].burnLastUpdated = uint64(curTime);
    ...
    function batchInitExpiryTime(uint256[] calldata heartIds) public onlyStorage {
        ...
        for (uint256 i = 0; i < heartIds.length; i++) {
            ...
            _heartsInfo[heartId].burnLastUpdated = curTime;
...
abstract contract StorageLayerProto {
    ...
    function mint(address to, HeartColor color) public virtual returns (uint256);
