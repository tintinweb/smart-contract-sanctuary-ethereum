...
interface CreatorCore {
    function mintExtensionNew(
        address[] calldata to, uint256[] calldata amounts, string[] calldata uris) external returns (uint256[] memory);
    function mintExtensionExisting(
        address[] calldata to, uint256[] calldata tokenIds, uint256[] calldata amounts) external;
}
...
contract PrimordialMushrooms is AdminControl, ICreatorExtensionTokenURI {
    ...
    address public WAGDIE = 0x8d2Eb1c6Ab5D87C5091f09fFE4a5ed31B1D9CF71;
    ...
    function initialize(address _contractAddress) public adminRequired {
        ...
        CreatorCore(contractAddress).mintExtensionNew(to, amounts, uris);
    }
    ...
    function burnCorpse(uint256 amount) public {
        ...
        CreatorCore(contractAddress).mintExtensionExisting(to, newTokenIds, amounts);
        // Mint Concords
