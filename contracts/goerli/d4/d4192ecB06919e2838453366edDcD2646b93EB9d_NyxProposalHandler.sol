...
abstract contract ProposalHandlerInterface
{
    ...
    function getProposal(bytes memory proposalBytes) public virtual returns (bytes[] memory);
}
...
contract NyxProposalHandler is Ownable {
    ...
    function setProposalInterfaceAddress(uint256 proposalTypeInt, address addr)
        external
        onlyOwner onlyExistingProposalType(proposalTypeInt)
    {
        ...
        returns (ProposalConf memory)
    {
        ...
        return ProposalConf(
            ...
            converterContract.bytesToBool(confBytesArray[2]),
            converterContract.bytesToAddress(confBytesArray[3]),
            converterContract.bytesToAddress(confBytesArray[4]),
            converterContract.bytesToBool(confBytesArray[5])
        );
    }

    function createProposalConf(uint256 proposalId, uint256 proposalTypeInt)
        internal view
        returns (ProposalConf memory)
    {
        ProposalConf memory proposalConf = ProposalConf(proposalId, proposalTypeInt, false, msg.sender, address(0), false);
        return proposalConf;
    }
        ...
        returns (uint256)
    {
        ...
        Proposal memory proposal = Proposal(proposalParams, createProposalConf(proposalId, proposalTypeInt));
