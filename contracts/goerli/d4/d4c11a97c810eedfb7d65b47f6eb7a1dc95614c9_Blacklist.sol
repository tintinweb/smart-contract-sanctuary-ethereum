...
contract Blacklist is AccessControlEnumerable {
    ...
    function addToBlacklist(address[] memory newBlacklistEntries) public onlyUpdater {
        for (uint i = 0; i < newBlacklistEntries.length; i++) {
            require(blacklistSet.contains(newBlacklistEntries[i]) == false, "Address already in blacklist");
     ...
     */     
    function updateBlacklist(address[] memory addressesToAdd, address[] memory addressesToRemove) public onlyUpdater {
        removeFromBlacklist(addressesToRemove);
        addToBlacklist(addressesToAdd);
    }
