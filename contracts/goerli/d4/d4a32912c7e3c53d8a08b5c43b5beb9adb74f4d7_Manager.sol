...
// File: NFE-Contracts/IERC721A.sol
...
// File: NFE-Contracts/ERC721A.sol
...
// File: NFE-Contracts/Listings.sol
...
contract NFEListings is Ownable, ERC721A, ReentrancyGuard {
  ...
  uint256 public maxAllowedMultiples;
  ...
  modifier isNotExpiredProject() {
    require(
      block.timestamp >= projectStartTime && block.timestamp < projectEndTime,
  ...
  /// @param _projectSymbol The NFT token symbol
  ...
  function _participate(uint256 _amount, uint256 _multiples)
    external
    payable
    ...
    nonReentrant
  {
    require(_amount == contributionAmnt, "Has to be equal to eth per share!");
    require(_multiples > 0 && _multiples <= maxAllowedMultiples, "Has to be greater than 0 and lesser than max allowed");
    require(
      _amount * _multiples <= maxAllowedMultiples * contributionAmnt,
      "Project: contribution must be equal to specified amount in the contract!"
    );
    require((_amount * _multiples) + stableCoin.balanceOf(address(this)) <= fundraisingGoal, "Exceeding total fund raising goal!");

    if ((_amount * _multiples) + stableCoin.balanceOf(address(this)) >= fundraisingGoal) {
      isFinished = true;
    }

    uint256 _finalContributionAmnt = _amount * _multiples;

    stableCoin.transferFrom(msg.sender, address(this), _finalContributionAmnt);
    ...
    uint256 numNFTsAllocated = _multiples;

    addressToMintedNFTs[msg.sender] += numNFTsAllocated;

    _safeMint(msg.sender, numNFTsAllocated);

    totalNFTsMinted += numNFTsAllocated;

    emit Contribution(msg.sender, address(this), _amount);
  }
  ...
  function _claimRefund() external nonReentrant{
    require(
    ...
    addressToContributions[msg.sender] = 0;
    addressToMintedNFTs[msg.sender] == 0;

    transferFrom(msg.sender, address(0xdead), balanceOf(msg.sender));

    stableCoin.transferFrom(address(this), msg.sender, addressContributions);
  ...
  function rescueERC20(address tokenAdd, uint256 amount) external onlyOwner {
    require(tokenAdd != address(0), "Can't claim zero address tokens");
    IERC20(tokenAdd).transfer(owner(), amount);
  }
}
// File: NFE-Contracts/Factory.sol
...
contract Manager is Ownable {
  ...
  function createNewProject(
    ...
    NFEListings newProject = new NFEListings(
      _stableCoin,
      _name,
      _projectSymbol,
      _fundraisingGoal,
      _contributionAmnt,
      _maxAllowedMultiples,
      _startTime,
      _endTime,
      msg.sender
    );
