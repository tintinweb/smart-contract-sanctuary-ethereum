...
contract OnChainNFT is ERC721, Ownable {
  ...
  function getWeaponName(uint256 _id) public view returns (string memory) {
    if (_id == 0) {
      return "None";
    } else {
      return weapons[_id].name;
    }
  }
    ...
    string memory json = Base64.encode(
      bytes(string(
        abi.encodePacked(
          ...
          '{"trait_type": "Weapon", "value": "', getWeaponName(character[tokenId].weaponId), '"}',
          ']}'
        )
      ))
    );
  ...
  function getBaseModel(uint256 _class) private view returns (string memory) {
    if (_class == 1) {
      return Pirate;
    } else if (_class == 2) {
      return Mage;
      ...
      revert("Class doesnt exist.");
    }
  }

  function getWeaponModel(uint256 _weaponId) private view returns (string memory) {
    if (_weaponId == 0) {
      return "";
    } else {
      return weapons[_weaponId].code;
    }
  }

  function getModel(uint256 _tokenId) public view returns (string memory) {
    string memory svg = Base64.encode(
      bytes(string(
        abi.encodePacked(
          ...
          getBaseModel(character[_tokenId].classId),
          getWeaponModel(character[_tokenId].weaponId),
          '</svg>'
        )
      ))
    );
  ...
  function createCharacter(uint256 token, string memory _name, string memory _class) private {
    ...
    if (pirate) {
      character[token] = Attr(_name, _class, 1, 1, 50, 25, 25, 0);
    } else if (mage) {
      character[token] = Attr(_name, _class, 2, 1, 25, 50, 25, 0);
    } else if (lox) {
      character[token] = Attr(_name, _class, 3, 1, 10, 10, 10, 0);
    } else {
      revert("Class does not exist.");
    }
  }
  
  function equipWeapon(uint256 tokenId, uint256 _weaponId) public {
    require(msg.sender == ownerOf(tokenId), "Token owner access only.");
    character[tokenId].weaponId = _weaponId;
  }

  function dropWeapon(uint256 tokenId) public {
    require(msg.sender == ownerOf(tokenId), "Token owner access only.");
    character[tokenId].weaponId = 0;
  }
  ...
  function addWeapon(uint256 _id, string memory _name, string memory _code, string memory _class, uint256 _str, uint256 _agi, uint256 _vit) public onlyOwner {
    weapons[_id].name = _name;
    weapons[_id].code = _code;
    weapons[_id].class = _class;
    weapons[_id].incStr = _str;
    weapons[_id].incAgi = _agi;
    weapons[_id].incVit = _vit;
