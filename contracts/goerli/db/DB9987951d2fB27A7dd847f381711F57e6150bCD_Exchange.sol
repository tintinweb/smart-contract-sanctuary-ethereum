...
contract Exchange is SuAuthenticated, IExchange {
    ...
    function getOrderPrice(address token2sell, uint256 timePassed, DiscountType discountType) public view returns (uint256) {
        ...
        if (timePassed > curve[0]) {
            // minimum is 80%;
            return ORACLE.getFiatPrice1e18(token2sell) * 8 / 10;
        }

        return ORACLE.getFiatPrice1e18(token2sell) - ORACLE.getFiatPrice1e18(token2sell) * timePassed / curve[0] - curve[1];
    }
