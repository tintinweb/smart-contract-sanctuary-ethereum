...
library InterfaceDetectionStorage {
    ...
    bytes32 internal constant LAYOUT_STORAGE_SLOT =
        bytes32(
            uint256(
                keccak256(
                    "animoca.core.introspection.InterfaceDetection.storage"
                )
            ) - 1
        );
    ...
    function setSupportedInterface(
        ...
        require(
            interfaceId != ILLEGAL_INTERFACE_ID,
            "InterfaceDetection: wrong value"
        );
...
abstract contract InterfaceDetection is IERC165 {
    ...
    function supportsInterface(
        ...
        return
            InterfaceDetectionStorage.layout().supportsInterface(interfaceId);
    }
}
...
library ProxyInitialization {
    ...
    function setPhase(bytes32 storageSlot, uint256 phase) internal {
        StorageSlot.Uint256Slot storage currentVersion = StorageSlot
            .getUint256Slot(storageSlot);
...
library ContractOwnershipStorage {
    ...
    bytes32 internal constant LAYOUT_STORAGE_SLOT =
        bytes32(
            uint256(
                keccak256("animoca.core.access.ContractOwnership.storage")
            ) - 1
        );
    bytes32 internal constant PROXY_INIT_PHASE_SLOT =
        bytes32(
            uint256(keccak256("animoca.core.access.ContractOwnership.phase")) -
                1
        );
    ...
    function constructorInit(Layout storage s, address initialOwner) internal {
        ...
        InterfaceDetectionStorage.layout().setSupportedInterface(
    ...
    function owner(
        Layout storage s
    ) internal view returns (address contractOwner) {
...
abstract contract ContractOwnership is
...
interface IERC721 {
    ...
    function isApprovedForAll(
        address owner,
        address operator
    ) external view returns (bool approvedForAll);
}
...
contract TokenRecoveryBase is Context {
    ...
    function recoverERC721s(
        ...
        uint256 length = accounts.length;
        require(
            length == contracts.length && length == tokenIds.length,
            "Recovery: inconsistent arrays"
        );
        unchecked {
            for (uint256 i; i != length; ++i) {
                contracts[i].transferFrom(
                    address(this),
                    accounts[i],
                    tokenIds[i]
                );
...
// File @animoca/ethereum-contracts/contracts/token/ERC20/interfaces/[email protected]
pragma solidity ^0.8.8;
...
// File @animoca/ethereum-contracts/contracts/token/ERC20/libraries/[email protected]
pragma solidity ^0.8.8;
...
library ERC20Storage {
    ...
    bytes4 internal constant ERC20_RECEIVED =
    ...
    function init() internal {
        InterfaceDetectionStorage.Layout
            storage erc165Layout = InterfaceDetectionStorage.layout();
        erc165Layout.setSupportedInterface(type(IERC20).interfaceId, true);
        erc165Layout.setSupportedInterface(
    ...
    function initERC20BatchTransfers() internal {
        InterfaceDetectionStorage.layout().setSupportedInterface(
            type(IERC20BatchTransfers).interfaceId,
            true
        );
    }
    ...
    function initERC20SafeTransfers() internal {
        InterfaceDetectionStorage.layout().setSupportedInterface(
            type(IERC20SafeTransfers).interfaceId,
            true
        );
    }
    ...
    function initERC20Mintable() internal {
        InterfaceDetectionStorage.layout().setSupportedInterface(
            type(IERC20Mintable).interfaceId,
            true
        );
    }
    ...
    function initERC20Burnable() internal {
        InterfaceDetectionStorage.layout().setSupportedInterface(
            type(IERC20Burnable).interfaceId,
            true
        );
    }
    ...
    function _callOnERC20Received(
        ...
        require(
            IERC20Receiver(to).onERC20Received(sender, from, value, data) ==
                ERC20_RECEIVED,
            "ERC20: safe transfer rejected"
        );
    }
}

// File @animoca/ethereum-contracts/contracts/token/ERC20/base/[email protected]
pragma solidity ^0.8.8;
...
abstract contract ERC20Base is Context, IERC20, IERC20Allowance {
    ...
    function increaseAllowance(
        ...
        ERC20Storage.layout().increaseAllowance(
            _msgSender(),
            spender,
            addedValue
        );
    ...
    function decreaseAllowance(
        ...
        ERC20Storage.layout().decreaseAllowance(
            _msgSender(),
            spender,
            subtractedValue
        );
...
// File @animoca/ethereum-contracts/contracts/token/ERC20/[email protected]
pragma solidity ^0.8.8;
...
library ERC20DetailedStorage {
    ...
    bytes32 internal constant LAYOUT_STORAGE_SLOT =
        bytes32(
            uint256(
                keccak256("animoca.core.token.ERC20.ERC20Detailed.storage")
            ) - 1
        );
    bytes32 internal constant PROXY_INIT_PHASE_SLOT =
        bytes32(
            uint256(keccak256("animoca.core.token.ERC20.ERC20Detailed.phase")) -
                1
        );
    ...
    function constructorInit(
        ...
        InterfaceDetectionStorage.layout().setSupportedInterface(
            type(IERC20Detailed).interfaceId,
            true
        );
    }
    ...
    function decimals(
        Layout storage s
    ) internal view returns (uint8 nbDecimals) {
...
abstract contract ERC20Detailed is ERC20DetailedBase {
    ...
    constructor(
        ...
        ERC20DetailedStorage.layout().constructorInit(
            tokenName,
            tokenSymbol,
            tokenDecimals
        );
...
abstract contract ForwarderRegistryContextBase {
    ...
    function _msgSender() internal view virtual returns (address) {
        ...
        if (
            msg.sender == address(_forwarderRegistry) ||
            _forwarderRegistry.isApprovedForwarder(sender, msg.sender)
        ) {
            return sender;
        }
    ...
    function _msgData() internal view virtual returns (bytes calldata) {
        ...
        if (
            msg.sender == address(_forwarderRegistry) ||
            _forwarderRegistry.isApprovedForwarder(
                ERC2771Calldata.msgSender(),
                msg.sender
            )
        ) {
...
/// @dev This contract is to be used via inheritance in an immutable (non-proxied) implementation.
/// @dev Derived from https://github.com/wighawag/universal-forwarder (MIT licence)
abstract contract ForwarderRegistryContext is
    ForwarderRegistryContextBase,
    IERC2771
{
    constructor(
        IForwarderRegistry forwarderRegistry_
    ) ForwarderRegistryContextBase(forwarderRegistry_) {}
...
library ERC20MetadataStorage {
    ...
    bytes32 internal constant LAYOUT_STORAGE_SLOT =
        bytes32(
            uint256(
                keccak256("animoca.core.token.ERC20.ERC20Metadata.storage")
            ) - 1
        );
...
abstract contract ERC20SafeTransfersBase is Context, IERC20SafeTransfers {
    ...
    function safeTransferFrom(
        ...
        ERC20Storage.layout().safeTransferFrom(
            _msgSender(),
            from,
            to,
            value,
...
abstract contract ERC20BatchTransfersBase is Context, IERC20BatchTransfers {
    ...
    function batchTransferFrom(
        ...
        ERC20Storage.layout().batchTransferFrom(
            _msgSender(),
            from,
            recipients,
            values
...
library AccessControlStorage {
    ...
    bytes32 internal constant LAYOUT_STORAGE_SLOT =
        bytes32(
            uint256(keccak256("animoca.core.access.AccessControl.storage")) - 1
        );
...
library ERC20PermitStorage {
    ...
    bytes32 internal constant LAYOUT_STORAGE_SLOT =
        bytes32(
            uint256(keccak256("animoca.core.token.ERC20.ERC20Permit.storage")) -
                1
        );
...
abstract contract ERC20PermitBase is Context, IERC20Permit {
    ...
    function permit(
        ...
        ERC20PermitStorage.layout().permit(
            owner,
            spender,
            value,
            deadline,
...
// File contracts/token/erc20/PRIMATEv2.sol
