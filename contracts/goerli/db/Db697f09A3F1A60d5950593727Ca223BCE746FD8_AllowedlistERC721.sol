 ...
 */
contract AllowedlistERC721 is ERC721, ERC721URIStorage, Ownable {
  ...
  function mint(address to, uint256 amount) external payable {
    uint8 rank = _currentAllowedRank;
    uint256 price = getPrice(rank);

    require(msg.value == price * amount, "Not enough value for mint.");

    // @notice total minted token should be less than 3,333 by each phase.
    require(
      getRemainingTokenByAddress(to, rank) >= amount &&
        getRemainingToken(_currentPhase) >= amount,
      "No token left for mint, or you are not in the AllowedList."
    );
  ...
  function ownerMint(address to, uint256 amount) external onlyOwner {
    ...
    require(
      getRemainingToken(_currentPhase) >= amount,
      "Already reached Token Upper bounds."
    );
    for (uint256 i = 0; i < amount; i++) {
      uint256 tokenId = _tokenIdCounter.current();
      _tokenIdCounter.increment();
  ...
  /**
   * @notice Allowed List checker.
   * @param rank = 0 for gold, 1 for silver.
   * @return upper limit in rank
   * */
  ...
  /**
   * @param
   * phase: Mint Sale phase(1~3).
   * rank: parameter for allowedList gold/silver.
   * address: user address array.
   * amount: amount for each user mint-cap.
   */
  function setAllowedUserList(
    ...
    require(
      ...
      "INVALID PARAMETER: users and amount list must be same length."
    );
    require(
      rank == 0 || rank == 1,
      "INVALID PARAMETER: rank is only 0 for gold, 1 for silver."
    );
  ...
  /**
   * @param phase: Mint Sale phase(1~3).
   * @return amount of remaining token that user can mint.
   */
  ...
  function setWithdrawAccount(address payable to) external onlyOwner {
    require(to != address(0), "Can't set zero address.");
    _withdrawAccount = to;
  }
