...
contract TradeAboveThreshold is ConditionalOrder {
    ...
    constructor(IERC20 _sellToken, IERC20 _buyToken, address _receiver, uint256 _threshold, GPv2Settlement _settlementContract) {
        sellToken = _sellToken;
        buyToken = _buyToken;
        receiver = _receiver;
        threshold = _threshold;
        domainSeparator = _settlementContract.domainSeparator();
        _sellToken.approve(address(_settlementContract.vaultRelayer()), uint(-1));

        emit ConditionalOrderCreated();
    }

    function getTradeableOrder() override external view returns (GPv2Order.Data memory) {
    ...
    function isValidSignature(bytes32 orderDigest, bytes calldata encodedOrder) external view returns (bytes4) {
        ...
        require(order.hash(domainSeparator) == orderDigest, "encoded order doesn't match digest");
        ...
        require(ConditionalOrder(this).getTradeableOrder().hash(domainSeparator) == orderDigest, "encoded order is not equal to tradable order");
