  ...
  Ownable
{
	...
	mapping(uint256 => uint256) public numTransfersByTokenId;
      ...
      return string(
        abi.encodePacked(
          ...
          Base64.encode(
            bytes(
              abi.encodePacked(
                '{"description": "An on-chain (*)piece.","name":"Hypnoception","image":"<svg viewBox=\\"0 0 128 128\\" xmlns=\\"http://www.w3.org/2000/svg\\"></svg>"}'
              )
            )
          )
        )
      ...
      for (layerIndex = 0; layerIndex < artByTokenId[tokenId].layerCount; layerIndex++) {
        ...
        for (ringIndex = 0; ringIndex < 3; ringIndex++) {
          ...
          for (spikeIndex = 0; spikeIndex < thisLayer.numberOfSpikes; spikeIndex++) {
            ...
            sections[8] = abi.encodePacked(
              ', 512 512)\\" fill=\\"rgba(',
              thisLayer.color[0].toString(),
              ',',
              thisLayer.color[1].toString(),
              ',',
              thisLayer.color[2].toString(),
              ',0.7)\\" />'
            );
      ...
      return string(
        abi.encodePacked(
          ...
          Base64.encode(
            bytes(
              abi.encodePacked(
                '{"description":"An on-chain (*)piece.","name":"Hypnoception #',
                tokenId.toString(),
                '","image":"<svg viewBox=\\"0 0 1024 1024\\" xmlns=\\"http://www.w3.org/2000/svg\\"><defs>',
                defs,
                '</defs>',
                uses,
                '<mask id=\\"cut-off-bottom\\"><rect width=\\"100%\\" height=\\"100%\\" fill=\\"white\\"/><circle cx=\\"512\\" cy=\\"512\\" r=\\"512\\" fill=\\"black\\"/></mask><rect x=\\"0\\" y=\\"0\\" width=\\"1024\\" height=\\"1024\\" mask=\\"url(#cut-off-bottom)\\" fill=\\"black\\"/></svg>","attributes":[{"trait_type": "Level","value": "',
                numTransfersByTokenId[tokenId].toString(),
                '"}]}'
              )
            )
          )
  ...
  private {
    ...
    uint256 numLayers = 4;
    uint256[3][4] memory newColors;
    uint256[3][4] memory newStats;
    uint256[6][4] memory newCoords;

		for (uint256 i = 0; i < quantity; i += 1) {
      OnChainArt storage newPiece = artByTokenId[startTokenId + i];
      ...
      for (uint x = 0; x < numLayers; x++) {
        for(uint256 y = 0; y < 3; y++){
          randomHash = uint256(keccak256(abi.encodePacked(block.timestamp, msg.sender, i, x, y)));
          newColors[x][y] = randomHash % 255;
        }

        for(uint256 y= 0; y < 3; y++){
          randomHash = uint256(keccak256(abi.encodePacked(block.timestamp, msg.sender, i, x, y)));
          newStats[x][y] = randomHash % 4;
        }
        ...
        newPiece.layers[x] = Layer({
          ...
          numberOfRings: newStats[x][1] + 4,
          numberOfSpikes: newStats[x][2] + 4
        });
      }
		}
	}
