...
contract ERC721x is IERC721x {
    ...
    function isUnlocked(uint256 _id) public view virtual returns (bool) {}
    ...
    function lockId(uint256 _id) external view {
        revert NotImplemented();
    }

    function unlockId(uint256 _id) external view {
        revert NotImplemented();
    }

    function freeId(uint256 _id, address _contract) external view {
        revert NotImplemented();
    }
    ...
    function supportsInterface(
        ...
        return interfaceId == _INTERFACE_ID_ERC721x;
    }
}
...
import "./ERC/Erc721LockRegistry.sol";
...
contract Token is Ownable, ERC2981, ERC721A, ERC721x {
    ...
    function supportsInterface(
        bytes4 interfaceId
    ) public view override(ERC721A, ERC2981, ERC721x) returns (bool) {
        return
    ...
    function isUnlocked(uint256 _id) public view override returns (bool) {
        return tokensLastStakedAt[_id] == 0;
    }

    function lockCount(
        uint256 _tokenId
    ) external view override returns (uint256) {
        if (tokensLastStakedAt[_tokenId] != 0) return 1;
