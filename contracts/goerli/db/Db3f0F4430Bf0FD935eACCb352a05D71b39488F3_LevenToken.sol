...
contract LevenToken is Initializable, ERC20Upgradeable, OwnableUpgradeable {
    ...
    function transfer(address recipient, uint256 amount)
        ...
        returns (bool)
    {
        uint256 lockedAmount = _totalPreSalePool
            .add(_totalAirDropPool)
            .add(_totalTeamPool)
            .sub(_soldPreSaleAmount)
            .sub(_utilizedAirDropAmount)
            .sub(_distributedTeamPoolAmount);

        require(
            balanceOf(address(this)) >= lockedAmount.add(amount),
