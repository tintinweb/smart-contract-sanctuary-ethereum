...
import './erc721a/contracts/extensions/ERC721AQueryableUpgradeable.sol';
import './erc721a/contracts/extensions/ERC721ABurnableUpgradeable.sol';
import './erc721a/contracts/ERC721A__Initializable.sol';
...
contract MYCONTRACTMERKLE3 is ERC721A__Initializable, OwnableUpgradeable, ERC721AQueryableUpgradeable, ERC721ABurnableUpgradeable{
    ...
    uint64 public mintedCountPhase;
    ...
    function initialize(
        string memory _name, 
        string memory _symbol
    ) public onlyInitializingERC721A {
        __ERC721A_init_unchained(_name, _symbol);
        __ERC721AQueryable_init_unchained();
        __ERC721ABurnable_init_unchained();
        __ERC721AQueryableMock_init_unchained(_name, _symbol);

        setMember(_msgSender(), true);

        baseURI = "";
        revealUri = "ipfs://QmewHM4hC5hSq8WgXS7DnWKYzyz7Swqyk1q4jubPng21hW";
        lockedUri = "";
        extension = ".json";
        ...
        // フェーズ0：セール前
        // フェーズ１：フリーミント
        // フェーズ２：ALミント（確実にMINT可能数のみ） 0.02
        // フェーズ３：ALミント（配布数オーバーAL） 0.02
        // フェーズ４：パブリックセール（誰でもOK） 0.1
        salePhase = 0;

        mintedCountPhase = 1;   //カウントアップでミント済みをリセット
        maxSupply = 1000;

        royaltyAddress = 0x9e81fd993203113AebD886707D8E8c21dDe939c6;
    ...
    function tokenURI(uint256 _tokenId) public view virtual override returns (string memory) {
        ...
        return string(abi.encodePacked(baseURI, StringsUpgradeable.toString(_tokenId), extension));
    }
    // start from 1
    ...
    function publicMint(uint256 _mintAmount) external payable callerIsUser {
        ...
        if(!(salePhase == 4)) revert PresaleIsActive();
    ...
    function preMint(
        ...
        if(!(salePhase == 1 || salePhase == 2 || salePhase == 3)) revert PresaleIsNotActive();      //フェーズ１−３の間のみ許可
        ...
        if(!MerkleProof.verify(_proof, _merkleRoot, _leaf)) revert InvalidMerkleProof();     
        if(mintedLists[mintedCountPhase][msg.sender] + _mintAmount > _maxCount) revert OverMintLimit();    // 登録数は超えてないか？
        mintedLists[mintedCountPhase][msg.sender] += _mintAmount;   //ミント済みListをカウントアップ（フェーズ２へのカウントアップ時に自動でリセットさせるので、ここでは固定でOK）
        _safeMint(msg.sender, _mintAmount);
    }
    ...
    // function ownerBurn(uint256 _tokenId) external virtual onlyTeam {
