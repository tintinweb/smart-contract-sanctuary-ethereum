...
contract VotingMasterChef is BasicMetaTransaction, AccessControl {
   ...
   */
  constructor(address starsAddress, address _admin) {
  ...
  /**
   * @dev Sets the Voting contract address.
   *
   * Params:
   * votingAddr: the Voting contract address.
   *
   * Requirements:
   * - contract must not be initialized.
   */
  function init(address votingAddr) external onlyAdmin {
    require(!initialized, "Already initialized");
    votingContract = votingAddr;
    _setupRole(ROLE_ADMIN, votingAddr);
  ...
  /**
   * @dev Sets new reward per block for given pool ids
   *
   * Params:
   * _pid: the pool id
   * _newRewardsPerBlock: the new rewards per block
   *
   * Requirements:
   * - pool exists
   */
  function setPoolRewards(uint256 _pid, uint256 _newRewardsPerBlock)
    external
    onlyAdmin
  {
    PoolInfo storage pool = poolInfo[_pid];
    pool.rewardAmountPerBlock = _newRewardsPerBlock;
  }
  ...
  function add(
    ...
    poolInfo.push(
      PoolInfo({
        ...
        rewardAmountPerBlock: _rewardAmount / (_endBlock - _startBlock),
    ...
    returns (uint256)
  {
    ...
    uint256 currRateEndStarsPerShare =
      accStarsPerShareAtCurrRate(
        uint256(block.number) - pool.startBlock,
    ...
    uint256 starsReward =
      (currRateEndStarsPerShare - currRateStartStarsPerShare);

    uint256 pendingAccStarsPerShare = pool.accStarsPerShare + starsReward;
    return
      ((user.amount * pendingAccStarsPerShare) / ACC_STARS_PRECISION) -
      user.rewardDebt +
      user.penalizedRewards;
  }
  ...
  function accStarsPerShareAtCurrRate(
    ...
    if (blocks > endBlock - startBlock) {
      return
        (rewardAmountPerBlock * (endBlock - startBlock) * ACC_STARS_PRECISION) /
        poolSupply;
    } else {
      return (blocks * rewardAmountPerBlock * ACC_STARS_PRECISION) / poolSupply;
    }
  }
  ...
  function updatePool(uint256 _pid) public isInitialized {
    ...
    if (pool.poolSupply != 0) {
      ...
      pool.accStarsPerShare = pool.accStarsPerShare + starsReward;
    }
  ...
  function deposit(
    ...
    user.rewardDebt =
      user.rewardDebt +
      ((_amount * pool.accStarsPerShare) / ACC_STARS_PRECISION);
    pool.poolSupply = pool.poolSupply + _amount;
    ...
    isInitialized
  {
    ...
    uint256 pending =
      ((user.amount * pool.accStarsPerShare) / ACC_STARS_PRECISION) -
        user.rewardDebt;
    ...
    safeStarsTransfer(_staker, pending + _amount + _penalizedRewards);
  ...
  function withdrawPartial(
    ...
    if (_amount > 0) {
      ...
      user.penalizedRewards =
        ((user.penalizedRewards * user.amount) / oldUserAmount) +
        ((pending * user.amount) / oldUserAmount);
      pool.poolSupply = pool.poolSupply - _amount;
    }

    user.rewardDebt = ((user.amount * pool.accStarsPerShare) /
      ACC_STARS_PRECISION);
    ...
    isInitialized
  {
    ...
    pool.poolSupply = pool.poolSupply - user.amount;
    user.amount = 0;
    user.rewardDebt = 0;
