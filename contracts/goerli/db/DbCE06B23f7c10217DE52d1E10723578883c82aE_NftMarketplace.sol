...
contract NftMarketplace is ReentrancyGuard {
    ...
    constructor() {
        modOwner = msg.sender; // 'msg.sender' is sender of current call, contract deployer for a constructor
    }
    ...
    function modListItem(
        ...
        uint256 tokenId,
        uint256 price
    ) external notListed(nftAddress, tokenId) isMod {
        if (price <= 0) {
            revert PriceMustBeAboveZero();
        }
        IERC721 nft = IERC721(nftAddress);
