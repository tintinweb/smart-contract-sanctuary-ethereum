...
contract SafeMoneyUp is ERC20, ERC20Burnable, Ownable, ReentrancyGuard {
    ...
    event AdminFeePercentChanged(
        uint256 oldAdminFeePercent,
        uint256 newAdminFeePercent
    );
    event ReferralPercentChanged(
        uint256 oldReferralPercent,
        uint256 newReferralPercent
    );
    ...
    function setAdminFeePercent(uint256 newAdminFeePercent) external onlyOwner {
        require(
            newAdminFeePercent <= 100,
            "SafeMoneyUp: Admin fee percent must be less than or equal to 100"
        );
    ...
    function setReferralPercent(uint256 newReferralPercent) external onlyOwner {
        require(
            newReferralPercent <= 100,
            "SafeMoneyUp: Referral percent must be less than or equal to 100"
        );
    ...
    function buy(address referral) external payable nonReentrant {
        ...
        require(
            referral != msg.sender,
            "SafeMoneyUp: Referrer cannot be the buyer"
        );
        require(
            balanceOf(referral) >= MINIMUM_REFERRAL_HOLD_AMOUNT ||
                referral == owner(),
            "SafeMoneyUp: Referrer must hold a minimum amount of tokens"
        );
        ...
        uint256 referralReward = tokensToMint.mul(REFERRAL_REWARD_PERCENT).div(
            100
        );
    ...
    function sell(uint256 tokenAmount) external nonReentrant {
        ...
        require(
            ...
            "SafeMoneyUp: Insufficient token balance"
        );
    ...
    function calculateSellAmount(
        ...
        uint256 ethAmount = rawEth.sub(
            rawEth.mul(TRANSFER_FEE_PERCENT).div(100)
        );
        return ethAmount;
    }
    ...
    function _beforeTokenTransfer(
        ...
        require(
            !_blacklist[from],
            "SafeMoneyUp: Blacklisted addresses cannot transfer tokens"
        );

        if (to == address(this)) {
            uint256 ethAmount = calculateSellAmount(amount);
            uint256 tokensToBurn = amount.add(
                amount.mul(TRANSFER_FEE_PERCENT).div(100)
            );
