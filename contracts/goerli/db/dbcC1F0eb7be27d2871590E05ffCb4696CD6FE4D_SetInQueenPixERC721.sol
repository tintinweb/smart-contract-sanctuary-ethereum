...
contract SetInQueenPixERC721 is SetInBaseERC721 {
    ...
    mapping(address => mapping(uint => uint)) minted;
    ...
    constructor(string memory _prerevealMetadata, uint16 _qTotalSupply, uint16 _qReserveSupply) SetInBaseERC721("SET IN QUEEN PIX", "SIQP", "ipfs://", msg.sender, 500) {
     ...
     * Only manager access
     */
    function reveal(string calldata metadata) external onlyManager {

        metadataFolder = metadata;
        isTransferable = true;
    }

    function setPrerevealMetadata(string calldata metadata) external onlyManager {

        prerevealMetadata = metadata;
    }

    function setTransferable(bool flag) external onlyManager {
    ...
    function setTreasury(address payable _treasury) external onlyManager {
    ...
    function mint(uint8 wlId, uint count, bytes32[] calldata proof) external payable {
        ...
        uint price = wl.price * count;
        require(msg.value >= price, "SIQP: not enough funds sent");
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        ...
        if (bytes(metadataFolder).length == 0) {
            return
                string(
                    abi.encodePacked(baseURI, prerevealMetadata)
                );
        } else {
            return
                string(
    ...
    function _beforeTokenTransfer(
        ...
        require(isTransferable || from == address(0), "SIQP: token transfer is turned off");
        super._beforeTokenTransfer(from, to, tokenId);
    }
}
