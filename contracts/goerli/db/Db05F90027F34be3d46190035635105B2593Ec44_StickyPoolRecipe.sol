...
contract StickyPoolRecipe {
    ...
    */
    constructor(
        ...
        locked = true;
        maxPercentage = 20;
        imgUrl = "https://raw.githubusercontent.com/JellyProtocol/JellyAssets/main/Images/PoolNFTs/blue_jelly.png";
    }
     ...
     * @param _imgUrl Project logo URL
     * @param _maxPercentage Percentage staked for full sized image
     */
    function setDescriptorSettings(string calldata _imgUrl, uint256 _maxPercentage) external {
        require(
            accessControls.hasAdminRole(msg.sender),
            "setFeeAmount: Sender must be admin"
        );
        ...
        returns (address)
    {
        ...
        address pool_descriptor = jellyFactory.deployContract(
            ...
            jellyVault, 
            "");
        IJellyContract(pool_descriptor).initContract(abi.encode(sticky_pool, access_controls, maxPercentage)); 

        // Contract config
        IStickyPool(sticky_pool).setDescriptor(pool_descriptor);
        IStickyDescriptor(pool_descriptor).setImgUrl(imgUrl);
