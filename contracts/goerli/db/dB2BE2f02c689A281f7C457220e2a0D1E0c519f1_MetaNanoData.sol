...
// File: META NANOs/Meta NANO Data/MetaNanoDataI.sol
...
// File: META NANOs/Meta NANO Data/ENSReverseRegistrarI.sol
...
// File: META NANOs/Meta NANO Data/MetaNanoData.sol

/*
 * Implements a data storage for the metananos system.
...
contract MetaNanoData is MetaNanoDataI {

    // Various addresses, accessed via a name hash.
    mapping(bytes32 => address) private addresses;

    constructor(address _tokenAssignmentControl)
    {
        // During the deployment phase, set systemControl to deployer,
        // at the end of deployment, switch to actual systemControl address.
        _setAddress("systemControl", msg.sender);
        _setAddress("tokenAssignmentControl", _tokenAssignmentControl);
    }

    modifier onlySystemControl()
    {
        require(msg.sender == getAddress("systemControl"), "systemControl key required for this function.");
        _;
    }
    ...
    internal
    {
        ...
        require(_newAddress != address(0) || nameHash != _getNameHash("systemControl"), "systemControl cannot be the zero address.");
    ...
    returns (address)
    {
        address ret = addresses[_getNameHash(_name)];
        require(ret != address(0), "a requested named address was not set");
        return ret;
    }
