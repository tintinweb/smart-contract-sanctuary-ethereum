...
interface INFTBatchTransfer {
    ...
    function sendNFTs(uint256 startId, address recipient) external;

    function sendManyNFTs(uint256 startId, address[] calldata recipients) external;
...
contract NFTVRFDistributor is VRFv2Consumer {
    ...
    **/
    ...
    event RoundRequested(uint256 requestId, address nftAddress, uint8 numWinners);
    ...
    **/
    ...
    function requestRound(address nftAddress, uint8 numWinners) external onlyOwner returns (uint256 requestId) {
        if (numWinners == 0) {
            revert MustHaveAtLeastOneWinner();
        }
        uint256 availNFTs = remainingAllowance(nftAddress);
        if ((availNFTs > 256) || (availNFTs < numWinners)) {
          revert RoundIsTooBig();
        }
        requestId = requestRandomWords();
        requestIdToNFT[requestId] = nftAddress;

        emit RoundRequested(requestId, nftAddress, numWinners);
    }
    ...
    function claim(address nftAddress, uint8 index) external {
        ...
        uint256 startId = NFT_BATCH_TRANSFER.getStartId();
        ...
        NFT_BATCH_TRANSFER.sendNFTs(startId, owner);
    ...
    function claimMany(address nftAddress, uint8[] calldata indexes) external {
        ...
        NFT_BATCH_TRANSFER.sendManyNFTs(startId, recipients);
    }
