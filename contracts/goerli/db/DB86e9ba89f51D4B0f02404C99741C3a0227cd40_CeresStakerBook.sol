...
contract CeresStakerBook is IStakerBook, CeresBaseUpgradeable, UUPSUpgradeable{

    mapping(address => bool) public staked;
    mapping(address => address) public referers;
    
    mapping(address => address[]) public override stakers;
    mapping(address => mapping(address => bool)) public stakingStaked;
    ...
    /* ---------- Views ---------- */
    function getStakers(address staking) external view override returns (address[] memory){
        return stakers[staking];
    }

    function getStakersLength(address staking) external view override returns (uint256){
        return stakers[staking].length;
    }

    function getStakersLimit(address staking, uint256 start, uint256 end) external view returns (address[] memory values){
        uint256 _length = stakers[staking].length;
        end = end > _length ? _length : end;
        values = new address[](end - start);

        uint256 index = 0;
        for (uint256 i = start; i < end; i++) {
            values[index] = stakers[staking][i];
            index++;
        }
    }
    ...
    function stake(address staker) external override onlyCeresStaking {
        if (!staked[staker])
            staked[staker] = true;
        if (!stakingStaked[msg.sender][staker]) {
            stakingStaked[msg.sender][staker] = true;
