...
contract ItWasAllALie is ERC721A, Ownable {  
    using Strings for uint256;

    address public withdrawWallet = 0x4a91EC0bBD3C089a48fFBD29954Ef2Cc53b6fDc5;

    uint256 public max_supply = 8888;

    bool public public_sale = false;
    bool public WL_sale     = false;
    bool public revealed    = false;

    uint256 public mint_limit_team   = 10; // per wallet
    uint256 public mint_limit_public = 5;  // per wallet
    uint256 public mint_limit_wl     = 5;  // per wallet

    bytes32 public merkle_root_team;
    bytes32 public merkle_root_wl;

    uint256 public public_sale_cost = 0.0039 ether;
    uint256 public wl_sale_cost = 0 ether;

    string private _myBaseURI;
    string private _myHiddenURI;

	constructor() ERC721A("Itwasallalie NFT", "IWAAL") {}

    function setWithdrawWallet(address _withdrawWallet) external onlyOwner {
        withdrawWallet = _withdrawWallet;
    }

    function setMaxSupply_Akw(uint256 _maxSupply) external onlyOwner {
    ...
    function setWLSale(bool _wlSale) external onlyOwner {
        WL_sale = _wlSale;
    }
    ...
    function setPublicSaleCost(uint256 _cost) external onlyOwner {
        public_sale_cost = _cost;
    }

    function setWLSaleCost(uint256 _cost) external onlyOwner {
        wl_sale_cost = _cost;
    }
    ...
    function teamMint(uint256 _mintAmount, bytes32[] calldata _merkleProof) external nonReentrant {
        require(totalSupply() + _mintAmount <= max_supply, "error, teamMint tried to mint more than max supply");
        require(_numberMinted(_msgSender()) + _mintAmount <= mint_limit_team, "error, teamMint tried to mint more than maxTeamAmount");
        require (MerkleProof.verify(_merkleProof, merkle_root_team, keccak256(abi.encodePacked(_msgSender()))), "error, invalid merkle proof team");

        _safeMint(_msgSender(), _mintAmount);
    }

    function publicMint(uint256 _mintAmount) external payable nonReentrant {
        require (msg.value >= public_sale_cost, "error, public didn't send enough eth");
        require(public_sale, "error, pulic sale inactive");
        require(tx.origin == _msgSender(), "error, tried to mint from contract public");
        require(totalSupply() + _mintAmount <= max_supply, "error, tried to mint more than max supply");
        require (_numberMinted(_msgSender()) + _mintAmount <= mint_limit_public, "error, tried to mint more than maxPerWalletPublic");

        _safeMint(_msgSender(), _mintAmount);
    }

    // note v0.8.0 checks safe math by default :)
    function wl_mint(uint256 _mintAmount, bytes32[] calldata _merkleProof) external payable nonReentrant {
        require (msg.value >= wl_sale_cost, "error, WL didn't send enough eth");
        require (WL_sale, "error, WL sale inactive");
        require (tx.origin == _msgSender(), "error, tried to mint from contract WL");
        require(totalSupply() + _mintAmount <= max_supply, "error, tried to mint more than max supply WL");
        require (_numberMinted(_msgSender()) + _mintAmount <= mint_limit_wl, "error, tried to mint more than maxPerWalletWL");
        require (MerkleProof.verify(_merkleProof, merkle_root_wl, keccak256(abi.encodePacked(_msgSender()))), "error, invalid merkle proof WL");
