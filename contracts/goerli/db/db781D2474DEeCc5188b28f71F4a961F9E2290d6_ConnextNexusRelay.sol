...
contract ConnextNexusRelay is IXReceiver, AbstractNexusRelay {
	...
	mapping(uint256 => ChainData) public chain_data;
	mapping(uint32 => uint256) public domain_to_chain;
	uint256[] public supported_chains = [5, 420, 421613, 80001];

	constructor(
		address _owner,
		uint8 _providerId
	) AbstractNexusRelay(_owner, _providerId, false) {
		uint256 chainId = block.chainid;
		chain_data[5] = ChainData(
			0xFCa08024A6D4bCc87275b1E4A1E22B71fAD7f649,
			1735353714
		);
		chain_data[420] = ChainData(
			0x5Ea1bb242326044699C3d81341c5f535d5Af1504,
			1735356532
		);
		chain_data[421613] = ChainData(
			0x2075c9E31f973bb53CAE5BAC36a8eeB4B082ADC2,
			421613
		);
		chain_data[80001] = ChainData(
			0x2334937846Ab2A3FCE747b32587e1A1A2f6EEC5a,
			9991
		);

		for (uint256 i = 0; i < supported_chains.length; i++) {
			uint256 _chainId = supported_chains[i];
			ChainData memory _chainData = chain_data[_chainId];
			domain_to_chain[_chainData.domainId] = _chainId;
		}

		address _connext = chain_data[chainId].connext;
	...
	function _transmit(
		...
		sendTransferMessage(
			chain_data[destinationChainId].domainId,
			targetContract,
			amount,
			msg.value,
			message,
	...
	function sendTransferMessage(
		...
		connext.xcall{ value: relayerFee }(
			...
			address(_token), // _asset: address of the token contract
	...
	function xReceive(
		...
		uint32 _origin,
		bytes memory _callData
	) external onlyGateway(_msgSender()) returns (bytes memory _returnData) {
		if (_amount > 0) IERC20(_asset).approve(owner(), _amount);

		_forwardMessageToNexus(
			_amount,
			_asset,
			_originSender,
			domain_to_chain[_origin],
			_callData
		);

		_returnData = _callData;
	}

	modifier onlyGateway(address) override {
		require(msg.sender == address(connext), "Only connext can call this");
		_;
	}
}