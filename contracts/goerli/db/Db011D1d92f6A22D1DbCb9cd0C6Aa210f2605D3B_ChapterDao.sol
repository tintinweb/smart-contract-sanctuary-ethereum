...
import "./utils/ERC721PSIBurnable.sol";
...
import "./SchoolDao.sol";
import "./ChapterDaoFactory.sol";
...
contract ChapterDao is ERC721PsiBurnable, ReentrancyGuard {
    using Address for *;

    uint256 public mintPrice = 50e18;
    ...
    uint256 public ambassadorId;
    address public schoolDao;
    uint256 public totalCapitalRepaid = 0;
    uint256 public totalCapitalCollected = 0;
    string snapshot = "";
    ...
    // Modifier to check that the caller is an ambassdor of
    // the contract.
    modifier onlyAmbassador() {
        if (_msgSender() != getAmbassadorAddress()) revert NotAmbassador();
        _;
    }
    ...
    function getFeeToken() public view returns (address) {
        return ChapterDaoFactory(factory).getFeeToken();
    }

    function getFeePercent() public view returns (uint256) {
        return ChapterDaoFactory(factory).getFeePercent();
    }

    function getFeeTo() public view returns (address) {
        return ChapterDaoFactory(factory).getFeeTo();
    }

    function getAmbassadorAddress() public view returns (address) {
        return
            address(SchoolDao(schoolDao).getAmbassadorAddress(address(this)));
    }

    function calcFee(uint256 _amount) public view returns (uint256) {
        return (_amount * getFeePercent()) / 10000;
    }

    function setSnapshot(string memory _link) external onlyAmbassador {
        snapshot = _link;
    }

    function getSnapshot() external view returns (string memory) {
        return snapshot;
    }
    ...
    function setMintPrice(uint256 _price) external onlyAmbassador {
        mintPrice = _price;
    }

    function setURI(string memory _uri) external onlyAmbassador {
        _baseURILink = _uri;
    }
    ...
    function userClaimableBalance(
        ...
        return
            (dividendPerToken *
                balanceOf(_address) -
                xDividendPerToken[_address]) + credit[_address];
    }

    function userTotalBalance(
        address _address
    ) external view returns (uint256) {
        return
            (dividendPerToken *
                balanceOf(_address) -
                xDividendPerToken[_address]) +
            credit[_address] +
            (dividendPerToken * totalSupply());
    }

    function setSaleStatus(bool _saleStatus) external onlyAmbassador {
        saleActive = _saleStatus;
        emit SaleStatusSet(bool(saleActive));
    }
    ...
    function initialize(
        ...
        uint256 ambassadorId_,
        address schoolDao_
    ) external {
        if (_msgSender() != factory) revert NotFactory();
        _name = name_;
        _symbol = symbol_;
        _baseURILink = baseURI_;

        ambassadorId = ambassadorId_;
        schoolDao = schoolDao_;
    }

    function withdrawFunds() external onlyAmbassador {
        uint256 balance = IERC20(getFeeToken()).balanceOf(address(this));

        uint256 feeAmount = calcFee(balance);
        IERC20(getFeeToken()).transfer(getFeeTo(), feeAmount);

        IERC20(getFeeToken()).transfer(_msgSender(), balance - feeAmount);
        emit FundsWithdrawn(block.number, balance - feeAmount);
    }

    function adminMint(uint256 _mintAmount) external onlyAmbassador {
        //get if the _mint amount

        IERC20(getFeeToken()).transferFrom(
            _msgSender(),
            address(this),
            mintPrice * _mintAmount
        );
    ...
    function mintWhiteList() external {
        ...
        if (whitelistClaimed[_msgSender()]) revert WhitelistClaimed();
        ...
        if (!isWhitelisted[_msgSender()]) revert NotWhitelisted();
        whitelistClaimed[_msgSender()] = true;

        totalDeposited += mintPrice;

        _withdrawToCredit(_msgSender());
        _safeMint(_msgSender(), 1);

        setApprovalForAll(getAmbassadorAddress(), true);
        level[_nextTokenId()] = 1;
    }
    ...
    function _beforeTokenTransfers(
        ...
        if (
            _msgSender() != getAmbassadorAddress() &&
            to != address(0) &&
            from != address(0)
        ) revert UnauthorizedTransfer();

        level[startTokenId] = 1;
    }

    function upgrade(uint256 _tokenId, uint8 _level) external onlyAmbassador {
        if (!_exists(_tokenId)) revert NonexistentToken(_tokenId);
        level[_tokenId] += _level;
    }

    function downgrade(uint256 _tokenId, uint8 _level) external onlyAmbassador {
        if (!_exists(_tokenId)) revert NonexistentToken(_tokenId);
        if (_level >= level[_tokenId]) {
            level[_tokenId] = 1;
        } else {
            level[_tokenId] -= _level;
        }
    ...
    receive() external payable {
        // your code here…
        totalCapitalCollected += msg.value;
    }
    ...
    function withdraw() external {
        ...
        uint256 amount = dividendEarned(_msgSender());
        credit[_msgSender()] = 0;
        xDividendPerToken[_msgSender()] = dividendPerToken;

        bool success = IERC20(getFeeToken()).transfer(_msgSender(), amount);

        totalWithdrawn += amount;
        if (!success) revert WithdrawlFailed();
        emit WithdrawlMade(_msgSender(), amount);
    }

    function adminWithdraw(address _user) external onlyAmbassador {
        uint256 holderBalance = balanceOf(_user);
        if (holderBalance == 0) revert HoldingZeroTokens();

        uint256 amount = dividendEarned(_user);
        credit[_user] = 0;
        xDividendPerToken[_user] = dividendPerToken;

        bool success = IERC20(getFeeToken()).transfer(_msgSender(), amount);

        totalWithdrawn += amount;
        if (!success) revert WithdrawlFailed();
        emit AdminWithdrawlMade(_user, amount);
    }

    function distributeBackFunds(
        uint256 amount
    ) external payable onlyAmbassador {
        if (amount == 0) revert NoTokensToDistribute();
        if (totalSupply() == 0) revert SupplyIsZero();

        bool success = IERC20(getFeeToken()).transferFrom(
            _msgSender(),
            address(this),
            amount
        );
        ...
        totalCapitalRepaid += (amount - feeAmount);
        dividendPerToken += (amount - feeAmount) / totalSupply();
        if (!success) revert DepositFailed();
        emit DividendDeposited(block.number, amount - feeAmount);
    }
     ...
     */
    function burn(uint256 _tokenId) public virtual onlyAmbassador {
        if (!_exists(_tokenId)) revert NonexistentToken(_tokenId);
        _burn(_tokenId);
        emit AdminBurn(_tokenId, block.number);
    }

    function emergancyWithdraw() external onlyAmbassador {
...
import "./ChapterDao.sol";
import "./utils/ERC721PSI.sol";
...
contract ChapterDaoFactory is Ownable {
    bytes32 public constant INIT_CODE_PAIR_HASH =
        keccak256(abi.encodePacked(type(ChapterDao).creationCode));

    address public feeTo;
    address public feeToken;
    uint256 public feePercent = 100; //basePoints 10000 = 100% Defualt 1%

    mapping(uint256 => address) public allCollectionsIndex;
    ...
    constructor(address _feeToken, address _feeTo) {
        feeToken = _feeToken;
        feeTo = _feeTo;
    }
    ...
    function createChapterDao(
        string memory name_,
        string memory symbol_,
        string memory baseURI_,
        address ambassador_,
        address schoolDao_
    ) external onlyOwner returns (address payable collection) {
        bytes memory bytecode = type(ChapterDao).creationCode;
        bytes32 salt = keccak256(
            abi.encodePacked(name_, symbol_, block.number)
        );
        assembly {
            collection := create2(0, add(bytecode, 32), mload(bytecode), salt)
        }

        uint256 ambassador = SchoolDao(schoolDao_).mint(
            ambassador_,
            address(collection)
        );

        allCollectionsIndex[ambassador] = address(collection);

        allCollections.push(address(collection));
        isCollection[address(collection)] = true;
        collectionCount++;
        ChapterDao(collection).initialize(
            name_,
            symbol_,
            baseURI_,
            ambassador,
    ...
    function getCollections(uint256 _cursor, uint256 _length)
        public
        view
        returns (address[] memory)
    {
        uint256 length = allCollections.length;
        if (_cursor + _length > length) _length = length - _cursor;
        address[] memory result = new address[](_length);
        for (uint256 i = 0; i < _length; i++) {
            result[i] = allCollections[_cursor + i];
        }
        return result;
    }
}
...
contract SchoolDao is ERC721PsiBurnable, ReentrancyGuard, Ownable {
    constructor(
        string memory name_,
        string memory symbol_,
        string memory uri_
    ...
    mapping(address => uint256) AmbassadorsReverse;
    address public factory;
    ...
    function mint(address to, address chapterDao)
        external
        onlyFactory
        returns (uint256)
    {
        uint256 tokenId = _totalMinted() + 1;
        Ambassadors[tokenId] = chapterDao;
        AmbassadorsReverse[chapterDao] = tokenId;
        _safeMint(to, 1);
        return tokenId;
    }
    ...
    function getDao(uint256 tokenId) external view returns (address) {
        return Ambassadors[tokenId];
    }

    function isAmbassador(address _ambassador, address _chapter)
        external
        view
        returns (bool)
    {
        uint256 chapterId = AmbassadorsReverse[_chapter];
        if (ownerOf(chapterId) == _ambassador) {
            return true;
        } else {
            return false;
        }
    ...
    function getAmbassadorAddress(address chapterDao)
        external
        view
        returns (address)
    {
        return ownerOf(AmbassadorsReverse[chapterDao]);
    }
    ...
    function _afterTokenTransfers(
        ...
        super._afterTokenTransfers(from, to, tokenId, amount);
    }
}
