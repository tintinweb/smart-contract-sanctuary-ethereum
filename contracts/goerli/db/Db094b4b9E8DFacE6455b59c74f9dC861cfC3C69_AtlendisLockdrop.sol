 ...
 * @notice Lockdrop contract designed to rewards users that lock their Atlendis positions
 * for a given time with a corresponding amount of Atlendis tokens
 **/
contract AtlendisLockdrop is BaseLockdrop {
    ...
    bytes32 private immutable poolHash;
    uint256 private immutable minPositionAmount;
    address private immutable poolsController;
    ...
    function rescueRewards(address to) external override onlyOwner {
        (, bool defaulted, bool closed, , , , , , , , , ) = IPoolsController(
            address(poolsController)
        ).getPoolState(poolHash);
        require(defaulted || closed, "Pool is neither closed nor defaulted");
        uint256[] memory toRescue = new uint256[](tokens.length);
        for (uint256 i = 0; i < tokens.length; i++) {
            toRescue[i] =
                ERC20(tokens[i]).balanceOf(address(this)) -
                pendingRewards[tokens[i]];
            ERC20(tokens[i]).transfer(msg.sender, toRescue[i]);
        }
    ...
    function _lock(
        ...
        currentLocks[tokenId].owner = msg.sender;
...
abstract contract BaseLockdrop is Ownable {
    ...
    bytes32 public immutable root; // merkle tree root used for base token allocations, only used for the first token in the list
    ...
    mapping(address => uint256) public rates; // base tokens per second of locking
    ...
    /**
     * @notice Internal position lock ownership verification logic
     **/
    modifier isLockOwner(uint256 tokenId) {
        require(
            currentLocks[tokenId].owner != address(0),
            "Lock does not exist"
        );
        require(
            msg.sender == currentLocks[tokenId].owner,
    ...
    /**
     * @notice Preview rewards for lock
     **/
    function previewRewards(
        ...
        require(tokens.length > 0, "No token is registered");
        require(rates[token] > 0, "Token not registered");
     ...
     * A user locks its Atlendis position NFT in exchange for future rewards
    ...
    {
        _lock(tokenId, 0, lockingDuration);
    }
    ...
    function lock(
        ...
        bytes32 leaf = keccak256(abi.encode(msg.sender, baseAllocation));
        ...
        _lock(tokenId, baseAllocation, lockingDuration);
    }
     ...
     **/
    function quit(uint256 tokenId) external isLockOwner(tokenId) {
        require(
        ...
        emit Quit(tokenId, endDate, msg.sender);
    }

    /**
     * @notice Finalize lock, sends back position nft with corresponding rewards
     **/
    function withdraw(uint256 tokenId) external isLockOwner(tokenId) {
        require(
            block.timestamp >= currentLocks[tokenId].endDate,
            "Withdraw too early"
        );
        ...
        if (currentLocks[tokenId].claimed) delete currentLocks[tokenId];

        ERC721(nft).transferFrom(address(this), msg.sender, tokenId);

        emit Withdrawn(tokenId, endDate, msg.sender, rewards);
    }

    /**
     * @notice Finalize lock, sends back position nft with corresponding rewards
     **/
    function claim(uint256 tokenId) external isLockOwner(tokenId) {
        require(
            block.timestamp >= currentLocks[tokenId].endDate,
            "Claim too early"
        );
        ...
        bool toDelete = ERC721(nft).ownerOf(tokenId) != address(this);
        uint256[] memory rewards = new uint256[](tokens.length);
        for (uint256 i = 0; i < tokens.length; i++) {
            pendingRewards[tokens[i]] -= currentLocks[tokenId].rewards[
                tokens[i]
        ...
        for (uint256 i = 0; i < tokens.length; i++) {
            ERC20(tokens[i]).transfer(msg.sender, rewards[i]);
        }
        emit Claimed(tokenId, endDate, msg.sender, rewards);
    }

    ///////////////////////////
    // LOCKDROP CANCELLATION //
    ///////////////////////////

    /**
     * @notice Contract rewards can be withdrawn under target circumstances
     **/
