...
contract Settlement is BidSignatures {
    ...
    event SettlementFailure(address indexed bidder, string indexed reason);
    ...
    function _settle(
        ...
        if (weth.allowance(bidder, address(this)) >= totalWithoutTip) {
            ...
            if (p) {
                ...
                emit SettlementFailure(bidder, "Payment Failed");
            }
        } else {
            emit SettlementFailure(bidder, "Approve WETH");
        }
    }
        ...
        for (uint256 i; i < signatures.length; ) {
            if (signatures[i].bid.amount <= mintMax) {
                ...
                if (settle) { 
                    ...
                    emit SettlementFailure(signatures[i].bid.bidder, "Invalid Sig");
                }
            } else {
                emit SettlementFailure(signatures[i].bid.bidder, "Exceeds MintMax");
            }
