...
# ======================= GaugeControllerV2 ======================
# ====================================================================
# Frax Finance & Everipedia: https://github.com/FraxFinance
...
def _get_corrected_info(addr: address) -> CorrectedPoint:
    ...
    @notice Get the most recently recorded rate of voting power decrease for `addr`.
            Corrected by VOTE_WEIGHT_MULTIPLIER (hiIQ).
    @param addr Address of the user wallet
    @return CorrectedPoint
    """
    escrow: address = self.voting_escrow
    hiiq_balance: uint256 = VotingEscrow(escrow).balanceOf(addr)
    locked_balance: LockedBalance = VotingEscrow(escrow).locked(addr)
    locked_end: uint256 = locked_balance.end
    locked_iq: uint256 = convert(locked_balance.amount, uint256)

    corrected_slope: uint256 = 0

    # Decay to 0. Decaying to 1 hiIQ <> 1 IQ gives wrong expected results
    if (locked_end > block.timestamp):
        corrected_slope = hiiq_balance / (locked_end - block.timestamp)

    return CorrectedPoint({bias: hiiq_balance, slope: corrected_slope, lock_end: locked_end, iq_amount: locked_iq})
