...
{
	function trade(MetaPath[] calldata metaPath, uint256 amount) external returns (uint256);
...
{
	function trade(MetaPath[] calldata metaPath, uint256 amount) override external onlyOwner returns (uint256)
	{
		...
		unchecked
		{
			for(uint256 i = 0; i < len; ++i)
			{
				...
				IERC20(p.path[0]).approve(p.router, intermediateTokenBalance);
				if(p.hasFee)
				{
					IExofiCompatibleRouter(p.router).swapExactTokensForTokensSupportingFeeOnTransferTokens(
					...
					intermediateTokenBalance = IERC20(p.path[p.path.length - 1]).balanceOf(address(this));
				}
				else
				{
					uint256[] memory amountOuts = IExofiCompatibleRouter(p.router).swapExactTokensForTokens(
						intermediateTokenBalance,
						1,
						p.path,
						address(this),
	...
	function getAmountOutMin(MetaPath[] calldata metaPath, uint256 amount) override external view returns(uint256)
	{
		...
		unchecked
		{
			...
			{
				MetaPath memory p = metaPath[i];
				uint256[] memory amountOutMins = IExofiCompatibleRouter(p.router).getAmountsOut(amountOut, p.path);
				amountOut = amountOutMins[amountOutMins.length - 1];
			}
		}
		return amountOut;
	}
