...
library AppStorage {
  bytes32 constant APP_STORAGE_POSITION = keccak256("diamond.standard.black.baby.girls.storage");
  ...
  struct State {
        ...
        mapping(address => uint256) publicMintTotal;
        mapping(address => uint256) whitelistMintTotal;
...
contract SantaPosStore is ERC2981, ERC721A, ERC721AQueryable, ERC721ABurnable, ERC4907A, Ownable, DefaultOperatorFilterer721{
    using Strings for uint256;

    constructor() ERC721A("SantaPosStore", "SPS") {
      AppStorage.getState().MAX_SUPPLY = 50;
      AppStorage.getState().MAX_PUBLIC_MINT = 10;
      AppStorage.getState().TEAM_NFTS = 25;
      AppStorage.getState().PUBLIC_SALE_PRICE = .2 ether;
    }
    ...
    function mint(uint256 _quantity) external payable callerIsUser{
        require((totalSupply() + _quantity) <= AppStorage.getState().MAX_SUPPLY, "Exceeded Max Supply");
        require(msg.value >= (AppStorage.getState().PUBLIC_SALE_PRICE * _quantity), "Not Enough Eth");
        _safeMint(msg.sender, _quantity);
    }

    function teamMint() external onlyOwner{
        require(!AppStorage.getState().teamMinted, "Team NFTs already minted");
        AppStorage.getState().teamMinted = true;
        _safeMint(msg.sender, AppStorage.getState().TEAM_NFTS);
    }

    function _baseURI() internal view virtual override returns (string memory) {
        return AppStorage.getState().baseTokenUri;
    }

    function tokenURI(uint256 tokenId) public view virtual override(ERC721A, IERC721A) returns (string memory) {
        require(_exists(tokenId), "Nonexistent token");
        if(!AppStorage.getState().isRevealed){
            return AppStorage.getState().placeholderTokenUri;
        }
        return bytes(AppStorage.getState().baseTokenUri).length > 0 ? string(abi.encodePacked(AppStorage.getState().baseTokenUri, tokenId.toString(), ".json")) : "";
    }
    ...
    function setPublicPrice(uint256 _price) external onlyOwner{
        AppStorage.getState().PUBLIC_SALE_PRICE = _price;
    } 

    function setTokenUri(string memory _baseTokenUri) external onlyOwner{
       AppStorage.getState().baseTokenUri = _baseTokenUri;
    }

    function setPlaceHolderUri(string memory _placeholderTokenUri) external onlyOwner{
        AppStorage.getState().placeholderTokenUri = _placeholderTokenUri;
    }

    function toggleReveal() external onlyOwner{
        AppStorage.getState().isRevealed = !AppStorage.getState().isRevealed;
    }
    ...
    function getPlaceHolder() public view returns(string memory){
      return AppStorage.getState().placeholderTokenUri;
    }
}
