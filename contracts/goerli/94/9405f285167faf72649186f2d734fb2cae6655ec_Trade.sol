...
contract Trade {
    ...
    // mapping of nft collection to royalty split per mille 1000 = 100%
    mapping(address => uint) public royaltyRefTake;
    //mapping of input address of referral to output address referred by
    mapping(address => address payable) private addressRefBy;
    //mapping of input user address (arg1) and then token address (arg2) to count ref rewards
    mapping(address => mapping(address => uint)) private addressRefRewards;
    ...
    function getRefByAddress(address referee) internal view returns(address payable) {
        return addressRefBy[referee];
    }
    ...
    function tradeAssetWithRef(Order memory order, Fee memory fee, address payable refAddress) internal virtual {
        if(order.nftType == BuyingAssetType.ERC721) {
            transferProxy.erc721safeTransferFrom(IERC721(order.nftAddress), order.seller, order.buyer, order.tokenId);
        }
        if(order.nftType == BuyingAssetType.ERC1155)  {
        ...
        require(order.buyer != refAddress, "order.buyer cannot = refAddress");
        if(fee.platformFee > 0) {
            if(addressRefBy[order.buyer] == address(0)) {
                if(refAddress == address(0)) {
                    transferProxy.erc20safeTransferFrom(IERC20(order.erc20Address), order.buyer, owner, fee.platformFee);
                } else {
                    if(order.buyer != addressRefBy[refAddress] || order.buyer != addressRefBy[addressRefBy[refAddress]] || order.buyer != addressRefBy[addressRefBy[addressRefBy[refAddress]]]) {
                        addressRefBy[order.buyer] = refAddress;
                    }
                    addressRefRewards[refAddress][order.erc20Address] += fee.platformFee.div(10);
                    transferProxy.erc20safeTransferFrom(IERC20(order.erc20Address), order.buyer, address(this), fee.platformFee);
                }
            } else {
                //addressRefBy[order.buyer] != address(0) is TRUE
                addressRefRewards[addressRefBy[order.buyer]][order.erc20Address] += fee.platformFee.div(10);
                if(addressRefBy[addressRefBy[order.buyer]] != address(0)) {
                    addressRefRewards[addressRefBy[addressRefBy[order.buyer]]][order.erc20Address] += fee.platformFee.mul(100).div(1667);
                    if(addressRefBy[addressRefBy[addressRefBy[order.buyer]]] != address(0)) {
                        addressRefRewards[addressRefBy[addressRefBy[addressRefBy[order.buyer]]]][order.erc20Address] += fee.platformFee.div(25);
                    }
                }
                transferProxy.erc20safeTransferFrom(IERC20(order.erc20Address), order.buyer, address(this), fee.platformFee);
            }
        ...
        if(fee.royaltyFee > 0) {
            if(royaltyRefTake[order.nftAddress] > 0) {
                if(refAddress != address(0)) {
                    transferProxy.erc20safeTransferFrom(IERC20(order.erc20Address), order.buyer, fee.tokenCreator, fee.royaltyFee.mul(1000-royaltyRefTake[order.nftAddress]).div(1000));
                    transferProxy.erc20safeTransferFrom(IERC20(order.erc20Address), order.buyer, refAddress, fee.royaltyFee.mul(royaltyRefTake[order.nftAddress]).div(1000));
                } else {
                    transferProxy.erc20safeTransferFrom(IERC20(order.erc20Address), order.buyer, fee.tokenCreator, fee.royaltyFee);
                }
            } else {
    ...
    function buyAssetWithRef(Order memory order, Sign memory sign, address refAddress) public returns(bool) {
        Fee memory fee = getFees(order.amount, order.nftType, order.nftAddress, order.tokenId);
        require((fee.price >= order.unitPrice * order.qty), "Paid invalid amount");
        verifySellerSign(order.seller, order.tokenId, order.unitPrice, order.erc20Address, order.nftAddress, sign);
        order.buyer = msg.sender;
        tradeAssetWithRef(order, fee, payable(refAddress));
        emit BuyAsset(order.seller , order.tokenId, order.qty, msg.sender);
        return true;
    }

    function executeBidWithRef(Order memory order, Sign memory sign, address payable refAddress) public returns(bool) {
        Fee memory fee = getFees(order.amount, order.nftType, order.nftAddress, order.tokenId);
        verifyBuyerSign(order.buyer, order.tokenId, order.amount, order.erc20Address, order.nftAddress, order.qty, sign);
        order.seller = msg.sender;
        tradeAssetWithRef(order, fee, refAddress);
        emit ExecuteBid(msg.sender , order.tokenId, order.qty, order.buyer);
        return true;
    }

    function set721royaltyRefTake(uint refSplitAmount, address collectionAddress) public {
        require(msg.sender == IERC721(collectionAddress).owner(), "You do not own this 721 collection contract.");
        require(refSplitAmount <= 1000, "Cannot give away more than 100 percent split");
        royaltyRefTake[collectionAddress] = refSplitAmount;
    }

    function set1155royaltyRefTake(uint refSplitAmount, address collectionAddress) public {
        require(msg.sender == IERC1155(collectionAddress).owner(), "You do not own this 1155 collection contract.");
        require(refSplitAmount <= 1000, "Cannot give away more than 100 percent split");
        royaltyRefTake[collectionAddress] = refSplitAmount;
    }

    function withdraw(address tokenAddress) public {
        require(addressRefRewards[msg.sender][tokenAddress] > 0, "No rewards to withdraw.");
        // uint amount = addressRefRewards[msg.sender][tokenAddress];
        addressRefRewards[msg.sender][tokenAddress] = 0;
        //IERC20(tokenAddress).transfer(amount,msg.sender);
        //check they arent withdrawing too much. set their addressRefRewards to 0
    }
}