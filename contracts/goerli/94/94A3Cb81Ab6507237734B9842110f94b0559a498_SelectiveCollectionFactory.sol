...
contract SelectiveCollectionFactory is Pausable {
    ...
    function createCollection(
        ...
        SelectiveCollectionToken token = new SelectiveCollectionToken(
            name_,
            symbol_,
            msg.sender,
            limit_,
...
contract SelectiveCollectionToken is ERC721, ERC2981 {
    ...
    constructor(string memory name_, string memory symbol_, address creator_, uint256 limit_, uint96 fee_) ERC721(name_, symbol_) {
        _limit = limit_;
        _creator = creator_;
        _fee = fee_;
    }
    ...
    function batchMint(string[] memory token_uris) public {
        ...
        require(_limit >= tokenId + token_uris.length, "Limits tokens exceeded");
        require(msg.sender == _creator, "You are not an creator");
        for (uint256 i = 0; i < token_uris.length; i++) {
            _mint(msg.sender, tokenId);
            _token_uris[tokenId] = token_uris[i];
