    ...
    BumperAccessControl
{
    ...
    mapping(address => uint) public usdcForBumpPurchase;
    ...
    uint public constant BUMP_INITAL_PRICE = 6000;
    uint public constant SWAP_RATE_CONSTANT = 8;
    uint public constant BUMP_REWARDS_BONUS_DRAG = 68;
    uint public constant BUMP_REWARDS_BONUS_DRAG_DIVIDER = 11000;
    uint public constant BUMP_REWARDS_FORMULA_CONSTANT = 6 * (10**7);
    ...
    uint public swapRateParameter1;

    ///@dev This is used for bump token price calculations.
    ///@dev swapRateParameter2 has decimal precision of 4.
    uint public swapRateParameter2;
    ...
    event RewardIssued(address indexed rewardee, uint amount, uint price);

    ///@dev Emitted when BUMP is swapped for USDC during LPP
    event BumpPurchased(address indexed depositor, uint amount, uint price);
    ...
    event UpdatedMaxBumpPercent(
        string description,
        address sender,
        uint newMaxBumpPercent
    );
    event UpdatedBumpRewardAllocation(
        string description,
        address sender,
        uint newBumpRewardAllocation
    );
    event UpdatedBumpPurchaseAllocation(
        string description,
        address sender,
        uint newBumpPurchaseAllocation
    );
    ...
    event UpdatedFee(string description, address indexed sender, uint newFee);

    event UpdatedLevy(string description, address indexed sender, uint newLevy);

    event UpdateSwapRateParameter1(
        string description,
        address indexed sender,
        uint newSwapRateParameter1
    );

    event UpdateSwapRateParameter2(
        string description,
        address indexed sender,
        uint newSwapRateParameter2
    );
    ...
    event SwappedBUSDCToBUMP(address sender, uint amount);
    ...
    event TransferUsdc(address indexed receiver, uint amount);

    event DepositType(address indexed sender, uint amount, uint depositType);

    event UpdatedRewardPercent(address indexed sender, uint percent);
    ...
    function initialize(
        ...
        uint _maxBumpPercent,
        uint _bumpRewardAllocation,
        uint _bumpPurchaseAllocation
    ) public initializer {
        require(
    ...
    function swap(uint _amount)
        ...
        returns (bool)
    {
        uint currBalance = depositDetails[msg.sender].balance;
        uint bumpTokensAsRewards;
        uint bumpTokensPurchased;
    ...
    function _depositAmount(
        ...
        uint bumpPurchasePercent = (_amountForBumpPurchase * 10000) / _amount;
        uint amountToDeposit = _amount - _amountForBumpPurchase;
        uint bumpTokensAsRewards;
        uint bumpTokensPurchased;
        require(
    ...
    function approveUSDCToYearnVault(uint _amount)
        external
        virtual
        onlyGovernance
        whenNotPaused
    ...
    function depositUSDCInYearnVault(uint _amount)
        external
        virtual
        onlyGovernance
        nonReentrant
    ...
    function withdrawUSDCFromYearnVault(uint _amount)
        external
        virtual
        onlyGovernance
        returns (uint)
    {
        uint tokensRedeemed = IVault(usdcVault).withdraw(_amount);
    ...
    ///@return Returns the amount of USDC redeemed.
    function withdrawUSDCFromYearnVaultToAddress(uint _amount)
        external
        virtual
        onlyGovernance
        nonReentrant
        returns (uint)
    {
        uint tokensWithdrawn = IVault(usdcVault).withdraw(_amount);
    ...
    function updateMaxBumpPercent(uint _maxBumpPercent)
        external
        virtual
        onlyGovernance
    {
    ...
    function updateBumpRewardAllocation(uint _bumpRewardAllocation)
        external
        virtual
        onlyGovernance
    {
    ...
    ///@dev Decimal precision should be 18
    function updateBumpPurchaseAllocation(uint _bumpPurchaseAllocation)
        external
        virtual
        onlyGovernance
    {
    ...
    function updateSwapAllowedPeriod(uint _start, uint _end)
        external
        virtual
        onlyGovernance
    {
    ...
    function updateRrewardPercent(uint percent)
        external
        virtual
        onlyGovernance
    {
    ...
    ///@dev This function was modified in version 2.
    function estimateSwapRateBumpUsdc(uint _deposit)
        public
        view
        virtual
        returns (uint)
    {
        uint totalDepositsAfterDeposit = totalDeposits + _deposit;
        return
    ...
    function getCurrentPrice(StableCoins _coin)
        ...
        returns (int)
    {
        AggregatorV3Interface priceFeed = stableCoinsDetail[
    ...
    function getSwapRateBumpUsdc() public view virtual returns (uint) {
        return
    ...
    ///@dev This function was modified in version 2.
    function getBumpRewards(uint _totalDeposit, uint _amountForPurchase)
        internal
        view
        virtual
        returns (uint)
    {
        ...
        uint bumpRewards = (getBumpPurchaseAmount(_amountForPurchase) *
            rewardPercent) / (100 * 10**2);
        return bumpRewards;
    }
    ...
    function getBumpPurchaseAmount(uint _amountForPurchase)
        internal
        view
        virtual
        returns (uint)
    {
        ...
        uint bumpPurchaseAmount = (_amountForPurchase * 10**12 * 10**4) /
            (getSwapRateBumpUsdc());
    ...
    function getBumpAllocation(uint _amountForDeposit, uint _amountForPurchase)
        internal
        virtual
        returns (uint, uint)
    {
        uint bumpRewards = getBumpRewards(
        ...
        uint bumpPurchased = getBumpPurchaseAmount(_amountForPurchase);
        require(
    ...
    ///@param _amount Amount of USDC you want to approve to yearn vault.
    function _approveUSDCToYearnVault(uint _amount)
        internal
        virtual
        whenNotPaused
    {
    ...
    function _depositUSDCInYearnVault(uint _amount)
        internal
        virtual
        whenNotPaused
    {
    ...
    function updateFee(uint _fee) external virtual onlyGovernance {
        fee = _fee;
    ...
    ///@dev It has a decimal precision of 2 decimals.
    function updateLevy(uint _levy) external virtual onlyGovernance {
        levy = _levy;
    ...
    function updateSwapRateParameter1(uint _swapRateParameter1)
        external
        virtual
        onlyGovernance
    {
    ...
    function updateSwapRateParameter2(uint _swapRateParameter2)
        external
        virtual
        onlyGovernance
    {
    ...
    function withdrawLiquidity(uint amount)
        external
        virtual
        lockWithdrawal
        whenNotPaused
    ...
    ///@param amount Number of bUSDC tokens user wants to burn.
    ///@return true if everything is successful.
    function withdrawLiquidity(address receiver, uint amount)
        public
        virtual
        lockWithdrawal
        whenNotPaused
    ...
    function _withdrawLiquidity(address receiver, uint amount)
        internal
        returns (bool)
    {
        uint feeCalculated;
        uint levyCalculated;
        uint pricePerShare;
        uint yUSDCTokensBurn;
        uint tokensWithdrawn;
        uint amountAfterDeduduction;
