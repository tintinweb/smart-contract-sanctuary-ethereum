...
contract LizhiNiubi1111 is ERC721, Ownable {

    using Strings for uint256;

    //可以铸造的nft的最大数量
    uint private constant MAX_TOKENS = 1500;
    //测试第一次先mint几个
    uint private constant TOKENS_RESERVED = 3;
    //价格
    uint256 public price = 50000000000000000; //16ge0 0.05eth
    //白名单价格
    uint256 public whitePrice = 30000000000000000;
    //单次交易最大铸币数量
    uint256 public constant MAX_MINT_PER_TX = 5;

    //白名单名单数量
    uint256 public maxNumberOfWhitelistedAddresses = 40;
    ...
    constructor() ERC721("Lizhiniubi1111", "SYMBOL") {
        // root = _root;
        baseUri = "ipfs://QmaqVgKwtoyuWvj7JqiPexmcvik125Km2FhVmAsTKncPss/";
    }

    // Public Functions
    function mint1(address userAccount, uint256 _numTokens) external payable {
        ...
        if(isWhitelisted(userAccount) && verifyUserAddress(userAccount)) {
