...
// File contracts/UnimoonFarming.sol
...
contract UnimoonFarming is Ownable, ReentrancyGuard {
    // 3 MONTHS
    uint256 constant public UNLOCK_TIME = 20 minutes;//90 days;
    ...
    /**
     * @param _lpToken - address of lp token usdc/unimoon
     * @param _rewardToken - address of unimoon token
     */
        ...
        nonReentrant
    {
        ...
        farmingDuration = 20 minutes; //30 days;
    ...
    function getInfo(address _user)
        ...
        returns (
            ...
            uint256 _unimoonReward,
            uint256 _unimoonWithdrawed,
            uint256 _unimoonLpWithdrawed 
        )
    {
        UserInfo storage user = userInfo[_user];
        uint256 unimoonLpWithdrawed = lpWithdrawed[_user];
        if (isWithdraw[_user]) {
            return (
                farmingEnd,
                farmingDuration,
            ...
            uint256 unimoonReward = pendingReward(_user);
            return (
                farmingEnd,
                farmingDuration,
                rewardPerSec / 1e24,
