...
contract Render3NUM is IRender3NUM, Ownable {
    ...
    uint256 private _maxSubTokens = 1000;
    ...
    function setMaxMintCount(uint256 count) external onlyOwner {
        if (count <= _tokenSubIds.current()) revert InvalidMaxMintCount(count);
    ...
    function mintPFP(uint256 tokenId) external onlyMinter returns (uint256) {
        if (_tokenSubIds.current() >= _maxSubTokens) revert MintLimitReached();
    ...
    function tokenURI(uint256 _tokenId) external view returns (string memory) {
        ...
        string memory tokenLabel = string(abi.encodePacked("#", subId.toString()));
        string memory _name = string(
            abi.encodePacked("3NUM Shield ", edition, " Edition ", tokenLabel)
        );
        ...
        string memory image = generateSVGImage(subId, string(abi.encodePacked(edition, " ",tokenLabel)));
