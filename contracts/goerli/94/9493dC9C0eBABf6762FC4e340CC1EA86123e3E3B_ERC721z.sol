...
contract ERC721z is Ownable, SignerRole, IERC721, ERC721Base {
    ...
    //获取转账税
    function getTransferTax() public view returns(IERC20,address,uint256){ 
        return(token,recipientAddress,transferTax);
    }
    ...
    function taxTransferFrom(address from, address to, uint256 tax) private{
        require(token.transferFrom(from, to, tax), errorInfo); // 从转出者账户扣除转账税
    }
    ...
    function safeTransferFrom(address from, address to, uint256 tokenId, bytes memory _data) public override(ERC721,IERC721){
        ...
        taxTransferFrom(from,recipientAddress,tax);
