...
// LICENSE
// XNounsDAOLogicV2.sol is a modified version of Compound Lab's GovernorBravoDelegate.sol:
...
//
// MODIFICATIONS
// See XNounsDAOLogicV1 for initial GovernorBravoDelegate modifications.

// XNounsDAOLogicV2 adds:
...
contract XNounsDAOLogicV2 is XNounsDAOStorageV2, XNounsDAOEventsV2 {
     ...
     * @param timelock_ The address of the XNounsDAOExecutor
     * @param xnouns_ The address of the XNOUN tokens
    ...
    function initialize(
        ...
        require(address(timelock) == address(0), 'XNounsDAO::initialize: can only initialize once');
        if (msg.sender != admin) {
            revert AdminOnly();
        }
        require(timelock_ != address(0), 'XNounsDAO::initialize: invalid timelock address');
        require(xnouns_ != address(0), 'XNounsDAO::initialize: invalid xnouns address');
        require(
            votingPeriod_ >= MIN_VOTING_PERIOD && votingPeriod_ <= MAX_VOTING_PERIOD,
            'XNounsDAO::initialize: invalid voting period'
        );
        require(
            votingDelay_ >= MIN_VOTING_DELAY && votingDelay_ <= MAX_VOTING_DELAY,
            'XNounsDAO::initialize: invalid voting delay'
        );
        require(
            proposalThresholdBPS_ >= MIN_PROPOSAL_THRESHOLD_BPS && proposalThresholdBPS_ <= MAX_PROPOSAL_THRESHOLD_BPS,
            'XNounsDAO::initialize: invalid proposal threshold bps'
        );
        ...
        timelock = IXNounsDAOExecutor(timelock_);
        xnouns = XNounsTokenLike(xnouns_);
        vetoer = vetoer_;
    ...
    function propose(
        ...
        temp.totalSupply = xnouns.totalSupply();

        temp.proposalThreshold = bps2Uint(proposalThresholdBPS, temp.totalSupply);

        require(
            xnouns.getPriorVotes(msg.sender, block.number - 1) > temp.proposalThreshold,
            'XNounsDAO::propose: proposer votes below proposal threshold'
        );
        require(
            targets.length == values.length &&
                targets.length == signatures.length &&
                targets.length == calldatas.length,
            'XNounsDAO::propose: proposal function information arity mismatch'
        );
        require(targets.length != 0, 'XNounsDAO::propose: must provide actions');
        require(targets.length <= proposalMaxOperations, 'XNounsDAO::propose: too many actions');
        ...
        if (temp.latestProposalId != 0) {
            ...
            require(
                ...
                'XNounsDAO::propose: one live proposal per proposer, found an already active proposal'
            );
            require(
                proposersLatestProposalState != ProposalState.Pending,
                'XNounsDAO::propose: one live proposal per proposer, found an already pending proposal'
            );
        }
    ...
    function queue(uint256 proposalId) external {
        require(
            ...
            'XNounsDAO::queue: proposal can only be queued if it is succeeded'
        );
    ...
    function queueOrRevertInternal(
        ...
        require(
            ...
            'XNounsDAO::queueOrRevertInternal: identical proposal action already queued at eta'
        );
    ...
    function execute(uint256 proposalId) external {
        require(
            ...
            'XNounsDAO::execute: proposal can only be executed if it is queued'
        );
        Proposal storage proposal = _proposals[proposalId];
    ...
    function cancel(uint256 proposalId) external {
        ...
        require(
            msg.sender == proposal.proposer ||
                xnouns.getPriorVotes(proposal.proposer, block.number - 1) <= proposal.proposalThreshold,
            'XNounsDAO::cancel: proposer above threshold'
        );
    ...
    function state(uint256 proposalId) public view returns (ProposalState) {
        require(proposalCount >= proposalId, 'XNounsDAO::state: invalid proposal id');
        Proposal storage proposal = _proposals[proposalId];
        if (proposal.vetoed) {
    ...
    function castVoteBySig(
        ...
        require(signatory != address(0), 'XNounsDAO::castVoteBySig: invalid signature');
    ...
    function castVoteInternal(
        ...
        require(state(proposalId) == ProposalState.Active, 'XNounsDAO::castVoteInternal: voting is closed');
        require(support <= 2, 'XNounsDAO::castVoteInternal: invalid vote type');
        Proposal storage proposal = _proposals[proposalId];
        Receipt storage receipt = proposal.receipts[voter];
        require(receipt.hasVoted == false, 'XNounsDAO::castVoteInternal: voter already voted');

        /// @notice: Unlike GovernerBravo, votes are considered from the block the proposal was created in order to normalize quorumVotes and proposalThreshold metrics
        uint96 votes = xnouns.getPriorVotes(voter, proposalCreationBlock(proposal));
    ...
    function _setVotingDelay(uint256 newVotingDelay) external {
        ...
        require(
            ...
            'XNounsDAO::_setVotingDelay: invalid voting delay'
        );
    ...
    function _setVotingPeriod(uint256 newVotingPeriod) external {
        ...
        require(
            ...
            'XNounsDAO::_setVotingPeriod: invalid voting period'
        );
    ...
    function _setProposalThresholdBPS(uint256 newProposalThresholdBPS) external {
        ...
        require(
            ...
            'XNounsDAO::_setProposalThreshold: invalid proposal threshold bps'
        );
    ...
    function _setMinQuorumVotesBPS(uint16 newMinQuorumVotesBPS) external {
        ...
        require(
            ...
            'XNounsDAO::_setMinQuorumVotesBPS: invalid min quorum votes bps'
        );
        require(
            newMinQuorumVotesBPS <= params.maxQuorumVotesBPS,
            'XNounsDAO::_setMinQuorumVotesBPS: min quorum votes bps greater than max'
        );
    ...
    function _setMaxQuorumVotesBPS(uint16 newMaxQuorumVotesBPS) external {
        ...
        require(
            ...
            'XNounsDAO::_setMaxQuorumVotesBPS: invalid max quorum votes bps'
        );
        require(
            params.minQuorumVotesBPS <= newMaxQuorumVotesBPS,
            'XNounsDAO::_setMaxQuorumVotesBPS: min quorum votes bps greater than max'
        );
    ...
    function _setPendingAdmin(address newPendingAdmin) external {
        // Check caller = admin
        require(msg.sender == admin, 'XNounsDAO::_setPendingAdmin: admin only');
    ...
    function _acceptAdmin() external {
        ...
        require(msg.sender == pendingAdmin && msg.sender != address(0), 'XNounsDAO::_acceptAdmin: pending admin only');
    ...
    function _burnVetoPower() public {
        ...
        require(msg.sender == vetoer, 'XNounsDAO::_burnVetoPower: vetoer only');
    ...
    function proposalThreshold() public view returns (uint256) {
        return bps2Uint(proposalThresholdBPS, xnouns.totalSupply());
    }
    ...
    function getDynamicQuorumParamsAt(uint256 blockNumber_) public view returns (DynamicQuorumParams memory) {
        uint32 blockNumber = safe32(blockNumber_, 'XNounsDAO::getDynamicQuorumParamsAt: block number exceeds 32 bits');
    ...
    function minQuorumVotes() public view returns (uint256) {
        return bps2Uint(getDynamicQuorumParamsAt(block.number).minQuorumVotesBPS, xnouns.totalSupply());
    }
    ...
    function maxQuorumVotes() public view returns (uint256) {
        return bps2Uint(getDynamicQuorumParamsAt(block.number).maxQuorumVotesBPS, xnouns.totalSupply());
    }
