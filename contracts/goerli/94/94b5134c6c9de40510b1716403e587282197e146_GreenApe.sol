...
contract GreenApe is Ownable, ERC721A, ReentrancyGuard {
    ...
    mapping(address => bool) private GreenApeMinion;
    mapping(address => uint) private mintedPerAddress;

    constructor() ERC721A('GreenApe', 'GreenApe') {}

    function addMinion(address a) public onlyOwner {
        GreenApeMinion[a] = true;
    }

    function removeMinion(address a) public onlyOwner {
        GreenApeMinion[a] = false;
    }

    modifier onlyMinions() {
        require(GreenApeMinion[_msgSender()], 'you are not GreenApe minion');
        _;
    }
    ...
    function mintFromGA(address a, uint quantity) public onlyMinions mintCompliance(quantity) {
        _safeMint(a, quantity);
    }

    function burnFromGA(uint256 tokenId) public onlyMinions {
