...
contract ERC721 is IERC721, IERC721Metadata, ERC165{
    ...
    constructor(string memory _oname, string memory _osymbol, string memory _baseUri){
        owner = msg.sender;
        _name = _oname;
        _symbol = _osymbol;
        baseUri = _baseUri;
    ...
    function approve(address _spender, uint256 _tokenId) public {
        ...
        require(owners[_tokenId] == msg.sender || operatorAprovals[msg.sender][_spender] == true || tokenAprovals[_tokenId] == msg.sender, "ERC721: approve caller is not owner nor approved for all");
    ...
    function setApprovalForAll(address _operator, bool _approved) public {
        require(msg.sender != _operator, "ERC721: approval to current owner");
    ...
    function transferFrom(address _from, address _to, uint256 _tokenId) external {
        require(owners[_tokenId] == _from, "ERC721: from is not the owner of the tokenId");
        require(owners[_tokenId] == msg.sender || operatorAprovals[msg.sender][_from] == true || tokenAprovals[_tokenId] == msg.sender, "ERC721: transfer caller is not owner nor approved");
        //Удаляет разрешения для _tokenId
    ...
    function safeTransferFrom( address _from, address _to, uint256 _tokenId) external {
        require(owners[_tokenId] == _from, "ERC721: from is not the owner of the tokenId");
        require(owners[_tokenId] == msg.sender || operatorAprovals[msg.sender][_from] == true || tokenAprovals[_tokenId] == msg.sender, "ERC721: transfer caller is not owner nor approved");
        require(_checkOnERC721Received(_from, _to, _tokenId, bytes('a')) == true, "ERC721: transfer to non ERC721Receiver implementer");
    ...
    function safeTransferFrom(address _from, address _to, uint256 _tokenId, bytes memory _data) public {
        require(owners[_tokenId] == _from, "ERC721: from is not the owner of the tokenId");
        require(owners[_tokenId] == msg.sender || operatorAprovals[msg.sender][_from] == true || tokenAprovals[_tokenId] == msg.sender, "ERC721: transfer caller is not owner nor approved");
        require(_checkOnERC721Received(_from, _to, _tokenId, _data) == true, "ERC721: transfer to non ERC721Receiver implementer");
        tokenAprovals[_tokenId] = _to;
        owners[_tokenId] == _to;
        balances[_from] -= 1;
        balances[_to] += 1;
