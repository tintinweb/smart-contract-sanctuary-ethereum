...
error HamtaroNft__NoMoreFreeNft();
error HamtaroNft__NotEnoughETH();
error HamtaroNft__RangeOutOfBound();

/**
 * @title HamtaroNft
 * @author jrmunchkin
 * @notice This contract creates a NFT collection on the theme of Hamtaro.
 * Hamtaro characters are sort regarding the rarity from Grail to Common. The chance to get a character or another is based on randomness + the percentage of the rarity.
 ...
 */
contract HamtaroNft is VRFConsumerBaseV2, ERC721URIStorage, Ownable {
    enum Grail {
        HAMTARO
    }
    ...
    string[][5] private s_hamtaroTokenUris;
     ...
     * @param _hamtaroTokenUris Array of the token uris of each characters
    ...
    constructor(
        ...
        string[][5] memory _hamtaroTokenUris,
        uint256 _mintFee
    ) VRFConsumerBaseV2(_vrfCoordinatorV2) ERC721("Hamtaro", "HAM") {
        i_vrfCoordinator = VRFCoordinatorV2Interface(_vrfCoordinatorV2);
        i_subscriptionId = _subscriptionId;
        i_gasLane = _gasLane;
        i_callbackGasLimit = _callbackGasLimit;
        s_tokenCounter = 0;
        s_hamtaroTokenUris = _hamtaroTokenUris;
        s_mintFee = _mintFee;
    }
    ...
    function mintFreeNft() external {
        if (s_userFreeNft[msg.sender] >= 3) revert HamtaroNft__NoMoreFreeNft();
        requestNft();
    ...
    function mintNft() external payable {
        if (msg.value < s_mintFee) revert HamtaroNft__NotEnoughETH();
        requestNft();
    }
    ...
    function fulfillRandomWords(
        ...
        _setTokenURI(newTokenId, s_hamtaroTokenUris[rarity][character]);
    ...
    function getRandomCharacter(
        ...
        revert HamtaroNft__RangeOutOfBound();
    }
    ...
    function getRandomCharacterfromRarity(
        ...
        if (_indexRarity == 0)
            return uint256(Grail(_randomWord % (uint256(Grail.HAMTARO) + 1)));
        if (_indexRarity == 1)
            return
                uint256(
                    Legendary(_randomWord % (uint256(Legendary.HARMONY) + 1))
                );
        if (_indexRarity == 2)
            return uint256(Rare(_randomWord % (uint256(Rare.PASHMINA) + 1)));
        if (_indexRarity == 3)
            return
                uint256(Uncommon(_randomWord % (uint256(Uncommon.PANDA) + 1)));
        if (_indexRarity == 4)
            return uint256(Common(_randomWord % (uint256(Common.NINHAM) + 1)));
    ...
    function getChanceArray() internal pure returns (uint256[5] memory) {
        return [2, 6, 20, 50, MAX_CHANCE_VALUE];
    }
     ...
     */
    function getHamtaroTokenUri(
        uint256 _rarity,
        uint256 _index
    ) external view returns (string memory) {
        return s_hamtaroTokenUris[_rarity][_index];
    }
