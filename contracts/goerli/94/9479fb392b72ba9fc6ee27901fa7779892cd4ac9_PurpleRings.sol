...
contract PurpleRings is CosmeticERC721A {
  ...
  constructor() CosmeticERC721A("PurpleRings", "PR") {
  ...
  function isEligible(address _address) public view override returns (bool) {
    ...
    uint256 addressStreak = cargo.getAddressSteak(_address);
...
interface IDailyCargo is IERC721A {
    ...
    function getAddressSteak(address _address) external view returns (uint256);

    function burnCargo(uint256 _tokenId) external;
}
...
/**
 * @title CosmeticERC721A
 * @dev ERC721A for cosmetics on FreeNFT.xyz.
 */
abstract contract CosmeticERC721A is ERC721A, Ownable {
  ...
  modifier onlyCosmeticRegistry(address _address) {
    require(_address == cosmeticRegistry);
    _;
  }
   ...
   */
  function claim(address _to) public onlyCosmeticRegistry(msg.sender) {
    _mint(_to, 1);
  }
