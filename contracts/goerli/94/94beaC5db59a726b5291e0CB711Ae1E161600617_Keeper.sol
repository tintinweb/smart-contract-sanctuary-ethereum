...
import {IterableCollection} from "../Utils/Libs/Iterables.sol";
import "@openzeppelin/contracts/access/Ownable.sol";
import "@openzeppelin/contracts/token/ERC20/ERC20.sol";
...
contract UI2 is UserInterface {
    ...
    constructor(
        ...
        string memory instrument_
    ) UserInterface(m_, opt_type_, usdc_account_manager_, instrument_) {
        _consumer = new PriceConsumerV3(price_oracle_address_);
        _price_per_token = defaultPricePerToken();
        _individual_switch_limit = 10000 ether;
        _vault_max_limit = 100000000 ether;
    }
        ...
        returns (bool)
    {
        ...
        bool usr_riskcheck_ = _long_token.balanceOf(buyer_address_) +
            _short_token.balanceOf(buyer_address_) +
        ...
        for (uint256 i = max_i_; i > 0; ) {
            ...
            if (_no_riskcheck[mm_address_]) {
                mm_bal_ +=
                    ((_long_token.balanceOf(mm_address_) +
                        _short_token.balanceOf(mm_address_)) *
                        (_price_per_token)) /
                    (1 ether);
            } else {
    ...
    function preSettlement() internal override {
        _total_tokens = totalLongTokenSupply() + totalShortTokenSupply();
    }

    function postSettlement() internal override {
        uint256 new_total_tokens_ = totalLongTokenSupply() +
            totalShortTokenSupply();
        if (new_total_tokens_ > _total_tokens) {
            _short_token.burn(
                _short_token.allHolderAddresses()[0],
                new_total_tokens_ - _total_tokens
            );
        } else if (new_total_tokens_ < _total_tokens) {
            _short_token.mint(
                _short_token.allHolderAddresses()[0],
                _total_tokens - new_total_tokens_
            );
        }
    }
...
abstract contract UserInterface is RoleManager {
    ...
    event SettlementDone(
        uint256 prev_settleemt_price,
        uint256 curr_settlement_price
    );
    event ResetAllDone();
    ...
    mapping(address => bool) private _long_to_short_full_switch_map;
    ...
    mapping(address => bool) private _short_to_long_full_switch_map;
    ...
    SwitchToken internal _long_token;
    SwitchToken internal _short_token;
    ProxyUSDC internal _usdc;
    USDCAccountManager internal _usdc_account_manager;

    uint256 private _fees;
    address private _fee_collector;

    uint256 private constant _default_px_per_token = 1 ether;
    uint256 private _m;
    ...
    constructor(
        ...
        _m = m_;
        _opt_type = opt_type_;
        _usdc_account_manager = USDCAccountManager(usdc_account_manager_);
        _prev_settlement_time = block.timestamp;
        _long_token = new SwitchToken(
            string(abi.encodePacked("LONGSWITCH", instrument_)),
            string(abi.encodePacked("LSWITCH", instrument_))
        );
        _short_token = new SwitchToken(
            string(abi.encodePacked("SHORTSWITCH", instrument_)),
            string(abi.encodePacked("SSWITCH", instrument_))
        );
    }
    ...
    function preSettlement() internal virtual;

    function postSettlement() internal virtual;

    // =============== Admin block =================
    function setLongTokenAddress(address long_token_) public onlyRoleHolder(0) {
        _long_token = SwitchToken(long_token_);
    }
    ...
    function setVaultAddress(address vault_) public onlyRoleHolder(0) {
        _usdc_account_manager = USDCAccountManager(vault_);
    }
    ...
    function longTokenSellAmount(address long_token_seller_address_)
        ...
        returns (uint256)
    {
        if (_long_token_full_sellers_map[long_token_seller_address_]) {
            return _long_token.balanceOf(long_token_seller_address_);
        }
    ...
    function shortTokenSellAmount(address short_token_seller_address_)
        ...
        returns (uint256)
    {
        if (_short_token_full_sellers_map[short_token_seller_address_]) {
            return _short_token.balanceOf(short_token_seller_address_);
        }
    ...
    function longToShortSwitchAmount(address long_to_short_address_)
        ...
        returns (uint256)
    {
        if (_long_to_short_full_switch_map[long_to_short_address_]) {
            return _long_token.balanceOf(long_to_short_address_);
        }
    ...
    function shortToLongSwitchAmount(address short_to_long_address_)
        ...
        returns (uint256)
    {
        if (_short_to_long_full_switch_map[short_to_long_address_]) {
            return _short_token.balanceOf(short_to_long_address_);
        }
    ...
    function getUSDCValue() public view returns (uint256) {
        return _usdc_account_manager.totalBalance();
    }
    ...
    function sellLongToken(uint256 sell_amount_token_) external {
        require(!_long_token_full_sellers_map[msg.sender], "All sold");
        require(!_long_to_short_full_switch_map[msg.sender], "All switched");

        uint256 seller_balance_ = _long_token.balanceOf(msg.sender);
    ...
    function sellAllLongTokens() external {
        if (!_long_token_full_sellers_map[msg.sender]) {
            cancelSwitchFromLongToShort();
            uint256 token_balance_ = _long_token.balanceOf(msg.sender);
            uint256 sold_amount_ = _long_token_sellers_map[msg.sender];

            _long_token_sellers_map[msg.sender] = token_balance_;

            if (sold_amount_ == 0) {
                _long_token_sellers_addresses.push(msg.sender);
            }

            _long_token_full_sellers_map[msg.sender] = true;

            unchecked {
                _total_long_token_sell_amount =
                    _total_long_token_sell_amount +
                    token_balance_ -
                    sold_amount_;
            }

            emit SellLongTokenDone(msg.sender, token_balance_);
        }
    }

    function cancelLongTokenSell() public {
        uint256 sold_amount_ = _long_token_sellers_map[msg.sender];
        if (sold_amount_ > 0) {
            unchecked {
        ...
        _long_token_full_sellers_map[msg.sender] = false;
        emit CancelSellLongTokenDone(msg.sender);
    }

    function sellShortToken(uint256 sell_amount_token_) external {
        require(!_short_token_full_sellers_map[msg.sender], "All sold");
        require(!_short_to_long_full_switch_map[msg.sender], "All switched");

        uint256 seller_balance_ = _short_token.balanceOf(msg.sender);
    ...
    function sellAllShortTokens() external {
        if (!_short_token_full_sellers_map[msg.sender]) {
            cancelSwitchFromShortToLong();
            uint256 token_balance_ = _short_token.balanceOf(msg.sender);
            uint256 sold_amount_ = _short_token_sellers_map[msg.sender];

            _short_token_sellers_map[msg.sender] = token_balance_;

            if (sold_amount_ == 0) {
                _short_token_sellers_addresses.push(msg.sender);
            }

            _short_token_full_sellers_map[msg.sender] = true;

            unchecked {
                _total_short_token_sell_amount =
                    _total_short_token_sell_amount +
                    token_balance_ -
                    sold_amount_;
            }

            emit SellShortTokenDone(msg.sender, token_balance_);
        }
    }

    function cancelShortTokenSell() public {
        uint256 sold_amount_ = _short_token_sellers_map[msg.sender];
        if (sold_amount_ > 0) {
            unchecked {
        ...
        _short_token_full_sellers_map[msg.sender] = false;
    ...
    function switchFromLongToShort(uint256 switch_amount_) external {
        ...
        uint256 long_balance_ = _long_token.balanceOf(msg.sender);
    ...
    function switchAllFromLongToShort() external {
        if (!_long_to_short_full_switch_map[msg.sender]) {
            cancelLongTokenSell();
            uint256 token_balance_ = _long_token.balanceOf(msg.sender);
            uint256 switched_amount_ = _long_to_short_map[msg.sender];

            _long_to_short_map[msg.sender] = token_balance_;

            if (switched_amount_ == 0) {
                _long_to_short_addresses.push(msg.sender);
            }

            _long_to_short_full_switch_map[msg.sender] = true;

            unchecked {
                _total_long_to_short_amount =
                    _total_long_to_short_amount +
                    token_balance_ -
                    switched_amount_;
            }

            emit SwitchFromLongToShortDone(msg.sender, token_balance_);
        }
    }

    function cancelSwitchFromLongToShort() public {
        uint256 switched_amount_ = _long_to_short_map[msg.sender];
        if (switched_amount_ > 0) {
            unchecked {
        ...
        _long_to_short_full_switch_map[msg.sender] = false;
    ...
    function switchFromShortToLong(uint256 switch_amount_) external {
        ...
        uint256 short_balance_ = _short_token.balanceOf(msg.sender);
    ...
    function switchAllFromShortToLong() external {
        if (!_short_to_long_full_switch_map[msg.sender]) {
            cancelShortTokenSell();
            uint256 token_balance_ = _short_token.balanceOf(msg.sender);
            uint256 switched_amount_ = _short_to_long_map[msg.sender];

            _short_to_long_map[msg.sender] = token_balance_;

            if (switched_amount_ == 0) {
                _short_to_long_addresses.push(msg.sender);
            }

            _short_to_long_full_switch_map[msg.sender] = true;

            unchecked {
                _total_short_to_long_amount =
                    _total_short_to_long_amount +
                    token_balance_ -
                    switched_amount_;
            }

            emit SwitchFromShortToLongDone(msg.sender, token_balance_);
        }
    }

    function cancelSwitchFromShortToLong() public {
        uint256 switched_amount_ = _short_to_long_map[msg.sender];
        if (switched_amount_ > 0) {
            unchecked {
        ...
        _short_to_long_full_switch_map[msg.sender] = false;
    ...
    function adjustLongTokenHoldings(
        ...
        _long_token.runSettlement(adj_factor_);
    }
    ...
    function adjustShortTokenHoldings(
        ...
        _short_token.runSettlement(adj_factor_);
    }
        ...
        onlyRoleHolder(1)
    {
        ...
        uint256 long_tokens_ = _long_token.totalSupply();
        uint256 short_tokens_ = _short_token.totalSupply();
        ...
        // withdrawal
        uint256 max_i_ = _long_token_sellers_addresses.length;
        for (uint256 i = max_i_; i > 0; ) {
            (
            ...
            if (long_token_sell_amount_tokens_ > 0) {
                usr_bal_ = _long_token.balanceOf(long_token_seller_address_);
                ...
                _usdc_account_manager.withdrawBalance(settlement_amount_);
                unchecked {
                    settlement_amount_ -= fees_to_deduct_;
                }

                _usdc.transfer(address(_fee_collector), fees_to_deduct_);
                _usdc.transfer(
                    address(long_token_seller_address_),
                    settlement_amount_
                );
        ...
        for (uint256 i = max_i_; i > 0; ) {
            ...
            if (short_token_sell_amount_tokens_ > 0) {
                usr_bal_ = _short_token.balanceOf(short_token_seller_address_);
        ...
        for (uint256 i = max_i_; i > 0; ) {
            ...
            if (long_to_short_amount_tokens_ > 0) {
                usr_bal_ = _long_token.balanceOf(long_to_short_address_);
        ...
        for (uint256 i = max_i_; i > 0; ) {
            ...
            if (short_to_long_amount_tokens_ > 0) {
                usr_bal_ = _short_token.balanceOf(short_to_long_address_);
        ...
        // infusion
        max_i_ = _long_token_buyers_addresses.length;
        for (uint256 i = max_i_; i > 0; ) {
            (
            ...
            if (long_token_buy_amount_usdc_ > 0) {
                ...
                usr_bal_ = _usdc.balanceOf(long_token_buyer_address_);
                allowance_ = _usdc.allowance(
                ...
                _usdc_account_manager.addBalance(long_token_buy_amount_usdc_);
                ...
                _long_token.mint(long_token_buyer_address_, buy_amount_token_);
            }
        ...
        for (uint256 i = max_i_; i > 0; ) {
            ...
            if (short_token_buy_amount_usdc_ > 0) {
                usr_bal_ = _usdc.balanceOf(short_token_buyer_address_);
                allowance_ = _usdc.allowance(
                ...
                _usdc_account_manager.addBalance(short_token_buy_amount_usdc_);
        ...
        emit SettlementDone(_prev_settlement_price, current_price_);
    }

    function _longTokenBuyerReadAndPop()
        private
        returns (address last_address_, uint256 deposit_value_)
    {
        last_address_ = _long_token_buyers_addresses[
            _long_token_buyers_addresses.length - 1
        ];
        deposit_value_ = _long_token_buyers_map[last_address_];
        ...
        returns (address last_address_, uint256 deposit_value_)
    {
        ...
        deposit_value_ = _short_token_buyers_map[last_address_];
    ...
    function _longTokenSellerReadAndPop()
        private
        returns (address last_address_, uint256 sell_amount_)
    {
        last_address_ = _long_token_sellers_addresses[
            _long_token_sellers_addresses.length - 1
        ];

        sell_amount_ = _long_token_sellers_map[last_address_];
        unchecked {
            _total_long_token_sell_amount -= sell_amount_;
        }
        delete _long_token_sellers_map[last_address_];

        if (_long_token_full_sellers_map[last_address_]) {
            sell_amount_ = _long_token.balanceOf(last_address_);
            delete _long_token_full_sellers_map[last_address_];
        }
        ...
        returns (address last_address_, uint256 sell_amount_)
    {
        ...
        sell_amount_ = _short_token_sellers_map[last_address_];
        unchecked {
            _total_short_token_sell_amount -= sell_amount_;
        }
        delete _short_token_sellers_map[last_address_];

        if (_short_token_full_sellers_map[last_address_]) {
            sell_amount_ = _short_token.balanceOf(last_address_);
            delete _short_token_full_sellers_map[last_address_];
        }
    ...
    function _longToShortReadAndPop()
        private
        returns (address last_address_, uint256 switch_amount_)
    {
        last_address_ = _long_to_short_addresses[
            _long_to_short_addresses.length - 1
        ];

        switch_amount_ = _long_to_short_map[last_address_];
        unchecked {
            _total_long_to_short_amount -= switch_amount_;
        }
        delete _long_to_short_map[last_address_];

        if (_long_to_short_full_switch_map[last_address_]) {
            switch_amount_ = _long_token.balanceOf(last_address_);
            delete _long_to_short_full_switch_map[last_address_];
        }
        ...
        returns (address last_address_, uint256 switch_amount_)
    {
        ...
        switch_amount_ = _short_to_long_map[last_address_];
        unchecked {
            _total_short_to_long_amount -= switch_amount_;
        }
        delete _short_to_long_map[last_address_];

        if (_short_to_long_full_switch_map[last_address_]) {
            switch_amount_ = _short_token.balanceOf(last_address_);
            delete _short_to_long_full_switch_map[last_address_];
        }
    ...
    function resetAll(bool redeem_) public onlyRoleHolder(0) {
        ...
        if (redeem_) {
            ...
            max_i_ = (_long_token.allHolderAddresses()).length;
            for (uint256 i = max_i_; i > 0; ) {
                (
                    address long_token_seller_address_,
                    uint256 long_token_sell_amount_tokens_
                ) = _long_token.fetchLastHolderAndPop();
            ...
            max_i_ = (_short_token.allHolderAddresses()).length;
            for (uint256 i = max_i_; i > 0; ) {
                (
                    address short_token_seller_address_,
                    uint256 short_token_sell_amount_tokens_
                ) = _short_token.fetchLastHolderAndPop();

                if (short_token_sell_amount_tokens_ > 0) {
                    uint256 settlement_amount_;
                    unchecked {
                        if (i == 1) {
                            settlement_amount_ = _usdc_account_manager
                                .totalBalance();
                        } else {
                            settlement_amount_ =
                                (short_token_sell_amount_tokens_ *
...
contract Keeper is KeeperCompatibleInterface, Ownable {
    event SettlementCalled(
        uint256 last_settlement_time_,
        uint256 current_settlement_time_,
        uint256 last_settlement_px_,
        uint256 current_settlement_px_
    );
    /**
    ...
    constructor(address user_interface_address_, address price_oracle_address_)
    {
        ...
        )
    {
        ...
        upkeep_needed_ = last_round_time_ > _last_settlement_time;
    }
    ...
    function performUpkeep(
        ...
        if (last_round_time_ > _last_settlement_time) {
            _user_interface.executeSettlement(
                uint256(last_round_px_),
                last_round_time_
            );
            emit SettlementCalled(
                _last_settlement_time,
                last_round_time_,
                _last_settlement_px,
                uint256(last_round_px_)
            );
            _last_settlement_time = last_round_time_;
