...
contract Primes is ERC721r, Ownable {
    ...
    uint256 private constant MAX_SUPPLY = 1575 + 1; // ERC721r starts at 0.
    uint256 private constant GIFTS = 75;
    uint256 private constant PRESALE_MAX_SUPPLY = 600;
    uint256 private constant MAX_SUPPLY_MINUS_GIFTS = MAX_SUPPLY - GIFTS;
    ...
    function privateMint(
        ...
        require(
            totalSupply() + _quantity <= PRESALE_MAX_SUPPLY,
            "Maximum presale supply exceeded"
        );
        require(currentTime() >= start, "Sale has not begun yet");
        require(currentTime() < start + 2 days, "Sale is finished");
        require(_quantity > 0, "Mint at least 1 NFT");
        if (msg.sender != owner()) {
            require(isEligible(msg.sender), "Not eligible to mint");
            require(
                addressMintedBalance[msg.sender] + _quantity <= maxPerAddress,
        ...
        _mintRandom(_account, _quantity);
    }
    ...
    function publicMint(
        ...
        require(!paused, "Contract paused");
        require(step == Steps.PUBLIC_SALE, "Sale has not begun yet");
        require(
            totalSupply() + _quantity <= MAX_SUPPLY_MINUS_GIFTS,
        ...
        for (y = 0; y <= 800; y += 100) {
            for (x = 0; x <= 800; x += 100) {
                ...
                if (PrimesUtils.isPrime(uint256(count))) {
                    ...
                    string memory start_or_end = PrimesUtils.setAnimationEvent(
                        index,
                        (_tokenId + count)
                    );
    ...
    function isEligible(address _user) public view returns (bool) {
        for (uint i = 0; i < allowlisting.length; i++) {
            if (allowlisting[i] == _user) {
                return true;
            }
        }
        return false;
    ...
    // setting
    function setAllowlisting(address[] calldata _users) public onlyOwner {
        delete allowlisting;
        allowlisting = _users;
    }
