    ...
    ReentrancyGuard
{
    ...
    function merkWL(bytes32[] calldata _merkleProof, address _sender, uint8 _table)internal view returns(bool){
        bytes32 leaf = keccak256(abi.encodePacked(_sender));
        return MerkleProof.verify(_merkleProof, merkleRoot[_table], leaf);
    }


    function verifySigner(bytes memory _signature) internal view returns (address){
    ...
    function mintVIP(bytes32[] calldata _merkleProof) public virtual{
        require(!alreadymint[msg.sender],"You are already minted");
        require(!_isContract(msg.sender), "It is contracts");
        require(block.timestamp >= startTime[1] && block.timestamp < endTime[1], "It is not sellTime");
        require(merkWL(_merkleProof, msg.sender, 1),"You are not VIP");
        require(_tokenIdTracker + 1 <= 3000,"NFT is already limited");
        alreadymint[msg.sender] = true;
        _tokenIdTracker = _tokenIdTracker + 1;
    ...
    function mintWhitelist(bytes32[] calldata _merkleProof) public virtual{
        require(!alreadymint[msg.sender],"You are already minted");
        require(block.timestamp >= startTime[2] && block.timestamp < endTime[2], "It is not sellTime");
        require(!_isContract(msg.sender), "It is contracts");
        require(merkWL(_merkleProof, msg.sender, 2) || merkWL(_merkleProof, msg.sender, 1),"You are not whitelist or VIP");
        require(_tokenIdTracker + 1 <= 3000,"NFT is already limited");
        alreadymint[msg.sender] = true;
        _tokenIdTracker = _tokenIdTracker + 1;
        _safeMint(msg.sender, 1);
    ...
    function mintPublic(bytes memory _signature) public virtual nonReentrant{
        require(verifySigner(_signature)==msg.sender,"The signer is error");
        require(!_isContract(msg.sender), "It is contracts");
        require(!alreadymint[msg.sender],"You are already minted");
