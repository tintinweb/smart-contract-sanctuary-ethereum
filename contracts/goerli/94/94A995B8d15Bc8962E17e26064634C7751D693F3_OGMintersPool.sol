...
// File: contracts/OGMintersPool.sol
...
contract OGMintersPool is Ownable {
    ...
    function claim_OGMinters_RewardM(uint256 _tokenId, bytes32[] calldata merkleproof)public{
        newTokenId = 5001 + _tokenId;
        require(isclaimRewardPaused==false,"claimReward is paused");
        bytes32 leaf = keccak256(abi.encodePacked(msg.sender));
        require(MerkleProof.verify( merkleproof, merkleRoot, leaf),"Not on claim list");
        ...
        require(newTokenId <= 7001, "NFT Token ID is greater than 2000" );
        uint256 _userCut=address(this).balance/2000;
        payable(msg.sender).transfer(_userCut);
        claimuserStatus[msg.sender][newTokenId]=true;
    
    }

       function claim_OGMinters_Reward(uint256 _tokenId )public{
        newTokenId = 5001 + _tokenId;
        require(isclaimRewardPaused==false,"claimReward is paused");
        // bytes32 leaf = keccak256(abi.encodePacked(msg.sender));
        // require(MerkleProof.verify( merkleproof, merkleRoot, leaf),"Not on claim list");
