...
contract TheAIArtGame is Ownable, ERC2981, ERC721A, ReentrancyGuard {
    ...
    string[8] private split = [
        '<html><style>body{padding: 0; margin: 0; font-family: "Poppins", sans-serif; background:',
        "; height: 100vh; width: 100vw;}span{text-transform: uppercase;position: absolute;left: 50%; top: 50%; transform: translate(-50%, -50%); display: block; color: ",
        '; font-size: 124px; letter-spacing: 8px; cursor: pointer;}span::before{content: "',
        '"; position: absolute; color: transparent; background-image: repeating-linear-gradient( 45deg, transparent 0, transparent 2px, white 2px, ',
        '4px ); -webkit-background-clip: text; top: 0px; left: 0; z-index: -1; transition: 1s;}span::after{content: "',
        '"; position: absolute; color: transparent; background-image: repeating-linear-gradient( 135deg, transparent 0, transparent 2px, white 2px,',
        "4px ); -webkit-background-clip: text; top: 0px; left: 0px; transition: 1s;}span:hover:before{top: 10px; left: 10px;}span:hover:after{top: -10px; left: -10px;}</style><span>",
        "</span></html>"
    ];
    ...
    function animationCreator(
        ...
        if (animationType == 0) {
            ...
            for (uint256 i = 0; i < 7; i++) {
                string memory splitBytes = word;
                if (i < 3) {
                    splitBytes = string(
                        rgba(
                            generateNumber(_seed + 69 * i, 255),
                            generateNumber(_seed + 420 * i, 255),
                            generateNumber(_seed + 123 * i, 255),
                            1
                        )
                    );
                }
            ...
            animationBytes = abi.encodePacked(animationBytes, split[7]);
        ...
        returns (string memory)
    {
        ...
        string memory animation = animationCreator(word, _seed, animationType);
