...
contract Pool is IYieldFarmingV1Pool, Initializable, AccessControlUpgradeable {
    ...
    function initialize(YieldFarmConfig memory cfg, address roleAdmin) public initializer {
        __YieldFarm_init(cfg, roleAdmin);
    }

    function __YieldFarm_init(YieldFarmConfig memory cfg, address roleAdmin) internal onlyInitializing {
        __AccessControl_init();
        __YieldFarm_init_unchained(cfg, roleAdmin);
    }

    function __YieldFarm_init_unchained(YieldFarmConfig memory cfg, address roleAdmin) internal onlyInitializing {
        ...
        epochPoolSizeCache = new uint256[](cfg.numberOfEpochs + 1);
    ...
    function _harvest(uint128 epochId) internal returns (uint256) {
        ...
        return totalAmountPerEpoch * _getUserBalancePerEpoch(msg.sender, epochId) / epochPoolSizeCache[epochId];
    }
