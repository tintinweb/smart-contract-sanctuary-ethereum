...
contract YaaasMultipleExchange is
    ...
    function addOffer(
        CreateOffer calldata newOffer
    ) override external {
        (bool success, ) = address(newOffer._collection).call(
            abi.encodeWithSignature("isLoyalty()")
        );
        if (success) {
            require(ILoyalty(newOffer._collection).isResaleAllowed(newOffer._assetId, _msgSender()), "Marketplace: Resale not allowed"); 
        }  
        _addOffer(
            newOffer
        );
    ...
    function _addOffer(
        CreateOffer memory newOffer
    ) internal {

        require(newOffer._collection != address(0), "Marketplace: Collection address is not valid");
        require(newOffer._token != address(0), "Marketplace: Token address is not valid");
        require(newOffer._price > 0, "Marketplace: Price must be greater than zero");
        require(newOffer._amount > 0, "Marketplace: Amount must be greater than zero");
        require(newOffer._expiresAt > block.timestamp, "Marketplace: invalid expire time");
         
        // get NFT asset from seller
        IERC1155 multipleNFTCollection = IERC1155(newOffer._collection);
        require(
            multipleNFTCollection.balanceOf(_msgSender(), newOffer._assetId) >= newOffer._amount,
        ...
        bytes32 offerId = keccak256(
            abi.encodePacked(newOfferId, _msgSender(), newOffer._collection, newOffer._assetId)
        );
        ...
        emit Listed(offerId, _msgSender(), newOffer._collection, newOffer._assetId, newOffer._price, newOffer._amount, newOffer.eventIdListed);
    }

    function setOfferPrice(bytes32 offerID, uint256 price, uint256 eventIdSetOfferPrice)
        override
        external
    ...
    function setForSell(bytes32 offerID, bool isForSell, uint256 eventIdSetForSell)
        override
        external
        onlyOfferOwner(offerID)
    {
    ...
    function setForAuction(bytes32 offerID, bool isForAuction, uint256 eventIdSetForAuction)
        override
        external
        onlyOfferOwner(offerID)
    {
    ...
    function setExpiresAt(bytes32 offerID, uint256 expiresAt, uint256 eventIdSetExpireAt)
        override
        external
        onlyOfferOwner(offerID)
    {
    ...
    function cancelOffer(bytes32 offerID, uint256 eventIdCancelOffer) override external onlyOfferOwner(offerID) {
    ...
    function buyOffer(bytes32 id, uint256 amount, uint256 eventIdSwapped) override external payable {
    ...
    */
    function delegateBuy(bytes32 id, uint256 amount, address buyer, uint256 eventIdSwapped) external payable {
        Offer memory offer = offers[id];
    ...
    function _buyOffer(
        ...
        if (success) {
			(address creator, uint256 creatorBenif) = ILoyalty(offer.collection).computeCreatorLoyaltyByAmount (
            offer.assetId,
            offer.seller,
            sellerAmount
            );
    ...
    function _createBid(
        ...
        require(
            IERC20(offer.token).allowance(_msgSender(), address(this)) >= _price,
    ...
    function cancelBid(bytes32 _offerId, address _bidder, uint256 eventIdBidCancelled) override external {
    ...
    function acceptBid(bytes32 _offerID, address _bidder, uint256 eventIdBidSuccessful)
        override
        external
...
contract FeeManager is IFeeManager, Ownable {
    ...
    function setFeeTo(uint256 index, uint256 newFee) override external onlyOwner {
...
interface IYaaasMultipleExchange {
     ...
     */
    function setOfferPrice(bytes32 _offerId, uint256 _price, uint256 eventIdSetOfferPrice) external;
     ...
     */
    function setForSell(bytes32 _offerId, bool _isForSell, uint256 eventIdSetForSell) external;
    ...
    * @param  eventIdSetForAuction a tracking id used to sync db
     */
    function setForAuction(bytes32 _offerId, bool _isForAuction, uint256 eventIdSetForAuction) external;
    ...
    * @param  eventIdSetExpireAt a tracking id used to sync db
     */
    function setExpiresAt(bytes32 _offerId, uint256 _expiresAt, uint256 eventIdSetExpireAt) external;
    ...
    * @param  eventIdCancelOffer a tracking id used to sync db
     ...
     */
    function buyOffer(bytes32 _offerId, uint256 amount, uint256 eventIdSwapped) external payable;
     ...
     */
    function cancelBid(bytes32 _offerId, address _bidder, uint256 eventIdBidCancelled) external;
     ...
     */
    function acceptBid(bytes32 _offerId, address _bidder, uint256 eventIdBidSuccessful) external;
    ...
    * @param  eventIdListed a tracking id used to sync db
    ...
    event SetOfferPrice(bytes32 _offerId, uint256 price, uint256 eventIdSetOfferPrice);
    event SetForSell(bytes32 _offerId, bool isForSell, uint256 eventIdSetForSell);
    event SetForAuction(bytes32 _offerId, bool isForAuction, uint256 eventIdSetForAuction);
    event SetExpireAt(bytes32 _offerId, uint256 expiresAt, uint256 eventIdSetExpireAt);
