...
contract SaleClockAuction is ClockAuction {
    ...
    // Tracks last 5 sale price of gen0 jet sales
...
contract OsizJetBase is ERC721Base("Jet", "OSIZJet") {

    /// @dev The Build event is fired whenever a new jet comes into existence.
    event Build(address owner, uint256 jetId, uint256 inventorModelId, uint256 architectModelId);

  
    struct Jet {

        // The timestamp from the block when this jetcame into existence.
        uint64 buildTime;

        // The minimum timestamp after which this jet can able build
        // new jets again.
        uint64 recoveryEndTime;

        // The ID of the originator of this jet, set to 0 for gen0 jets.
        uint32 inventorModelId;
        uint32 architectModelId;

        // Set to the ID of the architectModel jetfor inventorModels that are preProduction,
        // zero otherwise. 
        uint32 ProcessingWithId;

        // Set to the index in the recovery array (see below) that represents
        // the current recovery duration for this Jet. This starts at zero
        // for gen0 jets, and is initialized to floor(generation/2) for others.
        // Incremented by one for each successful building action, regardless
        // of whether this jetis acting as inventorModel or architectModel.
        uint16 recoveryIndex;

        // The "generation number" of this jet.
        // for sale are called "gen0" and have a generation number of 0. The
        // generation number of all other jets is the larger of the two generation
    ...
    /// @dev A lookup table jeting the recovery duration after any successful
    ///  building action, called "processing time" for inventorModels and "Processing recovery"
    ///  for architectModels. Designed such that the recovery roughly doubles each time a jet
    ///  is build, encouraging owners not to just keep building the same jetover
    ///  and over again. Caps out at one week (a jetcan build an unbounded number
    ...
    /// @dev An array containing the Jet struct for all jets in existence. The ID
    ///  of each jetis actually an index into this array. 
    Jet[] jets;

    /// @dev A mapping from JetIDs to an address that has been approved to use
    ///  this Jet for Processing via buildWith(). Each Jet can only have one approved
    ...
    /// @dev The address of the ClockAuction contract that handles sales of jets. This
    ...
    constructor(){
     ...
     function _transfer(
        ...
        // once the jet is transferred also clear architectModel allowances
        delete architectModelAllowedToAddress[tokenId];
    }

    /// @dev An internal method that creates a new jet and stores it. This
    /// @param _inventorModelId The jet ID of the inventorModel of this jet(zero for gen0)
    /// @param _architectModelId The jet ID of the architectModel of this jet(zero for gen0)
    /// @param _generation The generation number of this jet, must be computed by caller.
    /// @param _owner The inital owner of this jet, must be non-zero (except for the unJet, ID 0)
    function _createJet(
        ...
        returns (uint)
    {
        ...
        // New jet starts with the same recovery as parent gen/2
        ...
        jets.push(_jet);
        uint256 newjetId =  jets.length - 1;

      
        require(newjetId == uint256(uint32(newjetId)));

        // emit the build event
        emit Build(
            _owner,
            newjetId,
            uint256(_jet.inventorModelId),
            uint256(_jet.architectModelId)
        );

        // This will assign ownership, and also emit the Transfer event as
        // per ERC721 draft
        _mint(_owner, newjetId);

        return newjetId;
    }

    /// @notice Returns the total number of jets currently in existence.
    /// @dev Required for ERC-721 compliance.
    function totalSupply() public view returns (uint) {
        return jets.length - 1;
    }

    /// @notice Returns a list of all Jet IDs assigned to an address.
    /// @param _owner The owner whose jets we are interested in.
    /// @dev This method MUST NEVER be called by smart contract code. First, it's fairly
    ///  expensive (it walks the entire Jet array looking for jets belonging to owner),
    ...
    function tokensOfOwner(address _owner) external view returns(uint256[] memory ownerTokens) {
        ...
        if (tokenCount == 0) {
            ...
            uint256 totalJets = totalSupply();
            uint256 resultIndex = 0;

            // We count on the fact that all jets have IDs starting at 1 and increasing
            // sequentially up to the totalJetcount.
            uint256 jetId;

            for (jetId = 1; jetId <= totalJets; jetId++) {
                if (_owners[jetId] == _owner) {
                    result[resultIndex] = jetId;
                    resultIndex++;
                }
            }
...
contract OsizJetBuildBase is OsizJetBase{

    event Inventing(address owner, uint256 inventorModelId, uint256 architectModelId, uint256 recoveryEndTime);

    /// @notice The minimum payment required to use build new jet 
    uint256 public autoBuildFee = 2e15;
    // platform charge percentage
    uint256 public charge = 10; // 10 equals 1% 
    // Keeps track of number of preProduction jet.
    uint256 public preProductionJets;
 
    /// @dev Checks that a given jet is able to build. Requires that the
    ///  current recovery is finished (for architectModels) and also checks that there is
    ///  no pending processing.
    function _isReadyToBuild(Jet memory _jet) internal view returns (bool) {
        // In addition to checking the recoveryEndTime, we also need to check to see if
        // the jethas a pending launch; there can be some period of time between the end
        // of the processing timer and the build event.
        return (_jet.ProcessingWithId == 0) && (_jet.recoveryEndTime <= uint64(block.timestamp));
    }
    ...
    /// @dev Set the recoveryEndTime for the given Jet, based on its current recoveryIndex.
    ///  Also increments the recoveryIndex (unless it has hit the cap).
    /// @param _jet A reference to the Jet in storage which needs its timer started.
    function _triggerRecovery(Jet storage _jet) internal {
        // Compute an estimation of the recovery time in blocks (based on current recoveryIndex).
        _jet.recoveryEndTime = uint64((recovery[_jet.recoveryIndex]) + block.timestamp);

        // Increment the building count, clamping it at 13
        if (_jet.recoveryIndex < 13) {
            _jet.recoveryIndex += 1;
        }
    }

    /// @notice Grants approval to another user to architectModel with one of your Jet.
    /// @param _addr The address that will be able to  create new jet using architectModel with your Jet. Set to
    ///  address(0) to clear all Processing approvals for this Jet.
    /// @param _architectModelId A Jet that you own that _addr will now be able to architectModel with.
    ...
    /// @dev Updates the minimum payment required for building new jet. 
    function setAutoBuildFee(uint256 val) external onlyOwner {
        autoBuildFee = val;
    }

    /// @dev Checks to see if a given Jet is preProduction and (if so) if the processing
    ///  period has passed.
    function _isReadyToLaunch(Jet memory _inventorModel) private view returns (bool) {
        return (_inventorModel.ProcessingWithId != 0) && (_inventorModel.recoveryEndTime <= uint64(block.timestamp));
    }

    /// @notice Checks that a given jet is able to build (i.e. it is not preProduction or
    ///  in the middle of a Processing recovery).
    /// @param _jetId reference the id of the jet, any user can inquire about it
    function isReadyToBuild(uint256 _jetId)
        public
        view
        returns (bool)
    {
        require(_jetId > 0);
        Jet storage _jet = jets[_jetId];
        return _isReadyToBuild(_jet);
    }

    /// @dev Checks whether a jet is currently preProduction.
    /// @param _jetId reference the id of the jet, any user can inquire about it
    function isInventing(uint256 _jetId)
        public
        view
        returns (bool)
    {
        require(_jetId > 0);
        // A jet is preProduction if and only if this field is set
        return jets[_jetId].ProcessingWithId != 0;
    }

    /// @dev Internal check to see if a given architectModel and inventorModel are a valid inventor model.
    /// @param _inventorModel A reference to the Jet struct of the potential inventorModel.
    /// @param _inventorModelId The inventorModel's ID.
    /// @param _architectModel A reference to the Jet struct of the potential architectModel.
    /// @param _architectModelId The architectModel's ID
    function _isValidInventorModel(
        Jet storage _inventorModel,
        uint256 _inventorModelId,
        Jet storage _architectModel,
        uint256 _architectModelId
    )
        private
        view
        returns(bool)
    {
        // A Jet can't build with itself!
        if (_inventorModelId == _architectModelId) {
            return false;
        }

        // Jet can't build with inventor .
    ...
    function _canBuildWithViaAuction(uint256 _inventorModelId, uint256 _architectModelId)
        ...
        returns (bool)
    {
        Jet storage inventorModel = jets[_inventorModelId];
        Jet storage architectModel = jets[_architectModelId];
        return _isValidInventorModel(inventorModel, _inventorModelId, architectModel, _architectModelId);
    }

    /// @notice Checks to see if two jet can build together, including checks for
    ...
    function _buildWith(uint256 _inventorModelId, uint256 _architectModelId) internal {
        // Grab a reference to the Jet from storage.
        Jet storage architectModel = jets[_architectModelId];
        Jet storage inventorModel = jets[_inventorModelId];
        ...
        // Every time a new jet gets in preProduction, counter is incremented.
        preProductionJets++;
    ...
    /// @notice Build a Jet you own (as inventorModel) with a architectModel that you own, or for which you
    ///  have previously been given ArchitectModel approval. Will either make your jet preProduction, or will
    ///  fail entirely. Requires a pre-payment of the fee given out to the first caller of giveBuild()
    /// @param _inventorModelId The ID of the Jet acting as inventorModel (will end up preProduction if successful)
    /// @param _architectModelId The ID of the Jet acting as architectModel (will begin its Processing recovery if successful)
    function buildNewJet(uint256 _inventorModelId, uint256 _architectModelId)
        external
        payable
        whenNotPaused
    {
        ...
        // Test that these jet are a valid inventor model.
        ...
        // All checks passed, jet gets preProduction!
        _buildWith(_inventorModelId, _architectModelId);
    }

    /// @notice Have a preProduction Jet give build!
    /// @param _inventorModelId A Jet ready to give build.
    /// @return The Jet ID of the new jet.
    /// @dev Looks at a given Jet and, if preProduction and if the processing period has passed,
    ///  combines the  of the two inventor  to create a new jet. The new Jet is assigned
    ///  to the current owner of the inventorModel. Upon successful completion, both the inventorModel and the
    ///  new jet will be ready to build again. Note that anyone can call this function (if they
    ///  are willing to pay the gas!), but the new jet always goes to the inventorModel's owner.
    function LaunchJet(uint256 _inventorModelId)
        external
        whenNotPaused
        returns(uint256)
    {
        ...
        // Check that the inventorModel is a valid jet.
        ...
        Jet storage architectModel = jets[architectModelId];
        ...
        uint256 jetId = _createJet(_inventorModelId, inventorModel.ProcessingWithId, parentGen + 1,  owner);

        delete inventorModel.ProcessingWithId;

        // Every time a jet gives build counter is decremented.
        preProductionJets--;
...
contract AuctionBase is OsizJetBuildBase {
        ...
        whenNotPaused
    {
        ...
        // If jet is already on any auction, this will throw
        // because it will be owned by the auction contract.
        require(_owns(msg.sender, _jetId));
        // Ensure the jet is not inventing to prevent the auction
        // contract accidentally receiving ownership of the child.
        // NOTE: the jet IS allowed to be in a recovery.
        require(!isInventing(_jetId));
        _approve(address(saleAuction),_jetId);
        // Sale auction throws if inputs are invalid and clears
        // transfer and architectModel approval after escrowing the jet.
    ...
    /// to the JetCore contract. We use two-step withdrawal to
...
contract OsizJetMinting is AuctionBase {

    // Constants for gen0 auctions.
    uint256 public constant GEN0_STARTING_PRICE = 10e15;
    uint256 public constant GEN0_AUCTION_DURATION = 1 days;

    // Counts the number of jets the contract owner has created.
    uint256 public promoCreatedCount;
    uint256 public gen0CreatedCount;

    /// @dev we can create promo jets, up to a limit. Only callable by owner
    /// @param _owner the future owner of the created jets. Default to contract owner
    function createPromoJet(address _owner) external onlyOwner {
        address jetOwner = _owner;
        if (jetOwner == address(0)) {
             jetOwner = owner();
        }

        promoCreatedCount++;
        _createJet(0, 0, 0,  jetOwner);
    }

    /// @dev Creates a new gen0 jet
    ///  creates an auction for it.
    function createGen0Auction(address _tokenAddress) external onlyOwner {

        uint256 jetId = _createJet(0, 0, 0, address(this));
        _approve(address(saleAuction), jetId);
...
contract OsizJetLaunch is OsizJetMinting{

    constructor (string memory _baseUri) {

        _createJet(0, 0, 0, address(0));
    ...
    function getJet(uint256 _id)
        ...
        Jet storage _jet = jets[_id];

        // if this variable is 0 then it's not building 
        isProcessing = (_jet.ProcessingWithId != 0);
        isReady = (_jet.recoveryEndTime <= block.timestamp);
        recoveryIndex = uint256(_jet.recoveryIndex);
        nextActionAt = uint256(_jet.recoveryEndTime);
        ProcessingWithId = uint256(_jet.ProcessingWithId);
        buildTime = uint256(_jet.buildTime);
        inventorModelId = uint256(_jet.inventorModelId);
        architectModelId = uint256(_jet.architectModelId);
        generation = uint256(_jet.generation);
    }

     // @dev Allows the owner to capture the balance available to the contract.
    function withdrawBalance() external onlyOwner {
        uint256 balance = address(this).balance;
        // Subtract all the currently preProduction jets we have, plus 1 of margin.
        uint256 subtractFees = (preProductionJets + 1) * autoBuildFee;
