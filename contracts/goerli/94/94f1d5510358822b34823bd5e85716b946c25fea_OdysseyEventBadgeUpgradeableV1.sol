    ...
    UUPSUpgradeable
{
    ...
    function mintEventBadgeAndTransfer(
        ...
        uint256 attendeeBalance = balanceOf(attendee, eventBadgeId);

        require(attendeeBalance == 0, "Attendee already having the badge");

        bytes memory tokenData = (bytes(eventData));

        _mint(msg.sender, eventBadgeId, 1, tokenData);

        safeTransferFrom(msg.sender, attendee, eventBadgeId, 1, tokenData);
    }
