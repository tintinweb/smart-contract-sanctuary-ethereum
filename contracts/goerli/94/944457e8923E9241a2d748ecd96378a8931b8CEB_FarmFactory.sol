...
import './FarmsStorage.sol';
import './Farm.sol';

contract FarmFactory is Initializable, OwnableUpgradeable {
  ...
  FarmsStorage public farmsStorage;
  address public farmImplementation;

  mapping(address => UserFarm[]) public userFarms;

  function initialize(address _feeManager, address _farmsStorage)
    public
    initializer
  {
    feeManager = FarmsFeeManager(_feeManager);
    farmsStorage = FarmsStorage(_farmsStorage);
    farmImplementation = address(new Farm());
    isEnabled = true;

    __Ownable_init();
  }

  function setFarmImplementation(address _farmImplementation)
    external
    onlyOwner
  {
  ...
  function upgradeFarm(address payable _farm) external onlyOwner {
    TransparentUpgradeableProxy(_farm).upgradeTo(farmImplementation);
  }
  ...
  function _deploy(CreateFarmParams memory info) internal {
    ...
    TransparentUpgradeableProxy farm = new TransparentUpgradeableProxy(
      farmImplementation,
      owner(),
    ...
    Farm(address(farm)).fund(info.amount);

    farmsStorage.addFarm(address(farm));
  ...
  function farms(uint256 index) external view returns (address) {
    return farmsStorage.farms(index);
  }

  function farmsLength() external view returns (uint256) {
    return farmsStorage.farmsLength();
  }
  ...
  function deposit(address farmAddress, uint256 amount) external {
    ...
    farmInstance.deposit(amount, msg.sender);

    farm.farm = farmInstance;
    farm.deposited = farm.deposited + amount;

    if (found) {
      userFarms[msg.sender][index] = farm;
      return;
    }
  ...
  function withdraw(address farmAddress, uint256 amount) external {
    ...
    Farm farmInstance = Farm(farmAddress);
    farmInstance.withdraw(amount, msg.sender);
  ...
  function emergencyWithdraw(address farmAddress) external {
    UserFarm[] storage _userFarms = userFarms[msg.sender];
    UserFarm memory farm;
    bool found = false;
    ...
    Farm farmInstance = Farm(farmAddress);
    farmInstance.emergencyWithdraw(msg.sender);
  ...
  function pending(address farm) external view returns (uint256) {
    Farm farmInstance = Farm(farm);
    uint256 pendingRewards = farmInstance.pending(msg.sender);
    return pendingRewards;
  }

  function userFarmsLength(address user) external view returns (uint256) {
    return userFarms[user].length;
  }
}
...
contract FarmsStorage is Initializable, OwnableUpgradeable {
  /**
   * Storage
   */
