...
contract ConnectFour {
    ...
    function initializeGame() external payable returns (uint256) {
        ...
        require(msg.value >= minBetAmount && msg.value <= maxBetAmount, "ConnectFour: msg.value must be above min and below max");
    ...
    function startGame(uint256 _gameId) external payable {
        ...
        // requires
        require(game.status == Status.Initialized, "ConnectFour: game doesn't exist or is already started");
        require(msg.sender != game.player1, "ConnectFour: can't start the same game you initialized");
        require(game.betAmount == msg.value, "ConnectFour: Bet amout must be equal Player 1's bet");

        // logic
        games[_gameId].status = Status.Started; 
        games[_gameId].player2 = msg.sender; 

        // emit events or return
    ...
    function playMove(uint256 _gameId, uint256 _col) external {
        ...
        //requre game started
        require(game.status == Status.Started, "ConnectFour: Game has not yet started");
        //require msg.sender to be player1 or player2
        require(msg.sender == game.player1 || msg.sender == game.player2, "ConnectFour: Must be a player");
        //require it to be msg.sender's turn
        if (game.isPlayer1Turn) {
            require(msg.sender == game.player1, "ConnectFour: It is Player1's turn");
        } else {
            require(msg.sender == game.player2, "ConnectFour: It is Player2's turn");
        }
        // require move to be in bounds
        require(_col <= COL_MAX_INDEX, "ConnectFour: This move is out of bounds");

       Disc disc = game.isPlayer1Turn ? Disc.Player1 : Disc.Player2;
        for (uint256 row = 0; row <= 5; row++) {
            uint256 index = boardIndex(_col, row);

            if (game.board[index] == Disc.Empty) {
                games[_gameId].board[index] = disc;
                emit BoardUpdated(_gameId, disc, index);
    ...
    function claimReward(
        ...
        // intentional vulnerability: anyone can call this function and claim reward if player2 wins
        // need to leave for future testing/learning/lessons
        // logical vulnerability: if player1 has 4 in a row and on next turn, if 
        //player2 can play a disc to get four in a row, then first to claim will get reward


        require(games[_gameId].status == Status.Started,
        "ConnectFour: Game has not started, doesn't exist, or is finished");
        ...
        for (uint i = 0; i < 3; i++) {
            ...
            require(game.board[index] == callerDisc, 
            "ConnectFour: To win, there must be 4 in a row of caller's discs");
            ...
            uint rewardAmount = 2 * game.betAmount;
    ...
    function boardIndex(uint256 _col, uint256 _row)
        ...
        returns (uint256)
    {
        require(_col >= 0 && _col <= 6, "ConnectFour: Column is out of bounds");
        require(_col >= 0 && _col <= 5, "ConnectFour: Row is out of bounds");
        return(_row * 7 + _col);
    }
}
