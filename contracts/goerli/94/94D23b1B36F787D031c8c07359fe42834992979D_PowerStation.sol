...
contract PowerStation is Initializable, UUPSUpgradeable, OwnableUpgradeable,IPowerStation{
    ...
    struct Pool {
        ...
        // uint256 redeemedReward;  //The reward that has been redeemed
  ...
  Pool[] public pools; // Staking pools
    ...
    function stakerJoinPool(uint256 _pid, uint256 shares, string memory payby) public override {
        ...
        require(pool.allocatedShares + shares<=pool.poolInfoVars[6], "over sell");
        ...
        if (keccak256(abi.encodePacked(payby)) == keccak256(abi.encodePacked("USDT"))){
            ...
            require(usdt.transferFrom(msg.sender,devAddr, (pool.poolInfoVars[4] + pool.poolInfoVars[2]) * shares),"transfer failed");
            emit PayPower(msg.sender, pool.poolInfoVars[4] * shares);
            emit PayIDC(msg.sender, pool.poolInfoVars[2] * shares);
        }
        else if (keccak256(abi.encodePacked(payby)) == keccak256(abi.encodePacked("RGP"))){

            //RGP
            require(rgp.transferFrom(msg.sender,devAddr, pool.poolInfoVars[3] * shares),"transfer failed");
            emit PayPower(msg.sender, pool.poolInfoVars[3] * shares);

            //IDC
            require(usdt.transferFrom(msg.sender, devAddr, pool.poolInfoVars[2]*shares),"transfer failed");
            emit PayIDC(msg.sender, pool.poolInfoVars[2] * shares);
        }
        else {
            revert("not legit");
        ...
        require(renFil.transferFrom(msg.sender,devAddr, pool.poolInfoVars[5] * shares),"transfer failed");
        emit PayPledge(msg.sender, pool.poolInfoVars[5] * shares);
        ...
        require(renFil.transferFrom(msg.sender, devAddr, 
                    ((pool.endingTime-pool.startTime)/30 days * ( pool.poolInfoVars[0]+pool.poolInfoVars[1]) * shares)),"transfer failed");          // 15/10 is ultility/T/month

        emit PayDataGas(msg.sender, (pool.endingTime-pool.startTime)/30 days
         * ( pool.poolInfoVars[0]+pool.poolInfoVars[1]) * shares);
        ...
        //max shares
        if(pool.allocatedShares == pool.poolInfoVars[6]){
...
//  }
    ...
    function withdraw(uint256 _pid) public override{
        ...
        renFil.approve(address(this), remainingRewardsPerShare* staker.shares);
        renFil.transfer(msg.sender, remainingRewardsPerShare* staker.shares);
    ...
    function addPool(uint256 periods,  uint256 pricePerShare, uint256 pledgePerShare, address poolMiner, 
                    ...
                    uint256 dataFee, uint256 gasFee, uint256 rgptoShare, uint256 idcFee)
    ...
    function topUpPool(uint256 _pid, uint256 _allocPoint) public override{
        ...
        uint256 rewardPerShare = _allocPoint* 80 /100 /pool.poolInfoVars[6];    // allocPoint/maxShares
        uint256 allocatedRewards = rewardPerShare * pool.allocatedShares;
        ...
        pool.remainBalance += _allocPoint*80 /100 - allocatedRewards;
    ...
    function pendingRewards(address user, uint256 _pid) public override view returns (uint256){
         _poolExist(_pid);
        _stakerExist(_pid, user);
        Staker storage staker = poolStaker[_pid][user];
        Pool storage pool = pools[_pid];
    ...
    function pendingRewardsWithInfo(address user, uint256 _pid) 
        public view returns (uint256, uint256, string memory, address, bool,uint256 [6] memory,uint256[7] memory){
         _poolExist(_pid);
        _stakerExist(_pid, user);
        Staker storage staker = poolStaker[_pid][user];
        Pool storage pool = pools[_pid];
        uint256[7] memory poolVars = getPoolVars(_pid);
        (string memory poolName,address miner, bool isOpen,uint256 [6] memory poolInfo)= getPoolInfo( _pid);
        uint256 remainingRewardsPerShare = pool.totalDailyRewardPershare -staker.redeemedRewards / staker.shares;
        return(remainingRewardsPerShare * staker.shares, staker.shares, poolName, miner, isOpen, poolInfo, poolVars);
    }
    ...
    function getPoolVars(uint256 _pid) public view override returns( uint256[7] memory){
     ...
     function getPoolInfo(uint256 _pid) private view returns(string memory, address, bool,uint256 [6] memory){
        _poolExist(_pid);
        Pool storage pool = pools[_pid];
        return(pool.poolInfo,pool.poolMiner,pool.isOpen,[pool.totalRewards,pool.endingTime,pool.remainBalance, 
        pool.startTime, pool.allocatedShares, pool.totalDailyRewardPershare]);
...
interface IPowerStation {
    ...
    function addPool(uint256 periods,  uint256 pricePerShare, uint256 pledgePerShare, address poolMiner, 
                    string memory poolInfo,uint256 maxShares, uint256 startTime, 
                    uint256 dataFee, uint256 gasFee, uint256 rgptoShare, uint256 idcFee) external;
