...
contract testWiz is ERC721AQueryable, ERC721ABurnable, EIP712, Ownable {
    ...
    uint public maxNormalSupply = 15;
    ...
    constructor() ERC721A("testWiz", "HD") EIP712("testWiz", "1.0.0") {
    }
    ...
    function _hash(address account) private view returns (bytes32) {
        return _hashTypedDataV4(
            keccak256(
                abi.encode(
                    keccak256("testWiz(address account)"),
                    account
                )
            )
        );
