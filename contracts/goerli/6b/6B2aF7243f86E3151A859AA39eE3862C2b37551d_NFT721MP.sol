...
interface IMemberPoints {
    function adminModifyMemberPoints(address _address ,int256 amount) external;
    function getMemberPoints(address _address)  external returns (uint256);
}
contract MemberPointsControl {
    address internal IMemberPointsAddress;
    function setIMemberPointsAddress(address _address) public virtual  {
        IMemberPointsAddress = _address;
    }
    function adminModifyMemberPoints(address _address ,int256 amount) internal virtual{
        IMemberPoints IMemberPointsContract = IMemberPoints(IMemberPointsAddress);
        IMemberPointsContract.adminModifyMemberPoints(_address, amount);
    }
    function getMemberPoints(address _address) internal virtual returns (uint256){
        IMemberPoints IMemberPointsContract = IMemberPoints(IMemberPointsAddress);
        return IMemberPointsContract.getMemberPoints(_address);
    }
}
...
import {MemberPointsControl} from "./Member/MemberPointsControl.sol";

contract NFT721MP is ERC721A, DefaultOperatorFilterer, MemberPointsControl,Ownable {
    ...
    //****** mp ******
    uint256 internal baseMemberPoints;

    constructor() ERC721A("FOUNDO", "FOUNDO") {}
    ...
    //****** set mp ******
    function setBaseMemberPoints(uint256 amount) public onlyOwner{
        baseMemberPoints = amount;
    }
    function setIMemberPointsAddress (address _address) public override onlyOwner {
        super.setIMemberPointsAddress(_address);
    } 
    ...
    function whiteListMint(
        ...
        require(hashWhiteListMint(quantity, blockHeight, nonce, "foundo_white_list_mint") == hash, "Invalid hash!");
        ...
        // add member points
        adminModifyMemberPoints(msg.sender,int256(quantity.mul(baseMemberPoints)));
    ...
    //******Tool******
    function hashWhiteListMint(uint256 quantity, uint256 blockHeight, string memory nonce, string memory code)
    private
    view
    returns (bytes32)
    {
