...
// File: erc721a-upgradeable/contracts/extensions/ERC721AQueryableUpgradeable.sol
...
// File: contracts/HeyMintLaunchpadERC721ABase.sol
...
1. Test that the directApproveMsgSender function still works with the same storage slots
2. Add a way to pass all the blocklisted contracts to blocklist and a way to make that more gas efficient
3. Test if child contracts are upgradeable if you add a function to change the implementation address stored
4. If you get diamond storage figured out, consider removing burn to mint or something to free up space and then adding all the variables upfront in the struct arguments so everything can be configured from the get go in one deploy tx
*/
...
// Prevents a stack too deep compilation error since structs can only store 14 variables
// Addresses are ordered so that all free claim variables consume a separate storage slot from burn
// So if only burn or free claim is enabled on a contract, users save 20k gas in deployment costs
struct AdditionalConfigSettings {
 ...
 */
contract HeyMintLaunchpadERC721ABase is
    ERC721AUpgradeable,
    ...
    IERC2981Upgradeable
{
    ...
    ConfigSettings public config;
    AdditionalConfigSettings public additionalConfig;
    ...
    // ============ EXCHANGE BLOCKLIST VARIABLES ============
    // Address of the smart contract used to check if an operator address is from a blocklisted exchange
    // NOTE - This is the Goerli address, change to 0xca71B13C9465c2117f3DBBA294c2f4134c629C72 for mainnet
    address public constant blocklistContractAddress = 0x7dB717725467dBD6405fa95298e40Cc14766F058;
    ...
    uint256[] public payoutBasisPoints;
    ...
    uint256 public publicSalePriceTest;

    // ============ PRESALE VARIABLES ============
    // Default HeyMint Launchpad presale signer address used to validate authorized presale mint addresses
    address private constant defaultPresaleSignerAddress = 0xA85755fD92F91A1dD11Eba8A613121C6D2654BbE;
    ...
    // ============ BURN CLAIM VARIABLES ===========
    // Address where burnt tokens are sent. Also used for HeyMint deposit token burning.
    ...
    bool public soulbindAdminTransfersPermanentlyDisabled;
     ...
     * @param _config The configuration settings for the contract
     */
    function initialize(
        address _owner,
        string memory _name,
        string memory _symbol,
        ConfigSettings memory _config,
        AdditionalConfigSettings memory _additionalConfig
        ...
        publicSalePriceTest = _config.publicPrice * 10**16;

        if (config.fundingDuration != 0) {
    ...
    modifier originalUser() {
        require(tx.origin == msg.sender, "BLOCKED_OP");
        _;
    }
    ...
    function _baseURI() internal view virtual override returns (string memory) {
        return bytes(config.uriBase).length == 0
            ? string(abi.encodePacked("https://api.heymint.xyz/tokens/", _toString(config.projectId), "/"))
            : config.uriBase;
    }
    ...
    function royaltyInfo(uint256, uint256 _salePrice)
        ...
        returns (address receiver, uint256 royaltyAmount)
    {
        address payoutAddress = royaltyPayoutAddress != address(0x0) ? royaltyPayoutAddress : owner();
        if (payoutAddress == address(0x0)) {
            return (payoutAddress, 0);
        }
        return (payoutAddress, (_salePrice * config.royaltyBps) / 10000);
    }
     ...
     */
    function setRoyaltyPayoutAddress(address _royaltyPayoutAddress) external onlyOwner {
     ...
     * @param _exchanges An array of integers mapped to exchanges to set or unset as blocklisted
     * @param _blocklisted An array of booleans - true to blocklist the respective exchange, false to allow
     */
     ...
     */
    function isBlocklistedOperator(address _operator) internal view returns (bool) {
        uint256 operatorExchangeId = IExchangeOperatorAddressList(blocklistContractAddress).operatorAddressToExchange(_operator);
        return isExchangeBlocklisted[operatorExchangeId];
    }
    ...
    {
        require(!isBlocklistedOperator(operator), "BLOCKED_OP");
    ...
    function approve(address to, uint256 tokenId)
        ...
        override(ERC721AUpgradeable, IERC721AUpgradeable)
    {
        require(!isBlocklistedOperator(to), "BLOCKED_OP");
    ...
    function updatePayoutAddressesAndBasisPoints(
        ...
        require(!payoutAddressesFrozen, "NOT_ACTIVE");
        require(
        ...
        require(totalBasisPoints == 10000, "INVALID_ARGS");
    ...
    function withdraw() external nonReentrant onlyOwner {
        if (config.fundingDuration != 0) {
            require(fundingTargetReached, "NOT_ACTIVE");
        }
        ...
        onlyOwner
    {
        ...
        require(totalSupply() + totalMint <= config.maxSupply, "TOO_LARGE");
     ...
     */
    function setPublicPrice(uint16 _publicPrice) external onlyOwner {
     ...
     */
    function setPublicMintsAllowedPerAddress(uint8 _mintsAllowed)
        external
        onlyOwner
    {
        config.publicMintsAllowedPerAddress = _mintsAllowed;
    }
     ...
     */
    function mint(uint256 _numTokens)
        external
        payable
        nonReentrant
        originalUser
    {
        require(publicSaleActive, "NOT_ACTIVE");
        require(
            config.publicMintsAllowedPerAddress == 0 ||
            _numberMinted(msg.sender) + _numTokens <=
            config.publicMintsAllowedPerAddress,
            "TOO_LARGE"
        );
        require(totalSupply() + _numTokens <= config.maxSupply, "TOO_LARGE");
        require(msg.value == (config.publicPrice * 10**16) * _numTokens);

        if (config.fundingDuration != 0) {
            uint256 firstTokenIdToMint = _nextTokenId();
            for (uint256 i = 0; i < _numTokens; i++) {
              pricePaid[firstTokenIdToMint + i] = config.publicPrice * 10**16;
            }
        }
     ...
     * @param _saleActiveState The new presale active state
     */
    function setPresaleState(bool _saleActiveState) external onlyOwner {
        presaleActive = _saleActiveState;
    }
    ...
    function setPresalePrice(uint16 _presalePrice) external onlyOwner {
        config.presalePrice = _presalePrice;
    }
     ...
     */
    function setPresaleMintsAllowedPerAddress(uint8 _mintsAllowed)
        external
        onlyOwner
    {
        config.presaleMintsAllowedPerAddress = _mintsAllowed;
    }
     ...
     */
    function verifySignerAddress(bytes32 _messageHash, bytes calldata _signature)
        private
        view
        returns (bool)
    {
        address signerAddress = presaleSignerAddress != address(0)
            ? presaleSignerAddress
            : defaultPresaleSignerAddress;
        return
            signerAddress ==
    ...
    function presaleMint(
        ...
        require(
            config.presaleMintsAllowedPerAddress == 0 ||
            _numberMinted(msg.sender) + _numTokens <=
                config.presaleMintsAllowedPerAddress,
            "TOO_LARGE"
        );
        require(
        ...
        require(
            config.presaleMaxSupply == 0 ||
            totalSupply() + _numTokens <= config.presaleMaxSupply,
            "TOO_LARGE"
        );
        require(
            totalSupply() + _numTokens <= config.maxSupply,
            "TOO_LARGE"
        );
        require(msg.value == (config.presalePrice * 10**16) * _numTokens);
        require(
        ...
        if (config.fundingDuration != 0) {
            uint256 firstTokenIdToMint = _nextTokenId();
            for (uint256 i = 0; i < _numTokens; i++) {
              pricePaid[firstTokenIdToMint + i] = config.presalePrice * 10**16;
            }
        }

        _safeMint(msg.sender, _numTokens);

        if (totalSupply() >= config.presaleMaxSupply) {
            presaleActive = false;
        }
    }
    ...
    /**
     * @notice To be called by anyone once the fundingDuration has passed to determine if the funding target was reached
    ...
    function determineFundingSuccess() external {
        require(config.fundingDuration > 0, "NOT_ACTIVE");
        require(address(this).balance > config.fundingTarget * 10**16, "NOT_ACTIVE");
        require(!fundingSuccessDetermined, "NOT_ACTIVE");
        fundingTargetReached = true;
        fundingSuccessDetermined = true;
    }
    ...
    function burnToRefund(uint256[] calldata _tokenIds) external nonReentrant {
        ...
        require(config.fundingDuration > 0, "NOT_ACTIVE");
        require(block.timestamp >= (contractDeployTime + ((config.fundingDuration + 1) * 86400)), "NOT_ACTIVE");
        require(!fundingTargetReached, "NOT_ACTIVE");
        require(address(this).balance < config.fundingTarget * 10**16, "NOT_ACTIVE");
        ...
        for (uint256 i = 0; i < _tokenIds.length; i++) {
          require(ownerOf(_tokenIds[i]) == msg.sender, "NOT_OWNED");
          require(pricePaid[_tokenIds[i]] > 0, "INVALID_ARGS");
          safeTransferFrom(msg.sender, 0x000000000000000000000000000000000000dEaD, _tokenIds[i]);
          totalRefund += pricePaid[_tokenIds[i]];
        }
    ...
    function setSoulboundAdminAddress(address _adminAddress)
        ...
        onlyOwner
    {
        require(!soulbindAdminTransfersPermanentlyDisabled);
    ...
    function disableSoulbindAdminTransfersPermanently() external onlyOwner {
        ...
        soulbindAdminTransfersPermanentlyDisabled = true;
    }
    ...
    function setSoulbindingState(bool _soulbindingActive) external onlyOwner {
        config.soulbindingActive = _soulbindingActive;
    }
    ...
    function soulboundAdminTransfer(
        ...
        address adminAddress = soulboundAdminAddress == address(0)
            ? owner()
            : soulboundAdminAddress;
        require(
    ...
    function stakeTokens(uint256[] calldata _tokenIds) external {
        require(stakingActive, "NOT_ACTIVE");
        for (uint256 i = 0; i < _tokenIds.length; i++) {
    ...
    function adminUnstake(uint256 _tokenId) external onlyOwner {
        require(currentTimeStaked[_tokenId] != 0);
        totalTimeStaked[_tokenId] +=
            block.timestamp -
            currentTimeStaked[_tokenId];
        currentTimeStaked[_tokenId] = 0;
     ...
     */
    function totalTokenStakeTime(uint256 _tokenId)
        external
        view
        returns (uint256)
    {
        uint256 currentStakeStartTime = currentTimeStaked[_tokenId];
        if (currentStakeStartTime != 0) {
            return
                (block.timestamp - currentStakeStartTime) +
                totalTimeStaked[_tokenId];
        }
        return totalTimeStaked[_tokenId];
    }
     ...
     * @param _tokenId The id of the token to check
     */
    function currentTokenStakeTime(uint256 _tokenId)
        external
        view
        returns (uint256)
    {
     ...
     */
    function loan(uint256 _tokenId, address _receiver)
        external
        nonReentrant
    {
        require(loaningActive, "NOT_ACTIVE");
        require(ownerOf(_tokenId) == msg.sender, "NOT_OWNED");
        require(
            tokenOwnersOnLoan[_tokenId] == address(0),
            "NOT_OWNED"
        );
        // Add it to the mapping of originally loaned tokens
        tokenOwnersOnLoan[_tokenId] = msg.sender;
    ...
    function retrieveLoan(uint256 _tokenId) external nonReentrant {
        ...
        delete tokenOwnersOnLoan[_tokenId];
    ...
    function adminRetrieveLoan(uint256 _tokenId) external onlyOwner {
        address borrowerAddress = ownerOf(_tokenId);
        address loanerAddress = tokenOwnersOnLoan[_tokenId];
        require(loanerAddress != address(0), "NOT_ACTIVE");
        // Remove it from the array of loaned out tokens
        delete tokenOwnersOnLoan[_tokenId];
        // Subtract from the owner's loan balance
    ...
    function setBurnClaimState(bool _burnClaimActive) external onlyOwner {
        if (_burnClaimActive) {
            require(
                additionalConfig.burnToMintContractAddress != address(0),
                "INVALID_ARGS"
            );
            require(
                additionalConfig.mintsPerBurn != 0,
                "INVALID_ARGS"
            );
            require(
                additionalConfig.burnToMintContractType != 0,
                "INVALID_ARGS"
            );
            if (additionalConfig.burnToMintContractType == 2) {
                require(additionalConfig.burnToMintTokenId != 0, "INVALID_ARGS");
            }
        }
     ...
     * @param _burnToMintContractAddress The new contract address
     */
    function setBurnToMintContractAddress(address _burnToMintContractAddress) external onlyOwner {
        additionalConfig.burnToMintContractAddress = _burnToMintContractAddress;
    }

    /**
     * @notice Set the contract type of the NFT to be burned in order to mint
     * @param _burnToMintContractType The new contract type - 1 for ERC-721, 2 for ERC-1155
     */
    function setBurnToMintContractType(uint8 _burnToMintContractType) external onlyOwner {
        additionalConfig.burnToMintContractType = _burnToMintContractType;
    }

    /**
     * @notice Update the number of free mints claimable per token burned
     * @param _mintsPerBurn The new number of free mints per token burned
     */
    function updateMintsPerBurn(uint8 _mintsPerBurn) external onlyOwner {
        additionalConfig.mintsPerBurn = _mintsPerBurn;
    }

    /**
     * @notice Burn ERC-721 tokens from the burnToMintContractAddress in order to mint a token
        ...
        originalUser
    {
        ...
        require(
            additionalConfig.burnToMintContractType == 1,
            "NOT_ACTIVE");
        require(
            totalSupply() + (_tokenIDs.length * additionalConfig.mintsPerBurn) <= config.maxSupply,
            "TOO_LARGE"
        );
        IERC721 ExternalERC721BurnContract = IERC721(additionalConfig.burnToMintContractAddress);
        for (uint256 i = 0; i < _tokenIDs.length; i++) {
            require(
        ...
        _safeMint(msg.sender, _tokenIDs.length * additionalConfig.mintsPerBurn);
    }

    /**
     * @notice Burn ERC-1155 tokens from the burnToMintContractAddress in order to mint a token
     * @dev This contract must be approved by the caller to transfer the tokens being burned
     * @param _amount The amount of tokens to burn
     */
    function burnERC1155ToMint(uint256 _amount) external nonReentrant originalUser {
        require(burnClaimActive, "NOT_ACTIVE");
        require(_amount > 0);
        require(additionalConfig.burnToMintContractType == 2, "NOT_ACTIVE");
        require(totalSupply() + (_amount * additionalConfig.mintsPerBurn) <= config.maxSupply, "TOO_LARGE");
        IERC1155 ExternalERC1155BurnContract = IERC1155(additionalConfig.burnToMintContractAddress);
        require(ExternalERC1155BurnContract.balanceOf(msg.sender, additionalConfig.burnToMintTokenId) >= _amount, "NOT_OWNED");

        ExternalERC1155BurnContract.safeTransferFrom(msg.sender, burnAddress, additionalConfig.burnToMintTokenId, _amount, '');
        _safeMint(msg.sender, _amount * additionalConfig.mintsPerBurn);
    }
    ...
    function setFreeClaimState(bool _freeClaimActive) external onlyOwner {
        if (_freeClaimActive) {
            require(
                additionalConfig.freeClaimContractAddress != address(0),
                "INVALID_ARGS"
            );
            require(
                additionalConfig.mintsPerFreeClaim != 0,
                "INVALID_ARGS"
            );
        }
     ...
     */
    function setFreeClaimContractAddress(address _freeClaimContractAddress) external onlyOwner {
        additionalConfig.freeClaimContractAddress = _freeClaimContractAddress;
    }
     ...
     */
    function updateMintsPerFreeClaim(uint8 _mintsPerFreeClaim) external onlyOwner {
        additionalConfig.mintsPerFreeClaim = _mintsPerFreeClaim;
    }
    ...
    function freeClaim(uint256[] calldata _tokenIDs)
        ...
        originalUser
    {
        require(freeClaimActive, "NOT_ACTIVE");
        require(
            totalSupply() + (_tokenIDs.length * additionalConfig.mintsPerFreeClaim) <= config.maxSupply,
            "TOO_LARGE"
        );
        IERC721 ExternalERC721FreeClaimContract = IERC721(
            additionalConfig.freeClaimContractAddress
        );
        for (uint256 i = 0; i < _tokenIDs.length; i++) {
            require(
            ...
            require(!freeClaimUsed[_tokenIDs[i]], "INVALID_ARGS");
            freeClaimUsed[_tokenIDs[i]] = true;
        }
        _safeMint(msg.sender, _tokenIDs.length * additionalConfig.mintsPerFreeClaim);
    }
    ...
    function _beforeTokenTransfers(
        ...
        require(!isBlocklistedOperator(msg.sender), "BLOCKED_OP");
        require(
            !stakingActive ||
        ...
        if (config.soulbindingActive && !soulboundAdminTransferInProgress) {
            require(from == address(0), "NOT_ACTIVE");
        }
...
contract HeyMintLaunchpadERC721ADelegated {
     ...
     * @param _config The configuration settings for the deposit NFT contract
     */
    constructor(
        ...
        (bool success, ) = _nftImplementation.delegatecall(
            abi.encodeWithSignature(
                "initialize(address,string,string,(bool,uint8,uint8,uint8,uint16,uint16,uint16,uint16,uint16,uint24,uint24,string),(uint8,address,address,uint8,uint8,uint16))",
                msg.sender,
                _name,
                _symbol,
                _config,
...
contract HeyMintDepositChild is HeyMintLaunchpadERC721ADelegated {
    constructor(
