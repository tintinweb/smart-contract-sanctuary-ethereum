...
contract Contract is ContractStorage, UUPSUpgradeable, OwnableUpgradeable, ERC721HolderUpgradeable, ERC1155HolderUpgradeable {
  ...
  function claimTag(
    ...
    require (tag.tagType == TagType.HotPotato || tag.tagType == TagType.Refillable1Of1 || tag.claimsMade[recipient] < tag.perUser, 'not HotP or ReF and pU drained');
    ...
    if (tag.tagType == TagType.WalletRestrictedFungible) {
      ...
      tag.claimsMade[recipient] += tag.fungiblePerClaim;
    } else {
      tag.numClaimed += 1;
      tag.claimsMade[recipient] += 1;
    }
...
interface IERC721CopyableUpgradeable is IERC721MetadataUpgradeable {
