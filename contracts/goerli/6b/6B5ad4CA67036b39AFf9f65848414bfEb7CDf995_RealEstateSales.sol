...
contract RealEstateSales is Ownable {
    ...
    // seaside nft
    IRealEstateNFT public immutable realEstateNFT;

    // coin price
    uint256 public constant coinPriceUSD = 1 * priceDecimal;

    // real estate sale enable flags
    bool public realEsateSalesEnabled;
    ...
    uint256 public constant rewardPercentage = 30;

    constructor(
        address seasideCoin_, 
        address realEstateNFT_, 
        ...
        realEstateNFT = IRealEstateNFT(realEstateNFT_);
    }

    /** 
     * @dev setting realEsateSaleEnabled variable
     * @param enabled boolean True if enables, False otherwise
    */

    function setRealEstateSalesEnabled(bool enabled) external onlyOwner {
        realEsateSalesEnabled = enabled;
    }
    ...
    */
    modifier whenAllowed() {
        require(isAllowed[msg.sender], "RealEstateSales: address is not allowed to call this function");
        _;
    }
    ...
    */
    ...
    /**
     * @dev Buy the coins in private Coins supply with USDC/USDT/BUSD coin
    ...
    */
    ...
    function buy(uint256 amount, uint256 splitNFTCount, Coin coinType) external whenAllowed {
        require(realEsateSalesEnabled, "RealEstateSales: sale is not enabled");
        require(splitNFTCount <= getMaxSplitCount(amount), "RealEstateSales: split count exceed it's limit");
        require(amount >= priceLimit, "RealEstateSales: amount should be greater than price limit ");
        ...
        seasideCoin.transfer(address(realEstateNFT), seasideCoins);
        uint256 total = amount + (amount * rewardPercentage / 100);
    ...
    /**
     * @dev airdroping the coins from private Coins supply with USDC/USDT/BUSD coin
     * @param amount USDC coins to be transferred
     * @param splitNFTCount nft count
     * @param coinType type of coin: USDC, USDT, BUSD
    */
...
interface IRealEstateNFT {
     ...
     * @param currentReservedCoins the coins which was reserved on current state
