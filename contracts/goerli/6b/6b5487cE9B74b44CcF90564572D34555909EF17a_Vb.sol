    ...
    Ownable
{
    ...
    function mint(uint256 tokenId, uint256 qty) external payable {
        if (block.timestamp < configurations[tokenId].startTime || configurations[tokenId].startTime == 0)
            revert MintNotStarted();
        if (block.timestamp > configurations[tokenId].endTime) revert MintEnded();
        doMint(tokenId, configurations[tokenId].tokenPrice, qty, msg.value);        
    }
    ...
    function whitelistMint(bytes32[] calldata proof,uint256 tokenId, uint256 qty)  external payable  {
        if (block.timestamp < configurations[tokenId].wlStartTime || configurations[tokenId].wlStartTime == 0)
            revert MintNotStarted();
        if (block.timestamp > configurations[tokenId].wlEndTime) revert MintEnded();
        // verify whitelisted
        require(merkleVerify(proof,tokenId), 'Auth failed');
        doMint(tokenId, configurations[tokenId].wlPrice, qty, msg.value);
    }

    function merkleVerify(bytes32[] calldata proof,uint256 tokenId) public view returns(bool result){
        bytes32 leaf = keccak256(abi.encodePacked(_msgSender()));
        return MerkleProof.verify(proof,configurations[tokenId].root,leaf);
    } 
    ...
    function doMint(
        ...
        if (val != price * qty) revert NotEnoughEth();
        if (mintedPerWallet[tokenId][_msgSender()] + qty > config.maxAllowance)
            revert ExceededAllowance();
        _mint(_msgSender(), tokenId, qty, "");
        mintedPerWallet[tokenId][_msgSender()] += qty;    
    ...
    function setupMint(
        ...
        config.uri = _uri;
        config.root = _root;         
        config.wlPrice = wlPrice; 
        config.wlStartTime = wlStartTime;         
        config.wlEndTime = wlEndTime;         

    }

    /// @notice Sets root for a given token
    function setRoot(bytes32 _root,uint256 tokenId)  external  onlyOwner{
        configurations[tokenId].root = _root;         
    }

    /// @notice Sets the start time for a give token
    /// @dev Can't be called after mint ended for that token
    function setWlStartTime(uint256 tokenId, uint32 t) external onlyOwner {
        if (
            configurations[tokenId].endTime != 0 &&
            block.timestamp > configurations[tokenId].endTime
        ) revert MintEnded();
        configurations[tokenId].wlStartTime = t;
    }  

    /// @notice Sets the end time for a give token
    /// @dev Can't be called after mint ended for that token
    function setWlEndTime(uint256 tokenId, uint32 t) external onlyOwner {
        if (
            configurations[tokenId].endTime != 0 &&
            block.timestamp > configurations[tokenId].endTime
        ) revert MintEnded();
        configurations[tokenId].wlEndTime = t;
    }      

    /// @notice Sets the mint price for a given token
    function setWlTokenPrice(uint256 tokenId, uint256 price) external onlyOwner {
        configurations[tokenId].wlPrice = price;
    }
