...
contract OXXRDrop is Initializable,
    ...
    * - The function caller must send an amount of Ether greater than or equal to the price of the NFTs being minted
    ...
    * - If the quantity of NFTs minted is less than the quantity available for sale at the specified level, the excess Ether is refunded to the function caller
    * - If the quantity of NFTs minted is greater than the quantity available for sale at the specified level, the entire amount of Ether is refunded to the function caller
    */
    function mint(uint256 drop, uint256 level, uint256 quantity) public payable {
        ...
        bool canMint = isMinter || (block.timestamp >= drops[drop].waitListStartTime && isWaitListed) || (block.timestamp >= drops[drop].waitListEndTime && (isAllowListed || isWaitListed)) || (block.timestamp > drops[drop].allowListEndTime);
