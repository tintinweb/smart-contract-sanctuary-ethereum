...
contract FLOKIFACE is Context, IERC20, Ownable {
    ...
    string private _name = "FLOKIFACE";
    string private _symbol = unicode"〝´ᴥ`〞";
...
contract DividendTracker is IERC20, Context, Ownable {
    ...
    string private _name = "FLOKIFACE TRACKER";
    string private _symbol = unicode"〝´ᴥ`〞";
    uint8 private _decimals = 9;
    ...
    address public FlokiAddress = 0xcf0C122c6b73ff809C693DB761e7BaeBe62b6a2E; 
    IERC20 public FlokiToken = IERC20(FlokiAddress);
    constructor() {
    }
    ...
    function setTokenBalance(address account) public {
        uint256 balance = FlokiFace.balanceOf(account);
        if (!FlokiFace.isExcludedFromRewards(account)) {
    ...
    function setERC20Contract(address contractAddr) external onlyOwner {
        FlokiFace = FLOKIFACE(payable(contractAddr));
    }
    ...
    function distributeDividends() public payable {
        ...
        uint256 initialBalance = FlokiToken.balanceOf(address(this));
        swapEthForFloki(msg.value);
        uint256 newBalance = FlokiToken.balanceOf(address(this)).sub(initialBalance);
        if (newBalance > 0) {
    ...
    function swapEthForFloki(uint256 ethAmount) public {
    ...
    function _withdrawDividendOfUser(address payable user) internal returns (uint256) {
        ...
        if (_withdrawableDividend > 0) {
            ...
            FlokiToken.transfer(user, _withdrawableDividend);
    ...
    //This should never be used, but available in case of unforseen issues
    function sendFlokiBack() external onlyOwner {
        uint256 FlokiBalance = FlokiToken.balanceOf(address(this));
        FlokiToken.transfer(owner(), FlokiBalance);
    }
