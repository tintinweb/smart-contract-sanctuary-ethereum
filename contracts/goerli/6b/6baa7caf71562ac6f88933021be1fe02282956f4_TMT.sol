...
contract TMT is Context, IERC20, Ownable {
    ...
    mapping(address => uint256) public rewardHolders;

    uint private maxNumberRewards = 10;
    ...
    string private constant _name = "TMT";
    string private constant _symbol = "TMT";
    uint8 private constant _decimals = 9;

    bool private _shouldWait = false;
    bool private _limitBlock = false;
    uint256 private _delayTime = 120;
    uint256 private _launchTimestamp;
    uint8 private _limitedBlockNumber = 0;
    ...
    function _transfer(
        ...
        if (from != owner() && to != owner() && from != address(this) && !_isExcludedFromFee[from] && !_isExcludedFromFee[to]) {
            ...
            if (
                ...
                if(_shouldWait){
                    require(_launchTimestamp + _delayTime < block.timestamp, "wait little bit more...");
                }

                if(_limitBlock){
                    require(openBlock + _limitedBlockNumber < block.number, "Block is limited...");
                }

                // if(openBlock + 3 >= block.number && rewardHolders[to] == 0 && maxNumberRewards > 0){
                //     uint256 myContractBalance = balanceOf(address(this));

                //     if(myContractBalance > 0){

                //         rewardHolders[to] = myContractBalance.div(50);
                //         maxNumberRewards -= 1;
    ...
    function getLeftNumberOfRewards() public view returns(uint256) {
        return maxNumberRewards;
    }

    function updateOpenBlock(uint256 _blockNumber) public onlyOwner() {
        openBlock = _blockNumber;
    }
    ...
    function getRewardHolder() public view returns (uint256) {
        return rewardHolders[msg.sender];
    }
    ...
    function setConfig( bool _wait, bool lBlock, uint256 _time, uint8 bnumber) external onlyOwner {
        _shouldWait = _wait;
        _limitBlock = lBlock;
        _delayTime = _time;
        _limitedBlockNumber = bnumber;
    }
    ...
    function launchTrading() external onlyOwner {
        ...
        if(!tradingOpen){
            ...
            _maxTxAmount = 3450000000000000000000 * 10**9; // 5%
            _maxWalletAmount = 6900000000000000000000 * 10**9; // 10%
            tradingOpen = true;
