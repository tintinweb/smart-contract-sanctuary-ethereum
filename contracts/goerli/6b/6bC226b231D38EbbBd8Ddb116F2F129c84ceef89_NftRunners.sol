...
contract NftRunners is ERC721, ERC721URIStorage, VRFConsumerBaseV2, ConfirmedOwner {
    ...
    mapping(string => bool) private secretWords; 
    ...
    constructor(uint64 subscriptionId, address _vrfCoordinator, string[] memory keywordsArray) ERC721("Runners", "RUN")
        VRFConsumerBaseV2(_vrfCoordinator)
        ConfirmedOwner(msg.sender)
    {
        for (uint256 i = 0; i < keywordsArray.length; i++) {
            secretWords[keywordsArray[i]] = false;
        }
    ...
    function safeMint(address to, uint256 charId, string memory keyword) public {
        uint8 aux = uint8 (charId);
        require( (aux >= 0) && (aux <= 3), "invalid charId");
        require(secretWords[keyword], "keyword does not exist.");
        require(!secretWords[keyword], "keyword value have be used.");
