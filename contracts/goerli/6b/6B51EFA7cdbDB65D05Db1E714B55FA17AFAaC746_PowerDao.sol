...
contract PowerDao is Context, IERC20, Ownable {
    ...
    uint256 private _tTotal = 50000000000 * (10 ** uint256(DECIMALS));
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal;
    address public marketingWallet;

    string private _name = "PowerFan";
    string private _symbol = "PFAN";
    
    uint256 public _taxFee = 5;
    uint256 private _previousTaxFee = _taxFee;

    uint256 public marketingFeePercent = 35;
    ...
    uint256 public _maxTxAmount = 300200100 * (10 ** uint256(DECIMALS)); //anti whale of 300 million token trades
    uint256 public numTokensSellToAddToLiquidity = 20100 * (10 ** uint256(DECIMALS)); //to provide liquidity. 
    ...
    /*
        set the uniswap pool and router for this token pair. 
    */
    constructor () {
        ...
        _isExcludedFromFee[address(0)] = true; //allow burn
        _isExcluded[address(0)] = true; //no reward to burn address
    ...
    function setTokenName(string memory tName) external onlyOwner() {
        _name = tName;
    }
    ...
    function setMarketingFeePercent(uint256 fee) public onlyOwner {
        require(fee < 69, "Marketing fee cannot be more than 69% nice of liquidity");
    ...
    function setMaxTxAmount(uint256 maxTxAmount) external onlyOwner() {
        require(maxTxAmount > 500, "Max Tx Amount cannot be less than 500");
        _maxTxAmount = maxTxAmount * 10**18;
    }
    
    function setSwapThresholdAmount(uint256 SwapThresholdAmount) external onlyOwner() {
        require(SwapThresholdAmount > 5, "Swap Threshold Amount cannot be less than 5");
    ...
    function _transfer(
        ...
        require(from != address(0), "ERC20: transfer from the zero address"); //no new tokens can be minted
        //require(to != address(0), "ERC20: transfer to the zero address");// allow burn and no rewards to 0 address
        ...
        // also, don't get caught in the circular liquidity event.
        // also, don't swap & liquify if sender is a uniswap pair who is buying.
        ...
        //remove the fee for any interaction with powerfan NFT contracts 
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        // swap tokens for ETH
        swapTokensForEth(half); // <- get eth from the lp for lp providing automatically
