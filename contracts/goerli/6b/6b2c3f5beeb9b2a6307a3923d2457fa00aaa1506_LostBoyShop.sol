...
contract LostBoyShop is IERC721Receiver, IERC1155Receiver {
    ...
    function createProduct(string memory slug, uint256 price, uint256 tokenId, address contractAddress, bool multiple) public pauseable onlyShopManager {
    ...
    function purchaseProduct(string memory slug) public payable pauseable {
		...
		uint256 tokenId;
        if(product.contractAddress != address(0)) 
		{
		...
		else if(product.multiple) 
		{
		    ERC1155 tokenContract = ERC1155(product.contractAddress);
		    uint256 balance = tokenContract.balanceOf(address(this), tokenId);
            require(balance != 0, "contract does not own any of these tokens");
			tokenContract.safeTransferFrom(address(this), msg.sender, tokenId, 1, '');
        }
        lostboyToken.transferFrom(msg.sender, address(this), product.price);
        Products[slug] = Product(
           true,
		   false,
           product.price,
