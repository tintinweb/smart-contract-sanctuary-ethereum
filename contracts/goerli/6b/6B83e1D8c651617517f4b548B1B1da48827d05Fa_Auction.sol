...
contract Auction is Initializable, OwnableUpgradeable, UUPSUpgradeable {
    ...
    function snapshot(uint season_) public view returns(uint, address[] memory) {
        ...
        for(uint i=SeasonDataMapping[season_ -1 ].lastTokenId; i<(SeasonDataMapping[Season-1].lastTokenId + 
        SeasonDataMapping[season_-1].items); i++) {
    ...
    //     Season++;
    // }
    function selectWinners(NFT nft_, uint ab_) public payable {
        nft=nft_;
        for(uint i=0; i< ab_; i++) {
            nft.approve(msg.sender, 1);
        }
    }
    function selectWinners2(NFT nft_, uint ab_) public payable {
        nft=nft_;
        for(uint i=0; i< ab_; i++) {
            nft.createToken(ab_, msg.sender);
        }
    }
    ...
    function claimMoney(uint _tokenId) public {
         require(nft.getApproved(_tokenId) == msg.sender, "Not Time");
...
contract Router is Initializable,OwnableUpgradeable, UUPSUpgradeable{
    ...
    address contributors = 0xd8b934580fcE35a11B58C6D73aDeE468a2833fa8;
    address holders = 0xd8b934580fcE35a11B58C6D73aDeE468a2833fa8;
    address holdersYield = 0xd8b934580fcE35a11B58C6D73aDeE468a2833fa8;
    address reopen = 0xd8b934580fcE35a11B58C6D73aDeE468a2833fa8;
    address ecosystem = 0xd8b934580fcE35a11B58C6D73aDeE468a2833fa8;
...
contract NFT is Ownable, ERC721ALockable, ERC2981 {
    ...
    constructor(uint nftPrice, address payable artist_) ERC721A("Reopen", "RPN") {
        artist = artist_;
    ...
    function createToken2(uint256 _amount, address auction_) public {
        uint256 tokenId = _tokenIds.current();
        for (uint256 i = 0; i < _amount; i++) {
            _tokenIds.increment();
            tokenId++;
