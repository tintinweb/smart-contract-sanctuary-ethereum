...
// File: BadBunnyYachtClub/ERC721R.sol
...
// File: BadBunnyYachtClub/ERC721REnumerable.sol
...
// File: BadBunnyYachtClub/BadBunnyYachtClub.sol
...
contract TTEESSTT is ERC721rEnumerable, Ownable, ReentrancyGuard {

    // Project dependent parameters
    uint256 private constant MAX_SUPPLY = 17;
    uint256 public whiteListCost = 0.000002 ether;
    ...
    function mint(uint _mintAmount) public payable nonReentrant {
        require(!contractPause, "Minting is paused.");
        require(_mintAmount > 0);
        require(_mintAmount < 4, "Can't mint more that 3 at a time.");
    ...
    function whiteListMint(uint _mintAmount) public payable nonReentrant {
        require(!whiteListPause, "Minting is paused.");
        require(_mintAmount > 0);
        require(_mintAmount < 4, "Can't mint more that 3 at a time.");
        uint256 supply = totalSupply();
        require(supply + _mintAmount <= MAX_SUPPLY, "Collecton Sold.");
        ...
        if (msg.sender != owner()) {
            require(msg.value >= whiteListCost * _mintAmount, "Insuficient funds.");
        }
    ...
    function airdrop(address addressToAirdrop) external onlyOwner {
        require(!contractPause, "Minting is paused.");
        uint256 supply = totalSupply();
        require(supply < MAX_SUPPLY, "Collecton Sold.");

        _mintRandom(addressToAirdrop, 1);
    }   

    function addToWhiteList(address[] memory _addressList) external onlyOwner {
        uint256 i;
        for(i = 0; i < _addressList.length; i++) {
            address addressToAdd = _addressList[i];
            whiteList[addressToAdd] = true;
        }
    }

    function removeAddressFromWhiteList(address addressToRemove) external onlyOwner {
    ...
    function wlPause(bool _state) external onlyOwner {
        whiteListPause = _state;
    }
