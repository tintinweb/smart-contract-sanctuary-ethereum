...
import "./interfaces/ICRYPTOS.sol";
...
contract CryptosTreasury is CryptosAccessControlled, ITreasury {
    /* ========== DEPENDENCIES ========== */
    ...
    ICRYPTOS public immutable CRYPTOS;
    ...
    /**
     * @notice allow approved address to deposit an asset for CRYPTOS
    ...
    function deposit(
        ...
        // mint CRYPTOS needed and store amount of rewards for distribution
        send_ = value.sub(_profit);
        CRYPTOS.mint(msg.sender, send_);
    ...
    /**
     * @notice allow approved address to burn CRYPTOS for reserves
     * @param _amount uint256
     * @param _token address
     */
    function withdraw(uint256 _amount, address _token) external override {
        ...
        uint256 value = tokenValue(_token, _amount);
        CRYPTOS.burnFrom(msg.sender, value);
    ...
    /**
     * @notice mint new CRYPTOS using excess reserves
    ...
    function mint(address _recipient, uint256 _amount) external override {
        ...
        CRYPTOS.mint(_recipient, _amount);
        emit Minted(msg.sender, _recipient, _amount);
    }

    /**
     * DEBT: The debt functions allow approved addresses to borrow treasury assets
     * or CRYPTOS from the treasury, using sCRYPTOS as collateral. This might allow an
     * sCRYPTOS holder to provide CRYPTOS liquidity without taking on the opportunity cost
    ...
    function incurDebt(uint256 _amount, address _token) external override {
        ...
        sCRYPTOS.changeDebt(value, msg.sender, true);
        require(sCRYPTOS.debtBalances(msg.sender) <= debtLimit[msg.sender], "Treasury: exceeds limit");
        totalDebt = totalDebt.add(value);

        if (_token == address(CRYPTOS)) {
            CRYPTOS.mint(msg.sender, value);
    ...
    function repayDebtWithReserve(uint256 _amount, address _token) external override {
        ...
        IERC20(_token).safeTransferFrom(msg.sender, address(this), _amount);
        uint256 value = tokenValue(_token, _amount);
        sCRYPTOS.changeDebt(value, msg.sender, false);
    ...
    /**
     * @notice allow approved address to repay borrowed reserves with CRYPTOS
     * @param _amount uint256
     */
    function repayDebtWithOHM(uint256 _amount) external {
        ...
        CRYPTOS.burnFrom(msg.sender, _amount);
        sCRYPTOS.changeDebt(_amount, msg.sender, false);
        totalDebt = totalDebt.sub(_amount);
        ohmDebt = ohmDebt.sub(_amount);
        emit RepayDebt(msg.sender, address(CRYPTOS), _amount, _amount);
    }
    ...
    function enable(
        ...
        if (_status == STATUS.SCRYPTOS) {
            sCRYPTOS = IsCRYPTOS(_address);
        } else {
    ...
    function execute(uint256 _index) external {
        ...
        if (info.managing == STATUS.SCRYPTOS) {
            // 9
            sCRYPTOS = IsCRYPTOS(info.toPermit);
        } else {
    ...
    function excessReserves() public view override returns (uint256) {
        return totalReserves.sub(CRYPTOS.totalSupply().sub(totalDebt));
    }

    /**
     * @notice returns CRYPTOS valuation of asset
     * @param _token address
     * @param _amount uint256
     * @return value_ uint256
     */
    function tokenValue(address _token, uint256 _amount) public view override returns (uint256 value_) {
        value_ = _amount.mul(10**IERC20Metadata(address(CRYPTOS)).decimals()).div(10**IERC20Metadata(_token).decimals());
    ...
    function baseSupply() external view override returns (uint256) {
        return CRYPTOS.totalSupply() - ohmDebt;
    }
}
