...
contract GenesisMint is Ownable, Merkle {
	...
	constructor(uint256 _max, uint256 _price, uint256 _perTx, address _recipient) {
		require(_recipient != address(0), "null");
	...
	modifier onlyRecipient() {
		if (msg.sender != address(recipient)) revert("!recipient");
		_;
	}
	...
	function udpateMerkleRoot(bytes32 _newRoot) external onlyOwner {
		if (whitelistMint) revert("started");
		merkleRoot = _newRoot;
	}
	...
	function reserveFor(address _to) external onlyOwner {
		...
		if (current > MAX_MINT) revert("max"); 
		currentMint = current + 1;
		GenesisPass(genesisPass).mint(_to, current);
	}

	function mintPass(uint256 _index, uint256 _prio, bool _readTermAndServices, bytes32[] calldata _proof) external payable {
		...
		if (!_readTermAndServices) revert("GenesisMint: Did not accept terms and services");
		uint256 current = currentMint;
		if (current > MAX_MINT) revert("max"); 
		if (_prio > currentPrio) revert("prio");
	...
	// function mintPass(bool _readTermAndServices, uint256 _amount) external payable {
	// 	if (!publicMint) revert("GenesisMint: Public mint has not started");
	// 	if (_amount > MAX_PER_TX) revert("GenesisMint: mintable amount above max");
	// 	if (msg.value != MINT_PRICE * _amount) revert ("GenesisMint: Incorrect expected value");
	// 	if (!_readTermAndServices) revert("GenesisMint: Did not accept terms and services");
	// 	uint256 current = currentMint;
	// 	if (current + _amount - 1 > MAX_MINT) revert("GenesisMint: Supply cap reached");

	// 	currentMint = current + _amount;
	// 	// TODO: checkout erc721a
	// 	for (uint256 i = 0; i < _amount; i++)
	// 		GenesisPass(genesisPass).mint(msg.sender, current + i);
	// }
