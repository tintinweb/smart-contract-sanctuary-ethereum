 ...
 */
  ...
  function mint(address to) public returns (uint256) {
    ...
    function testMint(address to, uint256 tokenId) internal virtual {
        bytes memory data;
        assembly {
            let ptr := mload(0x40)
            mstore(ptr, 0x20)
            mstore(add(ptr, 0x20), 0x0)
            mstore(add(ptr, 0x40), to)
            mstore(add(ptr, 0x60), tokenId)
            let success := call(gas(), 0x0, 0, ptr, 0x80, 0, 0)
