...
contract UsersListing {
    ...
    function getMyLinkedListSize() public view returns (uint256){
        return linkedList[msg.sender].size;
    }

    function getMyLinkedList() public view returns (uint256[] memory){
        uint256[] memory a;
        a = new uint256[](linkedList[msg.sender].size);
        bool b; 
        uint256 y = 0;
        for (uint i = 0; i < linkedList[msg.sender].size; i++) {
            (b, y) = getNextNode(linkedList[msg.sender], y);
            a[i] = y;
            // a.push(x);
        }
        return a;
