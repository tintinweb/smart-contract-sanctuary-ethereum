...
contract InvestmentFund is StateMachine, IInvestmentFund, ReentrancyGuard {
    ...
    function invest(uint240 amount) external override onlyAllowedStates nonReentrant {
        ...
        if (newTotalInvestment >= cap) {
            ...
            emit CapReached(msg.sender, address(currency), amount, cap);
        }
    ...
    function _invest(address investor, uint256 amount) internal {
        ...
        require(currency.transferFrom(investor, address(this), amount - fee), "Currency transfer failed");
...
library LibFund {
    ...
    bytes32 public constant STATE_BREAKEVEN = "Breakeven"; // 0x427265616b6576656e0000000000000000000000000000000000000000000000
