...
contract DPUNK is ERC20("$DPUNK", "$DPUNK") {
	...
	uint256 public FirstRedeemRate = 10 ether; 
	...
	constructor(address _punks, address _punks1) public{
		PunkContract = PunkC(_punks);
        PunkContract1 = PunkD(_punks1);
		_owner = _msgSender();
	}
   ...
   function setFirstRedeemRate(uint256 _FirstRedeemRate) external {
    ...
    function getRewards1(uint256[] calldata _tokenIds) external {
		uint256 ramount;
		uint256 tamount;
		for(uint256 i = 0; i < _tokenIds.length; i++) {
		uint256 _punk1 = _tokenIds[i];
		require(msg.sender == address(PunkContract1.ownerOf(_punk1)), "You are not the owner of this punk");
		require(!isPauseEnabled, "Staking is on pause");
		uint256 time = block.timestamp;
			if(lastUpdate[_punk1] == 0) {
		    ramount= FirstRedeemRate;
		    }else{
		    ramount = uint(time - lastUpdate[_punk1]) / rewardtimeframe * BASE_RATE; //daily 
		    }
		    if(ramount>1){
			lastUpdate[_punk1] = time;
			tamount=tamount+ramount;
	    	}}
		 require(tamount > 1, "Your punks didn't work hard enough");	
		_mint(msg.sender, tamount);
    ...
    function getTotalClaimableU1(address address_) external view returns(uint256) {		
		uint _balance = PunkContract1.balanceOf(address_); // get balance of address
        uint contractMaxToken = PunkContract1.totalSupply(); //total supply of contract
        uint[] memory _tokens = new uint[](_balance); // initialize array 
		require(_balance>=1,"You do not own a punk");
        uint _index;
            for (uint id = 1; id <= contractMaxToken; id++) {
            if (address_ == PunkContract1.ownerOf(id)) { _tokens[_index] = id; _index++;}
            }
        uint256 _punk1;
		uint256 tamount;
		    for (uint256 ind = 0; ind < _tokens.length; ind++) {
    ...
    function getTotalClaimable1(uint256[] calldata _tokenIds) external view returns(uint256) {		
		uint256 _punk1;
		uint256 tamount;
		    for (uint256 ind = 0; ind < _tokenIds.length; ind++) {
			_punk1=_tokenIds[ind];
		uint256 time = block.timestamp;
		uint256 ramount;
			if(lastUpdate[_punk1] == 0) {
			ramount= FirstRedeemRate;
    ...
    function getClaimable1(uint256 _punk1) external view returns(uint256) {
		uint256 time = block.timestamp;
		uint256 ramount;
			if(lastUpdate[_punk1] == 0) {
			ramount= FirstRedeemRate;
		    }else{
		    ramount = uint(time - lastUpdate[_punk1]) / rewardtimeframe * BASE_RATE;
            } //daily 
	//uint256 ramount = rewards[_punk] + pending;
	//require(lastUpdate[_punk] > 0, "Ask your punk to start grinding for you");
		return ramount;	
    ...
    function TokensOfOwner1(address address_) public view returns (uint[] memory) {
        uint _balance = PunkContract1.balanceOf(address_); // get balance of address
        uint contractMaxToken = PunkContract1.totalSupply(); //total supply of contract
        uint[] memory _tokens = new uint[](_balance); // initialize array 
		require(_balance>=1,"You do not own a punk");
