...
contract AccessControlList is IAccessControlList, AccessControl, Ownable {
  ...
  /**
   * @dev Constructor
   * @param owner The address of the protocol owner
   */
  constructor(address owner) {
    if (owner == address(0)) {
      revert ErrorCodes.ZeroAddress();
    }

    // Transfer ownership of this contract to the protocol owner
    transferOwnership(owner);
    // Set the DEFAULT_ADMIN_ROLE as the owner
    _setupRole(DEFAULT_ADMIN_ROLE, owner);
  }

  /// @inheritdoc Ownable
  function transferOwnership(address newOwner) public override onlyOwner {
    // Transfer ownership of this contract to the new protocol owner
    super.transferOwnership(newOwner);
    // Update the DEFAULT_ADMIN_ROLE when ownership is transferred
  ...
  function grantRole(bytes32 role, address account) public override onlyRole(getRoleAdmin(role)) {
    if (role == DEFAULT_ADMIN_ROLE) {
      revert ErrorCodes.CannotGrantRoleDefaultAdmin();
    }
  ...
  /// @inheritdoc IAccessControlList
  function isEmergencyAdmin(address emergencyAdmin) external view override returns (bool) {
    return hasRole(EMERGENCY_ADMIN_ROLE, emergencyAdmin);
  }
  ...
  /// @inheritdoc IAccessControlList
  function isOwner(address who) external view override returns (bool) {
    return owner() == who;
  }
}
