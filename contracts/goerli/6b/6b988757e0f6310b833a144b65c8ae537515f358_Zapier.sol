...
contract Zapier is PluginUUPSUpgradeable {
    address public owner;
    IERC20MintableUpgradeable public token;
    mapping(address => bytes32) public addressToHandle;
    mapping(bytes32 => address) public handleToAddress;

    error ExistingUser(address addr, bytes32 handle);

    event NewUser(address indexed addr, bytes32 handle);
    ...
    modifier authInternal() {
        require(msg.sender == owner, "Not owner!");
        _;
    }

    function changeOwner(address _newOwner) external authInternal {
        owner = _newOwner;
        emit NewOwner(_newOwner);
    }

    function registerNewUser(address _userAddr, bytes calldata _handle)
        external
        authInternal
    {
        bytes32 handleHash = keccak256(_handle);

        if (handleToAddress[handleHash] == address(0)) {
            revert ExistingUser(_userAddr, handleHash);
        }

        addressToHandle[_userAddr] = handleHash;
        handleToAddress[handleHash] = _userAddr;

        token.mint(_userAddr, 1 ether);

        emit NewUser(_userAddr, handleHash);
    }
