...
contract FarmStaking is IFarmStaking, ERC721Holder, ERC1155Holder, ReentrancyGuard, Initializable, Pausable, Recoverable {
    ...
    mapping(address => uint256) private currentStakingCountMap;

    uint8 constant STAKING_STATUS_STAKED = 1;
    uint8 constant STAKING_STATUS_UNSTAKED = 2;

    IAppConf appConf;
    IFarmReward farmReward;

    function init(IAppConf _appConf) public onlyOwner {
        appConf = _appConf;

        Model.FarmAddr memory farmAddr = appConf.getFarmAddr();
        farmReward = IFarmReward(farmAddr.farmRewardAddr);

        initialized = true;
    }

    function stake(address nftToken, uint256[] calldata tokenIds, string calldata wish) public whenNotPaused nonReentrant needInit {
        require(!appConf.validBlacklist(_msgSender()), "can not stake");
        require(appConf.validStakingNftToken(nftToken), "invalid nft token");
        require(tokenIds.length > 0, "tokenids length is 0");
        ...
        for (uint256 i = 0; i < tokenIds.length; i++) {
            require(IERC721(nftToken).ownerOf(tokenIds[i]) == _msgSender(), "invalid nft owner");
            require(isApprovedForAll || IERC721(nftToken).getApproved(tokenIds[i]) == address(this), "tokenId not approved");
            ...
            if (gen == 0) {
                ...
                require(gen == nftGen, "invalid nft gen");
            }
        ...
        stakingRecordMap[_msgSender()].push(Model.StakingRecord({
            ...
            userAddr: _msgSender(),
            tokenIds: tokenIds,
            status: STAKING_STATUS_STAKED,
            stakingBlockNumber: block.number,
            stakingTime: block.timestamp,
            unstakingBlockNumber: 0,
            unstakingTime: 0
        }));
        currentStakingCountMap[_msgSender()]++;

        emit Staked(_msgSender(), stakingIndex, nftToken, tokenIds, gen, wish, block.timestamp);
    }

    function unstake(uint256 stakingIndex) public whenNotPaused nonReentrant needInit {
        require(!appConf.validBlacklist(_msgSender()), "can not unstake");

        _unstake(_msgSender(), stakingIndex);
    }

    function _unstake(address userAddr, uint256 stakingIndex) private {
        require(stakingRecordMap[userAddr].length > stakingIndex, "invalid index");
        require(stakingRecordMap[userAddr][stakingIndex].status == STAKING_STATUS_STAKED, "invalid staking status");

        // claim reward
        farmReward.proxyClaim(userAddr, stakingIndex);

        // update status
        Model.StakingRecord storage record = stakingRecordMap[userAddr][stakingIndex];
        record.status = STAKING_STATUS_UNSTAKED;
        ...
        // stat staking count
        currentStakingCountMap[userAddr] -= record.tokenIds.length;

        emit Unstaked(userAddr, stakingIndex, record.nftToken, record.tokenIds, record.gen, block.timestamp);
    }

    function unstakeAll() public whenNotPaused nonReentrant needInit {
        require(!appConf.validBlacklist(_msgSender()), "can not unstake");
    ...
    function getCurrentStakingCount(address userAddr) public view override returns(uint256) {
        return currentStakingCountMap[userAddr];
    }

    function getStakingRecordByIndex(address userAddr, uint256 stakingIndex) public view returns(Model.StakingRecord memory) {
        return stakingRecordMap[userAddr][stakingIndex];
    }

    function getStakingRecords(address userAddr) public view returns(Model.StakingRecord[] memory) {
        return stakingRecordMap[userAddr];
    }

    function getStakingStatus(uint256 tokenId) public view returns(uint256) {
    ...
    function getTotalStakingCount() public view returns(uint256) {
...
abstract contract Recoverable is Ownable {
    ...
    function recoverToken(IERC20 token, address toAddr) public virtual onlyOwner {
        token.transfer(toAddr, token.balanceOf(address(this)));
    }
