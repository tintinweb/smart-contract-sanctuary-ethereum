...
contract testingMintInNft is ERC721AUpgradeable, OwnableUpgradeable, VRFConsumerBaseV2 {

    error IncorrectAmount();
    error NotAllowed();

    uint256 public executionCostAccumulated;
    uint256 public totalExecution;
    ...
    function initialize (
        ...
        executionCostAccumulated = 0.001 ether;
        mintingCost = 0.001 ether;
    }

    function mintToken(uint256 _amount) external payable {
        uint amount = _amount;
        if ((mintingCost * amount) + executionCostAccumulated != msg.value) {
    ...
    function fulfillRandomWords(
        ...
        uint256 _amount = requests[requestId].requestAmount;
            for (uint256 i =0;i<_amount; i++) {
                currentTokenId += 1;
                tokenSeed[currentTokenId] = uint256(keccak256(abi.encodePacked(randomNumber, currentTokenId)));
            }
    }

    function execute(uint256 _requestId, uint256[] memory _randomWords) external {
        fulfillRandomWords(_requestId, _randomWords);
    }

    function allowUser(address _suer) external onlyOwner {
        isAllowed[_suer] = true;
    }

    function changeGasLimit (bytes32 _keyhash,
        uint32 _callbackGasLimit,
        uint64 _subscriptionId) external onlyOwner {
        keyHash = _keyhash;
        callbackGasLimit = _callbackGasLimit;
