...
contract EgoNFTDEMO is EgoNFT {
    using Counters for Counters.Counter;

    constructor() EgoNFT("EGO", "EGO") {}

    function claim() external override whenClaimActive EOAOnly {
    ...
    function burn() private {
        uint tokenId = super.tokenOfOwnerByIndex(msg.sender, 0);
        if (tokenId > 0) {
            super._burn(tokenId);
        }
    }
...
contract EgoNFT is ERC721, ERC721Enumerable, Ownable {
    ...
    */
    function claim() external virtual whenClaimActive EOAOnly {
        if (balanceOf(msg.sender) > 0) {
