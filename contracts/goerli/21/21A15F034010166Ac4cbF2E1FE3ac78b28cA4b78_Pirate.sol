    ...
    UUPSUpgradeable
{
    ...
    function initialize(uint16 maxSupply, uint256 mintPrice) public initializer {
        ...
        // FemaleHair enum
        _weightPool.femaleHair = 0x1194_1194_03e8;
        // FemaleHat enum
        ...
        // MaleHair enum
        _weightPool.maleHair = 0x06b8_06b8_06b8_02bc_06b8_06b8_02bc;
        _weightPool.maleHairIfCrown = 0x0fa0_0fa0_07d0;
        _weightPool.maleHairIfMask = 0x0708_0708_0708_03e8_0708_0708;
        // MaleHat enum
    ...
    function mint(
        ...
        if (_launchDate >= block.timestamp + 1 days) {
            revert MintingUnavailable();
        }
        ...
        if (!MerkleProofUpgradeable.verify(proof, _merkleRoots[amount - 1], keccak256(abi.encodePacked(to)))) {
            revert InvalidProof();
        }

        if (_whitelistedMints[to] >= amount) {
            revert WhitelistLimitExceeded();
        }

        _mintCore(to, amount);

        _whitelistedMints[to] += amount;
    }
    ...
    function stake(uint256[] calldata tokenIds) external whenNotPaused noContract {
        for (uint256 i; i < tokenIds.length; i++) {
            ...
            _stakes[tokenId] = Stake({owner: msg.sender, timestamp: uint64(block.timestamp)});
        }
    }
    ...
    function _mintCore(address to, uint256 amount) private {
        if (msg.sender != owner() && msg.value < _mintPrice * amount) {
    ...
    function _randomTraits(uint256 seed) private view returns (TraitSet memory) {
        ...
        if (traits.gender == Gender.Male) {
                ...
                if (traits.special == Special.None) {
                    ...
                    if (hat != MaleHat.SamuraiHelmet) {
                        if (hat == MaleHat.Crown || hat == MaleHat.Headband || hat == MaleHat.RoyalCrown) {
                            ...
                            traits.hair = uint8(Random.weighted(_weightPool.maleHairIfCrown, 3, seed++) + 2);
                        } else {
                            ...
                            if (
                                ...
                                traits.hair = uint8(Random.weighted(_weightPool.maleHairIfMask, 6, seed++) + 1);
                            } else {
                                traits.hair = uint8(Random.weighted(_weightPool.maleHair, 7, seed++) + 1);
                            }
                        }
                    }
                } else {
            ...
            traits.hair = uint8(Random.weighted(_weightPool.femaleHair, 3, seed++));
            traits.hairColor = HairColor(Random.weighted(_weightPool.hairColor, 6, seed++) + 1);
