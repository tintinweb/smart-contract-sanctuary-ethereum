...
contract LotteryTicketNftFactory {
    ...
    function getBytecode(string memory _baseUri, uint256 _ticketPrice, uint256 _lotteryDurationInMinutes) internal view returns (bytes memory) {
        ...
        return abi.encodePacked(proxyCreationCode, abi.encode(beacon, data));
    }

    function getSalt(string memory _baseUri, uint256 _ticketPrice, uint256 _lotteryDurationInMinutes) internal pure returns (bytes32) {
        return keccak256(abi.encodePacked(_baseUri, _ticketPrice, _lotteryDurationInMinutes));
    }
}
...
contract LotteryTicketNft is Initializable, ERC721Upgradeable, ERC721URIStorageUpgradeable, OwnableUpgradeable, ReentrancyGuardUpgradeable {
  ...
  uint256 public lotteryEndDate;
  uint256 public surpriseWinnerStartDrawDate;
  uint256 private fee;
  ...
  event WinnerPayed(address winner, uint256 amount);
  ...
  function initialize(
    ...
    uint256 _lotteryDurationInMinutes
      ...
      require(_lotteryDurationInMinutes > 0, "Value for 'lotteryDurationInMinutes' must be greater than0.");
      baseURI = _baseUri;
      ticketPrice = _ticketPrice;
      lotteryEndDate = block.timestamp + _lotteryDurationInMinutes * 1 minutes;
      // The time after which if there is a mint transaction it will draw the surprise winner.
      surpriseWinnerStartDrawDate = block.timestamp + (_lotteryDurationInMinutes / 2) * 1 minutes;
  ...
  function safeMint(address to) public payable mintCompliance(_msgSender()) nonReentrant {
      ...
      if (surpriseWinner.isDrawn == false && block.timestamp >= surpriseWinnerStartDrawDate) {
        if(participants.length == 1) {
          surpriseWinner._address = participants[0];
        } else {
          uint256 winner = _drawRandomWinner();
          surpriseWinner._address = participants[winner];
        }
        surpriseWinner.isDrawn = true;
      }
  ...
  function drawAndPayWinners() public onlyOwner nonReentrant afterLotteryIsFinished {
    if (!surpriseWinner.isDrawn || !finalWinner.isDrawn) {
      drawWinners();
    }
    payWinners();
  }
  ...
  function drawWinners() public onlyOwner nonReentrant afterLotteryIsFinished {
    require(!surpriseWinner.isDrawn || !finalWinner.isDrawn, "All winners are already drawn.");

    if(participants.length == 1) {
        require(participants[0] != address(0), "There is no valid address as participant.");
        if (!surpriseWinner.isDrawn) {
          surpriseWinner.isDrawn = true;
        }
        finalWinner.isDrawn = true;
        finalWinner._address = participants[0];
        emit WinnerDeclared(finalWinner._address);
        return;
    }
    ...
    if (!surpriseWinner.isDrawn) {
      ...
      surpriseWinner._address = participants[winner];
      surpriseWinner.isDrawn = true;
      emit WinnerDeclared(surpriseWinner._address);
    }

    if (!finalWinner.isDrawn) {
      uint256 winner = _drawRandomWinner();
      if (participants[winner] == surpriseWinner._address) {
          while (participants[winner] == surpriseWinner._address) {
            winner = _drawRandomWinner();
          }
      }
      finalWinner._address = participants[winner];
      finalWinner.isDrawn = true;
      emit WinnerDeclared(finalWinner._address);
    }
  }
  ...
  function payWinners() public onlyOwner nonReentrant afterLotteryIsFinished {
    require(finalWinner._address != address(0) && (participants.length > 1 && surpriseWinner._address != address(0)), "The winners drawing is still in progress.");
    ...
    if (!surpriseWinner.isPaid) {
      ...
      (bool success, ) = surpriseWinner._address.call{value: surpriseWinnerAmount}("");
      require(success, "Transfer for surprise winner failed.");
    }

    if (!finalWinner.isPaid) {
      finalWinner.isPaid = true;
      (bool success, ) = finalWinner._address.call{value: finalWinnerAmount}("");
  ...
  function _drawRandomWinner() internal view returns(uint256) {
      ...
      return randomNumber.mod(participants.length);
  }
  ...
  modifier mintCompliance(address _payer) {
    require(block.timestamp < lotteryEndDate, "Lottery is already closed.");
    require(msg.value >= ticketPrice, "Insufficient funds!");
    _;
  }

  modifier afterLotteryIsFinished() {
    require(block.timestamp >= lotteryEndDate, "The Lottery isn't finished yet.");
    _;
  }
