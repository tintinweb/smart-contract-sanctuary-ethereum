    ...
    ERC1155SupplyUpgradeable
{
        ...
        onlyRole(EXTERNAL_CONTRACT_ROLE)
    {
        ...
        for (uint256 i = 0; i < _amount; i++) {
            ...
            if (chance < rarities[0]) {
                // LEGENDARY
                ids[i] = shopDropItemIds[0];
            } else if (chance < rarities[1]) {
                // SUPER RARE ITEM
                ids[i] = shopDropItemIds[1];
            } else if (chance < rarities[2]) {
                // RARE ITEM
                ids[i] = shopDropItemIds[2];
            } else if (chance < rarities[3]) {
                // UNCOMMON ITEM
                ids[i] = shopDropItemIds[3];
            } else if (chance < rarities[4]) {
                // COMMON ITEM 1
                ids[i] = shopDropItemIds[4];
            } else if (chance < rarities[5]) {
                // COMMON ITEM 2
                ids[i] = shopDropItemIds[5];
            } else if (chance < rarities[6]) {
                // COMMON ITEM 3
                ids[i] = shopDropItemIds[6];
            }
            amounts[i] = 1;
        }
