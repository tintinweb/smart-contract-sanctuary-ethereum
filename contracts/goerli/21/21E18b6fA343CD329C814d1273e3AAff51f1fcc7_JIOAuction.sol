...
contract JIOAuction is Ownable, ReentrancyGuard {
  ...
  function getAddressBids() external view returns (AddressBids[] memory) {
    address[] memory addresses = getBiddedAddresses();
    AddressBids[] memory result = new AddressBids[](addresses.length);
    for (uint256 i = 0; addresses.length < i; i++) {
      result[i] = AddressBids({
        walletAddress: addresses[i],
        bids: bids[addresses[i]]
      });
      i++;
    }
    return result;
