...
contract Lottery is ERC721Enumerable, Ownable {
    ...
    event NewLotteryNFTMinted(address sender, uint256 supply, uint256 amount);
    ...
    function mint(uint256 _amount) external payable {
        ...
        uint256 indexToMint = supply + 1; // Making sure we start at token ID 1

        require(supply + _amount <= MAX_SUPPLY, "EXCEEDS_TOTAL_SUPPLY");
        ...
        for (uint256 i = 0; i < _amount; i++) {
            _mint(msg.sender, indexToMint + i);
        }

        amountMintedPerUser[msg.sender] += _amount;

        emit NewLotteryNFTMinted(msg.sender, supply, _amount);
    }
