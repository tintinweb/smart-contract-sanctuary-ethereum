...
contract TimePieces is Satoshigoat {
	...
	constructor() Satoshigoat("GUD & EVOL x TIME", "", "ipfs://") 
	{
		...
		purchasePrice = 10000000000000;//testing (.00001 eth)
		isPrivate = true;
	}
	...
	modifier enoughEther(address payable to, uint256 amount) {
		uint256 plugBalance = plugNft.balanceOf(to);
		if (plugBalance > 0 && isPrivate) {
			...
			if (msg.value < firstTerm * secondTerm) 
				revert InsufficientFunds();
		} else {
			if (amount < purchasePrice) revert InsufficientFunds();
		}
		_;
	}
