...
contract LIFO is ERC1155Supply, Ownable, DefaultOperatorFilterer {
    ...
    constructor() ERC1155("https://nftstorage.link/ipfs/bafybeig5mp2mrw34nabim55fahbu5ysdcoozursuoiptidpw3bny32hwum/{id}.json") {
        isSaleOpen = false;
    }   

    function mint(uint256 _mintAmount) public payable callerIsUser {
        if (totalSupply(TOKEN_ID) + _mintAmount > collectionSize) revert MintingTooMany();
        if (!isSaleOpen) revert SaleNotStarted();
        if (numberMinted[msg.sender] + _mintAmount > maxMintAmount) revert MintingTooMany();
        if (msg.value < _mintAmount * mintPrice) revert InsufficientFunds();

        numberMinted[msg.sender] += _mintAmount;
        _mint(msg.sender, TOKEN_ID, _mintAmount, "");
    }
