...
contract Nemesis is ERC721A {
  ...
  mapping(bytes => bool) private _usedNonces;
  ...
  function mint(uint256 price, bytes memory nonce, bytes memory signature) external payable {
      ...
      require(recover(hashAddressPriceNonce(msg.sender, price, nonce), signature) == API_SIGNER, "Invalid signature");
  ...
  function renew(uint256 price, bytes memory nonce, bytes memory signature, uint256 _tokenId) public payable {
  ...
  function isNonceUsed(bytes memory nonce) public view returns (bool) {
  ...
  // ECDSA
    ...
    /**
      * @dev hashes data that would be encrypted on esrver
    */
    function hashAddressPriceNonce(address addr, uint256 price, bytes memory nonce) internal pure returns (bytes32) {
        return keccak256(
