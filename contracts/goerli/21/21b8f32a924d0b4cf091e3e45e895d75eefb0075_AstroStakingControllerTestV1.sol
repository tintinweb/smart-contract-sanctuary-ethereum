...
contract AstroStakingControllerV1 is Initializable, UUPSUpgradeable, OwnableUpgradeable {
  ...
  function initialize(address _tinyAstro, address _astroToken, address _emissionRateManager) public initializer {
  ...
  function _unstake(uint256 tokenId) internal returns (uint256 amountToMint) {
    ...
    if (!isMintingPaused) {
      (amountToMint, ) = IEmissionRateManager(emissionRateManager)
        .amountToMint(tokenId, stakedToken.timestamp, _secondsPerDay());
    }
    ...
    if (!isMintingPaused) {
      unchecked {
        for (uint256 i = 0; i < stakedTokenIds.length; i++) {
          ...
          uint256 emissionRate = IEmissionRateManager(emissionRateManager).currentEmissionRate(tokenId);
          dailyYield += emissionRate;

          (uint256 amount, ) = IEmissionRateManager(emissionRateManager)
            .amountToMint(tokenId, stakedTokens[tokenId].timestamp, _secondsPerDay());
...
interface IEmissionRateManager {
  ...
  function currentEmissionRate(uint256 tokenId) external view returns (uint256 emissionRate);

  function amountToMint(uint256 tokenId, uint256 timestamp, uint256 interval) external view returns (uint256 amount, uint256 newTimestamp);
}
