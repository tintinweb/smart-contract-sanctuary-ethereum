...
contract IndieBrokerV1 is EIP712, Ownable, Pausable, ReentrancyGuard {
    ...
    function completeSprint(
        ...
        for (uint256 i = 0; i < payees.length; i++) {
            ...
            totalCashVestingAmount += cashVestingAmount;

            uint256 payeeAmount = amount - treasuryAmount - leadAmount - salesAmount - cashVestingAmount;

            emit DistributePayment(
                payee, projectId, amount, payeeAmount, treasuryAmount, leadAmount, salesAmount, cashVestingAmount
                );
        ...
        if (totalTreasuryAmount > 0) {
            usdc.transfer(feeRecipients[Fees.Treasury], totalTreasuryAmount);
        }

        if (totalCashVestingAmount > 0) {
            usdc.transfer(feeRecipients[Fees.CashVesting], totalCashVestingAmount);
        }

        if (totalLeadAmount > 0) {
            usdc.transfer(project.leadAddress, totalLeadAmount);
        }

        if (totalSalesAmount > 0) {
            usdc.transfer(project.salesAddress, totalSalesAmount);
        }

        projectBalances[projectId] -= totalAmount;
