...
contract UTBadgeDropperV3 {
    ...
    string                          public constant     REGISTRY_KEY_RANDOM_CONTRACT = "RANDOMV2_SSP";
    ...
    /**
     * @dev Constructor: validate chain and prepare GalaxisRegistry
     */
    constructor(
    ) {
        uint256 id;
        ...
        // Only applicable to selected chains...
        if(chainid == 1 || chainid == 5 || chainid == 1337 || chainid == 31337) {
            ...
            revert("UTBadgeDropper: invalid chainId");
        }
    }
    ...
    //  * @dev Drop traits randomly (optimal for trait amounts more then half of the range)
    //  */
    // function _inverseDrop(uint16 _rangeStart, uint16 _rangeEnd, uint16 _traitId, uint16 _traitsToDrop, uint256 _randomNumber, address _ECrAddress) internal {
    //     uint16 offset;
    //     uint16 range = _rangeEnd - _rangeStart + 1;
    //     uint16[] memory selectedTokens = new uint16[](_traitsToDrop);
    //     uint16[] memory invertedTokens = new uint16[](range - _traitsToDrop);
    //     uint256 initRandom = _randomNumber;
    //     uint256 random = initRandom;
    ...
    //         if (random > range) {
    //             offset = 0;
    //             // while (offset < range && IECRegistry(_ECrAddress).hasTrait(_traitId, uint16((random + offset) % range + _rangeStart))) {
    //             //     offset++;
    //             // }
    //             // The above is coded here not using maps, only arrays (cheaper?)
    //             while(true) {
    //                 pos = uint16((random + offset) % range + _rangeStart);
    //                 found = false;
    //                 console.log("dropCount:", dropCount);
    //                 for (uint256 index = 0; index < dropCount; index++) {
    //                     if (selectedTokens[index] == pos) {
    //                         found = true;
    //                         break;
    //                     }
    //                 }
                    
    //                 if (offset < range && !found) {
    //                     if (IECRegistry(_ECrAddress).hasTrait(_traitId, pos)) {
    //                         offset++;
    //                         continue;
    //                     }
    //                 }
    ...
    //     console.log("range - _traitsToDrop:", range - _traitsToDrop);
    //     console.log("dropcount:",dropCount);
    //     for (uint16 i = _rangeStart; i <= _rangeEnd; i++) {
    //         found = false;
    //         for (uint256 index = 0; index < dropCount; index++) {
    //             if (selectedTokens[index] == i) {
    //                 found = true;
    //                 break;
    //             }
    //         }
    //         if (!found) {
    //             invertedTokens[pos++] = i;   
    //         }
    //     }
    //     dropCount = IECRegistry(_ECrAddress).setTraitOnMultiple(_traitId, invertedTokens, true);
    ...
    // Not needed as the Community Registry determination also needed in all functions we want to protect (so we do not calculate it twice)

    // modifier onlyAllowed(uint32 _communityId, bytes32 role) { 
    //     require(isAllowed(_communityId, role, msg.sender), "UTBadgeDropper: Unauthorised");
    //     _;
    // }

    // function isAllowed(uint32 _communityId, bytes32 role, address user) public view returns (bool) {
    //     // Find community registry
    //     CommunityList COMMUNITY_LIST = CommunityList(GalaxisRegistry.getRegistryAddress("COMMUNITY_LIST"));
    //     (,address crAddr,) = COMMUNITY_LIST.communities(_communityId);
    //     CommunityRegistry thisCommunityRegistry = CommunityRegistry(crAddr);
    //     return thisCommunityRegistry.community_admin() == user || thisCommunityRegistry.hasRole(role, user);
    // }
}
