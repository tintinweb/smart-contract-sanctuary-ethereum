    ...
    DefaultOperatorFilterer
{
    ...
    uint256 public publicSupply = 9500;
    uint256 public treasuryMax = 500;
    ...
    function mint(uint256 qty) external payable noContract nonReentrant {
        require(state == State.Open, "NO_SALE");
        require(mintPrice * qty == msg.value, "WRONG_ETHER_AMOUNT");
        require(totalSupply() + qty <= publicSupply, "SUPPLY_EXCEEDED");

        _safeMint(msg.sender, qty);
    }

    function presaleMint(uint256 qty) external payable noContract nonReentrant {
        require(state == State.Presale, "NO_PRESALE");
        require(mintPrice * qty == msg.value, "WRONG_ETHER_AMOUNT");
        require(totalSupply() + qty <= publicSupply, "SUPPLY_EXCEEDED");
    ...
    function treasuryMint(address wallet, uint256 amount) public onlyOwner {
        require(
            totalSupply() + amount <= treasuryMax,
    ...
    function adminMint(address[] calldata wallets, uint256[] calldata amounts)
        public
        onlyOwner
    {
        ...
        require(totalSupply() + total <= supply, "SUPPLY_EXCEEDED");
    ...
    function withdrawAll(address recipient) public onlyOwner {
        ...
        _withdrawCall(recipient, (balance * 99) / 100); // 99%

        emit EtherWithdrawn(recipient, (balance * 99) / 100);
    }
