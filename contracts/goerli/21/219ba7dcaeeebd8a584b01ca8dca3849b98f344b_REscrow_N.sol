...
contract AssetType is Utilities, Ownable {
    ...
    function addNewAssetTypeMap(string memory _assetTypeName, uint _assetPrice) public onlyOwner{
...
contract EscrowProducer is Utilities, Ownable, AssetType{
    ...
    function getProducerInfoArray() public view returns(ProducerInfoStruct[] memory){
        ...
        for (uint i = 0; i < producerArray.length; i++)
        {
            ...
            producerInfo.quantity = producerArray[i].postedAssets.length;
            ...
            AssetsTypeStruct memory assetType = AssetType.getAssetTypeByID(producerArray[i].assetTypeID);
    ...
    function findMatchedAssets(uint _escrowID, uint[] memory _collectedAssets) internal returns(string memory,address, uint[] memory,uint,uint){
        ...
        uint totalRedeemedAmount = matchedAssets.length*producerArray[_escrowID].amount/producerArray[_escrowID].postedAssets.length;
        ...
        return (producerArray[_escrowID].escrowName,producerArray[_escrowID].producer, matchedAssets,producerRedeemedAmount,providerRedeemedAmount);
...
contract EscrowProvider is Utilities, Ownable{
    ...
    function getProviderInfoArray() public view returns(ProviderInfoStruct[] memory){
        ...
        for (uint i = 0; i < providerArray.length; i++)
        {
            ...
            providerInfo.quantity = providerArray[i].collectedAssets.length;
    ...
    function getProviderInfoByID(uint _providerID) public view returns(ProviderInfoStruct memory){
        ...
        providerInfo.quantity = providerArray[_providerID].collectedAssets.length;
