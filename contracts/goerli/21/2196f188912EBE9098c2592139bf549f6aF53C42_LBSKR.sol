...
contract LBSKR is BaseBSKR, DSMath, Stakable {
    ...
    function penaltyIfUnstakedNow(
        ...
        // uint16 penaltyBasis = 0;
        // if (currStake.sinceBlock + 52 weeks > block.timestamp) {
        //     uint16 fourWeeksElapsed = uint16(
        ...
        uint16 penaltyBasis = penaltyFor(currStake.sinceBlock, block.timestamp);

        return (amount * penaltyBasis) / _BIPS;
    }
     ...
     */
    function rewardOf(address stakeholder, uint16 stakeIndex)
        external
        view
        returns (
        ...
        )
    {
        ...
        // eligibleBasis = _BIPS; //10000  --> 100%
        // if (currStake.sinceBlock + 52 weeks > block.timestamp) {
        //     uint16 fourWeeksElapsed = uint16(
        //         (block.timestamp - currStake.sinceBlock) / _SECS_IN_FOUR_WEEKS
        //     );
        //     if (fourWeeksElapsed < 13) {
        //         eligibleBasis -= ((13 - fourWeeksElapsed) * 100); // If one four weeks have elapsed - penalty is 12% or 1200/10000
        //     }
        // };
        eligibleBasis = _BIPS - penaltyFor(currStake.sinceBlock, block.timestamp);
        ...
        whenNotPaused
    {
        ...
        //     }
        // }
        uint16 eligibleBasis = _BIPS - penaltyFor(currStake.sinceBlock, block.timestamp);

        uint256 lbskrToSend = ((lbskrCurrRatio *
            lbskrShares2Deduct -
            stakeAmount) * eligibleBasis) / _BIPS; // stakeAmount never existed - it's notional
        ...
        if (eligibleBasis < _BIPS) {
            uint256 lbskrToBurn = ((lbskrCurrRatio *
                lbskrShares2Deduct -
                stakeAmount) * (_BIPS - eligibleBasis)) / _BIPS;
            ...
            uint256 bskrToBurn = ((bskrCurrRatio * bskrShares2Deduct) *
