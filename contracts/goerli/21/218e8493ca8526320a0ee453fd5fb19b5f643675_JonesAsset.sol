...
/// @dev Only supports one minter role at a time

contract JonesAsset is ERC20, ERC20Burnable, AccessControl, Ownable {
    bytes32 public constant MINTER_ROLE = keccak256("MINTER_ROLE");
    bool public MINTER_SET = false;
    ...
    constructor(
        ...
        transferOwnership(_multisigAddr);
    }
    ...
    function giveMinterRole(address _minterContract) public onlyOwner {
        require(MINTER_SET == false, "Minter role already set");
        MINTER_SET = true;
    ...
    function revokeMinterRole(address _minterContract) public onlyOwner {
        require(MINTER_SET == true, "Minter role not set");
        require(
            hasRole(MINTER_ROLE, _minterContract),
            "Minter contract does not have minter role"
        );
        MINTER_SET = false;
