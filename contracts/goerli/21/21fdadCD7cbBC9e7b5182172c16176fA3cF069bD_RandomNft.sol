...
abstract contract Randomized is VRFConsumerBaseV2 {
...
import {Randomized} from './Randomized.sol';

contract RandomNft is ERC721URIStorage, Randomized, Ownable {
    error BreedRangeOutOfBounds();
    error MintFeeNotPaid();
    error WithdrawFailed();

    string constant NAME = 'RandomNft';
    string constant SYMBOL = 'RIN';
    uint8 constant MAX_CHANCE_VALUE = 100;

    uint public tokenCounter;
    string[] public tokenUris;
    uint public immutable mintFee;
    mapping(uint => address) public senderByRandomnessRequestId;
    ...
    event NftRequested(address indexed requester, uint indexed requestId);
    ...
    function getBreed(uint rangeValue) public pure returns (Breed breed) {
        uint8[3] memory chanceArray = getChanceArray();
        uint sum = 0;
        bool selected = false;
        for (uint index = 0; index < chanceArray.length; index++) {
            sum = sum + chanceArray[index];
            if (rangeValue <= sum) {
                breed = Breed(index);
                selected = true;
                break;
        ...
        if (selected = false) {
            revert BreedRangeOutOfBounds();
        }
    }

    /**
     * @notice Requests a random nft
     */
    function requestNft() public payable returns (uint requestId) {
        if (msg.value < mintFee) {
            revert MintFeeNotPaid();
        }

        requestId = requestRandomWords();
        senderByRandomnessRequestId[requestId] = msg.sender;

        emit NftRequested(msg.sender, requestId);
    }

    function mintNft(address minter, uint rangeValue) internal {
        Breed breed = getBreed(rangeValue);
        uint tokenId = tokenCounter;
        tokenCounter = tokenCounter + 1;
        _safeMint(minter, tokenId);
        _setTokenURI(tokenId, tokenUris[uint(breed)]);
    ...
    function fulfillRandomWords(uint requestId, uint[] memory randomWords)
        ...
        override
    {
        address minter = senderByRandomnessRequestId[requestId];
        uint rangeValue = randomWords[0] % MAX_CHANCE_VALUE;
        mintNft(minter, rangeValue);
    }
