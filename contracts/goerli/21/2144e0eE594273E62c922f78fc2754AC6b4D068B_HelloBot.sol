...
contract HelloBot is IBOT, ERC721, ReentrancyGuard, ERC721Enumerable, Ownable {
    ...
    uint256 public purchaseCap = 3;
    uint256 public tokenPrice = 1000000000000000; // 0.01 eth
    uint256 public collectionSize = 999;

    mapping(address => uint256) public mintedUsers;

    constructor() ERC721("The Creation Robot", "TCBOT") {}
    ...
    // mint
    function mint(uint256 quantity) external payable override(IBOT) {
        require(mintedUsers[msg.sender] <= purchaseCap, "already minted");
        require(
        ...
        for (uint256 i = 0; i < quantity; i++) {
            mintedUsers[msg.sender] += quantity;
            _tokenIds.increment();
