...
contract SwapShop {
    string public purpose = "Swapping VROOM tokens";
    ...
    function init(uint256 tokens) public payable returns (uint256) {
      ...
      vroom.transferFrom(msg.sender, address(this), tokens);

      return totalLiquidity;
    }

    function price(uint256 input_amount, uint256 input_reserve, uint256 output_reserve) public pure returns (uint256) {
      // (amount of ETH in DEX) * (amount of tokens in DEX) = k ---> Uniswap formula
      // 997 represents .03% fee
    ...
    function ethToToken() public payable returns (uint256) {
      ...
      uint256 token_reserve = vroom.balanceOf(address(this));
      uint256 tokens_bought = price(msg.value, address(this).balance - msg.value, token_reserve);

      vroom.transfer(msg.sender, tokens_bought);
    ...
    function tokenToEth(uint256 tokens) public returns (uint256) {
      ...
      (bool success, bytes memory returnData) = msg.sender.call{value: eth_bought}("");
    ...
    function addLiquidity() public payable returns (uint256) {
      // this is a flaw in this design where you cannot add more
      // liquidity than the contract has ETH
      ...
      vroom.transferFrom(msg.sender, address(this), token_amount);
    ...
    function withdrawLiquidity(uint256 amount) public returns (uint256, uint256) {
      ...
      (bool success, bytes memory returnData) = msg.sender.call{value: eth_amount}("");
      require(success, "Transfer failed.");

      vroom.transfer(msg.sender, token_amount);
