...
contract PokerTicket is ERC721A, Ownable, ReentrancyGuard {
    ...
    constructor(string memory baseURI_, address signer_, MerkleRootConfig memory merkleRootConfig_) ERC721A("PokerTogether Pass Ticket", "PPT") {
        baseURI = baseURI_;
        signer = signer_;
    ...
    /**
     *  @notice alphaSale is used for alpha sale.
     *  @param signature_ merkel proof.
     */
    function alphaSale(bytes32[] calldata signature_) external callerIsUser nonReentrant {
        require(status == Status.Alpha, "alpha not enabled");
        require(isAlphaAddress(_msgSender(), signature_), "not alpha or invalid signature");
        uint256 alphaSaleMinted = _numberMinted(_msgSender()) + 1;
        require(alphaSaleMinted <= 1, "only 1 allowed");
        require(totalMinted() + alphaSaleMinted <= MAX_TOKEN, "max supply exceeded");
    ...
    function whitelistSale(uint64 numberOfTokens_, bytes32[] calldata signature_) external payable callerIsUser nonReentrant {
        ...
        require(isWhitelistAddress(_msgSender(), signature_), "not in whitelist or invalid signature");
    ...
    function _sale(uint64 numberOfTokens_, uint256 price_) internal {
     ...
     /**
     * @notice update ticket pass.
     * @param tokenId_ the ticket tokenId
     * @param updatedLevel_ the level that updated to
     * @param signature_ param signature signed with signer
     */
    function upgradePass(uint256 tokenId_, uint64 updatedLevel_, bytes memory signature_) external callerIsUser nonReentrant{
        ...
        require(signer != address(0), "signer not exist");
    ...
    //############################################################################
    /**
     * @notice isAlphaAddress is used to verify whether the given address_ and signature_ belong to merkleRoot.
     * @param address_ address of the caller
     * @param signature_ merkle proof
     */
    function isAlphaAddress(address address_, bytes32[] calldata signature_) public view returns (bool) {
        if (merkleRootConfig.alphaRoot == ""){
            return false;
        }
        return MerkleProof.verify(signature_, merkleRootConfig.alphaRoot , keccak256(abi.encodePacked(address_)));
    }
    ...
    function isWhitelistAddress(address address_, bytes32[] calldata signature_) public view returns (bool) {
        if (merkleRootConfig.whitelistRoot == "") {
            return false;
        }
        return MerkleProof.verify(
                signature_,
                merkleRootConfig.whitelistRoot,
    ...
    /**
     * @notice verify the param with the signature.
     * @param tokenId_ the ticket tokenId
     * @param updatedLevel_ the level that updated to
     * @param signature_ param signature signed with signer
     */
    function verfiySignature(uint256 tokenId_, uint64 updatedLevel_, bytes memory signature_) internal view returns (bool) {
        bytes32 message = ECDSA.toEthSignedMessageHash(keccak256(abi.encodePacked(tokenId_, updatedLevel_, _msgSender())));
