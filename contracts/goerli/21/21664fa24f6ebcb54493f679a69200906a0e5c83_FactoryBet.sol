...
contract FactoryBet is Struct, Ownable, ERC721A {
    //CONSTANTS
    ...
    // NEED TO CHECK
    mapping(address => bool) public addressIsParticipate;
    mapping(address => bool) public addressRemoved;
    mapping(uint256 => mapping(address => betResult))
    ...
    function bet(
        ...
        require(
            addressIsParticipate[tx.origin],
            "Address does not participate"
        );
        require(
            !poolToAddressBetResult[_poolId][tx.origin].isBet,
        ...
        poolToAddressBetResult[_poolId][tx.origin] = betResult(
            _pointA,
            _pointB,
            _winA,
            true
    ...
    function participate(uint256 _betIndex) external payable {
        ...
        addressIsParticipate[tx.origin] = true;
    ...
    function verifySignature(
        ...
        return ecrecover(digest, v, r, s) == creator;
    }
    ...
    function claim(
        ...
        require(
            verifySignature(_voucher, v, r, s, _bet.creator),
            "signature is wrong"
        );
    ...
    function endOfCup(uint256 _betIndex) external onlyCreator(_betIndex) {
        ...
        uint256 royalties = (_bet.totalPool * _bet.royalties) / 100;
        payable(0xa817a3D6C17d43612b1f790CC93D543d69ac6748).transfer(
            royalties
        );
        BetArray[_betIndex].pricePool = _bet.totalPool - royalties;
    }
    ...
    //not necessary anymore -> to remove
    // function getAddress(uint256 _betIndex) external view returns (address) {
    //     return address(BetArray[_betIndex]);
    // }
}