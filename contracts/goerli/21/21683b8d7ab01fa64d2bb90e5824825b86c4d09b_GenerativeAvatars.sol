...
/*
 ...
 ________  _______   ________  ________  _________  ___  ___      ___ _______      
|\   ____\|\  ___ \ |\   __  \|\   __  \|\___   ___\\  \|\  \    /  /|\  ___ \     
\ \  \___|\ \   __/|\ \  \|\  \ \  \|\  \|___ \  \_\ \  \ \  \  /  / | \   __/|    
 \ \  \  __\ \  \_|/_\ \   _  _\ \   __  \   \ \  \ \ \  \ \  \/  / / \ \  \_|/__  
  \ \  \|\  \ \  \_|\ \ \  \\  \\ \  \ \  \   \ \  \ \ \  \ \    / /   \ \  \_|\ \ 
   \ \_______\ \_______\ \__\\ _\\ \__\ \__\   \ \__\ \ \__\ \__/ /     \ \_______\
...
contract GenerativeAvatars is ERC721A {

    IDungeonRewards public dungeonRewardsContract;

    uint256 public MAX_NFTS_PLUS_1=6001;

    uint256 public MIN_STAKING_PERIOD = 1 days; // 7 days min staknig period to qualify for free avatar mint

    struct AccountInfo {
        uint256 freeMintedNFTs; // number of NFTs this wallet address has minted for free
        uint256 mintedNFTs; // number of non-free NFTs this wallet address has minted
    }
    ...
    constructor(address _dungeonRewardsContract) ERC721A("Generative Beep", "BEEP") {
    ...
    modifier dungeonsStaked() {
        require(dungeonRewardsContract.dungeonFirstStaked(msg.sender)!=0, "No dungeons staked");
        require(block.timestamp - dungeonRewardsContract.dungeonFirstStaked(msg.sender) > MIN_STAKING_PERIOD, 
                        "Dungeons have not been staked long enough to qualify");
        _;
    }
    ...
    function mintFree(uint256 quantity) external callerIsUser dungeonsStaked nonReentrant {
        uint256 numDungeonsOwned = dungeonRewardsContract.balanceOfDungeons(msg.sender); // store to save gas
        require((quantity + accountInfoList[msg.sender].freeMintedNFTs) <= numDungeonsOwned, "Exceeds number of dungeons owned--you can only mint as many dungeons as you have staked");
        // still have to check against max nft supply 
        require(totalSupply() + quantity < MAX_NFTS_PLUS_1, "Exceeds max NFT supply"); //use < rather than <= to save gas

        accountInfoList[msg.sender].freeMintedNFTs+=quantity;
        // ERC721a - _safeMint's second argument takes in a quantity, not a tokenId.
     ...
     */
    function getNFTsByAddress(address _owner)
        public
        view
