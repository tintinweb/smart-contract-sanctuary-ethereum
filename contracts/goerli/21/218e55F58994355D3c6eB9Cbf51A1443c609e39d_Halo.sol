...
contract Halo is ERC20 {
    ...
    uint256 constant private  _cap = 500_000_000*1e18;
    ...
    mapping(address=>uint256) private _balanceLocked;
    ...
    constructor() ERC20("HALO Token", "HALO") {
        ...
        _initTokenMint();
        _initDispatchLockedToken();
        _initDispatchLine();
    }

    function _initTokenMint() private {
        _mint(AddrBackup,CapBackup * decimals());
        _mint(AddrMarket,_marketMintInit * decimals());
    }

    function _initDispatchLockedToken() private {
        _balanceLocked[AddrInstPre] = CapInstPre * decimals();
        _balanceLocked[AddrInst] = CapInst * decimals();
        _balanceLocked[AddrInstA] = CapInstA * decimals();
        _balanceLocked[AddrFound] = CapFound * decimals();
        //_balanceLocked[AddrBackup] = 0;//already minted
        _balanceLocked[AddrTeam] = CapTeam * decimals();
        _balanceLocked[AddrEcosys] = CapEcosys * decimals();
        _balanceLocked[AddrMarket] = (CapMarket - _marketMintInit) * decimals();//already minted some
    }

    function _initDispatchLine() private {
        _dispatchLine[AddrInstPre] = DispatchLine(_tegTimestamp + SECONDS_PER_DAY*150,_tegTimestamp + SECONDS_PER_DAY*960,CapInstPre * decimals());
        _dispatchLine[AddrInst] = DispatchLine(_tegTimestamp + SECONDS_PER_DAY*60,_tegTimestamp + SECONDS_PER_DAY*870,CapInst * decimals());
        _dispatchLine[AddrInstA] = DispatchLine(_tegTimestamp + SECONDS_PER_DAY*60,_tegTimestamp + SECONDS_PER_DAY*870,CapInstPre * decimals());
        _dispatchLine[AddrFound] = DispatchLine(_tegTimestamp + SECONDS_PER_DAY*30,_tegTimestamp + SECONDS_PER_DAY*1800,CapInstPre * decimals());
        _dispatchLine[AddrBackup] = DispatchLine(0,0,0);//already minted
        _dispatchLine[AddrTeam] = DispatchLine(_tegTimestamp + SECONDS_PER_DAY*330,_tegTimestamp + SECONDS_PER_DAY*1140,CapInstPre * decimals());
        _dispatchLine[AddrEcosys] = DispatchLine(_tegTimestamp + SECONDS_PER_DAY*30,_tegTimestamp + SECONDS_PER_DAY*1800,CapInstPre * decimals());
        _dispatchLine[AddrMarket] = DispatchLine(_tegTimestamp + SECONDS_PER_DAY*30,_tegTimestamp + SECONDS_PER_DAY*1800,CapInstPre * decimals());
    }
    ...
    function balanceMintable(address _addr) public view returns(uint256){
        ...
        //market amount
        if(_addr == AddrMarket) {
            uint256 passDay = _now - _tegTimestamp < SECONDS_PER_DAY*12?(_now - _tegTimestamp)/12:12;
            mintable_ = _marketMintDaily * decimals() * passDay;
        }
    ...
    function _valueDispatchByLine(DispatchLine memory dl) private view  returns(uint256){
        ...
        return amount * decimals() * (ts - start)/(end - start);
    }
