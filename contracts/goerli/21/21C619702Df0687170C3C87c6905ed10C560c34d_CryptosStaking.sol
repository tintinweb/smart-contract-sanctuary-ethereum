...
import "../core/CryptosERC20Token.sol";
import "./SCryptos.sol";
...
/**
 * Mechancics of how a user can stake cryptos.
 */
contract CryptosStaking is Ownable {
    using ABDKMath64x64 for int128;
    
    IERC20 immutable public CRYPTOS; // The token being staked, for which CRYPTOS rewards are generated
    SCryptos immutable public SCRYPTOS; // Token used to redeem staked CRYPTOS
    ...
    constructor(
        ...
        // Each version of the staking contract needs it's own instance of SCryptos users can use to
        // claim back rewards
        SCRYPTOS = new SCryptos(); 
    ...
    /** Balance in CRYPTOS for a given amount of SCRYPTOS */
    function balance(uint256 amountSCryptos) public view returns(uint256) {
        return _overflowSafeMul1e18(
            ABDKMath64x64.divu(amountSCryptos, 1e18).mul(_accumulationFactorAt(currentEpoch()))
        );
    }
    ...
    function stakeFor(address _staker, uint256 _amountCryptos) public returns(uint256 amountSCryptos) {
        require(_amountCryptos > 0, "Cannot stake 0 tokens");

        _updateAccumulationFactor();

        // net past value/genesis value/OG Value for the cryptos you are putting in.
        amountSCryptos = _overflowSafeMul1e18(ABDKMath64x64.divu(_amountCryptos, 1e18).div(accumulationFactor));

        SafeERC20.safeTransferFrom(CRYPTOS, msg.sender, address(this), _amountCryptos);
        SCRYPTOS.mint(_staker, amountSCryptos);
        emit StakeCompleted(_staker, _amountCryptos, 0);

        return amountSCryptos;
    }

    /** Stake cryptos */
    function stake(uint256 _amountCryptos) external returns(uint256 amountSCryptos) {
        return stakeFor(msg.sender, _amountCryptos);
    }

    /** Unstake cryptos */
    function unstake(uint256 _amountSCryptos) external {      
        require(SCRYPTOS.allowance(msg.sender, address(this)) >= _amountSCryptos, 'Insufficient SCryptos allowance. Cannot unstake');

        _updateAccumulationFactor();
        uint256 unstakeBalanceCryptos = balance(_amountSCryptos);

        SCRYPTOS.burnFrom(msg.sender, _amountSCryptos);
        //Increased allowance from here
        SafeERC20.safeIncreaseAllowance(CRYPTOS, address(this), unstakeBalanceCryptos);
        //  Removed InstantExitQueue
        SafeERC20.safeTransferFrom(CRYPTOS, address(this), address(msg.sender), unstakeBalanceCryptos);

        emit UnstakeCompleted(msg.sender, _amountSCryptos);    
    }
...
contract CryptosERC20Token is ERC20, ERC20Burnable, Ownable, AccessControl {
    bytes32 public constant CAN_MINT = keccak256("CAN_MINT");

    constructor() ERC20("Cryptos", "CRYPTOS") {
 ...
 *
 * It mints and burns SCryptos as users stake/unstake
 */
contract SCryptos is ERC20, ERC20Burnable, Ownable {
    constructor() ERC20("SCryptos", "SCRYPTOS") {}
