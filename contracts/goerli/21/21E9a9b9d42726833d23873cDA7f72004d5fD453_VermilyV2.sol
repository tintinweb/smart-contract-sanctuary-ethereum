...
contract VermilyV2 is ERC721A, Ownable, ReentrancyGuard {
  ...
  constructor(string memory _baseURI, string memory _defaultURI)
    ERC721A("Vermily Yoga & Fitness", "VERMILY_V2")
  {
  ...
  modifier onlyAllowedQuantity(uint256 _num) {
    require(
      ...
      "Num must be less than maxSupply"
    );
    _;
  }
  ...
  modifier checkPrice(uint256 _num) {
    require(
      msg.value == publicMintPrice * _num,
      "Price  must eq publicMintPrice*num"
    );
    _;
  }

  event ItemCreated(uint256 tokenNumber, address owner, address seller);

  function setPrice(uint256 _publicMintPrice) external onlyOwner {
  ...
  function mint(address _addresss, uint256 _num) internal nonReentrant {
    ...
    emit ItemCreated(tokenCount, address(_addresss), msg.sender);
  }

  function preSale(address _addresss, uint256 _num)
    public
    payable
    onlyOwner
  ...
  function privateMint(address _addresss, uint256 _num)
    public
    payable
    onlyOwner
    onlyAllowedQuantity(_num)
  ...
  function publicMint(uint256 _num)
    public
    payable
    onlyAllowedQuantity(_num)
    checkPrice(_num)
...
contract ERC721A is IERC721A {
  ...
  function setAllowedMarketplace(address marketplace, bool allowed) public {
