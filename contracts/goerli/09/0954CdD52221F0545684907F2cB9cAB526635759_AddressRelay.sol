...
contract AddressRelay is IAddressRelay, Ownable {
    ...
    constructor() {
        ...
        // add all the other base facet types in here too
    }
     ...
     */
    function removeImplAddressAndAllSelectors(address _implAddress) external onlyOwner {
        for (uint256 i = 0; i < implAddresses.length; i++) {
            if (implAddresses[i] == _implAddress) {
                // this just sets the value to 0, but doesn't remove it from the array
     ...
     */
    function getImplAddress(bytes4 _functionSelector) external view returns (address implAddress_) {
     ...
     */
    function getAllImplAddressesAndSelectors() external view returns (Implementation[] memory impls_) {
     ...
     */
    function getSelectorsForImplAddress(address _implAddress) external view returns (bytes4[] memory selectors_) {
        uint256 selectorCount = 0;
        for (uint256 i = 0; i < selectors.length; i++) {
            if (selectorToImplAddress[selectors[i]] == _implAddress) {
                selectorCount++;
     ...
     */
    function updateSupportedInterfaces(bytes4 _interfaceId, bool _supported) external onlyOwner {
...
interface IAddressRelay {
     ...
     * @param _supported Whether the interface is supported or not
     */
    function updateSupportedInterfaces(bytes4 _interfaceId, bool _supported) external;
