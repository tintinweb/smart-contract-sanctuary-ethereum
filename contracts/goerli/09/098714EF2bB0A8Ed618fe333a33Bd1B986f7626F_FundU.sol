...
contract FundU is ReentrancyGuard, FundUtils {
    ...
    function resumeStream(uint256 id, bool paid) external onlyStreamOwner(id) {
        ...
        if (stream.status != StreamStatus.Completed) {
            ...
            emit ResumeStream(id, stream.owner, stream.beneficiary);
        }
    }
...
contract FundUtils {
    ...
    event PauseStream(uint256 indexed id, address indexed owner, address indexed beneficiary);

    event ResumeStream(uint256 indexed id, address indexed owner, address indexed beneficiary);
    ...
    event Withdraw(uint256 indexed id, address indexed owner, address indexed beneficiary, uint256 amount);
