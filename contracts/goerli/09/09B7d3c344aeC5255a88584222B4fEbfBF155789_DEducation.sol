...
contract DEducation is Ownable {
    ...
    function addMultiTranscript(StudentTranscript[] memory _studentTranscripts)
        public
    {
        for (uint256 i = 0; i < _studentTranscripts.length; i++) {
            StudentTranscript memory studentTranscript = _studentTranscripts[i];
            TranscriptUploaded[msg.sender][studentTranscript.StudentAddress][
                studentTranscript.HashCode
            ] = Transcript(
                studentTranscript.Semester,
