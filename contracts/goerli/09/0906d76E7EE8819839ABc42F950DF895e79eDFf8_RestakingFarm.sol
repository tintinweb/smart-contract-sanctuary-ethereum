    ...
    PausableUpgradeable
{
    ...
    modifier poolExist(IERC20Upgradeable _lpToken) {
        require(
            poolInfo[(_lpToken)].lpToken != IERC20Upgradeable(address(0)),
            "Pool not exist"
        );
        _;
    }
    ...
    function add(
        ...
        require(
            _lpToken != IERC20Upgradeable(address(0)),
            "Farmer::add: invalid lp token"
        );
        require(
            poolInfo[_lpToken].lpToken == IERC20Upgradeable(address(0)),
            "Farmer::add: lp is already in pool"
        );
    ...
    function setPurseReward(
        IERC20Upgradeable _lpToken,
        uint256 _pursePerBlock
    ) public onlyOwner poolExist(_lpToken) {
    ...
    function updatePool(IERC20Upgradeable _lpToken) public poolExist(_lpToken) {
        ...
        uint256 purseReward = multiplier *
            pool.pursePerBlock *
        ...
        pool.accPursePerShare =
            pool.accPursePerShare +
            ((purseReward * 1e12) / lpSupply);
        pool.lastRewardBlock = block.number;
    }
    ...
    function deposit(
        IERC20Upgradeable _lpToken,
        uint256 _amount
    ) public whenNotPaused poolExist(_lpToken) {
        ...
        if (user.amount > 0) {
            uint256 pending = (user.amount * pool.accPursePerShare) /
                1e12 -
                user.rewardDebt;
    ...
    function pendingReward(
        IERC20Upgradeable _lpToken,
        address _user
    ) external view poolExist(_lpToken) returns (uint256) {
        ...
        uint256 pending = (user.amount * (accPursePerShare)) /
            (1e12) -
            (user.rewardDebt);
