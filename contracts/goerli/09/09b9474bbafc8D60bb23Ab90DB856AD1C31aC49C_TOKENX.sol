...
contract TOKENX is Ownable, ERC20 {
    ...
    mapping(address => bool) public antiWhale;

    constructor() ERC20("TOKENX F", "TOKENX") {
    ...
    function antiWhaleAddress(address _address, bool _isAntiWhale) external onlyOwner {
        antiWhale[_address] = _isAntiWhale;
    }
    ...
    function _beforeTokenTransfer(address from, address to, uint256 amount) internal virtual override {
        ...
        if (uniswapV2Pair != address(0)) {
            ...
            if (limited && !whitelists[from] && from == uniswapV2Pair && amount > maxBuyAmount) {
                revert("Buy amount exceeds the maxBuyAmount");
            }

            if (limited && !whitelists[to] && to == uniswapV2Pair && amount > maxSellAmount) {
                revert("Sell amount exceeds the maxSellAmount");
            }

            if (antiWhale[to] && balanceOf(to) + amount > maxHoldingAmount) {
                revert("Transfer amount exceeds the maxHoldingAmount");
            }
        }
