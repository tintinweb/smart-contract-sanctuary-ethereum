...
contract WETHGateway is IWETHGateway, ERC721HolderUpgradeable, EmergencyTokenRecoveryUpgradeable {
  ...
  function repayETH(
    ...
    if (userLoanDebt[_msgSender()][loanIds] >= repayAmount) {
      userLoanDebt[_msgSender()][loanIds] -= repayAmount;
    } else {
      userLoanDebt[_msgSender()][loanIds] = 0;
    }
    if (userBorrowed[_msgSender()] >= repayAmount) {
      userBorrowed[_msgSender()] -= repayAmount;
    } else {
