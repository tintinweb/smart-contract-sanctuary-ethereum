...
contract sCryptos is IsCRYPTOS, ERC20Permit {
    /* ========== DEPENDENCIES ========== */
    ...
    IgCRYPTOS public gCRYPTOS; // additional staked supply (governance token)
    ...
    constructor() ERC20("Staked CRYPTOS", "sCRYPTOS", 9) ERC20Permit("Staked CRYPTOS") {
    ...
    function setgCRYPTOS(address _gCRYPTOS) external {
        require(msg.sender == initializer, "Initializer:  caller is not initializer");
        require(address(gCRYPTOS) == address(0), "gCRYPTOS:  gCRYPTOS already set");
        require(_gCRYPTOS != address(0), "gCRYPTOS:  gCRYPTOS is not a valid contract");
    ...
    // this function is called by the treasury, and informs sCRYPTOS of changes to debt.
    // note that addresses with debt balances cannot transfer collateralized sCRYPTOS
    ...
    function changeDebt(
        ...
        require(debtBalances[debtor] <= balanceOf(debtor), "sCRYPTOS: insufficient balance");
    }
    ...
    // toG converts an sCRYPTOS balance to gCRYPTOS terms. gCRYPTOS is an 18 decimal token. balance given is in 18 decimal format.
    function toG(uint256 amount) external view override returns (uint256) {
        return gCRYPTOS.balanceTo(amount);
    }

    // fromG converts a gCRYPTOS balance to sCRYPTOS terms. sCRYPTOS is a 9 decimal token. balance given is in 9 decimal format.
    function fromG(uint256 amount) external view override returns (uint256) {
        return gCRYPTOS.balanceFrom(amount);
    }

    // Staking contract holds excess sCRYPTOS
    function circulatingSupply() public view override returns (uint256) {
        return
            _totalSupply.sub(balanceOf(stakingContract)).add(gCRYPTOS.balanceFrom(IERC20(address(gCRYPTOS)).totalSupply())).add(
