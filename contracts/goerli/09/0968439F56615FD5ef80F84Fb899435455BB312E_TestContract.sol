...
contract TestContract is ERC721A, Ownable{
    ...
    function mint(uint256 _quantity) external payable callerIsUser{
        require(publicSale, "TEST: Public mint not yet Active!");
        require((totalSupply() + _quantity) <= MAX_SUPPLY, "TEST: Max supply reached!");
        require((totalPublicMint[msg.sender] +_quantity) <= MAX_PUBLIC_MINT, "TEST: Already minted 5 times!");
        require(msg.value >= (sale_price * _quantity), "TEST: Below mint price!");
    ...
    function voidListMint(bytes32[] memory _merkleProof, uint256 _quantity) external payable callerIsUser{
        require(voidListSale, "TEST: Voidlist mint not yet active!");
        require((totalSupply() + _quantity) <= MAX_SUPPLY, "TEST: Max supply reached!");
        require((totalVoidListMint[msg.sender] + _quantity)  <= MAX_VOIDLIST_MINT, "TEST: Cannot mint beyond voidList max mint!");
        require(msg.value >= (sale_price * _quantity), "TEST: Payment is below the mint price!");
        bytes32 sender = keccak256(abi.encodePacked(msg.sender));
        require(MerkleProof.verify(_merkleProof, merkleRoot, sender), "TEST: You are not void listed!");
    ...
    function chosenMint(bytes32[] memory _merkleProof, uint256 _quantity) external callerIsUser{
        require(chosenSale, "TEST: Chosen mint not yet active!");
        require((totalSupply() + _quantity) <= MAX_SUPPLY, "TEST: Cannot mint beyond max supply!");
        require((totalChosenMint[msg.sender] + _quantity)  <= MAX_CHOSEN_MINT, "TEST: Already minted 1!");
       
        bytes32 sender = keccak256(abi.encodePacked(msg.sender));
        require(MerkleProof.verify(_merkleProof, chosenMerkleRoot, sender), "TEST: You are not allowed to chosen mint!");
    ...
    function teamMint() external onlyOwner{
        ...
        // NEED TO CHANGE THIS TO MARKETING WALLET
        //****************************************************************** */
        //****************************************************************** */
        //****************************************************************** */
        //****************************************************************** */
