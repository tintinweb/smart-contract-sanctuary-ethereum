...
contract Metadata is Ownable {
  ...
  mapping(uint256 => CableData) public cables;
  uint256 totalCables;

  constructor() {}

  function addCables(CableData[] memory cables_) public {
    require(totalCables <= 545, "CABLE DATA LOADED");
    for (uint256 i = 0; i < cables_.length; i++) {
      cables[totalCables + 1] = cables_[i];
      totalCables += 1;
    }
  }
