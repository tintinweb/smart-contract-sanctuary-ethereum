...
contract Timith is ERC721A, Ownable, ReentrancyGuard, DefaultOperatorFilterer {
  ...
  // Only be used for tokens that were stolen
  ...
  modifier phaseLogicCheck(Phases _phase, bytes32[] calldata _merkleProof) {
    ...
    if (_phase == Phases.Whitelist || _phase == Phases.Waitlist) {
      bytes32 leaf = keccak256(abi.encodePacked(_msgSender()));
      require(MerkleProof.verify(_merkleProof, merkleRoot, leaf), "Invalid Whitelist proof!");
    }
    _;
  }
  modifier mintPriceCheck(TokenType _tokenType) {
    if (!freeWallets[_msgSender()]) {
  ...
  function setMerkleRoot(Phases _phase, bytes32 _merkleRoot) public onlyOwner {
    ...
    if (Phases.Whitelist == _phase || Phases.Waitlist == _phase) {
  ...
  function removeFreeWallets(address[] calldata _addresses) public onlyOwner {
    for (uint256 i = 0; i < _addresses.length; i++) {
      freeWallets[_addresses[i]] = false;
    }
  }

  function addFreeWallets(address[] calldata _addresses) public onlyOwner {
    for (uint256 i = 0; i < _addresses.length; i++) {
      freeWallets[_addresses[i]] = true;
    }
  }
  ...
  function renewToken(uint256 _tokenId) public payable callerIsUser validateRenewToken(_tokenId) validateRenewPrice {
    _renewToken(_tokenId);
  }
