...
contract Decoy is ERC721A, VRFV2WrapperConsumerBase, ConfirmedOwner {
    IERC721 mockContract = IERC721(0x19ebA460A0Db7418018E4453cA8beCa5E18372bc);

    uint256 private constant MAX_SUPPLY = 26;
    ...
    mapping(uint256 => bool) public claimedMockTokens;
    ...
    function freeMint(uint256 amount, uint256[] memory tokenIds) external {
        ...
        for (uint256 i = 0; i < tokenIds.length; i += 1) {
            if (mockContract.ownerOf(tokenIds[i]) != msg.sender) {
                revert WrongOwner(tokenIds[i]);
            }
            if (claimedMockTokens[tokenIds[i]]) {
        ...
        for (uint256 i = 0; i < tokenIds.length; i += 1) {
            claimedMockTokens[tokenIds[i]] = true;
        }
    }
