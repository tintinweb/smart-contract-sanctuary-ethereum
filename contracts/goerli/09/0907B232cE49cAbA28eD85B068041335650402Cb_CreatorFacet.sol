...
// import { platformFee, platformFeeRecipient, totalFeesCollected } from "./AdminFacet.sol";

contract CreatorFacet is Ownable, Pausable {
    /**
    ...
    uint256 public creatorsIDs = 0; // counter for creators' IDs
    uint256 public totalFundsCollected = 0; // all the money collected my Creators on the platform

    mapping(address => bool) public existingCreators; // a registry to check of a wallet is a Creator
    mapping(address => Creator) public creators; // a registry for Creators' addresses
    ...
    modifier notCreator() {
        ...
        require(
            ...
            "Not Creator: Creators are not allowed to do this for themselves"
        );
        _;
    }
    ...
    /**
     * @dev initiates Creator for a msg.sender
     *
     * @notice ONLY ONE Creator per wallet address
     */
    function initiateCreator() public returns (uint256) {
        require(!existingCreators[msg.sender], "Account already exists");
        Creator storage creator = creators[msg.sender];
        existingCreators[msg.sender] = true;
        creator.wallet = msg.sender;
        creator.rating = 0;
        creatorsIDs++;
        creator.id = creatorsIDs;
        emit CreatorInitialized(msg.sender, creator.id, block.timestamp);
        return creatorsIDs;
    }
