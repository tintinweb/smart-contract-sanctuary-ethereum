    ...
    DefaultOperatorFiltererUpgradeable
{
  ...
  //////////////////////////////////////////////////////////////*/
    ...
    function mint(
        ...
        if(_quantities.length != _proofs.length || _proofs.length != _allotments.length || _allotments.length != _allowlistIds.length || _allowlistIds.length != _config.length) revert MalformedArgs();
       MintCart memory cart;
        ...
        if(cart._quantity + minted[_msgSender()] > config.mintConfig.maxPerWallet) revert MaxPerWallet();
        minted[_msgSender()] += cart._quantity;
        if(nextTokenId + cart._quantity - 1 > config.mintConfig.maxSupply) revert MaxSupply();
    ...
    function revertOnConfigCheckFailure(
       ...
       if(mintedByConfig[_config[0]] + _quantity > configSupplyLimit[_config[0]]) revert ConfigurationSupply();
    }
