...
contract TheAIArtGame is Ownable, ERC2981, ERC721A, ReentrancyGuard {
    ...
    //     uint256 layout;
    // }
    bool private wordGeneration = false;
    bool private unhideWord = false;
    address private arcadeOwner;
    string public seed = "beforereveal";
    ...
    constructor()
        ERC721A("On chain AI NFT Game. ", "AI_ART_GAME", 10000, 10000)
    {
    ...
    modifier mustBeOpen(uint256 amount) {
        ...
        require(amount + currentIndex <= 100000, "Word generation is done.");
        _;
    }
        ...
        returns (uint256[6] memory)
    {
        ...
        uint256 randomNumber = randomSeed % 5;
        ...
        returns (uint256)
    {
        uint256[6] memory gladiators = battleArena(
            [uint256(0), 0, 0, 0, 0, 0],
            generateSeed(tokenId - (tokenId % 5))
        );

        for (uint256 i = 0; i < 5; i++) {
            if (gladiators[i] % 2 == 1 && generateSeed(tokenId) % 2 == 1) {
                uint8[3] memory newIndex = [6, 7, 8];
                gladiators[i] = newIndex[generateSeed(tokenId) % 3];
            }
            if (gladiators[i] == 2) {
                uint256 superRare = generateSeed(tokenId) % 30;
                if (superRare == 0) {
                    gladiators[i] = 9;
                }
                if (superRare == 1) {
        ...
        return gladiators[tokenId % 5];
    }

    function grabMyWord(uint256 tokenId) public view returns (string memory) {
        uint256 wordIndex = grabMyGladiatorPlace(tokenId);
        return
            wordlist[wordIndex - 1][
                generateSeed(tokenId) % wordlist[wordIndex - 1].length
            ];
    }

    function grabMyWordType(uint256 tokenId)
        public
        view
    ...
    function generateWords(
        ...
        nonReentrant
        mustBeOpen(amount)
        onlyValidAccess(_date, abi.encodePacked(amount, _max), _v, _r, _s)
    {
        if (maxMint[msg.sender] == 0) {
            maxMint[msg.sender] = _max + 1;
        }
        require(amount < maxMint[msg.sender], "Over mint amount.");
        ...
        returns (string memory)
    {
        ...
        string[3] memory metadataVars = [
            string(unhideWord ? grabMyWord(tokenId) : "Waiting on reveal..."),
            "svgwebsitehre",
            Strings.toString(tokenId)
        ];

        attributes[0] = Attributes("Word", grabMyWord(tokenId));
        attributes[1] = Attributes("Type", grabMyWordType(tokenId));
        attributesNumbers[0] = AttributesNumbers(
            "Daily Token Yield",
            "",
            generateNumber(generateSeed(tokenId), 10) + 1,
            10
        );
