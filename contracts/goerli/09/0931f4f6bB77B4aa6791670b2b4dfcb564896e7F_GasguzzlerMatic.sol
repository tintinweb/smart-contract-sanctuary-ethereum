...
contract GasguzzlerMatic is OwnableUpgradeable, ERC721EnumerableUpgradeable {


    function init () external initializer {
        __Ownable_init();
        __ERC721Enumerable_init();
        __ERC721_init("GasGuzzler","GG");
    }

    function mintTokens(address _toAccount, uint256 _amount) external  {
        for (uint256 i =0; i < _amount; i++) {
            _mint(_toAccount, totalSupply()+1);
        }
    }
