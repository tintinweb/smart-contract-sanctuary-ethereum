...
contract GachaDraw is Ownable {
        ...
        validActivity(_activityId)
    {
        ...
        if (activity.remainingRewardSupply < _ticketAmount) {
            revert InsufficientReward();
        }
        ...
        uint256 remainingSupplyRunning = activity.remainingRewardSupply;
        ...
        for (
            ...
            for (
                ...
                rewardTokenIndex < activity.rewardTokenIds.length;
                rewardTokenIndex = _uncheckedIncOne(rewardTokenIndex)
            ) {
                uint256 tokenAmountYetToBeMinted = activity.maximumRewardSupply[
                    rewardTokenIndex
                ] -
                ...
                if (tokenAmountYetToBeMinted > 0 && indexCount >= randomIndex) {
                    selectedTokenId = activity.rewardTokenIds[rewardTokenIndex];
                    activities[_activityId].totalGivenRewardSupply[
                            rewardTokenIndex
                        ] = _uncheckedIncOne(
                        activities[_activityId].totalGivenRewardSupply[
        ...
        activities[activity.activityId]
            .remainingRewardSupply = remainingSupplyRunning;
