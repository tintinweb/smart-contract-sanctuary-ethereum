...
contract ClaimBitmap {
     ...
     * @param tokenId the token id
     */
    function _setUsedForFreeClaim(uint256 tokenId) internal {
        uint256 claimedWordIndex = tokenId / 256;
        uint256 claimedBitIndex = tokenId % 256;
        claimedBitmap[claimedWordIndex] = claimedBitmap[claimedWordIndex] | (1 << claimedBitIndex);

        emit UsedForFreeClaimTokenId(tokenId);
    }
    ...
    Ownable
{
     ...
     */
    constructor(address payable shareholderAddress_, address contractAddress) ERC721A("y00tsDickButts", "YDB") {
    ...
    function claim(uint256 numberOfTokens) external payable {
        uint256[] memory tokenIds = availableIdsToClaimForFree(msg.sender);
     ...
     */
    function availableIdsToClaimForFree(address from) public view returns (uint256[] memory) {
