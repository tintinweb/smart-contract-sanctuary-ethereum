...
contract AdidasCapsule is ERC721, ERC721Enumerable, ERC2981, Ownable, DefaultOperatorFilterer {
    ...
    constructor(string memory __name, string memory __symbol, address _address) ERC721(__name, __symbol) {
        _name = __name;
        _symbol = __symbol;
        v1Contract = _address;
    }
    ...
    // Token Ownership
    function walletOfCapsuleOwner(address _owner) public view returns (uint256[] memory) {
        uint256 ownerTokenCount = ITMAirdrop(v1Contract).balanceOf(_owner);
        uint256[] memory ownedTokenIds = new uint256[](ownerTokenCount);
        uint256 currentTokenId = 0;
        uint256 ownedTokenIndex = 0;

        while (ownedTokenIndex < ownerTokenCount && currentTokenId <= ITMAirdrop(v1Contract).totalSupply()) {
