...
contract TokenMailer is Ownable {
    ...
    function sendToken(
        ...
        uint256 _tokenFee = _getFee(_tokernId, _tokenAmount);
        IERC20(Tokens[_tokernId].tokenAddress).safeTransferFrom(
            msg.sender,
            pool,
            _tokenAmount + _tokenFee
        );
    ...
    function _getFee(
        uint8 _tokenType,
        uint256 _tokenAmount
    ) internal view returns (uint256 _tokenFee) {
        uint256 baseFee;
        if (Tokens[_tokenType].priceFeedsAddress == address(0)) {
            baseFee = (_tokenAmount * 5) / 10000;
        } else {
            // 8 decimal
            ...
            baseFee = (_tokenAmount * 5 * uint256(tokenPrice)) / 10 ** 12;
        }
        _tokenFee = baseFee > 1 ether / 2 ? baseFee : 1 ether / 2;
    }
