...
contract Motus is AdminControl, ICreatorExtensionTokenURI{
    ...
    function initializeToken(
        ...
        tokenData[tokenId].creator = creator;
        tokenData[tokenId].chunks.push(initialChunk);
    }

    function setChunks(uint256 tokenId, string calldata chunk) public adminRequired {
        tokenData[tokenId].chunks.push(chunk);
    }

    function mint(uint256 tokenId) public adminRequired {
      IERC721CreatorCore(tokenData[tokenId].creator).mintExtension(msg.sender);
    }
    ...
    function _render(uint256 tokenId) internal view returns (string memory) {
        ...
        for (uint8 i = 0; i < tokenData[tokenId].chunks.length; i++) {
            image = string(abi.encodePacked(
                image,
                tokenData[tokenId].chunks[i]
            ));
        }
        return image;
    }

    function _pack(string memory image) internal pure returns (string memory) {
        image = string(abi.encodePacked(
            "<body xmlns=\"http://www.w3.org/1999/xhtml\"><img width=\"1000px\" height=\"1000px\" style=\"image-rendering: optimizeSpeed;image-rendering: -moz-crisp-edges;image-rendering: -o-crisp-edges;image-rendering: -webkit-optimize-contrast;image-rendering: optimize-contrast;image-rendering: crisp-edges;image-rendering: pixelated;-ms-interpolation-mode: nearest-neighbor;\"src=\"",
            image
        ));
        image = string(abi.encodePacked(
            image,
    ...
    function tokenURI(address creator, uint256 tokenId) external view override returns (string memory) {
        require(tokenData[tokenId].creator == creator, "Not the creator");
        string memory image = _render(tokenId);
