...
contract BlklavasEditions is ERC1155Supply, RevokableDefaultOperatorFilterer, Ownable, Pausable, ReentrancyGuard, ERC2981 {
  using Strings for uint256;
    ...
    /// @notice OFFCHAIN SIDE WHEELS FOR ADMIN PROVIDED (VIA WEBSITE)!!
    ...
    /// @notice Be mindfull of overrides (startingID of prev + DropAmount of prev)
    /// @dev provide offchain sidewheels for admin
    ...
    /// The tokenUri is prefixed by the baseUri and appended by the suffi
    ...
    /// (hence when no token specific uri is set)
    ...
    function _supplyCheck(uint256 mintAmount, uint256 tokenID, uint256 _maxSupply_) internal view {
        //!! underflow risk is mitigated: _maxSupply is set and over totalSupply (!) at this point in code
