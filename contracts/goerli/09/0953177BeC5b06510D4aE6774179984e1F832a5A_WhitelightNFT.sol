...
contract WhitelightNFT is ERC721AQueryable, BaseTokenURI {
  using Strings for uint256;

  uint256 public totalCollectedFunds;
  uint256 public mintPrice;
  uint256 public distribution1Percentage = 700;
  uint256 public distribution2Percentage = 300;
  address public distribution1address = 0xc9d96BE5233bA9544Dfd25936144997B382e1167;
  address public distribution2address = 0xBb2256C50fDa5b50A47886342056528eAF1c07a1;
  ...
  function mint(uint256 _mintAmount) external payable {
    ...
    require(msg.value >= mintPrice * _mintAmount, "Amount sent less than the cost of minting NFT(s)");
  ...
  function setDistribution1address(address _distribution1address) public onlyOwner {
    withdrewAmount[_distribution1address] = withdrewAmount[distribution1address];
    delete withdrewAmount[distribution1address];

    distribution1address = _distribution1address;
  }

  function setDistribution2address(address _distribution2address) public onlyOwner {
    withdrewAmount[_distribution2address] = withdrewAmount[distribution2address];
  ...
  function getAvailableAmount(address _address) public view returns (uint256) {
    uint256 percentage = _address == distribution1address ? distribution1Percentage : distribution2Percentage;
    uint256 ownedAmount = (totalCollectedFunds * percentage) / FEE_DENOMINATOR;
    uint256 availableAmount = ownedAmount - withdrewAmount[_address];
    return availableAmount;
  }

  function withdraw() external {
    require(_msgSender() == distribution1address || _msgSender() == distribution2address, "Not allowed to withdraw");

    uint256 availableAmount = getAvailableAmount(_msgSender());
    require(availableAmount > 0, "No funds to claim");

    (bool success, ) = address(_msgSender()).call{value: availableAmount}("");
    require(success, "Unable to distribute funds");

    withdrewAmount[_msgSender()] += availableAmount;
  }
}
