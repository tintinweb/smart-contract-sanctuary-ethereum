...
contract Pool is IPool {
  ...
  address public A_TOKEN_ADDRESS = address(0);
  ...
  function supply(
    ...
    IERC20(asset).safeTransferFrom(msg.sender, A_TOKEN_ADDRESS, amount);

    bool isFirstSupply = IAToken(A_TOKEN_ADDRESS).mint(
      msg.sender,
      onBehalfOf,
      amount,
      1 // we maintain the index at 1 as AAVE aToken will not increase as there is no borrowing
    );
  }
  ...
  function withdraw(
    ...
    uint256 userBalance = IAToken(A_TOKEN_ADDRESS)
    ...
    IAToken(A_TOKEN_ADDRESS).burn(msg.sender, to, amountToWithdraw, 1);
