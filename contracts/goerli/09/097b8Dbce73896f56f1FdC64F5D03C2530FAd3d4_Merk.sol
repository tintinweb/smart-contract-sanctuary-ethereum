    ...
    Ownable
{
    ...
    function merkWL(bytes32[] calldata _merkleProof, address add, uint8 _table)public view returns(bool){
        bytes32 leaf = sha256(abi.encodePacked(add));
    ...
    function signerOwner(bytes memory _signature) public view returns (address) {
		return ECDSA.recover(sha256(abi.encode(msg.sender)), _signature);
	}

    function signer() private view returns (bytes32) {
		return sha256(abi.encode(msg.sender));
	}
