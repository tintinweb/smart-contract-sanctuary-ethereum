...
contract NFTBridge is NFTBridgeGovernance {
    ...
    // 订单编号,token地址,ID,接收链ID，发送着地址，接收者地址
    ...
    // 订单编号,源链token地址，新token地址，tokenID，原始NFT链的ID，NFT接收人
    event ReceiveNFT(uint64 indexed sequence,address sourceToken, address token,uint256 tokenID, uint16 tokenChain,address onwer);
    ...
    function _completeTransfer(bytes memory proofBlob, bytes32[] memory merkleProof) internal {
        ...
        emit ReceiveNFT(vm.sequence,address(uint160(uint256(transfer.tokenAddress))),address(transferToken),transfer.tokenID,transfer.tokenChain,transferRecipient);
    }
