...
contract TestFarmContract is Ownable {
   ...
   struct DepositInfo {
    ...
    function claim(uint256 _pid) public validatePoolByPid(_pid) {
        ...
        uint256 actualReward = (deservedReward.add(IERC20Contract(rewardToken).totalSupply())).div(lpTokenAccuracy.div(rewardTokenAccuracy)).sub(IERC20Contract(rewardToken).totalSupply());
