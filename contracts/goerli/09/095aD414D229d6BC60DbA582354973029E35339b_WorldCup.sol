...
contract WorldCup {
    ...
    // 参赛球队
    string[] public countries = ["GERMANY", "FRANCE", "ENGLAND", "BRAZIL", "KOREA"];
    // 所有玩家：<第几期, <玩家地址，玩家>>
    ...
    constructor(uint256 _deadline) {
        admin = msg.sender;
        require(_deadline > block.timestamp, "WorldCup Lottery: invalid deadline!");
        deadline = _deadline;
    }
    ...
    function play(Country _selected) payable external {
        // 参数校验
        require(msg.value == 1 gwei, "invalid play funds!");
    ...
    function finialize(Country _country) onlyAdmin external{
        ...
        // 遍历所有winners, 计算winner奖金
        for (uint i = 0; i < winners.length; i++) {
            address currWinner = winners[i];

            // 获取每个地址应该得到的份额，<第几期, <玩家地址，玩家>>
            Player storage winner = players[currRound][currWinner];
            if(winner.isSet){
                console.log("this winner has been set already, will be skiped!");
                continue;
            }
            ...
            console.log("curr-winner reward amt: ", amt, "curr-winner total amt: ", winnerVaults[currWinner]);
        }
        // 
    ...
    function claimReward() external {
        // 获取winner的奖金
        uint256 reward = winnerVaults[msg.sender];
        require(reward > 0, "nothing reward to claim!");
        // 更新 winnerVaults
        winnerVaults[msg.sender] = 0;
        //
        lockedAmts -= reward;
        // 发放奖金
        (bool successed, ) = msg.sender.call{value: reward}("");
        require(successed, "claim reward failed!");

        console.log("winner: ", msg.sender,  "claim reward: ", reward);
