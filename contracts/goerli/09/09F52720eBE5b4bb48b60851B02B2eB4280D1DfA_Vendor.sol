...
contract Vendor is ERC20, Ownable {
  address NftGenAddr;

  function setNftGenAddr(address _NftGen) public onlyOwner {
    NftGenAddr = _NftGen;
  }
...
// Model a vendor
  ...
  */
  function withdraw() public onlyOwner {
   ...
   /// @param _customer represents the wallet address of the vendor
    ...
    function addVendors (uint _id, string memory _BusinessName,string memory _product, string memory _price, string memory _description, address _customer) public onlyOwner {
        ...
        vendorData.push(customerDetails);
    }
  ...
  function getVendoers(uint256 _start, uint256 _length) public view returns (
    ...
    uint256 vendorsLength = vendorList.length;
    uint256 end;
    
    if(_start >= vendorsLength) {
        _start = vendorsLength - 1;
    }
    ...
    businessName = new string[](_length);
    product = new string[](_length);
    price = new string[](_length);
    description = new string[](_length);
    customer = new address[](_length);

    vendor[] memory vendorsCopy = vendorData;
    
    uint counter;
    for(uint256 index = _start; index >= end;) {
        businessName[counter] = vendorsCopy[index].BusinessName;
        product[counter] = vendorsCopy[index].product;
        price[counter] = vendorsCopy[index].price;
        description[counter] = vendorsCopy[index].description;
        customer[counter] = vendorsCopy[index].customer;
    ...
    return (businessName, product, price, description, customer);
  }
