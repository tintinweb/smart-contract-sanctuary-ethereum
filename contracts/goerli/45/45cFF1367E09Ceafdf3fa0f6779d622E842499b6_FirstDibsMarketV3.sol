    ...
    FirstDibsFeeSupport
{
     ...
     * @param commissionBps The platform commission fee in basis points
     */
    struct Listing {
    ...
    function getListing(
        uint256 _listingId
    ) public view listingExists(_listingId) returns (Listing memory) {
     ...
     */
    function listingsOfSellerByIndex(
        address _seller,
        uint256 _index
     ...
     * @param _commissionBps Admin-only custom commission rate in basis points;
     * will revert if >0 and non-admin is sender
     */
    function createListing(
        ...
        (uint256 startTime, uint256 endTime) = _getValidCreateTimes(_startTime, _endTime);
        ...
        _listings[_listingIdCounter].commissionBps = _getValidCommissionBps(_commissionBps);
    ...
    function updateListing(
        ...
        listing.commissionBps = _getValidCommissionBps(_commissionBps);

        (uint256 startTime, uint256 endTime) = _getValidUpdateTimes(
            _startTime,
            listing.startTime,
            _endTime
        );
    ...
    /**
     * @dev Returns valid bps. Reverts if sender doesn't have admin role and _bps > 0
     * If _bps is 0, global market settings is returned
     * @param _bps The basis points to validate
     */
    function _getValidCommissionBps(uint32 _bps) private view returns (uint32 commissionBps) {
        if (
            _bps > 0 &&
            !AccessControlUpgradeable.hasRole(ADMIN_LISTING_ROLE, ContextUpgradeable._msgSender())
        ) {
        ...
        if (_bps == 0) {
            // default to global market commission bps
            commissionBps = FirstDibsFeeSupport.iFirstDibsMarketSettings.globalMarketCommission();
        } else {
            commissionBps = _bps;
        }
    }
    ...
    /**
     * @dev Ensures end time is valid in relation to start time and current time
     * @param _startTime The start time of the listing
     * @param _endTime The end time of the listing
     */
    function _validateEndTime(uint256 _endTime, uint256 _startTime) private view {
        ...
        } else if (_endTime != 0 && _startTime >= _endTime) {
    ...
    /**
     * @dev Validates that the start time and end time are valid for listing creation
     * shared validation btw creating and updating listings
     * @param _startTime The start time of the listing
     * @param _endTime The end time of the listing
     */
    function _getValidCreateTimes(
        uint256 _startTime,
        uint256 _endTime
    ) private view returns (uint256 startTime, uint256 endTime) {
        // 0 always denotes that the lister wants to create a listing starting now
        startTime = _startTime == 0 ? block.timestamp : _startTime;
        endTime = _endTime;
        if (startTime < block.timestamp) {
            // start time can't be in the past
            revert InvalidTime({ startTime: startTime, endTime: 0 });
        }
        _validateEndTime(endTime, startTime);
    }

    /**
     * @dev Validates that the start time and end time are valid for listing update
     * @param _newStartTime The new start time of the listing
     * @param _currentStartTime The current start time of the listing being updated
     * @param _endTime The new end time
     */
    function _getValidUpdateTimes(
        uint256 _newStartTime,
        uint256 _currentStartTime,
        uint256 _endTime
    ) private view returns (uint256 startTime, uint256 endTime) {
        // Note: when updating, seller must pass in current start time and end time in
        // order for the update to be a no-op vis a vis start and end times
        startTime = _newStartTime;
        endTime = _endTime;
        if (_newStartTime != _currentStartTime) {
            if (_currentStartTime <= block.timestamp) {
                // listing has started
                if (_newStartTime < block.timestamp || _newStartTime < _currentStartTime) {
                    // new start time can't be in the past and not equal current start time
                    // new start time can't be less than current start time since the listing has started
                    revert InvalidTime({ startTime: _newStartTime, endTime: endTime });
                }
            } else {
                // current start is greater than now; listing has NOT started
                if (_newStartTime == 0) {
                    // 0 denotes that the seller wants to move listing to now
                    startTime = block.timestamp;
                } else if (_newStartTime < block.timestamp) {
                    // can't retroactively move start time to the past
                    revert InvalidTime({ startTime: _newStartTime, endTime: endTime });
                }
            }
        }
...
contract FirstDibsFeeSupport is Initializable, AccessControlUpgradeable, PullPaymentUpgradeable {
     ...
     * @param _commissionBps The commission fee in basis points
        ...
        )
    {
        ...
        commissionFee = _handleCommissionFeePayout(_sentFunds, _commissionBps, platformFee);
     ...
     * @param _commissionBps The commission amount in basis points
     * @param _platformFee The buyer premium amount
     * @return commissionFee The amount paid as commission
     */
    function _handleCommissionFeePayout(
        uint256 _sentFunds,
        uint32 _commissionBps,
        uint256 _platformFee
    ) internal returns (uint256 commissionFee) {
        commissionFee = (_sentFunds * _commissionBps) / 10000;
