...
contract NftMultiSender is Ownable {
    ...
    function sendToken721(
        address _nft, /* nft address */
        address[] calldata _recipients, /* recipients address list */
        ...
        IERC721 token = IERC721(_nft);
        address sender = msg.sender;
        for (uint256 i = 0; i < _recipients.length; i++) {
            token.safeTransferFrom(sender, _recipients[i], _tokenIds[i], "");
        }
    }

    function sendToken1155(
        address _nft, /* nft address */
        uint256 _tokenId, /* token id */
        uint256 _amount, /* amount for each address */
        address[] calldata _recipients, /* recipient address list */
        uint64 _ts,
        bytes calldata _signature
    ) external {
        require(isSigValid(_ts, _signature), "signature invalid");
        IERC1155 token = IERC1155(_nft);
        address sender = msg.sender;
        for (uint256 i = 0; i < _recipients.length; i++) {
