...
// Librarires
import "./libraries/LegacySharedStructs.sol";
...
contract LegacyPartyFactory is UUPSUpgradeable, OwnableUpgradeable {
     ...
     * @param _partyBeacon Party Beacon address
     ...
     * @param _pfi_fee Party fee creation
     */
    function initialize(
        address _partyBeacon,
        address _sentinel,
        address _collector,
        uint256 _fee,
        address _pfi,
        ...
        require(_partyBeacon != address(0), "Beacon is zero address");
        require(_sentinel != address(0), "Sentinel is zero address");
        require(_collector != address(0), "Collector is zero address");
        ...
        setPfi(_pfi);
        setPfiFee(_pfi_fee);
        partyBeacon = _partyBeacon;
    }

    /**
     * @dev Set platform sentinel address
    ...
    function setPlatformSentinel(address _sentinel) public onlyOwner {
        require(_sentinel != address(0), "Platform is zero address");
        PLATFORM_SENTINEL = _sentinel;
    }

    /**
     * @dev Set platform fee collector address
    ...
    function setPlatformCollector(address _platform) public onlyOwner {
        require(_platform != address(0), "Platform is zero address");
    ...
    function createParty(
        LegacySharedStructs.PartyInfo memory partyInfo,
        ...
        BeaconProxy party = new BeaconProxy(
            ...
            abi.encodeWithSignature(
                "initialize(address,(string,string,string,string,string,bool,uint256,uint256),string,uint256,address,address,address)",
                msg.sender,
                partyInfo,
                tokenSymbol,
                initialDeposit,
        ...
        IERC20(denominationAsset).transfer(PLATFORM_ADDRESS, fee);
        IERC20(denominationAsset).transfer(address(party), initialDeposit);

        // Add created Party to PartyFactory
        parties[address(party)] = true;
    ...
    {
        return (PLATFORM_ADDRESS, PLATFORM_SENTINEL, address(this));
    }
