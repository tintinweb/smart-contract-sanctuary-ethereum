...
contract TokenStaking is Ownable, ReentrancyGuard {
    ...
    function deposit(uint256 amount) external nonReentrant {
        ...
        stakingMetaList[msg.sender] = stakingMeta(
            initialYield,
            amount,
            amount.mul(initialYield).div(DEMOMINATOR),
            block.timestamp,
            block.timestamp
        );
    ...
    function updateYield(address _address, uint256 _yield) external onlyOwner {
        ...
        uint256 elapsedPeriod = (block.timestamp.sub(meta.startDepositedAt)).div(ONE_DAY_UNIXTIME);
        uint256 totalReward = elapsedPeriod.div(depositPeriod).mul(meta.yield).mul(meta.depositAmount).div(DEMOMINATOR);
    ...
    function getTotalReward() external view returns(uint256) {
        ...
        uint256 elapsedPeriod = (block.timestamp.sub(meta.startDepositedAt)).div(ONE_DAY_UNIXTIME);
        if (elapsedPeriod > depositPeriod) {
            elapsedPeriod = depositPeriod;
        }
        elapsedPeriod = block.timestamp.sub(meta.yieldUpdatedAt);
        return meta.totalReward.add(
            elapsedPeriod.div(depositPeriod).mul(meta.yield).mul(meta.depositAmount).div(DEMOMINATOR)
        );
    }
