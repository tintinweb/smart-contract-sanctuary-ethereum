...
library LiquidatorLib {
   ...
   */
  function uniV3RemoveLiquidityAndCollection(UniV3POSInfo memory params)
    external
  {
...
contract LiquidatorConfig is Ownable {
   ...
   */
  function setPool(IPool _pool) public onlyOwner {
    pool = _pool;
  }

  /**
   * @dev update `flashLoanPool` address
   */
  function setFlashLoanPool(IFlashLoanPool _flashLoanPool) public onlyOwner {
    flashLoanPool = _flashLoanPool;
  }
}
  ...
  IERC1271
{
  ...
  event LiquidateERC721(
    ...
    uint256 indexed id,
    bytes fullParamsBytes
  );

  constructor(LiquidatorConfig config) LiquidatorBase(config) {
    IERC20 weth = IERC20(address(CONFIG.WETH()));
    weth.safeApprove(address(CONFIG.pool()), type(uint256).max);
    weth.safeApprove(address(CONFIG.flashLoanPool()), type(uint256).max);
  }
  ...
  function liquidateERC721(
    ...
    LiquidateCallParams memory liquidateParams = abi.decode(
      _liquidateParamsBytes,
      (LiquidateCallParams)
    );

    _flashLoanAndExecute(liquidateParams, _fullParamsBytes);
  ...
  function _flashLoanAndExecute(
    ...
    assets[0] = address(CONFIG.WETH());

    // the amount to be flashed for each asset
    uint256[] memory amounts = new uint256[](1);
    amounts[0] = _liquidateParams.liquidationAmount;
    ...
    // 2. call callback
    // 3. repay money
    CONFIG.flashLoanPool().flashLoan(
  ...
  function executeOperation(
    ...
    } else if (marketType == MarketType.NFTX) {
      _executeOperationForNFTX(params_, amounts, premiums);
    } else if (marketType == MarketType.SUDO) {
      _executeOperationForSudo(params_, amounts, premiums);
    } else {}
  ...
  function _executeOperationForSeaportBasic(
    ...
    LiquidatorLib.fulfillBasicOrder(
      params.order,
      params.market,
      params.nftApprover
    );
  ...
  function _executeOperationForSeaportAdvanced(
    ...
    LiquidatorLib.fulfillAdvancedOrder(
      params.order,
      params.market,
      params.nftApprover
    );
  ...
  function _executeOperationForLooksRare(
    ...
    LiquidatorLib.matchBidWithTakerAsk(
      params.takerAsk,
      params.makerBid,
      params.market,
      params.nftApprover
  ...
  function _executeOperationForParaspace(
    ...
    LiquidatorLib.acceptBidWithCredit(
    ...
    _afterSellNFT(
      ...
      _amounts,
      _premiums,
      params.acceptBidInfo.credit.token
    );
  }
  ...
  function _executeOperationForCryptoPunks(
    ...
    LiquidatorLib.acceptBidForPunk(
      params.liquidateParams.collateralAsset,
      params.bid
    );
  ...
  function _executeOperationForUniV3POS(
    ...
    LiquidatorLib.uniV3RemoveLiquidityAndCollection(params.info);
    ...
    _afterSellNFT(
      ...
      _amounts,
      _premiums,
      params.info.offerToken
    );
  }

  /**
   * @dev flashloan callback for NFTX type
   */
  function _executeOperationForNFTX(
    bytes memory _paramsBytes,
    uint256[] memory _amounts,
    uint256[] memory _premiums
  ) internal {
    (, NFTXLiquidateERC721Params memory params) = abi.decode(
      _paramsBytes,
      (uint256, NFTXLiquidateERC721Params)
    );
    _beforeSellNFT(params.liquidateParams, params.flashLoanParams, _amounts);

    LiquidatorLib.nftxMintAndSell721(
      params.quote,
      params.liquidateParams.collateralAsset,
      params.market
    );
    ...
    _afterSellNFT(
      ...
      _amounts,
      _premiums,
      params.quote.buyToken
    );
  }

  /**
   * @dev flashloan callback for SUDO type
   */
  function _executeOperationForSudo(
    bytes memory _paramsBytes,
    uint256[] memory _amounts,
    uint256[] memory _premiums
  ) internal {
    (, SudoLiquidateERC721Params memory params) = abi.decode(
      _paramsBytes,
      (uint256, SudoLiquidateERC721Params)
    );
    _beforeSellNFT(params.liquidateParams, params.flashLoanParams, _amounts);

    LiquidatorLib.sudoSwapNFTsForToken(
      params.quote,
      params.liquidateParams.collateralAsset,
      params.market
    );
    ...
    _afterSellNFT(
      ...
      _amounts,
      _premiums,
      params.quote.outToken
    );
  }

  /**
   * @dev execute liquidate NFT
   */
  function _beforeSellNFT(
    LiquidateCallParams memory _liquidateParams,
    FlashLoanParams memory,
    uint256[] memory
  ) internal {
    LiquidatorLib._safeERC20Approve(
      address(CONFIG.WETH()),
      address(CONFIG.pool()),
      _liquidateParams.liquidationAmount
    );

    CONFIG.pool().liquidateERC721(
    ...
    if (_liquidateParams.collateralAsset == address(CONFIG.MOONBIRDS())) {
      _unnestMoonBird(_liquidateParams.tokenId);
    }
  }
  ...
  function _afterSellNFT(
    LiquidateCallParams memory,
    FlashLoanParams memory _flashLoanParams,
    uint256[] memory amounts,
    uint256[] memory premiums,
    address _offerToken
  ) internal {
    address weth = address(CONFIG.WETH());
    uint256 repayAmount = amounts[0] + premiums[0];
    uint256 flashBalBefore = IERC20(weth).balanceOf(address(this));

    if (flashBalBefore < repayAmount) {
      if (_offerToken != weth) {
        uint256 amountNeeded = repayAmount - flashBalBefore;

        LiquidatorLib._swapTokensForExactTokens(
          _offerToken,
          IERC20(_offerToken).balanceOf(address(this)),
          amountNeeded,
          _flashLoanParams.offerToFlashLoan
        );
      }
    }

    uint256 flashBal = IERC20(weth).balanceOf(address(this));
    if (flashBal < repayAmount) {
      revert CannotRepayFlashLoan(repayAmount, flashBal);
    } else {
      _transferToAdmin(weth, flashBal - repayAmount);
    }

    if (_offerToken != weth) {
      _transferToAdmin(_offerToken);
    }

    // Approve the LendingPool contract allowance to *pull* the owed amount
    LiquidatorLib._safeERC20Approve(
      weth,
      address(CONFIG.flashLoanPool()),
      repayAmount
    );
  }
  ...
  /**
   * @dev if collateral asset is moonbird and the nft is nesting, we need un-nest this nft before sell it.
   */
  function _unnestMoonBird(uint256 tokenId) internal {
    IMoonBird moonBird = CONFIG.MOONBIRDS();
    (bool nesting, , ) = moonBird.nestingPeriod(tokenId);
    if (nesting) {
      uint256[] memory tokenIds = new uint256[](1);
      tokenIds[0] = tokenId;
      moonBird.toggleNesting(tokenIds);
    }
  }
