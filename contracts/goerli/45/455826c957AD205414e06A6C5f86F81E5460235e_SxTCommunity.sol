...
contract SxTCommunity is ISxTCommunity, SxTCommunityStorage, ERC1155, Ownable, ReentrancyGuard, Pausable {
    ...
    // function updateTokenDetails(uint256 id, string memory newTokenURI, uint256 maxNewTokenSupply) external override onlyOwner {
    //     require(!compareStrings(tokenDetails[id].tokenUri, ""), "SxTCommunity: URI nonexistent token");
    //     require(maxNewTokenSupply > 0, "SxTCommunity: Maximum supply cannot be 0");
    //     require(!compareStrings(newTokenURI, ""), "SxTCommunity: URI cannot be empty string");
 
    //     Token storage currentToken = tokenDetails[id];        
    //     currentToken.tokenUri = newTokenURI;
    //     currentToken.maxTokenSupply = maxNewTokenSupply;

    //     emit TokenDetailsUpdated(id, newTokenURI, maxNewTokenSupply);
    // }
    ...
    function addNewNFTs(string [] memory newTokenURIs, uint256 [] memory maxNewTokenSupplies) external override onlyOwner {
        require(newTokenURIs.length == maxNewTokenSupplies.length , "SxTCommunity: Array lengths should be same");
    ...
    function airDrop(uint256 id, address[] memory recipients, uint256[] memory quantities) external override onlyOwner nonReentrant{
        require(recipients.length == quantities.length, "SxTCommunity: Incorrect parameter length");
        require(!compareStrings(tokenDetails[id].tokenUri, ""), "SxTCommunity: URI nonexistent token");
        Token storage currentToken = tokenDetails[id];
        for (uint8 index = 0; index < recipients.length; index++) {
            require(recipients[index] != ZERO_ADDRESS, "SxTCommunity: Recepient address cannot be zero address");
            require(!checkMaxSupply(id, quantities[index]), "SxTCommunity: Total supply exceeded maximum supply");
            currentToken.currentTokenSupply += quantities[index];
            _mint(recipients[index], id, quantities[index], "");
            emit NftMinted(id, recipients[index]);
        }
    }
    ...
    function airDropDefaultAmount(uint256 id, address[] memory recipients) external override onlyOwner nonReentrant{
        require(!compareStrings(tokenDetails[id].tokenUri, ""), "SxTCommunity: URI nonexistent token");

        Token storage currentToken = tokenDetails[id];
        for (uint8 index = 0; index < recipients.length; index++) {
            require(recipients[index] != ZERO_ADDRESS, "SxTCommunity: Recepient address cannot be zero address");
            require(!checkMaxSupply(id, DEFAULT_AIRDROP_AMOUNT), "SxTCommunity: Total supply exceeded maximum supply");
            currentToken.currentTokenSupply += DEFAULT_AIRDROP_AMOUNT;
    ...
    function checkMaxSupply(uint256 id, uint256 quantity) view internal returns(bool){
        if((tokenDetails[id].currentTokenSupply + quantity) <= tokenDetails[id].maxTokenSupply)
            return false;
        return true;
    }
    ...
    function withdrawERC20(IERC20 erc20Token) external override onlyOwner nonReentrant {
        uint256 amount = erc20Token.balanceOf(address(this));
        address to = msg.sender;
        require(amount > 0, "SxTCommunity: Zero ERC20 token balance");
        bool sent = erc20Token.transfer(to, amount);
...
interface ISxTCommunity {
    ...
    // event TokenDetailsUpdated(uint256 id, string tokenURI, uint256 maxTokenSupply);
    ...
    function addNewNFTs(string [] memory newTokenURIs, uint256 [] memory maxNewTokenSupplies) external;
    ...
    function airDrop(uint256 id, address [] memory recipients, uint256 [] memory quantities) external;
    ...
    function airDropDefaultAmount(uint256 id, address [] memory recipients) external;
