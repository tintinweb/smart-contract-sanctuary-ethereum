...
contract TxSupplyMapping {
    ...
    function updateTxBalanceAndSupply(address holder, uint txTokenAmount, uint newTxSupplyValue) public {
        ...
        if (txSupply[holder].length == 0) {
            ...
            delete txSupply[holder][0]; // Delete existing array pointer and reinstantiate for gas refund
            txSupply[holder].push(TxSupplyWeight(totalTxBalance, weightedAvgTxSupply));          
        }
    }

    function updateTxBalanceOnSale(address holder, uint txTokenAmount) public {
        uint currentTxBalance = txSupply[holder][0].txBalance;
        uint currentTxSupply = txSupply[holder][0].txSupplyValue;
        uint adjustedTxBalance = currentTxBalance - txTokenAmount > 0 ? currentTxBalance - txTokenAmount : 0;
        
        if (adjustedTxBalance == 0) {
            delete txSupply[holder][0]; // Delete existing array txSupply[holder] for gas refund instead of setting to 0
        } else {
            delete txSupply[holder][0]; // Delete existing array pointer and reinstantiate for gas refund
            txSupply[holder].push(TxSupplyWeight(adjustedTxBalance, currentTxSupply));
        }    
    }
        ...
        if (txSupply[to].length == 0) {
            ...
            //require (transferTokenAmount == txBalance);
                if (transferTokenAmount >= txBalance) { // Could be > if transferTokenAmount includes extra token balance accumulated without purchasing
                    txSupply[to].push(TxSupplyWeight(transferTokenAmount, txSupplyValue)); 
            ...
            uint txBalance = txSupply[from][0].txBalance;
            uint currentTxBalance = txSupply[to][0].txBalance;
            uint currentTxSupply = txSupply[to][0].txSupplyValue;
            uint totalTxBalance = currentTxBalance + (transferTokenAmount);
            //require (transferTokenAmount == currentTxBalance);
                if (transferTokenAmount >= txBalance) { // Could be > if transferTokenAmount includes extra token balance accumulated without purchasing
                    uint weightedCurrentTxSupply = currentTxSupply.mul(currentTxBalance.div(totalTxBalance));
                    uint weightedNewTxSupply = currentTxSupply.mul(transferTokenAmount.div(totalTxBalance));
                    uint weightedAvgTxSupply = weightedCurrentTxSupply + (weightedNewTxSupply);
                    
                    delete txSupply[from]; // Delete existing array pointer as it is obsolete in this instance, get gas refund
                    delete txSupply[to]; // Delete existing array pointer and reinstantiate for gas refund
                    txSupply[to].push(TxSupplyWeight(totalTxBalance, weightedAvgTxSupply));

                } else if (transferTokenAmount < currentTxBalance) {                    
                    uint totalTxBalanceFrom = txBalance - transferTokenAmount;
                    txSupply[from][0].txBalance = totalTxBalanceFrom;
...
contract VTXPair is IVTXPair, VTXERC20LP, TxSupplyMapping {
    ...
    function swap(uint amount0Out, uint amount1Out, address to, bytes calldata data) external lock {
        ...
        if (amount0In > 0 && amount1Out > 0) {updateTxBalanceAndSupply(to, amount1Out, _reserve1); IVTXERC20(_token1).VTXMint(address(this), amount1Out*2);} // mint after BUY
        if (amount1In > 0 && amount0Out > 0) {updateTxBalanceOnSale(tx.origin, amount1In); IVTXERC20(_token1).VTXBurn(address(this), amount1In*2);} // burn after SELL
        ...
        //require(balance0Adjusted.mul(balance1Adjusted) >= uint(_reserve0).mul(_reserve1).mul(10000**2), 'VTX: K');
        }
