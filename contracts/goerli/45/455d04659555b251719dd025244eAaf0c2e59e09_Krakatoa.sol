...
contract Krakatoa is Ownable, Additional {
    ...
    mapping(uint64 => Order) public _orders;
    ...
    function _checkRequest(Request calldata req, bytes calldata signature) private view returns (uint) {
        ...
        require(length <= 255, "KR: Maximum is 255 products for order");
    ...
    function directSell(Request calldata req, bytes calldata signature) external payable {
        ...
        _orders[req.orderId] = Order(total, fees, 0, req.buyer, address(0), 0, Status.Withdrawn);
    ...
    function p2cSell(Request calldata req, bytes calldata signature) external payable {
        ...
        _orders[req.orderId] = Order(req.prices[0] - fee, fee, req.nonces[0], req.buyer, req.sellers[0], req.products[0], Status.Pending);
    ...
    function getOrder(uint64 orderId) external view returns (uint256, uint256, uint64, Status) {
        Order memory r = _orders[orderId];
        return (r.price, r.fee, r.product, r.status);
    }


    function _refundOrder(uint64 orderId) internal {
    ...
    function _withdrawOrder(uint64 orderId) internal {
