...
contract UniforgeCollection is ERC721Enumerable, Ownable {
    ...
    // Transfers ownership to the client right at deployment and declare all the variables
    constructor(
        address _owner,
        string memory _name,
    ...
    // Checks if the mint amount is at least the mintFee and that the amount of tokens is correct, then proceeds to mint the token
    function mintNft(uint256 _mintAmount) public payable {
        // Negative number giver out of bound error
    ...
    // Allows owner to mint freely without constrains for marketing / strategy purposes
    function mintForAddress(uint256 _mintAmount, address _receiver) public payable onlyOwner {
        _mintLoop(_receiver, _mintAmount);
    }

    // Edit the baseURI allowing owner to have a reveal party or upgrade the metadata
    function setBaseURI(string memory _baseURI) public onlyOwner {
        s_baseURI = _baseURI;
    }

    // Edit the max mint amount per transaction to manage a more fair mint (is per transaction and not per address)
    function setMaxMintAmountPerTx(uint256 _maxMintAmount) public onlyOwner {
        s_maxMintAmount = _maxMintAmount;
    }

    // Edit the starting block timestamp of the public sale
    function setStartSale(uint256 _startSale) public onlyOwner {
        s_startSale = _startSale;
    }

    // Owner can withdraw 100% of the profits from the sale stored in the contract
    ...
    // Procedure to mint n tokens safely
    ...
    // Returns the mint price for the public sale
    function mintFee() public view returns (uint256) {
        return i_mintFee;
    }

    // Returns the max mint amount per transaction
    function maxMintAmount() public view returns (uint256) {
        return s_maxMintAmount;
    }

    // Returns the timestamp of the sale's start
    function startSale() public view returns (uint256) {
        return s_startSale;
    }

    // Returns the baseURI of the collection
    function baseURI() public view returns (string memory) {
        return s_baseURI;
    }

    // Returns the specific URI for a given token
    // TokenURI ranges from 1 to maxSupply (if minted)
...
error UniforgeDeployerV0__NeedMoreETHSent();
error UniforgeDeployerV0__TransferFailed();
...
@notice visit uniforge.io
*/
contract UniforgeDeployerV0 is Ownable {
    // State variables
    ...
    // Deployer becomes owner of this contract
    constructor() {
        transferOwnership(msg.sender);
    }

    // Caller deploys a NFT collection
    function deployUniforgeCollection(
        ...
        if (msg.value < s_deployFee) {
            revert UniforgeDeployerV0__NeedMoreETHSent();
        }
    ...
    // Edit the fee for calling deployUniforgeERC712
    function setDeployFee(uint256 _amount) public onlyOwner {
        s_deployFee = _amount;
    }

    // Owner withdraws the ether from the contract
    function withdraw() public onlyOwner {
        uint256 amount = address(this).balance;
        (bool success, ) = payable(msg.sender).call{value: amount}("");
        if (!success) {
            revert UniforgeDeployerV0__TransferFailed();
        }
    }

    // Returns the number of contracts created from this contract
    function getDeployments() public view returns (uint256) {
        return s_deployedCollections.length;
    }

    // Returns the addres of a specific deployed contract
    function getDeployment(uint256 index) public view returns (address) {
        return s_deployedCollections[index];
    }

    // Returns the deployment fee
