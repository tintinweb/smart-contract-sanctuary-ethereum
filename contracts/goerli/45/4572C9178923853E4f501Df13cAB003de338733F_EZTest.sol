...
contract EZTest is ERC721A, ERC2981, Ownable {
...
TokenInfo[] public AllowedCrypto;
    ...
    string public baseURI = "https://ipfs.io/ipfs/QmeSjSinHpPnmXmspMjwiXyN6zS4E9zccariGR3jxcaWtq/";
      ...
      constructor(uint96 _royaltyFeesInBips) ERC721A("EZTest", "WHOP") {
    ...
    function mint(uint256 quantity, uint256 _pid) public payable {
        TokenInfo storage tokens = AllowedCrypto[_pid];
        IERC20 paytoken;
        uint256 cost;
        cost = tokens.costvalue;
        paytoken = tokens.paytoken;
        require(quantity + _numberMinted(msg.sender) <= MAX_MINTS, "you tried to mint too many");
        require(totalSupply() + quantity <= MAX_SUPPLY, "sold out, I think");
        require(msg.value >= (cost * quantity), "pls pay moar");
                require(!paused);

        _safeMint(msg.sender, quantity);
    }

      function withdrawMoney(uint256 _pid) public payable onlyOwner () {
          TokenInfo storage tokens = AllowedCrypto[_pid];
          IERC20 paytoken;
            paytoken = tokens.paytoken;
