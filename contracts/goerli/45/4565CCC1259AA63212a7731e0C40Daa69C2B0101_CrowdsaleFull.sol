...
import "@amxx/hre/contracts/Balances.sol";
...
contract CrowdsaleFull is ERC20Buyable, ERC20Vesting {
    ...
    constructor(
        ...
        SwapToUSDC(uniswapRouter_, USDC_)
        ERC20Buyable(rate_)
    ...
    function deposit(
        ...
        returns (uint256)
    {
        uint256 amount = super.deposit(receiver_, amountIn_, minimumTokenOut_, path_);
        require(amount >= buyInMinimum, "Minimum buy-in not reached");
        return amount;
    }
...
abstract contract ERC20Buyable is ERC20, Ownable, SwapToUSDC {
    ...
    function deposit(address receiver_, uint256 amountIn_, uint256 amountOutMinimum_, bytes memory path_) public payable virtual returns (uint256) {
        uint256 received = _swap(owner(), amountIn_, amountOutMinimum_, path_);
...
import "../../math/CommonFunctions.sol";
...
abstract contract SwapToUSDC {
    ...
    constructor(ISwapRouter uniswapRouter_, IERC20 USDC_) {
        uniswapRouter   = uniswapRouter_;
        USDC            = USDC_;
        WETH            = IWETH9(IPeripheryImmutableState(address(uniswapRouter_)).WETH9());
    }

    function _swap(address receiver_, uint256 amountIn_, uint256 amountOutMinimum_, bytes memory path_) internal returns (uint256) {
        if (msg.value > 0) {
        ...
        if (path_.length == 0) {
            ...
            return uniswapRouter.exactInput{ value: msg.value }(ISwapRouter.ExactInputParams({
                path:              path_,
                recipient:         receiver_,
