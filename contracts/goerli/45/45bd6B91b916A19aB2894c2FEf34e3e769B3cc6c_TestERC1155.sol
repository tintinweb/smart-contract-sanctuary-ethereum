...
contract TestERC1155 is ERC1155, Ownable {
    ...
    uint256 public MAX_PUBLIC_SALE_AMOUNT = 10;
    ...
    constructor(string memory initialBaseURI, address _paperKeyManagerAddress)
        ...
        )
    {
        ...
        require(
            (isFree &&
                balanceOf(toAddress, id) + amount > MAX_PRE_SALE_FREE_AMOUNT) ||
                (!isFree &&
                    balanceOf(toAddress, id) + amount >
                    MAX_PRE_SALE_DISCOUNT_AMOUNT),
    ...
    function checkPreSaleClaimEligibility(
        ...
        if (
            ...
            return "Max token amount for pre-sale exceeded";
        }
        ...
        returns (string memory)
    {
        ...
        if (balanceOf(toAddress, id) + amount > MAX_PUBLIC_SALE_AMOUNT) {
            return "Max token amount for public-sale exceeded";
        }
        ...
        payable
    {
        ...
        uint8 id = _publicSaleTokenId();
        require(
            balanceOf(toAddress, id) + amount > MAX_PUBLIC_SALE_AMOUNT,
            "Max token amount for public-sale exceeded"
        );
