...
interface IExpansion is IRoyalty {
     ...
     */
    function preparePack(Pack calldata pack) external;
     ...
     */
    function updateOperatorFilterAndRegister(address newRegistry) external;
...
abstract contract AbstractStorefront is IStorefront, AccessControlEnumerableUpgradeable, UUPSUpgradeable {
     ...
     * @param purchaserType Purchaser type
     */
...
contract RandomExpansionStorefront is AbstractStorefront {
    ...
    /**
     * @notice Data containing information on how to calculate a purchase's expected gas cost, that a fulfiller needs
     * @param operationBaseCost Upper bound on base cost of an entire mint
     * @param perPurchaseBaseCost Upper bound on base cost of each purchase
     * @param perItemMintGasCost Upper bound on cost to mint each item in a single purchase
     */
    struct FulfillmentGasConstants {
        uint32 operationBaseCost;
        uint32 perPurchaseBaseCost;
        uint32 perItemMintGasCost;
    }
    ...
    /**
     * @notice Emitted when a user is refunded because their purchase request cannot be executed
     * @param purchaseId Purchase ID
     */
    event RefundedPurchaseRequest(uint256 purchaseId);
    ...
    function requestFreeAllocatedPacks(uint256 saleId, uint32 mintQuantity, address nftRecipient) external payable {
        ...
        if (purchaserType == PurchaserType.artist) {
            ...
            require(msg.value >= _getGasSurcharge(sale, mintQuantity), "$ < fulfilling gas");
    ...
    function requestPurchaseExpansionPacks(
        ...
        _validatePurchaseTimeAndProcessQuantity(
            sale,
            saleId,
            purchaseQuantity,
            presaleWhitelistedQuantity,
            proof,
            purchaserType
        );
        uint64 numMintRequests = _packIdToOutstandingMintRequests[sale.packId];
        ...
        _validateAndProcessPurchasePaymentRandom(sale, purchaseQuantity, purchaserType);
    ...
    function executePurchaseExpansionPacks(
        ...
        if (tokenIdCombinations.length == 1) {
                ...
                )
            {
                ...
                _payout(requestingPurchaser, sale.erc20Token, purchaseQuantity * sale.price);
                emit RefundedPurchaseRequest(purchaseId);
            }
        } else {
            try
     ...
     */
    function getGasSurcharge(uint256 saleId, uint256 purchaseQuantity) external view returns (uint256) {
        return _getGasSurcharge(_sales[saleId], purchaseQuantity);
    }
     ...
     * @param purchaseQuantity The number of packs of the sale that the user wants to purchase
     */
    function _getGasSurcharge(Sale memory sale, uint256 purchaseQuantity) internal view returns (uint256) {
        ...
        uint256 gasCost = gasConstants.operationBaseCost +
            (gasConstants.perPurchaseBaseCost + numItems * gasConstants.perItemMintGasCost) *
            purchaseQuantity;
    ...
    /**
     * @notice Validate purchase and process payment
     * @param sale Sale
     * @param purchaseQuantity How many times the sale is being purchased in a process
     * @param purchaserType Purchaser type
     */
    function _validateAndProcessPurchasePaymentRandom(
        ...
        // Calculate gas cost
        uint256 gasCost = _getGasSurcharge(sale, purchaseQuantity);

        // Require valid payment
        if (sale.erc20Token == address(0)) {
            // The txn must come with a full ETH payment
            if (purchaserType == PurchaserType.general) {
                require(msg.value >= purchaseQuantity * sale.price + gasCost, "$ < expected");
            } else {
                // only gas required if artist or platform is minting
                require(msg.value >= gasCost, "$ < expected");
            }
            ...
            require(msg.value >= gasCost, "no gas");
            ...
            // use msg.value instead of gasCost, in case a little extra is sent
            /* solhint-disable avoid-low-level-calls */
            (bool success, ) = platformPurchaseFulfiller.call{ value: msg.value }("");
            /* solhint-enable avoid-low-level-calls */
            require(success, "fulfiller payment failed");
        }
    }
