...
// File contracts/v1/ERC721MultiCollection.sol
...
abstract contract ERC721MultiCollection is ERC721, IERC721MultiCollection {
    ...
    modifier onlyValidCollectionId(uint256 _collecionId) {
        require(
            _collecionId < _nextCollectionId,
    ...
    */
    function _addCollection() internal returns (uint256){
        require(!_newCollectionsForbidden, "Adding new collections is forbidden");
     ...
     * @notice Get the collection ID for a given token ID
     */
    function tokenIdToCollectionId(uint256 _tokenId) public pure returns (uint256 collectionId) {
        return _tokenId / ONE_MILLION;
    }

    /**
     * @notice Burns all tokens in a collection for user `owner`.
     *
     ...
     */
    function _burn(address owner, uint256 collectionId) internal returns (uint256 tokensBurnt) {
        uint256 balance = _collectionBalances[owner][collectionId];

        for (uint256 i = 0; i < balance; i++) {
            uint256 tokenId = _collectionOwnedTokens[owner][collectionId][0]; // Always burn token at index 0, as it will be swapped with the last token in the array
            _burn(tokenId);
        }
...
// File contracts/v1/governance/PRTCLVotes.sol
 ...
 * @author Particle Collection - valdi.eth
 */
abstract contract PRTCLVotes is IPRTCLVotes, Context, EIP712 {
     ...
     */
    function _getTotalSupply(uint256 collectionId) internal view virtual returns (uint256) {
    ...
    /**
     * @dev Alternative way to adding voting units.
     * More granular than `_transferVotingUnits`. Can be used to support more complex voting unit logic.
     */
    function _addVotes(address to, uint256 collectionId, uint256 amount) internal {
        _collectionTotalCheckpoints[collectionId].push(_add, amount);
        (uint256 oldValue, uint256 newValue) = _collectionDelegateCheckpoints[collectionId][delegates(to, collectionId)].push(_add, amount);
        emit DelegateVotesChanged(to, oldValue, newValue, collectionId);
    }

    /**
     * @dev Alternative way to removing voting units.
     * More granular than `_transferVotingUnits`. Can be used to support more complex voting unit logic.
     */
    function _removeVotes(address from, uint256 collectionId, uint256 amount) internal {
        _collectionTotalCheckpoints[collectionId].push(_subtract, amount);
        (uint256 oldValue, uint256 newValue) = _collectionDelegateCheckpoints[collectionId][delegates(from, collectionId)].push(_subtract, amount);
        emit DelegateVotesChanged(from, oldValue, newValue, collectionId);
    }
...
// File contracts/v1/governance/PRTCLCoreERC721Votes.sol
...
abstract contract PRTCLCoreERC721Votes is ERC721MultiCollection, PRTCLVotes {
    ...
    function _afterTokenTransfer(
        ...
        uint256 collectionId = tokenIdToCollectionId(firstTokenId);
        if (from != address(0) && balanceOf(from, collectionId) == 0) {
            // Removes one vote unit from total supply as `from` has just lost their last token
            // Removes vote from it's delegate
            _removeVotes(from, collectionId, 1);
        }
        if (to != address(0) && balanceOf(to, collectionId) == 1) {
            // Add one vote unit to total supply as `to` has just received their first token
            // Adds vote to it's delegate
            _addVotes(to, collectionId, 1);
        }
...
// File contracts/v1/PRTCLCollections721V1.sol
    ...
    IPRTCLCollections721V1
{
    ...
    struct Collection {
        ...
        // Block number, set to 10 blocks after last particle mint, to prevent speculation when setting seeds.
        uint256 setSeedsAfterBlock;
    ...
    constructor(
        string memory _bURI
    ) ERC721("Particle", "PRTCL") EIP712("Particle", "1") {
    ...
    /**
     * @notice Proceeds per token for a sale of `_priceInWei`.
     */
    function proceedsPerToken(uint256 _collectionId, uint256 _salePriceInWei) onlyValidCollectionId(_collectionId) external view returns (uint256) {
        return _salePriceInWei / collections[_collectionId].maxParticles;
    }
        ...
        )
    {
        ...
        // BPS should first be dividid by 100 to get the percentage, then by 100 to get the value in wei
        FJMRevenue_ =
    ...
    function getCoordinate(uint256 _tokenId) external view returns (uint256) {
        ...
        // p1 and p2 are primes in the 7-10000 range, guaranteeing no collisions
        // Using another prime to shift, to avoid 0 or 9999 always being the first element
        uint256 p1 = collection.seeds[0];
        uint256 p2 = collection.seeds[1];
     ...
     * @dev token price stored on minter contract.
     */
    function addCollection(
        ...
        returns (uint256)
    {
        require(_newCollectionsAllowed(), "New collections forbidden");
        require(_numberOfParticles > 0 && _numberOfParticles <= ONE_MILLION, "Number of particles must be > 0 && <= 1M");
        uint256 collectionId = _addCollection();

        collectionIdToFinancials[collectionId].artistAddress = _artistAddress;
        collectionIdToFinancials[collectionId].artistRoyaltyBPS = 500;
        collectionIdToFinancials[collectionId].FJMRoyaltyBPS = 250;
        collectionIdToFinancials[collectionId].DAORoyaltyBPS = 250;

        // Primary sales revenue split
        collectionIdToFinancials[collectionId].artistPrimaryBPS = 0;
        collectionIdToFinancials[collectionId].FJMPrimaryBPS = MAX_BPS;
        collectionIdToFinancials[collectionId].DAOPrimaryBPS = 0;
    ...
    function updateCollectionData(
        ...
        onlyNonEmptyString(_collectionName)
    {
        require(_maxParticles > 0, "Number of max particles has to be > 0");
    ...
    {
        // 10_000 BPS = 100%
        // require the sum of all royalties to be below MAX_ROYALTY_BPS
        require(_artistRoyaltyBPS + _FJMRoyaltyBPS + _DAORoyaltyBPS <= MAX_BPS, "Royalties cannot exceed 100%");
    ...
    {
        // 10_000 BPS = 100%
        // require the sum of all primary splits to be MAX_BPS
        require(_artistPrimaryBPS + _FJMPrimaryBPS + _DAOPrimaryBPS == MAX_BPS, "Primary split must equal 100%");
    ...
    function setCollectionSeeds(
        ...
        require(collection.seeds.length == 0, "Token hash already set");
    ...
    function requestCollectionSeeds(
        ...
        require(block.number > allowedBlock, "Too early to set seeds");
     ...
     * @param _FJMAddress Address of new Dependency
     * @param _DAOAddress Address of new Dependency
     * Registry.
     */
    ...
    function mint(
        ...
        uint24 newNParticles;
        unchecked {
            // nParticles guaranteed <= maxParticles <= 1_000_000,
            // 1_000_000 << max uint24, so no possible overflow
            newNParticles = oldNParticles + _amount;
        }
        ...
        unchecked {
            ...
            newTokenId = (_collectionId * ONE_MILLION) + oldNParticles;
        }

        if (newNParticles == maxParticles) {
            // Allow setCollectionSeeds to be called 10 blocks after the last mint, 
            // by a contract Admin (essentially revealing the final coordinates and metadata for each token)
            collection.setSeedsAfterBlock = block.number + 10;
     ...
     */
    function burn(address owner, uint256 collectionId)
        public
        onlyRole(GOVERNOR_ROLE)
        onlyValidCollectionId(collectionId)
        returns (uint256 tokensBurnt)
    {
        return _burn(owner, collectionId);
    }
    ...
    function _afterTokenTransfer(
        ...
        uint256 tokenId,
        uint256 batchSize
    ) internal override(ERC721, PRTCLCoreERC721Votes) {
