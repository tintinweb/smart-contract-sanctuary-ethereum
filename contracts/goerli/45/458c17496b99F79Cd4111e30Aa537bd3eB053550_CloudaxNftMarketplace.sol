...
import "@openzeppelin/[email protected]/utils/StringsUpgradeable.sol";
import "@openzeppelin/[email protected]/utils/math/SafeMathUpgradeable.sol";
...
contract CloudaxNftMarketplace is Initializable, ERC721Upgradeable, ReentrancyGuardUpgradeable, ERC721URIStorageUpgradeable, OwnableUpgradeable, UUPSUpgradeable {
    
    using SafeMathUpgradeable for uint256;
    using CountersUpgradeable for CountersUpgradeable.Counter;
    CountersUpgradeable.Counter private s_listedTokenId;
    // Counters.Counter private s_totalToken;
    CountersUpgradeable.Counter private s_tokenSold;
    CountersUpgradeable.Counter private s_itemID;
    string private s_contractBaseURI;

    // % times 200 = 2%
    uint256 private s_serviceFee = 200;
    ...
    mapping(uint256 => string) public s_itemIdToItemId;
    ...
    function initialize() initializer public {
        __ERC721_init("Cloudax Marketplace", "CLDX");
        __Ownable_init();
    ...
    modifier isListed(address nftAddress, uint256 tokenId) {
        ListedItem memory listing = s_listedItems[nftAddress][tokenId];
        if (listing.price <= 0) {
    ...
    function _createItem(
        ...
        s_itemID.increment();
        uint256 newItemId = s_itemID.current();
        s_itemIdToItemId[newItemId] = itemId;
        s_itemIdDBToItemId[itemId] = newItemId;
        emit itemIdPaired(itemId, newItemId);

        s_listedItems[address(this)][newItemId] = ListedItem({
        ...
        s_idToItemStatus[newItemId] = TokenStatus.ONSELL;
    ...
    function buyItemCopy(
        ...
        uint256 itemID = s_itemID.current();
        
        ListedItem memory listings = s_listedItems[address(this)][itemID];
        uint256 price = listings.price;
        uint32 quantity = listings.quantity;
        uint256 numSold = listings.numSold;
        if (listings.itemId != id || quantity <= 0 || itemID <= 0)  {
            _createItem(
                itemId,
                _fundingRecipient,
                _price,
        ...
        if (quantity <= numSold) {
            s_idToItemStatus[itemID] = TokenStatus.SOLDOUT;
        ...
        listing.numSold++;
        s_listedTokenId.increment();
        uint256 newTokenId = s_listedTokenId.current();
        ...
        string memory _tokenBaseURI = string(
            abi.encodePacked(_tokenBaseURI, "/", StringsUpgradeable.toString(newTokenId))
        );

        //Deduct the service fee
        uint256 serviceFee = msg.value.mul(s_serviceFee).div(10000);
        // uint256 r = _royaltyBPS/100;
        uint256 royalty = msg.value.mul(_royaltyBPS/100);
        ...
        s_depositedForItem[itemID] += finalAmount;
        
        s_platformEarning += serviceFee;
        s_indivivdualEarningInPlatform[listing.fundingRecipient] += finalAmount;
        s_totalAmount += msg.value;

        // Send funds to the funding recipient.
        _sendFunds(listing.fundingRecipient, finalAmount);

        // Mint a new copy or token from the Item for the buyer, using the `newTokenId`.
        safeMint(msg.sender, newTokenId, _tokenBaseURI);
        // s_totalToken.increment();
        // Store the mapping of the ID a sold item's copy or token id to the Item being purchased.
        s_tokenToListedItem[newTokenId] = itemID;

        s_idToItemStatus[newTokenId] = TokenStatus.ACTIVE;
        s_tokenSold.increment();

        s_soldItems[address(this)][newTokenId] = SoldItem({
            nftAddress: address(this),
            tokenId: newTokenId,
            itemId: itemID,
            buyer: payable(msg.sender),
            creator: listing.fundingRecipient,
        ...
        emit itemIdPaired(itemId, itemID);
    }
    ...
    function listToken(
        ...
        s_listedItems[nftAddress][tokenId] = ListedItem({
        ...
        emit TokenListed(nftAddress, tokenId, 0, msg.sender, price, block.timestamp);
    }

    function updateListing(address nftAddress, uint256 tokenId, uint96 newPrice)
        external
    ...
    function cancelListing(address nftAddress, uint256 tokenId)
        ...
        isListed(nftAddress, tokenId) 
    {
        delete (s_listedItems[nftAddress][tokenId]);
        ...
        isListed(nftAddress, tokenId)
    {
        ...
        // transfer nft to buyer
         ERC721Upgradeable(nftAddress).safeTransferFrom(creator, msg.sender, tokenId);
