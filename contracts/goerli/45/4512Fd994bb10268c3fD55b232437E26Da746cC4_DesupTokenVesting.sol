...
contract DesupTokenVesting is Ownable, ReentrancyGuard {
    ...
    IUniswapRouterV2 internal uniswapRouter =
    ...
    mapping(string => VestingSchedule) public vesting_schedules;

    mapping(address => Holder) private holders;

    address[] private holder_lists;
    ...
    constructor(address token_) {
        ...
        // TODO Think about this master, whether it is neccessary
        master = msg.sender;
    }
    ...
    function invest() public payable {
        ...
        // TODO convert msg.value to desup token amount and add to holder.total_amount

        Holder storage holder = holders[msg.sender];
        address[] memory path = new address[](2);
        path[0] = 0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2;
        path[1] = 0xdAC17F958D2ee523a2206206994597C13D831ec7;
        holder.total_amount += uniswapRouter.getAmountsOut(msg.value, path)[1];
    ...
    function withdraw(uint256 amount) public {
        ...
        uint256 available_amount = getAllowedAmount(msg.sender);

        require(amount <= available_amount, "Withdraw limitation");

        _token.safeTransfer(msg.sender, amount);

        Holder storage holder = holders[msg.sender];
        holder.withdrawed_amount += amount;
    }
    ...
    function getVestingSchedule(string memory role)
        public
        view
    ...
    function getTotalAmount(address holder_address)
        ...
        returns (uint256)
    {
        Holder memory holder = holders[holder_address];
        return holder.total_amount;
    }
        ...
        returns (uint256)
    {
        ...
        Holder memory holder = holders[holder_address];
        VestingSchedule memory holder_vesting_schedule = vesting_schedules[
            holder.role
        ];
        uint256 passed_time = block.timestamp - published_date;
        if (passed_time >= holder_vesting_schedule.duration) {
            return holder.total_amount;
        }

        uint256 allowed_percentage = holder_vesting_schedule.init_percentage +
            ((100 - holder_vesting_schedule.init_percentage) * passed_time) /
            holder_vesting_schedule.duration;
        uint256 allowed_amount = (holder.total_amount * allowed_percentage) /
            100;
        return allowed_amount;
    }

    function getAvailableAmount(address holder_address)
        public
        view
        returns (uint256)
    {
        ...
        Holder memory holder = holders[holder_address];
        return allowed_amount - holder.withdrawed_amount;
    }
    ...
    function vsDefined(address holder_address) private view returns (bool) {
        string memory role = holders[holder_address].role;
        if (
            vesting_schedules[role].duration != 0 ||
            vesting_schedules[role].init_percentage != 0
        ) {
            return true;
        }
        return false;
