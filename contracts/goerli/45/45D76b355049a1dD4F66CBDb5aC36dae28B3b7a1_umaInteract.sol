...
contract umaInteract {
    ...
    struct DAOdashboard{
        bool isValidated;
        uint16 limDeployments;
        uint256 defaultReward;


    }

    event ProposalUpdated(address DAOaddress, string IPFSHash, State State,bytes data);
    event DaoRegistered(address DAOAddress,uint16 limDeployments,uint256 defaultReward);
    event DeploymentSubmitted(address DAOAddress,string IPSFHash);
    mapping(string => Deployment) public propDetails;
    mapping(string=> Proposal) public lastDeploymentProposal;
    mapping(address=>DAOdashboard) public dashboards;

    uint256 requestTime = 0; // Store the request time so we can re-use it later.
    function registerDAO(address DAOaddress,uint16 limit, uint256 payment)public {
...
// Fetch the resolved price from the Optimistic Oracle that was settled.
    ...
    function updatePropState(string memory IPFSHash) public {
        ...
        emit ProposalUpdated(propDetails[IPFSHash].DAOaddress,IPFSHash, newState,lastDeploymentProposal[IPFSHash].data);
