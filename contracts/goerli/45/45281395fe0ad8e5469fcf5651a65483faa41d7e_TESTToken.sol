...
contract TESTToken is ERC20, Ownable {
    ...
    address private  burnWallet = address(0xdead);
    ...
    constructor() ERC20("Test Enjin", "TEST") {
    ...
    require(totalSupply() + amount <= 4097000000000000 * 10 **18);
        _mint(to, amount);
    }
    ...
    function setMaxTx (uint256 amount) external onlyOwner {
        require (amount >= (totalSupply().mul(1).div(409700)),"No rug pull!");
        maxTxAmount = amount;
    }

    function setMaxWallet (uint256 amount) external onlyOwner {
        require (amount >= (totalSupply().mul(1).div(409700)));
        maxWallet = amount;
    }
