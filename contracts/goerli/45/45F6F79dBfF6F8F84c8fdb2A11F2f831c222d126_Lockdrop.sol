...
import "./extensions/TokenRecoveryUpgradeable.sol";

import "./ILockdrop.sol";

/**
 * @notice Implementation of the {ILockdrop} interface.
 */
contract Lockdrop is ILockdrop, TokenRecoveryUpgradeable, GuardableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    function initialize(
        ...
        require(_asset != address(0), 'Lockdrop: INVALID ASSET');
        require(_withdrawTo != address(0), 'Lockdrop: INVALID WITHDRAW TO');
     ...
     * @inheritdoc ILockdrop
     */
    function setWithdrawTo(address account) external override onlyMaster {
        _setWithdrawTo(account);
    }

    /**
     * @notice Checks if Lockdrop inherits a given contract interface.
     ...
     * @inheritdoc ILockdrop
     */
    function updateDateConfig(DateConfig calldata newConfig) external override onlyAdmin {
        _updateDateConfig(newConfig);
    }
    ...
    function updatePool(
        ...
        _pools[_poolId].enabled = _enabled;
        _pools[_poolId].requiredPrivileges = _requiredPrivileges;

        emit PoolUpdated(_poolId, _requiredPrivileges, _enabled, _msgSender());
    }
     ...
     * @inheritdoc ILockdrop
     */
    function moveDelegation(bytes32 fromPoolId, bytes32 toPoolId, uint256 amount) external override whenNotPaused nonReentrant {
        Pool storage fromPool = _pools[fromPoolId];
        Pool storage toPool = _pools[toPoolId];
        DateConfig memory dates = _dateConfig;

        address sender = _msgSender();
        ISomaGuard guard = ISomaGuard(SOMA.guard());
        ...
        require(guard.check(sender, toPool.requiredPrivileges), 'Lockdrop: NO PRIVILEGES');
        require(guard.check(sender, fromPool.requiredPrivileges), 'Lockdrop: NO PRIVILEGES');
    ...
    function delegate(bytes32 poolId, uint256 amount) external override whenNotPaused nonReentrant {
        ...
        require(dates.phase1 <= block.timestamp, 'Lockdrop: NOT STARTED');
        require(block.timestamp < dates.phase2, 'Lockdrop: DELEGATIONS COMPLETED');
        require(ISomaGuard(SOMA.guard()).check(sender, toPool.requiredPrivileges), 'Lockdrop: NO PRIVILEGES');
     ...
     * @inheritdoc ILockdrop
     */
    function updateDelegationConfig(DelegationConfig calldata newConfig) external override whenNotPaused {
        address sender = _msgSender();

        DateConfig memory dates = _dateConfig;
        DelegationConfig memory _prevConfig = _delegationConfigs[sender];

        require(newConfig.percentLocked <= 100, 'Lockdrop: INVALID PERCENT');
        require(dates.phase1 <= block.timestamp, 'Lockdrop: NOT STARTED');
        require(block.timestamp < dates.phase3, 'Lockdrop: COMPLETED');
        require(block.timestamp < dates.phase2 || _prevConfig.percentLocked < newConfig.percentLocked, 'Lockdrop: DECREASE PERCENT LOCKED ENDED');
        require(block.timestamp < dates.phase2 || _prevConfig.lockDuration == newConfig.lockDuration, 'Lockdrop: UPDATE LOCK DURATION ENDED');

        _delegationConfigs[sender] = newConfig;

        emit DelegationConfigUpdated(_prevConfig, newConfig, _msgSender());
    }

    function _updateDateConfig(DateConfig calldata _newDateConfig) private {
        require(_newDateConfig.phase1 <= _newDateConfig.phase2, 'Lockdrop: INVALID DATE CONFIGURATION');
        require(_newDateConfig.phase2 <= _newDateConfig.phase3, 'Lockdrop: INVALID DATE CONFIGURATION');
    ...
    function _setWithdrawTo(address account) private {
        require(account != address(0), 'Lockdrop: INVALID WITHDRAW TO');
