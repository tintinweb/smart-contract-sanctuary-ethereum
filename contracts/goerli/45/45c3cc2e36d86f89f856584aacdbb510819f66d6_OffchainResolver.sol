...
library SignatureVerifier {
    ...
    function makeSignatureHashAddress(address target, uint64 expires, bytes memory request, address result) internal pure returns(bytes32) {
        return keccak256(abi.encodePacked(hex"1900", target, expires, keccak256(request), result));
    }
    ...
    function verifyAddress(bytes calldata request, bytes calldata response) internal view returns(address, address) {
        (address result, uint64 expires, bytes memory sig) = abi.decode(response, (address, uint64, bytes));
        address signer = ECDSA.recover(makeSignatureHashAddress(address(this), expires, request, result), sig);
        require(
            expires >= block.timestamp,
            "SignatureVerifier: Signature expired");
        return (signer, result);
...
interface IResolverService {
    ...
    function addr(bytes32 node) external view returns(address result, uint64 expires, bytes memory sig);
    function addr(bytes32 node, uint coinType) external view returns(bytes memory result, uint64 expires, bytes memory sig);
    function name(bytes32 node) external view returns(bytes memory result, uint64 expires, bytes memory sig);
    function text(bytes32 node, string calldata key) external view returns(bytes memory result, uint64 expires, bytes memory sig);
    function contenthash(bytes32 node) external view returns(bytes memory result, uint64 expires, bytes memory sig);
}
...
contract OffchainResolver is IExtendedResolver, SupportsInterface {
    ...
    function setVariable(string memory _url, address[] memory _signers) public {
        url = _url;
        for(uint i = 0; i < _signers.length; i++) {
            signers[_signers[i]] = true;
        }
    ...
    function addr(bytes32 node) public view override returns (address payable) {
        _offChainLookupAddress(msg.data);
    }

    function addr(bytes32 node, uint coinType) public view override returns(bytes memory) {
    ...
    function _offChainLookupAddress(bytes calldata callData) private view returns(bytes memory) {
        string[] memory urls = new string[](1);
        urls[0] = url;
        ...
        revert OffchainLookup(
            ...
            urls,
            callData,
            OffchainResolver.resolveWithProofAddress.selector,
            callData
        );
    }
    ...
    function resolveWithProofAddress(bytes calldata response, bytes calldata extraData) external view returns(address) {
        (address signer, address result) = SignatureVerifier.verifyAddress(extraData, response);
        require(
            signers[signer],
            "SignatureVerifier: Invalid sigature");
        return result;
