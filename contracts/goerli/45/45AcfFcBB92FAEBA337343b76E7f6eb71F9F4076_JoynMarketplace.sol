    ...
    OwnableUpgradeable
{
    ...
    function _validateListing(
        ...
        if (shouldListed) {
            if (listing.price == 0 || listing.buyer != address(0)) {
    ...
    function _distributeRoyalties(
        ...
        for (i = 0; i < recipients.length; i++) {
            ...
            (bool RoyaltyTransferSuccess, ) = payable(receiver).call{
                value: royalty
            }("");
            require(
                RoyaltyTransferSuccess,
                "Transfer ETH due to original creator failed"
            );
    ...
    function listItem(
    ...
    ) external isOwner(nftAddress, tokenId, _msgSender()) {
        _validateListing(nftAddress, tokenId, false);
        ...
        if (
            IERC721Upgradeable(collectionAddress).isApprovedForAll(
                _msgSender(),
                address(this)
            ) != true
        ) {
    ...
    {
        _validateListing(nftAddress, tokenId, true);
        ...
        nonReentrant
    {
        ...
        uint256 sumOfRoyalties = _distributeRoyalties(nftAddress, tokenId, listedItem.price, listedItem.listingId);

        amountDueToSeller -= sumOfRoyalties;
        ...
        require(success, "Transfer ETH due to seller failed");
        emit ItemBought(
            listedItem.seller,
            _msgSender(),
            nftAddress,
    ...
    function updateListingMarketplaceFeeAndPrice(
        ...
        uint256 marketplaceFee,
        uint256 price
    ) external nonReentrant isOwner(nftAddress, tokenId, _msgSender()) {
        _validateListing(nftAddress, tokenId, true);
