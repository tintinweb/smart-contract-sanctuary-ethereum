...
contract GamebopItem is ERC721AQueryable, Ownable, ReentrancyGuard {
    ...
    constructor() ERC721A("GamebopItem", "GI") {
    ...
    function mintToken(uint256 nMints, string memory tokenUri, uint256 royalty) public returns (uint256) {
        require(msg.sender == tx.origin, "Can't mint through another contract");
        require(msg.sender == contract_owner, "Can't mint since you are not contract owner");

        uint256 startItemId = _tokenIds.current();
        // set royalty and creator
        setRoyalty(startItemId, nMints, royalty);
        setCreator(startItemId, nMints, msg.sender);

        // increase total_supply
        _tokenIds.increment(nMints);
        uint256 lastItemId = _tokenIds.current();

        // mint & set tokenURI        
        _safeMint(msg.sender, nMints);
        setTokenURI(startItemId, nMints, tokenUri);
    ...
    function setRoyalty(uint256 startItemId, uint256 itemCount, uint256 royalty) private {
        for (uint i = 1; i <= itemCount ; i++) {
            _royalty[startItemId + i] = royalty;
        }
    }

    function setCreator(uint256 startItemId, uint256 itemCount, address creator) private {
        for (uint i = 1; i <= itemCount ; i++) {
            _creators[startItemId + i] = creator;
        }
    }

    function setTokenURI(uint256 startItemId, uint256 itemCount, string memory tokenUri) private {
        for (uint i = 1; i <= itemCount ; i++) {
            _setTokenURI(startItemId + i, tokenUri);
        }        
    }
