...
contract TestTopsy1 is
    ERC721A,
    Ownable
{
    string private _fixTokenURI = "";
    string private _baseTokenURI = "ipfs://QmX74aa8YqWFsLSxRKg7s84VSyoLtqNGPjSDqtXyYrVXoc/";

    address private constant MAIN_WALLET_ADDRESS = 0x107C1F211388d4C33fDEa51409ddeF0b8edB9168;

    mapping(address => uint256) private _whitelistAddressAndMaxMintQuantityMap;

    uint256 private _publicSaleMaxMintQuantity = 2;

    uint256 private _tokenCirculation = 1000;

    uint256 private _reservedSupply = 500;

    uint256 private _whitelistSaleTokenPrice = 0.003 ether;

    uint256 private _publicSaleTokenPrice = 0.004 ether;

    bool private _isWhitelistSaleActived = true;
    uint256 private _whitelistSaleStartTime = 1646636400;//2022年3月7日星期一 15:00:00 GMT+08:00
    uint256 private _whitelistSaleEndTime = 1669355476;//2022年11月25日星期五 13:51:16 GMT+08:00

    bool private _isPublicSaleActived = true;
    uint256 private _publicSaleStartTime = 1646637600;//2022年3月7日星期一 15:20:00 GMT+08:00
    uint256 private _publicSaleEndTime = 1669355476;//2022年11月25日星期五 13:51:16 GMT+08:00
    ...
    constructor() ERC721A("Test Topsy 1", "TestTopsy1") {
    }

    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        if(bytes(_fixTokenURI).length > 0) {
            return _fixTokenURI;
        }
    ...
    function whitelistSaleMint(uint256 quantity) public payable {
        ...
        require(totalSupply() + quantity < _tokenCirculation - _reservedSupply, "Exceed token circulation");
    ...
    function airdrop(address to, uint256 quantity) public onlyOwner {
        ...
        require(totalSupply() + quantity < _tokenCirculation, "Exceed token circulation");
    ...
    function batchAirdrop(address[] calldata addresses, uint256[] calldata quantities) public onlyOwner {
        ...
        for (uint256 i = 0; i < addresses.length; ++i) {
            this.airdrop(addresses[i], quantities[i]);
        }
    }
    ...
    function batchBurn(uint256[] calldata tokenIds) external callerIsUser {
        for (uint256 i = 0; i < tokenIds.length; ++i) {
            this.burn(tokenIds[i]);
        }
    }
    ...
    function setFixTokenURI(string calldata fixTokenURI) public onlyOwner {
        _fixTokenURI = fixTokenURI;
