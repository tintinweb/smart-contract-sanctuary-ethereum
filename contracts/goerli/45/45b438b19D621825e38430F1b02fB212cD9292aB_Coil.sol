...
contract Coil is ERC20, Ownable {
    ...
    address public _oraclePriceFeed = block.chainid == 5 ? 0x48731cF7e84dc94C5f84577882c14Be11a5B7456 
    ...
    constructor() ERC20("Coil", "Coil") payable {
        ...
        _marketingWallet = address(0x11Da420d36e59b20B55A0dE50095f4C71eDbDbF9);
        _developmentWallet = address(0xcD966Bd7b11B2c766B4cF2FfEBB1992634e8E0DB);
    ...
    function initBoost() private {
        ...
        if (_launchTime + _blockToInitBoostPrice <= block.timestamp && currentPrice > _initialPrice) {
            updateBoostPrice(_initialPrice + (currentPrice - _initialPrice).mul(7).div(10));
        }
    }
    ...
    function startBoost() private {
        ...
        if (currentPeriod > _previousPeriod) {
            if (currentPeriod % _dipPeriod == 0 && !_isDipPeriod) {
                _isDipPeriod = true;
                updateBoostPrice(_initialPrice + (_boostPrice - _initialPrice).mul(hundredMinusDipPercent).div(100));
            }
    ...
    function needTakeFee(address from, address to) public view returns (bool) {
        ...
        return isFeeSet && !_swappingBack && !isExcludedFromFee && !isBuyAndTaxFree && isSwap;
    }
    ...
    function updateFees(uint256 marketingFee, uint256 developmentFee, uint256 liquidityFee) external onlyOwner {
    ...
    function setBoostTokenUpdateTime(uint256 boostUpdateTime) external onlyOwner() {
