...
contract TransferRules is ITransferRules {
  ...
  constructor() {
    ...
    errorMessage[LOWER_THAN_RECIPIENT_MIN_BALANCE] = "LOWER THAN RECIPIENT MIN BALANCE";
    ...
    returns(uint8)
  {
    ...
    if ((token.getMinBalance(to) > 0) &&
          (token.balanceOf(to) + value < token.getMinBalance(to))
    ...
    if ( token.unlockedAmountOf(from) < value
