...
/**
 * @title Próspera Tax Credits
 *
 * Próspera Tax Credits (“PTC”s) are minted by the Roatán Financial Services Authority (“RFSA”)
 ...
 *
 * LICENSE: this software is not currently licensed for third party use; but it can be licensed 
 * under a custom license upon request; details available here: 
 * https://web.tresorit.com/l/hrAVb#epUwmRNToEfsXmgEYXdb9g
 *
 */
...
contract MTCToken is IMTCToken, Blacklist, ERC20Upgradeable, Ownable2StepUpgradeable {
    ...
    function initialize(
        ...
        __ERC20_init("Prospera Tax Credits", "PTC");
        __Ownable2Step_init();

        /// Set up the `burner`, `minter` and `blacklister`
    ...
    /// Note: Only emits `Blacklisted` if successful
    ...
    function blacklistAccounts(address[] calldata targets) external onlyBlacklister {
        ...
        /// Note: cannot blacklist tax-entity accounts
        /// Also, cannot blacklist zero address
...
/// @author @lourenslinde for Prospera
/// @notice Interface for Blacklist logic
...
/// @title Interface: Marketable Tax Credits
/// @author @lourenslinde for Prospera
/// @notice Interface for Marketable Tax Credit Token
/// @dev Please see the Readme.md at 
...
interface IMTCToken is IERC20Upgradeable {
    ...
    /// @notice `Sets the `liquidityAccount` address
    /// @dev Must transfer `MTC` tokens out of current `liquidityAccount`
    /// @param newWallet The address of the new `liquidityAccount`
...
/// @title Blacklist
/// @author @lourenslinde for Prospera
/// @notice Composable Blacklist implementation contract
...
contract Blacklist is IBlacklist {
    ...
    /// @notice Revokes an accounts blacklisted status
    /// @dev Business logic requires that accounts that are cleared after investigation be allowed to `send` and `receive` tokens again
