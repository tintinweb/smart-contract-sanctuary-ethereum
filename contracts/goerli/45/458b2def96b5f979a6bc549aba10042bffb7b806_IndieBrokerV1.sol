...
contract IndieBrokerV1 is EIP712, Ownable, Pausable {
    ...
    event StartProject(uint256 indexed projectId, address leadAddress, address clientAddress, address salesAddress);
    ...
    event ReassignProjectLead(uint256 indexed projectId, address indexed newLeadAddress);

    event ReassignProjectSales(uint256 indexed projectId, address indexed newSalesAddress);

    event ReassignProjectClient(uint256 indexed projectId, address indexed newClientAddress);
    ...
    uint256 constant MAX_FEES = 100_00; // The sum of fees cannot exceed 100%
    ...
    function completeSprint(
        ...
        for (uint256 i = 0; i < payees.length; i++) {
            uint256 theProjectId = projectId;
            address payee = payees[i];
            uint256 amount = amounts[i];
            uint256 treasuryAmount = (amount * fees[Fees.Treasury]) / FEE_DENOMINATOR;
            uint256 leadAmount = (amount * fees[Fees.Lead]) / FEE_DENOMINATOR;
            uint256 salesAmount = (amount * fees[Fees.Sales]) / FEE_DENOMINATOR;
            uint256 cashVestingAmount = (amount * fees[Fees.CashVesting]) / FEE_DENOMINATOR;
            uint256 payeeAmount = amount - treasuryAmount - leadAmount - salesAmount - cashVestingAmount;

            emit DistributePayment(
                payee, theProjectId, amount, payeeAmount, treasuryAmount, leadAmount, salesAmount, cashVestingAmount
                );

            projectBalance -= amount;

            if (treasuryAmount > 0) {
                usdc.transfer(feeRecipients[Fees.Treasury], treasuryAmount);
            }

            if (cashVestingAmount > 0) {
                usdc.transfer(feeRecipients[Fees.CashVesting], cashVestingAmount);
            }

            if (leadAmount > 0) {
                usdc.transfer(project.leadAddress, leadAmount);
            }

            if (salesAmount > 0) {
                usdc.transfer(project.salesAddress, salesAmount);
            }

            if (payeeAmount > 0) {
                usdc.transfer(payee, payeeAmount);
            }
        }
        ...
        emit CompleteProjectSprint(projectId, sprintId, totalAmount);
    }

    function setFee(Fees f, uint256 fee) external onlyOwner {
        fees[f] = fee;

        if (fees[Fees.Treasury] + fees[Fees.Lead] + fees[Fees.Sales] + fees[Fees.CashVesting] > MAX_FEES) {
