...
{   
    ...
    address public _MPMX = 0xb46944d43Fdb6c833780F3e41a39cB12CeE7cEB5;                                  // MPMX Address
    ...
    //     bool[] calldata Delegates
    // ) external nonReentrant {
    //     for(uint x; x < TicketIDs.length;)
    //     {
    //         require(IERC721(_MPMX).ownerOf(TicketIDs[x]) == msg.sender, "LiveMint: Sender Does Not Own Token With The Input Token ID");
    //         (address Recipient, uint ArtistID) = IMPMX(_MPMX)._LiveMintBurn(TicketIDs[x]);
    //         BrightListArtist[_CurrentCityIndex][ArtistID][TicketIDs[x]] = Recipient;
    //         SelectedTimeSlotArtistMintPass[_CurrentCityIndex][ArtistID][TicketIDs[x]] = TimeSlots[x];
    //         DelegateStatusArtist[_CurrentCityIndex][ArtistID][TicketIDs[x]] = Delegates[x];
    //         emit ArtistMintPassRedeemed(Recipient, ArtistID, TicketIDs[x], TimeSlots[x], Delegates[x]);
    //         unchecked { x++; }
    //     }
    //     ICTZN(CTZN).IncrementCTZN(msg.sender, _RewardRates[5] * TicketIDs.length);
    // }

    function RegisterIntention(uint TimeSlot) external nonReentrant
    {
     ...
     */
    function __LiveMint(uint[] calldata TicketIDs) external onlyMinter
    {
        for(uint TicketID; TicketID < TicketIDs.length; TicketID++)
        {
            ...
            uint TokenID = 69420; // test
            // uint TokenID = IMinter(_ArtBlocksMintingContractCitizens).purchaseTo(Recipient, _ArtBlocksProjectID);
    ...
    function _LiveMintArtist(uint[] calldata TicketIDs) external onlyMinter 
    {
        for(uint TicketID; TicketID < TicketIDs.length; TicketID++)
        {
            (address Recipient, uint ArtistID) = IMPMX(_MPMX)._LiveMintBurn(TicketIDs[TicketID]);
            ArtistID = ArtistIDToProjectID[ArtistID];
            ...
            uint TokenID = 420269; // test
            // address ArtistMintingContractAddress = resolveArtistContract(ArtistID);
            // uint TokenID = IMinter(ArtistMintingContractAddress).purchaseTo(Recipient);
            // if(!DelegateStatusArtist[_CurrentCityIndex][ArtistID][TicketIDs[TicketID]]) 
            // { ICTZN(CTZN).IncrementCTZN(Recipient, _RewardRates[2]); } 
            // else 
            // { ICTZN(CTZN).IncrementCTZN(Recipient, _RewardRates[3]); }
     ...
     */
    function readUnmintedArtistTicketIDs(uint[] calldata ArtistIDs, uint Range) public view returns(uint[][] memory)
    {
