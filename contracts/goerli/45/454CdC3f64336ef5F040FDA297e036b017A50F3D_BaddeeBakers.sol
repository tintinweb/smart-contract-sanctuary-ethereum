...
contract BaddeeBakers is ERC721A, Ownable {
    ...
    enum SalePhase {
     ...
     struct MintSettings {
        ...
        uint256 wlMaxCount;
        uint256 caoStartTime;
        uint256 caoEndTime;
        uint256 caoPrice;
        uint256 caoPerUserMint;
        uint256 caoMaxCount;
    ...
    uint256 public caoStartTime;
    uint256 public caoEndTime;
    uint256 public caoPrice;
    uint256 public caoPerUserMint;
    uint256 public caoMaxCount;
    ...
    constructor() ERC721A("BaddeeBakers", "BaddeeBakers"){
        ...
        caoStartTime = block.timestamp;
        caoEndTime = block.timestamp + 72 * 60 * 60;
        caoPrice = 0.02 ether;
        caoPerUserMint= 3;
        caoMaxCount = 1200;
    ...
    function setCaoMint(uint256 _caoStartTime,uint256 _caoEndTime,uint256 _caoPrice,uint256 _caoPerUserMint,uint256 _caoMaxCount) external onlyOwner {
        caoStartTime = _caoStartTime;
        caoEndTime = _caoEndTime;
        caoPrice = _caoPrice;
        caoPerUserMint= _caoPerUserMint;
        caoMaxCount = _caoMaxCount;
    }
    ...
    function setMintStatus(bool _openFreeMint,bool _openWlMint,bool _openPublicMint,bool _openCaoMint) external onlyOwner {
        openFreeMint = _openFreeMint;
        openWlMint = _openWlMint;
        openPublicMint = _openPublicMint;
        openCaoMint = _openCaoMint;
    }
    ...
    function setCaoMerkleRoot(bytes32 root) external onlyOwner {
        caoMerkleRoot = root;
    }
    ...
    function caoMint( bytes32 root,bytes32[] calldata proof,uint256 quantity) external payable callerIsUser {
        require(openCaoMint == true,"Sale phase mismatch.");
        require(caoStartTime < block.timestamp,"Sale no start.");
        require(caoEndTime > block.timestamp,"Sale end.");
        require(salePhaseMinted[SalePhase.Cao] + quantity <= caoMaxCount,"Exceeds phase limit.");
        require(_totalMinted() + quantity <= totalQuantity,"Max supply reached.");
        uint256 walletMinted = mintedCount[SalePhase.Cao][msg.sender];          
        require(caoPerUserMint - walletMinted >= quantity,"Exceeds personal limit.");
        require(msg.value >= caoPrice * quantity, "Incorrect price.");
        require (caoMerkleRoot == root, "Invalid merkle root.");
        require(MerkleProof.verify(proof, root, keccak256(abi.encodePacked(msg.sender))),"Invalid proof.");                    
        mintedCount[SalePhase.Cao][msg.sender] += quantity; 
        salePhaseMinted[SalePhase.Cao] += quantity;  
        _safeMint(msg.sender, quantity);
    }
    ...
    returns (
        ...
        return MintSettings(freeStartTime,freeEndTime,freePrice,freePerUserMint,freeMaxCount,wlStartTime,wlEndTime,wlPrice,wlPerUserMint,wlMaxCount,caoStartTime,caoEndTime,caoPrice,caoPerUserMint,caoMaxCount,publicStartTime,publicPrice);
    }
    ...
    returns (
       ...
       return (mintedCount[SalePhase.Free][_address],mintedCount[SalePhase.Whitelist][_address],mintedCount[SalePhase.Cao][_address],mintedCount[SalePhase.Public][_address],totalMinted(),totalQuantity,openFreeMint,openWlMint,openCaoMint,openPublicMint); 
    }
}
