...
import "../tokens/ERC721/ERC721A.chiru.sol";
import "../access/OperatorFilterer.sol";
import "../access/Ownable.sol";
import "../access/Paid.sol";


contract NFTrees is ERC721A, Ownable, OperatorFilterer {
    ...
    constructor(string memory name_, string memory symbol_, uint initMintAmount_, uint maxSupply_) 
        ERC721A(name_, symbol_) OperatorFilterer(address(0), false) 
    {
    ...
    function approve(address to, uint256 tokenId) public payable virtual override {
        ...
        super._approve(to, tokenId, true);
    }

    function setApprovalForAll(address operator, bool approved) public virtual override {
        if (operator == owner()) revert InvalidApproval();
    ...
    function mint() external payable {
        ...
        require(totalSupply() + 1 < maxSupply + 1, "No more");
    ...
    function mint(uint quantity_) external payable {
        ...
        require(msg.value >= quantity_ * cost, "Please send the exact amount.");
        require(totalSupply() + quantity_ < maxSupply + 1, "No more");
        require(mintEnabled, "Minting is not live yet");
        require(quantity_ < maxPerTx + 1, "Max per TX reached.");

        _safeMint(msg.sender, quantity_);
    }

    function mintTo(address to_) external onlyAllowedOperator(to_)  {
        require(totalSupply() + 1 < maxSupply + 1, "No more");

        _safeMint(to_, 1);
    }

    function mintTo(address to_, uint quantity_) external onlyAllowedOperator(to_)  {
        require(totalSupply() + quantity_ < maxSupply + 1, "No more");
    ...
    function _baseURI() internal pure override returns (string memory) {
        return 'ipfs://bafybeib652lmu7g6vtjgbkcdbnxm3zx3ziipw4w5frlztbnnz2qarxpmta/';
    }
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        ...
        return bytes(__baseURI).length != 0 
            ? string(abi.encodePacked(__baseURI, _toString(tokenId), ".json")) 
            : '';
    }

    function contractURI() external pure returns(string memory) {
        return "ipfs://bafkreibpjm4eybtkxmzrnmn6sguak5rqi5hmwie2ipszlbmbcbmydnnac4";
    }
    ...
    function setMintEnabled(bool value) external onlyOwner {
        mintEnabled = value;
        if (mintEnabled) {
            emit MintStarted();
        } else {
...
import "../../contracts/N_FTrees/NFTEsT.sol";
import "../../contracts/utils/LibString.sm.sol";

contract $NFTrees is NFTrees {
    bytes32 public __hh_exposed_bytecode_marker = "hardhat-exposed";

    constructor(string memory name_, string memory symbol_, uint256 initMintAmount_, uint256 maxSupply_) NFTrees(name_, symbol_, initMintAmount_, maxSupply_) {}
    ...
    function $_ownershipOf(uint256 tokenId) external view returns (IERC721A.TokenOwnership memory) {
        return super._ownershipOf(tokenId);
    }

    function $_ownershipAt(uint256 index) external view returns (IERC721A.TokenOwnership memory) {
        return super._ownershipAt(index);
    }

    function $_initializeOwnershipAt(uint256 index) external {
        return super._initializeOwnershipAt(index);
    }
    ...
    function $_mint(address to,uint256 quantity) external {
        return super._mint(to,quantity);
    }

    function $_mintERC2309(address to,uint256 quantity) external {
        return super._mintERC2309(to,quantity);
    }
    ...
    function $_approve(address to,uint256 tokenId,bool approvalCheck) external {
        return super._approve(to,tokenId,approvalCheck);
    }
    ...
    function $_setExtraDataAt(uint256 index,uint24 extraData) external {
        return super._setExtraDataAt(index,extraData);
    }

    function $_extraData(address from,address to,uint24 previousExtraData) external view returns (uint24) {
        return super._extraData(from,to,previousExtraData);
    }

    function $_msgSenderERC721A() external view returns (address) {
        return super._msgSenderERC721A();
    }

    function $_toString(uint256 value) external pure returns (string memory) {
...
abstract contract $IERC721A is IERC721A {
    bytes32 public __hh_exposed_bytecode_marker = "hardhat-exposed";
...
import "../../../contracts/tokens/ERC721/ERC721A.chiru.sol";

abstract contract $ERC721A__IERC721Receiver is ERC721A__IERC721Receiver {
    bytes32 public __hh_exposed_bytecode_marker = "hardhat-exposed";
