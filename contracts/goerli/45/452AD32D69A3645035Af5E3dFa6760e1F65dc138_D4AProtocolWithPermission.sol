...
contract D4AProtocolWithPermission is D4AProtocol, EIP712Upgradeable {
    ...
    function _verifySignature(
        ...
        require(signer != address(0) && signer == settings.owner_proxy().ownerOf(_canvas_id), "invalid signature");
    }
}
        ...
        returns (uint256)
    {
        ...
        if (_flat_price == 0) {
            price = all_prices.getCanvasNextPrice(
                settings, pi.floor_prices, pi.floor_price_rank, pi.start_prb, _project_id, _canvas_id
            );
        ...
        if (_flat_price == 0) all_prices.updateCanvasPrice(settings, _project_id, _canvas_id, price);
