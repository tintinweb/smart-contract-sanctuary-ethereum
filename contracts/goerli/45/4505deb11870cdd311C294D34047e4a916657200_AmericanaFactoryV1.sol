...
contract AmericanaFactoryV1 is ReentrancyGuard {
    ...
    IMinter immutable MinterContract;

    modifier onlyAuthorized {
      require(isAuthorized[msg.sender], "AmericanaFactoryV1: Unauthorized");
      _;
   }
   modifier notPaused {
    ...
    mapping(uint24 => AmericanaItem) public americanaItems;
    mapping(uint24 => bool) public availableToMint;


    event NewItemCreated(uint24 _itemId, address _seller, uint256 _amount);
    ...
    function createNewItem(uint24 _itemId, uint72 _price) external notPaused {
        ...
        require(_price > 0, "AmericanaFactoryV1: 0 price");
    ...
    function createEscrow(uint24 _itemid, bool _wantsNFT) external payable notPaused {
        require(americanaItems[_itemid].seller != payable(address(0)), "AmericanaFactoryV1: Item not active.");
        require(escrowByID[_itemid].buyer == payable(address(0)), "AmericanaFactoryV1: Item already in escrow.");
    ...
    function approveItem(uint24 _itemid) external onlyAuthorized nonReentrant{
        require(americanaItems[_itemid].seller != payable(address(0)), "AmericanaFactoryV1: Item not active.");
        require(escrowByID[_itemid].buyer != payable(address(0)),"AmericanaFactoryV1: Escrow not active");
        ...
        require(success, "AmericanaFactoryV1: Failed to withdraw ether after approval");
        ...
        americanaItems[_itemid].seller = payable(address(0));
    ...
    function rejectItem(uint24 _itemid) external onlyAuthorized nonReentrant{
        ...
        (bool success, ) = escrowByID[_itemid].buyer.call{value: americanaItems[_itemid].price}("");
        require(success, "AmericanaFactoryV1: Failed to withdraw ether after rejection");

        emit EscrowRejected( escrowByID[_itemid].buyer, americanaItems[_itemid].seller, americanaItems[_itemid].price);
