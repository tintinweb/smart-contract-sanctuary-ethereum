...
contract AutoglyphsRenderer {
    ...
    function getSvg(uint256 tokenId) public view returns (string memory) {
        ...
        for (uint16 rowIndex = 0; rowIndex < 64; rowIndex++) {
            string memory strRowIndex = stringUtils.numberToString(rowIndex);
            string memory strRowIndexPlusOne = stringUtils.numberToString(rowIndex + 1);
            for (uint16 colIndex = 0; colIndex < 64; colIndex++) {
                bytes1 glyph = bytes1(textGlyph[30 + rowIndex * 67 + colIndex]);
                string memory strColIndex = stringUtils.numberToString(colIndex);
                string memory strColIndexPlusOne = stringUtils.numberToString(colIndex + 1);
                if (glyph == "O") {
    ...
    function tokenURI(uint256 tokenId) external view returns (string memory) {
        ...
        string memory image = getDataUriBase64(tokenId);
