...
contract RandomNFT is Ownable {
    ...
    function getRandomNumber(
        ...
        bytes32 message = getMessageHash(msg.sender, timestamp);
        require(verify(message, _sig), "It's not a signer");
        uint256 ran = random(maxCount + 1) - 1;
    ...
    function myGiveaways(address user, uint256 startPoint)
        ...
        returns (
            uint256[] memory id,
            uint256[] memory isClosed,
            uint256[] memory IdFromAll
        )
    {
        uint256[] memory IdArray = new uint256[](adrToLotAmount[user]);
        uint256[] memory array = new uint256[](adrToLotAmount[user]);
        uint256[] memory IdFromAllArray = new uint256[](adrToLotAmount[user]);
        for (uint256 i = 1; i <= adrToLotAmount[user]; i++) {
        ...
        if (IdArray.length < startPoint + amount) {
            ...
            uint256[] memory IdArray2 = new uint256[](
                IdArray.length - newStartPoint
            );
            uint256[] memory array2 = new uint256[](
                IdArray.length - newStartPoint
            );
            uint256[] memory IdFromAllArray2 = new uint256[](
                IdArray.length - newStartPoint
            );
            for (uint256 i = newStartPoint; i < IdArray.length; i++) {
                IdArray2[count] = IdArray[i];
                array2[count] = array[i];
                IdFromAllArray2[count] = IdFromAllArray[i];

                count++;
            }
            return (IdArray2, array2, IdFromAllArray2);
        } else {
            uint256[] memory IdArray2 = new uint256[](amount);
            uint256[] memory array2 = new uint256[](amount);
            uint256[] memory IdFromAllArray2 = new uint256[](amount);
        ...
        if (participant == lotOwner) {
            if (block.timestamp > adrToLotIdToInfo[lotOwner][lotId].endDate) {
                if (adrToLotIdToInfo[lotOwner][lotId].done) {
                    ...
                    return 5; //кнопка забрать нфт овнеру
                }
            } else {
                return 4; //время еще окончания лота еще не вышло (нет кнопок)
            }
        } else {
            if (block.timestamp > adrToLotIdToInfo[lotOwner][lotId].endDate) {
                if (adrToLotIdToInfo[lotOwner][lotId].winner == participant) {
                    if (adrToLotIdToInfo[lotOwner][lotId].done) {
                        ...
                        return 3; //кнопка забрать награду
                    }
                } else {
                    return 4; //не победитель (нет кнопки)
                }
            } else {
                return 4;
            }
