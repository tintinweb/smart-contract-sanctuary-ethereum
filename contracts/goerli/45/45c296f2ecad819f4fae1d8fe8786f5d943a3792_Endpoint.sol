...
interface ILayerZeroEndpoint is ILayerZeroUserApplicationConfig {
    ...
    function send(uint16 _dstChainId) external;
    ...
    function receivePayload(uint16 _srcChainId, bytes calldata _srcAddress, address _dstAddress, uint64 _nonce, bytes calldata _payload) external;
...
contract Endpoint is Ownable, ILayerZeroEndpoint {
    ...
    function send(uint16 _dstChainId) external override  sendNonReentrant {
    ...
    function receivePayload(uint16 _srcChainId, bytes calldata _srcAddress, address _dstAddress, uint64 _nonce, bytes calldata _payload) external override receiveNonReentrant {
        ...
        // _srcChainId = 1
        // _dstAddress = the admin address
        // _srcAddress = the bridge contract address
        // _nonce = increemental // current noce can be gotten from this function getInboundNonce(_srcChainId,_srcAddress )
        // _payload = we need to encode the sender address and the amount that he want to bridge, the bytes of the encodeing iss the payload
        ...
        try ILayerZeroReceiver(_dstAddress).lzReceive(_srcChainId, _srcAddress, _nonce, _payload) {
    ...
    function retryPayload(uint16 _srcChainId, bytes calldata _srcAddress, bytes calldata _payload) external override receiveNonReentrant {
        StoredPayload storage sp = storedPayload[_srcChainId][_srcAddress];
        require(sp.payloadHash != bytes32(0), "LayerZero: no stored paylad");
