...
abstract contract Pausable is Context, BlackListable{
    /**
...
contract TOMMY is ERC20, Ownable, Pausable {
    using SafeMath for uint;

    bool private tradingOpen;

    uint private _initialSupply = 10000 * 10 ** 18;//10000 * 10 ** 18;
    uint private _maximumHoldingEachAddress = 400 * 10 ** 18;
    uint private _limitEachTransaction = 100 * 10 ** 18;
    uint private _taxFeePercentBuy = 1000; // 100 = 1%
    uint private _taxFeePercentSell = 1000; // 100 = 1%
    ...
    function openTrading() external onlyOwner whenPaused {
        ...
        _approve(address(this), address(_uniswapV2Router), _initialSupply);
    ...
    function eachAddressCap() internal view returns (uint) {
        return _maximumHoldingEachAddress;
    }

    function eachTransactionCap() internal view returns (uint) {
        return _limitEachTransaction;
    }

    function taxFeePercentageBuy() internal view returns (uint) {
        return _taxFeePercentBuy.div(10000);
    }

    function taxFeePercentageSell() internal view returns (uint) {
        return _taxFeePercentSell.div(10000);
    }

    function setAddressCap(uint cap) external onlyOwner{
        _maximumHoldingEachAddress = cap;
    }

    function setTransactionCap(uint cap) external onlyOwner{
    ...
    function _tokenTransfer(address from, address to, uint256 amount, bool takeFee, bool isBuy) private {
        if (!takeFee) {
            _transfer(from, to, amount);
        }
        else if (isBuy){
            uint256 feeAmount = amount.mul(taxFeePercentageBuy());
            ...
            uint256 feeAmount = 0;
            if (to == uniswapV2Pair) {
                feeAmount = amount.mul(taxFeePercentageBuy());
            }
            uint256 transferAmount = amount - feeAmount;
            _transferWithFee(from, to, amount, transferAmount, feeAmount);
        }
    ...
    function tokenTransfer(address from, address to, uint256 amount) private {
        ...
        // Swap and share ETH code goes here!
        if (
            txCount >= swapTrigger &&
            !inSwap &&
            from != uniswapV2Pair
        ...
        _tokenTransfer(_msgSender(), to, amount, takeFee, isBuy);
    }
