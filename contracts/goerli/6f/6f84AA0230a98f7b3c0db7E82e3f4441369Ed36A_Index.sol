...
contract Index is OwnableUpgradeable{
  ...
  function deposit(uint256 amountin) public payable {
    require(block.number <= endingtime, "time limit passed");

    require(
      IERC(usdc).balanceOf(msg.sender) >= amountin &&
  ...
  function purchase() public {
    uint256 balance = IERC(usdc).balanceOf(address(this));
  ...
  function rebalancesimple() public {
    sell();
    purchase();
  }

  function rebalancingcomplex() public {



  }

  function indexProfitCalculator() public view returns (bool, uint) {
    IndexInfo memory indexa = indexinfo;
    uint256 numoftokens = indexinfo.tokens.length;
    uint[] memory total;
    ...
    for (uint256 i; i < numoftokens; i++) {
      uint256 balance = IERC(indexa.tokens[i]).balanceOf(address(this));
          ...
          (total,)=IDexAggregator(dex).getRates(indexa.tokens[i], usdc, balance, 2);
  ...
  function initialize(
    ...
    endingtime = startingtime + _blocks;
    dex = _dex;
   __Ownable_init();
