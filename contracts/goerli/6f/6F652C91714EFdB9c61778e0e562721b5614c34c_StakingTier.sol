...
contract StakingTier is IStaking, Initializable, OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    function withdrawAll() external override nonReentrant {
        ...
        totalStakedBalance -= withdrawBalance;

        if (block.timestamp <= stakingData.startStakedAt + withdrawDelay + requiredStakingDays) {
            withdrawBalance = withdrawBalance - (penaltyPercentgage * withdrawBalance) / 10000;
        }
