...
contract TomiStates {
    ...
    struct ProposalSendTokenDevData {
        /// @notice Unique id for looking up a proposal
        uint256 id;
    ...
    struct ProposalChangeMintingData {
        /// @notice Unique id for looking up a proposal
        uint256 id;
        /// @notice Date/Amount of tokens being minted to development wallet
        uint256 value;
    }
    ...
    mapping (uint256 => ProposalSendTokenDevData) internal proposalsSendTokenDevData;
    mapping (uint256 => ProposalChangeMintingData) internal proposalsChangeMintingData;
...
contract TomiDevFundsDao is TomiEvents, TomiStates, Ownable {
    using SafeMath for uint256;

    constructor(ITomiMainDao _tomiMainDao, ITreasury _treasury, address _developmentWallet) {
    ...
    function getProposalsSendTokenDevData(uint256 proposalId) external view returns (ProposalSendTokenDevData memory) {
        return proposalsSendTokenDevData[proposalId];
    }

    function getProposalsChangeMintingData(uint256 proposalId) external view returns (ProposalChangeMintingData memory) {
        return proposalsChangeMintingData[proposalId];
    }
    ...
    function proposeSendTokenDev(
        ...
        require(functionId == 20, "incorrect function id");
        ...
        ProposalSendTokenDevData storage newProposalData = proposalsSendTokenDevData[tomiMainDao.proposalCount()];
        newProposalData.id = tomiMainDao.proposalCount();
        newProposalData.transferType = transferType;
        newProposalData.executionAmount = executionAmount;

        initializeProposal(nftIds, functionId, description, title);
    }

    function proposeChangeMinting(
        string memory description,
        uint256[] memory nftIds,
        string memory title,
        uint8 functionId,
        ...
        require(nftIds.length == tomiMainDao.amountNFTRequiredToPropose(), "insufficient NFT amount for proposal");
        require(23 <= functionId && functionId <= 24, "incorrect function id");
        tomiMainDao.updateProposalCount(tomiMainDao.proposalCount() + 1);
        ...
        ProposalChangeMintingData storage newProposalData = proposalsChangeMintingData[tomiMainDao.proposalCount()];
        newProposalData.id = tomiMainDao.proposalCount();
        newProposalData.value = value;
    ...
    function executeSendTokenDev(uint256 proposalId) external {
        Proposal storage proposal = proposals[proposalId];

        require(20 <= proposal.functionId && proposal.functionId <= 22, "incorrect function id");
        ...
        if (state(proposalId) == ProposalState.Succeeded) {
            ...
            ProposalSendTokenDevData memory proposalData = proposalsSendTokenDevData[proposal.id];
            if (proposalData.transferType == 0) {
                treasury.withdrawEth(payable(developmentWallet), proposalData.executionAmount);
            } else if (proposalData.transferType == 1) {
                treasury.withdrawToken(usdToken, developmentWallet, proposalData.executionAmount);
            } else if (proposalData.transferType == 2) {
                treasury.withdrawToken(tomiToken, developmentWallet, proposalData.executionAmount);
            }
            // SafeERC20.safeTransferFrom(proposalData.token, address(treasury), developmentWallet, proposalData.executionAmount);
        }
    ...
    function executeChangeMinting(uint256 proposalId) external {
        Proposal storage proposal = proposals[proposalId];

        require(23 <= proposal.functionId && proposal.functionId <= 24, "incorrect function id");
        require(state(proposalId) == ProposalState.Succeeded || state(proposalId) == ProposalState.Defeated,
            "TomiDAO::execute: proposal can only be executed if it has succeeded or failed");
        require(block.timestamp >= proposal.endTime, "proposal is currently active");
        ...
        if (state(proposalId) == ProposalState.Succeeded) {
            ...
            if (proposal.functionId == 23) {
                // TODO change minting date
            }
            else if (proposal.functionId == 24) {
                // TODO change minting amount
            }
        }
    ...
    function testUpdateDevelopmentWallet(address _developmentWallet) external {
        developmentWallet = _developmentWallet;
    }
}
