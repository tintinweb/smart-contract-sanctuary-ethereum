...
contract RiggedRoll is Ownable {
    ...
    function withdraw(address payable _addr, uint256 _amount) public payable {
    ...
    require(address(this).balance >= .002 ether, "Balance should be > 0.002");

     bytes32 prevHash = blockhash(block.number - 1);
      uint256 _nonce = diceGame.nonce();
        bytes32 hash = keccak256(abi.encodePacked(prevHash, diceGame, _nonce));
        uint256 roll = uint256(hash) % 16;
require(roll <= 2, "I won't roll this dice");
