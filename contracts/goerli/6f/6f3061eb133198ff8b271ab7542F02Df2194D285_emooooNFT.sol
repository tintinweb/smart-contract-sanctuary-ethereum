...
contract emooooNFT is ERC721 {
    ...
    mapping(address => uint256[]) public stakeOfOwner;
    using Strings for uint256;
    ...
    function DemoCallOption(uint256 _mintAmount) public payable checkbalance(){
        require((pointBalances[msg.sender] + balanceOf(msg.sender) + _mintAmount)<=3,"exceed the free amount");
        require(block.timestamp <= 1684605600,"Free MintAuthority is over");
        // uint256 costNum = calculatePoint(_mintAmount,msg.sender);
        // // Check that the user has approved the transfer of tokens to this contract
        // token.approve(address(this), costNum);
        // require(token.transferFrom(msg.sender, address(this), costNum), "Token transfer failed");
    ...
    //new version
        ...
        for (uint256 i = 0; i < _mintAmount; i++) {
            ...
            stakeOfOwner[msg.sender].push(newTokenId);
