...
contract ProxyLiquidityMiningETH is ProxyLiquidityMining {
    ...
    ) ProxyLiquidityMining(router_, _DFI, _secondToken, _pair, _rewardSpeed) {}
     ...
     * @param amountETHMin minimum amount of USDT to be deposited into DFI-USDT pool (required by UniswapV2Router02)
     ...
     * @param amountETHMin minimum USDT tokens that is going to be returned to staker (from the Uniswap DFI-USDT pool)
...
contract LiquidityMiningLogic is AccessControl {
    ...
    function claimRewards(address recipient) external returns(uint256) {
        ...
        return _claimRewards(recipient);
    }

    /**
     * @notice internal function to claim rewards
     */
    function _claimRewards(address recipient) internal returns(uint256) {
        if (rewardAccrueds[recipient] == 0) return 0;
        uint256 proxyBalance = DFI.balanceOf(address(this));
        if (proxyBalance == 0) return 0;
