...
contract Staking is Pausable, Ownable {
  ...
  event SetExpiresAfter(uint256 expiresAfter);
  ...
  uint256 private _percent = 5;
  uint256 private _expiresAfter = 90 days;
  ...
  function openPosition(uint256 amount)
    public
    payable
    whenNotPaused
    returns (bool)
  {
    ...
    uint256 interest = amount.mul(_percent).div(100);
    require(
    ...
    Position memory position = Position(
      ...
      amount,
      interest,
      block.timestamp.add(_expiresAfter),
      block.timestamp,
  ...
  function setPercent(uint256 newPercent) public onlyOwner {
    require(
      ...
      "Amount must be greater or equal than 0 and lower or equal than 100"
    );

    _percent = newPercent;

    emit SetPercent(newPercent);
  }

  function getPercent() public view virtual returns (uint256) {
    return _percent;
  }

  // # get and set expires after
  function getExpiresAfter() public view virtual returns (uint256) {
    return _expiresAfter;
  }

  function setExpiresAfter(uint256 newExpiresAfter) public onlyOwner {
    require(newExpiresAfter >= 0, "Amount must be greater or equal than 0");

    _expiresAfter = newExpiresAfter;

    emit SetExpiresAfter(newExpiresAfter);
  }
