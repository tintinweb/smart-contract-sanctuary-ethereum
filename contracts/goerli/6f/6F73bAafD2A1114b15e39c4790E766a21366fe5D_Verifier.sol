...
contract BridgeFacet is Base, IBridge {
    ...
    function _addPriorityRequest(
        ...
        require(_opTree == Operations.OpTree.Full);
...
contract GettersFacet is Base, IGetters {
    ...
    function facets() external view returns (Facet[] memory) {
        ...
        Facet[] memory facets = new Facet[](facetsLen);

        for (uint256 i = 0; i < facetsLen; ++i) {
            address facetAddress = ds.facets[i];
            Diamond.FacetToSelectors memory facetToSelectors = ds.facetToSelectors[facetAddress];

            facets[i] = Facet(facetAddress, facetToSelectors.selectors);
        }
    ...
    function facetsExtended() external view returns (FacetExtended[] memory) {
        ...
        uint256 facetsLen = ds.facets.length;
        FacetExtended[] memory facets = new FacetExtended[](facetsLen);

        for (uint256 i = 0; i < facetsLen; ++i) {
            address facetAddress = ds.facets[i];
            bytes4[] memory selectors = ds.facetToSelectors[facetAddress].selectors;
            ...
            facets[i] = FacetExtended(facetAddress, selectorsExt);
        }
