...
contract SUSDimp {
    ...
    string public constant name = "Stable USD"; // solium-disable-line
    string public constant symbol = "StUSD"; // solium-disable-line uppercase
    ...
    bytes32 constant internal EIP712_DOMAIN_SEPARATOR_SCHEMA_HASH = keccak256(
        "EIP712Domain(str name,address ver.Cont)"
    );
  ...
  function initializeDomainSeparator() public {
    ...
    */
    ...
    function proposeOwner(address _proposedOwner) public onlyOwner {
        require(_proposedOwner != address(0), "cannot transfer ownership");
     ...
     */
   function reclaimSUSD() external onlyOwner {
    uint256 _balance = balances[address(this)];
    balances[address(this)] = 0;
    balances[owner] = uint256(uint128(balances[owner]) + uint128(_balance));
   ...
   function decreaseSupply(uint256 _value) public onlySupplyController returns (bool success) {
        ...
        uint256 oldBalance = balances[supplyController];
        balances[supplyController] = oldBalance - _value;
        totalSupply_ = totalSupply_ - _value;
    ...
    function _betaDelegatedTransfer(
     ...
     bytes32 delegatedTransferHash = keccak256(abi.encode(EIP712_DELEGATED_TRANSFER_SCHEMA_HASH, address(to), uint256(value), uint256(fee), uint256(seq), uint256(deadline)));
bytes32 hash = keccak256(abi.encode(EIP191_HEADER, EIP712_DOMAIN_HASH, delegatedTransferHash));
address _from = ecrecover(hash, uint8(v), bytes32(r), bytes32(s));
...
uint256 temp1 = balances[_from] - uint256(value);
require(temp1 >= uint256(fee), "insufficient fund");
require(nextSeqs[_from] == uint256(seq), "incorrect seq");

nextSeqs[_from] += 1;
balances[_from] = temp1 - fee;
if (fee != 0) {
...
emit BetaDelegatedTransfer(_from, to, uint256(value), uint256(seq), uint256(fee));
return true;
}
    /**
