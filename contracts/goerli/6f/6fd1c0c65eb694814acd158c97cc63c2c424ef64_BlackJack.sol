        ...
        for (uint8 i = 0; i < uint8(playerAddresses.length); i++) {
            if (players[playerAddresses[i]].bet > 0) {
                for (
                    ...
                    if (dealerCardTotal > 21) {
                        uint48 winnings = (players[playerAddresses[i]].highestSplitNumber > 0) ? (players[playerAddresses[i]].bet /
                                (players[playerAddresses[i]].highestSplitNumber+1)) * 2 : players[playerAddresses[i]].bet * 2;
                        payout(playerAddresses[i], winnings);
                        emit PlayerWin(
                            playerAddresses[i],
                            winnings,
