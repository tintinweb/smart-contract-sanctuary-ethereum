...
contract WhitelistRegistrationRules is IRegister, PrimaryEns, IERC721Receiver {
    ...
    function registerSubdomain(
        ...
        if (msg.sender != domainManager.TokenOwnerMap(_id)) {
            ...
            require(
                maxMint == 0 || mintCount[_id][msg.sender] < maxMint,
                "mint count exceeded"
            );
            bytes32 leaf = keccak256(
                (maxMint == 0)
                    ? abi.encodePacked(msg.sender, _label)
    ...
    function updateMintPriceAndMerkleRoot(
        uint256 _tokenId,
        uint256 _price,
        bytes32 _merkleRoot,
        uint256 _maxMint
    ...
    modifier isTokenOwner(uint256 _tokenId) {
        require(domainManager.TokenOwnerMap(_tokenId) == msg.sender, "not authorised");
        _;
    }
}