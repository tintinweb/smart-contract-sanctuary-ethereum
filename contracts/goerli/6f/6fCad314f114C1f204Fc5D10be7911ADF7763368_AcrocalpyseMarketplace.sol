...
contract AcrocalpyseMarketplace is Initializable, OwnableUpgradeable, PausableUpgradeable {
    ...
    // paper token address
    IERC20Upgradeable public paperToken;
    ...
    event AuctionBid(address bidder, uint256 itemId, uint256 bidAmount, uint256 timestamp, uint256 bidQuantity);
    event ReclaimedBids(address bidder, uint256[] itemIds, uint256 timestamp);

    function initialize(address _signerAddress, IERC20Upgradeable _paperTokenAddress) public initializer {
        signerAddress = _signerAddress;

        if (address(_paperTokenAddress) != address(0)) {
            paperToken = IERC20Upgradeable(_paperTokenAddress);
        }
    }
    ...
    function bid(
        ...
        // verifying the signature
        bytes32 hash = ECDSAUpgradeable.toEthSignedMessageHash(
            keccak256(abi.encodePacked(_msgSender(), itemId, bidAmount, bidQuantity))
        );
        require(ECDSAUpgradeable.recover(hash, signature) == signerAddress, "Invalid Access");
        ...
        uint256 bidDifference = bidAmount - previousBid;
        uint256 tokensToTransfer = bidDifference.mul(bidQuantity);

        // checking the $PAPER Token balance
        require(paperToken.balanceOf(_msgSender()) >= tokensToTransfer, "Insufficient $PAPER balance");

        // Transfer $PAPER Token from Account to the Contract
        bool transferSuccess = paperToken.transferFrom(_msgSender(), address(this), tokensToTransfer);
        require(transferSuccess, "$PAPER transfer failed");
        ...
        emit AuctionBid(_msgSender(), itemId, bidAmount, block.timestamp, bidQuantity); // solhint-disable-line not-rely-on-time
    }

    function reclaimBids(bytes memory signature, uint256[] memory itemIds) external whenNotPaused callerIsUser {
        // verifying the signature
        bytes32 hash = ECDSAUpgradeable.toEthSignedMessageHash(keccak256(abi.encodePacked(_msgSender(), itemIds)));
        require(ECDSAUpgradeable.recover(hash, signature) == signerAddress, "Invalid Access");
    ...
    function setPaperToken(IERC20Upgradeable newAddress) external onlyOwner {
        if (address(newAddress) != address(0)) {
            paperToken = IERC20Upgradeable(newAddress);
        }
    }

    function viewContractTokenBalance(IERC20Upgradeable tokenAddress) public view returns (uint256) {
        return IERC20Upgradeable(tokenAddress).balanceOf(address(this));
    }

    function withdrawTokens(IERC20Upgradeable tokenAddress, uint256 percentageWithdrawl) external onlyOwner {
        ...
        bool success = IERC20Upgradeable(tokenAddress).transfer(owner(), balance.mul(percentageWithdrawl).div(100));
