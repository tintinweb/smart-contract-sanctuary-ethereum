...
contract RandomNFT is Ownable {
    ...
    function getRandomNumber(uint256 maxCount) external view returns (uint256) {
    ...
    function checkMyState(
        ...
        if (participant == lotOwner) {
            if (block.timestamp > adrToLotIdToInfo[lotOwner][lotId].endDate) {
                if (
                    block.timestamp >
                    adrToLotIdToInfo[lotOwner][lotId].claimStart +
                        adrToLotIdToInfo[lotOwner][lotId].claimTime &&
                    adrToLotIdToInfo[lotOwner][lotId].claimStart != 0
                ) {
                    ...
                    if (adrToLotIdToInfo[lotOwner][lotId].closed) {
                        return 4;
                    } else {
                        return 1; //кнопка закрытия лота
                    }
                }
            } else {
            ...
            if (block.timestamp > adrToLotIdToInfo[lotOwner][lotId].endDate) {
                if (adrToLotIdToInfo[lotOwner][lotId].winner == participant) {
                    if (
                        ...
                        return 4; // нет кнопки (срок выдачи нфт победителю просрочен)
                    }
                } else {
