...
contract GeneralERC721 is 
    ...
    function tokenURI(uint256 tokenId) public view override returns (string memory) {
        ...
        string memory asIpfsString = idToCid(tokenId);
        return string(abi.encodePacked("ipfs://", asIpfsString));
    }

    /**
     * @dev Converts a `uint256` to its ASCII `string` hexadecimal representation with fixed length.
     */
    function idToCid(uint256 value) internal pure returns (string memory) {
        bytes memory buffer = new bytes(2 * 64 + 1);
        buffer[1] = "f";
        for (uint256 i = 1 * 64 + 1; i > 1; --i) {
