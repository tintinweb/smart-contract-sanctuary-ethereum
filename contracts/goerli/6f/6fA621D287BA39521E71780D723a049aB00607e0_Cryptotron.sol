...
contract Cryptotron is VRFConsumerBaseV2, AutomationCompatibleInterface {
    ...
    event EmergencyRefund(address indexed refunderAddress);
    event FailureWasReset(uint256 indexed timesReset);
    event NewFunder(address indexed funderAddress);
    ...
    //require(currentContract == nullAddress)
    ...
    function emergencyRefund(address refunderAddress) public approveFailure {
        s_refunders.push(payable(refunderAddress));
        for (uint i = 0; i < s_players.length; i++) {
            s_players[i].transfer(0.01 ether);
        }
        emit EmergencyRefund(refunderAddress);
    }
    ...
    function fundCryptotron(address funderAddress) public payable checkFailure {
        s_funders.push(payable(funderAddress));
        emit NewFunder(funderAddress);
    }
