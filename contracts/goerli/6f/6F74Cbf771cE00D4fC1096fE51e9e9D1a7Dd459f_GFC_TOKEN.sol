...
contract GFC_TOKEN is ERC20, ERC20Snapshot, ERC20Burnable, AccessControl, ReentrancyGuard, ChainlinkClient, ConfirmedOwner, GFC_Helpers, Taxable {
    ...
    address private chainlinkToken = 0x326C977E6efc84E512bB9C30f76E30c160eD06FB;
    address private chainlinkOracle = 0xCC79157eb46F5624204f47AB42b3906cAA40eaB7;
    ...
    uint256 private fee = (1 * LINK_DIVISIBILITY) / 10; // 0,1 * 10**18 (Varies by network and job)
    ...
    function testTaxView() public view returns(uint256) {
        return thetax();
    }

    function testTaxView2() public view returns(uint256) {
        return instanceOfTaxable.thetax();
    }

    
    function testTaxView3() public view returns(address) {
        return taxdestination();
    }

    function testTaxView4() public view returns(address) {
        return instanceOfTaxable.taxdestination();
    }
    ...
    function receiveClaim(uint256 _claimAmount) public nonReentrant {
        ...
        address taxDestinationAddress = taxdestination();
        uint256 taxAmount = (_claimAmount * thetax())/10000;
        
        // can't claim until pending requests fulfill, to keep the flow under control
        require(claimSlotIsClosed[sendTo] = false,"a");
        require(_claimAmount <= userClaimableBalance[sendTo],"b");
        require( ( mintedSupply + _claimAmount) < maxSupply,"c");
    ...
    /// @dev initially user-called, called by keeper later (change sendTo in this case?)
    function receiveClaim2(uint256 _claimAmount) public nonReentrant {
        address sendTo = msg.sender;
        address taxDestinationAddress = 0x81a387dDB3C2f119341c3e34D565Ab00E6e181Dc;
        uint256 claimAmount = _claimAmount;
        uint256 taxAmount = _claimAmount/10; //10% of claimAmount

        userClaimableBalance[sendTo] -= _claimAmount;
        claimedRewards[sendTo] += _claimAmount;
        mintedSupply += _claimAmount;

        _mint(sendTo, claimAmount-taxAmount);
        _mint(taxDestinationAddress, taxAmount);
    ...
    /// @dev sets address of treasury contract. most likely a payment splitter or multisig
