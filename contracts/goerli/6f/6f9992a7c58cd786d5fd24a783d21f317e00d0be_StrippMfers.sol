...
using Address for address;
    ...
    function mintByFungibleTokens(uint256 qty, uint256 flag, uint256 amount) external reentryLock {
        requireAll(qty);
        require(flag == 1 || flag == 2, "Wrong Fungible token used to mint."); 
        if(flag == 1){
            require(reservedLove - qty >= 0, "Reserved Love tokens end.");
            require(loveTokenAddress.allowance(msg.sender, address(this)) >= amount, "Error");
            //loveTokenAddress.transferFrom(msg.sender, address(this), lovePrice * qty);
            loveTokenAddress.transferFrom(msg.sender, address(this), amount);
            reservedLove -= qty;
        }else if(flag == 2) {
            require(reservedGas - qty >= 0, "Reserved GasFactory tokens end.");
            require(gasFactoryAddress.allowance(msg.sender, address(this)) >= amount, "Error");
            //gasFactoryAddress.transferFrom(msg.sender, address(this), gasPrice * qty);
            gasFactoryAddress.transferFrom(msg.sender, address(this), amount);
            reservedGas -= qty;
        }
        tokensMint(qty);
    }
