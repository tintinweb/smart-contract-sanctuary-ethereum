...
import "../interfaces/IStakingCreator.sol";
import "../staking/CeresStakingCOL.sol";

contract StakingCreator is IStakingCreator {

    address public asc;
    address public crs;
    address public veCrs;
    address public stakerBook;
    address public ceresFactory;

    event StakingCreated(address indexed token, address indexed staking);

    constructor(address _asc, address _crs, address _veCrs, address _stakerBook, address _ceresFactory) {
        asc = _asc;
        crs = _crs;
        veCrs = _veCrs;
        stakerBook = _stakerBook;
        ceresFactory = _ceresFactory;
    }

    function createStaking(address token) external override returns (address staking) {
        require(msg.sender == ceresFactory, "StakingCreator: Only CeresFactory!");
        staking = address(new CeresStakingCOL());
        CeresStakingCOL(staking).initialize(ceresFactory, asc, crs, veCrs, token, stakerBook);
        emit StakingCreated(token, staking);
    }
}
...
contract CeresStakingCOL is CeresStakingRewards, IMintReceiver {
    ...
    function initialize(address _factory, address _asc, address _crs, address _veCrs, address _stakingToken, 
        address _stakerBook) public initializer {
    ...
    function mint() external {
        require(accountStakingRatio(msg.sender) > ICeresMiner(factory.ceresMiner()).minMinerStakingRatio(),
