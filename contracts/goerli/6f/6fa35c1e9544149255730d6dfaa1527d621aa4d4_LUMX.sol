...
contract LUMX is Ownable, ERC721A {
    ...
    uint256 public maxPerAddressWL = 1;
    uint256 public maxSupply = 120;

    // Timeframes
    uint256 public windowOpens = 1655918414;
    uint256 public presaleWindowOpens = 1661973184;
    uint256 public presaleWindowCloses = 1693509184;
    // Metadata uris
    ...
    function mintPreSale(uint256 quantity, bytes calldata signature) external payable mintConditions(quantity) {
        ...
        require(_numberMinted(msg.sender) + quantity <= (maxPerAddress), "You can only mint 2 (3) tokens during presale (OG role has one more).");
        
        _mint(msg.sender, quantity);
    }

    function mint(uint256 quantity, bytes calldata signature) external payable mintConditions(quantity) {
        require(block.timestamp >= windowOpens, "Purchase window closed.");
        require(_numberMinted(msg.sender) + quantity <= maxPerAddress, "You can only mint 3 tokens.");
        require(verifySignature(signature), "Account not eligible for claim.");
    ...
    function setMax(uint256 _maxPerAddress, uint256 _maxPerAddressWL) external onlyOwner {
