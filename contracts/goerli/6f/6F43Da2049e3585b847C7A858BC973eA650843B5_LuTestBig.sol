...
contract LuTestBig {
  ...
  mapping(string => IParts) partsContract;
  
  function register(string memory key, IParts _parts) external {
      partsContract[key] = _parts;
  }
  
  function getParts(string memory key) external view returns(string memory output) {
      (uint16[4] memory sizes, bytes[] memory path, string[] memory fill,  uint8[] memory stroke) = partsContract[key].svgData();
  ...
  function main() external view returns(string memory output) {
      (uint16[4] memory sizes, bytes[] memory path, string[] memory fill,  uint8[] memory stroke) = parts.svgData();
...
interface IParts {
    function svgData() external pure returns(uint16[4] memory sizes, bytes[] memory paths, string[] memory fill, uint8[] memory stroke);
}
