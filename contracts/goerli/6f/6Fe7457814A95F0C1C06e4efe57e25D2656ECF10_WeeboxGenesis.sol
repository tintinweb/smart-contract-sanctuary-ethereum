...
contract SalesTimestamps is Ownable {
  ...
  modifier isInValidPreSalePeriod() {
    ...
    if (block.timestamp > preSaleEndTimeInSeconds) revert PreSaleHasEnded();
    _;
  }
  /**
  ...
  SalesTimestamps
{
  ...
  // mint schedule:
  // 1: pre sale with ??? allowlist spots with a maximum 2 mints per wallet per transaction,
  //    which is equivalent to a capped 2250(30% of total collection) mint limit
  //     - TODO: are the allowlist spots the same amount as total allowlist cap divided by 2?
  //
  // 2: dev mint reserves 750 (10% of total collection) tokens after pre sale concludes
  //
  // 3: public sale after dev mint concludes with a maximum of 4 mints per wallet per transaction
  //    until all 7500 tokens are minted
  ...
  uint256 private constant COLLECTION_SIZE = 7500;
  uint256 private constant MAX_DEV_MINT_LIMIT = 750; // 10% of total collection
  ...
  // following minting functions inspired by Azuki contract and maru.eth
