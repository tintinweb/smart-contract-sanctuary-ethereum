...
contract PayrollNFT is ERC721, Ownable, Signature {
    /**
     * @dev
    ...
    /// @dev Mapping to record userID linked to tokenID
    /// tokenIDに対応するユーザーIDを記録するMapping
    mapping(uint256 => string) private _tokenIdtoUserId;

    /// @dev Mapping to record minted signature information
    /// mint済みのユーザーIDを記録するMapping
    mapping(string => bool) private _isUserIdMinted;
    ...
    /**
     * @dev
     * - mint時にtokenIdとuserIdを記録する
     */
    event mintRecord(uint256 indexed tokenId, string userId);

    constructor(string memory baseURI, address signatureAddress)
        ERC721("DFGC Payroll NFT", "DFGCNFT")
    {
        __baseURI = baseURI;
    ...
    function mint(
        ...
        require(!_isUserIdMinted[jointedId], "already minted");
        ...
        // mintした情報をMappingに格納
        _isUserIdMinted[jointedId] = true;
        _tokenIdtoUserId[tokenId] = jointedId;
    ...
    function burn(uint256 tokenId) public virtual {
        ...
        _isUserIdMinted[_tokenIdtoUserId[tokenId]] = false;
        _tokenIdtoUserId[tokenId] = "";
     ...
     * - NFTURIのbaseURIをコントラクトオーナーのみ変更可能
     */
    ...
    /**
     * @dev
     * - You can get the address to sign on the backend
     * - バックエンドで署名するアドレスを取得できます。
     */
    ...
    /**
     * @dev
     * - It is possible to obtain whether userId is minted or not.
     * - userIdがmintされているかどうか取得できます。
     */
    function isMintedUserId(string calldata userId) public view returns (bool) {
        return _isUserIdMinted[userId];
    }

    /**
     * @dev
     * - You can get tokenId from userId.
     * - userIdからtokenIdを取得できます。
     */
    function getMintedTokenId(string calldata userId) external view returns (uint256) {
        require(isMintedUserId(userId), "not minted");
        uint256 tokenId = 0;
        for (uint256 i = 1; i <= _tokenIds.current(); i++) {
            if(keccak256(abi.encodePacked(_tokenIdtoUserId[i])) == keccak256(abi.encodePacked(userId))){
                tokenId = i;
                break;
            }
        }
