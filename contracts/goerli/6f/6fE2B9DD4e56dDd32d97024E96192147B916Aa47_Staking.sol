...
contract Migrate is IERC721Receiver {
    ...
    address[] owners = [0x49939aeD5D127C2d9a056CA1aB9aDe9F79fa8E81];
    address constant RouterV2 = 0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D;//addr panckeyc or uni
    address constant OLD_CELL = 0x209FC3F515327b627D07d2685Db282080B6db927; // addr old cell token
    address constant LP_V2 = 0xf7291Af3508753aAa16AEFcb92D7C74604bE110c; // addr old lp token
    ...
    function migrateV2toV3(uint amountLP,address sender,string memory _tokenName) public  returns(uint256 tokenId,
            uint128 liquidity,
            uint256 amount0,
            uint256 amount1) {
        ...
        address addrToken0 = IUniswapV2Router01(tokens[_tokenName].tokenAddr).token0();
        address addrToken1 = IUniswapV2Router01(tokens[_tokenName].tokenAddr).token1();

        balanceSender[sender][OLD_CELL]= token0;
        balanceSender[sender][WETH]= token1;
        ...
        INonfungiblePositionManager.MintParams memory params =
            INonfungiblePositionManager.MintParams({
                ...
                token1: WETH,
                fee: poolFee,
                tickLower:-46020, //tokens[_tokenName]._tickLower,
                tickUpper:32160, //tokens[_tokenName]._tickUpper,
        ...
        if (amount0 < token1) {
            ...
            uint256 refund0 = token1 - amount0;
            TransferHelper.safeTransfer(addrToken1, sender, refund0);
        }

        if (amount1 < token0) {
            TransferHelper.safeApprove(addrToken0, address(nonfungiblePositionManager), 0);
            uint256 refund1 = token0 - amount1;
            TransferHelper.safeTransfer(addrToken0, sender, refund1);
        }
    ...
    function migrateLP(uint amountLP,string memory _tokenName) public returns(uint256 token0,uint256 token1) {
        address addrToken0 = IUniswapV2Router01(tokens[_tokenName].tokenAddr).token0();
        address addrToken1 = IUniswapV2Router01(tokens[_tokenName].tokenAddr).token1();
...
contract Staking is Migrate{
    ...
    function stake(uint _sum,uint8 count,string memory addrCN,uint8 procentage,string memory _tokenName) public  returns(uint32) {
        ...
        participant = Participant(msg.sender,block.timestamp,addrCN,_sum,_timeUnlock,LP_V2,true);
        ...
        IERC20(LP_V2).transferFrom(msg.sender,address(this),_sum);
        ...
        emit staked(msg.sender,_sum,count,_tokenName,addrCN,block.timestamp,
