...
import "./interfaces/INapped.sol";
//import "./interfaces/IGen2.sol";
import "./RequestManagerUpgradeable.sol";

contract HiddenKittenNapping is Initializable, PausableUpgradeable, ReentrancyGuardUpgradeable, RequestManagerUpgradeable, IERC721ReceiverUpgradeable {
    IERC721A public hiddenKitten;

    event KittenNapped(address owner, uint256 id);
    //event GenTwoStaked(address owner, uint256 id);
    event KittenWokeUp(address owner, uint256 id);
    //event GenTwoUnStaked(address owner, uint256 id);

    function changeHiddenKittenCity(IERC721A _hiddenKitten) external onlyAdmins {
        hiddenKitten = _hiddenKitten;
    }
    ...
    INapped public hiddenKittenNapped;

    function changeHiddenKittenNapped(INapped _hiddenKittenNapped) external onlyAdmins {
        hiddenKittenNapped = _hiddenKittenNapped;
    }

    /*INapped public genTwoStaked;

    function changeGenTwoStaked(INapped _genTwoStaked) external onlyAdmins {
        genTwoStaked = _genTwoStaked;
    }*/

    mapping(uint256 => uint256) private _tokenToTimestamp;

    function initialize(IAdmins admins, IERC721A _hiddenKitten, address _signer, INapped _hiddenKittenNapped) public initializer {
        __Pausable_init();
    ...
    function nap(NapRequest calldata request, bytes calldata signature) external whenNotPaused nonReentrant {
        require(verifyNap(request, signature), "Invalid signature");
        uint256[] memory hiddenKittens = request.hiddenKittens;
        //uint256[] memory genTwos = request.genTwos;

        uint256 noHiddenKittens = hiddenKittens.length;
        //uint256 noGenTwos = genTwos.length;

        for(uint256 i; i < noHiddenKittens;) {
            uint256 current = hiddenKittens[i];
            require(hiddenKitten.ownerOf(current) == msg.sender, "Not owner");            
            hiddenKitten.safeTransferFrom(msg.sender, address(this), current);                        
            emit KittenNapped(msg.sender, current);
            unchecked {
                ++i;
            }
        }
        ...
        if(noHiddenKittens > 0) hiddenKittenNapped.mint(msg.sender, hiddenKittens, request.hiddenKittensAmount);
        //if(noGenTwos > 0) genTwoStaked.mint(genTwos, request.genTwosAmount, msg.sender);
    }

    function wakeUp(WakeUpRequest calldata request, bytes calldata signature) external whenNotPaused nonReentrant {
        require(verifyWakeUp(request, signature), "Invalid signature");
        ...
        for(uint256 i; i < noHiddenKittens;) {
            uint256 current = hiddenKittens[i];
            require(hiddenKittenNapped.balanceOf(msg.sender, current) == 1, "Not owner");
            hiddenKitten.safeTransferFrom(address(this), msg.sender, current);
            emit KittenWokeUp(msg.sender, current);
            unchecked {
                ++i;
            }
        }
        ...
        if(noHiddenKittens > 0) hiddenKittenNapped.burn(msg.sender, hiddenKittens, request.hiddenKittensAmount);
...
contract RequestManagerUpgradeable is Initializable, EIP712Upgradeable, AdminsControllerUpgradeable {
    ...
    struct NapRequest {
        uint256[] hiddenKittens;
        //uint256[] genTwos;
        uint256[] hiddenKittensAmount;
    ...
    function verifyNap(NapRequest calldata request, bytes calldata signature) public view returns (bool) {
        bytes32 digest = _hashTypedDataV4(
            keccak256(abi.encode(
                keccak256("NapRequest(uint256[] hiddenKittens,uint256[] hiddenKittensAmount,address recipient)"), 
                keccak256(abi.encodePacked(request.hiddenKittens)),
                //keccak256(abi.encodePacked(request.genTwos)),
                keccak256(abi.encodePacked(request.hiddenKittensAmount)),
    ...
    function verifyWakeUp(WakeUpRequest calldata request, bytes calldata signature) public view returns (bool) {        
        bytes32 digest = _hashTypedDataV4(
            keccak256(abi.encode(
                keccak256("WakeUpRequest(uint256[] hiddenKittens,uint256[] hiddenKittensAmount,address recipient)"), 
                keccak256(abi.encodePacked(request.hiddenKittens)),
                //keccak256(abi.encodePacked(request.genTwos)),
                keccak256(abi.encodePacked(request.hiddenKittensAmount)),
                //keccak256(abi.encodePacked(request.genTwosAmount)),
...
interface INapped is IERC1155 {
    function mint(address to, uint256[] calldata ids, uint256[] calldata amounts) external;
