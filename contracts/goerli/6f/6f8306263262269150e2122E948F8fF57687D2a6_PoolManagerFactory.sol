...
interface IPoolManagerDeployer {
  ...
  /**
   * @notice Returns the pool manager bytecode hash
   * @return _poolManagerBytecodeHash The pool manager bytecode hash
   */
  function POOL_MANAGER_BYTECODE_HASH() external view returns (bytes32 _poolManagerBytecodeHash);
...
contract PoolManagerFactory is IPoolManagerFactory, Roles {
  ...
  uint256 internal constant _ROUNDING_THRESHOLD = 100;
    ...
    payable
  {
    ...
    POOL_MANAGER_BYTECODE_HASH = _poolManagerDeployer.POOL_MANAGER_BYTECODE_HASH();
  ...
  {
    (IUniswapV3Pool _pool,) =
      PriceLib.calculateTheoreticalAddress(WETH, _token, _fee, UNISWAP_V3_FACTORY, POOL_BYTECODE_HASH);
    IPoolManager _expectedPoolManager =
      PriceLib.getPoolManager(POOL_MANAGER_DEPLOYER, POOL_MANAGER_BYTECODE_HASH, _pool);

    // Reverts in case the pool manager was already created by this factory
    if (isChild[_expectedPoolManager]) {
      revert PoolManagerFactory_ExistingPoolManager();
    }
    ...
    if (_totalInWETH + _ROUNDING_THRESHOLD < minEthAmount) {
      revert PoolManagerFactory_SmallAmount();
    }
  ...
  function setDefaultTokenFee(IERC20 _token, uint24 _fee) external onlyRole(DEFAULT_ADMIN_ROLE) {
    ...
    if (_i == _tokenFeesLengthMinus1 && _tokenFees[_token][_i] != _fee) {
      revert PoolManagerFactory_InvalidPool();
    }
  ...
  function getPoolManagerAddress(IERC20 _token, uint24 _fee)
    ...
    returns (IPoolManager _theoreticalPoolManagerAddress)
  {
    (IUniswapV3Pool _theoreticalPoolAddress,) =
      PriceLib.calculateTheoreticalAddress(WETH, _token, _fee, UNISWAP_V3_FACTORY, POOL_BYTECODE_HASH);
    _theoreticalPoolManagerAddress =
      PriceLib.getPoolManager(POOL_MANAGER_DEPLOYER, POOL_MANAGER_BYTECODE_HASH, _theoreticalPoolAddress);
  }
  ...
  function getWethPoolAddress(IERC20 _token, uint24 _fee)
    ...
    returns (IUniswapV3Pool _theoreticalAddress, bool _isWethToken0)
  {
    (_theoreticalAddress, _isWethToken0) =
      PriceLib.calculateTheoreticalAddress(WETH, _token, _fee, UNISWAP_V3_FACTORY, POOL_BYTECODE_HASH);
  }

  /// @inheritdoc IPoolManagerFactory
  function getPoolManagers(IERC20 _token, uint24[] calldata _feeTiers)
    external
    view
    returns (address[] memory _poolManagerAddresses)
  {
    ...
    while (_i < _feeTiersCount) {
      (IUniswapV3Pool _theoreticalPoolAddress,) =
        PriceLib.calculateTheoreticalAddress(WETH, _token, _feeTiers[_i], UNISWAP_V3_FACTORY, POOL_BYTECODE_HASH);
      IPoolManager _expectedPoolManager =
        PriceLib.getPoolManager(_poolManagerDeployer, POOL_MANAGER_BYTECODE_HASH, _theoreticalPoolAddress);
  ...
  function setLockManagerFactory(ILockManagerFactory _lockManagerFactory) external onlyRole(FACTORY_SETTER_ROLE) {
    if (address(_lockManagerFactory) == address(0)) {
      revert PoolManagerFactory_ZeroAddress();
    }
    lockManagerFactory = _lockManagerFactory;
  ...
  function acceptOwnership() external {
    ...
    if (msg.sender != _newOwner) {
      revert PoolManagerFactory_InvalidPendingOwner();
    }
  ...
  function setFeeManager(IFeeManager _feeManager) external onlyRole(FEE_MANAGER_SETTER_ROLE) {
    if (address(_feeManager) == address(0)) {
      revert PoolManagerFactory_ZeroAddress();
    }
    feeManager = _feeManager;
  ...
  function setFeeCollectorJob(IFeeCollectorJob _feeCollectorJob) external onlyRole(FEE_COLLECTOR_SETTER_ROLE) {
    if (address(_feeCollectorJob) == address(0)) {
      revert PoolManagerFactory_ZeroAddress();
    }
  ...
  function setMinEthAmount(uint256 _minEthAmount) external onlyRole(MIN_ETH_AMOUNT_SETTER_ROLE) {
    if (_minEthAmount == 0) {
      revert PoolManagerFactory_InvalidMinEthAmount();
    }
  ...
  /**
   * @notice Returns the total amount in WETH for a given amount of liquidity, the current pool price and the prices at the tick boundaries
   * @param _sqrtPriceX96 A sqrt price representing the current pool prices
   ...
   */
  function _getTotalInWETHForLiquidity(uint160 _sqrtPriceX96, uint24 _fee, uint128 _liquidity, IERC20 _token)
    internal
    view
    returns (uint256 _totalInWeth)
  {
