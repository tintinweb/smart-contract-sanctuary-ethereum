...
contract DebtShareAggregator is BaseAggregator, NonblockingLzApp {
    using BytesLib for bytes;
    using EnumerableSet for EnumerableSet.Uint16Set;

    uint16 public constant PT_MINT = 1; // Mint signal
    uint16 public constant PT_BURN = 2; // Burn signal

    uint256 private _totalSupply;
    mapping(address => uint256) _balanceOf;

    event Mint(uint16 indexed srcChainId, address indexed account, uint256 amount);
    event Burn(uint16 indexed srcChainId, address indexed account, uint256 amount);
    ...
    function _nonblockingLzReceive(uint16 _srcChainId, bytes memory _srcAddress, uint64 _nonce, bytes memory _payload) internal override {
        ...
        if (packetType == PT_MINT) {
            ...
            emit Mint(_srcChainId, account, amount);
        } else if (packetType == PT_BURN) {
            (, bytes memory accountBytes, uint256 amount) = abi.decode(_payload, (uint16, bytes, uint256));
            address account = accountBytes.toAddress(0);

            _burn(account, amount);
            emit Burn(_srcChainId, account, amount);
        } else {
    ...
    // send messages
    function sendMint(address account, uint256 amount) external onlySrcContract {
        _mint(account, amount);

        bytes memory lzPayload = abi.encode(PT_MINT, abi.encodePacked(account), amount);
    ...
    function sendBurn(address account, uint256 amount) external onlySrcContract {
        _burn(account, amount);

        bytes memory lzPayload = abi.encode(PT_BURN, abi.encodePacked(account), amount);
