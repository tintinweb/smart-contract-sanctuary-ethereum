...
contract Fundraising01ERC20 is IFundraising01ERC20, Fundraising01Base, Vestable {
    ...
    constructor(
        ...
        Vestable(_periods)
    {
        if (address(_tokenToSell) == address(0)) revert CannotCreateFundraisingIfTokenToSellIsZeroAddressError();
    ...
    function claim(uint256 amountToClaim) external override notFailed {
        ...
        if (amountToClaim > maxAmountToClaim) revert CannotClaimIfExceedsClaimableMaximumError();
...
abstract contract BuildRounds is IBuildRounds {
    ...
    function _beforePushBuildRound(uint256) internal virtual {
        if (hasPendingBuildRound()) revert CannotPushRoundIfAnotherRoundIsPendingError();
...
abstract contract Fundraising01Base is Refunds, SaleRounds, BuildRounds, IFundraising01Base, Ownable {
    ...
    constructor(
        ...
        if (address(_tokenToRaise) == address(0)) revert CannotCreateFundraisingIfTokenToRaiseIsZeroAddressError();
        if (_amountToSellSoft == 0) revert CannotCreateFundraisingIfAmountToSellSoftIsZeroError();
    ...
    function invest(uint256 amount, bytes memory data) public virtual override {
        ...
        for (uint256 i = 0; i < saleRounds.length; i++) {
            ...
            passed = passed && saleRounds[i].condition.check(abi.encodePacked(msg.sender, data));
        }
    ...
    function executeBuildRound() external override(IBuildRounds) {
        if (buildRounds.length > 1 && !hasPendingBuildRound()) revert CannotExecuteRoundIfThereIsPendingProposalError();
        BuildRound storage round = buildRounds[buildRounds.length - 1];
        if (round.isExecuted) revert CannotExecuteRoundIfProposalIsExecutedError();
    ...
    function isRefundAvailable() public view override returns (bool) {
        // Refund becomes available either after two months of fundraising living or after the first proposal
    ...
    function _beforeRefundStart(bytes calldata) internal virtual {
        if (!isRefundAvailable()) revert CannotRefundStartIfRefundIsNotAvailableYetError();
    }
    /**
    ...
    function refundCancel() external override {
        if (investorsData[msg.sender].refundStartedAt == 0) revert CannotCancelRefundIfNotStartedError();
    ...
    function isFailed() public view override returns (bool) {
        ...
        // `SALE_STAGE_MIN_PERIOD` has passed and the `amountToSellSoft` was not reached
...
abstract contract SaleRounds is ISaleRounds {
    ...
    // TODO: Check conditions here too
    function applicableSaleRoundsOf() external view override returns (ISaleRounds.SaleRound[] memory) {
        uint256 count = 0;
        for (uint256 i = 0; i < saleRounds.length; i++) {
        ...
        ISaleRounds.SaleRound[] memory applicableSaleRounds = new ISaleRounds.SaleRound[](count);
        uint256 index = 0;
        for (uint256 i = 0; i < saleRounds.length; i++) {
            if (saleRounds[i].startsAt <= block.timestamp && block.timestamp <= saleRounds[i].endsAt) {
                applicableSaleRounds[index++] = saleRounds[i];
            }
        }

        return applicableSaleRounds;
    }
}
...
/// @title Vestable
/// @author Vladyslav Dalechyn <h0tw4t3r.eth>
/// @notice Abstract contract for calculating claimable amounts with unlock periods.
abstract contract Vestable is IVestable {
    ...
    /// @dev Calculates the releaseable amount for a given timestamp and deposited amount.
...
interface IFundraising01BaseActions {
    ...
    /// @dev If some tokens were transferred directly to the contract, they can be claimed.
...
/// @author Vladyslav Dalechyn <h0tw4t3r.eth>
/// @notice State of the sale that will never change and encoded in the contract.
interface IFundraising01BaseConstants {
    /// @notice The time period at which a sell should be up and running.
    /// If this period has lasted after the sale start, the sale will be destroyed.
    /// @return Start period.
...
interface IFundraising01BaseImmutableState {
    ...
    /// If sale doesn't sell the `amountToSellSoft` – it is considered as failed.
    /// @return Amount
...
interface ISaleRounds {
    ...
    function applicableSaleRoundsOf() external view returns (SaleRound[] memory);
}
