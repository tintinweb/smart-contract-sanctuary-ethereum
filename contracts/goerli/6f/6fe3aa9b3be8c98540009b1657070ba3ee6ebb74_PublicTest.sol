...
contract PublicTest is Ownable, ReentrancyGuard, Pausable, ERC721AQueryable {
    /* ============ Constants ============ */
    ...
    constructor(address _treasury, uint256 _mintPirce)
        ERC721A("Public Test", "PT")
    {
        require(
            _treasury != address(0),
            "PublicTest: treasury is the zero address"
        );
        // require(_mintPirce > 0, "PublicTest: mint price is 0");
        treasury = _treasury;
        ...
        whenNotPaused
    {
        require(
            ...
            "PublicTest: not start"
        );
        require(
            mintVerify(msg.sender, _merkleProof),
            "PublicTest: Address is not in allow list"
        );
        require(_totalMinted() + 1 <= MAX_SUPPLY, "PublicTest: Over max supply");
        require(msg.value >= mintPrice, "PublicTest: Insufficient ETH");

        // require(
        //     _numberMinted(msg.sender) == 0,
        //     "PublicTest: Over max mint per wallet"
        // );
        ...
        whenNotPaused
    {
        ...
        require(_totalMinted() + quantity <= MAX_SUPPLY, "PublicTest: Over max supply");

        require(msg.value >= mintPrice * quantity, "PublicTest: Insufficient ETH");
    ...
    function withdraw() public {
        require(
            msg.sender == treasury,
            "PublicTest: Caller is not the treasury"
        );
        (bool success, ) = treasury.call{value: address(this).balance}("");
        require(success, "PublicTest: Transfer failed.");
    }
    ...
    modifier onlyEOA() {
        require(
            ...
            "PublicTest: The caller is another contract"
        );
        _;
    }
