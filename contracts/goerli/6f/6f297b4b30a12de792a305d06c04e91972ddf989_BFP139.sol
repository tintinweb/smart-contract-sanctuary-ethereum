...
contract BFP139 is IInvite {
    ...
    mapping(address => bool) public revoteBlackList;//复投黑名单地址
    ...
    */
    ...
    constructor() {
        ...
        reInvestmentCollectionaddress = 0x21b0B9053DA81F00C2A7264B52c10B1118041E4E;//复投归集地址
    }
    ...
    function blind(address  parentAddress,address  sonAddress) public{
        ...
        //第一代
        uint256 directPushesNumber1 = inviteNumRecords[markAddress][0];// 获取直接邀请数量
    ...
    */
    function updateAddressData(address markAddress_th,uint inviteNumRecords_subscript,uint256 directPushesNumber) private {
        inviteNumRecords[markAddress_th][inviteNumRecords_subscript]++; // 第六代+1
        // remainNumberRerolls[markAddress_th]+=5;//复投次数+5
        remainNumberRerolls[markAddress_th]+=1;//复投次数+5

        //判断是否复投黑名单
        if(!revoteBlackList[markAddress_th]){
            //判断是否复投白名单用户
    ...
    function pay(address parentAddress) public payable {
        require(parentAddress != address(0), 'Invite: 0001'); // 不允许上级地址为0地址
        require(msg.value >= 13900000000000000, 'Invite: 0001'); // 不允许上级地址为0地址
    ...
    // 重复排号
      function payReply() public  payable onlyOwner{
        ...
        //判断用户是否满足复投条件
        if((whiteList[rank[0]] || revoteWhiteList[rank[0]] || remainNumberRerolls[rank[0]] > 0) && !revoteBlackList[rank[0]]){
