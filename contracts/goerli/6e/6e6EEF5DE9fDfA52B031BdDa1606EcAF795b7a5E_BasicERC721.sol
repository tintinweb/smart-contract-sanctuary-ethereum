    ...
    ITokenERC721
{
    ...
    function mintTo(address _to, string memory _uri) external onlyRole(MINTER_ROLE) {
        uint _tokenId = nextTokenIdToMint;
    ...
    function mintToBatch(address[] memory _tos, string[] memory _uris) external onlyRole(MINTER_ROLE) {
        uint _tokenId = nextTokenIdToMint;
        uint length = _tos.length;
    ...
    function safeBatchTransferFrom(address[] memory froms, address[] memory tos, uint256[] memory tokenIds) external {
    ...
    function tokenURI(uint256 tokenId) public view override returns (string memory) {
        ...
        return bytes(uri[tokenId]).length > 0 ? uri[tokenId] : string(abi.encodePacked(baseURI, tokenId.toString()));
    }
