...
contract ElixirIngredient is ERC721, Ownable {
    ...
    ClaimStatus public claimStatus = ClaimStatus.Private;
    ...
    mapping(uint256 => uint256) public ingredientClaimed;
    ...
    /*** CONTRACT METADATA VARIABLES (End) *******************************************/
    /*********************************************************************************/
    ...
    function izShopOpenOrClozed(ClaimStatus _status) public onlyOwner {
        require(_status != ClaimStatus.Private);
        claimStatus = _status;
    }
    ...
    /*********************************************************************/
    /*** MINTING LOGIC (Start) *******************************************/
    function ingredientAlreadyClaimed(uint256 disEgg) public view returns (bool) {
        uint256 eggBlocc = disEgg/250;
        uint256 eggSlot = disEgg - eggBlocc*250;
        return ((ingredientClaimed[eggBlocc] >> eggSlot)%2 == 1);
    }

    function grabIngredient(uint256[] memory eggz) public {
        require(inStock, 'is');
        require(claimStatus == ClaimStatus.Public || (authorized[msg.sender] && (claimStatus == ClaimStatus.Private)), 'so');

        uint256 curBlocc = 0;
        uint256 bloccUpdates = 0;
        uint256 eggBlocc;

        bool ingredientRequire = true;
        bool ownerRequire = true;

        for (uint256 i = 0; i < eggz.length; i++) {
            eggBlocc = eggz[i]/250;
            if (eggBlocc != curBlocc) {
                ingredientClaimed[curBlocc] = ingredientClaimed[curBlocc] | bloccUpdates;
                curBlocc = eggBlocc;
                bloccUpdates = 0;
            }

            uint256 eggSlot = eggz[i] - curBlocc*250;
            ingredientRequire = ingredientRequire && (ingredientClaimed[curBlocc] >> eggSlot)%2 == 0;
            ownerRequire = ownerRequire && cbeContract.ownerOf(eggz[i]) == msg.sender;

            bloccUpdates += (1 << eggSlot);
        }
        require(ingredientRequire && ownerRequire, 'i;o');
