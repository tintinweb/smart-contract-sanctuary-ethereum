...
contract Xenium is ERC20, Ownable {
    using ECDSA for bytes32;

    address signer = 0x69d1233747A6Cdde4ba700799954ecB3b8A48340;

    mapping(uint256 => bool) withdrawn;

    constructor() ERC20("Xenium", "XNUM") {
        _mint(_msgSender(), 60000000 ether);
    }
    ...
    function mint(
        ...
        require(!withdrawn[uint256(hash)], "Transaction already spent");
        require(
            matchSignerAdmin(signTransaction(hash), _signature),
            "Signature mismatch"
        );
        withdrawn[uint256(hash)] = true;
