...
contract OptimizedStaking is ERC20, ReentrancyGuard, Ownable {
    ...
    function stake(uint128 _amount, uint32 lockTime) external nonReentrant {
        ...
        address erc20Address = address(token0);
        ...
        uint256 tax = CustomIERC20(erc20Address).checkUserTax(user);
        if (lockTime == 2419200) {
            userStakes[user].push(
                StakingObject(currentTimestamp, 2419200, _amount, _amount)
            );
            if (tax > 1) {
                CustomIERC20(erc20Address).changeUserSellTax(user, 1);
            }
            ...
            if (tax > 5) {
                CustomIERC20(erc20Address).changeUserSellTax(user, 5);
            }
            ...
            if (tax > 10) {
                CustomIERC20(erc20Address).changeUserSellTax(user, 10);
            }
        }

        uint256 amountToSwap = CustomIERC20(erc20Address).shouldSwap();
        if (amountToSwap > 0) {
            CustomIERC20(erc20Address).triggerSwapAndLiquify(amountToSwap);
        }
    ...
    function unstake() external nonReentrant {
        ...
        if (amountToSwap > 0) {
            CustomIERC20(address(token0)).triggerSwapAndLiquify(amountToSwap);
        }
