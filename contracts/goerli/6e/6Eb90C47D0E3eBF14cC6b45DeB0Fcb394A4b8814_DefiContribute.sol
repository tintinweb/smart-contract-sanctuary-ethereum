...
contract DefiContribute is Ownable {
    ...
    event NewDonee(string indexed cause, uint256 indexed doneeId, address indexed donee,  string message);
    ...
    event RedFlag(string indexed cause, uint256 indexed doneeId, address donee, address indexed admin);
    event ReadyToRemove(string indexed cause, uint256 indexed id, address indexed wallet);
    ...
    function becomeDonee(string memory _name, string memory _message) public onlyAcceptedDonee() {
        ...
        emit NewDonee(_name, doneesId, msg.sender, _message);
        doneesId++;
    }
