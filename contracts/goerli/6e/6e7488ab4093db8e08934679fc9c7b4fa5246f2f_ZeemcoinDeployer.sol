...
contract ZeemcoinDeployer {
    ...
    address public rewardsMerkleVault;
    ...
    constructor(address ms, uint256 timestamp, address feed, IERC20Upgradeable wbtc) {
        ...
        address vaultImplementationAddress = address(new ZeemcoinDistributionVaultMerkleVesting());
...

        {
            ...
            privateVault = _createProxy(vaultImplementationAddress, privateInitializeData);
...

        {
            ...
            bytes memory privateInitializeData = abi.encodeWithSelector(
                ZeemcoinDistributionVaultMerkleVesting.initialize.selector, ms, address(token), timestamp, timestamp + 1
            );
            advisorsVault = _createProxy(vaultImplementationAddress, privateInitializeData);
            token.safeTransfer(advisorsVault, advisorsAmount);
        }

        {
            address implementation = address(new ZeemcoinDistributionVaultMerkle());
            ...
            airdropsVault = _createProxy(implementation, airdropsInitializeData);
...

        {
            ...
            marketingVault = _createProxy(vaultImplementationAddress, privateInitializeData);
            token.safeTransfer(marketingVault, marketingAmount);
        }

        {
            address implementation = address(new ZeemcoinDistributionVaultAllocation());
            uint256 rewardsAmount = 3e7 ether;
            bytes memory rewardsInitializeData = abi.encodeWithSelector(
...

        {
            ...
            bytes memory initializeData = abi.encodeWithSelector(
                ZeemcoinDistributionVaultMerkle.initialize.selector, ms, address(token), rewardsVault
            );
            rewardsMerkleVault = _createProxy(implementation, initializeData);
        }

        {
            ...
            yieldVault = _createProxy(implementation, incentivesInitializeData);
...

        {
            ...
            bytes memory teamInitializeData = abi.encodeWithSelector(
                ...
                address(token),
                timestamp,
                timestamp + (15 * ONE_MONTH)
            );
            teamVault = _createProxy(vaultImplementationAddress, teamInitializeData);
...

        {
            ...
            wbtcAmount = tokenAmount * 1e8 * uint256(25) / uint256(100) / btcToUsdPrice; //8e6 MPOLY * WBTC_decimals * MPOLY_USD_PRICE / BTC_USD_PRICE
