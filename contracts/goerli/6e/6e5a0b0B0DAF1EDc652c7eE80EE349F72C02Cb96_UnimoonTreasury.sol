...
contract UnimoonTreasury is AccessControl, ReentrancyGuard {
    ...
    event SellAltcoin(address altcoin, uint256 id, uint256 sold);
    ...
    function purchaseNewAltcoin(
        ...
        address[] memory path
    ) external onlyRole(BACKEND_ROLE) swapExistTokens nonReentrant {
        require(
    ...
    function sellAltcoin(
        ...
        if (amounts[amounts.length - 1] > oldValue) {
            ...
            if (alloc > 0 && weight > 0 && stakingProfitPerc > 0) {
                TransferHelper.safeTransfer(
                    USDC,
                    address(STAKING),
                    ((amounts[amounts.length - 1] - oldValue) *
        ...
        emit SellAltcoin(altcoin, id, amountToSell);
    }
