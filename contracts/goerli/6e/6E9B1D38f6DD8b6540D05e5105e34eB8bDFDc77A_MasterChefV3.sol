...
contract MasterChefV3 is Ownable, ReentrancyGuard {
    ...
    /// @dev userPositionInfos[pid][tokenId] => UserPositionInfo
    mapping(uint256 => mapping(uint256 => UserPositionInfo)) userPositionInfos;
    ...
    uint256 public immutable MASTER_V2_PID;

    event Init();
    event AddPool(uint256 indexed pid, uint256 allocPoint, IUniswapV3Pool indexed pool);
    event SetPool(uint256 indexed pid, uint256 allocPoint);

    /// @param _MASTER_CHEF_v2 The PancakeSwap MCV2 contract address.
    /// @param _nonfungiblePositionManager the NFT position manager contract address.
    /// @param _MASTER_V2_PID The pool id of the dummy pool on the MCV1.
    constructor(
        IMasterChefV2 _MASTER_CHEF_v2,
        INonfungiblePositionManager _nonfungiblePositionManager,
        uint256 _MASTER_V2_PID
    ) {
        MASTER_CHEF_V2 = _MASTER_CHEF_v2;
        nonfungiblePositionManager = _nonfungiblePositionManager;
        MASTER_V2_PID = _MASTER_V2_PID;
    }
    ...
    /// @param dummyToken The address of the BEP-20 token to be deposited into MCV2.
    function init(IERC20 dummyToken) external onlyOwner {
        uint256 balance = dummyToken.balanceOf(msg.sender);
        require(balance != 0, "MasterChefV3: Balance must exceed 0");
        dummyToken.safeTransferFrom(msg.sender, address(this), balance);
        dummyToken.approve(address(MASTER_CHEF_V2), balance);
        MASTER_CHEF_V2.deposit(MASTER_V2_PID, balance);
        // MCV3 start to earn CAKE reward from current block in MCV1 pool
        emit Init();
    }
    ...
    function add(uint256 _allocPoint, IUniswapV3Pool _pool, bool _withUpdate) external onlyOwner {
        if (_withUpdate) {
            // massUpdatePools();
        }
        ...
        emit AddPool(poolInfo.length - 1, _allocPoint, _pool);
    }
    ...
    /// @notice Upon receiving a ERC721, creates the token deposit setting owner to `from`.
    function onERC721Received(
        ...
        require(msg.sender == address(nonfungiblePositionManager), 'not an Pancake V3 nft');
        (uint256 pid) = abi.decode(_data, (uint256));
        ...
        require(token0 == pool.tokenA && token1 == pool.tokenB && fee == pool.fee, "Wrong V3 pool");
        UserPositionInfo storage positionInfo = userPositionInfos[pid][_tokenId];
        require(positionInfo.liquidity == 0, "Already deposited");
        positionInfo.liquidity = liquidity;
        positionInfo.tickLower = tickLower;
        positionInfo.tickUpper = tickUpper;
        positionInfo.innerRewardGrowth = 0;
    ...
    /// @notice Deposit NFT to pool.
    /// @param _pid The id of the pool. See `poolInfo`.
    /// @param _tokenId Token Id of NFT to deposit.
    /// @param _from Address from which NFT token to deposit.
    function deposit(uint256 _pid, uint256 _tokenId, address _from) external nonReentrant {
        bytes memory data = abi.encode(_pid);
        nonfungiblePositionManager.safeTransferFrom(_from, address(this), _tokenId, data);
    }
    ...
    function withdraw(uint256 _pid, uint256 _tokenId, address _to) external nonReentrant {
        UserPositionInfo storage positionInfo = userPositionInfos[_pid][_tokenId];
        require(positionInfo.user == msg.sender, "Not owner");
