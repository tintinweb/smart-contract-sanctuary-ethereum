...
contract Sidus_PreSale is Ownable {
    ...
    mapping(uint256 => Price) public priceForCard;
    ...
    function registerForPreSale(
        ...
        require(userWhiteList[destinationAddress], "you are not in whitelist");
        uint256 currLimit;
        uint256 userDebt;
        ...
        for (uint256 i = 0; i < _wanted.length; i++) {
            ...
            require(
                priceForCard[_wanted[i].tokenId].value > 0,
            ...
            userDebt += _getETHAmountPerItem(_wanted[i]);
            require(
                (_wanted[i].limit +
    ...
    function setETHPrice(
        ...
        priceForCard[_tokenId].value = _newPriceValue;
        priceForCard[_tokenId].decimals = _newPriceDecimals;
    }

    function setMaxTotalSupply(uint256 _tokenId, uint256 _cardsCount)
        external
        onlyOwner
    {
    ...
    function getCardPrice(uint256 _tokenId)
        external
        view
        returns (Price memory)
    {
        return priceForCard[_tokenId];
    }

    function getUserLimitsInfo(address _user)
        external
        view
        returns (LimitItem[] memory)
    {
    ...
    function getUserLimitForCard(address _user, uint256 _tokenId)
        external
        view
        returns (uint256)
    {
    ...
    function getETHAmount(LimitItem[] calldata _wanted)
        external
        view
        returns (uint256 ethAmount)
    {
        for (uint256 i = 0; i < _wanted.length; i++) {
            ethAmount += _getETHAmountPerItem(_wanted[i]);
        }
    }
    ...
    function _getUserLimitForCard(address _user, uint256 _tokenId)
        internal
        view
    ...
    function _getUserLimitTotal(address _user)
        internal
        view
        returns (uint256 limit)
    {
    ...
    function _getETHAmountPerItem(LimitItem calldata _wanted)
        internal
        view
        returns (uint256)
    {
        return
            (_wanted.limit * priceForCard[_wanted.tokenId].value) /
            priceForCard[_wanted.tokenId].decimals;
    }
