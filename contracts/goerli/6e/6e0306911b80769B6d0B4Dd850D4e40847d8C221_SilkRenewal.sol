...
contract SilkRenewal is ERC721, Ownable {
    ...
    address private signer = 0x2d0ADAAcbdeD579A14390C69d563aC5c89FbC4fF;
    ...
    constructor(string memory tokenURI_, string memory contractURI_) ERC721("Silk", "SILK") {
        _tokenURI = tokenURI_;
    ...
    function verifyAllowlisted(address account,bytes memory signature) internal view returns(bool) {
    ...
    function allowlistMint(bytes memory signature) public payable contractGuard {
        uint256 tokenIndex = _tokenIdCounter.current() + 1;
        ...
        if(!verifyAllowlisted(msg.sender, signature)) revert NotAuthorized();
