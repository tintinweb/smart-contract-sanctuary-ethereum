...
contract Garage is Ownable, ERC1155Holder {
    ...
    **/
    struct LiquidityPool { 
        ...
        uint32 stakingRatePerYear; // decimal 6
        uint32 deadline;
        uint32 lateRatePerDay; // decimal 6
        uint32[] periodTimestamps; // array of unix timestamp
    ...
    **/
    ...
    uint32 DAY = 1 days; // 86400 seconds
    uint32 public GRACIOUS_TIME = 4 days;
    
    // uint32 DAY = 120; // 2 min
    // uint32 GRACIOUS_TIME = DAY;

    uint32 public MAXIMUM_LATE_FEE_PERCENTAGE = 100 * (10 ** 6);
    ...
    **/
    function setGraciousTime(uint32 newGraciousTime) public onlyOwner {
        GRACIOUS_TIME = newGraciousTime;
    }

    function setMaximumLateFee(uint32 newMaximumLateFee) public onlyOwner {
    ...
    **/
    ...
    function togglePauseWithdraw(uint64 poolId) public onlyOwner {
    ...
    **/
    ...
    function getPoolPeriodTimestamps(uint64 poolId, uint16 period) public view returns (uint256) {
    ...
    **/
    function createLiquidityPool(LiquidityPool calldata pool, uint8 disburseMode) external onlyOwner {
        ...
        poolNft.create(address(this), _nextPoolId.current(), pool.liquidity, "", "0x0");
        _nextPoolId.increment();
    }
    ...
    **/
    function stakeToPool(uint256 amount, uint64 poolId, uint256 erc20Id) public {
        ...
        poolNFT.safeTransferFrom(address(this), msg.sender, uint256(poolId), amount, "0x0");
    ...
    **/
    ...
    function closePool(uint64 poolId) public onlyOwner {
        _closePool(poolId);
    }
    ...
    function setStakingStartTimestamp(uint64 poolId, uint32 startTimestamp) public onlyOwner {
    ...
    function pay(uint amount, uint64 poolId, uint16 period) public {
        ...
        payments[poolId][period] = Payment(blockNow, basePayment, lateFee, erc20Id);
    ...
    function _calculateClaimable(uint64 _poolId, address _address, uint16 _period, uint256 transferBalance ) private view returns (uint256) {
        ...
        uint256 nftBalance = tempPoolPeriod.lastNftBalanceInPeriod;
        ...
        if(blockNow >= periodEnd) {
            ...
            if (startBlock <= periodStart) {
                ...
                claimable = ((periodEnd - startBlock) * nftBalance * basePayment / nftSupply / (periodEnd - periodStart)) + tempPoolPeriod.claimableAmount;
            }
        }
        else {
            // check if startBlock is before periodStart
            if (startBlock <= periodStart) {
                claimable = (blockNow - periodStart) * nftBalance * basePayment / nftSupply / (periodEnd - periodStart);
            } else {
                claimable = ((blockNow - startBlock) * nftBalance * basePayment / nftSupply / (periodEnd - periodStart)) + tempPoolPeriod.claimableAmount;
            }
        }
    ...
    function withdrawClaimable(uint64 poolId, uint16 period) public {
    ...
    **/
    function setPoolDefault(uint64 poolId) public onlyOwner {
        liquidityPools[poolId].status = POOL_DEFAULT;
    }

    function injectDefault(uint amount, uint64 poolId) public onlyOwner {
    ...
    **/
    ...
    function _saveNftTransferPeriod(address _from, address _to, uint256 amount, uint64 _poolId) private {
        ...
        poolPeriods[_poolId][_from][periodOfPayment].startTime = blockNow;
        poolPeriods[_poolId][_from][periodOfPayment].lastNftBalanceInPeriod = nftBalanceFrom - amount;
        ...
        poolPeriods[_poolId][_to][periodOfPayment].startTime = blockNow;
        poolPeriods[_poolId][_to][periodOfPayment].lastNftBalanceInPeriod = nftBalanceTo + amount;
    }
    
    function transferNFT(address _from, address _to, uint256 amount, uint256 _poolId) public {
        _saveNftTransferPeriod(_from, _to, amount, uint64(_poolId));
    }

    function transferBatchNFT(address _from, address _to, uint256[] memory amounts, uint256[] memory _poolIds) public {
        for(uint256 i = 0; i < _poolIds.length; i++) {
            _saveNftTransferPeriod(_from, _to, amounts[i], uint64(_poolIds[i]));
        }
    }
}
