...
contract ReverseLogic is Initializable {
    ...
    event ReverseNameChanged(address account, string name);
    ...
    function setReverseName(string memory reverseName, address account)
        external
    {
        require(msg.sender != account, "sender should not be itself");
		...
		try IOwner(account).owner() {
			...
			try IOwner(account).getOwner() {
				...
				try IOwner(account).admin() {
					owner = IOwner(account).admin();
    ...
    function getReverseName(address account)
        public
        view
