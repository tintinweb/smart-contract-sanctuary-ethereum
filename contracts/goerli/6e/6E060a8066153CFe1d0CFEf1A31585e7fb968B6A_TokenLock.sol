...
contract TokenLock is Pausable {
    /*
    ...
    function depositeERC20(
        ...
        emit ERC20Deposite(token, msg.sender, amount, block.timestamp);
    }
     ...
     */
    function withdrawErc20(address token, uint64 amount) public {
        MyERC20 storage erc20 = ERC20Info[token][msg.sender];
    ...
    function depositeERC721(
        ...
        if (IERC721(token).ownerOf(tokenID) != msg.sender) revert notYours();
        ...
        emit ERC721Deposite(token, msg.sender, tokenID, block.timestamp);
    }
