...
interface IMatrix {
    /// @notice Eggを生成する。
    function spawn(string calldata metadataHash, address to)
        external
        returns (uint256);
...
// import "./IANMDailyCirculator.sol";
contract MatrixMaster is Ownable {
    ...
    // address private _taxCirculator = address(this);
    ...
    function getPrice(uint256 matrixId) external view returns (uint256) {
        require(
            registered[matrixId] != address(0x0),
    ...
    function spawn(
        ...
        // if(_taxCirculator != address(this)) {
        //     IAnima(_animaContract).transferFrom(msg.sender, _taxCirculator, price);
        //     IANMDailyCirculator(_taxCirculator).onSpawn(eggId,validator,price);
        // }
        return eggId;
    }
}