...
contract StakingFactory is Ownable, IStakingFactory {
     ...
     */
    function getPlasmaPerBlock(address poolAddr) external view override returns (uint256) {
        Pool memory _pool = pools[poolAddr];
        if (!_pool.isPool) {
            return 0;
        }
        if (_pool.ufoPool) {
...
interface IStakingFactory {
    ...
    function getPlasmaPerBlock(address poolAddr) external view returns (uint256 plasmaPerBlock);
