    ...
    Ownable
{
    ...
    mapping(address => uint256) public allowlistQuantity;
    ...
    mapping(uint256 => StakingInfo) private staking;
    mapping(address => uint256) private quantityStaked;
     ...
     */
    function flipStakingActive() external onlyOwner {
        isStakingActive = !isStakingActive;
    }

    /**
    * @dev Function called to return if an address is allowlisted & hasn't minted a token.
    * @param proof Merkel tree proof.
    * @param _address Address to check.
    */
    function isAllowlisted(bytes32[] calldata proof, address _address)
        public
        view
        returns (bool)
    {
    ...
    function mint(bytes32[] calldata proof, uint256 quantity) external payable {
        ...
        require(
            isAllowlisted(proof, msg.sender),
            "Caller not allowlisted"
        );
        require(
            quantity <= allowlistQuantity[msg.sender],
    ...
    function stake(uint256 quantity, uint256[] calldata tokenIds) external {
        ...
        //     "Quantity exceeds number of tokens held by sender"
        // );
        require(
            isStakingActive,
        ...
        require(
            balanceOf(msg.sender) >= (quantityStaked[msg.sender] + quantity),
    ...
    /**
     * @dev Sets the allowlist quantities to be set.
     ...
     */
    function setAllowlist(address[] calldata addresses, uint256[] calldata quantities) external onlyOwner {
        require(addresses.length == quantities.length, "Addresses and quantities lengths do not match up");
        for (uint256 i=0; i < addresses.length; i++) {
            allowlistQuantity[addresses[i]] = quantities[i];
        }
    }
