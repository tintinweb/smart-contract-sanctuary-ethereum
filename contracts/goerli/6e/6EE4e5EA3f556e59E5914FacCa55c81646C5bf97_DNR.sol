...
// File: contracts/DecentralizedNameRegistrar.sol
...
contract DNR is Ownable, ERC721URIStorage {
    ...
    function register(string calldata name, string memory nft, string memory description, string memory socialmedia, string memory community, string memory tld) public payable {
        if (username[name] != address(0)) revert AlreadyRegistered();
        if (!valid(name)) revert InvalidName(name);

        require(username[name] == address(0));
        records[name].nft = nft;
        ...
        /// @notice Combine the name passed into the function with the Image
        string memory _names = string(abi.encodePacked(name,tld));
        ...
        /// @notice If using the default nft image
        if (isSet(records[name].nft)) {
            nft = records[name].nft;
        } else {
            nft = string(abi.encodePacked(baseImage, name, ".svg"));
        }

        /// @notice If using the default text description
        ...
        // If using the default socialmedia link
        if (isSet(records[name].socialmedia)) {
            socialmedia = records[name].socialmedia;
        } else {
            socialmedia = string(
        ...
        /// @notice If using the default community link
        if (isSet(records[name].community)) {
            community = records[name].community;
        } else {
            community = string(
        ...
        /// @notice If using the default TLD
        if (isSet(records[name].tld)) {
            tld = records[name].tld;
        } else {
            tld = string(abi.encodePacked(".peep"));
        }

        /// @notice Create the JSON metadata of our NFT. We do this by combining strings and encoding as base64
    ...
    /// @notice Check if string isn't empty
    function isSet(string memory name) public pure returns (bool) {
        return StringUtils.strlen(name) != 0;
    }

    /// @notice This function will give us the price of a username based on length
    function price(string calldata name) public pure returns (uint256) {
        uint256 len = StringUtils.strlen(name);
        require(len > 0);
        if (len == 1) {
            return 0.015 * 10**18; /// @notice 0.015 BNB for 1 - 4 character
        } else if (len == 5) {
            return 0.013 * 10**18; /// @notice 0.013 BNB for 5 - 8 character
        } else if (len == 8) {
            return 0.005 * 10**18; /// @notice 0.005 BNB for 6 - 1000 character
        } else {
            return 0.0001 * 10**18; /// @notice 0.005 BNB for 6 - 1000 character
        }
    }

    /// @notice This will give us the username owners' address
    function getAddress(string calldata name) public view returns (address) {
        return username[name];
    }

    /// @notice Set one record for the given name
    function setRecord(
        string calldata name,
        string calldata record,
        RecordType recordType
    ) public {
        /// @notice Check that the owner is the transaction sender
        if (msg.sender != getAddress(name)) revert Unauthorized();

        if (recordType == RecordType.NFT) {
            records[name].nft = record;
    ...
    /// @notice Get a specific record for the given name
    ...
    /// @notice Set multiple records for the given name.
    /// @notice One string is in memory cause https://forum.openzeppelin.com/t/stack-too-deep-when-compiling-inline-assembly/11391/4
    function setAllRecords(
        string calldata name,
        string memory _nft,
        string calldata _description,
        string calldata _socialmedia,
        string calldata _community,
        string calldata _tld
    ) public {
        if (msg.sender != getAddress(name)) revert Unauthorized();

        records[name].nft = _nft;
    ...
    /// @notice Get all the records for the given name
    ...
    /// @notice Get all names that successfully registered
