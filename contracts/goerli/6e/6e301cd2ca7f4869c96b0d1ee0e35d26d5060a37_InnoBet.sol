...
contract InnoBet {
    ...
    struct betInstance {
        uint256 _price;
        uint256 _maxNumbers;
        mapping(uint256 => address payable) _bets;
    ...
    function getInstance(uint256 _id)
        public
        view
        returns (
            uint256,
        ...
        )
    {
        ...
        return (
            _instance._price,
            _instance._maxNumbers,
            _instance._percentFee,
            _instance._canBet,
            _instance._token
        );
    }
    ...
    function winner(uint256 id) public view returns (winnerStruct memory) {
    ...
    function betETH(uint256 id, uint256 number) external payable {
        require(
            _betInstances[id]._token == address(0),
            "Instance token is not ETH"
        );
        ...
        require(_betInstances[id]._price == msg.value);
    ...
    function betManyETH(uint256 id, uint256[] memory numbers) external payable {
        require(
            _betInstances[id]._token == address(0),
            "Instance token is not ETH"
        );
        uint256 _eachNumberPrice = msg.value / numbers.length;
        require(
            _betInstances[id]._price == _eachNumberPrice,
            "Value sent is not equals to need value"
        );
        require(msg.value % numbers.length == 0, "Wrong value sent");
    ...
    function betToken(uint256 id, uint256 number) external payable {
        require(
            _betInstances[id]._token != address(0),
            "Instance token is ETH"
        );
        address payable sender = payable(msg.sender);
        address addrOwner = _betInstances[id]._bets[number];
        require(_betInstances[id]._canBet == true, "Bets are closed");
    ...
    function betManyToken(uint256 id, uint256[] memory numbers)
        external
        payable
    {
        require(
        ...
        require(
            IERC20(_betInstances[id]._token).allowance(sender, address(this)) >=
                _betInstances[id]._price * numbers.length,
            "Insuficient allowance"
        );
    ...
    function prize(uint256 id) public view returns (uint256) {
        ...
        uint256 _total = _betInstances[id]._price *
            _betInstances[id]._maxNumbers;
        ...
        returns (address, uint256)
    {
        require(msg.sender == owner);
        require(
            _betInstances[id]._betNumbers.length ==
                _betInstances[id]._maxNumbers,
            "Insuficient bets"
        );
        ...
        address payable _winner = _betInstances[id]._bets[rand];

        uint256 value = prize(id);
        uint256 fee = (_betInstances[id]._maxNumbers *
            _betInstances[id]._price) - value;

        if (_betInstances[id]._token != address(0)) {
            ...
            bool sentPrize = _winner.send(value);
            require(sentPrize, "Failed to send prize");

            bool sentFee = owner.send(fee);
            require(sentFee, "Failed to send fee");
        }

        _betInstances[id]._winner = winnerStruct(
            _winner,
            block.number,
            value,
            rand
