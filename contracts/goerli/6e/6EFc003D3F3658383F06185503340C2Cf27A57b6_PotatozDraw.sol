...
contract PotatozDraw is VRFConsumerBaseV2 {
    ...
    mapping(string => uint32) public drawNumOfFinalists; // drawID => NumOfFinalists
    mapping(string => uint256) public drawSetupDataSequence; // drawID => setupDataSequence
    mapping(string => mapping(uint256 => address)) public drawETOAddresses; // drawID => ending ticket ordinals => draw finalist
    ...
    event DrawFulfilled(string indexed drawID, uint256 numFinalists);
    event DrawExtraRandomAdded(string indexed drawID, uint256 numFinalists);
    ...
    function updateVRFSubscriptionID(uint64 subscriptionId) external onlyOwner {
        s_subscriptionId = subscriptionId;
    }
    ...
    function doDraw(uint32 numFinalists) external onlyOwnerOrHelper {
        require(numFinalists >= 1, "At least 1 finalists");

        uint32 exist = drawNumOfFinalists[drawID];
        require(exist == 0, "Already drawed. Call setDrawID() before doing next draw.");

        drawNumOfFinalists[drawID] = numFinalists;
    ...
    function addExtraRandom(uint32 numRandoms) external onlyOwnerOrHelper {
        ...
        isAddingExtraRandom[s_requestId] = true;
    }
        ...
        override
    {
        ...
        if (isAddingExtraRandom[requestId]) {
            ...
            emit DrawFulfilled(drawID, drawNumOfFinalists[drawID]);
        }
    }

    function mockDraw(uint32 numFinalists, uint256[] memory randomWords)
        external
        onlyOwner
    {
        uint256[] memory exist = drawRandomWords[drawID];
    ...
    then drawETOAddresses[drawID][0], drawETOAddresses[drawID][4], drawETOAddresses[drawID][1] are finalists
    =======================================
    X = drawEndTicketOrdinals
    bisect X for i
        if RN <= X[i] && (i == 0 || RN > X[i - 1]):
            drawETOAddresses[drawID][X[i]] is finalist
    */
    function getFinalists(string memory _drawID)
        external
        view
        returns (address[] memory)
    {
        ...
        uint32 numOfFinalists = drawNumOfFinalists[_drawID];

        require(
            etoArray.length > 0 && rWords.length > 0 && numOfFinalists > 0,
            "No finalists yet for this draw"
        );

        address[] memory finalists = new address[](numOfFinalists);

        uint256 bound = etoArray[etoArray.length - 1] + 1;
        uint256 finalistsLength = 0;
        for (uint256 i = 0; i < rWords.length; i++) {
            ...
            address finalist = drawETOAddresses[_drawID][winningETO];
            bool isFinalistDuplicate = false;
            for (uint256 j = 0; j < finalistsLength; j++) {
                if (finalist == finalists[j]) {
                    isFinalistDuplicate = true;
                    break;
                }
            }
            if (!isFinalistDuplicate) {
                finalists[finalistsLength++] = finalist;
                if (finalistsLength == numOfFinalists) {
                    break;
                }
            }
        }
        require(
            finalistsLength == numOfFinalists,
