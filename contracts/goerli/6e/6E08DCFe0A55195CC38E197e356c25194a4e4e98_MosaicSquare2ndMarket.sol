...
* @dev 주석 주석
****************************************************************************************************/
contract MosaicSquare2ndMarket is Constants, Initializable, AdminTreasury, MarketCore, SendValueWithFallbackWithdraw, MarketFixedPrice, MarketOffer {
...
* @param originalMinIncrement Original minimal increase value
* @param newMinIncrement New minimal increase value
****************************************************************************************************/
  event UpdateMinIncrement(address indexed adminAddress, uint256 originalMinIncrement, uint256 newMinIncrement);
/***************************************************************************************************
...
****************************************************************************************************/
  function createFixedPrice(address nftContract, uint256 tokenId, uint256 price, uint256 duration, uint256 offerPrice) public {
...
****************************************************************************************************/
  function makeOffer(address nftContract, uint256 tokenId) public payable {
    FixedPrice memory fixedPrice = getFixedPrice(nftContract, tokenId);

    require(fixedPrice.seller != address(0), "MarketOffer: The FixedPrice is not registered");
    require(fixedPrice.endTime >= block.timestamp, "MarketOffer: Offer expired");
    require(fixedPrice.seller != msg.sender, "MarketOffer: Cannot make an offer on your own NFT");
...
* @param tokenId NFT token id
****************************************************************************************************/
  function closeFixedPrice(address nftContract, uint256 tokenId) external payable {
...
****************************************************************************************************/
  function refundOffer(address nftContract, uint256 tokenId) external {
    if (_cancelOffer(nftContract, tokenId)) {
      nftContractToTokenIdToAmount[nftContract][tokenId] = _getMinIncrement(0);
    } else {
      revert("MarketOffer: There is no buyer to refund");
    }
  }
...
****************************************************************************************************/
  function updateMinIncrement(uint256 _newMinIncrement) external onlyAdmin {
    uint256 originalMinIncrement = _getMinIncrement(0);
    _updateMinIncrement(_newMinIncrement);

    emit UpdateMinIncrement(msg.sender, originalMinIncrement, _newMinIncrement);
  }
...
****************************************************************************************************/
  function getFixedPrice(address nftContract, uint256 tokenId) public view returns (FixedPrice memory fixedPrice) {
...
***************************************************************************************************/
  function _buy(address nftContract, uint256 tokenId) internal {
...
* @param tokenId The id of the NFT.
****************************************************************************************************/
  function _cancelFixedPrice(address nftContract, uint256 tokenId) internal {
    FixedPrice memory fixedPrice = nftContractToTokenIdToFixedPrice[nftContract][tokenId];
...
****************************************************************************************************/
  function _transferToMarket(address nftContract, uint256 tokenId) internal virtual override {
    ...
    } /*else if (fixedPrice.seller != msg.sender) { // 재등록 할경우 필요한 코드
...
* @notice 
****************************************************************************************************/
abstract contract MarketOffer is MarketCore, SendValueWithFallbackWithdraw, MarketFees {
...
****************************************************************************************************/
  function acceptOffer(address nftContract, uint256 tokenId) external {
    ...
    require(offer.endTime + DURATION_OFFER >= block.timestamp, "MarketOffer: Offer expired");
...
* Returns `0` if there is no offer or the most recent offer has expired.
****************************************************************************************************/
  function getOfferData(address nftContract, uint256 tokenId) external view returns (Offer memory offer) {
    return nftContractToTokenIdToOffer[nftContract][tokenId];
  }
...
* @param nftContract The address of the NFT contract.
* @param tokenId The id of the NFT.
* @return minimum The minimum amount for a offer to be accepted.
****************************************************************************************************/
...
****************************************************************************************************/
  function _acceptOffer(address nftContract, uint256 tokenId, address payable buyer, uint256 amount) internal {
    ...
    require(buyer != address(0), "MarketOffer: There is no offer to accept");
    // Remove offer
...
****************************************************************************************************/
  function _cancelOffer(address nftContract, uint256 tokenId) internal returns (bool){
    Offer memory offer = nftContractToTokenIdToOffer[nftContract][tokenId];
    delete nftContractToTokenIdToOffer[nftContract][tokenId];
