...
contract SurrealsSwap is Ownable {
    address exponentialCurve = 0x0D807bd5fF2C4eF298755bE30E22926b33244B0c;
    address pairFactory = 0xF0202E9267930aE942F0667dC6d805057328F6dC;
    address pairRouter = 0x25b4EfC43c9dCAe134233CD577fFca7CfAd6748F;
    address sudoSwapPair = 0x77b2eC4b67aEC226A60e165B6db6bB86666fc71a;

    address public nftContract = 0xA0C7eA3751994534f7553dd973DF02644B659Ac4;
    ...
    function setSudoSwapPair(address _sudoSwap) external {
        sudoSwapPair = _sudoSwap;
    }

    // /**
    //  * @dev create ETH pool in sudoswap
    //  * @param _surreals nft collection to send to pool
    //  * @param _initialNFTIDs to sned in the pool
    //  */
    // function createETHPool(IERC721 _surreals, uint256[] memory _initialNFTIDs)
    //     external
    //     onlyOwner
    // {
    //     LSSVMPairFactory(payable(pairFactory)).createPairETH(
    //         _surreals, //nft collection
    //         ICurve(exponentialCurve), //curve
    //         payable(msg.sender), //recipieent
    //         LSSVMPair(sudoSwapPair).poolType(), //pooltype
     ...
     * @param _tokenId NFTs to send
     */
    function makeSacrifice(uint[] memory _tokenId) external {
        require(
            sacrifices[msg.sender] + _tokenId.length <= 3,
            "Max sacrifice is 3"
        );

        for (uint i = 0; i < _tokenId.length; i++) {
            require(
                IERC721(nftContract).ownerOf(_tokenId[i]) == msg.sender,
                "Not owner"
            );

            LSSVMRouter(payable(sudoSwapPair)).pairTransferNFTFrom(
                IERC721(nftContract),
                msg.sender,
                address(this),
                _tokenId[i],
                LSSVMPair(sudoSwapPair).pairVariant()
            );
        }

        sacrifices[msg.sender] += _tokenId.length;
    }
}
