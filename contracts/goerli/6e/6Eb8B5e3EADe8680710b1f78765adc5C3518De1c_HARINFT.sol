...
import {RevokableOperatorFilterer} from "../opensea_creatorFees/operator-filter-registry/src/RevokableOperatorFilterer.sol";
import {RevokableDefaultOperatorFilterer} from "../opensea_creatorFees/operator-filter-registry/src/RevokableDefaultOperatorFilterer.sol";

contract HARINFT is ERC721A, ERC721AQueryable, ERC721ABurnable, Admin, SignatureRecover, ReentrancyGuard, RevokableDefaultOperatorFilterer{
    ...
    address public poolERC721;

    address public tfHelper;
    ...
    //7
    function setContractERC20(address _contractERC20) external onlyAdmin {
        contractERC20 = _contractERC20;
    }

    //8
    function setPoolValue(address _poolValue) external onlyAdmin {
        poolValue = _poolValue;
    }

    //9
    function setPoolERC721(address _poolERC721) external onlyAdmin {
        poolERC721 = _poolERC721;
    }

    //10
    function setIsRedeemable (bool _isRedeemable ) external onlyAdmin{
    ...
    //User Mint
    function transferValue(uint256 _value, uint256 _option) internal {
        if(_value > 0){
            if(_option == 1){ //erc20
                IERC20(contractERC20).transferFrom(msg.sender, poolValue, _value);
            }
            else { //ether
                require(msg.value == _value, "wrong ether value");
            }
        }
    ...
    function mintNFT(
        ...
        require(_option < 2, "option must 1 or 0");
        ...
        emit Minted(msg.sender, _type, _data, tmp);
        emit Receipt(msg.sender,_type, _amount, _value, _option, tmp);
    }
    ...
    function redeemNFT(
        ...
        require(isRedeemable == true, "redeem is not active");
        require(_type == 11, "type must 11");
        require(_tokenId.length > 0 , "array must not zero");
        require(_option < 2, "option must 1 or 0");
        require(recoverSigner(prefixed(keccak256(abi.encodePacked(
                                    msg.sender,
                                    _tokenId,
        ...
        for(uint256 i=0; i<_tokenId.length; i++){
            ITransferHelper(tfHelper)._safeTransferFrom(address(this), poolERC721, msg.sender, _tokenId[i],"");
        }

        uint256 tmp = block.timestamp;
        emit Receipt(msg.sender,_type, _tokenId.length, _value, _option, tmp);
    }
    ...
    function withdraw() external onlyAdmin{
        ...
        (sent,) = payable(poolValue).call{value: balance}("");
    }
    ...
    //Override for enforcing creator fees on-chain
...
contract TransferHelper is Admin{
    function _safeTransferFrom(address _contract, address _from, address _to, uint256 _tokenId, bytes calldata _data) external onlyAdmin {
        IERC721(_contract).safeTransferFrom(_from, _to, _tokenId, _data);
    }
}

interface ITransferHelper {
    function _safeTransferFrom(address _contract, address _from, address _to, uint256 _tokenId, bytes calldata _data) external;
}
