...
contract CryptoMoney is Ownable {
    ...
    function fund(
        ...
        s_bills[billId].wordHash = wordHash;
    ...
    function redeem(
        ...
        bool success = s_daiContract.transfer(msg.sender, redeemValue);
        if (!success) {
            revert CryptoMoney__TransferFailed();
        }
        emit BillRedeemed(bill.id, redeemAddress, redeemValue);
    }

    function verifyWord(uint256 billId, string memory word) external view returns (bool) {
        return (keccak256((abi.encodePacked(s_bills[billId].wordHash))) == keccak256(abi.encodePacked(word)));
    }
