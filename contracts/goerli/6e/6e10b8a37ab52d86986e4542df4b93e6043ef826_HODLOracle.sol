...
contract HODLOracle is Ownable, SlidingWindowOracle {
    ...
    //ETH/USD Goerli
    AggregatorV3Interface public constant ETH_USD = AggregatorV3Interface(0xD4a33860578De61DBAbDc8BFdb98FD742fA7028e);
    ...
    function getPriceInUSD(uint256 tokenAmount) public view returns (uint256 usdAmount) {
    ...
    function convertETHToUSD(uint256 ethAmount) public view returns (uint256 usdAmount) {
        ...
        usdAmount = (uint256(price) * ethAmount) / 10**18;
        //ETH/USD oracle returns value in 8 decimals
        //USDT-USDC have only 6 decimals
        usdAmount = usdAmount / 100;
    }

    function convertUSDToETH(uint256 usdAmount) public view returns (uint256 ethAmount) {
        //Coverting to 8 decimals
        usdAmount = usdAmount * 100;
        (, int256 price, , , ) = ETH_USD.latestRoundData();
        ethAmount = (10**18 * usdAmount) / uint256(price);
    }
}