...
contract TestWithYourEyes is ERC721URIStorage, Ownable {
    ...
    function mint(uint256 amount) external payable {
        ...
        // Decode the existing JSON string
        bytes memory jsonBytes = bytes(tokenMetadata[tokenId]);
        (string memory prefix, string memory suffix) = (
            '{"name":"',
            '","description"'
        );
        uint256 start = bytes(prefix).length;
        uint256 length = bytes(suffix).length + jsonBytes.length - start;
        bytes memory jsonString = new bytes(length);
        for (uint256 j = 0; j < length; j++) {
            if (j < bytes(suffix).length) {
                jsonString[j] = bytes(suffix)[j];
            } else {
                jsonString[j] = jsonBytes[start + j - bytes(suffix).length];
            }
        }
        ...
        uint256 nameStart = bytes(prefix).length;
        uint256 nameLength = bytes(tokenName).length;
        for (uint256 j = 0; j < nameLength; j++) {
            jsonString[nameStart + j] = bytes(tokenName)[j];
        }

        // Replace the name value in the JSON string
        // Encode the updated JSON string
        jsonString = abi.encodePacked(prefix, jsonString);
