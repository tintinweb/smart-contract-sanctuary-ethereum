...
contract Index is Ownable, Initializable {
    ...
    function addrewardtokens(address token) public onlyOwner {
        rewardtokens.push(token);
    }

    function transferReward(address rewardgetter) public onlyOwner {
        ...
        uint rewardbalance;
        for (uint i; i < len; i++) {
            rewardbalance = IERC(_rewardtokens[i]).balanceOf(address(this));
            IERC(_rewardtokens[i]).transfer(rewardgetter, rewardbalance);
        }
    }

    function returnstates() public view onlyOwner returns (bool[5] memory) {
        return [purchased, updated, staked, sold, soldprevafterupdate];
    }

    function setFeeGetter(address _feeGetter) public {
    ...
    // will initialize the index contract with name, token addresses and their corresponding percentagess
    ...
    function updatepercent(uint16[] memory _percentages) external onlyOwner {
        percentagesprevious = percentagescurrent;
        percentagescurrent = _percentages;
        updated = true;
    }
    ...
    {
        return (tokenscurrent, percentagesprevious, owner());
    }
    ...
    function checkbalancebytoken(address token, address caller)
        internal
        view
        returns (uint)
    {
    ...
    function deposit(uint256 amountin, address depositer) public {
    ...
    function indextokenbalance() public view returns (uint256[] memory) {
        uint256 numberoftokens = tokenscurrent.length;
    ...
    function _purchase(uint _amount, uint[] calldata _allowedslippage)
        internal
    {
        IERC(ptoken).approve(dex, _amount);
    ...
    function _sell(
        ...
        uint[] calldata _allowedslippage
    ) internal {
        ...
        for (uint256 i; i < numoftokens; i++) {
            IERC(tokens[i]).approve(dex, amounts[i]);
            IDexAggregator(dex).bestrateswap(
                tokens[i],
                ptoken,
                amounts[i],
    ...
    function purchase(uint amount, uint[] calldata allowedslippage)
        public
        onlyOwner
    {
        require(
        ...
        require(block.timestamp < block.timestamp+maturitytime, "ending time already reached");
        require(!purchased, "already purchased");
        require(
            amount <= IERC(ptoken).balanceOf(address(this)),
            "Amount should be less than balance"
        );

        _purchase(amount, allowedslippage);
    ...
    function sell(uint[] calldata _amounts, uint[] calldata allowedslippage)
        public
        onlyOwner
        purchasedcheck
    {
        require(purchased, "Not purchased yet");
        require(!updated, "should not be in updated state");
        require(!staked, "tokens should not be staked");
        require(block.timestamp > maturitytime, "The index has not ended yet");
        _sell(tokenscurrent, _amounts, allowedslippage);
    ...
    function distributeamount() public onlyOwner soldcheck {
        require(sold, "Not sold yet");

        uint total = checkbalancebytoken(ptoken, address(this));
        if (total > totaldeposit) {
            uint fees = (total * 10) / 100;

            IERC(ptoken).transfer(feeGetter, fees);
            total = total - fees;
        }

        uint len = depositers.length;
        address[] memory _depositers = new address[](len);
        uint _totaldeposit = totaldeposit;
        uint rewardperc;
        _depositers = depositers;
        for (uint i; i < len; ++i) {
            rewardperc =
                (depositerbalances[_depositers[i]] * 100) /
                _totaldeposit;
            IERC(ptoken).transfer(_depositers[i], (rewardperc * total) / 100);
        }
    ...
    function rebalancesell(
        ...
        _sell(tokensprevious, amounts, allowedslippage);
        bool flag;
    ...
    function rebalancepurchase(uint amount, uint[] calldata allowedslippage)
        public
    {
    ...
    //     uint totaldeposited = totaldeposit;
    ...
    //     uint[] memory total;
    //     for (uint256 i; i < numoftokens; i++) {
    //         uint256 balance = IERC(tokens[i]).balanceOf(address(this));
    ...
    function initialize(
        ...
        percentagescurrent = _percentages;
        percentagesprevious = _percentages;
        tokenscurrent = _tokens;
        tokensprevious = _tokens;
        startingtime = block.timestamp;
        depositendtime = startingtime + _depositendingtime;
    ...
    function approvetokens(address _stakingContract) public onlyOwner {
        require(purchased, "tokens not purchased yet");
        ...
        for (uint i; i < tokens.length; i++) {
            amount = IERC(tokens[i]).balanceOf(address(this));

            IERC(tokens[i]).approve(_stakingContract, amount);
        }
    }
        ...
        onlyOwner
    {
        ...
        balance = IERC(token).balanceOf(address(this));
    ...
    function FunctionCall(address target, bytes memory data) public onlyOwner {
    ...
    function unstake(address target, bytes memory data) public onlyOwner {
        require(staked, "token not staked");
