...
contract GBKTrade is PullPayment {
	...
	event WithdrawNotice(address account, uint256 amount);

	function transderToWallet() external payable {
		_transfer(address(this), msg.value);
	...
	function asyncTransfer(address dest, uint256 amount) external {
...
abstract contract PullPayment {
	...
	function ownerOf_() public view returns (address) {
		return _escrow.owner();
	}
}