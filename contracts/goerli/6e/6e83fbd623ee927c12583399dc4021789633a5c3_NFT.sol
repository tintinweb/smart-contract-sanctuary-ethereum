...
contract NFT is ERC721A, Ownable, ReentrancyGuard {
    ...
    function presaleMint(uint8 _amount, bytes32[] memory proof) external payable callerIsUser {
        if (!presaleActive) revert PreSaleNotActive();
        if (totalSupply() + _amount > maxSupply) revert MaxSupplyExceeded();
        if (!isValid(proof, keccak256(abi.encodePacked(msg.sender)))) revert NotAllowlisted();
    ...
    function withdraw() external onlyOwner nonReentrant {
        ...
        payable(0xfb5D5d07cd909DfcAD822361Fcc24e8271542E6E).transfer(payoutAddress);
