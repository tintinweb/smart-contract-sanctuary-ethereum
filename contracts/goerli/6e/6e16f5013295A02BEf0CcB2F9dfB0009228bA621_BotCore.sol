...
contract BotExtension is BotBase {
    ...
    function extCreateBot(
        ...
        uint256 _genes,
        address _owner
    ) public onlyExtension returns (uint256) {
...
contract BotBreeding is BotOwnership {
    ...
    function _isReadyToBreed(uint256 _botId, Bot _bot)
        internal
        view
        returns (bool)
    {
    ...
    function _triggerCooldown(Bot storage _bot) internal {
        _bot.cooldownEndBlock = uint64(
            (cooldowns[_bot.cooldownIndex] / secondsPerBlock) + block.number
        );
