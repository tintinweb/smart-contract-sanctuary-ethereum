...
contract SahabaMarketplace is ERC721URIStorage, Ownable {
    ...
    // function to set item for sale
    function toggleItemSale(uint256 _tokenId) public {
        MarketItem storage item = idMarketItem[_tokenId];
        require(
            item.mintedBy == msg.sender,
            "You are not the creator of this token"
        );
        item.isForSale = !item.isForSale;

        idMarketItem[_tokenId] = item;
    }
