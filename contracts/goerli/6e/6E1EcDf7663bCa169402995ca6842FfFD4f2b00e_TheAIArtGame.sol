...
contract TheAIArtGame is Ownable, ERC2981, ERC721A, ReentrancyGuard {
    ...
    string public seed = "beforerevealtestanothertest";

    NEURAL token;

    mapping(uint256 => TokenInfo) public tokenInfo;
    mapping(address => uint256) private maxMint;
    mapping(address => MaintenanceDudes) public arcadeDudes;
    ...
    string[2] hanginChad = [
        "<html><style>p{color: #fff; font-family: Avenir Next, Helvetica Neue, Helvetica, Tahoma, sans-serif; font-size: 1em; font-weight: 700;}p span{display: inline-block; position: relative; transform-style: preserve-3d; perspective: 500; -webkit-font-smoothing: antialiased;}p span::before,p span::after{display: none; position: absolute; top: 0; left: -1px; transform-origin: left top; transition: all ease-out 0.3s; content: attr(data-text);}p span::before{z-index: 1; color: rgba(0,0,0,0.2); transform: scale(1.1, 1) skew(0deg, 20deg);}p span::after{z-index: 2; color: #684da3; text-shadow: -1px 0 1px #684da3, 1px 0 1px rgba(0,0,0,0.8); transform: rotateY(-40deg);}p span:hover::before{transform: scale(1.1, 1) skew(0deg, 5deg);}p span:hover::after{transform: rotateY(-10deg);}p span + span{margin-left: 0.3em;}@media (min-width: 20em){p{font-size: 2em;}p span::before, p span::after{display: block;}}@media (min-width: 30em){p{font-size: 3em;}}@media (min-width: 40em){p{font-size: 5em;}}@media (min-width: 60em){p{font-size: 8em;}}html,body{margin: 0; padding: 0; height: 100%;}body{display: flex; align-items: center; justify-content: center; background-color: #684da3;}</style><p>",
        "</p></html>"
    ];
    ...
    string[4] private split = [
        '<html><style>body{padding: 0; margin: 0; font-family: "Poppins", sans-serif; background: black; height: 100vh; width: 100vw;}span{text-transform: uppercase;position: absolute;left: 50%; top: 50%; transform: translate(-50%, -50%); display: block; color: #cf1b1b; font-size: 124px; letter-spacing: 8px; cursor: pointer;}span::before{content: "',
        '"; position: absolute; color: transparent; background-image: repeating-linear-gradient( 45deg, transparent 0, transparent 2px, white 2px, white 4px ); -webkit-background-clip: text; top: 0px; left: 0; z-index: -1; transition: 1s;}span::after{content: "',
        '"; position: absolute; color: transparent; background-image: repeating-linear-gradient( 135deg, transparent 0, transparent 2px, white 2px, white 4px ); -webkit-background-clip: text; top: 0px; left: 0px; transition: 1s;}span:hover:before{top: 10px; left: 10px;}span:hover:after{top: -10px; left: -10px;}</style><span>',
        "</span></html>"
    ];
        ...
        returns (string[] memory)
    {
        ...
        string[] memory words;
        for (uint256 index = 0; index < strLength(str); index++) {
    ...
    function generateWords(VerifiedMessage memory vm)
        public
        payable
        callerIsSender
        nonReentrant
    ...
    // function adminTokenMint(address _to, uint256 _quantity) public callerIsNeo {
    //     token.mint(_to, _quantity * 1 ether);
    // }
    ...
    function animationCreator(string memory word, uint256 _seed)
        private
        view
        returns (string memory)
    {
        ...
        if (animationType == 0) {
            ...
            base64 = Base64.encode(animationBytes);
        }
        ...
        if (animationType == 2) {
            ...
            animationBytes = abi.encodePacked(animationBytes, split[3]);
            base64 = Base64.encode(animationBytes);
        }
    ...
    function svgCreator(string[3] memory vars)
        private
        view
        returns (bytes memory)
    {
        ...
        returns (string memory)
    {
        ...
        string memory animation = animationCreator(word, _seed);
        ...
        attributes[3] = Attributes("Protected", freeProtection ? "Yes" : "");
        attributes[4] = Attributes("Phrase Upgrade", freePhrase ? "Yes" : "");
        count[0] = freePhrase ? 3 : 2;
