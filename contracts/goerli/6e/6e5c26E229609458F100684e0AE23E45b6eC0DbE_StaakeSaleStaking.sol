...
/// @title Passive Staking pool with progressive staking and unlock for tokens bought during the StaakeSale
contract StaakeSaleStaking is PassiveStaking {
    constructor(
        address _stakedToken,
        address _rewardToken,
        uint256 _progressiveStakingRatio,
        uint32 _progressiveStakingDuration,
        uint32 _cliffDuration,
        uint32 _linearVestingDuration
    ) {
        _grantRole(DEFAULT_ADMIN_ROLE, msg.sender);

        stakedToken = IStaakeToken(_stakedToken);
        rewardToken = IERC20(_rewardToken);

        progressiveStakingRatio = _progressiveStakingRatio;
        progressiveStakingDuration = _progressiveStakingDuration;
