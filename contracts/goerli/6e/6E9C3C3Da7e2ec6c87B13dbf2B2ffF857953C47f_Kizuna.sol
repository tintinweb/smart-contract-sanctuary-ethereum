 ...
 */
contract Kizuna is ERC721A, ERC2981, Ownable, MerkleRootManager, DefaultOperatorFilterer {
  ...
  uint256 public publicPrice = 0.095 ether;
  /// @notice How many NFTs 1 wallet can mint during the public sale
  ...
  constructor(
    ...
    _mintERC2309(_teamWallet, _reservedSupply);
    _setDefaultRoyalty(_teamWallet, 300);
  }

  function togglePauseForWhitelistMint() external onlyOwner {
    isWhitelistMintPaused = !isWhitelistMintPaused;

    emit PauseForWhitelistMintToggled(isWhitelistMintPaused);
  }
  ...
  ) external payable doesNotExceedSupply(_amountToMint) hasEnoughEther(whitelistPrice, _amountToMint) {
    require(!isWhitelistMintPaused, "Kizuna: Whitelist minting is paused");

    // Verify the merkle proof.
    bytes32 _node = keccak256(abi.encodePacked(msg.sender, _maxAmountForAddress));
    require(MerkleProof.verify(_merkleProof, whitelistMerkleRoot, _node), "Kizuna: Invalid proof");

    // Prevent an address from minting more than its allocation
    require(
      whitelistAddressesMintedAmount[msg.sender] + _amountToMint <= _maxAmountForAddress,
  ...
  function freeMint(
    ...
    // Prevent an address from minting more than its allocation
    require(
      freeAddressesMintedAmount[msg.sender] + _amountToMint <= _maxAmountForAddress,
      "Kizuna: Mint limit for address reached"
    );
  ...
  function mint(
    uint256 _amountToMint
  ) external payable doesNotExceedSupply(_amountToMint) hasEnoughEther(publicPrice, _amountToMint) {
    require(!isPublicMintPaused, "Kizuna: Minting is paused");
    require(_totalMinted() + _amountToMint <= maxSupply - freeMintSupply, "Kizuna: Exceeds public mint supply");
