 ...
 */
contract PoolFactory1155 is IPoolFactory1155,Initializable, UUPSUpgradeable, OwnableUpgradeable, ProxyBeaconDeployer {
    ...
    //collection address -> uint256[] of pools using it
    mapping(address => uint256[]) collectionPools;
    ...
    function initialize(address _poolLogic, DataTypes.FactoryFeeConfig calldata _feeConfig) public initializer {
        __Ownable_init();
    ...
    modifier onlyUpgrader() {
        require(IAccessManager(addressesRegistry.getAccessManager()).isUpgraderAdmin(msg.sender), Errors.ADDRESS_NOT_UPGRADER);
        _;
    }
    modifier onlyPoolAdmin() {
        require(IAccessManager(addressesRegistry.getAccessManager()).isPoolAdmin(msg.sender), Errors.ADDRESS_NOT_POOL_ADMIN);
        _;
    }
    ...
    function deployPool(
        ...
        require(
            _poolData.fee.lpBuyFee >= feeConfig.minLpFee &&
                _poolData.fee.lpBuyFee <= feeConfig.maxLpBuyFee &&
                _poolData.fee.lpSellFee >= feeConfig.minLpFee &&
                _poolData.fee.lpSellFee <= feeConfig.maxLpSellFee &&
                _poolData.fee.royaltiesBuyFee <= feeConfig.maxRoyaltiesFee &&
                _poolData.fee.royaltiesSellFee <= feeConfig.maxRoyaltiesFee,
            Errors.FEE_OUT_OF_BOUNDS
        );
        //Set protocol fee ratio to the factory global ratio
        _poolData.fee.protocolSellRatio = feeConfig.protocolSellRatio;
        _poolData.fee.protocolBuyRatio = feeConfig.protocolBuyRatio;
        collectionPools[_poolData.tokens[0]].push(pools.length);
        address proxy = deployBeaconProxy(poolLogic, "");
        IMME1155(proxy).initialize(_poolData, _symbol, _name);
        pools.push(proxy);
        emit PoolDeployed(
            msg.sender,
            _poolData.stable,
            ...
            _symbol,
            _name,
            _poolData.liquidityLimit.poolTvlLimit
        );
        return proxy;
    }
    ...
    function upgradePools(address _newLogic) external onlyUpgrader {
        poolLogic = _newLogic;
        //Change beacon logic
        upgradeBeacon(_newLogic);
        ++poolsVersion;
        emit PoolsUpgraded(msg.sender, _newLogic);
    }
     ...
     */
    function getPoolsVersion() public view returns (uint256) {
        return poolsVersion;
    }
    ...
    function setDeploymentByPoolAdminOnly(bool _status) external onlyPoolAdmin {
        onlyPoolAdminDeployments = _status;
        emit DeploymentByPoolAdminOnlySet(msg.sender, _status);
    }
...
contract ProxyBeaconDeployer {
    ...
    function _createBeacon(address _logic) internal {
        if (!_isSetBeacon) {
