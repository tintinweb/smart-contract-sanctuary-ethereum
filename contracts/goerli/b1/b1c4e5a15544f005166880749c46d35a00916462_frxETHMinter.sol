...
contract frxETHMinter is OperatorRegistry, ReentrancyGuard {
    ...
    mapping(bytes => bool) public usedValidators; // Tracks validators (via their pubkeys) that already have 32 ETH in them
    ...
    /// @notice Mint frxETH and stake it to receive sfrxETH in one transaction, method 1
    /// @dev Could try using EIP-712 / EIP-2612 here in the future if you replace this contract
    function depositAndStake(address recipient) external payable returns (uint256 shares) {
        // Note the frxETH balance of this contract beforehand
        uint256 balance_before = frxETHToken.balanceOf(address(this));

        // Give the frxETH to this contract after it is generated
        _submit(address(this));

        // Note the frxETH balance of this contract afterwards
        uint256 balance_after = frxETHToken.balanceOf(address(this));

        // Calculate the amount of frxETH generated
        uint256 frxeth_generated = balance_after - balance_before;
        require(frxeth_generated > 0, 'No frxETH generated');

        // Approve frxETH to sfrxETH for staking
        frxETHToken.approve(address(sfrxETHToken), frxeth_generated);

        // Stake the frxETH
        uint256 sfrxeth_recieved = sfrxETHToken.deposit(frxeth_generated, address(this));

        // Give the sfrxETH to the final recipient
        sfrxETHToken.transfer(recipient, frxeth_generated);

        return sfrxeth_recieved;
    }

    // /// @notice Mint frxETH and stake it to receive sfrxETH in one transaction, method 2
    // /// @dev Uses an EIP-712 signed approval (EIP-2612 permit)
    // /// @dev You might run into msg.sender issues in sfrxETH though
    // function depositAndStakeWithPermit(address recipient, uint256 deadline, bool approveMax, uint8 v, bytes32 r, bytes32 s) external payable returns (uint256 shares) {
    //     _submit(msg.sender);
    //     return sfrxETHToken.depositWithSignature(msg.sender, msg.value, recipient, deadline, approveMax, v, r, s);
    // }
    ...
    function _submit(address recipient) internal nonReentrant {
        ...
        if (withholdRatio != 0) {
            currentWithheldETH += (msg.value * withholdRatio) / RATIO_PRECISION;
        }

        // If enough ETH has piled up in the contract, deposit it in the ETH 2.0 deposit contract in DEPOSIT_SIZEd batches
        if (address(this).balance - currentWithheldETH >= DEPOSIT_SIZE) {
            // Execute the deposit
            depositEther();
        }

        emit ETHSubmitted(recipient, msg.value);
    }
    ...
    function submitAndGive(address recipient) public payable {
        _submit(recipient);
    }
    ...
    /// @dev Can be public because submit(1 gwei) effectively calls it anyways
    function depositEther() public {
        ...
        for (uint256 i = 0; i < numDeposits; ++i) {
            ...
            require(!usedValidators[pubKey], "Validator already has 32 ETH deposited");
    ...
    function setWithholdRatio(uint256 newRatio) public onlyByOwnGov {
    ...
    function moveWithheldETH(address payable to, uint256 amount) public onlyByOwnGov {
    ...
    event ETHSubmitted(address recipient, uint256 amount);
    event DepositSent(bytes pubKey, bytes withdrawalCredential);
    event WithheldETHMoved(address to, uint256 amount);
...
// Stablecoin pegged to Ether for use within the Frax ecosystem
// Does not accrue ETH 2.0 staking yield: it must be staked at the sfrxETH contract first
...
// frxETH adheres to EIP-712/EIP-2612 and can use permits
...
contract OperatorRegistry is Owned {
    ...
    bytes withdrawalCredential; // Pubkey for ETH 2.0 withdrawal creds. If you change it, you must empty the validators array
    address public timelock_address;

    constructor(address _owner, address _timelock_address, bytes memory _withdrawalCredential) Owned(_owner) {
    ...
    function addValidator(Validator calldata validator) public onlyByOwnGov {
        ...
        emit ValidatorAdded(validator.pubKey, withdrawalCredential);
    }

    /// @notice Add multiple new validators
    /// @dev You should verify offchain that the validators are indeed valid before adding them
    function addValidators(Validator[] calldata validatorArray) public onlyByOwnGov {
    ...
    /// @notice Return the information of any validator in the registry
    ...
    function setWithdrawalCredential(bytes memory _withdrawalCredential) public onlyByOwnGov {
        require(numValidators() == 0, "Clear validator array first");
        withdrawalCredential = _withdrawalCredential;

        emit WithdrawalCredentialSet(_withdrawalCredential);
    }

    /// @notice Empties the validator array
    /// @dev Need to do this before setWithdrawalCredential()
    function clearValidatorArray() public onlyByOwnGov {
        delete validators;
