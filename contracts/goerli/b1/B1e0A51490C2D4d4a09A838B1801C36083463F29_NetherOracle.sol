...
contract NetherOracle is Epoch {
    ...
    constructor(uint256 _epochPeriod) Epoch(_epochPeriod,block.timestamp,0)  {
        ...
        poolId = 0x5d563ca1e2daaae3402c36097b934630ab53702c00020000000000000000020c;

        token1 = IAsset(0x23A5FC17Edf8f195C06089d8Be0faA60Aee293F3);
        token2 = IAsset(0x293022FB86445e44362eB1c530c7c931aBFe8D0C);
    }
    ...
    function setPool(IMetaStablePool _pool) external onlyOperator {
        pool = _pool;
    }
    ...
    function getTwap() external checkEpoch returns(uint256) {
        ...
        (,,,oracleIndex,,) = pool.getMiscData();
        if (oracleIndex < 1024) {
            int256[] memory deltas = callQueryBatchSwap();
            twap = uint256(-deltas[1]);
            return twap;
        } else {
    ...
    function callQueryBatchSwap()
            ...
            returns (int256[] memory)
        {
            ...
            uint256 _tradeAmount = 100000000000000000; 
