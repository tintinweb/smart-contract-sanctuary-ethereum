...
contract SecretVerseVaultDistributor is AccessControl {
    ...
    uint256 public constant maxMintLimit = 5;
    ...
    modifier isTimeForPublicSale() {
        if (
            block.timestamp <=
            avatarHoldersSaleStartTime + avatarHoldersSaleEndTime ||
            (avatarHoldersSaleStartTime == 0 || avatarHoldersSaleStartTime == 0)
        ) {
    ...
    function avatarHolderMintVault(uint256 _num, uint256 _avatarTokenId)
        external
        payable
        isEligible(_num)
        ...
        returns (bool)
    {
        ...
        isAvatarHolder(user, _avatarTokenId);
    ...
    function publicMintVault(uint256 _num, uint256 _avatarTokenIds)
        external
        payable
        isEligible(_num)
        isTimeForPublicSale
        returns (bool)
    {
        ...
        //// checking if user wants to mint TWIN AVA : 0 means simple AVA, non-zero means TWIN AVA
        if (_avatarTokenIds != 0) {
            require(_num == 1, "Twin AVA should be minted singly");
            if (isAvatarHolder(user, _avatarTokenIds)) {
    ...
    function isAvatarHolder(address user, uint256 tokenIds)
        internal
        returns (bool)
    {
        if (isAvatarNFTUsed[tokenIds]) {
            revert AlreadyCreatedTwinVault();
        }

        if (avatarToken.ownerOf(tokenIds) != user) {
            revert NotEligibleForTwinVault();
        }

        isAvatarNFTUsed[tokenIds] = true;
