...
contract MintPassFactory is Ownable, ERC721MPF, DefaultOperatorFilterer
{
     ...
     */
    function _MintToFactory(address Recipient, uint Amount, uint MintPassProjectID) external onlyAuthorized
    {
     ...
     */
    function _LiveMintBurn(uint MintPassProjectID, uint TokenID) external onlyAuthorized returns (address _Recipient, uint _ArtistID)
    {
        address Recipient = IERC721(address(this)).ownerOf(TokenID);
        require(Recipient != address(0), "MPMX: Invalid Recipient");
        _burn(TokenID, false);
        return (Recipient, MintPasses[MintPassProjectID]._ArtBlocksProjectID);
    }
