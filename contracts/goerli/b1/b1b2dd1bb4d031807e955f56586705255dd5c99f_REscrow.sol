...
contract EscrowProducer is Utilities, Ownable, AssetType{
    ...
    function addNewProducer(string[] memory _postedAssets,string memory _escrowname, uint _assetTypeID) public payable returns(ProducerStruct memory){
    ...
    function addNewProducerUSDT(string[] memory _postedAssets,string memory _escrowname, uint _assetTypeID, uint _amount) public returns(ProducerStruct memory){
        ...
        newProducer.escrowFee = defaultEscrowFee;
        newProducer.isActive = true;
        newProducer.redeemAmountPerNumber = _amount/_postedAssets.length;
        newProducer.assetTypeName = assetType.assetTypeName;
        newProducer.assetPrice = assetType.assetPrice;
        newProducer.assetTypeID = _assetTypeID;
    ...
    function countOfMatchedAssets(uint _escrowID, string[] memory _collectedAssets) internal view returns(uint){  
        ...
        for (uint i = 0; i < producerArray[_escrowID].postedAssets.length; i++)
        {
            for (uint j = 0; j < _collectedAssets.length; j++)
            {
                ...
                if (keccak256(abi.encodePacked(producerArray[_escrowID].postedAssets[i])) == keccak256(abi.encodePacked(_collectedAssets[j]))) {
    ...
    function findMatchedAssets(uint _escrowID, string[] memory _collectedAssets) internal returns(ProducerInfoStruct memory, string[] memory,uint,uint){
...
contract EscrowProvider is Utilities, Ownable{
    ...
    function addNewProvider(uint _escrowID, string memory _escrowName, address _producer,address _provider,string[] memory _collectedAssets,uint _quantity, uint _redeemedAmount, uint _rebateReceivedAmount) internal returns(ProviderStruct memory){
...
contract REscrow is EscrowProducer, EscrowProvider {
    ...
    function addProvider(string[] memory _collectedAssets) public returns(string memory){
        string memory result;
        result = "No Match Found";
        uint matchedAssetsCount = 0;
        for (uint i = 0; i < EscrowProducer.currentEscrowCount; i++)
        {
            (ProducerInfoStruct memory producerInfo, string[] memory matchedAssets, uint rebateReceivedAmount,uint redeemedAmount) = EscrowProducer.findMatchedAssets(i, _collectedAssets);
            ProviderStruct memory newProvider;
    ...
    function addProviderUSDT(string[] memory _collectedAssets) public returns(string memory){
        ...
        for (uint i = 0; i < EscrowProducer.currentEscrowCount; i++)
        {
            ...
            usdtToken.transfer(producerInfo.producer, rebateReceivedAmount);
