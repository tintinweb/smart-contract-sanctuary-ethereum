...
contract KelePoolStaking  is Initializable, UUPSUpgradeable {
    ...
    address public constant ETH2_DEPOSIT_ADDRESS = 0x4242424242424242424242424242424242424242; // eth2.0 contract address
    ...
    event OnWithdraw(address indexed owner, address indexed receiver, uint256 amount);
    event OnValidatorCreated(address indexed owner,uint8 role,uint256 amount,uint256 time,bytes withdrawal_credentials,bytes pubkey);
    event OnFeeTakeOut(address indexed owner, address indexed receiver, uint256 amount);
    ...
    // create validator
    function createValidator(uint8 role,bytes calldata pubkeys,bytes calldata withdrawal_credentials,bytes calldata signatures,bytes32[] calldata deposit_data_roots) external payable 
    {
        // check parameters
        require(_system.status == true,"The system is under maintenance");
        ...
        if(role==0){// normal
            ...
            require(address(this).balance >= DEPOSIT_AMOUNT && _system.balance >= DEPOSIT_AMOUNT, "Invalid balance");
            _system.balance-=DEPOSIT_AMOUNT;
        }else if(role==1){// whales
        ...
        for (uint32 i = 0; i < pubkeyCount; ++i) {
            ...
            IDepositContract(ETH2_DEPOSIT_ADDRESS).deposit{value: DEPOSIT_AMOUNT}(
                pubkey,
                withdrawal_credentials,
                signature,
                deposit_data_roots[i]
            );
            emit OnValidatorCreated(msg.sender,role,DEPOSIT_AMOUNT,block.timestamp,withdrawal_credentials,pubkey);
        }
    ...
    modifier onlyOperator {
        require(_system.owner == msg.sender || _system.foundation == msg.sender || msg.sender == _system.operator,"Error operator");
        _;
    }
