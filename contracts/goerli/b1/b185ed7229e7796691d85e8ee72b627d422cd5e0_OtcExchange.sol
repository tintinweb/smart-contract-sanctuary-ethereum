...
contract OtcExchange is IOrderEscrow, IOrderFixed, Ownable, ReentrancyGuard {
  ...
  modifier canUpdateOrder(
    ...
    if (orderClass) {
      ...
      if (
        _order.orderType &&
        amountQuote > ERC20(_order.tokenQuote).allowance(msg.sender, address(this))
      )
      ...
      if (
        !_order.orderType &&
        amountBase > ERC20(_order.tokenBase).allowance(msg.sender, address(this))
      )
        revert InsufficientAllowance(
          _order.tokenBase,
          amountBase,
    ...
    returns (bool success)
  {
    ...
    availableOrdersIDs.add(uint256(thisOrderID));
  ...
  function takeOrderFixed(
    uint64 orderID
  ) external canTakeOrder(orderID, true) nonReentrant returns (bool success) {
  ...
  function cancelOrderFixed(
    uint64 orderID
  ) external canCancelOrder(orderID, true) nonReentrant returns (bool success) {
  ...
  function takeOrderEscrow(
    uint64 orderID,
    uint256 amountToken
  ) external canTakeOrder(orderID, false) nonReentrant returns (bool success) {
    ...
    if (orderSelected.orderType) {
      ...
      } else amountQuote = amountQuote / 10 ** 18;
      ...
      } else amountBase = amountBase / 10 ** 18;
      ...
      marketsEscrow[orderID].amountBase = orderSelected.amountBase - amountBase;
    ...
    returns (bool success)
  {
    ...
    if (oldOrder.orderType) {
      ...
      } else if (amountQuote < oldOrder.amountQuote) {
      ...
      } else if (amountBase < oldOrder.amountQuote) {
