...
contract DeadF is ERC721, ReentrancyGuard, Signable, AccessControl {
    bytes32 public constant BURNER_ROLE = keccak256("BURNER_ROLE");
    // bytes public constant deadFInterface = type(DeadF).interfaceId;
    ...
    function checkMyInterface() public pure returns (bytes4) {
        bytes4 test = type(IERC721).interfaceId;
        return test;
    }
    ...
    function testTupple(
        ...
        return _withdrawalAddresses.length;
    }
    ...
    function testErrors(uint256 number) external pure returns (string memory) {
        if (number == 1) revert Errors.SupplyLimitReached();
        if (number == 2) revert Errors.AccountAlreadyMintedMax();
        if (number == 3) revert Errors.NothingToWithdraw();
        if (number == 4) revert Errors.MaxSupplyTooSmall();
        if (number == 5) revert Errors.CanNotIncreaseMaxSupply();
        if (number == 6) revert Errors.TokenDoesNotExist();
        if (number == 7) revert Errors.InvalidOwner();
        if (number == 8) revert Errors.InvalidSignature();
        if (number == 9) revert Errors.MintNotAvailable();
        if (number == 10) revert Errors.InsufficientFunds();
        if (number == 11) revert Errors.InvalidSignature();
