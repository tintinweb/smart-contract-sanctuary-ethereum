...
contract ERC20Base is Context, ERC20, Ownable, Lockable {
    ...
    // This is the fee from each transaction going to burn so deflation can happen
    uint8 burnFee = 5; // 5% burn fee

    // function to adjust the burn fee
    function setBurnFee(uint8 _burnFee) public {
        require(_burnFee <= 100, "ERC20: burn fee must be less than 100%");
        burnFee = _burnFee;
    }
    ...
    function _updateTokenLockInfo() internal returns(LockInfo memory) { // this function is called when the token is locked
        bool isEntity = _manualEntity[msg.sender]; // if the token is locked by the entity, the entity can unlock the token
        require(isEntity == true, "ERC20: There is not lockinfo"); // if the token is not locked by the entity, the entity can not unlock the token

        LockInfo storage li = _lockInfos[msg.sender]; // get the lock info of the token
        if(li.nextTime < block.timestamp && li.releaseRatio < 100) { // li.nextTime < block.timestamp means the token is not locked and releaseRatio < 100 means the token is not fully locked
    ...
    function _Transfer(address to, uint256 amount) internal returns (bool) {
        ...
        _transfer(_msgSender(), to, amount.sub(burntAmount));
        return true;
    }

    function _TransferFrom(address from, address to, uint256 amount) internal returns (bool) {
        uint burntAmount = amount.mul(burnFee) / 100;
        _spendAllowance(from, msg.sender, burntAmount);
        _burn(from, burntAmount);
        _transfer(from, to, amount.sub(burntAmount));
        return true;
    }
