...
contract SmartWallet {
	...
	function coinTransfer(address payable _to, uint256 _value) public {
      bool sent = _to.send(_value);
      require(sent, "Failed to send Ether");
	}
	
	function tokenTransfer(IERC20 _token, address _to, uint256 _value) public {
		_token.transfer(_to, _value);
	}

    function tokenApprove(IERC20 _token, address _spender, uint256 _value) public {
		_token.approve(_spender, _value);
	}

    function transferFrom(IERC20 _token, address _from, address _to, uint256 _value) public {
		_token.transferFrom(_from, _to, _value);
	}

    function batchCoinTransfer(address payable[] calldata _to, uint256[] calldata _value) public {		
