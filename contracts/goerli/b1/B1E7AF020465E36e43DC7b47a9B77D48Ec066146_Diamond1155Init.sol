...
library ERC1155Lib {
     ...
     * @param _startTime When the token will be available for minting.
    ...
    function create(
        ...
        ERC1155Storage.TokenStructure storage tokenData = s.tokenData[_id];
        ...
        tokenData.startTime = _startTime;
    ...
    function batchCreate(
        ...
        ERC1155Storage.Layout storage s = ERC1155Storage.layout();
        ERC1155Storage.TokenStructure storage tokenData;
     ...
     * NOTE: remove onlyOwner if you want third parties to create new tokens on your contract (which may change your IDs)
     * @param _maxSupply Maxmium amount of new token (array).
     * @param _price Price of new token (array).
     * @param _tokenUri Optional, baseUri is set in ERC1155MetadataStorage (https://sample.com/{id}.json) would be valid) (array)
     * @param _allowListEnabled Whether or not the token is on the allow list (array).
    ...
    function batchCreate(
        ...
        string[] calldata _tokenUri,
        bool[] calldata _allowListEnabled,
        uint256[] calldata _startTime
    ) internal returns (bool success) {
        if (msg.sender != OwnableStorage.layout().owner) revert NotOwner();

        uint256 amount = _maxSupply.length;
        if (_price.length != amount) revert ArrayLengthsDiffer();
        if (_tokenUri.length != amount) revert ArrayLengthsDiffer();
        if (_allowListEnabled.length != amount) revert ArrayLengthsDiffer();
        if (_startTime.length != amount) revert ArrayLengthsDiffer();
        ...
        for (; i < amount; ) {
            ...
            tokenData.maxSupply = _maxSupply[i];
            tokenData.price = _price[i];
            tokenData.creator = msg.sender;
            tokenData.tokenUri = _tokenUri[i];
            tokenData.allowListEnabled = _allowListEnabled[i];
            tokenData.startTime = _startTime[i];

            if (bytes(_tokenUri[i]).length > 0) {
                emit URI(_tokenUri[i], _id);
            }
...
import {
    ERC165,
    ERC165Storage
} from "@solidstate/contracts/introspection/ERC165.sol";
// Storage
import {
    ERC2981Storage
...
contract Diamond1155Init is PaymentSplitterInternal, OwnableInternal {
    using ERC165Storage for ERC165Storage.Layout;
    ...
    function init(DiamondArgs calldata _args) external {
        // adding ERC165 data
        ERC165Storage.Layout storage ds = ERC165Storage.layout();
        ...
        ERC1155Storage.TokenStructure calldata _token;
        i = 0;
