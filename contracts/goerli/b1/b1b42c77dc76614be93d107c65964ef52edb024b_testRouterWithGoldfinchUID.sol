...
contract testRouterWithGoldfinchUID {
    ...
    */
    ...
    struct TokenData { 
        ...
        uint256 expiryPrice; // price = (2 * 10^18) / (USD * 10^6) e.g., 200USD = 10^18/20^8
        address issuanceTokenAddress;
    }

    //@dev this function sets each Asset-backed Token to a fixed address of UID, address of issuer, issuance price, expiry price, and address of accepted issuance token denominated
    function add(address outputTokenAddress, address _uIdContract, address _issuer, uint256 _issuancePrice, uint256 _expiryPrice, address _issuanceTokenAddress) external {
        ...
        tokenData[outputTokenAddress]=TokenData(_issuer, _uIdContract, _issuancePrice, _expiryPrice, _issuanceTokenAddress);
    }
    ...
    */ 
    ...
    function sell(address friggTokenAddress, uint256 inputFriggAmount) external {
            ...
            outputToken.transferFrom(
                tokenData[friggTokenAddress].issuer, 
                msg.sender,
                inputFriggAmount / tokenData[friggTokenAddress].expiryPrice);
