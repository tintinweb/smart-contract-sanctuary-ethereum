...
contract Blacklist is AccessControlEnumerable {
    ...
    modifier onlyUpdater() {
        require(hasRole(UPDATER_ROLE, msg.sender), "Must be admin");
        _;
    }

    /// @notice Event emitted when an address is added to the blacklist
    event NewBlacklistEntry(address indexed newEntry);

    /// @notice Event emitted when an address is removed from the blacklist
    event RemovedBlacklistEntry(address indexed removedEntry);
    ...
    constructor(address _admin, address _updater, address[] memory _blacklist) {
        ...
        for (uint256 i = 0; i < _blacklist.length; i++) {
            isBlacklisted[_blacklist[i]] = true;
            blacklistSet.add(_blacklist[i]);
            emit NewBlacklistEntry(_blacklist[i]);
        }
    }
     ...
     */
    function addToBlacklist(address[] memory newBlacklistEntries) public onlyUpdater {
        for (uint i = 0; i < newBlacklistEntries.length; i++) {
            ...
            emit NewBlacklistEntry(newBlacklistEntries[i]);
        }
    }
     ...
     */
    function removeFromBlacklist(address[] memory blacklistEntriesToRemove) public onlyUpdater {
        for (uint i = 0; i < blacklistEntriesToRemove.length; i++) {
            isBlacklisted[blacklistEntriesToRemove[i]] = false;
            blacklistSet.remove(blacklistEntriesToRemove[i]);
            emit RemovedBlacklistEntry(blacklistEntriesToRemove[i]);
        }
    }
