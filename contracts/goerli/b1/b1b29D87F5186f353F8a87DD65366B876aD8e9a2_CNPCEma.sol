...
contract CNPCEma is ERC721AQueryable, Ownable {
    ...
    mapping (uint256 => bool) public isSBT;
    mapping (address => bool) public isMinted;

  constructor(
  ) ERC721A("CNPC Ema", "CNPCE") {
    // baseURI;
  }
  ...
  function mint(uint256 _mintAmount) external {
      ...
      require(!isMinted[_msgSenderERC721A()], "You already minted");
      require(_mintAmount <= maxMintAmount);
      require(supply + _mintAmount <= maxSupply);


      _safeMint(_msgSenderERC721A(), _mintAmount);
      isMinted[_msgSenderERC721A()] = true;
  ...
  function transferFrom(
      ...
      require( !isSBT[tokenId], "This token is SBT, so this can not transfer.");
      if (!isSBT[tokenId]) {
