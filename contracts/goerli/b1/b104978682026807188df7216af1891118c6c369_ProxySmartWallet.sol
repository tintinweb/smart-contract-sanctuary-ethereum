...
contract ProxySmartWallet is Proxy {
	constructor(address owner, address manager, SmartWallet implementation) Proxy(owner, manager, implementation) {
        (bool success,) = address(this).call{value: 0}(abi.encodeWithSelector(implementation.initialize.selector, owner, manager));
		require(success);
    }
}