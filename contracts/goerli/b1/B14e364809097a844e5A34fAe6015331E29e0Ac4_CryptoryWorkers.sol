...
contract CryptoryWorkers is ERC721A, Ownable, ReentrancyGuard, Pausable {
    ...
    function mint(uint8 amount, uint8 step) external payable nonReentrant whenNotPaused isNotContract {
        ...
        if (step == OG_STEP) {
            ...
            require(allowedAmounts[msg.sender].og - amount >= 0, "Don't have mint authority on the 1st WH");
        } else if (step == WL1_STEP) {
            require(_totalMinted() + amount <= firstWhitelistReservedAmount, "Sold out in this step");
            require(allowedAmounts[msg.sender].wh1 - amount >= 0, "Don't have mint authority on the 1st WH");
            allowedAmounts[msg.sender].wh1 -= amount;
        } else if (step == WL2_STEP) {
            require(allowedAmounts[msg.sender].wh2 - amount >= 0, "Don't have mint authorityon the 2nd WH");
            allowedAmounts[msg.sender].wh2 -= amount;
    ...
    function setOriginalGangstersAllowedAmount(address[] calldata _addressList, uint8[] calldata _amountList) external onlyOwner {
        require(_addressList.length == _amountList.length, "Invalid argument");

        for (uint256 i = 0; i < _addressList.length; i++) {
            _setAllowedAmount(_addressList[i], _amountList[i], OG_STEP);
        }
    }
    ...
    function _setAllowedAmount(
        ...
        if (_step == OG_STEP) {
            allowedAmounts[_address].og = _amount;
        } else if (_step == WL1_STEP) {
            allowedAmounts[_address].wh1 = _amount;
        } else if (_step == WL2_STEP) {
        allowedAmounts[_address].wh2 = _amount;
        } else {
            revert("Not exist mint step");
        }
    }
