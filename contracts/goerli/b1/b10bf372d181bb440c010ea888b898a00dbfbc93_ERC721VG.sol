 ...
 * @title ERC721VG
 *
 * @dev NFTool.pro implementation of the [ERC721A](https://chiru-labs.github.io/ERC721A/#/)
 * Based on ERC721 Non-Fungible Token Standard, including the Metadata extension.
 * Optimized for lower gas during mints and transfers.
 *
...
contract ERC721VG is ERC721A, Ownable, ERC2981 {
    /**
     * ========================
     *          Events
     * ========================
    ...
    /**
     * @notice Event emitted when `globalMintCapPerWallet` has been modified
     */
    event GlobalMintCapPerWalletChanged(uint32 newGlobalMintCapPerWallet);
    ...
    /**
     * @notice Structure for packing the information of the uris and its status
     * @member unrevealedURI: The not reveal token URI
     * @member baseURI: The base token URI to add before the token id
     * @member baseURILocked: If the URI can definitely not be modified anymore
     * @member revealed: If tokens' uri is revealed
     */
    struct Uris {
    ...
    /**
     * @notice Structure for packing the information of sale settings for parametrized deployment
     * @member collectionSize: total collection size
     * @member globalMintCapPerWallet: Maximum global amount of tokens minted by wallet address
     * @member amountForFounders: list of amount tokens per founders (founding team)
     * @member founders: list of founder addresses to share deployment drop
     * @member withdrawAddresses: list of addresses to withdraw % of revenues
     * @member withdrawPercentages: % of revenue for each address of withdrawAddresses
     */
    struct SaleSettings {
        uint64 collectionSize;
        uint32 globalMintCapPerWallet;
        uint256[] amountForFounders;
        address[] founders;
        address[] withdrawAddresses;
     ...
     * @member maxMintsPerWallet Number of tokens that a wallet can mint in this round
     * @member merkleRoot root of Merkle tree for this round
    ...
    // Sale settings
    // total collection size = maximum supply of all contract
    uint64 public collectionSize;

    /// Number of tokens that a wallet can mint globally
    uint32 public globalMintCapPerWallet;
    ...
    mapping(uint256 => mapping(uint256 => mapping(address => uint256))) internal _waveToRoundsToOwnerTotalMinted;
    ...
    // withdraw addresses and corresponding percentages
    ...
    constructor(
        ...
        Uris memory uris_,
        SaleSettings memory saleSettings_,
        Round[] memory rounds_
    ) ERC721A(name_, symbol_) {
        // Storage initialisation
        unrevealedURI = uris_.unrevealedURI;
        baseURI = uris_.baseURI;
        baseURILocked = uris_.baseURILocked;
        revealed = uris_.revealed;
        // Max Global Mints Per Wallet
        globalMintCapPerWallet = saleSettings_.globalMintCapPerWallet;
        // withdraw settings
        withdrawAddresses = saleSettings_.withdrawAddresses;
        withdrawPercentages = saleSettings_.withdrawPercentages;
        // total collection size
        collectionSize = saleSettings_.collectionSize;

        // mint and transfer to founders
        // mint and transfer to founders
        if (saleSettings_.founders.length > 0 && saleSettings_.amountForFounders.length > 0) {
            _airdrops(saleSettings_.founders, saleSettings_.amountForFounders, false);
        }

        // rounds seti[]
        if (rounds_.length != 0) {
            for (uint256 i = 0; i < rounds_.length; ) {
                _setupRound(
                    1,
                    i + 1,
                    rounds_[i].supply,
                    rounds_[i].maxMintsPerWallet,
                    rounds_[i].merkleRoot,
                    rounds_[i].startTime,
                    rounds_[i].duration,
                    rounds_[i].price
                );
        ...
        // royalty
        if (saleSettings_.royaltyPercent > 0) {
            ERC2981._setDefaultRoyalty(msg.sender, saleSettings_.royaltyPercent);
        }
        // burnable
        _setBurnable(saleSettings_.burnable);
    }
     ...
     * - View {ERC721VG-_roundMint} requirements
     *
     * @param roundId The mint round index in the current wave
     * @param amount The number of tokens to mint
     */
    function mint1(uint256 roundId, uint256 amount) external payable virtual {
        require(waveId > 0, "Mint is closed");
        require(_waveToRounds[waveId][roundId].merkleRoot == bytes32(0), "Need merkle validation");
        require(
            balanceOf(msg.sender) + amount <= globalMintCapPerWallet || globalMintCapPerWallet == 0,
            "Max global allowed per address"
        );
        require(
            _waveToRoundsToOwnerTotalMinted[waveId][roundId][msg.sender] + amount <=
                _waveToRounds[waveId][roundId].maxMintsPerWallet ||
                _waveToRounds[waveId][roundId].maxMintsPerWallet == 0,
            "Max allowed per round per address"
        );
        _roundMint(roundId, amount);
    }

    /**
     * @notice Mint the `amount` of tokens with merkle tree whitelist in the current wave .
     *
     * @dev Requirements:
     * - Current wave must exist
     * - Round must have a merkleRoot
     * - Total minted for the user during this round must be less than `maxMintsPerWallet`.
     * - merkleProof has to be provided corresponding to the merkle root provided in roundsetup
     * - View {ERC721VG-_roundMint} requirements.
     *
     * @param roundId The mint round index in the current wave
     * @param amount The number of tokens to mint
     * @param merkleProof a `proof` must be provided, containing sibling hashes on the branch from the leaf to the root of the tree.
     */
    function mintWithWl(
        uint256 roundId,
        uint256 amount,
        ...
        require(waveId > 0, "Mint is closed");
        bytes32 _merkleRoot = _waveToRounds[waveId][roundId].merkleRoot;
        require(_merkleRoot != bytes32(0), "Whitelist proof required");
        require(
            balanceOf(msg.sender) + amount <= globalMintCapPerWallet || globalMintCapPerWallet == 0,
            "Max global allowed per address"
        );
        ...
        require(MerkleProof.verify(merkleProof, _merkleRoot, keccak256(abi.encodePacked(msg.sender))), "Invalid proof");

        _roundMint(roundId, amount);
    }

    /**
     * @notice airdrop Mint `amount` of tokens and transfers it to `wallet`
     *
     * @dev Requirements:
     * - View {ERC721VG-safeMint} Requirements
     *
     * @param wallet The address to send new tokens
     * @param amount The amount of tokens to send
     * @param amount useCurrentWave If we have to count these aidrops like a mint in the current wave (increase the current totalMinted)
     */
    function airdrop(
        address wallet,
        uint256 amount,
    ...
    function airdrops(
        ...
        _airdrops(wallets, amounts, useCurrentWave);
    }
     ...
     */
    function setupWave(uint256 newWaveId, uint256 supply) external virtual onlyOwner {
     ...
     * @param supply Number of tokens that can be minted in this wave. Can be 0 for use the total supply
     * @param maxMintsPerWallet Number of tokens that a wallet can mint in this round
     * @param _merkleRoot root of Merkle tree for this round
    ...
    function setupRound(
        ...
        _setupRound(roundWaveId, roundId, supply, maxMintsPerWallet, _merkleRoot, startTime, duration, price);
    }
    ...
    function _setupRound(
        ...
        require(roundId <= waveToRoundsLength[roundWaveId] + 1, "Invalid roundId");
        ...
        round.supply = supply;
        round.maxMintsPerWallet = maxMintsPerWallet;
        round.merkleRoot = _merkleRoot;
        round.startTime = startTime;
        round.duration = duration;
        round.price = price;

        emit RoundSetup(roundWaveId, roundId, supply, maxMintsPerWallet, _merkleRoot, startTime, duration, price);
    }

    /**
     * @notice Change number of tokens that a wallet can mint globally
     */
    function setGlobalMintCapPerWallet(uint32 newMaxMints) external virtual onlyOwner {
        globalMintCapPerWallet = newMaxMints;
        emit GlobalMintCapPerWalletChanged(newMaxMints);
    }

    /**
     * @notice All collections start at 1
     */
    ...
    function setBaseURI(string memory newBaseURI) external virtual onlyOwner {
        require(!baseURILocked, "baseURI permanently locked");
        baseURI = newBaseURI;
    ...
    /**
     * @notice Set royalties. Marketplace should be compatible with this protocol.
     * @dev Requirements:
     * - View {EIP2981-_setDefaultRoyalty} requirements
     * - fees are expressed in basis points of `feeNumerator` div by feeDenominator that defaults to 10000
    ...
    /**
     * @notice execute withdraw. If error, revert
     */
    ...
    /**
     * @notice withdraw ERC20(ETH) tokens
        to the list of addresses with their corresponding percentages
        that where provided upon deployment
        It calls _withdraw
     * @dev Requirements:
     * - Upon deployment checked:
     *      - a list of minimum 2 addresses is provided
     *      - length of addresses list is equal to percentage list
     * 
    */
     ...
     * @param newBurnable If users are authorized to burn their tokens or not
     */
    function _setBurnable(bool newBurnable) internal virtual onlyOwner {
     ...
     * - approvalCheck = True to clear approvals when the token is burned.
     */
    ...
    /**
     * @notice Returns details of the actual round
     */
    function rounds(uint256 roundWaveId, uint256 roundId) public view returns (Round memory) {
    ...
    function _roundMint(uint256 roundId, uint256 amount) internal virtual {
        ...
        require(
            block.timestamp >= round.startTime &&
                round.startTime > 0 &&
                (round.duration == 0 || block.timestamp < round.startTime + round.duration),
        ...
        require((round.supply == 0 || round.totalMinted + amount <= round.supply), "Round supply exceeded");

        // Wave supply requirements
        require((waveSupply == 0 || waveTotalMinted + amount <= waveSupply), "Wave supply exceeded");
     ...
     * - View {ERC721A-_mint} Requirements
     *
     * @dev Increase `totalMinted`
     *
     * @param wallet The wallet to transfer new tokens
     * @param amount The number of tokens to mint
     */
    function safeMint(address wallet, uint256 amount) internal virtual {
        require(amount > 0, "Zero amount");
        require(_totalMinted() + amount <= collectionSize, "Collection supply exceeded");
