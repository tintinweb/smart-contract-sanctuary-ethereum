...
contract ERC721Staking is Ownable, IERC721Receiver, Pausable, ReentrancyGuard {
    ...
    function unstake(uint256[] calldata tokenIds) external whenNotPaused nonReentrant {
        for (uint256 i; i < tokenIds.length; i++) {
            ...
            delete _depositTimes[msg.sender][tokenIds[i]];
            IERC721(erc721Address).safeTransferFrom(
                address(this),
                msg.sender,
                tokenIds[i],
    ...
    function setTokensPerSecond(uint256 _rate) public onlyOwner {
        rate = _rate;
    }
        ...
        returns (uint256[] memory rewards)
    {
        ...
        for (uint256 i; i < tokenIds.length; i++) {
            ...
            diff = block.timestamp - _depositTimes[account][tokenId];
    ...
    {
        require(
            block.timestamp > _depositTimes[account][tokenId],
            "Invalid timestamp"
        );
        uint256 diff = block.timestamp - _depositTimes[account][tokenId];
        return rate * (_deposits[account].contains(tokenId) ? 1 : 0) * diff;
    }
    ...
    function claimRewards() public whenNotPaused nonReentrant {
        ...
        for (uint256 i; i < tokenIds.length; i++) {
            ...
            _depositTimes[msg.sender][tokenIds[i]] = blockTimestamp;
        }
        if (reward > 0) {
            ERC20Token(erc20Address).mint(msg.sender, reward);
        }
    }
