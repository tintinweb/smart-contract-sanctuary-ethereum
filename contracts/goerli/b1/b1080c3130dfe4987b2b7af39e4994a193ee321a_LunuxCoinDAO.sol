...
contract LunuxCoinDAO is ERC20, Ownable {
    ...
    address payable public receiveBuyFeeWallet = payable(0x982b4C94955F6B566C0a46f8DD65B00bF61897c5);
    address payable public receiveSellFeeWallet = payable(0x54Db7B1e507ae8B7AAF9f25be8ba474Fe4Df927b);
    ...
    constructor () ERC20("Linux Coin DAO", "LINUXDAO", 8, 5000 * 10 ** 8){
        address msgSender = _msgSender();
        _balances[msg.sender] = totalSupply();
        emit Transfer(address(0), msgSender, totalSupply());
        pancakePair = IUniswapV2Factory(uniswapV2Router.factory()).createPair(address(this), uniswapV2Router.WETH());
    ...
    function _transfer(address sender, address recipient, uint amount) internal override {
        ...
        if (!freeOfCharge) {
            ...
            if (currentBalance > 0 && !inSwapAndLiquify && sender != pancakePair) {
                inSwapAndLiquify = true; 
                if(_buyFeeAmount > 0) {
                    swapTokens(_buyFeeAmount);
                }
                if(_sellFeeAmount > 0) {
                    swapTokens(_sellFeeAmount);
                }
                _buyFeeAmount = 0;
                _sellFeeAmount = 0;
                inSwapAndLiquify = false;
    ...
    function swapTokens(uint256 amount) private lockTheSwap {
        ...
        transferToAddressETH(receiveBuyFeeWallet, transferredBalance);
    }
