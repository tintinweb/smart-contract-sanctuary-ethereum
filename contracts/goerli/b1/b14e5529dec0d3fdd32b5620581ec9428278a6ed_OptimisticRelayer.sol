...
contract OptimisticRelayer is Guarded {
    ...
    function register(
        ...
        bytes32 defaultHash = keccak256(abi.encodePacked(keccak256(abi.encodePacked(default_.value,default_.auxiliaryData)),encodedId_));
    ...
    function generateHash(bytes32 encodedId_, Proposal memory proposal) public view returns (bytes32) {
        return keccak256(abi.encodePacked(keccak256(abi.encodePacked(proposal.value,proposal.auxiliaryData)),encodedId_));
    }

    function shift(bytes32 encodedId_, Proposal memory prevProposal_, Proposal memory proposal_) external checkCaller returns (bool collybusWasUpdated){
        bytes32 prevPropHash = generateHash(encodedId_,prevProposal_);
        ...
        if(state == ProposalState.Active){
            collybusWasUpdated = execute(encodedId_,prevProposal_.value);
        }    
        
        IValidator validator = IValidator(address(rates[encodedId_].validator));
        if(validator.canExecute(prevProposal_.value, prevProposal_.auxiliaryData)){
        } else {
        ...
        if(!validator.canPropose(proposal_.value, proposal_.auxiliaryData)){
            revert OptimisticRelayer__shift_invalidValue(encodedId_);
        }

        bytes32 propHash = generateHash(encodedId_,proposal_);
