...
contract RugPool is Context, Ownable {
    ...
    constructor() {         
        ...
        paymentTokenAddress = 0x58f68b0A84532976b80Fc255365a8D3723133a76; //USDT
    ...
    function Payout() external {      
        ...
        if(isScheduled == 1) {
            require (block.timestamp >= (player.lastWithdrawn + 60 ), "Not due yet for next payout!");
        }     
    ...
    function computePayout(address _addr) view external returns(uint256 value) {
        ...
        for(uint256 i = 0; i < player.deposits.length; i++) {
            ...
            uint256 time_end = dep.time + tarif.life_days * 60;
            uint40 from = player.lastWithdrawn > dep.time ? player.lastWithdrawn : dep.time;
            uint256 to = block.timestamp > time_end ? time_end : block.timestamp;
            if(from < to) {
                value += dep.amount * (to - from) * tarif.percent / tarif.life_days / 6000;
            }
        }
        return value;
    }
    ...
    function nextWithdraw(address _addr) view external returns(uint40 next_sked) {
        ...
        {
          return uint40(player.lastWithdrawn + (60));
        }
        return 0;
    }
