...
// TODO replace Elbow with actual contract name
contract Stand is ERC721V, Ownable, IERC2981 {
    ...
    mapping(uint256 => uint256) public tokenLockupTimestamp;
    ...
    constructor(
        ...
        address operatorFilterRegistryAddress_,
        ...
        operatorFilterRegistryAddress = operatorFilterRegistryAddress_;
    }
    ...
    function setRoyaltyBps(uint256 bps) external onlyOwner {
        // TODO change to a different max?
        if (bps > MAX_BPS) revert();
        _royaltyBPS = bps;
    }

    /**
     * Allows the owner to a new registrant address. 
     */
     ...
     * Allows the owner to withdraw a certain amount of ETH to a certain address.
     */
    function mintAir(address[] memory mintAddresses, uint256[] memory mintQuantities) external onlyOwner {
    ...
    function mintWater(address[] memory presaleUsers) external onlyOwner {
        ...
        unchecked {
            for (uint i = 0; i < presaleUsers.length; i++) {
                ...
                for (uint j = 0; j < tokensOwed / 3; j++) {
                    ...
                    emit TokenLocked(start);
                    emit TokenLocked(start + 1);
                    emit TokenLocked(start + 2);
                ...
                if (tokensOwed % 3 == 2) {
                    ...
                    emit TokenLocked(nextId - 2);

                    _setExtraDataAt(nextId - 1, secondUnlockTime);
                    emit TokenLocked(nextId - 1);
     ...
     * 
     * TODO clear all the mints at the lowest price.
     * 
     * @dev Only callable by the owner.
     */
    ...
    function mintFire(string memory tier, address addr, uint256 price, uint256 version, uint256 allowedAmount, uint256 buyAmount, bytes memory sig) external payable {
    ...
    function mintEarth() external payable {
    ...
    function _beforeApproval(
        ...
        if (IOperatorFilterRegistry(operatorFilterRegistryAddress).isOperatorFiltered(filterRegistrant, approvedAddr))
    ...
    function _beforeTokenTransfers(
        ...
        if (IOperatorFilterRegistry(operatorFilterRegistryAddress).isOperatorFiltered(filterRegistrant, msg.sender))
            revert OperatorNotApproved();
