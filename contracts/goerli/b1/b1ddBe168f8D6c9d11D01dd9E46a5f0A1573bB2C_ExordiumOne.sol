...
abstract contract Base721 is ERC721Enumerable, Ownable {
    ...
    function _mintToTarget(address target, uint256 quantity) internal virtual {
        ...
        for (uint256 i; i < quantity; i++) {
            _safeMint(target, _tokenIdTracker.current());
...
contract ExordiumOne is Base721 {
    using Counters for Counters.Counter;
    using MerkleProof for bytes32[];
    uint256 public constant mintPrice = 0;
    bytes32 public immutable claimMerkleRoot;
    ...
    constructor(
        ...
        "Atmos | Exordium Chapter One",
        "EXORDIUM-01",
        _uri,
        2500
    ) {
        mintLimit = _ml;
        claimMerkleRoot = _merkelRoot;
    }
    ...
    function allowlistMint(
        ...
        _mintToTarget(account, quantity);
    }
