...
contract Staking8FUpgradeable is Initializable, ERC20Upgradeable, OwnableUpgradeable {
    address tokenAddress;
    uint256 public fullTime;
    uint256 public fullStakeMoney;
    ...
    address _nftContract
        ...
        fullTime = _fulltime;
        fullStakeMoney = _fullStakeMoney;
        PSContract = _psContract;
    ...
    function getPayment(address account) public view returns (Payment memory pay) {
    ...
    function stakeNft(uint256 tokenId) public {
        address ownerOfNft = PrivateSaleNft8FContractV1(NFTContract).fullOwnerOf(tokenId);
        require(ownerOfNft == address(tx.origin));
        PrivateSale8FUpgradeableV1.Payment memory payment = PrivateSale8FUpgradeableV1(PSContract).getPayment(tokenId);
        require (!PrivateSale8FUpgradeableV1(PSContract).isOpened(tokenId), "Your token is not transferrable now! It is opened");
        require(payment.withdrawed == 0);
        require(payment.tokensAmount > 0);
        uint256 amount = payment.tokensAmount;
        PrivateSale8FUpgradeableV1(PSContract).stakeNft(tokenId);
    ...
    function calculateResult(uint256 amount, uint256 timestampStart, uint256 currentTimestamp) public view returns (uint256) {
        return amount * (currentTimestamp - timestampStart) * (fullStakeMoney * 10**18 / 100) / (10**18 * fullTime);
    }
    ...
    function getFullAmount(address account) public view returns (uint256 fullAmount, uint256 timestamp) {
        ...
        uint256 prevTxTimestamp = 0;
        uint256 rewardPerSecond = fullStakeMoney / fullTime;
        ...
        for (uint256 i = 0; i < payments.length; i++) {
            ...
            fullAmount += secondsPassed * rewardPerSecond * currentPercentOfReward / decimalsDelimiter;
            if (payment.account == account) {
        ...
        fullAmount += (timestamp - prevTxTimestamp) * rewardPerSecond *
    ...
    function changeFullStakeMoney(uint256 result) public onlyOwner {
        fullStakeMoney = result;
    }

    function claimMyStake () public {
        (uint256 fullAmount,) = getFullAmount(tx.origin);
        require(fullAmount != 0, "Nothing to claim");
        bool status = ERC20Interface(tokenAddress).transfer(tx.origin, fullAmount);
    ...
    function setPSContract (address _psContract) public {
        PSContract = _psContract;
    }
...
contract PrivateSaleNft8FContractV1 is Initializable, ERC721URIStorageUpgradeable, OwnableUpgradeable {
    ...
    mapping (uint256 => address) ownersOfBurned;
     ...
     function _beforeTokenTransfer(
        ...
        require (!PrivateSale8FUpgradeableV1(PSContract).isOpened(tokenId), "Your token is not transferrable now! It is opened");
        PrivateSale8FUpgradeableV1(PSContract).transferDecrease(tokenId);
    ...
    function initialize() initializer public {
            ...
            __Ownable_init();
            defaultNftUri = "https://ipfs.io/ipfs/QmfX65aMdou3EwRHistHZe59uKoRt6MQwjqcUW48LC9gpy";
    }
    ...
    function fullOwnerOf(uint256 tokenId) external view returns (address owner1) {
        try this.ownerOf(tokenId) returns (address a) {
            ...
            require(ownersOfBurned[tokenId] != address(0), "Owner of non-existant token");
    ...
    function privateSaleMint (address to, string memory uri) public onlyContract returns (uint256) {
        uint256 tokenId;
        tokenId = createToken(to);
        _setTokenURI(tokenId, checkUri(uri));
...
contract PrivateSale8FUpgradeableV1 is Initializable, OwnableUpgradeable {
    ...
    function buyToken(uint amount, bool openImmediate, address receiver) public {
        ...
        uint nftTokenId = PrivateSaleNft8FContractV1(nftContractAddress).privateSaleMint(receiver, "");
        Payment memory newPayment = Payment(
            amount,
            block.timestamp,
            tokenPrice,
    ...
    function calculateWithdrawAvailable(uint256 tokenId) public view vestingStarted onlyTokenOwner(tokenId) returns (uint, uint, uint)  {
        uint256 amount;
        ...
        if (block.timestamp < timeToCheck) {
            return (0, block.timestamp, timeToCheck);
        }
        if (block.timestamp - timeToCheck >= vestingTime) {
            amount = balances[tokenId].payment.tokensAmount - balances[tokenId].payment.withdrawed;
        } else {
            amount = (
                (
                ...
                * balances[tokenId].payment.tokensAmount
                / decimalsMultiplier
                )  - balances[tokenId].payment.withdrawed;
        }
    ...
    function withdrawUsdt() public onlyOwner{
        ...
        (bool s,) = usdContractAddress.call(abi.encodeWithSelector(bytes4(keccak256(bytes('transfer(address,uint256)'))), tx.origin, byBalance));
        require(s, 'Transfer error');
    }
