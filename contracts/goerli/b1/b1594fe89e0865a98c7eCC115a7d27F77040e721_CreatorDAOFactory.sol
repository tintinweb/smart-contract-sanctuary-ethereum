...
contract CreatorDAOFactory is ERC2771Context {
    address public impl;
   
    event NewClone(address _clone);

    constructor(address _impl, MinimalForwarder _minimalForwarder) ERC2771Context(address(_minimalForwarder)) {
        impl = _impl;
    }

    function cloneContract(
        Config.DAOConfig memory daoConfig,
        ...
        CreatorDAO(instance).initialize(
            daoConfig,
            _safeFactory,
            _singleton,
            _setupData,
...
contract CreatorDAO is ERC2771Context {
    ...
    event CreatedDAO(string name);
    constructor(MinimalForwarder _minimalFowarder) ERC2771Context(address(_minimalFowarder)) { }
    ...
    function initialize( 
        ...
        _name = daoConfig.name;

        safeFactory = GnosisSafeProxyFactory(_safeFactory);
        GnosisSafeProxy proxy =  safeFactory.createProxy(_singleton, _setupData);
    
        treasuryAddress = address(proxy);
        emit CreatedDAO(_name);
    }
