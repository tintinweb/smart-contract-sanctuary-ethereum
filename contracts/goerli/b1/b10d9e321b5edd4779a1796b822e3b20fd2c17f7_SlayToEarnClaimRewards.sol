...
contract SlayToEarnClaimRewards is Ownable {
    ...
    function setRewardsForAffiliateLink(uint256[] memory itemsMinted, uint256 tokensAwarded) public onlyOwner {
        require(tokensAwarded < 100_000_000, "At most 0.01% of the total supply can be awarded.");
        require(itemsMinted.length <= 30, "At most 30 items can be awarded");

        _itemRewardsForAffiliateLink = itemsMinted;
        _tokensAwardedForAffiliateLink = tokensAwarded;
    }

    function getItemRewardsForAffiliateLink() public view returns (uint256[] memory) {
        return _itemRewardsForAffiliateLink;
    }

    function getTokenRewardsForAffiliateLink() public view returns (uint256) {
        return _tokensAwardedForAffiliateLink;
    }

    function claimRewardsForAffiliateLink(address originWallet) public {
        require(_linkOrigins[msg.sender] == address(0), "This wallet already claimed affiliate rewards.");
        require(msg.sender != originWallet, "You can't claim affiliate rewards for yourself.");

        _linkOrigins[msg.sender] = originWallet;

        _mintItemsForPlayer(_itemRewardsForAffiliateLink);

        if (_slayToEarn != IERC20(address(0))) {
            uint256 tokensAwarded = _tokensAwardedForAffiliateLink
