 ...
 */
abstract contract Pausable is Context, Initializable {
    /**
...
contract BullPool is Ownable, Pausable {
    ...
    event NewVBullContract(address VBull);
     ...
     * @param _pid: bull pool ID in MasterChefV2
     */
    function initialize(
        IERC20 _token,
    ...
    function init(IERC20 dummyToken) external onlyOwner {
        ...
        masterchefV2.deposit(bullPoolPID, balance);
        emit Init();
    }
    ...
    /**
     * @notice Checks if the msg.sender is either the bull owner address or the operator address.
     */
    modifier onlyOperatorOrBullOwner(address _user) {
        require(
            msg.sender == _user || msg.sender == operator,
            "Not operator or bull owner"
        );
        _;
    }
    ...
    function updateBoostContractInfo(address _user) internal {
        if (boostContract != address(0)) {
            ...
            IBoostContract(boostContract).onBullPoolUpdate(
                _user,
                user.lockedAmount,
                lockDuration,
                totalLockedAmount,
    ...
    /**
     * @notice Unlock user bull funds.
     * @dev Only possible when contract not paused.
     * @param _user: User address
     */
    function unlock(address _user)
        external
        onlyOperatorOrBullOwner(_user)
        whenNotPaused
    {
        UserInfo storage user = userInfo[_user];
        require(
    ...
    /**
     * @notice Deposit funds into the Bull Pool.
     * @dev Only possible when contract not paused.
    ...
    function depositOperation(
        ...
        if (VBull != address(0)) {
            IVBull(VBull).deposit(_user, _amount, _lockDuration);
        }
    ...
    function withdrawOperation(uint256 _shares, uint256 _amount) internal {
        ...
        if (VBull != address(0)) {
            IVBull(VBull).withdraw(msg.sender);
        }
    ...
    /**
     * @notice Harvest pending BULL tokens from MasterChef
     */
    function harvest() internal {
        uint256 pendingBull = masterchefV2.pendingBull(
            bullPoolPID,
            address(this)
        );
        if (pendingBull > 0) {
            uint256 balBefore = available();
            masterchefV2.withdraw(bullPoolPID, 0);
    ...
    /**
     * @notice Set VBull Contract address
     * @dev Callable by the contract admin.
     */
    function setVBullContract(address _VBull) external onlyAdmin {
        require(_VBull != address(0), "Cannot be zero address");
        VBull = _VBull;
        emit NewVBullContract(VBull);
    }
    ...
    /**
     * @notice Withdraw unexpected tokens sent to the Bull Pool
     */
        ...
        returns (uint256)
    {
        ...
        if (
            ...
            uint256 pool = balanceOf() + calculateTotalPendingBullRewards();
     ...
     * @return Returns total pending bull rewards
     */
    function calculateTotalPendingBullRewards() public view returns (uint256) {
        uint256 amount = masterchefV2.pendingBull(bullPoolPID, address(this));
        return amount;
    }
    ...
    function getPricePerFullShare() external view returns (uint256) {
        return
            totalShares == 0
                ? 1e18
                : (((balanceOf() + calculateTotalPendingBullRewards()) *
                    (1e18)) / totalShares);
    }
