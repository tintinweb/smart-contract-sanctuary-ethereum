...
interface PreSale {
    ...
    function getTokenBuyers(uint256 _tokenBuyer) external view returns(address, uint256);
}
...
contract StakingContract {
    ...
    mapping(address => uint256) private areenaLastTransactionForBusd;
    mapping(address => uint256) private areenaLastTransactionForGen;
    ...
    event battleJoiner(
        address indexed battleJoiner,
        uint256 stakeAmount,
        uint256 indexed battleId
    );
    event winnerDetails(
        uint256 indexed winnerStakedAmount,
        uint256 winnerGenBonus,
        uint256 indexed winnerAreenaBonus
    );
    event loserDetails(
        uint256 indexed looserStakedAmount,
        uint256 looserGenBonus
    );
    event leaveBattleDetails(
        address creator,
        address joiner,
        uint256 _battleId
    );
    event withdrawThreePercentage(
        address withdrawerAddress,
        uint256 withdrawAmount,
        uint256 remainingAmount,
        uint256 nextTime
    );
    event withdrawFivePercentage(
        address withdrawerAddress,
        uint256 withdrawAmount,
        uint256 remainingAmount,
        uint256 nextTime
    );
    event withdrawSevenPercentage(
        address withdrawerAddress,
        uint256 withdrawAmount,
        uint256 remainingAmount,
        uint256 nextTime
    ...
    event areenaBooster(
        address buyer,
        uint256 priceUserPaid,
        uint256 newWalletLimit
    );
    event claimReferalAmount(
        uint256 referalAmount,
        uint256 nextTime,
        uint256 castleAmount
    );
    event areenaTokenSold(
        address sellerAddress,
        uint256 lowerMileStone,
        uint256 upperMileStone
    );
    event referalInfo(
        address joinerRefaralPerson,
        address creatorReferalPerson,
        uint256 joinerReferalAmount,
        uint256 creatorReferalAmount,
        uint256 battleId,
        uint256 joinerReferalTime,
        uint256 creatorReferalTime
    );
    ...
    function sendPreSaleTokens() external onlyOwner {

       uint256 length  = preSale.getTotalBuyers();

       for(uint i = 0; i<= length; i++){

            (address _playerAddress, uint256 _noOfTokens) = preSale.getTokenBuyers(i);

            players[_playerAddress].genAmountPlusBonus += _noOfTokens.mul(1e18);
            genToken.transferFrom(preSaleWallet, address(this), _noOfTokens);
    ...
    function CreateBattle(
        ...
        require(
            ...
            "You must stake atleast 25 Gen tokens to enter into the battle."
        );
        require(
            stakeAmount <= (stakeOptions[4].mul(1e18)),
            "You can not stake more then 1000 Gen tokens to create a battle."
        );

        require(
            (genToken.balanceOf(battle.creator) + player.genAmountPlusBonus) >=
                stakeAmount,
            "You does not have sufficent amount of gen Token."
        );
        require(
            _riskPercentage == riskOptions[0] ||
                _riskPercentage == riskOptions[1] ||
                _riskPercentage == riskOptions[2],
    ...
    function JoinBattle(
        ...
        require(
            (genToken.balanceOf(battle.joiner) + player.genAmountPlusBonus) >=
                stakeAmount,
            "You does not have sufficent amount of gen Token."
        );
        ...
        stakeCount[_battleId][battle.creator] = players[battle.creator]
            .battleCount;
        stakeCount[_battleId][battle.joiner] = players[battle.joiner]
            .battleCount;

        playerBattleIds[battle.joiner].push(_battleId);

        uint256 creatorDeductedAmount = calculateCreatorPercentage(stakeAmount);
        uint256 creatorAfterDeductedAmount = stakeAmount -
            creatorDeductedAmount;
        ...
        referalTime[_battleId][battle.joiner][
        ...
        sendCreatorDeductionAmount =
            creatorDeductedAmount -
            creatorReferalAmount;
        genToken.transfer(treasuryWallet, sendCreatorDeductionAmount);

        players[referalPerson[_battleId].creatorReferalPerson]
            .referalAmount += creatorReferalAmount;

        uint256 creatorReferalTime = block.timestamp;
        referalTime[_battleId][battle.creator][
        ...
        players[battle.creator]
            .battleRecord[stakeCount[_battleId][battle.creator]]
            .amount = creatorAfterDeductedAmount;
        players[battle.joiner]
            .battleRecord[stakeCount[_battleId][battle.joiner]]
            .amount = joinerAfterDeductedAmount;
    ...
    function settingReferalInfo(
        uint256 _referalTime,
        uint256 _referalAmount,
        address _referalPerson,
        address _battlePerson
    ) internal {
        claimRferalAmount[_battlePerson][_referalPerson][
            _referalTime
        ] = _referalAmount;
    }
    ...
    function LeaveBattle(uint256 _battleId) public {
        ...
        require(
            msg.sender == battle.creator || msg.sender == battle.joiner,
            "You must be a part of a battle before leaving it."
        );
        require(
            !battle.leaved,
            "You canot join this battle because battle creator Already leaved."
        );
        ...
        if (!battle.joined) {
            if (
                block.timestamp > (battle.startingTime + 15 minutes)
            ) //////////////////////48 hours add
            {
                uint256 _tokenAmount = battle.stakeAmount;

                uint256 deductedAmount = calculateSendBackPercentage(
                    _tokenAmount
                );
                ...
                players[battle.creator].genAmountPlusBonus += battle
                    .stakeAmount;
                battle.leaved = true;
            }
        } else {
            ...
            uint256 losertokenAmount = players[battle.loser]
                .battleRecord[stakeCount[_battleId][battle.loser]]
                .amount;
            uint256 winnertokenAmount = players[battle.winner]
                .battleRecord[stakeCount[_battleId][battle.winner]]
                .amount;
            ...
            uint256 loserGenReward = calculateRewardInGen(
                losertokenAmount,
                totalMinutes
            );
            uint256 winnerGenReward = calculateRewardInGen(
                winnertokenAmount,
                totalMinutes
            );

            uint256 riskDeductionFromLoser = calculateRiskPercentage(
                loserGenReward,
                battle.riskPercentage
            );

            uint256 loserFinalGenReward = loserGenReward -
                riskDeductionFromLoser;

            uint256 winnerAreenaReward = calculateRewardInAreena(
                battle.stakeAmount,
                totalMinutes
            );

            uint256 sendWinnerGenReward = winnerGenReward +
                riskDeductionFromLoser +
                winnertokenAmount;
            ...
            players[battle.winner]
                .battleRecord[stakeCount[_battleId][battle.winner]]
                .genBonus = (winnerGenReward + riskDeductionFromLoser);
            players[battle.winner]
                .battleRecord[stakeCount[_battleId][battle.winner]]
                .areenaBonus = winnerAreenaReward;
            players[battle.loser]
                .battleRecord[stakeCount[_battleId][battle.loser]]
                .genBonus = loserFinalGenReward;
    ...
    function GenWithdraw(uint256 _percentage) external {
        ...
        require(
            player.genAmountPlusBonus > 0,
            "You do not have sufficent amount of tokens to withdraw."
        );

        if (_percentage == 3) {
            require(
                genLastTransaction[msg.sender] < block.timestamp,
                "You canot withdraw amount before 13 minutes"
            );
            genLastTransaction[msg.sender] = block.timestamp + 13 minutes; //hours/////////////////////////

            uint256 sendgenReward = calculateWithdrawThreePercentage(
            ...
            require(
                genLastTransaction[msg.sender] < block.timestamp,
                "You canot withdraw amount before 15 minutes"
            );
            genLastTransaction[msg.sender] = block.timestamp + 15 minutes; //hours//////////////////////

            uint256 sendgenReward = calculateWithdrawFivePercentage(
                player.genAmountPlusBonus
            );
            genToken.transfer(msg.sender, sendgenReward);
            ...
            require(
                genLastTransaction[msg.sender] < block.timestamp,
                "You canot withdraw amount before 17 minutes"
            );
            genLastTransaction[msg.sender] = block.timestamp + 17 minutes; //hours/////////////////

            uint256 sendgenReward = calculateWithdrawSevenPercentage(
                player.genAmountPlusBonus
            );
            genToken.transfer(msg.sender, sendgenReward);
            ...
            require(
                _percentage == 3 || _percentage == 5 || _percentage == 7,
                "Enter the right amount of percentage."
            );
        }
    }
    ...
    function BuyAreenaBoster(uint256 _areenaBosterPrice) external {
        // _areenaBosterPrice get in weii
        ...
        require(
            busdToken.balanceOf(msg.sender) >= _areenaBosterPrice,
    ...
    function sellAreenaByBusd(uint256 _tokenAmount) external {
        ...
        require(
            _realTokenAmount < _walletLimit,
            "Please Buy Areena Boster To get All of your reward."
        );
        require(
            _realTokenAmount <= (3 * 1e18),
            "You can sell only three areena Token per day."
        );
        ...
        if (!onceGreater) {
            require(
                (busdToken.balanceOf(busdWallet) + (90000 * 1e18)) >=
                    (101000 * 1e18),
        ...
        if (
            ...
            require(
                block.timestamp > areenaLastTransactionForBusd[msg.sender],
                "You canot sell areena token again before 24 hours."
            );

            uint256 sendAmount = _tokenAmount.mul(calculateAreenaPrice());
            uint256 checkBalance = (busdToken.balanceOf(busdWallet) +
                (90000 * 1e18)) - sendAmount;

            require(
                checkBalance > lowerMileStone,
            ...
            areenaLastTransactionForBusd[msg.sender] =
                block.timestamp +
                4 minutes; //////////hours////////////////
    ...
    function sellAreenaByGen(uint256 _areenaAmount, uint256 _genPrice) external {
        ...
        uint256 _totalAmountOfGen = areenaPriceInBusd.div(_genPrice);

        console.log("areenaPriceInBusd", areenaPriceInBusd);
        console.log("_totalAmountOfGen", _totalAmountOfGen);

        uint256 amountAdd = calculateSellTax(_totalAmountOfGen);

            console.log("amountAdd", amountAdd);
        ...
        console.log("amountAdd1", amountAdd);
        players[msg.sender].genAmountPlusBonus += amountAdd;

            console.log("_realTokenAmount", _realTokenAmount);
            console.log("areenaInCirculation", areenaInCirculation);
        ...
        players[msg.sender].totalArenaTokens -= _realTokenAmount;
             ...
             console.log("amountAdd4", amountAdd);
    ...
    function claimReferalBonus(uint256 _battleId) public returns (bool v) {
        ...
        if (msg.sender == referalPerson[_battleId].creatorReferalPerson) {
            uint256 referallTime = referalTime[_battleId][battle.creator][
                msg.sender
            ];
            require(
                block.timestamp > referallTime.add(7 minutes),
                "You can not claim bonus before 7 minutes."
            ); //////////Add 7 days here /////////

            uint256 referallAmount = claimRferalAmount[battle.creator][
                msg.sender
            ][referallTime];

            require(
                players[msg.sender].genAmountPlusBonus >
                    (referallAmount.mul(5)),
                "Your castle gen amount must be five times of your referal amount"
            );
            ...
            uint256 referallTime = referalTime[_battleId][battle.joiner][
                msg.sender
            ];
            require(
                block.timestamp > referallTime.add(7 minutes),
            ...
            uint256 referallAmount = claimRferalAmount[battle.joiner][
                msg.sender
            ][referallTime];
            require(
                players[msg.sender].genAmountPlusBonus >
            ...
            require(
                referallPerson,
                "You can not claim reward because you are not the referalPerson of this battle."
            );
    ...
    function sellGenThroughDashboard(uint256 _busdAmount, uint256 priceOfGen) public {
        ...
        uint256 totalGenAmount = _busdAmount.div(priceOfGen);
        uint256 amountAdd = calculateSellTax(totalGenAmount);

        console.log("totalGenAmount", totalGenAmount);
        console.log("amountAdd", amountAdd);
        ...
        require(genToken.balanceOf(address(this)) >= amountAdd,"plateform didnt have sufficient amount of gen token to sell right now.");
    ...
    // ================ Join Battle Functionalities ===============

    function calculateCreatorPercentage(uint256 _amount)
        public
        pure
        returns (uint256)
    {
    ...
    function calculateJoinerPercentage(uint256 _amount)
        public
        pure
        returns (uint256)
    {
    ...
    function calculateReferalPercentage(uint256 _amount)
        public
        pure
        returns (uint256)
    {
    ...
    function calculateSendBackPercentage(uint256 _amount)
        public
        pure
        returns (uint256)
    {
    ...
    function calculateTotalMinutes(uint256 _endingTime, uint256 _startingTime)
        public
        pure
        returns (uint256 _totalMinutes)
    {
        _totalMinutes = ((_endingTime - _startingTime) / 60); // in minutes!
        return _totalMinutes;
    }

    function calculateRewardInGen(uint256 _amount, uint256 _totalMinutes)
        public
        view
        returns (uint256)
    {
        uint256 _initialPercentage = genRewardPercentage;
        _initialPercentage = (_initialPercentage * _totalMinutes).div(
            genRewardMultiplicationValue
        );

        uint256 value = (
            (_amount.mul(_initialPercentage)).div(
                100 * genRewardMultiplicationValue
            )
        );
        return value;
    }
    ...
    function calculateRewardInAreena(uint256 _amount, uint256 _battleLength)
        public
        view
        returns (uint256)
    {
        ...
        return
            (
                ((calculateZValue(realAreena) * _amount).div(525600)).mul(
                    _battleLength
                )
            ).div(100);
    }
    ...
    function calculateWithdrawThreePercentage(uint256 _amount)
        public
        pure
        returns (uint256)
    {
    ...
    function calculateWithdrawFivePercentage(uint256 _amount)
        public
        pure
        returns (uint256)
    {
    ...
    function calculateWithdrawSevenPercentage(uint256 _amount)
        public
        pure
        returns (uint256)
    {
    ...
    // ================= Areena Boster function ================

    function calculateBosterPercentage(uint256 _amount)
        public
        pure
        returns (uint256)
    {
    ...
    function calculateAreenaPriceForGen(uint256 _areenaAmount)
        public
        view
        returns (uint256)
    {
       uint256 _busdWalletBalance = BusdInTreasury();
        ...
        console.log("_busdWalletBalance", _busdWalletBalance);
        console.log("_areenaValue", _areenaValue);

        // return _areenaAmount.mul(_areenaValue);
        // console.log("_areenaAmount", _areenaAmount);

        uint256 b = _areenaAmount.mul(_areenaValue);
        console.log("b", b);
        return b;
    }
    ...
    // ============ sellGenThroughDashboard Functionalities ===============
    ...
    function playerStakeDetails(address _playerAddress, uint256 battleCount)
        public
        view
    ...
    function getAllBattleIds(address _playerAddress)
        external
        view
    ...
    function setGenRewardPercentage(uint256 _percentage, uint256 value)
        external
        onlyOwner
    {
    ...
    function getGenRewardPercentage() external view returns (uint256) {
        uint256 genReward = genRewardPercentage.div(
            genRewardMultiplicationValue
        );
        return genReward;
    }
