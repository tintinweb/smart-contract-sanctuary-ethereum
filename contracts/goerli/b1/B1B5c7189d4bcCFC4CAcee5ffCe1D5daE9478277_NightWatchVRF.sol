    ...
    IRandomSeedGenerator
{
    ...
    function getLatestSeed() external view returns (uint256) {
        ...
        if (requests[lastRequestId].state != 2) {
            revert RandomnessNeverFulfilled();
        }

        // Return the latest random seed
        return requests[lastRequestId].randomSeeds[0];
    }
        ...
        override
    {
        ...
        if (requests[requestId].state == 0) {
            revert RequestNotFound();
        }

        // Set the request data
        requests[requestId].randomSeeds = randomSeeds;
        requests[requestId].state = 2;
