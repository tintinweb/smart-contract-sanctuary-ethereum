...
contract OpenBox is Ownable {
    ...
    mapping(uint => string[]) public itemsFlagSub; // group => flags
    ...
    function setItemsFlagSub(uint index, string[] memory itemHash) external onlyOwner {
        itemsFlagSub[index] = itemHash;
    }
    ...
    function randomHash(uint group, uint nonce, uint tokenId, bool isSub) internal {
        uint resultNumber = random(++nonce, 100);
        string memory  hash = isSub ? itemsFlagSub[group][resultNumber] : itemsFlag[group][resultNumber];
        IERC721(items721).mint(_msgSender(), tokenId, hash);
    }
    function handleOpen(uint nonce, uint tokenId, bool isSub) internal {
        uint resultNumber = random(nonce, 100);
        nonces[nonce] = true;
        if(resultNumber < 2) randomHash(0, nonce, tokenId, isSub);
        else if(resultNumber >= 2 && resultNumber < 22) randomHash(1, nonce, tokenId, isSub);
        else randomHash(2, nonce, tokenId, isSub);
    }
    ...
    function openSub(uint nonce, uint[] memory tokenIds, uint[] memory amounts) public {
        require(!nonces[nonce], "OpenBox::open: nonce used");
        require(tokenIds.length == 1 && amounts.length == 1 && tokenIds[0] == 2, "OpenBox::open: invalid token ids");
        uint balanceOf = box.balanceOf(_msgSender(), 2);
        require(amounts[0] > 0 && amounts[0] <= 100 && amounts[0] <= balanceOf, "OpenBox::open: amount invalid");
        box.burnBatch(_msgSender(), tokenIds, amounts);
        uint tokenId = IERC721(items721).currentTokenId();
        for(uint i = 0; i < amounts[0] * 4; i++) {
            handleOpen(nonce+random(i, 1000), tokenId+1+i, true);
        }

    }
    function openFree(uint nonce, uint[] memory tokenIds, uint[] memory amounts) public {
        require(!nonces[nonce], "OpenBox::open: nonce used");
        require(tokenIds.length == 1 && amounts.length == 1 && tokenIds[0] == 0, "OpenBox::open: invalid token ids");
        uint balanceOf = box.balanceOf(_msgSender(), 0);
        require(amounts[0] > 0 && amounts[0] <= 100 && amounts[0] <= balanceOf, "OpenBox::open: amount invalid");
        box.burnBatch(_msgSender(), tokenIds, amounts);
        uint tokenId = IERC721(items721).currentTokenId();
        for(uint i = 0; i < amounts[0]; i++) {
