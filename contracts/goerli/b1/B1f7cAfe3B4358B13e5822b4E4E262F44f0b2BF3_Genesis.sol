    ...
    IGenesis
{
    ...
    /**
     *  @notice maxPerUser is amount limit of each user can buy
     */
    ...
    /**
     *  @notice baseURI store the value of the ipfs url of metadata royalties
     */
    ...
    event MintedBatch(uint256[] tokenIds, address receiver);
    event Minted(uint256 tokenId, address receiver);
    event MintedSpecialType(uint256 tokenId, address receiver);
     ...
     * @param maxPerUser_ Max of nft that one user can buy
    ...
    function initialize(
        ...
        genesisSupply = [4004, 1400, 140, 10, 1];
        rarities[0] = [65535, 7130, 6606, 472]; // [72.1, 25.2, 2.52, 0.18]
        aliases[0] = [0, 0, 1, 0];
    ...
    /**
     *  @notice Get limit nft from type ID.
     */
    ...
    function tokenURI(uint256 tokenId) public view override returns (string memory) {
        ...
        return
            bytes(currentBaseURI).length > 0
                ? string(
                    abi.encodePacked(currentBaseURI, "/", uint256(genesisInfos[tokenId].typeId).toString(), ".json")
                )
                : ".json";
    }
    ...
    function mintSpecialType(address receiver) external onlyAdmin notZeroAddress(receiver) {
        require(_tokenCounter.current() < TOTAL_SUPPLY && currentIndexes[TypeId.CREATOR] < genesisSupply[uint256(TypeId.CREATOR)], "Sold out");
        _tokenCounter.increment();
        uint256 tokenId = _tokenCounter.current();

        ++currentIndexes[TypeId.CREATOR];

        genesisInfos[tokenId].typeId = TypeId.CREATOR;
        genesisInfos[tokenId].slotId = currentIndexes[TypeId.CREATOR];
    ...
    function mint(address receiver) external onlyAdmin notZeroAddress(receiver) returns (uint256) {
        require(_tokenCounter.current() < TOTAL_SUPPLY, "Sold out");
        _tokenCounter.increment();
        uint256 tokenId = _tokenCounter.current();
    ...
    function mintBatch(
        ...
        require(_tokenCounter.current() + times <= TOTAL_SUPPLY, "Sold out");
    ...
    function _randomTypeId(uint256 seed) private view returns (TypeId) {
        ...
        if (currentIndexes[selectedType] < getMaxSupplyOf(selectedType) && selectedType != TypeId.CREATOR) {
            return selectedType;
        }

        // Always returns valid value
        for (uint256 i = 0; i < genesisSupply.length; i++) {
            if (currentIndexes[TypeId(i)] < getMaxSupplyOf(TypeId(i)) && TypeId(i) != TypeId.CREATOR) {
                return TypeId(i);
            }
        }
    ...
    function getAllTypeOfToken() external pure returns (string[5] memory) {
        return ["FAIRY", "FAIRYRARE", "TENSHI", "KAMI", "CREATOR"];
    }
