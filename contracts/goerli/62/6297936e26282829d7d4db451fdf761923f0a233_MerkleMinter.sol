...
contract MerkleMinter is Ownable {
  using Strings for uint256;

  string public contractURI;
  bytes32 public merkleRoot;
  uint256 public constant mintPrice = 0.07 ether;
  uint256 public constant mintMerklePrice = 0.21 ether;

  address public particlesAddress;
  Particles public particles;

  constructor(bytes32 _merkleRoot, address _particles) {
    merkleRoot = _merkleRoot;
    particlesAddress = _particles;
  ...
  function mintWithProof(
    ...
    if (msg.value != mintMerklePrice) {
      revert MintPriceNotPaid();
    }
    particles.mint(msg.sender, tokenId, editions);
  }

  function mint(uint256 tokenId, uint256 editions) public payable {
...
contract Particles is ERC1155, Ownable {
  using Strings for uint256;
  ...
  function spawn(
    ...
    // emit Spawned(id, ipfsHash, commit, publicMint);
  }
