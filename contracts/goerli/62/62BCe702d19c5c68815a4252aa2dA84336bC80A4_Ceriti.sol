    ...
    ERC2771Recipient
{
    ...
    function postMessage(string memory _text) public {
        messages.push(
            Message(_text, block.timestamp, _msgSender(), new uint256[](0))
        );
    }
    ...
    function getMessages(
        ...
        require(_startIndex < messages.length, "Invalid start index");
        uint256 endIndex = _startIndex + _pageSize;
        if (endIndex > messages.length) {
            endIndex = messages.length;
        }
        Message[] memory result = new Message[](endIndex - _startIndex);
    ...
    function getMessage(
        uint256 _id
    ) public view returns (string memory, uint256, address, uint256[] memory) {
        require(_id < messages.length, "Invalid message id");
        return (
            messages[_id].text,
            messages[_id].timestamp,
            messages[_id].author,
            messages[_id].commentIds
        );
    }
    ...
    function postComment(uint256 _messageId, string memory _text) public {
        require(_messageId < messages.length, "Invalid message id");
        comments.push(
            Comment(_text, block.timestamp, _msgSender(), new uint256[](0))
        );
        uint256 commentId = comments.length - 1;
        messages[_messageId].commentIds.push(commentId);
    }

    function getComment(
        uint256 _id
    ) public view returns (string memory, uint256, address, uint256[] memory) {
        require(_id < comments.length, "Invalid comment id");
        return (
            comments[_id].text,
            comments[_id].timestamp,
            comments[_id].author,
            comments[_id].commentIds
        );
    }
