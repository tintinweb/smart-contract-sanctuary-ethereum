...
contract RandomNFTMinter is Ownable, IERC721Receiver {
    ...
    mapping(address => bool) public allowedFreeMintAddresses;
    ...
    function setFreeMintAddresses(
        address[] calldata _addresses
    ) external onlyOwner {
        for (uint256 i = 0; i < _addresses.length; i++) {
            allowedFreeMintAddresses[_addresses[i]] = true;
        }
    }
...
*/
    ...
    function mintNFT(address recipient) external payable {
        ...
        bool isFreeMintAllowed = allowedFreeMintAddresses[recipient] &&
            !freeMintClaimed[recipient];

        if (isFreeMintAllowed) {
            require(msg.value == 0, "No payment required for a free mint.");
            freeMintClaimed[recipient] = true;
        } else {
            require(msg.value >= mintPrice, "Insufficient funds to mint NFT.");
        }
