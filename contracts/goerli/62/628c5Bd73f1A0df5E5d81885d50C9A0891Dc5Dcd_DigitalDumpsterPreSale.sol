...
contract DigitalDumpsterPreSale is ReentrancyGuard, Ownable {
    ...
    event Refund(address recipient, uint256 amount);
        ...
        modifier icoActive() {
        require(startICO > 0 && block.timestamp >= startICO && block.timestamp < endICO, "ICO must be active");
        _;
    }
    ...
    function setICOdates(uint startDate, uint endDate) external onlyOwner icoNotActive() {
        require(startDate >= block.timestamp, "Start date should be in the future");
        require(startDate < endDate, "Start date should be before end date");
