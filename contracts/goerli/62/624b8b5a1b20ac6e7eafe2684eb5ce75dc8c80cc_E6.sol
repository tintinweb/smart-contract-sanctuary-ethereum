...
import "./SmartConHuntBase.sol";
import "@chainlink/contracts/src/v0.8/interfaces/AggregatorV3Interface.sol";

contract E6 is SmartConHuntBase {
    AggregatorV3Interface internal priceFeed;

    constructor(address ethUsd) SmartConHuntBase("E6") {
        priceFeed = AggregatorV3Interface(ethUsd);
    }

    function mintNft(int256 priceOfEth) public {
        (, int256 priceFromFeed, , , ) = priceFeed.latestRoundData();
        require(priceOfEth == priceFromFeed, "Wrong price!");
        _mintNft();
    }
}
...
contract SmartConHuntBase is ERC721 {
    string public constant TOKEN_IMAGE_URI =
        "ipfs://QmfQJZUVF3Ujpk8LDK5fLsAmWeQmdCNDcEyKUgANYZizqM";
    uint256 internal s_tokenCounter;
    mapping(address => bool) private s_hasNft;

    constructor(string memory level)
        ERC721(
            string.concat("SmartCon 2022 Scavenger Hunt NFT | ", level),
            "S2S"
        )
    {
        s_tokenCounter = 0;
