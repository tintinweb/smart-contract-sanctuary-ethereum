...
contract NFTSwap {
    ...
    function createSwapRequest(
        address to,
        address[] memory nftsFrom,
        uint256[] memory tokenIdsFrom,
        address[] memory nftsTo,
        ...
        require(
            amountEther >= 0,
            "Amount Ether must be greater than or equal 0"
        );

        require(
            amountToken >= 0,
            "Amount Token must be greater than or equal 0"
        );

        for (uint i=0; i<nftsFrom.length; i++){
            require(
                IERC721(nftsFrom[i]).isApprovedForAll(msg.sender, address(this)),
                "Please approve this contract in the NFT contract before creating swap request"
            );
            require(
                IERC721(nftsFrom[i]).ownerOf(tokenIdsFrom[i]) == msg.sender,
                "The source NFT id does not belong to the sender"
            );
        }
        for (uint i=0; i<nftsTo.length; i++){
            require(
                IERC721(nftsTo[i]).ownerOf(tokenIdsTo[i]) == to,
        ...
        if (amountEther > 0) {
            ...
            address payable payable_to = payable(to);

            payable_to.transfer(amountEther);
        }

        if (amountToken > 0) {
            IERC20 paymentToken = IERC20(token);
            require(paymentToken.allowance(msg.sender, address(this)) >= amountToken,"Insuficient Token Allowance");
//            require(paymentToken.allowance(msg.sender, to) >= amountToken,"Insuficient Token Allowance");
            require(paymentToken.transferFrom(msg.sender,to,amountToken),"transfer token Failed");
        }
    ...
    function _swapNft(
        address _from,
        address _to,
        address[] memory _nftsFrom,
        uint256[] memory _tokenIdsFrom,
        address[] memory _nftsTo,
        uint256[] memory _tokenIdsTo
    ) internal {
        for (uint i=0; i<_nftsFrom.length; i++) {
            IERC721(_nftsFrom[i]).safeTransferFrom(_from, _to, _tokenIdsFrom[i]);
        }
        for (uint i=0; i<_nftsTo.length; i++) {
            IERC721(_nftsTo[i]).safeTransferFrom(_to, _from, _tokenIdsTo[i]);
        }
    ...
    function acceptSwapRequest(uint256 swapId) external  {
        ...
        for (uint i=0; i<currentRequest.nftsTo.length; i++){
            require(
                IERC721(currentRequest.nftsTo[i]).isApprovedForAll(
                    msg.sender,
                    address(this)
                ),
        ...
        _swapNft(
            currentRequest.from,
            currentRequest.to,
            currentRequest.nftsFrom,
            currentRequest.tokenIdsFrom,
            currentRequest.nftsTo,
            currentRequest.tokenIdsTo
        );
    }
        ...
        for (uint256 i = 0; i < requestCount; i++) {
            ...
            if (currentRequest.from == msg.sender) {
                SwapRequest memory request = SwapRequest(
                    ...
                    currentRequest.nftsTo,
                    currentRequest.tokenIdsTo,
                    currentRequest.amountEther,
                    currentRequest.amountToken,
