...
contract CyanPaymentPlanV2 is AccessControlUpgradeable, ReentrancyGuardUpgradeable {
    ...
    function fundBNPL(uint256 planId) external nonReentrant onlyRole(CYAN_ROLE) {
        ...
        CyanVaultV1(payable(items[planId].cyanVaultAddress)).lend(msg.sender, paymentPlan[planId].plan.amount);
    ...
    function activateBNPL(uint256 planId) external nonReentrant onlyRole(CYAN_ROLE) {
        ...
        if (paymentPlan[planId].status == PaymentPlanStatus.BNPL_CREATED) {
            ...
            CyanVaultV1(payable(_cyanVaultAddress)).lend(msg.sender, paymentPlan[planId].plan.amount);
        }
        transferItemAndLock(items[planId], paymentPlan[planId].cyanWalletAddress);
    ...
    function rejectBNPLPaymentPlan(uint256 planId) external payable nonReentrant onlyRole(CYAN_ROLE) {
        ...
        if (currencyAddress == address(0)) {
            payable(paymentPlan[planId].createdUserAddress).call{value: downPaymentAmount + singleServiceFee}("");
        } else {
            IERC20Upgradeable erc20Contract = IERC20Upgradeable(currencyAddress);
            erc20Contract.safeTransfer(paymentPlan[planId].createdUserAddress, downPaymentAmount + singleServiceFee);
        }
    ...
    function createPawn(
        ...
        if (cyanWalletAddress == msg.sender) {
            ...
            if (item.itemType == 3) {
                ICryptoPunk(item.contractAddress).buyPunk{value: 0}(item.tokenId);
            }
            transferItemAndLock(item, cyanWalletAddress);
        }

        CyanVaultV1 _cyanVaultV1 = CyanVaultV1(payable(item.cyanVaultAddress));
        _cyanVaultV1.lend(payable(msg.sender), plan.amount);
    ...
    function liquidate(uint256 planId, uint256 estimatedValue) external nonReentrant onlyRole(CYAN_ROLE) {
        ...
        CyanVaultV1(payable(_cyanVaultAddress)).nftDefaulted(unpaidAmount, estimatedValue);
    ...
    function transferEarnedAmountToCyanVault(
        ...
        if (currencyAddress == address(0)) {
            CyanVaultV1(payable(cyanVaultAddress)).earn{value: paidTokenPayment + paidInterestFee}(
                paidTokenPayment,
                paidInterestFee
            );
        } else {
            ...
            CyanVaultV1(payable(cyanVaultAddress)).earn(paidTokenPayment, paidInterestFee);
        }
    }
     ...
     */
    function transferItemAndLock(Item memory item, address cyanWalletAddress) private {
        if (item.itemType == 1) {
            // ERC721
            ERC721Upgradeable erc721Contract = ERC721Upgradeable(item.contractAddress);
            erc721Contract.safeTransferFrom(msg.sender, cyanWalletAddress, item.tokenId);
        } else if (item.itemType == 2) {
            // ERC1155
            ERC1155Upgradeable erc1155Contract = ERC1155Upgradeable(item.contractAddress);
            erc1155Contract.safeTransferFrom(msg.sender, cyanWalletAddress, item.tokenId, item.amount, bytes(""));
    ...
    function claimServiceFee(address currencyAddress) external nonReentrant onlyRole(DEFAULT_ADMIN_ROLE) {
        ...
        if (currencyAddress == address(0)) {
            payable(msg.sender).call{value: claimableServiceFee[currencyAddress]}("");
        } else {
            IERC20Upgradeable erc20Contract = IERC20Upgradeable(currencyAddress);
    ...
    function setAutoRepayStatus(uint256 planId, uint8 autoRepayStatus) external {
        if (
            paymentPlan[planId].createdUserAddress != msg.sender && paymentPlan[planId].cyanWalletAddress != msg.sender
        ) {
    ...
    function getCurrencyAddressByVaultAddress(address vaultAddress) private view returns (address) {
        return CyanVaultV1(payable(vaultAddress)).getCurrencyAddress();
    }
    ...
    function verifyRevivalSignature(
        ...
        bytes32 msgHash = keccak256(abi.encodePacked(planId, penaltyAmount));
        bytes32 signedHash = keccak256(abi.encodePacked("\x19Ethereum Signed Message:\n32", msgHash));
        if (signedHash.recover(signature) != cyanSigner) revert InvalidSignature();
    }
    ...
    PausableUpgradeable
{
    ...
    function withdraw(uint256 tokenAmount) external nonReentrant whenNotPaused {
        ...
        require(balance >= tokenAmount, "Check the token balance");
        ...
        if (hasNativeCurrency) {
            ...
            _currencyToken.safeTransfer(msg.sender, withdrawAmount);
        }
