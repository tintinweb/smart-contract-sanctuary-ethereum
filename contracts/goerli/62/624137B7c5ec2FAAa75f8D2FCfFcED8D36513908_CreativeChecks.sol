 ...
 */
contract CreativeChecks is ERC721A, Ownable, ReentrancyGuard {
	...
	string[] public badges = ["ffffff", "fdf401", "fe6401", "dc0302", "f00285", "4601a4", "0000d5", "00aee9", "1bb80c", "006506", "572800", "907135", "c1c1c1", "818080", "3f3e3f", "000000"];
	...
	function renderSvg(uint256[80] memory data_) public view returns (string memory) {
		...
		for (uint i; i < 10; i++) {
			for (uint j; j < 8; j++) {
				svgString = string(abi.encodePacked(svgString, string(abi.encodePacked('<use xlink:href="#badge" fill="#', badges[data_[(i * 8) + j]], '" x="', Strings.toString(398 + j * 40), '" y="', Strings.toString(354 + i * 40), '" /><use xlink:href="#tick" fill="#', ticks[data_[(i * 8) + j]], '" x="', Strings.toString(398 + j * 40), '" y="', Strings.toString(354 + i * 40), '" />'))));
			}
		}
	...
	function getUintArrayFromData(bytes memory data_) internal pure returns (uint256[80] memory returnData) {
		for (uint i=0; i < data_.length * 2; i++) {
			...
			assembly {
				// load bytes32 from bytes
				let temp := mload(add(data_, add(index, 1)))

				// Get 4 bits from the shifted position
