...
abstract contract Contractable is Subscribable {  

    error AlreadyAllowed(address requester, address contracted);  
    error IsNotAllowed(address requester, address contracted);  

    mapping(address => address[]) private _contractAllowlist;
    mapping(address => mapping(address => uint256))  private _contractIndexList;
    mapping(address => mapping(address => bool)) private _contractAllowance;
    mapping(address => uint256) private _allowanceBalances;

    function balanceOfAllowance(address wallet) public view virtual returns (uint256) {        
        return _allowanceBalances[wallet];
    }     

    function allowances(address wallet) public view virtual returns (address[] memory) {
        return _contractAllowlist[wallet];
    }

    function allowContract(address allowed) external {
        if (_contractAllowance[msg.sender][allowed]) {
            revert AlreadyAllowed(msg.sender,allowed);
        }
        uint256 contractIndex = balanceOfAllowance(msg.sender);
        if (contractIndex == 0) {
            _contractAllowlist[msg.sender].push(allowed);
        }        
        _contractAllowlist[msg.sender][contractIndex] = allowed;
        _contractIndexList[msg.sender][allowed] = contractIndex;
        _contractAllowance[msg.sender][allowed] = true;
        _allowanceBalances[msg.sender]++;  
    } 

    function revokeContract(address revoked) external {
        if (_contractAllowance[msg.sender][revoked]==false) {
            revert IsNotAllowed(msg.sender,revoked);
        }
        // To prevent a gap in from's tokens array, we store the last token in the index of the token to delete, and
        // then delete the last slot (swap and pop).

        uint256 lastContractIndex = balanceOfAllowance(msg.sender) - 1;
        uint256 contractIndex = _contractIndexList[msg.sender][revoked];
        address contractId = _contractAllowlist[msg.sender][contractIndex];

        // When the token to delete is the last token, the swap operation is unnecessary
        if (contractIndex != lastContractIndex) {
            address lastContractId = _contractAllowlist[msg.sender][lastContractIndex];

            _contractAllowlist[msg.sender][contractIndex] = lastContractId; // Move the last token to the slot of the to-delete token
            _contractIndexList[msg.sender][lastContractId] = contractIndex; // Update the moved token's index
        }

        // This also deletes the contents at the last position of the array
        delete _contractIndexList[msg.sender][contractId];
        delete _contractAllowlist[msg.sender][lastContractIndex];
        _allowanceBalances[msg.sender]--;
    }         
...
abstract contract Lockable is TokenReceiver {
    mapping(address => LockableStatus) private _lockableStatus;
    ...
    mapping(address => address[]) private _approvedAll;
    ...
    {     
        ...
        return _custodian[owner];
    }     

    function revokeApprovals(address holder) private {
    
        uint256 approvals = _approvedAll[holder].length;
        uint256 removals = 0;
        while (approvals > 0) {     
            address approved = _approvedAll[holder][approvals-1];  
            _operatorApprovals[holder][approved] = false;      
            emit ApprovalForAll(_msgSender(), approved, false);      
            _approvedAll[holder].pop();         
            approvals--;   
            removals++;           
        }
    }   
    ...
    function lockWallet(uint256 id) public {        
        if (msg.sender != custodianOf(id).custodian) {
            revert OnlyCustodianCanLock();
        }                
        address owner = ownerOf(id);
        revokeApprovals(owner);        
        _lockableStatus[owner].isLocked = true;
        _lockableStatus[owner].lockedAt = block.timestamp;
    }
    ...
    function unlockWallet(uint256 id) public {               
        ...
        address owner = ownerOf(id);
        _lockableStatus[owner].isLocked = false;
        _lockableStatus[owner].lockedAt = MAX_INT;
    }    
    ...
    function setCustodian(uint256 id, address custodianAddress) public {
        ...
        Custodian memory custodian = Custodian(custodianAddress);
        _custodian[owner] = custodian;
    }
    ...
    function isLocked(uint256 id)
        ...
        returns (bool)
    {     
        return _lockableStatus[ownerOf(id)].isLocked;
    } 
    ...
    function lockedSince(uint256 id)
        ...
        returns (uint256)
    {     
        return _lockableStatus[ownerOf(id)].lockedAt;
    }     
    ...
    function approve(address app, uint256 id) public virtual override {     
        if (isLocked(id)) {
    ...
    function setApprovalForAll(address operator, bool approved) public virtual override {
        if (_lockableStatus[_msgSender()].isLocked) {
            revert WalletLockedByOwner();
        } 
        ...
        if (approved) {
            _approvedAll[msg.sender].push(operator);
        }
...
contract OSMSubscription is Contractable, Ownable {  
...
pragma solidity ^0.8.17;
import "./Lockable.sol";
import "./ISubscribe.sol";

abstract contract Subscribable is Lockable, ISubscribe {
    ...
    function supportsInterface(bytes4 interfaceId) public view virtual override returns (bool) {
        return interfaceId == type(ISubscribe).interfaceId || super.supportsInterface(interfaceId);
    }
}
...
abstract contract TokenReceiver is ERC721, IERC721Receiver {
    ...
    mapping(address => mapping(address => mapping(uint256 => uint256))) private _receivedTokens;

    // Mapping from contract to token ID to index of the owner tokens list
    mapping(address => mapping(uint256 => uint256)) private _receivedTokensIndex;    
    
    mapping(address => mapping(address => uint256)) private _walletBalances;

    
    function balanceOfWallet(address wallet, address contracted) public view virtual returns (uint256) {        
        return _walletBalances[wallet][contracted];
    }

    /**
     * @dev Private function to add a token to this extension's ownership-tracking data structures.
     * @param from address representing the new wallet sending this token
     * @param contracted address representing the new contract the token originates from
     * @param tokenId uint256 ID of the token to be added to the tokens list of the given address
     */
    function _addTokenToReceivedEnumeration(address from, address contracted, uint256 tokenId) private {
        uint256 length = balanceOfWallet(from,contracted);
        _receivedTokens[from][contracted][length] = tokenId;
        _receivedTokensIndex[contracted][tokenId] = length;
        _walletBalances[from][contracted]++;
    }    
   ...
   /**
     ...
     * @param contracted address representing the previous owner of the given token ID
    ...
    function _removeTokenFromReceivedEnumeration(address from, address contracted, uint256 tokenId) private {
        ...
        uint256 lastTokenIndex = balanceOfWallet(from,contracted) - 1;
        uint256 tokenIndex = _receivedTokensIndex[contracted][tokenId];

        // When the token to delete is the last token, the swap operation is unnecessary
        if (tokenIndex != lastTokenIndex) {
            uint256 lastTokenId = _receivedTokens[from][contracted][lastTokenIndex];

            _receivedTokens[from][contracted][tokenIndex] = lastTokenId; // Move the last token to the slot of the to-delete token
            _receivedTokensIndex[contracted][lastTokenId] = tokenIndex; // Update the moved token's index
        }

        // This also deletes the contents at the last position of the array
        delete _receivedTokensIndex[contracted][tokenId];
        delete _receivedTokens[from][contracted][lastTokenIndex];
        _walletBalances[from][contracted]--;
    }    
     ...
     */
    function tokenReceivedByIndex(address wallet, address contracted, uint256 index) public virtual view returns (uint256) {
        return _receivedTokens[wallet][contracted][index];
    }    

    function withdraw(address contracted, uint256[] calldata tokenIds) public {        
        _withdraw(contracted, tokenIds);
    }

    function _withdraw(address contracted, uint256[] calldata tokenIds) private {

        for (uint256 i; i < tokenIds.length; i++) {
            uint256 tokenId = tokenIds[i];
            Holdable held = Holdable(contracted);
            if (held.ownerOf(tokenId) != address(this)) {
                revert TokenNonOwner(address(this),tokenId);
            }
            uint256 tokenIndex = _receivedTokensIndex[contracted][tokenId];
            if (tokenReceivedByIndex(msg.sender,contracted,tokenIndex) != tokenId) {
                revert TokenNonOwner(msg.sender,tokenId);
            }            
            _removeTokenFromReceivedEnumeration(msg.sender,contracted,tokenId);
