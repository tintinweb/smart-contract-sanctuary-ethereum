...
contract Marketplace is ReentrancyGuard, Ownable, Pausable {
    ...
    function createMarketItem(
        ...
        if (auction == true) {
            expDate = block.timestamp + (time * 1 days);
        } else {
            expDate = 0;
        }
        ...
        whenNotPaused
    {
        ...
        _createMarketSale(nftContract, itemId, msg.sender);
    }

    function makeOffer(uint256 itemId) public payable whenNotPaused {
        offers[itemId].push(Offer(msg.sender, msg.value));
    }

    function acceptOffer(uint256 itemId, uint256 index) public whenNotPaused {
        require(
            msg.sender == idToMarketItem[itemId].seller,
            "MUST BE SELLER TO ACCEPT OFFER"
        );
        _createMarketSale(
            idToMarketItem[itemId].nftContract,
            itemId,
            offers[itemId][index].user
        );
        idToMarketItem[itemId].seller.transfer(offers[itemId][index].amount);
        delete offers[itemId][index];
    }

    function deleteOffer(uint256 itemId, uint256 index) public whenNotPaused {
        require(
            msg.sender == idToMarketItem[itemId].seller ||
                msg.sender == offers[itemId][index].user,
            "MUST BE SELLER OR OFFERER"
        );
        delete offers[itemId][index];
    }

    function placeBid(uint256 itemId) public payable whenNotPaused {
        require(
            idToMarketItem[itemId].auction == true,
            "ITEM MUST BE AN AUCTION"
        );
        require(
            msg.value > idToMarketItem[itemId].price,
        ...
        idToMarketItem[itemId].highBidder = msg.sender;
    }
