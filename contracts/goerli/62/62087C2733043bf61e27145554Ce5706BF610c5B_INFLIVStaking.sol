...
contract INFLIVStaking is Initializable, OwnableUpgradeable{
	...
	function deposit(uint256 amount, uint256 plan) external {
		...
		require(mapUserInfo[msg.sender][plan].amount + amount <= maxStakingToken[plan], "amount is more than max staking amount");
		
		if(mapUserInfo[msg.sender][plan].amount > 0) {
            uint256 pending = (mapUserInfo[msg.sender][plan].amount * accTokenPerShare) - mapUserInfo[msg.sender][plan].rewardDebt;
            if (pending > 0) 
			{
			   uint256 governanceTax = pending * governanceFee / 10000;
			   IERC20Upgradeable(stakedToken).approve(address(stakedToken), governanceTax);
			   IGovernance(stakedToken).addGovernanceFee(governanceTax);
		...
		mapUserInfo[msg.sender][plan].rewardDebt = (mapUserInfo[msg.sender][plan].amount * accTokenPerShare);
	...
	function withdraw(uint256 plan) external {
	    if(mapUserInfo[msg.sender][plan].amount > 0) 
		{
			...
			if(mapUserInfo[msg.sender][plan].startTime + stakingPeriod[plan] > block.timestamp) 
			{
				...
				IERC20Upgradeable(stakedToken).approve(address(stakedToken), penalty);
				IGovernance(stakedToken).addGovernanceFee(penalty);
	...
	function pendingReward(address user, uint256 plan) external view returns (uint256) {
		if(mapUserInfo[user][plan].amount > 0) 
		{
            uint256 pending = (mapUserInfo[user][plan].amount * accTokenPerShare) - mapUserInfo[user][plan].rewardDebt;
			return pending;
        } 
		else 
		{
	...
	function SetStakingPeriod(uint256 P1StakingPeriod, uint256 P2StakingPeriod, uint256 P3StakingPeriod, uint256 P4StakingPeriod, uint256 P5StakingPeriod) external onlyOwner {
	    require(P1StakingPeriod >= 0, "Incorrect `P1 Staking Period` value");
		require(P2StakingPeriod >= 0, "Incorrect `P2 Staking Period` value");
		require(P3StakingPeriod >= 0, "Incorrect `P3 Staking Period` value");
		require(P4StakingPeriod >= 0, "Incorrect `P4 Staking Period` value");
		require(P5StakingPeriod >= 0, "Incorrect `P5 Staking Period` value");
		
	    stakingPeriod[0] = P1StakingPeriod;
        stakingPeriod[1] = P2StakingPeriod;
        stakingPeriod[2] = P3StakingPeriod;
		stakingPeriod[3] = P4StakingPeriod;
		stakingPeriod[4] = P5StakingPeriod;
		
		emit NewStakingPeriod(P1StakingPeriod, P2StakingPeriod, P3StakingPeriod, P4StakingPeriod, P5StakingPeriod);
    }
}
