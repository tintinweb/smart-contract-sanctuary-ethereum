...
contract Betting is VerifySignature, UUPSUpgradeable, AccessControlUpgradeable {
    ...
    function createPool(uint256 numberOfTeams_, string memory eventName_, uint256 startTime_, uint256 duration_, address mint_, Team[] memory teams_) external onlyRole(ADMIN_ROLE) {
        uint256 poolId = pools.length;
        require(teams_.length == numberOfTeams_, "Betting: Mismatching teams and numberOfTeams");
        pools.push(Pool(poolId, numberOfTeams_, eventName_, 0, 0, PoolStatus.Created, 0, startTime_, startTime_ + duration_, mint_));
    ...
    function createTeam(uint256 poolId_, Team memory team_) external onlyRole(ADMIN_ROLE) validPool(poolId_) {
    ...
    function cancelPool(uint256 poolId_) external onlyRole(ADMIN_ROLE) validPool(poolId_) {
        Pool storage pool = pools[poolId_];
    ...
    function startPool(uint256 poolId_) external onlyRole(ADMIN_ROLE) validPool(poolId_) {
        Pool storage pool = pools[poolId_];
        require(pool.status == PoolStatus.Created, "Betting: Pool status should be Created");
    ...
    function gradePool(uint256 poolId_, uint256 winnerId_) external onlyRole(ADMIN_ROLE) validPool(poolId_) {
        Pool storage pool = pools[poolId_];
        require(pool.status == PoolStatus.Running, "Betting: Pool status should be Running");

        // Mark pool as closed
        pool.status = PoolStatus.Decided;
        pool.winnerId = winnerId_;

        emit PoolGraded(poolId_, winnerId_);
    }

    function refundBet(uint256 poolId_, uint256 teamId_) external onlyRole(ADMIN_ROLE) validPool(poolId_) {
    ...
    function placeBet(uint256 poolId_, uint256 teamId_, uint256 amount_) external validPool(poolId_) {
        ...
        Team memory team = poolTeams[poolId_][teamId_];
        ...
        // Mint team tokens
        IERC1155PresetMinterPauser(pool.mintContract).mint(player, teamId_, amount_, "") ;
    ...
    function claimPayment(uint256 poolId_) external validPool(poolId_) {
        ...
        uint256 _userBalance = IERC1155PresetMinterPauser(pool.mintContract).balanceOf(winner, pool.winnerId);
        ...
        require(_userBalance > 0, "Betting: No winning tokens"); 
        ...
        IERC1155PresetMinterPauser(pool.mintContract).burn(winner, pool.winnerId, _userBalance);
    ...
    function claimRefund(uint256 poolId_) external validPool(poolId_) {
        ...
        uint256[] memory balances = IERC1155PresetMinterPauser(pool.mintContract).balanceOfBatch(_player, _tokens);
        IERC1155PresetMinterPauser(pool.mintContract).burnBatch(player, _tokens, balances);
    ...
    function claimTeamRefund(uint256 poolId_, uint256 teamId_) external validPool(poolId_) {
        ...
        uint256 balance = IERC1155PresetMinterPauser(pool.mintContract).balanceOf(player, teamId_);
        IERC1155PresetMinterPauser(pool.mintContract).burn(player, teamId_, balance);
    ...
    function viewPoolDistribution(uint256 poolId_) external view returns (uint256[] memory _betAmounts){
        ...
        for (uint256 i = 0; i < pool.numberOfTeams; i++) {
            _betAmounts[i] = IERC1155PresetMinterPauser(pool.mintContract).totalSupply(i);
        }
    ...
    function totalPayoutsClaimable(address who_, uint256 poolId_) external view returns(uint256) {
        return _totalAmountWon(who_, poolId_) - claimedPayouts[who_][poolId_];
    }

    function totalPayoutsClaimed(address who_, uint256 poolId_) external view returns(uint256) {
        return claimedPayouts[who_][poolId_];
    }
    ...
    function totalRefundClaimable(address who_, uint256 poolId_) external view returns(uint256) {
        return _totalAmountRefunded(who_, poolId_) - claimedPayouts[who_][poolId_];
    }

    function totalRefundClaimed(address who_, uint256 poolId_) external view returns(uint256) {
        return claimedRefunds[who_][poolId_];
    }
    ...
    function _totalAmountWon(address who_, uint256 poolId_) internal view returns(uint256) {
        ...
        uint256 _teamBalance = IERC1155PresetMinterPauser(pool.mintContract).totalSupply(pool.winnerId);
        if (_teamBalance == 0) {
            return 0;
        }

        uint256 _userBalance = IERC1155PresetMinterPauser(pool.mintContract).balanceOf(who_, pool.winnerId);
        uint256 _winningAmount = (((pool.totalAmount - _teamBalance) * _userBalance)/ _teamBalance) + _userBalance;
    ...
    function _totalCommissionGenerated(address who_, uint256 poolId_) internal view returns(uint256) {
        ...
        // console.log("Commision: %s, Player: %s", _commissionAmount, who_);
    ...
    function _calculateOdds(uint256 poolId_, uint256 teamId_) internal view returns (uint256) {
        Pool memory pool = pools[poolId_];
        uint256 _teamBalance = IERC1155PresetMinterPauser(pool.mintContract).totalSupply(teamId_);
        if (_teamBalance == 0) {
            return 0;
        }
        ...
        // console.log(
        //     "Amount: %s, Balance: %s, Odds: %s",
        //     pool.totalAmount,
        //     _teamBalance,
        //     ((pool.totalAmount - _teamBalance) * SCALING_FACTOR)/ _teamBalance
        // );
