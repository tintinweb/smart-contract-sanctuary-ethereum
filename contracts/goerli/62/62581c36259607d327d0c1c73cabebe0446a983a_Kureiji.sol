...
contract Kureiji is ERC721Enum, Ownable, PaymentSplitter {
    ...
    function whitelistMint(uint256 _mintAmount, bytes32[] calldata _proof) external payable mintCheck(_mintAmount) {
        ...
        require(_verifyLeaf(_leaf(msg.sender), whitelist_merkleRoot, _proof), "Invalid proof");
    ...
    function publiclistMint(uint256 _mintAmount, bytes32[] calldata _proof) external payable mintCheck(_mintAmount) {
        ...
        require(_verifyLeaf(_leaf(msg.sender), publiclist_merkleRoot, _proof), "Invalid proof");

        //Since this is a "whitelisted" public mint, child contract spawn minting check not required
