...
contract TheFarmGamePlay is Ownable, ERC165Storage, ReentrancyGuard, Pausable {
   ...
   * @param eggAmt $EGG amount to sacrifice caller's NFT
   */

  function sacrifice(uint16 tokenId, uint256 eggAmt) external whenNotPaused nonReentrant {
    ...
    // Must check this, as getTokenTraits will be allowed since this contract is a controller
    require(lastTokenWrite < block.number, 'TheFarmGame: what u doing?');
    // IFarmAnimals.Traits memory nft = farmAnimalsNFT.getTokenTraits(tokenId);
    uint16 minted = farmAnimalsNFT.minted();
    ...
    if (kind == IFarmAnimals.Kind.HEN) {
      // TODO: 	Removed sacrifice egg cost. Better to move this into a struct mapping per `kind`
      // 				OR separate contract, to define whats exchanged for the sacrifices so they can be adjusted later in game.
      // Hen sacrifice requires 3x $EGG curve
      // require(eggAmt >= eggMintCost * 3, 'TheFarmGame: not enough egg provided');
      ...
      // Coyote sacrifice requires 4x $EGG curve
      require(eggAmt >= eggMintCost * 4, 'TheFarmGame: not enough egg provided');
      // TODO: This egg amount should go to Roosters
      eggToken.burn(_msgSender(), eggAmt);
      // TODO: The Coyote needs a gift
      // This will check if origin is the owner of the token
      farmAnimalsNFT.burn(tokenId);
      emit Sacrificed(_msgSender(), uint16(tokenId), 'COYOTE');
    }
  }
