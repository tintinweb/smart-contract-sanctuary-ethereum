...
import { ABI_MULTISIG_REVEAL_SALE, ABI_REVEAL_SALE, ABI_VOUCHER_CLOSED_SALE_MINT, ABI_CLOSED_SALE_MINT, ABI_RESERVE_TOKENS, ABI_MULTISIG_RESERVE_TOKENS, ABI_RESERVE_TOKENS_TO, ABI_MULTISIG_RESERVE_TOKENS_TO} from "./FunctionAbiV5.sol";
...
contract Erc721ClosedSaleV5 is Erc721SaleV5 {
  ...
  function reserveTokens(uint256 tokenCount_, MultiSigParams calldata params_) external onlyRole(CONTRACT_ADMIN_ROLE) whenNotPaused {
    _verifyMultisig(
      abi.encode(ABI_MULTISIG_RESERVE_TOKENS, tokenCount_, params_.nonce, ABI_RESERVE_TOKENS),
      params_,
      CONTRACT_ADMIN_ROLE
    );
    address[] memory to_ = new address[](tokenCount_);
    for(uint256 i = 1; i <= tokenCount_; i++) {
      to_[i-1] = msg.sender; 
    }
    _reserveTokensTo(to_);
  }

  function reserveTokensTo(address[] memory toAddresses_, MultiSigParams calldata params_) external onlyRole(CONTRACT_ADMIN_ROLE) whenNotPaused {
     _verifyMultisig(
      abi.encode(ABI_MULTISIG_RESERVE_TOKENS_TO, keccak256(abi.encodePacked(toAddresses_)), params_.nonce, ABI_RESERVE_TOKENS_TO),
      params_,
      CONTRACT_ADMIN_ROLE
    );
    _reserveTokensTo(toAddresses_);
  }

  function _reserveTokensTo(address[] memory toAddresses_) internal {
    require(totalSupply + toAddresses_.length <= maxSupply, "17");
    require(!msg.sender.isContract(), "16");
    require(toAddresses_.length > 0, "20");
   
    for (uint256 i = 1; i <= toAddresses_.length; i++) {
      _safeMint(toAddresses_[i-1], _tokenIdCounter.current());
      _tokenIdCounter.increment();
    }
  }

  /**
        @dev This function is used to mint tokens. It can be called by MINTER_ROLE.
        @param tokenCount_ The number of tokens to be minted to `to_`.
    */
  function mintTokensTo(uint256 tokenCount_, address to_, VoucherParams calldata params_) 
  external onlyRole(MINTER_ROLE) payable whenNotPaused {
    _mintTokens(tokenCount_, to_, params_);
  }
  ...
  function mintTokens(uint256 tokenCount_, VoucherParams calldata params_) external payable whenNotPaused {
    _mintTokens(tokenCount_, msg.sender, params_);
  }

  function _mintTokens(uint256 tokenCount_, address to_, VoucherParams calldata params_) internal {
    ...
    uint256 totalPrice = tokenCount_ * params_.pricePerToken * 1000000000;
    require(totalPrice == msg.value, "22");
    require(balanceOf(to_) + tokenCount_ <= maxTokensPerWallet, "19");
...
bytes32 constant ABI_MULTISIG_RESERVE_TOKENS = keccak256("MultiSig(uint256 tokenCount,uint256 nonce,string functionABI)");
bytes32 constant ABI_MULTISIG_RESERVE_TOKENS_TO = keccak256("MultiSig(address[] toAddresses,uint256 nonce,string functionABI)");
...
bytes32 constant ABI_RESERVE_TOKENS = keccak256(bytes("reserveTokens(uint256,MultiSigParams)"));
bytes32 constant ABI_RESERVE_TOKENS_TO = keccak256(bytes("reserveTokensTo(address[],MultiSigParams)"));
 ...
 */
abstract contract Erc721SaleV5 is OperatorFilterer, AccessControlV5, ERC721, ERC721Royalty, ERC721Pausable {
  ...
  ) AccessControlV5(roles_, roleAddresses_, multisigRoles_, multisigThresholds_) ERC721(name_, symbol_) OperatorFilterer(DEFAULT_SUBSCRIPTION, true) {
