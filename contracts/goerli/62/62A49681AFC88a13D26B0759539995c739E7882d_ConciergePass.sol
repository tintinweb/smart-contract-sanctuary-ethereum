...
// Crypto Boss Concierge NFTs are governed by the following terms & conditions: https://XXXXXXXXXXXXXXXX
    ...
    EIP712Base
{
    ...
    address payable constant CRYPTOBOSS = payable(address(0xee861213DB6Fd2c6cbA783c3A041C99e0038f667));
    address payable constant JELLY = payable(address(0x77a06Af353bA3d792c1f704601c3440160e741d1));

    uint8 maxPerTx = 1;
    uint8 maxTxPublic = 3;
    uint256 public currentContractStage = 1;

    uint256 public mintPriceAlpha = 200000000000000000;
    uint256 public mintPriceWhale = 400000000000000000;
    uint256 public onboardingFeeAlpha = 200000000000000000;
    uint256 public onboardingFeeWhale = 400000000000000000;
     ...
     */
    constructor() ERC721("Crypto Boss Concierge", "BOSS") {
        _initializeEIP712("Crypto Boss Concierge");
        merkleRootPresale = bytes32("XXXXXXXXXXXXXXXX");
        merkleRootPublicSale = bytes32("XXXXXXXXXXXXX");
        baseUri = "https://XXXXXXXXXXXXXXXXXX";

        maxTokenIdByStageAndTier[1]["ALPHA"] = 800; // 651 - 800 --> 150 presale alpha
        maxTokenIdByStageAndTier[1]["WHALE"] = 50; // 1 - 50 --> 50 presale whale
        maxTokenIdByStageAndTier[2]["ALPHA"] = 1000; // 801 - 1000 --> 200 WL public sale alpha, may change
        maxTokenIdByStageAndTier[2]["WHALE"] = 100; // 51 - 100 --> 50 WL public sale whale, may change
        maxTokenIdByStageAndTier[3]["ALPHA"] = 5500; // 1001 - 5500 --> 4500 alpha public sale
        maxTokenIdByStageAndTier[3]["WHALE"] = 350; // 101 - 350 --> 250 whale public sale

        tokenIdToMintByTier["WHALE"] = 1;
        tokenIdToMintByTier["ALPHA"] = 651;
    ...
    * @param _maxPerTx new max limit per tx
    ...
    */
    ...
    function _mintConciergePass(address payable _recipient, string calldata _tier) internal {
        ...
        (bool success, ) = CRYPTOBOSS.call{value: (msg.value * 975) / 1000}("");
     ...
     * @notice sends 97% of ether to former owner, 1% to originalowner (us), 2% to client
    ...
    function purchaseTokenMarketPlaceRecipient(
        ...
        (bool success, ) = CRYPTOBOSS.call{value: (msg.value * 2) / 100}("");
        require(success, 'transfer to creator failed');
        (success, ) = JELLY.call{value: (msg.value * 1) / 100}("");
        require(success, 'transfer to jelly failed');
        (success, ) = owner.call{value: (msg.value * 97) / 100}("");
    ...
    function bounceFreeriders(uint256 _tokenId) external onlyAdmin {
        ...
        uint256 mintPrice = (_tokenId > 650) ? mintPriceAlpha : mintPriceWhale;
    ...
    - events for subgraph
    - the two different tiers are simply a flag in the NFT-metadata struct; set at minting depending on the tokenId and picked up by the subgraph
        - on the website, will see no tokenId, just mint, FE will send as string the kind you want (ALPHA/WHALE), will then automatically 
        increment the counter and determine the tokenId of yours, then you see what tokenId you got
