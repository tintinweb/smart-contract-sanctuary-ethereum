...
contract Becha is ERC721A,ERC2981,Ownable {
    ...
    function deleteWL(address addr) public virtual onlyOwner {
        tokenCount = tokenCount - Allowlists[addr];
        delete (Allowlists[addr]);
    }

    function updateWL(address addr, uint256 maxMint) public virtual onlyOwner {
        tokenCount = tokenCount - Allowlists[addr];
        Allowlists[addr] = maxMint;
        tokenCount = tokenCount + maxMint;
    }
    ...
    function preMint(uint256 quantity) public payable {
         ...
         require((quantity + tokenCount) <= (maxSupply), "Sold out");
         require(maxPerWallet >= quantity, "AllowlistMint: 2 max per tx");

        _safeMint(msg.sender, quantity);
        Minted[msg.sender] += quantity;
        tokenCount += quantity;
        Allowlists[msg.sender] -= quantity;
    }

    function publicMint(uint256 quantity) public payable {
        require(!publicsale, "Public mint is not yet.");
        require((quantity + tokenCount) <= (maxSupply), "Sold out");
    ...
    function mintCheck(
        ...
        require(quantity > 0, "Mint amount cannot be zero");
        require(
