...
contract AppConf is IAppConf, Ownable {
    ...
    mapping(address => uint8) donateTokenMap;
    ...
    function setDonateAddr(address _donateAddr) public onlyOwner {
        donateAddr = _donateAddr;
    }

    function getDonateAddr() public view override returns(address) {
        return donateAddr;
    }

    function setRedeemNftToken(address _redeemNftToken) public onlyOwner {
        redeemNftToken = _redeemNftToken;
    }

    function getRedeemNftToken() public view override returns(address) {
        return redeemNftToken;
    }

    function setGrantNftGen(uint8 gen) public onlyOwner {
        grantNftGen = gen;
    }

    function getGrantNftGen() public view override returns(uint8) {
        return grantNftGen;
    }

    function setDonateToken(address donateToken, uint8 status) public onlyOwner {
        donateTokenMap[donateToken] = status;
    }

    function validDonateToken(address donateToken) public view override returns(bool) {
        return donateTokenMap[donateToken] == 1;
    }
