...
contract AktionT4 is ERC721A, Ownable {

    using Strings for uint;

    string public baseURI;
    uint256 public PRICE = 1 ether;
    uint256 public MAX_SUPPLY = 430;
    uint256 public saleStartTime = 1646737200;


    constructor(string memory baseURI_) ERC721A("Aktion T4", "AktionT4") {
        baseURI = baseURI_;
    }

    function publicMint(address to, uint256 mintAmount) public payable {

        require(mintAmount > 0, "Mint amount must be greater than 0");
        require( (totalSupply() + mintAmount) <= MAX_SUPPLY, "Sale has already ended");
