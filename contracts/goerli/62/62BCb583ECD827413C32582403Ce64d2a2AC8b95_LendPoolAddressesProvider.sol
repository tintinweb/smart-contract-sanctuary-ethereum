...
contract LendPoolAddressesProvider is Ownable, ILendPoolAddressesProvider {
  ...
  bytes32 private constant CONFETTI_ORACLE = "CONFETTI_ORACLE";
  ...
  bytes32 private constant CONFETTI_DATA_PROVIDER = "CONFETTI_DATA_PROVIDER";
  ...
  /**
   * @dev Returns the id of the Confetti market to which this contracts points to
  ...
  function setCNFTRegistry(address factory) external override onlyOwner {
  ...
  function getConfettiDataProvider() external view override returns (address) {
    return getAddress(CONFETTI_DATA_PROVIDER);
  }

  function setConfettiDataProvider(address provider) external override onlyOwner {
    _addresses[CONFETTI_DATA_PROVIDER] = provider;
    emit ConfettiDataProviderUpdated(provider);
  }
  ...
  function _updateImpl(bytes32 id, address newAddress) internal {
    ...
    if (proxyAddress == address(0)) {
      ...
      ConfettiUpgradeableProxy proxy = new ConfettiUpgradeableProxy(newAddress, address(this), params);

      _addresses[id] = address(proxy);
      emit ProxyCreated(id, address(proxy));
    } else {
      // upgrade implementation
      ConfettiUpgradeableProxy proxy = ConfettiUpgradeableProxy(proxyAddress);
