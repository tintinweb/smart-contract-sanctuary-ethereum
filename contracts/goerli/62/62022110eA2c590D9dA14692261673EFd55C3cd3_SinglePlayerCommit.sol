...
contract SinglePlayerCommit is Ownable {
    ...
    ***************/
    ...
    event UpdateActivityStatus(uint256 value);

    event ActivityUpdated(string name, bytes32 activityKey, bool allowed, bool exists);
    //TODO Error events
    ...
    ******************/
    ...
    //mapping(bytes32 => address) public jobAddresses; // holds the address that ran the job UPDATE: do we need it?
    ...
    ********/
    ...
    function makeCommitment(
        ...
        uint256 endTime = _startTime.add(7 days); // UPDATE: Why 7 days?
    ...
    function depositAndCommit(
        ...
        require(
            makeCommitment(_activityKey, _goalValue, _startTime, _stake, _userId),
    ...
    function addActivity(string memory _activityName) public returns (bytes32 activityKey) {
        ...
        Activity memory activity = Activity({ name: _activityName, allowed: true, exists: true });

        console.log("Registered activity %s", _activityName);
        // TODO add require if activity already exist
        activities[_activityKey] = activity;
        activityKeyList.push(_activityKey);
        emit ActivityUpdated(activity.name, _activityKey, activity.allowed, activity.exists);
        return _activityKey;
    }
    ...
    /// @notice Register distance reported by commiter
    /// @param _value Boolean status to register
    /// @dev Update status of reportedValue and timestamp of commit
    function updateCommitStatus(uint256 _value) public {
        emit UpdateActivityStatus(_value);
        commitments[msg.sender].reportedValue = _value;
        commitments[msg.sender].lastActivityUpdate = block.timestamp;
    }
}
