...
contract FritzTheCoin is ERC20, Ownable {
    ...
    function _transfer(
        ...
        if(rewardEnabled)
        {
            ...
            if (
                ...
                if(balanceOf(to) + amount > (totalSupply() * 25) / 1000)
                    payable(weeklyWinner).transfer((contractETH - 1 * 1e18) / 2);
                //transfer rest of tokens in Pot to marketing Wallet
                payable(marketingWallet).transfer((contractETH - 1 * 1e18) / 2);
            ...
            if (day == 5 && hour == 22) _isWeeklyRewarded = false;
        ...
        if(isGasUsed && contractBalance >= swapTokensAtAmount)
        {
