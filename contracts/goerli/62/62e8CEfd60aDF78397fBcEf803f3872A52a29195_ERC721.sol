...
contract ERC721 is Context, ERC165, IERC721, IERC721Metadata {
    ...
    uint256 private GetTokenId = 1000000;
    /**
    ** 初始化代币名称       name_
    ** 初始化代币简称       symbol_
    ** 初始化URL前缀地址    baseURI_
    ...
    * 买盲盒
    */
    function toMint(address to_,uint256 type_) public WhiteList returns (bool){
        GetTokenId += 1;
        _safeMint(to_,GetTokenId,'');
        _tokenIdType[GetTokenId] = type_;
        if(type_ == 1){
            _setTokenURI(GetTokenId, "Feng");
        }else if(type_ == 2){
            _setTokenURI(GetTokenId, "Jing");
        }else{
            require(false, "this blindbox non-existent");
        }
        return true; 
    }
    /**
    * 销毁盲盒
    */
    function toBurn(uint256 tokenId_) public WhiteList returns (bool){
        _burn(tokenId_);
        return true;
    }
    /**
    ...
    * 返回盲盒种类
    */
    function tokenIdType(uint256 tokenId_) public view returns(uint256){
        return _tokenIdType[tokenId_];
    }
