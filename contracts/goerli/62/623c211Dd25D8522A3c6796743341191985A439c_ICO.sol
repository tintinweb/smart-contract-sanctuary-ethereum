...
error ICOEnded();
error TokenAlreadyReleased();
error TokenNotReleased();
error OnlyOwner();
error OnlyInvestor();
error ICOAlreadyActive();
error ICONotActive();
error DurationLessThan0();
error AvailableTotalNotGreaterThan0andLessThanTotalSupply();
error MinPurchaseNotGreaterThan0();
error MaxPurchaseNotGreaterThan0andLessThanAvailableTokens();
error ValueNotMultiple();
error NotBetweenMinandMax();
...
contract ICO is ERC20 {
    ...
    modifier icoEnded() {
        if (end <= 0 && (block.timestamp < end || availableTokens == 0)) {
            revert ICOEnded();
        }
        _;
    }
    modifier tokenNotReleased() {
        if (released != false) {
            revert TokenAlreadyReleased();
        }
        _;
    }
    modifier tokenReleased() {
        if (released != true) {
            revert TokenNotReleased();
        }
        _;
    }
    modifier onlyAdmin() {
    ...
    function start(
        ...
        if (duration < 0) {
            revert DurationLessThan0();
        }
        uint256 totalSupply = IERC20(address(this)).totalSupply();
        if (_availableTokens <= 0 && _availableTokens > totalSupply) {
            revert AvailableTotalNotGreaterThan0andLessThanTotalSupply();
        }
        if (_minPurchase <= 0) {
            revert MinPurchaseNotGreaterThan0();
        }
        if (_maxPurchase <= 0 && _maxPurchase > availableTokens) {
            revert MaxPurchaseNotGreaterThan0andLessThanAvailableTokens();
        }
    ...
    function buy() external payable onlyInvestors icoActive {
        if (msg.value % price != 0) {
            revert ValueNotMultiple();
        }
        if (
            msg.value / price < minPurchase && msg.value / price > maxPurchase
        ) {
            revert NotBetweenMinandMax();
        }
    ...
    function release() external onlyAdmin icoEnded tokenNotReleased {
        IERC20 tokenInstance = IERC20(address(this));
