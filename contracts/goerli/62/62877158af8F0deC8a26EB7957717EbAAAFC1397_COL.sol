...
contract COL is ERC721A, Ownable {
    // Variables
    ...
    event burned_(uint256 tokenId, bytes32 indexed adr);
    ...
    constructor(
        ...
        address adr2,
        address adr3
    ) ERC721A("Sinners & Saints", "SAS", 1, 1000) {
        receiver1 = adr1;
        receiver2 = adr2;
        receiver3 = adr3;
    ...
    function mint() external payable {
        if (!activity) revert ActivityOff();
        if (_totalMinted() + 1 > MAX_SUPPLY) revert SummonExceedsMaxSupply();
        if (isMinted[msg.sender]) revert AlreadyMinted();
    ...
    function burn(uint256 tokenId, bytes32 adr) public {
        _burn(tokenId);
