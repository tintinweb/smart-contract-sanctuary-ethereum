...
contract ConnextModuleFactory is IConnextModuleFactory {
  ...
  address internal immutable _ADDRESS_THIS;

  address public freshlyDeployedConnextModule;
  ...
  function _createSafeAndModule(
    ...
    _connextModule = new ConnextModule{salt: _moduleSalt}(
            // NOTE Hardcoded values are replaced with the correct values
            // after creating the safe (owner, avatar, target, connext)
            _ADDRESS_THIS, // NOTE Hardcoded for ownership control (owner)
            _ADDRESS_THIS, // NOTE Hardcoded for ownership control (avatar)
            _ADDRESS_THIS, // NOTE Hardcoded for ownership control (target)
            _moduleData.originSender,
            _moduleData.origin,
            _ADDRESS_THIS // NOTE Hardcoded, factory is connext (connext)
        );
    // set correct target and avatar after deployment

    freshlyDeployedConnextModule = address(_connextModule);

    // GnosisSafeProxy as GnosisSafe
    _safe = GnosisSafe(
      payable(SAFE_FACTORY.createProxyWithNonce(_safeData.singleton, _safeData.initializer, _safeData.saltNonce))
    );
    ...
    // WARNING, here we nee to make sure the module allows the factory to execute, rather than the origin sender.
    // _connextModule.execTransactionFromFactoryOnAtomicCreation(
    //     _safeTransactionData
    // );
    ...
    // remove deployed module address
    delete freshlyDeployedConnextModule;
  }

  // only to be called from Safe
  function enableModuleFromFactory() external {
    GnosisSafe(payable(address(this))).enableModule(ConnextModuleFactory(_ADDRESS_THIS).freshlyDeployedConnextModule());
  }
}
...
interface IConnextModuleFactory is IXReceiver {
  ...
  // /**
  //  * @notice Creates a Safe and Connext module
  //  * @return _connextModule The Connext module created
  //  * @return _safe The Safe created
  //  * @return _returnData The return data from the module XReceive call
  //  */
  // function _createSafeAndModule(
  //   SafeData memory _safeData,
  //   ModuleData memory _moduleData,
