...
// File: contracts\interfaces\ICrossRouter01.sol
...
interface ICrossRouter01 {
    ...
    function getAmountOutWhitelisted(
        uint256 amountIn,
        uint256 reserveIn,
        uint256 reserveOut,
        uint256 fee
    ...
    function getAmountInWhitelisted(
        uint256 amountOut,
        uint256 reserveIn,
        uint256 reserveOut,
        uint256 fee
    ...
    function getAmountsOutW(
        uint256 amountIn,
        address[] calldata path,
        uint256 fee
    ) external view returns (uint256[] memory amounts);

    function getAmountsInW(
        uint256 amountOut,
        address[] calldata path,
        uint256 fee
    ) external view returns (uint256[] memory amounts);
...
// File: contracts\interfaces\ICrossFactory.sol
...
// File: contracts\interfaces\ICrossPair.sol
...
// File: contracts\libraries\CrossLibrary.sol
...
library CrossLibrary {
            ...
            uint256(
                keccak256(
                    abi.encodePacked(
                        ...
                        hex"613a519f2c1710ce6d906ace2b7c5217126596384ab06b984da662738de0c72a" // init code hash
                    )
                )
            )
        );
    ...
    function getReserves(
        ...
        (uint256 reserve0, uint256 reserve1, ) = ICrossPair(
            pairFor(factory, tokenA, tokenB)
        ).getReserves();
    ...
    function quote(
        ...
        require(
            ...
            "CrossLibrary: INSUFFICIENT_LIQUIDITY"
        );
    ...
    function getAmountInWhitelisted(
        ...
        uint256 userFee = fee == 10000 ? 10000 : 10000 - fee;
        uint256 numerator = reserveIn.mul(amountOut).mul(10000);
        uint256 denominator = reserveOut.sub(amountOut).mul(userFee);
        amountIn = (numerator / denominator).add(1);
    }
    ...
    function getAmountOutWhitelisted(
        ...
        uint256 userFee = fee == 10000 ? 10000 : uint256(10000) - fee;
        uint256 amountInWithFee = amountIn.mul(userFee);
        uint256 numerator = amountInWithFee.mul(reserveOut);
        uint256 denominator = reserveIn.mul(10000).add(amountInWithFee);
        amountOut = numerator / denominator;
    }
    ...
    function getAmountsOutW(
        ...
        for (uint256 i; i < path.length - 1; i++) {
            ...
            amounts[i + 1] = getAmountOutWhitelisted(
                amounts[i],
                reserveIn,
                reserveOut,
                fee
    ...
    function getAmountsInW(
        ...
        for (uint256 i = path.length - 1; i > 0; i--) {
            ...
            amounts[i - 1] = getAmountInWhitelisted(
                amounts[i],
                reserveIn,
                reserveOut,
                fee
...
// File: contracts\CrossRouter.sol

pragma solidity =0.6.6;

contract CrossRouter is ICrossRouter02 {
    using SafeMath for uint256;
    ...
    function _addLiquidity(
        ...
        (uint256 reserveA, uint256 reserveB) = CrossLibrary.getReserves(
            factory,
            tokenA,
            tokenB
        );
        if (reserveA == 0 && reserveB == 0) {
            ...
            uint256 amountBOptimal = CrossLibrary.quote(
                amountADesired,
                reserveA,
                reserveB
            );
            if (amountBOptimal <= amountBDesired) {
                require(
                    ...
                    "CrossRouter: INSUFFICIENT_B_AMOUNT"
                );
                (amountA, amountB) = (amountADesired, amountBOptimal);
            } else {
                uint256 amountAOptimal = CrossLibrary.quote(
                    amountBDesired,
                    reserveB,
                    reserveA
                );
                ...
                require(
                    ...
                    "CrossRouter: INSUFFICIENT_A_AMOUNT"
                );
    ...
    function _swap(
        ...
        for (uint256 i; i < path.length - 1; i++) {
            ...
            address to = i < path.length - 2
                ? CrossLibrary.pairFor(factory, output, path[i + 2])
                : _to;
            ICrossPair(CrossLibrary.pairFor(factory, input, output)).swap(
                amount0Out,
                amount1Out,
                to,
                new bytes(0)
    ...
    /* function _safeSwap(
        ...
        for (uint256 i; i < path.length - 1; i++) {
            ...
            address to = i < path.length - 2
                ? CrossLibrary.pairFor(factory, output, path[i + 2])
                : _to;
            ICrossPair(CrossLibrary.pairFor(factory, input, output)).safeSwap(
                amount0Out,
                amount1Out,
                to,
                new bytes(0)
        ...
        returns (uint256[] memory amounts)
    {
        ...
        if (userFee == 0) {
            ...
            if (userFee == 0) {
                amounts = CrossLibrary.getAmountsOutW(
                    factory,
                    amountIn,
                    path,
                    userFee
        ...
        returns (uint256[] memory amounts)
    {
        ...
        if (userFee == 0) {
            ...
            require(
                ...
                "CrossRouter: EXCESSIVE_INPUT_AMOUNT"
            );
            TransferHelper.safeTransferFrom(
                path[0],
                msg.sender,
            ...
            amounts = CrossLibrary.getAmountsInW(
                factory,
                amountOut,
                path,
                userFee
    ...
    function _swapSupportingFeeOnTransferTokens(
        ...
        for (uint256 i; i < path.length - 1; i++) {
            (address input, address output) = (path[i], path[i + 1]);
            (address token0, ) = CrossLibrary.sortTokens(input, output);
            ICrossPair pair = ICrossPair(
                CrossLibrary.pairFor(factory, input, output)
            );
            uint256 amountInput;
            uint256 amountOutput;
            {
                ...
                if (userFee == 0) {
                    amountOutput = CrossLibrary.getAmountOut(
                        amountInput,
                        reserveInput,
                        reserveOutput
                    );
                } else {
                    amountOutput = CrossLibrary.getAmountOutWhitelisted(
                        amountInput,
                        reserveInput,
                        reserveOutput,
                        userFee
    ...
    function getAmountOutWhitelisted(
        ...
        return
            CrossLibrary.getAmountOutWhitelisted(
                amountIn,
                reserveIn,
                reserveOut,
                fee
    ...
    function getAmountInWhitelisted(
        ...
        return
            CrossLibrary.getAmountInWhitelisted(
                amountOut,
                reserveIn,
                reserveOut,
                fee
    ...
    function getAmountsOutW(
        ...
        return CrossLibrary.getAmountsOutW(factory, amountIn, path, fee);
    }
    ...
    function getAmountsInW(
        ...
        return CrossLibrary.getAmountsInW(factory, amountOut, path, fee);
    }
}