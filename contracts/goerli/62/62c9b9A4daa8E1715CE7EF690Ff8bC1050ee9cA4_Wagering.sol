...
struct Match {
  ...
  // Indicates if the match is valid (non-null)
  bool isValid;
...
contract Wagering is 
  ...
  // Fixed commission percentage rate
  uint16 private constant commissionRate = 10;
  ...
  // Storage gap to properly upgrade contract
  uint256[48] __gap;
  ...
  // Receive (Native token funds needed for refund&payout gas fees)
  ...
  modifier onlyOwnerOrReferee() {
    ...
    require(msg.sender == o || referees[msg.sender] == true, errorMsg);
    _;
  }
  ...
  function betOnMatch(
    ...
    IERC20Upgradeable(mt.paymentToken).transferFrom(payee, address(this), amount);

    // Store new bet
    betIdToBet[betId] = Bet(betId, matchId, payee, amount, block.timestamp, side, false, true);
    matchIdToBetIdSet[matchId].push(betId);
    userToBetIdSet[payee].push(betId);
    matchIdToBetAmount[matchId] += amount;

    // Emit bet event
    emit BetPut(payee, matchId, betId);
  }
  ...
  function createMatch(address[] memory _participants, address _paymentToken) external returns (uint256 matchId) {
    require(_participants.length > 0, "There should be at least one participant");
    ...
    // Store new match
    matchIdToMatch[matchId] = Match(matchId, _participants, address(0), _paymentToken, true, false);

    // Emit match creation event
    emit MatchCreated(_participants[0], matchId, _participants, _paymentToken);
  }

  function createPvPMatchAndBet(uint256 _amount, address _paymentToken) 
  external returns (uint256 matchId, uint256 betId) {
    address[] memory participants = new address[](2);
    participants[0] = msg.sender;
    matchId = this.createMatch(participants, _paymentToken);
    betId = this.betOnMatch(msg.sender, matchId, _amount, msg.sender);
  }

  function joinPvPMatchAndBet(uint256 matchId, uint256 _amount) 
  external returns (uint256 betId) {
    Match memory mt = getMatch(matchId);
    require(mt.isValid, "Invalid match ID");
    
    uint256 participantCount = mt.participants.length;
    require(mt.participants[participantCount-1] == address(0), "Someone else has already joined the match");

    if (!isUserParticipant(mt, msg.sender)) {
      mt.participants[participantCount] = msg.sender;
    }

    betId = this.betOnMatch(msg.sender, matchId, _amount, msg.sender);
  }

  function submitMatchResult(uint256 matchId, address winner) 
  external onlyParticipant(matchId) returns (bool ended, bool needsReview) {
  ...
  function _endMatch(uint256 matchId, address winner) private {
    Match memory mt = getMatch(matchId);
    require(mt.isValid, "Invalid match ID");
    require(!mt.ended, "Match already ended");
  ...
  function performPayouts(uint256 matchId) private {
    Match memory mt = getMatch(matchId);
    require(mt.isValid, "Invalid match ID");
    require(mt.winner != address(0), "Match isn't ended yet");

    uint256 winningAmount = this.getBetAmountForParticipant(matchId, mt.winner);
    ...
    for (uint256 i = 0; i < bets.length; i++) {
      Bet memory bet = bets[i];
      if (!bet.isValid) continue;

      if (bet.side == mt.winner) {
        // Calculate the amount to be paid out
        // Old calculation
        // uint256 amount = totalBetAmount * (bet.amount / winningAmount);
        uint256 amount = totalBetAmount * (bet.amount / winningAmount) * ((100 - commissionRate) / 100);

        // Transfer payout amount to the bettor
        IERC20Upgradeable(mt.paymentToken).transferFrom(address(this), bet.bettor, amount);
  ...
  function refundBets(uint256 matchId) external onlyOwnerOrReferee {
    Match memory mt = getMatch(matchId);
    require(mt.isValid, "Invalid match ID");
    require(!mt.ended, "Match already ended");
    ...
    for (uint256 i = 0; i < bets.length; i++) {
      ...
      if (!bet.isValid) continue;
      // Transfer payout amount to the bettor
      IERC20Upgradeable(mt.paymentToken).transferFrom(address(this), bet.bettor, bet.amount);
      bet.isValid = false;
    }
  ...
  function refundBet(uint256 betId) external onlyOwnerOrReferee {
    Bet memory bet = betIdToBet[betId];
    require(bet.isValid, "Invalid bet ID");

    Match memory mt = getMatch(bet.matchId);
    require(mt.isValid, "Invalid match ID");
    require(!mt.ended, "Match already ended");
