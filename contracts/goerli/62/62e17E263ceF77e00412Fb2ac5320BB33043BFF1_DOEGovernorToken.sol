...
interface IDOERewards {
    function stakeFor(address forWhom, uint128 amount) external payable;

    function withdraw(address account, uint128 amount) external;
...
contract DOEGovernorToken is ERC20, ERC20Permit, ERC20Votes, ERC20Wrapper {
    IDOERewards public DOERewardsContract;
    IERC20 public wrappedToken;

    constructor(IERC20 _wrappedToken, IDOERewards _DOERewardsAddress)
        ERC20("DOEGovernorToken", "gDOE")
        ERC20Permit("DOEGovernorToken")
        ERC20Wrapper(_wrappedToken)
    {
        DOERewardsContract = _DOERewardsAddress;
    ...
    function stake(address account, uint256 amount) public {
        ...
        // deposit DOE for gDOE
        super.depositFor(account, amount);

        // approve staking contract to spend DOE
        wrappedToken.approve(address(DOERewardsContract), amount);

        // stake DOE for sDOE
        DOERewardsContract.stakeFor(account, uint128(amount));
    }
    ...
    function unstake(address account, uint256 amount) public {
        ...
        // unstake
        DOERewardsContract.withdraw(account, uint128(amount));
