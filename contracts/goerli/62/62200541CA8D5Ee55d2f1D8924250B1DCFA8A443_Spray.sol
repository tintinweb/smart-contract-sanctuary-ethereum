...
What is worth more, kanker or life?          
*/

// Contract by @kankertje
// Thanks to Beeble and Devotion for the inspiration for some of this contract

contract Spray is ERC721A, Ownable {

  // Tracks the sprayId to the URI
  mapping(uint => string) public spraymation;
  ...
  event Setter(uint indexed sprayId, address indexed usingContractNFT, uint indexed tokenId);

  // Constructor

  constructor() ERC721A("Kankerdekanker", "KANKER") {
    _mint(msg.sender, 1);
  }
  ...
  function spray(uint sprayId, address usingContractNFT, uint usingTokenId) external {
    // Security is everywhere, no spraying...yet.
    if (sprayEnabled == false) revert MuseumSecurity();
