...
contract Bet is Ownable, ReentrancyGuard {
    ...
    function placeBet(uint256 _pick) payable external nonReentrant {
        ...
        total += msg.value;
        if (_pick == 0) {
            picks[0].totalBetAmount += msg.value;
            pickOneBetAmounts[msg.sender] += msg.value;
            if (pickOneBetAmounts[msg.sender] == 0) { // handle new better address
                addBetterAddress(payable(msg.sender));
            }
        } else {
            picks[1].totalBetAmount += msg.value; 
            pickTwoBetAmounts[msg.sender] += msg.value;
