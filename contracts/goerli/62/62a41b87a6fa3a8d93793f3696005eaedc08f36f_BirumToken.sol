...
contract BTCDividend is IDividendDistributor {     //WBTC
    ...
    IERC20 RewardToken = IERC20(0x6B9Ae4D64feBB297C5BF12b4667616b751d05b77);  // SHIBA
...
contract ETHDividend is IDividendDistributor {     //WBTC
    ...
    IERC20 RewardToken = IERC20(0xf064c0A72A23e0F9C8bEB186431ec47a191C16Bc);  // FLOKI
...
contract BirumToken is Context, IERC20, Ownable {

    using SafeMath for uint256;

    string private _name = "Birum";
    ...
    constructor() {
        ...
        totalBuy = buyMarketingFee.add(buyWBTCFee).add(buyWETHFee);
        totalSell = sellMarketingFee.add(sellWBTCFee).add(sellWETHFee);
    ...
    function swapBack() internal swapping {
        ...
        uint _btcshares = buyWBTCFee.add(sellWBTCFee);
    ...
    function getBTCRewardInfo(address _receiver) external view returns (uint) {
    ...
    function BtcRescueToken(address tokenAddress,address _receiver, uint256 tokens) external onlyRewardController {
        btcdividend.rescueToken(tokenAddress,_receiver,tokens);
    }

    function BtcRescueFunds(address _receiver) external onlyRewardController {
