...
contract KitsuMintuContract is ERC721Enumerable, Ownable {
  using Strings for uint256;
  ...
  event ReferralFunds(uint256 amount, uint256 token, address fromOwner, address to);
  ...
  function getTokenSubParent(uint256 tokenId) public view returns (address) {
    ...
    receive() external payable {
        ...
        address seller = ownerOf(lastTransferId);

        emit ReferralFunds(msg.value, lastTransferId, seller, referralOwner);
        referralOwner.transfer(msg.value);
    }
  ...
  function mint(string memory mint_code) public {
    ...
    for (uint256 i = 1; i <= maxMintAmount; i++) {
        parentOwners[supply] = codesToOwners[mint_code];
        subParentOwners[supply] = codesToOwners[subParentCode];
