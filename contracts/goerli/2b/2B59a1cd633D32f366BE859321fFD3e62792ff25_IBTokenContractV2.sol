...
contract IBTokenContractV2 is Initializable, ERC777Upgradeable, UUPSUpgradeable, OwnableUpgradeable {
    event OrderFinalized(address, uint, address);
    function initialize() initializer public {
        __ERC777_init("IBToken", "IBT",new address[](0));
        __Ownable_init();
    ...
    function sendIBT(address recipient, address orderAddress, uint orderIndex, uint256 amount, bytes memory data
    ) public {
        require(this.balanceOf(msg.sender) >= amount, "You don't have  enough IBToken in your wallet!!!");
        require(recipient != address(0), "Zero address is not accepted");
        require(orderAddress != address(0), "Zero address is not accepted");
        super.send(recipient, amount * 10 ** 18, data);
        
        emit OrderFinalized(recipient, orderIndex, orderAddress);
    }
