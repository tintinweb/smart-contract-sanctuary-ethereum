...
contract GoerliDummyExchange {
  ...
  constructor(
    ...
    slippage = _slippage;
    DAI_ADDRESS = _dai;
    WHITELISTED_CALLERS[msg.sender] = true;
  ...
  function addWhitelistedCaller(address caller) external onlyAuthorized {
    changeWhitelistedStatus(caller, true);
  }

  function removeWhitelistedCaller(address caller) external onlyAuthorized {
    changeWhitelistedStatus(caller, false);
  }

  function changeWhitelistedStatus(address caller, bool newStatus) internal {
    WHITELISTED_CALLERS[caller] = newStatus;
  }

  function setSlippage(uint8 _slippage) external onlyAuthorized {
    slippage = _slippage;
  }
  ...
  function transferOut(address asset, uint256 amount) public onlyAuthorized {
