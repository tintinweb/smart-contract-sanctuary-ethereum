...
contract AGORACLES is ERC721A, ERC2981, PaymentSplitter, Ownable {
  ...
  uint16 public collectionSize = 6300;
  ...
  uint256 public presale1Price = 0.10 ether;
  uint256 public presale2Price = 0.20 ether;
  ...
  event ProxyAddressUpdated(address _proxyRegistryAddress);
  ...
  function setProxyRegistry(address _proxyRegistryAddress) public onlyOwner {
    ...
    emit ProxyAddressUpdated(_proxyRegistryAddress);
  }
  ...
  function mintPresale1(uint256 amount, bytes32[] memory _proof) external payable {
    mintPresale1To(msg.sender, amount, _proof);
  }

  function mintPresale1To(address to, uint256 amount, bytes32[] memory _proof) public payable {
