...
contract Betting is Storage, UUPSUpgradeable, AccessControlUpgradeable {
    ...
    modifier validPool(uint256 poolId_) {
        require(poolId_ >= 0 && poolId_ < _getTotalPools(), "Betting: Id is not valid");
        _;
    }
    ...
    function _getTotalPools() internal view returns(uint256) {
        return bettingAdmin.getTotalPools();
    }

    function _getPool(uint256 poolId_) internal view returns(Pool memory) {
        return bettingAdmin.getPool(poolId_);
    }

    function _getPoolTeam(uint256 poolId_, uint256 teamId_) internal view returns(Team memory) {
        return bettingAdmin.getPoolTeam(poolId_, teamId_);
    }

    function _getUsdcContract() internal view returns(IERC20Upgradeable) {
    ...
    function placeBet(uint256 poolId_, uint256 teamId_, uint256 amount_) external validPool(poolId_) {
        Pool memory pool = _getPool(poolId_);
        ...
        Team memory team = _getPoolTeam(poolId_, teamId_);
        ...
        _getUsdcContract().transferFrom(player, address(this), _netAmount);
    ...
    function claimPayment(uint256 poolId_) external validPool(poolId_) {
        ...
        pool.mintContract.safeBatchTransferFrom(winner, _getSigner(), pool.winners, balances, "");        
        _getUsdcContract().transfer(winner, _winningAmount);
    ...
    function claimRefund(uint256 poolId_) external validPool(poolId_) {
        ...
        _getUsdcContract().transfer(player, _refundAmount);
    ...
    function claimTeamRefund(uint256 poolId_, uint256 teamId_) external validPool(poolId_) {
        ...
        _getUsdcContract().transfer(player, balance);
    ...
    function claimCommission(uint256 poolId_) external validPool(poolId_) {
        ...
        _getUsdcContract().transfer(player, _commissionAmount);
    ...
    function claimCommissionWithSignature(uint256 poolId_, uint256 amount_, uint256 signedBlockNum_, bytes memory signature_) external validPool(poolId_) {
        ...
        _getUsdcContract().transfer(player, amount_);
    ...
    function _verifySignature(
        ...
        require(
            recoverSigner(signedHash, signature_) == _getSigner(),
            "Invalid signature"
        );
    }
