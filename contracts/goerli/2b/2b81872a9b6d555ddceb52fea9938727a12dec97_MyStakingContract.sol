...
contract MyStakingContract is FxBaseRootTunnel, Ownable {
    ...
    function stake(uint256 _tokenId) public {
        ...
        userInfo[msg.sender].timestamps.push(block.timestamp);

        //Send the information to the MATIC network (user wallet address, time stamps, action)
        _sendMessageToChild(abi.encode(msg.sender, userInfo[msg.sender].timestamps, true));
    }
    ...
    function unstake(uint256 _tokenId) public {
        ...
        //Send the information to the MATIC network (user wallet address, time stamps, action)
        _sendMessageToChild(abi.encode(msg.sender, userInfo[msg.sender].timestamps, false));
    }
