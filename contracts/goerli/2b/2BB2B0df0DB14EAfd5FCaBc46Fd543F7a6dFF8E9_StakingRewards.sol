...
contract StakingRewards is IStakingRewards, RewardsDistributionRecipient, ReentrancyGuard, Pausable {
    ...
    function notifyRewardAmount(
        ...
        if (block.timestamp >= t.periodFinish) {
            ...
            // audit issue SRB-13 POSSIBLE OVERFLOW/UNDERFLOW response:
            // when notifyRewardAmount() is run for first time or after period is finish we will enter `if` statement
            // and we end up with:
            //      rewardRate = _reward / t.rewardsDuration;
            // when we ender else:
            //   - we using `remaining` value: this value is between [1 and t.rewardsDuration] otherwise we will not
            //     enter else block, with maximum `remaining = t.rewardsDuration` `leftover` is our `_reward` so we can
            //     not overflow here
            uint256 leftover = remaining * rewardRate;
            // we know `leftover` can not overflow,
            // if we choose `_reward` that will be big enough to overflow on `(_reward + leftover)`,
            // then we end up with `(_reward + leftover) % max(uint256)` this will be always lower than original
            // `(_reward + leftover)`, so in this case and invalid `_reward` we can only end up with
            // lower reward rate. However later on we will revert with "rewards overflow", because if we overflow
            // here, we will be over `maxEverTotalRewards` because this is sum of all `_reward`s.
