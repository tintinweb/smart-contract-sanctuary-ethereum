...
contract Traffic is ERC721 {
    ...
    function batchCreateERC20(
        uint256 ownershipId,
        bytes32[] calldata forwarderSalt,
        address token,
        uint256 value,
        address dest
    ) external {
        ...
        for (uint256 i = 0; i < forwarderSalt.length; i++) {
            address forwarderAddress = deployForwarder(ownershipId, forwarderSalt[i]);
            (address d, uint256 v) = rule.execERC20(forwarderAddress, token, value, dest);
            IForwarder forwarder = IForwarder(forwarderAddress);
            forwarder.flushTokens(token, v, d);
        }
