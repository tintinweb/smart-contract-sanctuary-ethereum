...
contract KintsugiRouter is IKintsugiRouter, FeeUtil {
        ...
        returns (uint256[] memory amounts)
    {
        ...
        amounts = KintsugiLibrary.getAmountsOut(factory, eth, path);
        require(
            amounts[amounts.length - 1] >= amountOutMin,
            "KintsugiRouter: INSUFFICIENT_OUTPUT_AMOUNT"
        );
    ...
    function pairFor(
        ...
        bytes32 _data = keccak256(
            abi.encodePacked(
                ...
                hex"7cacbdf35e0920ab515b10e0d488fa7cfcac7068fdb0411b7bab2b8c9f29f2ef" // init code hash
            )
        );
...
abstract contract FeeUtil is Ownable {
    ...
    function setFee(address _feeTo, uint256 _fee) external onlyOwner {
        require(_fee <= 5, "Kintsugi::FeeUtil: Fee exceeds 0.5%");
        feeTo = _feeTo;
        fee = _fee;
    }
}