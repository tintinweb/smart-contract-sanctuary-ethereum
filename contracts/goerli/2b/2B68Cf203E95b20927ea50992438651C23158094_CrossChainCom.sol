    ...
    MessageReceiverApp
{
    ...
    //mapping(uint64 => ExecutionStatus) public requestStatus;

    constructor(address _messageBusAddress) {
        setMessageBus(_messageBusAddress);
        iMessageBus = IMessageBus(_messageBusAddress);
    }

    function notifyDst(address _receiver, uint64 _dstChainId) external payable {
        //IERC721(address).safeTransferFrom(msg.sender, address(this), _tokenId);
        nonce += 1;
        bytes memory _message = abi.encode(
            Request({
                nonce: nonce,
                sender: msg.sender,
                message: "User's NFT is locked"
            })
        );
    ...
    function executeMessage(
        ...
        RequestReceipt memory receipt = abi.decode(
            (_message),
            (RequestReceipt)
        );
    ...
    function executeMessageWithTransfer(
        ...
        revert("CrossChainCom: executeMessageWithTransfer not available");
    }
    ...
    function calcFee(uint8 _status) external view returns (uint256) {
        ...
        return iMessageBus.calcFee(message);
    }
}
