...
contract Vault {
  ...
  function emitBookingsReady() public OnlyOperator {
    ...
    for (uint256 i = 0; i < usersBookings.length ; i += 1) {
      ...
      bookingsToCancelCount = bookingsToCancelCount.add(1);
    }

    for (uint256 i = 0; i < bookingsToCancelCount; i += 1) {
      cancelBooking(usersToBookingCancel[i]);
      usersToBookingCancel[i] = address(0);
    }
  }
}