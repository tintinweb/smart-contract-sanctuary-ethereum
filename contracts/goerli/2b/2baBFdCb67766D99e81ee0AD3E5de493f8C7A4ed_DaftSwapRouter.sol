...
contract DaftSwapPair is ReentrancyGuard, NoDelegateCall {
	...
	/// @dev Swaps tokenY for exact tokenXAmount
	...
	function swapTokenYForExactTokenX(uint256 tokenXAmount, uint256 maxTokenYAmount, address maker) external nonReentrant noDelegateCall onlyRouter {
		...
		// TODO: optimize while loop to not include any SLOAD ops.
		...
		require(tokenXAmountRemaining == 0 && tokenYAmountAccumulated <= maxTokenYAmount);

		//transfer tokenXAmount and refund tokenY if necessary
	...
	/// TODO: swapExactTokenYForTokenX

	// function swapExactTokenYForTokenX(uint256 tokenYAmount, uint256 minTokenXAmountOut, address maker) external nonReentrant noDelegateCall onlyRouter {
	// 	//TODO: executs limit orders with lowest price
	...
	// 	int24 currTick = lowestTick;
	// 	bool initialized = lowestTickInitialized;

	// 	uint256 tokenXAmountAccumulated;
	// 	uint256 tokenYAmountRemaining = tokenYAmount;
	// 	while (tokenYAmountRemaining > 0) {
	// 		(tokenXAmountAccumulated += TickMath.getOutputTokenXAmount(currTick, remainingTokenYAmount);)
	// 	}
	...
	function setProtocolFee(uint24 newProtocolFee) external onlyRouterOwner {
...
library Tick {
    ...
    /// @notice Executes a tick and returns if the tick is to be flipped and the required tokenYAmount for execution (partial or full)
    ...
    /// @return flipped Whether the tick was flipped from initialized to uninitialized
    function execute(
        mapping(int24 => Tick.Info) storage self,
        int24 tick,
        uint256 tickMode,
