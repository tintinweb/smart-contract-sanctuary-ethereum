 ...
 * This contract gets 1 random word when two players enter the match with opposing
 * choices of Heads or Tails as their fortune, and this random word determines the
 * winner and rewards them with the opponent's minimum fee.
...
/* Errors */
error Clover__InsufficientMinimumFee(uint256 minimumFee);
error Clover__InvalidChoice();
error Clover__WinnerTransferFailed();
error Clover__RemainderTransferFailed();
error Clover__InvalidStep();
error Clover__InvalidFee();
error Clover__InterestTransferFailed();
error Clover__WithdrawFailed(uint256 balance);
...
contract Clover is VRFConsumerBaseV2 {
    ...
    /**
     * @dev Gets players 1 and 2, and the flip result. The winner gets their minimum
     * @dev fee plus the other player's minimum fee, ie 2*minimumFee.
