...
contract MarketContract is ERC20, NFT, Ownable{
    ...
    function buyCard(address _from, address _to, uint256 _tokenId, uint256 _token) public {
        transferFrom(_from, _to, _token);
        transferFromNFT(_to, _from, _tokenId);
    }

    function addCard(address _account, uint256 _tokenId, string calldata _image, string calldata _name, uint256 _token) public {
        safeMintNFT(_account, _tokenId, _image, _name, _token);
    }
}
...
contract NFT is EventToken{
    ...
    function safeMintNFT(address _account, uint256 _tokenId, string calldata _image, string calldata _name, uint256 _token) public {
        require(_account != address(0), "ERC721: mint to the zero address");
        require(!exists(_tokenId), "ERC721: token already minted");
        Card memory card = Card(_tokenId, _image, _name, _token);
