...
contract Voting {
    ...
    function vote(
        ...
        emit SenderRequested(msg.sender);
        require(msg.sender == _sender, "not same sender");
    ...
    function hasVoted(uint256 _id) public view hasStarted returns (bool) {
        ...
        mapping(uint256 => Types.Vote) storage _votes = voters[msg.sender];

        Types.Vote memory _vote = _votes[_id];
        require(_vote.id == _id, "vote does not exist");

        return _vote.voted;
    }

    function getVote(uint256 _id, address _sender)
        public
        view
        hasStarted
