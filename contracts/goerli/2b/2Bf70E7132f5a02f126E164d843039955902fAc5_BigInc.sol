...
contract BigInc is ERC721A, Ownable, RevokableDefaultOperatorFilterer, ReentrancyGuard {
    ...
    constructor() ERC721A("BigInc", "BINC")  {}
    ...
    function upgrade(uint256 tokenId) public nonReentrant {
        ...
        require(id == 0,"Must be fron a general acceptance letter id");
        require(upgradeActive, "Mint not active");
    ...
    function mintAdmin(uint256 amount) external payable onlyOwner {
        require(_totalMinted() + amount <= MAX_SUPPLY, "Minted out");
        _mint(msg.sender, amount);
    }

    function setTokenData(uint256 tokenId, uint256 level, uint256 tokenType) public onlyOwner {
        tokenToId[tokenId] = tokenType;
        tokenToLevel[tokenId] = level;
