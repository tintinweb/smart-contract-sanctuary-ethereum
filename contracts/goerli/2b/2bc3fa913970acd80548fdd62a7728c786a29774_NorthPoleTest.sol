...
*/
/** 
As the Witch santa advances, a long staff thumps the ground, unleashing a gift for rudoplhs, santas and hodlers alike.

The Witchsanta ($WD) contract is constructed so that the success of the project is shared with the most dedicated.
The smart contract will be able to pick out transactions done from the previous hour to decide who gets to become the next santa and rudoplh. 
...
rudoplh     - Given to the Biggest Buyer accumulated
santa    - Given to the Biggest Buyer in one transaction

See more details on our website: www.witchsantatoken.com

Website: www.witchsantatoken.com
Telegram: https://t.me/thewitchsanta
*/
...
contract NorthPoleTest is IERC20, Auth {
    using SafeMath for uint256;

    string constant _name = "NorthPoleTest";
    string constant _symbol = "NP";
    ...
    uint256 public lastrudoplhChange = 0;
    ...
    uint256 public santaFee = 3; // Biggest buy sum
    uint256 public rudoplhFee = 1; // Biggest buy
    ...
    constructor() Auth(msg.sender) {
        ...
        rudoplh = msg.sender;
        Emerald = msg.sender;
        totalFee = marketingFee.add(rudoplhFee).add(emeraldFee).add(santaFee);
    ...
    function _resetrudoplh() internal {
        biggestBuy = 0;
        rudoplh = marketingWallet;
        lastrudoplhChange = block.timestamp;
    }
    ...
    function epochResetrudoplh() external view returns (uint256) {
        return lastrudoplhChange + resetPeriod;
    }
        ...
        )
    {
        ...
        uint256 amountETHrudoplh = receivedETH.mul(rudoplhFee).div(totalFee);
        uint256 amountETHEmerald = receivedETH.mul(emeraldFee).div(totalFee);
        return (amountETHsanta, amountETHrudoplh, amountETHEmerald);
    }
    ...
    function _checkTxLimit(
        ...
        if (block.timestamp - lastrudoplhChange > resetPeriod) {
            _resetrudoplh();
        }
        ...
        if (
            ...
            if (!hasSold[recipient]) {
                if (totalBuySumPerAddress[recipient] > biggestBuySum) {
                    ...
                    if (santa != recipient) {
                        santa = recipient;
                        emit Newsanta(santa, biggestBuySum);
                    }
                }
                if (usedEth > biggestBuy) {
                    biggestBuy = usedEth;
                    lastrudoplhChange = block.timestamp;
                    if (rudoplh != recipient) {
                        rudoplh = recipient;
                        emit Newrudoplh(rudoplh, biggestBuy);
                    }
                }
        ...
        if (
            ...
            require(amount <= _maxTxAmount, "MAX TX");
            if (santa == sender) {
                emit santaSold(santa, biggestBuySum);
                _resetsanta();
                hasSold[sender] = true;
            }
            if (rudoplh == sender) {
                emit rudoplhSold(rudoplh, biggestBuy);
                _resetrudoplh();
                hasSold[sender] = true;
            }
            if (Emerald == sender) {
    ...
    function swapBack() internal lockTheSwap {
        ...
        uint256 amountETHrudoplh = amountETH.mul(rudoplhFee).div(totalFee);
        ...
        (bool tmpSuccess2, ) = payable(santa).call{
        ...
        (bool tmpSuccess3, ) = payable(rudoplh).call{
            value: amountETHrudoplh,
            gas: 30000
        }("");
        if (totalRewardsPerAddress[rudoplh] == 0) {
            rewardedAddresses.push(rudoplh);
        }
        totalRewardsPerAddress[rudoplh] += amountETHrudoplh;
        emit rudoplhPayout(rudoplh, amountETHrudoplh);
