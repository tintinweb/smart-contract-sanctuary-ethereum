...
contract UUMining is Ownable {
    ...
    constructor(address _minerNftAddr, address _lootNftAddr, address _uuTokenAddr, address _usdtAddr) {
    ...
    function claimUU(uint _id) public onlyValidUsers {
        _internalClaimUU(_id, msg.sender);
    }
    ...
    function _internalClaimUU(uint _id, address _claimer) internal {
        // check if caller owns NFT
        IERC721 _thisNFT = IERC721(minerNftAddr);
        require (_claimer == _thisNFT.ownerOf(_id), "not NFT owner.");
        ...
        _transferUUToken(_claimer, available2ClaimUU);
    ...
    function splitNftInto2(uint _id, uint _uuAmt) public onlyValidUsers {
        ...
        _transferUUToken(msg.sender, available2ClaimUU);
    ...
    function obtainTreasures() public onlyValidUsers {
        UserDetail storage u = allUserDetails[msg.sender];
    ...
    function openTreasures(uint256 _id) public onlyValidUsers returns (uint256 value) {
        ...
        if (case1 || case2) {
            if (_generateRandomNum(loot.odds) == 1) {
                _transferUUToken(msg.sender,  loot.prize); // give prize
                prize = loot.prize;
            }
    ...
    function _transferUUToken (address _target, uint256 _amt) internal {
        ...
        require (UUtoken.balanceOf(address(this)) >= _amt * 10 ** decimals, 'Insuffient UU in contract for transfer');

        UUtoken.transfer(_target, _amt * 10 ** decimals); // todo: check result if has correct decimals
    }
    ...
    function withdrawErc20Token (address _tokenAddr, uint256 _withdrawAmt) public onlyOwner {
        ...
        erc20token.transfer(address(owner()), _withdrawAmt * 10 ** decimals); // todo: check result if has correct decimals
    }
