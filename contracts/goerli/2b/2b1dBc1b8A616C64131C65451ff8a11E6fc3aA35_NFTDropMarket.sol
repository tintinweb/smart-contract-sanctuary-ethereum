...
abstract contract NFTDropMarketFixedPriceDrop is MarketFees {
  ...
  function mintFromFixedPriceSale(
    ...
    return _mintFromFixedPriceSaleAndApprove(nftContract, count, totalBuyPrice, address(0), buyReferrer);
  }

  /**
   * @notice Used to mint `count` number of NFTS from the collection
   * and setApprovalForAll to the address specified.
   * @param nftContract The address of the collection.
   * @param count The count of NFTs to mint.
   * @param totalBuyPrice The total price to pay for `count` number of NFTs.
   * @param buyReferrer The address for the buy referral.
  ...
  function mintFromFixedPriceSaleAndApprove(
    ...
    return _mintFromFixedPriceSaleAndApprove(nftContract, count, totalBuyPrice, market, buyReferrer);
  }
  ...
  function _mintFromFixedPriceSaleAndApprove(
    ...
    if (addressToApprove == address(0)) {
      firstTokenId = INFTDropCollectionMint(nftContract).mintCountTo(count, msg.sender);
    } else {
      firstTokenId = INFTDropCollectionMintAndApprove(nftContract).mintCountToAndApprove(
        count,
        msg.sender,
        addressToApprove
      );
...
/**
 * @notice An optional additional method to allow users to setApprovalForAll when minting via the NFTDropMarket.
 * @dev This interface is optionally registered as an ERC165 supported interface.
 */
interface INFTDropCollectionMintAndApprove {
  function mintCountToAndApprove(
    uint16 count,
    address to,
