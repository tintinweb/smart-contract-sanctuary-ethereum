...
contract RewardsEscrow is IModule, IRewardsEscrowV1, Initializable, AccessControlUpgradeable {
    ...
    function enforcePrerequisites(address registryAddr, bytes32 orgId) external view {
        ...
        require(!registry.hasModule(orgId, Modules.REWARDS_ESCROW_V1), "Prerequisites: module already exists");
    }

    function enforceSafeRemove(address registryAddr, bytes32 orgId) external view {
        IRegistry registry = IRegistry(registryAddr);

        require(!registry.hasModule(orgId, Modules.YIELDFARMING_V1_POOL), "SafeRemove: required by YIELDFARMING_V1_POOL");
