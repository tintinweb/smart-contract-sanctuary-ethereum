    ...
    ReentrancyGuardUpgradeable
{
    ...
    function _computeTarget(
        ...
        uint256 left = Random.numberChosen(0, 1, nonceTemp);
        uint256 sum = 0;
        uint256 percent = 80;
        for (uint i = 0; i < nbGhost; i++) {
            nonceTemp++;
            if (left <= 0) {
                sum += Random.numberChosen(0, reaperChosen, nonceTemp);
            } else {
                sum += Random.numberChosen(reaperChosen, 100, nonceTemp);
            }
        }
...
contract GameEngine is GameBase {
    ...
    function claimPrize(
        uint256 gameID
    ) public virtual override canClaim(gameID) {
