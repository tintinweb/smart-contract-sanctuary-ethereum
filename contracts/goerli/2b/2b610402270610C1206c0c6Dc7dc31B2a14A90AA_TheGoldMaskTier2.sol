...
contract TheGoldMaskTier2 is ERC1155, Ownable {
    ...
    uint256 public tokenCount = 3200; // To ensure the users that only 3000 NFTs will exist!
    ...
    function mint(uint256 quantity) external payable {
        ...
        uint256 n = 0;
        for (uint256 i = mintedTokensAll++ ; i <= mintedTokensAll + quantity; ++i){ // Check how much token is minted and get the next one for ID and push, amounts always need 1 for each ID
            ids[n] = i;
            amounts[n] = 1;
            ++n;
        }
