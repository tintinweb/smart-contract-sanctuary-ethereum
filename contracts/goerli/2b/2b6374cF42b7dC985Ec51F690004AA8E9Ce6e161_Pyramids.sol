...
interface IPyramids {
    event itemAddedToPyramid(uint16 indexed _pyramidId, uint16 indexed _itemId);

    event itemsAddedToPyramid(
        uint16 indexed _pyramidId,
        uint16[] indexed _itemIds
    );

    event itemRemovedFromPyramid(
        uint16 indexed _pyramidId,
        uint16 indexed _itemId
    );
...
import "@flair-sdk/contracts/src/token/ERC721/base/ERC721ABase.sol";
    ...
    ReentrancyGuardUpgradeable
{
    ...
    function addItemsToPyramid(
        ...
        require(
            this.ownerOf(_pyramidId) == msg.sender,
    ...
    function removeItemsFromPyramid(uint16 _pyramidId) public nonReentrant {
        ...
        for (uint8 i; i < pyramidSlots.length; ) {
            ...
            emit itemRemovedFromPyramid(_pyramidId, pyramidSlots[i].tokenId);
            ++i;
        }
        emit MetadataUpdate(_pyramidId);
    }
