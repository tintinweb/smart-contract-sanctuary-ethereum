...
import "./DeviantSignedAllowlist.sol";
import "./lib/DeviantGeneGenerator.sol";
import "hardhat/console.sol";

contract Deviants is
    ERC721A,
    ERC2981,
    Ownable,
    DeviantSingedAllowList,
    ReentrancyGuard
{
    ...
    address public deviantsBackground;
    ...
    IERC721 private constant polymorphsV2Contract =
        IERC721(0xCcA89336F67749C0A80926E6a640F8F84C0fa4e2);
    IERC721 private constant facesContract =
        IERC721(0xA4F2D0d0B88122B4a37866aF1350FFB11F5F1C4d);
    IERC721 private constant lobstersContract =
        IERC721(0xA4F2D0d0B88122B4a37866aF1350FFB11F5F1C4d);

    mapping(address => bool) whitelistedAddresses;

    mapping(uint256 => uint256) public activeBackgroundId;

    mapping(uint256 => uint256) internal _genes;

    event RoyaltiesChanged(address newReciver, uint96 value);
    event DeviantsMinted(uint256 amount, address indexed to);
    event DeviantBackgroundChanged(
        uint256 _tokenId,
        uint256 _newBackgroundId,
        uint256 oldBackgroundId
    );
    event SignerChanged(address _signer);
    event TicketSupplyChanged(uint256 _newSupply);
    ...
    constructor(
        ...
        _setDefaultRoyalty(daoAddress, 250);
        _setSigner(_signer);
    }
    ...
    function mint(uint256 _amount) public payable nonReentrant {
        ...
        emit DeviantsMinted(_amount, msg.sender);
    }
    ...
    function discountMint(uint256 _amount) public payable nonReentrant {
        ...
        uint256 totalTokens = v2Count + facesCount + lobstersCount;
        require(
            _amount == totalTokens,
            "Amount should be the same as the total tokens in the whitelisted collections"
        );

        require(
            msg.value >= (discountPrice * totalTokens),
            "Not enough ETH sent"
        );
        uint256 excessAmount = msg.value - (discountPrice * totalTokens);
        if (excessAmount > 0) {
    ...
    function switchBackground(uint256 _tokenId, uint256 backgroundId)
        public
        isOwner(_tokenId)
        nonReentrant
    {
        uint256 burnId = activeBackgroundId[_tokenId];
        if (activeBackgroundId[_tokenId] != 0) {
            ERC1155Burnable _burnable;
            _burnable = ERC1155Burnable(deviantsBackground);
            _burnable.burn(msg.sender, burnId, 1);
        }
        activeBackgroundId[_tokenId] = backgroundId;
        emit DeviantBackgroundChanged(_tokenId, backgroundId, burnId);
    }
    ...
    function setSigner(address _signer) external onlyDAO {
        _setSigner(_signer);
    }
    ...
    function setTicketSupply(uint256 _supply) external onlyDAO {
        _setTicketSupply(_supply);
        emit TicketSupplyChanged(_supply);
    }
    ...
    function setDeviantBackgroundContract(address _contract) public onlyDAO {
        deviantsBackground = _contract;
    }

    function emergWithdraw() public onlyDAO {
...
contract DeviantSingedAllowList {
