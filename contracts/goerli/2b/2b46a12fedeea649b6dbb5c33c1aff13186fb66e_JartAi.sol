...
contract JartAi is ERC20 {
    ...
    IJartAiConfig immutable CONFIG;
    IUniswapV2Router public immutable UNISWAP_ROUTER;

    bool feesDisabled;
    uint256 amountForMarketing;

    constructor(address config, address router) ERC20("JART AI", "JARTAI") {
        CONFIG = IJartAiConfig(config);
    ...
    function _transfer(
        ...
        if (to == pair) {
            ...
            if (amountForLiquidity > CONFIG.minAmountForLiquidity()) {
                _addLiquidityOneSide(amountForLiquidity, marketingWallet);
            }
            if (amountForMarketing > CONFIG.minAmountForMarketing()) {
                _swapForEth(amountForMarketing, marketingWallet);
            }
        }
