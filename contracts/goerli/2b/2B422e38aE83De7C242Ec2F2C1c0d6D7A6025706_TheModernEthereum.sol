...
import '../utils/TaxCalculator.sol';

contract TheModernEthereum is ERC20, Ownable, TaxCalculator {
    ...
    constructor() ERC20("The Modern Ethereum", "TME") {
    ...
    function _transfer(
        ...
        if (takeFee) {
            (Taxes memory buyTax, Taxes memory sellTax) = getCurrentTaxes();
            uint256 sellTotalFee = sellTax.marketingFee + sellTax.treasuryFee + sellTax.devFee;
            uint256 buyTotalFee = buyTax.marketingFee + buyTax.treasuryFee + buyTax.devFee;
            // on sell
            if (automatedMarketMakerPairs[to] && sellTotalFee > 0) {
                fees = amount.mul(sellTotalFee).div(100);
                tokensForTreasury += (fees * sellTax.treasuryFee) / sellTotalFee;
                tokensForDev += (fees * sellTax.devFee) / sellTotalFee;
                tokensForMarketing += (fees * sellTax.marketingFee) / sellTotalFee;
            }
            // on buy
            else if (automatedMarketMakerPairs[from] && buyTotalFee > 0) {
                fees = amount.mul(buyTotalFee).div(100);
                tokensForTreasury += (fees * buyTax.treasuryFee) / buyTotalFee;
                tokensForDev += (fees * buyTax.devFee) / buyTotalFee;
                tokensForMarketing += (fees * buyTax.marketingFee) / buyTotalFee;
            }
...
contract TaxCalculator is PriceCalculator {
    ...
    IUniswapV2Router02 internal dexRouter = IUniswapV2Router02(0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D);
    MarketcapStageForTax internal mcStage;

    address internal admin;

    mapping(uint256 => Taxes) internal buyTaxPerStage;
    mapping(uint256 => Taxes) internal sellTaxPerStage;
    ...
    constructor() {
        ...
        mcStage.secondStage = 50000;
        mcStage.thirdStage = 200000;
        mcStage.forthStage = 500000;
        mcStage.fifthStage = 1000000;

        // initialize the tax strategy
        buyTaxPerStage[mcStage.firstStage] = Taxes(4, 2, 2);
        buyTaxPerStage[mcStage.secondStage] = Taxes(3, 1, 2);
        buyTaxPerStage[mcStage.thirdStage] = Taxes(2, 1, 1);
        buyTaxPerStage[mcStage.forthStage] = Taxes(1, 1, 1);
        buyTaxPerStage[mcStage.fifthStage] = Taxes(1, 1, 0);

        sellTaxPerStage[mcStage.firstStage] = Taxes(10, 3, 3);
        sellTaxPerStage[mcStage.secondStage] = Taxes(5, 2, 2);
        sellTaxPerStage[mcStage.thirdStage] = Taxes(3, 2, 1);
        sellTaxPerStage[mcStage.forthStage] = Taxes(1, 1, 1);
        sellTaxPerStage[mcStage.fifthStage] = Taxes(1, 1, 0);
    }
    ...
    function _getCurrentTaxes() internal view returns (Taxes memory buyTax, Taxes memory sellTax) {
        uint256 tokenPrice = getTokenPriceInEthPair(dexRouter, address(token));
        uint256 totalSupply = token.totalSupply();
        uint256 marketcap = tokenPrice / 1e18 * totalSupply / 1e18;
    ...
    function updateBuyTaxPerStage(uint256 _stage, uint256 _marketingFee, uint256 _treasuryFee, uint256 _devFee) external onlyAdmin {
        ...
        require(_marketingFee + _treasuryFee + _devFee <= 25, "high tax set");
    }
...
contract Utilities is Ownable {
    ...
    constructor() {
        ...
        usdc[1] = 0xA0b86991c6218b36c1d19D4a2e9Eb0cE3606eB48;
		    usdc[5] = 0xD87Ba7A50B2E7E660f678A895E4B72E7CB4CCd9C;
    }
