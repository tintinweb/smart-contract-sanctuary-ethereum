...
contract CamelToes is ERC721AQueryable, Ownable, ReentrancyGuard, DefaultOperatorFilterer {
  ...
  modifier mintPriceCompliance(uint256 _mintAmount) {
    if(_numberMinted(msg.sender) == 0) {
      uint256 firstMintCost = cost * (_mintAmount  - 1);
      require(msg.value >= firstMintCost, 'Insufficient funds!');
      _;
    } else {
  ...
  function mint(uint256 _mintAmount) public payable mintCompliance(_mintAmount) mintPriceCompliance(_mintAmount) {
    ...
    require(_numberMinted(msg.sender) < 8, "Max mints for this wallet exceeded");
