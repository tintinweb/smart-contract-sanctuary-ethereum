...
/**s
 * @title ERC1155 Base Token Contract
 * @notice This contract is meant to serve for both Semi- and Non- fungible tokens
 **/
    ...
    PaymentSplitter
{
        ...
        initializer
    {
        require(
            isBase == false,
            "Base1155: this is the base contract,cannot initialize"
        );

        require(owner == address(0), "Base1155: contract already initialized");

        require(owner_ != address(0), "Base1155: Owner address cannot be 0");
        ...
        onlyOwner
    {
        require(
            ...
            "Base1155: Mint Price should be different than the previous price"
        );
        ...
        onlyOwner
    {
        require(
            ...
            "Base1155: Max Supply should be different than the previous supply"
        );
    ...
    {
        require(exists(tokenId), "Base1155: NONEXISTENT_TOKEN");
        return
            string(
                abi.encodePacked(
                    super.uri(0),
    ...
    function mint(
        ...
        require(to_ != address(0), "Base1155: Address cannot be 0");
        require(
            received == mintPrice[tokenId_] * (amount_),
            "Base1155: Ether sent is not the right amount"
        );
        require(tokenId_ <= maxId, "Base1155: Token id mismatch");
        require(
            amount_ + totalSupply(tokenId_) <= supplyPerTokenId[tokenId_],
            "Base1155: Amount mismatch"
        );

        require(
            checkLimit(_msgSender(), amount_),
            "Base1155: max NFT mints per address exceeded"
        );
    ...
    function adminMint(
        ...
        require(to != address(0), "Base1155: Address cannot be 0");

        require(id <= maxId, "Base1155: Token id mismatch");
        require(
    ...
    function _initialAddTokens(
        ...
        require(
            ...
            "Base1155: IDs/MintCost arity mismatch"
        );
        require(
            maxId_ + 1 == supplyPerTokenId_.length,
            "Base1155: IDs/Supply arity mismatch"
        );
