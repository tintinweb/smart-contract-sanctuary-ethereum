...
contract PlatziPunksDNA {
    ...
    // funcion de prueva, tendriamos que usar Chailink
    function deterministicPseudoRandomDNA(uint256 _tokenId, address _minter)
        public
        pure
        returns (uint256)
    {   // se puede sumar ya que una addres es una compocision exadecimal derivada de una llave privada, que a su vez deriva la llave publica que luego se convierte en una adress, al fin de cuenta son datos exadecimales
        // es decir que son convertibles a un numero
        uint256 combinedParams = _tokenId + uint160(_minter);
        //abi.encodedPacked va a convertir nuestro entero en un dato representado en bytes
        bytes memory encodedParams = abi.encodePacked(combinedParams);
        //keccak256 es una funcion de hasshing, toma una entrada y genera una salida unica
        bytes32 hashedParams = keccak256(encodedParams);

        return uint256(hashedParams);
    }

    // Get attributes y cada seccion del adn son dos digitos
    uint8 constant ADN_SECTION_SIZE = 2;

    // uint8 descarta del lado derecho los digitos
    ...
    {
        return
            // Cuando recortemos los siguientes 2 digitos se agregan dos ceros en ambas partes como definimos arriba
            uint8(
                // 1 por 10 elevado a la cantidad de numeros que queremos a├▒adir  ADN_SECTION_SIZE(2), esta cantidad de digitos va a ser resultado de lo que se va a descartar del lado derecho + el ADN de seccion
        ...
        returns (string memory)
    {
        ...
        // Con esto siempre va a retornar un atributo valido, aunque en la pieza de adn sean mayor a la cantidad de elemento que estan dentro de esta lista
...
contract PlatziPunks is ERC721, ERC721Enumerable, PlatziPunksDNA {
    ...
    function mint() public {
        ...
        // esto creara un randon en lugar de usar chailink
        tokenDNA[current] = deterministicPseudoRandomDNA(current, msg.sender);
        _safeMint(msg.sender, current);
        _idCounter.increment();
    }

    // Donde esta la imagen alojada
    ...
    function _paramsURI(uint256 _dna) internal view returns (string memory) {
        ...
        // IMPORTANTE: Solidity se maneja en bloques de memoria, y cada bloque que esta encerrado entre corchetes, el compilador reserva un poco de espacio en memoria que esta limitado,
        // y si en un bloque declaramos variables de mas puede ser que "por acomodo de variables" estemos sobrepasando ese stack 
    ...
    function tokenURI(uint256 tokenId) public view override returns(string memory) {
        require(_exists(tokenId) , "ERC721 Metadata: URI no exist token" );
        
        uint256 dna = tokenDNA[tokenId];
        string memory image = imageByDNA(dna);

        // string en formato json codificado
        // el encode lo convierte en string
        string memory jsonURI = Base64.encode(
            abi.encodePacked(
                '{ "name": "PlatziPunks #',
                tokenId.toString(), // esto hara que el tokenId sera correctamente convertido a el numero que esperamos a su representacion en string
                '", "description": "Platzi Punks are randomized Avataaars stored on chain to teach DApp development on Platzi", "image": "',
                image,
                '"}'
            )
        );
        // formato de url en base 64 que contiene un json que tiene la info del tokenURI
