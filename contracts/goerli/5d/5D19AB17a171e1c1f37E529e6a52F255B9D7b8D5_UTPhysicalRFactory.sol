...
contract UTPhysicalRFactory is Ownable, BlackHolePrevention {
    ...
    uint8       public constant     traitType            = 3;            // Physical redeemables
    ...
    // Launch new storage contract
    function addTrait(
        inputTraitStruct calldata _inputTrait
    ) external returns (uint16 traitId) {

        // Get Galaxis registry
        IRegistryConsumer galaxisReg = IRegistryConsumer(0x1e8150050A7a4715aad42b905C08df76883f396F);
        // Get the community_list contract
        CommunityList COMMUNITY_LIST = CommunityList(galaxisReg.getRegistryAddress("COMMUNITY_LIST"));
        // Get the community data
        (,address crAddr,) = COMMUNITY_LIST.communities(_inputTrait.communityId);
        require(
            crAddr != address(0),
            "UTPhysicalRFactory: Invalid community ID"
        );
        // Get community registry
        CommunityRegistry thisCommunityRegistry = CommunityRegistry(crAddr);
        
        address communityAdmin = thisCommunityRegistry.community_admin();
        require(
            msg.sender == communityAdmin,
            "UTPhysicalRFactory: Only community admin may add traits"
        );
        ...
        require(
            address(traitRegistry) != address(0),
            "UTPhysicalRFactory: Trait Registry not installed"
        );
        ...
        require(
            ERC721 != address(0),
            "UTPhysicalRFactory: Token not installed"
        );

        // Deploy _controller_ ONLY if not yet deployed for this traitType
        UTPhysicalRedeemController controllerCtr;
        address controllerAddr = traitRegistry.getDefaultTraitControllerByType(traitType);
        if(controllerAddr == address(0)) {

            // Deploy _controller_
            controllerCtr = new UTPhysicalRedeemController(ERC721, address(traitRegistry), 0, 0);

            // Add role for the this factory to write into TraitRegistry
            thisCommunityRegistry.grantRole(TRAIT_REGISTRY_ADMIN, address(this));

            // Set this controller as default for this traitType
            traitRegistry.setDefaultTraitControllerType(address(controllerCtr), traitType);
        } else {
        ...
        traits[0] = ECRegistryV3c.traitStruct(traitId, traitType, _inputTrait.start, _inputTrait.end, _inputTrait.enabled, address(storageCtr), _inputTrait.ipfsHash, _inputTrait.name);
...
// TODO --- add enabled to addTrait, updateTrait
//          in getTokenData use the enabled flag
//          copy setTraitonMultiple and make modification to return uint16 state change numbers
//          settrait to copy and modify - setTraitUnchecked()
//                                      - setMultipleUnchecked()
//          invertedDrop to use a memory array for setTraitMultiple and use return number
//          
...
// ---- Change end ----
    ...
    */
    ...
    event newTraitMasterEvent(string _name, address _address, uint8 _traitType, uint16 _start, uint16 _end);
    ...
    // ---- Change start ----
    // constructor (uint32  _communityId, address _owner) {
    ...
    function _init(uint32  _communityId, address _owner) internal virtual {
        ...
        // Get Galaxis registry
        if(chainId == 1 || chainId == 5 || chainId == 137 || chainId == 80001 || chainId == 1337 || chainId == 31337) {
    ...
    function addTrait(
        ...
        for (uint8 i = 0; i < _newTraits.length; i++) {
            ...
            // The following lines were removed, as implementers are not known for future traits so can not be created by this contract
            // if(newT.traitType == 3) {
            ...
            emit newTraitMasterEvent(newT.name, newT.storageImplementer, newT.traitType, newT.start, newT.end );

            // ---- Change start ----

            // This is commented out, as we need to add the logic controller's address and not the storage controller's address
            // As for one traitId we can have multiple logic cottroller, it can not be added here, but need to be a separate call
            // with the parameter (logicControllerAddress, traitId)
            // setTraitControllerAccess(address(newT.storageImplementer), newTraitId, true);

            // The following is removed as owner will be added into onlyTraitController(), and also because this address would remain traitcontroller even after transferOwnership()
    ...
    function updateTrait(
        ...
        // ---- Change start ----
        //traits[_index].implementer = _implementer;
    ...
    // ---- Change start ----
    // Renamed from setTraitOnMultiple() to setTraitOnMultipleUnchecked()
    ...
    // ---- Change start ----
    // New parameter list for easier(cheeper?) TraitDropper access
    ...
    // ---- Change start ----
    // Renamed from _setTrait to _setTraitUnchecked
    ...
    // Reason: this is being called many times from the various rendom trait dropper contracts
    ...
    */
    ...
    // ---- Change start ----
    // // We will use roles (access control from the Community Registry) - so this was removed
    ...
    */
    ...
    // ---- Change start ----
        ...
        return _addr == owner() || hasRole(TRAIT_DROP_ADMIN, _addr) || traitControllerAccess[_addr][uint8(byteNum)] & (0x01 * 2**bitPos) != 0;
    // ---- Change end ----
    ...
    modifier onlyAllowed(bytes32 role) { 
        require(isAllowed(role, msg.sender), "Trait Registry: Unauthorised");
        _;
    }
...
contract UTPhysicalRedeemStorage {

    uint8       public constant     traitType = 3;            // Physical redemption
    ...
    modifier onlyAllowed() {
        require(
            ...
            "UTPhysicalRedeemStorage: Not Authorised"
        );
        _;
    }
}
...
interface ITraitRegistry {
    ...
    function getContractControllerContains(address _addr) external view returns (bool);
}
...
contract UTPhysicalRedeemController {
    ...
    function setValue(uint16[] memory tokenId, uint8 value, address traitStorage) public {
        require(
            !locked,
            "UT Physical Redeem Controller: contract locked"
        );
        require(
            tokenId.length > 0,
            "UT Physical Redeem Controller: at least 1 token"
        );
        require(
            getTimestamp() > startTime,
            "UT Physical Redeem Controller: before start time"
        );
        require(
            getTimestamp() < endTime,
            "UT Physical Redeem Controller: after end time"
        );
        ...
        for(uint8 i = 0; i < tokenId.length; i++) {
            ...
            if(currentValue == 1 && value == 2) {
                ...
                require(
                    ...
                    "UT Physical Redeem Controller: not owner of token"
                );
                ITraitStorage(traitStorage).setValue(currentTokenId, 2);
            } else {
                require(
                    false,
                    "UT Physical Redeem Controller: invalid state"
                );
            }
        }
    }
    ...
    */
    ...
    function toggleLock() public {
        require(
            registry.getContractControllerContains(msg.sender),
            "UT Physical Redeem Controller: Not authorised"
        );
        locked = !locked;
    }
}
