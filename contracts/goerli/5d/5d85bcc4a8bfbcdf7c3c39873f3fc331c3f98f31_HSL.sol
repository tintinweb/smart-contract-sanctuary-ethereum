...
//        bool isActive;
    ...
    function storage_genomeOf(bool active, uint256 tokenId) public view returns (string memory) {
        ...
        string[] memory genomeLetters = new string[](128);
        for (uint256 i = 0; i < 128; i++) {
            genomeLetters[i] = _bases[(rawGenome>>(i*2))%4];
        }
        return string(abi.encode(genomeLetters));
    }
...
//        if (active != _tokenData[tokenId].isActive) {
...
//            return ((_nextToMint - _activeSupply) - _balances[address(0)].inactiveBalance);
...
//        return _ownershipOrderings[owner][active ? (_balances[owner].inactiveBalance + index) : index];

        uint256 curIndex = 0;
        for (uint256 i = 0; i < (thisBalance + otherBalance); i++) {
            uint256 curToken = _ownershipOrderings[owner][i];
//            if (_tokenData[curToken].isActive == active) {
...
//    function _mintInternal(address to, uint256 tokenId) private {
//        bool ownedByZero = (_tokenData[tokenId].owner == address(0));
//        bool eligibleToMint = (tokenId <= _nextToMint);
//
//        if (!(ownedByZero && eligibleToMint)) {
//            revert(MintError(ownedByZero, eligibleToMint));
//        }
//
//        _tokenData[tokenId].owner = to;
//        _tokenData[tokenId].mintedAt = uint64(block.timestamp);
//    }
...
//        _tokenData[tokenId].isActive = false;
...
//        require(_exists(false, tokenId), "e_inact");

        address tokenOwner = storage_ownerOf(false, tokenId);

        _tokenData[tokenId].lastTransferred = uint64(block.timestamp);
//        _tokenData[tokenId].isActive = true;
...
//        _tokenData[tokenId].isActive = false;
        ...
        _burns[tokenId/256] += 1<<(tokenId%256);
...
////            allExist = allExist && _exists(true, tokenIds[i]);
//        }
//        require(allExist, "e_act_batch");
...
//            uint256 tokenBit = tokenIds[i]%256;
            accumulator += (1<<(tokenId%256));
//            allExist = allExist && _exists(true, tokenId);

//            address tokenOwner = _tokenData[i].owner;
...
//        bool allExist = true;
        ...
        for (uint256 i = 0; i < tokenIds.length; i++) {
            ...
            //            uint256 tokenBit = tokenIds[i]%256;
            accumulator += (1<<(tokenId%256));
//            allExist = allExist && _exists(false, tokenId);
...
//        require(allExist, "e_inact_batch");
...
//        return (((tokenId < _nextToMint) && (_tokenData[tokenId].owner != address(0))) && (_tokenData[tokenId].isActive == active));
    ...
    function _transfer(
        ...
        uint256 fromBalanceTotal = storage_balanceOf(active, from) + storage_balanceOf(!active, from);
        ...
        uint256 toBalanceTotal = storage_balanceOf(active, to) + storage_balanceOf(!active, to);
