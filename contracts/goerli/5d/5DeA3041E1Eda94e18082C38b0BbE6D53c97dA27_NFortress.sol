...
contract NFortress is OwnableUpgradeable {
    ...
    struct DiscountData {
        ...
        uint256 feeDiscount;    // percentage*10000 to reduce fee
    }
    ...
    function initialize() public initializer {
        __Ownable_init();
        fee = 10000000000000000; // 0.01 ETH * 10**18
    }
    ...
    function bindAddress(address proxy, uint256 year) external payable {
        unchecked {
            ...
            if(msg.value < getFee(msg.sender)*year) revert InvalidValue("msg.value", msg.value);
            userData[proxy] = UserData(msg.sender, block.timestamp + (year*31536000));
    ...
    function renewService(uint256 year) external payable {
        unchecked {
            ...
            if(proxy == address(0)) revert InvalidAddress("proxy", proxy);
            if(msg.value < getFee(msg.sender)*year) revert InvalidValue("msg.value", msg.value);
            ...
            uint256 newTime;
            if (endTime > curTime){
                newTime = endTime + (year*31536000);
            } else {
                newTime = curTime + (year*31536000);
            }
            if((newTime-curTime) / 31536 > 10000) revert InvalidValue("year", year);
    ...
    function setPromotion(address _nftAddress, uint256 _feeDiscount) external onlyOwner {
        discountData.push(DiscountData(_nftAddress, _feeDiscount*10000)); // percentage*10000 to reduce fee
    }
    ...
    function balanceOfERC721(address nftAddr, address proxy) external view returns (uint256) {
        if(nftAddr == address(0)) revert InvalidAddress("nftAddr", nftAddr);
        address safeAddr = userData[proxy].safeAddr;
        if(proxy == address(0) || safeAddr == address(0)) revert InvalidAddress("proxy", proxy);
        uint256 balance = IERC721(nftAddr).balanceOf(safeAddr) + IERC721(nftAddr).balanceOf(proxy);
        return balance;
    }

    function ownerOfERC721(address nftAddr, address proxy, uint256 tokenId) external view returns (bool) {
        if(nftAddr == address(0)) revert InvalidAddress("nftAddr", nftAddr);
        address safeAddr = userData[proxy].safeAddr;
        if(proxy == address(0) || safeAddr == address(0)) revert InvalidAddress("proxy", proxy);
        address owner = IERC721(nftAddr).ownerOf(tokenId);
        return owner == safeAddr || owner == proxy;
    }

    function balanceOfERC1155(address nftAddr, address proxy, uint256 tokenId) external view returns (uint256) {
        if(nftAddr == address(0)) revert InvalidAddress("nftAddr", nftAddr);
        address safeAddr = userData[proxy].safeAddr;
        if(proxy == address(0) || safeAddr == address(0)) revert InvalidAddress("proxy", proxy);
        uint256 balance = IERC1155(nftAddr).balanceOf(safeAddr, tokenId) + IERC1155(nftAddr).balanceOf(proxy, tokenId);
        return balance;
    }
}
