...
contract Depot is Owned, Pausable, ReentrancyGuard, MixinResolver, IDepot {
    ...
    // }
    receive() external payable nonReentrant rateNotInvalid(ETH) notPaused {
        _exchangeEtherForSynths();
    }
    ...
    function _exchangeEtherForSynths() internal returns (uint256) {
        ...
        uint256 requestedToPurchase = msg.value.multiplyDecimal(exchangeRates().rateForCurrency(ETH));
        uint256 remainingToFulfill = requestedToPurchase;

        // Iterate through our outstanding deposits and sell them one at a time.
        for (uint256 i = depositStartIndex; remainingToFulfill > 0 && i < depositEndIndex; i++) {
        ...
        if (remainingToFulfill > 0) {
            TransferHelper.safeTransferETH(msg.sender, remainingToFulfill.divideDecimal(exchangeRates().rateForCurrency(ETH)));
        }
     ...
     */
    function exchangeEtherForSNXAtRate(uint256 guaranteedEtherRate, uint256 guaranteedSynthetixRate)
        external
        payable
        rateNotInvalid(SNX)
        rateNotInvalid(ETH)
    ...
    function _exchangeSynthsForSNX(uint256 synthAmount) internal returns (uint256) {
        // How many SNX are they going to be receiving?
     ...
     */
    function exchangeSynthsForSNXAtRate(uint256 synthAmount, uint256 guaranteedRate)
        external
        rateNotInvalid(SNX)
        notPaused
        returns (
     ...
     */
    function synthetixReceivedForSynths(uint256 amount) public view returns (uint256) {
     ...
     */
    function synthetixReceivedForEther(uint256 amount) public view returns (uint256) {
        // How much is the ETH they sent us worth in sUSD (ignoring the transfer fee)?
        uint256 valueSentInSynths = amount.multiplyDecimal(exchangeRates().rateForCurrency(ETH));
     ...
     * @param amount The amount of ether (in wei) you want to ask about
     */
    function synthsReceivedForEther(uint256 amount) public view returns (uint256) {
