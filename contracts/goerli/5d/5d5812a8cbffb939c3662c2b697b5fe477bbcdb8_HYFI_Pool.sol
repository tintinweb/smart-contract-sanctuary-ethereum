    ...
    AccessControlUpgradeable
{
    ...
    IERC20Upgradeable _tokenHYFI;
    uint256 _HYFIexchangeRate;
    uint256 _tokenDecimals;

    mapping(uint256 => HYFITokenRewardInfo) _rewards;
    ...
    function initialize(
        ...
        _HYFIexchangeRate = HYFIexchangeRate;
    ...
    function setHYFIRewardType(
        ...
        amountNotZero(euroAmount)
        amountNotZero(rewardTotalAmount)
    {
        _rewards[rewarId].euroAmount = euroAmount;
        _rewards[rewarId].rewardTotalAmount = rewardTotalAmount;
    }
    ...
    function withdrawERC20Tokens(address recipient, uint256 amount)
        external
        onlyRole(DEFAULT_ADMIN_ROLE)
        amountNotZero(amount)
    ...
    function setTokenAddress(IERC20Upgradeable newtokenAddress)
        external
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
    ...
    {
        _HYFIexchangeRate = newExchangeRate;
    }

    function tokenPayoutCalculator(uint256 amount, uint256 rewardId)
        public
        view
        returns (uint256 tokenAmount)
    {
        tokenAmount =
            ((amount * _rewards[rewardId].euroAmount * _tokenDecimals) /
                _HYFIexchangeRate) *
            _tokenDecimals;
        return tokenAmount;
    }
    ...
    function getRewardInfo(uint256 rewardId)
        public
        view
        returns (
            uint256,
    ...
    {
        return (
            _rewards[rewardId].euroAmount,
            _rewards[rewardId].rewardTotalAmount,
            _rewards[rewardId].distributedRewardAmount
        );
    }

    function getRewardDistributedAmount(uint256 rewardId)
        public
        view
        returns (uint256)
    {
    ...
    function getUserRewardsAmount(address user, uint256 rewardId)
        public
        view
        returns (uint256)
    {
