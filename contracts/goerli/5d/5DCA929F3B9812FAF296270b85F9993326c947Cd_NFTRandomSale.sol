...
contract NFTRandomSale is Ownable, BaseNFTSale {
  ...
  function addRarities(Rarity[] memory rarities) external onlyOwner {
    for (uint256 i = 0; i < rarities.length; i++) {
      _addRarity(rarities[i].weight, rarities[i].tokenIds);
    }
  }

  //Add rarity in rarities array
  function _addRarity(uint8 _weight, uint256[] memory _tokenIds)
    internal
  {
    require(
      _tokenIds.length > 0,
      "NFTRandomSale: TokenIds must not be empty"
    );
    totalWeight += _weight;
    require(
      totalWeight <= MAX_WEIGHT,
      "NFTRandomSale: Weight Limit exceeds"
    );
  ...
  function updateRaritiesList(
    ...
    if (IERC1155(nft).balanceOf(address(this), tokenId) == 0) {
      ...
      rarity.tokenIds[tokenIdIndex] = rarity.tokenIds[
        rarity.tokenIds.length - 1
      ];
      rarity.tokenIds.pop();
    }
