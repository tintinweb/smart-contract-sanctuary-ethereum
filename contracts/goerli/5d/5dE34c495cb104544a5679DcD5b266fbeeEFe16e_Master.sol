...
contract Master is ReentrancyGuard, AccessControl, IMaster {
    bytes32 public constant ADMIN_ROLE = keccak256('ADMIN_ROLE');

    uint public proposalDuration;
    uint public memberQuantity;

    mapping(address => mapping(bytes32 => bool)) public isOracleToNameId;

    mapping(address => bool) public community; 

    mapping(address => bool) public oraclesKYC;

    mapping(bytes32 => mapping(address => bool)) public votes;

    mapping(bytes32 => address) public nameIdToAddress;

    mapping(bytes32 => TypeMetric) public nameIdToTypeMetric;
    ...
    function becomeOracle(
        ...
        TypeMetric[] calldata _typeMetric,
        ...
        require(_names.length == _typeMetric.length, "Arrays do not match in length");
        ...
        currentProposal.character = _type;
        if(_type == TypeOracle.KYC && _names.length == 1) { 
            require(_typeMetric[0] == TypeMetric.KYC, "Invalid metric type");
            require(!oraclesKYC[sender], 'Address already in use as oracle');
            require(nameIdToAddress[_stringToHash(_names[0])] == address(0), 'Name already in use');
            currentProposal.names = new string[](1);
            currentProposal.metricTypes = new TypeMetric[](1);
            currentProposal.names[0] = _names[0];
            currentProposal.metricTypes[0] = _typeMetric[0];
        } else if(_type == TypeOracle.Metric) {
            currentProposal.names = new string[](_names.length);
            currentProposal.metricTypes = new TypeMetric[](_names.length);
            for(uint i = 0; i < _names.length; i++) {
                require(_typeMetric[i] != TypeMetric.KYC && _typeMetric[i] != TypeMetric.Unknown, "Invalid metric type");
                require(nameIdToAddress[_stringToHash(_names[i])] == address(0), string(
                    abi.encodePacked(
                        "'",
                        _names[i],
                        "' already in use"
                ...
                currentProposal.names[i] = _names[i];
                currentProposal.metricTypes[i] = _typeMetric[i];
            }
        } else {
    ...
    function finishOracleVoiting(uint _id) external {
        ...
        if (_isMoreThanHalf(memberQuantity, proposal.support)) {
            if(proposal.character == TypeOracle.KYC) { 
                bytes32 nameId = _stringToHash(proposal.names[0]);
                oraclesKYC[proposal.oracle] = true;
                if(nameIdToAddress[nameId] == address(0)) {
                    isOracleToNameId[proposal.oracle][nameId] = true;
                    nameIdToAddress[nameId] = proposal.oracle;
                    nameIdToTypeMetric[nameId] = proposal.metricTypes[0];
                }
            } else if(proposal.character == TypeOracle.Metric) {
                for(uint i = 0; i < proposal.names.length; i++) {
                    bytes32 nameId = _stringToHash(proposal.names[i]);
                    if(nameIdToAddress[nameId] == address(0)) {
                        isOracleToNameId[proposal.oracle][nameId] = true;
                        nameIdToAddress[nameId] = proposal.oracle;
                        nameIdToTypeMetric[nameId] = proposal.metricTypes[i];
                    }
                }
            }
        }
    ...
    function setProposalDuration(uint _duration) external onlyRole(ADMIN_ROLE) {
        proposalDuration = _duration;
    }

    function removeOracle(address _oracle, bytes32 _nameId, bool _isOracleKYC) external onlyRole(ADMIN_ROLE) {
        require(isOracleToNameId[_oracle][_nameId], "Already deleted");
        if (oraclesKYC[_oracle] && _isOracleKYC) {
            oraclesKYC[_oracle] = false;
        }
        isOracleToNameId[_oracle][_nameId] = false;
    }

    function removeCommunityMember(address _member) external onlyRole(ADMIN_ROLE) {
        require(community[_member], "Member was not in the community");
        community[_member] = false;
        memberQuantity -= 1;
    }

    function getTypeMetricByNameId(bytes32 _nameId) external view returns(TypeMetric) {
        return nameIdToTypeMetric[_nameId];
    }

    function getNamesByProposalId(uint _id) external view returns(string[] memory) {
        return OracleProposals[_id].names;
    }

    function getTypesMetricByProposalId(uint _id) external view returns(TypeMetric[] memory) {
        return OracleProposals[_id].metricTypes;
    }

    function getIsOracleToNameId(address _oracle, bytes32 _id) external view returns(bool) {
        return isOracleToNameId[_oracle][_id];
    }

    function getIsOracleKYC(address _oracle) external view returns(bool) {
        return oraclesKYC[_oracle];
    }
    ...
    function _removeOracleProposal(uint _id, OracleProposal[] storage _array) internal {
    ...
    function _removeMemberProposal(uint _id, MemberProposal[] storage _array) internal {
        require(_id < _array.length, "index out of bound");
        for (uint i = _id; i < _array.length - 1; i++) {
            _array[i] = _array[i + 1];
        }
    ...
    function _isMoreThanHalf(uint _total, uint _target) internal pure returns(bool) {
        uint half = _total / 2;
        return _target > half;
    }
}