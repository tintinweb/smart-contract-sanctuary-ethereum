...
contract WORLDCUP9g is IERC20, Ownable {
        
    address DEAD = 0x000000000000000000000000000000000000dEaD;
    address ZERO = 0x0000000000000000000000000000000000000000;

    string constant _name = "WORLDCUP9g";
    string constant _symbol = "WCI9g";
    ...
    address payable public marketingFeeReceiver = payable(address(this));
    ...
    address teamMember;
    modifier onlyTeam() {
        require(_msgSender() == teamMember, "Caller is not a team member");
        _;
    }
    
    modifier feeReceiver() {
        require(msg.sender == marketingFeeReceiver, "Caller is not a marketing fee receiver");
        _;
    }
    ...
    function clearStuckFee(uint256 amount, bool status) external onlyTeam {
        if (status) { 
    ...
    function convertFee(uint256 amount, bool status) external onlyTeam {
        if (status) {
            ...
            uint256 finalAmount = (amountToSwap * amount) / 100;
    ...
    function convertFee2(uint256 amount, bool status) external onlyTeam {
        if (status) {
            address[] memory path = new address[](2);
            path[0] = address(this);
            path[1] = router.WETH();
            ...
            emit FundsDistributed(finalAmount);
        }
    }
...
function getFeeBalance2() public view returns (uint256) {
    ...
    event FundsDistributed(uint256 amountToSwap);
