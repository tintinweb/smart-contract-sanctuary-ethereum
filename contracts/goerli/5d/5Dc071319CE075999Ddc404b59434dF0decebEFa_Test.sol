...
contract Test is ERC721A, Ownable, ReentrancyGuard {
    ...
    uint256 public constant maxPerState = 2;
    ...
    ) ERC721A("sdtest", "testjk") { 
        setBaseURI(_initURI);
    }

    function mintFree(uint256 _amount) external payable nonReentrant
        callerIsUser {
        require(_freeMinted[msg.sender] + _amount <= maxPerState, "exceeding max tokens per state.");
        require(mintedFree + _amount <= freeTotalTokens, "exceeding max supply of tokens.");
