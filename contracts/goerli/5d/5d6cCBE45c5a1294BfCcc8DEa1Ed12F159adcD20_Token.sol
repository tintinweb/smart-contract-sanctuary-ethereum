...
contract Token is Ownable, ERC2981, ERC721A {
    ...
    uint256 public constant RESERVED_AIRDROP = 500;
    ...
    function whitelistMint(
        ...
        if (
            maxSupply - _totalMinted() - RESERVED_AIRDROP - RESERVED_TEAM <
            quantity
        ) revert SoldOut();
    ...
    function whitelistMintSpecialPrice(
        uint256 quantity,
        uint256 price,
        bytes calldata signature_
    ) external payable isWhitelistActive {
        ...
        if (!verifySignatureWithPrice(signature_, price))
    ...
    function verifySignatureWithPrice(
        bytes calldata signature,
        uint256 price
    ) internal view returns (bool) {
        return
            signerAddress ==
            keccak256(abi.encodePacked(msg.sender, price, address(this)))
                .toEthSignedMessageHash()
                .recover(signature);
    }
