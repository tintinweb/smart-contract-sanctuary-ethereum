...
contract SovereignV2 is ERC721PausableUpgradeable, AccessControlUpgradeable {
    using CountersUpgradeable for CountersUpgradeable.Counter;
    using StringsUpgradeable for uint256;

    uint256 public mintingPhase; // 0 closed, 1 OG1, 2 OG2, 3 public
    ...
    bytes32 public merkleRootOG1;
    bytes32 public merkleRootOG2;
    ...
    struct InitArgs {
        ...
        string contractURI;
        bool numberedMetadata;
        bytes32 merkleRootOG1;
        bytes32 merkleRootOG2;
    }   
    ...
    function initialize(InitArgs calldata _init) public initializer {
        ...
        crossmintWallet = _init.crossmintWallet;
        ...
        merkleRootOG1 = _init.merkleRootOG1;
        merkleRootOG2 = _init.merkleRootOG2;
    ...
    modifier isCrossmint(address _caller) {
        require(_caller == crossmintWallet, "Only Crossmint can call this function");
        _;
    }
    ...
    modifier isEligible(address _recipient, uint256 _quantity, bytes32[] calldata _proof) {
        if (mintingPhase == 1) {
            require(MerkleProofUpgradeable.verify(
                _proof, 
                merkleRootOG1, 
                keccak256(abi.encodePacked(_recipient))), "Not eligible for current phase (AT Holders)"
            );
        }
            ...
            require(MerkleProofUpgradeable.verify(
                _proof, 
                merkleRootOG2, 
                keccak256(abi.encodePacked(_recipient))), "Not eligible for current phase (Discord OGs)"
            );
        }
        _;
    }
    ...
    function mint(uint256 _quantity, bytes32[] calldata _proof) 
        external  
        payable
        isActive()
        isWithinWalletLimit(msg.sender, _quantity)
        isCorrectPayment(_quantity)
        isAvailable(_quantity)
        isEligible(msg.sender, _quantity, _proof)
    {
        mintInternal(msg.sender, _quantity);
    }

    function crossmint(address _recipient, uint256 _quantity, bytes32[] calldata _proof) 
        external  
        payable
        isActive()
        isCrossmint(msg.sender)
        isWithinWalletLimit(_recipient, _quantity)
        isCorrectPayment(_quantity)
        isAvailable(_quantity)
        isEligible(_recipient, _quantity, _proof)
    {
    ...
    function setCrossmintWallet(address payable _wallet) external onlyRole(ADMIN_ROLE) {
        crossmintWallet = _wallet;
    }
    ...
    function setMerkleRootOG1(bytes32 _root) external onlyRole(ADMIN_ROLE) {
        merkleRootOG1 = _root;
    }

    function setMerkleRootOG2(bytes32 _root) external onlyRole(ADMIN_ROLE) {
        merkleRootOG2 = _root;
    }
