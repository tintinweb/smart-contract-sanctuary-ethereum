...
contract Stabilizer {
  ...
  event Invested(uint sweep_amount, uint usdx_amount);
  event Paidback(uint amount);
  ...
  /**
   * @notice Invest SWEEP and USDX
   * Sends balances from the STABILIZER to the ASSET address.
   */
  function invest() external onlyBanker notFrozen {
    uint sweep_balance = sweep.balanceOf(address(this));
    uint usdx_balance = usdx.balanceOf(address(this));

    if(sweep_balance == 0 && usdx_balance == 0) { revert NotEnoughBalance(); }

    TransferHelper.safeApprove(address(sweep), address(asset), sweep_balance);
    if(usdx_balance > 0) TransferHelper.safeApprove(address(usdx), address(asset), usdx_balance);
    asset.deposit(sweep_balance, usdx_balance);

    emit Invested(sweep_balance, usdx_balance);
  }
   ...
   */
  function payback(uint sweep_amount) external onlyBanker {
    if(sweep_amount == 0) { revert InputIsZero(); }

    asset.withdraw(sweep_amount);
    emit Paidback(sweep_amount);
  }
   ...
   */
  function withdraw(uint sweep_amount) external onlyBanker notFrozen {
    if(sweep_amount == 0) { revert InputIsZero(); }
   ...
   */
  function repay(uint sweep_amount) public {
    if(sweep_amount == 0) { revert InputIsZero(); }
    TransferHelper.safeTransferFrom(address(sweep), msg.sender, address(this), sweep_amount);
    _burn(sweep_amount);
  }
   ...
   */
  function liquidate(uint sweep_amount) public {
    if(!defaulted()) { revert NotDefaulted(); }
    repay(sweep_amount);
    if(sweep_borrowed > getSpreadValue()) { revert DebtNotRepaid(); }
    banker = msg.sender;
  }

  function _burn(uint _sweep_amount) internal {
    uint spread_amount = getSpreadValue();
  ...
  function getSpreadValue() public view returns(uint) {
    if(sweep_borrowed == 0) return 0;
      ...
      return sweep_borrowed * uint(sp_unit) / (10 ** sweep.decimals()) - sweep_borrowed;
    }
  }
