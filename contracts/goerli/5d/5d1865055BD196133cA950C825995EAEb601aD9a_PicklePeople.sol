...
contract PicklePeople is ERC721, Pausable, AccessControl, ERC721Royalty, ERC721Burnable {
    ...
    string private _ipfsHash = "QmbRH2eUmygGihxo1EegnccrdM2EwBEVVSJZDwXjtKi1AT";
    string private metadataSuffix = ".json";
    string private _contractURI;
    uint public totalCount = 3044;
...
// View
    ...
    function verify2(bytes32[] memory _proof, address _addr, uint256 _limit) public view returns (bool) {
        bytes32 _leaf = keccak256(bytes.concat(keccak256(abi.encode(_addr, _limit))));
        return MerkleProof.verify(_proof, root, _leaf);
    }
...
// Admin
    ...
    function setBeneficiary(address newBeneficiary) external onlyRole(MINTER_ROLE) {
        beneficiary = newBeneficiary;
    }

    function setContractURI(string calldata newURI) external onlyRole(MINTER_ROLE) {
        _contractURI = newURI;
    }
