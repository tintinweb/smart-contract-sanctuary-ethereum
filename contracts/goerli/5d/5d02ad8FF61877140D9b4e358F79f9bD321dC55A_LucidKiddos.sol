...
contract LucidKiddos is Ownable, Pausable, ERC721A, EIP2981RoyaltyOverrideCore, ReentrancyGuard {
  ...
  function supportsInterface(bytes4 interfaceId) public view virtual override(ERC721A, EIP2981RoyaltyOverrideCore) returns (bool) {
      return ERC721A.supportsInterface(interfaceId) || EIP2981RoyaltyOverrideCore.supportsInterface(interfaceId);
  }
  ...
  function mint(uint64 _quantity) external payable whenNotPaused nonReentrant callerIsUser {
    ...
    for (uint64 i = 1; i <= _quantity; i++) {
      ...
      _updateRoyalties(tokenID, msg.sender);
    }
  }
  ...
  function rewardCount() external nonReentrant callerIsUser returns(uint256) {
    return _getAux(msg.sender) | 0;
  }
  ...
  function setTokenRoyalties(TokenRoyaltyConfig[] calldata royaltyConfigs) external override nonReentrant onlyOwner {
        _setTokenRoyalties(royaltyConfigs);
  }

  function setDefaultRoyalty(TokenRoyalty calldata royalty) external override nonReentrant callerIsUser onlyOwner {
      _setDefaultRoyalty(royalty);
  }

//transferFrom update royatlties
  function transferFrom(address from, address to, uint256 tokenId) public payable override {
    _updateRoyalties(tokenId, to);
...
//set burn token royalties
  function burn(uint256 tokenId) public  onlyOwner {
    _updateRoyalties(tokenId, address(0));
    super._burn(tokenId);
  }

  function _updateRoyalties(uint256 tokenId, address to) internal {
    TokenRoyaltyConfig[] memory config;
    config[0].bps = 1000;
    config[0].tokenId = tokenId;
    config[0].recipient = to;
