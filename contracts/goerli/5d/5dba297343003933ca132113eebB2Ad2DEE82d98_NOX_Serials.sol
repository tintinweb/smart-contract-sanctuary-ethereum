...
contract NOX_Serials is ERC1155DUpgradeable, IERC2981 {
    ...
    /**
     * @notice Function to mint tokens in the public sale.
    ...
    function mintPublicAT10(uint256 _amount) public payable {
        ...
        require(AT_id < 7000 && (AT_id + _amount) < 7000, "Error: max supply reached or will be reached");
    ...
    function mintPublicML10(uint256 _amount) public payable {
        require(paused == false, "Error: public mint is paused");
        require(_amount > 0, "Error: you can't mint less than one");  
        require((ML_id + _amount) < MAX_SUPPLY, "Error: max supply reached or will be reached");
        require(msg.value >= publicPrice * _amount, "Error: insufficient funds");
        for (uint256 i = 0; i < _amount; i++) {
            unchecked {ML_id++;}
    ...
    /**
     * @notice Function to mint tokens in the whitelist sale.
    ...
    function mintWhitelistAT10(bytes32[] calldata _merkleProof, uint256 _amount) public payable {
        ...
        require(_amount > 0, "Error: you can't mint less than one");
        require(AT_id < 5000 && (AT_id + _amount) < 5000, "Error: max supply reached or will be reached");
        require(AT_WLClaimed[msg.sender] + _amount <= 2, "Error: you cant own more than 2 NFTs in the whitelist");
    ...
    function mintWhitelistML10(bytes32[] calldata _merkleProof, uint256 _amount) public payable {
        require(whitelistPaused == false, "Error: whitelist mint is paused");
        require(_amount > 0, "Error: you can't mint less than one");
        require((ML_id + _amount) < 9000, "Error: max supply reached or will be reached");
        require(ML_WLClaimed[msg.sender] + _amount <= 2, "Error: you cant own more than 2 NFTs in the whitelist");
        require(msg.value >= whitelistPrice * _amount, "Error: insufficient funds");
        bytes32 leaf = keccak256(abi.encodePacked(msg.sender));
    ...
    /**
     * @notice Function to check if you are on the AT whitelist 
     * @param _merkleProof Proof that you are on the whitelist.
     * @return True if you are on the whitelist and use the correct proof, false otherwise.
     */
    function checkATWhitelisted(bytes32[] calldata _merkleProof) virtual public view returns (bool) {
        return (MerkleProofUpgradeable.verify(_merkleProof, ATmerkleSeed, keccak256(abi.encodePacked(msg.sender))));
    }

    /**
     * @notice Function to check if you are on the ML whitelist 
     * @param _merkleProof Proof that you are on the whitelist.
     * @return True if you are on the whitelist and use the correct proof, false otherwise.
     */
    function checkMLWhitelisted(bytes32[] calldata _merkleProof) virtual public view returns (bool) {
        return (MerkleProofUpgradeable.verify(_merkleProof, MLmerkleSeed, keccak256(abi.encodePacked(msg.sender))));
    }

    /**
     * @notice Set a new AT merkle seed containing a new AT whitelist.
     * @param _seed The new merkle seed containing the new whitelist.
     */
    function setATSeed(bytes32 _seed) virtual public onlyAdmin {
     ...
     /**
     * @notice Set a new ML merkle seed containing a new ML whitelist.
     * @param _seed The new merkle seed containing the new whitelist.
     */
