...
import { ReferralRewardsDistributorStorage } from '../ReferralRewardsDistributor.sol';
...
import { IReferralRewardsDistributorUpgrade } from './interfaces/IReferralRewardsDistributorUpgrade.sol';
...
contract ReferralRewardsDistributorUpgrade is
    NonReentrant,
    Pausable,
    ReferralRewardsDistributorAutoProxy,
    IReferralRewardsDistributorUpgrade
{
    // Storage
    ...
    constructor(address _proxy, address _owner)
        public
        Pausable(_owner)
        ReferralRewardsDistributorAutoProxy(_proxy, address(0), address(0), address(this))
    {}
    ...
    // This function is called on the new implementation for the upgrade
    function migrate(IGovernedContract _oldImpl) external requireProxy {
        _storage = ReferralRewardsDistributorStorage(
            IReferralRewardsDistributorUpgrade(address(_oldImpl))._storage()
        );
        _migrate(_oldImpl);
    }
...
interface IReferralRewardsDistributorUpgrade {
    function _storage() external returns (address);
