...
contract SillySociety is 
    ...
    
    {
        ...
        // bytes32 hash = "0x5B38Da6a701c568545dCfcB03FcB875f56beddC4";
        ERC721A ("SillySociety", "SSOC") 
        ...
        mapping(address => uint256) public freeOg;
        ...
        //OGMint
        function ogMint(uint256 _mintAmount) public payable isOgSaleActive mintCompliance(_mintAmount) {
            uint256 freeMint = 0;
            if (freeOg[msg.sender] == 0) {
                freeMint = freeOgMint();            
            }

            if (_mintAmount - freeMint > 0) {
                payableOgMint();
            }
        }
        
        function freeOgMint() private freeOgMintComplience() returns (uint256) {
            addressData[msg.sender] += 1;
            freeOg[msg.sender] += 1;
            totalSupplyOgSale += 1;
            _safeMint(msg.sender, 1);
            ogRecord.push(OgState(msg.sender, 1, 0));
            return freeOg[msg.sender];
        }
        
        //Free OgMint Complience
        modifier freeOgMintComplience() {
            require(totalSupplyOgSale + 1 <= OG_SUPPLY, "OG Maxed!");
            // require(freeOg[msg.sender] == 0, "Free mint already claimed");
            _;
        }

        function payableOgMint() private payableOgMintComplience() {
            addressData[msg.sender] += 1;
            totalSupplyOgSale += 1;
            _safeMint(msg.sender, 1);
            for (uint256 i = 0; i < ogRecord.length; i++) {
                if (ogRecord[i].account == msg.sender) {
                    ogRecord[i].paid = 1;
                }
            }
        }

        //Payable Mint Complience
        modifier payableOgMintComplience() {
            require(totalSupplyOgSale + 1 <= OG_SUPPLY, "OG Maxed!");
            require(msg.value >= getPrice() * 1, "Insufficient Funds!");
            _;
        }
...
contract OgSale is Ownable, ReentrancyGuard {
    ...
    uint256 public ogSaleStart = 1677988789;
    uint256 public ogSaleEnd = 1678075189;
