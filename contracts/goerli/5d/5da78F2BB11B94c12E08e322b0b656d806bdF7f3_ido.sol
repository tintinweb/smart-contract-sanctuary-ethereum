...
contract ido is Ownable {
    ...
    */
    ...
    //用户充值的总金额
     mapping ( address => uint256)  userRecharge;
        ...
        uint256 public inTokenCapacity;    // 项目方充入 token 的总额度
        uint256 public inTokenAmount;      // 项目方已经募集到 token 的代币
        uint256 public outTokenCapacity;   // 项目方充值 token 可供应的代币余量
  ...
  constructor(
    ...
    */
    ...
    //数据存储列表必须先初始化
    function initlockUser (uint num)  public view  onlyOwner {
       require(startTime < block.timestamp, "raise already start!");
    ...
    function setlockUser(uint place,uint _startTime)  public  onlyOwner {
        ...
        userList[place].startTime=_startTime;
    }
...
function swap(uint256 _amountIn) public  {
        ...
        //标记用户充值总金额
        userRecharge[msg.sender]+=_amountIn;
        //已经募集到的代币余额
        inTokenAmount+=_amountIn;
    ...
    function queryUser() public view  returns (lockUser[] memory){
        ...
        //新建用户提现列表
        lockUser[] memory items = new lockUser[](userList.length);
    ...
    function refillOwner() public onlyOwner{
     ...
     require(amountIn >= inTokenCapacity,"Insufficient balance");
      safeTransferUSDT(address(this),inTokenCapacity,sendAddrB);
     //标记项目已经可以运行
       statusRun=true;
