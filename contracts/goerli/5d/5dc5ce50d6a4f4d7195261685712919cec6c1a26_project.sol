...
abstract contract MarketPlace is ERC721, Ownable {
    ...
    mapping(uint256 => address) privateRecipient;
    ...
    function listPrivateSale(uint256 tokenId, uint256 price, address recipient) external {
    ...
    function listItem(uint256 tokenId, uint256 price) external {
        ...
        tokenPrice[tokenId] = price;
        isForSale[tokenId] = true;
        privateRecipient[tokenId] = address(0);
    ...
    function viewAllListings() external view returns (tokenSale[] memory){
        uint256 arraylen = saleItems.length;

        tokenSale[] memory activeSales = new tokenSale[](arraylen);

        for(uint256 i=0; i<arraylen; i++){
            uint256 tokenId = saleItems[i];
            activeSales[i].tokenid = tokenId;
            activeSales[i].tokenprice = tokenPrice[tokenId];
            activeSales[i].recipient = privateRecipient[tokenId];
        }

        return activeSales;
    }

    function checkPrice(uint tokenId) public view returns (uint256) {
    ...
    function buyItem(uint256 tokenId) external payable {
        ...
        if (privateRecipient[tokenId] != address(0)){
            require( _msgSender() == privateRecipient[tokenId], "invalid sale address");
        }

        _delistItem(tokenId);

        (bool success, ) = ownerOf(tokenId).call{value: totalPrice - royaltyPrice}("");
