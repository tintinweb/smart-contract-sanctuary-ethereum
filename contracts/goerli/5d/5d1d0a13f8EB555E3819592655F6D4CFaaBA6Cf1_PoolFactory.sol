...
abstract contract BoundNFTManager is GlobalBeaconProxyImpl, KeyValueStorage {
  ...
  function mintBoundNFTs(
    address to,
    GenericTokenInterface.Collection memory collection,
    uint256[] memory ids,
...
abstract contract PoolPolicy is OwnableUpgradeable {
  struct InitPolicy {
    address debtToken;
    PoolPolicy.PoolwidePolicy poolwidePolicy;
    GenericTokenInterface.Collection[] collaterals;
    PoolPolicy.CollateralwidePolicy[] collateralwidePolicy;
  }
  ...
  event PoolwidePolicySet(PoolwidePolicy poolwidePolicy);
  event CollateralSet(
    GenericTokenInterface.Collection collateral,
    CollateralwidePolicy collateralwidePolicy,
    bool isAdd
  );

  PoolwidePolicy public poolwidePolicy;
  mapping(bytes32 => CollateralwidePolicy) collateralPolicy;
  address public debtToken;

  function setAllPolicy(
    PoolwidePolicy calldata _poolwidePolicy,
    GenericTokenInterface.Collection[] calldata _collaterals,
    CollateralwidePolicy[] calldata _collateralwidePolicy
  ) external onlyOwner {
  ...
  function setPoolwidePolicy(PoolwidePolicy calldata _poolwidePolicy) external onlyOwner {
    _setPoolwidePolicy(_poolwidePolicy);
  }

  function _setPoolwidePolicy(PoolwidePolicy calldata _poolwidePolicy) internal {
  ...
  function _setCollateral(
    GenericTokenInterface.Collection calldata _collateral,
    CollateralwidePolicy calldata _collateralwidePolicy
  ) private {
  ...
  function delCollateralsBatch(GenericTokenInterface.Collection[] calldata _collaterals) external onlyOwner {
    for (uint256 i = 0; i < _collaterals.length; ++i) {
      delete collateralPolicy[_collaterals[i].hash()];
      emit CollateralSet(_collaterals[i], CollateralwidePolicy(0, 0, 0, 0), false);
    }
  }
}
...
contract PoolFactory {
  struct PoolConfig {
    PoolPolicy.InitPolicy initPolicy;
    address poolAdmin;
    string poolName;
  }
  ...
  function deployPool(PoolConfig calldata poolConfig, address initDepositToken, uint256 initDeposit) external payable {
    ...
    pool.initialize(poolConfig.initPolicy, poolConfig.poolAdmin, treasury);
    treasury.initialize(address(pool), poolConfig.poolAdmin);
    _deployCalculate(initDepositToken, initDeposit, address(treasury));
    emit PoolCreated(poolConfig.poolName, poolConfig.poolAdmin, address(pool), address(treasury));
  }
...
contract PoolLogic is ReentrancyGuardUpgradeable, PoolPolicy, BoundNFTManager, ERC1155Holder, ERC721Holder {
  ...
  struct Loan {
    ...
    uint256 principal;
    GenericTokenInterface.Collection collateralCollection;
  ...
  function initialize(InitPolicy calldata initPolicy, address poolAdmin, TreasuryLogic _treasury) external initializer {
    _transferOwnership(poolAdmin);
    _setDebtToken(initPolicy.debtToken);
    _setPoolwidePolicy(initPolicy.poolwidePolicy);
    _setCollateralsBatch(initPolicy.collaterals, initPolicy.collateralwidePolicy);
    treasury = _treasury;
    frozen = false;
  }
