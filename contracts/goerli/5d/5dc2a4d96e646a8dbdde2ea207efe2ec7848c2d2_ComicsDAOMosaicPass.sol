...
contract ComicsDAOMintingPass is ERC721A, Ownable, SlicerPurchasableConstructor, IERC2981 {
   ...
   *
   */
  function safeTransferFromBatch(address from, address to, uint256[] memory tokenIds) external {
...
/// @title ComicsDAO Mosaic Minting passes
/// @notice Basic {ComicsDAOMintingPass} with a time constraint minting period
/// @author Dom-Mac
/// @author jacopo <[email protected]>

contract ComicsDAOMosaicPass is ComicsDAOMintingPass {
  // =============================================================
  //                          Storage
  // =============================================================

  // Timestamp deadline before which minting is allowed
  ...
  /**
   * @notice Override function containing the requirements for an account to be eligible for the purchase. See {ISlicerPurchasable}.
   */
  function isPurchaseAllowed(
    ...
    // Check if minting period is not expired
  ...
  function onProductPurchase(
    ...
    // Check if minting period is not expired
    if (!isPurchaseAllowed(slicerId, 0, buyer, quantity, '', '')) revert NotAllowed();
  ...
  /**
   * @dev Set deadlineTimestamp, only Owner is allowed
   *
   */
  function setDeadlineTimestamp(uint256 deadlineTimestamp_) external onlyOwner {
    deadlineTimestamp = deadlineTimestamp_;
  }
}
