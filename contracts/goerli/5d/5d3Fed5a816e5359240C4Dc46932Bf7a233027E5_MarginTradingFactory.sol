...
contract MarginTradingFactory is Ownable, IMarginTradingFactory {
    ...
    mapping(address => bool) public ALLOWED_FLASH_LOAN;
    ...
    function addFlashLoanProxy(address _proxy) external onlyOwner {
        ALLOWED_FLASH_LOAN[_proxy] = true;
    }

    function removeFlashLoanProxy(address _oldProxy) external onlyOwner {
        ALLOWED_FLASH_LOAN[_oldProxy] = false;
    }

    function isAllowedProxy(address _proxy) external view returns (bool) {
        return ALLOWED_FLASH_LOAN[_proxy];
    }
    ...
    function setTokenOracleFeasible(address token, bool isAvailable)
        external
        onlyOwner
    {
    ...
    /// @param token The token address
    function isFeasible(address token) external view returns (bool) {
    ...
    IFlashLoanReceiver
{
    struct Close {
        ...
        // 做空 eth 借 1ETH
        // 保证金 DAI
        // 止盈 eth 跌 swap 后 dai数量
        // 止损 eth 涨 swap 后 dai数量
    ...
    function _proxyClose(
        ...
        require(
            _closeType == 4 && _tokenOraclePrice > _proxyClose.priceOracle,
            "OraclePrice is not reached"
        );
        if (_swapParams.length > 0) {
            // approve to swap route
