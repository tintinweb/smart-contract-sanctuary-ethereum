...
interface IMosaicsToken is IERC721A {
...
contract MosaicsToken is IMosaicsToken, ERC721A, Ownable {
    ...
    // The base URI for mosaics
    string private _baseTokenURI;

    // The default URI for mosaics
    string public defaultMosaicURI;
    ...
    modifier onlyAdmin() {
        require(msg.sender == admin, 'MosaicsToken: Sender is not admin');
        _;
    }
    ...
    */
     ...
     */
    function setMosaicURI(uint256 tokenId, string memory mosaicURI) external onlyAdmin {
        require(_exists(tokenId), 'MosaicsToken: This is a non-existent Mosaic token');
        _tokenURIs[tokenId] = mosaicURI;
    }

    /**
     * @notice Set the base URI for mosaics.
     * @dev Only callable by the owner.
     */
     ...
     * @dev Only callable by the owner.
     */
    function setDefaultMosaicURI(string memory _defaultMosaicURI) external onlyAdmin {
        defaultMosaicURI = _defaultMosaicURI;
    }
    ...
    function _mintTo(address to, uint256 mosaicId) internal returns (uint256) {
        _tokenURIs[mosaicId] = defaultMosaicURI;
