...
contract SLnotifyStaking is Ownable, ReentrancyGuard, Pausable {
    ...
    uint256 public rewardsPerHour = 1250000000000000000;
    uint256 public rewardsBurnTax = 10;
    uint256 public rewardsVaultTax = 10;
    uint256 constant LOCK_TIME = 86400; // 1 day lock time
...
function claimRewards() public nonReentrant whenNotPaused {
    Staker storage staker = stakers[msg.sender];
    require(staker.lockTime == 0 || block.timestamp >= staker.lockTime, "Token is still locked"); // check if token is locked
    uint256 unclaimedRewards = staker.unclaimedRewards;
    require(unclaimedRewards > 0, "You have no unclaimed rewards!"); // check if user has unclaimed rewards
...
function availableRewards(address staker) public view returns (uint256) {
    ...
    // If the staker has no tokens staked, return 0
    if (staker.amountStaked == 0) {
        return 0;
    }

    // Calculate the minimum lock time for all of the staker's tokens
    uint256 minLockTime = staker.lockTime;
    for (uint256 i = 0; i < staker.amountStaked; i++) {
        uint256 tokenId = staker.stakedTokens[i].tokenId;
        uint256 tokenLockTime = stakerAddress[tokenId] == msg.sender ? 0 : LOCK_TIME;
        if (staker.lockTime < block.timestamp + tokenLockTime) {
            minLockTime = staker.lockTime;
        } else {
            minLockTime = block.timestamp + tokenLockTime;
        }
    }

    // Calculate the remaining lock time for the staker's tokens
    uint256 remainingLockTime = 0;
    if (block.timestamp < minLockTime) {
        remainingLockTime = minLockTime - block.timestamp;
    }
    ...
    returns (uint256 _rewards)
{
    ...
    uint256 stakedAmount = stakers[_staker].amountStaked;
    uint256 rewardsRate = rewardsPerHour / 3600;
