...
contract IncomeRouter is Ownable {
    ...
    function withdrawIncome(address recipient) external onlyOwner {
        _checkPoolBalance(s_routerState.owedBalance);
    ...
    function getOwnerWithdrawableAmount() public view returns (uint256) {
        return
            IERC20(i_token).balanceOf(address(this)) -
            s_routerState.owedBalance;
    }
...
contract Pool is IPool, ERC721, Ownable2Step {
    ...
    /// @notice This error is thrown when an address is found
    /// to not be the income router's owner
    error NotRouterOwner();
    ...
    address[] private s_routers;

    /// @notice Mapping between income routers to their owners
    mapping(address => address) private s_routerOwners;
    ...
    /// @inheritdoc IPool
    function createRouter(IERC20 token) external override returns (address) {
        IncomeRouter router = new IncomeRouter(token);
        s_routerOwners[address(router)] = msg.sender;
    ...
    /// @inheritdoc IPool
    function setVerifier(bytes32 source, address verifier) external onlyOwner {
    ...
    function requestToMint(
        ...
        if (s_routerOwners[routerAddr] != msg.sender) revert NotRouterOwner();
    ...
    function mint(
        ...
        address routerOwner = s_routerOwners[routerAddr];
        if (routerOwner == address(0)) revert RouterNotFound();

        // Need to check this as mint will be called by the verifier
        if (owner != routerOwner) revert NotRouterOwner();

        uint256 nfrId = s_numNFRs;
        _safeMint(routerOwner, nfrId);
        s_numNFRs++;
        s_routers.push(routerAddr);
    ...
    /// @inheritdoc IPool
    function withdrawFromLatestNFR(address routerAddr) external override {
        if (routerAddr == address(0)) revert ZeroAddress();
        IncomeRouter(routerAddr).withdrawFromLatestNFR();
    }

    /// @inheritdoc IPool
    function withdrawFromIncomeRouter(address routerAddr) external override {
        if (routerAddr == address(0)) revert ZeroAddress();
        if (s_routerOwners[routerAddr] != msg.sender) revert NotRouterOwner();
        IncomeRouter(routerAddr).withdrawIncome(msg.sender);
    }
    ...
    function getRouterOwner(
        ...
        return s_routerOwners[routerAddress];
    }
    ...
    function tokenURI(
        ...
        address router = s_routers[nfrId];
        ...
        bytes memory meta;
        {
            meta = bytes(
                abi.encodePacked(
                    ...
                    name(),
                    '",',
                    '"description":"NFR Loan","image": "https://gateway.pinata.cloud/ipfs/QmeAKDXvQyGUdvwRSvazCyj4CYeN6qrcpQr4Lmgf7Cc2UC", "attributes": '
                )
            );
        }

        bytes memory attributesOne;
        {
            attributesOne = bytes(
                abi.encodePacked(
                    "[",
                    '{"trait_type": "Balance", "display_type": "number",',
                    '"value":"',
                    Strings.toString(nfr.balance),
                    '"},',
                    '{"trait_type": "Maturity Date","display_type": "date",',
                    '"value":"',
        ...
        bytes memory attributesTwo;
        {
            attributesTwo = bytes(
                abi.encodePacked(
                    ...
                    '"},',
                    '{"trait_type": "Face Value", "display_type": "number",',
                    '"value":"',
                    Strings.toString(nfr.faceValue),
                    '"}',
                    "]"
                )
            );
