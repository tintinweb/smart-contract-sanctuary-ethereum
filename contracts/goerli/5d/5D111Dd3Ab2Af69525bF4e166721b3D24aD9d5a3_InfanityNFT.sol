...
contract InfanityNFT is ERC721A, IInfanityERC721, AccessControl  {
    ...
    mapping(uint256 => uint256) brandsTracker;
    ...
    constructor() ERC721A("InfanityNFT", "INFT") {
        _setupRole(DEFAULT_ADMIN_ROLE, _msgSender());
    }

    /// @dev Mint NFT upto nth amount of token
    /// @param to The address to send the minted token
    /// @param quantity The number of token to mint
    function mint(address to, uint256 quantity) external override onlyHasRole(MINTER_ROLE) {
        _safeMint(to, quantity);
    }
