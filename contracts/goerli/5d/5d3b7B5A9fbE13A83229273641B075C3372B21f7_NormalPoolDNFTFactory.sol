...
contract NormalPoolDNFTFactory is NoDelegateCall {
    ...
    function addWhiteListAddress(address newWLAddress) public {
        require(msg.sender == whiteListUpdater, "Update failed. Please use update account.");
    ...
    function createDerivativeContract(
        ...
        require(
            getContractAddress[tokenAddress] == address(0),
            "already exist"
        );
    ...
    function getNFTPoolInfo()
        public
        view
        returns (NFTPoolInfo[] memory)
    {
        NFTPoolInfo[]
        memory result = new NFTPoolInfo[](NFTPoolInfoArray.length);
        for(uint256 i = 0; i < NFTPoolInfoArray.length; i++) {
            result[i] = NFTPoolInfo(NFTPoolInfoArray[i].originalContractAddress,
                NFTPoolInfoArray[i].derivativeContractAddress,
                NormalPoolNFT(NFTPoolInfoArray[i].derivativeContractAddress).totalNFTStakedInPool(),
                NormalPoolNFT(NFTPoolInfoArray[i].derivativeContractAddress).highestOffer(),
                NormalPoolNFT(NFTPoolInfoArray[i].derivativeContractAddress).highestOffer() *
                    NormalPoolNFT(NFTPoolInfoArray[i].derivativeContractAddress).taxNumerator() /
                    NormalPoolNFT(NFTPoolInfoArray[i].derivativeContractAddress).taxDenominator() /
                    1 days,
                NormalPoolNFT(NFTPoolInfoArray[i].derivativeContractAddress).getCounterValue()
            );
        }
        return result;
    }
