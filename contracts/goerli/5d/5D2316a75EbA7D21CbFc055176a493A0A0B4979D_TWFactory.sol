...
contract TWFactory is Multicall, ERC2771Context, AccessControlEnumerable, IContractFactory {
    ...
    // @dev Deploy conforming to the IContractFactory interface.
    function deployContract(address _implementation, bytes memory _data) external override returns (address) {
        bytes32 salthash = keccak256(abi.encodePacked(_msgSender(), block.number));
        return deployProxyByImplementation(_implementation, _data, salthash);
    }
...
interface IContractFactory {
     ...
     */
    function deployContract(address implementation, bytes memory data) external returns (address);
}
