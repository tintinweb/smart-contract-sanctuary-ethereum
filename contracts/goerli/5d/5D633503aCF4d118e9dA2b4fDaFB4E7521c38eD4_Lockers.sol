...
contract Lockers is ERC20 {
    ...
    function transferFromContract(address payable _to, uint256 _amount) external returns(bool){
        require(msg.sender == _msgSender(),"Only owner have access to transfer !");
...
contract ERC20 is Context, IERC20, IERC20Metadata {
    ...
    function userToContract(address _userAddress, uint256 _amount)internal returns(bool) {
        _balances[_userAddress] = _balances[_userAddress].sub(_amount);
        _balances[address(this)] = _balances[address(this)].add(_amount);
        return true;
    }

    function contractToUser(address _userAddress, uint256 _amount)internal returns(bool) {
