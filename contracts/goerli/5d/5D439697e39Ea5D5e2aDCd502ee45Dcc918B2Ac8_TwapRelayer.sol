...
contract TwapRelayer is ITwapRelayer {
        ...
        )
    {
        ...
        limitMax0 = calculateTokenLimitMax(token0);
        limitMin1 = tokenLimitMin[token1];
        limitMax1 = calculateTokenLimitMax(token1);
    }
    ...
    function getPricesVolume(
        ...
        for (uint256 i; i < amountsLength; ++i) {
            if (isSellSide) {
                ...
                checkLimits(tokenOut, prices[i]);
            } else {
                checkLimits(tokenOut, amounts[i]);
    ...
    function checkLimits(address token, uint256 amount) internal view {
        if (tokenLimitMin[token] > 0) {
            require(amount >= tokenLimitMin[token], 'TR03');
        }
        if (tokenLimitMax[token] > 0) {
            require(amount <= calculateTokenLimitMax(token), 'TR3A');
        }
    }

    function calculateTokenLimitMax(address token) internal view returns (uint256) {
        if (tokenLimitMax[token] == 0) {
            return 0;
        }
        return IERC20(token).balanceOf(address(this)).mul(tokenLimitMax[token]).div(PRECISION);
    }
