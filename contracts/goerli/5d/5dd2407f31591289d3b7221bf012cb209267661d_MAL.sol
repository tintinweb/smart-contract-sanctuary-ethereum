    ...
    Ownable
{
    ...
    constructor() ERC721("Moon Ape Lab Pass", "MALPASS") {
        ...
        team_addresses[
            total_teams
        ] = 0x5966932Ae92fbE75280e0371cb3bC27B558115A8;
        teams[address(0x5966932Ae92fbE75280e0371cb3bC27B558115A8)] = Team(
            0,
            30,
            "Pvl"
        );
        ...
        team_addresses[
            total_teams
        ] = 0xA45845299bD26af707D0F3C902519b41D8aAefca;
        teams[address(0xA45845299bD26af707D0F3C902519b41D8aAefca)] = Team(
            0,
            5,
            "Nkt"
        );
        ...
        team_addresses[
            total_teams
        ] = 0x77FeB62f865365f4b81ef49901BC012d017509F3;
        teams[address(0x77FeB62f865365f4b81ef49901BC012d017509F3)] = Team(
            0,
            20,
            "Sts"
        );
        ...
        team_addresses[
            total_teams
        ] = 0xc2a6035fB9200446f154cdce00EA6ff0160C2854;
        teams[address(0xc2a6035fB9200446f154cdce00EA6ff0160C2854)] = Team(
            0,
            20,
            "Vld"
        );
        ...
        team_addresses[
            total_teams
        ] = 0xd44ac9eB549DCF1D47D47d81398182265C312232;
        teams[address(0xd44ac9eB549DCF1D47D47d81398182265C312232)] = Team(
            0,
            5,
            "Nck"
        );
        ...
        team_addresses[
            total_teams
        ] = 0x1eAC73484bcf66564D896213E06d6c5D013244A4;
        teams[address(0x1eAC73484bcf66564D896213E06d6c5D013244A4)] = Team(
            0,
            20,
            "Andr"
        );
    ...
    function reservePasses(
        ...
        require(
            _pass_type == 1 || _pass_type == 2 || _pass_type == 3,
            "Invalid pass type to be minted. Allowed are: 1=Green, 2=Blue, 3=Gold"
        );
        if (_pass_type == 1) {
            require(
                MINTED_GREEN + quantity < MAX_SUPPLY_GREEN + 1,
                "Too many passes are to be created."
            );
        } else if (_pass_type == 2) {
            require(
                MINTED_BLUE + quantity < MAX_SUPPLY_BLUE + 1,
                "Too many passes are to be created."
            );
        } else {
            require(
                MINTED_GOLD + quantity < MAX_SUPPLY_GOLD + 1,
                "Too many passes are to be created."
            );
        }
    ...
    function myBalance(uint256 _pass_type) public view returns (uint256) {
        require(
            _pass_type == 1 || _pass_type == 2 || _pass_type == 3,
            "Invalid pass type. Allowed are: 1=Green, 2=Blue, 3=Gold"
        );
        if (_pass_type == 1) {
    ...
    function get_pass(
        ...
        if (_pass_type == 1) {
            require(
                quantity * PASS_PRICE_GREEN == msg.value,
                "Ether value sent is incorrect."
            );
            require(
                MINTED_GREEN + quantity < MAX_SUPPLY_GREEN - RESERVED_GREEN + 1,
                "Too many Green Passes are to be minted."
            );
            require(
                quantity + OWNERS_GREEN[msg.sender] <=
                    MAX_FOR_ONE_ADDRESS_GREEN,
                "Maximum quantity of Green Passes to buy from 1 address is 20."
            );
        } else if (_pass_type == 2) {
            require(
                quantity * PASS_PRICE_BLUE == msg.value,
                "Ether value sent is incorrect."
            );
            require(
                MINTED_BLUE + quantity < MAX_SUPPLY_BLUE + 1,
                "Too many Blue Passes are to be minted."
            );
            require(
                quantity + OWNERS_BLUE[msg.sender] <= MAX_FOR_ONE_ADDRESS_BLUE,
                "Maximum quantity of Blue Passes to buy from 1 address is 4."
            );
        } else {
            require(
                quantity * PASS_PRICE_GOLD == msg.value,
                "Ether value sent is incorrect."
            );
            require(
                MINTED_GOLD + quantity < MAX_SUPPLY_GOLD + 1,
                "Too many Gold Passes are to be minted."
            );
            require(
                quantity + OWNERS_GOLD[msg.sender] <= MAX_FOR_ONE_ADDRESS_GOLD,
                "Maximum quantity of Gold Passes to buy from 1 address is 1."
            );
        }
        ...
        for (uint256 i = 0; i < total_teams; i++) {
            teams[team_addresses[i]].balance +=
                (msg.value * teams[team_addresses[i]].share) /
                100;
        }
    }
    ...
    function team_get_percentage(
        address my_address
    ) public view returns (uint256) {
        return teams[my_address].share;
    }

    function team_get_share_balance(
        address my_address
    ) public view returns (uint256) {
    ...
    function burn(uint256 tokenId) public override {
        require(
            _isApprovedOrOwner(_msgSender(), tokenId) ||
                msg.sender == APE_CONTRACT,
            "Caller is not approved/owner nor the Ape contract"
        );
        _burn(tokenId);
    }
...
contract MAL is ERC721, ERC721URIStorage, Ownable {
    ...
    constructor() ERC721("Moon Ape Lab", "MAL") {
        ...
        teams[address(0x84629C04D07FC89220E7Dc970a1Fb127bCd86f47)] = Team(
            0,
            50,
            "Ph"
        );
        ...
        teams[address(0xC8e40F13434f1dFA4072382d15f8a00adD0ba481)] = Team(
            0,
            38,
            "StsVld"
        );
    ...
    function transformToAddressesWithNum(
        address[] memory ape_owners,
        uint8[] memory qs
    ) external onlyOwner {
        // Passes to Apes
    ...
    function mintApeForWhitelisted(uint256 number_of_tokens) external payable {
        require(
            whitelist_sale == true,
            "Public sale is active. Please use function mintApe(number_of_tokens)"
        );
        require(MINTING_PAUSED == false, "Sorry! Minting is currently paused.");
        require(msg.sender != address(0) && msg.sender != address(this));
        require(
            APE_PRICE * number_of_tokens <= msg.value,
            "Ether value sent is not correct"
        );
        require(number_of_tokens > 0, "Please mint at least 1 Ape");
        require(
            TOTAL_SUPPLY + number_of_tokens < MAX_SUPPLY + 2,
            "Too many tokens are to be created. Max supply is 8000"
        );
        require(
            number_of_tokens < whitelisted[msg.sender],
            "Whitelisted users can purchase up to 6 NFTs. Either you are not whitelisted or you have already bought the maximum amount."
        );
    ...
    function mintApe(uint256 number_of_tokens) external payable {
        require(
            whitelist_sale == false,
            "Public sale is not active. Please try again later"
        );
        require(MINTING_PAUSED == false, "Sorry! Minting is currently paused.");
        require(msg.sender != address(0) && msg.sender != address(this));
        require(
        ...
        require(
            number_of_tokens + balanceOf(msg.sender) <= 20,
            "1 address can hold max 20 NFTs"
        );
        require(number_of_tokens > 0, "Please mint at least 1 Ape");
