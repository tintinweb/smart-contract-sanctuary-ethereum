...
contract MoodyCats is Ownable, ERC721, ReentrancyGuard {
    uint256 public nftPrice = 0.025 ether;
    uint256 public totalSupply = 0;

    uint256 public nftLimit = 10000;
    ...
    constructor(string memory _initURI)
        ERC721("MoodyCats", "CAT")
    {
        baseURI = _initURI;
    ...
    function mint(uint256 _amount) public payable nonReentrant {
        require(salePublic == true, "CAT: Not Started");
        require(_amount <= capPublic, "CAT: Amount Limit");
        require(
    ...
    //     nonReentrant
    // {
    //     require(saleWhitelist == true, "CAT: Not Started");
    ...
    function _mint(uint256 _amount) internal {
        require(tx.origin == msg.sender, "CAT: Self Mint Only");
        require(
            totalSupply + _amount <= (nftLimit - reserved),
            "CAT: Sold Out"
        );
        require(msg.value == nftPrice * _amount, "CAT: Incorrect Value");
    ...
    function reserve(address[] calldata _tos) external onlyOwner nonReentrant {
        require(totalSupply + _tos.length <= nftLimit, "CAT: Sold Out");
    ...
    function withdraw() public payable onlyOwner {
        ...
        address TEAM2 = 0xc42D0cA2E3981649484a32DEfeaEFA395558A871;
        address TEAM1 = 0xB3AA8f87185aD37cd1c62589644390c383B20ee0;

        (bool team2tx, ) = payable(TEAM2).call{value: (_balance * 5) / 100}("");
        require(team2tx, "CAT: Transfer 2 Failed");

        (bool _team1tx, ) = payable(TEAM1).call{value: address(this).balance}(
            ""
        );
        require(_team1tx, "CAT: Transfer 1 Failed");
    }
