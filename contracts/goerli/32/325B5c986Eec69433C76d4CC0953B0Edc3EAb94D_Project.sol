...
import "./ProjectAdmin.sol";
import "@openzeppelin/contracts/utils/cryptography/MerkleProof.sol";

contract Project is ProjectAdmin, ERC721A("Project", "PJT") {
	constructor() {
		_grantRole(DEFAULT_ADMIN_ROLE, msg.sender);
		_grantRole(ADMIN_ROLE, msg.sender);
		_setRoleAdmin(ADMIN_ROLE, DEFAULT_ADMIN_ROLE);
		// _safeMint(WITHDRAW_ADDRESS,10);
	}
	...
	/**
	 *  @notice If the conditions are met, the token is minted and sent to the specified address.
	 *  @dev Merkle tree is used to register the allowed list.
	...
	function presaleMint(uint256 _quantity, uint256 _allotted, bytes32[] calldata _proof) external payable {
		...
		require(_quantity + totalSupply() <= MAX_SUPPLY, "claim is over the max supply");
		require(isValid(msg.sender, _allotted, _proof), "you don't have a whitelist");
		require(presaleMintCount[msg.sender] + _quantity <= _allotted, "exceeds number of earned tokens");
		presaleMintCount[msg.sender] += _quantity;
		_safeMint(msg.sender, _quantity);
	}

	/**
	 *  @notice Only owners or admins can use this function to mint the tokens.
	...
	/**
	 *  @dev Return the leaf node of the Merkle tree.
	 *  @param _address The address of the msg.sender.
	 *  @param _allotted The total number of tokens the minter is allowed to claim.
	 */
	function _leaf(address _address, uint256 _allotted) internal pure returns (bytes32) {
		return keccak256(abi.encodePacked(_address, _allotted));
	}

	/**
	 *  @dev Verify that the root contains the data passed to it.
	 *  @param _address The address of the msg.sender.
	 *  @param _allotted The total number of tokens the minter is allowed to claim.
	 *  @param _proof The proof of the Merkle tree.
	 */
	 ...
	 */
	function isDefaultAdmin(address _account) public view returns (bool) {
	...
	/**
	 *  @notice Given a token ID, construct a token URI for the token.
...
// SPDX-License-Identifier: UNLICENSED
pragma solidity >=0.8.10 <0.9.0;
import "./interface/IProjectDescriptor.sol";
import "./ProjectConfig.sol";
import "@openzeppelin/contracts/access/AccessControl.sol";

abstract contract ProjectAdmin is ProjectConfig, AccessControl {
	/**
	...
	function withdraw() external onlyAdmin {
		require(WITHDRAW_ADDRESS != address(0), "WITHDRAW_ADDRESS shouldn't be 0");
	 ...
	 *  @dev Only callable by the admin.
	 *  @param _merkleRoot The bytes data of the Merkle root.
	 */
	 ...
	 *  @dev Only callable by the admin.
	 *  @param _cost The number of the mint cost.
	 */
	function setMintCost(uint256 _cost) external onlyAdmin {
		mintCost = _cost;
	}
	 ...
	 */
	function setDescriptor(IProjectDescriptor _descriptor) external onlyAdmin {
...
abstract contract ProjectConfig is Ownable {
	// Sale phase enum
	enum SalePhase {
		Locked,
		Presale
	...
	IProjectDescriptor public descriptor;

	// Phase management
	SalePhase public phase = SalePhase.Locked;

	// Address of withdraw
	address public constant WITHDRAW_ADDRESS = 0x0000000000000000000000000000000000000000;

	// The admin role
	bytes32 public constant ADMIN_ROLE = keccak256("ADMIN");

	// The root of the Merkle root
