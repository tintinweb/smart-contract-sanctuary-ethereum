...
contract Felix is ERC1155RS, Ownable, ReentrancyGuard, DefaultOperatorFilterer {
...
// ================== Mint Functions Start =======================
    ...
    function whitelistMint(bytes32[] calldata _merkleProof, uint256 _amount) external payable{
        ...
        require(msg.value * _amount >= generations[GENERATION].whitelistPrice, "invalid value sent");
        
        require(generations[GENERATION].status == SaleStatus.Whitelist, "whitelist not started");

        bytes32 node = keccak256(abi.encodePacked(GENERATION, msg.sender));
        require(
            MerkleProof.verify(_merkleProof, generations[GENERATION].whitelistmerkleRoot, node),
            "invalid proof"
        );
    ...
    function collabMint(bytes32[] calldata _merkleProof, uint256 _amount) external payable{
        require(tx.origin == msg.sender, "only eoa");
        require(mintedPerWallet[GENERATION][msg.sender] + _amount <= generations[GENERATION].walletLimit, "wallet limit reached");
        require(msg.value * _amount >= generations[GENERATION].collabPrice, "invalid value sent");
        
        require(generations[GENERATION].status == SaleStatus.Collab, "collab not started");

        bytes32 node = keccak256(abi.encodePacked(GENERATION, msg.sender));
        require(
            MerkleProof.verify(_merkleProof, generations[GENERATION].collabmerkleRoot, node),
            "invalid proof"
        );
    ...
    function waitlistMint(bytes32[] calldata _merkleProof, uint256 _amount) external payable{
        require(tx.origin == msg.sender, "only eoa");
        require(mintedPerWallet[GENERATION][msg.sender] + _amount <= generations[GENERATION].walletLimit, "wallet limit reached");
        require(msg.value * _amount >= generations[GENERATION].waitlistPrice, "invalid value sent");
        
        require(generations[GENERATION].status == SaleStatus.Waitlist, "waitlist not started");

        bytes32 node = keccak256(abi.encodePacked(GENERATION, msg.sender));
        require(
            MerkleProof.verify(_merkleProof, generations[GENERATION].waitlistmerkleRoot, node),
            "invalid proof"
        );
