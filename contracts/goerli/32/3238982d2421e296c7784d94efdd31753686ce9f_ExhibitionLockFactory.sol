...
contract ExhibitionLockFactory is Ownable{
    ...
    event CreatedExhibition(string indexed exhibitor, string exhibitionName, string exhibitioLocation, address indexed exhibition,  uint256 indexed id);
    ...
    function createExhibitionLock(
        ...
        ExhibitionLock(exhibition).initialize(_exhibitorAddress, exhibitorNames[_exhibitorAddress],_exhibitionName,_exhibitioLocation, _exhibitioStart, _exhibitioEnd );

        allExhibitions.push(exhibition);


        emit CreatedExhibition(exhibitorNames[_exhibitorAddress], _exhibitionName,  _exhibitioLocation, exhibition,  totalExhibitions);
...
contract ExhibitionLock is IERC721Receiver{
    ...
    event DepositedERC721(string indexed exhibitor, string indexed exhibitionName, string exhibitioLocation, address indexed tokenAddress,  uint256 tokenId, address owner);
    event WithdrawnERC721(string indexed exhibitor, string indexed exhibitionName, string exhibitioLocation, address indexed tokenAddress,  uint256 tokenId, address owner);
    event TokensApproved(address indexed token, uint256 indexed tokenId);
    event ExhibitionApproved(string indexed exhibitor, string indexed exhibitionName, string exhibitioLocation, address indexed exhibitorAddress,  uint256 exhibitioStart, uint256 exhibitioEnd);
    ...
    function approveTokens(address[] memory _tokens, uint256[] memory _tokenIds) public {
        ...
        for(uint8 i = 0; i< _tokens.length; i++){
            ...
            emit TokensApproved(_tokens[i],_tokenIds[i]);
        }
    ...
    function approveExhibition() public returns(bool){
        ...
        emit ExhibitionApproved(exhibitor, exhibitionName, exhibitioLocation, exhibitorAddress, exhibitioStart, exhibitioEnd);
    ...
    function depositERC721(IERC721 _token, uint256 _tokenId) public returns (bool) {
       ...
       emit DepositedERC721(exhibitor, exhibitionName, exhibitioLocation, address(_token), _tokenId, msg.sender);
       return true;
    }
    ...
    function withdrawERC721(IERC721 _token, uint256 _tokenId) public returns (bool) {
       ...
       emit WithdrawnERC721(exhibitor, exhibitionName, exhibitioLocation, address(_token), _tokenId, owner);
       return true;
    }
