...
contract ERC721StakingVaultFactory is Ownable {
    ...
    function deploy(
        ...
        ERC721StakingVault _contract = new ERC721StakingVault(_softStaking, _hardStaking, msg.sender);
...
contract ERC721StakingVault is Ownable, IERC721Receiver, ReentrancyGuard {
    ...
    constructor(bool _softStaking, bool _hardStaking, address _owner) {
    ...
    function getConfiguration(uint8 _type) external view returns (bool) {
        return config[_type];
    }

    function setRewardContract(address _address) external onlyOwner {
    ...
    function getHardStaked(uint8 _type, address _contract) external view returns (address[] memory) {
    ...
    function clearDepositTimesOf(address _address, address _contract, uint8 _type) external {
        require(ERC721Rewards(rewardContract).approvedCollection(_contract), "Bad collection");
        require(msg.sender == rewardContract, "Not authorized");
        uint256[] memory _tokenIds = depositsOf(_address, _contract, _type);
        for (uint256 i; i < _tokenIds.length; i++) {
            if (_type == SOFT_STAKED) {
                softDepositTimes[_contract][_tokenIds[i]] = block.timestamp;
            } else {
                hardDepositTimes[_contract][_address][_tokenIds[i]] = block.timestamp;
            }
        }
    }
