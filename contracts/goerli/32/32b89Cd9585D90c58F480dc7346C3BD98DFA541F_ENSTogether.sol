...
contract ENSTogether is ReentrancyGuard, Ownable {
   ...
   function getUnited( address _from , address _to, string calldata ens1, string calldata ens2)  internal {
        Union memory engaged = unionWith[msg.sender];
        engaged.proposalStatus = uint8(Proposal.ACCEPTED);
        engaged.relationshipStatus = uint8(Status.TOGETHER);
        engaged.createdAt = block.timestamp;
        engaged.registryNumber = registryCounter;
        unionWith[_to] = engaged;
        unionWith[_from] = engaged;
        registryCounter++;
