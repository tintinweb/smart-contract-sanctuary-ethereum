...
import "./SmashMarblesArenaV2.sol";

contract SmashMarblesArenaV3 is SmashMarblesArenaV2 {
        ...
        isblacklistedAddresses
    {
        ...
        require(
            (noOfTokens + tokenCounter-1000+marbleForcliam) <= maxNFTs,
    ...
    function claimNft(uint256 _id) external virtual override isblacklistedAddresses {
        require(
            publicSale == true,
        ...
        require(
            marbleForcliam>0,
            "SmashMarblesArenaV1: marble Not avalible for claim"
        );
        require(
        ...
        onlyOwner
    {
        ...
        delete tokenArray;
        require(
            (_addresses.length + tokenCounter-1000+marbleForcliam) <= maxNFTs,
            "SmashMarblesArenaV1: NFTs not available for air drop"
        );
...
import "./SmashMarblesArenaV1.sol";

contract SmashMarblesArenaV2 is SmashMarblesArenaV1 {
    using StringsUpgradeable for uint256;
    
    uint256 public marbleForcliam;
     ...
     */
    function changeMarbleNFT(uint256 _value,uint256 _marbleForClaim) external virtual onlyOwner {
        maxNFTs = _value;
        marbleForcliam=_marbleForClaim;
    }
        ...
        isblacklistedAddresses
    {
        ...
        require(
            (noOfTokens + tokenCounter-500+marbleForcliam) <= maxNFTs,
            "SmashMarblesArenaV1: NFTs not available for minting"
        );
        ...
        onlyOwner
    {
        ...
        delete tokenArray;
        require(
            (_addresses.length + tokenCounter-500+marbleForcliam) <= maxNFTs,
            "SmashMarblesArenaV1: NFTs not available for air drop"
        );
        for (uint256 i = 0; i < _addresses.length; i++) {
            tokenCounter += 1;
