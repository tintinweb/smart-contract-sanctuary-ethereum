...
contract ExchangeCore is ReentrancyGuarded, StaticCaller, EIP712 {
    ...
    struct Order {
        ...
        /* Order maker address. */
        address maker;
        /* Order verifier target. */
        address verifierTarget;
        /* Order verifier selector. */
        bytes4 verifierSelector;
        /* Order verifier extradata. */
        bytes verifierExtradata;
    ...
    bytes32 constant ORDER_TYPEHASH = keccak256(
        "Order(address registry,address maker,address verifierTarget,bytes4 verifierSelector,bytes verifierExtradata,uint256 maximumFill,uint256 listingTime,uint256 expirationTime,uint256 salt)"
    );
    ...
    event OrderApproved     (bytes32 indexed hash, address registry, address indexed maker, address verifierTarget, bytes4 verifierSelector, bytes verifierExtradata, uint maximumFill, uint listingTime, uint expirationTime, uint salt, bool orderbookInclusionDesired);
        ...
        returns (bytes32 hash)
    {
        ...
        return keccak256(abi.encode(
            ...
            order.registry,
            order.maker,
            order.verifierTarget,
            order.verifierSelector,
            keccak256(order.verifierExtradata),
            order.maximumFill,
            order.listingTime,
        ...
        returns (bool)
    {
        ...
        /* Order verifier target must exist. */
        if (!exists(order.verifierTarget)) {
            return false;
        }
        ...
        internal
    {
        ...
        /* Log approval event. */
        emit OrderApproved(hash, order.registry, order.maker, order.verifierTarget, order.verifierSelector, order.verifierExtradata, order.maximumFill, order.listingTime, order.expirationTime, order.salt, orderbookInclusionDesired);
    }
