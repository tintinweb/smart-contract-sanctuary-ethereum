    ...
    ReentrancyGuard
{
    ...
    * NOTE: false is USDC, true is USDT
    ...
    function getRefPercentage(bytes32 _ref_code) public view returns (uint256) {
        return REF_DB.getRefInfoAt(_ref_code, 0);
    }
    ...
    /**
     * @dev get referral trade amount
     * @param _ref_code of the user in bytes32
     * @return uint256
     * Note: its a view function
     */
    function getRefBalance(bytes32 _ref_code) public view returns (uint256) {
        return referrals_amount[_ref_code];
    }
}
...
contract ReferralDB is Context, AccessControlEnumerable, Pausable {
    ...
    /**
     * @dev getting the referral info from the index of mapping
     * @param _ref_code of the user in bytes32
     * @param _index which we looking for
     * @return uint256
     */
    function getRefInfoAt(bytes32 _ref_code, uint256 _index)
        public
        view
        returns (uint256)
    {
        return referrals_mapping[_ref_code][_index];
    }

    /**
     * @dev getting the referrals' all info from the mapping
     * @param _ref_code of the user in bytes32
     * @return uint256
     */
    function getRefInfo(bytes32 _ref_code)
        public
        view
        returns (uint256[1000] memory)
    {
        return referrals_mapping[_ref_code];
    }
}