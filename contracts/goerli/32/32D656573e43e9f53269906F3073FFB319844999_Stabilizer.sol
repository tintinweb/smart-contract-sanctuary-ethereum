...
contract Stabilizer is Owned {
  ...
  // This will be moved to the SWEEP contract
  uint public MAX_BORROW = 100*1e18;
  bool public BORROW_ALLOWED = true;

  // USD
  uint public junior_tranche;
  uint public senior_tranche;
  ...
  function deposit(uint amount) external onlyBanker {
    ...
    TransferHelper.safeTransferFrom(address(USD), msg.sender, address(this), amount);
  }
  ...
  function invest() external onlyBanker isOpen {
    ...
    require(banker_amount > 0, "the banker_amount must be greater than 0");
    ...
    uint sweep_to_borrow = SWEEPtoUSD(calculateSenior(banker_amount), sweep_price);
    ...
    junior_tranche += banker_amount;

    if(sweep_to_exchange > 0 && BORROW_ALLOWED){
      ...
      senior_tranche += senior_investment;
    }
    ...
    TransferHelper.safeTransfer(address(USD), address(asset), banker_amount + senior_investment);
    banker_amount = 0;
  }
  ...
  function payback(uint256 amount) external {
    ...
    // get the usd back
    TransferHelper.safeTransferFrom(address(USD), address(asset), address(this), amount);
    ...
    if(sweep_borrowed > 0) {
      ...
      TransferHelper.safeTransfer(address(USD), address(amm), usd_to_exchange);
    ...
    if(amount > 0){
      ...
      TransferHelper.safeTransfer(address(USD), treasury, treasury_amount);
    }
  ...
  // allows the withdrawal of the banker_amount
  ...
  function withdraw() external onlyBanker {
    ...
    require(banker_amount > 0, "nothing to withdraw");

    TransferHelper.safeTransfer(address(this), banker, banker_amount);
    banker_amount = 0;
  }
  ...
  function calculateRatio() internal view returns(uint) {
    return PRECISION * senior_tranche / junior_tranche;
  }
