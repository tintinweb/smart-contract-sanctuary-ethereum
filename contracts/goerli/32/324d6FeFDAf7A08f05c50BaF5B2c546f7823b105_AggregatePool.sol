...
contract AggregatePool is ERC20Permit, TransferNFT, IAggregatePool {
    ...
    modifier unlockable(address _nft, uint256 _id) {
        ...
        require(lockInfo[fId].releaseTime > block.timestamp, "AP: NFT already released");
        _;
    }
    ...
    modifier releasable(address _nft, uint256 _id) {
        ...
        require(lockInfo[fId].releaseTime < block.timestamp, "AP: Release time not yet reached");
        _;
    }
     ...
     */
    function getFurionId(address _nft, uint256 _id) public pure returns (bytes32) {
     ...
     */
    function getLockInfo(address _nft, uint256 _id) public view returns (LockInfo memory) {
        bytes32 fId = getFurionId(_nft, _id);
     ...
     */
    function release(address _nft, uint256 _id) external onlyOwner nftRegistered(_nft) releasable(_nft, _id) {
        bytes32 fId = getFurionId(_nft, _id);
    ...
    function _store(address _nft, uint256 _id, bool _updateNow) private nftRegistered(_nft) {
        uint256 nftPrice = oracle.getNFTPrice(_nft, _id);
        uint256 fftPrice = refPricePerFFT();
    ...
    function _lock(address _nft, uint256 _id, bool _updateNow) private nftRegistered(_nft) returns (uint256) {
        _transferInNFT(_nft, _id);
    ...
    function _buy(address _nft, uint256 _id, bool _updateNow) private nftRegistered(_nft) {
        if (_updateNow) {
            _collectFee(buyFee);
