...
math: constant(address) = 0xB3E6b2e3CB1DAa19D369ebD9E97918cfac104fc4
token: constant(address) = 0x5A9cf8889Ac6C722b9D307ca73CD008392bcd156
...
def tweak_price(
    ...
    if last_prices_timestamp < block.timestamp:
        ...
        for k in range(N_COINS - 1):
            ...
            assert p < PRICE_MASK, "tweak_price assert 1"
            packed_prices = bitwise_or(p, packed_prices)
        self.price_oracle_packed = packed_prices
        self.last_prices_timestamp = block.timestamp
    ...
    for k in range(N_COINS - 1):
        ...
        assert p < PRICE_MASK, "tweak_price assert 2"
        packed_prices = bitwise_or(p, packed_prices)
    self.last_prices_packed = packed_prices
        ...
        if norm > adjustment_step**2 and old_virtual_price > 0:
            ...
            if (old_virtual_price > 10**18) and (
                ...
                for k in range(N_COINS - 1):
                    ...
                    assert p < PRICE_MASK, "tweak_price assert 3"
                    packed_prices = bitwise_or(p, packed_prices)
                self.price_scale_packed = packed_prices
                self.D = D
...
def add_liquidity(amounts: uint256[N_COINS], min_mint_amount: uint256):
    assert not self.is_killed, 'assert 1'  # dev: the pool is killed
        ...
        for i in range(N_COINS):
            if amounts[i] > 0:
                ...
                if ix == INF_COINS:
                    ...
                    ix = INF_COINS - 1
        assert ix != INF_COINS, 'add_liquidity 1'  # dev: no coins to add
    ...
    assert d_token > 0, 'add_liquidity 2'  # dev: nothing minted
