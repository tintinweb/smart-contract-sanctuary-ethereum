    ...
    IBaseCollection
{
    ...
    address private _aiskinFactory;

    mapping(address => bool) internal _allowedOperators;

    function __BaseCollection_init(
        address aiskinFactory_,
        address owner_
    ) internal onlyInitializing {
        __Ownable_init();
        __ERC2981_init();

        initializeRole(owner_);
        // _setDefaultRoyalty(royaltyReceiver_, royaltyFee_);

        _aiskinFactory = aiskinFactory_;
    }

    /// @dev Transfer the ownership to `_owner`
    function initializeRole(address _owner) internal onlyInitializing {
    ...
    function aiskinFactory() public view override returns (address) {
        return _aiskinFactory;
    }

    function supportsInterface(
        bytes4 interfaceId
    ) public view virtual override(ERC2981Upgradeable) returns (bool) {
        return
...
import {BaseCollection} from "./base/BaseCollection.sol";
...
contract DropCollection is BaseCollection, ERC1155Upgradeable, IDropCollection {
    ...
    // token -> sales
    mapping(address => CreatorSale) _creatorSales;
    // token -> fees
    mapping(address => AISkinFee) _aiskinFees;

    modifier onlyMintable(uint256 tokenId, uint256 quantity) {
        Item memory item = _items[tokenId];

        if (item.cap == item.minted) {
            revert SoldOut();
        }

        if (item.minted + quantity > item.cap) {
    ...
    modifier onlyFactory() {
        if (aiskinFactory() != _msgSender()) {
            revert CallerNotAiskinFactory();
        }
        _;
    }
    ...
    function initialize(
        ...
        __ERC1155_init("");
        __BaseCollection_init(aiskinFactory_, owner_);
        _name = name_;
        _symbol = symbol_;
    }
    ...
    function createItem(
        ...
        uint256 priceInWei,
        bool limited
    ) external payable override onlyFactory onlyCreator(creator) {
        _items[tokenId] = Item({
            cap: cap,
        ...
        emit CreateItem(tokenId, address(this), cap, priceInWei, limited);
    }
    ...
    function mintItem(
        ...
        uint256 tokenId,
        uint256 quantity
    ) external payable override onlyMintable(tokenId, quantity) {
        Item memory item = _items[tokenId];
        _items[tokenId].minted = item.minted + quantity;

        uint256 amount = quantity * item.priceInWei;

        if (item.tokenPayment == address(0) && amount != msg.value) {
            revert InvalidMintAmount();
        } else if (item.tokenPayment != address(0)) {
            require(
                ERC20Upgradeable(item.tokenPayment).transferFrom(
                    _msgSender(),
                    address(this),
                    amount
                )
        ...
        uint256 fee = IAISkinFactory(aiskinFactory()).calculateCommission(
            address(this),
            amount
        );

        _creatorSales[item.tokenPayment].totalSales =
            _creatorSales[item.tokenPayment].totalSales +
            (amount - fee);
        _aiskinFees[item.tokenPayment].totalFees =
            _aiskinFees[item.tokenPayment].totalFees +
            fee;
    ...
    function enableLimited(uint256 tokenId) external override onlyOwner {
        if (!_items[tokenId].limited) {
            revert AlreadyLimitedCollection();
        }

        _items[tokenId].limited = true;

        emit EnableLimitedCollection(tokenId);
    }

    function increaseCap(
        uint256 tokenId,
        uint256 cap
    ) external override onlyOwner {
        // should only increase cap if item is not set to limited and
        // cap is not lessthan previous cap
        if (_items[tokenId].limited || _items[tokenId].minted > cap) {
            revert UnableToIncreaseCap();
        }

        emit IncreaseCap(tokenId, _items[tokenId].cap, cap);

        _items[tokenId].cap = cap;
    }
    ...
    function getBaseUri() public view returns (string memory) {
        return IAISkinMasterCollection(_aiskinMasterCollection()).getBaseUri();
    }

    function withdrawCreatorSale(address tokenPayment) external onlyOwner {
        uint256 amount = _getCreatorWithdrawableAmount(tokenPayment);
        require(amount > 0);

        _creatorSales[tokenPayment].totalSalesClaimed =
            _creatorSales[tokenPayment].totalSalesClaimed +
            amount;

        if (tokenPayment == address(0) && address(this).balance >= amount) {
            AddressUpgradeable.sendValue(payable(msg.sender), amount);
        } else if (
            ERC20Upgradeable(tokenPayment).balanceOf(address(this)) >= amount
        ) {
            ERC20Upgradeable(tokenPayment).transfer(msg.sender, amount);
        }
    }

    function withdrawAISkinFee(address tokenPayment) external {
        require(IAISkinFactory(aiskinFactory()).getTreasury() == msg.sender);

        uint256 amount = _getAISkinWithdrawableAmount(tokenPayment);
        require(amount > 0);

        _aiskinFees[tokenPayment].totalFeesClaimed =
            _aiskinFees[tokenPayment].totalFeesClaimed +
            amount;
        if (tokenPayment == address(0) && address(this).balance >= amount) {
            AddressUpgradeable.sendValue(payable(msg.sender), amount);
    ...
    function _getCreatorWithdrawableAmount(
        address tokenPayment
    ) internal view returns (uint256) {
        return
            _creatorSales[tokenPayment].totalSales -
            _creatorSales[tokenPayment].totalSalesClaimed;
    }

    function _getAISkinWithdrawableAmount(
        address tokenPayment
    ) internal view returns (uint256) {
        return
            _aiskinFees[tokenPayment].totalFees -
            _aiskinFees[tokenPayment].totalFeesClaimed;
    }

    function _aiskinMasterCollection() internal view returns (address) {
        return IAISkinFactory(aiskinFactory()).getAiskinMasterCollection();
    }
    ...
    function supportsInterface(
        ...
        view
        virtual
        override(BaseCollection, ERC1155Upgradeable)
        returns (bool)
    {
        return
    ...
    function _afterTokenTransfer(
        ...
        if (ids.length == 1) {
            IAISkinMasterCollection(_aiskinMasterCollection())
                .emitTransferSingle(
                    operator,
                    from, // from is always address(0) for new minted item
                    to,
                    ids[0],
                    amounts[0]
                );
...
interface IAISkinFactory {
    ...
    event SetMasterCollection(address oldAddrs, address newAddrs);

    event SetTreasury(address oldAddrs, address newAddrs);

    event UpdateDefaultShare(uint256 oldShare, uint256 newShare);
    ...
    /**
     * @dev Publish new item either under aiskin master collection or creator's collection. Publishing new item emits {ICollection-CreateItem} on target collection
     *
     * Requirements:
     *  - signature is required to publish new item in a collection.
     *  - signature signer address must have `PUBLISHER_SIGNER` or `FREE_PUBLISHER_SIGNER` roles
     *
     * Note:
     *  - signer with `keccak256(PUBLISHER_SIGNER)` role requires caller/creator to pay fee to publish item
     *  - signer with `keccak256(FREE_PUBLISHER_SIGNER)` role bypasses payment validation
     *
     * @param tokenId The token id
     * @param deadline The signature expiry timestamp
     * @param signature The signer signature to validate
     * @param collection The collection address where to publish the item
     * @param cap The collection cap/supply
    ...
    /**
     * @dev Override aiskin share mint fee on each item sales on created item in a target collection
     *
     * Requirements:
     *  - caller must have `keccak256(MANAGER_ROLE)`
     *
     * @param collection The target collection address
     * @param share The mint share fee
     * @param isOverride If `True` get the fee override else get default aiskin share fee. See {AISkinFactory-getDefaultShareFee}
     */
    function setShareOverride(
        address collection,
        uint256 share,
        bool isOverride
    ) external;
    ...
    /**
     * @dev Update aiskin default item mint share. Emits {IAISkinFactory-UpdateDefaultShare}
     *
     * Requirements:
     *  - caller must have keccak256(MANAGER_ROLE) role
     *
     ...
     */
    function updateDefaultShare(uint256 defaultShare) external;
    ...
    /**
     * @dev Calculate aiskin share mint fee on collection item sales
     *
     * @param collection The target collection
     * @param amount The amount to calculate the aiskin shares mint fee
     *
     * @return Mint fee share
     */
    ...
    /**
     * @dev Getter for aiskin default share fee
     */
    function getDefaultShareFee() external view returns (uint256);
...
interface IAISkinMasterCollection is ICollection {
    ...
    /**
     * @dev Getter for AISkin item info. See {IAISkinMasterCollection-AISkin struct}
     *
     * @param tokenId The item token id
     */
    function getAISkinInfo(
        uint256 tokenId
    ) external view returns (AISkin memory);
    ...
    /**
     * @dev Getter for creator total claimable amount, computed as totalSales - totalSalesClaimed. See {ICollection-CreatorSale} struct.
     *
     * Note: `tokenPayment` is the token address to claim, address(0) to get native token (coin) claimable amount
     *
     * @param creator The creator address
     * @param tokenPayment The token to claim by creator
     */
    function getCreatorClaimableAmount(
        address creator,
        address tokenPayment
    ) external view returns (uint256);

    /**
     * @dev Getter for creator total claimable amount, computed as totalFees - totalFeesClaimed. See {ICollection-AISkinFee} struct.
     *
     * Note: `tokenPayment` is the token address to claim, address(0) to get native token (coin) claimable amount
     *
     * @param tokenPayment The token to claim
     */
    function getAISkinClamaibleFees(
        address tokenPayment
    ) external view returns (uint256);
}
...
interface IBaseCollection {
     ...
     * @param name The contract name. See {ICollection-name}
     * @param symbol The contract symbol. See (ICollection-symbol)
     */
    function initialize(
...
interface ICollection {
    ...
    /**
     * @dev Create item on master collection, Emits {ICollection-CreateItem}.
     *
     * Requires:
    ...
    /**
     * @dev Limit tokenId supply to the current cap. enabling item that already limited will revert {ICollection-AlreadyLimitedCollection}. Emits {ICollection-EnableLimitedCollection}
     *
     * Note: limited item cannot increase supply
     *
     * Requirements:
     *  - tokenId must not limited == True. See {ICollection-Item} struct
     *
     * @param tokenId The token id to limit
     */
    function enableLimited(uint256 tokenId) external;

    /// @dev Increase `cap` of `tokenId`, Note: item with limited == FALSE can increase cap

    /**
     *  @dev Can only increase item that is not limited and new cap must not less than item minted else it will revert {ICollection-UnableToIncreaseCap}.
     *  Emits {ICollection-IncreaseCap}
     *
     * Requirements:
...
interface IDropCollection is ICollection {
    /**
