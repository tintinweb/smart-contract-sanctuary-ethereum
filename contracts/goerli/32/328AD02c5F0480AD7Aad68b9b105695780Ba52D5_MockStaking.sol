...
// File contracts/farm/IERC20Mintable.sol
...
contract MasterChef is SafeOwnable {
  ...
  event Harvest(address harvester, uint amount, uint requestAt);
...
// File contracts/mock/MockStaking.sol
...
contract MockStaking {
  ...
  function deposit(uint amount) external {
    masterChef.processDeposit(msg.sender, amount);
  }

  function withdraw(uint amount) external {
    masterChef.processWithdraw(msg.sender, amount);
  }
}