...
interface IDepositExecute {
    ...
    // TODO: remove selector in production
...
interface IUsdcDepositAndBurn {
    ...
    function depositAndBurnUsdc(bytes32 destChainIdBytes, uint256 amount) external;
}

// SPDX-License-Identifier: MIT
pragma solidity ^0.8.4;

interface IVoyagerUtils {
    function isContractWhitelisted(address contractAddress) external view returns (bool);

    function isBurnable(address tokenAddress) external view returns (bool);

    /// @notice Function to set reserve handler address
    /// @dev Can only be called by the RESOURCE_SETTER
    /// @param reserveAddress Address of reserve handler.
    function setReserve(address reserveAddress) external;

    /// @notice Function to whitelist a token for swaps
    /// @dev Can only be called by the RESOURCE_SETTER
    /// @param contractAddress Address of contract to be called when a deposit is made and a deposited is executed.
    function setResource(address contractAddress, bool isResource) external;
...
import "./interfaces/IVoyagerUtils.sol";
...
contract VoyagerDepositHandler is
    UUPSUpgradeable,
    ContextUpgradeable,
    ...
    IApplication
{
    ...
    // Instance of the voyager utils contract
    IVoyagerUtils public voyagerUtils;
    // Instance of the USDC burner contract
    IUsdcDepositAndBurn public _usdcBurnerContract;

    // address of the Router Bridge contract on Router chain
    string public routerBridge;

    // keccak256(abi.encodePacked(destChainType, keccak256(abi.encode(destChainId)))) => nonce
    mapping(bytes32 => uint64) public depositNonce;

    // keccak256(abi.encodePacked(destChainType, keccak256(abi.encode(destChainId))))
    // => if USDC is burnable and mintable
    mapping(bytes32 => bool) public _isUsdcBurnableMintable;

    // TODO: Remove This in production
    // keccak256(abi.encodePacked(destChainType, keccak256(abi.encode(destChainId)))) => depositNonce
    // => usdcNonce if usdc was burnt else 0
    mapping(bytes32 => mapping(uint64 => uint64)) public _usdcBurnData;

    // keccak256(abi.encodePacked(destChainType, keccak256(abi.encode(destChainId))))
    mapping(bytes32 => bool) public isChainWhitelisted;

    // TODO: Remove this in production
    // keccak256(abi.encodePacked(destChainType, keccak256(abi.encode(destChainId)))) + depositNonce
    // => depositData
    mapping(bytes32 => mapping(uint64 => IDepositExecute.DepositData)) private _depositData;
    ...
    modifier isContractWhitelisted(address _contract) {
        _isContractWhitelisted(_contract);
        _;
    }

    function _isContractWhitelisted(address _contract) private view {
        require(voyagerUtils.isContractWhitelisted(_contract), "provided token not whitelisted");
    }
    ...
    function pause() external virtual onlyRole(PAUSER) whenNotPaused {
        _pause();
    }

    /// @notice Unpauses deposits on the handler.
    /// @notice Only callable by an address that currently has the PAUSER role.
    function unpause() external virtual onlyRole(PAUSER) whenPaused {
        _unpause();
    }

    /// @notice Function to set voyager utils contract
    /// @param  _voyagerUtils address of voyager utils contract
    function setVoyagerUtilsContract(address _voyagerUtils) external onlyRole(RESOURCE_SETTER) {
        voyagerUtils = IVoyagerUtils(_voyagerUtils);
    }
    ...
    function setMinAmountToSwap(address[] memory _tokens, uint256[] memory _amounts)
        external
    ...
    /// @notice Only RESOURCE_SETTER can call this function
    /// @param chainType Array of chain types
    /// @param chainId Array of chain ids
    /// @param shouldWhitelist Array of should whitelist boolean
    function whitelistChains(
        uint32[] memory chainType,
        string[] memory chainId,
        bool[] memory shouldWhitelist
    ) external onlyRole(RESOURCE_SETTER) {
        require(
            chainId.length == chainType.length && chainType.length == shouldWhitelist.length,
            "array length mismatch"
        );

        for (uint16 i = 0; i < chainType.length; i++) {
            bytes32 destChainIdBytes = getChainIdBytes(chainType[i], chainId[i]);
    ...
    /// @return amount of src stable tokens
    function getMinAmountToSwap(address _token) public view returns (uint256) {
        return _minAmountToSwap[_token];
    }

    /// @notice Function to get chain ID bytes
    /// @param  chainType type of chain (evm, solana etc)
    /// @param  chainId chain Id of the chain
    function getChainIdBytes(uint32 chainType, string memory chainId) public pure returns (bytes32) {
        return keccak256(abi.encodePacked(chainType, keccak256(abi.encode(chainId))));
    }
    ...
    /// @notice Set the USDC burner contract for the chain
    /// @notice Only RESOURCE_SETTER can call this function
    /// @param _usdcBurner address of USDC burner contract
    function setUsdcBurnerContract(address _usdcBurner) external onlyRole(RESOURCE_SETTER) {
        _usdcBurnerContract = IUsdcDepositAndBurn(_usdcBurner);
    }

    /// @notice Set if USDC is burnable and mintable for a chain pair
    /// @notice Only RESOURCE_SETTER can call this function
    /// @param _destChainType array of dest chain types
    ...
    function setUsdcBurnableAndMintable(
        uint32[] memory _destChainType,
        string[] memory _destChainID,
        bool[] memory _setTrue
    ) external onlyRole(RESOURCE_SETTER) {
        require(
            _destChainID.length == _destChainType.length && _destChainID.length == _setTrue.length,
            "Array length mismatch"
        );
        for (uint8 i = 0; i < _destChainType.length; i++) {
            bytes32 destChainIdBytes = getChainIdBytes(_destChainType[i], _destChainID[i]);
            require(isChainWhitelisted[destChainIdBytes], "Chain Id != 0");
            _isUsdcBurnableMintable[destChainIdBytes] = _setTrue[i];
        }
    }

    function decodeArbitraryData(bytes memory arbitraryData)
        internal
        pure
        returns (IDepositExecute.ArbitraryInstruction memory)
    {
        IDepositExecute.ArbitraryInstruction memory arbitraryInstruction;
        // TODO: Remove selector from here in production
        (
            arbitraryInstruction.destContractAddress,
            arbitraryInstruction.selector,
            arbitraryInstruction.data,
            arbitraryInstruction.refundAddress,
            arbitraryInstruction.gasLimit,
            arbitraryInstruction.gasPrice
        ) = abi.decode(arbitraryData, (bytes, bytes4, bytes, bytes, uint256, uint256));

        return arbitraryInstruction;
    }

    function decodeReserveOrLpSwapData(bytes memory swapData)
        internal
        pure
        returns (IDepositExecute.ReserveOrLPSwapInfo memory)
    {
    ...
    function decodeNonReserveSwapData(bytes memory swapData)
        internal
        pure
        returns (IDepositExecute.NonReserveSwapInfo memory)
    {
    ...
    function decodeExecuteData(bytes memory executeData)
        internal
        pure
        returns (IDepositExecute.ExecuteSwapInfo memory)
    {
        IDepositExecute.ExecuteSwapInfo memory executeDetails;
    ...
    function convertToWeth(uint256 amount, address tokenPayer) internal {
    ...
    function depositReserveToken(
        ...
        if (isSourceNative) {
            require(msg.value > 0, "No native assets sent");
            convertToWeth(msg.value, msg.sender);
        }
        ...
        uint64 nonce = _depositReserveToken(swapDetails);
        executeDetails.depositNonce = nonce;

        bytes memory payload = abi.encode(0, swapDetails, executeDetails);
        gatewayContract.requestToRouter{ value: msg.value }(payload, routerBridge, 1000000, Utils.FeePayer.APP);
    }
    ...
    function depositReseveTokenAndExecute(
        ...
        if (isSourceNative) {
            require(msg.value > 0, "No native assets sent");
            convertToWeth(msg.value, swapDetails.depositor);
        }

        IDepositExecute.ArbitraryInstruction memory arbitraryInstruction = decodeArbitraryData(arbitraryData);

        executeDetails.depositNonce = _depositReserveToken(swapDetails);

        bytes memory payload = abi.encode(2, msg.sender, swapDetails, executeDetails, arbitraryInstruction);

        gatewayContract.requestToRouter{ value: msg.value }(payload, routerBridge, 1000000, Utils.FeePayer.APP);
    }

    function _depositReserveToken(IDepositExecute.ReserveOrLPSwapInfo memory swapDetails)
        internal
        isContractWhitelisted(swapDetails.srcStableTokenAddress)
        returns (uint64)
    {
        require(
        ...
        _handleDepositForReserveToken(swapDetails);
        return depositNonce[swapDetails.destChainIdBytes];
    }

    /// @notice Function to handle deposit for reserve tokens
    /// @param  swapDetails swapInfo struct for the swap details
    function _handleDepositForReserveToken(IDepositExecute.ReserveOrLPSwapInfo memory swapDetails) internal {
        if (voyagerUtils.isBurnable(swapDetails.srcTokenAddress)) {
            ...
            if (_usdc == swapDetails.srcTokenAddress && _isUsdcBurnableMintable[swapDetails.destChainIdBytes]) {
                // Burn USDC
    ...
    /// @param executeData Execute data for the execution of transaction on the destination chain.
    function depositLPToken(bytes memory swapData, bytes memory executeData)
        external
        payable
        nonReentrant
        whenNotPaused
    ...
    function _depositLPToken(IDepositExecute.ReserveOrLPSwapInfo memory swapDetails)
        internal
        isContractWhitelisted(swapDetails.srcStableTokenAddress)
        returns (uint64)
    {
    ...
    function depositNonReserveToken(
        ...
        executeDetails.depositNonce = _depositNonReserveToken(swapDetails);

        bytes memory payload = abi.encode(1, swapDetails, executeDetails);
        gatewayContract.requestToRouter{ value: msg.value }(payload, routerBridge, 1000000, Utils.FeePayer.APP);
    }
    ...
    function depositNonReserveTokenAndExecute(
        ...
        bytes memory payload = abi.encode(3, msg.sender, swapDetails, executeDetails, arbitraryInstruction);
        gatewayContract.requestToRouter{ value: msg.value }(payload, routerBridge, 1000000, Utils.FeePayer.APP);
    }

    function _depositNonReserveToken(IDepositExecute.NonReserveSwapInfo memory swapDetails)
        internal
        isContractWhitelisted(swapDetails.srcStableTokenAddress)
        returns (uint64)
    {
        ...
        if (voyagerUtils.isBurnable(swapDetails.srcStableTokenAddress)) {
    ...
    /// @param sender Address of the sender of the transaction on the source chain.
    /// @param payload Payload coming from the router chain.
        ...
        whenNotPaused
    {
        ...
        uint8 txType = abi.decode(payload, (uint8));
        ...
        if (txType == 2) {
            (, bytes32 destChainIdBytes, uint64 _depositNonce, IDepositExecute.DepositData memory depositData) = abi
                .decode(payload, (uint8, bytes32, uint64, IDepositExecute.DepositData));
            ...
            if (voyagerUtils.isBurnable(depositData.srcStableTokenAddress)) {
                _reserve.mintERC20(
                ...
                IERC20Upgradeable(depositData.srcStableTokenAddress).transfer(
                    depositData.sender,
                    depositData.srcStableTokenAmount
                );
            }
    ...
    /// @param  destChainIdBytes keccak256(abi.encodePacked(destChainType, keccak256(abi.encode(destChainId))))
    ...
    function handleUsdcBurn(bytes32 destChainIdBytes, uint256 amount) internal {
        ...
        require(address(usdcBurnerContract) != address(0), "USDC Burner not set");

        _reserve.giveAllowance(_usdc, address(usdcBurnerContract), amount);
        usdcBurnerContract.depositAndBurnUsdc(destChainIdBytes, amount);
    }
}
