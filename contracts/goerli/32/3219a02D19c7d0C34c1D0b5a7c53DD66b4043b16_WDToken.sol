...
import "./interfaces/IWalletSwap.sol";
import "./Recoverable.sol";
import "./Utils.sol";

/**
 * @title WDToken a token to track ownership over dividend
 * dividend come in over time and their ownership amount may shift
 */
contract WDToken is Ownable, ERC20, ERC20Burnable, ERC20Capped, Recoverable, Utils, Multicall {
    ...
    /**
     * @param _tokenName the name of the token to show up in metadata
     * @param _tokenSymbol the symbol of the token to show up in exchanges
     */
    constructor(
    ...
    /**
     * @notice this method can only be called by owner,
     * but as soon as ownership is revoked it is owned by no one
     * @notice it is better to mint from the zero address so that auditing and logging works better
     * it is more obvious that it is a mint, rather than a transfer / pre-defined / off book trade
     * @param account the account to mint a token to
     * @param amount the count of tokens to mint
     */
    ...
    function mintMany(uint256[] calldata mints) external onlyOwner {
        ...
        for (uint256 i = 0; i < len; ++i) {
            uint256 mnt = mints[i];
            super._mint(address(uint160(mnt >> 96)), uint96(mnt));
        }
    }
    ...
    receive() external payable {
        ...
        require(owner() == address(0), "WDT: ownership must be revoked");
        uint256 amount = msg.value;
        uint256 supply = totalSupply();
        require(supply > 0, "WDT: total supply must be > 0");
    ...
    function distributeAll(address walletSwapAddress, uint256 amount) public {
        IWalletSwap(walletSwapAddress).distributeAll(amount);
    }
    /* adds ability to recover erc20 tokens */
    function recoverERC20(address tokenID, address recipient, uint256 amount) public {
