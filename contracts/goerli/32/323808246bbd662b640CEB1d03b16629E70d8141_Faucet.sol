...
contract Faucet is Ownable {
    ...
    uint256 internal previousAmount;
    uint256 public lockTime = 1 minutes;
    uint256 internal oldLockTime;
    ...
    event WithdrawlAmountChanged(
        uint256 indexed changedFrom, 
        uint256 indexed changedTo
    );

    event changeLockTime(
        uint256 indexed changedFrom, 
        uint256 indexed changedTo
    );
    ...
    function requestTokens() public {
        ...
        emit OneDrip(msg.sender, withdrawlAmount);
    }
    ...
    function SetWithDrawlAmount(uint256 amount) public onlyOwner {
        previousAmount = withdrawlAmount;
        withdrawlAmount = amount * (10**18);
        emit WithdrawlAmountChanged(previousAmount, withdrawlAmount);
    }

    function setLockTime(uint256 amount) public onlyOwner {
        oldLockTime = lockTime;
        lockTime = amount* 1 minutes;
        emit changeLockTime(oldLockTime, lockTime);
    }
