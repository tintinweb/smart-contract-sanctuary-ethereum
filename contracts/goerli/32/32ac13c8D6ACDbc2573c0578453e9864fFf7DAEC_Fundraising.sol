...
contract Fundraising is IFundraising, Ownable, ReentrancyGuard {
  ...
  function finalizeProject(uint256 _projectId) external payable nonReentrant {
    ...
    // generate fund flow by calculator
    int96 flow = _calculateFlow(projectFund);

    // create flow to send project fund to project owner
    cfaV1.createFlow(project.proposer, seth, flow);
    // payable(project.proposer).transfer(projectFund);

    emit ProjectFinalized(_projectId, project.proposer, projectFund, flow);
  }
  ...
  function _calculateFlow(uint256 _monthlyFundAmount) internal pure returns (int96) {
    return int96(int256(_monthlyFundAmount / 3600 / 24 / 30));
  }
  ...
  event ProjectFinalized(uint256 indexed projectId, address indexed projectOwner, uint256 projectFund, int96 flow);
}
