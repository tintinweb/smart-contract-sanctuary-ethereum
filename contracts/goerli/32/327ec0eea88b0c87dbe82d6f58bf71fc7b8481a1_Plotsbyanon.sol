...
/**
 * @title PBYA contract
 * @dev Anon duh Extends ERC721A - thanks azuki
 */
...
contract Plotsbyanon  is ERC721A, Ownable, ReentrancyGuard {
  ...
  constructor() ERC721A("Plots by anon", "PBYA"){}

  function mint(uint256 count) external payable {
  	uint256 cost = price;
        require(mintEnabled, "Minting is not live yet");
        require(_numberMinted(msg.sender) + count <= maxPerAddress, "ONLY 1 FOR YOU");
        require(count > 0, "So You Don't want art?");
        require(count <= maxPerTx, "Let's share the art");
        require(totalSupply() + count < maxSupply + 1, "Sold Out");

        require(msg.value >= cost*count, "INVALID_ETH");
