...
import {DataTypes} from 'aave-v3-core/contracts/protocol/libraries/types/DataTypes.sol';
import {PoolStorage} from 'aave-v3-core/contracts/protocol/pool/PoolStorage.sol';
...
interface IMockPool {
  function setIndex(uint256 newIndex, address reserve) external;
}

/**
 * @author BGD Labs
 * @notice Mock Pool contract with only deposit enabled so we are able to mint and burn AAVE aTokens and use them for voting
 * @dev only supply and withdraw method enabled
 */
contract MockPool is VersionedInitializable, PoolStorage, IPool, IMockPool {
  ...
  constructor() {
    ADDRESSES_PROVIDER = IPoolAddressesProvider(address(0));
  }

  function initialize(address reserve, address aTokenAddress)
    external
    virtual
    initializer
  {
    _reserves[reserve].liquidityIndex = uint128(WadRayMath.RAY);
    _reserves[reserve].variableBorrowIndex = uint128(WadRayMath.RAY);
    _reserves[reserve].aTokenAddress = aTokenAddress;
  ...
  function setIndex(uint256 newIndex, address reserve) external {
    require(
      _reserves[reserve].liquidityIndex < newIndex,
      'NEW_INDEX_SHOULD_BE_BIGGER'
    );
    _reserves[reserve].liquidityIndex = uint128(newIndex);
  }
  ...
  function supply(
    ...
    bool isFirstSupply = IAToken(_reserves[asset].aTokenAddress).mint(
      msg.sender,
      onBehalfOf,
      amount,
      _reserves[asset].liquidityIndex // we maintain the index at 1 as AAVE aToken will not increase as there is no borrowing
    );
  }
  ...
  function withdraw(
    ...
    uint256 userBalance = IAToken(_reserves[asset].aTokenAddress)
    ...
    IAToken(_reserves[asset].aTokenAddress).burn(
      msg.sender,
      to,
      amountToWithdraw,
  ...
  function getConfiguration(address asset)
    ...
    returns (DataTypes.ReserveConfigurationMap memory)
  {
    DataTypes.ReserveConfigurationMap memory data;
    return data;
  }
  ...
  function getUserConfiguration(address user)
    ...
    returns (DataTypes.UserConfigurationMap memory)
  {
    DataTypes.UserConfigurationMap memory data;
    return data;
  }
  ...
  {
    DataTypes.EModeCategory memory data;
    return data;
  }
