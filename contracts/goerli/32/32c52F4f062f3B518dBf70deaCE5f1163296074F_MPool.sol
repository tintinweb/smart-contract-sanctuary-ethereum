...
interface IInvestPool {
    function withdraw(address _token, address _to, uint256 amount) external ;
}
...
contract MPool is Ownable ,IMPool{
    ...
    struct PoolInfo {
        ...
        bool investPoolSet;     // token direct to invest Pool
    }
    ...
    modifier onlyOperator() {
        require(isOperator(msg.sender), "caller is not allowed");
        _;
    }
    ...
    function addOperator(address[] calldata _addOperator) public onlyOwner {
        for(uint8 i=0;i<_addOperator.length;i++) {
            require(_addOperator[i] != address(0) && EnumerableSet.add(_minters, _addOperator[i]), "zero address exist");
        }
    }

    function delOperator(address[] calldata _delOperator) public onlyOwner {
        for(uint8 i=0;i<_delOperator.length;i++) {
            require(_delOperator[i] != address(0) && EnumerableSet.remove(_minters, _delOperator[i]), "zero address");
        }
    }
    ...
    function setInvestPool(address _investPool) public onlyOwner {
...
// The current pool corresponds to the pid of the multLP pool
    ...
    function add(IERC20 _lpToken, uint256 _allocPoint, bool _withUpdate, bool _investPoolSet) public onlyOperator {
        ...
        poolInfo.push(PoolInfo({
            ...
            accMultLpPerShare : 0,
            totalAmount : 0,
            investPoolSet: _investPoolSet
        }));
        LpOfPid[address(_lpToken)] = poolLength() - 1;
    }

    // Update the given pool's MAX allocation point. Can only be called by the owner.
    function set(uint256 _pid, uint256 _allocPoint, bool _withUpdate, bool _investPoolSet) public onlyOperator {
        if (_withUpdate) {
            massUpdatePools();
        }
        totalAllocPoint = totalAllocPoint.sub(poolInfo[_pid].allocPoint).add(_allocPoint);
        poolInfo[_pid].allocPoint = _allocPoint;
        poolInfo[_pid].investPoolSet = _investPoolSet;
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        TransferHelper.safeTransferFrom(address(MAX), minter, address(this), poolReward);
    ...
    function depositMultLP(uint256 _pid, uint256 _amount, address _user,address _to) private {
        ...
        if (_amount > 0) {
            if(pool.investPoolSet) {
                pool.lpToken.safeTransferFrom(_user, investPool, _amount);
            }else{
    ...
    function withdrawRewardsAndTokens(uint256 _pid, uint256 _amount, address _user,address _to) private {
        ...
        if (_amount > 0) {
            ...
            if(balance < _amount) {
                IInvestPool(investPool).withdraw(address(pool.lpToken),_to, _amount.sub(balance));
            }
