...
import {IUBToken} from "../../interfaces/IUBToken.sol";
...
library LiquidateLogic {
  ...
  function executeAuction(
    ...
    // first time bid need to burn debt tokens and transfer reserve to UDTokens
    ...
    reserveData.updateInterestRates(loanData.reserveAsset, reserveData.UDTokenAddress, 0, 0);
  ...
  function executeRedeem(
    ...
    IUBToken(reserveData.UBTokenAddress).burn(loanData.borrower, vars.repayAmount, reserveData.variableBorrowIndex);

    // update interest rate according latest borrow amount (utilizaton)
    reserveData.updateInterestRates(loanData.reserveAsset, reserveData.UDTokenAddress, vars.repayAmount, 0);

    // transfer repay amount from borrower to UDToken
    IERC20Upgradeable(loanData.reserveAsset).safeTransferFrom(
      vars.initiator,
      reserveData.UDTokenAddress,
      vars.repayAmount
    );
  ...
  function executeLiquidate(
    ...
    IUBToken(reserveData.UBTokenAddress).burn(
      loanData.borrower,
      vars.borrowAmount,
      reserveData.variableBorrowIndex
    );
    ...
    // update interest rate according latest borrow amount (utilizaton)
    reserveData.updateInterestRates(loanData.reserveAsset, reserveData.UDTokenAddress, vars.borrowAmount, 0);
    ...
    // transfer borrow amount from lend pool to UDToken, repay debt
    IERC20Upgradeable(loanData.reserveAsset).safeTransfer(reserveData.UDTokenAddress, vars.borrowAmount);
...
interface IUDToken is IScaledBalanceToken, IERC20Upgradeable, IERC20MetadataUpgradeable {
  /**
   * @dev Emitted when an UDToken is initialized
   * @param underlyingAsset The address of the underlying asset
   * @param pool The address of the associated lending pool
   * @param treasury The address of the treasury
   * @param incentivesController The address of the incentives controller for this UDToken
   **/
  event Initialized(
  ...
  /**
   * @dev Initializes the UDToken
   * @param addressProvider The address of the address provider where this UDToken will be used
   * @param treasury The address of the Ubiloan treasury, receiving the fees on this UDToken
   * @param underlyingAsset The address of the underlying asset of this UDToken
   */
  function initialize(
    ...
    string calldata UDTokenName,
    string calldata UDTokenSymbol
  ) external;
  ...
  /**
   * @dev Mints `amount` UDTokens to `user`
  ...
  /**
   * @dev Emitted after UDTokens are burned
   * @param from The owner of the UDTokens, getting them burned
  ...
  /**
   * @dev Burns UDTokens from `user` and sends the equivalent amount of underlying to `receiverOfUnderlying`
   * @param user The owner of the UDTokens, getting them burned
  ...
  /**
   * @dev Mints UDTokens to the reserve treasury
   * @param amount The amount of tokens getting minted
   * @param index The new liquidity index of the reserve
   */
  ...
  /**
   * @dev Returns the address of the underlying asset of this UDToken
   **/
 ...
 **/
interface IUBToken is IScaledBalanceToken, IERC20Upgradeable, IERC20MetadataUpgradeable {
  /**
   ...
   * @param UBTokenDecimals the decimals of the debt token
   * @param UBTokenName the name of the debt token
   * @param UBTokenSymbol the symbol of the debt token
   **/
  event Initialized(
    address indexed underlyingAsset,
    address indexed pool,
    address incentivesController,
    uint8 UBTokenDecimals,
    string UBTokenName,
    string UBTokenSymbol
  );
   ...
   * @param underlyingAsset The address of the underlying asset
   * @param UBTokenDecimals The decimals of the UBToken, same as the underlying asset's
   * @param UBTokenName The name of the token
   * @param UBTokenSymbol The symbol of the token
   */
  function initialize(
    ILendPoolAddressesProvider addressProvider,
    address underlyingAsset,
    uint8 UBTokenDecimals,
    string memory UBTokenName,
    string memory UBTokenSymbol
  ) external;
...
library ReserveLogic {
  ...
  function updateState(DataTypes.ReserveData storage reserve) internal {
    uint256 scaledVariableDebt = IUBToken(reserve.UBTokenAddress).scaledTotalSupply();
   ...
   * @param UBTokenAddress The address of the overlying UBToken contract
   * @param interestRateAddress The address of the interest rate strategy contract
   **/
  function init(
    DataTypes.ReserveData storage reserve,
    address UDTokenAddress,
    address UBTokenAddress,
    address interestRateAddress
  ) external {
    require(reserve.UDTokenAddress == address(0), Errors.RL_RESERVE_ALREADY_INITIALIZED);

    reserve.liquidityIndex = uint128(WadRayMath.ray());
    reserve.variableBorrowIndex = uint128(WadRayMath.ray());
    reserve.UDTokenAddress = UDTokenAddress;
    reserve.UBTokenAddress = UBTokenAddress;
  ...
  function updateInterestRates(
    ...
    // updateInterestRates(params.asset, reserveData.UDTokenAddress, 0, params.amount);
    ...
    vars.totalVariableDebt = IUBToken(reserve.UBTokenAddress).scaledTotalSupply().rayMul(
      reserve.variableBorrowIndex
    );
  ...
  function _mintToTreasury(
    ...
    if (vars.amountToMint != 0) {
      IUDToken(reserve.UDTokenAddress).mintToTreasury(vars.amountToMint, newLiquidityIndex);
    }
  }
...
library ValidationLogic {
  ...
  function validateBorrow(
    ...
    require(reserveData.UDTokenAddress != address(0), Errors.VL_INVALID_RESERVE_ADDRESS);
  ...
  /**
   * @dev Validates an UDToken transfer
   * @param from The user from which the UDTokens are being transferred
...
library Errors {
  ...
  string public constant LP_CALLER_MUST_BE_AN_UDToken = "410";
