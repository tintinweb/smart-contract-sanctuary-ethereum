...
contract TestCF is ERC721, Ownable, Functional {
    ...
    constructor() ERC721("",""){
        ...
        priceWhitelist = 99000000000000000;//1980000000000000000
        maxMintPerTx = 20;       
        maxPerWallet = 20;
    ...
    function publicMint(uint256 qty) external payable reentryLock {
        ...
        require((qty + reservedTokens + tokenMinted) <= maxSupply, 
            "Mint: Not enough avaialability");      //Check it
        require(qty <= maxMintPerTx,"Mint: Max Mint tokens per transaction exceeded");
        require((tokenMintedby[_msgSender()] + qty) <= maxPerWallet, 
    ...
    function whitelistMint(uint256 qty) external payable reentryLock {
        ...
        require(whitelistUser[_msgSender()].tokenCount > 0, "Whitelist: Invalid User"); 
        require((qty + reservedTokens + tokenMinted) <= maxSupply, 
            "Whitelist: Not enough avaialability"); //check it
        require((tokenMintedby[_msgSender()] + qty) <= maxPerWallet, 
        ...
        require(tokenMintedby[_msgSender()] + qty <= whitelistUser[_msgSender()].tokenCount,
             "Whitelist: Limit exceeded");
        
        
        tokensMint(qty, priceWhitelist);
        if(tokenMintedby[_msgSender()] >= whitelistUser[_msgSender()].tokenCount){
    ...
    function setMaxSupply(uint256 newTokenLimit) public onlyOwner{
        require( newTokenLimit > tokenMinted, "Increase the limit of total tokens." );
        require(reservedTokens < newTokenLimit,"Change First reserved token quantity");
