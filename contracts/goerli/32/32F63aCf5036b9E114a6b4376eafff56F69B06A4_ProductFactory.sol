...
contract ProductFactory is ProductFactoryConfig, IProductFactory {
    ...
    function deploy(
        ...
        emit Deployed(alias_, proxy_, paymentToken_, currentPrice_, cashbackAmount_);
...
interface IPayment {
    ...
    /**
     * @notice Set `distributor`. Cashback amount will be transfered for `distributor`
     * @param newDistributor_ Contract address
     */
    function setDistributor(address newDistributor_) external;

    /**
     * @notice Set `holder`. Product amount will be transfered for `holder`
     * @param holder_ Aaddress
     */
     ...
     * @param paymentAmount_ Amount that will be transfered to `holder`
     * @param cashbackAmount_ Amount that will be transfered to `distributor`
     */
    function pay(
        address paymentToken_,
        address payer_,
     ...
     */
    function getInputSwapAmount(address paymentToken_, uint256 amountOut_)
        external
        view
        returns (uint256);
...
interface IProductFactory {
    event Deployed(bytes32 indexed productAlias, address indexed proxy, address paymentToken, uint256 price, uint256 cashback);
...
abstract contract ProductFactoryConfig is UUPSBase, IProductFactoryConfig {
    ...
    function addProduct(bytes32 alias_) external onlyOwner {
        aliases.add(alias_);
    ...
    function setImplementation(bytes32 alias_, address implementation_) external onlyOwner {
    ...
    function setStatus(bytes32 alias_, bool isActive_) external onlyOwner {
        Product storage product = _getProduct(alias_);

        product.isActive = isActive_;
