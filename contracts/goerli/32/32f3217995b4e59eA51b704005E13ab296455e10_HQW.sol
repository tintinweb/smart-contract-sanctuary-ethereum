...
contract HQW is ERC1155, DefaultOperatorFilterer, Ownable, ReentrancyGuard {
    ...
    uint256 public OWNER_MINT_PRICE = 2 ether;
    uint256 public OWNER_MAX_SUPPLY = 999;
    uint256 public OWNER_TOTAL_SUPPLY = 0;

    uint256 public LIMIT_PUBLIC_MINT = 1;
    ...
    function publicMintOwner(uint256 amount) external
        ...
        require(
            amount + totalPublicMinted[msg.sender] <= LIMIT_PUBLIC_MINT,
            "Limit exceeded"
        );

        _mintOwner(msg.sender, amount);
        totalPublicMinted[msg.sender] += amount;
    }

    function whitelistMintOwner(bytes32[] memory _merkleProof, uint256 amount)
        external
        payable
        nonReentrant
    {
        require(
            stage == Stage.WHITELIST,
            "Whitelist owner mint hasn't started yet"
        );
        require(
            OWNER_TOTAL_SUPPLY + amount <= OWNER_MAX_SUPPLY,
            "Supply Limit exceeded"
        ...
        require(
            msg.value == (OWNER_MINT_PRICE * amount),
    ...
    function setOwnerMintPrice(uint256 price) external onlyOwner nonReentrant {
        OWNER_MINT_PRICE = price;
    }
    ...
    function setMarkleRoot(bytes32 _merkleRoot) external onlyOwner nonReentrant {
        merkleRoot = _merkleRoot;
    }

    function setPublicLimit(uint256 limit) external onlyOwner nonReentrant {
        LIMIT_PUBLIC_MINT = limit;
    }
