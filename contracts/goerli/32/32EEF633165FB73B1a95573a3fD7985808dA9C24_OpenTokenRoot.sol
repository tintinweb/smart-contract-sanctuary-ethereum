...
contract OpenTokenRoot is ERC20 {
    ...
    mapping (uint256 => WeekEmission) public weekEmissionList;
    ...
    function initEmission() private onlyByOwner {
        weekEmissionList[0].amount = firstWeekEmission;
        weekEmissionList[0].balance = firstWeekEmission;
    }
    ...
    function setStartTime(uint256 t_) external onlyByOwner {
        if(startTime == 0  && t_ > block.timestamp){
            startTime = t_;
        }
    }

    function mint(address to_, uint256 amount_, uint256 t_) public onlyByOwner {
        weekNum = targetWeekNum(t_);
        require(weekEmissionList[weekNum].balance >= amount_, " insufficient week emission");

        totalOpenSupply = totalOpenSupply + amount_;
        weekEmissionList[weekNum].balance = weekEmissionList[weekNum].balance - amount_;

        _mint(to_, amount_);
        emit Minted(msg.sender, to_, amount_, t_, block.timestamp);
    }

    function mintBatch(address to_, uint256[] memory timeList_, uint256[] memory amountList_) public onlyByOwner {

        require(timeList_.length == amountList_.length, "time length need == amount length");
        uint256 batchTotalAmount = 0;
        for(uint i = 0; i < timeList_.length; i++) {
            weekNum = targetWeekNum(timeList_[i]);
            require(weekEmissionList[weekNum].balance >= amountList_[i], "insufficient week emission");
            weekEmissionList[weekNum].balance = weekEmissionList[weekNum].balance - amountList_[i];
            batchTotalAmount = batchTotalAmount + amountList_[i];
        }
        totalOpenSupply = totalOpenSupply + batchTotalAmount;
        
        _mint(to_, batchTotalAmount);
        emit MintedBatch(msg.sender, to_, amountList_, timeList_, block.timestamp);
    }
    ...
    function generateWeekEmissions(uint256 _maxWeekNum) external returns (bool) {
        ...
        uint256 lastWeekTotalAmount = weekEmissionList[lastWeekNum].amount;
        WeekEmission memory we;
        for(uint i = lastWeekNum+1; i <= _maxWeekNum; i++){
            lastWeekTotalAmount =  lastWeekTotalAmount / WEEKLY_REDUCTION_RATE * decimalsBase;
            we.amount = lastWeekTotalAmount;
            we.balance = lastWeekTotalAmount;
            weekEmissionList[i] = we;
        }
    ...
    function weekAvailableSupply(uint256 t_) external view returns (uint256) {
        ...
        return weekEmissionList[targetWeekNum(t_)].balance;
    }

    function weekEmissionAmount(uint256 t_) external view returns (uint256) {

        return weekEmissionList[targetWeekNum(t_)].amount;
    }
    ...
    // Track minted
    event Minted(address indexed from, address indexed to, uint256 amount, uint256 weekTime, uint256 mintTime);
    event MintedBatch(address indexed from, address indexed to, uint256[] amount, uint256[] weekTime, uint256 mintTime);
