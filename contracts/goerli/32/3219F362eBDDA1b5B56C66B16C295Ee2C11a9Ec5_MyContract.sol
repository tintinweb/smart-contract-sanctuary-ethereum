...
contract MyContract is Ownable, ERC721A, ReentrancyGuard {

    uint256 public immutable maxPerAddressDuringMint;
    uint256 public immutable maxPerAddressDuringPublicSale;
    uint256 public immutable amountForAuction;
    uint256 public immutable collectionSize;
    address private immutable payments;

    mapping(address => uint256) public allowlist;
    mapping (address => uint256) private mintedDuringPublic;
    ...
    constructor(
        ...
        uint256 _maxPerAddressDuringMint, // сколько всего токенов может заминтить человек в рамках всех стадий продаж вместе взятых
        uint256 _maxPerAddressDuringPublicSale,
        uint256 _collectionSize,
        address _payments
        )
    ERC721A(_name, _symbol) {
        amountForAuction = _amountForAuction;
        maxPerAddressDuringMint = _maxPerAddressDuringMint; // for auction & public sale mint 
        maxPerAddressDuringPublicSale = _maxPerAddressDuringPublicSale;
        collectionSize = _collectionSize;
        payments = _payments;
        require(amountForAuction <= collectionSize, "larger collection size needed");
    }
    ...
    function auctionMint(uint256 quantity) external payable callerIsUser {
        ...
        require(totalSupply() + quantity <= amountForAuction,"not enough remaining reserved for auction to support desired mint amount");
    ...
    function allowlistMint(uint256 quantity) external payable callerIsUser {
        ...
        require(!isPublicSaleOn(publicPrice, publicSaleStartTime), "allowlist minting stage is over");

        // можно добавить, но не обязательно. Количество минтов в рамках WL и так будет небольшим.
        require(numberMinted(msg.sender) + quantity <= maxPerAddressDuringMint,"can not mint this many");
    ...
    function publicSaleMint(uint256 quantity) external payable callerIsUser {
        ...
        require(totalSupply() + quantity <= collectionSize, "reached max supply");
        require(numberMinted(msg.sender) + quantity <= maxPerAddressDuringMint,"can not mint this many");
        require(mintedDuringPublic[msg.sender] + quantity <= maxPerAddressDuringPublicSale, "reached max token count permitted during public sale");
        mintedDuringPublic[msg.sender] += quantity;
        _safeMint(msg.sender, quantity);
    ...
    // https://gateway.pinata.cloud/ipfs/QmS1FayUNo4q5WTW2qrVqCs9Qa6KnU94phmjqjJPnXeeuJ/1.json
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        ...
        string memory baseURI = _baseURI();
        string memory metadata = !revealed ? 'unrevealed' : _toString(tokenId);
        string memory concatenation = string(abi.encodePacked(baseURI, metadata, '.json'));
        return bytes(baseURI).length != 0 ? concatenation : '';
    }
    ...
    function withdrawTokens(IERC20 token) external onlyOwner nonReentrant {
        ...
        SafeERC20.safeTransfer(token, payments, allTokens);
    }

    // добавить возможность вывода токенов erc20
