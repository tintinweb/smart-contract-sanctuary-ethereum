...
contract OCASnake is Ownable, ERC721A, ERC2981, ReentrancyGuard, OCASnakeArt {
    ...
    function recursiveNameFind(
        ...
        uint256 snakeNameIndex = (generateAppleSeed(tokenId, index) %
            snakes.length);
        uint256 adjectiveIndex = (generateAppleSeed(tokenId, index) %
            wackyAdjectives.length);

        uint256 indexCompare = snakeNameIndex + adjectiveIndex;

        uint256 snakeNameIndexCompare = (generateAppleSeed(index, index) %
            snakes.length);
        uint256 adjectiveIndexCompare = (generateAppleSeed(index, index) %
            wackyAdjectives.length);

        uint256 addCompare = snakeNameIndexCompare + adjectiveIndexCompare;

        if (snakeNameIndexes[0] == 0 && index == 0) {
            snakeNameIndexes[0] = snakeNameIndex;
            snakeNameIndexes[1] = adjectiveIndex;
            return
                this.recursiveNameFind(tokenId, index + 1, 0, snakeNameIndexes);
