...
contract SynthiaCustomTraits is ERC721AQueryable, Owned, Initializable, ISynthiaTraitsERC721 {
    ...
    function addItem(
        ...
        uint256 id = ++_itemId;
        itemsv1[id] = ItemV1(_name, _image, _onchainImgId, _description, _traitIdx, 1, _minter, _isHood);
        emit ItemAdded(id);
    }
    ...
    function clearCustomTrait(uint256 synthiaNftId, uint256 traitIdx)
        public
        checkSynthiaOwnership(synthiaNftId, msg.sender)
    {
        SynthiaRenderer(synthiaRenderer).clearCustomTrait(synthiaNftId, traitIdx);
    }
    ...
    function isHood(uint256 tokenId) external view returns (bool) {
        return itemsv1[mappingTokenIdToItemId[tokenId]].isHood;
    }
...
interface ISynthiaTraitsERC721 {
    ...
    /// @dev Function that returns true if the trait is a hood. This will place accessory below the clothing item.
    /// By default the accessory is layered above the clothing item.
    function isHood(uint256 tokenId) external view returns (bool);
}
...
contract SynthiaRenderer is Owned, Initializable {
    ...
    function _constructSvg(SvgParameters memory params) internal view returns (string memory) {
        ...
        if (params.traitInfo.hasCustomClothes && _getCustomTraitIsHood(params.tokenId)) {
            params.isHood = true;
        }
        return string.concat(
    ...
    function clearCustomTrait(uint256 tokenId, uint256 idx) public onlyTraitAdmin {
        if (checkBit(tokenIdOverrides[tokenId], uint8(idx))) {
            tokenIdOverrides[tokenId] = flipBit(tokenIdOverrides[tokenId], uint8(idx));
        }
    ...
    function _getCustomTraitIsHood(uint256 tokenId) internal view returns (bool) {
        CustomTrait memory trait = tokenIdToIdxToCustomTrait[tokenId][0];
        try ISynthiaTraitsERC721(trait.contractAddress).isHood(trait.tokenId) returns (bool isHood) {
            return isHood;
        } catch {
            return false;
        }
