...
contract PrimaryIndexTokenLeverage is Initializable, AccessControlUpgradeable, ReentrancyGuardUpgradeable
{
    ...
    function calculateCollateralTokenCount(address _projectToken, uint notionalValue) public view returns(uint collateralTokenCount) {
        collateralTokenCount = notionalValue / getTokenPrice(_projectToken);
    }

    function calculateLendingTokenCount(address _lendingToken, uint notionalValue) public view returns(uint lendingTokenCount) {
        lendingTokenCount = notionalValue / getTokenPrice(_lendingToken);
    }
    ...
    function calculateMargin(address projectToken, uint safetyMarginNumerator, uint safetyMarginDenominator, uint expAmount) public view returns(uint margin) {
        (uint8 lvrNumerator, uint8 lvrDenominator) = getLVR(projectToken);
        margin = (expAmount * (lvrDenominator * (safetyMarginDenominator + safetyMarginNumerator) - lvrNumerator * safetyMarginDenominator) / (lvrNumerator * safetyMarginDenominator));
    }
    ...
    function calculateSafetyMargin(address projectToken, uint margin, uint exp) public view returns(uint safetyMarginNumerator, uint safetyMarginDenominator){
        (uint8 lvrNumerator, uint8 lvrDenominator) = getLVR(projectToken);
        safetyMarginNumerator = (margin + exp) * lvrNumerator - exp * lvrDenominator;
        safetyMarginDenominator = (exp * lvrDenominator);
    }

    function transfer(address _from, address _to, address token, uint amount) public {
        ERC20Upgradeable(token).approve(address(this), amount);
        ERC20Upgradeable(token).safeTransferFrom(_from, _to, amount);
    }

    function deferLiquidityCheck(address projectToken, address lendingToken, uint lendingTokenAmount) internal view {
        uint pitRemaining = primaryIndexToken.pitRemaining(msg.sender, projectToken, lendingToken);
        uint lendingTokenAmountPrice = primaryIndexToken.getPriceConvert(lendingToken, lendingTokenAmount);
        uint newTotalBorrowPerCollateral = primaryIndexToken.getTotalBorrowPerCollateral(projectToken) + lendingTokenAmountPrice;
        uint borrowLimitPerCollateral =  primaryIndexToken.borrowLimitPerCollateral(projectToken);
        uint newTotalBorrowPerLendingToken = primaryIndexToken.getTotalBorrowPerLendingToken(lendingToken) + lendingTokenAmount;
        uint borrowLimitPerLendingToken = primaryIndexToken.borrowLimitPerLendingToken(lendingToken);
        require(lendingTokenAmountPrice <= pitRemaining, "PIT: lendingTokenAmount exceeds pitRemaining");
        require(newTotalBorrowPerCollateral <= borrowLimitPerCollateral, "PIT: totalBorrow exceeded borrowLimit per collateral asset");
        require(newTotalBorrowPerLendingToken <= borrowLimitPerLendingToken, "PIT: totalBorrow exceeded borrowLimit per lending asset");
    }

    function nakedBorrow(address lendingToken, uint lendingTokenAmount, address projectToken) public {
        address _lendingToken = primaryIndexToken.lendingTokenPerCollateral(msg.sender, projectToken);
        address usdcToken = primaryIndexToken.usdcToken();
        uint borrowedUSD = primaryIndexToken.totalOutstanding(msg.sender, projectToken, usdcToken);
        if (borrowedUSD > 0) {
        ...
        primaryIndexToken.updateInterestInBorrowPositions(msg.sender, lendingToken);

        primaryIndexToken.calcBorrowPosition(msg.sender, projectToken, lendingToken, lendingTokenAmount, _lendingToken);
        transfer(msg.sender, address(this), lendingToken, lendingTokenAmount);
    }

    function _buyOnParaSwap(address tokenTo, address _target, bytes memory buyCalldata) public returns (uint amountRecive) {
    ...
    function collateraliseLoan(uint collateralTokenCount, uint marginCollateralCount) internal pure returns(uint totalCollateral){
        totalCollateral = collateralTokenCount + marginCollateralCount;
    }
    ...
    function checkIsValidPosition(address user, address projectToken, address lendingToken, uint marginCollateralAmount) internal view {
        ...
        require(!isLeveragePosition[user][projectToken] && loanBody == 0 && accrual == 0 || isLeveragePosition[user][projectToken], "PIT: invalid position");
        require(marginCollateralAmount >= depositedProjectTokenAmount, "PIT: invalid position");
    }

    function leveragedBorrow(address projectToken, address lendingToken, uint notionalExposure, uint marginCollateralAmount, uint maxSlippage, address target, bytes memory buyCalldata) public {
        checkIsValidPosition(msg.sender, projectToken, lendingToken, marginCollateralAmount);
        ...
        nakedBorrow(lendingToken, lendingTokenCount, projectToken);

        uint amountRecive = _buyOnParaSwap(projectToken, target, buyCalldata);

        uint slippage = amountRecive * decimalPercent / collateralTokenCount;

        require(maxSlippage >= slippage, "PIT: over maxSlippage");

        transfer(msg.sender, address(primaryIndexToken), projectToken, marginCollateralAmount);

         uint totalCollateral = collateraliseLoan(amountRecive, marginCollateralAmount);

        deferLiquidityCheck(projectToken, lendingToken, lendingTokenCount);

        primaryIndexToken.calcDepositPosition(projectToken, totalCollateral, msg.sender);
        if(!isLeveragePosition[msg.sender][projectToken]){
            isLeveragePosition[msg.sender][projectToken] = true;
        }
    }
