...
contract SDAirdrop {
    ...
    function initialize(string memory _name, address _token, address _owner) external {
    ...
    function deposit(uint256 depositAmount, uint256 _eachAmount) external onlyAirdropOwner {
        SafeERC20.safeTransferFrom(IERC20(token), msg.sender, address(this), depositAmount);
        balance += depositAmount;
        claimAmount = _eachAmount;
    ...
    function claim() external {
        IFactory(factory).checkIdentity(msg.sender, string(abi.encodePacked("airdrop.claim.", name)));
        require(balance >= claimAmount, "SDAirdrop: Balance is not enough to claim.");
    ...
    function vipClaim(address account) external {
        require(msg.sender == IFactory(factory).vip(), "SDAirdrop: Only VIP.");
...
contract SDFactory is Ownable {
    ...
    event IdentifierChanged(string indexed methodId, address indexed identifier);
    
    function airdropsLength() external returns (uint256){
        return airdrops.length;
    }

    function createAirdrop(string memory name, address token, address identifier) external returns (address _airdrop) {
        ...
        airdrop.initialize(name, token, msg.sender);
        ...
        if (identifier != address(0))
            getIdentifier[string(abi.encodePacked("airdrop.claim.", name))] = identifier;
        emit AirdropCreated(msg.sender, token);
    }
    ...
    function checkIdentity(address account, string memory methodId) public view returns (bool) {
        if (getIdentifier[methodId] == address(0))
            return true;
        return Identifier(getIdentifier[methodId]).identify(account);
    }
...
interface IFactory {
    ...
    function checkIdentity(address account, string calldata _type) external returns (bool);
