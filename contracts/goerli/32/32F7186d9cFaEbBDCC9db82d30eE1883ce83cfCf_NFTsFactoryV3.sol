...
abstract contract OpenChecker is IOpenChecker, OpenERC165 {
    ...
    function checkSupportedInterfaces( 
        ...
        uint256 i;
        uint256 len = (withErc ? _ercInterfaceIds.length : 0) + interfaceIds.length;
...
abstract contract OpenGetter is IOpenGetter, OpenChecker {
    ...
    function _getCollectionsInfos(
        address[] memory collections,
        address account,
        bytes4[] memory interfaceIds
    ) internal view returns (CollectionInfos[] memory collectionsInfos) {
...
abstract contract OpenResolver is IOpenResolver, OpenRegistry, OpenGetter {
    function getResolverCollectionsInfos(address account)
        external
        view
        override(IOpenResolver)
        returns (CollectionInfos[] memory collectionsInfos)
    {
        collectionsInfos = _getResolverCollectionsInfos(account, new bytes4[](0));
    }
    ...
    function _getResolverCollectionsInfos(address account, bytes4[] memory interfaceIds)
        internal
        view
        returns (CollectionInfos[] memory collectionsInfos)
    {
        CollectionInfos[] memory collectionsInfosAll = _getCollectionsInfos(_addresses, account, interfaceIds);
...
contract NFTsResolver is INFTsResolver, OpenResolver {
    ...
    function getNFTsResolverCollectionsInfos(address account)
        ...
        returns (CollectionInfos[] memory collectionsInfos)
    {
        collectionsInfos = _getResolverCollectionsInfos(account, interfaceIds);
    }
