...
import "../interfaces/IBumpOracle.sol";
...
contract Bond is BondBase {
    ...
    address public bumpOracle;

    /// @notice internal initializer
    /// @param _gac Bumper Global Access Controller
    /// @param _bondToken Bond token address
    function initialize(address _gac, address _bondToken, address _config) external initializer {
        __BondBase__init(_gac, _bondToken, _config);
    }

    /// @notice set BUMP price oracle
    function setBumpOracle(address _oracle) external onlyGovernance {
        bumpOracle = _oracle;
    }
    ...
    function takerBond(
        ...
        BondConfig storage config = bondConfig[token];
        bondAmount = amount * (config.bumpPerAsset + config.thetaAsset2 ) / DIVIDER;
        // amount - 100 ETH, price 2000$ / ETH, bump 0.05$
        // thetaAsset1 - 1%, thetaAsset2 - 100 
        // bumpPerAsset = thetaAsset1 * assetPrice / bumpPrice = 1% * 2000$ / 0.05$ = 1% * 40000 = 400 BUMP / ETH
        // bondAmount = 100 ETH * (400 + 100) = 40000 + 10000 = 50000 BUMP per 100 ETH
    ...
    function makerBond(
        ...
        BondConfig storage config = bondConfig[token];        
        bondAmount = amount * (config.bumpPerStable + config.thetaStable2) / DIVIDER;
    }
...
/// @title BondBase
/// @notice Abstract contract that implements base functionality: access control, bonding storage, market bonding configuration, bonding methods for markets.
/// @dev This contract should be inherited with takerBond, makerBond, takerToSwap and makerToSwap functions impelementation.
    ...
    IBond
{
    ...
    uint constant internal DIVIDER_BOND = 10**18;
    uint constant internal DIVIDER_CONFIG = 10**18;    
    uint constant internal DIVIDER_STABLE = 10**18;
    uint constant internal DIVIDER_ORACLE = 10**8;

    // protocol configuration contract
    IProtocolConfig public config;

    /// @dev Bond token address (BUMP token)
    function BOND_TOKEN_ADDRESS()
        public
        view
        virtual
        override
    ...
    /// @dev record that represents bond config of particular market
    mapping(address => BondConfig) public bondConfig;

    /// @notice requires msg.sender to be adapter contract
    modifier onlyPermittedAdapter() {
        require(hasRole(ADAPTER_ROLE, msg.sender), "BB: not-permitted-adapter");
        _;
    }

    /// @notice requires msg.sender to be adapter contract
    modifier onlyMarket() {
        require(hasRole(MARKET_ROLE, msg.sender), "BB: not-permitted-market");
        _;
    }

    /// @notice initializer of this contract
    /// @param _gac - global access controller
    function __BondBase__init(
        address _gac,
        address _bondToken,
        address _config
        ...
        __ModuleAccessControllerUpgradeable__init(_gac);
    }
    ...
    /// @notice prepare multipliers for bonding
    function calcBonding(
        address token,
        uint bumpPrice,
        uint assetPrice
    ) external override onlyGovernance {
        uint DIVIDER_ASSET = 10**IERC20Metadata(token).decimals();

        BondConfig storage _config = bondConfig[token]; 

        _config.bumpPerAsset = _config.thetaAsset1 * assetPrice / DIVIDER_ORACLE * DIVIDER_BOND / bumpPrice ; 
        _config.bumpPerStable = _config.thetaStable1 * DIVIDER_BOND / bumpPrice ; 
        _config.assetPerBump = DIVIDER_ASSET * DIVIDER_CONFIG / _config.bumpPerAsset;  
        _config.stablePerBump = DIVIDER_STABLE * DIVIDER_CONFIG  / _config.bumpPerStable;
    }

    /// @notice Set thetas for bonding
    /// @param thetaAsset1 - percent of taker position (position size in USD -> bump size)
    /// @param thetaAsset2 - multiplier for taker position (asset size -> bump size) 
    /// @param thetaStable1 - percent of maker position (position size in USD -> bump size) 
    /// @param thetaStable2 - multiplier for maker position (stable size -> bump size)
    function setBondTheta(address token, uint thetaAsset1, uint thetaStable1, uint thetaAsset2, uint thetaStable2 ) external override onlyGovernance {
        BondConfig storage cfg = bondConfig[token];
        cfg.thetaAsset1 = thetaAsset1;
        cfg.thetaAsset2 = thetaAsset2;
        cfg.thetaStable1 = thetaStable1;
        cfg.thetaStable2 = thetaStable2;
    }

    /// @inheritdoc	IBond
    function deposit(address depositTo, uint amount)
        public
        virtual
        override
        whenNotPaused
    {
        require(amount > 0, "BB: bad-amount");
        require(depositTo != address(0), "BB: zero-address");

        depositedBalances[depositTo] += amount;

        IERC20(BOND_TOKEN_ADDRESS()).safeTransferFrom(
            _msgSender(),
            address(this),
            amount
        );
    ...
    function depositWithPermit(
        ...
        IERC20PermitUpgradeable(BOND_TOKEN_ADDRESS()).permit(
            msg.sender,
            address(this),
            amount,
            deadline,
    ...
    /// @inheritdoc	IBond
    function withdraw(address withdrawOf, uint amount)
        external
        virtual
        override
        whenNotPaused
    ...
    {
        _withdraw(withdrawOf, amount);
    }

    /// @inheritdoc	IBond
    function withdraw(uint amount) external virtual override whenNotPaused {
        _withdraw(_msgSender(), amount);
    }

    /// @inheritdoc IBond
    function unlock(address _owner, uint bondAmount)
        external
        virtual
        override
        whenNotPaused
        onlyPermittedAdapter
    {
        require(lockedBalances[_owner] >= bondAmount, "BB: exceed-balance");
        ...
        )
    {
        ...
        bondAmount += boostAmount + incentivesAmount;
        ...
        )
    {
        ...
        if (bondBalance >= toLock) {
            ...
            bondBalance + IERC20(BOND_TOKEN_ADDRESS()).balanceOf(recipient) >=
            toLock
        ) {
            toTransfer = toLock - bondBalance;
            toReduce = 0;
        } else {
            toTransfer = IERC20(BOND_TOKEN_ADDRESS()).balanceOf(recipient);
            toReduce = takerToSwap(token, toLock - toTransfer - bondBalance);
        }
    }
        ...
        )
    {
        ...
        if (bondBalance >= toLock) {
            ...
            toTransfer = IERC20(BOND_TOKEN_ADDRESS()).balanceOf(recipient);
            toReduce = makerToSwap(token, toLock - toTransfer - bondBalance);
        }
    }
    ...
    /// @inheritdoc IBond
    function lock(address addr, uint amount)
        external
        override
        whenNotPaused
        onlyPermittedAdapter
    ...
    function _withdraw(address withdrawOf, uint amount) private {
        require(
            amount > 0 && depositedBalances[withdrawOf] >= amount,
            "BB: bad-amount"
        );

        depositedBalances[withdrawOf] -= amount;

        IERC20(BOND_TOKEN_ADDRESS()).safeTransfer(withdrawOf, amount);
    }
    ...
    function _lockBondPosition(
        ...
        if (depositedBalances[recipient] >= bondAmount)
            depositedBalances[recipient] -= bondAmount;
        else depositedBalances[recipient] = 0;
    }

    function liquidate(address _owner, uint bondAmount)
        external
        virtual
        override
        whenNotPaused
...
interface IBumpOracle {
    function lastPrice() external view returns (uint);
    function lastPriceTimestamp() external view returns (uint);
    function updateBumpPrice() external returns (uint);
}
...
interface ISwap {
    ...
    /// @param from function caller
    /// @param ethAmountIn amountIn
    /// @param tokenOut output token address
    /// @param amountOut output amount of {tokenOut} get from swap
    ...
    /// @param ethAmountOut output token ETH
