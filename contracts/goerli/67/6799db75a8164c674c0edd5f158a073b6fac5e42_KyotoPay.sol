...
contract KyotoPay is Ownable, Pausable, IKyotoPay {
    ...
    // MAX_FEE is denominated in DECIMALs.  I.e. 500 = 5%
    uint256 public constant MAX_FEE = 500;

    address public immutable uniswapSwapRouterAddress;

    // fee is denominated in DECIMALS.  For example, a value for fee of 200 = 2%
    uint256 public fee;
    ...
    constructor(uint256 _fee, address _uniswapSwapRouterAddress) {
    ...
    function pay(
        ...
        if (!(whitelistedInputTokens[_tokenAddress])) revert InvalidToken(); 
        ...
        // If the sender's token is the recipient's preferred token, transfer directly and stop execution
        if ((_tokenAddress == _preferences.tokenAddress) || !(areValidPreferences)) {
            _sendRecipientFunds(_recipient, _preferences.tokenAddress, _amountIn);
            return;
        }

        uint256 swapOutput = _executeSwap(
            _tokenAddress, _preferences.tokenAddress, _amountIn, _amountOut, _fee, _preferences.slippageAllowed
        );
        ...
        emit Payment(msg.sender, _recipient, _amountIn, _data);
    }
    ...
    function _executeSwap(
        ...
        ISwapRouter.ExactInputSingleParams memory params = ISwapRouter.ExactInputSingleParams({
            tokenIn: _tokenIn,
            tokenOut: _tokenOut,
            fee: _fee, // e.g. fee for a pool at 0.3% tier is 3000
    ...
    // internal function to get funds from the sender (too much noise in payment function)
    ...
    // internal function to send funds to the recipient (too much noise in payment function)
    function _sendRecipientFunds(address _tokenAddress, address _recipient, uint256 _amount) internal {
        ...
        uint256 ownerPayment = (_amount * fee) / DECIMALS;
    ...
    function setPreferences(Preferences calldata _preferences) external whenNotPaused {
        if (_preferences.slippageAllowed == 0) revert InvalidRecipientSlippage();
    ...
    // function to withdraw funds from this contract (only the owner)
    // note: we could make this always withdraw the maximum amount, but allowing a variable amount seems harmless
    function withdraw(address _tokenAddress, uint256 _amount) external onlyOwner {
        if (IERC20(_tokenAddress).balanceOf(address(this)) == 0) revert ZeroBalance();
...
interface IKyotoPay {
     ...
     */
    event Payment(address sender, address recipient, uint256 amountIn, bytes32 indexed data);
}
