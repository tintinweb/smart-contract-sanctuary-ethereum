...
contract ZBBERC721A is Ownable,ERC721A{
  constructor (
      string memory _name,
      string memory _symbol
  ) ERC721A (_name,_symbol) {
  }
  mapping(uint256 => string) internal _tokenUri;//tokenURI by tokenid
  ...
  function ownerMint( address _address, string memory _uri) external virtual onlyOwner {
    _tokenUri[_nextTokenId()] = _uri;//set tokenuri
  ...
  function _beforeTokenTransfers(address from_,address to_,uint256 startTokenId_,uint256 quantity_) internal virtual override {
    require(msg.sender == owner() || from_ == address(0) || to_ == address(0) ,"SBT mode Enabled: token transfer while paused.");
