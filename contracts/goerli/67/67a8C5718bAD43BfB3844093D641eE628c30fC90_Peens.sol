...
contract Peens is ERC721A, Ownable, ERC2981 {
    ...
    mapping(address => uint256[]) public phase1Burners;
    mapping(address => uint256[]) public phase2Burners;
    mapping(address => uint256[]) public phase3Burners;
    mapping(address => uint256[]) public phase4Burners;

    uint[] public phase1Burns;
    uint[] public phase2Burns;
    uint[] public phase3Burns;
    uint[] public phase4Burns;
    ...
    event burnEvent(address _sender, uint[] tokens);
    ...
    function chadMint(uint256 qty) external payable {
        if (msg.sender != tx.origin) revert ContractMint();
        
        //0x8FA600364B93C53e0c71C7A33d2adE21f4351da3 -> Larva Chads mainnet
        uint balance = IERC721(0xE492a92a6148eBA1eD139d8a09a2A50a3ec48224).balanceOf(msg.sender);
        if (balance <= 0) revert NotAChadHolder();
        if (qty > (balance * 2)) revert InvalidQuantity();

        if(chadMintClaimed[msg.sender]) revert AlreadyClaimed();
    ...
    modifier burnCompliance(uint[] calldata tokens) {
        if (tokens.length % 2 != 0) revert InvalidQuantity();
    ...
    function phaseOne(uint[] calldata tokens) public burnCompliance(tokens) {
        for (uint i=0; i < tokens.length; i++) {
            phase1Burns.push(tokens[i]);
            phase1Burners[msg.sender].push(tokens[i]);
            _burn(tokens[i], true);
        }
    ...
    //// poolik
    /*
    function phaseTwo(uint[] calldata tokens) public burnCompliance(tokens) {
        uint[] storage valid;
        for (uint i=0; i < tokens.length; i++) {
            for (uint j=0; j < phase1Burns.length; j++) {
                if (tokens[i] == phase1Burns[i]) {
                    valid.push(tokens[i]);
                }
            }
            phase2Burns.push(tokens[i]);
            pahse2Burners[msg.sender].push(tokens[i]);
            _burn(tokens[i], true);
        }
        emit burnEvent(msg.sender, tokens);
    }
