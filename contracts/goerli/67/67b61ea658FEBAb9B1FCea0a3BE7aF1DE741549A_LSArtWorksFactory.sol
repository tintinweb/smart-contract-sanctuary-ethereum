...
// File contracts/license/ICantBeEvil.sol
...
// File contracts/license/CantBeEvil.sol
...
// File contracts/token/LSArtWorks721.sol
...
contract LSArtWorks721 is ERC721, CantBeEvil {
  ...
  ) ERC721("LSArtWorks721", "LSA") CantBeEvil(license) {
    // factory = msg.sender;
    owner = _owner;
  ...
  function mint() external {
    ...
    // mint works
    _mintArtWorks(mintAmount);
  }

  function recordForWhiteList(uint256 _mintAmount, address _receiver) external onlyFund {
    require(!fund.checkProjectCompleted(artWorkProjectId), "project has been completed");
  ...
  /** ========== internal functions ========== */
  function _mintArtWorks(uint256 _mintAmount) internal {
    for (uint256 i = 0; i < _mintAmount; i++) {
      ...
      emit ArtWorkMinted(msg.sender, artWorkProjectId, currentTokenId);
    }
  }
...
// File contracts/token/LSArtWorksFactory.sol
...
contract LSArtWorksFactory {
  IFundraising public immutable fundraising;

  constructor(address _fundraising) {
    fundraising = IFundraising(_fundraising);
  }

  /** ========== main function ========== */
  // function createArtWork(address _owner, uint256 _proposalId)
  //   external
  //   OnlyFundraising
  //   returns (address artwork)
  // {
  //   bytes memory bytecode = type(LSArtWorks).creationCode;
  //   bytes32 salt = keccak256(
  //     abi.encodePacked(keccak256(abi.encode("LeapStart NFT")), _owner)
  //   );
  //   assembly {
  //     artwork := create2(0, add(bytecode, 32), mload(bytecode), salt)
  //   }

  //   ILSArtWorks(artwork).initialize(_owner, _proposalId, address(fundraising));

  //   emit ArtWorksCreated(artwork);
  // }
  ...
  function createArtWork(
    ...
    uint256 _royaltyShare,
    License.LicenseVersion _license
  ) public OnlyFund returns (address) {
    LSArtWorks721 artwork = new LSArtWorks721(_artWorkOwner, _projectId, _royaltyShare, address(fundraising), _license);

    emit ArtWorkCreated(_artWorkOwner, address(artwork), _projectId, _royaltyShare, _license);

    return address(artwork);
  }

  modifier OnlyFund() {
    require(msg.sender == address(fundraising), "only allow internal contract");
    _;
  }

  /** ========== event ========== */
  event ArtWorkCreated(
    address indexed artWorkOwner,
    address indexed artWorkAddrses,
