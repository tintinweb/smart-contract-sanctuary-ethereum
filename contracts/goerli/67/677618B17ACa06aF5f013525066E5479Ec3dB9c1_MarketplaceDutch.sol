...
contract MarketplaceDutch is Ownable, ReentrancyGuard
{
    ...
    struct _UserSaleInformation
    {
        ...
        bool _Active;                  // [11] -> _Active                 | If The Sale Is Active
    }
    ...
    function Purchase (
        ...
        uint Credit;
        if(_IS._ActiveRespend) { Credit = __ActiveRespend(SaleIndex, CurrentPrice, PurchaseValue, msg.sender); }
        uint TotalETHContributed = msg.value + Credit;
        require(TotalETHContributed >= PurchaseValue, "DutchMarketplace: Incorrect ETH Amount Sent");
        if(TotalETHContributed > PurchaseValue) { __Refund(msg.sender, (TotalETHContributed - PurchaseValue - Credit)); }
        ...
        if(_IS._Type == 0) { IERC721(_IS._NFT)._MintToFactory(msg.sender, Amount, Sales[SaleIndex]._ProjectIDMintPass); } // Simple Direct Mint
        ...
        else if (_IS._Type == 4) // transferFrom() MintPack Sale Implementation
        {
        ...
        emit Purchased(Recipient, Amount, PurchaseValue, NewAmountSold, BrightListEligible, Priority, Credit);
    }
    ...
    function __StartSale(
        ...
        require(
            ...
            ||
            _InternalSale._Type == 2 // transferFrom() Sale (NFTs Already Minted)
            ||
            _InternalSale._Type == 3 // purchaseTo() Sale (ArtBlocks Or Custom Mint Pass)
            ||
            _InternalSale._Type == 4 // transferFrom() MintPack Sale (NFTs Already Minted)
...
interface IERC721 
{ 
    function _MintToFactory(address To, uint Amount, uint ProjectID) external;
