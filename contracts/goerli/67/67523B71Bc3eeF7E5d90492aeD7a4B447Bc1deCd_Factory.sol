...
contract Factory is ERC721("Unbond Vault", "Vault"), Ownable {
    ...
    function mint() external {
        address vault = _createClone();

        // cache
        uint256 _totalSupply = totalSupply;

        tokenIdToAddress[_totalSupply] = vault;
        addressToTokenId[vault] = _totalSupply;
    ...
    function ownerOfVault(address _vault) external view returns (address) {
        uint256 tokenId = addressToTokenId[_vault];
     ...
     */
    function _createClone() internal returns (address result) {
