...
import '../Uniswap/V2/TransferHelper.sol';
...
contract SweepAMOMinter is Owned {
    ...
    function mintSweepForAMO(address destination_amo, uint256 sweep_amount) external validAMO(destination_amo) {
    ...
    function burnSweepFromAMO(address destination_amo, uint256 sweep_amount) external validAMO(destination_amo) {
        int256 sweep_amt_i256 = int256(sweep_amount);
    ...
    // Adds an AMO
    function addAMO(address amo_address, bool sync_too) public {
    ...
    // Removes an AMO
    function removeAMO(address amo_address, bool sync_too) public {
        require(amo_address != address(0), "Zero address detected");
    ...
    function refreshTargetPrice(uint _price) external validAMO(msg.sender) {
        SWEEP.refreshTargetPrice(_price);
    }
...
interface IAMO {
    ...
    function swap(address _tokenA, address _tokenB, uint24 _fee_tier, uint256 _amountAtoB) external returns (uint256);
}
