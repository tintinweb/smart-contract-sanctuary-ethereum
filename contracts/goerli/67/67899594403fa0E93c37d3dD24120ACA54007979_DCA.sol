...
contract DCA {
    ...
    uint256 public totalFundedByOwner; // from the owner for the swap cost
    ...
    event NewUser(
        uint256 id,
        address userAddress,
        uint256 StableCoinAllowancePerPeriod,
        uint256 StableCoinFund
    );
    ...
    function deposit() external payable onlyOwner {
        totalFundedByOwner = totalFundedByOwner + msg.value;
    }
        ...
        payable
    {
        ...
        emit NewUser(totalUsers, msg.sender, amountPerPeriod, stablecoinAmount);
    ...
    function scheduleSwap() external {
        ...
        for (uint256 i = 0; i < totalUsers; i++) {
            ...
            if (user.stablecoinBalance >= user.amountPerPeriod + swapFee) {
                ...
                // add fee to contract balance
                _stablecoinEarned = _stablecoinEarned + swapFee;
                // update user in mapping
                _users[userIdToAddress[i]].stablecoinBalance = user
                    .stablecoinBalance;
        ...
        if (totalAmountIn > 0) {
            for (uint256 i = 0; i < totalUsers; i++) {
                ...
                // user.allocation will be zero, which is almost guaranteed with more than 1 user
                user.allocation = user.amountPerPeriod / totalAmountIn;
                // update user in mapping
                _users[userIdToAddress[i]].allocation = user.allocation;
            }
            ...
            for (uint256 i = 0; i < totalUsers; i++) {
                ...
                // update user in mapping
                _users[userIdToAddress[i]].ethBalance = user.ethBalance;
            }
        }
