    ...
    ContextMixin
{
    ...
    // Mapping PostId and TokenId
    mapping(string => uint256) public postId;
    ...
    function initialize (
      ...
      uint256 maxTokenId_
        ...
        external
    {
        // For the base contract, isBae == true, Imposible to use
        require((isBase == false), "ChildNFTERC1155: ERROR Base contract cannot be initialized");
        // Owner address default to address(0). Once this function is called,
        // There is no way to call again this function
        require(owner == address(0), "ChildNFTERC1155: ERROR Contract already initialized");
        owner = owner_;
    ...
    */
    ...
    function mint(
        ...
        require( ((id > uint(0)) && (id <= uint(MAX_TOKEN_ID))), "ChildNFTERC1155: INVALID_TOKEN_ID");
        require((bytes(_postId).length >= 7), "ChildNFTERC1155: INVALID_POST_ID");
    ...
    function mintBatch(
        ...
        for (uint i = 0; i < ids.length; i++) {
            require(((ids[i] > uint(0)) && (ids[i] <= uint(MAX_TOKEN_ID))), "ChildNFTERC1155: INVALID_TOKEN_ID");
            require((bytes(_postId[i]).length >= 7), "ChildNFTERC1155: INVALID_POST_ID");
            require(((amounts[i] > uint(0)) && (amounts[i] == restTotalSupply(ids[i], amounts[i]))) , "ChildNFTERC1155: INVALID_TOKEN_AMOUNT");
        }
        _mintBatch(to, ids, amounts, data);
        for (uint j = 0; j < ids.length; j++) {
            postId[_postId[j]] = ids[j];
        }
    }
