...
contract EventUnlockSchedule is BaseGovernanceWithUserUpgradable, KeeperCompatibleInterface {
    ...
    int public triggerOuterCount;
    int public triggerInnerCount;
    //////////TESTING ONLY///////////

    event Triggered(uint index, address lockAddress, address upkeepConsumer);
    ...
    function checkUpkeep(bytes calldata checkData) external view override returns (bool upkeepNeeded, bytes memory performData) {
        ...
        if(block.timestamp < lockStart()){
            ...
            if(block.timestamp >= lockEnd()){
                ...
                if(offsetPrices.length > 0 && !isPaused){
                    for(uint i = 0; i < offsetPrices.length; i++) {
                        if(conditions[i] == 3){
                            if(price == offsetPrices[i]){
                                idx = i;
                                upkeepNeeded = true;
                                performData = abi.encode(idx,lockAddress,upkeepConsumer);
            ...
            if(block.timestamp < lockEnd()){
                if(offsetPrices.length > 0){
                    //for(uint32 i = 0; i < offsetPrices.length; i++) {
                        if(conditions[_idx] == 0){
                            if(price == offsetPrices[_idx]){
                                _unlockAmounts(_idx, _lockAddress, _upkeepConsumer);
                                //triggerInnerCount += 1;
                            }
                        }
                        if(conditions[_idx] == 1){
                            if(price < offsetPrices[_idx]){
                                _unlockAmounts(_idx, _lockAddress, _upkeepConsumer);
                                //triggerInnerCount += 20;
                            }
                        }
                        if(conditions[_idx] == 2){
                            if(price > offsetPrices[_idx]){
                                _unlockAmounts(_idx, _lockAddress, _upkeepConsumer);
                                //triggerInnerCount += 600;
                            }
                        }
                    //}
                }
    ...
    function _unlockAmounts(uint _index, address _lockAddress, address _upkeepConsumer) internal {
        ...
        isPaused = false;
        emit Triggered(_index, _lockAddress, _upkeepConsumer);
    }
