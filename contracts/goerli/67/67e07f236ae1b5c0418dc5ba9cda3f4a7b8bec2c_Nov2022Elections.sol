...
contract Nov2022Elections is Ownable {
        ...
        returns (string memory fullname)
    {
        for (uint256 i = 0; i < winnerGovernors[state].length; ) {
            ...
            if (compareTwoStrings(governor, _fullname) == true)
                return (governor);
    ...
    function setWinnerGovernor(string calldata governor, string calldata state)
        external
        onlyOwner
    {
        winnerGovernors[state].push(governor);
        emit SetWinnerGovernor(state, governor);
    }
