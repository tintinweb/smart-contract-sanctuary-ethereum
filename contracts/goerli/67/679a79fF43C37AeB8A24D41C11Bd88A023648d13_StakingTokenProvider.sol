...
contract StakingTokenProvider is OwnableUpgradeable {
  ...
  uint24 public defaultFee;  // fee for uniswap v3 pair
  ...
  function stakingTokenForVaultToken(address _vaultToken) external view returns (address) {
    ...
    return pairFor(uniLikeExchange, _vaultToken, _pairedToken, defaultFee);
  }
  ...
  function pairFor(address factory, address tokenA, address tokenB, uint24 fee) internal pure returns (address pair) {
      (address token0, address token1) = sortTokens(tokenA, tokenB);
      pair = address(uint160(uint256(keccak256(abi.encodePacked(
              hex'ff',
              factory,
              keccak256(abi.encodePacked(token0, token1, fee)),
              hex'e34f199b19b2b4f47f68442619d555527d244f78a3297ea89325f843f87b8b54' // init code hash
      )))));
  }
}
