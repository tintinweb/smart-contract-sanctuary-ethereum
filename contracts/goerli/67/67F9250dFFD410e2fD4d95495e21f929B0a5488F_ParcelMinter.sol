...
import "../interfaces/IDirectSaleFundsStorage.sol";
import {Minter, EIP712_DOMAIN_HASH} from "../Minter.sol";
import "./Parcels.sol";
import "../interfaces/IERC20Burnable.sol";

contract ParcelMinter is Minter {
    IParcel public parcel;

    constructor(
        ContractControlList contractControlList_,
        IParcel parcel_,
        IDirectSaleFundsStorage fundsStorage_
    ...
    function _roleRequired(address signer) internal view {
        require(
            contractControlList.hasRole(
                contractControlList.PARCEL_MINTER_ROLE(),
                signer
            ),
            "ParcelMinter: Message signer has no minter role"
        );
    }

    /*
     * @dev Returns eip712-compatible encoded hash required to be signed by priv key with `PARCEL_MINTER_ROLE` for `mintParcel`
     */
    function getMintParcelSigHash(
        address to_,
        uint256 parcelId_,
        uint256 nativePrice_,
        IParcel.MintParams memory mintParams_,
            ...
            keccak256(
                abi.encodePacked(
                    ...
                    keccak256(
                        abi.encode(
                            keccak256(
                                "mintParcel(address to_, uint256 parcelId_, uint256 nativePrice_, IParcel.MintParams memory mintParams_, uint256 signatureDeadline_, bytes memory signature_)"
                            ),
                            to_,
                            parcelId_,
                            mintParams_,
    ...
    /**
     * @dev Payable public function which allows user to buy Parcel with ETH.
     * @param to_ - receiver address
     * @param parcelId_ - tokenId which user wants to mint
     * @param nativePrice_ - token price in ETH
     * @param mintParams_ - `IParcel.MintParams` struct with minting parameters
     * @param signatureDeadline_ - timestamp of signature invalidation
     * @param signature_ - message signed by user with `PARCEL_MINTER_ROLE`
     *
     */
    function mintParcel(
        address to_,
        ...
        require(msg.value == nativePrice_, "ParcelMinter: Wrong ETH amount sent");
        ++mintsPerWallet[to_];

        bytes32 messageHash = getMintParcelSigHash(
            to_,
            parcelId_,
            nativePrice_,
            mintParams_,
        ...
        parcel.mintTo(to_, parcelId_, mintParams_);

        _forwardPayment();
    }

    /*
    * @dev Returns eip712-compatible encoded hash required to be signed by priv key with `PARCEL_MINTER_ROLE` for `mintParcelWithERC20`
     */
    function getMintParcelWithERC20SigHash(
        address to_,
        uint256 parcelId_,
        uint256 erc20Price_,
        IERC20 erc20Address_,
        IParcel.MintParams memory mintParams_,
        uint256 signatureDeadline_
    ) public view returns (bytes32) {
        return
        keccak256(
            abi.encodePacked(
                ...
                keccak256(
                    abi.encode(
                        keccak256(
                            "mintParcelWithERC20(address to_, uint256 parcelId_, uint256 erc20Price_, IERC20 erc20Address_, IParcel.MintParams memory mintParams_, uint256 signatureDeadline_, bytes memory signature_)"
                        ),
                        to_,
                        parcelId_,
                        erc20Price_,
    ...
    /**
     * @dev Payable public function which allows user to buy Parcel with ERC20.
     * @param to_ - receiver address, this address pays for mint, erc20 'approval' might be required
     * @param parcelId_ - tokenId which user wants to mint
     * @param erc20Price_ - token price in ERC20
     * @param erc20Address_ - address of ERC20
     * @param mintParams_ - `IParcel.MintParams` struct with minting parameters
     * @param signatureDeadline_ - timestamp of signature invalidation
     * @param signature_ - message signed by user with `PARCEL_MINTER_ROLE`
     *
     */
    function mintParcelWithERC20(
        address to_,
        uint256 parcelId_,
        uint256 erc20Price_,
        IERC20 erc20Address_,
        ...
        bytes32 messageHash = getMintParcelWithERC20SigHash(
            to_,
            parcelId_,
            erc20Price_,
            erc20Address_,
        ...
        parcel.mintTo(to_, parcelId_, mintParams_);
        require(erc20Address_.transferFrom(to_, address(fundsStorage), erc20Price_), "ParcelMinter: transferFrom failed");
    }

    /*
    * @dev Returns eip712-compatible encoded hash required to be signed by priv key with `PARCEL_MINTER_ROLE` for `mintParcelAndBurnERC20`
     */
    function getMintParcelAndBurnERC20SigHash(
        address to_,
        uint256 parcelId_,
        uint256 erc20Price_,
        IERC20Burnable erc20Address_,
        IParcel.MintParams memory mintParams_,
        uint256 signatureDeadline_
    ) public view returns (bytes32) {
        return
        keccak256(
            abi.encodePacked(
                ...
                keccak256(
                    abi.encode(
                        keccak256(
                            "mintParcelAndBurnERC20(address to_, uint256 parcelId_, uint256 erc20Price_, IERC20Burnable erc20Address_, IParcel.MintParams memory mintParams_, uint256 signatureDeadline_, bytes memory signature_)"
                        ),
                        to_,
                        parcelId_,
                        mintParams_,
    ...
    /**
     * @dev Payable public function which allows user to buy Parcel and burn ERC20 in exchange.
     * @param to_ - receiver address, this address pays for mint, erc20 'approval' might be required
     * @param parcelId_ - tokenId which user wants to mint
     * @param erc20Price_ - token price in ERC20
     * @param erc20Address_ - address of ERC20 with IERC20Burnable interface
     * @param mintParams_ - `IParcel.MintParams` struct with minting parameters
     * @param signatureDeadline_ - timestamp of signature invalidation
     * @param signature_ - message signed by user with `PARCEL_MINTER_ROLE`
     *
     */
    function mintParcelAndBurnERC20(
        address to_,
        uint256 parcelId_,
        uint256 erc20Price_,
        IERC20Burnable erc20Address_,
        ...
        bytes32 messageHash = getMintParcelAndBurnERC20SigHash(
            to_,
            parcelId_,
            erc20Price_,
            erc20Address_,
        ...
        parcel.mintTo(to_, parcelId_, mintParams_);
        erc20Address_.burnFrom(to_, erc20Price_);
    }
}
...
import "./IDotEarth721.sol";

interface IParcel is IERC721, IDotEarth721 {
    enum PurchaseType {
        Undefined,
        PartnerDeal,
        DirectMint,
    ...
    function isPremiumParcel(uint256 tokenId) external view returns(bool);
...
import "./IFundsStorage.sol";

interface IDirectSaleFundsStorage is IFundsStorage, IERC20FundsStorage {
...
contract Minter is ActionContext {
    using ECDSA for bytes32;

    ContractControlList public contractControlList;
    IDirectSaleFundsStorage public fundsStorage;

    uint256 public maxMintsPerWallet = 2**256 - 1;
    ...
    modifier assertMaxMints(address to_) {
        require(
            mintsPerWallet[to_] < maxMintsPerWallet,
            "Minter: Cannot mint more (max mints)"
        );
        _;
    }

    modifier assertTotalSupply(IDotEarth721 token_, uint256 max_) {
        require(
            token_.totalSupply() < max_,
            "Minter: Cannot mint more (max supply)"
        );
        _;
    }
    ...
    function setMaxMintsLimit(uint256 maxMintsPerWallet_)
        external
        onlyAdmin
    {
    ...
    function setFundsWallet(IDirectSaleFundsStorage fundsStorage_) external onlyAdmin {
        require(
            address(fundsStorage_) != address(0),
            "Minter: WalletAddr cannot be zero "
        );
...
import "../interfaces/ISoftStaking.sol";

//solhint-disable no-empty-blocks
contract Parcels is DotEarthERC721, IParcel, ISoftStaking {
    uint256 public unlockOfPartnerToken;
    mapping(uint256 => MintParams) internal params;

    // soft-staking
    mapping(uint256 => bool) public override isStaking;
    mapping(uint256 => uint256) internal lastStakingStarted_;
    mapping(uint256 => uint256) internal totalStakingTime_;
    ...
    function supportsInterface(bytes4 interfaceId)
        ...
        view
        virtual
        override (DotEarthERC721Royalty, IERC165)
        returns (bool)
    {
        return
            interfaceId == type(IParcel).interfaceId ||
            interfaceId == type(ISoftStaking).interfaceId ||
            super.supportsInterface(interfaceId);
    }

    modifier onlyMinterRole() {
        accessController.checkParcelMinterRole(_msgSender());
        _;
    }

    function getAdminRole() public view override returns(bytes32) {
        return accessController.PARCEL_ADMIN_ROLE();
    }
    ...
    function mint(uint256 tokenId_, MintParams memory mintParams_)
        ...
        onlyMinterRole
    {
        _mintTo(_msgSender(), tokenId_, mintParams_);
    }
    ...
    function _beforeTokenTransfer(
        ...
        if (params[tokenId].purchaseType == PurchaseType.PartnerDeal) {
            require(
                ...
                "Parcels: Cannot transfer Partner token yet"
            );
        }

        require(lastStakingStarted_[tokenId] == 0, "Parcels: token is staking");
    ...
    function toggleStaking(uint256 tokenId) external override {
        require(
            ...
            "Parcels: not approved or owner"
        );

        if (!isStaking[tokenId]) {
            lastStakingStarted_[tokenId] = block.timestamp;
            isStaking[tokenId] = true;
            emit Staked(tokenId);
        } else {
            isStaking[tokenId] = false;
            totalStakingTime_[tokenId] = block.timestamp - lastStakingStarted_[tokenId];
            emit Unstaked(tokenId);
        }
    }

    function totalStakedTime(uint256 tokenId) external view override returns(uint256) {
        if (isStaking[tokenId]) {
            return totalStakingTime_[tokenId] + block.timestamp - lastStakingStarted_[tokenId];
        }
        return totalStakingTime_[tokenId];
    }
    ...
    function isPremiumParcel(uint256 tokenId)
        external
        view
        override
        returns (bool)
        ...
        returns (PurchaseType)
    {
        ...
        require(
            isPurchased(tokenId),
            "Parcel: has not been purchased yet or does not exist"
        );
...
interface IDotEarth721 {
    ...
    function emergencyTransferFrom(
        address from,
        address to,
        uint256 tokenId
    ) external;
...
contract ContractControlList is AccessControl {
    ...
    bytes32 public constant PARCEL_ADMIN_ROLE = keccak256("PARCEL_ADMIN_ROLE");
    ...
    bytes32 public constant PARCEL_MINTER_ROLE = keccak256("PARCEL_MINTER_ROLE");
    ...
    // Parcel, Erthling and Founder contracts should have this role assigned for acl-s checks between them
    bytes32 public constant PARCEL_ROLE = keccak256("PARCEL_ROLE");
    ...
    constructor(address admin_) {
        ...
        _setupRole(PARCEL_ADMIN_ROLE, admin_);
        _setupRole(ERTHLING_ADMIN_ROLE, admin_);
        _setupRole(ROYALTY_ADMIN_ROLE, admin_);
        _setupRole(FACTORY_ADMIN_ROLE, admin_);

        _setRoleAdmin(PARCEL_MINTER_ROLE, PARCEL_ADMIN_ROLE);
        _setRoleAdmin(PARCEL_ROLE, PARCEL_ADMIN_ROLE);
    ...
    function grantParcelMinterRole(address addr_) external {
        grantRole(PARCEL_MINTER_ROLE, addr_);
    }
    ...
    function grantParcelAdminRole(address addr_) external {
        grantRole(PARCEL_ADMIN_ROLE, addr_);
    }

    function grantParcelRole(address addr_) external {
        grantRole(PARCEL_ROLE, addr_);
    }
    ...
    function checkParcelMinterRole(address addr_) external view {
        _checkRole(PARCEL_MINTER_ROLE, addr_);
    }

    function checkParcelAdminRole(address addr_) external view {
        _checkRole(PARCEL_ADMIN_ROLE, addr_);
    }

    function checkParcelRole(address addr_) external view {
        _checkRole(PARCEL_ROLE, addr_);
    }
    ...
    function checkActionAdminRole(address addr_) external view {
        _checkRole(ACTION_ADMIN_ROLE, addr_);
    }
...
contract ActionContext is Context {
    ...
    function executeAction(bytes calldata call_, ActionParams calldata actionParams_) external payable keepOriginalSender returns(bytes memory) {
        // solhint-disable-next-line avoid-low-level-calls
        (bool success, bytes memory returnData) = address(this).call{value: msg.value}(call_);
        if (success) {
...
interface IERC20FundsStorage {
    function withdrawERC20(address to_, IERC20 erc20_) external;
    function balanceERC20(IERC20 erc20) external view returns(uint256);
}
...
interface IERC721Mintable {
     ...
     */
    function mintTo(address to_, uint256 tokenId_) external;
...
// Ownable is required and used only to set up collection on OpenSea
import "@openzeppelin/contracts/access/Ownable.sol";
import "./interfaces/IDotEarth721.sol";
import "./DotEarthERC721Royalty.sol";
import "./ActionContext.sol";

abstract contract DotEarthERC721 is Ownable, DotEarthERC721Royalty, IDotEarth721 {
    ...
    constructor(
        ...
        string memory baseURI_,
        address owner_
    ) DotEarthERC721Base(name_, symbol_) {
    ...
    modifier onlyAclAdminRole() {
        accessController.checkAclAdminRole(_msgSender());
        _;
    }
    ...
    function getAdminRole() public view virtual returns(bytes32);

    modifier onlyAdminRole() {
        accessController.checkRole(getAdminRole(), _msgSender());
        _;
    }
    ...
    /**
    * @dev Gas optimised version of regular OpenZeppelin _burn function with totalSupply counting and OpenSea support.
    *
    * Override of that function is required to support OpenSea burn flow.
    * OpenSea requires that burned token emits transfer event with `to` == `0x000000000000000000000000000000000000dEaD`
    */
    function _burn(uint256 tokenId) internal virtual override {
        address owner = DotEarthERC721Base.ownerOf(tokenId);

        _beforeTokenTransfer(owner, BURN_ADDRESS, tokenId);
        ...
        _afterTokenTransfer(owner, BURN_ADDRESS, tokenId);

        super._burn(tokenId);
    }

    /**
    * @dev Gas optimised version of regular OpenZeppelin _mint function with totalSupply counting
     */
    ...
    function emergencyTransferFrom(
        ...
        address to,
        uint256 tokenId
    ) external virtual override onlyAdminRole {
        _transfer(from, to, tokenId);
    }

    function _msgSender() internal view virtual override(ActionContext, Context) returns (address) {
        return ActionContext._msgSender();
    }
}
...
interface ISoftStaking {
    ...
    function totalStakedTime(uint256 tokenId) external view returns(uint256);
}
...
// SPDX-License-Identifier: MIT
// OpenZeppelin Contracts (last updated v4.5.0) (token/ERC721/extensions/DotEarthERC721Royalty.sol)

pragma solidity ^0.8.0;

import "@openzeppelin/contracts/token/common/ERC2981.sol";
import "@openzeppelin/contracts/utils/introspection/ERC165.sol";
import "./DotEarthERC721Base.sol";
 ...
 */
abstract contract DotEarthERC721Royalty is ERC2981, DotEarthERC721Base {
    /**
     * @dev See {IERC165-supportsInterface}.
     */
    function supportsInterface(bytes4 interfaceId) public view virtual override(DotEarthERC721Base, ERC2981) returns (bool) {
 ...
 * {ERC721Enumerable}.
 *
 * @dev Purpose of copying that code is to overwrite _burn and _mint methods as our NFT implementation requires that.
 */
abstract contract DotEarthERC721Base is ActionContext, ERC165, IERC721, IERC721Metadata {
    ...
    function _transfer(
        ...
        require(DotEarthERC721Base.ownerOf(tokenId) == from, "ERC721: transfer from incorrect owner");
    ...
    function _approve(address to, uint256 tokenId) internal virtual {
        ...
        emit Approval(DotEarthERC721Base.ownerOf(tokenId), to, tokenId);
    }
