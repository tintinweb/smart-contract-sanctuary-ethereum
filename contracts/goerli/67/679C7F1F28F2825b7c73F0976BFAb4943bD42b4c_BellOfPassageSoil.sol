...
contract BellOfPassageSoil is ERC721Enumerable, Ownable, ReentrancyGuard {
    ...
    modifier _checkPerUser1(uint256 amount) {
        require(checkPerUser1(amount), "exceed per user 1");
        _;
    }

    modifier _checkPerUser2(uint256 amount) {
        require(checkPerUser2(amount), "exceed per user 2");
        _;
    }

    modifier _checkPerUserP(uint256 amount) {
        require(checkPerUserP(amount), "exceed per user p");
        _;
    }
    ...
    // wl1 mint over or period starts
    ...
    function checkPerUser1(uint256 amount) public view returns (bool) {
        return _wl1MintedPerUser[msg.sender] + amount <= _wl1MaxPerUser;
    }

    function checkPerUser2(uint256 amount) public view returns (bool) {
        return _wl2MintedPerUser[msg.sender] + amount <= _wl2MaxPerUser;
    }

    function checkPerUserP(uint256 amount) public view returns (bool) {
        return _wlPMintedPerUser[msg.sender] + amount <= _wlPMaxPerUser;
    }
