...
contract Plasma is ERC20, AccessControlMixin, NativeMetaTransaction, ContextMixin, IMintableERC20 {
    ...
    function addMultipleMinters(address[] calldata newMinters) external only(DEFAULT_ADMIN_ROLE) {
        for (uint256 index = 0; index < newMinters.length; index++) {
            ...
            grantRole(DEPOSITOR_ROLE, _minter);
        }
    }
    ...
    function removeMultipleMinters(address[] calldata mintersToRemove) external only(DEFAULT_ADMIN_ROLE) {
        for (uint256 index = 0; index < mintersToRemove.length; index++) {
            ...
            revokeRole(DEPOSITOR_ROLE, _minter);
        }
    }
}