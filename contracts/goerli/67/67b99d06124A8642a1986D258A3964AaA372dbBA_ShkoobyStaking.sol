...
contract ShkoobyStaking is Ownable, Pausable, ReentrancyGuard {
    using SafeMath for uint256;
    ...
    modifier updateReward(bool _withdrawl, uint256 _id) {
        ...
        for(uint i = 0; i < user.orders.length; i ++) {
            if(user.orders[i] == _id) {
                emptyOrder = false;
            }
        }

        require(!emptyOrder, "There is no your order!");

        if(order.lockupDuration == 0) {

            uint256 multiplier = Math.min(block.timestamp, endTime).sub(order.depositedAt);
            uint256 reward = order.amount.mul(rewardRate).mul(1e12).div(100 * 365 * 24 * 60 * 60);
            uint256 rewardAmount = multiplier.mul(reward).div(1e12);
            ...
            require(block.timestamp >= order.depositedAt + order.lockupDuration, "You cannot withdraw yet!");
            ...
            bonus = rewardAmount.mul(lockOption[order.lockupDuration]).div(100);

            totalReward = totalReward.add(rewardAmount.add(bonus));
            lastUpdateTime = Math.min(block.timestamp, endTime);

            user.rewardAmount = rewardAmount.add(bonus);
    ...
    function deposit(uint256 amount, uint256 _lockupDuration) external nonReentrant whenNotPaused {
        require (block.timestamp < endTime, "expired");
        require (amount >= minDepositAmount && amount < maxDepositAmount, "invalid deposit amount");

        uint256 orderId = _getNextOrderID();
        ...
        order.lockupDuration = _lockupDuration;
    ...
    function withdraw(uint256 amount, uint256 _id) public nonReentrant updateReward(true, _id) {
        
        UserInfo storage user = userInfo[msg.sender];
        OrderInfo storage order = orderInfo[_id];

        require(amount > 0 && order.amount >= amount, "!amount");
        ...
        if (user.rewardAmount > 0) {
            claimedAmount = _safeTransferRewards(msg.sender, user.rewardAmount);
        }

        user.rewardAmount = user.rewardAmount.sub(claimedAmount);
        ...
        if (order.amount == 0) {
            ...
            for(uint i = 0; i < user.orders.length; i ++) {
                if(user.orders[i] == _id) {
                    uint256 lastId = user.orders[user.orders.length - 1];
                    user.orders[i] = lastId;
                    user.orders.pop();
                }
            }
        }
    ...
    function withdrawAll(uint256 _id) external {
        ...
        withdraw(order.amount, _id);
    }

    function claim(uint256 _id) public nonReentrant updateReward(false, _id) {
        UserInfo storage user = userInfo[msg.sender];
        // OrderInfo storage order = orderInfo[_id];

        require (block.timestamp.sub(user.claimedAt) >= rewardCycle, "!available still");
        
        uint256 claimedAmount = _safeTransferRewards(msg.sender, user.rewardAmount);
        user.rewardAmount = user.rewardAmount.sub(claimedAmount);
        user.claimedAt = block.timestamp;
        totalReward = totalReward.sub(claimedAmount);
    ...
    function claimable(uint256 _id) external view returns (uint256) {
        OrderInfo storage order = orderInfo[_id];
