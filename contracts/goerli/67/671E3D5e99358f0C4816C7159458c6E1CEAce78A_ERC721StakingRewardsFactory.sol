...
contract ERC721StakingRewardsFactory is Ownable {
    ...
    function deploy(
        ...
        string memory _backgroundUrl,
        string[] memory _colors
    ) external returns (address) {
        require(deployerAddress == msg.sender, "Not authorized");
        ERC721StakingRewards _contract = new ERC721StakingRewards(_erc20Address, _vaultAddress, _logoUrl, _backgroundUrl, _colors);
...
abstract contract ERC721Vault {
    ...
    function clearDepositTimesOf(address _address, address _contract, uint8 _type) public virtual;
    function getConfiguration(uint8 _type) public virtual view returns (bool);
    function stakedCount(uint8 _type, address _contract) public virtual returns (uint256);
}
...
contract ERC721StakingRewards is Ownable, ReentrancyGuard {
    ...
    constructor(
        ...
        for (uint256 i = 0; i < _colors.length; i++) colors.push(_colors[i]);
        token = ERC20Token(_erc20Address);
    ...
    function setLogoUrl(string memory _url) external onlyOwner {
        logoUrl = _url;
    }

    function setBackgroundUrl(string memory _url) external onlyOwner {
        backgroundUrl = _url;
    }
    ...
    function addCollections(address[] memory _contracts, uint72[] memory _softRates, uint72[] memory _hardRates) external onlyOwner {
        for (uint256 i = 0; i < _contracts.length; i++) {
            ...
            collections[_contract].hardRate = _hardRates[i];
            collections[_contract].softRate = _softRates[i];
    ...
    function updateCollection(address _contract, uint72 _softRate, uint72 _hardRate) public onlyOwner {
        require(collections[_contract].exists, "Bad collection");
        collections[_contract].hardRate = _hardRate;
        collections[_contract].softRate = _softRate;
    }

    function removeCollection(address _contract) public onlyOwner {
        require(vault.stakedCount(HARD_STAKED, _contract) == 0, "Can't delete");
        require(vault.stakedCount(SOFT_STAKED, _contract) == 0, "Can't delete");
    ...
    function removeMultipliers(address _contract, uint256[] calldata _tokenIds) public onlyOwner {
        for (uint256 i; i < _tokenIds.length; i++) {
            require(
                tokenMultipliers[_contract].contains(_tokenIds[i]),
                "Token is not a multiplier"
            );
    ...
    function claimRewards(uint8 _type) public nonReentrant {
        ...
        require(vault.getConfiguration(_type), "Not allowed");
        uint256 _rewards = calculateRewards(msg.sender, _type);
        require(_rewards > 0, "No rewards");
        _clearDepositTimesOf(msg.sender, _type);
    ...
    function _clearDepositTimesOf(address _address, uint8 _type) internal {
         for (uint256 i = 0; i < contracts.length; i++) {
            address _contract = contracts[i];
            vault.clearDepositTimesOf(_address, _contract, _type);
        }
    }
