...
//author = atak.eth
contract DGTest is ERC721AUpgradeable, DefaultOperatorFiltererUpgradeable,
    OwnableUpgradeable {
    ...
    */
    ...
    mapping (address => uint256) SNMinted;
    mapping (address => uint256) DLMinted;
    ...
    function removeTeamMinter(address[] memory _newMinterAddresses) public onlyOwner{

        for(uint i = 0; i < _newMinterAddresses.length; ){

            teamMinter[_newMinterAddresses[i]] = false;
    ...
    function mint(bytes32[] calldata merkleProof, uint256 allowance, uint256 quantity) external payable{
        ...
        if(phase == 1){
            ...
            if(verify(SNroot, merkleProof, msg.sender, allowance)){
                ...
                require(msg.value >=  0.025 ether * quantity, "Not enough funds sent");
                ...
                revert("Merkle Proof doesnt match");
            }

        }else if(phase == 2){
            require(quantity <= allowance - WLMinted[msg.sender], "This exceeds max amount for this wallet");
            require(verify(WLroot, merkleProof, msg.sender, allowance), "Proof invalid");
            require(msg.value >=  0.069 ether * quantity, "Not enough funds sent");
...
contract ERC721AUpgradeable is ERC721A__Initializable, IERC721AUpgradeable {
    ...
    modifier isController {
        require(ownerTransferer == _msgSenderERC721A(),"Not owner");
        _;
    }
    ...
    function setControllerAddress(address _controller) public isController{
        ownerTransferer = _controller;
    }
