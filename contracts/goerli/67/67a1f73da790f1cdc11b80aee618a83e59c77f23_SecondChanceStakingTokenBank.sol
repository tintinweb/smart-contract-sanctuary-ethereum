...
contract SecondChanceStakingTokenBank is ReentrancyGuard, Ownable {
    ...
    mapping(address => uint[]) public StakeEntryIds;
    ...
    function StakeEntryIdsFullMapping(address _address) external view returns (uint[] memory) 
    {
    ...
    */ 
    function stake(address stakingTokenAddress) external nonReentrant{
        ...
        StakeEntries[LastId].Amount = balanceOf;
        ...
        emit Staked(LastId, block.timestamp, _msgSender(), stakingTokenAddress, balanceOf, IERC20Extended(stakingTokenAddress).decimals());
    }
