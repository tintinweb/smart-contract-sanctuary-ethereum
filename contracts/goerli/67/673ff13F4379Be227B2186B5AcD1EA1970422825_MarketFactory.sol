...
contract MarketFactory is IMarketFactory {
    ...
    function createMarket(address token, address feed, address marketCloneInstance)
        external
        override
        returns (address newMarketAddr)
    {
        ...
        IMarket newMarket = IMarket(marketCloneInstance);
        newMarketAddr = address(newMarket);
        // newMarket.initialize(token, address(0), address(config), feed);
        
        // // config access
        BACUpgradable ac = BACUpgradable(address(newMarket));
    ...
    function createWrappedNativeMarket(address feed, address marketCloneInstance)
        external
        override
        returns (address)
    {
        ...
        newMarket.initialize( address(0), address(0), address(config),feed);
...
interface IMarketFactory {
    ...
    function createMarket(address token, address feed, address market)
        external
        returns (address);

    function createWrappedNativeMarket(address feed, address market) external returns (address);
}
