...
contract Rewards is AccessControl {
    ...
    mapping(uint256 => Reward) public alltypes;
    ...
    function distributionStart() external onlyOwner {
        for (uint256 rewardId = 1; rewardId <= 3; ) {
            require(
                alltypes[rewardId].isdistStart != true,
                "Distribution Already Started"
            );
            alltypes[rewardId].startTime = block.timestamp;
            alltypes[rewardId].isdistStart = true;
            unchecked {
                rewardId++;
            }
    ...
    function distributionEnd(uint256 rewardId) external onlyOwner {
        uint256 _cliff = alltypes[rewardId].cliff;
        uint256 _vested = alltypes[rewardId].vested;
        uint256 _startTime = alltypes[rewardId].startTime;
        ...
        alltypes[rewardId].endTime = block.timestamp;
        alltypes[rewardId].isdistEnd = false;
    }

    // transfer token to the user by contract owner and admins
    function transferToken(
        ...
        require(
            alltypes[rewardId].unlockToken >= amount,
            "Insufficient Token!"
        );
    ...
    function _sendToken(
        ...
        alltypes[_rewardId].unlockToken -= amount;
        token.safeTransfer(_user, amount);
        gettokens[_rewardId].push(GetToken(_user, amount));
    ...
    function checkCliff(uint256 rewardId) private {
        ...
        uint256 _vested = alltypes[rewardId].vested;
        uint256 _startTime = alltypes[rewardId].startTime;
        bool _commStart = alltypes[rewardId].isdistStart;
        bool _commEnd = alltypes[rewardId].isdistEnd;
    ...
    function _unlockToken(
        ...
        uint256 _unlocktoken = alltypes[_rewardId].unlockToken;
        uint256 _remaintoken = alltypes[_rewardId].remainToken;
        uint256 _avgtoken = alltypes[_rewardId].avgToken;
        ...
        alltypes[_rewardId].unlockToken = _unlocktoken;
        alltypes[_rewardId].remainToken = _remaintoken;
    }
    ...
    function allusers(uint256 _rewardId)
        public
        view
        returns (GetToken[] memory)
    {
        return gettokens[_rewardId];
    }
}
