 ...
 * @title Bi123NFT
 * @dev Bi123NFT
 * @custom:dev-run-script Bi123NFT.sol
 */
contract Bi123NFT is ERC721, Ownable {
    using Counters for Counters.Counter;

    Counters.Counter private _tokenIdCounter; 
    uint256 public constant MAX_TOKENS = 1000000000;
    uint256 public constant MONTHLY_PRICE = 0.00001 ether;
    uint256 public constant QUARTERLY_PRICE = 0.00003 ether;
    uint256 public constant ANNUAL_PRICE = 0.00005 ether; 
    ...
    function mintQuarterly() public payable { 
        require(msg.value == QUARTERLY_PRICE, "Incorrect payment amount");

        uint256 expiration = block.timestamp + 90 days;
        _mintWithExpiration(expiration, msg.sender);
    }

    function mintAnnually() public payable { 
        require(msg.value == ANNUAL_PRICE, "Incorrect payment amount");
    ...
    function _mintWithExpiration(uint256 expiration, address to) private {
        ...
        _expirationTimestamps[newTokenId] = expiration;
        _tokenIdCounter.increment();
    }
    ...
    function reclaimExpired(uint256 tokenId) public {
    ...
    function tokenIdsOf(address owner) public view returns (uint256[] memory) {
        ...
        for (uint256 i = start; i <= length; ++i) {
            ...
            if (tokenIdsIdx == tokenIdsMaxLength) {
                break;
            }
        }
