...
contract ERC721NFTCustom is Initializable, ERC721URIStorage, AccessControl, ERC2771Context {
    ...
    function initialize(
        ...
        if (runtimeConfig.royaltyAddress != address(0)) setRoyaltyAddress(runtimeConfig.royaltyAddress);

        // CREATOR of this contract get default admin role -- very simple access control 
        _owner = _msgSender();
    ...
    // TODO Define who can call this method 
    function mintToCaller(
        address caller,
        string memory tokenURI
    ) public onlyRole(DEFAULT_ADMIN_ROLE) returns (uint256) { // only owner can call this method
    ...
    ) public onlyRole(DEFAULT_ADMIN_ROLE) { // only admin can call this method
        require(
            _exists(_tokenId),
