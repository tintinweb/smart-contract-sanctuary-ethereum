...
contract Memory is ERC721, ERC721Royalty, Ownable {
    ...
    mapping(string => uint256) public tokenComponentsToTokenIds;

    event Mint(address indexed _to, uint256 _tokenId, string _components);
    ...
    function mintFromMinter(
        ...
        require(
            tokenComponentsToTokenIds[components] == 0,
            "Token components combination already exists"
        );
        ...
        address splitAddress = splitMain.predictImmutableSplitAddress(
            recipients,
            percentAllocations,
            0
        );

        if (splitAddress.isContract()) {
            _setTokenRoyalty(tokenId, splitAddress, ROYALTY);
        } else {
            address payoutSplit = splitMain.createSplit(
                recipients,
                percentAllocations,
                0,
                address(0)
            ...
            _setTokenRoyalty(tokenId, payoutSplit, ROYALTY);
        }

        tokenIdsToComponents[tokenId] = components;
        tokenComponentsToTokenIds[components] = tokenId;

        emit Mint(recipient, tokenId, components);
    ...
    function tokenURI(
        ...
        string memory tokenComponents = tokenIdsToComponents[tokenId];

        return string.concat(baseURI, tokenComponents);
    }
...
contract Minter is Ownable {
    ...
    function mintMemory(
        ...
        require(signer != address(0), "Verify signer address must to be set");
        require(
            address(mem) != address(0),
            "Memory contract address must be set"
        );
        require(
            isValidVerifySignature(components, signature),
            "Invalid verify signature provided"
        );
        require(msg.value > 0 && msg.value == PRICE, "Incorrect amount sent");

        mem.mintFromMinter(msg.sender, components);
    ...
    function allowlistMintMemory(
        ...
        require(
            mintingState == 2 || mintingState == 1,
            "Allowlist mint is not active"
        );
        require(signer != address(0), "Signer address must to be set");
        require(
            address(mem) != address(0),
            "Memory contract address must be set"
        );
        ...
        require(
            isValidVerifySignature(components, verifySignature),
            "Invalid verify signature provided"
        );
        require(msg.value > 0 && msg.value == PRICE, "Incorrect amount sent");
    ...
    function isValidVerifySignature(
        ...
        bytes32 hash = keccak256(abi.encodePacked(components));
