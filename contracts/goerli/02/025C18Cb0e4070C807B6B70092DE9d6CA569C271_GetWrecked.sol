...
contract GetWrecked is ERC721APausable, Ownable, ReentrancyGuard {
    string private baseURI;
    string public baseExtension = ".json";

    uint256 nftPerAddressLimit = 1;

    uint256 public Holy_PRICE = 0;
    ...
    event HolyArcadeNFTMinted(address sender, uint256 tokenId);
    ...
    constructor(string memory _initNotRevealedUri) ERC721A("GetWrecked", "HM") {
        setNotRevealedURI(_initNotRevealedUri);
    }

    function mintHoly(uint256 quantity)
        public
        payable
        underMaxSupply(quantity)
    {
        if (msg.sender != owner()) {
            ...
            require(msg.value >= Holy_PRICE * quantity, "insufficient funds");
            addressMintedBalance[msg.sender] += quantity;
    ...
    function setHolyPrice(uint256 _newPrice) public onlyOwner {
        Holy_PRICE = _newPrice * (10**16);
    }
