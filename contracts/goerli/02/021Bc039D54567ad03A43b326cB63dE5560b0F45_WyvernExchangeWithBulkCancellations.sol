...
contract ExchangeCore is ReentrancyGuarded, Ownable {
    ...
    bytes32 public constant DOMAIN_SEPARATOR = 0x83a846547741a5bf48a8d69fd8fcbfc2bee234c5b5927bb92443e5f359173f40;
        ...
        reentrancyGuard
    {
        ...
        if (buy.maker == msg.sender) {
            require(validateOrderParameters(buy), "validateOrderParameters(buy) returned false");
        } else {
        ...
        if (sell.maker == msg.sender) {
            require(validateOrderParameters(sell), "validateOrderParameters(sell) returned false");
        } else {
            sellHash = _requireValidOrderWithNonce(sell, sellSig);
        }

        /* Must be matchable. */
        require(ordersCanMatch(buy, sell), "ordersCanMatch(buy, sell) returned false");
        ...
        require(size > 0, "size > 0 returned false");
        ...
        require(ArrayUtils.arrayEq(buy.calldata, sell.calldata), "ArrayUtils.arrayEq(buy.calldata, sell.calldata) returned false");

        /* Retrieve delegateProxy contract. */
        OwnableDelegateProxy delegateProxy = registry.proxies(sell.maker);

        /* Proxy must exist. */
        require(delegateProxy != address(0), "delegateProxy != address(0) returned false");
        ...
        require(delegateProxy.implementation() == registry.delegateProxyImplementation(), "delegateProxy.implementation() == registry.delegateProxyImplementation() returned false");

        /* Execute specified call through proxy. */
        require(proxy.proxy(sell.target, sell.howToCall, sell.calldata), "proxy.proxy(sell.target, sell.howToCall, sell.calldata) returned false");

        /* Static calls are intentionally done after the effectful call so they can check resulting state. */

        /* Handle buy-side static call if specified. */
        if (buy.staticTarget != address(0)) {
            require(staticCall(buy.staticTarget, sell.calldata, buy.staticExtradata), "staticCall(buy.staticTarget, sell.calldata, buy.staticExtradata) returned false");
        }

        /* Handle sell-side static call if specified. */
        if (sell.staticTarget != address(0)) {
            require(staticCall(sell.staticTarget, sell.calldata, sell.staticExtradata), "require(staticCall(sell.staticTarget, sell.calldata, sell.staticExtradata) returned false");
        }
