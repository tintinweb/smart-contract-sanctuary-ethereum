...
//  Interfaces
import "./interfaces/IdegenExchangeData.sol";
...
/**
 *
 *               NYLON Blocks : Xhcange standard degenXchange Mother Contract
 *               Version 0.2.0a
 *
...
contract degenExchangeRegistry is XchangeBase, GasManager {
    ...
    modifier addressHasDraws() override {
        require(
            ...
            "Need to deposit an NFT inorder to draw a new one."
        );
        _;
    }
    ...
    modifier activeNFTCount() override {
        require(
            _getAvalabuleNFTsInDraw() > MINIMAL_TOKENS_FOR_DRAW,
    ...
    function enterNFTDraw(
        ...
        require(
            _userDepositCount[_msgSender()] < MAX_CONCURRENT_ENTRYS_PER_WALLET,
            "Max Entrys Per Wallet Reached"
        );
     ...
     *                   Used for entering an NFT to the pool but not reciving a draw entry
     *                   Made for NFT projects to build their comunity
     */
    function submitNFT(
        address addressForNFTContract_,
        uint256 tokenId_,
     ...
     *       This will be replaced with a direct submition function to buy link and submit to the VRN service
     */
    ...
    function getAvalabuleNFTsInDraw() public view returns (uint256) {
        return _getAvalabuleNFTsInDraw();
    }
    ...
    function getMaxEntrysPerWallet() public view returns (uint256) {
        return _getMaxEntrysPerWallet();
    }
...
abstract contract GasManager is IGasManager {
    ...
    modifier gasManager() override {
        if (_gasManagerActive) {
            if (tx.gasprice > _maxGasPrice)
                revert GasPriceIsToHigh(_maxGasPrice, tx.gasprice);
        }
        _;
    }
...
interface IdegenExchangeData {
    ...
    event DrawComplete(address drawer_, address nftContractAddress_, uint256 tokenId_, uint256 drawId_);
    event DepositedNFT(address depositor_, address nftContractAddress_, uint256 tokenId_);
    event GiftedNFT(address depositor_, address nftContractAddress_, uint256 tokenId_);
    event DrawInitiated(address drawer_, uint256 drawId_);

    event ForceRemoveNFT(address depositor_, address nftContractAddress_, uint256 tokenId_, string reason_);
    event RandomNumberReturned(uint256 requestId_);

    error InvaliedContractInterfce(bytes4 interfaceId_);
    error FaildToRequestRandomNumbers(uint256 requestId_);
...
abstract contract AddressControl is IAddressControl {
    ...
    mapping(address => bytes4) internal _acceptedAdddressInterface;
    ...
    function _addAddress(
        ...
        _acceptedAdddressInterface[contractAddress_] = interfaceId_;
    ...
    function _getProjectInterfaceId(address contractAddress_)
        ...
        returns (bytes4)
    {
        return _acceptedAdddressInterface[contractAddress_];
    }
    ...
    function _bulkAddAddress(SubmitionData[] memory inputData_) internal {
    ...
    function _getAllowedProjectData(address contractAddress_)
        ...
        returns (SubmitionData memory)
    {
        SubmitionData memory addressData;
 ...
 *   @note    -   IERC1155 interfaceId -
 *
 */
/**
 *
 *               NYLON Blocks : degenXhcange Base Contract
 *               Version 0.2.0a
 *
 *   @notice    -   Holds all logic for the Xchagne protocol
...
abstract contract XchangeBase is RandomNumber, ERC165, IXchange, Ownable {
    ...
    uint256 constant MAX_CONCURRENT_ENTRYS_PER_WALLET = 1;
    ...
    function _enterNFTDraw(
        ...
        unchecked {
            TokenData memory newEntry;
            newEntry.contractOfToken = addressForNFTContract_;
            newEntry.ownerAddress = _msgSender();
            newEntry.interfaceId = interfaceId_;
            newEntry.tokenId = tokenId_;
            newEntry.gifted = false;
            newEntry.gifter = address(0x0);
    ...
    function _submitNFT(
        ...
        TokenData memory newEntry;
        newEntry.contractOfToken = addressForNFTContract_;
        newEntry.ownerAddress = address(0x0);
        newEntry.interfaceId = interfaceId_;
        newEntry.tokenId = tokenId_;
        newEntry.gifted = true;
        newEntry.gifter = _msgSender();
    ...
    function _fulfillRandomWords(
        ...
        uint256 newNumber = randomWords[0];
        _DrawIdToVRN[_drawTriggers[requestId]] = newNumber;
        // TODO can be these requires be at the top to cancel out newNumber creation?
        //          @raymondchooi they could, but i was trying to catch all incomming data to store it
        require(
        ...
        require(
            ...
            "Draw has alredy been completed"
        );
        _fullfillFinalDraw(newNumber, requestId);
    }

    function _fullfillFinalDraw(uint256 returnedNumber_, uint256 requestId_)
        internal
    {
        ...
        unchecked {
            ...
            bool topReached;
            if (roll < 1) roll++;
            while (_tokensInDraw[roll].ownerAddress == drawerAddress) {
                if (topReached) roll--;
        ...
        TokenData memory drawnToken = _tokensInDraw[roll];
        _switchTransferInterface(
    ...
    function _removeNFTEntryFromDraw(uint256 NFTIndex_, string memory reason_)
        internal
    {
        TokenData memory toRemove = _tokensInDraw[NFTIndex_];
    ...
    function _switchTransferInterface(
        ...
        if (interfaceId_ == type(IERC721).interfaceId) {
            if (inOrOut_ > 0) {
                ...
                _transferOutERC721(addressForNFTContract_, tokenId_, reciver_);
            }
            return;
        }
        if (interfaceId_ == type(IERC1155).interfaceId) {
            if (inOrOut_ > 0) {
                ...
                _transferOutERC1155(addressForNFTContract_, tokenId_, reciver_);
            }
            return;
        }
        revert InvaliedContractInterfce(interfaceId_);
    }
    ...
    function _checkAddress(address _addr) internal view returns (bool) {
        uint256 length;
        assembly {
    ...
    function _getAvalabuleNFTsInDraw() internal view returns (uint256) {
    ...
    function _getAddressRemainingPlays(address playersAddress_)
        internal
        view
        returns (uint256)
    {
        return _userDepositCount[playersAddress_];
    }

    function _getMaxEntrysPerWallet() internal pure returns (uint256) {
        return MAX_CONCURRENT_ENTRYS_PER_WALLET;
    }
...
interface IGasManager {
    ...
    error GasPriceIsToHigh(uint256 maxAcceptedPrice_, uint256 currentGasPrice_);
...
interface IAddressControl {
    ...
    function addBulkAddress(SubmitionData[] memory inputData_) external;
...
abstract contract FundLINKSubscription is IFundLINKSubscription {
    ...
    function _exchangeAllEthAndFund() internal {
        ...
        _approveWeth(Weth);
        uint256 recivedLink = _swapWEthForLink(Weth);

        _fundSubscription(recivedLink);

        emit SubscriptionFunded(recivedLink, amountInWei, _getSubscriptionId());
    }
    ...
    function _exchangeEthAmountAndFund(uint256 amount_) internal {
        ...
        bool sucsess = _fundSubscription(recivedLink);

        if (!sucsess) revert FundLinkContractFailed();

        emit SubscriptionFunded(recivedLink, amount_, _getSubscriptionId());
    }
    ...
    function _exchangeAllWethAndFund() internal {
        ...
        emit SubscriptionFunded(recivedLink, weth, _getSubscriptionId());
    }

    /**
     *  @dev    -   Curreny converters
     */
    ...
    function _swapWEthForLink(uint256 amountInWei_)
        private
        returns (uint256 recivedLink)
    {
        //  Build the swap paramaters
        ...
        recivedLink = SWAP_ROUTER.exactInputSingle(params);

        if (recivedLink < 1) revert SwapEthForLinkFailed();

        emit SwapMade(amountInWei_, recivedLink);
        return recivedLink;
    }
    ...
    {
        sucsess = ILinkTransfer(CHAIN_LINK_TOKEN).transferAndCall(
            VRF_COORDINATOR,
            amountOfLink_,
            _subscriptionId
        );
    ...
    function _approveWeth(uint256 amount_) private {
        ...
        if (!success) revert FiledToApproveWeth();
    }
