...
contract ROKUTEST is ERC721A, Ownable, ReentrancyGuard, DefaultOperatorFilterer {
    ...
    uint256 public publicCost = 0.008 ether;
    uint256 private reserveLimit = 0;
    bool public ignore; // remove

    uint16 public maxSupply = 888;

    uint8 public maxMintAmount = 1;

    string private _baseTokenURI = "ipfs://INPUT/";
    ...
    bool public waitlistActive;

    constructor() ERC721A("ROKUs World", "ROKU") {}
    ...
    function isWaitlisted(address _user) external view returns (bool) {
        return _waitList[_user];
    }
    ...
    function waitlistMint()
        ...
        callerIsUser
    {
        if (!waitlistActive) revert PreSaleNotActive();
        if (totalSupply() + 1 > maxSupply) revert MaxSupplyExceeded();
        if (!_waitList[msg.sender]) revert NotWaitlisted();
    ...
    function mint() external payable callerIsUser {
        ...
        if (msg.value != publicCost * 1) revert InsufficientValue();

        _mint(msg.sender, 1);
    }

    function airDrop(address[] memory targets) external onlyOwner {
        if (targets.length + reserveLimit > 40) revert MaxSupplyExceeded();
    ...
    function toggleWaitlist() external onlyOwner {
    ...
    function setWaitlistAddresses(address[] calldata _users)
        external
        onlyOwner
    {
        for (uint256 i = 0; i < _users.length; i++) {
            _waitList[_users[i]] = true;
        }
    }
...
contract ERC721A is IERC721A {
        ...
        returns (string memory)
    {
        ...
        if (block.chainid != chainId) return "";
