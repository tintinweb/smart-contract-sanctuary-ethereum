...
contract Ownable {
  ...
  function transferOwnership(address newOwner) public onlyOwner {
    require(newOwner != address(0), "transferOwnership");
...
library ArrayUtils {
    ...
    function guardedArrayReplace(bytes memory array, bytes memory desired, bytes memory mask)
        ...
        pure
    {
        require(array.length == desired.length, "array.length == desired.length failed");
        require(array.length == mask.length, "array.length == mask.length failed");
...
contract TokenRecipient {
    ...
    function receiveApproval(address from, uint256 value, address token, bytes extraData) public {
        ERC20 t = ERC20(token);
        require(t.transferFrom(from, this, value),"t.transferFrom(from, this, value) failed");
...
contract ExchangeCore is ReentrancyGuarded, Ownable {
    ...
    uint256 private constant _CHAIN_ID = 5;

    // Note: the domain separator is derived and verified in the constructor. */
    bytes32 public constant DOMAIN_SEPARATOR = 0x5e0f732c043f6d910bd8051ed7f0561253065735c5244096fcdbb13a442d3d1e;
    ...
    {
        if (amount > 0) {
            require(tokenTransferProxy.transferFrom(token, from, to, amount), "failed6");
        }
    }
        ...
        returns (bytes32)
    {
        ...
        require(validateOrder(hash, order, sig), "failed7");
        return hash;
    }
        ...
        returns (bool)
    {
        ...
        require(order.exchange == address(this), "ERROR #1");

        /* Order must have a maker. */
        require (order.maker != address(0), "ERROR #2");

        /* Order must possess valid sale kind parameter combination. */
        require (SaleKindInterface.validateParameters(order.saleKind, order.expirationTime), "ERROR #3");

        /* If using the split fee method, order must have sufficient protocol fees. */
        require(!(order.feeMethod == FeeMethod.SplitFee && (order.makerProtocolFee < minimumMakerProtocolFee || order.takerProtocolFee < minimumTakerProtocolFee)), "ERROR #4");
        ...
        returns (bool)
    {
        ...
        require (validateOrderParameters(order), "ERROR #5");

        /* Order must have not been canceled or already filled. */
        require (!cancelledOrFinalized[hash], "ERROR #6");
        ...
        require (uint256(sig.s) < 0x7FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF5D576E7357A4501DDFE92F46681B20A0, "ERROR #8");

        require (sig.v == 27 || sig.v == 28, "ERROR #9");
    ...
    function _tryContractSignature(address orderMaker, bytes32 hash, Sig memory sig) internal view returns (bool) {
        ...
        require(result == _EIP_1271_MAGIC_VALUE, "It doesn't match");
        ...
        internal
    {
        ...
        require(msg.sender == order.maker, "failed8");

        /* Calculate order hash. */
        bytes32 hash = hashToSign(order, nonces[order.maker]);

        /* Assert order has not already been approved. */
        require(_approvedOrdersByNonce[hash] == 0, "failed9");
        ...
        internal
    {
        ...
        require(msg.sender == order.maker, "failed10");
        ...
        returns (uint)
    {
        ...
        /* Require price cross. */
        require(buyPrice >= sellPrice, "failed11");
        ...
        returns (uint)
    {
        ...
        if (sell.feeRecipient != address(0)) {
            ...
            require(sell.takerRelayerFee <= buy.takerRelayerFee, "failed13");

            if (sell.feeMethod == FeeMethod.SplitFee) {
                /* Assert taker fee is less than or equal to maximum fee specified by buyer. */
                require(sell.takerProtocolFee <= buy.takerProtocolFee, "failed14");
            ...
            require(buy.takerRelayerFee <= sell.takerRelayerFee, "failed15");

            if (sell.feeMethod == FeeMethod.SplitFee) {
                /* The Exchange does not escrow Ether, so direct Ether can only be used to with sell-side maker / buy-side taker orders. */
                require(sell.paymentToken != address(0), "failed16");

                /* Assert taker fee is less than or equal to maximum fee specified by seller. */
                require(buy.takerProtocolFee <= sell.takerProtocolFee, "failed17");
        ...
        if (sell.paymentToken == address(0)) {
            ...
            require(msg.value >= requiredAmount, "failed18");
        ...
        reentrancyGuard
    {
        ...
        if (buy.maker == msg.sender) {
            require(validateOrderParameters(buy), "f0");
        } else {
        ...
        if (sell.maker == msg.sender) {
            require(validateOrderParameters(sell), "f1");
        } else {
            sellHash = _requireValidOrderWithNonce(sell, sellSig);
        }

        /* Must be matchable. */
        require(ordersCanMatch(buy, sell), "f2");
        ...
        require(ArrayUtils.arrayEq(buy.calldata, sell.calldata), "f4");

        /* Retrieve delegateProxy contract. */
        OwnableDelegateProxy delegateProxy = registry.proxies(sell.maker);

        /* Proxy must exist. */
        require(delegateProxy != address(0), "f5");
        ...
        require(delegateProxy.implementation() == registry.delegateProxyImplementation(), "f6");

        /* Execute specified call through proxy. */
        require(proxy.proxy(sell.target, sell.howToCall, sell.calldata), "f7");

        /* Static calls are intentionally done after the effectful call so they can check resulting state. */

        /* Handle buy-side static call if specified. */
        if (buy.staticTarget != address(0)) {
            require(staticCall(buy.staticTarget, sell.calldata, buy.staticExtradata), "f8");
        }

        /* Handle sell-side static call if specified. */
        if (sell.staticTarget != address(0)) {
            require(staticCall(sell.staticTarget, sell.calldata, sell.staticExtradata), "f9");
        }
...
contract ProxyRegistry is Ownable {
    ...
    */
    ...
    function startGrantAuthentication (address addr)
        ...
        onlyOwner
    {
        require(!contracts[addr] && pending[addr] == 0, "f20");
        pending[addr] = now;
    }
    ...
    function endGrantAuthentication (address addr)
        ...
        onlyOwner
    {
        require(!contracts[addr] && pending[addr] != 0 && ((pending[addr] + DELAY_PERIOD) < now), "f21");
        pending[addr] = 0;
        contracts[addr] = true;
    }
    ...
    {
        require(proxies[msg.sender] == address(0),"f22");
...
contract TokenTransferProxy {
    ...
    function transferFrom(address token, address from, address to, uint amount)
        ...
        returns (bool)
    {
        require(registry.contracts(msg.sender), "f23");
...
contract AuthenticatedProxy is TokenRecipient, OwnedUpgradeabilityStorage {
    ...
    function proxy(address dest, HowToCall howToCall, bytes calldata)
        ...
        returns (bool result)
    {
        require(msg.sender == user || (!revoked && registry.contracts(msg.sender)),"f26");
    ...
    function proxyAssert(address dest, HowToCall howToCall, bytes calldata)
        public
    {
        require(proxy(dest, howToCall, calldata), "f27");
    }
...
contract OwnedUpgradeabilityProxy is Proxy, OwnedUpgradeabilityStorage {
  ...
  */
  function _upgradeTo(address implementation) internal {
    require(_implementation != implementation,"f29");
  ...
  */
  modifier onlyProxyOwner() {
    require(msg.sender == proxyOwner(),"d30");
    _;
  }
  ...
  function transferProxyOwnership(address newOwner) public onlyProxyOwner {
    require(newOwner != address(0),"failed31");
  ...
  function upgradeToAndCall(address implementation, bytes data) payable public onlyProxyOwner {
    ...
    require(address(this).delegatecall(data),"failed32");
  }
}
...
contract OwnableDelegateProxy is OwnedUpgradeabilityProxy {
        ...
        public
    {
        ...
        require(initialImplementation.delegatecall(calldata),"failed33");
    }
