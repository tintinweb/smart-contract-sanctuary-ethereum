...
contract VaultMock is ERC20 {
    ...
    function deposit(uint256 amount) external returns (uint256 issuedShares) {
        ...
        issuedShares = amount.mul(_pricePerShare).uncheckedDiv(PRECISION);
    ...
    function withdraw() external returns (uint256 value) {
        // Burn issued amount of shares yToken
        value = burn(msg.sender, msg.sender, this.balanceOf(msg.sender));
        return value;
    }
    ...
    function burn(
        ...
        value = shares.mul(_pricePerShare).uncheckedDiv(PRECISION);
