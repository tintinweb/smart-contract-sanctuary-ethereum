...
//        if (old_price != 0) {
//            // require you have enough money to spend
//            require(_remainder >= _new_price, "Insufficient balance to pay new price");
//            // Require min bid if old_price != 0
//            if (old_price + MIN_BID_AMOUNT > _new_price) {
...
//        }
        ...
        for (uint i = 0; i < _x.length; i++) {
            remainder -= _prices[i];
            _changePixelColor(_x[i], _y[i], _new_color[i], _prices[i], remainder + _prices[i]);
        }
    }
