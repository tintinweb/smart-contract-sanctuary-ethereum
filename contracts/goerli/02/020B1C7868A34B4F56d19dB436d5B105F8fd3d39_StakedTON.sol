...
interface IStakedTON {
    ...
    /// @notice BurnFrom function, see burn(uint256,bytes) for direct equivalent
    ///         Burning function to emit Burned() event which is monitored by validators
    ///         Converts underlying input amount to shares and checks it to be non-zero
    ///         Can be paused
    /// @param from Address for tokens to be burned from and allowance spending
    /// @param amountUnderlying Input amount of TON tokens that to be bridged to the TON network
    /// @param data Metadata with a valid address in TON network to receive tokens
    function burn(
        address from,
    ...
    IStakedTON
{
    ...
    function validateData(bytes calldata data) public pure returns (bool) {
        return
            data.length == SUPPORTED_DATA_LENGTH_FULL || data.length == SUPPORTED_DATA_LENGTH_EASY;
    }

    function burn(uint256 amountUnderlying, bytes calldata data) external whenNotPaused {
    ...
    function burn(
        ...
        if (!validateData(data)) revert InvalidData();
        _spendAllowance(from, _msgSender(), amountUnderlying);
        _burn(from, amountUnderlying, data);
    }
