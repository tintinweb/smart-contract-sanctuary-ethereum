    ...
    VRFConsumerBaseV2
{
    ...
    /// Key: Token Id
    /// Value: Word corresponding to the Token Id
    mapping(uint256 => string) public tokenWords;

    /// @dev Words (phrase) for requests
    /// Key: Request Id from Chainlink Any API
    /// Value: Word corresponding to image URL returned by the API request
    mapping(uint256 => string) public requestWords;

    /// @dev TokenIds to be burned
    /// Key: Request Id from Chainlink Any API
    /// Value: Array of token Ids used to create the combined token
    mapping(uint256 => uint256[]) public tokensToBurn;

    /// @dev Addresses of word creation requesters
    /// Key: Request Id from Chainlink Any API
    /// Value: Address of initial requester
    ...
    /// @dev Price required to mint a specific word
    ...
    function requestCreateWord(string memory _word) public payable {
        ...
        uint256[] memory noTokensToCombine; // No token Ids need to be provided
    ...
    function requestCreateWord() public {
        uint256 vrfRequestId = VRF_COORDINATOR.requestRandomWords(
            ...
            5_000_000, // Callback gas limit
            1 // Number of generated words
        );
        // Tie the VRF request to the sender to later be able to mint the token for the sender
    ...
    function _submitChainlinkApiRequest(
        ...
        // TODO: Replace url with server endpoint
        string memory drawUrl = string.concat(
            "https://93de-2600-387-15-181b-00-9.ngrok.io/draw?phrase=",
            phrase
        );
        req.add("get", drawUrl);
