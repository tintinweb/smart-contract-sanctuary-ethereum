...
contract Treasury is AccessControl, ITreasury {
    ...
    function incurDebt(uint256 _amount, address _token) external override {
        ...
        if (_token == address(mainToken)) {
            require(permissions[STATUS.SCRIPTDEBTOR][msg.sender], notApproved);
            value = _amount;
        } else {
    ...
    function enable(STATUS _status, address _address, address _calculator) external onlyGovernor {
        ...
        if (_status == STATUS.STAKINGTOKEN) {
    ...
    function execute(uint256 _index) external {
        ...
        if (info.managing == STATUS.STAKINGTOKEN) {
            // 9
            stakingToken = IStakingToken(info.toPermit);
        } else {
