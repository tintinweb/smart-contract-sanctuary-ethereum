...
/// @dev HashtrustPay contract
import "./Oracle.sol";
import "./Package.sol";

contract HashtrustPay is Ownable, Oracle, Package {
    ...
    /// @dev event deposit
    function DepositETH(
        string memory _idOrden,
        uint256 _idPackage
    ) public payable {
        /// @dev Get Package Contract Service
        uint price = getPackagePrice(_idPackage);

        /// @dev Get Price in ETH
        uint usdToEth = getAmount(price);

        require(
            msg.value >= usdToEth,
            "DepositETH: Specify an amount of token greater than zero"
        );
    ...
    /// @dev withdraw token
    function withdrawToken(uint256 _amount) external payable onlyOwner {
        (bool success, ) = payable(_msgSender()).call{value: _amount}("");
        require(success, "withdrawToken Comission Admin: transaction failed");
...
contract Oracle is Ownable {
    ...
    address public _addressOracle = 0xD4a33860578De61DBAbDc8BFdb98FD742fA7028e;
    ...
    function getAmountInETH(
        ...
        uint256 unitaryPrice = _amount.mul(1 ether).div(_ethInUsd);
        return unitaryPrice;
    }

    /**
     * @dev get amount in eth
     */

    function getAmount(uint256 _amount) public view returns (uint256) {
        uint256 ethInUsd = getLatestPrice(
            _addressOracle,
            _addressDecimalOracle
        );

        uint256 amountFinal = getAmountInETH(_amount, ethInUsd);
...
contract Package is Ownable {
    ...
    ///  @dev add Package
    function addPackage(uint256 _price) public onlyOwner {
    ...
    /// @dev remove Package
    function removePackage(uint256 _id) public onlyOwner {
        WhitelistPackage[_id].active = false;
    }
