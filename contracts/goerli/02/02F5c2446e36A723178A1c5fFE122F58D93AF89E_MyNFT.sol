...
contract MyNFT is myERC721, Ownable {
    ...
    event NftItemCreated(uint tokenId, uint price, address creator);
    ...
    function mintToken(string memory tokenUrl, uint price) public payable returns (uint) {
        require(!_tokenURIExists(tokenUrl), "Token URI already exists");
        require(msg.value > 0, "Price must be higher than zero");
        tokenId.increment();

        uint newTokenId = tokenId.current();

        mint(msg.sender, newTokenId);
        setTokenURI(newTokenId, tokenUrl);
        usedTokenURIs[tokenUrl] = true;

        idToNftItem[newTokenId] = NftItem(newTokenId, price, msg.sender);
        emit NftItemCreated(newTokenId, price, msg.sender);
        return newTokenId;
    }
  ...
  // Mapping owner address to token count
   ...
   /**
    ...
    function transferFrom(address from, address to, uint256 tokenId) internal {
        ...
        require(ownerOf(tokenId) == from, "ERC721: transfer from not the owner");
