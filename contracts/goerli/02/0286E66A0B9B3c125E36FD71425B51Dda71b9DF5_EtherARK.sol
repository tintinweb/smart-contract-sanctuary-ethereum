...
contract EtherARK is ERC721A, Ownable {
  ...
  event MintSucceed(address indexed _to, uint256 indexed amount);
  ...
  function _validate(bytes32 message, bytes memory signature) internal view returns (bool) {
    require(chairman != address(0) && signature.length == 65);
    ...
    return ecrecover(message, v, r, s) == chairman;
  }

  function mint(bytes memory evidence, uint amount) public payable {
    ...
    uint needPay = amount * mintPrice;
    require(msg.value >= needPay, "Payment is not enough.");
    ...
    if (msg.value > amount * mintPrice) {
      payable(msg.sender).transfer(amount * mintPrice - msg.value);
    }

    emit MintSucceed(msg.sender, amount);
  }
