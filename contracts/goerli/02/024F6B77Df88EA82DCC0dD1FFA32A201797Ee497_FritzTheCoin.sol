...
contract FritzTheCoin is ERC20, Ownable {
    ...
    uint256 public swapTokensAtAmount = 0.5 * 1e18;
    ...
    function _transfer(
        ...
        if(rewardEnabled)
        {
            ...
            if (
                ...
                if(balanceOf(to) + amount > (totalSupply() * 25) / 1000)
                    payable(weeklyWinner).transfer((contractETH - 0.5 * 1e18) / 2);
                //transfer rest of tokens in Pot to marketing Wallet
                payable(marketingWallet).transfer((contractETH - 0.5 * 1e18) / 2);
        ...
        if(reserve1 > 0 && reserve2 > 0){
            uint256 vContractEth = contractTokens.mul(reserve1).div(reserve2);
