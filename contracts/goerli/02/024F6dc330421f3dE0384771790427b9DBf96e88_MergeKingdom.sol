...
contract MergeKingdom is ERC721Enumerable, Ownable, Pausable {
    ...
    uint32[] public rewardModifier = [100, 125, 150, 200];            // +25%, +50%, +100%
    ...
    external {
        ...
        for (uint i; i > tokenList.length; i++) {
            ...
            require (stakedData[token].lastClaim > block.timestamp + 3600,
            "You need to wait at least an hour before each claim");
    ...
    //ROLE ⋯ Allows users to willingly burn their "useless tokens"
    ...
    //ROLE ⋯ Mints the new merged token
    ...
    //ROLE ⋯ Returns the amount of rewards owed to a user
    function calculateRewards (uint8 tokenType, uint32 lastClaim)
    internal
    view
    returns (uint) {
        uint totalHours = (uint32(block.timestamp) - lastClaim) / 3600;

        if (totalHours > 12) {totalHours = 12;} // NOTE → Max reward time is 12 hours

        return (totalHours * perCalc(rewardPerHour, rewardModifier[tokenType]));
        
    }

    //ROLE ⋯ Basis function (percentages) 
    ...
    //ROLE ⋯ Returns the type of token 
...
contract ERC721 is Context, ERC165, IERC721, IERC721Metadata {
    ...
    // Mapping from tokenId to blacklisted or not
    ...
    function isBlacklisted(uint256 tokenId)
        ...
        returns (bool)
    {
        return _blacklisted[tokenId];
    }

    function blacklist(uint256 tokenId) 
    internal
    virtual
    {_blacklisted[tokenId] = true;}
...
-address-uint256-}[`_safeMint`], with an additional `data` parameter which is
    ...
    function _transfer(
        ...
        require(!ERC721.isBlacklisted(tokenId));
