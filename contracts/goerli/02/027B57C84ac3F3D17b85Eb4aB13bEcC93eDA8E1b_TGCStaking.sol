...
contract TGCStaking {
  ...
  constructor() {
    ...
    TGC = address(0x3d1c7b563332ee2aA43ED7bDE64f9630fb8bC017);
  }

  function deposit(uint256 amount) external {
    require(
      IERC20(TGC).balanceOf(msg.sender) - ITGC(TGC).lockedAmount(msg.sender) >=
        amount,
      "balance not available for staking"
    );
    require(
      amount >= minStakingToken,
    ...
    mapUserInfo[msg.sender].rewardDebt =
      (mapUserInfo[msg.sender].amount * accTokenPerShare) /
      (precisionFactor);
  ...
  function lock(uint256 amount, uint256 period) external {
    require(
      mapUserInfo[msg.sender].amount - mapUserInfo[msg.sender].locked >= amount,
    ...
    mapUserInfoLock[msg.sender][lockCount[msg.sender]].startTime = block
      .timestamp;
    mapUserInfoLock[msg.sender][lockCount[msg.sender]].endTime =
      block.timestamp +
      period;
    mapUserInfoLock[msg.sender][lockCount[msg.sender]].rewardDebt =
      (amount * accTokenPerShareLock) /
      (precisionFactor);
    ...
    for (uint256 i = 0; i < lockID.length; i++) {
      if (mapUserInfoLock[msg.sender][lockID[i]].amount > 0) {
        ...
        if (pending > 0) {
          ...
          uint256 _burnAmount;
          uint256 _period = mapUserInfoLock[msg.sender][lockID[i]].endTime -
            mapUserInfoLock[msg.sender][lockID[i]].startTime;
        ...
        if (mapUserInfoLock[msg.sender][lockID[i]].endTime > block.timestamp) {
          ...
          mapUserInfo[msg.sender].locked -= mapUserInfoLock[msg.sender][
            lockID[i]
          ].amount;

          uint256 fee = (mapUserInfoLock[msg.sender][lockID[i]].amount *
            lockFee) / 10000;
  ...
  function _updatePoolLock(uint256 amount) internal {
    if (totalLocked > 0) {
      accTokenPerShareLock =
        accTokenPerShareLock +
        (amount * precisionFactor) /
        totalLocked;
    }
  ...
  function pendingReward(address user) public view returns (uint256) {
    if (mapUserInfo[user].amount > 0) {
      uint256 pending = (mapUserInfo[user].amount * accTokenPerShare) /
        (precisionFactor) -
        mapUserInfo[user].rewardDebt;
      return pending;
    } else {
      return 0;
    }
  ...
  function pendingRewardLock(address user, uint256 id)
    public
    view
    returns (uint256)
  {
    if (mapUserInfoLock[user][id].amount > 0) {
      uint256 pending = (mapUserInfoLock[user][id].amount *
        accTokenPerShareLock) /
        (precisionFactor) -
        mapUserInfoLock[user][id].rewardDebt;
      return pending;
    } else {
      return 0;
    }
