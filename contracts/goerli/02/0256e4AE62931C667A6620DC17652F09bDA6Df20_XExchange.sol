    ...
    XExchangeStorage
{
    ...
    function matchAskWithTakerBidUsingWeth(
        ...
        bytes32 orderHash = _validateOrder(makerAsk, takerBid);
        ...
        if (item.price > msg.value) {
            ...
            require(item.price == msg.value, "XE: invalid payment");
        }
        ...
        _emitTakerBid(makerAsk, takerBid, item, tokenId, amount);
    }
    ...
    function matchBidWithTakerAsk(
        ...
        bytes32 orderHash = _validateOrder(makerBid, takerAsk);
        orderStatus[orderHash] = true;
        (bool canExecute, uint256 tokenId, uint256 amount) = IStrategy(
            makerBid.strategy
        ...
        _emitTakerAsk(makerBid, takerAsk, item, tokenId, amount);
    }
    ...
    function cancelAllOrdersForSender(uint256 nonce) external override {
        ...
        emit CancalAllOrders(_msgSender(), nonce);
    }
    ...
    function _validateOrder(
        ...
        require(makerOrder.nonce >= userMinNonce[makerOrder.signer], "nonce");
        require(
        ...
        bytes32 orderHash = makerOrder.hashOrderItem(takerOrder.itemIdx);
        require(!orderStatus[orderHash], "XE: order processed");
        return orderHash;
    }
...
interface IXExchange {
    event CancalAllOrders(address indexed user, uint256 newNonce);
