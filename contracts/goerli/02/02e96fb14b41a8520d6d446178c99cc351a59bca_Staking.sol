...
contract Staking is Ownable {
    ...
    struct DepositTransactionHistory{
        uint256[] timestamp;
        uint256[] amount;
    }

    struct ClaimTransactionHistory{
        uint256[] timestamp;
        uint256[] amount;
    }
    ...
    uint256 public maxReturn = 30000;
    ...
    mapping(address => DepositTransactionHistory) private depositTransactionHistory;
    mapping(address => ClaimTransactionHistory) private claimTransactionHistory;
    ...
    function getDepositTransactionHistory(address _holder) public view returns(DepositTransactionHistory memory){
        return depositTransactionHistory[_holder];
    }

    function getClaimTransactionHistory(address _holder) public view returns(ClaimTransactionHistory memory){
        return claimTransactionHistory[_holder];
    }

    function getLastDistributionTime() public view returns(uint256){
        require(block.timestamp > lastDistributionTime, "Invalid time");
        uint times = block.timestamp.sub(lastDistributionTime).div(cliffTime);
        if(times == 0){
            return lastDistributionTime;
        }
        uint currentTime = lastDistributionTime.add(cliffTime.mul(times));
        return currentTime;
    }
    ...
    function updateAccount(address account) private {
        ...
        if (pendingDivs > 0) {
            ...
            claimTransactionHistory[msg.sender].timestamp.push(block.timestamp);
            claimTransactionHistory[msg.sender].amount.push(pendingDivs + referralIncome);
            require(
    ...
    function deposit(uint256 amountToStake, address _referral) public {
        ...
        depositTransactionHistory[msg.sender].timestamp.push(block.timestamp);
        depositTransactionHistory[msg.sender].amount.push(amountToStake);
        require(
