...
import "./interfaces/IPoolCreatorFactory.sol";
import "./Pool.sol";

/// @title Yieldspace v2 Factory contract
/// @notice Yieldspace v2 pool factory that creates instances of Yieldspace v2 pools
contract PoolCreatorFactory is IPoolCreatorFactory {
    event PoolCreated(address indexed base, address indexed fyToken, address pool);

    // maps the pool address to an fyToken address
    mapping(address => address) public poolToFyToken;

    // maps an fyToken address to a pool address
    mapping(address => address) public fyTokenToPool;

    /// @notice creates a pool
    /// @param base base of the pool
    /// @param fyToken fyToken of the pool
    /// @param ts timestretch
    /// @param g1 fee when selling base to the pool
    /// @param g2 fee when selling fyToken to the pool
    function createPool(
        IERC20 base,
        IFYToken fyToken,
        int128 ts,
        ...
        // TODO handle a pool with base/fyToken, but different fee and timestretch

        // make sure fyToken underlying matches the base
        IFYToken _fyToken = IFYToken(fyToken);
        require(_fyToken.underlying() == address(base), "fyToken underlying does not match base");

        // make sure pool with this base and this fyToken hasn't been created yet
        address fyTokenAddress = address(fyToken);
        require(_getPoolAddress(fyTokenAddress) == address(0), "pool already created");

        Pool pool = new Pool(base, fyToken, ts, g1, g2);
        address poolAddress = address(pool);

        poolToFyToken[poolAddress] = fyTokenAddress;
        fyTokenToPool[fyTokenAddress] = poolAddress;

        emit PoolCreated(address(base), fyTokenAddress, poolAddress);

        return poolAddress;
    }

    /// @notice return the Pool associated with a pool address
    /// @param pool pool address
    function getFyTokenAddress(address pool) external view override returns (address) {
        return poolToFyToken[pool];
    }

    /// @notice return the Pool associated with an fyToken address
    /// @param fyToken fyToken address
    function getPoolAddress(address fyToken) public view override returns (address) {
        return fyTokenToPool[fyToken];
    }

    function _getPoolAddress(address fyToken) internal view returns (address) {
        return fyTokenToPool[fyToken];
    }
}
...
interface IPoolCreatorFactory {
    ...
    function getFyTokenAddress(address pool) external view returns (address);

    function getPoolAddress(address fyToken) external view returns (address);
}
