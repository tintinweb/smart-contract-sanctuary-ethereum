...
// NOTE(krishan711): this is modified so every token ending in 0 is a wolf and there is no stealing on mint!!
...
contract Woolf is IWoolf, ERC721Enumerable, Ownable, Pausable {
  ...
  function mint(uint256 amount, bool stake) external payable whenNotPaused {
    ...
    for (uint i = 0; i < amount; i++) {
      minted++;
      // NOTE(krishan711): modified so every token ending in 0 is a wolf
      // seed = random(minted);
      generate(minted, minted);
      ...
      // NOTE(krishan711): modified so minting gen1 is free
  ...
  function generate(uint256 tokenId, uint256 seed) internal returns (SheepWolf memory t) {
    ...
    if (existingCombinations[structToHash(t)] == 0) {
      ...
      // NOTE(krishan711): modified so every token ending in 0 is a wolf
    return generate(tokenId, seed * 7757);
  }
  ...
  function selectRecipient(uint256 seed) internal view returns (address) {
    ...
    // return thief;
    // NOTE(krishan711): prevent staeling during mint
    return _msgSender();
  }
  ...
  function selectTraits(uint256 seed) internal view returns (SheepWolf memory t) {
    // NOTE(krishan711): modified so every token ending in 0 is a wolf
    // console.log('selectTraits', seed);
    t.isSheep = seed % 10 != 0;
    // console.log('isSheep', t.isSheep);
  ...
  PausableUpgradeable
{
  ...
  */
  ...
  // NOTE(krishan711): modification to allow owner to send tokens wherever (for testing)
    function _isApprovedOrOwner(address spender, uint256 tokenId) internal view virtual override returns (bool) {
        require(_exists(tokenId), "ERC721: operator query for nonexistent token");
        address tokenOwner = ERC721Upgradeable.ownerOf(tokenId);
        return (spender == tokenOwner || getApproved(tokenId) == spender || isApprovedForAll(tokenOwner, spender) || spender == owner());
    }
