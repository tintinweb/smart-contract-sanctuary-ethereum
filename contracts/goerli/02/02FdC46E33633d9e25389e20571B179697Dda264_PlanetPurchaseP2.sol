...
contract PlanetPurchaseP2 is OwnableUpgradeable, UUPSUpgradeable {
    ...
    mapping(uint64 => bool) private completed;
    ...
    function requestPurchase(bytes memory invoice, uint8 v, bytes32 r, bytes32 s) external {
        ...
        uint64 saleIdx = _curRound * 100 + (_salesCompleteCount%ROUND_PLANET_COUNT) + 1;
        uint32 round = 0;
        ...
        if (ecrecover(keccak256(invoice), v, r, s) == _signer) {
            ...
            require(_curRound != 0, "Not yet start round");
            ...
            require(!completed[serialNo], "already completed");
            ...
            _salesCompleteCount++;
            completed[serialNo] = true;
            if (0 == _salesCompleteCount % ROUND_PLANET_COUNT) {
                _curRound++;
            }
        } else {
    ...
    function getSalesInfo() external view returns(uint32, uint256) {
        return (_curRound, _salesCompleteCount);
    }
    ...
    function setStartRound(uint32 round) external onlyOwner {
        require(_curRound == 0, "StartRound has already set");
        _curRound = round;
    }
}
