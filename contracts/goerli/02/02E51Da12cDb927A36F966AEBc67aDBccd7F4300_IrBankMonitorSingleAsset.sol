...
contract IrBankMonitorSingleAsset {
    ...
    constructor(address _owner) {
        ...
        factory = StakingRewardsHelperInterface(stakeRewardsHelper).factory();
    }
    ...
    function _getPtokenInternal(address _underlying) internal view returns (address) {
        address _pToken;
        _pToken = StakingRewardsFactoryInterface(factory).getStakingToken(
            _underlying
        );
        return _pToken;
    ...
    function getStakeReward(address _underlying) external view returns (address) {
        ...
        _stakeReward = StakingRewardsFactoryInterface(factory).getStakingRewards(
            _pToken
        );
        return _stakeReward;
    }
    ...
    function _getStakeRewardInternal(address _underlying) internal view returns (address) {
        address _pToken;
        address _stakeReward;
        _pToken = _getPtokenInternal(_underlying);
    ...
    function getUnderlyingConfig(address _underlying) external view returns (address, address, uint256, uint256) {
        ...
        _stakeReward = StakingRewardsFactoryInterface(factory)
            .getStakingRewards(_pToken);
        _decimals = IERC20(_underlying).decimals();
        _baseUnit = 10**_decimals;

        return (_pToken, _stakeReward, _decimals, _baseUnit);
