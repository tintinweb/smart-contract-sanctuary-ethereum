...
contract PlatinumPrimates is ERC721A, Ownable, ERC721AQueryable, ReentrancyGuard {
    using SafeMath for uint256;
    using Strings for uint256;
    using Address for address;

    uint256 public MAX_TOKENS = 5555; //Tentative
    uint256 public constant MAX_PER_MINT = 2; //Tentative
    uint256 private constant maxBatchSize = 2; //Tentative

    address public withdrawalWallet;

    uint256 public price = .095 ether; //Tentative
    ...
    string private baseURI =
        "https://notsetyet.xyz/"; //Tentative
    bytes32 public merkleRoot;

    constructor() ERC721A("Platinum Primates ERC 721-A Contract", "PLTPRMTE") {
        withdrawalWallet = msg.sender; //initialize withdrawal wallet to deployer
    }
        ...
        returns (string memory)
    {
        if (isRevealed) {
            ...
            return
                string(
                    abi.encodePacked(
                        "https://platcols.s3.amazonaws.com/oQQKnVAH6NYI8d0r/",
                        "symbol.json"
                    )
                ); //Tentative
        }
