...
import { ResolverConfigurator } from "./ResolverConfigurator.sol";
...
import { IDepositor, IDepositorOptions } from "../interfaces/IDepositor.sol";
import { IWithdrawer, IWithdrawerOptions } from "../interfaces/IWithdrawer.sol";
import { ILPWorkerOptions } from "../interfaces/ILPWorker.sol";

contract ConvexPathResolver is IPathResolver, ResolverConfigurator {
    using StrategyPathTaskOps for StrategyPathTask;
    using BalanceOps for Balance[];

    ISwapAggregator public swapAggregator;

    IDepositorOptions public curveDepositor;
    IWithdrawerOptions public curveWithdrawer;

    IDepositorOptions public convexDepositor;
    IWithdrawerOptions public convexWithdrawer;

    /// @notice Pathfinder Component ID
    PathFinderComponent public override getComponentId =
        PathFinderComponent.CONVEX_PATH_RESOLVER;

    // Contract version
    uint256 public constant override version = 1;

    constructor(address _pathFinder) ResolverConfigurator(_pathFinder) {}

    /// @dev Updates components taking their latest versions from root path finder
    function _updatePathFinderComponents() internal override {
        swapAggregator = ISwapAggregator(
            pathFinder.componentAddressById(PathFinderComponent.SWAP_AGGREGATOR)
        );

        curveDepositor = IDepositorOptions(
            pathFinder.componentAddressById(
                PathFinderComponent.CURVE_LP_DEPOSITOR
            )
        );

        curveWithdrawer = IWithdrawerOptions(
            pathFinder.componentAddressById(
                PathFinderComponent.CURVE_LP_WITHDRAWER
            )
        );

        convexDepositor = IDepositorOptions(
            pathFinder.componentAddressById(
                PathFinderComponent.CONVEX_DEPOSITOR
            )
        );

        convexWithdrawer = IWithdrawerOptions(
            pathFinder.componentAddressById(
                PathFinderComponent.CONVEX_WITHDRAWER
            )
        );
    }
    ...
    function findOneTokenPath(
        ...
        if (
            typeIn == TokenType.NORMAL_TOKEN &&
            (task.targetType == TokenType.CONVEX_LP_TOKEN ||
                task.targetType == TokenType.CONVEX_STAKED_TOKEN)
        ) {
            return normal2AnyConvex(tokenIn, amount, task);
        }

        if (
            (typeIn == TokenType.CONVEX_LP_TOKEN ||
                typeIn == TokenType.CONVEX_STAKED_TOKEN) &&
            task.targetType == TokenType.NORMAL_TOKEN
        ) {
            return anyConvex2Normal(tokenIn, amount, task);
        }

        if (
            typeIn == TokenType.CURVE_LP_TOKEN &&
            (task.targetType == TokenType.CONVEX_LP_TOKEN ||
                task.targetType == TokenType.CONVEX_STAKED_TOKEN)
        ) {
            return curve2AnyConvex(tokenIn, amount, task);
        }

        if (
            (typeIn == TokenType.CONVEX_LP_TOKEN ||
                typeIn == TokenType.CONVEX_STAKED_TOKEN) &&
            task.targetType == TokenType.CURVE_LP_TOKEN
        ) {
            return anyConvex2Curve(tokenIn, amount, task);
        }
        ...
        if (
            ...
            return anyConvex2StakedConvex(tokenIn, amount, task);
        }
        ...
        if (
            ...
            return anyStakedConvex2Convex(tokenIn, amount, task);
        }
        revert PathNotFoundException(tokenIn, task.target);
    }

    function findOpenStrategyPath(StrategyPathTask memory task)
        public
        override
        returns (StrategyPathTask memory bestTask)
    {
        address convexUnderlying;
        {
            address[] memory convexUnderlyings;

            (convexUnderlyings, task) = convexDepositor.getUnderlyings(
                TokenType.CURVE_LP_TOKEN,
                task.target,
                task
            );

            convexUnderlying = convexUnderlyings[0];
        }

        address[] memory curveUnderlyings;
        (curveUnderlyings, task) = curveDepositor.getUnderlyings(
        ...
        uint256 len = curveUnderlyings.length;
        for (uint256 i; i < len; ++i) {
            StrategyPathTask memory currentTask = task.clone();
            currentTask = swapAggregator.swapAllNormalTokens(
                currentTask,
                curveUnderlyings[i]
            );

            (, currentTask) = curveDepositor.depositAllTokens(
                convexUnderlying,
                currentTask
            );

            (, currentTask) = convexDepositor.depositAllTokens(
                currentTask.target,
                currentTask
            );

            if (currentTask.isBetter(bestTask)) {
    ...
    /// @dev NORMAL TOKEN -> CONVEX LP/STAKED TOKEN
    function normal2AnyConvex(
        address tokenIn,
        uint256 amount,
        StrategyPathTask memory task
    ) public returns (StrategyPathTask memory bestTask) {
        address convexUnderlying;

        {
            address[] memory convexUnderlyings;

            (convexUnderlyings, task) = ILPWorkerOptions(convexDepositor)
                .getUnderlyings(TokenType.CURVE_LP_TOKEN, task.target, task);
        ...
        bool isAllCurve = task.balances.getBalance(convexUnderlying) < 2;
        ...
        for (uint256 i; i < len; ++i) {
            ...
            bool isAllMiddle = task.balances.getBalance(curveUnderlyings[i]) <
                2;

            uint256 amountOut = amount;

            if (tokenIn != curveUnderlyings[i]) {
                (amountOut, currentTask) = swapAggregator.findBestSwap(
                    tokenIn,
                    amountOut,
                    curveUnderlyings[i],
                    currentTask
            ...
            if (isAllMiddle) {
                (amountOut, currentTask) = curveDepositor.depositAll(
                    TokenType.NORMAL_TOKEN,
                    curveUnderlyings[i],
                    convexUnderlying,
                    currentTask
                ...
                (amountOut, currentTask) = curveDepositor.deposit(
                    TokenType.NORMAL_TOKEN,
                    curveUnderlyings[i],
                    amountOut,
                    convexUnderlying,
            ...
            if (isAllCurve) {
                (amountOut, currentTask) = convexDepositor.depositAll(
                    TokenType.CURVE_LP_TOKEN,
                    convexUnderlying,
                    currentTask.target,
                    currentTask
                ...
                (amountOut, currentTask) = convexDepositor.deposit(
                    TokenType.CURVE_LP_TOKEN,
                    convexUnderlying,
                    amountOut,
                    currentTask.target,
        ...
        if (!pathFound) revert PathNotFoundException(tokenIn, task.target);

        return bestTask;
    }

    /// @dev CONVEX LP/STAKED TOKEN ->  NORMAL TOKEN
    function anyConvex2Normal(
        address tokenIn,
        uint256 amount,
        StrategyPathTask memory task
    ) public returns (StrategyPathTask memory bestTask) {
        ...
        address[] memory curveUnderlyings;
        (curveUnderlyings, task) = curveWithdrawer.getUnderlyings(
            TokenType.NORMAL_TOKEN,
            convexUnderlying,
            task
        );
        ...
        (amount, task) = convexWithdrawer.withdraw(
            tokenIn,
            amount,
            TokenType.CURVE_LP_TOKEN,
            convexUnderlying,
        ...
        for (uint256 i; i < len; ++i) {
            ...
            if (isAllCurve) {
                (amountOut, currentTask) = curveWithdrawer.withdrawAll(
                    convexUnderlying,
                    TokenType.NORMAL_TOKEN,
                    curveUnderlyings[i],
                    currentTask
                ...
                (amountOut, currentTask) = curveWithdrawer.withdraw(
                    convexUnderlying,
                    amountOut,
                    TokenType.NORMAL_TOKEN,
                    curveUnderlyings[i],
            ...
            if (task.target != curveUnderlyings[i]) {
                if (isAllMiddle) {
                    (amountOut, currentTask) = swapAggregator
    ...
    /// @dev CURVE_LP TOKEN -> CONVEX LP/STAKED TOKEN
    function curve2AnyConvex(
        address tokenIn,
        uint256 amount,
        StrategyPathTask memory task
    ) public view returns (StrategyPathTask memory) {
        address convexUnderlying;
        ...
        if (tokenIn == convexUnderlying) {
            (amount, task) = convexDepositor.deposit(
                TokenType.CURVE_LP_TOKEN,
                tokenIn,
                amount,
                task.target,
            ...
            uint256 amountOut;
            (amountOut, task) = curveDepositor.deposit(
                TokenType.CURVE_LP_TOKEN,
                tokenIn,
                amount,
                convexUnderlying,
            ...
            if (isAllCurve) {
                (amountOut, task) = convexDepositor.depositAll(
                    TokenType.CURVE_LP_TOKEN,
                    convexUnderlying,
                    task.target,
                    task
                ...
                (amountOut, task) = convexDepositor.deposit(
                    TokenType.CURVE_LP_TOKEN,
                    convexUnderlying,
                    amountOut,
                    task.target,
    ...
    /// @dev CONVEX LP/STAKED TOKEN -> CURVE_LP TOKEN
    function anyConvex2Curve(
        address tokenIn,
        uint256 amount,
        StrategyPathTask memory task
    ) public view returns (StrategyPathTask memory) {
        ...
        (amountOut, task) = convexWithdrawer.withdraw(
            tokenIn,
            amountOut,
            TokenType.CURVE_LP_TOKEN,
            convexUnderlying,
        ...
        if (task.target != convexUnderlying) {
            if (isAllCurve) {
                (amountOut, task) = curveWithdrawer.withdrawAll(
                    convexUnderlying,
                    TokenType.CURVE_LP_TOKEN,
                    task.target,
                    task
                ...
                (amountOut, task) = curveWithdrawer.withdraw(
                    convexUnderlying,
                    amountOut,
                    TokenType.CURVE_LP_TOKEN,
                    task.target,
    ...
    function anyConvex2StakedConvex(
        address tokenIn,
        uint256 amount,
        StrategyPathTask memory task
    ) public view returns (StrategyPathTask memory) {
        (, task) = convexDepositor.deposit(
            TokenType.CONVEX_LP_TOKEN,
            tokenIn,
            amount,
            task.target,
            task
    ...
    function anyStakedConvex2Convex(
        address tokenIn,
        uint256 amount,
        StrategyPathTask memory task
    ) public view returns (StrategyPathTask memory) {
        (, task) = convexWithdrawer.withdraw(
            tokenIn,
            amount,
            TokenType.CONVEX_LP_TOKEN,
            task.target,
...
abstract contract ResolverConfigurator {
    ...
    function updateComponents() external pathFinderOwnerOnly {
        // [TODO]: Add events(?)
        _updatePathFinderComponents();
    }
...
interface ISwapAggregator is IPathFinderComponent {
    function findAllSwaps(SwapTask memory swapTask, address[] memory adapters)
        external
        returns (SwapQuote[] memory results);
...
interface IWithdrawer is ILPWorker {
    function withdraw(
        uint256 amountIn,
        address lpToken,
        StrategyPathTask memory task
    ...
    function withdrawAll(address lpToken, StrategyPathTask memory task)
        external
        view
        returns (uint256 amountOut, StrategyPathTask memory);

    function withdrawAllTokens(StrategyPathTask memory task)
        external
        view
        returns (StrategyPathTask memory);
}

interface IWithdrawerOptions is ILPWorkerOptions {
    function withdraw(
        address tokenIn,
        uint256 amount,
        TokenType ttOut,
    ...
    function withdrawAllTokens(
        StrategyPathTask memory task,
        PathOption[] memory pathOptions
    ) external view returns (StrategyPathTask memory);
}
