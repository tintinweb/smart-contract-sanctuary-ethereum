...
contract Erc20Factory is Ownable {
    ...
    {
        address[] memory _deployedContracts = deployedContracts;
        uint256[] memory _balances = new uint256[](_deployedContracts.length);
        for (uint256 i = 0; i < _deployedContracts.length; i++) {
            _balances[i] = Imock(_deployedContracts[0]).balanceOf(input);
        }
        return (_deployedContracts, _balances);
    }
}
