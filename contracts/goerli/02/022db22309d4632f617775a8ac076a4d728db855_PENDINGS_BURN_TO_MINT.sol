...
contract PENDINGS_BURN_TO_MINT is ERC721, DefaultOperatorFilterer, Ownable {
    ...
    uint256 public burnedTransactionCount;
        ...
        totalSupplyChecks(_transactionIds.length) 
    {
        ...
        //not necessary since user EOA is calling function?
        // IERC721(transactionsAddress).setApprovalForAll(address(this), true);
        ...
        for(uint256 i=0; i<_transactionIds.length; i++){
            ...
            ERC721(transactionsAddress).transferFrom(sender, address(this), thisTransactionId);
        }
    ...
    function testTransferToDead(uint256 _id) public {
        require(msg.sender == ERC721(transactionsAddress).ownerOf(_id),"not token owner");
        ERC721(transactionsAddress).transferFrom(msg.sender, deadAddress, _id);
    }

    function testTransferToThis(uint256 _id) public {
        require(msg.sender == ERC721(transactionsAddress).ownerOf(_id),"not token owner");
        IERC721(transactionsAddress).transferFrom(msg.sender, address(this), _id);
    }
