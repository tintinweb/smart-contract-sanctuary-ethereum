...
contract ApiensGen2 is Allowable {    
    ...
    // allowed phase
    uint256 ALLOW_PER = 2;
    uint256 ALLOW_SUPPLY = 5600;
    uint256 ALLOW_PRICE = (1 ether)*.15;
    
    // free claim phase
    uint256 FREE_PER = 0;
    uint256 FREE_SUPPLY = 3000;
    uint256 FREE_PRICE = (1 ether)*0;    
    ...
    function initializePhases() internal virtual override {
        ...
        phases.push(Phase("allowedBags", ALLOW_SUPPLY, ALLOW_SUPPLY, ALLOW_PRICE));
    ...
    external requiresClaimSig(signature,msg.sender,genesisBags,stakedApiens) {        
        for (uint i; i < genesisBags.length; i++) {
    ...
    external payable requiresClaimSig(signature,msg.sender,genesisBags,empty) { 
        for (uint i; i < genesisBags.length; i++) {
            if (hasBeenClaimed(genesisBags[i], GENESIS_BAG)) {
                revert BagAlreadyClaimed(genesisBags[i]);
            }
    ...
    /// @notice minting ALLOW_PHASE, requires trusted signature, maximum quantity 2 per wallet
    ...
    external payable requiresAllowSig(signature,msg.sender) {  
        ...
        phasedMint(ALLOW_PHASE, quantity, true);        

        setAux(msg.sender, getAux(msg.sender)+quantity);
    }
...
abstract contract Lockable is Approvable {    
    ...
    function isLocked(uint256 id) public view returns (bool) {     
        return soulBound || lockable.lockableStatus[ownerOf(id)].isLocked;
    } 
...
abstract contract Mintable is LockableTransferrable {  
    ...
    function hasBeenClaimed(uint256 tokenId, address addressed) public view returns (bool) {
        return mintables.isClaimed(addressed,tokenId);
    }

    function claim(uint256 tokenId, address addressed) internal {
        mintables.claim(addressed,tokenId);
    }
...
library SetPackable {
     ...
     * Returns the auxiliary data for `owner`. (e.g. number of whitelist mint slots used).
     */
    function _getAux(PackableData storage self,address owner) public view returns (uint64) {
     ...
     */
    function _startTokenId(PackableData storage self) internal pure returns (uint256) {
        return 1;
    }  
