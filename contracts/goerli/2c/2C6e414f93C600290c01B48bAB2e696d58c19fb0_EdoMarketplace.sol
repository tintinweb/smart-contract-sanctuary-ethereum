...
import "./EdoMarketplaceManagement.sol";

contract EdoMarketplace is EdoMarketplaceManagement, ReentrancyGuardUpgradeable {
    struct NftMeta {
        bytes32 nftId;
        address owner;
    ...
    modifier onlyEdoNft(address contractAddress) {
        require(
            isEdoNFTContractAddress[contractAddress],
            "EdoMarketplace: Invalid nft contract address."
        );
        _;
    }

    modifier onlyNftOwner(address nftContractAddress, uint256 tokenId) {
        NftMeta memory nftMeta = tokenMeta[
            getHash(nftContractAddress, tokenId)
        ];
        require(
            nftMeta.owner == msg.sender,
            "EdoMarketplace: Caller is not the owner."
        );
        _;
    }
    ...
    ) public initializer feeLimit(_serviceFeeRate) {
        uint256 totalFeeRate = _agentRefundFeeRate +
            _edoVerseFoundationFeeRate +
            _edoVerseIncFeeRate;
    ...
    function setListingNFT(
        ...
        address paymentContractAddress,
        uint256 price
    ) public onlyEdoNft(nftContractAddress) nonReentrant {
        require(
            isEdoPaymentContractAddress[paymentContractAddress],
            "EdoMarketplace: Invalid payment contract address."
        );
    ...
    function buyEdoNFT(
        ...
        require(
            nftMeta.owner != msg.sender,
            "EdoMarketplace: You cannot purchase your own NFTs."
        );
        require(nftMeta.owner != address(0), "EdoMarketplace: Not list.");

        uint256 balance = IERC20Upgradeable(nftMeta.paymentContractAddress)
            .balanceOf(msg.sender);
        require(
            nftMeta.price <= balance,
            "EdoMarketplace: Insufficient balance."
        );

        uint256 serviceProfit = (nftMeta.price * serviceFeeRate) / DEMOMINATOR;

        uint256 sellerProfit = nftMeta.price - serviceProfit;

        SafeERC20Upgradeable.safeTransferFrom(
            IERC20Upgradeable(nftMeta.paymentContractAddress),
            msg.sender,
            address(this),
            serviceProfit
        );
...
import "./EdoMarketplaceManagementAdmin.sol";
...
contract EdoMarketplaceManagement is EdoMarketplaceManagementAdmin {
    ...
    event AgentRefundAddress(address agentRefundAddress);
    event ServiceFeeRate(uint256 serviceFeeRate);

    modifier onlyAgent() {
        require(
            agentAddress == msg.sender,
            "EdoMarketplaceManagement: Caller is not the agent."
        );
        _;
    }
    ...
    modifier onlyAgentOrOwner() {
        require(
            agentAddress == msg.sender || super.owner() == msg.sender,
            "EdoMarketplaceManagement: Caller is not the agent."
        );
        _;
    }
    ...
    modifier feeLimit(uint256 feeRate) {
        require(
            ...
            "EdoMarketplaceManagement: Fee rates are too high."
        );
        require(
            feeRate >= MIN_FEE_RATE,
            "EdoMarketplaceManagement: Fee rates are too low."
        );
        _;
    }

    function setAgentRefundAddress(
        address _agentRefundAddress
    ) external onlyAgent {
        require(_agentRefundAddress != address(0));
        agentRefundAddress = _agentRefundAddress;
        emit AgentRefundAddress(agentRefundAddress);
    }

    function setServiceFeeRate(
        uint256 _serviceFeeRate
    ) external onlyAgent feeLimit(_serviceFeeRate) {
        serviceFeeRate = _serviceFeeRate;
        emit ServiceFeeRate(serviceFeeRate);
    }

    function depositFeeAmount(
        address paymentContractAddress
    ) external view onlyAgentOrOwner returns (uint256) {
        return
            IERC20Upgradeable(paymentContractAddress).balanceOf(address(this));
    }

    function withdraw(
        address paymentContractAddress
    ) external onlyAgentOrOwner {
        uint256 balance = IERC20Upgradeable(paymentContractAddress).balanceOf(
            address(this)
        );

        uint256 edoVerseFoundationProfit = (balance *
            edoVerseFoundationFeeRate) / DEMOMINATOR;

        uint256 edoVerseIncProfit = (balance * edoVerseIncFeeRate) /
            DEMOMINATOR;

        uint256 agentRefundProfit = balance -
            edoVerseFoundationProfit -
            edoVerseIncProfit;

        SafeERC20Upgradeable.safeTransfer(
            IERC20Upgradeable(paymentContractAddress),
            edoVerseFoundationAddress,
            edoVerseFoundationProfit
        );
        SafeERC20Upgradeable.safeTransfer(
            IERC20Upgradeable(paymentContractAddress),
...
contract EdoMarketplaceManagementAdmin is Initializable, OwnableUpgradeable {
    ...
    event AgentAddress(address agentAddress);
    event EdoVerseFoundationAddress(address edoVerseFoundationAddress);
    event EdoVerseIncAddress(address edoVerseIncAddress);
    event IncludeNFTContractAddress(bool nFTContractAddressExist);
    event ExcludeNFTContractAddress(bool nFTContractAddressNotExist);
    event IncludePaymentContractAddress(bool edoPaymentContractAddressExist);
    event ExcludePaymentContractAddress(bool edoPaymentContractAddressNotExist);
    event FeeRateDistribution(
        uint256 indexed agentRefundFeeRate,
        uint256 indexed edoVerseFoundationFeeRate,
        uint256 indexed edoVerseIncFeeRate
    );

    function setAgentAddress(address _agentAddress) external onlyOwner {
        require(_agentAddress != address(0));
        agentAddress = _agentAddress;
        emit AgentAddress(agentAddress);
    }

    function setEdoVerseFoundationAddress(
        address _edoVerseFoundationAddress
    ) external onlyOwner {
        require(_edoVerseFoundationAddress != address(0));
        edoVerseFoundationAddress = _edoVerseFoundationAddress;
        emit EdoVerseFoundationAddress(edoVerseFoundationAddress);
    }

    function setEdoVerseIncAddress(
        address _edoVerseIncAddress
    ) external onlyOwner {
        require(_edoVerseIncAddress != address(0));
        edoVerseIncAddress = _edoVerseIncAddress;
        emit EdoVerseIncAddress(edoVerseIncAddress);
    }
    ...
    function includeNFTContractAddress(
        ...
        emit IncludeNFTContractAddress(
            isEdoNFTContractAddress[_NFTContractAddress]
        );
    }

    function excludeNFTContractAddress(
        address _NFTContractAddress
    ) external onlyOwner {
        isEdoNFTContractAddress[_NFTContractAddress] = false;
        emit ExcludeNFTContractAddress(
            isEdoNFTContractAddress[_NFTContractAddress]
        );
    }

    function includePaymentContractAddress(
        address _edoPaymentContractAddress
    ) external onlyOwner {
        isEdoPaymentContractAddress[_edoPaymentContractAddress] = true;
        emit IncludePaymentContractAddress(
            isEdoPaymentContractAddress[_edoPaymentContractAddress]
        );
    }

    function excludePaymentContractAddress(
        address _edoPaymentContractAddress
    ) external onlyOwner {
        isEdoPaymentContractAddress[_edoPaymentContractAddress] = false;
        emit ExcludePaymentContractAddress(
            isEdoPaymentContractAddress[_edoPaymentContractAddress]
        );
    }
    ...
    function setFeeRateDistribution(
        ...
        require(
            totalFeeRate == 100,
            "EdoMarketplaceManagement: Total value is invalid."
        );

        agentRefundFeeRate = _agentRefundFeeRate;
        edoVerseFoundationFeeRate = _edoVerseFoundationFeeRate;
        edoVerseIncFeeRate = _edoVerseIncFeeRate;
        emit FeeRateDistribution(
            agentRefundFeeRate,
            edoVerseFoundationFeeRate,
            edoVerseIncFeeRate
        );
    }
