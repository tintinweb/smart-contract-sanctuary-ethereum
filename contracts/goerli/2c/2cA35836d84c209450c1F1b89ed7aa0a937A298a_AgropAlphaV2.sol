...
contract AgropAlphaV2 is IAgrop, IHub {
        ...
        )
    {
        ...
        // is hub exist?
        require(_hub.agrop() != address(0), "agrop3");
    ...
    function paginateCropsInHub(
        ...
        uint256 _from,
        uint256 _to
    ) external returns (CropOptions[] memory) {
        Hub _hub = Hub(_hubAddress);
    ...
    {
        require(_from >= _to, "agrop4");

        require((_from - _to) <= 10, "agrop5");
        ...
        onlyOwner
    {
        ...
        // _plan must be monthly, quarterly, yearly
        require(plan, "agrop10");

        require(_days > 0, "agrop11");
    ...
    function transferOwnership(address newOwner) external onlyOwner {
        require(newOwner != address(0), "own2");
...
interface IAgropQuality is IAgrop {
    ...
    function validateCropOptionsInput(CropOptions memory _crop)
        external
        pure
        returns (bool);
...
contract Hub is IHub, IAgrop {
    ...
    mapping(uint256 => CropOptions) public crops;
    uint256 public counter;

    // caller must be agrop contract
    modifier onlyAgrop() {
        require(agrop == msg.sender, "own3");
        _;
    }

    // is hub subscription active
    modifier isSubscriptionActive() {
        require(subscriptionExpiresAt >= block.timestamp, "hub1");
        _;
    }

    // modifiers for freezed
    modifier ifNotFreezed() {
        require(freezed == false, "hub2");
        _;
    }
    modifier ifFreezed() {
        require(freezed == true, "hub3");
        _;
    }

    // modifiers for verified
    modifier ifNotVerified() {
        require(verified == false, "hub4");
        _;
    }
    modifier ifVerified() {
        require(verified == true, "hub5");
        _;
    }
        ...
        returns (bool)
    {
        ...
        IAgropQuality _contract = IAgropQuality(
            0x09545862fe43dF796DFc56A38915101348fa4742
        );

        // check if all require data is submitted
        require(_contract.validateCropOptionsInput(_crop), "hub7");
    ...
    function paginateCrops(uint256 _from, uint256 _to)
        ...
        ifNotFreezed
        isSubscriptionActive
        returns (CropOptions[] memory)
    {
        require(_from >= _to, "hub8");

        require((_from - _to) <= 10, "hub9");

        CropOptions[] memory _crop = new CropOptions[]((_from - _to) + 1);
        uint256 k = 0;
