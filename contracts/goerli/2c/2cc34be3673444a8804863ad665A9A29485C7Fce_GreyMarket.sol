...
contract GreyMarket is OwnableUpgradeable, ReentrancyGuardUpgradeable, GreyMarketStorage, GreyMarketEvent {
    ...
    modifier validateCreateOrder(bytes calldata signature, uint256 id, address seller, address paymentToken, uint256 orderType, uint256 amount)
    {
        bytes32 hash = keccak256(abi.encodePacked("create_order", id, seller, paymentToken, orderType, amount));
        require(proofSigner == hash.toEthSignedMessageHash().recover(signature), "invalid create signature");
        _;
    }

    modifier validateClaimOrder(bytes calldata signature, uint256 id, uint256 orderStatus, address buyer, address seller)
    {
        bytes32 hash = keccak256(abi.encodePacked("claim_order", id, orderStatus, buyer, seller));
        require(proofSigner == hash.toEthSignedMessageHash().recover(signature), "invalid claim signature");
        _;
    }

    modifier validateWithdrawOrder(bytes calldata signature, uint256 id, uint256 orderStatus, address buyer, address seller)
    {
        bytes32 hash = keccak256(abi.encodePacked("withdraw_order", id, orderStatus, buyer, seller));
        require(proofSigner == hash.toEthSignedMessageHash().recover(signature), "invalid withdraw signature");
        _;
    }

    modifier validateReleaseDisputedOrder(bytes[] calldata signatures, uint256 id, uint256 orderStatus, address buyer, address seller, address winner)
    {
        require(admins.length == signatures.length, "invalid signature count");
        bytes32 hash = keccak256(abi.encodePacked("release_dispute_order", id, orderStatus, buyer, seller, winner));
        for (uint256 i; i < admins.length; i++) {
            require(admins[i] == hash.toEthSignedMessageHash().recover(signatures[i]), "invalid release dispute signature");
        }
        _;
    }
     ...
     */
    function createOrder(uint256 id, address seller, address paymentToken, OrderType orderType, uint256 amount, bytes calldata proofSignature)
        external
        payable
        validateCreateOrder(proofSignature, id, seller, paymentToken, uint256(orderType), amount)
    {
     ...
     * @param id The order id.
     */
    function claimOrder(uint256 id, address buyer, address seller, bytes calldata proofSignature)
        public
        validateClaimOrder(proofSignature, id, uint256(OrderStatus.ORDER_DELIVERED), buyer, seller)
    {
        OrderInfo storage orderInfo = orders[id];
     ...
     */
    function claimOrders(uint256[] calldata ids, address[] calldata buyers, address[] calldata sellers, bytes[] calldata proofSignatures)
        external
    {
        require(ids.length == proofSignatures.length, "invalid length");
        uint256 len = ids.length;
        uint256 i;

        unchecked {
            do {
               claimOrder(ids[i], buyers[i], sellers[i], proofSignatures[i]);
            } while(++i < len);
        }
    }
     ...
     * @param id The order id.
     */
    function withdrawOrder(uint256 id, address buyer, address seller, bytes calldata proofSignature)
        external
        validateWithdrawOrder(proofSignature, id, uint256(OrderStatus.ORDER_CANCELLED), buyer, seller)
    {
        OrderInfo storage orderInfo = orders[id];
     ...
     * @param id The order id.
     */
    function releaseDisputedOrder(uint256 id, address buyer, address seller, address winner, bytes[] calldata adminSignatures)
        external
        validateReleaseDisputedOrder(adminSignatures, id, uint256(OrderStatus.ORDER_DISPUTE), buyer, seller, winner)
    {
