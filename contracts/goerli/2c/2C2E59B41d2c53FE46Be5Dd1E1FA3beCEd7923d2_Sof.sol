...
contract Sof is ERC1155, ERC1155Supply, Ownable {
    uint256 public constant SOF_TOKEN = 0;
    ...
    uint256 public SOF_PRICE = 0.05 ether;
    uint256 public SOF_MINT_LIMIT = 2;
    ...
    mapping (address => uint256) public mintedSOF;
    ...
    modifier checkPhaseSupply(uint8 mintPhase, uint256 numOfTokens) {
        ...
        for (index = 0; index < mintPhase + 1; index++) {
            ...
            require(numOfTokens + totalSupply(SOF_TOKEN) <= currentPhaseSupply, 'NOT_ENOUGH_SUPPLY');
        }
        _;
    }
    ...
    function publicMint(uint256 numOfTokens, uint8 mintPhase) external isSecured(mintPhase) checkPhaseSupply(mintPhase, numOfTokens) payable {
        require(mintedSOF[msg.sender] + numOfTokens < SOF_MINT_LIMIT, "MINTED_ALREADY");
        require(msg.value == SOF_PRICE * numOfTokens, "WRONG_ETH_VALUE");

        addressBlockBought[msg.sender] = block.timestamp;
        mintedSOF[msg.sender] += numOfTokens;

        _mint(msg.sender, SOF_TOKEN, numOfTokens, "");
    }

    function freeMint(uint256 numOfTokens, uint8 mintPhase, bytes32[] memory proof) external isSecured(mintPhase) checkPhaseSupply(mintPhase, numOfTokens) payable {
