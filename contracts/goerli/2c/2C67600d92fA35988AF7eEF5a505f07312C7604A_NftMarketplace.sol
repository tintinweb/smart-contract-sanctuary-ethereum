...
error NftMarketplace__NotListing(address nftAddress, uint256 tokenId);
...
contract NftMarketplace is ReentrancyGuard{
    ...
    modifier isListed(address nftAddress, uint256 tokenId){
        Listing memory listing = s_listings[nftAddress][tokenId];
        if (listing.price <= 0) {
            revert NftMarketplace__NotListing(nftAddress,tokenId);
        }_;
    }
    ...
    function listItem(
        ...
        // 1. Send NFT to ther contract, Transfer --> Contract "hold" the NFT
        // 2. Owners can still hold their NFT, and give the marketplace approval to sell the NFT for them.abi
        // 3.
        IERC721 nft = IERC721(nftAddress);
