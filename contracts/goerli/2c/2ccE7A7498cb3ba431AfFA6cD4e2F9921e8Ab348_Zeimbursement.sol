...
contract Zeimbursement {
    ...
    mapping (address => mapping (uint256 => bool)) internal users;
    RSAGroth16Verifier internal rsaVerifier;
    ECDSAGroth16Verifier internal ecdsaVerifier;

    constructor(address rsaVerifierAddress, address ecdsaVerifierAddress, address adminAddress) {
        admins[adminAddress] = true;
        rsaVerifier = RSAGroth16Verifier(rsaVerifierAddress);
        ecdsaVerifier = ECDSAGroth16Verifier(ecdsaVerifierAddress);
    }

    function designateAdmin (address newAdmin) external {
        require(admins[msg.sender], "You are not an admin");
        admins[newAdmin] = true;
    }

    function setExpenseData (uint256[8] calldata publicKeyN, uint256[8] calldata publicKeyE, uint256 expenseId) external {
        require(admins[msg.sender], "You are not an admin");
        // check whether the expense already exists
        require(!expenseOccupied[expenseId], "Expense already exists");
        expenses[expenseId] = Expense(publicKeyN, publicKeyE);
        expenseOccupied[expenseId] = true;
    }

    function designateUser (address newUser, uint256 expenseId) external {
        require(admins[msg.sender], "You are not an admin");
        require(expenseOccupied[expenseId], "Expense does not exist");
        users[newUser][expenseId] = true;
    }

    event transactionUploaded(uint[4] messageHash, uint[2] proofAPartyA, uint[2][2] proofBPartyA, uint[2] proofCPartyA, uint[8] rsPartyA,
                                uint[2] proofAPartyB, uint[2][2] proofBPartyB, uint[2] proofCPartyB, uint[8] rsPartyB, uint256 expenseId);
    event amountIncrementUploaded(uint[2] proofARSA, uint[2][2] proofBRSA, uint[2] proofCRSA, uint[8] amountHash, uint256 expenseId, Expense expense);

    function uploadTransaction(uint[4] calldata messageHash, uint[2] calldata proofAPartyA, uint[2][2] calldata proofBPartyA, uint[2] calldata proofCPartyA, uint[8] calldata rsPartyA, 
    uint[2] calldata proofAPartyB, uint[2][2] calldata proofBPartyB, uint[2] calldata proofCPartyB, uint[8] calldata rsPartyB, uint256 expenseId) external {

        require(users[msg.sender][expenseId], "You are not a user of this expense");

        uint256[13] memory ecdsaPubInPartyA = [1, rsPartyA[0], rsPartyA[1], rsPartyA[2], rsPartyA[3], rsPartyA[4], rsPartyA[5], rsPartyA[6], rsPartyA[7], messageHash[0], messageHash[1], messageHash[2], messageHash[3]];
        uint256[13] memory ecdsaPubInPartyB = [1, rsPartyB[0], rsPartyB[1], rsPartyB[2], rsPartyB[3], rsPartyB[4], rsPartyB[5], rsPartyB[6], rsPartyB[7], messageHash[0], messageHash[1], messageHash[2], messageHash[3]];
        
        require(ecdsaVerifier.verifyProof(proofAPartyA, proofBPartyA, proofCPartyA, ecdsaPubInPartyA), "ECDSA proof for party A verification failed");
        require(ecdsaVerifier.verifyProof(proofAPartyB, proofBPartyB, proofCPartyB, ecdsaPubInPartyB), "ECDSA proof for party B verification failed");
        
        emit transactionUploaded(messageHash, proofAPartyA, proofBPartyA, proofCPartyA, rsPartyA, proofAPartyB, proofBPartyB, proofCPartyB, rsPartyB, expenseId);
        

    }
    function updateTotalAmount(uint[2] calldata proofARSA, uint[2][2] calldata proofBRSA, uint[2] calldata proofCRSA, uint[8] calldata amountHash, uint256 expenseId) external {
        require(users[msg.sender][expenseId], "You are not a user of this expense");

        uint256[24] memory rsaPubIn = [amountHash[0], amountHash[1], amountHash[2], amountHash[3], amountHash[4], amountHash[5], amountHash[6], amountHash[7],
            expenses[expenseId].publicKeyE[0], expenses[expenseId].publicKeyE[1], expenses[expenseId].publicKeyE[2], expenses[expenseId].publicKeyE[3], expenses[expenseId].publicKeyE[4], expenses[expenseId].publicKeyE[5], expenses[expenseId].publicKeyE[6], expenses[expenseId].publicKeyE[7],
            expenses[expenseId].publicKeyN[0], expenses[expenseId].publicKeyN[1], expenses[expenseId].publicKeyN[2], expenses[expenseId].publicKeyN[3], expenses[expenseId].publicKeyN[4], expenses[expenseId].publicKeyN[5], expenses[expenseId].publicKeyN[6], expenses[expenseId].publicKeyN[7]];
        require(rsaVerifier.verifyProof(proofARSA, proofBRSA, proofCRSA, rsaPubIn), "RSA proof verification failed");

        emit amountIncrementUploaded(proofARSA, proofBRSA, proofCRSA, amountHash, expenseId, expenses[expenseId]);
    }
