...
contract CLFactory {
    ...
    event evtHouseCreation( address houseAddr, string houseName, uint houseId );

	CLHouse[] public arrCLH;
    uint public numCLH = 0;
    mapping( address => uint ) public mapCLHId;     // TODO: remove
    address CLCMemberManagement;
    address CLCGovernance;

    constructor( address _CLCMemberManagement , address _CLCGovernance ) {
        CLCMemberManagement = _CLCMemberManagement;
        CLCGovernance = _CLCGovernance;
        address[] memory _ManagerWallets = new address[] ( 0 );
        CreateCLH( {
            _houseName: "TST DIC CLH",
            _housePrivate: false,
            _gov: __GOV_DICTATORSHIP__,
            _govRuleMaxManagerMembers: 1,
            _govRuleMaxActiveMembers: 99,
            _govRuleApprovPercentage: 50,
        ...
        returns( address )
    {
        ...
        require( 
            __GOV_DICTATORSHIP__ == _gov ||
            __GOV_COMMITTEE__ == _gov ||
            __GOV_SIMPLE_MAJORITY__ == _gov ,
...
contract CLHouse is CLStorage {
    ...
    )
    {
        ...
        arrProposals.push( strProposal( {
                            proponent: _owner,
                            typeProposal: proposalType.addMember,
                            description: "Founder",
                            propDataId: 0,
                            numVotes: 1,
                            againstVotes: 0,
        ...
        )
    {
        ...
        if( percent > govRuleApprovPercentage ) {
            ...
            emit evtProposal( eventType.rejectProposal, _propId, arrProposals[ _propId ].typeProposal, arrProposals[ _propId ].description );
        ...
        if( proposalType.addMember == typeProposal ) {
            (bool successDGTCLL, bytes memory dataDLGTCLL) = CLCMemberManagement.delegatecall(
                abi.encodeWithSignature( 
                    "InviteMember(uint16)", 
                    _propId
        ...
        } else if( proposalType.delMember == typeProposal ) {
            (bool successDGTCLL, bytes memory dataDLGTCLL) = CLCMemberManagement.delegatecall(
                abi.encodeWithSignature( 
                    "DelMember(address)", 
            ...
            if( !successDGTCLL )
                revert DebugDLGTCLL( successDGTCLL , dataDLGTCLL );
        } else if( proposalType.tranferEth == typeProposal ) {
            vaultCLH.TxWei(arrDataPropTxAssets[ propDataId ].to, arrDataPropTxAssets[ propDataId ].value );
        } else if( proposalType.tranferERC20 == typeProposal ) {
            vaultCLH.TxERC20(
        ...
        returns( uint )
    {
        ...
        arrProposals.push( 
            strProposal( { 
                ...
                typeProposal: proposalType.tranferEth,
        ...
        emit evtProposal( eventType.addProposal, propId, proposalType.tranferEth, _description );
        return propId;
    }
        ...
        returns( uint )
    {
        ...
        arrProposals.push( 
            strProposal( { 
                proponent: msg.sender,
                typeProposal: proposalType.tranferERC20,
                description: _description,
                propDataId: idDataNewPropTxWei,
                numVotes: 0,
                againstVotes: 0,
        ...
        emit evtProposal( eventType.addProposal, propId, proposalType.tranferERC20, _description );
        return propId;
    }
        ...
        returns( uint )
    {
        ...
        emit evtProposal( eventType.addProposal, propId, proposalType.requestJoin, _description );
...
contract CLStorage {
    ...
    event evtProposal( eventType typeEvent, uint propId, proposalType typeProposal, string description );
    ...
    event evtTxERC20( address walletAddr, uint value, address tokenAdd );
    event evtSwapERC20( address tokenIn, uint256 amountIn, address tokenOut, uint256 amountOut );
...
contract CLVault {
        ...
        external
    {
        ...
        emit evtTxERC20( _walletAddr, _value, _tokenAdd );
    }
        ...
        )
    {
        ...
        emit evtSwapERC20( _tokenIn, _amountIn, _tokenOut, amountOut );
        ...
        )
    {
        ...
        emit evtSwapERC20( address( 0 ), _amountIn, _tokenOut, amountOut );
        ...
        )
    {
        ...
        emit evtSwapERC20( _tokenIn, _amountIn, address( 0 ), amountOut );
