    ...
    Ownable
{
    ...
    function airdrop(address[] memory add)external onlyRole(MANAGER_ROLE){
        for(uint16 i = 0; i < add.length; i++){
            require(!isAirdrop[add[i]],"You already airdrop!");
        }
        for(uint16 i = 0; i < add.length; i++){
            isAirdrop[add[i]] = true;
            uint256 ownerAmount = fables2Address.balanceOf(add[i]);
            mintBatch(add[i], ownerAmount);
        }
    }
