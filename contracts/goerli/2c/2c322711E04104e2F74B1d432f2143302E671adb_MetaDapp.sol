...
contract MetaDapp {
  ...
  function transferTokens(address _owner, uint _price, address _buyer) private {
    require(_price <= token.balanceOf(_buyer), "No enough tokens");
    require(token.allowance(_buyer, address(this)) >= _price, "No enough allowance tokens");
    bool sent = token.transferFrom(_buyer, _owner, _price);
    require(sent, "Not Sent");
  }

  function updateProductPrice(uint _product_id, uint price) public {
    require(msg.sender != noOne, "Invalid address");
    Product storage product = products[_product_id];
    require(msg.sender == product.owner, "Not owner");
    require(product.reserved_by == noOne, "Product is reserved");
    product.price = __amount(price);
  }

  function updateUserContact(string memory _contact, string memory _name) public {
    require(msg.sender != noOne, "Invalid user");
  ...
  function buyProduct(uint _product_id) public {
    Product storage product = products[_product_id];
    require(msg.sender != product.owner, "You are the owner");
    transferTokens(product.owner, product.price, msg.sender);
    User storage buyer = users[msg.sender];
    buyer.total_products += 1;
    buyer.products.push(_product_id);
  ...
  function getProduct(uint _product_id) public view returns(Product memory) {
    return products[_product_id];
  }
  ...
  function withdrawETH(address payable _account) external isOwner {
