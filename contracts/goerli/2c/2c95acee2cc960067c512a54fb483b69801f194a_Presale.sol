...
contract Presale is OwnerWithdrawable {
    ...
    function buyToken(address _token, uint256 _amount) external payable{
        require(isPresaleStarted, "PreSale: Sale hasn't started yet!");
        require(block.timestamp < lockingPeriod, "PreSale: Locking period has ended, cannot buy");
    ...
    // function getUserDetails(address _user) external view returns(uint256, bool[4] memory){
    //     return(buyersAmount[_user].amount, buyersAmount[_user].lockingPeriodClaimed);
    // }
}