...
abstract contract NFT is ERC721A, DefaultOperatorFilterer {
    ...
    mapping(uint256 => bool) public nftLock;
     ...
     function setApprovalForAll(address operator, bool approved) public override onlyTransferAllowed(operator) onlyAllowedOperatorApproval(operator) {
    ...
    function transferFrom(address from, address to, uint256 tokenId) public payable override isNFTLock(tokenId) onlyTransferAllowed(msg.sender) onlyAllowedOperator(from) {
        super.transferFrom(from, to, tokenId);
    }

    function safeTransferFrom(address from, address to, uint256 tokenId) public payable override isNFTLock(tokenId)  onlyTransferAllowed(msg.sender) onlyAllowedOperator(from) {
        super.safeTransferFrom(from, to, tokenId);
    }

    function safeTransferFrom(address from, address to, uint256 tokenId, bytes memory data) public payable isNFTLock(tokenId)  onlyTransferAllowed(msg.sender) override onlyAllowedOperator(from) {
