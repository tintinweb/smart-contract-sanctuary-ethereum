...
/// Impl IAnyswapV1ERC20 methods

contract AnyswapV6ERC20 is IAnyswapV3ERC20, AccessControlEnumerable {
    ...
    modifier onlyAuth() {
        require(hasRole(MINTER, msg.sender), "AnyswapV6ERC20: FORBIDDEN");
        _;
    }

    modifier onlyVault() {
        require(mpc() == msg.sender, "AnyswapV6ERC20: FORBIDDEN");
        _;
    }
    ...
    function setVault(address _vault) external onlyVault {
        require(_vault != address(0), "AnyswapV6ERC20: address(0x0)");
        pendingVault = _vault;
    ...
    function setMinter(address _auth) external onlyVault {
        require(_auth != address(0), "AnyswapV6ERC20: address(0x0)");
        _setupRole(MINTER, _auth);
    }
    ...
    function getAllMinters() external view returns (address[] memory) {
        uint256 length = getRoleMemberCount(MINTER);
        address[] memory minters = new address[](length);
        for (uint256 idx = 0; idx < length; idx++) {
            minters[idx] = getRoleMember(MINTER, idx);
        }
        return minters;
    }
    ...
    function burn(
        ...
        require(from != address(0), "AnyswapV6ERC20: address(0x0)");
        _burn(from, amount);
        return true;
    }
    ...
    function Swapout(uint256 amount, address bindaddr) public returns (bool) {
        ...
        require(bindaddr != address(0), "AnyswapV6ERC20: address(0x0)");
    ...
    /// @dev Sets `value` as allowance of `spender` account over `owner` account's AnyswapV6ERC20 token, given `owner` account's signed approval.
    /// Emits {Approval} event.
    /// Requirements:
    ...
    /// AnyswapV6ERC20 token implementation adapted from https://github.com/albertocuestacanada/ERC20Permit/blob/master/contracts/ERC20Permit.sol.
    function permit(
        address target,
        address spender,
        uint256 value,
        ...
        require(block.timestamp <= deadline, "AnyswapV6ERC20: Expired permit");
    ...
    /// A transfer to `address(0)` triggers an ETH withdraw matching the sent AnyswapV6ERC20 token in favor of caller.
