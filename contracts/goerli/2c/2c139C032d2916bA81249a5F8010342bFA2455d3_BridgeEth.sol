...
if(tokens[_wrappedName]==address(0x0)) {       // check if the token exists. if not, it is created
    ...
    nativeToTargetTokenMapping[_tokenAddress] =_nativeToken; // mapping the native token to its sibling, deployed at the target  chain
}
else {
...
function unlock(address _nativeToken, uint256 _amount, address _receiver) external  onlyOwner {   //only the operator can unlock tokens. ui should include wein input in order to include the service fee. the input could be prefilled.
