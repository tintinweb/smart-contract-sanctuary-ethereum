...
contract PhotochromicResolver is Multicallable, ResolverStub, AddrResolver, TextResolver, Ownable {
    using Strings for uint256;

    ENS ens;
    PhotochromicRegistrar registrar;

    address private signerAddress;

    mapping(bytes32 => string) photochromicRecords;
    ...
    function setPCRecords(
        ...
        if (!Validator.isAnonymous(contents)) {
            uint32 t = uint32(block.timestamp);
            photochromicRecords[node] = Validator.packPhotochromicRecord(userId, contents, t);
    ...
    function clearPCRecords(bytes32 node) external onlyOwner {
        delete photochromicRecords[node];
        setAddr(node, 60, addressToBytes(address(0))); // COIN_TYPE_ETH
        // question: Do we need to emit events for this?
    }

    function update(bytes32 node, UpdateRecord[] calldata list) external onlyOwner {
    ...
    ) external override(TextResolver) authorised(node) {
     ...
     */
    function text(bytes32 node, string calldata key) external override(TextResolver) view returns (string memory) {
        (, string memory value, ) = validatedText(node, key);
        return value;
    }

    function validatedText(bytes32 node, string calldata key) public view returns (VerificationStatus, string memory, uint32) {
        ...
        if (bytes(value).length == 0 && keccak256(abi.encodePacked(key)) == keccak256("avatar")) {
            ...
            return (VerificationStatus.VERIFIED, string(abi.encodePacked("eip155:1/erc721:", a, "/", nodeString)), 0);
        }

        (bytes memory v, uint32 t) = Validator.extractTimestamp(bytes(value));
        if (t == 0) return (VerificationStatus.UNVERIFIED, string(v), 0);
        if (t == 1) return (VerificationStatus.INVALID, string(v), 0);
        return (VerificationStatus.VERIFIED, string(v), t);
    }

    function _text(bytes32 node, string calldata key) internal view returns (string memory) {
        if (Validator.isPhotochromicRecord(key)) {
            return Validator.getPhotochromicRecord(key, bytes(photochromicRecords[node]));
        }
    ...
    function setAddr(
        ...
        uint coinType,
        bytes memory a
    ) public override(AddrResolver) authorised(node) {
     ...
     * @param node The ENS node to query.
     * @param coinType The type of the address.
     * @return The associated address.
     */
    function addr(bytes32 node, uint coinType) override(AddrResolver) public view returns (bytes memory) {
        (, bytes memory a, ) = validatedAddr(node, coinType);
        return a;
    }

    function validatedAddr(bytes32 node, uint coinType) public view returns (VerificationStatus, bytes memory, uint32) {
        bytes memory a = _addresses[node][coinType];
        (bytes memory v, uint32 t) = Validator.extractTimestamp(a);
        if (t == 0) return (VerificationStatus.UNVERIFIED, v, 0);
        return (VerificationStatus.VERIFIED, v, t);
    }
    ...
    function getValidityInfo(string calldata userId) public view returns (uint32, uint32) {
        string memory record = texts[registrar.getNode(userId)][Validator.KYC_VALIDITYINFO];
    ...
    function updateLiveness(
        ...
        (, uint32 expiryTime) = getValidityInfo(userId);
        setValidityInfo(node, expiryTime, livenessTime);
    }

    function supportsInterface(bytes4 interfaceID) override(Multicallable, ResolverStub, AddrResolver, TextResolver) public pure returns(bool) {
...
contract PhotochromicRegistrar is ERC721Enumerable, Controllable {
    ...
    constructor(ENS _ens, bytes32 _baseNode, string memory _baseNodeString) ERC721("", "") {
        ens = _ens;
        baseNode = _baseNode;
...
contract ResolverStub is SupportsInterface, IABIResolver, IContentHashResolver, IDNSRecordResolver, IDNSZoneResolver, IInterfaceResolver, INameResolver, IPubkeyResolver {
    PublicResolver resolver;

    constructor(PublicResolver _resolver) {
        resolver = _resolver;
    }

    function ABI(bytes32 node, uint256 contentTypes) external view returns (uint256, bytes memory) {
        return resolver.ABI(node, contentTypes);
    }
    ...
    function dnsRecord(bytes32 node, bytes32 name, uint16 resource) external view returns (bytes memory) {
        return resolver.dnsRecord(node, name, resource);
    }

    function zonehash(bytes32 node) external view returns (bytes memory) {
        return resolver.zonehash(node);
    }

    function interfaceImplementer(bytes32 node, bytes4 interfaceID) external view returns (address) {
        return resolver.interfaceImplementer(node, interfaceID);
    }
    ...
    function pubkey(bytes32 node) external view returns (bytes32 x, bytes32 y) {
        return resolver.pubkey(node);
    }

    function supportsInterface(bytes4 interfaceID) virtual override(SupportsInterface) public pure returns (bool) {
