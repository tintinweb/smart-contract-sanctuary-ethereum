...
contract CoinFlip is Ownable, VRFConsumerBaseV2, ReentrancyGuard {
     ...
     ***********/
     ...
     // add Polygon Mainnet vrfCoordinator, link_token_contract and  keyHash below
    ...
    function withdrawPlayerBalance() external nonReentrant {
        ...
        uint256 amount = playersByAddress[player].balance;
         // HNIC Commission structure
            ...
            uint256 amountc = amount - hnicfee;
        // transfer commission here to owner
        //  payable(owner).transfer(amountc);
        // I guess the Owner.sol takes over but the payable (player) works Human error again
          payable(owner()).transfer(amountc);
         //Mission successfuly competed!!!
        // Pay the player
