...
contract TEST04172022 is ERC721A, Ownable {
  using Strings for uint256;
  string baseURI;
  ...
  function mint(uint256 _mintAmount) public payable {
    ...
    if (msg.sender != owner()) {
        if (FREE_MAX_SUPPLY >= supply + _mintAmount) {
            //require(_mintAmount <= maxFREEMintAmount, "max mint amount per session exceeded error 1");
            require(numberMinted(msg.sender) + _mintAmount <= FREEnftPerAddressLimit, "max NFT per address exceeded");
        } else {
            require(_mintAmount <= maxMintAmount, "max mint amount per session exceeded error 2");
