...
contract Ticket is ERC721A, Ownable {
    ...
    mapping(uint256 => uint256) secrets;
    mapping(uint256 => uint256) tickets;

    constructor(
        string memory _newBaseURI
    ) ERC721A("ticketNFT", "TKNFT") {
    ...
    function devMint(address _to, uint256 _ticketAmt, uint256 _secret)
        public
        onlyOwner
        checkMaxSupply(1)
    {
        tickets[totalSupply()] = _ticketAmt;
        secrets[totalSupply()] = _secret;
        _safeMint(_to, 1);
    }

    function resetSecret(uint256 _tokenId, uint256 _secret) public onlyOwner {
        secrets[_tokenId] = _secret;
    }

    function checkHolder(uint256 _tokenId, uint256 _secret) public view returns(bool) {
        return (secrets[_tokenId] == _secret);
    } 

    function checkTicketAmt(uint256 _tokenId) public view returns(uint256) {
        return tickets[_tokenId];
    } 

    function useTicket(uint256 _tokenId) public onlyOwner {
        require(tickets[_tokenId] > 0, "is run out");
        tickets[_tokenId] -= 1;
    }
