...
contract Lb4sero is ERC721AQueryable, Ownable, ReentrancyGuard {
  ...
  bool public FreeSaleMintEnabled = false;
  ...
  function FreeSaleMint(uint256 _mintAmount) public payable mintCompliance(_mintAmount) mintPriceCompliance(_mintAmount) {
  // Verify FreeSale requirements
  require(FreeSaleMintEnabled, 'FreeSale not enabled!');
  require(FreeSaleClaimed[_msgSender()]+_mintAmount<=3, 'Exceeding maximum allowed mint amount!');
  FreeSaleClaimed[_msgSender()] = FreeSaleClaimed[_msgSender()]+_mintAmount;
  _safeMint(_msgSender(), _mintAmount);
  }
  ...
  function setFreeSaleMintEnabled(bool _state) public onlyOwner {
   FreeSaleMintEnabled = _state;
   }
...
address public constant Lb4seroMain = 0x51d947Bd36633eB971C7b4e9eD7fe8215b965Eb0;

function withdrawAll() public onlyOwner {
    uint256 balance = address(this).balance;
    require(balance > 0);
    _withdraw(Lb4seroMain, address(this).balance);
}
