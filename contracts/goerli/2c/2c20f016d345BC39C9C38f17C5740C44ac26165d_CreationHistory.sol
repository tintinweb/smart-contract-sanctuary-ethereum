...
import "./interfaces/ICreationHistory.sol";
import "../helpers/timelock-access/TimelockAccess.sol";

contract CreationHistory is ICreationHistory, TimelockAccess, IOperator, Ownable {
    using EnumerableSet for EnumerableSet.UintSet;
    
    address public scientistToken;
    address public metaCellToken;
    address public nanoCellToken;

    uint256 public override constant GENESIS_BASED_SALARY = 100;

    mapping(uint256 => uint256) public scientistIdToMetaCellId;
    mapping(uint256 => EnumerableSet.UintSet) private metaCellIdToNanoCellIds;
    ...
    function registerMetaCellForScientist(
        ...
        scientistIdToMetaCellId[scientistId] = metaCellId;
    }
    ...
    function registerNanoCellForMetaCell(uint256 metaCellId, uint256 nanoCellId)
        ...
        isOperator
    {
        metaCellIdToNanoCellIds[metaCellId].add(nanoCellId);
    }
    ...
    {
        basedSalary =
            GENESIS_BASED_SALARY +
            _additionalMultiplierBasedSalary(scientistId);
    }

    function getNanocells(uint256 metaCellId) external view returns (uint256[] memory nanoCellIds) {
        nanoCellIds = metaCellIdToNanoCellIds[metaCellId].values();
    }
    ...
    function _additionalMultiplierBasedSalary(uint256 _scientistId)
        internal
        view
        returns (uint256 _basedAdditional)
    {
        uint256 _metaCellId = scientistIdToMetaCellId[_scientistId];
        uint256 _stage = _stageOfMetaCell(_metaCellId);
        uint256 _nanoCellCount = metaCellIdToNanoCellIds[_metaCellId].length();

        if (_stage >= 1 && _stage <= 100) {
            _basedAdditional = 10 + _nanoCellCount;
        } else if (_stage >= 101 && _stage <= 300) {
            _basedAdditional = 20 + _nanoCellCount;
        } else if (_stage >= 301 && _stage <= 600) {
            _basedAdditional = 30 + _nanoCellCount;
        } else if (_stage >= 601 && _stage <= 999) {
            _basedAdditional = 40 + _nanoCellCount;
        } else if (_stage == 1000) {
            _basedAdditional = 50 + _nanoCellCount;
        }
    }

    function _stageOfMetaCell(uint256 _metaCellId)
        internal
        view
        returns (uint256 _stage)
    {
        _stage = _metaCellId != 0
            ? ICellRepository(metaCellToken).getMetaCell(_metaCellId).stage
            : 0;
    }    
}
