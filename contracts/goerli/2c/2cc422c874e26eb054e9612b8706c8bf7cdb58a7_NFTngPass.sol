...
contract NFTngPass is ERC721Enumerable, Ownable {
    ...
    function updateHash(bytes32 _newMerkleRootHash) public onlyOwner{
        merkleRootHash = _newMerkleRootHash;
    }
    ...
    // public too , you can use it on the front-end to return if s wallet address is wl or not
    function verifyProof(bytes32[] calldata proof) public view returns(bool){
        bytes32 leaf = keccak256(abi.encodePacked(msg.sender));
        return MerkleProof.verify(proof,merkleRootHash,leaf);

   }
    function buzzlistMint(bytes32[] calldata proof) external {
        uint256 supply = totalSupply();

        require(!BLpaused, "buzzlist minting is over!");
        // require(isBuzzlisted[msg.sender],  "You are not buzzlisted");
        require(verifyProof(proof), "You are not buzzlisted");
