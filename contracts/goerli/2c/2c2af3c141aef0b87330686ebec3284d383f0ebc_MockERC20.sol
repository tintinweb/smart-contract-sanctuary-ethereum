...
contract LUSD3CRVValidator is IProofValidator, IValidator {
    ...
    function validateCurve(
        ...
        if (mode == 1){
            stableSwapPoolHelper.verifyAccount(CURVE3POOL_ADDRESS_HASH,blockHeader.stateRootHash,proofs[0].toList(),proofs[3].toList());
        }
...
contract StableSwap3PoolHelper {
    ...
    function verifyAccount(
        ...
        RLPReader.RLPItem[] memory accountProof,
        RLPReader.RLPItem[] memory proofs
    )public view returns (PoolStorage memory poolStorage) {
        StateProofVerifier.Account memory curve3PoolAccount = StateProofVerifier
            .extractAccountFromProof(
                poolAddressHash,
                headerRoot,
