...
abstract contract WhitelistMerkle is Ownable {
    ...
    function isWhitelisted(address _account, bytes32[] calldata _merkleProof)
        external
        view
        returns (bool)
    {
...
error MaxAmountReach();
error PriceNotPaid();
error NotWhitelisted();

contract GreenEarthNFT is StartTokenIdHelper, ERC721A, WhitelistMerkle {
    using Strings for uint256;
    using ECDSA for bytes32;

    uint256 public constant START_ID = 2022;
    uint256 public constant MAX_MINT_PER_ADDRESS = 5;

    uint256 public publicPrice = 0.19 ether;
    ...
    constructor(
        ...
        StartTokenIdHelper(START_ID)
        ERC721A("GreenEarthNFT", "EARTH")
    ...
    function whitelistMint(bytes32[] calldata _merkleProof, uint256 _amount)
        external
        payable
        onlyWhitelisted(_merkleProof)
        mintCompliance(_amount)
        whenNotPaused
    {
        if (msg.value < whitelistPrice * _amount) {
            revert PriceNotPaid();
        }
        _mint(msg.sender, _amount);
    }
    ...
    function setPause(bool _state) external payable onlyOwner {
        paused = _state;
    }
    ...
    modifier mintCompliance(uint256 _amount) {
        ...
        if (numberMinted(msg.sender) + _amount > MAX_MINT_PER_ADDRESS) {
            revert MaxAmountReach();
        }
