...
import {InsecureumToken} from "../tokens/Insecureum.sol";
import {InSecureumLenderPool} from "./Challenge1.sol";

contract FactoryChallenge1 is IChallengeFactory{
  
  function deploy() external returns (address[] memory ret) {
    /// @dev Deploy InsecureumToken with 10 ether in this constract address 
    address _token = address(new InsecureumToken(10 ether));

    address _challenge = address(new InSecureumLenderPool(_token));
    // InsecureumToken(_token).transfer(_challenge, 10 ether);
    InsecureumToken(_token).approve(_challenge, 10 ether);
    InSecureumLenderPool(_challenge).deposit(10 ether);
  ...
  function isComplete(address[] calldata _challenges) external view returns(bool) {
    require(_challenges.length == 2);
    // _challenges[0] is the challenge contract
    // _challenges[1] is the token contract
    return InsecureumToken(_challenges[1]).balanceOf(_challenges[0]) == 0;
  }
}
...
interface IChallengeFactory {
    function deploy() external returns (address[] memory);
