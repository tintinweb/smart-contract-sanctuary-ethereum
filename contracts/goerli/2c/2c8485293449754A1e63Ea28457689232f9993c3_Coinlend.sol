...
error Coinlend__InsufficientTokenPayback();
...
contract Coinlend {
    ...
    struct Loan {
        ...
        LoanState loanState;
        address borrower;
        uint256 timestamp; // When loan was created or taken
    }
    ...
    uint256 public loanExpirationLiquidationBufferSec = 60 * 60 * 24 * 14; // 14 days
    ...
    function payBackLoan(uint256 _id) public {
        ...
        // Ensure the caller of the function is the owner of the loan
        require(address(_loan.borrower) == msg.sender, "Address not matching the borrower");
        ...
        uint256 fee = calculateFee(interestAmount);

        // Check if borrower has enough funds for payback
        if (
            tokens[_loan.tokenLending][_loan.borrower] <
            _loan.tokenLendingAmount + interestAmount + fee
        ) {
            revert Coinlend__InsufficientTokenPayback();
        }
        ...
        uint256 interestAmountMinusFee = interestAmount - fee;
    ...
    function calculateInterest(
        uint256 tokenLendingAmount,
        uint256 interestRate,
        uint256 startTime,
        uint256 endTime
    ) internal pure returns (uint256) {
        require(endTime > startTime, "End time needs to be bigger than the start time");
        uint256 interestAmount = (((tokenLendingAmount * interestRate) * (endTime - startTime)) /
            365 /
            24 /
            60 /
            60 /
    ...
    function calculateFee(uint256 interestAmount) internal view returns (uint256) {
        uint256 fee = (interestAmount * feePercent) / 10000;
        return fee;
    }

    function forceLiquidateExpiredLoan(uint256 _id) public {
        // Fetch loan
        Loan storage _loan = loans[_id];
        ...
        require(address(_loan.lender) == msg.sender, "Addresses not matching the lender");

        // Check loan state
        require(_loan.loanState == LoanState.ACTIVE, "Only active loans can be liquidated");

        // check loan is expired + buffer
        require(
            _loan.timestamp + loanExpirationLiquidationBufferSec > block.timestamp,
    ...
    function forceLiquidateMarginExceeded(uint256 _id) public {}

    function increaseCollateral() public {}
