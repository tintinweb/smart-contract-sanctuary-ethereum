...
contract Stand is ERC721V, Ownable, IERC2981 {
    ...
    // Used to help calculate royalties
     ...
     * 
     * This means in {_beforeTokenTransfers} we will multiply by 3600 before checking with
    ...
    /**
     * TODO do we make this a constant?
     * Public sale is NOT enabled while this value is 0. 
     */
    ...
    uint256 public reservedAuctionETH;
    ...
    constructor(
        uint256 firstUnlockTime_, 
        uint256 secondUnlockTime_, 
        uint256 thirdUnlockTime_, 
        address initialPresale
    ) ERC721V("Name", "Symbol") {
        _royaltyWallet = msg.sender;
        firstUnlockTime = uint24(firstUnlockTime_);
        secondUnlockTime = uint24(secondUnlockTime_);
        thirdUnlockTime = uint24(thirdUnlockTime_);
        signer = msg.sender;
     ...
     */
    function supportsInterface(bytes4 interfaceId) public view virtual override(ERC721V, IERC165) returns (bool) {
    ...
    {
        uint256 royalty = (_salePrice * _royaltyBPS) / MAX_BPS;
    ...
    // =============================================================
    /**
     * Allows the owner to set a new address for EIP-2981 royalty standard. 
     */
    function setRoyaltyAddress(address newRoyalty) external onlyOwner {
        _royaltyWallet = newRoyalty;
    }

    /**
     * Allows the owner to set a new royalty percentage for EIP-2981 royalty standard. 
     */
    function setRoyaltyBps(uint256 bps) external onlyOwner {
        // TODO set a lower max?
        if (bps > MAX_BPS) revert InvalidBps();
        _royaltyBPS = bps;
    }

    /**
     * Allows the owner to set a new registrant contract. 
     */
    function setOperatorFilterRegistryAddress(address registryAddress) external onlyOwner {
        operatorFilterRegistryAddress = registryAddress;
    }

    /**
     * Allows the owner to set a new registrant address. 
     */
    ...
    /**
     * Allows the owner to set the new token URI. 
     */
    ...
    /**
     * Allows the owner to set the public mint price. If this is 0,
     * it is assumed that the public mint is not active. 
     */
    ...
    /**
     * Allows the owner to change the active version of their signatures, this also
     * allos a simple invalidation of all signatures they have created on old versions.
     */
    ...
    /**
     * Allows the owner to withdraw a certain amount of ETH to a certain address.
     */
    function withdraw(address withdrawAddress, uint256 amount) external onlyOwner {
        if (amount > address(this).balance - reservedAuctionETH) {
            amount = address(this).balance - reservedAuctionETH;
        }
     ...
     * Allows the owner to withdraw a certain amount of ETH to a certain address.
     */
    function mintDev(address[] memory mintAddresses, uint256[] memory mintQuantities) external onlyOwner {
    ...
    // Presale contract already gaurantees a cap on the # of presale tokens, so
    ...
    function mintPresale(address[] memory presaleUsers) external onlyOwner {
        ...
        unchecked {
            for (uint i = 0; i < presaleUsers.length; i++) {
                address presaleUser = presaleUsers[i];
                if (presaleMinted[presaleUser]) revert PresaleAddressAlreadyMinted();
                presaleMinted[presaleUser] = true;

                uint256 tokensOwed = Presale(PRESALE_ADDRESS).selectedBids(presaleUser);
                if (presaleDelegation[presaleUser] != address(0)) {
                    _mint(presaleDelegation[presaleUser], tokensOwed);
                } else {
                    _mint(presaleUser, tokensOwed);
                }

                // Cheaper gas wise to do every 3 tokens and deal with the remainder afterwards
                // than to do if statements within the loop.
                for (uint j = 0; j < tokensOwed / 3; j++) {
                ...
                if (tokensOwed % 3 == 2) {
                    _setExtraDataAt(nextId - 2, thirdUnlockTime);
                    emit TokenLocked(nextId - 2, thirdUnlockTime);

                    _setExtraDataAt(nextId - 1, secondUnlockTime);
                    emit TokenLocked(nextId - 1, secondUnlockTime);
                } else if (tokensOwed % 3 == 1) {
    ...
    /**
     * @notice Sets if a certain tier is active or not
     * @dev Only callable by the owner.
     */
    function setTierActive(string memory tier, bool active)
        external
        onlyOwner
    {
     ...
     * @dev Only callable by the owner.
     */
    function setAuctionStartPrice(uint256 _startPrice) external onlyOwner {
        auctionStartPrice = _startPrice;
    }
    ...
    /**
     * Auctions can only start after all minting has terminated. We cannot auction more than
    ...
    function startAuction(
        ...
        if (totalSupply() + numTokens > MINT_CAP + AUCTION_SUPPLY) revert OverTokenLimit();
        if (numTokens > MAX_NUM_BIDS) revert OverMaxBids();
        if (auctionStartTime != 0) revert AuctionStillLive();

        auctionTokens = numTokens;
        auctionStartTime = startTime;
        auctionEndTime = endTime;
    }

    /**
     * Auctions can end at any time by the owner's discretion and when it ends all
    ...
    function endAuction() external onlyOwner {
        if (auctionStartTime == 0) revert AuctionNotLive();
        auctionStartTime = 0;

        uint256 lowestPrice = activeBids[getBidIndexToUpdate()].amount;
        for(uint256 i = 0; i < auctionTokens; i++) {
            if (activeBids[i].bidder == address(0)) {
                break;
            }

            _mint(activeBids[i].bidder, 1);
            _transferETH(activeBids[i].bidder, activeBids[i].amount - lowestPrice);
        }

        delete activeBids;
        auctionNumber += 1;
        reservedAuctionETH = 0;
    }

    /**
     * This method does NOT need to be called for locked tokens to be unlocked. It is 
     * here to emit unlock events for marketplaces to know when tokens are eligible for 
     * trade. The burden to call this method on the right tokens at the correct timestamp 
     * is on the owner of the contract.
     * 
     * @dev Only callable by the owner.
     */
    ...
    // If a selected presale address wishes their tokens be minted to a different address,
    // they can call this method to switch it to another address. This does not do
    // anything for addresses that were not selected on the presale contract.
    function setPresaleMintAddress(address addr) external {
        presaleDelegation[msg.sender] = addr;
    }

    /**
     * After a tier is activated by the owner, users with the proper signature for that tier
     * are able to mint based on what the owner has approved for their wallet.
     */
    function mintTier(string memory tier, address addr, uint256 price, uint256 version, uint256 allowedAmount, uint256 buyAmount, bytes memory sig) external payable {
        if (totalSupply() + buyAmount > MINT_CAP) revert OverMintLimit();
        if (!isActive[tier]) revert TierNotActive();
        if (version != signatureVersion) revert InvalidSignatureVersion();
        if (addr != msg.sender) revert InvalidSignatureAddress();

        uint totalPrice = price * buyAmount;
        if (buyAmount > allowedAmount) revert InvalidSignatureBuyAmount();
        if (msg.value < totalPrice) revert InsufficientPayment();
        if (signatureUsed[sig]) revert SignatureAlreadyUsed();
        signatureUsed[sig] = true;
        
        bytes32 hash = ECDSA.toEthSignedMessageHash(keccak256(
            abi.encode(
                tier,
                address(this),
                price,
        ...
        if (hash.recover(sig) != signer) revert InvalidSignatureParams();

        _mint(addr, buyAmount);
        _transferETH(msg.sender, msg.value - totalPrice);
    }

    /**
     * This mint is fully public, it has a restriction on minting just one token per call.
     * However, there is no restrictions on wallet address mint limits because
     * any public mint method that does not require a signature already has no gaurantee
     * on USER minting restrictions. A user could create 100 addresses to do 100 mints
     * rather than call this mint function 100 times.
     * 
     * The one prevention we do have is that no contracts can call this method because
     * those would be able to mint many tokens in a single transaction.
     */ 
    function mintPublic() external payable {
        if (tx.origin != msg.sender) revert NotEOA();
        if (totalSupply() >= MINT_CAP) revert OverMintLimit();
        if (publicMintPrice == 0) revert PublicMintNotLive();
        if (msg.value < publicMintPrice) revert InsufficientPayment();
        _mint(msg.sender, 1);
        _transferETH(msg.sender, msg.value - publicMintPrice);
    }
    ...
    function bid() external payable {
        ...
        BidIndex memory existingIndex = bidIndexes[auctionNumber][msg.sender];
        if (existingIndex.isSet) {
            ...
            if (msg.value < auctionStartPrice || msg.value == 0) {
            ...
            if (lowestBidder != address(0)) {
                delete bidIndexes[auctionNumber][lowestBidder];
            ...
            bidIndexes[auctionNumber][msg.sender] = BidIndex({
        ...
        if (auctionEndTime - block.timestamp < timeBuffer) {
            unchecked {
                auctionEndTime = block.timestamp + timeBuffer;
            }
    ...
    // =============================================================
    /**
     * @dev Stops operators that not approved for transfers to also not be allowed
     * as an approved address to transfer.
     * 
     * `operator` - the address a wallet is trying to grant approval to.
     */
    function _beforeApproval(
        address operator
    ) internal virtual override {
        if (operatorFilterRegistryAddress.code.length > 0) {
            if (!IOperatorFilterRegistry(operatorFilterRegistryAddress).isOperatorAllowed(filterRegistrant, operator)) {
    ...
    /**
     * @dev Stops operators that not approved for transfers.
     * 
    ...
    function _beforeTokenTransfers(
        ...
        if (operatorFilterRegistryAddress.code.length > 0) {
            if (!IOperatorFilterRegistry(operatorFilterRegistryAddress).isOperatorAllowed(filterRegistrant, msg.sender)) {
                revert OperatorNotAllowed();
            }
        }
        if (_getExtraDataAt(tokenId) * 3600 > block.timestamp) revert TokenTransferLocked();
        super._beforeTokenTransfers(from, to, tokenId, quantity);
    }

    /**
     * @notice Transfers ETH to a specified address. Adjusted 
    ...
    error InvalidBps();
    error InvalidSignatureAddress();
    error InvalidSignatureBuyAmount();
    error InvalidSignatureParams();
...
/**
 * @title ERC721V is a slight improvment upon ERC721A for a few select purposes.
 * 
 ...
 * 
 * Ownership's extraData field was modified to be writable without ownership initialized. To allow for multiple token
...
contract ERC721V is IERC721A {
    ...
    function _unpackedOwnership(uint256 packed) private pure returns (TokenOwnership memory ownership) {
        ...
        // Burning disabled
        ownership.burned = false; // packed & _BITMASK_BURNED != 0;
