...
/// @dev The LendingPoolRegistry keeps track of deployed LendingPools.
contract LendingPoolRegistry is Ownable {
    event LendingPoolRegistered(string indexed lendingPoolId, address indexed lendingPool, bool existingPoolReplaced);

    // LendingPool.id => LendingPool
    mapping(string => address) private lendingPools;

    // Ids of all registered LendingPools
    string[] public lendingPoolIds;

    /// @dev Registers a LendingPool by getting its id and putting it into the lendingPools mapping
    /// @param lendingPool address of the LendingPool to be registered
    function registerLendingPool(address lendingPool) external onlyOwner {
        string memory lendingPoolId = LendingPool(lendingPool).id();
        emit LendingPoolRegistered(lendingPoolId, lendingPool, lendingPools[lendingPoolId] != address(0));
        lendingPools[lendingPoolId] = lendingPool;
        lendingPoolIds.push(lendingPoolId);
    }

    /// @dev Returns a LendingPool for a id or reverts if none is found
    /// @param lendingPoolId id
    /// @return the LendingPool associated with the id
    function getLendingPool(string calldata lendingPoolId) external view returns (address) {
        require(lendingPools[lendingPoolId] != address(0), "lendingPool not found");
        return lendingPools[lendingPoolId];
    }

    /// @dev Returns the ids of all registered LendingPools
    /// @return ids of all registered LendingPools
