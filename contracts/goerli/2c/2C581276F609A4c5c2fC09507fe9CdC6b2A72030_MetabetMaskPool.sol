...
contract MetabetMaskPool is 
    ...
    */
    ...
    /**
     * @dev retrieve token  with name
     */
    mapping(string => address) public tokenNameToAddress;

    /**
     * @dev check if token is added
     */
    ...
    */
     ...
     */
    event TokenAdded(string tokenName, address indexed token);


    /**
     * @dev Emitted when a token is removed
     */
    ...
    function initialize(
        ...
        METABET = IERC20Upgradeable(address(0));
        ...
        tokenFees[address(BUSD)] = tokenFees[address(0)] = 1;*/
    }
    ...
    /**
     * @notice get the array of tokens
     * @return tokens the array of token
     */
    function getTokens(uint cursor, uint length)
    ...
    /**
    @notice Method for adding token
    @dev Only admin
    @param _tokenName ERC20 token name
    @param _token ERC20 token address
    */
    function addToken(string memory _tokenName,address _token) external onlyOwner {
        require(!tokenAdded[_token], "Token already added");
        tokenAdded[_token] = true;
        tokenNameToAddress[_tokenName] = _token;
        tokens.push(_tokenName);

        tokenFees[_token] = 1;

        emit TokenAdded(_tokenName, _token);
    }

    /**
    @notice Method for removing token
    @dev Only admin
    @param _tokenName ERC20 token address
    */
    function removeToken(string memory _tokenName) external onlyOwner {
        address _token = tokenNameToAddress[_tokenName];
        require(tokenAdded[_token], "Token does not exist");
        tokenAdded[_token] = false;
        for (uint i = 0; i < tokens.length; i = i + 1) {
            if(keccak256(abi.encodePacked(tokens[i])) == keccak256(abi.encodePacked(_tokenName))){
    ...
    */
        ...
        public nonReentrant
    {
        ...
        address token = tokenNameToAddress[_token];
        //%fee
        uint fee = (_amount.mul(tokenFees[token])).div(100);
        ...
        eventToPoolTotal[_eventId][token] += _amount;

        eventToPoolTotalTeam[_eventId][token][_predictResult] += _amount;

        eventToPoolSize[_eventId][token] += 1;
        Bet memory _bet = Bet(
                msg.sender,
                token,
                _eventId,
        ...
        if(token != tokenNameToAddress["GOAL"] 
        || token != tokenNameToAddress["METABET"]){
            IPool(sportEvent.pool).withdrawToken(
                token, 
                address(treasury), 
                fee
            );
        ...
        public payable nonReentrant
    {
        ...
        payable(sportEvent.pool).transfer(deposit);
        ...
        external nonReentrant
    {
        ...
        if (_token == address(0)) {
            IPool(events[i].pool).withdraw(reward);
            (bool success, ) = msg.sender.call{value: reward}("");
            require(success, "claim of bnb was unsuccessful");
        } else{
            IPool(events[i].pool).withdrawToken(
                _token, 
                msg.sender, 
                reward
            );
    ...
    function getLeaderBoard(bytes32 _eventId) external view returns(Bet[] memory){

        
        // Collect up all the boards 
        Bet[] memory output;

        Bet[] memory bets = getBets(_eventId);
        bytes32[] memory ids = new bytes32[](1);

        uint index = 0;

        for(uint n = 0; n < bets.length; n++){
            ids[0] = bets[n].eventId;
            if(userPredictStatus(bets[n].user, ids)[0]){
                output[index] = bets[n];
                index = index + 1;
            }
        }
