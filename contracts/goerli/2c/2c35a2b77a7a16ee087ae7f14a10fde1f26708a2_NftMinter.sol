...
import {Admin} from './Utils/Admin.sol';
import {Pausable} from './Utils/Pausable.sol';

contract NftMinter is ERC721Enumerable, Admin, ReentrancyGuard, Pausable {
    ...
    constructor(
        ...
        bytes32 _root
    ) ERC721(_name, _symbol) Admin() Pausable() {
     ...
     */
    function _pause() public virtual override whenNotPaused onlyAdmin {
        _paused = true;
     ...
     */
    function _unpause() public virtual override whenPaused onlyAdmin {
        _paused = false;
    ...
    function airdropMint() external onlyAdmin whenNotPaused {
    ...
    */
    ...
    function reserveMint(uint _amount) external whenNotPaused {
    ...
    function setReserveList(address[] memory addresses, uint256[] memory numSlots) external onlyAdmin {
        (
    ...
    function setRootHash(bytes32 _newRoot) external onlyAdmin {
        root = _newRoot;
    }

    function setAirdropList(address[] memory addresses, uint256[] memory numSlots) external onlyAdmin {
        (
            addresses.length == numSlots.length,
            "addresses does not match numSlots length"
        );
    ...
    function setWhitelistMintPeriod(uint _timePeriod) public onlyAdmin {
...
abstract contract Admin {
    ...
    event NewAdminAdded(address _addr);
