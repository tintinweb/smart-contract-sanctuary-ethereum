...
contract MockVaultFactory {
    ...
    // This storage should be redundant if we deterministically deploy vaults
    mapping(address => mapping(string => address)) public directory;

    constructor(address _feeTo) {
        feeTo = _feeTo;
    }

    function createVault(address _owner, string memory _projectId)
        public
        returns (address addr)
    {
        ...
        directory[_owner][_projectId] = addr;
    }

    // Public for view + implementation purposes
    function computeDeterministicAddr(address _owner, string memory _projectId)
        public
        view
        returns (address)
    {
    ...
    function getCreationCode(address _owner, string memory _projectId)
        public
        view
        returns (bytes memory)
    {
        return
            abi.encodePacked(
                ...
                abi.encode(_owner, _projectId, feeTo)
            );
    }
}
