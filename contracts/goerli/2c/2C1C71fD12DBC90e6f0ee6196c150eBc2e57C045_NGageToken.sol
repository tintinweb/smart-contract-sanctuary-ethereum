 ...
 */
contract NGageToken is ERC3525SlotEnumerableUpgradeable {
    using Strings for uint256;

    uint256 public nextSlotId;

    mapping(uint256 => LoyaltyProgram) public lps;
    ...
    function createLoyaltyProgram(
        ...
        ERC3525SlotEnumerableUpgradeable._createSlot(slotId);
    }

    function mint(uint256 slotId_, uint256 amount_) public {
        require(_slotExists(slotId_), "Slot does not exist");
        require(
            lps[slotId_].circulatingSupply + amount_ <= lps[slotId_].maxSupply,
            "Exceeds max supply"
        );

        lps[slotId_].circulatingSupply += amount_;

        _mint(_msgSender(), slotId_, amount_);
    }

    function getSlotDetail(
        uint256 slotId_
    ) public view returns (LoyaltyProgram memory) {
        return lps[slotId_];
    }
            ...
            string(
                abi.encodePacked(
                    ...
                    abi.encodePacked(
                        ...
                        lps[slotId_].name,
                        '","description":"',
                        lps[slotId_].description,
                        '","image":"',
                        lps[slotId_].defaultImage,
                        '","maxSupply":"',
                        lps[slotId_].maxSupply,
                        '","circulatingSupply":"',
                        lps[slotId_].circulatingSupply,
                        "}"
                    )
                    // )
                    /* solhint-enable */
    ...
    function tokenURI(
        ...
        LoyaltyProgram memory loyaltyProgramData = lps[slotOf(tokenId_)];

        string memory imageUrl = bytes(_tokenMetadata[tokenId_].image).length ==
            0
            ? loyaltyProgramData.defaultImage
            : _tokenMetadata[tokenId_].image;
            ...
            abi.encodePacked(
                ...
                string(
                    abi.encodePacked(
                        ...
                        "/",
                        loyaltyProgramData.circulatingSupply.toString()
                    )
                ),
                '"}',
                ', {"trait_type": "',
                "Max Supply",
                '", "value": "',
                loyaltyProgramData.maxSupply.toString(),
                '"}'
            )
        );
        ...
        for (
            uint256 i = 0;
            i < _tokenMetadata[tokenId_].attributes.length;
            i++
        ) {
            json = string(
                abi.encodePacked(
                    json,
                    '{"trait_type": "',
                    _tokenMetadata[tokenId_].attributes[i].trait_type,
                    '", "value": "',
                    _tokenMetadata[tokenId_].attributes[i].value,
                    '"}'
                )
            );
            if (i < _tokenMetadata[tokenId_].attributes.length - 1) {
    ...
    // update tokenURI
    function updateTokenMetadataAll(
        uint256 tokenId,
        string memory name,
        string memory description,
        string memory image,
        string[] memory attribute_trait_type,
        string[] memory attribute_trait_value
    ) public {
        require(
            _exists(tokenId),
        ...
        _tokenMetadata[tokenId].image = image;
        for (uint256 i = 0; i < attribute_trait_type.length; i++) {
            _tokenMetadata[tokenId].attributes.push(
                Attribute(attribute_trait_type[i], attribute_trait_value[i])
            );
        }
    }

    // update token image
    function updateTokenImage(uint256 tokenId, string memory image) public {
        require(
            _exists(tokenId),
            "ERC721Metadata: URI set of nonexistent token"
        );
    ...
    // update token attributes, add if not exist, update if exist
    function updateTokenAttributes(
        ...
        for (uint256 i = 0; i < attribute_trait_type.length; i++) {
            ...
            for (
                uint256 j = 0;
                j < _tokenMetadata[tokenId].attributes.length;
                j++
            ) {
                if (
                    keccak256(
                        abi.encodePacked(
                            _tokenMetadata[tokenId].attributes[j].trait_type
                        )
                    ) == keccak256(abi.encodePacked(attribute_trait_type[i]))
                ) {
                    _tokenMetadata[tokenId]
                        .attributes[j]
                        .value = attribute_trait_value[i];
                    isExist = true;
                }
            }
            if (!isExist) {
    ...
    function removeTokenAttribute(
        ...
        for (
            uint256 i = 0;
            i < _tokenMetadata[tokenId].attributes.length;
            i++
        ) {
            if (
                keccak256(
                    abi.encodePacked(
                        _tokenMetadata[tokenId].attributes[i].trait_type
                    )
                ) == keccak256(abi.encodePacked(attribute_trait_type))
            ) {
                delete _tokenMetadata[tokenId].attributes[i];
            }
        }
    }
}
