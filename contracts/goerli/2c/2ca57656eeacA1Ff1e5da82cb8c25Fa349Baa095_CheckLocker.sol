...
import {ICheckLocker} from "./ICheckLocker.sol";
import {IERC20Mintable} from "../token/IERC20Mintable.sol";

/**
 * @title Locker Smart Contract
 *
 * @notice Locker Smart Contract which allows to lock CHECK ERC20 tokens and mint the
 * same amount of LCHECK tokens for specified period of time after which the users
 ...
 */
contract CheckLocker is ICheckLocker, ContextUpgradeable {
    ...
    // Total amount of CHECK tokens locked
    uint256 private _totalLocked;

    // Total amount of CHECK tokens released
    uint256 private _totalReleased;

    // Address -> amount of locked CHECK tokens
    mapping(address => uint256) private _usersLocked;

    // Address -> amount of already released CHECK tokens
    ...
    /**
     * @dev Creates a locker contract that locks CHECK tokens for lock period and then
     * after cliff date unlocks it in linear manner
     *
     * @param checkToken address of the CHECK token
     * @param lockToken address of the LCHECK token
    ...
    /**
     * @notice Locks the `amount` of CHECK token inside of the smart contract
     * It will mint the same amount of LCHECK tokens to the user address
     * @param amount amount of the tokens locked
     *
     * @dev CHECK has 6 DECIMALS so the amount should be specified accordingly
    ...
    /**
     * @notice Releases the `amount` of CHECK token from the smart contract
    ...
    function release(uint256 amount) external override {
        ...
        // We need to burn the same amount of LCHECk tokens
        _lockToken.burn(sender, amount);

        // Send CHECK token to the user wallet
     ...
     */
    function lockedOf(address account)
        external
        view
        override
        returns (uint256)
    ...
    /**
     * @notice Returns amount of released CHECK tokens
     * @param account address of the locker
    ...
    function _unlocked(uint256 locked, uint256 released) private view returns (uint256) {
        ...
        if (block.timestamp < _cliff) {
            ...
            uint256 vestedAmount = (totalAmount * (unlockTime - _cliff)) /
                _unlockDuration;
 ...
 * lock CHECK tokens and mint the same amount of LCHECK tokens for 1 month after
 * which the users will be able to make linear unlock
 *
 * @dev Locker Interface
 */
interface ICheckLocker is ILockableFund {
    /**
     * @notice Lock event
     * @dev Emitted when `depositor` locks `amount` of CHECK token.
     * @param account address of the locker
     * @param amount amount of CHECK tokens locked by `locker`
     */
     ...
     * @param account address of the locker
     * @param amount amount of CHECK tokens release by `locker`
     */
