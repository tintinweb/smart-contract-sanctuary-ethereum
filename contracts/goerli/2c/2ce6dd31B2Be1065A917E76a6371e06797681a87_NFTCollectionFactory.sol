...
abstract contract NFTCollectionFactoryACL {
  ...
  modifier onlyAdmin() {
    if (!_rolesManager.isAdmin(msg.sender)) {
...
abstract contract NFTCollectionFactoryNFTCollections is NFTCollectionFactorySharedTemplates {
    ...
    returns (address collection)
  {
    ...
    emit NFTCollectionCreated(collection, msg.sender, version, name, symbol, nonce);
...
abstract contract NFTCollectionFactoryNFTDropCollections is NFTCollectionFactorySharedTemplates {
  ...
  function _createNFTDropCollection(NFTDropCollectionCreationConfig memory creationConfig)
    private
    returns (address collection)
  {
  ...
  NFTCollectionFactoryTemplateInitializer
{
   ...
   * Initializer 1 was previously used on mainnet.
   */
  function initialize(address nftCollectionImplementation, address nftDropCollectionImplementation)
    external
    reinitializer(2)
  {
   ...
   */
  function _createCollection(CollectionType collectionType, uint96 nonce, string memory symbol)
    internal
    returns (address collection, uint256 version)
  {
    ...
    collection = implementation.cloneDeterministic(_getSalt(msg.sender, nonce));
   ...
   */
  function getCollectionTemplateDetails(CollectionType collectionType)
    external
    view
   ...
   * @dev Returns address(0) if the collection type is not supported.
   */
...
contract $NFTCollectionFactoryACL is NFTCollectionFactoryACL {
    ...
    constructor(address rolesManager_) NFTCollectionFactoryACL(rolesManager_) {}
...
contract $NFTCollectionFactoryNFTCollections is NFTCollectionFactoryNFTCollections {
    ...
    event $_createCollection_Returned(address arg0, uint256 arg1);

    constructor(address rolesManager_) NFTCollectionFactoryACL(rolesManager_) {}

    function $_createCollection(CollectionType collectionType,uint96 nonce,string calldata symbol) external returns (address, uint256) {
        (address ret0, uint256 ret1) = super._createCollection(collectionType,nonce,symbol);
        emit $_createCollection_Returned(ret0, ret1);
        return (ret0, ret1);
    }
