...
contract IP3 {
    ...
    function purchaseByAmount(
        ...
        if (nftOnRecord.contains(hashedNft)) {
            ...
            currentStartPrice = getCurrentStartingPricePerNFT(hashedNft);
        } else {
            ...
            authroizeRecordMap[hashedNft]
                .authorizedNFT
                .listStartTime = authorizedNFTSubmit.listStartTime;
            authroizeRecordMap[hashedNft]
                .authorizedNFT
                .listEndTime = authorizedNFTSubmit.listEndTime;
            authroizeRecordMap[hashedNft]
                .authorizedNFT
        ...
        authroizeRecordMap[hashedNft].authorizedNFT.lastActive = block
            .timestamp;
        ...
        updatePurchaseState(newAuthorizeCertificate, hashedAuthorizeNFT);
    ...
    function getCurrentStartingPriceAllNFT()
        public
        view
        returns (bytes32[] memory, uint256[] memory)
    {
        ...
        for (uint256 i = 0; i < size; i++) {
            ...
            prices[i] = getCurrentStartingPricePerNFT(hashedNfts[i]);
        }

        return (hashedNfts, prices);
    }

    function getCurrentStartingPricePerNFT(bytes32 hashedNft)
        public
        view
        returns (uint256)
    {
