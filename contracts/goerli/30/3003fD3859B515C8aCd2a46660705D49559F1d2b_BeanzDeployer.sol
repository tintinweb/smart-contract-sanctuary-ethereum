...
contract BeanzDeployer is ERC721A, Ownable, ReentrancyGuard {
    ...
    // Total amount of tokens to mint
    uint256 public maxSupply;

    // Total amount of tokens reserved for whitelist
    uint256 public whitelistReserved;

    // Max amount of tokens minted for one wallet
    uint256 public maxPerWallet;

    // Max amount of tokens minted with one transaction
    uint256 public maxMintAmountPerTx;

    // Total amount of free tokens
    uint256 public maxFree;

    // Max amount of free tokens minted with one transaction
    uint256 public maxFreeMintAmountPerTx;

    // Max amount of free tokens minted for one wallet
    ...
    // Mint available for wallets whitelist
    function whitelistMint(uint256 _mintAmount, bytes32[] calldata _merkleProof)
        public
        payable
        mintCompliance(_mintAmount)
        mintPriceCompliance(_mintAmount)
        totalSupplyCompliance(_mintAmount)
    {
        // Verify whitelist is not paused
        require(whitelistMintEnabled, "The whitelist sale is not enabled!");
        // Verify whitelist was not claimed before
        require(!whitelistClaimed[_msgSender()], "Address already claimed!");

        // Verify wallet is in the whitelist
    ...
    // Mint for provided address (doesn't require funding)
    ...
    // Check if totalSupply is not exceeded
    ...
    // Check if mint limits are not exceeded
    modifier mintCompliance(uint256 _mintAmount) {
        if ((isFree(_mintAmount) && msg.value == 0) || whitelistMintEnabled) {
            require(
    ...
    function isFree(uint256 _mintAmount) internal view returns (bool) {
        return (((totalSupply() + _mintAmount <= maxFree - whitelistReserved) &&
            (mintedWallets[_msgSender()] + _mintAmount <= maxFreePerWallet)) &&
            !whitelistMintEnabled);
    }

    function _baseURI() internal view virtual override returns (string memory) {
        return string(abi.encodePacked(uriPrefix, ipfsHash, '/'));
    }
