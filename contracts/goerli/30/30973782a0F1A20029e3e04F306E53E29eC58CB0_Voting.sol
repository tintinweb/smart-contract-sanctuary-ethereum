...
contract Voting is Initializable, ContextUpgradeable, EIP712Upgradeable, UUPS {
    ...
    function castVoteBySig(
        ...
        require(block.number <= expiry, "Voting, signature expired");
    ...
    function _castVote(
        ...
        Vote storage vote = _votes[voteId];
        require(
            block.number >= vote.voteStart && block.number <= vote.voteEnd,
    ...
    function _countVote(
        ...
        vote.hasVoted[voter] = block.number;
    ...
    // Delegate with signature, {expiry} is the expiration block number of the certificate not the delegation
    ...
    function state(uint256 voteId) public view returns (VoteState) {
        Vote storage vote = _votes[voteId];
        if (block.number < vote.voteStart) {
...
contract AccessControl is Initializable, ContextUpgradeable, UUPS {
    ...
    /**
     * Returns {true} if {account} has been granted the role {role} till the current block number (inclusive)
     */
    ...
    /**
     * Obtain the last block number the role is valid till (inclusively)
     */
     ...
     * It is practically impossible to have a block number that big
     */
     ...
     * the current block number would work
     */
    ...
    /**
     * Obtain the member's address and the last block number the role is granted till
     */
    ...
    UUPS
{
     ...
     *
     * {openning} is the block number the proposal becomes executable (if succeeded and not cancelled)
     *
     ...
     * can still cast their ballot at block number {voteEnd}
     * {openning} is the block number when the proposal becomes executable if the vote
    ...
    // token ID => voting criteria
    mapping(uint256 => VotingCriteria) private _votingCriteria;
    ...
    function getVotingCriteria(uint256 tokenId)
        public
        view
        returns (VotingCriteria memory)
    {
        return _votingCriteria[tokenId];
    }
    ...
    function setVotingCriteria(
        ...
        emit VotingCriteriaChanged(
            tokenId,
            _votingCriteria[tokenId].quorumNumerator,
            _votingCriteria[tokenId].approvalRateNumerator,
            newQuorumNumerator,
            newApprovalRateNumerator
        );
        _votingCriteria[tokenId].quorumNumerator = newQuorumNumerator;
        _votingCriteria[tokenId]
            .approvalRateNumerator = newApprovalRateNumerator;
    }
    ...
    function propose(
        ...
        if (schedule.voteStart > block.number) {
    ...
    function cancel(uint256 proposalId) public returns (uint256) {
        _groupAccessControl(PROPOSER_ROLE, _proposals[proposalId].tokenId);
