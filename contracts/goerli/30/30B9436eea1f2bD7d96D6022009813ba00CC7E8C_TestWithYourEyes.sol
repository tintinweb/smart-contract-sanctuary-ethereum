...
contract TestWithYourEyes is ERC721URIStorage, Ownable {
    ...
    function mint(uint256 amount) external payable {
        ...
        string memory metadata = string(
            abi.encodePacked(randomIndex.toString())
        );
        ...
        string memory updatedMetadata = string(
            abi.encodePacked('{"name":"', tokenName, '",', metadata, "}")
        );

        // Update the tokenMetadata mapping with the updated metadata
        tokenMetadata[tokenId] = updatedMetadata;
        _safeMint(msg.sender, tokenId);

        _setTokenURI(tokenId, updatedMetadata);
