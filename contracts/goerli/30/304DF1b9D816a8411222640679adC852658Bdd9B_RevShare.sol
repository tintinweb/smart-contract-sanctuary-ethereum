...
contract RevShare {
    ...
    function returnAuction() public view returns (uint256, uint256, uint256, uint256, address, bool){
       return NounsAuctionHouse(auctionHouseProxy).auction();
    }
    function createBid(uint256 nounId_,string memory referalCode_) external payable {
        (uint256 nounid, uint256 amount_, , uint256 endtime,,) = returnAuction();
        require(nounid == nounId_, 'Social Noun not up for auction');
        require(block.timestamp < endtime, 'Auction expired');
        require(msg.value >= reservePrice, 'Must send at least reservePrice');
        require(
            msg.value >= amount_ + ((amount_ * minBidIncrementPercentage) / 100),
