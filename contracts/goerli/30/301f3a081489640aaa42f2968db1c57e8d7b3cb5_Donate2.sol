...
contract Donate2 is Ownable {
    ...
    mapping(address => uint256) private _balancesTimestamp;
    ...
    event Bonused(address indexed user, address indexed client, uint256 amount);
    ...
    constructor() {
        // usdt = IBEP20(0x13119db2daD4026a795abB80450236fCecE151Bf); // USDT
        // token = IBEP20(0x74276D2B0061cB0E07b382B1680665946d830BC9); // token

        usdt = IBEP20(0x38EEEef21C306722146066F29d901FD8cc8017b1); // USDT goerli
        token = IBEP20(0xee58cdeA57f4B00C6Fb41eCD8b4734c501b25C30); // token goerli
    ...
    function getCode(address account ) private {
    ...
    function getBalancesTimestamp(address account) public view returns (uint256) {
        return _balancesTimestamp[account];
    }
    ...
    function buy(uint256 purchaseIndex, uint256 code) public {
        require(_balances[msg.sender] == 0, "Cannot buy");
        require(
        ...
        _balances[msg.sender] = _maxAmount;
        _balancesTimestamp[msg.sender] = block.timestamp;
        ...
        if (code != 0) {
            ...
            emit Bonused(_code2Addr[code], msg.sender, b);
        }
    ...
    function withdrawBonus() public {
        ...
        usdt.transfer(msg.sender, _bonus[msg.sender]);
        emit WithdrawBonus(msg.sender, _bonus[msg.sender]);
        _bonus[msg.sender] = 0;
    }
    ...
    function withdrawReward() public {
        require(_rewardPerTokenPaid[msg.sender] < _balances[msg.sender].mul(2), "no balance");
        uint256 reward =  myReward(msg.sender, block.timestamp).sub(_rewardPerTokenPaid[msg.sender]);
        _rewardPerTokenPaid[msg.sender] = _rewardPerTokenPaid[msg.sender].add(reward);
        token.transfer(msg.sender, reward);
        emit WithdrawReward(msg.sender, reward);
    }
    event MyReward(address indexed account, uint256 day, uint256 a, uint256 b);
    function myReward(address account, uint256 t) public returns(uint256) {
        if(_balancesTimestamp[account] == 0) return 0;
        uint256 endTime = _balancesTimestamp[account];
        uint256 daysDiff = DaysDiff(t, endTime);
        uint256 a = 0;
        if(daysDiff <= 20){
            a = _balances[account].mul(50).div(1000).mul(daysDiff);
        }else if(daysDiff > 20){
            a = _balances[account].mul(50).div(1000).mul(20);
        }
        uint256 b = 0;
        if(daysDiff > 20 && daysDiff <= 60){
            b = _balances[account].mul(25).div(1000).mul(daysDiff.sub(20));
        }else if(daysDiff > 60){
            b = _balances[account].mul(25).div(1000).mul(40);
        }

        emit MyReward(msg.sender, daysDiff, a, b);
