...
contract ChainlinkSwap is BaseSwap {
    ...
    function swap(
        ...
        if (_from == dexData.commodityToken) {
            ...
            require(dexData.reserveStable >= stableAmount, "not enough balance");
            ...
            require(dexData.reserveCommodity >= commodityAmount, "not enough balance");
    ...
    {
        uint amount = (commodityAmount * convertUSDToStable(getChainLinkFeedPrice())) / (10**8);
        amount = convertUSDToStable(amount);
        require(
            SwapLib.normalizeAmount(amount, dexData.commodityToken, dexData.stableToken) == stableAmount,
...
library SwapLib {
    ...
    struct DexSetting {
        ...
        address stablePriceFeed;//chainlink feed for stable/usd conversion
        address stableToUSDPriceFeed; // stable coin to USD price feed
    }
