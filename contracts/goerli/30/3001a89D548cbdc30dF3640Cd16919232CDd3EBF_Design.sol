...
contract Design {
    ...
    mapping(address => uint256) distribute;

    constructor(){
        signer = msg.sender;
    }

    function SetDistribute(address _account, uint256 _tokenId, bytes memory _signature) public {
        bytes32 _msgHash = keccak256(abi.encodePacked(_account, _tokenId));
