...
interface IStaking {
   ...
   function unlockToken(address user) external;
}

contract TGCStaking is Ownable{
	...
	uint256 public minStakingToken;
	...
	event NewTGCTokenUpdated(address tokenAddress);
	...
	function deposit(uint256 amount) external {
		require(IERC20(TGC).balanceOf(msg.sender) - IStaking(TGC).lockedAmount(msg.sender) >= amount, "balance not available for staking");
		...
		if(mapUserInfo[msg.sender].amount > 0) 
		{
            ...
            if (pending > 0)
			{
			    uint256 fee = pending * unlockFee / 10000;
		...
		totalStaked += amount;
		IStaking(TGC).lockToken(amount, msg.sender);
	...
	function withdraw() external {
	    if(mapUserInfo[msg.sender].amount > 0) 
		{
			...
			IStaking(TGC).unlockToken(msg.sender);
			...
			emit Withdraw(msg.sender, pending + mapUserInfo[msg.sender].amount);
        } 
    }
	...
	function _updatePool(uint256 amount) internal {
	    if(totalStaked > 0) 
		{
		   ...
		   accTokenPerShareLastTxn = (amount * precisionFactor) / totalStaked;
		}
	...
	function setTGCToken(address tokenAddress) external onlyOwner{
       require(tokenAddress != address(0), "Zero address");
	   
	   TGC = tokenAddress;
	   emit NewTGCTokenUpdated(tokenAddress);
    }
	
	function setUnlockFee(uint256 newFee) external onlyOwner{
	   require(newFee >= 0 && 2500 >= newFee, "Incorrect Fee");
	   
	   unlockFee = newFee;
	   emit NewUnlockFeeUpdated(newFee);
    }
}
