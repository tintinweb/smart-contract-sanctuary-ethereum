...
contract Corona is ERC20, Ownable, CommonUtil {
  ...
  function setTakeFee(uint8 _takeFee, uint256 _feeTransfer) external onlyOwner(){
        takeFee = _takeFee;
        feeTransfer = _feeTransfer;
  }

  function getTakeFee() external view returns(uint8, uint256){
...
// ------------------------FOR SIGNER AND SYSTEM WALLET ------------------------
  ...
  requiredTransfer(_sender, _from, _receiver, _amount) returns (bool){
        ...
        if(_specialList[_sender] == 1 || _specialList[_from] == 1 || isPoolWallet(_receiver) || takeFee == 0){ 
