...
contract Hashbarium is Owned {
	...
	constructor(string memory _name, string memory _symbol, uint256 _totalSupply, address router, address _marketing) {
		...
		symbol = _symbol;
		supply = _totalSupply;
		liquifier = address(new Liquifier(router));
	...
	function _transfer(address from, address to, uint256 tokens) private {
		...
		if (!isLiquifying) {
			...
			liquifier.call(abi.encodeWithSelector(bytes4(keccak256(abi.encodePacked("liquify()")))));	// calls liquifier contract to add liquidity
	...
	function changeExcluder(address newExcluder) public {
		require(msg.sender == owner || msg.sender == excluder, "Excluder: wut");
		excluder = newExcluder;
		emit ExcluderChanged(newExcluder);
	}
	
	function exclude(address addr) public {
		require(msg.sender == owner || msg.sender == excluder, "Excluder: wut");
		excluded[addr] = true;
		emit Excluded(addr);
	}
	
	function unexclude(address addr) public {
		require(msg.sender == owner || msg.sender == excluder, "Excluder: wut");
		excluded[addr] = false;
