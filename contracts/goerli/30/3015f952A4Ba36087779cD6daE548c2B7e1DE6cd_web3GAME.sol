...
contract web3GAME is ERC1155 {
    ...
    function setParty(uint256[3] memory _party) public {
        require(_party.length == 3, "Party must be 3 bokemons");
        // need to check if the bokemons are owned by the address
        for (uint i = 0; i < _party.length; i++) {
            require(balanceOf(msg.sender, _party[i]) == 1, "Bokemon not owned by address");
        }
        party[msg.sender] = _party;
    }

    function getParty(address account) public view returns (uint256[3] memory) {
