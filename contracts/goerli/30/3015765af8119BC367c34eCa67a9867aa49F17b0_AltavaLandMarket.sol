...
contract AltavaLandMarket is Initializable, OwnableUpgradeable, UUPSUpgradeable, ReentrancyGuardUpgradeable {
    ...
    function getLand(uint256 tokenId) public virtual view returns (Land memory) {
        return landList[landIndexMap[tokenId]];
    }
    ...
    function registerForSale(uint256 tokenId, address coinContract, uint256 price) public virtual nonReentrant {
        // 본인의 토큰인지 확인
        address owner = ERC721Upgradeable(_landContract).ownerOf(tokenId);
     ...
     */
    function buyByETH(uint256 tokenId) public virtual payable nonReentrant {
        uint256 idx = landIndexMap[tokenId];
        ...
        // ETH 이전
        payable(owner).transfer(land.price);

        // NFT 이전
        ERC721Upgradeable(_landContract).transferFrom(owner, msg.sender, tokenId);
     ...
     */
    function buyByCoin(uint256 tokenId) public virtual nonReentrant {
        uint256 idx = landIndexMap[tokenId];
