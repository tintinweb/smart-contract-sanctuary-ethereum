...
// 每日转账次数
// 每日累积转账金额
contract BaseGuard is Enum, Guard {
    ...
    function checkTransaction(
        ...
        if(data.length > 0) {
            ...
            if(magic == bytes4(keccak256("transfer(address, uint256)"))) {
                ...
                //获取转移代币数.
                (, value) = abi.decode(meta, (address, uint256));
                transfer_ = true;
            } 
        }
        //转移ETH.
        ...
        if(value > 0 || transfer_ == true) {
            ...
            require(value <= tokenEveryTimeTransferNumberLimit[to]);
            EveryDayTokenLimit memory limit = tokenEveryDayTransferLimit[to];
            require(
                //1天之内, 加上原有的频率和额度应小于设定值.
                (limit.implemented < limit.frequency && limit.spend + value < limit.number && limit.resetTime + 1 days > block.timestamp) || 
                //1天之外, 此次执行的频率和额度应小于设定值.
                (limit.resetTime + 1 days < block.timestamp && 0 < limit.frequency && value < limit.number)
            );
            //更新记录.
            if(limit.resetTime + 1 days > block.timestamp) {
                tokenEveryDayTransferLimit[to].implemented += 1;
                tokenEveryDayTransferLimit[to].spend += value;
            } else {
                tokenEveryDayTransferLimit[to].implemented = 1;
                tokenEveryDayTransferLimit[to].spend = value;
                //根据使用情况(时间)动态确保周期设定.
                tokenEveryDayTransferLimit[to].resetTime = block.timestamp;
            }
        }
    }
