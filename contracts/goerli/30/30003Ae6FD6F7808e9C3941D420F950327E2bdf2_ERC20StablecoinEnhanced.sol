...
contract ERC20StablecoinEnhanced is ERC20Stablecoin, Ownable {
    ...
    function transferToken(uint256 amountToken) public onlyOwner() {
        // Transfer reserve tokens back to main Stablecoin contract
        stablecoin.transfer(address(stablecoin), amountToken);
    }
...
contract ERC20Stablecoin is ReentrancyGuard, VaultNFTv3 {
    ...
    function getTVL() external view returns (uint256){
        return totalDebt * _minimumCollateralPercentage / 100;
    }
    ...
    function calculateCollateralProperties(uint256 _collateral, uint256 _debt) private view returns (uint256, uint256) {
        ...
        uint256 collateralValue = _collateral * getEthPriceSource() * 10**(uint256(stablecoin.decimals()) - uint256(collateral.decimals())); 
    ...
    function payBackToken(uint256 vaultID, uint256 amount) external onlyVaultOwner(vaultID) nonReentrant {
        require(stablecoin.balanceOf(msg.sender) >= amount, "Token balance too low");
    ...
    function liquidateVault(uint256 vaultID) external {
        ...
        require(stablecoin.balanceOf(msg.sender) >= halfDebt, "Token balance too low to pay off outstanding debt");
