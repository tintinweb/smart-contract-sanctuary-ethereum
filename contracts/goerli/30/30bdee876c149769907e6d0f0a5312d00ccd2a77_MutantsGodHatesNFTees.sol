...
contract MutantsGodHatesNFTees is ERC721Enumerable, Ownable , ReentrancyGuard {    
  ...
  //  RtBnno private immutable rtbnno;
   ...
   // address ghnftAddress = 0xE6d48bF4ee912235398b96E16Db6F310c21e82CB; 
    address bnnoAddress = 0x7a137D760687FBB2f818A1E17ed96E5E18841EE5;
    address ahcAddress = 0xE120Eb0DcC194431Aa09F4aA115EbE28c0Fd403e;
   // address rtbnnoAddress = 0x0912DAD1db4643368B029166AF217B8A9818dB15;
    ...
    function mutateWithSrBanano(uint256 amount, uint256[] memory ApeTokenIDs) external  {
      ...
      //  require(ghnft.balanceOf(msg.sender) > 0, "Must own at least 1 GodHatesNFTees to Mutate");
        require(bnno.balanceOf(msg.sender, 0) > 0 && bnno.balanceOf(msg.sender, 0) >= amount, "Must own at least 1 Sr Banano to mutate, or max bananas."); 
        require(ahc.balanceOf(msg.sender) > 0 && ahc.balanceOf(msg.sender) >= amount, "Must own at least 1 Ape to mutate, or exceed amount selected."); 
        for (uint256 i = 0; i < amount; i++) { 
            require( ahc.ownerOf(ApeTokenIDs[i]) == msg.sender, "Must own the Ape that you want to mutate." );
            M1_minted++;
    ...
    function internalMint(uint256 amount) external onlyOwner  {
        require(M2_minted + amount <= M2_MAX_SUPPLY, "Max supply exceeded!");
        M2_minted += amount;
        for (uint256 i = 0; i < amount; i++) { 
