...
contract PoorPleb is ERC20, Ownable {
    mapping(address => bool) private claimList;
    mapping(address => uint256) private claimListAmounts;
    mapping(address => uint256) private claimListCounter;
    uint256 private claimMaxDate;
    uint256 private claimMaxNumber;

    constructor() ERC20("PoorPleb", "PP") {
        _mint(msg.sender, 37000000000 * 10**decimals());
        claimMaxDate = block.timestamp + 69 days;
        claimMaxNumber = 1;
    }

    /**
     * @notice Update max claim number limit
     */
    function updateClaimMaxNumber(uint256 val) external onlyOwner {
        claimMaxNumber = val;
    }
    ...
    function claim() external {
        ...
        require(claimListCounter[msg.sender] < claimMaxNumber, "already claim");
        claimListAmounts[msg.sender] = 1420369000000000000000000;
        claimListCounter[msg.sender] += 1;
    ...
    /**
     * @notice Return number of claims from wallet
     */
    function getClaimNumber(address account) external view virtual returns (uint256) {
        return claimListCounter[account];
    }

    /**
     * @notice Return true or false if user can claim or not
     */
    function getClaimMaxDate() external view virtual returns (uint256) {
        return claimMaxDate;
    }

    /**
     * @notice Return amount claimed from wallet
     * @param account user address
     */
    function getClaimedAmount(address account)
        ...
        returns (uint256)
    {
        return claimListAmounts[account];
    }
}
