...
contract Calendar is ERC721, Ownable {
    ...
    address private _team_d;
    address private _team_a;
    ...
    mapping(uint256 => string) public day_to_uri; //private?
    mapping(address => mapping(uint256 => bool)) public days_of_owner; //private?
    ...
    modifier onlyTeam() {
        require(isTeam[msg.sender] == true, "No Team member.");
        _;
    }

    /**
     *  Check on how many days the owner minted.
     *  It dosen´t make sense to check more than 24 days.
     */
    ...
    {
        uint256 currentDay = calculateDay();
        require(currentDay <= 24 && currentDay > 0, "no discount");
        uint256 counter = 0;
        for (uint i = 1; i < currentDay; i++) {
        ...
        returns (string memory)
    {
        ...
        bytes memory dataURI = abi.encodePacked(
            "{",
            '"name": "Visions 2022 Day #', //Visions 2022 Day #1 Token #5
            calculateDay().toString(),
            "Token #",
            tokenId_.toString(),
            '",',
     ...
     *  A discount is calculated depending on the number of days the wallet minted before.
     *  Price can not be negative. If discount would be higher than price set price to 0. (should not happen)
    ...
    function mint(uint256 amount_) public payable {
        ...
        uint256 discount_granted = (checkDays(msg.sender) * _discount);
        if (discount_granted >= funds_needed) {
            funds_needed = 0;
        } else {
            funds_needed = funds_needed - discount_granted;
        }
        ...
        for (uint i = 0; i < amount_; i++) {
            ...
            token_to_day[id] = day; //Day 0 is the 01/12/2022 => +1
     ...
     */
    function setDaysReveal(string[] calldata allPaths) public onlyOwner {
        require(allPaths.length == 24, "Set all URIs at once.");
        for (uint i = 0; i < 24; i++) {
            day_to_uri[i + 1] = allPaths[i];
        }
    }
    ...
    function setDiscount(uint256 discount_) public onlyOwner {
        _discount = discount_;
    }

    /**
     * Withdraw funds and split it between team addresses.
     */

    function withdraw() public onlyTeam {
        uint256 _balance = address(this).balance;
        uint256 _split = (_balance * 50) / 100;

        payable(_team_a).transfer(_split);
        payable(_team_d).transfer(_balance - _split);
    }
}
