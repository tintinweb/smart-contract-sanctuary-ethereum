...
contract DeadlyPerks is ReentrancyGuard, ERC721A, Ownable {
    
    // to-do -> restructure data
    struct HolyBook {
        bytes32 root;
        address blueprint;
    ...
    // "bridge_handler" -> selector_name
    constructor(
        bytes32 _root, 
        address _proxyRegistryAddress, 
        address _takezoFromAccounting, 
        address _blueprint) ERC721A("Deadly Perks", "dPERKS") {
        holyBook.root = _root;
        ...
        holyBook.STARKNET_BRIDGE_SELECTOR = 964389997440004823239463948323422964565318033733625475948403104945866683179;
        publicSale = false;
        if (holyBook.RESERVES > 0) { // remove after tests
            collectReserves();
        }
    }
    ...
    function setStarknetContract (uint256 _starknetContract) external onlyOwner {
        holyBook.STARKNET_ADDRESS = _starknetContract;
    }
    ...
    function collectReserves() private onlyOwner {
        require(totalMinted == 0, 'Reserves already taken.');
        _mint(_msgSender(), holyBook.RESERVES);
        totalMinted += holyBook.RESERVES;
    }
    ...
    function isEligible(address _user, bytes32[] calldata _proof) external view returns(bool _eligible) {
        if (totalMinted < holyBook.MAX_SUPPLY) {
            if(!userMinted[_user]) {
                string memory payload = string(abi.encodePacked(_user));
                _eligible = _verify(_leaf(payload), _proof);
            } else if (userMinted[_user]) {
                _eligible = false;
            }
        } else {
            _eligible = false;
    ...
    function isCampaignEligible (address _user) public view returns(bool) {
        for (uint i; i < collections.length; i++) {
            if(IERC721(collections[i]).balanceOf(_user) > 0 && 
                campaignAmount[collections[i]] > 0 &&
                campaignDeadline[collections[i]] > block.timestamp
            ) {
                return true;
            }
        }
    ...
    // (whitelisted user) mints via whitelist
    function whitelistMint(bytes32[] calldata proof) public { // should allow more than one -> discuss w/ team
    ...
    // (collection holder) mints via timed-campaign
    function campaignMint(address _collection) public { // should allow more than one -> discuss w/ team
    ...
    // (user) unbundles perks into L2 assets
    function bridgePerks (uint256 _user, uint256 _id) public { // to-do -> convert to packed bits
    ...
    // (user) fills slots with earned sins
    function fillSlots (uint256 _id, uint256 _user, uint256[8] memory _slots) public { // to-do -> convert to packed bits
    ...
    // (user) goes unlocks passage with key
    ...
    function publicMint(uint256 amount) public payable {
        require(publicSale);
        require(totalMinted + amount < holyBook.MAX_SUPPLY, "Exceeds max supply.");
