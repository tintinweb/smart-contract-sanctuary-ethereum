...
// File: contracts/DenOfDragons.sol
...
// File: contracts/StakingOfDragons.sol
...
contract StakingOfDragons is ERC721Holder {
    ...
    function viewTrueTokenBalance () public view returns (uint256) {
        uint256 tokenBal = eggToken.balanceOf(msg.sender);
        uint256 combinedSecsStaked = 0;
        uint256 matches = 0;
        uint256 counter = 1;
        while (matches < stakedNFTs[msg.sender] && counter <= 4444) {
            if (originalOwner[counter] == msg.sender && isStaked[counter] == true) {
                matches ++;
                combinedSecsStaked = combinedSecsStaked + viewStakedTime(counter);
            }
            counter ++;
        }
        return(tokenBal + ((combinedSecsStaked / 86400) * (10 ** 18)));
    }
