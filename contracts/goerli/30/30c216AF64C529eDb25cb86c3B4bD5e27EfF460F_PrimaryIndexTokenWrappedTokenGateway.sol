...
contract PrimaryIndexTokenWrappedTokenGateway is Initializable, AccessControlUpgradeable, ReentrancyGuardUpgradeable
{
    ...
    function initialize(address pit, address weth) public initializer {
    ...
    function supply() public payable {
        ...
        primaryIndexToken.supply(address(WETH), msg.value, address(this), msg.sender);
    }
    ...
    function redeem(uint256 bLendingTokenAmount) public {
        ...
        ERC20Upgradeable(fWETH).transferFrom(msg.sender, address(this), amountToWithdraw);
        primaryIndexToken.redeem(address(WETH), amountToWithdraw, address(this), msg.sender);
    ...
    function repay(address projectToken, uint256 lendingTokenAmount) public payable {
        uint totalOutStanding = getTotalOutstanding(msg.sender, projectToken);
        uint paybackAmount = lendingTokenAmount <= totalOutStanding ? lendingTokenAmount : totalOutStanding;
    ...
    function borrow(address projectToken, uint256 lendingTokenAmount) public {
        primaryIndexToken.borrow(projectToken, address(WETH), lendingTokenAmount, address(this), msg.sender);
