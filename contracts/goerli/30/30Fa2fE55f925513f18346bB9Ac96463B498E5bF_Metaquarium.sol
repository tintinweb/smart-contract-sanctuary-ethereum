    ...
    ReentrancyGuard
{
    ...
    event MetadataUpdate(uint256 tokenId, string uri);
    ...
    modifier whenMintingEnabled() {
        require(publicMintEnabled, "Minting is currently disabled");
        _;
    }
    ...
    // <º))))>< <º))))>< <º))))>< <º))))>< <º))))>< <º))))>< <º))))>< <º))))><
    /// @notice 0o0o0o0o0o CONSTRUCTOR o0o0o0o0o~
    constructor(
        address[] memory admins,
        address payable treasury,
        address minter
    ) ERC721A("Metaquarium", "AQUA") {
        _treasuryAccount = treasury;
        _setDefaultRoyalty(treasury, 1500);
    ...
    // ~0o0o0o0o0o RECIEVE FUNCTIONS o0o0o0o0o~
    receive() external payable {
        emit Received(msg.sender, msg.value);
    }

    // ~0o0o0o0o0o FALLBACK FUNCTION o0o0o0o0o~
    fallback() external payable {
        emit Received(msg.sender, 0);
    }

    // ~0o0o0o0o0o EXTERNAL ACCESS CONTROLLED FUNCTIONS o0o0o0o0o~

    // Ability to pause contract
    function pause() external nonReentrant onlyRole(ADMIN) {
        _pause();
        emit PauseEvent();
    }

    // Ability to unpause contract
    function unpause() external nonReentrant onlyRole(ADMIN) {
        _unpause();
        emit PauseEvent();
    }

    // Ability to permanently freeze metadata
    function freezeMetadata() external nonReentrant onlyRole(ADMIN) {
        _metadataFrozen = true;
    }

    // Ability to set the baseURI
    function setBaseURI(
    ...
    // Ability to set the royalty rate
    function setRoyalty(uint96 points) external nonReentrant onlyRole(ADMIN) {
        _setDefaultRoyalty(_treasuryAccount, points);
    }

    // Ability to set the treasury account
    ...
    // Ability to set the default mint price
    function setPrice(
    ...
    // Ability to set the pauseMintAtTokenId
    ...
    // Ability to set the token metadata
    function setTokenMetadata(
        ...
        for (uint256 i = 0; i < tokenURIs.length; i++) {
            ...
            emit MetadataUpdate(tokenURIs[i].tokenId, tokenURIs[i].uri);
        }
    }

    // Ability to withdraw funds
    function withdraw() external nonReentrant onlyRole(ADMIN) {
        Address.sendValue(_treasuryAccount, address(this).balance);
        emit FundsWithdrawn();
    }

    // Ability for admin to burn tokens
    function burn(uint256 tokenId) external nonReentrant onlyRole(ADMIN) {
        if (!_exists(tokenId)) revert URIQueryForNonexistentToken();
        _burn(tokenId);
    }

    // Ability for admin to batch burn tokens
    function batchBurn(
    ...
    // Ability for admin to enable public minting
    function enableMinting() external nonReentrant onlyRole(ADMIN) {
        publicMintEnabled = true;
    }

    // Ability to disable public minting
    function disableMinting() external nonReentrant onlyRole(ADMIN) {
        publicMintEnabled = false;
    }

    /// @notice Private function for admin to mint
    ...
    /// @notice Private function to mint token for Minter Role
    /// @param to address to mint to
    /// @param amount uint256 quantity to mint
    function memberMint(
        address to,
        uint256 amount
    ) external payable nonReentrant onlyRole(MINTER) {
    ...
    // ~0o0o0o0o0o EXTERNAL PUBLIC FUNCTIONS o0o0o0o0o~

    /// @notice Public function to mint tokens
    /// @dev This function is payable, and will mint tokens if the correct amount of ETH is sent
    /// @dev This function will pause minting if the mint cap is reached`
    /// @dev This function will error if the max supply is reached
    /// @param to address to mint to
    /// @param amount uint256 quantity to mint
    function mint(
        address to,
        ...
        // pause the mint if we've reached the mint max
    ...
    // Ability to get the current baseURI
    function baseURI() external view returns (string memory) {
        return _currentBaseURI;
    }

    // Ability to get the current mint price
    function price() external view returns (uint256) {
        return _price;
    }

    // Get the current treasury account
    function treasuryAccount() public view returns (address) {
        return _treasuryAccount;
    }

    // ~0o0o0o0o0o 721A PUBLIC OVERRIDE FUNCTIONS o0o0o0o0o~=
    ...
    // Override ERC721Metadata Functions
    // ><((((º> ><((((º> ><((((º> ><((((º> ><((((º> ><((((º> ><((((º> ><((((º>
    // THE ONLY THING THAT MATTERS IS THE TOKEN ID
    // <º))))>< <º))))>< <º))))>< <º))))>< <º))))>< <º))))>< <º))))>< <º))))><
    function tokenURI(
        uint256 tokenId
    ...
    /// @dev 721A Private Function Overrides
