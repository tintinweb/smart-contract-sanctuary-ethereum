...
contract subscriptionFactory {
    Subscription[] private subscriptionAddresses;

    mapping(address => address[]) public contractsDeployed;
    mapping(address => uint256) public numContractsDeployed;
    ...
    function createSubscription(uint96[] memory _price, string memory _name, address _owner, bool _USDc, bool _Dai, bool _USDt, bool _recurring, bool _streaming) external {
        ...
        address deployedAddy = address(subscription);
        uint256 numDeployed = numContractsDeployed[_owner];
        contractsDeployed[_owner][numDeployed] = deployedAddy; 
        numContractsDeployed[_owner]++;
...
contract Subscription {
    ...
    function stopSubscription(address _sender, uint256 _stablesId, bytes memory _data) public {
        require(_sender == msg.sender, "you are not the address owner");
