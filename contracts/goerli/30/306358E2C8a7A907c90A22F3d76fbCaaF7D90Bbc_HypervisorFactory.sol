...
contract Hypervisor is IUniswapV3MintCallback, ERC20Permit, ReentrancyGuard {
   ...
   event Deposit(
    ...
    function withdraw(
        ...
        // uint256 totalAmount1 = amount0 * price / PRICE_PRECISION + amount1;
        TmpInfo memory tmp;
        tmp.sqrtPrice = TickMath.getSqrtRatioAtTick(currentTick());
        tmp.price = FullMath.mulDiv(uint256(tmp.sqrtPrice).mul(uint256(tmp.sqrtPrice)), PRICE_PRECISION, 2**(96 * 2));
        tmp.totalAmount1 = amount0 * tmp.price / PRICE_PRECISION + amount1;
        
        tmp.data = abi.encode(to);
        if(amount1 > tmp.totalAmount1 / 2 ){
            // need to swap token1 to token0
            tmp.swapAmount = tmp.price * (amount1 - amount0 ) / (tmp.price + PRICE_PRECISION );
            (tmp.amount0_swap, tmp.amount1_swap) = pool.swap(address(this), false, int256(tmp.swapAmount), TickMath.MIN_SQRT_RATIO + 1, tmp.data);
            amount0 += uint256(-tmp.amount0_swap);
            amount1 -= uint256(tmp.amount1_swap);
        }else{
            // need to swap token0 to token1
            tmp.swapAmount = PRICE_PRECISION *(amount0 - amount1 ) / (tmp.price  - PRICE_PRECISION );
            (tmp.amount0_swap, tmp.amount1_swap) = pool.swap(address(this), true, int256(tmp.swapAmount), TickMath.MAX_SQRT_RATIO - 1, tmp.data);
            amount0 -= uint256(tmp.amount0_swap);
            amount1 += uint256(tmp.amount1_swap);
        }
    ...
    function _burnLiquidity(
        ...
        if (liquidity > 0) {
            /// Burn liquidity
            (uint256 owed0, uint256 owed1) = pool.burn(tickLower, tickUpper, liquidity);  // 减少流动性
            ...
            if (collect0 > 0 || collect1 > 0) {
                (amount0, amount1) = pool.collect(to, tickLower, tickUpper, collect0, collect1);  // 收集手续费
            }
        }
    }
    ...
    function currentPrice() public view returns (uint256) {
        ...
        return uint256(sqrtPriceX96) **2 * PRICE_PRECISION / 2 ** 192;
    }
