...
contract DesupTokenVesting is Ownable, ReentrancyGuard {
    ...
    constructor(address token_) {
        ...
        token_price_for_seedA = 6000; // $0.006
    ...
    function invest() public payable {
        ...
        if (!isHolder) {
            ...
            holders[msg.sender].role = "INVESTOR";
        }
    }
    ...
    /**
     * Returns the latest price // real price is price ** 10 ** -8
     */
    ...
    function getUSDTFromEther(uint256 amount) public view returns (uint256) {
        return (getLatestPrice() * amount) / 10**18;
    }

    // Input ether amount as wei, and token price (one of token_price_for_public, token_price_for_seedA)
    ...
    {
        return
            (getUSDTFromEther(amount) / token_price) / 10**(8 - price_decimal); // 8 : ether price decimal offset
    }
