...
contract NormalPoolNFT is PoolBaseDerivativeNFT {
    ...
    returns (uint256 offerId) {
        ...
        _transfer(spanningMsgSender(), bytes32(uint256(uint160(currOffer.bidder))), tokenId);
    ...
    returns(uint256) {
        ...
        if (tax <= depositBalances[tokenOwner]) {
            ...
            _transfer(ownerOfSpanning(tokenId), bytes32(uint256(uint160(address(this)))), tokenId);
    ...
    function getLicenseOwnerList() public view returns(address[] memory) {
        ...
        for(uint256 i = 0; i < maxTokenNum; i++) {
            result[count] = tokenInfoMap[i].owner;
        }
        return result;
    }
