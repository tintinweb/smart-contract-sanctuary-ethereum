...
contract ChubbyKaijuDAOStakingV2 is IChubbyKaijuDAOStakingV2, Ownable, IERC721Receiver, Pausable, ReentrancyGuard {
  ...
  function claimNonUpgradedRewardsAndUnstake(uint16[] calldata tokenIds, bool unstake, bytes[] memory signatures) external whenNotPaused nonReentrant _updateEarnings {
    require(tx.origin == msg.sender, "eos only");

    uint128 reward;

    // if unstake is true, then we transfer from old contract to user
    // if unstake is false, then we transfer from old contract to new contract
    ...
    for (uint8 i = 0; i < tokenIds.length; i++) {
      ...
      // will fail if there is no token staked on old contract
      chubbyKaijuGen2.transferFrom(address(chubbyKaijuStakingOld), address(this), tokenIds[i]);
  ...
  function calculateNonUpgradedGEN2Rewards(uint256 tokenId, uint16 rewardType) external view returns (uint128) {
