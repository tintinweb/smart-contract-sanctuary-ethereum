...
contract RegensZero is ERC721Enumerable, Ownable, ReentrancyGuard {
    ...
    /// @dev tokenId to last time the content on a vault changed.
    mapping(uint256 => uint256) public lastTraitModification;

    /**
     *  @dev tokenId to timestamp, this timestamps indicates until when the
     *  token is locked (if timestamp < block.timestamp token not locked).
     *  If the token is locked user can't modify vaults.
     */
    ...
    /**
     *  @dev tokenId to controller of the token. Controller of a tokenId
     *  has some priviliged roles for that token. The token controller
     *  has all the privileges that the token signer has (and more).
     */
    mapping(uint256 => address) private controller;

    /**
     *  @dev tokenId to signer of the token. Signer of a tokenId
     *  has some priviliged roles for that token.
     */
    mapping(uint256 => address) private signer;

    /// @dev tokenId to timestamp, since when the current owner has been owner.
    mapping(uint256 => uint256) public ownerSince;

    /// @dev tokenId to timestamp, since when the current owner has been holder.
    mapping(uint256 => uint256) public holderSince;

    /// @dev tokenId to address of the traitVault they're using as DNA.
    mapping(uint256 => address) public tokenIdDNA;

    /// @dev emited when controller for a token changes.
    ...
    /// @dev emited when signer for a token changes.
    event signerChanged(
        uint256 indexed tokenId,
    ...
    /// @dev emited when timelock for a token changes.
    ...
    /**
     *  @notice allows owner or controller of tokenId to set controller.
     *
    ...
    /**
     *  @notice allows owner or controller of tokenId to set signer.
     *
     *  @param tokenId tokenId.
    ...
    /**
     *  @dev ERC721 beforeTokenTransfer hook. Checks that vaults for that tokenId have not been
     *  modified in the last 300 seconds to prevent MEV attacks. Updates ownerSince to block.timestamp
     *  (since owner is about to change)
     */
    function _beforeTokenTransfer(
        ...
        super._beforeTokenTransfer(from, to, tokenId); //overrides default ERC721 and ERC721Enumerable beforeTokenTransfer
        require(
    ...
    /**
     *  @dev Function that can only be called by vaults (that's what the require checks)
     *  when the content of a vault for tokenId has changed.
     *
     *  @param tokenId tokenId
     */
    ...
    /**
     *  @notice allows owner or controller of tokenId to lock token.
     *  @dev set's tokenTimelock for the tokenId to (2^256)-1 this means
     *  that the token is locked until that timestamp which is virtually
     *  infinite.
     *
     *  @param tokenId tokenId.
     */
    ...
    /**
     *  @notice allows owner or controller to unlock token,
     *  token will be unlocked in seven days (UNLOCKING_PERIOD).
     *  @dev Token has to be locked to be able to unlock it, set's tokenTimelock
     *  to block.timestamp + UNLOCKING_PERIOD for the tokenId
     *
     *  @param tokenId tokenId.
     */
    ...
    /**
     *  @notice function to transfer NFT
     *  @dev set's signer and controller to address zero.
     *  Changes holdership to block.timestamp
     */
     ...
     *  @notice function to transfer NFT
     *  @dev set's signer and controller to address zero.
     *  Changes holderSince to block.timestamp
     */
    ...
    /**
     *  @notice function to transfer NFT between own accounts.
     *  @dev does not change controller nor signer. Cannot be approved, only
     *  ownerOf(tokenId) can call it. Also doesn't change holderSince.
     */
        ...
        returns (string memory uri)
    {
        ...
        return ITraitsVault(tokenIdDNA[tokenId]).tokenURI(tokenId);
    }
        ...
        returns (address, uint256)
    {
        ...
        return
            ITraitsVault(tokenIdDNA[_tokenId]).royaltyInfo(
                _tokenId,
                _salePrice
            );
    }
