  ...
  Administrable
{
  ...
  mapping(uint256 => uint256) private burnCounter;
  ...
  function mint(uint256 tokenId, uint256 quantity) external payable noContracts {
    ...
    if (totalSupply(tokenId) + quantity > rule.maxSupply - rule.burnSupply) revert MaxSupplyReached();
  ...
  function airdropBatch(
    ...
    if (totalSupply(tokenId) + totalQuantity > rule.maxSupply - rule.burnSupply) revert MaxSupplyReached();
  ...
  function burnToken(address owner, uint256 tokenId) external onlyOperatorsAndOwner {
    ...
    if (!rule.exists) revert SaleRuleUndefined();
    if (!rule.burnActive) revert BurnNotActive();
    if (burnCounter[tokenId] >= saleRules[tokenId].burnSupply) revert BurnQuotaReached();
    if (balanceOf(owner, tokenId) < 1) revert BurnAmountExceedsBalance();
    if (tokenOwnerships[owner][tokenId].quantityBurned >= saleRules[tokenId].burnLimit) revert BurnLimitReached();

    tokenOwnerships[owner][tokenId].quantityBurned += 1;
    burnCounter[tokenId] += 1;

    _burn(owner, tokenId, 1);
  }

  function burnCount(uint256 tokenId) external view returns (uint256) {
    return burnCounter[tokenId];
  }
  ...
  function setSaleRules(
    ...
    if (rule.exists) {
      ...
      rule.burnLimit = burnLimit;
      rule.burnSupply = burnSupply;
