...
contract NFTCollection is ERC721, ERC721Enumerable {
    ...
    constructor() ERC721("MHJ Collection", "MHJ") {}
...
contract Marketplace is ReentrancyGuard {
    ...
    struct Auction {
        ...
        uint256 currentBidPrice; // Current highest bid of the auction
        uint256 endAuction; // Timestamp for the end day and time of the auction
    ...
    function createAuction(
        ...
        nftCollection = NFTCollection(_addressNFTCollection);
    ...
    function claimNFT(uint256 _auctionIndex) external {
        ...
        // Get NFT collection contract
        nftCollection = NFTCollection(auction.addressNFTCollection);
     ...
     * to withdraw his Balance when the auction is closed
     * When the Balance are withdrawn, the winned of the
    ...
    function refund(uint256 _auctionIndex) external {
        ...
        //Get Auction
        Auction storage auction = allAuctions[_auctionIndex];
        //Check if the caller is creator of the auction
        require(
            auction.creator == msg.sender,
            "Token by claimed only by the owner of the auction"
        );
        require(
            auction.currentBidOwner == address(0),
            "bider of the auction existing"
        );
        //Get NFT Collection contract
        nftCollection = NFTCollection(auction.addressNFTCollection);
        nftCollection.transferFrom(
    ...
    function makeOffer(uint256 _id, uint256 _price) public payable {
        require(
            msg.value == listingPrice,
            "Price must be equal to listing price"
        );
    ...
    function fillOffer(uint256 _offerId) public payable nonReentrant{
