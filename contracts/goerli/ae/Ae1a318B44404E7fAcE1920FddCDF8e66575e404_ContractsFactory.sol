 ...
 */
contract ContractsFactory is Whitelist, SystemContext {
    event CreatedContractInstance(
        bytes32 contractName,
        address contractAddress
    );

    ContractsFactory internal parentFactory;

    constructor(
        IRegistry contractRegistry,
        IAuthProvider authProvider,
        address parentFactory_
    ) SystemContext(contractRegistry, authProvider) {
        parentFactory = ContractsFactory(parentFactory_);
    }

    function whitelistContractChecksum(bytes32 checksum)
        external
        returns (bool)
    {
    ...
    function removeWhitelistedContractChecksum(bytes32 checksum)
        external
        returns (bool)
    {
        ...
        returns (bool)
    {
        ...
        return
            ...
            parentFactory.isChecksumWhitelisted(checksum);
    }
}
...
contract SystemContext {
    ...
    constructor(IRegistry registry_, IAuthProvider authProvider_) {
        _registry = registry_;
        _authProvider = authProvider_;
    }

    function _getContractAddress(bytes32 contractName_)
        internal
        view
        returns (address)
    {
        return _registry.resolver(contractName_);
    }
