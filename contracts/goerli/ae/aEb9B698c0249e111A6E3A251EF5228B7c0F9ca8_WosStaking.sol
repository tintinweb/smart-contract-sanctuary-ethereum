...
contract WosStaking is OnlyAuthorized, CollateralWrapper {
    ...
    uint256 private minBalance = 1 * 1e2;

    // Token amount variables
    mapping(address => uint256) public alreadyWithdrawn;
    mapping(address => uint256) public balances;
    mapping(address => uint256) public stakersIndex;
    ...
    event StakeMessage(string msg);
    event ERC20Change(address newAddress);
    ...
    function addStaker(address addr) private {
        if (stakersIndex[addr] <= 0) {
            //makes sure no duplicate is inserted
            stakers.push(addr);
            stakersIndex[addr] = stakers.length;
        }
    }

    function removeStaker(address addr) private {
        uint256 index = stakersIndex[addr] - 1;
        stakers[index] = stakers[stakers.length - 1];
        stakers.pop();
        stakersIndex[addr] = 0;
    }

    function setERC20Contract(address addr) onlyOwner public{
        require(
            addr != address(0),
            "_erc20_contract_address address can not be zero"
        );
        erc20Contract = IERC20(addr);
        emit ERC20Change(addr);
    }
    ...
    function recive() external onlyAuthorized {
        uint256 wagered = erc20Contract.balanceOf(address(this));
        uint256 availableCollateral = _collateralBalanceOf(address(this));

        if (availableCollateral < minBalance && wagered == 0) {
            emit StakeMessage(
                "There are not enough funds to make a distribution among stakers"
            );
            return;
        }
        ...
        for (uint256 index = 0; index < stakers.length; index++) {
            ...
            uint256 percentageDistribute = stakerBalance.mul(100).div(wagered);
            _collateralTransfer(
                staker,
                percentageDistribute.mul(availableCollateral).div(100)
            );
        }

        emit StakeMessage("Balances have been distributed to stakers");
    }
}
...
contract CollateralWrapper is Ownable {
    ...
    constructor() {
        collacteralAddress = 0x176BFF9918a4D6628188195b3def11F697cB5AB4;
    ...
    modifier validateBalance(uint256 price) {
        require(collacteral.balanceOf(address(this)) >= price, "you dont have enough balance");
        _;
    }
}
