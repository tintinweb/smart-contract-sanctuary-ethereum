...
import "../genesis/BatchOffsets.sol";
import "./OwnablePausable.sol";

contract SimpleNFT is ERC721, BatchOffsets, OwnablePausable {
  ...
  constructor() ERC721("GLASS", "GLASS") {
    _batchSize = 5000;
    offsets[1] = BatchOffset({
      seed: 1,
      within: 47,
      overall: 1
    });
    offsets[2] = BatchOffset({
      seed: 1,
      within: 2909,
      overall: 0
    });
  ...
  // limit function, to be overriden by the importing contract
  function limit() public pure override returns (uint256) {
    return 10000;
  }

  function mint(uint256 tokenId) public onlyOwner {
    require(totalSupply < limit(),"over limit");
    totalSupply += 1;
    _mint(msg.sender, tokenId);
  }

  function mintAmount(uint256 tokens) public whenNotPaused {
