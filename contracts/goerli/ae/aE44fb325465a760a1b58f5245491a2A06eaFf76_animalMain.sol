...
contract animalMain is  ERC721,Ownable {
  ...
  function createAnimal() public payable {
      ...
      //TO DO 
      //使用亂數來產生DNA, 星級, 動物種類
      dna = keccak256(abi.encodePacked(block.timestamp, block.difficulty, msg.sender));
      star = 0;
      for(uint i = 0; i < dna.length; i++) {
        star = star + uint(dna[i]) * (
            2 ** (
                8 * (
                    dna.length - (i + 1)
                    )
      ...
      star = star % 5 == 0 ? 1 : star % 5;
      roletype = roletype % 4 == 0 ? 1 : roletype % 4;
