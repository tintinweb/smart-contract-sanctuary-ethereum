...
contract SoulboundToken is ERC721 {
  struct TokenType {
    /// @dev Unique display name for the token type
    string tokenType;
    string metadataURI;
  }
  ...
  string metadataURI =
    "https://static.wikia.nocookie.net/sonicpokemon/images/b/bf/Charizard_AG_anime.png/revision/latest?cb=20130714192025";

  constructor() ERC721("Soulbound Test", "SBT") {}

  function addDescription(string memory _description) external {
    description = _description;
  }

  function getAllTokenTypes() external view returns (TokenType[] memory) {
    return tokenTypes;
  }
  ...
  function addTokenType(
    ...
    tokenTypes.push(TokenType(_tokenType, _metadataURI));
  }

  function addTokenTypes(TokenType[] calldata _tokenTypes) external {
    for (uint256 i = 0; i < _tokenTypes.length; i++) {
      tokenTypes.push(_tokenTypes[i]);
    }
  }

  uint256 _mintCount = 1;
  ...
  function _formatTokenType(
    TokenType memory _tokenType,
    uint256 _value,
    bool _addComma
  ) internal pure returns (bytes memory formattedTokenType) {
    formattedTokenType = abi.encodePacked(
      "{",
      _formatJsonField("trait_type", _tokenType.tokenType, true),
      _formatJsonField("value", _value.toString(), false),
      "}"
    );
  ...
  function _formatTokenTypesInMetadata()
    internal
    view
    returns (bytes memory attributes)
  {
    ...
    TokenType[] memory cachedTokenTypes = tokenTypes;

    for (uint256 i = 0; i < cachedTokenTypes.length; i++) {
      bytes memory formattedType = _formatTokenType(
        cachedTokenTypes[i],
        uint256(i),
        i != cachedTokenTypes.length - 1
      );
  ...
  function tokenURI(
    ...
    bytes memory dataURI = abi.encodePacked(
      ...
      metadataURI,
      '",',
      _formatJsonField("description", description, true),
      _formatTokenTypesInMetadata(),
      "}"
    );
...
interface ISoulboundToken is IERC165 {
  ...
  /// @notice TokenURI contains metadata for the individual token
