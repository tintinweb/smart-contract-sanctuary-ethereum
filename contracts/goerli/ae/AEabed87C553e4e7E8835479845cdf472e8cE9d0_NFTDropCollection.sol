  ...
  ERC721BurnableUpgradeable
{
   ...
   * @dev At collection creation the `creator` is given `admin` and `minter` roles by default.
   * This is the default royalty recipient if an different `paymentAddress` was not provided.
   */
   ...
   * @dev Notes:
   *   a) when collection isn't revealed, preRevealURIOrRevealedBaseURI is set to full path of the default image
   *      of the collection. e.g. `www.foodomain.com/my/default/pre/reveal/image.jpg`
   *   b) TokenId is naively concatenated with the preRevealURIOrRevealedBaseURI to obtain an NFT in the collection.
   *      e.g. The address for `tokenId`: '1' with `preRevealURIOrRevealedBaseURI`: "ipfs://foo/" is "ipfs://foo/1".
   */
  string public preRevealURIOrRevealedBaseURI;
  ...
  /**
   * @notice The address of the factory which was used to create this collection.
   */
  address public immutable collectionFactory;

  /**
   * @notice Default royalty cut paid out on secondary sales.
   * @dev Set to 10% of the secondary sale.
   */
   ...
   * @param preRevealURIOrRevealedBaseURI The preRevealURIOrRevealedBaseURI for the collection.
   ...
   * When both preRevealURIOrRevealedBaseURI and postRevealBaseURIHash are set,
   * preRevealURIOrRevealedBaseURI is the pre-reveal content and signals the collection has not been revealed yet.
   */
  event URIUpdated(string preRevealURIOrRevealedBaseURI, bytes32 postRevealBaseURIHash);
  ...
  constructor(address _collectionDropFactory) {
    require(_collectionDropFactory.isContract(), "NFTDropCollection: Factory is not a contract");
    collectionFactory = _collectionDropFactory;
  }
   ...
   * @param _preRevealURIOrRevealedBaseURI The full path to the pre-reveal image of the collection.
   ...
   *   c) For a collection without a reveal, set `_preRevealURIOrRevealedBaseURI`
  ...
  function initialize(
    ...
    string calldata _preRevealURIOrRevealedBaseURI,
    bytes32 _postRevealBaseURIHash,
    uint32 _maxTokenId,
    address _additionalMinter,
    address payable _paymentAddress
  ) external initializer {
    require(msg.sender == collectionFactory, "NFTDropCollection: Must be created via the factory");
    require(bytes(_symbol).length > 0, "NFTDropCollection: `_symbol` must be set");
    require(
      bytes(_preRevealURIOrRevealedBaseURI).length > 0,
      "NFTDropCollection: `_preRevealURIOrRevealedBaseURI` must be set"
    );
    ...
    if (_additionalMinter != address(0)) {
      _grantMinter(_additionalMinter);
    }
    ...
    preRevealURIOrRevealedBaseURI = _preRevealURIOrRevealedBaseURI;
   ...
   */
  function burn(uint256 tokenId) public virtual override(ERC721BurnableUpgradeable) onlyAdmin {
    super.burn(tokenId);
  }
   ...
   * @param marketToApprove The address of the contract to approve operator privileges
   * for the minted NFTS. If `address(0)` is passed in, no contracts are given operator role.
   */
  function mintCountFor(
    uint16 count,
    address to,
    address marketToApprove
  ) external onlyMinterOrAdmin returns (uint256 firstTokenId) {
    unchecked {
      // If the below overflows the attacker will run out of gas minting in the for loop below.
      require(latestTokenId + count <= maxTokenId, "NFTDropCollection: Exceeds max tokenId");
      firstTokenId = latestTokenId + 1;
      latestTokenId = latestTokenId + count;
      for (uint256 i = firstTokenId; i <= latestTokenId; ++i) {
        _mint(to, i);
      }
    }
    if (marketToApprove != address(0)) {
      _setApprovalForAll(to, marketToApprove, true);
    }
  }
   ...
   */
  function reveal(string memory _baseURI) external onlyAdmin {
    ...
    preRevealURIOrRevealedBaseURI = _baseURI;
    emit URIUpdated(preRevealURIOrRevealedBaseURI, postRevealBaseURIHash);
  }
  ...
  function selfDestruct() external onlyAdmin {
    require(totalSupply() == 0, "NFTDropCollection: Minted NFTs must be burned first");
   ...
   *         i) the unrevealed preRevealURIOrRevealedBaseURI has changed and the postRevealBaseURIHash needs to
   *            be updated (set `_postRevealBaseURIHash`)
   *         ii) pre-reveal URI needs to be updated (set `_preRevealURIOrRevealedBaseURI`)
   */
  function updatePreRevealContent(string memory _preRevealURIOrRevealedBaseURI, bytes32 _postRevealBaseURIHash)
    external
    onlyAdmin
  {
    require(postRevealBaseURIHash != bytes32(0), "NFTDropCollection: Already revealed");
    require(_postRevealBaseURIHash != bytes32(0), "NFTDropCollection: use `reveal` instead");
    // require preRevealURIOrRevealedBaseURI be set, just like in the initializer to avoid empty preReveal URIs.
    require(bytes(_preRevealURIOrRevealedBaseURI).length > 0, "NFTDropCollection: `_baseURI` must be set");
   ...
   * @dev Once this value has been set, it may be decreased but can't be increased if a mint has happened.
   */
  ...
  function _updateMaxTokenId(uint32 _maxTokenId) internal {
    require(_maxTokenId != 0, "NFTDropCollection: `_maxTokenId` may not be 0");
    require(_maxTokenId < maxTokenId || latestTokenId == 0, "NFTDropCollection: `_maxTokenId` may not increase");
    require(latestTokenId <= _maxTokenId, "NFTDropCollection: _maxTokenId must >= latestTokenId");
    maxTokenId = _maxTokenId;
  ...
  function supportsInterface(bytes4 interfaceId)
    public
    view
    override(ERC165Upgradeable, ERC721Upgradeable, AccessControlUpgradeable)
  ...
  function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
    ...
    if (postRevealBaseURIHash != bytes32(0)) {
      return preRevealURIOrRevealedBaseURI;
    }

    return string.concat(preRevealURIOrRevealedBaseURI, tokenId.toString());
  }
}
...
abstract contract MinterRole is Initializable, AccessControlUpgradeable, AdminRole {
  ...
  /**
   * @dev An internal helper to add minters while bypassing the role admin check.
   */
  function _grantMinter(address account) internal {
