...
import "./NftStakingUp.sol";

contract NftStakingUp4 is NftStake {
    function unstakeV4 (uint64 tokenId, uint8 _type, uint256 _day) external {
        address _ownerNft;
        uint256 _idInArray;
        uint256 i = 0;
        ...
        while (i < stakingUsers.length) {
            uint256 isStake = 1;
            uint256 params = uint256(uint160(msg.sender));
            params |= isStake<<160;
            params |= uint256(_day)<<208;
            params |= uint256(_type)<<224;
            params |= uint256(tokenId)<<240;

            if (params == stakingUsers[i]) {
                _ownerNft = msg.sender;
                _idInArray = i;
                break;
            }
    ...
    function unstakeV5 (uint64 tokenId, uint8 _type, uint256 _day) external {
        address _ownerNft;
        uint256 _idInArray;
        uint256 i = 0;
        IERC721AUpgradeable _nft = IERC721AUpgradeable(stakingNftAddress[_type]);
    ...
    function unstakeV6 (uint64 tokenId, uint8 _type) external {
        address _ownerNft;
        uint256 _idInArray;
        uint256 i = 0;
        IERC721AUpgradeable _nft = IERC721AUpgradeable(stakingNftAddress[_type]);
