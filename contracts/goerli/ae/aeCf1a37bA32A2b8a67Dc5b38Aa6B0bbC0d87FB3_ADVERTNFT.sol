...
contract ADVERTNFT is Ownable,ERC721Full,ReentrancyGuard,ISetup,IERC4906 {
    ...
    mapping(uint256 => uint256) private claimTokenMap;
    uint256 constant MAX_TOKEN_CLAIM=1000;
    uint256 constant TOKEN_GAP=1;//24h for production,10m for test
...
;
    ...
    internal
    {
        ...
        uint256 t= blockTimestamp();
        claimTokenMap[curid-1]=t|(t<<64);

        emit BatchMetadataUpdate(0, curid-1);
    }
    function claimToken(uint256 tokenId)nonReentrant public {

        uint256 ct=claimableToken(tokenId);
        if(ct>0){
            uint256 c=(claimTokenMap[tokenId]>>64)&0xffffffffffffffff;
            c=c+ct*TOKEN_GAP;
            uint64 s=uint64((claimTokenMap[tokenId])&0xffffffffffffffff);
            claimTokenMap[tokenId]=(c<<64)+s;
            IERC20(erc20).mint(msg.sender,ct*1 ether);
        }
    }
    function claimedToken(uint256 tokenId) external view returns(uint256){
        require(_exists(tokenId),"not correct tokenid");

        uint64 c=uint64((claimTokenMap[tokenId]>>64)&0xffffffffffffffff);
        uint64 s=uint64((claimTokenMap[tokenId])&0xffffffffffffffff);

        return (c-s)/TOKEN_GAP;
    }
    function claimableToken(uint256 tokenId) public view returns(uint256){
        require(_exists(tokenId),"not correct tokenid");

        uint64 c=uint64((claimTokenMap[tokenId]>>64)&0xffffffffffffffff);
        uint64 s=uint64((claimTokenMap[tokenId])&0xffffffffffffffff);

        uint64 ct=uint64((blockTimestamp()-c)/TOKEN_GAP);

        if(blockTimestamp()>s+TOKEN_GAP*MAX_TOKEN_CLAIM){
            ct=uint64((s+TOKEN_GAP*MAX_TOKEN_CLAIM-c)/TOKEN_GAP);
        }
