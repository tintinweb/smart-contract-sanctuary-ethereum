...
contract NftLog is ERC721AQueryable, Ownable, ReentrancyGuard {
    using Strings for uint256;

    string public uriPrefix = "";
    string public creatorMetadata = "";
    string public hiddenMetadataPublic ="";
    string public hiddenMetadataCreatorAccess = "";

    bytes32 public merkleRoot;
    mapping(address => bool) public whitelistClaimed;
    mapping(address => bool) public creatorAccessClaimed;

    uint256 public cost;
    uint256 public maxMintAmountPerTx;

    uint256 public constant MAX_SUPPLY = 50; // actual supply 5.555 + 88 NFTs
    ...
    constructor(
        ...
        string memory _hiddenMetadataCreatorAccessUri,
        string memory _hiddenMetadataPublicUri
    ) ERC721A(_tokenName, _tokenSymbol) {
        setCost(_cost);
        setMaxMintAmountPerTx(_maxMintAmountPerTx);
        hiddenMetadataCreatorAccessURI(_hiddenMetadataCreatorAccessUri);
        hiddenMetadataPublicURI(_hiddenMetadataPublicUri);
    }
     ...
     */
    /**
     * @dev for creator access that free minting
     * have same process like whitelist minting
     */
    function creatorAccessMint(bytes32[] calldata _merkleProof)
        public
        nonReentrant
    {
        require(totalSupply() + 1 <= 88, "Max supply exceeded!");
        require(
            _isOnList(_msgSender(), _merkleProof, merkleRoot),
            "Invalid proof"
        );
        require(!creatorAccessClaimed[_msgSender()], "Address already claimed");
        creatorAccessClaimed[_msgSender()] = true;
        ...
        returns (string memory)
    {
        ...
        uint256 maxTokenCreatorAccess = 25; // this is for testing, the actual value is 88
        string memory hiddenMetadata = hiddenMetadataPublic;
        string memory currentBaseURI = _baseURI();
        if (_tokenId < maxTokenCreatorAccess) {
            hiddenMetadata = hiddenMetadataCreatorAccess;
            currentBaseURI = creatorMetadata;
        }
        if (revealed == false) {
     ...
     */
    function hiddenMetadataCreatorAccessURI(string memory _hiddenMetadataUri)
        public
        onlyOwner
    {
        hiddenMetadataCreatorAccess = _hiddenMetadataUri;
    }

    function hiddenMetadataPublicURI(string memory _hiddenMetadataUri)
        public
        onlyOwner
    {
        hiddenMetadataPublic = _hiddenMetadataUri;
    }
    ...
    function setCreatorAccessMetadata(string memory _metadataCreator)
        public
        onlyOwner
    {
        creatorMetadata = _metadataCreator;
    }
