...
contract BrawlerBearzStake is FxBaseRootTunnel, Ownable, IBrawlerBearzStake {
     ...
     * @param amount of XP
     */
    function addXP(uint256 tokenId, uint256 amount) external onlyOwner {
     ...
     * @param amount of XP
     */
    function subtractXP(uint256 tokenId, uint256 amount) external onlyOwner {
    ...
    function _sendChildMessage(
        ...
        //        _sendMessageToChild(abi.encode(from, tokenIds, isInbound));
    }

    function _processMessageFromChild(bytes memory message) internal override {
        (uint256[] memory tokenIds, uint256[] memory amounts, bool isAdd) = abi
            .decode(message, (uint256[], uint256[], bool));
        if (isAdd) addXPBatch(tokenIds, amounts);
        else subtractXPBatch(tokenIds, amounts);
    }
}
