...
contract WagerRegistry is IWagerRegistry {
    ...
    function settleWager(uint256 wagerId) external override {
        ...
        (bool sent, ) = payable(winner).call{
            value: (wager.wagerAmount * 2), gas: 3600
        }("");
        require(sent, "W11");
    ...
    function voidWager(uint256 wagerId) external override {
        ...
        (bool sent, ) = payable(partyOne).call{value: wager.wagerAmount, gas: 3600}("");
        require(sent, "W6");
        if (partyTwo != address(0)) {
            (bool sentTwo, ) = payable(partyTwo).call{value: wager.wagerAmount, gas: 3600}(
                ""
            );
            require(sentTwo, "W6");
        }
