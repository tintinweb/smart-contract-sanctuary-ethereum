...
contract TestMOPassNFT is ERC721A, Ownable {
    using Strings for uint256;

    uint256 public constant MAX_SUPPLY = 10981;

    address public mopMetaDataRenderAddress;

    uint256 internal randomNonce;

    uint256[MAX_SUPPLY] internal indices;

    mapping(uint256 => bytes) public blockLevelsBytes;
    ...
    mapping(uint256 => bytes) public passBlockLevels;

    event Minted(address indexed sender, uint256 amount, uint256 quantity);

    constructor() ERC721A("MOPassNFT", "MOPN") {}

    function setImage(address mopMetaDataRenderAddress_) external onlyOwner {
        require(_isContract(mopMetaDataRenderAddress_), "Invalid address");
        mopMetaDataRenderAddress = mopMetaDataRenderAddress_;
    }
    ...
    function _setTokenIndex(uint256 id) internal {
        ...
        _buildBlockLevels(_index);
    }

    function _buildBlockLevels(uint256 index) internal {
        require(blockLevelsBytes[index].length == 0, "Cannot be set");
        bytes memory _blockLevelsBytes =
            HexGridsMath.buildBlockLevels(keccak256(abi.encodePacked(address(this), index)));

        blockLevelsBytes[index] = _blockLevelsBytes;
    }

    function _randomIndex() internal returns (uint256) {
        uint256 totalSize = MAX_SUPPLY - randomNonce;
        uint256 index =
            uint256(keccak256(abi.encodePacked(randomNonce, msg.sender, block.difficulty, block.timestamp))) % totalSize;
        uint256 value = 0;
        if (indices[index] != 0) {
            value = indices[index];
        } else {
    ...
    function tokenURI(uint256 id) public view override returns (string memory tokenuri) {
        ...
        if (mopMetaDataRenderAddress != address(0)) {
            IMOPassMetaDataRender metaDataRender = IMOPassMetaDataRender(mopMetaDataRenderAddress);
            tokenuri = metaDataRender.constructTokenURI(tokenIdToIndex[id], blockLevelsBytes[tokenIdToIndex[id]]);
        }
    }
}
