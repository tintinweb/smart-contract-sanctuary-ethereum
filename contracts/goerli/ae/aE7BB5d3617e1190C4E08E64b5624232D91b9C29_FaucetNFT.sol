...
contract FaucetNFT is ERC721A, Ownable, ReentrancyGuard  {
    using Strings for uint256;

    uint256 public immutable amountForOwner = 200;
    uint256 public maxMintSupply = 200;
    ...
    constructor(string memory contractName, string memory ticker, string memory _baseURI, uint256 _maxSupply) ERC721A(contractName, ticker) {
        baseURI = _baseURI;
    ...
    function ownerMint(uint256 quantity) external onlyOwner {
        ...
        require(
            ownerMinted + quantity <= amountForOwner,
