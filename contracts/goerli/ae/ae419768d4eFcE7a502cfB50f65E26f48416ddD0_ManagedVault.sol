...
contract ManagedVault is Initializable, OwnableUpgradeable, ERC20Upgradeable, IManagedVault {
     ...
     *         Used for locking redeemed tokens, unless the redemption is cancelled.
     */
    mapping(address => uint256) public locks;

    // Redemption settings
    uint256 public redemptionFeeBPS;
    IERC20 public nextRedemptionToken; // The token that will be used for the next redemption epoch. Must be among the deposit tokens.
    ...
    /**
     * @notice Updates the amount of tokens to be redeemed by the caller in the current redemption epoch. Redeemed tokens
     *         are withdrawable using withdraw(address account, uint256 redemptionEpochId) when the next redemption epoch begins
     *         and the settlement for the redemption epoch in which redeem(uint256 amount) was called is finalized.
     *
     * @dev Can be called more than once during the same redemption epoch to override amount to redeem for that redemption epoch.
     *      The amount that will be redeemed is locked in and can no longer be modified for the same redemption epoch
     *      when the owner starts a new redemption epoch by calling rollRedemptionEpoch() where consecutive redeem() calls will only
     *      modify amounts for the new redemption epoch.
     *
    ...
    /**
     * @notice Withdraws tokens corresponding to the amount of vault tokens that were redeemed for an epoch and
    ...
    function withdraw(address account, uint256 redemptionEpochId) whenNotPaused external {
        ...
        require(redemptionEpoch.state == RedemptionEpochState.Settled, "Settlement for given epoch has not been completed yet.");
     ...
     *
     * @dev The native price must be set regardless of its deposit status. The token prices should be set in the order which each were whitelisted in the vault contract. 
     ...
     * @param vaultPriceAtTheBlock New price of the vault's token.
     * @param nativePriceAtTheBlock New price of the native cryptocurrency(Ether for the Ethereum network).
    ...
    function setPrices(uint256 blockNumber, uint256 vaultPriceAtTheBlock, uint256 nativePriceAtTheBlock, IERC20[] calldata tokenAddresses, uint256[] calldata tokenPricesAtTheBlock) external onlyOwner {
        ...
        // Dilluting the vault a little and updating vaultPrice accordingly
    ...
    */
     ...
     *      allowance from the owner. Otherwise it will revert with TransferFailed().
    */
    function settleRedemptionEpoch(uint256 redemptionEpochId) external onlyOwner {
        ...
        // Performance fee logic
        // Audit: Is the order of withdrawableVaultTokenTotalAmount (therefore totalSupply()) reads and writes appropriate?
        performanceFeeExit += redemptionEpoch.vaultTokenTotalAmount  * netVaultPrice();
        if (_performanceFeePnL() < 0) {
            // Audit: Is use of totalSupply() is appropriate?
    ...
    function accruePerformanceFee(uint256 performanceFeePeriodCount) external onlyOwner {
        // Audit: Should we only allow the call of this function once per performanceFeePeriodCount?
        ...
        uint256 accuredPerformanceFee = _accruablePerformanceFee() / netVaultPrice();
        performanceFeeAccruedVaultTokenAmount += accuredPerformanceFee;
        if (_performanceFeePnL() < 0) {
            // Only non-negative int256s could be converted to uint256 and there won't be any exceptions below.
        ...
        emit AccruePerformanceFee(performanceFeePeriodCount, accuredPerformanceFee);
    }
     ...
     *
     * @dev If state is true, the token is allowed for deposits. Otherwise depositToken function reverts.
     ...
     *      If the account `redeem`s tokens, they are locked and these tokens could be unlocked until `rollEpoch`. After
     *      `rollEpoch`, these tokens cannot be unlocked anymore and not included in the balance. They can only be withdrawn.
     */
    ...
    /**
     * @dev Returns the amount of free vault tokens in existence. The definition of free here is unlocked or unlockable.
     *      The redeemed tokens in previous redemptionEpochs cannot be unlockable, only to be redeemable. Therefore, they're not included
    ...
    function netVaultPrice() public view returns (uint256) {
        if (0 < _performanceFeePnL()) {
            // precisions needs to be taken into account
    ...
    // For updating the management fee on setPrices
