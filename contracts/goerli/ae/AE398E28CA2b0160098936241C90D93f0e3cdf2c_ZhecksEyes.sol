    ...
    DefaultOperatorFilterer
{
    ...
    uint256 public whitelistMintPrice = 0.0016 ether;
    uint256 public maxMintPerAccount = 30;
    uint256 public maxWhitelistMintPerAccount = 15;
    uint256 public maxFreeMintPerAccount = 3;
    ...
    mapping(address => uint256) private _totalFreeMintPerAccount;
    ...
    constructor() ERC721A("ZhecksEyes", "ZHECKSEYES") EIP712("Zhecks - Eyes Edition", "1.0.0") {}
        ...
        payable
    {
        ...
        require(
            totalWhitelistMint + amount <= maxWhitelistSupply,
            "WL spots sold out"
        );
        require(
            amount + _totalWhitelistMintPerAccount[msg.sender] <=
                maxWhitelistMintPerAccount,
            "Max WL mints per account reached"
        );
        require(
        ...
        /////MINT FREE CONDITION
        if (_totalFreeMintPerAccount[msg.sender] < maxFreeMintPerAccount) {
            uint256 freeAllowed = maxFreeMintPerAccount - _totalFreeMintPerAccount[msg.sender];
             
             if (amount >= freeAllowed) {
                require(msg.value >= (amount * whitelistMintPrice) - (freeAllowed * whitelistMintPrice), "Insufficient funds!");
                _totalFreeMintPerAccount[msg.sender] += freeAllowed;
                } else {
                require(msg.value >= 0, "Invalid value!");
                _totalFreeMintPerAccount[msg.sender] += amount;
                }
        } else {
    ...
    function hasMintedUsingWhitelist(address account)
        ...
        returns (bool)
    {
        return _totalMintPerAccount[account] >= maxMintPerAccount;
    }
    ...
    function _hash(address account) private view returns (bytes32) {
        return
            _hashTypedDataV4(
                keccak256(
                    abi.encode(keccak256("ZhecksEyes(address account)"), account)
                )
            );
    }
