...
contract Vault {
    ...
    IConnextHandler public theConnextHandler;
    ...
    constructor(address aRouter, address aConnextHandler) {
        theRouter = aRouter;
        theConnextHandler = IConnextHandler(aConnextHandler);
    } 
    ...
    function deposit(uint256 aAmount) external payable {

        ERC20(theTestToken).transferFrom(msg.sender, address(this), aAmount);

        theConnextHandler.addRouterLiquidityFor(
            aAmount, 
            theTestToken, 
            theRouter
        );
