...
//            LiquidationBurnRewardsProto(lbrContract).disburseMigrationReward(tokenId, msgSender);
    ...
    function _transfer(
        ...
        ERC721TopLevelProto(msg.sender).emitTransfer(from, to, tokenId+1); // TODO: Remove after testing
...
//        }
        ...
        ERC721TopLevelProto(msg.sender).emitTransfer(from, to, tokenId+2); // TODO: Remove after testing

        _approve(address(0), tokenId, prevOwnership);

        ERC721TopLevelProto(msg.sender).emitTransfer(from, to, tokenId+2+16); // TODO: Remove after testing

        if (active) {
            ERC721TopLevelProto(msg.sender).emitTransfer(from, to, tokenId+2+32); // TODO: Remove after testing
            _balances[from].activeBalance -= 1;
        }
        else {
            ERC721TopLevelProto(msg.sender).emitTransfer(from, to, tokenId+2+48); // TODO: Remove after testing
            _balances[from].inactiveBalance -= 1;
        }

        ERC721TopLevelProto(msg.sender).emitTransfer(from, to, tokenId+2+64); // TODO: Remove after testing

        _tokenData[tokenId].owner = to;

        ERC721TopLevelProto(msg.sender).emitTransfer(from, to, tokenId+3); // TODO: Remove after testing
        ...
        ERC721TopLevelProto(msg.sender).emitTransfer(from, to, tokenId+4); // TODO: Remove after testing
        ...
        ERC721TopLevelProto(msg.sender).emitTransfer(from, to, tokenId+5); // TODO: Remove after testing
