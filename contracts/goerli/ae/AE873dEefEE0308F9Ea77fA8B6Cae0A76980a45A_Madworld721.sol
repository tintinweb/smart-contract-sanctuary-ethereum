 ...
 */
contract Madworld721 is ERC721Upgradeable, OwnableUpgradeable {
    using SafeMath for uint256;
    ...
    function mintAndTransfer(
        ...
        if(_exists(_tokenId)) {
            ...
            address _proxy = address(ProxyRegistry(proxyRegistryAddress).proxies(owner()));
            require(
            ProxyRegistry(proxyRegistryAddress).contracts(_msgSender()) ||
                _msgSender() == owner() || _msgSender() == _proxy,
            "ERC721Tradable::sender is not owner or approved!"
            );
