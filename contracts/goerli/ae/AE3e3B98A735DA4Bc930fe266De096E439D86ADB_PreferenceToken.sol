...
/// @dev Company (Pool) Preference Token
contract PreferenceToken is
    Initializable,
    OwnableUpgradeable,
    IPreferenceToken,
    ERC20CappedUpgradeable
{
    ...
    ///@dev Preference token description, allows up to 5000 characters
     ...
     * @param pool_ Pool
     * @param preferenceTGE Token symbol
     * @param cap_ Token cap
     */
    function initialize(address pool_, address preferenceTGE, uint256 cap_)
        external
        override
        initializer
    {
    ...
    function decimals()
        public
        pure
        override(ERC20Upgradeable, IPreferenceToken)
        returns (uint8)
    {
        return 18;
    }
    ...
    function cap()
        public
        view
        override(IPreferenceToken, ERC20CappedUpgradeable)
        returns (uint256)
    {
        return super.cap();
    }
    ...
    function _transfer(
        ...
        address to,
        uint256 amount
    ) internal override whenPoolNotPaused whenLockupReached {
    ...
    modifier whenLockupReached() {
        require(ITGE(tge).lockupTVLReached(), ExceptionsLibrary.SERVICE_PAUSED);
        _;
    }
