...
contract VehicleResume is ERC721, ERC721Enumerable, ERC721URIStorage, Ownable {
    using Counters for Counters.Counter;

    Counters.Counter private _tokenIdCounter;

    //bytes32 constant key_token_vin = "vin";
    bytes32 constant key_token_ivid = "ivid";
    bytes32 constant key_token_hash = "hash";
    bytes32 constant key_token_base64 = "base64";
    bytes32 constant key_token_make = "make";
    bytes32 constant key_token_model = "model";
    ...
    constructor() ERC721("VehicleResumeV2", "IDVR2") {}
    ...
    function safeMintWithMetadata(
        ...
        _setValue(tokenId, key_token_name, abi.encode(ivid));

        _setValue(
            tokenId,
            key_token_description,
            abi.encode(string(abi.encodePacked("NFT for IVID", " ", ivid)))
        );
        ...
        //hash base64
        //string memory calculatedHash = string(abi.encode(keccak256(abi.encodePacked(base64))));

        _setValue(tokenId, key_token_base64, abi.encode(base64));
        _setValue(tokenId, key_token_ivid, abi.encode(ivid));
        _setValue(tokenId, key_token_hash, abi.encode(hash));
        _setValue(
            tokenId,
            key_token_external_url,
            abi.encode(
                string(
                    abi.encodePacked(
                        "https://vrpoc.romeroej.me/svg/",
        ...
        trait_types[0] = abi.encode("base64");
        trait_types[1] = abi.encode("ivid");
        trait_types[2] = abi.encode("hash");
        trait_types[3] = abi.encode("make");
        trait_types[4] = abi.encode("model");
        trait_values[0] = abi.encode(base64);
        trait_values[1] = abi.encode(ivid);
        trait_values[2] = abi.encode(hash);
        trait_values[3] = abi.encode(make);
        trait_values[4] = abi.encode(model);
        trait_display[0] = abi.encode("string");
        trait_display[1] = abi.encode("string");
        trait_display[2] = abi.encode("string");
        trait_display[3] = abi.encode("string");
        trait_display[4] = abi.encode("string");
        ...
        // Set Token URI
        _setTokenURI(tokenId, tokenURIBuild(tokenId));
    }
    ...
    function getHash(uint256 tokenId) public view returns (string memory) {
        ...
        return string(abi.decode(trait_value[2], (string)));
    }

    function getIvid(uint256 tokenId) public view returns (string memory) {
        bytes[] memory trait_value = _getValues(
            tokenId,
            key_token_attributes_trait_value
        );
        return string(abi.decode(trait_value[1], (string)));
    }

    function getBase64(uint256 tokenId) public view returns (string memory) {
        bytes[] memory trait_value = _getValues(
            tokenId,
            key_token_attributes_trait_value
        );
        return string(abi.decode(trait_value[0], (string)));
    }
    ...
    function updateToken(
        ...
        if (compareStrings(hash, getHash(tokenId))) {
            return;
        }
        ...
        string memory ivid = string(
            abi.decode(_getValue(tokenId, key_token_ivid), (string))
        );
    ...
    function tokenURIBuild(uint256 tokenId)
        internal
        view
        returns (string memory)
    {
        ...
        string memory image = string(
            abi.decode(_getValue(tokenId, key_token_image), (string))
        );
