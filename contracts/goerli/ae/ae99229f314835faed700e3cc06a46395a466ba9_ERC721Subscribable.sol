  ...
  Teams
{
  ...
  */
  ...
  function _safeMint(address to, uint256 quantity, uint256 timeUnits, bool isAdminMint) internal {
    _safeMint(to, quantity, timeUnits, isAdminMint, "");
  }
  ...
  function extendOwnership(uint256 tokenId, uint256 _extendByTimeUnits) public payable {
    require(_exists(tokenId), "Token does not exist");
    require(_extendByTimeUnits > 0, "Cannot extend by zero time.");
    require(msg.value == getPrice(1, _extendByTimeUnits), "You have not paid enough to extend this expiry by that timeframe.");
    ...
    // set to. New + added time.
    bool expired = ownership.expiryTimestamp < block.timestamp;
    require(!expired, "Cannot extend ownership of an expired token. It must be minted again via reclaim mint to become active.");
  ...
  uint256 public baseSubscriptionTime = 86400; // 1 day
  uint256 public pricePerTimeUnit = 0 ether;

  function setUnitPrice(uint256 _feeInWei) public onlyTeamOrOwner {
    pricePerTimeUnit = _feeInWei;
  }

  // Sets the base time unit that expiry time is calculated against. eg = 86400 => 1 day
  function setSubscriptionTimeUnit(uint256 _secondsPerUnit) public onlyTeamOrOwner {
    require(_secondsPerUnit > 0, "Cannot set baseSubscriptionTime to zero units");
  ...
  function getPrice(uint256 _qty, uint256 _timeUnits) public view returns(uint256) {
    require(_qty > 0, "You must be purchasing at least one token.");
    require(_timeUnits > 0, "Time unit duration cannot be zero.");
    return (pricePerTimeUnit * _timeUnits) * _qty;
  }

  function calcExpiry(uint256 _timeUnits) public view returns(uint256) {
    require(_timeUnits > 0, "Cannot calc time against 0 time");
...
contract ERC721Subscribable is
  Ownable,
  Teams,
  ERC721A,
  Initializable,
  ...
  ReentrancyGuard
{
  ...
  function initialize(
    ...
    mintingOpen = mintStatus;
    baseSubscriptionTime = initSubTime;
    pricePerTimeUnit = initPrice;

    Ownable._transferOwnership(_owner);
    ERC721A._init(tokenName, tokenSymbol, _batchSize, _collectionSize);
  }

  uint8 public immutable CONTRACT_VERSION = 2;
  ...
  */
  function mintToAdmin(address _to, uint256 _timeUnits) public onlyTeamOrOwner{
    if (currentTokenId() != collectionSize) {
      ...
      _safeMint(_to, 1, _timeUnits, true);
      return;
    }
  ...
  /**
  * @dev Mints single tokens to an address.
  * fee may or may not be required
  * @param _to address of the future owner of the token
  * @param _timeUnits time units being paid for (days, mins, months..)
  */
  function mintTo(address _to, uint256 _timeUnits) public payable {
    ...
    require(_timeUnits >= 1, "Duration of subscription must be at least 1.");
    require(mintingOpen == true, "Minting is not open right now!");
    require(msg.value == getPrice(1, _timeUnits), "Value below required mint fee for amount");

    // need to determine if we should be minting from primary supply vs reclaiming a dead mint
    if (currentTokenId() != collectionSize) {
      require(currentTokenId() + 1 <= collectionSize, "Cannot mint over supply cap.");
      _safeMint(_to, 1, _timeUnits, false);
      return;
    }
    _reclaimMint(_to, _timeUnits);
  }
