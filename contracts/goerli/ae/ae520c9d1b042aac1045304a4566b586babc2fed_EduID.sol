...
contract EduID {
    ...
    function addStudent(uint256 _id , string memory _name, string memory _class) public {
        _student[_id].student_name = _name;
        _student[_id].student_identification = _id;
        _student[_id].class = _class;
        _student[_id].timeStamp = block.timestamp;
    }

    function addGrade(uint256 _id, string memory _subject, string memory grade) public {
        _getstudentgrade[_id].push(Grades(_subject , grade, block.timestamp));
    }
