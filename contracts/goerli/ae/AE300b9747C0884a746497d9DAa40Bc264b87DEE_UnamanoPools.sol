...
contract UnamanoPools is Ownable,ReentrancyGuard {
    ...
    event ProjectUnlockTime(uint256 indexed pid, uint256 unlockTime); // 修改项目方解锁时间
    ...
    function addPool(uint256 _lockDays, uint256 _candyPerBlock,
        ...
        if(_lockDays > 0) {
           _unlockTime = _lockDays.mul(secondsPerDay).add(block.timestamp);  //锁定的时间，单位秒 1d=86400 1h=3600
        }
