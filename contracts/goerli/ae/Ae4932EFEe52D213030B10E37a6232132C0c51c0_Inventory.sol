...
contract Inventory is IInventory, Ownable {
  ...
  // Mapping tokenID to total supply available to mint
  mapping(uint256 => uint256) public override totalSupply;

  // Map token ID to amount minted
  mapping(uint256 => uint256) public override tokensMinted;

  // Mapping tokenId to burn count
  mapping(uint256 => uint256) public override imptBurnCount;

  // Mapping token ID to burn amount sent to thallo
  mapping(uint256 => uint256) public override toBeBurned;

  // Mapping token ID to burns confirmed by thallo
  mapping(uint256 => uint256) public override confirmedBurned;
  ...
  constructor(InventoryConstructorParams memory _params) {
  ...
  function updateTotalMinted(
    ...
    uint256 alreadyMinted = tokensMinted[_tokenId];
    if (_amount + alreadyMinted > totalSupply[_tokenId]) {
      revert NotEnoughSupply();
    }
    tokensMinted[_tokenId] += _amount;
  ...
  function updateTotalSupply(
    uint256 _tokenId,
    uint256 _amount
  ) external override onlyStableWallet {
    totalSupply[_tokenId] = _amount;
    tokensMinted[_tokenId] = 0;
    emit TotalSupplyUpdated(_tokenId, _amount);
  }
  ...
  function incrementBurnCount(
    ...
    imptBurnCount[_tokenId] += _amount;
    emit BurnCountUpdated(_tokenId, _amount);
  }

  function sendBurnCounts(
    uint256 _tokenId,
    uint256 _amount
  ) external override onlyStableWallet {
    if (imptBurnCount[_tokenId] <= 0) {
    ...
    imptBurnCount[_tokenId] -= _amount;
    toBeBurned[_tokenId] += _amount;
    emit BurnSent(_tokenId, _amount);
  }

  function confirmBurned(
    uint256 _tokenId,
    uint256 _amount
  ) external override onlyStableWallet {
    uint256 burnCount = toBeBurned[_tokenId];
    if (_amount > burnCount) {
      revert TotalMismatch();
    }
    toBeBurned[_tokenId] -= _amount;
    confirmedBurned[_tokenId] += _amount;
    emit BurnConfirmed(_tokenId, _amount);
  }
}
