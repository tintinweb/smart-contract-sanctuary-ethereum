...
import "Ownable.sol";
import "CoboSubSafeModule.sol";

contract CoboSubSafeModuleFactory is TransferOwnable, Pausable {
    string public constant NAME = "Cobo SubSafeModule Factory";
    ...
    /// @dev The factory contract to create CoboSubSafeModule.
    ...
    /// @dev It is compatible with previous implementation. The factory contract to create CoboSubSafeModule
    ...
    {
        _module = createSubSafeModuleWithNonce(_safe, modulesSize());
    }

    /// @notice Create the module with nonce and subSafeFactory
    /// @dev The factory contract to create CoboSubSafeModule by using provided nonce and subSafeFactory.
    ///      If _subSafeFactory is zero address, the default is used.
    /// @param _safe the Gnosis Safe (GnosisSafeProxy) instance's address
    /// @param _nonce as the seed of salt
    function createSubSafeModuleWithNonce(address payable _safe, uint256 _nonce)
        public
        whenNotPaused
        returns (address _module)
    {
    ...
    /// @notice Set the SubSafeModule implementation address
...
contract CoboSubSafeModule is CoboSafeModule {
    /// @dev the subSafe factory to work with
    address public subSafeFactory;

    /// @notice Initializer function for CoboSubSafeModule
    ...
    function execTransactionsBySubSafe(address payable subSafe,
        ...
        onlyDelegate
    {
        ...
        for (uint256 i = 0; i < toList.length; i++) {
            require(_hasPermission(_msgSender(), toList[i], valueList[i], dataList[i]), "permission denied");
            bytes memory data = abi.encodeWithSignature('execTransaction(address,uint256,bytes)', toList[i], valueList[i], dataList[i]);
            ...
            emit ExecTransaction(subSafe, valueList[i], Enum.Operation.Call, data, _msgSender());
        }
    }
...
/// @dev This contract implements the core data structure and its related features.
contract CoboSafeModule is OwnableUpgradeable, UUPSUpgradeable {
...
contract CoboSubSafeFactory is Ownable {
    using EnumerableSet for EnumerableSet.AddressSet;
