...
contract aaa is ERC721A, ERC2981, Ownable {
  ...
  uint256 internal constant _presaleSupply = 2;
  uint256 internal _preSalePrice = 0.0001 ether;
  ...
  function presale(uint256 quantity) external payable {
    ...
    if (!existsInAllowList(msg.sender)) revert NotExistInArrowList();
   ...
   */
  function existsInAllowList(address user) public view returns (bool) {
    bool result = false;
    for (uint256 i = 0; i < _allowList.length; i++) {
      if (_allowList[i] == user) {
        result = true;
        break;
      }
    }
