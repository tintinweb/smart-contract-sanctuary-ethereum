...
/**
 * @title DomainExchange
 * @notice It is the core contract of the Domain exchange.
 */
contract DomainExchange is IDomainExchange, ReentrancyGuard, Ownable {
    ...
    // Limit (if enforced for fee referrer in percentage (10,000 = 100%)
    uint256 public referrerFeeLimit;
    ...
    event NewReferrerFeeLimit(uint256 indexed referrerFeeLimit);
    event NewRoyaltyFeeManager(address indexed royaltyFeeManager);
    event NewTransferSelectorNFT(address indexed transferSelectorNFT);

    event ProtocolPayment(
     ...
     * @param _referrerFeeLimit referrer fee limit  (2,000 = 20%, 8,000 = 80%)
     */
    constructor(
        ...
        uint256 _referrerFeeLimit
    ) {
        // Calculate the domain separator
        DOMAIN_SEPARATOR = keccak256(
            abi.encode(
                0x8b73c3c69bb8fe3d512ecc4cf759cc79239f7b179b0ffacaa9a75d522b39400f, // keccak256("EIP712Domain(string name,string version,uint256 chainId,address verifyingContract)")
                0x34fc0beb92b74e37a1a5118b5d0c31171593f02ed9f83b27897fffb8d6c81e34, // keccak256("DomainExchange")
        ...
        referrerFeeLimit = _referrerFeeLimit;
    }
    ...
    function matchAskWithTakerBid(
        ...
        _matchAskWithTakerBid(takerBid, makerAsk);
    }

    /**
     * @notice Batch match ask with a taker bid order using ERC20 or ETH/WETH
     * @param takerBidArray taker bid order array
     * @param makerAskArray maker ask order array
     */
    function batchMatchAskWithTakerBidUsingERC20OrETHAndWETH(
        OrderTypes.TakerOrder[] calldata takerBidArray,
        OrderTypes.MakerOrder[] calldata makerAskArray
        ...
        uint256 length = makerAskArray.length;
        for (uint _i = 0; _i < length;) {
            OrderTypes.TakerOrder calldata takerBid = takerBidArray[_i];
            OrderTypes.MakerOrder calldata makerAsk = makerAskArray[_i];
            if (makerAsk.currency == WETH) {
                _matchAskWithTakerBidUsingETHAndWETH(takerBid, makerAsk);
            } else {
                _matchAskWithTakerBid(takerBid, makerAsk);
            }
        unchecked{++_i;}
     ...
     * @param makerAsk maker ask order
     */
    function _matchAskWithTakerBidUsingETHAndWETH(
        OrderTypes.TakerOrder calldata takerBid,
        OrderTypes.MakerOrder calldata makerAsk
    ) private {
        require((makerAsk.isOrderAsk) && (!takerBid.isOrderAsk), "Order: Wrong sides");
        ...
        require(makerAsk.price <= IWETH(WETH).balanceOf(address(this)), "Order: WETH insufficient fund");
    ...
    /**
     * @notice Update referrer fee limit
     * @param _referrerFeeLimit new referrer fee limit  (2,000 = 20%, 8,000 = 80%)
     */
    function updateReferrerFeeLimit(uint256 _referrerFeeLimit) external onlyOwner {
        require(_referrerFeeLimit <= 10000, "Referrer Fee: referrer fee can't exceed 10000");
        referrerFeeLimit = _referrerFeeLimit;
        emit NewReferrerFeeLimit(_referrerFeeLimit);
    }
     ...
     * @param referrerFeeRecipient referrer fee recipient
    ...
    function _transferFeesAndFunds(
        ...
        // 1. Protocol fee
        {
            ...
            if ((referrerFeeRecipient != address(0) || protocolFeeRecipient != address(0)) && (protocolFeeAmount != 0)) {
                // referrer fee
                if (referrerFeeRecipient != address(0)) {
                    if (protocolFeeRecipient != address(0)) {
                        uint256 referrerFee = _calculateReferrerFee(protocolFeeAmount);
                        if (referrerFee > 0) {
                            // referrer fee
                            IERC20(currency).safeTransferFrom(from, referrerFeeRecipient, referrerFee);
                            emit ProtocolPayment(collection, tokenId, referrerFeeRecipient, curator, currency, referrerFee);
                        }
                        // protocol fee
                        IERC20(currency).safeTransferFrom(from, protocolFeeRecipient, protocolFeeAmount - referrerFee);
                        emit ProtocolPayment(collection, tokenId, protocolFeeRecipient, curator, currency, protocolFeeAmount - referrerFee);
                    } else {
                        // protocol fee
                        IERC20(currency).safeTransferFrom(from, referrerFeeRecipient, protocolFeeAmount);
                        emit ProtocolPayment(collection, tokenId, referrerFeeRecipient, curator, currency, protocolFeeAmount);
                    }
                } else {
                    IERC20(currency).safeTransferFrom(from, protocolFeeRecipient, protocolFeeAmount);
                    emit ProtocolPayment(collection, tokenId, protocolFeeRecipient, curator, currency, protocolFeeAmount);
                }
        ...
        // 2. Royalty fee
        {
            ...
            if ((royaltyFeeRecipient != address(0)) && (royaltyFeeAmount != 0)) {
                ...
                emit RoyaltyPayment(collection, tokenId, royaltyFeeRecipient, curator, currency, royaltyFeeAmount);
            }
        }
    ...
    function _transferFeesAndFundsWithWETH(
            ...
            if ((referrerFeeRecipient != address(0) || protocolFeeRecipient != address(0)) && (protocolFeeAmount != 0)) {
                ...
                if (referrerFeeRecipient != address(0)) {
                    if (protocolFeeRecipient != address(0)) {
                        ...
                        if (referrerFee > 0) {
                            // referrer fee
                            IERC20(WETH).safeTransfer(referrerFeeRecipient, referrerFee);
                            emit ProtocolPayment(collection, tokenId, referrerFeeRecipient, curator, WETH, referrerFee);
                        }
                        // protocol fee
                        IERC20(WETH).safeTransfer(protocolFeeRecipient, protocolFeeAmount - referrerFee);
                        emit ProtocolPayment(collection, tokenId, protocolFeeRecipient, curator, WETH, protocolFeeAmount - referrerFee);
                    } else {
                        // protocol fee
                        IERC20(WETH).safeTransfer(referrerFeeRecipient, protocolFeeAmount);
                        emit ProtocolPayment(collection, tokenId, referrerFeeRecipient, curator, WETH, protocolFeeAmount);
                    }
                } else {
                    IERC20(WETH).safeTransfer(protocolFeeRecipient, protocolFeeAmount);
                    emit ProtocolPayment(collection, tokenId, protocolFeeRecipient, curator, WETH, protocolFeeAmount);
                }
                finalSellerAmount -= protocolFeeAmount;
            }
        }
        ...
        // 2. Royalty fee
        {
            ...
            if ((royaltyFeeRecipient != address(0)) && (royaltyFeeAmount != 0)) {
                ...
                emit RoyaltyPayment(collection, tokenId, royaltyFeeRecipient, curator, WETH, royaltyFeeAmount);
            }
        }
    ...
    /**
     * @notice Calculate referrer fee for limit
     * @param protocolFeeAmount protocol fee amount
     */
    function _calculateReferrerFee(uint256 protocolFeeAmount) internal view returns (uint256) {
        return (referrerFeeLimit * protocolFeeAmount) / 10000;
    }
