...
contract LnbLending is Math, Initializable, OwnableUpgradeable, UUPSUpgradeable {
    ...
    mapping(address => int104) public userBasic;
    ...
    function balanceOf(address account) public view returns (uint256) {
        ...
        int104 principal = userBasic[account];
    ...
    /**
     * @notice Withdraw an amount of base from the protocol
    ...
    function withdrawTo(address to, uint amount) external {
        withdrawInternal(msg.sender, to, amount);

    }

    function withdrawInternal(address src, address dst, uint amount) internal {
        require(dst != address(0),'invalid address');
        uint balanceOfUser = balanceOf(src);
        require(balanceOfUser > 0,'insufficient amount');

        if (amount == type(uint256).max) {
            amount = balanceOfUser;
        }
        require(amount <= balanceOfUser,"invalid amount requested");

        Comet(cometAddress).withdrawTo(dst, baseToken, amount);

        int104 srcPrincipal = userBasic[src];
        int256 srcBalance = presentValue(srcPrincipal) - signed256(amount);
        int104 srcPrincipalNew = principalValue(srcBalance);

        userBasic[src] = srcPrincipalNew;

        emit Withdraw(dst, amount);
    }
    ...
    function supply(uint amount) external {
        require(
            ERC20(baseToken).allowance(msg.sender, address(this)) >= amount,
        ...
        // approve
        ERC20(baseToken).approve(cometAddress, amount);

        // repaying
        Comet(cometAddress).supply(baseToken, amount);

        int104 dstPrincipal = userBasic[msg.sender];
        int256 dstBalance = presentValue(dstPrincipal) + signed256(amount);
        int104 dstPrincipalNew = principalValue(dstBalance);

        userBasic[msg.sender] = dstPrincipalNew;
