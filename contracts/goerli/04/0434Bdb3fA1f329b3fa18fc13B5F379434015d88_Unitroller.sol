 ...
 * ZKTokens should reference this contract as their comptroller.
 */
...
/**
 * @title zkFinance's ZKToken Contract
 * @notice Abstract base for ZKTokens
 * @author zkFinance
 */
abstract contract ZKToken is ZKTokenInterface, ExponentialNoError, TokenErrorReporter {
    /**
    ...
    function transferTokens(address spender, address src, address dst, uint tokens) internal returns (uint) {
        ...
        uint srzkTokensNew = accountTokens[src] - tokens;
        ...
        accountTokens[src] = srzkTokensNew;
    ...
    /**
     * @notice Returns the current per-block borrow interest rate for this zkToken
    ...
    /**
     * @notice Returns the current per-block supply interest rate for this zkToken
    ...
    /**
     * @notice Calculates the exchange rate from the underlying to the ZKToken
    ...
    /**
     * @notice Get cash balance of this zkToken in the underlying asset
    ...
    /**
     * @notice User supplies assets into the market and receives zkTokens in exchange
    ...
    function mintFresh(address minter, uint mintAmount) internal {
         ...
         *  Note: The zkToken must handle variations between ERC-20 and ETH underlying.
         *  `doTransferIn` reverts if anything goes wrong, since we can't be sure if
         *  side-effects occurred. The function returns the amount actually transferred,
         *  in case of a fee. On success, the zkToken holds an additional `actualMintAmount`
         *  of cash.
         */
        uint actualMintAmount = doTransferIn(minter, mintAmount);

        /*
         * We get the current exchange rate and calculate the number of zkTokens to be minted:
        ...
        /*
         * We calculate the new total supply of zkTokens and minter token balance, checking for overflow:
     ...
     * @dev Accrues interest whether or not the operation succeeds, unless reverted
     * @param redeemAmount The amount of underlying to receive from redeeming zkTokens
     */
    ...
    /**
     * @notice User redeems zkTokens in exchange for the underlying asset
     * @dev Assumes interest has already been accrued up to the current block
     * @param redeemer The address of the account which is redeeming the tokens
     * @param redeemTokensIn The number of zkTokens to redeem into underlying (only one of redeemTokensIn or redeemAmountIn may be non-zero)
     * @param redeemAmountIn The number of underlying tokens to receive from redeeming zkTokens (only one of redeemTokensIn or redeemAmountIn may be non-zero)
     */
    function redeemFresh(address payable redeemer, uint redeemTokensIn, uint redeemAmountIn) internal {
         ...
         *  Note: The zkToken must handle variations between ERC-20 and ETH underlying.
         *  On success, the zkToken has redeemAmount less of cash.
    ...
    function repayBorrowFresh(address payer, address borrower, uint repayAmount) internal returns (uint) {
         ...
         *  Note: The zkToken must handle variations between ERC-20 and ETH underlying.
         *  On success, the zkToken holds an additional repayAmount of cash.
     ...
     */
    function liquidateBorrowInternal(address borrower, uint repayAmount, ZKTokenInterface zkTokenCollateral) internal nonReentrant {
        accrueInterest();

        uint error = zkTokenCollateral.accrueInterest();
        if (error != NO_ERROR) {
        ...
        liquidateBorrowFresh(msg.sender, borrower, repayAmount, zkTokenCollateral);
    }
     ...
     * @param repayAmount The amount of the underlying borrowed asset to repay
     */
    function liquidateBorrowFresh(address liquidator, address borrower, uint repayAmount, ZKTokenInterface zkTokenCollateral) internal {
        /* Fail if liquidate not allowed */
        uint allowed = comptroller.liquidateBorrowAllowed(address(this), address(zkTokenCollateral), liquidator, borrower, repayAmount);
        if (allowed != 0) {
        ...
        /* Verify zkTokenCollateral market's block number equals current block number */
        if (zkTokenCollateral.accrualBlockNumber() != getBlockNumber()) {
        ...
        (uint amountSeizeError, uint seizeTokens) = comptroller.liquidateCalculateSeizeTokens(address(this), address(zkTokenCollateral), actualRepayAmount);
        require(amountSeizeError == NO_ERROR, "LIQUIDATE_COMPTROLLER_CALCULATE_AMOUNT_SEIZE_FAILED");

        /* Revert if borrower collateral token balance < seizeTokens */
        require(zkTokenCollateral.balanceOf(borrower) >= seizeTokens, "LIQUIDATE_SEIZE_TOO_MUCH");

        // If this is also the collateral, run seizeInternal to avoid re-entrancy, otherwise make an external call
        if (address(zkTokenCollateral) == address(this)) {
            seizeInternal(address(this), liquidator, borrower, seizeTokens);
        } else {
            require(zkTokenCollateral.seize(liquidator, borrower, seizeTokens) == NO_ERROR, "token seizure failed");
        }

        /* We emit a LiquidateBorrow event */
        emit LiquidateBorrow(liquidator, borrower, actualRepayAmount, address(zkTokenCollateral), seizeTokens);
    }

    /**
     * @notice Transfers collateral tokens (this market) to the liquidator.
     * @dev Will fail unless called by another zkToken during the process of liquidation.
     *  Its absolutely critical to use msg.sender as the borrowed zkToken and not a parameter.
     * @param liquidator The account receiving seized collateral
     * @param borrower The account having collateral seized
     * @param seizeTokens The number of zkTokens to seize
     * @return uint 0=success, otherwise a failure (see ErrorReporter.sol for details)
     */
    ...
    /**
     * @notice Transfers collateral tokens (this market) to the liquidator.
     * @dev Called only during an in-kind liquidation, or by liquidateBorrow during the liquidation of another ZKToken.
     *  Its absolutely critical to use msg.sender as the seizer zkToken and not a parameter.
     * @param seizerToken The contract seizing the collateral (i.e. borrowed zkToken)
     * @param liquidator The account receiving seized collateral
     * @param borrower The account having collateral seized
     * @param seizeTokens The number of zkTokens to seize
     */
    ...
    function _addReservesFresh(uint addAmount) internal returns (uint, uint) {
         ...
         *  Note: The zkToken must handle variations between ERC-20 and ETH underlying.
         *  On success, the zkToken holds an additional addAmount of cash.
         *  doTransferIn reverts if anything goes wrong, since we can't be sure if side effects occurred.
         *  it returns the amount actually transferred, in case of a fee.
         */
...
abstract contract PriceOracle {
    ...
    /**
      ...
      * @param zkToken The zkToken to get the underlying price of
      * @return The underlying asset price mantissa (scaled by 1e18).
      *  Zero means the price is unavailable.
      */
    function getUnderlyingPrice(ZKToken zkToken) virtual external view returns (uint);
}
