...
contract MergeCanvas is IMergeCanvas {
        ...
        SufficientBid(_x, _y)
    {
        ...
        if (old_price != 0 && old_price + MIN_BID_AMOUNT > msg.value) {
            revert MergeCanvas_InsufficientBid(msg.value, old_price);
        }
        ...
        if (msg.value > 0) {
            (bool sent, ) = old_owner.call{value: msg.value}("");
            if (!sent) {
        ...
        NotAtMaxPixelCapacity
    {
        ...
        for (uint i = 0; i < _x.length; i++) {
            ...
            // at last for CIE pattern
            if (_prices[i] > 0) {
                (bool sent, ) = old_owner.call{value: _prices[i]}("");
                if (!sent) {
                    revert MergeCanvas_FailedBidRefund();
                }
            }
