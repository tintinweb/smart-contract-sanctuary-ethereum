...
interface PreSale {
    ...
    // mapping(uint256 => tokenBuyer) public tokenBuyerInfo;
     function getAllBuyersInfo() external view returns (tokenBuyer[] memory);
}
...
contract StakingContract {
    ...
    // function sendPreSaleTokens() external onlyOwner view returns({

    //     uint256 length = preSale.getAllBuyersInfo().length;
    // }
    ...
    function sellGenThroughDashboard(uint256 _busdAmount, uint256 priceOfGen) public {
        ...
        if (saleEnabled && (block.timestamp < saleEndTime)) {
            ...
            emit genSellInfo(msg.sender, amountAdd, _busdAmount);
    ...
    function enableTheBonus(bool _enable, uint256 _endingTime) public onlyOwner returns(uint256){
        saleEnabled = _enable;
        saleEndTime = _endingTime;
        return block.timestamp;
    }

    function setSellGenPercentage(uint256 _percentage) public onlyOwner {
