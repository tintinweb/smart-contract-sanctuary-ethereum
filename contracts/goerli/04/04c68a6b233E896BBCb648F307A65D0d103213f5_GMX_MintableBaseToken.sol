...
import "./interfaces/GMX_IYieldTracker.sol";
import "./interfaces/GMX_IBaseToken.sol";

contract GMX_BaseToken is GMX_IERC20, GMX_IBaseToken {
    ...
    function recoverClaim(address _account, address _receiver) external onlyAdmin {
        for (uint256 i = 0; i < yieldTrackers.length; i++) {
            ...
            GMX_IYieldTracker(yieldTracker).claim(_account, _receiver);
        }
    }
    ...
    function claim(address _receiver) external {
        for (uint256 i = 0; i < yieldTrackers.length; i++) {
            address yieldTracker = yieldTrackers[i];
            GMX_IYieldTracker(yieldTracker).claim(msg.sender, _receiver);
        }
    }
    ...
    function _updateRewards(address _account) private {
        for (uint256 i = 0; i < yieldTrackers.length; i++) {
            address yieldTracker = yieldTrackers[i];
            GMX_IYieldTracker(yieldTracker).updateRewards(_account);
        }
    }
}
