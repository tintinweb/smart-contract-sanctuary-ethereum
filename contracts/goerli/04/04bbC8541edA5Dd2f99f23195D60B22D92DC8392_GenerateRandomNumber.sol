...
contract GenerateRandomNumber is VRFV2WrapperConsumerBase {
    ...
    mapping(uint256 => ChainLinkRandomNumber) private randomNumbers;

    uint256 internal fee; // fee given to the oracle for fulfilling the request (LINK tokens)
    ...
    // Using the Goerli Testnet contract address
    // 1. LINK Token contract address
    // 2. VRF Wrapper contract address
    constructor() VRFV2WrapperConsumerBase(linkToken, vrfWrapper) {
        fee = 0.25 * 10**18; // 0.25 LINK is the flat fee in the docs
    }

    function generateRandomNumberWithBlockData(uint256 max)
        external
        view
        returns (uint256)
    {
    ...
    function generateRandomNumberWithChainLink() public returns (uint256) {
        ...
        uint256 requestId = requestRandomness(callbackGasLimit, 1, 1);
    ...
    // Callback function used by VRF Coordinator after the random number is generated
    ...
    // Get the random number generated for a specific request ID
    function getRandomNumberForRequestID(uint256 requestId)
        external
        view
        returns (uint256)
    {
        return randomNumbers[requestId].number;
    }
}