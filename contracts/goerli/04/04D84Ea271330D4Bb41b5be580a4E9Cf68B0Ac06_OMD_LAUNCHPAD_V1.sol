    ...
    OwnableUpgradeable
{
    ...
    function getSwapPair(bytes32 symbol) external view returns (address) {
        return swapTokens[symbol];
    }
    ...
    function swapToken(bytes32 symbol, uint256 _amount) external {
        require(
            _amount >= 1 * 10 ** 6,
            "Amount must be greater than or equal to 1 token."
        );
        IERC20Burnable tokenFrom = IERC20Burnable(whitelistedTokens[symbol]);
        bool success1 = tokenFrom.burnFrom(_msgSender(), _amount);
        require(
            success1,
            "Transfer failed! Please approve amount tokenFrom for this contract."
        );
        IERC20Upgradeable tokenTo = IERC20Upgradeable(
            swapTokens[swapPairs[symbol]]
        );
        IERC20Decimals tokenToDecimals = IERC20Decimals(
            swapTokens[swapPairs[symbol]]
        );
        uint8 tokenToDecimal = tokenToDecimals.decimals();
        uint256 amount = (((_amount *
            swapTokensPrice[symbol][swapPairs[symbol]]) / 10 ** 6) *
