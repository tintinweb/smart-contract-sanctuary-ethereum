...
contract XOXTreasury is Initializable {
    ...
    function setReward(
        ...
        require(levelReward_.length == rewardByLevel_.length, "XOX Treasury: ");
        require(levelReward_.length > 0, "XOX Treasury: ");
        levelReward = levelReward_;
        rewardByLevel = rewardByLevel_;
    ...
    function swapUSDtoXOX(
        ...
        if (block.number > buyer.lastBLockUpdated) {
            uint256 blocks = block.number.sub(buyer.lastBLockUpdated);
            uint256 totalComp = buyer.amount.add(buyer.reward);
            uint256 rewardPending = totalComp
                .mul(blocks)
                .add(apy)
                .div(BASE_APY)
                .div(TOTAL_BLOCK_IN_YEAR);
    ...
    function claimReferralByLevel(uint256 level) external {
        require(level < levelLength, "XOX Treasry: not fount point");
