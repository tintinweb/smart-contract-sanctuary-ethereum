...
/// @title BSGGPlay
/// @author flextra19
/// @notice Utility game contract for BSGG
contract BSGGPlay {
    ...
    // total amount of USDT invested during the entire Game
    ...
    address public USDT;
    address public aBSGG;

    event Initialized(address executor, uint256 at);
    event OwnershipTransferred(address oldOwner, address newOwner);
    event FeeOwnerChanged(address oldFeeOwner, address newFeeOwner);
    ...
    /// @notice initialize variables by owner
    /// @param _feeOwner fee owner address
    /// @param _usdt USDT token address
    /// @param _absgg aBSGG token address
    function initialize(
        address _feeOwner,
        address _usdt,
        address _absgg
        ...
        require(_usdt != address(0), "USDT address should be non-zero one");
        require(_absgg != address(0), "aBSGG address should be non-zero one");
    ...
    /// @notice proceed a order
    /// @return bool true if order is proceed, otherwise false
    function proceedOrder() internal returns (OrderStatus) {
        Order storage order = orders[0];
        Round storage round = rounds[currentRoundID - 1];
        uint256 rewardAmount = order.amount.mul(order.price).div(1e18);
        ...
        while (orderTempPrice < order.price) {
            orderTempPrice = orderTempPrice.add(priceRisingAmount);
            estimateOrderCompletedTime = estimateOrderCompletedTime.add(
                (priceRisingPeriod)
            );
        }
        if (estimateOrderCompletedTime <= block.timestamp) {
            UserInfo storage user = users[order.account];
            if (rewardAmount < round.fund) {
                ...
                for (uint256 nI = 0; nI < user.orderLists.length; nI++) {
                    if (user.orderLists[nI] == order.id) {
                        user.orderLists[nI] = user.orderLists[
                            user.orderLists.length - 1
                        ];
                        user.orderLists.pop();
                        break;
                    }
                ...
                // update order info
                order.amount = order.amount.sub(
                    round.fund.mul(1e18).div(order.price)
                );
                round.fund = 0;
                ...
                snapshots.push(
                    Snapshot({
                        user: order.account,
                        amount: round.fund.mul(1e18).div(order.price),
                        price: order.price,
                        time: estimateOrderCompletedTime,
                        orderID: order.id,
                        roundID: currentRoundID
                ...
                return OrderStatus.SUB_COMPLETED;
            }
        }

        return OrderStatus.UNCOMPLETED;
    }

    /// @notice proceed orders through order list
    function update() public {
        while (orders.length > 0) {
            // proceed order
            if (proceedOrder() == OrderStatus.COMPLETED) {
                // remove completed order from arrary
                for (uint256 nI = 0; nI < orders.length - 1; nI++) {
                    orders[nI] = orders[nI + 1];
                }
                orders.pop();
                continue;
            } else if (proceedOrder() == OrderStatus.SUB_COMPLETED) {
                continue;
            } else {
                break;
            }
    ...
    /// @notice sort order list by ordered price and time
    ...
    function newRound() internal {
        ...
        rounds.push(
            Round({
                ...
                endTime: 0,
                fund: totalAmount.mul(roundFundRate).div(10000)
            })
        );
    ...
    /// @notice deposit USDT on contract by owner
    ...
    function invest(uint256 _amount) public onlyOwner {
        ...
        totalInvestedAmount = totalInvestedAmount.add(_amount);
        emit Invested(_amount);
    }

    /// @notice withdraw USDT from contract by users
    /// @param _amount USDT amount to withdraw
    function withdraw(uint256 _amount) public {
        ...
        if (_amount > 0) {
            if (withdrawFee > 0) {
                ...
                IERC20(USDT).safeTransfer(msg.sender, _amount.sub(feeAmount));
    ...
    function createOrder(uint256 _amount, uint256 _price) public {
        ...
        IERC20(aBSGG).safeTransferFrom(msg.sender, address(this), _amount);
    ...
    /// @notice update existing order
    /// @param _orderID id of order to udpate
    /// @param _price new order price
    function updateOrder(uint256 _orderID, uint256 _price) public {
        require(_orderID < lastOrderID, "Order id exceed");
        ...
        require(isValid, "Users could update only their uncompleted order");
    ...
    /// @notice set game rules by owner
    /// @param _rate fund rate of each round
    /// @param _priceRisingPeriod time to next price update
    /// @param _priceRisingAmount raised price in every price update
    /// @param _basePrice start price of each round
    function setGameDetails(
        ...
        require(
            _priceRisingPeriod != 0,
            "Price rising up period should be non-zero value"
        );
        require(
            _priceRisingAmount != 0,
            "Price rising up amount should be non-zero value"
        );
    ...
    /// @notice get the specific user info
    /// @param _account user address
    /// @return users[_account] structured user info (UserInfo)
    function getUserInfo(address _account)
        public
        view
        returns (UserInfo memory)
    {
        require(_account != address(0), "User address should be non-zero one");

        return users[_account];
    }

    /// @notice get the specific round info
    /// @param _id round id
    /// @return rounds[index] structured round info (Round)
    function getRoundInfo(uint256 _id) public view returns (Round memory) {
        require(_id <= currentRoundID, "Round number exceed");

        uint256 length = rounds.length;
        uint256 index;
        for (index = 0; index < length; index++) {
            if (rounds[index].id == _id) {
                break;
            }
        }
    ...
    /// @notice get the specific order info by index
    /// @param _index index of order list
    /// @return orders[_index] structured Order info (Order)
    function getOrderInfoByIndex(uint256 _index)
        public
        view
        returns (Order memory)
    {
        require(_index < orders.length, "Order index exceed");

        return orders[_index];
    }

    /// @notice set withdraw fee by fee owner
    /// @param _withdrawFee witdraw fee
    function setFee(uint256 _withdrawFee) public onlyFeeOwner {
        require(_withdrawFee < 3000, "Withdraw fee should be less than 3000"); // original 10000
    ...
    function TransferOwnership(address _newOwner) public onlyOwner {
        require(_newOwner != address(0), "New owner be non-zero address");

        emit OwnershipTransferred(owner, _newOwner);
        owner = _newOwner;
    }

    /// @notice update fee owner by current one
    /// @param _feeOwner new fee owner address
