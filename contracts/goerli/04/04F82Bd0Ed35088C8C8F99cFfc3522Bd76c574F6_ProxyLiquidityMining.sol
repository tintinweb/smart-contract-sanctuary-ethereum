...
contract ProxyLiquidityMining is LiquidityMiningLogic, ReentrancyGuard {
    ...
    ) LiquidityMiningLogic(_DFI, _rewardSpeed) {
        ...
        returns (uint256 amountDFI, uint256 amountsecondToken)
    {
        ...
        // from my research, the only way for _claimRewards function to fail is that recipient is a zero address 
        // (DFI.safetransfer cannot transfer to zero address, can checkout DFI address on ethereummainnet to see)
        // sending a msg from zero address is virtually impossible
        // and even if someone has zero address, they can not add liquidity (DFI.safetransferfrom in function addLiquidity/addliquidityeth does not allow transfer from a zero address)
        // making zero address stake in this contract = 0 --> its rewardAccrued is always 0
        // with that, _claimRewards function always return when it comes to the line rewardsAccrued[recipient] == 0 --> making no effect and don't fail
...
contract LiquidityMiningLogic is AccessControl {
     ...
     * @param recipient the recipient of the reward
     */
    // Should we consider putting nonReentrant for this later?
    // The calling of balanceOf and transfer functions of DFI only
    // interacts with DFI contract internal state
     ...
     */
    function enterNextStage() external onlyRole(DEFAULT_ADMIN_ROLE) {
        _updateRewardIndex();
    ...
    function checkReward(address recipient) external view returns(uint256){
        ...
        uint256 recipientDelta = (stakingMap[recipient] * deltaIndex) / 1e18;
        uint256 totalReward = rewardAccrueds[recipient] + recipientDelta;
        return totalReward;
    }
}
