...
contract BlurFarmer is TinyERC721, Ownable, ReentrancyGuard {
    uint256 public constant INITIAL_MINT_PRICE = 0.01e18;
    // uint256 public constant INITIAL_MINT_PRICE = 1e18;
    uint256 public constant MINT_PRICE_INCREMENT = 0.001e18;
    ...
    constructor(string memory _baseURI)
        TinyERC721("Blur Farmer", "BLURFARM", 0)
        Ownable()
    {
        baseURI = _baseURI;
    }
    ...
    function mint(uint256 quantity) public payable nonReentrant {
        ...
        require(msg.value >= totalMintCost, "Send enough ETH");
        uint256 excessEth = msg.value - totalMintCost;
        if (excessEth > 0) {
            bool sent = payable(msg.sender).send(excessEth);
            require(sent);
        }
    ...
    function burn(uint256[] memory tokenIds) public nonReentrant {
        ...
        bool sent = payable(msg.sender).send(
            getRedeemableValuePerToken() * numTokens
        );
        require(sent);
    ...
    {
        uint256 currentTokenId = super.totalSupply();
    ...
    function totalSupply() public view override returns (uint256) {
        return super.totalSupply() - burnCounter;
    }
...
contract TinyERC721 is Context, ERC165, IERC721, IERC721Metadata {
  ...
  function _transfer(
    ...
    // if (to == address(0)) revert TransferToZeroAddress(); NB: DISABLED
