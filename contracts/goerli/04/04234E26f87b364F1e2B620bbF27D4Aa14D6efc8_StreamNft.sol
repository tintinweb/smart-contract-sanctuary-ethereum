...
contract StreamNft is NftPilarofEducation {
    ...
    //mapping(address => bool) public closeFriends;
    mapping(address => mapping(uint256 => bool)) public closeFriends;

    mapping(address => mapping(uint256 => bool)) public likedUsers;
    
    mapping(uint256 => uint256) public likes;

    mapping(uint256 => uint256) public closeFriendCount;
    ...
    function makeCloseFriend(address to, uint256 _id) onlyOwner external {
        Video storage _video = videos[_id];
        closeFriends[to][_id] = true;
        closeFriendCount[_id] = closeFriendCount[_id] + 1;
        _video.closefriend.push(to); 
    ...
    function likeVideo(uint256 _id) public {
        ...
        _video.likedUsers.push(msg.sender);    //_myfriends.closeFriends()
    ...
    function mintNft(uint256 value, address closeFriend, uint256 vidId) public {
        require(closeFriends[closeFriend][vidId], "sorry we arent that close");
