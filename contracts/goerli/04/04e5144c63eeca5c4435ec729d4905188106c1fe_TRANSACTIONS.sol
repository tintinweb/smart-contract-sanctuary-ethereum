...
contract TRANSACTIONS is ERC721, DefaultOperatorFilterer, Ownable {
    ...
    function airdropToBlurVictims(address[] memory _recipients) public onlyOwner {
        ...
        for(uint256 i = 2001; i <= 2009; i++){
            _safeMint(_recipients[i], i);
        }
    ...
    function adminFinishPublicMintExceptOne() public onlyOwner {
        for(uint256 i=(pendingsSupply+amountMinted+1); i<=nominalMaxSupply-1; i++){
            _safeMint(msg.sender, i);
        }
    }

    function adminMintIdTo(address _recipient, uint256 _id) public onlyOwner {
