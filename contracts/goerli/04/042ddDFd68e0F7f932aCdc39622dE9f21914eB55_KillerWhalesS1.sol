...
contract KillerWhalesS1 is Ownable {
    ...
    /**
     * @dev The max episodeID
     */
    uint256 private constant MAX_EPISODE_ID = 6;
     ...
     * @dev Assumptions:
     *     - There are only 7 episodes in Season 1 therefore the bitmap can never overflow
     * Examples:
    ...
    constructor(address _helloToken, address _signer) {
        ...
        if (_signer == address(0)) _revert(SignerCannotBeZeroAddress.selector);
        discountSigner = _signer;
    }
     ...
     * @param signature the signature signed by `discountSigner`
     */
    function purchaseEpisodeWithDiscount(
        ...
        // check signature
        _checkDiscountSignature(_discount, _expirationTimestamp, signature);
     ...
     * @param _signer Address of the discount signer
     */
    function setDiscountSigner(address _signer) external onlyOwner {
        if (_signer == address(0)) _revert(SignerCannotBeZeroAddress.selector);
        discountSigner = _signer;
    }
    ...
    function ownsEpisode(address account, uint256 episodeId) external view returns (bool) {
        if (episodeId > MAX_EPISODE_ID) _revert(EpisodeDoesNotExist.selector);
        return (episodePurchasedBitmap[account] & (1 << episodeId)) != 0;
    }
     ...
     */
    function _checkDiscountSignature(uint256 _discount, uint256 _expirationTimestamp, bytes memory signature)
        internal
        view
    {
        bytes32 hash =
            keccak256(abi.encodePacked(msg.sender, _discount, block.chainid, address(this), _expirationTimestamp));

        if (block.timestamp > _expirationTimestamp) _revert(VoucherExpired.selector);
        if (hash.toEthSignedMessageHash().recover(signature) != discountSigner) _revert(InvalidSignature.selector);
    }
    ...
    function _grantEpisodes(address user, uint256[] memory episodeIds) internal {
        ...
        for (uint256 i; i < episodeIds.length;) {
            ...
            if ((existingEpisodeBitmap & shiftedEpisodeId) != 0) _revert(EpisodeAlreadyPurchased.selector);
            existingEpisodeBitmap |= shiftedEpisodeId;
            unchecked {
                ++i;
            }
        }
        episodePurchasedBitmap[user] = existingEpisodeBitmap;
    }
}
