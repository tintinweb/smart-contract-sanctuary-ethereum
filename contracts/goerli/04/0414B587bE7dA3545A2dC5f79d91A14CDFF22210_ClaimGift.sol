...
contract ClaimGift is Ownable, IERC1155Receiver, ReentrancyGuard, IERC721Receiver, GiftIndexManager {
    ...
    address public giftHolderWallet;
    ...
    function addERC1155GiftTokensToSmartContract(
        ...
        if (GiftTokenContract.balanceOf(giftHolderWallet, _tokenId) < _amount) {
            revert InsufficientGiftTokenAmount();
        }

        if (!GiftTokenContract.isApprovedForAll(giftHolderWallet, address(this))) {
    ...
    ///@param _tokenId Gift tokenId
    ///@param _collectionAddress Gift collection contract address
    function addERC712GiftTokensToSmartContract(uint256 _tokenId, address _collectionAddress) public onlyOwner {
    ...
    function rescueGiftToken(uint256 _giftIndex) external onlyOwner {
    ...
    function sendGift(uint256 _claimRequestId, uint256 _randomSeed) noContracts public {
        ...
        for (uint256 index = 0; index < claimedAmount[_claimRequestId]; index = _uncheckedInc(index)) {
            ...
            if (giftTokens[giftTokenIndex].giftCollectionContractType == uint256(GiftContractType.ERC_1155)) {
                ...
                GiftTokenContract.transferFrom(giftHolderWallet, giftTokens[giftTokenIndex].receiverWallet, giftTokens[giftTokenIndex].giftTokenId);
            }
