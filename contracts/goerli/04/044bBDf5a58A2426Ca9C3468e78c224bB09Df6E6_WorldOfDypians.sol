...
contract WorldOfDypians is ERC721A, Ownable {
    ...
    constructor(string memory name, string memory symbol, uint256 maxNftSupply, uint256 saleStart, address _cawsContractAddress, address _stakeContractAddress) ERC721A(name, symbol, maxLandPurchase, maxNftSupply) {
        ...
        stakeContract = StakeContract(stakeContractAddress);
...
//
    function mintWodGenesisWithDiscount(uint numberOfTokens,  uint256[] memory _tokenID) public payable {
        // require(cawsContract.balanceOf(msg.sender) >= numberOfTokens, "You do not have enough CAWS NFTs")
        ...
        if (!usedCAWS[_tokenID[i]]) 
        continue;
        if (stakeContract.calculateReward(msg.sender, _tokenID[i]) > 0) {
        usedCAWS[_tokenID[i]] = true;
        j++;
         } 
         if (cawsContract.ownerOf(_tokenID[i]) == msg.sender) {
          usedCAWS[_tokenID[i]] = true;
         j++;
          }
         }
                   require(LandPriceDiscount.mul(j) <= msg.value, "Ether value sent is not correct");
        }
        else{
        require(landPrice.mul(numberOfTokens) <= msg.value, "Ether value sent is not correct");
        }
