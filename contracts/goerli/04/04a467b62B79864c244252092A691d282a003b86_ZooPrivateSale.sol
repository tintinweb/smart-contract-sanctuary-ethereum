...
// File: contracts/ZooPrivateSale.sol
...
contract ZooPrivateSale is AccessControl {
    ...
    function buyWithUSDT(uint256 _amount) external {
        ...
        require(
            whitelist[_msgSender()] ||
                IERC20(zoo).balanceOf(_msgSender()) + _amount <= maxLimit,
            "Above Max Limit"
        );
        // check if the contract received fee
        require(
            IERC20(usdt).allowance(_msgSender(), address(this)) >=
                ((fee * _amount) / 100),
            "USDT Not Approved"
        );
        require(
        ...
        returns (string memory)
    {
        if (whitelist[_user] == true && _amount >= minLimit) {
            ...
            if (
                _amount >= minLimit &&
                IERC20(zoo).balanceOf(_user) + _amount <= maxLimit
            ) {
                return "Mintable";
            } else {
                return "Not Mintable";
