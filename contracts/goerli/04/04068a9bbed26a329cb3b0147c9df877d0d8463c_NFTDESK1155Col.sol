 ...
 * _Available since v3.1._
 */
abstract contract ERC1155Burnable is
    ERC1155,
    Ownable
{
...
interface NFTDESK1155 {
    function getServiceFee() external view returns (uint256);
    function getServiceFeeESC() external view returns (uint256);
    function getOwner() external view returns (address);
}

contract NFTDESK1155Col is
    ERC1155,
    Ownable,
    ERC1155Burnable
{
    ...
    NFTDESK1155 public  NFTDESK1155M;
    ...
    ) ERC1155("", contractname, contractSymbol) { 
        _tid = id;
        deci = 18;
        whitelistAdmin = _whitelistAdmin;
        whitelistedUser = false;
        adminContract = originalContract;
        // transferOw?nership(msg.sender);

        NFTDESK1155 _NFTDESK1155M = NFTDESK1155(adminContract);
        // set the rest of the contract variables
        NFTDESK1155M = _NFTDESK1155M;
        tokentype[type1] = address1;
    }
    ...
    function getServiceFeeESC() public view returns (uint256) {
        return  NFTDESK1155M.getServiceFeeESC();
    }

    function getServiceFee() public view returns (uint256) {
        return  NFTDESK1155M.getServiceFee();
    }
    ...
    function enableTokenStatus(bool _status, uint256 _tokenId) public returns (bool) {
    ...
    function _saleToken(
        ...
        uint256 serviceVal = NFTDESK1155M.getServiceFee();
        if(keccak256(bytes(bidtoken)) == keccak256(bytes("NIL")))
        {
             serviceVal = NFTDESK1155M.getServiceFeeESC();
        }

        address payable admin =  payable(NFTDESK1155M.getOwner());
        ...
        if (
            ...
            IERC20 t = IERC20(tokentype[bidtoken]);
    ...
    function _acceptBId(
        ...
        uint256 serviceVal = NFTDESK1155M.getServiceFee();
        if(keccak256(bytes(tokenAss)) == keccak256(bytes("NIL")))
        {
             serviceVal = NFTDESK1155M.getServiceFeeESC();
        }
        ...
        uint256 val = pERCent(amount, serviceVal).add(amount);
        IERC20 t = IERC20(tokentype[tokenAss]);
        Tokendecimals = deci.sub(t.decimals());
