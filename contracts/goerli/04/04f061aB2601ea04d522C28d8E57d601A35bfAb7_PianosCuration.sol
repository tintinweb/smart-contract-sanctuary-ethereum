...
contract PianosCuration is AdminControl, ICreatorExtensionTokenURI {
    using Strings for uint256;

    mapping(address => bool) public editionsContracts;
    mapping(address => bool) public oneOfOneContracts;
    mapping(address => bool) public artists;
    address[] public artistsList;

    struct TokenForContract {
        uint tokenId;
        address contractAddress;
    }

    mapping(address => mapping(uint => string)) public tokensURIs;
    mapping(address => mapping(uint => address)) public artistForToken;
    mapping(address => TokenForContract[]) public tokensForArtist;
    ...
    modifier artistRequired {
      require(artists[msg.sender], "Invalid artist");
      _;
    }

    modifier artistToken(address _contract, uint _tokenId) {
      require(artistForToken[_contract][_tokenId] == msg.sender, "Invalid artist");
      _;
    }

    function getTokensForArtist(address artistAddress) public view returns(TokenForContract[] memory tokens) {
    ...
    function addArtist(address artistAddress) public adminRequired {
        artists[artistAddress] = true;
        artistsList.push(artistAddress);
    }

    function removeArtist(address artistAddress) public adminRequired {
        artists[artistAddress] = false;
    }

    function updateTokenURI(string memory _tokenURI, address _contract, uint _tokenId) public artistToken(_contract, _tokenId) {
        tokensURIs[_contract][_tokenId] = _tokenURI;
    }

    function mintMoreEditions(address _contract, uint _tokenId, uint numberToMint) public artistToken(_contract, _tokenId) {
        ...
        tokenIds[0] = _tokenId;
        IERC1155CreatorCore(_contract).mintExtensionExisting(receivers, tokenIds, amounts);
    }

    function mint(string memory _tokenURI, address _contract, bool isEdition, uint numberToMint) public artistRequired {
        uint tokenId;
        if (isEdition) {
            address[] memory receivers = new address[](1);
            receivers[0] = msg.sender;
            ...
            uris[0] = "";
            uint[] memory tokenIds = IERC1155CreatorCore(_contract).mintExtensionNew(receivers, amounts, uris);
            tokenId = tokenIds[0];
        } else {
            tokenId = IERC721CreatorCore(_contract).mintExtension(msg.sender);
        }

        tokensURIs[_contract][tokenId] = _tokenURI;
        artistForToken[_contract][tokenId] = msg.sender;
        tokensForArtist[msg.sender].push(TokenForContract(tokenId, _contract));
    }

    function add1155Contract(address _contract) public adminRequired {
        editionsContracts[_contract] = true;
    }

    function add721Contract(address _contract) public adminRequired {
        oneOfOneContracts[_contract] = true;
    }

    function tokenURI(address creator, uint256 tokenId) external view override returns(string memory) {
        require(editionsContracts[creator] || oneOfOneContracts[creator], "Invalid contract");
        return tokensURIs[creator][tokenId];
    }
}
