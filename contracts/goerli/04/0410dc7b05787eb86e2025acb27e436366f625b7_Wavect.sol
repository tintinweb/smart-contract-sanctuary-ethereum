...
contract Wavect is ERC721, ERC2981, LinearlyAssigned, AddRecover, Pausable, ONFT721Core, IONFT721, Multicall, ReentrancyGuard {
    ...
    address constant L0_ENDPOINT = address(0xf69186dfBa60DdB133E91E9A4B5673624293d8F8); // mumbai
    ...
    constructor(string memory contractURI_, string memory baseURI_, string memory name_,
        string memory ticker_, uint256 totalSupply_, address owner_)
    ...
    LinearlyAssigned(totalSupply_, 1)
    {
        ...
        * and for the reward nfts to ensure that nonces/signatures cannot be reused. */
        _pause();
        // NOTE: Enable mint on main chain

        super.transferOwnership(owner_); // using create2
    }
    ...
    function mint(bytes memory sig_, uint256 amount_) payable external whenNotPaused nonReentrant {
        require(isBelowMaxWallet(_msgSender(), amount_), "Max wallet");
        ...
        if (!publicSaleEnabled) {
            require(isWhitelisted(_msgSender(), sig_), "Invalid sig");
        }
        ...
        if (msg.value > sumMintPrice) {
            ...
            payable(_msgSender()).transfer(msg.value - sumMintPrice);
        }
    }
    ...
    function setEnableMint(bool enable_) external onlyOwner {
        require(address(lzEndpoint) != address(0), "L0 not configured");
    ...
    function supportsInterface(bytes4 interfaceId) public view virtual override(ONFT721Core, ERC721, IERC165, ERC2981) returns (bool) {
...
abstract contract ONFT721Core is NonblockingLzApp, ERC165, IONFT721Core {
    ...
    function _send(address _from, uint16 _dstChainId, bytes memory _toAddress, uint _tokenId, address payable _refundAddress, address _zroPaymentAddress, bytes memory _adapterParams) internal virtual {
        uint rank = _debitFrom(_from, _dstChainId, _toAddress, _tokenId);

        bytes memory payload = abi.encode(_toAddress, _tokenId, rank);
    ...
    function _nonblockingLzReceive(
        ...
        (bytes memory toAddressBytes, uint tokenId, uint rank) = abi.decode(_payload, (bytes, uint, uint));
        address toAddress;
        assembly {
            toAddress := mload(add(toAddressBytes, 20))
        }

        _creditTo(_srcChainId, toAddress, tokenId, rank);
    ...
    function _debitFrom(address _from, uint16 _dstChainId, bytes memory _toAddress, uint _tokenId) internal virtual returns(uint);

    function _creditTo(uint16 _srcChainId, address _toAddress, uint _tokenId, uint _rank) internal virtual;
}
...
import "./LzApp.sol";
import "@layer-zero/contracts/util/ExcessivelySafeCall.sol";
...
import "@openzeppelin/contracts/access/Ownable.sol";
import "@layer-zero/contracts/interfaces/ILayerZeroReceiver.sol";
import "@layer-zero/contracts/interfaces/ILayerZeroUserApplicationConfig.sol";
import "@layer-zero/contracts/interfaces/ILayerZeroEndpoint.sol";
import "@layer-zero/contracts/util/BytesLib.sol";
...
abstract contract LzApp is Ownable, ILayerZeroReceiver, ILayerZeroUserApplicationConfig {
    ...
    function setLzEndpointOnce(address _endpoint) external onlyOwner {
        require(address(lzEndpoint) == address(0), "Already set");
        lzEndpoint = ILayerZeroEndpoint(_endpoint);
    }
