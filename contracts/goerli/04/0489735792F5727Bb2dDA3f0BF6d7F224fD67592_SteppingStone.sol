...
contract SteppingStone is ERC721URIStorage, Ownable, ReentrancyGuard {
    ...
    // uint _interval removed from constructor
    ...
    function handleExpiredTokens() external {
        ...
        for (uint256 i = 0; i < amountOfTokens; i++) {
            ...
            if (!target.burned) {
                ...
                // removed line again because we only want to check if we are currently
                // past expiration, interval is no longer needed since chainlink keepers
                // cron job is handling the interval at which the function is ran
                console.log("target not burned");
                // int temp = int(target.expires) - int(block.timestamp);
                // console.log(temp);
