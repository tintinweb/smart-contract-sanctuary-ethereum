...
contract Earn is AccessControl {
    ...
    /// @notice Percentage daily yields - magnified by 1e18.
    uint256[] private _dailyYields;
    ...
    /**
     * @notice Initializes external dependencies and certain state variables.
    ...
    function initialize(address hydt_, address hygt_, address shydt_, address treasury_, uint256 initialMintStartTime_) external {
        ...
        require(hydt_ != address(0), "Earn: invalid hydt_ address");
        require(hygt_ != address(0), "Earn: invalid hygt_ address");
        require(shydt_ != address(0), "Earn: invalid shydt_ address");
        require(treasury_ != address(0), "Earn: invalid treasury_ address");
        ...
        depositFee = 15;
        _dailyYields = [0.000448888888888889 * 1e18, 0.000554444444444444 * 1e18, 0.000820273972602739 * 1e18];
        ...
        if (poolInfo.length == 0) {
            _addPool(0, 100, startBlock);   // 104%
            _addPool(1, 300, startBlock);   // 110%
            _addPool(2, 600, startBlock);   // 130%
        }
    ...
    /**
     * @notice Returns the number of pools added since creation.
     */
    function poolLength() external view returns (uint256) {
        return poolInfo.length;
    }

    function _addPool(uint8 stakeType, uint256 allocPoint, uint256 lastRewardBlock) internal {
     ...
     * @param stakeType Denotes which stake duration tier's allocation to change.
     * @param allocPoint New value for allocation points.
     */
    ...
    // View function to see pending HYGT.
    function getPending(address user, uint256 index) public view returns (uint256) {
        ...
        if (staking.status) {
            ...
            if (block.number > pool.lastRewardBlock && stakeSupply != 0) {
                uint256 numberOfBlocks = block.number - pool.lastRewardBlock;
                uint256 HYGTReward = (numberOfBlocks * HYGTPerBlock * pool.allocPoint) / totalAllocPoint;
                accHYGTPerShare += (HYGTReward * 1e12) / stakeSupply;
            }
            pending = ((staking.amount * accHYGTPerShare) / 1e12) - staking.rewardDebt;
        }
        return pending;
    }
        ...
        for (uint256 i = 0 ; i < getStakingLengths[user] ; i++) {
            ...
            if (staking.status) {
                ...
                if (block.timestamp <= staking.endTime) {
                    ...
                    dailyPayout = ((staking.amount * ratio) / 1e2) / 1e18;
                }
            }
    ...
    function getYieldBatch(address user) external view returns (uint256) {
        uint256 totalYield;

        for (uint256 i = 0 ; i < getStakingLengths[user] ; i++) {
            totalYield += getYield(user, i);
        }
        return totalYield;
    }

    function getYieldType(address user, uint8 stakeType) external view returns (uint256) {
        uint256 totalYield;
    ...
    function getYield(address user, uint256 index) public view returns (uint256) {
        uint256 yield;
        Staking memory staking = getStakings[user][index];

        if (staking.status) {
            uint256 endTime = block.timestamp > staking.endTime ? staking.endTime : block.timestamp;
            uint256 numberOfDays = (endTime - staking.startTime) / ONE_DAY_TIME;

            if (numberOfDays > 0) {
                // making sure users receive required reward amount
                bool hasEnded = endTime == staking.endTime;

                if (hasEnded) {
                    uint256 ratio = dailyPayouts[staking.stakeType];
                    uint256 payoutPerDay = (staking.amount * ratio) / 1e18;
                    uint256 accruedAmount = payoutPerDay * numberOfDays;
                    yield = accruedAmount - staking.amount;
                } else {
                    uint256 ratio = _dailyYields[staking.stakeType];
                    uint256 yieldPerDay = (staking.amount * ratio) / 1e18;
                    yield = yieldPerDay * numberOfDays;
                }
            }
        }
        return yield;
    ...
    function stake(uint256 amount, uint8 stakeType) external {
        ...
        require(0 <= stakeType && stakeType <= 2, "Earn: invalid stake type");
        ...
        PoolInfo storage pool = poolInfo[stakeType];
        updatePool(stakeType);
        pool.stakeSupply += amount;

        uint256 index = getStakingLengths[_msgSender()];
        ...
        getStakings[_msgSender()][index] = staking;
    ...
    function unstake(uint256 index) public {
        Staking memory staking = getStakings[_msgSender()][index];

        require(staking.status, "Earn: invalid staking");
        require(block.timestamp > staking.endTime, "Earn: maturity not reached");
        uint256 amountMint = staking.amount + getYield(_msgSender(), index);
        delete getStakings[_msgSender()][index];
        sHYDT.burn(staking.amount);

        PoolInfo storage pool = poolInfo[staking.stakeType];
        updatePool(staking.stakeType);
        uint256 pending = ((staking.amount * pool.accHYGTPerShare) / 1e12) - staking.rewardDebt;
        HYGT.mint(_msgSender(), pending);
        pool.stakeSupply -= staking.amount;

        HYDT.mint(_msgSender(), amountMint);

        uint256 activeDeposits = sHYDT.balanceOf(address(this));

        emit Unstake(_msgSender(), staking.index, staking.amount, activeDeposits);
    }
}
