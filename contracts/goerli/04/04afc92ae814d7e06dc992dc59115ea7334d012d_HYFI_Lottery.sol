    ...
    AccessControlUpgradeable
{
    ...
    uint256 _guaranteedRangeMax;

    /// @dev thw number of Vaults which should be opened at once to have Guaranteed rewards sets
    uint256 _guaranteedThreshold;
    ...
    /**
     * @dev the array with information about each reward using struct RewardsData
     * @param totalAmount - total amount of specific reward
     * @param freeAmount - the remaining available amount of rewards
     * @param rewardManager is smart contract responsible for revealing specific reward, follow interface IHYFI_RewardsManager
     */
    struct RewardsData {
        uint256 totalAmount;
        uint256 freeAmount;
     ...
     */
    function addReward(address rewardManager, uint256 totalAmount)
        external
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
     ...
     * @param rewardManager the address of reward manager responsible for rewards revealing logic
     * @param totalAmount the total available amount of such type of rewards
     * @param resetFreeAmount true if the freeAmount value for this reward should be reseted
     */
     ...
     * @param range array with min and max in range, for probability reflection, if the normilized range is 0-9, so the range 0-1 reflects probability 20%
     * @param rewards array of rewards, where key is rewardId and value is reward amount
     */
    function addRewardsSet(uint256[2] memory range, uint256[] memory rewards)
        external
        onlyRole(DEFAULT_ADMIN_ROLE)
        checkRewardsLength(rewards)
    {
     ...
     */
    function revealVaults(uint256 revealAmount)
        external
        checkUserOwnsVaultsAmount(revealAmount)
    {
     ...
     */
    function revealSpecificVaults(uint256[] memory vaultIds)
        external
        checkUserOwnsVaultsIds(vaultIds)
    {
     ...
     */
    function getWinningRewardsSetId(uint256 vaultId)
        public
        view
        returns (uint256)
    {
     ...
     */
    function isRewardAvailable(uint256 id, uint256 amount)
        public
        view
        returns (bool)
    {
     ...
     */
    function areSpecificRewardsAvailable(uint256[] memory rewards)
        public
        view
        returns (bool)
    {
     ...
     * @return random value, not normalized
     */
    function getRandomValueN(uint256 seed, uint256 normalized)
        public
        view
        returns (uint256)
    {
     ...
     * @return max amount of guaranteed packs
     */
    function getGuaranteedPacksAmount(uint256 amount)
        public
        view
        returns (uint256)
    {
    ...
    /**
     * @dev calculate the the number of vaults are needed to be revealed one-by-one, if vaults amount are 23, 3 tickets should be revealed one-by-one (23%5) or (23 - 4*5)
     * @param amount the amount of vaults are going to be revealed
     * @return amount of vaults needed to be revealed one-by-one
     */
     ...
     * @return (rangeMin, rangeMax, rewards array)
     */
     ...
     * @return (totalAmount, freeAmount, rewardManager address)
     */
     ...
     * @return max value in the normalized range of rewards sets
     */
     ...
     * @return max value in the normalized range of guaranteed rewards sets
     */
     ...
     * @return user address
     */
    function getUserVaultIds(address user)
        external
        view
        returns (uint256[] memory)
    {
        ...
        for (uint256 i = 0; i < tokensAmount; i++) {
            tokenIds[i] = _vault.tokenByIndex(i);
        }
        return tokenIds;
    }
     ...
     * @return array of user rewards where key is rewardId and value is won rewards amount
     */
    function _revealSpecificVaults(uint256[] memory vaultIds)
        internal
        returns (uint256[] memory)
    {
     ...
     * @return array where key is rewardsSet id and value is amount of won rewardsSets
     */
    function _processVaultsOneByOne(uint256[] memory vaultIds)
        internal
        returns (uint256[] memory)
    {
