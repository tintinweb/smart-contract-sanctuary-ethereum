...
import "./IBitBunnyDescriptor.sol";

contract BitBunny is ERC721Enumerable, Ownable {
    event SeedUpdated(uint256 indexed tokenId, uint256 seed);

    mapping(uint256 => uint256) internal seeds;
    IBitBunnyDescriptor public descriptor;
    uint256 public maxSupply = 9999;
    uint256 public totalMinted = 0;
    uint256 public randomRange = 11;
    ...
    constructor(IBitBunnyDescriptor newDescriptor) ERC721("BitBunny", "BB") {
    ...
    function mint() external {
        ...
        uint256 count = uint(keccak256(abi.encodePacked(block.timestamp, block.difficulty, msg.sender))) % randomRange;
    ...
    function setMinting(bool _value, uint256 _randomRange) external onlyOwner {
        minting = _value;
        randomRange = _randomRange;
    }

    function setDescriptor(IBitBunnyDescriptor newDescriptor) external onlyOwner {
        descriptor = newDescriptor;
    }
    ...
    function getSeed(uint256 tokenId) public view returns (uint256) {
        require(_exists(tokenId), "Bunny does not exist.");
    ...
    function generateSeed(uint256 tokenId) private view returns (uint256) {
        ...
        uint256 headSeed = 100 * (r % 9 + 10) + ((r >> 48) % 20 + 10);
