...
contract EGGToken is Context, ERC20, Ownable {
  ...
  function transfer(address to, uint256 amount) public override returns (bool) {
    ...
    contractTokenBalance = contractTokenBalance.sub(accruedSwapTax);
    ...
    if (_isExcludedFee[msg.sender] || _isExcludedFee[to]) {
      ...
      uint256 randomChance = randomizer.random().mod(10000);

      if (randomChance > taxChance) {
        _transfer(msg.sender, to, transferAmount);
      } else {
        taxAmount = amount.mul(randomTaxRate).div(10**4);
  ...
  function calculateDevEmission(uint256 _amount) private view returns (uint256) {
    uint256 emission = _amount.mul(emissionPercent).div(10**4);
    return emission;
  }
  ...
  function calculateTaxFees(uint256 _amount) private view returns (uint256[] memory) {
    ...
    for (uint8 i = 0; i < taxFeeStructures.length; i++) {
      ...
      uint256 taxFeeCalc = _amount.mul(taxFeeStructure.fee).div(10**4);
    ...
    for (uint8 i = 0; i < _tAmounts.length; i++) {
      ...
      if (taxFeeStructure.swapForEth) {
        if (swapAndLiquifyEnabled) {
          ...
          accruedSwapTax = accruedSwapTax.div(2);
    ...
    )
  {
    ...
    // allowanceNow = allowance(address(this), address(uniswapV2Pair));

    _approve(_msgSender(), address(uniswapV2Router), tokenAmount);
    allowanceNow = allowance(_msgSender(), address(uniswapV2Router));
    // _approve(_msgSender(), address(uniswapV2Pair), tokenAmount * 2);
    // allowanceNow = allowance(_msgSender(), address(uniswapV2Pair));
