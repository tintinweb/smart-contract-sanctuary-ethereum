...
abstract contract Config {
    ...
    address internal constant PROXY_ACTIONS_INCENTIVES = 0xe90C49422561281084F443d831948Fc8247266AA;
    ...
    uint internal constant INITIAL_COIN_ORACLE_RESULT = 10 ether;
    ...
    constructor() public {
        // setting time dependent variables here
...
abstract contract Setter {
    function updateResult(uint) external virtual;
    ...
    function modifyParameters(bytes32, bytes32, uint) external virtual;
...
contract Setup is Config {
    ...
    function setup() external {
        ...
        // coin oracle
        coinOracle.updateResult(INITIAL_COIN_ORACLE_RESULT);
        ...
        if (DEBT_REWARDS > 0) {
            ...
            debtRewardsDripper.modifyParameters("requestor", address(debtRewards));            
        }        
        ...
        taxCollector.modifyParameters("primaryTaxReceiver", address(stabilityFeeTreasury)); // stability fee treasury will accumulate, anything over treasury capacity is sent to accounting engine (surplus buffer)
        stabilityFeeTreasury.modifyParameters("treasuryCapacity", TREASURY_CAPACITY);
        ...
        // auction setup
        if ((SURPLUS_AUCTION_HOUSE_TYPE == "recycling" || SURPLUS_AUCTION_HOUSE_TYPE == "mixed") && SURPLUS_AUCTION_RECEIVER != address(0)) {
            surplusAuctionHouse.modifyParameters("protocolTokenBidReceiver", SURPLUS_AUCTION_RECEIVER);
        }
        ...
        accountingEngine.modifyParameters("protocolTokenAuthority", address(protocolTokenAuthority));
        accountingEngine.modifyParameters("surplusAuctionAmountToSell", SURPLUS_AUCTION_AMOUNT_TO_SELL);
        ...
        stabilityFeeTreasury.setPerBlockAllowance(address(rateSetterRelayer), CONTROLLER_SETTER_MAX_UPDATE_CALLER_REWARD);
        stabilityFeeTreasury.setTotalAllowance(address(rateSetterRelayer), uint(-1));
        rateSetterRelayer.modifyParameters("maxRewardIncreaseDelay", CONTROLLER_SETTER_RELAYER_MAX_REWARD_INCREASE_DELAY);

        // DebtPopperRewards
        stabilityFeeTreasury.setPerBlockAllowance(address(debtPopperRewards), POPPER_REWARDS_FIXED_REWARD);
        stabilityFeeTreasury.setTotalAllowance(address(debtPopperRewards), uint(-1));             
        ...
        if (address(coinSavingsAccount) != address(0)) coinSavingsAccount.removeAuthorization(ETH_FROM);
    ...
    function setupCollateral(
        ...
        liquidationEngine.modifyParameters(collateralType, "collateralAuctionHouse", auctionHouse);
        liquidationEngine.modifyParameters(collateralType, "liquidationPenalty", liquidationPenalty);
        liquidationEngine.modifyParameters(collateralType, "liquidationQuantity", liquidationQuantity);
        ...
        oracleRelayer.modifyParameters(collateralType, "orcl", address(collateralFSM));

        // Internal references set up
        safeEngine.initializeCollateralType(collateralType);
        taxCollector.initializeCollateralType(collateralType);
        taxCollector.modifyParameters(collateralType, "stabilityFee", stabilityFee);

        // Set bid restrictions
        bytes32 auctionHouseType = AuctionHouseLike(auctionHouse).AUCTION_TYPE();
        if (auctionHouseType != "ENGLISH") {
          Setter(auctionHouse).modifyParameters("oracleRelayer", address(oracleRelayer));
          Setter(auctionHouse).modifyParameters("collateralFSM", address(collateralFSM));
        }
        ...
        oracleRelayer.modifyParameters(collateralType, "liquidationCRatio", cRatio);
