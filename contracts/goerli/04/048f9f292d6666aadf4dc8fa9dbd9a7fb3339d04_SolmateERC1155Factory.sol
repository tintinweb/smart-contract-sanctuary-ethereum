...
contract SolmateMockERC1155 is ERC1155 {
    ...
    function uri(uint256 id) public view override returns (string memory) {
        return
            bytes(baseURI).length > 0 ? string(abi.encodePacked(baseURI, id.toString())) : baseURI;
    }
...
contract SolmateERC1155Factory {
    event Deployed(address indexed _token);

    function deploy() public returns (address token) {
        token = address(new SolmateMockERC1155());
        emit Deployed(token);
    }
}
