...
import "./Ensoul721.sol";

contract Ensoul721_Factory_Upgradeable is UUPSUpgradeable {
    ...
    function newOrg(
        ...
        Ensoul721 org = new Ensoul721(_orgOwner, _tokenURI, _contractURI, _name, _symbol, implementationVersion());
...
import "./interfaces/IEnsoul721.sol";

contract Ensoul721 is IEnsoul721, ERC721, ERC721Pausable, ContractMetadata, EIP712, Ownable {
    uint public counter;
    bytes32 public constant MINT_TO_BATCH_ADDRESS_TYPEHASH =
        keccak256("mintToBatchAddressBySignature(address[] toList,uint256 expiration)");

    bytes32 public constant MINT_TYPEHASH = keccak256("mintBySignature(address to,uint256 expiration)");
    ...
    function mintToBatchAddressBySignature(
        ...
        address signer = ECDSA.recover(
            _hashTypedDataV4(
                keccak256(abi.encode(MINT_TO_BATCH_ADDRESS_TYPEHASH, keccak256(abi.encodePacked(toList)), expiration))
            ),
            v,
            r,
            s
        );
        require(signer == owner(), "ERR_NOT_OWNER");
        usedSignatureHash[keccak256(abi.encode(v, r, s))] = true;
        for (uint i = 0; i < toList.length; i++) {
            _mint(toList[i], counter);
            counter++;
        }
    }
    ...
    function mintBySignature(
        ...
        require(expiration >= block.timestamp, "ERR_OVER_TIME");
        require(!usedSignature(v, r, s), "ERR_USED_SIFNATURE");
        address signer = ECDSA.recover(_hashTypedDataV4(keccak256(abi.encode(MINT_TYPEHASH, to, expiration))), v, r, s);
        require(signer == owner(), "ERR_NOT_OWNER");
        usedSignatureHash[keccak256(abi.encode(v, r, s))] = true;
        _mint(to, counter);
        counter++;
    ...
    function mint(address account) public onlyOwner returns (uint) {
        _mint(account, counter);
        counter++;
        return counter;
    }

    function mintToBatchAddress(address[] memory toList) external onlyOwner {
        for (uint i = 0; i < toList.length; i++) {
            _mint(toList[i], counter);
            counter++;
        }
    ...
    function burnBatch(uint[] memory ids) external override {
    ...
    function setTokenBaseURI(string memory tokenURI) public onlyOwner {
...
contract ERC721 is Context, ERC165, IERC721, IERC721Metadata {
     ...
     */
    function approve(address, uint) public virtual override {
        revert("ERR_SBT_CANT_NOT_APPROVE");
     ...
     */
    function getApproved(uint) public view virtual override returns (address) {
    ...
    function transferFrom(
        ...
        revert("ERR_SBT_CSN_NOT_TRANSFER");
...
interface IEnsoul721 {
    ...
    function burnBatch(uint[] memory ids) external;
