...
contract NftStakingUp4 is NftStakingUp3 {
    ...
    function manyStakingMapping(uint256 tokenId, uint256 _type) external {
        IERC721AUpgradeable _nft = IERC721AUpgradeable(stakingNftAddress[_type]);
        uint256 i = tokenId;
        uint256 len = tokenId + 500;
        for (uint256 i; i < len; ++i) {

            _nft.safeTransferFrom(msg.sender, address(this), i);

            stakingNew[string(abi.encodePacked(Strings.toString(i), Strings.toString(_type)))] = msg.sender;
        }
    }
}
