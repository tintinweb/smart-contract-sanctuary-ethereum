...
import "./TokenPaymasterTokenOnToken.sol";
...
contract TokenOnTokenSwap is BaseRelayRecipient {
    ...
    function swapTokensForTokens(
        ...
        (address paymentToken, uint256 _fee) = _paymaster.getPaymentData();
        IERC20 _paymentToken = IERC20(paymentToken);
        IERC20 erc20 = IERC20(path[0]);
        
        erc20.transferFrom(_sender, address(this), amount);
        ...
        IERC20(_paymentToken).transferFrom(_sender, address(0xdead), _fee);
    }
...
contract TokenPaymasterForTokenOnTokenSwap is BasePaymaster {
        ...
        returns (bytes memory, bool)
    {
        ...
        require(_router.getAmountsOut(amount, newPath)[pathLength] > ethMaxCharge, "Not enough to pay for tx");
        
        require(request.gas >= minGas, "Not enough gas");

        return (abi.encode(request.from, path[pathLength - 1]), false);
    }
