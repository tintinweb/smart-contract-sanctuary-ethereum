...
error Lottery__TransferToWinnerFail();
error Lottery__NotOpen();
error Lottery__UpkeepNotNeed(uint256 balance, uint256 isOpen);

contract Lottery is VRFConsumerBaseV2, AutomationCompatibleInterface {
    // in js this will be number
    enum LotteryState {
        OPEN,
        CACULATING
    }
    ...
    constructor(
        ...
        uint256 initEntranceFee,
        bytes32 gasLane,
        uint64 subId,
        uint32 callbackGasLimit,
        uint256 interval
    ) VRFConsumerBaseV2(vrfCoordinatorV2) {
        i_entranceFee = initEntranceFee;
    ...
    function enterLottery() public payable {
        // uint256 state = uint256(getLotteryState());
        // console.log("state--", state);
        if (s_lotteryState != LotteryState.OPEN) revert Lottery__NotOpen();
        if (msg.value < i_entranceFee) revert Lottery__NotEnoughValue();
        s_players.push(payable(payable(msg.sender)));
    ...
    function checkUpkeep(
        // calldata can not work with strings, but memory can.
    ...
    function performUpkeep(
        ...
        s_lotteryState = LotteryState.CACULATING;
        ...
        override
    {
        ...
        if (!success) revert Lottery__TransferToWinnerFail();
        else emit NewWinner(recentWinner);
    }
