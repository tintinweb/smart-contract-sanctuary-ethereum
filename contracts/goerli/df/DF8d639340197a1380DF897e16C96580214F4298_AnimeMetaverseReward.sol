...
contract AnimeMetaverseReward is ERC1155, Ownable, AmvUtils {
    ...
    uint256 totalTokenType = 6;
    uint256 maxMintLimit = 1;

    uint256 constant DIGITAL_COLLECTIBLE_HOODIE_TOKEN_ID = 19;
    uint256 constant DIGITAL_COLLECTIBLE_TSHIRT_TOKEN_ID = 20;
    uint256 constant DIGITAL_COLLECTIBLE_SOCK_TOKEN_ID = 21;
    ...
    constructor() ERC1155("") {
        setTokenInfo("Gift of Soul", 170, GIFT_BOX_TOKEN_TYPE);
        setTokenInfo("Sakura", 4, ARTIFACTS_TOKEN_TYPE);
        setTokenInfo("Starburst X", 4000, ARTIFACTS_TOKEN_TYPE);
        setTokenInfo("Starburst C", 2500, ARTIFACTS_TOKEN_TYPE);
        setTokenInfo("Starburst D", 1896, ARTIFACTS_TOKEN_TYPE);
        setTokenInfo("Starburst M", 1500, ARTIFACTS_TOKEN_TYPE);
        setTokenInfo("Starburst V", 100, ARTIFACTS_TOKEN_TYPE);
        setTokenInfo("God Rune", 450, RUNE_CHIP_TOKEN_TYPE);
        setTokenInfo("Man Rune", 1500, RUNE_CHIP_TOKEN_TYPE);
        setTokenInfo("Sun Rune", 3000, RUNE_CHIP_TOKEN_TYPE);
        setTokenInfo("Fire Rune", 4500, RUNE_CHIP_TOKEN_TYPE);
        setTokenInfo("Ice Rune", 5550, RUNE_CHIP_TOKEN_TYPE);
        setTokenInfo("Scroll of Desire", 4715, SCROLL_TOKEN_TYPE);
        setTokenInfo("Scroll of Prophecy", 3301, SCROLL_TOKEN_TYPE);
        setTokenInfo("Scroll of Fortitude", 1414, SCROLL_TOKEN_TYPE);
        setTokenInfo("Hoodie", 200, COLLECTIBLE__TOKEN_TYPE);
        setTokenInfo("Tshirt", 400, COLLECTIBLE__TOKEN_TYPE);
        setTokenInfo("Socks", 800, COLLECTIBLE__TOKEN_TYPE);
        setTokenInfo("Digital Hoodie", 200, DIGITAL_COLLECTIBLE_TOKEN_TYPE);
        setTokenInfo("Digital Tshirt", 400, DIGITAL_COLLECTIBLE_TOKEN_TYPE);
        setTokenInfo("Digital Sock", 800, DIGITAL_COLLECTIBLE_TOKEN_TYPE);
    }
    ...
    function setTokenInfo(
        ...
        totalToken++;
        tokenCollection[totalToken] = TokenInfo({
    ...
    function setMaxMintLimit(uint256 _mintLimit) external onlyOwner {
        ...
        require(
            _mintLimit <= 5550,
            "Can not set mintLimit more than max total supply."
        );
    ...
    function mintBatch(
        ...
        for (uint256 j = 0; j < _ids.length; j++) {
            ...
            if (
                tokenCollection[_id].tokenType == DIGITAL_COLLECTIBLE_TOKEN_TYPE
            ) {
                revert InvalidMintableTokenId();
            }
            if (
                tokenCollection[_id].totalSupply + _amounts[j] >
                tokenCollection[_id].maxSupply
            ) {
                revert MintAmountForTokenTypeExceeded(_amounts[j]);
            }
            tokenCollection[_id].totalSupply += _amounts[j];
        }

        _mintBatch(_to, _ids, _amounts, _data);
        emit MintBatch(_activityId, msg.sender, _to, _ids, _amounts);
    }
    ...
    function maxSupply(uint256 _id)
        external
        view
        validTokenId(_id)
        returns (uint256 supply)
    {
        supply = tokenCollection[_id].maxSupply;
    }
    ...
    function totalSupply(uint256 _id)
        ...
        returns (uint256 supply)
    {
        supply = tokenCollection[_id].totalSupply;
    }

    function getTokenType(uint256 _id) public view returns (uint256) {
        if (_id < 1 || _id > totalToken) revert InvalidTokenId();
        return tokenCollection[_id].tokenType;
    }

    function mintDigitalCollectible(address _to, uint256 collectible_id)
        internal
    {
        uint256 _id = collectible_id + 3;
        tokenCollection[_id].totalSupply++;
        _mint(_to, _id, 1, "");
        emit MintDigitalMerch(msg.sender, _to, _id, 1);
    }
