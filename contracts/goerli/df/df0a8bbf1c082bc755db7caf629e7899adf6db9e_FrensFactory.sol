...
import "@tableland/contracts/utils/URITemplate.sol";

contract FrensFactory is ERC721URIStorage, Ownable {
    using Counters for Counters.Counter;

    Counters.Counter private _tokenIds;
    ITablelandTables private _tableland;

    mapping(string => string) private _tables;
    mapping(string => uint256) private _tableIds;

    string private _baseURIString = "https://testnet.tableland.network/query?s=";

    constructor(address registry) ERC721("FrensFactory", "FRENS") {
        // The Tableland address on the current chain
        _tableland = ITablelandTables(registry);

        // Create frens table and store table name
        _tableIds["frens"] = _tableland.createTable(
            address(this),
            string.concat(
                "CREATE TABLE frens",
                Strings.toString(block.chainid),
                " (id INT, name TEXT NOT NULL, skin TEXT, ears TEXT, head TEXT, hair TEXT, eyes TEXT, clothes TEXT, mouth TEXT, accessory TEXT, background TEXT);"
            )
        );
        _tables["frens"] = string.concat(
            "frens_",
            Strings.toString(block.chainid),
            "_",
            Strings.toString(_tableIds["frens"])
        );

        // Create attributes table and store table name
        _tableIds["attributes"] = _tableland.createTable(
            address(this),
            string.concat(
                "CREATE TABLE attributes",
                Strings.toString(block.chainid),
                " (id INT, slug TEXT NOT NULL UNIQUE, name TEXT NOT NULL, type TEXT NOT NULL CHECK(type IN ('skin', 'ears', 'head', 'hair', 'eyes', 'clothes', 'mouth', 'accessory', 'background')), cost TEXT NOT NULL, collection_slug TEXT NOT NULL, image_url TEXT NOT NULL);"
            )
        );
        _tables["attributes"] = string.concat(
            "attributes_",
            Strings.toString(block.chainid),
            "_",
            Strings.toString(_tableIds["attributes"])
        );

        // Create collections table and store table name
        _tableIds["collections"] = _tableland.createTable(
            address(this),
            string.concat(
        ...
        _tables["collections"] = string.concat(
            "collections_",
            Strings.toString(block.chainid),
            "_",
            Strings.toString(_tableIds["collections"])
        );
    }
    ...
    function frensURI() public view returns (string memory) {
        string memory base = _baseURI();
        return string.concat(base, "SELECT%20*%20FROM%20", _tables["frens"]);
    }

    function attributesURI() public view returns (string memory) {
        string memory base = _baseURI();
        return string.concat(base, "SELECT%20*%20FROM%20", _tables["attributes"]);
    }

    function collectionsURI() public view returns (string memory) {
        string memory base = _baseURI();
        return string.concat(base, "SELECT%20*%20FROM%20", _tables["collections"]);
    }
}
