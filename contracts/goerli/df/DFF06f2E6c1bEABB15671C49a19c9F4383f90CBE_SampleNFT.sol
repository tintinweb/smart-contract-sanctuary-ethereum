...
contract SampleNFT is DefaultOperatorFilterer, ERC721A, Ownable, AccessControl {
    ...
    function _zeroTokenURI() internal view  returns (string memory) {
        return zeroTokenURI;
    }
    ...
    function setZeroTokenURI(string memory _newZeroTokenURI) external onlyOwner {
        zeroTokenURI = _newZeroTokenURI;
    }
    ...
    function tokenURI(uint256 tokenId) public view override returns (string memory) {
        ...
        if(tokenId != 0){
            // zeroTokenURI = _zeroTokenURI();
            return bytes(_zeroTokenURI()).length != 0 ? string(abi.encodePacked(_zeroTokenURI(), _toString(tokenId))) : '';
        } else {
            // baseURI = _baseURI();
