...
*/
contract DivBonusAggregator is Ownable, IBonusAggregator{
    using SafeMath for uint256;
 ...
 */
contract DivBEP20 is Context, IBEP20, Ownable {
    using SafeMath for uint256;
    using Address for address;
...
import "./DivBonusAggregator.sol";
import "./libs/DivBEP20.sol";

// MasterDiv is the master of DIVs and DIVx.
// The Ownership of this contract is going to be transferred to a timelock
contract MasterDiv is Ownable, IMasterBonus {
    using SafeMath for uint256;
    using SafeBEP20 for DivBEP20;
    ...
    struct UserInfo {
        ...
        //
        // We do some fancy math here. Basically, any point in time, the amount of DIVss
        // entitled to a user but is pending to be distributed is:
        //
        //   pending reward = (user.amount * pool.accDIVsPerShare) - user.rewardDebt
        //
        // Whenever a user deposits or withdraws LP tokens to a pool. Here's what happens:
        //   1. The pool's `accDIVsPerShare` (and `lastRewardBlock`) gets updated.
    ...
    struct PoolInfo {
        ...
        uint256 lpSupply;
        uint256 allocPoint;       // How many allocation points assigned to this pool. DIVss to distribute per block.
        uint256 lastRewardBlock;  // Last block number that DIVss distribution occurs.
        uint256 accDIVsPerShare; // Accumulated DIVss per share, times 1e12. See below.
        uint256 depositFeeBP;     // deposit Fee
        bool isDIVxRewards;
    }

    DivBonusAggregator public bonusAggregator;
    // The DIVs TOKEN!
    DivBEP20 public divs;
    // The DIVx TOKEN!
    DivBEP20 public divx;
    // Dev address.
    address public devaddr;
    // DIVs tokens created per block.
    uint256 public divsPerBlock;
    // Deposit Fee address
    ...
    // The block number when DIVs mining starts.
    uint256 public immutable startBlock;

    // Initial emission rate: 1 DIVs per block.
    uint256 public immutable initialEmissionRate;
    // Minimum emission rate: 0.5 DIVs per block.
    ...
    constructor(
        DivBEP20 _divs,
        DivBEP20 _divx,
        DivBonusAggregator _bonusAggregator,
        address _devaddr,
        address _feeAddress,
        uint256 _divsPerBlock,
        uint256 _startBlock
    ) public {
        divs = _divs;
        divx = _divx;
        ...
        divsPerBlock = _divsPerBlock;
        startBlock = _startBlock;
        initialEmissionRate = _divsPerBlock;
    ...
    function add(uint256 _allocPoint, IBEP20 _lpToken, uint256 _depositFeeBP, bool _isDIVxRewards, bool _withUpdate) external onlyOwner {
        ...
        poolInfo.push(PoolInfo({
            ...
            isDIVxRewards: _isDIVxRewards
        }));
    }

    // Update the given pool's DIVs allocation point. Can only be called by the owner.
    function set(uint256 _pid, uint256 _allocPoint, uint256 _depositFeeBP, bool _isDIVxRewards, bool _withUpdate) external onlyOwner {
        ...
        poolInfo[_pid].isDIVxRewards = _isDIVxRewards;
        if (prevAllocPoint != _allocPoint) {
            totalAllocPoint = totalAllocPoint.sub(prevAllocPoint).add(_allocPoint);
        }
    }

    // View function to see pending DIVss on frontend.
    function pendingDIVs(uint256 _pid, address _user) external view returns (uint256) {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][_user];
        uint256 accDIVsPerShare = pool.accDIVsPerShare;
        uint256 lpSupply = pool.lpSupply;
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
            uint256 divsReward = multiplier.mul(divsPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
            accDIVsPerShare = accDIVsPerShare.add(divsReward.mul(1e12).div(lpSupply));
        }
        uint256 userRewards = user.amountWithBonus.mul(accDIVsPerShare).div(1e12).sub(user.rewardDebt);
        if(!pool.isDIVxRewards){
            // taking account of the 2% auto-burn
            userRewards = userRewards.mul(98).div(100);
        }
        return userRewards; // taking account of the 2% auto burn on divs
    }
    ...
    function updateEmissionRate() internal {
        ...
        if(divsPerBlock <= minimumEmissionRate){
            return;
        }
        ...
        uint256 newEmissionRate = divsPerBlock;
        ...
        if (newEmissionRate >= divsPerBlock) {
            return;
        }

        lastReductionPeriodIndex = currentIndex;
        uint256 previousEmissionRate = divsPerBlock;
        divsPerBlock = newEmissionRate;
    ...
    function updatePool(uint256 _pid) public validatePool(_pid) {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 divsReward = multiplier.mul(divsPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        uint256 devMintAmount = divsReward.div(10);
        divs.mint(devaddr, devMintAmount);
        if (pool.isDIVxRewards){
            divx.mint(address(this), divsReward);
        }
        else{
            divs.mint(address(this), divsReward);
        }
        pool.accDIVsPerShare = pool.accDIVsPerShare.add(divsReward.mul(1e12).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }

    // Allow DivBonusAggregator to add bonus on a single pool by id to a specific user
    function updateUserBonus(address _user, uint256 _pid, uint256 bonus) external virtual override validatePool(_pid) onlyAggregator{
        ...
        updatePool(_pid);
        if (user.amount > 0) {
            uint256 pending = user.amountWithBonus.mul(pool.accDIVsPerShare).div(1e12).sub(user.rewardDebt);
            if(pending > 0) {
                if(pool.isDIVxRewards){
                    safeDIVxTransfer(_user, pending);
                }
                else{
                    safeDIVsTransfer(_user, pending);
                }
            }
        }
        ...
        user.rewardDebt = user.amountWithBonus.mul(pool.accDIVsPerShare).div(1e12);
    }

    // Deposit LP tokens to MasterDiv for DIVs allocation.
    function deposit(uint256 _pid, uint256 _amount) external validatePool(_pid) {
        ...
        if (_amount > 0) {
            ...
            if (address(pool.lpToken) == address(divs)) {
    ...
    // Withdraw LP tokens from MasterDiv.
    function withdraw(uint256 _pid, uint256 _amount) external validatePool(_pid) {
        ...
        if(pending > 0) {
            if(pool.isDIVxRewards){
                safeDIVxTransfer(msg.sender, pending);
            }
            else{
                safeDIVsTransfer(msg.sender, pending);
            }
        }
        if(_amount > 0) {
    ...
    returns(address lpToken, uint256 allocPoint, uint256 lastRewardBlock,
            uint256 accDIVsPerShare, uint256 depositFeeBP, bool isDIVxRewards) {
        return (
            address(poolInfo[_pid].lpToken),
            poolInfo[_pid].allocPoint,
            poolInfo[_pid].lastRewardBlock,
            poolInfo[_pid].accDIVsPerShare,
            poolInfo[_pid].depositFeeBP,
            poolInfo[_pid].isDIVxRewards
        );
    }

    // Safe divs transfer function, just in case if rounding error causes pool to not have enough DIVss.
    function safeDIVsTransfer(address _to, uint256 _amount) internal {
        uint256 divsBal = divs.balanceOf(address(this));
        bool transferSuccess = false;
        if (_amount > divsBal) {
            transferSuccess = divs.transfer(_to, divsBal);
        } else {
            transferSuccess = divs.transfer(_to, _amount);
        }
        require(transferSuccess, "safeDIVsTransfer: Transfer failed");
    }

    // Safe DIVx transfer function, just in case if rounding error causes pool to not have enough DIVx.
    function safeDIVxTransfer(address _to, uint256 _amount) internal {
        uint256 DIVxBal = divx.balanceOf(address(this));
        bool transferSuccess = false;
        if (_amount > DIVxBal) {
            transferSuccess = divx.transfer(_to, DIVxBal);
        } else {
            transferSuccess = divx.transfer(_to, _amount);
        }
        require(transferSuccess, "safeDIVxTransfer: Transfer failed");
    }
