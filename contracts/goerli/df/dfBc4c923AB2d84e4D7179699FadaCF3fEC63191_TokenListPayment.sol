...
contract TokenListPayment is Ownable {
    ...
    event PaymentReceiverChanged(address newReciever);
    ...
    function changePaymentAmount(uint256 _paymentAmount) external onlyOwner {
        paymentAmount = _paymentAmount;

        emit PaymentAmountChanged(_paymentAmount);
    }

    function changePaymentReceiver(address _paymentReceiver) external onlyOwner {
        paymentReceiver = _paymentReceiver;
