...
contract Resolver is KVS {
    ...
    function abiBytesToAddressWithTimestamp(bytes memory bys) public pure returns(address payable addr, uint64 time) {
    ...
    function getAddressItem(bytes32 node, uint256 item_key) external view returns (address ret) {
        if (item_key == KEY_ADDRESS_MAIN) {
            ...
            ret = abiBytesToAddressWithoutTimestamp(metaDB.getNodeOwnerItem(node, address(0), bytes32(item_key)));
        }
    }
    ...
    function getAddressItemWithTimestamp(bytes32 node, uint256 item_key) external view returns (address addr, uint64 time) {
        if (item_key == KEY_ADDRESS_MAIN) {
            ...
            (addr, time) = abiBytesToAddressWithTimestamp(metaDB.getNodeOwnerItem(node, address(0), bytes32(item_key)));
        }
    }
    ...
    function getAddressItemList(bytes32 node, uint256 begin, uint256 end) external view returns (address[] memory) {
        ...
        for (uint256 item_key = begin; item_key <= end; item_key++) {
            if (item_key == KEY_ADDRESS_MAIN) {
                ...
                addr_array[item_key - begin] = abiBytesToAddressWithoutTimestamp(metaDB.getNodeOwnerItem(node, address(0), bytes32(item_key)));
            }
        }
        return addr_array;
    }
    ...
    function getAddressItemListWithTimestamp(bytes32 node, uint256 begin, uint256 end) external view returns (address[] memory, uint64[] memory) {
        ...
        for (uint256 item_key = begin; item_key <= end; item_key++) {
            ...
            if (item_key == KEY_ADDRESS_MAIN) {
                ...
                (addr_array[i], time_array[i]) = abiBytesToAddressWithTimestamp(metaDB.getNodeOwnerItem(node, address(0), bytes32(item_key)));
            }
        }
...
abstract contract KVS is KeyEnum, Meta { // KVS: Key Value Storage
    ...
    function getEditor(bytes32 node) public view returns (address) {
        return abiBytesToAddressWithoutTimestamp(metaDB.getNodeItem(node, bytes32(KEY_EDITOR)));
    }
...
abstract contract KeyEnum {
    ...
    uint256 constant internal KEY_WEBSITE = 1000;
    uint256 constant internal KEY_EMAIL = 1001;
    uint256 constant internal KEY_GITHUB = 1002;
    uint256 constant internal KEY_TWITTER = 1003;
    uint256 constant internal KEY_INSTAGRAM = 1004;
    uint256 constant internal KEY_TELEGRAM = 1005;
    uint256 constant internal KEY_TELEPHONE = 1006;
...
contract Editor is KVS {
    ...
    function setNftImageURI(bytes32 node, string memory uri) external onlyCooperatorOfActiveNode(node) {
        metaDB.setNodeItem(node, bytes32(KEY_NFT_IMAGE_URI), abi.encode(uri));
    }

    function setNftMetadataURI(bytes32 node, string memory uri) external onlyCooperatorOfActiveNode(node) {
        metaDB.setNodeItem(node, bytes32(KEY_NFT_METADATA_URI), abi.encode(uri));
    }
...
contract Registrar is KeyEnum, Meta {
    ...
    function register(
        ...
        bytes32 node = metaDB.activateSubnode(top_level_node, owner, 0, default_ttl, name, "");
