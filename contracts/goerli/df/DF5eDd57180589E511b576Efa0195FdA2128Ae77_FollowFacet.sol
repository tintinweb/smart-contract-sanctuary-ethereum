...
interface IControllorFacet {
    function getOwnerContractForSignetor(address contractOwner) external view returns (address);

    function getOwnerNumContractOfSignetor(address contractOwner) external view returns (uint256);
}
...
contract FollowFacet {
    ...
    modifier Joined() {
        if (IControllorFacet(address(this)).getOwnerNumContractOfSignetor(msg.sender) == 0)
    ...
    function like(uint256 SignetId, address SignetIdOwner) public Joined {
    ...
    function unlike(uint256 SignetId, address SignetIdOwner) public Joined {
        if (msg.sender == SignetIdOwner) revert Wrong__UserSubmitted();
        if (checklikeable(SignetIdOwner, SignetId) == false) revert Can__NotLike();
    ...
    function star(address SignetIdOwner, uint256 SignetId) external payable Joined {
        if (msg.sender == SignetIdOwner) revert Wrong__UserSubmitted();
        if (checklikeable(SignetIdOwner, SignetId) == false) revert Can__NotLike();
        if (
    ...
    function getStaredNumForSignetor(address SignetorAddress) external view returns (uint256) {
        return (LibSignetStorage.getStaredNumForSignetor(SignetorAddress));
    }
    ...
    function getSignetIdNow() external view returns (uint256) {
        return (LibSignetStorage.getSignetId());
    }
