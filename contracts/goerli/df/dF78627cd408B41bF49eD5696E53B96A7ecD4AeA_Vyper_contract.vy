...
MESSAGE_TYPEHASH: constant(bytes32) = keccak256(b'Message(uint256 contents)')

@internal
@view
def hash_message(_message: Message) -> bytes32:
    return keccak256(_abi_encode(
        MESSAGE_TYPEHASH,
        _abi_encode(_message.contents),
    ))


@internal
@view
def digest_message(_message: Message) -> bytes32:
    ...
    return keccak256(_abi_encode(
        '\x19\x01',
        self.hash_domain(eip712Domain),
        self.hash_message(_message),
    ))


@external
@view
def verify_test(_message: Message,
                _signature: Bytes[65]) -> address:
    ...
    # https://vyper.readthedocs.io/en/stable/built-in-functions.html#ecrecover
    # return ecrecover(prefixedHashedMessage, v, r, s) == msg.sender
    return ecrecover(self.digest_message(_message), v, r, s)