...
//        return numTokens*mintPrice;

//        uint256 toReturn = numTokens*mintPrice;
//
//        if numTokens > 10
//
//        return toReturn;

        return mintPrice*min(numTokens, 10);
    }
...
//        uint256 curSlot = curIndex%128;
    ...
    function mintPublic(uint256 _quantity, uint256 genderDirection) public payable {
        require(mintStatus == MintStatus.Public, "ms");
        require(msg.value >= computePrice(_quantity));

        totalReceived += msg.value;
        _mintMain(msg.sender, _quantity, genderDirection);
    }

    function claimAllowlist(uint256 genderDirection) public {
        require(phunksContract.balanceOf(msg.sender) > 0, "nal");
        require(!(allowlistClaimed[msg.sender]), "ac");

        _mintMain(msg.sender, 1, genderDirection);
