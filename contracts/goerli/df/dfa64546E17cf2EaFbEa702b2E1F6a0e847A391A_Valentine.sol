...
contract Valentine is ERC721, Pausable, Ownable, DefaultOperatorFilterer {
  ...
  string private _BASE_URI = 'https://ipfs.io/ipfs/QmY6YXJaDsV1oJ7SJdGUwWkeZQEx8txd8yksCBi4mpui2H/mystery.json';

  uint256 private allowlistCanMintAt = 1668668400;
  uint256 private publicCanMintAt = 1668711600;
  ...
  function batchMintAdmin(address[] calldata _addresses) external onlyAdmins {
    require (_addresses.length + _count < 7001);
  ...
  function mint() external {
    ...
    require(canMint, 'User not allowed to mint');
  ...
  function reveal(string calldata _baseURIParam) external onlyAdmins {
  ...
  function withdraw(address _to) public onlyAdmins {
  ...
  function setAdmins(address[] calldata _addresses) external onlyOwner {
    for(uint256 i; i < _addresses.length; i++){
