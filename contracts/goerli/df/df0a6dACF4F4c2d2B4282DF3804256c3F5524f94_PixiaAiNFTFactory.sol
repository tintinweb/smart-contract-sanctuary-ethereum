...
contract PixiaAiNFTFactory is Ownable{
	...
	function initCollections() external onlyOwner {
		delete collections;
	}
	...
	function createCollection(Plan _plan, uint256 collectionId) external payable returns(address collection)  {
		...
		IPixiaAiNFT(collection).setMintPrice(0.001 ether);
		IPixiaAiNFT(collection).setBaseURI("https://dev.pixia.ai/api/token/");
		IPixiaAiNFT(collection).updateRoyaltyInfo( royaltyReceiver, _plan == Plan.PAID ? 0 : royaltyBasisPoints);
