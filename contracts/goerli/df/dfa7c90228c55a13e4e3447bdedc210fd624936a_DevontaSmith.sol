    ...
    UsingTellor
{
    ...
    uint256 public etherPrice = 1640;
    uint256 public cost = 250; // USD price
    uint256 _price = 0.005 ether; // price = cost/etherPrice

    constructor(address payable _tellorAddress)
        VariableEndstateNFT(
            "Devonta Smith - Honey Drip",
            "ENDSTATE",
    ...
    function setETHPrice() public {
        bytes memory _b = abi.encode("SpotPrice", abi.encode("eth","usd"));
        ...
        etherPrice = abi.decode(_value,(uint256));

        _price = (cost/etherPrice);
    }
