...
abstract contract BattlespeciesWithdrawer is Ownable {
    address[] private _team = [
        ...
        0x44dBDE9b0F5abfc0fA189168A7326bCd0AC8A31C
    ];
    ...
    function withdraw(uint256 amount) external onlyOwner {
        ...
        uint256 share = (amount * 333333) / 1000000;
...
contract BattleSpecies is ERC721Namable, ERC721Enumerable, BattlespeciesWithdrawer {
    ...
    event BattlerHyperwarped(uint256 tokenId, uint256 jumpClone, uint256 assist);

    event BattlerManifested(uint256 tokenId);
    ...
    uint256 public mintWhitelistPrice = 50000000 gwei; //0.05 ETH

    uint256 public autoManifestPrice = 10000000 gwei; //0.01 ETH

    uint256 public maxBattlers = 4500;
    ...
    constructor(address _hyperwarpManager, address _utilityHub) ERC721Namable("Battlespecies", "BATTLE") {
    ...
    function setMaxBattlers(uint256 _maxBatters) public onlyOwner {
        maxBattlers = _maxBatters;
    }
    ...
    function changeMintWhitleistPrice(uint256 _price) external onlyOwner {
        mintWhitelistPrice = _price;
    }
    ...
    function changeAutoManifestPrice(uint256 _price) external onlyOwner {
        autoManifestPrice = _price;
    }
    ...
    function _mint(uint numberOfTokens, bool autoManifest, uint256 _mintPrice) internal {
        uint _totalSupply = totalSupply();
        require(saleIsActive, "Sale must be active to mint Battler");
        require(balanceOf(_msgSender()) + numberOfTokens <= maxMintsPerWallet, "Reached max token allotment");
        require(_totalSupply + numberOfTokens <= maxBattlers, "Purchase would exceed max supply of Battlers");

        if (autoManifest) {
            require((_mintPrice * numberOfTokens) + (autoManifestPrice * numberOfTokens) <= msg.value, "Ether value sent is not correct");
        }
        else {
        ...
        for(i = 0; i < numberOfTokens; i++) {
            ...
            if (mintIndex <= maxBattlers) {
                _safeMint(_msgSender(), mintIndex);
                if (hyperwarpManager.battlerHasUtility(mintIndex)) {
                    battlers[mintIndex] = Battler(block.timestamp, false);
                    rewardableBalanceOf[_msgSender()]++;
                }
                //not a utility battler and should automanifest
                else if (autoManifest) {
                    battlers[mintIndex] = Battler(block.timestamp, true);
    ...
    function mint(uint numberOfTokens, bool autoManifest) public payable {
        _mint(numberOfTokens, autoManifest, mintPrice);
    }

    function mintWhitelist(uint numberOfTokens, bool autoManifest, bytes32[] calldata _wlProof) public payable {
        require(inWhitelist(_wlProof), "not in whitelist");
        _mint(numberOfTokens, autoManifest, mintWhitelistPrice);
    }

    //Only alpha species can hyperwarp a new NFT
    function hyperwarp(uint256 _jumpClone, uint256 _assist) external returns (uint256 mintIndex) {
        require(ownerOf(_jumpClone) == _msgSender() && ownerOf(_assist) == _msgSender(), "Must own both NFTs");
        ...
        require(!hyperwarpManager.battlerHasUtility(mintIndex), "Cannot hyperwarp utility species.");
        battlers[mintIndex] = Battler(block.timestamp, false);
        _safeMint(_msgSender(), mintIndex);
    ...
    function manifest(uint256 _tokenId) external {
        require(ownerOf(_tokenId) == _msgSender() || owner() == _msgSender(), "Must own NFT or be contract owner.");
        Battler storage battler = battlers[_tokenId];
        require(!battler.isManifested, "Battler has already manifested.");
    ...
    function getReward() external {
        require(rewardableBalanceOf[_msgSender()] > 0, "Must own utility species");
    ...
    function _beforeTokenTransfer(address from, address to, uint256 tokenId) internal override(ERC721, ERC721Enumerable) {
        ...
        if (tokenId > 30 && from != address(0) && to != address(0)) {
            //@TODO should the line below be moved inside of the battlerHasUtility condition?
            utilityHub.transferReward(from, to, rewardTokenIndex);
            if (hyperwarpManager.battlerHasUtility(tokenId)) {
