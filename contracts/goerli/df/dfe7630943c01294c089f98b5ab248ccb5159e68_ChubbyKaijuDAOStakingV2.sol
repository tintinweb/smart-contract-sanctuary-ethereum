...
contract ChubbyKaijuDAOStakingV2 is IChubbyKaijuDAOStakingV2, Ownable, IERC721Receiver, Pausable, ReentrancyGuard {
  ...
  function _stakeGEN2(address account, uint16 tokenId, uint48 stakeTime, bytes memory signature) internal {
    ...
    uint256 timeStake = _createTimeStake(account, tokenId, stakeTime, trait, rewardType);
    _setMap(tokensByAddressBitMap[account], tokenId, STAKE);
    timeStakeByToken[tokenId] = timeStake;
  ...
  function GEN2depositsOf(address account) external view returns (uint16[] memory) {
    uint256[40] storage bitMap = tokensByAddressBitMap[account];
    uint16 tokenNum = _getTokenNum(bitMap);
    uint16[] memory tokenIds = new uint16[] (tokenNum);
    uint16 tokenId = 1;
    uint16 tokenIdIndex = 0;
    ...
    for (uint16 i = 0; i < 40; i++) {
      ...
      uint256 iterator = 1;
      uint256 mapPortion = bitMap[i];

      while (iterator != 0) {
        uint256 bit = mapPortion & iterator;
        if (bit > 0) {
          tokenIds[tokenIdIndex++] = tokenId;
        }
        tokenId++;
        iterator << 1;
      }
