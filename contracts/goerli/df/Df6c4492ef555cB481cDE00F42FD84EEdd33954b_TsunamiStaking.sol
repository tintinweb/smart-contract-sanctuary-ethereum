...
contract TsunamiStaking is Ownable {
    ...
    constructor(IERC20 _tsunami, uint256 _rewardPerSecond) {
        tsunami = _tsunami;
    ...
    function pendingRewards(address _user) external view returns (uint256) {
        ...
        uint256 balance = tsunami.balanceOf(address(this));
    ...
    function _claimRewards(uint256 amount, uint256 rewardDebt) internal returns (uint256 amountToSend) {
        ...
        uint bal = tsunami.balanceOf(address(this)) - totalStaked;
        amountToSend = totalRewards > bal ? bal : totalRewards;
        IERC20(tsunami).transfer(msg.sender, amountToSend);
    ...
    function deposit(uint256 _tokenAmount) external {
        ...
        if (_tokenAmount > 0) {
            tsunami.safeTransferFrom(address(msg.sender), address(this), _tokenAmount);
    ...
    function withdraw(uint256 _tokenAmount) external {
        ...
        if (_tokenAmount > 0) {
            user.amount -= _tokenAmount;
            tsunami.safeTransfer(address(msg.sender), _tokenAmount);
    ...
    function finishPool() external onlyOwner {
        ...
        if (totalStaked > rewardSupply) {
            IERC20(tsunami).transfer(owner(), totalStaked - rewardSupply);
        }
    }
