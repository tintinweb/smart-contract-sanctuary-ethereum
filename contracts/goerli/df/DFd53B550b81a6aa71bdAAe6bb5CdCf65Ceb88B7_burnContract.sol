...
contract burnContract is ERC721AQueryable, Ownable, RevokableDefaultOperatorFilterer, ReentrancyGuard {
    ...
    mapping(uint256 => uint256) public tokenMappingRegular;
    mapping(uint256 => uint256) public tokenMappingSOTY;
    ...
    function burnToken(uint256[] memory tokenIds) public nonReentrant {
        ...
        for(uint256 i = 0; i < tokenIds.length; i++)
            _burnOGandMint(tokenIds[i]);
        
    }

    function _burnOGandMint(uint256 tokenId) internal {
        ...
        tokenMappingRegular[mintId] = regularMinted; 
    ...
    function claimSOTY(address wallet, bytes calldata voucher, bool delegate) public nonReentrant {
        ...
        tokenMappingSOTY[mintId] = SOTYMinted;
        tokenType[mintId] = 1;
    ...
    function tokenURI(uint256 tokenId) public view virtual override(IERC721A, ERC721A) returns (string memory) {
        ...
        uint regular = tokenMappingRegular[tokenId];
        uint soty = tokenMappingSOTY[tokenId];
        uint path = tokenType[tokenId];

        if(path == 0) return string(abi.encodePacked(baseURIRegular, Strings.toString(regular)));
        else return string(abi.encodePacked(baseURISOTY, Strings.toString(soty)));
	}
