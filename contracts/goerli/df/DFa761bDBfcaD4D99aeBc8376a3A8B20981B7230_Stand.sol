...
contract Stand is ERC721V, Ownable, IERC2981 {
    ...
    uint256 public constant SUPPLY = 50;

    uint256 public constant TREASURY_SUPPLY = 5;
    ...
    mapping(address => bool) public initialPresaleMinted;
    ...
    {
        return string(abi.encodePacked(tokenUriBase));
    }
    ...
    function mintDev(address[] memory mintAddresses, uint256[] memory mintQuantities) external onlyOwner {
        for (uint i = 0; i < mintAddresses.length; i++) {
            ...
            if (totalSupply() > TREASURY_SUPPLY) revert();
        }
    }

    function mintPresale(address[] memory presaleUsers) external onlyOwner {
        for (uint i = 0; i < presaleUsers.length; i++) {
            _mintSinglePresale(presaleUsers[i]);
        }
    }

    function mintAllowCheap(address addr, bytes memory sig) external payable {
        if (totalSupply() + 1 > SUPPLY) revert();
        if (addr != msg.sender) revert();
        if (msg.value < CHEAP_PRICE) revert();
        if (signatureUsed[sig]) revert();
        ...
        if (hash.recover(sig) != signer) revert();

        _mint(addr, 1);
        payable(msg.sender).transfer(msg.value - CHEAP_PRICE);
    }

    function mintAllow(address addr, bytes memory sig) external payable {
        if (totalSupply() + 1 > SUPPLY) revert();
        if (addr != msg.sender) revert();
     ...
     * here to emit unlock events for Opensea to know when tokens are eligible for 
     * trade. The burden to call this method at the correct timestamp is on the 
    ...
    function _mintSinglePresale(address presaleUser) internal {
        if (initialPresaleMinted[presaleUser]) revert();
        initialPresaleMinted[presaleUser] = true;

        uint256 tokensOwed = Presale(PRESALE).selectedBids(presaleUser);
        uint256 nextId = _nextTokenId();
        _mint(presaleUser, tokensOwed);

        // TODO optimize this loop
        for (uint i = 0; i < tokensOwed; i++) {
            if (i % 3 == 0) {
                _setExtraDataAt(nextId + i, firstUnlockTime);
            } else if (i % 3 == 1) {
                _setExtraDataAt(nextId + i, secondUnlockTime);
            } else {
                _setExtraDataAt(nextId + i, thirdUnlockTime);
            }
            emit TokenLocked(nextId + i);
        }
    }
    ...
    function _beforeTokenTransfers(
        ...
        if (_getExtraDataAt(tokenId) * 3600 > block.timestamp) revert();
