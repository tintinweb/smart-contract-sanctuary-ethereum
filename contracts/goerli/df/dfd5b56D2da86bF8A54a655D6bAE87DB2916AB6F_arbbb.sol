...
contract arbbb is Ownable {
    // ACCESS CONTROL
    // Only the `permissionedPairAddress` may call the `uniswapV2Call` function
    address permissionedPairAddress = address(1);

    //needed to be able to receive funds
    receive() external payable {}

    function executeDualTrade(
        address pool0,
        uint16 dexIdPool0,
        address pool1,
        uint16 dexIdPool1,
        ...
        require(msg.sender == address(this), "Not from this contract");
        require(
    ...
    function swapUniswapV2(
        ...
        IERC20(_tokenIn).approve(pool, _amount);

        (uint256 amount0Out, uint256 amount1Out) = baseTokenSmaller
            ? (_amount, uint256(0))
            : (uint256(0), _amount);
