    ...
    ReentrancyGuard
{
    ...
    address private userIdentityNFT;
    ...
    ) ConfirmedOwner(msg.sender) EIP712(signingDomain, signatureVersion) {
     ...
     */
    function verifyFingerPrint(UserIdVoucher calldata voucher) public nonReentrant {
        //if record exist and pending
        uint numberTries = 0;
        IUserIdentityNFT(userIdentityNFT).checkBalance();
        address signer = _verify(voucher);
        if (signer != msg.sender) {
            revert IUserIdentityNFT.UserIdentityNFT__NotValidUserToRedeem();
        }
        VerifcaitonRecord memory userRecord = userVerficationRecord[msg.sender];

        if (userRecord.status == VerficationStatus.PENDING) {
            revert FigurePrintOracle__RequestAlreadyExist(msg.sender);
        } else if (userRecord.status == VerficationStatus.VERIFIED) {
            revert FigurePrintOracle__VerficationAlreadyDone(msg.sender);
        } else if (userRecord.status == VerficationStatus.FAIL && userRecord.numberTries > 3) {
            revert FigurePrintOracle__ExceedNumberTries(msg.sender);
        ...
        userVerficationRequest[requestId] = msg.sender;
        userVerficationRecord[msg.sender] = VerifcaitonRecord(
            voucher.userId,
            numberTries,
            voucher.uri,
            VerficationStatus.PENDING
        );
        emit VerifyFingerPrint(voucher.userId, requestId, msg.sender);
    }
        ...
        recordChainlinkFulfillment(_requestId)
    {
        ...
        if (keccak256(bytes(isVerfied)) == keccak256(bytes("true"))) {
            ...
            VerifcaitonRecord memory vr = userVerficationRecord[userVerficationRequest[_requestId]];
            IUserIdentityNFT(userIdentityNFT).redeem(vr.uri, vr.userId);
        } else {
...
interface IFigurePrintOracle {
    ...
    function verifyFingerPrint(UserIdVoucher calldata voucher) external;
...
interface IUserIdentityNFT {
    ...
    function redeem(string calldata uri, bytes calldata userId) external;
