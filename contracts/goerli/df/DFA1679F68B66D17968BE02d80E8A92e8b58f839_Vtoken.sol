...
contract Vtoken is AccessControl, ERC20, ReentrancyGuard, IVToken {
    ...
    modifier onlyMinter() {
        require(hasRole(MINTER_ROLE, msg.sender), "VestedToken:MINT_DENIED");
        _;
    }

    modifier onlyAdmin() {
        require(hasRole(ADMIN_ROLE, msg.sender), "VestedToken:ACCESS_DENIED");
        _;
    }
    ...
    function mint(address account, uint256 initialAmount) public override onlyMinter {
        require(totalSupply() + token.totalSupply() + initialAmount <= token.MAX_SUPPLY(), "Vested:SUPPLY_OVERFLOW");
    ...
    function claim(address recipient) external override nonReentrant {
        ...
        require(claimable > 0, "Vested:NOTHING_VESTED");
        token.mint(recipient, claimable);
    }
    ...
    function _beforeTokenTransfer(
        ...
        require(from == address(0) || to == address(0), "Vested:TRANSFER_DENIED");
    }
}