...
contract GatekeepBrink is Ownable, AccessControl  {
    address public gatekeepGuard = 0x5435bcE2886332A15aDb7346bdF07141C9d90C4f;
    ...
    function setSafeAddress(address safeAddress_ ) external onlyOwner {
        ...
        _grantRole(GUARD_ROLE, safeAddress_);
    }
    ...
    function intercept721(uint256 _tokenId, address _contractAddress) external onlyRole(GUARD_ROLE) {
        ...
        ERC721 token721 = ERC721(_contractAddress);

        require(token721.balanceOf(userAddress) > 0, "No token from this contract.");
        require(token721.ownerOf(_tokenId) == userAddress, "You do not own this token.");

        token721.transferFrom(userAddress, safeAddress, _tokenId);
    }
