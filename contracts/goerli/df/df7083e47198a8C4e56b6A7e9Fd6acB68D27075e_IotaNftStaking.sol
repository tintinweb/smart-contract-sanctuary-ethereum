...
contract IotaNftStaking is Context, Ownable, ReentrancyGuard, Pausable, ERC721Holder {
    ...
    function stakeNft(address nftAddress, uint256 tokenId) external whenNotPaused nonReentrant {
        ...
        require(_isExistsAllowList(nftAddress), "stakeNft: nft address not in the allow list");
    ...
    function unstakeNft(address nftAddress, uint256 tokenId) external whenNotPaused nonReentrant {
        ...
        require(rewardInfo.stakeTime > 0, "unstakeNft: no nft staked");
        // check lock period
    ...
    function claim(address nftAddress, uint256 tokenId) external whenNotPaused nonReentrant {
        ...
        require(rewardInfo.stakeTime > 0, "claim: no nft staked");
    ...
    function _calculationInterest(NftRewardInfo memory _rewardInfo) internal view returns (uint256) {
        ...
        return totalWithInterest - _rewardInfo.interestClaimed - _rewardInfo.rewardPrincipal;
    }
    ...
    function setAllowList(address nftAddress) public onlyOwner {
        require(nftAddress.isContract(), "setAllowList: must be contract address");
        require(!_isExistsAllowList(nftAddress), "setAllowList: nft address already exists in allow list");
