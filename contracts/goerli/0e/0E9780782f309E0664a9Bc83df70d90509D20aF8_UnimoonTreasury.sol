...
contract UnimoonTreasury is AccessControl, ReentrancyGuard {
        ...
        returns (uint256)
    {
        ...
        uint256 beforeTokenBalance = IToken(path[path.length - 1]).balanceOf(
            address(this)
        );
        uint256[] memory amounts = ROUTER.swapExactTokensForTokens(
            amount,
        ...
        beforeTokenBalance =
            IToken(path[path.length - 1]).balanceOf(address(this)) -
            beforeTokenBalance;
        ...
        returns (uint256)
    {
        ...
        if (amounts[amounts.length - 1] > oldValue) {
            profit = amounts[amounts.length - 1] - oldValue;
