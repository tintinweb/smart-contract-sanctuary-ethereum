...
contract USDCTest is ERC721, Ownable, ReentrancyGuard {

    using Strings for uint256;
    using SafeMath for uint256;
    using SafeERC20 for IERC20;

    // @dev this is the USDC Goerli address
    address public constant USDCAddress = 0x07865c6E87B9F70255377e024ace6630C1Eaa37F;
    IERC20 USDC = IERC20(USDCAddress);

    event Bought(address indexed from);

    constructor() ERC721("USDCTest", "USDCT") { }

    function approveSpending() external onlyOwner {
        USDC.approve(address(this), 3);
    }

    function buy () external onlyOwner {
        require(USDC.transferFrom(msg.sender, address(this), 3), "TRANSACTION_FAILED");
