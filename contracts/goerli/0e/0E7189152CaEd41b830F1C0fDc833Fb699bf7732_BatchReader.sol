...
contract BatchReader 
{
    ...
    function readWalletOwnedMoments(
        ...
        for(uint x; x < ProjectIDs.length; x++)
        {
            ...
            uint ProjectCurrentTokenID = ProjectIDs[x] * 1000000;
            uint Range = ProjectCurrentTokenID + 100;
            uint NumOwnedTokens;
            for(uint y = ProjectCurrentTokenID; y <= Range; y++)
            {
                try Moments.ownerOf(y) 
                {
                    if(Moments.ownerOf(y) == Wallet)
                    {
            ...
            {
                FormattedOwnedIDs[FormattedIndex] = temp[z];
                FormattedIndex++;   
            }
            OwnedIDs[x] = FormattedOwnedIDs;
        }
