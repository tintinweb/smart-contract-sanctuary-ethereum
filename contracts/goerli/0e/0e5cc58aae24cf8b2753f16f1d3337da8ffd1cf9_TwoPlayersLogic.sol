...
contract TwoPlayersLogic is VRFConsumerBaseV2 {
    ...
    function move2(
        ...
        games[name_][room][id_].move2 = move_;
    ...
    function claim(
        ...
        if (msg.sender == game.player2 && block.timestamp <= game.time2 + time) {
            revert("Result is not ready");
        }
    ...
    function getRoom(uint256 value_) internal pure returns(Room room) {
        if (value_ == 1) {
            room = Room.One;
        } else if (value_ == 10) {
            room = Room.Ten;
    ...
    function cancelSubscription(address receivingWallet) external onlyOwner {
        COORDINATOR.cancelSubscription(subId, receivingWallet);
        subId = 0;
    }
