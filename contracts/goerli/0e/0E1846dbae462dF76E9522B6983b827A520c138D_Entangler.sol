...
import "@openzeppelin/contracts/access/Ownable.sol"; // Included for OpenSea, not actually used
...
struct TokenType {
  ...
  Mandate[] mandates;
  mapping(string => string[]) baseUris; // storage schema to base uri[currUriIdx]
}
...
contract Entangler is ERC1155, ERC1155Receiver, IERC2981, AccessControl, Ownable {
  ...
  bool public beneficaryWithdrawlsPaused = false;


  address payable ownerWithdrawAddress;
  ...
  modifier preMintChecks(uint256 typeId, bool fungibility, uint256 _count) {
    require(tokenTypes[typeId].fungible == fungibility, "Wrong fungibility");
    require(tokenTypes[typeId].mintable, "Token not currently mintable");
    require(_count > 0, "Mint amount cannot be less than one");
    require(_count <= tokenTypes[typeId].maxMint, "Mint amount too high");
    require(tokenTypes[typeId].totalSupply + _count <= tokenTypes[typeId].supplyCap, "Mint amount exceeds total supply of the token type");
    require(msg.value == tokenTypes[typeId].cost * _count, "Not enough or too many funds provided");
    _;
  }
  ...
  modifier withdrawlsPaused() {
    require(!beneficaryWithdrawlsPaused);
    _;
  }
  ...
  constructor(
    ...
    address lockerAdminAddress,
    address accountantAdminAddress,
    address payable _ownerWithdrawAddress,
    ...
    ERC1155(defaultUri)
  {
    ...
    _grantRole(OPERATOR_ROLE, operatorAdminAddress);
    _grantRole(LOCKER_ROLE, lockerAdminAddress);
    _grantRole(ACCOUNTANT_ROLE, accountantAdminAddress);

    ownerWithdrawAddress = _ownerWithdrawAddress;
  ...
  function mintNonFungible(address to, uint256 typeId, uint256 count)
    public
    payable
    preMintChecks(typeId, false, count)
  {
  ...
  function mintFungible(address to, uint256 typeId, uint256 count)
    external
    payable
    preMintChecks(typeId, true, count)
  {
    ...
    payable
  {
    ...
    withdrawAmounts[recipient] += msg.value - royaltyAmount;
  ...
  function withdraw(address withdrawAddress) external {
    require(withdrawAmounts[withdrawAddress] > 0, "Account does not have any ether allocated to it");

    uint256 withdrawAmount = withdrawAmounts[withdrawAddress];
    withdrawAmounts[withdrawAddress] = 0;
    ...
    tokenCreationChecks(newTokenType, startingStorageSchemas, startingBaseUris)
  {
    ...
    if(newTokenType.initialMintAmount > 0) {
      if(newTokenType.fungible) {
        _mint(newTokenType.initialMintTo, newTokenType.id, newTokenType.initialMintAmount, "");
      } else {
        mintNonFungible(newTokenType.initialMintTo, newTokenType.id, newTokenType.initialMintAmount);
      }
    }
    ...
    setTokenRoyaltiesCheck(tokenId, _royalties)
  {
    ...
    withdrawAmounts[ownerWithdrawAddress] += tokenTypes[typeId].cost - royaltyAmount;
    emit royaltyReceived(royaltyAmount);
  }
  ...
  function setOwnerWithdrawAddress(address payable _ownerWithdrawAddress)
    external
    onlyRole(OPERATOR_ROLE)
  {
    ownerWithdrawAddress = _ownerWithdrawAddress;
  ...
  function toggleBeneficaryWithdrawls()
    external
    onlyRole(OPERATOR_ROLE)
  {
    beneficaryWithdrawlsPaused = !beneficaryWithdrawlsPaused;
  }
