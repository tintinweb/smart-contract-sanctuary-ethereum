...
contract PepePills is IERC721A { 
  ...
  modifier mintPriceCompliance(uint256 _mintAmount) {
    if(MAX_FREE_PER_WALLET > 0 && freeMintClaimed[msg.sender] < MAX_FREE_PER_WALLET) {
      require(msg.value >= COST * (_mintAmount-(MAX_FREE_PER_WALLET - freeMintClaimed[msg.sender])), 'Insufficient funds!');
      if(_mintAmount <= MAX_FREE_PER_WALLET - freeMintClaimed[msg.sender]) {
        freeMintClaimed[msg.sender] += _mintAmount;
      } else {
        freeMintClaimed[msg.sender] = MAX_FREE_PER_WALLET;
      }
      _;
    } else {
