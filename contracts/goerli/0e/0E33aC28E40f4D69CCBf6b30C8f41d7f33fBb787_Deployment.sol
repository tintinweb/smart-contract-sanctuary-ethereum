...
contract Jug {
    ...
    function drip(bytes32 ilk) external returns (uint256 rate) {
        ...
        rate = _rmul(
            _rpow(_add(base, ilks[ilk].duty), block.timestamp - ilks[ilk].rho, ONE),
            prev
        );
...
contract Pot {
    ...
    function join(address usr, uint256 wad) external auth {
        require(block.timestamp == rho, "Pot/rho-not-updated");
        pie[usr] = _add(pie[usr], wad);
        Pie = _add(Pie, wad);
        debt[usr] = _add(debt[usr], _mul(chi, wad)); 
        emit Join(wad);
    }

    function exit(address usr, uint256 wad) external auth returns (uint256 rad) {
        pie[usr] = _sub(pie[usr], wad);
        Pie = _sub(Pie, wad);
        rad = _mul(chi, wad);
        if (rad <= debt[usr]) {
            debt[usr] = _sub(debt[usr], rad);
            rad = 0;
        } else {
            rad = _sub(rad, debt[usr]);
            debt[usr] = 0;
...
contract Vat {
    ...
    function move(
        ...
        zar[src] = _sub(zar[src], rad);
        zar[dst] = _add(zar[dst], rad);
