...
contract BSKR is IERC20, Ownable, Manageable, Pausable {
    ...
    address public constant _DEXV2_ROUTER_ADDR =
        0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D; // UniswapV2Router02
    address public constant _DEXV3_FACTORY_ADDR =
        0x1F98431c8aD98523631AE4a59f267346ea31F984; // UniswapV3Factory
    address public constant _DEXV3_ROUTER_ADDR =
        0xE592427A0AEce92De3Edee1F18E0157C05861564; // SwapRouter
    address public constant _NFPOSMAN_ADDR =
        0xC36442b4a4522E871399CD717aBDD847Ab11FE88; // NonfungiblePositionManager
    uint8 public constant _DECIMALS = 18;

    uint16 public constant _BIPS = 10**4; // bips or basis point divisor
    // uint256 public constant _TOTAL_SUPPLY = (10 ** 12) * (10 ** _DECIMALS); // 1 trillion for PulseChain
    uint256 public constant _TOTAL_SUPPLY = (10**9) * (10**_DECIMALS); // 1 billion for Goerli

    IUniswapV2Factory public immutable _dexFactoryV2;
    IUniswapV2Router02 public immutable _dexRouterV2;
    IUniswapV3Factory public immutable _dexFactoryV3;
    address public immutable _growthAddress; // 0x4F06FCcAa501B7BB9f9AFcEFb20f7862Be050B7d
    address public immutable _paydayAddress; // 0x13D44474B125B5582A42a826035A99e38a4962A7
    address public _ammBSKRPair;
    address public _ammLBSKRPair;

    address public _LBSKRContract;

    address[] public _sisterOAs;
    address[] public _noRfi;

    bool public _addingLiquidity;
    bool public _addLPEnabled = true;
    mapping(address => bool) public _isAMMPair;
    mapping(address => bool) public _getsNoRfi;
    mapping(address => bool) public _paysNoFee;
    mapping(address => mapping(address => uint256)) public _allowances;
    mapping(address => uint256) public _balances;
    mapping(address => uint256) public _rBalances;
    string public _name;
    string public _symbol;
    uint16[6] public _currFees = [uint16(200), 150, 100, 50, 50, 550];
    uint256 public _rTotal = (type(uint256).max -
        (type(uint256).max % _TOTAL_SUPPLY));
    uint256 public _tRfiFeesSum;
    uint256 public _numTokensForLP = _TOTAL_SUPPLY / 1000; // 0.1% of the total supply
    uint256 public _maxTxAmount = _TOTAL_SUPPLY / 25; // 4% of the total supply
    uint8 public _oaIndex;
    ...
    function _approve(
        ...
        require(owner != address(0), "FZA1");
        require(spender != address(0), "TZA1");
    ...
    function _isUniswapV2Pair(address target) private returns (bool) {
        // TODO can be set to view
        address token0;
        address token1;
    ...
    function _spendAllowance(
        ...
        if (currentAllowance != type(uint256).max) {
            require(currentAllowance >= amount, "IA1");
            unchecked {
    ...
    function _transfer(
        ...
        require(from != address(0), "FZA2");
        require(to != address(0), "TZA2");
        require(amount > 0, "ZTA1");
        ...
        returns (bool)
    {
        address owner = _msgSender();
        uint256 currentAllowance = allowance(owner, spender);
        require(currentAllowance >= subtractedValue, "DTM1");
        unchecked {
