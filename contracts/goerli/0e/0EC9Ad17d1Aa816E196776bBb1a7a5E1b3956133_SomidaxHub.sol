...
contract SomidaxHub is ReentrancyGuard {
    ...
    function transfer(
        ...
        if (
            ...
            require(sent, "Unable to send eth");
    ...
    function depositEth(bytes32 symbol) external payable {
    ...
    function depositSmdx(uint amount, bytes32 symbol) external {
        IERC20(SMDXADDRESS).transferFrom(msg.sender, address(this), amount);
        s_procceeds[msg.sender].smdxBalance += amount;
    ...
    function withdraw_eth_proceeds(bytes32 symbol) external payable {
        s_procceeds[msg.sender].ethBalance = 0;
        (bool sent, ) = payable(msg.sender).call{value: msg.value}("");
        require(sent, "Failed to send Ether");

        emit Withdraw(msg.sender, symbol, msg.value);
    }

    function withdraw_smdx_proceeds(uint256 amount, bytes32 symbol) external {
        s_procceeds[msg.sender].smdxBalance = 0;
        IERC20(SMDXADDRESS).transfer(msg.sender, amount);

        emit Withdraw(msg.sender, symbol, amount);
    }

    function buy_cofee(address userAddress) public payable {
        (bool sent, ) = address(this).call{value: msg.value}("");

        require(sent, "Failed to send Ether");
        s_procceeds[userAddress].ethBalance += msg.value;
        emit buyCofee(userAddress, msg.sender, msg.value);
    }
