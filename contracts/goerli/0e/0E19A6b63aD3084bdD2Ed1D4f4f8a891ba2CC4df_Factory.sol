...
contract NFTcontract is ERC721Enumerable, ReentrancyGuard {
    ...
    mapping(uint256 => string) public arrBaseURIs;
    mapping(uint256 => uint256) public stageTime;
    ...
    uint256 public cntOfPreMints;
    uint256 public cntOfPublicMints;
    uint256 private smallestCntOfPreMints;
    uint256 private smallestCntOfPublicMints;
    ...
    constructor(
        ...
        cntOfPublicMints = 0;
        publicMintId = 1;
        smallestCntOfPreMints = _maxPublicMint;
        smallestCntOfPublicMints = _maxPublicMint;
    ...
    modifier isCorrectPayment(uint256 stage, uint256 numberOfTokens) {
        ...
        if (stage == 0) {
            for (i = 0; i < numberOfTokens; i++) {
                ...
                if (tokenId < smallestCntOfPreMints) {
                    index = smallestCntOfPreMints;
                } else {
                    for (index = tokenId; index <= maxPublicMint; index++) {
                        if (keccak256(abi.encodePacked(PRE_MINT_PRICES[index])) != keccak256(abi.encodePacked(""))) {
                            break;
                        }
                    }
                }
            ...
            for (i = 0; i < numberOfTokens; i++) {
                tokenId = publicMintId + i;
                if (tokenId < smallestCntOfPublicMints) {
                    index = smallestCntOfPublicMints;
                } else {
                    for (index = tokenId; index <= maxPublicMint; index++) {
                        if (keccak256(abi.encodePacked(PUBLIC_MINT_PRICES[index])) != keccak256(abi.encodePacked(""))) {
                            break;
                        }
                    }
                }
    ...
    function updateOperatorsFilter(address[] calldata _operators, bool[] calldata _allowed) external onlyOwner {
        require(_operators.length > 0, "Please input at least one operator address.");
    ...
    function publicMint(uint256 numberOfTokens, uint256 stage)
        ...
        nonReentrant
    {
        require(stage < 2, "Invalid stage info.");
        require(stageTime[stage] > 0, "Please input valid stage time.");
        require(
        ...
        onlyOwner
    {
        ...
        require(
            _stageTimes.length == 2,
            "Please input two stageTimes, pre-mint & mint stage."
        );
    ...
    function updateFixedModelStagesInfo(
        ...
        require(_premintPrice > 0 && _publicMintPrice > 0, "Please input valid prices greater than 0.");
    ...
    function updateDynamicModelStagesInfo(
        ...
        for (index = 0; index < _arrPremintPrices.length; index++) {
            require(cntOfPreMints + _arrPremintTokens[index] <= maxPublicMint, "Total count of premint was overflow.");
            cntOfPreMints += _arrPremintTokens[index];
            if (smallestCntOfPreMints > cntOfPreMints) {
                smallestCntOfPreMints = cntOfPreMints;
            }
            PRE_MINT_PRICES[cntOfPreMints] = _arrPremintPrices[index];
        }
        for (index = 0; index < _arrPublicMintPrices.length; index++) {
            require(cntOfPublicMints + _arrPublicmintTokens[index] <= maxPublicMint, "Total count of publicmint was overflow.");
            cntOfPublicMints += _arrPremintTokens[index];
            if (smallestCntOfPublicMints > cntOfPublicMints) {
                smallestCntOfPublicMints = cntOfPublicMints;
            }
