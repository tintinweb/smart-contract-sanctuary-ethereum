...
contract HordETHStakingManager is HETH {
    ...
    // Represent maximal user deposit
    ...
    // Map validator address to his information
    ...
    event EtherReceived(address sender, uint256 amountETH);
    ...
    onlyMaintainer
    {
        ...
        require(liquidationReserves >= stakingConfiguration.amountETHInValidator(), "conditions are not met");
    ...
    whenNotPaused
    {
        ...
        if(user.amountDeposited > maxUserDeposit) {
            maxUserDeposit = user.amountDeposited;
        }
    ...
    /**
         * @notice          Function returns is liquidation reserves are running out
     */
    function isReservesRunningOut() view public returns (bool) {
        return liquidationReserves < maxUserDeposit;
    }
    ...
    function getAmountOfHETHforETH(uint256 amountETH) public view returns (uint256) {
        ...
        uint256 totalETHInPossession = totalRewardsCollected
            .add(amountETHInValidators)
            .add(liquidationReserves);

        uint256 result = amountETH.mul(totalHETHMinted).div(totalETHInPossession); // ETH x HETH / ETH = HETH
     ...
     */
    function setTotalRewardsCollected(
        uint256 rewardsAmount
    ) external {
        require(msg.sender == stakingConfiguration.permissionAddress(), "Only permission address");
        totalRewardsCollected = totalRewardsCollected.add(rewardsAmount);
    }
