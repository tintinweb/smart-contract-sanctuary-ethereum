...
import "./libraries/interfaces/IFundingRate.sol";
...
contract Vault {
    ...
    uint256 public liquidationFeeUsd = 5 * USD_PRECISION;
    ...
    IFundingRate public fundingRateContract; 

    constructor(address _fundingRateAddress) {
        ...
        errors[8] = "Action triggered position to become unhealthy";
        fundingRateContract = IFundingRate(_fundingRateAddress);
    }
    ...
    function increasePosition(
        ...
        // added the variables, if its a new position should basically have 0 for position size and 0 for funding rate
        uint256 feeUSD = _collectMarginFees(asset, sizeDelta, position.size, position.entryFundingRate);
        ...
        // funding fee insertion
        position.entryFundingRate = fundingRateContract.cumulativeFundingRates(asset);
        ...
        (uint256 liquidationState,) = _validateLiquidation(asset, price);
        _validate(liquidationState == 0, 8);
    ...
    function _validateLiquidation(
        ...
        uint256 fees = _computeMarginFee(position.size) + liquidationFeeUsd;
    ...
    function _reduceCollateral(
        ...
        // added in the two other parameters
        uint256 feeUSD = _collectMarginFees(asset, _sizeDelta, position.size, position.entryFundingRate);
    ...
    function _collectMarginFees(
        ...
        // should we rename this function? THis is the _getPositionFee function so idk ? we want to show margin fees include both
        ...
        // so either way, the funding rate relies on it either going up by a certain amount every hour or there is no funding fee
        uint256 fundingFee = getFundingFee(asset, _size, _entryFundingRate);
    ...
    function getFundingFee(string memory asset, uint256 _size, uint256 _entryFundingRate) public view returns (uint256) {
        if (_size == 0) { return 0; }

        uint256 fundingRate = fundingRateContract.cumulativeFundingRates(asset).sub(_entryFundingRate);
...
interface IFundingRate {
  ...
  function BITMASK_32 (  ) external view returns ( uint256 );
  function CUMULATIVE_DELTA_PRECISION (  ) external view returns ( uint256 );
  function FAKE_USDC (  ) external view returns ( address );
  ...
  function asset ( uint256 ) external view returns ( string memory );
  function cumulativeFundingRates ( string memory) external view returns ( uint256 );
  function fundingInterval (  ) external view returns ( uint256 );
  function fundingRateFactor (  ) external view returns ( uint256 );
  function getFundingFee ( string memory _asset, uint256 _size, uint256 _entryFundingRate ) external view returns ( uint256 );
  function getNextFundingRate ( string memory _asset ) external view returns ( uint256 );
  function gross_exposure ( string memory) external view returns ( uint256 );
  function initializeStarterFundingRate ( string memory _asset ) external returns ( uint256 );
  function lastFundingTimes ( string memory) external view returns ( uint256 );
  function lastUpdatedAt (  ) external view returns ( uint256 );
  function lastUpdatedBlock (  ) external view returns ( uint256 );
  function maxDeviationBasisPoints (  ) external view returns ( uint256 );
  function maxPriceUpdateDelay (  ) external view returns ( uint256 );
  function maxTimeDeviation (  ) external view returns ( uint256 );
  function minBlockInterval (  ) external view returns ( uint256 );
  function net_exposure ( string memory) external view returns ( uint256 );
  function poolAmounts ( address ) external view returns ( uint256 );
  function priceDataInterval (  ) external view returns ( uint256 );
  ...
  function stepDownFunding ( string memory _asset, uint256 _howMuch ) external returns ( uint256 );
  function stepUpFunding ( string memory _asset, uint256 _howMuch ) external returns ( uint256 );
  function tokenPrecisions ( uint256 ) external view returns ( uint256 );
  function tokens ( uint256 ) external view returns ( address );
  function updateCumulativeFundingRate ( string memory _asset ) external;
}
