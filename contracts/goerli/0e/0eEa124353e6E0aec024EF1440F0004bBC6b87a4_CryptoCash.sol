...
interface ERC20 {
  ...
  function approveAndCall(address spender, uint256 tokens, bytes calldata data) external returns (bool success);
...
contract CryptoCash is ERC20 {
  using SafeMath for uint256;

  mapping (address => uint256) private balances;
  mapping (address => mapping (address => uint256)) private allowed;
  string public constant name = "CryptoCash";
  string public constant symbol = "CRYPTOCASH";
  uint8 public constant decimals = 18;
  
  address owner = msg.sender;

  uint256 private _totalSupply = 10000000 * (10 ** 18); // 1 billion supply
