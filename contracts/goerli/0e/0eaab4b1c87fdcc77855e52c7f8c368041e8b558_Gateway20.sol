...
contract Gateway20 {
    ...
    function bridgeFromStarknetWidthdraw(
        ERC20 _l1TokenContract,
        ...
        mint(_amount, name, symbol);
    }

    function bridgeToStarknetDeposit(
        address _l2Recipient,
        uint256 _amount,
        string memory name,
        string memory symbol
    ) external {
        bytes memory bytecode = this.getBytecode(name, symbol);
        address newContractAdd = this.getAddress(bytecode, 0x123456788);
        uint256 size = 2;
        uint256[] memory payload = new uint256[](size);
        // build withdraw message payload
        payload[0] = addressToUint(address(_l2Recipient));
        payload[1] = _amount;

        // consume withdraw message
        starknetCore.sendMessageToL2(endpointGateway,ENDPOINT_GATEWAY_SELECTOR, payload);
        BridgeErc20 bridge = BridgeErc20(newContractAdd);
        bridge.burn(msg.sender, _amount);
    }
    ...
    function mint(
        ...
        BridgeErc20 bridge = BridgeErc20(newContractAdd);
        bridge.mint(msg.sender, _amount);
    }

    // 1. Get bytecode of contract to be deployed
    function getBytecode(string memory name,
        string memory symbol) external view returns (bytes memory) {
        bytes memory bytecode = type(BridgeErc20).creationCode;
...
contract BridgeErc20 is ERC20 {
    address public gatewayAddress;

    constructor(address _gatewayAddress, string memory _name, string memory _symbol)
        ERC20(
            _name, _symbol
        )
    {
