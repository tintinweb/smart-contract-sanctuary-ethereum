...
contract FixedValueLock is BaseGovernanceWithUserUpgradable, ERC721EnumerableUpgradeable {
    ...
    function addBeneficiaries(bytes calldata data, uint256 totalAmount) external onlyBeneficiaryManager {
        require(canAddBeneficiaries == true, "ERROR: Cannot add new beneficiaries");
    ...
    */
    ...
    //     bool _success;
    //     uint256 _ID;
    //     if(ownershipProof.length != 0) {
    //         (_success, _ID) = depositManager.verifyOwnership(_msgSender(), ownershipProof);
    //         require(_success, "ERROR: You are not the owner or are approved for this NFT");
    //         nftId = _ID;
    //     }
        
    //     _success = _isApprovedOrOwner(_msgSender(), nftId);
    //     require(_success, "ERROR: You are not the owner or are approved for this NFT");
    //     (, uint256 additionTime, uint256 initialAmount, uint256 claimed) =  depositManager.getProperties(nftId);
    //     uint256 unlockedAmount = schedule.unlockedAmount(initialAmount) - claimed;
    //     depositManager.updateClaimedAmount(nftId, unlockedAmount);
    //     tokenERC20.safeTransfer(_msgSender(), unlockedAmount);
    //     emit Claimed(nftId, _msgSender(), unlockedAmount);
     ...
     * @dev The data to proof the ownership is only require if the NFT is not minted yet
     */
    function split(uint256 originId, uint256[] memory splitParts, address[] memory addresses) external {
    ...
    function _withdrawRemainingLocked() internal {
    ...
    // function stillLockedAmounts() public view returns(uint256){
    //     uint256 stillLockedAmt = totalLockedAmount - schedule.unlockedAmount(totalLockedAmount);
    //     //uint256 stillLockedAmtInit = totalLockedAmount - schedule.unlockedAmount(totalLockedAmount);
    //     return stillLockedAmt;
    // }
    ...
    // function debugDetails() public view returns(bool, bool, uint256, uint256, uint256){
        
    //     bool isOwnerAsBen = false;
    //     if(schedule.ifOwnerAsBen()){
    //         isOwnerAsBen = true;
    //     }
    //     uint256 undistributedAmount = (totalLockedAmount - schedule.unlockedAmount(totalLockedAmount));

    //     return (schedule.ifOwnerAsBen(), isOwnerAsBen, assignedAmount, undistributedAmount, totalLockedAmount);
    // }
    ...
    function _setNFTs(address[] memory _addresses, uint256 _numIDs) internal returns(uint256[] memory){
        uint256[] memory IDs = new uint256[](_numIDs);
