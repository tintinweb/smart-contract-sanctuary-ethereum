...
contract test is  ERC721, ReentrancyGuard, OwnableRoles, DefaultOperatorFilterer, ERC2981{
  ...
  bool public tester = true;
    ...
    /** Planet Sale Status**/
      ...
      bool private Planet2SaleOpen = true;
      bool private Planet3SaleOpen = true;
      bool private Planet4SaleOpen = true;
      bool private Planet5SaleOpen = true;
      bool private Planet6SaleOpen = true;
      bool private Planet7SaleOpen = true;
      bool private Planet8SaleOpen = true;
      bool private Planet9SaleOpen = true;
      bool private Planet10SaleOpen = true;
  ...
  function adminMint(uint256 planetNumber, address recepient, uint256 count) public  onlyRolesOrOwner(ADMIN_ROLE | DEV_ROLE){
    ...
    function RainbowMintPlanet1(uint256 planetNumber, address recepient, uint256 count, bytes32[] calldata _proof) public payable  {
     ...
     if (planetNumber == 1) {
        ...
        require(P2RainbowOpen, "Sale not active"); //Public sale  
           P2checks(recepient,count);
    } else if (planetNumber == 3) {
        require(P3RainbowOpen, "Sale not active"); //Public sale  
           P3checks(recepient,count);
    } else if (planetNumber == 4) {
        require(P4RainbowOpen, "Sale not active"); //Public sale  
           P4checks(recepient,count);
    } else if (planetNumber == 5) {
        require(P5RainbowOpen, "Sale not active"); //Public sale  
           P5checks(recepient,count);
    } else if (planetNumber == 6) {
        require(P6RainbowOpen, "Sale not active"); //Public sale  
           P6checks(recepient,count);
    } else if (planetNumber == 7) {
        require(P7RainbowOpen, "Sale not active"); //Public sale  
           P7checks(recepient,count);
    } else if (planetNumber == 8) {
        require(P8RainbowOpen, "Sale not active"); //Public sale  
           P8checks(recepient,count);
    }else if (planetNumber == 9) {
        require(P9RainbowOpen, "Sale not active"); //Public sale  
           P9checks(recepient,count);
    }  else if (planetNumber == 10) {
        require(P10RainbowOpen, "Sale not active"); //Public sale  
           P10checks(recepient,count);
    } 
...
// PLANET 2 CHECKS
  ...
  function P2checks(address recepient, uint256 count) internal {
    require(count - 1 < MAX_MULTIMINT, "Trying to mint too many at a time");
    require(msg.value >= PLANET_PRICE * count, "Insufficient payment");

    if (allowedPlanet2MintCount(recepient) > 0) {
      updatePlanet2MintCount(recepient, count);
    } else {
      revert("Minting limit exceeded");
    }
    P2Mint(recepient,count);
...
// PLANET 3 CHECKS
  ...
  function P3checks(address recepient, uint256 count) internal {
    require(count - 1 < MAX_MULTIMINT, "Trying to mint too many at a time");
    require(msg.value >= PLANET_PRICE * count, "Insufficient payment");

    if (allowedPlanet3MintCount(recepient) > 0) {
      updatePlanet3MintCount(recepient, count);
    } else {
      revert("Minting limit exceeded");
    }
    P3Mint(recepient,count);
...
// PLANET 4 CHECKS
  ...
  function P4checks(address recepient, uint256 count) internal {
    require(count - 1 < MAX_MULTIMINT, "Trying to mint too many at a time");
    require(msg.value >= PLANET_PRICE * count, "Insufficient payment");

    if (allowedPlanet4MintCount(recepient) > 0) {
      updatePlanet4MintCount(recepient, count);
    } else {
      revert("Minting limit exceeded");
    }
    P4Mint(recepient,count);
...
// PLANET 5 CHECKS
  ...
  function P5checks(address recepient, uint256 count) internal {
    require(count - 1 < MAX_MULTIMINT, "Trying to mint too many at a time");
    require(msg.value >= PLANET_PRICE * count, "Insufficient payment");

    if (allowedPlanet5MintCount(recepient) > 0) {
      updatePlanet5MintCount(recepient, count);
    } else {
      revert("Minting limit exceeded");
    }
    P5Mint(recepient,count);
...
// PLANET 6 CHECKS
  ...
  function P6checks(address recepient, uint256 count) internal {
    require(count - 1 < MAX_MULTIMINT, "Trying to mint too many at a time");
    require(msg.value >= PLANET_PRICE * count, "Insufficient payment");

    if (allowedPlanet6MintCount(recepient) > 0) {
      updatePlanet6MintCount(recepient, count);
    } else {
      revert("Minting limit exceeded");
    }
    P6Mint(recepient,count);
...
// PLANET 7 CHECKS
  ...
  function P7checks(address recepient, uint256 count) internal {
    require(count - 1 < MAX_MULTIMINT, "Trying to mint too many at a time");
    require(msg.value >= PLANET_PRICE * count, "Insufficient payment");

    if (allowedPlanet7MintCount(recepient) > 0) {
      updatePlanet7MintCount(recepient, count);
    } else {
      revert("Minting limit exceeded");
    }
    P7Mint(recepient,count);
...
// PLANET 8 CHECKS
  ...
  function P8checks(address recepient, uint256 count) internal {
    require(count - 1 < MAX_MULTIMINT, "Trying to mint too many at a time");
    require(msg.value >= PLANET_PRICE * count, "Insufficient payment");

    if (allowedPlanet8MintCount(recepient) > 0) {
      updatePlanet8MintCount(recepient, count);
    } else {
      revert("Minting limit exceeded");
    }
    P8Mint(recepient,count);
...
// PLANET 9 CHECKS
  ...
  function P9checks(address recepient, uint256 count) internal {
    require(count - 1 < MAX_MULTIMINT, "Trying to mint too many at a time");
    require(msg.value >= PLANET_PRICE * count, "Insufficient payment");

    if (allowedPlanet9MintCount(recepient) > 0) {
      updatePlanet9MintCount(recepient, count);
    } else {
      revert("Minting limit exceeded");
    }
    P9Mint(recepient,count);
...
// PLANET 10 CHECKS
  function P10checks(address recepient, uint256 count) internal {
    require(count - 1 < MAX_MULTIMINT, "Trying to mint too many at a time");
    require(msg.value >= PLANET_PRICE * count, "Insufficient payment");

    if (allowedPlanet10MintCount(recepient) > 0) {
      updatePlanet10MintCount(recepient, count);
    } else {
      revert("Minting limit exceeded");
    }
    P10Mint(recepient,count);
  ...
  // ============================================================= //
   ...
   function P1Mint(address recepient, uint256 count) internal{
    ...
    function P2Mint(address recepient, uint256 count) internal{
    ...
    function P3Mint(address recepient, uint256 count) internal{
     require(totalPlanet3Supply() + count - 1 < PLANET3_SUPPLY, "Exceeds max supply");
      for (uint256 i = 0; i < count; i++) {
      planet3SupplyCounter.increment();
      _safeMint(recepient, totalPlanet3Supply() + PLANET1_SUPPLY + PLANET2_SUPPLY );
      }
    }

    function P4Mint(address recepient, uint256 count) internal{
      require(totalPlanet4Supply() + count - 1 < PLANET4_SUPPLY, "Exceeds max supply");
      for (uint256 i = 0; i < count; i++) {
      planet4SupplyCounter.increment();
      _safeMint(recepient, totalPlanet4Supply() + PLANET1_SUPPLY + PLANET2_SUPPLY + PLANET3_SUPPLY );
      }
  }

      function P5Mint(address recepient, uint256 count) internal{
      require(totalPlanet5Supply() + count - 1 < PLANET5_SUPPLY, "Exceeds max supply");
      for (uint256 i = 0; i < count; i++) {
      planet5SupplyCounter.increment();
      _safeMint(recepient, totalPlanet5Supply() + PLANET1_SUPPLY + PLANET2_SUPPLY + PLANET3_SUPPLY + PLANET4_SUPPLY);      }
  }
 
      function P6Mint(address recepient, uint256 count) internal{
      require(totalPlanet6Supply() + count - 1 < PLANET6_SUPPLY, "Exceeds max supply");
      for (uint256 i = 0; i < count; i++) {
      planet6SupplyCounter.increment();
      _safeMint(recepient, totalPlanet6Supply() + PLANET1_SUPPLY + PLANET2_SUPPLY + PLANET3_SUPPLY + PLANET4_SUPPLY + PLANET5_SUPPLY);
      }
  }

    function P7Mint(address recepient, uint256 count) internal{
      require(totalPlanet7Supply() + count - 1 < PLANET7_SUPPLY, "Exceeds max supply");
      for (uint256 i = 0; i < count; i++) {
      planet7SupplyCounter.increment();
      _safeMint(recepient, totalPlanet7Supply() + PLANET1_SUPPLY + PLANET2_SUPPLY + PLANET3_SUPPLY + PLANET4_SUPPLY + PLANET5_SUPPLY + PLANET6_SUPPLY);
      }
  }

    function P8Mint(address recepient, uint256 count) internal{
      require(totalPlanet8Supply() + count - 1 < PLANET8_SUPPLY, "Exceeds max supply");
      for (uint256 i = 0; i < count; i++) {
      planet8SupplyCounter.increment();
      _safeMint(recepient, totalPlanet8Supply() + PLANET1_SUPPLY + PLANET2_SUPPLY + PLANET3_SUPPLY + PLANET4_SUPPLY + PLANET5_SUPPLY + PLANET6_SUPPLY + PLANET7_SUPPLY);
      }
  }

    function P9Mint(address recepient, uint256 count) internal{
      require(totalPlanet9Supply() + count - 1 < PLANET9_SUPPLY, "Exceeds max supply");
      for (uint256 i = 0; i < count; i++) {
      planet9SupplyCounter.increment();
      _safeMint(recepient, totalPlanet9Supply() + PLANET1_SUPPLY + PLANET2_SUPPLY + PLANET3_SUPPLY + PLANET4_SUPPLY + PLANET5_SUPPLY + PLANET6_SUPPLY + PLANET7_SUPPLY + PLANET8_SUPPLY);
      }
  }


    function P10Mint(address recepient, uint256 count) internal{
      require(totalPlanet10Supply() + count - 1 < PLANET10_SUPPLY, "Exceeds max supply");
      for (uint256 i = 0; i < count; i++) {
      planet10SupplyCounter.increment();
      _safeMint(recepient, totalPlanet9Supply() + PLANET1_SUPPLY + PLANET2_SUPPLY + PLANET3_SUPPLY + PLANET4_SUPPLY + PLANET5_SUPPLY + PLANET6_SUPPLY + PLANET7_SUPPLY + PLANET8_SUPPLY + PLANET9_SUPPLY);
      }
  }
