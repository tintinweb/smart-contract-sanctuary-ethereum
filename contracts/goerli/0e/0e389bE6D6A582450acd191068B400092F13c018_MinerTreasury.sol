...
// File: contracts/helpers/interfaces/IMINERERC20.sol

pragma solidity ^0.8.4;

interface IMINERERC20 {
...
// File: contracts/regular/MinerTreasury.sol

pragma solidity ^0.8.4;

contract MinerTreasury is Ownable {
    using SafeMath for uint256;
    ...
    address public sMINER;
    uint256 public sMINERQueue; // Delays change to sMINER address
    ...
    function deposit(
        ...
        require(
            ...
            "Not accepted as a Reserve Token"
        );
        ...
        if (isReserveToken[_token]) {
            require(
                ...
                "Not approved as a Reserve Depositor"
            );
        } else {
            require(
                isLiquidityDepositor[msg.sender],
                "Not approved as a Liquidity Depositor"
            );
        }
        ...
        IERC20Mintable(MINER).mint(msg.sender, send_);
    ...
    function withdraw(uint256 _amount, address _token) external {
        ...
        require(
            ...
            "Not approved as a Reserve Spender"
        );

        uint256 value = valueOf(_token, _amount);
        IMINERERC20(MINER).burnFrom(msg.sender, value);
    ...
    function incurDebt(uint256 _amount, address _token) external {
        ...
        uint256 maximumDebt = IERC20(sMINER).balanceOf(msg.sender); // Can only borrow against sMINER held
    ...
    function repayDebtWithMINER(uint256 _amount) external {
        ...
        IMINERERC20(MINER).burnFrom(msg.sender, _amount);

        debtorBalance[msg.sender] = debtorBalance[msg.sender].sub(_amount);
        totalDebt = totalDebt.sub(_amount);

        emit RepayDebt(msg.sender, MINER, _amount, _amount);
    }
    ...
    function manage(address _token, uint256 _amount) external {
        if (isLiquidityToken[_token]) {
            require(
                ...
                "Not approved as Liquidity Manager"
            );
        } else {
            require(
                isReserveManager[msg.sender],
                "Not approved as Reserve Manager"
            );
        }
    ...
    function mintRewards(address _recipient, uint256 _amount) external {
        ...
        IERC20Mintable(MINER).mint(_recipient, _amount);
    ...
    function excessReserves() public view returns (uint256) {
        return totalReserves.sub(IERC20(MINER).totalSupply().sub(totalDebt));
    }
    ...
    {
        if (isReserveToken[_token]) {
            // convert amount to match MINER decimals
            value_ = _amount.mul(10**IERC20(MINER).decimals()).div(
        ...
        returns (bool)
    {
        ...
        } else if (_managing == MANAGING.SMINER) {
            // 9
            sMINERQueue = block.number.add(blocksNeededForQueue);
        } else return false;
