 ...
 * @title NFTStakePool
 *
 * Contract for Stake Pools that allow token holders to stake their NFTs
 * in exchange for more token rewards. NFTStakePool.sol contract works in
 * conjunction with a RewardsManager.sol contract that holds and distributes
 * the reward tokens to NFT stakers. Deployer and owner of the NFTStakePool.sol
 * has to ensure that the right Rewards Manager and NFT/Reward Token
 ...
 * NFTs and reward tokens are passed to the Stake Pool on deployment
 * accordingly.
*/

contract NFTStakePool is ReentrancyGuard, Ownable, Pausable, IERC721Receiver {
    ...
    IERC721 public stakedNFTCollection;
    ...
    // Internal calculation of rewards accrued per staked NFT
    uint256 private rewardsPerStakedNFT;
    // Block which this stake pool contract was last updated at
    uint256 private lastUpdatedAt;
    // Total amount of NFTs staked in this stake pool
    ...
    */
    modifier updatePool() {
        if (totalStaked > 0) {
            rewardsPerStakedNFT = block.timestamp.sub(lastUpdatedAt)
                                                .mul(rewardsDistributionRate)
                                                .mul(SAFE_MULTIPLIER)
                                                .div(totalStaked)
                                                .add(rewardsPerStakedNFT);
        }
        lastUpdatedAt = block.timestamp;
        _;
    }
    ...
    */
    ...
    /**
    * Calculate the current rewards accrued per NFT staked
    * @return currentRewardsPerStakedNFT      Current rewards per staked NFT
    */
    function currentRewardsPerStakedNFT() private view returns (uint256) {
        if (totalStaked <= 0) {
    ...
    */
    function rewardsEarned(address _account) public view returns (uint256) {
        return userInfo[_account].stakedAmount
                    .mul(currentRewardsPerStakedNFT())
    ...
    * @param _account           Address of a user
    * @param _amount            Amount of new NFTs staked or amount of NFTs left in stake pool
    */
    function _updateUserRewardsDebt(address _account, uint256 _amount) private {
        userInfo[_account].rewardsDebt = userInfo[_account].rewardsDebt
                                            .add(_amount.mul(rewardsPerStakedNFT));
    }

    /**
    * Private function used for updating the user rewardsAccrued variable
    * Called when user is withdrawing staked NFTs
    * @param _account           Address of a user
    */
    ...
    /**
    * External function called when a user wants to stake a NFT
    * Called when user is staking a NFT
    * @param _id           Unique ID of NFT to be staked
    */
    function stake(uint256 _id) external nonReentrant whenNotPaused updatePool {
        require(stakedNFTCollection.ownerOf(_id) == msg.sender, "NFT does not belong to owner");

        // TODO: For now, only allow 1 NFT to be staked at a time
        uint256 _amount = 1;
        ...
        stakedNFTCollection.safeTransferFrom(msg.sender, address(this), _id);
        emit Staked(msg.sender, _id);
    }

    /**
    * External function called when a user wants to unstake tokens
    * Called when user is withdrawing staked tokens
    * @param _id           Unique ID of NFT to be withdrawn/unstaked
    */
    function withdraw(uint256 _id) public nonReentrant whenNotPaused whenNotLocked(msg.sender) updatePool {
        require(stakedNFTCollection.ownerOf(_id) == msg.sender, "NFT does not belong to owner");
        ...
        stakedNFTCollection.safeTransferFrom(address(this), msg.sender, _id);
