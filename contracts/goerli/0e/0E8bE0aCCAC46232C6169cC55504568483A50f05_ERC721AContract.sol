...
contract ERC721AContract is ERC721A, Ownable, PaymentSplitter, DefaultOperatorFilterer {
    ...
    function checkClaimEligibility(address _address, uint256 _quantity, bytes32[] memory _proof) public view returns (string memory) {
        ...
        if (!_isEligiblePreSale && !_isEligiblePubSale) return "Not eligible.";
        return "";
    }
    ...
    function isEligible(
        ...
        if(token.preSaleIsActive) {
            if (token.preSalePhase == 1) {
                _isEligiblePreSale = uint16(totalSupply()) + _quantity <= token.maxSupply && 
                mintpass.balanceOf(_address, 1) >= _quantity;
            }
        ...
        if (token.pubSaleIsActive) {
            _isEligiblePubSale =
                uint16(totalSupply()) + _quantity <= token.maxSupply &&
                _quantity <= token.maxPerTransaction;
        }
