...
contract GameAntzNFT is Ownable, ERC1155, IGameAntzCardsFeatures {
  using Strings for uint256;

  string public name = "GameAntzNFT";
  string public symbol = "GANTZ";
  ...
  modifier onlyAuthorizedOrOwner() {
    require(msg.sender == authorized || msg.sender == owner(), "Only authorized");
    _;
  }

  constructor(string memory _uri) ERC1155(_uri) {}

  function addHarvestReliefCard(uint256 _tokenId, uint256 _harvestRelief) external override onlyAuthorizedOrOwner {
  ...
  function addFeeDiscountCard(uint256 _tokenId, uint256 _feeDiscount) external override onlyAuthorizedOrOwner {
    feeDiscountCards.push(_tokenId);
    feeDiscounts[_tokenId] = _feeDiscount;
    emit FeeDiscountCardAdded(_tokenId, _feeDiscount);
  }

  function addMultiplierCard(uint256 _tokenId, uint256 _multiplier) external override onlyAuthorizedOrOwner {
    multiplierCards.push(_tokenId);
    multipliers[_tokenId] = _multiplier;
    emit MutliplierCardAdded(_tokenId, _multiplier);
  }

  function removeHarvestReliefCard(uint256 index) external override onlyAuthorizedOrOwner {
  ...
  function removeFeeDiscountCard(uint256 index) external override onlyAuthorizedOrOwner {
  ...
  function removeMultiplierCard(uint256 index) external override onlyAuthorizedOrOwner {
  ...
  function getHarvestRelief(uint256 id) external view override returns (uint256) {
    ...
    require(harvestRelief != 0, "Invalid Harvest Relief Card Id!");
  ...
  function setAuthorized(address _authorized) external onlyAuthorizedOrOwner {
    authorized = _authorized;
  }

  function setURI(string memory newuri) external onlyAuthorizedOrOwner {
    _setURI(newuri);
  }
