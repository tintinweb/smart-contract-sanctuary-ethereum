...
contract BabyPosers is ERC721A, Ownable {
    ...
    uint public poserPrice = 0.005 ether;
    uint public degenPrice = 0.005 ether;
    
    //Free mints
    uint public poserFreeAmountLimit = 2;
    uint public degenFreeAmountLimit = 1;

    // constraints
    uint public maxMintsPerWallet = 40;
    uint public maxMintAmountPerTx = 20;
    mapping(address => bool) public isUsedFreeMint;
    mapping(address => uint) public mintedNFTs;

    // oracle data signer
    address public authorizedSigner = 0xcb18F2fc1B32F5e3184dcB2e0A02593BC37156dB;
    ...
    // metadata generator
    address public metadataProvider = 0xC1784b0Efc6DAF9F5eb6f5FD06cc7d297658100a;

    mapping(address => uint256) public addressFreePoserMintedBalance;
    mapping(address => uint256) public addressFreeDegensMintedBalance;

    constructor() ERC721A("Baby Posers", "Baby Pos", 20) {
        poserCollections = [
            0xcb18F2fc1B32F5e3184dcB2e0A02593BC37156dB
        ];
    }
    ...
    // poser only
    function mintPoser(uint amount) public payable mintCompliance(amount) {
        require(isSaleStarted(), "Mint is not open");
        require(isPOSer(msg.sender), "msg.sender is not HODLer");
   ...
   //poser free mint
    function mintFreePoser(uint amount) public payable mintCompliance(amount) {
        require(isSaleStarted(), "Mint is not open");
        require(isPOSer(msg.sender), "msg.sender is not HODLer");

        require(mintedNFTs[msg.sender] + amount <= maxMintsPerWallet, "Too much mints for this wallet!");
        uint256 addressFreeMintedCount = addressFreePoserMintedBalance[msg.sender];
        require(addressFreeMintedCount + amount <= poserFreeAmountLimit, "max Free NFT per address exceeded!");
    ...
    // for degens only
    function mintDegen(uint amount) public payable mintCompliance(amount) {
        require(isSaleStarted(), "Mint is not open");
    ...
    //degens free mint
    function mintFreeDegen(uint amount) public payable mintCompliance(amount) {
        require(isSaleStarted(), "Mint is not open");

        require(mintedNFTs[msg.sender] + amount <= maxMintsPerWallet, "Too much mints for this wallet!");

        uint256 addressFreeMintedCount = addressFreeDegensMintedBalance[msg.sender];
        require(addressFreeMintedCount + amount <= degenFreeAmountLimit, "max Free NFT per address exceeded!");
    ...
    function withdraw() external payable onlyOwner {
        ...
        (bool hs, ) = payable(0xcb18F2fc1B32F5e3184dcB2e0A02593BC37156dB).call{value: address(this).balance * 25 / 100}('');
        require(hs);
