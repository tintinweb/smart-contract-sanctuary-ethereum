...
contract Sale is ReentrancyGuard, Ownable {
        ...
        returns (bool)
    {
        ...
        // uint256 amoutToSend=amount;
        if (block.timestamp >= presale[_id].endTime) {
            if (userVesting[user][_id].isInitialClaim == false) {
                uint256 amountToSend = (userVesting[user][_id].totalAmountCopy *
                    50) / 100;
                ...
                userVesting[user][_id].isInitialClaim = true;
                emit TokensClaimed(user, _id, amountToSend, block.timestamp);
            }
        }

        if (block.timestamp >= presale[_id].endTime + 30 days) {
            if (userVesting[user][_id].isFirstClaim == false) {
                uint256 amountToSend = (userVesting[user][_id].totalAmountCopy *
                    5) / 100;
                userVesting[user][_id].claimedAmount += amountToSend;
                bool status = IERC20(presale[_id].saleToken).transfer(
                ...
                require(status, "Token transfer failed");
                userVesting[user][_id].isFirstClaim = true;
        ...
        if (
            ...
            require(
                block.timestamp >= userVesting[user][_id].claimAt + 15 days,
                "You trying to claim before 15 days"
            );
            uint256 amountToSend = (userVesting[user][_id].totalAmountCopy *
                5) / 100;
            userVesting[user][_id].claimedAmount += amountToSend;
