...
contract DevAuction is DutchAuction 
{
        ...
        public 
    {
        ...
        DutchAuction.initialize(nft, admin);
    }
    ...
    function devAddStages(AuctionInfo[] memory infos) 
        public 
        onlyRole(DEFAULT_ADMIN_ROLE) 
        whenNotPaused 
    {        
        for (uint256 index = 0; index < infos.length; ++index) {
            AuctionInfo memory info = infos[index];
            info.flags = 0;
                         ...
                         Versioned 
{
    ...
    uint8 internal constant FLAG_IS_WL = 1 << 0;
        ...
        whenNotPaused 
    {
        ...
        if (info.startPrice == info.endPrice) {
            require(info.priceDropIntervalSeconds == 0, "expect zero interval");
            require(info.dropPerInterval == 0, "expect zero drop px");
        } else {
            require(info.priceDropIntervalSeconds > 0, "expect non-zero interval");
            require(info.dropPerInterval > 0, "expect non-zero drop px");
        }
        ...
        returns (uint256) 
    {
        ...
        if (timestamp < startTimestamp) {
            ...
            uint256 drop = intervals * _stages[stage].dropPerInterval;
            if (startPrice > drop) {
                        ...
                        Versioned 
{
        ...
        returns (string memory) 
    {
        ...
        if (bytes(_baseTokenURI).length > 0) {
            // returns <base-uri>/<chain-id>/<contract-name>/<contract-address>/<token-id>.json
