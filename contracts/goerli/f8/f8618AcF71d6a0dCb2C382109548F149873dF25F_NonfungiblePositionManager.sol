...
import "../interfaces/callback/IXSwapMintCallback.sol";
...
/// @notice Internal functions for safely managing liquidity in XSwap
...
abstract contract PoolInitializer is IPoolInitializer, PeripheryImmutableState {
    ...
    function createAndInitializePoolIfNecessary(
        ...
        pool = IXSwapFactory(factory).getPool(token0, token1, fee);

        if (pool == address(0)) {
            pool = IXSwapFactory(factory).createPool(token0, token1, fee);
            IXSwapPool(pool).initialize(sqrtPriceX96);
        } else {
            (uint160 sqrtPriceX96Existing, , , , , , ) = IXSwapPool(pool)
                .slot0();
...
/// @notice Wraps XSwap positions in a non-fungible token interface which allows for them to be transferred
/// and authorized.
...
/// @title The interface for the XSwap Factory
/// @notice The XSwap Factory facilitates creation of XSwap pools and control over the protocol fees
...
/// @title NFT positions
/// @notice Wraps XSwap positions in the ERC721 non-fungible token interface
contract NonfungiblePositionManager is
    ...
    SelfPermit
{
    // details about the XSwap position
    struct Position {
    ...
    constructor(
        ...
        ERC721Permit("XSwap Positions NFT-V1", "XDC-Swap-POS", "1")
