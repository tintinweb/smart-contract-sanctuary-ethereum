...
contract LBSKR_VX is BaseBSKR, DSMath, Stakable {
     ...
     */
    function airdrop(Airdrop[] calldata receivers) external onlyOwner override {
    ...
    {
        uint256 stakerIndex = _stakes[account];
    ...
    function unstakePercent(uint256 percent, uint256 stakeIndex) external {
        ...
        if (stakerIndex > 0) {
            Stake memory currStake = _getCurrStake(stakerIndex, stakeIndex);
            unstake((currStake.amountLBSKR * percent) / 100, stakeIndex);
        }
    }
