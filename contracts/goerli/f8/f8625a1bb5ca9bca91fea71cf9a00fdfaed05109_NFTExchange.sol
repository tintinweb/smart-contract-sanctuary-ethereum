...
contract NFTExchange is ExchangeInterface, IERC721Receiver, NFTExchangeLayout {
    ...
    function sell(OriginOrder calldata order) external override nonReentrant {
        if (order.seller != msg.sender) revert InvalidSender();
        IERC721(order.token).transferFrom(msg.sender, address(this), order.tokenId);
        _sell(order);
    }

    /**
     * @notice Close the old order and reopen new one.
     */
    function reopen(bytes32 oldOrderHash, OriginOrder calldata order) external override nonReentrant {
        if (order.seller != msg.sender) revert InvalidSender();

        Order storage oldOrder = _orders[oldOrderHash];
        if (msg.sender != oldOrder.seller) revert InvalidOrderOwner();

        // token info must be same.
        if (oldOrder.token != order.token || oldOrder.tokenId != order.tokenId) revert OrderTokenMissmatch();

        _close(oldOrderHash, false, true);
        _sell(order);
    }
    ...
    function _close(
        ...
        //slither-disable-next-line timestamp
        if (order.endTime > block.timestamp) revert OrderIsListing();

        OrderKind kind = _getKind(order);
        if (kind == OrderKind.AuctionWithoutReserve && order.lastBidder != address(0)) {
            if (reopenOrder) revert OrderIsFilled();
            // to heighest bidder
            _sold(orderHash);
        } else {
            _unsold(orderHash, reopenOrder);
        }
    }
    ...
    function _unsold(bytes32 orderHash, bool reopenOrder) internal {
        Order storage order = _orders[orderHash];
        //expired
    ...
    function _reopen(bytes32 oldOrderHash) private {
        Order storage oldOrder = _orders[oldOrderHash];

        bytes32 orderHash = keccak256(abi.encode(oldOrderHash));
        if (_orders[orderHash].seller != address(0)) revert DuplicateOrder();
        ...
        _orders[orderHash] = Order({
            ...
            salt: oldOrder.salt,
            reservePrice: oldOrder.reservePrice,
            startingPrice: oldOrder.startingPrice,
            startTime: uint64(block.timestamp) + 1, //slither-disable timestamp
            endTime: uint64(block.timestamp) + (oldOrder.endTime - oldOrder.startTime), //slither-disable timestamp
            token: oldOrder.token,
            tokenId: oldOrder.tokenId,
            lastBidder: payable(address(0)),
            lastBid: 0,
            contextId: oldOrder.contextId
        });
        emit NewOrder(oldOrder.seller, orderHash);
    }
