...
interface ILotteryDeployer {
    ...
    function parameters() external view returns (address factory, uint256 ticketPrice, uint256 durationMinutes);
}
...
contract Lottery is ILottery, Ownable, Pausable, ReentrancyGuard, VRFConsumerBaseV2, PullPayment, NoDelegateCall {
    ...
    constructor() VRFConsumerBaseV2(0x0000000000000000000000000000000000000000) {
        (factory, ticketPrice, durationMinutes) = ILotteryDeployer(msg.sender).parameters();
    }
...
contract LotteryDeployer is ILotteryDeployer {
    ...
    /// @param factory The contract address of the Lottery Factory
    ...
    function deploy(address factory, uint256 ticketPrice, uint256 durationMinutes) internal returns (address pool) {
        parameters = Parameters({factory: factory, ticketPrice: ticketPrice, durationMinutes: durationMinutes});
        pool = address(new Lottery{salt: keccak256(abi.encode(ticketPrice, durationMinutes))}());
        delete parameters;
    }
}
