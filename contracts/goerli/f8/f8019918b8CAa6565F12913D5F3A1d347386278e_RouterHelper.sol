...
contract RouterHelper is Ownable {
    ...
    function getPairInfo(fatory Fatory, address tokenA, address tokenB, address _account) public view returns (address pair_, tokenInfoItem[] memory tokenInfoList, pairItem2[] memory PairInfo) {
        ...
        if (pair_ != address(0)) {
            ...
            uint256 pooled0  = reserve0.mul(pair(pair_).balanceOf(_account)).div(pair(pair_).totalSupply());
            uint256 pooled1  = reserve1.mul(pair(pair_).balanceOf(_account)).div(pair(pair_).totalSupply());
            ...
            PairInfo[0] = pairItem2(pair_, token0, token1, reserve0, reserve1, IERC20(token0).decimals(), IERC20(token1).decimals(), pair(pair_).decimals(), pair(pair_).totalSupply(), pair(pair_).balanceOf(_account),pooled0,pooled1);
        }
    }
    ...
    function getLpInfo(IERC20 _token, address _user) public view returns (TokenItem memory lpInfo) {
        ...
        try pair(address(_token)).token0() returns (address token){
            address token0 = token;
            address token1 = pair(address(_token)).token1();
            tokenList[0] = token0;
            tokenList[1] = token1;
