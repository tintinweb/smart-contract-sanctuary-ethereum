...
contract Whitelist is Ownable {
  ...
  function getWhitelist() public view returns (address[] memory) {
    address[] memory addresses = new address[](s_counter);
    for (uint256 i = 0; i < s_counter; i += 1) {
      addresses[i] = idToAddress[i];
    }
    return addresses;
  }

  function isUserIn(address user) external view returns (bool) {
