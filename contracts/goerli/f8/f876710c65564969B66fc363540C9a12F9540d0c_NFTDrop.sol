    ...
    ERC721EnumerableUpgradeable
{
    ...
    function setCrossmintAddy(address _crossmintAddy) external onlyRole(DEFAULT_ADMIN_ROLE) {
        require(crossmintAddy != _crossmintAddy, "Exist");
        crossmintAddy = _crossmintAddy;
    }
   ...
   /// @dev Checks whether caller has admin role
    ...
    function getCrossmintAddy() internal view override returns (address) {
        return crossmintAddy;
    }
    /*
    function _msgSender()
        internal
...
abstract contract Drop is IDrop, Role {
   ...
   /// @dev The next token ID of the NFT to claim.
    ...
    function claim(
        ...
        verifyClaim(_salesPhaseId, _dropMsgSender(), _quantity, _pricePerToken, _allowlistProof);

        // Update contract state.
        
        salesPhase.supplyClaimedByWallet[_salesPhaseId][_dropMsgSender()] += _quantity;
        salesPhase.limitLastClaimTimestamp[_salesPhaseId][_dropMsgSender()] = block.timestamp;
    ...
    function claimWithCrossmint(
        ...
        require(_dropMsgSender() == getCrossmintAddy(), "Non-CM-Addy");
        ...
        commonVerify(_salesPhaseId, _receiver, _quantity, _pricePerToken, claimLimit, claimPrice, currentSalesPhase.maxClaimableSupply,currentSalesPhase.supplyClaimed, currentSalesPhase.startTimestamp);
        // If there's a price, collect price.
        _collectPriceOnClaim(address(0), _quantity, _pricePerToken);
    ...
    function verifyClaim(
        ...
        if (currentSalesPhase.merkleRoot != bytes32(0)) {
            (isOverride, ) = MerkleProof.verify(
                ...
                keccak256(abi.encodePacked(_claimer, _allowlistProof.quantityLimitPerWallet, _allowlistProof.pricePerToken))
            );
        }
        ...
        commonVerify(_salesPhaseId, _claimer, _quantity, _pricePerToken, claimLimit, claimPrice, currentSalesPhase.maxClaimableSupply,currentSalesPhase.supplyClaimed, currentSalesPhase.startTimestamp);
      ...
      function commonVerify(
        ...
        if (supplyClaimed + _quantity > maxClaimableSupply) {
            revert("!MaxSupply");
        }
    ...
    */
    ...
    function getCrossmintAddy() internal view virtual returns (address);
}
