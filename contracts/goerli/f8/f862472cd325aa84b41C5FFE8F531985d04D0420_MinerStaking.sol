...
contract MinerStaking is MinerAccessControlled {
     ...
     * @param _claim  The amount of GONs to claim.
     * @return uint
     */
    function stake(
        address _to,
        ...
        // Claim storage info = warmupInfo[_to];
        if (_claim && warmupPeriod == 0) {
            ...
            warmupInfo[_to] = Claim({
                ...
                gons: info.gons.add(vMINER.gonsForBalance(_amount)),
    ...
    function claim(address _to) public returns (uint256) {
        ...
        if (epoch.number >= info.expiry && info.expiry != 0) {
            ...
            return _send(_to, vMINER.balanceForGons(info.gons));
        }
        return 0;
    }

    /**
     * @notice forfeit stake and retrieve MINER
     * @return uint
     */
    function forfeit() external returns (uint256) {
        ...
        miner.safeTransfer(msg.sender, info.deposit);
    ...
    function unstake(
        ...
        if (block.timestamp < user.lockEndTime) {
            require(_amount <= user.stakedAmount, "Cannot unstake more than staked before unlock");
        }
        amount_ = _amount;
        uint256 bounty;
        if (_trigger) {
