...
contract Market is ManagedMarketBase
{
    ...
    function initialize(address _token, address _stable, address _config, address _feed) public initializer { 
    ...
    function yield(uint id) public view override returns (int) {
        ...
        return calc.yield( address(token), pos ) * int(uint(pos.term)) * int(pos.stableAmount) / int(CP);
    }

    /// @notice deposit with permit 
    function depositWithPermit(uint amount, uint16 risk, uint16 term, bool autorenew, 
        uint256 deadline,
        uint8 v,
        bytes32 r,
    ...
    function calcNewMakerPosition(uint amount, uint16 term) public view override returns (uint fee) {
        fee = amount * makerFee/10000; 
    }

    /// @notice calculate fee for new taker position
    function calcNewTakerPosition(uint amount, uint risk, uint16 term) public view override
...
abstract contract ManagedMarketBase is IMarketManagement, MarketBase
{
    ...
    function govWithdraw(address _token, address to, uint amount) external virtual override onlyGovernance {
...
interface IMarket {
    ...
    function protect(uint amount, uint16 risk, uint16 term, bool autorenew) external returns (uint id);
    function protectNative(uint16 risk, uint16 term, bool autorenew) external payable returns (uint id);
    function protectWithPermit(uint amount, uint16 risk, uint16 term, bool autorenew, 
        uint256 deadline,
        uint8 v,
        bytes32 r,
        bytes32 s) external returns (uint id);
    function close(uint id) external;
    function claim(uint id) external;
    function deposit(uint amount, uint16 risk, uint16 term) external returns (uint id);
    function depositWithPermit(uint amount, uint16 risk, uint16 term, bool autorenew, 
        uint256 deadline,
        uint8 v,
        bytes32 r,
    ...
    function getState() external view returns (uint AP1, uint AP, uint AR, uint CP, uint CR, uint B, uint L, uint D);    
...
interface IProtocolConfig {
    ...
    function getPremiumMultiplier(address token, uint16 risk, uint16 term) external view returns (int128 multiplier);
    /// @notice Get yield multiplier
    function getYieldMultiplier(address token, uint16 risk, uint16 term) external view returns (int128 multiplier);
...
interface IRiskCalculation {
    ...
    function premium(address token, TakerPosition memory pos) external view returns (uint); 
    function yield(address token, MakerPosition memory pos) external view returns (int);    
}
...
/// @notice Implementaion of the IMarket interface
abstract contract MarketBase is IMarket, Initializable, MarketStorage, BACUpgradable
{
    using SafeERC20 for IERC20;
    ...
    // Events
    event Protected(address account, uint id, uint amount, uint16 risk, uint16 term );
    event Claimed(address account, uint id, uint amount, uint floor);
    event Closed(address account, uint id, uint amount, uint premium, uint claimed );
    event Deposited(address account, uint id, uint amount, uint16 risk, uint16 term);
    ...
    /// @notice initialize the contract
    function _initMarket(address _token, address _stable, address _config, address _feed) internal { 
        token = IERC20(_token);
    ...
    function getTakerPosition(uint id) public view virtual override returns (TakerPosition memory){
        return allTakerPositions[ id ];
    }

    /// @notice get structure containing maker position information
    function getMakerPosition(uint id) public view virtual override returns (MakerPosition memory){
        return allMakerPositions[ id ];
    }

    /// @notice protect native token (ETH for Etherium blockchain)
    function protectNative(uint16 risk, uint16 term, bool autorenew) external virtual override payable returns (uint id) {
        _wrapNativeToken();
    ...
    function protectWithPermit(uint amount, uint16 risk, uint16 term, bool autorenew, 
        ...
        uint8 v,
        bytes32 r,
        bytes32 s) external virtual override returns (uint id) {
    ...
    function close(uint id) external override takerTermEnds(id) {
        ...
        // calc 
        (int _price, uint updatedAt) = price();
        uint posSizeInStable = pos.assetAmount * uint(_price) / DIVIDER;
        ...
        emit Closed(msg.sender, id, amountToReturn, posPremium, claimAmount );
    }
    ...
    function claim(uint id) external virtual override takerTermEnds(id) {
        ...
        // calc 
        (int _price, uint updatedAt) = price();
        uint posSizeInStable = pos.assetAmount * uint(_price) / DIVIDER_ORACLE;
        uint claimAmount = pos.floor > posSizeInStable ? (pos.floor - posSizeInStable) : 0;
    ...
    /// @notice create new maker position
    function deposit(uint amount, uint16 risk, uint16 term) external virtual override returns (uint id) {
        require(amount > 0, "zero" );

        (uint fee) = calcNewMakerPosition(amount, term);

        // transfer tokens from msg.sender to the contract
        IERC20(stable).safeTransferFrom(msg.sender, address(this), amount + fee );

        uint amountInOraclePrecision = amount / DIVIDER_STABLE * DIVIDER_ORACLE;
        ...
        emit Deposited(msg.sender, makerPosIndex - 1, amount, risk, term );
    ...
    function withdraw(uint id) external virtual override {
    ...
    function getState() public view virtual override returns (uint _APT, uint _AP, uint _AR, uint _CP, uint _CR, uint _B, uint _L, uint _D) {
        (int _price,) = price();
        return (AP, AP * uint(_price) / DIVIDER, AR * uint(_price) / DIVIDER, CP, CR, B * uint(_price) / DIVIDER, L, D );
    }    

    // INTERNAL 
    /// @notice create new maker position
    function _protect(address account, uint amount, uint16 risk, uint16 term, bool autorenew) 
    ...
    /// HELPERS 
    /// @notice calculate fee for new taker position
    function calcNewTakerPosition(uint amount, uint risk, uint16 term) public view virtual
        returns (uint fee, uint floor);

    /// @notice calculate fee for new maker position
    function calcNewMakerPosition(uint amount, uint16 term) public view virtual returns (uint fee);

    /// @notice Wrap native token
    function _wrapNativeToken() internal virtual {
        IWETH9(config.getWrappedNativeToken()).deposit();
    }

    /// @notice Get current price from oracle
    function price() public view virtual override returns (int _price, uint _updatedAt) {
        (, _price, , _updatedAt, ) = AggregatorV3Interface(feed).latestRoundData();
    }
}
...
interface IMarketManagement {
    ...
    function govWithdraw(address _token, address to, uint amount) external;
}
...
{
    bytes32 public constant LOCAL_GOVERNANCE_ROLE = keccak256("LOCAL_GOVERNANCE_ROLE");
    bytes32 public constant GLOBAL_GOVERNANCE_ROLE = keccak256("GLOBAL_GOVERNANCE_ROLE");
    ...
    modifier onlyGovernance() {
        require(hasRole(LOCAL_GOVERNANCE_ROLE, msg.sender) || bac.userHasRole(GLOBAL_GOVERNANCE_ROLE, msg.sender), "!bacu-governance");
        _;
    }
    ...
    modifier onlyGlobalGovernance() {
        require(bac.userHasRole(GLOBAL_GOVERNANCE_ROLE, msg.sender), "!bacu-ggov");
        _;
    }
    ...
    function grantRole(bytes32 role, address account) override public onlyAdmin  {
        _grantRole(role, account);
    }

    function revokeRole(bytes32 role, address account) override public onlyAdmin  {
        _revokeRole(role, account);
    }

    function userHasRole(bytes32 role, address account) public view returns (bool) {
