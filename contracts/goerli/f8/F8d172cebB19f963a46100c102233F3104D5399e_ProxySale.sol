...
contract ProxySale is Ownable, ReentrancyGuard {
    ...
    event Mint(address owner, uint256 value, uint256 token, Status status);
    event ChangeStatus(Status status);

    address public nftAddress;
    INftMint private nftMint;

    uint256 public maxPreNumberMinted = 1;
    uint256 public maxPublicNumberMinted = 1;

    uint256 public _mintedPreAmount = 0;
    uint256 public _mintedPublicAmount = 0;
    ...
    constructor(address _signerMint, uint256 _MaxSupply, address _vault) public {
    ...
    function _preMint(bytes32[] memory _proof) internal {
        ...
        emit Mint(msg.sender, whiteListMintPrice, nftMint.totalSupply() - 1, status);
    }
    ...
    function _publicMint(string calldata _salt, bytes calldata _signature) internal
    {
        ...
        require(publicNumberMinted[msg.sender] < maxPublicNumberMinted, "Minting more than the max supply for a single address");
        ...
        _mintedPublicAmount += 1;

        emit Mint(msg.sender, publicMintPrice, nftMint.totalSupply() - 1, status);
    }
    ...
    function setMaxPublicNumberMinted(uint256 _maxPublicNumberMinted) public onlyOwner {
        maxPublicNumberMinted = _maxPublicNumberMinted;
    }
