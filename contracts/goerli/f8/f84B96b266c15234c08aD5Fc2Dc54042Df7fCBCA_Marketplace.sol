...
contract Marketplace is ReentrancyGuard, Ownable, Pausable {
    ...
    constructor(address _indexAddress) {
        Index = ContractGlossary(_indexAddress);
    }
    ...
    function refreshContracts() internal {
        ...
        SilksContractsbyAddress[HorseGovAddress] = "HorseGovernance";
        StableAddress = Index.getAddress("Stable");
        SilksContractsbyAddress[StableAddress] = "Stable";
    }

    function changeFloorPrice(string memory nftType, uint256 price)
        public
        onlyOwner
    {
    ...
    function createMarketItem(
        ...
        require(
            price > floorPrices[SilksContractsbyAddress[nftContract]],
        ...
        marketItems.push(idToMarketItem[itemId]);
        marketSize++;
    }
    ...
    function fetchMarketItemsbyAddress(address tokenAddress)
        public
        view
        returns (MarketItem[] memory)
    {
        require(marketSize > 0, "NO ITEMS IN MARKET");
        MarketItem[] memory items = new MarketItem[](marketSize);
        uint256 j = 0;
        for (uint256 i = 0; i <= marketSize; i++) {
            if (idToMarketItem[i].nftContract == tokenAddress) {
    ...
    function fetchMarketItemsbyID(uint256 id)
        public
        view
        returns (MarketItem memory)
    {
