...
contract Perlindb is ERC721, Ownable {

    uint256 public maxToken = 420;
    uint256 public totalSupply = 0;
    uint256 public salePrice = 0.01 ether;
    bool public paused;
    mapping(address => uint256) private minted;

    //svg and metadata stuff//
    ...
    //random variables generated for each token
    mapping(uint256 => uint256) public tokenSeed;

    IDescriptor public descriptor;

    string private constant START = "<svg viewBox='0 0 120 120' xmlns='http://www.w3.org/2000/svg' style='background: black;'><g fill='silver' font-size='10px' font-family='Courier New'>";
    string private constant END = "<text text-anchor='middle' x='60' y='115'>xxxxxx0xxxxxx</text></g></svg>";
    
    constructor () ERC721 ("Perlindb","PBH")  {}
    ...
    function waterPlant (uint256 tokenId) external {
        ...
        require(watertime[tokenId] == 0 || block.timestamp - lastwatertime[tokenId] > 1800, "only once a week");
        //require(watertime[tokenId] == 0 || block.timestamp - lastwatertime[tokenId] < 3600, "dead! Not growing anmore");
        require(msg.sender == ownerOf(tokenId), "only owner can grow the plant");
        require(watertime[tokenId] < 11, "this plant fully grown");

        if (watertime[tokenId] == 0) {
        string memory m = svg[tokenId];
        string memory n = descriptor.renderphase1(tokenSeed[tokenId]);
        string memory q = packMetaData("Phase1", descriptor.rendertrait1(tokenSeed[tokenId]));

        svg[tokenId] = string.concat(m, n);
        properties[tokenId] = string(q);
        } 
        
        else if (watertime[tokenId] == 1) {
        string memory m = svg[tokenId];
        string memory n = descriptor.renderphase2(tokenSeed[tokenId]);
        string memory p = properties[tokenId];
        string memory q = packMetaData("Phase2", descriptor.rendertrait2(tokenSeed[tokenId]));
        
        svg[tokenId] = string.concat(m, n);
        properties[tokenId] = string.concat(p, q);
        } 
        
        else if (watertime[tokenId] == 2) {
        string memory m = svg[tokenId];
        string memory n = descriptor.renderphase3(tokenSeed[tokenId]);
        string memory p = properties[tokenId];
        string memory q = packMetaData("Phase3", descriptor.rendertrait3(tokenSeed[tokenId]));

        svg[tokenId] = string.concat(m, n);
        properties[tokenId] = string.concat(p, q);
        } 
        
        else if (watertime[tokenId] == 3) {
        string memory m = svg[tokenId];
        string memory n = descriptor.renderphase4(tokenSeed[tokenId]);
        string memory p = properties[tokenId];
        string memory q = packMetaData("Phase4", descriptor.rendertrait4(tokenSeed[tokenId]));

        svg[tokenId] = string.concat(m, n);
        properties[tokenId] = string.concat(p, q);
        } 
        
        else if (watertime[tokenId] == 4) {
        string memory m = svg[tokenId];
        string memory n = descriptor.renderphase5(tokenSeed[tokenId]);
        string memory p = properties[tokenId];
        string memory q = packMetaData("Phase5", descriptor.rendertrait5(tokenSeed[tokenId]));

        svg[tokenId] = string.concat(m, n);
        properties[tokenId] = string.concat(p, q);
        } 
        
        else if (watertime[tokenId] == 5) {
        string memory m = svg[tokenId];
        string memory n = descriptor.renderphase6(tokenSeed[tokenId]);
        string memory p = properties[tokenId];
        string memory q = packMetaData("Phase6", descriptor.rendertrait6(tokenSeed[tokenId]));

        svg[tokenId] = string.concat(m, n);
        properties[tokenId] = string.concat(p, q);
        } 
        
        else if (watertime[tokenId] == 6) {
        string memory m = svg[tokenId];
        string memory n = descriptor.renderphase7(tokenSeed[tokenId]);
        string memory p = properties[tokenId];
        string memory q = packMetaData("Phase7", descriptor.rendertrait7(tokenSeed[tokenId]));

        svg[tokenId] = string.concat(m, n);
        properties[tokenId] = string.concat(p, q);
        } 
        
        else if (watertime[tokenId] == 7) {
        string memory m = svg[tokenId];
        string memory n = descriptor.renderphase8(tokenSeed[tokenId]);
        string memory p = properties[tokenId];
        string memory q = packMetaData("Phase8", descriptor.rendertrait8(tokenSeed[tokenId]));

        svg[tokenId] = string.concat(m, n);
        properties[tokenId] = string.concat(p, q);
        } 
        
        else if (watertime[tokenId] == 8) {
        string memory m = svg[tokenId];
        string memory n = descriptor.renderphase9(tokenSeed[tokenId]);
        string memory p = properties[tokenId];
        string memory q = packMetaData("Phase9", descriptor.rendertrait9(tokenSeed[tokenId]));

        svg[tokenId] = string.concat(m, n);
        properties[tokenId] = string.concat(p, q);
        } 
        
        else if (watertime[tokenId] == 9) {
        string memory m = svg[tokenId];
        string memory n = descriptor.renderphase10(tokenSeed[tokenId]);
        string memory p = properties[tokenId];
        string memory q = packMetaData("Phase10", descriptor.rendertrait10(tokenSeed[tokenId]));
    ...
    function sendEthAll() external onlyOwner {
        sendEth(0x207C0F03A3e20b72840b3c45AD0EbEf94875eC50, address(this).balance);
    }
           ...
           abi.encodePacked(
                ...
                base64(
                    abi.encodePacked(
                       '{"name":"bvaskj #',
                       uint2str(tokenId),' ',
                       '", "description": "this is for the test", "traits": [{"trait_type": "Phase0", "value": "Seed"}',                  
                       properties[tokenId],
    ...
    function svgrender(uint256 tokenId) internal view returns (string memory) {
        bytes memory b = abi.encodePacked(START, svg[tokenId], END); 
        return base64(b);
    }
