...
import "./IConnectFour.sol";
import "./Renderer.sol";
import "hardhat/console.sol";

contract ConnectFour is IConnectFour, ERC721, ERC721Holder, Ownable {
    ...
    constructor(Renderer _renderer) payable ERC721("Connect Four", "C4") {
        renderer = _renderer;
    }
    ...
    function begin(
        ...
        emit Begin(_gameId, msg.sender, game.player2);
    ...
    function move(
        ...
        uint256 height = game.height;

        if (game.state != State.ACTIVE) revert InvalidState();
        if (game.turn != msg.sender) revert InvalidTurn();
        if (board[_row][_col] != address(0) || _row > height) revert InvalidMove();
        if (_row >= ROW || _col >= COL) revert InvalidPlacement();

        ++game.moves;
        board[_row][_col] = msg.sender;
        emit Move(_gameId, msg.sender, _row, _col);

        if (game.moves >= 7) {
            result = _checkVertical(msg.sender, _row, _col, board);
            if (!result) result = _checkHorizontal(msg.sender, _row, _col, board);
            if (!result) result = _checkAscending(msg.sender, _row, _col, board);
            if (!result) result = _checkDescending(msg.sender, _row, _col, board);
        }

        if (result) {
            game.state = State.SUCCESS;
            game.winner = msg.sender;
            emit Result(_gameId, msg.sender, State.SUCCESS, board);
            if (totalSupply < 100) {
                _burn(_gameId);
    ...
    function printBoard(uint256 _gameId) public view {
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        ...
        string memory name = string.concat("Board Game #", tokenId.toString());
        string memory description = "Just a friendly game of Connect Four.";
        string memory url = string.concat("https://connectoooor.xyz/", tokenId.toString());
        string memory image = renderer.generateSVG(board, player1, player2);
...
contract Renderer {
    ...
    function generateSVG(
        ...
        string memory blue = "#254689";
        string memory red = "#FC7E69";
        ...
        for (uint256 y; y < COL; ++y) {
            ...
            for (uint256 x; x < ROW; ++x) {
                if (_board[x][y] == _player1) {
                    board = string.concat(board, generateCircle(x, blue));
                } else if (_board[x][y] == _player2) {
                    board = string.concat(board, generateCircle(x, red));
                }
            }
            board = string.concat(board, generateRectangle());
        }
    ...
    function generateRectangle() public pure returns (string memory) {
        return "<rect width='100' height='600' fill='cadetblue' mask='url(#cell-mask)'></rect></svg>";
    }

    function generateCircle(uint256 _row, string memory _piece) public pure returns (string memory) {
...
interface IConnectFour {
    ...
    event Challenge(uint256 indexed _gameId, address indexed _player2, address indexed _player1);

    event Begin(uint256 indexed _gameId, address indexed _player1, address indexed _player2);

    event Move(uint256 indexed _gameId, address indexed _player, uint256 _row, uint256 _column);
    ...
    function move(uint256 _gameId, uint256 _row, uint256 _col) external payable returns (bool result);

    function printBoard(uint256 _gameId) external view;
