...
contract SalePioneer is Ownable {
    ...
    mapping(address => uint) public waitlistMintedNFT;  
    ...
    function initialMint(uint paySelect,uint mintAllowed, uint256 quantity, bytes32[] calldata _merkleProof, bool marketingSelect) external payable {
    ...
    function whiteListClaimedPoints( uint paySelect, uint mintAllowed, uint quantity, bytes32[] calldata _merkleProof,bool marketingSelect ) internal {
        uint phase = 2;
        bytes32 leaf = keccak256(abi.encodePacked(msg.sender, phase, mintAllowed, marketingSelect) );

        require(MerkleProof.verify(_merkleProof, merkleRoot, leaf),"Invalid Proof" );
        require( waitlistMintedNFT[msg.sender].add(quantity) <= mintAllowed,"You can mint upto 10 nfts only");
        if(paySelect==1){
            require(msg.value >= priceOfNftinETH().mul(quantity), "Send Correct Eth Value");
            fundsWallet.transfer(priceOfNftinETH().mul(quantity));
            _mintUnpaid(quantity, marketingSelect);
           waitlistMintedNFT[msg.sender] = waitlistMintedNFT[ msg.sender].add(quantity);
    ...
    function _mintUnpaid(uint quantity , bool marketingSelect) internal{
        ...
        // addressWaitlisted[msg.sender] = true; 
    }
