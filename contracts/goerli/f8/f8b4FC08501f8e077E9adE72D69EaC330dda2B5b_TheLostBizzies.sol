...
contract TheLostBizzies is Ownable, ERC721A, MerkleAllowlist, ReentrancyGuard {

    string public CONTRACT_URI = "ipfs://QmRzY3rdV3rEB9eRLdzvpiTJbt983KQDkob7z1PXhQj31v";
    string public UNREVEALED_URI = "https://ipfs.io/ipfs/bafybeidwbdum2a7qr62xgdhprwkrgms6bvutid5xpgibds6eqixkqlyivu/284eeee.png";
    string public BASE_URI = "";

    enum MintState { Disabled, AllowOnly, Enabled }

    MintState public mintState;

    mapping(address => bool) public userHasMintedAL;

    uint256 public ALLOWLIST_MINT_PRICE = 0 ether;
    uint256 public MINT_PRICE = 0 ether;

    uint16 public MAX_ALLOWLIST_BATCH_SIZE = 1;

    uint16 public USER_ALLOCATION = 1;
    uint16 public COLLECTION_SIZE = 500;

    constructor() ERC721A("The Lost Bizzies", "TheLostBizzies") {
        mintState = MintState.Enabled;
    }
    ...
    function mintAllowlist(uint256 quantity, bytes32[] memory proof)
        public
        payable
        onlyPublicAllowlist(proof)
    {
        uint256 price = (MINT_PRICE) * quantity;
        require(!userHasMintedAL[msg.sender], "Can only mint once during public WL!");   
        require(totalSupply() + quantity <= COLLECTION_SIZE, "Max Collection Size reached!");
        require(quantity <= MAX_ALLOWLIST_BATCH_SIZE, "Cannot mint this quantity");
...
contract MerkleAllowlist is Ownable {
  ...
  */
  ...
  modifier onlyPublicAllowlist(bytes32[] memory proof) {
    require(_verifyPublicSender(proof), "Caller is not on the allowlist.");
    _;
  }
}
