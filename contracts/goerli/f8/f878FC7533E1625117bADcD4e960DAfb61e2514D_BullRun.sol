...
contract BullRunFeeHandler is Ownable {
    ...
    constructor(address _brfToken, address _marketingWallet, address _opsWallet, address _farmWallet, address _usdc, address _bulldozer) {
        IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D);

        uniswapV2Router = _uniswapV2Router;

        brfToken = IERC20(_brfToken);
        ...
        usdc = IERC20(_usdc);
        IERC20(brfToken).approve(address(_uniswapV2Router), type(uint256).max);
    ...
    function updateBulldozerWallet(address newWallet) external onlyOwner {
        bulldozer = newWallet;
    }
...
contract BullRun is Context, IERC20, Ownable {
    ...
    BullRunFeeHandler public brfFeeHandler;
    ...
    constructor (address _marketingWallet, address _opsWallet, address _farmWallet, address _usdc, address _bulldozer) {
        IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D);
        ...
        //brfFeeHandler = new BullRunFeeHandler(address(this), _marketingWallet, _opsWallet, _farmWallet, _usdc, _bulldozer);

        _isExcludedFromFees[owner()] = true;
        _isExcludedFromFees[address(this)] = true;
        //_isExcludedFromFees[address(brfFeeHandler)] = true;
        _isExcludedFromFees[address(0xdead)] = true;

        _isExcludedMaxTransactionAmount[owner()] = true;
        _isExcludedMaxTransactionAmount[address(this)] = true;
        //_isExcludedMaxTransactionAmount[address(brfFeeHandler)] = true;
       ...
       function setFeeHandler (address _feeHandler) public{	
        brfFeeHandler = BullRunFeeHandler(_feeHandler);	
        _isExcludedFromFees[address(brfFeeHandler)] = true;	
        _isExcludedMaxTransactionAmount[address(brfFeeHandler)] = true;	
    }
    ...
    function _transfer(
        ...
        bool canSwap = balanceOf(address(brfFeeHandler)) >= swapTokensAtAmount;
        ...
        if (
            ...
            brfFeeHandler.processFees(tokensForLiquidity, tokensForOps, tokensForMarketing, tokensForFarmRewards);
        ...
        if (!_isExcludedFromFees[from] && !_isExcludedFromFees[to]) {
            ...
            if (tRemainingFees > 0) {
                ...
                _rOwned[address(brfFeeHandler)] += rRemainingFees;
                if(_isExcluded[address(brfFeeHandler)])
                    _tOwned[address(brfFeeHandler)] += tRemainingFees;

                emit Transfer(from, address(brfFeeHandler), tRemainingFees);
    ...
    function updateMarketingWallet(address newWallet) external onlyOwner {
        brfFeeHandler.updateMarketingWallet(newWallet);
    }

    function updateOpsWallet(address newWallet) external onlyOwner {
        brfFeeHandler.updateOpsWallet(newWallet);
    }

    function updateFarmWallet(address newWallet) external onlyOwner {
        brfFeeHandler.updateFarmWallet(newWallet);
    }
    
    function updateBulldozerWallet(address newWallet) external onlyOwner {
        brfFeeHandler.updateBulldozerWallet(newWallet);
    }
    ...
    function updateFeeHandler(address newAddress) public onlyOwner {
        if (newAddress == address(brfFeeHandler)) {
        ...
        brfFeeHandler = newFeeHandler;

        emit UpdateFeeHandler(newAddress, address(brfFeeHandler));
    }
