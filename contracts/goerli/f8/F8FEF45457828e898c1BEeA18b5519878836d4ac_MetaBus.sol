...
contract MetaBus is ERC721A, IERC2981, IMetaBus, Ownable {
    ...
    constructor(string memory _uri, address _proxySaleAddress) ERC721A("Meta Bus", "Bus", 1) {
        uri = _uri;
    ...
    function mint(address _to) external override(IMetaBus) {
        ...
        require(1 <= maxTotalSupply - totalSupply(), "Minting more than the max total supply");
        require(_numberMinted(_to) > 0, "Minting more than the max supply for a single address");
        _safeMint(_to, 1);
    }
...
interface IProxySale {
    function isStake(uint256 _tokenId) external returns (bool);
}
