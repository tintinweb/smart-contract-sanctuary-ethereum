...
contract TokenMarketFacet is Modifiers {
    ...
    function createLoanToken(
        LibMarketStorage.LoanDetailsToken memory loanDetails
        ...
        if (
            LibMarketStorage.LoanTypeToken.SINGLE_TOKEN == loanDetails.loanType
        ) {
        ...
        ms.borrowerLoanIdsToken[LibMeta.msgSender()].push(newLoanId);
        ms.loanOfferIdsToken.push(newLoanId);
    ...
    function tokenLoanOfferCancel(uint256 _loanId) public whenNotPaused {
        LibMarketStorage.MarketStorage storage ms = LibMarketStorage
            .marketStorage();
        ...
        require(
            ms.borrowerLoanToken[_loanId].borrower == LibMeta.msgSender(),
    ...
    function tokenLoanCancelBulk(uint256[] memory _loanIds)
        external
    ...
    function activateLoanToken(
        ...
        for (uint256 i = 0; i < loanIds.length; i++) {
            ...
            ms.activatedLoanIdsToken[LibMeta.msgSender()].push(loanIds[i]);
...

            {
                ...
                ms.activatedLoanToken[loanIds[i]] = LibMarketStorage
                    .LenderDetails({
    ...
    function getActivatedLoanDetailsToken(uint256 _loanId)
        external
        view
    ...
    function getLoanOffersToken(uint256 _loanId)
        external
        view
        returns (LibMarketStorage.LoanDetailsToken memory)
    {
        LibMarketStorage.MarketStorage storage ms = LibMarketStorage
            .marketStorage();
