...
contract Wavect is ERC721, ERC2981, LinearlyAssigned, AddRecover, Pausable, ONFT721Core, IONFT721, Multicall, ReentrancyGuard {
    ...
    constructor(address _lzEndpoint, string memory contractURI_, string memory baseURI_, string memory name_,
        string memory ticker_, uint256 totalSupply_, bool disableMint_)
    ...
    function mint(bytes memory sig_, uint256 amount_) payable external whenNotPaused nonReentrant {
        require(isBelowMaxWallet(_msgSender(), amount_), "Max wallet"); // no nonce needed reuse sig until maxWallet reached
