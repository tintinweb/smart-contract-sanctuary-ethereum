...
contract CNPReborn is DefaultOperatorFilterer, ERC721AntiScam, AccessControl {
    // ==================================================================
    // Constants
    // ==================================================================
    ...
    // == For Grow ==
    // tokenId -> rate
    mapping(uint256 => uint256) private _growthRateToChild;
    mapping(uint256 => uint256) private _growthRateToAdult;

    // tokenId -> date
    mapping(uint256 => uint256) public birthDay;

    uint256 public standardPeriodToChild = 4 hours;
    uint256 public standardPeriodToAdult = 8 hours;

    mapping(uint256 => bool) public isSleeping;

    // == For Hold award
    // holder -> date
    mapping(address => uint256) public dateOfMeeting;

    constructor(address ownerAddress, uint256 ownerMintAmount)
        ERC721A("CNP Reborn", "CNPReborn")
    {
    ...
    function _birth(address to, uint256 amount) private {
        ...
        for (uint256 i = 0; i < amount; i++) {
            birthDay[_nextTokenId() + i] = timestamp;
        }
        _safeMint(to, amount);
    }
    ...
    function burn(address from, uint256[] calldata burnTokenIds)
        external
        payable
        onlyRole(BURNER)
    {
    ...
    {
        string memory lock = isLocked(tokenId) ? "_lock" : "";
        string memory stage = isAdult(tokenId) ? "adult" : isChild(tokenId)
            ? "child"
            : "egg";
        return
            string(
    ...
    function isAdult(uint256 tokenId) public view returns (bool) {
        return birthDay[tokenId] + standardPeriodToAdult <= block.timestamp;
    }

    function isChild(uint256 tokenId) public view returns (bool) {
        return birthDay[tokenId] + standardPeriodToChild <= block.timestamp;
    }
    ...
    function _beforeTokenTransfers(
        ...
        bool isReborn = dateOfMeeting[to] != 0 && from == address(0);
        if (to != address(0) && balanceOf(to) == 0 && !isReborn) {
            dateOfMeeting[to] = block.timestamp;
        }
    }
    ...
    function _afterTokenTransfers(
        ...
        bool isReborn = dateOfMeeting[from] != 0 && to == address(0);
        if (from != address(0) && balanceOf(from) == 0 && !isReborn) {
            delete dateOfMeeting[from];
        }
...
import "@openzeppelin/contracts/access/AccessControl.sol";
import "@openzeppelin/contracts/access/Ownable.sol";
import "./CNPReborn.sol";

contract CNPRebornManyHolder is ERC721A, AccessControl, Ownable {
    // ==================================================================
    // Constants
    // ==================================================================
    bytes32 public constant ADMIN = "ADMIN";
    ...
    string public baseExtension = ".json";
    address payable public withdrawAddress;
    CNPReborn private _reborn;

    constructor(address rebornAddress, address ownerAddress)
        ERC721A("CNP Reborn Many Holder", "CNPRebornMH")
    {
        _reborn = CNPReborn(rebornAddress);
        grantRole(ADMIN, msg.sender);
        withdrawAddress = payable(ownerAddress);
    }
    ...
    // original
    // ==================================================================
    function adminMint(address[] calldata to)
        external
        onlyRole(ADMIN)
    {
        for (uint256 i = 0; i < to.length; i++) {
    ...
    function mint() external payable {
        require(balanceOf(msg.sender) == 0, "You can only 1 token.");
        ...
        returns (string memory)
    {
        ...
        return string(abi.encodePacked(ERC721A.tokenURI(level), baseExtension));
    }

    function getLevel(uint256 tokenId) public view returns (uint256) {
        uint256 balance = _reborn.balanceOf(_reborn.ownerOf(tokenId));
    ...
    function _beforeTokenTransfers(
        ...
        require(from == address(0) || to == address(0), "This is SBT.");
    }
