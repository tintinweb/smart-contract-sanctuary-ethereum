...
contract DEX {
     ...
     */
    event LiquidityRemoved(address sender, uint256 liquidity, uint256 ethWithdrawn, uint256 tokenAmount);
    ...
    function price(
        ...
        // uint256 newYReserves = k.div(xInput + xReserves);
        // yOutput = yReserves.sub(newYReserves);

        uint256 k = xReserves.mul(yReserves).mul(1000);
        uint256 newYReserves = k.div(xInput.mul(997) + xReserves.mul(1000));
        yOutput = yReserves.sub(newYReserves);
    ...
    function tokenToEth(uint256 tokenInput) public returns (uint256 ethOutput) {
        ...
        require(token.transferFrom(msg.sender, address(this), tokenInput), 'DEX: tokenToEth - transfer did not transact');
        (bool sent,)=msg.sender.call{value: ethOutput}('');
        require(sent, 'Faild to send ETH');
    ...
    function deposit() public payable returns (uint256 tokensDeposited) {
        ...
        bool sent = token.transferFrom(msg.sender, address(this), tokenDeposit);
        require(sent, 'tokens not enough');
    ...
    function withdraw(uint256 amount) public returns (uint256 eth_amount, uint256 token_amount) {
        require(liquidity[msg.sender] > amount, 'sender does not have that much liquidity');
        ...
        require(tokenSent,'Failed to send token');
        (bool ethSent,)=msg.sender.call{value: ethWithdrawn}('');
