...
import "../manifold/soulbound/ERC721Soulbound.sol";
import "../edition/IERC721NumberedEdition.sol";


contract AlexCollabERC721Soulbound is ERC721Soulbound {
    
    function getOwners(address creatorContractAddress) public view returns (address[] memory) {
        uint256 totalSupply = IERC721NumberedEdition(creatorContractAddress).totalSupply();
        address[] memory owners = new address[](totalSupply);
        for (uint256 i = 0; i < totalSupply; i++) {
            owners[i] = IERC721(creatorContractAddress).ownerOf(i);
        }
        return owners;
    }
