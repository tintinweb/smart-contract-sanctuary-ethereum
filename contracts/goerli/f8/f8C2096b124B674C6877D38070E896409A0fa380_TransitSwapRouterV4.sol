...
contract TransitSwapRouterV4 is Ownable, ReentrancyGuard, Pausable {
    ...
    address constant private TetherToken = address(0xa614f803B6FD780986A42c78Ec9c7f77e6DeD13C);
        ...
        if (TransferHelper.isETH(desc.srcToken)) {
            ...
            if (preTradeModel) {
                ...
                if (TetherToken == desc.srcToken) {
                    TransferHelper.safeTransferWithoutRequire(desc.srcToken, desc.srcReceiver, desc.amount.sub(fee));
                } else {
        ...
        if (TransferHelper.isETH(desc.dstToken)) {
            ...
            if (preTradeModel) {
                ...
                if (TetherToken == desc.dstToken) {
                    TransferHelper.safeTransferWithoutRequire(desc.dstToken, desc.dstReceiver, returnAmount);
                } else {
