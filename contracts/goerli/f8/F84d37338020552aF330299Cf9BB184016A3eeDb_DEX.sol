...
contract DEX {
  ...
  function ethToToken() public payable returns (uint256 tokenOutput) {
    require(msg.value > 0, 'cannot swap 0 tokens');
    ...
    require(token.transfer(msg.sender, tokenOutput), 'ethToToken: transfer reverted');
  ...
  function deposit() public payable returns (uint256 tokensDeposited) {
    require(msg.value > 0, 'Must send value while depositing');
    ...
    require(token.transferFrom(msg.sender, address(this), tokenDeposit), 'deposit: transfer did not transact');
  ...
  function withdraw(uint256 amount) public returns (uint256 eth_amount, uint256 token_amount) {
    ...
    require(token.transfer(msg.sender, tokenAmount), 'withdraw: token transfer did not transact');
