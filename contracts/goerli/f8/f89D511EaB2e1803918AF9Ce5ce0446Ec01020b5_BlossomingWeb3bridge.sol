...
contract BlossomingWeb3bridge is ERC721A, AccessControl, PriceConsumerV3 {
    ...
    uint40 public whitelistAmount = 3;
    
    // Whitelist cost for minting 1 NFT; in USDT
    uint128 public costWhitelist = 300000000;
    // publicSale cost for minting 1 NFT; in USDT
    uint128 public costPublicSale = 500000000;
    // max total supply of the collection
    uint160 public constant TOTAL_COLLECTION_SUPPLY = 1000;
     ...
     * @param _uriPrefix link to where metadata is store
     * @param _usdt this the token use for payment
    ...
    ) ERC721A("BlossomingWeb3bridge", "Web3bridge") {
        usdt = _usdt;
    ...
    /**
     * @notice Whitelist mint function, this allow whitelist user  to mint max of 3 token and pay $300 in usdt for each
	...
	// function calWhitelistPriceEth() public view returns(uint){
    //    return (costWhitelist /uint(getLatestPrice()/1e6))*1e16;
    // }
    /**
     * @notice calculate price of eth in usd the reveal state
     * 
     ...
     * @param _uriPrefix The base extension
     */
    ...
    /**
     * @notice Sets the  extension
     * @param _uriSuffix The extension
     */
     ...
     * @param _usdt this the usdt contract address deploy on Ethereum mainnet
     */
    function setToken(address _usdt)
        external
        onlyRole(MODERATOR_FUNCTION_ROLE)
    {
        usdt = _usdt;
    ...
    /**
     * @notice Gets the whitelisted status of the given user and whitelist category
     * @param _user The user address
     * @param id The whitelist category id
     * @return True if user is whitelisted in the given whitelist category
     */
    function checkWhitelist(address _user, uint256 id)
        public
        view
        returns (bool)
    {
        uint256 addressCount = whitelistAddress[id].length;
        for (uint256 i = 0; i < addressCount; i++) {
            if (whitelistAddress[id][i] == _user) {
                return true;
            }
        }
        return false;
    ...
    /**
     * @notice Adds the given user address to the list of whitelisted users for the given whitelist category
     * @param _users The user address
     * @param _id The whitelist category id
     */
    function whiteListUsers(address[] calldata _users, uint256 _id)
        external
        onlyRole(MODERATOR_FUNCTION_ROLE)
    {
        delete whitelistAddress[_id];
        whitelistAddress[_id] = _users;
