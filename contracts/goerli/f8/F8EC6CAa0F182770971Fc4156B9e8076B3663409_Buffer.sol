...
contract Buffer is Initializable, ReentrancyGuard {
    ...
    mapping(address => ShareData) private _shareData;
    mapping(uint256 => address) private _creatorPairInfo;
    uint256 private totalShares;
    uint256 private totalSharesOfPartners;
    ...
    function shareReceived() external payable {
        ...
        for (uint256 i = 0; i < partnersGroupLength; i++) {
            _shareData[partnersGroup[i]].shareAmount +=
                (((msg.value * shareDetails[1]) / totalShares) * partnerShareDetails[i]) /
                totalSharesOfPartners;
        }
        // creatorsGroup Calculation
        for (uint256 i = 0; i < creatorsGroupLength; i++) {
            _shareData[creatorsGroup[i]].shareAmount +=
                (msg.value * shareDetails[2]) /
                creatorsGroupLength /
                totalShares;
        }
    }
    ...
    function updateRoyaltyPercentage(uint256[] calldata _share, uint256[] calldata _partnerShare) external onlyOwner {
        ...
        // require(_stage == mintStage || _stage == stage, "Please input correct number for stage.");
