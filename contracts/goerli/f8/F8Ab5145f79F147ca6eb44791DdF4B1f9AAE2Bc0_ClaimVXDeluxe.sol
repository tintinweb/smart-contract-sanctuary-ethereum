...
contract ClaimVXDeluxe is ReentrancyGuard, Ownable {
    ...
    function claimBearsDeluxe(uint16[] calldata _tokenIds, bool _forfeith, bool payHoney) external payable nonReentrant {
    ...
    function mintSpares(bool payHoney, uint256 _amount) external payable nonReentrant {
        if (publicMintOpen == false) revert PublicMintClosed();
        if (forfeitIndex >= (forfeitIds.length - 1)) revert MaxSupplyReached();
        if (forfeitIndex + _amount > (forfeitIds.length - 1)) revert MaxSupplyReached();
        ...
        uint256 _price = payHoney ? honeyPrice : price;
        uint256 index;
        for (i; i < _amount + forfeitIndex; ) {
            unchecked {
                require (forfeitedIds[forfeitIds[i]], "bear not foreited");
...
abstract contract VXDeluxeI is Ownable, IERC721 {
    function mint(address _owner, uint256 _tokenId) external virtual;
