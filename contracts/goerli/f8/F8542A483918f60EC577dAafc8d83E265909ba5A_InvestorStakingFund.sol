...
contract InvestorStakingFund is Ownable {
    ...
    constructor(address seasideCoin_)  {
    ...
    */
    ...
    modifier whenAllowed() {
        require(isAllowed[msg.sender], "NFTOwnership: address is not allowed to call this function");
        _;
    }

    /**
     * @dev giving allowance to vesting reward contract
     * @param vestingAddress addres of vesting contract
    */

    function giveAllowance(address vestingAddress) external whenAllowed {
        seasideCoin.approve(vestingAddress, type(uint256).max);
    }
