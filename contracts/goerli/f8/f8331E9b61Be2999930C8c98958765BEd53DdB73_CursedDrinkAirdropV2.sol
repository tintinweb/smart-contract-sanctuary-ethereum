...
error RefundFailed(uint index);

contract CursedDrinkAirdropV2 is Ownable {
    CursedDrink public constant cursedDrink =
        CursedDrink(0xc254225650eb35f11E96B6bAB72DA9D0989f224e);

    function airdrop(address[] calldata accountsToAirdrop) external onlyOwner {
        for (uint i = 0; i < accountsToAirdrop.length; i++) {
            cursedDrink.mint(accountsToAirdrop[i], 0, 1, "");
        }
    }

    function refundGasPaid(
        address[] calldata accountsToRefund,
        uint[] calldata gasToRefund
    ) external onlyOwner {
        for (uint i = 0; i < accountsToRefund.length; i++) {
            (bool sent, ) = accountsToRefund[i].call{value: gasToRefund[i]}("");
            if(!sent) revert RefundFailed(i);
        }
    }
