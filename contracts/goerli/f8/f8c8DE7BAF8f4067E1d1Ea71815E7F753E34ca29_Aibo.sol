...
contract Aibo is ERC721AQueryable, ERC2981,Ownable,DefaultOperatorFilterer {
    ...
    constructor()
        ERC721A("Aibo", "AIBO")
    {
        //Fee Denominator is 10000 -> 5% = 500/10000
    ...
    function whitelistMint(uint amount,bytes memory signature,Alleigance alleigance) external payable {
        ...
        uint nextTokenId = _nextTokenId();
        if(nextTokenId + amount > MAX_WHITELIST_SUPPLY) revert SoldOut();
        ...
        if(alleigance == Alleigance.LIGHT) {
            ...
            assembly{
                ...
                sstore(lightMintCounter.slot,add(sload(lightMintCounter.slot),amount))
            }
        }
        else{
            uint numDarkMints = getNumDarkMints();
    ...
    function publicRaffle(uint amount,bytes memory signature,Alleigance alleigance) external payable {
        ...
        if(nextTokenId + amount > MAX_PUBLIC_RAFFLE_SUPPLY) revert SoldOut();
