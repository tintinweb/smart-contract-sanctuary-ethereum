...
contract BaseRegistrarImplementation is ERC721, IBaseRegistrar, Ownable {
    ...
    constructor(DNS _dns, bytes32 _baseNode) ERC721("", "") {
        dns = _dns;
        baseNode = _baseNode;
    }

    modifier live() {
        require(dns.owner(baseNode) == address(this));
        _;
    }
    ...
    function setResolver(address resolver) external override onlyOwner {
        dns.setResolver(baseNode, resolver);
    }
    ...
    function _register(
        ...
        _mint(owner, id);
        if (updateRegistry) {
            dns.setSubnodeOwner(baseNode, bytes32(id), owner);
        }
...
contract ReverseRegistrar is Ownable, Controllable, IReverseRegistrar {
    ...
    /**
     * @dev Constructor
     * @param dnsAddr The address of the DNS registry.
     */
    constructor(DNS dnsAddr) {
        dns = dnsAddr;

        // Assign ownership of the reverse record to our deployer
        ReverseRegistrar oldRegistrar = ReverseRegistrar(
            dnsAddr.owner(ADDR_REVERSE_NODE)
        );
    ...
    modifier authorised(address addr) {
        require(
            addr == msg.sender ||
                ...
                dns.isApprovedForAll(addr, msg.sender) ||
                ownsContract(addr),
    ...
    /**
     * @dev Transfers ownership of the reverse DNS record associated with the
     *      calling account.
     * @param owner The address to set as the owner of the reverse record in DNS.
     * @return The DNS node hash of the reverse record.
     */
    ...
    function claimForAddr(
        ...
        dns.setSubnodeRecord(ADDR_REVERSE_NODE, labelHash, owner, resolver, 0);
        return reverseNode;
    }
    ...
    /**
     * @dev Sets the `name()` record for the reverse DNS record associated with
