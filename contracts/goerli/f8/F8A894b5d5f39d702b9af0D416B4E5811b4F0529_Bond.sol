...
import "../interfaces/IBumpTwapOracle.sol";
    ...
    IBond
{
    ...
    function calcBonding(
        ...
        _config.assetPerBump = bumpPrice * DIVIDER_ORACLE / assetPrice;  
        _config.stablePerBump = bumpPrice;
    }
    ...
    /// @inheritdoc	IBond
    function withdrawFrom(address from, uint amount)
        external
        virtual
        override
        whenNotPaused
        ...
        )
    {
        ...
        uint incentives = boostAmount + incentivesAmount;
        _lockBondPosition(recipient, bondAmount, incentives, toTransfer);
        bondAmount += incentives;        
    }
    ...
    function _withdraw(address to, uint amount) private {
        require(
            amount > 0 && depositedBalances[to] >= amount,
            "BB: bad-amount"
        );
        ...
        IERC20(BOND_TOKEN_ADDRESS()).safeTransfer(to, amount);
    }
    ...
    function _lockBondPosition(
        ...
        lockedBalances[recipient] += bondAmount + incentives;

        uint freeBonds = depositedBalances[recipient]; 

        if (freeBonds >= bondAmount) {
            ...
            total += incentives + bondAmount - freeBonds;
...
// SPDX-License-Identifier: AGPL-3.0-or-later
pragma solidity >=0.7.0; // TickMath requires old version

interface IBumpTwapOracle {
    function getPriceTwap(address uniswapV3Pool, uint32 twapInterval)
        external
        view
        returns (uint);
    ...
    function bumpPool() external view returns (address);

    function lastPrice() external view returns (uint);
    
    function lastPriceTimestamp() external view returns (uint32);    
}
