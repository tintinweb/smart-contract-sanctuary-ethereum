...
contract ArbOtc is Ownable, ReentrancyGuard {
    ...
    function createOffer(uint256 _costPerToken, uint256 _tokens) external nonReentrant {
    ...
    function cancelOffer(uint256 tradeId) external nonReentrant {
    ...
    function acceptOffer(uint256 tradeId) external nonReentrant {
        TradeOffer storage offer = tradeOffers[tradeId];
        require(offer.active, "Offer accepted or cancelled");
    ...
    function fufilOffer(uint256 agreementId) external nonReentrant {
    ...
    function getOffers(uint256 startIndex, uint256 endIndex) external view returns (TradeOffer[] memory) {
    ...
    function getAgreements(uint256 startIndex, uint256 endIndex) external view returns (Agreement[] memory) {
        require(startIndex < endIndex, "Invalid range");
    ...
    function setAddresses(address _USDC, address _ARB, address _AIRDROP) external onlyOwner {
        USDC_ADDRESS = _USDC;
        ARB_ADDRESS = _ARB;
        AIRDROP_ADDRESS = _AIRDROP;
    }

    function setMaxAndMin(uint256 _min, uint256 _max) external onlyOwner {
        MIN_COST = _min;
        MAX_COST = _max;
    }

    function setAirdropStart(uint256 startTimestamp) external onlyOwner {
