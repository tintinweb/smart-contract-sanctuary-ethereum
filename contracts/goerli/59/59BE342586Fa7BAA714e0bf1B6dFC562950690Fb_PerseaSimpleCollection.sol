...
contract PerseaSimpleCollection is ERC721URIStorage, ReentrancyGuard, Randomize() {
     ...
     */
    constructor() ERC721("M909","M909") {
      receiver =  payable(msg.sender);
    }

    function initialize(uint256 newPrice, uint256 newLimit) public {
        require(msg.sender == receiver,"You not are the owner");
        setTotalSupply(newLimit);
        setPrice(newPrice);
    }

    function initializeWhiteBaseURL(uint256 newPrice, uint256 newLimit, string memory _baseURL) public  {
        require(msg.sender == receiver,"You not are the owner");
        setTotalSupply(newLimit);
        setPrice(newPrice);
        baseURI = _baseURL;
    ...
    function safeURI( uint256 tokenId,string memory URI ) internal {
    ...
    function payableMint(uint256 quantity, string memory URI) public nonReentrant payable returns (uint256) {
    ...
    function getCurreentId() internal  returns (uint256){
        uint256 newItemId = 0;
        require(_totalSupply > 0, "Persea: doest have a total supply");
        require(totalLeft() > 0, "Persea: Not found ids in the list");
        if (totalLeft() > 1) {
            uint256 random = randomBetween(totalLeft(),1);
            newItemId = _listTokenIds[address(this)][random];
            _listTokenIds[address(this)][random] =  _listTokenIds[address(this)][totalLeft() - 1];
            _listTokenIds[address(this)].pop();
        } else {
            newItemId = _listTokenIds[address(this)][0];
            _listTokenIds[address(this)].pop();
        }
        return newItemId;
    }
    ...
    function totalLeft() public view returns(uint256) {
        return _listTokenIds[address(this)].length;
    }
    ...
    function setTotalSupply(uint256 newLimit) internal {
        delete _listTokenIds[address(this)];
        _totalSupply = newLimit;
        for (uint256 index = 1; index <= _totalSupply; index++) {
            _listTokenIds[address(this)].push(index);
        }
    }
