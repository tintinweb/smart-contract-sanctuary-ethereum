...
, IERC721Metadata
{
    ...
    function catBlocks_ownerOfTokenId(uint256 tokenId) external view returns (address){
        return _catBlocks.ownerOf(tokenId);
    }
    ...
    function airdropCatblockAsYoungKitten(uint256 tokenIdStart, uint256 tokenIdEndExclusive) public onlyArtist {
        // // TODO: Check if this is in range of catblocks
        // // TODO: Adjust the tokenId based on the accepted range
        // // TODO: Ensure not in range of already airdropped

        // require(tokenIdStart >= 73000000, 'm');
        // require(tokenIdEndExclusive <= 74000000, 'm');

        uint256 tokenId = tokenIdStart;
        while(tokenId < tokenIdEndExclusive){
            address ownerOfCatBlock = this.catBlocks_ownerOfTokenId(tokenId);
            _balances[ownerOfCatBlock] += 1;
            _owners[tokenId] = ownerOfCatBlock;
            // This does not have to be saved for the airdrop - it can be obtained at view time
            // _hashes[tokenId] = this.catBlocks_tokenIdToHash(tokenId);
        
            emit Transfer(address(0), ownerOfCatBlock, tokenId);
        }
    }
...
contract Art {
    ...
    function isDelegated () internal view returns (bool) {
        ...
        return address (this) != 0x9180111D01E0947880653e473F68Be59be13e337; // THIS_CONTRACT_ADDRESS
    }
