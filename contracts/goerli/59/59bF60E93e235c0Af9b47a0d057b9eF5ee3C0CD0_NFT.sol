...
contract NFT is ERC721 {
    ...
    uint256 public primaryPrice = 0 ether;
    ...
    function _baseURI() override internal pure returns (string memory) {
        return "https://bafybeicohdi3gagmhrlj36pvwtw44eh6jx5s5iau5z24qlo3edgpcwnzk4.ipfs.w3s.link/metadata/";
    }
  ...
  function mintAllowList(uint256 amountOfNFTs) external payable {
    require(!publicSaleStarted && presaleStarted, "PRESALE_CLOSED");
    require(_tokenIds.current() < totalSupply, "SOLD_OUT");
    require(
        _totalClaimed[msg.sender] + amountOfNFTs <= PRESALE_MAX_MINT,
        "EXCEED_PRESALE_MAX_MINT_PER_USER"
    );

    require(_allowList[msg.sender], "NOT_IN_ALLOW_LIST");
    require(presalePrice * amountOfNFTs >= msg.value, "INSUFFICIENT_ETH");
...
function mintPresale(uint256 amountOfNFTs) external payable {
  ...
  require(
      presaleAmountMinted + amountOfNFTs <= presaleSupply,
      "EXCEED_PRESALE_SUPPLY"
  );
  require(presalePrice * amountOfNFTs >= msg.value, "INSUFFICIENT_ETH");
...
function mint(uint256 amountOfNFTs) external payable {
  ...
  require(_tokenIds.current() < totalSupply, "SOLD_OUT");
  require(
    _tokenIds.current() + amountOfNFTs <= totalSupply,
      "EXCEEDS_MAX_SUPPLY"
  );
  require(amountOfNFTs <= MAX_PER_MINT, "EXCEEDS_MAX_PER_TRANSACTION");
  require(primaryPrice * amountOfNFTs >= msg.value, "INSUFFICIENT_ETH");
...
function togglePresaleStarted() external onlyOwner {
  publicSaleStarted = presaleStarted;
...
function devMint(uint256 amountOfNFTs) external onlyOwner {
  require(_tokenIds.current() + amountOfNFTs <= totalSupply, "SOLD_OUT");
