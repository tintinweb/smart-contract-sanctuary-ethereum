...
contract SettlementFacet is ISettlement, ReentrancyGuard {
    ...
    function batchSetFinalReferenceValue(
        FinalReferenceValueSet[] calldata _finalReferenceValues
    ) external override {
        // Get references to relevant storage slots
        LibDIVAStorage.PoolStorage storage ps = LibDIVAStorage._poolStorage();
        LibDIVAStorage.GovernanceStorage storage gs = LibDIVAStorage
        ...
        uint256 len = _finalReferenceValues.length;
        for (uint256 i = 0; i < len; ) {
            _setFinalReferenceValue(
                _finalReferenceValues[i].poolId,
                _finalReferenceValues[i].finalReferenceValue,
                _finalReferenceValues[i].allowChallenge,
                ps,
                gs
            );
            unchecked {
    ...
    function batchChallengeFinalReferenceValue(
        FinalReferenceValueChallenge[] calldata _finalReferenceValuesToChallenge
    ) external override {
        // Get references to relevant storage slots
        LibDIVAStorage.PoolStorage storage ps = LibDIVAStorage._poolStorage();
        LibDIVAStorage.GovernanceStorage storage gs = LibDIVAStorage
        ...
        uint256 len = _finalReferenceValuesToChallenge.length;
        for (uint256 i = 0; i < len; ) {
            _challengeFinalReferenceValue(
                _finalReferenceValuesToChallenge[i].poolId,
                _finalReferenceValuesToChallenge[i].proposedFinalReferenceValue,
                ps,
                gs
            );
            unchecked {
    ...
    function batchRedeemPositionToken(
        PositionTokenRedeem[] calldata _positionTokensRedeem
        ...
        uint256 len = _positionTokensRedeem.length;
        for (uint256 i = 0; i < len; ) {
            _redeemPositionToken(
                _positionTokensRedeem[i].positionToken,
                _positionTokensRedeem[i].amount,
                ps,
                gs
            );
            unchecked {
...
interface ISettlement {
     ...
     * @param _finalReferenceValues Struct array containing pool id,
     ...
     * @param _finalReferenceValuesToChallenge Struct array containing pool id
     ...
     * @param _positionTokensRedeem Struct array containing position token
     * and amount
     */
    function batchRedeemPositionToken(
        PositionTokenRedeem[] calldata _positionTokensRedeem
