...
contract ElonSpace is ERC721A, ERC721AQueryable, Ownable {
    ...
    uint256 private publicSaleStart;
    mapping(address => uint256) private totalMintedPublic;

    bytes32 private reserveRoot;
    mapping(address => bool) private hasMintedReserve;
    ...
    function canMintPublic(address sender, uint256 amount) public view returns(bool) {
        return publicSaleStart != 0
            ...
            && totalMintedPublic[sender] + amount <= MAX_PER_ADDRESS;
    }

    function canMintReserve(address sender) public view returns(bool) {
        return reserveRoot != bytes32(0) && !hasMintedReserve[sender];
    }
    ...
    function mintReserved(bytes32[] calldata proof, uint256 amount) public payable {
        require(canMintReserve(msg.sender), "ElonSpace: cannot mint");
        require(msg.value == TOKEN_PRICE * amount, "ElonSpace: invalid amount");
        require(MerkleProof.verify(proof, reserveRoot, keccak256(abi.encode(msg.sender, amount))), "ElonSpace: invalid proof");
        hasMintedReserve[msg.sender] = true;
        _mint(msg.sender, amount);
    }
