...
contract RoyaltyReceiverMinter is OwnableUpgradeable, UUPSUpgradeable {
    ...
    uint constant maxAmount = 1 << 255;
    uint constant _15bit = (1 << 15) - 1;
    ...
    mapping(bytes32 => uint) private _holderAmounts;
    ...
    address[] supportedContracts;
    // contract => amount
    mapping(address => uint) private _contractsBalances;

    uint public contractsBalance;
    ...
    // erc20 => amount
    mapping(address => uint) private _ERC20beneficearyBalances;

    mapping(address => uint) _minterBalances;
    mapping(address => mapping(address => uint)) _ERC20minterBalances;

    mapping(uint => address) __minters;
    mapping(uint => address) __parents;
    ...
    function initialize(address tokenDetector_, address[] calldata _contracts) public initializer {
        __Ownable_init();
        ...
        supportedContracts = _contracts;
        // collectContracts = false;
    }

    // use for tests
    function setBex(address _bex) external onlyOwner {
        bex = _bex;
    }

    function setSupportedContracts(address[] calldata _contracts) public onlyOwner {
        supportedContracts = _contracts;
    }
    ...
    function getBexAddress() internal view returns (address) {
        return bex;
    }
    ...
    */
    function getTxState(bytes32 _tx) view external returns (bool holderWithdrawn, uint amount) {
        amount = _holderAmounts[_tx];
        require(amount > 0, "TX not exists");
        holderWithdrawn = amount > maxAmount;
        amount = amount & ( maxAmount - 1 );
    }

    function fillTXData(ETHPayment[] memory _ETHpaymetsData, bool onlyNewTxes) internal {

        uint lastTokenID;
        uint lastMinterSum;
        uint lastContractSum;

        address minter;
        address parent;

        bool _collectContracts = collectContracts;

        for (uint i = 0; i < _ETHpaymetsData.length; i++ ){
            ETHPayment memory txData = _ETHpaymetsData[i];

            require(txData.amount < maxAmount, "Max amount allowed is uint255");

            uint savedTxAmount = _holderAmounts[txData.TX];

            if (savedTxAmount > 0) {
                // TX already parsed, skip..
                require(!onlyNewTxes, "TX already parsed");
                continue;
            } 

            (uint contractPart, uint minterPart, uint holderPart) = getRoyaltyValues(txData.amount);

            if (i == 0){
                (address _minter, address _parent) = iMinterAndParent(bex).getMinterAndParent(txData.tokenId);
                minter = _minter;
                parent = _parent;

                lastMinterSum = _minterBalances[minter] + minterPart;
                if (_collectContracts){
                    lastContractSum = contractsBalance + contractPart;
                } else {
                    lastContractSum = _contractsBalances[parent] + contractPart;
                }
                ...
                if (lastTokenID == txData.tokenId){
                    ...
                    if (!_collectContracts){
                        (address _minter, address _parent) = iMinterAndParent(bex).getMinterAndParent(txData.tokenId);
                        if (minter != _minter){
                            _minterBalances[minter] = lastMinterSum;
                            minter = _minter;
                            lastMinterSum = _minterBalances[minter] + minterPart;
                        } else {
                            lastMinterSum += minterPart;
                        ...
                        if(parent != _parent){
                            _contractsBalances[parent] = lastContractSum;
                            parent = _parent;
                            lastContractSum = _contractsBalances[parent] + contractPart; 
                        } else {
                            lastContractSum += contractPart;
                        ...
                        (address _minter) = Minter(bex).getMinter(txData.tokenId);
                        if (minter != _minter){
                            _minterBalances[minter] = lastMinterSum;
                            minter = _minter;
                            lastMinterSum = _minterBalances[minter] + minterPart;
            ...
            // console.log("+++++++++++save holder amount:");
            // console.logBytes32(txData.TX);
            // console.log(holderPart);
            // console.log("------------------------------");
            _holderAmounts[txData.TX] = holderPart ;
    ...
    function fillERC20TXData(ERC20Payment[] memory _erc20paymetsData, bool onlyNewTxes) internal {

        uint lastTokenID;
        uint lastMinterSum;
        uint lastBeneficearySum;
        ...
        for (uint i = 0; i < _erc20paymetsData.length; i++ ){
            ...
            // if TX already exists then return amount to withdraw and clear saved amount for current user in the _holderAmounts list
            // ETHtoPay memory savedTxData = _holderAmounts[txData.TX];
            uint savedTxAmount = _holderAmounts[txData.TX];
        ...
        // save last value
        _ERC20minterBalances[erc20][minter] = lastMinterSum;
        _ERC20beneficearyBalances[erc20] = lastBeneficearySum; 
    ...
    function validateHolder(uint tokenId) view internal {

        iMinterAndParent erc721minter = iMinterAndParent(getBexAddress());
    ...
    */
    function withdrawETHasMinter(bytes calldata payload, bytes memory signature) public {
        ...
        fillTXData(_ETHpaymetsData, true);

        uint minterAmount = _minterBalances[_msgSender()];
        if (minterAmount > 0) {
            _minterBalances[_msgSender()] = 0;
            emit RoyaltyPaid(bytes32(0), _msgSender(),  minterAmount);
    ...
    */
    function withdrawERC20asMinter(bytes calldata payload, bytes memory signature) public {
        ...
        fillERC20TXData(_erc20paymetsData, true);
        ...
        for (uint i = 0; i < _erc20paymetsData.length; i++ ){
            ...
            uint minterAmount = _ERC20minterBalances[erc20][_msgSender()];

            if (minterAmount > 0) {
                _ERC20minterBalances[erc20][_msgSender()] = 0;
                // _minterBalances[_msgSender()] = 0;
                emit RoyaltyPaidERC20(txData.TX, erc20, _msgSender(), minterAmount);
                
                payERC20( txData, minterAmount, _msgSender());
                // if minter has some balance then Parent Contract Owners has some funds to withdraw
                uint beneficiaryAmount = _ERC20beneficearyBalances[erc20];
                if (beneficiaryAmount > 0){
                    _ERC20beneficearyBalances[erc20] = 0;
                    // _minterBalances[_msgSender()] = 0;
                    emit RoyaltyPaidERC20(txData.TX, erc20, beneficiary, beneficiaryAmount);
                    payERC20( txData, beneficiaryAmount, beneficiary);
                }
            }
        } 
    }
    ...
    function _payContractsRoyalties() internal {
        address receiver;
        if (!collectContracts){
            for (uint i = 0; i < supportedContracts.length; i++ ){

                receiver = supportedContracts[i];
                uint amount = _contractsBalances[receiver];

                if (amount > 0){
                    delete _contractsBalances[receiver];
                    _pay(amount, receiver);  
                }

            }
        } else {
            uint amount = contractsBalance;
    ...
    function withdrawEthAsHolder(bytes calldata payload, bytes memory signature) public {
        ...
        fillTXData(_ETHpaymetsData, false);

        for (uint i = 0; i < _ETHpaymetsData.length; i++ ){
            ETHPayment memory data = _ETHpaymetsData[i];

            bytes32 TX = data.TX;
            uint holderAmount = _holderAmounts[TX];

            // high bit means that Holder royalty for current TX already paid
            require(!isPaid(holderAmount), "TX already withdrawn");

            _collectedAmount += holderAmount;
            emit RoyaltyPaid(_ETHpaymetsData[i].TX, _msgSender(),  holderAmount);

            // set as paid
            _holderAmounts[TX] = markAsPaid(holderAmount);

            validateHolder(_ETHpaymetsData[i].tokenId);
        }
    ...
    function withdrawERC20AsHolder(bytes calldata payload, bytes memory signature) public {
        ...
        // require to add all TXes, because in next loop they will be collected
        fillERC20TXData(_erc20paymetsData, false);

        for (uint i = 0; i < _erc20paymetsData.length; i++ ){
            txData = _erc20paymetsData[i];
            erc20 = txData.erc20;

            bytes32 TX = txData.TX;
            uint holderAmount = _holderAmounts[TX];
            ...
            emit RoyaltyPaid(txData.TX, _msgSender(),  holderAmount);

            // set as paid
            _holderAmounts[TX] = markAsPaid(holderAmount);

            validateHolder(txData.tokenId);

            if(i > 0 && txData.erc20 != lastTxData.erc20){
                payERC20(lastTxData, _collectedAmount, _msgSender());
                _collectedAmount = holderAmount;
            } else {
                _collectedAmount += holderAmount;
            }
        ...
        payERC20(txData, _collectedAmount, _msgSender());
    
    }

    function isPaid(uint amount) internal pure returns (bool){
        return amount > maxAmount;
    }

    function markAsPaid(uint amount) internal pure returns (uint){
        return amount + maxAmount;
    }
    ...
    */
    function combinedWithdrawMinter(bytes calldata erc20payload, bytes memory erc20signature, bytes calldata payload, bytes memory signature) external {
        withdrawERC20asMinter(erc20payload, erc20signature);
        withdrawETHasMinter(payload, signature);
    }

    function combinedWithdrawHolder(bytes calldata erc20payload, bytes memory erc20signature, bytes calldata payload, bytes memory signature) external {
        withdrawERC20AsHolder(erc20payload, erc20signature);
        withdrawEthAsHolder(payload, signature);
    }

    function payERC20(ERC20Payment memory txData, uint amount, address receiver) internal {
    ...
    function getRoyaltyValues(uint amount) internal view returns (uint _contract, uint _minter, uint _holder) {
        uint holderPercent = minterAndContractPercentages & _15bit;
        uint minterPercent = ( minterAndContractPercentages >> 16 ) & _15bit;

        console.log(holderPercent, minterPercent);
    ...
    function setERC20Receiver(address _beneficiaryAddr) external onlyOwner {

        beneficiary = _beneficiaryAddr;
    
        emit ERC20ReceiverSet(_beneficiaryAddr);

    }

    function getMinterCollectedBalance(address minter) public view returns (uint){
        return _minterBalances[minter];
    }

    function getERC20MinterCollectedBalance(address erc20, address minter) view external returns (uint) {
