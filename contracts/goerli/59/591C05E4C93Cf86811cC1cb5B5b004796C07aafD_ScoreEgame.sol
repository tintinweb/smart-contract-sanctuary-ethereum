...
contract ScoreEgame is ReentrancyGuard {
  ...
  function claimScore(uint256 _score) public nonReentrant returns (bool)  {
    ...
    // sum the score
    uint256 oldScore = getScore(msg.sender);
    ...
    // validating score
    // but make pointer first
    bytes32 newPointer = setPointer(msg.sender, oldScore);
    return validating(newPointer, getPointer(msg.sender));
  }

  function calculateToken(address _player) public nonReentrant {
    uint256 _score = getScore(_player) ;
    require(_score > 0,"Must be => 100 to claim");
    uint256 _token = 0;
    ...
    // reset the score
    uint256 resetScore = _score % 100;
    mappingPlayer[_player].score = resetScore;
  ...
  // transfer toke to player
  function transferTokenE(uint256 amount) public payable nonReentrant {
    uint256 balanceToken = tokenContract.balanceOf(address(this));
    require(amount > 0, "You need make score again");
    require(amount <= balanceToken, "Not enough tokens in the reserve");
  ...
  function sell(uint256 amount) public nonReentrant {
    require(amount > 0, "Sell at least some tokens");
  ...
  function validating(bytes32 newPointer, bytes32 oldPointer) private pure returns (bool) {
    return (newPointer == oldPointer);
  }

  function calculateReward(uint256 _token) private pure returns (uint256) {
    uint256 reward = _token * 1e18;
    return reward;
  }
