...
contract Tomi is Initializable, ERC20Upgradeable, OwnableUpgradeable {
    ...
    event tomiWalletUpdated(
        address indexed tomiWallet
    );

    event rewardPoolSuperComputerUpdated(
        address indexed rewardPoolSuperComputer
    );
    ...
    function inititalize(IPioneerNFT nftContract_, address marketingWallet_, address tomiWallet_, address vestingContract_) initializer public {
        __ERC20_init("Tomi Test Token", "tomi");
        __Ownable_init();
        nftContract = nftContract_;
        vestingContract = vestingContract_;
        marketingWallet = marketingWallet_;
        tomiWallet = tomiWallet_;
        emissions = emissionCriteria(18000 ether, 39000 ether, 8000 ether, 18000 ether, 17000 ether, 0, true);
    ...
    function updateTomiWallet(address newAddress) external onlyOwner {
        ...
        emit tomiWalletUpdated(newAddress);
    }
    ...
    function beforeAuctionMint(address buyer, uint256 quantity) internal {
        ...
        _mint(tomiWallet, emissions.beforeAuctionTomi.mul(quantity));
    }
