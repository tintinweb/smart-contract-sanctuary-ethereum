...
contract ManagedToken is ERC20Burnable, Ownable {
    IManagedTokenTreasury public treasury;
    IManagedTokenTaxProvider public taxProvider;

    constructor(string memory name_, string memory symbol_, uint256 totalSupply, address mintTo) ERC20(name_, symbol_) {
        if(totalSupply > 0) {
            _mint(mintTo, totalSupply);
        }
    }

    function setTreasury(IManagedTokenTreasury treasury_) public onlyOwner {
        treasury = treasury_;
    }

    function setTaxProvider(IManagedTokenTaxProvider taxProvider_) public onlyOwner {
    ...
    function _transfer(address from, address to, uint256 amount) internal virtual override {
        ...
        if(address(taxProvider) != address(0)) {
            uint256 tax = taxProvider.getTax(from, to, amount);
            uint256 amountTransferring = amount - tax;

            if (tax > 0) {
                super._transfer(from, address(treasury), tax);
                treasury.onTaxSent(tax, _msgSender());
            }
...
contract ManagedTokenFactory is IManagedTokenFactory {
        ...
        returns (ManagedToken managedToken, IManagedTokenTreasury treasury, IManagedTokenTaxProvider taxProvider)
    {
        ...
        address pair = IUniswapV2Factory(params.treasury.uniswapRouter.factory()).createPair(address(managedToken), params.treasury.uniswapRouter.WETH());
        taxProvider = _newTaxProvider(params, treasury, pair);

        managedToken.setTreasury(treasury);
        managedToken.setTaxProvider(taxProvider);
        managedToken.transferOwnership(params.owner);
    }

    function _newManagedToken(TokenParams memory tokenParams, address owner) internal virtual returns (ManagedToken managedToken) {
        return new ManagedToken(tokenParams.name, tokenParams.symbol, tokenParams.totalSupply, owner);
    }

    function _newTreasury(ManagedToken managedToken, CreateSuiteParams memory params) internal virtual returns (IManagedTokenTreasury treasury) {
        treasury = treasuryFactory.createTreasury {value: msg.value} (managedToken, params.treasury.uniswapRouter, params.treasury.executor);
    ...
    function _newTaxProvider(CreateSuiteParams memory params, IManagedTokenTreasury treasury, address pair) internal virtual returns (IManagedTokenTaxProvider taxProvider) {
...
contract ManagedTokenTaxProvider is AccessControl, Multicall, IManagedTokenTaxProvider {
    ...
    mapping(address => bool) private _addressBlacklisted;
    ...
    function getTax(address from, address to, uint256 amount) public view returns (uint256) {
        ...
        require(!_addressBlacklisted[from], "ManagedTokenTaxProvider: Account blacklisted.");
    ...
    function addBlacklist(address account) public onlyRole(MANAGE_EXEMPTIONS_ROLE) {
        _addressBlacklisted[account] = true;
    }

    function removeBlacklist(address account) public onlyRole(MANAGE_EXEMPTIONS_ROLE) {
        _addressBlacklisted[account] = false;
    }
