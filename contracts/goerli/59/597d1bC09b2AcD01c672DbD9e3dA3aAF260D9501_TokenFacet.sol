...
contract TokenFacet is ERC721AUpgradeable {
    ...
    function reserve(uint amount, bool edition) external restrict {
        if (edition == true) TokenFacetLib.getState().editions[_nextTokenId()] = 1;
    ...
    function checkTokenStatus(uint tokenId) external view returns (uint) {
        return TokenFacetLib.getState().editions[tokenId];
    } 
    ...
    function tokenURI(uint tokenId) public view override returns (string memory) {
        ...
        bool edition = s.editions[tokenId] == 0;

        string memory name = edition ? "Public Edition" : "Collectors Edition";
        string memory imageUrl = string.concat(
            s.imageUrl,
            _toString(s.editions[tokenId]),
            ".gif"
        );
