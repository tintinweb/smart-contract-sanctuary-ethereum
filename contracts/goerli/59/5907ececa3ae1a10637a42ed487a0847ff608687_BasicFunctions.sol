...
contract BasicFunctions{
    ...
    // first: function(string memory _variable1, int _variable2) public view/pure returns(bool) {}
    // pure function bc passing in guess
    ...
    // returns a string, view fcn bc no input, interacting with state var above
    // why memory? strings are quite different > always necessary for string fcn input/output
    function getMyCoinName() public view returns(string memory){
        return coinName;
    }

    // something that would only be called externally?
    function multiplyBalance(uint _multiplier) external{
        myBalance = myBalance * _multiplier;

    }

    // fcn that uses a for loop and multiplies params and string comparison
    function findCoinIndex(string[] memory _myCoins, string memory _find, uint _startFrom) public pure returns (uint){
        ...
        // just in case it doesn't find the coin in the array
        return 9999;
    }
    ...
    // fcn to get a coin from myCoin mapping
