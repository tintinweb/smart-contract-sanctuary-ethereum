...
contract TwapDelay is ITwapDelay {
    ...
    function _executeDeposit(
        Orders.DepositOrder memory depositOrder,
        ...
        if (depositOrder.swap) {
            bytes memory data = encodePriceInfo(pair, depositOrder.priceAccumulator, depositOrder.timestamp);
            if (amount0Left != 0) {
        ...
        )
    {
        ...
        uint256 amount0Desired = tokenShares.sharesToAmount(token0, depositOrder.share0, amountLimit0, depositOrder.to);
        uint256 amount1Desired = tokenShares.sharesToAmount(token1, depositOrder.share1, amountLimit1, depositOrder.to);
        ITwapPair(pair).sync();
        (amount0Left, amount1Left) = AddLiquidity.addLiquidityAndMint(
            pair,
            depositOrder.to,
    ...
    function _executeBuy(Orders.BuyOrder memory buyOrder, uint256 amountLimit) external {
        require(msg.sender == address(this), 'TD00');
        ...
        (uint256 amount0Out, uint256 amount1Out) = buyOrder.inverse ? (amountOut, uint256(0)) : (uint256(0), amountOut);
    ...
    function _executeSell(Orders.SellOrder memory sellOrder, uint256 amountLimit) external {
        require(msg.sender == address(this), 'TD00');
    ...
    function _executeSellHelper(
        Orders.SellOrder memory sellOrder,
        uint256 amountLimit,
        address pairAddress,
        address tokenIn,
