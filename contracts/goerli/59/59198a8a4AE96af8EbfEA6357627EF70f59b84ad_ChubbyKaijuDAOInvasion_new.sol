...
contract ChubbyKaijuDAOInvasion_new is ERC721Enumerable, Ownable{
    ...
    constructor(address _corpAddress, address _legacyAddress) ERC721("ChubbyKaijuDAOInvasion", "CKAIJU2"){
    ...
    function migration(uint256[] memory tokenIds) external {
        for(uint i=0; i<tokenIds.length;i++){
            require(kaiju_legacy.ownerOf(tokenIds[i])==msg.sender, "Not the Owner of Zombie");
            require(tokenIds[i]<6667, "Must Migrate Zombie");
    ...
    function callWithRadio(uint256 typeId, uint256 amount) external {
        require(balanceOf(msg.sender)>0, "Must Own Zombie");
