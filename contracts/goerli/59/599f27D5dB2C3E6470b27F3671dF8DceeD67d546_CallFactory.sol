...
contract CallPool is ICallPool, Pausable {
   ...
   */
    ...
    function withdrawETH(
        ...
        _balanceOf[_msgSender()] = userBalance - amount;
    ...
    function openCall(
        ...
        require(premiumToOwner + premiumToReserve == msg.value, Errors.CP_PREMIUM_AND_ETH_UNEQUAL);
        ...
        _balanceOf[address(this)] += premiumToReserve;
    ...
    function openCallBatch(
        ...
        require(totalPremium <= msg.value, Errors.CP_DID_NOT_SEND_ENOUGHT_ETH);
        _balanceOf[address(this)] += totalReservePremium;
        uint256 excessEth = msg.value - totalPremium;
        if(excessEth != 0){
            _balanceOf[_msgSender()] += excessEth;
            emit DepositETH(_msgSender(), _msgSender(), excessEth);
        }
    }
    ...
    function collectProtocol(
        ...
        if (amountSent > 0) {
            _balanceOf[address(this)] -= amountSent;
    ...
    function getNFTStatus(uint256 tokenId) external view override returns (bool, bool, uint8, uint8) {
        return (
            nftStatus[tokenId].ifOnMarket,
            nftStatus[tokenId].available,
            nftStatus[tokenId].lowerStrikePriceGapIdx,
            nftStatus[tokenId].upperDurationIdx
        );
    }
...
interface ICallPoolActions {
    ...
    // Get NFTStatus
    function getNFTStatus(uint256 tokenId) external view  returns (bool, bool, uint8, uint8);
}
