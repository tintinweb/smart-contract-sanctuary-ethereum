...
contract LilFarmBoy is 
   ...
   modifier mintMod(uint256 _qty, bytes32[] memory _proof) {
      ...
      if(mintPhaseData[currMintPhase].maxWallet > 0){
         require((userData[msg.sender].mintCount[currMintPhase] + _qty) <= mintPhaseData[currMintPhase].maxWallet, 'LFB :: Wallet minted out!');
      }
   ...
   function mint(uint256 _qty, bytes32[] memory _proof) external payable mintMod(_qty, _proof) {
   ...
   function setTeamAddress(address[] memory _team, bool _isAllowed) external onlyOwner {
      for(uint256 index = 0; index < _team.length; index++){
         allowedTeamAddress[_team[index]] = _isAllowed;
      }
   }

   function setApprovedAddress(address[] memory _user, bool _isAllowed) external onlyOwner {
      for(uint256 index = 0; index < _user.length; index++){
         approvedAddress[_user[index]] = _isAllowed;
      }
   }
...
interface IProofOfFunds {
  ...
  function depositNativeFund() external payable;
}
