...
contract OCASnake is Ownable, ERC721A, ERC2981, ReentrancyGuard, OCASnakeArt {
    ...
    mapping(uint256 => bytes32) snakenames;
    ...
    function testNumber(uint256 tokenId) public view returns (uint256) {
        return generateAppleSeed(tokenId, 0) % currentIndex;
    }
    ...
    function snakeNameSave(uint256 tokenId, bool extraName) private {
        ...
        if (isSet(name)) snakeNameSave(tokenId + block.timestamp, true);
        else {
    ...
    function _safeMint(
        ...
        for (uint256 i = 0; i < quantity; i++) {
            ...
            snakeNameSave(startTokenId + block.timestamp, false);
...
contract OCASnakeArt is Ownable, ReentrancyGuard {
    bool public turnOnThePower = false;
