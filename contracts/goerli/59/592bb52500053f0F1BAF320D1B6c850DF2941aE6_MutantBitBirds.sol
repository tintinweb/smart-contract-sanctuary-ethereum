...
contract MutantBitBirds is ERC721, ERC721Enumerable, Pausable, Ownable, ERC2981 { //, RoyaltiesV2Impl {
    ...
    uint256 public _nameChangePrice = 300 ether;
    
    MutantCawSeed public _yieldToken;
    /*
    <?xml version="1.0"?>
    ...
    */   
    ...
    constructor(uint16 maxTotalSupply, uint16 reserveSupply) ERC721("MutantBitBirds-20220205-1", "MTB") {
	...
	function setYieldToken(address yieldtkn) external onlyOwner {
		_yieldToken = MutantCawSeed(yieldtkn);
	}    
    ...
    */
	...
	function setNickName(uint256 tokenId, string calldata nickname) public {
        ...
        require(_yieldToken.balanceOf(msg.sender) >= _nameChangePrice, "more cawSeed");
        _yieldToken.burn(msg.sender, _nameChangePrice);
   ...
   function generateCharacterSvg(uint256 tokenId) internal view returns (bytes memory) {
	...
	function generateCharacter(uint256 tokenId) internal view returns(bytes memory){
        uint8 offset = 21;
        uint256 bof = balanceOf(ownerOf(tokenId));
        if (bof >= 10)
            offset = 19;
        else if (bof >= 100)
            offset = 17;
        ...
        */
        ...
        '<text x="',
        bytes(Strings.toString(offset)),
        '" y="4" font-size="4px">',
        bytes(Strings.toString(bof)),      
        '</text>',
