...
contract LPStakingV4 is Initializable, ReentrancyGuardUpgradeable, OwnableUpgradeable {
        ...
        if (_stake > 0) {
            ...
            if (isPaused) {
                if (stakeDetail.lastRewardCalculated > pausedTimestamp) {
                    ...
                    bool dynamic = stakeDetail.isDyanmic;
                    uint256 apr = dynamic
                        ? getDyanmicAPR(stakeDetail.poolId, stakeDetail.period, _ethUSD, _stake)
                        : stakeDetail.APR;
                    reward =  dynamic?_stake.mul(apr).mul(pausedTimestamp.sub(stakeDetail.lastRewardCalculated)).div(
                        APRTime.mul(100).mul(1000))
                        :_stake.mul(apr).mul(pausedTimestamp.sub(stakeDetail.lastRewardCalculated)).div(
                        APRTime.mul(100).mul(1000))/_mpwrUSD;
    ...
    function calculateReward(
        ...
        reward = stakeDetail.reward + _calculateReward(_stakeHolder, _dId, _ethUSD,_mpwrUSD, _amount, isProrata);
    }
    ...
    function stake(
        ...
        require(_verify(msg.sender, tokenId, amount, _ethUSD,_mpwrUSD, _signature), "invalidated");
        ...
        uint256 uncalculatedReward = _calculateReward(msg.sender, id, _ethUSD,_mpwrUSD, amount, true);
        ...
        deposits[msg.sender][id].APR = getAPR(poolId, period);
        ...
        deposits[msg.sender][id].isDyanmic = poolPeriodEth[poolId][period] == 0 ? false : true;
    ...
    function withdraw(
        ...
        require(_verify(msg.sender, details.NFTId, _stake, _ethUSD,_mpwrUSD, _signature), "invalidated");
       

        uint256 reward = _calculateReward(msg.sender, _dId, _ethUSD,_mpwrUSD, _stake, isForceWithdraw);
    ...
    function _verify(
        ...
        bytes32 signedHash = keccak256(abi.encodePacked(_staker, NAME_HASH, _tokenId, _amountUSD, _ETHUSD,_mpwrUSD));
