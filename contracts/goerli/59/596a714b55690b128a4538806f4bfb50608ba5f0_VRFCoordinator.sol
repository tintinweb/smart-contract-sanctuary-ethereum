    ...
    TypeAndVersionInterface
{
    ...
    function requestRandomness(
        ...
        billSubscriberForRequest(msg.sender); // throws on failure
    ...
    function processCallback(
        ...
        bytes32 cbCommitment = keccak256(
            abi.encode(blockHeight, confDelay, cb)
        );
        ...
        emit GasSpentCallWithExactGas(gasBefore - gasAfter);
    ...
    event GasSpent(uint256 amount);
    event GasSpentCallWithExactGas(uint256 amount);
        ...
        validateProducer
    {
        ...
        // control flow.
        // TODO: is the if check needed? use low level call
        if (!i_link.transfer(recipient, juelsAmount)) revert InsufficientBalance();
    }
    ...
    ERC677ReceiverInterface
{
    ...
    // this contract through requestRandomness() and requestRandomnessFulfillment()
    // A discrepancy with this contract's link balance indicates someone
    // sent tokens using transfer or transferAndCall and so we may need to use recoverFunds.
    // TODO: is recover needed?
    ...
    //TODO: some of these events can be removed
    ...
    // and bills the user using transferFrom()
    function billSubscriberForRequest(address requester)
        internal
        ifContract(requester)
    {
        // Calculate the request price.
        uint256 costJuels = calculateRequestPriceJuels();

        // Reverts if requester doesn't have enough balance or hasn't approved enough amount
        // TODO: does this bubble up error message?
        i_link.transferFrom(requester, address(this), costJuels);
    }
    ...
    function billSubscriberForCallback(VRFBeaconTypes.Callback memory callback)
        internal
        ifContract(callback.requester)
    {
        // Calculate the request price.
        uint256 costJuels = calculateRequestPriceCallbackJuels(callback);

        // Reverts if requester doesn't have enough balance or hasn't approved enough amount
        i_link.transferFrom(msg.sender, address(this), costJuels);
    }
    ...
    function refundCallback(
        ...
        // Refund the consumer.
        i_link.transfer(callback.requester, refundJuels);
    }
    ...
    modifier ifContract(address account) {
        uint256 size;
        assembly {
