...
contract MarketService is MarketDomain,Sequence,WalletService,ConfigCenter{
    ...
    internal
   {
    ...
    function _addMarketOptionEntitys(
      uint256 poolId,
...
//   function _findBetEntities(uint256 [] memory betIds)internal pure returns(MarketBetEntity [] memory entities){
//     for(uint256 i=0;i<betIds.length;i++){
//       MarketBetEntity memory tmp = _findBetEntity(betIds[i]);
//       if(tmp.betId>0){
//         entities.push(tmp);
//       }
//     }
...
contract OddsSwap is IOddsSwap,Ownable,MarketService{
  ...
  function addMarketPoolBetOptions(
    ...
    _addMarketOptionEntitys(poolId,optionArr,initOddsArr,betTotalAmountArr);
