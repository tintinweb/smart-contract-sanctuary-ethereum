...
contract BaseSweep is Initializable, ERC20Upgradeable, AccessControlUpgradeable, OwnableUpgradeable {
    // Core
    address public timelock_address;
    ...
    function setMinterMaxAmount(address minter_address, uint256 max_mint_amount) external onlyByOwnGov {
        require(minters[minter_address].is_listed == true, "Minter nonexistant");
    ...
    function setMinterEnabled(address minter_address, bool is_enabled) external onlyByOwnGov {
        require(minters[minter_address].is_listed == true, "Minter nonexistant");
...
contract SweepDollarCoin is BaseSweep {
    ...
    uint256 public yearly_interest_rate; // 6 decimals of precision, e.g. 50000 = 5%
    uint256 public second_interest_rate; // 27 decimals of precision, e.g. 30000 = 3%
    ...
    public initializer {
        BaseSweep.__Sweep_init(_timelock_address, "SWEEP Dollar Coin", "SWEEP");
        _mint(msg.sender, GENESIS_SUPPLY);

        setInterestRate(0, 0);
        setPriceTarget(1000000);
    ...
    /* ========== PUBLIC FUNCTIONS ========== */
    function beginNewPeriod(uint256 _new_period_time) public onlyByOwnGov {
    ...
    function refreshInterestRate() public {
        ...
        require(block.timestamp - last_call_time >= period_time, "Must wait for the refresh cooldown since last refresh");

        // Step increments are 0.1% (upon genesis, changable by setSweepStep())

        if (current_amm_price > current_target_price.add(price_band)) { // decrease interest rate
            yearly_interest_rate = yearly_interest_rate.sub(sweep_step);
        } else if (current_amm_price < current_target_price.sub(price_band)) { // increase interest rate
            yearly_interest_rate = yearly_interest_rate.add(sweep_step);
        }

        last_call_time = block.timestamp; // Set the time of the last expansion

        emit InterestRateRefreshed(yearly_interest_rate);
    }
    ...
    // Set Sweep Interest Rate
    function setInterestRate(uint256 _yearly_interest_rate, uint256 _second_interest_rate) public onlyByOwnGov {
        yearly_interest_rate = _yearly_interest_rate;
        second_interest_rate = _second_interest_rate;

        emit InterestRateSet(_yearly_interest_rate, _second_interest_rate);
    }
    ...
    function setPriceTarget(uint256 _new_target_price) public onlyByOwnGov {
        ...
        if (second_interest_rate > 0) {
            next_target_price = second_interest_rate._rpow(period_time, ONE)._rmul(current_target_price, PRICE_PRECISION);
        } else {
    ...
    function setPeriodTime(uint256 _period_time) public onlyByOwnGov {
    ...
    event InterestRateSet(uint256 yearly_rate, uint256 second_rate);
