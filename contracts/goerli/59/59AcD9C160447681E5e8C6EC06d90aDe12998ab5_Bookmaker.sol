...
contract Bookmaker is DataOracle{
    ...
    BetOffer[] _betOffers;
    uint256 _openBetsCounter = 0;       //active bet offers waiting for player2 
    mapping(address => uint) addressBetsCount;  //number of bets by user address

    event NewBetOffer();
    event BetOfferAccepted();
    event BetOfferExpired();
    event BetGameEnds();

    modifier notSettled(uint id) {
        require(!_betOffers[id].isSettled, _betSettledError);  
        _;
    }
    ...
    modifier noPlayer2(uint id) {
        require(_betOffers[id].player2 == address(0), _player2Error);
        _;
    }
    ...
    function getGameById(bytes32 id) public view returns(GameCreate memory){
        return gameIdGame[id];
    }

    function getOpenBetOffers() external view returns(BetOffer[] memory){
        BetOffer[] memory openBets = new BetOffer[](_openBetsCounter);
        uint counter = 0;

        for (uint i = 0; i < _betOffers.length; i++){
            if(!_betOffers[i].isSettled && _betOffers[i].player2 == address(0)){
                openBets[counter] = _betOffers[i];
                counter++;
            }
        }
        return openBets;
    ...
    function getSenderBets() external view returns(BetOffer[] memory){
        BetOffer[] memory acceptedBets = new BetOffer[](addressBetsCount[msg.sender]);
        uint counter = 0;

        for (uint i = 0; i < _betOffers.length; i++){
            if(_betOffers[i].player1 == msg.sender || _betOffers[i].player2 == msg.sender){
                acceptedBets[counter] = _betOffers[i];
                counter++;
            }
        }
        return acceptedBets;
    ...
    function createBetOffer(bytes32 gameId, uint16 bet, uint expectedWin) validBet(bet) gameDeadline(gameId) payable external {
        require(msg.value > 0, _stakeError);                    
        require(msg.value < expectedWin, _expectedWinError); 
        //todo: check if given gameId exist in _games or gameIdGame          
        
        _betOffers.push(BetOffer(_betOffers.length, msg.sender, gameId, bet, msg.value, expectedWin, address(0), 0, false));
        _openBetsCounter++;
        addressBetsCount[msg.sender]++;
        emit NewBetOffer();
    }

    function cancelBetOffer(uint betId) notSettled(betId) noPlayer2(betId) external{
        require(msg.sender == _betOffers[betId].player1, _cancelingAddressError);
        
        payable(msg.sender).transfer(_betOffers[betId].stake);  //refund stake to player
        _betOffers[betId].isSettled = true;
        _openBetsCounter--;
    }

    function joinBetOffer(uint betId, uint16 bet) notSettled(betId) validBet(bet) noPlayer2(betId) gameDeadline(_betOffers[betId].gameId) payable external {
        require(_betOffers[betId].player1 != msg.sender, _ownOfferError);      
        require(_betOffers[betId].player1Bet != bet, _sameBetError);      
        require(msg.value == (_betOffers[betId].expectedWin - _betOffers[betId].stake), _expectedWinError2);  
        
        _betOffers[betId].player2 = msg.sender;
        _betOffers[betId].player2Bet = bet;

        addressBetsCount[msg.sender]++;
        _openBetsCounter--;

        emit BetOfferAccepted();
    }

    function checkResultAndProcessCashOut(uint betId) notSettled(betId) external {
        GameCreate memory game = gameIdGame[_betOffers[betId].gameId];
        require(game.startTime < (block.timestamp - 3 hours), _gameEndedError);

        //check if winner is known or call oracle
        if(gameIdResult[game.gameId].statusId == 8){ //final
            address winner = address(0);
            if (gameIdResult[game.gameId].homeScore > gameIdResult[game.gameId].awayScore){
                if(_betOffers[betId].player1Bet == 1){
                    winner = _betOffers[betId].player1;
                }else if (_betOffers[betId].player2Bet == 1){
                    winner = _betOffers[betId].player2;
                }
            }else if (gameIdResult[game.gameId].homeScore < gameIdResult[game.gameId].awayScore){
                if(_betOffers[betId].player1Bet == 2){
                    winner = _betOffers[betId].player1;
                }else if (_betOffers[betId].player2Bet == 2){
                    winner = _betOffers[betId].player2;
                }
            }else{
                if(_betOffers[betId].player1Bet == 0){
                    winner = _betOffers[betId].player1;
                }else if (_betOffers[betId].player2Bet == 0){
                    winner = _betOffers[betId].player2;
                }
            }

            if(winner != address(0)){                               //if someone win bet
                uint winValue = _betOffers[betId].expectedWin;
                payable(winner).transfer(winValue);                 //send win to winner
            }       
            _betOffers[betId].isSettled = true;                   //mark bet as settled

        }else if(gameIdResult[game.gameId].statusId == 0){  //no record
            uint256 sportId = 4; //NBA
            uint256 date = game.startTime; 
            bytes32[] memory gamesId = new bytes32[](1);
            gamesId[0] = game.gameId;
            requestResolveGame(sportId, date, gamesId);
        }else{
            //handle result with unexpected status
        }
    }

    function cashOutWin(uint betId) notSettled(betId) external {
        GameCreate memory game = gameIdGame[_betOffers[betId].gameId];
        require(gameIdResult[game.gameId].statusId == 8);
...
contract DataOracle is ChainlinkClient {
     ...
     * todo: restrict calling this function to oracle address only
     **/
        ...
        recordChainlinkFulfillment(_requestId)
    {
        for (uint256 i = 0; i < _requestedGames.length; i++) {
            ...
            if(gameIdGame[game.gameId].gameId != 0){
                _games.push(game);
                gameIdGame[game.gameId] = game;
            }            
        }

        emit newAvaibleGames();
    }
    ...
    function requestResolveGame(uint256 _sportId, uint256 _date, bytes32[] memory _gamesId) public {
        uint256 payment = 100000000000000000; //0.1 LINK
        Chainlink.Request memory req = buildOperatorRequest(
            jobId,
