...
contract NanaOnline{
    ...
    function buyCoins(uint256 _amount) payable public notYetWhitelisted {
        ...
        addCoins(msg.sender, _amount);
    }

    function useCoins(address _user, uint256 _amount) public notYetWhitelisted {
        subCoins (_user, _amount);
    }

    function addCoins(address _user, uint256 _amount) internal notYetWhitelisted {
        coinHolders[_user] = coinHolders[_user] + _amount;
    }

    function subCoins(address _user, uint256 _amount) internal notYetWhitelisted {
        if(coinHolders[_user] < _amount){
