...
contract CrossFarming is MessageSenderApp, MessageReceiverApp {
    ...
    function sendFarmMessage(
        ...
        uint256 feeBNB = (tx.gasprice * estimateGaslimit(_account, _msgType) * exchangeRate * compensationRate) /
            (EXCHANGE_RATE_PRECISION * COMPENSATION_PRECISION);

        // paid to executor receiver
        receiver.transfer(feeBNB);

        // msg.value for 'sendMessage' messagebus only add 1.
        IMessageBus(messageBus).sendMessage{value: IMessageBus(messageBus).calcFee(message) + 1}(
            _receiver,
            _dstChainId,
            message
        );
