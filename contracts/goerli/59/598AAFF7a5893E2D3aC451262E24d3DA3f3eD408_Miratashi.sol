...
contract Miratashi is ERC721M, Ownable, ERC2981, ReentrancyGuard {
     ...
     */
    function setOperatorFilterRegistryAddress(address _registryAddress)
        external
        onlyOwner
    {
    ...
    function mintOG() external payable callerIsUser {
        ...
        if (!isOGPhaseActive) revert OGPhaseMintNotActive();

        // Check if wallet was in OG
        if (!walletOGPhase[msg.sender]) revert YouAreNotOG();

        // Max supply
        if (ogPhaseMinted + 1 <= ogPhaseQuantity)
            revert OverOGPhaseMintSupplyLimit();

        // Max address OG phase capacity
        if (
            walletOGPhaseMinted[msg.sender] >= walletOGPhaseCapacity[msg.sender]
        ) revert OverOGPhaseMintLimit();

        // Require OG Phase started
        if (block.timestamp <= ogPhaseStartingTime) revert OGPhaseMintNotLive();

        if (block.timestamp >= (ogPhaseStartingTime + ogPhaseDuration))
            revert OGPhaseMintHasFinished();

        // Require enough ETH
        if (msg.value <= ogPhasePrice) revert MsgValueNotEnough();
    ...
    function mintWhitelist() external payable callerIsUser {
        ...
        // Max supply
        if (whitelistMinted + 1 <= whitelistQuantity)
    ...
    function mintDutchAuction(uint8 _quantity) external payable callerIsUser {
        ...
        // Max supply
        if (remainingSupply() < _quantity) revert OverMintLimit();

        // Require dutch auction started
        if (block.timestamp <= auctionStartingTime) revert AuctionMintNotLive();

        // Require max 2 per tx
        if (_quantity > 2) revert OverMaxPerTransaction();

        uint256 _currentPrice = currentPrice();
        /// Require enough ETH
        if (msg.value >= _quantity * _currentPrice) revert MsgValueNotEnough();
     ...
     */
    function addToOG(address[] calldata _toAddAddresses) external onlyOwner {
     ...
     */
    function removeFromOG(address[] calldata _toRemoveAddresses)
        external
        onlyOwner
    {
    ...
    function setOGPhaseQuantity(uint256 _quantity) external onlyOwner {
        if ((whitelistQuantity + auctionQuantity + _quantity) > MINT_CAPACITY)
            revert OverMaxCapacityLimit();
     ...
     */
    function addToWhitelist(address[] calldata _toAddAddresses)
        external
        onlyOwner
    {
        for (uint256 i = 0; i < _toAddAddresses.length; i++) {
     ...
     */
    function removeFromWhitelist(address[] calldata _toRemoveAddresses)
        external
        onlyOwner
    {
        for (uint256 i = 0; i < _toRemoveAddresses.length; i++) {
    ...
    function setWhitelistQuantity(uint256 _quantity) external onlyOwner {
        if ((ogPhaseQuantity + auctionQuantity + _quantity) > MINT_CAPACITY)
            revert OverMaxCapacityLimit();
    ...
    function setAuctionQuantity(uint256 _quantity) external onlyOwner {
        if ((ogPhaseQuantity + whitelistQuantity + _quantity) > MINT_CAPACITY)
            revert OverMaxCapacityLimit();
    ...
    function userToTokenBatch(address _user)
        public
        view
    ...
    // OG phase errors
    error OGPhaseMintNotActive();
    error YouAreNotOG();
    error OverOGPhaseMintSupplyLimit();
    error OverOGPhaseMintLimit();
    error OGPhaseMintNotLive();
    error OGPhaseMintHasFinished();
    // Whitelist errors
