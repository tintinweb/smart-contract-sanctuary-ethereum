...
contract XPass is ERC2981, ERC721URIStorage, Pausable, OwnableExtended, Whitelist  {
	...
	mapping(address => uint96) public defaultRoyalty;
	...
	function setDefaultRoyalty(uint96 feeNumerator) external onlyCreator {
		defaultRoyalty[_msgSender()] = feeNumerator;
	}

	function resetDefaultRoyalty() external onlyCreator {
		delete defaultRoyalty[_msgSender()];
	}
	...
	function royaltyInfo(uint _tokenId, uint _salePrice) public view virtual override returns (address, uint) {
		address royaltyReceiver = ownerOf(_tokenId);
		(address tokenRoyaltyReceiver, uint tokenRoyaltyAmount) = super.royaltyInfo(_tokenId, _salePrice);

		if (tokenRoyaltyReceiver != address(0)) {
			return (tokenRoyaltyReceiver, tokenRoyaltyAmount);
		}

		uint royaltyFraction = defaultRoyalty[royaltyReceiver];

		uint256 royaltyAmount = (_salePrice * royaltyFraction) / super._feeDenominator();
...
contract OwnableExtended is Ownable {
	...
	mapping(address => bool) internal creatorApprovals;
	...
	modifier onlyCreator() virtual {
		require(creatorApprovals[_msgSender()], "Not creator");
		_;
	}
	...
	function setCreator(address operator) internal {
		if (!creatorApprovals[operator]) creatorApprovals[operator] = true;
	}
