...
contract TicketAuction is Ownable, IERC721Receiver, ReentrancyGuard {
    ...
    event BidEvent(address bidder, uint256 ethAmountInWei, uint256 ticketAmount, uint256 usdValue);
    ...
    */
    ...
    function bid(uint256 ticketAmount) external payable nonReentrant {
        ...
        require(bidValueToUSD(currentWinningBid.value) > newBidUSD, "Must be a greater bid than the current bid!");
        if (ticketAmount > 0) {
        ...
        emit BidEvent(msg.sender, msg.value, ticketAmount, newBidUSD);
    }
    ...
    function topupBid(uint256 ticketAmount) external payable nonReentrant {
        ...
        emit BidEvent(msg.sender, newEthAmount, newTicketAmount, newBidUSD);
    }
    ...
    function getInfo(address user) public view returns (Phase phase, Bid memory winningBid, uint256 tickets, uint256 auctionEndTime, bool hasAuctionEnded, uint256 topBidValue) {
        phase = currentPhase;
