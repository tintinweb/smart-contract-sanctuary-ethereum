...
contract FakeSilo {
    ...
    function flashLiquidate(address[] memory _users, bytes memory _flashReceiverData)
        ...
        assets = new IERC20[](debts.length);
        receivedCollaterals = new uint256[][](1);
        receivedCollaterals[0] = new uint256[](assets.length);
        shareAmountsToRepaid = new uint256[][](1);
        shareAmountsToRepaid[0] = new uint256[](assets.length);
        for (uint i = 0; i < assets.length; i++) {
            unchecked {
                assets[i] = debts[i].asset;
                shareAmountsToRepaid[0][i] = uint256(debts[i].debt);
                {
                    uint256 amount = receivedCollaterals[0][i] = assets[i].balanceOf(address(this));
                    if (amount > 0) {
                        assets[i].transfer(msg.sender, amount);
                    }
                }
            }
        }
        IFlashLiquidationReceiver(msg.sender).siloLiquidationCallback(_users[0], assets, receivedCollaterals[0], shareAmountsToRepaid[0], _flashReceiverData);
    }
}