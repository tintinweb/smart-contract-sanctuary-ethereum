...
contract SingleAssetVault is ERC4626, Ownable
{
	...
	function addVault(address vault) external onlyOwner
	{
		...
		vaults.add(vault);
		IERC20(asset()).safeIncreaseAllowance(address(vault), type(uint256).max);
	...
	function totalPortfolioScore() public view returns (uint256 total) {
        for (uint i = 0; i < vaults.length(); i++)
            total += oracle.getPortfolioScore(vaults.at(i));
    }
	...
	{
		uint totalBalance = totalAssets();

		uint256 newPricePerShare = pricePerToken();

		if (newPricePerShare > lastPricePerShare)
		{
			uint256 oldTotalBalance = totalSupply() * lastPricePerShare / 10 ** decimals();
			...
			/*lastPricePerShare = */newPricePerShare = (totalBalance - fee) / totalSupply() * 10 ** decimals();
	...
	function totalAssets() public view virtual override returns (uint256 totalBalance)
	{
		for (uint i = 0; i < vaults.length(); i++)
		{
			IERC4626 vault = IERC4626(vaults.at(i));
			uint shares = vault.balanceOf(address(this));
			uint balanceAtVault = vault.convertToAssets(shares);
	...
	function computeScoreDeviationInPpm(address vaultAddress) public view returns (int256)
	{
		...
		uint256 assetsInVault = vault.convertToAssets(vault.balanceOf(address(this)));
	 ...
	 */
	function rebalance(address sourceVaultAddress, address destinationVaultAddress, uint256 assets) external
	{
		...
		IERC4626(sourceVaultAddress).withdraw(assets, address(this), address(this));
		uint256 value = IERC20(asset()).balanceOf(address(this)) - previousBalance;
	...
	function _deposit(
        ...
        for (uint i = 0; i < vaults.length(); i++) {
            ...
            vault.deposit(
				(assets *
					oracle.getPortfolioScore(address(vault))) /
					totalScore,
				address(this)
			);
        }
...
contract ApyFlow is ERC20, Ownable {
    ...
    constructor(address _converter, address _oracle)
        ERC20("ApyFlow", "APYFLW")
    {
        require(_converter != address(0), "Zero address provided");
        require(_oracle != address(0), "Zero address provided");
        assetConverter = AssetConverter(_converter);
        oracle = PortfolioScore(_oracle);
    }
    ...
    function addVault(address vault) external onlyOwner {
        ...
        token.safeIncreaseAllowance(address(assetConverter), type(uint256).max);
    ...
    function _verifyAmounts(uint256[] memory amounts) internal view {
        ...
        for (uint i = 0; i < amounts.length; i++) {
            SingleAssetVault vault = SingleAssetVault(vaults.at(i));
            uint256 expected = (totalAmount *
                vault.totalPortfolioScore() *
                10**(vault.decimals())) / (totalScore * (10**18));
    ...
    function depositToVault(uint index, uint amount) external {
        ...
        returns (uint256[] memory amounts)
    {
        ...
        for (uint i = 0; i < vaults.length(); i++) {
            ...
            uint256 sharesToBurn = (shares *
                vault.convertToAssets(vault.balanceOf(address(this))) *
                (10**decimals())) / ((10**vault.decimals()) * totalAssets());
            uint amountToWithdraw = (convertToAssets(sharesToBurn) *
                (10**vault.decimals())) / (10**18);
