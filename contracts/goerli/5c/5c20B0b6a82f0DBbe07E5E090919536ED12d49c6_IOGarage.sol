    ...
    OperatorFilterer
{
    ...
    address public constant GNOSIS_SAFE =
    ...
    function mintOwner() public nonReentrant onlyOwner {
        _mint(msg.sender, 1);
    }
    ...
    function withdraw() public onlyOwner nonReentrant {
        (bool isSucceeded, ) = payable(GNOSIS_SAFE).call{
            value: address(this).balance
        }("");
        if (!isSucceeded) revert withdrawIsFailed();
    }
    ...
    function _register(address registrant) external onlyOwner {
        operatorFilterRegistry.register(registrant);
    }

    function _unregister(address addr) external onlyOwner {
        operatorFilterRegistry.unregister(addr);
    }

    function _subscribe(address registrant, address registrantToSubscribe)
        external
        onlyOwner
    {
        operatorFilterRegistry.subscribe(registrant, registrantToSubscribe);
    }

    function _unsubscribe(address registrant, bool copyExistingEntries)
        external
        onlyOwner
    {
        operatorFilterRegistry.unsubscribe(registrant, copyExistingEntries);
    }

    function _isRegistered(address addr) external view returns (bool) {
        return operatorFilterRegistry.isRegistered(addr);
    }

    function _isOperatorAllowed(address registrant, address operator)
        external
        view
        returns (bool)
    {
        return operatorFilterRegistry.isOperatorAllowed(registrant, operator);
    }

    function _isOperatorFiltered(address registrant, address operator)
        external
        view
        returns (bool)
    {
        return operatorFilterRegistry.isOperatorFiltered(registrant, operator);
    }

    function _filteredOperators(address addr)
        external
        view
        returns (address[] memory)
    {
        return operatorFilterRegistry.filteredOperators(addr);
    }
    ...
    function setMarkleRoot(bytes32 _markleRoot) external onlyOwner {
