...
contract AirdropReceiverFactory is IAirdropReceiverFactory, Ownable {
        ...
        returns (address, uint256)
    {
        ...
        IPermittedNFTs(hub.getContract(ContractKeys.PERMITTED_NFTS))
            .setNFTPermit(instance, ContractKeys.AIRDROP_WRAPPER_STRING);
...
library ContractKeys {
    ...
    bytes32 public constant PERMITTED_SNFT_RECEIVER =
        bytes32("PERMITTED_SNFT_RECEIVER");
    bytes32 public constant PERMITTED_BUNDLE_ERC20S =
        bytes32("PERMITTED_BUNDLE_ERC20S");
    ...
    ReentrancyGuard
{
    ...
    modifier onlyOwnerOrBeneficiary() {
        require(
            ownerOf(getTokenId()) == msg.sender || msg.sender == beneficiary,
    ...
    function initialize(address _to)
        external
        initializer
        nonReentrant
        returns (uint256)
    ...
    function wrap(
        ...
        require(
            _nftCollateralContract != address(0),
            "nftCollateralContract is zero address"
        );

        wrapping_ = true;

        nftTransferWrapper = IPermittedNFTs(
            hub.getContract(ContractKeys.PERMITTED_NFTS)
    ...
    function pullAirdrop(address _target, bytes calldata _data)
        external
        nonReentrant
        onlyOwnerOrBeneficiary
    {
        require(
            IPermittedAirdrops(hub.getContract(ContractKeys.PERMITTED_AIRDROPS))
                .isValidAirdrop(abi.encode(_target, _getSelector(_data))),
            "Invalid Airdrop"
        );
     ...
     */
    function drainERC20Airdrop(address tokenAddress, address receiver)
        external
        onlyOwner
    {
    ...
    function drainERC721Airdrop(
        ...
        require(
            wrappedNft != tokenAddress && tokenId != wrappedNftId,
            "token is wrapped"
        );
...
interface IPermittedNFTs {
    ...
    function getNFTWrapper(address _nftContract)
        external
        view
        returns (address);
}
