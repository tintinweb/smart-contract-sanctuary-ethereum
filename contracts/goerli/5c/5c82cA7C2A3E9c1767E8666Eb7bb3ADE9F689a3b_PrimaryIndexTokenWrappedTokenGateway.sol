...
interface IPrimaryIndexTokenLeverage 
{
    ...
    function leveragedBorrowFromRelatedContract(address projectToken, address lendingToken, uint notionalExposure, uint marginCollateralAmount, bytes memory buyCalldata, address borrower) external;
...
contract PrimaryIndexTokenWrappedTokenGateway is Initializable, AccessControlUpgradeable, ReentrancyGuardUpgradeable
{
    ...
    function liquidateWithLendingETH(address _account, address _projectToken, uint256 _lendingTokenAmount) public payable {
        WETH.deposit{value: msg.value}();
        WETH.transfer(msg.sender, msg.value);
    ...
    function liquidateWithProjectETH(address _account, address _lendingToken, uint256 _lendingTokenAmount) public {
    ...
    function leveragedBorrowWithProjectETH(address _lendingToken, uint _notionalExposure, uint _marginCollateralAmount, bytes memory buyCalldata) public payable{
        ...
        pitLeverage.leveragedBorrowFromRelatedContract(address(WETH), _lendingToken, _notionalExposure, _marginCollateralAmount, buyCalldata, msg.sender);
    }
