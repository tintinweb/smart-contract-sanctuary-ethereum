...
contract TokenManagementFacet {
    ...
    /// @notice
    event NewToken(address newTokenAddress, uint8 newTokenType, uint256 liquidityMin, uint8 newLisitingId);

    /// @notice
    event SetTokenLimits(uint256 liquidityMin, uint88 limitPerTx, uint8 listingId);
    ...
    /// @param _liquidityMin the balance minimum limit of this token, 0 mean no limit.
    ...
    function addToken(
        ...
        s.tokenType[_newListingId] = _tokenType;
        s.liquidityMin[_newListingId] = _liquidityMin;
        emit NewToken(_tokenAddress, _tokenType, _liquidityMin, _newListingId);
    }
    ...
    function validateLisitingId(uint8 _listingId) external view returns (address) {
