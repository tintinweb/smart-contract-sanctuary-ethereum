...
contract Pool is IPool, ERC721, Ownable2Step {
    ...
    function transferFrom(
        ...
        IncomeRouter router = IncomeRouter(s_nfrRouters[nfrId]);
        IncomeRouter.NFRData memory nfr = router.getNFR(nfrId);
        IIncomeVerifier verifier = IIncomeVerifier(
            s_verifiers[nfr.verificationSource]
        );
        verifier.preTransfer(from, to, nfrId, nfr);
        super.transferFrom(from, to, nfrId);
    }
