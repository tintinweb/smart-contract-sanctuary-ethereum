...
contract ChainpassMarketplace_1_0 is AuthHelper, ReentrancyGuard {
    ...
    constructor(address  _chainpassRegistry) {
        __AuthHelper_init(_chainpassRegistry);
    ...
    function buyTickets(
        ...
        MetaTransactionInput calldata attendeeMetaTXInput,
        address buyer,
        uint256 maxPrice,
        MetaTransactionInput calldata buyerMetaTXInput
    ) external payable nonReentrant {
        require(tokenIds.length > 0, "There needs to be at least one tokenId.");
        require(
            isSignerOrCaller(attendee, MetaTransaction(attendeeMetaTXInput, abi.encode("buyTickets", ticketContractAddress, tokenIds))),
            "Incorrect attendee signature."
        );
        require(
            isSignerOrCaller(buyer, MetaTransaction(buyerMetaTXInput, abi.encode("buyTickets", maxPrice))),
            "Incorrect buyer signature."
        );

        IChainpassEventTickets ticketContract = IChainpassEventTickets(ticketContractAddress);

        uint256 totalPrice = transferTicketsAndCalculateTotalPrice(
            ticketContract,
        ...
        if (royaltyReceiver != address(0)) {
            ...
            deductUSDC(buyer, royaltyReceiver, royaltyAmount);
        }

        // transfer CP cut
        uint256 chainpassCut = (totalPrice * getChainpassBPSForEvent(ticketContractAddress)) / 10_000;
        deductUSDC(buyer, getChainpassWallet(), chainpassCut);
    
        // transfer the rest to the event host
        deductUSDC(buyer, ticketContract.owner(), totalPrice - royaltyAmount - chainpassCut);

        emit TicketsBought(ticketContractAddress, tokenIds);
    }

    function transferTicketsAndCalculateTotalPrice(
        IChainpassEventTickets ticketContract,
        address ticketContractAddress,
        uint256[] calldata tokenIds,
        address attendee
        ...
        for (uint256 i = 0; i < tokenIds.length; i++) {
            ...
            ticketContract.transferFrom(ticketContract.ownerOf(tokenId), attendee, tokenId);
    ...
    function deductUSDC(address from, address to, uint256 amount) private {
        IERC20 usdcContract = IERC20(registry().getUSDCContractAddress());
        usdcContract.transferFrom(from, to, amount);
    }

    function getChainpassBPSForEvent(address ticketContractAddress) private view returns (uint256) {
        return eventFactory().getChainpassPrimaryBPS(ticketContractAddress);
    }

    function getChainpassWallet() private view returns (address) {
...
struct MetaTransactionInput {
    bytes32 nonce;
    uint8 v; bytes32 r; bytes32 s;
}

struct MetaTransaction {
    MetaTransactionInput input;
    bytes data;
}

abstract contract AuthHelper is OwnableUpgradeable {
    address chainpassRegistryContractAddress;
    mapping(address => mapping(bytes32 => bool)) usedNoncesBySignerAddress;

    function __AuthHelper_init(address _chainpassRegistryContractAddress) internal initializer {
        chainpassRegistryContractAddress = _chainpassRegistryContractAddress;
        __Ownable_init();
    }

    function isMetaChainpassWallet(MetaTransaction memory metaTx) public returns (bool) {
        address chainpassWallet = IChainpassRegistry(chainpassRegistryContractAddress).getChainpassWallet();
        return msg.sender == chainpassWallet || isSigner(chainpassWallet, metaTx);
    }

    function isChainpassWallet() public view returns (bool) {
        address chainpassWallet = IChainpassRegistry(chainpassRegistryContractAddress).getChainpassWallet();
        return msg.sender == chainpassWallet;
    }

    function isMetaOwner(MetaTransaction memory metaTx) public returns (bool) {
        return msg.sender == owner() || isSigner(owner(), metaTx);
    }
    ...
    function isSignerOrCaller(address signer, MetaTransaction memory metaTx) public returns (bool) {
        return msg.sender == signer || isSigner(signer, metaTx);
    }

    function isSigner(address signer, MetaTransaction memory metaTx) public returns (bool) {
        require(!usedNoncesBySignerAddress[signer][metaTx.input.nonce], "nonce already used");
        usedNoncesBySignerAddress[signer][metaTx.input.nonce] = true;
        bytes32 msgHash = keccak256(abi.encode(address(this), signer, metaTx.input.nonce, metaTx.data));
        return verifySignature(msgHash, metaTx.input.r, metaTx.input.s, metaTx.input.v) == signer;
    }
...
interface IChainpassRegistry {
    ...
    function getChainpassTicketsBaseContractAddress() external view returns (address);
}
