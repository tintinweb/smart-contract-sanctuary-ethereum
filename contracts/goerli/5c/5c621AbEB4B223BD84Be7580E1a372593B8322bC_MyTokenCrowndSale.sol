...
contract MyTokenCrowndSale is Ownable {
    ...
    constructor(uint256 eth_rate, uint256 usdt_rate, address payable wallet, IERC20 icotoken) {
        ETH_rate = eth_rate;
        USDT_rate = usdt_rate;
        _wallet = wallet;
        token = icotoken;
    ...
    function buyTokenByUSDT(uint256 USDTAmount) external payable {
        uint256 amount = getTokenAmountETH(USDTAmount);

        require(msg.sender.balance >= USDTAmount, 
