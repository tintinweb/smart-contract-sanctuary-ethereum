...
contract EggNft is ERC721, Ownable {
    ...
    uint256 private wlPrice = 0.002 ether;
    uint256 private plPrice = 0.003 ether;
    ...
    function getUserWhiteListed(bytes32[] calldata _merkleProof) external view returns (uint256) { 
        ...
        uint256 value;
        if (mintType == 0) {
            require(MerkleProof.verify(_merkleProof, merkleOgListRoot, leaf) == true, "Not registered at og");
            value = 1;
        } else if (mintType == 1) {
            require(MerkleProof.verify(_merkleProof, merkleWlListRoot, leaf) == true, "Not registered at wl");
            value = 2;
        } else {
            value = 3;
        }
