...
contract FairytalesAndConspiracies is ERC1155, Ownable {
    ...
    function mintNFTs(address payable buyer, uint16[] memory ids) external payable returns (uint16[] memory) {
        ...
        if (price == 0) {
            revert("Cannot mint already taken tokens");
        }

        for (uint16 i = 0; i < tokensToMint.length; i++) {
            if (tokensToMint[i] > 0) {
    ...
    function buyNFTs(address payable from, address to, uint256[] memory ids, uint256[] memory amounts, bytes memory data) external payable {
        uint price = 0;
    ...
    function lockNFTs(uint16[] memory ids) external returns (uint16[] memory) {
        ...
        for (uint16 i = 0; i < counter; i++) {
            if (tokensToLock[i] > 0) {
                tokensLocked[tokensToLock[i] - 1] = true;
            }
        }
