    ...
    ReentrancyGuard
{
    ...
    constructor(
        ...
        ERC721Premint("Example NFT", "NFT", premintTo_, maxTokenId_)
     ...
     * @param batch The batch to set the operator for
     * @param operator The operator contract that get's approval to all the minters tokens
     * @param approved If the operator is approved to manage the batch
     */
    function setBatchOperator(
        uint256 batch,
        address operator,
        bool approved
    ) public override onlyOwnerOrAdmin {
        // Manage token approvals for operator
        isApprovedForAll[minter][operator] = approved;
