...
contract SupperClubMint is ERC721AUpgradeable, OwnableUpgradeable,
    ...
    function initialize(string memory _name, string memory _symbol, address _designatedSigner) public initializer {
        ...
        ogList.startTime = 1658707200;
        ogList.endTime = 1758707600;
        ogList.individualCap = 1;
        ogList.spotCap = 25;

        whiteList.startTime = 1658708200;
        whiteList.endTime = 1758709900;
        whiteList.individualCap = 1;
        whiteList.spotCap = 288;
    }
    ...
    whenNotPaused nonReentrant {
        require(getSigner(_whitelist) == designatedSigner, "Designated Signer mismatch");
        require(_whitelist.userAddress == msg.sender, "Invalid Signature");

        if(_whitelist.isOgList) {
            require(block.timestamp > ogList.startTime && block.timestamp <= ogList.endTime, "OG List Mint Over");
            require(_amount + ogList.spotsSold <= ogList.spotCap, "All spots sold");
            require(_amount + ogListMintTracker[_whitelist.userAddress] <= ogList.individualCap,
                "Individual Cap exceeding");
            ogListMintTracker[_whitelist.userAddress] += _amount;
            ogList.spotsSold += _amount;
        }

        else {
            require(block.timestamp > whiteList.startTime && block.timestamp <= whiteList.endTime, "Whitelist Mint Over");
            require(_amount + whiteList.spotsSold <= whiteList.spotCap, "All spots sold");
            require(_amount + whiteListMintTracker[_whitelist.userAddress] <= whiteList.individualCap,
                "Individual Cap exceeding");
            whiteListMintTracker[_whitelist.userAddress] += _amount;
            whiteList.spotsSold += _amount;
        }
    ...
    function setOwnerCap(uint _newCap) external onlyOwner {
        ownerCap = _newCap;
    }

    function setWhitelistStartTime(uint _newWhitelistStartTime) external onlyOwner {
        whiteList.startTime = _newWhitelistStartTime;
    }

    function setWhitelistEndTime(uint _newWhitelistEndTime) external onlyOwner {
        whiteList.endTime = _newWhitelistEndTime;
    }

    function setWhitelistMintIndividualCap(uint _newWhitelistIndividualCap) external onlyOwner {
        whiteList.individualCap = _newWhitelistIndividualCap;
    }

    function setWhitelistSpotCap(uint _newWhitelistSpotCap) external onlyOwner {
        whiteList.spotCap = _newWhitelistSpotCap;
    }

    function setOgListStartTime(uint _newOgListStartTime) external onlyOwner {
        ogList.startTime = _newOgListStartTime;
    }

    function setOgListEndTime(uint _newOgListEndTime) external onlyOwner {
        ogList.endTime = _newOgListEndTime;
    }

    function setOgListMintIndividualCap(uint _newOgListIndividualCap) external onlyOwner {
        ogList.individualCap = _newOgListIndividualCap;
    }

    function setOgListSpotCap(uint _newOgListSpotCap) external onlyOwner {
        ogList.spotCap = _newOgListSpotCap;
    }
