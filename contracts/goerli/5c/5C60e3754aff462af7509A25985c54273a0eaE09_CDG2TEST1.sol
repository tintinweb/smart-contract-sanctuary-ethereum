 ...
 *   CDGen2 Test 1
 *  
 *   SM:  See claimFreeNFTs() below for description of functionality desired.
 *   And THANK YOU for being awesome! :-)
 *
 *   BTW, you can see the current status of the affiliate code here as well. Note also that I'm also
 *   editing the PaymentSplitter source code to change release() to internal. Will explain that
 *   change to you sometime. 
 *
 *   Currently deployed to:
 *   https://goerli.etherscan.io/address/0x5C60e3754aff462af7509A25985c54273a0eaE09
 */
...
import "erc721a/contracts/ERC721A.sol";
        // In imported ERC721A, we have imported the OpenSea Operator Filter Registry (DefaultOperatorFilterer721)
import "@openzeppelin/contracts/finance/PaymentSplitter.sol"; 
        // In imported PaymentSplitter, release() is changed to internal.
import "@openzeppelin/contracts/utils/cryptography/MerkleProof.sol";

contract CDG2TEST1 is Ownable, ERC721A, PaymentSplitter {
    ...
    // add ERC721 Contract address for checking if nft holder. 
    address public ERC721_contract_id = 0x1bc7234A2E6bF2F487771977AC22aeeD482b9A79;
    // SM: If you want I can send you some NFTs and we can test using that.
        ...
        PaymentSplitter(distro, distro_shares)
    {
        ...
        accounts[teamclaim[0]] = Account( 20, 0, 1 ); // Team Use NFTs
        accounts[teamclaim[1]] = Account( 20, 0, 1 ); // tbd
    ...
    function claimFreeNFTs() external payable noReentrant {
        ...
        /** SUMMARY: I've taken the freeMint() function from months back and ported it this ERC721A model. We need to expand this though to ensure that people cannot use it multiple times. I see it working like this, but am of course open to any approach that accomplishes the goal. :-)

        * Person requests to claimFreeNFTs() because they own one or more NFTs from outside ECR721 contract.
        * We get the balanceOf, which becomes the _amount of the mint
        * We then get a list of token IDs owned within that ERC721 (say they own tokens #123, #1234, and #7777)
        * We then look at an array of already-claimed IDs in that set (for example, let's say they already claimed a free mint using #1234, and then they went back and bought the other two off secondary).
        * If any of the person's owned IDs have been claimed, we reduce the _amount by 1 for each one already claimed.
        * So here, for example, the person owns #1234, so we reduce _amount by 1, givien this person 2 free mints.
        * We execute the free mints
        * We then add #123 and #7777 to our list of claimed IDs.
        * We also need a getter to manually check any ID to see if it's been claimed. (This might help people on secondary to decide if they want to purchase an NFT from the OG collection. If they know they can get a free one here by buying the OG one, then they may well do that. This would also potentially help us in case we want to go look and see what's been claimed.)
        * I'm not sure gas-wise if the above is the optimal approach. Totally open to ideas/thoughts. :-)

        */

        require(saleIsLive || presaleIsLive, "Error: Sale is not active.");        
        require(!isContract(msg.sender), "Error: Contracts cannot mint.");

        // ERC721 instance = ERC721(ERC721_contract_id);
        // uint _amount = instance.balanceOf(msg.sender);
        // Will not currently compile using above lines.
        // Note: This contract is an ERC721A, but the one we will be looking back at is an ERC721

        uint _amount=0; revert();  // temp line -- delete this -- just here so it'll compile

        require(_amount>0);
        require(totalSupply() + _amount <= (maxSupply - reservedNFTs), "Error: Purchase would exceed max supply.");
        // Removed wallet limit check... we do not need a tx or wallet limit on this function.
 ...
 * ever updated, the "DefaultOperatorFilterer721" ay change to "DefaultOperatorFilterer".
 */
import {DefaultOperatorFilterer721} from 'operator-filter-registry/src/example/DefaultOperatorFilterer721.sol';
