...
contract Staking is Pausable, Ownable, ReentrancyGuard {
    ...
    function claimReward(address _to) external nonReentrant returns (bool) {
        ...
        require(
            stakeInfos[_to][stakeInfos[msg.sender].length - 1].endTS <
                block.timestamp,
            "Stake Time is not over yet"
        );
        require(
            stakeInfos[_to][stakeInfos[msg.sender].length - 1].claimed == false,
            "Already claimed"
        );
        ...
        if (
            ...
            stakeInfos[_to][stakeInfos[msg.sender].length - 1]
                .finalPrice = getLatestPrice();
            uint256 stakeAmount = stakeInfos[_to][
                stakeInfos[msg.sender].length - 1
            ].amount;

            uint256 totalTokens = stakeAmount + tokensToPay(_to);

            stakeInfos[_to][stakeInfos[msg.sender].length - 1]
                .finalAmount = totalTokens;
    ...
    function getTokenExpiry() external view returns (uint256) {
        ...
        return
            stakeInfos[_msgSender()][stakeInfos[msg.sender].length - 1].endTS;
    }

    // stakeAmount must be scale by a factor of 10^18, that is we are working with microTokensToStake
    function stakeToken(uint256 microTokensToStake, uint256 _stakingDays)
        external
        nonReentrant
    {
        require(microTokensToStake > 0, "Stake amount should be correct");
        require(
            _stakingDays >= MIN_STAKING_DAYS,
    ...
    function calculationOfTheAmountOfInterest(address _user)
        public
        view
        returns (uint256)
    {
        ...
        uint256 _stakeAmount = stakeInfos[_user][
            stakeInfos[msg.sender].length - 1
        ].amount;
        uint256 _interestRate = stakeInfos[_user][
            stakeInfos[msg.sender].length - 1
        ].interest;
        uint256 _stakingDays = stakeInfos[_user][
            stakeInfos[msg.sender].length - 1
        ].stakingDays;
    ...
    function tokenCalculationToCoverDepreciation(address _user)
        internal
        view
        returns (uint256)
    {
        ...
        uint256 _initialPrice = stakeInfos[_user][
            stakeInfos[msg.sender].length - 1
        ].initialPrice;
        uint256 _finalPrice = stakeInfos[_user][
            stakeInfos[msg.sender].length - 1
        ].finalPrice;
    ...
    function tokensToPay(address _user) internal view returns (uint256) {
        ...
        uint256 tokensNeededForDepreciation = tokenCalculationToCoverDepreciation(
                _user
            );
        ...
        if (_initialPrice >= _finalPrice) {
            if (tokensNeededForDepreciation > interestAmount) {
                max =
                    ...
                    ((tokensNeededForDepreciation *
                        stakeInfos[_user][stakeInfos[msg.sender].length - 1]
                            .fee) / 1000);
            } else {
                max =
                    interestAmount -
            ...
            max =
                interestAmount -
                ((interestAmount *
                    stakeInfos[_user][stakeInfos[msg.sender].length - 1].fee) /
                    1000);
        }
