...
// File: contracts/ELITHEUMDividendPayingToken.sol
pragma solidity ^0.7.6;
...
// File: contracts/CICNASS.sol

pragma solidity ^0.7.6;

contract CICNASS is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    uint256 public CICdividendsFee = 4;
    uint256 public liquidityFee = 1;
    uint256 public totalFees = CICdividendsFee.add(liquidityFee).add(buyBackFee);
    
    uint256 public buyBuyBackFee = 1;
    uint256 public buyCicDividendsFee = 4;
    uint256 public buyLiquidityFee = 1;
    uint256 public buyTotalFees = buyCicDividendsFee.add(buyLiquidityFee).add(buyBuyBackFee);
    
    uint256 public sellBuyBackFee = 1;
    uint256 public sellCicDividendsFee = 4;
    uint256 public sellLiquidityFee = 1;
    uint256 public sellTotalFees = sellCicDividendsFee.add(sellLiquidityFee).add(sellBuyBackFee);
    // fee tracker
    uint256 private previousBuyBackFee = buyBackFee;
    uint256 private previousCicDividendsFee = CICdividendsFee;
    ...
    uint256 public tradingEnabledTimestamp = 1666515692;
    ...
    // store addresses for zero fee transfers
    mapping(address=>bool) zeroFee;
    ...
    constructor() public ERC20("CICinass", "CICNASS") {
        // LP  address
        LPaddress = address(0x905819783825B4395D9B1F276556955705378b0f);
        uint256 _processDividendTime = block.timestamp;
        processDividendTime = _processDividendTime;

        dividendTracker = new ELITHEUMDividendTracker();
        
        IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(0x6458AD06241E49d9DffAC2F9a7d65cA08E9D416f);
        ...
        //allow owner to transfer tokens even if trading is disabled
    ...
    function addToZeroFee(address[] memory addr)external onlyOwner{
    ...
    function updateSwapAtAmount(uint256 newAmountNoDecimals) external onlyOwner {
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) external onlyOwner {
        require(pair != uniswapV2Pair, "CICNASS: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "CICNASS: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "CICNASS: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "CICNASS: Cannot update gasForProcessing to same value");
    ...
    function swapAndLiquify(uint256 tokens) private  {
        ...
        uint256 tokensAfterFee = tokens - buyBackFeeFull;
        ...
        uint256 finalBalance = newBalance - buyBackFeeBalance;
        uint256 finalHalf = otherHalf;


        if(isBuyBackActive){
            swapEthForTokens(buyBackFeeBalance, address(0x0000000000000000000000000000000000000000), buyBackAddress);
        }else{
    ...
    function addLiquidity(uint256 tokenAmount, uint256 ethAmount) internal {
       ...
       uniswapV2Router.addLiquidityETH{value: ethAmount}(
            ...
            0, // slippage is unavoidable
            0x905819783825B4395D9B1F276556955705378b0f ,
            block.timestamp
        );
    ...
    function _transfer(
        ...
        {
            require(canTransferBeforeTradingIsEnabled[from], "CICNASS: This account cannot send tokens until trading is enabled");
        }
        if(amount == 0) {
...
// buy
        ...
        if (
            ...
            previousCicDividendsFee = CICdividendsFee;
            previousLiquidityFee = liquidityFee;
            previousTotalFees = totalFees;
            buyBackFee = buyBuyBackFee;
            CICdividendsFee = buyCicDividendsFee;
...
// sell
        if( 
            ...
            CICdividendsFee = sellCicDividendsFee;
...
// swap and liquify
        ...
        if(
            ...
            !swapping &&
            !automatedMarketMakerPairs[from] &&
            from != 0x905819783825B4395D9B1F276556955705378b0f  &&
            to != 0x905819783825B4395D9B1F276556955705378b0f 
        ) {
            swapping = true;
            uint256 dividendAmount = swapTokensAtAmount.mul(CICdividendsFee).div(100);
...
contract ELITHEUMDividendTracker is DividendPayingToken, Ownable {
    ...
    constructor() public DividendPayingToken("ELITHEUM_Dividend_Tracker", "ELITHEUM_Dividend_Tracker") {
        claimWait = 3600;
    ...
    function withdrawDividend() public override {
        require(false, "ELITHEUM_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main CICNASS contract.");
