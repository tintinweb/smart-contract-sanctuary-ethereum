...
//    bytes32 public constant SNAPSHOT_ROLE = keccak256("SNAPSHOT_ROLE");
    ...
    function initialize(BaseData memory __baseData, BondData memory __bondData)
        public
        initializer
    {
        ...
        __Snapshot_init_unchained(__bondData.start_date);
        __UUPSUpgradeable_init();

        _grantRole(DEFAULT_ADMIN_ROLE, msg.sender);
        _grantRole(SNAPSHOOTER_ROLE, msg.sender);
    ...
    function addPostInitialOfferingData(BondData memory __bondData)
        public
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        _storeBondData(__bondData);
    }

    function assignMinterRole(address __initialOffering)
        public
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
    ...
    /**
     * @dev Kill will destroy contract
     *
     */
    ...
    function decimals()
        ...
        view
        virtual
        override(ERC20Upgradeable, DataStorageModule)
        returns (uint8)
    {
...
//        override(ERC20Upgradeable, ERC20SnapshotUpgradeable)
        override(ERC20Upgradeable, SnapshotModule)
        whenNotPaused
    {
        require(!paused(), "RapidBondz: token transfer attempt while paused");
        require(
            !frozen(from),
            "RapidBondz: token transfer attempt while frozen"
        );
        ...
        if (address(ruleEngine) != address(0)) {
            require(
                ...
                "Rapid Bondz: transfer rejected by validation module"
            );
        }
    }
...
abstract contract DataStorageModule is Initializable, ERC20Upgradeable {
    ...
    struct BondData {
        ...
        uint256 start_date;
        uint256 par_value;
        uint8 coupon_frequency;
        uint256[] coupon_payments;

    }

    CouponFrequency public _coupon_frequency;
    ...
    event SetAdditionalData(
        uint256 _maturity_date,
        uint256 _interest_rate,
        uint256 _start_date,
        uint256 _par_value,
        uint8 _coupon_frequency
    );

    function get_coupon_frequency() public view returns (CouponFrequency) {
        return _coupon_frequency;
    }

    function set_coupon_frequency(uint8 _coupon_frequency_) internal {
        _coupon_frequency = CouponFrequency(_coupon_frequency_);
    }

    function _storeBaseData(BaseData memory baseData_) internal {
        baseData = baseData_;
        emit SetBaseData(
            baseData_.name,
            baseData_.symbol,
    ...
    function _storeBondData(BondData memory bondData_) internal {
        bondData = bondData_;
        emit SetAdditionalData(
            bondData_.maturity_date,
            bondData_.interest_rate,
            bondData_.start_date,
            bondData_.par_value,
            bondData_.coupon_frequency
        );
        require(
            bondData_.maturity_date > bondData_.start_date,
            "maturity date has to be greater than start date"
        );
        require(
            block.timestamp - 1 days < bondData_.start_date,
            "start date has to be today or later"
        );
        require(
            bondData_.coupon_frequency < 5,
            "coupon_frequency code has to be int fom 0 to 4"
        );
        set_coupon_frequency(bondData_.coupon_frequency);
    }


    function __DataStorage_init() internal onlyInitializing {}

    function __DataStorage_init_unchained(BaseData memory baseData__, BondData memory bondData__)
    internal
    onlyInitializing
    {
    ...
    function _findCouponPaymentsIndex(uint256 time) public view returns (uint256) {
        return bondData.coupon_payments.findUpperBound(time);
    }

    function _findNextCouponPayments() public view returns (uint256) {
        uint256 nextPaymentIndex = bondData.coupon_payments.findUpperBound(block.timestamp);
        return _findCouponPaymentsAtIndex(nextPaymentIndex);
    }

    function _findCouponPaymentsBefore(uint256 time) public view returns (uint256) {
        uint256 nextPaymentIndex = bondData.coupon_payments.findUpperBound(time);
        if (nextPaymentIndex - 1 >= 0) {
            return _findCouponPaymentsAtIndex(nextPaymentIndex - 1);
        } else {
            return time;
        }
    }

    function _findCouponPaymentsAtIndex(uint256 pos) public view returns (uint256) {
        return bondData.coupon_payments.unsafeAccess(pos).value;
    }
...
interface IRuleEngine is IERC1404Wrapper{
    ...
    */
    function setRules(IRule[] memory rules_) external;
    ...
    ERC20Upgradeable
{
    ...
    function __Snapshot_init(uint256 _time) internal initializer {
        __Context_init_unchained();
        __Snapshot_init_unchained(_time);
    }

    function __Snapshot_init_unchained(uint256 _time)
        internal
        onlyInitializing
    {
        _scheduleSnapshot(_time);
    }
    ...
    function _findScheduledSnapshotIndex(uint256 time)
        ...
        returns (bool, uint256)
    {
//        uint _scheduledSnapshotsLength = _scheduledSnapshots.length;
        for (uint256 i = 0; i < _scheduledSnapshots.length; ) {
