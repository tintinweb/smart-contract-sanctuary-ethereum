...
contract MosaicSquareMoneypipe {
  ...
  modifier initializer() {
    require(!_initializing, "MosaicSquareMoneypipe: contract is not initializing");
    _;
  }
...
****************************************************************************************************/
  function initialize(Member[] calldata memberData) initializer external {
    require(memberData.length > 0, "MosaicSquareMoneypipe: must have at least one member");
    
    for(uint16 i = 0; i < memberData.length; i++) {
      require(memberData[i].account != address(0), "MosaicSquareMoneypipe: invalid address");
      require(memberData[i].value > 0, "MosaicSquareMoneypipe: value must be greater than zero");
...
****************************************************************************************************/
  receive () external payable {
    require(_members.length > 0, "MosaicSquareMoneypipe: contract is not initialized");
...
****************************************************************************************************/
  function withdrawFor(address payable user) external nonReentrant {
    ...
    require(amount != 0, "MosaicSquareMoneypipe: No Funds Available"); 
    
    _pendingWithdrawals[user] = 0;
    (bool success, ) = user.call{ value: amount, gas: SEND_VALUE_GAS_LIMIT_SINGLE_RECIPIENT }("");
    if (!success) {
      revert("MosaicSquareMoneypipe: withdrawFor failed");
    }
  }
