...
contract Core is OwnableUpgradeable, ICore {
  ...
  /**
    ...
    function createCondition(
        ...
        emit ConditionCreated(_oracleCondId, _scopeId, lastConditionId, _timestamp, _ipfsHash);
    }
     ...
     */
    function shiftByMaintainer(uint256 conditionId, uint256 _newTimestamp)
        external
        onlyMaintainer
    {
    ...
    function shiftByOracle(uint256 _oracleCondId, uint256 _newTimestamp)
        external
        onlyOracle
    {
        uint256 conditionId = oracleConditionIds[_oracleCondId];
    ...
    function _shift(uint256 _oracleCondId, uint256 _conditionId, uint256 _newTimestamp)
        internal
    {
        if (conditions[_conditionId].timestamp == 0) revert ConditionNotExists();
