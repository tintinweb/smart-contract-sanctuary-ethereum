...
contract SimpleBatchTransfer {
     ...
     * @param _token The input token
     * @param _amount The input token
     * @param _dstChainId The destination chain id
     * @param _maxSlippage The max amount of slippage allowed at bridge, represented in 1e6 as 100%(i.e. 1e4=1%)
     * @param _bridgeType The bridge type
     * @param _accounts The accounts on the destination chain that should receive the transfered fund
     * @param _amount The amounts for each account
     */
    function batchTransfer(
        ...
        // step1: each transfer is assigned a nonce
        nonce += 1;

        // step2: pull funds from the sender
        IERC20(_token).safeTransferFrom(msg.sender, address(this), _amount);

        // step3: encode a message, specifying how we want to distribute the funds on the destination chain
        bytes memory message = abi.encode(
            TransferRequest({accounts: _accounts, amounts: _amounts})
        );

        // step4: send it. MessageSenderLib is your swiss army knife for sending message
