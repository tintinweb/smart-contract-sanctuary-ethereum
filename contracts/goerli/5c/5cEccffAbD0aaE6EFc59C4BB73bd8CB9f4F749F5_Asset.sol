...
abstract contract Mintable is Ownable, IMintable {
    ...
    bool public Control_Whitelist;

    uint256 public Total;
    uint256 public MaxTotal = 999;
    ...
    function AddWhitelist ( address adr) public {
        require(Control_Whitelist==true);
        Whitelist[adr]=true;
    }

    function delete_address ( address adr) public onlyOwner {
        Whitelist[adr]=false;
    }

    function Set_Control_Whitelist () public onlyOwner {
        if (Control_Whitelist == false) {
            Control_Whitelist=true;
        }
        if (Control_Whitelist == true) {
            Control_Whitelist=false;
        }
    }
