...
contract Game369 is 
    ...
    // ==== Variables declaration ====
        ...
        mapping(address => WinnerList[]) public Winners; // mapping of each winner to their prizes
        
        struct WinnerList { // record of each selected winner and their prize 
            address nft; 
    ...
    function pickWinners() public onlyOwnerOrAutomator whenNotPaused {
        ...
        for(uint256 i = 0; i < numberOfWinners; i++) {
            ...
            if (prizeMode == 0){
                ...
                prize = WinnerList(address(rewardNFTs[nftIndex]), rewardNFTIds[nftIndex], false, gameRound);
                Winners[winner].push(prize); 
                ...
                prize = WinnerList(address(0), rewardETHPerWinner, false, gameRound); // NFT address is 0x0 for ETH prize
                Winners[winner].push(prize);
            }
    ...
    function claimPrizes(address winner) external {
        require(Winners[winner].length > 0, "Not part of the winner list");
        for(uint256 i = 0; i < Winners[winner].length; i++) {
            // Only allow claiming of prizes from the current round
            if (!Winners[winner][i].claimed) {
                if (Winners[winner][i].nft == address(0)) {
                    // ETH prize
                    require(address(this).balance >= Winners[winner][i].prizeAmountOrId, "Contract doesn't have enough ETH");
                    payable(winner).transfer(Winners[winner][i].prizeAmountOrId); // transfer ETH to winner
                } else {
                    // NFT prize
                    IERC721 rewardNFT = IERC721(Winners[winner][i].nft);
                    uint256 rewardNFTId = Winners[winner][i].prizeAmountOrId;
                    require(rewardNFT.ownerOf(rewardNFTId) == address(this), "Contract doesn't own the NFT");
                    rewardNFT.transferFrom(address(this), winner, rewardNFTId); // transfer NFT to winner
                }
                Winners[winner][i].claimed = true;
            }
        }
    }
    
    function isWinnerWithUnclaimedPrizes(address winner) external view returns (bool) {
    if (Winners[winner].length > 0) {
        for(uint256 i = 0; i < Winners[winner].length; i++) {
            if (!Winners[winner][i].claimed) {
                return true;
            }
