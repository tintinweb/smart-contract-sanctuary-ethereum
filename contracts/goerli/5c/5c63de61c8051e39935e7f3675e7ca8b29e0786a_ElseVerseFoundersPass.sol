...
contract ElseVerseFoundersPass is Initializable, ERC721Upgradeable, ERC721BurnableUpgradeable, OwnableUpgradeable  {
    ...
    function safeBuy(bytes32[] calldata _merkleProof, uint256 mintCount, uint256 maxMintCount) public payable {
		...
		if (currStage.isPublic) {	
			...
			require(whiteListClaimedCount[msg.sender] + mintCount <= maxMintCount, "Address has already claimed max amount of founders pass");
        ...
        transfer(beneficiaryAddress, currStage.currency, currStage.price);
    ...
    function addOrUpdateStage(uint256 id, uint256 startDate, uint256 price, bytes32 merkleRoot, bool isPublic, uint256 publicMaxMintsPerWallet, address currency) external onlyOwner  {
        stages[id] = Stage(
			id,
			startDate,
			price,
