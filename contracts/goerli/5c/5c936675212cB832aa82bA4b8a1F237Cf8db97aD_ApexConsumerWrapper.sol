...
contract ApexConsumerWrapper is ChainlinkClient, Ownable, Pausable {
    ...
    function requestMetaData(string memory _jobId, string memory sport) public whenNotPaused isValidMetaDataRequest(sport) {
    ...
    function fulfillMatchup(
        ...
        consumer.fulfillMatchup(_requestId, _betTypeDetail, _probA, _probB, _timestamp, sport);
    }
    ...
    /// @notice setting payment for metadata request
    /// @param _paymentMetadata amount of LINK per request for metadata
    function setPaymentCreate(uint _paymentMetadata) external onlyOwner {
        require(_paymentMetadata > 0, "Can not be zero");
        paymentMetadata = _paymentMetadata;
        emit NewPaymentAmountMetadata(_paymentMetadata);
    }

    /// @notice setting payment for mathcup request
    /// @param _paymentMatchup amount of LINK per request for mathcup
    function setPaymentMatchup(uint _paymentMatchup) external onlyOwner {
        require(_paymentMatchup > 0, "Can not be zero");
        paymentMatchup = _paymentMatchup;
        emit NewPaymentAmountMatchup(_paymentMatchup);
    }

    /// @notice setting payment for results request
    /// @param _paymentResults amount of LINK per request for reuslatss
    function setPaymentOdds(uint _paymentResults) external onlyOwner {
        require(_paymentResults > 0, "Can not be zero");
        paymentResults = _paymentResults;
        emit NewPaymentAmountResults(_paymentResults);
    }
    ...
    event NewPaymentAmountMetadata(uint _paymentMetadata);
    event NewPaymentAmountMatchup(uint _paymentMatchup);
    event NewPaymentAmountResults(uint _paymentResults);
