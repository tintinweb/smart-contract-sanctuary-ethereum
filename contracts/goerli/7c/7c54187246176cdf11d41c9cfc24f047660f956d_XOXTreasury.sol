...
contract XOXTreasury is Initializable {
    ...
    modifier onlyRouter() {
        require(
            msg.sender == router,
            "XOX Treasury: Only XOX Roter can call this function"
        );
        _;
    }
    ...
    modifier onlyAdmin() {
        require(
            msg.sender == admin,
            "XOX Treasury: Only XOX admin can call this function"
        );
        _;
    }

    function setXOXRouter(address router_) external onlyAdmin {
        router = router_;
    }

    function setApy(uint256 apy_) external onlyAdmin {
        require(apy_ < 100, "XOX Treasury: Apy must be less than 100");
        apy = apy_;
    ...
    function pendingReward(address _user) public view returns (uint256) {
        ...
        uint256 blockDelta = block.number.sub(user.lastBLockUpdated);
        uint256 totalComp = user.amount.add(user.reward);
        return
            totalComp.mul(blockDelta).mul(apy).div(BASE_APY).div(
                TOTAL_BLOCK_IN_YEAR
            );
    }
    ...
    function swapUSDtoXOX(
        ...
        if (buyer.amount == 0) {
            buyer.lastBLockUpdated = block.number;
            buyer.amount = amount.div(10);
        }
        if (block.number > buyer.lastBLockUpdated) {
            uint256 rewardPending = pendingReward(from);
            buyer.amount = buyer.amount.add(amount.div(10));
            buyer.reward = buyer.reward.add(rewardPending);
            buyer.lastBLockUpdated = block.number;
        }
    ...
    function claimFarmingReward(uint256 _amount) external {
        ...
        uint256 rewardPending = pendingReward(msg.sender);
        require(
            user.reward + rewardPending >= _amount,
        ...
        safeUsdTransfer(msg.sender, _amount);

        emit ClaimFarmingReward(msg.sender, _amount);
    }
    ...
    function claimReferralByLevel(uint256 level) external {
        ...
        // claim
        safeUsdTransfer(msg.sender, rewardByLevel[level]);
        // update
        user.historyClaim.push(level);
        // log event
        emit ClaimReferral(msg.sender, rewardByLevel[level]);
    }
    ...
    function claimReferralAll() external {
        ...
        for (uint256 i; i < levelLength; i++) {
            if (user.point >= levelReward[i]) {
                ...
                if (rewardByLevel[i] > 0) {
                    emit ClaimReferral(msg.sender, rewardByLevel[i]);
                }
                // update claimed
        ...
        // claim
        safeUsdTransfer(msg.sender, rewardPending);
    }
    ...
    function pendingRewardByLevel(address user_, uint256 level_)
        public
        view
        returns (uint256)
    {
    ...
    function checkIsClaimLevel(UserInfo storage user, uint256 level)
        internal
        view
        returns (bool)
    {
