...
contract HelloWorld is ERC721 {
   ...
   event NewHospital(uint _hospId, string _hospName,address _hospAddress);

   
   function createAdmin(uint256 _id, address _address) public {
      admins.push(Admin(_id,_address));
      owners[_id] = _address;
   }
   ...
   function createHospital(uint256 _hospId, string memory _hospName, address _hospAddress) public {
      hospitals.push(Hospital(_hospId,_hospName,_hospAddress));
      owners[_hospId] = _hospAddress;
      //emit NewPatient(_id,_name,_age,_cc,_phone,_allergies,_medication,_hospitalNamePatients,_address);
   ...
   function showPatient(uint256 _idpaciente) public returns(string memory){
      ...
      return patients[_idpaciente].name;
      return patients[_idpaciente].Allergies;
   }
}
