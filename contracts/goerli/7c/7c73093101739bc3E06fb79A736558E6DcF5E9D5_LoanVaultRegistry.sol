...
contract LoanVault is ERC4626Upgradeable, OwnableUpgradeable, PausableUpgradeable {
    ...
    function addFundingRequest(uint256 amountFLR) external onlyBorrower returns (uint256 fundingRequestId) {
        require(amountFLR > 0, "Zero amount not supported");
        fundingRequestId = fundingRequests.length;
    ...
    function fund(IERC20Upgradeable fundingToken, uint256 fundingTokenAmount) external {
        (uint256 nextOpenFundingRequestId, 
        uint256 flrFundingAmount, 
        uint256 shares, 
        uint256 cappedFundingTokenAmount) = _previewFund(_msgSender(), fundingToken, fundingTokenAmount);
    ...
    function interestPayment(uint256 interestAmount) external {
        florinTreasury.addEUR(_msgSender(), interestAmount);
        payPool(interestAmount);
    }

    function paybackLoan(uint256 amountEUR) external {
        florinTreasury.addEUR(_msgSender(), amountEUR);
...
contract FlorinToken is ERC20BurnableUpgradeable, OwnableUpgradeable {
    function initialize() external initializer {
    ...
    function mint(address _receiver, uint256 _amount) public onlyOwner {
        require(_amount > 0, "FlorinToken: mint invalidAmount");
...
contract FlorinTreasury is AccessControlUpgradeable, PausableUpgradeable {
    ...
    function initialize(IERC20Upgradeable eurToken_) external initializer {
        __Pausable_init_unchained();
        __AccessControl_init_unchained();

        _grantRole(DEFAULT_ADMIN_ROLE, _msgSender());

        florinToken = new FlorinToken();
        florinToken.initialize();

        eurToken = eurToken_;
    }

    function mint(address receiver, uint256 amount) public {
        require(hasRole(LOAN_VAULT_ROLE, _msgSender()), "Only LoanVault can mint.");
    ...
    function addEUR(address from, uint256 amountEUR) public {
