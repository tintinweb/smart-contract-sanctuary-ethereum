...
contract ClientMarketAdapter is ModuleAccessController, IClientMarketAdapter {
    ...
    function protect(
        ...
        uint incentivesAmount = incentives.coordinationForTaker(
            address(asset),
            amount
        );
    ...
    function protectNative(
        ...
        require(amount > minTakerPositionSize, "MB: taker-minsize");
    ...
    function close(
        ...
        // get position size and locked BUMP token amount for this position
        (, uint bumpAmount) = market.getTakerPositionSize(id);
        ...
        // close position
        (uint premium, uint fee, Payout memory payout) = market.close(
            msg.sender, id, unwrap, address(stable)
            );

        if (payout.stableAmount == 0 && payout.assetAmount == 0) {
            // notify frontend side that position premium reached position size
            emit PremiumTooHigh(msg.sender, id);
        }
        ...
        onlyTakerPositionOwner(id)
    {
        ...
        // claim position
        (uint floor, uint fee, Payout memory payout) = market.claim(
            msg.sender, id, address(stable)
            );
    ...
    /// @inheritdoc ITakerPosition
    function cancel(uint id, bool unwrap)
        external
        virtual
        override
        onlyTakerPositionOwner(id)
    {
        ...
        // close position
        (uint premium, uint fee, Payout memory payout) = market.cancel(msg.sender, id, unwrap, address(stable));
    ...
    function deposit(
        ...
        uint incentivesAmount = incentives.coordinationForMaker(
            address(asset),
            bstAmount
        );
        ...
        require(bstAmount > minMakerPositionSize, "CMA: maker-minsize");
        ...
        onlyMakerPositionOwner(id)
    {
        ...
        (, uint bumpAmount) = market.getMakerPositionSize(id);
        ...
        // close position
        (int yield, Payout memory payout) = market.withdraw(
            msg.sender,
            id,
            address(stable)
        );
        require(
            payout.assetAmount > 0 || payout.stableAmount > 0,
            "CMA: liquidate-only"
        );
        ...
        onlyMakerPositionOwner(id)
    {
        ...
        // abandon position
        (int yield, Payout memory payout) = market.abandon(
            msg.sender,
            id,
            address(stable)
        );
        ...
        onlyTakerPositionOwner(id)
    {
        ...
        if (flags.getAutorenew()) {
            ...
            uint8 terms = uint8(
                (block.timestamp - start) / (term * 1 days) + 1
            );
    ...
    function ejectR(uint id) external override {
        ...
        market.withdraw(pos.owner, id, address(stable));

        emit LiquidateMakerPosition(pos.owner, id);
    }
