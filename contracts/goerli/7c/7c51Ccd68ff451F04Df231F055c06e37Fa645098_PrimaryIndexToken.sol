...
contract PrimaryIndexToken is Initializable, AccessControlUpgradeable, ReentrancyGuardUpgradeable
{
    ...
    mapping(address => uint256) totalBorrowPerLendingToken; //lending token address => total borrow by lending token [] - irrespective of the collateral assets used
    ...
    function setDepositAmount(address projectToken, address user, uint amount) public onlyAdmin {
        depositPosition[user][projectToken][usdcToken].depositedProjectTokenAmount = amount;
    }
    ...
    function borrow(address projectToken, address lendingToken, uint256 lendingTokenAmount) public isProjectTokenListed(projectToken) isLendingTokenListed(lendingToken) {
        ...
        require(getTokenEvaluation(lendingToken, lendingTokenAmount) <= pitRemaining(msg.sender, projectToken, lendingToken), "PIT: lendingTokenAmount exceeds pitRemaining");
    ...
    function pitOfLendingAsset(address account, address projectToken, address lendingToken) public view returns (uint256) {
        return getTokenEvaluation(lendingToken, totalOutstanding(account, projectToken, lendingToken));
    }
    ...
    function pitRemaining(address account, address projectToken, address lendingToken) public view returns (uint256) {
        ...
        uint256 _totalOutstandingInUSD = pitOfLendingAsset(account, projectToken, lendingToken);
    ...
    function getTotalBorrowPerCollateral(address projectToken) public view returns (uint) {
        ...
        for(uint i = 0; i < lendingTokensLength(); i++) {
            if(lendingTokens[i] == usdcToken) {
                totalBorrowInUSD += totalBorrow[projectToken][usdcToken];
            }
            totalBorrowInUSD += getTokenEvaluation(lendingTokens[i], totalBorrow[projectToken][lendingTokens[i]]);
        }
        return totalBorrowInUSD;
    }

    function getTotalBorrowPerLendingToken(address lendingToken) public view returns (uint) {
        return getTokenEvaluation(lendingToken, totalBorrowPerLendingToken[lendingToken]);
    }
}
