...
contract NFTStaking is Ownable, IERC721Receiver, Pausable {
  ...
  function stake(uint256 _pid, uint256[] calldata tokenIds) external {
    ...
    _pid == 0 ? totalAlienStaked += tokenIds.length : totalSpaceSoilderStaked += tokenIds.length;
  ...
  function _unstakeMany(address account, uint256[] calldata tokenIds, uint256 _pid) internal {
    ...
    for (uint i = 0; i < tokenIds.length; i++) {
      ...
      reward += 100 ether * (block.timestamp - stakedAt) / 1 days;
      earned += reward / 100;
      vault[tokenId] = Stake({
        owner: account,
        tokenId: uint24(tokenId),
  ...
  else if(_unstake && _pid == 1){
    _unstakeMany(account, tokenIds, _pid);
  }
  else if(!_unstake && _pid == 0 && earned > 0) {
  ...
  else if(!_unstake && _pid == 1 && earned > 0) {
      vaultid.token.mint(account, earned);
  }
