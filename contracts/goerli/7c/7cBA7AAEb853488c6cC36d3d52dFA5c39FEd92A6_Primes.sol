...
contract Primes is ERC721A, ERC2981, Ownable {
    ...
    constructor() ERC721A("Primes", "PRIMES") {
        setPathMap();
    }
    ...
    function tokenURI(
        ...
        string
            memory description = "One of 1575 unique artworks from the on-chain scalable, animated and controllable Primes collection";
        string memory serialStr;
        string memory dataImage;
        string memory serialIsPrime;
        uint primesCount = 0;
        uint256 serialIndex = _tokenId;
        serialStr = string(
        ...
        for (y = 0; y <= 800; y += 100) {
            for (x = 0; x <= 800; x += 100) {
                ...
                if (PrimesUtils.isPrime(uint256(count))) {
                    string memory d = getPath(offset);
                    string memory values = getValues(offset);
    ...
    // from https://chiru-labs.github.io/ERC721A
    // ERC2981 functions
...
library PrimesUtils {
    ...
    function randomOI(
        ...
        // return uint256(keccak256(abi.encodePacked(msg.sender, _seed, _salt))) % 2 == 1;
        return uint256(keccak256(abi.encodePacked(msg.sender, _seed, _salt))) % 2 == 1;
    }
