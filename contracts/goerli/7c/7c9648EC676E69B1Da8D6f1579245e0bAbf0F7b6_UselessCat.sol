...
contract UselessCat {
  ...
  IReverseResolver constant resolver =
   ...
   function openTheBox() public {
      ...
      string memory openerAddress = Strings.toHexString(uint256(uint160(msg.sender)), 20);
      string memory openerENS = getENSName(msg.sender);
      string memory openStr = string.concat(openerENS, openerAddress,
                                            " opened the box said \"Oh great ",
                                            catName,
   ...
   function closeTheBox() private {
    ...
    function name(string memory newCatName, string memory newCatWisdom) public {
        catName = newCatName;
        catWisdom = newCatWisdom;
        lastNamedBy = getENSName(msg.sender);
