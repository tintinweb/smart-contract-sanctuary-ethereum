...
contract VRFLens is Ownable {
    ...
    function getTasksBulk(uint64[] calldata nonces) external view returns(IVRFProviderViewOnly.Task[] memory) {
        uint256 len = nonces.length;
        IVRFProviderViewOnly.Task[] memory tasks = new IVRFProviderViewOnly.Task[](len);

        for (uint256 i = 0; i < len; i++) {
            (
                bool a, uint64 b, address c, uint256 d, bytes32 e, bytes32 f, string memory g
            ) = provider.tasks(nonces[i]);
            tasks[i] = IVRFProviderViewOnly.Task(a,b,c,d,e,f,g);
        }
        return tasks;
    }
