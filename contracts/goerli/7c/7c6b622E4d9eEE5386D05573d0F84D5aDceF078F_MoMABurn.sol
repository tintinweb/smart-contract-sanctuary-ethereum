...
contract MoMABurn is Ownable {
    ...
    ERC721 public burnContract; // Address of token need to burn
    FeralfileExhibitionV3_3 public tier1Contract; // Address of new token for Tier 1
    FeralfileExhibitionV3_3 public tier2Contract; // Address of new token for Tier 2
    FeralfileExhibitionV3_3 public tier3Contract; // Address of new token for Tier 3
    ...
    /// @notice setBurnContract use for update burn contract address
    /// @param _contract - new burn contract address
    function setBurnContract(address _contract) public onlyOwner {
        require(_contract != address(0), "Invalid contract address");
        burnContract = ERC721(_contract);
    }

    /// @notice setMintContracts use for update mint contract addresses
    ...
    function setMintContracts(
        ...
        tier1Contract = FeralfileExhibitionV3_3(_tier1Address);
        tier2Contract = FeralfileExhibitionV3_3(_tier2Address);
        tier3Contract = FeralfileExhibitionV3_3(_tier3Address);
    }

    /// @notice setMintArtworkIDs use for update mint artwork ID
    ...
    function burnAndMint(uint256[] memory _burnedEditions) public {
        ...
        bool isApprovalForAll = burnContract.isApprovedForAll(
            _msgSender(),
            address(this)
        );
        ...
        for (uint256 i = 0; i < _burnedEditions.length; i++) {
            ...
            require(
                burnContract.ownerOf(_burnedEditions[i]) == _msgSender(),
                "Invalid owner of burned token"
            );

            if (!isApprovalForAll) {
                require(
                    burnContract.getApproved(_burnedEditions[i]) ==
                        address(this),
        ...
        FeralfileExhibitionV3_3 mintContract = tier1Contract;
        uint256 artworkID = tier1ArtworkID;

        if (_burnedEditions.length == 6) {
            mintContract = tier2Contract;
            artworkID = tier2ArtworkID;
        }

        if (_burnedEditions.length == 9) {
            mintContract = tier3Contract;
            artworkID = tier3ArtworkID;
        }

        mintContract.mintArtworkEdition(artworkID, _msgSender());
    }
}
