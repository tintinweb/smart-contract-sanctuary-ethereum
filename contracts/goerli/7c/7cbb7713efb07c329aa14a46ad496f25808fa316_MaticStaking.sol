    ...
    OwnableUpgradeable
{
    ...
    function stake(
        ...
        emit Staked(
            receiver,
            IERC20Upgradeable(_matic).balanceOf(address(this)),
            isRebasing
        );
    }
    ...
    function unstakeAcrossToPolygon(uint256 amount) external onlyOperator {
        _depositManager.depositERC20ForUser(_matic, address(this), amount); //TODO Change receiver to OPERATOR
    }
    ...
    function changeToChainId(uint256 toChain) external onlyOwner {
        _toChainId = toChain;
        emit ToChainIdChanged(toChain);
    }

    // function delegate() external override {
    //     require(_requests.length > 0, "have not got any requests");
    //     Request memory request = _requests[_requests.length - 1];
    //     _rootChainManager.processExits(_matic);
    //     // delegate via StakeFi PolygonPool
    //     address token;
    //     uint256 amount;
    ...
    //     if (request.isRebasing) {
    //         token = _bondToken;
    //         balance = IERC20Upgradeable(token).balanceOf(address(this));
    //         _pool.stakeAndClaimBonds(request.amount);
    //         amount =
    ...
    //         token = _certToken;
    //         balance = IERC20Upgradeable(token).balanceOf(address(this));
    //         amount = (request.amount * IBondToken(_bondToken).ratio()) / 1e18;
    //         _pool.stakeAndClaimCerts(request.amount);
    //     }
    //     amount = IERC20Upgradeable(token).balanceOf(address(this)) - balance;
    //     _requests.pop();
    //     // transfer tokens across the bridge
    //     // _bridge.deposit(token, 80001, request.account, amount);
    //     // emit Delegated(
    //     //     request.account,
    //     //     IERC20Upgradeable(token).balanceOf(address(this)),
    //     //     request.isRebasing
...
interface IMaticStaking {
    ...
    event StartExit(address indexed receiver, uint256 amount, bool isRebasing);
    ...
    event Staked(address indexed staker, uint256 amount, bool isRebasing);

    event BondTokenChanged(address bondToken);

    event CertTokenChanged(address certToken);

    event PoolChanged(address pool);

    event BridgeChanged(address bridge);

    event DepositManagerChanged(address depositManager);
    ...
    event ToChainIdChanged(uint256 toChain);
