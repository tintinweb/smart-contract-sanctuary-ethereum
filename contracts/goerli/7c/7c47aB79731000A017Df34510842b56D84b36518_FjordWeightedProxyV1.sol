...
struct PoolData {
    bool isCorrectOrder; // todo, do we need to store in the contract anything other than the pool address?
}
...
/// @title FjordWeightedProxyV1
/// @notice TODO
contract FjordWeightedProxyV1 is Ownable {
    ...
    constructor(address _WeightedPoolFactoryAddress, address _vaultAddress) {
     ...
     * @dev TODO
     */
    function createWeightedPool(PoolConfig memory poolConfig) external returns (address) {
        // 1: deposit tokens and approve vault
        require(poolConfig.tokens.length == 2, "Weighted pool must have exactly two tokens");
        require(poolConfig.tokens[0] != poolConfig.tokens[1], "Tokens must be unique");
        ...
        // 3: store pool data
        // _poolData[pool] = PoolData(msg.sender, poolConfig.isCorrectOrder);
        //require(_pools.add(pool), "exists already");

        bytes memory userData = abi.encode(0, poolConfig.amounts); // JOIN_KIND_INIT = 0 // TODO check balancer!
        ...
        uint256 weightedPoolTokenBalance = IERC20(pool).balanceOf(address(this));
        // transfer Pool tokens to creator
        TransferHelper.safeTransferFrom(pool, address(this), msg.sender, weightedPoolTokenBalance);
        return pool;
    }
