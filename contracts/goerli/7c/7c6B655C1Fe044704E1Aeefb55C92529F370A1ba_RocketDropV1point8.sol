...
contract RocketDropV1point8 is Ownable {
    ...
    struct PoolInfo {
        ...
        uint32 lastRewardBlock; // Last block number that ERC20s distribution occurs.
        uint256 accERC20PerShare; // Accumulated ERC20s per share, times 1e36.
        IERC20 rewardToken; // pool specific reward token.
        uint32 startBlock; // pool specific block number when rewards start
        uint32 endBlock; // pool specific block number when rewards end
    ...
    struct PoolExtras {
        uint16 totalStakers;
        uint16 maxStakers;
        uint8 stakeTokenFee; // divide by 1000 ie 150 is 1.5%
        uint32 lockPeriod; // time in blocks needed before withdrawal
        IERC20 accessToken;
    ...
    function rewardPerBlock(uint256 index) external view returns (uint256) {
    ...
    function initialFund(
        ...
        require(
            pool.startBlock == 0,
            "initialFund: initial funding already complete"
        );
        IERC20 erc20 = pool.rewardToken;

        uint256 startTokenBalance = erc20.balanceOf(address(this));
        erc20.safeTransferFrom(address(msg.sender), address(this), _amount);
        uint256 trueDepositedTokens = erc20.balanceOf(address(this)) -
            startTokenBalance;

        (pool.lastRewardBlock, pool.startBlock, pool.endBlock) = (
            _startBlock,
            _startBlock,
            _startBlock + uint32(trueDepositedTokens / pool.rewardPerBlock)
        );
    }

    // Fund the farm, increase the end block
    function fundMore(uint256 _pid, uint256 _amount) external {
        PoolInfo memory pool = poolInfo[_pid];

        require(
            block.number < pool.endBlock,
            "fundMore: pool closed or use initialFund() first"
        );
        IERC20 erc20 = pool.rewardToken;
        ...
        pool.endBlock += uint32(trueDepositedTokens / pool.rewardPerBlock);
    }
    ...
    function minStake(uint256 _pid, uint256 _minStake) external onlyOwner {
        poolInfo[_pid].minStake = _minStake;
    }

    function maxStake(uint256 _pid, uint256 _maxStake) external onlyOwner {
        poolInfo[_pid].maxStake = _maxStake;
    }

    function maxStakersAdj(uint256 _pid, uint16 _maxStakers)
        external
        onlyOwner
    {
    ...
    function stakeTokenFeeAdj(uint256 _pid, uint8 _stakeTokenFee)
        external
        onlyOwner
    {
        poolExtras[_pid].stakeTokenFee = _stakeTokenFee;
    }

    function lockPeriodAdj(uint256 _pid, uint32 _lockPeriod)
        external
        onlyOwner
    {
    ...
    function poolAccessTokenReq(uint256 _pid, bool _accessTokenRequired)
        external
        onlyOwner
    {
    ...
    function poolAccessTokenAddy(uint256 _pid, IERC20 _accessToken)
        external
        onlyOwner
    {
    ...
    function poolAccessTokenMin(uint256 _pid, uint256 _accessTokenMin)
        external
        onlyOwner
    {
        ...
        returns (uint256)
    {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            uint256 lastBlock = block.number < pool.endBlock
                ? block.number
                : pool.endBlock;
            uint256 nrOfBlocks = lastBlock - pool.lastRewardBlock;
            uint256 erc20Reward = nrOfBlocks * pool.rewardPerBlock;
            accERC20PerShare =
                accERC20PerShare +
    ...
    function totalPending(uint256 _pid) external view returns (uint256) {
        if (block.number <= poolInfo[_pid].startBlock) return 0;

        uint256 lastBlock = block.number < poolInfo[_pid].endBlock
            ? block.number
            : poolInfo[_pid].endBlock;
        return
            poolInfo[_pid].rewardPerBlock *
            (lastBlock - poolInfo[_pid].startBlock) -
            poolInfo[_pid].paidOut;
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint32 lastBlock = uint32(block.number) < pool.endBlock
            ? uint32(block.number)
            : pool.endBlock;

        if (lastBlock <= pool.lastRewardBlock) return;
        ...
        uint256 erc20Reward = (lastBlock - pool.lastRewardBlock) *
            pool.rewardPerBlock;
    ...
    function deposit(uint256 _pid, uint256 _amount) external payable {
        ...
        if (poolEx.accessTokenRequired)
            require(
                poolEx.accessToken.balanceOf(msg.sender) >=
                    poolEx.accessTokenMin,
                "Must have minimum amount of access token!"
            );

        require(
            poolEx.totalStakers < poolEx.maxStakers,
            "Max stakers reached!"
        );
        ...
        require(
            _amount >= pool.minStake &&
                (_amount + user.amount) <= pool.maxStake,
        ...
        uint256 startTokenBalance = pool.stakeToken.balanceOf(address(this));
        pool.stakeToken.safeTransferFrom(
            address(msg.sender),
            address(this),
            _amount
        );
        uint256 endTokenBalance = pool.stakeToken.balanceOf(address(this));
        uint256 depositFee = (poolEx.stakeTokenFee * endTokenBalance) / DIVISOR; //1000);
        uint256 trueDepositedTokens = endTokenBalance -
            startTokenBalance -
            depositFee;

        if (user.amount == 0 && _amount > 0) poolEx.totalStakers++;
        user.amount += trueDepositedTokens;
        user.depositStamp = block.number;
        pool.tokensStaked += trueDepositedTokens;
        user.rewardDebt = (user.amount * pool.accERC20PerShare) / 1e36;

        // remit fees
        if (depositFee > 0) pool.stakeToken.safeTransfer(treasury, depositFee);
    ...
    function withdraw(uint256 _pid, uint256 _amount) external payable {
        ...
        if (pendingAmount > 0) erc20Transfer(msg.sender, _pid, pendingAmount);
        ...
        if (_amount > 0) {
            require(
                block.number >= user.depositStamp + poolEx.lockPeriod,
                "Lock period not fulfilled"
            );
        ...
        if (user.amount == 0) poolEx.totalStakers--;
    ...
    function erc20Transfer(
        ...
        IERC20(poolInfo[_pid].rewardToken).safeTransfer(_to, _amount);
    }

    // adjust default/global gas fee
    function adjustGasGlobal(uint256 newgas) external onlyOwner {
        gasAmount = newgas;
    }

    // adjust pool gas/eth fee
    function adjustPoolGas(uint256 _pid, uint256 _newgas) external onlyOwner {
        poolInfo[_pid].gasAmount = _newgas;
    }

    function adjustBlockReward(uint256 _pid, uint256 _newReward)
        external
        onlyOwner
    {
        poolInfo[_pid].rewardPerBlock = _newReward;
    }

    function adjustEndBlock(uint256 _pid, uint32 _newBlock) external onlyOwner {
        poolInfo[_pid].endBlock = _newBlock;
    }

    function adjustLastBlock(uint256 _pid, uint32 _newBlock)
        external
        onlyOwner
    {
        poolInfo[_pid].lastRewardBlock = _newBlock;
    }
