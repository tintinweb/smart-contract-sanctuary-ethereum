...
contract Primes is ERC721, Ownable {
    ...
    constructor() ERC721("Primes", "PRIMES") {}

    function publicMint(address _to, uint _quantity) external payable {
    ...
    function tokenURI(
        ...
        uint primesCount = PrimesUtils.getNumberOfPrimeNumbers(_tokenId);
        return
            string(
                abi.encodePacked(
        ...
        for (y = 0; y <= 800; y += 100) {
            for (x = 0; x <= 800; x += 100) {
                ...
                if (PrimesUtils.isPrime(uint256(count))) {
                    string memory d = PrimesUtils.setDefinition(x, y);
                    string memory values = setValues(x, y);
    ...
    function setValues(uint x, uint y) internal pure returns (string memory) {
        string memory min = PrimesUtils.setMinValuesAnimation(x, y);
        string memory max = PrimesUtils.setMaxValuesAnimation(x, y);
        bytes memory tempMinTest = bytes(min);
        bytes memory tempMaxTest = bytes(max);
        if (tempMinTest.length == 0 || tempMaxTest.length == 0) {
            revert("Primes: something wrong with values");
        } else {
            return string(abi.encodePacked(min, ";", max, ";", min));
        }
    }
        ...
        if (x == 200 && y == 0) {
            ...
            return
                string(
                    abi.encodePacked(
                        "M",
                        Strings.toString(x + 50),
                        " ",
                        Strings.toString(y),
                        "H",
                        Strings.toString(x + 100),
                        "V",
                        Strings.toString(y + 50),
                        "H",
                        Strings.toString(x + 50),
                        "Z"
                    )
    ...
    function setMaxValuesAnimation(
        ...
        if (x == 200 && y == 0) {
            ...
            if (x == 0 || x == 300 || x == 600) {
                if (y == 0 || y == 200 || y == 400 || y == 600 || y == 800) {
                    // lb
                    string memory _m = y == 0 ? "M-" : "M";
                    string memory _h = y == 0 ? "H-" : "H";
                    string memory z_ = x == 0 ? " 0" : " ";
                    return
                        string(
                            abi.encodePacked(
                                _m,
