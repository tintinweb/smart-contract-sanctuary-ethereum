...
contract HordTicketManager is HordUpgradable, ERC1155HolderUpgradeable {
    ...
    // Represent address of HPoolManager contract
    ...
    event HordTicketFactorySet(address hordTicketFactory);
    ...
    /**
        * @notice Function is setting address of HPoolManager contract.
        *
        * @param  _hPoolManager - address of HPoolManager contract
    */
    ...
    {
        require(
            _hPoolManager != address(0),
            "HPoolManager can not be 0x0 address"
        );

        hPoolManager = _hPoolManager;
        emit HPoolManagerSet(hPoolManager);
    }
    ...
    public
    {
        (, , address champion, , , , , , ,uint256 ticketSaleEndTime, ,) = IHPoolManager(hPoolManager).getPoolInfo(poolId);

        require(msg.sender != champion, "Champion can not stake on his own wallet.");
        require(block.timestamp <= ticketSaleEndTime, "The time has elapsed for the ticket sale phase.");
