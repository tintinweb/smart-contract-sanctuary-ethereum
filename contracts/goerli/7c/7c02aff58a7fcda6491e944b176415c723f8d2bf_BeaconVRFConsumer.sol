...
abstract contract IVRFCoordinatorConsumer is VRFBeaconTypes {
    IVRFCoordinatorExternalAPI internal immutable i_coordinator;

    constructor(address _coordinator) {
        i_coordinator = IVRFCoordinatorExternalAPI(_coordinator);
    }
    ...
    function rawFulfillRandomWords(
        ...
        if (address(i_coordinator) != msg.sender) revert MustBeCoordinator();
...
contract VRFBeaconTypes {
    ...
    RequestID internal constant MAX_REQUEST_ID =
        RequestID.wrap(type(uint48).max);
    uint8 internal constant NUM_CONF_DELAYS = 8;
...
contract ECCArithmetic {
    ...
    uint256 private constant PAIRING_GAS_COST = 34_000 * 2 + 45_000; // Gas cost as of Istanbul; see EIP-1108
    uint256 private constant PAIRING_PRECOMPILE_ADDRESS = 0x8;
    uint256 private constant PAIRING_INPUT_LENGTH = 12 * 0x20;
    uint256 private constant PAIRING_OUTPUT_LENGTH = 0x20;
