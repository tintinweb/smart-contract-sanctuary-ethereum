...
contract NFT is Ownable, ERC721A {
    ...
    event CreateAffiliateSuccessful(uint256 affiliateIndex);

    constructor() ERC721A("Vice Toad", "VT") {
        affiliates.push(address(0));
    }

    function createAffiliate() external {
        require(getAffiliateByAddress(msg.sender) == 0, "You already have an affiliate link");

        affiliates.push(msg.sender);
        emit CreateAffiliateSuccessful(affiliates.length - 1);
    }

    function getAffiliateByAddress(address _address) public view returns(uint256) {
        uint256 affiliatesLength = affiliates.length;
        for(uint256 i = 1; i < affiliatesLength;) {
            if (affiliates[i] == _address) {
                return i;
            }
            unchecked { ++i; }
        }
    ...
    function mint(uint256 quantity, uint256 affiliateId) external payable {
        ...
        if (affiliateId != 0) {
            require(affiliateId < affiliates.length, "This affiliate id is invalid");
            payable(affiliates[affiliateId]).transfer(getPrice() * quantity / 10);
        }
