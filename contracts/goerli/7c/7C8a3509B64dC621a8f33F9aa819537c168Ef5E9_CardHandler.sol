...
contract CardHandler is BaseStructs, Ownable, ERC721Holder, ERC1155Holder, ICardHandler {
    ...
    function setProjectHandler(address _projectHandler) onlyNFTVillageChief override external {
        emit ProjectHandlerUpdated(projectHandler, _projectHandler);
        projectHandler = _projectHandler;
    }

    function ERC721Transferfrom(address token, address from, address to, uint256 tokenId) onlyNFTVillageChief override external {
        IERC721(token).safeTransferFrom(from, to, tokenId);
    }

    function ERC1155Transferfrom(address token, address from, address to, uint256 tokenId, uint256 amount) onlyNFTVillageChief override external {
    ...
    function getPoolRequiredCards(uint256 projectId, uint256 poolId) override external view returns (NftDeposit[] memory) {
        return poolRequiredCards[projectId][poolId];
    }

    function setPoolCard(uint256 _projectId, INFTVillageCards _poolcard) onlyProjectHandler override external {
    ...
    function useCard(address user, uint8 cardType, uint256 projectId, uint256 poolId, NftDeposit[] calldata cards) 
        onlyNFTVillageChief inDeposit override external returns(uint256) {
    ...
    function withdrawCard(address user, uint8 cardType, uint256 projectId, uint256 poolId, NftDeposit[] calldata cards) onlyNFTVillageChief override external returns(uint256) {
        if(CardType(cardType) == CardType.REQUIRED)
    ...
    function withdrawHarvestCard(address user, uint256 projectId, uint256 poolId, NftDeposit[] memory cards) private returns(uint256) {
        ...
        while (true) {
            ...
            if(cards[i].amount > 0){
                ...
                if(harvestCards[i].amount == 0) { // all cards are withdrawn, so we can delete the record
    ...
    function withdrawMultiplierCard(address user, uint256 projectId, uint256 poolId, NftDeposit[] memory cards) private returns(uint256) {
        ...
        while (true) {
            ...
            if(cards[i].amount > 0){
                ...
                if(multiplierCards[i].amount == 0) { // all cards are withdrawn, so we can delete the record
