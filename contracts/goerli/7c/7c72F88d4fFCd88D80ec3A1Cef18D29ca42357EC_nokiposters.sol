...
contract nokiposters is ERC721A, ReentrancyGuard, Ownable {
  ...
  constructor (
    ...
    MAX_STANDARD_PER_WALLET = 10;
    MAX_MULTIMINT = 101;

    MAX_STANDARD_SUPPLY = 101;
    MAX_STANDARD_RESERVED_SUPPLY = 101;
  ...
  function mint(uint256 count) public payable nonReentrant {
   ...
   if(allowedStandardMintCount(_msgSender()) == MAX_MULTIMINT) {
    if(count==1){

    if (allowedStandardMintCount(_msgSender()) > 0) {
      updateStandardMintCount(_msgSender(), count);
    } else { revert("Minting limit exceeded"); }

    for (uint256 i = 0; i < count; i++) { standardSupplyCounter.increment(); }
    _safeMint(_msgSender(), count);

    } else { 

    require(msg.value>= (STANDARD_PRICE * count)-STANDARD_PRICE, "Insufficient payment");
    if (allowedStandardMintCount(_msgSender()) > 0) {
      updateStandardMintCount(_msgSender(), count);
    } else { revert("Minting limit exceeded"); }
