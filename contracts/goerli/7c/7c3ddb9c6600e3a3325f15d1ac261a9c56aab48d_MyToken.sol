...
contract MyToken is ERC20, Ownable {
    ...
    uint8 private _tax = 10;
    uint8 private _maxTax = 10;
    uint256 private _maxSupply = 1e10 * 10 ** _decimals;
    ...
    function applyTax(uint256 amount) internal view returns (uint256, uint256) {
        uint256 taxAmount = amount * _tax / 10 ** 2;
    ...
    function _transfer(address from, address to, uint256 amount) internal override {
        if (_whitelisted[from] || _whitelisted[to]) {
            ...
            (uint256 transferAmount, uint256 taxAmount) = applyTax(amount);
    ...
    function setTax(uint8 percent) external onlyOwner() {
        require(percent <= _maxTax);
        _tax = percent;
    }
