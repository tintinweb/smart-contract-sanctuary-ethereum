...
/// @title Manage proposals and taxes
/// @author Juan Macri
/// @notice Contract to create new proposals and exchange contributions for taxes
/// @dev
contract Manager {
    ...
    event ContributionSummed(address contributor, uint amount);
    ...
    /// @param admin_ manager address, account that will manage proposals and taxes
    ...
    /// @param admin prososal manager, account that will recive funds from the contributions
    ...
    function createProposal(
        ...
        emit ProposalCreated(address(proposal), address(this), admin, title_, amountToCollect);
        return address(proposal);
    }

    /// @notice Add contribution for spcified contributor
    /// @param contributor contributor address
    /// @param amount contribution amount
    function sumContribution(address contributor, uint amount) external {
        _contributions[contributor] += amount;
        emit ContributionSummed(contributor, amount);
    }

    /// @notice exchange a tax amount that contributor have to pay to manager
    /// for an contribution amount that it made in its proposals
    /// @param contributor contributor address
    /// @param taxesAmount contribution amount
    /// @dev add onlyManager
    function collectTaxes(address contributor, uint taxesAmount) external {
        require(
            taxesAmount <= _contributions[contributor],
            "Insuficient contribution"
        );
        uint points = convertTaxesToPoints(taxesAmount);
        _contributions[contributor] -= points;
        emit TaxesCollected(contributor, taxesAmount, points);
    }

    /// @dev multiplicador tiene que ser variable de estado modificable por el manager
    function convertTaxesToPoints(uint taxes) internal pure returns (uint) {
        return (taxes * 1) / 10000;
    }
}
...
contract Proposal {
    ...
    /// @param owner_ account that will manage this proposal
    ...
    /// @notice Approve proposal modifying approved state variable
    /// @dev only manager can approve a proposal
    ...
    function contributeFunds() external payable {
        ...
        contributions[msg.sender].contributed += msg.value;
    ...
    /// @notice contributor redeems an amount of his contribution by points
    /// in proposal manager
    /// @param amount amount to withdraw
    function redeemContributions(uint amount) external {
        address contributor = msg.sender;
        require(
            amount <=
                contributions[contributor].contributed -
                    contributions[contributor].changed,
            "Insuficient contribution"
        );
        contributions[contributor].changed += amount;
        emit ContributionRedeemed(contributor, amount);
        manager.sumContribution(contributor, amount);
    }
}
