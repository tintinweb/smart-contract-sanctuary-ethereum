...
error KaijuMerchPopup_MustBeAKaiju();
...
contract KaijuMerchPopup is AccessControl {
    ...
    /**
     * @notice Modifier that requires a sender to own a Kaiju
     * @param account Address to check holder status of
     */
    modifier onlyKaijuz(address account) {
        if (!isKaijuHolder(account)) revert KaijuMerchPopup_MustBeAKaiju();
        _;
    }

    /**
     * @notice Returns whether or not the specified address holds a genesis, baby, or mutant kaiju
     * @param account Address to return holder status of
     */
    function isKaijuHolder(address account) public view returns (bool) {
        return KITS.isHolder(account) || MUTANTS.balanceOf(account) > 0;
    }
    ...
    /**
     * @notice Purchase merch for a specified receiver
     * @notice Specified receiver must hold a kaiju
     * @param merchId Merch identifier
     * @param receiver Receiver of the purchase
     */
    function _purchase(uint256 merchId, address receiver) internal onlyKaijuz(receiver) {
