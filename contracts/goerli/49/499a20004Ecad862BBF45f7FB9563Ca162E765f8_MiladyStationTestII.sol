...
contract MiladyStationTestII is ERC721, ERC721Enumerable, ERC721Burnable, DefaultOperatorFilterer, Ownable {
    using SafeMath for uint256;

    string public MILADY_PROVENANCE = "";
    string public IPFSURI = "";
    uint public constant maxMiladyPurchase = 30;
    uint256 public constant MAX_MILADYS = 1212;
    bool public saleIsActive = false;
    uint256 public standardMiladyCount = 0;
    address public miladyAddress = 0x5Af0D9827E0c53E4799BB226655A1de152A425a5;
    //address public miladyAddress = 0xDA0bab807633f07f013f94DD0E6A4F96F8742B53; //remix miladystation
    ...
    constructor() ERC721("MiladyStationTestII", "MS") {}
    ...
    function miladyHolderCheck(address holder) public view returns (uint256) {
        uint256 tokenNum = 1219;
        try milady.tokenOfOwnerByIndex(holder, 0) returns (uint256 miladyHolderIndex) {
    ...
    function reserveMintMiladys() public {
        require(whitelistHunMint[msg.sender], "sender not whitelisted");
        uint mintAmount;
    ...
    function mintMiladys(uint256 numberOfTokens) public payable {
        ...
        uint256 miladyPrice;
        uint256 senderMiladyToken = miladyHolderCheck(msg.sender);
        //Mint for OG
        if (senderMiladyToken < 5){
            if (numberOfTokens == 30) {
                miladyPrice = 8000000000000000; // 0.008 ETH should be 9 dollars
                require(miladyPrice.mul(numberOfTokens) <= msg.value, "Ether value sent is not correct");
            } else if (numberOfTokens >= 15) {
                miladyPrice = 9000000000000000; // 0.009 ETH should be 10 dollars
                require(miladyPrice.mul(numberOfTokens) <= msg.value, "Ether value sent is not correct");
            } else if (numberOfTokens >= 5) {
                miladyPrice = 10500000000000000; // 0.0105 ETH should be 11.50 dollars
                require(miladyPrice.mul(numberOfTokens) <= msg.value, "Ether value sent is not correct");
            } else {
                miladyPrice = 11000000000000000; // 0.011 ETH should be 12 dollars
                require(miladyPrice.mul(numberOfTokens) <= msg.value, "Ether value sent is not correct");
            }
        //Mint for Miladys
        } else if (senderMiladyToken < 40){
            if (numberOfTokens == 30) {
                miladyPrice = 11000000000000000; // 0.011 ETH should be 12 dollars
                require(miladyPrice.mul(numberOfTokens) <= msg.value, "Ether value sent is not correct");
            } else if (numberOfTokens >= 15) {
                miladyPrice = 12000000000000000; // 0.012 ETH should be 13 dollars
                require(miladyPrice.mul(numberOfTokens) <= msg.value, "Ether value sent is not correct");
            } else if (numberOfTokens >= 5) {
                miladyPrice = 13000000000000000; // 0.013 ETH should be 14 dollars
                require(miladyPrice.mul(numberOfTokens) <= msg.value, "Ether value sent is not correct");
            } else {
                miladyPrice = 14000000000000000; // 0.014 ETH should be 15 dollars
                require(miladyPrice.mul(numberOfTokens) <= msg.value, "Ether value sent is not correct");
            }
        //Mint for Normie
        } else if (senderMiladyToken == 1219){
          if (numberOfTokens == 30) {
                miladyPrice = 14000000000000000; // 0.014 ETH should be 15 dollars
                require(miladyPrice.mul(numberOfTokens) <= msg.value, "Ether value sent is not correct");
            } else if (numberOfTokens >= 15) {
                miladyPrice = 15000000000000000; // 0.015 ETH should be 17 dollars
                require(miladyPrice.mul(numberOfTokens) <= msg.value, "Ether value sent is not correct");
            } else if (numberOfTokens >= 5) {
                miladyPrice = 16000000000000000; // 0.016 ETH should be 18 dollars
                require(miladyPrice.mul(numberOfTokens) <= msg.value, "Ether value sent is not correct");
            } else {
                miladyPrice = 18000000000000000; // 0.018 ETH should be 20 dollars
                require(miladyPrice.mul(numberOfTokens) <= msg.value, "Ether value sent is not correct");
            }
        }
