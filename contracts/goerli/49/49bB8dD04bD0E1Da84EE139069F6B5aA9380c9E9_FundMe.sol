...
contract FundMe {
    ...
    // Main function that allows users to fund
    function fund() public payable {
        // 18 digit number to be compared with donated amount
        uint256 minimumUSD = 50 * 10**18;
        //is the donated amount less than 50USD?
        require(
            fromEthToUsd(msg.value) >= minimumUSD,
    ...
    // Get the latest conversion ETH / USD
    ...
    // Convert the price from ETH to USD
    function fromEthToUsd(uint256 ethAmount) public view returns (uint256) {
    ...
    function fromUsdToEth(uint256 usdAmount) public view returns (uint256) {
        uint256 ethPrice = getEthPrice();
