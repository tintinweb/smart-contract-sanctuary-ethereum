...
contract EasyERC721 is ERC721Batch, Ownable{
...
abstract contract ERC721B is Context, ERC165, IERC721, IERC721Metadata {
  ...
  //TOOD: range
  uint16 internal _burned;
  uint16 internal _minted;
  uint16 internal _offset;
  uint16 internal _range;
  ...
  constructor(string memory name_, string memory symbol_, uint16 offset_ ){
    _name = name_;
    _symbol = symbol_;
  ...
  function _safeMint(address to, uint16 tokenId) internal {
    _safeMint(to, tokenId, "");
  }

  function _safeMint(address to, uint16 tokenId, bytes memory _data) internal {
    _mint(to, tokenId);
    require(
