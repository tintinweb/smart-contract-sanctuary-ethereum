// SPDX-License-Identifier: MIT
    ...
    contract PaymentPlan is Pausable, AccessControl, IERC721Receiver, ReentrancyGuard {
        ...
        enum PaymentStatus {NOT_STARTED, NOT_PAID, PAID, EXPIRED, COMPLETED, CANCELLED}
        ...
        //If user has already expired plan can they 'restart'?
        ...
        function newPlan() external payable whenNotPaused { 
            ...
            if 
            (
                paymentPlan[msg.sender].paymentStatus == PaymentStatus.NOT_PAID 
                || 
                paymentPlan[msg.sender].paymentStatus == PaymentStatus.PAID
                ||
        ...
        function payInstalment() external payable whenNotPaused {
            ...
            if (paymentPlan[msg.sender].paymentStatus == PaymentStatus.PAID) {
                revert PaymentAlreadyMade();
            }

            // check if user has already expired
            if (paymentPlan[msg.sender].paymentStatus == PaymentStatus.EXPIRED) {
                revert PaymentPlanExpired();
            }
        ...
        function fiatNewPlan(address _client, uint256 _amount) external onlyRole(MANAGER_ROLE) whenNotPaused {
            // check if user has already created a plan
        ...
        function fiatPayment(address _client, uint256 _amount) external onlyRole(MANAGER_ROLE) whenNotPaused {
            ...
            // check if user has already paid
            if (paymentPlan[_client].paymentStatus == PaymentStatus.PAID) {
                revert PaymentAlreadyMade();
            }

            // check if user has already expired
            if (paymentPlan[_client].paymentStatus == PaymentStatus.EXPIRED) {
                revert PaymentPlanExpired();
            }
            ...
            paymentPlan[_client].paymentStatus = PaymentStatus.PAID;
        ...
        function claimMembership(address claimer, Voucher calldata voucher) external whenNotPaused {
            ...
            //mint NFT
            // oneClub.safeMintToPaymentPlans(claimer, voucher);
        ...
        function refreshPaymentStatus(address user) external whenNotPaused onlyRole(MANAGER_ROLE) {
            // check if user has already created a plan
            if (paymentPlan[user].totalPayment == 0) {
                revert PaymentPlanDoesNotExist();
            }

            // check if user has already expired
            if (paymentPlan[user].paymentStatus == PaymentStatus.EXPIRED) {
                revert PaymentPlanExpired();
            }

            // check if user has already completed
            if (paymentPlan[user].paymentStatus == PaymentStatus.COMPLETED) {
                revert PaymentPlanCompleted();
            }

            // check if user has already cancelled
            if (paymentPlan[user].paymentStatus == PaymentStatus.CANCELLED) {
                revert PaymentPlanCancelled();
            }

            // check if user has already claimed membership
            if (paymentPlan[user].membershipClaimed == true) {
                revert MembershipAlreadyClaimed();
            }

            // check if user payment plan has expired
            if (block.timestamp > paymentPlan[user].lastPaymentStamp + 30 days + gracePeriod) {
                paymentPlan[user].paymentStatus = PaymentStatus.EXPIRED;
                emit PaymentExpired(
                    user,
                    block.timestamp,
                    paymentPlan[user].totalPayment - paymentPlan[user].paidAmount,
                    paymentPlan[user].paidAmount
                );
            }

            //check if payment is due
            if (block.timestamp > paymentPlan[user].lastPaymentStamp + 30 days && paymentPlan[user].paymentStatus == PaymentStatus.PAID) {
                paymentPlan[user].paymentStatus = PaymentStatus.NOT_PAID;
                emit PaymentDue(
                    user,
                    block.timestamp,
                    paymentPlan[user].monthlyPayment
                );
            }
        }
