...
contract StakeNFT {
    ...
    //enumerator
    enum StakingStatus {Active, Claimable, Cancelled}
    ...
    function checkStake(uint stakingId, address staker)public returns (Staking memory) {
        ...
        require(staking.status == StakingStatus.Active,"Staking is not active");
    ...
    function claimStake(uint stakingId) public returns(Staking memory){
        ...
        require(staking.status == StakingStatus.Claimable,"Your reward is not claimable");
