...
contract ProofOfDeath {
    ...
    // A -> [X1, X2, ..., Xn]
    mapping(address => address[]) public inheritance;
    ...
    // A -> timestamp (block)
    mapping(address => uint256) public timeBomb;
    ...
    function allowInheritance(
        ...
        allowances[msg.sender][smartContract] = amount;
    }
    ...
    function withdraw(
        ...
        require(allowances[from][smartContract] > 0, "Allowances not made");
        // TODO: check signatures of signers
        IERC20(smartContract).transferFrom(from, msg.sender, amount);
        allowances[from][smartContract] -= amount;
    }
}
