...
contract Exchange is ERC20 {
    ...
    // ETH -> ERC20
    function ethToTokenTransfer(uint256 _minTokens, address _receipent) public payable {
        ethToToken(_minTokens, _receipent);
    }

    function ethToToken(uint _minTokens, address _receipent) private {
        uint256 tokenReserve = token.balanceOf(address(this));
        ...
        //transfer token out
        require(token.transfer(_receipent, outputAmount)); 
    }
    ...
    // ERC20 -> ETH
    function tokenToTokenSwap (uint256 _tokenSold, uint256 _minTokenBought, uint256 _minEthBought, address _tokenAddress) public {

        address toTokenExchangeAddress = factory.getExchange(_tokenAddress);

        uint256 tokenReserve = token.balanceOf(address(this));
        uint256 ethOutputAmount = getOutputAmount(_tokenSold, tokenReserve, address(this).balance);
...
interface IExchange {
    ...
    function ethToTokenTransfer(uint256 _minTokens, address _receipent) external payable;
}
