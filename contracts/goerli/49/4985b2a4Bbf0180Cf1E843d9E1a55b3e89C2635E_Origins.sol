...
contract Origins is ERC721A, OperatorFilterer, Ownable, ERC2981 {
    ...
    bytes32 private constant OGWL_TYPEHASH =
        keccak256("Allowlist(address minter,uint256 alloc,uint256 og)");
    bytes32 private constant PUBLIC_TYPEHASH = keccak256("Allowlist(address minter,uint256 alloc)");
    ...
    constructor(
        ...
        baseURI = baseURI_;
        signer = signer_;
        ogwlPrices[0] = uint128(prices_[0]);
        ogwlPrices[1] = uint128(prices_[1]);
        publicPrice = prices_[2];
        // Reserve for Founders Pass holders
        _mintERC2309(owner, reservedQuantity);
    ...
    event PriceChanged(uint256 mintState, uint256 price);
    ...
    /// @dev We don't need to check for to=address(0) because ERC721A does the check for us.
    ...
    function getPrice(uint256 mintState_) external view returns (uint256 price) {
        if (mintState_ < PUBLIC_STATE) {
            price = uint256(ogwlPrices[mintState_]);
        } else {
            price = publicPrice;
        }
    }
    ...
    function setPrice(uint256 mintState_, uint128 price) external onlyOwner {
        if (mintState_ > PUBLIC_STATE) revert WrongMintState();
        if (mintState_ < PUBLIC_STATE) {
            ogwlPrices[mintState_] = price;
        } else {
            publicPrice = price;
        }
    }
    ...
    function setSigner(address signer_) external onlyOwner {
        if (signer_ == address(0)) revert SignerZeroAddress();
        signer = signer_;
    }
    ...
    function mintRemaining(address to) external onlyOwner {
        ...
        if (mintState != 2) revert WrongMintState();
    ...
    // Overrides
    /// @notice First 10 tokens (0 to 9) are excluded from the token shift to reserve fixed identity Origin characters
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        if (!_exists(tokenId)) revert URIQueryForNonexistentToken();
        if (revealed) {
            if (tokenId > 9) {
                tokenId = ((tokenId + tokenIdShift) % (MAX_SUPPLY - 10)) + 10;
            }
