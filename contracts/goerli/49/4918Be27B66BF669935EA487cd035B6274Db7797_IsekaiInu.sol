...
contract IsekaiInu is ERC20, Ownable {
    ...
    uint256 private _supply = 100000000000; // 100 Billion

    uint256 public buyTax = 3;
    uint256 public sellTax = 3;

    uint256 public maxTxAmount = 2000000000 * 10**_decimals; // 2%
    uint256 public maxWalletAmount = 2000000000 * 10**_decimals; // 2%

    address public devWallet = 0xF8df0be163284c39C99E32b80E16aef5C6CC9db4;
    address public marketingWallet = 0xADA8183a983f17a8391AbDd3f8A20Bc62C964e6D;
    address public treasuryWallet = 0x28B3Fe00e853aE8f93EE9Ae0854054dC09082CBe;

    IUniswapV2Router02 public immutable uniswapV2Router;
    address public immutable uniswapV2Pair;

    uint256 private _taxCollected = 0;
    mapping(address => bool) private _isExcludedFromFees;
    mapping(address => bool) private _isExcludedMaxLimits;
    uint256 private _tokensSwapThreshold = 200000000 * 10**_decimals; // 0.2%
    ...
    constructor() ERC20(_name, _symbol) {
        ...
        // TODO: check if pair/router is needed
    ...
    function _transfer(
        ...
        // TEMP
        emit TempTokenTransfer(from, to, amount, saleStatus);

        // launch
        // no limits + no fees
        // only internal wallets (owner, address(this), dev/marketing/treasury,
        //                        uniswap pair, uniswap router??? <-- check if transfer called with router)

        // private sale
        // check limits + no fees
        // only internal wallets + whitelisted wallets
        //              <-- check if transfer is called with uniswap Router

        // public sale
        // check limits + take fees
        // all wallets (check all transfers until LP removed)

        // trading active check
        if (saleStatus == 0) {
            // stop non internal wallets from buy/sell/transfer
            require(
                (_isExcludedFromFees[from] || _isExcludedFromFees[to]),
                "Trading/Sale is not yet active."
            );
        } else if (saleStatus == 1) {
            // stop non approved wallets from buying
            if (from == uniswapV2Pair && !_isExcludedMaxLimits[to]) {
                // skip whitelist check if internal wallet
                require(
                    _isWhitelisted[to],
                    "Not approved to buy during private sale."
                );
            }
        }

        if (limitsInEffect) {
            // Already handles saleStatus 0 cases
            if ((from != owner() && to != owner()) && !inSwapping) {
                // // trading active check
                // if (saleStatus == 0) {
                //     require((_isExcludedFromFees[from] || _isExcludedFromFees[to]), "Trading is not yet active.");
                // }
                ...
                if (from == uniswapV2Pair && !_isExcludedMaxLimits[to]) {
                    ...
                    // Sell
                } else if (to == uniswapV2Pair && !_isExcludedMaxLimits[from]) {
                    require(
                        amount <= maxTxAmount,
                        "Sell transfer amount exceeds the maxTxAmount."
                    );
                    // Transfer
                } else if (!_isExcludedMaxLimits[to]) {
                    require(
                        amount + balanceOf(to) <= maxWalletAmount,
        ...
        if (
            ...
            if (_taxCollected != contractTokenBalance) {
                _taxCollected = contractTokenBalance;
            }
            _taxCollected -= _tokensSwapThreshold;
    ...
    function updateWhitelist(address newWallet, bool isWhitelist)
        external
        onlyOwner
    {
        _isWhitelisted[newWallet] = isWhitelist;
    }

    function startPrivateSale() external onlyOwner {
        require(
            saleStatus == 0,
            "Can only start private sale once after launch."
        );
        saleStatus = 1;
    }
    ...
    function startPublicSale() external onlyOwner {
        require(
            saleStatus == 1,
            "Can only start public sale once after private sale."
        );
        saleStatus = 2;
    }

    // function enableTrading() external onlyOwner {
    //     // set trading active (cannot be turned off after)
    //     tradingActive = true;
    // }

    function removeLimits() external onlyOwner {
        // remove limits after token is stable (cannot be turned back on)
        require(saleStatus == 2, "Can only remove limits during public sale.");
    ...
    function updateBuySellTax(uint256 newBuy, uint256 newSell)
        public
        onlyOwner
    {
        require(newBuy <= 5, "Buy tax cannot exceed 5.");
        require(newSell <= 5, "Sell tax cannot exceed 5.");
        buyTax = newBuy;
        sellTax = newSell;
    }
