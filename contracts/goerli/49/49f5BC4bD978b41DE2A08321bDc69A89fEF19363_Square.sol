 ...
 * その他要件は開発Phaseで定義
 */
contract Square is Ownable, Burnable, ERC721Enumerable, SupportTokenUpdateHistory, ISquare {
...
//    mapping(uint256=>address) private _tokenValidator;
    ...
    function setMaxSquarePerValidator(uint256 maxSquarePerValidator) external onlyOwner {
    ...
    function tokens(uint256 tokenId) external view returns(TokenInfo memory) {
        require(tokenId > 0 && tokenId <= tokenInfo.length,"Square: invalid tokenId");
        require(tokenInfo[tokenId - 1].validator != address(0),"Square: invalid tokenId");
        return tokenInfo[tokenId - 1];
    }

    function mint(address to, string calldata _metadata) external returns (uint256) {
        require(_roundContract.isValidator(msg.sender),
                "Square: caller is not validator");
        require(_numIssued[msg.sender] < _maxSquarePerValidator,
    ...
    function burn(uint256 tokenId) external {
        require(msg.sender == validatorOf(tokenId) || isBurner(msg.sender),"Square: burner is not a validator of token nor burner");
        _burn(tokenId);
        _numIssued[msg.sender]--;
    ...
    function takeAway(uint256 tokenId) external {
        require(validatorOf(tokenId) == msg.sender,"Square: not a validator of the token");
     ...
     */
    function validatorOf(uint256 tokenId) public view override returns (address) {
        require(tokenId > 0 && tokenId <= tokenInfo.length,"Square: invalid tokenId");
