...
contract CampaignFactory {
  ...
  function createCampaign(
    ...
    address payable _recipient
      ...
      campaign.setParams(_name, _goal, _startAt, _endAt, _recipient);
      _campaigns.push(campaign);
      emit CampaignCreated(address(campaign));
  }

  function getAllCampaigns(uint startIndex, uint endIndex) 
    view 
    public 
    returns (Campaign[] memory) {
      
      require(endIndex <= getNumberOfCampaigns(), "Son index sinirlar disinda.");
      require(endIndex > startIndex, "Yanlis index araligi.");

      Campaign[] memory campaigns = new Campaign[](endIndex - startIndex);
      
      uint index = 0;
      for (uint i = startIndex; i < endIndex; i++) {
        campaigns[index] = _campaigns[i];
        index++;
      }
...
contract Campaign {
    ...
    mapping (uint => mapping (address => Drop)) public drops;
    ...
    function getAllDrops() public view returns (
        DropResponse[] memory
    ) {
        DropResponse[] memory arr = new DropResponse[](dropsCount);
        for (uint256 i = 0; i < dropsCount; i++) {
            Drop memory drop = drops[i+1][dropAddresses[i]];
            DropResponse memory dropResponse;
            dropResponse.dropId = i+1;
            dropResponse.addr = dropAddresses[i];
            dropResponse.drop = drop;
            arr[i] = dropResponse;
        }
        return arr;
    }
