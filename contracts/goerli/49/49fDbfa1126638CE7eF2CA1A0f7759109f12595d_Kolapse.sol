 ...
 */
contract Kolapse is ERC1155Supply, Ownable , ReentrancyGuard{
    ...
    function createItems(uint256[] memory _ids, uint256[] memory _maxPerWallet, uint256[] memory _maxToMint,uint256[] memory _initialSupply,uint256[] memory _maxSupply,uint256[] memory _price ) external onlyOwner{
        ...
        idStats[_ids[i]].price = _price[i];
        _mint(msg.sender,_ids[i],_initialSupply[i],"");
        }
    }

    function createItem(uint256 _id, uint256 _maxPerWallet, uint256 _maxToMint,uint256 _initialSupply,uint256 _maxSupply,uint256 _price ) external onlyOwner{
        idStats[_id].maxPerWallet = _maxPerWallet;
        idStats[_id].maxToMint = _maxToMint;
        idStats[_id].initialSupply = _initialSupply;
        idStats[_id].maxSupply = _maxSupply;
    ...
    function claimItem(uint256 _quantity,uint256 _id) external {
        ...
        require(
            totalSupply(_id) + _quantity <= idStats[_id].maxSupply,   //poner el max supply para ese id
            "Minting limit reached."
        );
        require(_quantity > 0, "Quantity cannot be 0.");
        require(
                balanceOf(msg.sender,_id) + _quantity <= idStats[_id].maxPerWallet, //poner el max per wallet para ese id
                "Exceeds wallet limit."
            );
        require(
                _quantity <= idStats[_id].maxToMint, //poner el max to mint para ese id
    ...
    function buyItem(uint256 _quantity,uint256 _id) external payable {
        require(idStats[_id].price * _quantity <= msg.value,
