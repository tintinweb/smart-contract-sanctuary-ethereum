...
contract EsterFinance is ERC20, Ownable {
    ...
    bool public vestingFinished = true;
    ...
    function _transfer(address sender, address receipient, uint256 amount) internal override isBuyInternal() {
        ...
        if (!vestingFinished) {
            uint256 airdroppedTokenAmount = airdropAmount[sender];
            ...
            if (airdroppedTokenAmount > 0) {
                ...
                if (elapsedPeriods < vestingPeriods) {
                    ...
                    require(balanceOf(sender) - amount >= minimumBalance);
                } else {
        ...
        if (receipient == marketingReceiver) {
          isExcludedFromWalletLimits[marketingReceiver] = false;
        }
    }
