 ...
 */
contract MaestrosGenesis is ERC721, ERC2981, Ownable {
    ...
    uint8 internal constant TYPE_TICKET = 0;
    uint8 internal constant TYPE_PUBLIC = 1;
    uint8 internal constant TYPE_TREASURY = 2;
    ...
    uint256 public available_Public = 148;
    uint256 public available_Treasury = reservedSupply_Treasury;
    uint256 public available_Tickets = reservedSupply_Tickets;

    uint256 internal nextTicket = 1; // 1-5 for founders, 6-1157 for ticket holders
    uint256 internal nextPublic = 1158; // 1158-1305 for OGs/public
    uint256 internal nextTreasury = 1306; // 1306-1555 for treasury NFTs
    ...
    constructor(
        ...
        for (uint256 i; i < reservedSupply_Founders; ) {
            // TYPE_TICKET or TYPE_PUBLIC
            mintInternal(_founders[i], 1, TYPE_TICKET);
            unchecked {
                ++i;
            }
        }
    ...
    function mint(
        ...
        unchecked {
            ...
            minted[_to] += _qty; // Adding uint8 will never overflow uint256
        }
    ...
    /**
     * @notice  Claim the Maestros associated with a Maestro - Tickets NFT
     * @param   _to     NFT destination address
     * @param   _token   Ticket tier
     */
    function claim(address _to, uint256 _token) external {
        require(claimActive, "!active");
        require(!claimed[_token], "!claimed");

        require(msg.sender == maestroTickets.ownerOf(_token), "!ticketOwner");

        uint256 tier = maestroTickets.ticketTier(_token);
        uint256 qty;
        if (tier == TICKET_DIAMOND) {
            qty = 5;
        } else if (tier == TICKET_GOLD) {
            qty = 3;
        } else if (tier == TICKET_SILVER) {
            qty = 2;
        } else if (tier == TICKET_BRONZE) {
            qty = 1;
        } else {
            revert("!ticket");
        }

        // This would revert if there were 2 left and somebody claimed TICKET_DIAMOND
        // Might want to just give them last 2??
        require(available_Tickets >= qty, "!ticketsSupply");
        unchecked {
            available_Tickets -= qty; // Already checked
        }
        claimed[_token] = true;
        mintInternal(_to, qty, TYPE_TICKET);
    }
    ...
    function totalSupply() external view returns (uint256) {
        // when none minted: (1 - 1) + (1158 - 1158) + (1306 - 1306) = 0
        // when all minted: (1158 - 1) + (1306 - 1158) + (1556 - 1306) = 1555
        return (nextTicket - 1) + (nextPublic - 1158) + (nextTreasury - 1306);
    }
