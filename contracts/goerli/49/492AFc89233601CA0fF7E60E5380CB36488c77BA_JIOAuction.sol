...
error EarningsAlreadyWithdrawn();
error CannotWithdrawWithoutFinalPrice();
...
contract JIOAuction is Ownable, ReentrancyGuard {
  ...
  event RefundClaimed(address indexed _from, uint256 _value);

  function setAuctionSupply(uint256 amount) external onlyOwner {
    auctionSupply = amount;
  }
  ...
  function withdrawFromSupply() external onlyOwner nonReentrant {
    if (earningsWithdrawn) revert EarningsAlreadyWithdrawn();
    if (block.timestamp < auctionEndTime) revert AuctionHasNotEnded();
    if (!bidWinnersSet) revert BidWinnersNotSet();
    if (finalMintPrice == 0) revert CannotWithdrawWithoutFinalPrice();
    uint256 amount = finalMintPrice * auctionSupply;
    if (address(this).balance < amount) revert InsufficientBalanceToWithdraw();
    payable(msg.sender).transfer(amount);
    earningsWithdrawn = true; 
  }

  function manualWithdraw(address walletAddress, uint256 amount) external onlyOwner nonReentrant {
    if (address(this).balance < amount) revert InsufficientBalanceToWithdraw();
  ...
  function claimRefund() external nonReentrant {
    ...
    emit RefundClaimed(msg.sender, bids[msg.sender] - (bidWinnersMints[msg.sender] * finalMintPrice));
  }

  function getBiddedAddresses() public view returns (address[] memory) {
    return biddedAddresses;
  }

  function getAddressBids() external view returns (address[] memory, uint256[] memory) {
    uint256[] memory addressBids = new uint256[](biddedAddresses.length);
    for (uint256 i = 0; i < biddedAddresses.length; i++) {
      addressBids[i] = bids[biddedAddresses[i]];
      i++;
    }
    return (biddedAddresses, addressBids);
  }
