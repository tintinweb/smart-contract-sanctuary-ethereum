...
// File: https://github.com/OpenZeppelin/openzeppelin-contracts-upgradeable/blob/v4.5.1/contracts/utils/CountersUpgradeable.sol
...
// File: https://github.com/OpenZeppelin/openzeppelin-contracts-upgradeable/blob/v4.5.1/contracts/token/ERC721/IERC721ReceiverUpgradeable.sol
...
// File: https://github.com/OpenZeppelin/openzeppelin-contracts-upgradeable/blob/v4.5.1/contracts/token/ERC721/extensions/IERC721MetadataUpgradeable.sol
...
// File: https://github.com/OpenZeppelin/openzeppelin-contracts-upgradeable/blob/v4.5.1/contracts/token/ERC721/ERC721Upgradeable.sol
...
// File: https://github.com/OpenZeppelin/openzeppelin-contracts-upgradeable/blob/v4.5.1/contracts/token/ERC721/extensions/ERC721EnumerableUpgradeable.sol
...
// File: contracts/tokens/CryptoPageNFT.sol
...
contract PageNFT is OwnableUpgradeable, ERC721EnumerableUpgradeable, IPageNFT {
    ...
    string constant _name = "Crypto.Page NFT";
    string constant _symbol = "PAGE.NFT";
    string private _baseTokenURI;

    struct TransferApprove {
        bool from;
        bool to;
    }

    //tokenId -> fromTo -> TransferApprove
    mapping(uint256 => mapping(address => TransferApprove)) private approvals;
     ...
     */
    function mint(address _owner, string memory ipfsHash) external override onlyCommunity returns (uint256) {
        require(_owner != address(0), "Address can't be null");
        return _mint(_owner, ipfsHash);
    }
    ...
    function burn(uint256 tokenId) external override onlyCommunity {
        ...
        address fromToHash = address(uint160(uint256(keccak256(abi.encodePacked(owner, address(0))))));
        TransferApprove storage approve = approvals[tokenId][fromToHash];
        approve.from = true;
        approve.to = true;
    ...
    /**
     * @dev Returns tokenURI for token ID.
     *
     * @param tokenId Id of token
     */
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        ...
        string memory baseURI = _baseURI();
        return bytes(baseURI).length > 0 ? string(abi.encodePacked(baseURI, ipfsHashes[tokenId])) : "";
    }

    /**
     * @dev Returns an ipfsHash of for token ID.
     *
     * @param tokenId Id of token
     */
    function ipfsHashOf(uint256 tokenId) external override view returns (string memory ipfsHash) {
        ipfsHash = ipfsHashes[tokenId];
    }
    ...
    /**
     * @dev Approves transfer of token.
     *
     * @param from Address of token sender
     * @param to Address of token receiver
     * @param tokenId Id of token
     */
    function approveTransfer(address from, address to, uint256 tokenId) external override {
        require(from != address(0), "Address can't be null");
        require(to != address(0), "Address can't be null");

        require(_msgSender() == to || _msgSender() == from, "Can't approve others transfers");

        address fromToHash = address(uint160(uint256(keccak256(abi.encodePacked(from, to)))));
    ...
    /**
     * @dev Disapproves transfer of token.
     *
     * @param from Address of token sender
     * @param to Address of token receiver
     * @param tokenId Id of token
     */
    function disapproveTransfer(address from, address to, uint256 tokenId) external override {
        require(from != address(0), "Address can't be null");
        require(to != address(0), "Address can't be null");
        ...
        internal virtual override
    {
        ...
        require(approve.from && approve.to, "Transfer not approved");
    }
     ...
     */
    function _mint(address _owner, string memory ipfsHash) private returns (uint256) {
        uint256 tokenId = _tokenIdCounter.current();

        address fromToHash = address(uint160(uint256(keccak256(abi.encodePacked(address(0), _owner)))));
        TransferApprove storage approve = approvals[tokenId][fromToHash];
        approve.from = true;
        approve.to = true;
    ...
    /**
     * @dev Voting transfer of NFT token.
     *
     * @param from Address of token sender
     * @param to Address of token receiver
     * @param tokenId Id of token
     */
    function transferVoting(address from, address to, uint256 tokenId) external override onlyCommunity {
        require(from != address(0), "Address can't be null");
        require(to != address(0), "Address can't be null");
