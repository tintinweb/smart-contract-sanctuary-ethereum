...
import { DexWallet, IEntryPoint } from "./DexWallet.sol";

/**
 * A sample factory contract for DexWallet
 * A UserOperations "initCode" holds the address of the factory, and a method call (to createAccount, in this sample factory).
 * The factory's createAccount returns the target account address even if it is already installed.
 * This way, the entryPoint.getSenderAddress() can be called either before or after the account is created.
 */
contract DexWalletFactory {
    DexWallet public immutable accountImplementation;

    constructor(IEntryPoint _entryPoint) {
        accountImplementation = new DexWallet(_entryPoint);
    }
     ...
     */
    function createAccount(address owner,uint256 salt) public returns (DexWallet ret) {
        address addr = getAddress(owner, salt);
        uint codeSize = addr.code.length;
        if (codeSize > 0) {
            return DexWallet(payable(addr));
        }
        ret = DexWallet(payable(new ERC1967Proxy{salt : bytes32(salt)}(
                address(accountImplementation),
                abi.encodeCall(DexWallet.initialize, (owner))
            )));
    }
