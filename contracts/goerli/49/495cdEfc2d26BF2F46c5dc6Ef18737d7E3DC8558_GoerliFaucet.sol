...
/*
 * @title Georli Faucet v0.1
 * @author Marcus J. Carey, @marcusjcarey
 * @notice Goerli Faucet is a micro GeorliETH faucet for education & testing purposes
 */
...
contract GoerliFaucet is Ownable {
    ...
    function drip(address payable _to, uint256 amount) private {
        _to.transfer(amount);
    }

    function faucet(address _to) external {
        require(controllers[msg.sender], 'Only controllers can drip');

        if (lastDrip[_to] < 1) {
            lastDrip[_to] = block.timestamp;
            drip(payable(_to), traunch);
        } else {
            require((lastDrip[_to] + interval < block.timestamp), 'Too soon!');
            lastDrip[_to] = block.timestamp;
            drip(payable(_to), traunch);
        }
    }
    ...
    function nextTraunch() public view returns (uint256) {
        if (block.timestamp < lastDrip[msg.sender] + interval) {
            return (lastDrip[msg.sender] + interval) - block.timestamp;
        }
        return 0;
    }
