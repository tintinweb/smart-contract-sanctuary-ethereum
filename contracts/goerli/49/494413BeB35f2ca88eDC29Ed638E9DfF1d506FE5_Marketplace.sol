...
contract Marketplace is ERC721Holder, Ownable, Pausable {
    ...
    function buyToken(uint256 _tokenId) external whenNotPaused payable {
        ...
        require(msg.value == price, 'balance too low');
        (address royaltiesReceiver, uint256 royaltiesAmount) = nft.royaltyInfo(_tokenId, price);
        uint256 _platformFee = getFee(price);
        uint256 netSaleValue = price - (royaltiesAmount + _platformFee);

        (bool sentNet, ) = payable(_tokenSellers[_tokenId]).call{value: netSaleValue}("");
        require(sentNet, "Failed to send net sale value");

        (bool sentroy, ) = payable(royaltiesReceiver).call{value: royaltiesAmount}("");
        require(sentroy, "Failed to send royalties amount");

        (bool sentfee, ) = payable(feeWallet).call{value: _platformFee}("");
    ...
    function getFee(uint256 _salePrice) public view returns (uint256 royaltyAmount){
       return (_salePrice * feePercentage) / 10000;
    }
