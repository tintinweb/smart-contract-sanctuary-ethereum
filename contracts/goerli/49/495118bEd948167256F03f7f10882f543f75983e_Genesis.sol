    ...
    IGenesis
{
    ...
    function initialize(
        ...
        genesisSupply = [4004, 1400, 140, 10];
     ...
     *  @dev    Only owner can call this function.
     */
    function setContracts(address _randomizer) external onlyAdmin {
     ...
     */
    function setMaxPerUser(uint256 _maxPerUser) external onlyAdmin notZeroAmount(_maxPerUser) {
     ...
     */
    function setIsLimitPerUser(bool _isLimitPerUser) external onlyAdmin {
    ...
    function _randomTypeId(uint256 seed) private view returns (TypeId) {
        ...
        for (uint256 i = 0; i < genesisSupply.length; i++) {
            if (currentIndexes[TypeId(i)] < getMaxSupplyOf(TypeId(i))) {
                return TypeId(i);
            }
        }
