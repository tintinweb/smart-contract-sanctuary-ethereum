...
contract VeToken is SafeOwnable, TransferHelper, ReentrancyGuard {

  uint public constant MIN_LOCK_PERIOD = 1 weeks;
  uint public constant MAX_LOCK_PERIOD = 1460 days; // 4 years
  uint public constant WITHDRAW_DELAY  = 1 days;
  ...
  function lock(uint _amount, uint _newLockedUntil) external nonReentrant {
    ...
    if (_amount > 0) {
      ...
      totalLocked += _amount;
      _safeTransferFrom(lockedToken, msg.sender, _amount);
    }
  ...
  function withdraw() external nonReentrant {
    ...
    _safeTransfer(lockedToken, msg.sender, withdrawAmount);
    _checkReserves();
  ...
  function addIncome(uint _addAmount) external onlyOwner {
    _accrue();
    _safeTransferFrom(lockedToken, msg.sender, _addAmount);
  ...
  function _claim() internal {
    ...
    if(pendingReward > 0) {
      _safeTransfer(lockedToken, msg.sender, pendingReward);
    }
