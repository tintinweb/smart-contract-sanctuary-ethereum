...
contract Marketplace  is Initializable, UUPSUpgradeable, ERC1155HolderUpgradeable ,ReentrancyGuardUpgradeable{
    ...
    event FixedPriceListing(uint256 tokenId , uint256 nftCopies , uint256 listingID  , address nftAddress , address nftListerOwner);
    event AuctionListing(uint256 tokenId , uint256 nftCopies , uint256 ListingID , uint256 auctionStartTime , uint256 auctionEndTime );
    event FixedPriceSelling(uint256 tokenId , uint256 listingID , uint256 buyingPrice , address buyer);
    event BiddingDetails(uint256 tokenid , address HighestBidderWinner , uint256 HighesBidAmount);
    event auctionEndDetail(uint256 tokenId , uint256 nftCopies ,address nftListerOwner , address highestBidderWinner);
    event claimNFT(uint256 tokenId , uint256 nftCopies ,  address NFTclaimer);
    ...
    // This Function is Setting PlatFormServiceFee Only Owner can do this
    ...
    // This Function Is Calculating Platform Fee The Onwer Sets
    ...
    // This Function is Calculating Royalty Fee For The User
    ...
    // This Function Is Listing NFT For Fixed Point Selling
    ...
    function listItemForFixedPrice(uint256 _tokenId, uint256 _amount , uint256 _price , uint256 _royaltyFee, address _nftAddress , address _royaltyRec)public  OnlyTokenHolders(_tokenId , _nftAddress) returns(uint256)
    {
        ...
        Fixedprices.push(
        FixedPrice(false,true,msg.sender,address(0),_nftAddress, _royaltyRec,_price,0,newItemId,_tokenId,_royaltyFee,_amount));

        IERC1155Upgradeable(_nftAddress).safeTransferFrom(msg.sender,address(this), _tokenId, _amount, "0x00");    

        emit FixedPriceListing
        (
            Fixedprices[_tokenId].tokenid ,
            Fixedprices[_tokenId].totalcopies,
            Fixedprices[_tokenId].fixedid , 
            Fixedprices[_tokenId].nftAddress,
            Fixedprices[_tokenId].owner
        );
        return newItemId;   

    }

    // This Function is Used For Buying Fixed Price Selling Item

    function BuyFixedPriceItem(uint256 Id) payable public nonReentrant() ItemExists(Id) IsForSale(Id)  returns(bool)
    {
        ...
        IERC1155Upgradeable(Fixedprices[Id].nftAddress).safeTransferFrom(address(this),Fixedprices[Id].newowner,Fixedprices[Id].tokenid,Fixedprices[Id].totalcopies,'0x00');
        ...
        emit FixedPriceSelling
        (
            Fixedprices[Id].tokenid,
            Fixedprices[Id].fixedid,
            Fixedprices[Id].paid,
            Fixedprices[Id].newowner
        );
        return true;
    }

    // This Function Is Listing Item For Auction 

    function listItemForAuction(uint256 _biddingStartTime, uint256 _biddingendtime , uint256 _royaltyFee , uint256 tokenId, uint256 _numberofcopies , address _nftAddress , address _royaltyRecipient ,
    uint256 _initialPrice) 
        public  
        OnlyTokenHolders(tokenId , _nftAddress) returns(uint256)
    {
        uint256 newauctionid = auctions.length;
        auctions.push(Auction
        (false,true,_royaltyFee,newauctionid,tokenId,_numberofcopies,_biddingendtime,_biddingStartTime,0,address(0) , _nftAddress,msg.sender, _royaltyRecipient,_initialPrice));
        IERC1155Upgradeable(_nftAddress).safeTransferFrom(msg.sender,address(this),auctions[tokenId].tokenId,auctions[tokenId].numberofcopies,'0x00');
     
        emit AuctionListing
        (
            auctions[tokenId].tokenId,
            auctions[tokenId].numberofcopies,
            auctions[tokenId].auctionid,
            auctions[tokenId].auctionStartTime,
            auctions[tokenId].auctionEndTime
        );
        return newauctionid;
    }

    // This Function Is Used For Placing Bidding On Auction Item 
    ...
    // This Function Is Used For Auction End For Auction Item By Seller 
    ...
    function auctionEnd(uint256 Id) public nonReentrant()
    {
        ...
        IERC1155Upgradeable(auctions[Id].nftAddress).safeTransferFrom(address(this),highestBidder,auctions[Id].tokenId,auctions[Id].numberofcopies,'0x00');
        auctions[Id].isSold = true;

        }

        emit auctionEndDetail
        (
            auctions[Id].tokenId,
            auctions[Id].numberofcopies , 
            auctions[Id].nftOwner , 
            msg.sender
        );
    }

    // This Function Is Used For Claim Winning NFT By Buyer
    ...
    function claimNft(uint256 Id) public nonReentrant() returns(bool) 
    {
        ...
        IERC1155Upgradeable(auctions[Id].nftAddress).safeTransferFrom(address(this),msg.sender,auctions[Id].tokenId,auctions[Id].numberofcopies,'0x00');
        auctions[Id].isSold = true;
