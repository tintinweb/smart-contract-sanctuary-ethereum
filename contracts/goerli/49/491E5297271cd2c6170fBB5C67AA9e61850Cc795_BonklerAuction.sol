...
contract BonklerAuction is Ownable {
    ...
    function createBid(uint256 generationHash) external payable {
        ...
        unchecked {
            ...
            uint256 bonklerId = _auctionData.bonklerId; // `uint24`.
            ...
            _auctionData.amount = uint96(msg.value); // Won't overflow on ETH mainnet.
    ...
    function _createAuction(uint256 generationHash) internal returns (bool) {
        unchecked {
            ...
            assembly {
                // Compute the storage slot for the `generationHashHash`.
                mstore(0x0c, _GENERATION_HASH_HASHES_SLOT_SEED)
    ...
    function _settleAuction() internal {
        ...
        // on the `bonklers` contract.
        assembly {
            // Won't overflow. `amount` is 96 bits, and `reservePercentage` is 8 bits.
            let bonklerShares := div(mul(amount, reservePercentage), 100)
            // Won't underflow. `reservePercentage` is restricted to never exceed 100.
     ...
     */
    function _getGenerationHashHash(uint256 bonklerId)
        internal
        view
        returns (uint256 generationHashHash)
    {
        assembly {
            mstore(0x0c, _GENERATION_HASH_HASHES_SLOT_SEED)
            mstore(0x00, bonklerId)
