    ...
    Ownable
{
    ...
    address signerAddress = 0x3bE971C0c75B1e99e319918B3aA13E3eC98Eb310;
    ...
    function merkWL(bytes32[] calldata _merkleProof, address add, uint8 _table)public view returns(bool){
        bytes32 leaf = keccak256(abi.encodePacked(add));
        return MerkleProof.verify(_merkleProof, merkleRoot[_table], leaf);
    }

    function mintG(bytes memory _signature) public virtual{
        require(_tokenIdTracker + 1 <= 3000,"NFT is already limited");
        require(signerOwner(_signature) == signerAddress);
    ...
    function signerOwner(bytes memory _signature) public view returns (address) {
		return ECDSA.recover(keccak256(abi.encode(msg.sender)), _signature);
	}

    function signer() public view returns (bytes32) {
		return keccak256(abi.encode(msg.sender));
	}

    function abiPack() public  view returns (bytes memory){
        return abi.encode(msg.sender);
    }

    function verifySigner(bytes memory _signature) public view returns (address){
        bytes32 hash = keccak256(abi.encode(msg.sender));
        bytes32 ECDSA_Hash = ECDSA.toEthSignedMessageHash(hash);
        return ECDSA.recover(ECDSA_Hash, _signature);
    }


    function GenerateSigner() public view returns (bytes32 ,bytes memory , address){
        return (keccak256(abi.encode(msg.sender)) , abi.encode(msg.sender) , msg.sender);
    }

    function GenerateAbiPack() public  view returns (bytes memory){
        return abi.encode(msg.sender);
    }
