...
interface IPreferenceToken is IERC20Upgradeable {
    struct TokenInfo {
        string symbol;
        uint256 cap;
    }

    function initialize(address pool_, address preferenceTGE_, uint256 cap_) external;
...
import "./interfaces/IPreferenceToken.sol";
    ...
    IService
{
    ...
    /// @dev Preference token beacon
    address public preferenceTokenBeacon;

    /// @dev TGE beacon
    address public preferenceTGEBeacon;
     ...
     * @param preferenceTokenBeacon_ Preference token beacon
     * @param preferenceTGEBeacon_ Preference TGE beacon
    ...
    function initialize(
        ...
        require(preferenceTokenBeacon_ != address(0), ExceptionsLibrary.ADDRESS_ZERO);
        require(preferenceTGEBeacon_ != address(0), ExceptionsLibrary.ADDRESS_ZERO);
        ...
        preferenceTokenBeacon = preferenceTokenBeacon_;
        preferenceTGEBeacon = preferenceTGEBeacon_;
    ...
    function createPool(
        ...
        _dispatcher.addContractRecord(address(tge), IDispatcher.ContractType.TGE);
        ...
        token.initialize(address(pool), tokenInfo.symbol, tokenInfo.cap);
        pool.setToken(address(token));

        tge.initialize(address(token), tgeInfo);
     ...
     */
    function createSecondaryTGE(ITGE.TGEInfo calldata tgeInfo, bool isPreferenceTokenCreation)
        external
        override
        onlyPool
        nonReentrant
        whenNotPaused
    {
        ...
        if (!isPreferenceTokenCreation) {
            ...
            tge.initialize(address(IPool(msg.sender).token()), tgeInfo);
            IPool(msg.sender).addTGE(address(tge));
        } else {
            tge = ITGE(address(new BeaconProxy(preferenceTGEBeacon, "")));
            token = address(new BeaconProxy(preferenceTokenBeacon, ""));
            tge.initialize(token, tgeInfo);
            IPreferenceToken(token).initialize(msg.sender, address(tge), tgeInfo.hardcap);
            IPool(msg.sender).addPreferenceToken(token);
        }
        dispatcher.addContractRecord(address(tge), IDispatcher.ContractType.TGE);

        emit SecondaryTGECreated(msg.sender, address(tge), token);
    }
