...
/// @title AlToken
///
/// @dev This is the contract for the Derived utillity token usd.
///
contract AlToken is AccessControl, ERC20("Derived USD", "USDx") {
  ...
  modifier onlyWhitelisted() {
    require(whiteList[msg.sender], "USDx: Derived is not whitelisted");
    _;
  }
  ...
  function mint(address _recipient, uint256 _amount) external onlyWhitelisted {
    require(!blacklist[msg.sender], "USDx: Derived is blacklisted.");
    uint256 _total = _amount.add(hasMinted[msg.sender]);
    require(_total <= ceiling[msg.sender],"USDx: Derived's ceiling was breached.");
    require(!paused[msg.sender], "USDx: user is currently paused.");
