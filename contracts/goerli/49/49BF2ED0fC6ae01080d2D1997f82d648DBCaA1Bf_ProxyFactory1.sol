...
//
    function nowItWorks() public view returns (bytes memory) {
    ...
    function getDeployNew() public returns (bytes memory) {
        bytes memory bytecode = abi.encodePacked(type(LogicContract).creationCode, abi.encode(mainAddress));

        return bytecode;
    }

    function getDeployNewNop() public returns (bytes memory) {

        return type(LogicContract).creationCode;
    }

    function deployNew(bytes32 _salt) public {
        bytes memory bytecode = abi.encodePacked(type(LogicContract).creationCode, abi.encode(mainAddress));
