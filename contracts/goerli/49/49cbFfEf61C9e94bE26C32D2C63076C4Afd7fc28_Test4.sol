...
contract Test4 is ERC721A, Ownable, ReentrancyGuard {
  ...
  uint256 public MaxforWhitelisted = 5;
  ...
  // public
  function publicSaleMint(uint256 tokens ,bytes32[] calldata merkleProof) public payable nonReentrant {
    ...
    if (MerkleProof.verify(merkleProof, merkleRoot, keccak256(abi.encodePacked(msg.sender))) && _numberMinted(_msgSender()) + tokens > MaxforWhitelisted)
	revert maxPerWalletExceeded ();
    if (! MerkleProof.verify(merkleProof, merkleRoot, keccak256(abi.encodePacked(msg.sender))) && _numberMinted(_msgSender()) + tokens > MaxperWallet )
	revert maxPerWalletExceeded ();
