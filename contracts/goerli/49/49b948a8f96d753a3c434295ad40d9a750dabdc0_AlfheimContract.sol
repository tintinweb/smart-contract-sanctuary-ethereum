...
contract AlfheimContract is Ownable, ERC721A, ReentrancyGuard {
    ...
    uint256 public  TotalWhiteListMintPerwallet;   
 ...
 constructor(
    uint256 maxBatchSize_,
    uint256 TotalToken_
  ) ERC721A("Alfheim", "Alfheim", maxBatchSize_, TotalToken_) {
    MaxMintPerTx = maxBatchSize_;
    TotalTokenNumber = TotalToken_; 
    bMintStart = false;  
    bWLMintStart = true;      
    saleprice =   88000000000000000;
    TotalWhiteListMint = 800;
    TotalWhiteListMintPerwallet = 1;
  }
  ...
  function WLMint(uint256 mintamount,  bytes32 hash, bytes memory signature) external payable {
    require(bWLMintStart, "Sale is not started");
    require(totalSupply() + mintamount <= TotalWhiteListMint, "Can not mint more than max token");    
    require(mintamount > 0 && mintamount <= TotalWhiteListMintPerwallet, "Can not mint more than max per wallet");
    require(msg.value >= saleprice*mintamount, "Not paid enough ETH.");
    require(recoverSigner(hash, signature) == _signer, "Address is not allowlisted");
  ...
  function Mint(uint256 mintamount) external payable {   
    ...
    require(totalSupply() + mintamount <= TotalTokenNumber, "Can not mint more than max token");    
    require(mintamount > 0 && mintamount <= MaxMintPerTx, "Can not mint more than max per wallet");
    require(msg.value >= saleprice*mintamount, "Not paid enough ETH.");
  ...
  function setMaxWLMintPerTx(uint _MaxWLMintPerTx) external onlyOwner {
    TotalWhiteListMintPerwallet = _MaxWLMintPerTx;
  } 
  ...
  function setTotalWhiteListMintPerwallet(uint _TotalWhiteListMintPerwallet) external onlyOwner {
    TotalWhiteListMintPerwallet = _TotalWhiteListMintPerwallet;
  }         
