...
// TODO: extend Upgradable from OZ/contracts
contract MessagePassingGateway is ICCMPGateway {
    ...
    function sendMessage(uint256 chainID, string calldata adaptorName, bytes calldata message) external override returns (bool sent) {
        uint32 nonce = nonces[msg.sender] + 1;

        // CCMPMessage memory _message = _deserializeMessage(message);

        address adaptor = adaptors[adaptorName];
        // console.log("using %s adaptor at address %s", adaptorName, adaptor);
        ICCMPRouterAdaptor(adaptor).routePayload(messageIDs.length, message, nonce); //_message.payload.data, nonce);
        
        messages[messageIDs.length] = message;
        messageIDs.push(messageIDs.length);

        emit MessageSent(chainID, adaptorName, message, msg.sender);
        return true;
    }
