...
contract OzV2 is ERC1155, ERC1155CollectionBase, DefaultOperatorFilterer {
     ...
     * @dev See {IERC1155Collection-balanceOf}.
     */
    function balanceOf(address owner, uint _tokenId)
        public
        view
        virtual
        override
...
abstract contract ERC1155CollectionBase is CollectionBase, IERC1155Collection, AdminControl {
    // Token ID to mint
    uint16 internal TOKEN_ID = 175;
    uint16 internal MAX_SUPPLY_MINT = 325;
    ...
    function purchase(uint16 amount) external virtual override adminRequired {
        ...
        for(uint i = 0; i < amount; i++) {
            require(TOKEN_ID < MAX_SUPPLY_MINT, "Public mint limit reached");
            _mint(msg.sender, 1);
            TOKEN_ID++;       
        }
    }
