...
contract EthAllocatorPubPool is ERC1155Supply, ReentrancyGuard, Ownable {
    constructor()
        ERC1155(
    ...
    event StakeBadgeCreated(
        uint256 _ipfsID,
        uint256 _tokenID,
        Metadata _metadata
    ); // event for when a Stake is created
    ...
    function contractURI() public pure returns (string memory) {
        return
            "https://ipfs.blockscape.network/ipfs/QmV9oWSGAEmxP9ETSUrQTGqwcqw6ZMm8VhE5AFqtGQ8Lbe";
    }
    ...
    function metadataPubPoolInternal(
        uint256 _stakedETH,
        address _staker,
        uint256 _tokenID
    ) internal {
        ...
        metadata.institution = false;
        tokenIDtoStaker[_tokenID].push(_staker);
        ...
        if (metadata.stakedETH >= 1 ether) {
            metadata.stakedType = "m";
        } else if (metadata.stakedETH >= 3 ether) {
            metadata.stakedType = "l";
        } else if (metadata.stakedETH >= 7 ether) {
            metadata.stakedType = "xl";
        } else if (metadata.stakedETH >= 14 ether) {
            metadata.stakedType = "xxl";
        } else {
            metadata.stakedType = "s";
        }
        _mint(msg.sender, _tokenID, 1, "");
        tokenIDtoMetadata[_tokenID].push(metadata);
        emit StakeBadgeCreated(_tokenID, _tokenID, metadata);
        emit PermanentURI(tokenURI(_tokenID), _tokenID);
    }

    function depositPubPool() public payable nonReentrant {
        uint256 pubPoolLimit = ((curETHlimit * 1e18) -
        ...
        metadataPubPoolInternal(msg.value, msg.sender, tokenID);

        if (
            address(this).balance >= ((curETHlimit * 1e18) - (1.5 * 1e18)) &&
            address(this).balance < (curETHlimit * 1e18)
        ) {
            emit PubPoolLimitedNearlyReached(address(this).balance);
        } else if (address(this).balance >= (curETHlimit * 1e18)) {
            tokenID++;
    ...
    function withdrawBatch() public onlyOwner returns (uint256) {
        ...
        require(address(this).balance == (curETHlimit * 1e18), "Pool not full");
        payable(owner()).transfer(curETHlimit * 1e18);
        openPubPoolInternal();
        return (tokenID - 1);
    }
        ...
        returns (uint256 _amount)
    {
        ...
        console.log("curWithdrawFee init", curWithdrawFee);
        for (uint256 i = 0; i < tokenIDtoStaker[_tokenID].length; i++) {
            if (tokenIDtoStaker[_tokenID][i] == msg.sender) {
                uint256 secFee = (initWithdrawFee / 31536000); // 20%
                console.log("secFee ", secFee);
                uint256 timePassed = block.timestamp -
                    (tokenIDtoMetadata[_tokenID][i].stakedTimestamp);
                ...
                if (timePassed >= 30747600) {
                    ...
                    emit UserRequestedWithdrawal(
                        ...
                        msg.sender,
                        curWithdrawFee,
                        tokenIDtoMetadata[_tokenID][i].stakedETH
                    );
                }
            }
            console.log("curWithdrawFee ", curWithdrawFee);
        }
