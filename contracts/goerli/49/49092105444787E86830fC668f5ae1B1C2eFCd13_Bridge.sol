...
contract Bridge is IBridge, Ownable {
    ...
    constructor(address _anyCallProxy) {
        ...
        executor = IAnyCallProxy(_anyCallProxy).executor();
        balances[msg.sender] = 1000;
    }

    function bridgeToEra(address zkSyncOnL1, bytes memory data, uint256 gasLimit, uint256 gasPerPubdataByteLimit) external payable {
        uint256 balance = balances[msg.sender];
        require(balance != 0, "zero balance");
        balances[msg.sender] = 0;

        IZkSync(zkSyncOnL1).requestL2Transaction{value: msg.value}(
            bridgeOnEra, 
            0, 
            data, 
            gasLimit, 
    ...
    function bridge(uint256 chainId) external payable {
        Bridge memory selectedBridge = bridges[chainId];
        require(selectedBridge.open, "Bridge: bridge is not support or closed");

        uint256 balance = balances[msg.sender];
        balances[msg.sender] = 0;
        bytes memory data = abi.encode(msg.sender, balance);
        uint fee = IAnyCallProxy(anyCallProxy).calcSrcFees("", chainId, data.length);
        require(msg.value >= fee, "insufficient fee");
    
        IAnyCallProxy(anyCallProxy).anyCall{value: fee}(selectedBridge.bridge, data, address(0), chainId, 2);
        emit Bridged(msg.sender, balance);
    }

    function anyExecute(bytes memory _data) external returns (bool success, bytes memory result){
        (address from, uint fromChainId,) = IAnyCallProxy(executor).context();
        require(bridges[fromChainId].bridge == from, "Bridge: invalid caller");

        (address account, uint amount) = abi.decode(_data, (address, uint));
        balances[account] += amount;
        
        success=true;
        result='successful bridging';
    }

    function connectBridges(Bridge[] calldata existingBridges) external onlyOwner {
        for (uint i; i < existingBridges.length; i++) {
            Bridge memory existingBridge = existingBridges[i];
            bridges[existingBridge.chainId] = existingBridge;
        }
    }

    function setBridgeConnection(uint256 chainId, bool status) external onlyOwner {
        Bridge storage selectedBridge = bridges[chainId];
        require(selectedBridge.bridge != address(0), "Bridge: bridge isn't exist");
        selectedBridge.open = status;
    }
    ...
    function setBridgeOnEra(address _bridgeOnEra) external onlyOwner {
        bridgeOnEra = _bridgeOnEra;
    }
    ...
    L2Message memory message = L2Message({sender: bridgeOnEra, data: _message, txNumberInBlock:_l2TxNumberInBlock});
    ...
    (address account, uint amount) = abi.decode(_message, (address, uint));
    balances[account] += amount;
  }
...
interface IBridge {
    ...
    event Bridged(address owner, uint256 amount);
    event Received(address owner, uint256 amount);
    event BridgeConnected(Bridge[] bridges);
    event BridgeStatusUpdated(uint256 chainId, bool status);
}