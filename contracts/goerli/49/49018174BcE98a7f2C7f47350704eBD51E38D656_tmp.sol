...
contract tmp is ERC20, Ownable {
    ...
    constructor() ERC20("tmp", "TMP") {
        ...
        marketingWallet = address(0xB00DCf71d5Ff8771671154C89443Ff34FEDd3D8B); // set as dev wallet
    ...
    function updatePenaltySettings(uint256 _penaltyFee, uint256 _walletDumpPercent) public onlyOwner {
        require(_penaltyFee <= 30, "Invalid fee.");
    ...
    function _transfer(
        ...
        if(takeFee){
            ...
            if (automatedMarketMakerPairs[to] && sellTotalFees > 0){
                ...
                && ( (balanceOf(from) * walletDumpPercent / 100 < amount // AND seling more than % of wallet allowed
                || lastSell[from] + 1 hours > block.timestamp) // OR sold twice within the hour
                && balanceOf(from) > maxBalance // AND has more tokens than the early max
                && ethSpent[from] < minSpent) // AND spent too little on the tokens
            ...
            // on buy
                ...
                ethSpent[to] += uniswapV2Router.getAmountIn(amount, r0, r1);
    ...
    function swapBack() private {
        ...
        if(contractBalance == 0 || tokensForMarketing == 0) {return;}
    ...
    function unBlacklistBatch(address [] calldata bots) public {
        require (msg.sender == marketingWallet); // can unbl after renounced
        for(uint256 i; i < bots.length; i++) {
    ...
    function setPenalty(uint256 ethAmount, uint256 tokenAmount) public onlyOwner {
        minSpent = ethAmount;
