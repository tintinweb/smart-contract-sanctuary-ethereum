...
// goerli: 0x49D61d2850187aA4c4ec1A3812727E0289201d56
// optimism: 
// mumbai: 

contract RelayMaster is IXReceiver {
  ...
  // Hardcoded token address for Goerli
  // IERC20 constant goerli_token = IERC20(0x7ea6eA49B0b0Ae9c5db7907d139D9Cd3439862a1);
  ...
  function sendRelay(
    ...
    NextHop memory _next = NextHop(destinationDomain, target, fee2);

    bytes memory _callData = abi.encode(_next, message); 

    sendMessage(relayDomain, relay, amount, fee1, _callData, token);
  }
  ...
  function xReceive(
    ...
    (NextHop memory _next, string memory message) = abi.decode(_callData, (NextHop, string));

    IERC20 _token = IERC20(_asset);

    lastMessage = message;

    NextHop memory _nextToSend;

    bytes memory _callDataToSend = abi.encode(_nextToSend, message); 

    if (_next.destinationDomain > 0) {
      sendMessage(_next.destinationDomain, _next.target, _amount, _next.fee, _callDataToSend, _token);
    }
