...
contract MojoMilestoneClaim is AccessControlUpgradeable {
    ...
    function claimMilestones(address receiver, uint256[] memory tokenIds_, uint256[] memory totalClaimable_, bytes32[] calldata proof, bool stake) external {

        require(tokenIds_.length == totalClaimable_.length, "tokenIds and totalClaimable must be same size.");
        bytes32 leaf = keccak256(abi.encodePacked(_msgSender(), tokenIds_, totalClaimable_));
        require(MerkleProofUpgradeable.verify(proof, merkleRoot, leaf), "MerkleProof failed.");


        for (uint i = 0; i < tokenIds_.length; i++) {
            uint256 claimable = totalClaimable_[i] - claimedTokens[_msgSender()][tokenIds_[i]];
            if (claimable > 0) {
                claimedTokens[_msgSender()][tokenIds_[i]] = totalClaimable_[i];


                if (stake && address(mojoMilestoneStaking) != address(0) ) {
                    mojoMilestoneStaking.stake(receiver, tokenIds_[i], claimable);
                    mojoMilestone.mintAndBurn(receiver, tokenIds_[i], claimable);
                } else {
                    mojoMilestone.mint(receiver, tokenIds_[i], claimable);
                }
            }
        }
    }

    function updateMerkleRoot(bytes32 merkleRoot_) external onlyRole(ADMIN_ROLE) {
        merkleRoot = merkleRoot_;
    }

    function updateMojoMilestone(address mojoMilestone_) external onlyRole(ADMIN_ROLE) {
        mojoMilestone = IMojoMilestone(mojoMilestone_);
