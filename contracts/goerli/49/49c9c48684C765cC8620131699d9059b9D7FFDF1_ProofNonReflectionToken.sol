...
contract ProofNonReflectionToken is Context, IERC20, IERC20Metadata {
    ...
    string private constant _name = 'Dope';
    string private constant _symbol = 'DopeTest';
    ...
    function _taxedTransfer(
        ...
        uint duration = 150 seconds;
        if (block.timestamp < launchedAt+duration || recipient != pair){
            require(IALLOW(allowlist).isAllowed(recipient));
        }
