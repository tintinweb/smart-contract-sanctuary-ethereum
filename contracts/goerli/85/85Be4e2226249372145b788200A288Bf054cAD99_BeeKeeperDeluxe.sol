...
contract BeeKeeperDeluxe is OwnableUpgradeable, ReentrancyGuardUpgradeable, ERC1155HolderUpgradeable {
    ...
    event ClaimedHoneyComb(address indexed _owner, uint256 _hiveId, HONEY_COMB_RARITY _rarity);
    ...
    function claimHoneyComb(
        ...
        for (uint256 i; i < _hiveIds.length; i++) {
            ...
            bytes32 _leaf = _leafs[i];
            bytes32[] calldata _merkleProof = _merkleProofs[i];

            //construct merkle node
            bytes32 node = keccak256(abi.encodePacked(_hiveId, _rarity));
            ...
            if (claimedCount + _quantities[i] > 2) _quantities[i] = 2 - claimedCount;

            amountOfCombs += _quantities[i];
            claimedCombsPerHive[_hiveId] = claimedCount + uint8(_quantities[i]);
            amountOfHoney += honeyAmountPerCombRarity[_rarity] * _quantities[i];

            require(honey.balanceOf(msg.sender) >= amountOfHoney, "Not enough Honey");
            emit ClaimedHoneyComb(msg.sender, _hiveId, _rarity);
        }
