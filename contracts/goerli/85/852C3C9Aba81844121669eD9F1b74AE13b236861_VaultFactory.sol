...
interface IVault {
    ...
    function withdraw(uint256 tokenId, uint256 shares, address receiver) external returns (uint256 assets);
...
contract Vault is Initializable, IVault {
    ...
    uint256 private constant NOMINATOR = 1e12;
    int256 private constant DENOMINATOR = 1e12;
    ...
    function _pendingTPI(
        ...
        return ((shares * rewardPerShare).toInt256() / DENOMINATOR - rewardDebtByID[tokenId]).toUint256();
    }

    function _sentTPI(uint256 tokenId, address receiver,  uint256 shares, uint256 rewardPerShare) internal {
    ...
    function _getComptroller() internal view returns (IComptroller) {
        return _getGNFT().TONPOUND_COMPTROLLER();
    }

    function _getTPI() internal view returns (ITPIToken) {
        return vaultFactory.TPI();
    }

    function _getGNFT() internal view returns (IgNFT) {
        return vaultFactory.gNFT();
    }

    modifier onlyFactory() {
        if (msg.sender != address(vaultFactory)) revert Auth();
        _;
    }
...
contract VaultFactory is UUPSUpgradeable, IVaultFactory {
    using EnumerableSetUpgradeable for EnumerableSetUpgradeable.AddressSet;

    IgNFT public gNFT;
    ITPIToken public TPI;
    ...
    function initialize(address gnft_) external initializer {
        if (gnft_ == address(0)) revert ZeroAddress();
        gNFT = IgNFT(gnft_);
        TPI = ITPIToken(IgNFT(gnft_).TONPOUND_COMPTROLLER().getCompAddress());
    }
    ...
    function lock(
        ...
        uint256 assets,
        address sender
    ) external onlyGNFT returns (uint256) {
    ...
    function _getVault(address market) internal returns (IVault) {
        IVault vault;
        if (market == address(TPI)) revert NotSupported();
        if (_markets.add(market)) {
            address implementation = vaultImplementation;
            address instance = ClonesUpgradeable.cloneDeterministic(implementation, keccak256(abi.encode(market)));
            vault = IVault(instance);
            vault.initialize(market);
            vaultsByMarket[market] = vault;

            emit MarketAdded(market, instance, implementation);
        } else {
    ...
    modifier onlyGNFT() {
        if (msg.sender != address(gNFT)) revert Auth();
        _;
    }
    ...
    modifier onlyAdmin() {
        if (!IAccessControlUpgradeable(address(gNFT)).hasRole(DEFAULT_ADMIN_ROLE, msg.sender)) {
            revert Auth();
        }
        _;
    }
