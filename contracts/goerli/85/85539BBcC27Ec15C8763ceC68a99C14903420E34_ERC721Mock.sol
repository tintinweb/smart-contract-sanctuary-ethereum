...
contract ERC721Mock is Ownable, ERC721Enumerable {
    using SafeMath for uint256;

    address constant ZERO = 0x0000000000000000000000000000000000000000;

    mapping(uint=>string) public nftURI;
    string public nftURIDefault;
    ...
    function _isEmtpy(string memory str) internal pure returns (bool) {
        bytes memory isEmptyTest = bytes(str);
        return isEmptyTest.length == 0;
    }
    function tokenURI(uint256 tokenId) public view override returns (string memory) {
        require(_exists(tokenId));
        string memory _uri = nftURI[tokenId];
        return _isEmtpy(_uri) ? nftURIDefault:_uri;
    }
    function setTokenURI(uint256 tokenId, string memory tokenUri) public {
        require(_exists(tokenId));
        nftURI[tokenId] = tokenUri;
    }
