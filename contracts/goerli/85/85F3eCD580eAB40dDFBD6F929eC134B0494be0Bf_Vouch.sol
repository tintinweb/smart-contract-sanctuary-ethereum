...
import "./Descriptors/NFTDescriptor.sol";
...
contract Vouch is 
    ERC721S("Vouch NFT", "VOUCH"), NFTDescriptor

 {
    using BokkyPooBahsDateTimeLibrary for uint;

    bytes32 public constant VOUCH_ID = keccak256("VOUCH");
    bytes32 public constant SOCIAL_ID = keccak256("SOCIAL");
    bytes16 private constant _SYMBOLS = "0123456789abcdef";
    uint8 private constant _ADDRESS_LENGTH = 20;
    address public ensRegistry = 0x00000000000C2E074eC69A0dFb2997BA6C7d2e1e;
    address public ensResolver = 0x19c2d5D0f035563344dBB7bE5fD09c8dad62b001;
    uint256 public tokenIds;
    uint256 public attestIds;
    ...
    /// @notice Mapping from tokenId => vouch info.
    mapping (uint256 => VouchInfo) public vouchInfo;
    ...
    /// @notice Mapping from user => user attestations.
    mapping (address => string[] ) public userAttest;
    ...
    function _vouch(address recipient, string calldata message, address sender) private returns (bool success) {
        ...
        userInfo[sender].push(tokenId);
        vouchInfo[tokenId] = VouchInfo(hashed, sender, uint48(block.timestamp));
        vouchMsg[hashed] = message;
        _mint(recipient, tokenId);
        return true;
    }

    // function burnVouch (address vouchId) public returns (bool success) {}

    /// @notice Return user vouches
    function getOwnerVouches(address _owner) public view returns (uint256[] memory) {
    ...
    function doesVouch(address sender, address recipient) public view returns (bool) {
        uint256 count = balanceOf(recipient);
        uint256[] memory vouches = getOwnerVouches(recipient);
        for(uint i = 0; i < count; i++) {
            if (vouchInfo[vouches[i]].sender == sender) {
                return true;
            }
        }
        return false;
    ...
    /// @notice Generates Social proof
    function generateSocialHash(string calldata socialId) public view returns (bytes32 hash) {
        hash = keccak256(abi.encode(address(this), SOCIAL_ID, socialId));
    }

    /// @notice Sign a message attesting to a message
    function attest(string calldata message) external returns (bool success){
        uint256 attestId = attestIds++;
        userAttest[msg.sender].push(message);
        return true;
    }

    /// @notice Return user attestations
    function getAttest(address _owner) public view returns (string[] memory) {
        return userAttest[_owner];
    }

    /// @notice Returns true if sender has attested to a given message
    function verifyAttestation (address sender, string memory message) public view returns (bool success){
        uint256 count = userAttest[sender].length;
        for(uint i = 0; i < count; i++) {
            if (keccak256(abi.encode(userAttest[sender][i])) == keccak256(abi.encode(message))) {
                return true;
            }
        }
        return false;   
    ...
    function dataURI(uint256 tokenId) public view returns (string memory) {
        ...
        string memory image = string(abi.encodePacked('data:image/svg+xml;base64,', Base64.encode(bytes(_vouchImage(tokenId)))));
     ...
     */
    function _vouchImage(uint _tokenId) internal view returns (string memory output) {
        ...
        output = string(abi.encodePacked(output, timestampToString(uint256(vouchInfo[_tokenId].lastUpdateTime)), '</text><text x="20" y="80" class="base">'));
        output = string(abi.encodePacked(output, vouchMsg[vouchInfo[_tokenId].hash], '</text><text x="20" y="265" class="base">from: </text><text x="20" y="290" class="base">'));
        output = string(abi.encodePacked(output, toHexString(vouchInfo[_tokenId].sender), '</text><text x="240" y="330" class="base">vouch # ' ,toString(_tokenId) , '</text></svg>'));
    }
    ...
    function generateAttributesList(uint256 tokenId) public view returns (string memory) {
        return string(
            abi.encodePacked(
                '{"trait_type":"Vouch","value":"', vouchMsg[vouchInfo[tokenId].hash],'"},',
