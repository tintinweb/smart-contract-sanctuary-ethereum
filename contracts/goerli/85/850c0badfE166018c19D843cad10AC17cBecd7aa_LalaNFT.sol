  ...
  RoyaltiesV2
{
  ...
  mapping(address => uint256) internal _revenueDeposit; // map from payee -> revenue
  ...
  event RevenueDistributed(uint256 amount, PaymentStrategy paymentStrategy);
  event RevenueWithdrawn(address indexed payee, uint256 amount);
  ...
  constructor(
    ...
    address distributioncoin_address,
    ...
    _totalShares = 0;
    _soldShares = 0;
    _distribution_coin = IERC20(distributioncoin_address);
  ...
  function redeem(
    ...
    for (uint256 i = 0; i < voucher.royalties.length; ++i) {
      ...
      (bool sent, ) = voucher.royalties[i].account.call{
        value: (msg.value * voucher.royalties[i].value) / 10000
      }('');
    ...
    (bool fullAmountSent, ) = _merchant.call{
      value: ((10000 - royaltiesSum) * msg.value) / 10000
    }('');
  ...
  // Return revenue royalties for a payee
  function revenueOf(address payee) public view returns (uint256) {
    return _revenueDeposit[payee];
  }

  // The allowance will be for a part of the full amount and we will only use that
  // part of the full amount
  function distributeRevenue(uint256 amount, PaymentStrategy paymentStrategy)
    public
  {
    require(amount > 0, 'You need to distribute at least some tokens');
    require(_totalShares > 0, "There aren't any shares to distribute");

    uint256 amountToDistribute = (_soldShares * amount) / 10000;
    // we require allowance for atleast the percentage of sold shares
    uint256 allowance = _distribution_coin.allowance(msg.sender, address(this));
    ...
    for (uint256 i = 0; i < _totalShares; i++) {
      ...
      if (paymentStrategy == PaymentStrategy.PUSH_PAYMENT) {
        // Push payment
        _distribution_coin.transfer(shareholder, shareAmount);
      } else {
        // Pull payment
        _revenueDeposit[shareholder] += shareAmount;
      }
    }

    emit RevenueDistributed(amount, paymentStrategy);
  }

  // Withdraw revenue for a payee, forwarding all gas to the recipient
  function withdrawRevenue(address payable payee) public {
    uint256 amount = _revenueDeposit[payee];

    _revenueDeposit[payee] = 0;

    _distribution_coin.transfer(payee, amount);

    emit RevenueWithdrawn(payee, amount);
  }
