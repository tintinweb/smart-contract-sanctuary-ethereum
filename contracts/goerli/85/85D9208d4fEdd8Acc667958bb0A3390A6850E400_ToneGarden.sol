...
contract ToneGarden is ERC721, Ownable {
    ...
    function allowListMint(address _vault, uint256 amount, uint256 allowance, bytes32[] calldata proof) public payable {
        require(phase1, "phase 1 not enabled");
        require(amount * ALLOW_LIST_PRICE == msg.value, "Insufficient funds.");
        require(amount < MAX_ALLOW_LIST_MINT, "amount is too high");
        ...
        require(_verify(_leaf(Strings.toString(allowance), payload), proof, allowList), "invalid proof");
        require(mints[requester] + amount <= allowance, "exceeds allowed mints");
        require(_totalSupply.current() + amount <= MAX_SUPPLY, "exceeds max supply");
        mints[requester] += amount;
