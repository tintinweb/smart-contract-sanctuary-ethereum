...
import {IERC1155BindableMintable} from "./interfaces/IERC1155BindableMintable.sol";
import {IERC721BindableMintable} from "./interfaces/IERC721BindableMintable.sol";
...
contract DopamineRegistrarController is IDopamineRegistrarController {

    bytes32 public constant REGISTER_TYPEHASH = keccak256("Register(bytes32 chip,address registrant)");

    address public owner;

    IDopamineRegistrar public immutable registrar;
    ...
    /// @inheritdoc IDopamineRegistrarController
	function registerAndBundle(
		...
		registrar.register(chip, registrant, resolver, perms);

		if (data.length > 0) {
            IDopamineResolver(resolver).multicall(chip, data);
		}
        ...
        for (uint256 i = 0; i < bindables.length; i++) {
            ...
            bindable = bindables[i];

            if (bindable.token.code.length == 0) {
                revert BindableInvalid(bindable.token);
            }

            if (bindable.bindableType == BindableType.ERC721) {
                IERC721BindableMintable(bindable.token).mint(bindable.id, uint256(chip), address(registrar));
            } else if (bindable.bindableType == BindableType.ERC1155) {
                IERC1155BindableMintable(bindable.token).mint(bindable.id, bindable.amount, uint256(chip), address(registrar));
            } else {
                revert BindableTypeInvalid(bindable.token);
            }
        }
    }
    ...
    function _buildDomainSeparator() internal view returns (bytes32) {
        return keccak256(
            abi.encode(
                ...
                keccak256(bytes("Dopamine Registrar Controller")),
                keccak256("1"),
                block.chainid,
                address(this)
            )
...
import {IERC1155Bindable} from "./IERC1155Bindable.sol";
...
interface IERC1155BindableMintable is IERC1155Bindable {

    /// @notice Mints `amount` bindables of type `tokenId`, binding them to the
    ///  asset at `bindAddress` identified by `bindId`.
	/// @param tokenId The identifier of the token type being minted.
    /// @param amount The number of tokens of type `tokenId` being minted.
	/// @param bindId The identifier of the asset being bound to.
	/// @param bindAddress The contract address of the asset being bound to.
    function mint(
        uint256 tokenId,
        uint256 amount,
        uint256 bindId,
...
interface IERC721BindableMintable is IERC721Bindable {

    /// @notice Mints `amount` bindables of type `tokenId`, binding them to the
    ///  asset at `bindAddress` identified by `bindId`.
    /// @param tokenId The identifier of the NFT being minted.
    /// @param bindId The identifier of the asset being bound to.
    /// @param bindAddress The contract address of the asset being bound to.
    function mint(
        uint256 tokenId,
