...
contract SynthrBridge is BaseBridge {
    ...
    event REAppend(address account, uint amount, uint16 srcChainId);
    event Vest(address indexed account, uint amount, uint16 srcChainId);
    ...
    function _nonblockingLzReceive(
        ...
        if (packetType == PT_SYNTH_MINT) {
            ...
            (, bytes memory accountBytes, bytes32 synthKey, uint synthAmount, uint debtShare, bool burnToTarget) = abi.decode(_payload, (uint16, bytes, bytes32, uint, uint, bool));
            ...
            (, bytes memory accountBytes, uint amount, bool isFromFeePool) = abi.decode(_payload, (uint16, bytes, uint, bool));
            address account = accountBytes.toAddress(0);
            _sendREAppend(account, amount, isFromFeePool);
            emit REAppend(account, amount, _srcChainId);
        } else if (packetType == PT_REWARD_ESCROW_VEST) {
            (, bytes memory accountBytes, uint amount) = abi.decode(_payload, (uint16, bytes, uint));
            address account = accountBytes.toAddress(0);
            _sendVest(account, amount);
            emit Vest(account, amount, _srcChainId);
        } else {
    ...
    function sendBurn(
        ...
        bytes memory lzPayload = abi.encode(PT_SYNTH_BURN, abi.encodePacked(accountForSynth), synthKey, synthAmount, debtShare, burnToTarget);
    ...
    function sendREAppend(address account, uint amount, bool isFromFeePool) external payable {
        _sendREAppend(account, amount, isFromFeePool);
        emit REAppend(account, amount, _selfLZChainId);

        bytes memory lzPayload = abi.encode(PT_REWARD_ESCROW_APPEND, abi.encodePacked(account), amount, isFromFeePool);
    ...
    function sendVest(
        ...
        _sendVest(account, amount);
        emit Vest(account, amount, _selfLZChainId);

        bytes memory lzPayload = abi.encode(PT_REWARD_ESCROW_VEST, abi.encodePacked(account), amount);
    ...
    function _sendREAppend(
        ...
        ISynthrAggregator(synthrAggregator).append(account, amount, isFromFeePool);
    }
    ...
    function _sendVest(
        ...
        ISynthrAggregator(synthrAggregator).vest(account, amount);
    }
