...
contract testERC721 is ERC721URIStorage, ERC2981, Ownable {
    /// @dev The token ID of the next token to mint.
    uint256 private _nextTokenId = 1;
    /// @dev The burned tokens counter.
    uint256 private _burned;

    constructor() ERC721("Test Arts", "TART") {}

    /// @notice Mint Angelo Art token.
    ...
    function burn(uint256 _tokenId) public {
        require(_isApprovedOrOwner(_msgSender(), _tokenId), "ANG: Not approved to burn.");
        _burn(_tokenId);
        unchecked {
            _burned++;
        }
