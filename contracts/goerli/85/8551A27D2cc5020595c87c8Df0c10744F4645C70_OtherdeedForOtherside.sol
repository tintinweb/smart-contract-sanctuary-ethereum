...
import "./Types.sol";
import "./UniversalLib.sol";

/// @notice Contract module that allows children to implement rollup finalization.
abstract contract L1Lib is UniversalLib {
    mapping(uint128 => uint256) private _executedBatches;

    // solhint-disable-next-line func-name-mixedcase
    function __L1Lib_init() internal onlyInitializing {
        __UniversalLib_init();
    }

    /// @notice Returns the number of successfully executed batches.
    function executedBatches(uint64 originDomainID, uint64 nonce)
        external
        view
        returns (uint256)
    {
        uint128 nonceAndID = (uint128(nonce) << 64) | uint128(originDomainID);
    ...
    ///
    /// @notice Requirements:
    /// - It must be called only by the rollup admin.
    ...
    function finalizeRollup(
        ...
        require(
            ...
            "L1Lib: failed to verify"
        );
        _executedBatches[nonceAndID]++;

        _finalizeRollup(pairs, resourceID);
    }

    function _finalizeRollup(KeyValuePair[] memory, bytes32) internal virtual;
}
...
abstract contract L1ERC721 is L1Lib, ERC721Upgradeable {
    uint256 private _totalMintCount;
    uint256 private _totalBurnCount;

    // solhint-disable-next-line func-name-mixedcase
    function __L1ERC721_init(string memory name_, string memory symbol_)
        internal
        onlyInitializing
    {
        __L1Lib_init();
    ...
    function totalSupply() public view returns (uint256) {
        return _totalMintCount - _totalBurnCount;
    }
...
abstract contract UniversalLib is AccessControlUpgradeable {
    bytes32 public constant ROLLUP_ADMIN_ROLE = keccak256("ROLLUP_ADMIN_ROLE");

    // solhint-disable-next-line func-name-mixedcase
    function __UniversalLib_init() internal onlyInitializing {
        _grantRole(DEFAULT_ADMIN_ROLE, _msgSender());
        _grantRole(ROLLUP_ADMIN_ROLE, _msgSender());
    }
    ...
    modifier onlyRollupAdmin() {
        require(hasRole(ROLLUP_ADMIN_ROLE, _msgSender()), "not a rollup admin");
        _;
    }
}
...
import "../../rollup/token/L1ERC721.sol";
import "@openzeppelin/contracts-upgradeable/security/PausableUpgradeable.sol";
import "@openzeppelin/contracts-upgradeable/access/OwnableUpgradeable.sol";

contract OtherdeedForOtherside is
    L1ERC721,
    PausableUpgradeable,
    OwnableUpgradeable
{
    ...
    constructor() initializer {
        __L1ERC721_init("Otherdeed for Otherside", "OTHR");
        __Pausable_init();
