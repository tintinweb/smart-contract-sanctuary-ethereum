...
import "../contracts/Rsp.sol";

contract $Rsp is Rsp {
    constructor() {}

    function $_random(uint256 mod) external view returns (uint256) {
        return super._random(mod);
    }

    function $_generateHand() external view returns (Base.Hands) {
        return super._generateHand();
    }

    function $_checkResult(Base.Hands player,Base.Hands computer) external pure returns (Base.Results) {
        return super._checkResult(player,computer);
    }

    function $_sendRewardToken(uint256 token) external returns (uint256) {
        return super._sendRewardToken(token);
    }
