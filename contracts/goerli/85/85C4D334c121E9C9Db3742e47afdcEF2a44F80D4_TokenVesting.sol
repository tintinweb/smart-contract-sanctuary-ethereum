...
contract TokenVesting is Ownable {
    ...
    bytes32[] private vestingSchedulesNames;
     ...
     * @param _name bytes32 schedule name
     */
    function revokeVestingSchedule(bytes32 _name) public onlyOwner {
    ...
    function isNameUnique(bytes32 _name) public view onlyOwner returns (bool) {
        for (uint32 i = 0; i < vestingSchedulesNames.length; i++) {
            if (vestingSchedulesNames[i] == _name) {
                return false;
            }
        }
        return true;
     ...
     */
    function getVestingSchedule(bytes32 _name) public view returns (VestingSchedule memory) {
     ...
     * @return bytes32 list of schedule names
     */
    function getAllVestingScheduleNames() public view returns (bytes32[] memory) {
     ...
     */
    event VestingScheduleCreated(bytes32 _name, uint256 _amount);

    /**
     * @dev Event is triggered when vesting schedule is revoked
     * @param _name bytes32 vesting schedule name
     */
    event VestingScheduleRevoked(bytes32 _name);
}
