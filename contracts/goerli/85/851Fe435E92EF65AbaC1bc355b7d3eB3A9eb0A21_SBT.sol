...
contract SBT is Initializable, AccessControl, ISBT721, IERC721Metadata{
    ...
    EnumerableSet.AddressSet private _whiteListDetailed;
    ...
    function whiteListDetailed(uint start, uint end) external view returns(WhiteListDetailed[] memory) {
        end = (end > _whiteListDetailed.length() ?  _whiteListDetailed.length() : end);
        if (start >= end) {
        ...
        for (uint i = start; i < end; i++) {
            addr = _whiteListDetailed.at(i);
            if (addr == address(0)) {
                return result;
            }
            result[i - start] =  WhiteListDetailed(addr, _whiteList[addr]);
        }
        return result;
    }
    ...
    function addWhiteList(
    ...
    
        {
            ...
            for (uint i = 0; i < newlength; i++) {
                _whiteListDetailed.add(whiteList_[i]);
            }
        }
        
        for (uint i; i < whiteList_.length; i++) {
            _whiteList[whiteList_[i]] = whiteListNum;
        }
    }



    function gain(address to) external {
        uint whiteNum = _whiteList[to];
        require(whiteNum > 0, "Not qualified");
        _attest(to, 1);
        if (whiteNum > 0) {
            --_whiteList[to];
        }
    ...
    function _attest(address to, uint minded) internal returns (uint256) {
        ...
        ++_balances[to];
        emit Attest(to, tokenId, minded);
        emit Transfer(address(0), to, tokenId);
        return tokenId;
    }

    function attest(address to) external onlyRole(OPERATOR_ROLE) returns (uint256) {
        return _attest(to, 0);
    }

    function batchAttest(address[] calldata addrs) external onlyRole(OPERATOR_ROLE){
    ...
    function revoke(uint tokenId) external onlyRole(OPERATOR_ROLE) {
        require(_ownerMap.contains(tokenId), "The account does not have any SBT");  
        address from = _ownerMap.get(tokenId);      
        _ownerMap.remove(tokenId);
        emit Revoke(from, tokenId);
        emit Transfer(from, address(0), tokenId);
    }

    function batchRevoke(uint[] calldata tokenIds) external onlyRole(OPERATOR_ROLE) {
        uint256 tokenIdLength = tokenIds.length;
        require(tokenIdLength <= 100, "The max length of addresses is 100");

        for (uint8 i = 0; i < tokenIdLength; i++) {
     ...
     */
    function setBaseTokenURI(string calldata uri) public onlyRole(DEFAULT_ADMIN_ROLE) {
        _baseTokenURI = uri;
    }
