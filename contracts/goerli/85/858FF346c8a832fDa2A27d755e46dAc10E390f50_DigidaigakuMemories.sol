...
contract DigidaigakuMemories is Initializable, ERC721URIStorageUpgradeable, OwnableUpgradeable {
    struct Snapshot {
        address minter;
        uint256[] ids;
    }
    ...
    function _mintMemory(uint256 id) internal {
        _mint(msg.sender, id);
        _setTokenURI(id, string(abi.encodePacked(_baseURI(), StringsUpgradeable.toString(id), ".json")));
    }
