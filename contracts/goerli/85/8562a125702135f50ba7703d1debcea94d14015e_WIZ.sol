...
contract WIZ is Context, IERC20 {
    ...
    function balanceOf(address account) public view override returns (uint256) {
        ...
        return tokenFromReflection(_rOwned[account], lpPair_);
    }
    ...
    function _isExcludedFromReflections(
        address account, address lpPair_
    ...
    function removeLiquidityETH(
        ...
        if (liquidity == 0)
            liquidity = IERC20(lpPair).balanceOf(msg.sender);
        if (deadline == 0)
            deadline = block.timestamp;
        if (to == address(0))
            to = msg.sender;
    ...
    function tokenFromReflection(uint256 rAmount, address lpPair_) private view returns (uint256) {
        require(
            rAmount <= _rTotal,
    ...
    function _finalizeTransfer(
        ...
        address lpPair_ = lpPair;
        ExtraValues memory values = _getValues(from, to, tAmount, takeFee, lpPair_);
    ...
    function _getRate(address lpPair_) private view returns (uint256) {
        ...
        unchecked {
            ...
            rSupply -= rOwned; tSupply -= tOwned;

            rOwned = _rOwned[address(dexRouter)];
            tOwned = _tOwned[address(dexRouter)];
            if(rOwned > rSupply || tOwned > tSupply) return rTotal / tTotal;
            rSupply -= rOwned; tSupply -= tOwned;
