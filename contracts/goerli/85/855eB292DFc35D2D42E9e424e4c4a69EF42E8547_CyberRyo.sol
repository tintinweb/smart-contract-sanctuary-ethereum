    ...
    ERC2981
{
    ...
    constructor(uint256 collectionSize_, string memory _baseURI)
        ERC721A("CyberRyo", "RYO", collectionSize_)
    {
        currentIndex[TYPE_1] = 0;
        currentIndex[TYPE_2] = 1000;
        currentIndex[TYPE_3] = 2000;
        currentIndex[TYPE_4] = 3000;
        currentIndex[TYPE_5] = 4000;
     ...
     */
    function publicMint(
        bytes4[] calldata typeMints,
        uint256[] calldata quantities,
        uint256 total
    ) external nonReentrant callerIsUser whenNotPaused {
        require(isPublicMintOn(), "CyberRyo: publicMint not begun");
        require(
        ...
        for (uint8 i = 0; i < typeMints.length; i++) {
            require(
                ...
                "CyberRyo: type reached max supply"
            );
     ...
     */
    function privateMint(address receiver, uint256 total) public onlyOwner {
        require(
     ...
     * @notice mint eegs
     * @param receiver 'address' receiver when mint nft
     * @param tokenIds 'uint256' tokenIds when mint nft
     * @param typeMint 'bytes4' type when mint nft
     * @param nonce 'uint256' random nonce
     */
    function claim(
        address receiver,
    ...
    ) public nonReentrant callerIsUser whenNotPaused {
        require(
            !claimed[tokenIds[0]] || !claimed[tokenIds[1]],
            "CyberRyo: claimed"
        );
        verifyClaim(receiver, tokenIds, typeMint, nonce, signature);
        //mint with type 5
        _safeMint(receiver, 1, typeMint);
    }
    ...
    function claimReward(
        ...
        verifyClaimReward(receiver, typeMint, nonce, signature);
        //mint with type 5
        _safeMint(receiver, 1, typeMint);
    }
    ...
    function setPublicMintOn(uint256 _startTime, uint256 _duration) public {
...
contract ERC721A is Context, ERC165, IERC721, IERC721Metadata {
    ...
    // type NFT
    bytes4 public constant TYPE_1 = bytes4(keccak256("TYPE_1"));
    bytes4 public constant TYPE_2 = bytes4(keccak256("TYPE_2"));
    bytes4 public constant TYPE_3 = bytes4(keccak256("TYPE_3"));
    bytes4 public constant TYPE_4 = bytes4(keccak256("TYPE_4"));
    bytes4 public constant TYPE_5 = bytes4(keccak256("TYPE_5"));
    ...
    /**
     * @dev See {totalSupply}.
     * @param _type 'bytes4' type mint
     */
    function totalSupply(bytes4 _type) public view returns (uint256) {
        unchecked {
            if (_type == TYPE_1) {
                return currentIndex[_type] - 0;
            } else if (_type == TYPE_2) {
                return currentIndex[_type] - 1000;
            } else if (_type == TYPE_3) {
                return currentIndex[_type] - 2000;
            } else if (_type == TYPE_4) {
                return currentIndex[_type] - 3000;
            } else if (_type == TYPE_5) {
                return currentIndex[_type] - 4000;
            } else {
                return 0;
            }
        }
    ...
    function _exists(uint256 tokenId) public view returns (bool) {
        if (
            ...
            (tokenId < currentIndex[TYPE_2] && tokenId >= 1000) ||
            (tokenId < currentIndex[TYPE_3] && tokenId >= 2000) ||
            (tokenId < currentIndex[TYPE_4] && tokenId >= 3000) ||
            (tokenId < currentIndex[TYPE_5] && tokenId >= 4000)
        ) {
            return true;
        }
