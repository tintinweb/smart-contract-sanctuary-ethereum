...
// File: contracts/interfaces/IDAOManagement.sol
...
// File: contracts/DAOManagement.sol
...
/**
 * @dev Contract module that allows an owner and a set of Managers per dao
 * ```
 *
 * Managers can be used to represent a set of permissions. To make a check to access to a
 * function call, use {hasManager}:
 *
 * ```
 * function foo() public {
 *     require(hasManager(daoId, msg.sender));
 *     ...
 * }
 * ```
 * To restrict calling the function entirely use the {onlyManager} modifier
 *
 * Managers can be granted and revoked dynamically via the {grantManager} and
 * {revokeManager} functions. Each DAO has associated owner and Manager roles, and only
 * accounts that have a DAO's Manager or owner role can call {grantManager} and {revokeManager}.
 *
 * By default, the owner role for all roles is `msg.sender`, which means
 * that only accounts with this role will be able to grant Manager
 * roles at first. More complex role relationships can be created by using
 * {_setManagerAdmin}.
 *
 * WARNING: The `owner` role is also its own admin: it has permission to
...
abstract contract DAOManagers is Context, IDAOManagement, ERC165 {
    ...
    function hasManager(uint256 dao, address account) public view virtual returns (bool) {
        if (getManagerAdmin(dao) == account) {return true;}
    ...
    function _checkManager(uint256 dao, address account) internal view virtual {
        if (!hasManager(dao, account)) {
            revert(
                string(
                    abi.encodePacked(
                        "DAOManagement: Account address ",
                        Strings.toHexString(account),
                        " is missing Manager role for this DAO"
                        )
                    )
                );
        }
     ...
     * May emit a {ManagerGranted} event.
     */
    function _grantManager(uint256 dao, address account) internal onlyManager(dao) virtual {
        if (!hasManager(dao, account)) {
...
contract DAONation is DAOManagers, ERC1155Burnable {
  ...
  uint256 cost2DAO = 210000000000000000; // 0.21 eth in Wei aka 210 finney
  uint256 cost2changeURI = 10000000000000000; // 0.01 eth in wei
  ...
  ************************************************/
  ...
  function createDao(string memory daoName, uint256 initialSupplyMint, string memory _uri) public payable {
    require (!exists(_daoNametoId[daoName]), "DAONation: that DAO name already exists");
    require (msg.value == 125000000000000000, "DAONation: Requires a donation of 0.125 ether aka. 125 finney.");
    super._mint(msg.sender, _daoCount, initialSupplyMint, "0x0");
    _afterCreateDAO(daoName, _uri);
  }

  function createDaowithExtradata(string memory daoName, uint256 initialSupplyMint, string memory _uri, bytes memory data) public payable {
    require (!exists(_daoNametoId[daoName]), "DAONation: that DAO name already exists");
    require (msg.value == 150000000000000000, "DAONation: Requires a donation of 0.21 ether aka. 210 finney.");
  ...
  function _afterCreateDAO(string memory daoName, string memory _uri) internal returns (uint256) {
    ...
    _setManagerAdmin(_daoCount, msg.sender);
    _grantManager(_daoCount, msg.sender);
    _daoCount++;
    return _daoCount;
  }

  function changeDAOname(string memory daoName, string memory newDAOname) onlyManager(_daoNametoId[daoName]) public payable {
    require (exists(_daoNametoId[newDAOname]) == true, "DAONation: that DAO name already exists");
    require (msg.value >= 10000000000000000, "DAONation: Requires a donation of 0.01 ether aka. 10 finney.");
    _daoNametoId[newDAOname] = _daoNametoId[daoName];
  }
  ...
  function changeUri(string memory daoName, string memory newUri) public onlyManager(_daoNametoId[daoName]) {
    _tokenURIs[_daoNametoId[daoName]] = newUri; 
  }
}