...
contract PolicyBook is IPolicyBook, ERC20PermitUpgradeable, AbstractDependant {
    ...
    uint256 public override lastDistributionEpoch;

    uint256 public override lastPremiumDistributionEpoch;
    int256 public override lastPremiumDistributionAmount;
    ...
    address public deinTreasury; //  use the deprecated storage - address same interaface
    ...
    mapping(uint256 => uint256) public override epochAmounts;
    mapping(uint256 => int256) public override premiumDistributionDeltas;
    ...
    // new state
    mapping(uint256 => uint256) public override epochCompoundedAmounts; // epoch => compounded amount
    mapping(address => uint256) public override policyHoldersCompoundedAmount; // holder => compounded amount
    ...
    modifier onlyPolicyBookAdmin() {
        require(_msgSender() == address(policyBookAdmin), "PB: Not a PBA");
        _;
    }
    ...
    function _distributePremiums() internal {
        ...
        uint256 currentEpoch = policyBookFacade.getPremiumDistributionEpoch();
        ...
        if (currentEpoch > lastEpoch) {
            (
                ...
                _totalLiquidity
            ) = policyBookFacade.getPremiumsDistribution(lastEpoch, currentEpoch);
            uint256 _newLiquidity = _totalLiquidity.sub(totalLiquidity);
            if (_newLiquidity > 0) {
                policyBookFacade.releaseCompoundedLiquidity(_newLiquidity);
    ...
    function __PolicyBook_init(
        ...
        policyBookFacade = IPolicyBookFacade(_policyBookFacadeAddress);

        lastPremiumDistributionEpoch = policyBookFacade.getPremiumDistributionEpoch();
    }
    ...
    function convertBMIXToSTBL(uint256 _amount) public view override returns (uint256) {
        (, uint256 currentLiquidity, ) = policyBookFacade.getNewCoverAndLiquidity();
    ...
    function submitClaimAndInitializeVoting(
        ...
        bool appeal
    ) external override onlyPolicyBookFacade withPremiumsDistribution {
        uint256 cover = policyHolders[policyHolder].coverTokens;
        uint256 compoundedCover = policyHoldersCompoundedAmount[policyHolder];

        uint256 virtualEndEpochNumber =
            policyHolders[policyHolder].endEpochNumber.tryAdd(VIRTUAL_EPOCHS);
        ...
        uint256 _targetEpochNumber;
        if (!appeal) {
            require(claimingRegistry.canClaim(claimProvenance), "CR: Cannot claim");
            _targetEpochNumber = virtualEndEpochNumber;
        } else {
            require(claimingRegistry.canAppeal(claimProvenance), "CR: Cannot appeal");
            uint256 claimIndex = claimingRegistry.getClaimIndex(claimProvenance);
            _targetEpochNumber = Math.max(
                getEpoch(claimingRegistry.getClaimDateEnd(claimIndex)).tryAdd(1),
                virtualEndEpochNumber
            );
        }
        epochAmounts[_targetEpochNumber] = epochAmounts[_targetEpochNumber].sub(cover);

        if (compoundedCover > 0) {
            epochCompoundedAmounts[_targetEpochNumber] = epochCompoundedAmounts[_targetEpochNumber]
                .sub(compoundedCover);
        }
    ...
    function commitClaim(
        ...
        uint256 virtualEndEpochNumber = policyHolders[claimer].endEpochNumber.add(VIRTUAL_EPOCHS);
        if (
        ...
        uint256 compoundedCover = policyHoldersCompoundedAmount[claimer];

        if (
            status == IClaimingRegistry.ClaimStatus.DENIED ||
            status == IClaimingRegistry.ClaimStatus.EXPIRED
        ) {
            uint256 endUnlockEpoch =
                Math.max(
                    getEpoch(claimEndTime).tryAdd(1),
                    policyHolders[claimer].endEpochNumber.tryAdd(VIRTUAL_EPOCHS)
                );

            epochAmounts[endUnlockEpoch] = epochAmounts[endUnlockEpoch].add(cover);
            if (compoundedCover > 0)
                epochCompoundedAmounts[endUnlockEpoch] = epochCompoundedAmounts[endUnlockEpoch]
                    .add(compoundedCover);
        } else {
            if (status != IClaimingRegistry.ClaimStatus.ACCEPTED) {
                if (lastDistributionEpoch <= virtualEndEpochNumber) {
                    ...
                    if (compoundedCover > 0)
                        epochCompoundedAmounts[virtualEndEpochNumber] = epochCompoundedAmounts[
                            virtualEndEpochNumber
                        ]
                            .add(compoundedCover);
                } else {
                    ...
                    if (compoundedCover > 0)
                        policyBookFacade.releaseCompoundedLiquidity(compoundedCover);
        ...
        updateBMICoverStakingReward
    {
        ...
        delete policyHoldersCompoundedAmount[claimer];
        policyRegistry.removePolicy(claimer);
        policyBookFacade.reevaluateProvidedLeverageStable();
    }

    function buyPolicy(BuyPolicyParameters memory parameters)
        external
        override
        ...
        returns (uint256)
    {
        ...
        updateEpochsInfo(false);
        // get available compounded liquidity
        parameters.compoundLiquidity = compoundPool.getCompoundLiquidityForPool(address(this));

        // total cover + new cover
        uint256 _totalCoverTokens = totalCoverTokens.add(parameters.coverTokens);

        // total cover + new cover + pending withdraw
        uint256 _totalCoverWithPendingWithdraw =
            _totalCoverTokens.add(parameters.pendingWithdrawalAmount);

        require(
            totalLiquidity.add(parameters.compoundLiquidity) >= _totalCoverWithPendingWithdraw,
            "PB: Not enough liquidity"
        );

        // get the amount of compound liq particpaitd in the cover
        parameters.deployedCompoundedLiquidity = _totalCoverWithPendingWithdraw.trySub(
            totalLiquidity
        );
        parameters.coverTokens = parameters.coverTokens.sub(
            parameters.deployedCompoundedLiquidity
        );
        ...
        (uint256 _totalSeconds, uint256 _totalPrice, ) =
            policyBookFacade.getPolicyPrice(
                ...
                totalCoverTokens,
                totalLiquidity,
                parameters.compoundLiquidity,
                parameters.deployedCompoundedLiquidity
            );

        uint256 _distributorFeeAmount;
        if (parameters.distributorFee > 0) {
            _distributorFeeAmount = _totalPrice.mul(parameters.distributorFee).uncheckedDiv(
                PERCENTAGE_100
            );
        }
        ...
        uint256 _protocolFeeAmount =
            _totalPrice
                .mul(policyBookAdmin.getProtocolFee(whitelisted))
                .uncheckedDiv(PERCENTAGE_100)
                .uncheckedSub(_distributorFeeAmount);

        uint256 _price =
            _totalPrice.uncheckedSub(_protocolFeeAmount).uncheckedSub(_distributorFeeAmount);

        _addPolicyHolder(parameters, _totalPrice, _protocolFeeAmount);

        totalCoverTokens = _totalCoverTokens.sub(parameters.deployedCompoundedLiquidity);
        ...
        policyRegistry.addPolicy(
            parameters.holder,
            parameters.coverTokens.add(parameters.deployedCompoundedLiquidity),
            _totalPrice,
            _totalSeconds
        );
        ...
        return parameters.deployedCompoundedLiquidity;
    }
    ...
    function _addPolicyHolder(
        ...
        uint256 _endEpochNumber = _currentEpochNumber.tryAdd(parameters.epochsNumber.trySub(1));
        uint256 _virtualEndEpochNumber = _endEpochNumber.tryAdd(VIRTUAL_EPOCHS);
        ...
        policyHoldersCompoundedAmount[parameters.holder] = parameters.deployedCompoundedLiquidity;

        epochAmounts[_virtualEndEpochNumber] = epochAmounts[_virtualEndEpochNumber].add(
            parameters.coverTokens
        );
        epochCompoundedAmounts[_virtualEndEpochNumber] = epochCompoundedAmounts[
            _virtualEndEpochNumber
        ]
            .add(parameters.deployedCompoundedLiquidity);
    }
    ...
    function endActivePolicy(address _holder) external override onlyClaimingRegistry {
        ...
        uint256 _compoundedCover = policyHoldersCompoundedAmount[_holder];

        // remove cover from epoch management
        uint256 _virtualEndEpochNumberPrevious =
            policyHolders[_holder].endEpochNumber.tryAdd(VIRTUAL_EPOCHS);
        ...
        if (_compoundedCover > 0) {
            epochCompoundedAmounts[_virtualEndEpochNumberPrevious] = epochCompoundedAmounts[
                _virtualEndEpochNumberPrevious
            ]
                .sub(_compoundedCover);
            policyBookFacade.releaseCompoundedLiquidity(_compoundedCover);
            delete policyHoldersCompoundedAmount[_holder];
        }
        ...
        internal
    {
        ...
        uint256 nextEpoch = policyBookFacade.getPremiumDistributionEpoch().tryAdd(1);
    ...
    function updateEpochsInfo(bool _rebalance) public override {
        ...
        uint256 _newDistributionEpoch =
            Math.min(
                ...
                _lastDistributionEpoch.tryAdd(policyBookAdmin.maxEpoch())
            );

        if (_lastDistributionEpoch < _newDistributionEpoch) {
            uint256 _newTotalCoverTokens = totalCoverTokens;
            uint256 _compoundedAmount;

            for (
                uint256 i = _lastDistributionEpoch;
                i < _newDistributionEpoch;
                i = uncheckedInc(i)
            ) {
                _newTotalCoverTokens = _newTotalCoverTokens.sub(epochAmounts[i]);
                _compoundedAmount = _compoundedAmount.add(epochCompoundedAmounts[i]);

                delete epochCompoundedAmounts[i];
                delete epochAmounts[i];
            }

            lastDistributionEpoch = _newDistributionEpoch;
            totalCoverTokens = _newTotalCoverTokens;
            if (_compoundedAmount > 0) {
                policyBookFacade.releaseCompoundedLiquidity(_compoundedAmount);
            }
            if (_rebalance) {
                policyBookFacade.reevaluateProvidedLeverageStable();
            }
        ...
        returns (WithdrawalStatus)
    {
        ...
        if (block.timestamp >= readyToWithdrawDate.tryAdd(READY_TO_WITHDRAW_PERIOD)) {
    ...
    function requestWithdrawal(
        ...
        require(
            totalLiquidity >=
                totalCoverTokens.add(_stblTokensToWithdraw).add(_pendingWithdrawalAmount),
            "PB: Not enough free liquidity"
        );
        require(
            capitalPool.canRequestWithdraw(_stblTokensToWithdraw),
            "PB: Not enough withdraw liquidity, please contact us"
        );
