...
contract Buns is ERC721Enumerable, Ownable, OperatorFilterer {
    ...
    uint256 public token_price = 0.009 ether;
    ...
    function mint(uint32 count) external payable {
        ...
        require(count < 101, "Exceeds max 100 per transaction.");
        uint256 nextTokenId = _owners.length;
        unchecked {
            require(nextTokenId + count < maxSupply, "Exceeds max supply of 10,000.");
        }
        unchecked {
            require(msg.value >= token_price * count, "Not enough ETH sent; Mint price is 0.009 ETH per Bun!");
        }
