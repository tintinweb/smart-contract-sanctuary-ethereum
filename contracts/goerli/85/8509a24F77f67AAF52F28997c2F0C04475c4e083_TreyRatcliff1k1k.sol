...
function safeMintWL() external payable callerIsUser nonReentrant {
    ...
    require(mints < groupLimit(group), "Maximum number of mints exceeded for group!");
    ...
    if (group == PROOF) {
        proofMints[msg.sender]++;
    } else if (group == MOONBIRDS) {
        moonbirdsMints[msg.sender]++;
    } else if (group == PREMINT) {
        premintMints[msg.sender]++;
    }
    mintedInWLed++;
...
function groupLimit(uint8 group) private view returns (uint8) {
  if (group == PROOF) {
    return PROOF_LIMIT;
  } else if (group == MOONBIRDS) {
