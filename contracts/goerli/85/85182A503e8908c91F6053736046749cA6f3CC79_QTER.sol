...
contract QTER is ERC20, Ownable {
    OCASnake oca;
    ...
    function setOCASnake(address _address) public {
    ...
    function claimSnakeQTER(uint256 tokenId) public returns (uint256) {
        TokenInfo memory info = getSnakeStakeTime(tokenId);
        //get total time staked to increase the reward
        uint256 totalTimeStaked = block.timestamp - info.stakeDate;
        ...
        uint256 rewardTotalMultiplayer = totalTimeStaked / (60 * 60 * 24 * 30);
        bool isOwner = oca.ownerOf(tokenId) == msg.sender;
        require(isOwner, "Only the owner can claim");
        //reward add multiplier for every minute staked since last claim
        uint256 rewardTotal = ((totalStakedSinceLastClaim +
            rewardTotalMultiplayer) / (60 * 60));
    ...
    function stakeToken(uint256 tokenId) public {
        ...
        // require(stakeDate == 0, "Token is already staked.");
        oca.stakeToken(tokenId);
