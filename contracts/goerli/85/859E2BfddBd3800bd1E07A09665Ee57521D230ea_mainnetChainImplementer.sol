...
contract mainnetChainImplementer is AccessControl {
    ...
    event MainnetCommunityRegistryCreated(uint16 community_id,address community_proxy);
    ...
    function freeCommunityRegistry(address community_reg, address[] memory admins) external {
        require(msg.sender == RegistryConsumer(reg).getRegistryAddress("COMMUNITY_REGISTRY_CONTROLLER"),"mainnetChainImplementer/freeCommunityRegistry: Unauthorised");
...
contract CommunityRegistry is AccessControl  {
    ...
    modifier onlyAdmin() {
        ...
        if (independant){
            ...
            IAccessControl ac = IAccessControl(owner);

            
            require(msg.sender == owner || ac.hasRole(COMMUNITY_REGISTRY_ADMIN,msg.sender),"CommunityRegistry : Unauthorised");
        }
        _;
    }

    constructor (
        uint16  _community_id, 
        ...
        _setupRole(DEFAULT_ADMIN_ROLE, _msgSender()); // default admin = launchpad
        owner = msg.sender;
        initialised = true;
    }
...
contract community_list is AccessControl { 
    ...
    mapping(uint256 => community_entry)  public communities;   // community_id => record
    mapping(uint256 => uint256)          public index;         // entryNumber => community_id for enumeration

    event CommunityAdded(uint256 pos, string community_name, address community_registry, uint256 community_id);
    ...
    function addCommunity(uint256 community_id, string memory community_name, address community_registry) external onlyRole(CONTRACT_ADMIN) {
