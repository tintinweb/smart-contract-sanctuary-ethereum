...
contract CodmaBears is ERC721URIStorage, Ownable{
 
    event MintBears (address indexed minter, uint256 startWith, uint256 times);
    
    
    uint256 public totalBears;
    uint256 public totalCount = 10000; //bruhTotal
    uint256 public maxBatch = 20; // bruhBatch
    uint256 public price = 20000000000000000; // 0.06 eth
    ...
    function mintBear(uint256 _times) payable public mintEnabled {
        require(_times >0 && _times <= maxBatch, "mint wrong number");
        require(totalBears + _times <= totalCount, "too much");
        require(msg.value == _times * price, "value error");
        payable(owner()).transfer(msg.value);
        emit MintBears(_msgSender(), totalBears+1, _times);
    ...
    function whitelistMint(uint _times) payable public canWhitelistMint {
        ...
        require(totalBears + _times <= totalCount, "Mint amount will exceed total collection amount.");
    ...
    function adminMintGiveaways(address _addr) public onlyOwner {
        require(totalBears + 1 <= totalCount, "Mint amount will exceed total collection amount.");
        emit MintBears(_addr, totalBears+1, 1);
        _mint(_addr, 1 + totalBears++);
    } 
