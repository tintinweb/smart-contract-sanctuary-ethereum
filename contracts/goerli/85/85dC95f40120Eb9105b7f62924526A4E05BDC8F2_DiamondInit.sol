...
// INFO STILL NEEDED:
// - full list of admins
// - name and symbol for ERC721 collection (I am assuming BarBearians & BEARS)
// - First token ID (usually 1 or 0)
// - full list of Revenue & Royalty recipients, with percentage shares
// - baseURI (api link)
// - authorisedSigner address

import { GlobalState } from "./libraries/GlobalState.sol";
import { AuctionFacetLib } from "./facets/AuctionFacet.sol";
...
contract DiamondInit {
    ...
    function initAdminPrivilegesFacet() public {
        ...
        admins[1] = 0xBD48774f5AEd996f90DC9A31F86b9Dc34DCF5C6c;
    ...
    // The address given here is a placeholder
    address private constant treasureContract = 0x699a1928EA12D21dd2138F36A3690059bf1253A0;

    uint256 private constant auctionStart = 1;
    uint256 private constant auctionLength = 2;
    uint256 private constant allowlistStart = 3;
    uint256 private constant allowlistLength = 4;

    function initAuctionFacet() public {
        AuctionFacetLib.state storage s = AuctionFacetLib.getState();

        s.treasureContract = treasureContract;
        s.allowlistPrice = 0.001 ether;
        s.allowlistMaxSupply = 2000;

        s.auctionTimings.auctionStart = auctionStart;
        s.auctionTimings.auctionLength = auctionLength;
        s.allowlistTimings.allowlistStart = allowlistStart;
        s.allowlistTimings.allowlistLength = allowlistLength;
    }

    // TokenFacet //

    address private constant authorisedSigner = 0x0000000000000000000000000000000000000000; // tbd
    uint256 private constant maxSupply = 10000;
    string private constant baseURI = "https://api.barbearians.xyz/token/";

    string private constant name = "BarBearians"; // tbd
    string private constant symbol = "BEARS"; // tbd
    ...
    function initTokenFacet() public {
        ...
        s1.authorisedSigner = msg.sender;
...
library RoyaltiesConfigLib {
     ...
     */
    function setRoyalties(address payable recipient, uint256 bps) internal {
        state storage s = RoyaltiesConfigLib.getState();
...
contract RoyaltiesConfigFacet {
    ...
    function setRoyalties(address payable recipient, uint256 bps) external {
        GlobalState.requireCallerIsAdmin();
        RoyaltiesConfigLib.setRoyalties(recipient, bps);
    }
}
