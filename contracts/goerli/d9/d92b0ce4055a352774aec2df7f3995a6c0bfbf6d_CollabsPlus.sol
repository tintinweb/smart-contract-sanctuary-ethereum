...
contract CollabsPlus is Ownable, ERC721A {
    ...
    constructor(string memory _baseURI) ERC721A("CollabsPlus", "CP"){
        baseURI = _baseURI;
    }

    function mintReserve(address _account, uint _quantity) external onlyOwner{
        if(totalSupply() + _quantity > MAX_SUPPLY) revert("Max supply exceeded");
        _mint(_account, _quantity);
    }

    function mint(address _account, uint _quantity) external payable {
