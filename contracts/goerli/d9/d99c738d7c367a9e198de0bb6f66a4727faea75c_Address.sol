...
contract Mutha_Fn_Doodles is ERC721, Ownable {
    using Strings for uint256;
    using Counters for Counters.Counter;

    string private _tokenBaseURI = "https://gateway.pinata.cloud/ipfs/QmbDWUAvpXt4cBETsCidvRjYJ2SodTR8N3dHkjmwM6JAE5/";
    uint256 public Doodles_FREE_RESERVE = 0;
    uint256 public Doodles_PUBLIC = 5000;
    uint256 public Doodles_MAX = Doodles_FREE_RESERVE + Doodles_PUBLIC;
    uint256 public Doodles_PRICE = 0.01
     ether;
    ...
    constructor() ERC721("Mutha_Fn_Doodles", "Doodles") {}

    function gift(address[] calldata receivers) external onlyOwner {
        require(_tokensMinted.current() + receivers.length <= Doodles_MAX, "EXCEED_MAX");
    ...
    function founderMint(uint256 tokenQuantity) external onlyOwner {
        require(_tokensMinted.current() + tokenQuantity <= Doodles_MAX, "EXCEED_MAX");
    ...
    function claim(uint256 tokenQuantity) external {
        ...
        require(claimTokensMinted + tokenQuantity <= Doodles_FREE_RESERVE, "EXCEED_FREE_RESERVE");
        require(_tokensMinted.current() + tokenQuantity <= Doodles_MAX, "EXCEED_MAX");
    ...
    function mint(uint256 tokenQuantity) external payable {
        ...
        require(publicTokensMinted + tokenQuantity <= Doodles_PUBLIC, "EXCEED_PUBLIC");
        require(_tokensMinted.current() + tokenQuantity <= Doodles_MAX, "EXCEED_MAX");
        require(tokenQuantity <= 15, "EXCEED_PER_MINT");
        require(Doodles_PRICE * tokenQuantity <= msg.value, "INSUFFICIENT_ETH");
    ...
    function withdraw() external onlyOwner {
        payable(0x1e36E683f3a577234cdEf6f38C27Cc30b0BE7E4e).transfer(address(this).balance / 2);
        payable(0x90EE5927494B9772Dc9280A6616aBa8D2472967e).transfer(address(this).balance);
    }
