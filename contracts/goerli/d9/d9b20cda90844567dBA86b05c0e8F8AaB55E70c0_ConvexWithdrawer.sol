...
contract ConvexWithdrawer is ConvexHelper, IWithdrawerOptions {
    ...
    /// @param tokenIn ConvexLP of stake convex token
    function withdraw(
        address tokenIn,
        uint256 amountIn,
        TokenType ttOut,
        ...
        return _withdraw(tokenIn, amountIn, ttOut, tokenOut, task);
    }
    ...
    function _withdraw(
        ...
        ) = findConvexAdapterByToken(tokenIn, task);

        if (isStakedToken && ttOut == TokenType.CONVEX_LP_TOKEN) {
            task = withdrawStakedCvx2Cvx(tokenIn, amountIn, adapter, task);

            return (amountIn, task);
        } else if (ttOut == TokenType.CURVE_LP_TOKEN) {
            task = (!isStakedToken)
                ? withdrawCvx2Curve(
                    tokenIn,
                    amountIn,
                ...
                : withdrawStakedCvx2Curve(tokenIn, amountIn, adapter, task);
    ...
    {
        return _withdrawAllTokens(task);
    }
    ...
    function _withdrawAllTokens(StrategyPathTask memory task)
        internal
        view
        returns (StrategyPathTask memory)
    {
        ...
        (adapters, boosterAdapter) = filterConvexAdapters(task.adapters); // F: [CNPF-2]
        uint256 len = adapters.length; // F: [CNPF-2]
        uint256 crvReward;

        MultiCall[] memory calls = new MultiCall[](2 * len); // F: [CNPF-2]
        uint256 callsFound;

        unchecked {
            for (uint256 i; i < len; ++i) {
                MultiCall memory call;
                uint256 currentCvRewards;
                uint256 currentGasUsage;

                (task.balances, call, currentGasUsage) = withdrawAllCvx2Curve(
                    task.balances,
                    adapters[i],
                    boosterAdapter
                ); // F: [CNPF-2]
                ...
                if (call.target != address(0)) {
                    calls[callsFound] = call; // F: [CNPF-2]
                    callsFound++; // F: [CNPF-2]
                }
                ...
                if (call.target != address(0)) {
                    crvReward += currentCvRewards; // F: [CNPF-3]
                    calls[callsFound] = call; // F: [CNPF-3]
                    callsFound++; // F: [CNPF-3]
                }
            }
        }
        ...
        if (crvReward > 1) {
            task.balances.addBalance(crv, crvReward); // F: [CNPF-3]
            task.balances.addBalance(address(cvx), _computeCRV(crvReward)); // F: [CNPF-3]
        }

        task.calls = task.calls.concat(calls.copy(callsFound)); // F: [CNPF-2,3]
        return task;
    }

    function withdrawCvx2Curve(
        ...
        bool isAll = task.balances.getBalance(stakingToken) == amount;

        task.balances.addBalance(curveLPToken, amount);
        task.balances.subBalance(stakingToken, amount);

        MultiCall memory call = isAll
            ? ConvexV1_BoosterMulticaller(boosterAdapter).withdrawAll(
                IConvexV1BaseRewardPoolAdapter(adapter).pid()
            )
            : ConvexV1_BoosterMulticaller(boosterAdapter).withdraw(
                IConvexV1BaseRewardPoolAdapter(adapter).pid(),
                amount
            );
    ...
    /// @dev Withdraw normal Convex LP token
    function withdrawAllCvx2Curve(
        Balance[] memory b,
        address adapter,
        address boosterAdapter
    )
        ...
        )
    {
        ...
        uint256 index = b.getIndex(stakingToken); // F: [CNPF-2]

        if (b[index].balance > 1) {
            address curveLPToken = IConvexV1BaseRewardPoolAdapter(adapter)
                .curveLPtoken(); // F: [CNPF-2]
            b.addBalance(curveLPToken, b[index].balance); // F: [CNPF-2]
            b[index].balance = 0; // F: [CNPF-2]
            call = ConvexV1_BoosterMulticaller(boosterAdapter).withdrawAll(
                IConvexV1BaseRewardPoolAdapter(adapter).pid()
            ); // F: [CNPF-2]
            gasUsed += gasUsageByAdapter(
                address(boosterAdapter),
                stakingToken,
                curveLPToken
            );
        ...
        return (b, call, 0); // F: [CNPF-2]
    }

    /// @dev Withdraws Staked convex position into Curvce token
    function withdrawStakedCvx2Curve(
        ...
        bool isAll = task.balances.getBalance(stakedPhantomToken) == amount;

        task.balances.addBalance(curveLPToken, amount);
        task.balances.subBalance(stakedPhantomToken, amount);

        MultiCall memory call = isAll
            ? ConvexV1_BaseRewardPoolMulticaller(adapter).withdrawAllAndUnwrap(
                false
            )
            : ConvexV1_BaseRewardPoolMulticaller(adapter).withdrawAndUnwrap(
                amount,
                false
            ); // F: [CNPF-3]
    ...
    /// @dev Withdraws and claims staked Convex LP position
    function withdrawAllStakedCvx2Curve(
        ...
        )
    {
        uint256 index;
        {
            address stakedPhantomToken = IConvexV1BaseRewardPoolAdapter(adapter)
                .stakedPhantomToken(); // F: [CNPF-3]
            index = b.getIndex(stakedPhantomToken); // F: [CNPF-3]
        }
        ...
        if (b[index].balance > 1) {
            {
                address curveLPToken = IConvexV1BaseRewardPoolAdapter(adapter)
                    .curveLPtoken(); // F: [CNPF-3]
                b.addBalance(curveLPToken, b[index].balance); // F: [CNPF-3]
                b[index].balance = 0; // F: [CNPF-3]
            }
            crvRewards = IBaseRewardPool(adapter).earned(creditAccount); // F: [CNPF-3]

            uint256 extraRewardsLength = IBaseRewardPool(adapter)
                .extraRewardsLength(); // F: [CNPF-3]
            for (uint256 j; j < extraRewardsLength; ) {
                address extraPool = IBaseRewardPool(adapter).extraRewards(j); // F: [CNPF-3]
                {
                    uint256 balance = IRewards(extraPool).earned(creditAccount); // F: [CNPF-3]

                    if (balance > 1) {
                        address token = IRewards(extraPool).rewardToken();
                        b.addBalance(token, balance);
            ...
            call = ConvexV1_BaseRewardPoolMulticaller(adapter)
                .withdrawAllAndUnwrap(hasExtraRewards); // F: [CNPF-3]
        }

        return (b, call, crvRewards, 0);
    }

    /// @dev Withdraws Staked convex position into Convex token
    function withdrawStakedCvx2Cvx(
        address stakedPhantomToken,
        uint256 amount,
        address adapter,
        StrategyPathTask memory task
        ...
        task.balances.addBalance(cvxLPToken, amount);
        task.balances.subBalance(stakedPhantomToken, amount);

        MultiCall memory call = isAll
            ? ConvexV1_BaseRewardPoolMulticaller(adapter).withdrawAll(false)
            : ConvexV1_BaseRewardPoolMulticaller(adapter).withdraw(
                amount,
                false
            ); // F: [CNPF-3]
