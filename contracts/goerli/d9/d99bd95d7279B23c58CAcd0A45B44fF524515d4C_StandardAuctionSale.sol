    ...
    UUPSUpgradeable
{
    ...
    function isItemInDrop(uint256 dropId, uint256 itemId)
        ...
        returns (bool)
    {
        (bool exists, uint256 value) = StandardAuctionSaleStorage
            .layout()
            .drops[dropId]
            .itemIds
            .tryGet(itemId);
        return
            exists &&
            (value ==
                StandardAuctionSaleStorage.layout().drops[dropId].revision);
    }
    ...
    function publishDrop(
        ...
        newDrop.revision = 1;
        newDrop.numItems = uint32(itemIds.length);

        for (uint256 i; i < itemIds.length; i++) {
            newDrop.itemIds.set(itemIds[i], newDrop.revision);
        }
    ...
    function rePublishDrop(
        ...
        if (block.timestamp < stdAuction.start) {
            ...
            layout.drops[dropId].revision++;
            layout.drops[dropId].numItems = uint32(itemIds.length);

            for (uint256 i; i < itemIds.length; i++) {
                layout.drops[dropId].itemIds.set(
                    itemIds[i],
                    layout.drops[dropId].revision
                );
            }
            ...
            emit StandardAuctionDropRePublished(
                voucherId,
                dropId,
                new uint256[](0), // Maybe create a new event for this?
                startingPrice,
                stdAuction.start,
                stdAuction.auctionPeriod
            );
    ...
    function _mint(
        ...
        if (currentBid.minted) revert ItemMinted(dropId, itemId);
...
struct StandardAuctionDrop {
    ...
    uint32 revision;
    uint32 numItems;
    EnumerableMap.UintToUintMap itemIds;
}
