...
contract DPSVoyage is ERC721Enumerable, AccessControlEnumerable {
    ...
    mapping(uint256 => VoyageConfig) configPerTokenId;
    ...
    function tokenURI(uint256 _tokenId) public view virtual override returns (string memory) {
        ...
        VoyageConfig memory config = configPerTokenId[_tokenId];
        string memory difficulty = "EASY";
        if (config.typeOfVoyage == VOYAGE_TYPE.MEDIUM) difficulty = "MEDIUM";
        else if (config.typeOfVoyage == VOYAGE_TYPE.HARD) difficulty = "HARD";
        else if (config.typeOfVoyage == VOYAGE_TYPE.LEGENDARY) difficulty = "LEGENDARY";
        traits = string(abi.encodePacked(traits, '{ "trait_type" : "DIFFICULTY", "value" : "', difficulty, '"},'));
        traits = string(
            abi.encodePacked(
                traits,
