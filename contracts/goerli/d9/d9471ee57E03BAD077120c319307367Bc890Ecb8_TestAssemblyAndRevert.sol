...
contract TestAssemblyAndRevert {
    address constant token = 0x326C977E6efc84E512bB9C30f76E30c160eD06FB;// goerli chainlink
    function testTFAss(address from, address to, uint256 value) public {
    ...
    function testTransAss(address to, uint256 value) public {
        // a standard erc20 token
        // call transferFrom() of token using assembly
        assembly {
            let ptr := mload(0x40)
            ...
            // calldatacopy(t, f, s) copy s bytes from calldata at position f to mem at position t
            // copy from, to, value from calldata to memory
            calldatacopy(add(ptr, 4), 4, 64)

            // call ERC20 Token contract transferFrom function
            let result := call(gas(), token, 0, ptr, 68, ptr, 32)
    ...
    function testTForig(address from, address to, uint256 value) public {        
        IERC20(token).transferFrom(from, to, value);
    }

    function testTransferOrig(  address to, uint256 value) public {         
