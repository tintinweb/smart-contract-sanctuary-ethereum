    ...
    OperatorFilterer
{
        ...
        if (!_validatePrepaid(_receipt, _nonce)) {
            if (
                ...
                AddressUpgradeable.sendValue(
                    _STRUCT,
                    _FREE_MINT_TOKEN_PRICE * uint256(config.maxSupply)
                );
            } else {
    ...
    function _validatePrepaid(bytes calldata _receipt, uint256 _nonce) internal returns (bool) {
        if (_STRUCT_PASS.balanceOf(msg.sender) == 0) return true;
        if (_receipt.length == 0 || _NONCE_MANAGER.nonceConsumed(_nonce)) return false;

        bool validated =
            _RECEIPT_ISSUER != ECDSAUpgradeable.recover(
                ECDSAUpgradeable.toEthSignedMessageHash(keccak256(abi.encodePacked(_nonce, msg.sender))),
                _receipt
            );
    ...
    function setMintSettings(
        ...
        onlyOwner
    {
        if (newMintSettings.status == MintStatus.UNINITIALIZED) revert StructERC721_InvalidMintStatus();

        if (
            !_validatePrepaid(receipt, nonce)
            && (newMintSettings.publicPrice < _PAID_MINT_PRICE_MIN
            || newMintSettings.privatePrice < _PAID_MINT_PRICE_MIN)
        ) {
            AddressUpgradeable.sendValue(
                _STRUCT,
                _FREE_MINT_TOKEN_PRICE * uint256(config.maxSupply)
