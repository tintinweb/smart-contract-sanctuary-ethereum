...
/// @author Mellow Finance
///
/// @notice An interface contract to buffer funds between the Refinery and the Interswap
contract InterswapBuffer is IInterswapBuffer, AccessControl, Initializable {
    ...
    /// @inheritdoc IInterswapBuffer
    string public constant override version = "2.2.0";

    /// @notice The refinery address.
    address public refinery;

    /// @notice The public interswap address for each address.
    ...
    /// @notice The yieldTokens of each underlying supported by the Refinery.
    mapping(address => address[]) public _yieldTokens;

    /// @notice The total amount of an underlying token that has been exchanged into the interswap, and has not been claimed.
    mapping(address => uint256) public currentExchanged;

    /// @notice The underlying-tokens registered in the InterswapBuffer.
    address[] public registeredUnderlyings;

    /// @notice The debt-token used by the InterswapBuffer.
    address public debtToken;

    /// @notice A mapping of weighting schemas to be used in actions taken on the Refinery (burn, deposit).
    ...
    /// @param _debtToken The debt token minted by the Refinery and accepted by the Interswap.
    ...
    /// @dev Only allows the interswap to call the modified function
    ///
    /// Reverts if the caller is not a correct interswap.
    ///
    /// @param underlyingToken the underlying token associated with the interswap.
    ...
    /// @notice Runs an action on the Refinery according to a given weighting schema.
    ///
    ...
    /// @notice Deposits funds into the Refinery.
    ///
    ...
    /// @notice Withdraws funds from the Refinery.
    ///
    ...
    /// @notice Pull necessary funds from the Refinery and exchange them.
    ///
