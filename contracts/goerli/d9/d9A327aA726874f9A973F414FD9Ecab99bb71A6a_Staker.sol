...
error Stake_NoAmount();
error Stake_DeadlineNotReached();
error Stake_AlreadyCompleted();
error Stake_NotEnoughStaked();
...
contract Staker {
  ...
  constructor(address exampleExternalContractAddress) {
    ...
    deadline = block.timestamp + 72 * 3600;
    state = State.STAKING;
  }

  function stake() public payable {
    require(state == State.STAKING, 'Staking is over');
  ...
  function execute() public {
    if (block.timestamp <= deadline) {
      revert Stake_DeadlineNotReached();
    }
    if (exampleExternalContract.completed()) {
      revert Stake_AlreadyCompleted();
    }
    if (address(this).balance < threshold) {
      state = State.OPEN_FOR_WITHDRAW;
    } else {
  ...
  function withdraw() public {
    if (state != State.OPEN_FOR_WITHDRAW) {
      revert Stake_NotOpenForWithdraw();
    }
    require(balances[msg.sender] > 0, 'No staking');
