...
contract GeneralAssimilator is IAssimilator {
    ...
    IOracle public immutable quoteOracle;
    uint256 public immutable baseOracleDecimals;
    uint256 public immutable quoteOracleDecimals;
    ...
    constructor(
        ...
        baseOracleDecimals = 10 ** uint256(IERC20Detailed(address(_baseOracle)).decimals());
        quoteOracleDecimals = 10 ** uint256(IERC20Detailed(address(_quoteOracle)).decimals());
    }

    // this is here in order to meet the IAssimilator interface
    ...
    function getBaseRate() public view returns (uint256) {
        ...
        require(rate > 0, "ORACLE_PRICE_ZERO");
    ...
    function getQuoteRate() public view returns (uint256) {
        (
            uint80 roundId,
            int256 rate,
            uint256 startedAt,
    ...
    // (relative to base token)
    function viewRawAmount(int128 _amount) external view override returns (uint256 amount_) {
        uint256 _rate = getBaseRate();

        amount_ = _amount.mulu(baseDecimals).mul(baseOracleDecimals).div(_rate);
    }
    ...
    function viewRawAmountLPRatio(
        ...
        uint256 _quoteRate = getQuoteRate();

        baseTokenBal = baseTokenBal.mul(1e18).div(_baseWeight);
        quoteTokenBal = quoteTokenBal.mul(_quoteRate).mul(1e18).div(_quoteWeight).div(quoteOracleDecimals);
        uint256 _ratio = quoteTokenBal.mul(baseDecimals).div(baseTokenBal);
        amount_ = _amount.mulu(baseDecimals).mul(quoteDecimals).div(_ratio);
    }
    ...
    function viewNumeraireAmount(uint256 _amount) external view override returns (int128 amount_) {
        ...
        amount_ = _amount.mul(_rate).div(baseOracleDecimals).divu(baseDecimals);
    }
    ...
    function viewNumeraireBalance(address vault, bytes32 poolId) external view override returns (int128 balance_) {
        ...
        balance_ = baseTokenBal.mul(_rate).div(baseOracleDecimals).divu(baseDecimals);
    }
    ...
    function viewNumeraireAmountAndBalance(
        ...
        uint256 _baseRate = getBaseRate();
        amount_ = _amount.mul(_baseRate).div(baseOracleDecimals).divu(baseDecimals);

        (uint256 baseTokenBal, ) = _getBalancesFromVault(vault, poolId, address(quoteToken));

        balance_ = baseTokenBal.mul(_baseRate).div(baseOracleDecimals).divu(baseDecimals);
    }
    ...
    function viewNumeraireBalanceLPRatio(
        ...
        uint256 _quoteRate = getQuoteRate();
        quoteTokenBal = quoteTokenBal.mul(_quoteRate).mul(1e18).div(_quoteWeight).div(quoteOracleDecimals);
        uint256 _ratio = quoteTokenBal.mul(baseDecimals).div(
          baseTokenBal.mul(1e18).div(_baseWeight)
        );

        balance_ = baseTokenBal.mul(_ratio).div(quoteDecimals).divu(baseDecimals);
    }
}
