...
contract XLATokenSaleMachine is Ownable {
    ...
    uint256 public constant k = 34500000000; // in wei
    uint256 public constant initialPrice = 100000000000000000; // 0.1 ETH in wei
    ...
    function calculateCost(uint256 _amount) public view returns (uint256 cost) {
        uint256 tmp = ((k * presaleToken.totalSupply() + (initialPrice * 10**18)) * _amount + (k / 2) * (_amount ** 2));
        cost = tmp / 10 ** 36;
    }
     ...
     */
    function calculateAmount(uint256 _cost) public view returns (int256 amount) {
        uint256 supply = presaleToken.totalSupply();
        uint underSqrt = sqrt(
            (2 * supply * k + 2 * (initialPrice * 10**18)) ** 2 + 8 * k * (_cost * 10**18)
        ); // shiftet by 24 decs for accuracy -> after sqrt only 12
        int temp = (
            (-2) * int256(supply) * int256(k) - 2 * int256(initialPrice)
        ) * (10 ** 18); // shifted to match decimals of sqrt
        amount = ((temp + int256(underSqrt)) * (10 ** 18)) / (2 * int256(k)); // shift again by 18 decs to assure accuracy
    }
    ...
    function buyFixedEthCost(bytes32 _affCode) external payable {
        uint256 amount = uint256(calculateAmount(msg.value));
