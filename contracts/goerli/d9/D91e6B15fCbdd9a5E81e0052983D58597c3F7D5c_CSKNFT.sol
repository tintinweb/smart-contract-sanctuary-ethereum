    ...
    Initializable
{
    ...
    uint256 public TOTAL_SUPPLY = 5_555;
    ...
    function setPeriods(MintType _mintType, uint256 startDate, uint256 _endTime)
        public
        onlyRole(DEV_ROLE)
    {   
        if(_mintType == MintType.Mint) {
            mintDates.MINT_START_DATE = startDate;
            mintDates.MINT_END_DATE = _endTime;
        } else if (_mintType == MintType.Whitelist) {
            mintDates.START_WHITELIST = startDate;
            mintDates.END_WHITELIST = _endTime;
        } else if (_mintType == MintType.Waitlist) {
            mintDates.START_WAITLIST = startDate;
            mintDates.END_WAITLIST = _endTime;
        }
    }
    ...
    function mint(
        ...
        if (_mintType == MintType.Mint) {
            require(block.timestamp >= mintDates.MINT_START_DATE, "not started.");
            require(block.timestamp <= mintDates.MINT_END_DATE, "ended.");
        } else if (_mintType == MintType.Whitelist) {
            require(block.timestamp >= mintDates.START_WHITELIST, "Wl not started.");
            require(block.timestamp <= mintDates.END_WHITELIST, "Wl ended.");
        } else if (_mintType == MintType.Waitlist) {
            require(block.timestamp >= mintDates.START_WAITLIST, "Waitlist not started.");
            require(block.timestamp <= mintDates.END_WAITLIST, "Waitlist ended.");
        }
