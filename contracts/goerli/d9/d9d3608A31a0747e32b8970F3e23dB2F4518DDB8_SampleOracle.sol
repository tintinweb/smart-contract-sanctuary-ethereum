...
contract SampleOracle is ChainlinkClient, Ownable {
    ...
    // Todo: _shop_name and _vendor and tag 
    // @notice: example json {"product":{"title":"Burton Custom Freestyle 151","body_html":"<strong>Good snowboard!</strong>","vendor":"Burton","product_type":"Snowboard","tags":["Barnes & Noble","Big Air","Johns Fav"]}}
    // @dev: Only body_html is alocated in memory. In the future, it is assumed that body_html will be modified by checkHtml.
    function createProductRequest(string calldata _shop_name,string calldata _title,string memory _body_html,string calldata _vendor,string calldata _product_type,string calldata _tag)
        public
        onlyOwner
        returns (bytes32 requestId)
    {
        ...
        req.add("shop_name",_shop_name);
        req.add("title", _title);
        req.add("body_html", _body_html);
        req.add("vendor", _vendor);
        req.add("product_type", _product_type);
        req.add("tag",_tag);
