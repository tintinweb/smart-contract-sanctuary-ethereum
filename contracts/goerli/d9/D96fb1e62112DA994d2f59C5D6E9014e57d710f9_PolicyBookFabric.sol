...
contract PolicyBookFabric is IPolicyBookFabric, OwnableUpgradeable, AbstractDependant {
    ...
    function createLeveragePools(
        ...
        ILeveragePool(address(_userLeverageProxy)).__UserLeveragePool_init(
            _contractType,
            _description,
            _projectSymbol
        );
...
contract PolicyBookFacade is IPolicyBookFacade, AbstractDependant, Initializable {
    ...
    IPolicyBookAdmin public policyBookAdmin;
    ILeveragePool public reinsurancePool; // deprecated
    ...
    modifier onlyPolicyBookAdmin() {
        require(msg.sender == address(policyBookAdmin), "PBF: Not a PBA");
        _;
    }

    modifier onlyLeveragePool() {
        require(policyBookRegistry.isLeveragePool(msg.sender), "PBF: only LeveragePool");
        _;
    }

    modifier onlyPolicyBook() {
        require(msg.sender == address(policyBook), "PBF: Not a policy book");
        _;
    }
    ...
    function submitClaimAndInitializeVoting(string calldata evidenceURI) external override {
        policyBook.submitClaimAndInitializeVoting(msg.sender, evidenceURI, false);
    }

    function submitAppealAndInitializeVoting(string calldata evidenceURI) external override {
        policyBook.submitClaimAndInitializeVoting(msg.sender, evidenceURI, true);
    }
    ...
    function requestWithdrawal(uint256 _tokensToWithdraw) external override {
        ...
        uint256 _availableSTBLBalance = _getUserAvailableSTBL(msg.sender);
        policyBook.requestWithdrawal(
            _tokensToWithdraw,
            _availableSTBLBalance,
            _getPendingWithdrawalAmount(),
            msg.sender
        );
    ...
    function _addLiquidity(
        ...
        _releaseCompoundedLiquidity(_liquidityAmount);
    ...
    function _buyPolicy(
        ...
        uint256 _demandIndex =
            demandBook.getDemandIndex(_policyHolderAddr, policyBook.insuranceContractAddress());

        if (_demandIndex > 0) {
            IDemandBook.DemandStatus status = demandBook.getDemandStatus(_demandIndex);
            require(
                status == IDemandBook.DemandStatus.CLOSED ||
                    status == IDemandBook.DemandStatus.ENDED,
                "PBF: There is a demand for this project"
            );
        }

        uint256 _deployedCompoundedLiquidity =
            policyBook.buyPolicy(
                IPolicyBook.BuyPolicyParameters(
                    _policyBuyerAddr,
                    _policyHolderAddr,
                    _epochsNumber,
                    _coverTokens,
        ...
        // deploy compound liquidity
        if (_deployedCompoundedLiquidity > 0) {
            compoundPool.deployLiquidity(address(policyBook), _deployedCompoundedLiquidity);
        }
    ...
    function releaseCompoundedLiquidity(uint256 _amount) public override onlyPolicyBook {
        _releaseCompoundedLiquidity(_amount);
    }

    function _releaseCompoundedLiquidity(uint256 _amount) internal {
        uint256 _totalCompoundedLiquidity = compoundPool.poolsUsage(address(policyBook));
        if (_totalCompoundedLiquidity > 0) {
            _amount = _amount > _totalCompoundedLiquidity ? _totalCompoundedLiquidity : _amount;

            compoundPool.undeployLiquidity(address(policyBook), _amount);
        }
    }
    ...
    /// @notice deploy leverage funds (LP lStable)
    /// @param  deployedAmount uint256 the deployed amount to be added or substracted from the total liquidity
    function deployLeverageFundsAfterRebalance(uint256 deployedAmount)
        external
        override
        onlyLeveragePool
    {
        ...
        totalLeveragedLiquidity = _LUuserLeveragePool;

        ///TODO to be removed before dein release
        // reset reinsurance pool balances
        VUreinsurnacePool = 0;
        LUreinsurnacePool = 0;
    ...
    function _deployLeveragedFunds() internal {
        ...
        for (uint256 i = 0; i < _userLeverageArr.length; i = uncheckedInc(i)) {
            ...
            uint256 _deployedAmount =
                ILeveragePool(_userLeverageArr[i]).deployLeverageStableToCoveragePools();
            // update user leverage pool apy after rebalancing
            ILeveragePool(_userLeverageArr[i]).forceUpdateBMICoverStakingRewardMultiplier();
    ...
    function setMPLs(uint256 _leveragePoolMPL) external override onlyPolicyBookAdmin {
        userleveragedMPL = _leveragePoolMPL;
    }
    ...
    function getClaimApprovalAmount(address user) external view override returns (uint256) {
        IPolicyBook.PolicyHolder memory policyHolder = userStats(user);

        return
            IPriceFeed(priceFeed).howManyDEINsInUSDT(
                DecimalsConverter.convertFrom18(policyHolder.coverTokens.uncheckedDiv(100), 6)
            );
    }
    ...
    {
        uint256 _pendingWithdrawlAmount = _getPendingWithdrawalAmount();
        ...
        uint256 _totalCoverWithPendingWithdraw =
            newTotalCoverTokens.add(_coverTokens).add(_pendingWithdrawlAmount);

        require(
            _totalCoverWithPendingWithdraw <= newTotalLiquidity.add(_availableCompoundLiquidity),
            "PBF: Not enough liquidity"
        );

        uint256 _deployedCompoundedLiquidity =
            _totalCoverWithPendingWithdraw.trySub(newTotalLiquidity);

        (totalSeconds, totalPrice, pricePercentage) = getPolicyPrice(
            msg.sender,
            _epochsNumber,
            _coverTokens.sub(_deployedCompoundedLiquidity),
            newTotalCoverTokens,
            newTotalLiquidity,
    ...
    {
        require(
            _coverTokens.add(_deployedCompoundedLiquidity) >= MINUMUM_COVERAGE,
            "PBF: Wrong cover"
        );
        uint256 _maxEpoch;

        _deployedCompoundedLiquidity > 0
            ? _maxEpoch = MAXIMUM_EPOCHS_FOR_COMPOUND_LIQUIDITY
            : _maxEpoch = policyBookAdmin.maxEpoch();

        require(_epochsNumber > 0 && _epochsNumber <= _maxEpoch, "PBF: Wrong epoch duration");
        ...
        (totalPrice, pricePercentage) = policyQuote.getQuotePredefined(
            IPolicyQuote.PolicyQuoteParameters(
                totalSeconds,
                _coverTokens,
                _deployedCompoundedLiquidity,
                newTotalCoverTokens,
    ...
    function getPremiumDistributionEpoch() public view override returns (uint256) {
    ...
    {
        currentDistribution = policyBook.lastPremiumDistributionAmount();
        newTotalLiquidity = policyBook.totalLiquidity();
        ...
        for (uint256 i = lastEpoch.tryAdd(1); i <= distributionEpoch; i = uncheckedInc(i)) {
            currentDistribution += policyBook.premiumDistributionDeltas(i);
    ...
    function _getUserAvailableSTBL(address _userAddr) internal view returns (uint256) {
        (uint256 withdrawalAmount, , ) = policyBook.withdrawalsInfo(_userAddr);

        uint256 availableSTBL =
            policyBook.convertBMIXToSTBL(
                IERC20(address(policyBook)).balanceOf(_userAddr).add(withdrawalAmount)
            );
    ...
    {
        newTotalLiquidity = policyBook.totalLiquidity();
        newTotalCoverTokens = policyBook.totalCoverTokens();
        availableCompoundLiquidity = compoundPool.getCompoundLiquidityForPool(address(policyBook));

        uint256 lastEpoch = policyBook.lastPremiumDistributionEpoch();
        uint256 currentEpoch = getPremiumDistributionEpoch();

        if (currentEpoch > lastEpoch) {
            (, , newTotalLiquidity) = getPremiumsDistribution(lastEpoch, currentEpoch);
        }

        uint256 newDistributionEpoch =
            Math.min(policyBook.getEpoch(block.timestamp), policyBookAdmin.maxEpoch());

        for (
            uint256 i = policyBook.lastDistributionEpoch();
            i < newDistributionEpoch;
            i = uncheckedInc(i)
        ) {
            newTotalCoverTokens = newTotalCoverTokens.sub(policyBook.epochAmounts(i));
            availableCompoundLiquidity = availableCompoundLiquidity.add(
                policyBook.epochCompoundedAmounts(i)
            );
        }
    }
    ...
    function getAPY() public view override returns (uint256) {
        uint256 lastEpoch = policyBook.lastPremiumDistributionEpoch();
        uint256 currentEpoch = getPremiumDistributionEpoch();
        int256 premiumDistributionAmount = policyBook.lastPremiumDistributionAmount();

        // simulates addLiquidity()
        if (currentEpoch > lastEpoch) {
            (premiumDistributionAmount, currentEpoch, ) = getPremiumsDistribution(
                lastEpoch,
                currentEpoch
            );
        }

        premiumDistributionAmount += policyBook.premiumDistributionDeltas(currentEpoch.tryAdd(1));

        return
            uint256(premiumDistributionAmount).mul(365).mul(10**7).uncheckedDiv(
                policyBook.convertBMIXToSTBL(IERC20(address(policyBook)).totalSupply()).add(
                    APY_TOKENS
                )
            );
    }
    ...
    function userStats(address _user)
        ...
        view
        override
        returns (IPolicyBook.PolicyHolder memory)
    {
        (
            uint256 coverTokens,
            ...
            uint256 paid,
            uint256 protocolFee
        ) = (policyBook.policyHolders(_user));

        return
            IPolicyBook.PolicyHolder(
                coverTokens.add(policyBook.policyHoldersCompoundedAmount(_user)),
                startEpochNumber,
                endEpochNumber,
                paid,
                protocolFee
        ...
        )
    {
        ...
        _maxCapacities = _totalSTBLLiquidity.sub(newTotalCoverTokens).sub(
            _getPendingWithdrawalAmount()
        );

        _stakedSTBL = rewardsGenerator.getStakedPolicyBookSTBL(address(policyBook));
        _annualProfitYields = getAPY();

        uint256 possibleCoverage = Math.min(ANNUAL_COVERAGE_TOKENS, _maxCapacities);
        // capacity include the compounded liquidity
        _maxCapacities = _maxCapacities.add(_availableCompoundLiquidity);
        _totalLeveragedLiquidity = totalLeveragedLiquidity;
        ...
        if (possibleCoverage >= MINUMUM_COVERAGE) {
            ...
            _annualInsuranceCost = _annualInsuranceCost
                .mul(ANNUAL_COVERAGE_TOKENS.mul(PRECISION).uncheckedDiv(possibleCoverage))
                .uncheckedDiv(PRECISION)
                .uncheckedDiv(10);
        }

        _bmiXRatio = policyBook.convertBMIXToSTBL(10**18);
    }
    ...
    function _getPendingWithdrawalAmount() internal view returns (uint256) {
        return
            liquidityRegistry.getPendingWithdrawalAmountByPolicyBook(
                address(policyBook),
                liquidityRegistry.getPoolWithdrawlRequestsUsersListCount(address(policyBook))
            );
    }
}
