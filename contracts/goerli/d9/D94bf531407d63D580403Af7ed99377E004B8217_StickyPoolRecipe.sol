...
contract StickyPoolRecipe {
    ...
    */
    ...
    /**
     * @notice Sets the current fee percentage to deploy.
     * @param _feeAmount The fee amount priced in ETH
     */
    ...
    function getFeeInTokens(address _addr) public view returns (uint256)  {
        ...
        if (rate == 0) {
            try IERC20(_addr).totalSupply() returns (uint256 _totalSupply) {
                return _totalSupply * feePercentage /  PERCENTAGE_PRECISION;
            } catch {
                return 0;
            }
        }
        return 1e18 * feeAmount / rate;
    }
    ...
    function prepareStickyPoolETH(
        ...
        returns (address)
    {
        require(msg.value >= feeAmount, "Failed to transfer minimumFee");
        if (msg.value > 0) {
