...
contract dminer is Ownable, ReentrancyGuard {
    ...
    // `keccak256(bytes("TicketPurchased(uint256 indexed ticketId, uint256 indexed roundId, address indexed buyer)"))`.
    bytes32 private constant _PURCHASE_EVENT_SIGNATURE =
    0x79e3446d8ca300bfe42170fbfb841d4ac3811016b68af2505fb72fec9a2bb249;
    ...
    */
    ...
    function buyTicketsNON(   uint256 quantity,  uint256 tokenIdHolding,  uint256 miningLevel) external payable nonReentrant {
        //require(msg.value == ticketPrice.mul(quantity), "DIGI: Send proper ticket fees");
     //   if(msg.value != ticketPrice.mul(quantity)) revert BadFees();
     ...
     //   if(checkSign(signature, quantity, tokenIdHolding, miningLevel, _msgSender(), roundId) != signerWallet) revert FakeSignature();
        ...
        for(uint256 i= 0;i<quantity;){
            ...
            TicketKey[roundId][ticketId] = Ticket(buyer, tokenIdHolding, false, winningOre);
            ticketHolding[buyer][roundId].push(ticketId);
            emit TicketPurchased(ticketId, roundId, buyer);
            unchecked {
             claimCounter[roundId][winningOre]++;               
	        i++;
	        }
    ...
    function getTicketKey(uint256 ticketId) private view returns(uint256){
        return uint(keccak256(abi.encodePacked(msg.sender, ticketId, block.difficulty, block.timestamp, block.number))) % 10**5;
    }
    ...
    function ticketKeyFactory(uint256 ticketKey, uint256 multiplier) public pure returns(uint256){
        return (ticketKey.add(multiplier).mul(multiplier)).sub(multiplier) % 10**5;
    }
