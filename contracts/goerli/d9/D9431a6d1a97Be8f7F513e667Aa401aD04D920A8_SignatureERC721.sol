...
import { OperatorFilterer } from "../third-party/vectorized-closedsea/OperatorFilterer.sol";
import "../mint-voucher/MintVoucher.sol";

contract SignatureERC721 is Pausable, MintVoucherContract, ERC721A, ERC2981, OperatorFilterer {
    ...
    ) MintVoucherContract(signer) ERC721A(name, symbol) {
    ...
    /**
     * @notice Mint an NFT - restricted to those with a valid MintVoucher and signature
     * @param voucher The MintVoucher that contains all the mint details such as quantity and price
     * @param signature The signed mint voucher data that must be signed by an authorized signer
     */
    function mintWithVoucher(MintVoucher calldata voucher, bytes calldata signature) public payable whenNotPaused {
        _mintWithVoucher(voucher, signature);
    }

    /**
     * @dev Caller for contract type minting function
     * ignored: "to" param - address used is from voucher - address to sent token(s) to
     * @param voucher The MintVoucher that contains all the mint details such as quantity and price
     */
    function _handleMint(address, MintVoucher calldata voucher) internal override {
        _safeMint(voucher.netRecipient, voucher.quantity);
    }

    /**
     * @notice Mint a quantity of tokens to a specified address
     * @dev Restricted to admins
     */
    function adminMintMultiple(address to, uint256 quantity) external onlyRole(DEFAULT_ADMIN_ROLE) {
     ...
     * @dev Restricted to admins
     * @param baseURI_ The baseURI for the NFT metadata
     */
    ...
    function setContractURI(string memory contractURI_) external onlyRole(DEFAULT_ADMIN_ROLE) {
        ...
        emit ContractURIUpdated(prevURI, contractURI_);
    }

    /**
     * @notice Pause the mint
     * @dev Restricted to admins
     */
    ...
    /**
     * @dev Overriding this function from the ERC721A contract
     */
    ...
    function resetTokenRoyalty(uint256 tokenId) external virtual onlyRole(DEFAULT_ADMIN_ROLE) {
    ...
    function _isPriorityOperator(address operator) internal pure override returns (bool) {
        // OpenSea Seaport Conduit for mainnet and goerli
 ...
 * Author: https://github.com/Vectorized/closedsea/blob/main/src/OperatorFilterer.sol
 *
 * Add `onlyAllowedOperator` modifier to the transferFrom() and both safeTransferFrom()
...
/**
 * @dev Base functions for MintVoucher verification and payment handling
 */

/**
 * @dev Revert with an error if zero address or insufficient funds
 */

error InsufficientPayment();
error CannotBeZeroAddress(string _message);
error InvalidVoucher(string _message);
error InvalidPaymentAmount(string _message);
...
abstract contract MintVoucherContract is MintVoucherVerification {
     ...
     *               this signer.
     */
    constructor(address signer) MintVoucherVerification(signer) {}

    /**
     * @dev Caller for contract type minting function
     * @param to An address to sent token(s) to
     * @param voucher The MintVoucher that contains all the mint details such as quantity and price
     */
     ...
     * @param signature The signed mint voucher data that must be signed by an authorized signer
     */
    function _mintWithVoucher(MintVoucher calldata voucher, bytes calldata signature) public payable virtual {
    ...
    function _handleEthPayment(MintVoucher calldata voucher) internal {
        ...
        if (msg.value > 0) {
            ...
            if (voucher.initialRecipientAmount > msg.value) {
                revert InvalidPaymentAmount("initialRecipientAmount can not be greater than eth payment");
            }
            if (voucher.initialRecipientAmount > 0) {
                if (voucher.initialRecipient == address(0)) {
                    revert CannotBeZeroAddress("initialRecipient can not be a zero address");
                }
            ...
            if (msg.value > voucher.initialRecipientAmount) {
                if (voucher.netRecipient == address(0)) {
                    revert CannotBeZeroAddress("netRecipient can not be a zero address");
                }
    ...
    /**
     * @notice handle an ERC20 payments
     * @param voucher The MintVoucher that contains all the mint details such as quantity and price
     */
    function _handleERC20Payment(MintVoucher calldata voucher) internal {
        if ((voucher.price * voucher.quantity) < 0) {
            revert InvalidVoucher("Payment can not be less than zero");
        }

        if (voucher.initialRecipientAmount > (voucher.price * voucher.quantity)) {
            revert InvalidPaymentAmount("initialRecipientAmount can not be greater than erc20 payment");
        }
        if (voucher.initialRecipientAmount > 0) {
            if (voucher.initialRecipient == address(0)) {
                revert CannotBeZeroAddress("initialRecipient can not be a zero address");
    ...
    /**
     * @notice Cancel a voucher by providing the voucher nonce
 ...
 */
abstract contract MintVoucherVerification is AccessControl, MintVoucherErrors, MintVoucherCapability {
    // Save on gas by caching address(this) in an immutable variable
    address private immutable _CACHED_CONTRACT_ADDRESS;
     ...
     *      the situation. Currency should be zero address if payment in native currency.
    ...
    constructor(address signer) {
        _CACHED_CONTRACT_ADDRESS = address(this);
...
/**
 * @dev This is the interfave for the MintVoucherCapability
 */
