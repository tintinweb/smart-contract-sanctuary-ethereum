...
contract OcarioFeature is IFeature, IOcarioFeature, FixinCommon, RouterTokenHelper {
    ...
    constructor(address weth, address uniFactory) public RouterTokenHelper(uniFactory, weth) {}
    ...
    function migrate() external returns (bytes4 success) {
        ...
        _registerFeatureFunction(this.swapCallback.selector);
    ...
    /// @param minBuyAmount Minimum amount of the last token in the path to buy.
    /// @return buyAmount Amount of the last token in the path bought.
    function sellEthForTokenToOcario(bytes memory encodedPath, uint256 minBuyAmount, address recipient) public payable override returns (uint256 buyAmount) {
        // Wrap ETH.
    ...
    /// @param recipient The recipient of the bought tokens. Can be zero for sender.
    /// @return buyAmount Amount of ETH bought.
    function sellTokenForEthToOcario(bytes memory encodedPath, uint256 sellAmount, uint256 minBuyAmount, address payable recipient) public override returns (uint256 buyAmount) {
        ExactInputParams memory item;
        item.path = encodedPath;
    ...
    function swapCallback(int256 deltaQty0, int256 deltaQty1, bytes calldata data) external override {
        ...
        require(msg.sender == address(_getPool(tokenIn, tokenOut, fee)), "OcarioFeature: invalid callback sender");

        (bool isExactInput, uint256 amountToTransfer) = deltaQty0 > 0 ? (tokenIn < tokenOut, uint256(deltaQty0)) : (tokenOut < tokenIn, uint256(deltaQty1));
        if (isExactInput) {
    ...
    function _swapExactOutputInternal(uint256 amountOut, address recipient, uint160 limitSqrtP, SwapCallbackData memory data) private returns (uint256 amountIn) {
        ...
        (int256 amount0Delta, int256 amount1Delta) = _getPool(tokenIn, tokenOut, fee).swap(
            ...
            -amountOut.toInt256(),
            isFromToken0,
            limitSqrtP == 0 ? (isFromToken0 ? TickMath.MAX_SQRT_RATIO - 1 : TickMath.MIN_SQRT_RATIO + 1) : limitSqrtP,
            abi.encode(data)
        );

        uint256 receivedAmountOut;
        (amountIn, receivedAmountOut) = isFromToken0 ? (uint256(amount1Delta), uint256(-amount0Delta)) : (uint256(amount0Delta), uint256(-amount1Delta));
    ...
    function _swap(ExactInputParams memory params) private returns (uint256 amountOut) {
    ...
    /// @dev Performs a single exact input swap
    function _swapExactInputInternal(uint256 amountIn, address recipient, uint160 limitSqrtP, SwapCallbackData memory data) private returns (uint256 amountOut) {
        ...
        (int256 amount0, int256 amount1) = _getPool(tokenIn, tokenOut, fee).swap(
            ...
            amountIn.toInt256(),
            isFromToken0,
            limitSqrtP == 0 ? (isFromToken0 ? TickMath.MIN_SQRT_RATIO + 1 : TickMath.MAX_SQRT_RATIO - 1) : limitSqrtP,
            abi.encode(data)
        );
        return uint256(-(isFromToken0 ? amount1 : amount0));
    }

    function _getPool(address tokenA, address tokenB, uint24 fee) private view returns (IOcarioPool) {
