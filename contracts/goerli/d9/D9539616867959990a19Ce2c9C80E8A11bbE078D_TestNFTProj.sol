...
contract TestNFTProj is Ownable, ERC721A, ReentrancyGuard {
    ...
    uint256 public maxMint = 5;     // mint per wallet
    ...
    uint256 public whitelistPrice = 5000000000000000; //0.05
    uint256 public publicPrice = 9000000000000000; //0.09
    ...
    function whiteListMint(uint256 quantity, bytes calldata mintKey) external payable nonReentrant {
        require(mintPause == 0, "Mint is not live");
        require(mintPhase == 1 || mintPhase == 2 || mintPhase == 3, "Whitelist mint is not live!");
        require(totalSupply() + quantity <= COLLECTION_SIZE, "All tokens were minted out!");
        require(quantity <= maxMint, "Quantity is too high!");
        ...
        require(quantity * whitelistPrice <= msg.value, "Value is not enought!");
    ...
    /// @param mintKey was generated off-chain and one address has only one key.
    function waitListMint(uint256 quantity, bytes calldata mintKey) external payable nonReentrant {
        require(mintPause == 0, "Mint is not live");
        require(mintPhase == 2 || mintPhase == 3, "WaitList mint is not live!");
        require(totalSupply() + quantity <= COLLECTION_SIZE, "All tokens were minted out!");
        require(quantity <= maxMint, "Quantity is too high!");
        require(!claimedKeys[mintKey], "Address and MintKey was used!");
    ...
    function publicMint(uint256 quantity) external payable nonReentrant {
        ...
        require(totalSupply() + quantity <= COLLECTION_SIZE, "All tokens were minted out!");
        require(quantity <= maxMint, "Quantity is too high!");
        require(quantity * publicPrice <= msg.value, "Value is not enought!");
        _safeMint(msg.sender, quantity);
    }
