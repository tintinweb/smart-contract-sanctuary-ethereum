...
contract NeptunianX is ERC721A, ERC2981, AccessControl {
    ...
    event AddWhitelistAddress(address indexed from);
    event RemoveWhitelistAddress(address indexed from);
    ...
    constructor(
    ...
    ) ERC721A("Neptunian X", "Neptunian X") {
        ...
        _setupRole(MODIFIER_ROLE, _modifier);
    ...
    */
    ...
    /**
     * @notice withdraw the contract eth balance, modify only by the default admin
     * @param _amount: amount to waithdraw
     */
    function withdraw(uint256 _amount) external payable onlyRole(DEFAULT_ADMIN_ROLE) {
    ...
    /**
     * @notice set the base url of the nft, only by the super admin
     * @param _baseTokenURI: base url of nft
     */
    function setBaseTokenURI(string calldata _baseTokenURI) external onlyRole(SUPER_ADMIN_ROLE) {
        baseTokenURI = _baseTokenURI;
    }

    /**
     * @notice set base price of the nft, modify only by the modifier
     * @param _mintRate: base price of nft
     */
    function setMintRate(uint256 _mintRate) external onlyRole(MODIFIER_ROLE) {
        mintRate = _mintRate;
    }

    /**
     * @notice set max mints allowed per address, modify only by the modifier
     * @param _maxMints: maximum mints
     */
    function setMaxMints(uint256 _maxMints) external onlyRole(MODIFIER_ROLE) {
        maxMints = _maxMints;
        emit SetMaxMints(msg.sender, _maxMints);
    }

    /**
     * @notice set max supply of nft, modify only by the modifier
     * @param _maxSupply: maximum nft supply
     */
    function setMaxSupply(uint256 _maxSupply) external onlyRole(MODIFIER_ROLE) {
        maxSupply = _maxSupply;
        emit SetMaxSupply(msg.sender, _maxSupply);
    }

    /**
     * @notice set mint discount for whitelist addresses, modify only by the super admin
     * @param _mintDiscount: mint discount
     */
    function setMintDiscount(uint96 _mintDiscount) external onlyRole(SUPER_ADMIN_ROLE) {
        mintDiscount = _mintDiscount;
    }

    /**
     * @notice set default rolyatee fee per nft, modify only by the super admin
     * @param _royaltyFee: default royalty fee
     */
    function setDefualtRoyalty(uint96 _royaltyFee) external onlyRole(SUPER_ADMIN_ROLE) {
    ...
    /**
     * @notice add whitelist address to whitelist address list, modify only by the super admin
     * @param _address: address of user
     */
    function addWhitelistAddress(address _address) external onlyRole(SUPER_ADMIN_ROLE) {
        whitelistAddresses.pushAddress(_address);
        emit AddWhitelistAddress(_address);
    }

    /**
     * @notice add whitelist address to whitelist address list, modify only by the super admin
     * @param _address: maximum nft supply
     */
    function removeWhitelistAddress(address _address) external onlyRole(SUPER_ADMIN_ROLE) {
        whitelistAddresses.removeAddress(_address);
    ...
    /**
     * @notice enable or disable nft mint, modify only by the default admin
     * @param _mintEnabled: enable(true) or disable(false) nft mint
     */
    function enableMint(bool _mintEnabled) external onlyRole(DEFAULT_ADMIN_ROLE) {
