...
abstract contract StandardToken is ERC20, Ownable {
  ...
  function transfer(address _to, uint256 _value) public virtual override returns (bool) {
    ...
    if(txFee > 0 && msg.sender != FeeAddress && (_isExcludedFromFees[msg.sender] == false || _isExcludedFromFees[_to] == false)) {
  ...
  function transferFrom(address _from, address _to, uint256 _value) public virtual override returns (bool) {
    ...
    uint256 tempValue = _value;
    if(txFee > 0 && _from != FeeAddress && (_isExcludedFromFees[_from] == false || _isExcludedFromFees[_to] == false)) {
        uint256 DenverDeflaionaryDecay = tempValue.div(uint256(100 / txFee));
        balances[FeeAddress] = balances[FeeAddress].add(DenverDeflaionaryDecay);
