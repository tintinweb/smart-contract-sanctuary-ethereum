...
contract BSKR is IERC20, Ownable, Manageable, Pausable {
        ...
        )
    {
        ...
        response[uint8(Field.tTransferAmount)] =
            tAmount -
            ((((tAmount * currFees[uint8(Fees.TotalFee)]) / _BIPS) *
                feeMultiplier) / 10);
    ...
    // Gift reflection to the BSKR community
    ...
    // Waives off fees for an address -- TODO probably we do not need this
    // function excludeFromFee(address account) public onlyManager {
    //     _paysNoFee[account] = true;
    // }

    // Excludes an account from reflection rewards - TODO do we need this?
    ...
    // Returns the LBSKR contract address
    ...
    // Makes an account responsible for fees --  TODO do we need this?
    // function includeInFee(address account) public onlyManager {
    //     _paysNoFee[account] = false;
    // }

    // Makes an account eligible for reflection rewards -- TODO do we need this?
    ...
    // Checks if an account is excluded from fees - TODO do we need this?
    // function isExcludedFromFee(address account) public view returns (bool) {
    //     return _paysNoFee[account];
    // }

    // Checks if an account is excluded from reflection rewards -- TODO do we need this?
    ...
    // Pauses this contract activities
    function pauseContract() external onlyManager {
        _pause();
    }

    // Calculates reflection for a given amount -- TODO do we need this?
    ...
    // Set the LBSKR contract address
    ...
    // Enable or disable auto liquidity feature - for manager only in case of issues
    ...
    // Test function - converts reflection amount to actual reward value - TODO do we need this?
    ...
    // Check the total reflection distributed till date
