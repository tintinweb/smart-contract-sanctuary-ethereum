...
contract BlackErcFactory is Ownable {
    ...
    function createToken(
        ...
        if(isDeployDestoryToken){ 
            ...
            tokenContract.transfer(fundAddress, destoryAmounts*fundPer/100);
            tokenContract.transfer(blackHole, destoryAmounts*(100-fundPer)/100);
        }
    ...
    function setFundAddress(address fund) external onlyOwner {
        require(fundAddress == address(0), "devAddress contract already set");
        fundAddress = fund;
    }

    function setFundPer(uint256 fundPer) external onlyOwner {
        fundPer = fundPer;
    }
}