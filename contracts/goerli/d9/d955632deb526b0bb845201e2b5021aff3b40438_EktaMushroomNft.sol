...
// File: my contract/ektaMushroomNft.sol
...
contract WhitelistMerkleTree is Ownable {
    using Strings for uint256;
    ...
    function _isWhitelisted(address addr, bytes32[] calldata proof)
        internal
        view
        returns (bool)
    {
    ...
    function isWhitelisted(address addr, bytes32[] calldata proof)
        ...
        returns (bool)
    {
        return _isWhitelisted(addr, proof);
    }
}
    ...
    WhitelistMerkleTree
{
    ...
    /// @notice mint NFTs in batch
    /// @param _mintQty number of (payable) Nfts to mint
    /// @param _merkleProof merkle proof of the whitelist merkle tree
    function batchMint(uint256 _mintQty, bytes32[] calldata _merkleProof)
        external
        payable
        nonReentrant
    {
        require(!isMintingPaused, "Minting is paused");
        require(_mintQty != 0, "Quantity can not be 0");
        require(_mintQty + totalSupply() <= maxSupply, "Exceeds max supply");
        ...
        if (owner() == msg.sender) {
            ...
            require(msg.value == _mintQty * price, "incorrect price");
        ...
        _batchMint(_mintQty, msg.sender);
    }
