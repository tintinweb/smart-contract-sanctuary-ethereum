...
contract bageContract {
    function balanceOf(address account) public view virtual returns (uint256) {
    }
}


contract KaprikaIDGoerly is ERC721, ERC721URIStorage, ERC721Burnable, Ownable, EIP712, ERC721Votes {

    bageContract public myBageContract;
    ...
    address public bageContractAdress = 0x49402D2E16090042922e78801c31bE0441D9a522;


    constructor() ERC721("KaprikaIDGoerly", "KIDt") EIP712("KaprikaIDGoerly", "1") {
         myBageContract = bageContract(bageContractAdress);
    }

    function Mint(address to, string memory uri) public payable {
        uint256 tokenId = _tokenIdCounter.current();
        require(balanceOf(to) < 1, "Oops! address alredy have Kaprika ID");
    ...
    function senderHasBage() external view returns (bool) {
        uint256 bageCount = myBageContract.balanceOf(msg.sender);
        if ( bageCount > 0 ) {
            return true;
        } else {
            return false;
...
// end test
        ...
        override(ERC721)
    {
        if (canTransfer) {
            ...
            require(from == address(0), "The Kaprika ID is a soulbound token and can't be transferred");  
        }
        if (from != address(0)) {
