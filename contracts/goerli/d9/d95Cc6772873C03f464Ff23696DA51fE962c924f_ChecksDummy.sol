...
contract ChecksDummy is ERC721 {
    constructor() ERC721("MyToken", "MTK") {}

    mapping (uint256 => Checks.Check) public checks;

    function safeMint(address to, uint256 tokenId, Checks.Check calldata check) public {
        _safeMint(to, tokenId);
        checks[tokenId] = check;
    }

     function getCheck(uint256 tokenId) external view returns (Checks.Check memory check){
        return checks[tokenId];
     }
}
