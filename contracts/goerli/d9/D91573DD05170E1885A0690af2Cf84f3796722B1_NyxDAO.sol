...
abstract contract NyxVault {
    function sendTo(address addr, uint256 value) external virtual;
}
...
abstract contract NyxProposalHandler
{
    ...
    function createProposal(uint256 proposalTypeInt, bytes[] memory params, address author) public virtual returns(uint256);
...
contract NyxDAO is Ownable {
    ...
    {
        // require(hasRole(STAKEHOLDER_ROLE, msg.sender), message);
        require(msg.sender == owner() || representativeRights[msg.sender][proposalTypeInt] == 1, "not representative for this proposal type");
        _;
    }

    modifier onlyProposalHandler()
    {
        require(msg.sender == owner() || msg.sender == address(proposalCreator), "not called by proposal creator");
        _;
    }
    ...
    * of the NyxDAO contract while NyxProposalHandler is not.
    *
    ...
    */
        ...
        onlyOwner
    {
        for (uint256 idx = 0; idx <= getNumOfProposals(proposalTypeInt); idx++)
        {
            ...
            {
                NyxProposalHandler.ProposalConf memory proposalConf = proposalCreator.getProposalConf(proposalTypeInt, idx);
                ...
                voteConf.livePeriod = proposalConf.creationTime + minimumVotingPeriod;
            }   
        }
    }
    ...
    function openRedeem(uint256 liquidity, uint256 price)
        public 
        onlyOwner
    {
        redeemOpen = true;
    ...
    */
    ...
    function createVoteConf(uint256 proposalTypeInt, uint256 proposalId, address author)
        internal
        returns (VoteConf memory)
    {
        address[] memory voters;
        VoteConf memory voteConf = VoteConf(proposalTypeInt, proposalId, author, false, false, 0, 0, block.timestamp + minimumVotingPeriod,
            voters, block.timestamp, block.number);
        voteConfMapping[proposalTypeInt][proposalId] = voteConf;
        return voteConf;        
    }

    function updateVoteConf(uint256 proposalTypeInt, uint256 proposalId, address author)
        external
        onlyOwner
        returns (VoteConf memory)
    {
    ...
    function getProposalType(uint256 proposalTypeInt)
        external view
        returns (NyxProposalHandler.ProposalType memory)
    {
        return proposalCreator.getProposalType(proposalTypeInt);
    }
    ...
    function createProposal(uint256 proposalTypeInt, bytes[] memory params)
        ...
        returns (uint256)
    {
        uint256 proposalId = proposalCreator.createProposal(proposalTypeInt, params, msg.sender);
        createVoteConf(proposalTypeInt, proposalId, msg.sender);
    ...
    function deleteProposal(uint256 proposalTypeInt, uint256 proposalId)
        ...
        onlyOwner
    {
        proposalCreator.deleteProposal(proposalTypeInt, proposalId);
    ...
    function getProposalConfs(uint256 proposalTypeInt)
        external view
        withProposalHandlerSetted
        returns (NyxProposalHandler.ProposalConf[] memory)
    {
        return proposalCreator.getProposalConfs(proposalTypeInt);
    }
    ...
    {
        return proposalCreator.getProposalReadables(proposalTypeInt);
    }
        ...
        public
    {
        ...
        address vault = nft_contract.vault_address();
        uint256 balance = vault.balance;

        uint256 redeemableAmount;
        uint256 tokenIdPrice = nft_contract.wavePrice(tokenId);
        uint256 tokenIdBalance = nft_contract.balanceOf(msg.sender, tokenId);

        require(tokenIdBalance >= nNfts, "not enough NFTs to redeem that much");

        uint256 tokenIdCurrentSupply = nft_contract.waveCurrentSupply(tokenId);
        redeemableAmount += (tokenIdPrice * tokenIdBalance * balance) / tokenIdCurrentSupply;
        NyxVault(vault).sendTo(msg.sender, redeemableAmount);
    }
}
