...
contract WhitelightNFT is ERC721AQueryable, BaseTokenURI {
  ...
  function setDistribution1address(address _distribution1address) external onlyOwner {
    distribution1address = _distribution1address;
  }

  function setDistribution2address(address _distribution2address) external onlyOwner {
  ...
  function distributeFunds(
    ...
    (bool success, ) = address(to).call{value: (amount * percentageFunds) / FEE_DENOMINATOR}("");
    require(success, "Unable to distribute funds");
  }

  function withdraw() external onlyOwner {
    uint256 balance = address(this).balance;
    distributeFunds(distribution1address, balance, distribution1Percentage);
    distributeFunds(distribution2address, balance, distribution2Percentage);
  }
  ...
  function withdrawToken(address tokenAddress) external onlyOwner {
    ...
    token.transfer(distribution1address, (balance * distribution1Percentage) / FEE_DENOMINATOR);
    token.transfer(distribution2address, (balance * distribution2Percentage) / FEE_DENOMINATOR);
  }
}
