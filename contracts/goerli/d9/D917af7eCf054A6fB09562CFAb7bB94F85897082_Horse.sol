...
abstract contract VRF {
    function rollDice() public virtual;
}
...
contract Horse is Ownable, ERC721AWithRoyalties, Pausable, PaymentSplitter {
    ...
    function reqSeed() public onlyOwner {
        require(randSeed == 0, "RANDOM SEED ALREADY SET");
        VRF VRFV2 = VRF(Index.getAddress("VRFV2"));
        VRFV2.rollDice();
    }

    function mintTransfer(uint256[] memory ids) public whenNotPaused {
        require(ids.length > 0, "LIST OF TOKENS CANNOT BE BLANK");
        refreshContracts();
        ERC721 HorsePassContract = ERC721(HorsePassAddress);
        uint256 randnum = randSeed;

        for (uint256 i = 0; i < ids.length; i++) {
            randnum = ((randnum * ids[i]) % ids.length) + 1;
            require(
            ...
            transferFrom(address(this), msg.sender, tokenIDs[randnum]);
            delete tokenIDs[randnum];
            tokenIDs[randnum] = tokenIDs[tokenIDs.length - 1];
            tokenIDs.pop;
        }
    }
