...
import {YieldAggregatorV3} from "./YieldAggregatorV3.sol";
    ...
    VRFConsumerBaseV2
{
    ...
    ) VRFConsumerBaseV2(vrfCoordinatorV2) YieldAggregatorV3(_provider, _weth) {
    ...
    function fulfillRandomWords(
        ...
        s_lastTimeStamp = block.timestamp;
        withdrawAaveYield();
        uint256 weth_balance = getWethBalance();
        exchangeToEth(weth_balance);
...
contract YieldAggregatorV3 is Ownable {
    //////// Variables ////////
    ...
    //////// Events ////////
    event AaveWithdrawn(uint256 withdrawAmount);
    event AaveEntered(uint256 value);
    ...
    //////// Main functions ////////
    // function enterAave(uint256 _value) internal {
    //     require(_value != 0, "transferFailed");
    //     ExchangeToWeth(_value);
    //     approveWeth(_value);
    //     depositToAave(_value);
    //     emit AaveEntered(_value);
    // }
    ...
    function withdrawAaveYield() internal {
        uint256 balance = getYieldBalance();
        uint256 yield = POOL.withdraw(address(WETH), balance, address(this));
        emit AaveWithdrawn(yield);
    }

    function withdrawAaveAll() public onlyOwner {
        uint256 amount = getUserAccountData();
        totalDeposited = 0;
    ...
    function exchangeToEth(uint256 _value) internal {
        bool success = WETH.approve(address(this), _value);
    ...
    function getYieldBalance() public view returns (uint256) {
        return getUserAccountData() - totalDeposited;
    }
