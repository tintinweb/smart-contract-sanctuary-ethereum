...
contract ITokenStakingV2 is Initializable, OwnableUpgradeable {
    using SafeMathUpgradeable for uint256;
    ...
    */
        ...
        )
    {
        ...
        (_amount,,,,,,) = IMasterChefV2(astraStakingContract).userInfo(_pid,_userAddress);
        (, _currentMultiplier, _maxMultiplier) = IMasterChefV2(astraStakingContract).stakingScoreAndMultiplier(_pid,_userAddress,_amount);
        return (_currentMultiplier, _maxMultiplier);
    }
    ...
    function claimAstra(uint256 _pid) public returns (uint256) {
        ...
        claimableReward = claimableReward
            ...
            .div(MULTIPLIER_DECIMAL)
            .sub(slashedReward);
        uint256 slashDays = block.timestamp.sub(user.lastDeposit).div(
            SECONDS_IN_DAY
        );
    ...
    function deposit(
        ...
        _deposit(_pid, _amount, _vault, msg.sender);
    }
    ...
    function depositFromOtherContract(
        ...
        _deposit(_pid, _amount, _vault, _userAddress);
    }
    ...
    function _deposit(
        ...
        updateUserDepositDetails(_pid, _userAddress, _amount, _vault);

        (
            _currentMultiplier,
            _maxMultiplier
        ) = stakingScoreAndMultiplier(_pid, _userAddress);
        ...
        if (user.cooldown == false) {
            ...
            if (
                ...
                user.cooldowntimestamp.add(
                    SECONDS_IN_DAY.mul(
                        coolDownPeriodTime.add(coolDownClaimTime)
                    )
                )
            ) {
                user.cooldown = true;
    ...
    function _withdraw(uint256 _pid, bool _withStake) internal {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
        uint256 _amount = checkEligibleAmount(_pid, msg.sender);
        ...
        if (pool.totalStaked.sub(_amount) > 0) {
            (
                _currentMultiplier,
                _maxMultiplier
            ) = stakingScoreAndMultiplier(_pid, msg.sender);
            newPoolMaxMultiplier = updateStakedAmount
                .mul(_maxMultiplier)
                .add(pool.totalStaked.mul(pool.maxMultiplier))
                .sub(user.amount.mul(user.maxMultiplier))
