...
contract ProxyIDO is Ownable {
     ...
     event createLog(string  func,address AddressMsg,address owner,address admin,IDO.projectInfo data);
     ...
     function open() public onlyOwner{
      ...
      function createIDO(IDO.projectInfo memory data) external {
       ...
       uint timeVlaue=userTime[msg.sender];
        ...
        //进行转账操作
        //授权合约得到代币
        //SafeERC20.safeApprove(token,address(contractAddr), data.inTokenCapacity);
        //token.approve(address(contractAddr), data.inTokenCapacity);

        //safeTransfer(data.tokenB,address(this),data.inTokenCapacity);
         //safeTransfer(data.tokenB,address(contractAddr),data.inTokenCapacity);
         //SafeERC20.safeTransferFrom(token,address(msg.sender),address(this), data.inTokenCapacity);
         //SafeERC20.safeTransferFrom(token,address(this),address(contractAddr), data.inTokenCapacity);
      
        contractAddr.initIdo();
        emit createLog("create", address(contractAddr),msg.sender,adminAddr,data);
       ...
       function createIDOTest() external {
        ...
        emit createLog("createIDOTest", address(contractAddr),msg.sender,adminAddr,data);
