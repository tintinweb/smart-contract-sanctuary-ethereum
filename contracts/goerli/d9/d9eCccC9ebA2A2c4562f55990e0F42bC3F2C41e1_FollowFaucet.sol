...
contract FollowFaucet {
    ...
    function _init(
        ...
        uint256 _amountInDollar,
        uint256 _starCommisionPercent
    ) public onlyOwner {
        require(
            _priceFeedAddress != address(0) && _amountInDollar >= 0 && _starCommisionPercent < 100,
    ...
    function like(uint256 SignetId, address SignetIdOwner) public {
        ...
        LibSignetStorage.like(msg.sender, SignetId, SignetIdOwner);
    ...
    function unlike(uint256 SignetId, address SignetIdOwner) public {
        if (msg.sender == SignetIdOwner) revert Wrong__UserSubmitted();
        LibSignetStorage.unlike(msg.sender, SignetId, SignetIdOwner);
    ...
    function star(address SignetIdOwner, uint256 SignetId) external payable {
        if (msg.sender == SignetIdOwner) revert Wrong__UserSubmitted();
        if (msg.value.getConversionRate(priceFeed) < LibSignetStorage.getAppreciateAmount())
...
library LibSignetStorage {
    ...
    function unfollow(address msgSender, address signetor) internal {
        AppStorage storage s = getStorage();
        if (checkfollowed(signetor, msgSender) == true) {
            ...
            if (totalFollowing == j) {
                ...
                s.fm.following[msgSender].FollowingNum -= 1;
                s.fm.following[msgSender].followedWho[j - 1] = s
                    .fm
                    .following[msgSender]
                    .followedWho[s.fm.following[msgSender].followedWho.length - 1];
                // for (j; j < ; j++) {
                //     ;
                // }
                s.fm.following[msgSender].followedWho.pop();
        ...
        if (checklikeable(SignetId) == true) {
            if (
                (s.fm.signetState[SignetId].SignetIdOwner != msgSender &&
                    s.fm.signetState[SignetId].SignetIdOwner == SignetIdOwner) == true
            ) {
