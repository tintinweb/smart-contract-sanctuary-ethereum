...
contract ClaimingRegistry is IClaimingRegistry, Initializable, AbstractDependant {
    ...
    mapping(address => mapping(uint256 => mapping(address => uint256))) internal _allClaimsToIndex; // book -> demandIndex (0 if policy) -> claimer -> index
    ...
    mapping(address => mapping(uint256 => EnumerableSet.UintSet)) internal _bookClaims; // book -> demandIndex (0 if policy) -> index
    IERC20Metadata public stblToken;
    uint256 public stblDecimals;

    IDemandBook public demandBook;

    address public claimVotingFacadeAddress;

    event AppealPending(ClaimProvenance claimProvenance, uint256 claimIndex);
    event ClaimPending(ClaimProvenance claimProvenance, uint256 claimIndex);
    event ClaimAccepted(
        address claimer,
        address bookAddress,
        uint256 demandIndex,
    ...
    modifier onlyClaimVotingFacade() {
        require(
            claimVotingFacadeAddress == msg.sender,
            "ClaimingRegistry: Caller is not a ClaimVotingFacade contract"
        );
        _;
    }
        ...
        onlyInjectorOrZero
    {
        ...
        claimVotingFacadeAddress = _contractsRegistry.getClaimVotingFacadeContract();
        ...
        returns (bool)
    {
        return (_allClaimsByIndexInfo[index].status == ClaimStatus.PENDING &&
            _allClaimsByIndexInfo[index].dateSubmitted.tryAdd(votingDuration(index)) <=
            block.timestamp);
    }
        ...
        returns (bool)
    {
        return (_allClaimsByIndexInfo[index].status == ClaimStatus.REJECTED_CAN_APPEAL &&
            _allClaimsByIndexInfo[index].dateEnded.tryAdd(policyRegistry.STILL_CLAIMABLE_FOR()) <=
            block.timestamp);
    }

    function _isClaimExpired(uint256 index) internal view withExistingClaim(index) returns (bool) {
        return (_allClaimsByIndexInfo[index].status == ClaimStatus.PENDING &&
            _allClaimsByIndexInfo[index].dateSubmitted.tryAdd(validityDuration(index)) <=
            block.timestamp);
    }
    ...
    function votingDuration(uint256 index) public view override returns (uint256) {
        return anonymousVotingDuration(index).tryAdd(EXPOSE_VOTE_DURATION);
    }
    ...
    function validityDuration(uint256 index)
        ...
        returns (uint256)
    {
        return votingDuration(index).tryAdd(VIEW_VERDICT_DURATION);
    }
    ...
    function anyoneCanCalculateClaimResultAfter(uint256 index)
        ...
        returns (uint256)
    {
        return votingDuration(index).tryAdd(PRIVATE_CLAIM_DURATION);
    }
        ...
        returns (bool)
    {
        ...
        return
            ...
            _allClaimsByIndexInfo[index].claimProvenance.claimer == calculator;
    }
        ...
        returns (bool isActive)
    {
        if (claimProvenance.provenance == Provenance.POLICY) {
            ...
            isActive = demandBook.isDemandActive(claimProvenance.demandIndex);
        }
    }
    ...
    function canBuyNewBook(ClaimProvenance calldata claimProvenance) external override {
        require(msg.sender == claimProvenance.bookAddress, "ClaimingRegistry: Not allowed");

        uint256 index =
            _allClaimsToIndex[claimProvenance.bookAddress][claimProvenance.demandIndex][
                claimProvenance.claimer
            ];
        ...
        require(
            ...
            "ClaimingRegistry: Claim is pending"
        );
    ...
    function canWithdrawLockedBMI(uint256 index) public view returns (bool) {
        return
            ...
            (_allClaimsByIndexInfo[index].status == ClaimStatus.ACCEPTED &&
                ...
                !_isBookActive(_allClaimsByIndexInfo[index].claimProvenance));
    }
        ...
        returns (WithdrawalStatus)
    {
        ...
        if (
            block.timestamp >=
            claimWithdrawalInfo[index].readyToWithdrawDate.tryAdd(READY_TO_WITHDRAW_PERIOD)
        ) {
        ...
        returns (WithdrawalStatus)
    {
        ...
        if (
            block.timestamp >=
            rewardWithdrawalInfo[voter].readyToWithdrawDate.tryAdd(READY_TO_WITHDRAW_PERIOD)
        ) {
            return WithdrawalStatus.EXPIRED;
        }
        ...
        if (bookAddress == address(demandBook)) {
            if (
                _hasProcedureOngoing(
                    bookAddress,
                    demandIndex,
                    getBookClaimsCount(bookAddress, demandIndex)
                )
            ) {
                return true;
            }
        } else if (policyBookRegistry.isUserLeveragePool(bookAddress)) {
            ILeveragePortfolio userLeveragePool = ILeveragePortfolio(bookAddress);
            ...
            for (uint256 i = 0; i < _coveragePools.length; i = uncheckedInc(i)) {
                if (
                    _hasProcedureOngoing(
                        _coveragePools[i],
                        0,
                        getBookClaimsCount(_coveragePools[i], 0)
                    )
                ) {
                    return true;
                }
            ...
            if (_hasProcedureOngoing(bookAddress, 0, getBookClaimsCount(bookAddress, 0))) {
                return true;
            }
        }
    ...
    function getBookClaimsCount(address bookAddress, uint256 demandIndex)
        internal
        view
        returns (uint256)
    {
    ...
    function _hasProcedureOngoing(
        ...
        for (uint256 i = 0; i < limit; i = uncheckedInc(i)) {
            uint256 index = _bookClaims[bookAddress][demandIndex].at(i);
            ...
            if (
                !(status == ClaimStatus.EXPIRED || // has expired
                    ...
                    (status == ClaimStatus.ACCEPTED &&
                        getClaimWithdrawalStatus(index) == WithdrawalStatus.EXPIRED &&
                        !_isBookActive(_allClaimsByIndexInfo[index].claimProvenance))) // has been accepted and never withdrawn but cannot request withdraw anymore
            ) {
                return true;
            }
        }
    ...
    function submitClaim(
        ...
        uint256 cover,
        bool appeal
    ) external override onlyClaimVotingFacade returns (uint256 _newClaimIndex) {
        uint256 index =
            _allClaimsToIndex[claimProvenance.bookAddress][claimProvenance.demandIndex][
                claimProvenance.claimer
            ];
        ClaimStatus status =
            _myClaims[claimProvenance.claimer].contains(index)
                ? claimStatus(index)
                : ClaimStatus.CAN_CLAIM;
        bool active = _isBookActive(claimProvenance);
            ...
            (!appeal && active && status == ClaimStatus.CAN_CLAIM) ||
                ...
                (!appeal &&
                    ...
                    (status == ClaimStatus.REJECTED ||
                        (_bookStartTime(claimProvenance) >
        ...
        if (!appeal) {
            emit ClaimPending(claimProvenance, _newClaimIndex);
        } else {
            emit AppealPending(claimProvenance, _newClaimIndex);
        }
    }
    ...
    function isClaimAnonymouslyVotable(uint256 index) external view override returns (bool) {
        return (_pendingClaimsIndexes.contains(index) &&
            _allClaimsByIndexInfo[index].dateSubmitted.tryAdd(anonymousVotingDuration(index)) >
            block.timestamp);
    }
    ...
    function isClaimExposablyVotable(uint256 index) external view override returns (bool) {
        ...
        return (dateSubmitted.tryAdd(anonymousDuration.tryAdd(EXPOSE_VOTE_DURATION)) >
            block.timestamp &&
            dateSubmitted.tryAdd(anonymousDuration) < block.timestamp);
    }
    ...
    function canClaimBeCalculatedByAnyone(uint256 index) public view override returns (bool) {
        return
            ...
            _allClaimsByIndexInfo[index].dateSubmitted.tryAdd(
    ...
    function countBookClaimerClaims(address claimer) external view override returns (uint256) {
    ...
    function claimIndex(ClaimProvenance calldata claimProvenance)
        external
        view
        override
        returns (uint256)
    ...
    function bookStatus(
        ...
        address bookAddress,
        uint256 demandIndex
    ) external view override returns (ClaimStatus) {
        if (!_isBookActive(ClaimProvenance(provenance, claimer, bookAddress, demandIndex))) {
            return ClaimStatus.UNCLAIMABLE;
        }

        uint256 index = _allClaimsToIndex[bookAddress][demandIndex][claimer];
        ...
        bool newPolicyBought =
            _bookStartTime(_allClaimsByIndexInfo[index].claimProvenance) >
                _allClaimsByIndexInfo[index].dateSubmitted;
    ...
    function claimOwner(uint256 index) external view override returns (address) {
        return _allClaimsByIndexInfo[index].claimProvenance.claimer;
    }

    /// @notice Gets the policybook address of a claim with a certain index
    /// @param index uint256, numeric index value
    /// @return address
    function claimBook(uint256 index) external view override returns (ClaimProvenance memory) {
        return _allClaimsByIndexInfo[index].claimProvenance;
    }
        ...
        returns (ClaimInfo memory _claimInfo)
    {
        _claimInfo = ClaimInfo(
            _allClaimsByIndexInfo[index].claimProvenance,
    ...
    function getAllPendingClaimsAmount(bool isRebalancing, uint256 _limit)
        external
        view
        override
    ...
    function getAllPendingRewardsAmount(bool isRebalancing, uint256 _limit)
        external
        view
        override
        ...
        returns (uint256)
    {
        ...
        for (uint256 i = 0; i < _claimIndexes.length; i = uncheckedInc(i)) {
    ...
    function _modifyClaim(uint256 index, ClaimStatus status) internal {
        address claimer = _allClaimsByIndexInfo[index].claimProvenance.claimer;
        address bookAddress = _allClaimsByIndexInfo[index].claimProvenance.bookAddress;
        uint256 demandIndex = _allClaimsByIndexInfo[index].claimProvenance.demandIndex;
        ...
        if (status == ClaimStatus.ACCEPTED) {
            ...
            emit ClaimAccepted(claimer, bookAddress, demandIndex, claimAmount, index);
        } else if (status == ClaimStatus.EXPIRED) {
            _allClaimsByIndexInfo[index].status = ClaimStatus.EXPIRED;

            emit ClaimExpired(claimer, bookAddress, demandIndex, index);
        } else if (!_allClaimsByIndexInfo[index].appeal) {
            _allClaimsByIndexInfo[index].status = ClaimStatus.REJECTED_CAN_APPEAL;

            emit ClaimRejected(claimer, bookAddress, demandIndex, index);
        } else {
            _allClaimsByIndexInfo[index].status = ClaimStatus.REJECTED;
            delete _allClaimsToIndex[bookAddress][demandIndex][claimer];
            _bookClaims[bookAddress][demandIndex].remove(index);

            emit AppealRejected(claimer, bookAddress, demandIndex, index);
        }
        ...
        _commitClaim(index, claimer, false);
    }

    function acceptClaim(uint256 index, uint256 amount) external override onlyClaimVotingFacade {
        _allClaimsByIndexInfo[index].claimRefund = amount;
        _modifyClaim(index, ClaimStatus.ACCEPTED);
    }

    function rejectClaim(uint256 index) external override onlyClaimVotingFacade {
        _modifyClaim(index, ClaimStatus.REJECTED);
    }

    function expireClaim(uint256 index) external override onlyClaimVotingFacade {
    ...
    function requestClaimWithdrawal(uint256 index) external override {
        ...
        require(
            _isBookActive(_allClaimsByIndexInfo[index].claimProvenance) &&
                _bookStartTime(_allClaimsByIndexInfo[index].claimProvenance) <
                _allClaimsByIndexInfo[index].dateEnded,
            "ClaimingRegistry: The book is expired"
        );
        require(
        ...
        onlyClaimVotingFacade
    {
        ...
        require(
            capitalPool.canRequestWithdraw(rewardAmount),
            "ClaimingRegistry: Not enough withdraw liquidity, please contact us"
        );
    ...
    function withdrawClaim(uint256 index) public virtual {
        ...
        if (!claimWithdrawalInfo[index].committed) {
            _commitClaim(index, msg.sender, true);
        ...
        claimVoting.transferBMILocked(index, claimer);
    ...
    function withdrawLockedBMI(uint256 index) public virtual {
        ...
        if (claimStatus(index) == ClaimStatus.ACCEPTED) {
            _commitClaim(index, claimer, true);
            _withdrawClaimRequestIndexList.remove(index);
            delete claimWithdrawalInfo[index];
        }
    ...
    function _commitClaim(
        ...
        if (_allClaimsByIndexInfo[index].claimProvenance.provenance == Provenance.POLICY) {
            if (isWithdrawn) {
                IPolicyBook(bookAddress).commitWithdrawnClaim(claimer);
            } else {
        ...
        } else if (_allClaimsByIndexInfo[index].claimProvenance.provenance == Provenance.DEMAND) {
            if (isWithdrawn) {
                demandBook.commitWithdrawnClaim(demandIndex);
            } else {
                demandBook.commitClaim(
                    demandIndex,
                    block.timestamp,
                    _allClaimsByIndexInfo[index].status // ACCEPTED, REJECTED_CAN_APPEAL, REJECTED, EXPIRED
                );
...
interface ICapitalPool {
    ...
    enum PoolType {COVERAGE, LEVERAGE, REINSURANCE, DEMAND}
    ...
    function addWithdrawalHardSTBL(uint256 _stblAmount) external;
