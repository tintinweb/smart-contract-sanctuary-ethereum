...
library Monetary {
    ...
    function equals(Monetary.Crypto memory self, Monetary.Crypto memory other) internal pure returns (bool) {
        return self.currency == other.currency && self.amount == other.amount;
    }

    function greaterThan(Monetary.Crypto memory self, Monetary.Crypto memory other) internal pure returns (bool) {
    ...
    function times(Monetary.Crypto memory self, uint value) internal pure returns (Monetary.Crypto memory) {
        return Crypto(self.amount * value, self.currency);
    }

    function divideBy(Monetary.Crypto memory self, uint value) internal pure returns (Monetary.Crypto memory) {
        return Crypto(self.amount / value, self.currency);
    }

    function sendTo(Monetary.Crypto memory self, address recipient) internal {
        if (!isZero(self)) {
            if (isNative(self)) payable(recipient).sendValue(self.amount);
            else if (isValidCurrency(self.currency)) IERC20(self.currency).safeTransferFrom(msg.sender, recipient, self.amount);
