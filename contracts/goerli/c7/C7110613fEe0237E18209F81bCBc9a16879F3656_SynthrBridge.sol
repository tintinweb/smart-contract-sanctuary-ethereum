...
abstract contract BaseBridge is Ownable, NonblockingLzApp {
    ...
    function _lzAdapterParam(uint16) internal view virtual returns (bytes memory);
...
import "./interfaces/ISynthAggregator.sol";
import "./interfaces/IDebtShareAggregator.sol";
import "./interfaces/ILiquidatorRewardsAggregator.sol";
import "./interfaces/IRewardEscrowV2Aggregator.sol";
...
contract SynthrBridge is BaseBridge {
    using BytesLib for bytes;

    address private _synthrAddressResolver;
    address private _synthrIssuer;
    address private _synthrExchanger;
    address private _synthrRewardEscrow;

    // LZ UAs for aggregation
    address private _collateralAggregator;
    address private _debtShareAggregator;
    address private _liquidatorRewardsAggregator;
    address private _synthAggregator;
    address private _rewardEscrowAggregator;
    ...
    function initialize(
        ...
        address __collateralAggregator,
        address __debtShareAggregator,
        address __liquidatorRewardsAggregator,
        address __synthAggregator,
        address __rewardEscrowAggregator,
        address __exchangeFeeAddress,
        uint16 __selfLZChainId
    ) external onlyOwner {
        _synthrAddressResolver = addressResolver;
        _synthrIssuer = issuer;
        _synthrRewardEscrow = rewardEscrow;
        _collateralAggregator = __collateralAggregator;
        _debtShareAggregator = __debtShareAggregator;
        _liquidatorRewardsAggregator = __liquidatorRewardsAggregator;
        _synthAggregator = __synthAggregator;
        _rewardEscrowAggregator = __rewardEscrowAggregator;
    ...
    modifier onlySynthr() {
        require (
            ...
            ISynthrIssuer(_synthrIssuer).synthsByAddress(msg.sender) != bytes32(0)
    ...
    function _nonblockingLzReceive(
        ...
        if (packetType == PT_SYNTH_MINT) {
            (
                ...
                bytes memory _accountForSynthBytes,
                bytes32 _synthKey,
                uint _synthAmount,
                uint _debtShare,
                uint16 _destChainId
            ...
            address _accountForSynth = _accountForSynthBytes.toAddress(0);

            _sendMint(_accountForCollateral, _collateralKey, _collateralAmount, _accountForSynth, _synthKey, _synthAmount, _debtShare, _destChainId);
            ...
            ISynthrIssuer(_synthrIssuer).destBurn(accountForSynth, synthKey, synthAmount, debtShare);
            ...
            if (destChainId == _selfLZChainId) {
                IExchanger(_synthrExchanger).updateDestinationForExchange(destAccount, destKey, destAmount);
            }

            _sendExchange(sourceAccount, sourceKey, sourceAmount, destAccount, destKey, destAmount, fee);
        } else if (packetType == PT_LIQUIDATE) {} else if (packetType == PT_REWARD_ESCROW_APPEND) {
    ...
    function sendMint(
        ...
        bytes memory lzPayload = abi.encode(
            ...
            _collateralKey,
            _collateralAmount,
            abi.encodePacked(_accountForSynth),
            _synthKey,
            _synthAmount,
            _debtShare,
            _destChainId
    ...
    function _sendMint(
        ...
        if (_collateralKey != bytes32(0) && _collateralAmount != 0) {
            ICollateralAggregator(_collateralAggregator).depositCollateral(_accountForCollateral, _collateralAmount, _collateralKey);
        }

        // update synth, debt share, liquidator reward
        if (_synthKey != bytes32(0) && _synthAmount != 0) {
            ISynthAggregator(_synthAggregator).issue(_synthKey, _accountForSynth, _synthAmount);
            IDebtShareAggregator(_debtShareAggregator).mint(_accountForSynth, _debtShare);
            ILiquidatorRewardsAggregator(_liquidatorRewardsAggregator).updateEntry(
                _collateralKey,
                IDebtShareAggregator(_debtShareAggregator).balanceOf(_accountForSynth),
                _accountForSynth
            );

            if (_destChainId != 0) {
                ISynthrIssuer(_synthrIssuer).destIssue(_accountForSynth, _synthKey, _synthAmount, _debtShare);
            }
        }
    }
    ...
    function _sendWithdraw(
        ...
        ICollateralAggregator(_collateralAggregator).withdrawCollateral(account, amount, collateralKey);
    }
    ...
    function sendBurn(
        ...
        bytes memory lzPayload = abi.encode(PT_SYNTH_BURN, abi.encodePacked(accountForSynth), synthKey, synthKey, synthAmount, debtShare);
    ...
    function _sendBurn(
        ...
        ISynthAggregator(_synthAggregator).burn(synthKey, accountForSynth, synthAmount);
        IDebtShareAggregator(_debtShareAggregator).burn(accountForSynth, debtShare);

        ILiquidatorRewardsAggregator(_liquidatorRewardsAggregator).updateEntry(synthKey, IDebtShareAggregator(_debtShareAggregator).balanceOf(accountForSynth), accountForSynth);
    }
    ...
    function _sendTransferFrom(
        ...
        ISynthAggregator(_synthAggregator).transferFrom(currencyKey, from, to, amount);
    }
    ...
    function _sendExchange(
        ...
        ISynthAggregator(_synthAggregator).burn(sourceKey, sourceAccount, sourceAmount);
        ISynthAggregator(_synthAggregator).issue(destKey, destAccount, destAmount);
        ISynthAggregator(_synthAggregator).issue(bytes32("sUSD"), _exchangeFeeAddress, fee);
    }
    ...
    function _sendLiquidate(
        ...
        uint currentDebtShareBalance = IDebtShareAggregator(_debtShareAggregator).balanceOf(account);

        ILiquidatorRewardsAggregator(_liquidatorRewardsAggregator).updateEntry(collateralKey, currentDebtShareBalance, account);

        IDebtShareAggregator(_debtShareAggregator).burn(account, debtShare);

        if (currentDebtShareBalance > debtShare) {
            ILiquidatorRewardsAggregator(_liquidatorRewardsAggregator).updateAccumulatedShare(
                collateralKey,
                notifyAmount / IDebtShareAggregator(_debtShareAggregator).balanceOf(account)
            );
        }

        ICollateralAggregator(_collateralAggregator).withdrawCollateral(account, collateralAmount, collateralKey);
    }
    ...
    function _sendREAppend(
        ...
        IRewardEscrowV2Aggregator(_rewardEscrowAggregator).append(currencyKey, account, amount);
    }
    ...
    function _sendVest(
        ...
        IRewardEscrowV2Aggregator(_rewardEscrowAggregator).vest(currencyKey, account, amount);
    }

    function _lzAdapterParam(uint16 packetType) internal view override returns (bytes memory) {
        uint16 version = 1;
        uint expectedGas;
        if (packetType == PT_SYNTH_MINT) {
    ...
    function setGasSynthMint(uint packetType, uint newGas) external onlyOwner {
...
interface ILiquidatorRewardsAggregator {
    ...
    function updateAccumulatedShare(bytes32 currencyKey, uint _increasedShare) external;
}
...
interface IRewardEscrowV2Aggregator {
    function append(
        bytes32 currencyKey,
        address account,
        uint amount
...
interface ISynthAggregator {
    function sendIssue(address account, uint amount) external;

    function sendBurn(address account, uint amount) external;
