...
/**
 * @dev ASM The Next Legend - Character contract
 */
contract Character is ICharacter, ERC721Royalty, MinterManaged {
    uint256 public totalSupply;
    string public baseURI;

    mapping(address => uint256[]) private _minted; // tokens minted by the wallet
    mapping(uint256 => bytes32) private _hash; // tokenId => hashId (used for tokenURI)

    event BaseURIChanged(string newBaseURI);

    constructor(address manager) MinterManaged(manager) ERC721("TNL Character", "TNLC") {}
    ...
    function mint(address recipient, bytes32 hashId) external onlyRole(MINTER_ROLE) returns (uint256) {
        ...
        if (_verifyUsedHash(recipient, hashId)) revert MintingError(TOKEN_ALREADY_MINTED, 0);

        _minted[recipient].push(totalSupply);
    ...
    /**
     * @notice How many tokens were minted by wallet
     * @return Amount of minted (but not necessary existing) tokens by wallet
     */
    function walletMinted(address addr) public view returns (uint256) {
        return _minted[addr].length;
    }
    ...
    /**
     * @notice Verify that hashID wasn't previously used
     * @return true if hash already was used
     */
    function _verifyUsedHash(address addr, bytes32 hashId) internal view returns (bool) {
        uint256[] memory tokens = _minted[addr];
        uint256 amount = tokens.length;

        for (uint256 i; i < amount; i++) {
            if (_hash[tokens[i]] == hashId) {
                return true;
            }
        }
        return false;
