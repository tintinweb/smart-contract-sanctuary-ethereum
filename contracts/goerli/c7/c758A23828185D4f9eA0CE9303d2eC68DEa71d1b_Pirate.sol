    ...
    UUPSUpgradeable
{
    ...
    function mint(uint256 amount, uint256[] calldata tokenIds) external onlyEOA {
        if (_launchDate > block.timestamp) {
            revert MintingUnavailable();
        }

        for (uint256 i; i < tokenIds.length; i++) {
            if (msg.sender != _stakes[tokenIds[i]].owner) {
                revert Unauthorized();
            }
        }
        ...
        if (amount < 1 || amount > maxAmount) {
            revert OutOfRange(1, maxAmount);
        }

        if (_mintCounts2[msg.sender] + amount > maxAmount) {
            revert MintLimitExceeded(maxAmount);
        }

        _mintCounts2[msg.sender] += amount;

        _mintCore(msg.sender, amount, false);
        _updateNonce();
    }
