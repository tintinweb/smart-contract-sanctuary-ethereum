...
// SPDX-License-Identifier: MIT
/* || Unseen Shades || 
This contract is inspired by LasogetteNFT contract thanks to the great Omnus chad dev (as a froggie I had to remove all the google trad french comments, sorry)
I have chosen to adopt the new OpenSea standard for royalties because, despite the obvious competition between various NFT marketplaces, 
I believe that royalties are essential for creators and provide the strongest motivation for an NFT collection to endure over time.
*/
...
import "../lib/IEPSPortal.sol";

contract UnseenShades is
    ERC721,
    Ownable,
    ERC721Burnable,
    DefaultOperatorFilterer
{
    ...
    // array of jpeg size
    uint256 public arrayOfJpegsize = 500;

    // array of each jpegs current price in wei, it's free for the first mint.
    uint256[] public jpegPrice = new uint256[](arrayOfJpegsize);

    // maximum allowed price is 10 ether
    uint256 public maxJpegPrice = 10 ether;
    ...
    error NftDoesNotExists();

    // ===================================
    // CONSTRUCTOR
    // ===================================
    constructor(
        uint256 baseMintPrice_,
        address payable treasuryAddress_,
        address eps_,
        string memory placeholderURI_,
        string memory baseURI_
    ) ERC721("UnseenShades", "UNSEEN") {
        baseMintPrice = baseMintPrice_;
     ...
     *
     */
    function publicMint(uint256 nftId_, bool useDelivery_) external payable {
        // test if minting is open
        if (!mintingOpen) {
            revert MintingNotOpen();
        }

        // test if the id provided is valid, and also
        // protects an out of bounds error
        if (nftId_ > arrayOfJpegsize - 1) {
            revert NftDoesNotExists();
        }

        uint256 nftMintPrice = getNftCurrentMintPrice(nftId_);

        if (msg.value != (nftMintPrice)) {
            revert IncorrectETHPayment({
                paid: msg.value,
                required: (nftMintPrice)
            });
        }
        ...
        _setNewMintPrice(nftId_);
    }

    /**
     * @dev getNftCurrentMintPrice to get the mint price using the jpg id
     */
    function getNftCurrentMintPrice(uint256 nftId_)
        public
        view
        returns (uint256)
    {
        return jpegPrice[nftId_];
    }

    /**
     * @dev getNftNextMintPrice to determine the mint price using the jpg id
     */
    function getNftNextMintPrice(uint256 nftId_) public view returns (uint256) {
        uint256 nextPrice = (jpegPrice[nftId_] * 14) / 10; // price is multiplied by 1.4 to get an exponential curve

        if (nextPrice > maxJpegPrice) {
            // price is capped
            return maxJpegPrice;
        } else {
            return nextPrice;
    ...
    /**
     * @dev _setNewMintPrice to set the new price after the NFT has been successfully minted
     */
    function _setNewMintPrice(uint256 nftId_) internal {
        uint256 newPrice = (jpegPrice[nftId_] * 14) / 10; // price is multiplied by 1.4 to get an exponential curve

        if (newPrice == 0) {
            jpegPrice[nftId_] = baseMintPrice;
        } else if (newPrice > maxJpegPrice) {
            jpegPrice[nftId_] = maxJpegPrice;
        } else {
            jpegPrice[nftId_] = newPrice;
        }
    }
    ...
    // ===================================
    // OVERRIDE ERC721 TRANSFER AND APPROVAL
    // ===================================
