...
contract release_v2 is ERC721, ERC721Enumerable, Ownable, Pausable, ReentrancyGuard {
    ...
    /// @notice The description for a PoS Phunk.
    ...
    /// @param _newPrice The new price to be set for a PoS Phunk.
    ...
    /// @notice The owner of the PoS Phunks contract can resume transactions.
    function unpause() public onlyOwner {
        _unpause();
    }

    /// @notice The PoS Phunk contract owner can withdraw ETH accumulated in the contract.
    ...
    //         v = uint(keccak256(abi.encodePacked("4c71ce6ba2ee0cfaa5acee977e8e67e2cd9b456dcdf1ab291519d32de27f4ece", block.timestamp, block.difficulty, toString(tokenId)))) % 10001;
    //         if ( assignedPunk[i] == v) {
    //             v = uint(keccak256(abi.encodePacked("4c71ce6ba2ee0cfaa5acee977e8e67e2cd9b456dcdf1ab291519d32de27f4ece", block.timestamp, block.difficulty, toString(tokenId)))) % 10001;
    //         }
    //     }
    ...
    // }
      ...
      function randomPunk(uint256 tokenId) public view returns (uint16) {
        uint256 v;
        for (uint i = 0; i < tokenId; i++) {
            v = uint(keccak256(abi.encodePacked("4c71ce6ba2ee0cfaa5acee977e8e67e2cd9b456dcdf1ab291519d32de27f4ece", block.timestamp, block.difficulty, toString(tokenId)))) % 11;
            if ( assignedPunk[i] == v) {
                v = uint(keccak256(abi.encodePacked("4c71ce6ba2ee0cfaa5acee977e8e67e2cd9b456dcdf1ab291519d32de27f4ece", block.timestamp, block.difficulty, toString(tokenId)))) % 11;
                i = 0;
            }
    ...
    /// @param tokenId The tokenId of a PoS Phunk used to select stored values and generate attributes.
