...
import "./StableInput/LendingPool.sol";

/// @notice This contract replaces some functionality in the StabilityPool contract
///         in order to make it play nice with non-LUSD tokens.
contract StabilityPoolDai is StabilityPool {
    ...
    // Transfer the DAI tokens from the user to the Stability Pool
    // and update its recorded DAI balance
    function _sendLUSDtoStabilityPool(address _address, uint _amount) internal override {
    ...
    // Send DAI to user and decrease teh DAI in the pool
    function _sendLUSDToDepositor(address _depositor, uint LUSDWithdrawal) internal override {
    ...
    // Used to adjust debt and collateral status of the Active Pool
    function _moveOffsetCollAndDebt(uint _collToAdd, uint _debtToOffset) internal override {
        ...
        // Give the offset debt to the Lending Pool
        lusdToken.transfer(address(lendingPool), _debtToOffset);
...
contract StabilityPool is LiquityBase, Ownable, CheckContract, IStabilityPool {
    ...
    */
    ...
    function _moveOffsetCollAndDebt(uint _collToAdd, uint _debtToOffset) internal virtual {
        IActivePool activePoolCached = activePool;
    ...
    */
    ...
    function _sendLUSDtoStabilityPool(address _address, uint _amount) internal virtual {
    ...
    function _sendLUSDToDepositor(address _depositor, uint LUSDWithdrawal) internal virtual {
        if (LUSDWithdrawal == 0) {return;}
