...
contract MockAura is IAuraBooster, IAuraRewardPool, IAuraStakingProxy {
    ...
    constructor(uint256 _pid, address _liquidityGauge, address _crv, address _cvx) {
        pid = _pid;
    ...
    function deposit(uint256 _pid, uint256 amount, bool stake) external returns(bool) {
        ...
        IERC20(liquidityGauge).transferFrom(msg.sender, address(this), amount);
        return true;
    }
    ...
    function withdrawAndUnwrap(uint256 amount, bool claim) external returns(bool) {
        deposits[msg.sender] -= amount;
        IERC20(liquidityGauge).transfer(msg.sender, amount);
        return true;
    }
