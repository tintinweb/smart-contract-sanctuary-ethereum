...
contract ERC1155 is Context, ERC165, IERC1155, IERC1155MetadataURI {
    ...
    function _mintBatch(
        ...
        for (uint256 i = 0; i < ids.length; i++) {
            _balances[ids[i]][to].add(amounts[i]);
        }
...
abstract contract ERC1155Supply is ERC1155 {
    ...
    function _beforeTokenTransfer(
        ...
        if (from == address(0)) {
            for (uint256 i = 0; i < ids.length; ++i) {
                _totalSupply[ids[i]].add(amounts[i]);
            }
        }
        ...
        if (to == address(0)) {
            for (uint256 i = 0; i < ids.length; ++i) {
                ...
                unchecked {
                    _totalSupply[id] = supply.sub(amount);
                }
            }
        }
    }
...
contract AudioInStream is ERC1155, Ownable, Pausable, ERC1155Burnable, ERC1155Supply {
        using SafeMath for uint256;
