...
error ArtDubaiAlexisChristodoulou__SaleConditionsNotSet();

error ArtDubaiAlexisChristodoulou__EmptyVouchersArray();
error ArtDubaiAlexisChristodoulou__RewardTokensMintingNotStarted();
error ArtDubaiAlexisChristodoulou__VoucherAlreadyUsed();

error ArtDubaiAlexisChristodoulou__HashComparisonFailed();
error ArtDubaiAlexisChristodoulou__UntrustedSigner();
error ArtDubaiAlexisChristodoulou__SignatureAlreadyUsed();
...
contract ArtDubaiAlexisChristodoulou is ArtDubai {
	...
	function mintRewardTokens(
		...
		if (rewardVouchers.length == 0) revert ArtDubaiAlexisChristodoulou__EmptyVouchersArray();
		...
		if (block.timestamp < conditions.startTime)
			revert ArtDubaiAlexisChristodoulou__RewardTokensMintingNotStarted();

		for (uint256 i = 0; i < rewardVouchers.length; i++) {
			...
			if (s_tokenIdsUsedForRewards[rewardVoucher.collection][rewardVoucher.tokenId])
				revert ArtDubaiAlexisChristodoulou__VoucherAlreadyUsed();
		...
		if (_rewardMintOperationHash(msg.sender, rewardVouchers, nonce) != hash)
			revert ArtDubaiAlexisChristodoulou__HashComparisonFailed();
		if (!_isTrustedSigner(hash, signature)) revert ArtDubaiAlexisChristodoulou__UntrustedSigner();
		if (s_usedNonces[nonce]) revert ArtDubaiAlexisChristodoulou__SignatureAlreadyUsed();
	...
	function setRewardTokenIds(
		...
		if (
			s_saleConditions[daytimeRewardTokenId].supplyLimit == 0 ||
			s_saleConditions[nighttimeRewardTokenId].supplyLimit == 0
		) revert ArtDubaiAlexisChristodoulou__SaleConditionsNotSet();
	...
	function getDaytimeRewardTokenId() public view returns (uint256 rewardTokenId) {
		rewardTokenId = s_daytimeRewardTokenId;
	}
	...
	function getNighttimeRewardTokenId() public view returns (uint256 rewardTokenId) {
		rewardTokenId = s_nighttimeRewardTokenId;
	}
	...
	/**
	 * @dev Generate hash of current reward minging operation.
	 *
	 * @param minter The address of the minter.
	 * @param rewardVouchers The reward vouchers.
	 * @param nonce The nonce of the mint operation.
	 *
	 * @return The hash of the mint operation.
	 */
	function _rewardMintOperationHash(
		address minter,
		RewardVoucher[] calldata rewardVouchers,
		uint64 nonce
	) internal view returns (bytes32) {
		address[] memory collections = new address[](rewardVouchers.length);
		uint64[] memory tokenIds = new uint64[](rewardVouchers.length);

		for (uint256 i = 0; i < rewardVouchers.length; i++) {
			collections[i] = rewardVouchers[i].collection;
			tokenIds[i] = rewardVouchers[i].tokenId;
		}

		return
			keccak256(
				abi.encodePacked(i_hashSalt, minter, uint64(block.chainid), collections, tokenIds, nonce)
			);
	}
}
