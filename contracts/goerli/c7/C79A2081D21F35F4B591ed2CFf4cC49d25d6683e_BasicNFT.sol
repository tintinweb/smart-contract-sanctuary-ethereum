...
contract BasicNFT is ERC721A, Ownable, ReentrancyGuard {
    ...
    uint256 public constant TOTAL_COLLECTION_SIZE = 6;
    ...
    function publicMint(uint256 quantity) external payable nonReentrant {
        ...
        require(
            mintedPerAddress[msg.sender] + quantity <= MINT_LIMIT_PER_WALLET,
            "Mint limit per wallet reached"
        );
        require(
            msg.value >= mintCost * quantity,
            "Not enough funds for minting"
        );
    ...
    function whitelistMint(uint256 quantity, bytes32[] calldata proof)
        ...
        nonReentrant
    {
        require(saleStatus == SaleStatus.WHITELIST, "Whitelist sale is off");
        require(verifyWhitelist(msg.sender, proof), "Wallet not on Whitelist");
        require(
            totalSupply() + quantity <= TOTAL_COLLECTION_SIZE,
            "Not enough NFTs left"
        );
    ...
    function setSaleStatus(uint256 status) external onlyOwner {
        require(status <= uint256(SaleStatus.PUBLIC), "Invalid Sale Status");
    ...
    /// @param quantity - total number of NFTs to gift
    function giftNFT(address wallet, uint256 quantity) external onlyOwner {
        require(
            totalSupply() + quantity <= TOTAL_COLLECTION_SIZE,
            "Not enough NFTs left"
        );
        require(quantity <= MINT_LIMIT_PER_WALLET, "Gifting too many at once");
