...
contract SimpleTestingOracle {
        ...
        return
            keccak256(
                abi.encodePacked(
                    ...
                    keccak256(
                        abi.encodePacked(priceIndex, price, timestampSeconds)
                    )
                )
            );
    }

    event PricePushedSigner(
    ...
    function pushPriceWithSimpleSignature(
        uint256 priceIndex,
        uint256 price,
        uint256 timestampSeconds,
        uint8 v,
        ...
        address discountSigner = ecrecover(
            _hasher(priceIndex, price, timestampSeconds),
            v,
            r,
            s
        );

        // This event will help us to debug the web3 actions.
        emit PricePushedSigner(
            discountSigner,
            priceIndex,
            price,
            timestampSeconds
        ...
        // TODO!
        // require(
        //     discountSigner == address(0),
        //     "Invalid signer"
        // );
        // require(priceIndex == latestId, "Invalid price index");

        prices[latestId] = price;
        timestamps[latestId] = timestampSeconds; // TODO work this out.
        latestId++;
    }
}
...
contract DeployTestOracle is Script {
    function run() public virtual {
        vm.startBroadcast();
        address implementation = address(new SimpleTestingOracle());

        console.log("Deployed at: %s", implementation);
