...
예시 : 800초 -> 13분 20초
9)  https://github.com/OpenZeppelin/openzeppelin-contracts/blob/master/contracts/utils/math/SafeMath.sol 
에서 tryadd, add, sub 함수를 도입해오세요.
10) A contract 에 있는 변수 a를 10 증가시켜주는 함수를 B contract에서 구현하세요. */ 

contract Test {
    //문제1-1 문자형
     function Q1_1 (string memory _a) public view returns(string memory) {
    ...
    //문제1-2 숫자형
    // function Q1_1 (uint _a) public view returns(uint) {
    //     uint b = new bytes(4);
    //     b[0] = (_a)[0];
    //     b[1] = (_a)[0];
    //     b[2] = (_a)[0];
    ...
    function sort() public returns(uint[] memory) {
        ...
        numberArr.push(5);
        uint Index;
        for (uint i; i < numberArr.length; i++) {
    ...
    //문제3
    function Q3 (uint year) public view returns(uint) {
        return (year/100)+1;
    }
