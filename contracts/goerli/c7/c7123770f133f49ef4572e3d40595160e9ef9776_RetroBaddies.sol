...
// File: contracts/RetroBaddies.sol




//Code By Raistilin.avax 
...
contract RetroBaddies is ERC721, ERC721Enumerable, ERC721URIStorage, Pausable, Ownable, ERC2981, ClampedRandomizer {
    constructor() 
        ERC721("RetroBaddies", "RETRO") ClampedRandomizer(maxSupply) {
    ...
    uint public price = 5;//scaled by 0.01 

    uint public publicTime= 1673911523; //Jan 16th 1
    address public royaltyAddress=0xD64730E0833D8eC073AD723BE410C0a279248e31;
    uint96 public royalty=696;
        ...
        whenNotPaused 
    {
        ...
        //mint start
        require(block.timestamp >= publicTime, "Mint not Started"); 
        
        //supply limits
        uint supply = totalSupply();
        require(supply + amount <= maxSupply, "Not enough Coq for you");

        //transaction limits
        require(amount > 0,"you want to mint more than just air right?");
        require(amount <= maxPerTx, "You cant handle that much Coq at once");

        
        if (price > 0) 
        {
            require(msg.value >= price * 0.01 ether * amount, "You didnt send enough  to mint");
        }
    ...
    {
        uint supply = totalSupply();
        require(supply + 1 <= maxSupply, "No more RetroBaddies left for you");
        uint256 tokenId=_genClampedNonce()+1;//+1 since we dont want to deal with token id 0
        _isMinted[tokenId]=true;
        _safeMint(to, tokenId);
        _setTokenURI(tokenId, string(abi.encodePacked(tokenId.toString(), ".json")));
    ...
    function revealbatch (uint _startId, uint _endId) 
        public 
        onlyOwner 
    {
    ...
    function setRoyalties(address _royaltyAddress, uint96 _royalty) 
        public 
        onlyOwner 
        {
    ...
    function setPublicTime(uint _publicTime) 
        public 
        onlyOwner 
    {
        ...
        returns (uint256 [] memory) 
    {
        ...
        uint256[] memory _Tokens = new uint[](balanceOf(owner));
