  ...
  Claimable
{
  ...
  mapping (uint256 => uint256) private stageMintFees;
  ...
  constructor(
    ...
    ERC721A("Supercute World", "selfies") 
  {
  ...
  {
    require(msg.value >= stageMintFees[stage] * _quantity, "Insufficient fee.");
    (bool paid, ) = paymentAddress.call{ value: msg.value }("");
        
    require(paid, "Unable to collect fee.");
    ...
    validGender(_gender)
  {
    ...
    require(tokenGenders[_tokenId] != _gender, "Cannot change to the same gender");

    if(getGenderSwitchFee(_tokenId) > 0) {
      require(msg.value >= genderSwitchFee, "Insufficient fee.");
      
      (bool paid, ) = paymentAddress.call{ value: msg.value }("");
      require(paid, "Unable to collect payment.");
    }

    genderSwitchCounts[_tokenId] += 1;
    tokenGenders[_tokenId] = _gender;

    emit SelfieGenderChange(_tokenId, _gender);
  }
    ...
    returns (string memory)
  {
    ...
    uint256 gender = tokenGenders[_tokenId];
  ...
  */
  ...
  {
    require(stage > 0, "Sale is not open.");
    require(stage == 3 ||
            MerkleProof.verify(_merkleProof, merkleRoots[stage], keccak256(abi.encodePacked(_to))),
            "Address not allowed to mint at this stage.");
    require(_totalMinted() + _quantity <= maxSupply, "Max supply exceeded.");
    uint256 limit = stageMintLimits[stage];
    if(limit > 0) {
      require(mintCounts[_to] + _quantity <= limit, "Address exceeding minting limits.");
      mintCounts[_to] += _quantity;
    }
  ...
  {
    require(_stage <= PUBLIC_STAGE, "Invalid stage.");
    _;
  }
  ...
  {
    require(_gender > 0 && _gender <= GENDERS, "Unknown gender code.");
    _;
  }
