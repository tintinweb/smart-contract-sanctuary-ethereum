...
contract ReverseLogic is Initializable {
        ...
        external
    {
		...
		try IOwner(addr).owner() {
			_owner = IOwner(addr).owner();
			require(_owner == msg.sender, "need owner to set reverse");
		} catch {
			try IOwner(addr).getOwner() {
				_owner = IOwner(addr).getOwner();
				require(_owner == msg.sender, "need getOwner to set reverse");
			} catch {
				try IOwner(addr).admin() {
					_owner = IOwner(addr).admin();
					require(_owner == msg.sender, "need admin to set reverse");
				} catch {}
			}
		}
		require(
