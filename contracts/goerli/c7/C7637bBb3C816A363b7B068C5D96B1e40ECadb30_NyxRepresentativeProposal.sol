...
contract NyxRepresentativeProposal is Ownable, converter {
    string public constant name = "Nyx Proposal - Representative";
    uint256 public constant nparams = 5;
        ...
        returns (RepresentativeProposalParams memory)
    {
        ...
        return RepresentativeProposalParams(converter.bytesToAddress(params[0]),
            RepresentativeAddressAction(converter.bytesToUint(params[1])),
            converter.bytesToUint(params[2]),
            converter.bytesToString(params[3]),
            converter.bytesToString(params[4]));
    }
    ...
    function parseProposalParams(bytes memory params)
        ...
        returns (RepresentativeProposalParams memory)
    {
        (bytes memory param1, bytes memory param2, bytes memory param3, bytes memory param4, bytes memory param5) = abi.decode(params, (bytes, bytes, bytes, bytes, bytes));
        return RepresentativeProposalParams(converter.bytesToAddress(param1),
            RepresentativeAddressAction(converter.bytesToUint(param2)),
            converter.bytesToUint(param3),
            converter.bytesToString(param4),
            converter.bytesToString(param5));
    }
        ...
        returns (RepresentativeProposalParams memory)
    {
        ...
        {
            (address param1, uint256 param2, uint256 param3, string memory param4, string memory param5) = abi.decode(params, (address, uint256, uint256, string, string));
    ...
    {
        return abi.encode(proposal.representativeAddress, proposal.action,
            proposal.socialNetworkLink, proposal.representativeDescription);
    }
        ...
        returns (bytes memory)
    {
        ...
        RepresentativeProposal memory proposal = RepresentativeProposal(proposalId, parsedParams.representativeAddress, parsedParams.action,
            parsedParams.proposalType, parsedParams.socialNetworkLink, parsedParams.representativeDescription);
    ...
    function getProposal(bytes memory proposalBytes)
        ...
        returns (bytes[] memory)
    {
        RepresentativeProposalParams memory params = parseProposalParams2(proposalBytes);
        ...
        prop[4] = abi.encode(params.representativeDescription);
        return prop;
    }
        ...
        returns (bytes[nparams][] memory props)
    {
        ...
        {
            RepresentativeProposalParams memory params = parseProposalParams2(proposalBytesArr[idx]);
            props[idx] = [abi.encode(params.representativeAddress), abi.encode(params.action), abi.encode(params.proposalType), abi.encode(params.socialNetworkLink), abi.encode(params.representativeDescription)];
        }
    }
}
