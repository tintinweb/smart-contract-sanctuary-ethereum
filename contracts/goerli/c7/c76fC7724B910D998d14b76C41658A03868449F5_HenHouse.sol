...
contract HenHouse is IHenHouse, Ownable, ReentrancyGuard, IERC721Receiver, Pausable {
  ...
  // HACK: LOCAL DEV change to 2 days
  uint256 public HEN_MINIMUM_TO_EXIT = 20 minutes; // hens must have 2 days worth of $EGG quota to unstake
  ...
  // HACK: LOCAL DEV change to 5 days
  uint256 public ROOSTER_MINIMUM_TO_EXIT = 20 minutes; // Roosters must have 5 days worth of $EGG quota to unstake
   ...
   */
  function _addHenToHenHouse(address account, uint256 tokenId) internal whenNotPaused _updateEarnings {
   ...
   */
  function addManyToHenHouse(address account, uint16[] calldata tokenIds) external override nonReentrant {
