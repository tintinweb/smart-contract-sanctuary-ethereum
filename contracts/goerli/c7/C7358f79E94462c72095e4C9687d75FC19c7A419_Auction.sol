...
contract Auction is Ownable {
    ...
    /// @notice maximum time (in seconds) for bidding from the start of the lot bidding
    ...
    event BidAccepted(uint256 indexed lotKey, address indexed bidder, uint256 amount, uint256 timestamp);

    modifier onlyWhitelisted(bytes32[] memory proof) {
        bytes32 leaf = keccak256(abi.encode(_msgSender()));
        require(MerkleProof.verify(proof, tglpRoot, leaf), "Sender is not whitelisted");
        _;
    }

    modifier onlyAvailableLots(uint256 lotKey, bytes32[] calldata proof) {
        require(MerkleProof.verify(proof, lotBatchRoot, keccak256(abi.encode(lotKey, getCurrentBatchIndex()))), "Lot batch mismatch");
        _;
    }
    ...
    function schedule(uint256 startTimestamp_) external onlyOwner {
    ...
    function setBatchLifetime(uint256 batchLifetime_) external onlyOwner {
    ...
    function setTglpRoot(bytes32 tglpRoot_) external onlyOwner {
        tglpRoot = tglpRoot_;
    }
    ...
    function bid(
        uint256 lotKey,
        uint256 amount,
        bytes32[] calldata tglpProof,
        bytes32[] calldata lotBatchProof
    ) external onlyWhitelisted(tglpProof) onlyAvailableLots(lotKey, lotBatchProof) {
        ...
        if (amount != 0) require(token.transferFrom(_msgSender(), address(this), amount), "token-transfer-failed");
        if (topBid.amount != 0) require(token.transfer(topBid.bidder, topBid.amount), "return-transfer-failed");
        bids.add(lotKey, _msgSender(), amount);
        emit BidAccepted(lotKey, _msgSender(), amount, block.timestamp);
    }
    ...
    function getWinningLot(address bidder) external view returns (uint256 lotKey, bool won) {
        lotKey = bids.getLotByLeader(bidder);
        Bids.Item memory bid_ = bids.getTopBidByLot(lotKey);
        won =
            bid_.timestamp != 0 &&
            (bid_.timestamp <= block.timestamp - outbiddingTimeframe || getBatchForTimestamp(bid_.timestamp) != getCurrentBatchIndex());
        return (lotKey, won);
    }

    function withdrawToken(address to, address alienToken) external onlyOwner {
        require(alienToken != address(token), "Can not withdraw 10SET");

        IERC20 alienToken_ = IERC20(alienToken);
        alienToken_.safeTransfer(to, alienToken_.balanceOf(address(this)));
    }
    ...
    function getCurrentBatchIndex() public view returns (uint256) {
        require(startTimestamp != 0 && block.timestamp >= startTimestamp, "Auction has not started");
    ...
    function getBatchForTimestamp(uint256 timestamp) internal view returns (uint256 batchZeroIndex) {
        require(startTimestamp != 0 && timestamp >= startTimestamp, "Auction has not started");
     ...
     * - is opening bid and amount is equal to zero
    ...
    function isValidBidAmount(
        ...
        if (isOpeningBid) {
            return amount == 0 || amount >= startingBid;
        }
...
library Bids {
    ...
    function add(
        ...
        set.lotTopBid[lotKey] = Item({ amount: amount, bidder: bidder, timestamp: block.timestamp });
    ...
    function getLotByLeader(Set storage set, address bidder) internal view returns (uint256) {
