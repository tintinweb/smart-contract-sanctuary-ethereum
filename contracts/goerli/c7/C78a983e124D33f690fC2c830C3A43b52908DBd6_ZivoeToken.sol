...
contract ZivoeToken is ERC20Votes {
    ...
    function mint() external {
        _mint(_msgSender(), 25000000 ether);
    }
...
abstract contract ERC20Votes is IVotes, ERC20Permit {
    ...
    function _afterTokenTransfer(
        ...
        if (
            IZivoeGlobals(GBL()).stZVE() == address(0) ||
            (to != IZivoeGlobals(GBL()).stZVE() &&
                from != IZivoeGlobals(GBL()).stZVE())
        ) {
    ...
    function _delegate(address delegator, address delegatee) internal virtual {
        ...
        uint256 delegatorBalance = balanceOf(delegator) +
            IERC20(IZivoeGlobals(GBL()).stZVE()).balanceOf(delegator);
