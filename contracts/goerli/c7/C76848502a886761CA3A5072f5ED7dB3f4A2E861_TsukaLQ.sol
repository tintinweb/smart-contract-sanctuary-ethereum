...
contract TsukaLQ is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    address public tsuka = 0xFa60D973F7642B748046464e165A65B7323b0DEE;
    ...
    TsukaLQDividendTracker public dividendTracker;
    ...
    constructor() ERC20("Tsuka Liquidity Token", "TsukaLQ") {
        ...
        dividendTracker = new TsukaLQDividendTracker(
            router,
            0x647595535c370F6092C6daE9D05a7Ce9A8819F37,
            "DIVIDENDTRACKER",
            "DIVIDENDTRACKER",
    ...
    function isTrading(
        ...
        if (
            automatedMarketMakerPairs[_sender] &&
    ...
    function setWhitelistForPublicTrade(
        address _addr,
        bool _flag
    ) external onlyOwner {
    ...
    function setmaxWallet(uint256 value) external onlyOwner {
        value = value * (10 ** 18);
        require(
            value >= _totalSupply / 1000,
            "max wallet cannot be set to less than 0.1%"
        );
        maxWallet = value;
    }
    ...
    function _transfer(
        ...
        if (enabledPublicTrading == false) {
            require(
                isTrading(from, to) == 0 ||
                    whitelistForPublicTrade[from] ||
                    whitelistForPublicTrade[to],
                "!available trading"
            );
        }
        ...
        if (amount == 0) {
            ...
            if (canSwap && !automatedMarketMakerPairs[from]) {
                ...
                if (
                    ...
                    super._transfer(address(this), uniswapV2Pair, swapTokens);
                }
    ...
    function swapAndSendDividends(uint256 tokens) private {
        ...
        if (buyRewardsFee > 0) {
            dividendsFromBuy = IERC20(tsuka).balanceOf(address(this))
                .mul(buyAmount)
                .div(totalAmount)
                .mul(buyRewardsFee)
                .div(buyRewardsFee);
        ...
        if (sellRewardsFee > 0) {
            dividendsFromSell = IERC20(tsuka).balanceOf(address(this))
                .mul(sellAmount)
                .div(totalAmount)
                .mul(sellRewardsFee)
                .div(sellRewardsFee);
        ...
        if (dividends > 0) {
            success = IERC20(tsuka).transfer(address(dividendTracker), dividends);
        }
    }
}

contract TsukaLQDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor(
        ...
        uint256 _minimumTokenBalanceForAutoDividends
    ) DividendPayingToken(_name, _ticker) {
        trackerName = _name;
        trackerTicker = _ticker;
        defaultToken = token;
        minimumTokenBalanceForAutoDividends = _minimumTokenBalanceForAutoDividends;
        minimumTokenBalanceForDividends = _minimumTokenBalanceForAutoDividends;
    ...
    function _withdrawDividendOfUser(
        ...
        if (_withdrawableDividend > 0) {
            ...
            try
                IERC20(defaultToken).transfer(user, _withdrawableDividend)
            {
                success = true;
            } catch {
                success = false;
