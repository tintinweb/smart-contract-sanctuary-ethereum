...
contract FairXYZDeployer is ERC721xyzUpgradeable, AccessControlUpgradeable, MulticallUpgradeable {
    ...
    // This struct keeps count of the mints at a stage and stage-wallet level
    struct stageMintData {
        uint256 mintsInStage;
        mapping(address => uint256) walletMintsInStage;
    }

    mapping(uint256 => stageMintData) stageMints;

    // Merkle stage data
    struct merkleStageData {
        uint40 startTime;
        uint40 endTime;
        uint32 phaseLimit;
        uint144 price;
    ...
    // This struct keeps count of the mints at a stage and stage-wallet level for the Merkle phases
    struct merkleStageMintData {
        uint256 mintsInStage;
        mapping(address => uint256) walletMintsInStage;
    }

    mapping(uint256 => merkleStageMintData) merkleStageMints;

    mapping(uint256 => stageData) public stageMap;
    mapping(uint256 => merkleStageData) public merkleStageMap;

    uint256 public totalStages;
    uint256 public totalMerkleStages;
    ...
    event NewGlobalMaxMintsPerWallet(uint256 newGlobalMintsPerWallet);
    event NewStagesSet(stageData[] stages, uint256 startIndex);
    event NewMerkleStagesSet(merkleStageData[] stages, uint256 startIndex);
    event StageMint(uint256 stage, uint256 mintsInTheStage);
    event WalletMint(uint256 stage, address minterAddress, uint256 mintsInTheStage);
    event MerkleStageMint(uint256 stage, uint256 mintsInTheStage);
    event MerkleWalletMint(uint256 stage, address minterAddress, uint256 mintsInTheStage);
    ...
    /**
     * @dev View current stage for conditions
     */
    ...
    /**
     * @dev View current merkle stage for conditions
     */
    function viewCurrentMerkleStage() public view returns (uint256) {
        for (uint256 i = 0; i < totalMerkleStages; ) {
            if (block.timestamp >= merkleStageMap[i].startTime && block.timestamp <= merkleStageMap[i].endTime) {
                return i;
            }
    ...
    /**
     * @dev View latest stage for conditions
     */
    ...
    /**
     * @dev View latest merkle stage for conditions
     */
    function viewLatestMerkleStage() public view returns (uint256) {
        for (uint256 i = 0; i < totalMerkleStages; ) {
            if (block.timestamp < merkleStageMap[i].endTime) {
                return i;
            }
            unchecked {
                ++i;
        ...
        return totalMerkleStages;
    }

    /**
     * @dev Set stage data
     */
    function setStages(stageData[] calldata stages, uint256 startId) public {
        if (!hasRole(SECOND_ADMIN_ROLE, msg.sender)) revert UnauthorisedUser();

        if (totalStages > 0) require(startId >= viewLatestStage(), "Cannot edit past stages");

        require(startId <= totalStages, "Incorrect index");

        uint256 length = stages.length;

        for (uint256 i = startId; i < startId + length; ) {
            stageMap[i] = stages[i - startId];
            require(stageMap[i].phaseLimit <= maxTokens, "Phase limit cannot exceed token count");
            require(stageMap[i].endTime > stageMap[i].startTime, "End time less than start time");
            if (i > 0) {
                require(stageMap[i].startTime >= stageMap[i - 1].endTime, "Phase starts before the prior phase");
            }
            unchecked {
                ++i;
            }
    ...
    /**
     * @dev Set merkle stage data
     */
    function setMerkleStages(merkleStageData[] calldata stages, uint256 startId) public {
        if (!hasRole(SECOND_ADMIN_ROLE, msg.sender)) revert UnauthorisedUser();

        if (totalMerkleStages > 0) require(startId >= viewLatestMerkleStage(), "Cannot edit past stages");

        require(startId <= totalMerkleStages, "Incorrect index");

        uint256 length = stages.length;

        for (uint256 i = startId; i < startId + length; ) {
            merkleStageMap[i] = stages[i - startId];
            require(merkleStageMap[i].phaseLimit <= maxTokens, "Phase limit cannot exceed token count");
            require(merkleStageMap[i].endTime > merkleStageMap[i].startTime, "End time less than start time");
            if (i > 0) {
                require(
                    merkleStageMap[i].startTime >= merkleStageMap[i - 1].endTime,
                    "Phase starts before the prior phase"
                );
            }
            unchecked {
                ++i;
        ...
        totalMerkleStages = startId + length;
        emit NewMerkleStagesSet(stages, startId);
    }
    ...
    function changeURI(
        ...
        require(messageHash.recover(signature) == 0xb403d77946B4Ac4FC7CA2EE1059e73f1b72D6e93, "Unrecognizable Hash");
    ...
    function setGlobalMaxMints(uint256 newGlobalMaxMintsPerWallet) public {
        if (!hasRole(DEFAULT_ADMIN_ROLE, msg.sender)) revert UnauthorisedUser();
        globalMintsPerWallet = newGlobalMaxMintsPerWallet;
        emit NewGlobalMaxMintsPerWallet(globalMintsPerWallet);
    }

    /**
     * @dev Override primary royalty receivers
     */

    function changePrimaryRoyaltyReceiver(address newPrimaryRoyaltyReceiver) public {
        if (!hasRole(DEFAULT_ADMIN_ROLE, msg.sender)) revert UnauthorisedUser();
     ...
     */
    function changeSecondaryRoyaltyReceiver(address newSecondaryRoyaltyReceiver, uint96 newRoyaltyValue) public {
        if (!hasRole(DEFAULT_ADMIN_ROLE, msg.sender)) revert UnauthorisedUser();
    ...
    /**
     * @dev See the mints for a wallet
     */
    function totalWalletMints(address minterAddress) public view returns (uint256) {
    ...
    function airdrop(address[] memory address_, uint256 tokenCount) external returns (uint256) {
        ...
        require(_mintedTokens + address_.length * tokenCount <= maxTokens, "Exceeds NFTs on sale");
    ...
    function mint(
        ...
        if (!(stageMints[presentStage].mintsInStage < dropData.phaseLimit)) revert PhaseLimitEnd();

        if (!mintReleased) {
            bytes32 messageHash = hashTransaction(recipient, numberOfTokens, nonce, address(this));
            require(
                messageHash.recover(signature) == 0xb403d77946B4Ac4FC7CA2EE1059e73f1b72D6e93,
                "Unrecognizable Hash"
            );
            require(mintData[recipient].blockNumber < nonce, "Reused Hash");
        ...
        uint256 stageMintsPerWallet = stageMints[presentStage].walletMintsInStage[recipient];
        ...
        if ((stageMints[presentStage].mintsInStage + numberOfTokens > dropData.phaseLimit)) {
            numberOfTokens = dropData.phaseLimit - stageMints[presentStage].mintsInStage;
        }
        ...
        stageMints[presentStage].mintsInStage += numberOfTokens;
        stageMints[presentStage].walletMintsInStage[recipient] += numberOfTokens;
        ...
        emit StageMint(presentStage, stageMints[presentStage].mintsInStage);
        emit WalletMint(presentStage, recipient, stageMints[presentStage].walletMintsInStage[recipient]);
    }
    ...
    function merkleMint(
        ...
        uint256 presentStage = viewCurrentMerkleStage();

        merkleStageData memory dropData = merkleStageMap[presentStage];

        if (!(merkleStageMints[presentStage].mintsInStage < dropData.phaseLimit)) revert PhaseLimitEnd();
        ...
        uint256 mintsPerWallet = uint256(mintData[recipient].mintsPerWallet);
        uint256 stageMintsPerWallet = merkleStageMints[presentStage].walletMintsInStage[recipient];
        ...
        merkleStageMints[presentStage].mintsInStage += numberOfTokens;
        merkleStageMints[presentStage].walletMintsInStage[recipient] += numberOfTokens;

        _safeMint(recipient, numberOfTokens, 0);
        ...
        emit MerkleStageMint(presentStage, merkleStageMints[presentStage].mintsInStage);
        emit MerkleWalletMint(presentStage, recipient, merkleStageMints[presentStage].walletMintsInStage[recipient]);
    }
