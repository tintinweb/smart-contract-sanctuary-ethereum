...
contract HelpMeDistributeReward is Ownable, ReentrancyGuard {
    ...
    mapping(uint256 => address[]) public HMPEWinnersPerDebogCycle;
    mapping(uint256 => address[]) public RoyaltyWinnersPerCycle;
    ...
    event HMPERewardDistributed(uint256 value, address winner);
    ...
    function distributeHmpeReward(address payable[] calldata _addresses, uint256[] calldata _values, uint256 _icycle) 
        ...
        for (uint256 i =0; i < _addresses.length; i++) {
            ...
            emit HMPERewardDistributed(_values[i], _addresses[i]);
            HMPEWinnersPerDebogCycle[_icycle].push(_addresses[i]);
    ...
    function distributeRoyaltyReward(address payable[] calldata _addresses, uint256[] calldata _values, uint256 _idate) 
        ...
        for (uint256 i =0; i < _addresses.length; i++) {
            ...
            require(sent, "Failed to send");
            emit HMPERewardDistributed(_values[i], _addresses[i]);
            RoyaltyWinnersPerCycle[_idate].push(_addresses[i]);
