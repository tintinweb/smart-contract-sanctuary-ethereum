...
// File @openzeppelin/contracts-upgradeable/access/[email protected]1.0
...
contract RewardVaultFactory is IRewardVaultFactory, OwnableUpgradeable {
    using UniversalERC20 for IERC20Upgradeable;

    /// @notice Reward vault implementation contract
    address private _vaultImpl;

    /// @notice Address which is being used on the BE to sign the claim message
    address private _signer;

    event RewardVaultLaunched(
        address vault,
        address admin,
        address projectToken,
        address rewardToken,
        uint256 minInvestAmt,
        uint256 maxInvestAmt
    );

    function initialize(address signer_, address vaultImpl_)
        public
        initializer
    {
        __Ownable_init();
    ...
    /// @notice Launch new reward vault
    function launchVault(
        ...
        address deployed = ClonesUpgradeable.clone(_vaultImpl);
        IRewardVault vault = IRewardVault(deployed);
        vault.initialize(
            minInvestAmt_,
            maxInvestAmt_,
            admin_,
    ...
    /// @notice View signer address
    function signer() external view override returns (address) {
        return _signer;
    }

    /// @notice Update the vault implementation contract address
    function updateVaultImpl(address vaultImpl_) external onlyOwner {
        _vaultImpl = vaultImpl_;
    }

    /// @notice View the vault implementation contract address
