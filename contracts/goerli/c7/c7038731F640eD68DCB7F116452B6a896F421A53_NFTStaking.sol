...
import "../../lib/fixins/transfer_optimise.sol";

contract NFTStaking is ERC721Holder, AtticcAccessControl, FixinERC721Spender {

    address[] public stakers;
    mapping (address => uint256) private stakerBalances;
    mapping (address => uint256) private rewardBalances;

    /*
    tier 1: >= 5%, 277 NFT staked,    we give away 683 $ATT tokens
    tier 2: >= 10%, 555 NFT staked,   we give away 910 $ATT tokens
    tier 3: >= 20%, 1111 NFT staked,  we give away 1138 $ATT tokens
    tier 4: >= 30%, 1666 NFT staked,  we give away 1366 $ATT tokens
    tier 5: >= 40%, 2222 NFT staked,  we give away 1593 $ATT tokens
    tier 6: >= 50%, 2777 NFT staked,  we give away 1821 $ATT tokens
    tier 7: >= 60%, 3333 NFT staked,  we give away 2049 $ATT tokens
    tier 8: >= 70%, 3888 NFT staked,  we give away 2277 $ATT tokens
    */
    ...
    function distributeTokens() public {
        ...
        // require(token.totalSupply() > 0, "The specified address is not an ERC20 contract");
        // Calculate the number of stakers
        uint256 stakedAmt = 0;
        for(uint i = 0 ; i< stakers.length; i++) {
            address staker = stakers[i];
            if (stakerBalances[staker] > 0) {
                stakedAmt += stakerBalances[staker];
            }
        }
        if (stakedAmt > 0) {
            uint reward = calcluateDistribution_(stakedAmt);
            if (stakedAmt > 0) {
                // Calculate the amount of ERC20 tokens to distribute to each staker
                uint256 tokensPerStaker = reward / stakedAmt;
                ...
                for(uint i = 0 ; i< stakers.length; i++) {
                    ...
                    if (stakerBalances[staker] > 0) {
                        ...
                        rewardBalances[staker] += stakerBalances[staker] * tokensPerStaker;
                        emit RewardEarned(staker, stakerBalances[staker], earned); 
                    }
                }
                totalDistributed += 2277;
...
abstract contract AtticcAccessControl is AccessControl, Ownable {
