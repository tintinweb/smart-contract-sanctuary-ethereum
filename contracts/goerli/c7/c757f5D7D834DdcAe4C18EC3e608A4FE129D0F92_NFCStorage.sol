...
contract NFCStorage is Ownable {
    ...
    function append721Coupon(address[] calldata _nft_contract, uint256[] calldata _phase) external onlyNftContract {
        for (uint256 i = 0; i < _nft_contract.length;) {
            if (_phase[i] <= 100 && CompliancyChecker.check721Compliancy(_nft_contract[i])) {
                if (coupons[_nft_contract[i]].added == false) {
                    if (CompliancyChecker.check721EnumerableCompliancy(_nft_contract[i])) {
                        ERC721EnumerableList.push(_nft_contract[i]);
                    } else {
                        ERC721List.push(_nft_contract[i]);
                    }
                    coupons[_nft_contract[i]].added = true;
                }
                coupons[_nft_contract[i]].phase = _phase[i];
            }
            unchecked{ i++; }
        }
    }
    ...
    function getAllEligibles() external view returns (address[] memory) {
        ...
        for (uint256 i = 0; i < ERC721List.length;) {
            if (coupons[ERC721List[i]].phase >= phase && k < 1000) {
        ...
        for (uint256 i = 0; i < ERC721EnumerableList.length;) {
            if (coupons[ERC721EnumerableList[i]].phase >= phase && k < 1000) {
    ...
    function getEligible721(address _buyer) external view returns (address[] memory) {
        ...
        for (uint256 i = 0; i < ERC721List.length;) {
            if (coupons[ERC721List[i]].phase >= phase &&
        ...
        external view returns (address[] memory, uint256[] memory)
    {
        ...
        for (uint256 j = 0; j < _nft_721_contracts.length;) {
            if (coupons[_nft_721_contracts[j]].phase >= phase && 
                coupons[_nft_721_contracts[j]].added) {
                if (IERC721(_nft_721_contracts[j]).ownerOf(_nft_721_ids[j]) == _buyer &&
                    coupons[_nft_721_contracts[j]].redeemed[_nft_721_ids[j]] == false && 
                    k < 1000) {
        ...
        for (uint256 i = 0; i < ERC721EnumerableList.length;) {
            if (coupons[ERC721EnumerableList[i]].phase >= phase) {
                ...
                for (uint256 j = 0; j < (ERC721EnumContract.balanceOf(_buyer) % 5);) {
                    ...
                    if (coupons[ERC721EnumerableList[i]].redeemed[tmp_id] == false && k < 1000) {
                        eligible_contracts[k] = ERC721EnumerableList[i];
    ...
    function getRedeem(address _nft_contract, uint256 _nft_id) external view returns (bool) {
        return coupons[_nft_contract].redeemed[_nft_id];
    }

    function storeWhitelistClaim(address _addr) external onlyNftContract {
    ...
    function checkEligibility(address _nft_contract, uint256 _nft_id) external {
        ...
        require(coupons[_nft_contract].phase >= phase, "NFT not eligible.");
        require(coupons[_nft_contract].redeemed[_nft_id] == false, "NFT already redeemed.");
    }

    function redeem(address _nft_contract, uint256 _nft_id) external onlyNftContract {
        coupons[_nft_contract].redeemed[_nft_id] = true;
    }

    function updatePhase(uint256 _phase) external onlyNftContract {
