...
contract Strategy is IStrategy {
  ...
  /// @inheritdoc IStrategy
  uint256 public constant VOLATILITY_SAFE_RANGE_MIN = 1200;

  /// @inheritdoc IStrategy
  uint256 public constant VOLATILITY_SAFE_RANGE_MAX = 1500;
    ...
    returns (LiquidityPosition memory _positionToMint)
  {
    ...
    if (_amount < MIN_WETH_TO_MINT) {
      if (_totalWeth > MIN_WETH_TO_MINT) {
        _amount = _totalWeth;
      } // use full amount (max 2xPERCENT_WETH_TO_MINT of MIN_WETH_TO_MINT)
      else {
        revert Strategy_NotEnoughWeth();
      }
    }
    ...
    if (_lockManagerState.isWethToken0) {
      ...
      uint160 _sqrtPUpperMinX96 =
        uint160(FixedPointMathLib.mulDivDown(_nextSqrtWethX96, VOLATILITY_SAFE_RANGE_MIN, 1000));
      uint160 _sqrtPUpperMaxX96 =
        uint160(FixedPointMathLib.mulDivDown(_nextSqrtWethX96, VOLATILITY_SAFE_RANGE_MAX, 1000));
      ...
      if (_amount > MAX_WETH_TO_MINT) {
        ...
        _sqrtPUpperX96 = uint160(
          ((_amount - MIN_WETH_TO_MINT) * (_sqrtPUpperMaxX96 - _sqrtPUpperMinX96)) / (MAX_WETH_TO_MINT - MIN_WETH_TO_MINT)
            + _sqrtPUpperMinX96
        );
      }
      ...
      uint160 _sqrtPLowerMaxX96 =
        uint160(FixedPointMathLib.mulDivDown(_previousSqrtWethX96, 1000, VOLATILITY_SAFE_RANGE_MIN));
      uint160 _sqrtPLowerMinX96 =
        uint160(FixedPointMathLib.mulDivDown(_previousSqrtWethX96, 1000, VOLATILITY_SAFE_RANGE_MAX));
      ...
      if (_amount > MAX_WETH_TO_MINT) {
        ...
        _sqrtPLowerX96 = uint160(
          ((_amount - MIN_WETH_TO_MINT) * (_sqrtPLowerMaxX96 - _sqrtPLowerMinX96)) / (MAX_WETH_TO_MINT - MIN_WETH_TO_MINT)
            + _sqrtPLowerMaxX96
        );
      }
  ...
  function getPositionToBurn(
    ...
    view
    virtual
    returns (LiquidityPosition memory _positionToBurn)
  {
    (, int24 _currentTick,,,,,) = _lockManagerState.pool.slot0();

    if (_lockManagerState.isWethToken0) {
      if (_position.lowerTick - LOWER_BURN_DIFF < _currentTick) {
        revert Strategy_NotFarEnoughToRight();
      }
    } else {
      if (_position.upperTick + UPPER_BURN_DIFF > _currentTick) {
        revert Strategy_NotFarEnoughToLeft();
      }
    }
    _positionToBurn =
      LiquidityPosition({lowerTick: _position.lowerTick, upperTick: _position.upperTick, liquidity: _positionLiquidity});
  }
}