...
uint256 constant FEE_OF_RAISED = 5;

uint256 constant MIN_LOCK = 5;

// 
interface IPresaleFactory {
    function LAUNCHPAD_FEE() external view returns (uint256);
    ...
    function presales(uint256) external view returns (address);

    function presalesCount() external view returns (uint256);
...
struct PresaleData {
    ...
    IUniswapV2Router02 prouter;
    IUniswapV2Pair pair;
    uint256 salePrice;
    uint256 offredAmount;
    uint256 soldAmount;
    uint256 minBuy; // in raised token
    uint256 maxBuy; // in raised token
    bool isWETH;
    uint256 lpPercentage;
    uint256 lPReserved;
    uint256 listingPrice;
    uint256 lpLockPeriodInMinutes;
    bool lpCreated;
    bool lpClaimed;
    uint256 softCap;
    bool isInitialized;
...
contract Presale is SaleTimeManagement, Whitelist {
    ...
    function hasFailedCritically() public view returns (bool) {
        return (!lpCreated && _checkLP(address(pair)));
    }

    function createLP() external onlySuccessful {
        _createLP();
    }
    ...
    constructor(
        ...
        factory = IPresaleFactory(msg.sender);
    }

    function _checkLP(address pair_) internal view returns (bool exists) {
        (uint256 reserve0, uint256 reserve1, ) = IUniswapV2Pair(pair_)
            .getReserves();
        exists = reserve0 != 0 && reserve1 != 0;
    }

    modifier onlySuccessful() {
        require(
            !canceled &&
                ((softCap * saleTokenDecimals) / salePrice) <= soldAmount &&
                !hasFailedCritically(),
            "not successful"
        );
        _;
    ...
    function buy(
        uint256 amount_
    ) external payable onlyWhenActive onlyWhitelisted notCanceled {
        if (!allowContracts) {
        ...
        require(inAmount >= minBuy, "below min buy");

        usersInfo[msg.sender].spentAmount += uint248(inAmount);
        require(usersInfo[msg.sender].spentAmount <= maxBuy, "above max buy");

        uint256 outAmount = (inAmount * saleTokenDecimals) / salePrice;

        require(outAmount <= (offredAmount - soldAmount), "sold out");
        usersInfo[msg.sender].boughtAmount += uint248(outAmount);

        soldAmount += outAmount;
    }

    function claim() external onlyWhenEnded onlySuccessful {
        _createLP();
        require(!usersInfo[msg.sender].claimed, "already claimed");
        usersInfo[msg.sender].claimed = true;
        saleToken.safeTransfer(msg.sender, usersInfo[msg.sender].boughtAmount);
    }

    function claimRefund() external onlyWhenEnded onlyNotSuccessful {
        require(!usersInfo[msg.sender].claimed, "already claimed");
        // avoids reentrancy
        usersInfo[msg.sender].claimed = true;
        if (isWETH) {
    ...
    function _createLP() internal {
        ...
        //bool created = _checkLP(address(pair));
        //require(!created, "Lp Was created externally");
    ...
    function widthrawRaisedToken() external onlyWhenEnded onlySuccessful {
        _createLP();
        address feeAddress = factory.feeAddress();
        require(
            msg.sender == owner() || msg.sender == feeAddress,
            "only owner or feecollector"
        );

        if (!isWETH) {
            uint256 amount = raisedToken.balanceOf(address(this));
            uint256 raiserAmount = (amount * (100 - FEE_OF_RAISED)) / 100;
            uint256 launchpadFee = amount - raiserAmount;
            raisedToken.safeTransfer(owner(), raiserAmount);
            raisedToken.safeTransfer(feeAddress, launchpadFee);
        } else {
            uint256 amount = address(this).balance;
            uint256 raiserAmount = (amount * (100 - FEE_OF_RAISED)) / 100;
            uint256 launchpadFee = amount - raiserAmount;
            payable(owner()).transfer(raiserAmount);
            payable(feeAddress).transfer(launchpadFee);
        }
    }

    function widthrawLp() external onlyOwner onlyWhenEnded {
        _createLP();
        require(
    ...
    function initializeSale(
        ...
        require(_offredAmount > 0, "offred amount == 0");
        _depositSaleToken(_offredAmount);

        _setMinBuy(minBuy_);
        _setMaxBuy(maxBuy_);

        require(listingPrice_ > 0, "listing price == 0");
        // listing price should not be lower than 90% of the sale price
        require(
            listingPrice_ >= ((salePrice_ * 9) / 10),
            "listing price too low"
        );
        ...
        require(softCap_ <= toBeRAised, "softCap > offredAmount");
        require(
            softCap_ >= (toBeRAised * MIN_SOFT_CAP) / 100,
            "softCap < 10% of offredAmount"
        );
        softCap = softCap_;
    ...
    function getAllData() external view returns (PresaleData memory data) {
        data = PresaleData(
            saleToken,
            raisedToken,
            saleTokenDecimals,
    ...
    function _setSalePrice(uint256 salePrice_) internal {
        require(salePrice_ > 0, "sale price == 0");
    ...
    function _setlPLockPeriodInMinutes(
        ...
        require(_lpLockPeriodInMinutes >= MIN_LOCK, "min 5 minutes");
        lpLockPeriodInMinutes = _lpLockPeriodInMinutes;
    }

    function _setLPPercentage(uint256 _lpPercentage) internal {
        require(_lpPercentage <= MAX_LP, "lpPercentage > MAX_LP");
        require(_lpPercentage >= MIN_LP, "lpPercentage < MIN_LP");
...
// 
//import "./FactoryPresale.sol";
contract FinanomyFactory {
    uint256 public constant LAUNCHPAD_FEE = FEE_OF_RAISED;
