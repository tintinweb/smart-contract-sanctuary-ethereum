...
contract Owla is ERC721A, Ownable, ReentrancyGuard, DefaultOperatorFilterer {
  ...
  function fuse(uint64[] memory tokens) public {
        ...
        baseColor[tokens[0]] = randoOwla(tokens[0], 1, 4);
  }

  function getOwla(uint64 tokenId) public view returns (uint64) {
        if (!_exists(tokenId)) {
            return 0;
  ...
  function randoOwla(uint64 input, uint64 min, uint64 max) internal pure returns (uint64) {
