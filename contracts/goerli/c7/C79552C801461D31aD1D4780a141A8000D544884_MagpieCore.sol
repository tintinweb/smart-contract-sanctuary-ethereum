...
contract MagpieCore is ReentrancyGuard, Ownable, IMagpieCore {
  ...
  function swapIn(SwapInArgs calldata args) external payable override returns(uint256[] memory amountOuts, uint64) {
    ...
    uint256 amountOut = amountOuts.sum();
    address recipient = address(uint160(uint256(args.payload.to)));
    toAssetId.increaseAllowance(config.liquidityPoolAddress, amountOut);
    IMagpiePool(config.liquidityPoolAddress).bridgeInToken(args.payload.recipientChainId, 
                                                              toAssetId, 
                                                              recipient, 
                                                              amountOut);
  ...
  function swapOut(SwapOutArgs calldata args, uint256 tokenGasPrice) external returns(uint256[] memory amountOuts) {
    ...
    require(uint256(payload.senderChainId) == senderChainID, 'MagpieCore: invalid recipientChainId');
    require(args.swapArgs.routes[0].amountIn == payload.amount, 'MagpieCore: invalid amount');
    // require(args.swapArgs.amountOutMin >= payload.amount, "MagpieCore: invalid amoutOutMin" );

    fromAssetId.increaseAllowance(config.routerAddress, amountIn);
    IMagpiePool(config.liquidityPoolAddress).bridgeOutToken(payload.fromAssetId, payload.amount, payload.to, tokenGasPrice, payload.senderChainId);
