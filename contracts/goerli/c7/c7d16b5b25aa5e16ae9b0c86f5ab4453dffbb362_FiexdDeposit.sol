...
contract FiexdDeposit is Durations,ReentrancyGuard{
  ...
  uint256 public decreaseApr; //50000/500%
  ...
  function extension(uint256 duration) nonReentrant external {
    ...
    depositSlip.apr = apr - ((extensionCount[_msgSender()] + 1) * decreaseApr);
    depositSlip.duration = duration;
    depositSlip.startTime = block.timestamp;
  ...
  function transferAsset(address token,uint256 amount) external onlyOwner{
