...
contract TransferToken is ERC20Interface, Owned, Oracled, Verify {
    ...
    event BridgeTransfer(address indexed from, string to, uint tokens, uint chainId);
    ...
    function bridgeTransfer(string memory to, uint tokens, uint chainId) public returns (bool success) {
        balances[msg.sender] = balances[msg.sender].sub(tokens);
