...
contract StormTrooper is ERC1155, ERC2981, AccessControl, VRFConsumerBaseV2, ConfirmedOwner {
    ...
    function premint(bytes32[] calldata _proof, uint256 _collectionId, uint256 _quantity) external payable {
        uint256 brandId = collectionIdsBrand[_collectionId];
        require(brands[brandId].mintPriceInWei * _quantity == msg.value, "Invalid eth price");
    ...
    function _premint(bytes32[] calldata _proof, address to, uint256 _collectionId, uint256 _quantity) internal {
        require(stormtroopers[_collectionId].exist, "Collection doesnt exist");
        require(stormtroopers[_collectionId].isPremintOpen, "Premint not yet open");
        require(!stormtroopers[_collectionId].enabled, "Collection is already open for public mint");
        require(stormtroopers[_collectionId].minted + _quantity <= stormtroopers[_collectionId].cap, "Exceed max cap");
        require(_quantity <= maxMintPerTx, "Exceeded max tx per mint");
        require(
            _verifySenderProof(_msgSender(), stormtroopers[_collectionId].whitelistMerkleRoot, _proof),
            "Invalid proof"
        );

        _mint(to, _collectionId, _quantity, "");
    }

    function isWhitelistedAddressOnCollection(bytes32[] calldata _proof, address to, uint256 collectionId) external view returns (bool) {
        return _verifySenderProof(to, stormtroopers[collectionId].whitelistMerkleRoot, _proof);
    }
    ...
    function createCollection(uint256 cap, uint256 brand, bool isEnabled, bool isPremintOpen) external onlyHasRole(ADMIN_ROLE) {
    ...
    function setWhitelistForCollection(bytes32 _whitelistMerkleRoot, uint256 collectionId) public onlyHasRole(ADMIN_ROLE) {
        require(stormtroopers[collectionId].exist, "Collection doesnt exist");
        stormtroopers[collectionId].whitelistMerkleRoot = _whitelistMerkleRoot;
    }

    function togglePremintStatus(uint256 collectionId) public onlyHasRole(ADMIN_ROLE) {
        require(stormtroopers[collectionId].exist, "Collection doesnt exist");
        // toggle only if collection is currently disable
        require(!stormtroopers[collectionId].enabled, "Please disable the collection to toggle");
        stormtroopers[collectionId].isPremintOpen = !stormtroopers[collectionId].isPremintOpen;
    }
    ...
    function enableCollection(uint256 _id) public onlyHasRole(ADMIN_ROLE) {
        ...
        // always make the premint close when mint is enable for public
        stormtroopers[_id].isPremintOpen = false;
