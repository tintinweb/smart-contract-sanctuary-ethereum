...
contract LotteryTicketNftFactory {
    ...
    function predictAddressForLottery(string memory _baseUri, uint256 _ticketPrice, uint256 _lotteryDurationInMinutes) public view returns (address predicted) {
        bytes32 salt = getSalt(_baseUri, _ticketPrice, _lotteryDurationInMinutes);
        predicted = Clones.predictDeterministicAddress(blueprintImplementation, salt, address(this));
    }
    ...
    function _getData(string memory _baseUri, uint256 _ticketPrice, uint256 _lotteryDurationInMinutes) internal pure returns (bytes memory) {
        return abi.encodeWithSignature("initialize(address,uint256)", _baseUri, _ticketPrice, _lotteryDurationInMinutes);
    }
...
contract LotteryTicketNft is Initializable, ERC721Upgradeable, ERC721URIStorageUpgradeable, OwnableUpgradeable, ReentrancyGuardUpgradeable {
  ...
  function safeMint(address to, string memory uri) public payable mintCompliance(_msgSender()) onlyOwner nonReentrant {
