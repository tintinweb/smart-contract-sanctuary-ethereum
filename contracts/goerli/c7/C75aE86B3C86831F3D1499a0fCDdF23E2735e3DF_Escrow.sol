...
contract Escrow is Ownable {
    ...
    function buy(uint256 amount, address to) public payable {
        uint256 balance = rodo.balanceOf(address(this));
        uint256 tokensPrice = (amount * price) / 1e18;
        uint256 fee = (tokensPrice * feePercentage) / 1e5;
        uint256 requiredEth = tokensPrice + fee;
        require(amount > 0, "Escrow: Amount cannot be 0");
        require(amount <= balance / 2, "Escrow: Cannot buy more that 50%");
        require(requiredEth <= msg.value, "Escrow: low price");
        rodo.transfer(to, amount);
        rodo.transfer(feeReceiver, (amount * 20) / 100);
        rodo.transfer(admin, (amount * 80) / 100);
