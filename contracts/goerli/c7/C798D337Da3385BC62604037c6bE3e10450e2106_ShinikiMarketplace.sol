...
interface IShinikiMarketplaceCore {
    ...
    function cancelExtend(LibOrder.Input memory input) external;
...
library LibLazyMint {
    ...
    bytes4 constant MINT_SIGNED = bytes4(keccak256("SIGNED"));
...
library LibOrder {
    ...
    bytes32 constant ORDER_TYPEHASH =
        keccak256(
            "Order(address maker,Asset makeAsset,address taker,Asset takeAsset,uint256 salt,uint256 start,uint256 end,bytes4 dataType,bytes data,bytes dataMint)Asset(AssetType assetType,uint256 value,address token,uint256 tokenId)AssetType(bytes4 assetClass,bytes data)"
        );

    bytes32 constant INPUT_TYPEHASH =
        keccak256(
            "Input(address maker,Asset makeAssetNormal,Asset makeAssetSigned,address taker,Asset takeAsset,uint256 salt,uint256 start,uint256 end,bytes4 dataType,bytes data,bytes dataMint)Asset(AssetType assetType,uint256 value,address token,uint256 tokenId)AssetType(bytes4 assetClass,bytes data)"
        );
    ...
    struct Input {
        ...
        LibAsset.Asset makeAssetSigned;
        address taker;
        LibAsset.Asset takeAsset;
        uint256 salt;
        uint256 start;
        ...
        return
            keccak256(
                abi.encode(
                    ...
                    LibAsset._hash(input.makeAssetSigned),
                    input.taker,
    ...
    function _verifyTimeExtend(LibOrder.Input memory input) internal view {
        require(
    ...
    EIP712Upgradeable
{
    ...
    function _verifySignatureExtend(LibOrder.Input memory input, bytes calldata signature)
        internal
        view
    {
        if (input.salt != 0) {
    ...
    PausableUpgradeable
{
    ...
    function cancelExtend(LibOrder.Input calldata input)
        external
        override
        nonReentrant
        whenNotPaused
    {
        ...
        LibOrder.Order memory orderSigned = LibOrder.Order(
            input.maker,
            input.makeAssetNormal,
            input.taker,
            input.takeAsset,
        ...
        bytes32 orderKeyHashSigned = LibOrder._hashKey(orderSigned);
        require(fills[orderKeyHashNormal] != UINT256_MAX, "already canceled");
        require(fills[orderKeyHashSigned] != UINT256_MAX, "already canceled");
        fills[orderKeyHashNormal] = UINT256_MAX;
        fills[orderKeyHashSigned] = UINT256_MAX;
    ...
    function matchOrderExtend(
        ...
        _validateExtendFull(input, signatureLeft);
        ...
        LibLazyMint.LazyMint memory dataLazyMint = LibLazyMint._decode(orderRight.dataMint);

        if (dataLazyMint.typeMint == LibLazyMint.MINT_NORMAL) {
            ...
            dataLazyMint.typeMint == LibLazyMint.MINT_SIGNED
        ) {
            orderLeft = LibOrder.Order(
                input.maker,
                input.makeAssetSigned,
                input.taker,
                input.takeAsset,
                input.salt,
                input.start,
    ...
    */
    function _matchAndTransfer(
        ...
        LibLazyMint.LazyMint memory dataLazyMint = LibLazyMint._decode(orderLeft.dataMint);
    ...
    */
    ...
    function _validateExtendFull(
        LibOrder.Input calldata input,
        bytes calldata signature
    ) internal view {
        LibOrder._verifyTimeExtend(input);
        _verifySignatureExtend(input, signature);
    }
...
import "../token/erc721/interfaces/INFT721Base.sol";
...
abstract contract TransferExecutor {
    ...
    function _transferNFT(
        LibAsset.Asset memory asset,
        LibLazyMint.LazyMint memory data,
        address from,
        address to
    ) internal {
...
abstract contract TransferManager is TransferExecutor {
    ...
    function _doTransfers(
        ...
        LibLazyMint.LazyMint memory dataLazyMint,
