...
contract PolicyQuote is IPolicyQuote, OwnableUpgradeable, AbstractDependant {
    ...
    IDEINStaking public deinStaking;
    ...
    mapping(uint256 => PricingDiscount[]) internal pricingDiscountModels; // index => discount prices[] modle
        ...
        onlyInjectorOrZero
    {
        ...
        deinStaking = IDEINStaking(_contractsRegistry.getDEINStakingContract());
    }

    function setPricingDiscountModelData(
        ...
        for (uint256 i = 0; i < _stakingValues.length; i = uncheckedInc(i)) {
            pricingDiscountModels[_prcinigDiscountModelIndex].push(
                PricingDiscount(_stakingValues[i], _discounts[i])
            );
        }
    ...
    function getUserDiscount(address _user) public view returns (uint256 _discount) {
        if (block.timestamp > validToTime) return _discount;

        uint256 _stakingValue = deinStaking.getUserStakingPower(_user);

        if (_stakingValue > 0) {
            PricingDiscount[] memory _priceDiscounts =
