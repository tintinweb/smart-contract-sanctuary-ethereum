...
contract ClaimFactory {
    ...
    event ClaimContractDeployed(
        ...
        address indexed _claimContract
    );
    ...
    function deployClaimContract(ClaimSettings memory _claimSettings) external {
        ...
        Claim newClaimContract = new Claim(msg.sender, _claimSettings);

        if (!_claimSettings.hasAllowanceMechanism) {
            if (_claimSettings.isNFT) {
                for (
                    uint i;
                    i < _claimSettings.claimAmountDetails.tokenIds.length;

                ) {
                    IERC721(_claimSettings.airdropToken).safeTransferFrom(
                        msg.sender,
                        address(newClaimContract),
                        _claimSettings.claimAmountDetails.tokenIds[i]
                    );
