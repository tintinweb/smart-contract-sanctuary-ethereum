...
contract DiceGame is Initializable, RandomNumber {
    address internal s_linkToNativeTokenPriceFeed;
    ...
    function initialize(
        ...
        s_linkToNativeTokenPriceFeed = linkToNativeTokenPriceFeed;
        // Adding the usd price feed of the native token
        s_tokenToUsdPriceFeed[address(0)] = nativeToUsdPriceFeed;
    ...
    function playGame(
        ...
        for (uint256 i = 0; i < tokens.length; i++) {
            ...
            if (tokens[i] == IERC20Upgradeable(address(0))) {
                ...
                require(
                    s_tokenToUsdPriceFeed[address(tokens[i])] != address(0),
            ...
            s_totalInBetsPerToken[address(tokens[i])] += betAmounts[i];
        }
        ...
        (bool success, ) = s_feeRecipient.call{value: remainingValueForFee}("");
        ...
        onlyOwner
    {
        require(
            (getAvailablePrize(_token) -
                totalUnpaidToInfluencersByToken[address(_token)]) >= _amount,
...
contract RandomNumber is ReferralSystem, VRFConsumerBaseV2Upgradeable {
    ...
    uint32 internal constant CALLBACK_GAS_LIMIT = 220000;
        ...
        override
    {
        ...
        for (uint256 i = 0; i < randomWords.length; i++) {
            ...
            uint256 totalInBets = s_totalInBetsPerToken[
                address(userGuess.token)
            ];
            s_totalInBetsPerToken[address(userGuess.token)] -= userGuess
                .betAmount;
...
contract ReferralSystem is OwnableUpgradeable {
    ...
    mapping(address => address) public s_tokenToUsdPriceFeed;
    ...
    function withdrawInfluencerProfit() external {
        ...
        ) = getInfluencerPaymentInstructions(msg.sender);
        for (uint256 i = 0; i < tokens.length; i++) {
    ...
    function getInfluencerPaymentInstructions(address influencer)
        public
        view
        returns (address[] memory, uint256[] memory)
    {
        InfluencerProfitCalculationData memory data;
        data.tokensLength = s_addedTokens.length;
        ...
        for (uint256 i = 0; i < data.tokensLength; i++) {
            data.currentToken = s_addedTokens[i];
            data.usdPriceFeed = s_tokenToUsdPriceFeed[data.currentToken];
        ...
        if (referredBy == address(0)) {
            ...
            if (influencer != address(0)) {
                require(
                    ...
                    "ReferralSystem: invalid influencer provided"
                );
    ...
    function addAllowedTokens(
        ...
        uint32 addedTokensLength = uint32(s_addedTokens.length);
        for (uint256 i = 0; i < tokens.length; i++) {
            addedTokensLength++;
            require(
            ...
            require(
                s_tokenToUsdPriceFeed[tokens[i]] == address(0),
            ...
            s_tokenToUsdPriceFeed[tokens[i]] = usdPriceFeeds[i];
            s_addedTokens.push(tokens[i]);
        }
    }
