...
contract Kureiji is ERC721Enum, Ownable, PaymentSplitter {
    ...
    bytes32 private publiclist_merkleRoot;
    ...
    constructor(string memory _hiddenURI, bytes32 _whitelistMerkleRoot, bytes32 _publiclistMerkleRoot) ERC721P("Kureiji", "KJ") PaymentSplitter(_splitterAddressList, _shareList) {
        setHiddenURI(_hiddenURI);
        whitelist_merkleRoot = _whitelistMerkleRoot;
        publiclist_merkleRoot= _publiclistMerkleRoot;
    }
    ...
    function publiclistMint(uint256 _mintAmount, bytes32[] calldata _proof) external payable mintCheck(_mintAmount) {
        require(!presaleOnlyActive && !paused, "Public sale is not active");
        require(_verifyLeaf(_leaf(msg.sender), publiclist_merkleRoot, _proof), "Invalid public proof");
    ...
    function setPubliclistMerkleRoot(bytes32 _root) public onlyOwner {
        publiclist_merkleRoot = _root;
    }
