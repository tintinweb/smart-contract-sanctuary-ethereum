...
contract macabroids is ERC721A, Ownable {
  ...
  string public unrevealed_uri = "https://ipfs.io/ipfs/QmYqN81ruDCqsftnJTB59z28PPPce4ycfYb7YBC2epmr5v";
...
// MINTING
  ...
  function airdrop_many(address[] calldata _recipients, uint[] calldata _mintAmounts) external onlyOwner {
...
// SETTERS
  ...
  function addcollections(address[] calldata _collections) external onlyOwner {
    ...
    // modify qualifying collections
  function modifyCollectionAt(uint index, address replaceCollection) external onlyOwner {
...
// GETTERS
  ...
  function canClaim(address _wallet, bytes32[] calldata _merkleProof)
    ...
    returns (bool)
  {
    if ((mintedByAddress[_wallet] < freePerWallet) && (isWhitelisted(_wallet, _merkleProof) || isHolder (_wallet))) {
    return true;
    } else {
    return false;
    }
