...
contract NFCertificate is ERC721, Ownable {
    ...
    address public signerAddress =
        address(0xf9248cf43DcDc361d3A1Af0318107294Fb82312d);
    ...
    mapping(bytes32 => bool) private isHashed;
    ...
    constructor() ERC721("NFCertificate", "NFC") {
    ...
    function verify(
        ...
        bytes32 msgHash = keccak256(
            abi.encodePacked(_message, _nonce, _courseId, _hash, timeStamp)
        );
    ...
    function nftTransferRequest(address to, uint256 tokenId) public {
        super.safeTransferFrom(msg.sender, vault, tokenId);
        emit NftTransferRequest(msg.sender, to, tokenId);
    }
    ...
    function safeTransferFrom(
        ...
        if (isOwnerShip[msg.sender] == true) {
            // If admin wants to transfer
            super.safeTransferFrom(from, to, tokenId);
        } else {
            // If user wants to transfer
            nftTransferRequest(to, tokenId);
        }
    }
    ...
    function safeTransferFrom(
        ...
        if (isOwnerShip[msg.sender] == true) {
            ...
            // If user wants to transfer
            super.safeTransferFrom(from, vault, tokenId, data);
            emit NftTransferRequest(msg.sender, to, tokenId);
        }
    }
    ...
    function mint(
        ...
        require(
            verify(_message, _nonce, signature, _courseId, _hash, _expires) ==
                true,
            "signature not verified"
        );
        require(isHashed[_hash] != true, "hash already used");
        require(_expires >= block.timestamp, "signature expired");
        isHashed[_hash] = true;
    ...
    function changeSignerAddress(address _newSignerAddress) public onlyOwner {
