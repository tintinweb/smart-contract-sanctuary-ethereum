...
modifier assignRarity(uint256 _mintAmount) {
    ...
    for(uint i = 0; i < _mintAmount; i++){
            ...
            if(rarityModulus == 0) {rarityAssigned = 4;}
            if(rarityModulus >= 1 && rarityModulus <= 2) {rarityAssigned = 3;}
            if(rarityModulus >= 3 && rarityModulus <= 5) {rarityAssigned = 2;}
            if(rarityModulus >= 6 && rarityModulus <= 10) {rarityAssigned = 1;}
            baseRarity[internalIndex] = rarityAssigned;
            internalIndex++;
            rarityModulus ++;
            seedRarity = rarityModulus;

            attackRarity[internalIndex] = (raritySeed%10 + a_seedRarity)%40;
            defenseRarity[internalIndex] = (raritySeed%10 + d_seedRarity + seedRarity)%40;
  ...
  function mint(uint256 _mintAmount) public payable mintCompliance(_mintAmount) mintPriceCompliance(_mintAmount) assignRarity(_mintAmount) {
