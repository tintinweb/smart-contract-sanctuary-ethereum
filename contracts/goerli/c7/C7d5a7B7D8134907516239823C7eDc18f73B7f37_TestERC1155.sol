...
contract TestERC1155 is ERC1155, Ownable {
    ...
    uint256 public MAX_PRE_SALE_FREE_AMOUNT = 1;
    uint256 public MAX_PRE_SALE_DISCOUNT_AMOUNT = 5;
    uint256 public MAX_SUPPLY_PER_TOKEN = 1000;
    uint256 public PRE_SALE_DISCOUNT_PRICE = 1000000000000000;
    uint256 public PUBLIC_SALE_PRICE = 2000000000000000;
    string public name = "ERC1155 trial token!!!!";
    ...
    // https://docs.paper.xyz/docs/only-paper-restricted-smart-contract-functions
    constructor(
    ...
    function preSaleMint(
        ...
        external
        payable
        onlyPaper(keccak256(abi.encode(toAddress, amount)), _nonce, _signature)
    {
        require(
            (isFree && amount <= MAX_PRE_SALE_FREE_AMOUNT) ||
                (!isFree && amount <= MAX_PRE_SALE_DISCOUNT_AMOUNT),
            "Max token amount for pre-sale exceeded"
        );

        uint8 id = _preSaleTokenId();
        require(
            balanceOf(toAddress, id) == 0,
            "You can only mint pre-sale tokens once"
        );

        uint256 minted = mintedTokens[id];
        require(
            minted + amount <= MAX_SUPPLY_PER_TOKEN,
        ...
        mintedTokens[id] += amount;
    }

    // https://docs.paper.xyz/reference/eligibilitymethod
    function checkPreSaleClaimEligibility(
        ...
        uint256 minted = mintedTokens[id];
        if (minted + amount > MAX_SUPPLY_PER_TOKEN) {
    ...
    // fix token ID for now
    function _preSaleTokenId() private pure returns (uint8) {
        return 1;
    }

    function _publicSaleTokenId() private pure returns (uint8) {
        return 2;
    }
}
