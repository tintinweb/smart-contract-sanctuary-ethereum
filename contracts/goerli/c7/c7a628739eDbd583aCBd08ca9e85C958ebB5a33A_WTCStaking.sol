...
contract WTCStaking is Context, Ownable {
    ...
    address public raffle = 0xe99f9143E44b28B2BDDFd7F5B11ce69F63F11667;
    address public whitelist = 0xD1C365F18F15d4868Bfd9f71bBEC5f7C5AE21E26; 
    address public futurecontract; //additional future proof
    //NB Variables//
    uint public basereward = 10e18; // base reward of 10 CTKN per NFT
    uint public wlreward = 3e18; // base reward of 3 CTKN per NFT
    uint public boostpernft = 5; //5%
    bool public stakingenabled;
    bool public mintpayout; //This is to mint directly from the token address
    uint private collectionfee = 620000000000000; //0.00062ETH @ $1 31/10/2022
  ...
  ////////////STAKING FUNCTIONS//////////
   ...
   function calculatePayoutAmount(address _payee) public view returns(uint)
   {
     ...
     if(block.timestamp >= lastcollect + 20 minutes) 
     {
         temp2 = temp.mul(20);
     }
     if(block.timestamp >= lastcollect + 30 minutes)
     {
         temp2 = temp.mul(30);
     }
     if(block.timestamp >= lastcollect + 60 minutes) 
     {
         temp2 = temp.mul(75);
     }
     if(block.timestamp >= lastcollect + 90 minutes) 
     {
         temp2 = temp.mul(100);
     }
     if(block.timestamp >= lastcollect + 180 minutes)
         temp2 = temp.mul(300);
     }
