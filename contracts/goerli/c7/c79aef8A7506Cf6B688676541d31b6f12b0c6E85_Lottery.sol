...
error Lottery__Pending();
error Lottery__WinTransferFailed();
error Lottery__UpkeepNotNeeded(uint256 balance, uint256 participants, uint256 status);
...
contract Lottery is VRFConsumerBaseV2, AutomationCompatibleInterface {
    ...
    uint16 constant MIN_REQUEST_CONFIRMATIONS = 3;
    uint32 private immutable i_callbackGasLimit;
    uint32 private constant RANDOM_WORDS_COUNT = 1;

    // chainlink automation vars
    ...
    function fulfillRandomWords(uint256, uint256[] memory _randomWords) internal override {
        ...
        s_recentWinner = s_participants[indexOfWinner];
        s_participants = new address payable[](0);
        s_lotteryStatus = LotteryStatus.OPEN;
        (bool success, ) = s_recentWinner.call{value: address(this).balance}("");
        s_lastTimeStamp = block.timestamp;
        if (!success) {
            revert Lottery__WinTransferFailed();
        }
    ...
    function performUpkeep(
        ...
        s_lotteryStatus = LotteryStatus.PENDING;
