...
import '@coti-cvi/contracts-cvi/contracts/RebaserV3.sol';
...
contract CVIUSDCRebaserV3 is RebaserV3 {
  constructor() RebaserV3() {}
}
...
import "./interfaces/IRebaserManagement.sol";
import "./KeepersBased.sol";

contract Rebaser is IRebaserManagement, KeepersBased {
    IVolatilityToken public volatilityToken;
    IUniswapV2Pair[] public pairs;

    function initialize(IVolatilityToken _volatilityToken, IUniswapV2Pair[] memory _uniswapPairs) external initializer {
        __KeepersBased_init();
        volatilityToken = _volatilityToken;
        pairs = _uniswapPairs;
    }

    function perform() internal override {
...
contract RebaserV3 is KeepersBased {
    IThetaVaultV3Management public thetaVault;

    function initialize(IThetaVaultV3Management _thetaVault) external initializer {
        __KeepersBased_init();
        thetaVault = _thetaVault;
    }

    function perform() internal override {
        require(address(thetaVault) != address(0), "Set theta vault");

        thetaVault.rebaseCVI();
    }
}
...
import "./interfaces/IKeepersBased.sol";

abstract contract KeepersBased is OwnableUpgradeable, KeeperCompatibleInterface, IKeepersBased {
    uint256 public lastUpkeepTime;
    uint32 public upkeepInterval;
    uint32 public upkeepTimeWindow;

    bool public enableWhitelist;
    mapping (address => bool) public operators;

    function __KeepersBased_init() internal onlyInitializing {
        OwnableUpgradeable.__Ownable_init();
        lastUpkeepTime = 0;
        upkeepInterval = 1 days;
        upkeepTimeWindow = 15 minutes;
    ...
    function setOperatorAddress(address user, bool isAllowed) external override onlyOwner {
        operators[user] = isAllowed;
    }
    ...
    function performUpkeep(
        ...
        require(!enableWhitelist || operators[msg.sender], "Whitelisted addresses only");
        require(isUpkeepNeeded(), "Bad time window");
        lastUpkeepTime = block.timestamp;
        perform();
    }

    function isUpkeepNeeded() internal virtual view returns (bool) {
        //TOOD: Relevant for rebaser, not for others
        return block.timestamp - (lastUpkeepTime  / 1 days) * 1 days >= upkeepInterval; // && block.timestamp % 1 days <= upkeepTimeWindow;
    }

    function perform() internal virtual;
}
...
interface IKeepersBased {
    function setOperatorAddress(address user, bool isAllowed) external;
