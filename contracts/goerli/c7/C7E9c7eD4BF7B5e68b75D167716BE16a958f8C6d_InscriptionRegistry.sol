 ...
 * @notice This registry stores a record of BTC inscriptions that ETH punkholders
...
contract InscriptionRegistry is Ownable {
    ...
    /// @notice Sets an inscription where the sender owns a punk
    /// @param _tokenId The token id
    /// @param _inscription The inscription id to set
    function setInscriptionWithPunk(
        uint256 _tokenId,
        string memory _inscription
    ) external {
        require(isWithPunk(_tokenId, msg.sender), "Punk not owned by sender");

        setInscription(_tokenId, punkContractAddress, _inscription);
    }
    ...
    /// @param _contractAddress The contract address for the NFT
    /// @param _inscription The inscription id to set
    function setInscriptionWithERC721(
        uint256 _tokenId,
        address _contractAddress,
        string memory _inscription
    ) external {
    ...
    /// @notice Sets an inscription where the sender has a punk delegated
    /// @param _tokenId The token id
    /// @param _inscription The inscription id to set
    /// @param _punkHolderAddress The address with the punk in
    function setInscriptionWithPunkDelegation(
        uint256 _tokenId,
        string memory _inscription,
        address _punkHolderAddress
    ) external {
        require(
            isWithPunk(_tokenId, _punkHolderAddress),
            "Punk not owned by address"
        );

        require(
            isWithDelegation(_tokenId, punkContractAddress, _punkHolderAddress),
            "Punk not delegated to sender"
        );

        setInscription(_tokenId, punkContractAddress, _inscription);
    }

    /// @notice Sets an inscription where the sender has an ERC721 delegated
    /// @param _tokenId The token id
    /// @param _contractAddress The contract address for the NFT
    /// @param _inscription The inscription id to set
    /// @param _tokenHolderAddress The address with the token in
    function setInscriptionWithERC721Delegation(
        uint256 _tokenId,
        address _contractAddress,
        string memory _inscription,
