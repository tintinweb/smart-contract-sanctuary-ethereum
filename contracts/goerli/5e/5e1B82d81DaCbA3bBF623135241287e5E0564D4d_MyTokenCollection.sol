...
contract MyTokenCollection is ERC721Enumerable, Ownable {
  ...
  string public constant CONCEALED_TOKEN_URI = string(abi.encodePacked("ipfs://mynft.io/meta/unrevealed.json"));

  // PROVENANCE: Use the same hash mechanism as BAYC's.
  string public constant MINT_PROVENANCE = "THIS_SHOULD_BE_CHANGED_BEFORE_DEPLOYMENT";
  string public constant CRAFT_PROVENANCE = "THIS_SHOULD_BE_CHANGED_BEFORE_DEPLOYMENT";
  ...
  constructor() ERC721("MyToken", "MT") {
    baseURI = "ipfs://mynft.io/meta/";
    craftBaseURI = "ipfs://mynftcraft.io/meta/";
    whitelistMerkleRoot = 0x42e089ff413a4f412fc90163cc749d032a00e636391ceced4d4037681e3bfe94;
    waitingListMerkleRoot = 0x42e089ff413a4f412fc90163cc749d032a00e636391ceced4d4037681e3bfe94;
  ...
  function isWaitingListActive() public view returns (bool) {
  ...
  function isEligibleForWhitelistMint(bytes32[] calldata _merkleProof) external view returns (bool) {
    require(isWhitelistActive(), "the whitelist mint is paused");
    ...
    require(MerkleProof.verify(_merkleProof, whitelistMerkleRoot, leaf), "invalid proof, you're not on the list.");
  ...
  function isEligibleForWaitingListMint(bytes32[] calldata _merkleProof) external view returns (bool) {
    require(isWaitingListActive(), "the waiting list mint is paused");
    require(block.timestamp >= waitingListMintStartTime, "the waiting list mint is not started yet");
    require(_maxAvailableTokens > 0, "all NFTs are minted!");

    bytes32 leaf = keccak256(abi.encodePacked(msg.sender));
    require(MerkleProof.verify(_merkleProof, waitingListMerkleRoot, leaf), "invalid proof, you're not on the list.");
  ...
  function whitelistMint(bytes32[] calldata _merkleProof) external returns (uint256) {
    this.isEligibleForWhitelistMint(_merkleProof);
  ...
  function waitingListMint(bytes32[] calldata _merkleProof) external returns (uint256) {
    this.isEligibleForWaitingListMint(_merkleProof);
