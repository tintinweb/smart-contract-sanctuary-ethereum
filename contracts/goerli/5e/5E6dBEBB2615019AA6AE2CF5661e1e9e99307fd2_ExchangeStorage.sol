...
contract ExchangeStorage is StorageBase, IExchangeStorage {
    ...
    // 100% of makerOrder is filled when => getFillsValue(rightOrderKeyHash) == rightOrder.makeAsset.value
    // 100% of takerOrder is filled when => getFillsValue(leftOrderKeyHash) == leftOrder.makeAsset.value
...
contract Exchange is Pausable, NonReentrant, ExchangeAutoProxy, IExchange {
    ...
    // Setter functions
    //
    // This function is only on testnet contracts for convenience.
    function setOrderBook(address orderBook) external onlyOwner {
        _storage.setOrderBook(orderBook);
    }
