...
/// @title CryptoDickButtDA
/// @notice contract to implement dutch auction mint functionality for the CDB Pass
...
interface CryptoDickButt {
    function mintExternal(address recipient) external returns(uint256);
    function totalSupply() external view returns(uint256);
}

contract CryptoDickButtDA is Ownable, ReentrancyGuard {
    ...
    CryptoDickButt public cdbContract;
    ...
    {   
        ...
        cdbContract = CryptoDickButt(passContractAddress);
    }
    ...
    /// @notice function to set pass contract address
    /// @dev requires contract owner
    function setCDBContract(address cdb) external onlyOwner {
        cdbContract = CryptoDickButt(cdb);
    }
    ...
    function ownerMint(address[] calldata recipients) external onlyOwner {
        require(cdbContract.totalSupply() + recipients.length <= maxSupply, "no supply left");
        for (uint256 i = 0; i < recipients.length; i++) {
            cdbContract.mintExternal(recipients[i]);
        }
    }
    ...
    function buy() external payable nonReentrant {
        ...
        require(cdbContract.totalSupply() < auctionSupply, "no supply left");

        uint256 price = getPrice();
        require(msg.value >= price, "not enough ether attached");

        _numMinted[msg.sender]++;
        uint256 tokenId = cdbContract.mintExternal(msg.sender);
        ...
        emit Sale(address(cdbContract), tokenId, price, msg.sender);
    }
