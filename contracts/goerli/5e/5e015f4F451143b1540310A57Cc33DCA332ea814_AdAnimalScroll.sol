...
contract AdAnimalScroll is ERC1155Burnable, Ownable, ReentrancyGuard, Pausable {
    ...
    modifier onlyValidTokenIDs(uint256 tokenId) {
        require(tokenId > 0 && tokenId < 5);
        _;
    }

    constructor() ERC1155("https://www.adanimals.art/scroll/") {
    ...
    function mint(
        ...
        uint256 id,
        uint256 amount
    ) public onlyValidTokenIDs(id) onlyOwner {
    ...
    function mintByClaimManager(
        ...
        uint256 id,
        uint256 amount
    ) public onlyValidTokenIDs(id) {
        require(_msgSender() == claimManager, "Only Claim Manager can mint");
        _mint(account, id, amount, "");
    }

    function addListing(uint256 tokenId, uint256 price)
        public
        onlyValidTokenIDs(tokenId)
        nonReentrant
        whenNotPaused
    {
    ...
    function buy(uint256 listingId) public payable nonReentrant whenNotPaused {
        require(listings[listingId].isActive, "Must be active");
        require(msg.value >= listings[listingId].price, "Not enough");
        listings[listingId].isActive = false;
        _safeTransferFrom(
            address(this),
            _msgSender(),
