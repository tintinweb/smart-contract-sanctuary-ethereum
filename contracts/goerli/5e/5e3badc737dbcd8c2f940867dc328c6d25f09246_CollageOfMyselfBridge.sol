...
// File contracts/CollageOfMyselfBridge.sol
...
contract CollageOfMyselfBridge is ERC721EnumerableUpgradeable, OwnableUpgradeable {
    ...
    function __CollageOfMyselfBridge_init_unchained(string memory initBaseURI_, string memory initNotRevealedUri_) internal onlyInitializing {
        unchecked {
            ...
            mintCost = 1 ether; // 1 MATIC
            transferCost = 1 ether; // 1 MATIC to transfer a token to another account without using a approved exchange
            wmatic = 0x0d500B1d8E8eF31E21C99d1Db9A6444d3ADf1270; // W-MATIC contract address to be used for token transfers
            bridge = address(0); // Bridge address
            maxSupply = 500;
            maxMintAmount = 20;
            paused = true;
            revealed = false;
            applyTransferFee = false;
            baseExtension = ".json";
    ...
    function isReservedFor(address _owner, uint256 _tokenId) 
        public 
        view 
        returns (bool) {
