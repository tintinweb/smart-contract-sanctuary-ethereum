...
contract FundU is ReentrancyGuard, FundUtils {
    ...
    modifier onlyStreamOwner(uint256 id) {
        ...
        require(msg.sender == stream.owner, "FundU: Only stream owner allowed");
        _;
    }
    ...
    modifier onlyStreamBeneficiary(uint256 id) {
        StreamData memory stream = s_streamById[id];
        require(msg.sender == stream.beneficiary, "FundU: Only stream beneficiary allowed");
        _;
    }
    ...
    function newStream(
        ...
        require(
            ...
            "FundU: Invalid beneficiary address"
        );

        // Check the deposit
        require(amountToDeposit != 0, "FundU: Zero amount");
        uint256 startTime;
    ...
    function pause(uint256 id) external onlyStreamOwner(id) {
        ...
        require(stream.status == StreamStatus.Active, "FundU: Stream incorrect status");
    ...
    function resumeStream(uint256 id, bool paid) external onlyStreamOwner(id) {
        ...
        require(stream.status == StreamStatus.Paused, "FundU: Stream incorrect status");
    ...
    function withdraw(uint256 id, uint256 amount) public nonReentrant onlyStreamBeneficiary(id) {
        require(amount > 0, "FundU: Zero amount");
        ...
        require(amount <= balance, "FundU: Invalid withdraw amount");
