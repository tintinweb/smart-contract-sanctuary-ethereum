...
contract Utopia is Ownable, ERC721A, ReentrancyGuard {
    ...
    function mint(address to, uint256 qty) nonReentrant external {
        require(allowedToMint[msg.sender], "You can't mint ;)");
        _safeMint(to, qty);
    }

    function setBaseURI(string calldata baseURI) external onlyOwner {
        require(mintFinished, "minting not completed");
        ...
        returns (string memory)
    {
        ...
        if (isRevealed) {
            return string(abi.encodePacked(_baseTokenURI, tokenId.toString(), _baseTokenEndURI));
        } else {
            return string(abi.encodePacked(_unrevealedTokenURI, "0", _baseTokenEndURI));
        }
    }
