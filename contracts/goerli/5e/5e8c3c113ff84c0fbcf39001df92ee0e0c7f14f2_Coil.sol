...
contract Coil is ERC20, Ownable {
    ...
    event SetStakingAddress(address indexed oldStakingContract, address indexed newStakingContract);
    ...
    function _transfer(
        ...
        if (_limitsInEffect && !isSkipLimits) {
            ...
            if (isBuy) {
                uint256 _stakingBalance = getStakingBalance(to);
                require(amount <= _maxTransactionAmount, "Buy transfer amount exceeds the maxTransactionAmount.");
                require(amount + balanceOf(to) + _stakingBalance <= _maxWallet, "Max wallet exceeded");
            } else if (isSell) {
    ...
    function enableTrading(uint256 deadblocks) public onlyOwner {
        _tradingActive = true;
        _launchTime = block.timestamp + deadblocks;
    }

    // @notice Change the address of the lottery wallet
    /// @param _stakingAddress The new address of the lottery wallet
    function setStakingContract(address _stakingAddress)
        external
onlyOwner    {
        require(
        ...
        address _oldStakingAddress = stakingAddress;
        stakingAddress = _stakingAddress;
        excludeFromFees(_stakingAddress, true);
        emit SetStakingAddress(
        _oldStakingAddress, _stakingAddress);
    }
    ...
    function getStakingBalance(address account) internal view returns (uint256){
                if(stakingAddress!=address(0)){
                return IStakingRewards(stakingAddress).balanceOf(account);} else return 0;
    }
