...
contract Staking4 is OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    event Deposit(address indexed user, uint128 stakeAmount, uint128 stakeType, uint128 stakePeriod, uint256 time, uint128 poolTotalStaked);
    ...
    function initialize(address _ownerWallet, address _stakedToken, 
                        ...
                        uint128 _withdrawlsFees, uint128 _rewardRate, uint128 _emergencyFees, uint128 _platformFee, address _treasury) 
    ...
    function deposit(address user,uint128 _amount, uint128 _stakeType) external payable nonReentrant onlyOwner returns(uint128 emitAmount, uint128 _period, uint128 _totalPoolStaked) {
        ...
        if(stakes.stakeAmount == 0) {
            ...
            emit Deposit(user, actualAmount, _stakeType, stakeType.stakePeriod, block.timestamp, stakeType.totalStakedIn);
            ...
            _totalPoolStaked = stakeType.totalStakedIn;
            emit Deposit(user, emitAmount, _stakeType, stakeType.stakePeriod, block.timestamp, stakeType.totalStakedIn);
    ...
    function compound(address user, uint128 _stakeType) external payable nonReentrant onlyOwner returns(uint128, uint128, uint128) {
        ...
        uint128 stakeTimeTillNow;
            ...
            emit Compound(user, totalRewardTillNow, _stakeType, block.timestamp, stakeType.totalStakedIn);
            return (totalRewardTillNow, stakeType.stakePeriod, stakeType.totalStakedIn);
