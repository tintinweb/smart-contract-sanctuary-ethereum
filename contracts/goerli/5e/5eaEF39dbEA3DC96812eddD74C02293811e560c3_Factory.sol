...
contract Factory is Helper, Initializable {
    ...
    ) external payable onlyAdmins(address(this), _daoAddress) {
    ...
    Helper
{
    ...
    function updateProposalAndExecution(
        address _contract,
        bytes memory _data
    ) external onlyAdmins(factoryAddress, address(this)) nonReentrant {
    ...
    Helper
{
    ...
    function updateProposalAndExecution(
        address _contract,
        bytes memory _data
    ) external onlyAdmins(factoryAddress, address(this)) {
        if (_contract == address(0))
            revert AddressInvalid("_contract", _contract);
        (bool success, ) = _contract.call(_data);
        require(success);
...
contract Helper {
    ...
    modifier onlyAdmins(address _factory, address _dao) {
        require(
            msg.sender == Factory(_factory).getDAOdetails(_dao).ownerAddress
        );
        _;
    }
