...
contract MintERC20 is ERC1155Holder {
    ...
    function mint(
        MintERC20Params calldata params,
        TierType tierType,
        uint32 projectId,
        uint32 raiseId,
        uint32 tierId,
        ...
        params.currency.safeIncreaseAllowance(
            address(params.raises),
            params.mintPrice
        );
        params.raises.mint(projectId, raiseId, tierId, amount, new bytes32[](0));
        params.tokens.token(params.tokenId).safeTransferFrom(
            address(this),
            msg.sender,
            params.tokenId,
            amount,
...
contract MintETH is ERC1155Holder {
    function mint(
        ...
        params.raises.mint{value: msg.value}(projectId, raiseId, tierId, amount, new bytes32[](0));
        params.tokens.token(params.tokenId).safeTransferFrom(
            address(this),
            msg.sender,
            params.tokenId,
...
contract PaperAdapter is IPaperAdapter, Controllable, ERC1155Holder, ReentrancyGuard {
    using SafeERC20 for IERC20;
    ...
    address mintETHImpl;
    address mintERC20Impl;

    constructor(address _controller) Controllable(_controller) {
        MintETH mintETH = new MintETH();
        mintETHImpl = address(mintETH);

        MintERC20 mintERC20 = new MintERC20();
        mintERC20Impl = address(mintERC20);
    }

    function mint(address receiver, uint32 projectId, uint32 raiseId, uint32 tierId, uint256 amount)
        external
        payable
        override
        nonReentrant
    ...
    {
        Raise memory raise = IRaises(raises).getRaise(projectId, raiseId);
        Tier memory tier = IRaises(raises).getTiers(projectId, raiseId)[tierId];
        ...
        if (currency == ETH) {
            ...
            uint256 mintPrice = tier.price * amount;
        ...
        ITokens(tokens).token(tokenId).safeTransferFrom(address(this), receiver, tokenId, amount, "");
    }
...
interface IPaperAdapter is IControllable, IAnnotated {
    ...
    /// @notice Mint `amount` of tokens to `receiver` for the given `projectId`,
    /// `raiseId`, and `tierId`. Caller must provide ETH or approve ERC20 amount
    /// equal to total cost.
    /// @param receiver address token recipient.
