...
contract OCASnake is Ownable, ERC721A, ERC2981, ReentrancyGuard, OCASnakeArt {
    ...
    function recursiveNameFind(
        ...
        uint256 snakeIndex = snakeNameIndex * adjectiveIndex;

        if (index == 0 && tokenId != 0) {
            return this.recursiveNameFind(tokenId - 1, snakeIndex, seed);
        } else if (snakeIndex != index && tokenId != 0) {
            return this.recursiveNameFind(tokenId - 1, snakeIndex, seed);
