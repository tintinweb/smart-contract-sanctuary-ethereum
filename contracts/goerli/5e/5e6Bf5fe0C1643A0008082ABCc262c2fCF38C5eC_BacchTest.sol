...
contract BacchTest is ERC721, ERC721Pausable, Ownable {
    using Strings for uint256;
    using Strings for address;

    //SUPPLY
    uint256 public MAX_SUPPLY = 1855;
    ...
    constructor(address _usdByEthFeedAddress, address _signer)
        ERC721("BacchTest", "BCT")
    {
    ...
    function drop(
        ...
        require(
            ECDSA.recover(ECDSA.toEthSignedMessageHash(hashMessageVar), signature) ==
                signer,
            "Invalid signature"
        );
        for (uint256 index = 0; index < tokenIds.length; index++) {
    ...
    function hashMessage(address to, uint256[] memory tokenIds) public pure returns (bytes32) {
        ...
        bytes memory message = (
            abi.encodePacked(to, stringTokenIds)
        );
    ...
    function setMinter(address _newSigner) external onlyOwner {
        signer = _newSigner;
    }
