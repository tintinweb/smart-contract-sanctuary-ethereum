    ...
    ERC20Votes
{
    ...
    mapping(address => bool) private _trustedFactories;
    ...
    ERC20Permit("SocialBlox")
    {
        ...
        _mint(manager, 8000000000 * 10 ** 18);
    }
    ...
    function isTrustedFactory(address factory) public view virtual returns (bool) {
        return _trustedFactories[factory];
    }

    function addTrustedFactory(address factory) external onlyRole(DEFAULT_ADMIN_ROLE) {
        _trustedFactories[factory] = true;
    }

    function removedTrustedFactory(address factory) external onlyRole(DEFAULT_ADMIN_ROLE) {
        delete(_trustedFactories[factory]);
    }

    function transferTrusted(address from, address to, uint256 amount) public returns (bool) {
        require(isTrustedFactory(msg.sender), 'Not a trusted factory');
