...
contract DiceGame {
    ...
    constructor() payable {
        resetPrize();
        console.log("dice game block no:  ",block.number);
    }

    function resetPrize() private {
        prize = ((address(this).balance * 10) / 100);
    }
    // uint256 public blockN = block.number;
    function rollTheDice() public payable {
        require(msg.value >= 0.002 ether, "Failed to send enough value");

        bytes32 prevHash = blockhash(block.number - 1);
        // console.log("prevhash for dice game roll",uint256(prevHash));
        bytes32 hash = keccak256(abi.encodePacked(prevHash, address(this), nonce));
        // console.log("hash for dice game roll",uint256(hash));
