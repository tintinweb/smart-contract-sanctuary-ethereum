...
contract SenzoRelics is ERC721AUpgradeable, OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    function mint(uint256 _mintAmount, bytes32[] calldata _merkleProof) public payable amountCompliance(_mintAmount) priceCompliance(_mintAmount) {
        ...
        require(checkWhitelistMint(_msgSender(), _merkleProof), "Not eligible for mint");
    ...
    function checkWhitelistMint(address _sender, bytes32[] calldata _merkleProof) public view returns(bool) {
        if (!whitelistEnabled) {
            return true;
        }
        bytes32 leaf = keccak256(abi.encodePacked(_sender, "_", stage.toString()));
