...
import "./GUMIToken.sol";

/**
 * @dev GUMIWithdraw, including:
 *
...
contract GUMIWithdraw is Context, Ownable, Pausable {
    ...
    /**
     * @dev Emitted when `value` tokens are minted to one account (`to`) with
     * Yay user id (`userId`).
     *
     * Note that `value` may be zero.
     */
    event Withdraw(address indexed to, uint256 value, string userId);

    address private _signer;
    mapping(bytes => bool) public isSignatureUsed;

    GUMIToken private gumiContract;

    /**
     * @dev Sets the values for {gumiContract}
     * {gumiContract} is immutable: they can only be set once during construction.
     */
    constructor(address gumiContractAddress) {
        gumiContract = GUMIToken(gumiContractAddress);
    ...
    function transferSigner(address newSigner) public virtual onlyOwner {
        require(newSigner != address(0), "GUMIWithdraw: new signer is the zero address");
    ...
    function hash(address to, uint256 amount, string calldata userId, uint expiryDateTime) public pure returns (bytes32) {
        bytes32 _hash = keccak256(abi.encodePacked(to, amount, userId, expiryDateTime));
     ...
     * total supply.
     *
     * Emits a {Withdraw} event with Yay user id `userId`.
     *
     * Requirements:
     *
     * - signature must be valid.
     */
    function withdraw(address to, uint256 amount, string calldata userId, uint expiryDateTime, bytes calldata signature) public {
        require(block.timestamp < expiryDateTime, "GUMIWithdraw: the signature is already expired");
        require(isSignatureUsed[signature] == false, "GUMIWithdraw: the signature already redeemed");
        require(SignatureChecker.isValidSignatureNow(_signer, hash(to, amount, userId, expiryDateTime), signature), "GUMIWithdraw: the signature is invalid");

        isSignatureUsed[signature] = true;
        gumiContract.mint(to, amount);
        emit Withdraw(to, amount, userId);
    }

    /**
     * @dev Pauses withdraw function.
     *
     * See {Pausable-_pause}.
     *
     * Requirements:
    ...
    /**
     * @dev Unpauses withdraw function.
     *
