...
import {
    ...
    PROOFTokens,
    IDelegationRegistry
} from "poc-ticket/PoCTicket.sol";

contract DevPoCTicket is PoCTicket {
    constructor(
        address admin,
        string memory baseTokenURI,
        PROOFTokens memory tokens,
        IDelegationRegistry delegationRegistry,
        address payable primarySalesReceiver,
        address bnSigner
    )
        PoCTicket(
            admin,
            PoCTicket.ERC721MetadataSetup({
                name: "DevPoCTicket",
                symbol: "DevPoCT",
    ...
    {
        _grantRole(DEFAULT_STEERING_ROLE, address(this));
    }

    function devmint(uint256 num) public {
        devmint(msg.sender, num);
    }

    function devmint(address to, uint256 num) public {
        _mintPurchase(to, num);
    }

    function devSetStage(Stage stage_) public {
        stage = stage_;
    }

    function devSetPrices(
        uint128 reducedTicketPrice_,
        uint128 fullTicketPrice_,
        uint128 upgradePrice_
    ) public {
        this.setPurchasePrices(reducedTicketPrice_, fullTicketPrice_);
        this.setUpgradePrice(upgradePrice_);
    }
}
...
contract DeployTestnet is Script {
    ...
    PROOFTokens public tokens;
    DevPoCTicket public poc;
    address payable public treasury;

    function setUp() public {
        assert(block.chainid == 5);

        tokens.proof = IERC721(0x1166ce7a0eb78fa0FcBD1AD42BEebA68447e07e6);
        tokens.moonbirds = IERC721(0x3Cb5c6448eD756233fe9b91aBC4dB27a1cc17080);
        tokens.oddities = IERC721(0xF355ED8Fbc10A22F775797394a52540Ce3B3eb6f);
        treasury = payable(0xBDDB3A33CB29EC89c4dFf699fEFCeB2800AA6d36);

        // poc = DevPoCTicket(0xfdC74cf42B439753eE0991da56b9b2AD370500CF);
    }

    function run() public {
        vm.startBroadcast();

        if (address(poc) == address(0)) {
            poc = new DevPoCTicket(
                treasury,
                "https://www.721.so/api/example/metadata/",
                tokens,
                delegationRegistry,
                treasury,
                address(0)
            ...
            poc.devSetPrices(
                poc.reducedTicketPrice() / 100,
                poc.fullTicketPrice() / 100,
                poc.upgradePrice() / 100
            );
            // poc.devmint(1);
        }

        // poc.setStage(PoCTicket.Stage.GeneralAdmission);
        // poc.airdrop(address(this), 250);
    }
}
...
abstract contract Airdropper is AccessControlEnumerable {
     ...
     */
    uint256 internal constant _MAX_NUM_AIRDROPS = 250;
    ...
    {
        if (_numAirdropped + num > _MAX_NUM_AIRDROPS) {
...
/**
 * @notice PoC Ticket module that verifies PROOF NFT delegations.
 */
contract DelegationChecker {
    ...
    function _checkDelegation(
        ...
        if (delegationType == IDelegationRegistry.DelegationType.CONTRACT) {
            _checkContractDelegation(delegate, delegator, _proof, pcRedemptions);
            _checkContractDelegation(
...
abstract contract Minter is ERC721ATransferRestricted {
     ...
     * @dev Reverts for airdropped tokens.
     */
    function _mixHash(uint256 ticketId)
        internal
        view
        virtual
        returns (uint256)
    ...
    BaseTokenURI
{
    ...
    error TooManyPurchasesRequestedFromToken(IERC721, uint256);
    error TooManyPurchasesRequested();
    ...
    constructor(
        ...
        Upgrader(125, bnSigner)
        PurchaseLimiter(4000)
        ...
        returns (TokenInfo[] memory infos)
    {
        ...
        for (uint256 i; i < tokenIds.length; ++i) {
            infos[i] = TokenInfo({
                upgraded: upgraded(tokenIds[i]),
                airdropped: _airdropped(tokenIds[i]),
                upgradable: _upgradable(tokenIds[i]),
                revealBlockNumber: _revealBlockNumber(tokenIds[i])
            });
        }
    }
     ...
     * @param vault The vault address if delegation is used. Has to be the
     * caller otherwise.
     * @param delegationType The lowest type of delegation that is used for all
     * tokens that will be used for redemption.
    ...
    function purchase(
        ...
        _checkStage(pcRedemptions, mbRedemptions, oddRedemptions, numGA);
        _checkDelegation(
            msg.sender,
            vault,
            delegationType,
        ...
        )
    {
        ...
        if (stage >= Stage.Moonbirds) {
            ...
            mbCosts = new uint[](2);
            mbCosts[0] = reducedTicketPrice;
            mbCosts[1] = reducedTicketPrice;
        }
        if (stage >= Stage.Oddities) {
            oddCosts = new uint[](1);
            oddCosts[0] = reducedTicketPrice;
        }
    }
    ...
    function _redemptionCost(
        ...
        uint256 maxUsage = costs.length;
        uint256 totalCost;
        for (uint256 i; i < redemptions.length; ++i) {
            uint256 usageStart = numTokenUsed(token, redemptions[i].tokenId);
            uint256 usageEnd = usageStart + redemptions[i].num;

            if (usageEnd > maxUsage) {
                revert TooManyPurchasesRequestedFromToken(
                    token, redemptions[i].tokenId
                );
            }

            for (uint256 usage = usageStart; usage < usageEnd; ++usage) {
    ...
    /**
     * @notice Convenience overload to check if redemptions are already allowed
    ...
    function _checkStage(
        ...
        // Technically, `redemptions.length` is not the actual number of
        // purchases as assumed in `_checkStage`. However, since we are only
        // interested in checking if it is non-zero, it is still fine and more
        // gas-efficient.
        _checkStage(Stage.ProofCollective, pcRedemptions.length);
        _checkStage(Stage.Moonbirds, mbRedemptions.length);
        _checkStage(Stage.Oddities, oddRedemptions.length);
        _checkStage(Stage.GeneralAdmission, numGA);
    }

    /**
     * @notice Checks if the requested purchases can be fulfilled at the
     * current stage.
     */

    function _checkStage(Stage minStage, uint256 numPurchase) internal view {
        if (numPurchase == 0) {
            return;
        }
    ...
    function _mixHash(uint256 ticketId)
        ...
        returns (uint256)
    {
        return Minter._mixHash(ticketId);
    }
...
contract PurchaseLimiter is AccessControlEnumerable {
    ...
    uint16 internal immutable _maxSellable;
    ...
    uint16 private _defaultTotalPurchaseLimit;
    uint16 private _defaultGeneralAdmissionPurchaseLimit;
    ...
    constructor(uint16 maxSellable) {
        _defaultTotalPurchaseLimit = 10;
        _defaultGeneralAdmissionPurchaseLimit = 2;
        _maxSellable = maxSellable;
    }
    ...
    {
        return _purchaseCountsAndLimits(wallet);
    }
    ...
    function _purchaseCountsAndLimits(address wallet)
        private
        view
        returns (PurchaseCountsAndLimits memory)
    {
...
abstract contract ERC721ATransferRestricted is ERC721ATransferRestrictedBase {
    ...
    modifier bypassTransferRestriction() {
        ...
        _transferRestriction = TransferRestriction.None;
        _;
...
abstract contract Upgrader is UpgraderEvents, AccessControlEnumerable {
     ...
     * @dev TODO: Should this be a ternary state to allow downgrades of
     * automatically upgraded tokens?
     */
        ...
        private
    {
        ...
        bytes32 msgHash =
            ECDSA.toEthSignedMessageHash(abi.encode(blockNumber, block.chainid));
        if (ECDSA.recover(msgHash, signature) != _bnSigner) {
     ...
     */
    function upgradedFreeOfCharge(uint256 ticketId)
        external
        view
        returns (bool)
    {
    ...
    function _isRandomlyUpgradedBy(uint256 ticketId, bytes32 salt)
        ...
        returns (bool)
    {
        // Airdropped tokens cannot participate in the randomsation
        if (_airdropped(ticketId)) {
            return false;
        }

        bytes32 rand =
            keccak256(abi.encodePacked(salt, _mixHash(ticketId), ticketId));
