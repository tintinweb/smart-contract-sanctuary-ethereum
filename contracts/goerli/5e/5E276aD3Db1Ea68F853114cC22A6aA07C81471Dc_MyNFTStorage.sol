...
contract MyNFTStorage is ERC1155Holder, ReentrancyGuard {
    ...
    function createOrder(
        ...
        listOrderOnMarketplace.push(orderId);
    ...
    function cancelOrder(uint orderId) public {
        require(orderId < _orderIdTracker.current(), "Out of index");
        OrderDetail storage myOrder = listOrderDetail[orderId];
        uint length = listOrderOnMarketplace.length;
        require(myOrder.owner == msg.sender, "You not owner this order");

        listOrderOnMarketplace[orderId] = listOrderOnMarketplace[length - 1];
        listOrderOnMarketplace.pop();
        _orderIdTracker.decrement();
    ...
    function buyOrder(uint orderId) public {
        ...
        require(orderId < length, "Out of index");

        uint256 totalPrice = order.price.mul(order.quantity);
        uint256 fee = totalPrice.mul(order.feePercent).div(10000);
        uint256 realOrderPrice = totalPrice.sub(fee);
        uint256 royaltyShared = 0;
        uint256 sellerReceived = realOrderPrice;

        require(totalPrice > 0, "Order price must be positive");

        // Checker balance msg.sender

        IERC20 payment = IERC20(order.tokenPayment);
        require(
            payment.allowance(msg.sender, address(this)) >= totalPrice,
            "Insufficient allowance for proccessing buy this order"
        );
        require(
            payment.balanceOf(msg.sender) >= totalPrice,
            "Total price exceeded balance"
        );
        payment.transferFrom(msg.sender, address(admin.getFeeAccount()), fee);
        ...
        for (uint i = 0; i < royalties.length; i++) {
            ...
            require(royalties[i].account != msg.sender, "Royalty same sender");
            uint256 shared = realOrderPrice.mul(royalties[i].value).div(10000);
            royaltyShared.add(shared);
            payment.transferFrom(msg.sender, address(royalties[i].account), shared);
        }

        payment.transferFrom(
            msg.sender,
            address(order.owner),
            sellerReceived.sub(royaltyShared)
        );
        ...
        emit BuyOrder(orderId, order.indexer, msg.sender, order);
    }

    function getOrders() public view returns (uint[] memory) {
        return listOrderOnMarketplace;
    }

    function getOrderDetail(uint orderId)
        public
        view
    ...
    RoyaltiesV2Impl
{
    ...
    constructor(address _operator, address _admin)
        ERC1155("https://api.nftencer.com/item/{id}.json")
    {
        operator = _operator;
        admin = AdminConsole(_admin);
    }
    ...
    function mint(address account, uint quantity) public {
    ...
    function getRoyalties(uint _tokenId)
        public
        view
