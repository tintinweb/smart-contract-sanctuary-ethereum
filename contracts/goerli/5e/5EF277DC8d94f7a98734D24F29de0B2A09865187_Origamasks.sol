    ...
    VRFV2WrapperConsumerBase
{
        ...
        isSaleState(SaleState.VIP)
    {
        ...
        if (totalSupply + 1 > RESERVED_TOKENS) revert ExceedReservedSpots();
        // if (msg.value != mintPrice) revert IncorrectPrice();
        if (publicMints[msg.sender] >= 1) revert LimitPerWalletExceeded();
        ...
        isSaleState(SaleState.Public)
    {
        ...
        if (publicMints[msg.sender] >= LIMIT_PER_WALLET)
        ...
        isSaleState(SaleState.BuddyList)
    {
        ...
        if (msg.value != mintPrice) revert IncorrectPrice();
        if (walletMints[msg.sender] >= 1) revert LimitPerWalletExceeded();
    ...
    function additionalBuddyListMint(
        ...
        if (!verifySignature(signature_, "BuddyListPh2"))
            revert InvalidSignature();
        if (totalSupply + 1 > (maxSupply - RESERVED_TOKENS - totalSupply))
            revert SoldOut();
        if (msg.value != mintPrice) revert IncorrectPrice();
        if (walletMints[msg.sender] >= LIMIT_PER_WALLET)
            revert LimitPerWalletExceeded();
    ...
    */
    ...
    function setNewMaxLevel(uint256 maxLevel_, uint256 daysToLevelUp_) public onlyOwner{
    ...
    function isLeveledUpFromCurrentFieldTrip(uint256 tokenId_) public view returns (bool) {
        bool isLeveledUp;
    ...
    */
    ...
    function setRewardContract(uint256 level_, address contractAddress_) public onlyOwner {
        rewardContractAddress[level_] = contractAddress_;
    }

    function setRewardClaimable(uint256 level_, bool claimable_) public onlyOwner {
     ...
     */
    function setBaseTokenURI(string memory baseTokenURI_, uint256 level_) external onlyOwner {
