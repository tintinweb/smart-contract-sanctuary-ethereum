...
contract GenerateMetadata is IGenerateMetadata {
    ...
    /// @dev must change the "NFT Title (name) "
    function tokenMetadata(
        uint256 tokenId
    ) external view returns(string memory) {
        require(msg.sender == targetAddress, "GenerateMetadata: Not the target address");
        ...
        return string(
            abi.encodePacked('data:application/json;base64,',
                Base64.encode(bytes(abi.encodePacked(
                    '{"name":"', name,
                    '", "description": "', IBaseMetadata(targetAddress).description(),
                    '", "external_url": "', IBaseMetadata(targetAddress).externalUrl(),
                    '", "image" : "', imageUrl,
                    '", "attributes" : [',
                        '{"trait_type": "gender", "value": "', IBaseMetadata(targetAddress).generateGender(tokenId), '"},',
                        // '{"trait_type": "addAttr", "value": "Updated Value"},',
                        '{"trait_type": "wave", "value": "', IBaseMetadata(targetAddress).getWave(tokenId), '"}]',
                    '}'
                )))
            )
        );
