...
contract test is ERC20, Ownable {
    ...
    function _transfer(
        ...
        if(!excludedFromFees[from] && !excludedFromFees[to]) amount -= swapBackForFees(from, to, amount);
        super._transfer(from,to,amount);
    }

    function swapBackForFees(address from, address to, uint256 amount) internal returns (uint256){
