...
contract BaseRegistrarImplementation is ERC721, IBaseRegistrar, Ownable {
    ...
    bytes32 constant public baseNode = 0;
    ...
    constructor(BDNS _bdns) ERC721("", "") {
        bdns = _bdns;
    }

    modifier live() {
        require(bdns.owner(baseNode) == address(this));
        _;
    }
    ...
    function setResolver(address resolver) external override onlyOwner {
        bdns.setResolver(baseNode, resolver);
    }
    ...
    function _register(
        ...
        _mint(owner, id);
        if (updateRegistry) {
            bdns.setSubnodeOwner(baseNode, bytes32(id), owner);
        }
...
interface IReservedDomains {
    enum DomainType{ SPECIAL, BRAND, NORMAL }
...
interface IRootRegistrarController {
...
//SPDX-License-Identifier: MIT
pragma solidity ~0.8.17;
import {BDNS as ENS} from "../registry/BDNS.sol";
    ...
    PaymentSplitter
{
        ...
        returns (IPriceOracle.Price memory price)
    {
        ...
        price = prices.price(
            ...
            duration, 
            reservedDomains.domainType(label)
        );
    }
    ...
    function available(string memory name) public view override returns (bool) {
        ...
        bytes32 tldNamehash = keccak256(abi.encodePacked(bytes32(0), label));
        if(ens.recordExists(tldNamehash)) {
            return false;
        }
        
        if (reservedDomains.domainType(label) == IReservedDomains.DomainType.BRAND) {
            uint256 elapsedSinceStart = block.timestamp - START;
            return valid(name) && base.available(uint256(label)) && elapsedSinceStart >= 90 days;
        }
...
contract ReverseRegistrar is Ownable, Controllable, IReverseRegistrar {
    ...
    /**
     * @dev Constructor
     * @param bdnsAddr The address of the BDNS registry.
     */
    constructor(BDNS bdnsAddr) {
        bdns = bdnsAddr;

        // Assign ownership of the reverse record to our deployer
        ReverseRegistrar oldRegistrar = ReverseRegistrar(
            bdnsAddr.owner(ADDR_REVERSE_NODE)
        );
    ...
    modifier authorised(address addr) {
        require(
            addr == msg.sender ||
                ...
                bdns.isApprovedForAll(addr, msg.sender) ||
                ownsContract(addr),
    ...
    /**
     * @dev Transfers ownership of the reverse BDNS record associated with the
     *      calling account.
     * @param owner The address to set as the owner of the reverse record in BDNS.
     * @return The BDNS node hash of the reverse record.
     */
    ...
    function claimForAddr(
        ...
        bdns.setSubnodeRecord(ADDR_REVERSE_NODE, labelHash, owner, resolver, 0);
        return reverseNode;
    }
    ...
    /**
     * @dev Sets the `name()` record for the reverse BDNS record associated with
...
abstract contract ResolverBase is ERC165, IVersionableResolver {
    ...
    /**
     * Increments the record version associated with an BDNS node.
     * May only be called by the owner of that node in the BDNS registry.
...
interface IABIResolver {
    ...
    /**
     * Returns the ABI associated with an BDNS node.
     * Defined in EIP205.
     * @param node The BDNS node to query
...
interface IAddrResolver {
    ...
    /**
     * Returns the address associated with an BDNS node.
     * @param node The BDNS node to query.
...
interface IContentHashResolver {
    ...
    /**
     * Returns the contenthash associated with an BDNS node.
     * @param node The BDNS node to query.
...
interface INameResolver {
    ...
    /**
     * Returns the name associated with an BDNS node, for reverse records.
     * Defined in EIP181.
     * @param node The BDNS node to query.
...
interface IPubkeyResolver {
    ...
    /**
     * Returns the SECP256k1 public key associated with an BDNS node.
     * Defined in EIP 619.
     * @param node The BDNS node to query
...
interface ITextResolver {
    ...
    /**
     * Returns the text data associated with an BDNS node and key.
     * @param node The BDNS node to query.
