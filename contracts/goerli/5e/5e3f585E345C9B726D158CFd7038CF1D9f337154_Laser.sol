...
contract Laser is ERC721A, Ownable {
    ...
    constructor() ERC721A("LaserEtch", "LZR") {
    ...
    function getTokenIDS(address _addr)
        ...
        {
            uint256 totalPieces = this.balanceOf(
                _addr
            );
            string memory tokenIds;

            for (uint256 x = 0; x < totalPieces; x++) {
    ...
    function getTokenHashes(address _addr)
            public
            view
            returns (bytes32[] memory)
        {
            ...
            bytes32[] memory tokenIds = new bytes32[](totalPieces);

            for (uint256 x = 0; x < totalPieces; x++) {
                uint256 tokenNo = this.tokenOfOwnerByIndex(_addr, x);
                
                tokenIds[x] = getHash(tokenNo);
    ...
    /**
     * @notice Mint a Laser Etch.
     *
     * @param _amount the tokensToClaim.
     * @param _merkleProof the proof for the tx.
     */
    function mintLaser( uint8 _amount, bytes32[] calldata _merkleProof) public {
    ...
    */
    function getHash(uint256 tokenId) public view returns (bytes32)
    {
        ...
        return keccak256(abi.encode(getSeed(tokenId)));
        // }
    }
