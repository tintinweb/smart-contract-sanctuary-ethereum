...
contract CommitmentCircuit is SchnorrSignature {
	...
	mapping(address => mapping(uint256 => PointEC)) public _commitmentById;
	 ...
	 * @param pubKey Public key with which message was signed.
	...
	function transferCommitment(
		...
		PointEC memory senderCommitment = _commitmentById[msg.sender][senderCommitmentId];
		PointEC memory _ecEqual;
		(_ecEqual.x, _ecEqual.y) = eSub(
			senderPubKey.x,
			senderPubKey.y,
			recipientPubKey.x,
			recipientPubKey.y
		...
		require(
			...
			"invalid signature");
		require(!_spents[msg.sender][senderCommitmentId], "commitment already used");
		PointEC memory _senderCommitment = _commitmentById[msg.sender][senderCommitmentId];
		PointEC memory _ecVerify;
		PointEC memory _ecSum;
		(_ecVerify.x, _ecVerify.y) = eAdd(recipientPubKey.x, recipientPubKey.y, excessPubKey.x, excessPubKey.y);
		(_ecVerify.x, _ecVerify.y) = eSub(senderPubKey.x, senderPubKey.y, _ecVerify.x, _ecVerify.y);

		(_ecSum.x, _ecSum.y) = eAdd(recipientCommitment.x, recipientCommitment.y, excessCommitment.x, excessCommitment.y);

		require(
			_CommitmentVerify(_senderCommitment, _ecSum, _ecVerify),
			"invalid commitments"
		);

		_spents[msg.sender][senderCommitmentId] = true;

		uint256 _id = _CommitmentNewAdd(recipient, recipientCommitment);
		...
		_id = _CommitmentNewAdd(msg.sender, excessCommitment);
	 ...
	 * @param amount of xft to be deposited.
	 * @param recipientCommitment new commitment that depend on the amount and user's pub key.
	...
	function deposit(
		...
		(uint256 x, uint256 y) = ePedersenCommitmentPK(amount, pubKey.x, pubKey.y, ecH.x, ecH.y);
		require(
			x == recipientCommitment.x && y == recipientCommitment.y,
		...
		emit Deposited(msg.sender, amount, id, _commitmentById[msg.sender][id]);
	}
	...
	/**
	 * @dev Create one commitment instead of several.
	...
	function groupCommitment(
		...
		PointEC memory grouped = _commitmentById[_user][_commIds[0]];

		for (uint256 i = 1; i < _commIds.length; i++) {
			(grouped.x, grouped.y) = eAdd(
				grouped.x,
				grouped.y,
				_commitmentById[_user][_commIds[i]].x,
				_commitmentById[_user][_commIds[i]].y
			);
		}
		...
		for (uint256 i = 0; i < _commIds.length; i++) {
			_spents[_user][_commIds[i]] = true;
		}

		uint256 id = _CommitmentNewAdd(_user, _ecNewCommitment);
		emit Grouped(id, _ecNewCommitment);
	}
		...
		returns (uint256)
	{
		...
		_commitmentById[_newOwner][_id] = _commitment;
