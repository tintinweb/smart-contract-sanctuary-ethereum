    ...
    OwnableUpgradeable
{
    ...
    bool public isOpenFeatureSplit;

    function setIsOpenFeatureSplitNanoCell(bool value) external onlyTimelock {
        isOpenFeatureSplitNanoCell = value;
    }

    function setIsOpenFeatureSplit(bool value) external onlyTimelock {
        isOpenFeatureSplit = value;
    }
    ...
    function _processSplit(CellData.Cell memory cell) private {
        ...
        if (isOpenFeatureSplit) {
            ...
            if (
                cell.class == CellData.Class.SPLITTABLE_BIOMETA ||
                replacement == 0
            ) {
                _splitBiometa();
            }
            if (
                cell.class == CellData.Class.SPLITTABLE_ENHANCER ||
                replacement == 1
            ) {
                _slpitEnhancer();
            }
    ...
    function _evolve(
        ...
        if (_scientistId != 0) {
            require(
                msg.sender ==
                    IERC721Upgradeable(scientistToken).ownerOf(_scientistId),
    ...
    function addNft(
        ...
        string memory _metadataUri = IERC721MetadataUpgradeable(_nftAddress)
            .tokenURI(_tokenId);
        require(
    ...
    function createMetaCell(uint256 positionId) external override {
        require(
            IERC721Upgradeable(scientistToken).ownerOf(positionId) ==
                msg.sender,
