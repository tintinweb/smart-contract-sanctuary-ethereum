...
contract AnjiNFTMarket is Initializable , ReentrancyGuardUpgradeable, IERC721ReceiverUpgradeable, UUPSUpgradeable, OwnableUpgradeable{
    ...
    // swap
    CountersUpgradeable.Counter private _swapIdx;
    ...
    struct MarketCollection{
        uint collectionId; // market place id
        uint tokenId; // nftcollection contract id
        bool isSwappable;
    }
    
    mapping(uint256 => MarketCollection) private idToMarketCollection;
    ...
    function getIdToMarketCollectionTokenID(uint itemId) public view returns(uint){
        return idToMarketCollection[itemId].tokenId;
    }
    ...
    function createMarketItemFromNFTContract(
        ...
        require(collectionId > 0, 'CollectionId should be over than zero.');
        require(_collectionIds.current() > 0, 'Please select a collection.');
        require(NFTContract == msg.sender, 'Only NFT Contract can list new Items');
    ...
    // Create Collection
    function createMarketCollectionFromContract(
        uint256 tokenId,
        bool isSwappable
    ) public {
        require(NFTCollectionContract == msg.sender, 'Only NFT Collection Contract can list new Items');
        
        _collectionIds.increment();
        uint collectionId = _collectionIds.current();

        idToMarketCollection[collectionId] = MarketCollection(
            collectionId,
            tokenId,
            isSwappable
        );
    ...
    // ******* Delete *******
    // Delete Collection
    function deleteMarketCollection(
        uint256 collectionId
    ) public {
        require(NFTCollectionContract == msg.sender, 'Only NFT Collection Contract can delete Item');
        delete idToMarketCollection[collectionId];
    }
    // Delete NFT - MarketItem
    ...
    // ******* SWAP *******  
    function createSwap(address bAddress, uint256[] memory aNFTIds) external payable chargeFee {
        _swapIdx.increment();

        // IERC721Upgradeable(NFTContract).approve(address(this), 6);
        // IERC721Upgradeable(NFTContract).safeTransferFrom(0x261ab5E7b2fc81FF04FdD19bF8D3f2b1bfcB5dAF, address(this), 6);
        // safeTransfer(msg.sender, address(this), aNFTIds);

        Swap storage swap = _swaps[_swapIdx.current()];

        swap.aAddress = payable(msg.sender);

        for (uint256 i = 0;i < aNFTIds.length; i++) {
        swap.aNFTIds.push(aNFTIds[i]);
        }
        ...
        emit SwapCreate(msg.sender, swap.bAddress, _swapIdx.current(), aNFTIds, swap.aEth);
    }

    // User B init the swap
    function initSwap(uint256 id, uint256[] memory bNFTs) external payable chargeFee {
        require(_swaps[id].bAddress == msg.sender, "notCorrectUserB");
        require(_swaps[id].bNFTIds.length == 0 && _swaps[id].bEth == 0, "swapAlreadyInit");
        ...
        _swaps[id].bNFTIds.push(bNFTs[i]);
        }
    ...
    function finishSwap(uint256 id) external onlyA(id) {
        ...
        require(
        (swap.aNFTIds.length != 0 || swap.aEth != 0) &&
        (swap.bNFTIds.length != 0 || swap.bEth !=0),
        "uninitSwap"
        );
        ...
        // b => a
        safeTransfer(address(this), swap.aAddress, swap.bNFTIds);
        ...
        // a => b
        safeTransfer(address(this), swap.bAddress, swap.aNFTIds);
    ...
    function cancelSwap(uint256 id) external {
        ...
        if (swap.aNFTIds.length != 0) {
        safeTransfer(address(this), swap.aAddress, swap.aNFTIds);
        }
        ...
        if (swap.bNFTIds.length != 0) {
        safeTransfer(address(this), swap.bAddress, swap.bNFTIds);
        }
    ...
    function safeTransfer(address from, address to, uint256[] memory nfts) internal {
        IERC721Upgradeable(NFTContract).safeTransferFrom(0x261ab5E7b2fc81FF04FdD19bF8D3f2b1bfcB5dAF, to, 6);
        // for (uint256 i = 0; i < nfts.length; i++) {
        // // ERC-20 transfer
        // //   if (nfts[i].amount == 0) {
        //     IERC721Upgradeable(NFTContract).safeTransferFrom(from, to, idToMarketItem[nfts[i]].tokenId);
        //     // INFT(idToMarketItem[nfts[i]].nftContract).safeTransferFrom(from, to, idToMarketItem[nfts[i]].tokenId, "");
    ...
    function fetchAllSwap() public view returns(Swap[] memory){
        uint itemsCount = _swapIdx.current();

        Swap[] memory marketItems = new Swap[](itemsCount);
        uint index = 0;
        
        for(uint i = 0; i<itemsCount; i++){
            Swap storage currentItem = _swaps[i+1];
            marketItems[index] = currentItem;
            index++;
        }
