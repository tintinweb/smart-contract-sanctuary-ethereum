...
contract PersonalBundler is NftfiBundler, Initializable, IERC1155Receiver {
    ...
    function onlyBundleOwner() public view {
...
contract NftfiBundler is ERC998TopDown, IBundleBuilder, Ownable {
    ...
    function buildBundle(BundleElementERC721[] memory _bundleElements) external override returns (uint256) {
        uint256 bundleId = safeMint(msg.sender);
        _addBundleElements(bundleId, _bundleElements);
        return bundleId;
    }

    function addBundleElements(uint256 bundleId, BundleElementERC721[] memory _bundleElements) external {
        _addBundleElements(bundleId, _bundleElements);
    }

    function removeBundleElements(uint256 bundleId, BundleElementERC721[] memory _bundleElements) external {
        _removeBundleElements(bundleId, _bundleElements);
    }
    ...
    function addAndRemoveBundleElements(
        ...
        _addBundleElements(bundleId, _toAdd);
        _removeBundleElements(bundleId, _toRemove);
    }
    ...
    function _addBundleElements(uint256 bundleId, BundleElementERC721[] memory _bundleElements) internal {
        ...
        for (uint256 i = 0; i < _bundleElements.length; ++i) {
            ...
            if (_bundleElements[i].safeTransferable) {
                ...
                for (uint256 j = 0; j < _bundleElements[i].ids.length; ++j) {
                    getChild(msg.sender, bundleId, _bundleElements[i].tokenContract, _bundleElements[i].ids[j]);
                }
            }
        }

        emit AddBundleElements(bundleId, _bundleElements);
    }

    function _removeBundleElements(uint256 bundleId, BundleElementERC721[] memory _bundleElements) internal {
        _validateTransferSender(bundleId);
        require(_bundleElements.length > 0, "bundle is empty");
        for (uint256 i = 0; i < _bundleElements.length; ++i) {
            address erc721Contract = _bundleElements[i].tokenContract;
            for (uint256 j = 0; j < _bundleElements[i].ids.length; ++j) {
                uint256 tokenId = _bundleElements[i].ids[j];

                _removeChild(bundleId, erc721Contract, tokenId);
                if (_bundleElements[i].safeTransferable) {
                    IERC721(erc721Contract).safeTransferFrom(address(this), msg.sender, tokenId);
                } else {
                    _oldNFTsTransfer(msg.sender, erc721Contract, tokenId);
                }
                emit TransferChild(tokenId, msg.sender, erc721Contract, tokenId);
            }
        }

        emit RemoveBundleElements(bundleId, _bundleElements);
    }
