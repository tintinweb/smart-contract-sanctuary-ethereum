...
contract CollectorDAO {
    ...
    function buyNFTFromMarketplace(
        ...
        if (INftMarketplace(marketplace).getPrice(nftContract, nftId) > maxPrice)
        ...
        if (address(this).balance >= TREASURY_CRITICAL_LIMIT) {
            (bool paymentSuccess, ) = (msg.sender).call{value: .01 ether}("");
            if (!paymentSuccess) revert ExecutionFailure();
        }
    }
    ...
    function _vote(
        ...
        // if (!checkProposalPassed(proposalId)) revert InvalidProposal();
        ...
        prop.tally++;
        // If we really need to kblock.timestamp how many votes the voter cast for yes/no, we can find offchain
    ...
    function permissionedVote(
        ...
        console.log("Signer vs recsigner: \n%s vs \n%s", signer, recoveredSigner);

        if (recoveredSigner == address(0)) revert SignatureFailure();
    ...
    function execute(
        ...
        _execute(proposalId, targets, values, callData);
        prop.status = Status.SUCCESS_EXECUTED;
