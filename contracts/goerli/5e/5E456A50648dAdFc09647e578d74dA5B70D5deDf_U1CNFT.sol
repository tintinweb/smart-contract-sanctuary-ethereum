    ...
    ERC721URIStorage
{
    ...
    function setSellTime(uint _start, uint _end) external onlyRole(MANAGER_ROLE){
        startTime = _start;
        endTime = _end;
    }

    function mintPublic(uint256 _amount)external payable virtual{
        require(msg.value >= ticketPrice * _amount,"Your eth is not enough!");
        require(block.timestamp >= startTime && block.timestamp < endTime, "It is not sellTime");
        require(_tokenIdTracker + _amount <= 999,"NFT is already limited");
    ...
    function mint(address _send) external onlyRole(MINTER_ROLE){
        require(_tokenIdTracker + 1 <= 999,"NFT is already limited");
