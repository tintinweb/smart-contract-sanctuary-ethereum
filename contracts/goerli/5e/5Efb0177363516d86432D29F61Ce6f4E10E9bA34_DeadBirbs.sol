...
contract DeadBirbs is ERC721A, Seller, AttributeStore, ReentrancyGuard {
    ...
    */
    function allowlistMint(string calldata attributeHash, bytes calldata attributeKey, bytes calldata mintKey) external payable nonReentrant {
        verifyAllowlist(1, mintKey);
        verifyAttributeHash(attributeHash);
    ...
    */
    function publicMint(string calldata attributeHash, bytes calldata attributeKey) external payable nonReentrant {
        verifyPublicSale(1);
        verifyAttributeHash(attributeHash);
        sotreAttributeHash(attributeHash, msg.sender, totalSupply());
        _safeMint(msg.sender, 1);
...
contract AttributeStore is Ownable {
    ...
    mapping(string => hashData) public firstMinters;
    mapping(uint256 => string) public tokenData;

    function verifyAttributeHash(string calldata key) internal virtual view {
        require(!firstMinters[key].isValid, "hash has been minted already");
    }

    function sotreAttributeHash(string calldata key, address minter, uint256 tokenId) internal {
        firstMinters[key] = hashData(minter, true);
        tokenData[tokenId] = key;
    }
}
