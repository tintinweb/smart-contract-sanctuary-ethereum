    ...
    UUPSUpgradeable
{
    ...
    function isItemInDrop(uint256 dropId, uint256 itemId)
        ...
        returns (bool)
    {
        (bool exists, uint256 value) = FixedPriceSaleStorage
            .layout()
            .drops[dropId]
            .itemIds
            .tryGet(itemId);
        return
            exists &&
            (value == FixedPriceSaleStorage.layout().drops[dropId].revision);
    }
    ...
    function isSoldOut(uint256 dropId) internal view returns (bool) {
        ...
        return
            fpDrop.numMinted > 0 &&
            (fpDrop.numMinted == fpDrop.itemIds.length());
    }
    ...
    function rePublishDrop(
        ...
        if (block.timestamp < fpDrop.start) {
            ...
            emit FixedPriceDropRePublished(
                voucherId,
                dropId,
                new uint256[](0), // Should we do a different event for post start?
                price,
                fpDrop.start
            );
        }
    ...
    function _mint(
        ...
        FixedPriceDrop storage fpDrop = fp.drops[dropId];
