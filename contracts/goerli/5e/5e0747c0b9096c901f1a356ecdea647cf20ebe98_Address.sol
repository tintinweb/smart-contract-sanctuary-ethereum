...
contract KeyVerse is ERC721, Ownable {
    ...
    uint256 public ogCost = 0.00001 ether;
    ...
    mapping(string => uint256) private uriToLevel;
    ...
    string[maxSupply] tokenURIs;

    constructor()
    ERC721("KeyVerse", "KV")
    {
        for (uint256 i = 0; i < levelToUri.length; ++i) {
            uriToLevel[levelToUri[i]] = i;
        }
    }
    ...
    function upgradeLevel(uint256 _tokenId) external {
        ...
        if(level < 20) {
            _setTokenURI(_tokenId, levelToUri[level + 1]);
        }
    }

    function getLevel(uint256 _tokenId) public view returns (uint256) {
        require(_exists(_tokenId), "Nonexistent token");
        return uriToLevel[tokenURIs[_tokenId]];
    }
    ...
    function internalMint(uint256 _mintAmount) internal {
        ...
        for (uint256 i = 0; i < _mintAmount; i++) {
            ...
            _setTokenURI(currentId + i, levelToUri[0]);
            tokenIds.increment();
        }
    }
