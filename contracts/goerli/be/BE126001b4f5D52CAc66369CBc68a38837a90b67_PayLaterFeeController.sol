...
import "./interfaces/IPayLaterFeeController.sol";
...
contract PayLaterFeeController is AccessControlEnumerable, IPayLaterFeeController, Ownable {
    // initial fee is 2%
    uint256 private originationFee = 2;
    uint256 private gracePeriod = 86400;
    uint256 private rentalFee = 200;
    ...
    /** Rental grace period for payment */

    function setGracePeriod(uint256 period) external override onlyOwner {
        gracePeriod = period;
        emit UpdatedGracePeriod(gracePeriod);
    }
    ...
    function setRentalFee(uint256 fee) external override onlyOwner {
        rentalFee = fee;
        emit UpdatedRentalFee(rentalFee);
    }

    function getRentalFee() public view override returns(uint256 fee) {
        return rentalFee;
    }
...
interface IPayLaterFeeController {
    /**
     * @dev Emitted when origination fee is updated
     */
    event UpdateOriginationFee(uint256 _newFee);
    event UpdatedGracePeriod(uint256 _newFee);
    event UpdatedRentalFee(uint256 _newFee);
    ...
    function setRentalFee(uint256 fee) external;

    function getRentalFee() external view returns(uint256 fee);
