...
contract SPEHR is AdminAuthorized {
    ...
    address[] patientsAddresses;

    // Store a single object where
    // Each authorized hospital's address corresponds to the Data.DoctorData
    // eg.   0xc43d......d8   =>   Data.DoctorData{.....}
    mapping(address => Data.DoctorData) authorizedDoctors;
    address[] doctorsAddresses;
    ...
    function doctorRegistration(
        ...
        require(authorizedDoctors[doctorAddress].authorityIssueDate == 0, "Authorized doctor data already available");
        Data.DoctorData memory newAuthorizedDoctor = Data.DoctorData({
            name: name,
        ...
        authorizedDoctors[doctorAddress] = newAuthorizedDoctor;
        doctorsAddresses.push(doctorAddress);
        return true;
    }

    // Function to modify doctor data, only permitted to admin
    function modifyDoctorData(
        address doctorAddress,
        string memory name,
        string memory residentialAddress,
        string memory phone,
        ...
        require(authorizedDoctors[doctorAddress].authorityIssueDate > 0, "Doctor is not registered/authorized");
        Data.DoctorData storage targetDoctor = authorizedDoctors[doctorAddress];
        targetDoctor.name = name;
        targetDoctor.Address = residentialAddress;
        targetDoctor.phone = phone;
        targetDoctor.govtLicenseNumber = govtLicenseNumber;
        targetDoctor.authorityIssueDate = authorityIssueDate;
        targetDoctor.authorityTerminalDate = authorityTerminalDate;
        targetDoctor.speciality = speciality;
        return true;
    }
    ...
    function userRegistration(
        ...
        patientsAddresses.push(patientAddress);
        return true;
    }
    ...
    function getDoctorInfo(address doctorAddress) public view returns (Data.DoctorData memory){
        return authorizedDoctors[doctorAddress];
    }

    function getAllDoctors() public view returns (address[] memory, Data.DoctorData[] memory){
        Data.DoctorData[] memory doctors = new Data.DoctorData[](doctorsAddresses.length);

        for(uint i = 0; i < doctorsAddresses.length; i++){
            doctors[i] = authorizedDoctors[doctorsAddresses[i]];
        }
        return (doctorsAddresses, doctors);
    }

    function getAllPatients() public view returns (address[] memory, Data.PatientData[] memory){
        Data.PatientData[] memory patients = new Data.PatientData[](patientsAddresses.length);

        for(uint i = 0; i < patientsAddresses.length; i++){
            patients[i] = registeredUsers[patientsAddresses[i]];
        }
        return (patientsAddresses, patients);
    }
...
library Data{
    ...
    // mapping(address => bool) grantAccess;
