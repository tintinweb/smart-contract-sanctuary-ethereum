...
contract Paraclear is Trade, Transfer, Liquidation {
    ...
    function liquidate(address account) public {
        Liquidation._liquidate(account);
    }
}
...
contract Liquidation is Events, Synthetic {
    ...
    function _isAccountHealthy(address account) internal view returns (bool) {
        ...
        Math.Number memory liquidatedShareReq = Synthetic._getTotalMarginRequirement(
            account, Types.MarginCheckType.Maintenance
        );

        Math.Number memory freeBalance = Math.sub(tokenAssetsValue, liquidatedShareReq);
    ...
    function _liquidate(address account) internal {
        ...
        require(!_isAccountHealthy(account), "Liquidation: Account is healthy, cannot liquidate");

        // Transfer token assets & synthetic assets to insurance fund account
        _transferBalances(account);

        Events._emitAccountLiquidated(account);
    }
}
...
contract Events {
    ...
    event AccountLiquidated(address indexed account);
     ...
     *  @param account Address of the user account
     */
    function _emitAccountLiquidated(address account) internal {
...
contract Token is Storage {
    Math.Number liquidationTakePct = Math.create(75_000_000);  // Take 75%, Keep 25%
    ...
    function _emptyAssetBalance(address account, address tokenAddress) internal {
        Types.ArrIndex memory balanceIndex = Storage.tokenAssetBalanceIndex[account][tokenAddress];
        Types.TokenAssetBalance memory emptyBalance = Types.TokenAssetBalance({
            tokenAddress: tokenAddress,
            amount: Math.zero()
        });
        Storage.tokenAssetBalances[account][balanceIndex.index] = emptyBalance;
    }
    ...
    function _transferToInsuranceFund(address from) internal {
        ...
        for (uint i = 0; i < balances.length; i++) {
            if (Math.isPositive(balances[i].amount)) {
                Math.Number memory amount = Math.mul(liquidationTakePct, balances[i].amount);

                // Decrease `from` account token balance
                Math.Number memory amountToDec = Math.create(amount, false);
                _updateAssetBalance(from, balances[i].tokenAddress, amountToDec);

                // Increase insurance fund token balance
                address insuranceFundAccount = Storage.paraclearAccounts.insuranceFundAccount;
                Math.Number memory amountToInc = Math.create(amount, true);
                _updateAssetBalance(insuranceFundAccount, balances[i].tokenAddress, amountToInc);
            } else {
