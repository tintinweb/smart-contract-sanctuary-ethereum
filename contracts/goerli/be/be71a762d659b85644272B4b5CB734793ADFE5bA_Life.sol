...
contract ManageLifeInvestorsNFT is ERC721A, Ownable {
     ...
     *
     * @dev If the sender of the NFTi is not the ML wallet, this hook performs the following:
     * - Checks if the NFTi has finished the locked up period. If not, this will fail.
     * - Checks if the NFTi has a claimable $MLIFE more than 100 tokens. If yes, that needs to be auto withdrawn,
     * if not, the transfer will proceed.
    ...
    function _beforeTokenTransfers(
        ...
        if (from != owner()) {
            ...
            /// If the NFT has a pending reward greater than 100 $LIFE tokens,
            // it should be claimed first before transferring.
            if (hasClamaible) {
