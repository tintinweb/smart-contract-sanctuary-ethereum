    ...
    ReentrancyGuardUpgradeable
{
        ...
        nonReentrant
    {
        ...
        commits[commitId].amount = amount;
    ...
    function batchReveal(uint256 _commitId) internal {
        MintCommit memory c = commits[_commitId];
        require(!c.revealed, "Already revealed");
        c.revealed = true;

        require(c.seeds.length == c.amount, "Not yet seeded");

        uint16 tokenId = uint16(_totalMinted());
        uint16[] memory tokenIds = new uint16[](c.amount);
        for (uint256 index = 0; index < c.seeds.length; index++) {
            tokenId++;
            generate(tokenId, c.seeds[index]);
    ...
    function batchUserRevealPending(address recipient) external whenNotPaused nonReentrant {
        for (uint256 index = userCommits[recipient].length; index > 0; index--) {
            uint256 _commitId = userCommits[recipient][index - 1];
            MintCommit memory c = commits[_commitId];
            if (c.revealed || c.amount == 0) return; // when we reach the next revealed one, stop
            if (c.seeds.length != c.amount) return; // when we reach the next seeded one, stop
            batchReveal(_commitId);
        }
    }

    function getPendingUserCommits(address recipient) external view returns (MintCommit[] memory) {
        MintCommit[] memory result = new MintCommit[](userCommits[recipient].length);
        for (uint256 index = userCommits[recipient].length; index > 0; index--) {
            uint256 _commitId = userCommits[recipient][index - 1];
            MintCommit memory c = commits[_commitId];
            if (c.revealed || c.amount == 0) continue; // when we reach the next revealed one, stop
            if (c.seeds.length != c.amount) continue; // when we reach the next seeded one, stop
            result[index - 1] = c;
        }
        return result;
    }

    function revealStuck(address recipient, uint256 userIndex) external onlyOwner {
        uint256 _commitId = userCommits[recipient][userIndex];
