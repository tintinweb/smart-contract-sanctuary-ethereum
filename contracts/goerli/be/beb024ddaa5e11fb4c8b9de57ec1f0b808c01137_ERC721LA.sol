 ...
 */
contract ERC721LA is PublicMintable, AirDropable, Burnable, Whitelistable {
...
import "./PayableMint.sol";

abstract contract PublicMintable is IPublicMintable, PayableMint {
        ...
        whenNotPaused
    {
        ...
        _payableMint(editionId, quantity, mintPriceInWei);
...
import "./PayableMint.sol";
import "./LimitedAmountMint.sol";
import "./WhitelistableState.sol";
import "./LimitedAmountMintState.sol";

abstract contract Whitelistable is IWhitelistable, PayableMint {
    /**
    ...
    function setWLConfig(
        ...
        if (!isEditionCreator(editionId)) {
            revert CustomErrors.NotAllowed();
        }

        WhitelistableState.State storage state = WhitelistableState
     ...
     * @param editionId Edition ID of the WL to be updated
     * @param amount Amount of the WL to be updated
    ...
    function updateWLConfig(
        ...
        if (amount != newAmount || mintPriceInFinney != newMintPriceInFinney) {
            ...
            wlId = uint256(
                keccak256(
                    abi.encodePacked(editionId, newAmount, newMintPriceInFinney)
                )
            );
            state._whitelistConfig[wlId] = whitelistConfig;
        }
    ...
    function whitelistMint(
        ...
        LimitedAmountMintState.State
            storage mintCountState = LimitedAmountMintState
                ._getLimitedAmountMintState();
        ...
        uint256 mintCountKey = uint256(
            keccak256(abi.encodePacked(editionId, msg.sender))
        );
        if (
            mintCountState._mintedPerWallet[mintCountKey] + quantity >
            whitelistConfig.amount
        ) {
            revert CustomErrors.MaximumMinAmountReached();
        }

        if (block.timestamp < whitelistConfig.mintStartTS) {
        ...
        mintCountState._mintedPerWallet[mintCountKey] += quantity;
        _payableMint(editionId, uint24(quantity), mintPriceInWei);
    }
    ...
    LAInitializable, AccessControl, Pausable, IERC721LA
{
    ...
    /// @dev Returns whether `caller` is the creator of the edition 
    function isEditionCreator(
        uint256 editionId
    ) public view returns (bool) {
        ERC721State.Edition memory edition = getEdition(editionId);
        bool isCreator = edition.createdBy == msg.sender;
        return isCreator;
    }
...
abstract contract PayableMint is ERC721Mintable {

    function _payableMint(uint256 editionId, uint24 quantity, uint256 unitMintPrice) internal {
        ERC721State.ERC721LAState storage state = ERC721State
            ._getERC721LAState();
        
        // Check if sufficiant
        if (msg.value < unitMintPrice * quantity) {
...
import "../libraries/LANFTUtils.sol";
import "../tokens/ERC721State.sol";
import "../extensions/LimitedAmountMintState.sol";

abstract contract LimitedAmountMint is ERC721Mintable {
    /**
     * @dev Internal batch minting function
     */
    function _safeMint(
        ...
        uint24 _quantity,
        address _recipient
    ) virtual override(ERC721Mintable) internal returns (uint256 firstTokenId) {
        LimitedAmountMintState.State storage state = LimitedAmountMintState
            ._getLimitedAmountMintState();

        uint256 key = uint256(keccak256(abi.encodePacked(_editionId, _recipient)));
        if (state._mintedPerWallet[key] + _quantity > state._maxMintPerWallet[_editionId]) {
            revert CustomErrors.MaximumMinAmountReached();
        }

        state._mintedPerWallet[key] += _quantity;
        return ERC721Mintable._safeMint(_editionId, _quantity,_recipient);
    }
}
...
library LimitedAmountMintState {
    struct State {
        // Minted counter per wallet + edition (hashes)
        mapping(uint256 => uint256) _mintedPerWallet;

        // Edition ID => Max per wallet/edition
        mapping(uint256 => uint256) _maxMintPerWallet;
    }
     ...
     * and give more flexibility when creating extensions
     */
    function _getLimitedAmountMintState()
        internal
        pure
        returns (State storage state)
    {
        bytes32 storageSlot = keccak256("liveart.LimitedAmountMintState");
        assembly {
            state.slot := storageSlot
        }
    }
