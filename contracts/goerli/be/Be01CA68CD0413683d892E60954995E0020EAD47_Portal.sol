...
contract Portal is OwnableUpgradeable {
    ...
    function _transferFund(address from, address to, uint256 amount, uint256 mtxBurnRate) internal {
        ...
        if (mtxBurnRate > 0) {
            if(from == address(this)) {
                mtx.transfer(to, mtxUsage(amount, mtxBurnRate));
            } else {
                mtx.transferFrom(from, to, mtxUsage(amount, mtxBurnRate));
            }
        }   
    }

    function mtxUsage(uint256 usdcAmount, uint256 mtxBurnRate) public pure returns(uint256) {
        // The decimal of MTX is much larger than USDC (18 vs 6), so we don't need denominator here.
        return usdcAmount * mtxBurnRate;
    }

    function _checkTime(uint64 startTime, uint64 endTime) internal view {
        require(block.timestamp > startTime, "Portal: Investment hasn't started." );
    ...
    function addInvestment(
        ...
        require(investAmountAvailable > 0);
        Investment storage _investment = investment[investmentId];

        require(_investment.pool1StartTime == 0, "Portal: Investment has been deployed."); // Cannot override existing investment

        _investment.mtxBurnRate = mtxBurnRate;
        _investment.investAmountAvailable = investAmountAvailable;
    ...
    function investPoolOne(uint256 investmentId, uint256 amount, uint256 allowedAmount, 
        bytes calldata signature) public {
        
        Investment storage _investment = investment[investmentId];

        _checkTime(
            _investment.pool1StartTime,
            _investment.pool1EndTime
        );
        
        bytes32 hash = keccak256( abi.encodePacked(
            address(this), investmentId, msg.sender, allowedAmount
        ));
        hash = ECDSAUpgradeable.toEthSignedMessageHash(hash);

        require(ECDSAUpgradeable.recover(hash, signature) == signer, "Portal: Invalid Signature");

        require(amount + poolOneInvested[investmentId][msg.sender] <= allowedAmount, "Portal: Invalid Amount");
        poolOneInvested[investmentId][msg.sender] += amount;
        _investment.investAmountAvailable -= amount;

        _transferFund(msg.sender, treasury, amount, _investment.mtxBurnRate);
    } 

    function investPoolTwo(uint256 investmentId, uint256 amount, bytes calldata signature) public {
        Investment storage _investment = investment[investmentId];

        _checkTime(
            _investment.pool2StartTime,
            _investment.pool2EndTime
        );


        bytes32 hash = keccak256( abi.encodePacked(
            address(this), investmentId, msg.sender
        ));
        hash = ECDSAUpgradeable.toEthSignedMessageHash(hash);

        require(ECDSAUpgradeable.recover(hash, signature) == signer, "Portal: Invalid Signature");

        require(amount <= _investment.investAmountAvailable, "Portal: Cannot invest more than the cap.");

        poolTwoInvested[investmentId][msg.sender] += amount;
        _investment.pool2TotalCommited += amount;

        _transferFund(msg.sender, address(this), amount, _investment.mtxBurnRate);
    } 

    function settlePoolTwo(uint256 investmentId) external onlyOwner {
        Investment storage _investment = investment[investmentId];
        
        require(block.timestamp > _investment.pool2EndTime, "Portal: Investment hasn't ended.");
        require(_investment.pool2Transfered == 0, "Portal: Investment has been settled.");

        uint256 toTransfer;

        if(_investment.pool2TotalCommited > _investment.investAmountAvailable) {
            toTransfer = _investment.investAmountAvailable;
        } else {
            toTransfer = _investment.pool2TotalCommited;
        }
        _investment.pool2Transfered = toTransfer;
        delete _investment.investAmountAvailable;

        _transferFund(address(this), treasury, toTransfer, _investment.mtxBurnRate);

    }

    /// @dev Amount of USDC refund for a user from a invetment opportunity
    function refundableAmount(address user, uint256 investmentId) public view returns (uint256) {
        Investment storage _investment = investment[investmentId];
        ...
        if (_investment.pool2TotalCommited > 0) {
            uint256 remaining = _investment.pool2TotalCommited - _investment.pool2Transfered;
            return poolTwoInvested[investmentId][user] * remaining / _investment.pool2TotalCommited;
        } else {
            return 0;
        }
    }
    ...
    function refundPoolTwo(uint256 investmentId) external {
        ...
        _transferFund(address(this), msg.sender, toRefund, _investment.mtxBurnRate);
    }

    /// @dev Amount of USDC an user actually invested. This number excludes the USDC refunded to the user.
    function actualInvested(address user, uint256 investmentId) public view returns (uint256 invested) {
        Investment storage _investment = investment[investmentId];

        invested = poolOneInvested[investmentId][user];

        if( _investment.pool2TotalCommited > 0) {
            invested += poolTwoInvested[investmentId][user] * 
                _investment.pool2Transfered  / _investment.pool2TotalCommited;
        }
    }

    function extendPoolOne(uint256 investmentId, uint64 time) external onlyOwner{
    ...
    function extendPoolTwo(uint256 investmentId, uint64 time) external onlyOwner{
        require(time <= 1 weeks); // Prevent accidently extending the investment period too long.
        Investment storage _investment = investment[investmentId];
