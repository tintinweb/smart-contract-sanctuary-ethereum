...
contract Lottery is VRFConsumerBaseV2 {
    ...
    function enterLottery() public payable {
        ...
        for (uint i = 0; i < l_players.length; i++) {
    ...
    function requestRandomWinner() external {
        ...
        LINKTOKEN.transferAndCall(address(i_vrfCoordinator), getLatestPrice(), abi.encode(i_subscriptionID));
    ...
    function getLatestPrice() public view returns (uint256) {
        ...
        //1.897503080414923500

        return ((30 * 250000 * 10000) / (1000000000));
    }
    ...
    function getSub(uint64 subId) external view
    returns (uint96 balance,
      uint64 reqCount,
      address owner,
      address[] memory consumers) {
            return  i_vrfCoordinator.getSubscription(subId);
    }
