...
contract CrowdFunding {
    ...
    function AdharVerification(uint256 num)public returns(uint)
    {
        if(Adhar[msg.sender] != 0)
        {
            return 0;
        }
        else{
    ...
    function donateToCampaign(uint256 _id) public payable {
        require((campaigns[_id].target-campaigns[_id].amountCollected) >= msg.value,"check the target value");
        require(campaigns[_id].amountCollected <= campaigns[_id].target,"taget achived");
