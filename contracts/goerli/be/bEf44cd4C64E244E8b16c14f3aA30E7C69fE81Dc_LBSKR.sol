...
abstract contract BaseBSKR is IERC20, Ownable, Manageable, Pausable {
    ...
    function approveContract(
        ...
        _approve(contractA, spenderA, amount);
    }
...
contract Stakable is Ownable {
    ...
    // /**
    //  * @notice
    //  * hasStake is used to check if an account has stakes and the total amount along with all the separate stakes
    //  * 
    //  */
...
contract LBSKR is BaseBSKR, DSMath, Stakable {
        ...
        whenNotPaused
    {
        ...
        if (currStake.sinceBlock + 52 weeks > block.timestamp) {
            ...
            if (fourWeeksElapsed < 13) {
                penaltyFactor -= ((13 - fourWeeksElapsed) * 100); // If one four-weeks have elapsed - penalty is 12% or 1200/10000
            }
        }
        ...
        if (bskrToSend > 0) {
            require(
                _BSKR.stakeTransfer(_inflationAddress, _msgSender(), bskrToSend),
    ...
    // TODO function penaltyFor(uint256 fromTimestamp, uint256 toTimestamp) {}
    // TODO function penaltyFor(userIndex/address, amount, stakeindex)
