...
contract UniFeeSwap {
    ...
    mapping(uint24 => int24) internal feeAmountTickSpacing;
    ...
    function feeSwap(uint256 tokenId, uint24 newFee) public {
        ...
        // mint new fee tier position
        _approveToken(token0, address(nonfungiblePositionManager), amount0);
        _approveToken(token1, address(nonfungiblePositionManager), amount1);

        (uint256 amount0Used, uint256 amount1Used) = _addLiquidity(
            token0,
            token1,
            tickLower,
            tickUpper,
    ...
    function _removeLiquidity(uint256 tokenId, uint128 liquidity)
        internal
    ...
    {
        int24 tickSpacing = feeAmountTickSpacing[newFee];
     ...
     * @param amount The amount of token to actually spend
     */
