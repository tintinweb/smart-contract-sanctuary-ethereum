...
// File: contracts/JustRightNFT.sol
...
contract JustRightNFT is ERC721AQueryable, Pausable, Ownable, PaymentSplitter {
    ...
    string public constant TOKEN_NAME = "JustRight";
    string public constant TOKEN_SYMBOL = "JRT";

    address[] public payees = [0x0d0F15B7FF1F02EDBAF333A0176440cF73A887F0]; // TODO: TBD
    uint256[] public payeesShares = [100]; // TODO: TBD

    string public JUSTRIGHT_PROVENANCE;
    ...
    event RevealedSet(bool _revealed);
    ...
    function setProvenanceHash(string calldata _provenanceHash)
        ...
        onlyOwner
    {
        JUSTRIGHT_PROVENANCE = _provenanceHash;
    ...
    modifier isPhaseEnabled(uint256 _phase) {
        require(
            phaseStorage[MintType(_phase)].enabled,
            "Sale phase is not active"
        );
        _;
    }
    ...
    modifier validateLimitPerAddress(uint256 _phase, uint256 _numberOfTokens) {
        MintPhase storage phase = phaseStorage[MintType(_phase)];
        uint256 limitPerAddress = phase.limitPerAddress[msg.sender];
        uint256 mintedTokens = phase.mintsPerAddress[msg.sender];
        require(
            (limitPerAddress == 0 && phase.defaultLimitPerAddress == 0) ||
                (limitPerAddress > 0 &&
                    mintedTokens + _numberOfTokens <= limitPerAddress) ||
                mintedTokens + _numberOfTokens <= phase.defaultLimitPerAddress,
    ...
    modifier validateEthPayment(uint256 _phase, uint256 _numberOfTokens) {
        require(
            phaseStorage[MintType(_phase)].price * _numberOfTokens == msg.value,
    ...
    function _executeCommonValidations(uint256 _phase, uint256 _numberOfTokens)
        internal
        whenNotPaused
        isPhaseEnabled(_phase)
        validateLimitPerTransaction(_phase, _numberOfTokens)
        validateLimitPerAddress(_phase, _numberOfTokens)
        ensureAvailabilityFor(_numberOfTokens)
        validateEthPayment(_phase, _numberOfTokens)
    {}
