...
contract ControllorFacet {
    ...
    function controllorCreateSignetor() external {
        Signetor st;
        if (getOwnerNumContractOfSignetor(msg.sender) != 0) revert Contract__Created();
        st = new Signetor("Signetor", "ST", msg.sender, address(this));
        address newContractAddress = st.SignetorContractAddress();
        LibSignetStorage.createSignetor(newContractAddress, msg.sender);
        LibSignetStorage.controllorCreateSignetor(msg.sender, newContractAddress);
        emit CollectionCreated(msg.sender, newContractAddress);
    }
    ...
    function sendMessage(
        ...
        if (addr != getOwnerContractForSignetor(msg.sender)) revert Wrong__Contract();
        uint256 messageId = Signetor(addr).sendMessage(tokenURI_);
        uint256 signetId = LibSignetStorage.messageSender(msg.sender);
        emit NewMessageSent(msg.sender, addr, messageId, signetId, tokenURI_, block.timestamp);
        return true;
    }

    function getOwnerContractForSignetor(address contractOwner) public view returns (address) {
        (, address b, ) = LibSignetStorage.getContractDetails(0, contractOwner);
        return b;
    }

    function getOwnerNumContractOfSignetor(address contractOwner) public view returns (uint256) {
        return (LibSignetStorage.getOwnerNumContractOfSignetor(contractOwner));
    }
