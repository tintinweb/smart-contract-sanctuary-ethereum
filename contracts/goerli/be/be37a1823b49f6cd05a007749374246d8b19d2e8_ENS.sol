...
contract ENS {
    ...
    function setAddr(bytes32 node, address add) external{
        records[node].owner=add;
        emit AddrChanged(node, add);
    }
    ...
    function setName(bytes32 node, string calldata nm) external{
        records[node].name=nm;
        emit NameChanged(node, nm);
    }
    ...
    function setContenthash(bytes32 node, bytes calldata hash) external{
        records[node].hash=bytes(hash);
    }

    function text(bytes32 node) external view returns (string memory){
        return records[node].name;
    }
