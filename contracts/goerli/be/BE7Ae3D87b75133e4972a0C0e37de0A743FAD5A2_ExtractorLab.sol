...
contract ExtractorLab is IERC721Receiver, IERC1155Receiver, Ownable, ReentrancyGuard, Initializable {
	...
	function extractDna(uint256 _mutantId, uint16 boostId, uint256 _fee) public nonReentrant {
		...
		contracts.Scales.deposit(extractionCost + _fee);

		contracts.DNA.runExtraction(_mutantId, boostId);
		contracts.DNA.completeExtraction(_mutantId); // event emitted: mutantId, batchId, boostId, results (tokenId, criticality, success)
		lastExtractor = msg.sender;
	}

	function transferDna() public {
		uint256 dnaId = getExtractedDnaId();
		if (lastExtractor != msg.sender) revert ExtractorLab__SenderNotTokenOwner();
		lastExtractor = address(0);
		extractedDnaId = DEFAULT_DNA_ID;
		contracts.DNA.safeTransferFrom(address(this), msg.sender, dnaId, 1, "");
	}
	...
	/**
	 * @notice returns true value for extractedDnaId. DEFAULT_DNA_ID is set to 0 for the cheaper SSTORE when
	 * resetting variable after transfer but causes overlap with DNA id: 0
	 */
	function getExtractedDnaId() public view returns (uint256) {
		return extractedDnaId - 1;
	}

	function withdraw(uint256 scales, uint256 rwaste) public onlyMutantOwner {
		contracts.Scales.transfer(msg.sender, scales);
		contracts.RWaste.transfer(msg.sender, rwaste);
	}
	...
	function onERC1155Received(
		...
		if (msg.sender == address(contracts.DNA)) {
			extractedDnaId = id + 1; // increment to prevent overlap with dna id: 0 when resetting extractedDnaId to DEFAULT_DNA_ID
		} else {
