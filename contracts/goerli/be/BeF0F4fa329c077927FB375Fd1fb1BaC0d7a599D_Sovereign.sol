...
contract Sovereign is ERC721PausableUpgradeable, AccessControlUpgradeable {
    ...
    function initialize(InitArgs calldata _init) public initializer {
        ...
        mintingPaused = _init.mintingPaused;
        maxSupply = _init.maxSupply;
        maxPerTxn = _init.maxPerTxn;
    ...
    // if this works, update this SO post: https://ethereum.stackexchange.com/questions/142245/how-to-check-the-balance-of-a-wallet-within-a-contract-and-assign-balance-to-a-v    
    modifier maxWalletCheck(address _recipient, uint256 _quantity) {
        require(balanceOf(_recipient) + _quantity <= maxPerWallet, "Exceeds max wallet count");
        _;
    }

    modifier maxPerTxnCheck(uint256 _quantity) {
        require(_quantity <= maxPerTxn, "Exceeds max NFTs per transaction");
        _;
    }
    ...
    modifier isCrossmint(address _sender) {
        require(_sender == crossmintWallet, "Crossmint only function");
        _;
    }
    ...
    function mint(uint256 _quantity) 
        ...
        maxPerTxnCheck(_quantity)
        maxWalletCheck(msg.sender, _quantity)
    ...
    function crossmint(address _recipient, uint256 _quantity) 
        ...
        isCorrectPayment(_quantity)
        maxPerTxnCheck(_quantity)
        maxWalletCheck(_recipient, _quantity)
        isAvailable(_quantity)
    ...
    function setMintingPaused(bool _paused) external onlyRole(ADMIN_ROLE) {
        mintingPaused = _paused;
    }

    function setMaxPerTx(uint256 _maxPerTxn) external onlyRole(ADMIN_ROLE) {
    ...
    function setCrossmintWallet(address _wallet) external onlyRole(ADMIN_ROLE) {
