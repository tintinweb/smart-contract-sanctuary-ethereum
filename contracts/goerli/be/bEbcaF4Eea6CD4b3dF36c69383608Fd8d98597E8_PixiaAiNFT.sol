...
contract PixiaAiNFT is ERC721A, Initializable {
    using Strings for uint256;
    using Address for address payable;
    
    string private BASE_URI = "https://dev.pixia.ai/api/token/";
    ...
    mapping(uint256 => uint256) public MAP_REQUEST;//Token Id -> Art Id

    constructor() ERC721A("PixiaAi", "PIXIA") {
        factory = msg.sender;
    }

    /**
		Initialize from Factory contract
	 */
    function initialize() external initializer {
        factory = msg.sender;
    }
    function updateOwner( address creator) external {
    ...
    function mintPack(uint256[] memory ids) external payable {
        uint256 packAmount = ids.length;
        require(msg.value >= _PRICE * packAmount, "Amount is not sufficient");

        payable(royaltyReceiver).transfer(msg.value);
        for (uint256 i = _currentIndex; i < _currentIndex + packAmount; i++){
            MAP_REQUEST[i] = ids[i - _currentIndex];
        }
    ...
    function TEST_BASE_URI(uint256 tokenId) external view returns (string memory, uint256, string memory ){
        return (BASE_URI, bytes(BASE_URI).length, string(abi.encodePacked(BASE_URI, address(this), "/", tokenId.toString())));
    }
