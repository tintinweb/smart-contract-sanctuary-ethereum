...
import {QRNGStorage, REQUEST_NUMBER_ROLE} from "@contracts/ponzu/types/qrng/QRNGStorage.sol";
...
library LibPonzu {
  ...
  function selectWinner(PonzuStorage storage ps, uint256 number, uint256 roundNum) internal {
    ...
    {
      (address winnerAddress, uint256 winnerNumber2) = ps._selectWinner(roundNum, winnerNumber);
  ...
  function _selectWinner(
    ...
    uint256 roundNum,
    uint256 number
  ) internal returns (address winnerAddress, uint256 winnerNumber) {
    winnerNumber = number;
    // iterate over participants to find winner
    for (uint256 i = 0; i < ps.participantsList.length; ++i) {
      ps.participantDepositsCleanup(ps.participantsList[i], roundNum);
  ...
  function currentPrizePool(PonzuStorage storage ps) internal view returns (uint256) {
    (, , , uint256 withdrawableRewards, , ) = IHamachi(ps.rewardToken).getRewardAccount(
      address(this)
    );
