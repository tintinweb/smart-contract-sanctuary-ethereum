...
library ArrayManager {
    function addId(uint256[] storage ids_, uint256 addId_) internal {
        ids_.push(addId_);
    }

    function removeId(uint256[] storage ids_, uint256 popId_) internal {
        ...
        for (uint256 i = uint256(index); i < length - 1; i ++) {
            ids_[i] = ids_[i + 1];
        }
...
import "./libraries/ArrayManager.sol";
import "hardhat/console.sol";
