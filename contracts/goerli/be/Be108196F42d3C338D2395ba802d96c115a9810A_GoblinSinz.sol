    ...
    Signature
{
    ...
    SaleState public saleState = SaleState.PUBLIC;
    mapping(bytes => bool) public usedSignatures;
    mapping(string => bool) public usedNounces;

    constructor(bool isPaying, uint256 deploymentPrice) ERC721A("Goblin Sinz", "SINZ") payable {
        if(isPaying){
    ...
    /** MINTING **/
    function mint(uint64 count, bytes calldata signature, string calldata tokens) 
        external 
        payable 
        requiresSignature(signature,tokens) 
    {
    ...
    /** SIGNATURE **/
    function checkSignature(bytes calldata signature, string calldata tokens)
        public
        view
        requiresSignature(signature,tokens)
        returns (bool)
    ...
    /** ADMIN FUNCTIONS **/
    /**
     * @dev Sets sale state to CLOSED (0), PUBLIC (1), PRESALE (2) if applicable.
     */
...
contract Signature is Ownable {
    ...
    bytes32 private immutable NAME = keccak256("Goblinsinz");
    bytes32 private immutable NUMBER = keccak256("1");

    bytes32 private immutable MINTER_TYPEHASH =
        keccak256("Minter(address wallet,string tokens)");
