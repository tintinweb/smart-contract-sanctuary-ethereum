...
contract Phase1 is ERC721, ERC721Burnable,DefaultOperatorFilterer, ERC721Enumerable, Ownable{
    ...
    address public phase0Address;
    string private URI;
    uint256 public revelTime;
    mapping(address => uint256[]) public userNftId;
    mapping(uint256 => bool) internal alreadyClaimed;
    
    event RevelTime(uint256 revelTime);

    constructor(string memory _name, string memory _symbol, address _phase0Address) ERC721(_name,_symbol) {
        phase0Address = _phase0Address;
        URI = "https://metadata.link/";
    ...
    */
    function claim(uint256 phase0NFTId) external {
        require(toggle == true, "Phase1: Toggle is off");
    	require(revelTime !=0, "Phase1: Set revel Timer");
        require(block.timestamp > revelTime, "Phase1: Redeem has not started yet");
        require(IERC721(phase0Address).ownerOf(phase0NFTId) == msg.sender, "Phase1: You are not the owner of this NFT");
        require(alreadyClaimed[phase0NFTId] == false, "Phase1: NFT already claimed for this token");
        require(_randomNumbers.length != 0, "Phase1: All NFT has been already minted");
        uint256 randomId = randomTokenId();
        _safeMint(msg.sender, randomId);
        userNftId[msg.sender].push(randomId);
        alreadyClaimed[phase0NFTId] = true;
    }
