...
contract QuasiStaking is ReentrancyGuard, Pausable, UUPSUpgradeable, IQuasiStaking {
    ...
    function initialize(string memory _name, address _operator, address _rewardsToken, uint256 _baseTotal, address[] memory _essentialNFT, address[] memory _allianceNFT) external {
        ...
        for(uint256 i = 0; i < _essentialNFT.length; i++) {
          essentialNFT.push(IERC721(_essentialNFT[i]));
        }
    ...
    function register(address nft, uint256 id) external notPaused ownEssentialNFT(msg.sender) override {
      _register(nft, id, msg.sender);
    }

    function batchRegister(address[] memory nfts, uint256[] memory ids) external notPaused ownEssentialNFT(msg.sender) override {
      require((nfts.length > 0) && (ids.length > 0), "Length should be greater than 0");
      require(nfts.length == ids.length, "Length needs to be the same");
      for (uint256 i = 0; i < nfts.length; i++) {
    ...
    function getRewards(address nft, uint256 id) external nonReentrant ownEssentialNFT(msg.sender) override {
      uint256 reward = _updateRewards(nft, id, msg.sender);

      rewardsToken.safeTransfer(msg.sender, reward);
    }

    function batchGetRewards(address[] memory nfts, uint256[] memory ids) external notPaused ownEssentialNFT(msg.sender) override {
      require((nfts.length > 0) && (ids.length > 0), "Length should be greater than 0");
      require(nfts.length == ids.length, "Length needs to be the same");
      uint256 rewards;
      for (uint256 i = 0; i < nfts.length; i++) {
    ...
    modifier ownEssentialNFT(address account) {
      bool isPass = false;
      for (uint256 i = 0; i < essentialNFT.length; i++) {
        if (essentialNFT[i].balanceOf(account) > 0) {
          isPass = true;
          break;
        }
      }
      require(isPass == true, "You should own the essential NFT");
      _;
    }
