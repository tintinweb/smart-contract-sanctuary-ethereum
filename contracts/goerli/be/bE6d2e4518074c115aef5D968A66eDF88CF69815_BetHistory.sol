...
contract BetHistory is IBetHistory, AccessHandler {
    ...
    // TODO: NOTE: Bets with identical details will get identical hashes, be additive and have a combined pot.
    //       This is probably fine for the contract alone, but not if the backend needs to address the bets individually.

    // bet hash => Bet (struct)
    mapping(bytes32 => BetData.Bet) private allBets; // TODO: Make it public in case TC needs it?
    // bet hash => pot size
    mapping(bytes32 => uint256) private unsettledPots; // TODO: Make it public in case TC needs it?
 ...
 */
     ...
     * @return pot is the combined potsize between better and LP.
    ...
    function createBet(BetData.Bet calldata bet)
        ...
        returns (uint256 pot, bytes32 betHash)
    {
        // TODO: Should we check if that hash/bet already exist?
        betHash = BetData.getBetHash(bet);

        // Lock the betters and the LPs tokens
        betLockBox.lockAmount(bet.owner, bet.token, bet.amount);
        //Transfer LPs tokens to the lockbox too.
        pot = bet.amount.mul(bet.decimalOdds).div(BetData.ODDS_PRECISION);
        uint256 matchedAmount = pot.sub(bet.amount);
        if (matchedAmount == 0)
            revert InvalidBet({bet: bet});

        matchBet(bet.token, matchedAmount);
     ...
     * @return better is the owner of the bet.
        ...
        )
    {
        ...
        console.log("potSize:", potSize.div(1 ether), "amount:", bet.amount.div(1 ether));
        uint256 lockedAmount = betLockBox.getLockedAmount(better, token);
        console.log("locked:", lockedAmount.div(1 ether));
        if (lockedAmount == 0 || potSize == 0) {
            return (better, token, 0, 0);
        }

        console.log("Unlock for better ....", bet.amount.div(1 ether));
        ...
        betLockBox.unlockAmountTo(
            ...
            token,
            bet.amount);
        console.log("Unlock for LP Box ....", matchedAmount.div(1 ether));
        betLockBox.unlockAmountTo(
            address(lpLockBox),
            address(tokenTransferProxy),
            token,
        ...
        console.log("pay to better:", paidToBetter.div(1 ether));
        console.log("pay to LP:", paidToLP.div(1 ether));
    }
