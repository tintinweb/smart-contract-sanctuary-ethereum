...
abstract contract BaseBridge is NonblockingLzApp {
    using EnumerableSet for EnumerableSet.Uint16Set;

    EnumerableSet.Uint16Set internal _supportedChains;

    // address internal _zroPaymentAddress; // _zroPaymentAddress - the address of the ZRO token holder who would pay for the transaction
    ...
    // function setZroPaymentAddress(address _paymentAddress)
}
...
contract SynthrBridge is BaseBridge {
    ...
    // aggregators
    address public synthrAggregator;
    ...
    function selfLZChainId() external view returns (uint16) {
        return _selfLZChainId;
    }
    ...
    function _sendMint(
        ...
        ISynthrAggregator(synthrAggregator).mintSynth(_accountForCollateral, _collateralKey, _collateralAmount, _synthKey, _synthAmount, _debtShare);
    ...
    function _sendWithdraw(
        ...
        ISynthrAggregator(synthrAggregator).withdrawCollateral(account, amount, collateralKey);
    }
    ...
    function _sendBurn(
        ...
        ISynthrAggregator(synthrAggregator).burnSynth(accountForSynth, synthKey, synthAmount, debtShare);
    }
    ...
    function _sendTransferFrom(
        ...
        ISynthrAggregator(synthrAggregator).synthTransferFrom(currencyKey, from, to, amount);
    }
    ...
    function sendExchange(ExchangeArgs calldata args) external payable onlySynthr {
        _sendExchange(
            args.fromAccount,
    ...
    function _sendExchange(
        ...
        ISynthrAggregator(synthrAggregator).exchangeSynth(sourceAccount, sourceKey, sourceAmount, destAccount, destKey, destAmount, fee);
    }
    ...
    function _sendLiquidate(
        ...
        ISynthrAggregator(synthrAggregator).liquidate(account, collateralKey, collateralAmount, debtShare, notifyAmount);
    }

    function sendREAppend(bytes32 currencyKey, address account, uint amount) external payable {
        _sendREAppend(currencyKey, account, amount);
    ...
    function _sendREAppend(
        ...
        ISynthrAggregator(synthrAggregator).append(currencyKey, account, amount);
    }
    ...
    function _sendVest(
        ...
        ISynthrAggregator(synthrAggregator).vest(currencyKey, account, amount);
    }
