...
contract ERC20 {
    ...
    //   buy, sell  -> interact with contract token balance
    //   transfer  ->  Transfer from one user to another. 
    //   transferToOwner -> Claim any ETH in contract
    //   track owner list
    //   
...
contract UChicagoFinMathInherited is ERC20 {
    ...
    address[] public hodlers2;
    constructor(address payable wallet, address payable token_)  public 
    {
        tokens_remaining = 10000000;
        total_tokens = tokens_remaining;
        owner = wallet;
    ...
    function addOwner(address OwnerToAdd) public payable onlyOwner {
        hodlers.push(OwnerToAdd);
    }

    function deleteOwner(address OwnerToDelete) public payable onlyOwner {
        
        for (uint i = 0; i < hodlerCount; i++){
            if(hodlers[i] != OwnerToDelete)
                hodlers2.push(hodlers[i]);
        }
        hodlers = hodlers2;
    ...
    function buyFromContractB() public payable {
        ERC20 tokenClass = ERC20(token);
        tokenClass.buyTokenB{value: msg.value}();
