...
contract Marketplace is ReentrancyGuard {
        ...
        hasEnoughFunds(nftAddress, tokenId, msg.value)
    {
        ...
        s_sellerProceeds[siteOwner] =
            s_sellerProceeds[siteOwner] +
            calculateComission(msg.value, OWNER_COMMISSION);

        s_sellerProceeds[author] =
            s_sellerProceeds[author] +
            calculateComission(msg.value, AUTHOR_COMMISSION);

        s_sellerProceeds[item.seller] =
            s_sellerProceeds[item.seller] +
            calculateComission(msg.value, STUDENT_COMMISSION);
    ...
    /**
     * @dev Calculates the comission based on a given value and percantage.
     * We have to calculate with basis points, since solidity doesn't support floating point numbers.
     */
    function calculateComission(uint256 value, uint256 percentage) private pure returns (uint256) {
