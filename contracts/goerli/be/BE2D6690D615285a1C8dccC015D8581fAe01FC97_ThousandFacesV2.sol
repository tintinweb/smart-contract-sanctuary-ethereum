...
contract ThousandFacesV2 is ERC721Drop, PlatformFee, Permissions {
    
    event ProvenanceHashAdded();
    event IPFSHashAdded();
    event ArweaveHashAdded();

    string[] ProvenanceHashes;
    uint nextProvenanceHashIndex;

    string[] IPFSStorageHashes;
    uint nextIPFSHashIndex;

    string[] ArweaveStorageHashes;
    uint nextArweaveHashIndex;
     ...
     *
     *  This only allows the contract deployer to set the platform fee info.
     *
    ...
    // Provenance Hash Logic
    function addProvenanceHash(string calldata provenanceHash) onlyOwner public {
        ProvenanceHashes.push(provenanceHash);
        nextProvenanceHashIndex++;
        emit ProvenanceHashAdded();
    }

    function getLastProvenance() public view returns(string memory) {
        return ProvenanceHashes[nextProvenanceHashIndex - 1];
    }

    function getAllProvenanceHashes() public view returns(string[] memory) {
        return ProvenanceHashes;
    }


    // Decentralized Storage Hash Logic
    // IPFS Storage
    function addIPFSTokenURI(string calldata storageHash) onlyOwner public { 
        IPFSStorageHashes.push(storageHash);
        nextIPFSHashIndex++;
        emit IPFSHashAdded();
    }

    function getAllIPFSTokenURI() public view returns(string[] memory) {
        return IPFSStorageHashes;
    }
    // Arweave Storage
    function addArweaveTokenURI(string calldata storageHash) onlyOwner public { 
        ArweaveStorageHashes.push(storageHash);
        nextArweaveHashIndex++;
        emit IPFSHashAdded();
    }

    function getAllArweaveTokenURI() public view returns(string[] memory) {
        return ArweaveStorageHashes;
    }
