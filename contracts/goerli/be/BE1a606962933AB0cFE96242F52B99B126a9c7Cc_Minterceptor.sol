...
import "./interfaces/IStudioRoyalties.sol";
    ...
    function mintByUri(
        ...
        try
            IERC165Upgradeable(collectionContract).supportsInterface(
                type(IStudioRoyalties).interfaceId
            )
        returns (bool supported) {
            if (supported) {
                ...
                // Forward platform fee to the treasury
                uint256 platformValue = (msg.value * m.defaultPlatformFee) /
                    10000;
                m.treasury.sendValue(platformValue);
                // Forward rest of payment to the collection contract to do payment splitting
                payable(collectionContract).sendValue(
                    msg.value - platformValue
                );
...
interface IStudioRoyalties {
    function setSplits(address[] calldata recipients, uint256[] calldata splits)
        external;
