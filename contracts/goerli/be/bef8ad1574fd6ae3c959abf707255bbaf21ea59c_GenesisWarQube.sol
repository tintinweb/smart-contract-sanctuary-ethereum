...
contract GenesisWarQube is ERC721A, Ownable, ReentrancyGuard {
    using Strings for uint256;

    string private _baseTokenURI;
    string public prerevealURL = 'ipfs://bafybeigdwtlmcunbyg6t4vd7i45bxftbdksnyxpytpnwki3jvj3sq6lgdq/default.json';
    bool private _addJson = true;

    uint16 constant MAX_GENESIS = 2500;
    uint16 constant TEAM_NFT = 200;
    ...
    mapping (address => bool) private _PartnerAddress;
    mapping (address => bool) private _OGAddress;
    bool constant _addressExists = true;

    //bool flags for wave
    bool private _partnersSaleActive = false;
    ...
    constructor() ERC721A("Genesis WarQube Test", "GENWQT") Ownable() ReentrancyGuard() { }
    
    function mint(uint16 quantity) public payable nonReentrant {
        
        require(_totalMinted() + quantity <= MAX_GENESIS - TEAM_NFT, "Maximum amount of avatars reached");
        ...
        if (msg.sender != owner()) {
            ...
            if (_PartnerAddress[msg.sender] == true) {
                maxquantity = 5;
            }

            if (_OGAddress[msg.sender] == true) {
                maxquantity = 5;
            }

            require(quantity > 0 && quantity <= maxquantity && balanceOf(msg.sender) + quantity <= maxquantity, "Minting is limited to max.");

            MintInfo memory info = getMintInfo(msg.sender, quantity);
            require(info.publicSale || info.partnerSale || info.ogSale || info.wlSale, "Sale has not started");
    ...
    function getMintInfo(address buyer, uint16 quantity) public view returns (MintInfo memory) {
        ...
        if (_WLaddress[buyer] == true) {
            _price = 0.08 ether;
        }

        if (_PartnerAddress[buyer] == true) {
            _price = 0.08 ether;
        }

        if (_OGAddress[buyer] == true) {
            _price = 0.08 ether;
        }
        ...
        return MintInfo(
            ...
            /* partnerSale */ _partnersSaleActive,
            /* ogSale */ _ogSaleActive,
    ...
    function setPartnersSale(bool partnersSaleActive) external onlyOwner {
        _partnersSaleActive = partnersSaleActive;
    }

    function isPartnersSale() public view returns (bool) {
        return _partnersSaleActive;
    }

    function setOGSale(bool ogSaleActive) external onlyOwner {
        _ogSaleActive = ogSaleActive;
    }
    ...
    function addPartnerAddress(address partnerAddress) external onlyOwner {
        _PartnerAddress[partnerAddress] = _addressExists;
    }

    function addOGAddress(address ogAddress) external onlyOwner {
        _OGAddress[ogAddress] = _addressExists;
    }
	...
	function airdrop(address to, uint16 quantity) external onlyOwner {
        require(_totalMinted() + quantity <= MAX_GENESIS, "Maximum amount of mints reached");
		_safeMint(to, quantity);
	}
