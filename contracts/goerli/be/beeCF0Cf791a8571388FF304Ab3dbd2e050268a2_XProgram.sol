...
abstract contract Matrix is IMatrix, Ownable2Step {
	...
	IBalance public Balances;
	ITuktu public Tuktus;
	API public TuktuAPI; // Tuktu API contract
...
contract XProgram is IXProgram, Matrix {
	...
	modifier OnlyAPI() {
		require(msg.sender == address(TuktuAPI), "XProgram: caller is not API");
		_;
	}
	...
	function ChangeStatusALU(uint _AID) external virtual OnlyAccountOwner(_AID, msg.sender) {
		_ChangeStatusALU(_AID);
	}

	function ChangeStatusALU(uint _AID, address _Owner) external virtual OnlyAPI OnlyAccountOwner(_AID, _Owner) {
		_ChangeStatusALU(_AID);
	}
