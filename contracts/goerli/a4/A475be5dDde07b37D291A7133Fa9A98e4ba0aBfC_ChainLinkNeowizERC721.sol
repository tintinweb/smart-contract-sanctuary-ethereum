...
contract ChainLinkNeowizERC721 is VRFConsumerBaseV2, NeowizERC721 {
    ...
    ) external whenSoldout onlyOwner {
...
abstract contract NeowizERC721 is ERC721ABurnable, ERC2771Context, Ownable {
    ...
    uint256 constant MINT_BUNDLE = 8;
    ...
    /// @notice Update all fields in the round with given arguments.
    ...
    /// @notice Mint nfts reserved for the team.
    function teamMint(address _to) external onlyOwner {
        _safeMint(_to, TEAM_SUPPLY);
    }

    /// @notice Mint unminted nfts to reveal.
    function mintResidue(address _to) external onlyOwner {
        _safeMint(_to, MAX_TOTAL_SUPPLY - totalSupply());
    }
    ...
    function _mintInRound(
        ...
        // Mint by bundle to avoid too expensive transfer even though paying more in minting.
        if (_quantity > MINT_BUNDLE) {
            uint256 times = _quantity / MINT_BUNDLE;
            for (uint256 i; i < times; i++) {
                _safeMint(_to, MINT_BUNDLE);
            }

            if (_quantity % MINT_BUNDLE != 0) {
                _safeMint(_to, _quantity % MINT_BUNDLE);
            }
        } else {
    ...
    {
        maxMintPerAccount = rounds[id].maxMintPerAccount;
        maxMint = rounds[id].maxMint;
        totalMinted = rounds[id].totalMinted;
        price = rounds[id].price;
        state = rounds[id].state;
        merkleRoot = rounds[id].merkleRoot;
        startTs = uint256(rounds[id].startTs);
        endTs = uint256(rounds[id].endTs);
    }
     ...
     *      To make the contract GSN compatible, we override this function to use ERC2771Context.
     */
