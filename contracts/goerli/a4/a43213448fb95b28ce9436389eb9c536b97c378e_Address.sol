...
contract xcyd is ERC721, Ownable, DefaultOperatorFilterer {
    ...
    constructor() ERC721("xcyd", "xcyd"){}
    
    
    function contractURI() public pure returns (string memory) {
        return "data:application/json;base64,eyJuYW1lIjoieGN5ZCIsImRlc2NyaXB0aW9uIjpudWxsLCJleHRlcm5hbF91cmwiOm51bGwsImZlZV9yZWNpcGllbnQiOiIweDEyOERhQjQwNjA3OENmQUUxOTk5YjE0M2NkNDIyMzk3OGUxODcxN2EiLCJzZWxsZXJfZmVlX2Jhc2lzX3BvaW50cyI6NTAwfQ==";
    }
    ...
    function calcTotal(uint count) public view returns(uint) {
        require(saleStatus != SaleStatus.PAUSED, "xcyd: Sales are off");
    ...
    function redeem(bytes32[] calldata merkleProof, uint count) external payable {
        require(saleStatus != SaleStatus.PAUSED, "xcyd: Sales are off");
        require(_tokenIds.current() + count <= COLLECTION_SIZE, "xcyd: Number of requested tokens will exceed collection size");
        require(count <= TOKENS_PER_TRAN_LIMIT, "xcyd: Number of requested tokens exceeds allowance (2)");
        require(msg.value >= calcTotal(count), "xcyd: Ether value sent is not sufficient");
        if(saleStatus == SaleStatus.PRESALE) {
            require(_whitelistMintedCount[msg.sender] + count <= TOKENS_PER_PERSON_WL_LIMIT, "xcyd: Number of requested tokens exceeds allowance (2)");
            bytes32 leaf = keccak256(abi.encodePacked(msg.sender));
            require(MerkleProof.verify(merkleProof, merkleRoot, leaf), "xcyd: You are not whitelisted");
            _whitelistMintedCount[msg.sender] += count;
        }
        else {
            require(_mintedCount[msg.sender] + count <= TOKENS_PER_PERSON_PUB_LIMIT, "xcyd: Number of requested tokens exceeds allowance (10)");
