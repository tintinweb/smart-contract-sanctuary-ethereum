...
contract SetInMergeERC721 is ERC721Enumerable, Ownable {
    constructor(
        string memory storageProtocol,
        string memory prerevealPowMetadata,
        string memory prerevealPosMetadata
    ) ERC721("Set In Merge", "SIM") {
        manager = msg.sender;
        metadataStorageProtocol = storageProtocol;
        prerevealPoWMetadata = prerevealPowMetadata;
        prerevealPoSMetadata = prerevealPosMetadata;
    }
    ...
    mapping(uint64 => uint64) public referrals;
    ...
    function referralMint(uint64 refCode) external {
        mint();
        referrals[refCode] = referrals[refCode] + 1;
    }
