...
{
    function liquidate(address _account, address _projectToken, address _lendingToken, uint256 _lendingTokenAmount, address liquidator) external;
}
...
contract PrimaryIndexTokenWrappedTokenGateway is Initializable, AccessControlUpgradeable, ReentrancyGuardUpgradeable
{
    ...
    function initialize(address pit, address weth, address _pitLiquidation) public initializer {
    ...
    // function withdraw(uint lendingTokenAmount) public {
    ...
    function deposit() public payable {
        ...
        primaryIndexToken.deposit(address(WETH), msg.value, address(this), msg.sender);
    }

    function withdraw(uint256 projectTokenAmount) public {
        primaryIndexToken.withdraw(address(WETH), projectTokenAmount, msg.sender, address(this));
        WETH.withdraw(projectTokenAmount);
        _safeTransferETH(msg.sender, projectTokenAmount);
    }

    function supply() public payable {
        WETH.deposit{value: msg.value}();
        WETH.transfer(msg.sender, msg.value);
        primaryIndexToken.supply(address(WETH), msg.value, msg.sender);
    }

    function redeem(uint256 bLendingTokenAmount) public {
        ...
        primaryIndexToken.redeem(address(WETH), amountToWithdraw, msg.sender);
        WETH.transferFrom(msg.sender, address(this), amountToWithdraw);
        WETH.withdraw(amountToWithdraw);
        _safeTransferETH(msg.sender, amountToWithdraw);
    }

    function redeemUnderlying(uint256 lendingTokenAmount) public {
        primaryIndexToken.redeemUnderlying(address(WETH), lendingTokenAmount, msg.sender);
        WETH.transferFrom(msg.sender, address(this), lendingTokenAmount);
        WETH.withdraw(lendingTokenAmount);
        _safeTransferETH(msg.sender, lendingTokenAmount);
    }

    function repay(address projectToken, uint lendingTokenAmount) public payable {
        ...
        require(msg.value >= paybackAmount, "PIT: msg value is less than repayment amount");
        WETH.deposit{value: paybackAmount}();    
        primaryIndexToken.repay(projectToken, address(WETH), paybackAmount, address(this), msg.sender);

        // refund remaining dust eth
        if (msg.value > paybackAmount) _safeTransferETH(msg.sender, msg.value - paybackAmount);
    }

    function borrow(address projectToken, uint256 lendingTokenAmount) public {
        primaryIndexToken.borrow(projectToken, address(WETH), lendingTokenAmount, msg.sender);
        WETH.transferFrom(msg.sender, address(this), lendingTokenAmount);
        WETH.withdraw(lendingTokenAmount);
        _safeTransferETH(msg.sender, lendingTokenAmount);
    }

    function liquidate(address _account, address _projectToken, uint256 _lendingTokenAmount) public payable {
        WETH.deposit{value: msg.value}();
        WETH.transfer(msg.sender, msg.value);
        require(msg.value == _lendingTokenAmount, "PIT: invalid value");
        pitLiquidation.liquidate(_account, _projectToken, address(WETH), _lendingTokenAmount, msg.sender);
    }
