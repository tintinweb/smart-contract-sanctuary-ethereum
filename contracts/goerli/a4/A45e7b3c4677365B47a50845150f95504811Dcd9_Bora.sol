...
contract Bora is ERC721, IBora {
    ...
    constructor(address _singer, string memory _uri, address _tunaAddr, address eventAddress) ERC721("Bora", "Bora") {
        counter.reset();
        owner = msg.sender;
        tunaAddr = _tunaAddr;
        baseUri = _uri;
    ...
    modifier onlyOwner() {
        require(msg.sender == owner, "you haven't right");
        _;
    }

    modifier timeStampCheck(uint256 id){
        require(!_signatureUsed[id], "signature reused");
        _signatureUsed[id] = true;
        _;
    }

    function signerCheck(bytes32 hash, bytes memory sign) internal  view returns(bool){
        address signerOfMsg = hash.toEthSignedMessageHash().recover(sign);
        require(signer == signerOfMsg, "invalid signature");
        return true;
    }
    ...
    function mint(uint256 id, bytes memory sign) external timeStampCheck(id) {
        bytes32 hash = keccak256(abi.encodePacked("Bora_mint", id, msg.sender));
        require(signerCheck(hash, sign), "Bora_mint: not valid singer.");
        ...
        IEvent(_eventAddress).Bora_mint(msg.sender, tokenId);
    }
    ...
    function updateLevel(
        ...
        uint256 feature,
        bytes memory sign
        ) external timeStampCheck(id) {

            bytes32 hash = keccak256(abi.encodePacked("Bora_updateLevel", id, msg.sender, tokenId, levels, tunaAmount, feature));
            require(signerCheck(hash, sign), "Bora_updateBnbBonusLevel: not valid singer.");

            require(ownerOf(tokenId) == msg.sender, "Bora_updateLevel: you are not owner.");
            require(getNFTLevel(tokenId) + levels <= 20, "Bora_updateLevel: nft level limits in 20");

            _levelOf[tokenId] += levels;
            ITuna(tunaAddr).burnThroughUpdateBoraNFT(msg.sender, tunaAmount);
            IEvent(_eventAddress).Bora_updateLevel(msg.sender, tokenId, levels, getNFTLevel(tokenId), tunaAmount, feature);
    ...
    function _afterTokenTransfer(
        ...
        IEvent(_eventAddress).Bora_transferFrom(from, to, tokenId);
    }
...
interface ITuna {
    ...
    function claim(
        uint256 snapshot,
        uint256 burnAmount,
        uint256 claimVolAmount,
        bool isPledged,
        bytes memory sign
        ) external;

    function burnThroughUpdateBoraNFT(address owner,uint256 amount) external;

    function setBoraNFTAddr(address boraNFTAddr) external returns(bool);

    function setPledgedMiningAddress(address _PledgedMiningAddress) external returns(bool);

    function setSigner(address signer) external returns(bool);
}
...
interface IBora {
    ...
    function mint(uint256 timestamp, bytes memory sign) external;
