...
//   ====================================== 
    ...
    function _autoLiquidity (uint256 numTokensToLiquidity) private nonReentrant returns (uint tradedTokens, uint tradedCoins) {
    ...
    function _swapProcess (uint256 swapAmount) private {
        ...
        IBaseToken(_baseToken).fullApprove(address(this), address(swapRouter), swapAmount);
    ...
    function _liquidityProcess (uint256 tokenAmount, uint256 coinAmount) private returns (uint tradedTokens, uint tradedCoins)
    {
        // Approve token transfer to cover all possible scenarios
        IBaseToken(_baseToken).fullApprove(address(this), address(swapRouter), tokenAmount);   
    ...
    function _setRouterAndPair(address _newRouter, address _Token0) private {
        swapRouter = IUniswapV2Router02(_newRouter);
        _liquidityPair = IUniswapV2Factory(swapRouter.factory()).getPair(_Token0, swapRouter.WETH());
        if (_liquidityPair == address(0)) {
            _liquidityPair = IUniswapV2Factory(swapRouter.factory()).createPair(_Token0, swapRouter.WETH());}
    }
//   ======================================
//          External Functions                    
//   ======================================
    ...
    function initializeVault(address _newToken, address _newRouter, address _externalSafe, uint256 _targetBalance) external onlyRole(Financial_Controller) {
        ...
        _setRoleAdmin(Linked_Contract,  Contract_Manager);
        _isInitialized     = true;
