...
contract ToyApeYachtClub is Ownable, ERC721A, ERC2981, ReentrancyGuard {
    ...
    Toybirds toybirds = Toybirds(0x2A75239A647c0C4aa22c8b2c6a74DC2bA1DA2687);
    ...
    constructor() ERC721A("Toy Ape Yacht Club", "TAYC") {
    ...
    {
        require(totalSupply() - HOLDER_MINT_TOTAL_SUPPLY + quantity <= WHITELIST_PUBLIC_TOTAL_SUPPLY, "Max supply reached");
        _safeMint(buyerAddress, quantity);
    }

    function holderMint(uint256 quantity)
        public
        payable
        virtual
        nonReentrant
        ...
        isEthAvailable(quantity)
    {
        ...
        require(toybirds.balanceOf(msg.sender) > 0, "Not a Toybirds holder");
        ...
        isEthAvailable(quantity)
    {
        ...
        require(totalSupply() - HOLDER_MINT_TOTAL_SUPPLY + quantity <= WHITELIST_PUBLIC_TOTAL_SUPPLY, "Max whitelist mint supply reached");
    ...
    function getOwnedToybirdIds(address holder) internal view returns(uint256[] memory) {
      uint256 amountToybirdsOwned = toybirds.balanceOf(holder);
      require(amountToybirdsOwned > 0, "Address does not own any Toybirds");
      uint256 totalToybirds = toybirds.totalSupply();
      uint256[] memory ownedToybirdIds = new uint256[](amountToybirdsOwned);
      uint256 idx = 0;

      for(uint256 i = 1; i <= totalToybirds; i++) {
        if(toybirds.ownerOf(i) == holder) {
    ...
    function holderClaim() public nonReentrant {
        ...
        uint256[] memory ownedToybirdIds = getOwnedToybirdIds(msg.sender);
         
        for (uint256 i = 0; i < ownedToybirdIds.length; i++) {
            if (!isClaimed(ownedToybirdIds[i])) {
