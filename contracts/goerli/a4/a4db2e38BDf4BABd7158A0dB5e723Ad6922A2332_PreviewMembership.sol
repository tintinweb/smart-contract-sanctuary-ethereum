...
contract PreviewMembership is Context, ERC721A, Ownable, ReentrancyGuard {
	...
	function mintingPrice() public view returns (uint256) {
		return (10**18 * usdPrice * 10**eth2usd.decimals()) / getETHPrice();
	}
	...
	function setBaseURI(string calldata newURI) external onlyOwner nonReentrant {
	...
	function setStage(uint256 _stage) external onlyOwner {
		require(_stage > 0, "10:stage");
		require(_stage != stage, "Is same as a current stage");
		stage = _stage;
	}
	...
	function setMintMax(uint256 limit) external onlyOwner {
		require(limit > 0, "10:limit");
		mintMax = limit;
	}

	function setHoldMax(uint256 limit) external onlyOwner {
		require(limit > 0, "10:limit");
		holdMax = limit;
	}

	function setAggregatorInterface(address _chainlinkTarget)
		external
		noZeroAddress(_chainlinkTarget)
		onlyOwner
	{
		eth2usd = AggregatorV3Interface(_chainlinkTarget);
		chainlinkTarget = _chainlinkTarget;
	...
	function setUSDPrice(uint256 price) external onlyOwner nonReentrant {
		...
		returns (bool)
	{
		...
		return signerAddress == recoverySigner;
	}
