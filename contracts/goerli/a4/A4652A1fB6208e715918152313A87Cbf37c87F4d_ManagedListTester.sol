...
interface ManagedListInterface {
    function checkWhitelistSpotsForAddress(uint256 _collabId, address _address) external view returns (uint256);
}

contract ManagedListTester is ERC721A, ReentrancyGuard, Ownable {
    using Strings for uint256;

    address public managedListContractAddress;
    ...
    constructor( address _managedListContractAddress) ERC721A("ManagedListTester", "MLT") {
        managedListContractAddress = _managedListContractAddress;
    }
    ...
    function testInterface(uint _collabId, address _address) public view returns (uint256) {
        ManagedListInterface managedListContract = ManagedListInterface(managedListContractAddress);
        return managedListContract.checkWhitelistSpotsForAddress(_collabId, _address);
    }

    function preMint(uint256  _mintAmount) public payable preMintCompliance(_mintAmount) nonReentrant {
        ManagedListInterface(managedListContractAddress).checkWhitelistSpotsForAddress(0, msg.sender);
