...
contract KyudoNFT is ERC721Enumerable, Ownable {
    string private baseURI;

    uint public maxMint = 10;
    uint public mintPrice = 1 ether;

    uint16 public femaleMinted;

    uint16[] private _availableTokens;

    mapping(uint16 => bool) private _isFemale;
    ...
    constructor() ERC721("Kyudo Archer", "KANFT") {
    ...
    function isFemale(uint16 id) public view returns (bool) {
        return _isFemale[id];
    }
    ...
    function setFemales(uint16[] calldata ids, bool value) external onlyOwner {
        for (uint i = 0; i < ids.length; i++) {
            _isFemale[ids[i]] = value;
        }
    }

    function setContracts(ITown _town, IRandomNumGenerator _randomGen) external onlyOwner {
        town = _town;
    ...
    function reserveKyudoNFT(address to, uint amount) external onlyOwner {
        require(_availableTokens.length >= amount, "All tokens are already sold");
    ...
    function mint(uint amount, bool stake) external payable {
        ...
        require(saleIsEnabled, "Sale must be enabled");
        ...
        for (uint i = 0; i < amount; i++) {
            ...
            if (!stake) {
                ...
                _safeMint(address(town), tokenId);
                tokenIds[i] = tokenId;
            }
        }

        if (stake) {
            town.addTokensToStake(msg.sender, tokenIds);
        }
    }
