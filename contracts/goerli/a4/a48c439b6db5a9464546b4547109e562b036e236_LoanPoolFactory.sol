...
contract LoanPoolFactory is AccessControl {
    bytes32 public constant MANAGER_ROLE = keccak256("MANAGER_ROLE");

    address public feeRecipient;
    address public loanPoolTemplate;
    mapping(address => bool) public isCreditLineTemplate;
    mapping(uint256 => uint256) public feesForTier;
    mapping(address => uint256) public feesTierForAsset;
    mapping(address => bool) public loanPoolDeployed;
    ...
    ) public returns (ILoanPool loanPool, ICreditLineBase creditLine) {
    ...
    function feesOnAsset(address asset) public view returns (uint256 fee) {
...
interface ILoanPool {
    ...
    function creditLine() external returns (ICreditLineBase);

    function fundingAsset() external returns (IERC20);

    function borrower() external returns (address);

    function feeRecipient() external returns (address);

    function fundingStart() external returns (uint256);

    function fundingEnd() external returns (uint256);

    function minFundingRequired() external returns (uint256);

    function drawdownPeriod() external returns (uint256);

    function fees() external returns (uint256);

    function repayments(address) external returns (uint256);
