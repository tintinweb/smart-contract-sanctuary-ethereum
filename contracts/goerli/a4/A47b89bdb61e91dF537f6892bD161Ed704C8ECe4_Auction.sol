    ...
    EIP712("Acution", "1")
{
    ...
    struct AuctionSaleInfo {
        address nftOwner; // NFT owner address
    ...
    function updateSale(uint256 _saleId, uint256 _endTime) external {
        require(saleInfo[_saleId].nftOwner == msg.sender, "NOT NFT OWNER");
        require(_endTime > block.timestamp, "Sale: INVALID_TIME");
    ...
    function cancelSale(uint256 _saleId) external {
        ...
        require(_saleInfo.nftOwner == msg.sender, "NOT NFT OWNER");

        address nftToken = _saleInfo.nftToken;
        uint256 tokenId = _saleInfo.tokenId;

        delete saleInfo[_saleId];

        if (IERC165(nftToken).supportsInterface(INTERFACE_ID_ERC1155)) {
    ...
    function endAuction(
        ...
        IInstantLiquidityVault(vault).repayWithAuction(
        ...
        if (IERC165(nftToken).supportsInterface(INTERFACE_ID_ERC1155)) {
            ...
            IERC721(nftToken).safeTransferFrom(address(this), _buyer, tokenId);
        }
