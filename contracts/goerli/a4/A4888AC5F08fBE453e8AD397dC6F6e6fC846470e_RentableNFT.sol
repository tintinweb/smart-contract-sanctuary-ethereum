...
contract RentableNFT is ERC4907, ReentrancyGuard {
    struct Listing {
        address owner;
        address renter;
    ...
    function mint(
        ...
        _listingMap[newTokenId] = Listing(
            msg.sender,
            address(0),
            newTokenId,
            pricePerDay,
        ...
        emit mintedToken(newTokenId, _tokenURI, block.timestamp);
    }
     ...
     * @param startDate  Start rental period
     * @param expiryDate  Expiry rental period
     */
    function rentNFT(
        uint256 tokenId,
        uint64 startDate,
        ...
        uint256 listedItemNumberOfDays = (listing.endDateUNIX - listing.startDateUNIX) / 60 / 60 / 24 + 1; // Calculate number of days
        uint256 enteredNumberOfDays = (expiryDate - startDate) / 60 / 60 / 24 + 1; // Calculate number of days
        require(
            enteredNumberOfDays <= listedItemNumberOfDays,
            "The rental period exceeds the maximum date rentable."
        );
        // Transfer rental fee
        uint256 rentalFee = listing.pricePerDay * enteredNumberOfDays; // Total amount of fees
        require(
            msg.value >= rentalFee,
        ...
        // Update listing
        setUser(tokenId, msg.sender, expiryDate); // Set owner
     ...
     * @param tokenId  Generated token id
     */
    function unsubscribe(uint256 tokenId) public payable nonReentrant {
        Listing storage listing = _listingMap[tokenId];
        require(
        ...
        emit unsubscribeNFT(msg.sender, tokenId, refund);
    }
