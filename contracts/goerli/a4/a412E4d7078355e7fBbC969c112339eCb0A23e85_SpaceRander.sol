...
contract SpaceRander is DesiderNft {
    ...
    uint256 public wlPrice = 0.023 ether;
    uint256 public pubPrice = 0.033 ether;
    uint256 public maxBalance = 5;

    string public baseURI;
    string public notRevealedUri = "https://arweave.net/jN-buTD8d2Co9GqYcRZGMoZv5-9KcNSqS4A9C4Jd2Mo";

    // time setting
    uint256 private ogStart = 1675145329;
    uint256 private wlStart = 1675145329;
    uint256 private pubStart = 1675145329;
    ...
    constructor(address _owner) ERC721("Desdier Space Renders", "DSR")
    {
    ...
    function ogMint() external {
        require(block.timestamp > ogStart, "mint not start");
        ...
        uint256 ogcheck = DesiderOG(0xe5726dA1bbeC43A13FeAc0BE915f91017baa1C1C).balanceOf(msg.sender);
        require(ogcheck >= 1,
    ...
    function wlMint(uint256 tokenQuantity, bytes32[] calldata merkleProof) external payable {
        ...
        require(
            num + tokenQuantity <= MAX_SUPPLY,
            "one whitelist can only use once"
        );
        require(
            totalSupply() + tokenQuantity <= MAX_SUPPLY,
            "Sale would exceed max supply"
        ...
        require(
            tokenQuantity * wlPrice <= msg.value,
            "Not enough ether sent"
        );

        _mintWithProof(tokenQuantity, merkleProof, saleMerkleRoot);
        _wlmint[msg.sender] += tokenQuantity;
    }
    ...
    function pubMint(uint256 tokenQuantity) external payable {
        ...
        require(
            tokenQuantity * pubPrice <= msg.value,
            "Not enough ether sent"
        );
        require(tokenQuantity <= maxBalance, "Can only mint {maxBalance} tokens at a time");
