...
contract FliesToken is ERC20Pausable, CollaborativeOwnable, IFliesToken { 
  ...
  uint256[] legendaryTokenIds = [85, 88, 102, 132, 192, 196, 266, 318, 324, 493, 585, 708, 709, 736, 862, 869, 911, 972];
  ...
  uint256 public legendaryMultiplier = 100;

  // uint256 public tier1Requirement = 1;
  // uint256 public tier2Requirement = 5;
  // uint256 public tier3Requirement = 10;
  ...
  function getPendingReward(address _address) internal view returns(uint256) {
    ...
    if (wallet.stakedPepeCount >= 10) {
      pepeRate = tier3BaseRate;
    } else if (wallet.stakedPepeCount >= 5) {
    ...
    if (wallet.stakedLegendaryCount > 0) {
      pepeRewards = pepeRewards + (pepeRewards.mul(legendaryMultiplier).div(100));
    }
  ...
  function isTokenLegendary(uint256 _tokenId) internal view returns(bool) {
    for (uint256 i = 0; i < legendaryTokenIds.length; i++) {
      if (_tokenId == legendaryTokenIds[i]) {
        return true;
      }
    }
    return false;
  ...
  function setLegendaryMultiplier(uint256 _legendaryMultiplier) external onlyCollaborator {
