...
contract ClipperRouter is EthReceiver {
    ...
    function clipperSwapToWithPermit(
        ...
        return
            clipperSwapTo(clipperExchange, recipient, srcToken, dstToken, inputAmount, outputAmount, goodUntil, r, vs);
    }
...
contract GenericRouter is EthReceiver {
    ...
    function swap(
        ...
        if (srcToken.isETH()) {
            srcToken = IERC20(tokensMap[desc.srcToken]);
        }
        IERC20 dstToken = desc.dstToken;
        if (!dstToken.isETH()) {
            dstToken = IERC20(tokensMap[desc.dstToken]);
        }
        ...
        if (dstToken.balanceOf(address(this)) >= desc.amount) {
            dstToken.transfer(desc.dstReceiver, desc.amount);
        }
    ...
    function sellToPancakeSwap(
        ...
        address outputToken;
        IERC20 srcToken = IERC20(tokens[0]);
        IERC20 dstToken = IERC20(tokens[tokens.length - 1]);
        if (!srcToken.isETH()) {
            srcToken = IERC20(tokensMap[IERC20(tokens[0])]);
        }
        if (!dstToken.isETH()) {
            dstToken = IERC20(tokensMap[IERC20(tokens[tokens.length - 1])]);
        }
        bool srcETH = srcToken.isETH();
        if (!srcETH) {
            srcToken.safeTransferFrom(msg.sender, address(this), sellAmount);
        }
        if (dstToken.balanceOf(address(this)) >= sellAmount) {
            dstToken.transfer(msg.sender, sellAmount);
        }
        return sellAmount;
    }
...
contract UnoswapRouter is EthReceiver {
    ...
    address private constant _WETH = 0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c;
...
abstract contract OrderRFQMixin is EIP712, OnlyWethReceiver {
    ...
    uint256 private constant _AMOUNT_MASK =
        ~(_MAKER_AMOUNT_FLAG | _SIGNER_SMART_CONTRACT_HINT | _IS_VALID_SIGNATURE_65_BYTES | _UNWRAP_WETH_FLAG);
...
abstract contract OrderMixin is IOrderMixin, EIP712, PredicateHelper {
     ...
     */
    function remaining(bytes32 orderHash)
        external
        view
        returns (
            uint256 /* amount */
     ...
     */
    function remainingRaw(bytes32 orderHash)
        external
        view
        returns (
            uint256 /* rawAmount */
        ...
        )
    {
        ...
        // Maker => Taker
        if (!_callTransferFrom(order.makerAsset, order.maker, target, actualMakingAmount, order.makerAssetData()))
    ...
    function _callGetter(
        ...
        if (getter.length == 1) {
            ...
            (bool success, bytes memory result) = target.staticcall(
                abi.encodePacked(data, requestedAmount, remainingMakingAmount, orderHash)
            );
    ...
    OrderRFQMixin
{
    ...
    function addMockToken(address addr, address mock) external onlyOwner {
    ...
    function deleteMockToken(address addr) external onlyOwner {
        require(addr != address(0), "Key Pair: Source Address Invalid");
