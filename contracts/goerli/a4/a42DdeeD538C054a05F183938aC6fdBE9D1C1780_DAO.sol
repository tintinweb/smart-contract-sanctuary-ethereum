...
contract DAO is OwnableUpgradeable {
    ...
    function vote(uint256 proposalId, bool supportProposal) external {
        ...
        if (supportProposal) daoProposal.votesFor++;
        else daoProposal.votesAgainst++;

        contributorVotes[msg.sender].push(daoProposal.id);
    }
    ...
    function votable(DaoProposals storage daoProposal) private {
        ...
        uint256[] memory tempVotes = contributorVotes[msg.sender];
