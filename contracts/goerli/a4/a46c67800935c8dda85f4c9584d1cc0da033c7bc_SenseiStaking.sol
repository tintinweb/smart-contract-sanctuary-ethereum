...
contract SenseiStaking is Ownable, ERC721Holder {
    ...
    function tokenExpiration(uint256 _tokenId)
        public
        view
        returns (uint256 expiration) 
    {
        return stakedToken[_tokenId].lockDownExpiration;
    }
    ...
    function _stake(address _user, uint256 _tokenId, uint256 _lockdownExpiration) internal {
        ...
        require(_lockdownExpiration > block.timestamp, "Expiration time is in the past must be in the future");
        require(
    ...
    function updateLockDown(uint256 lockdownExpiration, uint256[] memory tokenIds) public onlyOwner {
        require(tokenIds.length > 0,
        "You are trying to modify 0 tokens");
        for (uint256 i = 0; i < tokenIds.length; i++) {
    ...
    function _updateLockDown(uint256 _lockdownExpiration, uint256 _tokenId) internal {
        require(_lockdownExpiration > block.timestamp, "You are trying to update in the past");
        require(enabled, "Staking System: the staking has not started");
