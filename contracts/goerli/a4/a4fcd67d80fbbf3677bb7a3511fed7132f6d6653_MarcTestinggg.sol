...
contract MarcTestinggg is Ownable, ERC721A, ReentrancyGuard, DefaultOperatorFilterer {
  using Strings for uint256;
  ...
  uint256 public PhaseCollectionSize_ = 45; // total number of nfts in Phase 1
  ...
  string private fourth_baseTokenURI;

  uint256 public FIRST_TOKEN_NUMBER = 45;
  uint256 public SECOND_TOKEN_NUMBER = 50;
  uint256 public THIRD_TOKEN_NUMBER = 100;



  mapping(address => uint256) public publicClaimedBy;

  constructor() ERC721A("Marc Testinggg","MTSS") {
  ...
  function setFourthBaseURI(string memory baseURI) external onlyOwner {
        fourth_baseTokenURI = baseURI;
  }


    function changeFIRST_TOKEN_NUMBER(uint256 q) external onlyOwner {
        FIRST_TOKEN_NUMBER = q;
    }

    function changeSECOND_TOKEN_NUMBER(uint256 q) external onlyOwner {
        SECOND_TOKEN_NUMBER = q;
    }

   function changeTHIRD_TOKEN_NUMBER(uint256 q) external onlyOwner {
        THIRD_TOKEN_NUMBER = q;
    }
   ...
   function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
    ...
    } else if (tokenId <= SECOND_TOKEN_NUMBER) {

            return
                bytes(second_baseTokenURI).length > 0 ? string(abi.encodePacked(second_baseTokenURI,"/",tokenId.toString(),".json")): "";


    } else if(tokenId <= THIRD_TOKEN_NUMBER) {

            return
                bytes(third_baseTokenURI).length > 0 ? string(abi.encodePacked(third_baseTokenURI,"/",tokenId.toString(),".json")): "";

    } else {

            return
                bytes(fourth_baseTokenURI).length > 0 ? string(abi.encodePacked(fourth_baseTokenURI,"/",tokenId.toString(),".json")): "";
