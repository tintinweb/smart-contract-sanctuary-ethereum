...
struct Trade {
    address creator;
    bool creatorInEscrow;
    address counterparty;
    bool counterpartyInEscrow;
    address refer;
    bool cancelled;
    uint256 expireAt;
...
error TradeIsNotCancelledNorExpired();
...
contract Trader is ReentrancyGuard, Configurable, AssetHandler, Banker {
    ...
    /// @dev Only trades in progress are on-chain. Once a trade gets completed or cancelled (with all escrows withdrawn) it is deleted.
    ...
    function offerTrade(
    ...
    ) external nonReentrant newTradesNotSuspended allOperationsNotSuspended {
        ...
        // Put creator's assets in escrow.
        // Will fail any of the assets is not available/transferable
        ...
        for (uint256 i = 0; i < creator20Assets.length; ++i) {
            _transfer20Asset({asset: creator20Assets[i], from: msg.sender, to: address(this)});
            trades[id].creator20Assets.push(creator20Assets[i]);
        }
        for (uint256 i = 0; i < creator721Assets.length; ++i) {
            _transfer721Asset({asset: creator721Assets[i], from: msg.sender, to: address(this)});
            trades[id].creator721Assets.push(creator721Assets[i]);
        }
        for (uint256 i = 0; i < creator1155Assets.length; ++i) {
            _transfer1155Asset({asset: creator1155Assets[i], from: msg.sender, to: address(this)});
        ...
        trades[id].creatorInEscrow = true;
    ...
    function acceptTrade(string memory id)
        ...
        tradeIsOngoing(id)
        feesArePaid(trades[id].counterparty)
    {
        if (msg.sender != trades[id].counterparty) {
            revert UserNotInTrade();
        }

        if (trades[id].counterpartyInEscrow) {
            revert TradeIsAlreadyAccepted();
        }

        trades[id].counterpartyInEscrow = true;

        // Put counterparty's assets in escrow.
        _transferAssets({
    ...
    function executeTrade(string memory id)
        ...
        tradeIsOngoing(id)
        tradeIsAccepted(id)
        feesArePaid(trades[id].creator)
    {
        if (msg.sender != trades[id].creator) {
            revert UserNotInTrade();
        }

        // Transfer creator's assets: from ecrow to counterparty
        _transferAssets({
        ...
        // Transfer counterparty's assets: from escrow to creator
        _transferAssets({
            erc20: trades[id].counterparty20Assets,
            erc721: trades[id].counterparty721Assets,
            erc1155: trades[id].counterparty1155Assets,
    ...
    {
        // Counterparty can only cancel after accepting the trade
        if (msg.sender == trades[id].counterparty && !trades[id].counterpartyInEscrow) {
            revert TradeIsNotAccepted();
        }

        // Depending of the party cancelling the trade, return its asset from escrow.
        // The opposite party is responsible to pull its asset afterwards
        if (msg.sender == trades[id].creator) {
            trades[id].creatorInEscrow = false;

            // Remove creator's assets from escrow by transfer them back to creator
            _transferAssets({
                erc20: trades[id].creator20Assets,
                erc721: trades[id].creator721Assets,
                erc1155: trades[id].creator1155Assets,
            ...
            trades[id].counterpartyInEscrow = false;

            // Remove counterparty's assets from escrow by transfer them back to counterparty
            _transferAssets({
                erc20: trades[id].counterparty20Assets,
                erc721: trades[id].counterparty721Assets,
                erc1155: trades[id].counterparty1155Assets,
            ...
            // Counterparty having assets in escrow means it has also paid the trading fees.
            // Refund those fees in credts.
            _refundTradeInCredits(trades[id].counterparty);
        }

        // Remove the trade only if no more assets are in escrow,
        // otherwise mark it for removal later when both parties have withdrawn their escrows
        if (trades[id].creatorInEscrow || trades[id].counterpartyInEscrow) {
            trades[id].cancelled = true;
        } else {
            delete trades[id];
        }
    ...
    function withdrawEscrow(string memory id)
        external
        nonReentrant
        allOperationsNotSuspended
        tradeIsCancelledOrExpired(id)
        participatesInTrade(id)
    {
        // Mark trade as cancelled once a party withraw its escrow from expiration,
        // as to not allow any temper with the block timestamp from the other party
        trades[id].cancelled = true;

        if (msg.sender == trades[id].creator) {
            if (!trades[id].creatorInEscrow) {
                revert UserDontHaveEscrow();
            }
            trades[id].creatorInEscrow = false;
        ...
        // Remove the trade once both party have removed their escrow (1 party for manually cancelled trade, 2 parties for expired trade)
        // NOTE: Bonus point for the 'last party' to remove their assets, get some gas free gas in return
        if (!trades[id].creatorInEscrow && !trades[id].counterpartyInEscrow) {
            delete trades[id];
        }

        emit TradeUpdated(TradeEvent.EscrowWithdrawn, id, msg.sender);
    }
    ...
    modifier tradeIsCancelledOrExpired(string memory id) {
        if (trades[id].creator == address(0)) {
            revert TradeDoesNotExist();
        }

        if (!trades[id].cancelled && trades[id].expireAt > block.timestamp) {
            revert TradeIsNotCancelledNorExpired();
        }
    ...
    modifier feesArePaid(address account) {
        if (msg.value + _useCredits(account) != TradingFee) {
            revert InvalidFeesPayment();
        }
...
abstract contract Banker is Owned, Configurable {
    event CreditOperation(address account, int256 amount);

    mapping(address => uint256) public credits;
    uint256 public totalCredits;

    function _refundTradeInCredits(address account) internal {
        credits[account] += TradingFee;
        totalCredits += TradingFee;

        emit CreditOperation(account, int256(TradingFee));
    }

    function _useCredits(address account) internal returns (uint256) {
        uint256 availableCredit = credits[account];
        if (availableCredit == 0) {
            return 0;
        }

        if (availableCredit <= TradingFee) {
            delete credits[account];
        } else {
            credits[account] -= TradingFee;
            availableCredit = TradingFee;
        }
        totalCredits -= availableCredit;

        emit CreditOperation(account, -int256(availableCredit));

        return availableCredit;
    }

    function fundCredits(address account, uint256 amount) external onlyOwner {
        credits[account] += amount;
        totalCredits += amount;

        emit CreditOperation(account, int256(amount));
    }

    /// Withdraw all funds available, minus the amount belonging to users credits.
    /// The reason is to support refer payment (which has a variable configurable percentage) when both parties
    /// pay using credits.
    function withdraw(address account) external onlyOwner {
        uint256 funds = address(this).balance - totalCredits;
        if (funds <= 0) {
    ...
    /// Withdraw all funds, leaving nothing for ongoing credits if there are any.
    /// Because of solidity mappings limitation, the caller should provide all addresses present in `credits` mappings,
    /// so that the methods loops and removes all entries from the mapping.
    function unsafeWithdraw(address account, address[] memory accountsToReset)
        external
        onlyOwner
        allOperationsAreSuspended
    {
        ...
        totalCredits = 0;
        for (uint256 i = 0; i < accountsToReset.length; ++i) {
            delete credits[accountsToReset[i]];
        }
