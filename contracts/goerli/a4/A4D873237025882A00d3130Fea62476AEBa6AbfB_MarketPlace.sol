...
contract MarketPlace is Ownable, Pausable, ReentrancyGuard, ERC721Holder, IMarketPlace {

    using Address for address;
    using SafeMath for uint256;
    using SafeERC20 for IERC20;
    // TODO check accepted tokens
    ...
    /**
     * @dev Initialise this contract.
     * @param _acceptedToken - accepted currency for payments
     */
    constructor(address _acceptedToken, address _manager) {
        require(
        ...
        require(
            _manager.isContract(),
            "Marketplace: The manager address must be a deployed contract"
        );
     ...
     */
    function setPaused(bool _setPaused) external override onlyOwner {
     ...
     * @param _priceInAcceptedToken - Price in AcceptedToken for the supported coin
     * @param _expiresAt - Duration of the order (in hours)
     */
    function createOrder(
        address _nftAddress,
        uint256 _assetId,
        uint256 _priceInAcceptedToken,
        uint256 _expiresAt
    )
        external override whenNotPaused nonReentrant
    {
        require(manager.isNFTSupported(_nftAddress), "Marketplace: Cannot create an order with unsupported NFT token");
        _createOrder(_nftAddress, _assetId, _priceInAcceptedToken, _expiresAt);
    }
     ...
     * @param _nftAddress - Non fungible registry address
     * @param _assetId - ID of the published NFT
     * @param _priceInAcceptedToken - Price in Wei for the supported coin
        ...
        internal
    {
        ...
        require(_priceInAcceptedToken > 0, "Marketplace: Price should be greater than 0");
        ...
        require(nftRegistry.getApproved(_assetId) == address(this) || nftRegistry.isApprovedForAll(assetOwner, address(this)), "Marketplace: Token should be approved for marketplace");
        ...
        internal
    {
        ...
        // Bids will be updated off-chain upon this event being fired
        ...
        external override whenNotPaused nonReentrant
    {
        ...
        require(_priceInAcceptedToken > 0, "Marketplace: Price should be bigger than 0");
        require(
            _expiresAt > (block.timestamp).add(1 minutes),
            "Marketplace: Publication should be more than 1 minute in the future"
        );

        order.price = _priceInAcceptedToken;
        order.expiresAt = _expiresAt;

        emit OrderUpdated(order.id, _priceInAcceptedToken, _expiresAt);
    }


    /**
     * @dev Executes the sale for a published NFT and checks for the asset fingerprint. 
     * Buyer / msg.sender pays the tx fees
     * @param _nftAddress - Address of the NFT registry
     * @param _assetId - ID of the published NFT
     */
    function buyNow(
        ...
        external override whenNotPaused nonReentrant
    {
        ...
        (address feeCollector, uint256 cutPerMillion) = manager.getNFTFeeInfo(_nftAddress);

        if (cutPerMillion > 0) {
            // Calculate sale share
            saleShareAmount = (order.price)
                .mul(cutPerMillion)
                .div(1e6);
    ...
    /**
     * @dev Executes the sale for a published NFT and checks for the asset fingerprint. 
     * Seller / msg.sender pays the tx fees
     * @param _nftBidder - Address of the bidder (gotten off-chain)
     * @param _nftAddress - Address of the NFT registry
     * @param _assetId - ID of the published NFT
     * @param _priceInAcceptedToken - Order price
     */
        ...
        external override onlyOwner whenNotPaused nonReentrant
    {
        ...
        /// Check the order seller is the person accepting the offer
        require(_nftBidder != address(0), "Marketplace: invalid bidder address");
        require(_priceInAcceptedToken > 0, "Marketplace: invalid price");
        ...
        if (cutPerMillion > 0) {
            // Calculate sale share
            saleShareAmount = _priceInAcceptedToken
                .mul(cutPerMillion)
                .div(1e6);
        ...
        acceptedToken.safeTransferFrom(
            _nftBidder,
            order.seller,
            _priceInAcceptedToken.sub(saleShareAmount)
        );
    }
...
interface IMarketPlace {
    ...
    struct Order {
        ...
        address nftAddress;
        // Price (in dai) for the published item
        uint256 price;
