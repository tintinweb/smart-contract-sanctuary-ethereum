...
contract Lockdrop is ILockdrop, TokenRecoveryUpgradeable, GuardableUpgradeable, ReentrancyGuardUpgradeable {
     ...
     * @inheritdoc ILockdrop
     */
    function updatePool(bytes32 _poolId, bytes32 _requiredPrivileges, bool _enabled) external override onlyAdmin {
    ...
    function updateDelegationConfig(DelegationConfig calldata newConfig) external override whenNotPaused {
        ...
        require(
            block.timestamp < dates.phase2 || _prevConfig.percentLocked < newConfig.percentLocked,
            "Lockdrop: DECREASE PERCENT LOCKED ENDED"
        );
        require(
            block.timestamp < dates.phase2 || _prevConfig.lockDuration == newConfig.lockDuration,
            "Lockdrop: UPDATE LOCK DURATION ENDED"
        );
