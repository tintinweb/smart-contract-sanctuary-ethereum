...
contract StabilizerNode is Permissions {
  ...
  function _distributeRewards(uint256 rewarded) internal {
    ...
    if (callerCut > 0) {
      rewardToken.transferFrom(msg.sender,msg.sender, callerCut);
    }

    if (auctionPoolCut > 0) {
      rewardToken.transferFrom(msg.sender, auctionPool, auctionPoolCut);
    }

    if (swingTraderCut > 0) {
      rewardToken.transferFrom(msg.sender, address(swingTrader), swingTraderCut);
    }

    if (treasuryCut > 0) {
      rewardToken.transferFrom(msg.sender, treasuryMultisig, treasuryCut);
    }

    if (daoCut > 0) {
      rewardToken.transferFrom(msg.sender,address(dao), daoCut);
    }

    if (lpCut > 0) {
      rewardToken.transferFrom(msg.sender,address(rewardThrottle), lpCut);
      rewardThrottle.handleReward();
    }

    if (buyBurn > 0) {
      rewardToken.transferFrom(msg.sender,address(buyBurnToken), buyBurn);
    }
  ...
  function _replenishLiquidityExtension(uint256 rewards) internal returns (uint256 remaining) {
    ...
    if (deficit >= maxContrib) {
      rewardToken.transferFrom(msg.sender,address(liquidityExtension), maxContrib);
      return rewards - maxContrib;
    }

    rewardToken.transferFrom(msg.sender,address(liquidityExtension), deficit);
  ...
  function emergencyMintYsd(uint256 amount) external {
    ...
    rewardToken.transferFrom(msg.sender,address(swingTrader), amount);
