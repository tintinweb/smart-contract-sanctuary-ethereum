...
contract KryptosSaurus is ERC721, ERC721Enumerable, Ownable {
     using Strings for uint256;
 ...
 // 5000 SAURUS in total !!!
    constructor(
    string memory _tokenName,
    string memory _tokenSymbol,
    string memory _hiddenMetadataUri
  ) ERC721("Kryptos Saurus", "SAURUS") {
 ...
 WhitelistMintOpen = _WhitelistMintOpen;
    ...
    function airdrop(address _to, uint256 _quantity) external onlyOwner {
        require(
            _quantity + totalSupply() <= MAX_TOKENS,
            "Kryptos Saurus: Mint is not started"
        );
        _mint(_to, _quantity);
    }
 ...
 uint256 ownerMintedCount = addressMintedBalance[msg.sender];
    ...
    require(msg.value >= 0 * _mintAmount, "Ether value sent is not correct");  
    require(_mintAmount <= maxMintAmountPerTx, "max mint amount per session exceeded");
    require(ownerMintedCount + _mintAmount <= nftPerAddressLimit, "only 5 SAURUS free per Wallet");
  ...
  uint256 ownerMintedCount = addressMintedBalance[msg.sender];
    ...
    require(ownerMintedCount + _mintAmount <= nftPerAddressLimit, "max SAURUS per address exceeded");
