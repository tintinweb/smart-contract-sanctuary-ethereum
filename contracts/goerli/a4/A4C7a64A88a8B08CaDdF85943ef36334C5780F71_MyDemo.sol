...
// 合約
contract MyDemo is ERC721Enumerable, Ownable {
    // 使用 using-for 使 Strings 庫的函數成為 uint256 變量的成員函數
    using Strings for uint256;
    // 是否開始銷售
    ...
    // 鑄造
    function mintMyNFT(uint256 tokenQuantity) public payable {
        // 加上本次鑄造的累積數量不大於最大銷售限額
        // tokenQuantity -> 本次鑄造的數量
        require(
        ...
        // 餘額 balanceOf(msg.sender) + 本次鑄造數量不大於單一帳戶限額
        require(
    ...
    function _mintMyNFT(uint256 tokenQuantity) internal {
        ...
        for (uint256 i = 0; i < tokenQuantity; i++) {
            ...
            // 滿足累積鑄造數量不大於最大銷售限額
            // 按理說調用有鑄造函數時已經檢查過，這裡應該不需要重複檢查
            if (totalSupply() < MAX_SUPPLY) {
                // ERC721 的鑄造函數，傳入申請鑄造的地址 以及 該次鑄造的 tokenID
