...
contract UTAutographController {
    ...
    function setValue(uint16[] memory tokenId, storageStruct memory value, address traitStorage) public {
        ...
        for(uint8 i = 0; i < tokenId.length; i++) {
            ...
            if(currentData.state == 1 && value.state == 2) {
                // From state 1 - "Open to redeem" to state 2 - "Signature asked" - Only owner of the NFT can change it
                require(
                    erc721.ownerOf(currentTokenId) == msg.sender,
                    "Autograph Controller: not owner of token."
                );
    ...
    */
    ...
    function toggleLock() public {
        require(
            ...
            "AutographController: Not authorised"
        );
        locked = !locked;
    }
}
...
contract UTAutographStorage {
    ...
    struct storageStruct {
        uint8   state;                  // The state of the Autograph
        string  ipfsHash;               // The IPFS hash of the Autograph (if it was signed exists)
    }
