...
contract POTA is ERC721A, IERC721Receiver, Ownable, ReentrancyGuard, AccessControl {
  using ECDSA for bytes32;
  bytes32 public constant MINTER_ROLE = keccak256("MINTER_ROLE");
  bool public ENABLED = false;
  string public tokenUriBase;
  uint256 public maxSupply = 140;
  IERC1155 public OSContract; // = 0x495f947276749Ce646f68AC8c248420045cb7b5e;
  ...
  function setOSContract(IERC1155 _OSContract) external onlyOwner {
    OSContract = _OSContract;
  }
  ...
  function swap(bytes memory _encoded, bytes memory _signature) external {
    // get values out of ABI encoded string
    (uint256 oldTokenID, uint256 newTokenID) = abi.decode(_encoded, (uint256, uint256));
    // ensure this signature was generated by our server
    require(_verify(_encoded, _signature, signer), "invalid signature.");

    // TRANSFER OLD TOKEN TO DEAD ADDRESS
    OSContract.safeTransferFrom(msg.sender, dead, oldTokenID, 1, "");
    // TRANSFER NEW TOKEN TO USER
    IERC721A(address(this)).approve(msg.sender, newTokenID);
    safeTransferFrom(address(this), msg.sender, newTokenID);
  }
