    ...
    IService
{
    ...
    function createPool(
        ...
        if (address(pool) == address(0)) {
            ...
            require(
                ITGE(pool.getGovernanceToken().getTGEList()[0]).state() == ITGE.State.Failed,
...
interface IPool is IGovernorProposals {
    ...
    //function changePoolSecretary(address[] memory addSecretary, address[] memory removeSecretary) external;
...
library ExceptionsLibrary {
    ...
    string public constant ACTIVE_GOVERNANCE_SETTINGS_PROPOSAL_EXISTS = "ACTIVE_GOVERNANCE_SETTINGS_PROPOSAL_EXISTS";
