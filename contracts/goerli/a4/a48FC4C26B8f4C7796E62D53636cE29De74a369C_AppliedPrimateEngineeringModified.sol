...
contract AppliedPrimateEngineeringModified is ERC721, Ownable {
    ...
    bytes32 public constant ROBOT_KEY = keccak256("REBUILD");

    bytes32 public constant SWAMP_KEY = keccak256("ALPHA");
    bytes32 public constant NOISE_KEY = keccak256("BRAVO");
    bytes32 public constant ZOMBIE_KEY = keccak256("CHARLIE");
    bytes32 public constant ELECTRIC_KEY = keccak256("DELTA");
    bytes32 public constant DEATHBOT_KEY = keccak256("ECHO");
    ...
    constructor(address metadataStore_)
        ERC721("Applied Primate Engineering Modified", "KEYCARD")
        Ownable()
    {
    ...
    function mint(address to, uint256 id) public onlyAuthorized {
        if (id > MAX_SUPPLY || id < 1) revert InvalidTokenId();
        _safeMint(to, id);
    }

    function ownerApplyImage(uint256 id, bytes32 imageKey_) public onlyAuthorized {
        if (ownerMetadataFunctionLocked) revert OwnerMetadataFunctionLocked();
        _validateToken(id);
        imageKeys[id] = imageKey_;
    }

    function ownerApplyMetadata(uint256 id, bytes32 metadataKey) public onlyAuthorized {
        if (ownerMetadataFunctionLocked) revert OwnerMetadataFunctionLocked();
        _validateToken(id);
    ...
    function lockOwnerMetadataFunction() public onlyAuthorized {
    ...
    function setOperator(address operator_) public onlyAuthorized {
