...
contract VoteContract is AccessControl {
    ...
    mapping(uint256 => mapping(address => bool)) public proposalVotedUsers;
    mapping(uint256 => mapping(address => bool)) public proposalUserVotes;
    mapping(uint256 => mapping(address => uint256)) public proposalUserVotePowers;
    ...
    uint256 public createCooldown = 1 days;
    ...
    event CreateCooldownUpdated(uint256 oldValue, uint256 newValue);
    ...
    function addProposal() external {
        require(block.timestamp >= lastProposalCreated[msg.sender] + createCooldown, "cooldown");
    ...
    function updateCreateCooldown(uint256 newInterval) external onlyRole(DEFAULT_ADMIN_ROLE) {
        emit CreateCooldownUpdated(createCooldown, newInterval);
        createCooldown = newInterval;
    }
}