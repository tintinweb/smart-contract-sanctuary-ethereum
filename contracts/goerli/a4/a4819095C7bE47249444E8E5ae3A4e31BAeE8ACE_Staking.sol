...
contract Migrate is IERC721Receiver{
    ...
    address[] owners = [0x49939aeD5D127C2d9a056CA1aB9aDe9F79fa8E81,0x9219d3Fa809b0CC3Cbf994eb3db9C4Ab346D90A9];
    ...
    function addPair(string memory tokenName, address tokenAddrV2) public Owners{
        tokens[tokenName] = pairParams({tokenAddr:tokenAddrV2});
    }
...
function addLiquidity(
    ...
    function migrateLP(uint amountLP,string memory _tokenName)  internal returns(uint256 token0,uint256 token1) {
...
contract Staking is Migrate{
    ...
    uint64 constant MINIMAL_AMOUNT = 1e17;
    ...
    mapping(uint32 => uint256) tokenID;
    ...
    function stake(
        ...
        require(
            getPair(_tokenName) != address(0),
            "Not this token"
            );
        require(
            _sum >= MINIMAL_AMOUNT,
            "Error minimal amount"
            );
        ...
        (uint tokenId) = mintPosition(
                            _sum,
                            _tokenName,
                            tickLow,
                            tickUp
                        );
        tokenID[txId] = tokenId;
    ...
    function claimFund(uint32 _txID) external {
        require(
            block.timestamp >= 
            timeTokenLock[msg.sender][_txID].timeUnlock,
        ...
        require(
            msg.sender == 
            timeTokenLock[msg.sender][_txID].sender,
