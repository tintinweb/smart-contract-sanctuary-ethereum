...
error MaximumTotalSupplyReached(uint16 maximum);
...
contract StateNFT is IStateNFT, ERC721Upgradeable, PaymentSplitterUpgradeable, BaseContract, StateNFTStorage {
    ...
    event TokenStateSet(uint256 tokenId, uint8 state);
    ...
    function nextState(uint256 tokenId) external onlyOperator {
        _requireMinted(tokenId);
        if (_tokenStates[tokenId] == _lastState) revert LastStateReached(tokenId);
        _tokenStates[tokenId] += 1;
    ...
    function getMaxTokenSupply() external view returns (uint16) {
    ...
    function tokenURI(uint256 tokenId) public view virtual override(ERC721Upgradeable) returns (string memory) {
        ...
        string memory tokenId_ = Strings.toString(tokenId);
        string memory state = Strings.toString(_tokenStates[tokenId]);
        return string(abi.encodePacked(baseURI, tokenId_, "/", state));
    }
...
abstract contract StateNFTStorage {
    ...
    uint8 internal _lastState;
    uint16 internal _maxTokenSupply;
    uint256 internal _mintValue;
    bool internal _burnEnabled;

    mapping(uint256 => uint8) internal _tokenStates;
}
