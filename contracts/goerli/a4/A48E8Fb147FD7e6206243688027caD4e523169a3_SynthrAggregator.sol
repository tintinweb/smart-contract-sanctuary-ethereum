...
contract DebtShareAggregator {
    ...
    function balanceOfOnPeriod(address account, uint periodId) external view returns (uint) {
        _balanceOfOnPeriod(account, periodId);
    }
    ...
    // Temporary function
    function refreshDebtShare(address account, uint amount) external {
        _burnDebtShare(account, amount);
    }
}
...
contract LiquidatorRewardsAggregator {
    ...
    function earned(
        ...
        return (debtShare * (accumulatedRewardsPerShare - uint256(entry.entryAccumulatedRewards))) / (10**18) + entry.claimable;
    }
}
...
contract SynthAggregator {
    ...
    function _burnSynth(
        ...
        uint256 accountBal = _synthBalanceOf[currencyKey][account];
        _synthBalanceOf[currencyKey][account] = accountBal > amount ? accountBal - amount : 0;

        uint256 totalBal = _synthTotalSupply[currencyKey];
...
contract SynthrAggregator is Ownable, CollateralAggregator, DebtShareAggregator, LiquidatorRewardsAggregator, RewardEscrowV2Aggregator, SynthAggregator {
    ...
    function initialize(address __wrappedSynthr, address __exchangeFeeAddress, address __synthrBridge) external onlyOwner {
        _wrappedSynthr = __wrappedSynthr;
    ...
    function liquidate(
        ...
        uint totalDebtShare = _debtShareTotalSupply();
        if (totalDebtShare > 0) {
            _updateAccumulatedShare(notifyAmount * (10**18) / totalDebtShare);
        }
