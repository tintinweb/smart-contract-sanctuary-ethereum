...
contract SpaceAnimalsNFT is ERC721Enumerable, ERC721Burnable, ERC721URIStorage, Ownable {
    ...
    constructor(string memory name_, string memory symbol_, address _signatory) ERC721(name_, symbol_) {
    ...
    function permit(address account, uint256 tokenId, uint256 deadline, uint8 v, bytes32 r, bytes32 s) internal view returns (bool) {
        ...
        bytes32 structHash = keccak256(abi.encode(keccak256("Permit(address account,uint256 tokenId,uint256 deadline)"), account, tokenId, deadline));
