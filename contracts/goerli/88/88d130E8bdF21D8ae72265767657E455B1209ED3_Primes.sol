...
// a collection of scalable, animated and interactive digital art on-chain.
...
contract Primes is ERC721r, Ownable {
    ...
    uint256 private constant MAX_SUPPLY = 15; // the collection start at 1 but ERC721r at 0.
    uint256 private constant GIFTS = 1;
    uint256 private constant PRESALE_MAX_SUPPLY = 7; // 750
    uint256 private constant MAX_SUPPLY_MINUS_GIFTS = MAX_SUPPLY - GIFTS;
    // uint256 public specialprice = 0.06 ether;
    ...
    function privateMint(uint256 _quantity) external payable isUser {
        ...
        require(step == Steps.privateSale, "Primes: sale is not activated");
        require(
            currentTime() >= timestampStart,
            "Primes: sale has not started yet"
        );
        require(
            currentTime() < timestampStart + 24 hours,
            "Primes: sale is finished"
        );
        require(_quantity > 0, "Primes: mint at least 1 NFT");
        require(
            totalSupply() + _quantity <= PRESALE_MAX_SUPPLY,
            "Primes: max supply exceeded"
        );

        uint price = presalePrice;
        require(price > 0, "Primes: price is 0");

        if (msg.sender != owner()) {
            if (allowed == true) {
                require(isListed(msg.sender), "Primes: user is not allowed");
            }
            require(
    ...
    function publicMint(uint256 _quantity) external payable isUser {
        require(!paused, "Primes: contract is paused");
        require(step == Steps.publicSale, "Primes: sale is not activated");
        require(
        ...
        require(price > 0, "Primes: price is 0");
        require(msg.value >= price * _quantity, "Primes: insufficient funds");
    ...
    function tokenURI(
        ...
        string
            memory description = "Primes is the first on-chain collection of scalable, animated and interactive vector graphics NFTs by g56d";
    ...
    // utils
    modifier isUser() {
        require(tx.origin == msg.sender, "Primes: not allowed");
        _;
    }
    ...
    function isListed(address _user) public view returns (bool) {
