...
contract DigitalVehicle is AccessControl, ReentrancyGuard {
    ...
    //using array depending on how many logs are made data structure must be
    ...
    //idee: log maintance ist nur ein event und in der dapp werden alle events angezeigt
    function logMaintenance(string memory _parts) public {
        maintenance.push(MaintenanceEntry(msg.sender, block.timestamp, _parts));
    ...
    function getMaintenanceLog()
        public
        view
        returns (MaintenanceEntry[] memory maintenanceLog)
    {
        return maintenance;
    }
}
