...
/**
        cohortContract = ROM contract
        deployer = deployer address
        implementation = implementation ID

        membershipCriteria = BaalV3 DAO
        stakingAsset = token to stake for onboarding
        treasury = BaalV3 Avatar / safe
        threshold = minimum shares of BaalV3 for membership
        assetAmount = mimimum stakingAsset to stake
        stakeDuration
 */
    ...
    RiteOfMolochUtilities
{
    ...
    function initialize(
        ...
        bytes memory creatHatData;
        bytes memory mintHatData;
        ...
        if (baalHasTopHat) {
            ...
            creatHatData = _encodeHatProposal(caller_);
        } else {
    ...
    function _createAdminHat() public returns (uint256) {
    ...
    // function _encodeMintHatProposal(uint256 _adminHat, address _admin)
    //     internal
    //     pure
    //     returns (bytes memory)
    // {
    ...
    // from baal
    function createAdminHat(address deployer) public {
        (bool success, ) = address(HATS).call(
            abi.encodeWithSignature(
                "createHat(uint256,string,uint32,address,address,bool,string)",
                topHat,
                "ROM Admin",
        ...
        // HATS.mintHat(adminHat, deployer);
    }

    function _encodeHatProposal(address deployer)
        internal
        view
        returns (bytes memory)
    {
        return abi.encodeWithSignature("createAdminHat(address)", deployer);
    }
}
