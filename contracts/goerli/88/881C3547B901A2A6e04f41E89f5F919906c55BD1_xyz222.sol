...
contract xyz222 is Ownable, ERC721A, ReentrancyGuard, VRFConsumerBaseV2 {
    //chainlink VRF
    ...
    uint256 public finalizedJackpot;
    ...
    // quantity of outfit and headgear traits (traits get zeroed out with elimination)
    ...
    bool public _marketplaceActive = false;
    // minting has ended & game has started
    ...
    function setGameHasStarted(bool isActive)
        external
        onlyOwner
    {
    ...
    function setGameEnded(uint256 num, bool ended) external onlyOwner {
        // numWinners can be read from the contract, however it must be set manually here due to memory caps for functions
        numWinners = num;
        gameEnded = ended;
        finalizedJackpot = jackpot;
    }

    function setMarketplaceActive(bool isActive) external onlyOwner {
        _marketplaceActive = isActive;
    }
    ...
    // getters
    function getCost(uint256 traitId) public view returns (uint256) {
        if (traitAmountCounter[traitId] > 0){
            return 2.5 ether / traitAmountCounter[traitId];
        }
        else {
            return 0;
        }
    ...
    function getPurchaseTraitQuantities() public view returns (uint256[] memory) {
    ...
    function getAllTokenInfo (uint256 tokenId) public view returns (string memory) { 
        return
            string(
                abi.encodePacked(
                    "{'headgear':",
                    ...
                    ",'headgearElim':",
                    traitIsEliminated[headgearForToken[tokenId]] ? "true" : "false",
                    ",'outfit':",
                    Strings.toString(outfitForToken[tokenId]),
                    ",'outfitElim':",
                    traitIsEliminated[outfitForToken[tokenId]]  ? "true" : "false",
                    ",'completelyElim':",
    ...
    // // one of your traits is eliminated
    // function needsToUpgrade(uint256 tokenId) public view returns (bool) {
    //     if (isEliminated(tokenId)) {
    //         return false;
    //     } else {
    //         if (
    //             traitIsEliminated[headgearForToken[tokenId]] == true &&
    //             traitIsEliminated[outfitForToken[tokenId]] == false
    //         ) {
    //             return true;
    //         } else if (
    //             traitIsEliminated[headgearForToken[tokenId]] == false &&
    //             traitIsEliminated[outfitForToken[tokenId]] == true
    //         ) {
    //             return true;
    //         } else {
    //             return false;
    ...
    function eliminateTrait() external onlyOwner returns (uint256) {
        ...
        // // pick a random index to start at, then loop through until we land on the random number counting as we go along
        ...
        nonReentrant
    {
        ...
        require(msg.value == getCost(traitId), "Incorrect amount sent");
    ...
    function claimPrize(uint256 tokenId) external nonReentrant {
        ...
        uint256 prize = finalizedJackpot / numWinners;
