...
contract MetroGradGen2 is ERC721A, Ownable, ReentrancyGuard, DefaultOperatorFilterer {
    ...
    error NotTheCorrectMintingWallet();
    ...
    uint256 public reserved = 533;
    uint256 public maxSupplyForPresale = 2800;
    uint256 public reservedCounter = 0;
    uint256 private forTeam = 50;

    uint16 public maxSupply = 3333;

    uint8 public maxMintAmount = 2;

    string private _baseTokenURI = "ipfs://X/";

    bool public reservedClaimed;
    bool public presaleActive;
    bool public publicSaleActive;
    bool public claimedAll;
    ...
    function toggleReserveClaimed() external onlyOwner {
        reservedClaimed = !reservedClaimed;
    }
    ...
    function claimAllRemainingFree() external onlyOwner {
        ...
        uint256 claim = reserved - reservedCounter;
           if (totalSupply() + claim > maxSupply)
        ...
        callerIsUser
    {
        ...
        if (totalSupply() + _tokensToClaim.length > maxSupply - forTeam)
            revert MaxSupplyExceeded();
        for (uint256 i = 0; i < _tokensToClaim.length; i++) {
            if (_idClaimed[_tokensToClaim[i]] == true)
                revert NotAllowedToClaim();
            //    if (!staked.isTokenIdStaked(_tokensToClaim[i])) revert NotAllowedToClaim();
            if (
    ...
    function freeClaim() external callerIsUser {
        ...
        if (1 + totalSupply() > maxSupply - forTeam) revert MaxSupplyExceeded();
        reservedCounter += 1;
        _mint(msg.sender, 1);
    }
    ...
    function withdraw() external onlyOwner nonReentrant {
        ...
        uint256 payoutDev = balance.mul(5 * 1 ether).div(100 * 1 ether);
        ...
        payable(0xebE202b911d481936D1D73537D78959Ef5566969).transfer(payoutI);
