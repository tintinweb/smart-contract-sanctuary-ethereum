...
contract BooWhoBabies is ERC721, Ownable, ERC721Burnable, ERC721Pausable {
    ...
    function whitelistMint(uint256 _count, address _recipient, bytes32[] calldata _proof) public payable saleIsOpen {
        uint256 total = totalSupply();
        require(privateSaleOpen, "Private sale not open at this time.");
        require(msg.value == privatePrice * _count, "Value is over or under price.");
    ...
    function mint(uint256 _count, address _recipient) public payable saleIsOpen {
        ...
        require(msg.value == publicPrice * _count, "Value is over or under price.");
        require(total + _count <= max_elements, "Max limit");
        require(_count <= maxMintPerTransaction, "Max limit per transaction.");
