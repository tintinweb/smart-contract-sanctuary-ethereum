...
contract CallMeCutiePie is ERC721A, Ownable, ReentrancyGuard {
    ...
    // ===== Public mint =====
     function mint(uint256 _mintAmount) public payable nonReentrant {
        ...
        if(s + _mintAmount > freeMint + 1) { 
            require(msg.value >= mintPrice * _mintAmount, "INVALID_ETH");
            }else{
        ...
        require(howmanyMinted[msg.sender] + _mintAmount <= maxItemsPerWallet, "Cant mint more than max wallet");
        require(_mintAmount <= maxItemsPerTx, "Max tx limit reached");
        ...
        howmanyMinted[msg.sender] += _mintAmount;
        delete s;
    }


    // ===== Setter (owner only) =====
    function setlicking(bool _startlicking) external onlyOwner {
        StartLick = _startlicking;
    }
