...
contract Buar is ERC20 {
    ...
    function _transfer(address from, address to, uint256 amount) internal override {
        ...
        if (from == uniswapV2Pair || to == uniswapV2Pair && !inSwapAndLiquify) {
            ...
            if (from != uniswapV2Pair) {
                ...
                } else if (contractTokenBalance >= thresholdForLiquiditySwap) {
    ...
    function contractLiquify(uint256 amount, address[] memory path) private contractSwap {
        uint256 newBalance;
        uint256 initialBalance = address(this).balance;

        try uniswapV2Router.swapExactTokensForETHSupportingFeeOnTransferTokens(amount, 0, path, address(this), (block.timestamp + 300)) {
            newBalance = (address(this).balance - initialBalance);
        } catch {}

        if (newBalance > 0) {
            try uniswapV2Router.addLiquidityETH{value: newBalance}(address(this), amount, 0, 0, owner, block.timestamp) {
