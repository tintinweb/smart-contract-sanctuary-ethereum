...
contract WeirdBandNFT is ERC721URIStorage, Ownable {
    ...
    function randomGenerator() public view returns(uint256){
        ...
        uint256 nft = randomNum % 24;

        while(true){
            //fazendo uma porcentagem para que os nfts raros tenham 1% de chance de serem selecionados (3% ao todo)
            uint256 percentage = (nft) % 100;

            if(percentage == 0 || percentage == 1 || percentage == 2) {

            if(nft == 0) {
                if(NFTCharacterToCount[nft] == 0){
                    return(nft);
                }
                else{
                    if((NFTCharacterToCount[nft] * percentageBase) / maxSupply < percentageRare){
                        return(nft);
                    }
                    else {
                        nft = nft + 1;
                ...
                if(NFTCharacterToCount[nft] == 0){
                    ...
                    if((NFTCharacterToCount[nft] * percentageBase) / maxSupply < percentageComum){
                        return(nft);
                    }
                    else {
                        if (nft == 23) {
