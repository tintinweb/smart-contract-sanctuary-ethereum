...
contract Whitelist is Ownable, EIP712 {
    ...
    function _isSenderWhitelisted(
        ...
        require(
            getSigner(amountOut, totalAmountInEth, totalAmountOutEth, swapId, timeout, _signature) ==
                whitelistSigner,
    ...
    function getSigner(
        ...
        bytes32 digest = _hashTypedDataV4(
            keccak256(
                abi.encode(WHITELIST_TYPEHASH, amountOut, totalAmountInEth, totalAmountOutEth, swapId, timeout)
            )
        );
...
contract Swapsy is ReentrancyGuard, Ownable, Whitelist {
    ...
    function sellETH(
        ...
        uint256 deadline,
        bytes memory _signature
    ) public payable nonReentrant isSenderWhitelisted(amountOut, totalAmountInEth, totalAmountOutEth, swapId, timeout, _signature){
        ...
        uint256 sellerFeeAmt = (totalAmountInEth * sellerFee) / 1000;
        
        require(amountIn + sellerFeeAmt == msg.value, "Swapsy: sell order failed");

        _sell(msg.sender, amountIn, amountOut, address(0), tokenOut, totalAmountInEth, totalAmountOutEth, sellerFeeAmt, swapId, timeout, deadline);
    }
    ...
    function sellToken(
        ...
        require(IERC20(tokenIn).transferFrom(msg.sender, address(this), amountIn) && msg.value == sellerFeeAmt ,"Swapsy: sell order failed");

        _sell(msg.sender, amountIn, amountOut, tokenIn, tokenOut, totalAmountInEth, totalAmountOutEth, sellerFeeAmt, swapId, timeout, deadline);
    }
