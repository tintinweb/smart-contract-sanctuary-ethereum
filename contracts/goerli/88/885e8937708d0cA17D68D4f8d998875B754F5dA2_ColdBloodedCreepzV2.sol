 ...
 * {ERC721Enumerable}.
 */
contract ColdBloodedCreepzV2 is
  SoftStaking,
  DefaultOperatorFiltererUpgradeable
{
  ...
  uint256 private lastBatchNumber;

  //@TODO: update batch size
  uint256 private constant batchSize = 5;

  // event TokensMinted(uint256 indexed batchNumber);
   ...
   */
  function initialize(address _signerAddress, string memory _baseURI)
    external
    initializer
  {
  ...
  function _validateSignature(
    ...
    bytes32 dataHash = keccak256(abi.encodePacked(caller, tokenIds));
   ...
   */
  function airdrop(address[] calldata recipients, uint256 batchNumber)
    external
    onlyOwner
  {
    require(recipients.length == batchSize, "Batch size must be equal to 70");
    require(
      batchNumber == lastBatchNumber + 1,
      "Batch number must be greater than last batch number"
    );

    for (uint256 i = 0; i < recipients.length; i++) {
      uint256 tokenId = (batchNumber - 1) * batchSize + i;
  ...
  //@TODO: check this title with Joe
  /**
   ...
   */
  function claim(uint256[] calldata tokenIds, bytes calldata signature)
    external
  {
    require(
      _validateSignature(signature, tokenIds, msg.sender),
      "Invalid signature"
    );
  ...
  /**
   * @dev Sets provenance hash from CreepzV1.
   */
  ...
  /**
   * @dev Withdraws all funds from the contract and sends them to the owner.
   */
  ...
  */
  function _beforeTokenTransfer(
    ...
    require(
      stakedAtTimestamp[tokenId] == 0 || canTransferWhikeStaked,
      "Token is staked"
    );
    if (from != address(0)) {
...
abstract contract SoftStaking is
  ERC721Upgradeable,
  ...
  OwnableUpgradeable
{
  ...
  event ClaimStake(address indexed staker, uint256 stakeAmounts);

  bool public canTransferWhikeStaked;
  ...
  */
  ...
  /**
  @dev tokenId to total accumulated stake. This is the total amount of stake that has been accumulated by the token.
  */
  mapping(uint256 => uint256) public totalAccumulatedStake;
  ...
  function stake(uint256[] calldata tokenIds) public nonReentrant {
    require(tokenIds.length > 0, "No tokens passed");
    for (uint256 i = 0; i < tokenIds.length; i++) {
      uint256 tokenId = tokenIds[i];
      require(ownerOf(tokenId) == _msgSender(), "Not owner of token");
      require(stakedAtTimestamp[tokenId] == 0, "Token already staked");
      stakedAtTimestamp[tokenId] = block.timestamp;
      emit Stake(tokenId);
    }
  }
  ...
  function unstake(uint256[] calldata tokenIds) external nonReentrant {
    ...
    for (uint256 i = 0; i < tokenIds.length; i++) {
      uint256 tokenId = tokenIds[i];
      require(ownerOf(tokenId) == _msgSender(), "Not owner of token");
      require(stakedAtTimestamp[tokenId] != 0, "Token not staked");
      uint256 currentStake = block.timestamp - stakedAtTimestamp[tokenId];
      stakedAtTimestamp[tokenId] = 0;
      totalAccumulatedStake[tokenId] += currentStake;
      emit Unstake(tokenId);
    }
  }

  /**
   * @dev public function to calculate the stake of a token.
   * @param tokenId tokenId to calculate stake for.
   */
  function calculateStake(uint256 tokenId)
    public
    view
    returns (uint256 totalAccumulated)
  {
    uint256 currentStake = block.timestamp - stakedAtTimestamp[tokenId];
    return totalAccumulatedStake[tokenId] + currentStake;
  }
    ...
    returns (uint256 totalAccumulated)
  {
    for (uint256 i = 0; i < tokenIds.length; i++) {
      uint256 tokenId = tokenIds[i];
      totalAccumulated += calculateStake(tokenId);
    }
  }

  // @TODO: finalise this function logic
  /**
   * @dev public function to claim stake of a token.
   * @param tokenIds array of tokenIds to claim stake for.
   */
  function claimStake(uint256[] calldata tokenIds) external nonReentrant {
    require(tokenIds.length > 0, "No tokens passed");
    uint256 totalStake;
    ...
    for (uint256 i = 0; i < tokenIds.length; i++) {
      ...
      require(ownerOf(tokenId) == _msgSender(), "Not owner of token");
      require(stakedAtTimestamp[tokenId] != 0, "Token not staked");
      totalStake += calculateStake(tokenId);
      totalAccumulatedStake[tokenId] = 0;
      stakedAtTimestamp[tokenId] = block.timestamp;
    }

    emit ClaimStake(_msgSender(), totalStake);
  }
