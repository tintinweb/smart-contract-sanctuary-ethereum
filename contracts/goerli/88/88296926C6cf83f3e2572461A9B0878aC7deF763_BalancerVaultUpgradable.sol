...
contract BalancerVaultUpgradable is Initializable, OwnableUpgradeable, ReentrancyGuardUpgradeable {
  ...
  function deposit(address token, uint256 amount) public nonReentrant {
    ...
    if(token == address(DUSD) || token == address(DAI)) { // When user deposit DAI or DUSD
      ...
      if(depositBalances[msg.sender].depositToken != address(0)) {
        require(depositBalances[msg.sender].depositToken == token, "You already deposited with other token");
      }

      IERC20Upgradeable(token).safeTransferFrom(msg.sender, address(this), amount);
      IERC20Upgradeable(token).safeApprove(address(balancerVault), amount);
      ...
      depositBalances[msg.sender].poolBalance = depositBalances[msg.sender].poolBalance + DUSDDAI_POOL.balanceOf(address(this)) - originalBalance;
    }
      ...
      uint256 tokenIndex = depositBalances[msg.sender].depositToken == address(DAI) ? 0 : 1;
      uint256 tokenWithdrawalAmount = balancerQueryExit(tokens, minAmountsOut, balancerPoolId, amount, tokenIndex); // token index : 0 - dai, 1 - dusd
  ...
  function withdraw(address token) public nonReentrant returns(uint256) {
    ...
    DUSDDAI_POOL.safeApprove(address(balancerVault), depositBalances[msg.sender].poolBalance);
    ...
    if(amountWithdraw > originalDepositAmount) {
      ...
      IERC20Upgradeable(depositToken).safeTransfer(fluidTreasury, reward);
      amountWithdraw = amountWithdraw - reward;
    }

    if(token == depositToken) {
      IERC20Upgradeable(token).safeTransfer(msg.sender, amountWithdraw);
    }
    else {
      amountWithdraw = balancerSwap(amountWithdraw, depositToken, token, msg.sender, balancerPoolId);
    }
