...
contract NounsVisionVRFDistributor is VRFv2Consumer {
    ...
    **/
    ...
    function fulfillRandomWords(
        ...
        if (availGlasses == 0) {
          revert AvailGlassesZero(); // Either no glasses allocated or preprocess function getStartIdAndBatchAmount hasn't been run
        }
        ...
        NOUNS_VISION_BATCH_TRANSFER.sendManyGlasses(startId, winners);
        startId = 0;
        availGlasses = 0;
    ...
    **/
    ...
    /// @notice Is used to preprocess and load state variables startId and availGlasses before randomness is requested to keep gas usage of the VRFCoordinator call to fullfillRandomWords at a minimum
    function getStartIdAndBatchAmount() external {
        // Determine which glasses are available to be distributed
        (startId, availGlasses) = NOUNS_VISION_BATCH_TRANSFER.getStartIdAndBatchAmount(address(this));
    }
