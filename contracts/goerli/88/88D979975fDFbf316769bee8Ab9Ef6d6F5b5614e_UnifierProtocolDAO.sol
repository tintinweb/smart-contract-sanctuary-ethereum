...
import "Clones.sol";
import "ReviewerGroup.sol";
import "ReviewerVotes.sol";
import "ReviewerTimeLock.sol";

contract UnifierProtocolDAO is Governor, GovernorSettings, GovernorCountingSimple, GovernorVotes, GovernorTimelockControl {

    event NewReviewerGroup(string indexed name, address indexed DAOaddress, address DAOtoken, address DAOtimelock);
    ...
    function newReviewerGroup(string memory name, address DataCollectionTemplate) public {
        ReviewerVotes _token =  ReviewerVotes(Clones.clone(SubDAOTokenTemplate));
        _token.initialize(name);
        _token.transferOwnership(timelock());
        ReviewerTimeLock _timelock = ReviewerTimeLock(payable(Clones.clone(TimeLockTemplate)));
        ReviewerGroup reviwerGroup = ReviewerGroup(payable(Clones.clone(SubDAOTemplate)));
        _timelock.initialize(address(reviwerGroup));
        reviwerGroup.initialize(name, _token, _timelock, DataCollectionTemplate);
        emit NewReviewerGroup(name, address(reviwerGroup), address(_token), address(_timelock));
    }
...
import "Clones.sol";
import "DataCollection.sol";

contract ReviewerGroup is Initializable, GovernorUpgradeable, GovernorSettingsUpgradeable, GovernorCountingSimpleUpgradeable, GovernorVotesUpgradeable, GovernorTimelockControlUpgradeable {

    event NewDataCollection(address indexed dataCollectionAddress);

    address public DataCollectionTemplate;
    ...
    function initialize(string memory name, IVotesUpgradeable _token, TimelockControllerUpgradeable _timelock, address _DataCollectionTemplate)
        initializer public
    {
        __Governor_init(name);
        __GovernorSettings_init(1 /* 1 block */, 10 /* 1 week */, 0);
        __GovernorCountingSimple_init();
        __GovernorVotes_init(_token);
        __GovernorTimelockControl_init(_timelock);
        DataCollectionTemplate = _DataCollectionTemplate;
    }

    function addDataCollection() public{
        DataCollection dataCollection =  DataCollection(Clones.clone(DataCollectionTemplate));
        dataCollection.initialize();
        dataCollection.transferOwnership(timelock());
        emit NewDataCollection(address(dataCollection));
    }
...
contract DataCollection is Initializable, OwnableUpgradeable {
    string[] public data;
    ...
    function store(string memory _data) public onlyOwner {
        data.push(_data);
    }
}
...
contract ReviewerVotes is Initializable, ERC20Upgradeable, OwnableUpgradeable, ERC20PermitUpgradeable, ERC20VotesUpgradeable {
    /// @custom:oz-upgrades-unsafe-allow constructor
    constructor() {
        _disableInitializers();
    }
    ...
    function addNewReviewer(address to) public onlyOwner {
        _mint(to, 1);
    }
