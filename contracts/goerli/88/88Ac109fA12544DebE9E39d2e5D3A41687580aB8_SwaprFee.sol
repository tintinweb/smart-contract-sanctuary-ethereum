...
contract SwaprFee is BaseGovernanceUpgradeable {
    ...
    function _getBaseFee(FeePricing memory feePricing, uint subjectAmount, address token) internal view returns(uint totalBaseFee, uint percBaseFee, uint fixBaseFee) {
        ...
        uint fixedBaseFee = baseRecount * _getFeedDecimals(oracleFeedForToken[token]) / DPS;

        if(feePricing.isFixedBaseFee && feePricing.isPercentBaseFee){
            totalBaseFee = fixedBaseFee + percentBaseFee;
        }
        if(feePricing.isFixedBaseFee && !feePricing.isPercentBaseFee){
            fixBaseFee = fixedBaseFee;
        }
        if(!feePricing.isFixedBaseFee && feePricing.isPercentBaseFee){
            percBaseFee = percentBaseFee;
        }
