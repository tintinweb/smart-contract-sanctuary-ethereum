...
contract RsTokenICO is Initializable, ReentrancyGuardUpgradeable, PausableUpgradeable, OwnableUpgradeable {
    ...
    enum Status {  //0x4c341307f0fc34Ec972a932C3F43AF95d1e87296 deploy on bnb
        Register,
        CloseRegister,
        Purchase,
        ClosePurchase,
    ...
    IERC20Upgradeable public RsToken;
    ...
    function registerForWhitelist(uint256 _usdAmount) external onlyRegisterTime {
    ...
    function purchase(uint256 _usdAmount, address _receiver) external onlyPurchaseTime {
        address userAddress = msg.sender;
        //require(userAddress, "has not won whitelist");
    ...
    function withdraw() external onlyAfterPurchaseTime {
        address userAddress = msg.sender;
    ...
    function distributeWhitelist(address _receiver, uint256 _amount) external onlyOwner {
        require(_receiver != address(0) && _amount != 0, "invalid distribution");
        RsToken.safeTransfer(_receiver, _amount);
        emit Distributed(_receiver, _amount);
    }


    function updateStatusTime(uint64 _registerTime, uint64 _closeRegisterTime, uint64 _purchaseTime, uint64 _closePurchaseTime, uint256 _distributeTime) external onlyOwner {
        require(_distributeTime > _closePurchaseTime && _closePurchaseTime > _purchaseTime && _purchaseTime > _closeRegisterTime && _closeRegisterTime > _registerTime, "invalid time");
    ...
    function updateRsTokenAddress(address _RsToken_address) external onlyOwner {
        RsToken = IERC20Upgradeable(_RsToken_address);
    }


    function getDepositedAmount(address _userAddress) public view returns (uint256) {
        return userInfos[_userAddress].depositedAmount;
    }

    // get remaining amount that user can deposit to reach maximum deposit amount
    function getRemainingDepositAmount(address _userAddress) public view returns (uint256) {
