...
/// @title Mystical Menagerie Horned Exhibit
/// @dev TheRatKing
...
contract MysticalMenagerie is ERC721A, Ownable {
    ...
    address public mysticalMenagerieHEV1;
    uint256 supply = totalSupply();

    mapping(address => uint) public addressMintedBalance;
    mapping(address => uint) public addressMintedBalanceWL;
    mapping(address => uint) public addressMintedBalanceOG;
    mapping(address => string) public merkleProofWL;
    mapping(address => string) public merkleProofOG;
    

 constructor(
    string memory _baseURI,
    address _lead,
    address _mysticalMenagerieHEV1
  )ERC721A("Mystical Menagerie Horned Exhibit", "MMHE") {
    baseURI = _baseURI;
    lead = _lead;
    mysticalMenagerieHEV1 = _mysticalMenagerieHEV1;
  }
 ...
 function _startTokenId() internal view override virtual returns (uint256) {
  ...
  function setMysticalMenagerieHEV1Address(address _mysticalMenagerieHEV1) public onlyOwner {
    mysticalMenagerieHEV1 =_mysticalMenagerieHEV1;
  }
  ...
  function setMerkleRootWL(bytes32 _merkleRootWL) public onlyOwner {
    merkleRootWL = _merkleRootWL;
}

  function setMerkleRootOG(bytes32 _merkleRootOG) public onlyOwner {
    ...
    callerIsUser() 
  {
   ...
   function whitelistMint(bytes32[] calldata merkleProof, uint256 _quantitty)
    public 
    whitelistnotPaused() 
    callerIsUser() 
  {
    ...
    require(MerkleProof.verify(merkleProof, merkleRootWL, keccak256(abi.encodePacked(msg.sender))), "invalid merkleProof");
    addressMintedBalanceWL[msg.sender] += _quantitty;

     _safeMint(msg.sender, _quantitty);
  }

  function claimMint(bytes32[] calldata merkleProof, uint256 _quanttity)
    public 
    claimnotPaused() 
    callerIsUser() 
  {
    ...
    require(MerkleProof.verify(merkleProof, merkleRootOG, keccak256(abi.encodePacked(msg.sender))), "invalid merkleProof");
    addressMintedBalanceOG[msg.sender] += _quanttity;

     _safeMint(msg.sender, _quanttity);
  }

  function checkPeriod() public view returns (string memory) {
    if (publicPaused == false) {
      return "Public";
    } else if (whitelistPaused == false) {
      return "Whitelist";
    } else if (claimPaused == false) {
      return "Claim";
    } else {
      return "Paused";
    }
