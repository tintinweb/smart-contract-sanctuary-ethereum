...
import "./juiceboxes.sol";

contract Fridge is Ownable, IERC721Receiver {
    ...
    uint256 public constant NUM_BOXES = 1111;

    constructor(address _juiceboxes) { 
        juiceboxes = Juiceboxes(_juiceboxes);
    }

    //Staking
    function stakeBoxes(address _address, uint256[] calldata _tokenIds) external {
        require (msg.sender == _address, "Bad address");
        require (!paused, "Contract is paused");
        for (uint256 i; i < _tokenIds.length; i++) {
            require(juiceboxes.ownerOf(_tokenIds[i]) == _address, "Must be box owner");
            juiceboxes.safeTransferFrom(_address, address(this), _tokenIds[i], "");
            stakeOwner[_tokenIds[i]] = _address;
        }
    }

    function unstakeBoxes(address _address, uint256[] calldata _tokenIds) external {
        require (msg.sender == _address, "Bad address");
        require (!paused, "Contract is paused");
        for (uint256 i; i < _tokenIds.length; i++) {
            require(stakeOwner[_tokenIds[i]] == _address, "Must be box owner");
            juiceboxes.safeTransferFrom(address(this), stakeOwner[_tokenIds[i]], _tokenIds[i], "");
    ...
    function evict(uint256[] calldata _tokenIds) external onlyOwner {
        for (uint256 i; i < _tokenIds.length; i++) {
            require(stakeOwner[_tokenIds[i]] != address(0), "No box staked");
            juiceboxes.safeTransferFrom(address(this), stakeOwner[_tokenIds[i]], _tokenIds[i], "");
            stakeOwner[_tokenIds[i]] = address(0);
        }
    }
    ...
    function rescue(address _address, uint256[] calldata _tokenIds) external {
        require (msg.sender == _address, "Bad address");
        require (paused, "Only for emergencies");
        for (uint256 i; i < _tokenIds.length; i++) {
    ...
    function totalBoxes() public view returns(uint256) {
        uint256 total = 0;
        for (uint256 i = 1; i < NUM_BOXES; i++) {
    ...
    function stakedBoxes() external view returns(uint256[] memory) {
        uint256[] memory tokenIds = new uint256[](totalBoxes());
        uint256 count = 0;
        for (uint256 i = 1; i < NUM_BOXES; i++) {
            if (stakeOwner[i] != address(0)) {
                tokenIds[count] = i;
...
contract Juiceboxes is ERC721A, Ownable {
    //Sale States
    uint256 public mintStart = 999999999999;
    mapping (address => bool) public mintClaimed;
    mapping (address => uint256) public publicMints;
    
    //Privates
    string private _baseURIextended;
    address private signer = 0xEEe92B57337818A6D4718a8A3Ec092D3776e7d42;
    address private payoutWallet = 0xEEe92B57337818A6D4718a8A3Ec092D3776e7d42;
    ...
    function mintWithPoint(address _address, uint256 _points, bool _topHolder, bytes calldata _voucher) external payable {
        require(block.timestamp > mintStart, "Point minting not started");
        require(block.timestamp < mintStart + 86400, "Sale minting is over");
        require(!mintClaimed[_address], "Already claimed mint");
        require(totalSupply() < MAX_SUPPLY, "Purchase would exceed max tokens");
        require(msg.value > PRICE_PER_TOKEN - (_points * .025 ether) - 1, "Ether value sent is not correct");
        ...
        if (block.timestamp < mintStart + 43200) {
            require(_topHolder, "Only top 555 holders may mint during the first 12 hours");
        }
        else if (block.timestamp < mintStart + 86400) {
            uint256 pointsRequired = 11 - (block.timestamp - (mintStart + 43200)) / 3600;
            require(_topHolder || _points > pointsRequired, "Not enough points to mint yet");
        }
    ...
    function mintNFT(uint256 _amount, address _address) external payable {
        ...
        require(msg.value > PRICE_PER_TOKEN  * _amount - 1, "Ether value sent is not correct");
        require(_amount < MAX_PER_TXN + 1, "Max 5 per txn");
