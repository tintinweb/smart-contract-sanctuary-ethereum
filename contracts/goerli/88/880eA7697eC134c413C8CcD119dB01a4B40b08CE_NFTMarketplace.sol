...
contract NFTMarketplace is ERC721URIStorage {
    ...
    function buyNFT(uint256 tokenId) public payable {
        ...
        idToToken[tokenId].owner = payable(buyer);
    ...
    function putUpForSale(uint256 tokenId) public payable {
        address seller = ERC721.ownerOf(tokenId);
        require(msg.sender == seller, "You must be owner to do this action");

        // NEW
        // setApprovalForAll(msg.sender, true); approve to caller
         approve(address(this), tokenId); //  approve to current owner
         transferFrom(seller, address(this), tokenId);
        // safeTransferFrom(owner, address(this), tokenId); // transfer to non ERC721Receiver implementer

        // update the details of the token
        idToToken[tokenId].owner = payable(address(this));
        idToToken[tokenId].isForSale = true;
    ...
    function cancelSale(uint256 tokenId) public {
        address owner = ERC721.ownerOf(tokenId);
        require(msg.sender == owner, "You must be owner to do this action");
        idToToken[tokenId].isForSale = false;
    }
    ...
    function getLatestIdToToken() public view returns (Token memory) {
        uint256 currentTokenId = _tokenIds.current();
    ...
    function getOwner(uint256 tokenId) public view returns (address) {
        return payable(ERC721.ownerOf(tokenId));
    }
