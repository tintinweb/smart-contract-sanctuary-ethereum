 ...
 */
contract PledgeContributionCollector is
    IPledgeContributionCollector,
    Initializable
{
    ...
    mapping(address => Contributor) internal contributor;
    ...
    function updateCap(uint256 _newCap) external override {
        if (msg.sender != owner) revert Forbidden();
        if (state != State.ACTIVE) revert InactiveContribution();
