...
interface IVRFv2Consumer {
  function requestRandomWords(uint256 numWords) external returns (uint256); // call every time, the VRF Consumer will determine whether to formally request the randomWords
...
contract MetawinRaffle {
    ...
    function cancelNftRaffle(
        ...
        require(
            nftRaffles[_raffleId].totalPrice == 0, "There are participants!" 
        );
    ...
    function cancelEthRaffle(uint256 _raffleId) onlyOwner public {
        ...
        require(
            ethRaffles[_raffleId].totalPrice == 0, "There are participants" 
        );
    ...
    function chooseNftWinner(uint256 _raffleId) public returns (uint256) {
        ...
        require(uint256(raffle.status) == uint256(RaffleStatus.ONGOING), "Winner is already chosen");

        uint32 _numWords = 1;
        uint256 requestId = vrfConsumer.requestRandomWords(_numWords);
    ...
    function chooseEthWinner(uint256 _raffleId) public returns (uint256) {
        ...
        for(uint32 i = 0; i < raffle.numWinner; i++) {
            requestId = vrfConsumer.requestRandomWords(ethRaffles[_raffleId].numWinner);

            winnerIdx = requestId % raffle.tickets.length;
            raffle.winnerIndex[i] = winnerIdx;

            (bool sent, bytes memory data) = raffle.tickets[winnerIdx].call{value: (raffle.rewardEth.div(raffle.numWinner))}("");
            require(sent, "Failed to send Ether");
        }

        if(raffle.totalPrice > raffle.rewardEth) {
            (bool sent, bytes memory rewardData) = raffle.creator.call{value: (raffle.totalPrice - raffle.rewardEth)}("");
            require(sent, "Failed to send Ether");
        }
    ...
    event EthRaffleCompleted(uint256 id, uint256[] winnerIndex);
}
