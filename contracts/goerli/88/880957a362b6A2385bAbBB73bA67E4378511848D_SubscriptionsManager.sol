...
contract SubscriptionsManager is Ownable {
  ...
  /// @notice EEvent emited when a user subscribes to a SuperSubscription
  ...
  function _subscribe(address to, SubType subType) internal {
    require(isSubscribed(msg.sender) == false, 'ALREADY_SUBSCRIBED');
    require(uint256(subType) <= 1, 'INVALID_SUB_TYPE');
  ...
  function subscribe(
    ...
    bytes32 inputHash = keccak256(
      abi.encodePacked(msg.sender, address(this), uint256(subType), msg.value, timestamp)
    );
  ...
  function isSubscribed(address target) public view returns (bool) {
    return
  ...
  function prices() external view returns(Info memory) {
    uint256 NORMAL_PRICE =  ISubscriptions(S_ADDRESS).price();
    uint256 SUPER_PRICE =  ISubscriptions(SS_ADDRESS).price();
    Info memory _info =  Info(NORMAL_PRICE,SUPER_PRICE);
    return _info;
  }
