    ...
    Ownable2StepUpgradeable
{
    ...
    error PrincipalTokenFactoryError(string reason);
     ...
     * @param _principalTokenBeaconUpgradable The address of futurevault beacon.
     * @param _ytBeaconUpgradable The address of yt beacon.
     */
    constructor(address _principalTokenBeaconUpgradable,
        address _ytBeaconUpgradable) {
        if ((_principalTokenBeaconUpgradable == address(0))) {
            revert PrincipalTokenFactoryError(
                "PrincipalToken beacon address should not be zero"
            );
        }
        if ((_ytBeaconUpgradable == address(0))) {
            revert PrincipalTokenFactoryError(
                "YT beacon address should not be zero"
            );
        }
    ...
    {
        if ((principalTokenBeaconUpgradable == address(0))) {
            revert PrincipalTokenFactoryError(
                "PrincipalToken beacon is not set"
            );
        }
    ...
    * @param curveAddressProvider_ The address of the curveAddressProvider.
    ...
    function setCurveAddressProvider(
        ...
        if ((curveAddressProvider_ == address(0))) {
            revert PrincipalTokenFactoryError(
                "Cannot set curve address provider to zero address"
            );
        }
    ...
    function setRegistry(address _newRegistry) public override {
        ...
        if ((_newRegistry == address(0))) {
            revert PrincipalTokenFactoryError(
                "Cannot set registry to zero address"
            );
        }
    ...
    function _deployCurvePool(
        ...
        if ((curveFactory == address(0))) {
            revert PrincipalTokenFactoryError(
                "Curve factory address is not set"
            );
        }
        ...
        if (!(success)) {
            revert PrincipalTokenFactoryError("Deployment failed");
        }
        assembly {
    ...
    * @notice Function which sets the curve factory address used in
    ...
    function _setCurveFactory() internal {
        ...
        if (!(success)) {
            revert PrincipalTokenFactoryError(
                "Could not fetch curve factory address from the address provider"
            );
        }
