...
error NFTMarketplace__Owner_Cannot_Buy_Its_Own_NFT(
...
contract NFTMarketplace is ReentrancyGuard {
    ...
    modifier notOwner(
        ...
        if (buyer == owner)
            revert NFTMarketplace__Owner_Cannot_Buy_Its_Own_NFT(NFTContractAddress, tokenId, buyer);
        ...
        isOwner(NFTContractAddress, tokenId, msg.sender)
    {
        ...
        if (NFT.getApproved(tokenId) != address(this))
            revert NFTMarketplace__Not_Approved_For_Marketplace();
    ...
    function buyItem(address NFTContractAddress, uint256 tokenId)
        ...
        notOwner(NFTContractAddress, tokenId, msg.sender)
        nonReentrant
    {
        Listing memory listedItems = s_listings[NFTContractAddress][tokenId];

        if (msg.value < listedItems.price)
            revert NFTMarketplace__Price_Not_Met(NFTContractAddress, tokenId, listedItems.price);
