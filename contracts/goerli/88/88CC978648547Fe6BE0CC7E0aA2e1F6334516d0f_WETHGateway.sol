...
interface IWETHGateway {
    /// @notice Refreshes the wrapped ethereum ERC20 approval for an refinery contract.
    ///
    /// @param refinery The address of the refinery to refresh the allowance for.
    function refreshAllowance(address refinery) external;

    /// @notice Takes ethereum, converts it to wrapped ethereum, and then deposits it into an refinery.
    ///
    /// See [IRefineryV1Actions.depositUnderlying](./refinery/IRefineryV1Actions.md#depositunderlying) for more details.
    ///
    /// @param refinery        The address of the refinery to deposit wrapped ethereum into.
    ...
    /// @notice Withdraws a wrapped ethereum based yield token from an refinery, converts it to ethereum, and then
    ///         transfers it to the recipient.
    ///
    /// A withdraw approval on the refinery is required for this call to succeed.
    ///
    /// See [IRefineryV1Actions.withdrawUnderlying](./refinery/IRefineryV1Actions.md#withdrawunderlying) for more details.
    ///
    /// @param refinery        The address of the refinery to withdraw wrapped ethereum from.
...
contract WETHGateway is IWETHGateway, Ownable {
    ...
    function refreshAllowance(address refinery) external onlyOwner {
        WETH.approve(refinery, type(uint256).max);
    }
    ...
    function depositUnderlying(
        ...
        IRefineryV1(refinery).depositUnderlying(yieldToken, amount, recipient, minimumAmountOut);
    }
    ...
    function withdrawUnderlying(
        ...
        IRefineryV1.YieldTokenParams memory params = IRefineryV1(refinery).getYieldTokenParameters(yieldToken);
        if (params.underlyingToken != address(WETH)) {
            revert IllegalArgument();
        }

        IRefineryV1(refinery).withdrawUnderlyingFrom(msg.sender, yieldToken, shares, address(this), minimumAmountOut);
