...
/**
 * @title Sponsor
 * @dev Users transfer the L1 cost of their ETH withdrawals to the sponsor and get back the remaining amount (if any)
 */

contract Sponsor {
    address public currentSponsor;
    IOutbox public immutable outbox;
    uint256 public constant maxPriorityFee = 2 gwei;

    // @param _outbox Address of the Outbox contract on L1
    ...
    /**
     * @notice Executes a messages in an Outbox entry and sets the sponsor
    ...
    function execute(
        ...
        /**
         * Here we set the address of the executer to the currentSponsor variable.
         * The user will pay to the currentSponsor in the next tx
         */
        currentSponsor = msg.sender;
    }

    /**
     * @dev Transfers the cost to the sponsor. The remaining ETH will be sent back to the user (if any)
     * @param _gasAmount amount of gas that sponsor needs for executing the transaction on L1
     */
    function payToSponsor(uint256 _gasAmount, uint256 _index) external payable {
        // Let's first check if the withdrawal transaction is executed.
        require(outbox.isSpent(_index), "Execute the withdrawal transaction first");
        // here we calculate the cost of execution that is paid by sponsor to execute the user's withdrawal tx.
        // We calculate the cost by multiplying the number of gas spent by sponsor (and provided by user as input)
        // to the sum of the block base fee and a hardcoded maxPriorityFee (2 Gwei)
        uint256 cost = _gasAmount * (block.basefee + maxPriorityFee);
        require(msg.value >= cost, "Fund is not enough");

        /** 
         * Sending the "cost" value of ETH to the sponsor's address which supposed 
         * to be set in the previous tx executed by sponsor

        */
        (bool success1, ) = currentSponsor.call{ value: cost }("");
        require(success1, "ETH not transferred to the Sponsor");

        /**
         * Sending the remained ETH back to the user
         * We used unchecked because there's no need to check
         * for integer underflow (We have a require checking that in this function)
         */
        unchecked {
            (bool success2, ) = msg.sender.call{ value: msg.value - cost }("");
            require(success2, "ETH not transferred to the user");
        }
    }
}
