...
contract IDOFactory is Ownable {
    ...
    function createIDO(IPool.ProjectInfo memory data) external {
        ...
        contractAddr = new Pool(data, msg.sender, adminAddr, adminPoint);
...
interface IPool {
    ...
    function queryProjectInfo() external view returns (ProjectInfo memory);

    function queryNum() external view returns (uint256);

    function advanceRelease() external ;

    function setFee(uint256 fee) external;

    function swap(uint256 _amountIn) external;

    function receiveUser() external;

    function queryUser() external view returns (LockUser[] memory); 
...
contract Pool is IPool,Ownable {
    ...
    function queryProjectInfo() external override view returns (ProjectInfo memory) {
        return Info;
    }

    function queryNum() external override view returns (uint256) {
        return num;
    }

    function advanceRelease() external override onlyOwner {
    ...
    function setFee(uint256 fee) external override onlyAdminSender {
        adminPoint = fee;
    }

    function swap(uint256 _amountIn) external override {
    ...
    function receiveUser() external override {
        require(
    ...
    function queryUser() external view override returns (LockUser[] memory) {
        //check user
    ...
    function receiveOwner() external override onlyOwner {
