...
contract BottzArmy is ERC721A, Ownable, AccessControl, ReentrancyGuard {
  ...
  uint256 public maxSupplyMajors = 900;
  uint256 public maxSupplyGenerals = 1000;
  ...
  bytes32 public captainsMerkleRoot;
  bytes32 public majorsMerkleRoot;
  bytes32 public generalsMerkleRoot;
  ...
  function captainsMint(uint256 _mintAmount, bytes32[] calldata _merkleProof)
    external
    payable
    mintCompliance(_mintAmount, maxSupplyCaptains)
  {
    uint256 minted = paidMinted[_msgSender()];
    bytes32 leaf = keccak256(abi.encodePacked(_msgSender()));

    require(captainsStarted, "Captains sale is paused.");
    require(msg.value >= (captainsPrice * _mintAmount), "Insufficient balance to mint.");
    require(
      (minted + _mintAmount) <= maxPerWalletPaid,
      "Selected number of mints will exceed the maximum amount of allowed per wallet."
    );
    require(
      MerkleProof.verify(_merkleProof, captainsMerkleRoot, leaf),
      "Invalid proof, this wallet is not allowed to mint Bottz Army Captain."
    );

    paidMinted[_msgSender()] = minted + _mintAmount;
  ...
  function majorsMint(uint256 _mintAmount, bytes32[] calldata _merkleProof)
    external
    payable
    mintCompliance(_mintAmount, maxSupplyMajors)
  {
    uint256 minted = paidMinted[_msgSender()];
    bytes32 leaf = keccak256(abi.encodePacked(_msgSender()));

    require(majorsStarted, "Majors sale is paused.");
    require(msg.value >= (majorsPrice * _mintAmount), "Insufficient balance to mint.");
    require(
      (minted + _mintAmount) <= maxPerWalletPaid,
      "Selected number of mints will exceed the maximum amount of allowed per wallet."
    );
    require(
      MerkleProof.verify(_merkleProof, majorsMerkleRoot, leaf),
      "Invalid proof, this wallet is not allowed to mint Bottz Army Major."
    );
  ...
  function generalsMint(uint256 _mintAmount, bytes32[] calldata _merkleProof)
    external
    payable
    mintCompliance(_mintAmount, maxSupplyGenerals)
  {
    uint256 minted = paidMinted[_msgSender()];
    bytes32 leaf = keccak256(abi.encodePacked(_msgSender()));

    require(generalsStarted, "Generals sale is paused.");
    require(msg.value >= (generalsPrice * _mintAmount), "Insufficient balance to mint.");
    require(
      (minted + _mintAmount) <= maxPerWalletPaid,
      "Selected number of mints will exceed the maximum amount of allowed per wallet."
    );
    require(
      MerkleProof.verify(_merkleProof, generalsMerkleRoot, leaf),
      "Invalid proof, this wallet is not allowed to mint Bottz Army General."
    );
  ...
  function privatesMint(uint256 _mintAmount) external payable mintCompliance(_mintAmount, maxSupplyTotal) {
  ...
  function updateCaptainsRoot(bytes32 _merkleRoot) external onlyRole(DEFAULT_ADMIN_ROLE) {
    captainsMerkleRoot = _merkleRoot;
  }

  function updateMajorsRoot(bytes32 _merkleRoot) external onlyRole(DEFAULT_ADMIN_ROLE) {
    majorsMerkleRoot = _merkleRoot;
  }

  function updateGeneralsRoot(bytes32 _merkleRoot) external onlyRole(DEFAULT_ADMIN_ROLE) {
    generalsMerkleRoot = _merkleRoot;
  }
