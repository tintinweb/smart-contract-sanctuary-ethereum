...
interface IERC1155Cappuccino is IERC1155 {
 ...
 */
contract ERC1155YuzuInternal is Ownable, AccessControl {
...
contract ERC1155Ikasumi is ERC1155YuzuInternal, ReentrancyGuard {
    ...
    // IERC1155Cappuccino token to be used for trade
    IERC1155Cappuccino public erc1155Token;
    ...
    constructor(
        IERC20 _paymentToken, IERC1155Cappuccino _erc1155Token, address _owner, address _admin, uint256 _commissionRate, 
    ...
    function sell(uint256 tokenId, uint256 qty, uint256 price, address wallet) external {
        ...
        // Limit the sell to owned items, the user cannot sell items do not have
        require(erc1155Token.balanceOf(_msgSender(), tokenId) >= qty, "Ikasumi: you do not have enough tokens to sell");
        require(erc1155Token.isApprovedForAll(_msgSender(), address(this)), "Ikasumi: you do not approve tokens to sell");
    ...
    function bid(uint256 tokenId, uint256 qty, uint256 price) external nonReentrant {
        ...
        require(qty <= erc1155Token.totalSupply(tokenId), "Iksasumi: not enough items for sale");
    ...
    function setYield(address from, uint256 tokenId, uint256 price) external nonReentrant {
        // validate user
        require(from != address(0), "Ikasumi: cannot zero address");
        require(tokenId > 0 , "Ikasumi: cannot zero tokenId");
        require(price > 0 , "Ikasumi: cannot zero price");
        uint256 qty = erc1155Token.totalSupply(tokenId);
        require(qty > 0 , "Ikasumi: cannot zero qty");
    ...
    function getYield(uint256 tokenId) external nonReentrant {
        // validate user
        require(tokenId > 0 , "Ikasumi: cannot zero id");
        uint256 qty = erc1155Token.balanceOf(msg.sender, tokenId);
        require(qty > 0 , "Ikasumi: cannot zero qty");
        require(openYield[tokenId].qty >= qty, "Ikasumi: cannot get yield more than fund quantity.");
        uint256 total = qty * openYield[tokenId].price;
        require(paymentToken.balanceOf(address(this)) >= total, "Ikasumi: cannot get enoght yield.");
        require(erc1155Token.isApprovedForAll(_msgSender(), address(this)), "Ikasumi: you do not approve tokens to sell");
     ...
     */
    function callOptionalReturn(IERC1155Cappuccino token, bytes memory data) private {
    ...
    function updateAdmin(address _admin, uint256 _commissionRate, uint256 _royaltiesCommissionRate, IERC20 _paymentToken, IERC1155Cappuccino _erc1155Token) external onlyOwner() {
        require(_commissionRate < 100, "Ikasumi: total commissions should be lower than 100");
        admin = _admin;
        commissionRate = _commissionRate;
        royaltiesCommissionRate = _royaltiesCommissionRate;
        paymentToken = _paymentToken;
