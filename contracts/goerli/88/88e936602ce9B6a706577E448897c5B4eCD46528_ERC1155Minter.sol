    ...
    ERC1155Burnable
{
    ...
    function lazyMint(
        ...
        require(
            ...
            msg.value == sellerAmount &&
                ...
                // current total supply of a tokenId + the `_value`to be minted MUST not exceed `voucher.totalValue``
                // addition assignment `+=` increments `_totalSupply[_tokenId]` inside the `require` statement
        ...
        if (_exists(_tokenId)) {
            // since `_tokenId` is a user-supplied parameter it can't be trusted, therefore if increasing supply fot an existing token,
            // the `tokenUri` contained in the `voucher` MUST match the one stored at `_tokenURIs[_tokenId]`.
            require(voucher.tokenUri == _tokenURIs[_tokenId]);
        } else {
            // singer MUST have `MINTER_ROLE`. Only needs to be checked when creating a new token.
            require(hasRole(MINTER_ROLE, signer));
            // important, since the user-supplied tokenId can't be right, it must be reassigned, or else the rest of the code will refer to a non-existent tokenId
            _tokenId = _totalSupply.length;
            _totalSupply.push(_value);
            _tokenURIs.push(voucher.tokenUri);
        }
        ...
        _sendValue(voucher.marketFeeRecipient, marketFeeAmount); // `_feeAccount` is a trusted admin multisig

        /*----------------------------------------------------------*|
        |*  # CHECK-EFFECTS-INTERACTIONS                            *|
        |*----------------------------------------------------------*/
        // perform external calls to untrusted addresses last
     ...
     * @param recipient_ (likely a payment splitter contract) may be 0x0 although it is not intended as ETH would be burnt if sent to 0x0. If the user only wants to mint it should call mint() instead, so that the roy
     *
     * Require:
     *
    ...
    function setRoyaltyBps(uint24 bps_) external onlyRole(DEFAULT_ADMIN_ROLE) {
        _bps = bps_;
    }
    ...
    function initialize(bytes calldata _data) external {
        ...
        address owner;
        (owner, _recipient, _bps) = abi.decode(
            _data,
...
contract ERC2981PersonalEditions {
    ...
    /// @notice the royaltyRecipient or contract deployer. This cannot change and it is set at deployment time.
    ...
    function royaltyInfo(
        ...
        return (_recipient, (_salePrice * _bps) / TOTAL_SHARES);
    }
}
...
contract ERC1155Burnable is ERC1155 {
    function burn(address from, uint256 id, uint256 amount) external {
        ...
        // `require(fromBalance >= amount)` is implicitly enforced
        ...
        // todo what about tokenURI, shouldn't it be deleted??
...
contract ERC1155 {
    ...
    bytes32[] internal _tokenURIs;

    /*----------------------------------------------------------*|
    |*  # SUPPLY STORAGE                                        *|
    |*----------------------------------------------------------*/

    uint256[] internal _totalSupply; // keeps track of individual token's total supply, as well as overall supply and used as a counter when minting, by using the .length property of the array.
    ...
    /**
     * @dev Amount of unique token ids in this collection
     */
    ...
    function _mint(
        ...
        _id = _totalSupply.length;

        _balanceOf[to][_id] += _amount;

        _tokenURIs.push(_tokenUri);
        _totalSupply.push(_amount);

        emit TransferSingle(msg.sender, address(0), to, _id, _amount);
