...
// File: contracts/CyberPunksK9.sol
...
contract CyberPunkK9 is ERC721A, Ownable {
    ...
    uint256 public whitelistedMintCost = 40000000000000000;
    ...
    // public
    function mint(uint256 _mintQuantity, uint256 oGTokenId) public payable {
        ...
        if (msg.sender != owner()) {
            ...
            bool _isOG = isOG(msg.sender, oGTokenId);
            ...
            if (_isOG == false && _isWhitelisted == false) {
                require(publicCanMint, "Public users cannot mint yet.");
                cost = publicMintCost;
            }
        ...
        addressMintedBalance[msg.sender] += _mintQuantity;
        _mint(msg.sender, _mintQuantity);
    }

    function freeClaimMint(uint256[] memory tokenIds) public {
        require(!paused, "the contract is paused");
        require(freeClaimCanMint, "Free claiming is not active yet.");

        for (uint256 i = 0; i < tokenIds.length; i++) {
            require(
                IERC1155(oGContractAddress).balanceOf(
                    msg.sender,
                    oGTokenIds[tokenIds[i] - 1]
                ) == 1
            );
            IERC721A(address(this)).transferFrom(
                owner(),
                msg.sender,
                tokenIds[i]
            );
    ...
    function isOG(address _user, uint256 oGTokenId) public view returns (bool) {
        return
            IERC1155(oGContractAddress).balanceOf(
                _user,
                oGTokenIds[oGTokenId - 1]
            ) == 1;
    }
    ...
    function freeClaimTokenIds(address _user)
        public
        view
        returns (uint256[] memory)
    {
        ...
        for (uint256 i = 0; i < oGTokenIds.length; i++) {
            if (
                IERC1155(oGContractAddress).balanceOf(_user, oGTokenIds[i]) == 1
            ) {
                quantity++;
            }
        }
        ...
        for (uint256 i = 0; i < oGTokenIds.length; i++) {
            if (
                IERC1155(oGContractAddress).balanceOf(_user, oGTokenIds[i]) ==
                1 &&
                ownerOf(i + 1) == owner()
            ) {
                tokenIds[j++] = i + 1;
    ...
    function setOGTokenIds(uint256[] calldata _oGTokenIds) public onlyOwner {
        delete oGTokenIds;
        oGTokenIds = _oGTokenIds;
    }

    function setOGContractAddress(address _address) public onlyOwner {
        oGContractAddress = _address;
    }
    ...
    function withdraw() public onlyOwner {
        ...
        //        payable(0xa1Fbc680C30E5E1b19f5c5DAcd74fCB7645FFb57).transfer(balance * 10 / 100);
