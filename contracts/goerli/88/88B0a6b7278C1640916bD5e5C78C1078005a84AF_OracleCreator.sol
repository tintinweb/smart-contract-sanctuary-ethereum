...
contract OracleCreator is IOracleCreator {

    address public swapFactory;
    address public ceresFactory;
    
    event OracleCreated(address indexed token, address indexed quoteToken, address indexed oracle);

    constructor(address _swapFactory, address _ceresFactory) {
        swapFactory = _swapFactory;
        ceresFactory = _ceresFactory;
    }

    function createOracle(address token, address quoteToken) external override returns (address oracle){
        require(msg.sender == ceresFactory, "OracleCreator: Only CeresFactory!");
        oracle = address(new OracleAverage());
        OracleAverage(oracle).initialize(swapFactory, token, quoteToken, address(ceresFactory));
        emit OracleCreated(token, quoteToken, oracle);
    }
