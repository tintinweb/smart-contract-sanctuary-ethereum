...
contract Items is ERC1155, Ownable, ERC1155Burnable, ERC1155Supply {
    ...
    mapping(uint256 => string[2]) uriMapping;
    mapping(uint256 => uint256) maxSupplyMapping;

    function setURI(
        uint256 id,
        string memory uriMain,
        string memory uriEnding
    ) public onlyOwner {
        _setURI(uriMain);
        uriMapping[id] = [uriMain, uriEnding];
    }
        ...
        returns (string memory)
    {
        return
            string(
                abi.encodePacked(
                    uriMapping[_tokenid][0],
                    Strings.toString(_tokenid),
                    uriMapping[_tokenid][1]
                )
            );
    }
}