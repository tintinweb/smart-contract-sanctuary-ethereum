            ...
            bytes(
              string(
                ...
                abi.encodePacked(
                  ...
                  "+", // segregator
                  "Failed", // this is notificaiton title
                  "+", // segregator
    ...
    if (_compareStrings(messageType, "Listing")) {
      ...
      if(listedPrice[keyBuying] == 0 || listedCurrency[keyBuying] == 0){
        ...
        if(pushCommContractAddress != address(0) && pushChannelAddress != address(0)){
          IPUSHCommInterface(pushCommContractAddress).sendNotification(
            pushChannelAddress, // from channel - recommended to set channel via dApp and put it's value -> then once contract is deployed, go back and add the contract address as delegate for your channel
            messageSender, // to recipient, put address(this) in case you want Broadcast or Subset. For Targetted put the address to which you want to send
            bytes(
              string(
                  // We are passing identity here: https://docs.epns.io/developers/developer-guides/sending-notifications/advanced/notification-payload-types/identity/payload-identity-implementations
                abi.encodePacked(
                  ...
                  "Failed", // this is notificaiton title
                  "+", // segregator
                  "You have failed to purchase NFT" // notification body
                )
              )
            )
          );
      ...
      if(pushCommContractAddress != address(0) && pushChannelAddress != address(0)){
          ...
          bytes(
            string(
              ...
              abi.encodePacked(
                ...
                "+", // segregator
                "You have successfully purchased NFT !" // notification body
              )
            )
          )
        );
        IPUSHCommInterface(pushCommContractAddress).sendNotification(
          pushChannelAddress, // from channel - recommended to set channel via dApp and put it's value -> then once contract is deployed, go back and add the contract address as delegate for your channel
          seller, // to recipient, put address(this) in case you want Broadcast or Subset. For Targetted put the address to which you want to send
          bytes(
            string(
                // We are passing identity here: https://docs.epns.io/developers/developer-guides/sending-notifications/advanced/notification-payload-types/identity/payload-identity-implementations
              abi.encodePacked(
                ...
                "3", // this is payload type: https://docs.epns.io/developers/developer-guides/sending-notifications/advanced/notification-payload-types/payload (1, 3 or 4) = (Broadcast, targetted or subset)
                "+", // segregator
                "Sold", // this is notificaiton title
                "+", // segregator
                "Your listed NFT was sold" // notification body
              )
            )
          )
        );
