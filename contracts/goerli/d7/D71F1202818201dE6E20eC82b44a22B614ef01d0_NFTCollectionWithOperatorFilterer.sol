  ...
  CollectionRoyalties
{
   ...
   */
  function _initializeNFTCollectionCore(address payable _creator, string calldata _name, string calldata _symbol)
    internal
  {
...
import "./NFTCollectionCore.sol";
import "./mixins/collections/OperatorFiltererMixin.sol";
import "./interfaces/internal/INFTCollectionInitializer.sol";

/**
 * @title A collection of NFTs by a single creator with operator filterer.
 * @notice All NFTs from this contract are minted by the same creator.
 * A 10% royalty to the creator is included which may be split with collaborators on a per-NFT basis.
 * @author batu-inal & HardlyDifficult
 */
contract NFTCollectionWithOperatorFilterer is
  IGetRoyalties,
  ...
  OperatorFiltererMixin
{
  ...
  constructor(address _contractFactory, address nftMarket, address nftDropMarket)
    ...
    NFTCollectionCore(_contractFactory) // solhint-disable-next-line no-empty-blocks
  {}
  ...
  function initialize(
    ...
    _initializeNFTCollectionCore(_creator, _name, _symbol);
  ...
  /**
   * @inheritdoc NFTCollectionCore
   */
  function burn(uint256 tokenId) public override(ERC721BurnableUpgradeable, NFTCollectionCore) {
    super.burn(tokenId);
  }
  ...
  function _burn(uint256 tokenId) internal override(ERC721Upgradeable, SequentialMintCollection, NFTCollectionCore) {
    super._burn(tokenId);
  }
  ...
  function supportsInterface(bytes4 interfaceId)
    public
    view
    override(ERC165Upgradeable, ERC721Upgradeable, CollectionRoyalties, NFTCollectionCore)
    returns (bool interfaceSupported)
  {
    interfaceSupported = super.supportsInterface(interfaceId);
  }
  ...
  function tokenURI(uint256 tokenId)
    public
    view
    override(ERC721Upgradeable, NFTCollectionCore)
    returns (string memory uri)
  {
    uri = super.tokenURI(tokenId);
  }

  function _baseURI() internal view override(ERC721Upgradeable, NFTCollectionCore) returns (string memory uri) {
    uri = super._baseURI();
  }
}
...
import "../../contracts/NFTCollectionCore.sol";

contract $NFTCollectionCore is NFTCollectionCore {
    bytes32 public __hh_exposed_bytecode_marker = "hardhat-exposed";

    constructor(address _contractFactory) NFTCollectionCore(_contractFactory) {}

    function $_initializeNFTCollectionCore(address payable _creator,string calldata _name,string calldata _symbol) external {
        return super._initializeNFTCollectionCore(_creator,_name,_symbol);
    }
...
import "../../contracts/NFTCollectionWithOperatorFilterer.sol";

contract $NFTCollectionWithOperatorFilterer is NFTCollectionWithOperatorFilterer {
    bytes32 public __hh_exposed_bytecode_marker = "hardhat-exposed";

    constructor(address _contractFactory, address nftMarket, address nftDropMarket) NFTCollectionWithOperatorFilterer(_contractFactory, nftMarket, nftDropMarket) {}
