    ...
    */
        ...
        operatorExists(_operatorId)
    {
        ...
        for (uint256 id = 0; id < _keyIndexes.length; ++id) {
            uint256 keyId = _keyIndexes[id];
            require(keyId < lastDeleted, "DESC_ORDER");
            lastDeleted = keyId;
            if (keyId < operators[_operatorId].usedSigningKeys) {
                _removeActiveKey(_operatorId, keyId);
            } else {
                _removeSigningKey(_operatorId, keyId);
            }
        }
    }
}
