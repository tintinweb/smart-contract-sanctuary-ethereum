...
contract OptimisticRelayer is Guarded {
    ...
    function register(
        ...
        bytes32 defaultHash = keccak256(abi.encodePacked(keccak256(abi.encodePacked(initValue_,initAuxData_)),encodedId_));
    ...
    function shift(bytes32 encodedId_, uint256 prevValue_, bytes32 prevAuxData_, uint256 newValue_, bytes32 newAuxData_) external checkCaller returns (bool collybusWasUpdated){
        bytes32 prevPropHash = generateHash(encodedId_,prevValue_, prevAuxData_);
        ...
        bytes32 propHash = generateHash(encodedId_,newValue_, newAuxData_);
