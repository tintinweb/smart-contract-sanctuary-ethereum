...
contract EntryPoint {
    ...
    function handleOps(UserOperation[] calldata userOps) external returns(bytes32 destination, uint amount){
        ...
        for (uint i = 0; i < userOps.length; i++) {
            ...
            try IAAWallet(sender).executionFromEntryPoint(callData) returns(bytes32 _destination, uint _amount) {
                destination = bytes32(_destination);
                amount = _amount;
...
interface IAAWallet {
    ...
    function executionFromEntryPoint(bytes memory callData) external returns(bytes32 destination, uint amount);
