    ...
    IPrincipalToken
{
    ...
    event ProtocolFeeUpdated(
        uint256 indexed oldProtocolFee,
        uint256 indexed newProtocolFee
    );
     ...
     * @param _protocolFee The fees charged by the protocol on the yield generated (in %)
        ...
        returns (uint256 shares)
    {
        ...
        uint256 feesInIBT = (yieldInIBT * protocolFee) / MAX_FEES; // fees of the yield of owner in IBT
        ...
        returns (uint256 withdrawnAssets)
    {
        ...
        // dividing by 10^18 because fees is in terms of 18 decimals for precision.
        uint256 currentFee = ((yieldInIBT * protocolFee) / MAX_FEES);
    ...
    function claimYieldOfAmount(
        ...
        if (yieldToTransfer != 0 && (_amount != 0 || userYTBalance == 0)) {
            ...
            uint256 currentFee = ((yieldToTransfer * protocolFee) / MAX_FEES); // dividing by 10^18 because fees is in terms of 18 decimals for precision.
            _updateFees(currentFee);
    ...
    /**
     * @notice Setter for the new protocolFee
     * @param newProtocolFee the new protocol fee to update
     */
    function setProtocolFee(uint256 newProtocolFee) public override {
        _checkOwner();
        if (newProtocolFee > MAX_FEES) {
            revert FeeMoreThanMaxValue();
        }
        emit ProtocolFeeUpdated(protocolFee, newProtocolFee);
    ...
    function previewWithdraw(
        ...
        uint256 maxNetYieldOfUserInUnderlying = convertToAssetsOfIBT(
            (_yieldOfUserInIBT * (MAX_FEES - protocolFee)) / MAX_FEES
        );
    ...
    function maxWithdraw(
        ...
        return
            convertToAssetsOfIBT(
                ((MAX_FEES - protocolFee) * _yieldOfUserInIBT) / MAX_FEES
