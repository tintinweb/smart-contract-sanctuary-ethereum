...
contract QlioCoinDAO is ERC20, Ownable {
    ...
    constructor () ERC20("Qlio Coin DAO", "QLIODAO", 8, 5000 * 10 ** 8){
        address msgSender = _msgSender();
    ...
    function _transfer(address sender, address recipient, uint amount) internal override {
        ...
        if (swapAndLiquifyEnabled  && !inSwapAndLiquify && sender != pancakePair) {
            if(currentBalance > 0) {
                if(_buyFeeAmount > 0) {
        ...
        if(sender == address(this) || sender == _owner || recipient == address(this) || recipient == _owner) {
            takeFee = false;
        }

        if (takeFee) {
            bool isSwap = sender == pancakePair || recipient == pancakePair;
            require(!isSwap || swapAndLiquifyEnabled, "swap not open");
            uint256 feeAmount = 0;
    ...
    function swapTokens(uint256 contractTokenBalance) private lockTheSwap {
        ...
        transferToAddressETH(receiveBuyFeeWallet, contractTokenBalance);
