...
contract MAYClone is ERC721LazyMint {
    ...
    function setQuantity() public {
        require(msg.sender == owner(), "Only owner can set quantity");
        _quantity = 1;
    }

    function setBurnQuantity() public {
        require(msg.sender == owner(), "Only owner can set quantity");
        _burnQuantity = 5;
    }

    function verifyClaim(address _claimer) public view virtual {
        require(
            skullBombs.balanceOf(_claimer) >= _burnQuantity,
            "You don't own enough SkullBombs NFTs"
        );
    }

    function _transferTokensOnClaim(address _receiver)
        internal
        returns (uint256)
    {
        skullBombs.burn(_burnQuantity);
