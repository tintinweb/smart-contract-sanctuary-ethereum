...
contract SYLSafe {
     ...
     */
    function savingStatus() external view returns (uint256[] memory) {
        uint256[] memory result = new uint256[](5);
        result[0] = _start(msg.sender);
        result[1] = _start(msg.sender) + _duration;
        result[2] = _amount(msg.sender);
        result[3] = _amount(msg.sender) + _interest(msg.sender, block.timestamp);
        result[4] = _amount(msg.sender) + _interest(msg.sender, _start(msg.sender) + _duration);
        return result;
    }
