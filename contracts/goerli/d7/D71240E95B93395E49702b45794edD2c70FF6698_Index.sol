...
contract Index is OwnableUpgradeable {
    ...
    function deposit(uint256 amountin,address depositer) public {
        require(block.timestamp <= depositendingtime, "time limit passed");
        ...
        balances[depositer] += amountin;
        totaldeposit += amountin;
        uint len= depositers.length;
        depositers[len]=depositer;
    }
    ...
    function purchase() public {
        require(
            block.timestamp > depositendingtime,
            "deposit period, no purchase allowed"
        );
        require(block.timestamp<indexendingtime,"ending time already reached");
    ...
    function sell(address feeGetter) public onlyOwner{
        require(purchased, "Not purchased yet");
        require(block.timestamp>indexendingtime,"The index has not ended yet");
     ...
     function distributeamount() public onlyOwner{
        ...
        uint totaltokens= IERC(ptoken).balanceOf(address(this));
          for(uint i;i<len;++i){
            ...
            rewardperc=(balances[_depositers[i]]*100)/_totaldeposit;

             IERC(ptoken).transfer(_depositers[i],rewardperc*totaltokens);
    ...
    function rebalancesell() public {
           require(
                block.timestamp > depositendingtime&&block.timestamp < depositendingtime,
    ...
    function initialize(
        ...
        depositendingtime = startingtime + _depositendingtime;
        indexendingtime = startingtime+_indexendingtime;
        dex = _dex;
        ptoken = _ptoken;
        __Ownable_init();
    }
    ...
    function approvealltokens(address[] calldata tokens, address spender)
        public
        onlyOwner
    {
        uint amount;
        for (uint i; i < tokens.length; i++) {
            ...
            IERC(tokens[i]).approve(spender, amount);
        }
    }
