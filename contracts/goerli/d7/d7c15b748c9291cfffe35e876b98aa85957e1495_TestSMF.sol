...
contract TestSMF is Ownable, Functional, ERC721, DefaultOperatorFilterer{ //contract StrippMFers is Ownable, Functional {
using Address for address;
    ...
    //address private doxxd      = payable(0x1271Dc0C23099D985A7f3455D99AE35a4Be810c8); //70% Test
    //address private toronto    = payable(0x032F009A2D3E799D229b4612a5E3EF8fB8072672); //30% Test
    address private doxxd      = payable(0x583031D1113aD414F02576BD6afaBfb302140225); //70% Test
    address private toronto    = payable(0xdD870fA1b7C4700F2BD7f44238821C26f7392148); //30% Test
    
    /**
     * @dev Initializes the contract by setting a `name` and a `symbol` to the token collection.
     */
    constructor(address _cham, address _gas, address _love) ERC721("",""){
        _name = "TestSMF";
        _symbol = "TSMF";
        baseURI = "https://strippmfers.xyz/metadata/";
        
        price = 2 * (10 ** 16); // Replace leading value with price in finney = 0.02 Eth
        gasPrice = 5000 * (10 ** 18) ;//5000;        // it requires update
        lovePrice = 2500 * (10 ** 18);//2500;       // it requires update
        maxSupply = 2222;
        reservedTokens = 100;
        reservedChem = 400;
        reservedGas = 100;
        ...
        maxPerWallet = 100;
        maxMintPerTx = 50;
        //CT = ERC721(0x207aF75D65FF74Bd3E5B4570E8133C3756bFb7d9); //Chemeleons contract address.
        CT = ERC721(_cham); //Chemeleons contract address.
        gasFactoryAddress = IERC20(_gas);
        loveTokenAddress = IERC20(_love);
        /*
        CT = ERC721(0xC273350237368E9274d426E9DBF0e3eaA9ACd01e);
        gasFactoryAddress = IERC20(0xCBD6C0697A929800f6c5Ceb8c32aC469E521915D);
        loveTokenAddress = IERC20(0xcA70b3F193959F07b9461732Deb4Ba69e07815ed);
        */
    }
    ...
    function withdraw() external onlyOwner {
        ...
        uint256 sendGasTokens =loveTokenAddress.balanceOf(address(this));

        bool success;
        /*(success, ) = toronto.call{value: ((loveTokenAddress.balanceOf(address(this)) * 30)/100)}("");
        require(success, "Transaction Unsuccessful");

        (success, ) = doxxd.call{value: ((loveTokenAddress.balanceOf(address(this)) * 70)/100)}("");
        require(success, "Transaction Unsuccessful");

        (success, ) = toronto.call{value: ((gasFactoryAddress.balanceOf(address(this)) * 30)/100)}("");
        require(success, "Transaction Unsuccessful");

        (success, ) = doxxd.call{value: ((gasFactoryAddress.balanceOf(address(this)) * 70)/100)}("");
        require(success, "Transaction Unsuccessful");
        */
    ...
    function requireAll(uint256 qty) private view {
        ...
        require((qty + 
                ...
                reservedChem + 
                reservedGas + 
                reservedLove ) <= maxSupply,
            "Not enough avaialability"); 
        require(qty <= maxMintPerTx,"Mint tokens per transaction exceeded.");
        require(( tokensMintedby[ _msgSender() ] + qty ) <= maxPerWallet,
            "Mint per wallet exceeded.");
    }
    ...
    function mintByChameleons(uint256 [] memory qty) external reentryLock {
        ...
        require(CT.balanceOf(_msgSender()) > 0, "Must have Chemeleon Token.");
        require(reservedChem > 0, "Chemeleon reserved tokens end.");
        for(uint i =0; i < qty.length ; i++)
        {
           require(!excludedChemToken[ qty[i] ],"Chemeleon Token must not be excluded.");
           require(msg.sender == (CT).ownerOf( qty[i] ),"Called does not hold these token IDs.");
        }
    ...
    function mintByFungibleTokens(uint256 qty, uint256 flag) external reentryLock {
        requireAll(qty);
        require(flag == 1 || flag == 2, "Wrong Fungible token used to mint."); 
        if(flag == 1){
