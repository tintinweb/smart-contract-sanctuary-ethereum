// File: contracts\token\utils\IERC165.sol
...
// File: contracts\token\ERC1155\IERC1155MetadataURI.sol
...
// File: contracts\token\utils\ERC165.sol
...
contract DogTag is ERC1155, IDogTag {
    address kyodaiAddy;
    address shateiAddy;
    ...
    string[3] className = ["Cyborg", "Druggie", "Hacker"];

    constructor() ERC1155("") {}

    function mint(uint256 tagId, address who_) external override {
        require(
    ...
    function burn(
        ...
        _burn(who_, tagId, amount_);
    }

    function uri(uint256 _tagId) public view override returns (string memory) {
        return
            string(
                abi.encodePacked(
                    ...
                    KyodaiLibrary.encode(
                        bytes(
                            string(
                                abi.encodePacked(
                                    '{"name": "Dog Tag #',
                                    KyodaiLibrary.toString(_tagId),
                                    '", "description": "Dog Tag is a collection of 10,000 unique smol cats. Omnichain NFT so you can pspsps your cat accross chains. All the metadata and images are generated and stored 100% on-chain. No IPFS, no API.","image": "data:image/svg+xml;base64, ',
                                    KyodaiLibrary.encode(
                                        bytes(getTokenSVG(_tagId))
                                    ),
                                    '","attributes":',
                                    hashToMetadata(_tagId),
                                    "}"
                                )
                            )
                        )
    ...
    function getTokenSVG(uint256 _tagId) internal view returns (string memory) {
        return
            string(
                abi.encodePacked(
                    string(
    ...
    function hashToMetadata(uint256 _tagId)
        internal
        view
        returns (string memory)
    {
        ...
        for (uint8 i = 0; i < 2; ++i) {
            ...
            if (i == 0) {
                traitType = "Tier";
                traitName = tierName[uint16((_tagId - 1) / 3)];
            }
            if (i == 1) {
                traitType = "Class";
                traitName = className[_tagId % 3];
            }
