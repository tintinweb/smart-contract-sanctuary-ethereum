...
contract BDOGEToken is ERC20, Ownable{
    ...
    constructor() ERC20("Based Doge", "BDOGE")
    {
        ...
        _totalSupply = INITIAL_FRAGMENTS_SUPPLY * (_gonsPerFragment);
        _balances[msg.sender] = INITIAL_FRAGMENTS_SUPPLY * (_gonsPerFragment);
    ...
    function totalSupply()
        ...
        returns (uint256)
    {
        return _totalSupply / (_gonsPerFragment);
    }
    ...
    function balanceOf(address who)
        ...
        returns (uint256)
    {
        return _balances[who] / (_gonsPerFragment);
    }
        ...
        returns (bool)
    {
        ...
        _balances[msg.sender] = _balances[msg.sender] - (gonValue);
        _balances[to] = _balances[to] + (gonValue);
        ...
        returns (bool)
    {
        ...
        uint256 gonValue = value * (_gonsPerFragment);
        _balances[from] = _balances[from] - (gonValue);
        _balances[to] = _balances[to] + (gonValue);
...
contract BDOGEPolicy is Ownable {
    ...
    onlyOrchestrator
    {
        ...
        supplyDelta = supplyDelta / (rebaseLag.toInt256Safe());
        ...
        if(supplyDelta == 0) {
            ...
            require(lastRebaseTimestampSec + (minRebaseTimeIntervalSec) < block.timestamp , "30 minutes has not passed since last rebase");
            
            //this sets the total_supply to max_supply
            if (supplyDelta > 0 && uFrags.totalSupply() + (uint256(supplyDelta)) > MAX_SUPPLY) {
            supplyDelta = (MAX_SUPPLY - (uFrags.totalSupply())).toInt256Safe();
            }
    ...
    constructor(BDOGEToken uFrags_, uint256 _token_price)
    {
    ...
    function withinDeviationThreshold(uint256 new_price, uint256 old_price)
        ...
        returns (bool)
    {
        uint256 absoluteDeviationThreshold = old_price * (deviationThreshold) / (10**DECIMALS);
        
        return (new_price >= old_price && new_price - (old_price) < absoluteDeviationThreshold)
            || (new_price < old_price && old_price - (new_price) < absoluteDeviationThreshold);
    }
}

// File: BDOGEOrchestrator.sol
