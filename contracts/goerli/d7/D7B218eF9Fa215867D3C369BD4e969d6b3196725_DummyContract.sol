...
interface IGasVault   {
     ...
     */
    function transactionsRemaining(address targetAddress) external view returns(uint256);
...
contract DummyContract is Initializable, OwnableUpgradeable, UUPSUpgradeable, IImplementation {
    uint256 public num;
    ...
    function initialize(address _orchestrator, address _steer,address _internalGovernance, uint256 _tokenId, bytes calldata params) public initializer {
        __UUPSUpgradeable_init();
        __Ownable_init();
        address _gasVault = abi.decode(params, (address));
