...
contract SDBowl is ERC721A, ERC2981, Ownable,DefaultOperatorFilterer,ReentrancyGuard {
    bytes32 public merkleRootWL=0x3d5795530bfc05cd66361b7fca442424d2431d34635ef2eff01ca5eabf1b10b4;
    bytes32 public merkleRootDogList=0x1e9af3805f64877d36392c4b9bf47368180e3c8776175e7db0379f627c638a3d;
    string public baseURI="";
    string public tokenSuffix="";
    string public unrevealedURI="ipfs://bafkreih3ayr2dvsn2h2wvzfb2mwwo2eo3u3drasnfsfggofkzcnny6kwx4";
    bool public isRevealed = false;
    address public payoutAddress=0xFBC83fFD1DaFEec74D2d6183fE154e6c75BF73e1;
    uint256 public mintPrice=0.025 ether;
    uint256 public mintPriceWL=0.02 ether;
    uint256 public maxTokens = 2800;
    uint256 public mintPhase=0;
    uint256 public burnEnabled=0;
    uint256 public bonezburnAmt=3;
    uint256 public MAX_MINT_PUBLIC = 4;
    uint256 public MAX_MINT_FREE = 1;
    uint256 public MAX_MINT_DOGLIST = 3;
    ...
    mapping(address => uint256) public doglistMintedCount;  
    ...
    //constructor
    constructor(address daddr) ERC721A("SDBowl", "SDBowl") {
       setRoyalties(payoutAddress,750);
       dbonezaddress=daddr;
    }

    /* MINTING */
    function doBonezMint(uint256[] memory tokens) external nonReentrant{
        require(burnEnabled == 1,"Cannot Burn Yet");
        require(tokens.length >= bonezburnAmt, "Need more bonez");
        require(tokens.length % bonezburnAmt == 0, "Wrong amount of tokens to burn");
        uint256 tobeminted=tokens.length / bonezburnAmt;
        require(totalMinted()+tobeminted <= maxTokens, "Not enough tokens available to mint");
    ...
    function doDogListMint(uint256 numberOfTokens,bytes32[] calldata _merkleProof) external payable nonReentrant{
        require(mintPhase==1,"DogList Mint Phase is not available");
        require(totalMinted()+numberOfTokens <= maxTokens, "Not enough tokens available to mint");
        uint256 walletMintedAmt = doglistMintedCount[msg.sender];
        require(walletMintedAmt + numberOfTokens <= MAX_MINT_DOGLIST,"Exceeded DOGLIST Allocation");
        bytes32 leaf=keccak256(abi.encodePacked(msg.sender));
        require(MerkleProof.verifyCalldata(_merkleProof,merkleRootDogList,leaf), "Invalid Proof for DogList");

        //check if they already got their free
        ...
        doglistMintedCount[msg.sender] +=numberOfTokens;
     
       
    }

    function doMintFromWhiteList(uint256 numberOfTokens,bytes32[] calldata _merkleProof) external payable nonReentrant{
    ...
    function doPublicMint(uint256 numberOfTokens,bytes32[] calldata _merkleProof) external payable nonReentrant{
    ...
    function setMerkleRootDogList(bytes32 merk) external onlyOwner {
        merkleRootDogList=merk;
    }
    ...
    function setBonezBurnAmt(uint256 a) external onlyOwner{
        bonezburnAmt=a;
    }
    ...
    function setMAXDOGLIST(uint256 p) external onlyOwner{
        MAX_MINT_DOGLIST=p;
    }
    ...
    function setDBonezAddress(address s) external onlyOwner {
        dbonezaddress=s;
    }
    ...
    function contractURI() public pure returns (string memory) {
        return "http://spacedogworld.com/contractsdbowl.json";
    }
