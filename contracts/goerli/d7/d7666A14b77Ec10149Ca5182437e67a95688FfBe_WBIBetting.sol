...
contract WBIBetting is Ownable, ReentrancyGuard {
    ...
    event MatchCreated(uint256 indexed matchId, string description, uint256 indexed eventStartTime);
    event PlayerBet(uint256 indexed matchId, address indexed walletAddress, uint256 indexed winner, uint256 amount);
    ...
    // note: A = 1, B = 2, Draw = 3
    function bet(uint256 matchId, uint256 winnerSelection) external payable nonReentrant{
        ...
        require(matchInfo.eventStartTime > block.timestamp, "Cannot bet anymore");
        Player storage player = matchInfo.players[msg.sender];
        require(player.winnerSelected == 0, "Cannot select again or add more to bet");
        require(winnerSelection <= 3 && winnerSelection != 0, "Can only select team A, B, or Draw");
    ...
    function setWinner(uint256 matchId, uint256 winner) external onlyAuthorized {
        MatchInfo storage matchInfo = matchInformation[matchId];
        require(activeMatches.contains(matchId), "Match is closed");
        require(winner <= 4 && winner != 0, "Can only select team A, B, or Draw or Cancelled");
        ...
        if(shares == 0 && matchInfo.totalPayout > 0){
            (bool success,) = address(feeReceiver).call{value: matchInfo.totalPayout}("");
    ...
    function cashOut(uint256 matchId) external nonReentrant {
        ...
        MatchInfo storage matchInfo = matchInformation[matchId];
        Player storage player = matchInfo.players[msg.sender];
        if(closedMatches.contains(matchId) && accountMatchesPending[msg.sender].contains(matchId)){
            ...
            if(player.winnerSelected == matchInfo.winner){
                ...
                uint256 amountForPayout = player.amountBet * matchInfo.totalPayout / shares;
                amountWonByAccount[msg.sender] += amountForPayout;
                if(amountForPayout > 0){
                    (success,) = address(msg.sender).call{value: amountForPayout}("");
    ...
    function cashOutAll() external nonReentrant {
        ...
        for(uint256 i = 0; i < matchIds.length; i++){
            ...
            if(closedMatches.contains(matchIds[i]) && accountMatchesPending[msg.sender].contains(matchIds[i])){
                ...
                if(player.winnerSelected == matchInfo.winner){
                    amountForPayout += player.amountBet * matchInfo.totalPayout / shares;
                } else if(matchInfo.winner == 4){
    ...
    function getAmountClaimableByMatchId(uint256 matchId, address account) external view returns (uint256){
        MatchInfo storage matchInfo = matchInformation[matchId];
        Player storage player = matchInfo.players[account];
        if(closedMatches.contains(matchId) && accountMatchesPending[account].contains(matchId)){
    ...
    function getAmountTotalClaimable(address account) external view returns (uint256){
        uint256[] memory matchIds = accountMatchesPending[account].values();
        uint256 amountForPayout;
        for(uint256 i = 0; i < matchIds.length; i++){
            MatchInfo storage matchInfo = matchInformation[matchIds[i]];
            Player storage player = matchInfo.players[account];
            if(closedMatches.contains(matchIds[i]) && accountMatchesPending[account].contains(matchIds[i])){
                uint256 shares;
    ...
    function getMatchInfo(uint256 matchId) external view returns (string memory description,
            uint256 aShares,
            uint256 bShares,
            uint256 drawShares,
            uint256 totalPayout,
    ...
    function getPlayerInfoByMatchId(uint256 matchId, address account) external view returns (uint256 amountBet, uint256 winnerSelected) {
        MatchInfo storage matchInfo = matchInformation[matchId];
        Player storage player = matchInfo.players[account];
    ...
    function getAccountMatchesPending(address account) external view returns (uint256[] memory) {
