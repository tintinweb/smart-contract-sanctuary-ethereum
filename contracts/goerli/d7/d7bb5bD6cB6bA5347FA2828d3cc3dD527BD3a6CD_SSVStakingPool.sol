...
import "@ankr.com/contracts/earn/LiquidTokenStakingPool.sol";
import "./interfaces/IDepositContract.sol";
import "./interfaces/IWithdrawalManager.sol";
import "./interfaces/ISSVStakingPool.sol";

contract SSVStakingPool is LiquidTokenStakingPool, ISSVStakingPool {

    IDepositContract internal _depositContract;
    IWithdrawalManager internal _withdrawalManager;

    function initialize(IEarnConfig earnConfig, IDepositContract depositContract) external initializer {
        __Ownable_init();
    ...
    function setWithdrawalManager(address newValue) external override onlyGovernance {
        address prevValue = address(_withdrawalManager);
        _withdrawalManager = IWithdrawalManager(newValue);
        emit WithdrawalManagerChanged(newValue, prevValue);
    }

    function pushStake(uint32 id, bytes calldata pubkey, bytes calldata signature, bytes32 deposit_data_root) external override onlyConsensus {
        require(address(this).balance >= 32 ether, "SSVStakingPool: not enough balance");
        bytes memory withdrawalCredentials = getWithdrawalCredentials();
        _depositContract.deposit{value: 32 ether}(pubkey, withdrawalCredentials, signature, deposit_data_root);
        emit StakePushed(pubkey, id, withdrawalCredentials);
    }

    function getWithdrawalCredentials() public view override returns (bytes memory withdrawalCredentials) {
        withdrawalCredentials = abi.encodePacked(_withdrawalManager);
        bytes memory prefix = bytes.concat(abi.encodePacked(uint8(1), abi.encodePacked(uint88(0))));
        withdrawalCredentials = bytes.concat(prefix, withdrawalCredentials);
...
interface ISSVStakingPool {
    ...
    event WithdrawalManagerChanged(address newValue, address prevValue);

    event StakePushed(bytes pubkey, uint32 id, bytes withdrawalCredentials);

    function setWithdrawalManager(address newValue) external;

    function pushStake(uint32 id, bytes calldata pubkey, bytes calldata signature, bytes32 deposit_data_root) external;

    function getWithdrawalCredentials() external view returns (bytes memory withdrawalCredentials);
}
...
abstract contract LiquidTokenStakingPool is ILiquidTokenStakingPool, OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    modifier onlyGovernance() virtual {
        require(msg.sender == _earnConfig.getGovernanceAddress(), "LiquidTokenStakingPool: only governance allowed");
        _;
    }

    modifier onlyConsensus() virtual {
        require(msg.sender == _earnConfig.getConsensusAddress(), "LiquidTokenStakingPool: only consensus allowed");
        _;
    }

    modifier bondStakingUnpaused() virtual {
        require(!_earnConfig.isBondStakingPaused(), "LiquidTokenStakingPool: bond staking is paused");
        _;
    }

    function __LiquidTokenStakingPool_init(IEarnConfig earnConfig) internal onlyInitializing {
    ...
    function setBearingToken(address newValue) external override onlyGovernance {
        address prevValue = address(_bearingToken);
        _bearingToken = IBearingToken(newValue);
        emit BearingTokenChanged(prevValue, newValue);
    }

    function setCertificateToken(address newValue) external override onlyGovernance {
    ...
    function setMinimumStake(uint256 newValue) external override onlyGovernance {
        require(newValue % 1 gwei == 0, "LiquidTokenStakingPool: value should be multiplied of gwei");
    ...
    function stakeBonds() external override payable nonReentrant bondStakingUnpaused {
        uint256 shares = _bearingToken.bondsToShares(msg.value);
        _stake(msg.sender, msg.value, shares, true);
        _afterStake(msg.sender, msg.value, shares);
    }

    function stakeCerts() external override payable nonReentrant {
        uint256 shares = _certificateToken.bondsToShares(msg.value);
        _stake(msg.sender, msg.value, shares, false);
        _bearingToken.unlockSharesFor(msg.sender, shares);
        _afterStake(msg.sender, msg.value, shares);
    }

    function _stake(address staker, uint256 amount, uint256 shares, bool isRebasing) internal {
        _beforeStake(staker, amount, shares);
        require(amount >= getMinStake(), "LiquidTokenStakingPool: value must be greater than min amount");
        require(amount % getMinStake() == 0, "LiquidTokenStakingPool: value must be multiple of minimum staking amount");
    ...
    function _beforeStake(address account, uint256 amount, uint256 shares) internal virtual {}

    function _afterStake(address account, uint256 amount, uint256 shares) internal virtual {}
}