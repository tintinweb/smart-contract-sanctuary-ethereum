 ...
 */
contract CouncilNFT is ERC1155, Ownable {
  using Strings for uint256;

  /// @notice the total supply of the collection
  uint256 public constant TOTAL_SUPPLY_PER_TYPE = 85;
  ...
  /// @notice mapping to track total minted per type
  mapping(uint256 => uint256) public totalMintedPerType;
  ...
  /// @notice the different council seat types
  uint256 public constant SEAT_TYPE_0 = 0;
  uint256 public constant SEAT_TYPE_1 = 1;
  uint256 public constant SEAT_TYPE_2 = 2;
  uint256 public constant SEAT_TYPE_3 = 3;
  uint256 public constant SEAT_TYPE_4 = 4;
  uint256 public constant SEAT_TYPE_5 = 5;
  uint256 public constant SEAT_TYPE_6 = 6;
  uint256 public constant SEAT_TYPE_7 = 7;
  uint256 public constant SEAT_TYPE_8 = 8;
  uint256 public constant SEAT_TYPE_9 = 9;
  uint256 public constant SEAT_TYPE_10 = 10;
  uint256 public constant SEAT_TYPE_11 = 11;

  /// @notice the root of the merkle tree for the allowlist
  bytes32 internal immutable root;

  constructor(string memory _baseURI, bytes32 _root) ERC1155() {
    baseURI = _baseURI;
    root = _root;
    allowlistMint = true;
  }
  ...
  function privateMint(
    ...
    //If we are currently in the allowlist minting period, verify that the user is on the allowlist
    ...
    return mintTo(recipient, seatType);
  }
  ...
  function mintTo(
    ...
    //Get the total minted for this particular seat type, and increment it by 1
    uint256 seatTypeCount = ++totalMintedPerType[seatType];

    //If the total minted for this type are greater than the total per type, revert
    if (seatTypeCount > TOTAL_SUPPLY_PER_TYPE) revert CouncilErrors.MaxSupply();

    //Set minted to true for this user, a single user shouldn't be able to mint more than 1
    alreadyMinted[recipient] = true;

    //mint it
    _mint(recipient, seatType, 1, "");

    emit CouncilEvents.Minted(recipient);
    return seatType;
  }

  /// @notice Convenience method to get the total minted for a particular seat type
  /// @param seatType the seat type to get the total minted for
  function getTotalMintedForType(
    uint256 seatType
  ) public view returns (uint256) {
    return totalMintedPerType[seatType];
  }

  /// @notice Get the tokenURI for the given tokenId
  /// @param id the id of the token to get the tokenURI for
