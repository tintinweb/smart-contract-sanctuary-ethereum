...
contract ERC721SSMinter is Ownable, MerkleProof {
    ...
    IERC721SS public ERC721SS = IERC721SS(0xE0E8557f7e22Bb3271955BAa767400435Ff0A2B7);
    IERC721 public ERC721 = IERC721(0x3A7405A635fCfcb1bB4Bc59D15E63CC712F54996);
    uint256 public Ids = 34;
    ...
    function claimSBT(uint256 tokenId, string memory add) external payable {
        if (bytes(add).length > 0) {
            ...
            ADD[tokenId] = add;
            ERC721SS.mint(tokenId);
        }
        else if (bytes(add).length == 0) {
            require(ERC721.ownerOf(tokenId) == msg.sender, "Not owner");
            ERC721SS.mint(tokenId);
        }
    }
