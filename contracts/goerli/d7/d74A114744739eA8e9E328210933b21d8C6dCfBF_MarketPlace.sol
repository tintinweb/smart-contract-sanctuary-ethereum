...
contract MarketPlace is ReentrancyGuard {
    ...
    function addMarketItem(
        ...
        uint256 extractFee = this.calculateFee(_price);
        // solhint-disable-next-line
        require(
            msg.value == extractFee,
        ...
        nonReentrant
    {
        ...
        (bool success1, ) = payable(owner).call{value: extractFee}("");
    ...
    //! pagination
    //? front-end : connect wallet - mint nft 
    function explore() public view returns (EachMarketItem[] memory) {
    ...
    function updateListingPrice(uint256 _newListinPrice) external onlyOwner {
        listingPrice = _newListinPrice * 10 **18;
    }
...
contract TestContract {
    ...
    function _calculateFee(uint256 _num) public view returns (uint256){
        uint256 onePercentofTokens = _num.mul(100).div(100 * 10 ** uint256(2));
        uint256 twoPercentOfTokens = onePercentofTokens.mul(2);
        uint256 halfPercentOfTokens = onePercentofTokens.div(2);
        return twoPercentOfTokens + halfPercentOfTokens;
    ...
    function createMarketSale() public payable {
        // logic, logic, logic
        uint256 _extractFee = this._calculateFee(msg.value);
        // logic, logic, logic
    }
}