...
contract Engine is ERC721, IEngine {
    ...
    // NOTE: This is used as a scale factor
    uint256 public scaleFactorNotional = 1;

    // NOTE: FR has to granular and it is a percentage in 1e6 format
    uint256 public scaleFactorFR;

    // NOTE: Min Order Amount
    ...
    struct Match {
        ...
        uint256 fmfr; // The fair market funding rate when the match was done
    ...
    function setScaleFactorNotional(uint256 _scaleFactorNotional) external {
        scaleFactorNotional = _scaleFactorNotional;
    }

    function setMaxLeverageOpen(uint256 _maxLeverageOpen) external {
        leverage.maxLeverageOpen = _maxLeverageOpen;
    }

    function setMaxLeverageOngoing(uint256 _maxLeverageOngoing) external {
        leverage.maxLeverageOngoing = _maxLeverageOngoing;
    }

    function setMinGuaranteedLeverage(uint256 _minGuaranteedLeverage) external {
        leverage.minGuaranteedLeverage = _minGuaranteedLeverage;
    }

    function setScaleFactorFR(uint256 _x) external {
        scaleFactorFR = _x;
    }

    function setMinOrderAmount(uint256 x) external {
        orderMinAmount = x;
    }
    ...
    function open(
        ...
        require(
            ...
            ) <= leverage.getMaxLeverage(fr, timeElapsed, leverage.fmfr),
    ...
    function close(uint256 matchId) external {
        ...
        // console.log("[close()] m.trader = ", m.trader);
        ...
        if (msg.sender == _ownerOf(m.maker)) {
            ...
            require(
                LeverageMath.getLeverage(
                    ...
                    0,
                    oracle.getPrice().price
                ) <= leverage.getMaxLeverage(m.fr, timeElapsedInDays, m.fmfr),
                "engine/max-leverage-open"
            );
            m.collateralM -= amount;
            matches[matchId] = m;
    ...
    function join(PickOrder memory order) internal {
        ...
        if (order.owner == maker) {
            emit MakerIsTrader(order.matchid, maker);
            return;
        }
        ...
        // NOTE: Jason does not approve price checks for traders
        // if(priceData.price < order.slippageMinPrice) {
        //     console.log("[join()] Skip for price too low");
        // }

        // if(priceData.price > order.slippageMaxPrice) {
        //     console.log("[join()] Skip for price too high");
        // }
        ...
        emit ActiveMatch(order.matchid, maker, order.owner, m.maker, m.trader);
    }
    ...
    function unjoin(uint256 matchId) internal {
        ...
        //     _getMaxLeverage(m.fr, timeElapsed, leverage, leverage.fmfr)
        //     )
        //     {
        ...
        returns (uint256 matchId, uint256 tokenId)
    {
        ...
        newMatch.premium = (m.premium * int256(amount)) / int256(m.amount);
        newMatch.fr = m.fr;
        newMatch.pos = m.pos;
    ...
    function pnl(
        ...
        return
            _pnl(
                timestamp - m.start,
                int256(exitPrice) - int256(m.entryPrice),
                m.fr,
                m.amount,
                m.premium,
                tokenId == m.maker,
    ...
    function _pnl(
        uint256 deltaT,
        int256 deltaP, /** exitPrice - entryPrice */
        uint256 fr,
        uint256 amount,
        int256 premium,
        bool isMaker,
        ...
        // Match memory m = matches[matchId];
        // require(timestamp > m.start, "engine/wrong_timestamp");
        // require((tokenId == m.maker) || (tokenId == m.trader), "engine/invalid-tokenId");
        int256 aggregatedFR = int256(deltaT * fr * amount) / int256(10**18);
        int256 gl = (((isShort) ? -deltaP : deltaP) * int256(amount)) /
            int256(10**18);
        res =
            gl +
            ((isMaker) ? int256(1) : int256(-1)) *
            (aggregatedFR + premium);
    }
        ...
        )
    {
        ...
        _uPnL = _pnl(
            ...
            int256(priceData.price) - int256(m.entryPrice),
            m.fr,
            m.amount,
            m.premium,
            tokenId == m.maker,
    ...
    function runOpens(uint256 n) external {
        ...
        // n = (n==0) ? opens.length : _min(n, opens.length);
        uint256 priceTimestamp = oracle.getPrice().timestamp;

        // for (uint256 i = 0; i < opens.length; ++i)
        for (uint256 i = 0; i < n; ++i) {
            (bool success, PickOrder memory o) = opens.pop();
            // PickOrder memory o = opens[i];

            // NOTE: Queue is empty but this should not happen since n should have taken care of this
            if (!success) return;

            // TODO: Add check on the order.timestamp vs oracle price timestamp
            if (!o.canceled) {
                join(o);
            } else {
                console.log("[runOpens()] Skipping order as canceled ID = ", i);
            }
        }
        // empty the queue
        // delete opens;
...
interface IEngine {
    ...
    event MakerIsTrader(uint256 indexed matchId, address indexed maker);
