...
abstract contract GovernorMiniBravo is IGovernorMiniBravo, Roles {
  ...
  uint256 public constant EXECUTION_TIMELOCK = 1 weeks;
...
abstract contract LockManagerGovernor is ILockManagerGovernor, GovernorMiniBravo {
  ...
  constructor(address _admin, IPoolManager.LockManagerParams memory _lockManagerParams) payable GovernorMiniBravo() {
    poolManagerFactory = _lockManagerParams.factory;
  ...
  /// @notice Cancels an amount of votes for all proposals voted by the voter
  /// @param _voter The voter to remove the votes from
  /// @param _votes The votes to remove
  function _cancelVotes(address _voter, uint256 _votes) internal virtual {
  }
}

contract LockManager is ILockManager, ERC20, LockManagerGovernor {
  // event Locked(uint256 _priceAmount);
  // event ClaimedRewards(address _owner, address _to, uint256 _priceAmount, uint256 _tokenAmount);
  // event RewardsAdded(uint256 _priceAmount, uint256 _tokenAmount);
  // event FeesCollected(uint256 _priceFees, uint256 _tokenFees);
  ...
  constructor(IPoolManager _poolManager, IPoolManager.LockManagerParams memory _lockManagerParams)
    payable
    ERC20('LockedPRICE', '', 18)
    LockManagerGovernor(msg.sender, _lockManagerParams)
  {
  ...
  function claimRewards(address _to) external returns (uint256 _rewardPrice, uint256 _rewardToken) {
    (_rewardPrice, _rewardToken) = (block.number, block.number * 2);
  ...
  function _feesDistribution(uint256 _totalPrice, uint256 _totalToken) internal {
    poolManager.feeManager().depositFromLockManager(_totalPrice / 3, _totalPrice / 3);
