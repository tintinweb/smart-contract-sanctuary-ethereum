...
contract TestNFT is ERC721A, Ownable  {
    ...
    // Is approving allowed? (for ex: listing on OS)
    ...
    uint public constant MAX_SUPPLY = 9600;
    ...
    // Stake function, to stake your nft
    function stakeNFT(uint256 tokenId) external isStakingContract {
        isStaked[tokenId] = true;
        emit Staked(msg.sender, tokenId, block.timestamp);
    }
    // Stake function, to stake your nft
    function stakeByNFTOwner(uint256[] memory tokenId) external {
    ...
    // Unstake function, to unstake your nft
    function unstakeNFT(uint256 tokenId) external isStakingContract{
     ...
     // Unstake function, to unstake your nft
    function unstakeByNFTOwner(uint256[] memory tokenId) external {
        for(uint i = 0; i < tokenId.length; i++) {
            require(msg.sender==ownerOf(tokenId[i]),"TokenId not owned");
            isStaked[tokenId[i]] = false;
            emit Unstaked(msg.sender, tokenId[i], block.timestamp);
        }
    }


    // Frozen until timestamp is reached
    function setFrozen(uint256 timestamp) external onlyOwner {
    ...
    function tokenURI(uint _tokenId) public view virtual override returns (string memory) {
        ...
        if(isRevealed == true) {
            ...
            return string(abi.encodePacked(baseURI, "hidden"));
        }
    }
