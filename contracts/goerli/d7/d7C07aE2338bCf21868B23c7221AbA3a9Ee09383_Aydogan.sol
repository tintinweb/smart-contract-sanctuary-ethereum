// SPDX-License-Identifier: MIT
    ...
    contract Aydogan is ERC721, Ownable, ERC721Enumerable {
      using SafeMath for uint256;
      using Strings for uint256;
    
      uint256 public constant tokenPrice = 0.008 ether;
      uint256 public constant maxTokenPurchase = 3; // PERSON TOKEN MINT AMOUNT
      uint256 public constant MAX_TOKENS = 10000; // TOTAL NFT COUNT
      ...
      uint256 public devReserve = 64; // FOR DEVELOPERS
    
      event AydoganMinted(uint256 tokenId, address owner);
    
      constructor() ERC721("Aydogan", "ADG") {}
      ...
      function mintAydogan(uint256 numberOfTokens) external payable {
        ...
        for (uint256 i = 0; i < numberOfTokens; i++) {
          ...
          if (totalSupply() < MAX_TOKENS) {
            ...
            emit AydoganMinted(id, msg.sender);
          }
        }
      }
    
      function presaleAydogan(uint256 numberOfTokens) external payable {
        ...
        returns (string memory)
      {
        ...
        if (isRevealed == false) {
          return
            "ipfs://QmSzLtzij2SaFqUUmczJQYfeucvUUbNyzy5qM6HrBY9nBn";
        }
