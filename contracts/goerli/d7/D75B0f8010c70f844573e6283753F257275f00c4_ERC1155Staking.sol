...
contract ERC1155Staking is ReentrancyGuard,ERC1155Holder {
    ...
    constructor(IERC1155 _nftCollection, IERC20 _rewardsToken) {
    ...
    function stake(uint256 _tokenId , uint256 _amount) external nonReentrant {
        ...
        nftCollection.safeTransferFrom(msg.sender, address(this), _tokenId, _amount, "0x00");
        // Create StakedToken
    ...
    function withdraw(uint256 _tokenId,  uint256 _amount) external nonReentrant {
        ...
        nftCollection.safeTransferFrom( address(this), msg.sender, _tokenId, _amount, "0x00");
    ...
    //     rewardsToken.transferFrom(contractOwner, msg.sender, rewards);
    // }

    function claimRewards() external nonReentrant {
    // Calculate rewards and add to the unclaimed rewards
    uint256 rewards = calculateRewards(msg.sender);
    stakers[msg.sender].unclaimedRewards += rewards;
    // Ensure the user has some unclaimed rewards
    require(stakers[msg.sender].unclaimedRewards > 0,"You have no unclaimed rewards");
    // Transfer the rewards from the contract to the staker
    rewardsToken.safeTransfer(msg.sender, stakers[msg.sender].unclaimedRewards);
    // Reset the unclaimed rewards
    stakers[msg.sender].unclaimedRewards = 0;
    // Update the timeOfLastUpdate for the staker
    stakers[msg.sender].timeOfLastUpdate = block.timestamp;
}
