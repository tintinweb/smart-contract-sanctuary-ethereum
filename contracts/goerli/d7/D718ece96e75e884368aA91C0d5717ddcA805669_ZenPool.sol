...
contract ZenPool is IJellyPool, IJellyContract, JellyPoolNFT {
    ...
    function setLiquidityGauge(address _gauge) external {
        ...
        require(address(liquidityGauge) == address(0));
        // require(poolToken == liquidityGauge.lp_token());
    ...
    function _stake(address _user, uint256 _amount) internal {
        ...
        if (poolSettings.gaugeWhitelisted) {
            ...
            if (balance > 0) {
                OZIERC20(poolToken).safeApprove(address(liquidityGauge), 0);
                OZIERC20(poolToken).safeApprove(address(liquidityGauge), _amount);
     ...
     * @param _amount amount of tokens to unstake.
     */
    // TODO: check whether the unstake amount is equal to or greater than the staked amount
    function _unstake(
        address _user,
        uint256 _tokenId,
        uint256 _amount
    ...
    /**
     * @notice  Withdraw all LP tokens from a gauge to pool save gas on unstake
     * @dev     Only callable by the admin 
     ...
     * @param _bal_minter address of Balancer Minter.
