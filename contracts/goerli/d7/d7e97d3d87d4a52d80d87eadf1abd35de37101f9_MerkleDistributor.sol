...
contract MerkleDistributor is IMerkleDistributor, Initializable, AccessControlUpgradeable {
    ...
    // --- initialization ---
    function initialize(
        MerkleDistributorConfig memory cfg,
        address roleAdmin
    ) initializer public {
        __MerkleDistributor_init(cfg, roleAdmin);
    }

    function __MerkleDistributor_init(
        MerkleDistributorConfig memory cfg,
        address roleAdmin
    ) internal onlyInitializing {
        __AccessControl_init();

        __MerkleDistributor_init_unchained(cfg, roleAdmin);
    }

    function __MerkleDistributor_init_unchained(
        MerkleDistributorConfig memory cfg,
        address roleAdmin
    ) internal onlyInitializing {
        ...
        totalClaims = cfg.totalClaims;
        initialPoolSize = cfg.initialPoolSize;
        currentPoolSize = cfg.initialPoolSize;
        percentageIndex = PERCENTAGE_BASE;
        bonusStart = cfg.bonusStart;
        bonusEnd = cfg.bonusEnd;
        emergencyTimeout = cfg.emergencyTimeout;
        emergencyReceiver = cfg.emergencyReceiver;
    ...
    function claim(
        ...
        require(
            IERC20Upgradeable(token).transfer(account, adjustedAmount),
    ...
    function getBonus() public view returns (uint256) {
        ...
        block.timestamp > bonusEnd ? 0 : bonusEnd - block.timestamp;
        // bonus = maxBonus * timeRemaining / (bonusEnd - bonusStart)
        return MAX_BONUS * timeRemaining / (bonusEnd - bonusStart);
    }
    ...
    )
    {
        ...
        if (claimed + 1 == totalClaims) {
            return (amount + bonusSum, 0, 0);
        }
        // adjustedPercentage = amount / initialPoolSize * percentageIndex
        uint256 adjustedPercentage = amount * PERCENTAGE_BASE / initialPoolSize * percentageIndex / PERCENTAGE_BASE;

        // bonusPart = adjustedPercentage * bonusSum
        bonusPart = adjustedPercentage * bonusSum / PERCENTAGE_BASE;
        // totalToClaim = amount + bonusPart
        uint256 totalToClaim = amount + bonusPart;
        // bonus = totalToClaim * getBonus()
        bonus = totalToClaim * getBonus() / PERCENTAGE_BASE;
...
interface IMerkleDistributor {
    struct MerkleDistributorConfig {
        address token;              // token to be airdropped
        bytes32 merkleRoot;         // generated merkle root of airdrop recipients/amounts
        uint256 totalClaims;        // number of accounts eligible for airdrop
        uint256 initialPoolSize;    // sum of all airdrop amounts, in wei
        uint256 bonusStart;         // timestamp at which bonus starts
        uint256 bonusEnd;           // timestamp after which bonus is max
        uint256 emergencyTimeout;   // time after which an emergency withdrawal can be made
        address emergencyReceiver;  // receiver of emergency withdrawal, after emergency timeout passes
    }

    function initialize(MerkleDistributorConfig memory cfg, address roleAdmin) external;
