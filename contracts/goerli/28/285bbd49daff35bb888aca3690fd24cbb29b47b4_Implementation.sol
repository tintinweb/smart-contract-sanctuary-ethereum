...
contract Implementation is HatsAccessControl {
    ...
    function init() external {
        ...
        // create admin and operator hats
        ...
        // mint admin and operator hats
        Hats.mintHat(adminId, admin);
        Hats.mintHat(operatorId, operator);

        // set access control authority
        _grantRole(ADMIN, adminId);
        _grantRole(OPERATOR, operatorId);
    }

    // only admin can call
    function adminTest() external view onlyRole(ADMIN) returns (bool) {
        return true;
    }

    // only operator or admin can call?
    function operatorTest() external view onlyRole(OPERATOR) returns (bool) {
        return true;
    }
}
