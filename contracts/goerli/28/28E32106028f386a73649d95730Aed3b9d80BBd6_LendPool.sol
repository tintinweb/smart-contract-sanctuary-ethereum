...
contract LendPool is Ownable, ReentrancyGuard {
    ...
    fallback() external payable {
        console.log("fallback method called why though");
    }
    ...
    function deposit(ERC20 _token, uint256 _amount) external nonReentrant {
        ...
        // console.log("---- deposit request for amount %s", _amount);
        // make sure user has sufficient balance to deposit
        uint256 balance = _token.balanceOf(_user);
        if (balance < _amount) {
            // console.log("user balance is too low %s", balance);
            revert LendingPool__InsufficientAmountForDeposit(_user, address(_token), balance);
        }

        // console.log("---- user balance %s", _token.balanceOf(_user));
        // console.log("----- allowance for LP %s", _token.allowance(_user, address(this)));
        // console.log("----- allowance for LP core %s", _token.allowance(_user, address(core)));
