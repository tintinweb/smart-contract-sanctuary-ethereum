...
contract Ownable {
    ...
    modifier onlyOwner() {
        require(msg.sender == owner, "Your address is not an owner.");
        _;
    }
...
contract VDGStoreproofV2 is Ownable {
    ...
    function storeProof(string memory hash, uint256 tokenValue)
        public
        isWhitelisted(msg.sender)
    {
        require(!proofs[hash], "Proof already exists.");
        proofs[hash] = true;
        transferTokens(token, msg.sender, tokenValue);
    }

    function checkProof(string memory hash) public view returns (bool) {
        return proofs[hash];
    }
    ...
    modifier isWhitelisted(address _beneficiary) {
        require(
            ...
            "Your address needs to be whitelisted to store proofs."
        );
        _;
    }
    ...
    function modifyTokenAddress(address _address) external onlyOwner {
        token = _address;
    }
    ...
    function withdrawToken(address _tokenAddress) external onlyOwner {
        uint256 tokenValue = IERC20(_tokenAddress).balanceOf(address(this));
        IERC20(_tokenAddress).safeTransfer(msg.sender, tokenValue);
    }
}