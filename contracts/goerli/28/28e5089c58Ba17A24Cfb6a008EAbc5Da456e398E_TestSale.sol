...
contract TestSale is Ownable, ReentrancyGuard {
    ...
    // CHANGE TO MAINNET!!!!!!!
    uint public constant DUTCH_PRICE_START = 0.06 ether;
    uint public constant DUTCH_PRICE_END = 0.02 ether;
    uint public constant FIRST_DUTCH_DROPPING_STEP = 0.01 ether;
    uint public constant DUTCH_DROPPING_STEP = 0.005 ether;
    uint public constant DUTCH_ROUND_DURATION = 15*60 seconds;
    uint public constant DUTCH_ROUND_MAX_DURATION = 720*60 seconds;
    uint public constant DUTCH_AUCTION_MAX_DURATION = 1440*60 seconds;
    uint public constant DUTCH_AUCTION_MAX_ROUND = 7; // Round began to 0, there is 8
    ...
    // CHANGE TO MAINNET!!!!!!!
    uint public maxSupplySale = 3000;
    ...
    function getPrice() public view returns(uint) {
        ...
        if(block.timestamp - startTime >= DUTCH_AUCTION_MAX_DURATION || currentRound >= DUTCH_AUCTION_MAX_ROUND  ) {
         return DUTCH_PRICE_END;
            ...
            return DUTCH_PRICE_START - (FIRST_DUTCH_DROPPING_STEP);
        }
        else {
    ...
    function mint(uint _quantity, address _to, uint maxTokenPerWallet) private isInitialized isPublicSaleActive   {
        ...
        require(tokenPerWallet[_to] + _quantity <= maxTokenPerWallet, "Max tokens per wallet reached");
        tokenPerWallet[_to] += _quantity;
     ...
     */
    function unstake(uint[] memory _tokenId, address _to) external {
        ITestNFT(oxyalandsNFTAddress).unstakeNFT(_tokenId, _to);
        
         for(uint i = 0; i < 6; i++) {
                uint count = chanceToWinPerAddressWhitelist[_to][i];

                if(count != 0){
                    chanceToWinPerAddressWhitelist[_to][i] -= count;
                    return;
                }
         }
           for(uint i = 0; i < 6; i++) {
                uint count = chanceToWinPerAddress[_to][i];

                if(count != 0){
                    chanceToWinPerAddress[_to][i] -= count;
                    return;
                }
         }
    }
