...
contract HalloweenPoupelle is ERC721AntiScamControl, ERC2981, ReentrancyGuard {
...
//        _safeMint(withdrawAddress, 100);
        ...
        returns (bool)
    {
        ...
        interfaceId == type(ERC721AntiScam).interfaceId ||
   ...
   function wlMint(uint64 _saleType, uint256 _mintCount, uint256 _maxCount, bytes32[] calldata _merkleProof)
    ...
    function VIPMint(uint256 _mintCount)
        external
        payable
        nonReentrant
    {
    ...
    function setRoyaltyFee(uint96 _feeNumerator)
        external
        onlyOwner
    {
    ...
    //
    function setLock(LockStatus[] calldata statuses, uint256[] calldata tokenIds) external {
        require(statuses.length == tokenIds.length, 'invalid value');
        for (uint256 i = 0; i < statuses.length; i++) {
            require(ownerOf(tokenIds[i]) == msg.sender);
            _lock(statuses[i], tokenIds[i]);
        }
    }

    // return value : tokenId * 10 + lockStatus
    // example: tokenId = 5678 , lockStatus = 3 , return value = 56783 
        ...
        unchecked {
            ...
            for (uint256 i = _startTokenId(); tokenIdsIdx != tokenIdsLength; ++i) {
                ...
                if (currOwnershipAddr == owner) {
                    tokenIds[tokenIdsIdx++] = i*10 + uint256(getLockStatus(i));
                }
            }
            return tokenIds;
        }
