...
contract RandColors is ERC721Base {
    constructor(string memory _name,
        string memory _symbol,
        address _royaltyRecipient,
        uint128 _royaltyBps) ERC721Base(_name,
            _symbol,
            _royaltyRecipient,
            _royaltyBps) {}
    ...
    function safeMint(address to, uint256 tokenId) public onlyOwner {
        ...
        uint256 tokenId = uint256(keccak256(abi.encodePacked(block.timestamp, block.difficulty, msg.sender)));
        string memory color = generateRandomColor();
        string memory svgData = generateSvgData(color);
        TokenMetadata memory metadata = TokenMetadata(tokenId, svgData);
    ...
    function generateRandomColor() internal view returns (string memory) {
        ...
        return string(abi.encodePacked("#", uint2hex(randomValue)));
    }

    function uint2hex(uint256 n) internal pure returns (string memory) {
        bytes memory hexAlphabet = "0123456789abcdef";
        bytes memory result = new bytes(8);
        for (uint256 i = 0; i < 8; i++) {
            result[i] = hexAlphabet[(n >> (28 - 4 * i)) & 0xf];
        }
        return string(result);
    }

    function generateSvgData(string memory color) internal pure returns (string memory) {
