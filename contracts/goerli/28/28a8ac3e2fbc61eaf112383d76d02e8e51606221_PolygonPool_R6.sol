...
import "../interfaces/IinternetBond_R2.sol";

contract PolygonPool_R6 is PausableUpgradeable, ReentrancyGuardUpgradeable, OwnableUpgradeable, IGlobalPool_R2 {
    ...
    uint256 private _ethUnstakeFee;
    ...
    function stakeAndClaimCerts(uint256 amount) external override {
        ...
        IinternetBond_R2(_bondContract).unlockSharesFor(msg.sender, amount);
    }
    ...
    function _stake(address staker, uint256 amount, bool isRebasing) internal nonReentrant {
        ...
        IinternetBond_R2(_bondContract).mintBonds(staker, amount);
        /* emit events */
    ...
    function unstake(uint256 amount, uint256 fee, uint256 useBeforeBlock, bytes memory signature) override external payable nonReentrant {
        ...
        uint256 shares = IinternetBond_R2(_bondContract).balanceToShares(amount);
        _unstake(msg.sender, amount, shares, fee, true);
    }
    ...
    function unstakeCerts(uint256 shares, uint256 fee, uint256 useBeforeBlock, bytes memory signature) override external payable nonReentrant {
        _collectFee(msg.sender, fee, useBeforeBlock, signature);
        uint256 amount = IinternetBond_R2(_bondContract).sharesToBalance(shares);
        IinternetBond_R2(_bondContract).lockSharesFor(msg.sender, shares);
        _unstake(msg.sender, amount, shares, fee, false);
    }
    ...
    function _collectFee(address staker, uint256 fee, uint256 useBeforeBlock, bytes memory signature) internal {
        if (fee == 0 && useBeforeBlock == 0) {
            require(msg.value >= _ethUnstakeFee, "PolygonPool: not enough ETH to pay fee");
    ...
    function _unstake(address staker, uint256 amount, uint256 shares, uint256 fee, bool isRebasing) internal {
        ...
        IinternetBond_R2(_bondContract).burnBonds(staker, amount);
    ...
    function serveClaims(uint256 amountToUse, address payable residueAddress, uint256 minThreshold) public onlyOperator payable {
        ...
        for (i = _pendingMaticClaimGap; i < _pendingClaimers.length; i++) {
            ...
            j++;
            IinternetBond_R2(_bondContract).commitBurn(claimer, amount);
    ...
    function setEthUnstakeFee(uint256 newValue) external onlyOperator {
        uint256 prevValue = _ethUnstakeFee;
        _ethUnstakeFee = newValue;
    ...
    function migrateDelayedBurn() external onlyOwner {
        for (uint256 i = _pendingMaticClaimGap; i < _pendingClaimers.length; i++) {
            ...
            IinternetBond_R2(_bondContract).migrateBurnDelayed(claimer, amount);
        }
    }
...
interface IinternetBond_R2 {
    ...
    function burnBonds(address account, uint256 bonds) external;

    function commitBurn(address account, uint256 amount) external;
