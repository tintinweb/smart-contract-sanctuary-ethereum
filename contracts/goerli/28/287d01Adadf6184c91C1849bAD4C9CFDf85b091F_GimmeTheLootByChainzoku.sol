...
contract GimmeTheLootByChainzoku is Initialize, ERC1155Multi, MultiMint, MerkleProofVerify, ShareProxy {
    ...
    function GimmeTheBox(bytes32[] memory _proof, uint256 _id, uint256 _count, uint256 _max)
        ...
        nonReentrant
    {
        require(mintBalance(string.concat("BOX_", _id.toString()), _msgSender()) <= _max, "Stop, you have already minted too much boxes!");

        _mintTokens(_msgSender(), _id, uint64(_count));
    }

    function GimmeTheLoot(uint256 _id, uint64 _count) public {
        require(burnThemAllStart[_id] > 0 && block.timestamp >= burnThemAllStart[_id], "No, you can open it now!");
        burn(_id, _count);
    }
    ...
    function ThisIsMine(uint256 _id) public onlyOwnerOrAdmins {
        uint256 count = supplies[_id].max - supplies[_id].minted;
        require(mintIsOpen(string.concat("BOX_", _id.toString())) == false, "Mhh, it's not the good time");
        require(count > 0, "There are no box left");
