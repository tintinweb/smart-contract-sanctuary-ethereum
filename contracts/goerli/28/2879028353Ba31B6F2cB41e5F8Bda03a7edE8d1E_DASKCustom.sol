...
contract DASKCustom is IERC721, Proxy, Functional, DefaultOperatorFilterer {
    ...
    // Enumeration feature to keeping track of a token holder's collection
    ...
    mapping(uint256 => uint256) tierprice;
    ...
    constructor() {
        ...
        //whole token price for each tier
        tierprice[1] = 1000;
        tierprice[2] = 2000;
        tierprice[3] = 3000;
        
        KLUB = ERC20(0xa0DB234a35AaF919b51E1F6Dc21c395EeF2F959d);
    }
    ...
    function supportsInterface(bytes4 interfaceId) public view virtual override returns (bool) {
        return  interfaceId == type(IERC721).interfaceId ||
                ...
                interfaceId == DASKCustom.onERC721Received.selector;
    }
    ...
    function airDrop(address _to, uint256 setTier) external onlyOwner {
        require((numberMinted + 1) > numberMinted, "Math overflow error");
        require((numberMinted + 1) <= totalTokens, "Cannot fill order");
        
        uint256 mintSeedValue = numberMinted + 1; //increment before so we start at 1

        _safeMint( _to, mintSeedValue );
        numberMinted ++;  //increment here in case of gas overload
        
        tokenTier[mintSeedValue] = setTier;
    }

    function mint(uint256 setTier) public reentryLock {
        // There are three types of NFT purchases, the type will be determined by setTier
        // 1. Basic Trait generation request from the existing collection
        // 2. Customized trait for new colors and such
        // 3. New traits can be created for this order.
        require(mintActive, "Mint not open");
        require((numberMinted + 1) > numberMinted, "Math overflow error");
        require((numberMinted + 1) <= totalTokens, "Cannot fill order");
        require( 1 + _mintTracker[_msgSender()] <= maxPerWallet, "Wallet Limit Reached");

        uint256 mintSeedValue = numberMinted + 1;
        _mintTracker[_msgSender()] ++;
        numberMinted ++;

        //assign a designated token tier to this NFT
        tokenTier[mintSeedValue] = setTier;

        uint256 calculatedPrice = tierprice[setTier] * (10**18);

        require(calculatedPrice > 0);

        // mint the token
        _safeMint(_msgSender(), mintSeedValue);
        
        // REQUIRES APPROVAL TO BE SET FOR KLUB TOKEN
        //KLUB.transferFrom(_msgSender(), address(this), klubPrice * qty);

        // REQUIRES THIS CONTRACT SET AS PROXY FOR KLUB
        KLUB.burnFrom(_msgSender(), calculatedPrice);
    }
    ...
    function proxyTransfer(address from, address to, uint256[] memory tokenId) external proxyAccess {
    ...
    function getManyTokenTiers(uint256[] memory tokenIds) external view returns (uint256[] memory){
        uint256[] memory tokenTiers = new uint256[]( tokenIds.length );

        for (uint256 i; i < tokenIds.length; i++){
            tokenTiers[i] = tokenTier[ tokenIds[i] ];
        }

        return tokenTiers;
    }

    function getInventory(address holder) external view returns (uint256[] memory){
        return _inventory[holder];
    }

    function setCoinContract( address contractAddress) external onlyOwner {
        KLUB = ERC20( contractAddress );
    }
    ...
    function setPrice(uint256 tier, uint256 newPrice) public onlyOwner {
        //price set in whole coins.
    ...
    function _burn(uint256 tokenId) internal virtual {
        ...
        _removeInventory(owner, tokenId);

        emit Transfer(owner, address(0), tokenId);
    }

    function _removeInventory(address owner, uint256 tokenId) internal {
        // Search owner inventory for the token and remove it
        uint256 temp = _inventory[owner][ _inventory[owner].length -1 ]; //store the last value temporarily
        if ( temp == tokenId){
            _inventory[owner].pop();
        } else {
            for (uint256 i; i < _inventory[owner].length; i++){
                if (_inventory[owner][i] == tokenId){
                    //swap and pop
                    _inventory[owner][i] = temp;
                    _inventory[owner].pop();
                    break;
                }
            }
