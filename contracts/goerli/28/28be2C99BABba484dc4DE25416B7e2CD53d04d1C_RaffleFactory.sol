 ...
 * {revokeRole} functions. Each role has an associated deployer role, and only
 * accounts that have a role's deployer role can call {grantRole} and {revokeRole}.
 *
 * By default, the deployer role for all roles is `DEFAULT_ADMIN_ROLE`, which means
 * that only accounts with this role will be able to grant or revoke other
 * roles. More complex role relationships can be created by using
 * {_setRoleAdmin}.
 *
 * WARNING: The `DEFAULT_ADMIN_ROLE` is also its own deployer: it has permission to
...
abstract contract AccessControl is Context, IAccessControl, ERC165 {
     ...
     * {revokeRole}.
     *
     * To change a role's deployer, use {_setRoleAdmin}.
     */
     ...
     *
     * Using this function in any other way is effectively circumventing the deployer
    ...
    /**
     * @dev Sets `adminRole` as ``role``'s deployer role.
     *
