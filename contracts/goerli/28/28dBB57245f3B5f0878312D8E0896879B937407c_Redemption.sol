...
contract Redemption is AccessControl, ReentrancyGuard {
    ...
    function redeem(
        ...
        require(hasRole(OPERATOR_ROLE, msg.sender), "Redemption: not operator");
        transferERC20(_token, _to, _amount);
        emit Redeem(_redeemId, _token, _to, _amount);
    }

    function batchRedeem(
        uint256[] calldata _redeemIds,
        address _token,
        ...
        require(hasRole(OPERATOR_ROLE, msg.sender), "Redemption: not operator");
        require(
            _redeemIds.length == _to.length,
            "Redemption: redeemIds and to length not match"
        );
        
        uint256 totalAmount = _amount * _to.length;
        require(
            balanceOfERC20(_token) >= totalAmount,
            "Redemption: Insufficient balance"
        );

        for (uint256 i = 0; i < _redeemIds.length; i++) {
            transferERC20(_token, _to[i], _amount);
            emit Redeem(_redeemIds[i], _token, _to[i], _amount);
        }
    }
    ...
    function transferERC20(
        ...
        require(balance >= _amount, "Redemption: Insufficient balance");
