...
contract ApyFlowZap is Ownable {
    ...
    EnumerableSet.AddressSet private harvestableVaults;
    ...
    function addHarvestableVaults(address[] memory vaults) external onlyOwner {
        for (uint i = 0; i < vaults.length; i++) {
            harvestableVaults.add(vaults[i]);
        }
    }

    function removeHarvestableVaults(address[] memory vaults) external onlyOwner { 
        for (uint i = 0; i < vaults.length; i++) {
            harvestableVaults.remove(vaults[i]);
        }
    }

    function harvest() public {
        for (uint i = 0; i < harvestableVaults.length(); i++) {
            IHarvestableVault(harvestableVaults.at(i)).harvest();
        }
    }
    ...
    function pendingRewardsInclusivePricesPerToken() external returns (uint256 pricePerToken, uint256 feeInclusivePricePerToken) {
        harvest();
        pricePerToken = apyflow.pricePerToken();
        feeInclusivePricePerToken = apyflow.feeInclusivePricePerToken();
    }
}
...
contract ApyFlow is ERC20, Ownable {
    ...
    function totalAssets() public view returns (uint256 total) {
        for (uint256 i = 0; i < vaults.length(); i++) {
            ...
            total += vault.convertToAssets(balanceAtVault) * (10**decimals()) / (10**vault.decimals()); // todo: need tests for that
        }
    }
