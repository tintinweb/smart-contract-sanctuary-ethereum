...
contract MyUpgradableToken is Initializable, ERC20Upgradeable, OwnableUpgradeable, UUPSUpgradeable {
  ...
  {}
    ...
    function mint(uint256 amount) external returns (bool) {
        ...
        uint256 feeAmount = (amount * feePercentage) / 10000; // Calculate the fee amount
        uint256 mintAmount = amount - feeAmount; // Calculate the mint amount
    ...
    function approvePaymentToken(address _paymentToken, uint256 amount) external returns (bool) {
        // Get the payment token contract instance
        IERC20 paymentTokenContract = IERC20(_paymentToken);
        
        // Approve the contract to spend the user's tokens
        bool success = paymentTokenContract.approve(address(this), amount);
    ...
    function burn(uint256 amount) external {
        ...
        paymentToken.transfer(msg.sender, burnAmount);
        _transfer(msg.sender, feeAddress, feeAmount);
    }
    ...
    function setFeePercentage(uint256 _feePercentage) external onlyOwner {
        require(_feePercentage <= 10000, "Invalid fee percentage"); // Maximum fee percentage of 100%
        feePercentage = _feePercentage;
    }
    ...
    function transferFrom(
        ...
        _approve(
            sender,
            msg.sender,
            allowance(sender, msg.sender) - amount + feeAmount
        );
