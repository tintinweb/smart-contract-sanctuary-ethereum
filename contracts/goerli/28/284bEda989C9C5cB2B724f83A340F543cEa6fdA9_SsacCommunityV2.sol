...
contract SsacCommunityV2 is Initializable {
    ...
    mapping(uint256 => uint256) public contestWinner; // Maps a contestId to the winner EntryID

    function initialize(uint256 _votingCost) public initializer {
        // ssacToken = SSACTOKEN(getTokenAddress()); //0x03bECfd9152B01E28522AD6073eA1264EAE87c46
        ssacToken = new SSACTOKEN(); // For Tests
    ...
    function endContest(uint256 _contestId) external onlyOwner {
        ...
        contestWinner[_contestId] = winningEntryId;
    ...
    function getEntry(
        ...
        require(isEntry(_contestId, _entryId), "entry not found");
        return entries[_contestId][_entryId];
    }

    function getNoOfContests() public view returns (uint256) {
        return contests.length;
    }

    function getNoOfEntries(uint256 _contestId) public view returns (uint256) {
        return entriesCount[_contestId];
    }
    ...
    function getContestWinner(
        ...
        return contestWinner[_contestId];
    }
