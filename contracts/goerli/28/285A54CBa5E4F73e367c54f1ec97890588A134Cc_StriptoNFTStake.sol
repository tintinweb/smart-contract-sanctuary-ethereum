...
contract StriptoNFTStake is Ownable, NFTReceiver, IStripto {
    ...
    function init(bytes memory _data) public override {
        ...
        _setUpStake(data.rewardsPerPeriod, data.lockPeriods, data.penalties);
    ...
    function depositRewardToken(
        uint256 amount
    ) external payable onlyOwner onlyBeforeEnd onlyWithFee {
        _depositRewardToken(amount, msg.sender);
    }

    function setRewardsForPeriod(
        uint256 _tier,
    ...
    function userInfo(
        address user
    ) external view returns (ExtendedUserStake[] memory) {
    ...
    function _updateRewardsPerSecond(
        uint256 tier,
        uint256 newRewardsPerSeconds
    ) internal {
