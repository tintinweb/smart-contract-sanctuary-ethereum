    ...
    IClubrareMarketplace
{
    ...
    function _isLazyMint(uint256 _tokenId, address _contractAddress) internal view returns (bool) {
        return ((_tokenId == 0 && validator.cbrNFTAddress() == _contractAddress) ||
            (_tokenId == 0 && validator.adminContracts(_contractAddress)));
    }
    ...
    {
        (address signer, uint256 amt) = _validateOffer(bid, order, _amount);
        Order calldata _order = order;
        ...
        if (_isLazyMint(_tokenId, _order.contractAddress)) {
            ...
            isValidTransfer(taker,nftContract.ownerOf(_tokenId));
        }
    ...
    function claim(Order calldata order) external whenNotPaused nonReentrant isNotBlacklisted(msg.sender) {
        ...
        if (_isLazyMint(_tokenId, _order.contractAddress)) {
            ...
            isValidTransfer(auction.highestBidder,nftContract.ownerOf(_tokenId));
        }
    ...
    function _chargeAndSplit(
        ...
        if (_isClaim) {
            ptoken.safeTransfer(treasuryWallet, marketFee);
        } else {
            ptoken.safeTransferFrom(sender, treasuryWallet, marketFee);
        }
    ...
    function _executeExchange(
        ...
        if (order.orderType != Type.Escrow) {
            ...
            if (isToken) {
                if (isClaim) { // this condition only valid if item sell through auction 
