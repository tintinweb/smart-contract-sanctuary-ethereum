...
contract TexasPokerPremiumNFT is Ownable, ERC721ALockable {
    uint256 public constant PRICE = 2e18;
    uint256 public constant MAX_TOTAL_SUPPLY = 52;
    bytes32 public constant MAINTAIN_ROLE = keccak256("MAINTAIN_ROLE");
    string public baseURI = "";
    uint256 public AMOUNT_FOR_DEVS = 1;
    ...
    constructor() ERC721ALockable("TexasPokerPremiumNFT", "TPPN"){
    ...
    function mint(uint256 quantity) external payable {
        require(Address.isContract(msg.sender) == false, "E10001");
        require(_totalMinted() + quantity + AMOUNT_FOR_DEVS <= MAX_TOTAL_SUPPLY, "E10002");
        require(status == Status.Live, "E10003");
        require(quantity <= 5, "E10004");
        require(PRICE * quantity <= msg.value, "E10005");

        _safeMint(msg.sender, quantity);
    }

    function ownerMint(uint256 quantity) external onlyOwner {
        require(_totalMinted() + quantity <= MAX_TOTAL_SUPPLY, "E10002");
        require(status == Status.Live, "E10003");
        require(quantity <= AMOUNT_FOR_DEVS, "E10004");
...
contract ERC721ALockable is ERC721ABurnable, IERC721ALockable {
    ...
    public {
        for (uint i = 0; i < tokenIds.length; i++) {
            ...
            bool isApprovedOrOwner = (tx.origin == owner ||
            isApprovedForAll(owner, tx.origin) ||
            getApproved(tokenId) == tx.origin);
            require(isApprovedOrOwner, "ERC721ALockable: caller is not owner nor approved");
