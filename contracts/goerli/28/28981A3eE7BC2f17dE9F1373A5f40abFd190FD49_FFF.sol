    ...
    Ownable
{
    ...
    IFFF2 private fff2Address = IFFF2(0xFD7F90b4746d70281AAb4A4bBd64A6Dd29E5537D);
    mapping(uint8 => string) private _baseTokenURI;
    mapping(address => bool) public alreadymint;
    mapping(uint256 => uint8) public fffLevel;
    ...
    function tokenURI(uint256 tokenId) public view override returns (string memory) {
        ...
        string memory baseURI = _baseURI(fffLevel[tokenId]);
        return
    ...
    function airdrop(uint _start, uint _end)external onlyRole(MANAGER_ROLE){
        ...
        for(uint256 i = _start; i <= _end; i++){
            isAirdrop[i] = true;
            address ownerAmount = fff2Address.ownerOf(i);
            mint(ownerAmount);
        }
    }

    function isAlreadymint(address add)public view returns(bool){
        return fff2Address.alreadymint(add);
    }
    ...
    function addLevel(uint256 tokenId) external onlyRole(MINTER_ROLE){
        fffLevel[tokenId] = fffLevel[tokenId] + 1;
    }

    function redLevel(uint256 tokenId) external onlyRole(MINTER_ROLE){
        require(fffLevel[tokenId] >= 1,"Level is lowest!");
        fffLevel[tokenId] = fffLevel[tokenId] - 1;
    }
