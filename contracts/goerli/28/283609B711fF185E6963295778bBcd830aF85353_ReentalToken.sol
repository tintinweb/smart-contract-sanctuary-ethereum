...
contract ReentalToken is ERC20CappedUpgradeable, UUPSUpgradeableByRole, RefundableUpgradeable {
  ...
  event Swap(address indexed from, address indexed to, uint256 indexed amount);

  // solhint-disable-next-line var-name-mixedcase
  bytes32 private immutable _MINTER_ROLE = keccak256("MINTER_ROLE");

  // solhint-disable-next-line var-name-mixedcase
  bytes32 private immutable _TOKEN_ADMIN_ROLE = keccak256("TOKEN_ADMIN_ROLE");
  
  function initialize(string memory name, string memory symbol, uint256 supply) public initializer {
    __AccessControlProxyPausable_init(AccessControlProxyPausable(msg.sender).config());
  ...
  function mint(address account, uint256 amount) external onlyRole(_MINTER_ROLE) {
    _mint(account, amount);
  }

  function burn(address account, uint256 amount) external onlyRole(_MINTER_ROLE) {
  ...
  function swap(address from, address to, uint256 amount) public onlyRole(_TOKEN_ADMIN_ROLE) {
