...
import { IPGRegistry } from "./interfaces/IPGRegistry.sol";
...
contract PGNetworkRegistry is IXReceiver, IPGRegistry, MemberRegistry, OwnableUpgradeable {
    ...
    modifier onlyOwnerOrUpdater() {
        require(
            (updater != address(0) && msg.sender == address(connext)) ||
            owner() == _msgSender(),
            "PGNetworkRegistry: caller is not foreign updater or owner"
        );
        _;
    }

    modifier enoughForRelayerFees(uint256[] memory _relayerFees) {
        ...
        require(
            ...
            "PGNetworkRegistry: Value sent not enough to cover relayer fees"
        );
        _;
    }
    ...
    function initialize(
        ...
        updater = _updater;
        splitsMain = ISplitMain(_splitsMain);
        split = _split;
    }
    ...
    function _validateNetworkRegistry(NetworkRegistry storage _registry) internal view {
        require(_registry.domainId != 0, "PGNetworkRegistry: Not Supported Network");
        require(_registry.registryAddress != (address(0)), "PGNetworkRegistry:Not Supported Network");
        require(_registry.delegate != (address(0)), "PGNetworkRegistry: Missing delegate");
    }
    ...
    ) external payable onlyOwner enoughForRelayerFees(_relayerFees) {
        require(_chainIds.length == _relayerFees.length, "PGNetworkRegistry: Wrong network params");

        setNewMember(_member, _activityMultiplier, _startDate);

        for (uint8 i = 0; i < _chainIds.length; i++) {
            uint32 chainId = _chainIds[i];
            _validateNetworkRegistry(networkRegistry[chainId]);
            uint256 relayerFee = _relayerFees[i];
            bytes memory callData = abi.encodeWithSelector(IPGRegistry.updateMember.selector, _member, _activityMultiplier);
            connext.xcall{value: relayerFee}(
                networkRegistry[chainId].domainId, // _destination: domain ID of the destination chain
                networkRegistry[chainId].registryAddress,            // _to: address of the target contract (Pong)
                address(0),        // _asset: use address zero for 0-value transfers
                networkRegistry[chainId].delegate,        // _delegate: address that can revert or forceLocal on destination
    ...
    function syncUpdateMember(address _member, uint8 _activityMultiplier, uint32[] memory _chainIds, uint256[] memory _relayerFees)
        external payable
        onlyOwner
    {
        require(_chainIds.length == _relayerFees.length, "PGNetworkRegistry: Wrong network params");
    ...
    function xReceive(
        ...
        // TODO:
        bytes4 functionSelector = abi.decode(_callData, (bytes4));
        if (functionSelector == IPGRegistry.setNewMember.selector) {
            (, address _member, uint8 _activityMultiplier, uint32 _startDate) = abi.decode(_callData, (bytes4, address, uint8, uint32));
            setNewMember(_member, _activityMultiplier, _startDate);
            return bytes("");
        }
        if (functionSelector == IPGRegistry.updateMember.selector) {
            (, address _member, uint8 _activityMultiplier) = abi.decode(_callData, (bytes4, address, uint8));
            updateMember(_member, _activityMultiplier);
            return bytes("");
        }
        return bytes("");
...
interface IMultichainMemberRegistry {
    function syncSetNewMember(address _member, uint8 _activityMultiplier, uint32 _startDate, uint32[] memory _chainIds, uint256[] memory _relayerFees) external payable;
    function syncUpdateMember(address _member, uint8 _activityMultiplier, uint32[] memory chainIds, uint256[] memory relayerFees) external payable;
}

// SPDX-License-Identifier: MIT
pragma solidity ^0.8.13;

import { IMultichainMemberRegistry } from "./IMultichainMemberRegistry.sol";

interface IPGRegistry is IMultichainMemberRegistry {
