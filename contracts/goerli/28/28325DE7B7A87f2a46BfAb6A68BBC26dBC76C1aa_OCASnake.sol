...
contract OCASnake is Ownable, ERC721A, ERC2981, ReentrancyGuard {
    ...
    bool private arcadeIsOpen = true;
    ...
    bool private turnOnThePower = true;
    ...
    function allYourBaseAreBelongToUs(uint256 tokenId, uint256 variant)
        private
        view
        returns (string memory)
    {
        ...
        string[30] memory colors = generateAllColors(tokenId, variant);
    ...
    function image_funk(uint256 tokenId, uint256 variant)
        internal
        view
        returns (bytes memory)
    {
        bytes memory svgData = cloneSnake(tokenId, variant);
        return
            turnOnThePower
                ? (
                    abi.encodePacked(
                        allYourBaseAreBelongToUs(tokenId, variant),
                        '","image_data": "',
                        svgData
                    )
                )
    ...
    function generateAllColors(uint256 tokenId, uint256 variant)
        private
        view
        returns (string[30] memory)
    {
        string[30] memory colors = retrofy(tokenId, variant);
        uint256 rn = (generateAppleSeed(tokenId, 420 * variant, true) %
            (colors.length - 10)) + 10;
    ...
    function generateColorArray(
        ...
        for (uint256 index = 0; index < colors.length - 1; index++) {
            colors[index] = i == 1
                ? generateColors(tokenId, changes[index] + variant)
                : generateColors(tokenId, changes[index] + variant + i + index);
        }
        uint256 o = uint256(generateAppleSeed(tokenId, variant + i, true) + i);
        colors[3] = uint256(o) % uint256(opacity.length);
        return colors;
    }

    function retrofy(uint256 tokenId, uint256 variant)
        internal
        view
        returns (string[30] memory)
    {
        ...
        for (uint256 i = 0; i < parts.length; i++) {
            uint256[4] memory colors = generateColorArray(tokenId, i, variant);
    ...
    function cloneSnake(uint256 tokenId, uint256 variant)
        private
        view
        returns (bytes memory)
    {
        string[30] memory selectedColors = generateAllColors(tokenId, variant);
            ...
            svgData = abi.encodePacked(
                ...
                createPixel(
                    selectedColors[
                        generateAppleSeed(
                            tokenId,
                            420 * tokenInfoList[tokenId].variant,
                            true
                        ) % uint256(19)
                    ],
                    250 * i,
            ...
            funkPictureCreator(
                [
                    selectedColors[
                        generateAppleSeed(
                            tokenId,
                            69 * tokenInfoList[tokenId].variant,
                            true
                        ) % uint256(19)
                    ],
                    selectedColors[
                        generateAppleSeed(
                            tokenId,
                            23 * tokenInfoList[tokenId].variant,
                            true
                        ) % uint256(19)
                    ],
                    selectedColors[
                        generateAppleSeed(
                            tokenId,
                            7 * tokenInfoList[tokenId].variant,
                            true
                        ) % uint256(19)
                    ],
                    selectedColors[1],
                    selectedColors[
                        generateAppleSeed(
                            tokenId,
                            143 * tokenInfoList[tokenId].variant,
                            true
                        ) % uint256(19)
                    ],
                    selectedColors[
                        uint256(
                            generateAppleSeed(
                                tokenId,
                                1337 * tokenInfoList[tokenId].variant,
                                true
                            )
                        ) % uint256(19)
                    ],
    ...
    function getVariantSize(uint256 tokenId) private view returns (uint256) {
        return uint256(generateAppleSeed(tokenId, 69, false) % uint256(9)) + 1;
    }

    function changeVariant(uint256 tokenId, uint256 variant)
        public
        callerIsSender
    {
        bool isTokenOwner = msg.sender == ownerOf(tokenId);
        require(isTokenOwner, "Only the owner can change the variant");
        require(
            variant <= getVariantSize(tokenId),
            string(
                abi.encodePacked(
                    "Variant must be between 0 and ",
                    getVariantSize(tokenId)
                )
            )
        );
        tokenInfoList[tokenId].variant = variant;
    }
    ...
    function getSignature() public pure returns (bytes32) {
        return msg.sig;
    }
    ...
    function stakeToken(uint256 tokenId) public callerIsNeo {
        //coming from contract address
        //   require(msg.sender == , "Only the owner can stake tokens.");
        // require(
        //     tokenInfoList[tokenId].stakeDate == 0,
        ...
        returns (bytes memory)
    {
        ...
        string[3] memory metadataVars = [
            ...
            string(image_funk(tokenId, tokenInfoList[tokenId].variant)),
            Strings.toString(tokenId)
        ];

        string[30] memory colors = retrofy(
            tokenId,
            tokenInfoList[tokenId].variant
        );
