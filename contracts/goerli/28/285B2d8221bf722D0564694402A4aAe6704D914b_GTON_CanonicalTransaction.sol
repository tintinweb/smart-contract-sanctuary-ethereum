...
/* Interface Imports */
import { iGTON_CanonicalTransaction } from "./iGTON_CanonicalTransaction.sol";
...
contract GTON_CanonicalTransaction is iGTON_CanonicalTransaction, Lib_AddressResolver {
    /*************
     * Constants *
     *************/
    ...
    modifier onlyManager {
        require(
            ...
            "GTON_CanonicalTransaction: Function can only be called by the GTON_MANAGER."
        );
        _;
    }
    ...
    receive() external payable {
        // msg.sender
        if (msg.sender == resolve('GTON_DiscountOracle')) {
            uint256 _chainId = getAddressChainId(msg.sender);
            if (_chainId > 0) {
                address _to = resolve(string(abi.encodePacked(uint2str(_chainId),"_GTON_Sequencer_Wrapper")));
    ...
    */
    ...
    function appendSequencerBatchByChainId() override public {
        ...
        require(
            msg.sender == resolve(string(abi.encodePacked(uint2str(_chainId),"_GTON_Sequencer_Wrapper"))),
 ...
 */
interface iGTON_CanonicalTransaction {
...
contract StateCommitmentChain is IStateCommitmentChain, Lib_AddressResolver {
    ...
    uint256 public DEFAULT_CHAINID = 1000;
    ...
    function setFraudProofWindow (uint256 window) public {
        require (msg.sender == resolve("GTON_MANAGER"), "now allowed");
    ...
    function appendStateBatch(bytes32[] memory _batch, uint256 _shouldStartAtElement) public {
        ...
        //appendStateBatchByChainId(DEFAULT_CHAINID, _batch, _shouldStartAtElement, "1000_GTON_Proposer");
    }
        ...
        public
    {
        require(
            msg.sender == resolve(
              string(abi.encodePacked(uint2str(_chainId),"_GTON_FraudVerifier"))),
            "State batches can only be deleted by the GTON_FraudVerifier."
        );
