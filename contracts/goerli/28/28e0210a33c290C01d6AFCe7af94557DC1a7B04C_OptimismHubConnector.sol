...
contract OptimismHubConnector is HubConnector, BaseOptimism {
  ...
  function processMessageFromRoot(
    ...
    // get the data
    // the _message should be the `HubConnector.processMessage` calldata (100 bytes):
    // - 4 byte selector
    // - 32 byte merkle root
    // - 32 byte length for `bytes memory` including root
    // - 32 byte length for `bytes memory` including data
