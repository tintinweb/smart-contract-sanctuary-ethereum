...
import {ERC721A} from "./tokens/ERC721/ERC721A.chiru.sol";
import {Paid} from "./access/Paid.sol";
...
contract CrypTrees is 
    ERC721A, 
    OwnableOpenSeaDefaultOperatorFilterer, 
    Paid 
{
    ...
    uint256 public constant OPEN_MINT_PRICE = 0.005 ether;
    ...
    constructor(
        ...
        uint maxSupply_,
        address allowedSeaDrop_
    )
        ERC721A(name_, symbol_)
    {
        _allowedSeaDrop[allowedSeaDrop_] = true;
    ...
    function contractURI() external pure returns(string memory) {
        return "ipfs://bafkreig7cg7wshoiew2sk3fg7fswljpcbbijrnhh5lftihmepf4folhtz4";
    }

    function dropURI() external pure returns(string memory) {
        return "ipfs://bafkreidijgndlxvunqwtfmsgg7j5uduzoqhblimsyp3ff6chgm3x6lkbl4";
    }
    ...
    function mint() external payable {
        if (msg.value >= OPEN_MINT_PRICE) {
            _mintGuard(msg.sender, msg.value / OPEN_MINT_PRICE);
        }
    }
     ...
     * - be sure to send 0.05 * quantity eth 
     * - max of 29 quantity per transaction
     */
    function mint(uint quantity_) external payable onlyPaid(OPEN_MINT_PRICE * quantity_) {
    ...
    function safeMint(
        ...
        external payable 
        onlyPaid(OPEN_MINT_PRICE * quantity_)
    {
    ...
    {
        _safeMintGuard(minter, quantity, "");
    }
    ...
    // =============================================================
    //           OPENSEA OPERATOR FILTERED ONLY INTERFACE
    // =============================================================
    /**
     * @dev Filtered paid open mint
     * - 'batch mint to'
     * - this is to enable the mint live on OpenSea as well
     * - use the safeMint functions, they offer the same functionality
     * - max of 29 quantity per transaction
     */
    function mintTo(
        address to_, 
        ...
        external
        onlyAllowedOperatorOrOwner() 
    {
        _mintGuard(to_, quantity_);
    }

    /**
     * Restricted to only Operators on the OpenSea
     * Operator Filter Registry. 
     * - This effectively means post-mint, the trading
     *   must be done using an OpenSea friendly platform.
     * - NOTE: This contract has the ability to permanently
     *         remove this restriction at any time.
     */
    ...
    function _mintGuard(
        ...
        require(totalSupply() + quantity_ < _maxSupply + 1, "No more");
        require(quantity_ < MAX_PER_TX + 1, "Max per TX reached.");
    ...
    function _baseURI() internal pure override returns (string memory) {
        return "ipfs://bafybeidd5a32kckhwgo6akthh6jbkvr5dyrwt5lhkxaig6wv3dk4tubaha/";
    }
}
