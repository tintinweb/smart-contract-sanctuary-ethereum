...
contract PrizeDistributionFactory is Initializable, Manageable {
    ...
    /// @notice Emitted when a new Prize percentage is set.
    /// @param prizePercentage The prize percentage set
    event PrizePercentageSet(uint256 prizePercentage);

    /// @notice Emitted when a new Automatic calculation state is set.
    /// @param useAutomaticCalculation The state of the automatic calculation
    event AutomaticCalculationSet(bool useAutomaticCalculation);
    ...
    /// @notice The address of the prize distributor that holds the obtained tickets
    address public prizeDistributor;

    /// @notice This is the maximum percentage 100%
    uint256 private MAX_PERCENTAGE;

    /// @notice The prize percentage to be used for the draw, it takes it from the distributor
    ...
    /// @notice Indicates if the prize of the next draw will be calculated from a percentage of the total available in the distributor
    bool public useAutomaticCalculation;
    ...
    function initialize(
        ...
        MAX_PERCENTAGE = 100 ether;
        require(_owner != address(0), "PDC/owner-zero");
        require(address(_prizeDistributor) != address(0), "PDC/pd-zero");
        ...
        require(
            _prizePercentage > 0 && _prizePercentage <= MAX_PERCENTAGE,
            "PDC/pp-invalid"
        );
    ...
    /**
     * @notice Sets the prize percentage that will be assigned to the next draw, it takes the percentage from the total available of the prize distributor.
     * @param _prizePercentage The percentage input should be in the format of 1e18, 100% is 100 ether
     */
    function setPrizePercentage(uint256 _prizePercentage) external onlyOwner {
        ...
        emit PrizePercentageSet(_prizePercentage);
    }

    /**
     * @notice Sets the automatic calculation, it can be used to enable it or disable it, when disabled it takes the prize from the prize tier history.
     * @param _useAutomaticCalculation The value to indicate if it will be enabled or disabled
     */
    function setAutomaticCalculation(bool _useAutomaticCalculation)
        external
        onlyOwner
    {
        useAutomaticCalculation = _useAutomaticCalculation;
        emit AutomaticCalculationSet(useAutomaticCalculation);
    }
        ...
        returns (IPrizeDistributionBuffer.PrizeDistribution memory)
    {
        ...
        uint256 balanceDistributor = ticket.balanceOf(prizeDistributor);
        uint256 prizeForDraw;
        // Overrides the prize from the prize tier
        if (useAutomaticCalculation) {
            prizeForDraw =
                (balanceDistributor * prizePercentage) /
                MAX_PERCENTAGE;
        } else {
            prizeForDraw = prizeTier.prize;
            require(prizeForDraw <= balanceDistributor, "PDF/prize-too-high");
        }
        require(prizeForDraw > 0, "PDF/zero-prize]");
