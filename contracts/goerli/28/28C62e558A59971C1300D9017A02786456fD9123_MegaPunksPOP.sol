...
contract MegaPunksPOP is ERC721A, Ownable, ReentrancyGuard {
    ...
    function _verify(bytes32 leaf, bytes32[] memory proof, bytes32 rootOf) internal pure returns (bool) {
        ...
        callerIsUser
    {
        ...
        require(
            _verify(_leaf(Strings.toString(allowance), payload), proof, allowlistMerkleRoot), "Invalid proof supplissed"
        );

        bytes32 node = keccak256(abi.encodePacked(payload, Strings.toString(allowance)));
