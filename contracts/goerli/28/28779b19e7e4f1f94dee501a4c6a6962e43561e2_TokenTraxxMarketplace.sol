    ...
    ErrorCodes
{
        ...
        override
    {
        ...
        require(_checkIfZero(totalListing), throwError(SECONDARY_SALE_NOT_SUPPORTED));
        require(
        ...
        external
    {
        ...
        emit ListingModified(_listingId, _params.buyoutPrice, _params.quantityToList);
    }
    ...
    function delegatedBuy(
        ...
        bool isPrivileged = validateBuyParams(_listingId, _currency, _price, _buyer, _quantityToBuy, _signatureTimestamp, signature);
    ...
    function buy(
        ...
        require(_listingId < listingIdTracker, throwError(LISTING_ID_NOT_FOUND));
        Listing memory targetListing = listings[_listingId];
        address buyer = _msgSender();

        // Validate passed params based on membership status and listing info
        bool isPrivileged = validateBuyParams(_listingId, _currency, _price, buyer, _quantityToBuy, _signatureTimestamp, signature);
     ...
     */ 
    function withdrawTreasury(address to, address currency) external onlyAdmin nonReentrant{
        _nonZeroAddress(to, ZERO_ADDRESS);
   ...
   /**
    ...
    function _withdraw(
        ...
        if(royaltyTreasury == address(this)){
            ...
            if (isNativeToken) {
                if(IWETH(nativeTokenWrapper).allowance(royaltyTreasury, address(this)) >= amount) {
                    bool ifWithdrawn = IWETH(nativeTokenWrapper).transferFrom(royaltyTreasury, address(this), amount);
                    if(ifWithdrawn)
                        (transferSuccess, ) = payable(to).call{value: amount}("");
                }   
            } else {
    ...
    function removeListing(uint256 listingId) external {
        ...
        require(listing.assetContract != address(0), throwError(LISTING_ID_NOT_FOUND));
    ...
    function validateBuyParams(uint256 _listingId,
        address _currency,
        uint256 _price,
        address _buyer,
        uint256 _quantityToBuy,
        ...
        returns (bool)
    {
        ...
        if(signature.length > 0) {
            require(block.timestamp <= _signatureTimestamp + 30 minutes,
                throwError(SIGNATURE_MUST_BE_USED_WITHIN_30_MINUTES)
            );
            Authenticator authenticatorContractInstance = Authenticator(_authenticatorAddress); 
            isPrivileged = authenticatorContractInstance.verify(authenticatorContractInstance.getPrivilegedSigner(),_buyer, _signatureTimestamp, signature);
        }

        // Check whether the settled total price and currency to use are correct.
        if(isPrivileged) {
            require(_currency == targetListing.currency &&
                _price >= targetListing.buyoutPrice - ((targetListing.buyoutPrice * primaryFeeBps) / MAX_BPS),
        ...
        if (listings[_listingId].tokenType == TokenType.ERC721) {
            ...
            //tokenowner at the time of listing is the current owner
            require(
                IERC1155(targetListing.assetContract).balanceOf(targetListing.tokenOwner, listings[_listingId].tokenId) >= _quantityToBuy,
                throwError(INVALID_TOKEN_OWNER)
            );
        }
        return isPrivileged;
    ...
    function _checkIfZero(uint _toCheck) private pure returns(bool) {
        return _toCheck == 0;
    }
    ...
    /// @dev The address of the privileged txn signer
    address private _privilegedSigner;
