...
interface IMarketGovernance is IERC165 {
    ...
    function getCurrentBoostingContract() external view returns(address tokenAddress);
    function getBoostingPercentage() external view returns(uint256 percentage);
}
...
contract ZoraModuleManager {
    ...
    /// @notice Mapping of owners to their erc721 contracts
    /// @dev User address => NFT addresses
    mapping(address => address[]) private ownerToErc721;

    /// @notice Mapping of erc721 contracts to market approved status
    /// @dev Erc721 address => Approved
    mapping(address => bool) private marketErc721;
    ...
    /// @notice Emitted when the market lists new erc721 contract
    event ListedNFTCollection(
        address creator,
        address erc721
    );
    ...
    function importErc721IntoMarket(address _address) external onlyRegistrar {
        ownerToErc721[msg.sender].push(_address);
        marketErc721[_address] = true;
        emit ListedNFTCollection(msg.sender, _address);
    }

    function getOwnCollections() external view returns (address[] memory) {
        return ownerToErc721[msg.sender];
    }

    function isMarketErc721(address _nft) external view returns (bool) {
        return marketErc721[_nft];
    }
