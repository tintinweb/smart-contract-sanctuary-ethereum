...
import "../depend/LiquityMath.sol";
import "../depend/BaseMath.sol";
import "../interfaces/ICommunityIssuance.sol";
import "../interfaces/ICollateralManager.sol";

contract CommunityIssuance is OwnableUpgradeable, BaseMath, ICommunityIssuance {
    ...
    ICCVToken public concaveToken;
    ICollateralManager public collateralManager;

    // uint public totalLQTYIssued;
    // lpTokenAddr => totalLQTYIssued
    mapping (address => uint) lpTokenTotalLQTYIssued;
    uint public deploymentTime;

    // --- Events ---

    event LQTYTokenAddressSet(address _ccvTokenAddress);
    ...
    function setAddresses(
        ...
        CheckContract.isContract(_collateralManager);
        CheckContract.isContract(_concaveTokenAddress);

        collateralManager = ICollateralManager(_collateralManager);
        concaveToken = ICCVToken(_concaveTokenAddress);

        // When LQTYToken deployed, it should have transferred CommunityIssuance's LQTY entitlement
        uint LQTYBalance = concaveToken.balanceOf(address(this));
        // assert(LQTYBalance >= LQTYSupplyCap);

        emit LQTYTokenAddressSet(_concaveTokenAddress);
    }


    function issueLQTY(address _lpTokenAddress) external override returns (uint) {
        _requireCallerIsStabilityPool(_lpTokenAddress);

        uint latestTotalLQTYIssued = LQTYSupplyCap
            .mul(_getCumulativeIssuanceFraction())
            .div(DECIMAL_PRECISION);
        uint issuance = latestTotalLQTYIssued.sub(lpTokenTotalLQTYIssued[_lpTokenAddress]);

        lpTokenTotalLQTYIssued[_lpTokenAddress] = latestTotalLQTYIssued;
    ...
    function sendLQTY(address _lpTokenAddress, address _account, uint _LQTYamount) external override {
        _requireCallerIsStabilityPool(_lpTokenAddress);

        concaveToken.transfer(_account, _LQTYamount);
    }

    // --- 'require' functions ---

    function _requireCallerIsStabilityPool(address _lpTokenAddr) internal view {
        address _stabilityPoolAddress = collateralManager.find(_lpTokenAddr).stabilityPoolAddr;
        require(msg.sender == _stabilityPoolAddress, "CommunityIssuance: caller is not SP");
    }
}
