...
contract Contract is ContractStorage, UUPSUpgradeable, OwnableUpgradeable, ERC721HolderUpgradeable, ERC1155HolderUpgradeable {
  ...
  function addOrRefillTag(
    ...
    if (passedTag.tagType == TagType.LimitedOrOpenEdition) {
      ...
      // require (false, Strings.toString(tags[tagHash].numClaimed)); // Fails
      // require (false, Strings.toString(tags[tagHash].totalSupply)); // Fails
      // require (false, Strings.toString(passedTag.totalSupply)); // Fails
      // require (false, "test");
  ...
  function claimTag(
    ...
    require (tags[tagHash].tagType == TagType.HotPotato || tags[tagHash].tagType == TagType.Refillable1Of1 || tags[tagHash].numClaimed < tags[tagHash].totalSupply, 'not HotP or ReF & tot drained');
    require (tags[tagHash].tagType == TagType.HotPotato || tags[tagHash].tagType == TagType.Refillable1Of1 || tags[tagHash].claimsMade[recipient] < tags[tagHash].perUser, 'not HotP or ReF & pU drained');
