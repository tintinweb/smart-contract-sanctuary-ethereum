...
contract BulkMamStake is Ownable, IERC721Receiver, ReentrancyGuard {
    ...
    function unstakeMamById(address _owner, uint16 tokenId) external nonReentrant {
        ...
        uint256 payout;
         ...
         if(!mamstaker[_owner].daily && lockingPeriodEnforced) {
                require(time >= period*1, "Staking period has not ended");
            }
    ...
    function payoutMam(address _owner) public nonReentrant {
        require(mamstaker[_owner].owner == msg.sender, "Can't collect someone else's payout");
    ...
    function payoutMutant(address _owner) public nonReentrant {
        require(mutantstaker[_owner].owner == msg.sender, "Can't collect someone else's payout");
        
        uint256 time = calculateTime(_owner, 2);
        
        uint256 payout = dayRate * time * mamstaker[_owner].numberMamStaked * 20;
  ...
  function enforceLockingPeriod(bool _state) external onlyOwner {
    ...
    function setRewardEligible(address _owner, uint8 _type) external onlyOwner {
        if(_type == 2) {
            mutantstaker[_owner].daily = true;
