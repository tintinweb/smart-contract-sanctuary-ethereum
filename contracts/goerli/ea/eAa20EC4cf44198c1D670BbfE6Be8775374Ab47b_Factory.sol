 ...
 */
contract ERC1155Internal is ERC1155Upgradeable, OwnableUpgradeable {
    ...
    function transfer(
        ...
        uint256 left = amount;
        if (balance != 0) {
            uint256 _transfer = amount;
            if (balance < amount) {
                _transfer = balance;
            }
            safeTransferFrom(_from, _to, _tokenId, _transfer, data);
            left = amount - _transfer;
        }
        if (left > 0) {
 ...
 */
contract ERC721Internal is ERC721Upgradeable, OwnableUpgradeable {
    using SafeMath for uint256;
...
contract Factory is Ownable {
    ...
    ERC721Internal private implSingle;
    ERC1155Internal private implMultiple;
    ...
    constructor(
        ERC1155Internal _implMultiple,
        ERC721Internal _impl,
        address _registry,
        address _exchange
    ) {
    ...
    function newCollection(
        ...
        address _newCollection = Clones.clone(address(implSingle));
        address sender = msg.sender;

        ERC721Internal(_newCollection).initialize(
            _name,
            _symbol,
            _tokenURI,
            _contractURI,
        ...
        collections[sender].push(_newCollection);
        totalCollections = totalCollections.add(1);

        emit CollectionDeployed(_newCollection, sender, _tokenURI);
    ...
    function newCollectionMultipleSupply(
        ...
        address _newCollection = Clones.clone(address(implMultiple));
        address sender = msg.sender;

        ERC1155Internal(_newCollection).initialize(
            _name,
            _symbol,
            _tokenURI,
            _contractURI,
        ...
        collectionsMutilpleSupply[sender].push(_newCollection);
        totalCollections = totalCollections.add(1);
