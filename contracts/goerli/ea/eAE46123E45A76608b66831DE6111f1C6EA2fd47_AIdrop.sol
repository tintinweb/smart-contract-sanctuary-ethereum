...
contract AIdrop is IBEP20, Auth {
    ...
    string constant _name = "AIdrop1";
    string constant _symbol = "AIDROP1"; 
    ...
    function setSellFees(uint256 _liquidityFee, uint256 _reflectionFee, uint256 _marketingFee, uint256 _devFee) external authorized {
        require(SelltotalFee <= 8 * 10**16, "Total Sell Fees cannot exceed 7%"); // 10**16 represents the decimals
        SellliquidityFee = _liquidityFee;
        SellreflectionFee = _reflectionFee;
        SellmarketingFee = _marketingFee;
        SelltotalFee = _liquidityFee + (_reflectionFee) + (_marketingFee) + (_devFee);
    // The team has the ability to change the Sell Fees but those can never exceed 8%
    }
