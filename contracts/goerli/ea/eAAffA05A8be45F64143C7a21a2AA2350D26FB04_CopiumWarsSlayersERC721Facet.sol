    ...
    ICopiumWarsSlayers
{
    ...
    ///@inheritdoc ICopiumWarsSlayers
    function unlockBalance(uint256 amount) external payable {
        address account = msg.sender;
        uint requiredPayment = amount * unlockPrice(account);
...
interface ICopiumWarsSlayers is IERC721AUpgradeable, IERC173 {
    ...
    /**
     * @notice Resets approvals for a user if their unlock credit is 0
     * @dev This method should be accessible only by the owner
     * @param account Account to reset approvals for
     * @param operators Operators to remove
     */
    function adminResetApprovals(address account, address[] calldata operators) external;
