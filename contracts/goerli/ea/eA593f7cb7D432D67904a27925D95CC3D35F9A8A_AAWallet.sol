...
contract AAWallet {
    ...
    function executionFromEntryPoint(bytes memory callData) external view onlyEntryPoint returns(bytes memory destination, uint amount, address test1) {
        // unpacking callData.
        (destination, amount) = abi.decode(callData, (bytes, uint));
        test1 = address(uint160(bytes20(destination)));

        // main execution.
        // _call(destination, amount);

        return (destination, amount, test1);
    }
