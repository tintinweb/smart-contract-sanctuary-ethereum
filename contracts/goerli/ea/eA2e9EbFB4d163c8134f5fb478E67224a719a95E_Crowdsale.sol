...
contract Crowdsale is OwnableUpgradeable, PausableUpgradeable {
    ...
    struct SaleInfo {
        uint256 supply; // amount of tokens intended for sale
        uint256 price; // price per token
        string tokenUri; // URI of token metadata
        uint256 sold; // number of sold NFTs
        bool isActive; // whether sale is active
    }
    ...
    /**
     * @notice Amount of funds raised within all sales
     */
    uint256 public totalFundsRaised;

    /**
     * @notice List of sales
     */
    mapping(uint256 => SaleInfo) public sales;

    /**
     * @notice Number of sales in contract
     */
    ...
    function initialize(
        ...
        _addSale(6000, 1000e6, "ipfs://QmQmtbBdD1SvAAu6MfKmbk1t5hCVzPHnmti7SKqLZJwnqm"); // TODO set metadata URI
    }

    /**
     * @notice Changes fundraising wallet to new address
     * @param wallet_ New fundraising wallet address
     */
    ...
    function setCurrency(address currency_) external onlyOwner {
        require(currency_ != address(0), "Currency is the zero address");
    ...
    /**
     * @notice Adds new sale to crowdsale contract
     * @param supply Number of tokens to be sold
     * @param price Price per NFT in currency units
     * @param tokenUri Meratada URI for minted tokens
     */
    function addSale(uint256 supply, uint256 price, string calldata tokenUri) external onlyOwner {
    ...
    /**
     * @notice Activates sale
     * @param saleIndex Index of sale to be activated
     */
    function activateSale(uint256 saleIndex) external onlyOwner {
        require(sales[saleIndex].supply != 0, "Sale does not exist");

        sales[saleIndex].isActive = true;
    }

    /**
     * @notice Deactivates sale
     * @param saleIndex Index of sale to be deactivated
     */
    function deactivateSale(uint256 saleIndex) external onlyOwner {
        require(sales[saleIndex].supply != 0, "Sale does not exist");

        sales[saleIndex].isActive = false;
    }
     ...
     */
    function buyTokens(uint256 saleIdx, uint256 amount) external whenNotPaused {
        SaleInfo memory sale = sales[saleIdx];

        _validatePurchase(sale, amount);

        sales[saleIdx].sold = sale.sold + amount;
        _processPurchase(_msgSender(), amount, sale);
    }

    function _addSale(uint256 supply, uint256 price, string memory tokenUri) internal {
        uint256 newSalesCount = salesCount;
        sales[newSalesCount] = SaleInfo(supply, price, tokenUri, 0, false);
        salesCount = newSalesCount + 1;
    }

    /**
     * @dev Validation of an incoming purchase
     * @param sale Sale structure for which validate purchase
     * @param tokenAmount Token amount to purchase
     */
    function _validatePurchase(SaleInfo memory sale, uint256 tokenAmount) internal view virtual {
        require(sale.supply != 0, "Sale does not exist");
        require(sale.isActive == true, "Sale is inactive");
        require(sale.sold + tokenAmount <= sale.supply, "Too many tokens claimed");
     ...
     */
    function _processPurchase(address beneficiary, uint256 tokenAmount, SaleInfo memory sale) internal virtual {
        uint256 value = tokenAmount * sale.price;
        totalFundsRaised += value;

        emit TokensPurchased(beneficiary, value, tokenAmount);

        currency.safeTransferFrom(beneficiary, wallet, value);
        // TODO decide how purchase will be processed. Two options possible:
        //      - mint tokens directly to investor on purchase
        //      - mint tokens before crowdsale started (to chosen wallet) and transfer on purchase
        IERC721Mintable(token).mint(beneficiary, tokenAmount, sale.tokenUri);
    }
