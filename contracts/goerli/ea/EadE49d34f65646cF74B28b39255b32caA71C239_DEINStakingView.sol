...
contract DEINStakingView is IDEINStakingView, AbstractDependant, Initializable {
    using SafeMath for uint256;

    IDEINNFTStaking public deinNFTStaking;
    IStaking public deinStaking;

    mapping(uint256 => uint256) public voterMultipliers; // stake duration (months) => multiplier , for voter

    function __DEINStakingView_init() external initializer {
        _setVoterMultiplier();
    }
    ...
    function setDependencies(IContractsRegistry _contractsRegistry)
        ...
        onlyInjectorOrZero
    {
        deinStaking = IStaking(_contractsRegistry.getDEINStakingContract());
        deinNFTStaking = IDEINNFTStaking(_contractsRegistry.getDEINNFTStakingContract());
    }

    function _setVoterMultiplier() internal {
        voterMultipliers[1] = 1;
        voterMultipliers[6] = 3;
        voterMultipliers[12] = 10;
        voterMultipliers[24] = 40;
        voterMultipliers[36] = 100;
    }

    /// @notice returns APY% with 10**5 precision
    /// Max (General) APY
    function getAPY() external view override returns (uint256) {
        // multiply by Multiplier=5
        uint256 _initialStaked = INITIAL_STAKED_AMOUNT.mul(5);

        uint256 _totalPoolWithPower = deinStaking.totalPoolWithPower();

        if (_totalPoolWithPower == 0) {
            _totalPoolWithPower = _initialStaked;
        }
        ...
        return
            deinStaking
                ...
                .tryMul(10**7)
                .mul(_initialStaked)
                .uncheckedDiv(_totalPoolWithPower)
                .uncheckedDiv(INITIAL_STAKED_AMOUNT);
    }

    /// @notice returns APY% with 10**5 precision
    /// position APY
    function getPositionAPY(uint256 _tokenId) external view override returns (uint256) {
        if (!deinStaking.stakingHasBenefits(_tokenId)) return 0;

        uint256 _staked = deinStaking.getFullStakedAmount(_tokenId);
        uint256 _multiplier = deinStaking.getStakerMultiplier(_tokenId);

        uint256 _reward =
            deinStaking.rewardPerBlock().mul(BLOCKS_PER_YEAR).mul(_staked.mul(_multiplier)).tryDiv(
                deinStaking.totalPoolWithPower()
            );

        return _reward.mul(10**7).tryDiv(_staked);
    }

    ///@notice applied by voting multiplier (M2)
    function getVotingPower(uint256[] calldata _tokenIds)
        external
        view
        override
        returns (uint256 _totalVotingPower)
    {
        for (uint256 i = 0; i < _tokenIds.length; i = uncheckedInc(i)) {
            uint256 tokenID = _tokenIds[i];
            (uint256 _votingPower, ) = getNFTVotingPower(tokenID);
            _totalVotingPower = _totalVotingPower.add(_votingPower);
        }
    }
    ...
    {
        _votingPower = deinStaking.getFullStakedAmount(_tokenId);

        if (deinStaking.stakingHasBenefits(_tokenId)) {
            _voterMultiplier = voterMultipliers[deinStaking.getStakerLockingDuration(_tokenId)];
            _votingPower = _votingPower.mul(_voterMultiplier);
        }
    }

    // aggregate all user's nfts (staked and locked) power
    function getUserStakingPower(address _user)
        external
        view
        override
        returns (uint256 _totalStakingPower)
    {
        uint256[] memory _tokenIds = deinNFTStaking.getUserLockedNFTs(_user);
        uint256 _tokenId;
        for (uint256 i = 0; i < _tokenIds.length; i = uncheckedInc(i)) {
            _tokenId = _tokenIds[i];
            _totalStakingPower = _totalStakingPower.add(_getTokenStakingPower(_tokenId));
        }

        for (uint256 i = 0; i < deinStaking.balanceOf(_user); i = uncheckedInc(i)) {
            _tokenId = deinStaking.tokenOfOwnerByIndex(_user, i);
            _totalStakingPower = _totalStakingPower.add(_getTokenStakingPower(_tokenId));
        }
    }

    function _getTokenStakingPower(uint256 _tokenId) internal view returns (uint256) {
        if (!deinStaking.stakingHasBenefits(_tokenId)) return 0;

        uint256 _staked = deinStaking.getFullStakedAmount(_tokenId);
        uint256 _multiplier = deinStaking.getStakerMultiplier(_tokenId);

        return _staked.mul(_multiplier);
    }
}
