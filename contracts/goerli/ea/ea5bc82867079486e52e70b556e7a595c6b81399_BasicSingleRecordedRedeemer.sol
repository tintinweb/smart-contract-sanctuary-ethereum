...
/**
 * @notice Interface for a Voucher token that is redeemable through a redeemer
 * contract.
 */
interface IVoucherToken {
     ...
     * voucher.
     */
    error CallerNotOwnerNorApprovedForVoucher(uint256 tokenId);
     ...
     * @dev MUST revert with `RedeemerCallerNotOwnerNorApproved` if sender is
     * not allowed to spend tokenId.
     */
