...
// File: contracts/interfaces/IHUB.sol
...
// File: contracts/interfaces/ITopia.sol
...
// File: contracts/Wastelands/WastelandsGame.sol
...
contract WastelandsGame is Ownable, ReentrancyGuard {
    ...
    uint256 public DEV_FEE = .00005 ether;
    ...
    mapping(address => EnumerableSet.UintSet) StakedRatsOfWallet; // list of token IDs a user has staked

    mapping(uint16 => AStake) private AlphaStake;
    mapping(address => uint16) public NumberOfStakedAlphas;
    mapping(address => EnumerableSet.UintSet) StakedAlphasOfWallet;
    ...
    function getStakedAlphas(address _staker) external view returns (uint16[] memory) {
        uint16 length = uint16(StakedAlphasOfWallet[_staker].length());
        uint16[] memory stakedAlphas = new uint16[](length);
        for (uint16 i = 0; i < length;) {
            stakedAlphas[i] = uint16(StakedAlphasOfWallet[_staker].at(i));
            unchecked { i++; }
        }
        return stakedAlphas;
    }
    ...
    function stakeAlpha(uint16[] calldata _ids) external payable nonReentrant notContract() {
        ...
        for (uint i = 0; i < length;) {
            ...
            StakedAlphasOfWallet[msg.sender].add(_ids[i]);
            unchecked { i++; }
        }
    ...
    function claimAlphas(uint16[] calldata _ids, bool unstake) external payable nonReentrant notContract() {
        ...
        for (uint i = 0; i < length;) {
            ...
            if (unstake) {
                ...
                StakedAlphasOfWallet[msg.sender].remove(_ids[i]);
    ...
    function getTotalUnclaimedAlphaTopia(address _staker) external view returns (uint256 owed) {
        uint16 length = uint16(StakedAlphasOfWallet[_staker].length());
        owed = 0;

        for (uint16 i = 0; i < length;) {
            require(AlphaStake[uint16(StakedAlphasOfWallet[_staker].at(i))].owner == _staker , "not owner");

            if(block.timestamp <= claimEndTime) {
                owed += (block.timestamp - AlphaStake[uint16(StakedAlphasOfWallet[_staker].at(i))].value) * DAILY_ALPHA_RATE / PERIOD;
            } else if (AlphaStake[uint16(StakedAlphasOfWallet[_staker].at(i))].value < claimEndTime) {
                owed += (claimEndTime - AlphaStake[uint16(StakedAlphasOfWallet[_staker].at(i))].value) * DAILY_ALPHA_RATE / PERIOD;
            } else {
                owed += 0;
            }
            unchecked { i++; }
    ...
    function getStakedRats(address _staker) external view returns (uint16[] memory) {
        uint16 length = uint16(StakedRatsOfWallet[_staker].length());
        uint16[] memory stakedRats = new uint16[](length);
        for (uint16 i = 0; i < length;) {
            stakedRats[i] = uint16(StakedRatsOfWallet[_staker].at(i));
            unchecked { i++; }
        }
        return stakedRats;
    }
    ...
    function stakeRat(uint16[] calldata _ids) external payable nonReentrant notContract() {
        ...
        for (uint i = 0; i < length;) {
            ...
            StakedRatsOfWallet[msg.sender].add(_ids[i]);
            unchecked { i++; }
        }
    ...
    function claimRats(uint16[] calldata _ids, bool unstake) external payable nonReentrant notContract() {
        ...
        for (uint i = 0; i < length;) {
            ...
            if (unstake) {
                ...
                StakedRatsOfWallet[msg.sender].remove(_ids[i]);
    ...
    function getTotalUnclaimedRatTopia(address _staker) external view returns (uint256 owed) {
        uint16 length = uint16(StakedRatsOfWallet[_staker].length());
        owed = 0;

        for (uint16 i = 0; i < length;) {
            require(RatStake[uint16(StakedRatsOfWallet[_staker].at(i))].owner == _staker , "not owner");

            if(block.timestamp <= claimEndTime) {
                owed += (block.timestamp - RatStake[uint16(StakedRatsOfWallet[_staker].at(i))].value) * DAILY_RAT_RATE / PERIOD;
            } else if (RatStake[uint16(StakedRatsOfWallet[_staker].at(i))].value < claimEndTime) {
                owed += (claimEndTime - RatStake[uint16(StakedRatsOfWallet[_staker].at(i))].value) * DAILY_RAT_RATE / PERIOD;
            } else {
                owed += 0;
            }
            unchecked { i++; }
