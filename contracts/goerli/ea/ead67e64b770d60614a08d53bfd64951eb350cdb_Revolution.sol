...
contract Revolution is Ownable {
    ...
    uint16 public SPLIT_BPS; //50%
    IERC721VotesStripped public immutable daoToken;
    address payable public immutable daoTreasury;
    ...
    modifier hasVotes() {
        require(
            daoToken.getVotes(msg.sender) >= MIN_VOTES,
            "Does not have enough dao votes"
        );
        _;
    }

    function hasVotesTest() external view hasVotes returns (uint64) {
        return 42;
    }

    function setMinVotes(uint256 newMinVotes) public onlyOwner {
        MIN_VOTES = newMinVotes;
        emit MinVotesChanged(MIN_VOTES);
    }

    function setSplit(uint16 newSplit) public onlyOwner {
        SPLIT_BPS = newSplit;
    ...
    function getQueuedDrop() external view returns(QueuedDrop memory _queuedDrop){
        return queuedDrop;
    }

    function rejectQueuedDrop() external onlyOwner {
        delete queuedDrop;
    }

    function deployRevolutionDrop() external onlyOwner returns (address) {
...
import {ERC721AUpgradeable} from "../interfaces/ERC721AUpgradeable.sol";
...
import {IERC2981Upgradeable} from "openzeppelin-contracts-upgradeable/contracts/interfaces/IERC2981Upgradeable.sol";
