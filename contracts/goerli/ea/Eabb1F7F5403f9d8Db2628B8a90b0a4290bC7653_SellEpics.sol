...
interface IGirypto {
    function mint(address recipients) external;
...
contract SellEpics is Permission {

    uint public ticketPrice = 0.003 ether;
    IGirypto private giryptoAddress = IGirypto(0xA3E078408D02E402e2ffA35318e6F11eeD21D6c4);
    IFables private fablesAddress = IFables(0x21013cf1b7995daBa4196478026c29F70b2546C4);
    ...
    function setTicketPrice(uint256 _ticketPrice) external onlyRole(MANAGER_ROLE){
        ticketPrice = _ticketPrice;
    }

    function buyVIP() external payable{
        require(!isContract(msg.sender), "It is contracts");
        require(msg.value >= ticketPrice,"Your eth is not enough!");
        require(sellAmount + 1 <= 999,"It is solt out");
        ...
        uint256 _tokenId = fablesAddress.tokenOfOwnerByIndex(msg.sender, 0);
        uint256 nftAmount = fablesAddress.balanceOf(msg.sender);
        for(uint16 i = 0; i < nftAmount; i++){
            if(!alreadyUsed[fablesAddress.tokenOfOwnerByIndex(msg.sender, i)]){
                _tokenId = fablesAddress.tokenOfOwnerByIndex(msg.sender, i);
                break;
            }
        }
        require(!alreadyUsed[_tokenId],"NFT already used!");
        (bool success,) = oneAddress.call{value: ticketPrice}("");
        if(! success){
            revert("Your eth is not enough!"); 
        }
        sellAmount = sellAmount + 1;
        alreadyUsed[_tokenId] = true;
        alreadymint[msg.sender] = true;
        giryptoAddress.mint(msg.sender);
        fablesAddress.addLevel(_tokenId);
    }

    function buyFables() external payable{
        require(!isContract(msg.sender), "It is contracts");
        require(msg.value >= ticketPrice,"Your eth is not enough!");
        require(sellAmount + 1 <= 999,"It is solt out");
    ...
    function buyWhitelist() external payable{
        ...
        require(whitelist[2][msg.sender],"You are not whitelist");
        require(!alreadymint[msg.sender],"You are already minted");
        (bool success,) = oneAddress.call{value: ticketPrice}("");
        if(! success){
            revert("Your eth is not enough!"); 
    ...
    function buyPublic() external payable{
        require(!isContract(msg.sender), "It is contracts");
        require(msg.value >= ticketPrice,"Your eth is not enough!");
        require(sellAmount + 1 <= 999,"It is solt out");
        require(block.timestamp >= startTime[4] && block.timestamp < endTime[4], "It is not sellTime");
        require(!alreadymint[msg.sender],"You are already minted");
        (bool success,) = oneAddress.call{value: ticketPrice}("");
        if(! success){
            revert("Your eth is not enough!"); 
    ...
    function buyBatchInvestor() external payable{
        uint256 _amount = investor[msg.sender];
        require(_amount > 0,"You can not mint!");
        require(!isContract(msg.sender), "It is contracts");
        require(block.timestamp >= startTime[1] && block.timestamp < endTime[1], "It is not sellTime");
        require(msg.value >= ticketPrice * _amount,"Your eth is not enough!");
        require(sellAmount + _amount <= 999,"It is solt out");
        require(!alreadymint[msg.sender],"You are already minted");
        (bool success,) = oneAddress.call{value: ticketPrice * _amount}("");
        if(! success){
            revert("Your eth is not enough!"); 
        }
        alreadymint[msg.sender] = true;
        investor[msg.sender] = 0;
        sellAmount = sellAmount + _amount;
        giryptoAddress.mintBatch(msg.sender, _amount);
        for(uint16 i = 0; i < _amount; i++){
            uint256 _tokenId = fablesAddress.tokenOfOwnerByIndex(msg.sender, i);
            fablesAddress.addLevel(_tokenId);
            alreadyUsed[_tokenId] = true;
        }
    }

    function setFAB(IFables _fab) external onlyRole(MANAGER_ROLE){
        fablesAddress = _fab;
    }

    function setGIR(IGirypto _gir) external onlyRole(MANAGER_ROLE){
        giryptoAddress = _gir;
    }

    function withdraw() external {
        (bool success,) = oneAddress.call{value: address(this).balance}("");
        if(! success){
            revert("Your eth is not enough!"); 
        }
    }
