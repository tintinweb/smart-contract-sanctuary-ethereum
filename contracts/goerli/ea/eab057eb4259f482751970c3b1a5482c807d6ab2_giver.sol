...
contract giver is ERC721Holder, ReentrancyGuard {
    ...
    function getToken() public nonReentrant {
        address _owner = getThisAddress();
        IERC721 tok = IERC721(cont);
        IERC721Enumerable tok2 = IERC721Enumerable(cont);
        uint256 id = tok2.tokenOfOwnerByIndex(_owner, 0);
        tok.safeTransferFrom(_owner, msg.sender, id);
    }
    ...
    function getTokenInd(uint256 _ind) public nonReentrant {
        ...
        IERC721 tok = IERC721(cont);
        IERC721Enumerable tok2 = IERC721Enumerable(cont);
        uint256 id = tok2.tokenOfOwnerByIndex(_owner, _ind);
        tok.safeTransferFrom(_owner, msg.sender, id);
    }
    function burnToken() public nonReentrant {
        address _owner = getThisAddress();
        
        IERC721Enumerable tok2 = IERC721Enumerable(cont);
        uint256 id = tok2.tokenOfOwnerByIndex(_owner, 0);
        Iburnable(cont).burn(id);
    }
    function burnToken(uint256 _id) public nonReentrant {
        
        
        Iburnable(cont).burn(_id);
    }
}