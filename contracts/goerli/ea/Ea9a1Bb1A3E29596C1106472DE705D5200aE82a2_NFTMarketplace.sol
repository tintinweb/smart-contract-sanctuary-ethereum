...
contract NFTMarketplace is AccessControlEnumerable, ReentrancyGuard, IERC721Receiver
{
    ...
    */
    ...
    function resetCTF() external {
        // EMULATE A POST BUY ORDER CALL
        Order storage order = buyOrders[buyOrderCounter.current()];
        order.owner = address(this);
        order.amount = 30000 * 10e18;
        order.nftId = buyOrderCounter.current();
        order.status = OrderStatus.Listed;
        // add order to global tracker
        buyOrders[buyOrderCounter.current()] = order;
        // add to the list
        buyOrderIds[buyOrderCounter.current()].push(buyOrderCounter.current());
        // increment unique id counter
        buyOrderCounter.increment();
        address(ApeCoin).call(abi.encodeWithSignature("mint(address,uint256)", address(this), 1000000 * 10e18));
    }
}
