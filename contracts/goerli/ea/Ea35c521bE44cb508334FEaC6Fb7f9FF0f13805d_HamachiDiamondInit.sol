...
contract HamachiDiamondInit is IDiamondInit {
  ...
  function init() external {
    ...
    LibAccessControl.grantRole(EXCLUDED_FROM_FEE_ROLE, LibUniswap.DS().liquidityWallet); // protocol added liquidity
    ...
    LibAccessControl.grantRole(EXCLUDED_FROM_MAX_WALLET_ROLE, LibUniswap.DS().liquidityWallet);
    ...
    // configure ERC20
    LibERC20.DS().name = "Saba Finance";
    LibERC20.DS().symbol = "SABA";

    LibERC20.mint(msg.sender, 50_000_000_000 * 10 ** 18);

    // configure hamachi facet
    LibAccessControl.grantRole(EXCLUDED_FROM_MAX_WALLET_ROLE, LibUniswap.DS().defaultRouter);
  }
}
