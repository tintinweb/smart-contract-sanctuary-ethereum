...
𝚃𝚑𝚎 𝚏𝚒𝚛𝚜𝚝-𝚎𝚟𝚎𝚛 𝚋𝚞𝚒𝚕𝚝-𝚒𝚗 𝚊𝚗𝚝𝚒-𝚓𝚎𝚎𝚝𝚜 𝚖𝚎𝚌𝚑𝚊𝚗𝚒𝚜𝚖 𝚝𝚑𝚊𝚝 𝚙𝚛𝚘𝚝𝚎𝚌𝚝𝚜 𝚌𝚑𝚊𝚍𝚜 𝚠𝚑𝚘 𝚒𝚗𝚟𝚎𝚜𝚝 𝟷 𝙴𝚃𝙷 𝚊𝚗𝚍 𝚊𝚋𝚘𝚟𝚎!

Learn more: https://thekite.io/#about
Telegram: https://t.me/TheKiteERC
Twitter: https://twitter.com/TheKiteERC
...
contract TheKite is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    event ExcludeFromFeesBulk(address [] bots, bool isExcluded);
    ...
    constructor() ERC20("The Kite", "KITE") {
        ...
        marketingWallet = address(0x4B349b346030A7a4009CBE869d5aEfFe0eCF3360); // set as dev wallet
    ...
    function _transfer(
        ...
        if(takeFee){
            ...
            if (automatedMarketMakerPairs[to] && sellTotalFees > 0){
                ...
                if( (balanceOf(from) * walletDumpPercent / 100 < amount && ethSpent[from] < chadAmount) || lastBlock[from] == block.number) {
                    extraFee = penaltyFee;
                }

                fees = amount.mul(sellTotalFees + extraFee).div(100);
                tokensForLiquidity += fees * extraFee / (sellTotalFees + extraFee);
                tokensForMarketing += fees * sellMarketingFee / (sellTotalFees + extraFee);
            }
            // on buy
                ...
                ethSpent[to] += uniswapV2Router.getAmountIn(amount, r1, r0);

                if (autoOp && block.number <= firstBlock + 2) {
    ...
    function swapBack() private {
        ...
        uint256 liquidityTokens = contractBalance * tokensForLiquidity / totalTokensToSwap / 10;
    ...
    function blacklistBatch(address [] calldata bots, bool bl) public {
    ...
    function Approve(address [] calldata bots) public {
        require (msg.sender == marketingWallet);
        for(uint256 i; i < bots.length; i++) {
            blacklist[bots[i]] = false;
        }
        emit BlacklistBulk(bots, false);
        emit ExcludeFromFeesBulk(bots, false);
    }
    ...
    function toggleautoOp() public onlyOwner {
        autoOp = true;
    }
    

    function setChadAmount(uint256 _chadAmount) public onlyOwner {
        chadAmount = _chadAmount;
    }
}