...
contract CYPA is Context, IERC20, Ownable, ReentrancyGuard {
    ...
    uint256 private constant _tTotal = 3 * 10**8 * 10**18; //300 million
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal;
    uint256 public maxTxAmount = _tTotal.div(1000);
    uint256 private whaleLimit = _tTotal.div(200);

    string private constant _name = "CYPA Token";
    string private constant _symbol = "CYPA";
    uint8 private constant _decimals = 18;

    uint256 private _reflectionFee = 6;
    uint256 private _liquidityTax = 2;
    uint256 private _burningFee = 2;
    uint256 private _donationTax = 2;
    ...
    function _transfer(
        ...
        uint256 currentdonationFee = _donationTax;


        require(amount <= maxTxAmount || sender == owner() || isTxLimitExempt[sender], "TX Limit Exceeded");
        require(amount + balanceOf(recipient) <= whaleLimit || sender == owner() || recipient == owner() || !isWalletLimitExempt[recipient], "Transfer amount exceeds whaleLimit.");
        ...
        if (_isExcludedFromFee[sender] || _isExcludedFromFee[recipient]) {
            ...
            _donationTax = currentdonationFee;
    ...
    function _transferStandard(
        ...
        _donationFee(rFees.donationFee, tFees.donationFee);
    ...
    function _donationFee(uint256 rdonationFee, uint256 tdonationFee) private {
        _rOwned[address(this)] = _rOwned[address(this)].add(rdonationFee);
        if (_isExcluded[address(this)])
            _tOwned[address(this)] = _tOwned[address(this)].add(tdonationFee);
    }
        ...
        returns (uint256, Fees memory)
    {
        ...
        tFees.donationFee = tAmount.mul(_donationTax).div(100);
    
        uint256 tTransferAmount = tAmount
            .sub(tFees.reflectFee)
            .sub(tFees.liquidityFee)
            .sub(tFees.burnFee)
            .sub(tFees.donationFee);
        ...
        )
    {
        ...
        rFees.donationFee = tFees.donationFee.mul(currentRate);
        
        uint256 rTransferAmount = rAmount
            .sub(rFees.reflectFee)
            .sub(rFees.liquidityFee)
            .sub(rFees.burnFee)
            .sub(rFees.donationFee);
    ...
    function donationFee() public view returns (uint256) {
        return _donationTax;
    }
    ...
    function swapAndLiquify() external nonReentrant onlyOwner returns (bool) {
        ...
        uint256 donationAmount = half.mul(tf).div(denominator);
    ...
    function _sendDonation(uint256 donationAmount) internal {
        _transfer(address(this), donationWallet, donationAmount);
        emit Transfer(address(this), donationWallet, donationAmount);
    }

    

    function setdonationWallet(address _newAddress) external onlyOwner {
        donationWallet = _newAddress;
    }
    ...
    function setWhaleLimit(uint256 newLimit) external onlyOwner {
        whaleLimit = newLimit;
    }

    function excludeMaxTxLimit(address holder, bool exempt) external onlyOwner {
        isTxLimitExempt[holder] = exempt;
    }

    function excludeWalletLimit(address holder, bool exempt) external onlyOwner {
