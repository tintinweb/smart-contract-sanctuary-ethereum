...
contract Pool is IPool, OwnableUpgradeable, Governor {
    ...
    function getTVL() public returns (uint256) {
        ...
        address[] memory tokenWhitelist = IWhitelistedTokens(whitelistedTokensContract).tokenWhitelist();
        uint256 tvl;
            ...
            if (tokenWhitelist[i] == address(0)) {
                ...
                if (balance > 0) {
                    tvl += quoter.quoteExactInput(
                        IWhitelistedTokens(whitelistedTokensContract).tokenSwapPath(tokenWhitelist[i]),
                        balance
                    );
                }
            }
