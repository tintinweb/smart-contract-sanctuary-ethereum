...
import {
    ...
    IERC721Supply,
    IMoonbirds
} from "grails/season-03/Grails3MintPass.sol";
import {Grails3, IGrailsRoyaltyRouter} from "grails/season-03/Grails3.sol";
import {GrailBirds} from "grails/season-03/GrailBirds.sol";

import {ERC721ATransferRestrictedBase} from
    "grails/season-03/ERC721ATransferRestrictedBase.sol";

contract DevGrails3MintPass is Grails3MintPass {
    constructor(
        ...
        Grails3MintPass(
            "DevGrails III Mint Pass",
            "DG3PASS",
            "https://storage.googleapis.com/grails-mint-pass/season-03/metadata.json",
            royaltyReceiver,
            proof,
            moonbirds
        )
    ...
    function devmint(address[] calldata recipients) public {
        for (uint256 i; i < recipients.length; ++i) {
            devmint(recipients[i], 1);
        }
    }

    function devSetTransferRestriction(TransferRestriction restriction)
        public
    {
        _transferRestriction = restriction;
    }

    function nuke() public onlyOwner {
        selfdestruct(payable(msg.sender));
    }
}

contract DevGrails3 is Grails3 {
    constructor(DevGrails3MintPass pass, IGrailsRoyaltyRouter router)
        Grails3(
            "DevGrails III",
            "DG3",
...
/**
 * @notice A library that provides the tokenIDs for all moonbirds with the
 * "Grail" headwear trait.
 */
library GrailBirds {
    /**
     * @notice The number of moonbirds with grail trait.
     */
    uint256 internal constant _NUM_GRAILBIRDS = 176;

    /**
     * @notice Returns the tokenIds of moonbirds with the grail trait.
     * @dev Returned as statically sized array.
     */
    function grailBirdTokenIds()
        internal
        pure
        returns (uint16[_NUM_GRAILBIRDS] memory)
    {
        return [
            74,
            132,
    ...
    /**
     * @notice Returns the tokenIds of moonbirds with the grail trait.
     * @dev Returned as dynamically sized array.
     */
    function grailBirdTokenIdsDyn()
        internal
        pure
        returns (uint256[] memory birdIds)
    {
        uint16[_NUM_GRAILBIRDS] memory constIds = grailBirdTokenIds();
        birdIds = new uint[](_NUM_GRAILBIRDS);
        for (uint256 i; i < _NUM_GRAILBIRDS; ++i) {
            birdIds[i] = constIds[i];
        }
    }
}
...
import {OnlyOnce} from "grails/season-03/OnlyOnce.sol";
import {NextShuffler} from "./NextShuffler.sol";
    ...
    DefaultOperatorFilterer
{
        ...
        BaseTokenURI(baseTokenURI_)
    {
        ...
        // TODO Add correct setup.
        _setupLimitedEdition(0);
        _setupLimitedEdition(1);
        _setupLimitedEdition(2);
        _setupLimitedEdition(3);
        _setupLimitedEdition(4);
        _setupLimitedEdition(5);
        _setupLimitedEdition(6);
        _setupLimitedEdition(7);
        _setupLimitedEdition(8);
        _setupLimitedEdition(9);
        _setupLimitedEdition(10);
        _setupLimitedEdition(11);
        _setupLimitedEdition(12);
        _setupLimitedSeries(13, 150, 0);
        _setupLimitedSeries(14, 150, 0);
        _setupLimitedSeries(15, 150, 0);
        _setupLimitedSeries(16, 150, 0);
        _setupLimitedSeries(17, 150, 0);
        _setupLimitedSeries(18, 150, 0);
        _setupLimitedSeries(19, 150, 0);
    }
     ...
     */
    function mintReserve(address to)
        external
        onlyOwner
        onlyOnce(Grails3.mintReserve.selector)
    {
    ...
    function mintArtistChoices(address to, uint8[] calldata choices)
        external
        onlyOwner
        onlyOnce(Grails3.mintArtistChoices.selector)
    {
        ...
        internal
    {
        ...
        uint16 variant = uint16(shuffler.next(rand));
...
import "ethier/random/PRNG.sol";

/**
 * @notice Returns the next value in a shuffled list [0,n), amortising the
 * shuffle
 * across all calls to _next(). Can be used for randomly allocating a set of
 * tokens
 * but the caveats in `dev` docs MUST be noted.
 * @dev Although the final shuffle is uniformly random, it is entirely
 * deterministic if the seed to the PRNG.Source is known. This MUST NOT be used
 * for
 * applications that require secure (i.e. can't be manipulated) allocation
 * unless
 * parties who stand to gain from malicious use have no control over nor
 * knowledge
...
library NextShuffler {
     ...
     * _next(). This is the equivalent of `i` in the Wikipedia description of
     * the
     ...
     * have been shuffled are stored with their original index as the key and
     * their
     ...
     * differentiate between a default value and a new index of 0. See _get()
     * and
     * _set().
     */
     ...
     * standard Fisher–Yates shuffle.
     */
    function _set(State storage state, uint256 i, uint256 val) private {
