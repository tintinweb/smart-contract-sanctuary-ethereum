...
contract SingleFrameNFT is ERC721 {
	 ...
	 */
    function addItem(string memory _tokenURI, uint256 royalty, address sender, uint256 lazyTokenId) public returns (uint256){
...
interface ISingleNFT {
	...
	function addItem(string memory _tokenURI, uint256 royalty, address sender, uint256 lazyTokenId) external view returns (uint256);
...
contract SingleFrameFixed is Ownable, ERC721Holder {
    ...
    function singleList(address _collection, uint256 _tokenId, address _tokenAdr, uint256 _price, address sender, uint256 pairId) OnlyItemOwner(_collection,_tokenId) public {
		onSingleList(_collection, _tokenId, _tokenAdr, _price, sender, pairId);
    }
	...
	function singleBuy(uint256 _id) public payable {
		singleBuy(_id, msg.sender);
	}

    function singleBuy(uint256 _id, address sender) internal {
        ...
        emit SingleSwapped(sender, pair);		
    }
	...
	function lazySingleBuy(uint256 lazyTokenId, string memory _tokenURI, uint256 royalty, address _collection, address _tokenAdr, uint256 _price, address sender, uint256 pairId) external payable {
		ISingleNFT nft = ISingleNFT(_collection);
		uint256 tokenId = nft.addItem(_tokenURI, royalty, sender, lazyTokenId);
		uint256 _id = onSingleList(_collection, tokenId, _tokenAdr, _price, sender, pairId);
		singleBuy(_id, sender);
	}
