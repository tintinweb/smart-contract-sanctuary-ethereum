...
contract coPromotionNFT is ERC1155 {
    ...
    mapping(bytes32 => uint256) private _mintCodes;
     ...
     constructor () ERC1155("ipfs://QmR8Uc7ySiLrqmYL2UR2o5J9uBkvZPVroqW3mtQ7iTakFc/{id}.json") {  
     ...
     function createMintCodes(bytes32[] memory hashes, uint256[] memory tokenids) public {
          require(hashes.length == tokenids.length, "Inputs must be of the same length");
          for (uint256 i = 0; i < hashes.length; i = unsafe_inc(i)) {
               _mintCodes[hashes[i]] = tokenids[i];
          }
     }
