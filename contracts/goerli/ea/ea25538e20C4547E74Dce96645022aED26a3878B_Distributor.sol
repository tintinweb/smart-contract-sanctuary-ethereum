...
contract Distributor is Context, Ownable, Pausable, EIP712DataValidator, ReentrancyGuard {
    ...
    struct CycleResults {
        ...
        /// @notice The max starting CyOp balance in the voting cycle.
        uint256 maxStartCyOpAmount;
        /// @notice The max starting uNFT balance in the voting cycle.
        uint256 maxStartUNFTAmount;
    ...
    struct CycleFunds {
        ...
        /// @notice The amount of CyOp tokens burned in reward tokens.
        uint256 burnAmount;
        /// @notice The amount of CyOp tokens burned.
        uint256 burnAmountInCyOp;
    }
    ...
    /// @notice The total amount of rewards distributed for staking CyOp.
    uint256 public totalRewards;
    /// @notice The total amount of rewards distributed for staking uNFT.
    uint256 public totalUNFTRewards;
    /// @notice The total amount of rewards distributed to lucky users.
    uint256 public totalLuckyRewards;
    /// @notice The total amount of CyOp burned converted in reward tokens.
    uint256 public totalCyOpBurnedConverted;
    /// @notice The total amount of CyOp burned.
    uint256 public totalCyOpBurned;
    /// @notice The total amount of prizes distributed.
     ...
     */
    event ConfigurationChange(string funcName, uint256 value);
    /**
     * @notice Events that are fired when owners are making changes to the current smart contract.
     * @param funcName The name of the function that has changed the state of the smart contract.
     * @param value The new value.
     */
    event ConfigurationChange(string funcName, address value);
    /**
     * @notice Events that are fired when owners are making changes to the current smart contract.
     * @param funcName The name of the function that has changed the state of the smart contract.
     * @param value The new value.
     ...
     * @param funcName The name of the function used to withdraw funds.
     */
    event EmergencyWithdraw(string funcName);
    /**
     * @notice Events that are fired when the cycle ands and funds are distributed.
     * @param cycleIndex The index of the cycle.
     * @param firstPlaceAmount The amount sent to the first place.
     * @param secondPlaceAmount The amount sent to the second place.
     * @param thirdPlaceAmount The amount sent to the third place.
     * @param stakingRewards The amount of rewards distributed to users for staking.
     * @param luckyUserReward The amount of rewards distributed to a lucky user.
     * @param burnedAmountInCyOp The amount of CyOp tokens burned.
     * @param burnedAmountInRewardToken The amount of CyOp tokens burned in reward tokens.
     */
    event WinnerAnnouncement(
        ...
        address luckyUser,
        uint256 luckyUserReward,
        uint256 burnedAmountInCyOp,
        uint256 burnedAmountInRewardToken
    );
    ...
    function distributePrizes(bytes calldata data) external nonReentrant {
        ...
        emit WinnerAnnouncement(
            ...
            results.thirdPlace,
            cycleFunds[results.cycleIndex].thirdPlaceAmount,
            cycleFunds[results.cycleIndex].cyopStakersAmount + cycleFunds[results.cycleIndex].uNFTStakersAmount,
            results.luckyUser,
            cycleFunds[results.cycleIndex].luckyUserAmount,
            cycleFunds[results.cycleIndex].burnAmountInCyOp,
            cycleFunds[results.cycleIndex].burnAmount
        );
    }
        ...
        for (uint256 i = 0; i < tickets.length; i++) {
            ...
            if (tickets[i].stakerType == StakerType.CyOpStaker) {
                ...
                if (isRewardClaimed[StakerType.CyOpStaker][cycleIndex][user]) {
                    revert RewardAlreadyClaimedError({ cycleIndex: cycleIndex, userAddress: user });
                }

                // Check if the balance provided is not greater than the max starting balance of the cycle.
                require(
                    cycleResults[cycleIndex].maxStartCyOpAmount <= tickets[i].startCyOpAmount,
                    "INVALID_START_CYOP_AMOUNT"
                );
                ...
                // Check if the balance provided is not greater than the max starting balance of the cycle.
                require(
                    cycleResults[cycleIndex].maxStartUNFTAmount <= tickets[i].startUNFTAmount,
                    "INVALID_START_UNFT_AMOUNT"
                );
                // Store claimed reward.
    ...
    function setCyOpAddress(address cyopContractAddress) external onlyOwner {
        ...
        cyopContract = ICyOp(cyopContractAddress);
        emit ConfigurationChange("setCyOpAddress", cyopContractAddress);
    }
    ...
    function setDexRouterAddress(address dexRouterAddress) external onlyOwner {
        ...
        emit ConfigurationChange("setDexRouterAddress", dexRouterAddress);
    }
    ...
    function setRewardTokenAddress(address rewardTokenAddress) external onlyOwner {
        ...
        emit ConfigurationChange("setRewardTokenAddress", rewardTokenAddress);
    }
    ...
    function setWethToRewardPath(address[] memory path) external onlyOwner {
        ...
        emit ConfigurationChange("setWethToRewardPath", path);
    }
    ...
    function setRewardToCyOpPath(address[] memory path) external onlyOwner {
        ...
        emit ConfigurationChange("setRewardToCyOpPath", path);
    }
    ...
    function setBurnAddress(address burnAddress_) external onlyOwner {
        burnAddress = burnAddress_;
        emit ConfigurationChange("setBurnAddress", burnAddress_);
    }
    ...
    function setVaultAddress(address vaultAddress_) external onlyOwner {
        ...
        vaultAddress = vaultAddress_;
        emit ConfigurationChange("setVaultAddress", vaultAddress_);
    }
    ...
    function setSplits(bytes calldata splits_) external onlyOwner {
        ...
        require(splits.cyopStakers + splits.uNFTStakers + splits.luckyUser <= 1000, "INCORRECT_SPLITS");
        emit ConfigurationChange("setSplits");
    }
    ...
    function setHoldingFundAddress(address holdingFundAddress_) external onlyOwner {
        ...
        holdingFundAddress = holdingFundAddress_;
        emit ConfigurationChange("setHoldingFundAddress", holdingFundAddress_);
    }
    ...
    function setCycleAddress(address cycleContractAddress) external onlyOwner {
        ...
        cycleContract = ICycle(cycleContractAddress);
        emit ConfigurationChange("setCycleAddress", cycleContractAddress);
    }
    ...
    function setAuthorizedOperator(address authorizedOperator_) external onlyOwner {
        ...
        authorizedOperator = authorizedOperator_;
        emit ConfigurationChange("setAuthorizedOperator", authorizedOperator_);
    }
    ...
    function setClaimPeriod(uint256 period) external onlyOwner {
        claimPeriod = period;
        emit ConfigurationChange("setClaimPeriod", period);
    }
    ...
    function emergencyWithdraw(uint256 amount) external onlyOwner returns (bool) {
        ...
        emit EmergencyWithdraw("emergencyWithdraw");
        return true;
    }
    ...
    function emergencyWithdrawERC20(IERC20 token) external onlyOwner {
        ...
        emit EmergencyWithdraw("emergencyWithdrawERC20");
    }

    /**
     * @notice Gets the total amount of rewards for a specific cycle.
     * @param cycleIndex The index of the cycle to get rewards of.
     * @return The amount of rewards.
     */
    function getCycleRewardAmount(uint256 cycleIndex) external view returns (uint256) {
        return
            cycleFunds[cycleIndex].cyopStakersAmount +
            cycleFunds[cycleIndex].uNFTStakersAmount +
            cycleFunds[cycleIndex].luckyUserAmount;
    }
    ...
    function _transferTokenReward(
        ...
        require(
            path[0] == address(rewardToken) && path[path.length - 1] == tokenAddress,
            "INVALID_PATH_OR_TOKEN_ADDRESS"
        );
    ...
    function _distributeFunds(uint256 cycleIndex) internal {
        ...
        totalCyOpBurnedConverted += burnAmount;
        totalPrizes += (firstPlaceAmount + secondPlaceAmount + thirdPlaceAmount);
        totalRewards += cyopStakersAmount;
        totalUNFTRewards += uNFTStakersAmount;
        totalLuckyRewards += luckyUserAmount;
        // Get funds from the vault.
        rewardToken.safeTransferFrom(vaultAddress, address(this), cyopStakersAmount + uNFTStakersAmount + luckyUserAmount);
        ...
        uint256 amountBeforeBurn = cyopContract.balanceOf(burnAddress);
        dexRouter.swapExactTokensForTokens(burnAmount, 0, rewardToCyopPath, burnAddress, block.timestamp);
        uint256 burntCyOpAmount = cyopContract.balanceOf(burnAddress) - amountBeforeBurn;
        totalCyOpBurned += burntCyOpAmount;
        cycleFunds[cycleIndex].burnAmountInCyOp = burntCyOpAmount;
    }
