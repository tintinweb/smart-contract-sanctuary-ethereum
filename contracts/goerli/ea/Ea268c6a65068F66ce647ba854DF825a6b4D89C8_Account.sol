...
contract Account is Ownable, ReentrancyGuard, Pausable {
    ...
    function withdrawEth() external onlyAuthorized nonReentrant whenNotPaused {
    ...
    function _pendingPayment(
        ...
        return (_totalReceived * _share) / _population - _aleadyReleased;
    }
...
contract Securities is ERC1155, Ownable {
    ...
    function mint(uint256 _amount, address _to) public onlyOwner {
        if (!isAddressInArray(s_holders, _to)) {
            s_holders.push(_to);
        }
        _mint(_to, 0, _amount, "");
