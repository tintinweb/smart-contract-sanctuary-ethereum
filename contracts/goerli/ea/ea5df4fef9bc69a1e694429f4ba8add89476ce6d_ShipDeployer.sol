...
pragma solidity ^0.8.13;
import {ICaptainGuard} from "szns/interfaces/zodiac/guards/ICaptainGuard.sol";

import {ICaptainGuardEvents} from "szns/interfaces/zodiac/guards/ICaptainGuardEvents.sol";
...
contract CaptainGuard is ICaptainGuard, ICaptainGuardEvents {
    ...
    constructor(address _captain) {
        ...
        // TODO: Update this event params if needed
        emit CaptainAssigned(msg.sender, msg.sender, CAPTAIN);
    }

    function updateCaptain(address captain) public virtual override {
        CAPTAIN = captain;
...
import {ICrowdfundActions} from "szns/interfaces/crowdfund/ICrowdfundActions.sol";
import {ICrowdfundEvents} from "szns/interfaces/crowdfund/ICrowdfundEvents.sol";

contract Crowdfund is ERC20SnapshotUpgradeable, ICrowdfundEvents {
    error RaiseClosed();

    uint256 public endDuration; // When pooling is closed
    uint256 public tokensPerEth; // how many tokens to mint per eth
    uint256 public minRaise; // Minimum in ETH to raise
    uint256 public totalContributions; // Total ETH raised
    ...
    /// @notice Check if pool duration is over
    /// @return True if pool still open for contributions
    function isRaiseOpen() public view returns (bool) {
    ...
    function hasRaiseMet() public view returns (bool) {
...
import {IDeployerEvents} from "szns/interfaces/zodiac/deployer/IDeployerEvents.sol";
import {IDeployerActions, SetupReturnVars, SetupVars} from "szns/interfaces/zodiac/deployer/IDeployerActions.sol";
import {Ownable} from "@openzeppelin/contracts/access/Ownable.sol";

contract ShipDeployer is IDeployerEvents, IDeployerActions, Ownable {
    error MinRaiseLteZero();
    error PoolDurationInPast();
    error NFTAllowedEmpty();
    error NotAllowedError();
    error AllowListDisabled();

    // Used to deploy new safes
    ...
    uint256 public immutable TOKENS_PER_ETH = 1000;
    uint256 public immutable SZNS_DAO_FEE_RATE = 25e17; // 2.5%
    ...
    function createAndSetup(SetupVars calldata s)
        external
        returns (SetupReturnVars memory rv)
    {
...
library Sealib {
    function buildOrderFor(
        ...
        considerationItem.endAmount = amount; // 20%
        considerationItem.recipient = payable(recipient);
        ...
        seaportOrder.parameters.endTime = block.timestamp + 30 days;
    }

    function convertOrderToOrderComponents(
        Order memory seaportOrder
    ) internal pure returns (OrderComponents memory orderComponents) {
        orderComponents = OrderComponents(
            seaportOrder.parameters.offerer,
            seaportOrder.parameters.zone,
            seaportOrder.parameters.offer,
            seaportOrder.parameters.consideration,
            seaportOrder.parameters.orderType,
            seaportOrder.parameters.startTime,
            seaportOrder.parameters.endTime,
            seaportOrder.parameters.zoneHash,
            seaportOrder.parameters.salt,
            seaportOrder.parameters.conduitKey,
            seaportOrder.parameters.consideration.length
        );
    }
    ...
    function approveAndList(
        ...
        uint256 orderItems = order.parameters.offer.length;
        ...
        orders[0] = order;
        ConsiderationInterface(seaport).validate(orders);
    }
}
...
import {IBuyEvents} from "szns/interfaces/buy/IBuyEvents.sol";
import {IBuyActions} from "szns/interfaces/buy/IBuyActions.sol";
import {IERC721} from "@openzeppelin/contracts/token/ERC721/IERC721.sol";
import {ConsiderationInterface, Order} from "seaport/interfaces/ConsiderationInterface.sol";
...
contract BuyModule is Module, IBuyEvents, IBuyActions {
    ...
    function buy(Order calldata seaportOrder) public virtual {
        ...
        uint256 value = seaportOrder.parameters.consideration[0].startAmount; // should match endAmount

        // if (nfts[nftContract] == false) {
...
import {Module, Enum} from "@gnosis.pm/zodiac/contracts/core/Module.sol";
import {IListEvents} from "szns/interfaces/list/IListEvents.sol";
import {IListActions} from "szns/interfaces/list/IListActions.sol";
...
contract ListModule is Module, IListEvents, IListActions {
    error ApproveFail();
    error CancelFail();
    ...
    function cancel(OrderComponents[] calldata orders) public virtual {
        bytes memory callData = abi.encodeCall(seaport.cancel, (orders));
        if (!exec(address(seaport), 0, callData, Enum.Operation.Call))
            revert CancelFail();
    }
    ...
    function _list(Order memory seaportOrder) internal {
        ...
        for (uint256 i = 0; i < orderItems; ) {
            ...
            bytes memory callData = abi.encodeCall(
                IERC721(item.token).approve,
                (address(seaport), item.identifierOrCriteria)
            );
            if (!exec(item.token, 0, callData, Enum.Operation.Call))
                revert ApproveFail();
            ++i;
        }
        ...
        bytes memory listData = abi.encodeCall(seaport.validate, orders);
        (bool success, bytes memory _returnData) = execAndReturnData(
            address(seaport),
            0,
            listData,
            Enum.Operation.Call
        );
        require(success, string(_returnData));
    }
}
...
import {ListModule} from "szns/zodiac/modules/ListModule.sol";
import {IClaimActions} from "szns/interfaces/claim/IClaimActions.sol";
import {IClaimEvents} from "szns/interfaces/claim/IClaimEvents.sol";
...
struct ShipSetupArgs {
    ...
    address captain; // TODO: add captain param
    address[] nfts; // TODO: remove this on real implementation
}
    ...
    CaptainGuard
{
    ...
    function setUp(bytes memory initializeParams)
        public
        override(FactoryFriendly, BuyModule, ListModule)
        initializer
    {
        __Ownable_init();
        ...
        nftBought = false;
        CAPTAIN = _sv.captain; // TODO: add captain
    ...
    // TODO: generalize this later to handle send() and transfer() calls
    // This will only work via call.value() calls and not send()/transfer()
    // ref: https://github.com/ConsenSysMesh/Ethereum-Development-Best-Practices/wiki/Fallback-functions-and-the-fundamental-limitations-of-using-send()-in-Ethereum-&-Solidity
    function _receive() internal {
        // Don't take snapshots if we are currently taking contributions
        if (!isRaiseOpen()) {
    ...
    // Checks at if a wallet could claim for a snapshot
    ...
    // sends claimable amount for msg.sender at a snapshot
    function claim(uint256 claimID)
        public
        payable
    ...
    // TODO: Updated this with onlyCaptain modifier
    function claimCaptainFees(address captain) public onlyCaptain {
        uint256 fees = captainFees;
        delete captainFees;
        payable(captain).transfer(fees);
    }
    ...
    function contribute() public payable returns (uint256 minted) {
        ...
        // TODO: do we need to prevent reentrancy?
        (bool success, bytes memory data) = avatar.call{value: msg.value}("");
        require(success, string(data));
    }

    event Event1(bool, bool, bool, bool);
    event Event2(address);

    function buy(Order calldata seaportOrder) public override {
        emit Event1(
            isRaiseOpen(),
            hasRaiseMet(),
            nftBought,
            nfts[seaportOrder.parameters.offer[0].token]
        );
        emit Event2(this.CAPTAIN());
        if (!isRaiseOpen()) {
            revert BuyClosed();
        } else if (!hasRaiseMet()) {
            revert RaiseNotMet();
    ...
    function list(
        ...
        if (isRaiseOpen()) {
            ...
            super.list(nftContract, tokenID, amount);
        }
    }
