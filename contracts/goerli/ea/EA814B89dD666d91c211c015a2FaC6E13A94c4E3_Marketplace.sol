...
contract Marketplace is ReentrancyGuard, ERC721Holder, Ownable {
    ...
    constructor(address marketplaceOwner) {
        _transferOwnership(marketplaceOwner);
    }
    ...
    function createMarketItem(
        ...
        uint256 tokenId,
        uint256 price,
        address _reproductionNFT
        ) public payable nonReentrant {
        //require(msg.sender == _owner, "You are not the marketplace owner");
        require(price >= 0, "Price must be positive");
        require(IERC721(nftContractAddress).ownerOf(tokenId) != address(0), "No Token ID");
        ...
        // console.log("the token owner is ", ERC721(nftContractAddress).ownerOf(tokenId));
        ...
        emit TokenListed(itemId, tokenId, nftContractAddress, msg.sender, msg.sender, price, _reproductionNFT, false);
    }
    ...
    function buyToken(
        ...
        ListedToken[] memory availableItemsToSell = getAllNFTsNotSold();
        ...
        (bool sent, bytes memory data) = itemToSell.seller.call{value: msg.value}(""); // the seller receives the money
        require(sent, "Failed to send Ether to seller");
        //IERC721(nftContract).safeTransferFrom(_tokenList[itemToSellItemId].owner, address(this), tokenId);  // transfer the NFT ownership to the marketplace (in the contract)
    ...
    function transferNFTOwnership(
        ...
        ERC721(nftContract).safeTransferFrom(msg.sender, address(this), tokenId); // transfer the NFT ownership to buyer (in the contract)
    }
}
