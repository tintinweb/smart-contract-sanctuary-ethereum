...
abstract contract CNPScore is Ownable {
    ...
    bool public openSeaRoyaltyFlag = true;
}
    ...
    DefaultOperatorFilterer
{
    ...
    // onlyAdmin
    function setOpenSeaRoyaltyFlag(bool _flag) external onlyAdmin {
        openSeaRoyaltyFlag = _flag;
    }
...
contract CNPS is CNPSadmin {
            ...
            if (
                ...
                if (limitRelease == 1) {
                    if (_getAux(_address).get16_forAux(0) < alcount) {
                        ...
                        _Amount =
                            _alAmountMax -
                            _getAux(_address).get16_forAux(1);
                    }
                } else {
                    if (_getAux(_address).get16_forAux(0) < alcount) {
                        ...
                        _Amount =
                            limitReleaseMaxAmount -
                            _getAux(_address).get16_forAux(1); // After limit release
                    }
                }
            }
        }
    ...
    function mint(
        ...
        if (alcount == 0) {
            require(1668452400 >= block.timestamp , "can not mint"); // 2022-11-14 19:00:00
        }
