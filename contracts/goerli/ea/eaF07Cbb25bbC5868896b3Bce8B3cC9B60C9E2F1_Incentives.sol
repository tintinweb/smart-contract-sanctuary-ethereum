...
import "./Coordination.sol";
import "./Boosting.sol";

/// @title This contract add incentives calculation to Bond
contract Incentives is Boosting, Coordination, IIncentives {
    constructor(
        address _gac,
        address _bst,
        address _config
    ) IncentivesBase(_gac, _bst, _config) {}
    ...
    function boostForTaker(address token, uint amount)
        ...
        returns (uint)
    {
        address state = IMarket(IProtocolConfig(config).getMarket(token))
            .getState();
        (int _price, , ) = IState(state).price();
        return
            (boost[token].taker * amount) /
            (IState(state).B() + amount) /
            uint(_price);
    }
        ...
        returns (uint)
    {
        address state = IMarket(IProtocolConfig(config).getMarket(token))
            .getState();
        return (boost[token].maker * amount) / (IState(state).D() + amount);
    }
        ...
        returns (uint)
    {
        address state = IMarket(IProtocolConfig(config).getMarket(token))
            .getState();
        return (calculateCoordination() * amount) / (IState(state).D() + amount);
    }
}
...
import "./IncentivesBase.sol";

/// @title Implementation of protocol boost and protocol incentives calculation
abstract contract Coordination is IncentivesBase {
    int internal constant a = (int(DIVIDER) * 3) / 100;
    int internal constant b = (int(DIVIDER) * 80) / 100;
    int internal constant c = (int(DIVIDER) * 90) / 100;
    int internal constant d = int(DIVIDER) * 1;
    int internal constant f = (int(DIVIDER) * 26) / 100;
    int internal constant g = int(DIVIDER) * 1;
    int internal constant j = (int(DIVIDER) * 30) / 100;
    int internal constant h = (int(DIVIDER) * 74) / 100;

    uint public thetaScale;
    uint public thetaInflection;

    function setIncentiveConfig(uint _scale, uint _inflection)
        external
        onlyGovernance
    {
        thetaScale = _scale;
        thetaInflection = _inflection;
    }

    function calculateCoordination() public view returns (uint incentive) {
        uint tvlStable = bst.totalSupply();
        incentive = _incentive(tvlStable);
    }

    function _incentive(uint tvlStable) internal view returns (uint value) {
        int x = (int(DIVIDER) * int(tvlStable)) / int(thetaInflection);
        if (tvlStable < thetaInflection)
            value = uint((a * x) / ((b * x) / int(DIVIDER) - c) + d);
        else value = uint( (f * x) / ((g * x) / int(DIVIDER) - h) + j);
        return thetaScale * value;
    }
}
...
/// @title Implementation of protocol boost and protocol incentives calculation
abstract contract Boosting is IncentivesBase {
    int public boostMax;
    int public thetaBoost;

    struct BoostingStruct {
        uint taker;
        uint maker;
    }

    mapping(address => BoostingStruct) boost;

    function setBoostConfig(int _boostMax, int _thetaBoost)
        external
        onlyGovernance
    {
        boostMax = _boostMax;
        thetaBoost = _thetaBoost;
    }

    // update boosting parameters for given token (WETH, WBTC etc.)
    function updateBoosting(address token) external {
        IRiskCalculation rcalc;
        IState state;
        {
            IMarket market = IMarket(IProtocolConfig(config).getMarket(token));
            state = IState(market.getState());
            rcalc = IRiskCalculation(market.getRiskCalc());
        }
        uint boostTaker;
        uint boostMaker;
        {
            BoostingParameters memory b = rcalc.boostParameters();

            console.log("LRR1 %s, LRR11 %s", uint(b.lrr1), uint(b.lrr11));
            console.log("W1 %s, W11 %s", uint(b.w1), uint(b.w11));

            (boostTaker, boostMaker) = _boost(b);
        }
        boost[token] = BoostingStruct(boostTaker, boostMaker);
    }

    function _boost(BoostingParameters memory b)
        internal
        view
        returns (uint, uint)
    {
        int d11 = (b.w11 - b.lrr11);
        int d1 = (b.w1 - b.lrr1);
        console.log("pbl11 %s, pbl1 %s", uint(b.pbl11), uint(b.pbl1));
        int boostTaker = (thetaBoost *
            SignedMath.min(
                boostMax,
                (((((((DIVIDER * d11) / b.pbl11) * d11) / b.pbl11) * d11) /
                    b.pbl11) * d11) / b.pbl11
            )) / DIVIDER;
        int boostMaker = (thetaBoost *
            SignedMath.min(
                boostMax,
                (((((((DIVIDER * d1) / b.pbl1) * d1) / b.pbl1) * d1) / b.pbl1) *
                    d1) / b.pbl1
            )) / DIVIDER;
        return (uint(boostTaker), uint(boostMaker));
    }
}
...
/// @title This contract add incentives calculation to Bond
abstract contract IncentivesBase is ModuleAccessController {
    IProtocolConfig internal immutable config;
    IERC20 internal immutable bst;
    int public constant DIVIDER = 10**18;
