...
contract NFTStaking is Ownable {
    ...
    uint256 public RewardTokenPerDay;
    ...
    constructor(address _nftAddress, address _rewardTokenAddress, uint256 _rewardTokenPerDay) {
    ...
    function changeRewardTokenPerDay(uint256 _rewardTokenPerDay) public onlyOwner {
        RewardTokenPerDay = _rewardTokenPerDay;
    }
    ...
    function pendingReward(address _user, uint256 _tokenId) public view returns (uint256) {
        ...
        uint256 rewardAmount = (currentTime.sub(_startTime)).mul(RewardTokenPerDay).div(24 * 3600);
        return rewardAmount;
    }
    ...
    function getStakedItems(address _user) public view returns(uint256[] memory) {
        uint256 ownedTokenCount = userInfo[_user].length;
        uint256[] memory ownedTokenIds = new uint256[](ownedTokenCount);

        uint256 ownedTokenIndex = 0;
        while(ownedTokenIndex < ownedTokenCount) {
            ownedTokenIds[ownedTokenIndex] = userInfo[_user][ownedTokenIndex].tokenId;
            ownedTokenIndex++;
        }
        return ownedTokenIds;
    }

    function getUnstakedItems(address _user) public view returns(uint256[] memory) {
        uint256 ownerTokenCount = IERC721(nftAddress).balanceOf(_user);

		uint256[] memory ownedTokenIDs = new uint256[](ownerTokenCount);

		for (uint256 i = 0; i < ownerTokenCount; i++) {
            uint256 tokenId = IERC721Enumerable(nftAddress).tokenOfOwnerByIndex(_user, i);
