    ...
    IBrawlerBearzStake
{
    ...
    function stake(uint256[] calldata tokenIds) external override {
        ...
        for (uint256 i = 0; i < tokenIds.length; ) {
            ...
            staked[tokenId] = Stake(msg.sender, uint96(block.timestamp));
            // Lock token
            nftContract.lockId(tokenId);
            // Stake on child
            _sendChildMessage(msg.sender, tokenIds, true);
            unchecked {
                ++i;
            }
        }
    ...
    function unstake(uint256[] calldata tokenIds) external override {
        ...
        for (uint256 i = 0; i < tokenIds.length; ) {
            ...
            if (stakeInfo.owner != msg.sender) revert InvalidOwner();
            staked[tokenId] = Stake(address(0), 1);
            // Unlock token
            nftContract.unlockId(tokenId);
            // Unstake on child
            _sendChildMessage(msg.sender, tokenIds, false);
            unchecked {
                ++i;
            }
        }
     ...
     * @param tokenIds - the tokenIds staked/unstaked
    ...
    function _sendChildMessage(
        ...
        bytes memory message = abi.encode(
            isInbound ? STAKE : UNSTAKE,
            abi.encode(from, tokenIds)
        );
