...
contract Property is AccessControl {
    ...
    event AskingPriceUpdated(address indexed token, uint256 value);
    event OfferUpdate(address indexed bidder, address indexed token, uint256 value);
    event RoyaltiesPaid(address indexed beneficiary, uint256 value);
    event ClosingTable(
    ...
    bytes32 public constant PROPERTY_BUYER_ROLE = keccak256('PROPERTY_BUYER_ROLE');
    // 0xea53f3f4335c0e97dcbcb027b53f0824c6587837e0192917c8f81e4a7a90b698
    bytes32 public constant REGULATED_TRANSFER_AGENT_ROLE =
    ...
    bool private royaltiesReceiverAddressSet;
    ...
    constructor(
        ...
        emit RoyaltiesSet(_propertyOwner, _ownerRoyaltiesBps);
        ...
        emit RoyaltiesSet(_listingBroker, _listingBrokerRoyaltiesBps);
        Royalties[2].beneficiary = payable(_provider);
        Royalties[2].bps = _providerRoyaltiesBps;
        emit RoyaltiesSet(_provider, _providerRoyaltiesBps);
    }
    ...
    function transferCOIN(address to, uint256 amount) public {
        require(
            ...
            'Property::transferCOIN: must have RTA role to send coin'
        );
    ...
    function safeTransferERC20(address token, address to, uint256 amount) public {
        require(
            hasRole(REGULATED_TRANSFER_AGENT_ROLE, msg.sender),
            'Property::safeTransferERC20: must have RTA role to transfer'
        );
    ...
    function setAskingPrices(address[] calldata tokens, uint256[] calldata values) public virtual {
        require(
            hasRole(PROPERTY_OWNER_ROLE, msg.sender),
            'Property::setAskingPrices: must have property owner role to set'
        );
        ...
        for (uint256 i = 0; i < arrayLength; i++) {
            ...
            emit AskingPriceUpdated(tokens[i], values[i]);
        }
    }
    ...
    function makeOffer(address token, uint256 value, bool verify) external virtual {
        require(
            !hasRole(PROPERTY_OWNER_ROLE, msg.sender) &&
                !hasRole(REGULATED_TRANSFER_AGENT_ROLE, msg.sender),
            'Property::makeOffer: the property owner and the RTA can not make an offer'
        );
        require(
    ...
    function acceptOffer(address winningBidder, address token) external virtual {
        require(
            hasRole(PROPERTY_OWNER_ROLE, msg.sender),
            'Property::acceptOffer: only property owner role can call'
        );
        // this also eliminates testing wether 'winningBidder' is a smart contract as that is done in makeOffer()
    ...
    function resetOfferAcceptance() external virtual {
        require(
            hasRole(REGULATED_TRANSFER_AGENT_ROLE, msg.sender),
            'Property::resetOfferAcceptance: only RTA role can call'
        );
    ...
    function closeTheTable(address from, address to) external virtual {
        require(
            hasRole(REGULATED_TRANSFER_AGENT_ROLE, msg.sender),
            'Property::closeTheTable: must have RTA role to call'
        );
        require(hasRole(PROPERTY_OWNER_ROLE, from), 'Property::closeTheTable: incorrect owner');
        require(
            hasRole(PROPERTY_BUYER_ROLE, to),
            'Property::closeTheTable: incorrect buyer or no accepted offer available'
        );
        ...
        if (tokenAcceptedForClosing == address(0)) {
            ...
            for (uint256 i = 0; i < Royalties.length; i++) {
                ...
                emit RoyaltiesPaid(Royalties[i].beneficiary, amount);
            }
        ...
        emit RoyaltiesSet(Royalties[1].beneficiary, Royalties[1].bps);
    ...
    function destroy() external virtual {
        require(
            hasRole(REGULATED_TRANSFER_AGENT_ROLE, msg.sender),
            'Property::destroy: must have RTA role to destroy'
        );
    ...
    function setNewStreetAddress(string calldata newAddress) external virtual {
        require(
            hasRole(REGULATED_TRANSFER_AGENT_ROLE, msg.sender),
            'Property::setNewStreetAddress: only RTA role can call'
        );
        require(
    ...
    function setNewLegalDescription(string calldata newDescription) external virtual {
        require(
            hasRole(REGULATED_TRANSFER_AGENT_ROLE, msg.sender),
            'Property::setNewLegalDescription: only RTA role can call'
        );
        require(
    ...
    function setNewDocumentationURI(string calldata newURI) external virtual {
        require(
            hasRole(REGULATED_TRANSFER_AGENT_ROLE, msg.sender),
            'Property::setNewDocumentationURI: only RTA role can call'
        );
        require(
    ...
    function setOwnerRoyaltiesReceiverSmartContract(
        ...
        require(
            !royaltiesReceiverAddressSet,
            'Property::setOwnerRoyaltiesReceiverSmartContract: the royalties receiver address can only be set once'
        );
        require(
            hasRole(PROPERTY_OWNER_ROLE, msg.sender),
            'Property::setOwnerRoyaltiesReceiverSmartContract: must have property owner role to set'
        );
        require(
        ...
        royaltiesReceiverAddressSet = true;
        emit RoyaltiesSet(Royalties[0].beneficiary, Royalties[0].bps);
    }

    // To be called by the property owner when they have a new listing broker
    function setListingBrokerForRoyalties(address newListingBroker) external virtual {
        require(
            hasRole(PROPERTY_OWNER_ROLE, msg.sender),
            'Property::setListingBrokerForRoyalties: must have property owner role to set'
        );
        require(
        ...
        emit RoyaltiesSet(newListingBroker, Royalties[1].bps);
    }

    // To be called by the provider to change their wallet address
    function setProviderForRoyalties(address newProvider) external virtual {
        require(
            hasRole(ADMIN_ROLE, msg.sender),
            'Property::setProviderForRoyalties: must have admin role to set'
        );
        require(
        ...
        emit RoyaltiesSet(newProvider, Royalties[2].bps);
    }
