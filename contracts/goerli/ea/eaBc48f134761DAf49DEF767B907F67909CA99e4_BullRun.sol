...
contract BullRun is Ownable, ReentrancyGuard {
    ...
    IHUB public HubInterface = IHUB(0x9E323aa48683B16c257263Ecd7436f4c366d1e16);
    ...
    function stakeMany(uint16[] calldata _tokenIds) external payable nonReentrant notContract() {
        ...
        for (uint i = 0; i < _tokenIds.length;) {
            ...
            if (NFTType[_tokenIds[i]] == 1) {
                ...
                StakedTokensOfWallet[msg.sender].add(_tokenIds[i]);
    ...
    function claimRunner(uint16 tokenId, bool unstake, uint256 seed) internal returns (uint256 owed) {
        ...
        if(unstake) {
            ...
            if (HubInterface.alphaCount() > 0 && (seed % 100) < Risk[1]) { // nft gets stolen
    ...
    function getUnclaimedTopiaForUser(address _account) external view returns (uint256) {
        ...
        uint256 wastelandsLength = WastelandMatadorsOfWallet[msg.sender].length();
        
        for (uint i = 0; i < genesisLength;) {
        ...
        for (uint i = 0; i < wastelandsLength;) {
            uint16 tokenId = uint16(WastelandMatadorsOfWallet[_account].at(i));
            owed += WastelandMatadors[tokenId].value;
            unchecked{ i++; }
        }
    ...
    function addToMob(uint16 _id) external payable nonReentrant notContract() {
        ...
        StakedTokensOfWallet[msg.sender].add(_id);
    ...
    function claimMob(uint16[] calldata tokenIds, bool unstake) external payable notContract() {
        ...
        for (uint i = 0; i < tokenIds.length;) {
            ...
            if(unstake) {
                ...
                StakedTokensOfWallet[msg.sender].remove(tokenIds[i]);
                IsInMob[tokenIds[i]] = false;

                if (HubInterface.alphaCount() > 0 && (seed[i] % 100) < 10 - (theftModifier)) { // nft gets stolen
