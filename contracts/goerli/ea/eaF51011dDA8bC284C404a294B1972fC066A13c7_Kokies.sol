...
contract Kokies is ERC721, DefaultOperatorFilterer, Ownable {
    uint256 private currentTokenId;
    uint256 public price;
    uint256 public maxSupply;
    uint256 public mintDate;
    uint256 public maxAmountPerTx;
    uint256 public allowListRequired;
    ...
    constructor(string memory _uri, uint256 _mintDate, address _primaryRecipient) ERC721("Kokies", "KKI") {
        currentTokenId = 1;
    ...
    function setMintStartDate(uint256 _mintDate) public onlyOwner {
        mintDate = _mintDate;
    }

    function setPrimaryRecipient(address _recipient) public onlyOwner {
    ...
    function airdrop(address[] memory _addresses) public onlyOwner {
        ...
        require(totalSupply() + length <= maxSupply, "NFT is sold out.");
        for (uint256 i = 0; i < length; i++) {
            _mint(_addresses[i], currentTokenId);
            currentTokenId += 1;
    ...
    function airdrop(address _address, uint256 _amount) public onlyOwner {
        require(totalSupply() + _amount <= maxSupply, "NFT is sold out.");
    ...
    function mintPublic(uint256 _quantity) external payable {
        require(block.timestamp >= mintDate, "Mint did not start yet.");
        require(allowListRequired == 0, "Must use the allow list.");
        require(_quantity <= maxAmountPerTx, "Can not mint that many tokens per tx.");
        require(totalSupply() + _quantity <= maxSupply, "NFT is sold out.");
        require(msg.value >= _quantity * price, "Not enough to pay for that.");

        (bool success, ) = primaryRecipient.call{value: msg.value}("");
    ...
    function mintAllowed(uint256 _quantity, bytes32[] memory _proof) external payable {
        require(block.timestamp >= mintDate, "Mint did not start yet.");
