...
contract AdvertiserProxy is Initializable, OwnableUpgradeable {
    ...
    function fundCampaign(
        ...
        if ((isNativeToken && msg.value == 0) || (!isNativeToken && _amount == 0)) {
            revert BadPayment();
        }
    ...
    function getReferrer(uint32 _campaignId, address _referrerAddress) public view returns (ReferrerDetails memory) {
        ReferrerDetails memory referrer = campaigns[_campaignId].referrers[_referrerAddress];
    ...
    /// @notice withdraw referrer available earnings
    ...
    function setReferrerStatus(
        ...
        if (referrer.status == _status || _status == ReferrerStatus.NONE) {
            revert BadRequest();
        }
    ...
    function performUpkeep(bytes calldata performData) external virtual onlyOwnerAndWorker {
    ...
    /// @notice add new erc20 payment type to the advertiser
    ...
    /// @dev if campaign status is COMPLETED, advertiser can withdraw the remaining balance that wasn't spent
    function withdrawRemainingCampaignBalance(uint32 _campaignId) external virtual onlyAdvertiser {
        ...
        /// @dev we want to make advertiser cannot withdraw the same remainder twice
...
contract ProxyFactory is Ownable {
    ...
    /// @notice deploy proxy for advertiser
    function deployProxy(
        ...
        emit DeployedProxy(address(proxy), owner(), _advertiserAddress, _advertiserName, workerAddress, _paymentTypes);
