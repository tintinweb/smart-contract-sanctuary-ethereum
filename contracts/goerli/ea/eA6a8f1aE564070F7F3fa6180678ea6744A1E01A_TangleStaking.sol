...
contract TangleStaking is Context, Ownable, ReentrancyGuard, Pausable, ERC721Holder {
    ...
    function _calculationInterest(NftRewardInfo memory _rewardInfo) internal view returns (uint256) {
        ...
        // get total interest
        uint256 totalInterest = Calculator.calculator(
...
library Calculator {
    ...
    function calculator(
        ...
        int128 interval = ABDKMath64x64.divu(duration, year);
        uint256 annualReward = ABDKMath64x64.mulu(rate, principal);
        uint256 reward = ABDKMath64x64.mulu(interval, annualReward);
        uint256 res = reward * n;
        return res;
    }
}
