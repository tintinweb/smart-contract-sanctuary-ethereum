...
//use solmates version of erc721

/*
    solmate
    irc soulbounds
    lots of tests https://github.com/transmissions11/solmate/blob/main/src/test/ERC721.t.sol take these, add some specifics
*/

contract NFT is ERC721, Ownable {
    using Counters for Counters.Counter; // fix
    ...
    mapping(address => uint256[]) public userOwnedTokens; // use indexer watching for mint event? does zora or oz have logic for this? if not nbd
    ...
    function safeTransferFrom(address from, address to, uint256 id, bytes32 data) public {
        require(from == address(0) || to == address(0), "Not allowed to transfer token");
    }
    // two safe transfer methods ^^ look
    ...
    //create an only owner withdrawl function
    ...
    function setTokenURI(uint256 _tokenId) internal {
        _tokenURI = string(abi.encodePacked(baseTokenURI, _tokenId));
    }
    ...
    function mint(
        ...
        require(allowlisted(msg.sender, _proof), "You are not on the allowlist"); //test
