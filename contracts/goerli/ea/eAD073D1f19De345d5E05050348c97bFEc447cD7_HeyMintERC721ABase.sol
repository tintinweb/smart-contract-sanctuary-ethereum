...
import {BaseConfig, LaunchpadStorage} from "../libraries/LaunchpadStorage.sol";
...
contract HeyMintERC721ABase is HeyMintERC721AUpgradeable, IERC2981Upgradeable {
    ...
    function initialize(
        ...
        LaunchpadStorage.state().cfg = _config;
        // LaunchpadStorage.state().advCfg = _advancedConfig;
        // LaunchpadStorage.state().addrCfg = _addressConfig;

        // if (_config.fundingDuration > 0) {
        //     LaunchpadStorage.state().data.contractDeployTime = block.timestamp;
        // }
        ...
        //     LaunchpadStorage.state().cfg.publicSaleEndTime = _config.publicSaleStartTime + 520 weeks;
        // }
        ...
        //     LaunchpadStorage.state().cfg.presaleEndTime = _config.presaleStartTime + 520 weeks;
        // }

        // possibly not necessary, let's test and remove that if not that would save a lot of gas awesome
        // transferOwnership(_owner);

        // __OperatorFilterer_init(
        //     _config.enforceRoyalties == true ? CORI_SUBSCRIPTION_ADDRESS : EMPTY_SUBSCRIPTION_ADDRESS,
        //     true
        // );
    }
    ...
    function reduceMaxSupply(uint16 _newMaxSupply) external onlyOwner {
        require(
            _newMaxSupply < LaunchpadStorage.state().cfg.maxSupply,
        ...
        LaunchpadStorage.state().cfg.maxSupply = _newMaxSupply;
    }
    ...
    function _baseURI() internal view virtual override returns (string memory) {
        return
            bytes(LaunchpadStorage.state().cfg.uriBase).length == 0
                ? string(
                    abi.encodePacked(
                        "https://api.heymint.xyz/tokens/",
                        _toString(LaunchpadStorage.state().cfg.projectId),
                        "/"
                    )
                )
                : LaunchpadStorage.state().cfg.uriBase;
    }
    ...
    function setBaseURI(string calldata _newBaseURI) external onlyOwner {
        require(!LaunchpadStorage.state().advCfg.metadataFrozen);
        LaunchpadStorage.state().cfg.uriBase = _newBaseURI;
    }
    ...
    function freezeMetadata() external onlyOwner {
        LaunchpadStorage.state().advCfg.metadataFrozen = true;
    }
    ...
    function royaltyInfo(
        ...
        address payoutAddress = LaunchpadStorage
            .state()
            .addrCfg
            .royaltyPayoutAddress != address(0x0)
            ? LaunchpadStorage.state().addrCfg.royaltyPayoutAddress
            : owner();
        ...
        return (
            payoutAddress,
            (_salePrice * LaunchpadStorage.state().cfg.royaltyBps) / 10000
        );
    }
    ...
    function withdraw() external nonReentrant onlyOwner {
        ...
        if (LaunchpadStorage.state().advCfg.refundDuration != 0) {
            require(!refundGuaranteeActive(), "REFUND_GUARANTEE_STILL_ACTIVE");
        }
        uint256 balance = address(this).balance;
        if (LaunchpadStorage.state().addrCfg.payoutAddresses.length == 0) {
            payable(owner()).transfer(balance);
        } else {
            for (
                uint256 i = 0;
                i < LaunchpadStorage.state().addrCfg.payoutAddresses.length;
                i++
            ) {
                uint256 amount = (balance *
                    LaunchpadStorage.state().addrCfg.payoutBasisPoints[i]) /
                    10000;
                (bool success, ) = LaunchpadStorage
                    .state()
                    .addrCfg
                    .payoutAddresses[i]
                    .call{value: amount}("");
    ...
    function gift(
        ...
        require(
            totalSupply() + totalMint <= LaunchpadStorage.state().cfg.maxSupply,
            "MAX_SUPPLY_EXCEEDED"
        );
    ...
    function setPublicSaleState(bool _saleActiveState) external onlyOwner {
        LaunchpadStorage.state().cfg.publicSaleActive = _saleActiveState;
    }
    ...
    function setPublicPrice(uint16 _publicPrice) external onlyOwner {
        LaunchpadStorage.state().cfg.publicPrice = _publicPrice;
    }
    ...
    function setPublicSaleStartTime(
        ...
        LaunchpadStorage.state().cfg.publicSaleStartTime = _publicSaleStartTime;
    }
    ...
    function setPublicSaleEndTime(
        ...
        LaunchpadStorage.state().cfg.publicSaleEndTime = _publicSaleEndTime;
    }
    ...
    function setUsePublicSaleTimes(
        ...
        LaunchpadStorage.state().cfg.usePublicSaleTimes = _usePublicSaleTimes;
    }
    ...
    function publicSaleTimeIsActive() public view returns (bool) {
        if (
            LaunchpadStorage.state().cfg.usePublicSaleTimes == false ||
            LaunchpadStorage.state().cfg.publicSaleStartTime == 0 ||
            LaunchpadStorage.state().cfg.publicSaleEndTime == 0
        ) {
            return true;
        }
        return
            block.timestamp >=
            LaunchpadStorage.state().cfg.publicSaleStartTime &&
            block.timestamp <= LaunchpadStorage.state().cfg.publicSaleEndTime;
    }
    ...
    function mint(
        ...
        require(LaunchpadStorage.state().cfg.publicSaleActive, "NOT_ACTIVE");
        require(publicSaleTimeIsActive(), "NOT_ACTIVE");
        require(
            LaunchpadStorage.state().cfg.publicMintsAllowedPerAddress == 0 ||
                _numberMinted(msg.sender) + _numTokens <=
                LaunchpadStorage.state().cfg.publicMintsAllowedPerAddress,
            "MAX_MINTS_EXCEEDED"
        );
        require(
            totalSupply() + _numTokens <=
                LaunchpadStorage.state().cfg.maxSupply,
            "MAX_SUPPLY_EXCEEDED"
        );
        require(
            msg.value ==
                (LaunchpadStorage.state().cfg.publicPrice * 10 ** 16) *
                    _numTokens,
            "INVALID_PRICE_PAID"
        );
        ...
        if (LaunchpadStorage.state().cfg.fundingDuration > 0) {
            ...
            for (uint256 i = 0; i < _numTokens; i++) {
                LaunchpadStorage.state().data.pricePaid[
        ...
        if (totalSupply() >= LaunchpadStorage.state().cfg.maxSupply) {
            LaunchpadStorage.state().cfg.publicSaleActive = false;
        }
    }
    ...
    function refundGuaranteeActive() public view returns (bool) {
        ...
        return (block.timestamp <
            LaunchpadStorage.state().data.contractDeployTime +
                (LaunchpadStorage.state().advCfg.refundDuration * 86400));
    }
}
...
import {LaunchpadStorage} from "../libraries/LaunchpadStorage.sol";
    ...
    RevokableOperatorFiltererUpgradeable
{
    ...
    function _generateRandomHash(uint256 _tokenId) internal {
        if (
            LaunchpadStorage.state().data.randomHashStore[_tokenId] ==
            bytes32(0)
        ) {
            LaunchpadStorage.state().data.randomHashStore[_tokenId] = keccak256(
    ...
    function _beforeTokenTransfers(
        ...
        if (
            LaunchpadStorage.state().cfg.randomHashActive && from == address(0)
        ) {
