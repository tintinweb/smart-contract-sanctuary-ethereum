  ...
  PausableUpgradeable
{
  ...
  function initialize(ConstructorParams memory _params) public initializer {
    ...
    LibIMPT._checkZeroAddress(address(_params.superUser));
    LibIMPT._checkZeroAddress(address(_params.platformAdmin));
...
interface IInventory {
  ...
  /// @dev reverts function if updating totals will result in negatives
  error TotalMismatch();
  ...
  /// @dev Updates totals when IMPT sends tokens to be burned by Thallo
  /// @param _tokenId the Carbon Credit NFT's token ID (ERC-1155)
  /// @param _amount the amount of tokens sent to Thallo to be burned
  function sendBurnCounts(uint256 _tokenId, uint256 _amount) external;

  /// @dev Updates totals when Thallo confirms the number of tokens burned to IMPT
  /// @param _tokenId the Carbon Credit NFT's token ID (ERC-1155)
  /// @param _amount the amount of tokens Thallo has burned
  function confirmBurned(uint256 _tokenId, uint256 _amount) external;
  ...
  function tokensMinted(uint256 _tokenId) external returns (uint256);

  function totalSupply(uint256 _tokenId) external returns (uint256);

  function imptBurnCount(uint256 _tokenID) external returns (uint256);

  function toBeBurned(uint256 _tokenID) external returns (uint256);

  function confirmedBurned(uint256 _tokenID) external returns (uint256);
}
