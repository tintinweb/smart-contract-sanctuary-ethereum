...
contract Coinlend {
    /* Type declarations */
    enum LoanState {
        OPEN, // 0 Loans are in the state OPEN if they are not taken and are therefore loan offers (LEND or BORROW)
        CANCELED, // 1 Open loans that were cancelled by the creater before they were taken
        ACTIVE, // 2 Loan offers that were taken by a lender or borrower
        COMPLETE // 3 The loan was either payed back or liquidated
    }
    ...
    struct Loan {
        ...
        LoanState loanState; // The state of the loan (OPEN, CANCELED, ACTIVE, COMPLETE)
    ...
    function createLoanLend(
        ...
        uint256 tokenCollateralAmount = (tokenLendAmount * 100 * getLatestPrice(tokenLend)) /
            ltv /
            getLatestPrice(tokenCollateral);
    ...
    function cancelLoan(uint256 _id) public {
        ...
        require(_loan.loanState == LoanState.OPEN);
    ...
    function payBackLoanInternal(Loan memory _loan, uint256 interestAmount, uint256 fee) internal {
        ...
        bool senderHasEnoughCredits = senderHasEnoughCoinlendCredits(fee, _loan.tokenLending);

        if (senderHasEnoughCredits) {
            tokens[coinlendCredits][feeAccount] +=
                (((fee * getLatestPrice(_loan.tokenLending)) / 1e18) *
                    (100 - coinlendCreditsDiscount)) /
                100;
        } else {
    ...
    function hasBorrowerEnoughFundsForPayback(
        ...
        if (senderHasEnoughCoinlendCredits(fee, _loan.tokenLending)) {
            fee = 0;
        }
    ...
    function senderHasEnoughCoinlendCredits(
        uint256 amount,
        address tokenAddress
    ) internal view returns (bool) {
        return (balanceOf(coinlendCredits, msg.sender) >=
            (((amount * getLatestPrice(tokenAddress)) / 1e18) * (100 - coinlendCreditsDiscount)) /
                100);
    }
    ...
    function forceLiquidateExpiredLoan(uint256 _id) public {
        ...
        require(_loan.timestamp + expirationBuffer < block.timestamp);
    ...
    function getLTVOfLoan(Loan memory loan) internal view returns (uint256) {
        uint256 tokenLendingTotalValue = (getLatestPrice(loan.tokenLending) *
            loan.tokenLendingAmount) / 1e18;

        uint256 tokenCollateralTotalValue = (getLatestPrice(loan.tokenCollateral) *
            loan.tokenCollateralAmount) / 1e18;

        uint256 ltv = (tokenLendingTotalValue * 100) / tokenCollateralTotalValue;
    ...
    function addPriceFeed(address currencyAddress, address priceFeedAddress) public {
        ...
        priceFeeds[currencyAddress] = AggregatorV3Interface(priceFeedAddress);
    }
    ...
    function setExpirationBuffer(uint256 _expirationBuffer) public {
        require(msg.sender == owner);
        expirationBuffer = _expirationBuffer;
    }

    function setCoinlendCredits(address _coinlendCredits) public {
        require(msg.sender == owner);
