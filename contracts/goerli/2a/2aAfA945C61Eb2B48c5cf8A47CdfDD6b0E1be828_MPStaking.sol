    ...
    UUPSUpgradeable
{
    ...
    function unStakeWithRewards (
        ...
        if(_unStakingPercent == denominator) {
            ...
            holderStakeInfo[msg.sender][_token].unStaked = true;
        }

        holderStakeInfo[msg.sender][_token].amount -= unStakingAmount;
    ...
    function _calculateRewardsForToken(address _stakerAddress, address _rewardToken, bool _andClaim) private returns(uint256 amount){
        amount = 0;
        ...
        for(uint256 i = 0; i < stakeTokenAddress.length; ++i){
            ...
            if(pendingBalance/accuracyFactor > 0){
                if(_andClaim) {
                    stakeInfo.totalExcluded[_rewardToken] += pendingBalance;
                }
                amount += pendingBalance / accuracyFactor;
            }
        }
    }
    ...
    function claimRewards (
        ...
        for (uint idx = 0; idx < tokenCount; idx ++) {
            address token = rewardTokens.at(idx);
    ...
    function compoundRewards (
        ...
        require(compoundAmount > 0, "You have no rewards to withdraw!");
