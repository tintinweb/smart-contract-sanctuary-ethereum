...
contract NFTTrading is NFTTradingWrapper, OwnableUpgradeable, ReentrancyGuardUpgradeable, ERC721Holder {
    ...
    function registerNftCollection(uint256 _tokenId, string memory color) external delegatedOnly nonReentrant {
        ...
        Registration memory registration = Registration(msg.sender, address(0), _tokenId, color, false, getTimestamp(), 0);    
    ...
    function getListByTrait(string memory _color) public view returns(Registration [] memory) {
        ...
        for (i = 1; i <= _registered.current(); i ++) {
            if (keccak256(bytes(registrations[i].targetTrait)) == keccak256(bytes(_color)) && !registrations[i].traded) {
