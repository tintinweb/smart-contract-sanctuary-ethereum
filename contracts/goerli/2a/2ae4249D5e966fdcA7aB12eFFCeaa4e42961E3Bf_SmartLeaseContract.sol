...
contract SmartLeaseContract {
    ...
    // 租客潛在量(參與過議價的所有潛在租客)
    uint8 public potentialTenantAccumulated = 0;
    // 押金
    uint256 deposit;
    // 斡旋金(斡旋保證金 10%)
    uint256 earnestMoney;
    ...
    */
    ...
    // (傳入參數：租客地址、租金、押金)
    /* 這裡似乎可以把「_tenantAddress」修改為 sender */
    function applyForTenant()
        external
        isContractProposed
        notZeroAddres(msg.sender)
    {
        ...
        // 條件：租客地址不可重複
        require(
            addressToTenant[msg.sender].initialized == false,
            "Duplicate tenants are not allowed."
        );
        // 加入租客：將租客加入全部潛在租客陣列「allPotentialTenants」
        // 申請時還沒押金
        /* 假設有之前的押金該如何處置？之後來處理 */
        uint256 _depositAmount = 0;
        uint256 _rentAmount = 0;
