...
contract BoundCurve {
    ...
    uint256 FULL_DISCOUNT_PERIOD = 24 * 60 * 60;
    ...
    //BURN RELATED
    mapping(uint256 => uint256[]) private _tokenIdsByDay;
    mapping(uint256 => uint256) private _tokensCounterByDay;
    uint256 private _lastBurnedDay = 0;
    uint256 private _burnPeriodDays = 1000;
    ...
    constructor(address _feeReceiver) {
        ...
        _lastBurnedDay = getCurrentMintDay() - 1;
    }
        ...
        )
    {
        ...
        uint256 daysPass = (secondPassedStepped * PERCENT_PREC) / (FULL_DISCOUNT_PERIOD);
        ...
        currentMintPrice = ifMintsWereInCurrentTimestep ? holdPrice : _getPrice(currentMintPriceMultiplier);
        // currentSlope = slope;
    ...
    function _getPrice(uint256 _currentK) internal view returns (uint256) {
    ...
    function mintRegistration(uint256 tokenId) public {
        ...
        // burn related
        uint256 mintDay = getCurrentMintDay();
        _tokenIdsByDay[mintDay].push(tokenId);
        _tokensCounterByDay[mintDay] += 1;
    }
    ...
    function _getGrowK(uint256 daysPass, uint256 _slope) internal view returns (uint256 growK) {
    ...
    function _getPriceK(uint256 daysPass, uint256 _slope) internal pure returns (uint256) {
        uint256 k = (_slope * getDiscountK(daysPass)) / PERCENT_PREC;
        return k;
    }

    // BURN SECTION
    function burnOldHolders() public returns (uint256 coins, uint256 tokens) {
        uint256 mintDay = getCurrentMintDay();
        if (mintDay > _burnPeriodDays) {
            for (uint256 day = _lastBurnedDay; day < mintDay - _burnPeriodDays; day++) {
                tokens += _tokensCounterByDay[day];
                for (uint256 tokenNumber = 0; tokenNumber < _tokensCounterByDay[day]; tokenNumber++) {
                    uint256 tid = _tokenIdsByDay[day][tokenNumber];
                    uint256 c = _shareCoinsByTokenId[tid];
                    coins += c;
                    if (c > 0) {
                        _shareCoinsByTokenId[tid] = 0;
                        _shareCoinsTotalSupply -= c;
                    }
                }
            }
            _lastBurnedDay = mintDay;
        }
    }

    function getBurnInfo() public view returns (uint256 coins, uint256 tokens) {
        if (getCurrentMintDay() > _burnPeriodDays) {
            for (uint256 day = _lastBurnedDay; day < getCurrentMintDay() - _burnPeriodDays; day += 1) {
                tokens += _tokensCounterByDay[day];

                for (uint256 tokenNumber = 0; tokenNumber < _tokensCounterByDay[day]; tokenNumber += 1) {
                    coins += _shareCoinsByTokenId[_tokenIdsByDay[day][tokenNumber]];
                }
            }
        }
    }

    function getCurrentMintDay() private view returns (uint256 day) {
    ...
    function setParameters(
        ...
        require(kmin > 10000, "kmin > 1");
        require(kmax > 10000, "kmax > 1");
        require(timestep > 0, "timestep > 1");
        require(discperiod > 0, "discperiod > 1");

        minSlope = kmin;
        maxSlope = kmax;
        TIMESTEP_PERIOD = timestep;
        FULL_DISCOUNT_PERIOD = discperiod;
    }
}
