...
contract UnimoonTreasury is AccessControl, ReentrancyGuard {
    ...
    function purchaseNewAltcoin(
        ...
        purchases[path[path.length - 1]].push(
            Purchase(
                amounts[amounts.length - 1],
                amount,
                0,
                (amount * (10**IToken(path[path.length - 1]).decimals())) /
                    amounts[amounts.length - 1]
            )
        );
        emit PurchaseNewAltcoin(
            ...
            amounts[amounts.length - 1],
            amount,
            purchases[path[path.length - 1]].length - 1
        );
    }
    ...
    function sellAltcoin(
        ...
        require(purchases[path[0]].length > id, "UnimoonTreasury: wrong id");
        // require(sellPercent <= DENOMINATOR && sellPercent > 0, "UnimoonTreasury: wrong percent");
        Purchase storage purch = purchases[path[0]][id];
        ...
        require(
            amountToSell > 0 &&
                IToken(path[0]).balanceOf(address(this)) >= amountToSell,
            "UnimoonTreasury: wrong amount"
        );

        TransferHelper.safeApprove(path[0], address(ROUTER), amountToSell);
        uint256[] memory amounts = ROUTER.swapExactTokensForTokens(
            amountToSell,
            amountOutMin,
            path,
        ...
        emit SellAltcoin(path[0], id, amountToSell);
    }
    ...
    function changeProfitDistribution(uint8 _profitDistribution)
        ...
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        require(_profitDistribution <= DENOMINATOR, "UnimoonTreasury: wrong percent");
        ...
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        ...
        if (balance > 0) {
            // while (purchases[_token].length > 0) purchases[_token].pop();
