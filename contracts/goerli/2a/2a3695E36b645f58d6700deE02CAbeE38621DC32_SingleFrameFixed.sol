...
contract SingleFrameFixed is Ownable, ERC721Holder {
    ...
    event SingleOfferAccepted(address collection, uint256 tokenId, uint256 offerId);
    event SingleLazyOfferAccepted(uint256 tokenId, Offer[] offers);
    ...
    function onAcceptOffer(address _collectionId, uint256 _tokenId, uint256 _offerId, uint256 _pairId, uint256 _auctionId) public {
        ...
        require(governanceToken.transferFrom(offer.owner, msg.sender, offer.offerPrice), "transfer to contract failed");
        ...
        offer.active = false;
        emit SingleOfferAccepted(offer.collection, offer.tokenId, _offerId);
    }
    ...
    function onLazyAcceptOffer(LazyItem memory lazyItem, Offer[] memory _offers, uint256 _offerId) external returns (uint256){
        ...
        emit SingleLazyOfferAccepted(tokenId, _offers);
        return tokenId;
    }
