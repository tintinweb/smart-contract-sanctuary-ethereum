...
contract TokenCrowdsale is ReentrancyGuard {
    ...
    event Refunded(uint256 _id, address _investorAddress, uint256 amount);
    ...
    function refundTokens(uint256 _id) external nonReentrant {
        require(hasClosed(_id), "Crowdsale: Crowdsale not closed");
        //get user eth and token contributions
        uint256 ethContribution = crowdsales[_id].contributions[msg.sender];
        uint256 tokenContribution = crowdsales[_id].tokenBalances[msg.sender];
        
        //transfer tokens from the user to the crowdsale
        uint256 allowance = crowdsales[_id].token.allowance(msg.sender, address(this));
        require(allowance >= tokenContribution, "Invalid token allowance");
        crowdsales[_id].token.transferFrom(msg.sender, address(this), tokenContribution);
        crowdsales[_id].tokenBalances[msg.sender] = 0;
        
        //transfer ethcontribution to the user
        payable(msg.sender).transfer(ethContribution);
        crowdsales[_id].contributions[msg.sender] = 0;
        crowdsales[_id].weiRaised.sub(ethContribution);

        emit Refunded(_id, msg.sender, ethContribution);
    }
}
