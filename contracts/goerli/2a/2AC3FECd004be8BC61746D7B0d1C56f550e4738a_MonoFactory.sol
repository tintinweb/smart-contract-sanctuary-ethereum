...
contract Mono is ERC20PresetMinterPauserUpgradeable {
...
import "contracts/Mono.sol";
import "contracts/interfaces/IMulticall.sol";

/**
 * @title MonoFactory
 * @author Ondo Finance
 * @notice This contract serves as a Factory for the upgradable mono token contract array.
 *         Upon calling `deployMono` the `guardian` address (set in constructor) will
 *         deploy the following:
 *         1) mono - The implementation contract, ERC20 contract with the initializer disabled
 ...
 *                                          `_logic' is set to `address(mono)`.
 *
 *         Following the above mentioned deployment, the address of the MonoFactory contract will:
 ...
 * @notice `deployMono` is designed to be called once this factory is to be used once,
 *          e.g. throwaway contract.
 */

contract MonoFactory is IMulticall {
  bytes32 public constant MINTER_ROLE = keccak256("MINTER_ROLE");
  bytes32 public constant PAUSER_ROLE = keccak256("PAUSER_ROLE");
  ...
  /**
   * @dev This function will deploy an upgradable instance of Mono
   *
   ...
   *            address.
   *
   * @notice Guardian will need to grant Treasury: MINTER_ROLE
   */
  function deployMono(string calldata name, string calldata ticker)
    external
    onlyGuardian
    returns (
      address,
  ...
  {
    Mono impl = new Mono();
    ProxyAdmin monoProxyAdmin = new ProxyAdmin();
    TokenProxy monoProxy =
      new TokenProxy(address(impl), address(monoProxyAdmin), "");
    Mono monoProxied = Mono(address(monoProxy));
    monoProxied.initialize(name, ticker);

    monoProxied.grantRole(DEFAULT_ADMIN_ROLE, guardian);
    monoProxied.grantRole(PAUSER_ROLE, guardian);

    monoProxied.revokeRole(MINTER_ROLE, address(this));
    monoProxied.revokeRole(PAUSER_ROLE, address(this));
    monoProxied.revokeRole(DEFAULT_ADMIN_ROLE, address(this));

    monoProxyAdmin.transferOwnership(guardian);
    assert(monoProxyAdmin.owner() == guardian);
    emit MonoDeployed(
      address(monoProxied),
      address(monoProxyAdmin),
      address(impl)
    );
    return (address(monoProxied), address(monoProxyAdmin), address(impl));
  }
  ...
  /**
   * @dev Event emitted when upgradable mono is deployed
   *
   ...
   */
  event MonoDeployed(address proxy, address proxyAdmin, address implementation);

  modifier onlyGuardian() {
    require(msg.sender == guardian, "You are not the Guardian");
    _;
  }
}
