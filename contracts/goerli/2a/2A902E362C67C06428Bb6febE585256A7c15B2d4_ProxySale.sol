...
contract ProxySale is Ownable, ReentrancyGuard, IProxySale {
    ...
    event Stake(address indexed owner, StakeInfo indexed stakeInfo);
    ...
    mapping(bytes => uint256) public verifiedList;
    ...
    function stake(uint256 _tokenId, uint256 _stakeType) public {
        ...
        emit Stake(msg.sender, stakeList[_tokenId]);
    }
    ...
    function unStake(string calldata salt, bytes calldata _signature,  uint256 _tokenId) external {
        ...
        require(_verify(_hash(salt, msg.sender), _signature), "Invalid signature");

        require(verifiedList[_signature] > 0, "Already unStaked");

        delete stakeList[_tokenId];

        verifiedList[_signature] = _tokenId;
