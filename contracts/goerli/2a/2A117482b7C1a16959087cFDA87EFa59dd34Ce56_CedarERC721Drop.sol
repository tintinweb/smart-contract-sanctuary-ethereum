...
import "../terms/lib/TermsLogic.sol";
import "../drop/interfaces/IOwnable.sol";
import "../generated/impl/BaseTermsRegistryV0.sol";

/// @title TermsRegistry
/// @notice This contract is responsible for managing the terms of use for 3rd party ERC721 and ERC1155 contracts.
contract TermsRegistry is ContextUpgradeable, UUPSUpgradeable, AccessControlUpgradeable, BaseTermsRegistryV0 {
    /// ================================
    /// =========== Libraries ==========
    /// ================================
    using TermsLogic for TermsDataTypes.Terms;
    ...
    function supportsInterface(bytes4 interfaceId)
        public
        view
        override(BaseTermsRegistryV0, AccessControlUpgradeable)
        returns (bool)
    {
        return
            BaseTermsRegistryV0.supportsInterface(interfaceId) ||
            AccessControlUpgradeable.supportsInterface(interfaceId);
    }
    ...
    function acceptTerms(address _token) external {
        ...
        if (termsData.termsAccepted[_msgSender()] && termsData.acceptedVersion[_msgSender()] == termsData.termsVersion)
            revert TermsAlreadyAcceptedForToken(_token, termsData.termsVersion);
        termsData.termsAccepted[_msgSender()] = true;
        termsData.acceptedVersion[_msgSender()] = termsData.termsVersion;
    ...
    function setTermsActivation(address _token, bool _active) external {
        if (IOwnable(_token).owner() != _msgSender()) revert TermsCanOnlyBeSetByOwner(_token);
        TermsDataTypes.Terms storage termsData = terms[_token];
        if (_active) {
    ...
    function multicall(bytes[] calldata data) external override(IMulticallableV0) returns (bytes[] memory results) {
        return IMulticallableV0(this).multicall(data);
    }
...
// Generated by impl.ts. Will be overwritten.
// Filename: './BaseTermsRegistryV0.sol'
...
/// Inherit from this base to implement introspection
abstract contract BaseTermsRegistryV0 is ITermsRegistryV0 {
    function supportedFeatures() override public pure returns (string[] memory features) {
        features = new string[](4);
        features[0] = "ICedarFeatures.sol:ICedarFeaturesV0";
        features[1] = "ICedarVersioned.sol:ICedarVersionedV2";
        features[2] = "agreement/IAgreementsRegistry.sol:IAgreementsRegistryV0";
    ...
    function implementationInterfaceId() virtual override public pure returns (string memory interfaceId) {
        interfaceId = "impl/ITermsRegistry.sol:ITermsRegistryV0";
    }

    function supportsInterface(bytes4 interfaceID) virtual override public view returns (bool) {
        return (interfaceID == type(IERC165Upgradeable).interfaceId) || ((interfaceID == type(ICedarFeaturesV0).interfaceId) || ((interfaceID == type(ICedarVersionedV2).interfaceId) || ((interfaceID == type(IAgreementsRegistryV0).interfaceId) || ((interfaceID == type(IMulticallableV0).interfaceId) || (interfaceID == type(ITermsRegistryV0).interfaceId)))));
    }
...
interface ITermsRegistryV0 is ICedarFeaturesV0, ICedarVersionedV2, IAgreementsRegistryV0, IMulticallableV0 {}
