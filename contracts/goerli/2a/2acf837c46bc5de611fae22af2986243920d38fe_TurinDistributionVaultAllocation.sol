...
import {VestingUpgradeable} from "./base/VestingUpgradeable.sol";
...
abstract contract DistributionVaultAllocationUpgradeable is DistributionVaultUpgradeable {
    ...
    function addAccount(address account, uint256[] calldata allocations) public {
        _authorizeUpdateAllocation();
        _addAccount(account, allocations);
    }

    function updateAllocation(uint256[] calldata allocations) public {
        _authorizeUpdateAllocation();
    ...
    function getInfo(address account) public view returns (Info memory) {
    ...
    function distribute(address receiver, uint256 amount) external {
        ...
        require(amount <= _available(account), "");
...
abstract contract DistributionVaultUpgradeable is Initializable {
    ...
    function _distribute(address account, uint256 amount) internal {
        ...
        ERC20(_token).safeTransfer(account, amount);
...
contract VestingUpgradeable is Initializable {
    uint8 public vestingType; // 1: constant, 2: linear, 3: cuadratic, 4: cubic ...
    ...
    function __VestingUpgradeable_init_unchained(
        ...
        endDate = endDate_;
        increment = (vestingType_ * totalAmount_ * 1 ether) / ((endDate_ - startDate_) ** vestingType_);
    }
    ...
    function _releasedRange(uint256 from, uint256 to) internal view virtual returns (uint256) {
        ...
        uint256 comp0 = (increment * ((to - startDate) ** vestingType)) / vestingType;
        uint256 comp1 = (increment * ((from - startDate) ** vestingType)) / vestingType;
...
import {VestingUpgradeable} from "dappcomposer-contracts/contracts-upgradeable/distribute/base/VestingUpgradeable.sol";
import {Ownable2StepUpgradeable} from "openzeppelin-contracts-upgradeable/access/Ownable2StepUpgradeable.sol";
import {UUPSUpgradeable} from "openzeppelin-contracts-upgradeable/proxy/utils/UUPSUpgradeable.sol";

contract TurinDistributionVaultAllocation is
