...
contract OCASnake is Ownable, ERC721A, ERC2981, ReentrancyGuard, OCASnakeArt {
    ...
    function recursiveNameFind(
        ...
        if (
            ...
            compareArray[4] == indexCompare ||
            compareArray[5] == indexCompare ||
            compareArray[6] == indexCompare ||
            compareArray[7] == indexCompare ||
            compareArray[8] == indexCompare
        ) {
            found = true;
            // break;
        }
        ...
        if ((index < 1998 && !found) || tokenId == index) {
            ...
            supreme = generateAppleSeed(tokenId, seed + 69) % 2 == 1
        ...
        returns (bytes memory)
    {
        ...
        return abi.encodePacked(metaBytes, metadata[2]);
    }
    ...
    function getStakedTime(uint256 tokenId) public view returns (uint256) {
        uint256 timeStaked = tokenInfoList[tokenId].stakeDate;
        uint256 currentTime = block.timestamp;
        uint256 timeDiff = currentTime - timeStaked;
        //turn uint256 into timestamp
        return timeDiff;
    }
        ...
        returns (string memory)
    {
        ...
        string[2] memory metadataVars = [
            string(recursiveNameFind(tokenId, 0, 0, true, "0", arr)),
            string(image_funk(tokenId))
        ];
        string memory json = Base64.encode(metaDataCreator(metadataVars));
