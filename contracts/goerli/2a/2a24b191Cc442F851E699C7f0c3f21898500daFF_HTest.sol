...
contract HTest is Ownable, ERC721A, ReentrancyGuard {
    ...
    /// @notice total number of bids entered 
    uint256 public totalBids;
    /// @notice map bidder to boolean
    mapping(address => bool) public bidderToBoolMap;
    /// @notice number of bidders
    uint256 public numBidders;
    /// @notice last bidder
    address public lastBidder;
    /// @notice number of top bidders
    uint256 public MAX_TOP_BIDDERS;
    /// @notice top bidders
    Info[] public topBidders;
    ...
    /// @notice event emitted when the owner mint unsold NFTs
    event DevMinted(address indexed user, uint256 quantity);


    event NewBaseURI(string newURI, address updatedBy);
    event UpdatedMinBidPrice(uint256 price, address updatedBy);
        ...
        )
    {
        ...
        MAX_TOP_BIDDERS = _MAX_TOP_BIDDERS;
    }
    ...
    ///@notice extract top bidders according highest bid price.
    function refreshTopXOrdered(address addr, uint256 price) private {
        if ( topBidders.length < MAX_TOP_BIDDERS ) {
            topBidders.push(Info(addr, price));
        }   
        uint256 i;
        /** get the index of the current max element **/
        for(i = 0; i < topBidders.length; i++) {
            if(topBidders[i].bidPrice < price) {
                break;
            }
        }
        /** shift the array of one position (getting rid of the last element) **/
        for(uint256 j = topBidders.length - 1; j > i; j--) {
            topBidders[j] = topBidders[j - 1];
        }
        /** update the new max element **/
        if ( i < topBidders.length) {
            topBidders[i].addr =  addr;
            topBidders[i].bidPrice =  price;
        }
    }
    ...
    function enterBid(uint256 quantity, uint256 price) external payable callerIsUser {
        ...
        totalBids += quantity;
        if ( bidderToBoolMap[msg.sender] == false ) {
            bidderToBoolMap[msg.sender] = true;
            numBidders++;
        }
        lastBidder = msg.sender;
        refreshTopXOrdered(msg.sender, price);
    ...
    ///@notice mints unsold NFTs
    function devMint() external onlyOwner {
        require(block.timestamp > AUCTION_END_TIME, "Auction has not ended");
        uint256 remainingSupply = MAX_SUPPLY - numBids;
        require(remainingSupply > 0, "Nothing to mint");

        uint256 count = 0;
        uint256 numChunks = remainingSupply / MAX_BATCH_SIZE;
        if ( numChunks == 0) {
            ...
            uint256 leftover = remainingSupply % MAX_BATCH_SIZE;
            if ( leftover != 0 ) {
                _safeMint(msg.sender, leftover);
                count += remainingSupply;
            }
        }

        emit DevMinted( msg.sender, count);
    }

    ///@notice Air Drop NFTs to winning bids.
    function airDrop() external onlyOwner {
        ...
        for ( uint256 i = 1; i < bidPriorityQueue.bidIdList.length; i++ ) {
            uint256 curBidId = bidPriorityQueue.bidIdList[i];
            ...
            balanceContributed[curBid.owner] -= qty * clearingPrice;
            curBid.quantity = 0;

            require(qty > 0, "quantity must be greater than zero");
            require(qty <= MAX_BATCH_SIZE, "Too many batch mint");
            require(totalSupply() + qty <= MAX_SUPPLY, "Would exceed max supply");
            
            mint(curBid.owner, qty);
        }
    ...
    function claim() public nonReentrant callerIsUser {
        ...
        require(isAirDropped, "AirDrop has not ended");
        require(enableClaim, "Claim not enabled now");
        require(block.timestamp > AUCTION_END_TIME, "Auction has not ended");

        uint256 refundedAmount = balanceContributed[msg.sender];
        //refund any contributions not spent on mint
        (bool sent, ) = payable(msg.sender).call{value: refundedAmount}("");
        require(sent, "Unsuccessful in refund");

        //set contribution to zero to avoid double claiming
        balanceContributed[msg.sender] = 0;

        emit Claimed(msg.sender, refundedAmount);
    }
    ...
    function getTopBidders() public view returns (Info[] memory) {
        Info[] memory _topBidders = topBidders;
        return _topBidders;
    }

    function setEnableClaim(bool enable) external onlyOwner {
        enableClaim = enable;
    }
    ...
    function setMinBidPrice(uint256 price) external onlyOwner {
        require(price > 0, "Wrong value");
        MIN_BID_PRICE = price;
        emit UpdatedMinBidPrice(price, msg.sender);
    }
