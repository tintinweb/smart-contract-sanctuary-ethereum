...
contract Deployer {
    ...
    constructor(
        uint32 cycleSecs,
        address dripsHubAdmin,
        address addressDriverAdmin,
        address nftDriverAdmin,
        address immutableSplitsDriverAdmin
    ) {
        Managed logic;
        ManagedProxy proxy;

        logic = new DripsHub(cycleSecs);
        proxy = new ManagedProxy(logic, dripsHubAdmin);
        dripsHub = DripsHub(address(proxy));

        caller = new Caller();

        logic = new AddressDriver(dripsHub, address(caller), dripsHub.nextDriverId());
        proxy = new ManagedProxy(logic, addressDriverAdmin);
        addressDriver = AddressDriver(address(proxy));
        // slither-disable-next-line unused-return
        dripsHub.registerDriver(address(addressDriver));

        logic = new NFTDriver(dripsHub, address(caller), dripsHub.nextDriverId());
        proxy = new ManagedProxy(logic, nftDriverAdmin);
        nftDriver = NFTDriver(address(proxy));
        // slither-disable-next-line unused-return
        dripsHub.registerDriver(address(nftDriver));

        logic = new ImmutableSplitsDriver(dripsHub, dripsHub.nextDriverId());
        proxy = new ManagedProxy(logic, immutableSplitsDriverAdmin);
        immutableSplitsDriver = ImmutableSplitsDriver(address(proxy));
        // slither-disable-next-line unused-return
        dripsHub.registerDriver(address(immutableSplitsDriver));
    }
}
