...
abstract contract DistributionVaultAllocationUpgradeable is DistributionVaultUpgradeable {
    ...
    function _canUpdateAllocation(address sender) internal view virtual returns (bool);
    ...
    function addAccount(address account, uint256[] calldata allocations) public {
        require(_canUpdateAllocation(msg.sender), "");
...
abstract contract DistributionVaultUpgradeable is Initializable, UUPSUpgradeable {
...
abstract contract VestingUpgradeable is Initializable, UUPSUpgradeable {
    ...
    function __VestingUpgradeable_init_unchained(
        ...
        endDate = endDate_;
        increment = (vestingType_ * totalAmount_) / ((endDate_ - startDate_) ** vestingType_);
    }
...
contract TurinDistributionVault is DistributionVaultAllocationUpgradeable, VestingUpgradeable {
    constructor() {
        _disableInitializers();
    }

    function initialize(address _token, uint8 vestingType, uint256 totalAmount, uint256 startDate, uint256 endDate)
        public
        initializer
    {
        __TurinDistributionVault_init(_token, vestingType, totalAmount, startDate, endDate);
    }
    ...
    function _canUpdateAllocation(address sender) internal pure override returns (bool) {
        // TODO
        (sender);
        return true;
    }
    ...
    // solhint-disable-next-line
    function __TurinDistributionVault_init(
        address _token,
        uint8 vestingType,
        uint256 totalAmount,
        uint256 startDate,
        ...
        __TurinDistributionVault_init_unchained();
    }

    // solhint-disable-next-line
    function __TurinDistributionVault_init_unchained() internal onlyInitializing {}
}