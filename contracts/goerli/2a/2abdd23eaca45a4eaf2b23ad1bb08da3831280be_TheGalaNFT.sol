...
contract TheGalaNFT is ERC721Enumerable, Ownable {
  ...
  // External
    ...
    function mint(uint256 _mintAmount) public payable {
      ...
      if
      (whitelistSale == true) {
        require(isWhitelisted(msg.sender), "YOU ARE NOT WHITELISTED; AWAIT PUBLIC MINT!");
        ...
        require(whitelistMintCount + _mintAmount < maxPerAddressPlusOne, "MAX ASSETS PER WALLET EXCEEDED");
      ...
      (rsvpSale == true) {
        require(isRsvp(msg.sender), "YOU DID NOT RSVP; AWAIT PUBLIC MINT!");
        require(supply + _mintAmount < maxSupplyPlusOne, "MAX SUPPLY EXCEEDED");
        require(0 < _mintAmount && _mintAmount < maxPerSessionPlusOne, "MAX ASSETS PER SESSION EXCEEDED");
        uint256 rsvpMintCount = rsvpPerAddress[msg.sender];
        require(rsvpMintCount + _mintAmount < maxPerAddressPlusOne, "MAX ASSETS PER WALLET EXCEEDED");
        require(msg.value >= cost * _mintAmount, "INSUFFICIENT FUNDS");
        for (uint256 i = 1; i <= _mintAmount; i++) {
    ...
    function isWhitelisted(address _user) public view returns (bool) {
      ...
      if (phase3 == false) {
        for (uint m = 0; m < whitelistAddressesPhase4.length; m++) {
          if (whitelistAddressesPhase4[m] == _user) {
            phase3 = true;
            return true;
          }
        }
      }
      if (phase1 == false && phase2 == false && phase3 == false && phase4 == false) {
          return false;
      }
      */
    }
  ...
  // Only Owner
    ...
    function setMaxSupplyPlusOne(uint256 _newMaxSupplyPlusOne) public onlyOwner {
      maxSupplyPlusOne = _newMaxSupplyPlusOne;
    }
