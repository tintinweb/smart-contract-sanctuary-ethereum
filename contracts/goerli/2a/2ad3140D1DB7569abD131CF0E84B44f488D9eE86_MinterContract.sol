...
contract MinterContract is Ownable {
    ...
    Starify public _starify;

    address private _signer;

    constructor(address charityAddress, address starifyAddres, address signer) {
        _charity = Charity(charityAddress);
        _starify = Starify(starifyAddres);
        _signer = signer;
    }

    function _mint (uint256 charityId, uint256 amount, address tokenOwner, bytes memory signature ) public {
        require(verify(charityId, amount, tokenOwner, signature), "Invalid sign");
        _charity.addCharity(charityId, amount);
        _starify.safeMint(tokenOwner, amount, charityId);
    }

    function _mintforOwner (uint256 charityId, uint256 amount, address tokenOwner) public onlyOwner {
        //require(verify(charityId, amount, tokenOwner, signature), "Invalid sign");
        _charity.addCharity(charityId, amount);
        _starify.safeMint(tokenOwner, amount, charityId);
    }
    ...
    function getMessageHash(
        ...
        return keccak256(abi.encodePacked(charityId, amount, tokenOwner));
    }
    ...
    function verify(
        ...
        bytes32 messageHash = getMessageHash(charityId, amount, tokenOwner);
...
contract Charity {
    ...
    function balanceOf (uint256 charityId) public view returns(uint256 amount) {
...
contract Starify is Initializable, ERC721A, OwnableUpgradeable {
    
    
    // constructor(uint256 maxBatchSize_, uint256 collectionSize_) ERC721A("Starify", "STF", maxBatchSize_, collectionSize_) {
    // }

    function initialize (uint256 maxBatchSize_, uint256 collectionSize_) public initializer {
        __ERC721A_init("Starify", "STF", maxBatchSize_, collectionSize_);
        __Ownable_init();
        minter = 0xAE492E3873945F9af9B6caD802e030e2935073cE;
    }
    ...
    event tokenCreated(uint256 quantity, address tokenOwner, uint256 CharityId);
    ...
    function safeMint(address to, uint256 quantity, uint256 charityId) public onlyMinter {
        if(isLimitSet) {
    ...
    IERC721EnumerableUpgradeable
{
    ...
    struct TokenOwnership {
        ...
        uint256 charityId; //////////////////////////add charityId///////////////////////////
    }
    ...
    function ownershipOf(
        uint256 tokenId ///////////////////////internal ==>> public /////////////////////
        ...
        if (_ownerships[nextTokenId].addr == address(0)) {
            if (_exists(nextTokenId)) {
                _ownerships[nextTokenId] = TokenOwnership(
                    ...
                    prevOwnership.charityId /////////////////////////////////add charityId////////////////////////////////
                );
            }
        }
        ...
        for (uint256 i = oldNextOwnerToSet; i <= endIndex; i++) {
            if (_ownerships[i].addr == address(0)) {
                ...
                _ownerships[i] = TokenOwnership(
                    ...
                    ownership.charityId /////////////////////////////////add charityId////////////////////////////////
                );
            }
        }
