  ...
  function setEmissionRate(uint256[] calldata rarities, uint256[] calldata rates) external onlyOwner {
    ...
    unchecked {
      for (uint256 i = 0; i < rarities.length; i++) {
        tokenRarityToRate[rarities[i]].push(
          Rate(rates[i] * WAD, block.timestamp)
        );
      }
    }
  }

  function currentEmissionRate(uint256 tokenId) public view returns (uint256 emissionRate) {
    uint256 tokenRarity = tokenIdToRarity[tokenId];
    uint256 numRates = tokenRarityToRate[tokenRarity].length;
    if (numRates > 0) {
      emissionRate = tokenRarityToRate[tokenRarity][numRates - 1].value;
    }
  }

  function amountToMint(uint256 tokenId, uint256 timestamp, uint256 interval) public view returns (uint256 amount, uint256 newTimestamp) {
    Rate[] memory rates = tokenRarityToRate[tokenIdToRarity[tokenId]];
