    ...
    ReentrancyGuard
{
    ...
    function computePayout(address depositor) internal view returns (uint256, uint256) {
        ...
        uint256 alreadyPaid = users[depositor].collectedReward + users[depositor].collectedRewardExternal;
        if (totalPayout > alreadyPaid) {
            return (totalPayout - alreadyPaid, alreadyPaid);
        } else {
            return (0, alreadyPaid);
        }
    }
    ...
    function _collectReward(
        ...
        (uint256 pending, uint256 collected) = computePayout(depositorAddress);
        ...
        internal
    {
        ...
        require(validatorOwner == from, "Validator not owned by from address");
        ...
        emit ValidatorTransferred(validatorPubKey, from, to, transferTimestamp);
    }
    ...
    function transferValidatorByOwner(bytes calldata validatorPubKey, address to) external override nonReentrant {
        ...
        _transferValidator(validatorPubKey, msg.sender, to, block.timestamp);
    }
    ...
    function transferValidatorByAdmin(
        ...
        require(fromAddresses.length == toAddresses.length, "fromAddresses and toAddresses must have the same length");
        require(validatorPubkeys.length == fromAddresses.length * 48, "validatorPubkeys byte array length incorrect");
    ...
    // Used by our oracle to inform this contract that the given users are already paid some amount
    // of reward outside of this contract.
    // @param addresses: array of user addresses
    // @param alreadyPaidAmount: array of already paid amounts
    function setPaidForUsers(address[] calldata addresses, uint256[] calldata alreadyPaidAmount)
        external
        override
        adminOnly
    {
        for (uint256 i = 0; i < addresses.length; i++) {
            users[addresses[i]].collectedRewardExternal = uint128(alreadyPaidAmount[i]);
        }
    }
    ...
    function getUserState(address user) external override view returns (uint256, uint256, uint256, uint256, uint256) {
        return (
            users[user].validatorCount,
            users[user].totalStartTimestamps,
            users[user].partedUptime,
            users[user].collectedReward,
            users[user].collectedRewardExternal
        );
    }
...
interface IStakefishTransactionFeePoolV2 {
    ...
    function setPaidForUsers(address[] calldata addresses, uint256[] calldata alreadyPaidAmount) external;
    ...
    function transferValidatorByAdmin(
        bytes calldata validatorPubkeys, address[] calldata froms, address[] calldata tos, uint256 timestamp) external;
    ...
    function getUserState(address user) external view returns (uint256, uint256, uint256, uint256, uint256);
...
contract StakefishTransactionStorageV2 {
    struct UserSummary {
        ...
        // This variable records rewards that are paid outside of this contract. E.g. paid by the V1 contract.
        uint128 collectedRewardExternal;
    }
