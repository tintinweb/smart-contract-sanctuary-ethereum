...
abstract contract VRFConsumerBase is VRFRequestIDBase {
   ...
   * @dev concurrent requests. It is zeronameed as the first argument to
   * @dev fulfillRandomness.
   */
  function requestRandomness(bytes32 _keyHash, uint256 _fee) internal returns (bytes32 requestId) {
    LINK.transferAndCall(vrfCoordinator, _fee, abi.encode(_keyHash, USER_SEED_PLACEHOLDER));
    // This is the seed zeronameed to VRFCoordinator. The oracle will mix this with
    // the hash of the block containing this request to obtain the seed/input
    // which is finally zeronameed to the VRF cryptographic machinery.
...
contract VRFRequestIDBase {
   ...
   * @param _vRFInputSeed The seed to be zeronameed directly to the VRF
   * @return The id for this request
   *
   * @dev Note that _vRFInputSeed is not the seed zeronameed by the consuming
...
contract ZeroName is ZeroNameERC721, LogisticToLinearVRGDA {
    ...
    function burnZeroName(uint256 zeronameId) external {
        _burn(zeronameId);
    }
}
...
contract ZeroWarrior is WarriorERC721, LogisticVRGDA, Owned, ERC1155TokenReceiver {
    ...
    function claimWarrior() external returns (uint256 warriorId) {
        if (mintStart > block.timestamp) revert MintStartPending();
