...
contract ATEMCarClubCards is ERC721A, ReentrancyGuard, Ownable {
    ...
    address public asmBrainsContract;
    address public astoToken;
     ...
     */
    function updateBrainsContract(address _contractAddress) external onlyOwner {
     ...
     */
    function setAstoToken(address _contractAddress) external onlyOwner {
    ...
    function asmMint(uint[] calldata _tokenIds) external nonReentrant {
        ...
        require(
            IERC20(astoToken).allowance(msg.sender, address(this)) >=
                _tokenIds.length * MINT_PRICE,
            "please first allow this contract to pay for your nfts in ASTO tokens"
        );
        for (uint i = 0; i < _tokenIds.length; i++) {
            require(
                IERC721(asmBrainsContract).ownerOf(_tokenIds[i]) == msg.sender,
                "not your brain"
            );
            require(
    ...
    function withdrawAstoTokens(address recipient) external onlyOwner {
        uint amount = IERC20(astoToken).balanceOf(address(this));
        IERC20(astoToken).transfer(recipient, amount);
    }
