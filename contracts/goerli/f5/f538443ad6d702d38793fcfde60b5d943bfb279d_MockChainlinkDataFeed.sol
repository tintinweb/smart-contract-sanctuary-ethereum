...
contract MockFeed is AggregatorV3Interface{
    mapping (uint256 => int256) public roundData;
    mapping (uint256 => uint256) public timestamps;

    uint80 public latestRoundId;

    function setRoundData(uint256 roundId,uint256 timestamp, int256 data) external {
        roundData[roundId] = data;
        timestamps[roundId] = timestamp;
        if(uint256(latestRoundId) < roundId){
            latestRoundId = uint80(roundId);
        }
    }
    ...
    function getRoundData(uint80 roundId_)
        ...
        roundId = roundId_;
        answer = roundData[uint256(roundId_)];
        startedAt = uint256(timestamps[roundId_]);
        updatedAt = uint256(timestamps[roundId_]);
    ...
    function latestRoundData()
        ...
        answer = roundData[uint256(latestRoundId)];
        startedAt = uint256(timestamps[roundId]);
        updatedAt = uint256(timestamps[roundId]);
...
contract MockChainlinkDataFeed is Script {
    ...
    function deploy() public returns(MockFeed){
        return new MockFeed();
    }
}
    ...
    IChainlinkValidator
{
    ...
    error ChainlinkValidator__value_noFeedForToken(address tokenAddress);
    error ChainlinkValidator__validate_feedNotFoundForToken(
        address tokenAddress
    );
    error ChainlinkValidator__validate_disputeWindowActive();
    error ChainlinkValidator__value_timestampOverflow();
    ...
    /// - `prevNonce` has to be greater than `disputeWindow` has passes and the value
        ...
        returns (uint256 value_, bytes32 nonce)
    {
        ...
        if (dataFeed == address(0)) {
            revert ChainlinkValidator__value_noFeedForToken(token);
        }
        ...
        if (roundTimestamp > type(uint176).max) {
            revert ChainlinkValidator__value_timestampOverflow();
        }

        // Convert value from Chainlink feed to wad precision
        value_ = uconvert(uint256(feedValue), decimals, 18);
        // Encode roundId and roundTimestamp as nonce
        nonce = encodeNonce(uint256(roundId), roundTimestamp);
    }

    /// @notice Validates a value from for given `nonce` via the corresponding Chainlink feed
    /// @dev Reverts if there is no feed `token`
    /// @param value_ Value to be validated [wad]
    ...
    {
        if (canDispute(nonce)) {
            revert ChainlinkValidator__validate_disputeWindowActive();
        }

        address dataFeed = feeds[token];

        if (dataFeed == address(0)) {
            revert ChainlinkValidator__validate_feedNotFoundForToken(token);
        }

        // Retrieve and scale the data from the chainlink feed
        uint256 decimals = AggregatorV3Interface(dataFeed).decimals();
        (, int256 roundValue, , , ) = AggregatorV3Interface(dataFeed)
            .getRoundData(uint80(decodeRoundId(nonce)));

        computedValue = uconvert(uint256(roundValue), decimals, 18);
        valid = value_ == computedValue;

        emit Validate(token, value_, computedValue);
    }
