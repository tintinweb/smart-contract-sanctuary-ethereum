 ...
 */
contract FactoryERC20 is ERC20, AccessControl {
    address public deployer;
    bytes32 public constant MINTER_ROLE = keccak256("MINTER_ROLE");

    constructor(string memory name, string memory symbol) ERC20(name, symbol) {}

    function init(address deployer_) public {
        _setupRole(DEFAULT_ADMIN_ROLE, deployer_);
        _setupRole(MINTER_ROLE, deployer_);
        deployer = deployer_;
    }
...
contract ERC20Deployer {
     ...
     */
    constructor() {
        baseAccount = address(new FactoryERC20{ salt: keccak256("V0.1") }("STAB", "STAB"));
        FactoryERC20(baseAccount).init(address(this));
    }

    // TODO: minting should only be possible by the bridge
    function createERC20(address _deployer) public returns (address) {
        ...
        FactoryERC20(clone).init(_deployer);
        return clone;
    }
