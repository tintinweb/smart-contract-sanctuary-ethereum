...
contract SeedlingsNFT is ERC721AQueryable, ERC2981, Ownable, ReentrancyGuard {
   ...
   function allowlistMint(bytes32[] calldata _merkleProof, uint256  alQuantity_) external payable nonReentrant {
   	require(isAllowlistMintEnabled, 'Premint is not enabled.');
	uint256 totals = totalSupply();
        
	require(totals.add(alQuantity_) <= maxSupply, 'Exceeds maximum supply.');
	
	require(alMinted[msg.sender].add(alQuantity_) <= maxPerWallet, "Maximum Premint per wallet exceeded.");
	require(alQuantity_ <= maxPerTxn, "Maximum Premint per transaction exceeded.");
