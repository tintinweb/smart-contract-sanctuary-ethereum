...
interface INameWrapper is IERC1155 {
    ...
    function registerAndWrapROOT2LD(
        string calldata label,
        address wrappedOwner,
        uint256 duration,
        address resolver,
    ...
    ERC20Recoverable
{
    ...
    function wrapROOT2LD(
        ...
        return _wrapROOT2LD(label, wrappedOwner, fuses, expiry, resolver);
    }
    ...
    function registerAndWrapROOT2LD(
        ...
        _wrapROOT2LD(label, wrappedOwner, fuses, expiry, resolver);
    }
    ...
    function wrap(
        ...
        if (owner != msg.sender && !bdns.isApprovedForAll(owner, msg.sender)) {
            revert Unauthorised(node, msg.sender);
        }
        ...
        bdns.setOwner(node, address(this));
    ...
    /**
     * @notice Upgrades a .eth wrapped domain by calling the wrapROOT2LD function of the upgradeContract
        ...
        returns (bytes32 node)
    {
        ...
        if (!isWrapped(node)) {
            bdns.setSubnodeOwner(parentNode, labelhash, address(this));
    ...
    modifier canCallSetSubnodeOwner(bytes32 node, bytes32 labelhash) {
        ...
        address owner = bdns.owner(subnode);
    ...
    function onERC721Received(
        ...
        _wrapROOT2LD(label, owner, fuses, expiry, resolver);
    ...
    function _getROOT2LDDataAndNormaliseExpiry(
        bytes32 node,
        bytes32 labelhash,
        uint64 expiry
    )
    ...
    function _wrapROOT2LD(
        ...
        (, oldFuses, expiry) = _getROOT2LDDataAndNormaliseExpiry(
            node,
            labelhash,
            expiry
        );
