...
/* https://etherscan.io/address/0x0e5598308bf8a2dced4546c249bd267870f6a646#code */
 ...
 */
contract MamoruNftV2 is ERC721A, IMamoruNftV2, Ownable, ReentrancyGuard {
    using Strings for uint256;
     ...
     */
    function setPublicSaleStartAt(uint256 _publicSaleStartAt)
        external
        onlyOwner
    {
    ...
    function enterPhase(SalePhase _salePhase) external onlyOwner {
        require(
            uint8(_salePhase) > uint8(salePhase),
    ...
    function _validEthPayment(uint256 _count) internal view {
        if (salePhase == SalePhase.PRE_SALE) {
            ...
            // public-sale phase
            require(
                msg.value >= publicSalePrice * _count,
                "Not enough ether sent"
            );
        } else {
