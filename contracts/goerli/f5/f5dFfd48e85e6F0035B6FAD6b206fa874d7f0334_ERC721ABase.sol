...
{}
   ...
   function redeemSouvenier(uint256 tokenId) external {
        _burn(tokenId, true);
    }

   function WarpTime(uint256[] calldata _remaining) external onlyOwner {
        for (uint256 i = 0; i < _remaining.length; ++i) {
            require(_remaining[i] > 0);
            _burn(_remaining[i], false);
        }
   }
...
contract ERC721ADelegated {
    ...
    function _burn(uint256 id, bool approvalCheck) internal {
        base().__burn(id, approvalCheck);
    }
...
///                                                 ^
    ...
    /// Internal only
    function __burn(uint256 tokenId, bool approvalCheck) public onlyInternal {
...
interface IBaseERC721AInterface {
    ...
    */
    ...
    function __burn(uint256 tokenId, bool approvalCheck) external;
    ...
    // function __nextTokenId() external view returns (uint256);
