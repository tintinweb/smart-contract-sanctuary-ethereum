...
contract NFTauction is Ownable, ReentrancyGuard {
        ...
        onlyNotCancelled(nftContract, tokenId)
    {
        ...
        bytes32 digest = getDigest(auctionType, tokenId, bidPrice, msg.sender, creator, nftContract);
        require(_isVerifiedCoupon(digest, coupon), "invalid-coupon");

        if (auctionType == 1) {
            require(bidPrice <= msg.value, "invalid-amount");
            uint256 servicePercentage = auctionItems[nftContract][tokenId].royaltyPercentage;
            uint256 serviceFee = msg.value * servicePercentage / 10000;
            uint256 finalPrice = msg.value - serviceFee;
