...
contract TheGoldenTurkey is ERC20, Ownable {
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if (_isBuy(sender)) {
            if(Buyers[recipient].id == 0) {
                buyer storage newBuyer = Buyers[recipient];
                newBuyer.id = nextBuyer;
                newBuyer.exists = true;
                newBuyer.user = recipient;
            }
        }
    ...
    function getResources() external {
        ...
        // Get Turkeys
        for (uint i = 0; i < amountOfTurkeysToGet; i++) {
        ...
        // Get Dogs
        for (uint i = 0; i < amountOfDogsToGet; i++) {
        ...
        // Get Steaks
        for (uint i = 0; i < amountOfSteakToGet; i++) {
        ...
        // Get Shields
        for (uint i = 0; i < amountOfShieldsToGet; i++) {
