...
contract PoLidoAdapter is Helpers {
    function deposit(uint256 _amount) external payable {
        uint256 stTokenAmount = _stake(msg.sender, _amount);
    ...
    function depositFor(address _beneficiary, uint256 _amount) external {
        ...
        uint256 stTokenAmount = _stake(_beneficiary, _amount);
        TokenInterface(address(stMaticProxy)).transfer(
            _beneficiary,
            stTokenAmount
        );
    ...
    function _stake(address _beneficiary, uint256 _amount)
        ...
        returns (uint256)
    {
        maticToken.transferFrom(_beneficiary, address(this), _amount);
