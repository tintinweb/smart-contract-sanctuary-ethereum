...
abstract contract PositionAlgorithm is PositionLockerBase {
    ...
    modifier onlyPositionsControllerFactory() {
        require(
            positionsController.isFactory(msg.sender),
            'only for positions controller factories'
        );
        _;
    }
...
interface ITradingPairAlgorithm {
    ...
    function addLiquidity(uint256 position, uint256 assetCode, uint256 count) external;
