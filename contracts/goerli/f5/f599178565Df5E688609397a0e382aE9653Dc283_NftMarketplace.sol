...
contract NftMarketplace is ReentrancyGuard {
    ...
    function buyItem(address nftAddress, uint256 tokenId)
        ...
        nonReentrant
    {
        uint _receiverPercentage = getReceiverPercentage(tokenId,nftAddress);

        Listing memory listedItem = s_listings[nftAddress][tokenId];
        if (msg.value < listedItem.price) {
            revert PriceNotMet(nftAddress, tokenId, listedItem.price);
        }
        s_proceeds[listedItem.seller] += _receiverPercentage;

        delete (s_listings[nftAddress][tokenId]);

        // pay seller and feeAccount
        
        feeAccount.transfer(listedItem.price - _receiverPercentage);
    ...
    function getReceiverPercentage(uint tokenId,address nftAddress) view public returns(uint){
        return((s_listings[nftAddress][tokenId].price*(100 - feePercent))/100);
    }
