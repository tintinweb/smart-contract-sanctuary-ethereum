...
contract Marketplace is ReentrancyGuard {
    ...
    // declare a event for when a item price is updated on marketplace
    event MarketplaceItemPriceUpdated(
        uint256 indexed itemId,
        address seller,
        uint256 newPrice
    );

    event MarketplaceItemDelisted(
        address nftContract,
        uint256 indexed itemId,
        address seller
    );
    ...
    function createMarketplaceItem(
        ...
        emit MarketplaceItemCreated(
            itemId, // itemId to save at the server should be this from the event? This is from the market contract
            nftContract,
            tokenId, // This is from the NFTs contract
            msg.sender,
            address(0),
            feeVaultAddress,
            price,
    ...
    // Remove payable here?
    function updateMarketplaceItemPrice(
        uint256 itemId,
        uint256 newPrice
    ) public payable nonReentrant {
        
        bool sold = idToMarketplaceItem[itemId].sold;
        require(
            sold == false,
            "The price can only be updated when it is not sold already"
        );

        address seller = idToMarketplaceItem[itemId].seller;
        require(
            msg.sender == seller,
            "Only seller can update the NFT item price"
        );

        // When sell happens, the owner is address(0), should check that also?
        // require(_newVaultAddress != address(0), "Bad Address");
        
        idToMarketplaceItem[itemId].price = newPrice;

        emit MarketplaceItemPriceUpdated(
            itemId,
            msg.sender,
            newPrice
        );
    ...
    // Include payable here?
    // It is almost same to createMarketplaceSale currently
    function delistMarketplaceItem(
        address nftContract,
        uint256 itemId
    ) public nonReentrant {
        bool sold = idToMarketplaceItem[itemId].sold;
        require(
            sold == false,
            "You can only delist a unsold item"
        );

        address seller = idToMarketplaceItem[itemId].seller;
        require(
            msg.sender == seller,
            "Only the seller can delist an item"
        );

        // uint256 price = idToMarketplaceItem[itemId].price;
        uint256 tokenId = idToMarketplaceItem[itemId].tokenId;

        // There should be no transfer of ETH
        // idToMarketplaceItem[itemId].seller.transfer(price);
        IERC721(nftContract).transferFrom(address(this), msg.sender, tokenId);
        idToMarketplaceItem[itemId].owner = payable(msg.sender); // 0 address to seller address
        idToMarketplaceItem[itemId].sold = true; // This doens't allow resell?

        _soldItems.increment(); // No need for this because the sale didn't happen?

        emit MarketplaceItemDelisted(
            nftContract,
            itemId,
            msg.sender
        );
    ...
    function createMarketplaceSale(address nftContract, uint256 itemId) // marketItemId
        public
        payable
        nonReentrant
    {
        ...
        // require(
        //     msg.sender == seller,
        //     "Please use delistMarketplaceItem instead"
        // );
        ...
        idToMarketplaceItem[itemId].seller.transfer(price);
        IERC721(nftContract).transferFrom(address(this), msg.sender, tokenId); // msg.sender === seller or msg.sender === buyer currently?
    ...
    function fetchItemById(uint256 itemId) public view returns (MarketplaceItem memory) {
    ...
    function fetchItemsUserPurchased() public view returns (MarketplaceItem[] memory) {
    ...
    // Test again with itemId (from the market) and tokenId (from a nft contract)
    // function fetchItemsUserCreated()
    function fetchItemsUserCreated()
        public
        view
        returns (MarketplaceItem[] memory)
    ...
    // Can be useuful if we have items user bought only also?
}
