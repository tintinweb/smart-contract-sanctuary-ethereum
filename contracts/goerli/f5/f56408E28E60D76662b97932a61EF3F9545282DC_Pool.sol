    ...
    Governor
{
    ...
    [2:] - ballot type bound delays to allow pool shareholders prevent bugged/hacked ballot execution.
    */
    uint256[10] public ballotExecDelay;

    /**
    @dev unused threshold amounts for executeBallot delay
    [0] - TransferETH,
    [1] - TransferERC20,
...
abstract contract Governor {
        ...
        returns (bool)
    {
        ...
        if (
            ...
            uint256 valueUSDT = pool.service().uniswapQuoter().quoteExactInput(
                abi.encodePacked(
                    pool.service().weth(),
                    uint24(3000),
                    pool.service().usdt()
