...
contract SANSHU is Context, IERC20, Ownable {
    ...
    uint256 private _extraFeeUpdateTime;
    uint256 private _maxSellFee;
    uint256 private _maxBuyFee;
    uint256 private _feeSellImpact = 10;
    uint256 private _feeTimeImpact = 14;
    ...
    constructor(
        ...
        _isExcludedFromFee[ecoSystem] = true;
    ...
    function deliver(uint256 tAmount) public {
        ...
        _rTotal = _rTotal.sub(_values.rAmount);
    ...
    function setFeeSellImpact(uint256 feeSellImpact) external onlyOwner {
        _feeSellImpact = feeSellImpact;
    }
    ...
    function getCurrentFees() public view returns (uint256, uint256) {
        uint256 timeElapsed = block.timestamp - _extraFeeUpdateTime;
        uint256 timeImpact = (_feeTimeImpact * timeElapsed) / 60;

        //Enough time has passed that fees are back to base
        if (timeImpact >= _extraFee) {
            return (_baseFee, _baseFee);
        }

        uint256 realExtraFee = _extraFee - timeImpact;

        uint256 buyFee = 0;
        if (_baseFee >= realExtraFee) buyFee = _baseFee - realExtraFee;
        uint256 sellFee = _baseFee + realExtraFee;

        if (sellFee >= _maxSellFee) {
            sellFee = _maxSellFee;
        }
    ...
    {
        CFees memory fees_ = _calculateFee(tAmount);

        uint256 tTransferAmount = tAmount
            .sub(fees_.cFee)
            .sub(fees_.cLiquidity)
            .sub(fees_.cBurn)
            .sub(fees_.cEcoSystem);
        return (
            tTransferAmount,
            fees_.cFee,
            fees_.cLiquidity,
    ...
    function _takeFees(address sender, FeeValues memory values) private {
        ...
        _takeFee(sender, values.tBurn, _burnAddress);
        _takeFee(sender, values.tEcoSystem, ecoSystem);
    }
    ...
    function _calculateFee(uint256 _amount)
        private
        view
        returns (CFees memory)
    {
        uint256 fee = _amount.mul(_fee).div(10**2);

        return CFees(fee.div(4), fee.div(4), fee.div(4), fee.div(4));
    }
    ...
    function _setCurrentFees(address recipient, uint256 _amount) private {
        if (_isSelling(recipient)) {
            _fee = _handleSellFee(_amount);
        } else {
            (uint256 buyFee, ) = getCurrentFees();
            _fee = buyFee;
        }
    }
    ...
    function _tokenTransfer(
        ...
        else _setCurrentFees(recipient, amount);
    ...
    function _isBuying(address sender) private view returns (bool) {
        return sender == uniswapV2Pair;
    }

    function _handleSellFee(uint256 tAmount) private returns (uint256) {
        (, uint256 sellFee) = getCurrentFees();

        uint256 priceImpact = UniswapV2PriceImpactCalculator
            .calculateSellPriceImpact(address(this), uniswapV2Pair, tAmount);

        if (priceImpact == 0) return sellFee;

        uint256 increaseSellFee = (priceImpact * _feeSellImpact) / 100;
        ...
        _extraFee = sellFee - _extraFee;
        _extraFeeUpdateTime = block.timestamp;
