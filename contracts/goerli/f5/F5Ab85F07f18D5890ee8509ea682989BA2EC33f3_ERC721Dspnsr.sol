...
/**
 * @title ERC1155Ribeye
 * ERC721Dspnsr - ERC-721 smart contract for Satoshi's Closet DSPNSR
 */
contract ERC721Dspnsr is ERC721, Ownable {
    // Constants
     ...
     * @param uri Sets initial URI for metadata. Same for all tokens. Relies on id substitution by the client - https://token-cdn-domain/{id}.json
     * @param _shares The number of shares each shareholder has
     * @param _shareholder_addresses Payment address for each shareholder
     * @param _maxTotalSupply Total number of tokens that can be minted
    ...
    constructor(
        ...
        require(
            itemPrices_.length <= _maxTotalSupply,
            "itemPrices length must be lower or equal to _maxTotalSupply"
        );
    ...
    modifier validAmount(uint8 amount) {
        require(
            amount >= 1 && amount <= 3,
            "Mint amount must be between 1 and 3"
        );
        _;
    }
     ...
     * @param uri string - new value for metadata URI
     */
    ...
    function mint(address to, uint256 tokenId) public payable {
        require(
            tokenId <= _itemPrices.length,
            "Token not available for minting"
        );
        require(msg.value == _itemPrices[tokenId - 1], "Wrong minting fee"); // Starting from token ID 1
