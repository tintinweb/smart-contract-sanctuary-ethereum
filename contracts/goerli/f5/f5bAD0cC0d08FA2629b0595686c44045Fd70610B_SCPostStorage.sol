...
contract SCPostStorage is Ownable, ERC2771Recipient, Versioned {
   ...
   */
  function getPosts(uint256 _skip, uint256 _limit) external view returns (Post[] memory) {
    if (_skip > posts.length) {
      return new Post[](0);
    }
    uint256 length = _skip + _limit > posts.length - 1 ? posts.length - _skip : _limit;
    uint256 index = posts.length - _skip - 1;
    Post[] memory allPosts = new Post[](length);
    for (uint256 i = index; i > index - length; i--) {
      console.log("i", i);
      Post storage post = posts[i];
      allPosts[index - i] = post;
    }
    return allPosts;
  }
  ...
  function savePost(string memory post, string memory original) external {
    ...
    require(IERC721(derivativeAddress).balanceOf(_msgSender()) > 0, "You do not own this derivative");
    require(
      maxPostLength >
        post.toSlice().len() +
          infixLength +
    ...
    Post memory newPost = Post(id, post, derivativeAddress, _msgSender(), block.timestamp);
    posts.push(newPost);
    // Emit the psot event
