...
contract Morph is iMorph, protected {
    ...
    function swap(address token_1,
        ...
        // Getting eth if the paid is eth
        if (token_1 == WETH) {
            if(!(msg.value == amount)) revert("MORPH: Wrong amount of ETH sent");
        } 
        // Or getting the corresponding token if the paid is not eth
        else {
            if(!(IERC20(token_1).transferFrom(msg.sender, address(this), amount))) 
                revert("MORPH: Transfer failed");
        }
        // Paying out eth if the pair is in eth
        if (address(erc20_out)==WETH) {
            (bool success_, ) = msg.sender.call{value: amountOut}("");
            if(!(success_)) revert("MORPH: Transfer failed");
        } 
        // Or paying out the corresponding token if the pair is not in eth
        else {
            if(!(erc20_out.transfer(msg.sender, amountOut))) revert("MORPH: Transfer failed");
        }
        // Emitting the event
    ...
    /// @dev Method to extract publicy the liquidity owned by an account
    /// @param liquidity The liquidity hash
    /// @param provider The provider of the liquidity
    /// @return liquidity_owned_token_1 The amount of liquidity owned for token 1
    /// @return liquidity_owned_token_2 The amount of liquidity owned for token 2
    /// @return liquidity_owned_100000 The percentage of liquidity owned (in 100000)
    function ownedLiquidity(bytes32 liquidity, address provider) 
                            public view 
                            returns (uint liquidity_owned_token_1, 
                                     uint liquidity_owned_token_2,
                                     uint liquidity_owned_100000) {
        uint actual_owned_token_1;
        uint actual_owned_token_2;
        uint actual_percentage_owned;
        (actual_owned_token_1, actual_owned_token_2) = _percentageToOwnedTokens(liquidity, provider);
        actual_percentage_owned = _ownedTokensToPercentage(
                                                        liquidity, 
                                                        actual_owned_token_1, 
                                                        actual_owned_token_2);
        return(actual_owned_token_1, actual_owned_token_2, actual_percentage_owned);
    }
    
    /// @dev Method to extract publicy the price of a pair
    /// @param token_1 The first token of the liquidity
    /// @param token_2 The second token of the liquidity
    /// @return _price The price of the pair
    function price(address token_1, address token_2) public view returns (uint _price) {
        (bool _is, bytes32 _liq) = findLiquidityFromTokens(token_1, token_2);
        if(!(_is)) revert("MORPH: Liquidity does not exist");
        uint _total_1 = liquidities[_liq].amount_1;
        uint _total_2 = liquidities[_liq].amount_2;
        uint __price = _total_2 / _total_1;
        return __price;
    }
