...
contract MinterV1 is Ownable, ReentrancyGuard {
    ...
    /**
     * collectionId => ownedNFTCollectionIds => bool
     * collections whose holders are allowed to purchase a token on `collectionId`
     */
    mapping(uint256 => mapping(uint256 => bool)) public allowedCollectionHolders;

    /**
     * collectionId => address => bool
     * token addresses whose holders are allowed to purchase a token on `collectionId`
     */
    mapping(uint256 => mapping(address => bool)) public allowedExternalHolders;
    ...
    constructor(address _collCore721Address, address _signer)
        ReentrancyGuard() onlyNonZeroAddress(_collCore721Address)
    {
    ...
    function setAllowedHoldersOfCollections(
        ...
        for (uint256 i = 0; i < ownedIdsLength;) {
            ...
            // approve
            allowedCollectionHolders[_collectionId][
                toAllowCollectionId
            ] = _isAllowed;
     ...
     */
    function setAllowedExternalHolders(
        uint256 _collectionId,
        ...
        for (uint256 i = 0; i < tokenAddressesLength;) {
            ...
            // approve
            allowedExternalHolders[_collectionId][tokenAddress] = _isAllowed;
        ...
        if (_isAllowed) {
            // emit approve event
            emit AllowedExternalHolders(
                _collectionId,
                _tokenAddresses
            );
        } else {
            // emit disapprove event
            emit RemovedExternalHolders(
                _collectionId,
                _tokenAddresses
            );
        }
     ...
     * @dev assumes collection ID can be derived from tokenId / MAX_COLLECTION_SIZE
     */
    function isAllowlistedNFT(
        ...
        if (_ownedNFTAddress == address(collCoreContract)) {
            uint256 ownedNFTCollectionId = _ownedNFTTokenId / collCoreContract.MAX_COLLECTION_SIZE();
            return allowedCollectionHolders[_collectionId][ownedNFTCollectionId];
        } else {
            return allowedExternalHolders[_collectionId][_ownedNFTAddress];
        }
    }
