...
error NftMarketplace__PriceMustByAboveZero();
...
error NftMarketplace__TransferFailed(address seller);
...
contract NftMarketplace {
    ...
    mapping(address => mapping(uint256 => Listing)) public listings; // NFT contract address => NFT TokenID => Listing
    mapping(address => uint256) public proceeds; // seller => amount earned
    ...
    function listItem(
        ...
        if (price <= 0) {
            revert NftMarketplace__PriceMustByAboveZero();
        }
    ...
    function withdrawProceeds() external {
        ...
        (bool success, ) = payable(msg.sender).call{value: proceedsAmount}("");

        if (!success) {
            revert NftMarketplace__TransferFailed(msg.sender);
        }
    }
