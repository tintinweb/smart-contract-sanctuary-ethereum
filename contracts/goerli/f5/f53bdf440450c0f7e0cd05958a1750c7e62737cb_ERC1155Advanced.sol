...
contract ERC1155Advanced is ERC1155, Owned, ReentrancyGuard {
    ...
    mapping (uint => bool) public tokenMintEthActive;
    ...
    mapping (address => uint) public receiverAddressToTokenId;
    mapping (address => bool) public receiverAddressToValidityBool;
    ...
    modifier tokenNumericLimits(uint id, uint amount, address mintReceiver) {
        ...
        if (tokenMaxPerWallet[id] > 0) {
            require(tokenMintsByReceiverByToken[mintReceiver][id] + amount <= tokenMaxPerWallet[id], "You have reached your max mints for this token.");
        }
        _;
    }
    ...
    function mintViaReceiverChild(address mintTokenReceiver) external payable {
        require(receiverAddressToValidityBool[msg.sender], "You are not a valid receiver address.");
        uint tokenId = receiverAddressToTokenId[msg.sender];
        uint tokenAmount = msg.value / tokenCostETH[tokenId];
        if (tokenCostETH[tokenId] == 0){
            tokenAmount = 1;
        }
        require(msg.value == tokenAmount * tokenCostETH[tokenId], "You have sent an invalid amount of ETH.");
        _mint(mintTokenReceiver, tokenId, tokenAmount, "");
        _postMintNumericAdjustments(mintTokenReceiver, tokenId, tokenAmount);
    }

    function mintCurrencyERC20(address paymentCurrency, uint mintTokenId, uint mintTokenAmount, address mintTokenReceiver) external tokenNumericLimits(mintTokenId, mintTokenAmount, mintTokenReceiver) {
    ...
    function mintCurrencyERC721(address paymentCurrency, uint[] memory paymentTokenIds, uint mintTokenId, uint mintTokenAmount, address mintTokenReceiver) external tokenNumericLimits(mintTokenId, mintTokenAmount, mintTokenReceiver) {
    ...
    function mintCurrencyERC1155(address paymentCurrency, uint[] memory paymentTokenIds, uint[] memory paymentTokenAmounts, uint mintTokenId, uint mintTokenAmount, address mintTokenReceiver) external tokenNumericLimits(mintTokenId, mintTokenAmount, mintTokenReceiver) {
    ...
    function deployETHReceiverChildContract(uint mintToken) external onlyOwner {
        ...
        receiverAddressToTokenId[address(ethReceiverChild)] = mintToken;
        receiverAddressToValidityBool[address(ethReceiverChild)] = true;
    }
    ...
    receive() external payable {
        require(receiverAddressToValidityBool[msg.sender], "We are not expecting payment from you.");
...
contract ETHReceiverChild {
    ...
    receive() external payable {
        IERC1155Advanced(parent).mintViaReceiverChild{value: msg.value}(msg.sender);
    }
}