...
/// @author SphericonIO
contract SixthReseauLostIdentities is ERC721A, Ownable, Pausable, ReentrancyGuard, PaymentSplitter {
    ...
    uint[] private _shares = [1500, 1000, 400, 300, 250, 250, 100, 120, 30];
    ...
    constructor(address _mysterBox) ERC721A("SixthReseau: Lost Identities","SRS1") PaymentSplitter(_shareholders, _shares) {
        mysteryBox = _mysterBox;
        dutch.start = block.timestamp;
        dutch.duration = 5 minutes;
        dutch.startPrice = 0.4 ether;
        dutch.endPrice = 0.18 ether;
    }
    ...
    function getTotalSupply() public view returns (uint256 _supply) {
        _supply = _priorityCounter.current() + _publicCounter.current() + _whitelistCounter.current() + reservedTokens;
        return _supply;
    }
    ...
    function setMaxPriority(uint _maxPriority) external onlyOwner {
        maxPriority = _maxPriority;
    }

    function setMaxPublic(uint _maxPublic) external onlyOwner {
        maxPublic = _maxPublic;
    }

    function setMaxPrivate(uint _maxPrivate) external onlyOwner {
        maxPrivate = _maxPrivate;
    }
    ...
    function setMaxPriorityMint(uint _maxPriorityMint) external onlyOwner {
        maxPriorityMint = _maxPriorityMint;
    }
    ...
    function setMaxWhitelistMint(uint _maxWhitelistMint) external onlyOwner {
