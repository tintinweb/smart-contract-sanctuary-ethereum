...
import { IRarementCreatorV1 } from "./interfaces/IRarementCreatorV1.sol";
import { IRarementVolumeV1 } from "./interfaces/IRarementVolumeV1.sol";
import { IMetadataModule } from "./interfaces/IMetadataModule.sol";

import { OwnableRoles } from "solady/auth/OwnableRoles.sol";

/**
 * @title RarementCreatorV1
 * @notice A factory that deploys minimal proxies of `RarementVolumeV1.sol`.
 * @dev The proxies are OpenZeppelin's Clones implementation of https://eips.ethereum.org/EIPS/eip-1167
 */
contract RarementCreatorV1 is IRarementCreatorV1, OwnableRoles {
    // =============================================================
    //                            STORAGE
    // =============================================================

    /**
     * @dev The implementation contract delegated to by Rarement volume proxies.
     */
    address public rarementVolumeImplementation;

    // =============================================================
    //                          CONSTRUCTOR
    // =============================================================

    constructor(address _rarementVolumeImplementation) implementationNotZero(_rarementVolumeImplementation) {
        rarementVolumeImplementation = _rarementVolumeImplementation;
    ...
    /**
     * @inheritdoc IRarementCreatorV1
     */
    function createRarementAndMints(
        bytes32 salt,
        bytes calldata initData,
        address[] calldata contracts,
        bytes[] calldata data
    ) external returns (address RarementVolume, bytes[] memory results) {
        // Create Rarement Volume proxy.
        RarementVolume = payable(Clones.cloneDeterministic(rarementVolumeImplementation, _saltedSalt(msg.sender, salt)));
        ...
        assembly {
            ...
            if iszero(
                call(
                    gas(), // Gas remaining.
                    RarementVolume, // Address of thevolume.
        ...
        OwnableRoles(RarementVolume).transferOwnership(msg.sender);

        emit RarementVolumeCreated(RarementVolume, msg.sender, initData, contracts, data, results);
    }

    /**
     * @inheritdoc IRarementCreatorV1
     */
    function setVolumeImplementation(address newImplementation)
        external
        onlyOwner
        implementationNotZero(newImplementation)
    {
        rarementVolumeImplementation = newImplementation;

        emit RarementVolumeImplementationSet(rarementVolumeImplementation);
    }
     ...
     * @inheritdoc IRarementCreatorV1
     */
    function RarementVolumeAddress(address by, bytes32 salt) external view returns (address addr, bool exists) {
        addr = Clones.predictDeterministicAddress(rarementVolumeImplementation, _saltedSalt(by, salt), address(this));
     ...
     * @param by   The caller of the {createRarementAndMints} function.
...
/**
 * @title IRarementCreatorV1
 * @notice The interface for the Rarement volume factory.
 */
interface IRarementCreatorV1 {
    // =============================================================
    //                            EVENTS
    // =============================================================

    /**
     * @dev Emitted when an volume is created.
     * @param RarementVolume The address of thevolume.
     * @param deployer     The address of the deployer.
     * @param initData     The calldata to initialize RarementVolume via `abi.encodeWithSelector`.
     ...
     */
    event RarementVolumeCreated(
        address indexed RarementVolume,
    ...
    /**
     * @dev Emitted when the volume implementation address is set.
     * @param newImplementation The new implementation address to be set.
     */
    event RarementVolumeImplementationSet(address newImplementation);
    ...
    /**
     * @dev Creates a Rarement Volume proxy, initializes it,
     ...
     * @param initData  The calldata to initialize RarementVolume via
     ...
     * @return RarementVolume Returns the address of the created contract.
     * @return results      The results of calling the contracts.
    ...
    /**
     * @dev Changes the RarementVolume implementation contract address.
     *
     * Calling conditions:
     ...
     * @param newImplementation The new implementation address to be set.
     */
    function setVolumeImplementation(address newImplementation) external;
    ...
    /**
     * @dev The address of the rarement volume implementation.
     * @return The configured value.
     */
    function rarementVolumeImplementation() external returns (address);

    /**
     * @dev Returns the deterministic address for the rarement volume clone.
     * @param by   The caller of the {createRarementAndMints} function.
     * @param salt The salt, generated on the client side.
