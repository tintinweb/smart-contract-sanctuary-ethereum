...
contract ScrappyStaking is IERC721Receiver, ReentrancyGuard {
  ...
  mapping (address => stakedNFT) public _stakedNFT;
  ...
  function stakeNFT(uint256[] calldata _id) external nonReentrant{ 
    ...
    _stakedNFT[msg.sender].tokenId.push(_id[i]);
    _stakedNFT[msg.sender].totalStaked += 1;
    }
  }
  function unStakeNFT(uint256 _id) external payable nonReentrant{
    ...
    _stakedNFT[msg.sender].totalStaked -= 1;
    for (uint256 i = 0; i < _stakedNFT[msg.sender].tokenId.length; i++){
      if(_stakedNFT[msg.sender].tokenId[i] == _id){
        _stakedNFT[msg.sender].tokenId[i] = _stakedNFT[msg.sender].tokenId[_stakedNFT[msg.sender].tokenId.length-1];
        _stakedNFT[msg.sender].tokenId.pop();
        break;
      }
    }
    delete stake[_id];
