...
contract Factory {
    ...
    // TODO change
    function deployWallet(address owner) public returns (address wallet) {
        if (_wallets[owner] != address(0x0)) {
            return _wallets[owner];
        }

        wallet = address(new Wallet(_core));
        (bool success, bytes memory result) = wallet.call(
            abi.encodeWithSignature("setupWallet(address,address)", msg.sender, owner)
        );
        result; // TODO remove
...
contract Wallet {
    ...
    fallback() external payable {
        bytes32 slot = CORE_SLOT;
        assembly {
            let core := and(sload(slot), 0xffffffffffffffffffffffffffffffffffffffff)
            // 0xda4fbe52 == keccak("getCore()"). The value is right padded to 32-bytes with 0s
            if eq(calldataload(0), 0xda4fbe5200000000000000000000000000000000000000000000000000000000) {
                mstore(0, core)
                return(0, 0x20)
            }
