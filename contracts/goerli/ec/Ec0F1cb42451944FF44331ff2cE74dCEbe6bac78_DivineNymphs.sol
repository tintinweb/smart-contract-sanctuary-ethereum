...
contract DivineNymphs is ERC721A, Ownable, ReentrancyGuard {
    ...
    modifier hasNotClaimedFreeMintNymphs() {
        uint16 senderFreeMintsNymphs;
        uint16 senderFreeMintsFriends;
        uint16 senderGifts;
        (senderFreeMintsNymphs, senderFreeMintsFriends, senderGifts) = unpack(_getAux(msg.sender));
        require(
    ...
    {
        uint16 ownerFreeMintsNymphs;
        uint16 ownerFreeMintsFriends;
        uint16 ownerGifts;
        uint256 numToGift = addresses.length;
        numGifts += numToGift;
        for (uint256 i = 0; i < numToGift; i++) {
            (ownerFreeMintsNymphs, ownerFreeMintsFriends, ownerGifts) = unpack(_getAux(addresses[i]));
            ownerGifts++;
    ...
    function getFreeMintCountNymphs(address owner) public view returns (uint32) {
        uint16 ownerFreeMintsNymphs;
        uint16 ownerFreeMintsFriends;
        uint16 ownerGifts;
        (ownerFreeMintsNymphs, ownerFreeMintsFriends, ownerGifts) = unpack(_getAux(owner));
        return ownerFreeMintsNymphs;
    }

    function getFreeMintCountFriends(address owner) public view returns (uint32) {
        uint16 ownerFreeMintsNymphs;
        uint16 ownerFreeMintsFriends;
        uint16 ownerGifts;
        (ownerFreeMintsNymphs, ownerFreeMintsFriends, ownerGifts) = unpack(_getAux(owner));
    ...
    function getOwnerGifts(address owner) public view returns (uint32) {
        uint16 ownerFreeMintsNymphs;
        uint16 ownerFreeMintsFriends;
        uint16 ownerGifts;
        (ownerFreeMintsNymphs, ownerFreeMintsFriends, ownerGifts) = unpack(_getAux(owner));
