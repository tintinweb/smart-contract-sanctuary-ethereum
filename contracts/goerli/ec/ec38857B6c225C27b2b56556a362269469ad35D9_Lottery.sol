...
contract Lottery is Ownable, VRFConsumerBaseV2, AutomationCompatibleInterface, ERC721Holder {
    ...
    function enterRaffle(uint256 entries) public payable {
        ...
        for (uint256 i = 0; i < entries; i++) {
            ...
            points[msg.sender] += s_bonus * s_rate;
    ...
    function detailsNFT(uint256 _index) internal view returns (IERC721, uint256, uint256, uint256) {
        return (
            LoadedNFTs[_index].s_nftRaffled,
            LoadedNFTs[_index].s_tokenId,
            LoadedNFTs[_index].s_target,
            LoadedNFTs[_index].index
        );
    }
    ...
    function fulfillRandomWords(
        ...
        if (LoadedNFTs.length > 0) {
            ...
            if (
                ...
                balances[s_treasury] = s_raffleBalance - payout;
                // uint256 winners = s_players.length % indexOfWinner;
                // uint256 payout = s_raffleBalance / winners;
                // s_recentWinner = new address[](0);
                // s_recentPayout = payout;
                // for (uint256 i = 0; i < winners; i++) {
                //     s_recentWinner.push(s_players[indexOfWinner + i]); //Callback gas limit?
                //     balances[s_players[indexOfWinner + i]] += payout;
                // }
                s_raffleBalance = 0;
                s_players = new address[](0);
                s_lotteryState = LotteryState.OPEN;
    ...
    function withdrawETH() external onlyOwner {
        (bool success, ) = payable(msg.sender).call{value: s_collectedFunds}("");
        if (!success) {
            revert Lottery__TransferFailed();
        }
        s_collectedFunds = 0;
    ...
    function sendNFT(uint256 _index, address to) external onlyOwner {
        (IERC721 nftContract, uint256 tokenId, , uint256 index) = detailsNFT(_index);
    ...
    function deleteStruct(uint256 _index) internal {
        NFTLoad memory removeMe;
        for (uint i = 0; i < LoadedNFTs.length; i++) {
            if (LoadedNFTs[i].index == _index) {
    ...
    function refundALL() external onlyOwner {
        (bool success, ) = payable(msg.sender).call{value: s_raffleBalance}("");
        if (!success) {
            revert Lottery__TransferFailed();
        }
        s_raffleBalance = 0;
    ...
    function setNFTwithTarget(address nftContract, uint256 tokenId, uint256 target) external {
        ...
        nft.s_target = target;
        nft.index = s_index;
        if ((LoadedNFTs.length >= 1) && LoadedNFTs[s_index - 1].s_tokenId == tokenId) {
    ...
    function resetNFT(
        ...
        LoadedNFTs[_index].s_nftRaffled = IERC721(nftContract);
        LoadedNFTs[_index].s_tokenId = tokenId;
        LoadedNFTs[_index].s_target = target;
    }
    ...
    function getCurrentNFT() public view returns (address, uint256, uint256, uint256) {
        NFTLoad storage nft = LoadedNFTs[LoadedNFTs.length - 1];
        return (address(nft.s_nftRaffled), nft.s_tokenId, nft.s_target, nft.index);
    }

    function getSpecificNFT(
        uint256 _index
    ) public view returns (address, uint256, uint256, uint256, string memory) {
        return (
            address(LoadedNFTs[_index].s_nftRaffled),
            LoadedNFTs[_index].s_tokenId,
            LoadedNFTs[_index].s_target,
            LoadedNFTs[_index].index,
            IERC721Metadata(address(LoadedNFTs[_index].s_nftRaffled)).tokenURI(
                LoadedNFTs[_index].s_tokenId
            )
        );
    }
