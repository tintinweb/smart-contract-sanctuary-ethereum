    ...
    ReentrancyGuard
{
    ...
    /**
     * Updates contract uptime (idempotent)
     */
    ...
    /**
     * @notice Join a validator to the pool
     * @dev Join a validator to the pool
     * @param validatorPubKey: The validator's public key
     * @param depositor: The depositor that will be associated with the validator
     * @param joinTime: The timestamp as of which the validator started accruing payable uptime
     */
    function joinPool(
     ...
     * @dev Remove a validator from the pool
     * @param validatorPubKey: The validator's public key
     * @param leaveTime: The timestamp as of which the validator stopped accruing payable uptime
     */
    function partPool(
        bytes calldata validatorPubKey,
        uint256 leaveTime
    ...
    /**
     * @notice Join many validators to the pool
     * @dev Join many validators to the pool
     ...
     * @param ts: The timestamp as of which the validators started accruing payable uptime
     */
     ...
     * @dev Remove many validators from the pool
     * @param validatorPubkeyArray: The list of validator public keys to remove (must be a multiple of 48)
     * @param ts: The timestamp as of which the validators stopped accruing payable uptime
     */
    ...
    /**
     * @dev The depositor's uncollected pool rewards, and all rewards they have ever collected
     * @param depositor: The depositor address
     * @return (uint256 pendingRewards: The rewards eligible for collection by the depositor,
     *          uint256 collectedRewards: The rewards already collected by the depositor)
     */
    ...
    /**
     * @notice The depositor's uncollected pool rewards, and all rewards they have ever collected
     * @dev The depositor's uncollected pool rewards, and all rewards they have ever collected
   ...
   function pendingReward(address depositorAddress) external override view returns (uint256, uint256) {
    ...
    /**
     * @notice Collect rewards from the pool up to the `amountRequested`
     * @dev Collect rewards from the pool up to the `amountRequested`
     * @param beneficiary: The address that the `amountRequested` will be credited to
     * @param amountRequested: The amount that will be credited to the `beneficiary`. If 0, collect all pending rewards.
     */
    ...
    /**
     * @dev Set the contract commission rate
    ...
    /**
     * @dev Collect accumulated commission fees, up to `amountRequested`
     * @param beneficiary: The address that will be credited with the `amountRequested`
     * @param amountRequested: The amount that will be credited to the `beneficiary`. If 0, collect all fees.
     */
    ...
    /**
     * @notice Transfer one or more validators to new owners.
     * @dev Transfer one or more validators to new owners.
     * @param validatorPubkeys: The public keys of the validators that will be transferred
     * @param toAddresses: The depositor addresses that the validators will be transferred to
     * @param transferTimestamp: The time as of which the validators will be transferred
     */
     ...
     * @notice Transfer claim history from another contract into this one (used during upgrades)
     * @dev Transfer claim history from another contract into this one (used during upgrades)
     * @param addresses: Depositor addresses that collected
     * @param claimAmount: The total amount collected by each depositor
     */
     ...
     * @notice Disable collection of rewards by depositors
     * @dev Disable collection of rewards by depositors
     */
    ...
    /**
     * @notice Enable collection of rewards by depositors
     * @dev Enable collection of rewards by depositors
     */
     ...
     * @dev Change the contract operator
    ...
    /**
     * @notice Admin function to allow a user to recover rewards from a lost or stolen wallet
     * @dev Admin function to allow a user to recover rewards from a lost or stolen wallet
     * @param depositorAddresses: The depositor rewards to be recovered
     * @param beneficiaries: The addresses to be credited with the depositors rewards
     * @param maxAmount: The max amount to be credited
     */
     ...
     * @dev The count of all validators in the pool
     * @return The count of all validators in the pool
     */
     ...
     * @dev A summary of the pool's current state
     * @return (uint256 lastCachedUpdateTime: The timestamp when `totalValidatorUptime` was last updated,
     ...
     *          uint256 lifetimePaidUserRewards: The amount of user rewards ever paid from the pool)
     */
     ...
     * @dev A summary of the depositor's activity in the pool
     * @param user: A depositor address
     * @return (uint256 validatorCount: The count of all validators owned by the depositor,
