...
abstract contract HordeMintableA is EIP712Allowlisting, LockableA, FlexibleMetadata {  
    ...
    uint256 private MAX_PRIVATE_PER = 2;
    uint256 private MAX_PUBLIC_PER = 2;
    ...
    function setMaxPer(uint256 max, bool isPublic) external onlyOwner {        
        if (isPublic) {
            MAX_PUBLIC_PER = max;
        } else {
            MAX_PRIVATE_PER = max;
    ...
    function mint(
        ...
        // require(quant <= (publicMint ? MAX_PUBLIC_PER : MAX_PRIVATE_PER), INVALID_QUANT);     
        ...
        //     <= (publicMint ? MAX_PUBLIC_PER : MAX_PRIVATE_PER), ALLOC_OVER);
    ...
    function mintForRecipient(address recip, uint256 quant) external onlyOwner {  
        require((totalSupply()+quant) <= MAX_TOKENS, ALLOC_OVER);      
        _safeMint(recip,quant);    
    }      
...
abstract contract LockableA is ERC721A {
    ...
    function setCustodian(uint256 id, address custodianAddress) public {
        ...
        if (_cust[owner].isAssigned) {
            _lockedBalance = _cust[owner].lockedBalance;
        }
    ...
    function setApprovalForAll(address operator, bool approved) public virtual override {
        ...
        require(operator != _msgSenderERC721A(), "cannot grant approval to self");
