...
contract NFTHarvestooor is Ownable, Pausable, ReentrancyGuard, IERC721Receiver {
     ...
     */
    mapping(address => mapping(uint256 => address)) private _ownerByToken;

    /**
     * Maps an address (prior owner) to an array of NFTs currently held by the contract.
     */
    mapping(address => NFT[]) private _tokensByOwner;

    /**
     * Maps a NFT to its index in the 'tokensByOwner' array.
     */
    mapping(address => mapping(uint256 => uint256)) private _indexByToken;
    ...
    function getPriorOwnerByToken(address tokenAddress, uint256 tokenId)
        ...
        returns (address)
    {
        return _ownerByToken[tokenAddress][tokenId];
    }

    /**
     * Returns an array of NFTs currently stored in the contract
     * which have been previously sold by the given address.
     */
    function getTokensByPriorOwner(address account)
        external
        view
        returns (NFT[] memory)
    {
        return _tokensByOwner[account];
    }

    /**
     * Returns the index of a NFT in the 'tokensByOwner' array given its address and tokenId.
     */
    function getIndexByToken(address tokenAddress, uint256 tokenId)
        external
        view
        returns (uint256)
    {
        return _indexByToken[tokenAddress][tokenId];
    }
        ...
        whenNotPaused
    {
        ...
        require(
            tokens.length > 0,
            "NFTHarvestooor: 'tokens' must not be empty"
        );
        ...
        require(
            ...
            "NFTHarvestooor: Insufficient Balance"
        );
        ...
        for (uint256 i = 0; i < tokens.length; ++i) {
            ...
            require(
                ...
                "NFTHarvestooor: Token must implement IERC721 interface"
            );
            ...
            require(
                ...
                "NFTHarvestooor: NFT must be sold by its owner"
            );

            // Write owner to ownerByToken map
            _ownerByToken[tokenAddress][tokenId] = sender;

            // Add token to tokensByOwner map
            _tokensByOwner[sender].push(tokens[i]);

            // Write index of the token in array to indexByToken map
            uint256 tokenIndex = _tokensByOwner[sender].length - 1;
            _indexByToken[tokenAddress][tokenId] = tokenIndex;
        ...
        whenNotPaused
    {
        ...
        require(
            ...
            "NFTHarvestooor: Transaction must include correct ETH value"
        );
        ...
        for (uint256 i = 0; i < tokens.length; ++i) {
            ...
            require(
                _ownerByToken[tokenAddress][tokenId] == msg.sender,
                "NFTHarvestooor: NFT must have been previously sold to the contract"
            );

            // Remove previous token from storage (must be done before token transfer)
            removeToken(tokens[i], msg.sender);
    ...
    /**
     * Remove the given NFT from the contract's storage along with its related storage references.
     */
    function removeToken(NFT memory nft, address priorOwner) private {
        // Get reference to the the token which will be moved in the 'tokensByOwner' array (last element)
        NFT memory nftToMove = _tokensByOwner[priorOwner][
            _tokensByOwner[priorOwner].length - 1
        ];

        // Overwrite the index of the token being removed with the current index of 'nftToMove'
        uint256 idx = _indexByToken[nft.tokenAddress][nft.tokenId];
        _indexByToken[nftToMove.tokenAddress][nftToMove.tokenId] = idx;

        // Overwrite the array position of the token being deleted with 'nftToMove'
        _tokensByOwner[priorOwner][idx] = nftToMove;

        // Pop the last element of the array so 'nftToMove' is not duplicated
        _tokensByOwner[priorOwner].pop();

        // Delete 'indexByToken' and 'ownerByToken' references for the provided Token
        delete _indexByToken[nft.tokenAddress][nft.tokenId];
        delete _ownerByToken[nft.tokenAddress][nft.tokenId];
    }
    ...
    function onERC721Received(
        ...
        require(
            _ownerByToken[msg.sender][tokenId] == from,
            "NFTHarvestooor: NFT must have been previously sold to the contract"
        );
