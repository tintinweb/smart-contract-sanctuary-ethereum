...
error NftMarketplace__NotEnoughFundsProvided();
error NftMarketplace__NotOwner();
error NftMarketplace__NoEarnings();
error NftMarketplace__TransferFailed();

/**@title A minimal NFT marketplace contract
 * @author Hammad Tariq
 * @notice This contract is for listing nft in marketplace and selling them to other users.
 * @dev This implements the ERC-721 VRF Version 2
 */
contract NftMarketplace is ReentrancyGuard {
    ...
    mapping(address => uint256) private s_sellerEarnings;
    ...
    event ItemDelisted(address indexed nftAddress, uint256 indexed tokenId);
    ...
    modifier isListed(address nftAddress, uint256 tokenId) {
        if (s_nftList[nftAddress][tokenId].seller != address(0)) {
     ...
     * @dev We are only allowing the contract to list or sell nft from the approval of the seller.
     */
    function listItem(
        address nftAddress,
        uint256 tokenId,
     ...
     * @param nftAddress The address of the NFT contract.
     * @param tokenId The ID of the NFT.
     * @dev Dont send the money to seller directly have them withdraw it from the contract.
     */
        ...
        notListed(nftAddress, tokenId)
    {
        ...
        if (listing.price > msg.value) {
            revert NftMarketplace__NotEnoughFundsProvided();
        }
        // we don't just send money to seller directly due to pull over push solidity pattern.
        s_sellerEarnings[listing.seller] += msg.value;
        delete (s_nftList[nftAddress][tokenId]);
        IERC721 nft = IERC721(nftAddress);
        // all money transfering logic should be placed at the end to avoid Reentrant attack
        nft.safeTransferFrom(nft.ownerOf(tokenId), msg.sender, tokenId);
        emit ItemBought(msg.sender, nftAddress, tokenId, s_nftList[nftAddress][tokenId].price);
    }
    ...
    function withdrawSellerEarnings() external {
        uint256 sellerEarnings = s_sellerEarnings[msg.sender];
        if (sellerEarnings <= 0) {
            revert NftMarketplace__NoEarnings();
        }
        s_sellerEarnings[msg.sender] = 0;
        (bool success, ) = payable(msg.sender).call{value: sellerEarnings}("");
        if (!success) {
