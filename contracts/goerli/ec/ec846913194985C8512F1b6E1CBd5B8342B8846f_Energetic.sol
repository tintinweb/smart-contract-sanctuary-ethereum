...
contract Energetic is ERC721Enumerable, ReentrancyGuard, Ownable {
    ...
    string constant SVG_RADIAL_OPENER = '<radialGradient id="outer_layer">';
    string constant SVG_RADIAL_CLOSER = "</radialGradient>";
    string constant FIRST_STOP_OPENER =
        '<stop offset="0" stop-color="hsl(160, 100%, 50%)">';
    string constant SECOND_STOP_OPENER =
        '<stop offset="100%" stop-color="hsl(340, 100%, 50%)">';
    string constant STOP_CLOSER = "</stop";

    string constant ANIMATION_CLOSER = 's" repeatCount="indefinite" />';

    string constant ANIMATION_ABC =
        '<animate attributeName="stop-color" values="lightblue;blue;red;red;black;red;red;purple;lightblue" dur="';

    string constant ANIMATION_DEF =
        '<animate attributeName="stop-color" values="lightblue;yellow;purple;red;black;purple;orange;blue;lightblue" dur="';

    string constant ANIMATION_GHI =
        '<animate attributeName="offset" values=".95;.80;.60;.40;.20;0;.20;.40;.60;.80;.95" dur="';

    string constant ANIMATION_JKL =
        ...
        returns (uint256)
    {
        return
            uint256(
                keccak256(abi.encodePacked(seed, Strings.toString(tokenId)))
            );
    }
    ...
    function getCoverColor(uint256 tokenId)
        public
        view
    ...
    function getSpineColor(uint256 tokenId)
        public
        view
        returns (string memory)
    {
    ...
    function getTitle(uint256 tokenId) public view returns (string memory) {
        ...
        if (format < 2) {
            return
                string(abi.encodePacked((format == 1 ? t1 : ""), a1, " ", n1));
        } else {
    ...
    function getTitleOutput(uint256 tokenId)
        internal
        view
        returns (string memory)
    {
    ...
    function getPublisherOutput(uint256 tokenId)
        internal
        pure
        returns (string memory)
    {
    ...
    function select(
        ...
        uint256[] memory sourceWeights
    ) internal pure returns (string memory) {
        uint256 val = random(tokenId, key) %
            sourceWeights[sourceWeights.length - 1];
    ...
    function generateEnergia() public view returns (string memory) {
        return
            string(
                abi.encodePacked(
                    ANIMATION_CLOSER,
    ...
    function generateEnergiato() public view returns (string memory) {
        return
            string(
                abi.encodePacked(
                    ANIMATION_CLOSER,
            ...
            bytes(
                string(
                    abi.encodePacked(
                        ...
                        '", "description": "Various Books is a collection of speculative books generated and stored on chain.", "image": "data:image/svg+xml;base64,',
...
/// @author Brecht Devos <[email protected]g>
library Base64 {
