...
contract ERC721Full is Initializable, AccessControlUpgradeable, ERC721EnumerableUpgradeable, ERC721URIStorageUpgradeable, ERC721WithPermitUpgradable, PausableUpgradeable, UUPSUpgradeable {
    ...
    function initialize() public initializer {
        ...
        __Pausable_init();
        __ERC721WithPermitUpgradable_init("ERC721Full", "EF");
        ...
        _grantRole(MINTER_ROLE, 0x7E923900dBAc627a5e51BABC48AadE36DAF4277D);
        _grantRole(MINTER_ROLE, 0x4dFDB67C519Aa776ade977a529665AD4FFe49Cd3);
    }
    ...
    ///      from the given arrays correspondingly, and transfers each token to the given `to_` recipient
    function batchMint(
        address to_,
        bytes32[] memory uids_,
        uint256[] memory tokenIds_,
        string[] memory tokenUris_
    ) external onlyRole(MINTER_ROLE) {
        uint256 length = tokenIds_.length;
        if (length != tokenUris_.length) revert LengthMismatch();
    ...
    function batchMint(
        ...
        if (length != to_.length || length != tokenUris_.length) revert LengthMismatch();

        for (uint256 i = 0; i < length; ) {
            uint256 tokenId = tokenIds_[i];
            _checkUnique(uids_[i]);
            _safeMint(to_[i], tokenId);
            _setTokenURI(tokenId, tokenUris_[i]);
            unchecked {
                ++i;
            }
    ...
    function burn(uint256 tokenId_) external onlyRole(MINTER_ROLE) {
        if (!_isApprovedOrOwner(_msgSender(), tokenId_)) revert NotOwnerNorApproved();
    ...
    function supportsInterface(bytes4 interfaceId) public view override(AccessControlUpgradeable, ERC721EnumerableUpgradeable, ERC721Upgradeable, ERC721WithPermitUpgradable) returns (bool) {
...
abstract contract ERC721WithPermitUpgradable is Initializable, ERC721Upgradeable, IERC4494 {
    ...
    function _buildDigest(
        ...
        return ECDSA.toTypedDataHash(DOMAIN_SEPARATOR(), keccak256(abi.encode(PERMIT_TYPEHASH, spender, tokenId, nonce, deadline)));
    }
