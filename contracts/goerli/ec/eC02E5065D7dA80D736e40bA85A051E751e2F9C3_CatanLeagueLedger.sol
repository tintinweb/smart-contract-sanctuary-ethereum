...
contract CatanLeagueLedger is Ownable, LeagueLedgerStructs, GenericErrorsAndEvents {
    ...
    bool initialized;
    bool trophyInitialized;

    event MatchResult(uint256[] victoryPoints, address[] leagueParticipants);
    event LeagueMemberAdded(address newLeagueMember);
    ...
    function initializeLedger(
        ...
        leagueMembers[deployer_] = true;
        emit LeagueMemberAdded(deployer_);
        initialized = true;
    }

    function initializeTrophy(
        address trophyAddress
    ) external {
        if (trophyInitialized) {
            revert AlreadyInitialized();
        }
        _changeLeagueERC721Trophy(trophyAddress);
    }
    ...
    function _changeLeagueERC721Trophy(address newLeagueERC721Trophy) internal {
    ...
    function changeLeagueERC721Trophy(address newLeagueERC721Trophy) external onlyOwner {
        _changeLeagueERC721Trophy(newLeagueERC721Trophy);
    }
