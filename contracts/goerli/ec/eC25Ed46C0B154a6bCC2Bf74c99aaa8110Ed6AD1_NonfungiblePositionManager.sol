...
/// @title NFT positions
/// @notice Wraps Tangleswap positions in the ERC721 non-fungible token interface
    ...
    SelfPermit
{
    ...
    ) ERC721Permit('Tangleswap Positions NFT-V1', 'TANGLE-POS', '1') PeripheryImmutableState(_factory, _WETH9) {
        ...
        returns (uint256 amount0, uint256 amount1)
    {
        ...
        PoolAddress.PoolKey memory poolKey = _poolIdToPoolKey[position.poolId];
        ITangleswapPool pool = ITangleswapPool(PoolAddress.computeAddress(factory, poolKey));
...
import '../core/interfaces/ITangleswapFactory.sol';
import '../core/interfaces/callback/ITangleswapMintCallback.sol';
...
/// @notice Internal functions for safely managing liquidity in Tangleswap
abstract contract LiquidityManagement is ITangleswapMintCallback, PeripheryImmutableState, PeripheryPayments {
    struct MintCallbackData {
        PoolAddress.PoolKey poolKey;
        address payer;
    }

    /// @inheritdoc ITangleswapMintCallback
...
abstract contract PoolInitializer is IPoolInitializer, PeripheryImmutableState {
    ...
    function createAndInitializePoolIfNecessary(
        ...
        pool = ITangleswapFactory(factory).getPool(token0, token1, fee);

        if (pool == address(0)) {
            pool = ITangleswapFactory(factory).createPool(token0, token1, fee);
            ITangleswapPool(pool).initialize(sqrtPriceX96);
        } else {
            (uint160 sqrtPriceX96Existing, , , , , , ) = ITangleswapPool(pool).slot0();
