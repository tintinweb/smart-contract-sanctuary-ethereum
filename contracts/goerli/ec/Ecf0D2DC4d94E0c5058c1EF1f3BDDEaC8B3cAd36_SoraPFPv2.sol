    ...
    ERC2981Upgradeable
{
    ...
    function presaleMint(bytes32[] calldata _merkleProof, uint256 quantity) external payable {
        ...
        require(_totalMinted() + quantity <= phases[uint256(currentPhase)].supply, "Exceeds phase supply");
    ...
    function adminMint(address _to, uint256 quantity) external onlyOwner {
        require(quantity + _totalMinted() <= MAX_SUPPLY, "Exceeds max supply");
        _mint(_to, quantity);
    }
