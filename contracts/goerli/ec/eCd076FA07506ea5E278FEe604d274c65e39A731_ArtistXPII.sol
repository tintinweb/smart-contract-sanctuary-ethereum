...
contract ArtistXPII is ERC721, ERC721Enumerable, Ownable {
    ...
    uint256 public constant ARTISTXPPRICE = 50 ether;
    ...
    struct Track {
        ...
        address trackOwnerAddress;
    }

    uint [] mintedTracks;
    Track[] private allTracks;
    ...
    // Mint ArtistXP Track
    function mintArtistXP(uint256 tokenId, uint256 amountOfLostTokens) public {
        ...
        require(amountOfLostTokens == ARTISTXPPRICE, "You need more LOST to mint this track");


        // burn lost tokens
        burnToken(amountOfLostTokens);

        // mint tokenid to msg.sender
        _mintTokens(msg.sender, tokenId);

        // add 1 to mintedTracks
        mintedTracks.push(tokenId);
        // create new track
        Track memory newTrack = Track({
            trackTokenId: tokenId,
            trackOwnerAddress: msg.sender
        });

        // push new track to allTracks
    ...
    // check if address currently owns a track
    function _checkTrack(address owner) internal view returns (bool) {
        if(allTracks.length > 0) {
            for(uint i = 0; i < allTracks.length; i ++) {
                if(allTracks[i].trackOwnerAddress == owner) {
                    return false;
                }
            }
        }
