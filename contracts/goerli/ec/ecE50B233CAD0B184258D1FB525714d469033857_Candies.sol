...
contract Candies is ERC721, ERC721Royalty, AccessControl {
    ...
    string baseURI = "https://mustard-labs.s3.amazonaws.com/candies/";
    string movURI = "https://mustard-labs.s3.amazonaws.com/candies/";
    mapping(uint => string) public names;
    mapping(uint => string) public attributes;
    mapping(uint => uint) public codes;
    mapping(uint => uint) public countsByType;
    uint public numTypes = 4;
...
// Public
    ...
    function mint() public { // CLOSE BEFORE DEPLOY
        address to = msg.sender;
        uint _pull = collectionDeck.draw();
        console.log("Pulled:", _pull, " type: ", getName(_pull));
        _safeMint(to, _pull);
    }

    function unwrap(uint _tokenId) public {
        // check that i own the nft
        // check that nft has been minted
        ...
        console.log(_tokenId, getName(_tokenId), getName(_unwrappedId), _unwrappedId);
        emit Unwrap(_tokenId, msg.sender, _unwrappedId);
    }
...
// View
    ...
    function getMetadataJson(uint256 _tokenId) public view returns (string memory) {
        ...
        string memory _statusCaps = unwrapped.get(_tokenId) ? "Unwrapped" : "Wrapped";
        ...
        _attributes = string(abi.encodePacked(_attributes,'{"trait_type":"Status","value": "',_statusCaps,'"},\n'));
        _attributes = string(abi.encodePacked(_attributes,attributes[_code % 10]));
        string memory meta = string(
            abi.encodePacked(
            '{\n"name": "Grandma', "'", 's Candy #', _tokenId.toString(),
        ...
        meta = string(
            abi.encodePacked(
                meta,
                ',\n"external_url": "http://candies.shop/candies/', _tokenId.toString(),
                '",\n"image": "', baseURI, _status,'/', _imageId.toString(),'.gif"',
                ',\n"video": "', movURI, _status,'/', _imageId.toString(),'.mp4"'
            )
        );
...
// Initial Setup (Admin)
    ...
    function add(uint _startIndex, uint8[] memory arr) public onlyRole(MINTER_ROLE) {
        // console.log("adding ", arr.length, " @ ", _startIndex);
    ...
    function init() public onlyRole(MINTER_ROLE) {
        ...
        uint[] memory _counts = new uint[](numTypes);
        for (uint i = 0; i < totalCount; i++) {
            _counts[codes[i] / 10]++;
        }
        uint _checkTotal;
        for (uint i = 0; i < numTypes; i++) {
            console.log("Counted ", _counts[i], " of type ", i);
        ...
        for (uint i = 0; i < numTypes; i++) {
            console.log("Setup deck for ID ", i, " of ", _counts[i]);
            candyDecks[i].setup(_counts[i]);
        }
        initComplete = true;
        console.log("STARTING... Total Candies = ", totalCount);
    }
