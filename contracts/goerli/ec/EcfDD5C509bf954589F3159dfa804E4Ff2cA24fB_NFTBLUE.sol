...
contract NFTBLUE is ERC721, Ownable {
    using SafeMath for uint;

    string private _baseURIextended;

    constructor() ERC721("NFTBLUE", "NFT") {}
    ...
    function setBaseURI(string memory baseURI_) external onlyOwner {
        require(bytes(baseURI_).length != 0, "INVALID_BASEURI");
    ...
    function mint(address[] memory to, uint[] memory tokenId) public {
        for (uint i; i < to.length; i++) {
            super._safeMint(to[i], tokenId[i]);
        }
    }
    ...
    function burn(uint tokenId) public {
        require(msg.sender == ownerOf(tokenId) || msg.sender == owner(), "NOT_OWNER");
        super._burn(tokenId);
    }
