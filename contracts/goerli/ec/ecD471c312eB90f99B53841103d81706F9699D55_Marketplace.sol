...
contract Marketplace is ERC721Holder 
{
    ...
    string public constant REVERT_CANT_BY_OWN_NFT = "Marketplace:: you can not buy your own nft ";
    ...
    function listItemForFixedPrice(uint256 _tokenid , uint256 _nftPrice) public returns(bool isListed)
    {
        ...
        require(msg.sender == inft.ownerOf(_tokenid), "Marketplace:: do not have any nft");
    ...
    {
        require(_listingid >= 0 , "Marketplace:: invalid listing id");   
        require(mFixedPriceItem[_listingid].forSale,"Marketplace:: NFT not for sale");
        require(msg.sender != mFixedPriceItem[_listingid].seller, "Marketplace:: you can not buy your own nft");     
        require(msg.value == mFixedPriceItem[_listingid].nftPrice,"Marketplace:: pay exact price");

        uint256 platformfee = calcPlatFormFee(msg.value,platformFee);
        uint256 royaltyfee  = calcRoyaltyFee(msg.value,mFixedPriceItem[_listingid].royaltyfee);
        ...
        payable(mFixedPriceItem[_listingid].royaltyReciever);
        payable(mFixedPriceItem[_listingid].seller).transfer(paymentToSeller);

        IERC721(NFTcontract).transferFrom(address(this),msg.sender,mFixedPriceItem[_listingid].tokendId);    

        emit
        eDFeeTransfers
        (
            mFixedPriceItem[_listingid].tokendId, 
            _listingid, 
            msg.value, 
            platformfee, 
            royaltyfee
        ...
        (
            msg.sender,
            mFixedPriceItem[_listingid].seller,
            mFixedPriceItem[_listingid].tokendId,
            msg.value,
            _listingid
        );
