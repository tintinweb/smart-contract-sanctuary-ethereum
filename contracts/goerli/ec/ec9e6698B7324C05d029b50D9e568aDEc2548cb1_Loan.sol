...
contract Loan is LoanStructs, LoanInterface, Manageable, ReentrancyGuard {
    ...
    function getLoanAmount(
        ...
        return (loanList[NFTAddr][NFTId].billInfo[businessId].FTAmount, loanList[NFTAddr][NFTId].billInfo[businessId].total);
    }
    ...
    function getPeriodAmount(
        ...
        uint256 len = businessIds.length;
        if(NFTAddrs.length != len || NFTIds.length != len || loanPeriods.length != len){
            revert ArrayLengthMismatch();
        }
        PeriodAmounts = new uint256[](len);
        status = new bool[](len);
        for(uint256 i = 0; i < len; ++i){
            PeriodAmounts[i] = loanList[NFTAddrs[i]][NFTIds[i]].bill[businessIds[i]][loanPeriods[i]].amount;
            status[i] = loanList[NFTAddrs[i]][NFTIds[i]].bill[businessIds[i]][loanPeriods[i]].status;
        }
        return (PeriodAmounts, status);
    }
