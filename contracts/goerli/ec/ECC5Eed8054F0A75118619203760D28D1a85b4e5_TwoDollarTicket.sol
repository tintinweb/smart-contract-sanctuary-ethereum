...
contract TwoDollarTicket is VRFV2WrapperConsumerBase, ReentrancyGuard, Ownable, Pausable {
    ...
    receive() external payable {
        balance += msg.value;
        emit TransferReceived(msg.sender, address(this), msg.value, lotteryId, block.timestamp);
    }
    ...
    function enterDraw() whenNotPaused() public payable nonReentrant {
        require(msg.value >= ticketprice);
            ...
            emit ticketnumber(ticketcount);
            emit TransferReceived(msg.sender, address(this), msg.value, ticketcount, block.timestamp);
        if (ticketcount == freecount) {
            freecount = freecount + 2;
            ticketcount = ticketcount + 1;
            freeplaycount = freeplaycount +1;
            players.push(payable(msg.sender));
            emit FreePlay(msg.sender, address(this), freeplaycount, block.timestamp);
        }
    }
    ...
    function payWinner() whenPaused() public onlyOwner nonReentrant {
        ...
        emit TransferSent(msg.sender, address(this), players[index], address(this).balance, lotteryId, block.timestamp);
