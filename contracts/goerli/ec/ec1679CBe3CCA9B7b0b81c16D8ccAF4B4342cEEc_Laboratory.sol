...
contract Laboratory {
    ...
    function NewStreamWallet(TeamWallet.Payee[] calldata team, bytes32 root) external returns (address) {
...
contract StreamTeamWallet is Initializable, ReentrancyGuard {
    TeamWallet.Payee[] private _team;
    ...
    function initialize(TeamWallet.Payee[] calldata team, bytes32 _root) initializer public {
        root = _root;
    ...
    receive() external payable {
        ...
        for (uint i = 0; i < _team.length; i++) {
            TeamWallet.Payee memory payee = _team[i];
