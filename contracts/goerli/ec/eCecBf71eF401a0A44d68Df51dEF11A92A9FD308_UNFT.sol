 ...
 **/
contract UNFT is IUNFT, ERC721EnumerableUpgradeable, IERC721ReceiverUpgradeable, IERC1155ReceiverUpgradeable, IERC4907 {
   ...
   */
  modifier onlyOwner() {
    require(owner() == _msgSender(), "UNFT: caller is not the owner");
    _;
  }
  ...
  function transferOwnership(address newOwner) public virtual onlyOwner {
    require(newOwner != address(0), "UNFT: new owner is the zero address");
  ...
  modifier onlyClaimAdmin() {
    require(claimAdmin() == _msgSender(), "UNFT: caller is not the claim admin");
    _;
  }
  ...
  function setClaimAdmin(address newAdmin) public virtual onlyOwner {
    require(newAdmin != address(0), "UNFT: new admin is the zero address");
  ...
  function mint(address to, uint256 tokenId) external override nonReentrant {
    ...
    if (!isCA) {
      require(to == _msgSender(), "UNFT: caller is not to");
    }
  ...
  function setUser(uint256[] memory nftTokenIds, address[] memory users, uint64[] memory expires) public {
    require(nftTokenIds.length == users.length && users.length == expires.length, "mismatch length");
    // step 1: moving underlying asset forward to _msgSender()
    uint256 i;
    for (i = 0; i < nftTokenIds.length; i++) {
      require(_exists(nftTokenIds[i]), "UNFT: nonexist token");
      require(_minters[nftTokenIds[i]] == _msgSender(), "UNFT: caller is not minter");
      IERC721Upgradeable(_underlyingAsset).safeTransferFrom(address(this), _msgSender(), nftTokenIds[i]);
    }

    // setup 2: execute set user
    for (i = 0; i < nftTokenIds.length; i++) {
      IERC4907(_underlyingAsset).setUser(nftTokenIds[i], users[i], expires[i]);
    }
    // setup 3: moving underlying asset backword from _msgSender()
    for (i = 0; i < nftTokenIds.length; i++) {
      IERC721Upgradeable(_underlyingAsset).safeTransferFrom(_msgSender(), address(this), nftTokenIds[i]);
    }
  ...
  function setUser(uint256 tokenId, address user, uint64 expires) external override {
    ...
    require(_minters[tokenId] == _msgSender(), "UNFT: caller is not minter");
    IERC721Upgradeable(_underlyingAsset).safeTransferFrom(address(this), _msgSender(), tokenId);
    IERC4907(_underlyingAsset).setUser(tokenId, user, expires);
    IERC721Upgradeable(_underlyingAsset).safeTransferFrom(_msgSender(), address(this), tokenId);
  }

  function userOf(uint256 tokenId) external override view returns(address) {}

  function userExpires(uint256 tokenId) external override view returns(uint256) {}
  ...
  function flashLoan(
    ...
    require(receiverAddress != address(0), "UNFT: zero address");
    require(nftTokenIds.length > 0, "UNFT: empty token list");
  ...
  function claimERC20Airdrop(
    ...
    require(token != _underlyingAsset, "UNFT: token can not be underlying asset");
    require(token != address(this), "UNFT: token can not be self address");
