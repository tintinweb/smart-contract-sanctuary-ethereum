...
contract Multitest is ERC721A, Ownable {
    ...
    constructor() ERC721A("Multitest", "MT") {
        transferOwnership(0xd3543A1dD0a611cc2c2C7E7c48Fa6A679F4DEe57);
    }
    ...
    function mintOneFree(uint256 quantity) external payable  {
        require(quantity <= MAX_PER_TX, "Exceeded the limit tx");
        require(quantity + _numberMinted(msg.sender) <= MAX_PER_WALLET, "Exceeded the limit");
        require(msg.value >= mintRate * (_numberMinted(msg.sender) > 0 ? quantity : quantity - 1), "Insufficient funds!");
        _safeMint(msg.sender, quantity);
    }

    function mintHolder(IERC721 token, uint256 quantity) external payable  {
        require(token.balanceOf(msg.sender) > 0, "Need to hold one of this");
        require(quantity <= MAX_PER_TX, "Exceeded the limit tx");
        require(quantity + _numberMinted(msg.sender) <= MAX_PER_WALLET, "Exceeded the limit");
        require(msg.value >= mintRate * quantity, "Insufficient funds!");
        _safeMint(msg.sender, quantity);
