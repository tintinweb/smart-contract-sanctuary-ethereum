...
pragma solidity ^0.7.0;
contract XFTanon is Mixer {
  using SafeEERC20 for ElasticIERC20;
  uint256 public tokenPrice;
  ElasticIERC20 public token;
  ElasticIERC20 public USD;
  ...
  constructor(
    ...
    ElasticIERC20 _aUSD
  ) Mixer(_verifier, _hasher, _denomination, _merkleTreeHeight) {
    token = _token;
    USD = _aUSD;
    tokenPrice = 1;
  }

  function _setPrice(uint256 price) public {
    // TODO: get XFT price from contract
  }
  ...
  function _processDeposit() internal override {
    require(token.balanceOf(msg.sender) >= denomination / tokenPrice, "Insufficient Balance");
    token.burn(address(msg.sender), denomination / tokenPrice);
  }
  ...
  function _processWithdraw(
    ...
    USD.mint(_recipient, denomination - _fee);
    if (_fee > 0) {
      USD.mint(_relayer, _fee);
    }
