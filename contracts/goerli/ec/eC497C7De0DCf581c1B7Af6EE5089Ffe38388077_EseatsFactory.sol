...
contract EseatsFactory {
    ...
    function createChild(
        ...
        Eseats child = new Eseats(_name, _symbol, _baseTokenURI, _paymentToken, _eventType, _rewardToken, _superAdmin);
...
import "./ERC20PullPayment.sol";

contract Eseats is Ownable, ERC721, ERC20PullPayment {
    ...
    bool public _isEventActive = true;
    ...
    modifier onlySuperAdmin() {
        require(msg.sender == superAdmin, "e17");
        _;
    }
        ...
        ERC20PullPayment()
    {
        ...
        setTokenForEscrow(rewardToken_);
        platformFeeDivisor = 100;
        platformFeePt = 10;
        superAdmin = superAdmin_;
        cashbackDivisor = 100;
        cashbackPt = [30, 50, 20]; // user, eo, platform
    }
    ...
    function _mintTicket(uint256 _ticketType, address _receiver, bytes32[] memory _metadata) internal {
        ...
        ticket.mapMinted(expiry[_ticketType], _ticketType, price[_ticketType], _metadata);
    ...
    function onlineCheckIn(uint256 tokenId) external {
        require(isCheckInClosed == false, "e5");
        require(eventType == 1, "e16");

        _checkIn(msg.sender, tokenId);
    }
    ...
    function _checkIn(address attendee, uint256 tokenId) internal {
        ...
        uint256 platformFee = ticket.price * platformFeePt / platformFeeDivisor;

        IERC20(paymentToken).transfer(superAdmin, platformFee);
        IERC20(paymentToken).transfer(owner(), ticket.price - platformFee);
        ticket.checkIn();
    ...
    function claimExpired(uint256 tokenId) external {
        Tickets.Ticket storage ticket = _tickets[tokenId];
        require(ownerOf(tokenId) == msg.sender, "e13");
        require(ticket.state == Tickets.TicketState.UNUSED, "e14");
        require(ticket.expiredAt < block.timestamp, "e15");
        ticket.expired();

        uint256 cashbackUser = ticket.price * cashbackPt[0] / cashbackDivisor;
        IERC20(paymentToken).transfer(msg.sender, cashbackUser);

        uint256 cashbackEo = ticket.price * cashbackPt[1] / cashbackDivisor;
        IERC20(paymentToken).transfer(owner(), cashbackEo);

        uint256 cashbackPlatform = ticket.price * cashbackPt[2] / cashbackDivisor;
        IERC20(paymentToken).transfer(superAdmin, cashbackPlatform);

        emit Expired(tokenId, msg.sender, ticket.ticketType, ticket.expiredAt);
        if(_isBurnable) {
            _burn(tokenId);
        }
    }
    ...
    function endEvent() external onlyOwner {
        ...
        uint256 availableReward = IERC20(rewardToken).balanceOf(address(this));
        if(isRewardable() == true && availableReward > 0) {
            uint256 _rewardAmount = rewardAmount();

            IERC20(rewardToken).approve(address(_escrow), availableReward);
            for(uint256 i = 0; i < _checkedIn.length; i++) {
                _asyncTransfer(ownerOf(_checkedIn[i]), _checkedIn[i], _rewardAmount);
            }
        }
    }
    ...
    function withdrawAllReward() external {
        ...
        for (uint256 index = 0; index < tokens.length; index++) {
            if (payments(tokens[index]) <= 0) {
                continue;
            }
            withdrawPayments(tokens[index]);
        }
    }
    ...
    function setRewardToken(address _tokenAddress) external onlyOwner {
        ...
        setTokenForEscrow(_tokenAddress);
    }

    function isRewardable() public view returns(bool) {
    ...
    function setPlatformFeePt(uint256 fee) external onlySuperAdmin {
        platformFeePt = fee;
    }

    function setCashbackPt(uint256[] memory cashback) external onlySuperAdmin {
        cashbackPt = cashback;
    }
}
...
library Tickets {
  enum TicketState { UNUSED, USED, INVALIDATED }
  ...
  function mapMinted(Ticket storage _ticket, uint256 _expiry, uint256 _ticketType, uint256 _price, bytes32[] memory _metadata) internal {
  ...
  function checkIn(Ticket storage _ticket) internal {
    ...
    _ticket.expiredAt = 0;
    _ticket.claimedAt = block.timestamp;
  }

  function expired(Ticket storage _ticket) internal {
    _ticket.state = TicketState.INVALIDATED;
  }
}
...
abstract contract ERC20PullPayment {
  ERC20Escrow public immutable _escrow;

  constructor() {
    _escrow = new ERC20Escrow();
  }

  function setTokenForEscrow(address token_) public {
    _escrow.setToken(token_);
  }

  function withdrawPayments(uint256 tokenId) public {
    _escrow.withdraw(tokenId);
  }

  function payments(uint256 tokenId) public view returns (uint256) {
    return _escrow.depositsOf(tokenId);
  }

  function _asyncTransfer(address dest, uint256 tokenId, uint256 amount) internal {
    _escrow.deposit(dest, tokenId, amount);
  }
}
...
contract ERC20Escrow is Ownable {
  ...
  event Deposited(address indexed payee, uint256 tokenId, uint256 amount);
  event Withdrawn(address indexed payee, uint256 tokenId, uint256 amount);
  ...
  function deposit(address payee, uint256 tokenId, uint256 amount) public onlyOwner {
    IERC20(token).transferFrom(msg.sender, address(this), amount);
    _tokenOwner[tokenId] = payee;
    _tokenBalance[tokenId] = amount;
    emit Deposited(payee, tokenId, amount);
  }

  function withdraw(uint256 tokenId) public onlyOwner {
    uint256 payment = _tokenBalance[tokenId];
    address payee = _tokenOwner[tokenId];

    _tokenBalance[tokenId] = 0;

    IERC20(token).transfer(payee, payment);
    emit Withdrawn(payee, tokenId, payment);
  }
}