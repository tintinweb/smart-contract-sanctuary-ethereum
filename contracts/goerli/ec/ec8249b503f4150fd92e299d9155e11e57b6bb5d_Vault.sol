...
contract Vault is Ownable, IVault {
    ...
    IWTokenGateway internal wtokenGateway;
    ...
    function setGateway(address gateway) external onlyOwner {
        wtokenGateway = IWTokenGateway(gateway);
    }

    /**
     * @dev
     */
    function deposit(address pool, bool wrap, address asset, uint256 amount) external override whenNotPaused {
        // validate
        if (amount == 0) {
        ...
        if (wrap && (msg.sender != address(wtokenGateway))) {
        ...
        emit Deposit(pool, asset, msg.sender, amount);
    }

    /**
     * @dev
     */
    function withdraw(address asset, bool wrap, uint256 amount, address to) external override whenNotPaused {
        // validate
		// if (!wrap && msg.sender != _msgSender()) {
        //     revert Errors.InvalidCaller();
		// }
        ...
        // withdraw
		if(wrap) {
			IWToken(asset).transferFrom(address(this), msg.sender, amount);
		} else {
