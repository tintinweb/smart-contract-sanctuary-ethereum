    ...
    ReentrancyGuardUpgradeable
{
    ...
    uint256 private _limitAmountForDevMint;
    uint256 private _limitAmountForPrivateMint;

    // individual minted limit
    uint256 private _limitAmountPerWallet;

    // current minted
    uint256 private _privateMintedToken;
    ...
    function initialize(
        ...
        uint256 limitAmountForDevMintValue,
        uint256 limitAmountForPrivateMintValue,
        ...
        _limitAmountForDevMint = limitAmountForDevMintValue;
        _limitAmountForPrivateMint = limitAmountForPrivateMintValue;
        _limitAmountPerWallet = limitAmountPerWalletValue;
        _defaultURI = baseURIValue;
        _beneficiary = beneficiaryValue;
        _privateMintedToken = 0;
        _airdroppedToken = 0;
        _revealBlock = 0;
        _seed = 0;
    ...
    modifier isMintAllowed() {
        ...
        require(_currentState != States.PrivateMintEnded, "Private mint ended");
    ...
    function privateMint(address to, uint256 quantity)
        external
        onlyOwner
        isMintAllowed
    {
        require(
            _currentState == States.PrivateMintStarted,
            "Not during private mint"
        );
        require(quantity <= _maxBatchSize, "Can not mint this many");

        uint256 newPrivateMintedToken = _privateMintedToken + quantity;
        require(
            newPrivateMintedToken <= _limitAmountForPrivateMint,
            "Reached private mint limit"
        );

        _privateMintedToken = newPrivateMintedToken;

        uint256 startTokenId = _minted;
        emit PrivateMint(to, startTokenId, quantity);
    ...
    function devMint(uint256 quantity) external onlyOwner {
        require(
            totalSupply() + quantity <= _limitAmountForDevMint,
    ...
    function allowAirdrop(address addr) external onlyOwner {
        _airdropAllows[addr] = true;
        emit AllowAirdrop(addr);
    }

    function allowAirdrops(address[] memory addrs) external onlyOwner {
        uint256 count = addrs.length;
        require(count <= _maxBatchSize, "Can not allow to this many");
        for (uint256 i = 0; i < count; i++) {
            _airdropAllows[addrs[i]] = true;
        }
        emit AllowAirdrops(addrs);
    }

    function setAirdropAllowance(address addr, uint256 quantity)
        external
        onlyOwner
    {
        require(_airdropAllows[addr], "Allow airdrop first");
        _airdropAllowances[addr] = quantity;
        emit SetAirdropAllownace(addr, quantity);
    }
    ...
    function airdrop(address[] memory addresses) external airdropAllowed {
        ...
        uint256 quantity = addresses.length;
        uint256 airdropLimit = _airdropAllowances[msg.sender];
        require(airdropLimit >= quantity, "Exceed the airdrop allowance");
        ...
        _airdropAllowances[msg.sender] -= quantity;
        _airdroppedToken += quantity;

        emit Airdrop(addresses);
    }

    function airdroppedToken() external view onlyOwner returns (uint256) {
        return _airdroppedToken;
    }

    // TODO: Psi's chainlink extension
    function setTokenBaseURI(string memory newTokenBasURI) external onlyOwner {
    ...
    function limitAmountForDevMint() external view onlyOwner returns (uint256) {
        return _limitAmountForDevMint;
    }

    function limitAmountForPrivateMint()
        external
        view
        onlyOwner
        returns (uint256)
    {
        return _limitAmountForPrivateMint;
    }

    function limitAmountPerWallet() external view onlyOwner returns (uint256) {
        return _limitAmountPerWallet;
    }

    function maxBatchSize() external view onlyOwner returns (uint256) {
        return _maxBatchSize;
    }

    function defaultURI() external view onlyOwner returns (string memory) {
        return _defaultURI;
    }
    ...
    function tokenBaseURI() external view onlyOwner returns (string memory) {
        ...
        returns (string memory)
    {
        ...
        return
            isRevealed()
                ? string(
                    abi.encodePacked(_tokenBaseURI, metadata(tokenId), ".json")
                )
                : _defaultURI;
    }
}