...
contract ChillTunaV1 is ERC721AUpgradeable, ERC721AQueryableUpgradeable, DefaultOperatorFiltererUpgradeable, OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    mapping(address => uint256[]) private _staker;
    mapping(uint256 => StakeToken) public stakeToken;

    event StakeEvent(uint256[] tokenIds, address by, uint256 stakedAt);
    event UnstakeEvent(
        uint256 tokenId,
        address by,
        uint256 stakedAt,
    ...
    function stake(uint256[] calldata tokenIds) external nonReentrant {
        ...
        for (uint256 i = 0; i < tokenIds.length; i++) {
            require(ownerOf(tokenIds[i]) == _msgSender(), "Token does not belong to you");
            safeTransferFrom(_msgSender(), address(this), tokenIds[i]);

            _staker[_msgSender()].push(tokenIds[i]);
            stakeToken[tokenIds[i]] = StakeToken(
                _msgSender(),
        ...
        emit StakeEvent(tokenIds, _msgSender(), block.timestamp);
    }
    ...
    function getStaker(address by) external view returns (uint256[] memory) {
        return _staker[by];
    }
