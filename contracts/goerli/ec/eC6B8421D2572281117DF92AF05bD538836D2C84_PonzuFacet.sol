...
import {WithRoles} from "../libraries/LibAccessControl.sol";
...
contract PonzuFacet is WithRoles {
  ...
  function getCurrentRoundData() external view returns (PonzuRoundData memory) {
    ...
    return
      PonzuRoundData({
        currentRoundNumber: ps.currentRoundNumber(),
        currentRoundStartTime: ps.currentRoundStartTime(),
        currentRoundDeadline: ps.currentRoundDeadline(),
        currentRoundEndTime: ps.currentRoundEndTime(),
        currentRoundPrizePool: ps.currentPrizePool(),
        currentRoundPonzuPool: ps.sumTotalValidDeposits(),
  ...
  function ponzuPool() external view returns (uint256) {
    PonzuStorage storage ps = LibPonzu.DS();
    return ps.sumTotalValidDeposits();
  }

  function currentRoundInfo() external view returns (uint256, uint256, uint256) {
    PonzuStorage storage ps = LibPonzu.DS();
    return (ps.currentRoundStartTime(), ps.currentRoundDeadline(), ps.currentRoundEndTime());
  }
  ...
  function getParticipant(address _account) external view returns (uint256, uint256, uint256) {
    PonzuStorage storage ps = LibPonzu.DS();
    Participant memory participant = ps.participants[_account];

    return (
      participant.oldDepositAmount,
      participant.newDepositAmount,
      participant.newDepositRound
    );
  }
}
...
library LibPonzu {
  ...
  function deposit(PonzuStorage storage ps, uint256 amount) internal {
    ...
    uint currentRound = ps.currentRoundNumber();
    uint256 lastClosedRound = ps.lastClosedRound;
    if (lastClosedRound == 0 || lastClosedRound + 1 == currentRound) {
  ...
  function selectWinner(PonzuStorage storage ps, uint256 number, uint256 roundNum) internal {
    uint256 roundStartTime = ps.startTime + roundNum * ROUND_DURATION;
    if (block.timestamp < roundStartTime + ROUND_DURATION) revert RoundNotEnded(roundNum);
    ...
    uint256 winnerNumber = number / ps.sumTotalValidDeposits();
    // divide number by total deposited
    uint256 currentRewardTokenBalance = IERC20(ps.rewardToken).balanceOf(address(this));
    uint256 prizeAmount = ps.currentPrizePool() - currentRewardTokenBalance;
    ...
    address winnerAddress = ps._selectWinner(roundNum, winnerNumber);
  ...
  function _selectWinner(
    ...
    for (uint256 i = 0; i < ps.participantsList.length; ++i) {
      ...
      uint256 pDeposit = ps.participants[winnerAddress].oldDepositAmount;
  ...
  function handleRandomNumber(PonzuStorage storage ps, uint256 randomNumber) internal {
...
bytes32 constant REQUEST_NUMBER_ROLE = keccak256("REQUEST_NUMBER_ROLE");
