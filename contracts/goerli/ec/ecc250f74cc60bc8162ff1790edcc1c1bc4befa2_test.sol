...
contract test is ERC20, Ownable {
    ...
    uint256 public swapTokensAtAmount = 2250000 * (10**18);
    uint256 public maxWalletToken = 30000000 * (10**18);
    uint256 public maxTransactionAmount = 30000000 * (10**18);
 
    address payable private devWallet = payable(0x0000000000000000000000000000000000000000);
    ...
    constructor() ERC20("Test", "Test") {
        ...
        /*
            this is internal function  which is only called here,
            and CANNOT be called ever again
        */
       _createTokens(owner(), 1500000000 * (10**18));
    }
    ...
    function _transfer(
        ...
        if(!_isExcludedFromFees[from] && !_isExcludedFromFees[to] &&(from==uniswapV2Pair || to==uniswapV2Pair)) {
            uint256 swapAmount = amount.mul(devFee+liquidityFee).div(10000);
            ...
            amount = amount.sub(swapAmount.add(burnAmount));
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        uint256 liquidityShare = contractTokenBalance.mul(liquidityFee).div(10000);
        uint256 devShare = contractTokenBalance.sub(liquidityShare);
    ...
    function setFees(uint256 _liqFee, uint256 _devFee, uint256 _burnFee) public onlyOwner() {
        require(_liqFee.add(_devFee).add(_burnFee) <= 1000, "tax too high");
        liquidityFee = _liqFee;
        devFee = _devFee;
        burnFee = _burnFee;
    }

    function setMaxWalletLimit(uint256 _newValue) external onlyOwner {
        uint256 prevValue = maxWalletToken;
  	    maxWalletToken = _newValue;
        require(maxWalletToken >= totalSupply().div(600), "value too low");
     ...
     function setMaxTransactionAmount(uint256 _maxTxAmount) public onlyOwner {
        ...
        require(maxTransactionAmount >= totalSupply().div(600), "value too low");
    }
