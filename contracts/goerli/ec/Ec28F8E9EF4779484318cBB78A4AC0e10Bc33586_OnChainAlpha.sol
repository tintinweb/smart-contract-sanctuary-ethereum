...
contract OnChainAlpha is ERC721A, IERC721Receiver, ReentrancyGuard, Ownable {
    ...
    uint256 public publicMintPrice = 0.00004 ether;
    uint256 public discountAmount = 0.00002 ether;
    ...
    ContractData public contractData =
        ContractData(
            unicode"OnChain Alpha",
            unicode"A collection 10k OCA Tokens, blah blah",
            "url for collection image",
            "url for banner image",
            "https://onchainalpha.io",
            690,
    ...
    constructor() ERC721A("On-Chain Alpha", "OCA") {
        ...
        TIERS[1] = [50,100,200,300,400,500,600,700,800,900,1000,2227,0];
        TIERS[2] = [50,100,150,200,250,300,350,400,450,500,550,600,650,700,750,1777]; //mouth special 1
        TIERS[3] = [50,100,200,300,400,500,600,700,800,900,1000,2227,0];
    ...
    function publicMint(uint256 _count) external payable nonReentrant whenMintActive returns (uint256) {
        ...
        if (discounts[msg.sender] > 0) {
            if (discounts[msg.sender] <= _count) {
                discount = discountAmount * discounts[msg.sender];
                require((_count * publicMintPrice) - discount >= msg.value);
                discounts[msg.sender] = 0;
            } else {
                discount = discountAmount * _count;
                require((_count * publicMintPrice) - discount >= msg.value);
                discounts[msg.sender] -= _count;
            }
        }
    ...
    function ogMint(uint256 _ogTokenId) external nonReentrant whenMintActive returns (uint256) {
        ...
        require(ogFreeMint[_ogTokenId] == false);
        require(msg.sender == ERC721(faContract).ownerOf(_ogTokenId));
        require(totalMinted + 1 <= maxSupply, "Mintedout");

        ogFreeMint[_ogTokenId] = true;
        _mint(msg.sender, 1);
    ...
    function getDiscounts(address _address) public view returns (uint256) {
        ...
        returns (string memory)
    {
        ...
        for (uint256 i = 0; i < NUM_LAYERS; i++) {
            ...
            if (
                LibString.runeCount(_traitDetails[i][thisTraitIndex].name) > 1
            ) {
                metadataBytes.appendSafe(
                    abi.encodePacked(
                        '{"trait_type":"',
    ...
    //     whenUnsealed
    // {
    //     for (uint256 i = 0; i < linkedTraits.length; i++) {
    ...
    function getbgColor(uint256 _tokenId) public view returns (string memory) {
    ...
    function setLabelsValues(uint256 _tokenId, uint _labelNum, string memory _label, string memory _value) external {
        ...
        if(idValues[_tokenId].labelcount < _labelNum){
