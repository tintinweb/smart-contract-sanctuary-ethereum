...
contract ERC721Staking is Ownable, IERC721Receiver, Pausable, ReentrancyGuard {
    ...
    function setTokensPerDay(uint256 _rate) public onlyOwner {
        rate = _rate;
    }
        ...
        returns (uint256[] memory rewards)
    {
        ...
        for (uint256 i; i < tokenIds.length; i++) {
            ...
            diff = (block.timestamp - _depositTimes[account][tokenId]) / 60 / 60 / 24;
        ...
        returns (uint256)
    {
        ...
        uint256 diff = (block.timestamp - _depositTimes[account][tokenId]) / 60 / 60 / 24;
