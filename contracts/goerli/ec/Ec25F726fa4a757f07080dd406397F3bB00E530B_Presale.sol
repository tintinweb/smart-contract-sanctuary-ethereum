...
contract Presale is Airdrop {
    ...
    /* Structure BuyCollection */
    struct BuyCollection {
        /* The number of purchased tokens for the public price */
        uint256 publicCount;
        /* The number of purchased tokens according to the whitelist */
    ...
    mapping(address => mapping(address => BuyCollection)) public buyCollections;
    ...
    function addCollection(
        ...
        uint256 stopTimestamp,
        address erc20Address
    ) external onlyAdmin checkRemoveCollection(collection) {
    ...
    function removeCollection(address collection)
        ...
        override
        onlyAdmin
        checkAddCollection(collection)
    {
        require(
            block.timestamp <
                collectionInformations[collection].startWhitelistTimestamp ||
                block.timestamp <
                collectionInformations[collection].startPublicTimestamp,
    ...
    function burnAll(address collection) external onlyAdmin {
        ERC721Collection erc721 = ERC721Collection(collection);

        require(
            erc721.totalSupply() >= erc721.maxSupply(),
            "Burning tokens is not possible as not all tokens were sold out."
        );
    ...
    /// @dev Only Admin
    /// @param erc20Address Address ERC20
    /// @param amount The Number ER20 token
    function withdraw(address erc20Address, uint256 amount) external onlyAdmin {
        IERC20 erc20 = IERC20(erc20Address);
        uint256 balanceContract = erc20.balanceOf(address(this));
        ...
        if (amount <= balanceContract) {
            erc20.transfer(ownerContract, amount);
        } else {
            erc20.transfer(ownerContract, balanceContract);
        }
    }
    ...
    function isWhitelist(
        ...
        bytes32 leaf
    ) public view override checkAddCollection(collection) returns (bool) {
        bool isWhiteilst = Airdrop.isWhitelist(collection, proofs, leaf);
        ...
        if (
            ...
            (collectionInformations[collection].startWhitelistTimestamp <=
                block.timestamp &&
                block.timestamp <=
                collectionInformations[collection].stopWhitelistTimestamp)
        ) {
            return true;
        }
    ...
    function isWhitelistTest(
        address collection,
        bytes32[] calldata proofs,
        bytes32 leaf
    ) public view checkAddCollection(collection) returns (bool) {
        return Airdrop.isWhitelist(collection, proofs, leaf);
    }

    function isCheckTime(address collection)
        public
        view
        checkAddCollection(collection)
        returns (bool)
    {
        return (collectionInformations[collection].startWhitelistTimestamp <=
            block.timestamp &&
            block.timestamp <=
            collectionInformations[collection].stopWhitelistTimestamp);
    }
    ...
    function getTokens(
        ...
        uint256 tokenAmount,
        uint256 maxAmount
    ) external override checkAddCollection(collection) {
        ERC721Collection erc721 = ERC721Collection(collection);
        ...
        erc20.transferFrom(msg.sender, address(this), cTotalPrice);

        erc721.mint(msg.sender, tokenAmount);

        uint256 whitelistAmount = getWhitelistAmount(
            collection,
            accountHash,
            proofs,
            tokenAmount,
        ...
        if (whitelistAmount > 0) {
            whitelistAmountBuy[accountHash][collection] += whitelistAmount;
            buyCollections[collection][msg.sender]
                .whitelistCount += whitelistAmount;
            buyCollections[collection][msg.sender].publicCount +=
                tokenAmount -
                whitelistAmount;
        } else {
            buyCollections[collection][msg.sender].publicCount += tokenAmount;
        }
    ...
    /// @param collection Address NFT collection
    function returnFunds(address collection)
        external
        checkAddCollection(collection)
    {
        ERC721Collection erc721 = ERC721Collection(collection);
        ...
        require(erc721.isBurnt(), "Collection tokens were not burned.");

        uint256 cTotalFunds = buyCollections[collection][msg.sender]
            .publicCount *
            collectionInformations[collection].publicPrice +
            buyCollections[collection][msg.sender].whitelistCount *
            collectionInformations[collection].whitelistPrice;

        require(cTotalFunds > 0, "Nothing to return");

        require(
            erc20.balanceOf(address(this)) >= cTotalFunds,
            "Not enough money to return"
        );

        erc20.transferFrom(address(this), msg.sender, cTotalFunds);
    ...
    function clearDropTokenAccounts(DropToken[] calldata accountsDrop)
        external
        override
        onlyAdmin
    {
        for (uint256 i = 0; i < accountsDrop.length; i++) {
    ...
    /// @param _maxAmountWhiteList Max Number token for presale. For check amount
    function getTotalPriceCollection(
        address _collection,
        bytes32 _accountHash,
        bytes32[] calldata _proofs,
        uint256 _amount,
        uint256 _maxAmountWhiteList
    ) private view checkAddCollection(_collection) returns (uint256) {
        uint256 totalPrice;
        ...
        uint256 whitelistTokenAmount = getWhitelistAmount(
            _collection,
            _accountHash,
            _proofs,
            _amount,
        ...
        totalPrice =
            collectionInformation.whitelistPrice *
            whitelistTokenAmount +
            (collectionInformation.publicPrice *
...
contract Airdrop is OwnableExt {
    ...
    struct AirdropToken {
        ...
        uint256 amount;
        /* Max Number token for airdrop. For check amount */
        uint256 maxAmount;
    }
    ...
    */
    ...
    function setWhitelist(address collection, bytes32 merkleRoot)
        external
        virtual
        onlyAdmin
        checkAddCollection(collection)
    ...
    function getTokens(
        ...
        uint256 tokenAmount,
        uint256 maxAmount
    ) external virtual checkAddCollection(collection) {
        bytes32 accountHash = keccak256(
            abi.encodePacked(msg.sender, maxAmount)
        );
    ...
    ) external onlyAdmin checkAddCollection(collection) {
        for (uint256 i = 0; i < airdropAccounts.length; i++) {
            _getTokens(
                airdropAccounts[i].account,
                collection,
                airdropAccounts[i].amount,
                airdropAccounts[i].maxAmount
            );
        }
    }
    ...
    function isWhitelist(
        ...
        bytes32[] calldata proofs,
        bytes32 leaf
    ) public view virtual checkAddCollection(collection) returns (bool) {
    ...
    function _getTokens(
        ...
        require(
            dropTokenAccounts[accountHash][_collection] + _amount <= _maxAmount,
            "This account has already received a free token."
        );

        erc721.mint(_account, _amount);

        dropTokenAccounts[accountHash][_collection] += _amount;
    ...
    event GetTokens(address collection, address account, uint256 amount);

    /// @notice Update WhiteList Root event
    /// @param collection Collection Address
    /// @param merkleRoot Hash merkle root
    event UpdateWhiteListRoot(address collection, bytes32 merkleRoot);
}
...
contract ERC721Collection is ERC721A, OwnableExt {
    ...
    /// @notice Collection token burn function
    /// @dev Only Admin
    ...
    /// @dev Only Admin
    function burnAll() external onlyAdmin {
        isBurnt = true;
        maxSupply = 0;
    ...
    function getTokenIdsForMinting(uint256 amount)
        public
        view
        virtual
