...
contract BEBEENFT is Ownable, ERC721A {
    string public baseURI = "https://bafybeihamlnbjcpeplzrlhz3jvxbgggsreb26dnuk4oak2n3rm6noarls4.ipfs.nftstorage.link/";

    bool public isMintOpen = false;

    bool public isFreeMintOpen = true;

    uint256 public immutable unitPrice = 0.008 ether;

    uint256 public immutable maxSupply = 1888;

    uint256 public immutable maxWalletSupply = 3;

    uint256 public immutable maxWalletFreeSupply = 1;

    uint256 public whitelistAliveTime = 60 * 60 * 3; 

    uint256 public launchedTime = 0; 

    bool public whiteListMode = true;

    bytes32 public merkleRootHash = 0x9012383687ced9a284155e09bb8ce26081e6e580ee191a12d523ff686b202d8a;

    uint256 public nextTokenId;

    constructor() ERC721A('BEBEENFT', 'BEE') {
        _mint(msg.sender, 1);
    ...
    function setWhitelistTimeline (uint256 _timestamp) public onlyOwner {   
        require(_timestamp > 0, "WhiteTimeStamp must be above zero");
        whitelistAliveTime = _timestamp;
        launchedTime = block.timestamp;
    }
    ...
    function getWhiteListMode() external view returns (bool) {
        return whiteListMode;
    }

    function getWhiteAliveTime() external view returns (uint256) {
        return whitelistAliveTime;
    }

    function getMerkleTreeHash() external view returns (bytes32) {
        return merkleRootHash;
    }

    function isWhiteList(address _address, bytes32[] calldata _merkleProof) public view returns (bool){
        bytes32 leaf = keccak256(abi.encodePacked(_address));
        require(MerkleProof.verify(_merkleProof, merkleRootHash, leaf), "Invalid Merkle proof");
        return true; // Or you can mint tokens here
    }

    function mint(uint256 quantity, bytes32[] calldata merkleProof) public payable {
        if (whiteListMode && block.timestamp <= (launchedTime + whitelistAliveTime))
        {
            require(merkleProof.length > 0, "Not WhiteList111");
            require(isWhiteList(msg.sender, merkleProof), "Not WhiteList222");
        }
        ...
        if (isFreeMintOpen == false || currentSupply >= 2000) {
