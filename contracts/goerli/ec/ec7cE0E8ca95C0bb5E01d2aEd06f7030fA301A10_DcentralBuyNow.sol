...
contract DcentralBuyNow is Initializable, ReentrancyGuardUpgradeable, OwnableUpgradeable, EIP712Upgradeable {

    // Variables
    address public proxyWallet; // proxy wallet address
    address payable private feeWallet; // the wallet that receives fees
    uint private feePercent; // the fee percentage on secondary market
    ...
    function initialize(address _proxyWallet, address payable _feeWallet, uint _feePercent, string memory name, string memory version) initializer public {
    ...
    function calculateFee(uint amount, uint percentage)
        ...
        returns (uint)
    {
        require((amount / 10000) * 10000 == amount, "Amount too small to calculate fee");
        return (amount * percentage) / 10000;
    }

    function setProxyWallet(address _proxyWallet) onlyOwner public {
        proxyWallet = _proxyWallet;
    }

    function setFeeWallet(address payable _feeWallet) onlyOwner public {
        feeWallet = _feeWallet;
    }
    ...
    function recoverSigner(OfferItem calldata item) public view returns (address) {
        bytes32 digest = _hashTypedDataV4(keccak256(abi.encode(
            keccak256("OfferItem(string offerId,uint price,string currency,address seller,address nft,uint tokenId,address buyer,uint expiry)"),
    ...
    function purchaseItem(OfferItem calldata signerData) public payable nonReentrant {
        address signer = recoverSigner(signerData);

        require(signer == proxyWallet, "Signer is not a proxy");
