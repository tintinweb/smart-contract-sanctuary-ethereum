...
contract RoyaltiesRegistryStorage is StorageBase, IRoyaltiesRegistryStorage {
    ...
    function getRoyaltiesByTokenAndTokenId(
        address token,
        uint tokenId
    ) external override view returns(LibRoyaltiesV2.RoyaltiesSet memory) {
        return royaltiesByTokenAndTokenId[keccak256(abi.encode(token, tokenId))];
    }
    //
    // royaltiesByTokenAndTokenId setters
    //
    function initializeRoyaltiesByTokenAndTokenId(address token, uint tokenId) external override requireOwner {
    ...
    function deleteRoyaltiesByTokenAndTokenId(address token, uint tokenId) external override requireOwner {
        delete royaltiesByTokenAndTokenId[keccak256(abi.encode(token, tokenId))].royalties;
    }
    //
    // royaltiesByToken getter
    //
    function getRoyaltiesByToken(address token) external override view returns(LibRoyaltiesV2.RoyaltiesSet memory) {
    ...
    //
    function getProviderByToken(address token) external override view returns(address) {
...
contract RoyaltiesRegistry is Ownable, RoyaltiesRegistryAutoProxy, IRoyaltiesRegistry {
    ...
    function setRoyaltiesByToken(
        ...
        require(sumRoyalties < 10000, 'RoyaltiesRegistry: royalties for token cannot be more than 100%');
        ...
        IRoyaltiesRegistryGovernedProxy(proxy).emitRoyaltiesSetForContract(token, royaltiesRecipients, royaltiesAmounts);
    }
    ...
    function setRoyaltiesByTokenAndTokenId(
        ...
        IRoyaltiesRegistryGovernedProxy(proxy).emitRoyaltiesSetForToken(token, tokenId, royaltiesRecipients, royaltiesAmounts);
    }

    // Royalties getter
    function getRoyalties(address token, uint tokenId) external view override returns (LibPartTypes.Part[] memory) {
        // Get royalties from RoyaltiesRegistry storage using token address and token id
        LibRoyaltiesV2.RoyaltiesSet memory royaltiesSet = _storage.getRoyaltiesByTokenAndTokenId(token, tokenId);
        ...
        (bool result, LibPartTypes.Part[] memory resultRoyalties) = providerExtractor(token, tokenId);
        if (result == false) {
    ...
    function royaltiesFromContract(address token, uint tokenId) internal view returns (LibPartTypes.Part[] memory) {

        try IERC165(token).supportsInterface(LibRoyaltiesV2._INTERFACE_ID_ROYALTIES) returns (bool id_royalties_supported) {

            if (id_royalties_supported) {
                IRoyaltiesV2 royaltiesV2 = IRoyaltiesV2(token);
                try royaltiesV2.getRaribleV2Royalties(tokenId) returns (LibPartTypes.Part[] memory res) {
                    return res;
                } catch {}
            } else {
                address payable[] memory recipients;

                try IERC165(token).supportsInterface(LibRoyaltiesV1._INTERFACE_ID_FEE_RECIPIENTS) returns (bool id_fee_recipients_supported) {

                    if (id_fee_recipients_supported) {
                        IRoyaltiesV1 royaltiesV1 = IRoyaltiesV1(token);
                        try royaltiesV1.getFeeRecipients(tokenId) returns (address payable[] memory res) {
                            recipients = res;
                        } catch {
                ...
                try IERC165(token).supportsInterface(LibRoyaltiesV1._INTERFACE_ID_FEE_BPS) returns (bool id_fee_bps_supported) {

                    if (id_fee_bps_supported) {
                        IRoyaltiesV1 royaltiesV1 = IRoyaltiesV1(token);
                        try royaltiesV1.getFeeBps(tokenId) returns (uint[] memory res) {
                            values = res;
                        } catch {
                            return new LibPartTypes.Part[](0);
                        }
    ...
    function providerExtractor(
        address token,
        uint tokenId
    ) internal view returns (bool result, LibPartTypes.Part[] memory royalties) {
        result = false;
        ...
        IRoyaltiesRegistry provider = IRoyaltiesRegistry(providerAddress);
            try provider.getRoyalties(token, tokenId) returns (LibPartTypes.Part[] memory royaltiesByProvider) {
...
interface IRoyaltiesRegistryStorage {
    ...
    function initializeRoyaltiesByTokenAndTokenId(address token, uint tokenId) external;

    function pushRoyaltyByTokenAndTokenId(address token, uint tokenId, LibPartTypes.Part memory royalty) external;

    function deleteRoyaltiesByTokenAndTokenId(address token, uint tokenId) external;
