...
// File sgn-v2-contracts/contracts/safeguard/[email protected]
...
// File sgn-v2-contracts/contracts/message/framework/[email protected]
...
// File contracts/TestSendReceive.sol
...
contract TestSendReceive {
    ...
    event Got(address sender, uint64 srcChainId, address executor, uint64 nonceReceived, uint256 amountReceived, string stringReceived);
    ...
    function sendSomething(address to, uint256 amount, string calldata str) public payable {
        bytes memory message = abi.encode(nonce, amount, str);
        nonce++;

        // uint256 fee = feeBase + message.length * feePerByte;

        MessageSenderLib.sendMessage(to, 97, message, messageBus, msg.value);
    }
    ...
    function executeMessage(
        ...
        bytes calldata _message,
        address _executor
    ) external payable onlyMessageBus returns (IMessageReceiverApp.ExecutionStatus) {
        (uint64 nonceReceived, uint256 amountReceived, string memory stringReceived) = abi.decode(_message, (uint64, uint256, string));
        emit Got(_sender, _srcChainId, _executor, nonceReceived, amountReceived, stringReceived);
        return IMessageReceiverApp.ExecutionStatus.Success;
    }
}