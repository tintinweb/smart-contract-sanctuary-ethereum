...
abstract contract Matrix is IMatrix, Ownable2Step {
	...
	address public API; // Tuktu API contract
	...
	function InitializeMatrix() public notMatrixInitialized {
...
contract XProgram is IXProgram, Matrix {
	...
	modifier onlyAPI(uint _AID, address _Owner) {
		require(msg.sender == address(API), "XProgram: caller is not API");
		require(
	...
	function InitializeXProgram(IAccount _Account, IBalance _Balance, ITuktu _Tuktu) public notXProgramInitialized {
		InitializeMatrix();
		Accounts = _Account;
		Balances = _Balance;
		Tuktus = _Tuktu;
