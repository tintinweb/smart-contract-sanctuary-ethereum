...
contract Marketplace is ReentrancyGuard, MkpOperations, IMarketplace {
    address private marketplaceManager;
    ...
    function createMarketItem(
        ...
        Signature.verifyMessage(
            Signature.hashingInsertMessage(nftContract, tokenId, price),
            nonce,
            timestamp,
            signatures
        );
    ...
    function insertMarketItem(
        ...
        require(
            msg.sender == marketplaceManager,
            "It can only be inserted directly to the marketplace from the manager"
        );
        _insertMarketItem(nftContract, address(this), tokenId, price);
    }

    function insertMarketItemSignature(
        address nftContract,
        uint256 tokenId,
        uint256 price,
        uint256 nonce,
...
interface IMarketplace{
    function createMarketItem(address nftContract, uint256 tokenId, uint256 price, uint256 nonce, uint256 timestamp, bytes[] memory signatures) external;
...
import "../Security/interfaces/IECDSASignatureMarketplace.sol";

contract MkpOperations is Tax {
    ...
    uint private isPreSale;

    IECDSASignatureMarketplace internal Signature;
    ...
    struct MarketItem {
        ...
        uint256 price;
        bool sold;
        bytes32 typeContract;
        bytes32 typeNameContract;
    }

    mapping (uint256 => uint256) private idToItemsSold;
    mapping (uint256 => MarketItem) private idToMarketItem;
    mapping (bytes32 => uint256) private artifactPriceBase;
    

    event MarketItemSold(uint256 indexed itemId, address owner, uint256 salePrice);
    event WeAreLive(string message);
    event DistributionResult(uint256 amount, string message);
    event ChangedPriceBase(uint256 newPrice, bytes32 typeNameContract);
    ...
    constructor(
        ...
        address _aggAddress,
        address _signatureCollateral
    ) Tax(_wosStakingAddress, _collateral, _aggAddress, _signatureCollateral) {
        distributor = IDistributor(_distributorAddress);
        Signature = IECDSASignatureMarketplace(_signature);
        cruToken = _cruToken;
        exchangeAddress = _exchangeAddress;
        isPreSale = 1;
        artifactPriceBase[keccak256("PotionItem")] = raise(100);   //100 USDT
        artifactPriceBase[keccak256("DerrickItem")] = raise(100);  //100 USDT
    }
    ...
    function _insertMarketItem(
        ...
        bytes32 typeContract = IContractType(nftContract).getTypeContract();
        bytes32 typeNameContract = IContractType(nftContract).getTypeNameContract();
    ...
    function _marketSale(address nftContract, uint256 itemId) internal preSale(nftContract) {
        uint256 price = idToMarketItem[itemId].price;
        uint256 tokenId = idToMarketItem[itemId].tokenId;
        bool sold = idToMarketItem[itemId].sold;
        address seller = idToMarketItem[itemId].seller;
        address nftContractAddress = idToMarketItem[itemId].nftContract;
        bytes32 typeNameContract = idToMarketItem[itemId].typeNameContract;
        bytes32 typeContract = idToMarketItem[itemId].typeContract;

        require(nftContractAddress == nftContract, "The address of the contract, of the nft to buy is not valid");
        require(tokenId > 0, "token does not exist");
        require(sold != true, "This Sale has alredy finnished");

        if(typeContract == keccak256("artifact") && seller == address(this)) {
            price = getFinalSalePrice(nftContract, tokenId, typeNameContract);
        }
        ...
        if (typeContract == keccak256("well") && seller == address(this)) {
            foundExchange(nftContract, tokenId);
        }

        if (typeContract == keccak256("artifact") && seller == address(this)) {
            distribute(_getPriceWithoutTax(price));
        }
    ...
    function getFinalSalePrice(address nftContract, uint256 artifactId, bytes32 typeNameContract) private view returns (uint256) {
        uint256 priceBase = artifactPriceBase[typeNameContract];
        require(priceBase > 0, "The base price of the artifact must be greater than zero");

        uint priceModifier = IArtifact(nftContract).getPriceModifier(artifactId);
        if(priceModifier == 0) return priceBase;

        return priceBase + ((priceModifier * priceBase) / 100);
    }

    function changePriceBase(
        uint256 price, 
        bytes32 typeNameContract,
        uint256 nonce,
        uint256 timestamp,
        ...
        Signature.verifyMessage(
            Signature.hashingChangePriceBase(price, typeNameContract),
            nonce,
            timestamp,
            signatures
        );

        require(price >= raise(1), "The minimum price is 1 USDT");
        artifactPriceBase[typeNameContract] = price;
        emit ChangedPriceBase(price, typeNameContract);
    }

    function distribute(uint256 amount) private {
        bool result = distributor.distributeFunds(amount + distributionPool);

        if(result == false) {
            emit DistributionResult(
                amount, 
                "The collateral could not be distributed to the supply of wells, the collateral will be saved until a new distribution"
            );
            distributionPool += amount;
            return;
        }

        _collateralTransfer(exchangeAddress, amount + distributionPool);

        emit DistributionResult(
            (amount + distributionPool), 
            "Collateral was successfully distributed"
        );
    ...
    function goLive() external onlyOwner {
        isPreSale = 0;
        emit WeAreLive("Wos game, Hello world");
    }

    function setPresaleState(uint state) external onlyOwner {
        isPreSale = state;
    }

    modifier preSale(address nftContract) {
        bytes32 typeContract = IContractType(nftContract).getTypeContract();

        if (isPreSale == 1) {
            revert("Wos is on presale time, wait until go live");
        } 
        else if (isPreSale == 2) {
            require(
                typeContract == keccak256("artifact"),
                "It is only allowed to buy potions in presales stage 2"
            );
        } 
        else if (isPreSale == 3) {
            require(
                typeContract == keccak256("well"),
                "It is only allowed to buy oilWell in presales stage 3"
            );
        }
...
interface IArtifact {
    function createItem(address owner, string memory tokenUri,  uint256 typeId, uint256 nonce, uint256 timestamp, bytes[] memory signatures) external returns (uint256);
    function getArtifactType(uint256 artifactId) external view returns (uint256);
    function getPriceModifier(uint256 artifactId) external view returns (uint);
}
