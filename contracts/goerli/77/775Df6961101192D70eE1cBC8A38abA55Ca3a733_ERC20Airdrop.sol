...
// import {ERC20PresetMinterPauser} from "@openzeppelin/contracts/access/ERC20PresetMinterPauser.sol";
...
contract ERC20Airdrop is Initializable, ERC20Upgradeable, ERC20BurnableUpgradeable, PausableUpgradeable, AccessControlUpgradeable, NonReentrant {

    bytes32 public constant PAUSER_ROLE = keccak256("PAUSER_ROLE");
    bytes32 public constant MINTER_ROLE = keccak256("MINTER_ROLE");
    bytes32 public constant OWNER_ROLE = keccak256("OWNER_ROLE");
    
    // Signs ERC20 airdrop rewards claims
    address public airdropService;
   
    string public _name;
    string public _symbol;

    // ERC20 airdrops lastClaimNonce mappings are stored by airdropId
    mapping(bytes4 => mapping(address => uint256)) public airdropLastClaimNonce;
    ...
    function initialize(address default_admin, address pauser, address minter, address owner) initializer public {
        __ERC20_init("DragonBlood", "DB");
    ...
    // Setter functions
    //
 function setName(string calldata name) external onlyRole(OWNER_ROLE){
       _name = name;
    }

    function setSymbol(string calldata symbol) external onlyRole(OWNER_ROLE){
        _symbol = symbol;
    }

    function setAirdropService(address _airdropService) external onlyRole(OWNER_ROLE){
    ...
    function claimAirdrop(
        ...
        require(
            lastClaimNonce == airdropLastClaimNonce[airdropId][recipient],
        ...
        LibClaimAirdrop.validateClaim(
            ...
            airdropService // Airdrop service address
        );
        // Update recipient's last claim nonce to current claim nonce
        airdropLastClaimNonce[airdropId][recipient] = claimNonce;
    ...
    function mintAirdropClaim(
        ...
        uint256 totalClaimAmount = claimAmountAirdrop + claimAmountReferral1+claimAmountReferral2+claimAmountReferral3;
        // Mint total claim amount to recipient
        _mint(recipient, totalClaimAmount);
        // Emit Transfer event
       // emitTransfer(address(0x0), recipient, totalClaimAmount);
    }
}
