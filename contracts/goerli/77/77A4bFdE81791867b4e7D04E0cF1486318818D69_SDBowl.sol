...
contract SDBowl is ERC721A, ERC2981, Ownable,DefaultOperatorFilterer {
    bytes32 public merkleRootWL=0xcf274e4d438fc256aa9c4f206c00ad908b7ae4bc1081da5004db51e4413c3a1e;
    bytes32 public merkleRootDogList=0xaeb433ff4c2ec856cfa7b58d4e489dc4c8166732d5de4e51c54ae0d693430fc4;
    ...
    uint256 public MAX_MINT_DOGLIST = 2;
    ...
    function dogListMint(uint256 numberOfTokens,bytes32[] calldata _merkleProof) external payable{
        ...
        require(doglistMintedCount[msg.sender]+numberOfTokens <= MAX_MINT_DOGLIST,"Exceeded DOGLIST Allocation");
        bytes32 leaf=keccak256(abi.encodePacked(msg.sender));
        require(MerkleProof.verifyCalldata(_merkleProof,merkleRootDogList,leaf), "Invalid Proof for DogList");
        uint256 minterToBeAmount=doglistMintedCount[msg.sender]+numberOfTokens;
        if(minterToBeAmount>MAX_MINT_FREE)
        {
            uint256 numbertopay=numberOfTokens;
               if(doglistMintedCount[msg.sender]==0)
               {
                     numbertopay=numbertopay-MAX_MINT_FREE;
               }
               require(msg.value == (numbertopay * mintPriceWL), "Incorrect amount of ETH sent");

            _basicMint(msg.sender, numberOfTokens);
            doglistMintedCount[msg.sender] +=numberOfTokens;
        }
        else if(minterToBeAmount==MAX_MINT_FREE)
        {
            _basicMint(msg.sender, numberOfTokens);
            doglistMintedCount[msg.sender] +=numberOfTokens;
        }
