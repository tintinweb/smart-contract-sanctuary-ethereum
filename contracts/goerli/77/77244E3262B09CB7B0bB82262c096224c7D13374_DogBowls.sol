...
contract DogBowls is Ownable, ERC721A, ReentrancyGuard {

    string public baseURI;

    uint256 public teamDog = 51;
    ...
    uint256 public maxDBSupply = 100;
    ...
    mapping (address => bool) public mintedDBforPublic;
    mapping(address => uint256) addressBlockBought;

    constructor(
        ) 
        ERC721A("Dog Bowl", "DOGBOWLS")  {
    }
    ...
    function mintPublic(
        ...
        require(totalSupply() + maxPublicMint <= maxDBSupply, "EXCEEDS_MAX_SUPPLY" );
        require(!mintedDBforPublic[msg.sender], "EXCEEDS_MAX_PUBLIC_MINT" );
        
        addressBlockBought[msg.sender] = block.timestamp;
        mintedDBforPublic[msg.sender] = true;
        _safeMint(msg.sender, numberOfTokens);
    }

    /**
     * Mint DOGS for the Team
     */
    function mintDBForTeam(uint256 numberOfTokens) external onlyOwner {
        require(teamDog > 0, "NFTS_FOR_THE_TEAM_HAS_BEEN_MINTED");
        require(numberOfTokens <= teamDog, "EXCEEDS_MAX_MINT_FOR_TEAM");

        teamDog -= numberOfTokens;
        _safeMint(msg.sender, numberOfTokens);
    }

    /**
     * Returns Dog bowls of the Caller
     */
    // TODO: Find out/ declare function tokenOfOwnerByIndex
