  ...
  OwnableUpgradeable
{
  ...
  function uri(uint256 tokenId) public view virtual override returns (string memory) {
    return bytes(_baseTokenURI).length != 0 ? string(abi.encodePacked(_baseTokenURI, _toString(tokenId))) : "";
  }
   ...
   * @return A boolean that indicates if the operation was successful.
   */
  function _setBaseURI(string memory newURI) internal returns(bool)
  {
   ...
   * @param tokenIds the list of token to mint
   * @param amounts the list of amount to mint for each tokenId
