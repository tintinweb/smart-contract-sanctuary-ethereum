...
contract AO2023MinterDev is Ownable2Step, IMinter, VRFConsumerBase {
  ...
  function claim(
    ...
    bytes32 message = keccak256(abi.encodePacked(msg.sender, _encodedParams));
    ...
    require(claimedInfo[methodId].claimed + _amount <= maxSupply, "Exceed max supply");
  ...
  {
    (methodId, maxPerWallet, maxPerTxn, maxSupply, startTime, endTime, price) = abi.decode(_encodedParams, (uint8, uint16, uint16, uint32, uint64, uint64, uint256));
  }
  ...
  function tokenURI(uint256 _tokenId) public view virtual override returns (string memory) {
    ...
    if (!revealed) {
      return "ipfs://QmUMeAJdjrZYcaLbCRDF35MBXa1LdEPpt6RziwhTAbpgA4"; // TBC
    }
  ...
  function withdraw() public onlyOwner {
    uint256 balanceA = (address(this).balance * 75) / 1000;
    uint256 balanceB = (address(this).balance * 400) / 1000;
    uint256 balanceC = (address(this).balance * 200) / 1000;
    uint256 balanceD = address(this).balance - balanceA - balanceB - balanceC;
