 ...
 */
contract ChainpassTickets_1_0 is ERC721iEnumerable, Initializable, AuthHelper, ReentrancyGuard {
    ...
    function issueTickets(
        uint256 amount,
        uint256 price,
        MetaTransactionInput memory metaTxInput
    ) external {
        require(
            isMetaChainpassWallet(
                MetaTransaction(metaTxInput, abi.encode("issueTickets", amount, price))
            ),
            "Only the Chainpass wallet can issue tickets."
        );
    ...
    function transferMetaTx(
        ...
        require(
            isSignerOrCaller(owner, MetaTransaction(metaTxInput, abi.encode("transferMetaTx", to, tokenId))),
    ...
    function setRoyalty(Royalty memory _royalty, MetaTransactionInput memory metaTxInput) external {
        require(
            isMetaOwner(
                MetaTransaction(metaTxInput, abi.encode("setRoyalty", _royalty))
            ),
            "Only the contract owner can call setRoyalty."
        );
     ...
     *  Context
     */
    function _msgSender() override(Context, ContextUpgradeable) internal view returns (address) {
        return msg.sender;
    }

    function _msgData() override(Context, ContextUpgradeable) internal pure returns (bytes calldata) {
        return msg.data;
    }
}
