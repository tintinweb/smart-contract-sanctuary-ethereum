...
contract Shuffle {
    ...
    event ShuffledList(address[] list);
     ...
     * @return shuffled
     */
    function shuffleList(address[] memory list, uint256 seed) public payable returns (address[] memory) {
        uint256 n = list.length;
        address[] memory shuffled = new address[](n);

        if (msg.value > 0) {
            // record the value sent to the address that called the function
            balances[msg.sender] += msg.value;
        }
        ...
        // store the shuffled list as a variable
        addresses = shuffled;

        // emit an event with the shuffled list
        emit ShuffledList(shuffled);
