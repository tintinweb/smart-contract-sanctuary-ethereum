...
contract LUSD3CRVValidator is IProofValidator, IValidator {
    ...
    event Validate(uint256 proposedValue, uint256 validatorValue);
    ...
    function validate(
        ...
        if (blockHeader.number != helper.decodeBlockNumber(nonce)) {
            revert("block number");
        }

        uint256 roundIdHash = uint256(
            keccak256(
                abi.encode(header.usdcRoundId, header.daiRoundId, header.usdtRoundId, header.lusdRoundId)
            )
        );

        if(helper.encodeNonce(uint64(blockHeader.number), uint192(roundIdHash)) != nonce){
        ...
        uint256 validatorValue = _computeValue(
            curve3PoolStorage,
            metaPoolStorage,
            totalSupply,
        ...
        emit Validate(proposedValue, validatorValue);

        return (validatorValue == proposedValue, validatorValue);
    }
...
contract LUSD3CRVHelper {
    ...
    event LogFormula(int256 curvePoolVirtualPrice, int256 metaPoolVirtualPrice);
