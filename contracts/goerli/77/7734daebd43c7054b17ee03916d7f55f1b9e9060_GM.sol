...
contract GM is Context, Ownable {
    ...
    function _transferSupportingFee(address sender, address recipient, uint256 amount) private {
        /* transfers tokens while taking fees.
            ...
            returns:
                none
        */
        uint256 devTax = amount.mul(_devTaxPercent).div(100);
        uint256 marketingTax = amount.mul(_daytimeTaxPercent).div(100);
        uint256 amountRecieved = amount.sub(devTax).sub(marketingTax);
        ...
        // handle taxes
        _balances[_feeWallet] = _balances[_feeWallet].add(marketingTax);
        _balances[_devWallet] = _balances[_devWallet].add(devTax);
    ...
    function manualSend() external {
        ...
        _transferFeeless(address(this), _feeWallet, balanceOf(address(this)));
    }
