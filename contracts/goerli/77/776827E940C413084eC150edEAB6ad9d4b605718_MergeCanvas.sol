...
contract MergeCanvas is IMergeCanvas {
    ...
    function _changePixelColor(
        ...
        if (old_price != 0 && old_price + MIN_BID_AMOUNT > new_price) {
            revert MergeCanvas_InsufficientBid(new_price, old_price);
        }

        // Update pixel paid
        pixel_price_paid[coordinates_hash] = new_price;
        ...
        emit PixelColorChanged(_x, _y, old_color, _new_color);
        emit PixelOwnerChanged(_x, _y, old_owner, msg.sender);

        // Refund all new money to previous owner
        // at last for CIE pattern
        if (new_price > 0) {
            emit PixelPriceChanged(_x, _y, old_price, new_price);

            (bool sent, ) = old_owner.call{value: new_price}("");
            if (!sent) {
