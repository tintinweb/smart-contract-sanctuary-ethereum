...
abstract contract LottoDAO is LottoGratuity {
    ...
    function _accEth(address account) private view returns (uint256 eth) {
        ...
        // iterate through lotteries until the first transaction's blocknumber is < winning history (find the lottery that happens after first trans)
        // need to add structs to binarys search or something
        uint256 startingPoint = safeBinarySearchUpExclusive(
            winningHistory,
            transactions[0].blockNumber
        );
        // safeBinarySearchOver(uint256[] memory _array, uint256 _target) -> _array = winningHistory and _target = first transaction
        // iterate through transactions until the first transaction's blocknumber is > winning history (find the trans that is closest to before lottery start)
        for (uint256 i = startingPoint; i < len; ++i) {
            safeBinarySearchDownExclusive(transactions, i);
            // safeBinarySearchUnder(uint256[] memory _array, uint256 _target) -> _array = transactions and _target = lottery start
            // add whatever to eth
            eth += winningHistory[i].feeAmount.mulDiv(
                transactions[safeBinarySearchDownExclusive(transactions, i)]
                    .snapBalance,
                winningHistory[i].totalStakedSupply
            );
...
contract LottoTickets {
    ...
    function _findTicketOwner(
        ...
        unchecked {
            ...
            uint256 mid = (low + high) / 2; // Calculate the middle index of the search range as the average of low and high
