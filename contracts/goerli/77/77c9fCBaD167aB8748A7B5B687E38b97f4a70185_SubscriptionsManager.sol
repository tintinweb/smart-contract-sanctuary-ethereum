...
contract SubscriptionsManager is Ownable, ReentrancyGuard, ISubscriptionsManager {
  ...
  /// @param _ss Super subscription contract address
  /// @param _s Normal Subscroption contract address
  constructor(address _ss, address _s) {
    SS_ADDRESS = _ss;
    S_ADDRESS = _s;
    setSubPrice(SubType.NORMAL, 99900);
    setSubPrice(SubType.SUPER, 199900);
  }
  ...
  function _subscribe(address to, SubType subType) internal {
    ...
    if (subType == SubType.SUPER) {
      ISubscriptions(SS_ADDRESS).subscribe(to);
    } else if (subType == SubType.NORMAL) {
      ISubscriptions(S_ADDRESS).subscribe(to);
    } else {
  ...
  function isSubscribed(
    ...
    return
      ISubscriptions(SS_ADDRESS).isSubscribed(target) ||
      ISubscriptions(S_ADDRESS).isSubscribed(target);
  }

  function whichType(address target) public view returns (SubType) {
    if (ISubscriptions(SS_ADDRESS).isSubscribed(target)) return SubType.SUPER;
    else if (ISubscriptions(S_ADDRESS).isSubscribed(target))
  ...
  function subscriptionInfo(
    ...
    ISubscriptions superSubs = ISubscriptions(SS_ADDRESS);
    ISubscriptions normalSubs = ISubscriptions(S_ADDRESS);
