...
interface IListing {
    ...
    event PostingCreated(uint256 tokenId, address posting, string baseURI);
    event PostingTemplateUpdated(address oldTemplate, address newTemplate);
    ...
    function getPosting(uint256 tokenId) external view returns (address);

    function postingTemplate() external view returns (address);
    ...
    function createPosting(uint256 tokenId, string memory baseURI) external;

    function updatePosting(uint256 tokenId, string memory baseURI) external;

    function updatePostingTemplate(address newTemplate) external;
}
    ...
    Ownable
{
    ...
    // tokenId => posting contract address
    mapping(uint256 => address) public override getPosting;
    address public override postingTemplate;
    uint256 public override nodeCount;

    constructor(address postingTemplate_) ERC6150("Listing", "LIST") {
        postingTemplate = postingTemplate_;
    }
    ...
    function safeBurn(uint256 tokenId) public override {
        require(getPosting[tokenId] == address(0), "posting exists");
    ...
    function createPosting(
        ...
        require(isLeaf(tokenId), "only leaf");
        address posting = Clones.clone(postingTemplate);
        IPosting(posting).initialize(baseURI);
        getPosting[tokenId] = posting;
        emit PostingCreated(tokenId, posting, baseURI);
    }
    ...
    function updatePosting(
        ...
        address oldPosting = getPosting[tokenId];
        require(oldPosting != address(0), "posting not exist");
        require(isLeaf(tokenId), "only leaf");
        address newPosting = Clones.clone(postingTemplate);
        IPosting(newPosting).initialize(baseURI);
        getPosting[tokenId] = newPosting;
        emit PostingUpdated(tokenId, oldPosting, newPosting, baseURI);
    }

    function updatePostingTemplate(
        address newTemplate
    ) external override onlyOwner {
        emit PostingTemplateUpdated(postingTemplate, newTemplate);
        postingTemplate = newTemplate;
    }
