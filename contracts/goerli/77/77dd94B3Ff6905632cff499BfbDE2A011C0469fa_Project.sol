...
contract Project is IProject, ContextUpgradeable, ReentrancyGuardUpgradeable {
     ...
     * @param _saleInputs.priceDecrementAmt   -> Price decrement amt for dutch auction (default 0 if sale by Fixed type)
     */
    function addSales(uint256 _projectId, uint256 _minSales, SaleInput[] memory _saleInputs) external projectIsValid(_projectId) onlyManager(_projectId) {
        ...
        for (uint256 i = 0; i < _saleInputs.length; i++) {
            ...
            bool isMint = _saleInputs[i].tokenId == 0;
            bool isSetRoyalty = isMint && _saleInputs[i].royaltyReceiver != address(0);

            projectInfo.amount += _saleInputs[i].amount;
            saleIds[i] = sale.createSale(_msgSender(), isMint, isSetRoyalty, projectInfo, _saleInputs[i]);
        }
