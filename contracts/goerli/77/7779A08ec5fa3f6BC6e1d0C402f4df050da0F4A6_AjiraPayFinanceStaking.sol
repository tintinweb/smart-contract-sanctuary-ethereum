...
contract AjiraPayFinanceStaking is Ownable, AccessControl, ReentrancyGuard{
    ...
    event Stake(uint256 stakeId, address indexed beneficiary, uint256 indexed stakedAmount, uint256 indexed stakeDeadLine, uint256 stakedAt);
    event StakingTierAdded(address indexed caller, uint256 indexed tierId, uint256 indexed timeInTimestamp);
    ...
    function stake(uint256 _amount, uint256 _stakingTierId) external nonReentrant {
        ...
        require(_amount <= holderTokenBalance,"AJP Staking: Insufficient balance for staking");

        StakeTier memory _stakingTier = stakingTierById[_stakingTierId];

        stakeIds.increment();
        uint256 _currentStakeId = stakeIds.current();
        NewStake memory newStake = NewStake(
                _currentStakeId, 
                _amount, 
                block.timestamp, 
                _stakingTier.deadline,
                _stakingTier.apr, 
                payable(msg.sender), 
                payable(msg.sender)
            );
        ...
        NewStake memory _stakeItem = stakingItemById[_currentStakeId];
        emit Stake(_currentStakeId, _stakeItem.beneficiary, _amount, _stakeItem.stakeDuration, block.timestamp);
    }

    function stakeFor(address _beneficiary, uint256 _amount, uint _stakingTierId) external nonReentrant{

    }

    function withdrawStake(uint256 _amount, uint256 _stakingTierId) external nonReentrant{
        StakeTier memory _tier = stakingTierById[_stakingTierId];
        uint256 tierApr = _tier.apr;
        uint256 deadline = _tier.deadline;
    }
    ...
    function addStakingTier(uint56 _apr, uint256 _deadlineInDays) external onlyRole(MANAGER_ROLE) nonReentrant{
        require(_apr != 0,"AJP Staking: Invalid Tier Type");
        ...
        StakeTier memory newStakingTier = StakeTier(currentTierId, _apr, deadlineInDays, block.timestamp, msg.sender);
        emit StakingTierAdded(msg.sender, currentTierId, block.timestamp);
    }

    receive() external payable{
        revert("Cannot Accept external BNB");
    }
    ...
    function _recordNewStake(uint256 _id, uint256 _amount, uint256 _timestamp, uint256 _duration, uint256 _apr, 
        address payable _holder, address payable _beneficiary) private pure{
        NewStake memory _stake = NewStake(
                _id, 
                _amount, 
                _timestamp, 
                _duration,
