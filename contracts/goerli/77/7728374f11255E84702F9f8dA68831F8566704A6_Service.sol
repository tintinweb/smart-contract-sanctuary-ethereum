    ...
    IService
{
    ...
    function initialize(
        ...
        require(
            ...
            "Invalid ProposalGateway address"
        );
        ...
        require(
            ...
            "Invalid UniswapRouter address"
        );
        require(
            address(uniswapQuoter_) != address(0),
            "Invalid UniswapQuoter address"
        );
        require(
            address(whitelistedTokens_) != address(0),
            "Invalid WhitelistedTokens address"
        );
    ...
    function setProtocolTreasury(address _protocolTreasury) public onlyOwner {
        require(_protocolTreasury != address(0), "Invalid _protocolTreasury");
    ...
    function setProtocolTokenFee(uint256 _protocolTokenFee) public onlyOwner {
        require(_protocolTokenFee <= 1000000, "Invalid _protocolTokenFee");
    ...
    function getProtocolTokenFee(uint256 amount) public view returns (uint256) {
        ...
        return (((protocolTokenFee * 1000000) / 1000000) * amount) / 1000000;
    }
     ...
     */
    function getMaxHardCap2(address _pool) public view returns (uint256) {
        try IPool(_pool).isDAO() returns (bool isDAO) {
            if (isDAO) {
                return
    ...
    function getMaxHardCap3() public view returns (uint256) {
        return type(uint256).max - getProtocolTokenFee(type(uint256).max);
    }

    function getMaxHardCap4(address _pool) public view returns (bool) {
        return IPool(_pool).isDAO();
    }
