...
contract Lottery is VRFConsumerBaseV2, KeeperCompatibleInterface {
    ...
    // TODO: add 'uint256 private immutable i_intervalWithdraw' to automate OPEN_TO_WITHDRAW -> OPEN_TO_PLAY switch
    ...
    constructor(
        ...
        uint256 _interval,
        // TODO: add 'uint256 _intervalWithdraw', see fulfillRandomWords()
     ...
     * note: this call contains 2 calls from front-end:
     * 1. Player calls USDC to transfer i_lotteryTicketPrice USDC => Lottery
     * 2. Player calls Lottery to send i_lotteryFee ETH value => Lottery
    ...
    function enterLottery() public payable {
        ...
        lotteryToken.transfer(msg.sender, 1);
    ...
    /**
     * note: 2 possible internal calls:
     * both internal calls Approve Compound for all current Lottery USDC balance
     * 1. if call from Player enterLottery()
     * - if 1st Player => all current Lottery USDC balance --> supply Compound
     * - else => 1 Ticket Price USDC --> supply Compound
     * 2. if call from Admin adminPerformMyUpkeep()
    ...
    function _approveAndSupplyCompound() internal {
        ...
        if (isFirstPlayer || (msg.sender == i_owner)) {
            // if call from First Player or from Admin
            ...
            amountToSupply = lotteryUSDCBalance; // includes
            if (isFirstPlayer) {
                isFirstPlayer = false;
            }
        } else {
     ...
     * 1. Player call LotteryToken => transfer Player's LTK to Lottery
    ...
    function withdrawFromLottery() public {
        ...
        // transfer Player's LTK => Lottery
        lotteryToken.transfer(address(this), playerNumTickets);
        // set Player due USDC
        uint256 amountDueToPlayer = getUSDCAmountDueToPlayer(msg.sender);
        ...
        )
    {
        ...
        upkeepNeeded = (isOPEN_TO_PLAY && timePassed && hasPlayer && isFunded);
    }
    ...
    function fulfillRandomWords(
        ...
        uint256 lotteryCurrentUSDCBalance = _getLotteryUSDCBalance(); // with interests
        // check if GAINS > 0
        ...
        // note: next OPEN_TO_WITHDRAW -> OPEN_TO_PLAY switch will be performed by Admin
        // TODO: set a time-based ChainLink Keeper to automate OPEN_TO_WITHDRAW -> OPEN_TO_PLAY switch after i_intervalWithdraw has passed.
     ...
     * --- time-based performUpkeep => LotteryState.OPEN_TO_PLAY & ApproveAndSupplyCompound()
     */
     ...
     * /!\ transfers Lottery USDC to Admin
     */
    function __AdminWithdrawLotteryUSDC() external onlyOwner {
        uint256 lotteryUSDCBalance = _getLotteryUSDCBalance();
