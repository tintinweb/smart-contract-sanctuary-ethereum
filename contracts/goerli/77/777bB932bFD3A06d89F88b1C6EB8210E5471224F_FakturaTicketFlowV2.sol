 ...
 */
contract FakturaTicketFlowV2 is
TreasuryNode,
ERC721AUpgradeable,
OwnableUpgradeable,
...
UUPSUpgradeable
{
    struct DutchAuction {
        ...
        uint256 decreaseSize;       // ** be used for slope(decrease percent with 6 decimals)  ex: 99000001 = 99.000001% **
        uint256 numDecreases;       // ** be used for floorPrice **
    }

    struct Sales {
        uint256 salesType;      // 1: Fixed, 2: Dutch Auction
        uint256 startDate;
        uint256 endDate;
        uint256 mintPrice;
        uint256 maxPool;
    ...
    function getPrice(uint8 n, uint256 sales) public view returns (uint256) {
        Sales memory _salesPhase = salesPhase[sales];

        if (_salesPhase.salesType == 1) {
            return n * _salesPhase.mintPrice;
        } else if(_salesPhase.salesType == 2) {
            uint256 numDecreases = 0;

            if (block.timestamp < _salesPhase.startDate) {
                return 0;
            } else if (block.timestamp < _salesPhase.endDate) {
                numDecreases = (block.timestamp - _salesPhase.startDate) / _salesPhase.auction.decreaseInterval;
            } else if (salesPhaseRestart[sales] == 1) {
                uint256 auctionPeriod = (_salesPhase.endDate - _salesPhase.startDate);
                numDecreases = ((block.timestamp - _salesPhase.startDate) % auctionPeriod) / _salesPhase.auction.decreaseInterval;
            } else {
                return 0;
            }

            uint256 _price = _salesPhase.mintPrice;
            for (uint i = 0; i < numDecreases; i ++) {
                _price = _price * _salesPhase.auction.decreaseSize / (10 ** 8);
            }

            return n * MathUpgradeable.max(_price, _salesPhase.auction.numDecreases);
        }
    ...
    function getSales(uint256 sales) public view returns (Sales memory) {
        ...
        if (_salesPhase.salesType == 2 && block.timestamp >= _salesPhase.endDate && salesPhaseRestart[sales] == 1) {
            uint256 auctionPeriod = (_salesPhase.endDate - _salesPhase.startDate);
            uint256 nRepeated = (block.timestamp - _salesPhase.startDate) / auctionPeriod;
            _salesPhase.startDate = _salesPhase.startDate + auctionPeriod * nRepeated;
            _salesPhase.endDate = _salesPhase.startDate + auctionPeriod;
        }
    ...
    function safeMint(address to, uint8 amount, uint256 sales, uint256 category, bytes32 r, bytes32 s, uint8 v) public payable whenNotPaused {
        require(
            salesPhase[sales].salesType != 0,
            "SalesPhase not exist."
        );
        require(
            salesPhase[sales].startDate <= block.timestamp,
            "SalesPhase not started."
        );
        require(
            salesPhase[sales].endDate > block.timestamp || 
            salesPhase[sales].endDate == 0 ||
            salesPhaseRestart[sales] == 1,
            "SalesPhase finished."
        );
        require(
            _totalMinted() + amount <= expectedTokenSupply - mintReserve,
            "Collection soldout."
        );
        require(
            _totalMintedBySales(sales) + amount <= salesPhase[sales].maxPool || 
            salesPhase[sales].maxPool == 0,
            "SalesPhase soldout."
        );
        require(
            _totalMintedByAddress(to, sales) + amount <= salesPhase[sales].maxPerWallet || 
            salesPhase[sales].maxPerWallet == 0,
            "maxPerWallet limit."
        );
        require(
            msg.value >= getPrice(amount, sales),
    ...
    function setSalesPhase(Sales memory _salesPhase, uint8 fRestart, uint8 position) public onlyRole(ADMIN_ROLE) {
        salesPhase[position] = _salesPhase;
        salesPhaseRestart[position] = fRestart;
    }

    function setSalesPhaseRestart(uint256 sales, uint8 fRestart) public onlyRole(ADMIN_ROLE) {
        salesPhaseRestart[sales] = fRestart;
    }

    function setTokenBaseURI(string memory _baseURI) public onlyRole(ADMIN_ROLE) {
    ...
    mapping(uint256 => uint8) public salesPhaseRestart;
}
