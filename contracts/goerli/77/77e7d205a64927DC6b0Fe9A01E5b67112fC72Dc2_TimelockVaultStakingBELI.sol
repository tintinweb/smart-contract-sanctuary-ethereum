...
contract TimelockVaultStakingBELI is TimelockedVault {
    event Earned(
        uint256 oldWantLockedTotal,
        uint256 newWantLockedTotal
    );
    ...
    constructor(
        ...
        earnToBeliRate = 0;
        withdrawFeeFactor = 5000; // 50%
        minTimeToWithdraw = 15552000; // 180 days
    ...
    function earn() public override nonReentrant whenNotPaused {
        ...
        // earnedAmt = addTreasury(earnedAmt);   // No take to treasury
        ...
        wantLockedTotal = IERC20(BELIAddress).balanceOf(address(this));
...
abstract contract Vault is Ownable, ReentrancyGuard, Pausable, ERC20 {
    ...
    function setLastDepositTimestamp(
        address _userAddress
    ) external virtual onlyOwner {}

    function __farm(uint256 wantAmt) internal virtual {
        if (isCAKEStaking) {
    ...
    function convertEarnToBeli(uint256 _amount) internal returns (uint256) {
        ...
        accEarnBELIPerShare = accEarnBELIPerShare.add(
            reward.mul(1e12).div(sharesTotal.sub(totalSupply()))
        );
    ...
    function burnReceiptForShare(
        address _userAddress,
        uint256 _amount) public virtual onlyOwner {
...
abstract contract TimelockedVault is Vault {
    address[] public users;
    mapping(address => uint256) public userLastDepositedTimestamp;
    uint256 public minTimeToWithdraw = 31536000; // 1 year
    uint256 public minTimeToWithdrawUL = 31536000; // 1 year (365 days)
    ...
    event PenaltyAddressChanged(
        address indexed oldPenaltyAddress,
        address indexed newPenaltyAddress
    );
        ...
        returns (uint256)
    {
        ...
        sharesTotal = sharesTotal.sub(sharesRemoved);
        uint256 removedWant = _wantAmt;

        uint256 wantAmt = IERC20(wantAddress).balanceOf(address(this));
        if (removedWant > wantAmt) {
            removedWant = wantAmt;
        }

        if (wantLockedTotal < removedWant) {
            removedWant = wantLockedTotal;
        }

        wantLockedTotal = wantLockedTotal.sub(removedWant);
        uint256 earlyWithdrawalPenalty = 0;

        if (withdrawFeeFactor < withdrawFeeFactorMax) {
            earlyWithdrawalPenalty = removedWant.mul(
                    withdrawFeeFactorMax.sub(withdrawFeeFactor)
                ).div(withdrawFeeFactorMax);

            removedWant = removedWant.sub(earlyWithdrawalPenalty);
        }
        IERC20(wantAddress).safeTransfer(beliFarmAddress, removedWant);
        IERC20(wantAddress).safeTransfer(penaltyAddress, earlyWithdrawalPenalty);

        return sharesRemoved;
    }

    function setPenaltyAddress(address _newPenaltyAddress)
        external
        onlyAllowGov
    {
        require(_newPenaltyAddress != address(0), "penalty address must not be address zero");
        emit PenaltyAddressChanged(penaltyAddress, _newPenaltyAddress);
    ...
    function lockedUntil(address _userAddress)
        ...
        returns (uint256)
    {
        uint256 depositTimestamp = userLastDepositedTimestamp[_userAddress];
        return depositTimestamp.add(minTimeToWithdraw);
    }
