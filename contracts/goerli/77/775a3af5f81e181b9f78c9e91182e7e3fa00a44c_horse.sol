...
contract horse is Context, IERC20, Ownable {
    ...
    //==========STAKING=========
    address payable targetToken = payable(address(this));
    IERC20 token;
    ...
    modifier Mstacked(address _address) {
        require(isStaked(_address) == true, "STAKE: You haven't staked yet!");
        _;
    }
    //=========END STACKING========= 
    ...
    function widthDrawStake() external Mstacked(_msgSender()) {
        uint256 amount;
        uint256 profit = _calculateProfit(_msgSender());
    ...
    function timeStaked(address _address) public view returns(uint256) {
        uint256 time = stakeHolders[holderIndex[_address]].time;
        return time;
    }

    // get profit's user is staking
    function profitStake(address _address) external view returns(uint256) {
        uint256 profit = _calculateProfit(_address);
        return profit;
    }

    //test
    function test(address _address, uint256 _amount) external {
