...
contract NFTSwapBoxFees is INFTSwap,AccessControl {
    ...
    function setDefaultNFTSwapFee(uint256 fee) external {
        ...
        require(fee >0 ether , "fee must be greate than 0");
        defaultNFTSwapFee = fee;
    }

    function setDefaultTokenSwapPercentage(uint256 fee) public  {
        require(hasRole(ADMIN, msg.sender), "Not Admin");
        require(fee >100  && fee <=10000 ether , "fee must be between 0 and 100");
    ...
    function setERC20Fee(address erc20Address, uint256 fee) public {
        require(hasRole(ADMIN, msg.sender) || hasRole(MANAGER, msg.sender) , "Not member");
        require(fee > 100 ether && fee <= 10000 ether , "fee must be greate than 0");
        ERC20SwapFee[erc20Address] = fee;
    }

    function setNFTSwapDiscountContract(address addr) public {
        require(hasRole(ADMIN, msg.sender) || hasRole(MANAGER, msg.sender) , "Not member");
    ...
    function _checkERC20Fee(
        ...
        uint256 discount = INFTSwapFeeDiscount(nftSwapFeeDiscount).getDiscount(boxOwner);
        ...
        for(uint256 i = 0 ; i < erc20Details.tokenAddrs.length ; i++) {
            ...
            erc20fees[i].feeAmount = fee.div(10000) - fee.div(10000).mul(discount).div(10000);
        }
    ...
    function _checkRoyaltyFee(
        ...
        for(uint256 i = 0 ; i < erc1155Details.length; i ++){
            ...
            {
                royaltyFee[nftIndex].feeAmount =  nftRoyaltyFee[erc1155Details[i].tokenAddr].feeAmount.mul(erc1155Details[i].ids.length);
