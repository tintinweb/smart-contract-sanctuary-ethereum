...
// File: contracts/ChooSStaking.sol
...
contract ChookyStakersNFTStaking is ReentrancyGuard, Ownable {
    ...
    //APY rates
    uint16 stakingAPY = 300;
    // minimum stake duration
    uint256 minStakeDuration = 604800;
    ...
    function withdraw(uint256 _tokenId) external nonReentrant {
        ...
        require(block.timestamp - stakers[msg.sender].stakeTimestamp > minStakeDuration, "Can't withdraw before 1 week");
        ...
        // Update the stakeTime for the withdrawer
    ...
    {
        return (((
            ((block.timestamp - stakers[_staker].stakeTimestamp) *
                stakers[_staker].amountStaked)
        ) * stakingAPY) / 31536000);
    }

    // update APY
    function _updateAPY(uint16 _newAPY) external onlyOwner {
        require(_newAPY > 20, "Can't set below 20% annual return");
        stakingAPY = _newAPY;
    }
}