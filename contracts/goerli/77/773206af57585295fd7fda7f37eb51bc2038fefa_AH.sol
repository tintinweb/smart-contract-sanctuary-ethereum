...
//    mapping(address => SubInfo) public subInfo;
    ...
    function getHeartColor(uint256 heartId) public view returns (HeartColor) {
        require(StorageLayerProto(STORAGE_LAYER_ADDRESS)._exists(true, heartId));
//        return HeartColor((_heartsInfo[heartId].auxInfo)%8);
//        return HeartColor((_tokenData[heartId].auxData)%8);
        return HeartColor((_heartsInfo[heartId].auxData)%8);
    }

    function getBurnMultiplier(uint256 heartId) public view returns (uint256) {
        require(StorageLayerProto(STORAGE_LAYER_ADDRESS)._exists(true, heartId));
//        return ((((_heartsInfo[heartId].auxInfo)>>3)%128) + 1);
//        return ((((_tokenData[heartId].auxData)>>3)%1024) + 1);
        return ((((_heartsInfo[heartId].auxData)>>3)%1024) + 1);
    }

    function getBurnRate(uint256 heartId) public view returns (uint256) {
        require(StorageLayerProto(STORAGE_LAYER_ADDRESS)._exists(true, heartId));
        return baseBurnRate*getBurnMultiplier(heartId);
    }
    ...
    **/
    ...
    function getBurnMultiplierUpdateTime(uint256 heartId) public view returns (uint256) {
        require(StorageLayerProto(STORAGE_LAYER_ADDRESS)._exists(true, heartId));
        return uint256(_heartsInfo[heartId].burnLastUpdated);
    }

    function _addTime(uint256 heartId, uint64 timeToAdd) private {
        _heartsInfo[heartId].expiryTime += timeToAdd;
    }

    function _subtractTime(uint256 heartId, uint64 timeToSubtract) private {
        _heartsInfo[heartId].expiryTime -= timeToSubtract;
    }

    function _spend(uint256 heartId, uint256 amt) private {
        uint256 numMinIncrements = amt/minSpendIncrement;
        uint256 millisSpent = millisPerIncrement*numMinIncrements;

        require(amt == numMinIncrements*minSpendIncrement, "frac");
        require((block.timestamp + millisSpent) < getExpiryTime(heartId), "rekt");

        _subtractTime(heartId, uint64(millisSpent));
    }
    ...
    function topUp(uint256 heartId) public payable {
        uint256 topUpIncrements = msg.value/minSpendIncrement;
        uint256 topUpMillis = millisPerIncrement*topUpIncrements;

        require(msg.sender == ownerOf(heartId), "o");
        require(msg.value == topUpIncrements*minSpendIncrement, "frac");

        topUpMillis = topUpMillis/getBurnMultiplier(heartId);

        _addTime(heartId, uint64(topUpMillis));
    ...
    function printNew(uint256 heartId, uint256 burnMultiplier, HeartColor color) public payable {
        require(msg.sender == ownerOf(heartId), "o");
        require(msg.value == baseBurnRate*burnMultiplier, "bruh");



        StorageLayerProto(STORAGE_LAYER_ADDRESS).mint(msg.sender, color);

        LiqRewardsProto(
            LIQUIDATION_REWARDS_ADDRESS
        ).storeReward{value: msg.value/10}(
            StorageLayerProto(STORAGE_LAYER_ADDRESS)._nextToMint()
        );
    }
    ...
    function batchLiquidate(uint256[] memory heartIds) public {
        require(balanceOf(msg.sender) > 0, "need active <3's...");
...
//    function setBurnMultiplier(
//        address msgSender,
//        uint256 tokenId,
//        uint256 burnMultiplier
//    ) public onlyBurnModifier {
//        require(msgSender == tx.origin, "bad msgSender");
//        require(msgSender == ownerOf(tokenId), "not owner");
//        require(burnMultiplier > 0 && burnMultiplier <= 1024, "bm_range");
//
//        _tokenData[tokenId].auxData -= (getBurnMultiplier(tokenId))<<3;
//        _tokenData[tokenId].auxData += burnMultiplier<<3;
//    }

    function setBurnMultiplier(
        address msgSender,
        uint256 heartId,
        uint256 newBurnMultiplier
    ) public onlyBurnModifier {
        uint256 burnMultiplier = getBurnMultiplier(heartId);

        require(msgSender == tx.origin, "bad msgSender");
        require(msgSender == ownerOf(heartId), "not owner");
        require(newBurnMultiplier > 0 && newBurnMultiplier <= 1024, "bm_range");
        if (burnMultiplier > newBurnMultiplier) {
            require(block.timestamp >= (getBurnMultiplierUpdateTime(heartId) + secondsPerMonth), "waitaminnit");
        }

//        _tokenData[tokenId].auxData -= (getBurnMultiplier(tokenId))<<3;
//        _tokenData[tokenId].auxData += newBurnMultiplier<<3;
        _heartsInfo[heartId].auxData -= uint128((getBurnMultiplier(heartId))<<3);
        _heartsInfo[heartId].auxData += uint128(newBurnMultiplier<<3);

        uint256 remTime = getExpiryTime(heartId) - block.timestamp;
        _heartsInfo[heartId].expiryTime = uint64((remTime*burnMultiplier)/newBurnMultiplier);
    }
    ...
    function _applyTransferTax(uint256 heartId) private {
        ...
        _heartsInfo[heartId].expiryTime -= uint64(tax);
    }
...
abstract contract StorageLayerProto {
    ...
    function storage_batchLiquidate(uint256[] memory tokenIds) public virtual;
...
abstract contract LiqRewardsProto {
    ...
    function batchDisburseLiquidationReward(uint256[] memory heartIds, address to) public virtual;
}
