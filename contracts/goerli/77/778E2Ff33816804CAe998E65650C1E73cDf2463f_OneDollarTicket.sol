...
contract OneDollarTicket is VRFConsumerBase, ReentrancyGuard, Ownable, Pausable {
    ...
    uint public ticketnum = lotteryId + 1;
    ...
    function enterDraw() whenNotPaused() public payable nonReentrant {
        require(msg.value >= ticketprice);
            ...
            emit TransferReceived(msg.sender, msg.value, ticketnum, block.timestamp);
        if (ticketcount == freecount) {
            freecount = freecount + 2;
            players.push(payable(msg.sender));
            emit FreePlay(msg.sender, ticketnum, block.timestamp);
        }
    }
