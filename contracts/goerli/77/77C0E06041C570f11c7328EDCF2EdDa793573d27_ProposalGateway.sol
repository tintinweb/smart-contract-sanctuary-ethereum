...
contract ProposalGateway is Initializable, OwnableUpgradeable, UUPSUpgradeable {
    ...
    function createCancelProposalProposal(
        IPool pool,
        uint256 proposalId_,
        string memory description
    ) external onlyPoolShareholder(pool) returns (uint256 proposalId) {
        // Can't cancel CancelProposal
        require(
            pool.getProposalType(proposalId_) !=
                IProposalGateway.ProposalType.CancelProposal,
            ExceptionsLibrary.INVALID_PROPOSAL_TYPE
        );

        proposalId = pool.proposeSingleAction(
            address(pool),
            0,
            abi.encodeWithSelector(IPool.cancelBallot.selector, proposalId_),
            description,
            IProposalGateway.ProposalType.CancelProposal,
            0
        );
