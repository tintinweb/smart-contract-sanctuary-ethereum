...
abstract contract APPcore is  Ownable{
    ...
    mapping(uint256 => bool) public locked; //individual lock
}
...
contract APP is APPadmin {
    ...
    constructor() ERC721A('Test Panda 1013', 'TST') {
    ...
    function approve(address to, uint256 tokenId) public virtual override(IERC721A,ERC721A){
        ...
        if(isLocked == true){
            require(locked[tokenId] == false,"this tokenId is locked");
        }
    ...
    function _beforeTokenTransfers(
        ...
        if(isLocked == true && from != address(0)){
            require(locked[startTokenId] == false,"this tokenId is locked");
        }

        // Reward Lock-up
        if(block.timestamp > 1685577600){
    ...
    function setLock(uint256 _tokenId, bool _locked)external {
        // Only the holder can execute
        require(msg.sender == ownerOf(_tokenId),"Not the holder of tokenId");
        locked[_tokenId] = _locked;
    }

    function setLockArray(uint256[] calldata _tokenId, bool[] calldata _locked)external {
        require(_tokenId.length == _locked.length,"list length is different");
        for(uint256 i = 0; i < _tokenId.length;i++){
            require(msg.sender == ownerOf(_tokenId[i]),"Not the holder of tokenId");
            locked[_tokenId[i]] = _locked[i];
        }        
    }

    function getLockState(address owner)external view returns (bool[] memory){
        unchecked {
            uint256 tokenIdsIdx;
            ...
            for (uint256 i = _startTokenId(); tokenIdsIdx != tokenIdsLength; ++i) {
                ...
                if (currOwnershipAddr == owner) {
                    tokenIdLocked[tokenIdsIdx++] = locked[i];
                }
            }
