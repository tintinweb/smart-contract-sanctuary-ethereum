  ...
  Payable
{
  ...
  constructor() ERC721A('Pxlbot', 'PXLBOT') {}
  ...
  function setMintList(address[] memory addresses) external onlyController {
    for (uint256 i = 0; i < addresses.length; i++) {
      mint_list[addresses[i]] = true;
    }
  }
  ...
  function canMint() public view returns (bool) {
    return mint_list_enabled != true || mint_list[_msgSender()] == true;
  }
  ...
  function mint(uint256 amount, address to) external payable whenNotPaused {
    ...
    require(canMint() == true, 'Unable to mint');
    _safeMint(to, amount);
    curr_batch_total++;
