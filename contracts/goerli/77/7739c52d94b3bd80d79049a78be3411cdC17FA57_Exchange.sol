...
contract Exchange is Ownable{
    ...
    uint256 public nativeTokenMin = 1000000000;

    address public nativeTokenAddress;
    address constant ETHER = address(0); // store Ether in tokens mapping with blank address
    address public WETHER;
    ...
    constructor(
        ...
        WETHER = _weth;
        nativeTokenAddress = _natvieToken;
    }
    ...
    function fillOrder(uint256 _id) public payable{
        ...
        _Order storage _order = orders[_id];
        address tokenGet = _order.tokenGet;
        uint256 amountGet = _order.amountGet;
        bool isNativeHolder = isNativeTokenHolder(msg.sender);
        uint256 feeAmount = 0;
        if(isNativeHolder) {
            feeAmount = amountGet.mul(feeNum).div(feeDen);
        } else {
            feeAmount = amountGet.mul(feeNativeNum).div(feeDen);
        }
        if(tokenGet == ETHER || tokenGet == WETHER) {
            require(msg.value >= feeAmount, 'insufficient ETH');
            tokenBalances[ETHER][feeAccount] = tokenBalances[ETHER][feeAccount].add(feeAmount);
        } else {
            uint256 ethAmount = countEtherFromToken(tokenGet, feeAmount);
            tokenBalances[ETHER][feeAccount] = tokenBalances[ETHER][feeAccount].add(ethAmount);
        }
        require(tokenBalances[_order.tokenGet][msg.sender] >= _order.amountGet, 'low deposited blance');
        _trade(_order.id, _order.user, _order.tokenGet, _order.amountGet, feeAmount, _order.tokenGive, _order.amountGive);
        orderFilled[_order.id] = true;
    }

    function _trade(uint256 _orderId, address _user, address _tokenGet, uint256 _amountGet, uint256 _feeGet, address _tokenGive, uint256 _amountGive) internal {
        // Fee paid by the user that fills the order, a.k.a. msg.sender.
        // uint256 _feeAmount = _amountGet.mul(feePercent).div(100);
        uint256 restAmountGet = _amountGet.sub(_feeGet);
        tokenBalances[_tokenGet][msg.sender] = tokenBalances[_tokenGet][msg.sender].sub(_amountGet);
        tokenBalances[_tokenGet][_user] = tokenBalances[_tokenGet][_user].add(restAmountGet);
        ...
        emit Trade(_orderId, _user, _tokenGet, _amountGet, _feeGet, _tokenGive, _amountGive, msg.sender, block.timestamp);
    }
