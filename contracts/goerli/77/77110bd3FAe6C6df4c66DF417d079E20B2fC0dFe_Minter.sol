...
contract Minter is Pausable, AccessControl {
    ...
    event BreedMinted(uint256 tokenId, uint256 breedId, uint256 mainDna, uint256 shadowDna);
    ...
    function minterMint(address _owner, uint32 _amount, uint32 _setId, uint256[] calldata _options) external whenNotPaused onlyRole(MINTER_ROLE) passingStage(Stage.PRESALE) {
        ...
        _mintBreeds(startTokenId, _amount);
    }
    ...
    function _mintBreeds(uint256 _startTokenId, uint256 _amount) internal {
        Breed[] memory breeds = heterosisContract.mint(_startTokenId, uint8(_amount));
        for (uint256 i = 0; i < _amount; ++i) {
            Breed memory breed = breeds[i];
            emit BreedMinted(_startTokenId + i, breed.idUniq, breed.mainDna, breed.shadowDna);
        }
    }
}