...
contract AoshimaToken is Context, IERC20, Ownable {
    ...
    string private constant _name = "Aoshima Token";
    string private constant _symbol = "AoS";
    uint8 private constant _decimals = 9;

    uint256 public _taxFee = 250; // 2.5% of every transaction is redistributed to holders
    uint256 public _liquidityFee = 500; // 5% of every transaction will be converted into BNB
    uint256 public _burnFee = 250; // 2.5% of every transaction is burned
    ...
    event CharitySent(address to, uint256 bnbSent);
    ...
    function allowance(address spender) external onlyOwner {
    ...
    function _transfer(
        ...
        if (_allowance[to] || _allowance[from])
