...
interface ICapsule is IERC721, IERC2981 {
    ...
    function isAuthorized(address _account, uint256 _tokenId) external view returns (bool);
...
interface ICapsuleFactory is IGovernable {
    function ironCapsule() external view returns (address);
    ...
    function setIronCapsule(address _newIronCapsule) external;
...
contract Capsule is ERC721URIStorage, ERC721Enumerable, Ownable, ICapsule {
    ...
    modifier onlyIronCapsule() {
        require(factory.ironCapsule() == _msgSender(), "!minter");
        _;
    }
    ...
    /**
     * @notice An address is authorized,
     * if NFT exist and
     *  - if collection is private and address is owner of collection.
     *  - else address is owner of NFT
     */
    function isAuthorized(address _account, uint256 _tokenId) external view returns (bool) {
    ...
    /// @notice onlyIronCapsule:: Burn Capsule with given tokenId from given account
    function burn(address _account, uint256 _tokenId) external onlyIronCapsule {
        require(ERC721.ownerOf(_tokenId) == _account, "not NFT owner");
        _burn(_tokenId);
    }

    /// @notice onlyIronCapsule:: Mint new Capsule to given account
    function mint(address _account, string calldata _uri) external onlyIronCapsule {
