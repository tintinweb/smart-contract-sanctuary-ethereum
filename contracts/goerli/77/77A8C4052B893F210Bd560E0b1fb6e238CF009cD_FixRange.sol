...
abstract contract Base is Ownable, Multicall, ReentrancyGuard {
    ...
    /// @notice emit if contract owner successfully calls modifyStartBlock(...)
    /// @param startBlock startBlock
    event ModifyStartBlock(uint256 startBlock);
    ...
    function _getRewardBlockNum(uint256 _from, uint256 _to) internal view returns (uint256) {
        if (_from > _to) {
            return 0;
        }
        if (_to <= endBlock) {
    ...
    function modifyRewardPerBlock(uint256 rewardIdx, uint256 _rewardPerBlock) external onlyOwner {
    ...
    /// @notice Set new reward start block.
    ...
    function modifyStartBlock(uint256 _startBlock) external onlyOwner {
        ...
        emit ModifyStartBlock(startBlock);
    }
