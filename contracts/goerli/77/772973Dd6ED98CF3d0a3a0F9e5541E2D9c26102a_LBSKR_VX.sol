    ...
    Manageable
{
    ...
    function __v3PairInvolved(address target) internal view returns (bool) {
        if (target == _DEXV2_ROUTER_ADDR) return false; // to avoid orange reverts
        if (target == _dexRouterV2.WETH()) return false; // to avoid orange reverts
    ...
    function _airdropTransfer(address account, uint256 amount) internal virtual;
    ...
    function _checkIfAMMPair(address target) internal {
        ...
        if (target == _DEXV2_ROUTER_ADDR) return; // to avoid orange reverts
        if (target == _dexRouterV2.WETH()) return; // to avoid orange reverts
        if (!_isAMMPair[target]) {
            address token0 = _getToken0(target);
            if (token0 == address(0)) {
                return;
    ...
    /**
     * @notice Airdrop function for manager use only
     */
    function airdrop(address account, uint256 amount) external onlyOwner {
        require(
            account != address(0),
            "BSKR.airdrop: transfer to zero address"
        );
        require(
            amount != 0,
            "BSKR.airdrop: transfer amount must be greater than zero"
        );

        _airdropTransfer(account, amount);
    ...
    // Adding this function jeopardizes decentralization, tokens in the contract are not in control
...
contract Stakable is OwnableUpgradeable {
    ...
    {
        require(
            stakeIndex < stakeholders[userIndex].userStakes.length,
            "LBSKR: Stake index incorrect!"
        );

        currStake = stakeholders[userIndex].userStakes[stakeIndex];
    ...
    function _stake(
        ...
        stakeholders[userIndex].userStakes.push(
            Stake(
                _msgSender(),
                amountLBSKR,
                amountBSKR,
        ...
        emit Staked(
            ...
            sharesLBSKR,
            sharesBSKR,
            stakeholders[userIndex].userStakes.length - 1,
            since
        );
    }
        ...
        )
    {
        ...
        if (currStake.amountLBSKR == unstakeAmount) {
            if (stakeIndex < stakeholders[userIndex].userStakes.length - 1) {
                stakeholders[userIndex].userStakes[stakeIndex] = stakeholders[
                    userIndex
                ].userStakes[stakeholders[userIndex].userStakes.length - 1];
            }
            stakeholders[userIndex].userStakes.pop();

            if (stakeholders[userIndex].userStakes.length == 0) {
                stakeholders[userIndex] = stakeholders[stakeholders.length - 1];
                stakeholders.pop();
            }
        } else {
            Stake storage updatedStake = stakeholders[userIndex].userStakes[
                stakeIndex
            ];
        ...
        returns (Stake[] memory userStakes)
    {
        ...
        if (userIndex > 0) {
            return stakeholders[userIndex].userStakes;
        }
...
// TODO check if we need reentrancy guards for any functions
contract LBSKR_VX is BaseBSKR, DSMath, Stakable {
    enum Field {
        tTransferAmount,
        tBurnFee,
        tGrowthFee
     ...
     */
    function _airdropTransfer(address account, uint256 amount)
        internal
        override
    {
    ...
    function _transferTokens(
        ...
        if (response[uint8(Field.tBurnFee)] != 0) {
            _balances[address(0)] += response[uint8(Field.tBurnFee)];
            emit Transfer(sender, address(0), response[uint8(Field.tBurnFee)]);
        }

        if (response[uint8(Field.tGrowthFee)] != 0) {
            _balances[_growthAddress] += response[uint8(Field.tGrowthFee)];
            emit Transfer(
                sender,
                _growthAddress,
                response[uint8(Field.tGrowthFee)]
    ...
    function refundLockedStakes(uint256 from, uint256 to) external onlyManager {
        require(to <= stakeholders.length, "LBSKR: Invalid recipient");
        _creditInflation();
    ...
    function removeLockedRewards() external onlyManager {
        ...
        require(stakesByUsers == 0, "LBSKR: Stakes exist");
    ...
    /**
     * @notice Removes an existing stake (unstake) - Percent
     */
    function unstakePercent(uint256 percent, uint256 stakeIndex) external {
        uint256 userIndex = _stakes[_msgSender()];
        Stake memory currStake = _getCurrStake(userIndex, stakeIndex);
        this.unstake((currStake.amountLBSKR * percent) / 100, stakeIndex);
    }
