...
contract Slot is VRFConsumerBaseV2, Ownable, KeeperCompatibleInterface {
    ...
    uint16 private immutable i_startingStoredWords;
    ...
    constructor(
        ...
        unchecked {
            i_refreshStoredSpinsLength = refreshStoredWords / uint16(NUM_REELS);
            i_startingStoredSpinsLength = startingStoredWords / uint16(NUM_REELS);
        }
    }
    ...
    function fulfillRandomWords(
        ...
        uint8[] memory words = new uint8[](i_startingStoredWords);

        for (uint8 i = 0; i < startingStoredSpinsLength; ) {
            // uint256[] memory spin = new uint256[](NUM_REELS);
            uint8 randomWordsIdx;
            uint8 randWord;
            SpinStreakData memory spinStreak = SpinStreakData(0, 0, 99, 99, 0, 0);

            for (uint8 j = 0; j < NUM_REELS; ) {
                unchecked {
                    randomWordsIdx = i * NUM_REELS + j;
                    randWord = uint8(randomWords[randomWordsIdx] % NUM_STOPS);
                }

                words[randomWordsIdx] = randWord;

                // evaluate reel
                if (randWord == LUCKY_STOP_DIGIT) {
                    unchecked {
                    ...
                    if (randWord == spinStreak.spinStreakStopTemp) {
                        unchecked {
                        ...
                        spinStreak.spinStreakTemp = 1;
                        spinStreak.spinStreakStopTemp = randWord;
                    }
                }
                unchecked {
                    j += 1;
    ...
    function buySpins() public payable {
        ...
        uint8 startingSpinsIdx = uint8(s_payouts.length) - 1;
        uint8 startingWordsIdx = uint8(s_words.length) - 1;
        ...
        uint8 spinIdx;
        uint8 wordIdx;
        uint8 playerWordsIdx;

        for (uint8 i = 0; i < SPINS_PER_BUY; ) {
            unchecked {
    ...
    function getPlayerSpinsRemaining() public view returns (uint256) {
        return s_playerSpins[msg.sender].length / NUM_REELS;
    }
