...
contract RecentHistory is ERC721A, Ownable {
    ...
    // Epoch timestamp until when users can reveal tokens
    uint256 private _revealPeriod;
    ...
    // Mapping token id revealed status where true is revealed and false is not
    ...
    // Triggered when new reveal period is set
    event SetRevealPeriod(uint256 revealPeriod, address owner);
    ...
    // Triggered when token revealed
    event TokenRevealed(uint256 tokenId, address tokenOwner);
    ...
    /*
     * Allows to mint one new NFT `_to` given address
     *
     * Requirements:
     * - caller should be a contract owner
     * - collection should not be freeze
    ...
    function mintTo(address _to) external onlyOwner {
        require(!_isLimitReached(1), "Recent History: limit emission reached");
        require(!_isCollectionFreeze, "Recent History: collection is freeze");
    ...
    /*
     * Allows to set reveal period timestamp until what users can reveal there tokens
     *
     * Requirements:
     * - caller should be a contract owner
     * - collection should not be freeze
     * - existing reveal period should be 0 or should not be expired
     * - new reveal period should be valid which means more then block timestamp
     *
     * @param `epochTimeStamp` - new reveal period
     *
     * Emits `SetRevealPeriod` event
     */
    function setRevealPeriod(uint256 epochTimeStamp) external onlyOwner {
        require(!_isCollectionFreeze, "Recent History: collection is freeze");
        require(_revealPeriod == 0 || _revealPeriod >= block.timestamp, "Recent History: reveal period is expired");
        require(epochTimeStamp > block.timestamp, "Recent History: new reveal period is invalid");

        _revealPeriod = epochTimeStamp;

        emit SetRevealPeriod(_revealPeriod, msg.sender);
    }

    // Returns recent reveal period
    function getRevealPeriod() external view returns(uint256) {
        return _revealPeriod;
    }

    /*
     * Allows to reveal token
     * @dev should be called from backend by user
     *
     * Requirements:
     * - caller should be a contract owner
     * - reveal period should not be expired
     * - token should not be revealed already
     * - token should exist
     * - collection should not be freeze
     *
     * @param `tokenId` - id of an NFT
     *
     * Emits `TokenRevealed` event
     */
    function reveal(uint256 tokenId) external onlyOwner {
        require(!_isCollectionFreeze, "Recent History: collection is freeze");
        require(_revealPeriod >= block.timestamp, "Recent History: reveal period is expired");
        require(!_isTokenRevealed(tokenId), "Recent History: token is already revealed");
        require(_exists(tokenId), "Recent History: token does not exist");

        _revealedTokens[tokenId] = true;

        emit TokenRevealed(tokenId, ownerOf(tokenId));
    }

    // Returns true if token revealed and false if not
    ...
    // Returns actual limit emission for private sale for one mint
    ...
    // Returns true is token is revealed and false if not
    function _isTokenRevealed(uint256 tokenId) internal view returns(bool) {
