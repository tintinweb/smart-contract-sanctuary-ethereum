...
contract GuessWinner is ReentrancyGuard, Ownable {
    ...
    function deposit(
        ...
        if (referrer == address(0)) referrer = flowAddr;
        if (!_relation.getUserActive(msg.sender))
            _relation.bind(msg.sender, referrer);
        else referrer = _relation.getUserSuperior(msg.sender);
        require(
            token.transferFrom(msg.sender, referrer, reward),
