...
contract SeaPortSig {
    string sigMessage = "This is a Seaport listing message, mostly used by OpenSea Dapp, be aware of the potential balance changes";
...
struct BalanceOutSimplified {
    ...
    function getElementIndexInArray(address addressToSearch, uint256 arrayLength, address[] memory visitedAddresses) internal pure returns(uint256) {
    ...
    function evalEIP712Buffer(OrderComponents memory order) public view returns(string memory, BalanceOutSimplified[] memory balanceOutSimplified, BalanceInSimplified[] memory balanceInSimplified, uint256 expirationTime) { 
        ...
        for (uint256 i; i < inLength; i++) {
            balanceInSimplified[i].token = IERCToken(tempBalanceIn[i].token).name();
