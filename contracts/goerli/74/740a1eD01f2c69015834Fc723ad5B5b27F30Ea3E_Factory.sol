...
contract NFTcontract is ERC721Enumerable, ReentrancyGuard {
    ...
    uint256 private totalCountOfItems;
    ...
    function updateMultiBaseURIs(
        ...
        for (uint256 i = 0; i < _numOfTokens.length; i++) {
            for (uint256 j = 0; j < _numOfTokens[i]; j++) {
                totalCountOfItems += 1;
                BaseURIs[totalCountOfItems] = _baseURIs[i];
            }
        }
    }
    
    // ============ FUNCTION TO UPDATE MULTIPLE BASEURIS ============
    function updateOneBaseURI(uint256 tokenId, string calldata _baseURIs) external onlyOwner {
        require(tokenId <= totalCountOfItems + 1, "Please input valid tokenId to be updated.");
        if (tokenId < totalCountOfItems + 1) {
            BaseURIs[tokenId] = _baseURIs;
        }
        else {
            totalCountOfItems += 1;
            BaseURIs[totalCountOfItems] = _baseURIs;
        }
    }
