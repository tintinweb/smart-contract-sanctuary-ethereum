...
contract Infynyty is ReentrancyGuard, Ownable {
    ...
    function buy(
        ...
        transferETH(sellerInfo.seller, (msg.value - platformFeeInfo(msg.value)));
        transferETH(platformAddress, platformFeeInfo(msg.value));
        transferERC1155(
            sellerInfo.NFTAddress,
            sellerInfo.seller,
            msg.sender,
    ...
    function transferERC721(address nftAddress, address seller, address buyer, uint256 tokenId) internal {
        IERC721(nftAddress).safeTransferFrom(seller, buyer, tokenId);
    }
    function transferERC1155(address nftAddress, address seller, address buyer, uint256 tokenId, uint256 amount) internal {
    ...
    function updateFee(uint256 percentageFee) external onlyOwner {
        require(percentageFee <= _feeDenominator(), "ERC2981: royalty fee will exceed salePrice");
        platformFee = percentageFee;
    }

    function platformFeeInfo(uint256 _salePrice) internal view returns (uint256) {
        uint256 feeAmount = (_salePrice * platformFee) / _feeDenominator();
        return(feeAmount);
    }
...
contract nonceHandler is Ownable{
    ...
    function init(address _marketAddress) public onlyOwner {
