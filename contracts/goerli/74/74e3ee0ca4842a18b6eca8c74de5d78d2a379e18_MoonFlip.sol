...
// File: contracts/MoonFlipGameOracle.sol
...
contract MoonFlip is Ownable, ReentrancyGuard, RrpRequesterV0(0xa0AD79D995DdeeB18a14eAef56A549A04e3Aa1Bd)  {
    using Address for address;

    address constant API3_QRNG_AIRNODE_ADDRESS = 0x9d3C147cA16DB954873A498e0af5852AB39139f2;
    bytes32 constant API3_QRNG_ENDPOINT_ID = 0xfb6d017bb87991b7495f563db3c8cf59ff87b09781947bb1e417006ad7f55a78;
    ...
    // random request ids to the commit that needs to be completed
    mapping(bytes32 => Commit) _pendingGames;
   ...
   function setSponsorAddress(address _addr) external onlyOwner { 
       sponsorAddress = _addr; 
   }
    ...
    function completeFlip(bytes32 requestId, bytes calldata data) external onlyAirnodeRrp {
        Commit memory commit = _pendingGames[requestId];
        require(commit.exists, "commit does not exist");

        uint256 randomNumber = abi.decode(data, (uint256));

        address player = commit.player;
        ...
        delete _pendingGames[requestId];
        ...
        nonReentrant
    {
        ...
        Commit memory pendingCommit = Commit({
            player: msg.sender,
        ...
        _pendingCommits[msg.sender][nonce] = pendingCommit;
        ...
        bytes32 requestId = airnodeRrp.makeFullRequest(
            API3_QRNG_AIRNODE_ADDRESS,
            API3_QRNG_ENDPOINT_ID,
            address(this),
            sponsorAddress,
            address(this),
            this.completeFlip.selector,
            ""
        );

        _pendingGames[requestId] = pendingCommit;
