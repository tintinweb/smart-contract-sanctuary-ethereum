...
import {IKnightsApplication} from "./IKnightsApplication.sol";

/// @title IronHills Soulbound Tokens
/// @author Kfish n Chips
/// @custom:security-contact [email protected]
contract KnightsApplication is
    Initializable,
    UUPSUpgradeable,
    ERC721Upgradeable,
    ...
    IKnightsApplication
{
    ...
    function mint(bytes32 messageHash, uint8 _v, bytes32 _r, bytes32 _s) external {
        if(minters[msg.sender] || hashes[messageHash]) revert AlreadyMinted();
        address _signer = ECDSAUpgradeable.recover(messageHash, _v, _r, _s);

        if(signer != _signer) revert    ();
    ...
    function initialize(address _signer, string memory _newBaseURI) public initializer {
        ...
        __ERC721_init_unchained("Knights Application", "KNIGHTSAPP");
