...
contract CALStaker is ReentrancyGuard, Ownable {
    ...
    function stake(uint256[] calldata tokenIds, uint256 projectIdIndex) external nonReentrant {
        ...
        require(projectInfos[projectIdIndex].stakingTotal + tokenIds.length <= projectInfos[projectIdIndex].maxTotal,'max');
        require(stakingInfos[_msgSender()][projectIdIndex].stakingTotal + tokenIds.length <= projectInfos[projectIdIndex].maxTotalPerAddress,'max per address');
        require(stakingInfos[_msgSender()][projectIdIndex].stakingTotal + tokenIds.length > projectInfos[projectIdIndex].minTotalPerAddress,'min per address');
...
import '@openzeppelin/contracts/access/AccessControl.sol';
import '@openzeppelin/contracts/utils/Strings.sol';
import './ShurikenStakingMetadata.sol';
