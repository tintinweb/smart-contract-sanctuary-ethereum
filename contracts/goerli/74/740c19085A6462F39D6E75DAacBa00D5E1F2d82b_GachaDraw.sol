...
contract GachaDraw is Ownable {
    ...
    mapping(uint256 => uint256) public activityType;
    mapping(uint256 => bool) public isActive;
    mapping(uint256 => uint256[]) public rewardTokenIds;
    mapping(uint256 => uint256[]) public totalRewardSupply;
    mapping(uint256 => uint256[]) public maximumRewardSupply;
    ...
    function createActivity(
        ...
        startTimestamp[totalActivities] = _startTimestamp;
        endTimestamp[totalActivities] = _endTimestamp;
        isActive[totalActivities] = true;
        rewardTokenIds[totalActivities] = _rewardTokenIds;
        maximumRewardSupply[totalActivities] = _rewardTokenSupply;
        totalRewardSupply[totalActivities] = new uint256[](_rewardTokenIds.length);
    ...
    function getAvailableRewardToken(uint256 _activityId) internal view returns(AvailableRewardResult memory) {
        AvailableRewardItem[] memory selectedReward = new AvailableRewardItem[](rewardTokenIds[_activityId].length);
        uint256 totalAvailableReward = 0;

        for(uint256 i = 0; i < rewardTokenIds[_activityId].length; i++) {
            if(totalRewardSupply[_activityId][i] < maximumRewardSupply[_activityId][i]) {
                selectedReward[totalAvailableReward] = AvailableRewardItem(rewardTokenIds[_activityId][i], i);
    ...
    function drawTicket(
        ...
        for(uint256 i = 0; i < _amount; i++) {
            ...
            totalRewardSupply[_activityId][selectedReward.index] ++;
            totalRewardWon ++;
