...
error UniforgeCollection__SupplyLimitReached();
 ...
 */
contract UniforgeCollection is ERC721, Ownable {
    using Strings for uint256;
    ...
    address payable private immutable i_uniforgeDeployer;
    uint256 private constant INITIAL_SALE_FEE = 5;

    modifier mintCompliance(uint256 _mintAmount) {
        if (_mintAmount < 0 || _mintAmount > s_maxMintAmount) {
            revert UniforgeCollection__InvalidMintAmount();
        }
        if (s_supply.current() + _mintAmount > i_maxSupply) {
    ...
    constructor(
        ...
        i_uniforgeDeployer = payable(msg.sender);
    ...
    function mintNft(uint256 _mintAmount) public payable mintCompliance(_mintAmount) {
    ...
    function withdraw() public onlyOwner {
        (bool _uniforgeSuccess, ) = payable(msg.sender).call{
            value: (address(this).balance * INITIAL_SALE_FEE) / 100
        }("");
        if (!_uniforgeSuccess) {
    ...
    function _mintLoop(address _receiver, uint256 _mintAmount) internal {
        for (uint256 i = 0; i < _mintAmount; i++) {
            ...
            if (s_supply.current() >= i_maxSupply) {
                revert UniforgeCollection__SupplyLimitReached();
            }
    ...
    function getHolder(uint256 _tokenId) public view returns (address) {
        return s_holders[_tokenId];
    }

    function uniforgeDeployer() public view returns (address) {
        return i_uniforgeDeployer;
    }

    function initialSaleFee() public pure returns (uint256) {
        return INITIAL_SALE_FEE;
    }
}
...
error UniforgeDeployerV0__CallerIsNotTheOwner();
...
contract UniforgeDeployerV0 {
    ...
    modifier onlyOwner() {
        if (i_owner != msg.sender) {
            revert UniforgeDeployerV0__CallerIsNotTheOwner();
        }
        _;
    }
