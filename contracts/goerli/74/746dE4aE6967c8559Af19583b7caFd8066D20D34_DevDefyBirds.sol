...
library MerkleUtils {
    function hashPairs(bytes32[] memory leaves)
        internal
        pure
        returns (bytes32[] memory)
    {
        ...
        if (odd) {
            h[lenNew] = hashPair(leaves[lenOld - 1], leaves[lenOld - 1]);
        }
...
contract MerkleTree {
    ...
    constructor(bytes32[] memory leafs_) {
        _hashes.push(leafs_);
        for (uint256 i; _hashes[i].length > 1; ++i) {
            _hashes.push(_hashes[i].hashPairs());
        }
    }

    function proofLength() public view returns (uint256) {
    ...
    /**
     * @notice Computes a merkle proof compatible with OZ's merkle proof
     * validation.
     */
    function proof(uint256 leafIdx) public view returns (bytes32[] memory) {
        uint256 len = proofLength();
        ...
        for (uint256 i; i < len; ++i) {
            ...
            uint256 neighbour = odd
                ? leafIdx - 1
                : leafIdx == _hashes[i].length - 1 ? leafIdx : leafIdx + 1;

            proof_[i] = _hashes[i][neighbour];
    ...
    function leafs() public view returns (bytes32[] memory) {
        return _hashes[0];
    }

    function leaf(uint256 idx) public view returns (bytes32) {
        return _hashes[0][idx];
    }

    function size() public view returns (uint256) {
        return _hashes[0].length;
    }
}
...
import {MerkleTree} from "redeemable-voucher-test/MerkleTree.sol";

contract ProofGenerator is Script {
    function generateMerkleProofs(MerkleTree tree, string memory outputJSON)
        public
    {
        string memory objKey = "proofs";
        for (uint256 i; i < tree.size(); ++i) {
            _serializeProof(objKey, tree, i);
        }

        vm.writeJson(
            vm.serializeString(objKey, "", ""), // Just to get the json
            outputJSON
        );
    }

    function _serializeProof(string memory objKey, MerkleTree tree, uint256 idx)
        private
        resetMemory
    {
        string memory leaf = vm.toString(uint256(tree.leaf(idx)));
        vm.serializeBytes32(objKey, leaf, tree.proof(idx));
    }

    /**
     * @notice Resets the free-memory pointer after execution
     * @dev Little trick to prevent the proof generation from running out-fo-gas
     * due to memory bloat since `vm.pauseGasMetering` seems to not work.
     */
    modifier resetMemory() {
        uint256 fmp;
        assembly {
            fmp := mload(0x40)
        }
...
import {DefyBirds, IEntropyOracle} from "../src/DefyBirds.sol";
import {ClaimableWithSignature} from "../src/ClaimableWithSignature.sol";
import {ProofGenerator} from "./ProofGenerator.s.sol";
...
contract EntropyOracleStub is IEntropyOracle {
    ...
    function set(uint256 blocknumber, bytes32 entropy) public {
        blockEntropy[blocknumber] = entropy;
    }

    function requestEntropy(uint256 blocknumber) public {
        requested[blocknumber] = true;
        emit EntropyRequested(blocknumber);
        blockEntropy[blocknumber] = keccak256(
            // solhint-disable-next-line not-rely-on-time
            abi.encodePacked(address(this), block.timestamp, blocknumber)
        );
    }
...
contract DevDefyBirds is DefyBirds {
    constructor(DefyBirds.Config memory config) DefyBirds(config) {}

    function devmint(address to) public {
        devmint(to, 1);
    }

    function devmint(address to, uint16 num) public {
        _doClaim(to, num);
    }
}

contract Base is Script {
    DevDefyBirds public token;
    IEntropyOracle public entropyOracle;

    address public defyBirdsRoyalties;

    uint256[] public nudebirdIds;
    MerkleTree public nudebirdsTree;

    function setUp() public {
        nudebirdIds = new uint[](1500);
        for (uint256 i; i < nudebirdIds.length; ++i) {
            nudebirdIds[i] = i;
        }

        bytes32[] memory leafs = new bytes32[](nudebirdIds.length);
        for (uint256 i; i < nudebirdIds.length; ++i) {
            leafs[i] = bytes32(uint256(nudebirdIds[i]));
        }
        nudebirdsTree = new MerkleTree(leafs);

        defyBirdsRoyalties = AddressBook.treasury();

        entropyOracle =
            IEntropyOracle(0x661724f29a60f2D96919dF7eBB456C96e6416e53);
        // token = DevDefyBirds(0x866f64B11D3288535fb26C0Ac24211504335f790);
        // token = DevDefyBirds(0xa1Cdd29B3D83EfCBB49dcB8C24Ad8457af640ade);
        // token = DevDefyBirds(0x6E614B89083302C3eC09d6174B7C880003140C62);
    }
}
...
contract Deploy is Base {
    function run() public {
        ...
        if (address(entropyOracle) == address(0)) {
            entropyOracle = new EntropyOracleStub{salt: bytes32(uint256(1))}();
        }

        if (address(token) == address(0)) {
            token = new DevDefyBirds(
                DefyBirds.Config({
                    admin: AddressBook.engineering(),
                    steerer: msg.sender,
                    royaltiesReceiver: payable(defyBirdsRoyalties),
                    baseTokenURI: "ipfs://QmWsUiK9JPwTasaJDgAui5FA1nTeqbkacGGVTFVANskM5y/",
                    moonbirds: IMoonbirds(AddressBook.moonbirds()),
                    nudeBirdsRoot: nudebirdsTree.root(),
                    nudeBirdsProofLength: nudebirdsTree.proofLength(),
                    oracle: entropyOracle,
                    delegationRegistry: IDelegationRegistry(AddressBook.delegateCashRegistry())
                })
            );
            ...
            token.ownerMint(AddressBook.treasury(), 1);
        }

        // if (true) {
        //     token.toggleNestedMerkleClaim(true);
        //     token.toggleClaimWithSignature(true);
        // }

        // if (true) {
        //     ClaimableWithSignature.Claim[] memory claims =
        //         new ClaimableWithSignature.Claim[](1);
        //     claims[0] = ClaimableWithSignature.Claim({
        //         signedAllowance: ClaimableWithSignature.SignedAllowance({
        //             allowance: ClaimableWithSignature.Allowance({
        //                 receiver: 0x2cC39cefBa8765bE0879e752aa97E2D3D41e7bC5,
        //                 nonce: 0,
        //                 numMax: 22,
        //                 activeAfterTimestamp: 1_678_314_594
        //             }),
        //             signature:
        // hex"3ba29f9d9360395ee1d52eee0f5da1a71804e5cd2aa782f6925419374d4e8bb62ebc95c4ba98d3ebed50254358df1a982b5416ce17fda6cd9c0ef1680b5eef471b"
        //         }),
        //         numClaims: 2
        //     });
        //     token.claimMultipleWithSignature(claims);
        // }
...
contract GenerateProofs is Base, ProofGenerator {
    function run() public {
        generateMerkleProofs(
            nudebirdsTree, "./output/nudebirds-merkle-proofs-testnet.json"
        );
    }
}
    ...
    ClaimableWithSignature
{
     ...
     */
    uint16 internal constant _MAX_NUM_OWNER_MINTS = 10;
    ...
    function _doClaimsFromSignature(address to, uint16 num)
        ...
        override
    {
        if (numClaimableWithSignature < num) {
...
abstract contract DefyBirdsTraitMechanics is ERC721ACommonBaseTokenURI {
    ...
    function burn(uint256[8] calldata tokenIds) external {
        ...
        _metadata[_totalMinted()] = TokenMetadata({
            color: 0,
            body: body,
            glitch: true,
            placeholder: false
