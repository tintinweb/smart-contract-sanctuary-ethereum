...
contract SkelePhunks is ERC721, Ownable {
    using Strings for uint256;
    ...
    string collectionDescription = "SkelePhunks is a universal, adaptive PFP for the crypto/NFT community.";
    ...
    constructor (
        ...
        _gdToPath[0] = "phunk/female";
        _gdToPath[0] = "punk/male";
        _gdToPath[0] = "punk/female";
    }

    function setMintStatus(MintStatus newMintStatus) public onlyOwner {
        require(newMintStatus != MintStatus.PreMint && newMintStatus != MintStatus.AllowList, "ms");
    ...
    function _mintMain(address _to, uint256 _quantity, uint256 _gd) private {
        require(!paused, "p");
        require(
...
//        uint256 curSlot = curIndex%128;
    ...
    function mint(uint256 _quantity, uint256 genderDirection) public payable {
        require(mintStatus == MintStatus.Public, "ms");
        require(msg.value >= _quantity*mintPrice);

        totalReceived += msg.value;
        _mintMain(msg.sender, _quantity, genderDirection);
    }
...
//    function getGenderAndDirection(uint256 tokenId) private view returns (uint256) {
    ...
    function setGenderAndDirection(uint256 tokenId, uint256 gender, uint256 direction) public {
        ...
        uint256 toSet = gender*2 + direction;
...
//        require(_exists(tokenId), "z");
        ...
        if (bytes(_baseURI).length > 0) {
            return string(
                abi.encodePacked(
                    _baseURI, "/", _gdToPath[getGenderAndDirection(tokenId)], "/",
