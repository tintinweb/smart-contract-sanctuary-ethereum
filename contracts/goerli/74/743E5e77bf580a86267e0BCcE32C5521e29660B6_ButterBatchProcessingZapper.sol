...
contract ButterBatchProcessingZapper {
  ...
  function permitAndSpend(
    ...
    IUSDC(token).permit(msg.sender, address(this), _amounts[1], deadline, v, r, s);
    IERC20(token).safeTransferFrom(msg.sender, address(this), _amounts[1]);
    // require(deadline > block.timestamp, "Permit signature has expired");
    // address butterBatchProcessing = contractRegistry.getContract(keccak256("ButterBatchProcessing"));
    // for (uint8 i; i < _amounts.length; i++) {
    //   if (_amounts[i] > 0) {
    ...
    //     if (i == 1) {
    //       // index of 1 currently represents usdc
    //       IUSDC(curve3Pool.coins(i)).permit(msg.sender, address(this), _amounts[i], deadline, v, r, s);
    //     }
    //     IERC20(curve3Pool.coins(i)).safeTransferFrom(msg.sender, address(this), _amounts[i]);
    //   }
    // }
