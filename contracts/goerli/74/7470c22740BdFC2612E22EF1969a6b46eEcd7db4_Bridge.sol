...
contract Bridge is IBridge {
    ...
    /// @notice Function to receive a request from relayer to relay a message on the destination chain.
    ...
    /// @param receivingCounter Address of the recipient contract on destination chain.
    /// @param data Calldata to be passed to the receivingCounter contract on the destination chain.
    function receiveFromRelayer(
        uint256 nonce,
        address receivingCounter,
        bytes calldata data
    ) public {
        address sendingCounter = abi.decode(data[4:], (address));
        require(msg.sender == relayer, "only relayer");
        require(!executed[sendingCounter][nonce], "nonce already executed");
        require(
            receivingCounter != address(0) && sendingCounter != address(0),
            "sending and receiving counter can't be address(0)"
        );
