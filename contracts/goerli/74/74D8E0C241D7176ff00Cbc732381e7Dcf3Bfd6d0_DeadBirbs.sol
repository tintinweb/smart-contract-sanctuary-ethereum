...
contract DeadBirbs is ERC721A, Seller, AttributeStore, ReentrancyGuard {
    ...
    */
    function allowlistMint(bytes calldata attributeHash, bytes calldata attributeSignature, bytes calldata mintKey) external payable nonReentrant {
    ...
    */
    function publicMint(bytes calldata attributeHash, bytes calldata attributeSignature) external payable nonReentrant {
        verifyPublicSale(1);
        verifyAttributeHash(attributeHash, attributeSignature);
        sotreAttributeHash(attributeHash, msg.sender, totalSupply());
        _safeMint(msg.sender, 1);
    ...
    */
    function ownerMint(bytes calldata attributeHash, bytes calldata attributeSignature, address recipient) external onlyOwner nonReentrant {
        verifyAttributeHash(attributeHash, attributeSignature);
        sotreAttributeHash(attributeHash, msg.sender, totalSupply());
...
contract AttributeStore is Ownable {
    ...
    mapping(bytes => address) public firstMinters;
    ...
    function verifyAttributeHash(bytes memory attributeHash, bytes memory attributeSignature) internal virtual view {
    ...
    function checkSignature(address signer, bytes memory attributeHash, bytes memory signature) private pure returns (bool) {
        return signer == 
            keccak256(
                abi.encodePacked(
    ...
    function sotreAttributeHash(bytes memory attributeHash, address minter, uint256 tokenId) internal {
