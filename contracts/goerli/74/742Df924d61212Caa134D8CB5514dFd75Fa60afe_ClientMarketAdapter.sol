...
contract ClientMarketAdapter is ModuleAccessController, IClientMarketAdapter {
    ...
    function protect(
        ...
        (uint id, uint floor) = market.protect(msg.sender, amount, risk, term, bumpAmount, flags);
    ...
    function protectNative(
        ...
        uint bumpAmount; uint amount;
        // apply incentives
        uint boostAmount = incentives.boostForTaker(address(asset), amount);
        uint incentivesAmount = incentives.coordinationForTaker(address(asset), amount);        

        // lock BUMP tokens using Bond contract
        { // this is because stack too deep error
            uint reduceAmount;
            (bumpAmount, , reduceAmount) = bond.autoLockBondTakerPosition(
                msg.sender,
                ASSET_ADDRESS(),
                msg.value,
                risk,
    ...
    function close(
        ...
        bool unwrap,
        bool withdrawBond
    ) public virtual override onlyTakerPositionOwner(id) {
        // Update market state
        IState(market.getState()).updateStateToCurrentTime();
    ...
    function deposit(
        ...
        bstAmount = bstAmount - reduceAmount;
    ...
    /// @inheritdoc IMakerPosition
    function abandon(uint id, bool withdrawBond)
        public
        virtual
        override
        onlyMakerPositionOwner(id)
