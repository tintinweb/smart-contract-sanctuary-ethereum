...
contract Market is ERC721, ERC721Burnable, Ownable, ReentrancyGuard, Pausable, IMarket {
  ...
  function rent(
    ...
    require(_rentalStartTimestamp < (block.timestamp + reservationLimits), 'overReservationLimits');

    Lend memory _lend = lendRent[_lockId].lend;

    // Verify executor for private listing
    if (_lend.listType == ListType.Private)
      require(msg.sender == privateList[_lockId], 'invalidRenter');
  ...
  function _calcFee(
    ...
    uint256 _collectionOwnerFee = (_rentalFee * IVault(_vaultAddress).collectionOwnerFeeRatio()) /
      E5;
  ...
  function _createList(
    ...
    //TODO テストコード
    tokensLockId[IVault(_vaultAddress).originalCollection()][_tokenId].push(_lockId);
    //テストコードここまで

    // Add payment token to the list for claiming ProtocolAdminFee
    bool _alreadyExists = false;
    for (uint256 i = 0; i < paymentTokens.length; i++)
      if (paymentTokens[i] == _paymentToken) _alreadyExists = true;
...
library CalcDuration {
  function duration(
    ...
    require(
      _lend.minRentalDuration <= _rentalDurationByDay &&
        _rentalDurationByDay <= _lend.maxRentalDuration,
