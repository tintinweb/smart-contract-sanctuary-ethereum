...
interface IBridge {
    ...
    function relayAndVerifyView(bytes calldata data)
        external
        view
        returns (Result memory);

    function relayAndVerifyModern(bytes calldata data)
        external
        view
        returns (Result memory);
...
contract MockBridgeUser {

    IBridge public bridge;

    uint256 public count = 0;
    mapping(uint256 => IBridge.Result) public results;

    constructor(IBridge _bridge) {
        bridge = _bridge;
    }

    function setBridge(IBridge _bridge) public {
        bridge = _bridge;
    }

    function latestResult() public view returns(IBridge.Result memory) {
        return results[count];
    }

    function saveResult(IBridge.Result memory r) internal {
        results[count] = r;
        count++;
    }

    function testRelayAndVerifyMutate(bytes memory proof) public {
        saveResult(
            bridge.relayAndVerify(proof)
        );
    }

    function testRelayAndVerifyView(bytes memory proof) public {
        saveResult(
            bridge.relayAndVerifyView(proof)
        );
    }

    function testRelayAndVerifyModern(bytes memory proof) public {
        saveResult(
            bridge.relayAndVerifyModern(proof)
        );
    }

    function encodeResult(IBridge.Result memory r) public pure returns(bytes memory) {
        return abi.encode(r);
    }

    function getResultHash(IBridge.Result memory r) public pure returns(bytes32) {
        return keccak256(encodeResult(r));
    }

    function recoverHash(uint8 v, bytes32 r, bytes32 s, bytes32 h) public pure returns(address) {
        return ecrecover(h,v,r,s);
    }

    function encodeModernProof(
        IBridge.Result memory result,
        bytes32[] memory rs,
        bytes32[] memory ss,
        bytes32 vs
    ) public pure returns(bytes memory) {
        return abi.encode(vs, rs, ss, result);
    }
