...
contract Operators is Ownable {
    ...
    function renounceOperator() public onlyOperator() {
        ...
        emit OperatorUpdated( msg.sender, false );
    }

    function setOperator(address _operator, bool _en) public onlyOwner() {
        operators[_operator] = _en;
...
contract Authorization {
    bytes32 private constant NAME_SLOT = 0xf82181b5877b752c35b4d9c89e2995fcbd02b0dafcd60f5a3b74a1271377d80c;
    // 0x411722ef2d9189425df092ecebbaebb7213ef8022a94cc06c316c4041c945121 = keccak256("MARKI DAO TREASURY");
    bytes32 private constant NAME_HASH = 0x411722ef2d9189425df092ecebbaebb7213ef8022a94cc06c316c4041c945121;
...
contract MakiTreasury is Operators, Authorization {
    ...
    constructor (
        ...
        require( _Maki != address(0) );
        Maki = _Maki;

        isReserveToken[ _sMaki ] = true;
        reserveTokens.push( _sMaki );
    ...
    function deposit( uint _amount, address _token, uint _profit ) external returns ( uint send_ ) {
        ...
        IERC20Mintable( Maki ).mint( msg.sender, send_ );
    ...
    function withdraw( uint _amount, address _token ) external {
        ...
        uint value = valueOf( _token, _amount );
        IOHMERC20( Maki ).burnFrom( msg.sender, value );
    ...
    function incurDebt( uint _amount, address _token ) external {
        ...
        uint maximumDebt = IERC20( sMaki ).balanceOf( msg.sender ); // Can only borrow against sOHM held
    ...
    function repayDebtWithOHM( uint _amount ) external {
        ...
        IOHMERC20( Maki ).burnFrom( msg.sender, _amount );

        debtorBalance[ msg.sender ] = debtorBalance[ msg.sender ].sub( _amount );
        totalDebt = totalDebt.sub( _amount );

        emit RepayDebt( msg.sender, Maki, _amount, _amount );
    }
    ...
    function mintRewards( address _recipient, uint _amount ) external {
        ...
        IERC20Mintable( Maki ).mint( _recipient, _amount );
    ...
    function excessReserves() public view returns ( uint ) {
        return totalReserves.sub( IERC20( Maki ).totalSupply().sub( totalDebt ) );
    }
     ...
     */
    function auditReserves() external onlyOperator() {
        uint reserves;
    ...
    function valueOf( address _token, uint _amount ) public view returns ( uint value_ ) {
        if ( isReserveToken[ _token ] ) {
            ...
            value_ = _amount.mul( 10 ** IERC20( Maki ).decimals() ).div( 10 ** IERC20( _token ).decimals() );
    ...
    /**
        ...
        @return bool
     */
    function queue( MANAGING _managing, address _address ) external onlyOperator() returns ( bool ) {
