...
contract Bond is BondBase {
    ...
    function takerToSwap(
        ...
        amount = bondAmount * bondConfig[token].z / DIVIDER;
    }
    ...
    function makerToSwap(
        ...
        amount = bondAmount * bondConfig[token].k / DIVIDER;
    }
}
...
abstract contract BondBase is Initializable, BACUpgradable, IBond {
    ...
    function setBondConfig(address token,  uint x, uint y, uint z, uint k) external onlyGovernance {
        bondConfig[token].x = x;
        bondConfig[token].y = y;
        bondConfig[token].z = z;
        bondConfig[token].k = k;
    }


    /// @inheritdoc	IBond
    function deposit(address depositTo, uint256 amount) public virtual override {
    ...
    function takerToSwap(
        address token,
        uint bondAmount
    ) public view virtual override returns (uint amount);
    ...
    function bondTakerPosition(
        ...
        uint16 risk, 
        uint16 term
    ) external override onlyMarket(token) returns (uint bondAmount, uint toTransfer, uint toReduce) {
        (bondAmount, toTransfer, toReduce) = calcBondSizeForTakerPosition(recipient, token, amount, risk, term);
        _bondPosition(recipient, bondAmount, toTransfer);
    }
    ...
    function bondMakerPosition(
        ...
        (bondAmount, toTransfer, toReduce) = calcBondSizeForMakerPosition(recipient, token, amount, risk, term);
        _bondPosition(recipient, bondAmount, toTransfer);
    }
    ...
    function calcBondSizeForTakerPosition(
        ...
        uint16 risk, 
        uint16 term
    ) public view override returns (uint toLock, uint toTransfer, uint toReduce) {
        ...
        else if(bondBalance + IERC20Upgradeable(bondToken).balanceOf(recipient) >= toLock) {
    ...
    function lock(address marketToken, address addr, uint amount) external override onlyMarket(marketToken) {
        _lock(addr, amount);
    }
    ...
    function _bondPosition(
        ...
        if(toTransfer > 0) {
            ...
            IERC20Upgradeable(bondToken).safeTransferFrom(recipient, address(this), toTransfer);
        } else {
