...
contract EtherArena is ERC721, ERC721URIStorage, Pausable, Ownable, ERC721Burnable, VRFConsumerBaseV2 {
    ...
    event endGame(uint256 tokenId, uint256 powerLevel);
    ...
    function endFight(uint256 winnerTokenId) public {
        ...
        currentRound ++;
        emit endGame(winnerTokenId, fighters[winnerTokenId].powerLevel);
    }

    function upgradeFighter(uint256 _tokenId) public {

        if (fighters[_tokenId].round < currentRound) {
            fighters[_tokenId].round = currentRound;
        }
        
        fighters[_tokenId].powerLevel += 3333;     
    }
