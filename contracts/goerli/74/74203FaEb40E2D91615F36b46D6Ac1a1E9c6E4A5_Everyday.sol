...
contract Everyday is ERC721, ERC721Enumerable, AccessControl {
  using Address for address payable;
  using Math for uint256;

  uint256 public price = 0.01 ether;
  uint256 public limitPerTnx = 10;

  uint256 public highestTokenId = 1;
  ...
  // split percent as basis points (e.g 250 is 2.5%). noah is remainder
  uint256 private splitJacob = 500; // 5%
  uint256 private splitHenry = 250; // 2.5%
  uint256 private splitMatt = 250; // 2.5%
  ...
  function setWallets(
    ...
    require(noah != address(0x0) && jacob != address(0x0) && henry != address(0x0) && matt != address(0x0), 'invalid input');
  ...
  function setLimitPerTnx(uint256 amount) external onlyRole(DEFAULT_ADMIN_ROLE) {
    limitPerTnx = amount;
  }

  function setHighestTokenId(uint256 tokenId) external onlyRole(DEFAULT_ADMIN_ROLE) {
    // validate input
  ...
  function multiMint(uint256[] calldata tokenIds) public payable {
    // ensure price is met
  ...
  function adminMint(uint256[] calldata tokenIds, address to) public onlyRole(DEFAULT_ADMIN_ROLE) {
