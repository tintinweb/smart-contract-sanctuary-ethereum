...
contract Pool is Ownable, ReentrancyGuard, Pausable, Core, IPool {
    ...
    /// @notice Fee rewarder
    ICashmereDAOProfitRewarder public feeRewarder;
    ...
    /// @notice An event emitted when fee rewarder is updated
    event FeeRewarderUpdated(address indexed previousRewarder, address indexed newRewarder);
     ...
     * @param priceOracle_ new pool's priceOracle addres
     */
    ...
    /**
     * @notice Changes the pools feeRewarder. Can only be set by the contract owner.
     * @param feeRewarder_ new pool's feeRewarder addres
     */
    function setFeeRewarder(address feeRewarder_) external onlyOwner {
        require(feeRewarder_ != address(0), "ZERO");
        emit FeeRewarderUpdated(address(feeRewarder), feeRewarder_);
        feeRewarder = ICashmereDAOProfitRewarder(feeRewarder_);
    }
    ...
    function _deposit(
        ...
        if (fee > 0) {
            address underlyingToken = asset.underlyingToken();
            IERC20(underlyingToken).approve(address(feeRewarder), fee);
            feeRewarder.depositFees(underlyingToken, fee);
        }
    }
    ...
    function withdrawFromOtherAsset(
        ...
        if (fee > 0) {
            address underlyingToken = wantedAsset.underlyingToken();
            IERC20(underlyingToken).approve(address(feeRewarder), fee);
            feeRewarder.depositFees(underlyingToken, fee);
        }
    ...
    function swap(
        ...
        if (haircut > 0) {
            address underlyingToken = fromAsset.underlyingToken();
            IERC20(underlyingToken).approve(address(feeRewarder), haircut);
            feeRewarder.depositFees(underlyingToken, haircut);
        }
