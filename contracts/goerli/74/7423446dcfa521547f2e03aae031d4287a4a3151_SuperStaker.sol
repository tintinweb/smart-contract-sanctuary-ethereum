...
contract SuperStaker is ReentrancyGuard {
    ...
    function _extract32(bytes calldata data) private returns (uint32[] memory) {
        uint256 len = data.length / 4;
        uint32[] memory dataList = new uint32[](len);

        for (uint256 i = 0; i < len; i++) {
            dataList[i] = uint32(bytes4(data[i*4:(i+1)*4]));
        }

        return dataList;
    }

    function _extract(bytes calldata data, uint256 size) private returns (bytes[] memory) {
        uint256 len = data.length / size;
        bytes[] memory dataList = new bytes[](len);

        for (uint256 i = 0; i < len; i++) {
            dataList[i] = data[i*size:(i+1)*size];
        }
    ...
    function _registerValidator(bytes calldata data, bytes calldata pubkey) private {
        uint256 len = uint256(bytes32(data[0:32]));
        uint256 amount = uint256(bytes32(data[32:64]));
        bytes[] memory sharesEncrypted = _extract(data[64:64+416*len], 416);
        bytes[] memory sharesPubkeys = _extract(data[64+416*len:64+464*len], 48);
        uint32[] memory operatorIds = _extract32(data[64+464*len:64+468*len]);
        ssv.registerValidator(pubkey, operatorIds, sharesPubkeys, sharesEncrypted, amount);
    }

    function _updateValidator(bytes calldata data, bytes calldata pubkey) private {
        uint256 len = uint256(bytes32(data[0:32]));
        uint256 amount = uint256(bytes32(data[32:64]));
        bytes[] memory sharesEncrypted = _extract(data[64:64+416*len], 416);
        bytes[] memory sharesPubkeys = _extract(data[64+416*len:64+464*len], 48);
        uint32[] memory operatorIds = _extract32(data[64+464*len:64+468*len]);
        ssv.updateValidator(pubkey, operatorIds, sharesPubkeys, sharesEncrypted, amount);
    }

    function registerBatchValidator(bytes[] calldata data) external nonReentrant {
        uint256 totalAmount = 0;
        for (uint256 i = 0; i < data.length; i++) {
            totalAmount += uint256(bytes32(data[i][32:64]));
        }
        IERC20(ssvToken).transferFrom(msg.sender, address(this), totalAmount);

        for (uint256 i = 0; i < data.length; i++) {
            uint256 len = uint256(bytes32(data[i][0:32]));
            bytes calldata pubkey = data[i][64+468*len:112+468*len];
            _registerValidator(data[i], pubkey);
        }
    }
    ...
    function registerAndStakeBatchValidator(
        ...
        require(msg.value > 32 ether * data.length, "Not enough ether");

        uint256 totalAmount = 0;
        for (uint256 i = 0; i < data.length; i++) {
            totalAmount += uint256(bytes32(data[i][256:288]));
        }
        IERC20(ssvToken).transferFrom(msg.sender, address(this), totalAmount);

        for (uint128 i = 0; i < data.length; i++) {
            bytes calldata pubkey = data[i][16:64];
            _deposit(data[i]);
            _registerValidator(data[i][224:], pubkey);
        }
    }

    function updateBatchValidator(bytes[] calldata data) external nonReentrant {
        uint256 totalAmount = 0;
        for (uint256 i = 0; i < data.length; i++) {
            totalAmount += uint256(bytes32(data[i][32:64]));
        }
        ...
        for (uint256 i = 0; i < data.length; i++) {
            uint256 len = uint256(bytes32(data[i][0:32]));
            bytes calldata pubkey = data[i][64+468*len:112+468*len];
            _updateValidator(data[i], pubkey);
        }
    }
}