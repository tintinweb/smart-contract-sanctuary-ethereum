...
import "./AbodeBeacon.sol";
import "./AbodeCollection.sol";
import "@openzeppelin/contracts/proxy/beacon/BeaconProxy.sol";

contract AbodeFactory is Ownable{
    uint256 public counter;
    mapping(uint256 => address) private abodeCollections;
    AbodeBeacon immutable beacon;

    constructor(address _initBlueprint) {
        beacon = new AbodeBeacon(_initBlueprint);
        counter = 1;
    }
    ...
    function buildCollection(
        ...
        BeaconProxy ship = new BeaconProxy(
            address(beacon),
            abi.encodeWithSelector(AbodeCollection(address(0)).initialize.selector,
            msg.sender,
            _name,
            _symbol,
            _initBaseURI,
        ...
        abodeCollections[counter] = address(ship);
        counter++;
    }

    function getAbodeAddress(uint32 collectionId) external view returns (address) {
        return abodeCollections[collectionId];
    }
...
contract AbodeBeacon is Ownable {
...
contract AbodeCollection is Initializable, ERC721Upgradeable, OwnableUpgradeable, 
ERC721URIStorageUpgradeable {
    using SafeMath for uint256;
    using Strings for uint256;
    using Counters for Counters.Counter;
    /// @dev declaration of variables and structures 

    /// @dev new role created for validators
    ...
    /// @dev struct created to save the jurisdiction
    struct ObjAddress{
        address _address;
        string country;
    }
    ...
    /// @dev modifier created to validate minting
    ...
    /// @dev modifier created to validadate jurisdiction
    ...
    /// @dev modifier created to validadate transfers
    ...
    /// @dev minting of the tokens where the user need to pass the quantity argument
    function safeMint(uint256 quantity) public payable isPausedMint{
        ...
        require(quantity + balanceOf(msg.sender) <= maxMintAmount || msg.sender == this.owner(),"You are exceeding the max of amount of tokens per user!");
        require((msg.value >= price.mul(quantity) || msg.sender == this.owner()), "You have to pay the price in order to mint this token");
    ...
    /// @dev function created to be used only inside this contract
    ...
    function safeTransferFrom(address from, address to, uint256 tokenId, bytes memory _data)
        public
        override
        isOnlyByJurisdiction(to) transferToken(from, to, tokenId)
    {
    ...
    function transferFrom(
        ...
        address to,
        uint256 tokenId
    ) public virtual override isOnlyByJurisdiction(to) transferToken(from, to, tokenId){
    ...
    /// @dev if the user doesn't pass the regular validations can clain for refunds
    function rollback(address payable tokenOwner, uint256 _refundAmount) external onlyOwner {
      ...
      for(uint i = ownerTokens[tokenOwner].length; i > 0 ; i--){
        arrayTokensBurned.push(ownerTokens[tokenOwner][i-1]);
        _transfer(tokenOwner, address(this), (ownerTokens[tokenOwner][i-1]));
      }
    ...
    /// @dev return the URI associated to a tokenId
    ...
    function setOnlyByJurisdiction(bool _value) public onlyOwner{
     ...
     */
    function withdrawMoneyTo(address to, uint _amount) public onlyOwner returns(bool) {
        require(_amount <= address(this).balance, "the amount is higher than the balance");
        address payable payableTo = payable(to);
        (bool sent, ) = payableTo.call{value: _amount}("");
        return sent;
    }
}
