...
contract OhoohHaHaa is Ownable, ERC721A {
    ...
    uint256 public MAX_WHITELIST = 22; 
     ...
     */
    function WMint(uint256 _quantity, bytes32[] calldata _proof)
        external
        isWlSaleActive
    ...
    function CrewMonkeLanding() 
        external 
        payable 
        validateSupply(maxSupply - MAX_WHITELIST, MAX_TEAM_RESERVE)
        onlyOwner 
    {
    ...
    function airdropToHolders(address[] calldata holderAddresses) external onlyOwner{
        for (uint i = 0; i < holderAddresses.length; i++) {
            _safeMint(holderAddresses[i], 1);
        }
    }
