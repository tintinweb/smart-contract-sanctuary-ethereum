...
contract CNPCEma is ERC721AQueryable, Ownable {
    ...
    ICNPC public cnpc;
    bool isCNPCsale = true;
    uint256 public discountCost = 0.0025 ether;
    uint256 public borderAmount = 2;
    ...
    // external
    function mint(uint256 _mintAmount, address recipient) external payable {
        ...
        if(address(cnpc)==address(0)){
            if(isCNPCsale==true){
                if(cnpc.balanceOf(_msgSender()) >= borderAmount){
    ...
    // function minted(address _address) external view returns (bool) {
    ...
    function setDiscountCost(uint256 _newCost) external onlyOwner {
        discountCost = _newCost;
    }

    function setBorderAmount(uint256 _newBorder) external onlyOwner {
        borderAmount = _newBorder;
    }
