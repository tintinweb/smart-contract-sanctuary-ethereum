...
contract LittleMutants is AccessControl, ERC2981, ERC721Enumerable, ERC721Burnable, ERC721Pausable, ReentrancyGuard {
    ...
    uint256 public constant WL_MINT_CAP = 2500;
    uint256 public RESERVED = 100;
    uint256 public MINT_TRANSACTION_LIMIT = 5;
    ...
    function mint(uint256 amount) external payable nonReentrant {
        ...
        require( amount <= MINT_TRANSACTION_LIMIT, "Exceeds mint transaction limit");
    ...
    function mintWL(uint256 amount, bytes calldata signature) external payable nonReentrant {
        uint256 supply = totalSupply();
        require( supply + amount <= WL_MINT_CAP + 1, "Exceeds maximum supply" );
        require( amount <= MINT_TRANSACTION_LIMIT, "Exceeds per transaction limit" );
        require( msg.value >= MINT_PRICE * amount, "Incorrect ether amount" );
    ...
    /**
     * @dev Sets the mint transaction limit
     */
    function setMintTxnLimit(uint256 _mintTxnLimit) external onlyOwners {
        MINT_TRANSACTION_LIMIT = _mintTxnLimit;
    }

    /**
     * @dev Get the mint transaction limit
     */
    function getMintTxnLimit() external view returns (uint256) {
        return MINT_TRANSACTION_LIMIT;
    }
