...
contract BridgeContract {
    ...
    // Original token-----: address is coming from the second network and we need to check his equivalent in ethereum network
    function unlock(bytes32 txHash,address originalToken, uint256 amount) public {
    ...
    function mint(bytes32 txHash,address sourceToken, uint256 amount) public {
    ...
    // call it in separate service after the wrapped token is created
    function setWrappedToSourceToken(address wrappedToken, address sourceToken) public {
        require(
            wrappedToSourceTokenMap[wrappedToken] == address(0),
            "Token mapping already exists"
        );
