...
// NEED TO REMOVE PAUSABLE
contract sample_v1 is Context, ERC20, ERC20Burnable, Pausable, Ownable {
    using Math for uint256;
    using ABDKMath64x64 for int128;
    using ABDKMath64x64 for uint256;

    // INTERNAL TYPE TO DESCRIBE A VVDDRR MINT INFO
    struct MintInfo {
        address user;
        uint256 term;
    ...
    // Here setting a max supply
    uint256 public constant MAX_SUPPLY = 21_000_001;

    uint256 public constant MAX_CLAIMS = 2_628_001;
    // needs to be incremented
    uint256 public claimCount;

    // Base amount in Wei that is rewarded
    // Value in ether is 0.3805175038
    uint256 public constant CLAIM_REWARD_BASE = 380517503800000000;

    // Base amount in Wei that is rewarded
    // Value in ether is 0.3805175038
    uint256 public constant BLOCK_REWARD_BASE = 3805175038100;


    // this value is used to get the baseline values of tokens distributed
    // needs to be a fractional number or just removed
    // is 86_400
    uint256 public constant SECONDS_IN_DAY = 3_600 * 24;
    // uint256 public constant DAYS_IN_YEAR = 365;

    // Will be slightly over one million
    // uint256 public totalClaimed = 0;

    uint256 public constant GENESIS_RANK = 1;
    // TEMPORARY NEEDS TO BE SET BACK!!! *************************************
    ...
    // this is the value that needs to guessed
    // it is set as a value between 1 and 1_000_000 each time that the value increases

    // Need to have percentage roll off calculation

    // Whatever you get from claim you get percentage of what you holder

    // NEED TO EXPRESS MINT TERM RELATIVE TO 1_000_000
    // the minimum term is set to one day
    // here the multiplier is giving a high numerical base amount
    // thus minimum T is set to 86399 to allow use of >
    ...
    // This decreases by .1 percent for each 300_000 decrease in global rank
    ...
    function _calculateMaxTerm() private view returns (uint256) {
        if (globalRank > TERM_AMPLIFIER_THRESHOLD) {
            ...
            // here there is a max term end of 1000
            // return is in seconds
            return Math.min(newMax, MAX_TERM_END);
        }
        // retuns is in seconds
    ...
    /**
     * @dev cleans up User Mint storage
     */
    ...
    function _calculateEAARate() public view returns (uint256) {
        // This uses globalRank because it IS what is used to calculate cRank
        // So at that point in time the EAA is stored for that user the cRank is not available
    ...
    function getGrossReward(
        ...
        // uint256 termBasis = term * 100;

        // uint256 claimMaxTermBasis = claimMaxTerm * 100;

        // uint256 termPercentage = (BASIS + getPercentage(termBasis, BASIS, claimMaxTermBasis));
    ...
    function claimRank(uint256 term) external {
        // everything is being done in seconds
        // Require that users have not exceeded the total allowed number
        require(claimCount < MAX_CLAIMS);
        ...
        // this is a mapping
        // need to add mapping to rig purchases
        ...
        MintInfo memory mintInfo = MintInfo({
            ...
            rank: globalRank,
            // at this point globalRank will be equal to cRank
            eaaRate: _calculateEAARate()
        });
        userMints[_msgSender()] = mintInfo;
        // Could be the same so just one a single one of these
        activeMinters++;
        claimCount++;
    ...
    /**
     * @dev ends minting upon maturity (and within permitted Withdrawal Time Window), gets minted VVDDRR
     */
    ...
    //     // reward amount needs to be in Wei
    //     // base reward amount is 380517503800000000
    ...
    // need to separate the controllers for fixed and variable staking
    // Will thus have two separate classes of controllers that each have a max supply cap

    // This needs to have a burn mechanism implemented along with the mint process
    // Use varied calculation with the burn at mint to randomize the final remaining value


    function setGenesisTimestamp() public onlyOwner {
      require(setGenesisOnce == false);
    ...
    // Base controller and address mapping
    ...
    */
    function addFfxxddController(address controller) external onlyOwner {
        require(setFfxxddController == false);
    ...
    function addVvrrbbController(address controller) external onlyOwner {
        require(setVvrrbbController == false);
        controllers[controller] = true;
    ...
    */
    ...
    /**
    * mints $VVDDRR to a recipient
    * @param to the recipient of the $VVDDRR
    * @param amount the amount of $VVDDRR to mint
    */
    ...
    /**
    * burns $VVDDRR from a holder
    * @param from the holder of the $VVDDRR
    * @param amount the amount of $VVDDRR to burn
    */
    ...
    /**
    * transfers $VVDDRR
    * @param recipient the recipient of $VVDDRR
    * @param amount the amount of $VVDDRR to transfer
    */
