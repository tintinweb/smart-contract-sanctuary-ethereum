...
contract ChainpassEventFactoryV1_0 is AuthHelper, ReentrancyGuard {
    struct EventInfo {
        bool exists;
    ...
    constructor(address  _chainpassRegistry) AuthHelper(_chainpassRegistry) {
        chainpassRegistry = _chainpassRegistry;
    }

    function createEvent(address eventHost, MetaTransactionInput memory metaTXInput) external nonReentrant onlyMetaChainpassWallet(metaTXInput) {
        ChainpassEventTicketsV1_0 chainpassEventTickets = new ChainpassEventTicketsV1_0(chainpassRegistry);
        ...
        chainpassEventTickets.transferOwnership(eventHost);

        emit EventCreated(eventHost, address(chainpassEventTickets));
    }
    ...
    function proposePrimaryBPS(
        ...
        if (isMetaChainpassWallet(metaTXInput)) { // Chainpass is proposing a primary bps.
        ...
        if (isSignerOrCaller(ticketContract.owner(), metaTXInput)) { // The event host is proposing a primary bps.
...
contract AuthHelper is Ownable {
    address chainpassRegistryContractAddress;
    mapping(address => mapping(bytes32 => bool)) usedNoncesBySignerAddress;

    constructor(address _chainpassRegistryContractAddress) {
        chainpassRegistryContractAddress = _chainpassRegistryContractAddress;
    }

    modifier onlyMetaChainpassWallet(MetaTransactionInput memory metaTXInput) {
        require(
            isMetaChainpassWallet(metaTXInput),
            'Only the chainpass wallet address can call this method.'
        );

        _;
    }

    modifier onlyChainpassWallet() {
        require(
            isChainpassWallet(),
            'Only the chainpass wallet address can call this method.'
        );
    ...
    modifier onlyMetaOwner(MetaTransactionInput memory metaTXInput) {
        require(
            isMetaOwner(metaTXInput),
    ...
    function isMetaChainpassWallet(MetaTransactionInput memory metaTXInput) public returns (bool) {
        address chainpassWallet = IChainpassRegistry(chainpassRegistryContractAddress).getChainpassWallet();
        return msg.sender == chainpassWallet || isSigner(chainpassWallet, metaTXInput);
    }
    ...
    function isMetaOwner(MetaTransactionInput memory metaTXInput) public returns (bool) {
        return msg.sender == owner() || isSigner(owner(), metaTXInput);
    }
    ...
    function isSignerOrCaller(address signer, MetaTransactionInput memory input) public returns (bool) {
        return msg.sender == signer || isSigner(signer, input);
    }

    function isSigner(address signer, MetaTransactionInput memory input) public returns (bool) {
        require(!usedNoncesBySignerAddress[signer][input.nonce], "nonce already used");
        usedNoncesBySignerAddress[signer][input.nonce] = true;
        bytes32 msgHash = keccak256(abi.encode(address(this), signer, input.nonce));
        return verifySignature(msgHash, input.r, input.s, input.v) == signer;
    }
...
contract ChainpassEventTicketsV1_0 is AuthHelper, ERC721A, ReentrancyGuard {
    ...
    constructor(address  _chainpassRegistry) AuthHelper(_chainpassRegistry) ERC721A("Chainpass Event Tickets V1.0", "CP1.0") {
        chainpassRegistry = _chainpassRegistry;
    }
    ...
    function issueTickets(
        ...
        uint256 price,
        MetaTransactionInput memory metaTXInput
    ) external onlyMetaChainpassWallet(metaTXInput) {
        uint256 startTokenId = _nextTokenId();
        _safeMint(eventHost, amount);

        IChainpassMarketplace cpMarketplace = marketplace();
        cpMarketplace.setPrimaryAsks(address(this), startTokenId, amount, price);

        adminSetApprovalForAll(eventHost, address(cpMarketplace), true);
    ...
    function marketplace() private view returns (IChainpassMarketplace) {
        return IChainpassMarketplace(registry().getChainpassMarketplaceContractAddress());
    }
    ...
    function setRoyalty(Royalty memory _royalty, MetaTransactionInput memory metaTXInput) external onlyMetaOwner(metaTXInput) {
        royalty = _royalty;
    }
