...
contract WhosJi is ERC721A, Owned(msg.sender), ReentrancyGuard {
    ...
    uint256 private constant _PRICE = 0.033 ether;
    uint256 private constant _MAX_MINT = 3;
    uint256 private constant _MAX_MINT_OG = 1;
    uint256 private constant _AVAILABLE_WHITELIST = 2755;
    uint256 private constant _MAX_SUPPLY = 5555;
    ...
    //Merkle root for OG and Whitelist mints.
    ...
    function mintPublic(uint256 amount)
        ...
        payable
        nonReentrant
        requireMintablePublic(_MAX_MINT, amount, msg.sender)
    ...
    function mintOG(bytes32[] calldata proof)
        ...
        payable
        nonReentrant
        requireProof(proof, msg.sender, _MAX_MINT_OG, msg.value)
    ...
    function mintWhitelist(uint256 amount, bytes32[] calldata proof)
        ...
        payable
        nonReentrant
        requireProof(proof, msg.sender, amount, 0)
    ...
    modifier requireProof(
        ...
        require(
            MerkleProofLib.verify(
                ...
                merkleRoot,
                keccak256(
                    abi.encodePacked(addy, _toString(amount), _toString(price))
                )
            ),
            "INVALID_PROOF"
        );
    ...
    modifier requireMintablePublic(
        ...
        unchecked {
            ...
            require(
                _totalMinted() + quantity + _AVAILABLE_WHITELIST <= _MAX_SUPPLY,
                "MAX_SUPPLY_PUBLIC"
            );
        }
        _;
    ...
    modifier requireMintableWhitelist(address addy, uint256 amount) {
        ...
        unchecked {
            require(_totalMinted() + amount <= _MAX_SUPPLY, "MAX_SUPPLY");
        }
        _;
    }
    ...
    function adminMint(uint256 amount) external payable onlyOwner {
        unchecked {
            require(amount + _totalMinted() <= _MAX_SUPPLY, "MAX_SUPPLY");
        }
