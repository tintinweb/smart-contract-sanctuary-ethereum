...
contract Primes is ERC721r, Ownable {
        ...
        for (y = 0; y <= 800; y += 100) {
            for (x = 0; x <= 800; x += 100) {
                ...
                if (PrimesUtils.isPrime(uint256(count))) {
                    ...
                    string memory minV = PrimesUtils.getMinValuesAnimation(
                        x,
                        y
                    );
                    string memory maxV = PrimesUtils.getMaxValuesAnimation(
                        x,
                        y
                    );
                    string memory d = PrimesUtils.getDefinition(x, y);
                    anime = string(
                        abi.encodePacked(
    ...
    function getD(
        uint x,
        uint y
    ) public view virtual returns (string memory d) {
        return PrimesUtils.getDefinition(x, y);
    }
