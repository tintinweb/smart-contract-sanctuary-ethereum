...
contract CrossTokenHub {
    ...
    event TransferOut(uint64 seq, uint32 dstChainId, address indexed dstContractAddr, address indexed recipient, uint256 amount);
    event TransferIn(uint64 seq, uint32 srcchainId, address indexed contractAddr, address indexed recipient, uint256 amount);
    ...
    function addMirrorToken(address contractAddr, address dstContractAddr, uint32 dstChainId) public onlyOwner {
    ...
    function delMirrorToken(address contractAddr, uint32 dstChainId) public onlyOwner {
        bytes32 mkey = getMirrorKey(contractAddr, dstChainId);
        require(mirrors[mkey].dstContractAddr != address(0), "No mirror");
        mirrors[mkey].dstContractAddr = address(0);
    }

    function transferOut(uint32 dstChainId, address contractAddr, uint amount, address recipient) public {
        bytes32 mkey = getMirrorKey(contractAddr, dstChainId);
    ...
    function transferIn(uint64 seq, uint32 srcChainId, address contractAddr, uint amount, address recipient) public onlyOwner {
    ...
    function getMirrorKey(address contractAddr, uint32 chainId) public pure returns(bytes32 key){
        uint256 cid = uint256(chainId) * 2 ** 224;
        assembly{
