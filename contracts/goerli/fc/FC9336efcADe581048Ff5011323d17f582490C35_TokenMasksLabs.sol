...
contract TokenMasksLabs is ERC721A, Ownable{
    ...
    constructor(
    )ERC721A('ETH MASKS v098','EM'){//コレクション名：ETH MASK, トークンの名前：EM
        setBaseURI('');
    }
    ...
    function mint(uint _mintAmount) public payable{
        ...
        if(_mintAmount > maxMintAmount) revert("max mint amount per session exceede");

        if(supply + _mintAmount > maxSupply) revert("max NFT limit exceede");
    ...
    function airdoropMint(address[] calldata _airdropAddresses, uint[] memory _userMintAmount) public onlyOwner{
        uint supply = totalSupply();
