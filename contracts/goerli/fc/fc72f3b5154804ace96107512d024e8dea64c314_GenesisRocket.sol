    ...
    ERC2981 
{
    ...
    ERC721A("Testing", "Testing"){
        ...
        _setDefaultRoyalty(0x816A69485cEC9F035Bd3398EaBB9E9C408C0bccF, 1000);
    }
    ...
    function setNewTokenURI(uint256 typeOfURI, uint256 tokenId, string calldata newURI)
    external
    onlyRolesOrOwner(ADMIN_ROLE | MINTER_ROLE | FACTORY_ROLE){
        if(typeOfURI == 0) 
            _normalUri = newURI;
        else 
   ...
   function tokenURI(uint256 tokenId) public view virtual override(ERC721A, IERC721A) returns (string memory) {
    ...
    onlyRolesOrOwner(ADMIN_ROLE) 
    {
        ...
        emit newMint(currentIndex, msg.sender);
    }

   


        function minterMint(address to)
        external
        onlyRolesOrOwner(MINTER_ROLE)
    {
    ...
    //probably irrelevent 
    function setMaxMintPerWallet(uint256 quantity)
    external 
    onlyRolesOrOwner(ADMIN_ROLE) 
    {
    ...
    // Fail safe incase someone sends eth to the contract for any reason.
    function withdrawMoney(address to)
    external
    onlyRolesOrOwner(ADMIN_ROLE)
    {
    ...
    // Authoried Contract for burn/migration functionality
    function setAuthorizedContract(address authorizedContract)
    external
    onlyRolesOrOwner(ADMIN_ROLE)
    {
    ...
    // Supports interface for combatibility
