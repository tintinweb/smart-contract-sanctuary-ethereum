...
contract TeamPoolVesting is Ownable, ReentrancyGuard {
    ...
    uint256 public immutable AmountUnlockPerPeriod;
    uint256 public InitialBlock;
    uint256 public immutable BlocksPerPeriod;
    uint256 public MaxWithdrawForNextPeriod;
    ...
    constructor(
        ...
        AmountUnlockPerPeriod = _maxWithdraw;

        MaxWithdrawForNextPeriod = _maxWithdraw;
    ...
    function vestingStart() public nonReentrant onlyOwner {
        ...
        nextBlockForUnlock = InitialBlock + BlocksPerPeriod;
    }

    function unlockdegenIDToken() external nonReentrant onlyOwner {
        require(
            (numberPastUnlocks == TotalPeriods) ||
                (block.number >= nextBlockForUnlock),
            "Cannot unlock yet!"
        );

        uint256 balanceToWithdraw = DegenIDToken.balanceOf(address(this));

        if (numberPastUnlocks < TotalPeriods) {
            // Adjust next block for unlock
            nextBlockForUnlock += BlocksPerPeriod;
            // Adjust number of past unlocks
            numberPastUnlocks++;

            if (balanceToWithdraw >= MaxWithdrawForNextPeriod) {
                // Adjust balance to withdraw to match linear schedule
                balanceToWithdraw = MaxWithdrawForNextPeriod;
                MaxWithdrawForNextPeriod = AmountUnlockPerPeriod;
            } else {
                // Adjust next period maximum based on the missing amount for this period
                MaxWithdrawForNextPeriod =
                    MaxWithdrawForNextPeriod +
                    (MaxWithdrawForNextPeriod - balanceToWithdraw);
            }
        }

        DegenIDToken.safeTransfer(msg.sender, balanceToWithdraw);
