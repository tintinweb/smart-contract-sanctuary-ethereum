...
contract SenseiStaking is Ownable, ERC721Holder {
    ...
    event LockDownExpirationChanged(address user, uint256 tokenId, uint256 lockdownExpiration);
    ...
    //modify to return tokenids and lockdownExpiration
    ...
    function stakeBatch(uint256[] memory tokenIds, uint256 _lockdownExpiration) public {
        require(tokenIds.length > 0,
        "You are trying to stake 0 tokens");
    ...
    // Function called stake that updates an array but doesn't actually stake :flushed:
    function _stake(address _user, uint256 _tokenId, uint256 _lockdownExpiration) internal {
        require(enabled, "Staking System: the staking has not started");
        require(_lockdownExpiration > block.timestamp, "Hello zion, you have been a bad boy trying to go back in time"); // im sorry
        require(
        ...
        // ---->>> not staked | senseiNFT.safeTransferFrom(_user, address(this), _tokenId);
    ...
    function unstakeBatch(uint256[] memory tokenIds) public  {
        ...
        require(tokenIds.length > 0,
        "You are trying to unstake 0 tokens");
        for (uint256 i = 0; i < tokenIds.length; i++) {
    ...
    function _unstake(address _user, uint256 _tokenId) internal {
        require(enabled, "Staking System: the staking system is not enabled, cannot unstake tokens.");
        require(
            block.timestamp >= stakedToken[_tokenId].lockDownExpiration,
            "Your token is currently locked down"
        );
        require(stakedToken[_tokenId].owner == _user,
        "You are not the owner if this token or token is not staked");

        delete stakedToken[_tokenId];
    ...
    function updateLockDown(uint256 _lockdownExpiration, uint256 _tokenId) public onlyOwner {
        require(_lockdownExpiration > block.timestamp);
        require(enabled, "Staking System: the staking has not started");

        stakedToken[_tokenId].lockDownExpiration = _lockdownExpiration;
        emit LockDownExpirationChanged(stakedToken[_tokenId].owner, _tokenId, _lockdownExpiration);
