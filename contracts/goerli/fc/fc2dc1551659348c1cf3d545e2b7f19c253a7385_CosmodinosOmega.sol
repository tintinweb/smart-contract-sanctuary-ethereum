...
contract CosmodinosOmega is Ownable, ERC721A, ReentrancyGuard {
    ...
    uint256 public immutable amountForDevs = 150;
    ...
    function allowlistMint(
        ...
        require(quantity <= total, "quantity error");
        uint256 totalPrice = allowlistPrice.mul(quantity);
        require(msg.value >= totalPrice, "Need to send more ETH.");
        require(_verify(_leaf(msg.sender, total), proof), "Invalid merkle proof");
        uint256 numberToClaim = quantity.sub(_numberMinted(msg.sender));
