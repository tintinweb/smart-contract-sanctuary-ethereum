...
//0xFc9D4E396678e1e8112b177A30F0Bddec27CEEBC
...
DefaultOperatorFilterer
{
    ...
    uint internal _editionPrice = 0.1 ether;
    ...
    constructor(
        ...
        super._setDefaultRoyalty(royaltyReceiver, royaltyFeesInBips);
         ...
         checksOriginals = Checks(0xd95Cc6772873C03f464Ff23696DA51fE962c924f);
    }

    //Contract logic

    function setTokenUri(uint256 tokenId, string calldata metadataHash) external onlyOwner{
    ...
    nonReentrant{
        ...
        uint256[] memory amountsToMint = new uint256[](checksonClaims.length);
         ...
         for (uint i = 0; i < checksonClaims.length; i++) {
            ...
            require(bytes(_metadataHashes[checksonTokenId]).length != 0, "Token not exists");
    ...
    function claimedChecksons() public view returns(bool[] memory){
    ...
    function isCheckClaimed(uint256 checkTokenId) public view returns(bool){
        return _claimedCheckTokens[checkTokenId];
    }

    function isChecksonClaimed(uint256 checksonTokenId) public view returns(bool){
    ...
    function setEditionPriceInEth(uint editionPriceInEthToSet) external onlyOwner{
        _editionPrice = editionPriceInEthToSet * 1 ether;
    }

   function editionPriceInEth()
    public
    view
    returns (uint priceInEth) {
        return _editionPrice / 1 ether;
    }
