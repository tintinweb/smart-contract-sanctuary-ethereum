...
//SPDX-License-Identifier: MIT
/** 
    *   The timeFlow protocol is created by whalehat, which is the timeFlowV1 version。
 */
...
contract dustProtocol is Ownable{
    ...
    //得到所有用户
    function fetchAllUsers()public view returns(thisUser[] memory){
        thisUser[] memory allUsers = new thisUser[](_thisUser.length);
        for(uint256 i=0;i<_thisUser.length;i++){
            allUsers[i]=_thisUser[i];
        }
        return allUsers;
    }

    //得到所有审查者
    function fetchAllSupervisors()public view returns(supervisor[] memory){
        supervisor[] memory allSupervisors = new supervisor[](_supervisor.length);
        for(uint256 i=0;i<_supervisor.length;i++){
            allSupervisors[i]=_supervisor[i];
        }
        return allSupervisors;
    }

    //得到所有事件
    function fetchAllThings()public view returns(contentThing[] memory){
...
contract timeFlow is dustProtocol{
    //合约地址:0xFc54fDc557D7E3d4c4B46969C7DdB83e602e211d
    ...
    //添加审查人事件
    event addSuperHumanEvent(address waitSuperAddress,string waitSuperName);
    //移除审查人事件
    event removeSuperHumanEvent(address waitSuperAddress);
    //创建事务事件
    event createThingEvent(uint256 eventId,string thisName,string thing,uint256 Money,uint256 thisStartTime,uint256 thisEndTime,address startAddress,address receiveAddress,bool sendIf,bool receiveIf);
    //审查者投票事件
    event voteThingEvent(address thisWillVoteAddress,string voteThingName,uint256 allVotedAmount);
    ...
    function voteContent(string memory contentName)public payable{
        ...
        //判断地址是否投票
        require(supervisorVoteProof[msg.sender][getVotedId]!=true,"You are already voted!");
