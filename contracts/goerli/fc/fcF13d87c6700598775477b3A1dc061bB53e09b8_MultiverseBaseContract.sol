...
// SPDX-License-Identifier: MIT
  ...
  // File: contracts/MultiverseBaseContract.sol
  ...
  contract MultiverseBaseContract is ERC721A, ERC2981, Ownable {
    ...
    uint256 public MAX_SUPPLY;
     ...
     constructor() ERC721A("MultiverseBaseC", "MBC") {
          MAX_SUPPLY = 0;
          mintRate = 0;
     ...
     modifier mintCompliance(uint256 _quantity) {
      mintSupplyCheck(_quantity);
      mintPaymentCheck(_quantity);
      _;
      mintSupplyUpdates(_quantity);
    }
  
    function mintSupplyCheck(uint256 _quantity) internal view {
      if(MAX_SUPPLY == 0){
        return;
      }
      require(supply.current() + _quantity <= MAX_SUPPLY, "Not enough nft left");
    }
  
    function mintPaymentCheck(uint256 _quantity) internal view {
      require(msg.value >= (mintRate * _quantity), "Not enough ether sent");
    }
  
    function mintSupplyUpdates(uint256 _quantity) internal {
