...
// @author DeDe
contract NFTPass is ERC2981, ERC721ABurnable, DefaultOperatorFilterer {
    ...
    function mint(
        ...
        require(
            _isValidSignature(
                keccak256(abi.encodePacked(msg.sender, _uri, _formulaType, _totalMint, address(this))),
                _signature
            ),
            "Invalid signature"
        );
        ...
        require(_totalMinted + _totalMint <= mintLimit, "Maximum limit has been reached");

        // mint a token using erc721a
        _safeMint(_to, _totalMint);

        // set token uri
        for (uint256 i = 0; i < _totalMint; i++) {
            _setTokenURI(_totalMinted + i, _uri);
        }
    }
    ...
    /**
     * @dev get total NFT minted
     * @dev burned token will not be considered for the calculation here
     *
     */
