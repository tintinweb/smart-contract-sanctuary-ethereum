...
contract UnimoonStaking is Ownable {
    ...
    {
        uint256 totalPending = pendingRewards(user, 0) +
            pendingRewards(user, 1);
        uint256 totalClaimed = userInfo[0][user].totalClaimed +
            userInfo[1][user].totalClaimed;
        return (totalPending + totalClaimed, totalClaimed, totalPending);
    }
    ...
    /** @dev Function to unstake several deposits from current pool
     * @param pid pool id
     * @param stakeIds indexies of user's deposits in array of all user stakes
     * @param amounts to unstake
     */
    function unstakeMultiple(
        ...
        user.totalStaked -= valueToUnstake;
    ...
    function _claimRewards(uint8 _pid) internal {
        ...
        user.totalClaimed += pendingYieldToClaim;
