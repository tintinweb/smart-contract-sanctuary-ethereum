...
contract AmericanaVault is ERC721ARedeemable("Americana", "VAULT"), Ownable2Step, OperatorFilterer {
    ...
    event BundleCreated(address auxTokenAddress, uint48 auxTokenId, uint48 indexed americanaTokenId, address indexed redeemer);

    event BundleUnwrapped(
        address auxTokenAddress, uint48 auxTokenId, uint48 indexed physicalTokenId, address indexed redeemer, uint256 indexed bundleTokenId
    );
     ...
     */
    function bundle(address _auxTokenAddress, uint48 _auxTokenId, uint48 _americanaTokenId, bytes calldata _sig)
        external
    {
        // Verify sig against the authorized addresses.
        if (
            !isAuthorized[(
                keccak256(abi.encode(_auxTokenAddress, _auxTokenId, _americanaTokenId)).toEthSignedMessageHash().recover(
                    _sig
                )
            )]
        ) {
    ...
    function flipPause() external onlyAuth {
        isPaused = !isPaused;
    }
...
/**
 * @title ERC721ARedeemable
 *
 * @dev Adaptation of Chiru Labs ERC721A (https://github.com/chiru-labs/ERC721A) to permanently store the
 *      mint timestamp within `_packedOwnerships`. Meant to facilitate token redeemability functionality
 *      for Americana (https://www.americana.io/)
 *
 *      Changes:
 *
 *      - The `startTimestamp` bits of `_packedOwnership` are now referred to as `mintTimestamp`
 *         These bits permanently store the timestamp of the tokens mint to be used to determine
 *         whether or not a token is eligible to be redeemed for the physical item.
 *
 *      - Addition of `_packOwnershipDataTransfer()` function that is called instead of `_packOwnershipData()`
 *        every time a token is transfered other than minting. This new function removes the update of the
 *        `mintTimestamp` bits. Minting still calls the original `_packOwnershipData()` function in order to set
 *        that persistent `mintTimestamp` bits.
 *
 *
 */

contract ERC721ARedeemable is IERC721ARedeemable {
    ...
    // The bit position of `mintTimestamp` in packed ownership.
    uint256 private constant _BITPOS_MINT_TIMESTAMP = 160;
    ...
    function _unpackedOwnership(uint256 packed) private pure returns (TokenOwnership memory ownership) {
        ...
        ownership.mintTimestamp = uint64(packed >> _BITPOS_MINT_TIMESTAMP);
        ownership.burned = packed & _BITMASK_BURNED != 0;
        ownership.extraData = uint24(packed >> _BITPOS_EXTRA_DATA);
    }

    /**
     * @dev Packs ownership data into a single uint256 and sets mintTimestamp.
     */
    function _packOwnershipData(address owner, uint256 flags) private view returns (uint256 result) {
        assembly {
            // Mask `owner` to the lower 160 bits, in case the upper bits somehow aren't clean.
            owner := and(owner, _BITMASK_ADDRESS)
            // `owner | (block.timestamp << _BITPOS_MINT_TIMESTAMP) | flags`.
            result := or(owner, or(shl(_BITPOS_MINT_TIMESTAMP, timestamp()), flags))
        }
    }

    /**
     * @dev Packs ownership data into a single uint256 without updating mintTimestamp.
     */

    function _packOwnershipDataTransfer(address owner, uint256 tokenId, uint256 flags)
        private
        view
        returns (uint256 result)
    {
        ...
        assembly {
            // Mask `owner` to the lower 160 bits, in case the upper bits somehow aren't clean.
            owner := and(owner, _BITMASK_ADDRESS)
            // `owner | (mintTimestamp << _BITPOS_MINT_TIMESTAMP) | flags`.
            result := or(owner, or(shl(_BITPOS_MINT_TIMESTAMP, mintTimestamp), flags))
        }
    }
    ...
    function transferFrom(address from, address to, uint256 tokenId) public payable virtual override {
        ...
        unchecked {
            ...
            _packedOwnerships[tokenId] = _packOwnershipDataTransfer(
                to, tokenId, _BITMASK_NEXT_INITIALIZED | _nextExtraData(from, to, prevOwnershipPacked)
            );
    ...
    function _mint(address to, uint256 quantity) internal virtual {
        ...
        unchecked {
            ...
            // - `mintTimestamp` to the timestamp of minting.
            // - `burned` to `false`.
...
//Adapted by 0xFloop to store only mint time in TokenOwnership struct
...
interface IERC721ARedeemable {
    ...
    struct TokenOwnership {
        ...
        address addr;
        // Stores the timestamp that the token was minted at.
        uint64 mintTimestamp;
        // Whether the token has been burned.
        bool burned;
        // Arbitrary data similar to `mintTimestamp` that can be set via {_extraData}.
        uint24 extraData;
    }
