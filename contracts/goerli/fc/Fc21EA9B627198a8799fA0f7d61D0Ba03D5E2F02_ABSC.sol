...
import "./AdvancedCollection.sol"; 
import "@openzeppelin/contracts/utils/Counters.sol";
import "@openzeppelin/contracts/access/AccessControl.sol";


/**
 * @title Artist_Base_Smart_Contract.
 * @author @Dadogg80 - Viken Blockchain Solutions.
 * @notice This contract is deployed by the AdminFactory and represents an artist/band on the Centaurify NFT Marketplace.
 * @notice The main purpose of this contract is to deploy new child collections for the base artist and manage
 *         the royalty splitting for all child contracts of this ABSC.
 * 
 * @dev Features: 
 *         - Deploy new erc721 child collections for the base artist.
 *               
 */

contract ABSC is AccessControl {
    ...
    /**
     * @dev The BaseArtist struct contains the data for the base artist of this contract.
     * @param artistName The name of the artist.
     * @param artistId The id of the artist.
     * @param totalChild The total deployed child-collections.
     * @param royaltySplitter Contract address of the deployed PaymentSplitter.sol.
     */
    struct BaseArtist {
        bytes32 artistName;
        bytes32 artistId;
    ...
    /// @notice Returns the BaseArtist struct.
    BaseArtist public baseArtist;

    /// @notice List with the addresses of all the StandardChild  contracts deployed. 
    address[] public childsArray;

    /// @notice Mapping with all child-collections of the base artist/band.
    mapping (uint256 => AdvancedCollection) public AdvancedCollections;

    /**  
     * @notice Event emitted when a new child collection is deployed.
     * @param child The new deployed child contract.
    */
    event DeployedChild(
        address child
    );

     constructor(address _superAdmin, bytes32 _artistId, bytes32 _artistName) {
        _grantRole(DEFAULT_ADMIN_ROLE, _superAdmin);
        _grantRole(OPERATOR_ROLE, _msgSender());
        baseArtist.artistName = _artistName;
        baseArtist.artistId = _artistId;
        baseArtist.totalChild = _counter.current();
    }

    /// @notice Restricted method used to deploy a new ABSC StandardChild contract. 
    /// @param _superAdmin The _superAdmin is the account with the DEFAULT_ADMIN_ROLE.
    /// @param _operator The account with the OPERATOR_ROLE.
    /// @param _artist The artist account.
    /// @param _ownerMintAmount The amount of tokens to mint at deployment.
        ...
        onlyRole(OPERATOR_ROLE)
    {
        ...
        AdvancedCollection template = new AdvancedCollection{salt: bytes32(_salt)}(_superAdmin, _operator, _artist, _ownerMintAmount, _name, _symbol);       
        AdvancedCollections[baseArtist.totalChild] = template;
        childsArray.push(address(template));
        
        emit DeployedChild(address(template));
    }

    
    /// @notice Method is used to register the deployed RoyaltySplitter.sol address for this ABSC.
    /// @param _royaltySplitter The address of royalty splitter contract for this artist.
    function registerRoyaltySplitter(address _royaltySplitter) external onlyRole(OPERATOR_ROLE) {
        baseArtist.royaltySplitter = _royaltySplitter;
    }
    
    /// @notice Method returns the addresses of all the ABSC deployed collections.
    /// @return childsArray The address of all deployed childcollections from this artist.
    function getChildsArray() external view returns (address[] memory) {
        return childsArray;
    }
...
/// @notice This is the Centaurify Advanced Collection smart contract used for the Artist Collections.
...
/** 
 * @notice Advanced Collection Features:
 *          - Three pre-mint phases.  
...
contract AdvancedCollection is CollectionStorage {
        ...
        ERC721A(_name, _symbol)
    {
        ...
        _mintERC2309(_artist, _ownerMintAmount);
    }
    ...
    /// @param leaf the leaf node of the three.
    /// @param proof the proof from the merkletree.
    function whitelistPhase2Mint(uint256 amount, bytes32 leaf, bytes32[] memory proof) 
        external 
        payable 
        phaseTwoIsOpen 
        costs(amount) 
    ...
    /// @param leaf the leaf node of the three.
    /// @param proof the proof from the merkletree.
    function whitelistPhase3Mint(uint256 amount, bytes32 leaf, bytes32[] memory proof) 
        external 
        payable 
        phaseThreeIsOpen 
        costs(amount) 
    ...
    function getRemainingMints(address user) external view returns (uint256 remaining) {
        Status _status = status;

        if (_status == Status(1)) {
            if (!whitelistPhase1Used[user]) return maxItemsPerTx;
            return whitelistPhase1Remaining[user];
        }

        else if (_status == Status(2)) {
            if (!whitelistPhase2Used[user]) return maxItemsPerTx;
            return whitelistPhase2Remaining[user];
        }

        else if (_status == Status(3)) {
            if (!whitelistPhase3Used[user]) return maxItemsPerTx;
            return whitelistPhase3Remaining[user];
        }
        
        else if (_status == Status(4)) {
            if (!publicMintUsed[user]) return maxItemsPerTx;
    ...
    function _mintWithoutValidation(address to, uint256 amount, bool skipMaxItems) private {
        ...
        _mint(to, amount);
        uint256 _nextToken = _nextTokenId();
...
/// @title Centaurify Collection Storage - CollectionStorage.sol
/// @author @dadogg80 - Viken Blockchain Solutions.

/// @notice This is one of the Centaurify Collection smart contracts used for the Artist/Band collections.
/// @notice This smart contract contains the state variables and admin methods for the Centaurify Collections contracts.


abstract contract CollectionStorage is AccessControl, Ownable, ERC721A, ERC721AQueryable, ERC2981 {
    ...
    /// @notice dev is the receiver of the saveStuckTokens method.
    address internal _dev = payable(0x0c051a1f4E209b00c8E7C00AD0ce79B3630a7401);
    ...
    mapping(address => uint256) internal whitelistPhase1Remaining;
    mapping(address => uint256) internal whitelistPhase2Remaining;
    mapping(address => uint256) internal whitelistPhase3Remaining;
    mapping(address => uint256) internal publicMintRemaining;
    ...
    error MintPhaseEnded(uint256 EndTimestamp);
    ...
    /// @param MintPrice The new price to mint. 
    event NewMintPrice(uint256 MintPrice);
...
/* ------------------------------------------------------------  ADMIN FUNCTIONS  ----------------------------------------------------------- */
    ...
    function setMaxItemsPerTx(uint256 _maxItemsPerTx) external onlyRole(ADMIN_ROLE) publicMintingIsOpen {
    ...
    function removeStuckTokens(address _token, address _to) external onlyRole(DEV_ROLE) {
    ...
    function setTokenRoyalty(uint256 tokenId, address payable _royaltyAddress, uint96 feeNumerator) 
        external
        onlyRole(ADMIN_ROLE)
    {
    ...
    /// @param _royaltyAddress The _royalty address is the adderss that will receive all the royalty from secondary sales.
    /// @param _recipient The _recipient address is the adderss that will receive all the funds from the collection sales.
    /// @param _maxSupply The MAXSUPPLY for this collection.
    ...
    function setPhaseOneMintValues(uint256 _pricePhaseOne, uint256 _maxItemsPerTx, uint256 _startTimestamp, uint256 _endTimestamp, bytes32 _merkleRoot)
        external
        onlyRole(OPERATOR_ROLE)
    {
    ...
    /// @dev Restricted to onlyRole(OPERATOR_ROLE).
    /// @param  _pricePhaseTwo The mint price for Premint Phase 2.
    /// @param  _maxItemsPerTx The max Items Per Tx for Premint Phase 2.
    ...
    function setPhaseTwoMintValues(uint256 _pricePhaseTwo, uint256 _maxItemsPerTx, uint256 _startTimestamp, uint256 _endTimestamp, bytes32 _merkleRoot)
        external
        onlyRole(OPERATOR_ROLE)
        phaseOneIsOpen
    {
    ...
    /// @dev Restricted to onlyRole(OPERATOR_ROLE).
    /// @param  _pricePhaseThree The mint price for Premint Phase 2.
    /// @param  _maxItemsPerTx The max Items Per Tx for Premint Phase 2.
    ...
    function setPhaseThreeMintValues(uint256 _pricePhaseThree, uint256 _maxItemsPerTx, uint256 _startTimestamp, uint256 _endTimestamp, bytes32 _merkleRoot)
        external
        onlyRole(OPERATOR_ROLE)
        phaseTwoIsOpen
    {
    ...
    function setPublicMintValues(uint256 _pricePublicPhase, uint256 _maxItemsPerTx, uint256 _startTimestamp, uint256 _endTimestamp)
        external
        onlyRole(OPERATOR_ROLE)
        phaseThreeIsOpen
    {
    ...
    function setEarlyRevealValues(uint256 _earlyRevealTimestamp)
        external
        onlyRole(OPERATOR_ROLE)
        publicMintingIsOpen
    {
    ...
    /// @notice Restricted method to adjust the Status.
    /// @param _status The status to set.
    function setStatus(uint256 _status) external onlyRole(OPERATOR_ROLE) {
