...
contract ERC721 is Context, ERC165, IERC721, IERC721Metadata {
    ...
    */
    constructor(string memory name_, string memory symbol_,string memory baseURI_,address TeamContract_) {
        _name = name_;
        _symbol = symbol_;
        __baseURI = baseURI_;
        //上下级合约
    ...
    */
    function toMint(address to_,uint256 type_) public WhiteList returns (bool){
        ...
        _tokenIdType[tokenId] = type_;
        if(type_ == 1){
            _setTokenURI(tokenId, "Model3");
            _horsePower[tokenId] = 90;
        }else if(type_ == 2){
            _setTokenURI(tokenId, "Modely");
            _horsePower[tokenId] = _PoweRand(480, 60);
        }else if(type_ == 3){
            _setTokenURI(tokenId, "Modelx");
            _horsePower[tokenId] = _PoweRand(600, 60);
        }else if(type_ == 4){
            _setTokenURI(tokenId, "Models");
            _horsePower[tokenId] = _PoweRand(700, 60);
        }else if(type_ == 5){
            _setTokenURI(tokenId, "Roadster");
            _horsePower[tokenId] = _PoweRand(860, 60);
        }else if(type_ == 6){
            _setTokenURI(tokenId, "SpaceX");
            _horsePower[tokenId] = _PoweRand(1200, 60);
        }else{
            require(false, "this Tesla non-existent");
        }
        return true;
    }
    /**
