...
contract StakeContract is Pausable, Ownable, ReentrancyGuard {
    ...
    function approveClaimAmount(uint256 _indexNum, string calldata _tokenType) public onlyOwner {
        if(keccak256(bytes(_tokenType)) == keccak256(bytes("usdc"))){
            require(USDCstakeInfos[_msgSender()].timeOfLastUpdate > 0, "Fund Error");
        }else{
            require(USDTstakeInfos[_msgSender()].timeOfLastUpdate > 0, "Fund Error");
        }
        if(keccak256(bytes(_tokenType)) == keccak256(bytes("usdc"))){
            ...
            // usdcRequerstList[_indexNum]._isApprove = _status;
            ...
            // usdtRequerstList[_indexNum]._isApprove = _status;
    ...
    function unstakeToken(uint256 _unstakeAmount, string calldata _tokenType) external whenNotPaused amountGreaterThenZero(_unstakeAmount) nonReentrant returns (bool){
        ...
        if(keccak256(bytes(_tokenType)) == keccak256(bytes("usdc"))){
                ...
                usdcRequerstList[usdcRequestCount] = RequestInfo(_msgSender(),_unstakeAmount, false);
                usdcRequestCount++;
                ...
                usdtRequerstList[usdtRequestCount] = RequestInfo(_msgSender(), _unstakeAmount, false);
                usdtRequestCount++;
