...
contract MagpieCore is ReentrancyGuard, Ownable, Pausable, IMagpieCore {
    ...
    returns (uint256[] memory amountOuts)
  {
    ...
    (bytes32 hashSendTransaction, bool status) = IHyphenLiquidityPool(config.hyphenLiquidityPool)
      .checkHashStatus(
        payload.fromAssetId,
        payload.amountIn,
    ...
    swapArgs.amountOutMin = payload.amountIn + payload.destGasTokenAmount;
    ...
    gasFeeAccumulatedByToken[payload.toAssetId] += payload.swapOutGasFee;
    gasFeeAccumulated[payload.toAssetId][payload.recipientCoreAddress] += payload.swapOutGasFee;
