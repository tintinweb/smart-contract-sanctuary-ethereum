...
/*
this contract is forked from safemoon and edited to allow manual dev burns,
tax fee burn and different fees when buy / selling or transfer
everything about $DBALL on dragonballtoken.com
*/
...
contract DragonBall is Context, IERC20, Ownable {
    ...
    uint256 private _tTotal = 1 * 10**6 * 10**9;   // 1.000.000.000.000
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal;

    string private _name = "Dra";
    ...
    bool private uniswapV2PairAdded = false;
    
    bool inSwapAndLiquify;
    bool public swapAndLiquifyEnabled = false;
    
    uint256 public _maxTxAmount = 5000 * 10**6 * 10**9;      //    1.000.000.000.000*0.005 = 5.000.000.000
    ...
    //  set the uniswap pair after the presale has ended and liquidity has been added
    function setUniswapV2Pair (address _uniswapV2Pair) public onlyOwner() {
        require(!uniswapV2PairAdded,"uniswapV2Pair address has been added already");
        uniswapV2Pair = _uniswapV2Pair;
        uniswapV2PairAdded = true;
    }
    
    // function to manually burn amount of tokens decided by the dev team
    ...
    function setTaxFeePercent(uint256 taxFee, uint256 buyTaxFee) external onlyOwner() {
        _taxFee = taxFee;
        _buyTaxFee = buyTaxFee;
    }
    function setLiquidityFeePercent(uint256 liquidityFee, uint256 buyLiquidityFee) external onlyOwner() {
        _liquidityFee = liquidityFee;
        _buyLiquidityFee = buyLiquidityFee;
    }
    function setBurnFeePercent(uint256 burnFee,uint256 buyBurn) external onlyOwner() {
        _burnFee = burnFee;
        _buyBurnFee = buyBurn;
    }
    ...
    function _burnFeeFunction(uint256 tBurn) private {
        uint256 currentRate =  _getRate();
        uint256 rBurn = tBurn.mul(currentRate);
    ...
    function _transfer(
        ...
        //or if the sender is uniswapV2Pair change the fee for the buy
        ...
        _tokenTransfer(from,to,amount,takeFee,buyFee);
    }
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        // swap tokens for ETH
        swapTokensForEth(half); // <- this breaks the BNB -> DBALL swap when swap+liquify is triggered
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount,bool takeFee, bool buyFee) private {
        if(!takeFee) {
            removeAllFee();
        } else if (buyFee) {
            setBuyFee();
