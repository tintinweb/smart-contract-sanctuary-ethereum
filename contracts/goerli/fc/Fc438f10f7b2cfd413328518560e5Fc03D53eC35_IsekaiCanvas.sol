...
import { IIsekaiCanvas } from "./interfaces/IIsekaiCanvas.sol";

contract IsekaiCanvas is IIsekaiCanvas, Ownable, ERC721AQueryable {
    ...
    /// @dev Token ID of the collated line artwork.
    uint256 private constant _MASTER_ID = 1;

    /// @dev Array used to track if an Isekai Meta token has already drawn a line.
    ...
    /// @dev Interface the Isekai Meta NFT contract.
    ...
    /**
        ...
        @param id Associated Isekai token identifer.
    ...
    function drawLine(uint256 id, bytes calldata signature) external {
        ...
        // if (!_verifySignature(id, signature)) revert SignerMismatch();
    ...
    /**
        ...
        @param drawState_ Newly desired `drawState` value.
     */
    function setDrawState(uint256 drawState_) external onlyOwner {
        if (drawState_ > uint256(DrawStates.ENABLED)) revert InvalidState();
        drawState = DrawStates(drawState_);
    }
...
interface IIsekaiCanvas is IERC721AQueryable {
    error NonOwner();
    error HasDrawn();
    error HashSet();
    ...
    event LineDrawn(uint256 isekaiId, uint256 lineId);

    function drawLine(uint256 id, bytes calldata signature) external;
    function hasDrawn(uint256 id) external view returns (bool);
    function setDrawState(uint256 newDrawState) external;
