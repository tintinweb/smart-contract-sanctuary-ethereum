...
contract SecretVerseAvatarDistributor is AccessControl {
    ...
    uint256 public constant maxMintLimit = 5; // max NFTs that can be minted per wallet
    uint256 private constant maxAdminMintCount = 365; // Admin AVAs and Vaults mint count to TWIN mint
    uint256 public adminMintCount; // number of Minted Avas and Vaults by admin
    uint256 private avaCap = 10000 - maxAdminMintCount;
    uint256 private usersMintCount;

    mapping(address => uint256) public userMintedCount;
    mapping(uint256 => bool) public isVaultUsed;
    mapping(address => bool) public whiteList;

    address public withdrawWallet;
    address public upgradedToAddress = address(0);

    bool public mintingPause = true;
    ...
    modifier isEligible(uint256 _num) {
        if (mintingPause) {
    ...
    modifier isTimeForPublicSale() {
        if (address(vaultToken) == address(0x00)) {
            revert ZeroAddressNotAllowed();
        }
        if (
            block.timestamp <= whitelistStartTime + whitelistEndTime ||
            (whitelistStartTime == 0 || whitelistEndTime == 0)
        ) {
    ...
    modifier isAdminMintCountReached(uint256 _count) {
        if (adminMintCount + _count > maxAdminMintCount) {
        ...
        returns (bool)
    {
        ...
        checkIfWhiteListed(user, _num, _timeStamp, _signature);
        userMintedCount[user] += _num;
        usersMintCount = usersMintCount + _num;
    ...
    /// @notice allows to mint ava in public sale
    /// @param _num number of Avas to be minted , max 5
    /// @param _vaultTokenId 0: when simple mint, tokenId: of vault in case of twin vault mint
    function publicMintAvatar(uint256 _num, uint256 _vaultTokenId)
        external
        payable
        isEligible(_num)
        isTimeForPublicSale
    ...
    {
        address user = _msgSender();
        // checking if user wants to mint twin vault : 0 means simple vault, non-zero means TWIN VAULT
        if (_vaultTokenId != 0) {
            require(_num == 1, "Twin Vault should be minted singly");
            isVaultHolder(user, _vaultTokenId);
        }

        if (msg.value < publicMintPrice * _num) {
    ...
    /// @dev to set vault address
    /// @param _newVaultToken new vault Address
    function setVaultToken(address _newVaultToken) external onlyAdmin {
        if (_newVaultToken == address(0x00)) {
            revert ZeroAddressNotAllowed();
        }
        vaultToken = IVault(_newVaultToken);
    }

    function isVaultHolder(address user, uint256 _vaultTokenId)
        internal
        returns (bool)
    {
        if (isVaultUsed[_vaultTokenId]) {
            revert AlreadyCreatedTwinAVA();
        }
        if (vaultToken.ownerOf(_vaultTokenId) != user) {
            revert NotEligibleForTwinAVA();
        }
        isVaultUsed[_vaultTokenId] = true;
        return true;
    }

    /// @dev for admin to mint TWIN vault
    /// @param _to receiver of AVAs and VAULTs
    /// @param _count number of AVAS and VAULTS
    function adminMint(address _to, uint256 _count)
        external
        onlyAdmin
        isAdminMintCountReached(_count)
    {
        uint256 avaCurrentTokenId = avatarToken.getCurrentTokenId();
        uint256 vaultCurrentTokenId = vaultToken.getCurrentTokenId();
        adminMintCount = adminMintCount + _count;
        for (uint256 i; i < _count; i++) {
            avatarToken.mintNextToken(_to); // minting AVAs to admin
            vaultToken.adminMint(_to); // minting Vaults to admin
        }
        emit AdminMinted(
            getTokenIdsArray(avaCurrentTokenId, _count), // minted avas token ids
            getTokenIdsArray(vaultCurrentTokenId, _count), // minted vaults token ids
            _count
        );
    }
    ...
    function checkIfWhiteListed(
        ...
        if (whiteList[_wallet]) return true;
        address signerOwner = ECDSA.recover(
            keccak256(abi.encode(_wallet, _mintCount, _timeStamp)),
            _signature
        );
        require(
            hasRole(DEFAULT_ADMIN_ROLE, signerOwner),
            "SecretVerseNftDistributor: Not Whitelisted"
        );
...
interface IVault {
    ...
    function adminMint(address _mintTo) external returns (bool);
