...
contract Trade is AccessControl {
    ...
    constructor(
        ...
        _setupRole("TREASURYHOLDER_ROLE", msg.sender);                
    }
    ...
    */
    ...
    function changetTreasuryWallet(address _treasuryWallet)
        external
        onlyRole("TREASURYHOLDER_ROLE")
        returns (bool)
    {
        require(
        ...
        _revokeRole("TREASURYHOLDER_ROLE", treasuryHolder);
        emit treasuryHolderChanged(treasuryHolder, _treasuryWallet);
        treasuryHolder = _treasuryWallet;
        _setupRole("TREASURYHOLDER_ROLE", _treasuryWallet);
        return true;
    }
        ...
        returns (bool)
    {
        ...
        emit BuyAsset(order.nftAddress, order.seller, order.tokenId, order.qty, msg.sender, fee.price, fee.platformFee);
        return true;
    }
        ...
        returns (bool)
    {
        ...
        emit ExecuteBid(order.nftAddress, msg.sender, order.tokenId, order.qty, order.buyer, fee.price, fee.platformFee);
        return true;
    }
    ...
    //@param assetAddress NFT contract address(Viku721, Viku1155)
    ...
    /** 
        @notice transfer and Mint the NFTs(Viku721, Viku1155) and tokens through the transferproxy.
