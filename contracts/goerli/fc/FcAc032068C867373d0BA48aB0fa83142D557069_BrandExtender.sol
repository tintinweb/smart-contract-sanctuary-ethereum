...
contract BrandExtender is ERC5169, RoyaltySpliterStatic, VerifySignature, ERC721URIStorage, UriChanger, DerivedERC2981Royalty, IERC721Enumerable {
    ...
    event SharedTokenHolderUpdated(address prevAddress, address newAddress);
    ...
    event MintedDerived(address parentContract, uint parentId, uint mintedId, string tmpUri);
    ...
    constructor(address _rr, address _newUriChanger) ERC721("BrandExtender", "BEX") UriChanger(_newUriChanger){
    ...
    function setSharedTokenHolder(address newAddress) external onlyOwner {
        emit SharedTokenHolderUpdated(sharedTokenHolder, newAddress);
        sharedTokenHolder = newAddress;
    }
    ...
    function validateMintRequest(mintRequestData calldata data) internal view returns(bool){
        ...
        // bytes memory toSign = abi.encodePacked(msg.sender, tokenOfOriginCounter(erc721, data.tokenId), erc721, data.tokenId, data.tokenURI );
        bytes memory toSign = abi.encodePacked(msg.sender, balanceOf(msg.sender), erc721, data.tokenId, data.tokenURI );
    ...
    function mintDerived(mintRequestData[] calldata data) external payable {
        require(msg.value >= data.length * mintPrice, "Wrong amount attached");
        uint diff = msg.value - data.length * mintPrice;
        if (diff > 0) {
        ...
        for (i=0; i < data.length; i++) {
            ...
            mintedID = _mintDerivedMulti(data[i].ERC721, data[i].tokenId);
            emit MintedDerived(data[i].ERC721, data[i].tokenId, mintedID, data[i].tokenURI );
        }
    }

    function _mintDerivedMulti(address erc721, uint256 tokenId) internal returns (uint256 newTokenId) {
        ...
        if (sharedTokenHolder != address(0)) {
    ...
    function contractURI() public pure returns (string memory) {
        return string(abi.encodePacked(_metadataURI, "/contracts/brend_extender.json"));
    }
    ...
    // required to solve inheritance
    function supportsInterface(bytes4 interfaceId) public view virtual override(ERC5169, IERC165, ERC721, DerivedERC2981Royalty) returns (bool) {
        return 
    ...
    function _isSharedHolderTokenOwner(address _contract, uint256 tokenId) internal view returns (bool) {
        ERC721 t = ERC721(_contract);
        return sharedTokenHolder == t.ownerOf(tokenId);
    }
