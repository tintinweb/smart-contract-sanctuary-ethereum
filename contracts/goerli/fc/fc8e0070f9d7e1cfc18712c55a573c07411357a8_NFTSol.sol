...
// File: contracts/Mintingsmartcontract.sol
...
contract NFTSol is ERC721r {

    uint256 public constant Maxsupply = 1000;
    uint256 public constant Maxmintperwallet = 50;
    uint256 public MintPrice = 100000000000000 wei;
    uint256 public PublicMintPrice = 200000000000000 wei ;
    uint256 private constant Reservedtoken = 30 ;
    uint256 public TotalMinted = 0 ;
    bytes32 private merkleRoot = 0x87548675cb892755b3f26f4ae59eeba8c4970a9a6b3faacc0f82f2e7b5fbeca6;
...
//--------------------------------------  /* Using ER721---------------------------*/
    ...
    /*constructor () ERC721R("SniperKill", "SN-KL", 20){
        BaseUri = "ipfs://QmXQADAiNDwwTvmF1pec5KfnHaXfGF7hbcKvE3xneUig6Ry";
...
//--------------------------------------  //* Using ER721r---------------------------*/
        ...
        constructor () ERC721r("SniperKill", "SN-KL", 1000){
        BaseUri = "ipfs://QmXQADAiNDwwTvmF1pec5KfnHaXfGF7hbcKvE3xneUig6Ry";
        for (uint256 i = Reservedtoken ; i > 0 ; i--) {
            _mintAtIndex(payable(msg.sender) , i  );
            TotalMinted = Reservedtoken;}}

//--------------------------------------/* Whitlisting Method 2---------------------------*/
...
//--------------------------------------
    ...
    function Mint(uint256 _NumToken, bytes32[] calldata _proof) public payable {

        
        if(WhithlistMint == true){
        require(isWhiteListed(msg.sender, _proof),"You are not whitlisted");}


        require(Issalesactive,"The sales is paused");
        require( _NumToken <= Maxmintperwallet ,"You cannot mint more than 5");
        require( Mintedperwallet[msg.sender] + _NumToken <= Maxmintperwallet ,"Max mint reached");
        uint256 CurrentTotalMinted = TotalMinted ;
        require(CurrentTotalMinted + _NumToken <=  Maxsupply , "Mint has ended" );
        require( _NumToken * MintPrice >= msg.value , "Insufficient funds");

        for (uint256 i = 1 ; i <= _NumToken ; i++){
            _mintRandom(payable(msg.sender) , TotalMinted + i);
...
//--------------------------------------RevealNFT
        ...
        function RevealNFT(string memory _baseUri) public onlyOwner {
          BaseUri = _baseUri ;}
...
//--------------------------------------
     ...
     function StartPresaleMint() public onlyOwner {
            MintPrice = 1 ether;
            Issalesactive = true;
            WhithlistMint = true; }
...
//--------------------------------------
        ...
        function Withdraw() public onlyOwner {
           ...
           /* uint256 BalanceOne = BALANCE * 50 /100 ;
            ...
            (bool TransferOne,) = address(0x588740b8cD86ba0eaE510294053816251cDac3ee).call{value : BalanceOne}("");
           /* (bool TransferTwo,) = address(0x7ceB3cAf7cA83D837F9d04c59f41a92c1dC71C7d).call{value : BalanceTwo}("");*/
            require(TransferOne /*&& TransferTwo*/, "Withdraw Money Failed");}

//--------------------------------------

        function SetTokenURI(uint256 _tokenID) public view virtual returns (string memory ){
            require(_exists(_tokenID),"ERC721Metadata: URI query for nonexistent token");

             string memory currentBaseURI = LastBaseURL();
            ...
            if(bytes(CurrentBaseUri).length > 0){return string(abi.encodePacked(CurrentBaseUri, Strings.toString(tokenID) , BaseExtension));}
            */ }
        function LastBaseURL() internal view virtual returns (string memory){
            return BaseUri; }
}