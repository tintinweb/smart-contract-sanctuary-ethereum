...
library NFTfiSigningUtils {
    ...
    function isValidBorrowerSignature(
        ...
        return
            isValidBorrowerSignature(_listingTerms, _signature, address(this));
    }
    ...
    function isValidBorrowerSignature(
        ...
        require(
            block.timestamp <= _signature.expiry,
            "Borrower Signature has expired"
        );
    ...
    function isValidBorrowerSignatureBundle(
        ...
        return
            isValidBorrowerSignatureBundle(
                _listingTerms,
                _bundleElements,
                _signature,
                address(this)
    ...
    function isValidLenderRenegotiationSignature(
        ...
        require(
            block.timestamp <= _signature.expiry,
            "Renegotiation Signature has expired"
        );
        require(_loanContract != address(0), "Loan is zero address");
        if (_signature.signer == address(0)) {
            return false;
     ...
     */
    function getEncodedListing(LoanData.ListingTerms memory _listingTerms)
        internal
        pure
     ...
     * @dev We need this to avoid stack too deep errors.
     */
    function getEncodedOffer(LoanData.Offer memory _offer)
        internal
        pure
        returns (bytes memory)
    {
     ...
     * @dev We need this to avoid stack too deep errors.
     */
    function getEncodedSignature(LoanData.Signature memory _signature)
        internal
        pure
        returns (bytes memory)
    {
