...
contract Escrow is ContextUpgradeable, ImexAccessControl {
    ...
    function __Escrow_init(
        ...
        grantRoleAgreementCreator(admin);
    }
    ...
    function createNewAgreement(
        ...
        isAgreement[agreementClone] = true;
    ...
    function lockFundsinAgreement(
        address _importer,
        address _agreementClone,
        uint256 _tokens
    ) public {
        require(_importer != address(0x0), "Invalid importer");
        require(_tokens > 0, "Not enough tokens");
        require(
            isAgreement[_agreementClone] == true,
            "No agreement for given address"
        );
        require(token.balanceOf(_importer) >= _tokens, "Not enough balance");
        require(
            token.allowance(_importer, address(this)) >= _tokens,
