...
contract NF3GatedSwap is Ownable {
    ...
    event GatedListingCancelled(NF3GatedListing listing, address indexed owner);
    ...
    /// @dev Emits when banner collection is set
    /// @param oldBannerCollection old banner collection address
    /// @param newBannerCollection new banner collection address
    event BannerCollectionSet(
        address oldBannerCollection,
        address newBannerCollection
    );
    ...
    function cancelNF3GatedListing(
        ...
        if (_listing.editions > editionCounts[_listing.tokenId]) {
            // check not already filled up
            editionCounts[_listing.tokenId] = _listing.editions;

            emit GatedListingCancelled(_listing, _listing.owner);
        }
    }

    function setBannerCollection(address _bannerCollection) external onlyOwner {
        if (_bannerCollection == address(0)) {
            revert NF3GatedSwapError(NF3GatedSwapErrorCodes.INVALID_ADDRESS);
        }
        emit BannerCollectionSet(NF3BannerCollection, _bannerCollection);
