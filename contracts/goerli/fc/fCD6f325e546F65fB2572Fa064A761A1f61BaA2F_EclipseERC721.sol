    ...
    IEclipseERC721
{
        ...
        returns (address, uint256)
    {
        return (
            _royaltyReceiver,
            ((IEclipsePaymentSplitter(_royaltyReceiver).getTotalRoyaltyShares()) *
                salePrice_) / 10_000
        );
    }
...
abstract contract EclipseAccessUpgradable is OwnableUpgradeable {
    ...
    function __EclipseAccessUpgradable_init(address owner, address admin)
        internal
        onlyInitializing
    {
        __EclipseAccessUpgradable_init_unchained(owner, admin);
    }
