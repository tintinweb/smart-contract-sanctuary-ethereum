...
contract Raffle is VRFConsumerBaseV2, AutomationCompatibleInterface {
    ...
    address payable private constant SAFE_ADDRESS = payable(0x5b03E39b98a97527DCb3149Fa58b79B2c2Faa1E2);
...
//        nTickets[s_raffle_id][msg.sender] += 1;
    ...
    function fulfillRandomWords(
        ...
        bool winnerTxSuccess = false;
        bool safeTxSuccess = false;
        uint256 ownerIncome = 0;
        (winnerTxSuccess, ) = recentWinner.call{value: prize}("");
        if (winnerTxSuccess) {
            ownerIncome = address(this).balance;
            (safeTxSuccess, ) = SAFE_ADDRESS.call{value: ownerIncome}("");
            if (safeTxSuccess) {
                if (s_autoStart) {
                    s_raffleState = RaffleState.OPEN;
                    s_lastTimeStamp = block.timestamp;
...
//    function getNumberOfTickets(address playerAddress) public view returns(uint32) {
