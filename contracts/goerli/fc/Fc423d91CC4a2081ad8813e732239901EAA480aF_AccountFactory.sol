...
* A factory contract that deploys delegated diamonds. Designed to be the logic
...
import {AccountFactoryStorageGenesis} from "./AccountFactoryStorage.sol";

contract AccountFactory is AccountFactoryStorageGenesis {
    ...
    function createAccount(bool _enterAndSetUp) external returns (address account) {
        ...
        if (_enterAndSetUp) {
            IAccountSetUp(account).approveUnderlyings(IDataProvider(dataProvider).allUnderlyings());
            IAccountSetUp(account).enterMarkets(IDataProvider(dataProvider).allCTokens());
        }
...
interface IDataProvider {
    ...
    function cTokens(address _underlyingIn, address _underlyingOut) external returns (CErc20Interface, CErc20Interface);
