...
contract BAGC is ERC721, ERC721Enumerable, ERC721URIStorage, Ownable {
    ...
    // Mapping from tokenID to revealed status
    mapping(uint256 => bool) _revealed;
    ...
    /// @notice Emitted when the nft is Revealed.
    /// @dev If a token is minted and the status is unlocked, this event should be emitted.
    /// @param tokenId The identifier for a token.
    ...
    function lock(uint256 tokenId, uint256 period) public {
        require(ERC721.ownerOf(tokenId) == msg.sender, "L01");
        require(_locked[tokenId] == 0, "L02");
        require(period > 0, "L03");
    ...
    function unlock(uint256 tokenId) public {
        require(ERC721.ownerOf(tokenId) == msg.sender, "U01");
        require(_locked[tokenId] < block.timestamp, "U02");
        _locked[tokenId] = 0;
    ...
    function reveal(uint256 tokenId) public {
        require(ERC721.ownerOf(tokenId) == msg.sender, "R01");
        require(_revealed[tokenId] == false, "R02"); // revealed status must be not revealed
