...
error InvalidTreasureContractType();
...
/// @notice Thrown when claiming treasure functionality is disabled
error TreasureClaimNotActive();
/// @notice Thrown when block.timestamp already pass or ahead of the scheduled claiming time
error ClaimingNotAllowed();
error InvalidTreasureListManagerIndex();
...
contract PixelmonTrainerAdventure is WinnerSelectionManager, Utils, ReentrancyGuard {
    ...
    /// @notice Signature for the signer
    bytes32 public constant SIGNER_ROLE = keccak256("SIGNER_ROLE");
    /// @notice Domain for singing
    string private constant SIGNING_DOMAIN = "Pixelmon-Trainer-Adventure";
    ...
    /// @notice Treasure claim status, 'true' means enabled and 'false' means disabled
    bool public treasureClaimActive = true;
    ...
    function addERC1155TreasuresToVault(TreasureInput[] calldata _treasures) public onlyAdmin(msg.sender) {
        if (_treasures.length < 1) {
    ...
    /// @dev Only user that has role "Admin" can call this function
    /// @param _treasures Information about the treasure
    function addERC721TreasuresToVault(TreasureInput[] calldata _treasures) public onlyAdmin(msg.sender) {
        if (_treasures.length < 1) {
            revert InvalidDataLength();
        }
        for (uint256 index = 0; index < _treasures.length; index = _uncheckedInc(index)) {
    ...
    function rescueTreasureFromPool(
        ...
        uint256 _poolIndex,
        address _transferTo
    ) external nonReentrant onlyOwner validUpdationPeriod {
        ...
        if (treasureVault[_vaultIndex].contractType == uint256(ContractType.ERC_1155)) {
            IERC1155 erc1155Contract = IERC1155(treasureVault[_vaultIndex].collectionAddress);
            erc1155Contract.safeTransferFrom(address(this), _transferTo, treasureVault[_vaultIndex].tokenId, 1, "");
        } else {
            IERC721 erc721Contract = IERC721(treasureVault[_vaultIndex].collectionAddress);
            erc721Contract.safeTransferFrom(address(this), _transferTo, treasureVault[_vaultIndex].tokenId);
        }
    }
    ...
    /// @param _vaultIndex Index of the treasure in the vault
    /// @param _transferTo Receiver of the treasure
    function rescueTreasureFromVault(uint256 _vaultIndex, address _transferTo) external nonReentrant onlyOwner validUpdationPeriod {
    ...
    function claimTreasure(bytes32[] calldata proof) external nonReentrant noContracts {
        if (!verify(proof, currentWeekNumber)) {
            revert NotAWinner();
        }
        if (!treasureClaimActive) {
            revert TreasureClaimNotActive();
        }
        // if (weekInfos[currentWeekNumber].claimed[msg.sender]) {
        //     revert ALreadyClaimed();
        // }
        if (block.timestamp >= weekInfos[currentWeekNumber].startTimeStamp && block.timestamp <= weekInfos[currentWeekNumber].endTimeStamp) {
            revert ClaimingNotAllowed();
        }

        if (availableTreasure == 0) {
            revert TreasureNotAvailable();
        }
        weekInfos[currentWeekNumber].claimed[msg.sender] = true;
        ...
        treasureVault[vaultIndex].claimIndex = claimIndex;

        if (treasureVault[vaultIndex].contractType == uint256(ContractType.ERC_1155)) {
            IERC1155 erc1155Contract = IERC1155(treasureVault[vaultIndex].collectionAddress);

            erc1155Contract.safeTransferFrom(address(this), treasureVault[vaultIndex].receiverWallet, treasureVault[vaultIndex].tokenId, 1, "");
        } else {
            IERC721 erc721Contract = IERC721(treasureVault[vaultIndex].collectionAddress);
            erc721Contract.transferFrom(address(this), treasureVault[vaultIndex].receiverWallet, treasureVault[vaultIndex].tokenId);
        }
    ...
    /// @notice Enable/disable claim treasure functionality
    function setTreasureClaimStatus(bool _treasureClaimStatus) external onlyOwner {
        treasureClaimActive = _treasureClaimStatus;
    }
    ...
    function removeTreasureFromPool(uint256 _poolIndex) internal {
        if (_poolIndex > MAXIMUM_POOL_SIZE) {
            revert InvalidPoolIndex();
        }
    ...
    /// @param _poolIndex Index of the treasure in the pool
    function removeTreasureFromPoolExternal(uint256 _poolIndex) external onlyAdmin(msg.sender) {
        treasureVault[treasurePool[_poolIndex]].status = uint256(TreasureStatus.AtVault);
        removeTreasureFromPool(_poolIndex);
    }

    /// @notice Move treasure from vault to pool
    /// @param _vaultIndex Index of the treasure in the vault
    function addTreasuresToPool(uint256[] calldata _vaultIndex) public onlyAdmin(msg.sender) validUpdationPeriod {
        if (treasurePoolSize + _vaultIndex.length > MAXIMUM_POOL_SIZE) {
            revert MaximumPoolSizeExceeded();
        }

        for (uint256 index = 0; index < _vaultIndex.length; index++) {
            if (treasureVault[_vaultIndex[index]].status == uint256(TreasureStatus.AtVault)) {
                treasurePoolSize++;
                treasurePool[treasurePoolSize] = _vaultIndex[index];
                treasureVault[_vaultIndex[index]].status = uint256(TreasureStatus.AtPool);
            } else {
                revert TreasureNotAvailable();
            }
        }
    ...
    function resetTreasurePool() public onlyAdmin(msg.sender) {
    ...
    /// @notice Get index of treasure in the pool
    /// @param _vaultIndex Index of treasure in the vault
    /// @return Treasure index in the pool
    function getPoolIndexOfTreasure(uint256 _vaultIndex) public view returns (uint256) {
        uint256 selectedIndex = 0;
        for (uint256 index = 1; index <= treasurePoolSize; index++) {
            if (treasurePool[index] == _vaultIndex) {
                selectedIndex = index;
                break;
            }
        }
    ...
    /// @param _tokenId Treasure token ID
    ...
    function getTreasureVaultInfo(
        ...
        uint256 _tokenId,
        uint256 _status
    ) public view returns (Treasure[] memory datas) {
        ...
        for (uint256 index = 1; index <= totalTreasure; index++) {
            if (
                ...
                treasureVault[index].tokenId == _tokenId &&
                treasureVault[index].status == _status
            ) {
...
contract WinnerSelectionManager is Ownable, VRFConsumerBaseV2 {
    ...
    modifier validUpdationPeriod() {
        if (block.timestamp <= weekInfos[currentWeekNumber].endTimeStamp || block.timestamp >= weekInfos[currentWeekNumber + 1].startTimeStamp) {
    ...
    /// @notice Amount of second in a week
    uint32 public constant Seconds_In_Week = 604800;
    /// @notice Amount of second in a day
    uint32 public constant Seconds_In_Day = 86400;
    /// @notice Number of the current week
    uint256 public currentWeekNumber = 0;
    ...
    /// @return requestId from Chainlink
    //
    function generateChainLinkRandomNumbers() external onlyModerator(msg.sender) validUpdationPeriod returns (uint256 requestId) {
        ...
        requests[requestId] = Request({randomWords: new uint256[](0), exists: true, fulfilled: false, weekNumber: currentWeekNumber + 1});
    ...
    function setMerkleRoot(bytes32 merkle) external onlyModerator(msg.sender) validUpdationPeriod {
        if (merkle[0] == 0) {
            revert InvalidMerkleRoot();
        }
        // with this condition be checked ever?
        if (weekInfos[currentWeekNumber + 1].winnersMerkleRoot[0] != 0) {
            revert MerkleRootAlreadySet();
        }
        currentWeekNumber++;
        weekInfos[currentWeekNumber].winnersMerkleRoot = merkle;

        emit SetWinnerMerkelRoot(currentWeekNumber, merkle);
    }
    ...
    function updateMerkleRoot(bytes32 merkle, uint256 _weekNumber) external onlyModerator(msg.sender) {
        if (merkle[0] == 0) {
            revert InvalidMerkleRoot();
        }
    ...
    /// @notice Set treasure claim timestamp for specific week
    /// @dev Avoid overlap with setting claiming timestamp for each week
    /// @param _weekNumber The week number that will be set
    /// @param _startTimeStamp The start timestamp for the treasure claim
    /// @param _endTimeStamp The end timestamp for the treasure claim
    function setWeeklyTreasureClaimingTimeStamp(
        uint256 _weekNumber,
        ...
        if (_weekNumber <= currentWeekNumber) {
            revert InvalidWeekNumber();
        }

        weekInfos[_weekNumber].startTimeStamp = _startTimeStamp;
        weekInfos[_weekNumber].endTimeStamp = _endTimeStamp;
    }

    /// @notice Set treasure claim timestamp for a range of week
    /// @param _startWeekNumber The start week number of the treasure claim event
    /// @param _endWeekNumber The end week number of the treasure claim event
    /// @param _startTimeStamp The start time for the treasure claim event
    /// @param _claimingDuration How long the event will take
    function setTreasureClaimingTimeStamp(
        uint256 _startWeekNumber,
        uint256 _endWeekNumber,
        uint256 _startTimeStamp,
        uint256 _claimingDuration,
        uint256 _weeklyUpdationDuration
    ) external onlyAdmin(msg.sender) {
        if (_startWeekNumber > _endWeekNumber) {
            revert InvalidWeekNumber();
        }
        // if (_claimingDuration <= (Seconds_In_Week - Seconds_In_Day)) {
        //     revert InvalidClaimingDuration();
        // }

        for (uint256 weekNumber = _startWeekNumber; weekNumber <= _endWeekNumber; weekNumber++) {
            weekInfos[weekNumber].startTimeStamp = _startTimeStamp;
            weekInfos[weekNumber].endTimeStamp = _startTimeStamp + _claimingDuration;
            _startTimeStamp = weekInfos[weekNumber].endTimeStamp + _weeklyUpdationDuration;
        }
    }
    ...
    function verify(bytes32[] calldata proof, uint256 _weekNumber) public view returns (bool) {
    ...
    /// @notice Get random number for specific week
    /// @param _weekNumber The number of the week
    function getRandomNumbers(uint256 _weekNumber) public view returns (uint256[] memory numbers) {
        return weekInfos[_weekNumber].randomNumbers;
    }
