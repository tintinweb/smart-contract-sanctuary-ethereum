...
contract OtcExchange is IOrderEscrow, IOrderFixed, Ownable, ReentrancyGuard {
  ...
  modifier canTakeOrder(uint256 orderID, bool orderClass) {
  ...
  modifier canCancelOrder(uint256 orderID, bool orderClass) {
    if (!isActive(orderID, orderClass))
      revert OrderNotActive(orderID, orderClass);
    if (msg.sender != getMaker(orderID, orderClass))
  ...
  function takeOrderFixed(uint256 orderID)
    external
  ...
  function cancelOrderFixed(uint256 orderID)
    external
  ...
  function takeOrderEscrow(uint256 orderID, uint256 amountToken)
    external
  ...
  function cancelOrderEscrow(uint256 orderID)
    external
    ...
    returns (bool success)
  {
    ...
    if (_orderToBeDeleted.orderType) {
      ...
      tokenBaseERC.transfer(address(this), _orderToBeDeleted.amountBase);
    }
  ...
  function getPlatformStatistics()
    external
    view
    returns (uint256 totalOrders, uint256 totalTrades)
  {
   ...
   */
  function isActive(uint256 orderID, bool orderClass)
    internal
    view
    returns (bool)
  {
   ...
   *
   */
  function getMaker(uint256 orderID, bool orderClass)
    internal
    view
  ...
  function getOrderFixed(uint256 orderID)
    external
    view
...
interface IOrderEscrow {
   ...
   */
  function cancelOrderEscrow(uint256 orderID) external returns (bool success);
