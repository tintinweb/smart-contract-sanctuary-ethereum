...
contract SYLSafe is AccessControl {
    ...
    uint256 private _depositEndsAt;
    ...
    function deposit(uint256 depositAmount) external {
        require(_depositEndsAt >= block.timestamp, "deposit period expired");
        require(_savings[msg.sender].amount == 0, "already deposited");
        ...
        _savings[msg.sender] = Savings(block.timestamp, depositAmount);
    ...
    function withdraw() external {
        uint256 amount = _amount(msg.sender);
        uint256 interest = _interest(msg.sender, block.timestamp);

        require(amount != 0, "saving nonexists");
        require(_done(msg.sender), "cannot withdraw");
    ...
    /**
     * @dev Sets ends of deposit timestamp.
     */
    function setDepositEndsAt(uint256 timestamp) external onlyRole(DEFAULT_ADMIN_ROLE)  {
        _depositEndsAt = timestamp;
    }
    ...
    /**
     * @return duration, annual percentage rate, total allocation, current allocation, minimum deposit, maximum deposit amount of safe, deposit end period
     */
    function safeStatus() external view returns (uint256[] memory) {
        uint256[] memory result = new uint256[](7);
        result[0] = _duration;
        result[1] = _annualPercentageRate;
        result[2] = _totalAllocation;
        result[3] = _allocated;
        result[4] = _minDeposit;
        result[5] = _maxDeposit;
        result[6] = _depositEndsAt;
        return result;
    }

    /**
     * @return start, end, amount, accumulated amount, closing amount of saving
     */
    function savingStatus(address addr) external view returns (uint256[] memory) {
        uint256[] memory result = new uint256[](5);
        result[0] = _start(addr);
        if (_start(addr) == 0) {
            result[1] = 0;
        } else {
            result[1] = _start(addr) + _duration;
        }
        result[2] = _amount(addr);
        result[3] = _amount(addr) + _interest(addr, block.timestamp);
        result[4] = _amount(addr) + _interest(addr, _start(addr) + _duration);
        return result;
    }
    ...
    function _interest(address addr, uint256 timestamp) private view returns (uint256) {
        ...
        uint256 interestPerSecond = _amount(addr) * _annualPercentageRate / 100 / (86400 * 365);
        if (timestamp - _start(addr) < _duration) {
            return interestPerSecond * (timestamp - _start(addr));
        }
        return interestPerSecond * _duration;
    }

    function _start(address addr) private view returns (uint256) {
        return _savings[addr].start;
    }

    function _amount(address addr) private view returns (uint256) {
        return _savings[addr].amount;
    }

    function _done(address addr) private view returns (bool) {
        if (_start(addr) == 0) {
            return false;
        }
        return block.timestamp - _start(addr) >= _duration;
    }
}
