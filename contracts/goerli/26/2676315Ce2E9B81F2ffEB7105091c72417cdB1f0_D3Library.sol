 ...
 * @author  DODO
 * @title   D3Liquidation
 * @dev     This contract contains the real code implementations for D3Funding and D3Liquidation.
 */
...
library D3Library {
    ...
    function lpDeposit(
        ...
        require(
            getCollateralRatio(state) >= state._INITIAL_MARGIN_RATIO_,
    ...
    function setNextEpoch(
        ...
        uint256 epoch = currentEpoch(state);
    ...
    /// @param state pool state
    function refund(uint256 index, Types.D3MMState storage state)
        external
        returns (
            address user,
            address token,
        ...
        )
    {
        ...
        updateReserve(withdrawInfo.token, state);
        ...
        return (withdrawInfo.user, withdrawInfo.token, originTokenAmount);
    }
    ...
    function ownerWithdraw(
        ...
        updateReserve(token, state);
        uint256 ratio = getCollateralRatio(state);
    ...
    /// @param state pool state
    function accrueInterests(Types.D3MMState storage state) public {
    ...
    function startLiquidation(Types.D3MMState storage state) external {
        uint256 collateralRatio = getCollateralRatio(state);
        require(
            collateralRatio < state._MAINTENANCE_MARGIN_RATIO_,
            Errors.ABOVE_MM_RATIO
        );
        accrueInterests(state); // accrue interests for the last time
        state._POOL_STATUS_ = Types.PoolStatus.Liquidating;

        uint256 ratio;
        if (collateralRatio == 0) {
            uint256 totalValue = getTotalAssetsValue(state);
            uint256 totalDebt = getTotalDebtValue(state);
    ...
    function forceRefund(
        ...
        updateReserve(wInfo.token, state);
        ...
        emit Refund(wInfo.user, wInfo.token, refundAmount);
    }
}
...
interface ID3Factory {
    ...
    function _DODO_WHITELIST_(address) external returns (bool);
}
