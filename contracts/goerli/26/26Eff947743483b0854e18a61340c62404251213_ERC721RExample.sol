...
contract ERC721RExample is ChainlinkClient, ERC721A, Ownable {

    using Chainlink for Chainlink.Request;

    // TODO: revill function
    
    uint256 public constant mintPrice = 0.0001 ether; // 0.022
    uint256 public constant refundAmount = 0.00095 ether;
    uint256 public constant refundPeriod = 1 days;
    uint256 internal maxMintSupply = 2662; 
    string public unreveiledUri;
    ...
    mapping(bytes32 => ReqData) private ChainlinkRequest;

    string public baseURI;

    constructor(string memory _apiUrl) ERC721A("Artropods", "ARTP") {
        apiUrl = _apiUrl;
    ...
    function preSaleMint(uint amount) public payable returns (bytes32 requestId) {
        ...
        Chainlink.Request memory req = buildChainlinkRequest(
            jobId,
            address(this),
            this._chainlinkMint.selector
        );

        req.add(
            "get",
            string(abi.encodePacked(apiUrl, toAsciiString(msg.sender)))
        );
        ...
        ChainlinkRequest[_reqId] = ReqData(amount, msg.sender);
    ...
    function _chainlinkMint(
        bytes32 _requestId,
        bool _isWhitelisted
    ) public recordChainlinkFulfillment(_requestId) {
        response = true;
        if (_isWhitelisted) {
            ReqData memory _reqData = ChainlinkRequest[_requestId];
            _safeMint(_reqData.to, _reqData.amount);
        }
    }
    ...
    // function getNftIds(address _address) external view returns(uint[] memory ) {
    //     uint[] memory ids;
    //     uint index = 0;
    //      for(uint i = 0; i < totalSupply(); i++){ 
    //          address owner = IERC721A(address(this)).ownerOf(i);
    //          if (owner == _address) {
    //              ids[index] = i;
    //              index+=1;
    //          }
    ...
    function toggleReveil() external onlyOwner {
        isReveiled = !isReveiled;
    }
    ...
    function serUnreveiledUri(string memory uri) external onlyOwner { 
        unreveiledUri = uri;
    }
