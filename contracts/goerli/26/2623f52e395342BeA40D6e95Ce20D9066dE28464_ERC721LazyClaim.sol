    ...
    ReentrancyGuard
{
    ...
    {
        return
            interfaceId == type(IERC721LazyClaim).interfaceId ||
        ...
        returns (uint256)
    {
        // Sanity checks
        require(
            claimParameters.storageProtocol != StorageProtocol.INVALID,
            "Cannot initialize with invalid storage protocol"
        );
        require(
            claimParameters.endDate == 0 ||
                claimParameters.startDate < claimParameters.endDate,
    ...
    function updateClaim(
        ...
        // Sanity checks
        require(
            _claims[creatorContractAddress][claimIndex].storageProtocol !=
        ...
        require(
            _claims[creatorContractAddress][claimIndex].totalMax == 0 ||
                _claims[creatorContractAddress][claimIndex].totalMax <=
                claimParameters.totalMax,
            "Cannot decrease totalMax"
        );
        require(
            _claims[creatorContractAddress][claimIndex].walletMax == 0 ||
                _claims[creatorContractAddress][claimIndex].walletMax <=
                claimParameters.walletMax,
     ...
     */
    function getClaimCount(address creatorContractAddress)
        external
        view
        override
        returns (uint256)
     ...
     */
    function getClaim(address creatorContractAddress, uint256 claimIndex)
        external
        view
        override
    ...
    function checkMintIndex(
        ...
        require(
            claim.storageProtocol != StorageProtocol.INVALID,
            "Claim not initialized"
        );
        require(claim.merkleRoot != "", "Can only check merkle claims");
        uint256 claimMintIndex = mintIndex >> 8;
        uint256 claimMintTracking = _claimMintIndices[creatorContractAddress][
            claimIndex
        ][claimMintIndex];
    ...
    ) external view override returns (bool[] memory minted) {
        minted = new bool[](mintIndices.length);
        for (uint256 i = 0; i < mintIndices.length; i++) {
            minted[i] = checkMintIndex(
    ...
    function getTotalMints(
        ...
        require(
            claim.walletMax != 0,
            "Can only retrieve for non-merkle claims with walletMax"
        );
        return
            uint32(_mintsPerWallet[creatorContractAddress][claimIndex][minter]);
    }
    ...
    function mint(
        ...
        // Check totalMax
        require(
            claim.totalMax == 0 || claim.total < claim.totalMax,
        ...
        if (claim.merkleRoot != "") {
            ...
            if (claim.walletMax != 0) {
                require(
                    _mintsPerWallet[creatorContractAddress][claimIndex][
                        msg.sender
                    ] < claim.walletMax,
                    "Maximum tokens already minted for this wallet"
                );
                unchecked {
                    _mintsPerWallet[creatorContractAddress][claimIndex][
                        msg.sender
        ...
        // Do mint
        uint256 newTokenId = IERC721CreatorCore(creatorContractAddress)
            .mintExtension(msg.sender);

        // Insert the new tokenId into _tokenClaims for the current claim address & index
        _tokenClaims[creatorContractAddress][newTokenId] = TokenClaim(
            uint224(claimIndex),
            claim.total
        );
    ...
    function mintBatch(
        ...
        // Check totalMax
        require(
            claim.totalMax == 0 || claim.total + mintCount <= claim.totalMax,
        ...
        if (claim.merkleRoot != "") {
            require(
                mintCount == mintIndices.length &&
                    mintCount == merkleProofs.length,
                "Invalid input"
            );
            // Merkle mint
            ...
            if (claim.walletMax != 0) {
                require(
                    _mintsPerWallet[creatorContractAddress][claimIndex][
                        ...
                        mintCount <=
                        claim.walletMax,
                    "Too many requested for this wallet"
                );
                unchecked {
                    _mintsPerWallet[creatorContractAddress][claimIndex][
                        msg.sender
        ...
        uint256[] memory newTokenIds = IERC721CreatorCore(
            creatorContractAddress
        ).mintExtensionBatch(msg.sender, mintCount);
        for (uint256 i = 0; i < mintCount; ) {
            _tokenClaims[creatorContractAddress][newTokenIds[i]] = TokenClaim(
                uint224(claimIndex),
                uint32(newMintIndex + i)
            );
            unchecked {
                i++;
            }
    ...
    function _checkMerkleAndUpdate(
        ...
        require(
            MerkleProof.verify(merkleProof, claim.merkleRoot, leaf),
            "Could not verify merkle proof"
        );
        ...
        _claimMintIndices[creatorContractAddress][claimIndex][claimMintIndex] =
            claimMintTracking |
            mintBitmask;
    }
    ...
    {
        TokenClaim memory tokenClaim = _tokenClaims[creatorContractAddress][
            tokenId
        ];
        require(tokenClaim.claimIndex > 0, "Token does not exist");
        Claim memory claim = _claims[creatorContractAddress][
            tokenClaim.claimIndex
        ];
        ...
        if (!claim.identical) {
            uri = string(
                abi.encodePacked(
                    uri,
                    "/",
                    uint256(tokenClaim.mintOrder).toString()
                )
            );
        }
    }
