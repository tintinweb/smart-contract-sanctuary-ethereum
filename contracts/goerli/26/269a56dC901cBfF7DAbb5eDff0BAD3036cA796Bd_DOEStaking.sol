...
contract DOEStaking is TokenWrapper, NFTWrapper, Ownable {
    ...
    function setMaxNFTBalance(uint8 _maxNFTBalance) external onlyOwner {
...
contract NFTWrapper is ERC721Holder {
    ...
    function stakeNFT(uint256 tokenId) public payable virtual {
        uint8 balance = uint8(balanceOfNFT(msg.sender));

        require(balance <= maxNFTBalance, "NFT BALANCE EXCEEDED");
