...
contract NGageToken is ERC3525SlotEnumerableUpgradeable {
    ...
    function tokenURI(
        ...
        LoyaltyProgram memory loyaltyProgramData = lps[slotOf(_tokenId)];
        ...
        string memory json = string(
            abi.encodePacked(
                ...
                '", "description":"',
                loyaltyProgramData.circulatingSupply,
                '", "image": "',
                image,
                '", "attributes": ['
            )
        ...
        for (
            uint256 i = 0;
            i < _tokenMetadata[_tokenId].attributes.length;
            i++
        ) {
            json = string(
                abi.encodePacked(
                    json,
                    '{"trait_type": "',
                    _tokenMetadata[_tokenId].attributes[i].trait_type,
                    '", "value": "',
                    _tokenMetadata[_tokenId].attributes[i].value,
                    '"}'
                )
            );
            if (i < _tokenMetadata[_tokenId].attributes.length - 1) {
    ...
    // update token image
    function updateTokenImage(uint256 tokenId, string memory image)
        public
    {
        require(
            _exists(tokenId),
