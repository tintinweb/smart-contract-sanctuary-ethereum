...
contract MonMarketplace is Ownable, Pausable {
    ...
    mapping(uint256 => address) public actualMonsterOwner; // slotId -> a Monster owner
    ...
    event UpdatePrice(uint256 indexed id, uint256 newPrice, address indexed user);

    modifier onlyMonsterOwner(uint256 _id) {
        _checkMonsterOwner(_id);
        _;
    }
    ...
    /**
     * @dev Get list prices for Monster token ids
     * @param _ids is a list of Monster token IDs. Max 50 Ids
     */
     ...
     * @param _id is a Monster token ID
     */
    function cancel(uint256 _id) public whenNotPaused {
        require(prices[_id] != 0, 'Token should be listed');
        require(msg.sender == actualMonsterOwner[_id], 'Listing can be canceled only by owner');
        _cancel(_id);
     ...
     * @param _tokenIDs is a list with IDs of Monster NFT token
     */
     ...
     * @param _id is an ID of Monster NFT token
     * @param _price is a listed price that can be changed. Check updatePrice()
     */
    function listToken(uint256 _id, uint256 _price) public onlyMonsterOwner(_id) whenNotPaused {
        require(_price > 0, 'The price can not be a zero value');
        require(prices[_id] == 0, 'The token was already listed');
        prices[_id] = _price;
        actualMonsterOwner[_id] = msg.sender;
     ...
     * @param _tokenIDs is a list with IDs of Monster NFT token
     * @param _prices is listed prices that can be changed. Check updatePrice()
     */
    ...
    /**
     * The function for updating price
     * @param _id is a list with IDs of Monster NFT token
     * @param _newPrice is new prices for tokens
     */
    function updatePrice(uint256 _id, uint256 _newPrice) public onlyMonsterOwner(_id) whenNotPaused {
        require(_newPrice > 0, 'The price can not be a zero value');
        require(prices[_id] > 0, 'The current price can not be a zero value');
        require(prices[_id] != _newPrice, 'The price can not be the same');
        prices[_id] = _newPrice;
        emit UpdatePrice(_id, _newPrice, msg.sender);
    }

    /**
     * The function of updating prices
     * @param _tokenIDs is a list with IDs of Monster NFT token
     * @param _newPrices is new prices for tokens
     */
    function updatePrices(uint256[] memory _tokenIDs, uint256[] memory _newPrices) public whenNotPaused {
        require(_tokenIDs.length > 0, 'The list of IDs can not be empty');
        require(_newPrices.length > 0, 'The list of prices can not be empty');
        require(_tokenIDs.length == _newPrices.length, 'The length of lists should be the same');
        for (uint256 i = 0; i < _tokenIDs.length; i++) {
            updatePrice(_tokenIDs[i], _newPrices[i]);
        }
    }
    ...
    function _buyToken(uint256 _id) internal {
        ...
        IMonNFT(monsterToken).transferFrom(actualMonsterOwner[_id], msg.sender, _id);
        _cancel(_id);
     ...
     * @param _id is an ID of Monster token
     */
    function _checkMonsterOwner(uint256 _id) internal view {
        address _monsterOwner = IMonNFT(monsterToken).ownerOf(_id);
     ...
     * and sends to the feeReceiver address. Other amount of Monster cost is sent to seller
     * @param _id is an ID of Monster token
     * @param _amount is a current price of a Monster token
     */
    function _transferFees(uint256 _id, uint256 _amount) internal {
        ...
        payable(actualMonsterOwner[_id]).transfer(_amount - amount);
    }

    /**
     * @dev The function reset price and actual monster owner for provided Monster token
     * @param _id is an ID of Monster token
     */
    function _cancel(uint256 _id) internal {
        prices[_id] = 0;
        actualMonsterOwner[_id] = address(0);
    }
}