...
contract StakingFactory is Ownable, IStakingFactory {
    ...
    function getPlasmaPerBlock() external view override returns (uint256) {
        ...
        if (_pool.ufoPool) {
            ...
            if (plasmaRewardsForUfoPools < claimedPlasmaRewardsForUfoPools) return 0;
            return (((plasmaRewardsForUfoPools - claimedPlasmaRewardsForUfoPools) * _pool.weight)) / totalUfoWeight;
        } else {
            if (totalLpWeight == 0) {
                // to avoid division overflow when tvl is 0
                return 0;
            }
            if (plasmaRewardsForLpPools < claimedPlasmaRewardsForLpPools) return 0;
            return (((plasmaRewardsForLpPools - claimedPlasmaRewardsForLpPools) * _pool.weight)) / totalLpWeight;
        }
    }
