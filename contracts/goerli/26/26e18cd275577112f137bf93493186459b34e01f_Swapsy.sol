...
contract Whitelist is Ownable, EIP712 {
    bytes32 public constant WHITELIST_TYPEHASH =
        keccak256("Whitelist(string amountIn, string amountOut)"); //, address tokenIn, address tokenOut, string totalAmountInEth, string totalAmountOutEth, bytes32 swapId, uint256 timeout)");
    ...
    function _isSenderWhitelisted(
       ...
       /* address tokenIn, 
        address tokenOut,
        string memory totalAmountInEth,
        string memory totalAmountOutEth, 
        bytes32 swapId,
        uint256 timeout,*/
...
contract Swapsy is ReentrancyGuard, Ownable, Whitelist {
    ...
    {
        uint _amountIn = stringToUint(amountIn);
        uint _amountOut = stringToUint(amountOut);
        uint _totalAmountInEth = stringToUint(totalAmountInEth);
        uint _totalAmountOutEth = stringToUint(totalAmountOutEth);
