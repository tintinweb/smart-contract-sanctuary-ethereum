...
contract TeamMATESMilton is Ownable, ERC721A {
  // Contract Constants
  ...
  uint128 public cost = .001 ether;
  string public baseURI;

  modifier canMint(uint8 _amount, uint256 _value){
      require(_getAux(_msgSender()) + _amount <= 2, "Exceeds wallet mint amount.");
      ...
      require(_value >= cost, "Not enough eth being sent!");
      _;
  }

  constructor(uint128 _saleStartTime) ERC721A("BackOut", "OBSH") {
    saleStartTime = _saleStartTime;
  }

  function mint(address _recipient, uint8 _amount) external payable canMint(_amount, msg.value){
    _mint(_recipient, _amount);
    _setAux(_recipient, _getAux(_recipient) + _amount);
  }
  ...
  function setSaleLock(uint128 _newTime) public onlyOwner {
