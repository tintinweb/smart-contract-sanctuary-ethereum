...
interface IPrizeDistributor {
    ...
    function setDrawCalculator(
        IDrawCalculator newCalculator
    ) external returns (IDrawCalculator);
    ...
    Manageable
{
        ...
        returns (uint32)
    {
        ...
        if (
            prizeDistributionRingBuffer[bufferNextIndex].matchCardinality != 0
        ) {
    ...
    function _pushPrizeDistribution(
        ...
        require(
            _prizeDistribution.matchCardinality > 0,
            "DrawCalc/matchCardinality-gt-0"
        );
        require(
            _prizeDistribution.bitRangeSize <=
                256 / _prizeDistribution.matchCardinality,
            "DrawCalc/bitRangeSize-too-large"
        );

        require(
            _prizeDistribution.bitRangeSize > 0,
            "DrawCalc/bitRangeSize-gt-0"
        );
        require(
            _prizeDistribution.maxPicksPerUser > 0,
            "DrawCalc/maxPicksPerUser-gt-0"
        );
        require(
            _prizeDistribution.expiryDuration > 0,
            "DrawCalc/expiryDuration-gt-0"
        );
...
*/
contract PrizeDistributor is Initializable, IPrizeDistributor, Ownable {
    ...
    /// @notice basic points that equal 100%
    uint256 private BASIC_POINTS;

    /// @notice Fee for protocol. The fee is withdrawn at the moment of claiming
    uint256 public feeForProtocol;

    /// @notice address where fee will go
    ...
    function initialize(
        ...
        __PrizeDistributor_init_unchained(_owner, _token, _drawCalculator);
    }
    ...
    function claim(
        ...
        (uint256[] memory drawPayouts, ) = drawCalculator.calculate(
            ...
            _drawIds,
            _data
        ); // neglect the prizeCounts since we are not interested in them here

        uint256 drawPayoutsLength = drawPayouts.length;
        for (
            uint256 payoutIndex = 0;
            payoutIndex < drawPayoutsLength;
            payoutIndex++
        ) {
    ...
    /// @inheritdoc IPrizeDistributor
    function setDrawCalculator(
        IDrawCalculator _newCalculator
    ) external override onlyOwner returns (IDrawCalculator) {
        _setDrawCalculator(_newCalculator);
        return _newCalculator;
    }

    /**
     * @notice Set fee for protocol.
     * @param _feeForProtocol fee for Protocol
     */
    function setWinningFee(uint256 _feeForProtocol) external onlyOwner {
        require(_feeForProtocol <= BASIC_POINTS, "PrizeDistributor/fee-to-big");
        feeForProtocol = _feeForProtocol;
    }

    /**
     * @notice Set address where fee will go.
     * @param _foundationWallet address for Protocol fee
     */
    function setFoundationWallet(address _foundationWallet) external onlyOwner {
        require(
            _foundationWallet != address(0),
            "PrizeDistributor/foundationWallet-is-addr-zero"
        );
    ...
    function _getDrawPayoutBalanceOf(
        address _user,
        uint32 _drawId
    ) internal view returns (uint256) {
    ...
    function _setDrawCalculator(IDrawCalculator _newCalculator) internal {
        require(
            address(_newCalculator) != address(0),
