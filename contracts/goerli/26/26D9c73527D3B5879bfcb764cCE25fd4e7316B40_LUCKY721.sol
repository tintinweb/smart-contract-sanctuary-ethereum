...
// 
contract LUCKY721 is ERC721, Ownable {
    uint256 count = 1;
    ...
    constructor(
        ...
        setContractURI(_contractURI, _royaltyAddress, _royaltyFee);
    }
    ...
    function royaltyLength() public view returns (uint256) {
        return royaltyAddress.length;
    }
    ...
    event NFTY_ContractURIChanged(string, address[], uint256[]);
...
// 
contract LUCKY1155 is ERC1155, Ownable {
    ...
    function setURI(
        ...
        uris[_id] = _uri;
        unlockContentUris[_id] = _contentUri;
        emit NFTY_TokenURIChanged(_id, _uri, _contentUri);
    }

    function uri(uint256 _id) public view override returns (string memory) {
        require(_exists(_id), "ERC1155#uri: NONEXISTENT_TOKEN");

        string memory _tokenURI = uris[_id];
        return _tokenURI;
    }
    ...
    function mint(
        ...
        uint256 tokenId = _id;
        require(
            tokenSupply[tokenId] + _quantity <= tokenMaxSupply[tokenId],
            "Max supply reached"
        );
...
contract OrderManager is Ownable {
    ...
    function createOrder(Order memory order) external returns (bool) {
        ...
        if (order._contractType == 0) {
            require(
                msg.sender == ERC721(order._nftAddress).ownerOf(order._tokenId)
            );
        } else if (order._contractType == 1) {
            require(
                ERC1155(order._nftAddress).balanceOf(
                    msg.sender,
                    order._tokenId
                ) >= order._amount
            );
    ...
    function buyAsset(Order memory order, uint256 amount)
        external
        payable
        returns (bool)
    {
        ...
        if (orders[hash]._currencyTokenAddress == address(0)) {
            ...
            IERC20(orders[hash]._currencyTokenAddress).transferFrom(
                msg.sender,
                address(this),
                order._price * amount
            );
        ...
        finalizeOrder(orders[hash], amount);
    ...
    function placeBid(Order memory order) external payable returns (bool) {
        ...
        if (orders[hash]._buyPrice != orders[hash]._price) {
            if (order._currencyTokenAddress == address(0)) {
                ...
                IERC20(order._currencyTokenAddress).transfer(
                    order._buyer,
                    orders[hash]._buyPrice * orders[hash]._amount
                );
            }
    ...
    function makeOffer(
        ...
        if (_offerCurrencyTokenAddress == address(0)) {
            ...
            IERC20(_offerCurrencyTokenAddress).transferFrom(
                msg.sender,
                address(this),
                _price * _amount
            );
    ...
    function acceptOffer(Token memory token, Offer memory offer)
        external
        payable
        returns (bool)
    {
        ...
        if (token._contractType == 0) {
            require(
                msg.sender == ERC721(token._address).ownerOf(token._tokenId)
            );
        } else if (token._contractType == 1) {
            require(
                ERC1155(token._address).balanceOf(msg.sender, token._tokenId) >=
                    offer._offerAmount
            );
        } else {
            return false;
        ...
        returns (bool)
    {
        ...
        if (offer._offerCurrencyTokenAddress == address(0)) {
            ...
            IERC20(offer._offerCurrencyTokenAddress).transfer(
                offer._offerAddress,
                offer._offerPrice * offer._offerAmount
            );
        }
    ...
    function finalizeOrder(Order memory order, uint256 amount)
        public
        returns (bool)
    {
        bytes32 hash = hashOrder(order);
    ...
    function withdrawToken(
        ...
        if (_token._contractType == 0) {
            LUCKY721(_token._address).transferFrom(_from, _to, _token._tokenId);
        } else if (_token._contractType == 1) {
            LUCKY1155(_token._address).safeTransferFrom(
                _from,
                _to,
                _token._tokenId,
                _amount,
    ...
    function withdrawFunds(
        ...
        uint256 total_royalty_fee = 0;

        uint256 royalty_length = 0;

        if (_contractType == 0) {
            royalty_length = LUCKY721(_nftAddress).royaltyLength();
        } else {
            royalty_length = LUCKY1155(_nftAddress).royaltyLength();
        }
            ...
            for (uint8 i = 0; i < 2; i++) {
                if (marketplaceRoyaltyAddress[i] != address(0)) {
                    payable(address(uint160(marketplaceRoyaltyAddress[i])))
                        .transfer((_price * marketplaceRoyaltyFee[i]) / 10**8);
                    total_royalty_fee += marketplaceRoyaltyFee[i];
                }
            }

            for (uint8 i = 0; i < royalty_length; i++) {
                if (_contractType == 0) {
                    royalty_fee = LUCKY721(_nftAddress).royaltyFee(i) * 10**6;
                    royalty_address = LUCKY721(_nftAddress).royaltyAddress(i);
                } else {
                    royalty_fee = LUCKY1155(_nftAddress).royaltyFee(i) * 10**6;
                    royalty_address = LUCKY1155(_nftAddress).royaltyAddress(i);
                }

                if (royalty_address != address(0)) {
                    payable(address(uint160(royalty_address))).transfer(
                        (_price * royalty_fee) / 10**8
                    );

                    total_royalty_fee += royalty_fee;
                }
            }

            payable(address(uint160(_to))).transfer(
                (_price * (10**8 - total_royalty_fee)) / 10**8
            );
        }
        //transfer Token
        else {
            for (uint8 i = 0; i < 2; i++) {
                if (marketplaceRoyaltyAddress[i] != address(0)) {
                    IERC20(_currencyTokenAddress).transfer(
                        marketplaceRoyaltyAddress[i],
                        (_price * marketplaceRoyaltyFee[i]) / 10**8
                    );
                    total_royalty_fee += marketplaceRoyaltyFee[i];
                }
            }
