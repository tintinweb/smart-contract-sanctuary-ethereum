...
contract Vendor is Ownable {
    ...
    function sellTokens(uint256 amountToSell) public payable {
        // console.log('amountToSell', amountToSell);  // The print number unit is Wei

        // Sender send token to vendor
        yourToken.transferFrom(msg.sender, address(this), amountToSell);
        // Send ETH to the sender
        (bool success, ) = msg.sender.call{value: amountToSell / tokensPerEth}('');
        require(success, 'ETH withdraw failed.');

        emit SellTokens(msg.sender, amountToSell, amountToSell / tokensPerEth);
    }
}
