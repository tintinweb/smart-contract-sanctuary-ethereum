    ...
    IRebalancer
{
    ...
    /// @notice max slippage precision
    uint public constant MAX_SLIPPAGE_DECIMALS = 4;

    /// @notice oracle precision
    uint public constant ORACLE_DECIMALS = 8;
    ...
    function rebalance(address _market)
        ...
        whenNotPaused
    {
        IRebalanceCalculation calc = IRebalanceCalculation(IMarket(_market).getRiskCalc());

        (uint sellToken, uint sellStable) = calc.getSellAmount();
        require(sellToken != 0 ||sellStable != 0, "R: rebalance-not-needed");

        (int changeToken, int changeStable) = _swap(sellToken, sellStable, _market);

        RebalanceAmount memory upd = calc.rebalanceAmount(changeToken, changeStable);
    ...
    /// @inheritdoc IRebalancer
    /// @dev {_maxSlippage} should be set with precision of {MAX_SLIPPAGE_DECIMALS}
    function setSwap(
        address _swapFrom,
        address _swapTo,
        uint _maxSwapAmount,
    ...
    function isRebalanceNeeded(address _market)
        ...
        returns (bool)
    {
        (uint sellToken, uint sellStable) = IRebalanceCalculation(
            IMarket(_market).getRiskCalc()
        ).getSellAmount();

        if (sellToken != 0 ||sellStable != 0) return true;
    ...
    function _swap(uint sellToken, uint sellStable, address _market) internal returns (int changeToken, int changeStable){
        console.log("_rebalance", sellToken, sellStable);

        (int oraclePrice, , ) = IState(IMarket(_market).getState()).price();
        if (sellToken > 0) {
            ...
            uint sellTokenLimited = sellToken;
            if (sellTokenLimited > swap.maxSwapAmount && swap.maxSwapAmount != 0)
                sellTokenLimited = swap.maxSwapAmount;

            token.safeTransferFrom(_market, address(this), sellToken);

            _approveToMax(token, swap.swapAdapter, sellToken);
            ...
            changeToken = -int(sellTokenLimited);
            changeStable = int(bstAmount);
        }
        ...
        if (sellStable > 0) {
            ...
            if (sellStable > swap.maxSwapAmount && swap.maxSwapAmount != 0)
                sellStable = swap.maxSwapAmount;

            bst.withdraw(stableToken, sellStable, _market, address(this));
            ...
            uint amountInStableToken = sellStable * 
                (10**IERC20Metadata(stableToken).decimals()) / 10**18;

            uint amountOut = ISwap(swap.swapAdapter).swapExactTokensForTokensMult(
                    swap.path,
                    amountInStableToken,
                    _getSlippageStable(
                        address(token),
            ...
            changeToken = int(amountOut);
            changeStable = -int(sellStable);
        }
    }

    function _getSlippageAsset(
        address assetToken,
        address stableToken,
        uint amouintIn,
        uint slippage,
        ...
        uint decimalsAsset = IERC20Metadata(assetToken).decimals();
        uint decimalsStable = IERC20Metadata(stableToken).decimals();

        uint amountOut = (oraclePrice * amouintIn) /
            (10**(decimalsAsset + ORACLE_DECIMALS - decimalsStable));

        return
            amountOut -
            ((amountOut * slippage) / (100 * 10**MAX_SLIPPAGE_DECIMALS));
    }

    function _getSlippageStable(
        address assetToken,
        address stableToken,
        uint amouintIn,
        uint slippage,
