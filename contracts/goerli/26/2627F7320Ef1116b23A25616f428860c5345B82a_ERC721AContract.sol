...
contract ERC721AContract is ERC721A, Ownable, PaymentSplitter, DefaultOperatorFilterer {
    ...
    modifier onlyPaper(bytes32 _hash, bytes32 _nonce, bytes calldata _signature) {
        ...
        require(success, "Signature failure");
        _;
    }
    ...
    function updateConfig(
        ...
        if (token.supplyLock) {
            require(_maxSupply == token.maxSupply, "Supply locked");
        }
    ...
    function setReferralPercent(uint8 _percent) external onlyOwner {
    ...
    function updateSaleState(
        ...
        require(_preSalePhase == 1 || _preSalePhase == 2, "Invalid phase");
    ...
    function isEligible(
        ...
        bool _pubSaleIsActive = token.pubSaleIsActive;
        bool _isAllowlisted;
        require(_preSaleIsActive || _pubSaleIsActive, "Sale inactive");
        require(uint16(totalSupply()) + _quantity <= token.maxSupply, "Insufficient supply");
        if(_preSaleIsActive) {
            if (token.preSalePhase == 1) {
                _isAllowlisted = mintpass.balanceOf(_address, 1) >= _quantity;
            }
            if (token.preSalePhase == 2) {
                require((hasMinted[_address] + _quantity) <= token.maxPerWallet, "Invalid quantity");
                bytes32 leaf = keccak256(abi.encodePacked(_address));
                _isAllowlisted = MerkleProof.verify(_proof, saleMerkleRoot, leaf);
            }
        }
        require(_isAllowlisted || _pubSaleIsActive, "Not allowlisted");
        if (!_isAllowlisted) {
            require(_pubSaleIsActive, "Sale inactive");
            require(_quantity <= token.maxPerTransaction, "Invalid quantity");
        }
        return (true, _isAllowlisted);
    }
    ...
    function _payReferral(address _recipient, address payable _referrer, uint256 _quantity, uint256 _value) internal {
        if (_referrer != address(0) && _referrer != _recipient) {
            ...
            payable(_referrer).transfer(_commission);
        }
    }
    ...
    function _mintTo(
        ...
        (bool _success, bool _isAllowlisted) = isEligible(_address, uint16(_quantity), _proof);
        if (_success) {
            if (_isAllowlisted) {
                require(token.preSalePrice * _quantity <= _value, "ETH incorrect");
                if (token.preSalePhase == 1) mintpass.burnForAddress(1, _quantity, msg.sender);
                if (token.preSalePhase == 2) hasMinted[_address] += uint16(_quantity);
            } else {
    ...
    function crossmint(address _address, uint256 _quantity, bytes32[] memory _proof, address payable _referrer) external payable onlyCrossmint {
    ...
    function paper(address _address, uint256 _quantity, bytes32[] memory _proof, address payable _referrer, bytes32 _nonce, bytes calldata _signature) public payable 
        onlyPaper(keccak256(abi.encode(_address, _quantity, _proof, _referrer)), _nonce, _signature)
    {
        _mintTo(_address, _quantity, _proof, _referrer, msg.value);
    }
