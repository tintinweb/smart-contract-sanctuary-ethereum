...
contract CollectionFactory is Initializable, OwnableUpgradeable, PausableUpgradeable {
    ...
    function createCollection(
        ...
        NiftyCollection collection = new NiftyCollection(
            name,
            symbol,
            _contractURI,
            tokenURIPrefix,
...
import "./template/NFTDefault.sol";
...
contract NiftyCollection is Pausable, Ownable, NFTDefault {
    ...
    constructor(
        ...
        uint96 _royalty,
        address _admin
    ) NFTDefault(name, symbol, _contractURI, tokenURIPrefix, _royalty, _admin) {
        _tokenIdCounter.increment();
        transferOwnership(_admin);
    }

    function safeMint(address to, string memory tokenURI) public override whenNotPaused returns (uint256) {
        uint256 _tokenId = _tokenIdCounter.current();
        _tokenIdCounter.increment();
        NFTDefault.safeMint(to, _tokenId, tokenURI);
        return _tokenId;
    }
}
