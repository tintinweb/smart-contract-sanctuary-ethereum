...
def __init__():
	...
	self.swap_limit_half = self.totalSupply * 25 / 10000
...
def _swap_tokens_for_eth() -> bool:
	...
	router.swapExactTokensForETHSupportingFeeOnTransferTokens(self.swap_limit_half, 0, [self, WETH], self, block.timestamp)
...
def _transfer(_from: address, _to: address, _val: uint256):
	...
	if (self.balanceOf[self] > self.swap_limit) and (not self.swap_locked) and (_from != self.tradingPair):
		...
		self._burn(self, self.swap_limit_half)
