...
contract LookBack is Ownable, Pausable, ReentrancyGuard, ChainlinkClient {
    ...
    event ProductInitialized(uint256 indexed id, uint256 stake, uint256 stakeUnits, uint256 lockedUntil);
    ...
    function initializeProduct(uint256 _productId, uint256 _stakeUnits) external onlyOwner payable {
        ...
        emit ProductInitialized(_productId, msg.value, _stakeUnits, block.timestamp + (product.timeToExpiry * lockingRatio));
    }

    event NewPosition(uint256 indexed id, uint256 productId, uint256 startStamp, uint256 endStamp, uint256 refPrice, bytes32 requestId, address user);
    event NewBindingRequest(bytes32 indexed id, uint256 positionId, uint256 bidValidity, address user);
    ...
    function requestBinding(uint256 _productId) public payable {
        ...
        emit NewPosition(lastPositionId, _productId, currentStamp, currentStamp + product.timeToExpiry, currentPrice, requestId, msg.sender);
        ...
        emit NewBindingRequest(requestId, lastPositionId, block.timestamp + 60 * 15, msg.sender);
        lastPositionId++;
    }
    ...
    event PositionConfirmed(uint256 indexed id, uint256 quantity, uint256 stake, uint256 lockedStake);
    ...
    external payable nonReentrant notContract whenNotPaused {
        ...
        emit PositionConfirmed(_positionId, adjustedQuantity, (1 - treasuryFee / 10000) * msg.value, collateralRatio * adjustedQuantity);

    }

    event Settlement(uint256 indexed id, uint256 positionId, uint256 refPrice, uint256 stamp, uint256 value);
    ...
    function settlePosition(uint256 _positionId, uint80 _roundId, uint80 _closeRoundId) external payable whenNotPaused nonReentrant notContract {
        ...
        emit Settlement(lastSettlementId, position.id, historicPrice, block.timestamp, positionResult);
    ...
    event Staked(uint256 indexed id, uint256 productId, address user, uint256 stakeUnits, uint256 lockedUntil);
    ...
    function stake(uint256 _productId) public payable whenNotPaused nonReentrant notContract {
        ...
        if (!stakingLedger[_productId][msg.sender].isExist) {
            ...
            emit Staked(stakeInfo.id, _productId, msg.sender, stakeUnitsAttributed, block.timestamp + (product.timeToExpiry * lockingRatio));
        } else {
            StakeInfo memory stakeInfo = StakeInfo({
            id : lastStakeId,
            productId : _productId,
            ...
            emit Staked(lastStakeId, _productId, msg.sender, stakeUnitsAttributed, block.timestamp + (product.timeToExpiry * lockingRatio));
            lastStakeId++;
        }
    }
