...
abstract contract BaseAggregator is Ownable {
    ...
    address internal _srcContract;
    uint256 internal LZ_FEE = 0.3 ether;

    modifier onlySrcContract {
        require(msg.sender == _srcContract, "Caller is not the source smart contract");
        _;
    }

    receive() external payable {}

    function currentLZFee() external view returns(uint256) {
        return LZ_FEE;
    }
    ...
    function setSrcContract(address __srcContract) external onlyOwner {
        _srcContract = __srcContract;
    }
    ...
    function setLZFee(uint256 amount) external onlyOwner {
        LZ_FEE = amount;
    }
}
 ...
 * Each collateral asset should have its own RewardEscrowV2Aggregator
 */

contract RewardEscrowV2Aggregator is BaseAggregator, NonblockingLzApp {
    using BytesLib for bytes;
    using EnumerableSet for EnumerableSet.Uint16Set;

    // uint public counter;
    uint16 public constant PT_APPEND = 1;
    uint16 public constant PT_VEST = 2;

    uint256 private _totalEscrowedBalance;
    mapping(address => uint256) _totalEscrowedAccountBalance;

    event Append(uint16 indexed srcChainId, address indexed account, uint256 amount);
    event Vest(uint16 indexed srcChainId, address indexed account, uint256 amount);
    ...
    function _nonblockingLzReceive(uint16 _srcChainId, bytes memory _srcAddress, uint64 _nonce, bytes memory _payload) internal override {
        ...
        if (packetType == PT_APPEND) {
            (, bytes memory accountBytes, uint256 amount) = abi.decode(_payload, (uint16, bytes, uint256));
            address account = accountBytes.toAddress(0);

            _append(account, amount);(account, amount);
            emit Append(_srcChainId, account, amount);
        } else if (packetType == PT_VEST) {
            (, bytes memory accountBytes, uint256 amount) = abi.decode(_payload, (uint16, bytes, uint256));
            address account = accountBytes.toAddress(0);

            _vest(account, amount);
            emit Vest(_srcChainId, account, amount);
        } else {
            revert("SynthAggregator: unknown packet type");
        }
    }

    function _append(address account, uint256 amount) private {
        _totalEscrowedAccountBalance[account] += amount;
        _totalEscrowedBalance += amount;
    }

    function _vest(address account, uint256 amount) private {
        _totalEscrowedAccountBalance[account] -= amount;
        _totalEscrowedBalance -= amount;
    }

    // send messages
    function sendAppend(address account, uint256 amount) external onlySrcContract {
        _append(account, amount);

        bytes memory lzPayload = abi.encode(PT_APPEND, abi.encodePacked(account), amount);

        uint256 length = _supportedChains.length();

        for (uint256 ii = 0; ii < length; ii++) {
            _lzSend(_supportedChains.at(ii), lzPayload, payable(address(this)), address(0x0), bytes(""), LZ_FEE);
        }
    }

    function sendVest(address account, uint256 amount) external onlySrcContract {
        _vest(account, amount);

        bytes memory lzPayload = abi.encode(PT_VEST, abi.encodePacked(account), amount);
...
abstract contract LzApp is Ownable, ILayerZeroReceiver, ILayerZeroUserApplicationConfig {
    ...
    function _lzSend(uint16 _dstChainId, bytes memory _payload, address payable _refundAddress, address _zroPaymentAddress, bytes memory _adapterParams, uint _nativeFee) internal virtual {
        ...
        console.log("_lzSend", address(this));
        lzEndpoint.send{value: _nativeFee}(_dstChainId, trustedRemote, _payload, _refundAddress, _zroPaymentAddress, _adapterParams);
        console.log("msg sent", address(this));
    }
...
library EnumerableSet {
    ...
    function at(Uint16Set storage set, uint256 index) internal view returns (uint16) {
        return uint16(uint256(_at(set._inner, index)));
    }
