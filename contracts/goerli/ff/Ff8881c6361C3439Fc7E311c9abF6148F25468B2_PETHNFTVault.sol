...
contract PETHNFTVault is AccessControlUpgradeable, ReentrancyGuardUpgradeable {
    ...
    /// @notice Allows the router to import a position with the specified parameters without minting any PETH.
    ...
    function importPosition(
        ...
        if (_strategy != address(0)) {
            if (
                ...
                INFTStrategy(_strategy).kind() != INFTStrategy.Kind.STANDARD ||
                !INFTStrategy(_strategy).isDeposited(_account, _nftIndex)
            ) revert InvalidStrategy();

            position.strategy = INFTStrategy(_strategy);
    ...
    function forceClosePosition(
        ...
        INFTStrategy _strategy = position.strategy;
    ...
    function _withdrawFromStrategy(uint256[] calldata _nftIndexes) internal {
        uint256 length = _nftIndexes.length;
        if (length == 0) revert InvalidLength();
...
interface INFTValueProvider {
    function getCreditLimitETH(address _owner, uint256 _nftIndex) external view returns (uint256);
    function getLiquidationLimitETH(address _owner, uint256 _nftIndex) external view returns (uint256);
}
