...
contract saleteste is IBEP20, ReentrancyGuard, ERC721Holder, Context, Ownable {
    ...
    uint256 public EMISSION_RATE = (15 * 10 ** _decimals) / 1 days;
	...
	uint256 private constant _totalSupply = 80 * 10 ** 6 * 10 ** _decimals; // 80M
    ...
    mapping(uint256 => uint256) public mapeiatoken;
    ...
    function stake(uint256 tokenId, uint256 cargo) external {
        require(balanceOf(address(this)) >= 10000, "Insufficient tokens for stake");
    ...
    function unstake(uint256 tokenId, uint256 cargo) external {
        ...
        if (cargo == 1) {
            ...
            if (mapeiatoken[tokenId] >= 3) {

                nft.transferFrom(address(this), address(0), tokenId); // new
    ...
    function refRewards() public {
        ...
        uint256 mult = _points[msg.sender] * 30_000000000;
