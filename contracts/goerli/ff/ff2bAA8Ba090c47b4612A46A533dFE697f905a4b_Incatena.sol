...
contract Incatena is AdminControl, ICreatorExtensionTokenURI{
    ...
    function _render(uint256 tokenId) internal view returns (string memory) {
        string memory image = string(abi.encodePacked(
            ...
            "7e8ffa' d='M",
            tokenData[tokenId].initialOffset,
            " "
        ));

        for (uint16 r = 0; r < tokenData[tokenId].chunks.length; r++) {
            uint16 pm = tokenData[tokenId].chunks[r][1];
            string memory pmstr = Strings.toString(pm);
            if (pm > 512) {
                pmstr = string.concat("-", Strings.toString(pm - 512));
            }

            image = string.concat(image, string(abi.encodePacked(
                Strings.toString(tokenData[tokenId].chunks[r][0]),
                "h",
                Strings.toString(tokenData[tokenId].chunks[r][1]),
                "m",
                pmstr,
                " "
            )));
