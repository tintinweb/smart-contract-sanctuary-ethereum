...
// File contracts/plugins/ETHPlugin.sol



pragma solidity ^0.8.4;
/// File: contracts/ETHPlugin.sol
///
/// @notice This plugin is for unwrapping WETH in the proxy contracts

contract ETHPlugin is IETHPlugin, Ownable {
    ...
    receive() external payable {
        require(
            ...
            "ETHPlugin: do not send ETH directly"
        );
    }

    function unwrap(address weth_, uint256 amount_) external override {
        IERC20(weth_).safeTransferFrom(_msgSender(), address(this), amount_);
        IWETH(weth_).withdraw(amount_);
