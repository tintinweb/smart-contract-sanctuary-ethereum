...
contract MyContract is ERC721Drop, PermissionsEnumerable
{
    using TWStrings for uint256;
    string private _cover_url = "";
    bool private _isCovered = true;
    //the role id.
    bytes32 public constant COVER_REVEAL_ROLE = keccak256("COVER_REVEAL_ROLE");
    ...
    constructor(
        ...
        //set the default role to reveal...
        _setupRole(COVER_REVEAL_ROLE, msg.sender);

    }

    function setCoverUrl(string calldata url) public onlyRole(COVER_REVEAL_ROLE){
//        string memory old = _cover_url;
        _cover_url = url;
//        emit Message(string.concat("replace the cover url from ", old));
    }

    function getIsCovered()
        public
        view
        onlyRole(COVER_REVEAL_ROLE)
        returns (bool)
    {
        return _isCovered;
    }
    ...
    function tokenURI(
        ...
        if (_isCovered == true) {
            if( bytes(_cover_url).length > 0 ){
//                returnValue = string(abi.encodePacked(_cover_url, _tokenId.toString()));
                returnValue = _cover_url;
            }
        } else {
            returnValue = super.tokenURI(_tokenId);
        }
        return returnValue;
    }

    function switchMeta() public onlyRole(COVER_REVEAL_ROLE) {
        _isCovered = _isCovered ? false : true;
        emit Message(string(abi.encodePacked("Switch meta to the ", _isCovered ? "cover" : "real one")));
    }
