...
contract PGNetworkRegistry is IXReceiver, IPGRegistry, MemberRegistry, OwnableUpgradeable {
    ...
    event SyncedActionPerfored(bytes4 indexed _action, uint32 indexed _chainId, bytes32 indexed _transferId, address _registryAddress);
    ...
    function _executeXCall(uint32 _chainId, bytes memory _callData, uint256 _relayerFee) internal returns (bytes32 transferId) {
        _validateNetworkRegistry(networkRegistry[_chainId]);
    ...
    function syncSetNewMember(
        ...
        for (uint8 i = 0; i < _chainIds.length; i++) {
            ...
            bytes memory callData = abi.encode(IPGRegistry.setNewMember.selector, _member, _activityMultiplier, _startDate);
            bytes32 transferId = _executeXCall(chainId, callData, relayerFee);
            emit SyncedActionPerfored(IPGRegistry.setNewMember.selector, chainId, transferId, networkRegistry[chainId].registryAddress);
        }
    }
        ...
        onlyOwner
    {
        ...
        for (uint8 i = 0; i < _chainIds.length; i++) {
            uint32 chainId = _chainIds[i];
            uint256 relayerFee = _relayerFees[i];
            bytes memory callData = abi.encode(IPGRegistry.updateMember.selector, _member, _activityMultiplier);
            bytes32 transferId = _executeXCall(chainId, callData, relayerFee);
            emit SyncedActionPerfored(IPGRegistry.updateMember.selector, chainId, transferId, networkRegistry[chainId].registryAddress);
        }
    }
