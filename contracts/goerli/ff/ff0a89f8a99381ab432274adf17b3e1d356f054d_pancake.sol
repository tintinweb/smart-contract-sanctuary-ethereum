...
contract pancake is VRFConsumerBaseV2, Ownable{
    ...
    event LotteryRolled(uint256 requestId, uint256 round);
    event LotteryLanded(uint256 requestId, uint256 value0, uint256 value1, uint256 value2);
    ...
    function fulfillRandomWords(
        ...
        winning_numbers[this_round][0] = value0;
        winning_numbers[this_round][1] = value1;
        winning_numbers[this_round][2] = value2;

        emit LotteryLanded(requestId, value0, value1, value2);
    }

    function lottery_set(uint256 this_round) public onlyOwner returns (uint256 requestId) {
        require(winning_numbers[this_round].length == 0, "Already rolled");
        ...
        emit LotteryRolled(requestId, this_round);
    }
    ...
    function check_round_info(uint256 request_id) public view returns(uint256){
        return round_mapping[request_id];
    }
