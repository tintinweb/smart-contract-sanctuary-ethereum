...
contract MyFirstNFT is ERC721A, Ownable {
    ...
    string public baseURI = "ipfs://QmcHFsQTdn8akZ1GGwLtW9pHMD8QKPhmPsjRXausKNZiba"; // 這一行是 NFT 該去哪裡找你的 MetaData
    bool public paused = false; // 可以拿來暫停或者開啟 Mint
    uint256 public cost = 0.0012 ether; // Mint 價格
    uint256 public maxSupply = 10; // 只有三個 NFT
    uint256 public maxMintAmount = 3; // 一次最多只能 Mint 一個

    constructor() ERC721A("GameCard", "game") payable {
    ...
    function mint(uint256 quantity) public payable {
        require(paused != true, "Sale must be active"); // 合約必須不是暫停
        require(quantity > 0); // 每次必須鑄造超過 0 個
        require(quantity <= maxMintAmount, "You can only adopt 1 BigBenFun at a time"); // 鑄造的數量不可以大於每次最大鑄造數量
        require(quantity + _tokenIdCounter.current() < maxSupply, "You can't mint over max nft supply amount"); // 鑄造的數量加已鑄造的數量不可以大於供應的總量
        require(cost * quantity <= msg.value, "Ether value sent is not correct"); // Mint 的價格不可以少於我們訂定的價格
    ...
    function getMintedToken() public view returns (uint256){
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        ...
        return bytes(currentBaseURI).length > 0 ? string(abi.encodePacked(currentBaseURI, "/METAFANTASY-", Strings.toString(tokenId), ".json")) : "";
    }
