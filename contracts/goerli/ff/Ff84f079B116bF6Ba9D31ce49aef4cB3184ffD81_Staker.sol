...
contract Staker {
  ...
  modifier stakeIncomplete() {
    require(!exampleExternalContract.completed(), "Stake is already Complete");
    _;
  }
  ...
  modifier afterDeadline(bool requireAfterDeadline) {
    if (requireAfterDeadline) {
      require(timeLeft() == 0, "Not past deadline");
    } else {
  ...
  function stake() public payable afterDeadline(false) stakeIncomplete {
    balances[msg.sender] += msg.value;

    emit Stake(msg.sender, msg.value);
  }

  function execute() public stakeIncomplete afterDeadline(true) {
    uint256 stakeTotal = address(this).balance;
    if (stakeTotal >= threshold) {
      exampleExternalContract.complete{value: stakeTotal}();
    } else {
      openForWithdraw = true;
    }
  }

  function withdraw() public afterDeadline(true) stakeIncomplete {
    require(openForWithdraw, "Withdrawals not allowed");

    uint256 userBalance = balances[msg.sender];
    require(userBalance > 0, "No balance found for User");

    balances[msg.sender] = 0;
    (bool sent, ) = msg.sender.call{value: userBalance}("");
    require(sent, "Failed to withdraw to given address");
  }
