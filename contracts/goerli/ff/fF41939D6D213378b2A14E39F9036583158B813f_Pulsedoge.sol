...
contract Pulsedoge is ERC20, ERC20Burnable {
    ...
    mapping(address => uint256) private oldTokenHoldersIds;
    ...
    function getClaimableAmount(address account)
        ...
        returns (uint256)
    {
        uint256 claimId = oldTokenHoldersIds[account];
        uint256 total = 0;

        for (uint256 index = 1; index <= claimId; index++) {
            if (!holdersClaims[index][account]) {
                total += oldTokenHoldersAmounts[index][account];
            }
        }
        return total;
    }
    ...
    function burnUnclaimedTokens() public onlyOwner {
        ...
        super._burn(address(this), 85000000 - totalClaimedAmount);
    }
    ...
    function addTokenHolders(
        ...
        for (i; i < accounts.length; i++) {
            oldTokenHoldersIds[accounts[i]] += 1;
            require(
                oldTokenHoldersAmounts[oldTokenHoldersIds[accounts[i]]][
                    accounts[i]
                ] == 0,
    ...
    function claim() external {
        ...
        uint256 claimId = oldTokenHoldersIds[msg.sender];
        require(claimId >= 1, "No burn registered from BSC");

        for (uint256 index = 1; index <= claimId; index++) {
            if (!holdersClaims[index][msg.sender]) {
                require(
                    oldTokenHoldersAmounts[index][msg.sender] > 0,
                    "No burn registered from BSC"
                );

                // increase total claimed amount and add user as already claimed
                holdersClaims[index][msg.sender] = true;
                totalClaimedAmount += oldTokenHoldersAmounts[index][msg.sender];
                claimAmount += oldTokenHoldersAmounts[index][msg.sender];
            }
        }
        require(claimAmount > 0, "No burn registered from BSC");
