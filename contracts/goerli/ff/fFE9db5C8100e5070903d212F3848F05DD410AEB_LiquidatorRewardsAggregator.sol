...
contract LiquidatorRewardsAggregator is BaseAggregator {
    ...
    uint16 public constant PT_NOTIFY_REWARD = 1;
    uint16 public constant PT_UPDATE_ENTRY = 2;

    uint private _gasNotify = 200000;
    uint private _gasUpdate = 200000;
    ...
    event NotifyReward(uint16 indexed srcChainId, bytes32 currencyKey, uint accumulatedPerShare, uint increasedShare);
    event UpdateEntry(uint16 indexed srcChainId, bytes32 currencyKey, address indexed account);
    ...
    function _nonblockingLzReceive(
        ...
        if (packetType == PT_NOTIFY_REWARD) {
            (, bytes32 currencyKey, uint increasedShare) = abi.decode(_payload, (uint16, bytes32, uint));
            _updateAccumulatedShare(currencyKey, increasedShare);
            emit NotifyReward(_srcChainId, currencyKey, accumulatedRewardsPerShare[currencyKey], increasedShare);
        } else if (packetType == PT_UPDATE_ENTRY) {
            (, bytes32 currencyKey, uint debtShare, bytes memory accountBytes) = abi.decode(_payload, (uint16, bytes32, uint, bytes));
            address account = accountBytes.toAddress(0);
            _updateEntry(currencyKey, debtShare, account);
            emit UpdateEntry(_srcChainId, currencyKey, account);
        } else {
    ...
    function sendNotifyReward(bytes32 currencyKey, uint increasedShare) external onlySrcContract {
        _updateAccumulatedShare(currencyKey, increasedShare);

        bytes memory lzPayload = abi.encode(PT_NOTIFY_REWARD, currencyKey, increasedShare);

        _broadcast(lzPayload, PT_NOTIFY_REWARD);
    }
    ...
    function sendUpdateEntry(
        ...
        _broadcast(lzPayload, PT_UPDATE_ENTRY);
    }
    ...
    /** Gas for LZ fee */
    function gasNotify() external view returns (uint) {
        return _gasNotify;
    }

    function gasUpdate() external view returns (uint) {
        return _gasUpdate;
    }

    function setGasNotify(uint newGas) external onlyOwner {
        _gasNotify = newGas;
    }

    function setGasUpdate(uint newGas) external onlyOwner {
        _gasUpdate = newGas;
    }
    ...
    function _lzAdapterParam(uint16 packetType) internal view override returns (bytes memory) {
        uint16 version = 1;
        if (packetType == PT_NOTIFY_REWARD) {
            return abi.encodePacked(version, _gasNotify);
        }

        return abi.encodePacked(version, _gasUpdate);
    }
}
