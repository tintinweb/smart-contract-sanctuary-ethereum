...
contract MarketPlace is TimelockAccess, IMarketPlace, IModuleBox, Ownable {
    ...
    function setBiometaToken(address _address) external override onlyTimelock nonZeroAddress(_address) {
        emit TokenAdressChanged(biometaToken, _address);
        biometaToken = _address;
    }

    function setMetaCellToken(address _address) external override onlyTimelock nonZeroAddress(_address) {
        emit TokenAdressChanged(metaCellToken, _address);
        metaCellToken = _address;
    }

    function setNanoCellToken(address _address) external override onlyTimelock nonZeroAddress(_address) {
        emit TokenAdressChanged(nanoCellToken, _address);
        nanoCellToken = _address;
    }

    function setLaboratory(address _address) external override onlyTimelock nonZeroAddress(_address) {
        emit LaboratoryAddressChanged(laboratory, _address);
        laboratory = _address;
    }

    function setModulesAddress(address _address) external override onlyTimelock nonZeroAddress(_address) {
    ...
    function setBiometaTokenPrice(uint256 _price) external override onlyTimelock {
    ...
    function setFeeWallet(address payable _feeWallet) external onlyTimelock nonZeroAddress(_feeWallet) {
    ...
    function buyBiometaToken(uint256 _amount) external payable override {
        ...
        require(
            msg.value == _amount.mul(biometaTokenPrice) / 1 ether,
        ...
        returns (address[] memory owners, uint256[] memory metaCells)
    {
        ...
        for (uint256 i = 0; i < metaCells.length; i++) {
            owners[i] = IERC721(metaCellToken).ownerOf(
                metaCells[i]
            );
        }
    ...
    function buyModule(uint256 id) external override {
        ...
        IERC20(biometaToken).approve(address(this), price.sub(feeAmount));
    ...
    function buyBox(address owner) external override {
        ...
        IERC20(biometaToken).transferFrom(msg.sender, address(this), pricePerBox);
