...
contract Lock {
    ...
    function transfer(address to, int amount) external {
        require(balance[msg.sender] >= amount, "Msg Sender value is larger than amount");
        balance[msg.sender] = balance[msg.sender] - amount;
        balance[to] = balance[to] + amount;
    }

    function transfer_points(address from, address to, int amount) external {
        require(balance[from] >= amount, "Balance is Greater than amount");
        require(from == msg.sender, "sender address is same");
