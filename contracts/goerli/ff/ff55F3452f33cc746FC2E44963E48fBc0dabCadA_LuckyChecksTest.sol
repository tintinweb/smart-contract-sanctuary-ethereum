...
contract LuckyChecksTest is ERC721Enumerable, Ownable {
    ...
    address descriptorAddress = 0x7294EDf01a1bC9297da9077C0EBbAB4b9cde7485;
    ICheckDescriptor public descriptor = ICheckDescriptor(descriptorAddress);
    
    uint256 token_price = 0.005 ether;

    constructor() ERC721("LuckyChecksTest", "LKYCHKS") {}
    ...
    function mint(uint32 count) external payable {
        ...
        for (uint32 i; i < count;) {
            ...
            unchecked { _tokenIds.increment(); ++i; }
        }
    }
    ...
    function generateSeed(uint256 tokenId) private pure returns (string memory) {
        return random(tokenId);
    }

    function random(uint256 tokenId) private pure returns (string memory) {
        return toHex(keccak256(abi.encodePacked(tokenId)));
    }
