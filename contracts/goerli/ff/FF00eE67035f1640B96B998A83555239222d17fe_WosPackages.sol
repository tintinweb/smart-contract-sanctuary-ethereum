...
contract WosPackages is ReentrancyGuard, Ownable, Tax {
    ...
    event WosPackageSold(uint256 indexed itemId, address buyer);

    mapping(uint256 => WosPackage) private wosPackageId;

    constructor(IERC20 _wos_contract_address) {
        require(
    ...
    function createWosPackage(uint256 price, uint256 wosAmount)
        external
        onlyOwner
    {
        ...
        wosPackageId[itemId] = WosPackage(
            itemId,
            price,
            wosAmount,
            address(0),
        ...
        IOperable(address(wosContract)).mint(wosAmount);

        emit NewWosPackage(itemId, price, wosAmount);
    }

    function buyWosPackage(uint256 itemId) external nonReentrant {
        ...
        bool sold = wosPackageId[itemId].sold;
        ...
        wosContract.safeTransfer(msg.sender, wosAmount);

        wosPackageId[itemId].sold = true;
        wosPackageId[itemId].buyer = msg.sender;

        emit WosPackageSold(itemId, msg.sender);
    }

    function getPackageData(uint256 itemId) external view returns (WosPackage memory) {
        return wosPackageId[itemId];
    }

    function totalWosPackages() external view returns  (uint256) {
        uint256 total = _total.current();
        return total;   
    }

    function getLastPackageId() external view returns  (uint256) {
...
contract Tax is CollateralWrapper{
    ...
    address private DG;
    address private JS;
    address private OpWallet;       //0.5 Marketing and sales, 0.5 Operations
    address private Agregadores;  

    constructor() {
        WosStakingAddress = 0x501593b50DBf10FDdfdd71b7dc4A2a984482DB0d;
        AutoSupply = 0xFd3E005D6Dd8C564C2Ed569F8f9790C6196608D4;
    ...
    function setAutoSupply(address cnt) external onlyOwner {
        require(cnt != address(0), "Address can not be zero");
        AutoSupply = cnt;    
    }

    function setStaking(address stakingAddress) external onlyOwner {
        require(stakingAddress != address(0), "Address can not be zero");
        WosStakingAddress = stakingAddress;    
    }

    function setOperators(address dg, address js) external onlyOwner {
        require(dg != address(0), "Address can not be zero");
        require(js != address(0), "Address can not be zero");
        DG = dg;
        JS = js;
    }

    function setOpWallet(address opAddress) external onlyOwner {
        require(opAddress != address(0), "Address can not be zero");
        OpWallet = opAddress;
    }

    function setAgg(address aggAddress) external onlyOwner {
        require(aggAddress != address(0), "Address can not be zero");
        Agregadores = aggAddress;
    }
    
    function _getPriceWithoutTax(uint256 price) internal pure returns (uint256){
    ...
    function _taxDistribution(uint256 price) internal {
        ...
        ISimpleStaking(WosStakingAddress).distribute();

        _ctSign("TX_T_STEP_2");
        _collateralTransfer(AutoSupply, amountDistribute / 2);

        _ctSign("TX_T_STEP_3");
        _collateralTransfer(OpWallet, amountDistribute);

        _ctSign("TX_T_STEP_4");
        _collateralTransfer(Agregadores, amountDistribute / 2);
...
contract CollateralWrapper is Ownable, CriticalTracer {
    using SafeERC20 for IERC20;
    ...
    constructor() {
        collateral = IERC20(address(0xf6CB913963FA2AF77EfC756f8Dd83C179fec71d8));
        tokenDecimals = IERC20Collateral(address(collateral)).decimals();
    }
    ...
    function setCollateralAddress(address tokenAddress) external onlyOwner {
    ...
    function approveCollateral(address spender, uint256 amount) external onlyOwner() {
        require(spender != address(0), _ctMsg("spender cannot be zero"));
        require(amount > 0, _ctMsg("amount cannot be zero"));
