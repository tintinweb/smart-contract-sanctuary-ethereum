...
contract GaspackPBT is PBTSimple, Ownable {
    ...
    constructor(string memory _baseuri, address[] memory chipAddresses, uint256[] memory tokenIds)
        PBTSimple("GaspackPBT", "GaspackPBT")
    {
        baseURI = _baseuri;
        _seedChipToTokenMapping(chipAddresses, tokenIds);
    }

    /// @param _signature The signature is an EIP-191 signature of (msgSender, blockhash),
    ///        where blockhash is the block hash for a recent block (blockNumberUsedInSig).
    function mintPBT(
        bytes calldata _signature,
        uint256 _blockNum
    ) external {
        _mintTokenWithChip(_signature, _blockNum);
    }

    function seedChip(address[] memory chipAddresses, uint256[] memory tokenIds) external onlyOwner {
        _seedChipToTokenMapping(chipAddresses, tokenIds);
    }
