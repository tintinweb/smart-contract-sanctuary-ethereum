...
// Import the Multiwrap base contract from the Solidity SDK.
import "@thirdweb-dev/contracts/base/ERC721Multiwrap.sol";

contract CustomMultiwrap is ERC721Multiwrap {
    // Call the constructor for the inherited `ERC721Multiwrap`.
    constructor(
        string memory _name,
        string memory _symbol,
        address _royaltyRecipient,
    ...
    // Override the `wrap` function.
    function wrap(
        Token[] calldata _tokensToWrap,
        string calldata _uriForWrappedToken,
        address _recipient
    ) public payable virtual override returns (uint256 tokenId) {
        // Add any custom code you like, here.
        for (uint256 i = 0; i < _tokensToWrap.length; i += 1) {
            // 2. You can add the 75 tokens only requirement here.
            require(
                _tokensToWrap[i].totalAmount == 75 ether,
                "Must wrap only 75 tokens."
            );

            /**
             *  3. To require that the token being wrapped is the FWB ERC20, you can do that by:
             *      - grant the FWB ERC20 contract address the ASSET_ROLE
             *      - revoke the ASSET_ROLE from address(0) to apply that restriction
             *
             *  These actions can be done post-deployment.
             */
        }
