...
contract HiIamAnAlien is ERC721A, Ownable {
    ...
    //Pricing
    //***************************************************************************************************
    uint256 public pricePresale = 0.006 ether; //set NFT price
    uint256 public pricePublic = 0.01 ether; //set NFT price

    //***************************************************************************************************
    //Supply and Reserve
    //***************************************************************************************************
    uint256 public maxSupply = 1111; // set max supply according to your need
    uint256 public reserve = 11; // management tokens

    //***************************************************************************************************
    //Max per Wallets
    //***************************************************************************************************
    uint256 public maxPreSaleMint = 2; //set how many NFTs can be minted per wallet in pre sale
    uint256 public maxPublicSaleMint = 1111; //set how many NFTs can be minted per wallet in public sale
    ...
    constructor(
        ...
        reserveWallet = 0xa1311b2FD826B24904F6Ef4e0428488B059a8465;
    }
    ...
    function mintReserve() external onlyOwner {
        ...
        require(
            supply + reserve <= maxSupply,
            "This transaction would exceed max supply."
        );

        _safeMint(reserveWallet, reserve);
    }

    //***************************************************************************************************
    //Presale Mint
    //***************************************************************************************************
    function preSale(bytes32[] calldata __proof, uint256 __count)
        external
        payable
    {
        uint256 supply = totalSupply();
        ...
        require(
            __count <= maxPreSaleMint,
            string(
                abi.encodePacked(
                    "You can only mint ",
                    Strings.toString(maxPreSaleMint)
                )
            )
        );
        require(
            tokenCount + __count <= maxPreSaleMint,
            string(
                abi.encodePacked(
                    "You can only mint ",
                    Strings.toString(maxPreSaleMint)
        ...
        require(
            MerkleProof.verify(__proof, merkleRootPresale, leaf),
            "Not whitelisted."
        );
    ...
    function publicSale(uint256 __count) external payable {
        ...
        require(__count > 0, "Count can not be 0");
        require(
            __count <= maxPublicSaleMint,
            string(
                abi.encodePacked(
                    "You can only mint ",
                    Strings.toString(maxPublicSaleMint)
                )
            )
        );
        require(
            tokenCount + __count <= maxPublicSaleMint,
            string(
                abi.encodePacked(
                    "You can only mint ",
                    Strings.toString(maxPublicSaleMint)
    ...
    //***************************************************************************************************
    function setPresalePrice(uint256 __price) public onlyOwner {
        pricePresale = __price;
    }

    function setPublicPrice(uint256 __price) public onlyOwner {
        pricePublic = __price;
    }
    ...
    function setMaxPublicSale(uint256 __number) public onlyOwner {
        maxPublicSaleMint = __number;
    }

    //***************************************************************************************************
    //Change merkle roots
    //***************************************************************************************************
    function setPresaleMerkleRoot(bytes32 __merkleRoot) public onlyOwner {
        merkleRootPresale = __merkleRoot;
    }
