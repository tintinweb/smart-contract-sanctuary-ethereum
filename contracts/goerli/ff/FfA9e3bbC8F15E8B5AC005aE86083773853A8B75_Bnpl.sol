...
contract Bnpl is NftReceiver, Ownable2Step, ReentrancyGuard {
    ...
    /**
     * @notice Address of the AllowedMarketModules contract.
     */
     ...
     *
     * @param serviceFee - A percentage of the price at which the token is offered in the market where the purchase will
     * be made, taken by the contract admin's as a fee on top of the buyer portion, measured in basis points (hundredths
     * of a percent). The max allowed value is 10000.
     * @param nonce - Nonce used by the lender when signing the off-chain service fee.
     * @param expiry - Date when the signature expires
     * @param signature - The ECDSA signature of the lender, obtained off-chain signing the following combination of
     * parameters:
     * - serviceFee,
     * - lender address
     * - nonce
    ...
    /**
     * @notice Initialize the `allowedMarketModules` and the `deployer` as the `admin`.
     */
    ...
    /**
     * @notice Used by the admin to change the address of the AllowedMarketModules contract.
     *
     * @param _allowedMarketModules - The new address of the AllowedMarketModules contract.
     */
    ...
    function execute(Execution calldata _params) external nonReentrant returns (bool) {
        if (_params.assetType != ASSET_TYPE_ERC721 && _params.assetType != ASSET_TYPE_ERC1155) revert Bnpl__execute_invalidAssetType();
        if (!allowedMarketModules.isModuleAllowed(_params.module)) revert Bnpl__execute_invalidModule();

        // _validateServiceFee(_params);

        // get buyer portion of the total and calculate service fee
...
/**
 * @title  AllowedMarketModules
 * @author GoblinSax
 * @dev Registry for MarketModule contracts supported by BNPL. Each contract is
 * associated with a boolean allowance.
 */
