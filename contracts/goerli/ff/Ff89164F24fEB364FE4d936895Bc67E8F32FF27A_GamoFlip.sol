...
contract GamoFlip is VRFConsumerBaseV2, ReentrancyGuard {
    ...
    function getResults(address degenToPay, uint choice, uint ethBetted, string memory randomNumber, uint flipId, string memory requestId) external nonReentrant {

        uint requestIdUint = st2num(requestId);
        uint randomNumberUint = st2num(randomNumber);


        require(PreFlips[requestIdUint].requestId == requestIdUint, "Not correct flip");
        require(PreFlips[requestIdUint].degen == degenToPay, "Not correct degen");
        require(PreFlips[requestIdUint].open == true, "Flip closed");
        require(PreFlips[requestIdUint].choice == choice, "Not correct choice");
        require(PreFlips[requestIdUint].ethBetted == ethBetted, "Not correct amount");
        require(PreFlips[requestIdUint].flipId == flipId, "Not correct id");
        ...
        preFlip storage userPreFlip = PreFlips[requestIdUint];
