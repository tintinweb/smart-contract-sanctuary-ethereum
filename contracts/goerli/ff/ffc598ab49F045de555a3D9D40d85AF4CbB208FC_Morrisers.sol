...
contract Morrisers is ERC721, Ownable {
    ...
    // GET NFT DETAILS
    function getNFTDetails2(uint256 tokenId) public view returns (string memory) {
        return string(abi.encodePacked(uriPrefix, tokenId.toString(), uriSuffix));
    }

    //GET NFT DETAILS
    function getNFTDetails(uint256 tokenId) public view returns (string memory name , string memory description , string memory image , string memory animation_url , string memory external_url , string memory attributes) {
        return (name , description , image , animation_url , external_url , attributes);
    }
