...
// File: https://github.com/OpenZeppelin/openzeppelin-contracts/blob/v4.5.0/contracts/utils/introspection/IERC165.sol
...
// File: https://github.com/OpenZeppelin/openzeppelin-contracts/blob/v4.5.0/contracts/utils/introspection/ERC165.sol
...
// File: https://github.com/OpenZeppelin/openzeppelin-contracts/blob/v4.5.0/contracts/access/IAccessControl.sol
...
// File: https://github.com/OpenZeppelin/openzeppelin-contracts/blob/v4.5.0/contracts/access/AccessControl.sol
...
contract ChubbyKaijuDAOMarketplace is Ownable, AccessControl {
    ...
    function grantMarketingRole(address account) public onlyOwner {
        _grantRole(MARKETING_ROLE, account);
    }

    function revokeMarketingRole(address account) public onlyOwner {
        _revokeRole(MARKETING_ROLE, account);
    }

    function addItem(uint _id, uint _common_price, uint _holders_price, uint _max_supply) public {
        require(hasRole(MARKETING_ROLE, msg.sender), "Not allowed");
    ...
    function setItemActive(uint _id, bool _active) public {
        require(hasRole(MARKETING_ROLE, msg.sender), "Not allowed");
        require(getItemMaxSupply(_id) != 0, "Token does not exist");
        items[_id].active = _active;
    }

    function setItemMaxSupply(uint _id, uint _max_supply) public {
        require(hasRole(MARKETING_ROLE, msg.sender), "Not allowed");
        require(getItemMaxSupply(_id) != 0, "Token does not exist");
        items[_id].max_supply = _max_supply;
    }

    function setItemPrice(uint _id, uint _holders_price, uint _common_price) public {
        require(hasRole(MARKETING_ROLE, msg.sender), "Not allowed");
        require(getItemMaxSupply(_id) != 0, "Token does not exist");
        items[_id].common_price = _common_price;
        items[_id].holders_price = _holders_price;
    ...
    function isItemActive(uint _id) public view returns(bool){
        require(getItemMaxSupply(_id) != 0, "Token does not exist");
        return items[_id].active;
    }

    function isItemSoldOut(uint _id) public view returns(bool){
        require(getItemMaxSupply(_id) != 0, "Token does not exist");
        return items[_id].total_supply >= items[_id].max_supply;
    }
    ...
    function getItem(uint _id) public view returns(MarketplaceItem memory){
        MarketplaceItem memory _item = items[_id];
        return _item;
    }
    ...
    function whitelist(address _to, uint _id) public {
        ...
        require(isItemActive(_id), "Sale has not started");
        require(hasWhitelist(_to, _id) == false, "Address already whitelisted");
        
        uint price = getItemPrice(_id, _to);

        require(crunchContract.balanceOf(msg.sender) >= price, "Not enough balance");
        require(crunchContract.allowance(msg.sender, address(this)) >= price, "Low allowance");

        crunchContract.transferFrom(msg.sender, address(this), price);
    ...
    function gift(address _to, uint _id) public {
        require(hasRole(MARKETING_ROLE, msg.sender), "Not allowed");
        require(getItemTotalSupply(_id) <= getItemMaxSupply(_id), "Total supply limit reached or token does not exist");
