...
contract GPSmarketplace is Ownable, Pausable, FeeManager, IMarketplace, ERC721Holder {
    ...
    event versionPublished(address  nftOwner, uint256 _assetID, string nftVersion ,string  ipfshash);
    ...
    function publishVersion(address _nftAddress, uint256 _assetId, string memory versionNo, string memory ipfshasH) public {
             
        
        IERC721 nftTokenContract = IERC721(_nftAddress);
        address nftTokenOwner = nftTokenContract.ownerOf(_assetId);
        require (nftTokenOwner == msg.sender, "GPS : only nft owner can publish");

         nftID = _assetId;
         ipfshash = ipfshasH;
         emit versionPublished(nftTokenOwner,_assetId,versionNo,ipfshasH);
