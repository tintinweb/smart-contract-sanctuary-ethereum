...
contract wtcRaffle{
    ...
    address public stakingcontract = 0x515c137a94168E4db3802e84EE44a2913F7BF9e9;
    ...
    uint wtcraffleprice = 1000000000000000000; //default price for a single ticket
    uint nonwtcraffleprice = 1000000000000000000;// Default price for non WTC holders.
    ...
    function configOptions(uint option,address _address,uint _value,bool _yesorno) external onlyOwner{
        ...
        if (option==6)
        {
        wtcraffleprice = _value;
        }
        if (option==7)
        {
        nonwtcraffleprice = _value;
        }
        if (option==8)
        {
...
*/
    ...
    ///Setup a new Raffle
    function newRaffle(string memory _name,string memory _url,address _creator,uint _endtime,address _projectaddress,bool _istoken,bool _isopen,bool _nonwtconly,bool _wtconly,uint _valueornftnum) external {
    ...
    function getRaffleDetails(uint _rafflenumber) external view returns(string memory,string memory,address,uint,uint,address,address,bool,bool,uint)
    {
        Raffle storage temp = raffles[_rafflenumber];
        return (temp.rafflename,temp.imgurl,temp.creator,temp.tickets.length,temp.endtime,temp.winner,temp.project,temp.isopen,temp.wtconly,temp.prizevalue);
