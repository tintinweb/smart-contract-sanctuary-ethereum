...
contract CreatorDAO is ERC2771Context {
    ...
    function initialize( 
        ...
        safeFactory = GnosisSafeProxyFactory(address(_safeFactory));
        GnosisSafeProxy proxy =  safeFactory.createProxyWithNonce(address(_singleton), _setupData, _nonce);
    
        treasuryAddress = address(proxy);
        emit CreatedDAO(_name, treasuryAddress);
    }
