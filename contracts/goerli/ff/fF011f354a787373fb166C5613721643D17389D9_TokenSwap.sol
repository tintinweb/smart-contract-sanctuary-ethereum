...
// File @maticnetwork/fx-portal/contracts/lib/[email protected]
...
// File contracts/polygon/EFT.sol
...
// File contracts/polygon/Treasury.sol
...
contract TokenSwap is FxBaseRootTunnel {
  ...
  /// @notice ETH corresponding to EFT that was withdrawn on Polygon, but not yet retrieved
  ...
  function withdrawETH() external {
    require(withdrawBalances[msg.sender] > 0, "Nothing to withdraw");
    withdrawBalances[msg.sender] = 0;
    payable(msg.sender).transfer(withdrawBalances[msg.sender]);
  }

  /// @notice Called with messages from Polygon
  function _processMessageFromChild(
      bytes memory data
  ) internal override {
    //@todo Check if authentication needed
    address user;
    uint256 amount;
    (user, amount) = abi.decode(data, (address, uint256));
    withdrawBalances[user] += amount;
  }
}