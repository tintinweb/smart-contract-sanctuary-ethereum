...
import "./Collection/openzeppelin-solidity/contracts/ReentrancyGuard.sol";
// import "./Collection/multi-token-standard/contracts/Context.sol";
import "./Collection/multi-token-standard/contracts/utils/SafeMath.sol";
import "./Collection/multi-token-standard/contracts/interfaces/IERC1155TokenReceiver.sol";
import "./Collection/multi-token-standard/contracts/utils/Address.sol";
import "./Collection/multi-token-standard/contracts/tokens/ERC1155/ERC1155.sol";
import "./Collection/BELSContract.sol";
import "./Collection/TokenIdentifiers.sol";

/**
 * @title BELS2Collection
 * BELS collection contract - This contract used for creator create own NFT.
 */
contract BELS2Collection is BELSContract, ReentrancyGuard {
    ...
    modifier creatorOnly(uint256 _id) {
        require(
            ...
            "BELS2Collection #creatorOnly: ONLY_CREATOR_ALLOWED"
        );
        _;
    }
    ...
    function balanceOf(address _owner, uint256 _id)
        ...
        returns (uint256)
    {
        uint256 balance = ERC1155.balanceOf(_owner, _id);
        return
    ...
    function _requireMintable(
        ...
        require(
            ...
            "BELS2Collection #_requireMintable: ONLY_CREATOR_ALLOWED"
        );
        require(
            _remainingSupply(_id) >= _amount,
            "BELS2Collection #_requireMintable: SUPPLY_EXCEEDED"
        );
    }
    ...
    // s ERC1155Tradable to allow a shared proxy address
...
/**
 * @title BELSContract
 * BELSContract - used for creator to create own NFT.
 */
contract BELSContract is ERC1155Tradable {
    ...
    function BELSVersion() public pure returns (string memory) {
        return "2.0.0";
    }

    /**
     * Available to check the token balance,
     * and query the quantity that has not been minted.
     */
    ...
    function safeTransferFrom(
        ...
        if (!exists(_id)) {
            // Hack to allow marketplaces to mint
    ...
    function safeBatchTransferFrom(
        ...
        bool isMinting = !exists(_ids[0]);
        for (uint256 i = 1; i < _ids.length; i++) {
            require(
                !exists(_ids[i]) == isMinting,
                "BELSContract #safeBatchTransferFrom: MIXED_EXISTENCE_NOT_ALLOWED"
            );
        }
        if (isMinting) {
            batchMint(_to, _ids, _amounts, _data);
        } else {
    ...
    function mint(
        ...
        require(!exists(_id), "BELSContract #mint: TOKEN_EXISTS");
        _mint(_to, _id, _quantity, _data);
    }
    ...
    function batchMint(
        ...
        for (uint256 i = 0; i < _ids.length; i++) {
            require(!exists(_ids[i]), "BELSContract #batchMint: TOKEN_EXISTS");
        }
        _batchMint(_to, _ids, _quantities, _data);
    }
...
interface IERC1155 {
  ...
  */
   ...
   * @param _owner     The owner of the Tokensb/
