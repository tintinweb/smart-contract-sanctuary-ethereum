...
contract FactoryChallenge2 is IChallengeFactory {
  ...
  function deploy() external returns (address[] memory ret) {
    ...
    IERC20(_tokenInsecureum).transfer(msg.sender, 1 ether);
    IERC20(_tokenERC223).transfer(msg.sender, 1 ether);
...
/// @title This is a simple modified ERC223 token, copy from captureTheEther
/// @dev taken from https://capturetheether.com/challenges/miscellaneous/token-bank/
contract SimpleERC223Token is IERC20 {
    ...
    function transfer(address to, uint256 value, bytes memory data) public returns (bool) {
        ...
        if (Address.isContract(to)) {
            ...
            // instead of a try catch pehaps we should use a ERC165...
            // the tokenFallback function is run if the contract has this function
            try ITokenReceiver(to).tokenFallback(msg.sender, value, data) {
              // empty
            } catch {
              // empty
            }
