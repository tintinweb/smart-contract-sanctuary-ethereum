...
contract BSAPE is Context, IERC20, Ownable {
    ...
    address payable public treasuryWallet =
        payable(0xf99D628B68CeE70f38d02b7d2bF00fd4054b8f66);
    ...
    uint256 private FeeSendAfterEth=0;
    ...
    function setMaxTxnAmount(uint256 maxTxAmountPercetange) external onlyOwner {
        require(
            maxTxAmountPercetange < 1000,
            'Maximum amount per transaction must be lower than 100%'
        );
        require(
            maxTxAmountPercetange > 1,
            'Maximum amount per transaction must be higher than 0.1%'
        );
        _maxTxAmount = _tTotal.mul(maxTxAmountPercetange).div(1000);
        emit MaxTxAmountUpdated(_maxTxAmount);
    }

    function setMaxWalletSize(uint256 maxWalletSizePercentage)
        external
        onlyOwner
    {
        require(
            maxWalletSizePercentage < 1000,
            'Maximum wallet size must be lower than 100%'
        );
        require(
            maxWalletSizePercentage > 20,
            'Maximum wallet size must be higher than 2%'
        );
        ...
        if (
            ...
            if (_contractTokenBalance > 0) {
                if (
                    ...
                    balanceOf(uniswapV2Pair).mul(feeRate).div(100)
                ) {
    ...
    function _swapTokens(uint256 _contractTokenBalance) private lockTheSwap {
        ...
        buybackTokenBalance = buybackTokenBalance.add(treasuryETHBalance);
        if (buybackTokenBalance >= FeeSendAfterEth ) {
             _buyBackTokens(buybackTokenBalance);
    ...
    function SetAmountToReciveEth(uint256 amount) public onlyOwner {
        FeeSendAfterEth = amount ;//* 1e18
    }
        ...
        returns (uint256)
    {
        uint256 feeMultiplier = _isSelling && !canClaimRewards(seller)
            ...
            : 1;
        return
            (treasuryFee.add(ethRewardsFee).add(buybackFee)).mul(feeMultiplier);
    }
    ...
    function eligibleForRewardBooster(address wallet)
        public
        view
        returns (bool)
    {
    ...
    function setRewardsClaimTimeSeconds(uint256 _seconds) external onlyOwner {
        require(
            _seconds >= 0 && _seconds <= 60 * 60 * 24 * 7,
            'claim time delay must be greater or equal to 0 seconds and less than or equal to 7 days'
        );
        rewardsClaimTimeSeconds = _seconds;
    }

    // tax can be raised to maximum 10% - buy and 20% - sell
    function setNewFeesPercentages(
        uint256 _reflectionNewFee,
        uint256 _treasuryNewFee,
        uint256 _ethRewardsNewFee,
        uint256 _buybackRewardsNewFee
    ) external onlyOwner {
        require(
            _reflectionNewFee +
                _treasuryNewFee +
                _ethRewardsNewFee +
                _buybackRewardsNewFee <=
                10,
            'Tax cannot be higher than 10%'
        );
    ...
    function calculateETHRewards(address wallet) public view returns (uint256) {
        ...
        uint256 rewardsWithBooster = eligibleForRewardBooster(wallet)
            ...
            : baseRewards;
        return
            rewardsWithBooster > ethRewardsBalance
                ? baseRewards
                : rewardsWithBooster;
    }
    ...
    function canClaimRewards(address user) public view returns (bool) {
        if (_rewardsLastClaim[user] == 0) {
            return block.timestamp > launchTime.add(rewardsClaimTimeSeconds);
        } else {
            return
                block.timestamp >
