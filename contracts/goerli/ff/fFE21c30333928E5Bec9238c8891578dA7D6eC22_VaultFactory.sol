...
contract BeaconCache is IBeaconDetailed, AccessControlEnumerableUpgradeable {
    ...
    bytes32 private constant _PROXY_INITHASH = 0x41e4c1c8f7d0e969ed8d46bd94dd1a10e39d3c9f89565dc4ed31af2da9a6ae0a;
     ...
     * @dev this + transferRole implementation below enforces: each role has exactly 1 addr at a time
     */
    ...
    /**
     * @dev atomically transfers roles
    ...
    /**
     * @dev Set a new implementation for all the proxies created by this factory. The
     * system can be effectively paused by upgrading to the zero address.
    ...
    /**
     * @dev Returns the address of the cache contract. This contract does not
     * necessarily exist, but can be created any time the implementation is
     * nonzero by the owner calling `updateCache`.
     */
    ...
    /**
     * @dev Deploys the cache contract. This is `onlyOwner` in case a future hardfork
     * creates a DoS opportunity (e.g. proposals that reprice `selfdestruct` to
     * become very expensive). This function caches the bytecode contents of
     * `implementation()`.
    ...
    /**
     * @dev Predict the address that a particular caller and salt will (or has) deploy
     * @param deployer The address calling the deploy function
     * @param salt The salt to be used during the deployment
     */
...
contract VaultFactory is BeaconCache, IVaultFactory {
    ...
    /**
     * @dev Gets addr associated with immunefi to send fees to
...
contract Vault is VaultStorageV1, TokenHandler, OwnableUpgradeable, BeaconImpl {
    ...
    /**
     * @dev Withdraws tokens to the owner account
     * @param payout The payoutay of tokens/token amounts to withdraw
     */
...
contract FooBeacon is BeaconImpl, Foo {
    bool public initialized;
    address public owner;
    event Initialized(address thisContract);

    constructor(IBeaconCache _beacon) BeaconImpl(_beacon) {}

    function initialize(address _owner) public onlyProxy {
        initialized = true;
        owner = _owner;
...
contract BarBeacon is BeaconImpl, Bar {
    constructor(IBeaconCache _beacon) BeaconImpl(_beacon) {}
}
