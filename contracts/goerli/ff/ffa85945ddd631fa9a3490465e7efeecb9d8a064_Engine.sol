...
import "@dex/lib/pnl.sol";
import "@dex/lib/NFTDescriptor.sol";
import "@dex/lib/queue.sol";
...
contract Engine is ERC721, IEngine {
    ...
    struct Match {
        ...
        int256 premium;
        int256 fr;
        bool short; // If maker is short = true
    ...
    // --- Init ---
    constructor(address oracle_, IERC20 collateralToken_)
        ERC721("test", "TST")
    {
    ...
    function deposit(uint256 amount, address recipient) external {
        ...
        // action
        IERC20(collateralToken).transferFrom(recipient, address(this), amount);
        // effects
    ...
    function open(
        ...
        uint256 collateral,
        address recipient
    ) public returns (uint256 matchId, uint256 tokenId) {
        require(amount > 0, "engine/amount-zero");
        require(balances[recipient] >= collateral, "engine/missing-collateral");
    ...
    function pick(
        ...
        require(m.maker != 0, "engin/unexisting_match");
        Order memory o;
        o.owner = msg.sender;
        o.matchid = matchId;
        o.amount = amount;
        ...
        opens.push(o);
        emit NewTraderOrder(msg.sender, matchId, (orderId = opens.length));
    }
    ...
    function close(uint256 matchId) external {
        ...
        require(trader != address(0), "engine/token-nonexistent");
    ...
    function redeem(uint256 matchId, uint256 tokenId) external {
        ...
        // load in memory gl
        int256 gl = pnl(matchId, tokenId, m.end, m.exitPrice);

        balances[ownerOf(tokenId)] += gl > 0 ? uint256(gl) : uint256(-gl);
    ...
    function join(Order memory order) internal {
        ...
        // if (order.owner == maker) return;
        // console.log("owner != maker");
        require(order.owner == maker, "owner=maker");
        // if (balances[order.owner] < order.collateral) return;
        // console.log("enough trader collateral");
        require(
            balances[order.owner] < order.collateral,
            "not enough trader collateral"
        );
        // if (balances[maker] < m.collateralM) return;
        // console.log("enough maker collateral");
        require(balances[maker] < m.collateralM, "not enough maker collateral");
        ...
        if (order.amount < m.amount) _split(m, order.amount);
        // update the balance available to trader/maker
        balances[order.owner] -= m.collateralT;
        balances[maker] -= m.collateralM;
        ...
        returns (uint256 matchId, uint256 tokenId)
    {
        ...
        newMatch.amount = m.amount;
        newMatch.premium = m.premium;
        newMatch.fr = m.fr;
        newMatch.short = m.short;
    ...
    // return pnls for Maker/Trader in one call
    // use WAD for all decimals except for delta time, that is 1
    function pnl(
        uint256 matchId,
        uint256 tokenId,
        uint256 timestamp,
        ...
        int256 aggregatedFR = (int256(timestamp - m.start) *
            m.fr *
            int256(m.amount)) / 10**18;
        int256 gl = m.short
            ? int256(m.entryPrice - exitPrice)
            : int256(exitPrice - m.entryPrice);
        if (tokenId == m.maker) {
            return gl + aggregatedFR + m.premium;
    ...
    function runOpens() external {
        for (uint256 i = 0; i < opens.length; ++i) {
            Order memory o = opens[i];
            join(o);
        }
        // empty the queue
        delete opens;
    ...
    function runCloses() external {
        for (uint256 i = 0; i < closes.length; ++i) {
...
library PnL {
    ...
    function add(Data memory x, Data memory y) internal pure returns(Data memory) {
        x.gl += y.gl;
        x.fr += y.fr;
        x.premium += y.premium;
        return x;
    ...
    function aggregate(Data memory x) internal pure returns(int256) {
        return x.gl + x.fr + x.premium;
    }
}
...
library Queue {
    ...
    function isEmpty(Data storage x) internal view returns(bool) {
        return x.data.isEmpty();
    }

    function head(Data storage x) internal view returns(uint256 id, uint256 tokenId) {
        id = x.data.head;
        tokenId = x.data.nodes[id].data;
    }

    function push(Data storage x, uint256 id) internal returns(uint256) {
        return x.data.push(id);
    }

    function pop(Data storage x) internal returns(uint256 tokenId) {
        tokenId = x.data.nodes[x.data.head].data;
        x.data.removeWithId(x.data.head);
    }

    function rem(Data storage x, uint256 id) internal {
        x.data.removeWithId(id);
    }

    function length(Data storage x) internal view returns(uint256) {
...
library LinkedList {
    ...
    struct Data {
        ...
        mapping(uint256 => uint256) reverseLookup;
    }

    function isValidId(Data storage x, uint256 id) internal view returns(bool) {
        return (id != type(uint256).max) && (x.nodes[id].nextId > 0);
    }

    function isValidIdx(Data storage x, uint256 idx) internal view returns(bool) {
        return (idx < x.length);
    }
    ...
    function fromIdxToNode(Data storage x, uint256 idx) internal view returns(Node storage res, uint256 id) {
        require(isValidIdx(x, idx), "Invalid Index");
        res = x.nodes[x.head];
    ...
    function get(Data storage x, uint256 idx) external view returns(uint256) {
        require(isValidIdx(x, idx), "Invalid Index");
        (Node storage node, ) = fromIdxToNode(x, idx);
        return node.data;
    }

    function getFromNodeId(Data storage x, uint256 id) internal view returns(uint256) {
        require(isValidId(x, id), "Invalid ID");
        return x.nodes[id].data;
    }

    function generateId(Data storage x) internal returns(uint256) {
        x.lastId += 1;
        return x.lastId;
    }

    function push(Data storage x, uint256 data) internal returns(uint256) {
        uint256 id = generateId(x);
        x.nodes[id].data = data;
        if(isEmpty(x)) {
            x.nodes[id].nextId = type(uint256).max;
            x.head = id;
            x.tail = id;
        } else {
            x.nodes[x.tail].nextId = id;
            x.nodes[id].prevId = x.tail;
            x.nodes[id].nextId = type(uint256).max;
        }
        x.tail = id;
        x.reverseLookup[data] = id;
        x.length += 1;
        return x.tail;
    }

    function removeWithId(Data storage x, uint256 id) internal {
        require(!isEmpty(x), "Can not be empty");
        require(isValidId(x, id), "Invalid ID");
        Node storage node = x.nodes[id];
        x.reverseLookup[node.data] = 0;

        if(id == x.head) {
            x.head = node.nextId;
            x.nodes[x.head].prevId = 0;
        } else {
            x.nodes[node.prevId].nextId = node.nextId;
            x.nodes[node.nextId].prevId = node.prevId;
            if(id == x.tail) x.tail = node.prevId;
        }
    ...
    function lookup(Data storage x, uint256 tokenId) internal view returns(uint256 nodeId) {
        nodeId = x.reverseLookup[tokenId];
    }

    function removeWithData(Data storage x, uint256 tokenId) internal {
        uint256 nodeId = lookup(x, tokenId);
        require(nodeId > 0, "TokenId not present in the OrderBook");
        removeWithId(x, nodeId);
    }
}
