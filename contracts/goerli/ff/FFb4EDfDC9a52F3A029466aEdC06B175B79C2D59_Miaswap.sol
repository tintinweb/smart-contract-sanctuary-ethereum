...
error Miaswap__PoolExists(address poolAddress);
...
contract Miaswap is Ownable, ReentrancyGuard {
    ...
    function createPool(
        ...
        if (poolAddress != address(0)) {
            revert Miaswap__PoolExists(poolAddress);
        }
        ...
        MiaLiquidityPool(poolAddress).mint(sender, tokenScore);
        (bool sent, ) = poolAddress.call{value: value}("");
        if (!sent) {
            revert Miaswap__FailedToSendEth();
        }
        IERC721(collection).transferFrom(sender, poolAddress, tokenId);
        emit PoolCreated(
            collection,
            poolAddress,
            sender,
        ...
        nonReentrant
    {
        ...
        s_poolInfos[collection].totalEth = poolInfo.totalEth;
        s_poolInfos[collection].totalScore = poolInfo.totalScore;
        (bool sent, ) = poolInfo.poolAddress.call{
    ...
    function swapNftsToEth(address collection, uint256[] memory tokenIds) external nonReentrant {
        ...
        miaLiquidityPool.transferEth(address(this), serviceFee);
    ...
    function withdraw(address collection, uint256[] memory tokenIds) external nonReentrant {
        ...
        miaLiquidityPool.burnFrom(sender, uint256(scoresToWithdraw));
        ...
        returns (uint256, bool)
    {
        ...
        return (
            equivalentEthAmount - serviceFee - creatorFee - liquidityProviderFee,
...
contract MiaLiquidityPool is ERC20Burnable {
     ...
     * Main functions *
     ******************/
    constructor(address collection) ERC20("MiaLiquidityPool", "MiaLP") {
