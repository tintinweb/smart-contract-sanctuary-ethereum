...
contract TolPass is DefaultOperatorFilterer, EIP2981RoyaltyOverrideCore, ITolPass, ERC721AntiScam, AccessControl, Pausable {
    // Manage
    ...
    modifier withinMaxAmountPerAddress(address minter, uint256 amount, uint256 allowedAmount) {
    ...
    // Constructor
    constructor(address _withdrawAddress) ERC721A("TolPass", "TOL") {
    ...
    function pieMint(address minter, uint256 _amount, uint256 _allowedAmount, bytes32[] calldata _merkleProof) external payable onlyRole(PIE)
        whenNotPaused
        enoughEth(_amount)
        withinMaxSupply(_amount)
        withinMaxAmountPerAddress(minter, _amount, _allowedAmount)
        validProof(minter, _allowedAmount, _merkleProof)
    {
        mintedAmountBySales[salesId][minter] += _amount;
