...
contract NftVault is Ownable, ReentrancyGuard {
    ...
    function airDrop(address[] memory users, uint256[] memory tokens) external onlyOwner {
        require(users.length == tokens.length, "arrays not equals");
        for (uint i=0; i < users.length; i++ ) {
            require(nft_v2.ownerOf(tokens[i]) == owner(), "missing token on owner wallet");
            nft_v2.safeTransferFrom(_msgSender(), users[i], tokens[i]);
        }
    }
