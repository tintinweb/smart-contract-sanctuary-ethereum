...
contract TestToken is ERC20, Ownable {
    ...
    uint256 public maxSupply = 100 * 1e18;
    ...
    constructor()  ERC20("Test", "TEST") {
        //test
        // uint256[] memory task = new uint256[](3);
        // task[0] = block.timestamp;
        // task[1] = block.timestamp + 100;
        // task[2] = block.timestamp + 200;
        // _createWorkflow(task, 100);
        // uint256[] memory task2 = new uint256[](3);
        // task2[0] = block.timestamp + 50;
        // task2[1] = block.timestamp + 150;
        // task2[2] = block.timestamp + 250;
        // _createWorkflow(task2, 150);
        // uint256[] memory task3 = new uint256[](3);
        // task3[0] = block.timestamp + 60;
        // task3[1] = block.timestamp + 160;
        // task3[2] = block.timestamp + 260;
        // _createWorkflow(task3, 200);
    }
        ...
        for (uint256 i = 0; i < walletWorkflows[wallet].length; i++) {
            ...
            for (
                ...
                if (
                    ...
                    uint256 finished = workflows[workeflowId].index + 1;
                    unlocked = unlocked.add(
                        lockedAmount[wallet]
                            .mul(
                                finished.mul(workflows[workeflowId].unlockPoint)
                            )
                            .div(1000)
                    );
                } else {
    ...
    function _setWorkflowRole(
        ...
        for (uint256 i = 0; i < workeflowIds.length; i++) {
            require(
                workflows[workeflowIds[i]].releaseTimes.length > 0,
                "workflow not exist"
            );

            walletWorkflows[wallet].push(workeflowIds[i]);
        }
    }
    ...
    function checkTaskAndAdvanceTask(address wallet) internal {
        if (walletWorkflows[wallet].length <= 0) {
            return;
        }
