...
contract ChildStorage is Ownable {
    ...
    mapping(address => uint256) public ownedNFTId;
    ...
    // address public delegateRegistry = 0x469788fE6E9E9681C6ebF3bF78e7Fd26Fc015446;
    address public delegateRegistry = 0x48e014AdAe80100820B1483841C8A4A2028B183E;
    bytes32 public spaceId = bytes32("kudasaijp.eth");

    modifier onlyOperater() {
        require(operater[msg.sender], "ChildStorage: No Call");
        _;
    }

    /**
     * @dev Sets the given contract as the operator of this contract
     * Only the contract owner can perform this action
     * @param _contract The address of the contract to be set as operator
     */
    function setOperater(address _contract) external onlyOwner {
        operater[_contract] = true;
    }

    /**
     * @dev Sets the given contract as the kudasai contract
     * Only the contract owner can perform this action
     * @param _contract The address of the contract to be set as kudasai
     */
    function setKudasai(address _contract) external onlyOwner {
        kudasai = _contract;
    }

    /**
     * @dev Sets the given contract as the delegate registry contract
     * Only the contract owner can perform this action
     * @param _contract The address of the contract to be set as delegate registry
     */
    function setDelegateRegistry(address _contract) external onlyOwner {
        delegateRegistry = _contract;
    }

    /**
     * @dev function to add a new child address for a user
     * @param _user address of the user
     * @param _newChild new child address to be added
     */
    function addChildAddress(address _user, address _newChild) external onlyOperater {
        child[_user][childCount[_user]] = _newChild;
        childCount[_user]++;
    }

    /**
     * @dev Sets the NFT ID for a specific user
     * @param _user address The user to set the NFT ID for
     * @param _nftId uint256 ID of the NFT to be set
     */
    function setNFTId(address _user, uint256 _nftId) external onlyOperater {
        ownedNFTId[_user] = _nftId;
    }
}

contract MultiWalletCallerV2 is Ownable, IERC721Receiver {
    bytes32 private _spaceId = bytes32("kudasaijp.eth");

    MinterChaild private immutable _childContract;
    CloneFactory private immutable _cloneContract;
    ChildStorage private immutable _childStorage;

    constructor () {
        _childStorage = new ChildStorage();
        ChildStorage(_childStorage).setOperater(address(this));
        ChildStorage(_childStorage).transferOwnership(msg.sender);
    ...
    modifier onlyHolder() {
        address nftOwner = IERC721(ChildStorage(_childStorage).kudasai()).ownerOf(ChildStorage(_childStorage).ownedNFTId(msg.sender));
        address delegation = _checkDelegation(nftOwner);
        if (delegation != address(0)) {
            require(delegation == msg.sender, "MultiWalletCaller: No Call");
        } else {
            require(nftOwner == msg.sender, "MultiWalletCaller: No Call");
        }
        _;
    }
    ...
    /**
     * @dev Returns the delegate of an NFT owner, if one exists. If a specific space delegation is set, returns it. If not, returns the default delegation.
     * @param _nftOwner The address of the owner of the NFT
     * @return The address of the delegate
     */
    function _checkDelegation(address _nftOwner) private view returns(address) {
        DelegateRegistry registry = DelegateRegistry(ChildStorage(_childStorage).delegateRegistry());
        address delegation = registry.delegation(_nftOwner, _spaceId);
        delegation = delegation == address(0) ? registry.delegation(_nftOwner, bytes32(0)) : delegation;
        return delegation;
    }
    ...
    function setNFTId(uint256 _nftId) public {
        ChildStorage(_childStorage).setNFTId(msg.sender, _nftId);
    }
...
contract MinterChaild is IERC721Receiver {
    ...
    /**
     * @dev Withdraws ERC20 tokens from the contract to an address
     * @param _contract The address of the ERC20 contract
    ...
    /**
     * @dev Withdraws ERC721 tokens from the contract to an address
    ...
    /**
     * @dev Withdraws ERC1155 tokens from the contract to an address
    ...
    /**
     * @dev Withdraws ETH from the contract to an address
    ...
    /**
     * @dev Calls a function on a contract with value
     * @param _callContract The address of the contract to call
     * @param _callData The data for the function call
     * @param _value The value to send with the function call
     */
