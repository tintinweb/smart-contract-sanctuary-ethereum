...
/******************************************************************************\
/// @title Contract Commit Facet - exclusive features for committer
/// @author Ather Labs
/******************************************************************************/
...
contract CommitFacet is IOperations {
    /* ------ TYPE & VARIABLES ------ */
    /// @notice declare AppStorage
    AppStorage internal s;

    /// @dev
    uint256 internal constant COMMIT_TIME_INTERVAL = 10 minutes;
    ...
    function commitBatch(
        bytes32 _stateRoot,
        bytes32 _pendingOperationHash,
        uint32 _numOnchainOperations
    ) external {
        LibAccessControl._requireCommitter();
        LibStatusControl._requireOperationsEnable();
        require(
            _numOnchainOperations <= s.numberOfPendingRequest,
            "CF: Number of request must be less than or equal to number of pending request"
        );
        // Duration between two commits must be greater than `COMMIT_TIME_INTERVAL`
        // require(block.timestamp > s.latestCommittedBatch.timestamp + COMMIT_TIME_INTERVAL, "CF: Too soon to commit");
        CommittedBatchInfo memory newCommit = CommittedBatchInfo({
            stateRoot: _stateRoot,
            pendingOperationsHash: _pendingOperationHash,
            batchId: s.currentBatchId + 1,
            numOnchainOperations: _numOnchainOperations,
            timestamp: LibSafeCast.toUint32(block.timestamp)
        });

        s.latestCommittedBatch = newCommit;

        emit NewCommit(
            newCommit.stateRoot,
            newCommit.pendingOperationsHash,
            newCommit.batchId,
            newCommit.numOnchainOperations,
            newCommit.timestamp
        );
    }
...
library LibAccessControl {
    ...
    function _requireGov() internal view {
        require(_hasRole(GOV_ROLE, _msgSender()), "Gov:not gov");
    }
    ...
    function _requireCommitter() internal view {
        require(_hasRole(COMMITTER_ROLE, _msgSender()), "Gov:not committer");
    }
    ...
    function _requireExecutor() internal view {
        require(_hasRole(EXECUTOR_ROLE, _msgSender()), "Gov:not executor");
    }
