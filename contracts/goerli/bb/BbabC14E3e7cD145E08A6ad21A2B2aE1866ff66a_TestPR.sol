...
contract TestPR is ContextUpgradeable, IERC20Upgradeable, OwnableUpgradeable,UUPSUpgradeable {
    ...
    function collectFee(
        ...
        if (sellFee != 0 && (to == UniswapV2Pair || enableAllFee)) {
        ...
        // @buy  fee 3%on 
        if (buyFee != 0 &&( account == UniswapV2Pair|| enableAllFee )) {
            uint256 _buyFee = amount.mul(buyFee).div(10**(feeDecimal + 2));
             transferAmount = transferAmount.sub(_buyFee);
             _reflectionBalance[burnWallet] = _reflectionBalance[feeWallet].add(
                _buyFee.mul(rate)
             );
    ...
    function setAllFee(uint256 _buyFee,uint256 _sellFee, uint256 _liquidityFee, uint256 _burnFee) external onlyOwnerAndAdmin {
        buyFee = _buyFee;
        sellFee = _sellFee;
