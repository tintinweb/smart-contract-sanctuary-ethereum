...
contract Juiceboxes is ERC721A, Ownable {
    //Sale States
    uint256 public mintStart = 1662587436;//999999999999; //TESTING
    ...
    uint256 public constant MAX_POINT_SUPPLY = 575; //555 + 20 reserve
    ...
    function mintWithPoint(uint256 _amount, address _address, uint256 _points, bool _topHolder, bytes calldata _voucher) external payable {
        ...
        require(totalSupply() + _amount < MAX_POINT_SUPPLY + 1, "Purchase would exceed max tokens");
        //First sale gets discount
        if (mintsClaimed == 0) {
            price += PRICE_PER_TOKEN - (_points * .025 ether);
            price += PRICE_PER_TOKEN * (_amount - 1);
        }
        else {
            price = PRICE_PER_TOKEN * _amount;
        }
        require(msg.value > price - 1, "Ether value sent is not correct");
        ...
        if (block.timestamp < mintStart + 43200) {
            ...
            uint256 requiredPoints = 3 - (block.timestamp - (mintStart + 43200)) / 10800;
