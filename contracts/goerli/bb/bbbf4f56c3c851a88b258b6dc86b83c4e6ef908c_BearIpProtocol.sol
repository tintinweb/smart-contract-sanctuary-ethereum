...
import "@openzeppelin-upgradeable/contracts-upgradeable/proxy/utils/Initializable.sol";
import "@openzeppelin-upgradeable/contracts-upgradeable/security/ReentrancyGuardUpgradeable.sol";

import "@openzeppelin-upgradeable/contracts-upgradeable/security/PausableUpgradeable.sol";
import "@openzeppelin-upgradeable/contracts-upgradeable/access/OwnableUpgradeable.sol";
...
import "./impl/BearIpBox.sol";
import "./interface/IBearIpProtocol.sol";
...
contract BearIpProtocol is
    ...
    IBearIpProtocol
{
    using BearIpBox for mapping(bytes32 => BearIpBox.BoxInfo);
   ...
   /// @custom:oz-upgrades-unsafe-allow constructor
    ...
    function initialize(address _settings) public initializer {
        ...
        settings = IBearIpSetting(_settings);
    }
    ...
    function createBox(
        ...
        // uint256[] calldata _ERC1155_cardIds,
        uint256[] calldata _cardamounts
    ) public payable  nonReentrant returns (bytes32 box_id) {
        emit NewBox(box_id, _start_block, _mintable_block_index, _target_amount, _relist_block);
    ...
    event NewBid(bytes32 bid_id,bytes32 box_id, uint96 _amount);
    function bid(bytes32 box_id, uint96 amount)
        public
        payable
        nonReentrant
        returns (bytes32 bid_id)
    {
        emit NewBid(bid_id,box_id, amount);
        bid_id = keccak256(abi.encodePacked(block.number, msg.sender, msg.data, tx.origin));
        return bid_id;
    }

    event ClaimEth(bytes32 box_id, uint256 _amount);
   ...
   event ClaimRewardNft(address received_address,
    address nft_address, 
    uint256 id, 
    uint256 amount);    
    function ClaimReward(bytes32 box_id)
        public
        payable
        nonReentrant
...
abstract contract IBearIpProtocol {
    using BearIpBox for mapping(bytes32 => BearIpBox.BoxInfo);
  
    mapping(bytes32 => BearIpBox.BoxInfo) public allBoxs;

    IBearIpSetting public settings;
