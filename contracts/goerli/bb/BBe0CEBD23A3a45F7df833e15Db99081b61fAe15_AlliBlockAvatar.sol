...
/**
 * @title The avatars of allimeta BLOCK
 *
 * @dev Avatar will be admin rights to manage metaverse BLOCK
 * @dev TokenId starting from 1000
 *
 */
...
contract AlliBlockAvatar is ERC721Metadata {
   
    using Strings for uint256;

    uint256 start_id = 1000;
    ...
    constructor() ERC721("AlliBlockKey", "ABK") {
            MAX_SUPPLY = 1000;
    }
    ...
    function mint(address to, uint256 tokenId ) external returns (bool) 
    {
        ...
        require( tokenId >= start_id, "mint id too small");        
    ...
    {
        require(_minters[msg.sender], "minter invalid");
        uint256 tokenId = start_id;
        for(; tokenId < start_id + MAX_SUPPLY;tokenId++)
        {
            if( !_exists(tokenId) )
            {
              break;
