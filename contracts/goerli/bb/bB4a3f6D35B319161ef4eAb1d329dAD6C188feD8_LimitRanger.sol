...
contract LimitRanger {
    ...
    modifier onlyOwner(uint256 tokenId) {
        require(
            positionInfos[tokenId].owner == msg.sender,
            "Operation only allowed for owner of position"
        );
        _;
    }
    ...
    function getOwnedPositions(address owner)
        external
        view
    ...
    function retrieve(uint256 tokenId) public onlyOwner(tokenId) {
    ...
    function _removePosition(uint256 tokenId, address receiver) internal {
        ...
        // index of token to be removed
        uint256 tokenIndex = ownedTokensIndex[tokenId];
        // index of last token user owned
        uint256 lastTokenIndex = (ownedTokens[receiver].length - 1);
        // last token get by index
        uint256 lastToken = ownedTokens[receiver][lastTokenIndex];
        ownedTokens[receiver][tokenIndex] = lastToken;
        ownedTokens[receiver].pop();

        // uptill now we have removed the last element from the ownedTokens mapping and replace the removed token with last token and now
        // the token index of removed token represents last item in ownedTokens mapping

        // now we'll sort the the ownedTokenIndex mapping

        ownedTokensIndex[tokenId] = 0; // this should now make the tokenId's index to 0
        // this now makes the last item now on index 2 and both ownedToken
        // and ownedTokenIndex  are now sorted and equal
        ownedTokensIndex[lastToken] = tokenIndex;

        // Now transferring the TokenId to msg.sender
