    ...
    DeadlineValidation
{
    ...
    constructor(address _factory, address _WETH)
        RouterTokenHelperWithFee(_factory, _WETH)
    {}
    ...
    function swapCallback(
        ...
        (address tokenIn, address tokenOut, uint24 fee) = swapData
            .path
            .decodeFirstPool();
        require(
    ...
    function _swapExactInputInternal(
        ...
        (int256 amount0, int256 amount1) = _getPool(tokenIn, tokenOut, fee)
            .swap(
                recipient,
                amountIn.toInt256(),
                isFromToken0,
...
abstract contract RouterTokenHelper is
    IRouterTokenHelper,
    ImmutablePeripheryStorage
{
    constructor(address _factory, address _WETH)
        ImmutablePeripheryStorage(_factory, _WETH)
    {}
    ...
    function unwrapWeth(uint256 minAmount, address recipient)
        external
        payable
        override
    {
    ...
    /// @dev Send all ETH balance of this contract to the recipient
    function refundEth(address recipient) external payable override {
        if (address(this).balance > 0)
            TokenHelper.transferEth(recipient, address(this).balance);
    }
...
abstract contract RouterTokenHelperWithFee is
    RouterTokenHelper,
    IRouterTokenHelperWithFee
{
    uint256 constant FEE_UNITS = 100000;

    constructor(address _factory, address _WETH)
        RouterTokenHelper(_factory, _WETH)
    {}
...
library PathHelper {
    ...
    uint256 private constant POOL_DATA_OFFSET =
        TOKEN_AND_POOL_OFFSET + ADDR_SIZE;
    /// @dev The minimum length of an encoding that contains 2 or more pools
    uint256 private constant MULTIPLE_POOLS_MIN_LENGTH =
        POOL_DATA_OFFSET + TOKEN_AND_POOL_OFFSET;
    ...
    function skipToken(bytes memory path) internal pure returns (bytes memory) {
        return
            path.slice(
                TOKEN_AND_POOL_OFFSET,
                path.length - TOKEN_AND_POOL_OFFSET
            );
    }
}
