...
// ERROR
error E_Raffle__EntranceFeeLess();
...
contract Raffle is VRFConsumerBaseV2, KeeperCompatibleInterface {
  ...
  enum RaffleState {
    OPEN,
    GETTING_WINNER
  } // same as uint256 where 0 => open ,1 means close;
  ...
  function enterRaffle() public payable {
    if (msg.value < i_entranceFee) revert E_Raffle__EntranceFeeLess();
    players.push(payable(msg.sender));

    if (s_raffleState != RaffleState.OPEN) revert Raffle__LotteryIsClosed();
  ...
  function performUpkeep(bytes calldata /*performData */) external override {
    ...
    s_raffleState = RaffleState.GETTING_WINNER;
  ...
  function getPlayerByID(uint256 index) public view returns (address) {
