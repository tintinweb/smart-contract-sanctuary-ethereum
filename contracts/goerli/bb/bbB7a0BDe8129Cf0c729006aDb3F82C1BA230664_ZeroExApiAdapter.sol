...
contract ZeroExApiAdapter {
        ...
        )
    {
...

        {
            ...
            if (selector == 0x415565b0 || selector == 0x8182b61f) {
                ...
                // sellToUniswap()
                address[] memory path;
                (path, inputTokenAmount, minOutputTokenAmount) = abi.decode(
                    _data[4:],
                ...
                (order, , takerTokenFillAmount) = abi.decode(
                    _data[4:],
                    (RfqOrder, Signature, uint128)
                );
                inputTokenAmount = uint256(takerTokenFillAmount);
                inputToken = order.takerToken;
                outputToken = order.makerToken;
                minOutputTokenAmount = getRfqOrderMakerFillAmount(
                    order,
                    inputTokenAmount
                );
                ...
                (orders, , takerTokenFillAmounts, revertIfIncomplete) = abi
                    .decode(_data[4:], (RfqOrder[], uint256, uint128[], bool));
                require(orders.length > 0, "Empty RFQ orders");
                require(
                    revertIfIncomplete,
                    "batchFillRfqOrder must be all or nothing"
                );
                ...
                (tokens, , inputTokenAmount, minOutputTokenAmount) = abi.decode(
                    _data[4:],
        ...
        require(
            !supportsRecipient || recipient == _destinationAddress,
        ...
        require(
            minOutputTokenAmount >= _minDestinationQuantity,
    ...
    function getRfqOrderMakerFillAmount(
        RfqOrder memory order,
        uint256 takerTokenFillAmount
    ) private pure returns (uint256 makerTokenFillAmount) {
        if (
            order.takerAmount == 0 ||
            order.makerAmount == 0 ||
        ...
        return
            uint256(
                (order.makerAmount * takerTokenFillAmount) / order.takerAmount
            );
    }
