...
contract Looook is Ownable, ERC721, ReentrancyGuard {
    ...
    uint256 private _puzzleAnchorId = 1;
    ...
    constructor() ERC721('lookaga.in', 'LOOOOK') {
        description = 'Look Again is a daily visual puzzle. A picture is worth a thousand words, but you only need two. https://lookaga.in';
        _baseTokenURI = 'https://meta.lookaga.in/nfts/';
        _baseContractURI = 'https://meta.lookaga.in/contract';
    }
    ...
    function initialize(
        ...
        require(
            ...
            'Start time must be a time in the future'
        );
        require(startTime == 0, 'Can only be initialized once');
    ...
    */
    ...
    function tokenCurrent() public view returns (uint256) {
        return currentTokenId.current();
    }

    // Current number of past + queued puzzles
    function queuedCurrent() public view returns (uint256) {
        return currentQueuedPuzzleId.current();
    }
    ...
    function setStartAndInterval(
        ...
        // Set active puzzle ID
        uint256 diff = SafeMath.div(
            SafeMath.sub(block.timestamp, startTime),
            puzzleInterval
        );
        _puzzleAnchorId = _puzzleAnchorId + diff + 1;
