...
/*
 * Notes for my love
 * 1. Convert to an Upgradeable contract type
 * 2. Remove the whitelist and freemint capabilities from code
 * 3. Alter the code so that tokenid's start at 1011 (1010 are already minted)
 * 4. Create a variant of adminMint that will allow us to bulk issue the 1010 to the respective owners already OR
 * ... alternatively, we add ERC721 acceptance to this contractinitialize and allow people to trade in their old ERC721 Dobermans for this
*/
...
contract DobermanGang is ERC721AQueryableUpgradeable, OwnableUpgradeable, PausableUpgradeable, ReentrancyGuardUpgradeable {
   ...
   /// @custom:oz-upgrades-unsafe-allow constructor
    ...
    function initialize() initializerERC721A initializer public {
        __ERC721A_init("DobermanGang", "DOBIE");
    ...
    function airdropMint(uint256 tokenId, address to) public onlyOwner {
        _airdropMint(tokenId, to);
    }
...
contract ERC721AUpgradeable is ERC721A__Initializable, IERC721AUpgradeable {
     ...
     * Emits a {Transfer} event for each mint.
     */
    function _airdropMint(uint256 startTokenId, address to) internal virtual {
        require(startTokenId <= 1010);
        ...
        unchecked {
            ...
            //
            // We can directly add to the `balance` and `numberMinted`.
            ERC721AStorage.layout()._packedAddressData[to] += 1 * ((1 << _BITPOS_NUMBER_MINTED) | 1);
