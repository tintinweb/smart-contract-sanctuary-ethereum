...
interface IBookOfLore {
    ...
    function scribeAllowlist(address _scribeAddress) external returns (bool);
}
...
contract EnchantedMirror {
    ...
    event ShardFound(string code, uint256 finderId, address finderAddress);

    struct Epoch {
        mapping(string => bool) codesUsed;
        mapping(address => uint8) finderScore;
        address[] finders;
        address[] pastOwners;
    ...
    uint256 public claimPrice = 0.017 ether;
    ...
    uint8 public shardsFound;

    bool public mirrorBroken;
    ...
    function breakMirror(uint256 breakerId, address breakerAddress) external {
        require(!mirrorBroken, "mirror is already broken");
        require(
            victimId != MAX_INT,
        ...
        _writeLore(
            ...
            string.concat(
                "@",
                breakerAssetType,
                Strings.toString(breakerId),
                " has broken the [Enchanted Mirror](https://cabinetofcuriosities.xyz/mirror), where ",
                "@",
                _getAssetType(victimAddress),
                Strings.toString(victimId),
                " is trapped, its shards are now scattered across the Runiverse."
            )
        );
    ...
    function getScoreboard(uint256 epoch)
        external
        view
        returns (
            address[] memory finders,
            uint256[] memory shardBalance,
            uint8[] memory finderScore
        )
    {
        finders = epochs[epoch].finders;
        uint256[] memory ids = new uint256[](finders.length);
        finderScore = new uint8[](finders.length);
        for (uint256 i = 0; i < finders.length; ++i) {
            ids[i] = shardId;
            finderScore[i] = epochs[epoch].finderScore[finders[i]];
        }

        shardBalance = IERC1155(cabinetAddress).balanceOfBatch(
            epochs[counter].pastOwners,
            ids
        );
    }

    function getAllShardsFoundtimestamp(uint256 epoch)
        external
        view
        returns (uint256)
    {
        return epochs[epoch].allShardsFoundTimestamp;
    }
    ...
    function repairMirror(uint256 repairerId, address repairerAddress) public {
        require(mirrorBroken, "The mirror is not broken");
        ...
        require(
            block.timestamp > epochs[counter].allShardsFoundTimestamp + 1 weeks,
            "a week has to pass after finding all shards"
        );
        ...
        _writeLore(
            ...
            string.concat(
                "@",
                _getAssetType(repairerAddress),
                Strings.toString(repairerId),
                " has helped returning the shards of the [Enchanted Mirror](https://cabinetofcuriosities.xyz/mirror), and performed a repair incantation. ",
                "@",
                _getAssetType(victimAddress),
                Strings.toString(victimId),
                " is free again!"
        ...
        _writeLore(
            ...
            string.concat(
                ...
                _getAssetType(victimAddress),
                Strings.toString(victimId),
                " was saved from the [Enchanted Mirror](https://cabinetofcuriosities.xyz/mirror) by ",
                "@",
                _getAssetType(repairerAddress),
                Strings.toString(repairerId),
                "!"
    ...
    function _writeLore(
        ...
        if (
            bookOfLoreAddress != address(0) &&
            IBookOfLore(bookOfLoreAddress).scribeAllowlist(address(this))
        ) {
    ...
    function claimShard(
        ...
        if (IERC1155(cabinetAddress).balanceOf(msg.sender, shardId) == 1) {
            _writeLore(
                finderId,
                finderAddress,
                string.concat(
                    "@",
                    finderAssetType,
                    Strings.toString(finderId),
                    " has found their first shard of the [Enchanted Mirror](https://cabinetofcuriosities.xyz/mirror). There are ",
                    Strings.toString(TOTAL_SHARDS - shardsFound),
                    " pieces left to save ",
                    "@",
                    _getAssetType(victimAddress),
                    Strings.toString(victimId),
        ...
        emit ShardFound(code, finderId, finderAddress);
    }
    ...
    function trap(
        ...
        string memory lore = string.concat(
            "@",
            _getAssetType(_culpritAddress),
            Strings.toString(_culpritId),
            " has trapped ",
            "@",
            _getAssetType(_victimAddress),
            Strings.toString(_victimId),
            " in the [Enchanted Mirror](https://cabinetofcuriosities.xyz/mirror)."
        );
        _writeLore(_victimId, _victimAddress, lore);
        _writeLore(_culpritId, _culpritAddress, lore);
    ...
    function setClaimPrice(uint256 _claimPrice) external onlyManager {
        require(!mirrorBroken, "Cannot set price when mirror is broken");
