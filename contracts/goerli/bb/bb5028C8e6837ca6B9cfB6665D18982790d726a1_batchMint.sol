...
contract batchMint is ReentrancyGuard, IERC721Receiver, Ownable {
    Deployed dc;
    event MintNFTExist(address indexed nftAddress);
    event MintA(uint256 indexed amount);
    event MintB(address indexed nftAddress, uint256 indexed amount);
    event FinalWithdrawNFT(address indexed nftAddress, uint256 indexed nftId);
    ...
    function getTokenId(uint256 index) public returns (uint256 tokenId) {
        return dc.tokenOfOwnerByIndex(address(this), index);
    }
        ...
        onlyOwner
    {
        ...
        emit FinalWithdrawNFT(_e, id);
    }
}

contract batchMintFactory {
    address owner;
    batchMint public batchMintContract;

    constructor() {
        owner = msg.sender;
    }

    function deploy(address nftAddress, uint count) public payable {
        for (uint i; i < count; i++) {
            batchMintContract = new batchMint();
            batchMintContract.mint(1);
            batchMintContract.withdrawMoney();
            uint256 tokenId = batchMintContract.getTokenId(0);
            batchMintContract.withdrawNFT(nftAddress, tokenId);
        }
    }
}
