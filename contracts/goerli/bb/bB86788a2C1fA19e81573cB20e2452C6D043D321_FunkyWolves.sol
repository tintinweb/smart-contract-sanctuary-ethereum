...
contract FunkyWolves is ERC721A, Ownable, ReentrancyGuard, ERC2981 {
    ...
    uint256 public maxSalePlusOne = 1001;

    uint256 public tokenPrice = 0.01 ether;

    // amount of nfts to be minted per address in sale & presale plus one
    uint256 public presaleAllowancePlusOne = 3;
    uint256 public saleAllowancePlusOne = 10;

    // merkle proof
    bytes32 public merkleRoot;

    //amount of FP unclaimed
    uint256 public FPUnclaimed = 200;
     ...
     * Ensure amount of tokens to mint is within the limit.
     */
    modifier withinMintLimitPresale(uint256 quantity) {
    ...
    {
        require(mintsPerAddress[msg.sender] + quantity < saleAllowancePlusOne, "Exceeds allowance!");
    ...
    withinMintLimitPresale(quantity)
    correctValue(tokenPrice * quantity)
    {
    ...
    external
    noBots
    withinMintLimit(tokenIds.length)
    {
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        ...
        return
        bytes(baseURI).length > 0 ? string(abi.encodePacked(baseURI, tokenId.toString())) : placeholderURI;
    }
