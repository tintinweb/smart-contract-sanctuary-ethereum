...
contract BSKR_VX is BaseBSKR {
    ...
    function _transferTokens(
        ...
        if (response[uint8(Field.tRfiFee)] != 0) {
            _rTotal -= response[uint8(Field.rRfiFee)];
            totalReflection += response[uint8(Field.tRfiFee)];
    ...
    function balanceOf(address account) public view override returns (uint256) {
        ...
        require(_rBalances[account] <= _rTotal, "BSKR: Amount too large");
        uint256 currentRate = _getRate();
    ...
    function giftReflection(uint256 tAmount) external whenNotPaused {
        require(tAmount != 0, "BSKR: Zero gift amount");
    ...
    Manageable
{
     ...
     */
    function airdrop(Airdrop[] calldata receivers) external virtual;
