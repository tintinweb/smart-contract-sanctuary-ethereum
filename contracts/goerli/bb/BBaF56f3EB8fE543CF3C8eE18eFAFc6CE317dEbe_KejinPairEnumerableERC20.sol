...
import {KejinPairERC20} from "./KejinPairERC20.sol";
import {KejinPairEnumerable} from "./KejinPairEnumerable.sol";
import {IKejinPairFactoryLike} from "./KejinPairFactoryLike.sol";

/**
    @title An NFT/Token pair where the NFT implements ERC721Enumerable, and the token is an ERC20
    Inspired by 0xmons; Modified from https://github.com/sudoswap/lssvm
 */
contract KejinPairEnumerableERC20 is KejinPairEnumerable, KejinPairERC20 {
    /**
        @notice Returns the KejinPair type
     */
    function pairVariant()
        public
        pure
        override
        returns (IKejinPairFactoryLike.PairVariant)
    {
        return IKejinPairFactoryLike.PairVariant.ENUMERABLE_ERC20;
    }
}
...
import {KejinPair} from "./KejinPair.sol";
import {IKejinPairFactoryLike} from "./KejinPairFactoryLike.sol";
import {KejinRouter} from "./KejinRouter.sol";
...
/**
    ...
    Inspired by 0xmons; Modified from https://github.com/sudoswap/lssvm 
 */
abstract contract KejinPairERC20 is KejinPair {
    ...
    /**
        ...
        @dev See KejinPairCloner for an explanation on how this works
     */
    ...
    /// @inheritdoc KejinPair
    function _pullTokenInputAndPayProtocolFee(
        uint256 inputAmount,
        bool isRouter,
        address routerCaller,
        IKejinPairFactoryLike _factory,
        uint256 protocolFee
    ) internal override {
        ...
        if (isRouter) {
            ...
            KejinRouter router = KejinRouter(payable(msg.sender));
    ...
    /// @inheritdoc KejinPair
    // @dev see KejinPairCloner for params length calculation
...
/**
    ...
    Inspired by 0xmons; Modified from https://github.com/sudoswap/lssvm
 */
abstract contract KejinPairEnumerable is KejinPair {
    /// @inheritdoc KejinPair
...
interface IKejinPairFactoryLike {
    enum PairVariant {
        ENUMERABLE_ETH,
    ...
    function routerStatus(KejinRouter router)
        external
        view
...
abstract contract KejinPair is
    ...
    ERC1155Holder
{
    ...
    /**
        ...
        amount is greater than this value, the transaction will be reverted.
        @param nftRecipient The recipient of the NFTs
        @param isRouter True if calling from KejinRouter, false otherwise. Not used for
        ETH pairs.
        @param routerCaller If isRouter is true, ERC20 tokens will be transferred from this address. Not used for
        ETH pairs.
        @return inputAmount The amount of token used for purchase
    ...
    function factory() public pure returns (IKejinPairFactoryLike _factory) {
        uint256 paramsLength = _immutableParamsLength();
        assembly {
            _factory := shr(
                0x60,
    ...
    /**
        ...
        @param isRouter Whether or not the caller is KejinRouter
        @param routerCaller If called from KejinRouter, store the original caller
        @param _factory The KejinPairFactory which stores KejinRouter allowlist info
    ...
    /**
        ...
        @dev We send ETH back to the caller even when called from KejinRouter because we do an aggregate slippage check for certain bulk swaps. (Instead of sending directly back to the router caller) 
        Excess ETH sent for one swap can then be used to help pay for the next swap.
     */
    function _refundTokenToSender(uint256 inputAmount) internal virtual;

    /**
        @notice Sends protocol fee (if it exists) back to the KejinPairFactory from the pair
     */
    function _payProtocolFeeFromPair(
        IKejinPairFactoryLike _factory,
        uint256 protocolFee
    ...
    /**
        ...
        @dev This is used by the KejinPair's swapNFTForToken function. 
    ...
    /**
        @dev Used internally to grab pair parameters from calldata, see KejinPairCloner for technical details
     */
...
contract KejinRouter {
    ...
    IKejinPairFactoryLike public immutable factory;

    constructor(IKejinPairFactoryLike _factory) {
        factory = _factory;
    }
    ...
    function pairTransferERC20From(
        ...
        address to,
        uint256 amount,
        IKejinPairFactoryLike.PairVariant variant
    ) external {
        ...
        require(
            variant == IKejinPairFactoryLike.PairVariant.ENUMERABLE_ERC20 ||
                variant ==
                IKejinPairFactoryLike.PairVariant.MISSING_ENUMERABLE_ERC20,
            "Not ERC20 pair"
        );
