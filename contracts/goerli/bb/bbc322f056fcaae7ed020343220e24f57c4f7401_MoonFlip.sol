...
contract MoonFlip is Ownable, ReentrancyGuard {
    ...
    struct Commit {
        ...
        uint256 blocknumber; // the block number
    ...
    mapping(address => Commit) _pendingCommits;
    ...
    function clearPendingCommit(address addr) external onlyOwner {
        delete _pendingCommits[addr];
    }
    ...
    modifier gameChecks() {
        ...
        require(
            !_pendingCommits[msg.sender].exists,
    ...
    function flipCoin(CoinSide guess) public payable gameChecks nonReentrant {
        ...
        _pendingCommits[msg.sender] = Commit({
            guess: uint8(guess),
            value: wager,
        ...
        emit GuessSubmitted(
            Game.COIN_FLIP,
            _pendingCommits[msg.sender],
            msg.sender
        );

        // take the fee and send it to the house.
        (bool ok, ) = payable(houseAddress).call{value: fee}("");
