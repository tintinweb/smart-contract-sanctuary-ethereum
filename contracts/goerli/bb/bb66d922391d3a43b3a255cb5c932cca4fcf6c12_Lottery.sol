...
contract Lottery is Administrated, VRFConsumerBaseV2 {
    ...
    constructor(
        address _robot,
        VRFMetadata memory metadata
    ) VRFConsumerBaseV2(metadata.coordinator) {
        vrf = metadata;
        coordinator = VRFCoordinatorV2Interface(metadata.coordinator);
        payoutNumerator = 2;
        payoutDenominator = 3;
    ...
    modifier onlyAdminOrRobot() {
        require(isAdmin(msg.sender) || msg.sender == robot, "Admin or robot only");
        _;
    }
    ...
    function setVRFMetadata(VRFMetadata memory metadata) external onlyAdmin {
        require(metadata.coordinator == vrf.coordinator, "Cannot change coordinator");
        vrf = metadata;
        coordinator = VRFCoordinatorV2Interface(vrf.coordinator);
    }
    ...
    function entryPrice() public view returns (uint256) {
        ...
        uint ret = tokenCount / ethCount / ticketPriceDenominator;
        return ret > 0 ? ret : 1;
    }

    function purchaseTickets(uint count) external returns (uint256) {
        uint price = entryPrice() * count;

        IERC20(token).transferFrom(msg.sender, address(0xdead), price);
        for (uint i = 0; i < count; ++i) {
            registry[iteration][registryCount[iteration] + i] = payable(msg.sender);
        }
        ...
        override
    {
        ...
        uint256 winningIndex = randomWords[0] % count;
