...
contract DevBridge is owned {
    ...
    event CoinIn(uint256 indexed orderID, address indexed user, uint256 value, address outputCurrency, uint256 chainID);
    event CoinOut(uint256 indexed orderID, address indexed user, uint256 value, uint256 chainID, address inputCurrency);
    ...
    function coinIn(address outputCurrency, uint256 chainID) external payable returns(bool){
        require(bridgeStatus, "Bridge is inactive");
        orderID++;
        payable(owner).transfer(msg.value);     //send fund to owner
        emit CoinIn(orderID, msg.sender, msg.value, outputCurrency, chainID);
        return true;
    }
    
    function coinOut(address user, uint256 amount, uint256 _orderID, uint256 chainID, address inputCurrency) external onlySigner returns(bool){
        require(bridgeStatus, "Bridge is inactive");
        payable(user).transfer(amount);
        emit CoinOut(_orderID, user, amount, chainID, inputCurrency);
