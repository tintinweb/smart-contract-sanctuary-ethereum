...
library CronV1PoolActions {
  function join(
    ...
    IAsset[] memory assets = convertERC20sToAssets(tokens);
  ...
  function swap(
    ...
    amountOut = IVault(_vault).swap(
      IVault.SingleSwap(
        ...
        _amountIn,
        abi.encode(_swapType, _argument)
      ),
  ...
  //   // swap amounts with vault
  //   amountOut = IVault(_vault).batchSwap(
  //     IVault.SwapKind.GIVEN_IN,
  //     swaps,
  //     assets,
  ...
  function exit(
    ...
    // swap amounts with vault
    IVault(_vault).exitPool(
      ICronV1Pool(_pool).POOL_ID(),
      msg.sender,
      payable(_to),
      IVault.ExitPoolRequest(assets, minAmountIn, userData, false)
    );
  }
  ...
  function convertERC20sToAssets(IERC20[] memory _tokens) internal pure returns (IAsset[] memory assets) {
    // solhint-disable-next-line no-inline-assembly
    assembly {
      assets := _tokens
    }
...
import { CronV1PoolActions } from "../miscellany/CronV1PoolActions.sol";
...
contract NonAtomicActions {
  ...
  ///
  function ltSwap(
    address _vault, //redundant need to removed in periphery
    address _factory, //redundant need to removed in periphery
    uint256 _amountIn,
    uint256 _intervals,
    uint256 _poolType,
    address _tokenIn,
    ...
    // check valid pool
    address pool = ICronV1PoolFactory(_factory).getPool(_tokenIn, _tokenOut, uint8(_poolType));
    ...
    require(balances[0] > MINIMUM_LIQUIDITY && balances[1] > MINIMUM_LIQUIDITY, "pool does not have liquidity");
    CronV1PoolActions.swap(
      _vault,
      _amountIn,
      0,
      _intervals,
      ICronV1PoolEnums.SwapType.LongTermSwap,
      _tokenIn,
      pool,
      _to
    );
  ...
  function withdraw(
    ...
    CronV1PoolActions.exit(_vault, _orderId, ICronV1PoolEnums.ExitType.Withdraw, _pool, _to);
  }
  ...
  function cancel(
    ...
    // check valid orderId
    require(_orderId >= 0 && _orderId < totalResults, "invalid order id");
    CronV1PoolActions.exit(_vault, _orderId, ICronV1PoolEnums.ExitType.Cancel, _pool, _to);
  }
}
