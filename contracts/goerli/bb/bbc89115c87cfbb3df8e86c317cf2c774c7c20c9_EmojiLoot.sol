...
contract EmojiLoot is ERC721("Emoji Loot", "eLOOT"), ReentrancyGuard, Ownable {
    ...
    function getFaces(uint256 tokenId) public view returns (string memory) {
        return pluck(tokenId, "FACES", faces);
    }
    ...
    function getFlowers(uint256 tokenId) public view returns (string memory) {
        return pluck(tokenId, "FLOWERS", flowers);
    }
    ...
    function pluck(
        ...
        require(tokenId < maxSupply, "Token ID invalid");
        uint256 rand = random(
        ...
        returns (string memory)
    {
        ...
        parts[
            0
        ] = '<svg xmlns="http://www.w3.org/2000/svg" preserveAspectRatio="xMinYMin meet" viewBox="0 0 350 350"><style>.base { fill: white; font-family: serif; font-size: 30px; }</style><rect width="100%" height="100%" fill="black" /><text x="10" y="40" class="base">';

        parts[1] = getWeapon(tokenId);

        parts[2] = '</text><text x="10" y="80" class="base">';

        parts[3] = getFaces(tokenId);
    ...
    function claim(uint256 tokenId) public nonReentrant onlyEOA {
    ...
    function ownerClaim(uint256 tokenId) public nonReentrant onlyOwner {
        require(tokenId > 7777 && tokenId < maxSupply, "Token ID invalid");
