...
abstract contract NFTFarmTemplate is Ownable, ReentrancyGuard {
    ...
    // Instances
    IERC20 private constant hiIQ =
        IERC20(0x016301ef432b8908d14ba809909346Da6029212d);
        ...
        returns (uint256 rwd_rate)
    {
        ...
        if (gauge_controller_address != address(0)) {
            rwd_rate =
                (IGaugeController(gauge_controller_address)
    ...
    function maxLPForMaxBoost(address account) external view returns (uint256) {
        return
            (hiIQ.balanceOf(account) * MULTIPLIER_PRECISION) /
            hiiq_per_frax_for_max_boost;
    }
    ...
    function minHiIQForMaxBoost(address account) public view returns (uint256) {
        return
            (userStaked(account) * hiiq_per_frax_for_max_boost) /
            MULTIPLIER_PRECISION;
    }

    function minHiIQForMaxBoostProxy(address proxy_address)
        public
        view
        returns (uint256)
    {
        return
            (proxyStaked(proxy_address) * hiiq_per_frax_for_max_boost) /
            MULTIPLIER_PRECISION;
    }
    ...
    function hiIQMultiplier(address account)
        public
        view
        returns (uint256 hiiq_multiplier)
    {
        // Use either the user's or their proxy's hiIQ balance
        uint256 hiiq_bal_to_use = 0;
        address the_proxy = getProxyFor(account);
        hiiq_bal_to_use = (the_proxy == address(0))
            ? hiIQ.balanceOf(account)
            : hiIQ.balanceOf(the_proxy);

        // First option based on fraction of total hiIQ supply, with an added scale factor
        uint256 mult_optn_1 = (hiiq_bal_to_use *
            hiiq_max_multiplier *
            hiiq_boost_scale_factor) /
            (hiIQ.totalSupply() * MULTIPLIER_PRECISION);
        ...
        uint256 mult_optn_2;
        {
            ...
            hiIQ_needed_for_max_boost = (the_proxy == address(0))
                ? minHiIQForMaxBoost(account)
                : minHiIQForMaxBoostProxy(the_proxy);

            if (hiIQ_needed_for_max_boost > 0) {
                uint256 user_hiiq_fraction = (hiiq_bal_to_use *
                    MULTIPLIER_PRECISION) / hiIQ_needed_for_max_boost;

                mult_optn_2 =
                    (user_hiiq_fraction * hiiq_max_multiplier) /
                    MULTIPLIER_PRECISION;
            } else {
                mult_optn_2 = 0;
    ...
    function getRewardExtraLogic(address destination_address)
        public
        nonReentrant
    {
    ...
    function _getRewardExtraLogic(address rewardee, address destination_address)
        internal
        virtual;
    ...
    function retroCatchUp() internal {
        ...
        for (uint256 i = 0; i < rewardDistributors.length; i++) {
            ...
            if (reward_distributor_address != address(0)) {
                IGaugeRewardsDistributor(reward_distributor_address)
        ...
        for (uint256 i = 0; i < gaugeControllers.length; i++) {
            ...
            if (gauge_controller_address != address(0)) {
                if (
                    ...
                    last_gauge_relative_weights[i] = IGaugeController(
                        gauge_controller_address
                    ).gauge_relative_weight_write(
                            address(this),
                            block.timestamp
                        );
                    last_gauge_time_totals[i] = IGaugeController(
                        gauge_controller_address
                    ).time_total();
                }
            }
