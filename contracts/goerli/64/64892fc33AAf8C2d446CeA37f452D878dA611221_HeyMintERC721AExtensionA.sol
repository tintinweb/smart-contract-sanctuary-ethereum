...
import {AdvancedConfig, AddressConfig, BurnToken, LaunchpadStorage} from "../libraries/LaunchpadStorage.sol";
...
contract HeyMintERC721AExtensionA is HeyMintERC721AUpgradeable {
    ...
    /**
     * @notice Initializes advanced configuration for the contract
     */
    function initializeAdvancedConfig(
        AdvancedConfig memory _advancedConfig
    ) external onlyOwner {
        require(
            !LaunchpadStorage.state().data.advancedConfigInitialized,
            "ALREADY_INITIALIZED"
        );
        LaunchpadStorage.state().data.advancedConfigInitialized = true;
    ...
    function freezePayoutAddresses() external onlyOwner {
        LaunchpadStorage.state().advCfg.payoutAddressesFrozen = true;
    }
    ...
    function updatePayoutAddressesAndBasisPoints(
        ...
        require(
            !LaunchpadStorage.state().advCfg.payoutAddressesFrozen,
        ...
        uint16 totalBasisPoints = 0;
        for (uint16 i = 0; i < _payoutBasisPoints.length; i++) {
            totalBasisPoints += _payoutBasisPoints[i];
        }
        require(totalBasisPoints == 10000, "BASIS_POINTS_MUST_EQUAL_10000");
        LaunchpadStorage.state().addrCfg.payoutAddresses = _payoutAddresses;
        LaunchpadStorage.state().addrCfg.payoutBasisPoints = _payoutBasisPoints;
    }
    ...
    function setRoyaltyBasisPoints(uint16 _royaltyBps) external onlyOwner {
        LaunchpadStorage.state().cfg.royaltyBps = _royaltyBps;
    }
    ...
    function setPresaleState(bool _saleActiveState) external onlyOwner {
        LaunchpadStorage.state().cfg.presaleActive = _saleActiveState;
    }
    ...
    function setPresalePrice(uint16 _presalePrice) external onlyOwner {
        LaunchpadStorage.state().cfg.presalePrice = _presalePrice;
    }
    ...
    function reducePresaleMaxSupply(
        ...
        require(
            _newPresaleMaxSupply <
                LaunchpadStorage.state().cfg.presaleMaxSupply,
            "NEW_MAX_SUPPLY_TOO_HIGH"
        );
        LaunchpadStorage.state().cfg.presaleMaxSupply = _newPresaleMaxSupply;
    }
    ...
    function setPresaleStartTime(uint32 _presaleStartTime) external onlyOwner {
        ...
        LaunchpadStorage.state().cfg.presaleStartTime = _presaleStartTime;
    }
    ...
    function setPresaleEndTime(uint32 _presaleEndTime) external onlyOwner {
        ...
        LaunchpadStorage.state().cfg.presaleEndTime = _presaleEndTime;
    }
    ...
    function setUsePresaleTimes(bool _usePresaleTimes) external onlyOwner {
        LaunchpadStorage.state().cfg.usePresaleTimes = _usePresaleTimes;
    }
    ...
    function presaleTimeIsActive() public view returns (bool) {
        if (
            LaunchpadStorage.state().cfg.usePresaleTimes == false ||
            LaunchpadStorage.state().cfg.presaleStartTime == 0 ||
            LaunchpadStorage.state().cfg.presaleEndTime == 0
        ) {
            return true;
        }
        return
            block.timestamp >= LaunchpadStorage.state().cfg.presaleStartTime &&
            block.timestamp <= LaunchpadStorage.state().cfg.presaleEndTime;
    }
    ...
    function verifySignerAddress(
        ...
        address signerAddress = LaunchpadStorage
            .state()
            .addrCfg
            .presaleSignerAddress != address(0)
            ? LaunchpadStorage.state().addrCfg.presaleSignerAddress
    ...
    function presaleMint(
        ...
        require(LaunchpadStorage.state().cfg.presaleActive, "NOT_ACTIVE");
        require(presaleTimeIsActive(), "NOT_ACTIVE");
        require(
            LaunchpadStorage.state().cfg.presaleMintsAllowedPerAddress == 0 ||
                _numberMinted(msg.sender) + _numTokens <=
                LaunchpadStorage.state().cfg.presaleMintsAllowedPerAddress,
            "MAX_MINTS_EXCEEDED"
        );
        require(
        ...
        require(
            LaunchpadStorage.state().cfg.presaleMaxSupply == 0 ||
        ...
        require(
            msg.value ==
                (LaunchpadStorage.state().cfg.presalePrice * 10 ** 16) *
                    _numTokens,
            "INVALID_PRICE_PAID"
        );
        require(
        ...
        if (totalSupply() >= LaunchpadStorage.state().cfg.presaleMaxSupply) {
            LaunchpadStorage.state().cfg.presaleActive = false;
        }
    }
    ...
    function setBurnClaimState(bool _burnClaimActive) external onlyOwner {
        if (_burnClaimActive) {
            require(
                LaunchpadStorage.state().burnTokens.length != 0,
                "BURN_TOKENS_NOT_SET"
            );
            require(
                LaunchpadStorage.state().advCfg.mintsPerBurn != 0,
                "MINTS_PER_BURN_NOT_SET"
            );
        }
        LaunchpadStorage.state().advCfg.burnClaimActive = _burnClaimActive;
    }
    ...
    function updateBurnTokens(
        ...
        uint256 oldBurnTokensLength = LaunchpadStorage
            .state()
            .burnTokens
            .length;
        if (newBurnTokensLength < oldBurnTokensLength) {
            for (
                ...
                LaunchpadStorage.state().burnTokens.pop();
            }
            for (uint256 i = 0; i < newBurnTokensLength; i++) {
                LaunchpadStorage.state().burnTokens[i] = _burnTokens[i];
            }
        } else {
    ...
    function updateMintsPerBurn(uint8 _mintsPerBurn) external onlyOwner {
        LaunchpadStorage.state().advCfg.mintsPerBurn = _mintsPerBurn;
    }
    ...
    function burnToMint(
        ...
        require(LaunchpadStorage.state().advCfg.burnClaimActive, "NOT_ACTIVE");
        uint256 contractsLength = _contracts.length;
        uint256 burnTokenLength = LaunchpadStorage.state().burnTokens.length;
        require(
        ...
        require(
            totalSupply() +
                (_timesToBurn * LaunchpadStorage.state().advCfg.mintsPerBurn) <=
                LaunchpadStorage.state().cfg.maxSupply,
            "MAX_SUPPLY_EXCEEDED"
        );
        for (uint256 i = 0; i < burnTokenLength; i++) {
            BurnToken memory burnToken = LaunchpadStorage.state().burnTokens[i];
            require(
