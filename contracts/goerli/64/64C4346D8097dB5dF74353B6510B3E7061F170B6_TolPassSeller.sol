...
contract TolPassSeller is AccessControl, Ownable, Pausable {
    ...
    uint256 public mintableAmountForPublic;
    bytes32 merkleRoot;
    ...
    modifier withinMintableAmountPublic(address minter, uint256 amount) {
        require(mintedAmountBySales[salesId][minter] + amount <= mintableAmountForPublic, 'Over Max Amount Per Address');
        _;
    }
    modifier withinMintableAmountAllowlist(address minter, uint256 amount, uint256 allowedAmount) {
    ...
    function airdrop(address[] calldata _addresses, uint256[] calldata _amounts) payable external onlyRole(ADMIN)
        withinMaxSupply(_addresses.length)
    {
        require(_addresses.length == _amounts.length, 'Invalid Arguments');
        for (uint256 i = 0; i < _addresses.length; i++) {
            uint256 _amount = _amounts[i];
            if (tolPass.getTotalSupply() + _amount <= maxSupply) {
    ...
    function publicMint(uint256 _amount) external payable
        ...
        enoughEth(_amount)
        withinMintableAmountPublic(msg.sender, _amount)
    {
    ...
    function allowlistMint(uint256 _amount, uint256 _allowedAmount, bytes32[] calldata _merkleProof) external payable
        ...
        withinMaxSupply(_amount)
        enoughEth(_amount)
        withinMintableAmountAllowlist(msg.sender, _amount, _allowedAmount)
    ...
    function pieMint(address _minter, uint256 _amount, uint256 _allowedAmount, bytes32[] calldata _merkleProof) external payable onlyRole(PIE)
        ...
        withinMaxSupply(_amount)
        enoughEth(_amount)
        withinMintableAmountAllowlist(_minter, _amount, _allowedAmount)
    ...
    function setPublicSalesInfo(uint256 _salesId, uint256 _maxSupply, uint256 _mintCost, uint256 _mintableAmountForPublic) public onlyRole(ADMIN) {
        isPublicSale = true;
        salesId = _salesId;
        maxSupply = _maxSupply;
        mintCost = _mintCost;
        mintableAmountForPublic = _mintableAmountForPublic;
    }
    function setAllowlistSalesInfo(uint256 _salesId, uint256 _maxSupply, uint256 _mintCost, bytes32 _merkleRoot) public onlyRole(ADMIN) {
        isPublicSale = false;
        salesId = _salesId;
        maxSupply = _maxSupply;
        mintCost = _mintCost;
    ...
    function setIsPublicSale(bool _value) public onlyRole(ADMIN) {
        isPublicSale = _value;
    }
    ...
    function setMintableAmount(uint256 _value) public onlyRole(ADMIN) {
        mintableAmountForPublic = _value;
    }
