...
contract PrivateRound{
    ...
    mapping(address => MilestoneSetup[]) private _milestone;
    ...
    mapping(uint => mapping(uint => address)) private fundsRequested;   
    // round id => funds => investor
    mapping(uint => mapping(address => uint)) private initialPercentage;
    // round id => investor => initialPercentage
    mapping(uint => mapping(address => address)) private tokenExpected;
    ...
    */
    ...
    MilestoneSetup[] memory _mile) public {
        ...
        for(uint i = 0; i < _mile.length; ++i){
            _milestone[_investor].push(_mile[i]);
        }
    ...
    function depositTokens(address _tokenContract, address _investorSM, address _founder, uint _tokens, uint _roundId) public {
        ...
        initialTokensForFounder[_roundId][_founder] = _tokens * (initialPercentage[_roundId][msg.sender] / 100);
    ...
    function withdrawInitialPercentage(address _tokenContract, address _founderSM, uint _roundId) public {
        ...
        initialTokensForFounder[_roundId][msg.sender] = 0;
    }

    mapping(uint => mapping(address => uint)) public approvedByInvestor;
    mapping(uint => mapping(address => uint)) public rejectedByInvestor;
    mapping(uint => bool) public projectCancel;
    // milestone id => founder address => uint

    function validateMilestones(address _founder, address _investorSM, uint _milestoneId, uint _roundId, bool _status) public {
        require(msg.sender != address(0), "The address is not valid or the address is 0");
        InvestorLogin investor = InvestorLogin(_investorSM);
        require(investor.verifyInvestor(msg.sender) == true, "The address is not registered in the 'InvestorLogin' contract");
        if(_status == true){
            approvedByInvestor[_milestoneId][_founder] += 1;
        }else if(_status == false){
            rejectedByInvestor[_milestoneId][_founder] += 1;
        }
        if(rejectedByInvestor[_milestoneId][_founder] >= 3){
            projectCancel[_roundId] = true;
        }
    }

    uint public roundCancelStructure;
    uint private releasedTokenToFounder;
    ...
    function withdrawMilestoneTokens(address _founderSM, address _investor, uint _roundId, address _tokenContract) public {
        ...
        FounderLogin founder = FounderLogin(_founderSM);
        require(founder.verifyFounder(msg.sender) == true, "The address is not registered in the 'FounderLogin' contract");
        for(uint i = 0; i < _milestone[_investor].length; i++){
            if(block.timestamp > _milestone[_investor][i]._date){
                roundCancelStructure += 1;
            }
        }
        if(projectCancel[_roundId] = true || roundCancelStructure > 2){
            defaultedByFounder = true;
            revert("The Founder has failed to withdraw atleast 2 milestone released tokens or maximum milestone updation has exceeded");
        }
        if(roundCancelStructure < 2) {
            uint unlockedAmount = 0;
            uint localPercent;
            for(uint i = 0; i < _milestone[_investor].length; i++){
                if(_milestone[_investor][i]._date <= block.timestamp){
                    localPercent = remainingTokensOfInvestor[_roundId][_investor] * (_milestone[_investor][i]._percent / 100);
            ...
            releasedTokenToFounder += unlockedAmount;
            FundLock fl = FundLock(seperateContractLink[_roundId][msg.sender]);
    ...
    function withdrawByInvestors(address _investorSM, uint _roundId, address _founder, address _tokenContract) public{
        ...
        if(projectCancel[_roundId] = true || roundCancelStructure > 2){
            ...
            uint tax = 2 * (remainingTokensOfInvestor[_roundId][msg.sender] / 100);
            taxedTokens[_tokenContract] += tax;
            remainingTokensOfInvestor[_roundId][msg.sender] -= tax;
            ERC20(_tokenContract).transferFrom(address(fl), msg.sender, remainingTokensOfInvestor[_roundId][msg.sender]);
            remainingTokensOfInvestor[_roundId][msg.sender] = 0;   
        }
    }
    ...
    */
    ...
    function getMilestonesDetails(address _investor) public view returns(MilestoneSetup[] memory){
        return _milestone[_investor];
    }
    ...
    function projectStatus(uint _roundId) public view returns(bool project){
        return projectCancel[_roundId];
    }

    function depositedTokenStatus(uint _roundId, address _tokenContract) public view returns(uint investorTokens,
    uint escrowBal, uint tax, uint tokensReleasedToFounder){
        return (remainingTokensOfInvestor[_roundId][msg.sender],
        escrowBalance,taxedTokens[_tokenContract],releasedTokenToFounder);
    }

    function milestoneRejectionStatus(uint _milestoneId, address _founder) public view returns(uint){
        return rejectedByInvestor[_milestoneId][_founder];
    }
