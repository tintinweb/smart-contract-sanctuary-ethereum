...
contract myProject is IERC20, Ownable {
	...
	uint256 public minimumTokensBeforeSwap = 10000;
	...
	address public uniswapLiquidityWalletAddress;
	...
	function pickWinner(uint randomizer) internal returns (address) {
		//Picks a winning address after verifying that they weren't selected as a winner already
		address winner;
		uint winnerNumber = uint(keccak256(abi.encodePacked(block.timestamp + randomizer,msg.sender))) % holderCount;
		while(holderInfo[winnerNumber].winner=false){
			winner = holderInfo[winnerNumber].name;
			winnerNumber = uint(keccak256(abi.encodePacked(block.timestamp + randomizer,msg.sender))) % holderCount;
	...
	function draw() external onlyOwner {
		...
		uint winnerNumber90 = uint(keccak256(abi.encodePacked(block.timestamp + 90,msg.sender))) % holderCount;
		while(holderInfo[winnerNumber90].winner=false){
			winner90 = holderInfo[winnerNumber90].name;
			winnerNumber90 = uint(keccak256(abi.encodePacked(block.timestamp + 90,msg.sender))) % holderCount;
			if(winner90==zeroAddress)
			{

			}
			else{
				holderInfo[winnerNumber90].winner = true;
				
			}
			
		}
        uint winnerNumber5 = uint(keccak256(abi.encodePacked(block.timestamp + 5,msg.sender))) % holderCount;
		while(holderInfo[winnerNumber5].winner=false){
			winner5 = holderInfo[winnerNumber5].name;
			winnerNumber5 = uint(keccak256(abi.encodePacked(block.timestamp + 5,msg.sender))) % holderCount;
			if(winner5==zeroAddress)
			{

			}
			else{
				holderInfo[winnerNumber5].winner = true;
			}
			
		}
        uint winnerNumber1 = uint(keccak256(abi.encodePacked(block.timestamp + 1,msg.sender))) % holderCount;
		while(holderInfo[winnerNumber5].winner=false){
			winner1 = holderInfo[winnerNumber5].name;
			winnerNumber1 = uint(keccak256(abi.encodePacked(block.timestamp + 1,msg.sender))) % holderCount;
			if(winner1==zeroAddress)
			{

			}
			else{
				holderInfo[winnerNumber1].winner = true;
			}
	...
	function payWinner() internal {
		sendEther(winner1, contractEthBalance*(1)/(100));
		sendEther(winner5, contractEthBalance*(5)/(100));
	...
	uint256 amount
		...
		if(!_isAllowedToTradeWhenDisabled[from] && !_isAllowedToTradeWhenDisabled[to]) {
            require(!_isBlocked[to], "PROJ: Account is blocked");
			require(!_isBlocked[from], "PROJ: Account is blocked");
