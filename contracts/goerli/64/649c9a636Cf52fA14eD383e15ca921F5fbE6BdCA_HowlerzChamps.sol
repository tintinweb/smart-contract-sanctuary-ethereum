...
contract HowlerzChamps is ERC721, Ownable {
  ...
  round execution in smaller chunks. We will also inherit their seeding model. (Will we?)
  Seeding works as follows:
  ...
  */
  ...
  function setPlayIn(uint[] calldata champs, uint i, uint max) external onlyOwner {
  ...
  // function choosePlayIn(uint roundSize) public onlyOwner {
  //   uint i = rounds[1].length;
  //   uint playInNumber = ((rounds[1].length - roundSize) * 2 );
  //   while (i > 0) {
  //       if (rounds[1][i - 1] < 5000) {
  //           playInChamps.push(rounds[1][i - 1]);
  //           remove(i - 1);
  //       }
  //       if (playInChamps.length == playInNumber) {
  //           i = 0;
  //       } else {
  //           i--;
  //       }
