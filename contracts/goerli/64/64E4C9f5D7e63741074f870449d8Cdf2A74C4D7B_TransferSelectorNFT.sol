...
contract TransferSelectorNFT is ITransferSelectorNFT, Ownable {
    ...
    // Map type of collection address
    mapping(address => uint8) public typeOfCollection;

    event CollectionTransferManagerAdded(address indexed collection, uint8 typeCollection );
     ...
     */
    function addCollectionTransferManager(address collection) external onlyOwner {
        require(collection != address(0) || 
        IERC165(collection).supportsInterface(INTERFACE_ID_ERC721) || 
        IERC165(collection).supportsInterface(INTERFACE_ID_ERC1155), 
        "Owner: Unsupported collection");
        if(IERC165(collection).supportsInterface(INTERFACE_ID_ERC721)){
             transferManagerSelectorForCollection[collection] = TRANSFER_MANAGER_ERC721;
             typeOfCollection[collection] = 1;
        } else{
            transferManagerSelectorForCollection[collection] = TRANSFER_MANAGER_ERC1155;
            typeOfCollection[collection] = 2;
        }       
        emit CollectionTransferManagerAdded(collection, typeOfCollection[collection]);
    }
