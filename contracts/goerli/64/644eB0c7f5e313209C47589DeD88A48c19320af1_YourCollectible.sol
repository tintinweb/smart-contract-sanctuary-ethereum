...
interface IERC721JPromo is IERC721J {
    ...
    function claimCheck(address _contract)
        external
        view
        returns (uint256 _songIdOut, uint256 _generationOut, uint256 _promoPercentOut, uint256 _promoMintPrice);
    ...
    ) external view returns (uint256 _songIdOut, uint256 _generationOut, uint256 _promoPercentOut, uint256 _promoMintPrice);
...
import "./IERC721J.sol"; //Interface Id: 0x0a5ba4c5
import "./IERC721JFull.sol"; //Interface Id: 0x024aa692
import "./IERC721JPromo.sol"; //Interface Id: 0x814fedca
    ...
    IERC2981
{
    ...
    //returns a songURI, when given the songId and songGeneration
    function songURITest(uint256 songId, uint256 songGeneration)
        public
        view
        virtual
        returns (string memory)
    ...
    function setPromo(
        ...
        uint256 _songOut = (promoPercentOut * (10**69)) + (songIdOut * (10**18)) + generationOut;
        ...
        returns (uint256 _songIdOut, uint256 _generationOut, uint256 _promoPercentOut, uint256 _promoMintPrice)
    {
        ...
        _songIdOut = (_songOut / (10**18)) % (10**51);
        _generationOut = _songOut % (10**18);
        _promoPercentOut = _songOut / (10**69);
        _promoMintPrice = (_mintPrice * _promoPercentOut) / 100;
    }
    ...
    function claimCheckSpecific(
        ...
        uint256 songIdIn,
        uint256 generationIn
    ) public view virtual override returns (uint256 _songIdOut, uint256 _generationOut, uint256 _promoPercentOut, uint256 _promoMintPrice) {
        uint256 _songIn = (songIdIn * (10**18)) + generationIn;
    ...
    function payPromo(uint256 _songOut) internal {
        uint256 _songIdOut = (_songOut / (10**18)) % (10**51);
        uint256 _promoPercentOut = _songOut / (10**69);
    ...
    //
    //Promo claim specific is for YourCollectible tokens
    function promoClaimSpecific(address _contract, uint256 tokenId) public payable override {
        //Sets contract address as external nft
        YourCollectible _claimContract = YourCollectible(_contract);
        //Checks if the token has been claimed
    ...
    //
    //Promo claim song reward is for YourCollectible tokens to claim a song specific reward
    ...
    //
    //Promo claim rarity reward is for YourCollectible tokens to claim a rarity specific reward
