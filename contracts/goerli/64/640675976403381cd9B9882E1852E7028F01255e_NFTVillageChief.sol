...
contract NFTVillageChief is BaseStructs, Ownable, ERC721Holder, ERC1155Holder {
    ...
    uint256 constant public REWARD_MULTIPLICATOR = 1e12;
    uint256 constant public BASE_SHARE_MULTIPLIER = 10000;
    ...
    function depositRewardToken(uint256 projectId, uint256 poolId, uint256 rewardId, uint256 amount) nonReentrant external {
    ...
    function _updatePool(uint256 projectId, uint256 poolId) private {
        ...
        for (uint256 i = 0; i < rewardInfo.length; i++) {
            ...
            if(_rewardInfo.mintable) {
                projectHandler.setRewardPerShare(projectId, poolId, i, _rewardInfo.accRewardPerShare.add(rewardAmount.mul(REWARD_MULTIPLICATOR).div(_pool.totalShares)));
                if(devFee > 0) {
                ...
                if(rewardAmount > 0) projectHandler.setRewardPerShare(projectId, poolId, i, _rewardInfo.accRewardPerShare.add(rewardAmount.mul(REWARD_MULTIPLICATOR).div(_pool.totalShares)));
                if(devFee > 0) {
    ...
    function _updateRewardDebt(uint256 projectId, uint256 poolId) private {
        ...
        while (userReward.length < _rewardInfo.length)
            userReward.push();
    ...
    function deposit(uint256 projectId, uint256 poolId, uint256 amount,
        ...
        NftDeposit[] memory harvestCards, NftDeposit[] memory multiplierCards, address referrer)  external validatePoolByPoolId(projectId, poolId) nonReentrant inDeposit {
    ...
    function _depositCards(uint256 projectId, uint256 poolId, 
        ...
        if (user.canHarvestAt > 0)
            user.canHarvestAt -= harvestRelief;
        
        user.shareMultiplier += cardHandler.useCard(msg.sender, uint8(CardType.MULTIPLIER), projectId, poolId, multiplierCards);
    }
    ...
    function _depositTokens(uint256 projectId, uint256 poolId, uint256 amount) private {
        ...
        if (user.depositFeeDiscount < FEE_DENOMINATOR && pool.stakedTokenStandard == uint8(TokenStandard.ERC20))
            adminDepositFee = amount * pool.depositFee * (FEE_DENOMINATOR - user.depositFeeDiscount) / FEE_DENOMINATOR / FEE_DENOMINATOR;
    ...
    function _transferStakedToken(PoolInfo memory pool, address from, address to, uint256 amount) private returns(uint256) {
    ...
    function withdraw(uint256 projectId, uint256 poolId, uint256 amount) external nonReentrant {
        require (poolId < projectHandler.getProjectInfo(projectId).pools.length , "NFTVillageChief: Pool does not exist");

        ProjectInfo memory project = projectHandler.getProjectInfo(projectId);
        PoolInfo memory pool = project.pools[poolId];
        UserInfo storage user = userInfo[projectId][poolId][msg.sender];
        uint256 amountToTransfer = pool.stakedTokenStandard == uint8(TokenStandard.ERC20) ? amount : amount * CARD_AMOUNT_MULTIPLIER;
        ...
        if(user.amount == amountToTransfer) {
            ...
            user.shareMultiplier -= cardHandler.withdrawCard(msg.sender, uint8(CardType.MULTIPLIER), projectId, poolId, cardHandler.getUserCardsInfo(projectId, poolId, msg.sender).multiplier);
        ...
        if(amount > 0) {
            ...
            _transferStakedToken(pool, address(this), address(msg.sender), pool.stakedTokenStandard == uint8(TokenStandard.ERC20) ? amountToTransfer : amount);
        }
        _updateShares(projectId, poolId);
        _updateRewardDebt(projectId, poolId);
    ...
    function withdrawHarvestCard(uint256 projectId, uint256 poolId, NftDeposit[] calldata cards) nonReentrant external {
        UserInfo storage user = userInfo[projectId][poolId][msg.sender];
        uint256 withdrawnRelief = cardHandler.withdrawCard(msg.sender, uint8(CardType.HARVEST_RELIEF), projectId, poolId, cards);
        user.harvestRelief -= withdrawnRelief;
        user.canHarvestAt += withdrawnRelief;
    }

    function withdrawMultiplierCard(uint256 projectId, uint256 poolId, NftDeposit[] calldata cards) nonReentrant validatePoolByPoolId(projectId, poolId) external {
        _updatePool(projectId, poolId);
        _payOrLockupPendingToken(projectId, poolId);
        UserInfo storage user = userInfo[projectId][poolId][msg.sender];
    ...
    function _payOrLockupPendingToken(uint256 projectId, uint256 poolId) private {
        ...
        if (user.canHarvestAt == 0) {
            user.canHarvestAt = block.timestamp.add(user.harvestRelief > pool.harvestInterval ? 0 : pool.harvestInterval - user.harvestRelief);
        }
        ...
        for (uint256 i = 0; i < _rewardInfo.length; i++) {
            ...
            uint256 pending = user.shares.mul(_rewardInfo[i].accRewardPerShare).div(REWARD_MULTIPLICATOR).sub(userReward[i].rewardDebt);
            if (_canHarvest) {
    ...
    function _payReferralCommission(uint256 projectId, uint256 poolId, uint256 rewardInfoIndex, address _user, uint256 _pending, uint256 _referralCommissionRate, RewardInfo memory rewardInfo) internal {
    ...
    function emergencyWithdraw(uint256 projectId, uint256 poolId) validatePoolByPoolId(projectId, poolId) external nonReentrant {
        ...
        _transferStakedToken(pool, address(this), address(msg.sender), pool.stakedTokenStandard == uint8(TokenStandard.ERC20) ? user.amount : user.amount / CARD_AMOUNT_MULTIPLIER);
    ...
    function emergencyProjectAdminWithdraw(uint256 projectId, uint256 poolId, uint256 rewardId) external nonReentrant {
        ProjectInfo memory project = projectHandler.getProjectInfo(projectId);
...
interface IProjectHandler is BaseStructs {
    ...
    function addProject(address _admin, uint256 _adminReward, uint256 _referralFee, uint256 _startBlock, INFTVillageCards _poolCards) external payable;
    function setProject(uint256 projectId, address _admin, uint256 _adminReward, uint256 _referralFee, INFTVillageCards _poolCards) external;
    function addPool(uint256 projectId, PoolInfo memory _pool, RewardInfo[] memory _rewardInfo, NftDeposit[] calldata _requiredCards) external payable;
