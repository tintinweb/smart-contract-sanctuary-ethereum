...
contract ChainPotsTest {
    ...
    uint256 public bronzeLotteryEntryPrice = 0.01 ether;
    uint256 public bronzeLotteryPotMaxAmount = 0.02 ether;
    uint256 public bronzeLotteryMaxEntries = 2;

    address payable[] public playersSilverLottery;
    uint256 public silverLotteryEntryPrice = 0.01 ether;
    uint256 public silverLotteryPotMaxAmount = 0.03 ether;
    uint256 public silverLotteryMaxEntries = 3;

    address payable[] public playersGoldLottery;
    uint256 public goldLotteryEntryPrice = 0.01 ether;
    uint256 public goldLotteryPotMaxAmount = 0.04 ether;
    uint256 public goldLotteryMaxEntries = 4;
    ...
    uint256 public bronzeHTHEntryPrice = 0.01 ether;
    uint256 public bronzeHTHPotMaxAmount = 0.02 ether;
    uint256 public bronzeHTHMaxEntries = 2;

    address payable[] public playersSilverHTH;
    uint256 public silverHTHEntryPrice = 0.01 ether;
    uint256 public silverHTHPotMaxAmount = 0.02 ether;
    uint256 public silverHTHMaxEntries = 2;

    address payable[] public playersGoldHTH;
    uint256 public goldHTHEntryPrice = 0.01 ether;
    uint256 public goldHTHPotMaxAmount = 0.02 ether;
    ...
    function enterDailyPot(uint256 _count) public payable {
        ...
        for (uint256 i = 0; i < _count; i++) {
            ...
            dailyPotAmount = dailyPotAmount + dailyPotEntryPrice * _count;
        }
        emit EntryDailyPot(msg.sender, msg.value);
    }

    function pickDailyPotWinner() public {
        // require(
        //     block.timestamp - lastDailyPot > 2 minutes,
        //     "dailyPot timer is still running"
        // );
        // lastDailyPot = block.timestamp;
        uint256 index = randomDailyPot() % playersDailyPot.length;
        address payable winningAddress = playersDailyPot[index];
        uint256 winningAmount = (dailyPotAmount * winnerCut) / 100;
        winningAddress.transfer(winningAmount);
        payable(houseAddressOne).transfer((dailyPotAmount * houseCutOne) / 100);
        payable(houseAddressTwo).transfer((dailyPotAmount * houseCutTwo) / 100);
