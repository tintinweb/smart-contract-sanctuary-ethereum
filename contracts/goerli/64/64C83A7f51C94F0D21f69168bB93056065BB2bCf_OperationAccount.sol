...
interface IAccount {
    ...
    event TransferredTokens(address token, address to, uint256 amount);

    /**
     * @dev Send all Eth to the owner.
     */
    function withdraw(uint256 amount) external;

    /**
     * @dev Send token amount to the owner.
     */
    function withdrawTokens(address token, uint256 amount) external;

    /**
     * @dev Send token amount to any address.
     */
...
interface IClientAccount is IAccount {

    /**
     * @dev Initialize client account.
     */
    function initialize(bytes32 clientIdentifier) external;
}
...
import "./IClientAccount.sol";
import "./IClientAccountBeacon.sol";
import "./MultisigTransferOwnership.sol";

contract MultisigClientAccountFactory is MultisigTransferOwnership {
    event ClientAccountCreated(address proxy);

    mapping(bytes32 => address) private _clientAccountProxies;

    address internal _treasureAccount;

    modifier onlyExistingClientAccount(bytes32 clientId) {
        _checkExistingClientAccount(clientId);
        _;
    }

    function __MultisigClientAccountFactory_init(address treasureAccount) internal onlyInitializing {
        __MultisigClientAccountFactory_init_unchained(treasureAccount);
    }

    function __MultisigClientAccountFactory_init_unchained(address treasureAccount) internal onlyInitializing {
        _treasureAccount = treasureAccount;
    }

    function createClientAccount(bytes32 clientId) external
        onlyOwner
        returns (address) {

        if (_clientAccountProxies[clientId] == address(0)) {
            address proxy = _deployClientAccountBeaconProxy(_clientAccountPayload(clientId));
            emit ClientAccountCreated(proxy);
            _clientAccountProxies[clientId] = proxy;
        }
        return _clientAccountProxies[clientId];
    }

    function getClientAccount(bytes32 clientId) public view
        returns (address) {

        return address(_clientAccountProxies[clientId]);
    }

    function withdrawClientAccount(bytes32 clientId, uint256 amount) external
        onlyOwner
        onlyExistingClientAccount(clientId) {

        IClientAccount(_clientAccountProxies[clientId]).withdraw(amount);
    }

    function withdrawClientAccountTokens(bytes32 clientId, address token, uint256 amount) external
        onlyOwner
        onlyExistingClientAccount(clientId) {

        IClientAccount(_clientAccountProxies[clientId]).withdrawTokens(token, amount);
    }

    /**
     * @dev Transfer token from clientId client account to specified address
     */
    function transferClientAccountTokens(bytes32 clientId, address token, address to, uint256 amount) external
        onlyOwner
        onlyExistingClientAccount(clientId) {

        IClientAccount(_clientAccountProxies[clientId]).transferTokens(token, to, amount);
    }

    function _clientAccountPayload(bytes32 clientId) internal pure returns (bytes memory) {
        return abi.encodeWithSelector(IClientAccount.initialize.selector, clientId);
    }

    function _codeForBeaconProxy(address beacon, bytes memory data) internal pure returns (bytes memory) {
    ...
    function _deployClientAccountBeaconProxy(bytes memory payload) internal returns (address) {
        bytes memory code = _codeForBeaconProxy(
            IClientAccountBeacon(_treasureAccount).getClientAccountBeacon(), payload);
        BeaconProxy proxy = _deployBeaconProxy(code);
        return address(proxy);
    }

    function _checkExistingClientAccount(bytes32 clientId) internal view virtual {
        require(_clientAccountProxies[clientId] != address(0), "Client account not exists");
    }
}
...
contract MultisigTransferTokenManageable is Multisignable {

    uint8 private _operation;

    struct TransferableTokenTrans {
        address token;
        bool del;
    }
    mapping (uint256 => TransferableTokenTrans) private _transactions;
    mapping (address => bool) private _isTransferableToken;
    address[] private _transferableTokens;

    modifier checkTokenTransferable(address token) {
        require(_isTransferableToken[token]);
        _;
    }

    function __MultisigTransferTokenManageable_init(uint8 operation, uint signCount, address[] memory transferableTokens) internal onlyInitializing {
        __MultisigTransferTokenManageable_init_unchained(operation, signCount, transferableTokens);
    }

    function __MultisigTransferTokenManageable_init_unchained(uint8 operation, uint signCount, address[] memory transferableTokens) internal onlyInitializing {
        _operation = operation;
        _signatureCount[operation] = signCount;
        _operations.push(operation);
        for (uint i = 0; i < transferableTokens.length; i++) {
            _isTransferableToken[transferableTokens[i]] = true;
        }
        _transferableTokens = transferableTokens;
    }

    function requestTransferTokenOp(address token, bool del) external
        ownerExists(msg.sender)
        returns (uint256 transferableTokenTransId) {

        transferableTokenTransId = createTransferableTokenTrans(token, del);
        confirmTransferTokenOp(transferableTokenTransId);
        return transferableTokenTransId;
    }

    function confirmTransferTokenOp(uint256 transactionId)
        public
        ownerExists(msg.sender) {

        confirmTransaction(_operation, transactionId);
        executeTransferTokenOp(transactionId);
    }

    function revokeTransferTokenOp(uint256 transactionId)
        public
        ownerExists(msg.sender) {
    ...
    function createTransferableTokenTrans(address token, bool del)
    private
    returns (uint256 transferableTokenTransId)
    {

        transferableTokenTransId = getTransactionCount(_operation);

        TransferableTokenTrans storage transferableTokenTrans = _transactions[transferableTokenTransId];
        createTransaction(_operation, transferableTokenTransId);
        transferableTokenTrans.token = token;
        transferableTokenTrans.del = del;

        return transferableTokenTransId;
    }

    function executeTransferTokenOp(uint256 transactionId)
    private
    ownerExists(msg.sender)
    transactionConfirmed(_operation, transactionId, msg.sender)
    transactionNotExecuted(_operation, transactionId)
    {
        if (isConfirmed(_operation, transactionId)) {
            TransferableTokenTrans storage transferableTokenTrans = _transactions[transactionId];
            address token = transferableTokenTrans.token;

            _isTransferableToken[token] = !transferableTokenTrans.del;
            if (transferableTokenTrans.del) {
                for (uint i = 0; i < _transferableTokens.length; i++)
                    if (_transferableTokens[i] == token) {
                        _transferableTokens[i] = _transferableTokens[_transferableTokens.length - 1];
                        _transferableTokens.pop();
                        break;
                    }
            } else {
                _transferableTokens.push(token);
            }
    ...
    function getTransferableTokenTrans(uint256 transId) public view
        returns(TransferableTokenTrans memory, Transaction memory) {

        return (_transactions[transId], getTrans(_operation, transId));
    }

    function getTransferableTokens() public view
        returns(address[] memory) {

        return _transferableTokens;
    }

    function isTransferableToken(address token) public view
        returns(bool) {

        return _isTransferableToken[token];
    }
...
import "./MultisigClientAccountFactory.sol";
import "./IAccount.sol";
import "./Multisignable.sol";
import "./MultisigTransferOwnership.sol";
import "./MultisigUpgradeable.sol";
import "./MultisigOwnersManageable.sol";
import "./MultisigTransferTokenManageable.sol";

contract OperationAccount is
    IAccount,
    Multisignable,
    MultisigUpgradeable,
    MultisigOwnersManageable,
    MultisigTransferOwnership,
    MultisigClientAccountFactory,
    MultisigTransferTokenManageable,
    UUPSUpgradeable {
    ...
    enum Operation {
        ...
        MANAGE_TRANSFER_TOKENS
    }

    function initialize(
        address[] calldata owners,
        uint signCount,
        address treasureAccount,
        address owner,
        address[] calldata transferableTokens
    ) initializer public {
        __Ownable_init();
        __Multisignable_init(owners);
        __MultisigTransferOwnership_init(uint8(Operation.TRANSFER_OWNERSHIP), signCount, owner);
        __MultisigUpgradeable_init(uint8(Operation.UPGRADE), signCount);
        __MultisigOwnersManageable_init(uint8(Operation.MANAGE_MULTISIG_ADDRESS), signCount);
        __MultisigClientAccountFactory_init(treasureAccount);
        __MultisigTransferTokenManageable_init(uint8(Operation.MANAGE_TRANSFER_TOKENS), signCount, transferableTokens);
    }

    /**
     * @dev Send Eth amount to the treasure.
     */
    function withdraw(uint256 amount) external override
        onlyOwner {

        AddressUpgradeable.sendValue(payable(_treasureAccount), amount);
        emit Withdrew(_treasureAccount, amount);
    }

    /**
     * @dev Send token amount to the treasure.
     */
    function withdrawTokens(address token, uint256 amount) external override
        onlyOwner {

        SafeERC20Upgradeable.safeTransfer(IERC20Upgradeable(token), _treasureAccount, amount);
        emit WithdrewTokens(token, _treasureAccount, amount);
    }

    /**
     * @dev Send allowed token amount to any address.
     */
    function transferTokens(address token, address to, uint256 amount) external override
        onlyOwner
        checkTokenTransferable(token) {

        SafeERC20Upgradeable.safeTransfer(IERC20Upgradeable(token), to, amount);
        emit TransferredTokens(token, to, amount);
    }
