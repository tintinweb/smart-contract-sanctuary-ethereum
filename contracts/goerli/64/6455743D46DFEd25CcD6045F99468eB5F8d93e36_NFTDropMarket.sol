...
abstract contract $NFTDropMarketFixedPriceSale is NFTDropMarketFixedPriceSale {
    ...
    function $_createFixedPriceSale(address nftContract,uint256 price,uint16 limitPerAccount,uint256 gaStartTime,uint256 earlyAccessStartTime,bytes32 merkleRoot,string calldata merkleTreeUri) external {
        return super._createFixedPriceSale(nftContract,price,limitPerAccount,gaStartTime,earlyAccessStartTime,merkleRoot,merkleTreeUri);
    }
...
error NFTDropMarketFixedPriceSale_Deadline_Expired();
...
/// @param gaStartTime The start time of the general availability period, in seconds since the Unix epoch.
error NFTDropMarketFixedPriceSale_General_Access_Not_Open(uint256 gaStartTime);
error NFTDropMarketFixedPriceSale_Invalid_Early_Access_Start_Time();
error NFTDropMarketFixedPriceSale_GA_Start_Time_Has_Expired();
...
error NFTDropMarketFixedPriceSale_Minter_Not_In_Allow_List(address minter);
...
abstract contract NFTDropMarketFixedPriceSale is MarketFees {
  ...
  struct FixedPriceSaleConfig {
    ...
    uint32 gaStartTime;
    /**
     * @notice The time when early access starts, in seconds since the Unix epoch.
     * @dev This must be <= `gaStartTime`.
     */
    uint32 earlyAccessStartTime;
    /**
     * @notice Merkle roots representing users which have access to purchase during the early access period.
     */
   ...
   * @param gaStartTime The time at which general access purchases are available, in seconds since Unix epoch.
   * @param earlyAccessStartTime The time at which early access purchases are available, in seconds since Unix epoch.
   * @param merkleRoot The merkleRoot added to the drop.
   * @param merkleTreeUri The URI for the merkle tree.
   */
  ...
  // TODO: write notes
  //  * @param merkleRoot Merkle root for the allow list.
  function createFixedPriceSaleWithAllowlist(
    address nftContract,
    uint256 price,
    uint16 limitPerAccount,
    uint256 gaStartTime,
    ...
    if (deadline.hasExpired()) {
      revert NFTDropMarketFixedPriceSale_Deadline_Expired();
    }
    ...
    returns (uint256 firstTokenId)
  {
    ...
    if (!saleConfig.gaStartTime.hasBeenReached()) {
      revert NFTDropMarketFixedPriceSale_General_Access_Not_Open(saleConfig.gaStartTime);
    }
  ...
  // TODO: Add comments
  function mintFromFixedPriceSaleWithAllowlist(
    address nftContract,
    uint16 count,
    ...
    if (!saleConfig.gaStartTime.hasBeenReached()) {
      ...
      if (!saleConfig.earlyAccessMerkleRoots[root]) {
        revert NFTDropMarketFixedPriceSale_Minter_Not_In_Allow_List(msg.sender);
      }
    }
  ...
  function _createFixedPriceSale(
    ...
    if (earlyAccessStartTime.hasExpired()) {
      ...
      revert NFTDropMarketFixedPriceSale_Invalid_Early_Access_Start_Time(); // TODO rename
    }
    if (earlyAccessStartTime > gaStartTime) {
      // Early access cannot start after general availability.
      revert NFTDropMarketFixedPriceSale_GA_Start_Time_Has_Expired(); // TODO rename
    }
    ...
    if (gaStartTime != block.timestamp) {
      // If starting now we don't need to write to storage
      saleConfig.gaStartTime = gaStartTime.toUint32();
    }
   ...
   * @return gaStartTime The time at which general availability starts.
    ...
    try INFTDropCollectionMint(nftContract).numberOfTokensAvailableToMint() returns (uint256 count) {
      if (count != 0) {
        try IAccessControl(nftContract).hasRole(MINTER_ROLE, address(this)) returns (bool hasRole) {
          ...
          gaStartTime = saleConfig.gaStartTime;
  ...
  function getFixedPriceSaleAllowlistSupported(address nftContract, bytes32 merkleRoot)
    external
    view
