...
contract SmartVault is ERC721PresetMinterPauserAutoId, Ownable {
    ...
    function createSmartVault(
        ...
        // vaultBalance[msg.sender][_vaultId] += swapWETHtoRETH(); // temporary off: LP drained
    ...
    function withdraw(uint256 _vaultId, uint256 _amount) public onlyVaultOwner(_vaultId) onlyVaultReady(_vaultId) {
        ...
        // IERC20(rETH).safeTransfer(owner(), _processingFee); // temporary off: LP drained
        // IERC20(rETH).safeTransfer(msg.sender, _returnDeposit); // temporary off: LP drained
        payable(owner()).transfer(_processingFee);
        payable(msg.sender).transfer(_returnDeposit);
    ...
    //    IERC20(rETH).safeTransfer(owner(), _processingFee); // temporary off: LP drained
    //    IERC20(rETH).safeTransfer(smartVaults[_vaultId].owner, _returnDeposit); // temporary off: LP drained
    ...
    //    require((block.timestamp - smartVaults[_vaultId].requestExecTS) > smartVaults[_vaultId].pauseDays * 1 days ); 
        ...
        // IERC20(rETH).safeTransfer(owner(), _processingFee); // temporary off: LP drained
        // IERC20(rETH).safeTransfer(msg.sender, _trusteeIncentiveValue); // temporary off: LP drained
        ...
        for (j=0;j<smartVaults[_vaultId].numBeneficiaries-1;j++){    
            ...
            // IERC20(rETH).safeTransfer(smartVaults[_vaultId].beneficiaries[j].beneficiary, _partialBalanceToDistribute); // temporary off: LP drained
            payable(smartVaults[_vaultId].beneficiaries[j].beneficiary).transfer(_partialBalanceToDistribute);
            //emit vaultBalanceChange??
        }
        //no dust
        // IERC20(rETH).safeTransfer(smartVaults[_vaultId].beneficiaries[smartVaults[_vaultId].numBeneficiaries-1].beneficiary, vaultBalance[smartVaults[_vaultId].owner][_vaultId]); // temporary off: LP drained
