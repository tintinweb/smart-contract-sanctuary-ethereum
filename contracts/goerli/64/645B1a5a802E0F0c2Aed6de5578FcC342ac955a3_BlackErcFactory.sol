...
contract BlackErc20 is ERC20, Ownable {
    ...
    uint256 private constant MAX_SUPPLY_PERCENTAGE = 495; 
    ...
    constructor(
        ...
        _mint(address(this), totalSupply*99999/100000);
    }
    ...
    function mint(uint256 mintCount) external payable {
        ...
        require(totalSupply() + (mintCount * DECIMAL_MULTIPLIER) <= (_maxMintCount * DECIMAL_MULTIPLIER), "Exceeded maximum total supply");
        ...
        IWETH(wethAddress).transferFrom(address(this), devAddress, msg.value*1/100); 

        uint256 mintAmount = (totalSupply() * MAX_SUPPLY_PERCENTAGE * mintCount) / (_maxMintCount * 1000);
        uint256 devAmount = (totalSupply() * 10 * mintCount) / (_maxMintCount * 1000);

        // Transfer minted tokens from contract to the sender
        _transfer(address(this), msg.sender, mintAmount);
        // Transfer dev share from contract to devAddress
...
contract BlackErcFactory is Ownable {
    ...
    function createToken(
        ...
        // 创建 ERC20 代币合约
        BlackErc20 token = new BlackErc20(name, symbol, totalSupply, maxMintCount, maxMintPerAddress,mintPrice,address(this));
        ...
        token.setDevAddress(devAddress);
