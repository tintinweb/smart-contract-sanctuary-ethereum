...
contract BatchNFTs is ERC721AUpgradeable, OwnableUpgradeable, ERC2981 ,DefaultOperatorFilterer,UUPSUpgradeable
{
    ...
    function initialize(string memory name, string memory symbol,IERC20 _USDC) initializerERC721A initializer public {
        ...
        MAX_SUPPLY = 9888;
        baseURI = "ipfs://QmWqqkMf5pAmKeurDXJLT5hfcYtBRnnjPWkHRRg5Lt7TyZ/";
        pausedUSDCMint = true;
        pausedETHMint = true;
    }
    ...
    function changeUSDCPrice(
        uint256[] memory _updatedPrice
    ) public onlyOwner {
    ...
    function changePauseStateUSDC(bool _state) public onlyOwner() {
            pausedUSDCMint = _state;
    }

    function changePauseStateETH(bool _state) public onlyOwner() {
            pausedETHMint = _state;
    }

    function mintNFTwithUSDC(uint256 quantity) public  
    {
        ...
        uint256 cost = _discountNFTPriceUSDC(quantity);
    ...
    function mintNFT(uint256 quantity) payable public  
    {
        require(pausedETHMint == false, "Function Paused");
        require(quantity >0, "Not enough quantity to mint");
        require(totalSupply() + quantity <= MAX_SUPPLY, "Not enough tokens left");
        require(CostETH.length != 0, "Owner didnot set price yet");
        require(msg.value >= _discountNFTPriceETH(quantity), "Not enough ether sent");

        _safeMint(msg.sender, quantity);
    }
     
    function _discountNFTPriceUSDC(uint256 _quantity) view public returns(uint256){
        require(CostUSDC.length != 0, "Owner didnot set price yet");
        uint256 price;
        if(_quantity <= 4){
            price = CostUSDC[0];
    ...
    function _discountNFTPriceETH(uint256 _quantity) view public returns(uint256){
        require(CostETH.length != 0, "Owner didnot set price yet");
        uint256 price;
        if(_quantity <= 4){
            price = CostETH[0];
