...
contract KingKacy is ERC721AUpgradeable, OwnableUpgradeable {
    bytes32 root;

    // Take note of the initializer modifiers.
    // - `initializerERC721A` for `ERC721AUpgradeable`.
    // - `initializer` for OpenZeppelin's `OwnableUpgradeable`.
    function initialize(bytes32 _root) public initializerERC721A initializer {
        __ERC721A_init("KingKacy", "KK");
        __Ownable_init();
        root = _root;
    }

    function mint(uint256 quantity, bytes32[] calldata proof) external payable {
        require(block.timestamp >= 1669240800, "Mint not yet started");
        if (block.timestamp <= 1669251600)
            require(
