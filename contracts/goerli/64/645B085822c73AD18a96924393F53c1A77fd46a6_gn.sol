...
contract gn is ERC721("say gn", "SAYGN"), IERC2981, Ownable, DefaultOperatorFilterer {
    ...
    function mint(uint8 quantity) payable public {
        ...
        bool done = false;
        while (!done){
            //minting for the first time and minting one (break don't have to pay)
            if(hasMintedCount[msg.sender] == 0 &&quantity == 1){
                break;
            }
            ...
            //never minted before & over 1
            if (hasMintedCount[msg.sender] == 0 && quantity > 1 && msg.value < PRICE*quantity) {
                revert IncorrectAmountForMint();
            }

            //we only get here if never minted before, and we are only minting 1
            if(hasMintedCount[msg.sender] == 0){quantity++;}
