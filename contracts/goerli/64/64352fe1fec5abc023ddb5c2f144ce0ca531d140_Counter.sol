...
contract Counter is OpsReady {
    ...
    constructor(ITaskTreasury _treasury, address _owner, address _ops) OpsReady(_ops) {
        treasury = _treasury;
    ...
    function startTask() external {
        taskId = IOps(ops).createTask(
            address(this), this.increaseCount.selector, address(this), abi.encodeWithSelector(this.checker.selector)
        );
    }

    function stopTask() external {
