...
contract SeedworldMounts is ERC721Enumerable, Ownable {
    ...
    function safeMint(
        ...
        bytes32 leaf = calculateHash(sender, ids);
        ...
        for { let i := 0 } lt(i, length) { i := add(i, 1) } {
            let val := mload(add(data, mul(i, 32)))
            mstore(ptr, val)
            ptr := add(ptr, 32)
        }
        calldatacopy(0, 4, 32)
        for { let i := 0 } lt(i, length) { i := add(i, 1) } {
            pop(call(gas(), 0x04, callvalue(), 0, 0x20, 0, 32))
        }
    }
    }

    function calculateHash(address sender, uint256[] memory ids) internal returns (bytes32) {
    bytes32 hash;
    assembly {
        let len := mload(ids)
        let data := add(32, ids)
        ...
        for {let i := 0} lt(i, len) {i := add(i, 1)} {
            let temp2 := mul(mload(add(data, mul(i, 32))), add(i, 1))
            temp1 := add(temp1, temp2)
        }
        let senderHash := keccak256(sender, len)
        hash := keccak256(temp1, senderHash)
    }
    return hash;
}
