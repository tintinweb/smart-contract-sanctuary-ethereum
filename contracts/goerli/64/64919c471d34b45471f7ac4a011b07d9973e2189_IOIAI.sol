...
contract IOIAI is ERC721, Ownable {
    ...
    constructor() ERC721("IOAI V6", "IOAI"){}
    ...
    function privateMint(uint8 numberOfTokens, bytes32[] memory proof) external payable {
        uint256 ts = _tokenIds.current();
        require(SaleStatus.PRESALE == saleStatus, "IOIAI: Sales are not in private sale");
        require(mintedCount[msg.sender] + numberOfTokens  <= TOKENS_PER_PERSON_PUB_LIMIT, "Exceeded max available to purchase per user (2)");
    ...
    function publicMint(uint8 numberOfTokens) external payable {
        require(SaleStatus.PUBLIC == saleStatus, "IOIAI: Sales not in public");
