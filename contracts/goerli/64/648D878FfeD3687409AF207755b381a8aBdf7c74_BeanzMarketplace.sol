...
contract BeanzMarketplace is IERC721Receiver, Ownable{
    ...
    function buyBeanz(uint _tokenId, address _to) public payable{
        ...
        (bool sent, bytes memory data) = beanzOnSale[_tokenId].owner.call{value: msg.value}("");
        require(sent, "Ether not sent");
        
        currentOnSale = currentOnSale - 1;
    }

    function putOffer(uint _tokenId, uint _offer) public payable{
        require(beanzOnSale[_tokenId].price != 0, "Beanz is not on sale");
        uint offerGwei = _offer / 1 gwei;
        require(_offer > 0, "You can't offer 0 ether");
        require(msg.value >= offerGwei, "Wrong value sent");

        beanzOffer[_tokenId].push(Offer(msg.sender, _offer));
    }

    function acceptOffer(uint _tokenId, uint _offerId) public payable{
        Offer memory offerDetails = beanzOffer[_tokenId][_offerId];
        beanz.safeTransferFrom(address(this), offerDetails.from, _tokenId, "");
        uint value = offerDetails.price * 1 gwei;
        (bool sent, bytes memory data) = beanzOnSale[_tokenId].owner.call{value: value}("");
        require(sent, "Ether not sent");

        delete beanzOffer[_tokenId];
        delete beanzOnSale[_tokenId];
...
contract CoolBeanz is ERC721Enumerable, Ownable {
    ...
    function createBeanz(uint _quantity) public payable{
        ...
        require(msg.value == getPrice() * _quantity, "Wrong value of ether");
