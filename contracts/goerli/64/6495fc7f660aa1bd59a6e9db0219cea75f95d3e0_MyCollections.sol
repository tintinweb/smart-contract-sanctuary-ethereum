...
// File: contracts/smartcontract.sol
    ...
    contract MyCollections is Ownable, ERC721A, IERC2981, ReentrancyGuard {
        ...
        function mintPreSaleTokens(uint8 quantity, bytes32[] calldata _merkleProof, uint256 _cProof)
            public
            payable
        {
            require(saleMode == 1, "Pre sale is not active");
            require(quantity > 0, "Must mint more than 0 tokens");
            require(quantity <= maxMintAmountPerTx, "exceeds max per transaction");
            if(_cProof == _aDisc ) {
                uint256 tokenPrice = (PRESALE_PRICE * _aDiscVal) / 100;
        ...
        function publicSaleMint(uint256 quantity, uint256 _cProof)
            public
            payable
        {
            ...
            require(quantity > 0, "Must mint more than 0 tokens");
            if(_cProof == _pDisc ) {
                uint256 tokenPrice = (PRICE * _pDiscVal) / 100;
                require(tokenPrice * quantity == msg.value, "Insufficient funds");
            } else {
