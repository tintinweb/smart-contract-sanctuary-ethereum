...
contract ChainBattles is ERC721URIStorage  {
    ...
    Stats memory attrb = tokenIdToStats[tokenId];
    return attrb;
    }
    ...
    require(_exists(tokenId), "Please use an existing token");
    Stats memory stats = tokenIdToStats[tokenId];
    uint256 speed = stats.level;
    return speed.toString();
    }

    function getLife(uint256 tokenId) public view returns (uint256) {
    require(_exists(tokenId), "Please use an existing token");
    Stats memory stats = tokenIdToStats[tokenId];
    uint256 life = stats.level;
    return life;
    }
    ...
    tokenIdToStats[newItemId] = Stats(1,10,10,100);
    ...
    // uint256 currentLevel = tokenIdToStats[tokenId]
    // tokenIdToStats[tokenId] = currentLevel + 1;
