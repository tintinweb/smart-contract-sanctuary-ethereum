...
contract PixiaAIDistributor is Ownable, ReentrancyGuard {
    ...
    address public contractStaking;
    ...
    constructor () {
        walletRunningCost = address(0x6C0baB1feA347a3008b398d8AeA5669C67d54DDC); // Running Cost Wallet Address
        walletDev = address(0xDE0946D1819B00cc5feeFCA02f1025243Dfb9D9F); // Dev Wallet Address

        contractStaking = address(0x123); // Staking Contract Address

        WETH = 0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2 ;// WETH token address (Mainnet WETH: 0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2 / Testnet WETH: 0xB4FBF271143F4FBf7B91A5ded31805e42b2208d6)
        token = IERC20(0x67675239Fa58c84e75f947c14f566842Dccb69Ae); // Token Address that will be Buy Back And Burn, and, Provide AutoLP and Burn.
        reward = IERC20(0xdAC17F958D2ee523a2206206994597C13D831ec7); // Token Address that will be added to staking pool

        isAdmin[msg.sender] = true; // By default, the deployer wallet will be called Admin.
        isAdmin[address(0xe9bAc2c7d0cbcA263690C5d3FcFEb3f63c7946C5)]; // This address is set as Admin to support the Owner (Owner can remove admin anytime).
    ...
    // Admins can claim any ERC20 token
    ...
    // Any Address can call the trigger Distribution function, caller will be rewarded with x% from the contract balance
    function triggerDistribution() public nonReentrant {
        ...
        if (contractStakingPart > 0) {
            if (address(reward) != address(WETH)) {
                swapETHForTokens(contractStakingPart, address(this), reward);
            }

        if (reward.balanceOf(address(this)) >= minAmount){
            uint256 balance = reward.balanceOf(address(this));
            if (reward.allowance(address(this), contractStaking) < balance) {
                reward.approve(contractStaking, type(uint256).max);
            }

            IStaking(contractStaking).addRewards(balance); // Adding rewards to pool
    ...
    // setter functions //
    function setWallets (address WalletRunningCost, address WalletDev) external {
        require(isAdmin[msg.sender], "only Admin can set new wallets");
        walletRunningCost = WalletRunningCost;
        walletDev = WalletDev;
    }

    function updateStakingContract (address _contractStaking) external {
        require(isAdmin[msg.sender], "only Admin can set new wallets");
        contractStaking = _contractStaking;
    }

    // Update Token address for autoLP and Burn part
    function updateToken (IERC20 _newToken) external {
        require(isAdmin[msg.sender], "only Admin can change");
        token = _newToken;
    }

    // Update Reward Token Address for Staking Contract Distribution
    function setRewardToken (IERC20 _reward) external {
        require(isAdmin[msg.sender], "only Admin can change");
        reward = _reward;
    }

    // Update Thousand Percentage Distribution for each Wallet and Contract
    function updateWalletPercentage (
        ...
        uint256 autoLPBurn,
        uint256 buybackAndBurn) external {
        require(isAdmin[msg.sender], "only Admin  is allowed to change percentage");
    ...
    // Add or remove admins with true or false values
    function updateAdmins (address _newAdmin, bool value) external onlyOwner {
        isAdmin[_newAdmin] = value;
    }

    //getter functions//
    // Returns available ETH to distribute for each wallet
    ...
    // Returns total ETH sent to wallets till date
   ...
   // Returns total ETH sent to Auto LP Burn and Buy Back Burn till date
    ...
    // Returns available ETH to distribute for Auto LP Burn and Buy Back Burn
