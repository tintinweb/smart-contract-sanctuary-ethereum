...
abstract contract LazyMint is ILazyMint, BatchMintMetadata {
    ...
    function lazyMint(
        ...
        string memory _uri = string(abi.encodePacked(
            _baseURIForTokens,
            Strings.toString(startId),
            ".json?alt=media"
        ));

        (nextTokenIdToLazyMint, batchId) = _batchMintMetadata(startId, _amount, _uri);
...
/**
 * @title Crystal Key Contract
 * @author Dversed
 * 
 * @notice Coordinates the creation of NFTs Crystal Keys
 * 
 */

contract CrystalKeyContract is ERC721LazyMint {

    /**
     * @dev custom variables for extra contract functions
     */

    // claim price
    uint256 public price = 36 * 10**15;

    // storage URI
    string public metaUri = "https://firebasestorage.googleapis.com/v0/b/dversed-8a864.appspot.com/o/metadata%2F";

    /** Custom Event */
    event WithDraw(address indexed _wallet, uint256 _withDraw);
    ...
    constructor(
        ...
        address _royaltyRecipient,
        uint128 _royaltyBps
    ) payable ERC721LazyMint(_name, _symbol, _royaltyRecipient, _royaltyBps) {}

    /**
     * @notice internal function for receiving balance in the contract
     */
    function deposit(uint256 _value) internal {}

    /**
     * @notice override of the _transferTokensOnClaim function to allow claim NFT
     */
    function _transferTokensOnClaim(address _receiver, uint256 _quantity) internal override returns (uint256 startTokenId) {
        ...
        require(msg.value >= totalPrice, "You need to transfer the price");
        deposit(totalPrice);
    ...
    /**
     * @notice function to allow withdrawal of the contract balance by the owner
     */
    function withdraw() public { 
        if (!_canSetOwner()) {
            revert("Not authorized");
        }// Allow the owner of the contract to withdraw balance
    ...
    /**
     * @notice override of the tokenURI function to allow URI Storage
     */
    function tokenURI(uint256 _tokenId) public view virtual override returns (string memory) {
        _getBaseURI(_tokenId);
    ...
    /**
     * @notice function where the owner of the contract can update the claim price
     * @param _price new price Ex: 20 == 0.02
     * @param _decimals new decimals for calculate price Ex: 15 = == 0.02
     */
    function setPrice(uint256 _price, uint256 _decimals) public {
        if (!_canSetOwner()) {
            revert("Not authorized");
        }
        price = _price * 10**_decimals;
    }

    /**
     * @notice function where the owner of the contract can update the metaURI link
     * @param _uri new link
     */
    function setMetaLink(string memory _uri) public {
        if (!_canSetOwner()) {
            revert("Not authorized");
        }
       metaUri = _uri;
