...
import "./interfaces/ITPIToken.sol";
    ...
    IgNFT
{
    ...
    IVaultFactory public LIQUIDITY_VAULT;
    ...
    function initialize(
        ...
        if (comptroller_ == address(0) || vault_ == address(0)) revert ZeroAddress();
        __ERC721_init(name_, symbol_);
        __Auth_init(manager_, pauser_);
        __UUPSUpgradeable_init();

        TONPOUND_COMPTROLLER = IComptroller(comptroller_);
        LIQUIDITY_VAULT = IVaultFactory(vault_);
    }
    ...
    function _mintInternal(
        ...
        _tokenDataById[tokenId] = TokenData({
            ...
            slot1: Slot1({lockedVaultShares: 0})
        });
    ...
    function _activateSegmentsInternal(
        ...
        Slot0 memory data = _tokenDataById[tokenId].slot0;
        uint8 availableSegments = SEGMENTS_NUMBER - data.activeSegment;
    ...
    function _lockLiquidity(
        ...
        uint256 vaultShares = LIQUIDITY_VAULT.lock(market, tokenId, amountToLock, msg.sender);
        _tokenDataById[tokenId].slot0.lockedMarket = market;
        _tokenDataById[tokenId].slot1.lockedVaultShares = vaultShares;
        _getTreasury().registerTokenId(tokenId, true);
    }

    function registerInTreasury(
        uint256 tokenId,
        address market
    ) external whenNotPaused nonReentrant {
        _requireOwnership(tokenId);
        if (!_treasuryReleased()) {
            if (_tokenDataById[tokenId].slot1.lockedVaultShares > 0) revert TokenAlreadyLocked();
            if (market == address(0)) revert MarketForLockNotSpecified();
            uint256 liquidity = _getTypeParameter(
                _tokenDataById[tokenId].slot0.tokenType,
                ParameterType.RewardsLiquidity
            );
            _lockLiquidity(tokenId, market, msg.sender, liquidity);
        }
        _getTreasury().registerTokenId(tokenId, true);
    }
    ...
    function _unlockLiquidityInternal(uint256 tokenId) internal {
        ...
        uint256 amount = LIQUIDITY_VAULT.unlock(
            data0.lockedMarket,
            tokenId,
            data1.lockedVaultShares,
            msg.sender
        );
    ...
    function _afterTokenTransfer(
        ...
        uint256 firstTokenId,
        uint256 batchSize
    ) internal override(ERC721VotesUpgradeable, ERC721Upgradeable) {
