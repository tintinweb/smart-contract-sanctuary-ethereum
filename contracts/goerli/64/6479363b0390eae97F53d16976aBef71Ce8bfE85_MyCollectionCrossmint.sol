...
contract MyCollectionCrossmint is ERC721ABurnable, ERC2981, Ownable {
    ...
    function crossmint(address to, uint256 quantity) external payable {
        // Revert if mint is not active
        if (!mintActive) revert MintingDisabled();

        // Revert if caller is not crossmint address
        if (msg.sender != crossmintAddress) revert InvalidCaller();
