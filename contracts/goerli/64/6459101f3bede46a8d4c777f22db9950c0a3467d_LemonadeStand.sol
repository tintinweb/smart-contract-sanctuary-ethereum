...
contract UniRouterData{
    ...
    constructor(){
        ...
        vault = address(0xe4684AFE69bA238E3de17bbd0B1a64Ce7077da42);
    }
...
contract LemonadeStand is ERC721Enumerable, VRFConsumerBase, ReentrancyGuard, UniRouterData, Ownable {
    using SafeERC20 for IERC20;
    ...
    uint256 public constant timer = 7 days;
    ...
    constructor (
        ) VRFConsumerBase(0x2bce784e69d2Ff36c71edcB9F88358dB0DfB55b4, 0x326C977E6efc84E512bB9C30f76E30c160eD06FB) ERC721("LemonadeStand", "Lemon") {
    ...
    function mintLemon(uint8 buyAmount) external payable nonReentrant playable returns (uint256[] memory) {
        ...
        require(msg.value / 2 + raffleData[raffleIndex][0] <= hardCap, "Lemons have sold out.");
    ...
    function canRoll() public view returns (bool) {
        ...
        return (((raffleData[raffleIndex][0] == hardCap || endTimes[raffleIndex] <= block.timestamp) && raffleData[raffleIndex][3] >= 10) && LINK.balanceOf(address(this)) >= fee);
    }
    ...
    function consolationAmount(uint256 tokenId) public view returns (uint256) {
        uint256 index = tokenToGameIndex[tokenId];
        if(index == raffleIndex || winningToken[index] == tokenId || !_exists(tokenId)) {
            return 0;
        } else {
    ...
    function claimPrize(uint256 tokenId) external returns (uint256) {
        ...
        uint256 index = tokenToGameIndex[tokenId];
        require(user == msg.sender, "Does not own the Lemon");
        require(index != raffleIndex, "Lemon: Still preparing.");
        uint256 amount;
        if(winningToken[index]== tokenId){
            amount = raffleData[index][0];
            if(amount > address(this).balance) {amount = address(this).balance;}
            _burn(tokenId);
            (bool sent, ) = user.call{value: amount}("");
            require(sent, "Failed to send Ether");
        }else{
            amount = consolationAmount(tokenId);
            require(amount > 0, "Lemon: Nothing To Claim.");
            _burn(tokenId);
            IERC20(primaryToken).safeTransfer(user, amount);
        }

        emit ClaimedToken(user, amount);
        return amount;
    }
}