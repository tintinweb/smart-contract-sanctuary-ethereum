 ...
 */
contract CollectionFactory is Withdrawable, ERC721A{
     ...
     */
    uint256 private s_marketplace_percentage = 25; // 2.5%
    ...
    constructor(address _s_payment_token_address)ERC721A("FSA Marketplace", "FMA"){
       s_signer = msg.sender;
    ...
    */
    function buyMultipleArt(
      ...
      if(msg.sender == _artDetails.buyer){
        ...
        mintMultiple(_artDetails.tokenURI, _artDetails.buyer, currentTokenSupply, _artDetails.amount);
    ...
    */
    function mintMultiple(string memory _tokenURI, address _buyer, uint256 _currentTokenSupply, uint256 _amount) internal {
    ...
    */
    function verifyParamsForAuction(AuctionDetails memory _auctionDetails, bytes calldata _signature) internal view returns(bool) {
       ...
       string memory concatenatedParams = SignerVerification.concatParams(
            _auctionDetails.auctionID,
            _auctionDetails.price,
            _auctionDetails.buyer,
             _auctionDetails.artist,
    ...
    */
    ...
    function verifyMessage(string memory _concatenatedParams, bytes calldata _signature) internal view returns(bool){
         return SignerVerification.isMessageVerified(s_signer, _signature, _concatenatedParams);
    }
...
library SignerVerification {
    ...
    function concatParams(
        ...
        string memory totalSupply = _totalSupply == 0 ? "":Strings.toString(_totalSupply);
...
interface ICollection {
   /**
     * @notice Mints NFT for specific auction
     * @param _auctionId Auciton ID :)
     * @param _tokenURI TokenURI
     * @param _buyer Address of buyer
   */
    function mint(uint256 _auctionId, string memory _tokenURI, address _buyer) external;
