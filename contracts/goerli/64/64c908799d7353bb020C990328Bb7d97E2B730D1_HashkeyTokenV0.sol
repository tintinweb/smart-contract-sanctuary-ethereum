...
abstract contract ERC20 is Access, IERC20 {
    ...
    function mintToPartner(
        ...
        if (evenVestingAmount > 0) {
            ...
            emit MintEvenVestFund(recipient, deadline, end, amount);
        }
    }
    ...
    function mintToTeamMember(address recipient, uint256 amount) public accessible(TEAM_ROLE) {
        ...
        emit MintVestFund(recipient, deadline, vestAmount);
        
        uint256 evenVestAmount = amount.sub(vestAmount);
        uint256 end = deadline.add(2 * PER_YEAR);
        _mintEvenVestFund(state, Fund.OwnerType.Team, evenVestAmount, deadline, end);
        emit MintEvenVestFund(recipient, deadline, end, evenVestAmount);
    }
