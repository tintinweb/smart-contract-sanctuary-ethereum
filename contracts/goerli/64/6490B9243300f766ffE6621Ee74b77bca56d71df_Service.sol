...
import "./interfaces/IGnosisGovernance.sol";
    ...
    IService
{
    ...
    function createPool(
        ...
        if (address(pool) == address(0)) {
            ...
            pool.setGnosisGovernance(
                address(new BeaconProxy(gnosisGovernanceBeacon, ""))
            );

            IGnosisGovernance(pool.gnosisGovernance()).initialize(
                address(pool)
            );
        ...
        returns (address proxy)
    {
        ...
        address z = IGnosisSafeProxyFactory(gnosisProxyFactory)
