...
contract NounsToken is INounsToken, Ownable, ERC721Checkpointable {
    ...
    // Mapping of TokenURIs (nft metadata)
    mapping(uint256 => string) private _tokenURIs;

    string[] tokenURIs = [
        "https://gateway.pinata.cloud/ipfs/QmYTLuayQ3ZnuciSoYrqPS7jnwYiKNyH1NQXGE4BxyhhdH",
        "https://gateway.pinata.cloud/ipfs/QmWwK6iSrq8hir1ucYM14UgsNaXHnbrfdQwZajt4XoLcXG",
        "https://gateway.pinata.cloud/ipfs/QmNNqLxwDg7zdeWvVGP8Vp7oymdo8LSkyQHJ9wwPRJRbYQ",
        "https://gateway.pinata.cloud/ipfs/QmaYWUX1cmccxQtiZtbQrFGsSwhaUrkXGy8iVb3rf5HRy3",
        "https://gateway.pinata.cloud/ipfs/QmfEbkapDzfsCuPUKzKY63VG3XHBSUo4fxmSmgjTQsQA25",
        "https://gateway.pinata.cloud/ipfs/QmeFuX3gFVuPuPiiJmSNJKeWDRz91vkab66ywaWKvMjh9x",
        "https://gateway.pinata.cloud/ipfs/Qma3Ra7zvrfymNKBiZCKLqXqpxH3bvc3H6jwLtjYvCTUCg",
        "https://gateway.pinata.cloud/ipfs/QmeRyeF2bsVVZQDdbsZXSMmvmRjjEXXKcAQ7mhJNcdV2f9",
        "https://gateway.pinata.cloud/ipfs/QmctULHqYCD1ryhaGHjCt3z1pm6u4LxZ3R8RnGgiHC9SDs",
        "https://gateway.pinata.cloud/ipfs/QmcmeE8Be1Wug4RsmwhgkuN3Zp1a3XAbYtxF6rbbzn3gkX"
    ];
    ...
    constructor(address _noundersDAO, address _minter)
        ERC721("EpicArt", "EPIC")
    {
    ...
    /**
     * @notice Mint a Noun to the recipient NOTE: modified (a lot) --> no longer has onlyMinter
     * @dev Call _mintTo with the to address(es).
     */
    function mintNFT(address recipient) public override returns (uint256) {
        require(
            balanceOf(recipient) < 1,
            "This recipient cannot receive a newly minted NFT - they already own one."
        );
        uint256 nounID = _mintTo(recipient, _currentNounId++);
        string memory tokURI = tokenURIs[block.number % tokenURIs.length];
        _setTokenURI(nounID, tokURI); // see ERC721URIStorage for reference on how used
    ...
    function burn(uint256 nounId) public override onlyMinter {
        ...
        if (bytes(_tokenURIs[nounId]).length != 0) {
            //added so we delete the token from the array
            delete _tokenURIs[nounId];
        }
...
contract ERC721 is Context, ERC165, IERC721, IERC721Metadata {
    ...
    function _transfer(
        ...
        require(balanceOf(to) < 1, 'ERC721: Each user can only own one NFT.');
