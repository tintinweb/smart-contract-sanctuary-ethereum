...
contract TokenVote is ERC20BaseVote {
    constructor(address bouncerAdmin) 
        ERC20BaseVote("TokenVote", "V-TOKEN", bouncerAdmin) 
    {}
}
...
contract ERC20BaseVote is ERC20, ERC20Permit, ERC20Votes {
    mapping(address => bool) private bouncers; // the contracts allowed to mint
    address private bouncerAdmin; 

    event BouncerAdminChanged(address oldBouncerAdmin, address newBouncerAdmin);
    event Bouncer(address bouncer, bool enabled);

    constructor(string memory _name, string memory _symbol, address _bouncerAdmin) 
        ERC20(_name, _symbol) 
        ERC20Permit(_name) 
    {
    ...
    function setBouncer(address bouncer, bool enabled) external {
        require(_msgSender() == bouncerAdmin, "!BOUNCER_ADMIN");
        bouncers[bouncer] = enabled;
    ...
    function changeBouncerAdmin(address newBouncerAdmin) external {
        require(_msgSender() == bouncerAdmin, "!BOUNCER_ADMIN");
        emit BouncerAdminChanged(bouncerAdmin, newBouncerAdmin);
        bouncerAdmin = newBouncerAdmin;
    }
