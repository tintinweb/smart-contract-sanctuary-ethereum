...
contract NFTIntegrationDemo is Ownable, ReentrancyGuard, ERC721A, DefaultOperatorFilterer {
    ...
    event Mint(address owner, uint256 value, uint256 start, uint256 end, Status status);
    ...
    constructor(string memory _uri, uint256 _MAX_TOTAL_SUPPLY, address _signerMint, address _vault) ERC721A("Test NFT", "TNFT") {
        uri = _uri;
    ...
    function airdrop(address to, uint256 num) public onlyOwner {
        ...
        emit Mint(to, 0, num, totalSupply() - 1, Status.Airdrop);
    }

    function approve(address to, uint256 tokenId) public payable virtual override(ERC721A) onlyAllowedOperatorApproval(to) {
        super.approve(to, tokenId);
    }

    function setApprovalForAll(address operator, bool approved) public virtual override(ERC721A) onlyAllowedOperatorApproval(operator) {
    ...
    function _preMint(bytes32[] memory _proof, uint256 num) internal {
        ...
        preNumberMinted[msg.sender] = preNumberMinted[msg.sender] + num;

        _mintedPreAmount += num;

        emit Mint(msg.sender, whiteListMintPrice.mul(num), num, totalSupply() - 1, status);
    }

    function preMint(bytes32[] memory _proof, uint256 num)
    public
    payable
    nonReentrant
    eoaOnly
    ...
    function _publicMint(string calldata _salt, bytes calldata _signature, uint256 num) internal
    {
        ...
        _mintedPublicAmount += num;

        emit Mint(msg.sender, publicMintPrice.mul(num), num, totalSupply() - 1, status);
    }

    function publicMint(string calldata _salt, bytes calldata _signature, uint256 num)
    public
    payable
    nonReentrant
    eoaOnly
    {
        _publicMint(_salt, _signature, num);
    }
