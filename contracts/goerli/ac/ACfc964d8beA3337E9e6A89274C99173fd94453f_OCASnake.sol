...
// @@@@@@@@@@@@#55B&&&&&&@@@@@@@&&G5PPPPP[email protected]@@[email protected]@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@//
...
contract OCASnake is Ownable, ERC721A, ERC2981, ReentrancyGuard, OCASnakeArt {
    ...
    string[20] internal snakeSpecies = [
        "_Python",
        "",
        "_Cobra",
        "",
    ...
    function recursiveCompareArrayCreation(
        ...
        array[index % 4] = snakeNameIndexCompare * adjectiveIndexCompare;
        if (array[0] == 0 || array[1] == 0 || array[2] == 0 || array[3] == 0) {
            return this.recursiveCompareArrayCreation(index + 1, seed, array);
        } else {
            return array;
        }
    }
    ...
    function roshambo(
        ...
        uint256 youThrow = generateAppleSeed(you, seed + 69) % 3;
        uint256 opponentThrow = generateAppleSeed(opponent, seed + 69) % 3;
        if (youThrow > opponentThrow) return true;
        else if (youThrow == opponentThrow)
            return this.roshambo(you, opponent, seed + 420);
        else return false;
        // if (youThrow == 0) {
        //     if (opponentThrow == 0) {
        //         return this.roshambo(you, opponent, seed + 420);
        ...
        //         return false;
        //     } else if (opponentThrow == 1) {
        //         return this.roshambo(you, opponent, seed + 69);
        //     } else {
        //         return true;
        //     }
        // } else {
        ...
        //         return true;
        //     } else {
        //         return this.roshambo(you, opponent, seed + 1337);
        //     }
        // }
    }
    ...
    function recursiveNameFind(
        ...
        if (resetTokens) {
            ...
            uint256 adjectiveIndex = (generateAppleSeed(tokenId, seed) %
                wackyAdjectives.length);
            uint256 speciesName = (generateAppleSeed(tokenId, seed) %
                snakeSpecies.length);
        ...
        uint256[4] memory compareArray = this.recursiveCompareArrayCreation(
            index - 1,
            seed,
            arr
        );
        ...
        if (
            compareArray[0] == indexCompare ||
            compareArray[1] == indexCompare ||
            compareArray[2] == indexCompare ||
            compareArray[3] == indexCompare
        ) {
            found = true;
            // break;
        }
        bool change = this.roshambo(tokenId, index, seed);
        if ((index < 1998 && !found) || tokenId == index) {
            ...
            uint256 supremeIndex = (generateAppleSeed(tokenId, seed) %
                supremeAdjectives.length);
            snakeNameIndexes[0] = generateAppleSeed(tokenId, seed + 420) % 17;
            snakeNameIndexes[1] = generateAppleSeed(tokenId, seed + 69) % 250;
        ...
        returns (bytes memory)
    {
        ...
        return
            abi.encodePacked(
                '{"name": "',
                recursiveNameFind(tokenId, 0, 0, true, "", arr),
