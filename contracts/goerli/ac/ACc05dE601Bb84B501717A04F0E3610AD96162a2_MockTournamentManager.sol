 ...
 * 
 **/
contract ChainlinkRandomInterval is VRFConsumerBaseV2 {
    ...
    // TODO: can probably remove this to avoid writing every update to storage
    mapping(uint256 => mapping(uint256 => uint256)) public epochs; // id => epoch => value
	
    event RequestSent(uint256 requestId, uint32 numWords);
    event RequestFulfilled(uint256 requestId, uint256[] randomWords);

    constructor(uint64 _subscriptionId, address coordinatorAddress) VRFConsumerBaseV2(coordinatorAddress) {
...
contract MockTournamentManager is TournamentManager {
	...
	function mockStartRound() public {
		startRound();
	}

	// end round without ending tournament for testing
	// will not automatically start next round
	// allows measurement of game state in between rounds
	function mockEndRound() public {
		...
		// additional logic to stop VRF
		isActive = false;
		intervalId++;
	}

	// @dev mock start tournament without starting first round
	// allows measurement of game state before first round starts
	function mockStartTournament() external onlyManager {
	...
	// mock end tournament 
	function mockEndTournament() public {
		endTournament();
	}
}
...
import { ChainlinkRandomInterval } from "./ChainlinkRandomInterval.sol";
import { IERC20 } from "@openzeppelin/contracts/token/ERC20/IERC20.sol";
 ...
 * 
 **/
contract TournamentManager is ChainlinkRandomInterval {
	...
	uint256 public fee; // Basis points: 10,000 = 100%
	...
	function setTokenImplementation(address newImplementation) onlyOwner external {
		tokenImplementation = newImplementation;
		emit TokenImplementationChanged(newImplementation);
	}

	// @dev Set fee
	function setFee(uint256 newFee) external onlyOwner {
		require(newFee <= 10000, "setFee: new fee is above maximum");
		fee = newFee;
	...
	// @def Triggered by Chainlink VRF
	...
	// TODO: calculate amountAMin, amountBMin, amountOutMin correctly (avoid slippage)
	...
	// @dev Remove liquidity for winning token
	// Calculate reward
	function resolveTournament() internal {
		...
		// TODO: might have unnecessary precision
		uint256 reward =
		(
	...
	// Add uniswap liquidity for game tokens
    ...
    // View function to get spot price of token in USDC using reserves
    function getSpotPriceAndPair(address token) public view returns (uint256 spotPrice, IUniswapV2PairCustomFee) {
    	// could save gas by returning price and pair
    ...
    function getTokenById(uint8 tokenId) public view returns (address) {
    	require(tokenId < 8, "getTokenById: id out of bounds");
    	return tournaments[id].tokens[tokenId];
    }
