...
/**
 * @title MintableToken
 * @dev minting is under whitelist
 * @dev users in whitelist have rules that allow or restrict minting
 */
contract MintableToken is Ownable, IERC721, IERC721Metadata, ERC721Burnable, ERC721Base {

    /*  Users can be allowed to mint up tp 4.294.967.295 NFT with one credit batch
        0xFFFFFFFF credits (-1) means infinite minting
     */
    mapping (address => uint32) public  Credits;
    uint    public                      MintCost;
    bool    public                      UseCredits;
    ...
    constructor (
        ...
        Credits[newOwner] = 0xFFFFFFFF;
        MintCost = mintCost;
        UseCredits = useCredits;
    }

    function setCredits(address _add, uint32 _cr) public onlyOwner {
        Credits[_add] = _cr;
    }

    function setMintCost(uint _mc) public onlyOwner {
        MintCost = _mc;
    }

    function enableCredits(bool _c) public onlyOwner {
        UseCredits = _c;
    }

    function mint(uint256 tokenId, Fee[] memory _fees, string memory tokenURI, address to) public payable {
        require(msg.value >= MintCost, 'No minting cost');

        if ( UseCredits ) {
            require(Credits[to]>0, 'No Credits');
            if (Credits[to] != 0xFFFFFFFF) {
                Credits[to] -= 1;
            }
        }
