...
contract Guns is ERC721AQueryable, ERC721ABurnable, Ownable {
    ...
    constructor() ERC721A("Guns", "gu") {
    }

    event Mint(address to, uint256 id);

    function mint(uint id, bytes calldata signature)  external payable {
        require( _isMessageValid(signature,msg.sender,id) == _signerPublicKey , "invalid signature");
        require(isSalesActive(), "sales is not active");

        _totalMintPerAccount[msg.sender] += 1;
    ...
    function _isMessageValid(bytes memory _signature, address account, uint id) public view returns (address){
        bytes32 messageHash = keccak256(abi.encodePacked(address(this),account, _totalMintPerAccount[account],id));
