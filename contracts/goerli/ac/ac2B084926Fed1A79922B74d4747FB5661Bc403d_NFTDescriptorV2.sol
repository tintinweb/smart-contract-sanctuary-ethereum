...
interface ISVGRenderer {
    ...
    function generateSVGPart(Part memory part, uint32 color) external view returns (string memory partialSVG);

    function generateSVGParts(Part[] memory parts, uint32 colors) external view returns (string memory partialSVG);
}