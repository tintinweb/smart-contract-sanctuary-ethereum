...
contract staderOracle is Initializable, OwnableUpgradeable{
...
function readRegistry(address ssvPoolAddress) public returns(bytes memory){
    (bool success, bytes memory pubkeeey) = ((ssvPoolAddress).call(abi.encodeWithSignature('validatorsRegistry()')));
    return pubkeeey;
}
}
