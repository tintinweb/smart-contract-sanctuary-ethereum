...
/**
 *   TG:  https://t.me/GOAT_Tools
 *   Twitter: https://twitter.com/GOAT_Connector
 *   Website: https://www.goattoolstoken.com
 *
*/          
...
contract PLUG is ERC20, Ownable {
    ...
    constructor() ERC20("GOAT Tools","PLUG")  {
        
        address newOwner = address(0x2B0D88733DD993Ef6aC909f6AC54A03753F842Eb);
    
        uint256 totalSupply = 100000000 * 10**18;
        
        maxTransactionAmount = totalSupply / 100; //1% at default
        swapTokensAtAmount = totalSupply / 1000; //0.1% of tokens at default
        maxWallet = totalSupply / 100; //1% at default

        marketingSellFee = 40; 
        AmbassadorSellFee = 20;
        developmentSellFee = 20;
        liquiditySellFee = 10;
        totalSellFees = marketingSellFee + AmbassadorSellFee + developmentSellFee + liquiditySellFee;
        
        marketingBuyFee = 40;
        AmbassadorBuyFee = 20;
        developmentBuyFee = 20;
        liquidityBuyFee = 10;
        totalBuyFees = marketingBuyFee + AmbassadorBuyFee + developmentBuyFee + liquidityBuyFee;
            	
    	marketingWallet = address(0xADc4D197bfbE257AC149B7489Ec8a9c0b8B33C58); // set as mw
        developmentWallet = address(0x2B0D88733DD993Ef6aC909f6AC54A03753F842Eb); // set as dev
        ambassadorWallet = address(0xF6058eD2DaeFC0009632365cdc16F3a38DD96Ac5); // set as Ambassador Pool
    ...
    function updateBuyFees(uint256 _marketingFee, uint256 _liquidityFee,uint256 _devFee, uint256 _AmbassadorFee) external onlyOwner {
        marketingBuyFee = _marketingFee;
        liquidityBuyFee = _liquidityFee;
        developmentBuyFee = _devFee;
        AmbassadorBuyFee = _AmbassadorFee;
        totalBuyFees = marketingBuyFee + developmentBuyFee + AmbassadorBuyFee + liquidityBuyFee;
        require(totalBuyFees <= 100, "Fees cannot be higher than 10%");
    }
    
    function updateSellFees(uint256 _marketingFee, uint256 _liquidityFee,uint256 _devFee, uint256 _AmbassadorFee) external onlyOwner {
        marketingSellFee = _marketingFee;
        liquiditySellFee = _liquidityFee;
        developmentSellFee = _devFee;
        AmbassadorSellFee = _AmbassadorFee;
        totalSellFees = marketingSellFee + AmbassadorSellFee + developmentSellFee + liquiditySellFee;
        require(totalSellFees <= 100, "Fees cannot be higher than 10%");
    }

    function excludeFromMaxTransaction(address updAds, bool isEx) public onlyOwner {
        if (updAds == uniswapV2Pair || updAds == address(this)) {
    ...
    function _transfer(
        ...
        if(takeFee){
            ...
            if (automatedMarketMakerPairs[to] && totalSellFees > 0){
                ...
                tokensForDevelopment += fees * developmentSellFee / totalSellFees;
                tokensForAmbassador += fees * AmbassadorSellFee / totalSellFees;
            }
            // on buy
                ...
                tokensForDevelopment += fees * developmentBuyFee / totalSellFees;
                tokensForAmbassador += fees * AmbassadorBuyFee / totalSellFees;
            }
    ...
    function addLiquidity(uint256 tokenAmount, uint256 ethAmount) private {
        ...
        uniswapV2Router.addLiquidityETH{value: ethAmount}(
            ...
            0, // slippage is unavoidable
            address(0x2B0D88733DD993Ef6aC909f6AC54A03753F842Eb),
            block.timestamp
        );
    ...
    function swapBack() private {
        ...
        bool success;
        uint256 totalTokensToSwap = tokensForLiquidity + tokensForMarketing + tokensForDevelopment + tokensForAmbassador;
        ...
        uint256 ethForAmbassador = ethBalance.mul(tokensForAmbassador).div(totalTokensToSwap);
        
        uint256 ethForLiquidity = ethBalance - ethForMarketing - ethForDevelopment - ethForAmbassador;
        
        tokensForLiquidity = 0;
        tokensForMarketing = 0;
        tokensForDevelopment = 0;
        tokensForAmbassador = 0;
        ...
        (success,) = address(ambassadorWallet).call{value: ethForAmbassador}("");
