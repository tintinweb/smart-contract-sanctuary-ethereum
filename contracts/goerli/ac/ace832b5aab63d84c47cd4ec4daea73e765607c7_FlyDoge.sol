...
// File: contracts/treasury/ITreasuryHandler.sol
...
// File: contracts/tax/ITaxHandler.sol
...
// File: contracts/governance/IGovernanceToken.sol
...
/**
 * @title FlyDoge token contract
 * @dev The FlyDoge token has modular systems for tax and treasury handler as well as governance capabilities.
 */
contract FlyDoge is IERC20, IGovernanceToken, Ownable {
    ...
    function transferFrom(
        ...
        require(
            ...
            "FlyDoge:transferFrom:ALLOWANCE_EXCEEDED: Transfer amount exceeds allowance."
        );
        unchecked {
    ...
    function decreaseAllowance(address spender, uint256 subtractedValue) external returns (bool) {
        ...
        require(
            ...
            "FlyDoge:decreaseAllowance:ALLOWANCE_UNDERFLOW: Subtraction results in sub-zero allowance."
        );
        unchecked {
    ...
    function delegateBySig(
        ...
        require(signatory != address(0), "FlyDoge:delegateBySig:INVALID_SIGNATURE: Received signature was invalid.");
        require(block.timestamp <= expiry, "FlyDoge:delegateBySig:EXPIRED_SIGNATURE: Received signature has expired.");
        require(nonce == nonces[signatory]++, "FlyDoge:delegateBySig:INVALID_NONCE: Received nonce was invalid.");
    ...
    function getVotesAtBlock(address account, uint32 blockNumber) public view returns (uint224) {
        require(
            ...
            "FlyDoge:getVotesAtBlock:FUTURE_BLOCK: Cannot get votes at a block in the future."
        );
    ...
    function _approve(
        ...
        require(owner != address(0), "FlyDoge:_approve:OWNER_ZERO: Cannot approve for the zero address.");
        require(spender != address(0), "FlyDoge:_approve:SPENDER_ZERO: Cannot approve to the zero address.");
    ...
    function _transfer(
        ...
        require(from != address(0), "FlyDoge:_transfer:FROM_ZERO: Cannot transfer from the zero address.");
        require(to != address(0), "FlyDoge:_transfer:TO_ZERO: Cannot transfer to the zero address.");
        require(amount > 0, "FlyDoge:_transfer:ZERO_AMOUNT: Transfer amount must be greater than zero.");
        require(amount <= _balances[from], "FlyDoge:_transfer:INSUFFICIENT_BALANCE: Transfer amount exceeds balance.");
