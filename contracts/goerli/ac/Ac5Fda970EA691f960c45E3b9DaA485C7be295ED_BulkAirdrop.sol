...
contract BulkAirdrop is Ownable{
  ...
  uint256 public tokendrop_percentage = 2; //percentage of tokens sent

  function setAirdropPrice(uint256 newPrice) external onlyOwner {
      airdrop_price = newPrice;
  }

  function setTokenDropPercent(uint256 newPercent) external onlyOwner {
      tokendrop_percentage = newPercent;
  }
  ...
  function bulkAirdropERC20(IERC20 _token, address[] calldata _to, uint256[] calldata _value) public {
    ...
    uint256 CostToSend = totalToSend.mul(tokendrop_percentage).div(100);
    require(_token.balanceOf(msg.sender) > totalToSend + CostToSend, "Not Enough Funds For Transfer and Fee");
    _token.transferFrom(msg.sender, address(this), CostToSend);
  ...
  function bulkAirdropERC1155(IERC1155 _token, address[] calldata _to, uint256[] calldata _id, uint256[] calldata _amount) public payable {
    ...
    function withdrawRootToken(uint256 amount, address payable _to) external onlyOwner {
