...
/// @title Bumper Protocol Config contract
/// @notice Stores protocol-wide configuration data.
    ...
    ModuleAccessControllerUpgradeable
{
    ...
    /// @notice bToken address mapping
    /// @dev assetToken => bToken
    mapping(address => address) internal bTokens;

    /// @notice protocol markets
    /// @dev assetToken => market
    mapping(address => address) internal markets;

    /// @notice Enumerable set of protocol market tokens
    EnumerableSet.AddressSet iterator;

    /// @notice NFT maker position addresses
    /// @dev assetToken => nft
    mapping(address => address) public makerNFTPosition;

    /// @notice NFT taker position addresses
    /// @dev assetToken => nft
    mapping(address => address) public takerNFTPosition; // bToken addresses mapping

    /// @notice Initialize using upgradable pattern
    /// @param _gac GlobalAccessContoller instance
    function initialize(address _gac) external initializer {
        _setGlobalAccessController(_gac);
        _grantRole(DEFAULT_ADMIN_ROLE, msg.sender);
    }

    /// @notice sets a current version of a protocol
    function setVersion(uint16 _version) external onlyGovernance {
        version = _version;
    }

    /// @notice Set market stablecoin address
    ...
    /// @notice Set market position NFT tokens
    function setNFT(
        address token,
        address taker,
        address maker
    ) external onlyGovernance {
        takerNFTPosition[token] = taker;
        makerNFTPosition[token] = maker;
    }

    /// @notice Set market contract address for asset token and add it to enumerable set
    function setMarket(address token, address _market) external onlyGovernance {
        markets[token] = _market;
        EnumerableSet.add(iterator, token);
    }

    /// @notice Remove given market from the protocol and from enumerable set
    function removeMarket(address token) external onlyGovernance {
        delete markets[token];
        EnumerableSet.remove(iterator, token);
    }

    function getAssets() external view override returns (address[] memory) {
        return EnumerableSet.values(iterator);
    }

    /// @inheritdoc IProtocolConfig
    function getStable() external view override returns (address) {
        return stable;
    }

    /// @inheritdoc IProtocolConfig
    function getGAC() external view override returns (address) {
        return address(gac);
    }
    ...
    /// @inheritdoc IProtocolConfig
    function getNFTMaker(address token)
        external
        view
        override
        returns (address)
    {
        return makerNFTPosition[token];
    }

    /// @inheritdoc IProtocolConfig
    function getNFTTaker(address token)
        external
        view
        override
        returns (address)
    {
        return takerNFTPosition[token];
    }

    /// @inheritdoc IProtocolConfig
    function getMarket(address token) external view override returns (address) {
    ...
    /// @inheritdoc IProtocolConfig
    function getWrappedNativeMarket() external view override returns (address) {
        return markets[wNative];
    }

    /// @inheritdoc IProtocolConfig
    function getRebalancer() external view override returns (address) {
        return rebalancer;
    }
