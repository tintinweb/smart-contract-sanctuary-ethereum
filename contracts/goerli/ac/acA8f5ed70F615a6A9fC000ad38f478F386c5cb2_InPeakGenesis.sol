...
contract InPeakGenesis is ERC721, Ownable, ReentrancyGuard {
    ...
    uint256 public pendingReservedSupply = 0;
    ...
    mapping(address => bool) minted;
     ...
     constructor() ERC721("Inpeak Genesis", "IPGEN") {}

    /// @dev Mint 1 token to `recipient`. The `price` parameter is used to validate the proof. Incorrect price reverts the tx.
    function mint(address recipient, uint256 price, bytes32[] memory proof) external payable {
    ...
    /// @dev Mint 1 token to `recipient`. The `price` parameter is used to validate the proof. Incorrect price reverts the tx.
    function mintReserved(address recipient) onlyOwner external {
        require(pendingReservedSupply > 0, "not enough reserved supply");
        require(maxSupply -tokenCounter > 0, "max supply reached");

        tokenCounter += 1;
        pendingReservedSupply -= 1;
        _mint(recipient, tokenCounter);
    }
    ...
    /// @dev Set pending reserved tokens
    function setPendingReserved(uint256 pPendingReserved) onlyOwner public {
        require(getRemainingSupply() - pPendingReserved >= 0, "not enough remaining supply");
        pendingReservedSupply = pPendingReserved;
    }
