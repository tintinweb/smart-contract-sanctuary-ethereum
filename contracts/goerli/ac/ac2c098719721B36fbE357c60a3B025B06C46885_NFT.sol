...
contract NFT is ERC721, PullPayment, Ownable {
  ...
  // Mint Price
  uint256 public constant rMintPrice = 0.02 ether;
  uint256 public constant srMintPrice = 0.04 ether;
  uint256 public constant ssrMintPrice = 0.08 ether;
  uint256 public constant spMintPrice = 0.3 ether;
  uint256 public constant rMintPriceDiscount = 0.014 ether;
  uint256 public constant srMintPriceDiscount = 0.028 ether;
  uint256 public constant ssrMintPriceDiscount = 0.056 ether;
  ...
  constructor() ERC721("DAKETEST", "NFT") {
    baseTokenURI = "";
    merkleRoot = "";
  }
  ...
  function mintTo(address recipient, uint256 tokenId) public onlyOwner returns (uint256) {
    //require(msg.value >= nfts[tokenId].mintPrice, "Transaction value did not equal the mint price");
  ...
  function mintSSR(bytes32[] calldata merkleProof) public payable returns (uint256) {
    uint256 currentTokenId = ssrTokenId;
    bytes32 leaf = keccak256(abi.encodePacked(msg.sender));
    bool iswhitelisted = MerkleProof.verify(merkleProof, merkleRoot, leaf);
    require(currentTokenId >= 64 && currentTokenId < 356);
    require(mintedNFTs[msg.sender].length <= 3, "You already own 3 NFTs");
    if (iswhitelisted && whitelistused[msg.sender] == false && block.timestamp <=  firstsalestart  + 3 hours) {
      require(msg.value >= ssrMintPriceDiscount, "Transaction value did not equal the mint price");
      _safeMint(msg.sender, currentTokenId);
      _asyncTransfer(owner(), msg.value);
      whitelistused[msg.sender] = true;
      mintedNFTs[msg.sender].push(currentTokenId);
      ssrTokenId++;
      return currentTokenId;
    }
    else if ((block.timestamp >=  firstsalestart  + 1 hours) && (block.timestamp <=  firstsalestart  + 5 hours)) { // 应该大于test
  ...
  function mintSR(bytes32[] calldata merkleProof) public payable returns (uint256) {
    uint256 currentTokenId = srTokenId;
    bytes32 leaf = keccak256(abi.encodePacked(msg.sender));
    bool iswhitelisted = MerkleProof.verify(merkleProof, merkleRoot, leaf);
    require(currentTokenId >= 356 && currentTokenId < 938);
    require(mintedNFTs[msg.sender].length <= 3, "You already own 3 NFTs");
    if (iswhitelisted && whitelistused[msg.sender] == false && block.timestamp <=  firstsalestart  + 3 hours) {
  ...
  function mintR(bytes32[] calldata merkleProof) public payable returns (uint256) {
    uint256 currentTokenId = rTokenId;
    bytes32 leaf = keccak256(abi.encodePacked(msg.sender));
    bool iswhitelisted = MerkleProof.verify(merkleProof, merkleRoot, leaf);
    require(currentTokenId >= 938 && currentTokenId <= 2100);
    require(mintedNFTs[msg.sender].length <= 3, "You already own 3 NFTs");
    if (iswhitelisted && whitelistused[msg.sender] == false && block.timestamp <=  firstsalestart  + 3 hours) {
    ...
    else if ((block.timestamp >=  firstsalestart  + 1 hours) && (block.timestamp <=  firstsalestart  + 5 hours)) { // greater   test
