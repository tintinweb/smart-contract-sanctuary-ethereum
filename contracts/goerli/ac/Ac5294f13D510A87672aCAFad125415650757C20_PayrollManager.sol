...
contract PayrollManager is SignatureEIP712, Storage {
    ...
    // @def: pack the boolean value for a payoutNonce in uint256 Slot using 1 bit
    // @param(payoutNonce): payoutNonce which needs to be set true or false
    // @param(flag): Value of the payout Nonce either True or False
    ...
    // @def: Returns the current value of payoutNonce
    // @param(payoutNonce): payoutNonce which you  need to retrieve
    // @return (bool): Value of the payout Nonce either True or False
    function getPayoutNonce(uint payoutNonce) internal view returns (bool) {
        uint slotIndex = payoutNonce / 256;
        uint bitIndex = payoutNonce % 256;
        if (
    ...
    // @def: Returns the encodedHash of to, tokenAddress, amount, payoutNonce
    // @param(to): recipient address
    // @param(tokenAddress): token address
    // @param(amount): amount in wei to be paid
    // @param(payoutNonce): payoutNonce for replay protection
    // @return (bytes32): returns the keccak256 hash of params
    ...
    // @def: This function will validates the Signatures provided for each rootNode and then
    //       it will set the mapping of approvedNode to true
    // @param(safeAddress): safeAddress on which you want to add approvals
    // @param(roots[]): array of root nodes you want to verify
    // @param(signatures[]): Array of Signatures for respective rootnodes
    // @param(paymentTokens[]): tokenAddress array on which you want to retrieve funds aggregate unique tokens for all payouts
    // @param (payoutAmounts[]): amount array on which you want to retrieve funds aggregate unique tokens for all payouts
    ...
    // @def: This function will execute the payout of a specific payout with validating the leaf with merkle root and will check
    //       the number of approvals added to that merkle root
    // @param(to): recipient address
    // @param(tokenAddress): token address
    // @param(amount): amount in wei to be paid
    // @param(payoutNonce): payoutNonce for replay protection
    // @param(safeAddress): safeAddress on which you want to add approvals
    // @param(roots[]): array of root nodes you want to verify
    // @param(proof[]): array of proof nodes you want to verify
