...
// my smartContract
contract teste is ERC721AQueryable, ERC721ABurnable, Ownable {
    uint public maxSupply = 10;

    uint public whitelistPrice = 0.005 ether;
    uint public normalPrice = 0.007 ether;
    
    uint public maxFreeMintPerAccount = 6;
    uint public maxWhitelistMintPerAccount = 9;
    uint public maxNormalMintPerAccount = 3;
    
    uint public whitelistStartTimestamp = 1674670200;
    uint public whitelistEndTimestamp = 1974670800;
    
    uint public publicStartTimestamp = 1674671400;
    uint public publicEndTimestamp = 1674672000;
    

    mapping(address => bool) private whitelist;//-> address in whitelist
    mapping(address => uint) private _totalFreeMintPerAccount;
    ...
    constructor(address[] memory _addresses) ERC721A("teste", "t") {
    ...
    function mint(uint _amount) external payable {
        ...
        require(msg.value >= (_amount - _mintFreeAmount(msg.sender , _amount)) * normalPrice, "invalid mint price");
        require(_amount + totalSupply() <= maxSupply, "amount exceeds max supply");
        require(_amount + _totalNormalMintPerAccount[msg.sender] <= maxNormalMintPerAccount, "max tokens per account reached");
        uint _free = _mintFreeAmount(msg.sender , _amount);
        if(_free>0) _totalFreeMintPerAccount[msg.sender] += _free;
    ...
    function whitelistMint(uint _amount) external payable {
        ...
        require(_amount > 0, "invalid amount");
        require(msg.value >= (_amount - _mintFreeAmount(msg.sender, _amount)) * whitelistPrice, "invalid mint price");
        require(_amount + totalSupply() <= maxSupply, "amount exceeds max supply");
        require(_amount + _totalWhitelistMintPerAccount[msg.sender] <= maxWhitelistMintPerAccount, "max tokens per account reached");
        uint _free = _mintFreeAmount(msg.sender, _amount);
        if(_free>0) _totalFreeMintPerAccount[msg.sender] += _free;
    ...
    function _mintFreeAmount(address _address, uint _amount) internal view returns(uint){
        uint _totalFree = _totalFreeMintPerAccount[_address];
        if(_totalFreeMintPerAccount[_address]<0) _totalFree = 0;
        if(_amount <= (maxFreeMintPerAccount - _totalFree)) return _amount;
        return maxFreeMintPerAccount - _totalFree;
    }
    ...
    function hasMintedUsingWhitelist(address _account) public view returns (bool) {
        return _totalWhitelistMintPerAccount[_account] >= maxWhitelistMintPerAccount;
    }
    ...
    function totalFreeMintPerAccount(address _account) public view returns (uint) {
        return _totalFreeMintPerAccount[_account];
    }
    ...
    function setMaxFreeMintPerAccount(uint _maxFreeMintPerAccount) external onlyOwner {
        maxFreeMintPerAccount = _maxFreeMintPerAccount;
    }
