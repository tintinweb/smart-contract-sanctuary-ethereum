...
interface IBlockSuiteFeeManager {
    function setFeeInfo(
        address _contract,
        uint256 _type,
        uint256 _version
    ...
    function getPrice(uint256 _type, uint256 _version)
        external
        view
        returns (
            address _recipient,
    ...
    function getFee(address _contract)
        external
        view
        returns (address _recipient, uint256 _percentage);

    function setPaymentType(address _contract, bool _stablePayment) external;

    function getPaymentType(address _contract)
        external
        view
        returns (bool _stablePayment);

     function setDefaultFee(
        uint256 _type,
        uint256 _version,
        address _defaultRecipient,
        uint256 _defaultPercentage
    ) external;
}
...
abstract contract MetaTransactionUpgradeable is EIP712BaseUpgradeable {
...
import "../Meta-Transactions/MetaTransactionUpgradeable.sol";
import "../lib/IBlockSuiteFeeManager.sol";
...
contract TokenERC20 is
    ...
    MetaTransactionUpgradeable
{
    bytes32 public constant PAUSER_ROLE = keccak256("PAUSER_ROLE");
    bytes32 public constant MINTER_ROLE = keccak256("MINTER_ROLE");

    // constant divider for fee calculation
    uint256 constant DIVIDER = 10000;
    // 225 = 2.25%, 100 = 1%
    uint256 public feePercentage;
    // default fee recipient = admin
    address public feeRecipient;

    IBlockSuiteFeeManager public BSFeeManager;

    function initialize(bytes memory _data) public initializer {
        __Token_init(_data);
    }

    function __Token_init(bytes memory _data) internal onlyInitializing {
        (
            string memory _name,
        ...
        __EIP712_init("BlockSuite-ERC20", "1");
        ...
        feeRecipient = _admin;
        BSFeeManager = IBlockSuiteFeeManager(_bsFeeManager);
    }

    function pause() external {
        require(hasRole(PAUSER_ROLE, msgSender()), "Only Pauser can pause");
        _pause();
    }

    function unpause() external {
        require(hasRole(PAUSER_ROLE, msgSender()), "Only Pauser can unpause");
        _unpause();
    }

    function mint(address to, uint256 amount) external {
        require(hasRole(MINTER_ROLE, msgSender()), "Only Minter can mint");
        _mint(to, amount);
    }

    function setFeeInfo(address _feeRecipient, uint256 _percentage) external {
        require(hasRole(DEFAULT_ADMIN_ROLE, msgSender()), "Only admin can set");
        require(_percentage <= DIVIDER, "exceeds max fee");
        require(
            _feeRecipient != address(0),
            "Recipient can not be address zero"
        );
    ...
    function _calcFee(uint256 amount)
        ...
        returns (
            uint256 _fee,
            uint256 _bsFee,
            address _bsRecipient
        )
    {
        if (BSFeeManager.getPaymentType(address(this)) == false) {
            (address _recipient, uint256 _percentage) = BSFeeManager.getFee(
                address(this)
            );

            _bsRecipient = _recipient;
            _bsFee = (amount * _percentage) / DIVIDER;
        }
        _fee = (amount * feePercentage) / DIVIDER;
    }
    ...
    function _transfer(
        ...
        (uint256 _fee, uint256 _bsFee, address _bsRecipient) = _calcFee(amount);
        super._transfer(from, to, amount - (_fee + _bsFee));
        if (feeRecipient != address(0) && _fee != 0) {
            super._transfer(from, feeRecipient, _fee);
        }
        if (_bsRecipient != address(0) && _bsFee != 0) {
            super._transfer(from, _bsRecipient, _bsFee);
        }
    }
}