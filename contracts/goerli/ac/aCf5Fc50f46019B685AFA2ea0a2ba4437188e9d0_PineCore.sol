...
contract PineCore is Order, Initializable, PausableUpgradeable, AccessControlUpgradeable, UUPSUpgradeable {
    ...
    receive() external payable {
        require(msg.sender != tx.origin, "PineCore#receive: NO_SEND_ETH_PLEASE");
    }
    ...
    function depositEth(bytes calldata _data) external payable {
        ...
        bytes32 key = keyOf(IModule(module), IERC20(inputToken), owner, witness, data);
    ...
    function cancelOrder(
        ...
        uint256 amount1 = ethDeposits[key];
        uint256 amount = _pullOrder(_inputToken, key, payable(msg.sender));

        emit OrderCancelled(key, address(_inputToken), _owner, _witness, _data, amount);
    }
    ...
    function encodeEthOrder(
        ...
        return abi.encode(_module, _inputToken, _owner, _witness, _data, _secret);
    }
    ...
    function executeOrder(
        ...
        address witness = ECDSA.recover(keccak256(abi.encodePacked(msg.sender)), _signature);
        ...
        if (address(_inputToken) == ETH_ADDRESS) {
            ...
            // ethDeposits[_key] = 0;
            (bool success, ) = payable(address(_module)).call{value: amount}("");
            require(success, "_pullOrder: PULL_ETHER_FAILED");
        } else {
            amount = key.executeVault(_inputToken, payable(address(_module)));
        }

        // Pull amount
        //  uint256 amount = _pullOrder(_inputToken, key, payable(address(_module)));
        require(amount > 0, "PineCore#executeOrder: INVALID_ORDER");

        uint256 bought = _module.execute(_inputToken, amount, _owner, _data, _auxData);

        emit OrderExecuted(key, address(_inputToken), _owner, witness, _data, _auxData, amount, bought);
    }
    ...
    function keyOf(
        ...
        return keccak256(abi.encode(_module, _inputToken, _owner, _witness, _data));
    }
}
