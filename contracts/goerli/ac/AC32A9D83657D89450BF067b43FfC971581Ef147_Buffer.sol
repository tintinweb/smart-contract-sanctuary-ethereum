...
contract Buffer is Initializable, ReentrancyGuard {
    ...
    receive() external payable {
        ...
        for (uint256 i = 0; i < 5; i++) {
            Fees[i] = totalAmount * shareDetails[i] / totalShares;
        }
        _transfer(marketWallet, Fees[4]);
        Fees[4] = 0;
        // artistFee = (totalAmount * shareDetails[0]) / totalShares;
        // totalPartnersFee = (totalAmount * shareDetails[1]) / totalShares;
        // totalCreatorsFee = (totalAmount * shareDetails[2]) / totalShares;
        // totalOwnersFee = (totalAmount * shareDetails[3]) / totalShares;
        // montageFee = (totalAmount * shareDetails[4]) / totalShares;
    }
    ...
    function updateCreatorsGroup(address[] calldata _creatorsGroup) external onlyOwner {
        ...
        // uint256 tmp = totalCreatorsFee / creatorsGroupLength;
