...
contract ERC721StakingModuleFactory is IModuleFactory {
        ...
        returns (address)
    {
        ...
        // create module
        ERC721StakingModule module = new ERC721StakingModule(
            token,
            address(this)
        );
...
/**
 * @title CML event system
 *
 * @notice common interface to define CML event system
 */
interface IEvents {
    // staking
    event Staked(
    ...
    // cml
    event CMLSpent(address indexed user, uint256 amount);
    event CMLVested(address indexed user, uint256 amount);
    event CMLWithdrawn(uint256 amount);
}
