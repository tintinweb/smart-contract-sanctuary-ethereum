...
contract LBSKR is Context, IERC20, Ownable {
        ...
        )
    {
        ...
        return (tTransferAmount, burnFee, growthFee);
    }
    ...
    {
        (uint256 burnFee, uint256 growthFee) = calculateFees(tAmount);
        uint256 tTransferAmount = tAmount.sub(burnFee).sub(growthFee);
        return (tTransferAmount, burnFee, growthFee);
    }
    ...
    {
        return (
            _amount.mul(_burnFee).div(10**2),
            _amount.mul(_growthFee).div(10**2)
        );
    }
    ...
    function _transferStandard(
        ...
        // tTransferAmount, burnFee, growthFee
        (
            uint256 tTransferAmount,
            uint256 burnFee,
            uint256 growthFee
        ...
        _takeFees(burnFee, growthFee);

        emit Transfer(sender, recipient, tTransferAmount);
    }

    function _takeFees(uint256 burnFee, uint256 growthFee) private {
        _balances[address(0)] += burnFee;
        _balances[address(0)] += growthFee; // TODO change to growth address
    }
