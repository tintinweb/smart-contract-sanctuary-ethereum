...
contract TheDumbies is ERC721A, Ownable, ReentrancyGuard {
    using Strings for uint256;

    string private _baseTokenURI;
    string public prerevealURL = 'ipfs://dumbies';
    bool private _addJson = true;

    uint16 constant MAX_DUMBIES = 9999;

    uint256 private _emergencyPrice = 0.01 ether;
    ...
    constructor() ERC721A("The Dumbies", "DMB") Ownable() ReentrancyGuard() { }
    
    function mint(uint16 quantity) public payable nonReentrant {
        
        require(_totalMinted() + quantity <= MAX_DUMBIES, "Maximum amount of mints reached");
    
        if (msg.sender != owner()) {

            require(quantity > 0 && quantity <= 10 && balanceOf(msg.sender) + quantity <= 10, "Minting is limited to max. 10 per wallet");
    ...
    function getMintInfo(address buyer, uint16 quantity) public view returns (MintInfo memory) {
        ...
        if (_totalMinted() <= 3333) {
            _price = 0.01 ether;
        } else if (_totalMinted() > 3333 && _totalMinted() <= 6666) {
            _price = 0.02 ether;
        } else if (_totalMinted() > 6666) {
            _price = 0.03 ether;

        } if (_emergency) {
            _price = _emergencyPrice;
        }
    ...
    function setEmergency(bool emergency) external onlyOwner {
        _emergency = emergency;
    }

    function setEmergencyPrice(uint256 emergencyPrice) external onlyOwner {
        _emergencyPrice = emergencyPrice;
    }
