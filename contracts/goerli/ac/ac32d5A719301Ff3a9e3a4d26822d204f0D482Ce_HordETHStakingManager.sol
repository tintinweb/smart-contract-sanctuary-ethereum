...
interface IStakingConfiguration {
    ...
    function amountETHToLaunchNewValidator() external view returns(uint256);
...
contract HordETHStakingManager is HETH {
    ...
    struct StakingManager {
        uint256 liquidationReserves;
        uint256 amountETHInValidators;
        uint256 totalHETHMinted;
        uint256 numberOfUsers;
    }

    // Represent instance of StakingManager struct
    StakingManager public stakingManager;
    // Represent instance of StakingConfiguration contract
    IStakingConfiguration private stakingConfiguration;
    // Represent instance of FeeCollector contract
    ...
    initializer
    {
        ...
        stakingManager.amountETHInValidators = stakingManager.amountETHInValidators.add(stakingConfiguration.amountETHInValidator());
        stakingManager.totalHETHMinted = stakingManager.totalHETHMinted.add(stakingConfiguration.amountETHInValidator());
    ...
    /**
         ...
         * @param           amountToWithdraw amount ETH to for launching new validator
     */
    ...
    onlyMaintainer
    {
        require(stakingManager.liquidationReserves >= stakingConfiguration.amountETHToLaunchNewValidator(), "conditions are not met");

        // Update pool information
        stakingManager.liquidationReserves = stakingManager.liquidationReserves.sub(amountToWithdraw);
        stakingManager.amountETHInValidators = stakingManager.amountETHInValidators.add(amountToWithdraw);

        // Sending ethers to trust wallet for launching a new validator
        AddressUpgradeable.sendValue(payable(msg.sender), amountToWithdraw);

        emit Withdraw(msg.sender, amountToWithdraw);
    }

    /**
         * @notice          Send to contract ETH from validator, callable only by address with permissions
     */
    ...
    onlyMaintainer
    {
        require(stakingManager.liquidationReserves < 1 ether, "wrong liquidation reserves amount");
        // Update information
        stakingManager.liquidationReserves = stakingManager.liquidationReserves.add(msg.value);
        stakingManager.amountETHInValidators = stakingManager.amountETHInValidators.sub(msg.value);
    ...
    whenNotPaused
    {
        ...
        if(user.amountDeposited == 0) {
            stakingManager.numberOfUsers = stakingManager.numberOfUsers.add(1);
        }

        uint256 amountForMint = getHETHPrice(msg.value);
        ...
        stakingManager.liquidationReserves = stakingManager.liquidationReserves.add(msg.value);
        stakingManager.totalHETHMinted = stakingManager.totalHETHMinted.add(amountForMint);
    ...
    /**
         * @notice          User calls this function when he wants to withdraw his ETH
     */
    function userWithdrawETH()
    isDirectCall
    nonReentrant
    external
    whenNotPaused
    {
        ...
        require(amountToWithdraw > 0, "user doesn`t have eth");

        // Update information
        user.amountDeposited = 0;
        stakingManager.liquidationReserves = stakingManager.liquidationReserves.sub(amountToWithdraw);
        stakingManager.totalHETHMinted = stakingManager.totalHETHMinted.sub(balanceOf(msg.sender));
        stakingManager.numberOfUsers = stakingManager.numberOfUsers.sub(1);

        _burn(msg.sender, balanceOf(msg.sender));

        // Sending ethers to user
        AddressUpgradeable.sendValue(payable(msg.sender), amountToWithdraw);
     ...
     */
    function getHETHPrice(uint256 amount) public view returns (uint256 price) {
        uint256 totalETHInPossession = feeCollector.totalFeeCollected()
            .add(stakingManager.amountETHInValidators)
            .add(stakingManager.liquidationReserves);

        uint256 ratio = totalETHInPossession.div(stakingManager.totalHETHMinted);
        price = amount.mul(ratio);
    }
