...
contract BooWhoBabies is ERC721, Ownable, ERC721Burnable, ERC721Pausable {
    ...
    uint256 public constant maxMintPerTransaction = 1;
    ...
    function setPrivateSale(bool val) public onlyOwner {
        privateSaleOpen = val;
    }
    ...
    function whitelistMint(address _recipient, bytes32[] calldata _proof) public payable saleIsOpen {
        uint256 total = totalSupply();
        require(privateSaleOpen, "Private sale not open at this time.");
        require(total + maxMintPerTransaction <= max_elements, "Max limit");
        require(whitelistClaimed[_recipient] + maxMintPerTransaction <= maxMint, "This recipient has received the maximum amount allowed.");
        require(verifySender(_proof, msg.sender), "Sender is not whitelisted");
        require(verifySender(_proof, _recipient), "Recipient is not whitelisted");
        require(msg.value == privatePrice, "Value is over or under price.");
    ...
    function mint() public payable saleIsOpen {
        uint256 total = totalSupply();
        require(publicSaleOpen, "Public sale not open at this time.");
        require(total + maxMintPerTransaction <= max_elements, "Max limit");
        require(msg.value == publicPrice, "Value is over or under price.");
    ...
    function verifySender(bytes32[] calldata proof, address recipient) public view returns (bool) {
        return MerkleProof.verify(proof, merkleRoot, _hash(recipient));
    }
