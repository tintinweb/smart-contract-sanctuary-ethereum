...
contract ExchangeHelper is Ownable, ExchangeAutoProxy, IExchangeHelper {
    ...
    function validateOrder(
        ...
        LibExchange.validateOrder(order, signature, callerAddress, verifyingContractProxy);
    }
    ...
    function checkCounterparties(
        ...
        return LibExchange.checkCounterparties(orderLeft, orderRight);
    }
    ...
    function calculateTotalTakeAndMakeValues(
        ...
        LibFeeSideTypes.FeeSide feeSide = LibFeeSide.getFeeSide(
        ...
        if (feeSide == LibFeeSideTypes.FeeSide.MAKE) {
            _totalMakeValue = LibExchange.calculateTotalAmount(
                _makeValue,
                rightOrderData.originFees
            );
        } else if (feeSide == LibFeeSideTypes.FeeSide.TAKE) {
            _totalTakeValue = LibExchange.calculateTotalAmount(
                _takeValue,
    ...
    function checkCounterparties(
        ...
        if (orderLeft.taker != address(0)) {
            require(
                ...
                'LibExchange: leftOrder.taker verification failed'
            );
        }
        if (orderRight.taker != address(0)) {
            require(
                orderRight.taker == orderLeft.maker,
                'LibExchange: rightOrder.taker verification failed'
            );
        }
    }
}
