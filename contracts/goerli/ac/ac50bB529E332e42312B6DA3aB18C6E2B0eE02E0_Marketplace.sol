...
contract Marketplace is EIP712Upgradeable {
    ...
    function initialize(address _admin, uint256 _platformFee)
        external
        initializer
    {
    ...
    function buyShare(
        ...
        if (isPrimary) {
            ...
            if (currency == address(1)) {
                uint256 ETHAmount = (amountToPay - finalFeeAmount);
            ...
            emit voucher.buyShare(seller.seller, msg.sender, amountToBuy);
        } else {
            require(
            ...
            if (currency == address(1)) {
                // require(msg.value==amountToPay,"IP");
    ...
    function setPlatformfee(uint256 _newFee) external onlyAdmin {
        platformFee = _newFee;
        emit voucher.platFormFee(_newFee);
    }

    // function setPlatformFeeWallet(address _newWallet) external onlyAdmin {
    //     require(_newWallet != address(0), "ZA"); //Zero Address
