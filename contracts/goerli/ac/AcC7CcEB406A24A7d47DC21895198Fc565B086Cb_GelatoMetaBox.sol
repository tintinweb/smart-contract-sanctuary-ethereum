...
contract GelatoMetaBox is IGelatoMetaBox, Proxied {
        ...
        returns (bytes memory encodedReq)
    {
        encodedReq = abi.encode(
            // solhint-disable-next-line max-line-length
            keccak256("Request(uint256 chainId,address target,bytes data,address feeToken,address user,address sponsor,uint256 nonce,uint256 deadline,bool isEIP2771)"),
            _req.chainId,
            _req.target,
            keccak256(_req.data),
            _req.feeToken,
