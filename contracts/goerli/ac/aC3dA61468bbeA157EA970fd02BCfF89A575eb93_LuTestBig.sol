...
contract LuTestBig {
  ...
  function getTransformData(uint16 key) public pure returns(uint16[3] memory) { 
  ...
  /*
  function getColor(string memory fill, uint8[] memory swapKeys)  public view returns(string memory output) {
      string[2][] memory swapTable = new string[2][](10);
      swapTable[0] = ["aa", "bb"];
      uint size = swapKeys.length;
      output = fill;
      for(uint i = 0; i < size; i++) {
          if (keccak256(abi.encodePacked(swapTable[swapKeys[i]][0])) == keccak256(abi.encodePacked(fill))) {
              output = swapTable[swapKeys[i]][1];
          }
      }
  }
  */  

  // function getParts(uint16 key, uint8[] memory swapKeys) public view returns(bytes memory output) {
  ...
  function getResizedParts(uint16 index) public view returns(SVG.Element memory output) {
      uint16[3] memory tdata = getTransformData(index);
      return SVG.group(
                       SVG.group(
                                 SVG.group(SVG.item(getParts(index)))
  ...
  */
  ...
  // 0, 1, 2
  function windows(uint16 index) public view returns(SVG.Element memory output) {
      return getResizedParts((index % 3) + 15);
  }
  // 0, 1
  function door(uint16 index) public view returns(SVG.Element memory output) {
      return getResizedParts((index % 2) + 3);
  }
  // 0, 1
  function roof(uint16 index) public view returns(SVG.Element memory output) {
      return getResizedParts((index % 2) + 12);
  }
  // 0, 1, 2, 3
  function heart(uint16 index) public view returns(SVG.Element memory output) {
      return getResizedParts((index % 4) + 5);
  }
  // 0, 1
  function lu(uint16 index) public view returns(SVG.Element memory output) {
