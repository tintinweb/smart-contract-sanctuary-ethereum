...
import "../Keys/VerifiedMint721Key.sol";

contract Verified721LockSmith is Ownable {
    constructor() {}
    ...
    function createVerifiedMint721Key(
        ...
        VerifiedMint721Key key = new VerifiedMint721Key(
            name,
            symbol,
            baseURL,
            verifiedSigner,
        ...
        key.transferOwnership(newOwner);
        return address(key);
    }
}
...
contract VerifiedMint721Key is Base721Key {
    ...
    mapping(string => uint256) public mintCount;
    ...
    function mint(bytes calldata signature, string calldata username)
        external
        payable
    {
        ...
        require(
            mintCount[username] < maxMint,
            "Cannot mint more any more with this account!"
        );

        uint256 tokenId = _tokenIdCounter.current();
        require(verify(signature, username), "Signature is not valid!");

        _tokenIdCounter.increment();
        mintCount[username] = mintCount[username] + 1;
     ...
     */
    function verify(bytes memory signature, string calldata username)
        public
        view
        returns (bool)
    {
     ...
     */
    function hashPayload(string calldata username)
        public
        pure
        returns (bytes32)
    {
     ...
     * @dev Verifies the data hosted at the URI matches the signature
     */
    function recoverSignerAddress(bytes32 data, bytes memory signedData)
        public
        pure
        returns (address)
    {
    ...
    Ownable
{
    ...
    function tokenURI(uint256) public view override returns (string memory) {
        return string(abi.encodePacked(_baseURL, "/token"));
    }
