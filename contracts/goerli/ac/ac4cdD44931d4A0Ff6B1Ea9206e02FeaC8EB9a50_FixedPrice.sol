...
contract FixedPrice is Ownable, IFixedPrice {
    ...
    // ------- var ------- //
    address public constant EMPTY_ADDRESS =
    ...
    function sell(
        ...
        // must be owner
        require(
            contractSale.ownerOf(_token_id) == msg.sender ||
                msg.sender == _contract_sale,
            "NOT_OWNED"
        );
        // transfer to sale contract
        contractSale.transferFrom(
            msg.sender == _contract_sale
                ? contractSale.ownerOf(_token_id)
                : msg.sender,
            address(this),
            _token_id
        );
    ...
    function buy(
        ...
        // get data out
        IERC721 contractSale = IERC721(_contract_sale);
        bytes32 hashedFixedPriceItem = sha256(
            abi.encodePacked(_contract_sale, _token_id)
        );
        uint256 itemPrice = mapTokenToFixedPriceItem[hashedFixedPriceItem]
            .price;
        address creator = mapTokenToFixedPriceItem[hashedFixedPriceItem]
            .creator;
        ...
        mapTokenToFixedPriceItem[hashedFixedPriceItem].creator = EMPTY_ADDRESS;
    ...
    // return sell info
    function sellInfo(address _contract_sale, uint256 _token_id)
        external
        view
        override
        returns (uint256 price, address creator)
    {
        bytes32 hashed = sha256(abi.encodePacked(_contract_sale, _token_id));
    ...
    // Cancel sell
    function cancelSell(address _contract_sale, uint256 _token_id)
        external
        override
    {
        bytes32 hashedFixedPriceItem = sha256(
        ...
        mapTokenToFixedPriceItem[hashedFixedPriceItem].creator = EMPTY_ADDRESS;
        contractSale.transferFrom(address(this), creator, _token_id);
    }
}
