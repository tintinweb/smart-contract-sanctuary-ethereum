...
contract MyCryptoNinja is ERC721A, Ownable {
    enum NinjaStatus {
        FOCUS,
        STRIVE,
        DESERT,
    ...
    uint256 public whitelistStartTime = 1667480400; // Nov 3 2022 20:00:00 GMT+07

    uint256 public publicStartTime = 1667484000; // Nov 3 2022 21:00:00 GMT+07
    ...
    address public cryptoNinjaPartners = 0x128615C009aaa7e88e6E533D6Ebbf14bd8ac22Bc; // todo mainnet 0x845a007d9f283614f403a24e3eb3455f720559ca

    constructor(string memory defaultTokenURI_, bytes32 merkleRoot_) ERC721A("MyCryptoNinja", "MCN") {
    ...
    function whitelistMint(uint256 quantity, bytes32[] calldata _merkleProof) external callerIsUser payable {
        ...
        require(whitelistMinted + quantity <= 2000, "Exceed whitelist supply");
        require(isInWhitelist(msg.sender, _merkleProof) || isCryptoNinjaPartnersOwner(msg.sender), "Caller can not whitelist mint");
    ...
    function publicMint(uint256 quantity) external callerIsUser payable {
        ...
        require(block.timestamp >= publicStartTime || totalMinted_ >= 2000, "Not in public stage");
        require(totalMinted_ + quantity <= maxSupply, "Exceed supply");
    ...
    function isCryptoNinjaPartnersOwner(address owner) public view returns (bool) {
        return ERC721A(cryptoNinjaPartners).balanceOf(owner) > 0;
    }
    ...
    function getNinjaLevel(uint256 tokenId) public view returns (uint256) {
        ...
        if (trainDaysSinceLast >= 14) {
            ...
            upgradeLevel = 300 + ((trainSecondsSinceLast * 100) / (5 * DAY_SECOND));
        } else if (trainDaysSinceLast >= 5) {
            upgradeLevel = 200 + ((trainSecondsSinceLast * 100) / (4 * DAY_SECOND));
        } else if (trainDaysSinceLast >= 2) {
            upgradeLevel = 100 + ((trainSecondsSinceLast * 100) / (3 * DAY_SECOND));
        } else {
