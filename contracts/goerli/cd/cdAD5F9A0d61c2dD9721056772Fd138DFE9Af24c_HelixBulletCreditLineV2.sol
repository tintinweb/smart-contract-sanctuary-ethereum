...
import "../libraries/BulletAccountant.sol";
import "../interfaces/IERC20withDec.sol";
import "../interfaces/IHelixCreditLineV2.sol";
...
contract HelixBulletCreditLineV2 is BaseUpgradeablePausable, IHelixCreditLineV2 {
  ...
  function initialize(
    ...
    require(_minLimit < _maxLimit, "Min limit must be less than max limit");
  ...
  function drawdown(uint256 amount, uint256 startAccured) external override onlyAdmin {
    ...
    if (balance == 0) {
      ...
      if (termEndTime == 0) {
        setTermEndTime(startAccured + (SECONDS_PER_DAY * termInDays));
      }
    }
    ...
    )
  {
    ...
    BulletAccountant.PaymentAllocation memory pa = BulletAccountant.allocatePayment(
      paymentAmount,
      balance,
      newInterestOwed,
      newPrincipalOwed
  ...
  function _updateAndGetInterestAndPrincipalOwedAsOf(uint256 timestamp) internal returns (uint256, uint256) {
    (uint256 interestAccrued, uint256 principalAccrued) = BulletAccountant.calculateInterestAndPrincipalAccrued(
      this,
      interestModel,
      timestamp,
...
library BulletAccountant {
  ...
  function calculateAmountOwedForOneDay(IHelixCreditLineV2 cl) public view returns (uint256 interestOwed) {
