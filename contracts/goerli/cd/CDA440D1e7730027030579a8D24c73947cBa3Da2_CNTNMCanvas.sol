...
contract CNTNMCanvas is Ownable, ERC721A, VRFConsumerBaseV2, ReentrancyGuard {
  ...
  // Declare variables to emit event the second time the fullfillrandomwords callback is called
  uint256 second_req_id; // second requested id (used to identify the second call to requestRandomWords)
  bool winner_ids_set = false;
  uint32 rand_num_owners; // Random number of nfts/owners selected to get rewards
  uint32 max_winners;
  
  constructor(uint64 subscriptionId, address cntnm_address) ERC721A("The Continuum Canvas", "CNTNM-CANVAS") VRFConsumerBaseV2(vrfCoordinator) {
  ...
  event winnersList(string indexed canvas_edition, address[] winners_addresses);
  ...
  function randNumWinners(uint32 min_callbackGasLimit, uint32 _max_winners) external onlyOwner nonReentrant {
    ...
    // Storing each word/random value costs about 20,000 gas, so using 50,000 is safe.
  ...
  function fulfillRandomWords(uint256, /* requestId */ uint256[] memory randomWords) internal override {
    ...
    if (second_req_id == 0) {
      ...
      emit numOfWinners(rand_num_owners);
        ...
        // Instantiate quantity for each winner (equally distributed)
        address[] memory winners_list = new address[](rand_num_owners);
        for (uint i=0; i < rand_num_owners; i++) {
            uint256 rand_winner_nft_index = (s_randomWords[i] % 2); //replace 2 with 998
            address winner = cntnm.ownerOf(rand_winner_nft_index);
            winners_list[i] = winner;
        }
        emit winnersList(string(abi.encodePacked("Canvas Edition: ", _toString(totalSupply()))), winners_list);
        // Reset variables 
        second_req_id = 0;
