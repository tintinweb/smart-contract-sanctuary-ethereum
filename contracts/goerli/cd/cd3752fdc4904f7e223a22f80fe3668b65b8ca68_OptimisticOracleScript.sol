...
contract OptimisticOracleScript is Script {
    function run() external {
        vm.startBroadcast();

        deploy(0x0a9171BeDA8c7EAAEAe05AB305278b4A346417B7,IERC20(address(0xf440F5a9AF6c9c620Ab6b60e16c78458a4F82E04)));

        vm.stopBroadcast();
    }

    function deploy(address collybusAddress, IERC20 token) public returns(OptimisticOracle){
        OptimisticOracle oracle = new OptimisticOracle(collybusAddress,token,1e18);
...
contract OptimisticOracle is Guarded {
    ...
    error OptimisticOracle__dispute_invalidProposalState();
    error OptimisticOracle__dispute_invalidDispute();

    error OptimisticOracle__settleDispute_invalidProposalState();
    ...
    /// - previous ProposalState == Null
    /// - `proposeWindow` and `disputeWindow` have been exceeded
    /// - if the current proposed value is disputable then `dispute` method should be used instead of `shift`.
    /// For the initial shift for a given `RateId` `prevValue` and `prevNonce` have to be 0.
    ...
    function shift(
        ...
        // If the previous proposal is the 'Proposed` state, push it to Collybus
        ...
        // Update the ProposalState of the new proposal
    ...
    function _settleDispute(
        ...
        // Verify the previous proposal state
        ...
        if (proposals[rateId] != disputedProposalId) {
            revert OptimisticOracle__settleDispute_invalidProposalState();
        }
