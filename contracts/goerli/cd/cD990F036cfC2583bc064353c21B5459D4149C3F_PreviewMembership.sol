...
contract PreviewMembership is Context, ERC721A, Ownable, ReentrancyGuard {
	...
	) ERC721A(name, symbol) noZeroAddress(_signer) noZeroAddress(_payoutAddress) {
		usdPrice = _usdPrice;
	...
	function setPayoutAddress(
		address _payoutAddress
	) external onlyOwner noZeroAddress(_payoutAddress) {
	...
	function airdrop(
		address receiver,
		uint256 amount
	) external onlyOwner nonReentrant noZeroAddress(receiver) {
