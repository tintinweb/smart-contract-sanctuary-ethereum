...
contract ERC20 is Context {
        ...
        returns (bool)
    {
        ...
        _approve(owner, spender, current_allowance + addedValue);
        return true;
    }
        ...
        returns (bool)
    {
        // TODO: use `_approve` defined later in the contract
        address owner = _msgSender();
        uint256 remaining_allowance = allowance(owner, spender);
        require(spender != address(0));
        require(remaining_allowance >= subtractedValue);

        _approve(owner, spender, remaining_allowance - subtractedValue);
    ...
    function _transfer(
        ...
        require(from != address(0));
        require(to != address(0));
        uint256 bal_from = _balances[from];
        require(bal_from >= amount);

        _balances[from] = bal_from - amount;
        _balances[to] += amount;
    ...
    function _burn(address account, uint256 amount) internal virtual {
        // TODO
        require(account != address(0));
        uint256 account_balance = _balances[account];
        require(account_balance >= amount);
        _balances[account] = account_balance - amount;
        _totalSupply -= amount;
...
contract LC0 is LuckyToken {
    // TODO: replace names
    constructor() LuckyToken("LuckyTokenYZT", "LCYZT") {}
}