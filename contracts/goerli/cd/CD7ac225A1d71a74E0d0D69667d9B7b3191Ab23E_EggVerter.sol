...
contract EggVerter is ERC721, ERC721Burnable, Ownable, ERC2981 {
    ...
    uint256 public eggCostWithEth = .01 ether;
    ...
    function buyEggs(uint256 quantity, bool _buyingWithEther) public payable
    {
        ...
        if(_buyingWithEther){
            require(msg.value >= eggCostWithEth * quantity,"not enough ether sent");
        } else {
            ( bool transferOccured ) = dinoToken.transferFrom(msg.sender, address(this), (eggCostWithDino * quantity));
        ...
        while (eggSentCount < quantity && currentIndex < depositedEggIds.length) {
            ...
            if (!eggIDsSold[eggId]) {
                eggToken.safeTransferFrom(address(this), msg.sender, eggId);
    ...
    function withdrawEggs() public onlyOwner
    {
        ...
        // 0 < length of deposited array
        while (currentIndex < depositedEggIds.length) {
            uint256 eggId = depositedEggIds[currentIndex];
            if (!eggIDsSold[eggId]) {
                eggToken.safeTransferFrom(address(this), msg.sender, eggId);
                eggIDsSold[eggId] = true;
        ...
        delete depositedEggIds;
    }
    //requires token approval

    //++
    function depositEggs(uint256[] memory eggIDs) public onlyOwner
    {

        for(uint256 i = 0; i < eggIDs.length; i++){
            //make sure token exists
            require(eggToken.ownerOf(eggIDs[i]) != address(0), "token doesnt exist");
            //pile up the transfers to contract
            eggToken.transferFrom(msg.sender,address(this), eggIDs[i]);
            //push each ID to deposited array
    ...
    /*Dino Badge -- can make for egg purchases or a premier purchase */
    ...
    {
        require(checkIsClaimable(msg.sender), "sorry you havent claimed enough eggs to earn a badge.");
