...
contract SmartVault is ERC721PresetMinterPauserAutoId, Ownable {
    ...
    function createSmartVault(
        ...
        vaultBalance[msg.sender][_vaultId] += swapETHtoRETH();
    ...
    function cancelSmartVault(uint256 _vaultId) public onlyVaultOwner(_vaultId) onlyVaultReady(_vaultId) { 
        ...
        uint256 ethAmount = swapRETHtoETH(vaultBalance[msg.sender][_vaultId]);

        uint256 _processingFee = ethAmount*processingFee/100;
        uint256 _returnDeposit = ethAmount-_processingFee;
    ...
    function swapETHtoRETH() private returns (uint256) {
        // wrap ETH
    ...
    function swapRETHtoETH(uint256 rethAmount) private returns (uint256 wethAmount) {
        // Approve the router to spend rETH.
        TransferHelper.safeApprove(rETH, address(ISwapRouter(_swapRouter)), rethAmount);
        ...
        //Swap from Uniswap Liquidity Pool
        wethAmount = ISwapRouter(_swapRouter).exactInputSingle(params);
        // unwrap ETH   
