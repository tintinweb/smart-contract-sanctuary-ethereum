...
contract JingleBeanz is Ownable {
  ...
  uint256 constant public STEAL_COOLDOWN = 10 minutes; // TODO re-instante before prod  6 hours;

  /// @dev lockout so owner can withdraw and raffle off stray gifts that were never picked up
  uint256 constant public OWNER_WITHDRAW_LOCKOUT = 1 minutes; // TODO re-instante before prod 30 days;
  ...
  function withdrawGift() public onlySubmitted onlyWhenState(State.CLOSED) returns (uint256) {
    ...
    if (isERC721(contractAddress)) {
      IERC721(contractAddress).safeTransferFrom(address(this), msg.sender, withdrawnGiftId);
    } else {
      IERC1155(contractAddress).safeTransferFrom(address(this), msg.sender, withdrawnGiftId, 1, "");
    }
  ...
  /// @notice owner can withdraw gifts after withdraw lockout period (30 days) to raffle off
