...
/// @title LIITA
/// @notice LIITA contract for Particle NFTs representing Love is in the air by Banksy and a surprise drop
...
contract LIITA is ERC721, ERC721Enumerable, ERC2981, Manageable {
    ...
    constructor(string memory liitaURI, string memory ggURI) ERC721("LIITA", "LIITA") {
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        ...
        string memory baseURI = tokenId > 10000 ? GGBaseURI : LIITABaseURI;
        return bytes(baseURI).length > 0 ? string(abi.encodePacked(baseURI, tokenId.toString())) : "";
    }
    ...
    /// @param feeNumerator royalties fee numerator (has to be greater than denominator: 10000)
    ...
    function mint(address[] calldata owners, uint256[][] calldata tokenIds) public onlyManagers {
        ...
        for (uint i = 0; i < owners.length; i++) {
            for (uint j = 0; j < tokenIds[i].length; j++) {
                ...
                // Mint surprise Particle NFT to be revealed
                _safeMint(owners[i], tokenIds[i][j] + 10000);
            }
        }
    }
