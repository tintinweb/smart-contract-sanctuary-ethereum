...
contract OrganisationHelper is Initializable, UUPSUpgradeable, AccessControlUpgradeable {
    ...
    function getOrganisationDetails(address orgRegistry, bytes32 orgId) public view returns (OrganisationDetails memory) {
        ...
        IOrganisationRegistry.DAOConfig memory dao,
        IOrganisationRegistry.YFConfig memory yieldFarming,
        IOrganisationRegistry.VestingConfig memory vesting,
        IOrganisationRegistry.AirdropConfig memory airdrop
        ...
        if (token != address(0)) {
            ...
            details.token = TokenDetails(token, t.name(), t.symbol(), t.decimals());
        }

        if (yieldFarming.staking != address(0)) {
            details.yieldFarmingV1.staking = yieldFarming.staking;

            YieldFarmingV1_Pool memory poolDetails;

            for (uint256 i = 0; i < yieldFarming.pools.length; i++) {
                poolDetails.addr = yieldFarming.pools[i];

                IYieldFarm p = IYieldFarm(poolDetails.addr);
                address[] memory poolTokens = p.getPoolTokens();

                poolDetails.poolTokens = new TokenDetails[](poolTokens.length);

                for (uint256 j = 0; j < poolTokens.length; j++) {
                    IERC20MetadataUpgradeable pt = IERC20MetadataUpgradeable(poolTokens[j]);
                    poolDetails.poolTokens[j] = TokenDetails(poolTokens[j], pt.name(), pt.symbol(), pt.decimals());
                }

                address rewardToken = address(p.rewardToken());
                IERC20MetadataUpgradeable rt = IERC20MetadataUpgradeable(rewardToken);
                poolDetails.rewardToken = TokenDetails(rewardToken, rt.name(), rt.symbol(), rt.decimals());
            }
        }
