...
contract BrandExtenderSuitUp is ERC5169, VerifySignature, IERC721Enumerable, ERC721Enumerable, ERC721URIStorage, UriChanger, DerivedERC2981Royalty {
    ...
    function mintDerivedByService(address erc721, uint256 tokenId, address receiver, uint ticketId, string calldata tmpMetadataURL) external onlyUriChanger payable {
        require(allowedParents[erc721] > 0, "Contract not supported");
        _mintDerivedMulti(erc721, tokenId, receiver, ticketId);
        minter[ticketId] = receiver;
        emit MintedDerived(erc721, tokenId, ticketId, tmpMetadataURL, tokenOfOriginCounter(erc721, tokenId) - 1 );
    }

    function _mintDerivedMulti(address erc721, uint256 tokenId, address to, uint ticketId) internal {
    ...
    function contractURI() public pure returns (string memory) {
        return string(abi.encodePacked(_metadataURI, "/contracts/devcon_suitup.json"));
    }
