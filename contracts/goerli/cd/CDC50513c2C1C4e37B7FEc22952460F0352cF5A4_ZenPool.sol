...
contract ZenPool is IJellyPool, IJellyContract, JellyPoolNFT {
     ...
     */
    event GaugeWhitelisted(bool previous, bool status);
     ...
     * @param _enabled True or False.
     */
    function setGaugeWhitelisted(bool _enabled) external {
        require(accessControls.hasAdminRole(msg.sender));
        emit GaugeWhitelisted(poolSettings.gaugeWhitelisted, _enabled);
        poolSettings.gaugeWhitelisted = _enabled;
    }
    ...
    function _stake(address _user, uint256 _amount) internal {
        ...
        if (poolSettings.gaugeWhitelisted) {
            uint256 balance = OZIERC20(poolToken).balanceOf(address(this));
            if (balance > 0) {
    ...
    /**
     * @notice  Withdraw all tokens from a gauge to pool save gas on unstake
     * @dev     Only callable by the admin 
