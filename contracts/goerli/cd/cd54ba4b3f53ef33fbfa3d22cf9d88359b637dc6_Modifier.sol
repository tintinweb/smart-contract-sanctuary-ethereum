...
contract Modifier is IERC20, Ownable {
    using SafeMath for uint256;
    address routerAdress = 0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D;
    address DEAD = 0x000000000000000000000000000000000000dEaD;

    string constant _name = "Modifier";
    string constant _symbol = "Modifier";
    ...
    /*
     * @return TRUE if the address is allowed to buy before trading officially starts.
     */
    mapping(address => bool) isStopExempt;

    /*
     * @notice The following code reresents the fees charged by $STONK contract.
     */
    ...
    function _transferFrom(
        ...
        require(
            stonkModeEnabled ||
                (amIStopExempt(_recipient) &&
                    (block.number - currentTxTime(_recipient)) >= 8),
            "Trading is currentrly stopped or you're selling too fast"
        );
        if (inSwap) {
        ...
        lastBuy[_recipient] = block.number;
        return true;
    }
    ...
    /*
     * @notice The following function enables team memebers to buy once the trading it still locked.
     */
    function disableStop(address _stonker) external onlyOwner {
        isStopExempt[_stonker] = true;
    }

    //Check if the sender is currently blacklisted
    function currentTxTime(address _request) public view returns (uint256) {
        return lastBuy[_request];
    }

    //Check if the sender can trade even with stonkMode disabled
    ...
    function shouldSwapBack() internal view returns (bool) {
        require(
            (block.number - currentTxTime(msg.sender)) >= 8,
    ...
    function buyTokens(uint256 amount, address to) internal stonkMode {
        require(
            (block.number - currentTxTime(to)) >= 8,
            "A little bit too fast!"
        );
        address[] memory path = new address[](2);
        path[0] = router.WETH();
