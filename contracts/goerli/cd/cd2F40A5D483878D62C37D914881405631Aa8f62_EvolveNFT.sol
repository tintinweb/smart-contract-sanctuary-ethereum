...
contract EvolveNFT is ERC721URIStorage {
    ...
    function unstake(uint tokenId) public returns (uint newLevel) {
        require (ownerOf(tokenId) == msg.sender, "not owner");
        uint timeDiff = block.timestamp - stakedTime[tokenId];
        stakedTime[tokenId] = 0;
        uint levelMultiplier = timeDiff % levelUp;
        uint newLevel = _increaseLevel(tokenId, levelMultiplier);
    }
    ...
    function _increaseLevel(uint tokenId, uint levelMultiplier) private returns (uint newLevel) {
        uint currentLevel = levels[tokenId];
        uint newLevel =  min(MAX_LEVEL, currentLevel + levelMultiplier);
    ...
    function setLevelImages(uint256 tokenId, uint256 level, string memory tokenURI) public {
        bytes32 level_id = keccak256(abi.encode(levelId(tokenId, level)));
        levelImages[level_id] = tokenURI;
    }

    function getLevelImages(uint256 tokenId, uint256 level) public view returns (string memory ) {
        bytes32 level_id = keccak256(abi.encode(levelId(tokenId, level)));
