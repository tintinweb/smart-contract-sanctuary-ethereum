...
import "./interfaces/IActionFundsStorage.sol";
...
contract ActionFundsStorage is IActionFundsStorage, ActionContext {
    using ECDSA for bytes32;

    ContractControlList public accessController;

    constructor (ContractControlList accessController_) {
        accessController = accessController_;
    }

    event ActionFundsReceived(string actionType, string actionId, uint256 tokenId, uint256 amount, address from);
    event FundsWithdrawn(address to, uint256 amount);

    modifier onlyAdminRole() {
        accessController.checkFundsAdminRole(_msgSender());
        _;
    }
    ...
    function withdraw(address payable to_) external override onlyAdminRole {
        require(to_ != address(0), "ActionFundsStorage: Zero address");
    ...
    modifier checkSigDeadline(uint256 sigDeadline_) {
        require(block.timestamp <= sigDeadline_, "ActionFundsStorage: Signature expired");
        _;
    }

    function getPayForTheActionHash(
        ...
        keccak256(
            abi.encodePacked(
                ...
                keccak256(
                    abi.encode(
                        keccak256(
                            "payForTheAction(uint256 nativePrice_, string memory actionType_, string memory actionId_, uint256 tokenId_, uint256 signatureDeadline_, bytes memory signature_)"
                        ),
                        nativePrice_,
                        actionType_,
                        actionId_,
    ...
    * @notice Returns address of message signer.
    ...
    function payForTheAction(
        uint256 nativePrice_, string memory actionType_, string memory actionId_, uint256 tokenId_, uint256 signatureDeadline_, bytes memory signature_
    ) external payable override checkSigDeadline(signatureDeadline_) {
        bytes32 messageHash = getPayForTheActionHash(
            nativePrice_,
            actionType_,
            actionId_,
            tokenId_,
        ...
        accessController.checkActionAdminRole(_getSigner(messageHash, signature_));
        emit ActionFundsReceived(actionType_, actionId_, tokenId_, msg.value, _msgSender());
    }
}
...
interface IActionFundsStorage is IFundsStorage {
    function getPayForTheActionHash(
        uint256 nativePrice_,
        string memory actionType_,
        string memory actionId_,
    ...
    function payForTheAction(
        uint256 nativePrice, string memory actionType, string memory actionId, uint256 tokenId, uint256 signatureDeadline, bytes memory signature
    ) external payable;
}
