...
contract TestToken is Context, IERC20, Ownable {
    ...
    address payable public buybacks;

    uint256 private _tTotal = 516780 * 10**3 * 10**9;

    string private _name = "Test token 1";
    string private _symbol = "Test1";
    uint8 private _decimals = 9;

    uint256 constant public _dividentFee = 25;
    uint256 constant public _buybackFee = 125;
    uint256 constant public _dividendsAllocation = 51678 * 10**3 * 10**9;

    uint256 private _buybackFees;
    ...
    uint256 public _maxWalletHolding = 25839 * 10**3 * 10**9;
    ...
    constructor (address payable _dividendsWallet, address payable _buybackWallet) public {
      dividends = _dividendsWallet;
      buybacks = _buybackWallet;
      ...
      _balances[_dividendsWallet] = _dividendsAllocation;
      _balances[_msgSender()] = _tTotal.sub(_dividendsAllocation);

      emit Transfer(address(0), _msgSender(), _tTotal);
      emit Transfer(_msgSender(), _dividendsWallet, _dividendsAllocation);
    }
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        uint256 buybackBalance = _buybackFees.mul(newBalance).div(contractTokenBalance);
        if (buybackBalance > newBalance) buybackBalance = newBalance;
        uint256 divBalance = newBalance.sub(buybackBalance);

        if (buybackBalance > 0 && buybackBalance <= address(this).balance) buybacks.sendValue(buybackBalance);
    ...
    function _transferStandard(address sender, address recipient, uint256 tAmount, bool takeFee, bool isTransferBuy, bool isTransferSell) private {
        ...
        if (takeFee) {
          uint256 buybackTax = tAmount.mul(_buybackFee).div(1000);
          uint256 dividendsFee = tAmount.mul(_dividentFee).div(1000);
          if (isTransferBuy) {
          ...
          tTransferAmount = tTransferAmount.sub(buybackTax).sub(dividendsFee);
          _buybackFees = _buybackFees.add(buybackTax);
        }
