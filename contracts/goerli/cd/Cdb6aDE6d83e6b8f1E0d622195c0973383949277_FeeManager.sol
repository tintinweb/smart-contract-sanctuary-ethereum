...
import "./interfaces/IDivRouter02.sol";
import "./interfaces/IDivPair.sol";
...
contract FeeManager is Ownable{
    ...
    constructor (IBEP20 _divs, address _teamAddr,
    ...
    function removeLiquidityToToken(address _token) external onlyOwner{
        IDivPair _pair = IDivPair(_token);
    ...
    function swapBalanceToToken(address _token0, address _token1) external onlyOwner {
        require(_token0 != address(divs), "divs can only be burn");
    ...
    function distributeFee() external onlyOwner {
        ...
        if (divs.balanceOf(address(this)) > 0){
            divs.transfer(BURN_ADDRESS, divs.balanceOf(address(this)));
        }
    }
}
...
import './IDivRouter01.sol';

interface IDivRouter02 is IDivRouter01 {
