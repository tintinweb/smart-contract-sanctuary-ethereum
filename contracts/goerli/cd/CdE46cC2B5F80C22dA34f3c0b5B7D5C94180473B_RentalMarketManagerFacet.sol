...
library LibRentalStorage {
    ...
    function setMarketRevenueBalance(bool _removeOrAdd, uint256 _Revenue) internal {
        AppStorage storage s = getStorage();
        if (_removeOrAdd == true) {
            s.rms.marketRevenueBalance += _Revenue;
        } else {
            s.rms.marketRevenueBalance = s.rms.marketRevenueBalance - _Revenue;
        }
    }
    ...
    function setMarketStorage(bool _removeOrAdd, uint256 _amount) internal {
        AppStorage storage s = getStorage();
        if (_removeOrAdd == true) {
            s.rms.marketBalance += _amount;
        } else {
            s.rms.marketBalance = s.rms.marketBalance - _amount;
        }
    }

    function setMarketOverDue(
        uint256 _overDueSafeLineamount,
        uint256 _overDueLimitamount
    ) internal {
        AppStorage storage s = getStorage();
        s.rms.overDueSafeLine = _overDueSafeLineamount;
        s.rms.overDueLimit = _overDueLimitamount;
    }

    function getMarketOverDue()
        internal
        view
        returns (uint256 _overDueSafeLineamount, uint256 _overDueLimitamount)
    {
        AppStorage storage s = getStorage();
        return (s.rms.overDueSafeLine, s.rms.overDueLimit);
    }

    function setMarketCommission(uint256 _newMarketCommission) internal {
        AppStorage storage s = getStorage();
        s.rms.commission = _newMarketCommission;
    }

    function getMarketCommission() internal view returns (uint256 _MarketCommission) {
        AppStorage storage s = getStorage();
        return (s.rms.commission);
    }
    ...
    function getLeftCollateral(
        ...
        AppStorage storage s = getStorage();
        if (s.cs._collection[_collection]._users[_tokenId].renting == true) {
            if (s.cs._collection[_collection]._users[_tokenId].returned == true) {
                return (s.cs._collection[_collection]._users[_tokenId].collateralToBeClaimed -
                    s.cs._collection[_collection]._users[_tokenId].collateralClaimed);
            } else {
                uint256 rentingexpires = s
                    .cs
                    ._collection[_collection]
                    ._users[_tokenId]
                    .rentingexpires;
                ...
                if (
                    rentingexpires + s.rms.overDueSafeLine <= block.timestamp &&
                    block.timestamp <= rentingexpires + s.rms.overDueLimit
                ) {
                    uint256 overDueTime = block.timestamp - rentingexpires;
                    uint256 overDueTimeLeft = s.rms.overDueLimit - overDueTime;
                    uint256 leftCollateralWhole = overDueTimeLeft * collateral;
                    uint256 leftCollateral = SafeMath.div(leftCollateralWhole, s.rms.overDueLimit);
                    return (leftCollateral);
                } else if (block.timestamp > rentingexpires + s.rms.overDueLimit) {
                    return (0);
                } else {
                    return (collateral);
                }
    ...
    function getClaimableCollateral(
        ...
        if (s.cs._collection[_collection]._users[_tokenId].renting == true) {
            ...
            if (
                ...
                uint256 collateralToClaim = SafeMath.div(
                    collateralToClaimWhole,
                    s.rms.overDueLimit
                );
...
contract RentalMarketManagerFacet {
    ...
    function listRental(
        ...
        if (tokenType == 721) {
            (address a, ) = LibRentalStorage.getRenterDetails(collection, tokenId);
            if (a == address(0)) {
                ...
                LibRentalStorage.setRentalStorage(
                    tokenType,
                    collection,
                    msg.sender,
                    tokenId,
    ...
    function rentRental(
        ...
        if (isRenting(collection, tokenId) == true) revert Is__Reting();
        if (rentingPeirodInDays < 1) revert Not_Permit();
        if (tokenType == 721) {
            ...
            if (checkVaild(collection, tokenId, tokenType) == true) {
                ...
                ) = LibRentalStorage.getRentalDetails(collection, tokenId);
                ...
                LibRentalStorage.setRenterStorage(
                    collection,
                    msg.sender,
                    tokenId,
                    rentingexpires,
                ...
                LibRentalStorage.setMarketRevenueBalance(true, rentalFee - afterCommission);
            }
        }
    ...
    function returnRental(
        ...
        if (isRenting(collection, tokenId) == false) revert Not_Renting();
        if (tokenType == 721) {
            (address renter, ) = LibRentalStorage.getRenterDetails(collection, tokenId);
            if (IERC721(collection).ownerOf(tokenId) != msg.sender)
                revert You_DontHaveTheNftThatIsGoingToReturn();
            if (IERC721(collection).isApprovedForAll(msg.sender, address(this)) == false)
                revert Not_Approved();
            (address rentalowner, uint256 collateral, , , , ) = LibRentalStorage.getRentalDetails(
                collection,
                tokenId
            );
            if (renter != msg.sender) revert Not_Owner();
            IERC721(collection).transferFrom(msg.sender, rentalowner, tokenId);
            uint256 leftCollateral = LibRentalStorage.getLeftCollateral(collection, tokenId);
            if (collateral == leftCollateral) {
                (bool callSuccess, ) = payable(msg.sender).call{value: leftCollateral}("");
                require(callSuccess, "Call failed");
                LibRentalStorage.setRenterStorage(collection, address(0), tokenId, 0, false);
            } else {
                if (leftCollateral > 0) {
                    (bool callSuccess, ) = payable(msg.sender).call{value: leftCollateral}("");
                    require(callSuccess, "Call failed");
                }
                LibRentalStorage.setRenterStorage(collection, address(0), tokenId, 0, true);
                (, , uint256 collateralToBeClaimed) = LibRentalStorage.getCollateralDetails(
                    collection,
                    tokenId
                );
                LibRentalStorage.setToBeClaimed(
                    collection,
                    tokenId,
                    collateralToBeClaimed + collateral - leftCollateral
                );
                LibRentalStorage.setReturned(collection, tokenId, true);
            }
        }
        if (tokenType == 1155) {} else {}
    }
    ...
    function claimCollateral(address collection, uint256 tokenId) external noReentrant {
        (address rentalowner, , , , , ) = LibRentalStorage.getRentalDetails(collection, tokenId);
        (, uint256 collateralClaimed, uint256 collateralToBeClaimed) = LibRentalStorage
            .getCollateralDetails(collection, tokenId);
        if (msg.sender != rentalowner) revert Not_Owner();
        if (collateralToBeClaimed == 0) {
            uint256 collateralClaimable = LibRentalStorage.getClaimableCollateral(
                collection,
                tokenId
            );
            uint256 amountToClaim = collateralClaimable;
            if (amountToClaim == 0) revert Not_EnoughCollateral();
            LibRentalStorage.setClaimed(collection, tokenId, collateralClaimed + amountToClaim);
            ...
            if (collateralToBeClaimed > collateralClaimed) {
                ...
                LibRentalStorage.setClaimed(collection, tokenId, collateralToBeClaimed);
                (bool callSuccess, ) = payable(msg.sender).call{value: amountToClaim}("");
                require(callSuccess, "Call failed");
                LibRentalStorage.setRenterStorage(collection, address(0), tokenId, 0, false);
                LibRentalStorage.setReturned(collection, tokenId, true);
    ...
    function calculateComssion(uint256 rentalFee) internal view returns (uint256) {
        uint256 commission = LibRentalStorage.getMarketCommission();
    ...
    function isRental(address collection, uint256 tokenId) public view returns (bool) {
        return LibRentalStorage.isRental(collection, tokenId);
    }

    function isRenting(address collection, uint256 tokenId) public view returns (bool) {
        return LibRentalStorage.isRenting(collection, tokenId);
    }
...
struct RentalMarketStorage {
    ...
    uint256 overDueSafeLine;
    uint256 overDueLimit;
    uint256 commission;
}
