...
contract UiPoolDataProvider is IUiPoolDataProvider {
    ...
    function _fillUserReserveData(
        ...
        userReserveData.cTokenBalance = ICToken(baseData.cTokenAddress)
            .balanceOf(user);
    ...
    function _fillNftData(
        ...
        (
            ...
            nftData.buyItNowDuration,
            nftData.redeemFine,
            nftData.redeemThreshold
        ) = baseData.configuration.getBuyItNowParamsMemory();
        (nftData.isActive, nftData.isFrozen) = baseData
            .configuration
            .getFlagsMemory();
        nftData.minBuyItNowFine = baseData
            .configuration
            .getMinBuyItNowFineMemory();
    }
    ...
    function getSimpleLoansData(
        ...
        for (uint256 i = 0; i < nftAssets.length; i++) {
            ...
            // NFT buy-it-now sale data
            (
                ,
                loanData.buyItNowBuyerAddress,
                loanData.buyItNowPrice,
                loanData.buyItNowBorrowAmount,
                loanData.buyItNowFine
            ) = lendPool.getNftBuyItNowData(nftAssets[i], nftTokenIds[i]);
        }
...
contract InterestRate is IInterestRate {
     ...
     * @param reserve The address of the reserve
     * @param availableLiquidity The liquidity available in the corresponding cToken
     * @param totalVariableDebt The total borrowed from the reserve at a variable rate
     * @param reserveFactor The reserve portion of the interest that goes to the treasury of the market
