...
contract Piggy is Ownable, ReentrancyGuard, Pausable {
    ...
    function deposit(address _token) public payable onlyAllowedToken(_token) {
        ...
        if (_token != address(0)) {
            ...
            value = msg.value;
            tokenBalances[msg.sender][_token] += value;
        } else {
        ...
        emit Deposit(
            msg.sender,
            value,
            _token == address(0) ? address(0) : _token,
            block.timestamp
        );
    }
    ...
    function withdrawToken(
        address _token,
        uint256 _amount
    ) public whenNotPaused nonReentrant onlyOwner onlyAllowedToken(_token) {
