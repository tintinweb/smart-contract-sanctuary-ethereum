    ...
    PausableUpgradeable
{
    ...
    uint256 public override totalAddedAmountForRewards;
    ...
    function addRewards(uint256 amountToAdd_) external override {
        ...
        totalAddedAmountForRewards += amountToAdd_;

        emit RewardsAdded(msg.sender, amountToAdd_);
    }
        ...
        returns (NFTInfo[] memory nftInfoArr_)
    {
        ...
        for (uint256 i = 0; i < tokenIds_.length; i++) {
            if (!_exists(tokenIds_[i])) {
                nftInfoArr_[i].tokenId = type(uint256).max;
                continue;
            }
