    ...
    OwnableUpgradeable 
{
    ...
    function _beforeTokenTransfer(
        ...
        _beforeTokenTransfer(from, to, ids[0]);
    } 
...
abstract contract Dividends {
    ...
    function payDividends(uint256 id) public isExist(id) virtual payable {
        _totalDividends[id] += msg.value / tokenSupply(id);
        emit PayDividends(id, msg.value);
    }
    ...
    function withdrawDividends(
        ...
        unchecked {
            _dividendPayments[id][msg.sender] = dividendPayment + coef; 
        }
    ...
    function _beforeTokenTransfer(
        ...
        if (from != address(0)){ 
            _assignDividends(id, payable(from));
            _assignDividends(id, payable(to));
        }
    } 
    ...
    function _assignDividends(
        ...
        uint256 coef;
        unchecked {
            coef = _totalDividends[id] - _dividendPayments[id][holder];
        }
    ...
    function _sendDividends(
        ...
        if (dividendAmount > 0) {
            recipient.call{value: dividendAmount}('');
