...
contract Exchanger is Owned, MixinSystemSettings, IExchanger {
    ...
    function updateDestinationForExchange(
        ...
        if (fee > 0) {
            fee = exchangeRates().effectiveValue(destinationKey, fee, sUSD);
    ...
    function _convert(
        ...
        issuer().synths(args.sourceCurrencyKey).burn(args.fromAccount, sourceAmountAfterSettlement);

        if (args.destChainId == 0) {
            ISynth dest = issuer().synths(args.destCurrencyKey);

            dest.issue(args.destAccount, amountReceived);
