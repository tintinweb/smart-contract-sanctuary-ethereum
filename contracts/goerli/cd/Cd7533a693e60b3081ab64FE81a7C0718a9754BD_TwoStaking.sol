...
contract TwoStaking is ERC20Burnable, Ownable {
    ...
    uint256 public CLAIM_END_TIME = 1741013200;
    ...
    //Mapping of staker to wolfgang
    mapping(address => uint256[]) internal stakerToTokenIds;

    constructor() ERC20("NameToken", "NMT") {}
    ...
    function stakeByIds(uint256[] memory tokenIds) public {
        ...
        require(
            stakerToTokenIds[msg.sender].length + tokenIds.length <=
                MAX_WALLET_STAKED,
            "Must have less than 11 Wolfgang staked!"
        );

        for (uint256 i = 0; i < tokenIds.length; i++) {
            require(
                IERC721A(nftAddress).ownerOf(tokenIds[i]) == msg.sender &&
                    tokenIdToStaker[tokenIds[i]] == nullAddress,
                "Token must be stakable by you!"
            );

            IERC721A(nftAddress).transferFrom(
                msg.sender,
                address(this),
                tokenIds[i]
            );
