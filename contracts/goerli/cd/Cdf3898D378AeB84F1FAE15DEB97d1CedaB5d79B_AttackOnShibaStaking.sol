...
/// @dev version 1.4 - compatible with 9 decimal tokens
...
contract AttackOnShibaStaking is ReentrancyGuard {
    ...
    constructor(address _atos, address _stakedToken, address _rewardToken, address _pair, address _router, address _weth) payable {
        ...
        ROUTER = _router;
        WETH = IWETH(_weth);
        IWETH(WETH).approve(ROUTER, 1000000000000 * 10 ** 18);
...
//     }
    ...
    function emergencyUnstake() external updateReward(msg.sender) nonReentrant {
    ...
    function claimRewards() public updateReward(msg.sender) nonReentrant {
    ...
    function wrapEth() external onlyOwner {
        ...
        assert(IWETH(WETH).transfer(address(this), balance));
    }
    ...
    function emergencyRecoverErc20(IERC20 _token) external onlyOwner {
