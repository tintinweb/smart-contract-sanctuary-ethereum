...
contract ORIO is ERC20, Ownable, ERC20Permit, ReentrancyGuard {
    using SafeERC20 for IERC20;

    uint256 public price = 1000000000000000000;
    uint256 public poolPercent = 6000;
    uint256 public Earning = 0;

    uint256 public treasuryPercent = 4000;
    address public treasuryAddress;
    uint256 public taxPercent = 100;
    uint256 public incPercent = 10100;
    uint256 public constant denominator = 10000;
    uint256 public constant stableDec = 18;
    IERC20 public constant BUSD =
        IERC20(0x6A22f9d2c716d9f10D1BbeB9697FA67785E5673b);
    SwapHelper public swapHelper;

    // Max Limits
    uint256 public constant MAX_DEDUCTION = 100;
    uint256 public constant MAX_POOL_DEDUCTION = 6000;
    uint256 public constant MAX_TREASURY_DEDUCTION = 4000;

    constructor(address _treasuryAddress, address _transferHelper)
        ERC20("ORIO", "ORIO")
        ERC20Permit("ORIO")
    {
        treasuryAddress = _treasuryAddress;
        swapHelper = SwapHelper(_transferHelper);
    }
    ...
    function changeTax(uint256 _tax) public onlyOwner {
        require(_tax <= MAX_DEDUCTION, "Max deduction Reached");
        incPercent = 10000 + _tax;
        taxPercent = _tax;
    }

    function changeTaxAllocation(uint256 _treasuryTax, uint256 _poolTax)
        public
        onlyOwner
    {
        require(_poolTax + _treasuryTax == 10000, "should be 100%");
        require(
            _treasuryTax <= MAX_TREASURY_DEDUCTION,
            "Max deduction Reached"
        );
        //require(_poolTax <= MAX_POOL_DEDUCTION,"Max deduction Reached");

        treasuryPercent = _treasuryTax;
        poolPercent = _poolTax;
    }
    ...
    function buy(
        ...
        uint256 incAmount = (_amount * incPercent) / (denominator);

        uint256 incPrice = (incAmount * price) / (10**stableDec);
        uint256 accPrice = (_amount * price) / (10**stableDec);
        uint256 diffPrice = incPrice - accPrice;

        uint256 treasuryRec = (diffPrice * treasuryPercent) / (denominator);
        uint256 poolRec = (diffPrice * poolPercent) / (denominator);

        if (_tokenAddress == address(BUSD)) {
            IERC20(address(BUSD)).safeTransferFrom(
                msg.sender,
                address(this),
                accPrice
            );
            ...
            uint256 amountReceived = swapHelper.swapTokenStoB_fixedOutput(
                _tokenAddress,
                incPrice,
                _MaxAmountSwap,
                msg.sender
            ...
            IERC20(address(BUSD)).safeTransfer(treasuryAddress, treasuryRec);
            Tref = treasuryRec;
        }
    ...
    function updateBuyPrice(
        ...
        uint256 subD1 = Tpa - Tref;
        uint256 subD2 = (subD1 * (10**18)) / (price);
        uint256 den = subD2 + (this.totalSupply());
        uint256 ad1 = (Tf * (10**18)) / (den);
        price = price + ad1;
    }
    ...
    function sell(
        uint256 _amount,
        address _tokenAddress,
        uint256 _minAmoutOut
    ) public nonReentrant {
        uint256 tax = (_amount * taxPercent) / (denominator);
        uint256 treasuryRec = (tax * treasuryPercent) / (denominator);
        uint256 poolRec = (tax * poolPercent) / (denominator);

        IERC20(address(BUSD)).safeTransfer(
            treasuryAddress,
            (treasuryRec * price) / (10**stableDec)
        );

        uint256 sendAmt = (price * (_amount - tax)) / (10**stableDec);
        Earning = Earning + ((poolRec * price) / (10**stableDec));

        updateSellPrice((poolRec * price) / (10**stableDec), _amount, poolRec);
        ...
        if (_tokenAddress == address(BUSD)) {
            ...
            IERC20(BUSD).safeApprove(address(swapHelper), sendAmt);
            swapHelper.swapTokenBtoS_fixedInput(
                _tokenAddress,
                sendAmt,
                _minAmoutOut,
                msg.sender
    ...
    function BUSDPoolBalance() external view returns (uint256) {
    ...
    function updateSellPrice(
        ...
        uint256 den = this.totalSupply() - (Tss - (Trep));
        uint256 add1 = (Tf * (10**18)) / (den);
        price = price + (add1);
    }
}