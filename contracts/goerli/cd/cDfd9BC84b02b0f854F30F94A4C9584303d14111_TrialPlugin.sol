...
contract TrialPlugin is IPlugin, Ownable {
  using Strings for uint256;
  using ECDSA for bytes32;

  // events
  event Enrolled(address indexed user);
  event Credited(address indexed user, uint256 duration, uint256 amount);
  event Redeemed(address indexed user, uint256 duration, uint256 amount);
  ...
  /**
   * @param duration_ free trial period in seconds (set 0 to disable public enrollment)
   * @param amount_ free trial shares amount
   * @param controller_ address of controller for issuing trial credits
  ...
  constructor(
    ...
    require(amount_ > 0, "TrialPlugin: amount is zero");
  ...
  function metadata(uint256 pclass) external view returns (string memory) {
    require(pclass == 1, "TrialPlugin: invalid class id");

    return
      string(
        abi.encodePacked(
          "Trial plugin: ",
          (duration / 86400).toString(),
          " days"
        )
      );
  }
    ...
    returns (uint256)
  {
    ...
    Trial storage t = trials[user];

    if (block.timestamp > t.end) return 0;

    return t.amount;
  }

  /**
   * @notice enroll in public free trial. This can only be done once!
   */
  function enroll() external {
    require(duration > 0, "TrialPlugin: public trial is disabled");
    require(trials[msg.sender].amount == 0, "TrialPlugin: trial already used");

    trials[msg.sender] = Trial(
      uint192(amount),
      uint64(block.timestamp + duration)
    );

    IMembership(membership).update(msg.sender);

    emit Enrolled(msg.sender);
  }

  /**
   * @notice admin assignment of free trial
   */
  function credit(
    address user_,
    uint256 duration_,
    ...
    require(
      ...
      "TrialPlugin: caller is not the controller"
    );
    require(duration_ > 0, "TrialPlugin: duration is zero");
    require(amount_ > 0, "TrialPlugin: amount is zero");

    trials[user_] = Trial(
      uint192(amount_),
      uint64(block.timestamp + duration_)
    );

    IMembership(membership).update(user_);

    emit Credited(user_, duration_, amount_);
  }

  /**
   * @notice redeem signature for free trial
   */
  function redeem(
    uint256 duration_,
    uint256 amount_,
    ...
    require(
      keccak256(
        abi.encode(msg.sender, duration_, amount_, trials[msg.sender].end)
      ).toEthSignedMessageHash().recover(signature_) == controller,
      "TrialPlugin: invalid signature"
    );
    ...
    emit Redeemed(msg.sender, duration_, amount_);
  }

  /**
   * @notice set new controller for issuing of trial credits
   * @param controller_ address of new controller
   */
  function setController(address controller_) external onlyOwner {
    require(
      controller_ != address(0),
      "TrialPlugin: controller address is zero"
    );
