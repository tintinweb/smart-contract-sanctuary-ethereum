...
//import "@ermine/contracts/myToken.sol";
import "@ermine/contracts/nft_3.sol";

contract app is ERC1155Receiver, Ownable {
    //uint256 public totalETH = 0;
    //uint256 public totalToken = 0;
    //uint256 sumTax = 0 ether;
    address AddressEVM;
    uint[] public inAppVM = new uint256[](12);
    ...
    constructor(address _AddressEVM){
       // mycoin = myToken(_tokenAddress);
        AddressEVM = _AddressEVM;
        miners = nft_3(AddressEVM);
        for (uint i = 0; i < 12; i++) {
            inAppVM[i] = 0;
        }
    }
    //For onlyOperator
    // modifier onlyErmi() {
    // require(msg.sender == addressErmi);
    // _;                              
    // } 
    ...
    //     addressErmi = operator;
    // }

    // function addLiq(uint256 amount, uint256 value) external onlyErmi {
    //     totalToken += (amount * 1e18);
    //     totalETH += value;
    // }
...
//View my balance
    function addNFT(uint256 _typeNFT, uint256 _value) external {
    miners.safeTransferFrom(msg.sender, address(this), _typeNFT, _value, "");
    if (_users[msg.sender].myNFT.length < 1) {
    for (uint i = 0; i < 12; i++) {
            _users[msg.sender].myNFT.push(0);
        }
    }
    _users[msg.sender].myNFT[_typeNFT] += _value;
    }

//gift
    function giftNFT(uint256 _typeNFT, uint256 _value) external {
    miners.safeTransferFrom(address(this), msg.sender, _typeNFT, _value, "");
    }

    function getMyNFT(address myadd) external view returns (uint256[] memory) {
    return _users[myadd].myNFT;
    }
    ...
    function setURIEVM(string memory newuri) public onlyOwner {
    miners.setURI(newuri);
}

function burnMyEMV(uint evm_id, uint256 amount) external {
    miners.burnMyEVM(evm_id, amount);
}
}
...
contract nft_3 is ERC1155Supply, Ownable  {
...
function setErmineStore(address _ErmineStore) external onlyOwner {
    ...
    _admin[addressErmineStore].sys = true;
}
...
function RemoveAdmin() public onlyAdmin {
     require(_admin[msg.sender].sys, "You cannot relinquish admin rights as you are not an admin!");
    _admin[msg.sender].sys = !_admin[msg.sender].sys;
}
...
function checkAdmin(address checkWal) public view returns (bool) {
    return _admin[checkWal].sys;
}
