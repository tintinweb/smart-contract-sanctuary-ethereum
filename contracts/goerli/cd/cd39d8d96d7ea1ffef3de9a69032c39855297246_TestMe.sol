...
contract TestMe {
    ...
    function bign(uint256 easy1, bytes memory easy) external {
        emit TestKMS(easy1);
        emit TestKYS(easy);
    }

    function aign(bytes memory easy) external {
        emit TestKYS(easy);
    }

    function tign(bytes memory easy1, bytes memory easy2) external {
        emit TestKYS(easy1);
        emit TestKYS(easy2);
    }

    function nign(Alpha[] memory test) external {
        emit TestAlp(test);
    }
