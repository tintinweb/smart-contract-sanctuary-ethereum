...
//                                                /\___/
// Special thanks to Hashlips

// nov 16 2022
// -presale por merkle tree (whitelistMerkleRoot, merkleproof)
// -contador de transacoes (addrThatTransacted)
...
contract borgsvsmutes is ERC721, Ownable, DefaultOperatorFilterer {
    ...
    //bloqueio para apenas uma transacao por carteira
    address[] private addrThatTransacted;

    constructor() ERC721("Borg vs Mutes", "BVM") {
        setHiddenMetadataUri(
            "ipfs://bafkreid6cqrfron23oakhp5zhxeoicpxv3asrz44nqrlsco7z5t2i6lkgu"
        );
    }
    ...
    //--------^---------    garantindo royalty pela opensea/////////////////////
    ...
    function isInList(bytes32[] calldata merkleProof)
        public
        view
        returns (bool)
    {
    ...
    function resgate() public onlyOwner {
        ...
        (bool magro, ) = payable(0xDbAcb4e7F280859bb9b6F4741aCF8Db8F25ddbD2)
            .call{value: (balanco * 45) / 100}("");
        require(magro);

        (bool eu, ) = payable(0x682575665500B067BB4D2B2EF3F0f0Cb89D48C27).call{
            value: (balanco * 45) / 100
        }("");
        require(eu);
    ...
    /////////////////// avoiding same wallet transactiong twice ------ beggining
