    ...
    Ownable
{
    ...
    /// @dev Mint configurations Max Supply    
    uint256 public maxSupply = 1000;

    /// @dev Mint configurations Total Supply    
    ...
    constructor() {
        ...
        mintedPerWallet[0][msg.sender] += 30;
        totalSupply += 30;
    }
    ...
    function mint(uint256 tokenId, uint256 qty) external payable {
        ...
        if ( mintedPerToken[tokenId] + qty > config.maxSup) revert MintEnded();
    ...
    function mint(
        ...
        if ( mintedPerToken[tokenId] >= config.maxSup) revert MintEnded();
    ...
    function airdrop(
        ...
        for (uint256 i = 0; i < recipients.length; i++) {
            ...
            if(tokenId == 0 ) totalSupply += amounts[i];        
        }
    }
