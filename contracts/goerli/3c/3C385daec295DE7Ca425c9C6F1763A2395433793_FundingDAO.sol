...
contract FundingDAO is ReentrancyGuard, AccessControl {
  ...
  event Payment(address indexed stakeholder, address indexed projectAddress, uint256 amount);
  ...
  function createProposal(
    ...
    require(msg.value == 5 * 10**18, "You need to add 5 MATIC to create a proposal");
    ...
    onlyStakeholder("Only Stakeholders can vote on a proposal.")
  {
    ...
    for (uint256 i = 0; i < votes[msg.sender].length; i++) {
      if (proposal.id == votes[msg.sender][i]) revert("You can only vote once.");
    }
    ...
    onlyStakeholder("Only Stakeholders can make payments")
  {
    ...
    if (proposal.totalFundRaised >= proposal.amount) revert("Required funds are provided.");
