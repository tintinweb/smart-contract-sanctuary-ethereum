...
contract UniswapExample {
    ...
    event TokenSwapEvent(uint[] amounts);

    constructor() {
        uniswapRouter = IUniswapV2Router02(UNISWAP_ROUTER_ADDRESS);
    }

    function swapTokens(uint256 _tokenAmount, address _tokenIn, address _tokenOut) public payable {
        address[] memory path;
        path = new address[](2);
        path[0] = _tokenIn;
        path[1] = _tokenOut;
        ...
        IERC20(_tokenIn).approve(address(uniswapRouter), _tokenAmount);

        uint amountOut = uniswapRouter.getAmountsOut(
            _tokenAmount,
            path
        )[1];
