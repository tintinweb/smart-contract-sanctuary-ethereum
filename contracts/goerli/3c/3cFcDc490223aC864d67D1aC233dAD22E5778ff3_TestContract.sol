...
contract TestContract is ERC721A, Ownable, ReentrancyGuard {
     ...
     */
    address public constant SEA_TURTLE_FOUNDATION_WALLET = 0x8492AfE3Abf3a0275E8e7514585893cFE3907577;
    ...
    function OGMint(uint256 quantity, bytes32[] calldata merkleProof) external payable { 
        require(_isValid(freeMintMerkleRoot, merkleProof, msg.sender), "Not on the OG list"); 
        require(totalSupply() + quantity < (maxSupply + 1), "Quantity of tokens exceeds maxSupply");

        // Calculate costs including freeMint if still available
     ...
     * @param _freeMintMerkleRoot The new Merkle Root for the whitelist
     */
    ...
    function withdraw(address receiver) external onlyOwner nonReentrant {
        ...
        (bool turtleSuccess,) = payable(SEA_TURTLE_FOUNDATION_WALLET)
