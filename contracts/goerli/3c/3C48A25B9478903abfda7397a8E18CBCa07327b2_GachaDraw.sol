...
contract GachaDraw is Ownable {
    ...
    uint256 public wonRewards;
    uint256 public completedDraws;
        ...
        validTimestamp(_startTimestamp, _endTimestamp)
    {
        ...
        for (uint256 index = 0; index < _rewardTokenIds.length; index++) {
            ...
            if (
                _rewardTokenIds[index] > maxTokenId ||
        ...
        activities[totalActivities] = Activity({
            Id: totalActivities,
            starstAt: _startTimestamp,
            endsAt: _endTimestamp,
            activityType: _activityType,
            isActive: true,
            mintedSupply: new uint256[](_maxRewardSupply.length),
            maxSupply: _maxRewardSupply,
            remainingSupply: remainingSupply,
            rewardIds: _rewardTokenIds
        });
    ...
    function setActivityTimestamp(
        ...
        validTimestamp(_startTimestamp, _endTimestamp)
    {
        activities[_activityId].starstAt = _startTimestamp;
        activities[_activityId].endsAt = _endTimestamp;
    }
    ...
    function updateMaximumRewardSupply(
        ...
        if (
            _maxRewardSupply.length != activities[_activityId].rewardIds.length
        ) {
            revert InvalidInputLength();
        }
        ...
        for (uint256 index = 0; index < _maxRewardSupply.length; index++) {
            if (
                ...
                activities[_activityId].mintedSupply[index]
            ) {
                revert InvalidInput(
            ...
            totalSupplyInCirculation += activities[_activityId].mintedSupply[
                index
            ];
            newMaxRewardSupply = newMaxRewardSupply + _maxRewardSupply[index];
            activities[_activityId].maxSupply[index] = _maxRewardSupply[index];
        }
        ...
        validActivity(_activityId)
    {
        ...
        if (
            block.timestamp < activities[_activityId].starstAt ||
            block.timestamp > activities[_activityId].endsAt
        ) {
        ...
        for (uint256 i = 0; i < _ticketAmount; i = _uncheckedInc(i)) {
            ...
            for (
                uint256 j = 0;
                j < activities[_activityId].rewardIds.length;
                j = _uncheckedInc(j)
            ) {
                uint256 remaining = activities[_activityId].maxSupply[j] -
                    activities[_activityId].mintedSupply[j];

                unchecked {
                    indexCount += remaining;
                }
                // indexCount += _uncheckedIncDelta(indexCount, remaining);
                if (remaining > 0 && indexCount >= randomIndex) {
                    selectedId = activities[_activityId].rewardIds[j];
                    activities[_activityId].mintedSupply[j] = _uncheckedInc(
                        activities[_activityId].mintedSupply[j]
                    );
                    break;
                }
    ...
    {
        return activities[_activityId].mintedSupply;
    }
    ...
    function getMaximumRewardSupply(uint256 _activityId)
        ...
        returns (uint256[] memory)
    {
        return activities[_activityId].maxSupply;
    }
    ...
    function getRewardTokenIds(uint256 _activityId)
        ...
        returns (uint256[] memory)
    {
        return activities[_activityId].rewardIds;
    }
