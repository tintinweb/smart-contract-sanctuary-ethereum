...
contract TokenSwap {
  ...
  ISETH public seth;
  int96 public constant DEFAULT_FLOW = 3858024691; // 0.01 eth / month

  constructor(ISuperfluid host, address _seth) {
    seth = ISETH(_seth);
  ...
  function sethSwap() external payable {
    seth.upgradeByETH{value: msg.value}();
  }

  function createFlow() public {
    cfaV1.createFlow(msg.sender, seth, DEFAULT_FLOW);
  }
}
