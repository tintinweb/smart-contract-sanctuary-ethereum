...
contract ERC721A is IERC721A {
    ...
    function _mintERC2309(address to, uint256 quantity) internal virtual {
        ...
        if (to == address(0)) revert ();
        if (quantity == 0) revert ();
        if (quantity > _MAX_MINT_ERC2309_QUANTITY_LIMIT) revert ();
...
/// @title TestMint
/// @author TestMint
/// @notice Standard ERC721A with a minting mechanism that allows for a merkle whitelist. Mint with us at https://Testmint.com!
/// @custom:security-contact [email protected]
contract TestMint is ERC721A, Ownable, ReentrancyGuard, PaymentSplitter {
    ...
    // @dev TestMint platform data.
    TestMintData private _TestMintData;
    ...
    function mint(uint16 _quantity, bytes32[] memory _proof) public payable nonReentrant {
        require(getPrice() * _quantity == msg.value, "TestMint_ETH_VALUE_INCORRECT");

        // Retrieve in memory the token data for maximums.
        uint16 _totalSupply = _TestMintData.supplyTotal;
        uint16 _maxPerWallet = _TestMintData.maxPerWallet;
        uint16 _maxPerTx = _TestMintData.maxPerTx;

        // Ensure the user has not exceeded the maximums.
        require(_quantity <= _maxPerTx, "TestMint_EXCEEDS_MAX_PER_TX");
        require(totalSupply() + _quantity <= _totalSupply, "TestMint_EXCEEDS_TOTAL_SUPPLY");

        // Retrieve the current sale status.
        bool _saleActive = _TestMintData.saleActive;
        bool _preSaleActive = _TestMintData.preSaleActive;
        ...
        if (_mintState == MintState.Public) {
            require(_saleActive, "TestMint_SALE_NOT_ACTIVE");
        } else {
            require(_preSaleActive, "TestMint_PRE_SALE_NOT_ACTIVE");
        }

        // Ensure the user has not exceeded the maximum per wallet.
        require(hasMinted[msg.sender] + _quantity <= _maxPerWallet, "TestMint_EXCEEDS_MAX_PER_WALLET");

        // If the sale is private, ensure the user is whitelisted.
        
            require(_verifyWhitelist(msg.sender, _proof), "TestMint_NOT_WHITELISTED");
    ...
    function reserve(address _address, uint16 _quantity) public nonReentrant onlyOwner {
        require(totalSupply() + _quantity <= _TestMintData.supplyTotal, "TestMint_EXCEEDS_TOTAL_SUPPLY");
    ...
    function readData() public view returns (TestMintData memory) {
        return _TestMintData;
    }

    function getPrice() public view returns (uint72) {
        if (_TestMintData.preSaleActive) {
            return _TestMintData.preSalePrice;
        } else {
            return _TestMintData.salePrice;
        }
    }
    ...
    function getSaleActive() public view returns (bool) {
        return _TestMintData.saleActive;
    }
    ...
    function updateData(uint16 _supplyTotal, uint16 _maxPerWallet, uint16 _maxPerTx, uint72 _preSalePrice, uint72 _salePrice) public onlyOwner {
        require(_supplyTotal >= totalSupply(), "TestMint_MANAGE_ERR_SUPPLY_LESS_THAN_TOTAL_SUPPLY");

        _TestMintData.supplyTotal = _supplyTotal;
        _TestMintData.maxPerWallet = _maxPerWallet;
        _TestMintData.maxPerTx = _maxPerTx;
        _TestMintData.preSalePrice = _preSalePrice;
        _TestMintData.salePrice = _salePrice;
    }

    // @dev Change the mint state.
    function setState(MintState _mint, bool _presale, bool _sale) public onlyOwner {
        if (_presale) {
            require(!_sale, "TestMint_MANAGE_ERR_SALE_AND_PRE_SALE_ACTIVE");
        } else if (_sale) {
            require(!_presale, "TestMint_MANAGE_ERR_SALE_AND_PRE_SALE_ACTIVE");
        }

        _mintState = _mint;
        _TestMintData.preSaleActive = _presale;
        _TestMintData.saleActive = _sale;
    }
}