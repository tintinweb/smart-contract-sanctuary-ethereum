...
abstract contract Account is IAccount, Ownable2Step, ReentrancyGuard {
	...
	function _AffiliateCreator() private view returns (bytes16 affiliate_) {
		while (true) {
			affiliate_ = AffiliateCreator.Create(8);
...
contract Tuktu is ITuktu, Balance {
	...
	constructor(uint _Starting) Balance(_Starting) {}

	function Initialize(IXProgram _XProgram, IMatrix _Matrix) public onlyOwner {
		XPrograms = _XProgram;
		Matrixes = _Matrix;
	}
