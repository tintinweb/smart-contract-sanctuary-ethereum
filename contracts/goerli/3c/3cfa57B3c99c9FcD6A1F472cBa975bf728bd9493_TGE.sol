    ...
    ITGE
{
    ...
    function initialize(
        ...
        require(false, "Revered 2");

        require(
            info.hardcap >= IPool(token.pool()).service().getMinSoftCap(),
            "Hardcap is less than minSoftCap"
        );

        require(
            info.hardcap +
                IPool(token.pool()).service().getProtocolTokenFee(
                    info.hardcap
                ) <=
                remainingSupply,
    ...
    function claimProtocolTokenFee() private onlyState(State.Successful) {
        ...
        token.mint(
            IPool(token.pool()).service().protocolTreasury(),
            IPool(token.pool()).service().getProtocolTokenFee(_totalPurchased)
        );
