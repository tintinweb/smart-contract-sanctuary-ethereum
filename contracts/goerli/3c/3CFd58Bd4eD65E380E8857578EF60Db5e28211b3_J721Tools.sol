...
contract J721Tools is Ownable, SpecialTransferHelper, ReentrancyGuard {
    ...
    address public constant GOV = 0xf03d320deD1058E21208d0e5B5C67213AFeB74E0; // temporary address
    ...
    function fillOrder(
        ...
        {
            nft.safeTransferFrom(msg.sender, offerOrder.offerer, tokenId);

            uint256 _price = offerOrder.price;

            if (baseFees != 0) {
                uint256 _fee = (offerOrder.amount * baseFees) / 100;
                // TODO: DO NOT TRANSFER assets to contract address
                token.safeTransferFrom(offerOrder.offerer, address(this), _price);
                _price = _price - _fee;
            }
            token.safeTransferFrom(offerOrder.offerer, msg.sender, _price);
    ...
    function aggregate(Call[] memory calls) public payable returns (uint256 blockNumber, bytes[] memory returnData) {
        ...
        for (uint256 i = 0; i < calls.length; i++) {
            (bool success, bytes memory ret) = calls[i].target.call{value: calls[i].amount}(calls[i].callData);
