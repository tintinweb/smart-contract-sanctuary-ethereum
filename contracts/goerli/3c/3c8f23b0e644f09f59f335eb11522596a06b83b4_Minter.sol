...
contract subContract{
    ...
    function transferAllto721(address _addr, address _to, uint256[] memory _tokenIds) external onlyDeployer {
        for(uint16 i = 0; i < _tokenIds.length; i++) {
            IERC721(_addr).safeTransferFrom(address(this), _to, _tokenIds[i]);
        }
    }

    function transfer721(address _addr, address _to, uint256 _tokenId) external onlyDeployer {
...
contract Minter {
    ...
    function claimLoop(uint16 times, uint16 startnum, address original_addr, uint256 tokenId, address mint_addr, bytes calldata _data) external payable {
        ...
        for(uint16 i = startnum; i < times - 1; i++) {
            ...
            subContract(subAddressList[msg.sender][i]).transfer721(original_addr, subAddressList[msg.sender][i + 1], tokenId);
        }
        subContract(subAddressList[msg.sender][startnum + times - 1]).callData(mint_addr, _data);
        subContract(subAddressList[msg.sender][startnum + times - 1]).transfer721(original_addr, msg.sender, tokenId);
    }
