...
contract YoorHelper is Pausable, Ownable {
    ...
    function makeOffer(address _token, uint256 _tokenId)
        public
        payable
        whenNotPaused
    {
        offersData[abi.encodePacked(_token, _tokenId, msg.sender)] = msg.value;
    }
    ...
    function getOfferDataOf(
        ...
        return offersData[abi.encodePacked(_token, _tokenId, _sender)];
    }

    function cancelOffer(address _token, uint256 _tokenId) public {
        require(
            offersData[abi.encodePacked(_token, _tokenId, msg.sender)] > 0,
            'No offer found'
        );
        uint256 amount = offersData[
            abi.encodePacked(_token, _tokenId, msg.sender)
        ];
        delete offersData[abi.encodePacked(_token, _tokenId, msg.sender)];
    ...
    function acceptOffer(
        ...
        require(
            offersData[abi.encodePacked(_token, _tokenId, _sender)] > 0,
            'No offer found'
        );
        require(
        ...
        uint256 amount = offersData[
            abi.encodePacked(_token, _tokenId, _sender)
        ];
        delete offersData[abi.encodePacked(_token, _tokenId, _sender)];
