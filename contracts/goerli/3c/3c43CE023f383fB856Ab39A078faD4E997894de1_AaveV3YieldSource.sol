...
import {Manageable, Ownable} from "../owner-manager/Manageable.sol";
...
contract AaveV3YieldSource is ERC20, IYieldSource, Manageable, ReentrancyGuard {
    ...
    IPoolAddressesProviderRegistry
        public immutable poolAddressesProviderRegistry;
     ...
     */
    function balanceOfToken(address _user)
        external
        view
        override
        returns (uint256)
     ...
     */
    function redeemToken(uint256 _redeemAmount)
        external
        override
        nonReentrant
        returns (uint256)
    ...
    function claimRewards(address _to) external onlyManagerOrOwner {
        ...
        ) = rewardsController.claimAllRewards(_assets, _to);
    ...
    function _pricePerShare() internal view returns (uint256) {
        ...
        return
            _supply == 0
                ? _tokenUnit
                : (_tokenUnit * aToken.balanceOf(address(this))) / _supply;
    }
     ...
     */
    function _tokenToShares(uint256 _tokens, uint256 _fullShare)
        internal
        view
        returns (uint256)
    {
     ...
     */
    function _sharesToToken(uint256 _shares, uint256 _fullShare)
        internal
        view
        returns (uint256)
    {
