...
contract BridgeMinter{
    ...
    modifier checkNonce(bytes32 nonce) {
        require(nonces[nonce]==false, "dev: already processed"); // dev: already processed
        _;
    }
    ...
    external checkNonce(nonce){
        require(bridging == false,"//dev: re-entrancy guard");                                                //dev: re-entrancy guard
        bridging = true;
        ...
        require(checkEncoding(approvedMessage,messageHash,hashToVerify,approver),"//dev: invalid signature"); //dev: invalid signature
        require(checkEncoding(notarizedMessage,messageHash,hashToVerify,notary),"//dev: invalid signature");  //dev: invalid signature
        nonces[nonce]=true;
    ...
    internal view returns(bool){
        ...
        require(messageHash == domainSeparatorHash,"//dev: values do not match"); //dev: values do not match
    ...
    internal pure returns (uint8 v, bytes32 r, bytes32 s){
        require(sig.length == 65,"// dev: signature invalid"); // dev: signature invalid
