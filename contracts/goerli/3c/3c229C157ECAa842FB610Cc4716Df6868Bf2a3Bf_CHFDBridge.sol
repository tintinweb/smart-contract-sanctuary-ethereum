...
/// @title DeCashBridge implementation based on the DeCash perpetual storage
/// @author Shadowy Coders

import "../interfaces/token/ERC20.sol";
import "../interfaces/rootchain/IRootChainManager.sol";
import "./DeCashBase.sol";
import "./DeCashToken.sol";
import "../lib/ContextMixin.sol";
import "../lib/NativeMetaTransaction.sol";
import "../lib/SafeMath.sol";
import "../lib/FeeMetaTransaction.sol";

contract DeCashBridge is DeCashBase, FeeMetaTransaction {
  using SafeMath for uint256;
  bytes4 private constant depositForFunctionSignature =
    bytes4(keccak256("depositFor(address,address,bytes)"));

  // Event
  event NewRootChainManager(
  ...
  constructor(address _decashStorageAddress) DeCashBase(_decashStorageAddress) {
    version = 1;
    _initializeEIP712("DeCashBridge");
  }

  function setRootChainManager(address newRootChainManager)
    external
    onlySuperUser
    onlyLatestContract("bridge", address(this))
  {
    bytes32 rootChainManagerKey = keccak256(
      abi.encodePacked("contract.address", "rootChainManager")
    );
    emit NewRootChainManager(
      _getAddress(rootChainManagerKey),
      newRootChainManager
    );
    _setAddress(rootChainManagerKey, newRootChainManager);
  }

  function _getRootChainManager() internal view returns (address) {
    return
      _getAddress(
        keccak256(abi.encodePacked("contract.address", "rootChainManager"))
      );
  }

  function bridgeTokenViaSignature(
    FeeStruct calldata feeStruct,
    SigStruct calldata sigFee,
    uint256 depositAmount,
    SigStruct calldata sigDeposit
  )
    external
    ...
    onlyLatestContract("bridge", address(this))
  {
    address tokenAddress = _getAddress(
      keccak256(abi.encodePacked("contract.address", "proxy"))
    );
    _executeFeePayment(feeStruct, sigFee, sigDeposit, tokenAddress);

    // deposit for
    NativeMetaTransaction rootChainManager = NativeMetaTransaction(
      _getRootChainManager()
    );
    rootChainManager.executeMetaTransaction(
      feeStruct.from,
      abi.encodeWithSelector(
        depositForFunctionSignature,
        feeStruct.from,
        tokenAddress,
  ...
  function exitViaSignature(
    FeeStruct calldata feeStruct,
    SigStruct calldata sigFee,
    uint256 withdrawAmount,
    bytes calldata exitInputData
  )
    external
    onlyDelegator(msg.sender)
    onlyFeeRecipient(feeStruct.feeRecipient)
    onlyLatestContract("bridge", address(this))
  {
    ...
    _executeFeePayment(
      feeStruct,
      sigFee,
      WithdrawStruct(feeStruct.from, withdrawAmount),
      tokenAddress
    );

    ERC20 tokenContract = ERC20(tokenAddress);
    uint256 prevBalance = tokenContract.balanceOf(feeStruct.from);

    // claim
    IRootChainManager rootChainManager = IRootChainManager(
      _getRootChainManager()
    );
    rootChainManager.exit(exitInputData);

    uint256 actualBalance = tokenContract.balanceOf(feeStruct.from);
    require(actualBalance == prevBalance.add(withdrawAmount), "Wrong withdraw");
  }
}
...
import "../../contract/DeCashBridge.sol";

contract CHFDBridge is DeCashBridge {
    constructor(address _storage) DeCashBridge(_storage) {}
}
