...
contract DAM_Escrow is UUPSUpgradeable {
	...
	function joinTransaction(uint transactionID, bytes memory receipt, uint counterAmount) external {
		require(transactions[transactionID].seller == address(0) && !transactions[transactionID].finished , "26");
		if(counterAmount > 0) { // joining transaction with own funds
			...
			delete transactions[transactionID].sellerSignature;
		}
		else {
			transactions[transactionID].sellerSignature = receipt;
		}
	...
	function takePartialOrder(uint transactionID, bytes32 spec, uint offerAmount) external {
		require(transactions[transactionID].seller == address(0) && !transactions[transactionID].finished , "26");
		require(offerAmount < transactions[transactionID].paymentAmount, "26");
