...
contract DaftSwapRouter is Ownable {
	...
	// /// TODO: swap exact tokenY for tokenX
	// function swapExactTokenYForTokenX(address tokenX, address tokenY, uint256 tokenYAmount, uint256 minTokenXAmountOut) external {
	// 	address pairAddress = pairAddresses[tokenX][tokenY];
	// 	require(pairAddress != address(0));
	// 	DaftSwapPair(pairAddress).swapExactTokenYForTokenX(tokenYAmount, minTokenXAmountOut, msg.sender);
	// }
...
contract DaftSwapPair is ReentrancyGuard, NoDelegateCall {
	...
	function getExactTokenYForTokenX(uint256 tokenXAmount) external view returns (uint256 tokenYAmountRequired) {
		//TODO: executs limit orders with lowest price
		...
		while (tokenXAmountRemaining > 0) {
			...
			if (initialized) {
				(uint256 requiredTokenYAmount, uint256 usedTokenXAmount) = ticks.simulateExecute(currTick, tickModeForPair, tokenXAmountRemaining);
				tokenXAmountRemaining -= usedTokenXAmount;
				tokenYAmountRequired += requiredTokenYAmount;
			}

			if (tokenXAmountRemaining > 0) {
	...
	function getExactTokenXForTokenY(uint256 tokenYAmount) external view returns (uint256 tokenXAmountOut) {
		...
		while (tokenYAmountRemaining > 0) {
			//only executing if initialized saves an unnecessary SLOAD in tick.execute(). If not initialized we simply proceed to next initialized tick.
			if (initialized) {
				(uint256 requiredTokenXAmount, uint256 usedTokenYAmount) = ticks.simulateExecuteWithTokenY(currTick, tickModeForPair, tokenYAmountRemaining);
				tokenYAmountRemaining -= usedTokenYAmount;
				tokenXAmountOut += requiredTokenXAmount;
			}

			if (tokenYAmountRemaining > 0) {
				(currTick, initialized) = tickBitmap.nextInitializedTickWithinOneWord(currTick + 1);
			}
		}
	}
	...
	function createLimitOrder(uint256 tokenXAmount, int24 tick, address maker) external nonReentrant noDelegateCall onlyRouter returns (uint256 orderId) {
		...
		//transfer tokenX to contract, adjust tokenXAmount based on token transfer fees (if any)
		{
			uint256 preBalance = tokenBalance(tokenX);
			if (!IERC20Minimal(tokenX).transferFrom(maker, address(this), tokenXAmount)) revert TransferFailed();
			tokenXAmount = tokenBalance(tokenX) - preBalance;
		}
		...
		(bool flipped, uint256 partialExecutionIndex) = ticks.update(tick, tokenXAmount, false);
		...
		orderIdMapping[orderId] = Order({
			...
			partialExecutionIndex: partialExecutionIndex
		});

		//update lowestTick
		if (lowestTick > tick) lowestTick = tick;
	}
	...
	function swapTokenYForExactTokenX(uint256 tokenXAmount, uint256 maxTokenYAmount, address maker) external nonReentrant noDelegateCall onlyRouter {
		...
		{
			uint256 preBalance = tokenBalance(tokenY);
			if (!IERC20Minimal(tokenY).transferFrom(maker, address(this), maxTokenYAmount)) revert TransferFailed();
			maxTokenYAmount = tokenBalance(tokenY) - preBalance;
		}
	...
	function swapExactTokenYForTokenX(uint256 tokenYAmount, uint256 minTokenXAmount, address maker) external nonReentrant noDelegateCall onlyRouter {
		...
		{
			uint256 preBalance = tokenBalance(tokenY);
			if (!IERC20Minimal(tokenY).transferFrom(maker, address(this), tokenYAmount)) revert TransferFailed();
			tokenYAmount = tokenBalance(tokenY) - preBalance;
		}
	...
	function withdraw(uint256 orderId, address maker) external nonReentrant noDelegateCall onlyRouter {
		...
		} else if (tickInfo.partialExecutions.length > order.partialExecutionIndex) {
			//tick has been partially executed since order creation
			uint256 totalTokenXAmountSold;
			uint256 currTokenXAmountSold;
			for (uint256 i = order.partialExecutionIndex; i < tickInfo.partialExecutions.length; i++) {
				currTokenXAmountSold = (order.tokenXAmount - totalTokenXAmountSold) * tickInfo.partialExecutions[i].tokenXAmountExecuted / tickInfo.partialExecutions[i].tokenXAmountBeforeExecution;
				totalTokenXAmountSold += currTokenXAmountSold;
			}
			tokenXAmountWithdrawable = order.tokenXAmount - totalTokenXAmountSold;
			tokenYAmountWithdrawable = TickMath.getOutputTokenYAmount(tick, tickMode, totalTokenXAmountSold);
		} else {
...
library TickMath {
    ...
    function getOutputTokenXAmount(int24 tick, uint256 tickMode, uint256 tokenYAmount) internal pure returns (uint256 tokenXAmount) {
        int128 priceX64 = getPriceRatioAtTick(tick, tickMode);
        int128 priceY64 = ABDKMath64x64.inv(priceX64);
...
library Tick {
    ...
    struct PartialExecutionInfo {
        uint256 tokenXAmountBeforeExecution;
        uint256 tokenXAmountExecuted;
    }

    /// Tick indexed state
    struct Info {
        uint256 currTokenXAmount;
        uint256 lastInitializationTimestamp;
        PartialExecutionInfo[] partialExecutions;
        bool initialized;
    }
    ...
    function update(
        ...
        uint256 tokenXDelta,
        bool isRemove
    ) internal returns (bool flipped, uint256 partialExecutionIndex) {
        require(tokenXDelta != 0);
        Tick.Info storage info = self[tick];
        uint256 prevTokenXAmount = info.currTokenXAmount;

        if (isRemove) {
            require(tokenXDelta <= prevTokenXAmount, 'E');
            info.currTokenXAmount -= tokenXDelta;
        } else {
            info.currTokenXAmount += tokenXDelta;
        }
        ...
        partialExecutionIndex = info.partialExecutions.length;
    }
    ...
    function execute(
        ...
        if (usedTokenXAmount >= prevTokenXAmount) {
            ...
            info.currTokenXAmount = 0;
            info.initialized = false;
            flipped = true;
        } else {
            info.partialExecutions.push(PartialExecutionInfo({
                tokenXAmountBeforeExecution: prevTokenXAmount,
                tokenXAmountExecuted: usedTokenXAmount
            }));
            info.currTokenXAmount -= usedTokenXAmount;
        }
    ...
    function executeWithTokenY(
        ...
        if (requiredTokenXAmount >= prevTokenXAmount) {
            ...
            info.partialExecutions.push(PartialExecutionInfo({
                tokenXAmountBeforeExecution: prevTokenXAmount,
                tokenXAmountExecuted: requiredTokenXAmount
            }));
            info.currTokenXAmount -= requiredTokenXAmount;
        }
