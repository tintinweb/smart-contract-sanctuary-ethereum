...
contract MultiChainDeployer is IAxelarExecutable  {
    ...
    function deploy(bytes memory bytecode, bytes32 salt, string[] memory chainNames) external returns (address deployedAddress_) {
        deployedAddress_ = cad.deployedAddress(bytecode, address(cad), salt);
        bytes memory payload = abi.encode(bytecode, salt);
        string memory stringAddress = address(this).toString();
        gasReceiver.payNativeGasForContractCall{ value: 0.01 ether }(
            address(this),
            "Polygon",
            address(cad).toString(),
            payload,
            msg.sender
        );
        gateway.callContract("Polygon", stringAddress, payload);
