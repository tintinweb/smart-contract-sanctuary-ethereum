...
contract OCASnakeArt is Ownable, ReentrancyGuard {
        ...
        returns (string[] memory)
    {
        uint256 rn = randomNumber(
            10,
            10 +
                (uint256(generateAppleSeed(tokenId, 420)) %
                    uint256(colors.length))
        );
    ...
    function checkRandomNumber(uint256 tokenId, uint256 number)
        internal
        view
        returns (uint256)
    {
        uint256 rn = randomNumber(
            10,
            uint256(generateAppleSeed(tokenId, 420)) % uint256(number)
        );
        return rn;
    }
