...
N_COINS: constant(uint256) = 4  # <- change
PRECISION: constant(uint256) = 10**18  # The precision to convert to
A_MULTIPLIER: constant(uint256) = 10000

# These addresses are replaced by the deployer
math: constant(address) = 0xFD640396AC323A57446402dB262c96238e29C7e7
token: constant(address) = 0x12cFCA6CAc0689c91E3262969f1582B4742dbcA7
views: constant(address) = 0xDE5aCa9F0e8F590d3ABB5D450a56208F90f29E9b
coins: constant(address[N_COINS]) = [
    0xa4a7213e5bE9d5205769F584bb4dA1fa79753774,
    0x4663B78be35a6Ed4117047410279FD76668874f3,
    0x90B1851890e1aDA7CD869904c3f1110dAeEcDf1c,
    0xBaDcd67AdBF926aFFfb3E2B1ba8d73427648512B,
]
...
def get_virtual_price() -> uint256:
    return 10**18 * self.get_xcp(self.D) / Token(token).totalSupply()
    ...
    if xcp_profit > xcp_profit_a:
        ...
        if fees > 0:
            ...
            if receiver != empty(address):
                ...
                claimed: uint256 = Token(token).mint_relative(
                    receiver, frac
                )
                xcp_profit -= fees * 2
                self.xcp_profit = xcp_profit
                log ClaimAdminFee(receiver, claimed)
    total_supply: uint256 = Token(token).totalSupply()
...
def add_liquidity(amounts: uint256[N_COINS], min_mint_amount: uint256):
    ...
    token_supply: uint256 = Token(token).totalSupply()
    if old_D > 0:
    ...
    if old_D > 0:
        ...
        Token(token).mint(msg.sender, d_token)
...
def remove_liquidity_one_coin(
    ...
    self.balances[i] -= dy
    Token(token).burnFrom(msg.sender, token_amount)
    _coins: address[N_COINS] = coins
    # assert might be needed for some tokens - removed one to save bytespace
