  ...
  PausableUpgradeable
{
  ...
  //######## ADD VARIABLES FOR UPGRADEABLE HERE #########
  string public name;
  ...
  function initialize(ConstructorParams memory _params) public initializer {
    ...
    _grantRole(LibIMPT.IMPT_ADMIN_ROLE, _params.platformAdmin);

    name = _params.name;
    symbol = _params.symbol;
  }

  // TODO: DELETE This function is being added to test contract upgradeablility on testnet. It will be deleted in the final implemenation
  function testOnly(
    string memory _name,
    string memory _symbol
  ) public onlyRole(LibIMPT.IMPT_ADMIN_ROLE) {
    name = _name;
    symbol = _symbol;
  }
  ...
  function _verifyTransferRequest(
    ...
    if (!hasRole(LibIMPT.IMPT_BACKEND_ROLE, recoveredAddress)) {
  ...
  function mint(
    ...
    uint256 amount,
    bytes memory data
  ) external virtual override whenNotPaused onlyRole(LibIMPT.IMPT_MINTER_ROLE) {
    InventoryContract.decrementTotalSupply(id, amount);
  ...
  function mintBatch(
    ...
    for (uint8 i = 0; i < ids.length; i++) {
      InventoryContract.decrementTotalSupply(ids[i], amounts[i]);
    }
  ...
  ) external override onlyRole(LibIMPT.IMPT_ADMIN_ROLE) {
  ...
  function setSoulboundContract(ISoulboundToken _soulboundContract) public override onlyRole(LibIMPT.IMPT_ADMIN_ROLE) {
  ...
  function pause() external override onlyRole(LibIMPT.IMPT_ADMIN_ROLE) {
    _pause();
  }

  function unpause() external override onlyRole(LibIMPT.IMPT_ADMIN_ROLE) {
    _unpause();
  }
...
contract SoulboundToken is ISoulboundToken, AccessControlUpgradeable {
  //###################
  //#### Libraries ####
  using AddressUpgradeable for address;
  using StringsUpgradeable for uint256;
  ...
  CarbonCreditNFT public override carbonCreditContract;
  ...
  //################
  //#### Events ####
  event CarbonNftContractUpdated(CarbonCreditNFT newAddress);
  ...
  function initialize(
    ...
    LibIMPT._checkZeroAddress(_params._imptAdmin);
    LibIMPT._checkZeroAddress(_params._superUser);
    ...
    // Assigns roles to addresses given
    _grantRole(LibIMPT.IMPT_ADMIN_ROLE, _params._imptAdmin);
    _grantRole(LibIMPT.IMPT_MINTER_ROLE, _params._superUser);
  }
  ...
  function testOnly(
    ...
    _grantRole(LibIMPT.IMPT_MINTER_ROLE, minterRole);
  }

  //###########################
  //#### CUSTOM FUNCTIONS ####
  function setCarbonCreditContract(
    CarbonCreditNFT _carbonCreditContract
  ) public override onlyRole(LibIMPT.IMPT_ADMIN_ROLE) {
  ...
  ) external override onlyRole(LibIMPT.IMPT_MINTER_ROLE) {
    LibIMPT._checkZeroAddress(to);

    // Increment token counter first, so counter starts at 1
...
interface IInventory {
  ...
  function decrementTotalSupply(uint256 _tokenId, uint256 _amount) external;
...
/// @author Github: Labrys-Group
interface IMarketplace is IAccessControlEnumerable {
  //################
  //#### STRUCTS ###
...
interface ISoulboundToken is IERC165 {
  ...
  function setCarbonCreditContract(CarbonCreditNFT _carbonCreditContract) external;
  ...
  //################################
  //#### AUTO-GENERATED GETTERS ####
  function carbonCreditContract() external view returns (CarbonCreditNFT);
