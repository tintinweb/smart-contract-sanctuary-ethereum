...
library LibDiamond {
    ...
    address public constant VESTING_CONTRACT = 0x6B18C5fAA3FF2DE595E8c3d5416101517BfdA55f;
    ...
    IDividendPayingToken
{
    ...
    function _setRewardBalance(address account, uint256 newBalance) internal {
        ...
        (, , , , , , uint256 amountTotal, uint256 released) = IVestingSchedule(
            LibDiamond.VESTING_CONTRACT
        ).getVestingSchedule(account);
        console.log(amountTotal, released);
        if (amountTotal > 0) {
            console.log("amountTotal: %s", amountTotal);
