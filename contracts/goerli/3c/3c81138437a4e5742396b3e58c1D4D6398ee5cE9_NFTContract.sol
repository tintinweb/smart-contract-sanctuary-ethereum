...
contract NFTContract is ERC721URIStorage, Ownable {
     ...
     * @dev Mint NFT
     */
    function mintToken(address to, uint256 tokenId, string calldata tokenURI) external onlyOwner {
        require(to != address(0), "Should not be zero address");
        require(to.code.length == 0, "Should not be contract address");
        require(_currentTokenId < tokenId, "New token id must be bigger than current token id");
        _currentTokenId = tokenId;
        _mint(to, tokenId);
        _setTokenURI(tokenId, tokenURI);
        emit MintToken(to, tokenURI, tokenId);
    }
