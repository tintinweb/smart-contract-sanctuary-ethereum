...
interface ITomiMainDao {
    ...
    function checkAndUpdate(uint256[] memory nftIds, address proposer) external returns (uint256);
}
...
contract TomiStates {
    ...
    /// @notice Tokens for price fetching
    // address public weth;
    // address public usdt;
    ...
    struct ProposalSendTokenData {
        /// @notice Unique id for looking up a proposal
        uint256 id;
        /// @notice Token address
        address token;
        /// @notice Type of transfer (Coin/Token)
        uint8 tokenType;
        /// @notice Recipient of tokens
    ...
    struct ProposalSendNFTData {
        ...
        /// @notice Token address
        address token;
        /// @notice Type of token (ERC721/ERC1155)
        uint8 tokenType;
        /// @notice NFTs to send
        address[] executionTargets;
        /// @notice Targets for sending
        uint256[] executionNFTIds;
        /// @notice amount if ERC1155
        uint256[] executionAmounts;
    }
    ...
    mapping (uint256 => ProposalSendTokenData) internal proposalsSendTokenData;
...
contract TomiFundsDao is TomiEvents, TomiStates, Initializable, OwnableUpgradeable  {
    ...
    function getProposalsSendTokenData(uint256 proposalId) external view returns (ProposalSendTokenData memory) {
        return proposalsSendTokenData[proposalId];
    }
    ...
    function proposeSendToken(
        ...
        uint256 decimals;
        if (tokenType == 0) {
            require(address(treasury).balance >= executionAmount, "insufficient ETH amount in treasury");
            decimals = 18;
        }
        else {
            require(IERC20(token).balanceOf(address(treasury)) >= executionAmount, "insufficient token amount in treasury");
            decimals = IERC20(token).decimals();
        }
        require(functionId == 16, "incorrect function id");
        // TODO ask WQ
        if (executionAmount > 100_000 * (10 ** decimals) && executionAmount <= 500_000 * (10 ** decimals)) {
            functionId = 17;
        } else if (executionAmount > 500_000 * (10 ** decimals)) {
            functionId = 18;
        }
        uint256 proposalCount = tomiMainDao.checkAndUpdate(nftIds, _msgSender());
        ...
        initializeProposal(nftIds, functionId, description, title, proposalCount);

        ProposalSendTokenData storage newProposalData = proposalsSendTokenData[proposalCount];
        newProposalData.id = proposalCount;
        newProposalData.token = token;
        newProposalData.tokenType = tokenType;
        newProposalData.executionTarget = executionTarget;
        newProposalData.executionAmount = executionAmount;
    }
    ...
    function proposeSendNFT(
        ...
        uint256[] memory executionAmounts
    ) public processPayment() {
        if (tokenType == 0) {
            for (uint256 i = 0 ; i < executionNFTIds.length ; i++) {
                require(IERC721(token).ownerOf(executionNFTIds[i]) == address(treasury), "treasury doesn't have execution NFT Ids");
            }
        }
        else {
            for (uint256 i = 0 ; i < executionNFTIds.length ; i++) {
                require(IERC1155(token).balanceOf(address(treasury), executionNFTIds[i]) >= executionAmounts[i], "treasury doesn't have execution NFT Ids");
            }
        }
        require(functionId == 19, "incorrect function id");
        uint256 proposalCount = tomiMainDao.checkAndUpdate(nftIds, _msgSender());
        ...
        ProposalSendNFTData storage newProposalData = proposalsSendNFTData[proposalCount];
        newProposalData.id = proposalCount;
        newProposalData.token = token;
        newProposalData.tokenType = tokenType;
        newProposalData.executionTargets = executionTargets;
        newProposalData.executionNFTIds = executionNFTIds;
        newProposalData.executionAmounts = executionAmounts;
    }
    ...
    // function getLatestPrice(address token, uint256 amountToken) internal view returns (uint256) {
    //     address[] memory path;
    //     path[0] = address(token);
    //     path[1] = address(weth);
    //     path[2] = address(usdt);

    //     uint256[] memory latestPrice = uniswapRouter.getAmountsOut(amountToken, path);

    //     return latestPrice[2];
    // }
    ...
    function executeSendToken(uint256 proposalId) external {
        Proposal storage proposal = proposals[proposalId];

        require(16 <= proposal.functionId && proposal.functionId <= 18, "incorrect function id");
        ...
        if (state(proposalId) == ProposalState.Succeeded) {
            ...
            ProposalSendTokenData memory proposalData = proposalsSendTokenData[proposal.id];
            if (proposalData.tokenType == 0) {
                treasury.withdrawEth(payable(proposalData.executionTarget), proposalData.executionAmount);
            } else {
                treasury.withdrawToken(IERC20(proposalData.token), proposalData.executionTarget, proposalData.executionAmount);
            }
        }
    ...
    function executeSendNFT(uint256 proposalId) external {
        ...
        require(proposal.functionId == 19, "incorrect function id");
        require(state(proposalId) == ProposalState.Succeeded || state(proposalId) == ProposalState.Defeated,
            "TomiDAO::execute: proposal can only be executed if it has succeeded or failed");
        require(block.timestamp >= proposal.endTime, "proposal is currently active");
            ...
            if (proposalData.tokenType == 0) {
                for(uint256 i = 0 ; i < proposal.nftIds.length ; i++) {
                    treasury.withdrawNFT(
                        IERC721(proposalData.token),
                        proposalData.executionTargets[i],
                        proposalData.executionNFTIds[i]
                    );
                }
            }
            else {
                for(uint256 i = 0 ; i < proposal.nftIds.length ; i++) {
                    treasury.withdraw1155NFT(
                        IERC1155(proposalData.token),
                        proposalData.executionTargets[i],
                        proposalData.executionNFTIds[i],
                        proposalData.executionAmounts[i]
                    );
                }
            }
        }
    ...
    function testABC() public pure returns (uint) {
        return 99;
    }
