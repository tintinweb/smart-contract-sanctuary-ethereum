...
contract Core is Initializable, UUPSUpgradeable, OwnableUpgradeable {
    ...
    modifier supportedProtocol(string memory _protocol) {
        ...
        require(supportedProtocols[protocolHash], "ERR_UNSUPPORTED_PROTOCOL");
        _;
    }
    ...
    /**
     * @dev Initialize the contract and set the treasury address.
     */
    function initialize(address _treasuryAddress) public initializer{
        treasuryAddress = _treasuryAddress;

        ///@dev initialise the OwnableUpgradeable explicitly
        __Ownable_init();
    }

    /**
     * @dev override _authorizeUpgrade from UUPSUpgradeable to provide access control
     */
     ...
     */
    function addProtocol(string memory _protocol) external onlyOwner {
        require(!supportedProtocols[keccak256(abi.encode(_protocol))], "ERR_PROTOCOL_ALREADY_SUPPORTED");
        supportedProtocols[keccak256(abi.encode(_protocol))] = true;
    }
    ...
    */
    ...
    function deposit(bytes32 _streamKey, uint256 _amountUnderlying) external returns (uint256, uint256) {
        ...
        if (yield > 0) {
            uint256 totalSupply = IERC20Upgradeable(currentEpoch.getYT()).totalSupply();
        ...
        IERC20Upgradeable(currentEpoch.underlying()).safeTransferFrom(msg.sender, _treasuryAddress, _amountUnderlying);
    ...
    function redeemPrinciple(bytes32 _streamKey, uint256 _epoch) external isEpochExpired(_streamKey, _epoch) {
        ...
        require(IERC20Upgradeable(epochInstance.getOT()).balanceOf(msg.sender) > 0, "ERR_INSUFFICIENT_BALANCE");
