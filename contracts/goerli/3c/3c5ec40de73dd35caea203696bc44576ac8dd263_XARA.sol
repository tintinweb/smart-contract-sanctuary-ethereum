...
contract XARA is Functional, Proxy, IERC20, IERC20Metadata {
    ...
    function stakeZ(uint256 tokenId) public reentryLock {
    	require( ZZZZ.ownerOf(tokenId) == _msgSender(), "Invalid Owner");
    	_stakeNFT(address(ZZZZ), tokenId);
    }
    ...
    function _stakeNFT(address contractAddress, uint256 tokenId) private {
    	...
    	XAR.proxyTransfer( _msgSender(), address(this), tokenId);
    }

    function unstakeNFT(uint index) public reentryLock {
    ...
    function checkRewards(address claimer) public view returns (uint256) {
    	...
    	for (uint256 i=0; i<stakerBag[claimer].length; i++){
    		...
    		if (stakedContract[currentIndex] == address(ZZZZ)){
    ...
    function setZZZZ(address contractAddress) external onlyOwner {
    	ZZZZ = ERC721(contractAddress);
    }
