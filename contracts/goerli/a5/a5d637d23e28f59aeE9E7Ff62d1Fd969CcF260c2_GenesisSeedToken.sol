...
import './interfaces/IGenesisSeedToken.sol';

contract GenesisSeedToken is ERC721, AccessControl, CustomErrors, IGenesisSeedToken {
    ...
    function grantMinterRole(
        address _minter
    ) external onlyRole(DEFAULT_ADMIN_ROLE) checkAddress(_minter) {
     ...
     */
    function setBaseImageURI(string memory baseImageURI_) external onlyRole(DEFAULT_ADMIN_ROLE) {
        if (metadataFrozen) revert MetadataIsFrozen();

        _baseImageURI = baseImageURI_;
    }
