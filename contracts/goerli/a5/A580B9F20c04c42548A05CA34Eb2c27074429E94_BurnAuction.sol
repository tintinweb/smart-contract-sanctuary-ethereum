...
contract BurnAuction is ReentrancyGuard, Ownable {
    ...
    function enterBid(uint256[] calldata nfts, uint256[] calldata nftValues, bytes calldata signature) external nonReentrant {
        ...
        require(IERC721(BidNFT).isApprovedForAll(msg.sender, address(this)), "Contract not approved to transfer");
        ...
        for(uint256 i = 0; i < nfts.length; ) {
            ...
            IERC721(BidNFT).transferFrom(_msgSender(), address(this), nfts[i]);
            // Mapping will be used to burn NFTs
    ...
    function sendNFTs(uint256 quantityToSend) external onlyOwner {
        ...
        // Iterate through the joinedAuction array.
        bool setRefundIndex;
        while(auctionEntries > 0 && !setRefundIndex) {
            uint256 amountSentForAddress;
            uint256[] memory bidderIds = addressBidsByAuction[currentAuctionIndex][joinedAuction[auctionEntries - 1]];
            uint256 bidderPlacement = winners[joinedAuction[auctionEntries - 1]];
            bool isBidderWinner = bidderPlacement > 0;
            address toDeadAddress = 0x000000000000000000000000000000000000dEaD;

            // Iterate through the array containing all the tokenIDs that the address put up for bid, and burn them
            for(uint256 i = refundIndex; i < bidderIds.length; ) {
                ...
                IERC721(BidNFT).transferFrom(address(this), toDeadAddress, bidderIds[i]);
                unchecked { ++totalSent; ++amountSentForAddress; ++i; }
            }
            // Set winners mapping to 0 if all reserved nfts are burned
            if (setRefundIndex == false && (amountSentForAddress + refundIndex) == bidderIds.length) {                    
                if(isBidderWinner) {
                    winners[joinedAuction[auctionEntries - 1]] = 0;
                }
