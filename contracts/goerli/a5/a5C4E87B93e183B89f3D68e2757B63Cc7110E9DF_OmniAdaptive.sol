...
contract OmniAdaptive is Ownable, Pausable {
    ...
    uint256 public constant FALSE = 0;
    ...
    modifier swapping() {
        require(swapbackSettings.inSwap == FALSE, "Already inSwap");
    ...
    function setSwapBackSettings(
        ...
        onlyOwner
    {
        require(swapbackSettings.inSwap == FALSE, "Can't run while inSwap");
        require(
        ...
        if (routerAddr != address(0x0) || pairedCoinAddr != address(0x0)) {
            ...
            if (transferFees.pairs[swapPair] == FALSE) {
                transferFees.pairs[swapPair] = TRUE;
                emit PairUpdated(swapPair, true);
            }
    ...
    function maxTaxReceiverRestrictionsApply(
        ...
        returns (bool)
    {
        return getMaxTaxReceiver(from) && transferFees.isFeeExempt[to] == FALSE;
    }
