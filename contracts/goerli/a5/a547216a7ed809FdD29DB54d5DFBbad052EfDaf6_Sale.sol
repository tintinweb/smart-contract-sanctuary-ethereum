...
contract Sale is ReentrancyGuard, Ownable {
    ...
    mapping(address => bool) public isExcludeMinFee;
        ...
        onlyOwner
    {
        ...
        presaleId++;
        require(presale[presaleId].isStartSale, "sale is already Active");
    ...
    function startPresale() public onlyOwner {
        presale[presaleId].isStartSale = true;
        presale[presaleId].startTime = block.timestamp;
    }

    function endPresale() public onlyOwner {
        require(!(presale[presaleId].isStartSale), "Sale is not enable");
        presale[presaleId].endTime = block.timestamp;
        presale[presaleId].isStartSale = false;
    }
    ...
    function setExcludeFromMinFee(address _user, bool _status)
        external
        onlyOwner
    {
        isExcludeMinFee[_user] = _status;
    }
        ...
        onlyOwner
    {
        ...
        uint256 prevValue = presale[presaleId].price;
        presale[presaleId].price = _newPrice;
        emit PresaleUpdated(
            bytes32("PRICE"),
            prevValue,
            _newPrice,
     ...
     */
    function pausePresale() external checkPresaleId(presaleId) onlyOwner {
    ...
    {
        uint256 usdPrice = (amount * presale[presaleId].price) / (10**18);
    ...
    {
        usdPrice = (amount * presale[presaleId].price) / (10**18);
    }
    ...
    /**
     * @dev Helper funtion to get tokens for given usdt amoun
     ...
     * @param _id Presale id
     */
    function claim(uint256 _id) public checkPresaleId(_id) returns (bool) {
        ...
        require(!(presale[_id].isEnableClaim), "Claim is not enable");

        uint256 amountToSend;
        if (block.timestamp >= _presale.endTime) {
            amountToSend = userClaimData[msg.sender][_id].totalAmount;
            userClaimData[msg.sender][_id].claimAt = block.timestamp;

            userClaimData[msg.sender][_id].claimedAmount += amountToSend;

            bool status = IERC20(saleToken).transfer(msg.sender, amountToSend);
            require(status, "Token transfer failed");
            emit TokensClaimed(msg.sender, _id, amountToSend, block.timestamp);
        }
        return true;
    }

    function removeContractTokens(address _token, uint256 amount)
        external
        onlyOwner
    {
    ...
    function removeContractFunds(uint256 amount) external onlyOwner {
    ...
    function claimMultiple(uint256[] calldata _id) external returns (bool) {
        require(_id.length > 0, "Zero users length");
        for (uint256 i; i < _id.length; i++) {
            require(claim(_id[i]), "Claim failed");
        }
        return true;
    }
}
