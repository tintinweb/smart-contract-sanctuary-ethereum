...
contract VehicleResume is ERC721, ERC721Enumerable, ERC721URIStorage, Ownable {
    ...
    bytes32 constant key_token_ivdl = "ivdl";
    ...
    constructor() ERC721("VehicleResume", "IDVR") {}
    ...
    function safeMintWithMetadata(
        ...
        _setValue(tokenId, key_token_name, abi.encode(ivdl));
       

        _setValue(
            tokenId,
            key_token_description,
            abi.encode("Car NFT for IVDL:", ivdl)
            //abi.encodePacked("Car NFT for IVDL:", ivdl)
        );
        _setValue(tokenId, key_token_image, abi.encode(image));
        //

        //hash base64
        string memory calculatedHash = string(
            abi.encodePacked(keccak256(abi.encodePacked(base64)))
        );

        _setValue(tokenId, key_token_base64, abi.encode(base64));
        _setValue(tokenId, key_token_ivdl, abi.encode(ivdl));
        _setValue(tokenId, key_token_hash, abi.encode(calculatedHash));
        _setValue(tokenId, key_token_external_url, abi.encode(external_url));
        ...
        trait_types[1] = abi.encode("ivdl");
        trait_types[2] = abi.encode("hash");
        trait_values[0] = abi.encode(base64);
        trait_values[1] = abi.encode(ivdl);
        trait_values[2] = abi.encode(calculatedHash);
    ...
    function updateToken(
        ...
        //metadata changes
        _setValue(tokenId, key_token_base64, abi.encode(base64));
        //_setValue(tokenId, key_token_vin, abi.encode(vin));
        _setValue(tokenId, key_token_ivdl, abi.encode(ivdl));
