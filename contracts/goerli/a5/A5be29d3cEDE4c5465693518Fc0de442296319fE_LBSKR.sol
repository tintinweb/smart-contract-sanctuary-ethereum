...
contract Stakable is Ownable {
     ...
     */
    // function _withdrawStake(uint256 user_index, Stake memory currStake, uint256 amount, // TODO may be this should be percentage
    // uint256 index)
    //     internal
    //     // returns (uint256)
    // {
    ...
    //     // Stake memory currStake = _stakeholders[user_index].userStakes[index];

    //     // require(
    //     //     block.timestamp >= currStake.untilBlock,
    //     //     "Staking: You cannot withdraw, it is still in its authorized blocking time"
    //     // );

    //     require(
    //         currStake.amountLBSKR >= amount,
    //         "Staking: Cannot withdraw more than you have staked"
    //     );

    //     // Calculate available Reward first before we start modifying data
    //     // uint256 reward = calculateStakeRewardBlock(currStake);

    //     // Remove by subtracting the money unstaked
    //     currStake.amountLBSKR = currStake.amountLBSKR - amount;
    //     // If stake is empty, 0, then remove it from the array of stakes
    //     if (currStake.amountLBSKR == 0) {
    //         delete _stakeholders[user_index].userStakes[index];
    //     } else {
    //         // If not empty then replace the value of it
    //         _stakeholders[user_index].userStakes[index].amountLBSKR = currStake
    //             .amountLBSKR;
...
// Stake function Unstake(uint256 stakeAmount) external whenNotPaused { - ok
...
contract LBSKR is BaseBSKR, DSMath, Stakable {
     ...
     */
    function unstake(uint256 stakeAmount, uint256 index)
        external
        isInitialRatioSet
        whenNotPaused
    {
        ...
        require(index < _stakeholders[userIndex].userStakes.length, "LBSKR: Stake index incorrect!");
        ...
        uint256 lbskrShares2Deduct = (stakeAmount * currStake.sharesLBSKR) /
            currStake.amountLBSKR;
        uint256 bskrAmount2Deduct = (stakeAmount * currStake.amountBSKR) /
            currStake.amountLBSKR;
        uint256 bskrShares2Deduct = (stakeAmount * currStake.sharesBSKR) /
            currStake.amountLBSKR;

        // If stake is empty, 0, then remove it from the array of stakes
        if ((currStake.amountLBSKR - stakeAmount) == 0) {
            delete _stakeholders[userIndex].userStakes[index];
        } else {
            // If not empty then replace the value of it
            ...
            // _stakeholders[userIndex].userStakes[index].sinceBlock = block
            //     .timestamp;

            emit Debug05L(
                "after",
                _msgSender(),
                _stakeholders[userIndex].userStakes[index].amountLBSKR,
                _stakeholders[userIndex].userStakes[index].amountBSKR,
                _stakeholders[userIndex].userStakes[index].sharesLBSKR,
                _stakeholders[userIndex].userStakes[index].sharesBSKR,
                _stakeholders[userIndex].userStakes[index].sinceBlock
            );
        }
        ...
        if (_lastDistTS + 52 weeks > block.timestamp) {
            uint256 fourWeeksElapsed = uint256(
                (block.timestamp - currStake.sinceBlock) /
                    _SECS_IN_FOUR_WEEKS
            );
    ...
    // TODO to remove this test function
    // function testCheckStakes() external {
    //     uint256 userIndex = _stakes[_msgSender()];
