    ...
    PullPaymentUpgradeable
{
    ...
    uint256 public constant feeCountLimit = 20;
    ...
    event TransactionProcessed(uint256 transctionId);
    ...
    function initialize(bytes32 _nativeToken) external initializer {
    ...
    function createCollection(
        ...
        NFT nft = new NFT{ salt: salt }(name, symbol);
        collections[collectionId] = address(nft);
    ...
    function mintItem(
        ...
        address nft = collections[collectionId];
        if (nft == address(0)) {
        ...
        INFT(nft).mint(recipient, tokenId, tokenURI);

        emit TokenMinted(collectionId, tokenId);
        emit TransactionProcessed(transactionId);
    }

    function transferFees(Fee[] calldata fees) internal {
        ...
        for (uint i; i < feeCount; ) {
            ...
            if (fee.token != nativeToken) {
                revert UnsupportedFeeToken(fee.token);
            }

            fee.payee.transfer(fee.amount);
        ...
        if (msg.value > totalFeeAmount) {
            _asyncTransfer(msg.sender, msg.value - totalFeeAmount);
        }
    }
}