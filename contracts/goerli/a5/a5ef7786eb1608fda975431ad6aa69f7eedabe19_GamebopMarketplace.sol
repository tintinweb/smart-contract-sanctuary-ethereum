...
contract GamebopMarketplace is ReentrancyGuard, ERC1155Holder {
  using Counters for Counters.Counter;
  Counters.Counter private _itemCounter; // start from 1
  Counters.Counter private _itemSoldCounter;

  address payable public marketowner;
  uint public serviceFeeForInitialSale = 0;
  ...
  /**
   * @dev Returns the amount of marketplace items
   */
  ...
  function createMarketItem(
    ...
    if (isSingle) {
        ...
        IERC1155Upgradeable(nftContract).safeTransferFrom(msg.sender, address(this), tokenId, supply, "");
  ...
  function createMarketSale(
    ...
    // send nft to buyer and price from buyer to seller
    if (item.supply > 0) {
        uint remainAmount = item.supply - item.sold;
        require(remainAmount >= buyAmount, "Buy amount is not correct.");
        IERC1155Upgradeable(nftContract).safeTransferFrom(address(this), msg.sender, tokenId, buyAmount, "");
        item.seller.transfer(price * buyAmount);

        // update marketItems sold value
        marketItems[id].sold = marketItems[id].sold + buyAmount;
    } else {
        IERC721(nftContract).transferFrom(address(this), msg.sender, tokenId);
        item.seller.transfer(price);
    }

    // initialize marketItemSold amount
  ...
  function fetchAvailableMarketItems() public view returns (MarketItem[] memory) {
    ...
    for (uint i = 1; i <= total; i++) {
      ...
      if (item.id != 0 && item.buyer == address(0) && item.state == State.Created) {
        itemCount ++;
      }
    }
