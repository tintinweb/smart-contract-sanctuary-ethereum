...
contract FeeManager is IFeeManager, Roles {
  ...
  /// @inheritdoc IFeeManager
  uint16 public poolCardinalityMax = uint16(65_535) / 100;
  /// @inheritdoc IFeeManager
  address public maintenanceGovernance;

  constructor(IPoolManagerFactory _poolManagerFactory, address _admin, IERC20 _weth) payable {
  ...
  function migrateTo(IFeeManager _newFeeManager) external {
    ...
    if (!POOL_MANAGER_FACTORY.isChild(_poolManager)) {
      revert FeeManager_InvalidPoolManager(_poolManager);
    }
    ...
    external
  {
    ...
    if (_oldFeeManager != _poolManager.feeManager()) {
      revert FeeManager_InvalidOldFeeManager(_oldFeeManager);
    }

    if (poolCardinality[_poolManager].weth > 0) {
      revert FeeManager_NonZeroCardinality();
    }
    poolCardinality[_poolManager] = _poolCardinality;

    FeeStore memory _poolDeposits = poolManagerDeposits[_poolManager];

    if (_poolDeposits.wethForFullRange + _poolDeposits.tokenForFullRange > 0) {
      revert FeeManager_NonZeroPoolDeposits();
    }
    poolManagerDeposits[_poolManager] = _poolManagerDeposits;

    if (poolDistribution[_poolManager].isInitialized) {
      revert FeeManager_InitializedPoolDistribution();
    }
   ...
   * @param  _tokenFees The total amount of non-WETH token fees collected from the pool
   */
  function _deposit(uint256 _wethFees, uint256 _tokenFees, IPoolManager _poolManager) internal {
    PoolDistributionFees memory _poolDistributionFees = poolDistribution[_poolManager];
    ...
    external
  {
    ...
    if (_neededToken > _balanceToken) {
      revert FeeManager_InvalidTokenLiquidity();
    }
    ...
    if (_neededWeth > _balanceWeth) {
      ...
      if (_wethToBuy < swapGasCostMultiplier * _SWAP_COST * block.basefee) {
        revert FeeManager_SmallSwap();
      }
      ...
      (int256 _amount0, int256 _amount1) =
        _pool.swap(address(this), !_isWethToken0, -int256(_wethToBuy), _sqrtPriceLimitX96, _data);
  ...
  function fullRangeCallback(IUniswapV3Pool _pool, IERC20 _token, uint256 _neededWeth, uint256 _neededToken) external {
    ...
    if (
      _balanceWeth > _poolManagerFee.wethForFullRange / 100 || _balanceToken > _poolManagerFee.tokenForFullRange / 100
    ) {
  ...
  function uniswapV3SwapCallback(int256 _amount0Delta, int256 _amount1Delta, bytes calldata _data) external {
    ...
    if (!POOL_MANAGER_FACTORY.isSupportedPool(IUniswapV3Pool(_poolAddress))) {
      revert FeeManager_InvalidUniswapPool(_poolAddress);
    }
  ...
  /// @inheritdoc IFeeManager
  function increaseCardinality(IPoolManager _poolManager, uint256 _weth, uint16 _cardinality) external {
    if (ICardinalityJob(msg.sender) != cardinalityJob) {
      revert FeeManager_NotCardinalityJob();
    }
    PoolCardinality memory _poolCardinality = poolCardinality[_poolManager];
    uint256 _maxCardinality = getMaxCardinalityForPool(_poolManager);
    if (_maxCardinality < _cardinality) {
      revert FeeManager_CardinalityExceeded();
    }
    if (_maxCardinality == _cardinality) {
      _poolCardinality.currentMax = _cardinality;
    }
  ...
  function getMaxCardinalityForPool(IPoolManager _poolManager) public view returns (uint256 _maxCardinality) {
    PoolCardinality memory _poolCardinality = poolCardinality[_poolManager];
    _maxCardinality =
      (poolCardinalityMax > _poolCardinality.customMax) ? poolCardinalityMax : _poolCardinality.customMax;
  }
  ...
  /// @inheritdoc IFeeManager
  function setPoolCardinalityTarget(IPoolManager _poolManager, uint16 _cardinality)
    external
  ...
  /// @inheritdoc IFeeManager
  function setWethForMaintenance(IPoolManager _poolManager, uint256 _wethForMaintenance)
    external
    onlyRole(DEFAULT_ADMIN_ROLE)
  {
    PoolDistributionFees memory _poolDistributionFees = poolDistribution[_poolManager];
  ...
  /// @inheritdoc IFeeManager
  function setWethForCardinality(IPoolManager _poolManager, uint256 _wethForCardinality)
    external
    onlyRole(DEFAULT_ADMIN_ROLE)
  {
    PoolDistributionFees memory _poolDistributionFees = poolDistribution[_poolManager];
   ...
   * @dev    Fetching the lock manager from the pool manager is necessary to ensure the function wasn't called from a malicious contract returning a valid pool manager
   * @param  _lockManager The lock manager to be checked
   * @return _poolManager The valid poolManager for that lockManager
   */
  function _checkPoolManagerValidityFromLockManager(ILockManager _lockManager)
    internal
    view
    returns (IPoolManager _poolManager)
  {
    ...
    if (_lockManagerFromPoolManager != _lockManager) {
      revert FeeManager_InvalidLockManager(_lockManager);
    }
  }
}