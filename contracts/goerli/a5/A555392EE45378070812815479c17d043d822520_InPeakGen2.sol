...
contract InPeakGen2 is ERC721, Ownable, ReentrancyGuard {
    ...
    mapping(uint256 => uint8) public tokenLevelOf;
     ...
     constructor(uint256 pMaxSupply, uint256 pPendingReservedSupply) ERC721("Inpeak Gen 2", "IPGEN2") {
    ...
    //     if(curStage == Stage.Inactive) revert MintInactive();
        
    //     // Price is checked either from the proof or from public price (if on Public Stage)
    //     if(curStage == Stage.Public) {
    //         price = publicPrice;
    //     } else {
    //         if(allowListRoot == 0) revert RootNotSet();// Check merkle root for stage
    //         if(!proof.verify(allowListRoot, keccak256(abi.encodePacked(recipient, price)))) revert InvalidProof();
    //     } 

    //     if(msg.value != price) revert InvalidPricePaid();
