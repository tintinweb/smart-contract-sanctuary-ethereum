...
contract Botanik is ERC721, Pausable, Ownable {
    ...
    function mint(address to, uint256 nftAmount)
        ...
        whenNotPaused
    {
        require(tokenId.current() + nftAmount <= phaseLimit, "Limit reached");
    ...
    /*
     * only owner can set the phase limit
     * Requirements:
     * ownable function
     * 'limit' length should be greater than 0
     */

    function setPhaseLimit(uint256 limit) external onlyOwner {
        require(limit > 0, "Not allowed");
        require(
            phaseLimit + limit <= MAX_SUPPLY,
            "phase limit <= Max Supply Limit"
        );
        phaseLimit = phaseLimit + limit;
    }
