...
contract SStrategy is ContractMeta, DefaultAccessControl {
    ...
    function startCycleMocked(uint256 strike, uint256 optionPriceUSD, address safe, bool reusePerp)
        external
    {
        ...
        uint256 amountD9 = FullMath.mulDiv(strategyParams.upperHedgingThresholdD9 - D9, strategyParams.upperHedgingThresholdD9 - strikeD9, D9);
        {
            ...
            squeethVault.takeShort(strategyParams.upperHedgingThresholdD9, reusePerp);
        ...
        external
    {
        ...
        if (IERC20(wPowerPerp).balanceOf(address(this)) > D9) {
            SwapParams memory swapSqueeth = SwapParams({
                tokenIn: wPowerPerp,
                tokenOut: mainToken,
                pool: squeethPool,
    ...
    function _swapToToken(SwapParams memory params) internal returns (uint256 amountIn, uint256 amountOut) {
        ...
        bytes memory routerResult = params.spender.externalCall(address(swapRouter_), EXACT_INPUT_SINGLE_SELECTOR, data); //swap
...
contract SqueethHelper {
    ...
    function getMinMaxPrice(IOracle oracle, address token0, address token1) public view returns (uint256 minPriceX96, uint256 maxPriceX96) {
    ...
    function openRecollateraizedAmounts(uint256 wethAmount, uint256 collateralFactorD9, address vaultGovernance) external view returns (uint256 wethToBorrow, uint256 newWethAmount, uint256 wPowerPerpAmountExpected) {
        ...
        ISqueethVaultGovernance.DelayedProtocolParams memory protocolParams = ISqueethVaultGovernance(vaultGovernance).delayedProtocolParams();  
        uint256 wethFromCollateralWeiD9 = FullMath.mulDiv(spotPriceD18, D4 * D9, indexPriceNormalized);
        wethFromCollateralWeiD9 = FullMath.mulDiv(wethFromCollateralWeiD9, D9 - protocolParams.slippageD9, collateralFactorD9);
        if (wethFromCollateralWeiD9 > D9) {
            wethToBorrow = 0;
        } else {
            wethToBorrow = FullMath.mulDiv(wethAmount, wethFromCollateralWeiD9, D9 + FullMath.mulDiv(IUniV3Pool(protocolParams.wethBorrowPool).fee(), D9, D6) - wethFromCollateralWeiD9);
        }
    ...
    function openAmounts(uint256 wethAmount, uint256 collateralFactorD9) external view returns (uint256 wPowerPerpAmountExpected) {
        uint256 ethPriceNormalized = FullMath.mulDiv(twapIndexPrice(), controller.getExpectedNormalizationFactor(), D18);
    ...
    function closeAmounts(uint256 wPowerPerpRemaining, address vaultGovernance) external view returns (uint256 wethToBorrow, uint256 wethAmountMax) {
    ...
    function minMaxAmounts(uint256 wethAmount, uint256 wPowerPerpDebt, address vaultGovernance) external view returns (uint256[] memory minTokenAmounts, uint256[] memory maxTokenAmounts) {
