...
contract ChaoticCirclesNFT is ERC721Royalty, Ownable {
    ...
    uint public TOTAL_LIMIT = 10;
    
    uint public PRICE_OG     = 0.009 ether;
    uint public PRICE_WL     = 0.01 ether;
    uint public PRICE_PUBLIC = 0.012 ether;
    ...
    function setWLList1(address[] calldata _list, uint number) public onlyOwner{
        for (uint i = 0; i < _list.length; i++) {
            WL[_list[i]] += number;
        }
    }

    function setWLList2(address[] calldata _list, uint number) public onlyOwner{
        for (uint i = 0; i < _list.length; i++) {
            WL[_list[i]] = number;
        }
    }

    function setWLList3(address[] calldata _list, uint number) public onlyOwner{
    ...
    function setWLList4(address[] calldata _list) public onlyOwner{
        uint len = _list.length;
        for (uint i = 0; i < len; i++) {
            WL[_list[i]] = 1;
        }
