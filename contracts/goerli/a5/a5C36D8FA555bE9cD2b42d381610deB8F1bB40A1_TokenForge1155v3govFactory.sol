...
import "../nft/native/TokenForge1155v3gov.sol";

contract TokenForge1155v3govFactory is Context, AccessControlEnumerable {
    ...
    function createTokenForge1155v3gov(
        string memory name,
        address signer,
        string memory baseUri,
        bool transferOwnership
    ) public returns (TokenForge1155v3gov) {
        TokenForge1155v3gov inst = new TokenForge1155v3gov(name, signer, baseUri);
...
contract TokenForge1155v3gov is TokenForge1155v3 {
    bytes32 public constant BURNER_ROLE = keccak256("BURNER_ROLE");
    bytes32 public constant TRANSFEROR_ROLE = keccak256("TRANSFEROR_ROLE");

    modifier onlyBurner() {
        require(hasRole(BURNER_ROLE, _msgSender()), "TokenForge1155v3gov: caller has no burner role");
        _;
    }

    modifier onlyTransferor() {
        require(hasRole(TRANSFEROR_ROLE, _msgSender()), "TokenForge1155v3gov: caller has no transferor role");
        _;
    }
    ...
    constructor(
        ...
        address signer_,
        string memory baseUri_
    ) TokenForge1155v3(name_, signer_, baseUri_) {
     ...
     * @dev See {IERC1155-safeBatchTransferFrom}.
     */
    function safeBatchTransferFromAs(
        address from,
        address to,
        uint256[] memory ids,
        uint256[] memory amounts,
