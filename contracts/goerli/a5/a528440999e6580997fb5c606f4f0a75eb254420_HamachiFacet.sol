...
import {LibHamachi, WithLockTheSwap} from "../libraries/LibHamachi.sol";
import {WithRoles} from "../libraries/LibAccessControl.sol";
import {DEFAULT_ADMIN_ROLE, EXCLUDED_FROM_MAX_WALLET_ROLE, EXCLUDED_FROM_FEE_ROLE, PROCESS_FEE_ROLE} from "../types/access/AccessControlStorage.sol";
import {LibAccessControl} from "../libraries/LibAccessControl.sol";
import {LibERC20} from "../libraries/LibERC20.sol";
import {LibReward} from "../libraries/LibReward.sol";

//  WithReward,
contract HamachiFacet is WithLockTheSwap, WithRoles {
  using Strings for uint256;

  // function initialize() external initializer {
  //   WithReward(address(this)).supportsInterface(type(IERC20Upgradeable).interfaceId);
  ...
  //   _grantRole(LibDiamond.EXCLUDED_FROM_MAX_WALLET_ROLE, LibHamachi.DS().defaultRouter);
  // }

  // ==================== Management ==================== //

  function setLiquidityWallet(address _liquidityWallet) external onlyRole(DEFAULT_ADMIN_ROLE) {
    LibHamachi.DS().liquidityWallet = _liquidityWallet;
  }
  ...
  function setDefaultRouter(address _router) external onlyRole(DEFAULT_ADMIN_ROLE) {
    LibHamachi.DS().defaultRouter = _router;
  ...
  function processFees() external onlyRole(PROCESS_FEE_ROLE) {
    if (LibERC20.balanceOf(address(this)) >= LibHamachi.DS().numTokensToSwap) {
      LibHamachi._processFees(LibHamachi.DS().numTokensToSwap);
    }
  }
  ...
  function liquidityWallet() external view returns (address) {
    return LibHamachi.DS().liquidityWallet;
  }
  ...
  function defaultRouter() external view returns (address) {
    return LibHamachi.DS().defaultRouter;
  }
