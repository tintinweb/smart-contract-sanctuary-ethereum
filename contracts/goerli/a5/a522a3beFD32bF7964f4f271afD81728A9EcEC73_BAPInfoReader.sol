...
contract BAPInfoReader {
    ...
    function getTeenInfo(uint256 tokenId)
        public
        view
        returns (address owner, uint256 claimableMeth)
    {
        owner = bapTeenBulls.ownerOf(tokenId);
        claimableMeth = getTeenClaimableMeth(tokenId);
    }
    ...
    function teenPrevInfo(uint256 tokenId)
        public
        view
        returns (uint256 claimed, bool isResurrected)
    {
        claimed = V3Interface.claimedTeenMeth(tokenId);
        isResurrected = V3Interface.isResurrected(tokenId);
    }
    ...
    function getTeenClaimableMeth(uint256 tokenId)
        public
        view
        returns (uint256 claimableMeth)
    {
