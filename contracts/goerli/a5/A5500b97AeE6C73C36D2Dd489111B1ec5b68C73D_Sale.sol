...
contract Sale is ISale, Market {
    ...
    mapping(uint256 => SaleDetail) public saleDetails;
    ...
    function newSale(
        ...
        uint256 itemId = IRegistry(registry).createMarketItem(
            tokenContract, tokenId, msg.sender, paymentToken, uint16(SaleType.SALE));

        newSaleDetail(itemId, price);

        // Transfer the token to the market contract during sale
    ...
    function buy(
        ...
        require(item.saleType == uint16(SaleType.SALE), "not for sale");

        if(paymentToken == address(0)) {
            require(msg.value == detail.price, "price not match");
        } else {
            // check proper token type of the payment
            require(item.paymentToken == paymentToken, "wrong payment type");
            // check right price
            require(detail.price == price, "wrong price");
            // check if msg.sender has enough balance of the paymentToken
            require(IERC20(paymentToken).balanceOf(msg.sender) >= price, "not enough balance");
        }
        ...
        try IERC721(tokenContract).safeTransferFrom(
            ...
            handlePayment(seller, msg.sender, price, saleFee, paymentToken);
    ...
    /**
        Cancel sale
     */
    function cancelSale(uint256 itemId) external whenNotPaused {
        MarketItem memory item = getMarketItem(itemId);

        require(item.seller == msg.sender || owner() == msg.sender,
            "seller/admin can cancel the item");

        require(item.saleType == uint16(SaleType.SALE),
            "not for sale");

        // return the item to the seller
        try IERC721(item.tokenContract).safeTransferFrom(address(this), item.seller, item.tokenId) {
        ...
        emit SaleItemRemoved(itemId, item.tokenContract, item.tokenId);
    ...
    function changeSalePrice(uint256 itemId, uint256 newPrice)
        ...
        require(item.saleType == uint16(SaleType.SALE), "no market sale. not change price");

        SaleDetail storage detail = saleDetails[itemId];
        detail.price = newPrice;

        emit SaleItemUpdated(
            itemId,
            item.tokenContract,
            item.tokenId,
            newPrice,
