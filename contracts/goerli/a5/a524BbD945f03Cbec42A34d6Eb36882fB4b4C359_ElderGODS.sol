...
contract ElderGODS is ERC721Enumerable, Ownable, IERC4907 {
    ...
    uint256 private slothAirdropCount;
    uint256 private slothAirdropTokenCount;
    uint256 private normalAirdropTokenCount;

    uint256 private sale_start_time = 1662298631;
    uint256 private sale_end_time = 1662899831;
    ...
    constructor (address _ig) ERC721("Elder Gods Test 2.0", "EGT2") {
        INFINIGODS = _ig;
    }
    ...
    function airdropSloth() external onlyOwner {
        ...
        uint256 gasUsed;
        for (uint256 x = slothAirdropTokenCount + 1; x <= IERC721Enumerable(INFINIGODS).totalSupply() && gasUsed < 5000000; x ++) {
    ...
    function airdropStandard() external onlyOwner {
        ...
        uint256 gasLeft = gasleft();
        uint256 gasUsed;
        for (uint256 x = normalAirdropTokenCount + 1; x <= IERC721Enumerable(INFINIGODS).totalSupply() && gasUsed < 5000000; x ++) {
            if (IInfiniGods(INFINIGODS).getTokenRarity(x) == 1) {
    ...
    function mint(uint256[] memory _tokenId, uint256 amount) external {
        ...
        require(mintAbility >= amount, "GODS: Token Ids you are trying to mint with do not have enough mint ability left");
    ...
    function returnToTreasury() external onlyOwner {
        ...
        uint256 gasLeft = gasleft();
        uint256 gasUsed;
        while (totalSupply() <= MAX_SUPPLY && gasUsed < 5000000) {
    ...
    function getConfig() public view returns (uint256[2] memory) {
    ...
    function getAvailable(uint256[] memory _tokenIds) public view returns (uint256[] memory) {
        ...
        for (uint i = 0; i < _tokenIds.length; i++) {
            if (IInfiniGods(INFINIGODS).getTokenRarity(_tokenIds[i]) == 0) {
    ...
    mapping (uint256 => Rent) private rent;
    ...
    mapping (uint256 => RentableItems) private rentableItems;
    ...
    function _setUser(uint256 tokenId, address user, uint64 expires) internal {
        ...
        rent[tokenId].user = user;
        rent[tokenId].expires = expires;
    }
    ...
    function putForRent(uint256 _tokenId, uint64 _expiry, uint64 _duration, uint256 _price, address _token) external rentability {
        ...
        rentableItems[_tokenId].expiry = _expiry;
        rentableItems[_tokenId].duration = _duration;
        rentableItems[_tokenId].price = _price;
        rentableItems[_tokenId].payToken = _token;
    }
    ...
    function deleteFromRent(uint256 _tokenId) external {
        ...
        emit NFTListedForRent(_tokenId, rentableItems[_tokenId].expiry, rentableItems[_tokenId].duration, rentableItems[_tokenId].price, rentableItems[_tokenId].payToken);
    }

    function acceptRent(uint256 _tokenId) public rentability {
         _requireMinted(_tokenId);
        ...
        if (godsFee > 0) {
            ...
            IERC20(rentableItems[_tokenId].payToken).transferFrom(_msgSender(), ownerOf(_tokenId), toOwner);
            IERC20(rentableItems[_tokenId].payToken).transferFrom(_msgSender(), treasury, toTreasury);
        } else {
            IERC20(rentableItems[_tokenId].payToken).transferFrom(_msgSender(), ownerOf(_tokenId), cost);
        }
    ...
    function placeBid(uint256 _tokenId, uint64 _expiry, uint64 _duration, uint256 _price, address _token) external rentability {
        ...
        require(IERC20(_token).balanceOf(_msgSender()) >= _price, "GODS: You do not have enough funds");
        rentBids.push(RentBids({
            tokenId: _tokenId,
            user: _msgSender(),
            expiry: _expiry,
            duration: _duration,
        ...
        emit BidPlaced(rentBids.length-1, _tokenId, _expiry, _duration, _price, _token);
    }

    function deleteBid(uint256 bidId) external {
        RentBids memory bid = rentBids[bidId];
        require(_msgSender() == bid.user, "GODS: Not authorised");
        require(block.timestamp >= bid.expiry, "GODS: Bid expired");
        bid.expiry = uint64(block.timestamp - 1);
        emit BidPlaced(bidId, bid.tokenId, bid.expiry, bid.duration, bid.price, bid.payToken);
    }

    function acceptBid(uint256 bidId) external rentability {
        RentBids memory bid = rentBids[bidId];
        ...
        if (godsFee > 0) {
            uint256 toOwner = cost * (denominator - godsFee) / denominator;
            uint256 toTreasury = cost * (godsFee) / denominator;
            IERC20(bid.payToken).transferFrom(bid.user, ownerOf(bid.tokenId), toOwner);
            IERC20(bid.payToken).transferFrom(bid.user, treasury, toTreasury);
        } else {
            IERC20(bid.payToken).transferFrom(bid.user, ownerOf(bid.tokenId), cost);
        }
        setUser(bid.tokenId, bid.user, uint64(block.timestamp) + bid.duration);
    } 

    function getBidsForNFT(uint256 _tokenId) external view returns (RentBids[] memory) {
        uint256 count;
        for (uint256 i; i < rentBids.length; i++) {
            if (rentBids[i].tokenId == _tokenId) {
                count ++;
            }
        }
        RentBids[] memory allBids = new RentBids[](count);
        uint256 count2;
        for (uint256 i; i < rentBids.length; i++) {
            if (rentBids[i].tokenId == _tokenId) {
                allBids[count2] = rentBids[i];
                count2 ++;
            }
        }
        return allBids;
    ...
    function getActiveBids(uint256 _tokenId) external view returns (RentBids[] memory) {
        uint256 count;
        for (uint256 i; i < rentBids.length; i++) {
            if (rentBids[i].tokenId == _tokenId && rentBids[i].expiry >= block.timestamp) {
                count ++;
            }
        }
        RentBids[] memory allBids = new RentBids[](count);
