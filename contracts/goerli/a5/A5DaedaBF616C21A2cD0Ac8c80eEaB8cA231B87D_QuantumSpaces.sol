    ...
    */
    ...
    function _mint(
        ...
        if (
            ...
            _minted[dropId] + amount > _dropMaxSupply[dropId]
        ) revert MintExceedsDropSupply();
        if (numOfVariants != 0 && amount != numOfVariants)
            revert VariantAndMintAmountMismatch();

        uint256 startTokenId = _tokenId(dropId, uint128(_minted[dropId]));
    ...
    function preAllocateDrop(uint128 dropId, uint128 quantity)
        public
        onlyRole(MINTER_ROLE)
    {
        _preAllocateDropGas(dropId, quantity);
    }
    ...
    IERC721MetadataUpgradeable
{
    ...
    mapping(uint256 => uint256) private _gasRefunds;

    uint256 internal _availableGasRefunds;
    uint256 internal _maxGasRefunds;
        ...
        returns (TokenOwnership memory)
    {
        ...
        unchecked {
            if (
                _startTokenId(tokenId) <= curr &&
                curr < _minted[_dropIdOf(tokenId)]
            ) {
                ...
                if (!ownership.burned) {
                    ...
                    while (curr >= _startTokenId(tokenId)) {
                        curr--;
    ...
    function _exists(uint256 tokenId) internal view returns (bool) {
        return
            _startTokenId(tokenId) <= tokenId &&
            _mintIdOf(tokenId) < _minted[_dropIdOf(tokenId)] &&
    ...
    function _mint(
        ...
        uint256 startTokenId = _tokenId(dropId, _minted[dropId]);
        ...
        unchecked {
            ...
            if (_addressData[to].numberMinted == 0 && _availableGasRefunds > 0)
                delete _gasRefunds[_availableGasRefunds--];
            ...
            uint128 updatedIndex = _minted[dropId];
            uint128 end = updatedIndex + quantity;

            if (safe && to.isContract()) {
                do {
                    uint256 tokenId = _tokenId(dropId, updatedIndex++);
                ...
                if (_minted[dropId] != _mintIdOf(startTokenId)) revert();
            } else {
                do {
                    uint256 tokenId = _tokenId(dropId, updatedIndex++);
                    emit Transfer(address(0), to, tokenId);
     ...
     *
     */
    function _preAllocateDropGas(uint128 dropId, uint128 quantity) internal {
        if (quantity == 0) revert MintZeroQuantity();

        unchecked {
            uint256 startTokenId = _tokenId(dropId, _minted[dropId]);
            uint256 endTokenId = _tokenId(
                dropId,
                _minted[dropId] + quantity - 1
            );
            uint256 currentTokenId = endTokenId;

            do {
                _ownerships[currentTokenId--].addr = address(0);
            } while (currentTokenId >= startTokenId);
            _topUpGasRefunds();
        }
    }
    ...
    function _topUpGasRefunds() internal {
        unchecked {
            do {
                _gasRefunds[_availableGasRefunds++] = 0;
    ...
    function _transfer(
        ...
        unchecked {
            ...
            if (nextSlot.addr == address(0)) {
                ...
                if (_mintIdOf(nextTokenId) != _minted[_dropIdOf(tokenId)]) {
                    nextSlot.addr = from;
    ...
    function _burn(uint256 tokenId, bool approvalCheck) internal virtual {
        ...
        // ownership above and the recipient's balance can't realistically overflow.
        // Counter overflow is incredibly unrealistic as tokenId would have to be 2**256.
        uint128 dropId = _dropIdOf(tokenId);
        unchecked {
            ...
            if (nextSlot.addr == address(0)) {
                // This will suffice for checking _exists(nextTokenId),
                // as a burned slot cannot contain the zero address.
                if (_mintIdOf(nextTokenId) < _minted[dropId]) {
                    nextSlot.addr = from;
                    nextSlot.startTimestamp = prevOwnership.startTimestamp;
                }
            }
...
abstract contract ERC2981 {
    ...
    function royaltyInfo(uint256 tokenId, uint256 salePrice) public view virtual returns (
        ...
        uint256 dropFee = _dropRoyaltyRecipient[dropId] != address(0) ? _dropRoyaltyFee[dropId] : _royaltyFee;
        receiver = _dropRoyaltyRecipient[dropId] != address(0) ? _dropRoyaltyRecipient[dropId] : _royaltyRecipient;
