...
contract CASHBY is protected, IERC20 {
    ...
    function transferFrom(address _from, address _to, uint256 _value) public override returns (bool success) {
        require(_allowances[_from][msg.sender] >= _value, "CASH: Not enough allowance");
    ...
    function approve(address _spender, uint256 _value) public override returns (bool success) {
        require(msg.sender != _spender, "CASH: Approve to yourself");
    ...
    function setCooldownTimerInterval(uint _list, uint _interval) public onlyOwner {
        require(_interval >= 1 seconds, "CASH: Interval must be at least 1 second");
        require(_interval <= 1 minutes, "CASH: Interval must be at most 1 minute");
        require(_list >= 0 && _list <= 3, "CASH: List must be between 0 and 3");
    ...
    function setMaxWallet(uint _list, uint _maxWallet) public onlyOwner {
        require(_maxWallet >= 1 * 10**4 * 10**decimals, "CASH: Max wallet must be at least 10 thousand");
        require(_maxWallet <= 2 * 10**6 * 10**decimals, "CASH: Max wallet must be at most 2 million");
        require(_list >= 0 && _list <= 3, "CASH: List must be between 0 and 3");
    ...
    function setMaxSale(uint _list, uint _maxSale) public onlyOwner {
        require(_maxSale >= 1 * 10**3 * 10**decimals, "CASH: Max sale must be at least 1 thousand");
        require(_list >= 0 && _list <= 3, "CASH: List must be between 0 and 3");
    ...
    function setBuyFee(uint8 _list, uint _buyFee) public onlyOwner {
        require(_buyFee >= 0 && _buyFee <= 49, "CASH: Buy fee must be between 0 and 100");
        require(_list >= 0 && _list <= 3, "CASH: List must be between 0 and 3");
    ...
    function setTransferFee(uint8 _list, uint _transferFee) public onlyOwner {
        require(_transferFee >= 0 && _transferFee <= 49, "CASH: Transfer fee must be between 0 and 100");
        require(_list >= 0 && _list <= 3, "CASH: List must be between 0 and 3");
    ...
    function setSellFee(uint8 _list, uint _sellFee) public onlyOwner {
        require(_sellFee >= 0 && _sellFee <= 49, "CASH: Sell fee must be between 0 and 100");
        require(_list >= 0 && _list <= 3, "CASH: List must be between 0 and 3");
    ...
    function setLiquidityShareFee(uint8 _list, uint _liquidityShareFee) public onlyOwner {
        require(_liquidityShareFee >= 0 && _liquidityShareFee <= 100, "CASH: Liquidity share fee must be between 0 and 100");
        require(_list >= 0 && _list <= 3, "CASH: List must be between 0 and 3");
    ...
    function setBurnShareFee(uint8 _list, uint _burnShareFee) public onlyOwner {
        require(_burnShareFee >= 0 && _burnShareFee <= 100, "CASH: Burn share fee must be between 0 and 100");
        require(_list >= 0 && _list <= 3, "CASH: List must be between 0 and 3");
    ...
    function setRevenueShareFee(uint8 _list, uint _revenueShareFee) public onlyOwner {
        require(_revenueShareFee >= 0 && _revenueShareFee <= 100, "CASH: Marketing share fee must be between 0 and 100");
        require(_list >= 0 && _list <= 3, "CASH: List must be between 0 and 3");
    ...
    function swapTokensForEth(uint _amount) internal returns(uint _gain) {
        if (! (_balances[address(this)] >= _amount) ) {
            revert("CASH: Not enough tokens to swap");
        }
    ...
    function swapEthForTokens(uint _amount) internal {
        if (! (address(this).balance >= _amount) ) {
            revert("CASH: Not enough ETH to swap");
        }
        address[] memory path = new address[](2);
        path[0] = router.WETH();
        path[1] = address(this);
    ...
    function _transfer(address _from, address _to, uint _value, address txSender) private {
        ...
        if (blacklisted[txSender]) {
            revert("CASH: Sender is blacklisted");
        }
        // Then we check if the recipient is blacklisted
        if (blacklisted[_to]) {
            revert("CASH: Recipient is blacklisted");
        }
        ...
        // Then we check if the address is whitelisted
        ...
        if (isSell) {
            if (_value > limits[listNumber].maxSale) {
                revert("CASH: Sell limit exceeded");
            }
        } else {
            if (_value > limits[listNumber].maxTx) {
                revert("CASH: tx limit exceeded");
            }
            ...
            if (limits[listNumber].maxWallet > 0) {
                ...
                if (walletBalance + _value > limits[listNumber].maxWallet) {
                    revert("CASH: Wallet limit exceeded");
                }
            }
        }
        ...
        if (!isExcludedFromCooldown[_from]) {
            ...
            if (cooldown > 0) {
                if (lastTxTime[_from] + cooldown > block.timestamp) {
                    revert("CASH: Cooldown not expired");
                }
    ...
    function _transferTokens(address _from, address _to, uint _value) private {
        if (_balances[_from] < _value) {
            revert("CASH: Not enough tokens to transfer");
        }
    ...
    function _burnTokens(uint _value) private {
        if (_balances[address(this)] < _value) {
            revert("CASH: Not enough tokens to burn");
        }
    ...
    function withdrawETH() external onlyOwner {
        ...
        require(success, "CASH: ETH transfer failed");
