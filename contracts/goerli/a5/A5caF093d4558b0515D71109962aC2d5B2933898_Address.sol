...
contract TestOwn is ERC721, Ownable, Functional {
    ...
    mapping(address => whitelist1) private whitelistUser;
     ...
     * @dev Initializes the contract by setting a `name` and a `symbol` to the token collection.
     */
    constructor() ERC721("TestOne","T1"){
        _name = "Test Own";
        _symbol = "TXTO";
        baseURI = "https://";
        
        totalTokens = 10000; // 0-9999
        pricePublic = 1000000000000000000; // Replace leading value with price 
    ...
    function publicMint(uint256 qty) external payable reentryLock {
        ...
        require((tokensMintedby[_msgSender()] + qty) <= maxPerWallet, "Mint: Max tokens per wallet exceeded");
    ...
    function whitelistMint(uint256 qty) external payable reentryLock {
        //uint256 qty = WLU.balanceOf(_msgSender());
        require(sellActive, "Sell Inactive");
        require(whitelistActive, "Whitelist: White;ist Mint is not enable.");
        require((qty + reservedTokens + numberMinted) < totalTokens, "Whitelist: Not enough avaialability");
    ...
    function setTotalTokens(uint256 newTokenLimit) public onlyOwner{
        require( newTokenLimit > numberMinted, "Increase the limit of total tokens." );
        totalTokens = newTokenLimit;
    }
