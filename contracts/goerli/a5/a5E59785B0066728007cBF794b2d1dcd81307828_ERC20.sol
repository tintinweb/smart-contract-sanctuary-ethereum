...
contract ERC20 is Context, IERC20, IERC20Metadata {
    ...
    modifier onlyWhitelistSupervisor() {
        require(
            msg.sender == _whitelistSupervisor || msg.sender == _owner,
    ...
    modifier onlyCandidateWhitelistSupervisor() {
        require(
            msg.sender == _candidateWhitelistSupervisor || msg.sender == _owner,
    ...
    modifier onlyFeeSupervisor() {
        require(
            msg.sender == _feeSupervisor || msg.sender == _owner,
    ...
    modifier onlyFeeRecipient() {
        require(
            msg.sender == _feeRecipient || msg.sender == _owner,
    ...
    function setCandidateTokenSupplySupervisor(
        ...
        onlyOneRole(candidateTokenSupplySupervisor_)
        notNullAddress(candidateTokenSupplySupervisor_)
    {
    ...
    function cancelCandidateTokenSupplySupervisor()
        public
        onlyTokenSupplySupervisor
    {
        require(
    ...
    // cancel candidate account supervisor
    function cancelCandidateAccountSupervisor() public onlyAccountSupervisor {
        require(
    ...
    function setCandidateWhitelistSupervisor(
        ...
        onlyOneRole(candidateWhitelistSupervisorAddress_)
        notNullAddress(candidateWhitelistSupervisorAddress_)
    {
        _candidateWhitelistSupervisor = candidateWhitelistSupervisorAddress_;
        emit setCandidateWhitelistSupervisorEvent(
            msg.sender,
            candidateWhitelistSupervisorAddress_,
            block.timestamp
        );
    }

    // cancel candidate whitelist supervisor
    function cancelCandidateWhitelistSupervisor()
        public
        onlyCandidateWhitelistSupervisor
    {
        require(
            _candidateWhitelistSupervisor != address(0),
            'We can not cancel zero address.'
        );
        address oldCandidate = _candidateWhitelistSupervisor;
        _candidateWhitelistSupervisor = address(0);
        emit cancelCandidateWhitelistSupervisorEvent(
            msg.sender,
            oldCandidate,
            block.timestamp
        );
    ...
    function updateWhitelistSupervisor() public onlyOwner {
        address oldAddress = _whitelistSupervisor;
        _whitelistSupervisor = _candidateWhitelistSupervisor;
        _candidateWhitelistSupervisor = address(0);
        ...
        onlyWhitelistSupervisor
    {
        require(
            account_ != address(0),
            'We can not add zero address to whitelist.'
        );
        _whitelistAddresses[account_] = true;
        emit appendToWhitelistEvent(
            _whitelistSupervisor,
            account_,
            block.timestamp
        );
        ...
        onlyWhitelistSupervisor
    {
        require(
            account_ != address(0),
            'We can not remove zero address to whitelist.'
        );
        _whitelistAddresses[account_] = false;
        emit removeFromWhitelistEvent(
            _whitelistSupervisor,
            account_,
            block.timestamp
        );
    ...
    function setCandidateFeeSupervisor(address candidateFeeSupervisorAddress_)
        public
        onlyFeeSupervisor
        onlyOneRole(candidateFeeSupervisorAddress_)
        notNullAddress(candidateFeeSupervisorAddress_)
    {
        _candidateFeeSupervisor = candidateFeeSupervisorAddress_;
        emit setCandidateFeeSupervisorEvent(
            msg.sender,
            candidateFeeSupervisorAddress_,
            block.timestamp
        );
    }

    // cancel candidate fee supervisor
    function cancelCandidateFeeSupervisor() public onlyFeeSupervisor {
        require(
            _candidateFeeSupervisor != address(0),
            'We can not cancel zero address.'
        );
        address oldCandidate = _candidateFeeSupervisor;
        _candidateFeeSupervisor = address(0);
        emit cancelCandidateFeeSupervisorEvent(
            msg.sender,
            oldCandidate,
            block.timestamp
        );
    ...
    // update fee supervisor
    function updateFeeSupervisor() public onlyOwner {
        address oldAddress = _feeSupervisor;
        _feeSupervisor = _candidateFeeSupervisor;
        _candidateFeeSupervisor = address(0);
    ...
    function setCandidateFeeRecipient(address candidateFeeRecipientAddress_)
        public
        onlyFeeRecipient
        onlyOneRole(candidateFeeRecipientAddress_)
        notNullAddress(candidateFeeRecipientAddress_)
    {
        _candidateFeeRecipient = candidateFeeRecipientAddress_;
        emit setCandidateFeeRecipientEvent(
            msg.sender,
            _candidateFeeRecipient,
            block.timestamp
        );
    }

    // cancel candidate fee recipient
    function cancelCandidateFeeRecipient() public onlyFeeRecipient {
        require(
            _candidateFeeRecipient != address(0),
            'We can not cancel zero address'
        );
        address oldCandidate = _candidateFeeRecipient;
        _candidateFeeRecipient = address(0);
        emit cancelCandidateFeeRecipientEvent(
            msg.sender,
            oldCandidate,
            block.timestamp
        );
    ...
    // update fee recipient
    function updateFeeRecipient() public onlyOwner {
        address oldAddress = _feeRecipient;
        _feeRecipient = _candidateFeeRecipient;
        _candidateFeeRecipient = address(0);
    ...
    function setFeeDecimals(uint256 feeDecimals_) public onlyFeeSupervisor {
        require(feeDecimals_ != 0, 'Fee Decimal can not be zero');
        uint256 oldFee = _feeDecimals;
        _feeDecimals = feeDecimals_;
        emit setFeeDecimalsEvent(msg.sender, oldFee, _feeDecimals);
    }
