...
contract OnChainAlpha is ERC721A, IERC721Receiver, ReentrancyGuard, Ownable {
    ...
    struct AlphaToken {
        ...
        mapping(uint256 => string) customLabels;
        mapping(uint256 => string) customValues;
        uint256 labelCount;
    }
    ...
    mapping(uint256 => mapping(uint256 => bool)) internal _hideTraits;
    ...
    uint256 public discountAmount = 0.002 ether;
    ...
    function getLabels(uint256 _tokenId) public view returns (string[] memory) {
        ...
        for (uint256 i; i < idValues[_tokenId].labelCount; i++) {
    ...
    function getProfileName(uint256 _tokenId)
        external
        view
        returns (string memory)
    {
