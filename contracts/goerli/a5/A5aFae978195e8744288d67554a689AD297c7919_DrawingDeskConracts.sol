...
contract DrawingDeskConracts is Initializable, ERC721Upgradeable, ERC721RoyaltyUpgradeable,  PausableUpgradeable, AccessControlUpgradeable, ERC721BurnableUpgradeable, UUPSUpgradeable,NativeMetaTransactionUpgradeable,ContextMixin,OperatorFilterer {
    ...
    function initialize(string memory name, string memory symbol,uint mintPrice, uint saleStartsIn ,string memory baseURI,uint maxSupply,address royaltyReceiver,uint96 royaltyFeeBasePoint,bytes32 merkleRoot) initializer public {
        _merkleRoot = merkleRoot;
        _mintPrice = mintPrice;
        _mintWhitelistedPrice = mintPrice;
        _maxWhitelistCountPerWallet = 1;
    ...
    function mintAsset(uint256 _mintAmount) public payable whenNotPaused(){
    ...
    function mintAssetWhitelisted(uint256 _mintAmount,bytes32[] calldata _merkleProof) public payable whenNotPaused(){
        ...
        require((balanceOf(_msgSender()) + _mintAmount) <= _maxWhitelistCountPerWallet, "Max Per Wallet Limit Reached");
        require((totalSupply() + _mintAmount ) < _maxSupply, "maxSupply Reached");
        for (uint i=0; i<_mintAmount; i++) {
            _tokenIdCounter.increment();
            uint256 tokenId = _tokenIdCounter.current();        
