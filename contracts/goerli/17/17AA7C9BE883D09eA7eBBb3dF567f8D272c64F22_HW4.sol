...
contract HW4 is IOwnable, IPausable, ISplitter {
    ...
    modifier enoughMoney(uint256 requestedAmount) {
        require(
            _balances[msg.sender] >= requestedAmount,
    ...
    function withdraw(
        uint256 amount
    ) external override onlyNotPaused enoughMoney(amount) {
        address payable callerPayable = payable(msg.sender);
        callerPayable.transfer(amount);
