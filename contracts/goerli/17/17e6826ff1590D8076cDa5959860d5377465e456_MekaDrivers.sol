...
contract MekaDrivers is ERC721AMint, MultiMint, MerkleProofVerify, Withdraw {
    ...
    ERC721A("MekaDrivers", "DRIVERS") {
        setBaseUri(baseURI);
        setReserve(100); // test value
        setMaxSupply(120); // test value

        _setMint("HOLDERS", Mint(1674846000, 1674889200, 200, 200, 0.001 ether, false, true)); // test value
        _setMint("WHITELIST", Mint(1674889200, 1674932400, 5, 5, 0.002 ether, false, true)); // test value
        _setMint("PUBLIC", Mint(1674932400, 2654875480, 5, 5, 0.002 ether, false, true)); // test value

        withdrawAdd(Part(0xfB94B6F2F90A3bbe59d5EbAdB228A4b12842a361, 100)); // test value
    }

    receive() external payable {} // test value
    ...
    public payable notSoldOut(_count) canMint("WHITELIST", _count) merkleVerify(_proof, keccak256(abi.encodePacked(_msgSender(), "WHITELIST", uint256(5)))) nonReentrant {
        _mintTokens(_msgSender(), _count);
    }
