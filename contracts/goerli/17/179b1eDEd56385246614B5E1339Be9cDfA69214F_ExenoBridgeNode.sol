...
// File: contracts/ExenoBridgeNodeX.sol
    ...
    ERC1363Payable
{
    ...
    function _validateDeposit(
        ...
        internal
    {
        require(!fulfilled, "E01");
        require(msg.sender == d.beneficiary[0], "E01");
        if (_isNative(d.token[0])) {
            require(msg.value >= d.amount[0] + fee[0], "E01");
        } else {
            require(msg.value >= fee[0], "E01");
            require(IERC20(d.token[0]).allowance(msg.sender, address(this)) >= d.amount[0], "E01");
            require(IERC20(d.token[0]).transferFrom(msg.sender, address(this), d.amount[0]), "E01");
        }
        earnedCash += fee[0];
    }
    ...
    function _validateDeposit(
        ...
        internal
    {
        require(sender == d.beneficiary[0], "E01");
        if (_isCore(d.token[0])) {
            require(amount >= d.amount[0] + fee[1], "E01");
        } else {
            require(amount >= fee[1], "E01");
            require(IERC20(d.token[0]).allowance(sender, address(this)) >= d.amount[0], "E01");
            require(IERC20(d.token[0]).transferFrom(sender, address(this), d.amount[0]), "E01");
        }
        earnedTokens += fee[1];
    }
    ...
    function _processDeposit(
        ...
        internal
    {
        require(!depositIds[d.id], "E01");
        depositIds[d.id] = true;
        require(d.network[0] == blockchain, "E01");
        require(d.timeout[0] == 0 || block.timestamp <= d.timeout[0], "E01");
        if (d.operation[0] == bytes32("burn")) {
            require(_isMintable(d.token[0]), "E01");
            Mintable(d.token[0]).burn(d.amount[0]);
        } else if (d.operation[0] == bytes32("freeze")) {
            require(_isCore(d.token[0]), "E01");
    ...
    function _processWithdraw(
        ...
        internal
    {
        require(!withdrawIds[d.id], "E01");
        withdrawIds[d.id] = true;
        require(d.network[1] == blockchain, "E01");
        require(d.timeout[1] == 0 || block.timestamp >= d.timeout[1], "E01");
        if (d.operation[1] == bytes32("mint")) {
            require(_isMintable(d.token[1]), "E01");
            Mintable(d.token[1]).mint(d.beneficiary[1], d.amount[1]);
        } else if (d.operation[1] == bytes32("unfreeze")) {
            require(_isCore(d.token[1]), "E01");
            require(ownedTokens(d.token[1]) >= d.amount[1], "E01");
            require(frozenTokens >= d.amount[1], "E01");
            require(IERC20(d.token[1]).transfer(d.beneficiary[1], d.amount[1]), "E01");
            frozenTokens -= d.amount[1];
        } else {
            if (_isNative(d.token[1])) {
                require(availableCash() >= d.amount[1], "E01");
                Address.sendValue(payable(d.beneficiary[1]), d.amount[1]);
            } else {
                require(availableTokens(d.token[1]) >= d.amount[1], "E01");
                require(IERC20(d.token[1]).transfer(d.beneficiary[1], d.amount[1]), "E01");
            }
        }
        ...
        public view returns(uint256)
    {
        ...
        if (_isCore(token)) {
            return Math.max(ownedTokens(address(acceptedToken())) - frozenTokens, 0);
        }
    ...
    function releaseAvailableCash(uint256 amount)
        public onlyOwner
    {
        require(amount <= availableCash(), "E01");
    ...
    {
        require(amount <= availableTokens(token), "E01");
        require(IERC20(token).transfer(wallet, amount), "E01");
    ...
    {
        require(acceptedToken().allowance(msg.sender, address(this)) >= amount, "E01");
        require(acceptedToken().transferFrom(msg.sender, address(this), amount), "E01");
        frozenTokens += amount;
    ...
    function unfreezeTokens(uint256 amount)
        external onlyOwner nonReentrant
    {
        require(amount <= frozenTokens, "E01");
        require(acceptedToken().transfer(wallet, amount), "E01");
        frozenTokens -= amount;
