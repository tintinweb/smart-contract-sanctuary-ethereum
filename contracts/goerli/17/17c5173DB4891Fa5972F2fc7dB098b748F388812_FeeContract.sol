...
contract FeeContract is Governable {
    ...
    constructor(address _governor) Governable(_governor) {
        ...
        vars.stakingFee = 0;
        vars.treasuryFee = 2000;
        vars.GHNYPercent = 0;
        vars.uniStableFee = 500; // 0.05% Pool
        vars.uniGHNYFee = 10000; // 1% Pool
        vars.slippageMax = 9700; // 3% diff from the oracle price
        vars.optimal = 2; // USDC
    ...
    function valueOfTokens() public view returns (uint256 _totalUSDC) {
        _totalUSDC = valueOfMeta().add(valueOfTriCrypto()).add(valueOfStETH());
    }

    function valueOfMeta() public view returns (uint256 _USDCInMeta) {
        address lpTokenFRAX = IVault(hsFRAX).token();
        uint256 amountLpFRAX = _tokenBalance(hsFRAX).mul(IVault(hsFRAX).pricePerShare()).div(1e18);
        uint256 USDCInFRAX = zapContract.calc_withdraw_one_coin(lpTokenFRAX, amountLpFRAX, 2);

        address lpTokenLUSD = IVault(hsLUSD).token();
        uint256 amountLpLUSD = _tokenBalance(hsLUSD).mul(IVault(hsLUSD).pricePerShare()).div(1e18);
        uint256 USDCInLUSD = zapContract.calc_withdraw_one_coin(lpTokenLUSD, amountLpLUSD, 2);

        _USDCInMeta = USDCInFRAX.add(USDCInLUSD);
    }

    function valueOfTriCrypto() public view returns (uint256 _USDCInTriCrypto) {
        uint256 amountLpTriCrypto = _tokenBalance(hsTriCrypto).mul(IVault(hsTriCrypto).pricePerShare()).div(
            1e18
        );

        _USDCInTriCrypto = ITriCryptoPool(triCryptoPool).calc_withdraw_one_coin(amountLpTriCrypto, 0);
    }

    function valueOfStETH() public view returns (uint256 _USDCInStETH) {
        ...
        _USDCInStETH = ETHInStETH.mul(ethPrice).div(1e20);
    }
