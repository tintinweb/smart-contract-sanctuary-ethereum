...
contract FabrizioNFT is ERC721URIStorage, Ownable {
    using Counters for Counters.Counter;
    Counters.Counter private _tokenIds;

    constructor() ERC721("FabrizioNFT", "FNFT") {}

    //asigno al address recibido un nuevo token recibido por string url,
    // solo el owner del contrato lo puede llamar
    function awardItem(address player, string memory tokenURI)
        public
        onlyOwner
        returns (uint256)
    {
        //incremento el contador de token
        _tokenIds.increment();

        //creo un tokenId
        uint256 newItemId = _tokenIds.current();
        //asigna a player el token recien creado newItemId, todavia no tiene el valor de la url
        _mint(player, newItemId);
        //setea la url recibida al id del token
