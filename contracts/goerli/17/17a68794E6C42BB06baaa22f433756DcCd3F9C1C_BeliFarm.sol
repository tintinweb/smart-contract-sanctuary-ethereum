...
contract BeliFarm is Ownable, ReentrancyGuard {
    ...
    /**
     * @dev move user's share who triggers mintShareToReceipt to FeeReceiver
     */
    function distributeShareToFeeReceiver(
        IFeeReceiver _receiverAddress,
        uint256 _pid,
        uint256 _amount
    ) private {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage feeReceiverInfo = userInfo[_pid][address(_receiverAddress)];

        feeReceiverInfo.shares = feeReceiverInfo.shares.add(_amount);
        feeReceiverInfo.rewardDebt = feeReceiverInfo.rewardDebt.add(
            _amount.mul(pool.accBELIPerShare).div(1e12)
        );
        feeReceiverInfo.rewardFeeReceiverDebt = feeReceiverInfo.rewardFeeReceiverDebt.add(
            _amount.mul(pool.accBELIPerShareFromFees).div(1e12)
        );
        feeReceiverInfo.rewardEarnDebt = feeReceiverInfo.rewardEarnDebt.add(
            _amount.mul(IVault(pool.vault).accEarnBELIPerShare()).div(1e12)
        );
    }

    /**
     * @dev return the share from minted receipt to the receipt liquidator
     */
    function rebalanceFeeReceiverShare(
        IFeeReceiver _receiverAddress,
        uint256 _pid,
        uint256 _amount
    ) private {
        IFeeReceiver(_receiverAddress).harvestReward(_pid);

        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage feeReceiverInfo = userInfo[_pid][address(_receiverAddress)];

        // handle rounding
        if (_amount > feeReceiverInfo.shares) {
            feeReceiverInfo.shares = 0;
        } else {
            feeReceiverInfo.shares = feeReceiverInfo.shares.sub(_amount);
        }
        feeReceiverInfo.rewardDebt = feeReceiverInfo.shares.mul(pool.accBELIPerShare).div(1e12);
        feeReceiverInfo.rewardEarnDebt = feeReceiverInfo.shares.mul(
            IVault(pool.vault).accEarnBELIPerShare()
        ).div(1e12);
        feeReceiverInfo.rewardFeeReceiverDebt = feeReceiverInfo.shares.mul(
            pool.accBELIPerShareFromFees
        ).div(1e12);
    }
    ...
    function mintShareToReceipt(
        ...
        uint256 feeReceiverShare = _amount.div(2);
        uint256 lpFeeReceiverShare = _amount.sub(feeReceiverShare);
        distributeShareToFeeReceiver(feeReceiver, _pid, feeReceiverShare);
        distributeShareToFeeReceiver(feeLpReceiver, _pid, lpFeeReceiverShare);
    ...
    function burnReceiptForShare(
        ...
        uint256 feeReceiverShare = _amount.div(2);
        uint256 lpFeeReceiverShare = _amount.sub(feeReceiverShare);
        rebalanceFeeReceiverShare(feeReceiver, _pid, feeReceiverShare);
        rebalanceFeeReceiverShare(feeLpReceiver, _pid, lpFeeReceiverShare);
...
interface IFeeReceiver {
  ...
  function harvestReward(uint256 _pid) external;
}
