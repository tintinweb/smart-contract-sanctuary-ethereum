...
contract ProvethVerifier {
    ...
    IBlockUpdater public blockUpdater;
...
//    constructor(address blockUpdaterAddress) public {
//        blockUpdater = IBlockUpdater(blockUpdaterAddress);
//    }
    constructor() public {
    }
    ...
    function receiptProof(
        ...
        (result, index, chainId, t) = validateReceiptProof(blockHash, proofBlob, merkleProof);
        state = t.state;
        gasUsed = t.gasUsed;
        bloom = t.bloom;
        logs = t.logs;
    ...
    function validateReceiptProof(
        ...
        bytes memory proofBlob,
        bytes32[] memory merkleProof
    ) internal view returns (uint8 result, uint256 index, uint16 chainId, ReceiptTransaction memory t) {
        result = 0;
        index = 0;
...
//
//        bool exists = blockUpdater.checkBlockHash(blockHash, merkleProof);
//        if (!exists) {
//            revert("Block Header Unsynchronized");
//        }

        bytes memory rlpTx = validateMPTProof(proof.receiptsRoot, proof.mptKey, proof.stack);
    ...
    function decodeReceipt(bytes memory rlpSignedTx) internal pure returns (ReceiptTransaction memory t) {
    ...
    function decodeProofBlob(bytes memory proofBlob) internal pure returns (Proof memory proof) {
        ...
        proof = Proof(
            ...
            bytes32(proofFields[1].toList()[RECEIPTS_ROOT_HASH_INDEX].toUint()),
            rlpTxIndex,
