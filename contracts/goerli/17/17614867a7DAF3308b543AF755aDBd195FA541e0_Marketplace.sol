...
import {IMarketplace} from "./int/IMarketplace.sol";
    ...
    IERC1155ReceiverUpgradeable
{
    ...
    /// @dev Mapping of uid of listing => index of originFee => originFee
    mapping(uint256 => OriginFee[]) public listingOriginFees;
    ...
    function createListing(ListingParameters memory _params) external override {
        ...
        // storing originFees
        for (uint256 i = 0; i < _params.originFees.length; i++) {
            listingOriginFees[listingId].push(_params.originFees[i]);
        }
    ...
    function payout(
        ...
        totalFeesPaid += royaltyCut;

        OriginFee[] memory originFees = listingOriginFees[_listing.listingId];

        // check if new
        for (uint256 i = 0; i < originFees.length; i++) {
            require(
                originFees[i].value <= MAX_BPS,
                "origin fee exceeds max bps"
            );
            uint256 fee = (_totalPayoutAmount * originFees[i].value) / MAX_BPS;

            totalFeesPaid += fee;
            require(totalFeesPaid <= _totalPayoutAmount, "fees exceed price");
        ...
        CurrencyTransferLib.transferCurrencyWithWrapper(
            ...
            _payer,
            _payee,
            _totalPayoutAmount - totalFeesPaid,
            _nativeTokenWrapper
        );
    }
...
import "../int/IContract.sol";
import "../int/IPlatformFee.sol";
import "../int/IOriginFeeUser.sol";

interface IMarketplace is IContract, IPlatformFee, IOriginFeeUser {
