...
contract dAgoraMembership is ERC721A, Ownable{
    ...
    function mintMembership(
        ...
        Membership memory tier,
        address _ERC20
    )   public 
        isPaused
    {
        require(
            _ERC20 == DAI || _ERC20 == USDC,
            "Payable token must be DAI or USDC"
        );
        require(
            tier.tier  < 3,
            "Membership tier must be 0,1,2"
        );
        require(
            _durationInMonths > 0,
            "min Membership be 1 month yo"
        );
        require(
            _durationInMonths <= 12,
            "Max Membership is year time broski"
        );
        require(
            addressMinted[msg.sender] != true,
            "Already minted membership"
        );
        ...
        if (tier.tier == 2 ) {
            if(_durationInMonths == 12) {
                price = ((monthlyPrice * _durationInMonths) + periclesiaPrice) - discountRate;
            } else {
                // 5 * 12 + 1000 = 1060
            ...
            if(_durationInMonths == 12) {
                price = ((monthlyPrice * _durationInMonths) + hoplitePrice) - discountRate;
            } else {
                price = (monthlyPrice * _durationInMonths) + hoplitePrice;
            }
        } else {
            if(_durationInMonths == 12) {
                price = ((monthlyPrice * _durationInMonths) + dagorianPrice) - discountRate;
            } else {
        ...
        require(
            IERC20(_ERC20).balanceOf(msg.sender) >= price, 
            "Not enough tokens"
        );
        require(
        ...
        isPaused
    {
        ...
        require(
            block.timestamp + 30 days + GRACE_PERIOD >= expires[_tokenId], 
        ...
        if(_durationInMonths == 12) {
            price = (monthlyPrice * _durationInMonths) - discountRate;
        }
        else {
        ...
        isPaused 
    {
        ...
        require(
            tokenTier[_tokenId].tier < tier.tier, 
        ...
        if (tier.tier == 1 && oldTier == 0) {
            ...
            price = periclesiaPrice - hoplitePrice;
        }
    ...
    function cancelMembership(uint256 _tokenId) public onlyController(_tokenId) isPaused {
        expires[_tokenId] = block.timestamp;
    ...
    function serPerclesiaPrice(uint256 _perclesiaPrice) public onlyOwner {
        periclesiaPrice = _perclesiaPrice;
    }
