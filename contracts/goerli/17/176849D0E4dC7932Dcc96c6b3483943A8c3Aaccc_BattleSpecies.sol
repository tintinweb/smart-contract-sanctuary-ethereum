...
contract ERC721Profile is ERC721A {
    ...
    function reserveName(string memory newName, string memory oldName) internal {
        ...
        require(sha256(bytes(newName)) != sha256(bytes(oldName)), "NameMustBeDifferent"); 
        require(!isNameReserved(newName), "NameAlreadyReserved");

        // If already named, dereserve old name
        if (bytes(oldName).length > 0) {
            toggleReserveName(oldName, false);
        }
...
interface IHyperwarpManager {
    function tryHyperwarp(uint256 _jumpClone, uint256 _assist, uint256 _mintIndex) external returns(bool);
    function tryManifest(uint256 _tokenId) external returns(bool);
    function battlerHasUtility(uint256 _tokenId, uint8 _calledFrom) external returns(bool);
    function battlerHasUtilityCount(uint256 _startTokenId, uint256 _batchSize, uint8 _calledFrom) external returns(uint256 count);
}
...
contract BattleSpecies is ERC721Profile, BattlespeciesWithdrawer, OperatorFilterer {
    ...
    address public remoteMintManager;
    ...
    uint256 public hyperwarpPrice = 100 ether;
    ...
    bool public hypercloningEnabled = true;

    constructor(address _hyperwarpManager, address _utilityHub, bytes32 _wlRoot) ERC721Profile("Battlespecies", "BATTLE") {
        setHyperwarpManager(_hyperwarpManager);
        setUtilityHub(_utilityHub);
        setWlRoot(_wlRoot);
        _registerForOperatorFiltering();
        baseURI = "https://api.battlespecies.net/metadata/";
        contractMetadataURI = "https://ipfs.battlespecies.net/ipfs/QmPfhRyvtnRgyv5XYbvtWckYttdHbu86C4SCh1ox8ksBNz";
        _mintERC2309(_msgSender(), 30);
    ...
    function setHypercloningEnabled(bool _enabled) external onlyOwner {
        hypercloningEnabled = _enabled;
    }
    ...
    function setRemoteMintManager(address _address) public onlyOwner {
        remoteMintManager = _address;
    }

    function _baseMint(address _mintRecip, uint8 _numberOfTokens, bool _autoManifest, uint256 _mintPrice) internal {

        require(saleIsActive, "SaleIsClosed");

        uint256 totalMinted = _totalMinted();

        //wallet amount cap check 
        if (hyperwarpManager.battlerHasUtility(totalMinted + _numberOfTokens, 1)) {
            require(balanceOf(_mintRecip) + _numberOfTokens <= maxMintsPerWalletBeforeTranscendance, "MaxTokenAllotment: 1");
        }
        else {
            require(balanceOf(_mintRecip) + _numberOfTokens <= maxMintsPerWalletAfterTranscendance, "MaxTokenAllotment: 2");
        }

        //price checks
        if (_autoManifest) {
            require((_mintPrice * _numberOfTokens) + (autoManifestPrice * _numberOfTokens) <= msg.value, "InsuffientEthPaid: 1");
        }
        else {
            require(_mintPrice * _numberOfTokens <= msg.value, "InsuffientEthPaid: 2");
        }

        if (hypercloningEnabled) {
            _numberOfTokens *= 2;
        }

        require(totalMinted + _numberOfTokens <= maxBattlers, "MaxTokenAllotment: 3");

        _mint(_mintRecip, _numberOfTokens);

        for(uint256 i = 1; i <= _numberOfTokens;) {
            uint256 mintIndex = totalMinted + i;
            battlers[mintIndex] = Battler(block.timestamp, _autoManifest, "", "");
            if (_autoManifest) {
    ...
    function _remoteMint(address _mintRecip, uint8 _numberOfTokens, bool _autoManifest, uint256 _mintPrice) external {
        require(msg.sender == remoteMintManager, "NotMintManager");
        _baseMint(_mintRecip, _numberOfTokens, _autoManifest, _mintPrice);
    }
    ...
    function mint(uint8 _numberOfTokens, bool _autoManifest) public payable {
        _baseMint(_msgSender(), _numberOfTokens, _autoManifest, mintPrice);
    }

    function mintWhitelist(uint8 _numberOfTokens, bool _autoManifest, bytes32[] calldata _wlProof) public payable {
        require(inWhitelist(_wlProof), "NotInWhitelist");
        _baseMint(_msgSender(), _numberOfTokens, _autoManifest, mintWhitelistPrice);
    }

    function hyperwarp(uint256 _jumpClone, uint256 _assist) external returns (uint256 mintIndex) {
        require(ownerOf(_jumpClone) == _msgSender() && ownerOf(_assist) == _msgSender(), "MustOwnBothNfts");
        mintIndex = _nextTokenId();
        hyperwarpManager.tryHyperwarp(_jumpClone, _assist, mintIndex);
    ...
    function manifest(uint256 _tokenId) external {
        require(ownerOf(_tokenId) == _msgSender() || owner() == _msgSender(), "InvalidOwner");
    ...
    function changeName(uint256 _tokenId, string memory _newName) public {
        ...
        Battler storage battler = battlers[_tokenId];
        super.reserveName(_newName, battler.name);
        battler.name = _newName;
    ...
    function _beforeTokenTransfers(
        ...
        if (tokenId > uint(30)) {
            uint256 utilityCount = hyperwarpManager.battlerHasUtilityCount(tokenId, batchSize, 0);   
            if (utilityCount > 0) {
                if (from != address(0)) {
                    rewardableBalanceOf[from] -= utilityCount;
                }
                if (to != address(0)) {
                    rewardableBalanceOf[to] += utilityCount;
                }     
