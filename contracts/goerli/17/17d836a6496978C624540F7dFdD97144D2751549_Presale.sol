    ...
    PausableUpgradeable
{
    ...
    uint256 public totalTokensSoldBSC;
    uint256 public totalBonus;
    uint256 public totalBonusBSC;
    ...
    /**
     * @dev To update number of tokens to be sold on particular round
     * @param _changeForRound round number of token's amount to be updated
     * @param _tokensToBeSold updated value of token's to sell
     */

     function changeTokensToSell(uint256 _changeForRound,uint256 _tokensToBeSold) public onlyOwner returns (bool){
        // require(totalTokensSold <= _tokensToBeSold,"amount less than tokens sold");
        rounds[0][_changeForRound] = _tokensToBeSold;
        return true;
    }

    /**
     * @dev To add tx made on other network to this network
     * @param _user array of user address
     * @param _txInfo userDeposits emmits captured from other network
     */

     function txFromBSC(address[] memory _user, uint256[5][] memory _txInfo)public onlyOwner returns(bool){
        for(uint256 i=0; i<_user.length;i++){
        // require(_user.length == _txInfo.length,"array length does not match");
        // require(_user[i] != address(0),"Incorrect Address");
        totalTokensSoldBSC = totalTokensSoldBSC +  (_txInfo[i][0]/baseDecimals);
        totalBonusBSC = totalBonusBSC +  _txInfo[i][1];
        userDeposits[_user[i]].push(
                    UserDeposits(
                        _txInfo[i][0], //depositAmount 
                        _txInfo[i][1], //bonusAmount 
                        _txInfo[i][2], //initialClaim 
                        _txInfo[i][3], //claimedAmount 
                        _txInfo[i][4]  //claimTime 
                    )
                );
        }
        return true;
