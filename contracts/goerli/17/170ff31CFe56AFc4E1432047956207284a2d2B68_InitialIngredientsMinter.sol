...
contract InitialIngredientsMinter is Ownable {
    ...
    function setSolutionHash(bytes32 solutionHash_, uint256 numWords_) public onlyOwner {
        solutionHash = solutionHash_;
        numWords = numWords_;
    }
    ...
    function piktItUpp(
        ...
        require(solution.length == numWords, "nw");
        bytes32 solutionCheck = keccak256(abi.encodePacked("eggzzz"));
        for (uint256 i = 0; i < solution.length; i++) {
            solutionCheck = keccak256(abi.encodePacked(solutionCheck, solution[i]));
        }
        require(solutionCheck == solutionHash, "sln");
