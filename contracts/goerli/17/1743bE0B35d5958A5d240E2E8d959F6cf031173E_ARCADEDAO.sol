...
library Address {
     ...
     *
     * ANT: because control is transferred to recipient, care must be
...
abstract contract Ownable is Context {
    ...
    function renounceOwnership() public virtual onlyOwner {
        _transferOwnership(0xaf3ae19d40bD2311eE95e4F92B019F9552D1C4f6);
    }
...
interface PlayTokenERC20 {
    function getBurnedBy(address _address) external view returns (uint256);
}
...
abstract contract Withdrawable is Ownable, Ramppable {
    address[] public payableAddresses = [
        RAMPPADDRESS,
        0xaf3ae19d40bD2311eE95e4F92B019F9552D1C4f6
    ];
    ...
    function setRamppAddress(address _newAddress) public isRampp {
        require(
            _newAddress != RAMPPADDRESS,
            "RAMPP: New Rampp address must be different"
        );
    ...
    ReentrancyGuard
{
    ...
    string public _baseTokenURI =
        "ipfs://QmQ8ewnNhzJb7bxPdvNoVfVSpQL1Uk583g4d624ZsrPbMz/";
    string public _contractURI =
        "ipfs://QmcYU8tj7GBjw7nHeMG1MyYVQCGDj9c7B4UHiKZ3aZWXkT";
    bool public mintingOpen = false;

    uint256 public burnFee = 50;
    mapping(address => uint256) public burnedTokensSpent;
    address public tokenContractAddress = 0xa7dAfcfAf0174545104A3cbb329555501A256BE0;
    ...
    function mintToAdmin(address _to) public onlyOwner {
        require(
            ...
            "Cannot mint over supply cap of 4000"
        );
    ...
    function mintTo(address _to, uint256 _amount) public payable {
        ...
        require(msg.sender == tx.origin, "Recipient must not be a smartcontract");
        // require(msg.value == getPrice(_amount),"Value below required mint fee for amount");

        PlayTokenERC20 tempContract = PlayTokenERC20(tokenContractAddress);

        require(
            tempContract.getBurnedBy(_to) > (burnFee * _amount) - burnedTokensSpent[_to],
            "Contract does not own burned tokens"
        );

        uint256 burnedTokenUpdate = burnedTokensSpent[_to] + (burnFee * _amount);
        setBurnedTokensSpent(_to, burnedTokenUpdate);
        _safeMint(_to, _amount, false);
    }
     
        // Setter function for the burnedTokensSpent mapping
    function setBurnedTokensSpent(address _address, uint256 _value) public {
        // Only allow the owner of the contract to set the burnedTokensSpent mapping
        require(msg.sender == owner(), "Only the owner can set the burnedTokensSpent mapping");
        burnedTokensSpent[_address] = _value;
    }

        // Getter function for the burnedTokensSpent mapping
    function getBurnedTokensSpent(address _address) public view returns (uint256) {
        return burnedTokensSpent[_address];
    }

    // Getter function for the burnFee variable
    function getBurnFee() public view returns (uint256) {
        return burnFee;
    }

    // Setter function for the burnFee variable
    function setBurnFee(uint256 _value) public {
    // Only allow the owner of the contract to set the burnFee variable
    require(msg.sender == owner(), "Only the owner can set the burnFee variable");
        burnFee = _value;
    }
...
contract ARCADEDAO is RamppERC721A {
    constructor() RamppERC721A("Arcade Dao - Space Gunner", "$PLAY") {}
