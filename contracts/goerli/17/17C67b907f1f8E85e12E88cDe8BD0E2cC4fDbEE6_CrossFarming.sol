...
/** @title Celer crosschain application for users participate Pancakeswap MCV2 farming in BSC chain from other EVM chains */
contract CrossFarming is MessageSenderApp, MessageReceiverApp {
    ...
    // Ratio of compensation based on estimated gas fee.
    // prevent price fluctuations from causing the gas balance of the executor run out.
    uint256 public marginRate; // 100%-120%
    // Margin rate precision.
    uint256 public constant MARGIN_PRECISION = 1e5;
    uint256 public constant MAX_MARGIN_RATE = 12e4;
    ...
    // chainlink oracle
    // ETH/USD https://bscscan.com/address/0x9ef1b8c0e4f7dc8bf5719ea496883dc6401d5b2e
    ...
    // oracle latest roundId for every feeds
    ...
    // different message type have different gas limit.
    mapping(MessageTypes => uint256) public gaslimits;

    event NewOracle(address oracle);
    event MaginRateUpdated(uint256 rate);
    ...
    constructor(
        ...
        // No compensation for executor initialy.
        marginRate = MARGIN_PRECISION;
    }
    ...
    function sendFarmMessage(
        ...
        uint256 _pid, // mock pool id in Pancake MasterchefV2.
        ...
        // estimate gaslimit in BSC
        uint256 bscGaslimit = estimateGaslimit(_account, _msgType);
        ...
        // paid to executor receiver
        uint256 feeBNB = (bscGaslimit * uint256(ethPrice) * EXCHANGE_RATE_PRECISION * marginRate) /
            (EXCHANGE_RATE_PRECISION * MARGIN_PRECISION * uint256(bnbPrice));

        receiver.transfer(feeBNB);

        uint256 messageFee = MessageBus(messageBus).calcFee(message);
        require(msg.value > feeBNB + messageFee, "sendFarmMessage: Insufficient fee");

        // celer sendMessage.
        sendMessage(_receiver, _dstChainId, message, messageFee);
    ...
    function setGaslimits(MessageTypes _type, uint256 _gaslimit) external onlyOwner {
        require(_gaslimit > 0, "setGaslimits: gaslimit should be > zero");
    ...
    function setMarginRate(uint256 _rate) external onlyOwner {
        require(_rate >= MARGIN_PRECISION && _rate <= MAX_MARGIN_RATE, "setFeeMargin: Invalid margin fee");
        marginRate = _rate;
        emit MaginRateUpdated(marginRate);
    }
    ...
    function estimateGaslimit(address _account, MessageTypes _msgType) public view returns (uint256 gaslimit) {
    ...
    function _createPrxoy(address _user) internal returns (address proxy) {
        require(cProxy[_user] == address(0), "createFarmingPrxoy: User already has a proxy");
    ...
    function _getPriceFromOracle(Feeds _feed) internal view returns (uint80, int256) {
        ...
        require(timestamp <= leastAllowedTimestamp, "Oracle update exceeded max timestamp buffer");
        require(
            uint256(roundId) > latestRoundId[_feed],
            "Oracle updated roundId must be larger than oracleLatestRoundId"
        );
...
/** @title A proxy contract that stake LP tokens on behalf of all cross-chain users to MCV2 called by cross farming contract */
contract CrossFarmingProxy is ReentrancyGuard {
    ...
    function emergencyWithdraw(
        ...
        IMintable(MASTER_CHEF_V2.lpToken(_pid)).burn(userInfo[_pid][_user]);
