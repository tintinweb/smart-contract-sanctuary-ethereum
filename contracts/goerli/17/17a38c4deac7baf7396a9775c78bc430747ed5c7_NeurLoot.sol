...
contract NeurLoot is ERC721A, InitializableOwnable {
  ...
  function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
    ...
    return string(abi.encodePacked(
      "https://nft.neurloot.com/metadata/", 
      _toString(tokenId),
      ".json"
      ));
  }
  ...
  function mint(address to, uint256 quantity) external payable {
    uint256 totalPrice = nftPrice * quantity;
    require(msg.value >= totalPrice, "Insufficient funds to mint");
    (bool success,) = feeReceipient.call{ value: totalPrice } ("");
  ...
  function updateNFTPrice(uint256 _nftPrice) external onlyAdminOrOwner {
    nftPrice = _nftPrice;
    emit UpdatePrice(_nftPrice);
  }

  function updateFeeRecipient(address payable _feeReceipient) external onlyAdminOrOwner {
    feeReceipient = _feeReceipient;
