...
contract SwaprGL is BaseGovernanceUpgradeable {
    ...
    */
    function createListing(uint listingType, bytes calldata data) external onlyMarketplace(data) {
        ...
        (bool isValid, address lock, uint nftId, bytes memory sig, string memory res, address sender) = _validateListing(listingType, listingData);
        require(isValid, res);
        swaprWallet.lockNFT(sig, lock, nftId, _msgSender());
    ...
    */
    ...
    function isListableAuction(bytes memory listingData) external view returns(bool isValid, string memory res, address sender) {
        
        Auction memory auction = _extractAuctionInfo(listingData);
        (isValid,,,,res, sender) = _validateListing(auction.depositType, listingData);
    ...
    */
    /// @return isValid true if succeeds
    /// @return res reason of failure
    function isListableOrder(bytes memory listingData) external view returns(bool isValid, string memory res, address sender, bytes memory data) {
        
        Order memory order = _extractOrderInfo(listingData);
        (isValid,,,,res,sender) = _validateListing(order.depositType, listingData);
        (data,) = abi.decode(listingData, (bytes, bytes));
    ...
    */
    ...
    function _validateListing(uint listingType, bytes memory listingData) internal view returns(bool isValid, address lock, uint nftId, bytes memory sig, string memory res, address sender) {
    ...
    returns(bool isValid, string memory res, Bid memory validatedBid){
        ...
        if(auction.depositType == 1){
            ...
            if(_verify(_msgSender(), proposedBidData)){
                ...
                uint bidderTotalBalance = swaprWallet.getBalance(_msgSender(), auction.acceptedToken);
    ...
    */
    ...
    function _verify(address signer, bytes memory _data) internal view returns(bool) {
        ...
        return SignatureCheckerUpgradeable.isValidSignatureNow(signer, _toEthSignedMessageHash(toMessageHash(message)), signature);
    }
...
contract SwaprWallet is BaseGovernanceUpgradeable, ERC721HolderUpgradeable {
    ...
    function lockNFT(bytes calldata sig, address lock, uint nftId, address owner) external onlySwapr {
        if(_getLockedPart(lock, nftId) < EXP){
            uint tokenId = nftId;
            if(!_isNFT(lock, nftId)){
                if(_getLockedPart(lock, nftId) > 0){
    ...
    function _splitLockedPart(address lock, uint nftId, address[] memory addresses) public returns(uint256[] memory newIDs) {
        ILock lockContract = ILock(lock);
        ISplitManager splitManager = ISplitManager(lockContract.splitManager());

        uint256 lockedPart = splitManager.getLockedPart(nftId);
    ...
    function _getLockedPart(address lock, uint nftId) internal view returns(uint256 lockedPart) {
        ILock lockContract = ILock(lock);
        ISplitManager splitManager = ISplitManager(lockContract.splitManager());
