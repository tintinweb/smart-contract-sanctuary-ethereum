...
contract SubdomainManager is IERC1155Receiver {
    ...
    mapping(address => bytes32) public addressToCreatedSubdomainNode;
    ...
    function mintSubdomain(bytes32 _parentNode, string memory _label) public {
        ...
        addressToCreatedSubdomainNode[msg.sender] = subdomainNode;
    }

    function relaySubdomainToMsgSender(
        uint256 _tokenId,
        bytes32 _parentNode
    ) public {
        if (
            addressToCreatedSubdomainNode[msg.sender] != _parentNode ||
            !nameWrapperContract.isApprovedForAll(msg.sender, address(this))
