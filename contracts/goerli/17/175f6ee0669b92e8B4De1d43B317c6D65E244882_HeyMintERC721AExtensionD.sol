...
contract HeyMintERC721AExtensionD is HeyMintERC721AUpgradeable {
    ...
    function determineFundingSuccess() external {
        HeyMintStorage.State storage state = HeyMintStorage.state();
        require(state.cfg.fundingDuration > 0, "NOT_CONFIGURED");
        require(
    ...
    function burnToRefund(uint256[] calldata _tokenIds) external nonReentrant {
        ...
        require(state.cfg.fundingDuration > 0, "NOT_CONFIGURED");
        require(
            block.timestamp >= uint256(state.cfg.fundingDuration) + 1 days,
    ...
    function totalLoaned() public view returns (uint256) {
        return HeyMintStorage.state().data.currentLoanIndex;
    }
     ...
     * @param _newRefundDuration The amount of days to increase the refund period by
     */
    function increaseRefundDuration(
        uint32 _newRefundDuration
    ) external onlyOwner {
        AdvancedConfig storage advCfg = HeyMintStorage.state().advCfg;
        require(
            _newRefundDuration > advCfg.refundDuration,
            "MUST_INCREASE_DURATION"
        );
        HeyMintStorage.state().advCfg.refundDuration = _newRefundDuration;
    }
