...
contract WorkerANT is ERC721Upgradeable, OwnableUpgradeable {
    ...
    function stakeWorker(uint256 _amount) public {
        ...
        for (uint256 i; i < _amount; i++) {
            ...
            transferFrom(msg.sender, address(this), availableWorkerList[i]);
    ...
    function claimFunghi(uint256 _missionId) public {
        uint256[] memory _workerList = getWorkers(msg.sender);
        uint256 _speed = getSpeed(msg.sender);
        uint256 _now = block.timestamp;
        for (uint256 i; i < _workerList.length; i++) {
            if (
                idToStaked[_workerList[i]] &&
                (STAKE_EPOCHS * duration) / _speed <=
                _now - idToStakeDate[_workerList[i]]
            ) {
                IFunghiToken(funghi).mint(msg.sender, 1);
                setStaked(_workerList[i], false);
                transferFrom(address(this), msg.sender, _workerList[i]);
                reduceHP(msg.sender, _workerList[i]);
            }
        }
        finalizeMission(msg.sender, _missionId);
    }
    ...
    function getAvailableSpace(address _user)
        public
        view
        returns (uint256 _availableSpace)
    {
        _availableSpace = playerToAvailableSpace[_user];
        return _availableSpace;
    }

    function increaseCapacity(address _user) public {
        playerToCapacity[_user] += 10;
    ...
    function burn(address _user, uint256 _index) public {
        require(playerToWorkers[_user].length > 0, "index out of bound");
        for (uint256 i = 0; i < playerToWorkers[_user].length - 1; i++) {
            playerToWorkers[_user][i] = playerToWorkers[_user][i + 1];
        }
        playerToWorkers[_user].pop();
        _burn(_index);
    }
