 ...
 * Developed by Fab
 */
contract DuckiesRewardSplitter is Recoverable {
    ...
    event ReceivedRewards(uint256 amount, uint256 rewardPerToken);
    event ClaimedRewards(address holder, uint256 tokenId, uint256 amount);

    mapping(uint256 => uint256) private claimedForToken;


    constructor(address _nft, uint256 _maxTokenSupply) {
        token = IERC721Enumerable(_nft);
        maxTokenSupply = _maxTokenSupply;
    }


    receive() external payable {
        rewardPerToken += (msg.value / maxTokenSupply);
        emit ReceivedRewards(msg.value, rewardPerToken);
    }
    ...
    function claim() external {
        ...
        for (uint256 i = 0; i < balance; ++i) {
            ...
            emit ClaimedRewards(msg.sender, tokenId, pending);
        }
    ...
    /**
     * @notice Claim pending rewards of all owned tokens given a `_cursor` and `_size` of it's token list
     * @dev Use this method for holders with more than 1000 tokens in total
     * @param _cursor: _cursor
     * @param _size: size (max 1000)
     */
    function claimRewardsForTokensBySize(
        uint256 _cursor,
        uint256 _size
    ) external {
        uint256 balance = token.balanceOf(msg.sender);
        require(balance > 0, "No tokens owned");
        require(_size <= 1000, "Max claim size exeeded");

        uint256 length = _size;
        if (length > balance - _cursor) {
            length = balance - _cursor;
        }
        ...
        for (uint256 i = 0; i < length; i++) {
            tokenId = token.tokenOfOwnerByIndex(msg.sender, _cursor + i);
            pending = rewardPerToken - claimedForToken[tokenId];
     ...
     */
    function pendingRewardsForTokensBySize(address _user, uint256 _cursor, uint256 _size)
        external
        view
        returns (uint256 pending, uint256 length)
    {
        uint256 balance = token.balanceOf(_user);
        require(_size <= 3000, "max size 3000");
        ...
        for (uint256 i = 0; i < length; i++) {
            pending -= claimedForToken[token.tokenOfOwnerByIndex(_user, _cursor + i)];
        }
    }
     ...
     */
    function pendingRewardsForToken(uint256 _tokenId)
        external
        view
        returns (uint256 pending)
    {
