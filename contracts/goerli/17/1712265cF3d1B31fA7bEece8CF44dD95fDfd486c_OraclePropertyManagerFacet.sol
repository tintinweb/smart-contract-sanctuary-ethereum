...
contract OraclePropertyManagerFacet is BaseMultiPropertyManager {
     ...
     * @param mtid_ MetaNFT id, to prevent minting multiples (0 if new)
    ...
    function createRequest(
        ...
        if (mtid_ == 0) {
            mtid_ = _mnft().getOrMintToken(msg.sender, property_);
        } else {
            require(_mnft().ownerOf(mtid_) == msg.sender, "wrong nft id");
        }
