...
contract Fractional is ERC721URIStorage {
    ...
    function _create(address receiver, string memory _tokenURI) internal returns (uint256) {
    ...
    function createNFT(string memory _tokenURI, uint256 _price) public onlyPlatformAdmin {
        // create NFT
    ...
    function lockNFT(uint256 _tokenID, string memory _tokenURI, uint256 _sharesAmount) public {
        ...
        // create new token
        ERC20 fToken = new ERC20(tokenName, tokenSymbol);
        ...
        idToShareValue[_tokenID] = _price / (_sharesAmount);
    }

    // function for user to buy shares of NFT and hold ERC20 as validation token of the purchase
    function buyFractionalShares(uint256 _tokenID, uint256 _totalShares) public payable {
        require(msg.value >= idToShareValue[_tokenID] * (_totalShares), "Insufficient funds");

        // user sends ETH to owner
        address payable nftOwner = idToOwner[_tokenID];
        uint256 _amount = idToShareValue[_tokenID] * (_totalShares);
