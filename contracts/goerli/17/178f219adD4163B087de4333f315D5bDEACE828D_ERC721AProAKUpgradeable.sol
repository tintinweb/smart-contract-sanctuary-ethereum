...
UUPSUpgradeable
{
    ...
    function setCategoryReserveAndSupply(
        uint256 _mintReserve,
        uint256 _tokenSupply, 
        uint256 category
    ) public onlyRole(ADMIN_ROLE) {
        if (_totalMintedByCategories(category) + _mintReserve > categories[category].tokenSupply) revert CategoryReserveWillExceed();
        categories[category].mintReserve = _mintReserve;
        categories[category].tokenSupply = _tokenSupply;
    }

    function setCategoryFreeze(
        uint256 category
    ) public onlyRole(ADMIN_ROLE) {
        if (categories[category].isFrozen != false) revert CategoryFreezed();
        categories[category].isFrozen = true;
    }
     ...
     */
    function setCategoryRoyalty(
        uint96 feeNumerator,
        uint256 category
    ) public onlyRole(ADMIN_ROLE) {
        if (feeNumerator > _feeDenominator()) revert RoyaltyFeeWillExceed();
        categories[category].royaltyFraction = feeNumerator;
    }
...
import '@faktura-art/acs-erc721a-upgradeable/contracts/IERC721AUpgradeable.sol';
/**
