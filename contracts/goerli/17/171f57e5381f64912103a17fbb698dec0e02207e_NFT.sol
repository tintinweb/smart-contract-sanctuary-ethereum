...
contract NFT is ERC721Royalty {
    ...
    modifier whenPresalePaused() {
      require(presaleStarted == false, "Presale is not paused.");
      _;
    }

    constructor() ERC721("Accessories", "AA") {
        owner = msg.sender;
        setRoyalties(royaltyReceiver, royaltyFee);
    }

    function publicLimit() public view virtual returns (uint256) {
        return totalSupply - presaleSupply;
    }

    function _baseURI() internal pure override returns (string memory) {
        return
            "https://bafybeidgizxtypu4reuqe7lkkta45iqx5zjc7o7jaudgl2x47m4hiekwe4.ipfs.w3s.link/metadata/";
    }
    ...
    function setAddToAllowListLimit(uint16 addToAllowListLimit_)
        external
        onlyOwner
    {
    ...
    function setRemoveFromAllowListLimit(uint16 removeFromAllowListLimit_)
        external
        onlyOwner
    {
    ...
    {
        require(
            addresses.length <= addToAllowListLimit,
    ...
    {
        require(
            addresses.length <= removeFromAllowListLimit,
            "Passed list of addresses is too long."
        );
        for (uint256 index = 0; index < addresses.length; index += 1) {
    ...
    function _preValidatePurchase(uint256 amountOfNFTs) internal view {
        ...
        require(getAmountMinted() + amountOfNFTs <= totalSupply, "Requested quantity would exceed max supply.");
        require(_tokenIds.current() < totalSupply, "Collection is sold out.");
        if (presaleStarted) {
            require(!publicSaleStarted && presaleStarted, "Presale is closed.");
            ...
            require(publicSaleStarted && !presaleStarted, "PublicSale is closed.");
            require(amountOfNFTs <= MAX_PER_MINT, "PublicSale: Limited amount of tokens per transaction.");
            require(preSaleMinted + amountOfNFTs <= presaleSupply, "PublicSale: Requested quantity would exceed max presale supply.");
            require(primaryPrice * amountOfNFTs <= msg.value, "PublicSale: Insufficient funds.");
        }
    }
