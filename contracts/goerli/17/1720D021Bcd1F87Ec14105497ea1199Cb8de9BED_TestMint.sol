...
contract TestMint is ERC721A, Ownable, ReentrancyGuard, Pausable{

    // uint256 mint variables
    uint256 public maxSupply = 6969;
    uint256 public maxMint = 20;
    uint256 public mintPrice = 0.005 ether; // @dev 10 finney = 0.01 ether
    uint256 public wlMaxMint = 10;
    uint256 public wlMintPrice = 0.003 ether;

    //base uri, base extension and pre-reveal
    ...
    // https://filesite/CID/
    // initialize gatewayURL(Pinata, ipfs etc) for baseUri and contractUri, make sure / is at end and metadata files named as "x.png" "x.json" not "name x.png" etc
    // https://gateway.pinata.cloud/ipfs/CID/
    // initialize pre-reveal gateway url, baseExtension has to be name.json
    // https://gateway.pinata.cloud/ipfs/CID/hidden.json

    constructor (
        string memory _initBaseURI,
        bytes32 _root
        ) ERC721A("TestMint", "TEST") {
    ...
    function whitelistMint(uint256 _quantity, bytes32[] memory proof) external payable whenNotPaused nonReentrant {
        ...
        if (totalFreeMints[msg.sender] == 0) {
            ...
            require(wlMintEnabled, "Public mint is currently paused");
            ...
            require(wlMintEnabled, "Public mint is currently paused");
            require(totalSupply() + _quantity <= maxSupply, "Error: max supply reached");
            require((totalWlMint[msg.sender] + _quantity) <= wlMaxMint, "Error: Cannot mint more than 20 per tx");
    ...
    function enableBothMints() external onlyOwner {
        wlMintEnabled = true;
    ...
    function setWlPrice(uint256 _wlMintPrice) external onlyOwner {
    wlMintPrice = _wlMintPrice;
    }

    function setMaxPublic(uint256 _maxMint) external onlyOwner {
    maxMint = _maxMint;
    }
