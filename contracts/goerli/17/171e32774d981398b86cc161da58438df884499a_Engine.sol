...
contract Engine is ERC721, IEngineEvents {
    ...
    /// @dev The ID of the next match, skip 0
    uint128 private _nextMatchId = 1;
    uint128 public lastCloseOrderIndexExecuted;
    uint128 public lastOpenOrderIndexExecuted;
    ...
    function runOpens(uint256 n) external {
        n = (n == 0)
            ? opens.length
            : Math.min(lastOpenOrderIndexExecuted + n, opens.length);

        PriceData memory priceData = oracle.getPrice();
        // run the last queue and update the price
        while (lastOpenOrderIndexExecuted < n) {
            PickOrder memory order = opens[lastOpenOrderIndexExecuted];
            ...
            join(order, priceData, lastOpenOrderIndexExecuted);
            delete opens[lastOpenOrderIndexExecuted];
            lastOpenOrderIndexExecuted++;
        }
    }
    ...
    function runCloses(uint256 n) external {
        n = (n == 0)
            ? opens.length
            : Math.min(lastCloseOrderIndexExecuted + n, closes.length);

        PriceData memory priceData = oracle.getPrice();
        while (lastCloseOrderIndexExecuted < n) {
            CloseOrder memory order = closes[lastCloseOrderIndexExecuted];
            ...
            delete closes[lastCloseOrderIndexExecuted];
            lastCloseOrderIndexExecuted++;
        }
    }
}
