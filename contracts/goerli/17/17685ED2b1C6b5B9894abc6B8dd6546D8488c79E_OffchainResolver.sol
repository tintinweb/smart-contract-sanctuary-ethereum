...
// ======================= NamehashController =========================
// ====================================================================
// NameHash: https://github.com/namehash
...
contract OffchainResolver is IExtendedResolver, SupportsInterface {
    ...
    string public constant MESSAGE_TYPE =
        "Message(string message,Record[] records)Record(string key,string value,bytes identifier)";

    string public constant RECORD_TYPE =
        "Record(string key,string value,bytes identifier)";
    ...
    function makeHashToSign(bytes[] memory records)
        public
        pure
        returns (bytes32)
    {
        return keccak256(abi.encode(hex"1900", records));
    }
     ...
     */
    function resolve(bytes calldata name, bytes calldata query)
        external
        view
        override
    ...
    function resolveWithProof(
        ...
        for (uint256 i = 0; i < recordsArray.length; i++) {
            if (
                ...
                return recordsArray[i].parsedValue;
            }
        }
    ...
    /**
     * Get from index next 32 bytes from a bytes memory array
     * @param _bytes bytes memory array to slice
     * @param _start uint256 index to start slice and get next 32 bytes
     */
    function sliceBytesToBytes32(bytes memory _bytes, uint256 _start)
        internal
        pure
     ...
     */
    function parseRecords(bytes[] memory records)
        public
        pure
        returns (Record[] memory)
    {
        ...
        for (uint256 i = 0; i < records.length; i++) {
            (
                ...
                bytes memory parsedValue
            ) = abi.decode(records[i], (string, string, bytes, bytes));
            recordsArray[i] = Record(key, value, identifier, parsedValue);
        }
        return recordsArray;
    }
     ...
     */
    function hashRecords(Record[] memory recordsArray)
        public
        pure
        returns (bytes32)
    {
    ...
    /**
     * @notice Gets EIP712 domain hash
     * @param contractName string - Name of the contract
     * @param version string - Version of the contract
     * @param chainId uint256 - Chain ID of the contract
     * @param verifyingContract address - Address of the contract
     * @return bytes32 - EIP712 domain hash
     */
