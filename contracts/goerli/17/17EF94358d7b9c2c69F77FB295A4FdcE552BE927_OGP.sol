...
contract OGP is ERC721, ERC2981 {
    ...
    function mint(address to, uint256 amount) external payable {
        ...
        for (uint i = 0; i < amount; ) {
            _mint(to, supply[Rarity.COMMON] + i);
            unchecked {
                ++i;
            }
        }
    ...
    function mintVaulted(address to, Rarity _rarity) external onlyAdmin {
        ...
        if (_rarity == Rarity.RARE) {
            tokenId = 3318 + supply[_rarity];
        } else if (_rarity == Rarity.SUPER_RARE) {
            tokenId = 3738 + supply[_rarity];
        } else if (_rarity == Rarity.LEGENDARY) {
            tokenId = 4158 + supply[_rarity];
        }
