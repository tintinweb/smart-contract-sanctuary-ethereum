...
contract ApeCoinStaking is OwnableUpgradeable {
    ...
    function commit(
        ...
        require(
            coinAmounts.length == mainTokenIds.length,
            "inconsistent amount params"
        );
        if (pairedTokenIds.length > 0) {
            require(
                pairedTokenIds.length == mainTokenIds.length,
                "inconsistent paired params"
            );
        }

        for (uint256 i = 0; i < mainTokenIds.length; i++) {
            require(
                IERC721(mainPool.nftAsset).ownerOf(mainTokenIds[i]) ==
                    msg.sender,
                "caller not nft owner"
            );
            require(
                tokenDatas[mainPool.nftAsset][mainTokenIds[i]].valid != true,
                "nft already commited"
            );
        }
        ...
        if (pairedTokenIds.length > 0) {
            ...
            for (uint256 i = 0; i < pairedTokenIds.length; i++) {
                require(
                    IERC721(pairedPool.nftAsset).ownerOf(pairedTokenIds[i]) ==
                        msg.sender,
                    "caller not nf owner"
                );
                require(
                    tokenDatas[pairedPool.nftAsset][pairedTokenIds[i]].valid !=
                        true,
                    "paired nft already commited "
                );
            }
        }
        ...
        for (uint256 i = 0; i < mainTokenIds.length; i++) {
            ...
            tokenDatas[mainPool.nftAsset][mainTokenIds[i]].valid = true;
            tokenDatas[mainPool.nftAsset][mainTokenIds[i]]
                .stakedAmount = mainItemCoinAmount;
            ...
            if (pairedTokenIds.length > 0) {
                ...
                tokenDatas[pairedPool.nftAsset][pairedTokenIds[i]]
                    .stakedAmount = pairedItemCoinAmount;

                pairedPool.userTotalStakedAmounts[
                    msg.sender
                ] += pairedItemCoinAmount;
                stakingAmount += pairedItemCoinAmount;

                tokenDatas[pairedPool.nftAsset][pairedTokenIds[i]]
                    .pairedAsset = mainPool.nftAsset;
                tokenDatas[pairedPool.nftAsset][pairedTokenIds[i]]
                    .pairedTokenId = mainTokenIds[i];
                tokenDatas[mainPool.nftAsset][mainTokenIds[i]]
                    .pairedTokenId = pairedTokenIds[i];
            }
        }
    ...
    function uncommit(string calldata poolName, uint256[] calldata mainTokenIds)
        public
    {
        PoolData storage mainPool = poolDatas[poolName];
        ...
        for (uint256 i = 0; i < mainTokenIds.length; i++) {
            TokenData storage tokenData = tokenDatas[mainPool.nftAsset][
                mainTokenIds[i]
            ];
            if (!tokenData.valid) {
                continue;
            ...
            if (tokenData.pairedAsset == bakcToken) {
                ...
                TokenData storage pairedTokenData = tokenDatas[
                    tokenData.pairedAsset
                ][tokenData.pairedTokenId];
                ...
                delete tokenDatas[tokenData.pairedAsset][
                    tokenData.pairedTokenId
                ];
            }
    ...
    function withdraw(string calldata poolName, uint256 coinAmount) public {
        ...
        require(
            coinAmount <= mainPool.userTotalStakedAmounts[msg.sender],
    ...
    function claimRewards(string calldata poolName, uint256[] calldata tokenIds)
        public
    {
        ...
        for (uint256 i = 0; i < tokenIds.length; i++) {
            require(
                IERC721(claimPool.nftAsset).ownerOf(tokenIds[i]) == msg.sender,
                "caller not nft owner"
            );
            require(
                tokenDatas[claimPool.nftAsset][tokenIds[i]].valid == true,
                "nft not commited"
            );
        }
