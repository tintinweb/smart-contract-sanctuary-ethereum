...
/**
@title Voting Escrow
@author Curve Finance - Ported by Stargate - Forked by Cashmere
@license MIT
...
contract VeCSM is IVeCSM, AccessControlEnumerable, ReentrancyGuard {
    using SafeERC20 for IERC20;
    using EnumerableSet for EnumerableSet.AddressSet;

    uint256 internal constant WEEK = 1 weeks;
    uint256 public constant MINTIME = 1 weeks;
    uint256 public constant MAXTIME = 4 * 365 days;
    int128 internal constant iMAXTIME = int128(int256(MAXTIME));
    ...
    uint256 immutable deployBlock;

    // Aragon's view methods for compatibility
    address public controller;
    bool public constant transfersEnabled = true;

    string public constant override name = "veCSM";
    string public constant override symbol = "veCSM";
    string public constant override version = "1.0.0";
    uint8 public constant override decimals = 18;

    // Governance privileged roles
    bytes32 public constant WHITELIST_ADD_ROLE = keccak256("WHITELIST_ADD");
    bytes32 public constant WHITELIST_REMOVE_ROLE = keccak256("WHITELIST_REMOVE");
    bytes32 public constant LISTENER_ADD_ROLE = keccak256("LISTENER_ADD");
    bytes32 public constant LISTENER_REMOVE_ROLE = keccak256("LISTENER_REMOVE");
    bytes32 public constant WHITELISTED_CONTRACT_ROLE = keccak256("WHITELISTED_CONTRACT");

    // Listeners for balance updates by users
    EnumerableSet.AddressSet private listeners;

    /// @notice Contract constructor
    /// @param _token `ERC20CSM` token address
    ...
    modifier onlyUserOrWhitelist() {
        if (msg.sender != tx.origin) {
            require(hasRole(WHITELISTED_CONTRACT_ROLE, msg.sender), "Smart contract not allowed");
        }
        _;
    }
    ...
    function addUpdateListener(address addr) external override nonReentrant onlyRole(LISTENER_ADD_ROLE) {
        require(listeners.add(addr), "Already set");

        emit ListenerSet(addr, true);
    }

    /// @notice Remove a smart contract address from whitelist
    /// @param addr Address to be removed from whitelist
    function removeUpdateListener(address addr) external override nonReentrant onlyRole(LISTENER_REMOVE_ROLE) {
        require(listeners.remove(addr), "Already set");

        emit ListenerSet(addr, false);
    }

    /// @notice Add address to whitelist smart contract depositors `addr`
    /// @param addr Address to be whitelisted
    function addWhitelistContract(address addr) external override nonReentrant onlyRole(WHITELIST_ADD_ROLE) {
        _grantRole(WHITELISTED_CONTRACT_ROLE, addr);
    }

    /// @notice Remove a smart contract address from whitelist
    /// @param addr Address to be removed from whitelist
    function removeWhitelistContract(address addr) external override nonReentrant onlyRole(WHITELIST_REMOVE_ROLE) {
        _revokeRole(WHITELISTED_CONTRACT_ROLE, addr);
    }

    /// @notice Unlock all locked balances in case of emergency allowing anyone to withdraw immediately
    function unlock() external override nonReentrant onlyRole(DEFAULT_ADMIN_ROLE) {
    ...
    function user_point_history__ts(address _addr, uint256 _idx) external view override returns (uint256) {
    ...
    function checkpoint() external override nonReentrant onlyUserOrWhitelist {
        _checkpoint(address(0x0), LockedBalance(0, 0), LockedBalance(0, 0));
        emit ManualCheckpoint(epoch);
    }
    ...
    function deposit_for(address _addr, uint256 _value) external override nonReentrant onlyUserOrWhitelist notUnlocked {
    ...
    /// @param _value Amount to deposit
    /// @param _unlock_time Epoch time when tokens unlock, will be rounded down to whole weeks
    function _create_lock(uint256 _value, uint256 _unlock_time) internal {
        ...
        require(unlock_time >= block.timestamp + MINTIME, "Voting lock must be at least 1 week");
    ...
    function increase_amount(uint256 _value) external override nonReentrant onlyUserOrWhitelist notUnlocked {
    ...
    /// @dev Only possible if the lock has expired or the emergency unlock() function has been called by governance.
    ...
    function _notifyListeners(address _addr) internal {
        uint256 len = listeners.length();
        for (uint256 i = 0; i < len; i++) {
            address listener = listeners.at(i);
            IVeListener(listener).onVotingPowerUpdated(_addr);
        }
    }
    ...
    /// @dev Only perfectly accurate if `_t` is after the last user_point_epoch
    ...
    function _balanceOf(address addr, uint256 _t) internal view returns (uint256) {
        ...
        if (_epoch == 0) {
            require(_t >= deployTimestamp, "Before deployment");
            return 0;
        } else {
            Point memory last_point = user_point_history[addr][_epoch];
            require(_t >= last_point.ts, "Unsupported");
    ...
    /// @notice Returns the balance of an address at a provided timestamp, becomes more inaccurate the further one goes in the past.
    /// @dev Consider using balanceOfAt in case balances further in the past need to be fetched with accuracy.
    function balanceOfAtT(address addr, uint256 _t) external view override returns (uint256) {
        return _balanceOf(addr, _t);
    }

    /// @notice Returns an accurate balance of a user at the current timestamp
    ...
    function balanceOfAt(address addr, uint256 _block) external view override returns (uint256) {
        ...
        // reference yet
        require(_block <= block.number, "Future");
        require(_block >= deployBlock, "Before deployment");

        // Binary search of user point nearest but before _block, returns 0 in case _block is before first point
        uint256 _min = 0;
    ...
    /// @dev For this function to be accurate, point must be the first historical point before t.
    /// @param point The point (bias/slope) to start search from
    /// @param t Time to calculate the total voting power at
    /// @return Total voting power at that time
    function _supply_at(Point memory point, uint256 t) internal view returns (uint256) {
        Point memory last_point = point;
        require(t >= last_point.ts, "Unsupported");
    ...
    /// @notice Calculate total voting power, becomes more inaccurate the further one goes in the past.
    ...
    /// @notice Calculate total voting power, becomes more inaccurate the further one goes in the past.
    /// @dev Consider using totalSupplyAt in case balances further in the past need to be fetched with accuracy.
    function totalSupplyAtT(uint256 t) external view override returns (uint256) {
        return _totalSupply(t);
    }

    /// @notice Returns an accurate total voting power at the current timestamp
    ...
    // Listeners view functions
    function getListenerAt(uint256 listenerId) external view override returns (address) {
        require(listenerId < listeners.length(), "!Exist");
        return listeners.at(listenerId);
    }

    function getListenerLength() external view override returns (uint256) {
        return listeners.length();
    }

    // Dummy methods for compatibility with Aragon
    function changeController(address _newController) external override {
        require(msg.sender == controller || hasRole(DEFAULT_ADMIN_ROLE, msg.sender), "!controller");
