...
function payout(uint256 listId, address _addr, uint256 _amount, bool cashPayout, uint8 _prize) external {
    ...
    if(listIdToMonthly[id].isMonthly == true) {
        ...
        if(amount <= adminPoints) {
            ...
            if(remainder <= royaltyPoints) {
                ...
                listIdToMonthly[id].currentRoyaltyPoints -= remainder;
                ...
                if(remainder <= referralPoints) {
                    ...
                    listIdToMonthly[id].currentReferralPoints -= remainder;
        ...
        if(canClaim) {
            ...
            if(remainder <= royaltyPoints) {
                ...
                listIdToQuarterly[id].currentRoyaltyPoints -= remainder;
                ...
                if(remainder <= referralPoints) {
                    ...
                    listIdToQuarterly[id].currentReferralPoints -= remainder;
        ...
        if(canClaim) {
            ...
            if(remainder <= royaltyPoints) {
                ...
                listIdToSemiAnnually[id].currentRoyaltyPoints -= remainder;
                ...
                if(remainder <= referralPoints) {
                    ...
                    listIdToSemiAnnually[id].currentReferralPoints -= remainder;
    ...
    paymentTracker[programs].totalLastMonth+= amount;
...
function getPoints(uint256 listId, address _addr) public view returns (uint256, uint256, uint256) {
    ...
    uint256 reward = rewardThisPeriod;
        ...
        uint256 rewardPointsThisPeriod = currentRewardPoints + (time * reward.div(30).div(86400));
        if(rewardPointsThisPeriod > amountPaid) {
            rewardPointsThisPeriod = amountPaid - getPreviousRewardPoints(id) + currentRewardPoints;
        }
        
       
        uint256 referralPointsThisPeriod = currentReferralPoints + (time * referralThisPeriod.div(30).div(86400));
        if(referralPointsThisPeriod > amountPaid) {
            referralPointsThisPeriod = amountPaid - getPreviousReferralPoints(id) + currentReferralPoints;
        }

        
        uint256 royaltyPointsThisPeriod = currentRoyaltyPoints + (time * royaltyThisPeriod.div(30).div(86400));
        if(royaltyPointsThisPeriod > amountPaid) {
            royaltyPointsThisPeriod = amountPaid - getPreviousRoyaltyPoints(id) + currentRoyaltyPoints;
        }
        

        return (rewardPointsThisPeriod, referralPointsThisPeriod, royaltyPointsThisPeriod);
