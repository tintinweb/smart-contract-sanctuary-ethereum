...
contract LiquidityIncreaserJob is ILiquidityIncreaserJob, Keep3rJob {
   ...
   */
  constructor(IPoolManagerFactory _poolManagerFactory, address _governor, IERC20 _weth) payable Governable(_governor) {
  ...
  /// @inheritdoc ILiquidityIncreaserJob
  function work(IPoolManager _poolManager, uint256 _wethAmount, uint256 _tokenAmount)
    external
    upkeep(msg.sender)
    notPaused
  {
    if (!POOL_MANAGER_FACTORY.isChild(_poolManager)) {
      revert LiquidityIncreaserJob_InvalidPoolManager();
    }
    ...
    if (_totalEth < minIncreaseWeth) {
      revert LiquidityIncreaserJob_InsufficientIncrease();
    }
  ...
  function _quoteTokenToEth(IPoolManager _poolManager, uint256 _tokenAmount) internal returns (uint256 _ethAmount) {
    ...
    if (_priceOracle.isManipulated(_poolManager.POOL())) {
      revert LiquidityIncreaserJob_PoolManipulated();
    }
