...
abstract contract Staking is ValidatorRegistry, Multicall {
    ...
    bool public _VALIDATORS_MIGRATED;
    ...
    function _calcRewards(address validator, address delegator) internal view returns (uint256 rewards) {
        ...
        for (uint256 i = history.delegationGap; i < delegations.length; i++) {
            ...
            uint256 balance = _fromShares(validatorPool, delegations[i].shares);
    ...
    function _claimRewards(address validator, address delegator) internal returns (uint96 availableRewards) {
        ...
        for (uint i = history.delegationGap; i < delegations.length; i++) {
            ...
            uint256 amount = uint256(delegations[i].amount) * BALANCE_COMPACT_PRECISION;
            if (balance > amount) {
                recordReward = balance - amount;
                // add reward to collected
                availableRewards += uint96(recordReward);
        ...
        // save changes
        _validatorPools[validator].sharesSupply = validatorPool.sharesSupply;
        _validatorPools[validator].totalRewards = validatorPool.totalRewards;
    }
    ...
    function _undelegate(address validator, address delegator, uint256 amount) internal {
        ...
        (uint96 availableRewards, uint256 spentShares) = _stashUnlocked(validatorPool, delegator, compactAmount, beforeEpoch);
        ...
        _safeTransferWithGasLimit(payable(delegator), availableRewards);
        // emit event
        emit Claimed(validator, delegator, availableRewards, beforeEpoch);
    ...
    function _stashUnlocked(
        ...
        uint64 beforeEpoch
    ) internal returns (uint96 availableRewards, uint256 spentShares) {
        address validatorAddr = validatorPool.validatorAddress;

        DelegationHistory memory history = _delegationHistory[validatorAddr][delegator];
        Delegation[] memory delegations = history.delegations;

        // work with memory because we can't copy array
        Delegation[] storage storageDelegations = _delegationHistory[validatorAddr][delegator].delegations;

        uint64 lockPeriod = _stakingConfig.getLockPeriod();
        uint256 unlockedAmount = uint256(expectedAmount) * BALANCE_COMPACT_PRECISION;
        uint256 gap = history.delegationGap;

        while(gap < delegations.length && delegations[gap].epoch + lockPeriod < beforeEpoch && expectedAmount > 0) {
            if (delegations[gap].amount > expectedAmount) {
                // deduct undelegated amount
                uint256 shares = _toShares(validatorPool, uint256(expectedAmount) * BALANCE_COMPACT_PRECISION);
                delegations[gap].amount -= expectedAmount;
                delegations[gap].shares -= shares;
                spentShares += shares;
                // expected amount is filled
                expectedAmount = 0;
                // save changes to storage
                storageDelegations[gap] = delegations[gap];
                break;
            }
            spentShares += delegations[gap].shares;
            expectedAmount -= delegations[gap].amount;
            delete storageDelegations[gap++];
        }

        require(expectedAmount == 0, "still locked");
        // deduct unlocked amount in shares from spentShare to get reward
        availableRewards = uint96(_fromShares(validatorPool, spentShares) - unlockedAmount);

        // save new state
        _delegationHistory[validatorAddr][delegator].delegationGap = gap;
        _delegationHistory[validatorAddr][delegator].unlockedAmount += unlockedAmount;
        _delegationHistory[validatorAddr][delegator].lastUnlockEpoch = beforeEpoch;
    }
    ...
    function migrateValidators() external onlyFromGovernance {
        ...
        require(!_VALIDATORS_MIGRATED, "Staking: already migrated");
    ...
    function migrateDelegator(address delegator) public {
        ...
        for (uint256 i; i < validators.length; i++) {
            ...
            {
                uint112 delegated;
                for (uint256 j = delegations.delegateGap; j < delegations.delegateQueue.length; j++) {
                    if (delegations.delegateQueue[j].amount < delegated) {
                        delegated -= delegated - delegations.delegateQueue[j].amount;
                        continue;
                    } else if (delegations.delegateQueue[j].amount == delegated) {
                        continue;
                    }
                    uint112 realAmount = delegations.delegateQueue[j].amount - delegated;
                    delegated += realAmount;
                    newDelegations.push(Delegation(delegations.delegateQueue[j].epoch, realAmount, uint256(realAmount) * BALANCE_COMPACT_PRECISION));
                    delegated += delegations.delegateQueue[j].amount;
                }
                _stakerAmounts[validatorAddress][delegator] += delegated;
                _stakerShares[validatorAddress][delegator] += uint256(delegated) * BALANCE_COMPACT_PRECISION;
            }

            {
                ...
                _delegationHistory[validatorAddress][delegator].unlockedAmount = undelegated;
                validatorPool.unlocked += undelegated;
            }

            delete _validatorDelegations[validatorAddress][delegator];
            _validatorPools[validatorAddress] = validatorPool;
            _delegationHistory[validatorAddress][delegator].unlockedAmount = history.unlockedAmount;
            _delegationHistory[validatorAddress][delegator].lastUnlockEpoch = history.lastUnlockEpoch;
    ...
    function _processDelegateQueue(address validator, address delegator, uint64 beforeEpochExclude) internal returns (uint256 availableFunds) {
