...
contract FarmReward is IFarmReward, Initializable, Pausable, Ownable {
    ...
    // useraddr -> recordindex -> Checkpoint
    mapping(address => mapping(uint256 => Model.Checkpoint[])) public checkpointMap;
    address[] checkpointAddrs; // all checkpoint addrs
    ...
    function _claimAll(address userAddr) public whenNotPaused needInit {
        require(!appConf.validBlacklist(userAddr), "can not claim");
        require(farmStaking.getCurrentStakingCount(userAddr) > 0, "staking count is 0");
        ...
        for (uint256 index = 0; index < stakingRecords.length; index++) {
            ...
            if (stakingRecords[index].status == Model.STAKING_RECORD_STATUS_UNSTAKED) {
                continue;
            }
    ...
    function claimAll(address userAddr) external whenNotPaused needInit claimLock {
        _claimAll(userAddr);
    }
    ...
    function claim(uint256 index) public whenNotPaused needInit claimLock {
        require(!appConf.validBlacklist(_msgSender()), "can not claim");

        // staking record
        Model.StakingRecord[] memory stakingRecords = farmStaking.getStakingRecords(_msgSender());
        require(stakingRecords.length > index, "index out of range");

        _claim(_msgSender(), index, stakingRecords[index]);
    }

    function proxyClaim(address userAddr, uint256 index) public onlyFarm whenNotPaused needInit claimLock {
        // staking record
        Model.StakingRecord[] memory stakingRecords = farmStaking.getStakingRecords(userAddr);
        require(stakingRecords.length > index, "index out of range");
    ...
    function _claim(address userAddr, uint256 stakingIndex, Model.StakingRecord memory stakingRecord) private {
        require(stakingRecord.status == Model.STAKING_RECORD_STATUS_STAKED, "invalid staking status");
        require(stakingRecord.userAddr == userAddr, "invalid claim user addr");
        ...
        // save checkpoint
        if (checkpointMap[userAddr][stakingIndex].length == 0) {
            checkpointAddrs.push(userAddr);
        }
        ...
        uint256[] memory rewardTokenIds = nftFactory.mintDamo(userAddr, rewardNftType, appConf.getRewardNftTokenGen(), uint8(rewardAmount));
    ...
    function importCheckpoints(address farmRewardAddr) public onlyOwner {
        address[] memory addrs = IFarmReward(farmRewardAddr).getCheckpointAddrs();
        
        for (uint256 aIndex = 0; aIndex < addrs.length; aIndex++) {
            Model.StakingRecord[] memory stakingRecords = farmStaking.getStakingRecords(addrs[aIndex]);
            for (uint256 srIndex = 0; srIndex < stakingRecords.length; srIndex++) {
                Model.Checkpoint[] memory checkpoints = IFarmReward(farmRewardAddr).getCheckpoints(addrs[aIndex], srIndex);
                if (checkpoints.length == 0) {
                    continue;
                }

                // remove old data
                delete checkpointMap[addrs[aIndex]][srIndex];

                for (uint256 cpIndex = 0; cpIndex < checkpoints.length; cpIndex++) {
                    checkpointMap[addrs[aIndex]][srIndex].push(Model.Checkpoint({
                        userAddr: checkpoints[cpIndex].userAddr,
                        blockNumber: checkpoints[cpIndex].blockNumber,
                        timestamp: checkpoints[cpIndex].timestamp,
                        rewardAmount: checkpoints[cpIndex].rewardAmount
                    }));
                }
            }
        }
