    ...
    IERC721Enumerable
{
    ...
    function _mintMultiple(address _owner, uint256[] calldata tokenIds)
        internal
        virtual
    {
        require(
        ...
        for (uint256 i = 0; i < tokenIds.length; i++) {
            _beforeTokenTransfers(address(0), _owner, tokenIds[i]);
            _mint(tokenIds[i], index);
            emit Transfer(address(0), _owner, tokenIds[i]);
            _afterTokenTransfers(address(0), _owner, tokenIds[i]);
        }
