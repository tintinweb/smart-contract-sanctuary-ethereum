...
/**
 * @title EGG for SQFish
 * 
 * @dev EGG material assets building by MinfonTsai
* 
 */
...
contract ERC721 is Context, ERC165, IERC721, IERC721Metadata, Governance {
    ...
    function name() public view virtual override returns (string memory) {
        return string.concat(_name, ' ', thisTrait);
    }
...
interface IFish{
    ...
    function AllBody() external view returns( bytes memory);
}
interface IEgg{

    function eggInfo( uint256 index) external view returns(bytes memory);
...
contract EGGNFT is  IEgg, ERC721Metadata  {
    ...
    mapping(uint256 => bytes) t_pattern;
    mapping(uint256 => string) t_attr;

    bytes public svgHead = hex"3c7376672076696577426f783d2730203020353030203530302720786d6c6e733d27687474703a2f2f7777772e77332e6f72672f323030302f7376672720786d6c6e733a786c696e6b3d27687474703a2f2f7777772e77332e6f72672f313939392f786c696e6b273e";

    bytes constant svgEnd = hex"3c2f7376673e"; 
    bytes constant defs_begin = hex"3c646566733e";
    bytes constant defs_end = hex"3c2f646566733e";
    ...
    mapping(uint256 => uint256) public fishdraw;
    ...
    event MakeUp(uint256 eggid, uint256 bodyid);

    constructor()  ERC721( "EGS", "EGS") { 
        publicmint = true;
        MAX_SUPPLY = 1000;
        thisTrait = "";
    }
    ...
    function mint( uint256 tokenId, string memory data, string memory new_attr) public payable {
        require(publicmint, "Public Minting not enabled!");
        require(msg.value >= publicMintPrice, "mint value not enough!");
        _internal_mint(msg.sender, tokenId);
        t_pattern[tokenId] = StringToBytes(data); 
        t_attr[tokenId] = new_attr;
    }

    /*
    function Sign_mint(uint256 tokenId, bytes memory eggpat, string memory new_attr, bytes memory sig) public payable
    {
        bytes32 message = prefixed(keccak256(abi.encodePacked(tokenId, eggpat, new_attr, this)));
        require( (recoverSigner( message, sig ) == checker),  "sign_key fail" );
        
        require(publicmint, "Public Minting not enabled!");
        require(msg.value >= publicMintPrice, "mint value not enough!");
        _internal_mint(msg.sender, tokenId);
        t_pattern[tokenId] = eggpat; 
        t_attr[tokenId] = new_attr;
    }
    
    function old_Sign_fillEgg(uint256 tokenId, bytes memory eggpat, bytes memory sig) external
    {
        bytes32 message = prefixed(keccak256(abi.encodePacked(tokenId, eggpat, this)));
        require( (recoverSigner( message, sig ) == checker),  "sign_key fail" );
        t_pattern[tokenId] = eggpat; 
    }
    */
    ...
    function _internal_mint(address sender, uint256 tokenId) internal {
        require( sender != address(0x0) , "minter invalid");
        require( totalSupply() + 1 <= MAX_SUPPLY, "mint is over!");   
        require(!_exists(tokenId), "mint invalid tokenid");

        _mint(sender, tokenId);
        eggs[tokenId] = EGG(address(0x0), sender, false);
        emit EggMinted(sender, tokenId);
     ...
     */
    function egg_burn(uint256 tokenId) internal {
    ...
    function eggInfo(uint256 idx) public view returns(bytes memory)
    {
        bytes memory _patt_1 = hex"";

        if( fishdraw[idx] != 0 )
        {
            bytes memory _patt_0 = bytes.concat(t_pattern[ fishdraw[idx] ], defs_end);
            _patt_1 = bytes.concat(defs_begin, _patt_0);   
        }

        if ( nextnode != address(0x0) )
        {
            IEgg _egg = IEgg(nextnode);
            return  bytes.concat(_patt_1, _egg.eggInfo(idx) );
        }
        return _patt_1; 
    }
    ...
    function _defs_begin() internal view returns( bytes memory )
    {
    ...
    function _defs_end() internal view returns( bytes memory )
    {
        if( bodyAddress != address(0x0) )
           return defs_end;
        else
    ...
    function _AllBody() internal view returns( bytes memory )
    {
        if( bodyAddress != address(0x0) )
        {
            IFish f = IFish(bodyAddress);
            return f.AllBody();
        }
        return empty;
    }
    ...
    function formatTokenURI(bytes memory imageURI, uint tokenId) public view returns (string memory) {

            bytes memory now_trait = empty;
            if( bytes(t_attr[tokenId]).length != 0)
            {
                now_trait = abi.encodePacked('{"trait_type": "',thisTrait,'", "value": "',t_attr[tokenId],'"}');
            }
            return string(
                abi.encodePacked(
                    ...
                    Base64.encode(
                        abi.encodePacked(
                            ...
                            '", "description": "On-chain components", ',
                            '"image": "',imageURI,'", ',
                            '"attributes": [',now_trait,']}'
                        /*    '"attributes": [',
                            now_trait,
                            '],"image":"',imageURI,'"}'
                        */    
    ...
    function tokenURI(uint256 tokenId) public override view returns (string memory theURI) {
        ...
        bytes memory svgBytes = _generateSVG(tokenId);
    ...
    function draw(uint256 eggId, uint256 fishId) external 
    {
        IERC721 _body = IERC721(bodyAddress);
        require( _body.ownerOf(fishId)==msg.sender, "owner invalid" );
        
        eggs[eggId].used = true;
        eggs[eggId].drawer = msg.sender;

        egg_burn(eggId);
        fishdraw[fishId] = eggId;
        emit MakeUp( eggId, fishId );
    }

    function setTraitName( string memory _trait_name ) external onlyGovernance
    {
        thisTrait = _trait_name;
    }

    function setMainBody( address _bodyaddr ) external onlyGovernance
    {
        bodyAddress = _bodyaddr;
    }
    
    function nextTrait(uint256 fishid) public view returns ( bool )
    {
        bool now_state = false;
        uint256 draw_idx =  fishdraw[fishid];
        ...
        if( draw_idx != 0 )
        {
            ...
            {
                    IEgg _egg = IEgg(nextnode);
                    bool next_state = _egg.nextTrait(fishid);
    ...
    function getTraits(uint256 fishid) public view returns ( bytes memory )
    {
        bytes memory now_trait = empty;
        bool now_state = false;
        uint256 draw_idx =  fishdraw[fishid];
        ...
        if( draw_idx != 0 )
        {
            ...
            {
                    IEgg _egg = IEgg(nextnode);
                    bytes memory _datas = _egg.getTraits(fishid);
                    bool next_state = _egg.nextTrait(fishid);
