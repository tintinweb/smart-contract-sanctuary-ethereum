...
contract ToyzNFT is ERC721A, MultiSigOwnable, Pausable, ReentrancyGuard {
    ...
    // Provenance hash of all tokens metdata, keccak256("ipfs://{CID}/")
    bytes32 public constant PROVENANCE_HASH =
        0x13d1c361df87adb48e35d4f10316be308c5d4fee46322404d1eb8d9422c303da;

    // Price of a token
    uint256 public constant PRICE = 0.077 ether;
    ...
    /// @dev allow to set base URI only if new hash of the new url is equal to the provenance hash
    /// sales must be finished to change the url
    /// @param url new base URI
    function updateBaseURI(string memory url) external onlyMultiSigOwner {
        // Check if the new URL is valid according to the provenance hash
        if (keccak256(abi.encodePacked(url)) != PROVENANCE_HASH) {
            revert InvalidNewURL();
        }
    ...
    /// @notice unpause the contract, allowing actions to be taken
    function unpause() public onlyMultiSigOwner {
        _unpause();
    }

    /// @dev mint a new token during the whitelist period
    ...
    function mintWhitelist(
        ...
        address signer = ECDSA.recover(
            ECDSA.toEthSignedMessageHash(
                keccak256(abi.encode(_msgSender(), maximum))
            ),
            v,
            r,
            s
    ...
    /// @dev mint a new token after the public release
    /// @param quantity of tokens to mint
    ...
    /// so `_msgSender()` is the multisig aka `owner()`
