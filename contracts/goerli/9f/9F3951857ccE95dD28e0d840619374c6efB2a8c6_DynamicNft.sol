...
contract DynamicNft is ERC721, Ownable {
    ...
    mapping(uint256 => int256) tokenIdToHighValue;

    event CreatedNFT(uint256 indexed tokenId, int256 highValue);

    constructor(
        string memory lowImgSvg,
        string memory highImgSvg,
        address priceFeedAddress
    ) ERC721("Dynamic SVG NFT", "DSN") {
        s_tokenCounter = 0;
        s_lowImgSvgUri = svgToImageURI(lowImgSvg);
        s_highImgSvgUri = svgToImageURI(highImgSvg);
    ...
    function tokenURI(
        ...
        string memory imageURI = s_lowImgSvgUri;
        (, int256 price, , , ) = i_priceFeed.latestRoundData();
        if (price >= tokenIdToHighValue[tokenId]) {
            imageURI = s_highImgSvgUri;
        }
        return
            string(
                abi.encodePacked(
