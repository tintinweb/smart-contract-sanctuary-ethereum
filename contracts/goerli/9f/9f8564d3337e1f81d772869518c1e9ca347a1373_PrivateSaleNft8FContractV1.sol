...
contract PrivateSale8FUpgradeableV1 is Initializable, OwnableUpgradeable {
    ...
    function buyToken(uint amount) public {
        ...
        balances[nftTokenId].totalPayments++;
        ...
        balances[nftTokenId].totalUsd += amount;
        balances[nftTokenId].totalPreparedTokens += amount / tokenPrice;
    ...
    function calculateWithdrawAvailable(uint256 tokenId) public view vestingStarted onlyTokenOwner(tokenId) returns (uint, uint)  {
        uint amount = 0;
        for(uint i = 0; i <= balances[tokenId].totalPayments - 1; i++) {
    ...
    function claimMyMoney (uint256 tokenId) public vestingStarted onlyTokenOwner(tokenId) {
        ...
        for(uint i = 0; i <= balances[tokenId].totalPayments - 1; i++) {
            if (block.timestamp - vestingStartTime >= vestingTime) {
                uint money = balances[tokenId].payment.amount;
                amount += money - balances[tokenId].payment.withdrawed;
                balances[tokenId].payment.withdrawed = money;
            } else {
                uint money = balances[tokenId].payment.amount / (vestingTime / (block.timestamp - vestingStartTime));
                amount += money - balances[tokenId].payment.withdrawed;
                balances[tokenId].payment.withdrawed += money - balances[tokenId].payment.withdrawed;
            }
        }
