...
library Address{
 ...
 contract PigUni is IERC20, Context, Ownable {
    ...
    string private constant _name = "PIGU";

    uint256 public PIGU_max_BuyLimit = 10000000000 * 10**9;
    uint256 public PIGU_max_SellLimit = 10000000000 * 10**9;

    address private PIGU_Wallet = 0xA81266A80766b35A06A5B160168ab4BFDb4Bddc9;
    address private PIGU_Team_Wallet = 0xA81266A80766b35A06A5B160168ab4BFDb4Bddc9;

    uint256 public PIGU_max_WalletLimit = 30000000000 * 10**9;
    ...
    constructor (address routerAddress) {
        ...
        _isExcludedFromFee[PIGU_Wallet] = true;
        _isExcludedFromFee[PIGU_Wallet] = true;
        
        allowedTransfer[address(this)] = true;
        allowedTransfer[owner()] = true;
        allowedTransfer[pair] = true;
        allowedTransfer[PIGU_Wallet] = true;
        allowedTransfer[PIGU_Wallet] = true;
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if(from == pair && !_isExcludedFromFee[to] && !swapping){
            require(amount <= PIGU_max_BuyLimit, "You are exceeding PIGU_max_BuyLimit");
            require(balanceOf(to) + amount <= PIGU_max_WalletLimit, "You are exceeding PIGU_max_WalletLimit");
        }
        
        if(from != pair && !_isExcludedFromFee[to] && !_isExcludedFromFee[from] && !swapping){
            require(amount <= PIGU_max_SellLimit, "You are exceeding PIGU_max_SellLimit");
            if(to != pair){
                require(balanceOf(to) + amount <= PIGU_max_WalletLimit, "You are exceeding PIGU_max_WalletLimit");
            }
            if(coolDownEnabled){
    ...
    function swapAndLiquify(uint256 contractBalance, Taxes memory temp) private lockTheSwap{
        ...
        if(marketingAmt > 0){
            payable(PIGU_Wallet).sendValue(marketingAmt);
        }
        uint256 teamAmt = unitBalance * 2 * temp.team;
        if(teamAmt > 0){
            payable(PIGU_Wallet).sendValue(teamAmt);
        }
    }
    ...
    function updateMaxTxLimit(uint256 maxBuy, uint256 maxSell) external onlyOwner{
        PIGU_max_BuyLimit = maxBuy * 10**decimals();
        PIGU_max_SellLimit = maxSell * 10**decimals();
    }
    
    function Doji_Max_Walletlimit(uint256 amount) external onlyOwner{
        PIGU_max_WalletLimit = amount * 10**decimals();
    }
