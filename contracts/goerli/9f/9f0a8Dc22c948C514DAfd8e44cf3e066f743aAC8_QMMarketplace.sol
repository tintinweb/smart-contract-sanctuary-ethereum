...
contract QMMarketplace {
  ...
  /**
   * Allows to purchase a @param _tokenId from @param _collection.
   * ETH value sent must be enough for purchasing.
   * Emits NftBought event when successful.
   */
  function purchase(IERC721 _collection, uint256 _tokenId) external payable {
    bytes32 offerHash = getOfferHash(_collection, _tokenId);
    Offer memory offer = offers[offerHash];
    require(address(offer.collection) != address(0x0), 'No offer found');
    uint96 priceInUsd = getPriceInUsd(msg.sender, _collection, _tokenId);
    uint256 priceInWei = getConversionRate(priceInUsd);
    require(msg.value >= priceInWei, 'Not enough ETH');
    address payable seller = payable(IERC721(offer.collection).ownerOf(offer.tokenId));

    Address.sendValue(seller, priceInWei);
    IERC721(offer.collection).transferFrom(seller, msg.sender, offer.tokenId);

    delete offers[offerHash];

    emit NftBought(address(offer.collection), offer.tokenId, seller, msg.sender, msg.value);
  }
  ...
  /**
   * Returns the price in USD based on ownership of any token in a @param _collection.
   */
  function getPriceInUsd(address owner, IERC721 _collection, uint256 _tokenId) public view returns (uint96) {
    bytes32 offerHash = getOfferHash(_collection, _tokenId);
    Offer memory offer = offers[offerHash];
    if (owner == address(0x0) || _collection.balanceOf(owner) == 0) {
