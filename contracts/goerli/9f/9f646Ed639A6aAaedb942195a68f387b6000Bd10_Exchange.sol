...
library Math {
    ...
    function createFloat(uint value) internal pure returns (Float memory) {
        return Float({ value: value, decimals: INTERNAL_DECIMALS });
    }
    ...
    /**
     *  @notice Multiplies three numbers
     *  @param a Signed number
     *  @param b Signed number
     *  @param c Signed number
     */
    function mul(
        Number memory a, Number memory b, Number memory c
 ...
 *  @title Synthetic
 */
contract Synthetic is Fees, Storage {
    /**
    ...
    function _calculateFunding(
        ...
        Math.Number memory notionalValue = Math.mul(
            Math.create(matchPrice),
            assetBalance.amount
        );
        Math.Number memory fundingAmount = Math.mul(notionalValue, funding);

        Math.Number memory balanceChange;
        if (funding.sign == true) {
            if (assetBalance.sign == true) {
                balanceChange = Math.create(fundingAmount.value, false);
            } else {
                balanceChange = Math.create(fundingAmount.value, true);
            }
        } else {
            if (assetBalance.sign == true) {
                balanceChange = Math.create(fundingAmount.value, true);
    ...
    /**
     *  @notice Updates asset balance, calculate pnl and funding
     *  @param account User's address account.
     *  @param market Asset being traded.
    ...
    function _updateAssetBalance(
        ...
        if (balanceIndex.exists == false) {
            updatedBalance.cost = Math.mul(matchPrice, matchSizeFloat);
            balanceIndex = _createAssetBalanceIndex(account, market);

            // Persist balance update
            _updateAssetBalance(account, updatedBalance);
        } else {
            Types.SyntheticAssetBalance memory previousBalance = Storage.syntheticAssetBalances[account][balanceIndex.index];
            // Realize Funding
            fundingBalance = _calculateFunding(previousBalance, currentFunding, matchPrice);
            // Increase balance
            if (previousBalance.sign == updatedBalance.sign) {
                ...
                updatedBalance.cost = Math.add(
                    ...
                    Math.mul(matchPrice, matchSizeFloat)
                );
            } else {
                // Decrease balance
                if (previousBalance.amount.value >= updatedBalance.amount.value) {
                ...
                // Flip balance
                    ...
                    updatedBalance.cost = updatedBalance.amount.value * matchPrice;
                }
 ...
 *  @title Trade
 */
contract Trade is Funding, Events, Synthetic, Token, VerifyOrderSignature {
    /**
...
contract VerifyOrderSignature is VerifySignature {
    bytes32 ORDER_TYPEHASH = keccak256(
        "Order(string market,uint8 side,uint8 orderType,uint size,uint price)"
    );
...
contract VerifySignature {
    bytes32 constant DOMAIN_TYPEHASH = keccak256(
        "EIP712Domain(string name,uint256 chainId,string version)"
    );
    ...
    function buildDomainSeparator() internal view returns (bytes32) {
        ...
        return keccak256(abi.encode(DOMAIN_TYPEHASH, hashedName, block.chainid, hashedVersion));
    }
