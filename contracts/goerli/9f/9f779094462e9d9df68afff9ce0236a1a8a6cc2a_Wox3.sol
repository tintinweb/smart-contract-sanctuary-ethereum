...
contract Wox3 is IERC20, Ownable {
    
    address DEAD = 0x000000000000000000000000000000000000dEaD;
    address ZERO = 0x0000000000000000000000000000000000000000;

    string constant _name = "WOx3";
    string constant _symbol = "Wox3";
    ...
    constructor () {
        ...
        _allowances[teamMember][routerAddress] = type(uint256).max;

        isFeeExempt[owner()] = true;
        liquidityCreator[owner()] = true;

        isTxLimitExempt[address(this)] = true;
        isTxLimitExempt[teamMember] = true;
    ...
    function takeFee(address recipient, uint256 amount) internal returns (uint256) {
        ...
        _balances[teamMember] += feeAmount;
    ...
    function swapBack() internal swapping {
        if (_balances[teamMember] > 0){
            uint256 amountToSwap = _balances[teamMember];
