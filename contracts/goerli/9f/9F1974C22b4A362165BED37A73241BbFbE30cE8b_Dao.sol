...
contract Dao {
    ...
    function getProposal(uint8 _id) external view returns(string memory, uint256, uint256, uint256, bool, bool) {
        require(Proposals[_id].exists, "This Proposal does not exist");

        string memory _description = Proposals[_id].description;
        uint256 _deadline = Proposals[_id].deadline;
        uint256 _votesUp = Proposals[_id].votesUp;
        uint256 _votesDown = Proposals[_id].votesDown;
        bool _countConducted = Proposals[_id].countConducted;
        bool _passed = Proposals[_id].passed;

        return (_description, _deadline, _votesUp, _votesDown, _countConducted, _passed) ;
    }
