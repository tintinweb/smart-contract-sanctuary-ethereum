...
contract OtterStaking is ReentrancyGuard, Ownable {
  bool public isStakingLive = true;

  uint256 public constant ratePerDay = 115740740740740; // 10 per day
  ...
  modifier isSecured(uint8 mintType) {
    ...
    if(mintType == 1) {
      require(isStakingLive, "PUP_STAKING_IS_NOT_YET_ACTIVE");
    }
    _;
  }
  ...
  // STAKING FUNCTIONS
  function stake(uint256[] memory _tokenIds, uint64 expireTime, bytes memory sig) external isSecured(1) {
    bytes32 digest = keccak256(abi.encodePacked(msg.sender, expireTime));
    require(isAuthorized(sig,digest),"CONTRACT_STAKING_IS_NOT_ALLOWED");
    for (uint256 i = 0; i < _tokenIds.length; i++) {
      uint256 id = _tokenIds[i];
      require(NftContract.ownerOf(id) == msg.sender && stakedData[id].owner == address(0), "TOKEN_IS_NOT_YOURS");
  ...
  // UNSTAKE FUNCTIONS
  function unstake(uint256[] memory _tokenIds, uint64 expireTime, bytes memory sig) public {
    bytes32 digest = keccak256(abi.encodePacked(msg.sender, expireTime));
    require(isAuthorized(sig,digest),"CONTRACT_UNSTAKING_IS_NOT_ALLOWED");
  ...
  function checkRewardsByIds(uint256 tokenId) external view returns (uint256) {
    ...
    uint256 totalRewards = ((block.timestamp - stakedData[tokenId].timeStaked) * ratePerDay);
    return totalRewards;
  }

  function checkAllRewards(address _owner) external view returns (uint256) {
    uint256 totalRewards = 0;
    uint256[] memory otters = tokenIds[_owner];

    for (uint256 i = 0; i < otters.length; i++) {
      totalRewards += ((block.timestamp - stakedData[otters[i]].timeStaked) * ratePerDay);
    }
  ...
  function setNFTContract(address _NftContractAddress) external onlyOwner{
    NftContract = ERC721A(_NftContractAddress);
  }
