...
contract VaultFactory {
    ...
    // index => proxyAddress
    mapping(address => uint256) public vaultAddress_to_vaultId;
    mapping(uint256 => address) public vaultDetails;
    ...
    function buildVault(
        ...
        BeaconProxy vault = new BeaconProxy(
            ...
            abi.encodeWithSelector(
                ...
                _name,
                _symbol,
                getDecimals(_assetAddress)
            )
        );
        vaultAddress_to_strategyAddress[address(vault)] = _strategyAddress;
        vaultAddress_to_vaultId[address(vault)] = _vaultId;
        vaultDetails[_vaultId] = address(vault);
...
contract SquiddyCore is ERC4626, AccessControl {
    ...
    function initializeBeaconProxy(bytes memory _data) public onlyVaultFactory {
        (
            address _underlying,
            string memory _name,
            string memory _symbol,
            uint8 _decimals
        ) = abi.decode(_data, (address, string, string, uint8));
