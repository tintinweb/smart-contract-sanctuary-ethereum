...
contract UniV3TWAPOracle is Owned {
    ...
    // Misc
    uint32 public lookback_secs = 1; // 1 seconds
    ...
    PausableUpgradeable
{
    ...
    function isValidMinter(address _minter) external view returns (bool) {
        return minters[_minter].is_listed && minters[_minter].approved_loan_amount > 0;
    }
    ...
    /**
     * @notice Set approved loan amount of a Minter
     * Update approved loan amount of a minter.
     * @param _minter Address of a minter.
     * @param _amount Approved Loan Amount.
     */
    function setApprovedLoanAmount(address _minter, uint256 _amount)
        external
        onlyOwner
        validMinter(_minter)
    {
        minters[_minter].approved_loan_amount = _amount;
        ...
        whenNotPaused
    {
        ...
        if (minters[msg.sender].minted_amount + _amount > minters[msg.sender].max_sweep_amount) 
     ...
     * @param _approved_amount Approved loan amount for mint.
     * @param _max_amount Sweep maximum amount.
     */
    function addMinter(address _minter, uint256 _approved_amount, uint256 _max_amount) public onlyOwner {
        if (_minter == address(0)) revert ZeroAddressDetected();
        if (_approved_amount == 0 || _max_amount == 0) revert ZeroAmountDetected();
        if (minters[_minter].is_listed) revert MinterExist();

        Minter memory new_minter = Minter({
            approved_loan_amount: _approved_amount,
            minted_amount: 0,
            max_sweep_amount: _max_amount,
            is_listed: true,
            is_enabled: true
        });
...
contract SweepDollarCoin is BaseSweep {
    ...
    /**
     * @notice Set max sweep amount of a Minter
     * @param _minter Address of a minter.
     * @param _amount Max Sweep amount.
     */
    function setMaxAmount(address _minter, uint256 _amount)
        external
        onlyOwnerOrBalancer
        validMinter(_minter)
    {
        minters[_minter].max_sweep_amount = _amount;
