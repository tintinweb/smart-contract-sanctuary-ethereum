...
contract SuperfluidVerifier is BaseVerifier {
    ...
    mapping(uint256 => bool) s_hasBeenTraded;
    ...
    function preTransfer(
        ...
        ISuperToken spToken = ISuperToken(nfr.token);
        ...
        if (s_hasBeenTraded[nfrId]) {
            ...
            uint256 flowRateToNewOwnerPerSecond = (uint256(
                int(flowRatePerSecond)
            ) * nfr.faceValue) / nfr.totalIncome;

            s_cfaV1Forwarder.setFlowrateFrom(
                spToken,
                nfr.from,
                currentOwner,
                flowRatePerSecond - int96(int(flowRateToNewOwnerPerSecond))
            );
            s_cfaV1Forwarder.setFlowrateFrom(
                spToken,
                nfr.from,
                newOwner,
                int96(int(flowRateToNewOwnerPerSecond))
            );
        }
    }
}
