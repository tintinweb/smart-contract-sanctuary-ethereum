...
contract SqushyLabsMintUpgradeable is ERC721AUpgradeable, OwnableUpgradeable {
    ...
    address public squshyLabsAddress;
    ...
    function _initialize(Config memory _config) initializerERC721A initializer public {
        ...
        squshyLabsAddress = _config.squshyLabsAddress;
        withdrawalAddress = _config.withdrawalAddress;
    ...
    function mint(uint256 amount, address mintAddress) external payable {
        ...
        require(msg.value == amount * publicMintPrice, "CONTRACT ERROR: not enough ether sent");
        _safeMint(mintAddress, amount);
    }

    // allow list minting function
    function allowListMint(uint8 v, bytes32 r, bytes32 s, uint256 amount, uint256 allowedAmount, address mintAddress) external payable {
    ...
    function withdraw() external payable {
        AddressUpgradeable.sendValue(payable(squshyLabsAddress), ((commission * address(this).balance) / 1000));
        AddressUpgradeable.sendValue(payable(withdrawalAddress), address(this).balance);
    }
}
