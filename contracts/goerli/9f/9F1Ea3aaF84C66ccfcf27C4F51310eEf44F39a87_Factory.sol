...
contract NFTcontract is ERC721Enumerable, ReentrancyGuard {
    ...
    mapping(uint256 => uint256) private arrNumOfContents;
    uint256 private lengthOfBaseURIs;
    ...
    uint256 private totalCntOfContent;
    ...
    constructor(
        ...
        totalCntOfContent = 0;
        lengthOfBaseURIs = 0;
        for (uint256 i = 1; i <= maxPublicMint; i++) {
            PUBLIC_MINT_PRICES[i] = _publicMintPrice;
        }
    }
    ...
    modifier isCorrectPayment(uint256 stage, uint256 numberOfTokens) {
        uint256 tmpId = publicMintId + numberOfTokens - 1;
        uint256 payAmount = 0;
        if (stage == 0) {
            require(
                PRE_MINT_PRICES[tmpId] > 0,
                "Pre-Mint of these tokens not available yet."
            );
            for (uint256 i = 0; i < numberOfTokens; i++) {
                payAmount += PRE_MINT_PRICES[publicMintId + i];
            }
        } else if (stage == 1) {
            require(
                PUBLIC_MINT_PRICES[tmpId] > 0,
                "Mint of these tokens not available yet."
            );
            for (uint256 i = 0; i < numberOfTokens; i++) {
                payAmount += PUBLIC_MINT_PRICES[publicMintId + i];
            }
        }
        require(
            payAmount == msg.value,
    ...
    function updateOperatorsFilter(address[] calldata _operators, bool[] calldata _allowed) external onlyOwner {
        ...
        for (uint256 i = 0; i < _allowed.length; i++) {
            if (_allowed[i] != operatorFilterRegistry.isOperatorAllowed(address(this), _operators[i])) {
        ...
        returns (string memory)
    {
        ...
        for (uint256 i = 0; i < lengthOfBaseURIs; i++) {
            if (tokenId <= arrNumOfContents[i]) {
                return
                    string(
                        abi.encodePacked(
                            arrBaseURIs[i],
    ...
    // ============ FUNCTION TO UPDATE MULTIPLE BASEURIS ============
    function updateBaseURI(
        uint256[] calldata _numOfTokens,
        string[] calldata _baseURIs
    ) external {
        require(
            _numOfTokens.length == _baseURIs.length,
            "Please input the numOfTokens and baseURIs info as same length."
        );
        for (uint256 i = 0; i < _numOfTokens.length; i++) {
            totalCntOfContent += _numOfTokens[i];
            arrNumOfContents[lengthOfBaseURIs] = totalCntOfContent;
            arrBaseURIs[lengthOfBaseURIs] = _baseURIs[i];
            lengthOfBaseURIs++;
        }
    }
    ...
    function updateFixedModelStagesInfo(
        ...
        for (uint256 i = 1; i <= maxPublicMint; i++) {
            PRE_MINT_PRICES[i] = _premintPrice;
            PUBLIC_MINT_PRICES[i] = _publicMintPrice;
        }
    }
    ...
    function updateDynamicModelStagesInfo(
        uint256[] calldata _premintPrices,
        uint256[] calldata _publicMintPrices
    ) external onlyOwner {
        for (uint256 i = 0; i < _premintPrices.length; i++) {
            PRE_MINT_PRICES[i + 1] = _premintPrices[i];
        }
        for (uint256 i = 0; i < _publicMintPrices.length; i++) {
            PUBLIC_MINT_PRICES[i + 1] = _publicMintPrices[i];
        }
    }
}