...
contract InventoryFreaks is Ownable {
  ...
  mapping(uint256 => Layer)[4] internal _layers;

  function addLayers(LayerInput[] memory inputs) external onlyOwner {
    for (uint256 i = 0; i < inputs.length; i++) {
      _layers[inputs[i].itemIndex][inputs[i].layerIndex] = Layer(inputs[i].name, inputs[i].data);
    }
  }

  function getLayer(uint8 itemIndex, uint8 layerIndex) external view returns (Layer memory) {
    return _layers[itemIndex][layerIndex];
  }
  ...
  function getAttributes(Freak memory character) external view returns (bytes memory) {
    return
      abi.encodePacked(
        ...
        character.species == 1 ? "Fairy" : character.species == 2 ? "Ogre" : "Troll",
        '"},'
        '{"trait_type": "Body", "value": "',
        _layers[0][character.body].name,
        '"},',
        '{"trait_type": "Armor", "value": "',
        _layers[1][character.armor].name,
        '"},',
        '{"trait_type": "Main Hand", "value": "',
        _layers[2][character.mainHand].name,
        '"},',
        // '{"trait_type": "Off Hand", "value": "',
        // _layers[3][character.offHand].name,
        // '"},',
  ...
  function getImage(Freak memory character) external view returns (bytes memory) {
    return
      abi.encodePacked(
        _buildImage(_layers[0][character.body].data),
        _buildImage(_layers[1][character.armor].data),
        _buildImage(_layers[2][character.mainHand].data),
        _buildImage(_layers[3][character.offHand].data)
      );
  }
