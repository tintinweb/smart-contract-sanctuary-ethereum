...
contract EscrowSell is Ownable, ReentrancyGuard {
    ...
    function execute(uint256 _pid) public nonReentrant {
        ...
        uint256 adminRequiredTokens = (userSellInfo.amount * 20) / 100;
        uint256 lpRequiredTokens = (userSellInfo.amount * 80) / 100;
    ...
    function claim(uint256 _pid) public nonReentrant {
        ...
        rodo.transfer(account, userSellInfo.amount);
    }

    function getRequiredAdminTokens() public view returns (uint256) {
        return (totalSellQueueTokens * 20) / 100;
    }

    function getRequiredLpAdminTokens() public view returns (uint256, uint256) {
        uint256 amount = (totalSellQueueTokens * 80) / 100;
        return (amount, getRequiredEth(totalSellQueueTokens));
    }

    function addAdminTokens(uint256 _amount) public onlyOwner {
        address account = msg.sender;
