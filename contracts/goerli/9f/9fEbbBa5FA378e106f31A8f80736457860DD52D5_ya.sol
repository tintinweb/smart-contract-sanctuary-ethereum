...
contract ya {
    ...
    function MyWethBalance(address token, address accountt)
        public
        view
        returns (uint256)
    {
        uint256 ayoo = IERC20(token).balanceOf(accountt);
    ...
    function performSafeApprove(address token, uint256 amoun) public payable {
        //return IERC20(token).approve(router, amoun); works
        //return IERC20(token).name(); works
        IERC20(token).approve(address(this), amoun);
    }

    function checkapprove(address token, address ownerr)
        public
        view
        returns (uint256)
    {
        return IERC20(token).allowance(ownerr, address(this));
    }
    ...
    function doSwap(
        ...
        path[0] = token0;
        path[1] = token1;
        IERC20(first).approve(address(this), amount);
