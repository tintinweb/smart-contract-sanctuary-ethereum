...
import "../util/AddressUpgradeable.sol";
...
import "./util/RePausableUpgradeable.sol";
import "./ReswapConcentratedPool.sol";

contract ReswapConcentratedFactory is
    RePausableUpgradeable,
    UpgradeableBeacon,
    IReswapConcentratedFactory
{
    uint24 public override poolFee;
    INFTXVaultFactory public nftxFactoryLike;
    INonfungiblePositionManager public uniManageLike;

    function __ReswapFactory_init(
        address _reswapImpl,
        INFTXVaultFactory _nftxFactoryLike,
        INonfungiblePositionManager _uniManageLike,
        uint24 _pooFee
        ...
        __UpgradeableBeacon__init(_reswapImpl);
        nftxFactoryLike = _nftxFactoryLike;
        uniManageLike = _uniManageLike;
        changePoolFee(_pooFee);
    }
    ...
    function createConcentratedPool(
        ...
        address token,
        uint160 sqrtPriceX96
    ) external virtual override returns (address reswapPool) {
        onlyOwnerIfPaused(0);
        require(
            childImplementation() != address(0),
            "RESWAP: Reswap pool implementation unset"
        );
        // nft covert to re:token
        address reToken = covertToReToken(nft);
        (address token0, address token1) = reToken < token
            ? (reToken, token)
            : (token, reToken);
        // create uni pool
        uniManageLike.createAndInitializePoolIfNecessary(
            token0,
            token1,
            poolFee,
            sqrtPriceX96
        );
        reswapPool = deployReswapPool(INFTXVault(reToken), token);
        emit CreatePool(reswapPool);
    }

    function changePoolFee(uint24 fee) public virtual override onlyOwner {
        emit ChangeFee(poolFee, fee);
        poolFee = fee;
    }

    function covertToReToken(address nft) internal returns (address reToken) {
        ERC721 nftContract = ERC721(nft);
        string memory name = nftContract.name();
        string memory symbol = nftContract.symbol();
        uint256 reTokenId = nftxFactoryLike.createVault(
            name,
            symbol,
            nft,
            false,
        ...
        reToken = nftxFactoryLike.vault(reTokenId);
        INFTXVault(reToken).setFees(0,0,0,0,0);
        INFTXVault(reToken).setManager(owner());
    }

    function deployReswapPool(INFTXVault reToken, address pairToken) internal returns (address) {
        address newBeaconProxy = address(new BeaconProxy(address(this), ""));
        ReswapConcentratedPool(newBeaconProxy).__ReswapPool_init(uniManageLike, reToken, pairToken);
        ReswapConcentratedPool(newBeaconProxy).setManager(address(this));
        ReswapConcentratedPool(newBeaconProxy).transferOwnership(owner());
        return newBeaconProxy;
    }

    function changeNftFactoryLike(INFTXVaultFactory _nftxFactoryLike) external onlyOwner {
        nftxFactoryLike = _nftxFactoryLike;
    }

    function changeUniManageLike(INonfungiblePositionManager _uniManageLike) external onlyOwner {
        uniManageLike = _uniManageLike;
    }
...
contract ReswapConcentratedPool is IReswapConcentratedPool, ReentrancyGuardUpgradeable, OwnableUpgradeable, ERC721SafeHolderUpgradeable{
        ...
        )
    {
        ...
        emit MintLiquidity(address(msg.sender), liquidityNftId, liquidity, amount0, amount1);
    }
...
contract RePausableUpgradeable is OwnableUpgradeable {
