...
contract AssetBundleSale is IERC1155Receiver, IERC721Receiver, Ownable {
    ...
    /**
     * @notice IERC721Receiver callback, creates a new Sale
    ...
    function onERC721Received(
        ...
        bytes calldata data
         ...
         _safeSetupBundleForSingleTransfer(operator, from, tokenId, 1, data);
    ...
    function onERC1155Received(
        ...
        _safeSetupBundleForSingleTransfer(operator, from, id, value, data);
        return ERC1155_RECEIVED;
    }

    function _safeSetupBundleForSingleTransfer(
        address operator,
        address from,
        uint256 id,
        uint256 value,
...
contract ERC20BaseToken is ERC20, ERC2771Context {
    mapping(address => bool) private _bouncers; // the contracts allowed to mint
    address private _bouncerAdmin; 
...
library ERC1155ERC721Helper {
    ...
    function toFullURI(bytes32 hash, uint256 id) public pure returns (string memory) {
        return string(abi.encodePacked("bafybei", hash2base32(hash), "/", uint2str(id & PACK_INDEX), ".json"));
    }
