...
contract DakMatterSerie2 is ERC721Enumerable, Ownable {
    using Strings for uint256;
   ...
   //the owner must not mint during the sale of the first 2000.
   //this function will be used to mint the 1000 nfts that will be rewarded for the planets.

    function mintOnlyFans(uint256 _amount) public onlyOwner {
    ...
    function mint(uint256 _amount) public payable {
        ...
        if (alien == true)
            require(
                dmd.balanceOf(msg.sender) > 100000000000000000000 wei,
                "to buy NFT you need + de 100 DMD in your address"
            );
        require(!paused);
        require(
            _amount > 0 && _amount <= MAX_MINT_PER_TX,
            "Can only mint between 5 tokens at once"
        );
        require(
            supply + _amount <= maxSupply,
            "Can't mint more than max supply"
    ...
    function burner (uint256 tokenId) public virtual {
        require(_isApprovedOrOwner(_msgSender(), tokenId), "You are not the owner of this collection");
        _burn(tokenId);
    }
