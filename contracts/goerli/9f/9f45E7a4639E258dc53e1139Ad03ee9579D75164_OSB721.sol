...
contract OSB721 is ERC721EnumerableUpgradeable, ERC2981Upgradeable, OwnableUpgradeable {
     ...
     */
    function mint(address _to, string memory _tokenUri) public payable onlyOwnerOrController returns (uint256 tokenId) {
        require(bytes(_tokenUri).length > 0, "Invalid tokenUri");
        lastId.increment();
     ...
     * @return tokenId The unique identifier of the token that was minted, assigned to the owner's address.
     */
    function mintV2(address _to, string memory _tokenUri) public payable returns (uint256 tokenId) {
        require(bytes(_tokenUri).length > 0, "Invalid tokenUri");
        lastId.increment();
        _safeMint(_to, lastId.current());
        _tokenURIs[lastId.current()] = _tokenUri;
     ...
     * @return tokenId The unique identifier of the token that was minted, assigned to the owner's address.
     */
    function mintV3(address _to, string memory _tokenUri) public payable returns (uint256 tokenId) {
     ...
     * @return tokenId The unique identifier of the token that was minted, assigned to the owner's address.
     */
    function mintV4(address _to, uint256 _quantity, string memory _tokenUri) public payable returns (uint256 tokenId) {
        require(msg.value >= mintFee, "Invalid mintFee");
        require(bytes(_tokenUri).length > 0, "Invalid tokenUri");
        lastId.increment();
        _safeMint(_to, lastId.current());
