...
contract Neverland{
    ...
    function createaddress(uint256 total)public{
    ...
    function call_proxy(uint256 begin,uint256 end,address destination,bytes memory data) external payable {
        require(end<=contract_address.length);
        uint256 i=begin;
        bytes memory encoded_data=abi.encodeWithSignature("external_call(address,bytes)", destination,data);
        for (i; i <= end; ++i) {
            ...
            assembly {
                let succeeded := call(
                    gas(),
                    proxy_address,
                    //1 ether=1000,00000,00000,000000 wei
                    0,
    ...
    function external_call(address destination,bytes memory data) external payable{
        assembly {
            let succeeded := call(
                gas(),
                destination,
