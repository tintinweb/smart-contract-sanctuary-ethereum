...
contract Minter is Ownable {
    address public immutable kudasai = 0x2159698bFA0b9F9305D043d65a2840051b60C5CA;
    // address public immutable kudasai = 0xECD0CBBdbFB07986E22981C8D78e17a952605854;
    address public immutable manifoldContract = 0x44e94034AFcE2Dd3CD5Eb62528f239686Fc8f162;
    ...
    function mintBatch(address _creatorContractAddress, uint256 _claimIndex, uint16 _mintCount, uint256 _cost, uint256 _startId, uint256 _endId) public onlyHolder checkId(_startId, _endId) {
        for (uint256 i = _startId; i <= _endId; i++) {
            minterChaild(payable(child[msg.sender][i])).mintBatch(_creatorContractAddress, _claimIndex, _mintCount, _cost, manifoldContract);
        }
    }
...
contract minterChaild is IERC721Receiver {
    ...
    function mintBatch(address _creatorContractAddress, uint256 _claimIndex, uint16 _mintCount, uint256 _cost, address _callContract) public onlyOwner {
        uint32[] memory mintIndices;
        bytes32[][] memory merkleProofs;
        Manifold(_callContract).mintBatch{value: _cost}(_creatorContractAddress, _claimIndex, _mintCount, mintIndices, merkleProofs, address(this));
    }
}
