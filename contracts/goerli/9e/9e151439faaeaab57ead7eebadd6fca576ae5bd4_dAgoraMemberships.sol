...
abstract contract RequestGuildRole is ChainlinkClient {
    ...
    {
        Chainlink.Request memory req =
            buildChainlinkRequest(jobId, address(this), callbackFn);
        req.add(
            "get",
            string.concat(
    ...
    modifier checkRole(bytes32 requestId, uint256 access) {
        ...
        if (access == uint256(Access.NO_ACCESS)) {
            revert NoRole(lastRequest.userAddress, lastRequest.roleId);
        }
        if (access >= uint256(Access.CHECK_FAILED)) {
            revert CheckingRoleFailed(
                lastRequest.userAddress, lastRequest.roleId
            );
        }
    ...
    ReentrancyGuard
{
    ...
    /// @notice Allows contract owner to set a new CID.
    /// @param _newCID The string of the new CID.
    function setNewCID(string memory _newCID) public onlyOwner {
        cid = _newCID;
    }
        ...
        returns (string memory)
    {
        ...
        return
            bytes(cid).length > 0
            ? string(abi.encodePacked(cid, tokenId))
            : "";
    }
