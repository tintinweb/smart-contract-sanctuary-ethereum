...
contract Beacon is IBeaconInterface,Ownable {
    ...
    /// @notice Upgrades the implementation address of the beacon or the address that the beacon points to.
 ...
 */
contract KeeperRegistry is Initializable, OwnableUpgradeable, UUPSUpgradeable, IKeeperRegistry {
    ...
    address[] public registeredKeepers;                 //Array of keepers--useful for looping through to find vote numbers
    ...
    /**
     * @dev Check whether there is a license available for a new keeper.
     * @return True if a license is available, false otherwise.
     */
    function isLicenseAvailable() external view returns (bool) {
        return (registeredKeepers.length < maxNumKeepers);
    }

    /**
     * @dev get keeper array
    ...
    /**
     * @dev get keeper array length
     * @return length of keeper array, i.e. number of keepers registered
     */
     ...
     */
    function joiningForOwner(address[] calldata joiners)
        public
        onlyOwner
    {
        uint256 joinersLength = joiners.length;
        uint256 _firstEmptyKeeperIndex = registeredKeepers.length;
        uint256 _bondAmount = bondAmount;

        //Make sure there are licenses available for these keepers
        require(
            _firstEmptyKeeperIndex + joinersLength <= maxNumKeepers,
            "Insufficient licenses."
        );

        for (uint256 i = 0; i != joinersLength; i++) {
            //Make sure the address isn't already registered. note that this means any given address with any bondAmount will cause the whole function to revert.
            //This is the cheapest way to ensure this address hasn't already been added (which would break the contract).
            require(registry[joiners[i]].bondHeld == 0, "Address not new.");
            registry[joiners[i]] = WorkerDetails({
                bondHeld: _bondAmount,
                arrayIndex: uint128(_firstEmptyKeeperIndex + i),
                leaveDate: 0
            });
            registeredKeepers.push(joiners[i]);
        }

        bondCoin.transferFrom(msg.sender, address(this), _bondAmount * joinersLength);
    }
    ...
    function join(uint256 amount) public   {
        ...
        if (_workerDetails.bondHeld != 0 && _workerDetails.leaveDate == 0)   {
            /*
                If their bondAmount is not zero and they're not queued to leave, they are already a keeper, and join() will 
                deposit more keeper tokens to put them back into good standing if necessary.
            */
            ...
            /*
                Two scenarios here:
                1. If their bondAmount is zero and their leaveDate is zero, they are not yet a keeper, so this is a new address attempting to become a keeper.
                2. If they are queued to leave but have not yet left, they are not a keeper, so this will cancel their leave request (by zeroing out leaveDate) 
                and attempt to make them a keeper.
                Either way the solution is the same.
            */
            uint256 thisKeeperIndex = registeredKeepers.length;
            require(
                thisKeeperIndex < maxNumKeepers,
            ...
            registry[msg.sender] = WorkerDetails({
                ...
                arrayIndex: uint128(thisKeeperIndex),
                leaveDate: 0
            });
            emit permissionChanged(msg.sender, permissionType.FULL);

            //Add the new keeper to the array
            registeredKeepers.push(msg.sender);
        }
    }

    function queueToLeave() public {
        require (isKeeper(msg.sender), "Target keeper is not a keeper.");
        //Remove permissions immediately, allow keeper to remove funds once a transferDelay has passed. This ensures that user can be slashed if they misbehaved just before leaving.
        registry[msg.sender].leaveDate = uint128(block.timestamp + transferDelay);

        //Remove user as keeper--this removes user from the array. Their index will remain for now, but be meaningless.
    ...
    function leave() external {
        ...
        require(info.leaveDate != 0 && info.leaveDate <= block.timestamp, 'LEAVE_DELAY');
    ...
    function isKeeper(address targetAddress)
        ...
        returns (bool)
    {
        uint256 keeperIndex = registry[targetAddress].arrayIndex;
        //firstEmptyKeeperIndex must be checked to avoid out of bounds error
        return keeperIndex < registeredKeepers.length && registeredKeepers[keeperIndex] == targetAddress;
    }
    ...
    function denounce(address targetKeeper, uint256 amount) external onlyOwner {
        require (isKeeper(targetKeeper), "Target keeper is not a keeper.");
        //Remove bondCoin from keeper who is being denounced, add to freeCoin (to be withdrawn by owner)
        uint256 currentBondHeld = registry[targetKeeper].bondHeld;
        ...
        if (newBond < bondAmount) {
            _removeKeeper(targetKeeper);
        }
    ...
    /**
     * @dev Removes target keeper from keeper array and deletes their entry in the registry. Moves other keepers in the array around as necessary.
     * @dev does not touch bondHeld, leaveDate, or generally the targetKeeper's registry--these are only removed if and when the keeper calls leave() or is slashed.
        Before then the registry is still useful to see how much bondCoin the keeper has, and when their leave date is.
     * @param targetKeeper keeper to remove
     */

    function _removeKeeper(address targetKeeper) internal   {
        //Swap last keeper with target keeper, then delete last keeper
        uint256 lastArrayIndex = registeredKeepers.length - 1;

        address lastKeeperAddress = registeredKeepers[lastArrayIndex];
        //Get keeper info for lastKeeper and leaving keeper
        uint128 targetArrayIndex = registry[targetKeeper].arrayIndex;

        //If keeper to be removed is not the last keeper, it must first be swapped with the last keeper to keep the array functional.
        if (targetArrayIndex != lastArrayIndex) {
            //Set lastKeeper's array index to targetKeeper's former array index
            registry[lastKeeperAddress].arrayIndex = targetArrayIndex;

            //Set keeper array at targetKeeper's former location to lastKeeper address
            registeredKeepers[targetArrayIndex] = lastKeeperAddress;
        }
        //Remove last element from registeredKeepers array. The keeper stored at this element (if it was not targetKeeper) is now at targetKeeper's former index.
     ...
     */
    function withdrawFreeCoin(uint256 amount, address targetAddress) external onlyOwner {
        freeCoin -= amount;
     ...
     */
    function changeNumKeepers(uint16 newNumKeepers) external onlyOwner   {
...
contract Orchestrator is IOrchestrator, OwnableUpgradeable, UUPSUpgradeable {
    uint16 public constant actionThresholdPercent = 51; //If an action is approved by >= approvalThresholdPercent members, it is approved
    ...
    */
    ...
    /*  
        actionHash => keeperAddress => vote
    */
    mapping(bytes32 => mapping(address => uint8)) public actionVotes;
    ...
    function initialize(KeeperRegistry _keeperRegistry)
        public
        initializer
    {
        __Ownable_init();
     ...
     * @dev Create an action/proposal which is open for approval for all members of the group.
        An action consists of one or more methods meant to be executed once the action is approved.
        This function is basically equivalent to vote(), except that it also emits an event containing the action information.
     * @param targetAddress the address of the contract to be called with the calldatas.
        Note that a targetAddress pointing to the orchestrator cannot be executed. Since createAction is not actually necessary to run an action,
        we check for this when the action is executed.
     * @param jobEpoch is the job epoch of this action.
     * @param calldatas the calldatas of the methods to be executed.         
        Since calldata includes the function selector, and time-sensitive data can only add parameters, time-sensitive data can only be added to calldata
        constructed to accept it. Passing in time-sensitive data when the selector doesn't accept it, or failing to pass it in when the selector does, will simply cause the
        call to fail.
     *
    ...
    function createAction(
        ...
        // Get # of timeIndependentLengths passed in. If this is greater than 0, that means that this action is using time-sensitive data in one of its methods.
        bytes32 _actionHash = keccak256(
            abi.encode(
                targetAddress,
                jobEpoch,
                calldatas
        ...
        //Have action creator vote for this action (no keeper should ever create an action they aren't endorsing)
        actionVotes[_actionHash][_msgSender()] = 2;
     ...
     * @dev 1 represents no, 2 represents yes. 0 represents abstaining, or the default value.
     *
     * Emits a {vote} event.
     */
    function approveAction(bytes32 actionHash, uint8 _vote)
        public
        override
        onlyKeeper
        returns (bool)
    {
        ...
        actionVotes[actionHash][_msgSender()] = _vote;
        emit vote(actionHash, msg.sender, _vote);
        return true;
    }
     ...
     */
    function actionApprovalStatus(bytes32 actionHash) public view returns (uint8) {
        ...
        for (uint256 i = 0; i != _keeperAmount; i++) {
            uint8 approval = actionVotes[actionHash][keepers[i]];
            if (approval == 1)   {
                noApprovals += 1;
            }
            else if (approval == 2)    {
        ...
        // If there happen to be no keepers, div by zero error will happen here. Not really necessary (all the important functions are onlyKeeper anyways) but nice insurance.
        if ((yesApprovals * 100) / keepers.length >= actionThresholdPercent)   {
            return 2;
        }
        else if 
        (((keepers.length - noApprovals) * 100) / keepers.length < actionThresholdPercent)   {//If enough keepers have voted no for yes to be impossible, the action is rejected
            return 1;
        } 
        else return (0);
    }
        ...
        returns (ActionState)
    {
        ...
        require (actions[newActionHash] == ActionState.PENDING, 'Action already executed');

        //Check action approval status, execute accordingly.
        uint8 actionApproval = actionApprovalStatus(newActionHash);
        if (actionApproval == 2) {
            ...
            (success, ) = address(this).call{
                gas: gasVault.gasAvailableForTransaction(targetAddress) //Withdraw required gas from strategy account
            }(
            ...
            gasVault.reimburseGas(targetAddress, originalGas);

            //Record result
            if (success) {
                emit ActionExecuted(newActionHash, _msgSender());
            ...
            actions[newActionHash] = ActionState.REJECTED;
            emit ActionRejected(newActionHash, _msgSender());
            return ActionState.REJECTED;
        } else   {
            //If actionApproval == 0, action is not approved yet
            revert("Votes lacking; state still pending");
        }
    }

    function _executeAction(address targetAddress, bytes calldata encodedCalldata) external {
        require(
    ...
    IVaultRegistry
{
    ...
    mapping(uint256 => mapping(uint256 => address)) internal _linkedVaults;

    // Mapping for strategy token ID to number of vaults created using that strategy. 
    mapping(uint256 => uint256) internal _linkedVaultsCounts;
    ...
    // Mapping beacon address => beaconName. Used to find what vault type a given beacon is (mostly used by nodes)
    ...
    function initialize(
        ...
        registerBeacon(_beaconName, _beacon, _ipfsConfigForBeacon);
    }
    ...
    /// @notice Registers any address as a new beacon. Useful for alternative beacon types.
    function registerBeacon(string calldata _name, address _address, string memory _ipfsConfigForBeacon)
        public
    ...
    ///@notice Updates the ipfs link storing the beaconConfig
    /// @param _name The name of the beacon (identifier)
    /// @param _newIPFSConfigForBeacon IPFS hash for the config of this beacon
    function updateBeaconConfig(string calldata _name,string calldata _newIPFSConfigForBeacon) external onlyRole(BEACON_CREATOR) {
        require(beaconAddresses[_name] != address(0),"Beacon doesnot exists");
    ...
    function deregisterBeacon(string calldata _name) external onlyRole(BEACON_CREATOR) {
    ...
    function createVault(
        ...
        emit VaultCreated(msg.sender, deployedAddress, _beaconName, _tokenId);
...
interface IBeaconInterface {
    ...
    ///@notice Event emitted when the address that the beacon is pointing to is upgraded.
...
interface IOrchestrator {
    ...
    function actionVotes(bytes32, address) external view returns (uint8);
     ...
     *
     */
    function actionApprovalStatus(bytes32 actionHash) external view returns (uint8);
...
interface IVaultRegistry {
    ...
    /// @notice Beacon registeration event
