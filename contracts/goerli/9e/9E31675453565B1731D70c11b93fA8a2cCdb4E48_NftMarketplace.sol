...
error NftMarketPlace__NftNotFromSellersNftFactory(address nftAddress, address seller);

contract NftMarketplace is ReentrancyGuard {
  SellersNftFactory private sellersNftFactory;
  ...
  modifier isOwner(
    ...
    //如果該NFT不是屬於sellersNftFactory底下的子合約所發出的,就revert
    //檢查的方法是比對該nft合約地址,是不是存在於sellersNftFactory的mapping中
    if (nftAddress != sellersNftFactory.getContractAddressFromUser(msg.sender)) {
      revert NftMarketPlace__NftNotFromSellersNftFactory(nftAddress, msg.sender);
    }
    _;
  }

  //傳遞一個address變數給constructor
  constructor(address sellersNftFactoryAddress) {
    //將全域變數設定為interface(智能合約地址)
    sellersNftFactory = SellersNftFactory(sellersNftFactoryAddress);
  }

  //前端 => 上傳圖檔(得到IPFS) 設定金額
  //從前端呼叫,輸入tokenUri並鑄造NFT,紀錄tokenId跟錢包地址的對應
  //修改成只能列出從sellersNFTFactory來的子合約,用mapping的方式檢查
  function listItem(
    address nftAddress,
    uint256 tokenId,
