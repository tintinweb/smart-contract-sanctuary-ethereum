...
import {IAnomuraBackPack, BackpackData} from "./interfaces/IAnomuraBackPack.sol";
import {IERC721} from "@openzeppelin/contracts/token/ERC721/IERC721.sol";
import { Object } from "@sharedstate/verbs/contracts/interfaces/IERC5050.sol";
import { IUniversalEquipmentManager } from "./UniversalEquipmentManager.sol";

/// @notice Anomura BackPack contract
contract AnomuraBackPack is
    IAnomuraBackPack,
    Initializable,
    ...
    ERC1155ReceiverUpgradeable
{
    ...
    IUniversalEquipmentManager public universalManager;
    bool public isPaused;

    bytes32 constant ERC721_TYPE = keccak256(abi.encodePacked("ERC721"));
    bytes32 constant ERC1155_TYPE = keccak256(abi.encodePacked("ERC1155"));
    bytes32 public constant MINTER_ROLE = keccak256("MINTER_ROLE");

    mapping(address => bool) private _collectionToApproval;

    /// @notice SIMPLE Mapping from anomuraId to its equipments array on collection
    mapping(uint256 => mapping(address => uint256[]))
        public _anomuraToContractEquipmentIds;

    ////////////////////////////////////////////////////////////////////////////////////////
    /// @notice maps anomura ids to number of equipments bound to them.
    mapping(uint256 => uint256) public equipmentCountOf;
    /// @notice maps anomura ids to map of their equipments.
    mapping(uint256 => mapping(uint256 => BackpackData)) public equipmentsOf;
    /// @notice maps anomura ids to map of equipment data indexes.
    mapping(uint256 => mapping(address => mapping(uint256 => uint256)))
        public equipmentIndexOf;
    ...
    function initialize() external initializer {
        ...
        isPaused = true; // to not use this on prod for now, except calling to the universal
    ...
    function universalBindOrUnbind(bytes4 actionName, Object calldata from, Object calldata to, bytes memory nftType) external {
        require(from._address == address(anomura), "Wrong anomura address");
        if (IERC721(from._address).ownerOf(from._tokenId) != _msgSender()) {
            revert ("Invalid Owner");
        }
        universalManager.universalBindOrUnbind(actionName, from, to, nftType);
    }

    function bind(
        address _equipmentCollection,
        uint256 _anomuraId,
        uint256[] memory _equipmentIds,
        string calldata typeOf
    ) external {
        if (!isPaused) {
            revert IsPaused();
        }
        if (anomura.ownerOf(_anomuraId) != _msgSender()) {
            revert InvalidOwner();
        }
        if (!_approvalCollections.contains(_equipmentCollection)) {
            revert InvalidEquipmentAddress();
        }

        bytes32 collectionType = keccak256(abi.encodePacked(typeOf));
    ...
    function unbind(
        address _equipmentCollection,
        uint256 _anomuraId,
        uint256[] memory _unequipmentIds,
        string calldata typeOf
    ) public {
        if (!isPaused) {
            revert IsPaused();
        }
        if (anomura.ownerOf(_anomuraId) != msg.sender) {
            revert InvalidOwner();
        }

        bytes32 _collectionType = keccak256(abi.encodePacked(typeOf));

        for (uint256 i = 0; i < _unequipmentIds.length; i++) {
            _unbind(
                _equipmentCollection,
                _anomuraId,
                _unequipmentIds[i],
    ...
    /**
    @notice Bind an `_equipmentId` from `_equipmentContract` into `_anomuraId`
    ...
    @param _equipmentCollection Address of the equipment contract
    @param _anomuraId ID of the anomura
    @param _equipmentId ID of the equipment to bound into `_anomuraId`
     */
    function _bind(
        address _equipmentCollection,
        uint256 _anomuraId,
        uint256 _equipmentId,
        bytes32 _collectionType
    ) internal {
        uint256 equipmentIndex = equipmentCountOf[_anomuraId];
        equipmentIndexOf[_anomuraId][_equipmentCollection][
            _equipmentId
        ] = equipmentIndex;

        equipmentsOf[_anomuraId][equipmentIndex] = BackpackData({
            equipment: _equipmentCollection,
            id: _equipmentId
        });

        equipmentCountOf[_anomuraId] = equipmentIndex + 1;

        if (_collectionType == ERC721_TYPE) {
            IERC721(_equipmentCollection).transferFrom(
                _msgSender(),
                address(this),
                _equipmentId
            ); //this check for owner of equipmentId already
        } else if (_collectionType == ERC1155_TYPE) {} else {
    ...
    */
    function _unbind(
        ...
        uint256 equipmentIndex = equipmentIndexOf[_anomuraId][
            _equipmentCollection
        ][_equipmentId];

        {
            if (equipmentsOf[_anomuraId][equipmentIndex].id != _equipmentId) {
                revert("Not bind to equipment");
            }
        }
        uint256 lastIndex = --equipmentCountOf[_anomuraId];
        BackpackData memory lastEquipment = equipmentsOf[_anomuraId][lastIndex];

        equipmentsOf[_anomuraId][equipmentIndex] = lastEquipment;
        equipmentIndexOf[_anomuraId][lastEquipment.equipment][
            lastEquipment.id
        ] = equipmentIndex;

        delete equipmentIndexOf[_anomuraId][_equipmentCollection][_equipmentId];
        delete equipmentsOf[_anomuraId][lastIndex];
    ...
    /// @param _anomuraId id of the anomura to get the equipment list.
    /// @return equipments array.
    function allEquipmentsOf(uint256 _anomuraId)
        external
        view
        returns (BackpackData[] memory equipments)
    {
        uint256 count = equipmentCountOf[_anomuraId];
        equipments = new BackpackData[](count);
        for (uint256 i = 0; i < count; i++)
            equipments[i] = equipmentsOf[_anomuraId][i];
    }

    /**
    @dev Sets the anomura contract address 
     */
    function setAnomuraAddress(address anomura_)
        external
...
interface IAnomuraBackPack is IAnomuraErrors {
    event EquipmentBind(address indexed collection, uint256 anomuraId, uint256[] equipmentIds, address sender);
    event EquipmentUnbind(address indexed collection, uint256 anomuraId, uint256[] equipmentIds, address sender);
    function bind(
        address collection,
        uint256 anomuraId,
    ...
    function unbind(address collection, uint256 anomuraId, uint256[] memory equipmentIds, string calldata typeOf) external;
}
    ...
    ERC5050State
{
    ...
    function universalBindOrUnbind(bytes4 actionName, Object calldata from, Object calldata to, bytes memory nftType) external onlyRole(EXTERNAL_BIND)
    {
        ...
        assembly {  // Get first 4 bytes
            collectionType := shr(224, nftType)
        }
