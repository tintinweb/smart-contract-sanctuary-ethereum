...
contract Meow is Ownable {
    ...
    function enterRoom(uint256 tokenId, uint256 roomnum) external payable {
        ...
        require(gameStatus[roomnum] == false, "Finished");
        require(NFT.getApproved(tokenId) == address(this), "Token cannot be transfered");
        big = true;
    ...
    function claimFight(uint256 tokenId, uint256 roomnum) external payable {
        ...
        betRoom.fighter2 = msg.sender;
        require(betRoom.tokenid1 > 0 && secondrandom > 0, "Not Prepared");
        require(NFT.ownerOf(betRoom.tokenid1) == address(this), "NFT not transfered");
        startGame(roomnum);
        emit GameStarted(betRoom.tokenid1, tokenId);
        gameStatus[roomnum] = true;
    }
    ...
    function startGame(uint256 roomnum) internal {
        // start game
        Room storage betRoom = room[roomnum];
        ...
        if (betRoom.random1 == 77777)
            jackpot(waitingAddress, oppositeAddress, nextNumber);
        if (nextNumber == 77777)
            jackpot(oppositeAddress, waitingAddress, betRoom.random1);
        if (betRoom.random1 == nextNumber) {
            sendPrice(waitingAddress, tmpgamePrice);
            sendPrice(oppositeAddress, tmpgamePrice);
            NFT.transferFrom(address(this), waitingAddress, betRoom.tokenid1);
            NFT.transferFrom(address(this), oppositeAddress, betRoom.tokenid2);
        } else {
            if (betRoom.random1 > nextNumber) {
                sendPrice(waitingAddress, tmpgamePrice.mul(12).div(10));
                NFT.transferFrom(address(this), waitingAddress, betRoom.tokenid2);
                NFT.transferFrom(address(this), waitingAddress, betRoom.tokenid1);
            } else {
                sendPrice(oppositeAddress, tmpgamePrice.mul(12).div(10));
                NFT.transferFrom(address(this), oppositeAddress, betRoom.tokenid2);
                NFT.transferFrom(address(this), oppositeAddress, betRoom.tokenid1);
            }
