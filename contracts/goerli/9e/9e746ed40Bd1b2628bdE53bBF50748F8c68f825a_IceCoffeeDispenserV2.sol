...
contract IceCoffeeDispenserV2 is Initializable {
    address payable public owner;
    uint8 public numCoffees;
    

    mapping(address => uint) public CoffeePurchases;

    event Purchase(address indexed buyer, uint time);

    function initialize(uint8 _numCoffees) public initializer{
        numCoffees = _numCoffees;
        owner = payable(msg.sender);
    }

    function buyCoffee() public payable {
        require(msg.value >= 0.005 ether, "A coffee costs 0.005 ether");
        numCoffees--;
        CoffeePurchases[msg.sender] += 1;
