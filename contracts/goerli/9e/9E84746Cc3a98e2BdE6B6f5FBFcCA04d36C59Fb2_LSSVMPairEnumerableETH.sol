    ...
    ERC1155Holder
{
    ...
    // Events
    event SwapNFTInPair(IERC721 nft, uint256[] nftIds, uint256 outputAmount, uint256 protocolFee);
    event SwapNFTOutPair(uint256 inputAmount, uint256 protocolFee, IERC721 nft, uint256[] nftIds);
    event SpotPriceUpdate(uint128 newSpotPrice);
    event TokenDeposit(uint256 amount);
    event TokenWithdrawal(uint256 amount);
    event NFTWithdrawal(IERC721 nft, uint256[] nftIds);
    ...
    function initialize(
        ...
        ILSSVMPairFactoryLike.PoolType _poolType = poolType();

        if ((_poolType == ILSSVMPairFactoryLike.PoolType.TOKEN) || (_poolType == ILSSVMPairFactoryLike.PoolType.NFT)) {
            require(_fee == 0, "Only Trade Pools can have nonzero fee");
            assetRecipient = _assetRecipient;
        } else if (_poolType == ILSSVMPairFactoryLike.PoolType.TRADE) {
    ...
    function swapTokenForAnyNFTs(
        ...
        // Input validation
        {
            ILSSVMPairFactoryLike.PoolType _poolType = poolType();
            require(
                _poolType == ILSSVMPairFactoryLike.PoolType.NFT || _poolType == ILSSVMPairFactoryLike.PoolType.TRADE,
                "Wrong Pool type"
            );
            require(
        ...
        uint256[] memory nftIds = _sendAnyNFTsToRecipient(_nft, nftRecipient, numNFTs);

        _refundTokenToSender(inputAmount);

        emit SwapNFTOutPair(inputAmount, protocolFee, _nft, nftIds);
    }
    ...
    function swapNFTsForToken(
        ...
        // Input validation
        {
            ILSSVMPairFactoryLike.PoolType _poolType = poolType();
            require(
                _poolType == ILSSVMPairFactoryLike.PoolType.TOKEN || _poolType == ILSSVMPairFactoryLike.PoolType.TRADE,
                "Wrong Pool type"
            );
            require(nftIds.length > 0, "Must ask for > 0 NFTs");
        }
        ...
        _takeNFTsFromSender(_nft, nftIds, _factory, isRouter, routerCaller);

        emit SwapNFTInPair(_nft, nftIds, outputAmount, protocolFee);
    }
     ...
     */
    function poolType() public pure returns (ILSSVMPairFactoryLike.PoolType _poolType) {
        uint256 paramsLength = _immutableParamsLength();
        assembly {
            _poolType := shr(
                0xf8,
        ...
        returns (address payable _assetRecipient)
    {
        ...
        if (poolType() == ILSSVMPairFactoryLike.PoolType.TRADE) {
    ...
    function changeFee(uint96 newFee) external onlyOwner {
        ILSSVMPairFactoryLike.PoolType _poolType = poolType();
        require(_poolType == ILSSVMPairFactoryLike.PoolType.TRADE, "Only for Trade pools");
    ...
    {
        ILSSVMPairFactoryLike.PoolType _poolType = poolType();
        require(_poolType != ILSSVMPairFactoryLike.PoolType.TRADE, "Not for Trade pools");
...
abstract contract LSSVMPairEnumerable is LSSVMPair {
    ...
    function _sendAnyNFTsToRecipient(
        ...
        address nftRecipient,
        uint256 numNFTs
    ) internal override  returns (uint256[] memory nftIds) {
