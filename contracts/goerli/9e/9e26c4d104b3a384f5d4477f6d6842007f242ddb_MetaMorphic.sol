...
contract MetaMorphic is ERC721EnumerableUpgradeable, OwnableUpgradeable {
     ...
     */
    function mintAux(uint256 amount) external payable onlyStage1 {
        if (mintedAmount1 + amount > maxAmount1) {
            revert MaxStageMint();
        }
        uint256 mintedAmount = mintedAmounts1[msg.sender];
        // add minted amount
        mintedAmount1 = mintedAmount1.add(amount);
        mintedAmounts1[msg.sender] = mintedAmount.add(amount);
        // mint NFT
