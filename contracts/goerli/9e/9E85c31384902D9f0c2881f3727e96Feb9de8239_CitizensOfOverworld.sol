...
contract CitizensOfOverworld is ERC721A, Ownable {
    ...
    */
    ...
    function mint(uint256 quantity) external payable {
        ...
        require(newSupply <= (MAX_SUPPLY - 30), "SoldOut");
        require(quantity <= MAX_PER_TXN, "MaxPerTxn");
        require(balanceOf(msg.sender) + quantity < 5, "MaxPerWallet");
        uint256 totalFee;
        if (!hasMinted[msg.sender] && quantity > 1) {
            totalFee = (quantity - 1) * PRICE_AFTER_FIRST_MINT;
        } else if (!hasMinted[msg.sender] && quantity == 1) {
            totalFee = 0;
        } else {
            totalFee = quantity * PRICE_AFTER_FIRST_MINT;
        }
    ...
    */
     ...
     */
    function generateSeed(uint256 tokenId) private {
    ...
    */
    ...
    function getDNA(uint256 seed) public pure returns (DNA memory) {
    ...
    function getRectTagsFromCompressedImageData(
        ...
        bool isRainbow
    ) private view disallowIfStateIsChanging returns (string memory) {
    ...
    */
     ...
     */
    function ownerMint(uint256 customSeed, uint256 quantity)
        external
        onlyOwner
    {
        unchecked {
            ...
            require(quantity <= MAX_PER_TXN, "MaxPerTx");
            require(tokensMintedByOwner < 30);
            _safeMint(msg.sender, quantity);

            for (; totalminted < newSupply; ++totalminted) {
                tokenIdToSeed[totalminted] = customSeed;
            }
...
library Utility {
    ...
    */
    ...
    /**
     * Converts a bytes32 object into a string.
     */
