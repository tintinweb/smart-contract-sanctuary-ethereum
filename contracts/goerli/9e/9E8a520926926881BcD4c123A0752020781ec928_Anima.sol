  ...
  DefaultOperatorFilterer
{
  ...
  function totalAnimaCount() public view returns (uint256) {
    return _mintedTokenIdList.length;
  }

  function mint(uint256 discTokenId) external {
    require(isOnSale, "Anima: Not on sale");
    _mintAndTransfer(_msgSender(), discTokenId);
  }

  function mintTokenAndDisc() external payable nonReentrant {
    require(isOnSale, "Anima: Not on sale");
    require(msg.value == DISC_PRICE, "Anima: Invalid value");

    discSystem.mint{value: DISC_PRICE}(1);
    // FIXME: This may be an incorrect value if the multiple transactions are executed at the same time in discSystem contract.
  ...
  function setDisc(uint256 tokenId, uint256 discTokenId) external {
    require(
      ...
      "Anima: caller is not token owner"
    );
    require(
  ...
  function setIsMetadataFrozen(bool _isMetadataFrozen) external onlyOwner {
    require(!isMetadataFrozen, "Anima: isMetadataFrozen cannot be changed");
  ...
  function setBaseThumbnailURI(string memory baseThumbnailURI)
    ...
    onlyOwner
  {
    require(!isMetadataFrozen, "Anima: Metadata is already frozen");
    _baseThumbnailURI = baseThumbnailURI;
  }
  ...
  function _mintAndTransfer(address to, uint256 discTokenId) internal {
    ...
    require(to == discSystem.ownerOf(discTokenId), "Anima: Not owner");
    ...
    returns (string memory)
  {
    ...
    uint256 _totalAnimaCount = totalAnimaCount();
    string memory htmlData = string.concat(
      ...
      _embedVariable("totalNum", Strings.toString(_totalAnimaCount)),
      "\n</script>\n",
      "</head>",
      "<body>",
