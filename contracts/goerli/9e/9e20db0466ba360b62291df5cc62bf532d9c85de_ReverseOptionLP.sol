...
contract ReverseOptionLP is BaseLP {
    using SafeERC20 for IERC20;

    struct OptionTokenLpPosition {
        uint256 lpId;
        uint256 epoch;
        uint256 strike;
        uint256 numTokensProvided;
    ...
    // mapping (epoch strike token address) => OptionTokenLpPosition[])
    mapping(address => OptionTokenLpPosition[]) internal allLpPositions;
    ...
    /**
     * Provides LP position for a token
     * @param token Token address
     * @param isPut Is the LP for puts
     * @param strike Strikes to sell at
     * @param numTokensProvided Num of tokens to sell
     * @param markup Markup on top of the option's premium in %
     * @param seller Address to send usd to if purchase succeeds
     * @return Whether new LP position was created
     */
    function provideOptionTokenLp(
        address token,
        bool isPut,
        uint256 strike,
        uint256 numTokensProvided,
    ...
    /**
     * Provides multiple LP position for a token
     * @param token Token address
     * @param isPut Is the LP for puts
     ...
     * @return Whether new LP position was created
     */
    function multiProvideOptionTokenLps(
        address token,
        bool isPut,
        ...
        if (
            ...
            strikes.length != amounts.length ||
            amounts.length != markups.length
        ) {
    ...
    function _provideOptionTokenLp(
        address ssov,
        ...
        address optionToken = getSsovOptionToken(ssov, currentEpoch, strike);
        if (optionToken == address(0)) {
            revert InvalidStrike();
        }

        if (getOptionTokenInfo[optionToken].ssov == address(0)) {
            getOptionTokenInfo[optionToken].ssov = address(ssov);
            getOptionTokenInfo[optionToken].strike = strike;
        }
        getOptionTokenInfo[optionToken].tokenLiquidity += numTokensProvided;

        uint256 lpId = allLpPositions[optionToken].length;
        OptionTokenLpPosition memory lpPos = OptionTokenLpPosition({
            lpId: lpId,
            epoch: currentEpoch,
            strike: strike,
            numTokensProvided: numTokensProvided,
            numTokensSold: 0,
            markup: markup,
            usdReceived: 0,
            seller: seller,
        ...
        allLpPositions[optionToken].push(lpPos);
        userLpPositions[seller][optionToken].push(lpId);

        IERC20(optionToken).safeTransferFrom(
            msg.sender,
            address(this),
            numTokensProvided
        );

        emit LiquidityForStrikeAdded(optionToken, lpId);
        return true;
    }

    /**
     * Fills an LP position with num token
     * @param optionToken epoch strike token address
    ...
    function fillLpPosition(
        ...
        address ssov = getOptionTokenInfo[optionToken].ssov;
        if (ssov == address(0)) {
            revert SsovDoesNotExist();
        }
        if (
            ...
            revert LpPositionFailedToFill();
        }
        return true;
    }

    /**
     * Fills multiple LP positions
     * @param optionToken epoch strike token address
     * @param lpIndices Indices of LP position
     * @param amounts Amounts of options to buy from each LP position
     * @return Whether LP positions were filled
     */
    function multifillLpPositions(
        address optionToken,
        ...
        if (lpIndices.length == 0 || lpIndices.length != amounts.length) {
            revert InvalidParams();
        }
        uint256 epoch = getSsovEpoch(ssov);
    ...
    function _fillLpPosition(
        ...
        if (numTokens <= TOKEN_DUST_THRESHOLD) {
            revert AmountTooSmall();
        }
        if (lpIndex >= allLpPositions[optionToken].length) {
            revert InvalidLpIndex();
        }

        OptionTokenLpPosition memory lpPosition = allLpPositions[optionToken][
            lpIndex
        ];
        ...
        if (lpPosition.numTokensProvided < numTokens) {
        ...
        uint256 premium = getSsovUsdPremiumCalculation(
            ssov,
            getOptionTokenInfo[optionToken].strike,
            numTokens
        );

        // Add markup
        premium = (premium * (PERCENT + lpPosition.markup)) / PERCENT;

        // Update LP position
        allLpPositions[optionToken][lpIndex].numTokensProvided -= numTokens;
        allLpPositions[optionToken][lpIndex].numTokensSold += numTokens;
        allLpPositions[optionToken][lpIndex].usdReceived += premium;
        getOptionTokenInfo[optionToken].tokenLiquidity -= numTokens;

        // if liquidity is lower than threshold, kill the position
        if (
            allLpPositions[optionToken][lpIndex].numTokensProvided <
            TOKEN_DUST_THRESHOLD
        ) {
            if (!_clearLpDust(optionToken, lpIndex)) {
        ...
        IERC20(optionToken).safeTransfer(msg.sender, numTokens);
     ...
     */
    function killLpPosition(address optionToken, uint256 lpIndex)
        external
        nonReentrant
        returns (bool)
    {
        ...
        if (!_killLpPosition(optionToken, lpIndex)) {
    ...
    function multikillLpPositions(
        ...
        for (uint256 i; i < lpIndices.length; ) {
            _killLpPosition(optionToken, lpIndices[i]);
            unchecked {
                ++i;
            }
        }
    ...
    function _killLpPosition(address optionToken, uint256 lpIndex)
        internal
        returns (bool)
    {
        if (lpIndex >= allLpPositions[optionToken].length) {
        ...
        if (
            !_killAndTransfer(
                ...
                lpIndex,
                lpPosition.seller,
                lpPosition.numTokensProvided
            )
        ) {
            revert PositionFailedToKillAndTransfer();
        }
        emit LPPositionKilled(optionToken, lpIndex);
        return true;
    }

    /// @dev helper function to clear LP dust position at index
    function _clearLpDust(address optionToken, uint256 lpIndex)
        internal
        returns (bool)
    {
        OptionTokenLpPosition memory lpPosition = allLpPositions[optionToken][
        ...
        emit LPDustCleared(optionToken, lpIndex);
        return true;
    }
    ...
    function _killAndTransfer(
        ...
        allLpPositions[optionToken][lpIndex].killed = true;
        getOptionTokenInfo[optionToken].tokenLiquidity -= liquidity;
        ...
        IERC20(optionToken).safeTransfer(buyer, liquidity);
        return true;
    }
     ...
     */
    function getUserLpPositions(address user, address optionToken)
        external
        view
        returns (OptionTokenLpPosition[] memory positions)
    {
        uint256[] memory userPositionsId = userLpPositions[user][optionToken];
        uint256 numPositions = userPositionsId.length;
        positions = new OptionTokenLpPosition[](numPositions);
        for (uint256 i; i < numPositions; ) {
            positions[i] = allLpPositions[optionToken][userPositionsId[i]];
            unchecked {
                ++i;
            }
        }
    ...
    /**
     * Returns all LP positions for a given optionToken
     * @param optionToken epoch strike token address
     * @return all LP positions
     */
    function getAllLpPositions(address optionToken)
        external
        view
        returns (OptionTokenLpPosition[] memory)
    {
        return allLpPositions[optionToken];
    }
}
