...
contract Marketplace is Ownable {
	...
	*/    
    ...
    event ProviderCreated(bytes32 id, string name, string description, string picture, address addr, uint commission, string banner, bool isEnabled, uint dateCreated, uint dateModified);
    event ProviderEdited(bytes32 id, string name, string description, string picture, address addr, uint commission, string banner, bool isEnabled, uint dateCreated, uint dateModified);
    event ArtCreated(bytes32 id, bytes32 providerID, string name, string description, string uri, string metadataUri, uint256 price, uint256 totalSupply, uint dateCreated);
    event ArtMinted(uint256 tokenId, bytes32 id, bytes32 providerID, string name, string description, string uri, string metadataUri, uint256 price, uint256 totalSupply, uint dateCreated, address owner);
	...
	*/
    function addProvider(string memory name, string memory description, string memory logo, address addr, uint commission, string memory banner) public onlyOwner {
        ...
        Provider memory p = Provider(id, name, description, logo, payable(addr), commission, banner, true, block.timestamp, block.timestamp);

        providers.push(id);
        providerByOwner[addr] = id;
        providerByID[id] = p;

        emit ProviderCreated(p.id, p.name, p.description, p.logo, p.addr, p.commission, p.banner, p.isEnabled, block.timestamp, block.timestamp);
    }
	...
	*/
    function setIsProviderEnabled(bytes32 id, bool value) public onlyOwner {
        ...
        emit ProviderEdited(p.id, p.name, p.description, p.logo, p.addr, p.commission, p.banner, p.isEnabled, p.dateCreated, p.dateModified);
    }
	...
	*/
    function editProvider(bytes32 id, string memory name, string memory description, string memory logo, address addr, uint commission, string memory banner) public { 
        ...
        p.addr = payable(addr);
        p.banner = banner;
        p.dateModified = block.timestamp;
    ...
    */
    ...
    function addArtwork(string memory name, string memory description, string memory uri, string memory metadataUri, uint256 price, uint256 totalSupply) public {
        require(bytes(name).length > 0, "The name can't be empty");
        require(bytes(description).length > 0, "The description can't be empty");
        ...
        bytes32 id = keccak256(abi.encodePacked(msg.sender, name, block.timestamp));
        Art memory art = Art(id, providerID, name, description, uri, metadataUri, price, totalSupply, 0, block.timestamp);

        artsByProvider[providerID].push(id);
        artByID[id] = art;

        emit ArtCreated(art.id, art.providerID, art.name, art.description, art.uri, art.metadataUri, art.price, art.totalSupply, block.timestamp);
    }
    ...
    function buyArtwork(bytes32 aID) public payable {
        ...
        emit ArtMinted(tokenId, a.id, a.providerID, a.name, a.description, a.uri, a.metadataUri, a.totalSupply, a.price, a.dateCreated, msg.sender);
    }
}
...
contract NFTToken is ERC721URIStorage, Ownable {
    ...
    constructor() Ownable() ERC721("ScaleUP-NFT-Token", "SUT")  {
    }
