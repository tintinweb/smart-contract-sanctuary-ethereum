...
/*
      The upgraded and amazing Venture Pass
 Contract by the [sometimes] controversial Aleph0ne
       (created with some help from Stefs)

       Stefs Test 5 / onChainTraits
...
contract VenturePass is ERC721A, Ownable, OperatorFilterer {
    bool public operatorFilteringEnabled;
    address public ApeLiquid = 0xa2ec462e0Fa83b5A33e48399E0788c2640edf0cD;
    address public TheForge = 0xd7ff9F4593C15E6778038D3bDF565413d1C7D774;
    address public metl = 0xE94201fBC6a6BCFD71a8A8A60781889DF8cB07b5;
    ...
    // The completely new and improved rewards system.
    // Caveman math approved. If it were more simple, you'd need
    // a botched pre-frontal lobe lobotomy to fail to understand it.
    //
    // metlRate * days held, it's that fckn simple
    // Example:  Held for 10 days
    // 0.25 * 10 = 2.5 METL
    uint256 public metlRate = 250000000000000000;
    uint256 public daysRequired = 30;

    // Mapping to store locked tokens
    mapping(uint256 => bool) private _lockedTokens;
    mapping(uint256 => uint256) private _lockTimestamps;
    mapping(uint256 => uint256) private _extraMetl; // extra METL awarded to owner
    ...
    // NOTE: Mints 1:1 the existing Membership to the new
    //       Venture Pass token id!
    mapping(uint => bool) public vpassClaimed;

    constructor() ERC721A("Venture Pass", "VPASS") {
    ...
    function _buildTokenURI(uint256 tokenId) internal view returns (string memory) {
        string memory json = Base64.encode(
            bytes(string(
                abi.encodePacked(
                    '{"name": "pass # ', uint2str(tokenId), '",',
                    '"image": "', attributes[tokenId].image, '",',
                    '"description": "Venture Pass. Your guide to Mythic Journeys, The Arena, METL rewards, and much more. Dynamically on chain!",',
                    '"attributes": [{"trait_type": "Arena Wins", "value": "', uint2str(attributes[tokenId].arena), '"},',
                    '{"trait_type": "Liquid METL", "value": "', uint2str(calculateRewards(tokenId)), '"},',
                    '{"trait_type": "Artifact", "value": "', attributes[tokenId].artifact, '"},',
                    '{"trait_type": "Journey Pass", "value": "', attributes[tokenId].jpass, '"},',
                    '{"trait_type": "Metl Rewards", "value": "', attributes[tokenId].rewards, '"}',
                    ']}'
                )
            ))
        );
    ...
    function mintApe(uint256 tokenId) internal {
        ...
        attributes[tokenId].artifact = "none";
        attributes[tokenId].rewards = "0.25 Per Day";
        attributes[tokenId].jpass = "none";
    ...
    function lockToken(uint256 tokenId) external {
        ...
        // set image to "locked"
        attributes[tokenId].image = "https://ipfs.io/ipfs/QmQ6xLY94h5kdm5xy759Gvz9FVSFaqbvgdR3KtzPTuMgor";
    ...
    function unlockToken(uint256 tokenId) external {
        ...
        attributes[tokenId].image = "https://ipfs.io/ipfs/QmZ3SJB2oV8DxDGcAaiSGaGj7apjiawfzXj4pxFsAgJxdF";
        _buildTokenURI(tokenId);
        // Make sure the caller is the owner of the NFT
        require(ownerOf(tokenId) == msg.sender, "Not the owner of the NFT");
    ...
    // NOTE: Calculate the METL earned by a given VenturePass
    //       Including extra metl earned
    function calculateRewards(uint256 tokenId) public view returns (uint256) {
        if (!_lockedTokens[tokenId]) {
            return 0;
        }

        uint256 extraMetlRewards = _extraMetl[tokenId];
        uint256 ratedMetlRewards = metlRate * this.getDaysLocked(tokenId);

        return extraMetlRewards + ratedMetlRewards;
    }

    // NOTE: This is a mapping so we can grab all the information about all
    //       the rewards in a single call instead of one-by-one.
    //
    //       It's heavier gas creating the contract, but much lighter on the
    //       backend (and one call instead of 1,650) to get all the rewards.
    struct TokenReward {
        uint256 tokenId;
        uint256 reward;
    }
    ...
    {
        TokenReward[] memory rewards = new TokenReward[](maxSupply);
        uint256 count = 0;

        for (uint256 i = 0; i < maxSupply; i++) {
            uint256 rw = calculateRewards(i);
            if (rw > 0) {
                rewards[count] = TokenReward({tokenId: i, reward: rw});
                count++;
            }
        }

        // Resize the rewards array to remove unused slots.
        assembly {
    ...
    // NOTE: Requires you OWN the token
    //       Checks the rate, if above x, then returns METL
    //       and resets the claim/lock information so the
    //       counter restarts
    function claimRewards(uint256 tokenId) external {
        require(_lockedTokens[tokenId], "You must have a locked token");
        require(msg.sender == ownerOf(tokenId), "Must own the NFT to claim");
        require(
            this.getDaysLocked(tokenId) > daysRequired,
            "Not enough days to claim"
        );
        uint256 mEarned = calculateRewards(tokenId);
        IERC20(metl).transfer(msg.sender, mEarned);
        // you have to re-lock to go again [Council decided against this]
        //_lockedTokens[tokenId] = false;
        _lockTimestamps[tokenId] = block.timestamp;
    }
    ...
    function addExtraMetl(
        ...
        uint256[] memory extraMetls
    ) external onlyOwner {
        require(tokenIds.length == extraMetls.length, "Arrays length mismatch");

        for (uint256 i = 0; i < extraMetls.length; i++) {
            _extraMetl[tokenIds[i]] += extraMetls[i];
        }
    }

    // NOTE: This is expressed in wei (10^18) as an integer!
    ...
    function mint(address to, uint256 theToken) external onlyOwner {
        ...
        apeClaimed[theToken] = 1;
        attributes[theToken].artifact = "none";
        attributes[theToken].rewards = "0.25 Per Day";
        attributes[theToken].jpass = "none";
        attributes[theToken].image = "https://ipfs.io/ipfs/QmZ3SJB2oV8DxDGcAaiSGaGj7apjiawfzXj4pxFsAgJxdF";
    }
