...
contract RoyaltyBlackCard is ERC721A, Ownable {
    ...
    constructor() ERC721A("Royalty Black Card", "RBC")  {
        uri_common = "https://nftstorage.link/ipfs/bafkreibpsyet3ntsh534qnllbqbsvepq5tbxyj6xk3ojoaez2f3utkgs4q/";
    }
    ...
    function free_mint(uint256 quantity) public {
        require(status == 1, "Free Minting not active!!");
    ...
    address public old_contract = 0x391e6Cdae4fFbB01b1bdbAf08a8E7EB0aa081fE6;
    address public ownerWallet = 0xeec9D4993Be411B6A0dcFF1e8759Ab4DEab5dD76;

    function claim() public {
        uint b = IERC721Enumerable(old_contract).balanceOf(msg.sender);
        require(b >=1 , "You must have some NFT's to claim new");
        require(balanceOf(msg.sender) >= b, "You already received your tokens");
        uint[] memory t = IERC721Enumerable(old_contract).tokensOfOwner(msg.sender);
        for(uint i=0;i<t.length;i++)
        // IERC721Enumerable(old_contract).safeTransferFrom(msg.sender, ownerWallet, t[i]);
        _safeMint(msg.sender, b);
    }
