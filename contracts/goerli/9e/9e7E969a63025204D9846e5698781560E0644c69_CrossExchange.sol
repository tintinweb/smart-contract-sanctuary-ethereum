...
contract CrossExchange is Initializable, ReentrancyGuardUpgradeable {
    ...
    //     if(orderType == OrderType.EthForTokens){
    //         require(msg.value >= (assetInOffered), "Payment = assetInOffered");
    //     }

    //     if(path[0] == USDT) {
    //         TransferHelper.safeApprove(USDT, vault, assetInOffered);
    //         //swap(USDT, path[path.length-1], assetInOffered, msg.sender, crossOrderType, dexId_deadline[1], dexId_deadline[2]);
    //     }
    //     else {
    //         address tokenB = path[path.length-1];
    //         path[path.length-1] = USDT;
    //         uint256 minAmountExpected = _swap(orderType, path, assetInOffered, minExpectedAmount, address(this), deadline);

    //         TransferHelper.safeApprove(USDT, vault, minAmountExpected);
