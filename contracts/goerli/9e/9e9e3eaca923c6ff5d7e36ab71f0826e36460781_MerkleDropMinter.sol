...
interface IMetadataModule {
    /**
     * @dev When implemented, RarementVolume's `tokenURI` redirects execution to this `tokenURI`.
...
import { IRarementFeeRegistry } from "./IRarementFeeRegistry.sol";

/**
 * @title IMinterModule
 * @notice The interface for Rarement protocol minter modules.
 */
interface IMinterModule is IERC165 {
    ...
    /**
     * @dev Emitted when the mint instance for an `volume` is created.
     * @param volume The volume address.
    ...
    /**
     * @dev Emitted when the `paused` status of `volume` is updated.
     * @param volume The volume address.
     * @param mintId  The mint ID, to distinguish between multiple mints for the samevolume.
     * @param paused  The new paused status.
     */
    event MintPausedSet(address indexed volume, uint128 mintId, bool paused);
     ...
     * @param endTime   The end time of the mint.
     */
    event TimeRangeSet(address indexed volume, uint128 indexed mintId, uint32 startTime, uint32 endTime);
     ...
     */
    event AffiliateFeeSet(address indexed volume, uint128 indexed mintId, uint16 bps);
    ...
    /**
     * @dev Sets the paused status for (`volume`, `mintId`).
     *
     * Calling conditions:
     * - The caller must be the volume's owner or admin.
     */
    function setVolumeMintPaused(
        address volume,
        uint128 mintId,
        bool paused
    ) external;
    ...
    /**
     * @dev Sets the time range for an volume mint.
     *
     * Calling conditions:
     * - The caller must be the volume's owner or admin.
     *
    ...
    /**
     * @dev Sets the affiliate fee for (`volume`, `mintId`).
     *
     * Calling conditions:
     * - The caller must be the volume's owner or admin.
     */
    ...
    /**
     * @dev Whether `affiliate` is affiliated for (`volume`, `mintId`).
     * @param volume   The volume's address.
    ...
    /**
     * @dev The total price for `quantity` tokens for (`volume`, `mintId`).
     * @param volume   The volume's address.
     * @param mintId    The mint ID.
     ...
     *      and is shared amongst all volumes connected to the minter contract.
     * @return The latest value.
     */
     ...
     */
    function feeRegistry() external view returns (IRarementFeeRegistry);
}
...
/**
 * @title IRarementFeeRegistry
 * @author rmnt.xyz
 */
interface IRarementFeeRegistry {
    // =============================================================
    //                            EVENTS
    // =============================================================

    /**
     * @dev Emitted when the `rarementFeeAddress` is changed.
     */
    event RarementFeeAddressSet(address rarementFeeAddress);
    ...
    /**
     * @dev The new `rarementFeeAddress` must not be address(0).
     */
    error InvalidRarementFeeAddress();
    ...
    /**
     * @dev Sets the `rarementFeeAddress`.
     *
     * Calling conditions:
     * - The caller must be the owner of the contract.
     *
     * @param rarementFeeAddress_ The rarement fee address.
     */
    function setRarementFeeAddress(address rarementFeeAddress_) external;
    ...
    /**
     * @dev The rarement contract's address that receives platform fees.
     * @return The configured value.
     */
    function rarementFeeAddress() external view returns (address);
...
/**
 * @dev The information pertaining to thisvolume.
 */
struct VolumeInfo {
    ...
    uint32 volumeMaxMintable;
    // The lower limit of the maximum number of tokens that can be minted.
    uint32 volumeMaxMintableUpper;
    // The upper limit of the maximum number of tokens that can be minted.
    uint32 volumeMaxMintableLower;
    // The timestamp (in seconds since unix epoch) after which the
    // max amount of tokens mintable will drop from
    // `maxMintableUpper` to `maxMintableLower`.
    uint32 volumeCutoffTime;
...
/**
 * @title IRarementVolumeV1
 * @notice The interface for Rarement volume contracts.
 */
interface IRarementVolumeV1 is IERC721AUpgradeable, IERC2981Upgradeable {
    // =============================================================
    //                            EVENTS
    // =============================================================
     ...
     * @param baseURI the base URI of thevolume.
     */
    event BaseURISet(string baseURI);

    /**
     * @dev Emitted when the `contractURI` is set.
     * @param contractURI The contract URI of thevolume.
     */
    ...
    /**
     * @dev Emitted when the volume's maximum mintable token quantity range is set.
     * @param volumeMaxMintableLower_ The lower limit of the maximum number of tokens that can be minted.
     * @param volumeMaxMintableUpper_ The upper limit of the maximum number of tokens that can be minted.
     */
    event VolumeMaxMintableRangeSet(uint32 volumeMaxMintableLower_, uint32 volumeMaxMintableUpper_);

    /**
     * @dev Emitted when the volume's cutoff time set.
     * @param volumeCutoffTime_ The timestamp.
     */
    event VolumeCutoffTimeSet(uint32 volumeCutoffTime_);
     ...
     * @param volume_                 The address of thevolume.
     ...
     * @param volumeMaxMintableLower_ The lower bound of the max mintable quantity for thevolume.
     * @param volumeMaxMintableUpper_ The upper bound of the max mintable quantity for thevolume.
     * @param volumeCutoffTime_       The timestamp after which `volumeMaxMintable` drops from
     *                                 `volumeMaxMintableUpper` to
     *                                 `max(_totalMinted(), volumeMaxMintableLower)`.
     ...
     */
    event RarementVolumeInitialized(
        address indexed volume_,
        string name_,
        string symbol_,
        ...
        uint16 royaltyBPS_,
        uint32 volumeMaxMintableLower_,
        uint32 volumeMaxMintableUpper_,
        uint32 volumeCutoffTime_,
        uint8 flags_
    );
    ...
    /**
     * @dev The volume's metadata is frozen (e.g.: `baseURI` can no longer be changed).
     */
    ...
    /**
     * @dev The requested quantity exceeds the volume's remaining mintable token quantity.
     * @param available The number of tokens remaining available for mint.
     */
    error ExceedsVolumeAvailableSupply(uint32 available);
    ...
    /**
     * @dev The `volumeMaxMintableLower` must not be greater than `volumeMaxMintableUpper`.
     */
    error InvalidVolumeMaxMintableRange();

    /**
     * @dev The `volumeMaxMintable` has already been reached.
     */
    ...
    /**
     * @dev Sets the volume max mintable range.
     *
     * Calling conditions:
     * - The caller must be the owner of the contract, or have the `ADMIN_ROLE`.
     *
     ...
     * @param volumeMaxMintableUpper_ The upper limit of the maximum number of tokens that can be minted.
     */
    function setVolumeMaxMintableRange(uint32 volumeMaxMintableLower_, uint32 volumeMaxMintableUpper_) external;

    /**
     * @dev Sets the timestamp after which, the `volumeMaxMintable` drops
     *      from `volumeMaxMintableUpper` to `volumeMaxMintableLower.
     *
     * Calling conditions:
     * - The caller must be the owner of the contract, or have the `ADMIN_ROLE`.
     *
     * @param volumeCutoffTime_ The timestamp.
     */
    function setVolumeCutoffTime(uint32 volumeCutoffTime_) external;
    ...
    /**
     * @dev Returns the volume info.
     * @return volumeInfo The latest value.
     */
    function volumeInfo() external view returns (VolumeInfo memory volumeInfo);
    ...
    /**
     * @dev Returns the maximum amount of tokens mintable for thisvolume.
     * @return The configured value.
     */
    function volumeMaxMintable() external view returns (uint32);

    /**
     * @dev Returns the upper bound for the maximum tokens that can be minted for thisvolume.
     * @return The configured value.
     */
    function volumeMaxMintableUpper() external view returns (uint32);

    /**
     * @dev Returns the lower bound for the maximum tokens that can be minted for thisvolume.
     * @return The configured value.
     */
    function volumeMaxMintableLower() external view returns (uint32);

    /**
     * @dev Returns the timestamp after which `volumeMaxMintable` drops from
     *      `volumeMaxMintableUpper` to `volumeMaxMintableLower`.
     * @return The configured value.
     */
    function volumeCutoffTime() external view returns (uint32);
     ...
     *      Used for game mechanics like the Rarement Golden Egg.
...
import { IRarementVolumeV1 } from "@core/interfaces/IRarementVolumeV1.sol";
import { IMinterModule } from "@core/interfaces/IMinterModule.sol";
import { IRarementFeeRegistry } from "@core/interfaces/IRarementFeeRegistry.sol";
import { IERC165 } from "openzeppelin/utils/introspection/IERC165.sol";
...
/**
 * @title Minter Base
 * @dev The `BaseMinter` class maintains a central storage record of volume mint instances.
 */
abstract contract BaseMinter is IMinterModule {
    ...
    /**
     * @dev The next mint ID. Shared amongst all volumes connected.
     */
    ...
    /**
     * @dev Maps an volume and the mint ID to a mint instance.
     */
     ...
     * @dev The fee registry. Used for handling platform fees.
     */
    IRarementFeeRegistry public immutable feeRegistry;

    // =============================================================
    //                          CONSTRUCTOR
    // =============================================================

    constructor(IRarementFeeRegistry feeRegistry_) {
    ...
    function setVolumeMintPaused(
        ...
        uint128 mintId,
        bool paused
    ) public virtual onlyVolumeOwnerOrAdmin(volume) {
        _baseData[volume][mintId].mintPaused = paused;
        emit MintPausedSet(volume, mintId, paused);
    }
    ...
    function setTimeRange(
        ...
        uint32 startTime,
        uint32 endTime
    ) public virtual onlyVolumeOwnerOrAdmin(volume) onlyValidTimeRange(startTime, endTime) {
        _baseData[volume][mintId].startTime = startTime;
        _baseData[volume][mintId].endTime = endTime;

        emit TimeRangeSet(volume, mintId, startTime, endTime);
    }
    ...
    function setAffiliateFee(
        ...
        uint128 mintId,
        uint16 feeBPS
    ) public virtual override onlyVolumeOwnerOrAdmin(volume) onlyValidAffiliateFeeBPS(feeBPS) {
        _baseData[volume][mintId].affiliateFeeBPS = feeBPS;
        emit AffiliateFeeSet(volume, mintId, feeBPS);
    }
    ...
    function withdrawForPlatform() public override {
        ...
        if (accrued != 0) {
            ...
            SafeTransferLib.safeTransferETH(feeRegistry.rarementFeeAddress(), accrued);
        }
    }
    ...
    /**
     * @dev Restricts the function to be only callable by the owner or admin of `volume`.
     * @param volume The volume address.
     */
    modifier onlyVolumeOwnerOrAdmin(address volume) virtual {
        if (
            msg.sender != OwnableRoles(volume).owner() &&
            !OwnableRoles(volume).hasAnyRole(msg.sender, IRarementVolumeV1(volume).ADMIN_ROLE())
        ) revert Unauthorized();
    ...
    /**
     * @dev Creates an volume mint instance.
     * @param volume The volume address.
     * @param startTime The start time of the mint.
     * @param endTime The end time of the mint.
     * @param affiliateFeeBPS The affiliate fee in basis points.
     ...
     * Calling conditions:
     * - Must be owner or admin of thevolume.
     */
    function _createVolumeMint(
        address volume,
        uint32 startTime,
        uint32 endTime,
        uint16 affiliateFeeBPS
        ...
        returns (uint128 mintId)
    {
        ...
        BaseData storage data = _baseData[volume][mintId];
        ...
        emit MintConfigCreated(volume, msg.sender, mintId, startTime, endTime, affiliateFeeBPS);
    }

    /**
     * @dev Mints `quantity` of `volume` to `to` with a required payment of `requiredEtherValue`.
    ...
    function _mint(
        ...
        BaseData storage baseData = _baseData[volume][mintId];
        ...
        uint128 requiredEtherValue = totalPrice(volume, mintId, msg.sender, quantity);
        ...
        bool affiliated = isAffiliated(volume, mintId, affiliate);
        uint128 affiliateFee;
        unchecked {
            if (affiliated) {
        ...
        emit Minted(
            ...
            uint32(IRarementVolumeV1(volume).mint{ value: remainingPayment }(msg.sender, quantity)),
            quantity,
            requiredEtherValue,
            platformFee,
            affiliateFee,
...
import { IMerkleDropMinter, VolumeMintData, MintInfo } from "./interfaces/IMerkleDropMinter.sol";
import { IMinterModule } from "@core/interfaces/IMinterModule.sol";
import { IRarementVolumeV1 } from "@core/interfaces/IRarementVolumeV1.sol";

/**
 * @title MerkleDropMinter
 * @dev Module for minting Rarement volumes using a merkle tree of approved accounts.
 */
contract MerkleDropMinter is IMerkleDropMinter, BaseMinter {
    ...
    /**
     * @dev Volume mint data.
     *      Maps `volume` => `mintId` => value.
     */
    mapping(address => mapping(uint128 => VolumeMintData)) internal _volumeMintData;

    // =============================================================
    //                          CONSTRUCTOR
    // =============================================================

    constructor(IRarementFeeRegistry feeRegistry_) BaseMinter(feeRegistry_) {}
    ...
    function createVolumeMint(
        ...
        mintId = _createVolumeMint(volume, startTime, endTime, affiliateFeeBPS);

        VolumeMintData storage data = _volumeMintData[volume][mintId];
    ...
    function mint(
        ...
        unchecked {
            ...
            uint256 numberMinted = IRarementVolumeV1(volume).numberMinted(msg.sender);
            // Won't overflow. The total number of tokens minted in `volume` won't exceed `type(uint32).max`,
            // and `quantity` has 32 bits.
            if (numberMinted + requestedQuantity > data.maxMintablePerAccount) revert ExceedsMaxPerAccount();
        }

        _mint(volume, mintId, requestedQuantity, affiliate);
    ...
    function setPrice(
        ...
        uint128 mintId,
        uint96 price
    ) public onlyVolumeOwnerOrAdmin(volume) {
        _volumeMintData[volume][mintId].price = price;
        emit PriceSet(volume, mintId, price);
    }
    ...
    function setMaxMintablePerAccount(
        ...
        if (maxMintablePerAccount == 0) revert MaxMintablePerAccountIsZero();
        _volumeMintData[volume][mintId].maxMintablePerAccount = maxMintablePerAccount;
        emit MaxMintablePerAccountSet(volume, mintId, maxMintablePerAccount);
    }
    ...
    function setMaxMintable(
        ...
        _volumeMintData[volume][mintId].maxMintable = maxMintable;
        emit MaxMintableSet(volume, mintId, maxMintable);
    }
    ...
    function setMerkleRootHash(
        ...
        _volumeMintData[volume][mintId].merkleRootHash = merkleRootHash;
        emit MerkleRootHashSet(volume, mintId, merkleRootHash);
    }
    ...
    function totalPrice(
        ...
        unchecked {
            ...
            return uint128(uint256(_volumeMintData[volume][mintId].price) * uint256(quantity));
        }
    }
     ...
     * @inheritdoc IMerkleDropMinter
     */
    function mintInfo(address volume, uint128 mintId) external view returns (MintInfo memory) {
        BaseData memory baseData = _baseData[volume][mintId];
        VolumeMintData storage mintData = _volumeMintData[volume][mintId];
...
/**
 * @dev All the information about a merkle drop mint (combines VolumeMintData with BaseData).
 */
struct MintInfo {
    uint32 startTime;
    uint32 endTime;
...
interface IMerkleDropMinter is IMinterModule {
    ...
    /**
     * @dev Emitted when the `price` is changed for (`volume`, `mintId`).
     * @param volume Address of the song volume contract we are minting for.
     * @param mintId  The mint ID.
     * @param price   Sale price in ETH for minting a single token in `volume`.
     */
    event PriceSet(address indexed volume, uint128 indexed mintId, uint96 price);

    /**
     * @dev Emitted when the `maxMintable` is changed for (`volume`, `mintId`).
     * @param volume               Address of the song volume contract we are minting for.
     * @param mintId                The mint ID.
     * @param maxMintable The maximum number of tokens that can be minted on this schedule.
     */
    event MaxMintableSet(address indexed volume, uint128 indexed mintId, uint32 maxMintable);

    /**
     * @dev Emitted when the `maxMintablePerAccount` is changed for (`volume`, `mintId`).
     * @param volume               Address of the song volume contract we are minting for.
     * @param mintId                The mint ID.
     * @param maxMintablePerAccount The maximum number of tokens that can be minted per account.
     */
    event MaxMintablePerAccountSet(address indexed volume, uint128 indexed mintId, uint32 maxMintablePerAccount);

    /**
     * @dev Emitted when the `merkleRootHash` is changed for (`volume`, `mintId`).
     * @param volume        Address of the song volume contract we are minting for.
     * @param mintId         The mint ID.
     * @param merkleRootHash The merkle root hash of the entries.
     */
    event MerkleRootHashSet(address indexed volume, uint128 indexed mintId, bytes32 merkleRootHash);
    ...
    /*
     * @dev Sets the `price` for (`volume`, `mintId`).
     * @param volume Address of the song volume contract we are minting for.
     * @param mintId  The mint ID.
     * @param price   Sale price in ETH for minting a single token in `volume`.
     */
    ...
    /*
     * @dev Sets the `maxMintablePerAccount` for (`volume`, `mintId`).
     * @param volume               Address of the song volume contract we are minting for.
     * @param mintId                The mint ID.
    ...
    /*
     * @dev Sets the `maxMintable` for (`volume`, `mintId`).
     * @param volume               Address of the song volume contract we are minting for.
     * @param mintId                The mint ID.
     * @param maxMintable The maximum number of tokens that can be minted on this schedule.
     */
    ...
    /*
     * @dev Sets the `merkleRootHash` for (`volume`, `mintId`).
     * @param volume        Address of the song volume contract we are minting for.
     * @param mintId         The mint ID.
     * @param merkleRootHash The merkle root hash of the entries.
     */
     ...
     * @param volume The volume to get the mint instance for.
     * @param mintId The ID of the mint instance.
