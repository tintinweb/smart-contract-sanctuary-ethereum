...
{
    IERC721Upgradeable public landV1;
    ...
    function initialize(
        ...
        baseURI = _baseURI;
        landV1 = IERC721Upgradeable(_landV1Address);
    }
     ...
     */
    function migrate(uint256[] calldata _tokenIds) external whenNotPaused nonReentrant {
        uint256 _tokenIdsNumber = _tokenIds.length;
        require(_tokenIdsNumber <= 50, "Too many");

        for (uint256 _index = 0; _index < _tokenIdsNumber; _index++) {
            landV1.transferFrom(_msgSender(), address(this), _tokenIds[_index]);
            _mint(_msgSender(), _tokenIds[_index]);
        }
    }
     ...
     * @param _baseURI the root folder
   */
