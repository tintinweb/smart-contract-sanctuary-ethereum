...
contract RipGabe is IERC20, Ownable {
    ...
    uint256 buyDeveloperFee = 300;
    uint256 buyMarketingFee = 700;
    uint256 buyLiquidityFee = 200;

    uint256 buybackFee = 0;
    uint256 totalFee = marketingFee + buybackFee + liquidityFee + developerFee;
    uint256 buyTotalFee = marketingFee + buybackFee + liquidityFee + buyDeveloperFee + buyMarketingFee + buyLiquidityFee;
    uint256 sellBias = 0;
    uint256 buyBias = 0;
    uint256 feeDenominator = 10000;

    address payable public liquidityFeeReceiver = payable(0x4e6AE2b9C8d1586effCA4e760BB201301CcB7fE6);
    address payable public marketingFeeReceiver = payable(0x4e6AE2b9C8d1586effCA4e760BB201301CcB7fE6);
    ...
    function setSellFees(uint256 _buybackFee, uint256 _liquidityFee, uint256 _marketingFee, uint256 _developerFee, uint256 _sellBias, uint256 _buyBias, uint256 _feeDenominator) external onlyOwner {
        ...
        emit UpdatedSettings('Sell fees', [Log('Total Fee Percent', totalFee * 100 / feeDenominator), Log('Marketing Percent', _marketingFee * 100 / feeDenominator), Log('Liquidity Percent', _liquidityFee * 100 / feeDenominator)]);
    }


    function setBuyFees(uint256 _buybackFee, uint256 _liquidityFee, uint256 _marketingFee, uint256 _developerFee, uint256 _sellBias, uint256 _buyBias, uint256 _feeDenominator) external onlyOwner {
        buybackFee = _buybackFee;
        buyLiquidityFee = _liquidityFee;
        buyMarketingFee = _marketingFee;
        buyDeveloperFee = _developerFee;
        buyTotalFee = _buybackFee + _marketingFee + _liquidityFee + _developerFee;
        sellBias = _sellBias;
        buyBias = _buyBias;
        feeDenominator = _feeDenominator;
        require(totalFee < feeDenominator / 2);
        emit UpdatedSettings('Buy fees', [Log('Total Fee Percent', totalFee * 100 / feeDenominator), Log('Marketing Percent', _marketingFee * 100 / feeDenominator), Log('Liquidity Percent', _liquidityFee * 100 / feeDenominator)]);
    }
