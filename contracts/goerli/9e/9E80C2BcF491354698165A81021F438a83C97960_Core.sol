...
contract Core is ICoreStorage, IFallbackManager, Executor {
    ...
    function execute(
        ...
        uint256 value,
        bytes calldata data
    ) external payable onlyDelegateCall onlyOwner returns (bytes memory) {
        ...
        IModule module = Core(_this).getModule(to, funcHash);
        require(address(module) != address(0x0), "Not supported method.");

        (bool success, bytes memory result) = address(module).delegatecall(
    ...
    function executeModule(bytes calldata data) external override onlyDelegateCall onlyOperator returns (bytes memory) {
        bytes4 funcHash = Utils.parseFunctionSelector(data);
        IModule module = Core(_this).getInternalModule(funcHash);
        require(address(module) != address(0x0), "Not supported method.");

        (bool success, bytes memory result) = address(module).delegatecall(data);
        if (success == false) {
            assembly {
                let ptr := mload(0x40)
                returndatacopy(ptr, 0, returndatasize())
...
abstract contract IFallbackManager {
    ...
    /// @notice Returns an ETH balance of the wallet.
    /// return ETH balance of the wallet.
    function getBalance() external view returns (uint256) {
        return address(this).balance;
    }

    /// @notice Allows the wallet to receive an ETH.
