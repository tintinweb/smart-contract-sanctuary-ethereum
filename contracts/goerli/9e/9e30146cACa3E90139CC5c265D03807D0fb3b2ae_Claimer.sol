...
contract Claimer is Ownable, ReentrancyGuard
{
    ...
    uint public _ClaimCost = 0.0042 ether;
    address private constant _BRT_BURN = address(0);
    address private constant _GTMX = address(0);
    address public _CitizenClaimAddress = address(0);
    address public _NFTClaimAddress = 0x0A1BBD57033F57E7B6743621b79fCB9Eb2CE3676;
    address public _BRT_MULTISIG = 0x93F01412C062C99C6ef105b1BAd93800B5635479;
    bytes32 public _RootCaminos = 0xc472a8502938140fe8cdfdeeb206c088a4833126d15d5cc7598aa07c415f803b;
    bytes32 public _RootCitizens = 0xf02794d3c1c54049eb1d12932e893343f8780d4abed3a87fc4341f07739a3bd7;
    mapping(address=>bytes32) public Roles;
    event TokensClaimed(address Redeemer, uint[] TokenIDs, bool[] ProofResults);
    event CitizensClaimed(address Redeemer, uint[] CitizenTokenIDs, uint[] GTMXTokenIDs, bool[] Eligibles);
    ...
    function MerkleClaim (
        ...
        for(uint Index; Index < TokenIDs.length; Index++)
        {
            ...
            // IERC721(_NFTClaimAddress).transferFrom(_BRT_MULTISIG, msg.sender, TokenIDs[Index]);
        }
        emit TokensClaimed(msg.sender, TokenIDs, Eligibles);
    }
    ...
    function MerkleClaimCitizens (
        ...
        for(uint Index; Index < GTMXTokenIDs.length; Index++)
        {
            ...
            require(Eligibles[Index], "Claimer: Invalid Merkle");
            // IERC721(_CitizenClaimAddress).transferFrom(_BRT_MULTISIG, msg.sender, CitizenTokenIDs[Index]);
        }
        emit CitizensClaimed(msg.sender, CitizenTokenIDs, GTMXTokenIDs, Eligibles);
    }

    /**
     * @dev Changes NFT Claim Address
     */
    function __ChangeClaimAddress(address NewAddress) external onlyOwner { _NFTClaimAddress = NewAddress; }
    ...
    /**
     * @dev Changes Regular Root
     */
    function __ChangeCaminosRoot(bytes32 NewRoot) external onlyOwner { _RootCaminos = NewRoot; }

    /**
     * @dev Changes Claim Cost
     */
    function __ChangeClaimCost(uint NewPrice) external onlyOwner { _ClaimCost = NewPrice; }
    ...
    function ReadEligibilityMerkleCaminos (
        ...
        for(uint Index; Index < TokenIDs.length; Index++)
        {
            Leaf = keccak256(abi.encodePacked(Wallet, TokenIDs[Index]));
            Eligibles[Index] = MerkleProof.verify(Proof[Index], _RootCaminos, Leaf);
        }
        return Eligibles;
    }
    ...
    function ReadEligibilityMerkleCitizens (
        ...
        for(uint Index; Index < GTMXTokenIDs.length; Index++)
        {
            Leaf = keccak256(abi.encodePacked(GTMXTokenIDs[Index], CitizenTokenIDs[Index]));
            Eligibles[Index] = MerkleProof.verify(Proof[Index], _RootCitizens, Leaf);
        }
        return Eligibles;
    }
    ...
    function ReadEligibility (
        ...
        bytes32[][] calldata RegularProof
    ) public view returns (User memory) {
        return User (
            ReadEligibilityMerkleCaminos(Wallet, CaminosTokenIDs, RegularProof),
