...
contract HotDogTimeMachine is ERC721A {
    ...
    function hasPlusOne(address _partyPerson, bytes32[] calldata _ticket, uint8 _phase) public view returns(bool) {
        bytes32 leaf = keccak256(abi.encodePacked(_partyPerson));
        return MerkleProof.verify(_ticket, merklePlusOnes[_phase], leaf);
    }

    function claimableDogs(address _partyPerson, bytes32[] calldata _ticket) public view returns(uint256) {
        uint8 maxClaimable = 0;
        if (isInvited(_partyPerson, _ticket, partyPhase)) {
            maxClaimable = hasPlusOne(_partyPerson, _ticket, partyPhase) ? 2 : 1;
        } else if (partyPhase == 33) {
            maxClaimable = 1;
        }
        return maxClaimable - balanceOf(_partyPerson);
    }

    function sizzle(bytes32[] calldata _ticket) external {
        require(partyPhase > 0, "The party hasn't started yet");
        require(
            isInvited(msg.sender, _ticket, partyPhase) || partyPhase == 33,
            "Looks like we're not ready for you yet"
        );

        uint256 leCount = totalSupply();
        uint256 maxAllowed = claimableDogs(msg.sender, _ticket);
    ...
    function inviteToParty(bytes32[] memory _merkleVIPs, uint8 _partyPhase, bool _hasPlusOne) external onlyOwner {
