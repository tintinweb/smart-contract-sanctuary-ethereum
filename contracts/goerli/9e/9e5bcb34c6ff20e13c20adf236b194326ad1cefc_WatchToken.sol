...
contract WatchToken is ERC721, AccessControl {
    struct TokenInfo {
        uint256 tokenId;
        string tokenURI;
        address owner;
    ...
    constructor(
        ...
        for (uint16 i = 0; i < _admins.length; i++) {
    ...
    function _checkSignature(
        ...
        bytes32 digest = keccak256(
            abi.encodePacked(
                ...
                keccak256(
                    abi.encode(
                        keccak256("Msg(string hashedData,uint256 deadline)"),
                        keccak256(bytes(hashedData)),
                        deadline
                    )
                )
            )
    ...
    function ownershipToContractOwner(uint256 tokenId)
        public
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        require(
