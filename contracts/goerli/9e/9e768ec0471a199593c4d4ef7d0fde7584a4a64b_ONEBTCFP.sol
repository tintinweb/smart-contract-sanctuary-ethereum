...
contract ONEBTCFP is ERC721A, ERC2981, Owned, ReentrancyGuard, DefaultOperatorFilterer {
    ...
    modifier mintCompliance(uint256 _amount) {
        if (!saleState.Public && !saleState.Whitelist) revert SaleNotActive();
        if (_amount > maxPerWallet) revert ExceedsMaxPerWallet();
    ...
    function mint(uint256 _amount) external payable SupplyCompliance(_amount) mintCompliance(_amount) nonReentrant {
        uint256 cost = _amount * mintPrice;
        bool claimed = freeClaim[msg.sender];

        if (saleState.Whitelist && !claimed) {
            if (IERC721(phaseThreeContract).balanceOf(msg.sender) < 1) revert PublicNotActive();
            if (_amount >= 1) {
                cost = (_amount - 1) * mintPrice;
            } else {
                cost = _amount * mintPrice;
            }
        }

        if (msg.value != cost) revert IncorrectAmountOfETH();
    ...
    function teamMint(uint256 _amount) external onlyOwner SupplyCompliance(_amount) {
        _mint(msg.sender, _amount);
    }
    ...
    function changeMaxPerWallet(uint256 _amount) external onlyOwner {
