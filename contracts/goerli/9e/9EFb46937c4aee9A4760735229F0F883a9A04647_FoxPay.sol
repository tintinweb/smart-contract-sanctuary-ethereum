    ...
    IERC721Enumerable
{
    ...
    function setMessage(uint256 tokenId_, string[] memory message_) public virtual {
        for (uint i = 0; i < message_.length; i++) {
    ...
    function _generateSVGmessage(
        ...
        return
            string(
                abi.encodePacked(
                    /* solhint-disable */
                    _generateSVGmessagePart(messageAarray_.length >= 1 ? messageAarray_[0] : "", '"600"'),
                    _generateSVGmessagePart(messageAarray_.length >= 2 ? messageAarray_[1] : "", '"668"'),
                    _generateSVGmessagePart(messageAarray_.length >= 3 ? messageAarray_[2] : "", '"736"')
                    /* solhint-enable */
                )
            );
    }
