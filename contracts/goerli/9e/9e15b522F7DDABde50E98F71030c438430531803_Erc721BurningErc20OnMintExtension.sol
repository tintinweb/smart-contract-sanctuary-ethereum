...
contract Erc721BurningErc20OnMintExtension is ERC721CreatorExtensionApproveTransfer {
    mapping(address => address) private nftContractToErc20Mapping;

    function setErc20TokenAddress(address nftContractAddress, address erc20TokenAddress) public
    {
        require(Ownable(nftContractAddress).owner() == msg.sender, "you must be the owner of nftContractAddress");
        nftContractToErc20Mapping[nftContractAddress] = erc20TokenAddress;
    }
    ...
    function approveTransfer(address from, address to, uint256 tokenId) external override returns (bool) {
        ...
        if (from == address(0) && to != address(0)) {
            address erc20TokenAddress = nftContractToErc20Mapping[msg.sender];
            require(
