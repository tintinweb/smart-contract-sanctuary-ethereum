...
interface IWTokenGateway {
    function depositWToken() external payable;

    function withdrawWToken(uint256 amount) external;
}
...
contract WTokenGateway is IWTokenGateway {
     ...
     */
    function depositWToken() external payable override {
        WTOKEN.deposit{value: msg.value}();
        VAULT.deposit(address(WTOKEN), msg.value);
    }

    /**
     * @dev withdraw naive token
     */
    function withdrawWToken(uint256 amount) external override {}
}