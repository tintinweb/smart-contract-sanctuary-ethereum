...
contract PxTrainerAdventure is Utils, ReentrancyGuard {
    ...
    function claimTreasure(uint256 weekNumber) external {
        ...
        if (
            ...
            weekInfos[weekNumber].winners[msg.sender].claimLimit
        ) {
    ...
    function multiClaim(uint256 weekNumber) internal {
        ...
        for (uint256 index = 1; index <= week.treasureCount; index++) {
            if (
                week.winners[msg.sender].claimedTreasureType[
                    treasures[week.distributions[index].treasureIndex]
                        .treasureType
                ]
            ) {
        ...
        if (altRemaining == week.remainingSupply) {
            uint256 random = randomNumber -
                ((randomNumber / altRemaining) * altRemaining) +
                1;
            for (uint256 index = 1; index <= week.treasureCount; index++) {
                if (
            ...
            uint256 random = randomNumber -
                ((randomNumber / remaining) * remaining) +
                1;
        ...
        uint256 selectedTreasureIndex = week
            .distributions[selectedIndex]
            .treasureIndex;
        week.distributions[selectedIndex].totalSupply--;
        week.winners[msg.sender].claimedTreasureType[
            treasures[selectedTreasureIndex].treasureType
        ] = true;
        week.winners[msg.sender].claimed++;
    ...
    function firstClaim(uint256 weekNumber) internal {
        ...
        uint256 randomNumber = getRandomNumber();
        uint256 random = randomNumber -
            ((randomNumber / week.remainingSupply) * week.remainingSupply) +
            1;
    ...
    function updateWeeklyWinner(
        ...
        for (uint256 index = 0; index < winners.length; index++) {
            weekInfos[weekNumber].winners[winners[index]].claimLimit = counts[
                index
            ];
        }
    }

    function updateWeeklyTreasureDistribution(
        uint256 weekNumber,
        uint256[] calldata treasureindexes,
        uint256[] calldata counts
    ) external {
        Week storage week = weekInfos[weekNumber];
        ...
        for (uint256 index = 0; index < treasureindexes.length; index++) {
            week.treasureCount++;
            week
                .distributions[week.treasureCount]
                .treasureIndex = treasureindexes[index];
    ...
    function setWeeklyTimeStamp(
        ...
        if (
            _weeklyDuration <=
            (_prizeUpdationDuration + _winnerUpdationDuration)
        ) {
        ...
        for (uint256 index = 0; index < _numberOfWeeks; index++) {
            ...
            weekInfos[totalWeek].ticketDrawTimeStamp =
                _startTimeStamp +
                _prizeUpdationDuration;
            weekInfos[totalWeek].claimStartTimeStamp =
                _startTimeStamp +
                _prizeUpdationDuration +
                _winnerUpdationDuration;
            weekInfos[totalWeek].endTimeStamp =
                _startTimeStamp +
                _weeklyDuration -
                1;
