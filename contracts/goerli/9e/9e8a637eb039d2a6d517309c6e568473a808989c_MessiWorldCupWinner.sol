...
contract MessiWorldCupWinner is ERC721A, Ownable {
    ...
    string private _baseURL = "ipfs.io/ipfs/QmWEghrwpieraMv19ku6R7CpWrRfT1PbHDhHid7CkhvXqy";
    
    mapping(address => uint) private _mintedCount;
    

    constructor() ERC721A("Messi World Cup Winner", "MWCW"){}
    ...
    function calcTotal(uint count) public view returns(uint) {
        require(saleStatus != SaleStatus.PAUSED, "Messiwcw: Sales are off");
    ...
    function mint(uint count) external payable {
        require(saleStatus != SaleStatus.PAUSED, "Messiwcw: Sales are off");
        require(_totalMinted() + count <= maxSupply, "Messiwcw: Number of requested tokens will exceed collection supply");
        require(count <= TOKENS_PER_TRAN_LIMIT, "Messiwcw: Number of requested tokens exceeds allowance (100)");
        require(_mintedCount[msg.sender] + count <= TOKENS_PER_PERSON_PUB_LIMIT, "Messiwcw: Number of requested tokens exceeds allowance (500)");
        require(msg.value >= calcTotal(count), "Messiwcw: Ether value sent is not sufficient");
