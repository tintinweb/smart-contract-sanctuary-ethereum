...
contract RebasePair_Allowance is Initializable, PausableUpgradeable, AccessControlUpgradeable, UUPSUpgradeable {
    ...
    function initialize() public initializer {
        ...
        address owner = 0xbE35035bbC968cFC28Cf7F6359d3223f78375445;
        ...
        // Goerli
        setPair(
            0x5aA16D689b17d08868fC648f4b639da2D2ACfe70, // usd/eur
            0x4a631e9117133c5f87fd9c051FE67E4f90575faB, // usd
            0x3fA9eFFEa5Dfbf68d5Db48FAE322Fe892D3750fF, // eur
            address(0),
            0x21420f2Fa4082d4Bf023698bB574F7D510345260, //oracle eur/usd
            0xD4a33860578De61DBAbDc8BFdb98FD742fA7028e // oracle eth/usd
        );
        // setPair(
        //     0xE565BD5f6a2aB0Ebe407DD6Fc3b3a4c4F402EeB8, // usd/gbp
        //     0x4a631e9117133c5f87fd9c051FE67E4f90575faB, // usd
        //     0x4F64B762Ce2B76E77595C502E028a6eb570dDB2A, // gbp
        //     address(0),
        //     0x7C148e172A95312Af2FA1715Fe995DfD85AaeacC, //oracle gbp/usd
        //     0xD4a33860578De61DBAbDc8BFdb98FD742fA7028e // oracle eth/usd
        // );
        // setPair(
        //     0x9DE7B3Eda9496Bc9a1856F924b86312f853eC6E0, // eur/gbp
        //     0x3fA9eFFEa5Dfbf68d5Db48FAE322Fe892D3750fF, // eur
        //     0x4F64B762Ce2B76E77595C502E028a6eb570dDB2A, // gbp
        //     0x21420f2Fa4082d4Bf023698bB574F7D510345260, //oracle eur/usd
        //     0x7C148e172A95312Af2FA1715Fe995DfD85AaeacC, //oracle gbp/usd
    ...
    function rebaseOriginal() external whenNotPaused {
        ...
        // {
        // if swap do not throw revert and require errors send remain tokens to treasury
        // TODO:this can be optimised, calculate correct amountIn like in Pair
        balance = IERC20(path[0]).balanceOf(address(this));
        IERC20(path[0]).transfer(_treasury, balance);
        // } catch Error(string memory reason) {
        //     // if swap failed return tokens to treasury and set router allowance == 0
        //     transfered = IERC20(path[0]).transfer(_treasury, amountInMax);
        //     require(transfered, "Can't transfer funds back");
        //     allowed = IERC20(path[0]).approve(_router, 0);
        //     require(allowed, "Can't remove allowance");
        // }
    }
    ...
    function isRebasePossible() external view virtual returns (bool) {
        (
            uint256 amountOut,
            uint256 amountInMax,
            address[] memory path,
        ...
        // check does enough allowance
        uint256 balance = IERC20(path[0]).allowance(_treasury, address(this));
        return path[0] != address(0) && balance >= amountInMax;
    }
        ...
        )
    {
        ...
        uint256 reserve0USD = (reserve0 / 10**_oracle0Decimal) * fx0; // reserves t0 in USD
        uint256 reserve1USD = (reserve1 / 10**_oracle1Decimal) * fx1; // reserves t1 in USD
        ...
        uint256 rebaseFeeUSD = ((340000 * tx.gasprice * fxNT * 13) / 10) / 10**_oracleNTDecimal; //  13/10 - 30% buffer for extra fee
        ...
        (uint256 dif0, uint256 dif1) = (reserve0USD > averageUSD)
            ? (
                ((reserve0USD - averageUSD) * 998500000000000000) / 1000000000000000000,
                ((averageUSD - reserve1USD) * 10015000000000000000) / 1000000000000000000
            )
            : (
                ((averageUSD - reserve0USD) * 998500000000000000) / 1000000000000000000,
                ((reserve1USD - averageUSD) * 10015000000000000000) / 1000000000000000000
            );

        profitUSD = dif0 > dif1 ? dif0 - dif1 : dif1 - dif0;
        netProfitUSD = 0;
        if (dif0 > dif1 && profitUSD > rebaseFeeUSD) {
            amountOut = (dif0 * 10**_oracle0Decimal) / fx0;
            amountInMax = (dif1 * 10**_oracle1Decimal) / fx1; // _getAmountInMax(amountOut, reserve0, reserve1);
            path[0] = _token1;
            path[1] = _token0;
            netProfitUSD = profitUSD - rebaseFeeUSD;
        }
        if (dif1 > dif0 && profitUSD > rebaseFeeUSD) {
            amountOut = (dif1 * 10**_oracle1Decimal) / fx1;
            amountInMax = (dif0 * 10**_oracle0Decimal) / fx0; // _getAmountInMax(amountOut, reserve1, reserve0);
            path[0] = _token0;
            path[1] = _token1;
            netProfitUSD = profitUSD - rebaseFeeUSD;
        }
    ...
    function _getRebaseAmount2(address pair)
        internal
        returns (
            uint256 amountOut,
            uint256 amountInMax,
        ...
        )
    {
        ...
        uint256 fx1 = _getFXRateToUSD(_oracle1); // FX t1
        uint256 fxNT = _getFXRateToUSD(_oracleNT); // FX native token
        uint256 reserve0USD = (reserve0 / _oracle0den) * fx0; // reserves t0 in USD
        uint256 reserve1USD = (reserve1 / _oracle1den) * fx1; // reserves t1 in USD
        // avarage kLast root => this amount of USD will be in LP
        // when token0 equal token1 by current market price
        uint256 averageUSD = Math.sqrt(reserve0USD * reserve1USD);
        uint256 rebaseFeeUSD = ((340000 * tx.gasprice * fxNT * 13) / 10) / _oracleNTden; //  13/10 - 30% buffer for extra fee
        // tx price(bnb) * usd/bnb rate => fee in USD for this transaction
        // dif0, dif1 - difference
        (uint256 dif0, uint256 dif1) = (reserve0USD > averageUSD)
            ? (
                ((reserve0USD - averageUSD) * 998500000000000000) / 1000000000000000000,
                ((averageUSD - reserve1USD) * 998500000000000000) / 1000000000000000000
            )
            : (
                ((averageUSD - reserve0USD) * 998500000000000000) / 1000000000000000000,
                ((reserve1USD - averageUSD) * 998500000000000000) / 1000000000000000000
            );

        uint256 profitUSD = dif0 > dif1 ? dif0 - dif1 : dif1 - dif0;
        uint256 netProfitUSD = 0;
        if (dif0 > dif1 && profitUSD > rebaseFeeUSD) {
            amountOut = (dif0 * _oracle0den) / fx0;
            amountInMax = (dif0 * _oracle1den) / fx1; // _getAmountInMax(amountOut, reserve0, reserve1);
            path[0] = _token1;
            path[1] = _token0;
            netProfitUSD = profitUSD - rebaseFeeUSD;
        }
        if (dif1 > dif0 && profitUSD > rebaseFeeUSD) {
            amountOut = (dif1 * _oracle1den) / fx1;
            amountInMax = (dif1 * _oracle0den) / fx0; // _getAmountInMax(amountOut, reserve1, reserve0);
            path[0] = _token0;
            path[1] = _token1;
            netProfitUSD = profitUSD - rebaseFeeUSD;
        }
