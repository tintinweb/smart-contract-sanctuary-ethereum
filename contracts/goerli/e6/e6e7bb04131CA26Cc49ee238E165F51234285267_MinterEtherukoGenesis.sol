    ...
    AccessControl
{
    ...
    uint256 public wlPrice = 1000;
    uint256 public pbPrice = 10000;

    uint256 public maxTxLimit = 5;

    bool public onSale = false;

    uint256 public maxSale = 2000;

    mapping(address => bool) public og;
    mapping(address => bool) public whitelist;

    mapping(address => bool) public ogSaled;
    mapping(address => bool) public whitelistSaled;
    ...
    {
        require(
            whitelistSaled[msg.sender] == false,
            "msg.sender already purchased by whitelist"
        );
        whitelistSaled[msg.sender] = true;
        require(amount <= 2, "require: amount must be less than 2");
        uint256 totalSupply = etherukoGenesis.totalSupply();
        require(
            totalSupply + amount <= maxSale,
    ...
    function pbPriceMint(uint256 amount) public payable onlyOnSale {
        ...
        require(msg.value >= totalPrice, "require: msg.value >= totalPrice");
        etherukoGenesis.safeMint(msg.sender, amount);
        feeTo.transfer(msg.value);
        emit PbPriceMint(msg.sender, amount, totalSupply + amount, msg.value);
    }
}
