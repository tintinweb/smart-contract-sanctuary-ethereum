...
contract BarleyTown is ERC721A, Ownable {
    ...
    string  public baseTokenURI = "ipfs://xxx/";
    string  public defaultTokenURI;
    uint256 public maxSupply = 10000;
    uint256 public publicSalePrice = 0.5 ether;
    uint256 public allowListSalePrice = 0.003 ether;
    bytes32 private _merkleRoot = 0x93d03a9fe0d22f4879230e0f5bc758b711ce5a4181b3412f92eb2e5702de43df;
    bytes32 private _nswapmerkleRoot = 0x6dbc5fc51df4b7142af0eed57a795240dbb53fdc12a6e0056aa167861e44c5dd;
    uint256 public nswapLaunch;

    mapping(address => uint256) public allowlistmint;
    mapping(address => uint256) public nsawpAllowlistmint;

    address payable public payMent;

    constructor() ERC721A ("BarleyTown", "BT") {
    ...
    function mint(uint256 _quantity) external payable {
        ...
        require(_quantity <= 2, "Invalid quantity");
        require(msg.value >= publicSalePrice * _quantity, "Ether is not enough");
        if (msg.value > 0) {
    ...
    function allowListMint(bytes32[] calldata _merkleProof, uint256 _quantity) external payable  {
        ...
        require(totalSupply() + _quantity <= maxSupply, "Exceed supply");
        require(allowlistmint[msg.sender] + _quantity <= 3, "Exceed supply");
    ...
    function allowListMintNswap(bytes32[] calldata _merkleProof, uint256 _quantity) external  payable  {
        require(publicMintStatus == EPublicMintStatus.ALLOWLIST_MINT, "Allowlist sale closed");
        require(totalSupply() + _quantity <= maxSupply, "Exceed supply");
        require(nsawpAllowlistmint[msg.sender] + _quantity <= 3, "Exceed supply");
        require(msg.value >= allowListSalePrice * _quantity);
        require(isNswapWhitelistAddress(msg.sender, _merkleProof), "Caller is not in whitelist or invalid signature");
        ...
        nsawpAllowlistmint[msg.sender] += _quantity;
        nswapLaunch += _quantity;
        _safeMint(msg.sender, _quantity);
    }
    ...
    function isNswapWhitelistAddress(address _address, bytes32[] calldata _signature) public view returns (bool) {
        return MerkleProof.verify(_signature, _nswapmerkleRoot, keccak256(abi.encodePacked(_address)));
    }
    ...
    function setNswapMerkleRoot(bytes32 nswapmerkleRoot) external onlyOwner {
        _nswapmerkleRoot = nswapmerkleRoot;
    }
    ...
    function setallowListSalePrice(uint256 _allowListSalePrice) external onlyOwner {
