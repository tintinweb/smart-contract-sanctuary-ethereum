...
import {PoCTicket, IERC721} from "poc-ticket/PoCTicket.sol";
...
contract DeployTestnet is Script {
    ...
    IERC721 public moonbirds;
    IERC721 public oddities;
    ...
    function setUp() public {
        ...
        proof = IERC721(0x1166ce7a0eb78fa0FcBD1AD42BEebA68447e07e6);
        moonbirds = IERC721(0x3Cb5c6448eD756233fe9b91aBC4dB27a1cc17080);
        oddities = IERC721(0xF355ED8Fbc10A22F775797394a52540Ce3B3eb6f);
...
import {PurchaseLimits} from "./PurchaseLimits.sol";
import {VIP} from "./VIP.sol";
 ...
 * @author cxkoda
 */
contract PoCTicket is ERC721ACommon, BaseTokenURI, PurchaseLimits, VIP {
    ...
    IERC721 internal immutable _oddities;

    IDelegationRegistry internal immutable _delegationRegistry;

    uint256 internal constant _MAX_AIRDROPPABLE = 250;
    ...
    uint256 public fullTicketPrice;
    uint256 public reducedTicketPrice;
    uint256 public upgradePrice;

    address payable internal _primarySalesReceiver;

    mapping(IERC721 => mapping(uint256 => uint256)) internal _numTokenUsed;

    /**
     * @dev We use this to reduce the storage requirements for the block number
     * in which the tickets have been minted.
     */
    uint256 internal immutable _blockNumberOffset;
        ...
        VIP(125)
    {
        ...
        upgradePrice = 0.5 ether;

        _primarySalesReceiver = primarySalesReceiver_;

        _blockNumberOffset = block.number;
    }
    ...
    function purchase(
        ...
        // race for tickets.
        uint256 num = _numTickets(pcRedemptions) + _numTickets(mbRedemptions)
            + _numTickets(oddRedemptions) + numGA;
        _checkAndTrackPurchaseLimits(vault, num, numGA);

        _checkStage(Stage.ProofCollective, pcRedemptions);
        _checkStage(Stage.Moonbirds, mbRedemptions);
        _checkStage(Stage.Oddities, oddRedemptions);
        _checkStage(Stage.GeneralAdmission, numGA);

        _checkDelegation(
            vault, delegationType, pcRedemptions, mbRedemptions, oddRedemptions
        );
        ...
        // Interactions
        _primarySalesReceiver.sendValue(msg.value);
        _doPurchase(vault, num);
    }

    function _numTickets(PoCTicket.TokenRedemption[] memory redemptions)
        internal
        pure
        returns (uint256)
    {
    ...
    function _redemptionCost(
        ...
        // This guarantees that no token is supplied twice, which allows us to
        // write the rest of the view function more efficiently because we don't
        // have to track usage.
        uint256 leftTokenId = redemptions[0].tokenId;
        for (uint256 i = 1; i < redemptions.length; ++i) {
            uint256 id = redemptions[i].tokenId;
            if (id <= leftTokenId) {
        ...
        for (uint256 i; i < redemptions.length; ++i) {
            uint256 usageStart = _numTokenUsed[token][redemptions[i].tokenId];
    ...
    function _checkDelegation(
        ...
        if (delegationType == IDelegationRegistry.DelegationType.ALL) {
            if (_delegationRegistry.checkDelegateForAll(msg.sender, vault)) {
                return;
            }
        ...
        if (delegationType == IDelegationRegistry.DelegationType.CONTRACT) {
            _checkContractDelegation(vault, _proof, pcRedemptions);
            _checkContractDelegation(vault, _moonbirds, mbRedemptions);
            _checkContractDelegation(vault, _oddities, oddRedemptions);
            return;
        }

        if (delegationType == IDelegationRegistry.DelegationType.TOKEN) {
            _checkTokenDelegation(vault, _proof, pcRedemptions);
            _checkTokenDelegation(vault, _moonbirds, mbRedemptions);
            _checkTokenDelegation(vault, _oddities, oddRedemptions);
            return;
        }
    ...
    function _checkContractDelegation(
        ...
        if (
            !_delegationRegistry.checkDelegateForContract(
                msg.sender, vault, address(token)
            )
        ) {
        ...
        for (uint256 i; i < redemptions.length; ++i) {
            if (
                !_delegationRegistry.checkDelegateForToken(
                    msg.sender, vault, address(token), redemptions[i].tokenId
                )
            ) {
    ...
    function _checkStage(Stage stage_, TokenRedemption[] calldata redemptions)
        internal
        view
    {
        _checkStage(stage_, redemptions.length);
    }

    function _checkStage(Stage stage_, uint256 numPurchase) internal view {
        if (numPurchase == 0) {
            return;
        }
    ...
    function _doPurchase(address to, uint256 num) internal {
        uint16 startTokenId = uint16(_nextTokenId());
        _mint(to, num);
        // using the `startTokenId` as mint batch identifier
        _setExtraDataAt(
            startTokenId, uint24(block.number + 1 - _blockNumberOffset)
        );
        _setMixHash(block.number, block.difficulty);
    }

    function _doAirdrop(address to, uint256 num) internal {
        _mint(to, num);
    }
    ...
    {
        return _primarySalesReceiver;
    }

    function _airdropped(uint256 tokenId)
        internal
        view
        virtual
        override
        returns (bool)
    {
        return _ownershipOf(tokenId).extraData == 0;
    }

    function _revealBlockNumber(uint256 tokenId)
        internal
        view
        virtual
        override
    ...
    function _upgradePrice() internal view virtual override returns (uint256) {
        return upgradePrice;
    }
...
contract PurchaseLimits {
    ...
    function _checkAndTrackPurchaseLimits(
        ...
        if (_numTicketsSold + numTotal > _MAX_SELLABLE) {
...
abstract contract VIP {
    ...
    error TicketNotEligibleForVIPUpgrade(uint256);
    error IncorrectSigner();

    uint256 internal constant _ONE = 1 << 64;
    uint256 internal immutable _freeUpgradeProbability;
    ...
    constructor(uint256 freeUpgradeProbabilityBasePoints_) {
        _freeUpgradeProbability =
            (freeUpgradeProbabilityBasePoints_ * _ONE) / 10_000;
    }

    function _setMixHash(uint256 blockNumber, uint256 mixHash) internal {
        if (_mixHashes[blockNumber] == 0) {
            _mixHashes[blockNumber] = mixHash;
        }
    }
     ...
     */
    function upgraded(uint256 tokenId) public view returns (bool) {
        return _upgraded[tokenId] || _hasFreeUpgrade(tokenId);
    }

    function _upgradable(uint256 tokenId) internal view returns (bool) {
        return !upgraded(tokenId) && !_airdropped(tokenId);
    }

    /**
     * @notice Upgrades a list of tickets to VIP.
     * @dev Ticket ownership is not checked.
     * @dev Reverts if a ticket was already upgraded.
     */
    function upgrade(uint256[] calldata ticketIds) external payable {
        ...
        for (uint256 i; i < numUpgrades; ++i) {
            if (!_upgradable(ticketIds[i])) {
                revert TicketNotEligibleForVIPUpgrade(ticketIds[i]);
            }
            _upgraded[ticketIds[i]] = true;
        }
        ...
        returns (bool)
    {
        return !_upgraded[ticketId]
            && _hasFreeUpgrade(keccak256(signature), ticketId);
    }

    /**
     * @notice Checks if a ticket was upgraded via the random lottery.
     */
     ...
     */
    function publishBlockSalts(SignedBlockNumber[] calldata bns) external {
        ...
        private
    {
        ...
        bytes32 msgHash = ECDSA.toEthSignedMessageHash(abi.encode(blockNumber));
    ...
    function _hasFreeUpgrade(bytes32 salt, uint256 tokenId)
        private
        view
        returns (bool)
    {
        // Airdropped tokens cannot participate in the randomsation
        if (_airdropped(tokenId)) {
            return false;
        }

        bytes32 rand = keccak256(
            abi.encodePacked(
                salt, _mixHashes[_revealBlockNumber(tokenId)], tokenId
            )
        );
        return uint256(rand) % _ONE < _freeUpgradeProbability;
    }

    function _hasFreeUpgrade(uint256 tokenId) internal view returns (bool) {
        bytes32 salt = _salts[_revealBlockNumber(tokenId)];
        if (salt == 0) {
            return false;
        }

        return _hasFreeUpgrade(salt, tokenId);
    }
    ...
    function _upgradeSalesReceiver()
        ...
        virtual
        returns (address payable);
    function _upgradePrice() internal view virtual returns (uint256);
