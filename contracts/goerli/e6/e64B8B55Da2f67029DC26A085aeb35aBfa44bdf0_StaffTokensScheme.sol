...
contract StaffTokensScheme {
  ...
  mapping(address => address[]) public AccountsByToken;
  ...
  function removeAccountByToken(address token, address account) public {
    ...
    uint256 lastAccount = accounts.length - 1;
    bool found;
    uint256 i;
    for (i; i <= lastAccount; i++) {
    ...
    if (found) {
      if (i < lastAccount) AccountsByToken[token][i] = AccountsByToken[token][lastAccount];
