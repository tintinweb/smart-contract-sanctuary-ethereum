...
contract LKStaking is Ownable, Pausable, ReentrancyGuard {
    ...
    event Staked(address indexed owner, address indexed membershipContract, uint256 indexed membershipId, address[] tokensContracts, uint256[] tokenIds);

    /// @notice event emitted when a user has unstaked a membership card and/or LK NFTs
    event Unstaked(address indexed owner, address indexed membershipContract, uint256 indexed membershipId, address[] tokensContracts, uint256[] tokenIds);
    ...
    function stake(uint256 _membershipId, address _membershipAddress, uint256[] calldata _tokenIds, address[] calldata _contractsAddresses) external whenNotPaused nonReentrant {
        ...
        require(
            memberships[_membershipAddress].ownerOf(_membershipId) == msg.sender,
            "Can't stake memberships cards you don't own!"
        );
        ...
        for (uint256 i = 0; i < numTokens; i++) {
            require(
                collections[_contractsAddresses[i]].ownerOf(_tokenIds[i]) == msg.sender,
    ...
    function unstake(uint256 _membershipId, address _membershipAddress, uint256[] calldata _tokenIds, address[] calldata _contractsAddresses, bool unstakeAll) external whenNotPaused nonReentrant {
        require(_tokenIds.length == _contractsAddresses.length,'Ids and Addresses list not match');
        require(_tokenIds.length > 0 || unstakeAll,'Need one or more NFT');
        require(
            stakers[msg.sender].membershipId > 0 && stakers[msg.sender].membershipId == _membershipId,
