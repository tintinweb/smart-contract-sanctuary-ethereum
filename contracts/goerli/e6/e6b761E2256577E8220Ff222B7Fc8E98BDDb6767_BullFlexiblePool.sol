...
abstract contract Pausable is Initializable, ContextUpgradeable {
     ...
     */
    function __pausable_initialize() public initializer {
        __Context_init();
        _paused = false;
    }
...
interface IBullPool {
    struct UserInfo {
        ...
        uint256 bullAtLastUserAction; // keep track of bull deposited at the last user action.
...
contract BullFlexiblePool is OwnableUpgradeable, Pausable {
    using SafeERC20Upgradeable for IERC20Upgradeable;

    struct UserInfo {
        uint256 shares; // number of shares for a user
        uint256 lastDepositedTime; // keeps track of deposited time for potential penalty
        uint256 bullAtLastUserAction; // keeps track of bull deposited at the last user action
        uint256 lastUserActionTime; // keeps track of the last user action time
    }

    IERC20Upgradeable public token; // Bull token
    IBullPool public bullPool; //Bull pool
    ...
    //When call bullpool.withdrawByAmount function,there will be a loss of precision, so need to withdraw more.
    uint256 public withdrawAmountBooster; // 1.0001
    ...
    /**
     * @notice Constructor
     * @param _token: Bull token contract
     * @param _bullPool: BullPool contract
    ...
    function initialize(
        ...
        staking = true;
        withdrawAmountBooster = 10001; // 1.0001
    ...
    /**
     * @notice Deposits funds into the Bull Flexible Pool.
     * @dev Only possible when contract not paused.
     * @param _amount: number of tokens to deposit (in BULL)
     */
    function deposit(uint256 _amount) external whenNotPaused {
        ...
        if (user.shares > 0) {
            ...
            uint256 earnAmount = totalAmount - user.bullAtLastUserAction;
            ...
            if (currentPerformanceFee > 0) {
                ...
                if (_amount >= currentPerformanceFee) {
                    ...
                    withdrawAmount =
                        ...
                        10000;
                    bullPool.withdrawByAmount(withdrawAmount);
        ...
        user.bullAtLastUserAction = (user.shares * balanceOf()) / totalShares;
        user.lastUserActionTime = block.timestamp;

        emit DepositBull(msg.sender, _amount, currentShares, block.timestamp);
    }

    /**
     * @notice Withdraws funds from the Bull Flexible Pool
    ...
    function withdraw(uint256 _shares) public {
        ...
        if (user.shares > 0) {
            user.bullAtLastUserAction =
                (user.shares * balanceOf()) /
                totalShares;
        } else {
            user.bullAtLastUserAction = 0;
        }
    ...
    /**
     * @notice Withdraw funds from the Bull Pool.
    ...
    function withdrawByAmount(uint256 _amount) public whenNotPaused {
        ...
        require(
            _amount <= totalAmount,
            "Withdraw amount must be less than totalAmount"
        );
        uint256 earnAmount = totalAmount - user.bullAtLastUserAction;
        uint256 currentPerformanceFee;
        uint256 performanceFeeShares;
        ...
        //The current pool balance should not include currentPerformanceFee.
        uint256 _shares = (totalShares * _amount) /
            (balanceOf() - currentPerformanceFee);
        user.shares -= _shares;
        totalShares -= _shares;
        uint256 withdrawAmount = _amount + currentPerformanceFee;
        if (staking) {
            // withdrawByAmount have a MIN_WITHDRAW_AMOUNT limit ,so need to withdraw more than MIN_WITHDRAW_AMOUNT.
            withdrawAmount = withdrawAmount < MIN_WITHDRAW_AMOUNT
                ? MIN_WITHDRAW_AMOUNT
        ...
        uint256 currentWithdrawFee;
        if (block.timestamp < user.lastDepositedTime + withdrawFeePeriod) {
            currentWithdrawFee = (_amount * withdrawFee) / 10000;
            _amount -= currentWithdrawFee;
        }
        //Combine two fees to reduce gas
        uint256 totalFee = currentPerformanceFee + currentWithdrawFee;
        if (totalFee > 0) {
        ...
        _amount = available() >= _amount ? _amount : available();
        ...
        emit WithdrawShares(msg.sender, _amount, _shares);
    }
     ...
     * @dev Only callable by the contract admin.
     */
    function setWithdrawAmountBooster(uint256 _withdrawAmountBooster)
        external
        onlyAdmin
    {
        require(
    ...
    /**
     * @notice Withdraws from Bull Pool without caring about rewards.
     * @dev EMERGENCY ONLY. Only callable by the contract admin.
     */
    function emergencyWithdraw() external onlyAdmin {
        require(staking, "No staking bull");
        staking = false;
        bullPool.withdrawAll();
    }

    /**
     * @notice Withdraw unexpected tokens sent to the Bull Flexible Pool
     */
     ...
     * @dev It includes tokens held by the contract and held in BullPool
     */
    function balanceOf() public view returns (uint256) {
        (uint256 shares, , , , , , , , ) = bullPool.userInfo(address(this));
        uint256 pricePerFullShare = bullPool.getPricePerFullShare();
    ...
    /**
     * @notice Deposits tokens into BullPool to earn staking rewards
     */
    function _earn() internal {
        uint256 bal = available();
        if (bal > 0) {
            bullPool.deposit(bal, 0);
        }
    }
}