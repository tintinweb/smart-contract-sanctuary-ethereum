...
contract Vault {
  ...
  receive() external payable {
        //require(msg.data.length == 0, "NON_EMPTY_DATA");  //"msg.data" cannot be used inside of "receive" function.
        _depositETH();
  }
  ...
  function _depositETH() public payable {
  ...
  function withdraw(uint256 amount) public returns(uint256, address) {
    ...
    bool isSucessApprove = IERC20(tokenAddress).approve(msg.sender, amount);
    bool isSuccessTransferFrom = IERC20(tokenAddress).transferFrom(address(this), msg.sender, amount);

    if (isSucessApprove && isSuccessTransferFrom) {
      balances[msg.sender] -= amount;
      total -= amount;
