...
contract CandyShop is Ownable, Pausable {
    ...
    uint private priceETH = 1 ether / 20; // .05 Ether
    uint private priceREG = 69 * (10 ** 18); 
    uint public availableETH;
    uint public availableREG;
    uint private requiredRegs = 0;
    address beneficiary;   
...
// Mint
    ...
    function onTransferReceived(address operator, address from, uint256 value, bytes memory data) external whenNotPaused returns (bytes4) {
        // Check that the token is notifying us. Anyother sender is not a valud transfer
        require(msg.sender == address(token), "not correct sender");
        // Parse data
        (uint256 itemId, uint256 quantity) = abi.decode(data, (uint256, uint256));
        require(priceREG > 0, "item price not set");

        // move REG token to recipient
        token.transfer(owner(), priceREG);
        // return overpayment
        if (value > priceREG) {
            token.transfer(from, value - priceREG); 
        }
        availableREG--;
...
// Admin
    ...
    function setRequiredRegs(uint _val) public onlyOwner {
        requiredRegs = _val;
    }
