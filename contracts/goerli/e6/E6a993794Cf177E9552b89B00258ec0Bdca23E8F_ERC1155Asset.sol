...
contract ERC1155Asset is ERC1155AssetAutoProxy, IERC1155Asset {
    ...
    constructor(
        ...
        uint256 _mintFee
    ) ERC1155AssetAutoProxy(_proxy, address(this), _baseURI) {
    ...
    function implMint(
        ...
        _mint(caller, account, id, value, data);
    }
    ...
    function implMintBatch(
        ...
        _mintBatch(caller, account, ids, values, data);
    }
    ...
    function _mint(
        ...
        if (totalSupply == 0) {
            eRC1155AssetStorage.setCreator(id, account);
        }
    ...
    function _mintBatch(
        ...
        for (uint256 i = 0; i < ids.length; i++) {
            ...
            // the first owner is authorized to set royalties and to mint again
            if (totalSupply == 0) {
                eRC1155AssetStorage.setCreator(ids[i], account);
            }
            ...
            governedERC1155Storage.setTotalSupply(
                ids[i],
                governedERC1155Storage.getTotalSupply(ids[i]) + values[i]
            );
        }
...
contract ERC1155AssetAutoProxy is GovernedERC1155 {
    constructor(
        ...
        string memory _baseURI
    ) GovernedERC1155(_proxy, _baseURI) {
        proxy = _proxy;
