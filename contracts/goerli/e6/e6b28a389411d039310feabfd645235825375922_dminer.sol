...
contract dminer is Ownable, ReentrancyGuard {
    ...
    */
    ...
    mapping(uint256=>mapping(uint256=>Ticket2)) public TicketKey2;  
    ...
    mapping(uint256=>mapping(uint256=>uint256)) private claimCounterNew;
  ...
  bytes32 private constant _TRANSFER_EVENT_SIGNATURE =
    ...
    event TicketPurchased(uint256 indexed ticketId, uint256 indexed roundId, address indexed buyer, uint256 tokenIdholding);
    ...
    function buyTickets( bytes calldata signature,  uint256 quantity,  uint256 tokenIdHolding,  uint256 miningLevel) external payable nonReentrant {
        ...
        if(checkSign(signature, quantity, tokenIdHolding, miningLevel, _msgSender(), roundId) != signerWallet) revert FakeSignature();
        ...
        for(uint256 i= 0;i<quantity;){
            ...
            TicketKey[roundId][ticketId] = Ticket(buyer, tokenIdHolding, false, winningOre);
    ...
    function buyTicketsML( /* bytes calldata signature,  */ uint256 quantity,  uint256 tokenIdHolding,  uint256 multiplier) external payable nonReentrant {
      ...
      //  if(round.endTime < block.timestamp) revert LotteryEnded();
        ...
        for(uint256 i= 0;i<quantity;){
            ...
            uint256 ticketKey = getTicketKey(ticketId);
            Ores winningOre = getWinningOre(winningKey_, roundId, ticketKey, multiplier);
            TicketKey2[roundId][ticketId] = Ticket2(buyer, winningOre);
            emitPurchase(ticketId, roundId, buyer);
    ...
    function buyTicketsNON( bytes calldata signature,  uint256 quantity,  uint256 tokenIdHolding,  uint256 miningLevel) external payable nonReentrant {
        //require(msg.value == ticketPrice.mul(quantity), "DIGI: Send proper ticket fees");
        if(msg.value != ticketPrice.mul(quantity)) revert BadFees();
        ...
        for(uint256 i= 0;i<quantity;){
            ...
            TicketKey[roundId][ticketId] = Ticket(buyer, tokenIdHolding, false, winningOre);
            ticketHolding[buyer][roundId].push(ticketId);
            emit TicketPurchased(ticketId, roundId, buyer, tokenIdHolding);
            unchecked {
             ++claimCounter[roundId][winningOre];               
	         ++i;
	        }
    ...
    function buyTicketsNON2( bytes calldata signature,  uint256 quantity,  uint256 tokenIdHolding,  uint256 miningLevel) external payable nonReentrant {
        //require(msg.value == ticketPrice.mul(quantity), "DIGI: Send proper ticket fees");
        if(msg.value != ticketPrice.mul(quantity)) revert BadFees();
        ...
        for(uint256 i=0;i<data.length;++i){
            ...
            for(uint256 j=0;j<data[i].ticketIds.length;j++){
                if(TicketKey[roundId][data[i].ticketIds[i]].claimed == false){
                    _claimWinning(roundId, data[i].ticketIds[j]);
                }
            }
        }
    }
