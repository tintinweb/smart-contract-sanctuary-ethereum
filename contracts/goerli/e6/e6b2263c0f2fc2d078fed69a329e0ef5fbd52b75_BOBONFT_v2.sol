...
contract BOBONFT_v2 is ERC721A, ERC721AQueryable, Ownable {
    struct RoundData {
    ...
    constructor(address tokenAddress, address treasure) ERC721A("go", "go") {
    ...
    function batchClaimRewards(uint256[] memory tokenIds) public {
        require(tokenIds.length > 0 && tokenIds.length < 31, "not more than 30 nft per transaction");
    ...
    function claimAndDepositReward(uint256 tokenId) public {
    ...
    function claim(uint256 tokenId, address rewardRecipient) internal {
        ...
        (uint256 totalEthReward, uint256 totalTokenReward) = getUnclaimedAmount(tokenId);
        ...
        emit ClaimRewards(tokenId, totalEthClaimed, totalTokenReward, rewardRecipient);
    }
    ...
    onlyOwner
    {
        ...
        uint256 amountPerToken = amount / maxTokenId;
        roundData[roundId] = RoundData(amountPerToken, false, maxTokenId);
        roundId++;
    }
    ...
    onlyOwner
    {
        ...
        roundData[roundId] = RoundData(amountPerToken, true, maxTokenId);
        roundId++;
    }
    ...
    function setMaxTokenId(uint256 newMaxTokenId) external onlyOwner {
        require(newMaxTokenId > 0 && newMaxTokenId <= MAX_SUPPLY, "Out of collection bounds");
