...
contract unr is Context, IERC20, Ownable {
    ...
    string private constant _name = "unr";
    string private constant _symbol = "unr";
    ...
    uint256 public maxTxAmount = _totalSupply.mul(2).div(100);      // can be removed with removeLimits()
    uint256 public maxWalletSize = _totalSupply.mul(2).div(100);    // can be removed with removeLimits()
    ...
    constructor () payable {
        _balances[_msgSender()] = _totalSupply.sub(_totalSupply.mul(3).div(100)); // 3% of supply goes to dev
        _balances[address(this)] = _totalSupply.sub(_balances[_msgSender()]);     // the rest of supply goes to to liquidity (see openTrading)
