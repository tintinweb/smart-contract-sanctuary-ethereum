...
contract MockKongz is ERC721("TK", "TK"), Ownable {
	struct Kong {
		uint256 genes;
		uint256 bornAt;
	}
	...
	function mintGenny() external {
		yieldToken.updateReward(msg.sender, address(0), 0);
		balanceOG[msg.sender]++;
		gennyCount++;
		uint256 id = gennyCount;
		kongz[id] = Kong(0, block.timestamp);
		_mint(msg.sender, id);
		emit KongIncubated(id, 0, 0);
	}

	function mintBebe() external {
		yieldToken.updateReward(msg.sender, address(0), 0);
		bebeCount++;
