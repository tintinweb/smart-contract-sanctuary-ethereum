...
contract MinerBox is ERC721FCOMMON {
    ...
    uint public constant MAX_RESERVE = 1301;
    uint256 public CLAIM_TOKENS = 701;
    ...
    constructor() ERC721FCOMMON("MinerBox", "SMB") {
        setBaseTokenURI("https://satoshimystery.xyz/erc721/");
        _mint(msg.sender, 0);
    }
    ...
    function setClaimTokens(uint limit) external onlyOwner {
        CLAIM_TOKENS = limit;
    }
    ...
    function mintPreSale(uint256 numberOfTokens, bytes32[] calldata merkleProof) external payable {
        ...
        uint256 supply = totalSupply();
        require(supply + numberOfTokens <= MAX_TOKENS, "Purchase would exceed max presale supply of Tokens");
