...
contract NftStakingFactory is AccessControl {
 ...
 function addVault(
        ...
        stakingContract = new NftStaking(msg.sender,_nft,_token,_rewardPerDay,_lockTime);
 ...
 function removeVault(uint256 index) external onlyRole(DEFAULT_ADMIN_ROLE){
...
function updateNftMinLockTime(uint256 index,uint256 _locktime) external onlyRole(OPERATOR_ROLE){
    require(index < VaultInfo.length,"Pool does not exists");
    INftStaking nftStakingContract= INftStaking(address(VaultInfo[index].stakingContract));
    nftStakingContract.updateNftMinLockTime(_locktime);
}

function updateRewardsPerDay(uint256 index,uint256 _rewardsPerDay) external onlyRole(OPERATOR_ROLE){
    require(index < VaultInfo.length,"Pool does not exists");
    INftStaking nftStakingContract= INftStaking(address(VaultInfo[index].stakingContract));
    nftStakingContract.updateRewardsPerDay(_rewardsPerDay);
}

function addOperator(uint256 index,address _operator) external onlyRole(DEFAULT_ADMIN_ROLE){
    require(index < VaultInfo.length,"Pool does not exists");
    INftStaking nftStakingContract= INftStaking(address(VaultInfo[index].stakingContract));
    nftStakingContract.addOperator(_operator);
 }

function removeOperator(uint256 index,address _operator) external onlyRole(DEFAULT_ADMIN_ROLE){
    require(index < VaultInfo.length,"Pool does not exists");
    INftStaking nftStakingContract= INftStaking(address(VaultInfo[index].stakingContract));
    nftStakingContract.removeOperator(_operator);
 }

function pause(uint256 index,bool _isPause) external onlyRole(DEFAULT_ADMIN_ROLE){
   require(index < VaultInfo.length,"Pool does not exists");
    INftStaking nftStakingContract= INftStaking(address(VaultInfo[index].stakingContract));
    nftStakingContract.pause(_isPause);
 }
...
interface INftStaking {
 ...
 function updateNftMinLockTime(uint256 _locktime) external;
 function updateRewardsPerDay(uint256 _rewardsPerDay) external;
...
contract NftStaking is AccessControl, ERC721Holder, Pausable {
    struct poolInfo {
        IERC721  nft;
   ...
   struct Staker {
    ...
    constructor(address _operator,IERC721 _nft, IRewardToken _rewardToken, uint256 _rewardPerDay, uint256 _lockTime ) {
   ...
   */
  function updateNftMinLockTime(uint256 _locktime) external onlyRole(OPERATOR_ROLE) {
   ...
   * @param _locktime Locktime value.
   */
  function updateRewardsPerDay(uint256 _rewardsPerDay) external onlyRole(OPERATOR_ROLE) {
   ...
   function addRewards(uint256 _amount) external whenNotPaused {
     require(PoolInfo.rewardToken.transferFrom(msg.sender, address(this), _amount), "Transfer failed");
     availableRewards =availableRewards+_amount;
  ...
  function getRewardBalance() external view returns (uint256) {
        ...
        for (uint256 i = 0; i < ids.length; i++) {
            if (
              ...
              //  uint256 partialTime = ((block.timestamp - uint(staker.tokenStakingCoolDown[ids[i]]))) % PoolInfo.lockTime;  
                uint256 stakedDays = ((block.timestamp - uint(staker.tokenStakingCoolDown[ids[i]])));
                staker.balance +=  (PoolInfo.rewardsPerDay * stakedDays)/86400;
    ...
    function claimReward(address _user) public whenNotPaused{
        updateReward(msg.sender);
        require(availableRewards >= stakers[_user].balance , "Withdraw is paused. Not sufficient rewards avail.");
        ...
        availableRewards =availableRewards-stakers[_user].balance;
   ...
   */
  function pause(bool _isPause) external onlyRole(OPERATOR_ROLE) {
