...
contract OnChainAlpha is ERC721A, IERC721Receiver, Multicall, ReentrancyGuard, Ownable {
    ...
    address public ocaContract = 0x6A04d2443Be907DD7E83236398c985688acB796E; //testnet
    ...
    constructor() ERC721A("On-Chain Alpha", "OCA") {
        ...
        TIERS[8] = [25,40,80,110,140,170,200,230,260,290,320,350,380,410,440,470,500,530,570,610,650,725]; //eyes 4
        TIERS[9] = [35,65,100,130,160,190,220,250,280,310,340,370,400,450,500,650,700,750,850,1027];
        TIERS[10] = [100,200,250,300,350,400,450,500,550,600,650,700,750,850,1000,1450]; //mouth 5
    ...
    // function ocaClaim(uint256 _ocaTokenId) external nonReentrant whenMintActive returns (uint256) {
    //     uint256 totalMinted = _totalMinted();
    //     require(ocaClaims[_ocaTokenId] == false);
    //     require(msg.sender == ERC721(ocaContract).ownerOf(_ocaTokenId));
    //     require(totalMinted + 1 <= maxSupply);

    //     ocaClaims[_ocaTokenId] = true;
    //     _mint(msg.sender, 1);
    //     return totalMinted;
    // }

    function claimed(uint256 _tokenId, bool _isOcaClaim) public view returns (bool){
        if(_isOcaClaim){
