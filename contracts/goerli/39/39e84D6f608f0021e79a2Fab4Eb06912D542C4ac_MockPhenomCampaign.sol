...
contract MockPhenomCampaign is PhenomCampaign {
    ...
    function setVestingStartTime(uint256 _time) external{
...
//        vestingStartTime = _tgeTime;
    ...
    function setTgeTime(uint256 _tgeTime) external onlyOwner {
        require(vestingStartTime == 0, "TGE time already settled");
        require(_tgeTime > block.timestamp, "TGE should be in future");

        vestingStartTime = _tgeTime;

        for (uint i = 0; i < preTgePurchases.length; i++) {
            PreTgePurchase memory _purchase = preTgePurchases[i];
            registerPurchase(_purchase.phase, _purchase.buyer, _purchase.phetaAmount);
        }
    }
    ...
    function registerUsdPurchase(Phase _phase, address _investor, uint256 _usdAmount) public onlyOwner {
        ...
        registerPurchase(_phase, _investor, _tokenAmount);
    }
    ...
    function registerPurchase(Phase _phase, address _investor, uint256 _amount) public onlyOwner {
        require(_investor != address(0x0), "Invalid beneficiary address");
        ...
        if (vestingStartTime == 0) {
            ...
            else if (_phase == Phase.COMMUNITY_SALE_1 || _phase == Phase.COMMUNITY_SALE_2 || _phase == Phase.COMMUNITY_SALE_3) {
                // linear vesting without lock-up period
                _vestingController.createVestingSchedule(
                    _investor,
                    vestingStartTime,
    ...
    function registerMultiplyPurchases(Phase _phase, address[] calldata _investors, uint256[] calldata _amounts) external onlyOwner {
        ...
        for(uint i = 0; i < _investors.length; i++) {
            registerPurchase(_phase, _investors[i], _amounts[i]);
        }
    }
    ...
    function revoke(Phase _phase, bytes32 _vestingScheduleId) public onlyOwner {
