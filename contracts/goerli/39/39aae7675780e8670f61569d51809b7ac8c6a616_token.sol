...
contract token is ERC20("WBCTest", "WBCT"){
    ...
    function mintNewTokens(uint tokenNumber) public isOwner{

        _mint(msg.sender, tokenNumber);

    }

    //Actually this checkBalance function is not necessary because the former one already does the same.
    ...
    function burned(address whoBurned) public view returns (uint){

        return points[whoBurned];
