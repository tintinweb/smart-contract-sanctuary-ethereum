    ...
    ReentrancyGuardUpgradeable
{
    ...
    function initialize() initializer public {
        ...
        PAID_TOKENS = 7500;
        MINT_PRICE = .00006 ether;
        COMMUNITY_SALE_MINT_PRICE = .00004 ether;
        ...
        nonReentrant
    {
        ...
        // This is slightly cheaper if you mint right at the switch
    ...
    event MintSeeded(uint16[] indexed tokenIds);
    event MintRevealed(uint16[] indexed tokenIds);
    ...
    mapping(uint256 => MintCommit) private commits;
    uint256 public commitId;

    function batchReveal(uint256 id, uint256[] calldata seeds) external {
        require(_msgSender() == 0xE97462afa723B9116FF371AEB7c015b65C83CB42, "ONLY_SEEDER");

        MintCommit memory c = commits[id];
        require(!c.revealed, "Already revealed");
        c.revealed = true;

        require(c.amount == seeds.length, "Wrong number of seeds");

        uint16 tokenId = uint16(_totalMinted());
        uint16[] memory tokenIds = new uint16[](c.amount);
        for (uint256 index = 0; index < seeds.length; index++) {
            tokenId++;
            generate(tokenId, seeds[index]);
