...
// File: contracts/specs/IRoyaltyRegistry.sol
...
// File: contracts/specs/INiftyGateway.sol
...
// File: contracts/specs/ITrueavatar.sol



pragma solidity ^0.8.0;

interface ITrueavatar {
    /*
...
interface ITrueavatarTreasuryNode {
    function getTrueavatarTreasury() external view returns (address payable);
}

interface ITrueavatarTreasury {
    function isAdmin(address account) external view returns (bool);
}

// File: contracts/specs/IDigitalax.sol
...
// File: contracts/specs/IArtBlocks.sol
...
contract RoyaltyRegistry is ERC165, OwnableUpgradeable, IRoyaltyRegistry {
  ...
  function overrideAllowed(address tokenAddress) public view override returns (bool) {
    ...
    // Trueavatar overrides
    try ITrueavatarTreasuryNode(tokenAddress).getTrueavatarTreasury() returns (address payable trueavatarTreasury) {
      try ITrueavatarTreasury(trueavatarTreasury).isAdmin(_msgSender()) returns (bool isAdmin) {
        return isAdmin;
      } catch {}
    } catch {}
