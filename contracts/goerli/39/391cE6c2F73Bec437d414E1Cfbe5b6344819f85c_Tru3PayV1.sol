...
contract Tru3PayV1 is Ownable, ReentrancyGuard {
    ...
    event JoblessDistribution(address indexed _from, uint _value, address[] _payees, uint16[] _shares);
    ...
    function pay(string memory id) external payable nonReentrant isActive jobExists(id) jobOpen(id) {
        uint fee = _processFee(msg.value, jobs[id].options.feeIncluded);
    ...
    function _processFee(uint payableValue, bool feeIncluded) private returns(uint) {
        uint fee = 0;
        if(feeIncluded) { fee = (_fee * payableValue) / _shareDenominator; } 
        else { fee = (_fee * payableValue) / (_shareDenominator + _fee); }
    ...
    function payAndDistribute(address[] memory payees, uint16[] memory shares) payable external nonReentrant isActive {
        ...
        uint fee = _processFee(msg.value, true);
        ...
        emit JoblessDistribution(msg.sender, toPay, payees, shares);
    }
