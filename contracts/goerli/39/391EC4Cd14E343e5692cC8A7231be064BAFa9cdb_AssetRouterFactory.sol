...
contract AssetRouterFactory is OwnableUpgradeable, PausableUpgradeable {
    address payable immutable tokenImplementation;
    address feeTaker; // Enable the ability to charge for factory use.
    uint256 price; // Enable setting a price for the service.

    event WalletCreated(address indexed _from, address indexed _walletAddress, string _walletName);

    constructor()  {
        tokenImplementation = payable(address(new AssetRouter()));
        feeTaker = 0x052ab6bFa8aA30CC47ea02e648f90c3B24749B7b; // Initial fee taker address.
    ...
    function createWallet(address[] memory payees, uint256[] memory shares, string memory name) whenNotPaused payable external returns (address) {
        ...
        AssetRouter(clone).initialize(payees, shares, name);
        ...
        for(uint256 i = 0; i < payees.length; i++) {
            emit WalletCreated(payees[i], clone, name);
        }
...
contract AssetRouter is PaymentSplitterUpgradeable {
