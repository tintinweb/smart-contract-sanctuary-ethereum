...
contract Marketplace is ReentrancyGuard {
    ...
    function bid(
        ...
        require(
            !nftListingIds[tokenContract][tokenId].valueExists,//fix bug with listing after buying
        ...
        console.log("expectVal",expectedValue);
    ...
    function buyTokenInternal(
        ...
        require(isBuyable(listingId), "not buyable");
        //todo:выставляется на продажу токен,потом перекидывается на другой акк и кто получит деньги при покупке?
    ...
    function quoteForStaking(
        ...
        //check if nft is present on acc
        require(
            IERC721(tokenContract).ownerOf(tokenId) == msg.sender,
            "token ownership"
        );
    ...
    function isEnoughValueWasSend(uint messageValue, uint256 collateral, uint256 premium , uint256 _premiumFeePercentage)internal view returns(bool){ 
        console.log("valueToPay",collateral + premium + (premium * _premiumFeePercentage / 100));
    ...
    function rentNFTInternal(
        ...
        emit Rental(stakingId, msg.sender);
    ...
    function giveCashback(address maker, address taker,uint256 cashback ,bool isTokenFee)internal {
        if (isTokenFee)
        {
            console.log("giveCashb in Token");
            Platform(platform).addCashback(maker, taker, cashback, isTokenFee);
        }
        else
        {   
            console.log("here");
            //todo: коммисия за выставление на нфт на продажу идёт на аддрес платформы сразу,
            //для addCashback {value} не будет хватать баланса если 
    ...
    function bidAndStake(
        ...
        bid(tokenContract, tokenId, priceWei, isTokenFee);//emit for bid 
    ...
    function _takeFeeValueDistribution (bool isToken, uint256 feeValue) internal returns (uint256 etherFeeTaken, uint256 cashbackAmount) { 
        ...
        if (isToken) {
            ...
            cashbackAmount = (tokenFee * _cashbackPercent) / 100;//50*2
              console.log("cashbackAmount ",cashbackAmount);

            return (0, cashbackAmount);
        } else {
            cashbackAmount = (feeValue * _cashbackPercent) / 100;
            console.log("cashbackAmount !tokenfee",cashbackAmount);

            payable(platform).transfer(cashbackAmount);//we cant transfer feeValue because in case if we are calling
