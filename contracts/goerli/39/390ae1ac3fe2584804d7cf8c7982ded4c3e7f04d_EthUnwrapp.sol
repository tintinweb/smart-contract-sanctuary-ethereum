...
contract EthUnwrapp is ERC1155, ERC1155Holder {
    ...
    function _doMint(uint256 _editionId, address _mintForAddress) internal {
        ...
        // Because we create all to contract and transfer on mint we don't need 
        // to track if we sold them all since ERC1155: insufficient balance for transfer
        ...
        // Transfer from this smart contract to the user
        _safeTransferFrom(address(this), _mintForAddress, _editionId, 1, "");
    }

    function isMintingOpen(uint8 _edition) public view returns (bool isOpen) {
        return
            editions[_edition].openTimestamp >= block.timestamp &&
            editions[_edition].closeTimestamp < block.timestamp;
    }
    ...
    function createEdition(
        ...
        _mint(address(this), editionId, _maxMintLimit, "");
