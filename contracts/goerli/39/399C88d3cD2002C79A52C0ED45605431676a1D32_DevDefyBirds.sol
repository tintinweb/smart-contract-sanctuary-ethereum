...
library MoonbirdsLists {
    /**
     * @notice Moonbirds with `Outerwear == Hoodie Down`
     */
    function hoodieDownBirds() internal pure returns (uint16[295] memory) {
        return [
            0,
            49,
            54,
    ...
    /**
     * @notice Moonbirds with `Outerwear == Hoodie`
     */
    function hoodieBirds() internal pure returns (uint16[247] memory) {
        return [
            191,
            270,
            351,
    ...
    /**
     * @notice Moonbirds with `Headwear == Flower`
     */
    function flowerBirds() internal pure returns (uint16[105] memory) {
        return [
            41,
            182,
            222,
    ...
    /**
     * @notice Moonbirds with no special eye traits (anything rarer than
     * adorable eyes), no `Headwear` traits and no special `Body` (supplied by
     * Rachel)
     */
    function nudeBirds() internal pure returns (uint16[1498] memory) {
        return [
            5,
            18,
            21,
    ...
    function toUint256s(uint16[295] memory input)
        internal
        pure
    ...
    function toUint256s(uint16[247] memory input)
        internal
        pure
        returns (uint256[] memory output)
    {
    ...
    function toUint256s(uint16[105] memory input)
        internal
        pure
        returns (uint256[] memory output)
    {
    ...
    function toUint256s(uint16[1498] memory input)
        internal
        pure
        returns (uint256[] memory output)
    {
...
abstract contract NestedMerkleClaimableBase {
    ...
    /**
     * @notice Interface to claim the airdrop for a given moonbird.
     ...
     * opening timestamp.
     */
    function claimWithNestedMerkle(
        uint256 birbId,
        bytes32[] calldata merkleProof
    ) public onlyIfNestedMerkleClaimEnabled {
        _claimWithNestedMerkle(birbId, merkleProof);
    }

    /**
     * @notice A moonbird token with merkle proof.
     */
    struct MerkleBird {
        uint256 mbTokenId;
        bytes32[] merkleProof;
    ...
    /**
     * @notice Interface to claim the airdrop to multiple moonbirds.
     * @dev See also `_claimWithNestedMerkle`.
     */
        ...
        onlyIfNestedMerkleClaimEnabled
    {
        for (uint256 i; i < merkleBirds.length; ++i) {
            _claimWithNestedMerkle(
                merkleBirds[i].mbTokenId, merkleBirds[i].merkleProof
            );
        }
    }
...
import {MoonbirdsLists} from "constants/MoonbirdsLists.sol";
...
import {
    digest, ClaimableWithSignature
} from "../src/ClaimableWithSignature.sol";
...
contract Deploy is Script {
    ...
    address public defyBirdsConduit;
    ...
    function setUp() public {
        ...
        // TODO(Dave): set up gnosis for this.
        defyBirdsConduit = AddressBook.treasury();
    }

    function generateNudebirdsProofs() internal {
        string memory objKey = "nudeBirdsProofs";
        string memory json;
        uint256 fmp;
        assembly {
            fmp := mload(0x40)
        ...
        for (uint256 i; i < nudebirdIds.length; ++i) {
            ...
            string memory tokenId = vm.toString(nudebirdIds[i]);
            json = vm.serializeBytes32(objKey, tokenId, nudebirdsTree.proof(i));
        }
        vm.writeJson(
            json,
            string.concat(
    ...
    function run() public {
        ...
        if (address(token) == address(0)) {
            token = new DevDefyBirds{salt: bytes32(uint256(1))}(
                DefyBirds.Config({
                    admin: AddressBook.engineering(),
                    steerer: msg.sender,
                    royaltiesReceiver: payable(defyBirdsConduit),
                    baseTokenURI: "ipfs://QmPBU5AzVruVgfExUDop3oZ3FhLCKvWiXNy2ZevGwQRGdf/",
 ...
 */
function digest(ClaimableWithSignature.Allowance memory params, address token)
    view
    returns (bytes32)
{
    return SignatureChecker.generateMessage(
        abi.encodePacked(params.receiver, params.nonce, block.chainid, token)
    );
}
...
abstract contract ClaimableWithSignature {
    ...
    /**
     * @notice Thrown if a given allowance was already used.
     */
    error AllowanceAlreadyUsed(Allowance);
    ...
    /**
     * @notice Interface to claim the airdrop with a given signed allowance.
     * @dev The minted token will always end up on the receiver address
     * specified on the allowance.
     * @dev Reverts if the allowances are not correctly signed by an approved
     * signer.
     */
    function claimWithSignature(SignedAllowance calldata sig)
        external
        onlyIfClaimWithSignatureEnabled
    {
        _claimWithSignature(sig);
    }
     ...
     * signer.
     */
    function claimMultipleWithSignature(SignedAllowance[] calldata sigs)
        external
        onlyIfClaimWithSignatureEnabled
    {
        for (uint256 i; i < sigs.length; ++i) {
            _claimWithSignature(sigs[i]);
        }
    }
     ...
     * @notice Computes the hash of a given allowance.
     */
    function _digest(Allowance calldata params)
        internal
        view
        returns (bytes32)
    {
        return digest(params, address(this));
    }
     ...
     * signer.
     */
    function _claimWithSignature(SignedAllowance calldata signed) internal {
        bytes32 digest_ = _digest(signed.allowance);
        if (_usedDigests[digest_]) {
            revert AllowanceAlreadyUsed(signed.allowance);
        }
        _signers.requireValidSignature(digest_, signed.signature);
        _usedDigests[digest_] = true;

        _doClaimFromSignature(signed.allowance.receiver);
    }
     ...
     * signature.
     */
    function usedAllowance(Allowance calldata allowance)
        external
        view
        returns (bool)
    {
        return _usedDigests[_digest(allowance)];
    }
     ...
     */
    function _doClaimFromSignature(address receiver) internal virtual;
}
    ...
    ClaimableWithSignature
{
     ...
     */
    error ClaimsWithSignatureExhausted();
     ...
     */
    IEntropyOracle internal immutable _entropyOracle;
     ...
     */
    function _doClaimFromSignature(address to) internal virtual override {
        if (numClaimableWithSignature == 0) {
            revert ClaimsWithSignatureExhausted();
        }
     ...
     */
    function _doClaim(address to) internal {
        uint256 tokenId = _totalMinted();
        _mintRecorded(to, 1);
        _entropyOracle.requestEntropy(revealBlockNumber(tokenId));
    }

    /**
     * @notice The blocknumber at which a given token will be revealed.
     * @dev The entropy provided by `_entropyOracle` for this block will be used
     ...
     * @dev Uses block entropy provided by the `_entropyOracle`.
     */
    function _seed(uint256 tokenId) internal view override returns (uint256) {
        bytes32 salt = _entropyOracle.blockEntropy(revealBlockNumber(tokenId));
        if (salt == 0) {
            return 0;
        }
...
interface IEntropyOracleEventsAndErrors {
    /**
    ...
    /**
     * @notice The signature for the provided entropy was not provided by the
     * oracle.
     */
...
interface IEntropyOracle is IEntropyOracleEventsAndErrors {
    /**
     * @notice Equivalent to requestEntropy(block.number). This is safe as the
     * request will only be fulfilled once the
     * block is mined.
     */
    function requestEntropy() external;

    /**
     * @notice Signal to the oracle that entropy is requested for the specified
     * block. The request will only be
     * fulfilled once the block is mined.
     * @dev NOTE that this must be used with care. If a historical block is
     * requested, the entropy may be known by a bad
     * actor. It is only safe to request entropy for a historical block i.f.f.
     * said block was commited to before it was
     * mined.
     */
     ...
     * @dev Not all blocks will have entropy available; check that the returned
     * value is non-zero.
     */
...
library TraitSampling {
     ...
     */
    uint256 private constant _PROBABILITY_ONE = 1000;
    ...
    function sampleBody(uint128 seed) internal pure returns (uint8) {
        ...
        uint256[NUM_BODIES] memory cdf =
            [125, 250, 375, 500, 625, 750, 875, _PROBABILITY_ONE];
