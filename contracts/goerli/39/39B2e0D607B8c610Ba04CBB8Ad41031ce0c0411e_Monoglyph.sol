...
contract Monoglyph is ReentrancyGuard, ERC721, Ownable {
    ...
    uint public constant MAX_SUPPLY = 256;
    ...
    mapping(address => uint) public _allowListNumMinted;
    ...
    constructor(address parentAddress, uint _parentSupply) ERC721('Monoglyph', 'MNGL') {
    ...
    function claim(uint256 startingIndex, uint256 numberOfTokens) external nonReentrant callerIsUser{
        _claim(startingIndex, numberOfTokens);
    }

    function claimAll() external nonReentrant callerIsUser{
        _claim(0, parent.balanceOf(msg.sender));
    }

    function claimByTokenIds(uint256[] calldata _parentTokenIds) external nonReentrant callerIsUser{
        require(isClaimActive, 'Claim must be active to mint tokens');
    ...
    function mintAllowList(uint numberOfTokens, bytes32[] memory merkleProof) external payable nonReentrant callerIsUser{
        ...
        require(this.totalSupply() + numberOfTokens <= MAX_SUPPLY, "Purchase would exceed max tokens");
    ...
    function mint(uint256 numberOfTokens) external payable nonReentrant callerIsUser{
    ...
    function _mintToken(address _to, uint256 _tokenId) internal {
        bytes32 seed = keccak256(abi.encodePacked(_tokenId, provenanceHash, communityHash));
