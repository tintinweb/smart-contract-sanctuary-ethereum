...
contract MVCCOnChainMetadata is Ownable {
    ...
    function setUniquesDrawn(bool state, uint256 random) external {
        ...
        uint256 maxSupply = IMVCC(MVCC).maxSupply();
        uniquesDrawn = state;
        randomHash = random;

        uint256 first = ((randomHash % maxSupply) + 1);
        uint256 second = (uint256(
            keccak256(abi.encodePacked(randomHash, first))
        ) % maxSupply) + 1;

        while (second == first) {
            second =
                (uint256(keccak256(abi.encodePacked(second, first))) %
                    maxSupply) +
                1;
        }

        uint256 third = (uint256(
            keccak256(abi.encodePacked(randomHash, second))
        ) % maxSupply) + 1;
...
contract MVCC is ERC721A, Ownable, ReentrancyGuard, ERC2981, OperatorFilterer {
    ...
    function whitelistMint(
        ...
        if (whitelistMinted[msg.sender] + quantity > limit)
            revert Errors.MintLimitReached();
        if (quantity != skills.length) revert Errors.LengthMismatch();
        ...
        returns (string memory indexes)
    {
        ...
        indexes = renderer.getIndexes(tokenId, info.dna);
    }
...
interface IMVCCRenderer {
    ...
    function getIndexes(uint256 tokenId, uint256 dna)
        external
        view
        returns (string memory indexes);
