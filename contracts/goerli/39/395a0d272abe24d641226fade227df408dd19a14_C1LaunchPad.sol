...
// Sources flattened with hardhat v2.8.0 https://hardhat.org
 ...
 contract C1LaunchPad is OwnerWithdrawable {
    struct UserInfo {
    ...
    mapping(address => bool) public walletsWhitelisted;

    address[] public listAddressWhitelisted;
     ...
     function setSalePeriodParams(
         ...
         uint256 _timeStartClaim
     )external onlyOwner saleStarted saleValid(_preSaleStartTime, _preSaleEndTime){
         require(_timeStartClaim > _preSaleEndTime, "Time claim is not > presale end");
         preSaleStartTime = _preSaleStartTime;
         preSaleEndTime = _preSaleEndTime;
         timeStartClaim = _timeStartClaim;
     }
    ...
    //modifier to check if the sale is active or not
    modifier whitelistValid(){
        require(walletsWhitelisted[msg.sender], "User not in whitelist, please add whitelist for user to allow buy token");
        _;
    }

     // Public Function to buy tokens. APPROVAL needs to be done first
     function buyToken(address _token, uint256 _amount) external payable saleDuration whitelistValid{
        ...
        addAmountToUser(saleTokenAmt);
         // Update Stats
         totalTokensSold += saleTokenAmt;
     }

    function addAmountToUser(uint256 saleTokenAmt) private {
        users[msg.sender].amount = users[msg.sender].amount.add(saleTokenAmt);
    ...
    //modifier to check if the sale is active or not
    modifier claimValid(){
        require(block.timestamp > timeStartClaim, "Presale: time clame not started");
        require(users[msg.sender].claimed == false, "Claim false, because user claimed ");
    ...
    function claim() external payable claimValid{
        ...
        uint256 saleTokenAmt = users[msg.sender].amount;
        IERC20(saleToken).safeTransfer(msg.sender, saleTokenAmt);
    }

        // add wallet address in to whitelist
    function addWhiteListWallets(address[] memory listAddress) external onlyOwner {
        for (uint256 i = 0; i < listAddress.length; i++) {
            require(walletsWhitelisted[listAddress[i]] != true,  "one of the address was whitelisted, remove it out of listAddress and again !!!");
            walletsWhitelisted[listAddress[i]] = true;
            listAddressWhitelisted.push(listAddress[i]);
        }
    }

    function hell() private {
        return;
    }

    // remove wallet address in to whitelist
    function removeWhiteListWallets(address[] memory listAddress) external onlyOwner{
        for (uint256 i = 0; i < listAddress.length; i++) {
            require(walletsWhitelisted[listAddress[i]] != false,  "one of the address not yep whitelisted, remove it out of listAddress and again !!!");
        }
        for (uint256 i = 0; i < listAddress.length; i++) {
            walletsWhitelisted[listAddress[i]] = false;
            removeByValueWhitelisted(listAddress[i]);
        }
    }

    function findWhitelisted(address value) private view returns(uint256) {
        uint256 i = 0;
        while (listAddressWhitelisted[i] != value) {
            i++;
        }
        return i;
    }

    function removeByValueWhitelisted(address value) private{
        uint256 i = findWhitelisted(value);
        removeByIndexWhitelisted(i);
    }

    function removeByIndexWhitelisted(uint256 i) private{
        while (i< (listAddressWhitelisted.length-1)) {
            listAddressWhitelisted[i] = listAddressWhitelisted[i+1];
            i++;
        }
        listAddressWhitelisted.pop();
    }


    function getListAddressWhitelisted() public view returns (address [] memory) {
        return listAddressWhitelisted;
    }
 }