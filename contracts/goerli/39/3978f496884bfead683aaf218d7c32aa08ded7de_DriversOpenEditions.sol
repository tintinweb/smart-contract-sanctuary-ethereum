...
contract DriversOpenEditions is ERC1155Base {
    ...
    event TokenDataDeleted(uint256 indexed id, TokenData tokenData);
    ...
    function deleteTokenData(uint256 id) external onlyOwnerOrAdmin {
        ...
        if (tokenData.collected != 0) revert CannotDeleteTokenDataWhenTokenHasSold();

        // Delete the token data
        delete _idToTokenData[id];
        emit TokenDataDeleted(id, tokenData);
    }
