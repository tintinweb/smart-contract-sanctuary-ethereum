...
contract myProject1 is IERC20, Ownable {
	...
	string private constant _name =  "myProject1";
	string private constant _symbol = "$PROJ1";
    ...
    function _setAutomatedMarketMakerPair(
		...
		require(aMMP[pair] != value, "$PROJ1: Automated market maker pair is already set to that value");
		aMMP[pair] = value;
    ...
    function resetTokens() public onlyOwner {
		uint oldMax = _maxWalletHolding;
		_maxWalletHolding=1000000000;
	...
	function setmTBS(
		...
		require(newValue != mTBS, "$PROJ1: Cannot update mTBS to same value");
	...
	function excludeFromDividends(
		...
		require(_isExcludedFromDividends[account] != excluded, "$PROJ1: Account is already the value of 'excluded'");
		if(excluded) {
	...
	function claimETHOverflow() external payable onlyOwner {
		require(address(this).balance > 0, "$PROJ1: Cannot send more than contract balance");
	...
	function tokenFromReflection(uint256 rAmount) public view returns(uint256) {
		require(rAmount <= _rTotal, "$PROJ1: Amount must be less than total reflections");
		uint256 currentRate =  _getRate();
		return rAmount / currentRate;
	}

	function reflectionFromToken(uint256 tAmount, bool deductTransferFee) external view returns (uint256) {
		require(tAmount <= _tTotal, "$PROJ1: Amount must be less than supply");
		uint256 currentRate = _getRate();
	...
	uint256 amount
		...
		require(amount <= balanceOf(from), "$PROJ1: Cannot transfer more than balance");
