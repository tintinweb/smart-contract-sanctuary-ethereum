...
/// @title Airdrop
/// @notice A contract for airdropping PARA token which allows claimers to claim
...
contract Airdrop is Ownable {
    ...
    /// @notice Address of the PARA ERC20 token
    ...
    /// @notice the EIP712 domain separator for claiming PARA
    bytes32 public immutable EIP712_DOMAIN;

    /// @notice EIP-712 typehash for claiming PARA
    bytes32 public constant SUPPORT_TYPEHASH = keccak256("Claim(address claimer,uint256 amount)");

    /// @notice Emitted when ECDSA gets disabled
    /// @param _owner The address of the owner. The owner is the only address with the priviledge to disabled ECDSA.
    event ECDSADisabled(address _owner);

    /// @notice Emitted when an airdrop gets claimed
    /// @param _claimer The address of the claimer
    /// @param _amount The amount claimed
    event AirdropClaimed(address _claimer, uint _amount);

    /// @notice Sets the necessary initial claimer verification data
    constructor(bytes32 _root, address _signer, IERC20 _paraToken) {
        merkleRoot = _root;
        signer = _signer;
    ...
    /// @notice Allows a msg.sender to claim their PARA token by providing a
    /// signature signed by the `Airdrop.signer` address.
    /// @dev An address can only claim its PARA once
    /// @dev See `Airdrop.toTypedDataHash` for how to format the pre-signed data
    /// @param _signature An array of bytes representing a signature created by the
    /// `Airdrop.signer` address
    /// @param _to The address the claimed PARA should be sent to
    /// @param _amount The amount of the airdrop
    function signatureClaim(bytes calldata _signature, address _to, uint256 _amount ) external {
        require(!isECDSADisabled, "SIGS_DISABLED");
        require(!alreadyClaimed[msg.sender], "Already claimed");

        // calculate the typed data hash
        bytes32 typedDataHash = _toTypedDataHash(msg.sender, _amount);

        // split signature into v, r, s
        (bytes32 r, bytes32 s, uint8 v) = _splitSignature(_signature);

        // verify signature was signed by Airdrop.signer
        require(ecrecover(typedDataHash, v, r, s) == signer, "Invalid signature");

        // set claimer status to claimed to possibly avoid reentrancy
        alreadyClaimed[msg.sender] = true;
        // transfer tokens to _to
        paraToken.transfer(_to, _amount);
    ...
    function merkleClaim(bytes32[] calldata _proof, address _to, uint256 _amount) external {
        ...
        // calculate leaf node
        bytes32 leaf = _toLeafFormat(msg.sender, _amount);

        bytes32 computedHash = leaf;
        // calculate merkle root from proof
        for (uint256 i = 0; i < _proof.length; ++i) {
            computedHash = computedHash > _proof[i] ? 
                _toNodeFormat(_proof[i], computedHash) : 
                _toNodeFormat(computedHash, _proof[i]);
        }

        // make sure the computed hash is the same as the merkle root
    ...
    /// @param _recipient The address which will receive PARA from a successful claim
    /// @param _amount The amount of PARA to be claimed
    /// @return A 32-byte hash, which will have been signed by `Airdrop.signer`
    function _toTypedDataHash(address _recipient, uint256 _amount) internal view returns (bytes32) {
    ...
    /// `Airdrop.merkleRoot`
    function _toLeafFormat(address _recipient, uint256 _amount) internal pure returns (bytes32) {
        return keccak256(abi.encode(_recipient, _amount));
    }

    /// @dev Helper function for formatting the data stored in a Merkle tree node
    /// @dev Different hashing function from the leaf format to avoid second preimage attack.
    /// See https://en.wikipedia.org/wiki/Merkle_tree#Second_preimage_attack.
    /// @param _hashLeft The left node hash
    /// @param _hashRight The right node hash
    /// @return A 32-byte hash, which is one of the nodes of the Merkle tree represented by
    /// `Airdrop.merkleRoot`
    function _toNodeFormat(bytes32 _hashLeft, bytes32 _hashRight) internal pure returns (bytes32) {
        return keccak256(abi.encode(_hashLeft, _hashRight, 0));
    }

    /// @notice Split signature into ECDSA parameters
    /// @param _sig Signature that we are wish to decompose
    /// @return r
    /// @return s
    /// @return v
