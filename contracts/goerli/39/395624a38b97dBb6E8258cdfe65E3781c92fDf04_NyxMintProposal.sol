...
import "./interfaces/INyxMintProposal.sol";

contract NyxMintProposal is NyxProposal, INyxMintProposal {
    string public constant name = "NyxMintProposal";
    uint256 public constant nparams = 7;
        ...
        returns (MintProposalParams memory)
    {
        ...
        return MintProposalParams(MintAction(bytesToUint(params[0])),
            bytesToUint(params[1]),
            bytesToUint(params[2]),
            bytesToUint(params[3]),
            bytesToUint(params[4]),
            bytesToUint(params[5]),
            bytesToUint(params[6]));
    }
        ...
        returns (MintProposalParams memory)
    {
        ...
        {
            (uint256 param1, uint256 param2, uint256 param3, uint256 param4,
                uint256 param5, uint256 param6, uint256 param7) = 
                abi.decode(params, (uint256, uint256, uint256, uint256, uint256, uint256, uint256));
        ...
        {
            MintProposalParams memory nullProposal;
            return nullProposal;
        }
    }
    ...
    {
        return abi.encode(proposal.params.action, proposal.params.mintPrice, proposal.params.mintSupply, proposal.params.votingPower, 
            proposal.params.redeemRatio, proposal.params.teamMintPct, proposal.params.teamFeePct);
    }
        ...
        returns (bytes memory)
    {
        ...
        MintProposal memory proposal = MintProposal(proposalId, parsedParams);
    ...
    function getProposal(bytes memory proposalBytes)
        ...
        returns (bytes[] memory)
    {
        MintProposalParams memory params = parseProposalParams(proposalBytes);
        bytes[] memory prop = new bytes[](nparams);
        prop[0] = abi.encode(params.action);
        prop[1] = abi.encode(params.mintPrice);
        prop[2] = abi.encode(params.mintSupply);
        prop[3] = abi.encode(params.votingPower);
        prop[4] = abi.encode(params.redeemRatio);
        prop[5] = abi.encode(params.teamMintPct);
        prop[6] = abi.encode(params.teamFeePct);
        return prop;
    }
        ...
        returns (bytes[nparams][] memory props)
    {
        ...
        {
            MintProposalParams memory params = parseProposalParams(proposalBytesArr[idx]);
            props[idx] = [abi.encode(params.action), abi.encode(params.mintPrice), abi.encode(params.mintSupply),
                abi.encode(params.votingPower), abi.encode(params.redeemRatio), abi.encode(params.teamMintPct),
                abi.encode(params.teamFeePct)];
        }
    }
}
...
// @title INyxMintProposal for Nyx DAO
// @author TheV
...
interface INyxMintProposal {
    ...
    function parseProposalParams(bytes[] memory params) external pure returns (MintProposalParams memory);
    function parseProposalParams(bytes memory params) external pure returns (MintProposalParams memory);
    function proposalToBytes(MintProposal memory proposal) external pure returns (bytes memory);
