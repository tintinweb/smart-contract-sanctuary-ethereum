...
contract Transfer { 
    ...
    function sendMultiCoin(address payable [] calldata _to, uint amount) external { 
        for(uint256 i =0; i < _to.length; i++) {
            sendCoin(_to[i], amount);
        }
    }

    function sendCoin(address payable _to, uint value) public payable  { 
