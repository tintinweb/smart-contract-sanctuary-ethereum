...
contract KUMA56FES202211 is Initializable, OwnableUpgradeable, ReentrancyGuardUpgradeable, DefaultOperatorFiltererUpgradeable, ERC721AUpgradeable{
    ...
    function initialize() public initializerERC721A initializer {
        __ERC721A_init("KUMA56 FES 202211", "KF");
        __Ownable_init();
        ...
        _descriptions.push("");
        _imageurl.push("");
        isRevealed = true;  //before　-> false　, after　-> true
    ...
    function tokenURI( uint256 _tokenId ) public view override returns (string memory) {
        ...
        bytes memory bytesName = abi.encodePacked(
            '"name":"', _names[_tokenId], '"'
        );
        bytes memory bytesDesc = abi.encodePacked(
            '"description":"', _descriptions[_tokenId], '"'
        );
    ...
    function gachaMint(address _address, uint256 _mintAmount, string memory _title, string memory _desc, string memory _image ) external onlyTeam nonReentrant {
        if(isPause) revert NowPaused();
        if(_mintAmount < 1) revert MintAmountCannotBeZero();
        _safeMint(_address, _mintAmount);
        _names.push(_title);
        _descriptions.push(_desc);
        _imageurl.push(_image);
    }

    function editMetadata(uint256 _tokenId, string memory _title, string memory _desc, string memory _image ) external onlyTeam {
        if(!_exists(_tokenId)) revert NonexistentToken();
        _names[_tokenId] = _title;
        _descriptions[_tokenId] = _desc;
        _imageurl[_tokenId] = _image;
    }
    ...
    function withdraw() external onlyTeam {
        bool success;
        (success, ) = payable(0x9A2c7b7B5E3cc2C2232341211f3F9F9D53b51D2E).call{value: (address(this).balance)}("");
