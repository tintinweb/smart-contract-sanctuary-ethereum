...
//Error(s)
error Raffle__notEnoughFeePaid();
error Raffle__paymentToWinnerFailed();
error Raffle__raffleStateNotOpen(uint256 currentRaffleState);
error Raffle__upkeepNotNeeded(uint256 numPlayers, uint256 currentBalance, uint256 currentRaffleState);
...
contract Raffle is VRFConsumerBaseV2, KeeperCompatibleInterface {
    ...
    // Mutational Function(s)
    function raffleEnter() public payable {
        if (s_raffleState != RaffleState.OPEN) {
            revert Raffle__raffleStateNotOpen(uint256(s_raffleState));
        }

        if (msg.value < i_entranceFee) {
            revert Raffle__notEnoughFeePaid();
        }
