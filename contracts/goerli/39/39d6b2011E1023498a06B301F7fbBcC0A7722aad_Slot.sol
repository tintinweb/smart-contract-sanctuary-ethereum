...
contract Slot is VRFConsumerBaseV2, Ownable, KeeperCompatibleInterface {
    ...
    uint256 private constant LUCKY_STOP_DIGIT = 4;
    ...
    mapping(address => uint256[]) private s_playerLuckyWinsArray;
    ...
    {
        // 1) stored random words less than the refresh value
        // 2) vrf is not already requesting
        // 3) subscription funded with link
        bool needsWords = s_storedRandomWords.length <= i_refreshStoredWords;
        bool hasBalance = address(this).balance > 0;
        upkeepNeeded = (needsWords && hasBalance && !s_vrfRequesting);
        // if (needsWords && (!s_vrfRequesting)) {
        //     requestMoreSpins();
        // }
    }
    ...
    function buySpins() public payable {
        address payable playerAddress = payable(msg.sender);
        uint256 weiPassed = msg.value;
        uint256 contractBalance = address(this).balance;
        // check for reverts
        if (weiPassed < BUY_SPINS_WEI) {
            revert Slot__NotEnoughEthEntered(playerAddress, weiPassed, ENTRANCE_FEE_WEI);
        }
        if (contractBalance < PAYOUT_CUSHION) {
            revert Slot__MachineEmpty(contractBalance, PAYOUT_WEI);
        }
        if (s_storedRandomWords.length < 50) {
        ...
        uint256[] memory playerLuckyWins = new uint256[](SPINS_PER_BUY);
        uint256[] memory playerWinningStops = new uint256[](SPINS_PER_BUY);
        uint256 balanceChg = 0;
        uint256 startingStoredRandomWordsLength = s_storedRandomWords.length;

        // "spin" the slot machine

        for (uint256 i = 0; i < (NUM_REELS * SPINS_PER_BUY); ) {
            // console.log(i);
            // gets n random words (number of slot reels) from the previously saved VRF random words and removes them from the array.
            slotDigits[i] =
                s_storedRandomWords[startingStoredRandomWordsLength - 1 - i] %
                NUM_STOPS;
        ...
        s_playerSpinsFull[playerAddress] = slotDigits;

        for (uint16 e = 0; e < SPINS_PER_BUY; ) {
            ...
            for (uint16 i = 0; i < NUM_REELS; ) {
                ...
                if (spinStreak == 0) {
                    ...
                    spinStreakStop = slotDigits[slotDigitIdx]; // so e.g. if this is the 3rd spin per buy (2nd element), we've already gone through 10 stops. so 0th element of this should be the 10th element of the array
                } else {
                    if (spinStreakStop == slotDigits[slotDigitIdx]) {
                        unchecked {
                            spinStreak += 1;
                        }
                    } else {
                        spinStreak = 1;
                        spinStreakStop = slotDigits[slotDigitIdx];
                    }
                }
                unchecked {
                    i += 1;
            ...
            uint256 spinStreakLuckyTemp = 0;
            for (uint16 i = 0; i < NUM_REELS; ) {
                uint256 slotDigitIdx;
                unchecked {
                    slotDigitIdx = i + (e * NUM_REELS);
                }
                if (slotDigits[slotDigitIdx] == LUCKY_STOP_DIGIT) {
                    unchecked {
                        spinStreakLuckyTemp += 1;
                    }
                } else {
                    spinStreakLucky = spinStreakLuckyTemp > spinStreakLucky
                        ? spinStreakLuckyTemp
                        : spinStreakLucky;
                    spinStreakLuckyTemp = 0;
                }
                unchecked {
                    i += 1;
                }
            ...
            // check for winner
            bool isWinner = false;
            uint256 isLuckyWinner = 0;
            uint16 stopsToWinThree;
            uint16 stopsToWinFour;
            uint16 stopsToWinFive;

            unchecked {
                stopsToWinThree = STOPS_TO_WIN + 1;
                stopsToWinFour = STOPS_TO_WIN + 2;
                stopsToWinFive = STOPS_TO_WIN + 3;
            }
            ...
            if (spinStreakLucky >= STOPS_TO_WIN) {
                if (spinStreakLucky == STOPS_TO_WIN) {
                    isLuckyWinner = 2;
                    unchecked {
                        balanceChg += LUCKY_PAYOUT_WEI;
                    }
                } else if (spinStreakLucky == stopsToWinThree) {
                    isLuckyWinner = 3;
                    unchecked {
                        balanceChg += LUCKY_PAYOUT_WEI * 10;
                    }
                } else if (spinStreakLucky == stopsToWinFour) {
                    isLuckyWinner = 4;
                    unchecked {
                        balanceChg += LUCKY_PAYOUT_WEI * 10**2;
                    }
                } else if (spinStreakLucky == stopsToWinFive) {
                    isLuckyWinner = 5;
                    unchecked {
                        balanceChg += LUCKY_PAYOUT_WEI * 10**3;
                    }
                }

                spinStreakStop = LUCKY_STOP_DIGIT;

                // s_playerWins[msg.sender] += 1;
            } else if (spinStreak >= STOPS_TO_WIN) {
                isWinner = true;
                unchecked {
                    balanceChg += PAYOUT_WEI;
                }
                // s_playerWins[msg.sender] += 1;
            }
        ...
        s_playerBalance[msg.sender] += balanceChg;
    ...
    function getPlayerLuckyWinsArr() public view returns (uint256[] memory) {
