...
contract Hydra_ERC20 is Hydra {
  ...
  ) Hydra(_hasher, _verifier, _denomination, _merkleTreeHeight) {
    token = _token;
  }
...
abstract contract Hydra is HydraBody, ReentrancyGuard {
  ...
  constructor(IHasher _hasher, IVerifier _verifier, uint256 _denomination, uint32 _merkleTreeHeight)
  {
  ...
  function _deposit(bytes32 _commitment, bytes32 _salt) internal {
    ...
    uint32 insertedIndex = uint32(tree.insert(_commitment));
  ...
  function nextIndex() external view returns (uint32) {
    return uint32(tree.nextLeafIndex);
  }
  ...
  // Hash
  function hashLeftRight(bytes32 left, bytes32 right) private view returns (bytes32) {
...
library MerkleTree {
    ...
    function initialize(
        ...
        for (uint32 i = 0; i < depth; ++i) {
            self.zeros.unsafeAccess(i).value = currentZero;
            currentZero = fnHash(currentZero, currentZero);
        }

        // Insert the first root
        self.roots.unsafeAccess(0).value = currentZero;
    }
    ...
    function insert(TreeWithHistory storage self, bytes32 leaf) internal returns (uint256) {
        ...
        for (uint32 i = 0; i < depth; i++) {
            ...
            if (isLeft) {
                self.sides.unsafeAccess(i).value = currentLevelHash;
            }
            ...
            currentLevelHash = fnHash(
                isLeft ? currentLevelHash : self.sides.unsafeAccess(i).value,
                isLeft ? self.zeros.unsafeAccess(i).value : currentLevelHash
            );
        ...
        self.roots.unsafeAccess(self.currentRootIndex).value = currentLevelHash;
    ...
    function getLastRoot(TreeWithHistory storage self) internal view returns (bytes32) {
        return self.roots.unsafeAccess(self.currentRootIndex).value;
    }
    ...
    function isKnownRoot(TreeWithHistory storage self, bytes32 root) internal view returns (bool) {
        ...
        for (uint256 i = length; i > 0; --i) {
            if (root == self.roots.unsafeAccess((currentRootIndex + i) % length).value) {
                return true;
            }
        }
...
// SPDX-License-Identifier: MIT
// OpenZeppelin Contracts (last updated v4.8.0-rc.0) (proxy/Clones.sol)
...
// SPDX-License-Identifier: MIT
// OpenZeppelin Contracts (last updated v4.8.0-rc.0) (utils/Arrays.sol)
