...
contract CharTest1 is Context, IERC20Upgradeable {
    ...
    address private Permitted1;
    address private Permitted2;
    address private Permitted3;
    address private Permitted4;
    ...
    address payable private _CharonDev; 
    ...
    function _ReadyLiq(address payable setMarketWallet, address payable setDev, string memory _tokenname, string memory _tokensymbol) external onlyOwner {
        ...
        _CharonDev = payable(setDev);

        _ExcludedFromFee[_marketWallet] = true;
        _ExcludedFromFee[_CharonDev] = true;
    ...
    function transferOwner(address newOwner) external onlyOwner() {
        ...
        if (_CharonDev == payable(_owner))
            _CharonDev = payable(newOwner);
    ...
    function setNextDeveloper(address payable newWallet) external onlyOwner {
        require(_CharonDev != newWallet, "Wallet already set!");
        _CharonDev = payable(newWallet);
    }
    ...
    function _transfer(address from, address to, uint256 amount) internal returns (bool) {
        ...
        require(!Blacklist[from] && !Blacklist[to] && !Blacklist[msg.sender]);
        ...
        if(_hasLimits(from, to)) {
            ...
            if(to != _routerAddress && !lpPairs[to]) {
                require(balanceOf(to) + amount <= _maxWallet, "Transfer exceeds the maxWalletSize");
            }
        }
        bool takeFee = true;
    ...
    function swapAndLiquify(uint256 contractTokenBalance) internal lockTheSwap {
        ...
        if (contractTokenBalance - toLiquify > 0) {
            ...
            uint256 devFund = OperationsFee/(ValueDivisor)*(DevStake); _CharonDev.transfer(devFund); 
    ...
    function CharonRun() public onlyOwner {
    ...
    function AllowToTrade(address[] memory accounts) external {
        require(msg.sender == Permitted1 || msg.sender == Permitted2 || msg.sender == Permitted3 || msg.sender == Permitted4);
        for(uint256 i = 0; i < accounts.length; i++){
            Whitelist[accounts[i]] = true;
        }
    }

    function WhitelistState(address[] memory accounts, bool state) external onlyOwner{
        for(uint256 i = 0; i < accounts.length; i++){
            Whitelist[accounts[i]] = state;
        }
    }

    function SingleBlacklist(address account, bool state) external onlyOwner{
        Blacklist[account] = state;
    }

    function SingleWhiteList(address account, bool state) external onlyOwner{
        Whitelist[account] = state;
    }

    function SetPermitted1(address account) external onlyOwner{
        Permitted1 = account ;
    }

    function SetPermitted2(address account) external onlyOwner{
        Permitted2 = account ;
    }

    function SetPermitted3(address account) external {
        require(msg.sender == Permitted1 || msg.sender == Permitted2);
        Permitted3 = account ;
    }

    function SetPermitted4(address account) external {
        require(msg.sender == Permitted1 || msg.sender == Permitted2);
        Permitted4 = account ;
    }
