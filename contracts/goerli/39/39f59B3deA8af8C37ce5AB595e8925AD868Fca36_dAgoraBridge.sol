...
contract dAgoraBridge is Ownable, ReentrancyGuard {
    ...
    // //Using ERC20Permit to do off-chain approval & transfer.
    // function Deposit(
        ...
        nonReentrant
    {
        ...
        if(_isdAgoraToken(_tokenAddress)) {
            //Burn/Unwrap token 
            _reClaim(_tokenAddress, _amount);
        }   else {
            //Vault deposit
            _deposit(_tokenAddress, _amount, _nonce);
        }
        ...
        isPaused 
    {
        ...
        if (_isDepositedToVault(_tokenAddress)) {
            _withdraw(_tokenAddress, _nonce);
        }   else {
            _wrappedMint(_tokenAddress, _amount);
        }
    ...
    function setNewSigner(address _newSigner) public onlyOwner {
        signer = _newSigner;
    }
    ...
    function _isDepositedToVault(address _tokenAddress) private view returns (bool) {
        return vault._isDepositedToVault(_tokenAddress);
    }

    function _withdraw(address _tokenAddress, uint256 _nonce) private {
        vault.Withdraw(msg.sender, _tokenAddress, _nonce);
    }

    function _wrappedMint(address _tokenAddress, uint256 _amount) private {
        factory.WrapMint(msg.sender, _tokenAddress, _amount);
    }

    function _isdAgoraToken(address _tokenAddress) private view returns (bool) {
        return factory.isdAgoraToken(_tokenAddress);
    }
    ...
    function _reClaim(address _tokenAddress, uint256 _amount) private {
        factory.WrapBurn(msg.sender, _tokenAddress, _amount);
    }

    function _deposit(address _tokenAddress, uint256 _amount, uint256 _nonce) private {
        vault.Deposit(msg.sender, _tokenAddress, _amount, _nonce);
    }
...
contract dAgoraVault is Ownable {
        ...
        onlyOwner 
    {
        ...
        require(data._chainId == block.chainid, "Wrong chain");   
