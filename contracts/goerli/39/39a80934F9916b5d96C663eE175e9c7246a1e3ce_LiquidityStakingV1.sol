  ...
  LS1EpochSchedule
{
  ...
  /**
   * @notice Get the reward of staker. Formula: rewards = _user_rewards_balances_ + userBalance * (settledGlobalIndex - _user_index_) / INDEX_BASE;
   *
    ...
    returns (uint256)
  {
    ...
    if (didRolloverOccur) {
      ...
      if (oldUserIndex != globalIndexBeforeRolloverEpoch && uint256(balance.currentEpochBalance) != 0) {
    ...
    if(MathUpgradeable.min(block.timestamp, DISTRIBUTION_END) > _global_index_timestamp_) {
      ...
      if (_REWARDS_PER_SECOND_ != 0 && totalBalance != 0) {
        indexDelta = timeDelta.mul(_REWARDS_PER_SECOND_).mul(10**18).div(totalBalance);
      }
