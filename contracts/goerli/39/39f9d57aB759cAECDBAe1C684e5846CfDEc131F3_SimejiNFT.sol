...
contract SimejiNFT is ERC721Common, SimejiSeller {
        ...
        )
    {
        operateConfig = OperateConfig({
            whiteListSaleBegin: 1,
            whiteListSaleEnd: 1663471507,
            merkleRoot: 0x11e8cbed00672be3dc4acb4a51f3a2aeef859f838c60b4fb716ec81579292e5c,
            publicSaleBegin: 1
        });
    }
...
abstract contract SimejiSeller is OwnerPausable, ReentrancyGuard {
    ...
    function setSellerConfig(SellerConfig memory config) public onlyOwner {
        require(
            config.totalInventory >= (config.airdropQuota + config.reserveQuota),
            "SimejiSeller: excessive public sale quota"
        );

        require(
            config.totalInventory >= _totalSold.current(),
            "SimejiSeller: inventory < already sold"
        );

        require(
            (config.airdropQuota + config.reserveQuota) >= (airdrop.current() + reserve.current()),
            "SimejiSeller: free quota < already used"
        );
        ...
        if (sellerConfig.lockFreeQuota) {
            ...
            config.airdropQuota = sellerConfig.airdropQuota;
            config.reserveQuota = sellerConfig.reserveQuota;
        }
        sellerConfig = config;
    }
}
