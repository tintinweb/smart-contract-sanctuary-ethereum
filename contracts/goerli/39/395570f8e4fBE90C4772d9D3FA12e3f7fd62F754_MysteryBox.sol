...
interface IForcedOffline {
    ...
    function mint(address to, string memory name_, string memory description_,
        uint background_, uint talent_, uint weapon_, uint hair_, uint eye_) external;
    function setBackground(uint tokenId_, uint background_) external;
...
ERC721HolderUpgradeable
{
    ...
    bool isMintingStarted;
    bool isRevealingStarted;
    bool isWhiteListRevealingOnly;
    ...
    function initialize(IForcedOffline nftToken_, IRandomGenerator randomGenerator_, uint totalQuota_)
        ...
        whiteListOnly = true;
//        isWhiteListMintingOnly = true;
        isRevealingStarted = false;
        isWhiteListRevealingOnly = true;
    }
    ...
    function mintMulti(bytes32[] calldata merkleProof, uint amount) whenNotPaused onlyEOA external {
        require(isMintingStarted, "Mintting has not started yet");
...
//        string memory image = generateSVGImage();
        return string(
            abi.encodePacked(
                ...
                Base64.encode(
                    bytes(
                        abi.encodePacked('{"image": "',  imageUrl, '"}')
                    )
                )
            )
        );
    ...
    function startWhiteListMinting() external {
        isMintingStarted = true;
        whiteListOnly = true;
    }

    function startPublicMinting() external {
        isMintingStarted = true;
        whiteListOnly = false;
    }

    function startWhiteListRevealing() external {
        isRevealingStarted = true;
        isWhiteListRevealingOnly = true;
    }

    function startPublicRevealing() external {
        isRevealingStarted = true;
        isWhiteListRevealingOnly = false;
    }

    function endAll() external {
        isMintingStarted = false;
        isRevealingStarted = false;
    }

    function setImageUrl(string calldata imageUrl_) external onlyRole(DEFAULT_ADMIN_ROLE) {
        imageUrl = imageUrl;
    }
