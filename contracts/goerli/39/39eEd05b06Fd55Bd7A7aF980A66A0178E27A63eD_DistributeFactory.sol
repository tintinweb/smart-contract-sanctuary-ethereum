...
contract DistributeFactory is Ownable {
    ...
    function updateMerkleRoot(address _groupContractAddress, bytes32 _merkleRoot)
        external
        onlyDeployer
    {
        BaseGroup(_groupContractAddress).updateMerkleRoot(_merkleRoot);
    }
...
contract CustomGroup is BaseGroup {
    ...
    function claimableAmount(address receiverAddress, uint256 amount, uint256[] calldata dates, uint256[] calldata percentages) external view returns(uint256 lastPaidDate, uint256 claimable) {
        uint256 currentTime = block.timestamp;
