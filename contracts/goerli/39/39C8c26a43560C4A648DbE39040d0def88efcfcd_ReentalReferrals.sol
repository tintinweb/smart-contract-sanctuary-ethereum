...
contract ReentalReferrals is UUPSUpgradeableByRole, RefundableUpgradeable {
  ...
  event Distributed(address account, address to, uint256 amount);

  function initialize () public initializer {
    __AccessControlProxyPausable_init(msg.sender);
  }

  function payReferrals(bytes32 merkleRoot_, string memory uri_) public onlyRole(keccak256("REFERRALS_PAYER_ROLE")){
  ...
  function _distribute(uint256 index, address account, uint256 amount, bytes32[] calldata merkleProof, address to, uint256 distribution) internal whenNotPaused {
    ...
    require(distributable > 0, "ReentalReferrals: nothing to distribute");
    require(distribution <= distributable, "ReentalReferrals: amount to distribute exceeds referrals balance claimable");

    IERC20 dividendsInterface = IERC20(IReentalManager(config).get(keccak256("RNT")));
    ...
    emit Distributed(account, to, distribution);
  }
  ...
  function claim(
    ...
    require(account == msg.sender || isAuth[account] == msg.sender, "ReentalReferrals: claimer not authorized");
