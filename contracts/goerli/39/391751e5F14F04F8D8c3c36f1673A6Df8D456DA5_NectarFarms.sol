    ...
    ResourceImplementationPointer
{
    ...
    function mint(uint256 _count) external nonReentrant whenNotPaused resourceSet {
        address sender = _msgSender();

        uint256 supply = totalSupply();

        for(uint256 i; i < _count; i++){
            _mint(sender, supply + i );
        }

        resource.accountBurn(sender, mintPrice * _count);
        resource.accountReward(treasury, mintPrice * _count);
    }
