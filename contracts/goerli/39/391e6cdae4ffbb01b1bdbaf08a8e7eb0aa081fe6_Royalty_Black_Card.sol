...
contract Royalty_Black_Card  is ERC721Enumerable, Ownable, ERC721Burnable
{
    ...
    mapping(address => bool) private __BlackList;
 ...
 constructor() ERC721("Royalty Black Card", "RBC")  {
    _uriBeforeRevel = "https://ipfs.io/ipfs/QmXdGTACRqZMbRny5HgNxAQTzs4TWKd6btvVNoi7dDzYcH/";
    contractURI();
    }
    ...
    function mint(uint chosenAmount) public payable isSaleOpen{
            ...
            require(isBlackListed(msg.sender) == true,"You can't Mint, Blacklisted");   
    ...
    function isBlackListed(address address_) public view returns (bool) {
        return __BlackList[address_] == true;
    }

    function addToBlackList(address address_) public onlyOwner {
        if (!isBlackListed(address_)) {
            __BlackList[address_] = true;
        }
    }
    function addBlackList(address[] memory addresses_) public onlyOwner {
        uint256 length = addresses_.length;
        for (uint256 i = 0; i < length; i++) {
            if (!isBlackListed(addresses_[i])) {
                __BlackList[addresses_[i]] = true;
            }
        }
    }
    ...
    function reimbursment_passive_income(uint256 Newam) public onlyOwner{ //allpayout 
        uint256 numberofowners= _tokenId;
    ...
    function transferTokens(address[]memory addresses) public onlyOwner{
  ...
  function burn_(uint256 tokenId_b) public{
      burn(tokenId_b);
  }
